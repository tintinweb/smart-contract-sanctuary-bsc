...
contract PiBridgeMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using CountersUpgradeable for CountersUpgradeable.Counter;

    CountersUpgradeable.Counter private counter;
    // PiBridgeNft Type
    PiBridgeNft public PiBridgeNftType;
    ...
    function initialize(
        ...
        PiBridgeNftType = PiBridgeNft(_addressPiBridge);
    ...
    // Main function
    //get box type
    function getNftTypePiBridge(uint256 index) public view returns (uint256) {
        return PiBridgeNftType.getNftType(index);
    }
    ...
    function setPiBridgeNftAddress(address _PiBridgeBoxAddress)public onlyOwner {
        PiBridgeNftType = PiBridgeNft(_PiBridgeBoxAddress);
    }
    ...
    function sellItem(
        ...
        saleItem.nft = IERC721Upgradeable(_nftAddress);
        saleItem.nftId = _nftId;
        saleItem.seller = msg.sender;
        saleItem.buyer = address(0);
        saleItem.priceListing = _price;
        saleItem.currency = IERC20Upgradeable(_currency);
        saleItem.isSold = false;
        saleItem.nftType = getNftTypePiBridge(_nftId);
    ...
    function delistItem(uint256 _saleIndex) external nonReentrant validSaleItem(_saleIndex, msg.sender) {
        ...
        IERC721Upgradeable piBridgeNft = saleItem.nft;

        piBridgeNft.safeTransferFrom(address(this), itemSeller, nftId);
    ...
    function buyItem(uint256 _saleIndex) external nonReentrant validOpenMarket {
        ...
        IERC20Upgradeable currency = saleItem.currency;
        IERC721Upgradeable piBridgeNft = saleItem.nft;
        uint256 nftId = saleItem.nftId;
        uint256 nftType = saleItem.nftType;
        ...
        piBridgeNft.safeTransferFrom(address(this), msg.sender, nftId);

        emit EventBuyCompleted(saleIndex, nftId, nftType, sellerAddress, msg.sender, pricing);
    }
    ...
    function nftVersion() external pure returns (uint256) {
        return 1;
    }
}