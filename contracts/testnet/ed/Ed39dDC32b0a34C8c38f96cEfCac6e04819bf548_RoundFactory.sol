...
contract Round {
    ...
    function start(uint256 price, uint256 priceTimestamp) public returns (bool) {
        require(msg.sender == owner, "Access Denied");
        require(started==false, "Alreaded started");
        require(canceled==false, "Canceled");
        require(block.timestamp >= round_start, "Too early to start");
        if (block.timestamp > round_start+30) {
            ...
            cancel(address(this), "Started too late");
            return false;
        } else if (upAddr.length==0 || downAddr.length==0) {
            // We cancel the round and trigger refunds because:
            // - Betting is over and there are no odds (No bet on at least one side)
            // or
            // - The round is over 30sec late to start, the price obtained wouldn't be valid
            cancel(address(this), "Not enough bets");
            return false;
        } else {
    ...
    function end(uint256 price, uint256 priceTimestamp) public returns (bool, uint256, uint256) {
        require(msg.sender == owner, "Access Denied");
        ...
        if (block.timestamp > round_end+30) {
            ...
            cancel(address(this), "Ended too late");
            return (false, 0, 0);
        } else {
            ended = true;
            ...
            return (true, creatorFeeValue, feeValue);
        }
    }
    ...
    function cancel(address canceledBy, string memory reason) public {
        ...
        canceled = true;
        factory.logCancelation(round_id, canceledBy, reason);
    }
...
contract RoundFactory {
    ...
    event roundCanceled(uint256 round_id, address canceledBy, string reason);
    ...
    event statusChanged(bool isActive, string reason);
    ...
    constructor (address _referralContract) {
        ...
        owner            = msg.sender;
        vault            = BankVault(0xfAbEe939647804253B226f9cEE026a6e55C729C0);
        emit statusChanged(true, "Creation");
    }
    ...
    function start(uint256 roundId, uint256 price, uint256 priceTimestamp) public {
        ...
        bool started = round.start(price, priceTimestamp);
        if (started) {
    ...
    function end(uint256 roundId, uint256 price, uint256 priceTimestamp) public {
        require(msg.sender == owner, "Access Denied");
        Round round = Round(payable(rounds[roundId]));
        (bool ended, uint256 creatorFeeValue, uint256 feeValue) = round.end(price, priceTimestamp);
    ...
    /// @param roundId Round ID
    function cancel(uint256 roundId, string memory reason) public {
        require(msg.sender == owner || msg.sender==roundOwners[roundId], "Access Denied");
        Round round = Round(payable(rounds[roundId]));
        round.cancel(msg.sender, reason);
    }
    ...
    function logCancelation(uint256 roundId, address canceledBy, string memory reason) public {
        ...
        emit roundCanceled(roundId, canceledBy, reason);
    }
