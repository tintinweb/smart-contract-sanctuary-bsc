...
contract provideHelp is Context, Ownable {
    ...
    function provideHelpFunction(uint256 amount ) public payable {
        ...
        _helpPoolId++;
        users[_msgSender()].totalHelp.push(_helpPoolId);
        clearGetHelp();
    ...
    function getWithdrawalAmount(address userAddress) public view returns (uint256){
        uint256 withdrawalAmount=0;
         for (uint256 index = 0; index < users[userAddress].totalHelp.length -1; index++) {
            HelpPoolStruct memory helpPoolDetails = helpPool[users[userAddress].totalHelp[index]];
    ...
    function percentage(uint256 price, uint256 per) internal pure returns (uint256) {
     ...
     function clearGetHelp() internal{
        if(getHelp[currentGetHelp].isPaid && getHelp[currentGetHelp].amount>0 && getHelp[currentGetHelp].amount<=address(this).balance)
        {
           payable(getHelp[currentGetHelp].userAddress).transfer(getHelp[currentGetHelp].amount);
           currentGetHelp++;
        }
     }
}
