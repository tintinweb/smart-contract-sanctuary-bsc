...
contract DebtShareAggregator {
    ...
    mapping(address => uint) internal _debtShareBalanceOf;
    ...
    function mintDebtShare(address account, uint amount) internal {
        _debtShareBalanceOf[account] += amount;
...
contract LiquidatorRewardsAggregator {
    ...
    function updateEntry(
        ...
        uint len = currencyKeys.length;

        for (uint ii = 0; ii < len; ii++) {
            bytes32 currencyKey = currencyKeys[ii];
...
contract SynthrBridge is BaseBridge, CollateralAggregator, DebtShareAggregator, LiquidatorRewardsAggregator, RewardEscrowV2Aggregator, SynthAggregator {
    ...
    function _sendMint(
        ...
        if (_synthKey != bytes32(0) && _synthAmount != 0) {
            ...
            bytes32[] memory collateralKeys = IWrappedSynthr(_wrappedSynthr).getAvailableCollaterals();
            updateEntry(collateralKeys, _debtShareBalanceOf[_accountForCollateral], _accountForCollateral);
        }
    ...
    function _sendBurn(
        ...
        bytes32[] memory collateralKeys = IWrappedSynthr(_wrappedSynthr).getAvailableCollaterals();
        updateEntry(collateralKeys, _debtShareBalanceOf[accountForSynth], accountForSynth);
    }
    ...
    function _sendLiquidate(
        ...
        bytes32[] memory collateralKeys = IWrappedSynthr(_wrappedSynthr).getAvailableCollaterals();
        updateEntry(collateralKeys, currentDebtShareBalance, account);
