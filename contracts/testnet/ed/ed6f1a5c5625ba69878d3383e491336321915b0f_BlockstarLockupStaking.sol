...
contract BlockstarLockupStaking is Ownable {
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        uint256 total = 0;
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > userdata[i].endtime){
                total += (userdata[i].amount * userdata[i].returnPer) / 10000 ;
            }
        }
        return total;
    }
    ...
    function withdraw() external {
        ...
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > userdata[i].endtime && userdata[i].amount > 0){
                ...
                uint256 reward = (userdata[i].amount * userdata[i].returnPer) / 10000;
    ...
    function emergencyWithdraw() external {
        ...
        for (uint256 i = 0; i < len; i++) {
            if( userdata[i].amount > 0){
                uint256 reward = (userdata[i].amount * emergencyWithdrawFess) / 10000;
                uint256 amount =  userdata[i].amount;
                uint256 total = amount - reward  ;
