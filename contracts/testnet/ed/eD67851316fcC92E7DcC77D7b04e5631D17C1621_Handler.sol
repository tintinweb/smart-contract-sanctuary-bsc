...
/**
 * @title Handler
 * @notice The handler is the main entry point for rebase operations. It coordinates the policy
 * actions with external consumers.
*/
contract Handler is Ownable {

    RUSDFractionPolicy public policy;
    IRouter public router;
    IRUSDFraction public rusd;
    address public wbnb;
    address public reflecto;
    address public reflectoPool;
    address[] public bnbPath;
    address[] public reflectoPath;

    IProviderPair[] public providerPairs;

    event Swap(uint256,uint256,address[],address,uint256);
    ...
    /**
     * @param _policy Address of the RUSDFraction policy.
     * @param _router Address of the RUSDFraction policy.
     * @param _rusd Address of the RUSDFraction policy.
     * @param _wbnb Address of the RUSDFraction policy.
     * @param _reflecto Address of the RUSDFraction policy.
     * @param _reflectoPool Address of the RUSDFraction policy.
    */
    constructor(address _policy, address _router, address _rusd, address _wbnb, address _reflecto, address _reflectoPool) public {
        Ownable.initialize(msg.sender);
        policy = RUSDFractionPolicy(_policy);
        router = IRouter(_router);
        rusd = IRUSDFraction(_rusd);
        wbnb = _wbnb;
        reflecto = _reflecto;
        reflectoPool = _reflectoPool;
    }


    /**
     * @notice Main entry point to initiate a rebase operation.
     *         The Handler calls rebase on the policy and notifies downstream applications.
     *         Contracts are guarded from calling, to avoid flash loan attacks on liquidity
     *         providers.
     *         If a transaction in the transaction list fails, Handler will stop execution
    ...
    function setRUSDAddress(IRUSDFraction _rusd) external onlyOwner {
        rusd = _rusd;
    }
    ...
    function setReflectoAddress(address _reflecto) external onlyOwner {
        reflecto = _reflecto;
    }

    function setReflectoPoolAddress(address _reflectoPool) external onlyOwner {
        reflectoPool = _reflectoPool;
    }

    function setPolicyAddress(address _policy) external onlyOwner {
        policy = RUSDFractionPolicy(_policy);
    }

    function _getAmountOut(uint256 _amountIn, address[] memory _path) public view returns (uint){
        _amountIn = (_amountIn*95)/100;
        uint[] memory amountOut = router.getAmountsOut(_amountIn,_path);
        return amountOut[1];
    }

    function _getAmountIn() public view returns (uint){
        uint _amountIn = (rusd.balanceOf(address(this)));
        return _amountIn;
    }

    function _convertRUSDtoBNB() internal{
        bnbPath.push(address(rusd));
        bnbPath.push(wbnb);
        uint amountIn = (rusd.balanceOf(address(this)))/2;
        uint amountInmin = (amountIn*95)/100;
        uint256 amountOut = _getAmountOut(amountInmin,bnbPath);
        uint deadline = block.timestamp+1000;
        address to = reflecto;
        rusd.approve(address(router),amountIn);
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(amountIn,amountOut,bnbPath,to,deadline);
        emit Swap(amountIn,amountOut,bnbPath,to,block.timestamp);
    }

    function _convertRUSDtoReflecto() internal{
        reflectoPath.push(address(rusd));
        reflectoPath.push(reflecto);
        uint amountIn = (rusd.balanceOf(address(this)))/2;
        uint amountInmin = (amountIn*95)/100;
        uint256 amountOut = _getAmountOut(amountInmin,reflectoPath);
        uint deadline = block.timestamp+1000;
        address to = reflectoPool;
        rusd.approve(address(router),amountIn);
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn,amountOut,reflectoPath,to,deadline);
        emit Swap(amountIn,amountOut,reflectoPath,to,block.timestamp);
    }
}