 ...
 * Takes in KNB as votes
 * Allows addition and removal of votes during a proposal is open
 * Allows withdrawal of all KNB once the latest voted proposal of a user is closed
...
contract Governance {
    ...
    //address public constant TRUSTED_TOKEN_ADDRESS = 0x197dc4bfc82ad08E3FBaa930A869485236e6A792; //Binance mainnet token address
    address public constant TRUSTED_TOKEN_ADDRESS = 0x1BEb56FEcD26c1Ecf4BfEBbE5112D16CD4414331; // BSC testnet KNB token address
    ...
    // address user => total deposited KNB
    ...
    // Any KNB holder with a minimum required KNB balance may initiate a proposal
    ...
    // Any KNB holder may add votes for a particular open proposal, 
    // with options YES / NO | DISBURSE / BURN | ONE / TWO
    // with `amount` KNB, each KNB unit corresponds to one vote unit
    ...
    // Any voter may remove their votes (KNB) from any proposal they voted for 
    // only when the proposal is open - removing votes refund KNB to user and deduct their votes
    ...
    // The user may remove all KNB they added to this contract
