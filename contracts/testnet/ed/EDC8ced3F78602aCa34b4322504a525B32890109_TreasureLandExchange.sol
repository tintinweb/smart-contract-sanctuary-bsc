    ...
    ReentrancyGuardUpgradeable
{
    ...
    function matchAskWithTakerBid(
        ...
        if (item.itemType == ItemType.ERC1155) {
            uint256 remainAmount = item.amount - orderFilledAmount[makerAsk.signer][itemNonce];
            require(
    ...
    function matchBidWithTakerAsk(
        ...
        if (item.itemType == ItemType.ERC1155) {
            uint256 remainAmount = item.amount - orderFilledAmount[makerBid.signer][itemNonce];
            require(
                remainAmount >= buyAmount,
                "Order: ERC1155 insufficient amount"
            );
        ...
        returns (bool[] memory successList)
    {
        ...
        for (uint256 i = 0; i < takerBid.offerComponents.length; i++) {
            ...
            if (item.itemType == ItemType.ERC1155) {
                uint256 remainAmount = item.amount - orderFilledAmount[advanceOrder.signer][itemNonce];
                require(
                    remainAmount >= buyAmount,
                    "Order: ERC1155 insufficient amount"
                );
    ...
    function batchMatchBidWithTakerAsk(
        ...
        for (uint256 i = 0; i < takerAsk.offerComponents.length; i++) {
            ...
            if (item.itemType == ItemType.ERC1155) {
                uint256 remainAmount = advanceOrder.items[itemIndex].amount - orderFilledAmount[advanceOrder.signer][itemNonce];
                require(
                    remainAmount >= buyAmount,
                    "Order: ERC1155 insufficient amount"
                );
