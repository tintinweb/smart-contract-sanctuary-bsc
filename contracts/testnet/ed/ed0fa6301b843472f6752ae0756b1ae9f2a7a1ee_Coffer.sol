...
// File: contracts/coffer/Coffer.sol
...
contract Coffer is Ownable, ReentrancyGuard {
    ...
    event Withdraw(uint256 serialNum, address payee, address tokenAddr, uint256 amount);
    ...
    returns(bool){
        ...
        require(balanceNow >= balanceBefor + receipt.tokenAmount,"Token not received");
    ...
    nonReentrant {
        ...
        if (tokenAddr == address(0)) {//withdraw ETH
            ...
            emit Withdraw(serialNum, payee, tokenAddr, amount);
        } else {//withdraw ERC20
    ...
    public 
    nonReentrant {
        verify(serialNum, tokenAddr, amount, payee, sign);
        if (serialNums[serialNum] == true) revert("Withdraw invalid");
        if (tokenAddr == address(0)) {//withdraw ETH
            require(address(this).balance >= amount, "Insufficient ETH");
            serialNums[serialNum] = true;
    ...
    view 
    returns (bool) {
        bytes memory message = abi.encode(serialNum, tokenAddr, amount, payee, msg.sender);
