 ...
 */
contract TokenModelFinal is ERC20, Ownable {
    ...
    /** Cooldowns */
    bool public sellCooldown = true;
    uint256 public sellCooldownTime = 60 seconds;
    mapping (address => uint256) private lastSellTx;
    bool public buyCooldown = true;
    uint256 public buyCooldownTime = 60 seconds;
    mapping (address => uint256) private lastBuyTx;
    bool public txCooldown = true;
    uint256 public txCooldownTime = 60 seconds;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        //Sell                   
            ...
            // Cooldown 
            if (sellCooldown && !exemptFee[tx.origin]) {
                require(lastSellTx[tx.origin] < block.timestamp, "Please wait for cooldown to finish.");
                lastSellTx[tx.origin] = block.timestamp + sellCooldownTime;
            }         
        //Buy
            ...
            // Cooldown
            if (buyCooldown && !exemptFee[tx.origin]) {
                require(lastBuyTx[tx.origin] < block.timestamp, "Please wait for cooldown to finish.");
                lastBuyTx[tx.origin] = block.timestamp + buyCooldownTime;
            }              
        //All        
            ...
            _taxDenominator = taxes.denominator;                  
            // Cooldown
            if (txCooldown && !exemptFee[tx.origin]) {
                require(lastTx[tx.origin] < block.timestamp, "Please wait for cooldown to finish.");
                lastTx[tx.origin] = block.timestamp + txCooldownTime;
            }              
        //Launch
        } else if (useLaunchFee) {
            feeswap = launchtax;
    ...
    function setBuyCooldown(bool _status, uint _seconds) external onlyOwner {
        require(_seconds <= 60, "Max cooldown duration must be at most of 60 seconds");
        buyCooldown = _status;
        buyCooldownTime = _seconds;
    }

    function setSellCooldown(bool _status, uint _seconds) external onlyOwner {
        require(_seconds <= 60, "Max cooldown duration must be at most of 60 seconds");
        sellCooldown = _status;
        sellCooldownTime = _seconds;
    }

    function setTxCooldown(bool _status, uint _seconds) external onlyOwner {
        require(_seconds <= 60, "Max cooldown duration must be at most of 60 seconds");
        txCooldown = _status;
        txCooldownTime = _seconds;
    }
