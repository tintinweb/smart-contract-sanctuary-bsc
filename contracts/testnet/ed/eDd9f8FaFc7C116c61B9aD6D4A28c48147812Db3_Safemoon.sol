...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
   ...
   function excludeFromReward(address account) public onlyOwner() {
    ...
    function refVal() public view returns(uint256,uint256){
        uint256 a = 0;
        uint256 b = 0;

        for (uint256 i = 0; i < _excluded.length; i++) {
             a = a.add(_rOwned[_excluded[i]]);
             b = b.add(_tOwned[_excluded[i]]);
         }
        return (a,b);
    }

    function excluLength() public view returns(uint256)
    {
        return _excluded.length;
    }


    function excluValue(uint256 i) public view returns(uint256)
    {
        return (_tOwned[_excluded[i]]);
    }
    ...
    function _getCurrentSupply() public view returns(uint256, uint256) {
      ...
      uint256 _rOwnedSum = 0;
      uint256 _tOwnedSum = 0;  
      for (uint256 i = 0; i < _excluded.length; i++) {
          _rOwnedSum = _rOwnedSum.add(_rOwned[_excluded[i]]);
          _tOwnedSum = _tOwnedSum.add(_tOwned[_excluded[i]]);
          if (_rOwnedSum > _rTotal || _tOwnedSum > _tTotal) return (_rTotal, _tTotal);
      }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount, uint256 tierIndex) private {
       ...
       // rSupply = rSupply.sub(_values.rFee);
    ...
    function feeValues(uint256 tAmount, uint256 tierIndex) public view returns(uint256,uint256){
        FeeValues memory _values = _getValues(tAmount, tierIndex);
        return (_values.rFee, _values.tFee);
    }
