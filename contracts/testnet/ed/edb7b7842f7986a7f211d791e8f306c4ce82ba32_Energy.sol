...
interface IEnergy {
    ...
    /**
     * @dev Moves `amount` energy from an `exchange` to `recipient` using the
     ...
     * Emits a {Transfer} event.
     */
    function transferFrom(address exchange, address recipient, uint256 amount) external returns (bool);
...
contract Energy is IEnergy, Ownable {
     ...
     */
    function produce(uint256 amount, uint cleanness) internal virtual {
        require(amount > 0, "Energy: Cannot produce zero energy");
        require(cleanness >= 1, "Energy: cleanness cannot be less than 1 degree");
