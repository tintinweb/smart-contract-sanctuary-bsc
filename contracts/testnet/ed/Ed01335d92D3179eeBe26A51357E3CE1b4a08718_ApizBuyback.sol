...
contract ApizBuyback is Ownable, ReentrancyGuard {
    ...
    function createDeposit(uint256 amount) external nonReentrant {
        require(
            amount > 0 &&
                ...
                IToken(APIZ).purchasedOnCrowdsale(_msgSender()),
    ...
    function claimUsdt(uint256 tokenId) external nonReentrant {
        ...
        require(
            deposits[tokenId].amount > 0 && deposits[tokenId].end <= _getTime(),
            "ApizBuyback: wrong deposit params"
        );

        unfreezedAmount += deposits[tokenId].amount;

        uint256 outputAmount = _getAmountToReceive(tokenId);
    ...
    function getAvailableTokensToDeposit(address user)
        ...
        returns (uint256)
    {
        uint256 available = (IToken(APIZ).purchasedOnCrowdsale(user) -
            alreadySold[user]);
        return
...
interface IToken {
    function purchasedOnCrowdsale(address user) external view returns (uint256);
}
