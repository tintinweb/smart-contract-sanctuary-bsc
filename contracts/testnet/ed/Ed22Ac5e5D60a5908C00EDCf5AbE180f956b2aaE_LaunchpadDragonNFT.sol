...
// File: contracts\LaunchpadDragonNFT.sol
...
contract LaunchpadDragonNFT is ERC721Enumerable, Ownable {
    ...
    // end
    constructor(string memory name_, string memory symbol_, string memory globalURI_,address launchpad, uint256 maxSupply) ERC721(name_, symbol_) {
        baseURI = "";
        suffix = "";
    ...
    function setGlobalURI(string memory _globalURI) external onlyOwner {
        globalURI = _globalURI;
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token.");
        return bytes(globalURI).length > 0 ? globalURI : string(abi.encodePacked(baseURI, tokenId.toString(), suffix));
    }
