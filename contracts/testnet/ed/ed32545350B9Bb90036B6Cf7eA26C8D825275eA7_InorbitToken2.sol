...
contract InorbitToken2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeCast for int256;

    string private _name = "InorbitTokenTest2";
    string private _symbol = "ITXTest2";
    ...
    uint256 internal _tokenTotal = 2_000_000_000e9;
    ...
    address public developmentWallet=0x1C68DFB90D6F2acb05b7b34dF191e9C6B38E9Cb1;
    ...
    constructor(address router) {
        // IPancakeswapV2Router02 _pancakeswapV2Router = IPancakeswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // Uniswap Router For Ethereum
    ...
    function collectFee(
        ...
        uint256 transferAmount = amount;
        //This 3% fee is applicable when the  contract is going to reward to the token holders.
         if(taxFee != 0 && (to == pancakeSwapV2Pair|| enableAllFee)){
       ...
       //developmentFee  fee 3% On sale
        if (developmentFee != 0 && (to == pancakeSwapV2Pair || enableAllFee)) {
            uint256 _developmentFee = amount.mul(developmentFee).div(10**(feeDecimal + 2));
            transferAmount = transferAmount.sub(_developmentFee);
            _reflectionBalance[developmentWallet] = _reflectionBalance[developmentWallet].add(
                _developmentFee.mul(rate)
            );
            if (_isExcluded[developmentWallet]) {
                _tokenBalance[developmentWallet] = _tokenBalance[developmentWallet].add(
                    _developmentFee
                );
            }
            feeTotal = feeTotal.add(_developmentFee);
            emit Transfer(account, developmentWallet, _developmentFee);
        }
        ...
        if (burnFee != 0 &&( to == pancakeSwapV2Pair|| enableAllFee )) {
            ...
            //address(this)
            _tokenBalance[burnWallet] = _tokenBalance[burnWallet].add(_burnFee);
    ...
    function setdevelopmentFee(uint256 fee) external onlyOwnerAndAdmin {
        developmentFee = fee; //4
    }
    ...
    function setDevelopmentWallet(address wallet) external onlyOwnerAndAdmin {
