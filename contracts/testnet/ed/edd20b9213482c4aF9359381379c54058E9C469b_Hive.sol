...
contract Hive is Ownable {
    ...
    function allocateBNBHoney(address[] calldata addresses, uint256 amount)
        external
        onlyOwner
    {
        for (uint8 i = 0; i < addresses.length; i++) {
    ...
    function withdrawToken(address token, address _to) external onlyOwner {
