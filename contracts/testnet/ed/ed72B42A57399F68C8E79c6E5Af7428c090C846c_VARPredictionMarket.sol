...
contract VARPredictionMarket is Ownable, Pausable, ReentrancyGuard {
    ...
    struct Round {
        ...
        int256 closePrice;
        uint256 totalAmount;
        uint256 upAmount; // total bet up amount
        uint256 downAmount; // total bet down amount
        uint256 rewardBaseCalAmount; // Amount of reward share among winner (if up win, rewardBase = downAmount)
        uint256 rewardAmount; // = total amount - fee
        bool roundExecuted; // is executed this round
    }

    struct BetInfo {
        Position position; // Up or Down
    ...
    function claim(
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            if (rounds[epochs[i]].roundExecuted) {
                require(
