...
contract USBT is Ownable{
    ...
    function getRemainingTime(address _user) public view returns(uint256){
        uint256 orderNo = (orderInfos[_user].length).sub(1);
        uint256 days_;
        uint256 dayIncreament;
        uint256 totalDays;
        uint256 remainingTime;
        OrderInfo storage order = orderInfos[_user][orderNo];
        days_ = getCurDay1(order.start);
        for(uint256 j = 1; j <= 9; j++)
        {
            dayIncreament = daysCycle1 + daysCycle1.mul(j);
        ...
        remainingTime = totalDays.sub(days_);
