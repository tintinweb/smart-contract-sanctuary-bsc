...
contract FlowToken is IBEP20, Auth {
    ...
    string constant _name = "Flow Token";
    string constant _symbol = "FLW";
...
//        emit Transfer(sender, recipient, amount);
    ...
    function triggerBuyback(uint256 amount, bool triggerBuybackMultiplier) external authorized {
        buyTokens(amount, buybackReceiver);
    ...
    function setTokenAddresses(address nBUSD) external authorized {
        distributor.setBUSD(nBUSD);
    }

    function setRewardDexRouter(address nRouter) external authorized {
        distributor.setRouter(nRouter);
    }

    function recoverTokenFromDistributor(IBEP20 _tokenaddress, address _receiver, uint256 _amount) external authorized {
        distributor.recoverToken(_tokenaddress, _receiver, _amount);
    }

    function claimBNB(address _receiver, uint256 _amount) external authorized {
