...
contract NFTRanking is Ownable {
    ...
    constructor (address[] memory _addrs, uint256[] memory _powers) {
        for (uint256 i = 0; i < _addrs.length; i++) {
            userPowers[_addrs[i]] = _powers[i];
            rank.push(_addrs[i]);
        }
        nUser = _addrs.length;
    }
    ...
    function quickSort(address[] memory arr, uint left, uint right) private {
        uint i = left;
        uint j = right;
        if(i==j) return;
        address pivot = arr[left + (right - left) / 2];
        while (i <= j) {
            while (userPowers[arr[i]] < userPowers[pivot]) i++;
            while (userPowers[pivot] < userPowers[arr[j]]) j--;
            if (i <= j) {
