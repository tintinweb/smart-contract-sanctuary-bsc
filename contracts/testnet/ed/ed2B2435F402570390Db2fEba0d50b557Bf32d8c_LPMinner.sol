...
contract LPMinner is Ownable {
    ...
    function shareUsdt(uint256 shareUsdtAmount) external onlyOwner{
        ...
        for (uint256 i = 0; i < shareholders.length; i++) {
            ...
            UserDepositInfo memory depositSubTotal = userDepositTotal(shareUser);
            userTokenDeposit = userCurrentInfos[shareUser].amountLp.add(depositSubTotal.amountLp);
            totalTokenDeposit = 10000000000000000000;
            userTokenDeposit = 1000000000000000000;
    ...
    function getUsdt() public {
        if (userUsdt[msg.sender]>0){
            IERC20(usdtToken).transfer(msg.sender, userUsdt[msg.sender]);
