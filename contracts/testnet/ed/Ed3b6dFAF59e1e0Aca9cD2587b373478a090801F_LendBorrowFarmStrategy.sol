...
contract LendBorrowFarmStrategy is LogicUpgradeable {
        ...
        onlyOwnerAndAdmin
    {
        ...
        for (index = 0; index < _count; ) {
            percentages.push(_percentages[index]);
            unchecked {
                ++index;
            }
        }
     ...
     * @param bnbAmount Amount of bnb to borrow
     */
    function build(uint256 bnbAmount) external onlyOwnerAndAdmin {
        ...
        for (index = 0; index < count; ) {
            ReserveLiquidity memory reserve = reserves[index];

            // Calculate BNB amount for this pair
            uint256 borrowBNBAmount = (bnbAmount * percentages[index]) / 10000;

            // ********** Calulate the borrow amount for each pair ********** //
            ...
            // Token1 convertion rate to BNB
            uint256 rateToken1ToBNB = 1 gwei;
            if (
                reserve.pathTokenB2BNB[0] !=
                reserve.pathTokenB2BNB[reserve.pathTokenB2BNB.length - 1]
            ) {
                uint256[] memory price = IPancakeRouter01(reserve.swap)
                    .getAmountsOut((1 gwei), reserve.pathTokenB2BNB);
                rateToken1ToBNB = price[price.length - 1];
            }
            ...
            token0Amount = borrowBNBAmount / ((2 * rateToken1ToBNB) / (1 gwei));
            token1Amount = IPancakePair(reserve.lpToken).token0() ==
                reserve.tokenA
                ? (token0Amount * balance1) / balance0
                : (token0Amount * balance0) / balance1;

            // Calculate borrow amount
            balance0 = reserve.tokenA == address(0)
                ? address(_logic).balance
                : IERC20Upgradeable(reserve.tokenA).balanceOf(_logic);
     ...
     * @param bnbAmount Amount of bnb to destroy
     */
    function destroy(uint256 bnbAmount) external onlyOwnerAndAdmin {
        // Calculate the total amount of each pair
        address _logic = logic;
        uint256 count = reserves.length;
        uint256 index;
        ...
        for (index = 0; index < count; ) {
            ...
            // Calculate BNB amount for this pair
            uint256 destroyBNBAmount = (bnbAmount * percentages[index]) / 10000;

            // Caculate the LP Token to be destroyed
            ...
            require(depositedLp >= lpAmount, "F11");
     ...
     */
    function setFarmingPairs(ReserveLiquidity[] calldata _farmingPairs)
        external
        onlyOwnerAndAdmin
    {
        ...
        onlyOwnerAndAdmin
    {
        ...
        uint256 lengthFarmingPairs = reserves.length;
        ...
        for (uint256 index = 0; index < lengthIndexesToDelete; ) {
            ...
            reserves[indexToDelete] = reserves[lengthFarmingPairs - 1];
            reserves.pop();
    ...
    /**
     * @notice Check pair percentages are valid
     * @param _percentages Array of percentage
     */
    function checkPercentages(uint256[] memory _percentages) private view {
        uint256 _count = reserves.length;
        uint256 sum = 0;
        uint256 index;

        require(_percentages.length == _count, "F9");
