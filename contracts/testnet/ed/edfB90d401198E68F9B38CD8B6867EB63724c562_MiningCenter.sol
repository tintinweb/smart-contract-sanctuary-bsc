...
contract MiningCenter is Ownable {
    ...
    function getAvailableMintedAmount(
        ...
        if (boosterStatus.boosterStyle == 0) {
            if (
                beginStampList[_owner][_nftType][_nftNum] + 10 minutes <=
                block.timestamp
            ) {
                return 1;
            } else {
     ...
     */
    function dailyMining(uint256 _nftType, uint256 _nftNum) external {
        require(
        ...
        for (
            ...
            for (uint256 i = 0; i < nftHistoryLength[_owner][nftType]; i++) {
                ...
                if (
                    ...
                    if (
                        miningBoosterStatus[_owner].endStamp + 10 minutes >=
                        block.timestamp
                    ) {
                        beginStampList[_owner][_nftType][_nftNum] = block
                            .timestamp;
