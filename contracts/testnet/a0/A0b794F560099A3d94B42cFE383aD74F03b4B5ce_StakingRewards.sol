...
contract StakingRewards is IStakingRewards, ReentrancyGuard, Ownable {
    ...
    uint256 public constant MAX_HARVEST = 14 days;
        ...
        external
    {
        ...
        require(
            ...
            "zero address has been given"
        );
        require(_depositFee <= MAX_FEE,"deposit fee must lesser than maximum fee");
        require(_harvestTime <= MAX_HARVEST,"harvest must lesser than maximum harvest");
    ...
    modifier onlyRewardsDistribution() {
        require(msg.sender == rewardsDistribution, "onlyRewardsDistribution: onlyRewardsDistribution");
        _;
    }

    modifier zeroCheck(address account) {
        require(account != address(0), "zeroCheck::account is zero");
        _;
    }
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        require(
            (userLastHarvest[msg.sender] == 0) || 
            (userLastHarvest[msg.sender] + harvestTime < block.timestamp), 
            "getReward::harvest has not reached"
        );
    ...
    function _stake(uint256 amount, address onBehalf) internal {
        require(amount > 0, "_stake::amount is zero");
        
        uint _fee = amount * depositFee / 100;
        assert(_fee <= amount);
        amount = amount - _fee;

        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        stakingToken.safeTransferFrom(msg.sender, feeDistributor, _fee);
        ...
        updateReward(address(0))
    {
        ...
        require(
            ...
            "notifyRewardAmount::reward rate has crossed the maximum rate"
        );
    ...
    function recoverERC20(
        ...
        require(
            ...
            "recoverERC20::tokenAddress must not be a staking or reward contract");
    ...
    function setFeeDistributor(address newFeeDistributor) external onlyOwner {
        require(newFeeDistributor != address(0), "setFeeDistributor::new fee distributor is zero");
        feeDistributor = newFeeDistributor;
    }

    function setHavestTime(uint256 newHarvestTime) external onlyOwner {
        require(newHarvestTime <= MAX_HARVEST, "setHavestTime::new harvest period is higher than max harvest");
        harvestTime = newHarvestTime;
    }

    function setDepositFee(uint256 newDepositFee) external onlyOwner {
        require(newDepositFee <= MAX_FEE, "setDepositFee::new deposit fee is higher than max harvest");
