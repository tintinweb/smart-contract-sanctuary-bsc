...
contract YDT is ERC20, AccessControl, ReentrancyGuard {
    ...
    // @notice deposit funds to You Donate Protocol
    function deposit(address _token, uint256 _amount) public nonReentrant  {
        ...
        transfer(msg.sender, 100000000000000000000);
    ...
    function donateToProject(uint _proposalId, uint256 _amount) public nonReentrant {
    ...
    function withdrawFunds(address _tokenAddress, uint256 _tokenAmount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(isAcceptedToken[_tokenAddress], "Unaccepted token");
        IERC20(_tokenAddress).safeTransfer(address(msg.sender), _tokenAmount);
        emit WithdrawFunds(_tokenAddress, _tokenAmount);
    }
