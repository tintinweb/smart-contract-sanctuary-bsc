...
contract DecubateICO is OwnableUpgradeable {
  struct Investment {
    uint256 amount;
    uint32 time;
  }
  ...
  struct Sale {
    ...
    uint256 minPerUser;
    uint256 maxPerUser;
    uint256 tokenPrice; // Token priced in decimals of PAYMEMT token: 0.05 BUSD -> 50000000000000000
  ...
  IDecubateWalletStore public whitelist;
  IDecubateVestingV2 public vesting;
  IERC20Upgradeable public paymentToken;
  Sale[] public sales;
  uint8 public vestedTokenDecimals;

  function initialize(
    address _paymentToken,
    address _whitelist,
    address _vesting,
    uint8 _vestedTokenDecimals
  ) external initializer {
    __Ownable_init();
    paymentToken = IERC20Upgradeable(_paymentToken);
    whitelist = IDecubateWalletStore(_whitelist);
    vesting = IDecubateVestingV2(_vesting);
    vestedTokenDecimals = _vestedTokenDecimals;
  }
  ...
  function add(
    ...
    sale.minPerUser = _minPerUser;
    sale.maxPerUser = _maxPerUser;
    sale.tokenPrice = _tokenPrice;
    sale.start = _start;
    sale.end = _end;
    sale.created = uint32(block.timestamp);
  }
  ...
  function set(
    ...
    require(sales.length > _pid, "Sale does not exist");

    Sale storage sale = sales[_pid];
  ...
  function pause(uint256 _pid) external onlyOwner {
    require(sales.length > _pid, "Sale does not exist");
    sales[_pid].paused = true;
  }

  function unpause(uint256 _pid) external onlyOwner {
    require(sales.length > _pid, "Sale does not exist");
    sales[_pid].paused = false;
  }

  function invest(uint256 _pid, uint256 _amount) external {
    require(sales.length > _pid, "Sale does not exist");

    Sale storage sale = sales[_pid];
    Investment storage userInvestment = sale.investments[msg.sender];

    require(!sale.paused, "Sale has stopped");
    require(block.timestamp > sale.start, "Sale has not started");
    require(block.timestamp < sale.end, "Sale has ended");
    require(whitelist.isVerified(msg.sender), "User not whitelisted");
    require(sale.totalInvested + _amount <= sale.hardcap, "Sale is full");
    require(userInvestment.amount + _amount > sale.minPerUser, "Less than min investment");
    require(userInvestment.amount + _amount <= sale.maxPerUser, "More than max investment");

    if (userInvestment.time > 0) {
      userInvestment.time = uint32(block.timestamp);
    ...
    // sale price is 0.05 BUSD. Invest amount is 200 BUSD. How to calulcate?

    uint256 vestAmount = (userInvestment.amount * (10**vestedTokenDecimals)) / sale.tokenPrice;

    vesting.addWhitelist(msg.sender, vestAmount, _pid); // Requires matching vesting strategy
  }
...
//** Decubate vesting Contract interface */
