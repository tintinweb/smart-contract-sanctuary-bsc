...
import {IBluechipsBooster, IConfigStore} from "./interfaces/IBluechipsBooster.sol";

contract BluechipsBooster is Initializable, IBluechipsBooster, ERC721Upgradeable {
  /*╔═════════════════════════════╗
  ...
  mapping(bytes32 => address) private _proofBy;

  // Multiplier for each Bluechips NFT
  // 123 => 1.23x Max multiplier: 150
  mapping(address => uint16) private _multiplierOf;

  // Final Boost ratio for Bluechips NFT in each wallet address
  mapping(address => uint256) private _boosterWeights;

  // The Bytescode of bluechips NFT => The amount of staked ETH
  mapping(bytes32 => uint256) private _stakedOf;

  // daoId => Customized NFT Token Address => multiper
  // 123 => 1.23x Max multiplier: 300
  mapping(uint256 => mapping(address => uint16)) private _customMultiplierOf;

  // daoId => Wallet Address => booster weights
  mapping(uint256 => mapping(address => uint256)) private _customBoosterWeights;
  ...
  uint256 public constant PROOF_ETH = 1 ether;
  ...
  {
    _weight =
      _getNoZero(_boosterWeights[_from]) *
      _getNoZero(_customBoosterWeights[_projectId][_from]);
  }
  ...
  function initialize(
    ...
    __ERC721_init("BlueChip Stake Proof", "STAKEPROOF");
    ...
    for (uint256 i; i < _bluechips.length; ) {
      ...
      if (_multipers[i] == 0 || _multipers[i] > MAX_MULTIPLIER_OF_BLUECHIPS) revert BadMultiper();

      _multiplierOf[_bluechips[i]] = _multipers[i];
  ...
  function createCustomBooster(
    uint256 _projectId,
    address[] calldata _bluechips,
    uint16[] calldata _multipers
  ) external override {
    ...
    for (uint256 i; i < _bluechips.length; ) {
      if (_bluechips[i] == address(0)) revert ZeroAddress();
      if (_customMultiplierOf[_projectId][_bluechips[i]] > 0) revert BoosterRegisterd();
      if (_multipers[i] == 0 || _multipers[i] > MAX_MULTIPLIER_OF_CUSTOMIZED) revert BadMultiper();
      _customMultiplierOf[_projectId][_bluechips[i]] = _multipers[i];
      unchecked {
        i++;
      }
    }
  ...
  function createProof(address _bluechip, uint256 _tokenId) external payable override {
    ...
    if (_multiplierOf[_bluechip] == 0) revert BoosterNotRegisterd();
    ...
    address _preOwner = _proofBy[_proof];
    ...
    _proofBy[_proof] = _from;
    uint256 _weight = _getNoZero(_boosterWeights[_from]) * _multiplierOf[_bluechip];
    _boosterWeights[_from] = _weight;

    uint256 _proofExpiry = block.timestamp + 30 * SECONDS_IN_DAY;
    proofExpiryOf[_proof] = _proofExpiry;
    _stakedOf[_proof] = msg.value;

    _count++;

    // Mint the stake proof
    _safeMint(_from, _count);

    _tokenIdOf[_proof] = _count;

    emit CreateProof(_from, _bluechip, _tokenId, _proof, _proofExpiry, _weight, msg.value);
  }
  ...
  function createCustomProof(
    ...
    if (_customMultiplierOf[_projectId][_bluechip] == 0) revert BoosterNotRegisterd();
    if (IERC721(_bluechip).ownerOf(_tokenId) != _from) revert NotNFTOwner();
    ...
    uint256 _weight = _getNoZero(_customBoosterWeights[_projectId][_from]) *
      _customMultiplierOf[_projectId][_bluechip];

    _customBoosterWeights[_projectId][_from] = _weight;
  ...
  function challengeProof(address _bluechip, uint256 _tokenId) external override {
    bytes32 _proof = keccak256(abi.encodePacked(_bluechip, _tokenId));
    address _creator = _proofBy[_proof];
  ...
  function renewProof(address _bluechip, uint256 _tokenId) external override {
    bytes32 _proof = keccak256(abi.encodePacked(_bluechip, _tokenId));
    address _creator = _proofBy[_proof];
    if (msg.sender != _creator || IERC721(_bluechip).ownerOf(_tokenId) != msg.sender)
      revert UnAuthorized();

    uint256 _proofExpiry = block.timestamp + 30 * SECONDS_IN_DAY;
    proofExpiryOf[_proof] = _proofExpiry;

    emit RenewProof(msg.sender, _bluechip, _tokenId, _proof, _proofExpiry);
  }
  ...
  function renewCustomProof(
    ...
    emit RenewCustomizedProof(_projectId, msg.sender, _bluechip, _tokenId, _proof, _proofExpiry);
  }

  /**
   * @notice
   * The proof owner can redeem the proof on anytime
  ...
  function redeemProof(address _bluechip, uint256 _tokenId) external override {
    ...
    bytes32 _proof = keccak256(abi.encodePacked(_bluechip, _tokenId));
    address _creator = _proofBy[_proof];
    if (_from != _creator || _from != IERC721(_bluechip).ownerOf(_tokenId)) revert UnAuthorized();
   ...
   */
  function addBlueChip(address _bluechip, uint16 _multiper) external override {
    if (_bluechip == address(0)) revert ZeroAddress();
    if (msg.sender != configStore.superAdmin()) revert UnAuthorized();
    if (_multiplierOf[_bluechip] != 0) revert ForbiddenUpdate();

    _multiplierOf[_bluechip] = _multiper;
  ...
  function _redeem(
    ...
    uint256 _releaseAmount = _stakedOf[_proof];
    Address.sendValue(payable(_beneficiary), _releaseAmount);
    _stakedOf[_proof] = 0;
    proofExpiryOf[_proof] = 0;
    _boosterWeights[_from] = _boosterWeights[_from] / _multiplierOf[_bluechip];

    _burn(_tokenIdOf[_proof]);
    delete _tokenIdOf[_proof];

    delete _proofBy[_proof];
  ...
  function _redeem(
    ...
    _customBoosterWeights[_projectId][_from] =
      _customBoosterWeights[_projectId][_from] /
      _customMultiplierOf[_projectId][_bluechip];
...
interface IBluechipsBooster {
  ...
  event RedeemProof(address indexed from, address indexed bluechip, uint256 tokenId, bytes32 proof);
...
struct MetaConfig {
  string baseProjectURI;
  string baseMembershipPassURI;
...
interface IConfigStore {
  ...
  event SetMinRoyaltyFee(uint256 minLockRate);

  event SetMinLockRate(uint256 royaltyFee);

  event SetProposalRate(uint256 proposalRate);

  event SetProposalQuorum(uint256 proposalQuorum);
  ...
  function minRoyaltyFee() external view returns (uint256);

  function proposalRate() external view returns (uint256);
  ...
  function setMinRoyaltyFee(uint256 _royaltyFee) external;

  function setProposalRate(uint256 _proposalRate) external;

  function setProposalQuorum(uint256 _proposalQuorum) external;
