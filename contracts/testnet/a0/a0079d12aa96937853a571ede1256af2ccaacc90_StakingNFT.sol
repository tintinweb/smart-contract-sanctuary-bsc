...
contract StakingNFT is  OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, GlobalsAndUtils {
    ...
    function initialize(address _oracle) public virtual initializer {
        ...
        LAND_ADDRESS = address(0x2Ff2B5f314662CFdFA8bB070AeA96d53d21DeD37);
        GENESIS_ADDRESS = address(0xE12396B30cD18468B872a5CD34FcF41333B3f659);
     ...
     */
    function stake(uint256 tokenId, uint256 size, string calldata rarity, uint256[] memory genesisTokenIds, bytes32[] calldata proof) whenNotPaused external payable nonReentrant CampaignOnGoing {
       require(size.mul(size) == genesisTokenIds.length, "invalid genesis tokens");
        ...
        emit StakeStart(msg.sender, latestStakeId, tokenId);
    }
    ...
    function claimUnstake(uint256 _stakeId) whenNotPaused nonReentrant public {
        ...
        emit StakeEnd(msg.sender, _stakeId, usrStake.tokenId);
    }
    /**
    ...
    function appplyForUnstake(uint256 _stakeId)  whenNotPaused external {
        ...
        require(usrStake.tokenId > 0, "stake doesn't exist");
        _unStake(_stakeId);
        emit AppliedUnstake(msg.sender, _stakeId, usrStake.tokenId);
    }
...
abstract contract GlobalsAndUtils {
    ...
    /******************* Events **********************/
    event StakeStart(address staker, uint256 stakeIndex, uint256 tokenId);
    event StakeEnd(address staker, uint256 stakeIndex, uint256 tokenId);
    event AppliedUnstake(address staker, uint256 stakeIndex, uint256 tokenId);
