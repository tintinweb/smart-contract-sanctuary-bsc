...
contract PledgeMint is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint8) public pledgesAmount;
    ...
    function setupPledgePhase(uint256 _mintPrice, uint8 _maxPerWallet) external onlyOwner {
    ...
    function pledge(uint8 number) external payable callerIsUser {
        if(open == false){
    ...
    function _mintPledge(address[] memory addresses, uint startIdx, uint count) internal {
        for (uint i = startIdx; i < count; i++) {
            ...
            uint8 quantity = pledgesAmount[pledger];
    ...
    function refundAll() public onlyOwner {
        for (uint8 i = 0; i < pledgers.length; i++) {
