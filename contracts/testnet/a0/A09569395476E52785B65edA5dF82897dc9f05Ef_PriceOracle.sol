...
contract PriceOracle {
    ...
    constructor(address router_, address WETH_, address LP_) {
    ...
    function priceOf(address token) public view returns (uint256) {
        return token == WETH ? priceOfWETH() : priceOfToken(token);
    }
    ...
    function priceOfToken(address token) public view returns (uint256) {
        if (token == _LP) {
            uint bal = IERC20(WETH).balanceOf(_LP);
            return ( bal * 2 * priceOfWETH() ) / 10**18;
        }
        (uint256 p0, uint256 p1) = _lpAmountsForToken(token);
        return( ( p1 * priceOfWETH()) / p0);
    }

    function priceOfWETH() public pure returns (uint256) {
        return 300 * 10**18;
    }
