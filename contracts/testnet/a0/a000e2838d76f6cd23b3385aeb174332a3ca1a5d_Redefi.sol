...
contract Redefi is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2000000 * 10**3 * 10**18;
    ...
    // uint256 public _maxWalletToken = 10000 * 10**3 * 10**18;
    uint256 public swapTokensAtAmount = 50 * 10**3 * 10**18;
    uint256 public buyBackUpperLimit = 1 * 10**18;


    address public marketingAddress = 0x252f3202c55474d96140f0218e346507b7c04E82; //Marketing Wallet
    address public burningAddress = 0x000000000000000000000000000000000000dEaD; //Automatic Burning Wallet
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD; //Manual Burning if neccessary
    ...
    function setFeeRates(uint256 _rfi, uint256 _marketing, uint256 _burning, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
        feeRates.marketing = _marketing;
        feeRates.burning = _burning;
        feeRates.liquidity = _liquidity;
        feeRates.buyback = _buyback;
        emit FeesChanged();
    }

    function setSellFeeRates(uint256 _rfi, uint256 _marketing, uint256 _burning, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.marketing = _marketing;
        sellFeeRates.burning = _burning;
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rMarketing, to_return.rBurning, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rMarketing, uint256 rBurning, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rBurning-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rBurning,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        _takeBurning(s.rBurning,s.tBurning);
        _takeLiquidity(s.rLiquidity,s.tLiquidity);
        _takeBuyback(s.rBuyback, s.tBuyback);
        emit Transfer(sender, recipient, s.tTransferAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tBurning + s.tBuyback);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.marketing + feeRates.burning) * 2;
        ...
        // Send BNB to developmentWallet
        uint256 burningAmt = unitBalance * 2 * feeRates.burning;
        if(burningAmt > 0){
          payable(burningAddress).transfer(burningAmt);
        }
    }
