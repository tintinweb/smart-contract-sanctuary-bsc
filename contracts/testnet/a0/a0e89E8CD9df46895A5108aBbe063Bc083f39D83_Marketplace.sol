...
contract Marketplace is MarketFee, ReentrancyGuard, Ownable, Pausable {
    ...
    mapping(address => bool) public _allowUnits;
    ...
    function setAllowUnit(address unitAddress, bool isAllow) public onlyOwner {
        ...
        if (_allowUnits[unitAddress] != isAllow) {
            _allowUnits[unitAddress] = isAllow;
        }
    }
    ...
    function sell(
        ...
        require(_allowUnits[unitAddress], "Invalid pay unit");
        // Todo
     ...
     */
    function getLatestMarketItemByTokenId(uint256 tokenId, address nftContractAddress)
        public
        view
        returns (MarketItem memory, bool)
    {
        ...
        for (uint256 i = itemsCount; i > 0; i--) {
            ...
            if (item.tokenId != tokenId || item.nftContractAddress != nftContractAddress) continue;
            return (item, true);
        }
