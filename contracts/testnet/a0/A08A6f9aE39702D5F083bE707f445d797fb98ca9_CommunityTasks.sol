...
contract CommunityTasks
{
    ...
    constructor() 
    {
        ...
        // LPOP
        internalToken = block.chainid == 56 ?  address(0xEA96DAA186C1fa0A709929822867Ec176Af8bafF) : 
                    (block.chainid == 137 ?     address(0) :
                    (block.chainid == 1 ?       address(0) : 
                    (block.chainid == 43114 ?   address(0) : 
                    (block.chainid == 97 ?      address(0xEA96DAA186C1fa0A709929822867Ec176Af8bafF) : 
                                                address(0) ) ) ) );
        ...
        */
        ...
        /*
        56: Lollipop Swap Factory
        137: SushiSwap
        1: UniswapV2Factory
        43114: PangolinFactory
        97: Lollipop Swap Factory testnet
        */
        swapFactory = block.chainid == 56 ?     address(0x67eFC2Bb5c6f39d50E6Be6f0aA71b35767e8bC28) : 
                    ...
                    (block.chainid == 97 ?      address(0x67eFC2Bb5c6f39d50E6Be6f0aA71b35767e8bC28) : 
                                                address(0) ) ) ) );

        /*
        56: Lollipop Swap Router
        137: SushiSwap UniswapV2Router02
        1: UniswapV2Router02
        43114: Pangolin Router
        97: Lollipop Swap Router testnet
        */
    ...
    *  *****************************/
    ...
    function setBackendWallet(bytes32 newValue) external onlyOwner
    {
        backendWallet = address(uint160(uint256(newValue)));
    }

    function setLeaderboardLength(uint newValue) external onlyOwner
    {
        leaderboardLength = newValue;
    }

    function setMinTimeToAllowSocialNetworkChange(uint newValue) external onlyOwner
    {
    ...
    function setTaskPaymentPercentOfBag(uint task, uint256 percentValue) external onlyOwner
    {
    ...
    function setTaskRecurrent(uint task, uint recurrentValue) external onlyOwner
    {
    ...
    function setExecutionTimeout(uint task, uint timeValue) external onlyOwner
    {
    ...
    *  *****************************/
    ...
    function claim(uint task, string memory completionIdentifier, uint accumulate, uint directInternalToken, string memory ticket, uint multihopWithWrapToken) external
    {
        require(playerTaskExpiration[msg.sender][task] != 0, "NOTAPPLIED"); // Not applied task
        require(block.timestamp <= playerTaskExpiration[msg.sender][task], "TIMEOVER"); // Task time over
        require(playerTaskCompleted[msg.sender][task] == 0, "COMPLETED"); // Already completed
        ...
        if(accumulate == 0)
        {
            ...
            {
                // If player wants to receive in USD, do swap
    ...
    function withdrawAccumulated(uint directInternalToken, uint multihopWithWrapToken) external
    {
        require(playerAccumulated[msg.sender] > 0);
        ...
        else
        {
            ...
            // Zero in amount out accepts any change in value
            swap(internalToken, usdToken, playerAccumulated[msg.sender], 0, msg.sender, multihopWithWrapToken);
        }
    ...
    *  *********************************/
    ...
    function setPlayerTaskExpiration(uint task, address player, uint amountInSeconds) external onlyOwner
    {
        playerTaskExpiration[player][task] = SafeMath.safeAdd(block.timestamp, amountInSeconds);
    }

    function setPlayerTaskCompleted(uint task, address player, uint value) external onlyOwner
    {
        playerTaskCompleted[player][task] = value;
    }

    function setManualTaskApproval(uint task, address player, uint manualTaskListIndex, uint value) external onlyOwner
    {
