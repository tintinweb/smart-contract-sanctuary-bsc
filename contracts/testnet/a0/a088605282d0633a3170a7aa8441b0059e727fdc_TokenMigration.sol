...
contract TokenMigration is Ownable, ReentrancyGuard {
    ...
    function estimateNewBalance() public view returns (uint256 estimatedBalance) {
        ...
        if(oldBalance > 0) {
            uint256 calculateNewBalance = oldBalance.mul(newSwapMultiple).div(newTokenDecimal);
    ...
    function swapBalanceToNewContract() public {
        ...
        if(oldBalance > 0) {
            ...
            require(balance >= calculateNewBalance, "BALANCE NOT ENOUGH");
            if(balance >= calculateNewBalance) {
                ...
                IERC20(CONTRACTV2).safeTransferFrom(address(this), msg.sender, calculateNewBalance);
                emit NewSwap(msg.sender, oldBalance, calculateNewBalance);
            }
        }
    }
