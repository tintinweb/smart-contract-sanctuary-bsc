...
contract ISL is IERC20, Ownable {
    ...
    string constant _name = "Island Token";
    string constant _symbol = "ISL";
    ...
    uint256 public buyH4gBurnFee = 1;
    ...
    uint256 public sellH4gBurnFee = 1;
    ...
    uint256 public marketingSwap = 1;
    uint256 public h4gBurnSwap = 1;
    ...
    function takeFee(
        ...
        if (to == pair) {
            ...
            if (sellStakingFee > 0)
                stakingAmount = feeAmount.mul(sellStakingFee).div(
                    sellTotalFees
                );
        } else {
            ...
            if (buyStakingFee > 0)
                stakingAmount = feeAmount.mul(buyStakingFee).div(buyTotalFees);
        }
        ...
        _balances[address(this)] = _balances[address(this)].add(
            feeAmount.sub(burnAmount).sub(stakingAmount)
        );
        emit Transfer(
            sender,
            address(this),
    ...
    function updateBuyFees(
        ...
        buyH4gBurnFee = h4gBurn;
        buyStakingFee = staking;

        buyTotalFees = reward.add(dev).add(liquidity).add(burn).add(marketing);
        buyTotalFees = buyTotalFees.add(h4gBurn).add(staking);

        require(buyTotalFees <= 30, "Fees can not greater than 30%");
    }
    ...
    function updateSellFees(
        ...
        sellH4gBurnFee = h4gBurn;
        sellStakingFee = staking;

        sellTotalFees = reward.add(dev).add(liquidity).add(burn).add(marketing);
        sellTotalFees = sellTotalFees.add(h4gBurn).add(staking);

        require(sellTotalFees <= 30, "Fees can not greater than 30%");
    }
    ...
    function updateSwapPercentages(
        ...
        totalSwap = reward.add(dev).add(liquidity).add(marketing).add(h4g);
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 tokensToBurnH4g = contractTokenBalance.mul(h4gBurnSwap).div(
            totalSwap
        );
        uint256 tokensToSwapFee = totalSwap.sub(liquiditySwap).sub(h4gBurnSwap);
        ...
        if (tokensToSwap > 0) {
            ...
            uint256 tokensToMarketing = swappedBusdBalance
                .sub(tokensToReward)
                .sub(tokensToDev);
        ...
        if (tokensToBurnH4g > 0) swapTokensAndBurn(tokensToBurnH4g, BURN);
    ...
    function swapTokensAndBurn(uint256 tokenAmount, address tokenToSwap)
        private
    {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](3);
