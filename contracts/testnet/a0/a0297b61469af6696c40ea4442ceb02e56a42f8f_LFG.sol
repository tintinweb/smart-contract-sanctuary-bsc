...
contract LFG is ERC20, Ownable {
            ...
            if(takeFee) {
                ...
                if(automatedMarketMakerPairs[from] && totalFeesBuy > 0){
                    ...
                    if(maxWalletLimitEnabled){
                        if (((balanceOf(address(from)) + amount.sub(fees)) > maxWalletLimit) && !limitExempt[from]) 
                        {
                    ...
                    if(maxTxAmountLimitEnabled){
                        if ((amount.sub(fees) > maxTxAmountLimit) && !limitExempt[from]) 
                        {
    ...
    function swapBack() private {
        ...
        (bool tmpSuccess,) = address(operationsWallet).call{value: address(this).balance}("");
