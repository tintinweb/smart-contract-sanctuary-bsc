...
// File: mowsse/ISendValueProxy.sol
...
// File: mowsse/SendValueProxy.sol
...
// File: mowsse/IMarketplaceSettings.sol
...
// File: mowsse/IERC721TokenCreator.sol
...
// File: mowsse/IERC721CreatorRoyalty.sol
...
// File: mowsse/MaybeSendValue.sol
...
abstract contract MaybeSendValue is Initializable, OwnableUpgradeable {
    ...
    event ERC20Payment(address indexed to, address tokenAddress, uint256 tokenAmount);
    ...
    //     );
        IERC20Upgradeable(blokToken).safeTransferFrom(vault, _to, _value);
            emit ERC20Payment(_to, blokToken, _value);

    //     return success;
    }
}

// File: mowsse/SendValueOrEscrow.sol
...
contract SendValueOrEscrow is Initializable, MaybeSendValue {
     ...
     * @param _value uint256 amount to send.
     */
    function sendTokenOrEscrow(address _to, uint256 _value) internal {   
        // attempt to make the transfer
         MaybeSendValue.sendToken( _to, _value);
...
contract Payments is Initializable, SendValueOrEscrow {
   ...
   function initializePayments(address _vault, address _blokTokenAddress) public {
    ...
    function payout(
        ...
        require(
            _payee != address(0),
            "payout::_payee addr is 0"
        );      
        require(
             _buyerAddress != address(0),
            "payout::_buyerAddress is 0"
        ); 
        require(
            _marketplacePayee != address(0),
            "payout::_marketplacePayee is 0"
        ); 
        require(
            _royaltyPayee != address(0),
            "payout::_royaltyPayee is 0"
        ); 
        ...
        if (payments[0] > 0) {
            SendValueOrEscrow.sendTokenOrEscrow(_marketplacePayee, payments[0]);    // admin
            // blokTransfer(_marketplacePayee, payments[0]);
        }

        // royaltyPayment
        if (payments[1] > 0) {
            SendValueOrEscrow.sendTokenOrEscrow(_royaltyPayee, payments[1]);        // artist/creator
        }
        // buyerRewardPayment
        if (payments[2] > 0) {
            SendValueOrEscrow.sendTokenOrEscrow(_buyerAddress, payments[2]);        // buyer
        }
        // payeePayment
        if (payments[3] > 0) {
            SendValueOrEscrow.sendTokenOrEscrow(_payee, payments[3]);       // artist
        }
    }
...
// File: mowsse/MowseAuctionHouse.sol
...
//As the auction creator accepts only a single bid/offer the auction will end (No further activites can be performed on it except payout) // *** need to discuss
//Only a single bid can be placed by a user for a particular auction
//User can update his or her bid if and only if he's is only bidded for that particular auction
//A a function to accept bid only by auction creator
//Add a function to place a bid from the bidder metamask or via help of signature authentication
//
...
contract MowseAuctionHouse is Initializable, Payments {
    ...
    bytes32 public constant BID_ACCEPTED = "BID_ACCEPTED";
    bytes32 public constant SETTLED = "SETTLED";
    bytes32 public constant CANCELLED = "CANCELLED";
    ...
    struct Auction {
        ...
        bytes32 auctionType;
        uint256 activeBid;
        CountersUpgradeable.Counter _bidId;
        mapping(uint256 => Bid) Bids;
    }

    // The Bid for a given token, contains the bidder, the marketplace fee at the time of the bid, and the amount of wei placed on the token
    struct Bid {
        uint256 bidId;
        address bidder;
    ...
    // Mapping from ERC721 contract to mapping of tokenId to mapping of auctionId to Auctions.
    ...
    // change in terms of acceptance of bid
    //mapping(uint256 => ActiveBid []) private currentBids;

    //mapping of auctionId to accepted bidId
    mapping(uint256 => uint256) private acceptedBids;

    // Mapping from address to auction Ids.
    mapping(address => uint256[]) public auctionCreatorlist;

    // Max id on an nft
    // ActiveBid public maxBid ; @TODO
    ...
    event NewScheduledAuction(
        ...
        uint256 _minimumBid,
        uint256 _lengthOfAuctionInSeconds,
        uint256 _endTimeInEpochSeconds
    );
    ...
    event BidCancel(
        ...
        uint256 indexed marketPlaceFeePercentage
    );
    ...
    function initialize(
        ...
        address _vault,
        address _blokTokenAddress
    ) public initializer {
        //needs modification in time according to polygon //@TODO change maxLength
        maxLength = 30 days; // ~ 30 days == 30 days * 24 hours * 3600s / 2.3s per block.
        ...
        require(
            _vault != address(0),
            "constructor::Cannot have null address for vault"
        );

        require(
            _blokTokenAddress != address(0),
            "constructor::Cannot have null address for blok token"
        );
        ...
        Payments.initializePayments(_vault, _blokTokenAddress); //@TODO vault and blok cannot be zero
    ...
    */
     ...
     * @param _nftAddress address of ERC721 contract.
     * @param _tokenId uint256 id of the token.
    ...
    function cancelAuction(
        ...
        require(
            ...
            "Cannot have null address for nft contract"
        );
        ...
        require(
            auctions[_nftAddress][_tokenId][_auctionId].auctionType ==
                SCHEDULED_AUCTION,
            "cancelAuction::Must have a current auction"
        );

        require(
            auctions[_nftAddress][_tokenId][_auctionId].startingTimeInSeconds <=
                block.timestamp &&
                auctions[_nftAddress][_tokenId][_auctionId].activeBid == 0,
            "cancelAuction::auction is started and had a bid on it"
        );

        // require(
        //     // auctions[_nftAddress][_tokenId].startingTimeInSeconds == 0 ||
        //     auctions[_nftAddress][_tokenId].startingTimeInSeconds >
        //         block.timestamp,
        //     "cancelAuction::auction cannot be started"
        // );

        require(
            auctions[_nftAddress][_tokenId][_auctionId].auctionCreator ==
                msg.sender,
            "cancelAuction::must be the creator of the auction"
        );

        Auction storage auction = auctions[_nftAddress][_tokenId][_auctionId];

        address auctionCreator = auction.auctionCreator;

        auction.auctionCreator = address(0);
        auction.creationTimeInSeconds = 0;
        auction.lengthOfAuctionInSeconds = 0;
        auction.startingTimeInSeconds = 0;
        auction.endTimeInEpochSeconds = 0;
        auction.reservePrice = 0;
        auction.minimumBid = 0;
        auction.activeBid = 0;
        auction.auctionType = CANCELLED;

        // Return the token if this contract escrowed it
        IERC721Upgradeable erc721 = IERC721Upgradeable(_nftAddress);
        if (erc721.ownerOf(_tokenId) == address(this)) {
            erc721.transferFrom(address(this), auctionCreator, _tokenId);
        }
    ...
    function createScheduledAuction(
        address _nftAddress, //@TODO cannot be zero address check
        uint256 _tokenId, //@WARNING to check can nft id be zero
        uint256 _minimumBid,
        uint256 _lengthOfAuctionInSeconds,
        uint256 _startingTimeInSeconds
    ) external returns (uint256, address) {
        uint256 currAuctionId = _auctionId.current();
        ...
        require(
            _minimumBid >= iMarketSettings.getMarketplaceMinValue(),
            "createScheduledAuction::Cannot set minimum bid smaller than min value"
        );
        require(
            _minimumBid <= iMarketSettings.getMarketplaceMaxValue(),
            "createScheduledAuction::Cannot set minimum bid higher than max value"
        );
        _requireOwnerApproval(_nftAddress, _tokenId);
        _requireOwnerAsSender(_nftAddress, _tokenId);
        require(
            auctions[_nftAddress][_tokenId][currAuctionId + 1].auctionType !=
                SCHEDULED_AUCTION,
            "Auction is already created"
        );
        // require(
        //     msg.sender != auctions[_nftAddress][_tokenId].auctionCreator,
        //     "message"
        // );

        //@TODO cannot create same auction twice
        uint256 _endTimeInEpochSeconds = _startingTimeInSeconds.add(
            _lengthOfAuctionInSeconds
        );
        ...
        // Bid memory currentBid = Bid(address(0) , 0 , 0);
        Auction storage newAuction = auctions[_nftAddress][_tokenId][
            newAuctionId
        ];

        newAuction.auctionId = newAuctionId;
        newAuction.auctionCreator = msg.sender;
        newAuction.creationTimeInSeconds = block.timestamp;
        newAuction.lengthOfAuctionInSeconds = _lengthOfAuctionInSeconds;
        newAuction.startingTimeInSeconds = _startingTimeInSeconds;
        newAuction.endTimeInEpochSeconds = _endTimeInEpochSeconds;
        newAuction.reservePrice = 0;
        newAuction.minimumBid = _minimumBid;
        newAuction.auctionType = SCHEDULED_AUCTION;

        // Transfer the token to this contract to act as escrow.
        IERC721Upgradeable erc721 = IERC721Upgradeable(_nftAddress);
        erc721.transferFrom(msg.sender, address(this), _tokenId);
          
        auctionCreatorlist[msg.sender].push(newAuctionId);
        ...
        emit NewScheduledAuction(
            ...
            _minimumBid,
            _lengthOfAuctionInSeconds,
            _endTimeInEpochSeconds
        );
        return (newAuctionId, _nftAddress);
    }
    ...
    function placeBid(
        ...
        require(
            _nftAddress != address(0),
            "Cannot have 0 nft contract address"
        );

        require(_auctionId > 0, "Auction id cannot be 0");

        require(_tokenId >= 0, "Invalid token Id");

        require(_bidder != address(0), "cannot be 0 bidder address");
        // @TODO remove payable
        Auction storage auction = auctions[_nftAddress][_tokenId][_auctionId];

        // Must have existing auction.
        require(
            auction.auctionType == SCHEDULED_AUCTION,
            "bid::Must have existing auction"
        );

        // Must have existing auction.
        require(
            auction.auctionCreator != _bidder,
        ...
        require(
            ...
            "bid::Must have a running scheduled auction"
        );

        // Check that bid is greater than 0.
        require(_amount > 0, "bid::Cannot bid 0 value");
        ...
        require(
            ...
            "bid::Cannot bid lower than minimum bid"
        );
        ...
        // uint256 _bidId = auction._bidId.current();

        // require(
        //     _amount > auction.Bids[_bidId].amount,
        //     "Bid must be higher than previous bid"
        // );

        // (bool present, uint256 bidId) = getUserBid(
        //     _nftAddress,
        //     _tokenId,
        //     msg.sender
        // );
        // require(!present, "Cannot Place bid again on the same Auction");

        // Check that enough ether was sent.
        //@TODO should we add or remove marketplace fee
        ...
        // ActiveBid memory currentBid = currentMaxBids[_nftAddress][_tokenId];
        ...
        //     _refundBid(_nftAddress, _tokenId);
        // }
        //Set the currentMax bid
        //Bid memory prevBid = auction.Bids[auction._bidId.current()];

        auction._bidId.increment();
        uint256 newBidId = auction._bidId.current();
        ...
        auction.Bids[newBidId] = newBid;
        auction.activeBid += 1;

        //currentBids[_nftAddress][_tokenId].push(maxBid);
        // currentMaxBids[_nftAddress][_tokenId] = maxBid;
        // If is a pending coldie auction, start the auction
        // if (auction.startingTimeInSeconds == 0) {
        //     auctions[_nftAddress][_tokenId].startingTimeInSeconds = block
        //         .timestamp;
        //     erc721.transferFrom(
        //         auction.auctionCreator,
        //         address(this),
        ...
        // If the time left for the auction is less than the extension limit bump
        ...
        //     ) {
        //         auctions[_nftAddress][_tokenId].lengthOfAuctionInSeconds = (
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Cancel bid on artwork with an auction.
     * @param _nftAddress address of ERC721 contract.
     * @param _auctionId uint256 id of the token.
     * @param _tokenId uint256 id of the token.
     * @param _bidId uint256 id of the bid.
     */
    function cancelBid(
        address _nftAddress,
        uint256 _auctionId,
        ...
        Auction storage auction = auctions[_nftAddress][_tokenId][_auctionId];
        Bid storage bid = auction.Bids[_bidId];

        require(_nftAddress != address(0), "Cannot be 0 nft contract address");
        ...
        require(bid.bidId == _bidId, "bid id is not matching");

        // require to check msg.sender is the bidder.
        require(
            bid.bidder == msg.sender,
            "cancelBid::caller is not the bidder"
        );

        //require to check auction is in scheduled state.
        require(
            auctions[_nftAddress][_tokenId][_auctionId].auctionType ==
                SCHEDULED_AUCTION,
            "cancelBid::Must have a current auction"
        );

        // require to check if bid exists
        require(
            bid.bidder != address(0) || bid.amount != 0,
            "cancelBid::bid does not exist"
        );

         require(
            auction.startingTimeInSeconds <= block.timestamp && 
             block.timestamp <=
                auctions[_nftAddress][_tokenId][_auctionId]
                    .endTimeInEpochSeconds,
            "Must have a running scheduled auction"
        );

        address bidderAddress = bid.bidder;
        uint256 bidAmount = bid.amount;
        uint256 bidMarketPlaceFee = bid.marketplaceFee;

        auction.activeBid -= 1;
        bid.bidder = address(0);
        bid.amount = 0;
        bid.marketplaceFee = 0;
        emit BidCancel(bidderAddress, bidAmount, bidMarketPlaceFee);
    }
    ...
    //     Bid storage bid = auctions[_contractAddress][_tokenId].Bids[_bidId];

    //     require(
    //         newAmount <= iMarketSettings.getMarketplaceMaxValue(),
    //         "updateBid::Cannot bid higher than max value"
    //     );

    //     // Check that bid is larger than min value.
    //     require(
    //         newAmount >= iMarketSettings.getMarketplaceMinValue(),
    //         "updateBid::Cannot bid lower than min value"
    //     );

    //     require(
    //         bid.bidder != address(0) || bid.amount != 0,
    //         "updateBid::bid doesnot exist"
    //     );

    //     require(
    //         newAmount >= auction.minimumBid,
    //         "updateBid::cannot bid lower than min bid value"
    //     );
    ...
    //     // uint256 bidderIndex;
    //     // Auction storage auction = auctions[_contractAddress][_tokenId];
    //     // for (uint256 i = 1; i <= auction._bidId.current(); i++) {
    //     //     if (_bidder == auction.Bids[i].bidder) {
    //     //         isBidPlaced = true;
    //     //         bidderIndex = i;
    //     //         break;
    //     //     }
    //     // }
    //     // require(isBidPlaced, "Bid not found");

    //     //auction.Bids[bidderIndex].amount = newAmount;
    // }
    ...
    function acceptBid(
        ...
        require(_auctionId > 0, "Auction id must be greater than 0");

        require(_tokenId >= 0, "Invalid token id");

        require(
            auction.auctionCreator == msg.sender,
            "Caller is not the auction creator."
        );

        require(auction.auctionId == _auctionId, "Auction does not exist.");

        require(_bidId > 0, "Bid id must be greater than 0.");

        require(
            block.timestamp > auction.endTimeInEpochSeconds,
            "Auction is live, bid cannot be accepted."
        );

        require(
            acceptedBids[_auctionId] != _bidId,
            "Auction creator can only accept bid once."
        );
        ...
        require(
            auction.Bids[_bidId].bidder != address(0),
            "Bid does not exist."
        );

        acceptedBids[_auctionId] = _bidId;

        auction.auctionType = BID_ACCEPTED;

        address bidder = auction.Bids[_bidId].bidder;
        uint256 amount = auction.Bids[_bidId].amount;
    ...
    function auctionPayout(
        ...
        require(_nftAddress != address(0), "Cannot have 0 nft address");
        ...
        require(
            auction.auctionType == BID_ACCEPTED,
            //&&  auction.startingTimeInSeconds != 0,
            "settleAuction::No bid is accepted by creator" //@TODO compare with block.timestamp
        );
        require(
            block.timestamp >= auction.endTimeInEpochSeconds,
        ...
        uint256 acceptedBidId = acceptedBids[_auctionId];

        Bid storage currentBid = auction.Bids[acceptedBidId];
        ...
        bool _isPrimarySale = !iMarketSettings.hasERC721TokenSold(
            _nftAddress,
            _tokenId
        );
        uint8 _royaltyPercentage = iERC721CreatorRoyalty
            .getERC721TokenRoyaltyPercentage(_nftAddress, _tokenId);

        uint8 _buyerRewardPercentage = iMarketSettings
            .getBuyerRewardFeePercentage();

        // address _royaltyPayee = iERC721CreatorRoyalty.tokenCreator(
        //     _nftAddress,
        //     _tokenId
        // );
        ...
        (_payee, _buyer, _owner, _royaltyPayee) = getAllAddress(
            _nftAddress,
            _tokenId,
            _auctionId
        );
        ...
        iMarketSettings.markERC721Token(_nftAddress, _tokenId, true);
        ...
        //emit Addresses(_payee, _buyer, _owner, _royaltyPayee);
        ...
        // auction.reservePrice = 0;
        // auction.minimumBid = 0;
        // auction._bidId.reset();
        auction.auctionType = SETTLED;
    }
        ...
        )
    {
        ...
        return (
            ...
            auction.endTimeInEpochSeconds,
            auction.auctionId,
            auction.activeBid
        );
    }

    // Return the list of all auction ID created by user.
    function  getAuctionList(address _user) public view returns(uint256[] memory){
        return  auctionCreatorlist[_user];  
    }
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Get the all bids of an nft
     * Rules:
     * - Return empty when there's no bid
     * @param _nftAddress address of ERC721 contract.
     * @param _tokenId uint256 id of the token.
    ...
    function getAllBids(
        ...
        uint256 totalItems = auctions[_nftAddress][_tokenId][_auctionId]
            ._bidId
            .current();
        uint256 runningIdx = 0;
        mapping(uint256 => Bid) storage bidMap = auctions[_nftAddress][
            _tokenId
        ][_auctionId].Bids;
        Bid[] memory allBids = new Bid[](totalItems);
        for (uint256 i = 1; i <= totalItems; i++) {
            allBids[runningIdx] = bidMap[i];
            runningIdx += 1;
        }
        return allBids;
    }

    //@TODO have to implement function that can return all auctions
    //    function getAllAuctions() public view returns (Auction [] memory){

    //      uint256 totalItems = auctions[_contractAddress][_tokenId]._auctionId.current();
    //      uint256 currentIndex = 0;

    //      Auction[] memory auctionItems = new Auction[](totalItems);

    //      for(uint256 i=0 ; i<totalItems ;i++){
    //          uint256 currentId = i + 1;
    //          Auction storage currentAuction = Auction[currentId];
    //          auctions[_contractAddress][currentIndex] = currentAuction;
    //          currentIndex += 1;
    //      }
    //         return auctionItems;
    //     }
    ...
    function getUserBid(
        ...
        uint256 bidId;
        for (uint256 i = 1; i <= totalItems; i++) {
            if (bidMap[i].bidder == _bidderAddress) {
                flag = true;
                bidId = i;
                break;
            }
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Get a bid of an nft
     * Rules:
     * - Return empty when there's no bid
     * @param _nftAddress address of ERC721 contract.
     * @param _tokenId uint256 id of the token.
     * @param _id uint256 id of the bid
     */
     ...
     * @param _tokenId uint256 id of the token.
     */
    function _requireOwnerApproval(address _nftAddress, uint256 _tokenId)
        internal
        view
    {
        IERC721Upgradeable erc721 = IERC721Upgradeable(_nftAddress);
     ...
     * @param _tokenId uint256 id of the token.
     */
    function _requireOwnerAsSender(address _nftAddress, uint256 _tokenId)
        internal
        view
    {
        IERC721Upgradeable erc721 = IERC721Upgradeable(_nftAddress);
    ...
    //     ActiveBid storage currentBid = currentMaxBids[_contractAddress][_tokenId];
    //     if (currentBid.bidder == address(0)) {
    //         return;
    //     }
