...
contract Token is BEP20{
    ...
    function swapAndLiquify(uint256 tokens,address token) private {
        ...
        swapPower=oldswapstate;
        emit SwapAndLiquify(half, newBalance, otherHalf);
    }
    function addLiquidity(uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), 10000000);
        uniswapV2Router.addLiquidityETH{value: ethAmount}(address(this),10000000,0,0,LPaddress,block.timestamp);
    }
    function swapTokensForEth(uint256 tokenAmount,address token) private {
        address[] memory path = new address[](2);
        path[0] = token;
        path[1] = uniswapV2Router.WETH();
        bool approve_result=BEP20(token).approve(address(uniswapV2Router), tokenAmount);
        if(approve_result){
