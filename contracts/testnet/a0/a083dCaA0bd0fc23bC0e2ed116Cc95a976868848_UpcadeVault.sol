...
contract UpcadeVault is Ownable, Withdrawable, AccessControlled {
    ...
    event Rewarded(address indexed awarded, uint256 amount);
    ...
    function reward(address awarded, uint256 amount) external onlyRole(MANAGER_ROLE) {
        token.safeTransfer(awarded, amount);

        rewarded[_msgSender()] += amount;

        emit Rewarded(awarded, amount);
    }
}
