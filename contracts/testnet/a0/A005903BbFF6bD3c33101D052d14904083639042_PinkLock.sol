...
contract PinkLock is IPinkLock, OwnableUpgradeable, UUPSUpgradeable {
    ...
    uint256 public vivafee;
    uint256 public tokenfee;
    address public feeAddress;
    address public vivatoken;
    ...
    constructor() {
        ...
        vivatoken = address(0xC18083cc09dA5fb3E29Ae39e186A817EFcE6CB3f);
    }
    ...
    function setTokenFee(uint256 newFee) external onlyOwner {
        tokenfee = newFee;
    }
    ...
    function setVivaFee(uint256 newFee) external onlyOwner {
        vivafee = newFee;
    }

    function setFeeAddress(address newaccount) external onlyOwner {
        feeAddress = newaccount;
    }

    function setVivaTokenAddress(address token) external onlyOwner {
        vivatoken = token;
    }
    ...
    function lock(
        ...
        require(feetype == 1 && msg.value == bnbfee, "Fee must pay");
        if (isLpToken) {
        ...
        payfee(msg.sender, feetype);
        IERC20Upgradeable(token).safeTransferFrom(
            msg.sender,
            feeAddress,
            (tokenfee * amount) / 1000
        );
    ...
    function payfee(address from, uint256 feetype) internal {
        if (feetype == 1) payable(feeAddress).sendValue(bnbfee);
        if (feetype == 2)
            IERC20Upgradeable(vivatoken).safeTransferFrom(
                from,
                feeAddress,
                vivafee
            );
