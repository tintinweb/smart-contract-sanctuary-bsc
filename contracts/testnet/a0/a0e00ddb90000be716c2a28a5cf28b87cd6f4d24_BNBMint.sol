...
contract BNBMint is Ownable, ReentrancyGuard {
    ...
    function _referrals(uint _package, address _user, uint256 _amount, uint256 _type) internal {
        ...
        for(uint i = 0; i < 5; i++) {
            ...
            if((user.deposits[0].isExist && user.deposits[0].amount > 0 ) || (user.deposits[1].isExist && user.deposits[1].amount > 0 ) || (user.deposits[2].isExist && user.deposits[2].amount > 0)) {
			    uint256 fee;
			    if(_type==2)
				{
    ...
    function deposit(address _ref, uint256 _pa) external nonReentrant payable {
        ...
        if(((sponsor.deposits[0].isExist && sponsor.deposits[0].amount > 0 ) || (sponsor.deposits[1].isExist && sponsor.deposits[1].amount > 0 ) || (sponsor.deposits[2].isExist && sponsor.deposits[2].amount > 0)) && firstDeposit[_ref][msg.sender] == false) 
		{
        ...
        if(user.deposits[_pa].isExist && user.deposits[_pa].amount > 0 && this.pendingReward(msg.sender,_pa) > 0) 
		{
		...
		user.deposits[_pa].isExist = true;
    }
    ...
    function withdraw(address _user) external nonReentrant {
        ...
        if(user.deposits[0].isExist && user.deposits[0].amount > 0 && pendingOne > 0) 
		{
		    _claim(_user, 0);
		}
		
		if(user.deposits[1].isExist && user.deposits[2].amount > 0 && pendingTwo > 0) 
		{
		    _claim(_user, 1);
		}
		
		if(user.deposits[2].isExist && user.deposits[2].amount > 0 && pendingThree > 0) 
		{
		    _claim(_user, 2);
		}
    }
    ...
    function pendingReward(address _user, uint256 _pa) external view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        packagesStruct storage package = packages[_pa];
        if(user.deposits[_pa].isExist && user.deposits[_pa].amount > 0 && user.deposits[_pa].lastClaim > 0 && user.deposits[_pa].nextWithdraw <= block.timestamp){
