                    ...
                    uint256(
                        keccak256(
                            abi.encodePacked(
                                ...
                                hex"5126b4d0b8ee9378aa2b960950d298024c7d66f558e9e803492c0083f7ee65f7" // init code hash
                            )
                        )
                    )
                )
...
contract CrossRouter is ICrossRouter02 {
    ...
    string private constant error_wrongPath = "Crosswise Router: INVALID_PATH ";
    string private constant error_wrongTokenRatio =
        "Crosswise Router: Wrong token ratio";
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, error_wrongTokenRatio);
                ...
                require(amountAOptimal >= amountAMin, error_wrongTokenRatio);
        ...
        )
    {
        ...
        address pair = CrossLibrary.pairFor(factory, token, WBNB);
    ...
    function swapExactETHForTokens(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[0] == WBNB, error_wrongPath);
    ...
    function swapTokensForExactETH(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[path.length - 1] == WBNB, error_wrongPath);
        amounts = CrossLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax);
        TransferHelper.safeTransferFrom(
            path[0],
