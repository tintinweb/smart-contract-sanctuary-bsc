...
contract SpringMarketPlace is OwnersUpgradeable, ReentrancyGuardUpgradeable {
    struct Offer {
        address owner;
        uint256 tokenId;
        uint256 price;
    ...
    struct MapOffer {
        uint256[] keys; // tokenId to offers
        mapping(uint256 => Offer) values;
        mapping(uint256 => uint256) indexOf;
        mapping(uint256 => bool) inserted;
    }

    struct MapAuction {
        uint256[] keys; // tokenId to auctions
        mapping(uint256 => Auction) values;
        mapping(uint256 => uint256) indexOf;
        mapping(uint256 => bool) inserted;
    }
    ...
    uint256 public auctionBeforeRef;
    uint256 public auctionBeforeAdd;

    function initialize(
        address _spring,
        address _swapper,
        uint256 _fee,
        uint256 _auctionBeforeRef,
        uint256 _auctionBeforeAdd
    ) external initializer {
        __SpringMarketPlace_init(
            _spring,
            _swapper,
            _fee,
            _auctionBeforeRef,
    ...
    function __SpringMarketPlace_init(
        address _spring,
        address _swapper,
        uint256 _fee,
        uint256 _auctionBeforeRef,
        ...
        __SpringMarketPlace_init_unchained(
            _spring,
            _swapper,
            _fee,
            _auctionBeforeRef,
    ...
    function __SpringMarketPlace_init_unchained(
        address _spring,
        address _swapper,
        uint256 _fee,
        uint256 _auctionBeforeRef,
    ...
    function setMinPrices(
        ...
        uint256[] memory _auctionPrices
    ) external onlyOwners {
        require(
            _names.length == _offerPrices.length,
            "SpringMarketPlace: Length mismatch"
        );
        require(
            _names.length == _auctionPrices.length,
            "SpringMarketPlace: Length mismatch"
        );
    ...
    function deleteMapPrice(address _nft, string[] memory names)
        external
        onlyOwners
    {
    ...
    function sellOfferItem(
        ...
        require(
            !isBlacklistedNft[_nft],
            "SpringMarketPlace: Not authorized contract"
        );
        ...
        if (nftPrices.inserted[_nft]) {
            ...
            MapPrices storage mapPrices = nftPrices.values[_nft];
            require(
                mapPrices.inserted[name],
                "SpringMarketPlace: Contact core team"
            );
            require(
                mapPrices.values[name].offerPrice <= _price,
                "SpringMarketPlace: Price lower than min price"
            );

            if (onlyAttribute[_nft]) {
                string memory attribute = ISpringNft(_nft).getAttribute(
                    _tokenId
                );
                require(
                    bytes(attribute).length > 0,
    ...
    function purchaseOfferItem(address _nft, uint256 _tokenId)
        external
        nonReentrant
    {
        require(
            nft.inserted[_nft],
            "SpringMarketPlace: Nft contract is not setup"
        );
        require(
            nft.values[_nft].mapOffer.inserted[_tokenId],
            "SpringMarketPlace: Item doesnt exist"
        );
        ...
        require(
            ...
            "SpringMarketPlace: You cannot buy your own nft"
        );

        uint256 feePrice = (offer.price * fee) / 10000;

        IERC20(spring).transferFrom(
            msg.sender,
            offer.owner,
            offer.price - feePrice
        );
        if (feePrice > 0)
    ...
    function recoverOfferItem(address _nft, uint256 _tokenId)
        external
        nonReentrant
    {
        require(
        ...
        require(
            offer.owner == msg.sender || isOwner[msg.sender],
            "SpringMarketPlace: Unauthorized"
        );
    ...
    function sellAuctionItem(
        ...
        if (nftPrices.inserted[_nft]) {
            ...
            require(
                mapPrices.values[name].auctionPrice <= _currentPrice,
                "SpringMarketPlace: Price lower than min price"
            );
    ...
    function purchaseAuctionItem(
        ...
        require(
            nft.values[_nft].mapAuction.inserted[_tokenId],
            "SpringMarketPlace: Item doesnt exist"
        );
        ...
        require(
            ...
            "SpringMarketPlace: Auction already finished"
        );
        require(
    ...
    function recoverAuctionItem(address _nft, uint256 _tokenId)
        external
        nonReentrant
    {
        require(
        ...
        require(
            auction.nextOwner == msg.sender || isOwner[msg.sender],
            "SpringMarketPlace: Unauthorized"
        );
        require(
            block.timestamp >= auction.end,
            "SpringMarketPlace: Auction not finished yet"
        );

        if (auction.owner != auction.nextOwner) {
            uint256 feePrice = (auction.currentPrice * fee) / 10000;

            IERC20(spring).transfer(
                auction.owner,
                auction.currentPrice - feePrice
            );
            if (feePrice > 0) IERC20(spring).transfer(swapper, feePrice);
        }
    ...
    function forceAuctionEnd(address _nft, uint256 _tokenId)
        external
        onlyOwners
    {
        require(
    ...
    function setFee(uint256 _new) external onlyOwners {
        fee = _new;
    }
    ...
    //external view
    // map nft
    function getNftSize() external view returns (uint256) {
        return nft.keys.length;
    }

    function getNftAddressBetweenIndexes(uint256 iStart, uint256 iEnd)
        external
        view
    ...
    // map offer
    function getOfferOfSize(address _nft) external view returns (uint256) {
    ...
    // map auction
    function getAuctionOfSize(address _nft) external view returns (uint256) {
    ...
    // map nft price
    function getNftPricesSize() external view returns (uint256) {
        return nftPrices.keys.length;
    }

    function getNftPricesAddressBetweenIndexes(uint256 iStart, uint256 iEnd)
        external
        view
        returns (address[] memory)
    {
    ...
    // map prices
    function getPricesOfSize(address _nft) external view returns (uint256) {
    ...
    function getPricesOfBetweenIndexes(
        ...
        uint256 iStart,
        uint256 iEnd
    ) external view returns (MinPrices[] memory) {
    ...
    function mapOfferRemove(MapOffer storage map, uint256 key) private {
        if (!map.inserted[key]) {
            return;
        }
    ...
    function mapAuctionRemove(MapAuction storage map, uint256 key) private {
        if (!map.inserted[key]) {
            return;
        }
    ...
    function mapNftAdd(address key) private {
        if (nft.inserted[key]) {
            return;
        } else {
    ...
    function mapNftDelete(address key) private {
        if (!nft.inserted[key]) {
            return;
        } else {
            uint256 sizeOffer = nft.values[key].mapOffer.keys.length;
            uint256 sizeAuction = nft.values[key].mapAuction.keys.length;
            if (sizeOffer == 0 && sizeAuction == 0) {
                ...
                if (lastIndex != index) nft.keys[index] = lastKey;
                nft.keys.pop();
            }
        }
    }
    ...
    function mapNftPricesAdd(address key) private {
    ...
    function mapNftPricesDelete(address key) private {
        if (!nftPrices.inserted[key]) {
            return;
        } else {
            uint256 sizePrices = nftPrices.values[key].keys.length;
            if (sizePrices == 0) {
                ...
                if (lastIndex != index) nftPrices.keys[index] = lastKey;
                nftPrices.keys.pop();
            }
        }
    }
