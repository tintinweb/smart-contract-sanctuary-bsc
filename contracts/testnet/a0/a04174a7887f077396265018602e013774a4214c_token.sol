...
contract token is ERC20 {
    ...
    //name,symbol
    string internal tokenName = "LUNA DAO";
    string internal tokenSymbol = "LUNA DAO";
    ...
    IERC20 public RewardTokenTwo = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); //busd
    ...
    function _transfer(
        ...
        if(startTime.add(endtime) > block.timestamp){
            require(amount <= 10 * 10**18,"max 10");
            require(balanceOf(to).add(amount) <= 30 * 10**18,"max 30");
        }
        ...
        if (takeFee) {
            super._transfer(from, _marketWallet, amount.div(100).mul(1));
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
        uint256 usdtNumber = contractTokenBalance.div(100).mul(40);
        uint256 twoNumber = contractTokenBalance.div(100).mul(40);
        swapTokensForOther(usdtNumber);
        swapTokensForOtherTwo(twoNumber);
    }
    ...
    function swapTokensForOtherTwo(uint256 tokenAmount) public {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(RewardToken);
        path[2] = address(RewardTokenTwo);
    ...
    function _splitHolderToken() public {
        uint256 thisAmount = RewardTokenTwo.balanceOf(address(this));
        if(thisAmount >= 10**18){
            uint256 splitTwo = thisAmount;
            _splitHolderTokenSecond(splitTwo);
        }
    }

    uint256 private holderindex;
    function _splitHolderTokenSecond(uint256 thisAmount) private {
        uint256 holderSize = holderUser.length;
        if(holderSize>0){
            address user;
            ...
            if(holderSize >20){
                for(uint256 i=0;i<20;i++){
                    holderindex = holderindex.add(1);
                    if(holderindex >= holderSize){holderindex = 0;}
                    user = holderUser[holderindex];
                    if(balanceOf(user) >= 0){
                        rate = balanceOf(user).div(totalAmount).mul(100);
                        uint256 amountReward = thisAmount.mul(rate);
                        if(amountReward>10**13){
                            RewardTokenTwo.transfer(user,amountReward);
                        }
                    }
                }
            }else{
                for(uint256 i=0;i<holderSize;i++){
                    user = holderUser[i];
                    if(balanceOf(user) >= 0){
                        rate = balanceOf(user).div(totalAmount).mul(100);
                        uint256 amountReward = thisAmount.mul(rate);
