...
/**
 * @title ReflexMigrator
 * @author Ruban
 * @notice You can use this contract for only reflex migration
 ...
 * ```
 */
contract ReflexMigrator is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  */
   ...
   */
  function setNewToken(address newToken) external onlyOwner whenNotPaused {
     _newToken = IERC20Upgradeable(newToken);
  }
   ...
   */
  function setLegacyToken(address legacyToken) external onlyOwner whenNotPaused {
     _legacyToken = IERC20Upgradeable(legacyToken);
  }
  ...
  function migrate(address account, uint256 amount) public {
    ...
    _legacyToken.transferFrom(account, address(this), amount);
    uint256 v2TokenTransferAmount = amount * 10 ** 9;
    _newToken.transfer(account, v2TokenTransferAmount);
    emit Migration(account, v2TokenTransferAmount);
