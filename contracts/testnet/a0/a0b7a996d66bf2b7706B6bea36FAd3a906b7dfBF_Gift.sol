...
contract Gift {
    ...
    mapping(uint => mapping(uint => record)) giftRecords;

    function getGiftRecord(uint chainId, uint internalId) public view returns (record memory){
        return giftRecords[chainId][internalId];
    }
    ...
    function putOnGift(uint chainId, uint internalId, uint time, address receiver) public currentOwner(chainId, internalId) returns (bool){
    	...
    	record storage ptr = giftRecords[chainId][internalId];
    	...
    	ptr.time = time + block.timestamp;
    	ptr.receiver = receiver;

        return true;
    }

    function cancelGift(uint chainId, uint internalId) public currentOwner(chainId, internalId) returns (bool){
    ...
    function gift(uint chainId, uint internalId) public returns (bool){
    	record storage ptr = giftRecords[chainId][internalId];
    	require(ptr.time != 0, "Not for gift");
    	require(ptr.time < block.timestamp, "still waiting");
