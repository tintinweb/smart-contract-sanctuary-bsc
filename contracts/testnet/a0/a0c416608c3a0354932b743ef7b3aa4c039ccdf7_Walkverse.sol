...
/**
 * @title Walkverse token contract
 * @dev The Walkverse token has modular systems for tax and treasury handler as well as governance capabilities.
 */
contract Walkverse is IERC20, IGovernanceToken, Ownable {
    ...
    function transferFrom(
        ...
        require(
            ...
            "Walkverse:transferFrom:ALLOWANCE_EXCEEDED: Transfer amount exceeds allowance."
        );
        unchecked {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external returns (bool) {
        ...
        require(
            ...
            "Walkverse:decreaseAllowance:ALLOWANCE_UNDERFLOW: Subtraction results in sub-zero allowance."
        );
        unchecked {
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "Walkverse:delegateBySig:INVALID_SIGNATURE: Received signature was invalid.");
        require(block.timestamp <= expiry, "Walkverse:delegateBySig:EXPIRED_SIGNATURE: Received signature has expired.");
        require(nonce == nonces[signatory]++, "Walkverse:delegateBySig:INVALID_NONCE: Received nonce was invalid.");
    ...
    function getVotesAtBlock(address account, uint32 blockNumber) public view returns (uint224) {
        require(
            ...
            "Walkverse:getVotesAtBlock:FUTURE_BLOCK: Cannot get votes at a block in the future."
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "Walkverse:_approve:OWNER_ZERO: Cannot approve for the zero address.");
        require(spender != address(0), "Walkverse:_approve:SPENDER_ZERO: Cannot approve to the zero address.");
    ...
    function _transfer(
        ...
        require(from != address(0), "Walkverse:_transfer:FROM_ZERO: Cannot transfer from the zero address.");
        require(to != address(0), "Walkverse:_transfer:TO_ZERO: Cannot transfer to the zero address.");
        require(amount > 0, "Walkverse:_transfer:ZERO_AMOUNT: Transfer amount must be greater than zero.");
        require(amount <= _balances[from], "Walkverse:_transfer:INSUFFICIENT_BALANCE: Transfer amount exceeds balance.");
