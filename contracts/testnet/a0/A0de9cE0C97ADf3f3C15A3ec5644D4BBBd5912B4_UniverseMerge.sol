...
import "./interfaces/ITypeNft.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract UniverseMerge is EIP712, Ownable {
    using SafeMath for uint256;
    address public tokenIn;
    address public signAddress;
    address public poolAddress;
    IERC20Burnable public token;
    ITypeNft public universe;
    ...
    bytes32 public constant _TYPEHASH =
        keccak256(
            "Merge(address user,uint256 nft1,uint256 nft2,uint256 amount,uint256 typeId,uint256 deadline,uint256 nonce)"
        );

    constructor() EIP712("Merge", "1.0") {}
    ...
    function nowMerge(
        ...
        universe.burn(nft2);
        _pay(amount);
        uint256 newNft = universe.mint(msg.sender, typeId);
        nonces[user]++;
        emit Merge(user, nft1, nft2, amount, newNft, typeId);
        return true;
    }
      ...
      function _pay(
        ...
        uint256 rewardAmount = amount.mul(500).div(1000);
        uint256 burnAmount = amount.mul(400).div(1000);
        token.transferFrom(msg.sender,address(this), amount);
        token.transferFrom(address(this), tokenIn, feeAmount);
        token.transferFrom(address(this), poolAddress, rewardAmount);
