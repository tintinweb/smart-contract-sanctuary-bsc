...
contract GoenSchedule is IGoenSchedule, OwnableUpgradeable, CoreUtility {
    using SafeMath for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event RateChanged(uint256 oldRate, uint256 newRate, uint256 timeUpdate);
    event Harvested(address pool, uint256 timestamp);

    IERC20Upgradeable public token;
    IVotingEmission public votingEmission;
    uint256 public rate;
    uint256 public lastTimeUpdateRate;
    address public operator;

    function initialize(address token_, address votingEmission_, uint256 totalFarmingReward_, uint256 maxtime_, address operator_) public initializer {
        __Ownable_init();
        token =  IERC20Upgradeable(token_);
        votingEmission = IVotingEmission(votingEmission_);
        rate = totalFarmingReward_ * 1e18 / maxtime_;
        lastTimeUpdateRate = block.timestamp;
        operator = operator_;
    }
    ...
    function removeToken(uint256 amount) public onlyOwner returns (uint256) {
    ...
    function setRate(uint256 newRate) public {
        ...
        require(newRate != rate, "rate already set");
        uint256 size = votingEmission.poolSize();
        address[] memory pools = votingEmission.getPools();
        uint256 oldRate = rate;
        for (uint256 i = 0; i < size; i++) {
            IStrategy(pools[i]).updateGoenReward();
        }
        lastTimeUpdateRate = block.timestamp;
        rate = newRate;
        emit RateChanged(oldRate, newRate, lastTimeUpdateRate);
    }

    function harvest() external override {
        require (_poolExisted(msg.sender), "caller is not pool");
        address pool = msg.sender;
        IStrategy(pool).updateGoenReward();
        emit Harvested(pool, block.timestamp);
    }

    function rates(address pool, uint256 timestamp) public override view returns (uint256) {
        uint256 weekCursor = _endOfWeek(timestamp).sub(System.SYSTEM_INTERVAL);
        return votingEmission.weightsPerWeek(weekCursor, pool).mul(rate).div(1e18);
    }

    function getGoen(address receiver, uint256 reward) override external returns (uint256) {
        require (_poolExisted(msg.sender), "caller is not pool");
        token.safeTransfer(receiver, reward);
        return reward;
    }

    function _poolExisted(address pool) internal view returns (bool) {
        uint256 size = votingEmission.poolSize();
        address[] memory pools = votingEmission.getPools();
        for (uint256 i = 0; i < size; i++) {
            if(pools[i] == pool) {
