...
contract DreamFarmFish is INft, Context, ERC165, IERC721, IERC721Metadata, IERC721Receiver, Ownable {
    ...
    function mintAndPricing(uint256 _num, uint256 _limit, uint256 _time) public onlyOwner {
    ...
    function offload(uint256 _tokenId,address receiver) public {
        require(_exists(_tokenId), "Vsnft_setTokenAsset_notoken");
        address sender = _msgSender();
        require(_onSaleList[_tokenId] == sender || ownerOf(_tokenId) == sender, "Invalid_Owner");
    ...
    function mintToAddress(address to, uint amount, uint adv_time) public onlyOwner {
