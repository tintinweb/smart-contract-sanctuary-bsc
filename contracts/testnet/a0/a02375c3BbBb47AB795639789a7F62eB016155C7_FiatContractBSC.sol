...
contract FiatContractBSC is Ownable {
    ...
    function setTokenPrice(string[] memory _symbols, address[] memory _assets, uint256[] memory _prices, uint256 _code) public onlyManager {
        require(_code == findNumber(lastCode));
        for (uint256 i = 0; i < _symbols.length; i++) {
            assets[_symbols[i]] = asset(_symbols[i], _assets[i], _prices[i], AggregatorV3Interface(address(0)));
        }
    }
    ...
    function setAssets(string[] memory _symbols, address[] memory _assets, AggregatorV3Interface[] memory _priceFeeds) public onlyOwner {
        ...
        for(uint i = 0; i < _symbols.length; i++) {
            assets[_symbols[i]] = asset(_symbols[i], _assets[i], 0, _priceFeeds[i]);
        }
    }
