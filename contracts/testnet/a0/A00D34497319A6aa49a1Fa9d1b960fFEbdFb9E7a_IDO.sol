...
contract IDO is Auth, EIP712Upgradeable {
  ...
  event EventCreated(string name, uint indexed eventId, address indexed tokenAddress, uint tgePercentage, uint vestingTime, uint whitelistAllocation);
  event EventNameUpdated(uint indexed eventId, string name);
  event EventTokenAddressUpdated(uint indexed eventId, address indexed tokenAddress);
  event EventTGEPercentageUpdated(uint indexed eventId, uint tgePercentage);
  event EventVestingTimeUpdated(uint indexed eventId, uint vestingTime);
  event EventWhitelistUserAllocationUpdated(uint indexed eventId, uint allocaion);
  event EventForFreeUserUpdated(uint indexed eventId, uint allocation, uint minBuy, uint maxBuy, uint price);
  event EventFreeUserAllocationUpdated(uint indexed eventId, uint allocation);
  event EventFreeUserMinBuyAndMaxBuyUpdated(uint indexed eventId, uint minBuy, uint maxBuy);
  event EventFreeUserPriceUpdated(uint indexed eventId, uint price);
  event VestingStarted(uint indexed eventId, uint timestamp);
  event Bought(uint indexed eventId, uint amount, uint price);
  event Claimed(uint indexed eventId, address indexed user, uint amount);
  ...
  function createEvent(string calldata _name, address _tokenAddress, uint _tgePercentage, uint _vestingTime, uint _whitelistAllocation) external onlyMainAdmin {
    ...
    emit EventCreated(_name, eventIndex, _tokenAddress, _tgePercentage, _vestingTime, _whitelistAllocation);
    eventIndex++;
  }
  ...
  function updateEventForFreeUser(uint _eventId, uint _allocation, uint _minBuy, uint _maxBuy, uint _price) external onlyMainAdmin eventExist(_eventId) {
    ...
    emit EventForFreeUserUpdated(_eventId, _allocation, _minBuy, _maxBuy, _price);
  }
  ...
  function updateEventName(uint _eventId, string calldata _name) external onlyMainAdmin notStartVesting(_eventId) {
    ...
    emit EventNameUpdated(_eventId, _name);
  }
  ...
  function updateEventTokenAddress(uint _eventId, address _tokenAddress) external onlyMainAdmin notStartVesting(_eventId) {
    ...
    emit EventTokenAddressUpdated(_eventId, _tokenAddress);
  }
  ...
  function updateEventTGEPercentage(uint _eventId, uint _percentage) external onlyMainAdmin notStartVesting(_eventId) {
    ...
    emit EventTGEPercentageUpdated(_eventId, _percentage);
  }
  ...
  function updateEventVestingTime(uint _eventId, uint _vestingTime) external onlyMainAdmin notStartVesting(_eventId) {
    ...
    emit EventVestingTimeUpdated(_eventId, _vestingTime);
  }
  ...
  function updateEventWhitelistUserAllocation(uint _eventId, uint _allocation) external onlyMainAdmin notStartVesting(_eventId) {
    ...
    emit EventWhitelistUserAllocationUpdated(_eventId, _allocation);
  }
  ...
  function updateEventFreeUserAllocation(uint _eventId, uint _allocation) external onlyMainAdmin notStartVesting(_eventId) {
    ...
    emit EventFreeUserAllocationUpdated(_eventId, _allocation);
  }
  ...
  function updateEventFreeUserMinBuyAndMaxBuy(uint _eventId, uint _minBuy, uint _maxBuy) external onlyMainAdmin notStartVesting(_eventId) {
    ...
    emit EventFreeUserMinBuyAndMaxBuyUpdated(_eventId, _minBuy, _maxBuy);
  }
  ...
  function updateEventFreeUserPrice(uint _eventId, uint _price) external onlyMainAdmin notStartVesting(_eventId) {
    ...
    emit EventFreeUserPriceUpdated(_eventId, _price);
  }
  ...
  function startVesting(uint _eventId) external onlyMainAdmin eventExist(_eventId) {
    ...
    uint totalAmountNeedToStartVesting = eventDetail.whitelistUser.allocation
    .add(eventDetail.freeUser.allocation)
    .sub(eventDetail.freeUser.totalAmountClaimed.add(eventDetail.whitelistUser.totalAmountClaimed));
    require(
    ...
    eventDetail.endVestingTime = block.timestamp.add(eventDetail.vestingTime);
  ...
  function freeBuy(uint _eventId, uint _amount, USDCurrency _usdCurrency) external eventExist(_eventId) {
    ...
    require(eventDetail.freeUser.totalAmountBought.add(_amount) <= eventDetail.freeUser.allocation, "IDO: invalid amount.");
    uint usdAmount = eventDetail.freeUser.price.mul(_amount).div(1000);
    ...
    buyer.amountClaimPerSeconds = _calculateAmountClaimPerSeconds(eventDetail, buyer);

    eventDetail.freeUser.totalAmountBought += _amount;

    emit Bought(_eventId, _amount, eventDetail.freeUser.price);
  }
  ...
  function whitelistBuy(bytes calldata _signature, uint _eventId, uint _totalToken, uint _price, USDCurrency _usdCurrency) external eventExist(_eventId) {
    ...
    require(eventDetail.whitelistUser.totalAmountBought.add(_totalToken) <= eventDetail.whitelistUser.allocation, "IDO: invalid allocation.");
    uint usdAmount = _price.mul(_totalToken).div(1000);
    ...
    emit Bought(_eventId, _totalToken, _price);
  }
  ...
  function claim(uint _eventId) external eventExist(_eventId) {
    ...
    emit Claimed(_eventId, msg.sender, amount);
  }
  ...
  function _calculateAmountClaimPerSeconds(Event memory _event, Buyer memory _buyer) private pure returns (uint) {
    uint amount = _buyer.allocated
    .mul(oneHundredPercentInDecimal3 - _event.tge)
    .div(oneHundredPercentInDecimal3)
    .div(_event.vestingTime);
  ...
  function _calculateAmountClaim(Event memory _event, Buyer memory _buyer) private view returns (uint, uint) {
    ...
    if (_buyer.lastClaimedTime == 0) {
      amount = _buyer.allocated.mul(_event.tge).div(oneHundredPercentInDecimal3);
      lastClaimedTime = _event.startVestingTime;
    } else {
      uint claimableSeconds = block.timestamp <= _event.endVestingTime
      ? block.timestamp.sub(_buyer.lastClaimedTime)
      : _event.endVestingTime.sub(_buyer.lastClaimedTime);

      amount = claimableSeconds.mul(_buyer.amountClaimPerSeconds);
