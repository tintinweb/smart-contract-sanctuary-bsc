 ...
 */
contract ERC20 is Context, IERC20, IERC20Metadata, Lockable {
    ...
    function approve(address spender, uint256 amount)
        ...
        returns (bool)
    {
        require(!isLocked(msg.sender), "Lockable: Account locked");
...
contract GaStream is Pausable, Ownable, Burnable, Lockable, ERC20 {
    uint256 private constant _initialSupply = 16_987_500_000e18;
    string public constant maxSupply = "Nil";

    constructor() ERC20("GaStream", "GSTRM") {
    ...
    function addBurner(address account) public onlyOwner whenNotPaused {
        require(!isLocked(account), "Lockable: Account is locked");
        _addBurner(account);
    }
    ...
    function burn(uint256 amount) public onlyBurner whenNotPaused {
        require(!isLocked(msg.sender), "Lockable: Account is locked");
