...
contract DoGecoin is IERC20, Ownable {
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;  
        if(IERC20(usdtAddress).balanceOf(address(this)) >= 1 * 10**18 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _transferStandard(
        ...
        if (tAmount.div(10000).mul(_LPFee.add(_luckyFee)) > 0)
        swapTokensForCake(tAmount.div(10000).mul(_LPFee.add(_luckyFee)));
