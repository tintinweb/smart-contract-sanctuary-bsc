...
contract Betting is ReentrancyGuard, Ownable, BetTokenWrapper {
    ...
    function bet(
        ...
        if (predictResult_ == Result.RIGHT) {
            //check join or not
            if (
                games[gameId_].right.playerBettingInfo[msg.sender].joined ==
                false
            ) {
                games[gameId_]
                    .right
                ...
                games[gameId_]
                    .right
                    .playerBettingInfo[msg.sender]
                    .index = games[gameId_].right.players.length;
        ...
        if (predictResult_ == Result.WRONG) {
            //check join or not
            if (
                games[gameId_].wrong.playerBettingInfo[msg.sender].joined ==
                false
            ) {
                games[gameId_]
                    .wrong
                ...
                games[gameId_]
                    .wrong
                    .playerBettingInfo[msg.sender]
                    .index = games[gameId_].wrong.players.length;
    ...
    function finalizeGame(uint256 gameId_, Result result_) public nonReentrant {
        require(
            super.owner() == msg.sender || games[gameId_].creator == msg.sender,
        ...
        if (games[gameId_].creator != msg.sender) {
    ...
    function penalty(uint256 gameId_, Result correctResult_) private {
        address host = games[gameId_].creator;
        uint256 hostRightPenaltyAmount;
        uint256 hostWrongPenaltyAmount;

        if (correctResult_ == Result.RIGHT) {
            //calculate right penalty
            hostRightPenaltyAmount =
                (games[gameId_].right.playerBettingInfo[host].bettingAmount *
                    _NOT_REVEAL_RESULT_PENALTY_RATE) /
                _RATE_DENOMINATOR;

            // Penalize on right result
            games[gameId_].right.totalBettingAmount -= hostRightPenaltyAmount;

            games[gameId_]
                .right
                .playerBettingInfo[host]
                .bettingAmount -= hostRightPenaltyAmount;
        }

        if (correctResult_ == Result.WRONG) {
            //calculate wrong penalty
            hostWrongPenaltyAmount =
                (games[gameId_].wrong.playerBettingInfo[host].bettingAmount *
                    _NOT_REVEAL_RESULT_PENALTY_RATE) /
                _RATE_DENOMINATOR;

            // Penalize on wrong result
            games[gameId_].wrong.totalBettingAmount -= hostWrongPenaltyAmount;

            games[gameId_]
                .wrong
                .playerBettingInfo[host]
                .bettingAmount -= hostWrongPenaltyAmount;
        }

        uint256 totalPenalty = hostRightPenaltyAmount + hostWrongPenaltyAmount;
    ...
    function allowClaim(uint256 gameId_) public onlyOwner {
        ...
        if (games[gameId_].penalty) {
            // Penalize host
            penalty(gameId_, games[gameId_].result);
        }
    ...
    function claim(uint256 gameId_) public nonReentrant {
        ...
        if (games[gameId_].result == Result.RIGHT) {
            ...
            require(
                games[gameId_].right.playerBettingInfo[msg.sender].claimed ==
                    false,
        ...
        if (games[gameId_].result == Result.WRONG) {
            ...
            require(
                games[gameId_].wrong.playerBettingInfo[msg.sender].claimed ==
                    false,
                "Error: you have claimed on this game!"
            );
        ...
        uint256 award = (totalLoserBetting * winnerbettingAmount) /
            totalWinnerBetting;
        ...
        (
            ...
            uint256 revenueFee,
            uint256 hostCommission
        ) = computeProtocolFee(winnerbettingAmount + award);
