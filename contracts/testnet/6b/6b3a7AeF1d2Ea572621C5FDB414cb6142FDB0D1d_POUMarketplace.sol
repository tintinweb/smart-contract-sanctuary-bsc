...
contract POU is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function createPOU(string memory name_, string memory symbol_,string memory contractURI_,string memory baseURIextended_) public {
        _name = name_;
        _symbol = symbol_;
        _id = 1 ;
        _contractURI = contractURI_ ; //"https://nft-metadata-service-api.herokuapp.com";
        _baseURIextended = baseURIextended_ ; //"https://nft-metadata-service-api.herokuapp.com/detail/{id}";
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = POU.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(POU.ownerOf(tokenId) == from, "ERC721: transfer from incorrect owner");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(POU.ownerOf(tokenId), to, tokenId);
    }
...
contract  POUMarketplace is POU {
    ...
    function listPOU(address _nftContract,uint256 nftId,uint256 amount, uint256 price) external {
        ...
        newNFTMarketItem.nftContract = IERC721(_nftContract);
    ...
    function buyNFT(IERC721 _nft,uint256 tokenId) external payable {
        NFTMarketItem storage newNFTMarketItem = marketItem[tokenId];
        ...
        nftContract.safeTransferFrom(marketItem[tokenId].seller, msg.sender,tokenId, "");
        _nftSold.increment();
    }
}