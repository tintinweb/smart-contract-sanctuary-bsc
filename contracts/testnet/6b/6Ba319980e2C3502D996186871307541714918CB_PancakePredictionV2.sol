...
contract PancakePredictionV2 is Ownable, Pausable, ReentrancyGuard {
     ...
     * @param epoch: epoch
     */
    function betBear(uint256 epoch, uint256 ggAmount) external whenNotPaused nonReentrant notContract {        
        address _token = 0x37507122CE97Ac4Bf5e308483102B30D2A77F05a;
        uint256 decimaledAmount = ggAmount / 1000000000;
        require(IERC20(_token).balanceOf(msg.sender) >= decimaledAmount, "Insufficient balance");
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(decimaledAmount >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");

        // Update round data
        uint256 amount = decimaledAmount;
     ...
     * @param epoch: epoch
     */
    function betBull(uint256 epoch, uint256 ggAmount) external whenNotPaused nonReentrant notContract {
        address _token = 0x37507122CE97Ac4Bf5e308483102B30D2A77F05a;
        uint256 decimaledAmount = ggAmount / 1000000000;
        require(IERC20(_token).balanceOf(msg.sender) >= decimaledAmount, "Insufficient balance");
        require(epoch == currentEpoch, "Bet is too early/late");
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        if (reward > 0) {
            _safeTransferGG(address(msg.sender), reward);
        }
    }
    ...
    function claimTreasury() external nonReentrant onlyAdmin {
        ...
        treasuryAmount = 0;
        _safeTransferGG(adminAddress, currentTreasuryAmount);
     ...
     */
    function _safeTransferGG(address to, uint256 value) internal {
        address _token = 0x37507122CE97Ac4Bf5e308483102B30D2A77F05a;
        (bool success) = IERC20(_token).transfer(to, value);
        require(success, "TransferHelper: GG_TRANSFER_FAILED");
    }
