...
contract NetGymFarming is OwnableUpgradeable, ReentrancyGuardUpgradeable { 
    ...
    function speedStake(
        ...
        updatePool(_pid);
        IUniswapV2Pair lpToken = IUniswapV2Pair(address(poolInfo[_pid].lpToken));

        require((lpToken.token0() == router.WETH()) || (lpToken.token1() == router.WETH()), "Wrong poolID");

        uint256 bnbAmount = msg.value;
        console.log("bnbAmount 0",bnbAmount);
        if (_tokenAmount > 0) {
        ...
        IWETHGym(wbnbAddress).deposit{value: (bnbAmount * buyBackComission) / 100}();
        IERC20Upgradeable(wbnbAddress).safeTransfer(buyBack, (bnbAmount * buyBackComission) / 100);

        console.log("bnbAmount 1",bnbAmount);
        ...
        console.log("bnbAmount 2",bnbAmount);
    ...
    function claimAndDeposit(
        ...
        uint256 lp = liquidityProvider.addLiquidityETHByPair{value: bnbAmount}(
            IUniswapV2Pair(address(poolInfo[_pid].lpToken)),
            address(this),
            _amountAMin,
            _amountBMin,
            _minAmountOutA,
