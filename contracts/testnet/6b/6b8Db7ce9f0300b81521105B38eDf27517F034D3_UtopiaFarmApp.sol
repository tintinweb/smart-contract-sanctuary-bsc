...
contract UtopiaFarmApp is ModuleBase, Lockable, ERC721Holder, SafeMath, ECDSA {
    ...
    function checkMaturedV2(address account, uint256 shortenTime, bytes memory signature) external view returns(bool res, uint256 sowAmount, uint256 outAmount) {
        (res, sowAmount, outAmount) = _checkMaturedV2(account, shortenTime, signature);
    }

    function _checkMaturedV2(address account, uint256 shortenTime, bytes memory signature) internal view returns(bool res, uint256 sowAmount, uint256 outAmount) {
        string memory message = string(abi.encodePacked(Strings.addressToString(account), Strings.uint256ToString(shortenTime)));
        if(_IsSignValid(message, signature)) {
            if(mapUserSowData[account] > 0) {
                SowData memory sd = mapSowData[mapUserSowData[account]];
                if(block.timestamp >= sub(add(sd.sowTime, SystemSetting(moduleMgr.getSystemSetting()).getMatureTime(sd.ssIndex)), shortenTime)) {
                    res = true;
                    sowAmount = sd.sowAmount;
                    uint256 cycleYieldsPercent = SystemSetting(moduleMgr.getSystemSetting()).getCycleYieldsPercent(sd.ssIndex);
                    uint256 usdtOutAmount = add(sd.usdtAmount, div(mul(sd.usdtAmount, cycleYieldsPercent), 1000));
    ...
    function claimedSeedV2(uint256 shortenTime, bytes memory signature) external lock {
        (bool res, , uint256 outAmount) = _checkMaturedV2(msg.sender, shortenTime, signature);
        require(res, "have no matured seed");
        uint32 _sowIndex = mapUserSowData[msg.sender];
        SowData memory sd = mapSowData[_sowIndex];
