...
contract FarmBooster is Ownable {
    ...
    function onCakePoolUpdate(
        ...
        for (uint256 i = itmap.iterateStart(); itmap.iterateValid(i); i = itmap.iterateNext(i)) {
            ...
            if (!flag) {
                avgDuration = _avgLockedDuration();
                flag = true;
            }
    ...
    function activedPools(address _user) external view returns (uint256[] memory pools) {
        ...
        if (itmap.size == 0) return pools;

        pools = new uint256[](itmap.size);
    ...
    function getUserMultiplier(address _user, uint256 _pid) external view returns (uint256) {
        return _boostCalculate(_user, proxyContract[_user], _pid, _avgLockedDuration());
    }
    ...
    function _avgLockedDuration() internal view returns (uint256) {
        ...
        return ((boostedRatio - BOOST_RATIO_PRECISION) * 365 * BOOST_WEIGHT_PRECISION) / boostWeight;
    }
}
