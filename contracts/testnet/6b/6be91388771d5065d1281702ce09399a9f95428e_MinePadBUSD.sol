...
contract MinePadBUSD is Context, Ownable , ReentrancyGuard, Events {
    ...
    function _updateAllocations(uint256 _amount) internal {
        ...
        vesting[msg.sender] = vestingTime(msg.sender, 0, arg, vesting_percent, arg[0]);
    ...
    function _calculate(uint256 _amount, uint256 _percent) internal view returns(uint256) {
        uint256 get_multiplication = SafeMath.mul(_amount, _percent);
        return SafeMath.div(get_multiplication, (10 * 10 ** tokenAllocationInterface.decimals()));
    }
    ...
    function _claimedCheck() internal returns(uint256) {
        uint256 currentlyClaimed = vesting[msg.sender].nextClaimTime;
        require(block.timestamp > currentlyClaimed, "Patience" );
        uint256 lent = vesting[msg.sender].vestingPeriod.length;
        require(vesting[msg.sender].claimCount <= lent, "Complete");
        uint256 amountPerTime;
        for (uint256 i; i < lent; ) {
            uint256 vestP = vesting[msg.sender].vestingPeriod[i];
            if (vestP > currentlyClaimed) {
                vesting[msg.sender].nextClaimTime = vestP;
                amountPerTime = vesting[msg.sender].amountPerPeriod[i];
                unchecked {
                    vesting[msg.sender].claimCount ++;
                }
                break;
            }
            unchecked {
