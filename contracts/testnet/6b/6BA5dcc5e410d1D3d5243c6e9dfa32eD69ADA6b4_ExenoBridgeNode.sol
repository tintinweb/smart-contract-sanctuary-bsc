    ...
    ERC1363Payable
{
    ...
    // List of router addresses whose private keys are used for signing playloads
    address[] public routerList;

    // Current number of routers
    uint256 public routerSize;
    ...
    // Indicates that routers have been reset
    event ResetRouters(
        uint256 size
    );
        ...
        ERC1363Payable(IERC1363(_token))
    {
        ...
        for (uint256 i = 0; i < _routers.length; i++) {
            routerList.push(_routers[i]);
            routers[_routers[i]] = true;
        }
        routerSize = _routers.length;
    }
     ...
     * Configure routers
     */
    function resetRouters(address[] calldata newRouters)
        external onlyOwner
    {
        ...
        for (uint256 i = 0; i < newRouters.length; i++) {
            routerList.push(newRouters[i]);
            routers[newRouters[i]] = true;
        }
        routerSize = newRouters.length;
        emit ResetRouters(newRouters.length);
    }
