    ...
    ERC721HolderUpgradeable
{
    ...
    function setNftRares(
        uint256[] memory tokenIds,
        uint8[] memory rares
    ) public onlyRole(ADMIN_ROLE) {
        require(tokenIds.length == rares.length, "LENGTH_RARE_NOT_MATCH");
        uint256 length = tokenIds.length;
        for (uint256 index = 0; index < length; index++) {
            uint256 tokenId = tokenIds[index];
            uint8 rare = rares[index];
            setNftRare(tokenId, rare);
        }
    }
    ...
    function setNftRare(
        ...
        nftRare[tokenId] = rare;
    }

    function getNftRare(uint256 tokenId) public view returns (uint8) {
        return nftRare[tokenId];
    }

    function getNftRares(
        uint256[] memory tokenIds
