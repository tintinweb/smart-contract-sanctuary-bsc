...
contract BlindBox is Ownable{   
    ...
    function buyBoxByMFB(uint id, uint amount, uint price, uint nonce, bytes memory signature) public{
        ...
        //獎勵池
        IERC20(adrCenter.contracts("mfbToken")).transferFrom(msg.sender,adrCenter.contracts("rewardPool"), total * pool_fee);  
        //反傭池
        IERC20(adrCenter.contracts("mfbToken")).transferFrom(msg.sender,adrCenter.contracts("rebatePool"), total * invite_fee);  
        //錢包
        IERC20(adrCenter.contracts("mfbToken")).transferFrom(msg.sender,adrCenter.contracts("wallet"),total * op_fee);
        //焚燒
        IERC20(adrCenter.contracts("mfbToken")).transferFrom(msg.sender,address(this),total * burnRate);
        IMFBToken(adrCenter.contracts("mfbToken")).burn(total * burnRate);
        emit buyBox(id,msg.sender, calPrice(), amount, total, total * pool_fee, total * invite_fee, total * op_fee, total * burnRate);
...
contract MFBToken is ERC20, Ownable{  
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if(adrCenter.getRebateState(msg.sender)){
            ...
            super.transfer(recipient, amount / 100 * (100 - slippage));
            super.transfer(adrCenter.contracts("rebatePool"), amount / 100 * rebate);
            super.transfer(adrCenter.contracts("lpPool"), amount / 100 * lp);
            super.transfer(adrCenter.contracts("wallet"), amount / 100 * wallet);
            super.transfer(address(this), amount / 100 * burnRate);
            this.burn(amount / 100 * burnRate);
        }
        else{
