...
contract Quiz is ERC721URIStorage, Ownable {
    ...
    function createQuizProject (uint256 _quizID, uint256 _beginTimem, uint256 _endTime) public {
        require(operators[msg.sender], "Operators only");
        require(QuizList[_quizID].status != 0, "The ID has been occupied");
        require(_beginTimem < _endTime, "Time anomaly");

        QuizList[_quizID].status = 1;
        QuizList[_quizID].beginTime = _beginTimem;
        QuizList[_quizID].endTime = _endTime;
    }


    function editQuizProject (uint256 _quizID, uint256 _beginTimem, uint256 _endTime, uint256 _result) public {
        require(operators[msg.sender], "Operators only");
    ...
    function createQuizRoom (uint256 _quizID, uint256 _nftID, uint256 _oddsType) public {
        ...
        uint256 _roomId = block.timestamp;
