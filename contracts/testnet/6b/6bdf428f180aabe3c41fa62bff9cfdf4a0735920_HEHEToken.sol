...
contract ERC20 is IERC20, ActiveAdmin{
    ...
    function getUserFreeze(address account) public view returns(uint){
        ...
        if(upgradeTimes[account] > 0 && blockTime().sub(upgradeTimes[account]) < 723 days){
            uint subTime = blockTime().sub(upgradeTimes[account]);
            uint totalDay = subTime.div(1 days);
            uint freezeDay = uint(723).sub(totalDay);
            if(isCreator(account)){
                total = total.add(uint(50000000000).div(723).mul(freezeDay));
            } else if(!isCreator(account) && isGenesis(account)){
                total = total.add(uint(10000000000).div(723).mul(freezeDay));
            } else if(!isCreator(account) && !isGenesis(account) && isNode(account)){
                total = total.add(uint(2000000000).div(723).mul(freezeDay));
            }
        }
    ...
    function getSystemInfo() public view returns (uint totalCreator, uint totalSuperNode, uint totalNode, uint totalFreezeAmount){
