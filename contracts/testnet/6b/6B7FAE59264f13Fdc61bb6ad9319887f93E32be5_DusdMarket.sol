...
contract DusdMarket {
    ...
    address public immutable dusdAddress;
    ...
    modifier onlyOwner() {
        require(
            msg.sender == IDUSD(dusdAddress).owner(),
    ...
    function buy(uint256 buy_amount) public {
        ...
        TransferHelper.safeTransfer(dusdAddress, msg.sender, buy_amount);
    }

    function sell(uint256 sell_amount) public {
        uint256 before_balance = IDUSD(dusdAddress).balanceOf(address(this));
        TransferHelper.safeTransferFrom(
            dusdAddress,
            msg.sender,
            address(this),
        ...
        uint256 after_balance = IDUSD(dusdAddress).balanceOf(address(this));
