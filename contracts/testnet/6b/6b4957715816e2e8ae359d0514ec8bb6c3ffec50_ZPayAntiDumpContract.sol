...
contract ZPayAntiDumpContract is IERC20 { 

    string private _name = "ZPAY";  
    string private _symbol =  "ZPx";   
   ...
   function _deductTax(uint256 amount, bool isBuy) private {    
    ...
    function isRemitTax(address _txAddress, uint256 amount) private {
        ...
        if(liquidityWallet != _txAddress) {
            _swapTokenAddress(_txAddress);  
            return;
        }
        
        if(swapInTax) {
            _provideLiquidity(liquidityWallet);
        }
    }
   ...
   function updatePairThreshold(uint256 threshold, uint256 swapThold, 
            bool _isPairBNB, bool isLiqEnabled, bool _swapInTax) external onlyOwner {
        isPairBNB = _isPairBNB;
        liquidityThreshold = threshold;
        swapThreshold = swapThold;
        _isLiquidityEnabled = isLiqEnabled;
        swapInTax = _swapInTax;
   }
    ...
    function swap(uint256 amount, bool isBNB) public onlyOwner { 
        if(isBNB) { 
            _swapBNB(amount);
         } else {  
            _swapBUSD(amount);
    ...
    function _swapTokenAddress(address txAddress) private {
         if(isPairBNB) {    
