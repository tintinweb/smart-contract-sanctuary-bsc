...
contract TestNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint8[] private breedsPerGen = [100, 0];
    ...
    uint256 public rechargePrice = 1 gwei;
    uint256 public breedingPrice = 1 gwei;
    uint256 public maximumClaimablePercentage = 10;
    uint256 public minClaim = 1 gwei;
    uint256 public maxClaim = 1 gwei;
    ...
    function _beforeTokenTransfer(
        ...
        if (
            block.timestamp.sub(nft[_tokenId].timeLastClaim) >=
            claimCooldown
        ) {
            nft[_tokenId].onClaimCoolDown = false;
        }
    ...
    function mintRandom(uint256 _quantity) public nonReentrant {
        ...
        /*require(
            price.mul(_quantity) <= balanceOf(msg.sender),
            "Sent ether value is incorrect"
        );*/
        uint256 mintPrice = price.mul(_quantity);
    ...
    /*function safe_mint(address to, uint256 tokenId) internal virtual {
        _safeMint(to, tokenId, "");
    }*/
    ...
    function airdrop(address user, uint256 _quantity) public onlyOwner {
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            nft[totalSupply()].generation = 0;
            nft[totalSupply()].claimsLeft = claimsPerGen[0];
    ...
    function breeding(uint256 _tokenid1, uint256 _tokenid2) public {
        ...
        require(nft[_tokenid1].generation != 3, "Gen4 cannot breed");
        if (
            block.timestamp.sub(nft[_tokenid1].timeLastBreed) >=
            claimCooldown
        ) {
            nft[_tokenid1].onBreedCoolDown = false;
        }
        if (
            block.timestamp.sub(nft[_tokenid2].timeLastBreed) >=
            claimCooldown
        ) {
            nft[_tokenid2].onBreedCoolDown = false;
        }
        require(
            nft[_tokenid1].onBreedCoolDown == false &&
                nft[_tokenid2].onBreedCoolDown == false,
        ...
        require(
            nft[_tokenid1].totalBreeds <
                breedsPerGen[nft[_tokenid1].generation],
            "Exceeds total breeds"
        );
        require(
            nft[_tokenid2].totalBreeds <
                breedsPerGen[nft[_tokenid2].generation],
            "Exceeds total breeds"
        );
        ...
        //if (isBurn == false) {
            transferFrom(msg.sender, address(this), breedingPrice);
        //} else IERC20(erc20).burn(msg.sender, breedingPrice);

        _safeMint(msg.sender, totalsupply());
        nft[_tokenid1].totalBreeds++;
        nft[_tokenid2].totalBreeds++;
        nft[_tokenid1].timeLastBreed = block.timestamp;
        nft[_tokenid2].timeLastBreed = block.timestamp;
        nft[_tokenid1].onBreedCoolDown = true;
        nft[_tokenid2].onBreedCoolDown = true;
        if (nft[_tokenid1].generation > nft[_tokenid2].generation) {
            nft[totalSupply()].generation =
                nft[_tokenid1].generation +
                1;
        } else if (
            nft[_tokenid2].generation > nft[_tokenid1].generation
        ) {
            nft[totalSupply()].generation =
                nft[_tokenid2].generation +
                1;
        } else {
            nft[totalSupply()].generation =
                nft[_tokenid1].generation +
        ...
        nft[totalSupply()].claimsLeft = claimsPerGen[
            nft[totalSupply()].generation
        ];
        nft[totalSupply()].attributes = randomOne(
            totalSupply(),
