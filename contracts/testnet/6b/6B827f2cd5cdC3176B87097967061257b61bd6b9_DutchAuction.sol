    ...
    IERC721ReceiverUpgradeable
{
    ...
    /// Change the decreasePeriod

    function changeCurrentAuctionTimemap(address nftContractAddress, uint256 tokenId, uint256 newdecreasePeriod)
    external
        whenNotPaused
        nonReentrant
        isNftSeller(nftContractAddress, tokenId)
    ...
    {
    uint256 endTime = nftContractAuctions[nftContractAddress][tokenId].endTime;
    uint256 decreasePeriod = nftContractAuctionTimming[nftContractAddress][tokenId].decreasePeriod;
    AuctionTime memory auctionTime = mapAuctionTime[endTime][decreasePeriod];
    require(auctionTime.active, "Not a valid bid period");
    nftContractAuctionTimming[nftContractAddress][tokenId].decreasePeriod = newdecreasePeriod;
    }
