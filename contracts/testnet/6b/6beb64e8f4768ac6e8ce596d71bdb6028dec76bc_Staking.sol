...
contract Staking is ReentrancyGuard {
    ...
    uint256 public REWARD_RATE = 100; // yearly APR per wallet amount staked
    uint256 public s_totalSupply;
    uint256 public s_rewardPerTokenStored;
    uint256 public s_lastUpdateTime;
    uint256 public dev_walletFee = 3; // Calculate the percentage of the dev fee that is split between dev wallets
    address payable dev_wallet_1;
    address payable dev_wallet_2;

    // Calculate the percentage of the fee that goes to each dev wallet
    uint256 public dev_wallet_1_fee_percentage = 34;
    uint256 public dev_wallet_2_fee_percentage = 66;
    ...
    function getRateOfReturn(uint256 amount) public view returns (uint256) {
        // Calculate the reward per token by calling the rewardPerToken function
        uint256 _rewardPerToken = rewardPerToken();
        // Calculate the rate of return by dividing the reward per token by the amount staked and multiplying by 100 (to get a percentage)
        uint256 rateOfReturn = (_rewardPerToken / amount) * 100;
        return rateOfReturn;
    }
        ...
        moreThanZero(amount)
    {
        ...
        uint256 fee = amount.mul(dev_walletFee).div(100);
        // Calculate the amount of the fee that goes to each dev wallet
        uint256 dev_wallet_fee_amount = fee
            .mul(dev_wallet_1_fee_percentage)
            .div(100);
        uint256 dev_wallet2_fee_amount = fee
            .mul(dev_wallet_2_fee_percentage)
            .div(100);
        // Transfer the fee to the dev wallets
        require(
            s_stakingToken.transferFrom(
                msg.sender,
                dev_wallet_1,
                dev_wallet_fee_amount
            ),
            "please pay staking fee to dev_wallet"
        );
        require(
            s_stakingToken.transferFrom(
                msg.sender,
                dev_wallet_2,
                dev_wallet2_fee_amount
            ),
            "please pay staking fee to dev_wallet2"
        );

        s_totalSupply += amount;
        s_balances[msg.sender] += amount;
        staking_start[msg.sender] = block.timestamp;
        staking_end[msg.sender] = staking_start[msg.sender] + lockedTime;
    ...
    function setDevWalletOne(address _devwalletone) public {
        dev_wallet_1 = payable(_devwalletone);
    }

    function setDevWalletTwo(address _devwallettwo) public {
        dev_wallet_2 = payable(_devwallettwo);
    }
    ...
    function setDevWalletOneFee(uint256 _dev_wallet_1_fee_percentage) public {
        dev_wallet_1_fee_percentage = _dev_wallet_1_fee_percentage;
    }

    function setDevWalletTwoFee(uint256 _dev_wallet_2_fee_percentage) public {
        dev_wallet_2_fee_percentage = _dev_wallet_2_fee_percentage;
    }
}