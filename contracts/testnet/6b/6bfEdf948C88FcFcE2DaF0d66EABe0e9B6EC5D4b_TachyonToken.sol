...
contract TachyonToken is ERC20, Ownable {
    ...
    event TokensConverted(address indexed by, uint256 tokens, uint256 bnb);
...
//        uint256 adjustedSupply = _currentSupply.sub(_rewardsGiven);
    ...
    function buyTACMufi(MUFI _mufi) external payable{
        ...
        emit Stake(_msgSender(), token_wei);
    ...
    function MFToken(uint256 amount, MUFI _mufi) external {
    ...
    function MFTokenWei(uint256 amount, MUFI _mufi) external {
        amount = amount ;
        require(balanceOf(_msgSender()) >= amount, "TAC: recipient account doesn't have enough balance");
    ...
    function convertTokenToBNB(address to, uint256 value) external {
        require(SELL_ALLOWED, "TAC: Sell is disabled for now");

        uint256 currentPrice = getCurrentPrice();

        //reduce value by conversion fee
        uint256 fee = uint256((value * _conversionFee ) / 100000 );
        uint256 netValue = value.sub(fee);

        require(balanceOf(_msgSender()) >= value, "TAC: recipient account doesn't have enough balance");
        //convert these tokens to bnb amount

        uint256 bnbAmount = netValue * currentPrice;

        require(address(this).balance >= bnbAmount, "TAC: invalid digits");

        transferTo(owner(), value, false);//transfer to root address
        _msgSender().transfer(bnbAmount);

        emit TokensConverted(to, value, bnbAmount);
    }
