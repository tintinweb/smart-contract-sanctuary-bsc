...
contract CliqStaking is AccessControl {
    ...
    uint256 private constant TIME_UNIT = 60;
    ...
    constructor(address _stakedToken, address _CLIQ) {
        ...
        _definePackage("Silver Package", 3, 3, 15, 1000000); // in 30 days you receive: 15% of staked token OR 1 cliq for 1 token staked
        _definePackage("Gold Package", 6, 10, 18, 1500000); // 1.5 cliq for 1 token staked
        _definePackage("Platinum Package", 10, 12, 30, 2000000); // 2 cliq for 1 token staked
    }
    ...
    function unstake(uint256 stakeIndex) public {
        ...
        if (stakes[msg.sender][stakeIndex]._stakeRewardType == 0) {
            ...
            uint256 totalStake =
                stakes[msg.sender][stakeIndex]._amount.add(reward) *1e9;
