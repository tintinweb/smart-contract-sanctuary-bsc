...
contract RewardsPayer is ReentrancyGuard, AccessControl {
    ...
    constructor(address _MortgageAddress, address _Busd, address _Usdc,address _controlContract,address _VaultRewardsContract) {
    ...
    //ToDo: llamar a estas funciones en las funciones de pago y validar que primero se pague la deuda
    // antes de que el usuario reciba los rewards, si no tiene deuda entonces si recibe los rewards.

    ///@dev Function to check if the user has a Mortgage debt
    function checkDebt() private {
        //ToDo:
        // obtener los id de hipotecas y checar si alguno esta asociado al sender
        // ver cuanto tiene de deuda en la coleccion o en otras colecciones.
        // regresar la deuda que tiene(struct mortgage interes), el id del mortgage, bool indicando si tiene adeudo
    }

    ///@dev Function to pay the user Mortgage debt with the rewards
    function payDebt() private {
        //ToDo:
        // usar los rewards de holder, builder y renta para pagar esa deuda
        // pagar los interes y el capital con esos rewards, enviandolos a los vaults correspondientes
        // restarlo de la deuda total y actualizar la struct con la nueva deuda.
    }
    ...
    function CalcBuilderRewardsForCollection(
        ...
        uint length = _walletsUsers.length;
        if (length != _numNFTsMinted.length) {
