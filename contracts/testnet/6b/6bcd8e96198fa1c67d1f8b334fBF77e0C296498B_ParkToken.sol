...
contract ParkToken is Context, IParkToken{
    ...
    uint256 public constant LIQUIDITY_PRICE =    5_000_000_000_000_000; //0%  discount 0.005BNB
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_,address[] memory wallets_,address[] memory MOBs_,uint256 minHoldBalance_,bool isMainNetwork_) {
    ...
    function _beforeTokenTransfer (
        ...
        //  if(!_isExcludedFromFees[from_]){
                
        //         uint256 taxPrice= getParkTokenCurrentPrice().mul((amount_.mul(80).div(1000)));
  ...
  function addLiquidity(uint256 amountTokenDesired_,uint amountTokenMin_,uint amountETHMin_,uint deadline_) public payable override returns(bool){

        // uint256 liquidityTotalPrice=LIQUIDITY_PRICE.mul(amountTokenDesired_);
        // require(liquidityTotalPrice<=msg.value,"LIQUIDITY PRICE per token is less than defined value");
        uint256 fromBalance = _balances[_msgSender()];
        require(fromBalance >= amountTokenDesired_, "ERC20: transfer amount exceeds balance");
        unchecked {
            _balances[_msgSender()] = fromBalance - amountTokenDesired_;
        }
        _balances[address(this)] += amountTokenDesired_;

        emit Transfer(_msgSender(), address(this), amountTokenDesired_);
        //Approve the amount
...
interface IParkToken is  IERC20, IERC20Metadata {
    ...
    function setPrivateSaleStatus(bool newStatus_) external  returns(bool);
     ...
     /**
     * @dev Adds `amount` liquidity to tokens from `pancakeswap`
     *
     * Emits a {LiquidityAdded} event 
     *
     */
    function addLiquidity(uint256 amountTokenDesired_,uint amountTokenMin_,uint amountETHMin_,uint deadline_)  external payable returns(bool);
