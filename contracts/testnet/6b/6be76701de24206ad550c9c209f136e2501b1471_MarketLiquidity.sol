...
contract MarketLiquidity is Ownable, ReentrancyGuard {
    ...
    address payable public WPACKStakingManager = payable(0xA16E33729A424EE51B862548416B8061227498B2); 
    ...
    function requestMarketPayout(uint256 payout) public returns (uint256) {
        ...
        require(bal <= ret, "return exceeds contract balance");
        rewards -= ret;
        address payable callingMarket = payable(msg.sender);
        callingMarket.transfer(ret);
    ...
    function distributeWithdrawalFee(uint256 fee) private {
        ...
        operator.transfer(dividend);
        WPACKStakingManager.transfer(dividend);
    }
