...
contract Token is ERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        _isWhiteList[address(_uniswapV2Pair)] = true;
        _isWhiteList[_marketingWalletAddress] = true;
    ...
    function addWhiteList(address[] memory list) external onlyOwner {
        for (uint256 i = 0; i < list.length; i++) {
            _isWhiteList[list[i]] = true;
        }
    }

    function removeWhiteList(address account) external onlyOwner {
        delete _isWhiteList[account];
    }
    ...
    function _transfer(
        ...
        if (block.timestamp <= swapTime) {
            if (!_isWhiteList[to]) {
                uint256 marketFee = amount.mul(9).div(10);
