...
contract MidasMultinetworkRouterManager {
    ...
    function getChainlinkDataFeedsManager()
        external
        view
        returns (ChainlinkDataFeedsManager)
    {
        return chainlinkDataFeedsManager;
    }
        ...
        returns (uint256)
    {
        uint256 nativeNetworkCurrencyPrice = uint256(
            chainlinkDataFeedsManager.getLatestPrice()
        ) * 1e10;
        address[] memory path = new address[](3);
...
contract ChainlinkDataFeedsManager {
    ...
    function getAmountOutUSD(
        ...
        uint256 nativeNetworkCurrencyPrice = uint256(getLatestPrice()) * 1e10;
        address[] memory path = new address[](2);
        path[0] = _tokenAddress;
