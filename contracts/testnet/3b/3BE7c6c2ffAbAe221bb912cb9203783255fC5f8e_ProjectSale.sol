...
contract ProjectSale is IProjectSale, ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 private constant NOT_FINALIZED = 1;
    uint256 private constant FINALIZED = 2;
    uint256 private _saleStatus;
    ...
    modifier onlyOnce() {
        require(_saleStatus == NOT_FINALIZED, "ProjectSale: sale is finalized");
        _;
        _saleStatus = FINALIZED;
    }
        ...
        onlyDuringRegisteration
    {
        ...
        uint256 allocation = allocationProvider.allocationOf(msg.sender);
        require(allocation > 0, "ProjectSale: no allocation");

        _allocation[msg.sender] = allocation;
        _totalAllocation = _totalAllocation + allocation;
    ...
    function _remaingAllocationOf(address _account)
        ...
        returns (uint256)
    {
        uint256 remainingAllocation = _allocation[_account]
        ...
        onlyWhiteListed(msg.sender)
    {
        ...
        totalSaleValue = totalSaleValue + _amount;
    }
    ...
    function vesitingContract() public view returns (bool, address) {
        if (_saleStatus == NOT_FINALIZED) {
            return (false, address(0));
        }
        assert(_vestingContract != address(0));
    ...
    function finalizeSale() public onlyOwner onlyOnce {
        ...
        // Calculate the amount of tokens for each address
        uint256 depositerCount = _depositBalances.length();
        address[] memory payees = new address[](depositerCount);
        uint256[] memory shares = new uint256[](depositerCount);
        for (uint256 i = 0; i < depositerCount; i++) {
            (address depositer, uint256 deposited_) = _depositBalances.at(i);
            payees[i] = depositer;
            shares[i] = deposited_;
        }
        ...
        SplittedVesting splitter = new SplittedVesting(
        ...
        projectToken.safeTransferFrom(
            msg.sender,
            address(splitter.vestingWallet()),
            tokensSold
        );
...
contract SplittedVesting is PaymentSplitter, Ownable, Pausable, IVesting {
