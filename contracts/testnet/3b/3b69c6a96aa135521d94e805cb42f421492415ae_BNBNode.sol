...
contract BNBNode {
    ...
    function sellNodes() public {
        uint256 nodesToBNB = energyToBNB(calculateEnergy());
    ...
    function calculateEnergy() public view returns(uint256) {
        return SafeMath.mul(
            SafeMath.sub(block.timestamp, lastBought[msg.sender]),
            SafeMath.div(nodesOwned[msg.sender], 604800)
        );
    }

    function getNodes() public view returns(uint256) {
        return nodesOwned[msg.sender];
    }
