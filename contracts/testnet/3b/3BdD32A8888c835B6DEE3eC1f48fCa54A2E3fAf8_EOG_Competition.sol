...
contract EOG_Competition is Context, Ownable {
    ...
    function updateTokenActivate(uint256 tokenIndex, bool isActive)
        external
        onlyOwner
    {
        isTokenExists(tokenIndex);
    ...
    function addNewCompetition(
        ...
        require(entryFeeInUSD > 0, "preset entry fee must be greater than 0!");
        ...
        for (uint8 i = 0; i < _teams.length; i++) {
            for (uint8 j = 0; j < teamMemberLength; j++) {
                ...
                require(
                    payableInUSD > rakeAmountInUSD,
                    "Payable amount by competitor should be greater than rake amount"
                );
    ...
    function transferDesiredTokenFromCompetitor(
        ...
        require(
            competitorBalanceInUSD >= payableInUSD,
        ...
        uint256 entryFeeInUSDBasedOnPresetInToken = entryFeeInUSD.mul(1e18).div(
            stablePrice
        );
        tokens[_tokenIndex].lockedBalance = DesiredToken.lockedBalance.add(
            entryFeeInUSDBasedOnPresetInToken * 2
        );
        ...
        returns (bool)
    {
        ...
        if (_winnerTeam == 0) {
            pay(teamA, rakeAmountInUSD, entryFeeInUSD, false);
            handleUnlockTokenForLooserTeam(teamB, entryFeeInUSD);
        } else if (_winnerTeam == 1) {
            pay(teamB, rakeAmountInUSD, entryFeeInUSD, false);
            handleUnlockTokenForLooserTeam(teamA, entryFeeInUSD);
        } else if (_winnerTeam == 2) {
            pay(teamA, rakeAmountInUSD, entryFeeInUSD, true);
            pay(teamB, rakeAmountInUSD, entryFeeInUSD, true);
        }
        return true;
    }
        ...
        )
    {
        ...
        entryFeeInUSD = competionPreset.entryFeeInUSD;
