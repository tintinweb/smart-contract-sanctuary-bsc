...
contract PEAirdrop is ReentrancyGuard, Ownable, Pausable {
    ...
    function claimAirdrop(uint256 _amount, uint256 _voucher)  external nonReentrant {
        ...
        uint256 remainAmount = user.value - user.receivedVoucher * 266 * 10 ** 18 + user.receivedAmount;
        require(_amount + _voucher * 266 * 10 ** 18 <= remainAmount, "Claim airdrop exceeds allowance");
