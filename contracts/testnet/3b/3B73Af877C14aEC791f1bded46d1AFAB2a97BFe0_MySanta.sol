...
contract MySanta is Ownable, ReentrancyGuard {
    ...
    function transfer(
        ...
        require(charity.fundReceiver != address(0), "My Santa: no available charity");
        ...
        if (token == Token.BUSD) {
            busd.transfer(receiver, amountReceived);
            busd.transfer(developmentFeeReceiver, developmentFeeAmount);
            busd.transfer(daoFeeReceiver, daoFeeAmount);

            donor[msg.sender].totalBUSDDonation = amount;
        } else if (token == Token.USDT) {
            usdt.transfer(receiver, amountReceived);
            usdt.transfer(developmentFeeReceiver, developmentFeeAmount);
            usdt.transfer(daoFeeReceiver, daoFeeAmount);

            donor[msg.sender].totalUSDTDonation = amount;
        } else if (token == Token.USDC) {
            usdc.transfer(receiver, amountReceived);
            usdc.transfer(developmentFeeReceiver, developmentFeeAmount);
            usdc.transfer(daoFeeReceiver, daoFeeAmount);
