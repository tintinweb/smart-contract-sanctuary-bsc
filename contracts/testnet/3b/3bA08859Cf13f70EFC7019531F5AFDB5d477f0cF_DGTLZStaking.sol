...
contract DGTLZStaking is Ownable {
  ...
  address public constant teamWallet = 0x138532955EE01A5DAf9800aa57a6Ae2cDD81d572;
  ...
  function staking(uint _amount, uint _duration) external {
    ...
    s.isStaker = true;
    s.level = _amountLevel - 1 + (_duration * 3) ;

    totalRewardAmount += _amount.mul(rate[s.level]).mul(decimals).div(1000);
    BUSD.transferFrom(msg.sender, address(this), _amount * decimals);
 
    totalStakeAmount += _amount * decimals;
  }
  ...
  function _calcReward(uint256 _duration) public returns(uint256 reward) {
    ...
    reward = _pastTime.mul(s.stakingAmount).mul(rate[s.level]).div(1000).div(s.stakingDuration);
  ...
  function unStaking(uint256 _duration) public {
    ...
    require(s.isStaker == true, "You are not staker.");
    uint256 rewardAmount = _calcReward(_duration);
    if (block.timestamp > s.expireDate) {
        unStakableAmount = s.stakingAmount + rewardAmount;
    } else {
        if(_duration == 0) unStakableAmount = s.stakingAmount.mul(80).div(100);
        else if(_duration == 1) unStakableAmount = s.stakingAmount.mul(85).div(100);
        else unStakableAmount = rewardAmount;
    }
    BUSD.transfer(msg.sender, unStakableAmount);
    totalStakeAmount -= s.stakingAmount;
  }
  ...
  function getBenefit() public onlyOwner {
    uint256 withdrawalAmount = BUSD.balanceOf(address(this)) - totalStakeAmount - totalRewardAmount;
    uint airdropAmount = withdrawalAmount.mul(40).div(100);
    uint teamAmount = withdrawalAmount.mul(60).div(100);
    BUSD.transfer(owner(), airdropAmount);
