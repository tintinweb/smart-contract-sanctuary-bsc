...
contract xYSLBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        ...
        convertBUSDToLP((amount * percentYSL/100), (reservexYSL * (amount * percentYSL/100)) / reservesBUSD, Admin.BUSD(), Admin.xYSL(), address(xyslBUSD));
        ...
        emit Deposit("xYSLBUSDVault",address(this),user,amount,block.number,block.timestamp);
    }
    ...
    */
    ...
    function distribute(uint256 _rebaseReward) external nonReentrant{
        currentReward = currentReward + ((_rebaseReward * 10 ** 18)/(IERC20(xyslBUSD).totalSupply()));
    }
