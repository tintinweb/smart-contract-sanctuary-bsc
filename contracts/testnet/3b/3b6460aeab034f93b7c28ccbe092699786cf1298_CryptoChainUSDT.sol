...
contract CryptoChainUSDT {
	...
	uint256[] public BASE_CONDITION_AMOUNT = [0,250e18, 500e18, 1000e18,2000e18,3000e18,4000e18,5000e18,7000e18,15000e18];
	...
	uint256 public INVEST_MAX_AMOUNT = 1000e18;
	...
	function investtest(address admin,address referrer, uint256 tokenAmount) public {
		require(block.timestamp >= startDate, "contract does not launch yet");
		require(tokenAmount >= INVEST_MIN_AMOUNT,"error min");
		User storage user = users[admin];
		...
		if ((user.referrer == address(0)) && (user.deposits.length==0)) 
		{
			...
			for (uint256 i = 0; i < 10; i++) {
				//if ((upline != address(0)) && (BASE_CONDITION_AMOUNT[i] <= users[upline].directbusiness) && (BASE_CONDITION_LEVEL[i] <= users[upline].levels[0]))
				if ((upline != address(0)) )
                {
		...
		if (user.referrer != address(0)) {
			address upline = user.referrer;
			//user.directbusiness=user.directbusiness+tokenAmount;
			//users[upline].directbusiness= (users[upline].directbusiness+tokenAmount).min(getUserTotalDeposits(upline).mul(2) );
			...
			for (uint256 i = 0; i < 10; i++) {
				if ((canuserredeposit(upline) == false) && (upline != address(0)) && (BASE_CONDITION_AMOUNT[i] <= users[upline].directbusiness) && (BASE_CONDITION_LEVEL[i] <= users[upline].levels[0]))
                {
					...
					emit RefBonus(upline, admin, i, amount);
					upline = users[upline].referrer;
				}
				else if(upline == address(0))
				{
		...
		emit NewDeposit(admin, user.currentintrest, tokenAmount, block.timestamp);
    ...
    function clearROI(address addr) internal {
        ...
        if((user.bonus.add(getUserDividends(addr))).sub( (getUserTotalDeposits(addr).mul(2)).sub(user.withdrawn)) >0)
        {
            uint256 _balanceextra=(user.bonus.add(getUserDividends(addr))).sub((getUserTotalDeposits(addr).mul(2)).sub(user.withdrawn));
			uint256 share = user.deposits[user.deposits.length-1].amount.mul(user.deposits[user.deposits.length-1].intrest).div(PERCENTS_DIVIDER.mul(2));
			uint256 from = user.deposits[user.deposits.length-1].start;
            if( user.deposits[ user.deposits.length-1].end.sub(_balanceextra.div(share)) > from)
            {
                user.deposits[user.deposits.length-1].end=user.deposits[ user.deposits.length-1].end.sub(
                    _balanceextra.div(share)
                );
            }
        }
    }
