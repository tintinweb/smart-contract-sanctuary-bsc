...
contract SafeswapFactory is ISafeswapFactory {
    ...
    function whitelistAddress(address account) external onlyOwner() {
        require((isBlacklistedStatus[account] == true),"Already Whitelisted");
...
interface ISafeswapFactory {
    ...
    // function approveLiquidityPartner(address) external;
}
...
contract SafeswapERC20 is ISafeswapERC20 {
  ...
  //  address router = ISafeswapFactory(factory).setRouter();
    ...
    function _transfer(address from, address to, uint value) private {
        //require(to == router,"You can only trade through SafeSwap");
        balanceOf[from] = balanceOf[from].sub(value);
        balanceOf[to] = balanceOf[to].add(value);
