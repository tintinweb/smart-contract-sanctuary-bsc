...
contract FixedSwapv3 is Pausable, Whitelist {
 ...
 //   uint256 public secondUnlockDate;
    ...
    event DebugEvent(uint256 purchaseId, uint256 val);
  ...
  function setUnlockDates(uint _startDate, uint _endDate, uint256 _distributionDate, uint _lastUnlockDate) public onlyOwner {
    ...
    function ceil(uint a, uint m) public view returns (uint ) {
        return ((a + m - 1) / m) * m;
    }
    ...
    function getLocked(uint256 purchase_id) public view returns(uint) {
        if(block.timestamp > lastUnlockDate) {
            ...
            return purchases[purchase_id].amount.sub( purchases[purchase_id].amount.mul(selectedUnlockPerc).div(10000));
        }
    }

     function getLocked2(uint256 purchase_id) public view returns(uint) {
        if(block.timestamp > lastUnlockDate) {
            return 0;
        } else {
            uint256 diff = lastUnlockDate - now;
            uint256 totalDuration = lastUnlockDate - distributionDate;
            // emit DebugEvent(0, totalDuration);
            uint256 ratio = diff.mul(100000).div(totalDuration);
            //emit DebugEvent(1, ratio);

            uint256 ratio2 = ceil(ratio, 12500);
          ...
          if (ratio2 < 0) {
            ...
            // 1600
            uint256 locked =  purchases[purchase_id].amount.mul(ratio2).div(100000);
           //  emit DebugEvent(3, ratio);
            if (locked < 0) {
                locked = 0;
            }
            // 2000
           // uint256 tge = purchases[purchase_id].amount.mul(125).div(1000);
