...
contract Lottery is Ownable{
  ...
  struct  _player{
    ...
    function randomNumberSelector(uint256 _level)
        ...
        {
          return uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,fin[_level].players)));
        }
    ...
    function selectWinner(uint256 _level) public onlyOwner {
           ...
           uint256 winerRatio =((fin[_level].received_entry)/(fin[_level].total_entry))*(100);
        ...
        winner=fin[_level].players[randomNumberSelector(_level) % fin[_level].players.length];
