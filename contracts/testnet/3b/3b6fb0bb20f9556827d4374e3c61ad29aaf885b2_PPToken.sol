...
// File: contracts/FavourToken.sol
...
contract Owned is Context {
    ...
    modifier onlyOwner {
        require(_msgSender() == _owner, "FavourToken: Only Owner can perform this task");
        _;
    }
    ...
    function acceptOwnership() public {
        require(_msgSender() == _newOwner, "FavourToken: Token Contract Ownership has not been set for the address");
 ...
 *
 */
contract PPToken is IERC20, Owned {
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        ...
        require(spender != address(0), "FavourToken: approve to the zero address");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        require(amount <= _allowances[sender][_msgSender()],"FavourToken: Check for approved token count failed");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(recipient != address(0),"FavourToken: Cannot have recipient as zero address");
        require(sender != address(0),"FavourToken: Cannot have sender as zero address");
        require(_balances[sender] >= amount,"FavourToken: Insufficient Balance" );
        require(_balances[recipient] + amount >= _balances[recipient],"FavourToken: Balance check failed");
    ...
    function burn(address account, uint256 amount) public onlyOwner {
        ...
        require(account != address(0), "FavourToken: burn from the zero address");

        if( _balances[account] == _unlockedTokens[account]){
            _unlockedTokens[account] = _unlockedTokens[account].sub(amount, "FavourToken: burn amount exceeds balance");
        }

        _balances[account] = _balances[account].sub(amount, "FavourToken: burn amount exceeds balance");
        ...
        if(account != _msgSender()){
            ...
            require(amount <= _allowances[account][_msgSender()],"FavourToken: Check for approved token count failed");
    ...
    function _updateUnLockedTokens(address _from, uint tokens) private returns (bool success) {
        ...
        // if _unlockedTokens are less than "tokens" of "to", update _unlockedTokens by checking record with "now" time
            ...
            // otherwise revert
                ...
                revert("FavourToken: Insufficient unlocked tokens");
            }
        }
    }
