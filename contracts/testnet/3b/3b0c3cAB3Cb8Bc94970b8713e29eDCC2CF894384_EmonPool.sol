...
// File: Emonster/CakePool/interfaces/IVEmon.sol
...
// File: Emonster/CakePool/interfaces/IBoostContract.sol


pragma solidity ^0.8.0;

interface IBoostContract {
    function onEmonPoolUpdate(
        address _user,
...
// File: Emonster/CakePool/interfaces/IMasterChef.sol
...
// File: Emonster/CakePool/CakePool.sol
...
contract EmonPool is Ownable, Pausable {
    ...
    event NewVEmonContract(address VEmon);
     ...
     * @param _pid: emon pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchef.deposit(emonPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the emon owner address or the operator address.
     */
    modifier onlyOperatorOrEmonOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or emon owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onEmonPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    /**
     * @notice Unlock user emon funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrEmonOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Emon Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VEmon != address(0)) {
            IVEmon(VEmon).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.emonAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Emon Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VEmon != address(0)) {
            IVEmon(VEmon).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending EMON tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingEmon = masterchef.pendingEmon(emonPoolPID, address(this));
        if (pendingEmon > 0) {
            uint256 balBefore = available();
            masterchef.withdraw(emonPoolPID, 0);
    ...
    /**
     * @notice Set VEmon Contract address
     * @dev Callable by the contract admin.
     */
    function setVEmonContract(address _VEmon) external onlyAdmin {
        require(_VEmon != address(0), "Cannot be zero address");
        VEmon = _VEmon;
        emit NewVEmonContract(VEmon);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Emon Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingEmonRewards();
     ...
     */
    function calculateTotalPendingEmonRewards() public view returns (uint256) {
        uint256 amount = masterchef.pendingEmon(emonPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingEmonRewards()) * (1e18)) / totalShares);
    }
