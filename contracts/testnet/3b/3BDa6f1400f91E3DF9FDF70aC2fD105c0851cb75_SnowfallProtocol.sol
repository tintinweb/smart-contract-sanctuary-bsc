...
contract  SnowfallProtocol is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    address payable public marketingwallet; // market wallet address
    ...
    uint256 public minTokenToSwap = 1 * 1e6  ether; // 1M amount will trigger the swap and add liquidity
    ...
    uint256 private _accumulatedautoburn;
        ...
        uint256 percentage = tAmount
            .mul(
                _currentRedistributionFee
                    ...
                    .add(_currentmarketingwalletFee)
                    .add(_currentautoburnFee)
            )
            .div(1e3);
        return percentage;
    }
    ...
    // take fees for liquidity, market and earth autoburn
    function _takeAllFee(uint256 tAmount, uint256 currentRate) internal {
        ...
        if(tFee > 0){
            _accumulatedLiquidity = _accumulatedLiquidity.add(tAmount.mul(_currentLiquidityFee).div(1e3));
             ...
             _tOwned[marketingwallet] += tAmount.mul(_currentmarketingwalletFee).div(1e3);
             emit Transfer(_msgSender(), marketingwallet, tAmount.mul(_currentmarketingwalletFee).div(1e3));
             _tOwned[burnAddress] += tAmount.mul(_currentautoburnFee).div(1e3);
             emit Transfer(_msgSender(), burnAddress, tAmount.mul(_currentautoburnFee).div(1e3));
