...
contract RaoToken is Context, IERC20, Ownable {
    ...
    /// @notice Address for burning
    address public constant HYPERBURNADDR = address(0);
    ...
    constructor (
        ...
        _previosTreasuryFee = _treasuryFee;
        _previosTxFee = _txFee;
        maxTxAmount = ((_supply * 5) / 1000) * 10 ** _decimals; // 5 000 000 * 10 ** 18
        numTokensSellToAddToLiquidity = ((_supply * 5) / 1000) * 10 ** _decimals; // 5 000 000 * 10 ** 18
    ...
    function _getValuesInternal(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rTxFee, tTransferAmount, tTxFee);
    }
    ...
    function _transferInternal(address sender, address recipient, uint256 tAmount) private {
         (
            uint256 rAmount,
    ...
    /// @notice The owner can withdraw Rao Token from the contract address
