...
contract SolutionBear is BEP20 {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "SUL::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "SUL::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "SUL::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "SUL::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "SUL::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    function _transfer(
        ...
        require(sender != address(0), "SUL: transfer from the zero address");
        require(recipient != address(0), "SUL: transfer to the zero address");

        _balances[sender] = _balances[sender].sub(
            amount,
            "SUL: transfer amount exceeds balance"
        );
