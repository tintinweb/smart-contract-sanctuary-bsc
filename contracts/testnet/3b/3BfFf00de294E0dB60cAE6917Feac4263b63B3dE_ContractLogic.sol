    ...
    OwnableUpgradeable
{
    ...
    modifier isRouter(address _sender) {
        ...
        require(Address.isContract(_sender), "Address is not a contract.");

        if (_accountsTier[_sender] == 0) {
            try IUniswapV2Router02(_sender).factory() returns (
                address factory
            ) {
    ...
    function __ContractLogic_tiers_init() internal initializer {
        _defaultFees = _addTier(0, 0, 0, 0, 0, address(0), address(0));
        _addTier(0, 0, 0, 0, 0, address(0), address(0));
        _addTier(0, 0, 0, 0, 0, address(0), address(0));
