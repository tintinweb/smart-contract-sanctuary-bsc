...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function stake(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        require(
            _poolInfo.token.balanceOf(msg.sender) >= _amount,
            "Insufficient token"
        );

        // Transfer token into the contract
        bool _status = _poolInfo.token.transferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function unstake(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        require(
            _poolInfo.token.balanceOf(address(this)) >= _amount,
    ...
    function registerPool(address _token) external onlyOwner {
        poolInfo.push(PoolInfo({token: IERC20(_token), tvl: 0, paused: false}));
