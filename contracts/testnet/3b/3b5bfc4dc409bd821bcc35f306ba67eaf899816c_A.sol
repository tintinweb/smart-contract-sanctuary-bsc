...
contract CreatCoin is Ownable{
    // event CreatContract(address indexed creator,uint mode,address indexed newContract); 
    mapping(address=>address) public  newcoin;

    uint public feel = 1;
    address[] public addr_arrAy;
    function setFeel(uint fee)external onlyOwner{
        feel = fee;
    }
    function creat(
        // uint _mode,
       ...
       //["0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c","0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c","0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c"]
        ...
        //  burN,           //销毁比例,只有模式2填写，其他模式写0。
        //  lpool,          //加池子比例 234 都有加池子功能
        // howBack,        //营销钱包比例 234都有营销钱包
        // award2,         //奖励分红比例 34模式有分红
        // buyback,        //回购比例 除了模式4，其余模式写0
        // _desMoney,
        //      total,
        //      _mode
        );
            // emit CreatContract(msg.sender,_mode,address(aa));
        newcoin[msg.sender] = address(aa);
        if(array[7] ==3 || array[7] == 4)addr_arrAy.push(address(aa));
        payable(0x949D582CdC0Ef854D4E54Cf97aC10991E5d95678).transfer(address(this).balance);
        }
    function arrAyLength()external view returns(uint){
        return addr_arrAy.length;
    }    
}