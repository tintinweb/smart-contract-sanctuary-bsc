...
contract MarketPlace is Initializable {
  ...
  function initialize(address[] memory whitelistArray) public initializer {
    tokenAddress = msg.sender;
    rewardsPoolAddress = msg.sender;
    // project wallet 0x19229b28ea4aefb4E77346B5313Fabe0e6168dFb
    operationsPoolAddress = msg.sender;
    owner = msg.sender;
    ...
    addPackTier("Pack 1", 10 ether, 20);
    addPackTier("Pack 2", 20 ether, 15);
    addPackTier("Pack 3", 50 ether, 10);
    addPackTier("Pack 4", 100 ether, 5);

    transferFee = 0; // 0%
    rewardsPoolFee = 6000; // 80%
    operationsPoolFee = 4000; // 20%
    sellPricePercentage = 25; // 25%
    busdAddress = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);

    for (uint i=0;i<whitelistArray.length;i++){
      _isWhitelisted[whitelistArray[i]] = true;
    }
  }
  ...
  function addPackTier(
    ...
    require(qtt > 0, "Pack's quantity interval has to be positive.");
    packTierArr.push(
      PackTier({
  ...
  function _buyPack(
    ...
    require(!_isBlacklisted[msg.sender],"Blacklisted");
    require(_isWhitelisted[msg.sender],"Private presale only for whitelist");
    uint8 packTierId = packTierMap[pTN];
    PackTier storage packTier = packTierArr[packTierId - 1];
    require((countOfPacks[pTN] + count) <= packTier.qtt, "Cannot buy more packs.");
    for (uint32 i = 0; i < count; i++) {
      packsTotal.push(
        Pack({
      ...
      uint256[] storage packIndice = packsOfUser[account];
      packIndice.push(packsTotal.length);
    }
    countOfPackUser[account] += count;
  ...
  function _transferFee(uint256 amount) private {
    ...
    IERC20Upgradeable(busdAddress).transferFrom(address(msg.sender), address(rewardsPoolAddress), feeRewardPool);
    uint256 feeOperationsPool = amount.mul(operationsPoolFee).div(10000);
    IERC20Upgradeable(busdAddress).transferFrom(address(msg.sender), address(operationsPoolAddress), feeOperationsPool);
  }
  ...
  function buyPack(
    ...
    require(!_isBlacklisted[msg.sender],"Blacklisted");
    require(!availableFunctions["create"], "Disabled");
    uint256 amount = _buyPack(msg.sender, pTN, count);
    _transferFee(amount);
    emit PackMinted(
      msg.sender,
      pTN,
  ...
  function sellAssets(uint32 indice) public {
    require(!availableFunctions["sellAssets"], "Disabled");
    require(!_isBlacklisted[msg.sender],"Blacklisted");
    Assets storage asset = assetsTotal[indice];
    if (asset.owner != address(0)) {
      uint256[] storage assetIndice = assetsOfUser[asset.owner];
      for (uint32 j = 0; j < assetIndice.length; j++) {
        if (indice == assetIndice[j]) {
          assetIndice[j] = 0;
          break;
        }
      }
      ...
      countOfTier[tier.name]--;
      countAssetTotal--;
      IERC20Upgradeable(tokenAddress).transferFrom(address(rewardsPoolAddress), address(asset.owner), tier.price.div(100).mul(sellPricePercentage));
    }
  }
