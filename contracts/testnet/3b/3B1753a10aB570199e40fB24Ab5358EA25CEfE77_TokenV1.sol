...
contract TokenV1 is BaseContract, ERC20Upgradeable {
    ...
    function _shouldAddLiquidity() internal view returns (bool) {
        return !_inSwap && block.timestamp >= (_lastAddLiquidityTime + 2 days);
    }
    ...
    function _transfer(
        ...
        if (
            _properties.lpAddress == address(0) ||
        ...
        if (
            from_ == _properties.vaultAddress || to_ == _properties.vaultAddress
        ) {
    ...
    function _beforeTokenTransfer(
        ...
        if (_restrictSwaps) {
            if (from == _properties.lpAddress) {
                require(
                    _addressClearedForSwap[to],
                    "Unable to swap directly from liquidity pool"
                );
            }
            if (to == _properties.lpAddress) {
                require(
                    _addressClearedForSwap[from],
                    "Unable to swap directly from liquidity pool"
                );
            }
        }
