    ...
    Pausable
{
    ...
    function getOwnedTokens(address account) external view returns (uint256[] memory unlockedTokens) {
        uint256 tokenAmount = token.balanceOf(account);
        unlockedTokens = new uint256[](tokenAmount);

        for(uint256 i = 0; i < tokenAmount; i++) {
            unlockedTokens[i] = token.tokenOfOwnerByIndex(account, i);
        }
    }
