...
contract AirdropFactory is Ownable, Pausable {
    ...
    {
        require(_feeAddress != address(0), "Invalid parameter");
    ...
    function createairdrop(string memory _name, address _token) public payable whenNotPaused {
        ...
        tokenToairdrop[msg.sender] = clone;
...
contract Airdrop is Initializable, ReentrancyGuard, Ownable
{
   ...
   function initialize (string memory _name, IERC20 _token) public initializer 
   {
    ...
    {
        require(block.timestamp < startTime , "Airdrop has been started");
        require(isCancel == false, "Airdrop cancelled");
    ...
    { 
       startTime = timestamp == 0 ? block.timestamp : timestamp;
    }

    function calculateTokens(address account) public view returns(uint256 tokens)
    {
        uint256 available = claimed[account];
        return tokens = addressToamount[currentRemoveCount][account] - available;
    }
    ...
    function pendingClaim(address account) public view returns (uint256 tokens)
    {
            ...
            if (vpar.vesting)
            {
                ...
                {
                    tokens += (uclaimed * vpar.vestingRelease[i]) / 10000;
                }
            } else 
            {
                tokens = uclaimed;
