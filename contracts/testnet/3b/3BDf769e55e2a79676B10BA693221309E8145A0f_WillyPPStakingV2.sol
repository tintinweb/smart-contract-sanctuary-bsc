...
contract WillyPPStakingV2 is NFTStaking {
    ...
    function unstakeByAmount(uint256 _input) external nonReentrant {
        require(
            stakerToTokenIds[msg.sender].length > 0,
            "You do not have any NFT staked"
        );
        require(
            stakerToTokenIds[msg.sender].length >= _input,
            "You cannot unstake more tokens than you have staked"
        );

        uint256 totalRewards = 0;
        for (
            uint256 i = stakerToTokenIds[msg.sender].length;
            i > stakerToTokenIds[msg.sender].length - _input;
            i--
        ) {
            uint256 tokenId = stakerToTokenIds[msg.sender][i - 1];
