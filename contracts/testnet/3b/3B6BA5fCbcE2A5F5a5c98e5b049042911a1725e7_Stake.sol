...
contract Stake is Ownable {
    ...
    function userLevel( address _user) external view returns (uint level) {
        if(userStaked[_user] < 250 ether) level = 1;
        else if(userStaked[_user] < 500 ether) level = 2;
        else if(userStaked[_user] < 1000 ether) level = 3;
        else if(userStaked[_user] < 2500 ether) level = 4;
        else level = 5;
    }
