...
contract INRx is  Ownable, Pausable, Blacklistable,Initializable, IERC20Permit, EIP712  {
    ...
    function mint(address _to, uint256 _amount) external override whenNotPaused onlyMinters notBlacklisted(msg.sender) notBlacklisted(_to) returns (bool) {
        require(_to != address(0), "ERC20: mint to the zero address");
        require(_amount > 0, "ERC20: mint amount not greater than 0");

        uint256 mintingAllowedAmount = minterAllowed[msg.sender];
        require(_amount <= mintingAllowedAmount,"ERC20: mint amount exceeds minterAllowance");
