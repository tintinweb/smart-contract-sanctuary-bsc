...
contract EatherBonus is Auth, EIP712Upgradeable {
  ...
  function getBonus(bytes calldata _signature, uint _timestamp, uint _quantity) external {
    ...
    bool success = _haveBonus(_signature);
    if (success) {
  ...
  function _haveBonus(bytes calldata _signature) private view returns (bool) {
    (uint l0, uint l1) = lpToken.getReserves();
    uint randomNumber = Math.genRandomNumber(string(_signature), l0 + l1);
