...
interface StakeContract {
    ...
    function verifyPidValid(uint256 _pid) external view returns(bool);
}
...
contract VoteContractTestV3 is Ownable {
    ...
    function destroyVote(uint256 pid, address addr) public {
        ...
        if(userData[addr].remainingVotes > userClaimVotes[pid][addr]) {
        ...
        } else if (userData[addr].remainingVotes < userClaimVotes[pid][addr]) {
