...
contract DecubateWalletStore is Initializable, OwnableUpgradeable, AccessControlUpgradeable {
  struct User {
    bool isVerified;
    uint256 arrayIndex;
  }
  ...
  function setManager(address _address) public onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
  ...
  function removeManager(address _address) public onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    if (hasRole(MANAGER_ROLE, _address)) {
