...
contract Trade is Ownable{
    ...
    function buy(Sale calldata _saleToken, Sig calldata _sellerSig, Sig calldata _buyerSig, address _royaltyRecipent, uint256 _royaltyFee  ) external payable {
        ...
        require(_saleToken.buyToken == address(0x0) || isApproved[_saleToken.buyToken],"Not approved Token");
    ...
    function takeFee(Sale calldata _saleToken, address _royaltyRecipent, uint256 _royaltyFee) internal {
        ...
        if(_saleToken.buyToken == address(0x0)){
            ...
            IERC20(_saleToken.buyToken).transferFrom(_msgSender(),_royaltyRecipent,royaltyFee);
            IERC20(_saleToken.buyToken).transferFrom(_msgSender(),address(this),beneficiary);
            IERC20(_saleToken.buyToken).transferFrom(_msgSender(),_saleToken.owner,_saleToken.buyAmount - royaltyFee - beneficiary);
        }
    }
