...
contract GaiaLifeMother is Initializable {
        ...
        returns (bool)
    {
        if (
            keccak256(
                bytes(
                    bytes32ToString(soulProfiles[_profiler][_soul].fingerprint)
                )
            ) == zeroHash
        ) {
            return false;
    ...
    /*
    function link(address soulHolder) external {
        //require(!linked, "Soul Already Linked");
        bytes32 color = keccak256(abi.encodePacked(Strings.toString(0))); // by default the color of the soul is black until some light refracts on it
        bytes32 weight = keccak256(
            abi.encodePacked(Strings.toString(21 ether))
        ); // by default soul weight are 21 grams
        bytes32 shadowDepth = keccak256(abi.encodePacked(Strings.toString(0)));
        bytes32 holder = keccak256(abi.encodePacked(soulHolder));
        bytes32 fingerprint = keccak256(
            abi.encodePacked(color, weight, shadowDepth, holder)
        );
    }
    */
...
My own implementation of Soul Bound inspired by Vitalik's whitepaper.
Soul ownership is a perfect use case, because once received it cannot be transferred, not being able to be transferred but lost.

the fingerprint is defined by the depth of iris grooves

 */

struct Soul {
    bytes32 fingerprint; // defined by the depth of iris grooves
    bool linked; // determines the ability to remain in the environment
