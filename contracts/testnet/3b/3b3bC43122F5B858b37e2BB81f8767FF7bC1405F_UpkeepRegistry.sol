...
contract UpkeepRegistry is UUPSUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable, KeeperCompatibleInterface {
    address[] public _upkeeps;
    uint256 public _lastUpkeepPerformedIndex;

    uint256 public _maxUpkeeps;

    event UpkeepAdded(address upkeep);
    event UpkeepRemoved(address upkeep);
    event UpkeepPerformed(address upkeep);
    ...
    /**
     * Admins can remove upkeeps, and upkeeps can remove themselves too.
     */
    ...
    function checkUpkeep(bytes calldata checkData) external override view returns (bool, bytes memory) {
        ...
        for (uint256 i=_lastUpkeepPerformedIndex+1; i<upkeepsCount; i++) {
            ...
            (bool upkeepNeeded, bytes memory performData) = upkeep.checkUpkeep(checkData);
            if (upkeepNeeded)
    ...
    function performUpkeep(bytes calldata performData) external override whenNotPaused {
        ...
        for (uint256 i=0; i<upkeepsCount; i++) {
            if (upkeepAddress == _upkeeps[i]) {
                ...
                upkeep.performUpkeep(upkeepData);

                _lastUpkeepPerformedIndex = i;
                emit UpkeepPerformed(upkeepAddress);

                if (upkeep.shouldRemoveUpkeep()) {
                    if (i != upkeepsCount - 1)
                        _upkeeps[i] = _upkeeps[upkeepsCount - 1];
                    _upkeeps.pop();
