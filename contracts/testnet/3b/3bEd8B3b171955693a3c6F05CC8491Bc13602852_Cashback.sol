...
contract Cashback is Ownable {
    ...
    // Pancake swap addresses (testnet)
    address routerAddress = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    address BUSD = 0x665aE6c8B332cCE9B1B50d9B2c79d1731516d2fB;
    ...
    constructor(IERC20 _fitToken, address _backend) {
        fitToken = _fitToken;
        busdToken = IERC20(BUSD);
        pancakeRouter = IPancakeRouter(routerAddress);
        backend = _backend;
        busdToken.approve(routerAddress, 2**256 - 1); // approve all BUSD tokens to pancake router
    }
    ...
    function stake(uint256 _amount, bytes memory _sig) external {
        require(cashbackPool() >= _amount, 'Not enough tokens in cashback pool');
        require(verify(_amount, _sig), 'Wrong signature');
        nonce[msg.sender]++;
        flexibleStaking.stakeFromCashback(msg.sender, _amount);
    }

    function withdraw(uint256 _amount, bytes memory _sig) external {
        require(cashbackPool() >= _amount, 'Not enough tokens in cashback pool');
        require(verify(_amount, _sig), 'Wrong signature');
        nonce[msg.sender]++;
    ...
    function stakeWithBNB(
        ...
        require(
            _tokenAmount > tokenAmountForBNB(_bnbAmount),
            'Not enough tokens for swapping to BNB'
        );
        uint256 tokenSwapped = swapTokensForBNB(_bnbAmount);
        payable(backend).transfer(_bnbAmount / 3);
        payable(_stakeholder).transfer((_bnbAmount * 2) / 3);
        flexibleStaking.stakeFromCashback(_stakeholder, _tokenAmount - tokenSwapped);
    }
    ...
    function swapTokensForBNB(uint256 _bnbAmount) public onlyBackend returns (uint256) {
        ...
        return
            pancakeRouter.swapTokensForExactETH(
                _bnbAmount,
                pancakeRouter.getAmountsIn(_bnbAmount, pair)[0], // TODO: change to _amount in stake function
                pair,
                backend,
    ...
    function tokenAmountForBNB(uint256 _bnbAmount) public view returns (uint256) {
        address[] memory pair = new address[](2);
        pair[0] = BUSD;
        pair[1] = WBNB;
    ...
    function verify(uint256 _amount, bytes memory _sig) public view returns (bool) {
        bytes32 hashedMessage = ethMessageHash(msg.sender, nonce[msg.sender], _amount);
...
contract FlexibleStaking is Ownable {
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            ...
            (((block.timestamp - lastUpdateTime) * rewardRate * 1e32) / totalSupply);
    }
    ...
    function _stake(address _staker, uint256 _amount, address _payer) internal updateReward(_staker){
    ...
    function stakeFromCashback(address _user, uint256 _amount) external onlyCashback {
        _stake(_user, _amount, msg.sender);
    }

    function withdraw(uint256 _amount) external updateReward(msg.sender) {
        require(_amount > 0, 'Amount should be more than 0');
        require(totalSupply >= _amount, 'Total supply is less than amount to withdraw');
        require(
            balances[msg.sender] >= _amount,
            'Balance is less than amount to withdraw'
        );
        totalSupply -= _amount;
        balances[msg.sender] -= _amount;
        fitToken.transfer(msg.sender, _amount);
    ...
    function userStake(address _account) public view returns (uint256) {
