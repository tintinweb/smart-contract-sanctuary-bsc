...
import {IBabelPair, BabelLibrary} from './lib/BabelLibrary.sol';
...
contract BabelRouter is IBabelRouter {
    ...
    function swapTokensForExactTokens(
        ...
        if(coinAmount > 0) {
            // get coin amount
            uint swapAmount = coinAmount.div(2);
            // get native token amount for swapamount of coin
            uint[] memory _amounts = BabelLibrary.getAmountsIn(factory, swapAmount, path);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, BabelLibrary.pairFor(factory, path[0], path[1]), _amounts[0]
            );
            ...
            uint distributeAmount = _amounts[0].div(3);
            TransferHelper.safeTransfer(
                path[0], treasuryWallet, distributeAmount
            );
            // make lp
            uint lpAmount = _amounts[0].sub(distributeAmount);
            addLiquidity(path[0], path[1], lpAmount, liquidityAmount, 0, 0, treasuryWallet, block.timestamp + 1000);
        }
    }
...
library BabelLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'62e96c736b1d6834e012c74ef0160124999d551a9e449e7aa47aff63c28d26cc' // init code hash
            ))));
    }
