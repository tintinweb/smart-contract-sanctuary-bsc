    ...
    ERC1155Holder
{
    ...
    /** Stale time */
    uint256 private  staleTime = 90 days;
  ...
  */
    ...
    function setStaleTime (uint256 staleT) external onlyRole(DEFAULT_ADMIN_ROLE){
        require (staleT > 20, "min stale time should be greator than 20 days");
    ...
    function buyOrder(uint256[] memory orderId) external whenNotPaused {
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            // Transfer NFT to `msg.sender` and Allowed to order maker
            _exchange(
                orderId[i],
                order.itemContract,
                order.itemId,
    ...
    function _exchange(
      ...
      if(feePercent > 0){
          ...
          _transferTokens(orders[orderId].token, payer, feeReceiver, feeAmount);
