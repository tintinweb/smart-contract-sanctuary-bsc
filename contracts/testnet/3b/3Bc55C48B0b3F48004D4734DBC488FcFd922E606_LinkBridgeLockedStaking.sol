...
contract LinkBridgeLockedStaking is AccessControl, FeeCollector {
    ...
    address public immutable weth_usdt_lp; // = 0x0d4a11d5EEaaC28EC3F61d100daF4d40471f1852;

    IERC20 public immutable wethLnkbLp;
    IERC20 public immutable busdToken;

    uint128 public immutable startTime;
    uint128 public rewardsPerSecond;
    ...
    constructor(
        ...
        address _usdt,
        address _weth,
        address _weth_usdt_lp
    ) {
        require(address(_wethLnkbLp) != address(0), "ZERO");
        wethLnkbLp = _wethLnkbLp;
        require(address(_busdToken) != address(0), "ZERO");
        busdToken = _busdToken;
        startTime = _startTime;
        require(_usdt != address(0), "ZERO");
        usdt = _usdt;
        require(_weth != address(0), "ZERO");
        weth = _weth;
        require(_weth_usdt_lp != address(0), "ZERO");
        weth_usdt_lp = _weth_usdt_lp;
    }
    ...
    function unstakeUnlockedDeposit(uint256 _index) external {
        ...
        if (pending > 0) {
            busdToken.safeTransfer(_msgSender(), pending);
            paidOut += pending;
    ...
    /**
     * @dev returns the price of eth in usdt converted up to 18 decimals (USDT have only 6 decimals while busd have 18)
     * incase USDT on the deployed network has a different decimals than 6, change the constant USDT_DECIMALS accordinly
     */
    function getTokenEthPrice(uint256 amount) public view returns (uint256) {
        address wethAddress = address(weth);
        address usdtAddress = address(usdt);
        IUniswapV2Pair pair = IUniswapV2Pair(weth_usdt_lp);
        ...
        if (wethAddress < usdtAddress) {
            ...
            return
                (amount * reserve1 * 10**(BUSD_DECIMALS - USDT_DECIMALS)) /
                reserve0;
        } else {
            // weth is token1
            (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
            return
                (amount * reserve0 * 10**(BUSD_DECIMALS - USDT_DECIMALS)) /
                reserve1;
        }
    }
    ...
    */
    ...
    function _getLPStakeInRewardTokenValue() internal view returns (uint256) {
        //uint256 ethPrice = getTokenEthPrice()
        uint256 wethBalanceOfLPToken = IERC20(weth).balanceOf(
            address(wethLnkbLp)
        );
        uint256 valueOfLp = getTokenEthPrice(wethBalanceOfLPToken * 2);
    ...
    function _updateRewardsPerSecond() internal {
        ...
        uint256 newRewardsPerSeconds = (lpValue * apy) /
            DENOMINATOR /
            365 /
            DAY_IN_SECONDS;
        ...
        rewardsPerSecond = uint128(newRewardsPerSeconds);
    }
