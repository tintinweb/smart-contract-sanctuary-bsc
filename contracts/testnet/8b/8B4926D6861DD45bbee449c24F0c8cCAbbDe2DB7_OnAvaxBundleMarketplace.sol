...
contract OnAvaxBundleMarketplace is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IOnAvaxTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "OnAvaxMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "OnAvaxMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IOnAvaxMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IOnAvaxMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
        ...
        nonReentrant
    {
        ...
        emit ItemSold(
            ...
            IOnAvaxMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)),
            offer.price
        );
