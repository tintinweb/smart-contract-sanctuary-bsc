...
contract TokenExchange is ITokenExchange, Caller, Fallback {
    ...
    function takeFixPrice(uint256 orderId_) external payable override {
        ...
        require(orderIdSet.removeUintSet(orderId_), "1317");
    ...
    function cancel_(uint256 orderId_, bool isPlatform) private {
        ...
        require(orderIdSet.removeUintSet(orderId_), "1353");
        delete order[orderId_];
        if (address(1) != order_.bidder) {
            config.getAsset().claimMainOrERC20(
                order_.quoteToken,
    ...
    function delivery(uint256[] memory orderId_) external override onlyCaller {
        ...
        for (uint256 i = 0; i < orderId_.length; i++) {
            ...
            if (address(1) == order_.bidder) {
                ...
                require(orderIdSet.removeUintSet(orderId__), "1348");
    ...
    function acceptOffer(uint256 orderId_) external override {
        ...
        require(orderIdSet.removeUintSet(orderId_), "1342");
        delete order[orderId_];
        (uint256 platformFee, uint256 creatorFee) = delivery_(orderId_, order_);
        emit AcceptEvent(
            orderId_,
