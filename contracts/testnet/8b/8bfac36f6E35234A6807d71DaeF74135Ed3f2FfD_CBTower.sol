...
contract CBTower is Ownable, ReentrancyGuard {
    ...
    // testnet - 0xF2Fca45eFaeCa40E2451aa501966A7aE1afFFdC8
    IERC20 public USDT = IERC20(0xF2Fca45eFaeCa40E2451aa501966A7aE1afFFdC8);
    ...
    mapping(address => mapping(uint8 => uint256)) public userFloorInvestment;
    ...
    function _payInvestors() internal {
        FloorInfo storage floor = floorInfo[currentFloor + 1];
        for (uint256 i = 0; i < investors.length; i ++) {
            UserInfo storage user = userInfo[investors[i]];
            uint256 reinvestAmount = userFloorInvestment[investors[i]][currentFloor] * user.reinvestRatio / 1000;
            userFloorInvestment[investors[i]][currentFloor + 1] += reinvestAmount;
            floor.totalInvested += reinvestAmount;
            if (user.reinvestRatio < 1000) {
                USDT.safeTransfer(investors[i], userFloorInvestment[investors[i]][currentFloor] - reinvestAmount);
            }
        }
    }
    ...
    function _invest(uint256 amount) internal returns (uint256 left){
        ...
        if (leftForCurrentFloor_ > amount) {
            ...
            userFloorInvestment[_msgSender()][currentFloor] += amount;
        } else {
            left = amount - leftForCurrentFloor_;
            floor.totalInvested += leftForCurrentFloor_;
            userFloorInvestment[_msgSender()][currentFloor] += leftForCurrentFloor_;
        }
