...
contract ReignToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function initialize(address busdAddress, address _router) public initializer {
    ...
    function _transferFrom(
        ...
        uint256 gonAmountReceived = shouldTakeFee(sender, recipient)
            ? takeFee(sender, recipient, gonAmountTo, getYield())
            : gonAmountTo;
    ...
    function checkSwapThreshold() external view returns (uint256) {
        return gonSwapThreshold.div(getYield());
    }
    ...
    mapping(address => uint256) public tokenBuyPerPerson;
    ...
    function setPresalePeriod(bool _value) external onlyOwner {
        isInPresale = _value;
    }
    ...
    function buyPresale(uint256 amount, bytes32[] calldata merkleProof) external {
        require(isInPresale, "Not yet in presale period");

        require(amount >= 100 ether, "You can't buy less than 100 BUSD");
        require(tokenBuyPerPerson[msg.sender] + amount <= 2500 ether, "You can't buy more than 2,5k BUSD");

        require(isAddressWhitelisted(msg.sender, merkleProof), "You are not whitelitsted");

        IERC20(busdToken).transferFrom(msg.sender, 0x71b3F97726a6220A779Ab3469780e2824c36Eb7e, amount);
    
        tokenBuyPerPerson[msg.sender] += amount;

        amount = amount.mul(10000).div(23);
    ...
    function getPresaleAmount(address who) external view returns(uint256) {
        return balanceOf(who) + newBalance[who];
    }
