...
contract HoldersPartition is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function _getTierIndexOf(address account, address[] memory sortedArr) internal pure returns (uint256) {
        ...
        uint256 tIndex = 0;
        bool tIsDeterminded = false;

        while(tIndex < 10 && !tIsDeterminded){
            if(100 >= (10 - tIndex ) * 10 + rank / sortedArr.length * 100 ){
                tIsDeterminded = true;
            }
            tIndex ++;
        }
    ...
    function _withdrawableDividendOf(address account, address[] memory sortedArr) internal view returns (uint256, uint256){
        uint256 tierIndex = _getTierIndexOf(account, sortedArr);
        uint256 percentage = tiers[tierIndex];
        uint256 defaultWalletCnt = sortedArr.length / 10;
        if(tierIndex == 10){
            if(sortedArr.length > defaultWalletCnt * tierIndex){
                defaultWalletCnt = sortedArr.length - defaultWalletCnt * tierIndex + defaultWalletCnt;
            }
        }
        uint256 dividend = percentage * totalBUSD / (100 * defaultWalletCnt);
        return (dividend, tierIndex);
    }
    ...
    function getTierIndexOf(address account) public view returns (uint256) {
        ...
        while(rank < sortedAddressArr.length && !isRankDeterminded) {
            if(sortedAddressArr[rank] == account){
                isRankDeterminded = true;
            }else{
                rank ++;
            }
        ...
        while(tIndex < 10 && !tIsDeterminded){
            if(100 >= (10 - tIndex ) * 10 + rank / sortedAddressArr.length * 100 ){
                tIsDeterminded = true;
            }
            tIndex ++;
        }
    ...
    function getTokenHoldersAvailableAmount() public view returns( uint256[] memory, address[] memory ){
        ...
        while(_lastProcessedIndex < totalHoldersCnt) {
            ...
            result1[_lastProcessedIndex] = availalbeAmount;
            result2[_lastProcessedIndex] = key;
            _lastProcessedIndex ++;
        }
    ...
    function getDivAmountArr() public view onlyOwner returns( uint256[] memory, address[] memory, uint256[] memory, uint256[] memory) {
        ...
        while(index < tokenHoldersAddressArr.length){
            ...
            (uint256 div, uint256 tierIndex) = _withdrawableDividendOf(account, tokenHoldersAddressArr);
            divAmountArr[index] = div;
