...
contract AuraStakingProgram  {
    ...
    constructor(address ownerAddress, IBEP20 _busdToken, IBEP20 _AuraToken) public 
    {
        ...
        stakingPrograms_.push(Program(6,43800,6));  //  730 Days
           ...
           for (index; index < numberOfDays; index++) {
               if(_dailyInterestRate + index <= _maxDailyInterest ){
                   secondsLeft -= INTEREST_CYCLE;
                     result += (_amount * (_dailyInterestRate + index) / 100000 * INTEREST_CYCLE) / (60);
               }
               else
               {
                 break;
            ...
            result += (((_amount.mul(_dailyInterestRate)).div(100000)) * secondsLeft) / (60);

            return result;

        }else{
            return (_amount * _dailyInterestRate / 100000 * (_now - _start)) / (60);
        }
