...
contract UnsToken is ERC20Burnable, Pausable, AccessControl {
    using SafeERC20 for IERC20;

    bytes32 public constant ICO_GOVERNANCE_ROLE = keccak256("ICO_GOVERNANCE_ROLE");
    bytes32 public constant BLACKLIST_ROLE = keccak256("BLACKLIST_ROLE");

    constructor(uint256 totalSupply) ERC20("UNS Token", "UNS") {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(ICO_GOVERNANCE_ROLE, _msgSender());
    ...
    function _beforeTokenTransfer(
        ...
        if (!hasRole(ICO_GOVERNANCE_ROLE, _msgSender())) {
    ...
    /**
     * @notice Function to recover BNB
     *
     * Requirements:
     *
     * - the caller must have the `DEFAULT_ADMIN_ROLE`.
    ...
    function recoverBNB() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(payable(_msgSender()).send(bnbBalance), "!recoverBNB");
    }
}
