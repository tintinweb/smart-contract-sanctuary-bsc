...
contract AuctionNFT is Accept {
    ...
    function bid(
        ...
        if (keccak256(bytes(tokenOf[order.tokenDet[orderNumber].tokenAddress])) == keccak256(bytes("bnb"))) 
       ...
       if(order.tokenDet[orderNumber].bidder[msg.sender])
        ...
        order.bids[order.bidCount].user = msg.sender;
          if (keccak256(bytes(tokenOf[order.tokenDet[orderNumber].tokenAddress])) == keccak256(bytes("bnb"))){
        if (order.highestBid < msg.value) order.highestBid = msg.value;
        order.bids[order.bidCount].amount = msg.value;

          }
          else{
               if (order.highestBid < _amount) order.highestBid = _amount;
    ...
    function finalizeAuction(
        ...
        bool isConfirm
       ) public isSellerOrAdmin(orderNumber) {
        ...
        if (isConfirm) {
            ...
            for(uint256 i = 0; i< order.tokenDet[orderNumber].creators.length ; i++){
               ...
               payable(creatorAdd).transfer(calAmount);
                ...
                IBEP20(order.tokenDet[orderNumber].tokenAddress).transfer(order.seller, amount - (_fee + _royality));
                // console.log("Amount Transfer",order.seller, amount - (_fee + _royality));
        ...
        emit FinalizeAuction(
            ...
            isConfirm,
            msg.sender,
            order.tokenDet[orderNumber].nftAddress
        );
    }
    ...
    function addNftBundle(
        ...
        orderList.push(orderNumber);
          for(uint256 i =0; i<  pendingOrders[orderNumber].bundleIds.length; i++){
                if(pendingOrders[orderNumber].tokenDet[orderNumber].sameCreator[IERC721(nftAddress).creator(pendingOrders[orderNumber].bundleIds[i])]>0){
                ...
                pendingOrders[orderNumber].tokenDet[orderNumber].sameCreator[IERC721(nftAddress).creator(pendingOrders[orderNumber].bundleIds[i])] = 1;
                pendingOrders[orderNumber].tokenDet[orderNumber].creators.push(payable(IERC721(nftAddress).creator(pendingOrders[orderNumber].bundleIds[i])));
                }
            }
    ...
    pendOrd.price = pendOrd.price - (pendOrd.price * _percentage)/10000;
