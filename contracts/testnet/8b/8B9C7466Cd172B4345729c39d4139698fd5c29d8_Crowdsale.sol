...
contract Crowdsale is Ownable, ReentrancyGuard {
  ...
  event CommitmentAdded(address indexed _user, uint256 _amount, uint256 overCommitAmount, uint256 actualCommitAmount);
  ...
  function getInstantTokenAmount(address _user) public view returns (uint256) {
    require(COMMITMENTS[_user] > 0, "NO_COMMITMENTS");
    uint256 tokenAmount = (COMMITMENTS[_user] * (10**uint256(PRECISION))) / AUCTION_TOKEN_PRICE;
    return tokenAmount * 7 / 10; // instant claim 70%
  }
   ...
   */
  function getCommitLimit() public view returns (uint256) {
    return (auctionData.totalOfferingTokens * AUCTION_TOKEN_PRICE) / (10**uint256(PRECISION));
  }
  ...
  function _addCommitment(address _user, uint256 _amount, bytes calldata _signature)
    ...
    require(auctionData.totalCommitments < getCommitLimit(), "EXCEED_COMMIT_LIMIT");

    /// @dev Calculate over commit amount
    uint256 overCommitAmount = 0;
    if((auctionData.totalCommitments + _amount) > getCommitLimit()) {
      overCommitAmount = (auctionData.totalCommitments + _amount) - getCommitLimit();
    }

    /// @dev Calculate actual commit amount
    uint256 actualCommitAmount = _amount - overCommitAmount;
    ...
    emit CommitmentAdded(_user, _amount, overCommitAmount, actualCommitAmount);
  ...
  function _calcExpectedReturn(uint256 _userCommitment, uint256 _deposit)
    ...
    require((_userCommitment + _deposit) <= getCommitLimit(), "EXCEED_COMMIT_LIMIT");
