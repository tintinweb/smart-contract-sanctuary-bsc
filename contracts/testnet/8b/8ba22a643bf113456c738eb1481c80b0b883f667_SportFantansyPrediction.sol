...
abstract contract Updatable is Context{
    ...
    function updater() public view virtual returns (address) {
        return _updater;
    }

    function _checkUpdater() internal view virtual {
        require(updater() == _msgSender(), "Updatable: caller is not the updater");
    }

    function transferUpdater(address newUpdater) public virtual onlyUpdater {
        require(newUpdater != address(0), "Updatable: new updater is the zero address");
        _transferUpdater(newUpdater);
    }

    function _transferUpdater(address newUpdater) internal virtual {
        address oldOUpdater = _updater;
        _updater = newUpdater;
        emit UpdaterTransferred(oldOUpdater, newUpdater);
    }
...
contract SportFantansyPrediction is Ownable,Updatable{
    ...
    address public rewardPoolAddress = 0x68Acb7D164C1F0a49da6D6e80e3b279581Eddf0d;
    ...
    function claimReward(uint256 _matchId) public{
        ...
        uint256 rewardValue = totalReward.div(totalSupply.div(totalHold)+1) + totalReward.div(totalSupply.mod(totalHold)+1) ;
    ...
    function updateResult(uint256 _matchId,uint256 _stResult,uint256 _ndResult)public onlyUpdater{
        require(_matchId >= 0 && _matchId < matchCount, "not found matchId");
