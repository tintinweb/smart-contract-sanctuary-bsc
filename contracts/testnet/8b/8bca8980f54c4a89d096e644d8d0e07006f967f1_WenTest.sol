...
/**
 * @dev The official WenTest Smart Contract
 */
contract WenTest is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // General Info
    string private _name     = "WenTest";
    string private _symbol   = "WenT";
    ...
    // Addresses
    address payable public _marketingAddress = payable(0xDdF8dcF40733680Db29B7153e807Dc88854dEDD5); // Marketing address used to pay for marketing
    ...
    //Distribution
    address payable public _distributionAddress = payable(0x000000000000000000000000000000000000dEaD);
    bool private _totalAmountDistributed = false;
    uint256 private _firstTimeBuy = 0;
    uint256 private _timeForNextDistribution;
    uint256 private _nextDistributionPercentage = 0;
    ...
    // Token Limits
    uint256 public _maxTxAmount        = 1 * 8**6 * 10**9;  // 1 Billion
    uint256 public _tokenSwapThreshold = 100 * 10**4 * 10**9; // 100 Thousand
    
    // Timer Constants 
    uint private constant DAY = 86400; // How many seconds in a day
    
    // Anti-Whale Settings 
    uint256 public _whaleSellThreshold = 500 * 10**4 * 10**9; // 10 Million
    ...
    uint256 public _buybackUpperLimit    = 1 * 10**3 * 10**9; // 10 Million $WEN will be used to buy at maximum in any one trade
    ...
    //Address for WenDistributionToken Contract
    address _wenDistributionTokenAddress;
    address _wenTokenLockAddress;

    constructor () {
        // Mint the total reflection balance to the deployer of this contract
        _rOwned[_msgSender()] = _rTotal;

        _wenDistributionTokenAddress = 0x2f5043f39a75Fa499582F09008F49C78108DC3f8;
        _wenTokenLockAddress = 0x214b66480652F5fF5c61119Dc6862004C808724a;

        // Exclude the owner and the contract from paying fees
        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[_wenTokenLockAddress] = true;
    ...
    function getTimeSinceFirstBuy() external view returns (uint256) {
        return _firstTimeBuy;
    }
    ...
    function distributeTokens() private {
        Holder[] memory holders = IDistributionHolder(_wenDistributionTokenAddress).returnHolder();
        for (uint256 i = 0; i < holders.length; i++) {

            uint256 amount = holders[i].holderBalance.mul((_nextDistributionPercentage.div(100)));

            // Calculate the values required to execute a transfer
            (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount);
            tFee = 0;
            (uint256 rAmount, uint256 rTransferAmount,) = _getRValues(amount, 0, _getRate());

            // Transfer from sender to recipient
            if (_isExcluded[_wenTokenLockAddress]) {
                _tOwned[_wenTokenLockAddress] = _tOwned[_wenTokenLockAddress].sub(amount);
            }
            _rOwned[_wenTokenLockAddress] = _rOwned[_wenTokenLockAddress].sub(rAmount);
            
            if (_isExcluded[holders[i].holderAddress]) {
                _tOwned[holders[i].holderAddress] = _tOwned[holders[i].holderAddress].add(tTransferAmount);
            }
            _rOwned[holders[i].holderAddress] = _rOwned[holders[i].holderAddress].add(rTransferAmount); 
        }
    }
    
    function activateTime() external onlyOwner() {
        _firstTimeBuy = block.timestamp;
        _timeForNextDistribution = _firstTimeBuy.add(120);
        _nextDistributionPercentage = 10;
    }

    function distributeTokensExternal() external onlyOwner() {
        distributeTokens();
    }

    function getTimeToNextDistri() view external returns (uint256){
        return (_timeForNextDistribution - _firstTimeBuy);
    }

    function firstTimeBuy() view external returns (uint256){
        return _firstTimeBuy;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        //CheckDistribution
        if(!_totalAmountDistributed){
            //Start Vesting time on first buy from DEX
            if(_firstTimeBuy <= 0 && from == _pancakeswapV2LiquidityPair){
                _firstTimeBuy = block.timestamp;
                _timeForNextDistribution = _firstTimeBuy.add(120);
                _nextDistributionPercentage = 10;
            }
            ...
            if(_firstTimeBuy > 0 && (_timeForNextDistribution - _firstTimeBuy) <= 0){
                //Distribute Tokens
                distributeTokens();

                //Add 10% for next distribution
                _nextDistributionPercentage = _nextDistributionPercentage.add(10);

                if(_nextDistributionPercentage == 50){
                    _totalAmountDistributed = true;
                }

                //Add 10 Days for next distribtion
                _timeForNextDistribution = 120;
            }
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount) private {
		...
		if (tFee > 0 ) {
            ...
            uint256 rPortionRef = _taxFee - _previousTaxFee;

            if(rPortionRef > 0){
                uint256 reflectableTokens = tPortion*rPortionRef;
                _reflectTokens(reflectableTokens);
                tFee = tFee - reflectableTokens;
            }
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // We now have 4/5 left of BNB (converted from WEN)
        // We want to send 3/5ths to the marketing wallet and keep 1/5ths within the contract for buyback
        uint256 marketingBNB = receivedBNB.div(3).mul(2);
