...
contract TeazeLotto is Ownable, Authorized, ReentrancyGuard {
    ...
    struct globalLotteryData {
        uint32 globalBNBJackpots;
        uint32 globalSpins;
        uint32 globalWins;
        uint32 globalJackpots;
        uint32 globalSBX;
        uint128 globalTeaze;
    ...
    struct userLotteryData {
        ...
        uint32 totalJackpots; //Mapping for total user jackpots on the SimpWheel of Fortune
        uint32 totalJackpotsBNB; //Mapping for total user jackpot BNB on the SimpWheel of Fortune
    }
    ...
    uint16 public spinFrequencyReduction = 14400;
    uint24 marketBuyGas = 200000;  
    uint24 public spinFrequency = 86400; 
    uint32 public lastPriceCheck;
    uint32 public priceCheckInterval = 60;   
    ...
    function SpinSimpWheel(bool _override) external payable nonReentrant {
       ...
       // require(staked, "User must be staked to spin the SimpWheel of Fortune");
        ...
        if(roll != winningNumber && !adminWinner) {
            ...
            User.totalJackpotsBNB += uint32(netamount);

            Lotto.globalBNBJackpots += uint32(netamount);
        ...
        if (block.timestamp > uint(lastPriceCheck).add(priceCheckInterval)) {
            saveLatestPrice();
        }
    ...
    function saveLatestPrice() internal {
        ...
        prices.lastPrice = uint128(lastPrice);
        prices.priceTime = uint128(block.timestamp);

        lastPriceCheck = uint32(block.timestamp);
    }
