...
contract Staking is Ownable, ReentrancyGuard {
    ...
    struct Stake{
        ...
        uint stake_end_time;        // Time of end stake
    ...
    *   @dev Emitted when a referral reward is sent
    ...
    mapping (address => bool) private isActive; // checks if the current address already have a stake.
    address[] private activeAccounts;   //Store all active accounts
    ...
    uint256 private _INTEREST_PERIOD = 1 days;    //One day 86400 Seconds
    ...
    uint256 private _MIN_STAKE_TIME = 2 weeks;     // 2 Weeks 1209600 Seconds
    ...
    // Set intrest Period Default 1 DAY 86400 seconds
    ...
    function withdraw() external onlyOwner nonReentrant{
        ...
        if(!busdContract.transfer(owner(), amount)){
    ...
    *   @dev Check if the pot has enough balance to satisfy the potential withdraw
    ...
    function getAllActiveAccounts() external onlyOwner view returns (address[] memory){
    ...
    *   @param _amount Amoun to stake
     */
    function stakeToken(uint _amount, uint256 _stake_end_time ) external nonReentrant {
        require(_amount <= busdContract.balanceOf(msg.sender), "Not enough tokens in your wallet, please try lesser amount");
        require(_stake_end_time >= block.timestamp + _MIN_STAKE_TIME , "Minimum stake time must be 2 weeks");
        ...
        newStake.stake_end_time = _stake_end_time;
    ...
    *   @param _stakeID The ID of the stake to be returned
    ...
    function unStakeToken(uint _stakeID) external nonReentrant returns (bool){
        ...
        uint end_time = selectedStake.stake_end_time;
    ...
    * @return Amount of stake
    ...
    function getStakeInfo(uint _stakeID) external view returns(
        uint stakedAmount,
        bool returned, 
        uint startTime,
        uint endTime,
        uint totalApr,
        uint penaltyonWithdraw){

        Stake memory selectedStake = stake[msg.sender][_stakeID];

        uint earnedIntrest = calculateInterest(_stakeID);
    ...
    * @return Number of active stake
    ...
    // Calulates the periods and returns the number of days staker stake the amount
    function calculatePeriods(uint _stakeID) public view returns (uint){
        ...
        uint periods = total_period.div(_INTEREST_PERIOD); //Default 1 Day 86400 Seconds
