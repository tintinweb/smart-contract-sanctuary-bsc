...
contract HoldersPartition is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function process() public onlyRematicFinanceAdmin {
        ...
        if(block.timestamp > lastClaimTime + claimWait){
            ...
            while(gasUsed < gasForProcessing && _lastProcessedIndex < sortedAddressArr.length) {
                ...
                if(processAccount(payable(account), _lastProcessedIndex)) {
                    claims++;
                }
                _lastProcessedIndex ++;
    ...
    function processAccount(address payable account, uint256 index) public onlyRematicFinanceAdmin returns (bool) {
    ...
    function withdrawableDividendOf(address account) public view returns (uint256){
        ...
        if(tierIndex > 0){
            ...
            if(sortedAddressArr.length > tierIndex * 10){
    ...
    function getTierIndexOf(address account) public view returns (uint256) {
        ...
        while(index < sortedAddressArr.length && !isDeterminted) {
            if(sortedAddressArr[index] == account){
                isDeterminted = true;
            }
            index ++;
        }

        expectedTierIndex = index / 10 + 1;
    ...
    function setDistributtionOn(bool flag) public onlyOwner() {
        require(distributtionOn != flag, "Range Index should be greater than 0");
        distributtionOn = flag;
    }

    function setSortedAddressArr(address[] memory addressArr, uint256[] memory amountArr) public onlyOwner() {
