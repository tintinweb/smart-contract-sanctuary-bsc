...
interface IKazamaPool {
    ...
    function calculateTotalPendingKazamaRewards() external view returns (uint256);
...
interface IKazamaSwapLotteryPoolVault is IERC4626 {
    function lockKazama() external;

    function unlockKazama() external;
...
library LotteryPool {
    ...
    struct Draw {
        uint256 requestId;
        IKazamaSwapLotteryPoolVault vault;
...
interface IKazamaSwapLotteryPoolDraw {
    function generateLotteryPool(
        uint256 _totalPrize,
        uint256 _lockTime,
        uint256 _drawTime,
    ...
    function getDraw(uint256 _drawId) external view returns (LotteryPool.Draw memory);

    function getPot(address _vault) external view returns (LotteryPool.Pot memory);

    function getNumOfDraw() external view returns (uint8);

    function getNumOfWinner() external view returns (uint8);

    function getLotteryPoolPeriod() external view returns (uint256);
...
interface ILotteryPoolVaultFactory {
    function generateVault(
        IERC20 _kazama,
        IKazamaPool _kazamaPool,
        IKazamaSwapLotteryPoolDraw _lotterypoolDraw,
        address _admin,
        address _keeper,
        uint256 _lockTime,
        uint256 _maxTotalDeposit
...
contract KazamaSwapLotteryPoolVault is Share, IKazamaSwapLotteryPoolVault, Ownable {
    using SafeERC20 for IERC20;
    using SortitionSumTreeFactory for SortitionSumTreeFactory.SortitionSumTrees;

    IERC20 immutable kazama;
    IKazamaPool immutable kazamaPool;
    IKazamaSwapLotteryPoolDraw immutable lotterypoolDraw;
    address keeper;
    ...
    uint256 public totalLockKazama;
    uint256 immutable maxTotalDeposit;

    uint16 public burnPercentage = 7000; //70%

    SortitionSumTreeFactory.SortitionSumTrees internal sortitionSumTrees;
    bytes32 private constant TREE_KEY = keccak256("KazamaLotteryPool/Shares");
    ...
    constructor(
        ...
        require(
            address(_kazama) != address(0) &&
                address(_kazamaPool) != address(0) &&
                address(_lotterypoolDraw) != address(0) &&
                _admin != address(0) &&
                _keeper != address(0),
            "zero address"
        );
        ...
        kazamaPool = _kazamaPool;
        kazama = _kazama;
        lotterypoolDraw = _lotterypoolDraw;
        keeper = _keeper;
        lockTime = _lockTime;
    ...
    function deposit(uint256 _assets, address _receiver) external override returns (uint256 shares) {
        ...
        kazama.safeTransferFrom(msg.sender, address(this), _assets);
    ...
    function mint(uint256 _shares, address _receiver) external override returns (uint256 assets) {
        ...
        // Need to transfer before minting or ERC777s could reenter.
        kazama.safeTransferFrom(msg.sender, address(this), assets);
    ...
    function withdraw(
        ...
        _burn(_owner, shares);
        kazama.safeTransfer(_receiver, _assets);
    ...
    function redeem(
        ...
        kazama.safeTransfer(_receiver, assets);

        emit Withdraw(msg.sender, _receiver, _owner, assets, _shares);
    }

    function asset() public view override returns (address) {
        return address(kazama);
    }

    function totalAssets() public view override returns (uint256) {
        return kazama.balanceOf(address(this));
    }
    ...
    function lockKazama() external override onlyKeeperOrOwner {
        require(status == Vault.Status.BEFORE_LOCK, "status not correct");
        LotteryPool.Pot memory pot = lotterypoolDraw.getPot(address(this));
        require(pot.totalPrize > 0, "lotterypool not exist");
        require(pot.numOfDraw == 0, "lotterypool cancelled");
        ...
        if (balance < maxTotalDeposit) lotterypoolDraw.redeemPrizeByRatio();
        kazama.safeIncreaseAllowance(address(kazamaPool), balance);
        kazamaPool.deposit(balance, LOCK_PERIOD);
        totalLockKazama = balance;
        lockStartTime = block.timestamp;
        status = Vault.Status.LOCK;

        emit Lock(msg.sender, lockStartTime, totalLockKazama);
    }

    function unlockKazama() external override onlyKeeperOrOwner {
        require(status == Vault.Status.LOCK, "status not correct");
        require(passLockTime(), "lock not over");
        kazamaPool.withdrawAll();
        uint256 balance = kazama.balanceOf(address(this));
        uint256 totalEarnKazama = balance > totalLockKazama ? balance - totalLockKazama : 0;
        uint256 burnAmount = (totalEarnKazama * burnPercentage) / 10000;
        address treasury = lotterypoolDraw.getTreasury();
        if (burnAmount > 0) kazama.safeTransfer(treasury, burnAmount);
        uint256 apyEarnKazama = totalEarnKazama - burnAmount;
        lockEndTime = block.timestamp;
        status = Vault.Status.UNLOCK;

        emit Unlock(msg.sender, lockEndTime, totalEarnKazama, burnAmount, apyEarnKazama);
    }
...
contract LotteryPoolVaultFactory is ILotteryPoolVaultFactory {
    function generateVault(
        IERC20 _kazama,
        IKazamaPool _kazamaPool,
        IKazamaSwapLotteryPoolDraw _lotterypoolDraw,
        ...
        KazamaSwapLotteryPoolVault vault = new KazamaSwapLotteryPoolVault(
            _kazama,
            _kazamaPool,
            _lotterypoolDraw,
            _admin,
