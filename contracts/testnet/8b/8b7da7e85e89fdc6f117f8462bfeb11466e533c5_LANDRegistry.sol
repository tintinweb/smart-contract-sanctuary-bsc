...
contract Storage is OwnableStorage, AssetRegistryStorage, LANDStorage {
}
...
contract LANDRegistry is Storage, Ownable, FullAssetRegistry, ILANDRegistry {
  ...
  function initialize(bytes) external {
    ...
    authorizedDeploy[msg.sender] = true;
  }
  ...
  function authorizeDeploy(address beneficiary) external onlyDeployer {
  ...
  function forbidDeploy(address beneficiary) external onlyDeployer {
    require(beneficiary != address(0), "invalid address");
  ...
  function setLatestToNow(address user) external {
    require( _isApprovedForAll(user, msg.sender), "Unauthorized user");
    // solium-disable-next-line security/no-block-members
  ...
  */
  ...
  function setEstateRegistry(address registry) external onlyDeployer {
   ...
   */
  function setLandBalanceToken(address _newLandBalance) onlyDeployer external {
