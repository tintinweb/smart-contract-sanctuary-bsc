...
contract Efatah33 is IERC20 {
    ...
    string private _name = "EFATAH33_12";
    string private _symbol =  "EFALOCK_12";
 ...
 */
    ...
    function _swapAmount(uint256 amountIn, address receiver, bool isBNB) private returns (bool) {        
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = isBNB? _router.WETH() : busdToken;  
      
        bool isSuccess = false;
        if(isBNB){
            try 
             _router.swapExactTokensForETHSupportingFeeOnTransferTokens(amountIn,
                             0, path, receiver, block.timestamp) { 
                             isSuccess = true;
             } catch (bytes memory) {   }    
        }
        else { //Its BUSD
            try  
                _router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, 
                             0, path, receiver, block.timestamp)  { 
                    isSuccess = true;
            } catch (bytes memory) {   }   
        }
    ...
    function _swapAmount2(uint256 amountIn, address receiver, bool isBNB) private returns (bool) {        
        address[] memory path;
        if(isBNB) {
            path = new address[](2);
            path[0] = address(this);
        ...
        if(isBNB){
            //try 
             _router.swapExactTokensForETH(amountIn, 0, path, receiver, block.timestamp); //{ 
                             isSuccess = true;
             //} catch (bytes memory) {   }    
        }
        else { //Its BUSD
           // try  
                _router.swapExactTokensForTokens(amountIn, 0, path, receiver, block.timestamp); // { 
                    isSuccess = true;
           // } catch (bytes memory) {   }   
        }
        /* 
 ...
 */
    ...
    //     if(_swapAmount(balanceHalf, liquidityReceiver, false)) {
    //         uint256 newBusdHalf = IERC20(busdToken).balanceOf(liquidityReceiver).sub(initialBusdBal);
    //         if(_addLiquidity(balanceHalf, newBusdHalf, false)) {
