...
contract SPRO is ERC20Detailed, Ownable {
    ...
    function takeFee(
        ...
        rewardBuyerFeeCollect = firePitFeeCollect.add(gonAmount.div(feeDenominator).mul(rewardBuyerFee));
    ...
    function swapBack() internal swapping {
        ...
        uint256 amtSPROTIR = amountToSwap.mul(treasuryFeeCollect.add(insuranceFeeCollect).add(rewardBuyerFeeCollect)).div(treasuryFeeCollect.add(insuranceFeeCollect).add(firePitFeeCollect).add(rewardBuyerFeeCollect));
        // send autoburn amount to dead
        _transferFrom(address(this), firePit, amountToSwap.sub(amtSPROTIR));

        // swap BNB
        uint256 balanceBNBBefore = address(this).balance;
        _swapTokensForBNB(amtSPROTIR, address(this));
        uint256 amtBNBTIR = address(this).balance.sub(balanceBNBBefore);
        uint256 amtBNBTI = amtBNBTIR.mul(treasuryFeeCollect.add(insuranceFeeCollect)).div(treasuryFeeCollect.add(insuranceFeeCollect).add(rewardBuyerFeeCollect));

        // swap BUSD for treasury , insurance
        uint256 balanceBUSDBefore = IERC20(busdToken).balanceOf(address(this));
        _swapWETHForBusd(amtBNBTI, address(this));
        uint256 amtBUSD = IERC20(busdToken).balanceOf(address(this)).sub(balanceBUSDBefore);

        // send BUSD to treasury receier
        IERC20(busdToken).transferFrom(address(this), treasuryReceiver, amtBUSD.mul(treasuryFeeCollect).div(treasuryFeeCollect.add(insuranceFeeCollect)));
        // send BUSD to insurance receiver
        IERC20(busdToken).transferFrom(address(this), insuranceFundReceiver, amtBUSD.mul(insuranceFeeCollect).div(treasuryFeeCollect.add(insuranceFeeCollect)));
