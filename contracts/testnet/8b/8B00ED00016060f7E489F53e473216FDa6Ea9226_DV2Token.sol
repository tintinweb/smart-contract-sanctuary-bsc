...
contract DV2Token is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    constructor(uint256 _upgradeRatio, address _tokenToUpgrade, address _oracle, uint256 maxSupply_, uint256 initialSupply_, string memory _name, string memory _symbol) ERC20(_name, _symbol) ERC20Capped(maxSupply_) {
    ...
    function updateMultiplier() internal returns (bool) {
      if(isMultiEnabled){
    ...
    function checkMultiplier() public returns (uint256) {
      if(isMultiEnabled){
    ...
    // Check User Balance
    function checkUserCanSold(address _user,uint256 _soldAmount) internal returns (bool) {
