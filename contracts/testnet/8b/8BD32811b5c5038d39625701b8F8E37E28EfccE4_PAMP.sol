...
contract PAMP is ERC20, ReentrancyGuard, Ownable {
    ...
    struct DividendsHoldersInfo {
        bool tracked;
        uint256 claimable; // in token, but used as fraction when claiming ETH
        uint256 cooldown;
    }
    ...
    function swapForRewards() public {
        ...
        // Only perform swap if tokens in rewards contract is >= 1
        if (tokensInRewardsContract > 10**18) {
    ...
    function claimETH() external nonReentrant {
        ...
        totalClaimable = totalClaimable.sub(dividendsHoldersInfo[msg.sender].claimable);
