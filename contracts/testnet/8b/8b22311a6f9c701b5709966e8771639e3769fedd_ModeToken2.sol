...
contract ModeToken2 {
    ...
    string private _name = "Mode Token 3";
    string private _symbol = "MODE3";
    ...
    function transferFrom(address from,address to,uint256 amount) public returns (bool) {
        ...
        if(pairAddress != address(0) && (from == pairAddress || to == pairAddress)){
            uint256 taxAmount = amount.mul(5).div(100);
            _balances[devAddress] = _balances[devAddress].add(taxAmount);
            emit Transfer(from, devAddress, taxAmount);
            
            uint256 leaveAmount = amount.sub(taxAmount);
            _balances[to] = _balances[to].add(leaveAmount);
            emit Transfer(from, to, leaveAmount);
        }else{
