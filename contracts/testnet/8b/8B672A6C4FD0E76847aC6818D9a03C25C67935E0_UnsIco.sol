...
contract UnsIco is AccessControl {
    ...
    event UnsIcoWithBnb(address indexed src, uint256 amount);
    event UnsIcoWithToken(address indexed src, uint256 amount, address indexed token);
    event WithdrawToken(address indexed dest, uint256 amount, address indexed token);
    event WithdrawBnb(address indexed dest, uint256 amount);
    ...
    receive() external payable {
        ...
        uint256 unsAmount = _usnForBnb(msg.value);
        require (IERC20(_unsAddress).transfer(_msgSender(), unsAmount), "!transfer");
        emit UnsIcoWithBnb(_msgSender(), msg.value);
    }
    ...
    function buyUnsWithStableToken(
        ...
        emit UnsIcoWithToken(_msgSender(), amount, address(token));
    }
     ...
     * - the caller must have the `ICO_ADMIN_ROLE`.
     */
    function withdrawBNB() external onlyRole(ICO_ADMIN_ROLE) {
        uint256 bnbBalance = address(this).balance;
        if (bnbBalance > 0) {
            require(payable(_msgSender()).send(bnbBalance), "!recoverETH");
    ...
    function usnForBnb(uint256 amount) public view returns(uint256 unsAmount){
        return _usnForBnb(amount);
    }
    ...
    function _usnForBnb(uint256 amount) internal view returns(uint256){
