...
// License-Identifier: MIT
pragma solidity ^0.8.17;
contract InvestorsDAO is Ownable {
    struct Vesting {
        uint256 amount;
        uint256 startDate;
        uint256 duration;
    ...
    function getProgress(uint256 _startDate, uint256 _duration)
        public
        view
        returns (uint256)
    {
        ...
        uint256 progress = (dateDiff * 100) / _duration;
        return progress;
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < investors[_investor].vestings.length; i++) {
            ...
            if (progress > 0) {
                vestingAmount +=
                    (investors[_investor].vestings[i].amount * progress) /
                    100 -
                    investors[_investor].vestings[i].amountWithdrawn;
            }
        }
    ...
    function setTreasury(address[] memory _treasuries) public onlyOwner {
        treasuries = _treasuries;
    }
}