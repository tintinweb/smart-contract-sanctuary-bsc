...
contract GboBUSD {
	...
	function getUserReferrerDividends(address userAddress) internal returns (uint256) {
		User storage user = users[userAddress];
		uint256 refferAmount = 0;
		for (uint256 i = 0; i < user.users.length; i++) {
			User storage user1 =user.users[i];
			uint256 userId = user1.id;
			bool parentReffererFlag1 = user1.parentReffererFlag1;
			bool parentReffererFlag2 = user1.parentReffererFlag2;
			bool parentReffererFlag3 = user1.parentReffererFlag3;

			uint256 pay1=(userId*4)+1;
			if(n>=pay1 && !parentReffererFlag1){
                refferAmount+=12e18;
				user1.parentReffererFlag1 = true;
			}

			uint256 pay2=(userId*16)+1;
			if(n>=pay2 && !parentReffererFlag2){
                refferAmount+=24e18;
				user1.parentReffererFlag2 = true;
			}

			uint256 pay3=(userId*64)+1;
			if(n>=pay3 && !parentReffererFlag3){
                refferAmount+=48e18;
				user1.parentReffererFlag3 = true;
			}
