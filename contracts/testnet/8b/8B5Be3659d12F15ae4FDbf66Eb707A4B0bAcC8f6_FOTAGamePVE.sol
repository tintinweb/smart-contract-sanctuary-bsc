...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  event MissionAdded(uint id, uint reward, uint32 experience, address owner, uint timestamp);
  event MissionUpdated(uint id, uint reward, uint32 experience, address owner, uint timestamp);
  event EnergyConsumptionUpdated(uint energyConsumption);
  event MaxRewardUpdated(uint amount, uint timestamp);
  ...
  function finishGame(uint _mission, uint16[] calldata _heroIds, bytes memory _signature) external {
    _validateFinishGame(_mission, _heroIds, _signature);
    ...
    if (firstConsumeInDay || energy >= energyConsumption) {
      _giveReward(_mission, firstConsumeInDay);
  ...
  function finishGameByPassSignature(uint _mission, uint16[] calldata _heroIds) external {
    _validateFinishGameBypassSignature(_mission, _heroIds);
    uint energy = energyManager.getUserCurrentEnergy(msg.sender);
    bool firstConsumeInDay = energyManager.updateEnergy(msg.sender, energyConsumption);
    if (firstConsumeInDay) {
      todayFreeMissionFinished[msg.sender] = 0;
...
//      _giveReward(_mission, firstConsumeInDay);
  ...
  function haveTodayFreeMission(address _user) public view returns (bool) {
    uint todayFreeMission = pvp.getUserTodayLose(_user) * extraMissionWhenRunOutOfEnergyRatio;
  ...
  function _giveReward(uint _mission, bool _firstConsumeInDay) private {
    uint reward = _calculateReward(_mission, _firstConsumeInDay);
    if (missions[_mission].owner != address(0)) {
      uint shared = reward / 100;
      reward -= shared;
      rewardManager.addLandShared(missions[_mission].owner, shared);
    }
    rewardManager.addReward(1, msg.sender, reward);
  }

  function _calculateReward(uint _mission, bool _firstConsumeInDay) private returns (uint) {
    uint reward = missions[_mission].reward;
  ...
  function _experienceUp(uint _mission, uint16[] calldata _heroIds) private {
    for(uint i = 0; i < _heroIds.length; i++) {
      heroNft.experienceUp(_heroIds[i], missions[_mission].experience);
    }
  }

  function _validateFinishGame(uint _mission, uint16[] calldata _heroIds, bytes memory _signature) private {
    require(_heroIds.length >= 3 && _heroIds.length <= 5, "HeroIds is invalid");
    require(_mission <= totalMissions, "Mission invalid");
  ...
  function _validateFinishGameBypassSignature(uint _mission, uint16[] calldata _heroIds) private view {
    require(_heroIds.length >= 3 && _heroIds.length <= 5, "HeroIds is invalid");
    require(_mission <= totalMissions, "Mission invalid");
  }

  function _buildDigest(uint _mission, uint16[] calldata _heroIds) private view returns (bytes32) {
    if (_heroIds.length == 3) {
      return _hashTypedDataV4(keccak256(abi.encode(
        keccak256("FinishGamePVE(address user,uint256 nonce,uint256 mission,uint16 hero1,uint16 hero2,uint16 hero3)"),
        msg.sender,
        nonces[msg.sender],
        _mission,
        _heroIds[0],
      ...
      return _hashTypedDataV4(keccak256(abi.encode(
        keccak256("FinishGamePVE(address user,uint256 nonce,uint256 mission,uint16 hero1,uint16 hero2,uint16 hero3,uint16 hero4)"),
        msg.sender,
        nonces[msg.sender],
        _mission,
        _heroIds[0],
      ...
      return _hashTypedDataV4(keccak256(abi.encode(
        keccak256("FinishGamePVE(address user,uint256 nonce,uint256 mission,uint16 hero1,uint16 hero2,uint16 hero3,uint16 hero4,uint16 hero5)"),
        msg.sender,
        nonces[msg.sender],
        _mission,
        _heroIds[0],
  ...
  function _initData() private {
    ...
    rewardManager = IRewardManager(address(0)); // TODO
    for(uint i = 1; i <= totalMissions; i++) {
      missions[i] = Mission(i, 1e18, uint32(i), address(0));
    }
  }

  // ADMIN FUNCTIONS

  function addMission(uint _reward, address _owner, uint32 _experience) external onlyMainAdmin {
    totalMissions++;
    missions[totalMissions] = Mission(totalMissions, _reward, _experience, _owner);
    emit MissionAdded(totalMissions, _reward, _experience, _owner, block.timestamp);
  }

  function updateMission(uint _id, uint _reward, uint32 _experience, address _owner) external onlyMainAdmin {
    missions[_id].reward = _reward;
    missions[_id].experience = _experience;
    missions[_id].owner = _owner;
    emit MissionUpdated(_id, _reward, _experience, _owner, block.timestamp);
  }
...
interface IRewardManager {
  function addReward(uint8 _source, address _user, uint _amount) external;
  function addLandShared(address _landLord, uint _amount) external;
}
