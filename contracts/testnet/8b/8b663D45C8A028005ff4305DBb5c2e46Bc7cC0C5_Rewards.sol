...
contract Rewards is Ownable {
    ...
    function addToWhitelist(address payable _address, uint256 _amount)
        external
        onlyOwner
    {
        amountClaim[_address] = _amount;
        whilelist[_address] = true;
    }

    function _removeFromWhitelist(address _address) internal onlyOwner {
        delete amountClaim[_address];
    ...
    function claimNativeToken() external {
        require(
            amountClaim[msg.sender] > 0 && whilelist[msg.sender],
            "Your address is not in the whitelist."
        );
        _transferNativeToken(msg.sender, amountClaim[msg.sender]);
