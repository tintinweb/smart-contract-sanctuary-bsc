...
contract DiamondHands {
    ...
    function deposit(
        ...
        orderBook[msg.sender][_inputToken][block.number] = Order(_uniswapRouter, _amount, _outputToken, _minOutputAmount);
        emit OrderCreated(_uniswapRouter, msg.sender, _inputToken, _amount, _outputToken, _minOutputAmount);
    }
    ...
    function withdraw(
        ...
        require(isWithdrawable(msg.sender, _token, _blockNumber), "Your order is not withdrawable!");
    }
    ...
    function isWithdrawable(address _from, address _token, uint _blockNumber) view public returns (bool){
        Order memory order = _getOrder(_from, _token, _blockNumber);
        address[] memory paths = new address[](2);
        paths[0] = _token;
        paths[1] = order.outputToken;
        uint[] memory amounts = IUniswapV2Router01(order.uniswapRouter).getAmountsOut(order.inputAmount, paths);
