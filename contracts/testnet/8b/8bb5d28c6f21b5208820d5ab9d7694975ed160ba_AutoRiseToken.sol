...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (address _router, address busd) {
...
contract AutoRiseToken is ERC20Detailed, Ownable {
	using SafeMath for uint256;
	...
	string public _name = "AutoRise";
	string public _symbol = "AUTOR";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 925 * 10**6 * 10**DECIMALS;
	...
	uint256 public totalFee = liquidityFee.add(treasuryFee).add(safuDividendFee).add(apecInsuranceFundFee).add(firePitFee);
    ...
    uint256 private constant MAX_SUPPLY = 925 * 10**12 * 10**DECIMALS;
	...
	constructor(
        ...
        address _busd
	) ERC20Detailed("AutoRise", "AUTOR", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = _autoLpReceiver;
        ...
        // unlocked[address(pairAddress)] = true;
        // unlocked[address(treasuryReceiver)] = true;
        // unlocked[address(autoLiquidityReceiver)] = true;
        // unlocked[address(apecInsuranceFundReceiver)] = true;
        // unlocked[address(firePit)] = true;
	...
	function takeFee(
		...
		_gonBalances[address(this)] = _gonBalances[address(this)].add(gonAmount.div(feeDenominator).mul(_treasuryFee.add(safuDividendFee)));
    ...
    function toggleUnlocked(address _addr) public onlyOwner {
        unlocked[_addr] = !unlocked[_addr];
    } 
    
    function activateTrading(bool flag) external onlyOwner {
        locked = flag;
    }  
