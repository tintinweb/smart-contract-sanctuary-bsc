...
contract BakedVilleUpgradeable is OwnableUpgradeable {
    using SafeMath for uint256;

    uint256 private EGGS_TO_HATCH_1MINERS; //for final version should be seconds in a day
    uint256 private PSN;
    ...
    function _hatchEggsOnBuy(address ref) private {
        require(initialized);
    ...
    function sellEggs() public {
        require(initialized, "BakedVille is not initialized");
        ...
        if (fee > 0) {
            tokenContract.transfer(recAdd, fee);
        }
    ...
    function buyEggs(uint256 _amount, address ref) public {
        require(initialized, "BakedVille is not initialized");
        tokenContract.transferFrom(msg.sender, address(this), _amount);
        uint256 eggsBought = calculateEggBuy(
            _amount,
            SafeMath.sub(tokenContract.balanceOf(address(this)), _amount)
        );
    ...
    function seedMarket(uint256 _amount) public onlyOwner {
        require(marketEggs == 0, "Market is already seeded");
        initialized = true;
