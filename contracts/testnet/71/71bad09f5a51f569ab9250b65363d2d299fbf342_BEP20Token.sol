...
contract BEP20Token is Context, BEP20, Ownable {
  ...
  /// PLAN 
    ...
    uint256 constant private min_investment = 1;
    uint256 private final_package_amount = 0;
    uint256 constant private percent_devider = 1000;
    uint256[] private level_income 	=   [400, 150 ,100, 50, 50, 30, 30, 20, 20, 10, 10, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
    ...
    event new_register(address user);
  ...
  constructor() public {
    ...
    function invest(address referrer, uint8 package_amount) public payable {

        if(package_amount == 1) {   // if else statement
            final_package_amount = 1;
        } 
        require(final_package_amount == 0, "Invaild Package Amount!");
		require(msg.value < min_investment, "Minimum Package Amount is BUSD 1!");
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 21; i++) {
				if (upline != address(0)) {
					uint256 amount = msg.value.mul(level_income[i]).div(percent_devider);
		...
		if (user.deposits.length == 0) {
			...
			emit new_register(msg.sender);
		}

		user.deposits.push(Deposit(package_amount, msg.value, block.timestamp));

		totalInvested = totalInvested.add(msg.value);

		emit NewDeposit(msg.sender, package_amount, msg.value);

        uint256 totalAmount = users[referrer].directs;
        uint256 totalAmount2 = 1;
        user.directs = totalAmount.add(totalAmount2);
	}


    // function update_directs(address referrer) private {
