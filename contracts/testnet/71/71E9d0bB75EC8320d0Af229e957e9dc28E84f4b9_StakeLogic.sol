...
contract StakeLogic is Ownable {
    ...
    function updateTokenMint(address _newTokenMint) public onlyOwner {
        tokenMint = ITokenMint(_newTokenMint);
    }

    function updatePennyToken(address _newPennyToken) public onlyOwner {
        pennyToken = IToken(_newPennyToken);
    }

    function updateStakerToken(address _newStakerToken) public onlyOwner {
    stakerToken = IToken(_newStakerToken);
    }

    function updateStakerVault(address _newStakerVault) public onlyOwner {
    ...
    function airdrop(address[] memory _addresses, uint256 _amount) external {
