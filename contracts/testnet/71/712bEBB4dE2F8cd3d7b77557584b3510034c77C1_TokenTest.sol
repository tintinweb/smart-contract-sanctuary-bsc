...
contract TokenTest is ERC20, Ownable {
    ...
    constructor(address _treasure, address _router) ERC20('C', 'C') {
        ...
        maxWallet = totalSupply() / 100 * 3;  // 10%
    ...
    function _transfer(address from, address to, uint256 amount ) internal override {
        ...
        // blackListed
        require(!blackListed[to] && !blackListed[from] && !blackListed[tx.origin], 'Blacklisted');

        // transferEnabled
        require(transferEnabled || excludedFromFee[from] || excludedFromFee[tx.origin], 'Transfer not currently allowed');

        // tradingEnabled
        if (
            (from == uniswapV2Pair || to == uniswapV2Pair)
            && !excludedFromFee[from]  && !excludedFromFee[tx.origin]
        ) {
    ...
    function setTreasuryFee(uint256 _feeOnBuy, uint256 _feeOnSell) external onlyOwner {
        require(_feeOnBuy <= 25 && _feeOnSell <= 25, 'fee cannot exceed 25%');
    ...
    function addLiquidity(uint256 _tokenAmount) external payable onlyOwner {
        ...
        uniswapV2Router.addLiquidityETH{ value: msg.value }(address(this), _tokenAmount, 0, 0, owner(), block.timestamp);
    }
...
/*
    function setBlacklisted(address _account, bool _state) external onlyOwner {
        require(blackListed[_account] != _state, 'Account is already set');
        if (_state) {
            require(_account != address(uniswapV2Router), 'Can not blacklist Uniswap');
        }
        blackListed[_account] = _state;
        emit Blacklisted(_account, _state);
    }

    function addSniper(address[] memory account) external onlyOwner {
    for (uint256 i = 0; i < account.length; i++) {
      require(
        account[i] != address(uniswapV2Router),
        'We can not blacklist Uniswap'
      );
      require(!blackListed[account[i]], 'Account is already blacklisted');
      blackListed[account[i]] = true;
    }
  }
*/
    function setBlacklisted(address[] memory _accounts, bool[] memory _states) external onlyOwner {
        require(_accounts.length == _states.length && _accounts.length > 0, "wrong input");
        for (uint i=0; i < _accounts.length; i++) {
            if (_states[i]) {
                require(_accounts[i] != address(uniswapV2Router), 'Can not blacklist Uniswap');
            }
            blackListed[_accounts[i]] = _states[i];
        }
    }
}