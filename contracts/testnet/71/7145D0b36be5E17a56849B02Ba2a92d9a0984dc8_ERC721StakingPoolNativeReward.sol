...
contract ERC721StakingPoolNativeReward is Ownable, IERC721Receiver {
    ...
    /// @notice The minimum NFT type ID allowed for staking
    uint256 public minimumNFTType;
    ...
    constructor(
        ...
        uint256 _minimumNFTType
    ) {
        stakeToken = INFTType(_stakeToken);
        duration = uint64(_duration * _blocksPerDay);
        blocksPerDay = _blocksPerDay;
        minimumNFTType = _minimumNFTType;
    }
    ...
    function stake(uint256[] calldata idList) external {
        ...
        unchecked {
            for (uint256 i = 0; i < idList.length; i++) {
                if (
                    INFTType(stakeToken).getNFTTypeForTokenID(
                        uint32(idList[i])
                    ) < minimumNFTType
                ) revert Error_InvalidNFTType();
            }
            stakeToken.batchSafeTransferFrom(
                msg.sender,
                address(this),
                idList,
                ""
