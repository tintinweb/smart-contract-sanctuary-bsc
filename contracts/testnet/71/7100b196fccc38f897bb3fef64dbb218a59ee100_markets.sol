...
library marketdata{
    ...
    function makeOfferToken(userMakeOffersInfo storage obj,  IERC20 _token,uint256 _price,uint256 _amount,uint256 indexG) internal{
        makeOfferBase(obj,_price,_amount);
        obj.offers[obj.index].token = _token;
        obj.offers[obj.index].style = 1;
        obj.offers[obj.index].indexGlobal = indexG;
    }
   //_price：1个token币(ewei)可以兑换的 基准币数量(单位：wei)
    function makeOffer(userMakeOffersInfo storage obj,  uint256 _price,uint256 _amount,uint256 indexG) internal{
        makeOfferBase(obj,_price,_amount);
...
contract markets is Ownable {
    ...
    //报价者，IERC20,全局index,当前用户的index, 数量，计划兑换的数量，单价,
    event MakeOfferToken(address,address,uint256,uint256,uint256 ,uint256,uint256);
     //报价者，  全局index ,当前用户的index, 数量，计划兑换的数量，单价,
    event MakeOffer(address,uint256,uint256,uint256 ,uint256,uint256);
    //取消报价
   ...
   //购买:购买用户，卖出用户， 卖出用户的index
    ...
    uint256 public  indexGlobal ;
        //有效报价
    uint256 public validOfferGlobal;

    address public  mgrAddress;
    //服务费(买)
    uint8 public serviceChargeBuy = 1;
    //服务费(卖)
    uint8 public serviceChargeSell = 2;
    
    mapping(uint256=>marketIndex) public makeOffersIndex;
    mapping(address=>marketdata.userMakeOffersInfo) public makeOffers;
    
    function setMakeOffersIndex(uint256 indexG,address user,uint256 indexU) internal {
        makeOffersIndex[indexG].user = user;
        makeOffersIndex[indexG].index = indexU;
    }
    ...
    function getMakeOfferIndex(address _user) view public returns(uint256,uint256) {
     ...
     function getMakeOffer(address _user,uint256 _index) view public  returns( marketdata.makeOffersInfo memory) {
        require(makeOffers[_user].index >= _index && _index > 0,"out of bounds" );
    ...
    function makeOfferToken(IERC20 _token,uint256 _price,uint256 _amount)  public{
        ...
        require(_token.transferFrom(_msgSender(),((address)(this)),_amount),"transferFrom  fail");
        indexGlobal++;
        validOfferGlobal++;
        makeOffers[_msgSender()].makeOfferToken(_token,_price,_amount,indexGlobal);
  
        setMakeOffersIndex(indexGlobal,_msgSender(),makeOffers[_msgSender()].index);
    ...
    function makeOffer(uint256 _price,uint256 _amount) payable  public{
        ...
        indexGlobal ++;
        validOfferGlobal++;
        makeOffers[_msgSender()].makeOffer(_price, _amount,indexGlobal); 
    ...
    //购买平台币
    function buy(address _user,uint256 _index) public  {
        require(_user != _msgSender());
        require(makeOffers[_user].index >= _index,"out of bounds");
        require(makeOffers[_user].offers[_index].state == 1,"The current offer is invalid");
