...
contract Coffe_Matrix {
    ...
    uint256 public crestTokenAddressFee = 10 ether;
    uint256 public tctTokenAddressFee = 10 ether;
    ...
    uint256[7] directsRequirement = [3, 6, 9, 12, 15, 18, 21];
    ...
    function register(address ref) external isNotContract {
        ...
        require(
            token.balanceOf(msg.sender) >= joiningFee,
            "Insufficient balance"
        );
        // Transfer the joining fee from the sender to the contract
        require(
            token.transferFrom(msg.sender, address(this), joiningFee),
            "Transfer failed"
        );

        // Transfer the crestTokenAddressFee to the crestTokenAddress contract
        token.transfer(crestTokenAddress, crestTokenAddressFee);
        // Transfer the tctTokenAddressFee to the tctTokenAddress contract
        token.transfer(tctTokenAddress, tctTokenAddressFee);

        // Calculate the amount that will be distributed as rewards
        uint256 amount = joiningFee -
            (crestTokenAddressFee + tctTokenAddressFee);
        ...
        token.transfer(upline, directBonus[uint256(users[upline].rank)]);
        ...
        while (upline != address(0)) {
            ...
            if (amount >= teamBonus[uint256(users[upline].rank)]) {
                ...
                token.transfer(upline, teamBonus[uint256(users[upline].rank)]);
    ...
    function changecrestTokenAddressFee(uint256 _crestTokenAddressFee)
        external
        onlyAdmin
    {
        // Ensure that the new crestTokenAddressFee is greater than 0
        require(
            _crestTokenAddressFee > 0,
            "Crest token fee must be greater than 0"
        );
        crestTokenAddressFee = _crestTokenAddressFee;
    }

    // to change tct token fee
    function changetctTokenAddressFee(uint256 _tctTokenAddressFee)
        external
        onlyAdmin
    {
        // Ensure that the new tctTokenAddressFee is greater than 0
        require(
            _tctTokenAddressFee > 0,
            "Tct token fee must be greater than 0"
        );
        tctTokenAddressFee = _tctTokenAddressFee;
    }
    ...
    function rewardDistribution(uint256 rank, uint256 amount)
        external
        onlyAdmin
    {
