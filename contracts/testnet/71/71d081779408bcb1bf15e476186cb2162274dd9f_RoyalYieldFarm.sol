...
contract RoyalYieldFarm {
	...
	uint256 public WALLET_LIMIT = 100 ether;  
    ...
    uint public startTime = 1661300454; // Fri Aug 30 2022 21:00:00 GMT+0000 https://www.unixtimestamp.com/
	...
	constructor(address payable _dev1, address payable _dev2, address payable _dev3) {
		require(!isContract(_dev1) && !isContract(_dev2) && !isContract(_dev3));
        ...
        plans.push(Plan(20,  60,  1 * 1e17, 25 ether, 0, 0, 0, 0, true));
        plans.push(Plan(40,  50,  2 * 1e17, 30 ether, 0, 0, 0, 0, true));
        plans.push(Plan(100,  40,  3 * 1e17, 35 ether, 0, 0, 0, 0, true));
	}
	...
	function withdraw() public {
        ...
        totalAmount = totalAmount.sub(totalAmount.mul(MKT_BUYBACK_FEE).div(PERCENTS_DIVIDER)); 
    ...
    function SET_WALLET_LIMIT(uint256 value) external {
        ...
        function CHANGE_PROJECT_VALUE(uint256 value) external {
        require(msg.sender == contractOwner, "Admin use only");
        require(value <= 300);
        PROJECT_FEE = value;
    }

    function CHANGE_MKT_BUYBACK_VALUE(uint256 value) external {
        require(msg.sender == contractOwner, "Admin use only");
        require(value <= 300);
