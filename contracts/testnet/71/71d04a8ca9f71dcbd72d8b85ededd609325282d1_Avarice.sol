...
// Copyright © 2022 Avarice Token Ltd. All Rights Reserved.
...
contract Avarice is ERC20, Ownable {
    ...
    address internal constant avariceTeam_addr_1 = 0xcB3E60F5e5649534aF7077E44080057d527672A0;
    address internal constant avariceTeam_addr_2 = 0xcB3E60F5e5649534aF7077E44080057d527672A0;
    address internal constant avariceTeam_addr_3 = 0xcB3E60F5e5649534aF7077E44080057d527672A0;
    address internal marketing_addr = 0xcB3E60F5e5649534aF7077E44080057d527672A0;
    address internal buyBack_addr = 0xcB3E60F5e5649534aF7077E44080057d527672A0;

    /* Starting amount of tokens that are minted daily to be distributed between lobby participants */
    uint256 internal constant startingLobbyPool = 3 * 1e8;
    /* last amount of lobby pool which starts from startingLobbyPool */
    uint256 internal lastLobbyPool = startingLobbyPool;
    ...
    /* Min penalty for canceling stake */
    uint256 internal constant penalty_min = 2;  

    /* Max penalty for canceling stake */
    uint256 internal constant penalty_max = 50;  
    ...
    bool public stakeSelingIsPaused = false; 
    ...
    function switchStakeSellingStatus() external onlyOwner() {
        if (stakeSelingIsPaused == true) {
            stakeSelingIsPaused = false;
        } 
        else if (stakeSelingIsPaused == false) {
            stakeSelingIsPaused = true;
        }
    }
    ...
    function flushFirstDayLobbyEntry() external onlyOwner() {
        require(firstDayFlushed == false);
        firstDayFlushed = true;

        payable(avariceTeam_addr_1).transfer((lobbyEntry[1] * 96) /100);
    }
     ...
     */
    function flushdevShareOfStakeSells() external onlyOwner() {
        require(devShareOfStakeSellsAndLoanFee > 0);
        
        payable(avariceTeam_addr_1).transfer(devShareOfStakeSellsAndLoanFee);
        devShareOfStakeSellsAndLoanFee = 0;
    }


    /* Time of contract launch (0000-00-00|00:00:00) */
    uint256 internal constant LAUNCH_TIME = 1650881563;   
    uint256 currentDay;
    ...
    // debug
    // uint256 currentDay = 1;
    // uint256 currentDay_tobeset = 1;
    ...
    // debug
    // function _clcDay_tobeset() public view returns (uint256) {
    //     return currentDay_tobeset;
    // }

    // debug
    // function debug_setCurrentDay(uint256 __Day) public {
    //     currentDay_tobeset = __Day;
    //     _updateDaily();
    // }
    ...
    function _sendDevShare() internal {
        ...
        // [60%, 30%, 10%] 
        payable(avariceTeam_addr_1).transfer((avariceTeamPercentage * 60) /100);
        payable(avariceTeam_addr_2).transfer((avariceTeamPercentage * 30) /100);
    ...
    function _sendMarketingShare() internal {
        require(currentDay > 1);
    ...
    function _buyLobbyBuybackShare() internal {
        require(currentDay > 1);
    ...
    function _clcTokenValue (address _address, uint256 _Day) public view returns (uint256) {
        ...
        if(entryDay != 0 && entryDay < currentDay) {
            _tokenVlaue = ((lastLobbyPool * 1e18) / lobbyEntry[entryDay]) * mapMemberLobby[_address][_Day].memberLobbyValue; 
        }else{
            _tokenVlaue = 0;
        }
     ...
     * @param stakeId Id of the target stake
     */
    function calcStakeCollecting(address _address, uint256 stakeId) public view returns (uint256) {
        uint256 userDivs;
        uint256 _endDay = mapMemberStake[_address][stakeId].endDay;
        uint256 _startDay = mapMemberStake[_address][stakeId].startDay;

        for (uint256 subDay = _endDay -1 ; subDay >= _startDay ; subDay--) {
            userDivs += clcDayDivs(_address, stakeId, subDay);
        }
    
        /* stake divs minus it's loans return */
        return(userDivs - mapMemberStake[_address][stakeId].loansReturnAmount);
    }
     ...
     */
    function clcDayDivs(address _address, uint256 stakeId, uint256 _day) public view returns (uint256) {
        uint256 amount = mapMemberStake[_address][stakeId].tokenValue;
        uint256 profitDay;
        

        for(uint256 subDay = _day/2 ; subDay < _day ; subDay++){
            bool _isOdd = false;
            uint256 _dSC = _day / 2 ;
            ...
            if(_isOdd == true){
                if(subDay > _day / 2){
                    uint256 dayPoolADMEdit ;
                    if(subDay > dividendsPoolCapDays){
                        dayPoolADMEdit = ((lobbyEntry[subDay] * 94) /100) / dividendsPoolCapDays;
                    }else{
                        dayPoolADMEdit = ((lobbyEntry[subDay] * 94) /100) / subDay;
                    }
                     
                    if(updateActiveStakesTokens(_day) != 0){
                        profitDay += (amount * dayPoolADMEdit ) / updateActiveStakesTokens(_day);
                    }
    ...
    function updateActiveStakesTokens(uint256 day) public view returns(uint256) {
        uint256 totalplus;
        uint256 totalnegative;

        for (uint256 _day = 0 ; _day <= day ; _day++) {
            totalplus += daysActiveInStakeTokensIncrese[_day];
        }
        for(uint256 _day = 0 ; _day <= day ; _day++){
            totalnegative += daysActiveInStakeTokensDecrase[_day];
        }

        return (totalplus - totalnegative) ;
    }
    ...
    function sellStakeRequest(uint256 stakeId , uint256 price) external {
        ...
        require(stakeSelingIsPaused == false, 'functionality is paused');
    ...
    function withdrawSoldStakeFunds() external {
        ...
        payable(msg.sender).transfer(soldStakeFunds[msg.sender]);
    ...
    function getLoanOnStake(uint256 stakeId ,uint256 loanAmount , uint256 returnAmount , uint256 loanDuration) external {
        ...
        require(mapMemberStake[msg.sender][stakeId].stake_hasLoan == false, 'Target stake has an active loan on it');
        require(mapMemberStake[msg.sender][stakeId].stake_hasSold == false, 'Target stake has been sold');
        require(mapMemberStake[msg.sender][stakeId].endDay - loanDuration > currentDay + 1); 
    
        /* calc stake divs */
        uint256 stakeDivs = calcStakeCollecting(msg.sender , stakeId);

        /* max amount of possible stake return should be less that stake's current divs amount (minus 2% loan fee) */
        require(returnAmount <= (stakeDivs * 98) / 100); 
     ...
     * @param stakeId stake id
     */
    function lendOnStake(address loanerAddress , uint256 stakeId) external payable {
        _updateDaily();
        ...
        require(mapMemberStake[loanerAddress][stakeId].stake_hasSold == false);
        ...
        uint256 theLoanFee = (returnAmount * 2) / 100;  
        devShareOfStakeSellsAndLoanFee += theLoanFee /2;     
        lobbyEntry[currentDay] += theLoanFee /2;

        mapMemberStake[loanerAddress][stakeId].loansReturnAmount += returnAmount + theLoanFee;
        ...
        mapRequestingLoans[loanerAddress][stakeId].lend_endDay = currentDay + 1 + mapRequestingLoans[loanerAddress][stakeId].duration + 1 ;
    ...
    */
     ...
     * @param stakeId stake id
     */
    function updateFinishedLoan(address lenderAddress, address loanerAddress, uint256 lenderStakeId, uint256 stakeId) internal {
        _updateDaily();
        ...
        lendersPaidAmount[lenderAddress] += mapRequestingLoans[loanerAddress][stakeId].returnAmount;
        payable(lenderAddress).transfer(mapRequestingLoans[loanerAddress][stakeId].returnAmount);
    }
}