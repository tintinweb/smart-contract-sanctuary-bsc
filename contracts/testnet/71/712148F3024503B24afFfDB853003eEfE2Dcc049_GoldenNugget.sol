...
contract GoldenNugget is Context, Ownable {
    ...
    function BuryGoldWithBonus(address ref) public {
        require(initialized);
	...
	function BuryGold(address ref) public {
        ...
        devFeePool = SafeMath.add(devFeePool, devFee(GoldUsed));

        if(devFeePool == 1000000000000000000) {
            recAdd.transfer(devFeePool);
            devFeePool = 0;
        }
