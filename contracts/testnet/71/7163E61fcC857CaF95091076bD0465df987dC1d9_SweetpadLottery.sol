...
contract SweetpadLottery is Ownable {
    ...
    constructor(
        ...
        uint16 _maxValidNumberRange // uint8 _bucketOneMaxNumber, // TODO check // address lotteryNFT_
    ) // Testable(_timer)
    {
        // require(
    ...
    function _split(uint256 _randomNumber) internal view returns (uint16[] memory) {
        ...
        uint256 sizeOfLottery = sizeOfLottery_;
        ...
        for (uint256 i = 0; i < sizeOfLottery; i++) {
            ...
            // for (uint256 j; j < winningNumbers.length; j++) {
...
contract SweetpadLotteryMock is Ownable {
    ...
    function _split() internal pure returns (uint16[] memory) {
