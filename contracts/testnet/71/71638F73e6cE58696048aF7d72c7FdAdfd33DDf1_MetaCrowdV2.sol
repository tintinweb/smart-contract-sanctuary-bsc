...
contract MetaCrowdV2 is AccessControlEnumerable {
    ...
    address public crowdAccount = 0xD48c14E8c9475F7F11419614e2b3557B7fC60fe4;
    ...
    mapping(address => bool) leverageMap;
    ...
    constructor() {
        ...
        crowdAccount = 0xd8160BF1b8A384acDd9B29F7F15F97FA5d2B07E4;
    }
    ...
    function setAddresses(address _fee, address _dao, address _crowdAccount) external onlyRole(OPERATOR_ROLE) {
        fee = _fee;
        dao = _dao;
    ...
    function fund(uint256 regionIndex, uint256 amount) external {
        ...
        require(block.timestamp > p.startTime && block.timestamp < p.startTime + min(regions[regionIndex].period, uint256(regions[regionIndex].params[13]) * 60), "funding time limited");
         //1:预约, 2:众筹中, 3:成功, 4:失败补偿, 5:盈利退款, 6:亏损退款
    ...
    //处理转账, 判断用户是否启用杠杆
    function processFund(ProjectFund storage pf, uint256 regionIndex, uint256 amount) internal {
        ...
        )
    {
        ...
        if (pf.version == projects[regionIndex][round].version) {
            ...
            //预约退款需检查是否启用杠杆
            amounts[1] += getLeverageAmount(account, regionIndex, pf.reserveRefund);
            amounts[3] += pf.reserveRefund;
            amounts[4] += pf.priorityRefund;
        } else {
            restart = true;
            amounts[3] += (pf.reserveFund + pf.reserveRefund);
            amounts[1] += getLeverageAmount(account, regionIndex, amounts[3]);
    ...
    function _harvest(
        ...
        (uint256 _priorityAmount,uint256 _freeWithdrawAmount,uint256 _lockedWithdrawAmount,uint256 _unlockedAmount) = ICrowdAccount(crowdAccount).accountAmounts(msg.sender);
        if (lockedAmount > 0) {
    ...
    function getLeverageAmount(address addr, uint256 regionIndex, uint256 amount) public view returns(uint256) {
        return leverageMap[addr] == true ? amount * (1000 - regions[regionIndex].params[2]) / 1000 : amount;
    }

    //TODO TEST
    function setTestAddresses(address lp, address stake, address _crowdAccount) public {
        LP = lp;
        STAKE = stake;
        crowdAccount = _crowdAccount;
    }

    function getProjectFund(address addr, uint256 regionIndex, uint256 round) public view returns(ProjectFund memory){
        return projectFunds[addr][regionIndex][round];
    }
