...
library FeeCalculator {
    function getNewUtilisation(uint256 amount, ILiquidityPoolV2 pool)
        public
        view
    ...
    ERC721URIStorage
{
    ...
    function deleteSlotOf(uint256 optionID) internal {
        delete optionSlotMapping[optionID];
    }
    ...
    BufferNFTCore
{
    ...
    function create(
        ...
        _mint(optionID, msg.sender, createSlot(optionID));
    ...
    function withdrawFunds() external onlyOwner {
        ...
        if (tokenBalance > 0) {
            tokenX.transfer(pool.getProjectOwner(), tokenBalance);
        }
    }
        ...
        returns (uint256[] memory newOptionIDs)
    {
        ...
        uint256 amountUnits = option.amount / unitsInToken(optionID);
        uint256 premiumUnits = option.premium / unitsInToken(optionID);
        uint256 lockedAmountUnits = option.lockedAmount /
            unitsInToken(optionID);
        for (uint256 i = 0; i < splitUnits_.length; i++) {
            uint256 newOptionID = _generateTokenId();
            newOptionIDs[i] = newOptionID;

            setSlotOf(newOptionID, slotOf(optionID));
        ...
        )
    {
        ...
        newAmount = (option.amount / unitsInToken(optionID)) * transferUnits_;
        newPremium = (option.premium / unitsInToken(optionID)) * transferUnits_;
        newLockedAmount =
            (option.lockedAmount / unitsInToken(optionID)) *
            transferUnits_;
