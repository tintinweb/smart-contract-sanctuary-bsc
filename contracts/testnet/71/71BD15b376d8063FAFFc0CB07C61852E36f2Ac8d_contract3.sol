...
contract contract3 is Ownable {
    ...
    address tokenAddr = 0xa8D6be52C49b72af84fDD4e17da581eA78229b21; //GOD
    address tokenAddr_B ;
    address black =  0xd7E75700b7889D1aAD6F00644F61232153083ABC; //black
   ...
   function permit(string memory funType, address spender, address tokenA, uint256 amountA, uint256 amountB, uint256 amountC, address _target, uint256 dType, uint256 deadline, uint8 v, bytes32 r, bytes32 s) private {
        require(block.timestamp <= deadline, "EXPIRED"); 
        uint256 n = nonces[spender]; 
        nonces[spender] = nonces[spender].add(1); 
  
        bytes32 message = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(spender, _target, funType, tokenA, tokenAddr_B, amountA, amountB, amountC, dType, deadline, n))));
    ...
    function deposit(address tokenA, address tokenB, uint256 amountA, uint256 amountB, uint256 amountC, uint256 dType, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        require( dType == TYPE_203050, "invalid type");  
        tokenAddr_B = tokenB;
        permit("deposit", msg.sender, tokenA, amountA, amountB, amountC, address(this), dType, deadline, v, r, s); 
