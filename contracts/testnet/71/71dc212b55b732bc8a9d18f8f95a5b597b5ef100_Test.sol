...
contract SharedConstants {
    ...
    address public constant BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; //0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; 
...
contract Test is ERC20, Ownable, SharedConstants {
    ...
    constructor() ERC20("Beispiel", "BSP") { //Token name here
        dividendManager = new DividendManager();
        BeispielDT = dividendManager.addDividendTracker("BeispielDividendTracker", "BeispielDT", address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7), 60, 1000 * 10**18, 5, 5); //WBNB
        ...
        dividendManager.excludeFromAllDividends(BeispielDT);

        // exclude from paying fees or having mzax transaction amount
        isExcludedFromFees[owner()] = true;
        isExcludedFromFees[marketingWalletAddress] = true;
        isExcludedFromFees[address(this)] = true;
        isExcludedFromFees[BURN_ADDRESS] = true;
        isExcludedFromFees[BeispielDT] = true;

        _mint(owner(), 100_000_000 * (10**18)); //Set total Supply
    }

    receive() external payable {}

    function updateDividendManager (address newAddress) external onlyOwner {
        require (newAddress != address(dividendManager), "Beispiel: The dividend manager already exists");
        DividendManager newDividendManager = DividendManager(newAddress);
        
        require (newDividendManager.owner() == address(this), "Beispiel: The new dividend manager must be owned by the token contract");
        ...
        // newDividendManager.excludeFromAllDividends(BeispielDT);
        emit UpdateDividendManager (newAddress, address(dividendManager));
        dividendManager = newDividendManager;
    }

    function updateUniswapV2Router (address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Beispiel: The router already has that address");
    ...
    function excludeFromFees (address account, bool excluded) external onlyOwner {
        require (isExcludedFromFees[account] != excluded, "Beispiel: Account is already the value of 'excluded'");
    ...
    function setMarketingWallet (address payable wallet) external onlyOwner {
        require (wallet != address(0), "Beispiel: Can't set marketing wallet to zero address");
    ...
    function setBuyFees (uint8[] memory _rewardsFee, uint8 _liquidityFee, uint8 _marketingFee, uint8 _burnFee) external onlyOwner {
        require (_rewardsFee.length == dividendManager.dividendTrackers(), "Beispiel: Must provide a fee per dividend tracker (array wrong length)");
        ...
        require (totalBuyFees <= 35, "Beispiel: Fees can't be > 35%");
    ...
    function setSellFees (uint8[] memory _rewardsFee, uint8 _liquidityFee, uint8 _marketingFee, uint8 _burnFee) external onlyOwner {
        ...
        require (totalSellFees <= 35, "Beispiel: Fees can't be > 35%");
    ...
    function setAutomatedMarketMakerPair (address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "Beispiel: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function setSwapAmount (uint256 _swapTokensAtAmount) external onlyOwner {
        require (_swapTokensAtAmount >= totalSupply() / 1_000_000 && _swapTokensAtAmount <= totalSupply() / 100, "Beispiel: Swap amount must be between 0.00001% and 1% of total supply");
    ...
    function getRewardTokenPercentages (uint8 feeType) public view returns (uint256 BeispielFeePercentage, uint256 BUSDFeePercentage) {
        uint8 BUSDFee = dividendManager.getFee (feeType, BUSD); 
        uint8 BeispielFee = dividendManager.getFee (feeType, address(this));
        uint256 sumSells = transactionBuffer.sumPerKey[SELL];
        uint256 sumBuys = transactionBuffer.sumPerKey[BUY];
        
        // Beispiel Fee In Percent  = Sum Sells * Original Beispiel Fee * Total Dividend Fees / (Sum Buys * Original BUSD Fee + Sum Sells * Original Beispiel Fee)
        // increase Beispiel fee on sells, decrease on buys
        BeispielFeePercentage = sumSells * BeispielFee * dividendManager.getSummedFees (feeType) / (sumSells * BeispielFee + sumBuys * BUSDFee);
            
        // BUSD Fee In Percent = Sum Buys * Original BUSD Fee * Total Dividend Fees / (Sum Buys * Original BUSD Fee + Sum Sells * Original Beispiel Fee)
        // increase BUSD fee on buys, decrease on sells
        BUSDFeePercentage = dividendManager.getSummedFees (feeType) - BeispielFeePercentage;
    }
    ...
    function getTransferAmounts (uint256 amount, uint8 feeType) private returns (uint256, uint256, uint256) {
        ...
        (uint256 BeispielFeePercentage, uint256 BUSDFeePercentage) = getRewardTokenPercentages (feeType);
        uint256 BeispielTokens = amount * BeispielFeePercentage / 100;
        uint256 BUSDTokens = amount * BUSDFeePercentage / 100;
            
        // Keep track of balances so we can split the address balance
        dividendManager.incrementFeeTokens (BeispielTokens, address(this));
        dividendManager.incrementFeeTokens (BUSDTokens, BUSD);
        marketingTokens += _marketingTokens;
        liquidityTokens += _liquidityTokens;
        uint256 fees = _marketingTokens + _liquidityTokens + BeispielTokens + BUSDTokens;
    ...
    function _transfer (address from, address to, uint256 amount) internal override {
        ...
        else if (from == BeispielDT) { // Don't get caught in circular payout events
        ...
        if (from != owner() && !isExcludedFromFees[to] && to != uniswapV2Pair)
            require (balanceOf(to) + amount <= maxWalletAmount, "Beispiel: Receiver's wallet balance exceeds the max wallet amount");
    ...
    function sellTokensForBNBAndTakeFees (uint256 swapAmount) private {
        ...
        // Send native dividends to Beispiel dividend tracker and distribute
        uint256 BeispielDividends = dividendManager.getFeeTokensFromRewardAddress(address(this)) * swapAmount / contractTokenBalance;
        super._transfer (address(this), dividendManager.getTrackerAddress(address(this)), BeispielDividends);
        
        try dividendManager.distributeDividends(BeispielDividends, address(this)) {
            emit SendDividends (address(this), BeispielDividends, BeispielDividends);
            // Make sure we zero the count of tokens set aside for Beispiel dividends
            dividendManager.decrementFeeTokens(BeispielDividends, address(this));
        } catch {
