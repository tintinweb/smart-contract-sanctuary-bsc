  ...
  function registerClaimTransaction(
   ...
   function validateTransaction(bytes32 transactionId , bytes[] memory signatures ,bool mintable) external  {
       ...
       for (uint256 i ; i < signatures.length; i++)  {
          address signer = getSigner(Ibridge(owner()).chainId() ,transaction.chainId , transaction.assetAddress , transaction.amount , transaction.receiver , transaction.nounce , signatures[i]);
