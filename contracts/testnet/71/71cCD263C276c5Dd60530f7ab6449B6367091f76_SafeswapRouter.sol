...
contract SafeswapRouter is ISafeswapRouter02, Initializable {
    ...
    {
//        bool isBlacklist = ISafeswapFactory(factory).isBlacklistedStatus(to);
//        require(isBlacklist == false, "Address is blacklisted");
       // bool approvePartner = ISafeswapFactory(factory).approvePartnerStatus(to);
        require(ISafeswapFactory(factory).approvePartnerStatus(to) == true, "Waiting for partner approval");
        ...
        )
    {
        ...
        bool approvePartner = ISafeswapFactory(factory).approvePartnerStatus(to);
        require(approvePartner == true, "Waiting for partner approval");
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 amountOutput;
            {
                ...
                amountOutput = SafeswapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, factory);
            }
            (uint256 amount0Out, uint256 amount1Out) = input == token0
...
//        require(isBlacklist == false, "Address is blacklisted");
    ...
    function getAmountOut(
        ...
        return SafeswapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, factory);
    }
    ...
    function getAmountIn(
        ...
        return SafeswapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, factory);
    }
...
library SafeswapLibrary {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"597472b984721199f10ecb9cf1bf0285f2182df18fe50c8d2ea043e1e5519055" // init code hash
                    )
                )
            )
        );
    ...
    function getAmountOut(
        ...
        (uint256 totalFee, uint256 precision) = getConfigFee(factory);
        uint256 amountInWithFee = amountIn.mul(precision.sub(totalFee));
        uint256 numerator = amountInWithFee.mul(reserveOut);
        uint256 denominator = reserveIn.mul(precision).add(amountInWithFee);
    ...
    function getAmountIn(
        ...
        require(reserveIn > 0 && reserveOut > 0, "SafeswapLibrary: INSUFFICIENT_LIQUIDITY");
        (uint256 totalFee, uint256 precision) = getConfigFee(factory);
        uint256 numerator = reserveIn.mul(amountOut).mul(precision);
        uint256 denominator = reserveOut.sub(amountOut).mul(precision.sub(totalFee));
    ...
    function getConfigFee(address factory) public view returns (uint256, uint256) {
        return ISafeswapFactory(factory).getTotalFee();
    }
}
