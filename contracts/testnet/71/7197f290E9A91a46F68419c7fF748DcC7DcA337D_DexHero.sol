...
contract DexHero is ERC20, Ownable {
    ...
    uint128 private PRICE_USD = 100 ether;

    AggregatorV3Interface private _priceFeedETH; // ETH/USD
    IERC20 private _tokenUSD; // USD token

    mapping(address => uint32) private usdSums;
    mapping(address => uint32) private paidUpTo;
    ...
    function getUserUsdSum(address user) public view returns (uint256) {
        return usdSums[user] * 1e16;
    }
    ...
    function getUSDPerNFT(uint8 count) public view returns (uint256){
        return count * PRICE_USD;
    }

    function getETHperNFT(uint8 count) public view returns (uint256) {
    ...
    function mint(address to, uint256 mintAmount) private {
        ...
        paidUpTo[to] = uint32(timestamp + mintAmount * 2592000); // months
    }
    ...
    function buyForUSD(address to,uint8 mintAmount ) external {
        ...
        mint(to, mintAmount);
        usdSums[to]+= uint32(priceUSD / 1e16);
    }
    ...
    receive() external payable {
        ...
        sendETH(msg.sender, msg.value - priceETH);
        usdSums[msg.sender] += uint32(getUSDPerNFT(mintAmount) / 1e16);
    }
