...
contract SupplyChain {
    ...
    function newItem(string memory _BNum, string memory _RGNum ,string memory _PNtext, string memory _GNtext,string memory _DSS , string memory _COtext,string memory _MR,string memory _TR,string memory _IM,string memory _DI,  string memory _MDdate, string memory _EDtext) public returns (bool) {
        Product memory added = Product({batchNumber: _BNum,registrationNumber: _RGNum,creator: msg.sender, totalStates: 0, productName: _PNtext, productId: items, genericName: _GNtext,dosageStrength: _DSS, countryOrigin: _COtext, manufacturer: _MR , trader: _TR,distributor: _DI,importer:_IM, manufactureDate: _MDdate, expiryDate: _EDtext, viewCount: 0});
    ...
    function searchProduct(uint _productId) public returns (bool) {
        ...
        output=concat(output, "<br>Batch Number: ");
        output=concat(output, allProducts[_productId].batchNumber);
        ...
        output=concat(output, "<br>Dosage Strength and Form: ");
