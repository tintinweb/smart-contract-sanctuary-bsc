...
contract DDAContract is AccessControl {
    ...
    */
    function donate(uint256 _to, address _currency, uint256 _amount) external notBlackRole payable {
        ...
        if (_currency == ETH_COMPARE_ADDRESS) {
            console.log("msg.vaue: ", transferAmount);
            payable(charities[_to].walletAddress).transfer(transferAmount);
            address[] memory path = new address[](2);
            path[0] = WETH_ADDRESS;
            path[1] = OKAPI_ADDRESS;
            IUniswapV2Router02(SWAP_ROUTER_ADDRESS).swapExactETHForTokens{value: buyAmount}(0, path, msg.sender, block.timestamp + 60);
        }
        else {
