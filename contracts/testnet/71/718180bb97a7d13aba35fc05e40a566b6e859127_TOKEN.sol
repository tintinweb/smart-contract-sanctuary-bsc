...
abstract contract Ownable is Context {
  ...
  modifier onlyOwner() {
    require(
      owner() == _msgSender(),
      "Only admin can do this! and You're not the owner"
    );
    _;
  }
  ...
  function transferOwnership(address newOwner) public virtual onlyOwner {
    require(newOwner != address(0), "Ownership has been renounced");
    _setOwner(newOwner);
  }
...
contract TOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
  ...
  function _transfer(
    ...
    require(false, "Reward Tracker: Transfere are not allowed");
  }

  function withdrawDividend() public pure override {
    require(
      false,
      "Reward Tracker: Reward withdrawal is disabled! Use the 'claim' function on the token explorer to get the rewards."
    );
  }
  ...
  function updateClaimWait(uint256 newClaimWait) external onlyOwner {
    require(
      ...
      "Reward Tracker: The claimWait must be updated to between 1 and 24 hours"
    );
    require(
      newClaimWait != claimWait,
      "Reward Tracker: The claimWait can't update to the same value"
    );
...
contract TOKEN is BEP20, Ownable, BaseToken {
  ...
  uint256 public changeTaxFour = 100;
  ...
  constructor(address _cloneadd) BEP20("Reward Token", "RWT") {
    uint256 totalSupply_ = 100000000000000;
    rewardToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684; //Reward USDT For Now
    liquiditytoken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;

    swapTokensAtAmount = 100000000000; // 0.002%

    gasForProcessing = 300000;

    dividendTracker = TOKENDividendTracker(payable(Clones.clone(_cloneadd)));
  ...
  function burn(address account, uint256 amount) public {
    require(
      ...
      "Can't Burn: Your address should be whitelisted before burning the tokens"
    );
  ...
  function mint(address account, uint256 amount) public {
    require(
      ...
      "Can't Mint: Only token bridge can mint the tokens"
    );
    ...
    onlyOwner
  {
    require(
      pair != DEXV2Pair,
      "Token: The reward pair can't be removed from automated market maker pairs"
    );
  ...
  function _setAutomatedMarketMakerPair(address pair, bool value) private {
    require(
      ...
      "Token: The automated market maker pair is already set to that value"
    );
    automatedMarketMakerPairs[pair] = value;
  ...
  function updateGasForProcessing(uint256 newValue) public onlyOwner {
    require(
      ...
      "Token: The gas fee for processing the transaction, must be between 200000 to 500000"
    );
    require(
      newValue != gasForProcessing,
      "Token: Can't update the gas fee for processing to the same value"
    );
  ...
  function _transfer(
    ...
    if (WhiteListTransferable[from] || WhiteListTransferable[to]) {
      require(
        !Paused,
        "The transactions are paused! You can't make any transaction until the team resumes it"
      );
    }
