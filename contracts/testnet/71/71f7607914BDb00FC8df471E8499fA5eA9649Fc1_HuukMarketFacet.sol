...
contract HuukMarketFacet is IHuukMarketFacet {
        ...
        returns (uint256 _orderId)
    {
        ...
        if (
            ds.isHuukNFTs[_input.token.token] &&
            IHuukNFT(_input.token.token).getCreator(_input.token.id) ==
            _input.token.owner &&
            ds.amountFirstSale[_input.token.token][_input.token.id] == 0 &&
            ds.lastBuyPriceInUSDT[_input.token.owner][
                LibFunc.getTokenUnique(_input.token)
            ] ==
            0
        ) {
            ds.amountFirstSale[_input.token.token][_input.token.id] = balance;
        }
    ...
    function _acceptBid(
        ...
        uint256 exactPaymentAmount = (orderAmount * (ZOOM_FEE + ds.xUser)) /
            ZOOM_FEE;
        // 1.025
    ...
    function _match(
        ...
        if (buyerRef != address(0)) {
            uint256 amountToBuyerRef = (orderAmount *
                ds.xUser *
                (ZOOM_FEE - ds.yRefRate)) / (ZOOM_FEE**2);
        ...
        if (ds.isHuukNFTs[order.tokenAddress]) {
            ...
            if (
                ds.amountFirstSale[order.tokenAddress][order.tokenId] > 0 &&
                (creator == order.owner)
            ) {
                if (sellerRef != address(0)) {
                    {
                        ...
                        if (
                            ds.amountFirstSale[order.tokenAddress][
                                order.tokenId
                            ] >= _quantity
                        ) {
                            amountToSellerRef =
                                (orderAmount *
                                    ds.xCreator *
                                    (ZOOM_FEE - ds.yRefRate)) /
                                (ZOOM_FEE**2);
                            // take 15%*50% fee
                            amountToSeller =
                                amountToSeller -
                                ((orderAmount * ds.xCreator) / ZOOM_FEE);
                            // take 15% fee
                            ds.amountFirstSale[order.tokenAddress][
                                order.tokenId
                            ] -= _quantity;
                            ...
                            {
                                uint256 a = ds.amountFirstSale[order.tokenAddress][
                                    order.tokenId
                                ] *
                                    _price *
                                    ds.xCreator;
                                amountToSeller -= (a / ZOOM_FEE);
                                a = (a * (ZOOM_FEE - ds.yRefRate)) / (ZOOM_FEE**2);
                                amountToSellerRef = a;
                                a =
                                    _quantity -
                                    ds.amountFirstSale[order.tokenAddress][
                    ...
                    if (
                        ds.amountFirstSale[order.tokenAddress][order.tokenId] >=
                        _quantity
                    ) {
                        amountToSeller -= (orderAmount * ds.xCreator) / ZOOM_FEE;
                        ds.amountFirstSale[order.tokenAddress][
                            order.tokenId
                        ] -= _quantity;
                    } else {
                        uint256 b = (ds.amountFirstSale[order.tokenAddress][
                            order.tokenId
                        ] *
                            _price *
                            ds.xCreator) / ZOOM_FEE;
                        amountToSeller -= b;
                        b =
                            _quantity -
                            ds.amountFirstSale[order.tokenAddress][order.tokenId];
                        b = (b * _price * ds.xUser) / ZOOM_FEE;
                        amountToSeller -= b;
                    }
                }
                _paid(_paymentToken, order.owner, amountToSeller);
            } else {
                amountToSeller -= (orderAmount * ds.xUser) / ZOOM_FEE;
                if (sellerRef != address(0)) {
                    _paid(
                        _paymentToken,
                        sellerRef,
                        (orderAmount * ds.xUser * (ZOOM_FEE - ds.yRefRate)) /
                            (ZOOM_FEE**2)
                    );
                }
                ...
                if (order.owner == creator) {
                    ...
                    uint256 amountToCreator = IProfitEstimator(ds.profitSender)
                        .profitToCreator(
                            order.tokenAddress,
                            _paymentToken,
                            order.tokenId,
                            _quantity,
                            _price,
                            ds.lastBuyPriceInUSDT[order.owner][
                                keccak256(
                                    abi.encodePacked(
                                        order.tokenAddress,
                                        order.tokenId
    ...
    function _updateOrderAfterMatch(
        ...
        ds.lastBuyPriceInUSDT[_buyer][_id] = estimateUSDT(
            _paymentToken,
            _price
        );
        return true;
    ...
    function orders(uint256 _orderId) external override view returns (LibStruct.Order memory) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
...
interface IHuukMarketFacet {
    ...
    function orders(uint256 _orderId) external view returns (LibStruct.Order memory);
}
