...
contract PreSale is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
     ...
     * @param value weis paid for purchase
     */
    event Refunded(address beneficiary, uint256 value);
    /**
     * Event for emergency withdraw while active
     * @param beneficiary who got the tokens
     * @param value weis paid for purchase
     */
    event EmergencyWithdrawn(address beneficiary, uint256 value, uint256 fee);
    ...
    function initialize(
        ...
        __Presale_init_unchanged(params, _stageTimes, _stagePercents);
    }
    ...
    function __Presale_init_unchanged(
        ...
        // token to sell
        mainToken = IERC20(params.mainToken);
    ...
    function claimableAmount(address user) public view returns (uint256) {
        ...
        uint totalClaimable = percent.mul(userTokenTally[user]).div(100);
            ...
            for (uint256 i = 0; i < contributorIndex; i++) {
                ...
                if (contribution > 0) {
                    ...
                    if (payToken == address(0)) {
                        ...
                        IERC20(payToken).transfer(payable(contributors[i]), contribution);
                    }
                }
            }
        }
    ...
    function finalize() external onlyOwner nonReentrant {
        ...
        if (payToken == address(0)) {
            ...
            uint256 ethBalance = IERC20(payToken).balanceOf(address(this));
            if (ethBalance > 0) {
                IERC20(payToken).safeTransfer(devaddr, ethBalance);
            }
        }
    ...
    function buyWithToken(uint256 weiAmount) external nonReentrant {
        require(!finalized && !canceled, "Presale: has been ended already");
        ...
        IERC20(payToken).safeTransferFrom(msg.sender, address(this), weiAmount);
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (payToken == address(0)) {
            ...
            IERC20(payToken).safeTransfer(msg.sender, value);
            IERC20(payToken).safeTransfer(devaddr, fee);
        }
        emit EmergencyWithdrawn(msg.sender, value, fee);
    }

    function claim() external nonReentrant {
        require(finalized, "Presale: not finalized yet");
        uint256 amount = claimableAmount(msg.sender);
        require(amount > 0, "Presale: no claimable tokens");
    ...
    function claimRefund() external nonReentrant {
        ...
        if (payToken == address(0)) {
            ...
            IERC20(payToken).safeTransfer(msg.sender, ethAmount);
        }
        
        emit Refunded(msg.sender, ethAmount);
    }
    ...
    function addLiquidityToken() private returns (uint256) {
        ...
        mainToken.approve(address(uniswapV2Router), tokenAmount);
        IERC20(payToken).approve(address(uniswapV2Router), ethAmount);
