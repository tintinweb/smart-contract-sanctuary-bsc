...
contract balahap {
    ...
    enum result{ NONE, WIN, LOST, DRAW, DISPUTE}

    struct offer{
        address hostAddress;
        uint256 offerAmount;
        uint256 offerAmtLeft;
    ...
    struct bidder{
        address bidderAddr;
        uint256 betAmt;
        uint256 sportmanshipFee; // requirded ?
        bool isHostClaimed;
    ...
    function areAllBidderClaimed(uint256 _offerNo) public view returns (bool){
        uint256 _num = bidderLength(_offerNo);
        ...
        if (_num == 0){
            ...
            for(uint256 i = 0 ; i < _num; i++){
                if(!biddersList[_offerNo][i].isBidderClaimed){
                    _allClaimed = false;
                    break;
                }
            }
    ...
    function getBidderBetAmt(uint256 _offerNo, address _address) public view returns(uint256){
        uint256 _bidderNum = bidderNum(_address,_offerNo);
        return biddersList[_offerNo][_bidderNum].betAmt;
    }
    ...
    function makeOffer(uint256 _offerAmount,uint256 _closeBetTime,string memory _condition) external {
        ...
        offerNo[_id] = offer(msg.sender, _offerAmount, _offerAmount, _closeBetTime, _condition, false, offerStatus.OPEN, result.NONE);
    }

    function bet(uint256 _offerNo, uint256 _betAmt) external {
        uint256 _offerAmtLeft = offerNo[_offerNo].offerAmtLeft;
        ...
        require(_offerAmtLeft > _betAmt, "Bet amount excess max bet");
        
        offerNo[_offerNo].offerAmtLeft = _offerAmtLeft - _betAmt;
    ...
    function claimPrize(uint256 _offerNo) external {
        ...
        if(msg.sender == offerNo[_offerNo].hostAddress){
            ...
            if(offerNo[_offerNo].hostResult == result.WIN){
                /* Host Win */

                for(uint256 i = 0 ; i < _num; i++){
                    if(biddersList[_offerNo][i].bidderResult == result.NONE || biddersList[_offerNo][i].bidderResult == result.LOST){
                        /*Check Dispute*/
                        if(!biddersList[_offerNo][i].isHostClaimed){
                            address _bidderAddr = biddersList[_offerNo][i].bidderAddr;
                            uint256 _bidderSmsFee = sportmanshipFeeCal(biddersList[_offerNo][i].betAmt);
                            _totalClaim += biddersList[_offerNo][i].betAmt;
                            biddersList[_offerNo][i].isHostClaimed = true;
                            biddersList[_offerNo][i].isBidderClaimed = true;
                            /* Refund SMS Fee to bidder by Host */
                            IERC20(BUSD).transfer(_bidderAddr,_bidderSmsFee);
                ...
                if(_isAllBidderAccept){   
                    ...
                    IERC20(BUSD).transfer(address(msg.sender),_bet + _totalClaim - betFeeCal(_totalClaim) + _smsFee);
                } else {
                ...
                if(_isAllBidderAccept){
                    ...
                    IERC20(BUSD).transfer(address(msg.sender),_totalClaim + _offerAmtLeft);
                    /* No manage smsFee yet for this condition*/
                }
                

            }else if (offerNo[_offerNo].hostResult == result.NONE){
                require(bidderLength(_offerNo) == 0, "Please submit result");
                offerNo[_offerNo].status = offerStatus.CLOSE;
                IERC20(BUSD).transfer(address(msg.sender),_bet + _smsFee);
            }
        ...
        /* No dispute claim yet*/
            ...
            uint256 _num = bidderNum(msg.sender,_offerNo);
            uint256 _prize = _bet;
            ...
            if(biddersList[_offerNo][_num].bidderResult == result.WIN){
                /* Bidder Win */
                require(offerNo[_offerNo].hostResult == result.NONE || offerNo[_offerNo].hostResult == result.LOST , "Host is not accept your win");
                
                biddersList[_offerNo][_num].isBidderClaimed = true;

                if(areAllBidderClaimed(_offerNo)){
                    offerNo[_offerNo].status = offerStatus.CLOSE;
                }

                IERC20(BUSD).transfer(address(msg.sender), _bet + _totalPrize + _smsFee);

            }else if(biddersList[_offerNo][_num].bidderResult == result.DRAW){
                /* Bidder Draw */
                require(offerNo[_offerNo].hostResult == result.NONE || offerNo[_offerNo].hostResult == result.DRAW , "Host is not accept your Draw");
                //DRAW is no fee
                IERC20(BUSD).transfer(address(msg.sender), _bet + _smsFee);
                /* No offer close with draw condiftion yet */
            }
        }
    }
    ...
    /* REMOVE ON PRODUCTION, FOR DEV PERION ONLY */

    function changeStatus(uint256 _offerNo, offerStatus _result)external onlyAdmin{
        offerNo[_offerNo].status = _result;
    }
