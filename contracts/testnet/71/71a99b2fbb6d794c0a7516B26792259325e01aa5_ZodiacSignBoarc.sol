...
contract ZodiacSignBoarc is ERC721, ERC2981, SafeTransfer, Pausable, Ownable {
	using EnumerableSet for EnumerableSet.AddressSet;
	using EnumerableSet for EnumerableSet.UintSet;

	uint256 public constant MAX_SUPPLY = 156;
	...
	event Minted(address owner, uint256 tokenId, uint256 timestamp);
	...
	) ERC721("Zodiac Sign", "ZOS") {
		require(_feeRecipient != address(0), "ZOS: zero address");
		baseURI = _uri;
		feeMint = _feeMint;
		feeRecipient = _feeRecipient;

		_setDefaultRoyalty(_feeRecipient, ROYALTY_FEE);
		_initId();
	}

	function tokenURI(uint256 tokenId) public view override returns (string memory) {
		uint256 id = tokenId % 12;
		return string(abi.encodePacked(super.tokenURI(id), ".json"));
	}

	function totalSupply() public view returns (uint256) {
		return MAX_SUPPLY - tokenIds.length();
	}
	...
	function setBaseURI(string memory uri) public onlyOwner {
		require(bytes(uri).length > 0, "ZOS: invalid uri");
		baseURI = uri;
	...
	function claim() public payable whenNotPaused {
		require(msg.value >= feeMint, "ZOS: Insufficient fee");
		require(tokenIds.length() > 0, "ZOS: Sold out");

		payable(feeRecipient).transfer(feeMint);
		uint256 diff = msg.value - feeMint;
		if (diff > 0) payable(_msgSender()).transfer(diff);

		uint256 tokenId = _pickRandomId();
		_safeMint(_msgSender(), tokenId);
		tokenIds.remove(tokenId);

		_setTokenRoyalty(tokenId, _msgSender(), ROYALTY_FEE);
		if (!whitelist.contains(_msgSender())) whitelist.add(_msgSender());
	...
	function _initId() internal {
		for (uint256 i = 1; i <= MAX_SUPPLY; i++) {
			tokenIds.add(i);
		}
	}

	function _pickRandomId() internal view returns (uint256) {
		bytes memory randomNumber;
		uint256 index;

		randomNumber = abi.encodePacked(_msgSender(), address(this), block.gaslimit, gasleft(), block.timestamp, block.number, msg.sig, blockhash(block.number), block.difficulty);
		index = uint256(keccak256(randomNumber)) % (tokenIds.length());
