...
contract BetWorldcupRegister is Ownable {
  ...
  */
  ...
  uint256 private _rewardTokenRegister = 50 * 1e18;
  ...
  */
  ...
  mapping (address => address[]) public referered;
  ...
  */
  ...
  function rewardReferered(address referer) internal {
    bool exist = false;
    for (uint256 i = 0; i < referered[referer].length; i++) {
      if (referered[referer][i] == msg.sender) {
        exist = true;
        break;
      }
    }
    ...
    if (!exist) {
      referered[referer].push(msg.sender);
    }
  }
