...
contract GT3HashGame is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _gt, address _gt1, address _gt3, address _deadAddr, 
    ...
    function joinGame() external nonReentrant {
        ...
        // Check balance
        uint256 _balance = GT3.balanceOf(msg.sender);
        require(_balance >= _amount, "Insufficient token");

        // Check allowance 
        uint256 _allowance = GT3.allowance(msg.sender, address(this));
        require(_allowance >= _amount, "Insufficient allowance");

        // Transfer fund
        GT3.safeTransferFrom(msg.sender, address(deadAddr), _amount);
    ...
    function distributePremiumRewards(uint256 _gameMode, address _userAddress) internal {
        ...
        IGT3.mint(_userAddress, _amount);
    ...
    function distributeGameReward(uint256 _gameMode, address _userAddress, uint256 _amount) internal {
        ...
        // Mint sub reward 
        if(_subReward > 0)
            IGT3.mint(_userAddress, _subReward);
