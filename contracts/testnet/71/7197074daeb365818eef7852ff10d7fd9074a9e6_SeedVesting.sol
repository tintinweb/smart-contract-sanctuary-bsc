...
contract SeedVesting{
  ...
  function setUsers(address[] calldata  _userAddress, uint256[] calldata _percentage) external onlyOwner{
    ...
    require(percentage == 100 * 10 ** 18, "~100%");
  }
  ...
  function claimnew(address _beneficiary) public whenNotPaused{
    address beneficiary;
  ...
  function readClaim(address _beneficiary) public view returns(uint256,uint256,uint256,uint256){
    ...
    uint256 transf = (amount * users[beneficiary].percentage) / 10 ** decimals;
    return (transf,amount,distributed,users[beneficiary].percentage);
  }
