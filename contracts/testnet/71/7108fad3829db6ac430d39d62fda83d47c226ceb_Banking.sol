...
contract Banking is Ownable {
    ...
    // Link address of token to this smart contract
    constructor(ERC20 _tokenAddress) {
        tokenAddress = _tokenAddress;
    }
    // Change address of token
    function setTokenAddress(ERC20 _tokenAddress) external onlyOwner {
        tokenAddress = _tokenAddress;
    }

    mapping(address => uint) public amountUserOwn;

    // Transfer token
    function deposit(uint _amount) external {
        // tokenAddress is ERC20 => inherit function of ERC20
        tokenAddress.transferFrom(msg.sender, address(this), _amount);
        amountUserOwn[msg.sender] += _amount;
    }

    function withdraw(uint _amount) external {
        require(amountUserOwn[msg.sender] >= _amount, "You don't have enough token in your current account");
        amountUserOwn[msg.sender] -= _amount;
