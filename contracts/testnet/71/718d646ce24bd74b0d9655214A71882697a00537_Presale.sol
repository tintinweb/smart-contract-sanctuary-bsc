...
contract Presale is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 winGiveAway; //total token wined
    ...
    mapping(address => mapping(uint => UserInfo)) private _balances;
    ...
    uint256 public apr = 15000; //150%/year
    ...
    function balanceOf(uint pid, address account) public view returns (uint256) {
        return _balances[account][pid].locked - _balances[account][pid].withdrawed;
    }

    function getUserInfo(uint pid, address account) public view returns (UserInfo memory) {
        return _balances[account][pid];
    }
    ...
    function buyToken(uint256 pid, address refaddr, uint ln) public payable {
        ...
        //USER
        _balances[beneficiary][pid].pid = pid;
        _balances[beneficiary][pid].locked = _balances[beneficiary][pid].locked.add(tokenAmount);
        ...
        if (refaddr != address(0)) {
            //total ref:
            _balances[refaddr][pid].refRaised = _balances[refaddr][pid].refRaised.add(weiAmount);
            //log ref
            _refLogs[refaddr].count ++;
    ...
    //Claim locked token:
    function amountToClaim(uint pid, address holder) public view returns(uint256){
        if(_balances[holder][pid].locked == 0){
            return 0;
        }
        //before release time
        if(block.timestamp < releaseTime){
        ...
        for (uint256 i = 0; i < vestingLength - 1; i++) {
            if(block.timestamp >= releaseTime.add(_vesting[i].openingTime) && block.timestamp < releaseTime.add(_vesting[i + 1].openingTime)){
                return _balances[holder][pid].locked.mul(_vesting[i].percent).div(denominator).sub(_balances[holder][pid].withdrawed);
            }
        }
        // release all
        if(block.timestamp >= releaseTime.add(_vesting[vestingLength - 1].openingTime)){
            return _balances[holder][pid].locked.sub(_balances[holder][pid].withdrawed);
        }
        return 0;
    }
