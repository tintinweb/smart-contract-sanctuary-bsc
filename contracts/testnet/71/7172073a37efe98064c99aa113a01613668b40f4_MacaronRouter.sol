...
library MacaronLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'ecba335299a6693cb2ebc4782e74669b84290b6378ea3a3873c7231a8d7d1074'   // Change to INIT_CODE_PAIR_HASH of Macaron Factory
            ))));
    }
...
contract MacaronRouter is IMacaronRouter02 {
    ...
    bool public advanceOption = false;
    ...
    function swapExactTokensForTokens(
        ...
        (uint fees , uint afterFees , uint amountOutMinAfterFees)  = countTokenFees(amountIn, msg.sender, to, path , amountOutMin );
        _takeTokenFees(path[0],fees,msg.sender);
        amounts = MacaronLibrary.getAmountsOut(factory, afterFees, path);
        require(amounts[amounts.length - 1] >= amountOutMinAfterFees, 'MacaronRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        (uint fees , uint afterFees , uint amountInMaxAfterFees)  = countTokenFees(amountOut, msg.sender, to, path , amountInMax);
         _takeTokenFees(path[0],fees,msg.sender);
        amounts = MacaronLibrary.getAmountsIn(factory, afterFees, path);
        require(amounts[0] <= amountInMaxAfterFees, 'MacaronRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, MacaronLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
        ...
        returns (uint[] memory amounts)
    {
        ...
        if (_isExcludedFromFee[msg.sender] ||  _isExcludedFromFee[path[path.length - 1]]) {
            takeFee = false;
        }
        ...
        uint256 amountOutFees = amountOutMin.mul(tFees) / 10000;
        uint256 amountOuts = amountOutMin.sub(amountOutFees);
        uint256 fees = msg.value.mul(tFees) / 10000;
        ...
        returns (uint[] memory amounts)
    {
        ...
        (uint fees , uint afterFees , )  = countTokenFees(amounts[amounts.length - 1], msg.sender, to, path , amounts[amounts.length - 1]);
        _takeFees(fees);
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(advanceOption, "Process not allowed!!");
        require(path[0] == WETH, 'MacaronRouter: INVALID_PATH');
        amounts = MacaronLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'MacaronRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
       ...
       (uint fees , uint afterFees , uint amountOutMinAfterFees)  = countTokenFees(amountIn, msg.sender, to, path , amountOutMin );
        _takeTokenFees(path[0],fees,msg.sender);
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, MacaronLibrary.pairFor(factory, path[0], path[1]), afterFees
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMinAfterFees,
            'MacaronRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        ensure(deadline)
    {
        ...
        IWETH(WETH).deposit{value: afterFees}();
        assert(IWETH(WETH).transfer(MacaronLibrary.pairFor(factory, path[0], path[1]), afterFees));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        address tokenAddress = path[path.length - 1];
        require(
            IERC20(tokenAddress).balanceOf(to).sub(balanceBefore) >= amountOuts,
            'MacaronRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        ensure(deadline)
    {
        ...
        (uint fees , uint afterFees , )  = countTokenFees(amountOut, msg.sender, to, path , amountOut);
         _takeFees(fees);
        TransferHelper.safeTransferETH(to, afterFees);
    }
    ...
    function countTokenFees(uint256 amount , address _userAddress , address _toAddress , address[] memory path , uint amountOut  ) internal returns(uint fees , uint afterFees , uint amountOutMin){ 
        ...
        uint256 afterFeesCount = amount.sub(fees);
        
        return (feesCount,afterFeesCount,amountOuts);

    }

    function setAdvanceOption(bool _setoptionvalue) public adminOnly{
        advanceOption = _setoptionvalue;
    }
}