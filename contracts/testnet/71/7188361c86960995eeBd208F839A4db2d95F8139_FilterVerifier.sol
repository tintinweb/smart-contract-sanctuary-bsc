...
contract FilterVerifier {
    ...
    function rejectVerificationRequest(address _tokenAddress) external onlyAdmin {
        ...
        payable(verificationRequestMaker[_tokenAddress]).transfer(filterManager.verificationRequestFee() / 2);
    ...
    function makeVerificationRequest(address _tokenAddress) external payable {
        require(verificationRequestStatuses[_tokenAddress] == 0 || verificationRequestStatuses[_tokenAddress] == 4, "FilterVerifier: ALREADY_SUBMITTED");
    ...
    function claimExpiredRequestFee(address _tokenAddress) external {
        require(msg.sender == verificationRequestMaker[_tokenAddress], "FilterVerifier: NOT_REQUEST_MAKER");
        ...
        payable(verificationRequestMaker[_tokenAddress]).transfer(filterManager.verificationRequestFee());
    }
}