...
contract KojiMarket is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function createMarketItem(
      ...
      tokens[_itemsHeld.current()] = tokenId;
      ...
      items.owner = payable(address(this));
      items.price = price;
      items.sold = false;
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
      ...
      for (uint i = 0; i < itemCount; i++) {
        if (idToMarketItem[tokens[i]].owner == address(this)) {
          MarketItem storage currentItem = idToMarketItem[tokens[i]];
    ...
    function fetchMyNFTs() public view returns (MarketItem[] memory) {
      ...
      for (uint i = 0; i < totalItemCount; i++) {
        if (idToMarketItem[tokens[i]].owner == msg.sender) {
          itemCount++;
        }
      }
    ...
    function fetchItemsListed() public view returns (MarketItem[] memory) {
      ...
      for (uint i = 0; i < totalItemCount; i++) {
        if (idToMarketItem[tokens[i]].seller == msg.sender) {
          itemCount++;
        }
      }
