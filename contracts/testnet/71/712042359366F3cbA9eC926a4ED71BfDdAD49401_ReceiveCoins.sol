...
contract ReceiveCoins is Ownable {
    ...
    address public tokenAddr = 0x7AcF8eF99aCF4E8ae12C6f1953622997be11F0e5; //FPN
    ...
    event EventDepositSingle(address indexed from, uint256 amount, address tokenA, uint256 time, address black); 
   ...
   function permit(string memory funType, uint256 numID, address spender, address tokenA, uint256 amount, address _target, uint256 deadline, uint8 v, bytes32 r, bytes32 s) private {
     ...
     function depositSingle(uint256 numID, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public ensure(deadline) {
        ...
        safeTransferFrom(tokenAddr, msg.sender, black, amount); 

        emit EventDepositSingle(msg.sender, amount, tokenAddr,block.timestamp,black); 
    ...
    function setblack(address _black) public onlyOwner{
        require(_black != address(0),"zero address!");
        black = _black;
