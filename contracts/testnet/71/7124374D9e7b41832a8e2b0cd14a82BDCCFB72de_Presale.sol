...
contract Presale {
	...
	ERC20 _busd = ERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);
	address payable public _admin = payable(0x06B0C1f32BAB7bcC00a18EB888Da11847Ed3c5F5);
	bool public _active;
	uint256 public _rate;

	constructor() {
		_rate = 1000; // 1000 tokens for 1 BUSD
		_active = true; // setting presale to be active
	}

	function getTokens(address beneficiary, uint256 busdAmount) public payable {
		// this line checks if the presale is active, if it's inactive it will stop the function
		require(_active, 'presale inactive');

		// calculate how much tokens to send
		uint256 value = busdAmount * _rate;
		...
		// send BUSD to contract first
		_busd.transferFrom(beneficiary, address(this), busdAmount);

		// send BUSD to the admin address
		_busd.transfer(_admin, busdAmount);
	}
