...
import "../contracts/interfaces/IMVCR.sol";
import "../contracts/interfaces/ITreasury.sol";

contract MvcrHashGame is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public MVCC;
    ...
    struct UserGameInfo {
        ...
        uint256 previousGameWonTimestamp;
    }
    ...
    function initialize(address _mvcc, address _mvcr, address _deadAddr,
        uint256 _gameMinThreshold, uint256 _gameMaxThreshold, uint256 _premiumDistributionThreshold, 
        uint256 _maxRoundPerGame, uint256 _cooldownInterval, bool _gamePaused
    ) public initializer {

        MVCC = IERC20Upgradeable(_mvcc);
    ...
    function joinGame(uint256 _amount) external nonReentrant {
        UserGameInfo storage _userInfo = userGameInfo[msg.sender];

        require(!gamePaused, "Game paused");
        require(!blacklists[msg.sender], "Blacklisted");
        require(isValidPremium(msg.sender), "Require premium to play the game");
        require(_amount >= gameInfo[_userInfo.gameMode].initialGameFee, "Amount must be larger than the initial game fee");
        ...
        // Transfer MVCR
        MVCR.safeTransferFrom(msg.sender, DEAD_ADDR, _amount);

        // Trick play
        if(_amount != getNextRoundFee(msg.sender)) {
            voidPremium(msg.sender);
        }
        ...
        emit JoinGame(_userInfo.gameMode, msg.sender, _userInfo.currentGameRound, _amount);
    }
    ...
    function processGame(address _userAddress, bool _result) external nonReentrant onlyOperator {
        ...
        if(_result) {    
            ...
            distributeGameRewards(_userInfo.gameMode, _userAddress, _rewards);

            // Update user last won game time
            _userInfo.previousGameWonTimestamp = block.timestamp;
            
        } else if(_userInfo.currentGameRound == maxRoundPerGame && _userInfo.currentGameCheckpoint == maxRoundPerGame) {
            // User game burst
            // Mint MVCR to user if with valid premium
    ...
    function purchasePremium(uint256 _gid, address _userAddress) internal {
        ...
        MVCC.safeTransferFrom(_userAddress, address(this), _gameInfo.premiumFee);
    ...
    function distributePremium() internal {
        ...
        if(_balance >= premiumDistributionThreshold) {
            MVCC.safeTransfer(genesisNodeReceiver, _balance * genesisNodePercentage / 100);
            MVCC.safeTransfer(tokenHolderReceiver, _balance * tokenHolderPercentage / 100);
            MVCC.safeTransfer(liquidityPoolReceiver, _balance * liquidityPoolPercentage / 100);
            MVCC.safeTransfer(insurancePoolReceiver, _balance * insurancePoolPercentage / 100);
            MVCC.safeTransfer(marketSystemReceiver, _balance * marketSystemPercentage / 100);
        }
    ...
    function distributeGameRewards(uint256 _gameMode, address _userAddress, uint256 _amount) internal {
        uint256 _busdRewards = gameInfo[_gameMode].initialGameFee;
        uint256 _mvcrRewards = (_amount * 2) - _busdRewards;

        ITreasury.withdraw(_userAddress, _busdRewards);

        // Mint MVCR rewards 
        if(_mvcrRewards > 0)
            IMVCR.mint(_userAddress, _mvcrRewards);
        
        emit DistributeGameRewards(_gameMode, _userAddress, _busdRewards, _mvcrRewards);
    }
    ...
    // SETTERS
    // ===================================================================
    function setMvccAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Address zero");
        MVCC = IERC20Upgradeable(_newAddress);
    ...
    function pauseGame(bool _boolValue) external onlyOperator {
        gamePaused = _boolValue;

        emit PauseGame(_boolValue);
    }
    ...
    event JoinGame(uint256 gid, address userAddress, uint256 currentGameRound, uint256 amount);
    event ProcessGame(uint256 gid, address userAddress, bool result, uint256 rewards);
    event DistributeGameRewards(uint256 gid, address userAddress, uint256 busdReward, uint256 mvcrReward);
    event DistributeBusdRewards(address userAddress, uint256 amount);
    ...
    event RegisterGameInfo(uint256 premiumFee, uint256 _initialGameFee);
    ...
    event SetPremiumFee(uint256[] mvcrPremiumFeeList);
    event SetBlacklist(address[] userAddress, bool boolValue);
    event PauseGame(bool boolValue);
