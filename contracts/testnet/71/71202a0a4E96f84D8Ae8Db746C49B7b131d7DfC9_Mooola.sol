...
contract Mooola is Ownable,ReentrancyGuard{
    using SafeMath for uint256;

    uint256 public developerFee = 500; // 500 : 5 %. 10000 : 100 %
    uint256 public referrerFee = 500; // 500 : 5 %. 10000 : 100 %
    uint256 public rewardPeriod = 1 days;
    uint256 public lockPeriod = 30 days;
    uint256 public apr = 75; // 75 : 0.75 %. 10000 : 100 %
    ...
    function claimAllReward() public nonReentrant {
        ...
        if (investors[msg.sender].referrer == address(0)) {
            ...
            referrerAmount = claimableAmount.mul(referrerFee).div(percentRate);
            investors[msg.sender].claimedAmount = investors[msg.sender].claimedAmount.add(claimableAmount.sub(referrerAmount));
        }
        ...
        if (investors[msg.sender].referrer == address(0)) {
            ...
            payable(msg.sender).transfer(claimableAmount.sub(referrerAmount));
    ...
    function getInvestor(address _investorAddress) public view returns(
        ...
        referrer = investors[_investorAddress].referrer;
    }
