...
contract PVTB is TokenVesting, TokenSaleWhitelist {
    ...
    constructor(
        ...
        uint256 _minBNB,
        uint256 _maxBNB
    ) TokenVesting(_token, _startDate) {
        price = _price;
        minBNB = _minBNB;
        maxBNB = _maxBNB;
    ...
    function getBNBtoUSDPrice() public view returns (int256) {
        (
            uint80 roundID,
            int256 _price,
            uint256 startedAt,
    ...
    modifier buyValidate() {
        ...
        require(
            startDatePrivateSale() < block.timestamp,
            "Private sale is not yet started."
        );
    ...
    function buyToken(address _account) public payable buyValidate {
        require(_account != address(0), "Address zero forbidden");

        require(
            (totalBNBSpend[_account] + msg.value) <= maxBNB,
            "Max BNB reach"
        );
    ...
    function tokenQty(uint256 _wei) public view returns (uint256) {
        uint256 usdt_price = uint256(getBNBtoUSDPrice());
    ...
    function addPrivateSaleBeneficiariesToWhitelist(
        address[] memory beneficiaries
    ) public onlyOwner {
        return addToWhitelist(beneficiaries);
    }

    function removePrivateSaleBeneficiaryToWhitelist(address beneficiary)
        public
        onlyOwner
    {
