...
contract MasterChef is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cakeReward = multiplier.mul(cakePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        syrup.transferFrom(msg.sender,address(this),cakeReward);
        syrup.transfer(devaddr, cakeReward.div(10));
    ...
    function deposit(uint256 _pid, uint256 _tokenId) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                uint per = pending.mul(10).div(100);
                safeCakeTransfer(msg.sender, pending.sub(per));
                safeCakeTransfer(devaddr,per);
            }
        }
        if (_tokenId >= 0) {
            pool.nft.transferFrom(address(msg.sender), address(this), _tokenId);
    ...
    function withdraw(uint256 _pid, uint256 _tokenId) public {
        ...
        if(_tokenId >= 0) {
            ...
            pool.nft.transferFrom(address(this),address(msg.sender), _tokenId);
        }
        user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e12);
    ...
    function enterStaking(uint256 tokenId) public {
        ...
        if(tokenId >= 0) {
            pool.nft.transferFrom(address(msg.sender), address(this), tokenId);
            user.amount = user.amount.add(1);
        }
        user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e12);

       
        emit Deposit(msg.sender, 0, tokenId);
    }

    // Withdraw CAKE tokens from STAKING.
    function leaveStaking(uint256 _tokenId) public {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[0][msg.sender];
        require(user.amount >= 0, "withdraw: not good");
        updatePool(0);
        ...
        emit Withdraw(msg.sender, 0, _tokenId);
    }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        pool.nft.transferFrom(address(this),address(msg.sender), user.amount);
