...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint8 private constant BUNDLE_TYPE_PARCELS_2 = 10;
    uint8 private constant BUNDLE_TYPE_PARCELS_3 = 20;
    uint8 private constant BUNDLE_TYPE_MINERS_1 = 30;
    uint8 private constant BUNDLE_TYPE_MINERS_2 = 40;
    uint8 private constant BUNDLE_TYPE_MINERS_3 = 50;
    ...
    function purchaseMinersBundle(uint256 _referrerId, uint8 _bundleType) external onlySaleActive notZeroAddress {
        ...
        _automaticallyAttachMinersToParcels(msg.sender, firstMinerId, count);
        emit BundlePurchased(msg.sender, _bundleType);
    }
    ...
    function _automaticallyAttachMinersToParcels(address user, uint256 firstMinerId, uint256 minersAmount) private {
        uint256 minerId = firstMinerId;
        uint256[] memory parcelIds = IERC721Base(standardParcelNFTAddress).tokensOf(user);
        for(uint256 i = 0; i < parcelIds.length; ++i) {
            uint256 parcelId = parcelIds[i];
            uint256[] memory miners = parcelMinersMapping[parcelId];
            uint256 availableSize = standardParcelSlotsCount - miners.length;
            if(availableSize > (firstMinerId + minersAmount) - minerId) {
                availableSize = (firstMinerId + minersAmount) - minerId;
            }
            uint256[] memory newMiners = new uint256[](availableSize + miners.length);
            if(miners.length < standardParcelSlotsCount) {
                for(uint256 j = 0; j < newMiners.length; ++j) {
                    if(j < miners.length) {
                        newMiners[j] = miners[j];
                    } else {
                        newMiners[j] = minerId++;
                    }
                }
