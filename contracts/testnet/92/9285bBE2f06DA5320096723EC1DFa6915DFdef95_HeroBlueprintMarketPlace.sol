...
contract HeroBlueprintMarketPlace is OwnableUpgradeable, PausableUpgradeable  {
    struct Auction {
        uint256 highestBid;
        uint256 closingTime;
    ...
    function initByOwner(HeroBlueprintNFT _heroBlueprintNFT, address _tokenBEP20) public onlyOwner{
    ...
    function closeAuctionSudden(uint256 _nftId) external whenNotPaused {
        ...
        heroBlueprintNFT.transfer(_nftId, auctions[_nftId].originalOwner);
...
function openAuction(
        ...
        require(heroBlueprintNFT.ownerOf(_nftId) == msg.sender, "Not NFT owner");
        heroBlueprintNFT.approveMarketPlace(address(this),_nftId);
        // NFT Transfer to contract
        heroBlueprintNFT.transfer(_nftId, address(this));
    ...
    function buy(uint256 _nftId, uint256 _amount) external whenNotPaused {
        ...
        // Transfer NFT to Highest Bidder
        heroBlueprintNFT.transfer(_nftId, auctions[_nftId].highestBidder);
    ...
    function setHeroBlueprintNFT(HeroBlueprintNFT _heroBlueprintNFT) public onlyOwner{
    heroBlueprintNFT=_heroBlueprintNFT;
    }
...
function getHeroBlueprintPublic(address _owner, uint256 _id) public view returns (
  ...
  function getHeroBlueprintOfSenderWithRange(address sender,uint256 from, uint256 to) external view returns (HeroBlueprint[] memory ) {
