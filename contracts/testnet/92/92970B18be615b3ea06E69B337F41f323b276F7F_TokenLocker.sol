...
contract TokenLocker is Pausable, Ownable {
    ...
    function lock(address token, uint256 amount, uint256 unlockTime) payable public {
    ...
    function getPayableLockedAmount(    
        address sender, 
        address token, 
        bool filterUnlocked) internal returns (uint256) {
        holder storage holder0 = holders[sender];
        Token[] memory locks = holder0.tokens[token];
        uint256 amount = 0;
        // Withdraw all amounts that are unlocked
        // Then delete them from the holders variable
        for (uint256 i = 0; i < locks.length; i++) {
            Token memory lock = locks[i];
            if (!filterUnlocked || lock.unlockTime < block.timestamp) {
                amount += lock.balance;
                if (i != locks.length - 1) {
                    locks[i] = locks[locks.length - 1];
                }
                holder0.tokens[token].pop();
            }
        }
        return amount;
    }
