...
contract ZaiMeta is Ownable {
    ...
    event InterfacesUpdated(address ipfsStorage, address oracle, address delegate);
    event GameV2Setted(address gameV2);
    event GodNamesSetted(uint256 season, string[7] godsName);
    event ZaiStatusUpdated(uint256 indexed zaiId, uint256 status, uint256 onCenter, uint256 spotId);
    event ManaUpdated(uint256 indexed zaiId, uint256 manaUp, uint256 manaDown, uint256 manaMax);
        ...
        returns (ZaiStruct.ZaiMinDatasForFight memory zaiMinDatas)
    {
        ...
        zaiMinDatas = 
            ZaiStruct.ZaiMinDatasForFight(
                z.level,z.metadata.state,
                z.activity.statusId,
                z.powers.water,
                z.powers.fire,
                z.powers.metal,
        ...
        returns (uint256 level)
    {
        ...
        if(z.level < 50){ // UPDATE AUDIT: Add this condition because no need to check level > 50 (all Zai in level >= 50 can fight together cauz' got same number of points)
        ...
        if (level > z.level) {
            if (z.level < 50) { // UPDATE AUDIT: This condition has been moove up because math overflow when Zai level already in >= 50 , raising another level make crash tx
                // update new level
                ...
                z.creditForUpgrade = uint8(z.creditForUpgrade + (_numberOfLevelUp * 3));
    ...
    function _getRandomPowers(
        ...
        if (level >= FOUR_POWERS_MIN_LEVEL && level < FIVE_POWERS_MIN_LEVEL) {
            ...
            _random /= 10;
        } else if (level >= THREE_POWERS_MIN_LEVEL && level < FOUR_POWERS_MIN_LEVEL) {
            numberOfElements = 3;
            elements[_random % 5] = elements[4];
            _random /= 10;
