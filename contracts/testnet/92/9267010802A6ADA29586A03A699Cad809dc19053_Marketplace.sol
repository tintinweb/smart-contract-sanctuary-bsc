...
contract Marketplace is ReentrancyGuard, Ownable, ERC20Recoverable {
  ...
  function buy(Detail calldata detail, bytes calldata signature) external nonReentrant {
    ...
    IERC20(price.tokenAddress).transferFrom(
      msg.sender,
      detail.seller,
      price.amount - feeAmount
    );
    ...
    for (uint256 i = 0; i < detail.bundle.length; i++) {
      ...
      IERC721(detail.bundle[i].tokenAddress).safeTransferFrom(
        detail.seller,
        msg.sender,
        detail.bundle[i].tokenId
      );
    }
   ...
   * @param nftAddress NFT address to update support.
   * @param value Boolean value of whether to add or remove the NFT address.
   */
  function supportNFT(address nftAddress, bool value) external onlyOwner {
    isNFTSupported[nftAddress] = value;
    emit SupportNFTUpdated(nftAddress, value);
  }
  ...
  event SupportNFTUpdated(address indexed nftAddress, bool value);
