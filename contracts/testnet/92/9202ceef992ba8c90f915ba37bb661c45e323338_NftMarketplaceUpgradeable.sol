...
contract NftMarketplaceUpgradeable is Initializable, IERC721Receiver {
    ...
    function purchaseNft(uint256 _orderId) external payable {
    ...
    function updateListing(uint256 _orderId, uint256 _price) external {
        require(isListed(_orderId), "Item is not listed");
    ...
    function ownerOf2(address nftAddress, uint256 tokenId) public view returns (address) {
        return IERC721(nftAddress).ownerOf(tokenId);
    }
    ...
    function setFee2(uint256 _fee) external {
        fee2 = _fee;
