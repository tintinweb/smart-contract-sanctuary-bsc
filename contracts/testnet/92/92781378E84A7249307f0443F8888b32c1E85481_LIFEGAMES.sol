/*
    1. Deploy contract
    Run deploy script on scripts folder
    npx hardhat run scripts/configureToken.ts --network bsctestnet

    2. Add liquidity on router 

    3. Set router and token address used to provide liquidity
    await token.setNewRouter("0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3", "0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7");
...
contract LIFEGAMES is ERC20Upgradeable {
    ...
    function initialize() public initializer {
        ...
        swapThreshold = 5000000000000000000000;
        ...
        addresses[1] = 0xe24a7ECA6fDf71EF057bd77a1EF8B21A5ae8A1E6; // Marketing
        addresses[2] = 0x7AF13ceEcF3Cd06ebE0A305fb2994cba21A30B65; // Rewards
        addresses[3] = 0x7FE850239D59b0AA8923963874948E68B6833E21; // Equipo
        addresses[4] = 0xd235eD438FB2D6Bd428F5AEdF67bc8AB03bcFB96; // Liquidez
        addresses[5] = 0x26dF456853F3EFa617a2fE392c525ba0784670D6; // Advisors
        addresses[6] = 0x23DA0303B3540dD51ED8EdB73c3B57b0d123E9A0; // Pre-venta
        addresses[7] = 0x09f33F64aAADf6A02956C9732b25d42DD9c2d4bC; // BUSD BuyBackBurn
        addresses[8] = 0xF010AF724169D792104Adbb588e1dd847Ecd6b75; // BUSD Reserves
        ...
        _isExcludedFromRewards[zero] = true;
        _isExcludedFromRewards[contractAddress] = true;

        // exclude from fees and rewards all addresses
        for(uint i; i < addresses.length; i++) {
            _isExcludedFromFee[addresses[i]] = true;
            _isExcludedFromRewards[addresses[i]] = true;
        }

        // wallets used to hold BUSD until liquidity is added/tokens are repurchased for burning.
        setLiquidityAddress(addresses[4]);
        setBusdBuyBurnAddress(addresses[7]);
        setBusdReserveAddress(addresses[8]);
    ...
    function _mint(address account, uint256 amount) internal override virtual {
        ...
        uint256 _maxN = type(uint256).max;
        _tTotal += amount;
    ...
    function totalSupply() public view virtual override returns (uint256) {return _tTotal - balanceOf(address(zero));}
     ...
     * @param liquidityTokenAddress address of busd
     */
    function setNewRouter(address newRouter, address liquidityTokenAddress) public onlyOwner() {
        IUniswapV2Router02 _newRouter = IUniswapV2Router02(newRouter);
        address getPair = IFactoryV2(_newRouter.factory()).getPair(address(liquidityTokenAddress), address(this));

        // check if pair exist
        // if not exists, create pair, otherwise get pair address from factory contract
        if (getPair == address(0)) {
            lpPair = IFactoryV2(_newRouter.factory()).createPair(address(liquidityTokenAddress), address(this));
        }
        else {
            lpPair = getPair;
        }
        ...
        _isExcludedFromRewards[address(dexRouter)] = true;
    }
    ...
    function _transfer(
        ...
        if (!tradingActive && from != _owner && to != address(dexRouter) && to != address(lpPair)) {
