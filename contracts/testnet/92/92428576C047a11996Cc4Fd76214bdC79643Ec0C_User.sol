...
contract User is Pausable, Ownable {
    ...
    function register(address _referrer) public {
        if (referrers[msg.sender] == address(0) && address(airdropContract) != address(0)) {
            referrers[msg.sender] = IBigexAirdrop(airdropContract).getRef(msg.sender);
        }

        if (referrers[msg.sender] == address(0) && msg.sender != _referrer) {
            referrers[msg.sender] = address(_referrer);
    ...
    function updateRefUser(address account, address newRefAccount) public onlyOwner {
        ...
        for (uint256 i = 0; i < listRef[referrers[account]].length; i++) {
            if (listRef[referrers[account]][i] == account) {
                listRef[referrers[account]][i] = listRef[referrers[account]][listRef[referrers[account]].length - 1];
        ...
        // update
        userRefCount[referrers[account]] = userRefCount[referrers[account]].sub(1);
    ...
    function getRef(address account) public view returns (address){
        ...
        if (address(airdropContract) != address(0)) {
            result = IBigexAirdrop(airdropContract).getRef(account);
        }
        if (result == address(0)) {
            result = referrers[account];
        }
        return result;
    }

    function getRefContract(address account) public view returns (address){
    ...
    function setAirdropContract(address _address) public onlyOwner {
        airdropContract = _address;
    }
