...
contract UniswapV2ERC20 {
    ...
    string public constant name = 'BABYPINK-LP';
    string public constant symbol = 'BABYPINK';
...
contract SwapPair is UniswapV2ERC20 {
    ...
    // if fee is on, mint liquidity equivalent to 8/25th of the growth in sqrt(k)
    function _mintFeeTest(uint112 _reserve0, uint112 _reserve1) public view returns (uint,uint,uint,uint,uint) {
       // address feeTo = IUniswapV2Factory(factory).feeTo();
        //bool feeOn = true;
        uint _kLast = kLast; // gas savings
        uint rootK;
        ...
        if (1==1) {
            if (_kLast != 0) {
                 rootK = Math.sqrt(uint(_reserve0).mul(_reserve1));
                 rootKLast = Math.sqrt(_kLast);
                if (rootK > rootKLast) {
                     numerator = totalSupply.mul(rootK.sub(rootKLast)).mul(8);
                     denominator = rootK.mul(17).add(rootKLast.mul(8));
        ...
        return (rootK,rootKLast,numerator,denominator,liquidity);
    }
