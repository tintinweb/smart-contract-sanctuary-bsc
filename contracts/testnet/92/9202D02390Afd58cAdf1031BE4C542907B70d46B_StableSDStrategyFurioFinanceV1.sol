...
import "./Strategy/StableStandardStrategyV1.sol";

/// @title The Stable Standard Strategy FurioFinance contract
contract StableSDStrategyFurioFinanceV1 is
    Initializable,
    StableBaseConfig,
    StableStandardStrategyV1,
    ReentrancyGuardUpgradeable
{
    ...
    function withdrawAll(
        ...
        _stakeRewards();
        uint256 currentDeposits = getStandardStrategyBalance(msg.sender);
    ...
    function stakeStandardRewards(uint256 bnbReward) internal {
        ...
        ) = DEX.convertEthToPairLP{ value: tokenPairLpShare } (address(StableSwap));
    ...
    )
    {
        ...
        balance = getStandardStrategyBalance(msg.sender);
        totalReinvested =
            participantData.totalReinvested +
            balance -
            deposited;

        earnedFurFi = getStandardStrategyFurFiRewards(msg.sender);
        earnedBnb = 0;
        stakedFurFi = 0;
    }
...
abstract contract StableStandardStrategyV1 is Initializable, StableBaseConfig {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    function standardStrategyDeposit(uint256 amount) internal {
        ...
        uint256 currentDeposit = getStandardStrategyBalance(msg.sender);
    ...
    function getStandardStrategyBalance(address participant) public view returns (uint256) {
        if (participantData[participant].lpMask == 0) return 0;

        return
            participantData[participant].amount +
            ((lpRoundMask - participantData[participant].lpMask) *
                participantData[participant].amount) /
            DECIMAL_OFFSET;
    }
    ...
    function getStandardStrategyFurFiRewards(address participant) public view returns (uint256) {
        if (participantData[participant].rewardMask == 0) return 0;

        return
            participantData[participant].pendingRewards +
            ((furFiRoundMask - participantData[participant].rewardMask) *
                participantData[participant].amount) /
            DECIMAL_OFFSET;
    }
    ...
    function updateStandardRewardMask() private {
        uint256 currentRewardBalance = getStandardStrategyFurFiRewards(msg.sender);
