...
contract NFTAuction is ProofsVerifier{
    ...
    function expectedRound() internal returns(uint256){
    ...
    function bidPlot(int x, int y, uint256 amount) public{
        ...
        require(block.timestamp - lastBidTime[plotSlotHash] > BID_TIMEOUT, "bid-too-late");
    ...
    function timeLeft(int x, int y) public view returns(int256){
        ...
        return int256(BID_TIMEOUT - (block.timestamp - lastBidTime[getHash(x, y, round)]));
    }
    ...
    function getPlotDetails(int x, int y) public view  returns(uint256 price, address owner, bool isFinal){
        ...
        return (highestPrice[getHash(x,y,round)], tokenOwner[getHash(x,y,round)], round<currentRound || (block.timestamp - lastBidTime[getHash(x, y, round)] > BID_TIMEOUT));
    }
