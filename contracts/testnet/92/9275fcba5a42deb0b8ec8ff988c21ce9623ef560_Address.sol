...
contract Floki22 is Context, IBEP20, Ownable {
    ...
    address payable public MarketingWallet = payable(0x5E678bC229c5FBAe0ef5d3f1b1E0ab12Beefd768);
    ...
    function setBuyFees(uint256 liquidityFee,uint256 MarketingFee) external onlyOwner() {
        require(liquidityFee.add(MarketingFee) <= 20);
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount >= _tTotal.mul(1).div(100), "Cannot set transaction amount as zero");
    ...
    function _isAccoutblackList(address Account)external view returns(bool){
        return _isblackList[Account];
    }
    
    function setAccoutblackListStatus(address Account, bool status)external onlyOwner(){
    ...
    function setAutomaticMarketMaker(address _autoMM,bool _status) external onlyOwner(){
        _isAutoMaticMarketMaker[_autoMM] = _status;
    }
