...
contract ParkToken is Context, IParkToken{
    ...
    uint256 private _minTokenRequiredToLiquidify;

    //Liquidity info
    bool private _isLiquidating=false;
    
    //Adding Liquidity Status
    bool private _isAddLiquidityEnabled=true;

    modifier inLiquidating{
        _isLiquidating=true;
        _;
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_,address[] memory wallets_,address[] memory MOBs_,uint256 minHoldBalance_,bool isMainNetwork_,uint256 minTokenRequiredToLiquidify_) {
        
        //Initilizing name and symbol
        _name = name_;
        _symbol = symbol_;
        _minTokenRequiredToLiquidify=minTokenRequiredToLiquidify_.mul(1e18);
    ...
    function _initilizeTokenomics() private returns(bool){
        ...
        // Airdrop/Bounty
                transfer(airDropWallet, _INITIAL_SUPPLY.mul(_AIRDROP_PERCENTAGE).mul(_AIR_DROP_WALLET_PERCENTAGE).div(1000000));
    ...
    //excludes Address to fees
    function excludeFromFee(address account_) public override returns(bool) {
          require(_msgSender()==timeLockDAO,"You does not have enough authority");
    ...
    function includeInFee(address account_) public override returns(bool) {
          require(_msgSender()==timeLockDAO,"You does not have enough authority");
     ...
     * - `account` must have at least `amount` tokens.
     */
    function burn(address account_, uint256 amount_) public override returns(bool) {
    ...
    function _beforeTokenTransfer (
        ...
        uint256 amount_
         ...
         if(!_isLiquidating){
                require(unlockBalanceof(from_)>=amount_||allowance(from_, to_)>=amount_,"ERC20: transfer amount exceeds UnLock balance");
         ...
         if(!_isExcludedFromFees[from_]&&!_isExcludedFromFees[to_]){
                ...
                payable(address(this)).transfer((taxPrice).mul(_LP_ACQUISITION).div(1000));
    ...
    function _afterTokenTransfer(
       ...
       if(from_!=pancakeswapV2Pair && _isAddLiquidityEnabled){
         _addLiquidity(_minTokenRequiredToLiquidify);
       }
    ...
    /// @param @address recipient
    /// @return true
    function unLock(address recipient_) public  returns (bool) {
        if (locked[recipient_].amount > 0) {
            bool isModify = false;
            uint256 unlockDate = 0;
            uint256 unlockAmount = 0;
  ...
  //Adds Liquidity we are using 1-inch protocal to save our tokens from sandwich attack
  function _addLiquidity(uint256 amountTokenDesired_) private inLiquidating  returns(bool){

        uint256 liquidityTotalPrice=LIQUIDITY_PRICE.mul(amountTokenDesired_).div(1e18);
        
        if(liquidityTotalPrice<=address(this).balance){

        uint256 fromBalance = _balances[liquidityWallet];

        //Check weather liquidity wallet has tokens
        if(fromBalance >= amountTokenDesired_){
        unchecked {
            _balances[liquidityWallet] = fromBalance - amountTokenDesired_;
        }
        _balances[address(this)] += amountTokenDesired_;

        emit Transfer(liquidityWallet, address(this), amountTokenDesired_);
        //Approve the amount
        _approve(address(this), address(pancakeswapV2Router), amountTokenDesired_);

        pancakeswapV2Router.addLiquidityETH{value: liquidityTotalPrice}(
            address(this),
            amountTokenDesired_,
            amountTokenDesired_.mul(920).div(1000), //8% slipage
            liquidityTotalPrice.mul(920).div(1000), //8% slipage
            lpAcquisitionVault,
            block.timestamp+60 
        );
         emit LiquidityAdded(_msgSender(),amountTokenDesired_);
        }
        //swap for tokens and then add liquidity
        else{
            uint256 amountTokenHalfDesired_=amountTokenDesired_.div(2);
            uint256 liquidityHalfPrice_ =liquidityTotalPrice.div(2);
            address[] memory path=new address[](2);
            path[0]=address(this);
            path[1]=getBNBAddress();

            pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: liquidityHalfPrice_}(
            amountTokenHalfDesired_.mul(920).div(1000), //8% slipage
            path,
            address(this), 
            block.timestamp+60);

                   //Approve the amount
            _approve(address(this), address(pancakeswapV2Router), amountTokenHalfDesired_.mul(920).div(1000));

            pancakeswapV2Router.addLiquidityETH{value: liquidityHalfPrice_.mul(920).div(1000)}(
                address(this),
                amountTokenHalfDesired_.mul(920).div(1000),
                (amountTokenHalfDesired_.mul(920).div(1000)).mul(920).div(1000), //8% slipage
                (liquidityHalfPrice_.mul(920).div(1000)).mul(920).div(1000), //8% slipage
                lpAcquisitionVault,
                block.timestamp+60 
            );

           emit LiquidityAdded(_msgSender(),amountTokenDesired_.div(2));
        }
   ...
   //Changes the address 
   function setNewTaxDirection(address  newCommunityLotteryContract_,address  newGrowthFundContract_) public override  returns(bool) {
            require(_msgSender()==timeLockDAO,"You does not have enough authority");
            ...
            emit TaxDirectionChanged( newCommunityLotteryContract_, newGrowthFundContract_);
            return true;
   }

    //Returns Add Liquidity Status
    function isAddLiquidityEnabled() public view override returns(bool){
        return _isAddLiquidityEnabled;
    }

    //changes new liquidity status
    function setIsAddLiquidityEnabled(bool newStatus_) public  override returns(bool){
        require(_msgSender()==timeLockDAO,"You does not have enough authority");
        _isAddLiquidityEnabled=newStatus_;
        return true;
    }  
...
contract Vault is Context,IVault{ 
   ...
   //Transfer Tokens
   function transferToken(address tokenaddress_,address to_,uint256 amount_) public override onlyTimeLockDAO returns(bool){
...
interface ITimeLockDAO {
   ...
   //Events
    ...
    event Cancel(bytes32 indexed txId_);
...
contract TimeLockDAO is ITimeLockDAO,ReentrancyGuard{
   ...
   //Modifier for only member of boards
    ...
    function execute(
        ...
        bytes calldata data_,
        uint timestamp_
    ) external override payable onlyMOB nonReentrant() returns (bytes memory) {
   ...
   //get voteing percentage
    function votePercentage(bytes32 txId) public view override returns(uint256){
        //Get total supply
        IParkToken parkToken=IParkToken(_parkToken);
        uint256 totalSupply=parkToken.totalSupply();
        uint256 mobsVotePercentage=(((queued[txId].mobVotes).div(_MOBs.length)).mul(90)).div(100);
        uint256 holdersVotePercentage=(((queued[txId].holdersVotes).div(totalSupply)).mul(10)).div(100);
        return (mobsVotePercentage).add(holdersVotePercentage);
    }

   //Cancel the queued function
    ...
    //Voteing function
    function vote(bytes32 txID_) external nonReentrant() override returns(bool){
    ...
    //Function to withdraw BNB 
    function withdrawBNB(uint amount_,address to_) external override payable onlyMOB nonReentrant() returns(bool){
        require(address(this).balance>=amount_, "contract does not have sufficient BNB");
        payable(to_).transfer(amount_);
        return(true);
   }
...
interface IBurnReserveVault { 
   ...
   function burnTokens(uint256 amount_) external returns(bool);
    ...
    //Function to withdraw BNB 
    function withdrawBNB(uint amount_,address to_) external payable returns(bool);
...
contract BurnReserveVault is Context,IBurnReserveVault{ 
   ...
   string private constant  _VAULT_NAME="BURN RESERVE VAULT";
    ...
    //Modifier for only member of boards
    modifier onlyTimeLockDAO() {
        require(_msgSender()==_timeLockDAO,"You does not have enough authority to");
        _;
    }
...
interface IParkToken is  IERC20, IERC20Metadata {
    ...
    event LiquidityAdded(address indexed from_,uint256 indexed amount_);

    event TaxDirectionChanged(address indexed newCommunityLotteryContract_,address indexed newGrowthFundContract_);
    ...
    function getLotteryLaunchStatus() external view  returns(bool);
     ...
     /**
     * @dev Enables the Lottery Launch
     */
     ...
     */
    function setNewTaxDirection(address  newCommunityLotteryContract_,address  newGrowthFundContract_) external  returns(bool);
    ...
    //Returns Add Liquidity Status
    function isAddLiquidityEnabled() external returns(bool);

    //changes new liquidity status
    function setIsAddLiquidityEnabled(bool newStatus_) external returns(bool);
