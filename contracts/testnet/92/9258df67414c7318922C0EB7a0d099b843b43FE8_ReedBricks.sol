...
contract ReedBricks is Context, IERC20, Ownable {
    ...
    uint256 public localSalesRate = 50000; // Number of tokens per BNB
    // uint256 public tokenSold = 0; // keeping track of REED sold
    // uint256 public raisedAmount = 0; // keeping track of BNB raised
    ...
    function localBuyToken() external payable whenLocalSaleIsActive {
    ...
    function settlePurchase(address _address) external onlyOwner {
        require(_address != address(0));
        _approve(address(this), _owner, purchases[msg.sender]);
        _transfer(address(this), _address, purchases[msg.sender]);
    }

    function updateRate(uint256 _rate) external returns (bool) {
        require(localSalesRate != _rate, "REED: Rate value must change");
        localSalesRate = _rate;
        return true;
    }
