...
contract RollerLevel is IRollerLevel, Ownable, ReentrancyGuard {
    ...
    // Level quantity info of each user.
    struct UserInfo {
        uint256 quantity;
        uint256 lpQuantity;
    ...
    event UpdateLevel(address indexed user, uint256 quantity, uint256 lpQuantity, uint256 bankerQuantity, uint256 playerQuantity, uint256 referrerQuantity, uint256 lotteryQuantity);
    ...
    constructor(
        ...
        rollerToken = IBEP20(_rollerTokenAddr);
    ...
    function getLevel(address account) public override view returns (uint256) {
    ...
    function pendingLevel(address account) public override view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tmpQuantity, uint256 tmpLpQuantity, uint256 tmpBankerQuantity) = masterChef.getRollerLevel(account);

        uint256 tmpPlayerQuantity = 0;
        uint256 tmpReferrerQuantity = 0;
        uint256 tmpLotteryQuantity = 0;
        if(address(betMining) != address(0)){
            tmpPlayerQuantity = betMining.getRollerLevel(account);
            tmpQuantity = tmpQuantity.add(tmpPlayerQuantity);
        }
        
        if(address(referral) != address(0)){
            tmpReferrerQuantity = referral.getRollerLevel(account);
            tmpQuantity = tmpQuantity.add(tmpReferrerQuantity);
        }

        if(address(lottery) != address(0)){
            tmpLotteryQuantity = lottery.getRollerLevel(account);
    ...
    function updateLevel(address account) public override{
        require(account != address(0), "RollerLevel: account is zero address");

        for(uint256 i = 0; i < bonusInfo.length; i ++){
            BonusInfo storage bonus = bonusInfo[i];
            if(bonus.token != address(rollerToken)){
                oracle.update(bonus.token, address(rollerToken));
            }
        }

        addPendingRewards(account);

        (uint256 tmpQuantity, uint256 tmpLpQuantity, uint256 tmpBankerQuantity, uint256 tmpPlayerQuantity, uint256 tmpReferrerQuantity, uint256 tmpLotteryQuantity) = pendingLevel(account);
        ...
        emit UpdateLevel(account, tmpQuantity, tmpLpQuantity, tmpBankerQuantity, tmpPlayerQuantity, tmpReferrerQuantity, tmpLotteryQuantity);
    }
