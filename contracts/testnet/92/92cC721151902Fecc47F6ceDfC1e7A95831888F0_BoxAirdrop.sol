...
import "./interfaces/IBoxNft.sol";

contract BoxAirdrop is Context, AccessControlEnumerable {
    ...
    constructor(address boxNftAddress, uint256 total, uint256 _timestamp) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());

        remain = total;
        timestamp = _timestamp;

        boxNft = IBoxNft(boxNftAddress);
    }
    ...
    function claim() external {
        require(block.timestamp > timestamp, 'event not started');
        require(remain > 0, 'event finished');
        require(whiteList[msg.sender], "Sender not in whitelist");

        remain--;
        whiteList[msg.sender] = false;

        boxNft.buyBox(msg.sender, 1, 0);
    }
}
...
interface IBoxNft {
    event itemSummoned(uint256 tokenId, uint256 itemTokenId);

    function buyBox(address toAddress, uint256 quantity, uint256 price) external;
