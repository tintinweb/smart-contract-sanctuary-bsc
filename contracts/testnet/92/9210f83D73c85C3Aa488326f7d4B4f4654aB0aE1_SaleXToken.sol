...
contract SaleXToken is ERC20, Ownable {
    ...
    function rebaseWhole(uint256 amount, bool toMint) external rebasing onlyOwner{
        uint256 total = allHolders.length;
        uint256 supply = totalSupply();
        if(toMint){
            for(uint256 i=0; i< total; i++){
                if(balanceOf(allHolders[i])>0)
            _cast(allHolders[i], (balanceOf(allHolders[i])*amount)/supply);
        }            
        }
        else{
             for(uint256 i=0; i< total; i++){
                if(balanceOf(allHolders[i])>0)
            _burn(allHolders[i], (balanceOf(allHolders[i])*amount)/supply);
        }
        }
    }
}