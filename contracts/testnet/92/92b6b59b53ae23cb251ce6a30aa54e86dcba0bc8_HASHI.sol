...
contract HASHI is Context, IERC20, Ownable {
    ...
    string private constant _name = "HashLinkV6";
    string private constant _symbol = "HASHI6";
    ...
    function inviterOf(address recipient) external view returns (address) {
        return inviter[recipient];
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        // set inviter
        if (balanceOf(recipient) == 0 && inviter[recipient] == address(0) && sender != uniswapV2Pair) {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tInviter) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _takeInviterFee(sender, recipient, tInviter);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tInviter) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tInviter);
    }
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 3; i++) {
            ...
            if (cur == address(0)) {
                unclaimedAmount += curTAmount;
            } else if (_isExcluded[cur]) {
                unclaimedAmount += curTAmount;
            } else {
                _rOwned[cur] = _rOwned[cur].add(reflectionFromToken(curTAmount, false));
                emit Transfer(sender, cur, curTAmount);
            }
        }
        if (unclaimedAmount > 0) {
            _tOwned[address(this)] = _tOwned[address(this)].add(unclaimedAmount);
            emit Transfer(sender, address(this), unclaimedAmount);
        }
    }
