...
contract FamPredition is Ownable, VRFConsumerBaseV2, KeeperCompatibleInterface {
        ...
        )
    {
        ...
        uint256 curRound = meta.currentRound; // 当前轮id

        if (curRound >= 1) {
            lastResult = rounds[curRound - 1].luckNumber; // 上一轮结果
        }

        infos[0] = curRound;

        infos[1] = rounds[curRound].startTime; // 当前轮开始时间
        infos[2] = rounds[curRound].closeTime; // 当前轮结束时间

        infos[3] = meta.betLimit; // 单轮投注限制
        infos[4] = meta.betRateLimit; // 单注倍率限制
        infos[5] = meta.epochInterval; // 轮次间隔

        if (account_ != address(0)) {
            infos[6] = getUserRoundsLength(account_); // 用户总共参与的轮次数
        }
    }
