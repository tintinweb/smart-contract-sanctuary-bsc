...
contract GENIPassSaleV2 is IGENIPassSaleV2, IERC721ReceiverUpgradeable, PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    uint public stonePoint;
    uint public topazPoint;
    uint public citrinePoint;
    uint public rubyPoint;
    uint public diamondPoint;
    ...
    function putOnSale(uint passId, uint price, uint startTime, uint expirationTime) public override whenNotPaused onlyOperator {
    ...
    function removeFromSale(uint passId) external override onlyOperator whenNotPaused nonReentrant {
        SaleInfo memory saleInfo = saleInfos[passId];
        require(saleInfo.startTime > 0, "GENIPassSale: not for sale");
    ...
    function getCurrentPricePass(uint passId) public override view returns(uint pricePass) {
        ...
        if(price != 0) {
            ...
            pricePass = getDefauftPrice(passId);
        }
    ...
    function putOnSaleBatch(CreateSalePassesWithoutMint[] memory input) external override whenNotPaused onlyOperator {
        for (uint i = 0; i < input.length; i++) {
            putOnSale(input[i].passId, input[i].price, input[i].startTime, input[i].expirationTime);
        }
    }
    ...
    function purchase(uint passId, uint buyPrice) external override whenNotPaused nonReentrant {
        SaleInfo storage saleInfo = saleInfos[passId];
        ...
        uint salePrice = getCurrentPricePass(passId);
    ...
    function purchaseWithReferral(uint passId, uint buyPrice, address referralAddress, uint nonce, bytes memory signature) external override whenNotPaused nonReentrant {
        SaleInfo storage saleInfo = saleInfos[passId];
        ...
        salePrice = payPrice * (1000000 -  getDiscountReferral(passAmount + getReferalPoint(passId))) / decimalRate / 100;
        ...
        passCount[referralAddress][monthIndex] += getReferalPoint(passId);
