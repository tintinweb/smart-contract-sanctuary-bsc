    ...
    RewardRateConfigurable
{
    ...
    /**
     * @notice Function to see pending reward on frontend.
     * @param _pid: Pool id where user has assets
     */
    function alpacaPendingReward(uint256 _pid) external returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][msg.sender];
        return IStrategy(pool.strategy).getPendingRewards(user.dollarValue * 1e18, msg.sender);
    }

    /**
     * @notice View function to see alpaca pending reward on frontend.
     * @param _pid: Pool id where user has assets
    ...
    function claimAlpacaRewards(uint256 _pid) external {
        ...
        if (_pid == 0) {
            ...
            IStrategy(pool.strategy).claimAlpacaRewards(msg.sender, user.dollarValue * 1e18);
        }
    }
