...
contract ReedBricks is Context, IBEP20, Ownable, ReedLock {
    ...
    string private _name = "JAXI";
    string private _symbol = "JAXI";
    ...
    uint256 private buyBackUpperLimit = 10 * 10**3 * 10**18;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (onFreezer(account)) return getFreeFund(account);
    ...
    function getFreeFund(address _account) private view returns (uint256) {
        ...
        //Check if user if frozen
        if (onFreezer(_account) == false) {
            return balance;
        }
        ...
        // Check freez is ended
        if (frozen.endsInDays >= _now) {
            return balance;
        }

        // Get time difference between start and end//
        uint256 _spreadTime = (frozen.endsInDays).sub(frozen.startsInDays);
        // Get spread amount between start and end//
        uint256 _spreadAmount_per_seconds = balance.div(_spreadTime);

        // Get the time that has passed (start - end = time_diff)//
        uint256 _timePassed = _now.sub(frozen.startsInDays);

        // Get the amount since time passed
        uint256 _totalBurst = _timePassed.mul(_spreadAmount_per_seconds);
...
contract ReedLock is Ownable {
    ...
    function onFreezer(address _account) public view returns (bool) {
        Frozen memory frozen = _frozen[_account];
        if (frozen.id != 0) {
            return true;
        }
