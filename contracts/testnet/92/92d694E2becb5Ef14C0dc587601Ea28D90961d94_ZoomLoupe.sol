...
contract ZoomLoupe is Ownable, Pausable,IZoomLoupe{
    ...
    uint256 _MAXColl_NFTWA=27;

    event TransferSent(address _from, address _destAddr, uint _amount);

constructor() {
        Contr=IContracts_TITIMITI(0xB9b84a5bd43FAE99e55bF3374ddB9E206150b538);
        CNF=IColl_NFTWA_NFTA(0x0247F524c75FAc2DC939982fAC8e46f2EE61aCEF);
       ...
       address sender = msg.sender;
        ...
        USDT.transfer(Contr.getLandLord(), _USDT*Decimals/100*procent_landlord);
         ...
         TrueChest.Take_chest(num_Track, chest_num,sender);
            uint256 ID_NFTWA = CNF.Get_NFTWA_num(NumColl_NFTWA);
            address addressNFTWA = Coll.GetID_Collection(ID_NFTWA);
         if(addressNFTWA!=address(0)){
    ...
    function UP_col (address adr) internal {
        NFTWA_get=INFTWA(address(adr));
    }

    function Take_detail() public {
        uint256 Summ = TrueChest.Get_detail(msg.sender);
        require(Summ>=1);
        Details.mint_Detail_1(msg.sender, Summ);
    ...
    function  update()  public virtual override{
        ...
        Details = IDetails(address(Contr.getDetails()));
...
interface IDetails { 
    ...
    function update_URL() external;
    function  update() external;
    function mint_Detail_1(address adr,uint256 amount) external;
}
