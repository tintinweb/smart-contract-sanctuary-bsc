...
interface IVault {
    function transferToMarketplace(
        address market,
        address seller,
        uint256 _tokenId
    ) external;
...
contract YLNFTMarketplace is ReentrancyGuard, Ownable {
    ...
    event AdminListedNFT(
        address user,
        uint256 tokenId,
        uint256 price,
        uint256 timestamp
    );
    event UserlistedNFTtoMarket(
        address user,
        uint256 tokenId,
        uint256 price,
        address market,
    ...
    event AdminPauselistedNFT(
        address user,
        uint256 tokenId,
        address marketplace,
        uint256 timestamp
    );
    event AdminUnpauselistedNFT(
        address user,
        uint256 tokenId,
        address marketplace,
        uint256 timestamp
    ...
    event AdminWithdrawFromEscrow(
        address admin,
        uint256 amount,
        uint256 timestamp
    );
    event EscrowTransferFundsToSeller(
        address market,
        uint256 price,
        address user
    ); //???
    event WithdrawNFTfromMarkettoWallet(
        uint256 tokenId,
        address user,
        uint256 commission,
        uint256 timestamp
    );
    event TransferedNFTfromMarkettoVault(
        uint256 tokenId,
        address vault,
        uint256 timestamp
    );
    event TransferedNFTfromVaulttoMarket(
        uint256 tokenId,
        address vault,
        uint256 timestamp
    );
    event AdminApprovalNFTwithdrawtoWallet(
        address admin,
        uint256 tokenId,
        address user,
        uint256 commission,
    ...
    event DepositNFTFromWallettoMarketApproval(
        uint256 tokenId,
        address user,
        uint256 commission,
        address admin,
    ...
    event DepositNFTFromWallettoTeamsApproval(
        uint256 tokenId,
        address user,
        uint256 commission,
        address admin,
    ...
    event RevertDepositFromWalletToTeams(
        uint256 tokenId,
        address user,
        address admin,
        uint256 timestamp
    );
    event RevertDepositFromWalletToMarket(
        uint256 tokenId,
        address user,
        address admin,
        uint256 timestamp
    );
    event AdminTransferNFT(
        address admin,
        uint256 tokenId,
        uint256 amount,
        address user,
    ...
    event MarketPerCommissionSet(
        address admin,
        uint256 commission,
        uint256 timestamp
    );
    event MarketVCommisionSet(
        address admin,
        uint256 commission,
        uint256 timestamp
    );
    ...
    function allowCredential(address _mOwner, bool _flag)
        public
        ylOwners
        returns (bool)
    {
    ...
    function minterListedNFT(uint256 _tokenId, uint256 _price)
        public
        returns (uint256)
    {
        require(
            ylnft.ownerOf(_tokenId) == msg.sender,
            "User haven't this token ID."
        );
        require(
            proxy.isMintableAccount(msg.sender),
            "You aren't Minter account"
        );
        require(
            ylnft.getApproved(_tokenId) == address(this),
    ...
    function buyerListedNFT(uint256 _tokenId, uint256 _price)
        public
        payable
        returns (uint256)
    {
        ...
        require(
            depositUsers[msg.sender][_tokenId] == true,
            "This token has not been approved by administrator."
        );
        require(
            ylnft.getApproved(_tokenId) == address(this),
            "NFT must be approved to market"
    ...
    function directTransferToBuyer(
        address _from,
        uint256 _tokenId,
        uint256 _price
        ...
        require(
            msg.value >= _price + marketfee,
            "Insufficient fund in marketplace"
        );
        require(
            ylnft.getApproved(_tokenId) == address(this),
            "NFT must be approved to market"
    ...
    //f.
    function bidMinterNFT(
        uint256 _tokenId,
        uint256 _price,
        uint256 _period
    ) public ylOwners returns (uint256) {
        require(
            ylnft.ownerOf(_tokenId) == msg.sender,
    ...
    function userBidOffer(uint256 _auctionId, uint256 _price) public {
        require(
            ylnft.ownerOf(idToAuctionItem[_auctionId].tokenId) == address(this),
    ...
    function withdrawBid(uint256 _auctionId) public payable nonReentrant {
        ...
        if (idToAuctionItem[_auctionId].owner == msg.sender) {
            ...
            require(
                msg.value >= idToAuctionItem[_auctionId].highestBid + marketfee,
                "Insufficient fund"
            );
            ylnft.transferFrom(
                address(this),
            ...
            (bool sent, ) = payable(idToAuctionItem[_auctionId].owner).call{
                value: idToAuctionItem[_auctionId].highestBid
            }("");
    ...
    function adminPauseToggle(uint256 _itemId, bool _flag) public {
        ...
        require(
            ylnft.ownerOf(_tokenId) == address(this),
            "You haven't this tokenID."
        );
        require(
            idToMarketItem[_itemId].seller == msg.sender ||
                marketplaceOwners[msg.sender] == true
    ...
    function withdrawNFT721(uint256 itemId) public payable nonReentrant {
        ...
        require(
            ...
            "You haven't this NFT"
        );
        require(msg.value >= marketfee, "insufficient fund");
        require(
            withdrawUsers[msg.sender][itemId] == true,
            "This token has not been approved by admin"
        );
        ylnft.transferFrom(address(this), msg.sender, _tokenId);
        idToMarketItem[itemId].state = State.Release;
        idToMarketItem[itemId].owner = msg.sender;

        emit WithdrawNFTfromMarkettoWallet(
            _tokenId,
            msg.sender,
            marketfee,
            block.timestamp
    ...
    //j. deposit NFT
    function depositNFT721(uint256 _tokenId, uint256 _price)
        public
        payable
        returns (uint256)
    {
    ...
    function depositApproval(
        ...
        require(
            ylnft.ownerOf(_tokenId) == _user,
            "The User aren't owner of this token."
        );
        depositUsers[_user][_tokenId] = _flag;
        if (_flag == true) {
            emit DepositNFTFromWallettoMarketApproval(
                _tokenId,
                _user,
                marketfee,
                msg.sender,
            ...
            emit RevertDepositFromWalletToMarket(
                _tokenId,
                _user,
                msg.sender,
                block.timestamp
    ...
    function withdrawApproval(
        ...
        require(
            idToMarketItem[_itemId].seller == _user,
            "You don't owner of this NFT."
        );
        require(
            ylnft.ownerOf(idToMarketItem[_itemId].tokenId) == address(this),
            "This token don't exist in market."
        );
        withdrawUsers[_user][_itemId] = _flag;
        if (_flag == true) {
            emit AdminApprovalNFTwithdrawtoWallet(
                msg.sender,
    ...
    function transferToVault(uint256 _itemId, address _vault)
        public
        nonReentrant
        returns (uint256)
    {
        ...
        require(
            ylnft.ownerOf(_tokenId) == address(this),
            "This token didn't list on marketplace"
        );
        require(
            idToMarketItem[_itemId].seller == msg.sender,
            "You don't owner of this token"
        );
        require(
            depositTeamUsers[msg.sender][_itemId] == true,
            "This token has not been approved by admin"
        );
    ...
    function depositTeamApproval(
        ...
        require(
            idToMarketItem[_itemId].seller == _user,
            "The user isn't the owner of token"
        );
        depositTeamUsers[_user][_itemId] = _flag;
        if (_flag == true) {
            emit DepositNFTFromWallettoTeamsApproval(
                idToMarketItem[_itemId].tokenId,
                _user,
                marketfee,
                msg.sender,
            ...
            emit RevertDepositFromWalletToTeams(
                idToMarketItem[_itemId].tokenId,
                _user,
                msg.sender,
                block.timestamp
    ...
    function transferFromVaultToMarketplace(
        uint256 _tokenId,
        address _vault,
        uint256 _price
    ) public {
        require(
            ylnft.ownerOf(_tokenId) == _vault,
            "The team haven't this token."
        );
    ...
    //o.
    function adminTransfer(address _to, uint256 _itemId)
        public
        payable
        ylOwners
    {
        require(
            ylnft.ownerOf(idToMarketItem[_itemId].tokenId) == address(this),
            "This contract haven't this NFT."
        );
        require(
        ...
        (bool sent, ) = payable(idToMarketItem[_itemId].seller).call{
            value: idToMarketItem[_itemId].price
        }("");
        require(sent, "Failed to send Ether");
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 1; i <= total; i++) {
            if (
                idToMarketItem[i].state == State.Active &&
    ...
    // Purchased NFT
    function MarketItemSale(uint256 itemId)
        public
        payable
        nonReentrant
        returns (uint256)
    {
        ...
        require(
            msg.value >= idToMarketItem[itemId].price + marketfee,
            "insufficient fund"
        );
        require(
            idToMarketItem[itemId].seller != msg.sender,
            "This token is your NFT."
        );
        require(
            idToMarketItem[itemId].owner == address(this),
            "This NFT don't exist in market"
        );
        ...
        (bool sent, ) = payable(idToMarketItem[itemId].seller).call{
            value: idToMarketItem[itemId].price
        }("");
        require(sent, "Failed to send Ether to the seller");
        idToMarketItem[itemId].state = State.Release;
        idToMarketItem[itemId].owner = msg.sender;
