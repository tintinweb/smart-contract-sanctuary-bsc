...
contract ForTubeGovernor is Initializable, ForTubeGovernorStorage, ForTubeGovernorEvents {
    ...
    /**
      ...
      * @param minVotingDelay_ The initial voting delay
    ...
    /**
      ...
      * @param votingDelay voting Delay in block unit
      * @param votingPeriod voting Peroid in block unit
      * @return Proposal id of new proposal
      */
    function propose(bytes[] memory executables, string memory description, uint options, uint256 votingDelay, uint256 votingPeriod) public returns (uint) {
        require(executables.length == options, "ForTubeGovernor::propose: executables != options");
        require(votingDelay >= minVotingDelay && votingDelay <= MAX_VOTING_DELAY, "ForTubeGovernor::propose: invalid voting delay");
        ...
        for (uint i = 0; i < options; i++) {
            (address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas) = abi.decode(executables[i], (address[], uint[], string[], bytes[]));
    ...
    function execute(uint proposalId) external payable {
        ...
        (address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas) = abi.decode(proposal.executables[proposal.winning], (address[], uint[], string[], bytes[]));
    ...
    function cancel(uint proposalId) external {
        require(state(proposalId) != ProposalState.Executed, "ForTubeGovernor::cancel: cannot cancel executed proposal");
        ...
        (address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas) = abi.decode(proposal.executables[proposal.winning], (address[], uint[], string[], bytes[]));
        for (uint i = 0; i < targets.length; i++) {
            cancelTransaction(targets[i], values[i], signatures[i], calldatas[i]);
        }
    ...
    /**
      ...
      */
    function getActions(uint proposalId, uint option) external view returns (address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas) {
        Proposal storage p = proposals[proposalId];
        require(option < p.executables.length, "option out of range");
        (targets, values, signatures, calldatas) = abi.decode(p.executables[option], (address[], uint[], string[], bytes[]));
    }
    ...
    function winningOption(uint proposalId) public view returns (uint winning) {
        require(proposalCount >= proposalId, "ForTubeGovernor::state: invalid proposal id");
        Proposal storage proposal = proposals[proposalId];

        uint _max = 0;
        winning = uint(-1); // 0 is valid, so init to -1
        for (uint i = 0; i < proposal.options; ++i) {
            if (proposal.votes[i] > _max) {
                _max = proposal.votes[i];
                winning = i;
            }
        }
    }
    ...
    function state(uint proposalId) public view returns (ProposalState) {
        require(proposalCount >= proposalId, "ForTubeGovernor::state: invalid proposal id");
        Proposal storage proposal = proposals[proposalId];
        uint winning = winningOption(proposalId);
        ...
        } else if (winning == uint(-1) || proposal.votes[winning] < quorumVotes) {
    ...
    function cancelTransaction(address target, uint value, string memory signature, bytes memory data) internal {
        bytes32 txHash = keccak256(abi.encode(target, value, signature, data));
        emit CancelTransaction(txHash, target, value, signature, data);
    }

    function executeTransaction(address target, uint value, string memory signature, bytes memory data) internal {
        bytes32 txHash = keccak256(abi.encode(target, value, signature, data));
