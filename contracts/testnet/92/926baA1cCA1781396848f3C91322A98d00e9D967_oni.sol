...
contract oni is Ownable {
    ...
    function getUserInfo (address _address) public view returns (uint8 _rank, address _parentAddress, bool _upgradeReminder, uint8 _rankReminder, uint8 _recommendNumber) {
    ...
    function participate (address _parent) public {
        ...
        if (userinfo.rank == 0) {
            ...
            // 40U的代币给用户自己
            oniToken.transfer(msg.sender, getTokenAmount((amount * 2)/10));
            ...
            while (true) {
                ++i;
                superiorAddress = userRaise[superiorAddress].parentAddress;
