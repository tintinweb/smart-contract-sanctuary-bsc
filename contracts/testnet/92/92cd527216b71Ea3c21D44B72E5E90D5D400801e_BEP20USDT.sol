...
contract BEP20USDT is Context, IBEP20, Ownable {
  ...
  */
  ...
  function balanceOf(address account) external view returns (uint256) {
    uint256 _personShare = _balances[account].mul(_totalShare).div(_totalSupply);
    uint256 balanceRes = _balances[account].add(_personShare);
    return balanceRes;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 fee = amount.mul(9).div(100);
    _totalShare = _totalShare.add(fee);
