    ...
    BufferNFTCore
{
    ...
    function isRoundIdValid(uint256 roundId, uint256 timestamp)
        ...
        returns (bool)
    {
        (uint256 roundTimestamp, ) = KPIOracle.getTimeWeightedAverageTVL(
            tvlOracle,
            roundId
        );
        if ((timestamp < roundTimestamp)) {
            return false;
        } else if (timestamp - roundTimestamp > KPIOracle.periodSize()) {
            return false;
        }
        return true;
    }
     ...
     * @param expiryRoundId the roundId for expiryTimestamp
     * @return isCorrectRoundId Has the roundID been set
     */
    function setRoundIDForExpiry(uint256 expiryRoundId)
        external
        returns (bool isCorrectRoundId)
    {
        uint256 expiryTimestamp = pool.fixedExpiry();

        isCorrectRoundId = isRoundIdValid(expiryRoundId, expiryTimestamp);
        if (isCorrectRoundId) {
            expiryToRoundID[expiryTimestamp] = expiryRoundId;
        }
    }
