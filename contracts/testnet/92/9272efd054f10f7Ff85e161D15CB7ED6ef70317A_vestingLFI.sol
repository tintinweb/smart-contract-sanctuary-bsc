...
contract vestingLFI is Ownable, ReentrancyGuard {
    ...
    function createVestingSchedule(
        ...
        require(vestingScheduleIdsByAddress[_recipient] == 0, "Vesting is already existing for this recipient");
        require(totalVestingAmount + _amount <= _token.balanceOf(address(this)), "Not enough amount to create a vesting.");
     ...
     */
    function _claimTo(address to)
        internal
    {
        require(vestingScheduleIdsByAddress[msg.sender] != 0, "No existing vesting.");

        uint256 id = vestingScheduleIdsByAddress[msg.sender];

        require(!vestingSchedulesById[id].isEnded, "Vesting is ended.");

        require(block.timestamp > startTime + vestingSchedulesById[id].vestingStart, "Vesting is not started");

        uint256 _amount = computeClaimableAmount(id);

        require(remainingVestingAmount > _amount, "No remained vesting token");

        if (block.timestamp >= startTime + vestingSchedulesById[id].vestingStart + vestingSchedulesById[id].vestingPeriod) {
     ...
     */
    function claimVestedTokensTo(address to)
        external
    {
        _claimTo(to);
    }
        ...
        returns (uint256 _amount)
    {
        if (vestingSchedulesById[id].isEnded) {
            ...
            if (block.timestamp < startTime) {
                ...
                block.timestamp >=
                startTime + vestingSchedulesById[id].vestingStart + vestingSchedulesById[id].vestingPeriod
            ) {
                _amount =
