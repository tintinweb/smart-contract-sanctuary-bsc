...
//        uint createBlock;
    ...
    // For off chain tracking user data
    mapping(address => OfferInfo[])                 public  userOffers;
    mapping(address => uint[])                      public  userListings;
    mapping(address => uint[])                      public  userCollections;
    ...
    */
    ...
    function createCollection(string memory _name, address _treasury, uint _royalties) public {
        ...
        address[] memory nfts;
        collections.push(Collection({owner: msg.sender, name: _name, treasury: _treasury, royalties: _royalties, nfts: nfts, createBlock: block.number}));
...
//            createBlock: block.number,
        ...
        userListings[msg.sender].push(listings.length - 1);
...
//        listing.closedBlock = block.number;
    ...
    */
    ...
    function totalCollections(address _user) public view returns (uint) { return userCollections[_user].length; }
