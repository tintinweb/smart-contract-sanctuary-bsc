...
contract StableFundBUSD  is Ownable, ReentrancyGuard{
    ...
    function claimAllReward() public nonReentrant {
        ...
        // transfer reward to the user
        if(getBalance()<allClaimableReward){
            allClaimableReward = getBalance();
        }
        IERC20(BUSDContract).safeTransfer(msg.sender,allClaimableReward);
        totalReward = totalReward.add(allClaimableReward);
    }
