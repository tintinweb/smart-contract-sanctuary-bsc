...
contract Priidud is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FinalDSDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Priidud", "$PR") {
        ...
        dividendTracker = new FinalDSDividendTracker(
            payable(this),
            router,
...
contract FinalDSDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Priidud public PriidudContract;
    ...
    constructor(
        ...
        defaultToken = token;
        PriidudContract = Priidud(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FinalDS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FinalDS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FinalDS contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PriidudContract);

            uint256 prevBalance = PriidudContract.balanceOf(address(this));
            ...
            {
                uint256 received = PriidudContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PriidudContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
