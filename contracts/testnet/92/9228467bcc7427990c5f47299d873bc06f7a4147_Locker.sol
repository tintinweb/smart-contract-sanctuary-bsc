         ...
         uint256 _timePeriod) external {
          require(!existingToken[msg.sender][_token],"User has the same token");
           lockerInfo.push(
            ...
            LockerInfo storage locker = lockerInfo[lockerId];
            uint256 balanceOfUser = locker.token.balanceOf(msg.sender);
            balanceOfUser  -= _amount;
            uint256 balanceOfContract = locker.token.balanceOf(address(this));
