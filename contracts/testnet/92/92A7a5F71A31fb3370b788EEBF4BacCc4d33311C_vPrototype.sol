...
contract vPrototype {
    ...
    function deposit(uint256 tier_index, address ref) public payable {
        ...
        uint256 watts_amount = convertFromBNB(after_tax).div(1 ether);
    ...
    function convertFromBNB(uint256 _bnb_amount) public view returns(uint256){
        return _bnb_amount.mul(exchangeMultiplier);
    }

    function convertToBNB(uint256 _reward_amount) public view returns(uint256){
        return _reward_amount.div(exchangeMultiplier).mul(1 ether);
    }
