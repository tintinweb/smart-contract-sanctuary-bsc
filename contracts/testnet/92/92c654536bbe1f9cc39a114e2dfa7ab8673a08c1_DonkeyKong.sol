...
library SafeMath {
    ...
    function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
        unchecked {
            // Gas oKOAimization: this is cheaper than requiring 'a' not being zero, but the
...
abstract contract Ownable is Context {
    ...
    event OwnershiKOAransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        _owner = _msgSender();
        emit OwnershiKOAransferred(address(0), _owner);
    }
    ...
    function renounceOwnership() public virtual onlyOwner {
        emit OwnershiKOAransferred(_owner, address(0));
        _owner = address(0);
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        ...
        emit OwnershiKOAransferred(_owner, newOwner);
        _owner = newOwner;
    }
}
...
interface IUniswapV2Router01 {
    ...
    function swaKOAokensForExactTokens(
        uint amountOut,
        uint amountInMax,
        address[] calldata path,
        address to,
    ...
    function swaKOAokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        returns (uint[] memory amounts);
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    uint256 public maxTransactionAmount = 20000000000 * 10**uint256(_decimals);
    ...
    uint256 public numTokensSellToAddToLiquidity = 1500000000 * 10**uint256(_decimals);  // contract balance to trigger swap & liquify + wallet transfer.
    ...
    function _transfer(
        ...
        require(_isExcludedFromFee[from]||_isExcludedFromFee[to]||isTokenLive,"KOA is not live yet");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // caKOAure the contract's current BNB balance.
        // this is so that we can caKOAure exactly the amount of BNB that the
        // swap creates, and not make the liquidity event include any BNB that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for BNB
        swaKOAokensForBNB(liquifyHalves[0]); // <- this breaks the BNB -> KOA swap when swap+liquify is triggered
    ...
    function swaKOAokensForBNB(uint256 tokenAmount) private {
