...
interface IEnh {
      function isWhiteListed(address _sender) external returns (bool);
}
...
contract LpStakingPool is USDTWrapper,Ownable {
   using SafeMath for uint256;
    ...
    IEnh   public enhToken;

    uint256 public totalOutput = 12000e18;
    uint256 public totalDuration = 60 * 60 * 24 * 30 * 5;
    ...
    constructor()  {
        ...
        address inToken_ = 0x30A125afB08b75b64d54836C8c2445546f3cb9E8;

        address outToken_ = 0x2240f40DCB72Fa2cf6850E6d73ad886F010f1A78;

        address usdtToken_ = 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd;

        uint256 starttime_ = 1647619200;

        unitprice = 10 * 10 ** 18;

        stakeInToken = IERC20(inToken_);
        stakeOutToken = IERC20(outToken_);
        enhToken = IEnh(outToken_);
    ...
    modifier updateReward(address account) {
        if (block.timestamp < starttime + 30 days) {
             rewardRate = 462962962962963;
        } else if (block.timestamp < starttime + 60 days) {
             rewardRate = 694444444444444;
        }  else if (block.timestamp < starttime + 90 days) { 
             rewardRate = 925925925925926;
        }  else if (block.timestamp < starttime + 120 days) { 
             rewardRate = 1157407407407407;
        } else  { 
             rewardRate = 1388888888888888;
        }
    ...
    function preSale(uint256 _num) public returns(bool) {
       
        usdtToken.safeTransferFrom(msg.sender, 0xC6e0aADAB4780dAa5787e9064d36f49C7557c401, _num * unitprice);

        stakeOutToken.safeTransfer(msg.sender, _num * 10 ** 18);

        if (referrerAddress[msg.sender] != address(0)) {
             stakeOutToken.safeTransfer(referrerAddress[msg.sender], _num * 10 ** 18 * 10 / 100);
        }
