...
contract HobbitPirateFarmFactory is Ownable, ReentrancyGuard{
    ...
    event updateImplement(
        address indexed oldImplement,
        address indexed newImplement
    );
    ...
    function createFarmPair(
        ...
        address newContract = Clones.cloneDeterministic(
            implementation,
            bytes32(abi.encodePacked(getTotalPair()))
        );
...
interface IHobbitPirateFarm{
    ...
    function addPoolReward(uint256 amountPool) external payable;
    ...
    function removePoolReward(uint256 amountPool) external;
