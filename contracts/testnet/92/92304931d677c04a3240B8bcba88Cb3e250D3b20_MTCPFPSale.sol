...
contract MTCPFPSale is math{
    ...
    function AngelSale() external payable{
		...
		require(AngelSaleAmount.add(_tradeAmount) <= MaxAngelSaleAmount, "MTC Angel Sale : Sold out.");
		require(whitelistInfos[msg.sender]._buyAmount.add(_tradeAmount) <= whitelistInfos[msg.sender]._AngelQuota, "MTC Angel Sale : MTC amounts error.");

		uint256 _tokenAmountsOut = _tradeAmount.mul(AngelSalePrice);

		require(IERC20(MTCAddr()).transfer(msg.sender, _tokenAmountsOut));
		AngelSaleAmount = AngelSaleAmount.add(_tradeAmount);
