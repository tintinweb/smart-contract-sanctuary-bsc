...
contract HeroNFT is BaseNFT {
  ...
  function getHeroPrices(uint _tokenId) external view returns (uint, uint) {
    return (
      ...
      heroes[_tokenId].failedUpgradingAmount
    );
  }
  ...
  function _initData() private {
    ...
    strengthIndexReferences[1] = [100, 100, 100, 100, 100, 100, 100];

    strengthBonuses[1] = [100, 100, 100, 100, 100, 100, 100];

    experienceCheckpoint[1] = 10;
    experienceCheckpoint[2] = 30;
    experienceCheckpoint[3] = 60;
    experienceCheckpoint[4] = 100;
    experienceCheckpoint[5] = 150;
    experienceCheckpoint[6] = 210;
    experienceCheckpoint[7] = 280;
  }
  ...
  function skillUp(uint _tokenId, uint8 _index) external onlyUpgradingContract {
    ...
    _levelUp(_tokenId);
    if (heroes[_tokenId].failedUpgradingAmount > 0) {
      heroes[_tokenId].ownPrice += heroes[_tokenId].failedUpgradingAmount;
  ...
  function updateEnergyManager(address _energyManager) external onlyMainAdmin {
    require(_energyManager != address(0), "Invalid address");
