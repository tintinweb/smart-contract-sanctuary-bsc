...
contract Staking is Initializable {
    ...
    function getMyStakes() public view returns (Stake[] memory) {
        Stake[] memory myStakes = new Stake[](ownersStakes[msg.sender].length);
        for(uint256 i = 0;i<ownersStakes[msg.sender].length;i++){
            myStakes[i] = stakes[ownersStakes[msg.sender][i]];
        }
        return myStakes;
    }
    
    function distributeRewards() external payable onlyOwner {
        uint256 perToken = address(this).balance / totalStake;
