...
contract IshToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    IshTokenDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public marketingAddress = payable(msg.sender);
    address public deployerAddress = 0xE58BaF94B10122c056C3F0514Cd9C5331f82dDEb;
    ...
    constructor() ERC20("IshToken", "IshToken") {
        dividendTracker = new IshTokenDividendTracker(payable(this));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        IshTokenDividendTracker newDividendTracker = IshTokenDividendTracker(
            payable(newAddress)
        );
    ...
    function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
            _istxLimitExempt[account] = excluded;
    ...
    function updatePayoutToken(address token) public {
        require(balanceOf(msg.sender) > 0, "You must own more than zero $ISH tokens to switch your payout token!");
...
contract IshTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "IshToken_Dividend_Tracker",
            "IshToken_Dividend_Tracker"
        )
    {
        ishToken = IshToken(mainContract);
    ...
    function _transfer(
        ...
        require(false, "IshToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "IshToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main IshToken contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(ishToken);

            uint256 prevBalance = ishToken.balanceOf(address(this));
            ...
            {
                uint256 received = ishToken.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    ishToken.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
