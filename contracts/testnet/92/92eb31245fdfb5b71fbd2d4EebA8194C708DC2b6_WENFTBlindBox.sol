...
contract WENFTBlindBox is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(address payable _toWallet, address _weNFT) {
        ...
        blindBoxInfos.push(BlindBoxInfo(1, 1, 1651, 90000000000000000));
        blindBoxInfos.push(BlindBoxInfo(1652, 1652, 2060, 460000000000000000));
        blindBoxInfos.push(BlindBoxInfo(2061, 2061, 2500, 700000000000000000));
    }
    ...
    function modifyAllBlindBoxInfo(BlindBoxInfo[] calldata _infos) public onlyOwner {
        ...
        for (uint inx = 0; inx < _infos.length; inx++) {
            blindBoxInfos.push(BlindBoxInfo(
                _infos[inx].startTokenId,
                _infos[inx].nextTokenId,
                _infos[inx].endTokenId,
                _infos[inx].price));
        }
    }
