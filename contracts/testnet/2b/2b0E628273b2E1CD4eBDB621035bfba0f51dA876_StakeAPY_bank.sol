...
contract StakeAPY_bank is Initializable, ReentrancyGuardUpgradeable {
    ...
    event BurnFee(address target, uint256 amount);
   ...
   */
    ...
    */
    function withdraw() external nonReentrant() {
        require(isBlacklisted[msg.sender] == false, "Address in blacklist");
    ...
    function _removeStake(address stakeholder) internal onlyStakeUnheld(stakeholder) {
        ...
        totalFee = totalFee.add(balance.div(100));

        Bank(_bank).transferTo(stakeholder, balance.div(100).mul(99));
    ...
    function transferFee(address target, uint256 amount) external {
        require(msg.sender == ADMIN, "Not ADMIN");
        require(amount <= totalFee, "Amount exceeds fee");
        
        totalFee = totalFee.sub(amount);
        Bank(_bank).transferTo(target, amount);

        emit BurnFee(target, amount);
    }

    function adminSetBank(address bank) external {
        require(msg.sender == ADMIN, "Not ADMIN");
        _bank = bank;
    }

    function adminSetBlacklist(address user, bool status) external {
        require(msg.sender == ADMIN, "Not ADMIN");
