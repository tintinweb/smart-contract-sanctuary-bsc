...
/*
 * The Default Pool holds the collateral and VST debt (but not VST tokens) from liquidations that have been redistributed
 * to active troves but not yet "applied", i.e. not yet recorded on a recipient active trove's struct.
 *
 * When a trove makes an operation that applies its pending collateral and VST debt, its pending collateral and VST debt is moved
...
contract DefaultPool is OwnableUpgradeable, CheckContract, IDefaultPool, PoolBase2 {
    ...
    address internal vestaFinanceTreasury;
    ...
    event DefaultPoolVSTDebtUpdated(uint256 _VSTDebt);
    ...
    function setAddresses(
        ...
        address _vestaTreasuryAddress
    ) external onlyOwner {
        checkContract(_troveManagerAddress);
        checkContract(_activePoolAddress);
        checkContract(_whitelistAddress);
        checkContract(_vestaTreasuryAddress);

        troveManagerAddress = _troveManagerAddress;
        activePoolAddress = _activePoolAddress;
        whitelist = IWhitelist(_whitelistAddress);
        whitelistAddress = _whitelistAddress;
        vestaFinanceTreasury = _vestaTreasuryAddress;
    ...
    function getVSTDebt() external view override returns (uint256) {
        return VSTDebt;
    }
    ...
    function increaseVSTDebt(uint256 _amount) external override {
        _requireCallerIsTroveManager();
        VSTDebt = VSTDebt.add(_amount);
        emit DefaultPoolVSTDebtUpdated(VSTDebt);
    }

    // Decreases the VST Debt of this pool.
    function decreaseVSTDebt(uint256 _amount) external override {
        _requireCallerIsTroveManager();
