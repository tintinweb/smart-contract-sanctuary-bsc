    ...
    Pausable
{
    ...
    ) public ERC721Base("MintableHON721", "MintableHON721", contractURI, tokenURIPrefix) {
    ...
    function batchMint(
        ...
        require(tokenIdArr.length > 0, "Wrong parameters");
        require(tokenIdArr.length == _feesArr.length, "Wrong parameters");
        require(tokenIdArr.length == tokenURIArr.length, "Wrong parameters");

        require(
            _signer ==
                ecrecover(keccak256(abi.encodePacked(this, tokenIdArr)), v, r, s),
            "owner should sign tokenIdArr"
        );

        for (uint256 i = 0; i < tokenIdArr.length; i++) {
            _mint(msg.sender, tokenIdArr[i], _feesArr[i]);
            _setTokenURI(tokenIdArr[i], tokenURIArr[i]);
        }

        emit BatchMinted(msg.sender, tokenIdArr);
    }
