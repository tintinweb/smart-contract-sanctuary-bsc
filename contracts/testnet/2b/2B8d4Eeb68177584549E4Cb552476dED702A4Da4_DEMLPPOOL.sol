...
contract DEMLPPOOL is LPTokenWrapper, CoinFactoryAdminRole {
    ...
    modifier updateReward(uint256 nodeId, address account) {
        if (block.timestamp > starttime) {
            ...
            if (account != address(0)) {
                ...
                rewards[account] = rk;
                userRewardPerTokenPaid[account][nodeId] = rewardPerTokenStored;
            }
        }
        _;
    }
    ...
    function earned(uint256 nodeId, address account) public view returns (uint256) {
        ...
        return
        ban.mul(rewardPerToken().sub(userRewardPerTokenPaid[account][nodeId]))
        .div(1e18)
        .add(rewards[account]);
    }
    ...
    function getReward(uint256 nodeId, address user) internal updateReward(nodeId, user) {
        ...
        if (damToken.balanceOf(address(this)) <= 0) return;
    ...
    function notifyRewardAmount(uint256 reward, uint256 _DURATION) public updateReward(99999, address(0)) onlyOwner {
        rewardRate = reward;
        changeDay(_DURATION);
