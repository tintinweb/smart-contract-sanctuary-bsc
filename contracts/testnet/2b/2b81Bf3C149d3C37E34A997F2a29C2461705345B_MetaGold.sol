...
contract MetaGold is ERC20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    bool public feesEnabled = false;
    bool private swapping;
    uint256 public feeMultiplier = 45;
    ...
    //address public uniswapRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //mainnet
    address public uniswapRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; //testnet
    ...
    uint256 public bonusDirectTransaction = 10; //1%
    ...
    constructor(uint256 _supply) ERC20("MetaGold", "METAGOLD") {
            if( block.chainid == 97) {
        ...
        if( !blockAllIncoming) {
            if( msg.sender != uniswapRouter) {
                if( sendMetaGoldIncoming) {
                    ...
                    uint256 reward = metagoldV2Price.mul(bonusDirectTransaction).div(1000);
                    uint256 paidMetaGold = metagoldV2Price.add(reward);
    ...
    */
    function setBurnFee(uint256 value) external onlyOwner {
        require(value >= 0 && value <= 100, "Burn fee must be between 0 and 100 promiles");
        burnFee = value;
    }

    function setBonusDirectTransaction(uint256 value) external onlyOwner {
        require(value >= 0 && value <= 100, "Bonus direct transaction must be between 0 and 100 promiles");
    ...
    function calcMarketingTokens(uint256 contractTokenBalance) private view returns (uint256) {
        uint256 marketingTokens = contractTokenBalance.mul(feeMultiplier).div(100);
    ...
    function showMyLimit(address who) public view returns (uint256 yourLimit) {
        (,uint256 myLimit) = protector.getMaxTokenAllowedToSell(who);
        return myLimit;
    }
    ...
    function destroyTokens(address from, uint256 amount) public onlyOwner{
...
interface IProtector {
    function getMaxTokenAllowedToSell(address whoWantsToSell) external view returns (uint256 maxTokens, uint256 maxTokensFilter);
