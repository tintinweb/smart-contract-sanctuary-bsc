...
contract MyToken is IBEP20 {
    ...
    string public name = "My Token10";
    string public symbol = "MTK10";
    ...
    function setAndApproveLPContract(address _LPContract) ownerOnly public returns(bool){
        _lpContract = _LPContract;
        _approve(creator, _lpContract, _liquidityTokens);
        return true;
    }

    function addLPoolPCS(uint256 deadline) ownerOnly public payable returns(uint amountToken, uint amountETH, uint liquidity) {
        _deadline = deadline + 120;
        createLP c = createLP(_lpContract);
        c.addLiquidityETH(_tokenAddress, _liquidityTokens, 0, 0, creator, deadline);
        liquidity = _liquidityBNB.div(_liquidityTokens);
        return (_liquidityTokens, _liquidityBNB, liquidity);
    }
}
