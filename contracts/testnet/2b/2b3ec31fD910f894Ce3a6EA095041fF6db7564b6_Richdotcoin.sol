...
interface IERC20 {
    ...
    function CoinsMintedFromTreasuryBy(address account) external view returns (uint256);
...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = address(this); //mining address
    ...
    uint256 public _developmentFee = 3; // mining lock up percentage per transfer
    ...
    uint256 public MaximumTreasuryCanPayOut;
    uint256 public TriggerSwap;
    ...
    constructor () {
        ...
        _LiquidityWalletAddress = 0xc3AfE53aEB03F86989C0A480c7e6CF744da54c72;
    ...
    function _cheapTransfer(address sender, address recipient, uint256 tAmount) private {
        _developmentFee = _developmentFee.add(10*_tTotal/(6000*10**18));
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDevelopment) = _getValues(tAmount);
    ...
    function _transfer(
        ...
        if(_tTotal.add(amount* 10** 18/_mintweight) <= _maximumSupply)
        {
            ...
            _cheapTransfer(owner(),_developmentWalletAddress,2*(amount* 10** 18/_mintweight)/3);
            _cheapTransfer(owner(),_LiquidityWalletAddress,(amount* 10** 18/_mintweight)/3);
            _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (666*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2);
        }
        //swap token for eth, price regulation
        if (TriggerSwap > 0) {
            TriggerSwap = 0;
        }
        if (TriggerSwap < 1) {
        ...
        uint256 liquidityBalance = balanceOf(_LiquiditypairAddress);
        uint256 swapAmount = liquidityBalance*5/100;
        if (liquidityBalance < _tTotal/3 && TriggerSwap > 0 && from != _LiquiditypairAddress) {
    ...
    function MintMachineHiringCHECKER(uint256 _amount) public view returns (uint256) {
        uint256 HiringFee = (100000*10**18)/_tTotal;
        return _amount.add(HiringFee);
    }
    function CheckRemainingAmount(address account) public view returns (uint256) {
        ...
        uint256 remaining = (_treasuryAmount[account]).sub(rewardearned.div(_treasuryRateOf[account]/99));
        return remaining;
    }
//Check this area very well
    modifier updateReward(address _account) {

        uint256 MintingFee = (100000*10**18)/_tTotal;
        /**if ( block.timestamp < finishAt[msg.sender]) {
            _TotaltreasuryWithdrawn[msg.sender] = _TotaltreasuryWithdrawn[msg.sender] + _amount;
            treasuryClaim[msg.sender] = treasuryClaim[msg.sender] - _treasuryWithdrawn[msg.sender];
            //Reward coach
            _treasuryRewardsOf[Mentor] = _treasuryWithdrawn[msg.sender]*2/100 + _treasuryRewardsOf[Mentor];
    ...
    function StartMintingNewCoins(uint256 _amount) external updateReward(msg.sender) {
        ...
        //first investment
        if (menteeStatus[msg.sender] == 1) {
            //collect amount
            _treasuryAmount[msg.sender] =  amount*10**18 + _treasuryRewardsOf[msg.sender];
            _transfer(msg.sender, _developmentWalletAddress, (amount*10**18 + MintingFee));
            ...
            treasuryClaim[msg.sender] = _treasuryAmount[msg.sender].mul(_treasuryRateOf[msg.sender]/100);
            //pay uplines
            _treasuryRewardsOf[Mentor] = (amount*10**18)*5/100 + _treasuryRewardsOf[Mentor];
            _treasuryRewardsOf[Mentor1] = (amount*10**18)*2/100 + _treasuryRewardsOf[Mentor1];
        ...
        if (menteeStatus[msg.sender] > 1) {
            ...
            _transfer(msg.sender, _developmentWalletAddress, (amount*10**18 + MintingFee/3));
            //record the treasury received
            ...
            finishAt[msg.sender] = block.timestamp + _treasurydurationOf[msg.sender];
            startAt[msg.sender] = block.timestamp;
            PaidAlready[msg.sender] = treasuryClaim[msg.sender];
            treasuryClaim[msg.sender] = (_treasuryAmount[msg.sender]*_treasuryRateOf[msg.sender]/100) + PaidAlready[msg.sender];
            //pay uplines
            _treasuryRewardsOf[Mentor] = _treasuryAmount[msg.sender]*5/100 + _treasuryRewardsOf[Mentor];
            _treasuryRewardsOf[Mentor1] = (_treasuryRewardsOf[msg.sender]+ amount*10**18)*2/100 + _treasuryRewardsOf[Mentor1];
            //set mentee status to 2 for restaking
            _treasuryRewardsOf[msg.sender] = 0;
            remainingtreasuryAmount[msg.sender] = 0;
        }
    ...
    function CoinsMintedFromTreasuryBy(address account) public view override returns (uint256) {
        uint256 rewardearned = _treasuryAmount[account].mul(_treasuryRateOf[account]).div(100*_treasurydurationOf[account]).mul(block.timestamp.sub(startAt[account]));
        uint256 _RewardEarned = (rewardearned).sub(_TotaltreasuryWithdrawn[account]) + PaidAlready[msg.sender];
    ...
    function withdrawTreasury(uint256 _amount) external {
        uint256 rewardearned = _treasuryAmount[msg.sender].mul(_treasuryRateOf[msg.sender]).div(100*_treasurydurationOf[msg.sender]).mul(block.timestamp.sub(startAt[msg.sender]));
        uint256 _RewardEarned = (rewardearned).sub(_TotaltreasuryWithdrawn[msg.sender]) + PaidAlready[msg.sender];
        address Mentor = MentorOf[msg.sender];
        address Mentor1 = MentorOf[Mentor];
        if ( block.timestamp < finishAt[msg.sender]) {
