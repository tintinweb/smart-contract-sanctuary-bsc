...
contract ZeroLossFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ZLT
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. ZeroLoss to distribute per block.
        uint256 lastRewardBlock;    // Last block number that ZeroLoss distribution occurs.
        uint256 accTokenPerShare;   // Accumulated ZeroLoss per share, times 1e12. See below.
    ...
    // ZeroLoss tokens created per block.
    ...
    // The block number when ZeroLoss mining starts.
    ...
    uint16 emergencyWithdrawFee = 500;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint16 _withdrawFeeBP, uint256 _harvestInterval, bool _withUpdate,
    ...
    // Update the given pool's ZeroLoss allocation point and deposit fee. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint16 _withdrawFeeBP, uint256 _harvestInterval, bool _withUpdate,
        uint256 _withdrawLockPeriod ) external onlyOwner {
    ...
    // View function to see pending ZeroLoss on frontend.
    ...
    // View function to see if user can harvest ZeroLoss.
    ...
    // Deposit LP tokens to MasterChef for ZeroLoss allocation.
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        uint256 amountFee = amount.mul(emergencyWithdrawFee).div(10000);
    ...
    // Pay or lockup pending ZeroLoss.
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough ZeroLoss.
    ...
    function showTreasuryAmount() external view returns (uint256) {
        return token.balanceOf(address(this));
    }

    function setEmergencyWithdrawFee(uint16 _fee) external onlyOwner {
