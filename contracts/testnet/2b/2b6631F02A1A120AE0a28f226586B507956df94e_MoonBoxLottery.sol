...
contract MoonBoxCashBack is Ownable, ReentrancyGuard {
    ...
    function claim(uint256 id) external nonReentrant {
        require(enable, "Disable");
        Log storage log = logs[id];
        require(log.buyer != address(0), "Not found");
        require(log.buyer == msg.sender, "Not buyer");
        require(!log.claim, "Claimed");

        logs[id].claim = true;
        totalClaim = totalClaim.add(log.amount);
        ...
        emit Claim(id, msg.sender, log.amount);
    }
    ...
    event Claim(uint256 id, address buyer, uint256 amount);
}
