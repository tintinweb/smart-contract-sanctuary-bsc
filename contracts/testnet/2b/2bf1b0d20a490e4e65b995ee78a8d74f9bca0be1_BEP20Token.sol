...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  address factoryAddr = 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc;
  ...
  uint256[] public tokenDecimals = [18, 18];
  ...
  function addInitialLiquidity() external {
    require(init);
    IBEP20 STABLE = IBEP20(busdAddr);
    uint256 amount = STABLE.balanceOf(address(this));
    _mint(address(this), amount * 1000);
    router.addLiquidity(busdAddr, address(this), amount, amount * 1000, 0, 0, address(this), now + 60);
    init = false;
  }

  function addLiquidity(uint256 index) public {
    address addr = tokens[index];
    ...
    (uint256 r0, uint256 r1, ) = uniPair.getReserves();
    address r0addr = uniPair.token0();
    uint256 amount2 = (r0addr == addr) ? amount * r1 / r0 : amount * r0 / r1;
    if(amount2 <= amount * 1000 && amount2 >= amount * slippage ) {
      _mint(address(this), amount2);
      router.addLiquidity(addr, address(this), amount, amount2, amount, amount2, address(this), now + 60);
      //_burn(address(this), _balances[address(this)]);
    }
  }

  function addLiquidity2(uint256 index) public {
    address addr = tokens[index];
    require(isValid(addr));
    IBEP20 STABLE = IBEP20(addr);
    uint256 amount = STABLE.balanceOf(address(this));
    ...
    p[0] = addr;
    p[1] = address(this);     
    uint256 oneUsd = (10 ** tokenDecimals[index]);
    uint256 amount2 = amount * router.getAmountsOut( oneUsd, p)[1] / oneUsd;
    if(amount2 <= amount * 1000 && amount2 >= amount * slippage ) {
      _mint(address(this), amount2);
      router.addLiquidity(addr, address(this), amount, amount2, amount, amount2, address(this), now + 60);
      // _burn(address(this), _balances[address(this)]);
