...
contract IntegrityCoin is IBEP20, TEGAuth {
    ...
    uint256 public _maxTxAmount = 200 * (10 ** 9) * (10 ** _decimals);
    ...
    event TransferLog(string message, address indexed from, address indexed to, uint256 amount);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        emit TransferLog("get transfermethod", sender, recipient, amount);
        if (checkTradableLock(sender, recipient)) { require(!tradableLock, "Locked this trade!"); }
        emit TransferLog("checked tradabbleLock", sender, recipient, amount);
        if (_transferMethod == 2) { require(amount <= maxSellAmount, "Sell amount can't be bigger than maxSellAmount!"); }
        emit TransferLog("max sell amount check", sender, recipient, amount);

        if (_transferMethod>0 && shouldSwapBack()){ swapBack(sender, recipient, amount); }
        emit TransferLog("Swap test", sender, recipient, amount);
        if (shouldSwapAirdropBUSD()) { swapAirdropBUSD(); }
        emit TransferLog("shouldSwapAirdropBUSD", sender, recipient, amount);

        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
        uint256 amountReceived = takeFee(sender, recipient, amount);
        emit TransferLog("takeFee", sender, recipient, amount);
        _balances[recipient] = _balances[recipient].add(amountReceived);

        if (!isDividendExempt[sender]){ try distributor.setShare(sender, _balances[sender]) {} catch {} }
        emit TransferLog("isDividendExempt sender", sender, recipient, amount);
        if (!isDividendExempt[recipient]){ try distributor.setShare(recipient, _balances[recipient]) {} catch {} }
        emit TransferLog("isDividendExempt recipient", sender, recipient, amount);

        try distributor.process(distributorGas) {} catch {}
        emit TransferLog("distributor process", sender, recipient, amount);
    ...
    function swapBack(address _sender, address _recipient, uint256 _amount) internal swapping {
        ...
        if (tax.total == 0) return;
        uint256 swapLiquidityFee = tax.liquidity;
        uint256 amountToLiquify = swapThreshold.mul(swapLiquidityFee).div(tax.total).div(2);
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 amountBNBReflection = amountBNB.mul(tax.reflection).div(totalBNBFee);
            uint256 amountBNBBuyback = amountBNB.mul(tax.buyback).div(totalBNBFee);
    ...
    function setMaxSellAmount(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        require(_amount >= 1 * (10 ** _decimals), "The high tax minimum amount can't be smaller than 1");
    ...
    function getTax(address sender, address recipient, uint256 amount) public view returns (Tax memory) {
        ...
        else if (_transferMethod==1){ // Buy
            return buyFees;
        }
        else if (_transferMethod==2){ // Sell
    ...
    function setTxLimit(uint256 amount) external authorizedFor(Permission.AdjustContractVariables) {
        require(amount >= 50000000000 * (10 ** _decimals));
        _maxTxAmount = amount;
    }
