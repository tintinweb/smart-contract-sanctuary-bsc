...
// File: ICrocoProfile.sol

/**
 * @title ICrocoProfile
 */
interface ICrocoProfile {
...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // Croco profile
    ICrocoProfile public immutable crocoProfile;

    // Croco Profile is requested
    bool public crocoProfileIsRequested;

    // Croco Profile points threshold
    uint256 public crocoProfileThresholdPoints;
    ...
    /**
     * @notice Constructor
     * @param _crocoProfile: Croco Profile address
     * @param _crocoProfileIsRequested: Croco Profile is requested
     * @param _crocoProfileThresholdPoints: Croco Profile need threshold points
     */
    constructor(
        address _crocoProfile,
        bool _crocoProfileIsRequested,
        uint256 _crocoProfileThresholdPoints
    ) {
        SMART_CHEF_FACTORY = msg.sender;

        // Call to verify the address is correct
        ICrocoProfile(_crocoProfile).getTeamProfile(1);
        crocoProfile = ICrocoProfile(_crocoProfile);

        // if crocoProfile is requested
        crocoProfileIsRequested = _crocoProfileIsRequested;

        // crocoProfile threshold points when profile & points are requested
        crocoProfileThresholdPoints = _crocoProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!crocoProfileIsRequested && crocoProfileThresholdPoints == 0) ||
            crocoProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (crocoProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = crocoProfile.getUserProfile(msg.sender);
        }

        require(
            crocoProfileThresholdPoints == 0 || numberUserPoints >= crocoProfileThresholdPoints,
    ...
    function updateProfileAndThresholdPointsRequirement(bool _isRequested, uint256 _thresholdPoints) external onlyOwner {
        ...
        crocoProfileIsRequested = _isRequested;
        crocoProfileThresholdPoints = _thresholdPoints;
...
contract SmartChefFactory is Ownable {
    ...
    function deployPool(
        ...
        bytecode = abi.encodePacked(
            bytecode,
            abi.encode(_crocoProfile, _crocoProfileIsRequested, _crocoProfileThresholdPoints)
        );
