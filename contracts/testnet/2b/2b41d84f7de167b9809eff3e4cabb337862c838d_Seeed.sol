...
contract Seeed is ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    function mint(uint8 _quantityToMint) public{
        ...
        require(
            (_quantityToMint + totalSupply()) <= _maxSupply,
    ...
    function customMint(string memory _tokenURI, string memory _tokenMeta) public{
        require((totalSupply()) < _maxSupply, "Exceeds maximum supply");
        uint256 newItemId = _tokenIds.current();
        _mint(msg.sender, newItemId);
    ...
    function setMaxSupply (uint256 _supply) public onlyAdmin{
        _setMaxSupply(_supply);
    }

    function resetMaxSupplyToDefault () public onlyAdmin {
        _setMaxSupply(10 ** 18);
    }
}