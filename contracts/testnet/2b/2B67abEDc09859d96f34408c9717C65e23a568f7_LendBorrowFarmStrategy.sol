...
contract LendBorrowFarmStrategy is LogicUpgradeable {
    ...
    address[] private pathToBNBToBLID;
    ...
    /**
     * @notice Set pathToBNBToBLID
     * @param path path to BNB to BLID
     */
    function setPathToBNBToBLID(address[] calldata path) external onlyOwner {
        uint256 length = path.length;
        require(length >= 2, "F16");
        require(path[length - 1] == blid, "F18");

        pathToBNBToBLID = new address[](length);
        for (uint256 i = 0; i < length; ) {
            pathToBNBToBLID[i] = path[i];
     ...
     */
    function addVToken(address token, address vToken) public onlyOwner {
    ...
    function build(uint256 usdAmount) external onlyOwnerAndAdmin {
        ...
        FarmingPair[] memory reserves = ILendBorrowFarmingPair(farmingPair)
            .getFarmingPairs();
        uint256[] memory percentages = ILendBorrowFarmingPair(farmingPair)
            .getPercentages();

        // Check percentage and farmingPair are matched
        ILendBorrowFarmingPair(farmingPair).checkPercentages(percentages);
        ...
        for (index = 0; index < count; ) {
            ...
            // Calculate BNB amount for this pair
            uint256 borrowUSDAmount = (usdAmount * percentages[index]) / 10000;

            // ********** Calculate the borrow amount for each pair ********** //
            ...
            uint256 token0Price = IOracle(
                IComptrollerVenus(comptroller).oracle()
            ).getUnderlyingPrice(reserve.xTokenA);
            uint256 token1Price = IOracle(
                IComptrollerVenus(comptroller).oracle()
            ).getUnderlyingPrice(reserve.xTokenB);

            // get Reserves
            (balance0, balance1, ) = IPancakePair(reserve.lpToken)
                .getReserves();
            ...
            // Calculate amount for addToLiquidity
            token0Amount =
            ...
            // Borrow Token A, Token B
            if (token0BorrowAmount > 0) {
                balance0 = ILogicContract(_logic).borrow(
                    reserve.xTokenA,
                    token0BorrowAmount,
                    0
                );

                require(balance0 == 0, "F13"); // Borrow should be successed
            }

            if (token1BorrowAmount > 0) {
                balance0 = ILogicContract(_logic).borrow(
                    reserve.xTokenB,
                    token1BorrowAmount,
                    0
            ...
            emit Borrow(reserve.xTokenA, token0BorrowAmount);
            emit Borrow(reserve.xTokenB, token1BorrowAmount);
    ...
    function destroy(uint256 _percentage) public onlyOwnerAndAdmin {
        ...
        for (index = 0; index < count; ) {
            ...
            if (depositedLp > 0)
                withdrawAndRepay(reserve, (depositedLp * _percentage) / 10000);
    ...
    function destroyAll() public onlyOwnerAndAdmin {
        ...
        // Swap available BNB to BLID
        sendRewardsToStorage();
    }
    ...
    function returnAllTokensToStorage() external onlyOwnerAndAdmin {
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            // Get redeemUnderlying amount
            uint256 amount = IMultiLogicProxy(multiLogicProxy).getTokenBalance(
                token,
                _logic
            );
            if (token != address(0))
                amount -= IERC20Upgradeable(token).balanceOf(_logic); // Logic doesn't have BNB after destroyAll();

            // RedeemUnderlying
            if (amount > 0)
                ILogicContract(_logic).redeemUnderlying(vTokens[token], amount);

            // Return existing Token To Storage
            amount = token == address(0)
                ? address(_logic).balance
                : IERC20Upgradeable(token).balanceOf(_logic);
            if (amount > 0)
                ILogicContract(_logic).returnTokenToStorage(amount, token);
    ...
    function claimRewards(uint8 mode) external onlyOwnerAndAdmin {
        require(pathToBNBToBLID.length >= 2, "F15");
        ...
        // Convert BNB to BLID
        uint256 amountBLID = sendRewardsToStorage();
     ...
     * @param mode 0 : all, 1 : Venus only, 2 : Farm only
     */
    function claimInternal(uint8 mode) private {
     ...
     */
    function sendRewardsToStorage() internal returns (uint256 amountBLID) {
        address _logic = logic;

        // Convert BNB to BLID
        uint256 amountIn = address(_logic).balance;
     ...
     */
    function withdrawAndRepay(FarmingPair memory reserve, uint256 lpAmount)
        private
    {
