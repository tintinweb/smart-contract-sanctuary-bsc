...
contract HoneybeePledge is Initializable, OwnableUpgradeable, DivestorUpgradeable {
    ...
    function _changeOpt() private {
        for (uint i = 4; i >= 0; i--) {
            if (debt.totalValue >= burnPower[i]) {
    ...
    function _allocateInviterReward(address account_, uint reward) private {
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            if (inviter == address(0)) {
                addrInfo.HBEE.safeTransfer(inviter, reward * inviterRate[i] / 100);
            } else {
    ...
    function getLevel(address account_) public view returns(uint) {
        ...
        uint level;
        if (info.directInviter >= 10 && info.stakeValue >= 5000 ether && info.commPower >= 1000000 ether ) {
            level = 5;
        } else if (info.directInviter >= 8 && info.stakeValue >= 4000 ether && info.commPower >= 500000 ether ) {
            level = 4;
        } else if (info.directInviter >= 7 && info.stakeValue >= 3000 ether && info.commPower >= 100000 ether ) {
            level = 3;
        } else if (info.directInviter >= 6 && info.stakeValue >= 2000 ether && info.commPower >= 30000 ether ) {
            level = 2;
        } else if (info.directInviter >= 5 && info.stakeValue >= 1000 ether && info.commPower >= 10000 ether ) {
            level = 1;
        } else {
            level = 0;
        }
    ...
    function viewSlotReward(address account_, uint slot_) public view returns (uint) {
        ...
        uint rewards = slotInfo.stakeValue * (coutingDebt() - slotInfo.debt) / 1 ether;

        // if (slotInfo.stakeTime < debt.startTime) {
        //     uint deb = slotInfo.stakeValue * (debt.startDebted - slotInfo.debt) / 1 ether;
        //     rewards -= deb;
        // }
        rewards -= (rewards * dailyInfo.burnRate / 100);
        return rewards;
    } 
    ...
    function viewNodeSlotReward(uint slowId) public view returns(uint) {
        ...
        reward -= (reward * dailyInfo.burnRate / 100);
        return reward;
    }
