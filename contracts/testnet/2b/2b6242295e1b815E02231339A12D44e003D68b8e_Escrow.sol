...
contract Escrow is Ownable, ReentrancyGuard{
   ...
   // mapping (address => bool) public lock;
    ...
    function EnterMatch(uint256 tokens_) nonReentrant external {
       
        require(tokens_ >= 1000000000000000000 ,"Price cannot be less than 1");
        require(token.balanceOf(msg.sender) >= tokens_, "Not Enough PlayZap Tokens In Wallet") ;
        require (!blacklist[msg.sender],"Address is black listed");
 
        token.transferFrom(msg.sender,address(this), tokens_);
        
        emit TokenReceived(msg.sender,tokens_);
    }

    //distribute winning and commission tokens 
    function TokenDistribution(address[] memory _addresses, uint256[] memory prizes,uint256 fees,address commission) nonReentrant external onlyOwner{
        ...
        for(uint256 i=0; i<length; i++)
        {
           ...
           _balances[_addresses[i]] += prizes[i];
        }
