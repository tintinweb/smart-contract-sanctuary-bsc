 ...
 *
*/
contract MgNftDex is DEXConfig, OrderHelper, ReentrancyGuard, OrderValidator, TransferHelper,Initializable {
    using Strings for uint256;
    //订单可购买的数量
    mapping(uint256 => uint256) public purchasedNumber;

    function initialize(address _daoAddr, address feeRecipientAddr, string memory _name, string memory _version, uint256 _chainId) public initializer {
        __ReentrancyGuard_init();
        __DEXConfig_init(_daoAddr, feeRecipientAddr, _name, _version, _chainId);
    }
    ...
    */
    ...
    /**
     * 单笔交易
     * @param takerInfo 买方信息
     * @param _makerOrder 卖家订单信息
     * @param _makerSig 签名信息
    ...
    nonReentrant {
        ...
        _validateOrder(_makerOrder, _makerSig, _taker);
        ...
        _transferNFT(_makerOrder, _maker, _taker, buyNftNumber, _ftAmount);

        // transfer token  转移支付代币
        //:第二参数是to地址，第三参数是from地址，买家给卖家转账
        _transferFT(_makerOrder, _taker, _maker, _ftAmount);
    ...
    function caculateMustValue(uint256 saleOrderId, uint256 _ftAmount, uint256 _royaltyRate, uint256 value) private view {
        ...
        require(value >= payAmount, string.concat("saleOrderId:", saleOrderId.toString(), ",Require Value:", saleOrderId.toString()));
    }
    ...
    function cancelOrder(FixedPriceOrder[] calldata _orders) external {
        ...
        for (uint256 i = 0; i < _orders.length; i++) {
            ...
            (, , bytes32 orderHash,) = _deriveOrder(order);
    ...
    function _isPlatformNft(address _nft) internal view override returns (bool) {
...
contract DEXConfig is EIP712, DEXEventsAndErrors, Manager {
    ...
    function __DEXConfig_init(address _daoAddr, address _feeRecipient, string memory _name, string memory _version, uint256 _chainId) internal{
        __EIP712_init(_name, _version, _chainId);
        __Manager_init(_daoAddr);
...
abstract contract EIP712 {
    ...
    function __EIP712_init(string memory _name, string memory _version, uint256 _chainId) internal {
        _NAME = _name;
        _VERSION = _version;
        _CHAINID = _chainId;
    ...
    function _buildDomainSeparator(
        ...
        return keccak256(abi.encode(_typeHash, _nameHash, _versionHash, _chainId, address(this)));
    }
...
contract Manager is IManager, DEXEventsAndErrors {
    ...
    function __Manager_init(address _dao) internal{
        dao = _dao;
        operators[_dao] = true;
    }
...
abstract contract OrderDeriver is DEXConstantsAndStructs {
    ...
    function _hashAssets(Assets calldata assets) internal pure returns (bytes32, bytes memory) {
    ...
    function _eip712Encode(FixedPriceOrder calldata _order, bytes32 _assetsHash)
    internal
    pure
    ...
    function _hashOrder(bytes memory _orderBytes) internal view virtual returns (bytes32);
    ...
    returns (
        ...
        (assetsHash, assetsBytes) = _hashAssets(_order.assets);

        orderBytes = _eip712Encode(_order, assetsHash);
...
contract OrderHelper is DEXConstantsAndStructs {
    ...
    * @param nft nft地址
    * @param nftId  nftId号
    * @param firstPrice 初始地址
    * @return 该订单号`saleOrderId`最终使用的价格`firstPrice`
    */
    ...
    /**
    * @param saleOrderId 订单号
    * @param buyNftNumber 购买的NFT数量
    * @param taker 买方地址
    * @param currentPrice 当前价格
    * @return 如果授权的地址为0.那么返回(false，0),如果授权地址存在，那么返回(true，和授权价格)
    */
...
abstract contract OrderValidator is NonceManager, OrderDeriver {
    ...
    internal
    view {
        (,, bytes32 orderHash,) = _deriveOrder(_makerOrder);
...
//  erc721  =》 erc 20
//  erc1155 =》 erc 20
abstract contract TransferHelper is DEXConstantsAndStructs, ReentrancyGuard {
    ...
    function _transferFT(
        ...
        if (msgVal > payAmount) {
            _sendValue(payable(msg.sender), msgVal - payAmount);
        }
        //转账版税
        _sendValue(payable(_royaltyRecipient), royaltyAmount);
        //平台抽成
        _sendValue(payable(feeRecipient), platformAmount);
        //转给卖家
        _sendValue(payable(_ftRecipient), _ftAmount);
    }
     ...
     * @dev send eth
     */
    function _sendValue(address payable _recipient, uint256 _amount) internal nonReentrant {
        require(address(this).balance >= _amount, "InsufficientBalance");
        (bool success,) = _recipient.call{value : _amount}("");
        //bool success = _recipient.send(_amount);
        require(success, string.concat(Strings.toHexString(_recipient), " SendValue:", Strings.toHexString(_amount), " fail"));
    }


    function _isPlatformNft(address _nft) internal view virtual returns (bool);
