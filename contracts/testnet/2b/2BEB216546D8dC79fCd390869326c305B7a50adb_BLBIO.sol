...
// get -------------------------------------------------------------------------
    ...
    /**
     * @return amount BLBs is earned for 1 USD.
     *
     * @notice the private and public amount are calculated automatically.
     */
    function BLBsPerUSD(uint256 amountUSD) public view returns(uint256) {
        require(!soldOut, "BLBIO: sold out!");
        return amountUSD >= retailLimit ? privateBLBsPerUSD : publicBLBsPerUSD;
    }

    /**
     * @return amount BLBs is earned for 1 BNB.
     *
     * @notice the private and public amount are calculated automatically.
     */
    function BLBsPerBNB(uint256 amountBNB) public view returns(uint256) {
        require(!soldOut, "BLBIO: sold out!");
        uint256 amountUSD = amountBNB * priceBNB() / 10 ** 8;
        return BLBsPerUSD(amountUSD) * priceBNB() / 10 ** 8;
    }
...
// set -------------------------------------------------------------------------
    ...
    function purchaseInBNB() public payable {
        ...
        uint256 amountBLB = BLBsPerBNB(amountBNB) * amountBNB / 10 ** 18;
        require(
    ...
    function purchaseInBUSD(uint256 amountBUSD) public {
        ...
        address purchaser = msg.sender;
        uint256 amountBLB = BLBsPerUSD(amountBUSD) * amountBUSD / 10 ** 18;
