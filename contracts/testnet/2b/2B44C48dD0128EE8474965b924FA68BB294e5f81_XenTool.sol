...
contract XenTool  {
    ...
    function claimRank(uint256 term, uint256 total) external {
        uint256 i = countMap[msg.sender][term];
        for (; i < total; i++) {
            bytes32 salt = keccak256(abi.encodePacked(msg.sender, term, i));
            address cloneAddress = Clones.cloneDeterministic(address(this), salt);
            addressMap[msg.sender][term][i] = cloneAddress;
            XenTool(cloneAddress).subClaimRank(address(this), term);
            console.log("sub contract:", cloneAddress, 'owner:', XenTool(cloneAddress).getOwner());
            countMap[msg.sender][term]++;
        }
    }

    function claimRank(uint256 term, uint256 total, address _owner) internal {
        uint256 i = countMap[_owner][term];
        for (; i < total; i++) {
            bytes32 salt = keccak256(abi.encodePacked(_owner, term, i));
            address cloneAddress = Clones.cloneDeterministic(address(this), salt);
            addressMap[_owner][term][i] = cloneAddress;
            XenTool(cloneAddress).subClaimRank(address(this), term);
            console.log("getOwner", XenTool(cloneAddress).getOwner());
    ...
    function claimMintReward(uint256 term, uint256[] calldata ids) external {
        claimMintReward(term, ids, msg.sender);
    }

    function claimMintReward(uint256 term, uint256[] calldata ids, address _owner) internal {
        for (uint256 i = 0; i < ids.length; i++) {
            address cloneAddress = addressMap[_owner][term][ids[i]];
            XenTool(cloneAddress).subClaimMintReward();
        }
    }
