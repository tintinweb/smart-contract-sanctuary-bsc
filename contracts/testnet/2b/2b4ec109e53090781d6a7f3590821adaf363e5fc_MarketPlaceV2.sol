...
contract MarketPlaceV2 is MarketPlaceV2Auth, EIP712Upgradeable, PausableUpgradeable {
  ...
  event HeroSummoned(address indexed owner, uint tokenId, uint timestamp);
  event HeroesRevoked(uint[] tokenIds, uint timestamp);
  event OrderCreated(uint indexed id, address maker, uint[] tokenIds, uint takerShare, uint timestamp);
  ...
  function makeOrder(uint[] calldata _tokenIds, uint _takerShare) external whenNotPaused {
    ...
    emit OrderCreated(orderCounter, msg.sender, _tokenIds, _takerShare, block.timestamp);
  }
  ...
  function summonHero(uint16 _classId, bytes memory _signature, uint _timestamp) external whenNotPaused {
    ...
    emit HeroSummoned(msg.sender, tokenId, block.timestamp);
  }
  ...
  function revokeOrders(uint[] calldata _orderIds) external onlyPVE {
    ...
    for (uint i = 0; i < _orderIds.length; i++) {
      ...
      require(order.taker != address(0), "EatherTrading: order invalid");
  ...
  function _validateUser() private view {
    require(!lockedUser[msg.sender], "EatherTrading: user locked");
  }
