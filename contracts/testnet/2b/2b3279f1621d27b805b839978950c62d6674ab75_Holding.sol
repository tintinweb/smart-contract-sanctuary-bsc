...
constructor() {
    ...
    isExempt[mainContractAddress] = true;
...
// staking
   function stakeTokens(uint8 _percentToStake) external {
        IBEP20 token = IBEP20(mainContractAddress);
    ...
    function payStakingDividend() external restricted() {
        ...
        uint256 _walletMax = (IBEP20(mainContractAddress).totalSupply() * 45) / 1000;
        for (uint i=0; i<stakers.length; i++) {
            ...
            if (balance >= _walletMax) {
                ...
                IBEP20(mainContractAddress).transfer(recipient, dividend);
