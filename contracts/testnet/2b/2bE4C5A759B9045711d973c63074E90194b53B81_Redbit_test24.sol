...
contract Redbit_test24 is Context, IBEP20 {
 ...
 function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            ...
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(MarketingAmt).sub(TaxAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(MarketingAmt).sub(TaxAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(MarketingAmt).sub(TaxAmt)));
        }
