...
contract Pool is IPool, Secure {
  ...
  IPool private constant oldPool = IPool(0xa8631c38e28a68d16BD733F422FaDBF1Ae4eCe6F);
  ...
  // calculate rewards
  function totalInterest(address sender, uint256 requestTime)
    public
    view
    override
  ...
  function withdrawToInvest() external override {
    uint256 daily = totalInterest(_msgSender(), block.timestamp);
...
interface IPool {
  ...
  function totalInterest(address user, uint256 requestTime)
    external
    view
    returns (uint256);
