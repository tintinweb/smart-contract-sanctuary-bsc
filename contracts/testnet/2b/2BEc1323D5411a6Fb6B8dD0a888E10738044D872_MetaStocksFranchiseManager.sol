    ...
    OwnableUpgradeable
{
        ...
        returns (uint256)
    {
        ...
        for (
            uint256 index = 0;
            index < this.getNumberOfMetaStocksFranchises(companyId);
            index++
        ) {
            totalUnclaimed += ((uint256(
                block.timestamp - franchisesLastClaimDates[companyId][0]
    ...
    function createMetaStocksFranchiseUsingBNB(
        ...
        //metaStocksFranchise.mint(to, 0, 1, "0x0");

        companyFranchises[companyId][0] += 1;
        franchisesWorkers[companyId][0] += 1;
        franchisesLastClaimDates[companyId][0] = block.timestamp;
        franchiseContinents[_continentId] += 1;
        emit CreateFranchise(msg.sender, companyId, 0);
    }
    ...
    function claimFromAllFranchisesBNB(uint256 _companyId, address companyOwner)
        external
    {
        uint256 totalUnclaimed = this
            .getMetaStocksFranchisesUnclaimedRewardsBNB(_companyId);

        for (uint256 typeIndex = 0; typeIndex < 10; typeIndex++) {
            franchisesLastClaimDates[_companyId][typeIndex] = block.timestamp;
        }

        payable(teamAddress).transfer(maintainceFranchiseExpensesBNB);
        payable(companyOwner).transfer(totalUnclaimed - maintainceFranchiseExpensesBNB);
    }
...
enum MetaStocksFranchiseType {
    MetaStocksFranchiseType1,
    MetaStocksFranchiseType2,
    MetaStocksFranchiseType3,
    MetaStocksFranchiseType4,
    MetaStocksFranchiseType5,
    MetaStocksFranchiseType6,
