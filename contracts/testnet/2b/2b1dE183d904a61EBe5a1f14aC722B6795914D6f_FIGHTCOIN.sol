...
contract FIGHTCOIN is Context, IERC20 {
    ...
    constructor(address admin_, address recoveryAdmin_, uint256 timelockPeriod_, address lossless_) {
        // _totalSupply = 555_000_000 * 10**9;
        _mint(_msgSender(), 555_000_000 * 10**9);
        _name = "FIGHTCOIN";
        _symbol = "FIGHT";
        _decimals = 9;
        require(admin_ != address(0), "LERC20: Admin cannot be zero address");
    ...
    // --- FIGHTCOIN management ---
    function setSymbol(string memory symbol) external onlyAdmin {
        require(keccak256(abi.encodePacked(_symbol)) != keccak256(abi.encodePacked(symbol)), "LERC20: Symbols must not be the same");
        _symbol = symbol;
    }
