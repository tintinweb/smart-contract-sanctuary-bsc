...
contract EdufyTechToken is Context, IERC20, Ownable {
    ...
    bool public BPDisabledForever; //Line 780 to 801 will trigger this function as True/ False (Default is False)
    ...
    uint256 public _buyMarketingFee = 1; // Charity

    //Sell Fees
    uint256 public _sellTaxFee = 4;
    uint256 public _sellDevFee = 2;
    uint256 public _sellLiquidityFee = 3;
    uint256 public _sellMarketingFee = 1; // Charity

    // Fees (Current)
    uint256 private _taxFee;
    uint256 private _devFee;
    uint256 private _liquidityFee;
    uint256 private _marketingFee; // Charity
...
// Set Bot Protection Enabled Forever - END -> KD
    ...
    */
    ...
    /* Returns BNB (wei) left-over in the contract address
       Owner must set the recipient address
       If owner set to zero (0), all BNB from the contract address will be transferred to the recipient address
    */
    ...
    // WithdrawLeftoverBNB - END -> KD
     ...
     /* WithdrawLeftoverToken - START -> KD
        Recovers any tokens stuck in Contract's balance
        If ownership is renounced then it will not work
        Contract's Address and Owner's address MUST NOT be excluded from reflection reward
     */
    ...
    //New router address availability check function - START -> KD
    function setRouterAddress(address newRouter) external onlyOwner {
    //Thanks to FreezyEx (https://github.com/freezyex)
    ...
    //New router address exist? - END -> KD
}