...
contract BatchAuction is Ownable, ReentrancyGuard {
   ...
   */
  function finalizeAuction(uint256 _minLP) external canFinalizeAuction onlyOwner {
    // The auction was successful, transfer contributed tokens to the auction wallet
  ...
  /**
   * @notice Checks if the auction has been finalized or not
  ...
  function getLockedAmount(address _user) external view returns (uint256) {
    ...
    return COMMITMENTS[_user] - _amount;
  }
  ...
  function getLockedAmountAfterDeposit(address _user, uint256 _deposit) external view returns (uint256) {
    uint256 total = COMMITMENTS[_user] + _deposit;
    return total - _withdrawableAmountAfterDeposit(_user, _deposit);
  }
  ...
  function _addCommitment(address _user, uint256 _amount) internal isAuctionWindow {
    ...
    if ((COMMITMENTS[_user] + _amount) > MAX_COMMITMENTS[_user]) {
       MAX_COMMITMENTS[_user] = COMMITMENTS[_user] + _amount;
    }
  ...
  function _withdrawCommitment(address _user, uint256 _amount) internal isAuctionWindow {
    ...
    COMMITMENTS[_user] -= _amount;
    ACCUMULATED_WITHDRAW[_user] += _amount;
    auctionData.totalCommitments -= _amount;
  ...
  function _withdrawableAmountAfterDeposit(address _user, uint256 _amount) internal view returns (uint256)  {
    ...
    if ((COMMITMENTS[_user] + _amount) > max_withdraw) {
      max_withdraw = (COMMITMENTS[_user] + _amount);
    }

    uint256 max_withdraw_update = _withdrawableAmountPerCommitments(max_withdraw);
    return max_withdraw_update - ACCUMULATED_WITHDRAW[_user];
  }
   ...
   * @param _minLP minimum LP value expected to be minted
   * @return (LP Token Address, LP Token Amount)
   */
  function _setupPancakeSwapPool(
    ...
    IERC20(_token1).safeTransfer(lpAddress, _token1Amount);

    if(_token2 == WETH_ADDRESS) {
      // If second token is WETH_ADDRESS
      IWETH(_token2).deposit{value: _token2Amount}();
      assert(IWETH(_token2).transfer(lpAddress, _token2Amount));
    } else {
      // If the second token is ERC20 (BEP20 in BSC Network)
      IERC20(_token2).safeTransfer(lpAddress, _token2Amount);
    }

    ///@dev mint LP token
    LPTokenAmount = IPancakePair(lpAddress).mint(address(this));

    require(_minLP <= LPTokenAmount, "LOWER_THAN_EXPECTED_LP_AMOUNT");

    emit PancakeSwapPoolCreated(lpAddress, _token1Amount, _token2Amount, LPTokenAmount);
