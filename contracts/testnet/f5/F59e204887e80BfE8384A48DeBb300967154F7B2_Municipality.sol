...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    function setParcelsSoldCountPricing(uint256[] calldata _standardParcelPrices, uint256[] calldata _businessParcelPrices) external onlyOwner {
    ...
    function setBundles(BundleInfo[6] memory _bundles) external onlyOwner {
        bundles = _bundles;
    }

    function setNFTContractAddresses(address[3] memory _nftContractAddresses) external onlyOwner {
    ...
    function setTokensAddresses(address[4] memory _tokenAddresses) external onlyOwner {
    ...
    function setDistributionReciverAddresses(address[7] memory _distributionReciverAddresses) external onlyOwner {
    ...
    function setParcelsSlotsCount(uint8[2] memory _parcelsSlotsCount) external onlyOwner {
    ...
    function setPurchasePrices(uint256[4] memory _purchasePrices) external onlyOwner {
    ...
    function mintParcels(ParcelMintSignature calldata mintingSignature, uint256 _referrerId)
        external
        onlySaleActive
        returns (MintedNFT memory mintInfo)
    { 
        ...
        mintInfo = _mintParcels(mintingSignature.parcels);
    ...
    returns (MintedNFT memory mintInfo) 
    {
        ...
        mintInfo = _mintMiners(_count);

    }

    function purchaseBundle(
        ParcelMintSignature calldata _mintingSignature,
        uint8 _bundleType,
        uint256 _referrerId
    )
    external
    ...
    returns 
    (
        MintedNFT memory parcelMintedNFT, 
        MintedNFT memory minerMintedNFT
    )
    {
        ...
        uint minersAmount = bundle.minersAmount;
        uint minersPurchasePrice = _discountPrice(
        ...
        parcelMintedNFT = _mintParcels(_mintingSignature.parcels);
        minerMintedNFT = _mintMiners(minersAmount);

        uint currentMinerIndex = minerMintedNFT.firstNFTId;
        for(uint parcelIndex; parcelIndex < _mintingSignature.parcels.length; parcelIndex++){
            uint[] memory miners;
            for(uint minerIndex; minerIndex < standardParcelSlotsCount; minerIndex++){
                miners[minerIndex] = currentMinerIndex++;
            }
            _attachMinersToParcel(
                miners, 
                IParcelInterface(standardParcelNFTAddress).getParcelId(
                    _mintingSignature.parcels[parcelIndex].x,
                    _mintingSignature.parcels[parcelIndex].y
                )
            );
        }
    ...
    function attachDetachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) external notZeroAddress {
        ...
        _requireNoElementsDuplication(minersToAttach);
        require(
    ...
    {
        discountedPrice = _price - _price * _percentage / 100;
    }
    ...
    function _validateBundleParcelsAmount
    (
        Parcel[] memory _parcels, 
        uint8 _bundleType
    )
        private
        view
    ...
    function _requireNoElementsDuplication(uint256[] memory _list) private pure {
        for(uint256 firstElementIndex; firstElementIndex < _list.length; firstElementIndex++)
        {
            for(uint256 secondElementIndex = firstElementIndex + 1; secondElementIndex < _list.length; secondElementIndex++)
            {
                require(
                    _list[firstElementIndex] != _list[secondElementIndex],
                    "Municipality:: _requireNoElementsDuplication: Duplicate list element"
                );
            }
        }
    }

    function _requireNewAddressValid(address _oldAddress, address _newAddress) private pure {
        require(
    ...
    function _requireOnlyStandardParcels 
    (
        Parcel[] memory parcels
    )
        private
        pure
    {
        for(uint index; index < parcels.length; index++ ) {
            require(
    ...
    function _attachDetachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) private {
    ...
    function _attachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) private {
        for (uint256 index; index < minersToAttach.length; index++) {
            uint256 tokenId = minersToAttach[index];
            minerParcelMapping[tokenId] = parcelId;
            uint256 minerHashrate = minerIdToMinerInfoMapping[tokenId].hashrate;
            IMining(miningAddress).deposit(msg.sender, tokenId, minerHashrate);
        }
    ...
    function _mintParcels(Parcel[] calldata parcels)
        private
        returns (MintedNFT memory)
    {
        MintedNFT memory mintInfo;
        mintInfo.count = parcels.length;
        for (uint256 i = 0; i < parcels.length; ++i) { 
            Parcel memory parcel = parcels[i];
            require(
            ...
            if(parcel.parcelType == PARCEL_TYPE_STANDARD) {	
                ...
                currentStandardParcelPrice = bsParcelPrice != 0 ? bsParcelPrice : currentBusinessParcelPrice;	
                addr = businessParcelNFTAddress;	
            }	

            uint256 parcelId = IParcelInterface(addr).mint(msg.sender, parcel.x, parcel.y);
            if (i == 0) {
                mintInfo.firstNFTId = parcelId;
            }
    ...
    function _mintMiners(uint256 _count) private returns (MintedNFT memory) {
        MintedNFT memory mintInfo;
...
interface IParcelInterface {
    ...
    function getParcelId(uint _x, uint _y) external pure returns (uint parcelId);
}
