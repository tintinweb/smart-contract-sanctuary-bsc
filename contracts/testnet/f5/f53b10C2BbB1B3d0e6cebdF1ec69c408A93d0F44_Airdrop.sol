...
contract Airdrop is Ownable, ReentrancyGuard {
    ...
    address public devWallet = 0x241A7Ca4145D2065D1076361c1C3927F5F169e1c;
    uint256 public airdropFee = 10**16;
    uint256 public dropersCount;
    uint256 public totalDropped;
    mapping(address => uint256) public dropers;

    function setCoinToken(address _coinToken) external onlyOwner {
    ...
    function setAirdropFee(uint256 _airdropFee) external onlyOwner {
        airdropFee = _airdropFee;
    }

    function airdrop() external payable {
        require(dropers[msg.sender] == 0, "Can Claim only once");
        require(msg.value == airdropFee, "Not enough fee");
        IERC20(coinToken).transfer(msg.sender, airdropAmount);
        dropers[msg.sender] = airdropAmount;
        dropersCount++;
        totalDropped += airdropAmount;
    }
