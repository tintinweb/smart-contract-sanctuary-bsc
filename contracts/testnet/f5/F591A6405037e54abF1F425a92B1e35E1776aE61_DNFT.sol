    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(string => uint256) mergeSetCreatedAt;
    mapping(uint256 => uint256) public nextTimeUsingKey;
    mapping(address => uint256) public nextTimeMerge;

    /*---------- CONSTANTS -----------*/
    uint256 public constant ONE_MONTH = 30 days;
    uint256 public constant SEVEN_DAYS = 7 days;
    uint256 public constant ONE_DAY = 1 days;
    uint256 public constant BUSD_DECIMALS = 1e18;
    uint256 public constant ONE_HUNDRED_PERCENT = 10000;
    uint256 public constant MINT_TAX_RATE = 800; // 8%
    uint256 public constant RESCUE_TAX_RATE = 1200; // 12%
    ...
    modifier inSalePhase() {
        require(block.timestamp <= salePhaseStatistics[currentSalePhase].endTime, "DNFT: NOT IN SALE PHASE");
        require(block.timestamp >= salePhaseStatistics[currentSalePhase].startTime, "DNFT: NOT IN SALE PHASE");
        _;
    }

    modifier claimable() {
        require(block.timestamp >= claimableTime, "DNFT: NOT CLAIMABLE");
        _;
    }
    ...
    function editSalePhase(
        ...
        require(_salePhase >= 1 && _salePhase <= 4, "DNFT: SALE PHASE NOT EXISTED");
        require(_salePhase > currentSalePhase, "DNFT: SALE PHASE IS NOT FUTURE SALE PHASE");
        require(_startTime > block.timestamp, "DNFT: INVALID START TIME");
        require(_endTime > _startTime, "DNFT: INVALID END TIME");
        require(_priceInBUSD > 0, "DNFT: INVALID PRICE");
        require(_priceAfter24Hours > _priceInBUSD, "DNFT: INVALID PRICE AFTER 24 HOURS");
        require(_maxAmountUserCanBuy > 0, "DNFT: INVALID MAX AMOUNT USER CAN BUY");
        ...
        emit SalePhaseEdited(_salePhase, _startTime, _endTime, _priceInBUSD, _priceAfter24Hours, _maxAmountUserCanBuy);
    }

    function setCurrentSalePhase(uint8 _currentSalePhase) external onlyOwner {
        require(_currentSalePhase >= 1 && _currentSalePhase <= 4, "DNFT: SALE PHASE NOT EXISTED");
        require(_currentSalePhase > currentSalePhase, "DNFT: SALE PHASE IS NOT IN FUTURE");
    ...
    function setClaimableTime(uint256 _claimableTime) external onlyOwner {
    ...
    function buyUsingBUSD(
        ...
        require(verifyBuySignature(_salePhase, _to, _signature), "DNFT: INVALID SIGNATURE");
        ...
        require(busdBalanceOfBuyer >= priceInBUSD + taxAmount, "DNFT: INSUFFICIENT BUSD");

        BUSDToken.transferFrom(msg.sender, treasuryAddress, priceInBUSD + taxAmount);
    ...
    function buyUsingBNB(
        ...
        uint256 priceInBNB = convertBUSDToBNB(priceInBUSD);
        require(msg.value >= priceInBNB, "DNFT: INSUFFICIENT BNB");
        payable(treasuryAddress).transfer(msg.value);

        uint256 taxAmount = getMintTaxAmount(priceInBUSD);

        uint256 busdBalanceOfBuyer = BUSDToken.balanceOf(msg.sender);
        require(busdBalanceOfBuyer >= taxAmount, "DNFT: INSUFFICIENT BUSD");
    ...
    function rescueUsingKey(uint256 _keyTokenId) external whenNotPaused hasTreasuryAddress nonReentrant {
        uint256 unSoldToken = getUnSoldTokenOfAllPhase();
        require(totalRescued < unSoldToken, "DNFT: ALL TOKENS ARE SOLD OR RESCUED");
        require(block.timestamp >= claimableTime, "DNFT: NOT YET RESCUE TIME");
        ...
        require(BUSDBalanceOfBuyer >= rescuePrice + taxAmount, "DNFT: INSUFFICIENT BUSD");
        BUSDToken.transferFrom(msg.sender, treasuryAddress, rescuePrice + taxAmount);
    ...
    function claimPurchasedToken(
        address _to,
        uint256 _amountOfToken
    ) external whenNotPaused claimable nonReentrant {
        uint256 claimableAmount = getClaimableAmountOfBuyer(msg.sender);
        require(claimableAmount >= _amountOfToken, "DNFT: INSUFFICIENT CLAIMABLE AMOUNT");
    ...
    /*---------- MERGE FUNCTIONS -----------*/
    function temporaryMerge(uint256[] memory _tokenIds, string memory _sessionId) external whenNotPaused nonReentrant {
    ...
    function cancelTemporaryMerge(uint256[] memory _tokenIds, string memory _sessionId)
        external
        whenNotPaused
        nonReentrant
    {
        string memory mergeSet = generateMergeSet(_tokenIds);
        require(mergeSetCreatedAt[mergeSet] > 0, "DNFT: MERGE SET NOT FOUND");
        require(block.timestamp <= mergeSetCreatedAt[mergeSet] + ONE_MONTH, "DNFT: CAN NOT CANCEL MERGE SET");
    ...
    function executeTemporaryMerge(uint256[] memory _tokenIds) external whenNotPaused nonReentrant {
        string memory mergeSet = generateMergeSet(_tokenIds);
        require(mergeSetCreatedAt[mergeSet] > 0, "DNFT: MERGE SET NOT EXIST");
        uint256 totalTokens = _tokenIds.length;
        for (uint256 i = 0; i < totalTokens; i++) {
            require(ownerOf(_tokenIds[i]) == msg.sender, "DNFT: NOT OWNER");
            require(isLocked[_tokenIds[i]], "DNFT: TOKEN NOT LOCKED");
    ...
    function permanentMerge(uint256[] memory _tokenIds) external whenNotPaused nonReentrant {
        uint256 totalTokens = _tokenIds.length;
        for (uint256 i = 0; i < totalTokens; i++) {
            require(ownerOf(_tokenIds[i]) == msg.sender, "DNFT: NOT OWNER");
            require(!isLocked[_tokenIds[i]], "DNFT: TOKEN IS LOCKED");
    ...
    function verifyBuySignature(
        ...
        return (Whitelist.verifySignatureWhenBuyDNFT(_salePhase, signer, _to, _signature));
    }
    ...
    function getRescueTaxAmount() private view returns (uint256) {
        require(launchPrice > 0, "DNFT: LAUNCH PRICE NOT SET");
        return (launchPrice * RESCUE_TAX_RATE) / ONE_HUNDRED_PERCENT;
    }

    function getMintTaxAmount(uint256 _priceInBUSD) private pure returns (uint256) {
    ...
    function safeTransferFrom(
        ...
        require(BUSDToken.balanceOf(from) >= taxAmount, "DNFT: NOT ENOUGH BUSD");
    ...
    /*---------- GETTERS -----------*/
    function getUnSoldTokenOfAllPhase() public view returns (uint256) {
