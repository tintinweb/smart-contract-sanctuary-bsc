...
contract LyLevel is Initializable, OwnableUpgradeable, IERC20 {
    ...
    function getNextBatch() external view returns (uint256 _nextBatchTimestamp, uint256 _nextBatchAmount) {
        _nextBatchTimestamp = nextBatchTimestamp;
        _nextBatchAmount = nextBatchAmount;
    }
    ...
    function setDistributor(address _distributor) external onlyOwner {
        require(_distributor != address(0), "LyLevel: zero address");
        distributor = _distributor;
        emit DistributorSet(_distributor);
    }

    function setNextBatch(uint256 _nextBatchTimestamp, uint256 _nextBatchAmount) public onlyDistributor {
        nextBatchTimestamp = _nextBatchTimestamp;
        nextBatchAmount = _nextBatchAmount;
        emit NextBatchSet(_nextBatchTimestamp, _nextBatchAmount);
    }

    function allocateNextBatch() external onlyDistributor {
        require(nextBatchTimestamp > 0, "Invalid trigger time");
        require(nextBatchAmount > 0, "Invalid amount");
        require(block.timestamp >= nextBatchTimestamp, "now < trigger_time");
        BatchInfo memory newBatch = BatchInfo({
            totalBalance: totalSupply(),
            rewardPerShare: nextBatchAmount * PRECISION / totalSupply(),
            allocatedTime: block.timestamp
        });
        batches[currentBatchId] = newBatch;
        emit RewardAllocated(currentBatchId, nextBatchAmount);
    ...
    event NextBatchSet(uint256 nextBatchTimestamp, uint256 nextBatchAmount);
