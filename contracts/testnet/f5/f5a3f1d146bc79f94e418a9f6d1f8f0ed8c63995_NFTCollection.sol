...
// File @openzeppelin/contracts/token/ERC721/IERC721R[email protected]
...
contract NFTCollection is ERC721A, ERC2981, Ownable {
    ...
    string baseURISuffix; //Set the base URI Suffix
    uint96 royalty; //Set the Royalty
    uint256 whitelistedFee = 400; //Set the whitelisted Fee
    uint256 whitelistedQuantity = 50; //Set the whitelisted Quantity
    uint256 mintFee = 600; //Set the mint Fee
    uint256 Quantity = 10; //Set the Quantity
    mapping(uint256 => bool) public proceedNonce;
    address private receiver; //address of Receiver
    address private constant platformPubKey =
        0x6A50f49DE5fe0f526550d46c88E2EBE52BEaE426;
    uint256 brokerage;
    ...
    function verifyWhiteListUser(
        ...
        bytes32 hash = keccak256(abi.encodePacked(whitelistUser,nonce,mintTime,isWhiteListed));
    ...
    function mint(
        ...
        require(
            quantity <= maxSupply,
            "NFTCollection: Max supply should be less"
        );
        require(!proceedNonce[nonce], "NFTCollection: Nonce already proceed !");
        require(
            block.timestamp > mintTime,
            "NFTCollection: Mint Time should be less"
        );

        address verifiedUser = verifyWhiteListUser(
                msg.sender,
                nonce,
                mintTime,
                isWhiteListed,
        ...
        if (
            platformPubKey ==verifiedUser
             &&
            isWhiteListed
        ) {
            require(
                msg.value >= whitelistedFee * quantity,
                "NFTCollection: You don't have enough amount to mint."
            );
        } else if (
            platformPubKey == verifiedUser
        ) {
        ...
        require(
            amount <= WBNB.balanceOf(msg.sender),
            "NFTCollection: Insufficient fund"
        );
        require(
            WBNB.allowance(msg.sender, address(this)) >= amount,
            "NFTCollection: Check allowance"
        );
        if (brokerage > 0) {
            uint256 brokerageAmount = (amount * brokerage) /
                (100 * DECIMAL_PRECISION);
            WBNB.transferFrom(msg.sender, brokerAddress, brokerageAmount);
    ...
    function withdrawMintFee(address _receiver) external onlyOwner {

        WBNB.transfer(_receiver, WBNB.balanceOf(address(this)));
    }
...
// File contracts/libraries/AddressArrayLib.sol
...
// File contracts/NFTLaunchPad.sol
...
contract NFTLaunchPad is Initializable, OwnableUpgradeable {
    ...
    AddressArrayLib.addresses private _allCollections;
    mapping(address => AddressArrayLib.addresses) _userCollections;
    uint256 brokerage;
    address brokerAddress;
    IWBNB WBNB;
     ...
     * @param amount of new brokerage.
     */

    function setBrokerage(uint256 amount) external onlyOwner {
        brokerage = amount;
    }
     ...
     * @notice allow only autorized user to call this function.
     * @param newBrokerAddress of new broker.
     */
    ...
    /**
     * @dev Method to create LaunchPad.
     * @notice This method used to create LaunchPad.
     * @param _name Name of the collection.
     * @param _symbol Symbol of collection.
     * @param _maxSupply Set the maximum supply.
     * @param _baseURI Set base URI.
     * @param _uriSuffix Set URISuffix.
     * @param _royalty Set royalty for the token.
     * @param _reciever User who will get the royalty.
     * @return Return the new created collection address.
     */
    ...
    function createLaunchPad(
        ...
        _allCollections.add(address(launchpadCollection));
        _userCollections[msg.sender].add(address(launchpadCollection));
    ...
    {
        return _userCollections[user].array;
    }
}