...
contract Betlify is VRFConsumerBaseV2, ReentrancyGuard {
  ...
  uint256 public totalLostLockedValue;
  ...
  */
  ...
  function autoStake(address _playerAddress, uint256 _amount) internal {
    if(userStakes[_playerAddress] == 0){
        lastReawardClaimedTime[_playerAddress] = block.timestamp;
    }
  ...
  function unstake(uint256 _amount) public nonReentrant {
      ...
      require(userLostBalance[msg.sender] <= _amount,"You can not unstake lost locked stakes" );
      
      uint256 lastStakeDuration = (block.timestamp - lastStakeTime[msg.sender]) / 1 days;
      require(lastStakeDuration > stakeLockDuration,"You can not unstake until minimum lock duration is over");
      
      userStakes[msg.sender] -= _amount;
      totalStakeLockedValue -= _amount;

      uint256 unstakableAmount = (_amount - (_amount * getTotalFees())/100);

      (bool success, ) = msg.sender.call{value: unstakableAmount}("");
      require(success, "Transfer failed.");
  }

  function claimRewards() public nonReentrant {
    uint256 claimableRewards = getUnclaimedRewards();
    userStakes[msg.sender] -= claimableRewards;

    if(claimableRewards <= userLostBalance[msg.sender]){
        userStakes[msg.sender] -= claimableRewards;
        userLostBalance[msg.sender] -= claimableRewards;
        totalLostLockedValue -= claimableRewards;
    }else{
        userLostBalance[msg.sender] = 0;
    }

    //userLostBalance[playerAddress] = 
    //uint256 claimableRewards = 0.0001 ether;
    lastReawardClaimedTime[msg.sender] = block.timestamp;
  ...
  */
  ...
  function setUserStake(uint256 _amount) public {
    ...
    function setLastStakeTime(uint256 _timestamp) public {
        lastStakeTime[msg.sender] = _timestamp;
    }
    ...
    function getAPR() public view returns(uint256){
        if((totalStakeLockedValue > totalLostLockedValue) && (rewardPoolBalance >= totalStakeLockedValue)){
            return (((rewardPoolBalance * magnitude) / (totalStakeLockedValue-totalLostLockedValue))*100)/magnitude;
        }else{
            return minAPR;
        }
    }
