...
contract Test is IERC20, Ownable {
    ...
    // Fees
    uint256 public rewardFee = 300;
    uint256 public marketingAndDAOFee = 100;
    uint256 public donationFee = 50;
    ...
    // Because people don't like waiting too long to get their dividends,
    // and we want to avoid massive red candle from a contract swap,
    // trigger a contract swap when an hour has passed to deploy,
    // or the amount of tokens exceeds the contract swap threshold.
    ...
    function doContractSwap() internal swapping {
        ...
        uint256 busdForReward = (swappedTokensAmount * rewardFee) / totalFee;

        uint256 busdForMarketingAndDAO = (swappedTokensAmount *
            marketingAndDAOFee) / totalFee;

        uint256 busdForDonation = (swappedTokensAmount * donationFee) /
            totalFee;

        uint256 busdForDevelopment = swappedTokensAmount -
            busdForDonation -
