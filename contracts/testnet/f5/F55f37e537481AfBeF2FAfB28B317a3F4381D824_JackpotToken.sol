...
contract JackpotToken is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "JCK";
    uint8 private constant _decimals = 18;
    uint256 public constant maxTokenSupply = 10000000 * 10**_decimals;
    uint256 private _tTotal = 0;

    // Max wallet size initially set to 1%
    uint256 public maxWalletSize = maxTokenSupply.div(100);
    ...
    uint256 presaleMaxAllowance = 250 * 10**USDT_DECIMALS;
    uint256 totalPresaleBuy;
    uint256 presaleMinBuy = 50 * 10**USDT_DECIMALS;
    /// 1 JCK token = 0.10$ for presale
    uint256 presaleTokenPriceStable = 10;
    mapping(address => uint256) userAmountBuyPresale;
    bool presaleOpen = false;

    //// TODO : change fees depending on the context
    ...
    address private _liquidityPresaleWallet;
    /// TODO : possibility to change it
    ...
    uint256 public maxTxAmount = maxTokenSupply;
    ...
    constructor(address cOwner, address presaleWallet) Ownable(cOwner) {

        
        _liquidityPresaleWallet = presaleWallet; 
    ...
    function liquidityPresaleWallet() public view returns (address) {
        return _liquidityPresaleWallet;
    }

    function launchJackpotGame() external onlyOwner {
        require(presaleOpen == false, "Presale open, close it before launch");
        _mint(owner(), maxTokenSupply.sub(totalPresaleBuy));
    }
    ...
    function presaleBuy(bytes32 merkleTreeRoot, uint256 amountStable) public whenPresaleOpen {
        
        if(userAmountBuyPresale[_msgSender()] > 0){
            require(userAmountBuyPresale[_msgSender()] <= presaleMaxAllowance, "Wrong amount Buy");
        }
        else {
            require(amountStable >= presaleMinBuy, "Wrong amount Buy");
        }
        /// TODO : require merkleTree
        userAmountBuyPresale[_msgSender()] += amountStable;
        uint256 amountInToken = amountStable.mul(presaleTokenPriceStable);
        IERC20(USDT).transferFrom(_msgSender(), _liquidityPresaleWallet, amountStable);
        _mint(_msgSender(), amountInToken);
