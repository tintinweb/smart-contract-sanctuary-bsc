...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    function openTrade(uint256 _id, uint256 _price) public {
        ...
        NFT(nftContract).approve(address(this), _id);
        NFT(nftContract).transferFrom(msg.sender, address(this), _id);
        listedMap[_id] = true;
        price[_id] = _price;
    }
    ...
    function closeTrade(uint256 _id) public {
        ...
        NFT(nftContract).transferFrom(address(this), msg.sender, _id);
        listedMap[_id] = false;
    }
    ...
    function _validate(uint256 _id) internal view {
        ...
        require(
            msg.sender != NFT(nftContract).ownerOf(_id),
    ...
    {
        require(
            msg.sender == NFT(nftContract).ownerOf(_tokenId),
            "Error, you are not the owner"
        );
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if (winner == address(0)) {
            NFT(nftContract).transferFrom(address(this), Creator, tokenId);
            ownerMap[tokenId] = Creator;
        } else {
            NFT(nftContract).transferFrom(address(this), winner, tokenId);
    ...
    function mint(
        ...
        NFT(nftContract).mint(newTokenId, msg.sender, _tokenURI);
        ...
        if (_isListOnMarketplace)
            createAuction(newTokenId, startDate, auctionLength, _price);
    }
