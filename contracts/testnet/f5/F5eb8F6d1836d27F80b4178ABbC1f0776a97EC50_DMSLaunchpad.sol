...
contract DMSLaunchpad is ReentrancyGuard, Pausable, AccessControl {
    ...
    mapping(address => OwnedPackage) ownedPackage;
    ...
    constructor(address _dmstoken, address _locker) {
    ...
    function addTotalSupplyinPackage(uint256 _packageIndex, uint256 _totalSupply)
        public
        whenNotPaused
    {
        packages[_packageIndex].totalSupply = packages[_packageIndex].totalSupply + _totalSupply;
    }
    ...
    function buy(
        ...
        ownedPackage[msg.sender] = OwnedPackage(
            _packageIndex,
            false,
            false,
            false
        ...
        packages[_packageIndex].totalSupply = packages[_packageIndex].totalSupply - 1;
    ...
    function getPackageIdbyOwner(address _owner) public view returns (uint256) {
        uint256 _packageId = ownedPackage[_owner].packageId;
        return _packageId;
    }

    function getClaimStatus(address _owner, string memory _assetName)
        public
        view
        returns (bool)
    {
        if (
            ...
            return ownedPackage[_owner].isClaimMonster;
        } else if (
            keccak256(abi.encodePacked(_assetName)) ==
            keccak256(abi.encodePacked(("land")))
        ) {
            return ownedPackage[_owner].isClaimLand;
        } else if (
            keccak256(abi.encodePacked(_assetName)) ==
            keccak256(abi.encodePacked(("item")))
        ) {
            return ownedPackage[_owner].isClaimItem;
        }
        return false;
    }
    ...
    function claimAsset(
        ...
        require(_owner == msg.sender, "claimAsset: Only owner can claim asset");
        require(
            _packageId == ownedPackage[_owner].packageId,
            "claimAsset: User didn't but the package"
        );
        ...
        if (
            ...
            ownedPackage[_owner].isClaimMonster = true;
        } else if (
            keccak256(abi.encodePacked(_assetName)) ==
            keccak256(abi.encodePacked(("land")))
        ) {
            ownedPackage[_owner].isClaimLand = true;
        } else if (
            keccak256(abi.encodePacked(_assetName)) ==
            keccak256(abi.encodePacked(("item")))
        ) {
            ownedPackage[_owner].isClaimItem = true;
        }
    }
}
