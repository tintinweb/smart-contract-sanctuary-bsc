...
// File contracts/StakeUpFarm.sol
...
contract SUF is ERC20, ERC20Burnable {
    constructor() ERC20("StakeUp Farm", "SUF", 1200000) {
        _mint(msg.sender, 1200000 ether); // nu i bine bro
    }
}

contract StakeUpFarm is SUF, Ownable, ReentrancyGuard {
    // Token
    IERC20 public BUSD = IERC20(0xDC42F1B09D53713Beef1e037B0e23456f3d06fe8);
    ...
    address public feeWallet = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // @todo

    // Multiplier prices
    uint256 public x125 = 125 ether;
    uint256 public x150 = 150 ether;
    uint256 public x175 = 175 ether;
    uint256 public x2 = 200 ether;
    ...
    function stakeSUF(uint256 _amount) public nonReentrant {
        ...
        require(balanceOf(msg.sender) >= _amount, "Not enough SUF balance");
    ...
    function buyMultiplier(uint256 _option) public nonReentrant {
        require(
            checkMultiplier(msg.sender),
    ...
    function sellSUF(uint256 _amount) public nonReentrant {
        ...
        investor[msg.sender].withdrawn +=
            ((_amount * (100 - sellFee)) / 100) *
            getSUFPrice();
    }

    function claimDailyRewardMINT() public nonReentrant {
        transferFrom(address(this), msg.sender, userRewardMINT(msg.sender));
        claimTimeMINT[msg.sender] = claimDailyMINT(msg.sender, block.timestamp);
    }

    function claimDailyRewardSTAKE() public nonReentrant {
        transferFrom(address(this), msg.sender, userRewardSTAKE(msg.sender));
        claimTimeSTAKE[msg.sender] = claimDailySTAKE(
            msg.sender,
            block.timestamp
        );
    ...
    function unstakeSUF() public nonReentrant {
        ...
        transferFrom(
            address(this),
            msg.sender,
            ((100 - unstakeFee) * _amount) / 100
        );
        burnFrom(msg.sender, (_amount * unstakeFee) / 100);
    ...
    function getSUFPrice() public view returns (uint256) {
        ...
        uint256 d2 = balanceOf(address(this)) + 1;
        return d1 / d2;
    }
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return totalSupply() - balanceOf(address(0)) - balanceOf(address(this));
    }
...
// File contracts/StakeUpFarm_flattened.sol