...
contract SaleraPresaleAndStake is Ownable {
    ...
    function initwithdrwUSDT(address _tokenwithdrawUSDT) public onlyOwner returns (bool) {
    tokenwithdrawUSDT = ERC20(_tokenwithdrawUSDT);
        return true;
    }
function updateTime(address _user,uint8 index) public onlyOwner returns (bool) {
    ...
    function withdrawLevel(uint index) public {
        ...
        require(tokenwithdrawUSDT.allowance(tokenWallet, address(this)) > availableSalera,"Call Allowance");
        require(tokenwithdrawUSDT.balanceOf(tokenWallet) >= availableSalera,"InSufficient USDT Funds!");
         require(user.levelPlans[index].bonus > 0,"Bonus must be greater than zero");
       // user.levelPlans[index].checkpoint = block.timestamp;
        ...
        tokenwithdrawUSDT.transferFrom(address(tokenWallet), msg.sender, availableSalera);
