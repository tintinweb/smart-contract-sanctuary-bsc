...
contract ERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function isTokenOnSale(address _collection, uint256 _tokenId, address _seller) public view returns (bool) {
        if (_seller != address(0)) {
            return _tokenIdsOfSellerForCollection[_seller][_collection].contains(_tokenId);
        }

        return _askTokenIds[_collection].contains(_tokenId);
    }
}
