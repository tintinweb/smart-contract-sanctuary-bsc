...
contract TRDCvault is BEP20 {
    ...
    uint public vVault = 1000 * fractions;
  ...
  modifier onlyOperator(){
      require(gameOperator[msg.sender] == true,"Only game operator!");
      _;
  }  
  ...
  modifier setPower(address _Player){
    ...
    function editThief(uint thiefIndex, string memory _tName, uint _tPower) external onlyOperator{
    ...
    function editCop(uint copIndex, string memory _cName, uint _cPower) external onlyOperator{
  ...
  function getInVaults() public view returns(uint _amount){
      uint amount;
      for(uint i= 0; i<playerCount; i++){
          address _player = PlayerID[i];
          _amount = playerStats[_player].gamerVault;
          amount += _amount;
      }
      return(amount.div(fractions));
  }
  function getBigVault() public view returns(address _player, uint _amount){
      for(uint i=0; i<playerCount; i++){
          _player = PlayerID[i];
          _amount = playerStats[_player].gamerVault;
          if(_amount >= vVault && isShielded[_player] == false){
  ...
  function runCop(uint cardToUse) external returns(uint _toCop, uint _toBurn, uint _toRewards){
      ...
      if(_cPower >= 2){
         ...
         // uint v1; uint v2; uint v3; uint v4;

              //for (uint i=0; i<playerCount; i++){
                 // _pVault = thiefRewards[i].rewardsAmount;
                  //_thief = getBigVault(player);
                  //_pVault = playerStats[_thief].gamerVault;
  ...
  function claimReward() external haveRewards(msg.sender) returns(uint totalAmt){
      uint256 claimTime = endTime.sub(block.timestamp);
      require(gameRunning != false, "Game is paused by owners!");
      require(claimTime <= 1 hours, "not"); //%2 != 0
