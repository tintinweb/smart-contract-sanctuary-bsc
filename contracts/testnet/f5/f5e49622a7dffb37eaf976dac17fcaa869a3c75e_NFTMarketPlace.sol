...
// File: contracts/5_Nft_Marketplace.sol


pragma solidity >=0.4.22 <0.9.0;

interface IASIX_TOKEN {
    function balanceOf(address account) external returns (uint balance);
    function approve(address spender, uint tokens) external returns (bool success);
    function transfer(address recipient, uint256 amount) external returns (bool);
    function transferFrom(address spender, address recipient, uint tokens) external returns (bool success);
}

contract NFTMarketPlace is ReentrancyGuard {
    // uint256   marketFees = 5*10**9;
    ...
    function transferBNB(uint256 value) public payable {
        address recipient = address(msg.sender);
        payable(recipient).transfer(msg.value);
    }

    function calculateBalance(address addr) public returns(uint256) {
        require(addr != address(0), "Invalid Address");
        return IASIX_TOKEN(0x256068c7B4cc265833133AB0651C221ea4A830De).balanceOf(addr);
    }
    ...
    ///////////////////////////////////
    function createItemForSale(address nftContract,uint256 tokenId,uint256 price)public nonReentrant {
    ...
    //Create Maket
    function createMarketForSale(address nftContract,uint256 nftItemId) public nonReentrant {
        ...
        require(calculateBalance(msg.sender) >= price, "Ballance is not enough.");

        ASIX = IASIX_TOKEN(0x256068c7B4cc265833133AB0651C221ea4A830De);
        
        ASIX.transferFrom(msg.sender, address(this), price);

        ASIX.approve(idForMarketItem[nftItemId].seller, price);

        ASIX.transferFrom(address(this), idForMarketItem[nftItemId].seller, price);
        ...
        // ASIX.transferFrom(address(this), owner, marketFees);
