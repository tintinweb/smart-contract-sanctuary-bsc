...
contract SPRO is ERC20Detailed, Ownable {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 150 * 10**4 * 10**DECIMALS;
    ...
    uint256 private constant MAX_SUPPLY = 150 * 10**5 * 10**DECIMALS;
    uint256 public maxSellTransactionAmount = 50000 * 10 ** DECIMALS;

    uint256 public rebaseCycle = 60; // minutes unit
    uint256 public rebaseRateFirstYear = 94729; // minutes unit 
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountTofired = amountToSwap.mul(firePitFee).div(treasuryFee.add(insuranceFundFee).add(firePitFee));
        _basicTransfer(address(this), DEAD, amountTofired);

        amountToSwap = amountToSwap.sub(amountTofired);
        ...
        uint256 amountBUSDToTreasuryAndSIF = IERC20(busdToken).balanceOf(address(this)).sub(balanceBusdBefore);

        // send BUSD to treasury receier
        IERC20(busdToken).transferFrom(address(this), treasuryReceiver, amountBUSDToTreasuryAndSIF.mul(treasuryFee).div(treasuryFee.add(insuranceFundFee)));
        // send BUSD to insurance receiver
        IERC20(busdToken).transferFrom(address(this), insuranceFundReceiver, amountBUSDToTreasuryAndSIF.mul(insuranceFundFee).div(treasuryFee.add(insuranceFundFee)));
