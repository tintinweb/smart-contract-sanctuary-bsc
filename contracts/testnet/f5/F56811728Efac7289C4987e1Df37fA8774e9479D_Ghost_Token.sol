...
contract GOKDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("GOK_Dividend_Tracker", "GOK_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 100000000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "GOK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "GOK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FCF contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "GOK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "GOK: Cannot update claimWait to same value");
...
contract Ghost_Token is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    GOKDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GHOST TOKEN", "GOK") {
        BNBRewardsFee = 6;
        liquidityFee = 2;
        extraFeeOnSell = 5;
        marketingFee = 2;
        marketingWallet = payable(0xC3209b1E8FF9A5F3435e9CAc2D79e325448A2e4D);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new GOKDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "GOK: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "GOK: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "GOK: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "GOK: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 750000, "GOK: gasForProcessing must be between 200,000 and 750,000");
        require(newValue != gasForProcessing, "GOK: Cannot update gasForProcessing to same value");
