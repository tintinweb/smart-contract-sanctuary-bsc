...
contract SellBox is Ownable {
    ...
    uint public goldId;
    mapping(address => bool) public isBuyGolds;
    address public ceo;
    ...
    constructor(IRefferal _refer, IERC1155 _box, IERC20 _tokenBuy, address _ceo, uint[] memory _tokenIds, uint[] memory _prices, uint _goldId) {
        refer = _refer;
        box = _box;
        tokenBuy = _tokenBuy;
        ceo = _ceo;
    ...
    function buy(uint _tokenId, uint _amount) external {
        ...
        uint commPercent = commTreasury.commPercent(_msgSender());
        uint comm = totalBill * commPercent / 100;

        box.safeTransferFrom(ceo, _msgSender(), _tokenId, _amount, '0x');
        remains[_tokenId] -= _amount;
        if(_tokenId == goldId) isBuyGolds[_msgSender()] = true;
