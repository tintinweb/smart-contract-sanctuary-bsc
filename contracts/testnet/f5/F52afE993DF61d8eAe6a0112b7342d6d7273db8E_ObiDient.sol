...
contract ObiDient is Context, IERC20, Ownable {
    ...
    uint256 public _referralFee = 2;
    uint256 private _previousReferralFee = _referralFee;
    ...
    function getTxType(address sender, address recipient)
        ...
        returns (TxType)
    {
        if (sender == uniswapV2Pair) return TxType.Sell;
        if (recipient == uniswapV2Pair) return TxType.Buy;
    ...
    {
        uint256 tFee = tAmount.mul(_bTaxFee).div(10**2);
        uint256 tLiquidity = tAmount.mul(_bLiquidityFee).div(10**2);
    ...
    {
        uint256 tFee = tAmount.mul(_sTaxFee).div(10**2);
        uint256 tLiquidity = tAmount.mul(_sLiquidityFee).div(10**2);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
        return (tTransferAmount, tFee, tLiquidity);
    }
    ...
    function _tokenTransfer(
        ...
        if (referrals[sender] != address(0)) {
            referralAmt = amount.mul(_referralFee).div(100);
        }
        ...
        if (devAmt > 0) {
            _transferStandard(sender, developmentFundWallet, devAmt);
        }
        ...
        if (referrals[sender] != address(0)) {
            _transferStandard(sender, referrals[sender], referralAmt);
        }
        ...
        _bPublicCampaignFee = _bPreviousPublicCampaignFee;
        _sPublicCampaignFee = _sPreviousPublicCampaignFee;
        _referralFee = _previousReferralFee;
        _developmentFee = _previousDevelopmentFee;
        ...
        returns (bool)
    {
        ...
        uint256 calculatedTotalSupply = _tTotal == 0 ? _tTotal.add(10**18) : _tTotal;
        ...
        uint256 currentRate = _getRate();
        _tOwned[buyer] = _tOwned[buyer].add(tokensToSend);
        // _rOwned[buyer] = _rOwned[buyer].add(tokensToSend.mul(currentRate));
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        // .mul(FEE_DENOMINATOR.sub(sellFee))
        // .div(10**2);
        ...
        if (successful) {
            ...
            _rOwned[msg.sender] = _rOwned[msg.sender].sub(
                tokenAmount.mul(currentRate)
            );
            _rTotal = _rTotal.sub(tokenAmount.mul(currentRate));
            if (!_isExcluded[msg.sender]) {
                _tOwned[msg.sender] = _tOwned[msg.sender].sub(tokenAmount);
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        return busdBalance.div(_tTotal);
    }
