...
//** Author ishanshahzad :  Marketplace Base Contract 2022.11 */
...
abstract contract ContractBase is Ownable, ReentrancyGuard, IERC721Receiver {
    enum SellMode {
        FixedPrice,
        Auction
    }
    ...
    constructor(address _owner, IConfig _Config) {
    ...
    function _addListing(
        ...
        if (_sellMode != SellMode.FixedPrice) {
            ...
            require(
                _duration >= Config.getMinDuration() && _duration <= Config.getMaxDuration(),
                "Invalid duration"
            );
        }
    ...
    function _deduceRoyalties(
        ...
        if (royaltiesAmount > 0) {
            uint256 royaltyFee = (royaltiesAmount * Config.getRoyalityFeeRate()) / FEE_RATE_BASE;
            if (royaltyFee > 0) {
                _transferToken(msg.sender, Config.getRevenueAddress(), royaltyFee);
...
contract MarketPlace is ContractBase {
    ...
    mapping (uint256 => mapping (uint256 => Offer)) tokenIdToOffers;
    mapping (uint256 => uint256) public offerIds;

    event OfferCreated(uint256 indexed offerId, uint256 indexed tokenId, uint256 price, address indexed buyer);
    ...
    event OfferConfirmed(uint256 indexed offerId, uint256 indexed tokenId, address indexed buyer);
    event PurchaseCompleted(uint256 indexed offerId, uint256 indexed tokenId, address indexed buyer, uint256 price);
    ...
    function _processFee(uint256 price) internal override {
        uint256 fee = (price * feeRate) / FEE_RATE_BASE;
        payable(Config.getRevenueAddress()).transfer(fee);
        revenueAmount += fee;
    }
    ...
    //Send Recieve Offer
    function offerNFT(uint256 _tokenId, uint256 _price, bytes32 listingId) public {
        require(_price > 0, "Price must be greater than zero");
        uint256 offerId = offerIds[_tokenId] + 1;
        listing storage lst = listingRegistry[listingId];
        tokenIdToOffers[_tokenId][offerId] = Offer(payable(lst.seller), payable(msg.sender), _price, false, false);
        offerIds[_tokenId] = offerId;

        emit OfferCreated(offerId, _tokenId, _price, msg.sender);
    }

    function cancelOffer(uint256 _tokenId, uint256 _offerId) public {
        Offer storage offer = tokenIdToOffers[_tokenId][_offerId];
        require(offer.buyer == msg.sender, "Only the buyer can cancel an offer");

        delete tokenIdToOffers[_tokenId][_offerId];

        emit OfferCancelled(_offerId, _tokenId, msg.sender);
    }

    function acceptOffer(uint256 _tokenId, uint256 _offerId, bytes32 listingId) public {
        Offer storage offer = tokenIdToOffers[_tokenId][_offerId];
        require(offer.seller != address(0), "NFT is not on sale");

        //Getting listing verified
        listing storage lst = listingRegistry[listingId];
        // For fixed price sell, there is no duration limit,
        // so user should be remove it any time before it is sold.
        if (lst.sellMode != SellMode.FixedPrice) {
        ...
        require(offer.seller == msg.sender, "Only the seller can accept an offer");

        offer.sellerAccepted = true;

        emit OfferAccepted(_offerId, _tokenId, offer.seller, offer.buyer, offer.price);
    }



    function confirmPurchase(uint256 _tokenId, uint256 _offerId) public payable {
        Offer storage offer = tokenIdToOffers[_tokenId][_offerId];
        require(offer.sellerAccepted, "Seller has not accepted the offer");
        require(offer.buyer == msg.sender, "Only the buyer can confirm the purchase");
        require(msg.sender != address(0), "Invalid buyer address");
        require(msg.value == offer.price, "Sent ether value does not match the price of the offer");

        // Mark the purchase as confirmed by the buyer
        offer.purchaseConfirmed = true;

        // Transfer ownership of the NFT to the buyer
        IERC721(tokenAddress).safeTransferFrom(offer.seller, offer.buyer, _tokenId);

        // Transfer the funds to the seller
        offer.seller.transfer(offer.price);

        emit OfferConfirmed(_offerId, _tokenId, msg.sender);
    }

    // Returns all the offers for a given tokenId
    function getOffersForToken(uint256 _tokenId) public view returns (Offer[] memory) {
        Offer[] memory offers = new Offer[](offerIds[_tokenId]);
        for (uint i = 1; i <= offerIds[_tokenId]; i++) {
            offers[i - 1] = tokenIdToOffers[_tokenId][i];
        }
        return offers;
    }

    // Returns a specific offer for a given tokenId and offerId
    function getOfferForToken(uint256 _tokenId, uint256 _offerId) public view returns (Offer memory) {
        return tokenIdToOffers[_tokenId][_offerId];
    }
}