...
contract CryptosTribeGame is Ownable, Pausable, IGAME {
    ...
    mapping(uint256 => mapping(address => EnumerableSet.UintSet)) private lotteryStatus;
    ...
    function openRandomBox(uint256 num) external whenNotPaused returns (bool) {
        ...
        IERC20(token).transferFrom(from, address(0), amountOut[1]);
    ...
    function giveOutBonuses(address account, uint256 amount)
        public
        onlyOwner
        whenNotPaused
    {
        require(
            IERC20(token).balanceOf(pveBonusesPool) >= amount,
        ...
        // burn
        uint256 burnAmount = fee - bonusesPoolAmount;
        IERC20(token).transferFrom(pveBonusesPool, address(0), burnAmount);

        emit Bonuses(pveBonusesPool, bonusesPoolAmount, block.timestamp);
    ...
    function lotteryBonus(
        ...
        require(addrList.length == amountList.length, "Not the same length");
        for (uint256 i = 0; i < addrList.length; i++) {
            address to = addrList[i];
            uint256 amount = amountList[i];

            require(lotterWinnings >= amount, "Bonus amount not enough");
            ...
            uint256 bonusesPoolAmount = calculateFee(
                fee,
                lotteryBonusesPoolFee
            );
            uint256 burnAmount = fee - bonusesPoolAmount;
            
            IERC20(token).transferFrom(lotteryBonusesPool, address(0), burnAmount);

            lotterWinnings -= amount;

            emit Transfer(lotteryBonusesPool, address(0), burnAmount);
        }

        if (status) {
            lotteryNumber += 1;

            IERC20(token).transferFrom(lotteryBonusesPool, address(0), lotterWinnings);

            emit Transfer(lotteryBonusesPool, address(0), lotterWinnings);
