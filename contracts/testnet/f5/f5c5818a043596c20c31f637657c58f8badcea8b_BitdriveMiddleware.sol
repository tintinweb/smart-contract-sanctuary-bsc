...
contract BitdriveMiddleware is ContextUpgradeable, OwnableUpgradeable {
    ...
    function swapExactETHForTokensMiddleware(
        ...
        if (inputType == 1) {
            ...
            IWETH(inputToken).deposit{value: inputAmount}();
            assert(IWETH(inputToken).transfer(address(this), inputAmount));
            ...
            IWETH(inputToken).deposit{value: calculation.totalAmount}();
            assert(
                IWETH(inputToken).transfer(
                    address(this),
