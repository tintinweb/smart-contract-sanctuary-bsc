...
//tokeneconomist means calculation of token transfer

contract WEMOVE is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    
    string private constant _name = "WEMOVE";
    string private constant _symbol = "WEMOVE";
    ...
    address payable private _marketingAddress = payable(0xFe98652eA9Dc96b2CF69b1c60980D2B118E1F991);
    address payable private _teamAddress = payable(0x0F4783E31c4D7067A339DD48927b997B19b79aa3);
    address payable private _devAddress = payable(0x2a3FDb38E2CC99B1e66282E44ffC1baBF8a6dFFe);
...
//this function is used to restore all the fees
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        //if sender is excluded and recipient is not! then call _transferFrom Excluded
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        //Calculation : is sender is excluded and recipient is not then call this method.
        //
    ...
    function _getValues(uint256 tAmount) public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        // 20  + 15 + 3 = 38
        uint256 taxFee = _marketingFee.add(_liquidityFee).add(_teamFee).add(_devFee);//38
        (uint256 tTransferAmount, uint256 tRfi, uint256 tTax) = _getTValues(tAmount, _rfiFee, taxFee);//0.61,trfi = 0.01 , ttax = 0.38
        uint256 currentRate = _getRate();//1488755433066095781205145050780895980049851619787
        ...
        //return the t amount
        function _getTValues(uint256 tAmount, uint256 rfiFee, uint256 taxFee) public view returns (uint256, uint256, uint256) {
            //tamount = rfiFee - taxFee
            uint256 tRfi = tAmount.mul(rfiFee).div(1000);//
            uint256 tTax = tAmount.mul(taxFee).div(1000);//0.38
        ...
        function _getRValues(uint256 tAmount, uint256 tRfi, uint256 tTax, uint256 currentRate) public view returns (uint256, uint256, uint256) {
        ...
        function _getRate() public view returns(uint256) {
            ...
            return rSupply.div(tSupply);//1488755433066095781205145050780895980049851619787
        }

        function _getCurrentSupply() public view returns(uint256, uint256) {
            uint256 rSupply = _rTotal;//
            uint256 tSupply = _tTotal;//7777
            for (uint256 i = 0; i < _excluded.length; i++) {
