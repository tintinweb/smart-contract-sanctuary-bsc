...
interface IAgeioController {
  ...
  function swapAgtAndBurn(uint256 amount) external payable returns (bool);
...
contract AWThetaToken is Ownable, IERC20, IERC20Metadata {
  ...
  */
  ...
  function balanceByRound(uint256 rId, address account) public view returns(uint256 balance, uint256 balanceInAMM, uint256 rewardEarned, uint256 rewardEarnedInAMM) {
    ...
    rewardEarned = 0;
    rewardEarnedInAMM = 0;
    if (rId == 0 || rId > roundIndex) return (0, 0, 0, 0); 
    if (balance > 0) {
      rewardEarned = rewardEarned.add(balancesByRound[account][rId].mul(rounds[rId].tfuelShare).div(1e12));

      for(uint256 i=0;i<listAMM.length;i++) {
        balanceInAMM = balanceInAMM.add(userLpInAMM[listAMM[i]][account][rId]);
        rewardEarnedInAMM = rewardEarnedInAMM.add(userLpInAMM[listAMM[i]][account][rId].mul(rounds[rId].tfuelShare).div(1e12));
      }
    }
  }
  function earned(address account) public view returns(uint256 rewardEarned, uint256 rewardEarnedInAMM) {
    ...
    if (balanceOf(account) > 0) {
      for(uint256 roundIdx=lastClaimedRoundIndex[account]+1;roundIdx<=roundIndex;roundIdx++) {
        rewardEarned = rewardEarned.add(balancesByRound[account][roundIdx].mul(rounds[roundIdx].tfuelShare).div(1e12));
      }

      for(uint256 i=0;i<listAMM.length;i++) {
        for(uint256 roundIdx=lastClaimedRoundIndex[account]+1;roundIdx<=roundIndex;roundIdx++) {
          rewardEarnedInAMM = rewardEarnedInAMM.add(userLpInAMM[listAMM[i]][account][roundIdx].mul(rounds[roundIdx].tfuelShare).div(1e12));
        }
      }
    }
  }
  ...
  function _claimReward(address account, bool mode) internal {
    ...
    if (rewardBal > 0) {
      ...
      if (mode) {
        ...
        IAgeioController(ageioController).swapAgtAndBurn{value: tfuelForAgt}(tfuelForAgt);
