...
// Allowed all tokens dynamically and handle their decimal values
...
contract ChainPalTransaction is Ownable, ReEntrancyGuard {
    ...
    function createTransaction(
        ...
        uint256 decimal = 12;
        if (!compareToIgnoreCase(_feePaymentCurrency, "BNB")) {
            decimal = uint256(IBEP20(_feePaymentCurrencyAddress).decimals())
                .sub(6);
        }
        uint256 decimalMul;
        if (decimal > 0) {
            decimalMul = 10**decimal;
        } else {
            decimalMul = 1;
        }
        transaction.paymentAmountFees = feeTotal.mul(decimalMul);
        ...
        transaction.fees.admin = feeValues[3].mul(decimalMul);
        transaction.fees.referralBonus = feeValues[4].mul(decimalMul);
