...
contract GOFIRSTCOIN is Context, IERC20, Ownable {
    ...
    uint256 public _liquiditySellTax = 2;
    ...
    uint256 public _maxWalletToken = 3000000000000000000000;
    ...
    function _transfer(
      ...
      if (from != owner() && to != owner() && to != address(1)) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount (1,000).");
        }
        
        if(from != owner() && to != owner() && to != address(1) && to != uniswapV2Pair){
            uint256 contractBalanceRecepient = balanceOf(to);
            require(contractBalanceRecepient + amount <= _maxWalletToken, "Exceeds maximum wallet token amount (3,000)"); 
        }
