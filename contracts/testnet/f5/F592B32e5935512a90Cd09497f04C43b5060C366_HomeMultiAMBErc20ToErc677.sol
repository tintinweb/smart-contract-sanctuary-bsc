...
contract HomeFeeManagerMultiAMBErc20ToErc677 is BaseRewardAddressList, Ownable, BasicMultiTokenBridge {
    ...
    */
    function getFee(bytes32 _feeType, address _token, bool _isDiscounted) public view validFeeType(_feeType) returns (uint256) {
        ...
        assembly {
            let result := staticcall(gas, _delegate, add(data, 0x20), mload(data), 0, 0)
    ...
    */
    function calculateFee(bytes32 _feeType, address _token, uint256 _value, address _receiver) public view returns (uint256, bool) {
        bytes memory data = abi.encodeWithSignature("calculateFee(bytes32,address,_value,address)",_feeType,_token,_value,_receiver);
        address _delegate = getHomeFeeExtAddress();
        assembly {
            let result := staticcall(gas, _delegate, add(data, 0x20), mload(data), 0, 0)
    ...
    */
    function _distributeFee(bytes32 _feeType, address _token, uint256 _value, address _receiver) internal returns (uint256) {
        bytes memory data = abi.encodeWithSignature("distributeFee(bytes32,address,uint256,address,uint256)",_feeType,_token,_value,_receiver);
        address _delegate = getHomeFeeExtAddress();
        assembly {
