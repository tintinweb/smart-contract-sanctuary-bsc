...
// File: contracts/IterateMapping.sol
...
contract FarmBooster is Ownable {
    ...
    /// @notice evoc token.
    address public immutable EVOC;
    /// @notice evoc pool.
    address public immutable EVOC_POOL;
    ...
    /// @notice Evoc pool BOOST_WEIGHT precision
    ...
    /// @param _evoc EVOC token contract address.
    /// @param _evocPool Evoc Pool contract address.
    ...
    /// @notice Checks if the msg.sender is the evoc pool.
    modifier onlyEvocPool() {
        require(msg.sender == EVOC_POOL, "onlyEvocPool: Not evoc pool");
        _;
    }
    ...
    /// @notice Evocpool operation(deposit/withdraw) automatically call this function.
    /// @param _user user address.
    /// @param _lockedAmount user locked amount in evoc pool.
    /// @param _lockedDuration user locked duration in evoc pool.
    /// @param _totalLockedAmount Total locked evoc amount in evoc pool.
    /// @param _maxLockDuration maximum locked duration in evoc pool.
    function onEvocPoolUpdate(
        ...
        emit UpdateEvocPool(_user, _lockedAmount, _lockedDuration, _totalLockedAmount, _maxLockDuration);
    }
    ...
    /// @notice evoc pool average locked duration calculator.
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IBEP20(EVOC).balanceOf(EVOC_POOL);

        uint256 totalLockedAmount = IEvocPool(EVOC_POOL).totalLockedAmount();

        uint256 pricePerFullShare = IEvocPool(EVOC_POOL).getPricePerFullShare();
        ...
        uint256 boostedRatio = ((IEvocPool(EVOC_POOL).totalShares() - flexibleShares) * BOOST_RATIO_PRECISION) /
            originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = IEvocPool(EVOC_POOL).BOOST_WEIGHT();
        uint256 maxLockDuration = IEvocPool(EVOC_POOL).MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    /// @param _pid pool id.
    /// @param _duration evoc pool average locked duration.
    ...
    function _boostCalculate(
        ...
        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = IEvocPool(EVOC_POOL)
            .userInfo(_user);
