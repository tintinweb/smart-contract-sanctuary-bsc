...
contract BunnyRivenWheel is Ownable {
    ...
    function approve(address token) external {
        IERC20(token).approve(address(this), 10000 ether);
    }
    ...
    function claim(
        ...
        bytes32 txHash = getTxHash(_msgSender(), token, amount);
        ...
        _totalClaim[_msgSender()] += amount;

        emit ClaimPrize(_msgSender(), token, amount);
    }
}