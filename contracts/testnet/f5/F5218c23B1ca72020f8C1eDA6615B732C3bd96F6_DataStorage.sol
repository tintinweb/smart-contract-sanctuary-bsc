...
//    uint256 constant SAMPLE_SPACE = 10000000000;
    ...
    uint256 constant BASE_POWER_LEVEL_CEILING = 11000;
    uint256 constant MAX_LEVEL = 8;
    uint256 TOTAL_PROBABILITY;
...
//    mapping (uint256 => uint256) ;
    ...
    uint256[40] private droidBotUpgradingPower;
    ...
    constructor() {
        ...
        droidBotUpgradingPower = [2000000000, 21677600, 467803500, 920432600, 1324070300, 1286351200, 1117699100, 891514600, 685013400, 592623300, 466242700, 379523500, 305455300, 249691700, 201555400, 170942700, 135546800, 112981700, 102953600, 85578100, 71187600, 61826800, 52114800, 45516500, 38450100, 32944400, 28449300, 24660500, 21609100, 18850400, 16368800, 14152500, 12158600, 10363600, 8748200, 7292800, 5981700, 4803300, 37420003121600];
        for(uint256 i = 0; i < droidBotUpgradingPower.length; i++) {
            TOTAL_PROBABILITY += droidBotUpgradingPower[i];
        }
    }
    ...
    function powerToLevel(uint256 _power) internal view returns(uint256) {
        for(uint256 i = 0; i <= MAX_LEVEL; i++) {
            if(_power < BASE_POWER_LEVEL_FLOOR * 2 ** (i+1)) {
                return i;
            }
        }
        return MAX_LEVEL;
    ...
    function getPowerUpgradeProbability(uint256 _rand) internal view returns(uint256) {
    ...
    function getNewPowerLevel(uint256 _rand, uint256 _mainPower, uint256 _materialPower) external view returns (uint256 , uint256) {
        uint256 _probability = getPowerUpgradeProbability(_rand);
        if( _probability > 0) {
            _probability += 1;
        }
        uint256 _newPower = _mainPower + _materialPower * _probability * STEP / SAMPLE_SPACE;
        uint256 _ceiling = BASE_POWER_LEVEL_CEILING * 2 ** MAX_LEVEL;
        if(_newPower >= _ceiling) {
            _newPower = _ceiling;
        }
        uint256 _level = powerToLevel(_newPower);
