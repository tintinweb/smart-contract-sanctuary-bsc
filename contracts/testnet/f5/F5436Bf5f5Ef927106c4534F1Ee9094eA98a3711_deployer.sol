...
contract deployer is Ownable {
    ...
    uint256 public feeAmount = 7500 * 10**15;
    ...
    function createNewPool(
        ...
        uint256 requireTokensToPreSale = _tokensPerBnb.mul(_hardCap).div(
            10**18
        );
    ...
    **/
    ...
    function participateToSale() public payable nonReentrant {
        ...
        require(
            ...
            "You Already participated to the pool"
        );
        require(
        ...
        shares[msg.sender].bnbAmount = msg.value;
        shares[msg.sender].tokensAmount = tokensToUser;
    ...
    function claimTokens() public nonReentrant {
        syncPool();
        require(poolStatus == 2, "sales not ended");
        require(
            shares[msg.sender].bnbAmount > 0,
            "You do not have bnb in the pool"
        );
    ...
    function syncPool() public {
        ...
        if (filledBNB >= hardCap) {
            poolStatus = 2;
        } else if (filledBNB >= softCap && endTime <= block.timestamp) {
            poolStatus = 2;
        } else if (filledBNB < softCap && endTime <= block.timestamp) {
            poolStatus = 3;
        }
    ...
    function finalizePool() external onlyOwner nonReentrant {
        syncPool();
