...
contract Ptest15 is IERC20 {
    ...
    string constant private _name = "PTEST15";
    string constant private _symbol = "PTEST15";
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (lpPairs[to] && !excludedAccount) {
            ...
            uint256 onePercent = balanceOf(from) * SellLimit / 100; 
        ...
        if (sell) {
            if (!inSwap) {
                if (contractSwapEnabled && !excludedAccount
                ) {
