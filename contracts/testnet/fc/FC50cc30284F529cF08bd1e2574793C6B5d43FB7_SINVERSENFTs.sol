...
contract SINVERSENFTs is ERC721Enumerable, Ownable {
  using Strings for uint256;
  using SafeMath for uint256;
  bool public paused = false;
  string public baseURI;
  IBEP20 public ERC20_TOKEN = IBEP20(0x37591D144f3229121efc1E9F125b8d3D2C3A9D32);
  address public settlementFeeAddress = 0x309725349b23241f0a3113D2219FE8865075EA48;
 ...
 mapping(uint256 => _bidDetail) public bidDetail;
  ...
  function pause(bool _state) public onlyOwner {
    ...
    function _mint(uint256 tokenId,uint256 salePrice, uint256 royalty,SellType sellType, uint256 auctionStart, uint256 auctionExpire ) public  returns(uint256){
    ...
    function buy(uint256 tokenId, uint256 token) public {
        ...
        require(token >= tokenDetails[tokenId].salePrice, "Insufficient amount to buy");
        require(token <= ERC20_TOKEN.balanceOf(msg.sender), "Insufficient token balance for transfer");
        require(token <= ERC20_TOKEN.allowance(msg.sender,address(this)), "Insufficient allowance for transfer");

        ERC20_TOKEN.transferFrom(msg.sender,address(this),token);
        ...
        if(tokenDetails[tokenId].royalty > 0){
            ...
            ERC20_TOKEN.transfer(tokenDetails[tokenId].creator,royaltyAmt);
        ...
        if(settlementFeePercentage > 0){
            ...
            ERC20_TOKEN.transfer(settlementFeeAddress,feeAmt);
            // payable(settlementFeeAddress).transfer(feeAmt);
        }
        ERC20_TOKEN.transfer(tokenDetails[tokenId].owner,tokenDetails[tokenId].salePrice.sub(royaltyAmt).sub(feeAmt));
    ...
    function sendTokens(uint256 token) public {       
        require(token <= ERC20_TOKEN.balanceOf(msg.sender), "Insufficient token balance for transfer");
        require(token <= ERC20_TOKEN.allowance(msg.sender,address(this)), "Insufficient allowance for transfer");
        ERC20_TOKEN.transferFrom(address(this),msg.sender,token);      
    }


  function placeBid(uint256 tokenId, uint256 token) external {
        require(tokenDetails[tokenId].isActive, "Invalid token ID");
        require(tokenDetails[tokenId].forSale, "Item not available for sale");     
        ...
        require(bidDetail[tokenId].amount < token, "Amount should be more than previous bid");
        require(token <= ERC20_TOKEN.balanceOf(msg.sender), "Insufficient token balance for transfer");
        require(token <= ERC20_TOKEN.allowance(msg.sender,address(this)), "Insufficient allowance for transfer");
        ...
        if(bidDetail[tokenId].isActive){
            ERC20_TOKEN.transfer(bidDetail[tokenId].bidder,bidDetail[tokenId].amount);
            // payable(bidDetail[tokenId].bidder).transfer(bidDetail[tokenId].amount);
        }
        bidDetail[tokenId] = _bidDetail(token,msg.sender,true);
  }
  ...
  function acceptBid(uint256 tokenId) external {
    ...
    require(bidDetail[tokenId].amount <= ERC20_TOKEN.balanceOf(address(this)), "Insufficient token balance in pool for transfer"); 
    ...
    ERC20_TOKEN.transfer(tokenDetails[tokenId].owner,bidDetail[tokenId].amount.sub(royaltyAmt).sub(feeAmt));
  ...
  function withdrawToken(uint256 token) public onlyOwner {
       ERC20_TOKEN.transfer(msg.sender,token);
  }
