...
contract BEP20Redeem is Ownable{
    // Contract libs
    using SafeMath for uint256;
    ...
    function setLockDuration(uint256 _lockduration) external onlyOwner() {
        require(_lockduration > 0, "The lockduration cannot be 0");
        lockDuration = _lockduration;
    }

    function setRate(uint256 _rate) external onlyOwner() {
        require(_rate > 0, "The rate cannot be 0");
        rate = _rate;
    }
    ...
    function addBeneficiary(
        ...
        beneficiaries[_beneficiary].amount = beneficiaries[_beneficiary].amount + _amount*rate/100;
        beneficiaries[_beneficiary].releaseTime = block.timestamp + lockDuration;
    ...
    function releaseMyTokens() external onlyBeneficiaries {
        require(
            beneficiaries[_msgSender()].amount >0 && beneficiaries[_msgSender()].releaseTime > block.timestamp,
            "The beneficiary's address cannot be 0"
        );

        // Release new vested token to the beneficiary
        uint256 _amount = beneficiaries[_msgSender()].amount;
        beneficiaries[_msgSender()].amount = 0;
    ...
    function initBenefit( address _beneficiary, uint256 _amount,uint256 initTime) public onlyOwner{
        require(
            _beneficiary != address(0),
            "The beneficiary's address cannot be 0"
        );
        require(_amount > 0, "Shares amount has to be greater than 0");
        tokenVestingCap = tokenVestingCap.add(_amount).sub(beneficiaries[_beneficiary].amount*100/rate);
        beneficiaries[_beneficiary].amount = _amount*rate/100;
        beneficiaries[_beneficiary].releaseTime = initTime + lockDuration;
    }
    ...
    function withdrawToken(address _token,address payable dest, uint amount) public onlyOwner{
        if (_token == address(0x0))
            ...
            IBEP20(_token).transfer(dest, amount);
    }
}