...
contract Growwin is Context, IERC20, Ownable {
  ...
  uint256 public marketingAndDevelopment = 100; // 2% Marketing and Development

  uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 1000000000000000000000000000;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 lockedToken = locks[sender].lockedToken;
    if(lockedToken > 0){
            uint256 balance = balanceOf(sender); 
            uint256 withdrawable = balance - lockedToken;
            uint256 millis_days = locks[sender].timeInDays * 1 days;
            uint256 expiry = locks[sender].startTime + millis_days; 
            if(amount > withdrawable){
                ...
                locks[sender].lockedToken  = lockedToken - required_from_locked_mode;
            }
        }
    ...
    emit Transfer(sender, recipient, amount.sub(calculateMarketingAndDevFee(amount)));
  }
  ...
  function setMarketingAndDevFeePercent(uint256 newValue) external onlyOwner {
      //  require(
       //     newValue <= 1,
       //     "marketingAndDevelopment fee must be less than or equal to  1"
      //  );
    ...
    function calculateMarketingAndDevFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(marketingAndDevelopment).div(10000);
    }

    function transferWithLock(address recipient, uint256 amount, uint256 timeindays) external onlyOwner {
    require(recipient != address(0), "ERC20: transfer to the zero address");
    _balances[_msgSender()] = _balances[_msgSender()].sub(amount, "ERC20: transfer amount exceeds balance");
    _balances[recipient] = _balances[recipient].add(amount);
    lock(recipient, amount, timeindays);
