...
contract CODL is ERC20, Ownable {
    ...
    IERC20 public constant BUSD = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee); // Testnet: 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee  //Mainnet: 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
    ...
    address public overwatchAddress;
    ...
    uint256 public sellOverwatchFee;
    uint256 public sellLiquidityFee;
    uint256 public sellRewardsFee;
    uint256 public sellWarchestFee;

    uint256 public tokensForOverwatch;
    ...
    event UpdatedOverwatchAddress(address indexed newWallet);
    event UpdatedRewardsAddress(address indexed newWallet);
    event UpdatedWarchestAddress(address indexed newWallet);
    ...
    constructor() ERC20("Call Of HODL", "CODL") {
        
        address newOwner = 0xC75ff42DBF99d44662e0d1eDF89b7CDDdE0a9F8e; // can leave alone if owner is deployer.
        ...
        sellRewardsFee = 15;
        sellWarchestFee = 2;
        sellTotalFees = sellOverwatchFee + sellLiquidityFee + sellRewardsFee + sellWarchestFee;

        // update these!
        overwatchAddress = address(0xC75ff42DBF99d44662e0d1eDF89b7CDDdE0a9F8e);
        rewardsAddress = address(0xC75ff42DBF99d44662e0d1eDF89b7CDDdE0a9F8e);
        warchestAddress = address(0xC75ff42DBF99d44662e0d1eDF89b7CDDdE0a9F8e);
        liquidityAddress = address(0xC75ff42DBF99d44662e0d1eDF89b7CDDdE0a9F8e);
        futureOwnerAddress = address(0xC75ff42DBF99d44662e0d1eDF89b7CDDdE0a9F8e);

        _excludeFromMaxTransaction(newOwner, true);
        _excludeFromMaxTransaction(futureOwnerAddress, true);
        _excludeFromMaxTransaction(address(this), true);
        _excludeFromMaxTransaction(address(0xdead), true);
        _excludeFromMaxTransaction(address(overwatchAddress), true);
        _excludeFromMaxTransaction(address(warchestAddress), true);
        ...
        excludeFromFees(address(overwatchAddress), true);
        excludeFromFees(address(warchestAddress), true);
    ...
    function updateSellFees(uint256 _overwatchFee, uint256 _liquidityFee, uint256 _rewardsFee, uint256 _warchestFee) external onlyOwner {
        require(_overwatchFee <= 1, "Exceeded max fee for overwatch");
        sellOverwatchFee = _overwatchFee;
        ...
        require(_warchestFee <= 3, "Exceeded max fee for warchest");
        sellWarchestFee = _warchestFee;
        sellTotalFees = sellOverwatchFee + sellLiquidityFee + sellRewardsFee + sellWarchestFee;
        require(sellTotalFees <= 20, "Must keep fees at 29% or less");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(takeFee){
            ...
            // on sell
                ...
                tokensForOverwatch += fees * sellOverwatchFee / sellTotalFees;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForOverwatch + tokensForWarchest;
        ...
        uint256 busdForOverwatch = busdBalance * tokensForOverwatch / (totalTokensToSwap - (tokensForLiquidity/2));
        uint256 busdForWarchest = busdBalance * tokensForWarchest / (totalTokensToSwap - (tokensForLiquidity/2));

        busdForLiquidity -= busdForOverwatch + busdForWarchest;
            
        tokensForLiquidity = 0;
        tokensForOverwatch = 0;
        tokensForWarchest = 0;
        ...
        if(busdForWarchest > 0){
            BUSD.transfer(warchestAddress, busdForWarchest);
        }

        if(BUSD.balanceOf(address(this)) > 0){
            BUSD.transfer(overwatchAddress, BUSD.balanceOf(address(this)));
        }
    }
    ...
    function setOverwatchAddress(address _overwatchAddress) external onlyOwner {
        require(_overwatchAddress != address(0), "address cannot be 0");
        overwatchAddress = payable(_overwatchAddress);
        emit UpdatedOverwatchAddress(_overwatchAddress);
    }
    ...
    function setWarchestAddress(address _warchestAddress) external onlyOwner {
        require(_warchestAddress != address(0), "address cannot be 0");
        warchestAddress = payable(_warchestAddress);
        emit UpdatedWarchestAddress(_warchestAddress);
    }
