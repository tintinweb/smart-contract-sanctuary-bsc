...
contract PTP is ERC20, Ownable {
    ...
    uint256 fixAmount = 1000 * 10 ** 18;
    uint256 startDate = 1645603200;
    uint256 counterId = 0;
    ...
    function calculateReward(uint256 amount, uint256 day) public view returns(uint256) {
        uint256 poolROIPercent = findPercentSlot();
    ...
    function findEndDayTimeStamp(uint256 day) public view returns(uint256){
       uint256 futureDays = day.mul(86400);
    ...
    function createStake(uint256 _stake,uint day) public {
        ...
        stakeRecord memory myRecord = stakeRecord({id:id,stakeAmount:_stake, numOfDays:day, currentTime:block.timestamp,claimed:false,startDay:currentDay,endDay:endDay,endDayTimeStamp:endDayTimeStamp});
    ...
    function claimStakeReward(uint256 id) public  {
        ...
        uint256 reward = calculateReward(myRecord[id].stakeAmount, myRecord[id].numOfDays);
        //on end Day
        ...
        if(block.timestamp < myRecord[id].endDayTimeStamp){
            ...
            uint256 penalty = 0;
            uint256 intrest = calculateReward(myRecord[id].stakeAmount, emergencyDayEnd);
        ...
        if(emergencyDayEnd > halfOfStakeDays){
            ...
            uint256 interestAccruedAfterHalf = calculateReward(totalStaked, daysAfterHalf);
            uint256 intrestAccruedAtHalf = calculateReward(totalStaked, halfOfStakeDays);
    ...
    function testEarlyStake(uint256 emergencyDayEnd,uint256 endDay,uint256 stakeAmount) public {
        // uint256 emergencyDayEnd = findDayDiff(myRecord[id].currentTime);
            // uint256 endDay = myRecord[id].numOfDays;
            uint256 halfOfStakeDays = endDay.div(2);
            uint256 penalty = 0;
            uint256 intrest = calculateReward(stakeAmount, emergencyDayEnd);
            // uint256 totalStaked = stakeAmount;
            //if served day equal to half days
            if(emergencyDayEnd == halfOfStakeDays){
                uint256 halfPenalty = intrest.div(2);
                totalPenalties = totalPenalties.add(halfPenalty);
                originAmount = originAmount.add(halfPenalty);
                testingData = testingData.add(stakeAmount);
                // mintAmount(msg.sender,stakeAmount);
            }
            //If served days less than half day
    //      if(emergencyDayEnd < halfOfStakeDays){
    //         //If half of total days less than 90
    ...
    //             totalPenalties = totalPenalties.add(halfPenalty);
    //             originAmount = originAmount.add(halfPenalty);
    //             uint256 ptpRecivedBack = stakeAmount.add(intrest);
    //             ptpRecivedBack = ptpRecivedBack.sub(penalty);
    //             if(ptpRecivedBack < 0){
    //                 ptpRecivedBack = 0;
    //             }
    //             else{
    //                 testingData = testingData.add(ptpRecivedBack);
    //             // mintAmount(msg.sender,ptpRecivedBack);
    //             }
    //          }
    //         //if half of total days greater or equal to 90
    ...
    //     if(emergencyDayEnd > halfOfStakeDays){
    //         uint256 daysAfterHalf = emergencyDayEnd.sub(halfOfStakeDays);
    //         uint256 interestAccruedAfterHalf = calculateReward(stakeAmount, daysAfterHalf);
    //         uint256 intrestAccruedAtHalf = calculateReward(stakeAmount, halfOfStakeDays);
    //         uint256 halfPenalty = intrestAccruedAtHalf.div(2);
    //         totalPenalties = totalPenalties.add(halfPenalty);
    //         originAmount = originAmount.add(halfPenalty);
    //         uint256 ptpRecivedBack = interestAccruedAfterHalf.add(stakeAmount);
    //        testingData = testingData.add(ptpRecivedBack);

    //     }
    }

    function asdef () public view returns (uint256){
        return testingData;
    }
    ...
    function findReferalBonus(address user) public view returns(uint256) {
       uint256 totalAmount = fixAmount;
    ...
    function findSpeedBonus(uint256 day) public view returns (uint256){
      uint256 speedBonus = 0;
      uint256 initialAmount = fixAmount;
    ...
    function distributeFreeStake( ) public onlyOwner{
        require(block.timestamp < startDate + 31536000,'Free stakes available just 1 year');
        ...
        require(freeStakeUsers.length > 0,'No Free Stake Holder Find');
        ...
        uint256 compeleteBonus = fixAmount;
        //Referal Mints
         if(userReferer != freeStakeUsers[i] && userReferer !=address(0)){
            uint256 amount = fixAmount;
        ...
        //Referal Bonus
        uint256 referalBonus = findReferalBonus(freeStakeUsers[i]);
        compeleteBonus = compeleteBonus.add(referalBonus);
        originAmount = originAmount.add(referalBonus);
        if(userReferer != address(0)){
         createReferalRecords(userReferer,freeStakeUsers[i],referalBonus);
        }
        //Speed Bonus
        uint256 userSpeedBonus = findSpeedBonus(freeStakeDay);
        ...
        createFreeStakeRecord(freeStakeUsers[i],mintedValue);
        }
        distributeTimeStamps[msg.sender] = block.timestamp;
    }

    function createFreeStakeRecord(address freeStakeHolder, uint256 mintedAmount) internal onlyOwner{
    ...
    function withDrawFreeStake() public {
        ...
        require(_isFreeStakeholder,'Not a free stake holder');
        require(freeStakes[msg.sender] > 0,'Staked Amount is 0');
        mintAmount(msg.sender,freeStakes[msg.sender]);
        removeFreeStakeholder(msg.sender);
    ...
    function setPayDayAmount (uint256 unClaimedAmount) public onlyOwner {
        payDayAmount = payDayAmount + unClaimedAmount;
    }

    function getPayDayAmount () public view onlyOwner returns(uint256)  {
        return payDayAmount;
    }
    ...
    function setAvailabePTP () public onlyOwner {
        uint256 totalBitcoin = 19000000 * 10 ** 18;
        uint256 ptpPerBtc = 10000 * 10 ** 18;
        uint256  dayOfAA = 350 * 10 ** 18;
        uint256 hexAvailabel = 0;
        hexAvailabel = totalBitcoin.mul(ptpPerBtc);
        hexAvailabel = hexAvailabel.div(dayOfAA);
        hexAvailabel = hexAvailabel;
        uint day = findDay();
