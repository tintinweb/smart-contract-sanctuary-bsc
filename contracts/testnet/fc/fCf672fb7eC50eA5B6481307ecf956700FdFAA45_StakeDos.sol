...
contract StakeDos is AccessControl {
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlockTime;  // Last time number that HE distribution occurs.
    ...
    constructor(
        ...
        HE = IERC20(_HE);
        HePerBlockTime = _hePerBlockTime;
    ...
    function changeHePerBlockTime(uint256 _hePerBlockTime) external onlyRole(DEFAULT_ADMIN_ROLE) {
        massUpdatePools();
        HePerBlockTime = _hePerBlockTime;
    }
    ...
    function add(uint256 _allocPoint, IERC20 _heToken, bool _withUpdate) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardBlockTime && lpSupply != 0) {
            ...
            uint256 heReward = multiplier.mul(rewardThisBlockTime).mul(pool.allocPoint).div(totalAllocPoint);
