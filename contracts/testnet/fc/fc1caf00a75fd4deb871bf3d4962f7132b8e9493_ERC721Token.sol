...
//contract ERC721Token is ERC721, ERC721URIStorage, Pausable, Ownable, ERC721Burnable, MinterRole {
contract ERC721Token is ERC721, ERC721URIStorage, Pausable, Ownable, MinterRole {
    ...
    function mintWithTokenURI(address to, uint256 tokenId, string memory uri_) public onlyMinter returns (bool) {
        _mint(to, tokenId);
    ...
    function batchMintWithTokenURI(address to, uint256[] memory tokenIds, string[] memory tokenURIs) public onlyMinter returns (bool) {
        require(tokenIds.length == tokenURIs.length, 'array length is different');
