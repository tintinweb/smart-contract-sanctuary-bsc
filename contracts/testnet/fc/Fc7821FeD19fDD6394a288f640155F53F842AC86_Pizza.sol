...
contract Pizza is Ownable {
    ...
    function withdrawal(address payable _account, uint256 _amount) public multiReviewer {
        require(address(this).balance >= _amount, "BNB balance not enough");
        _account.transfer(_amount);       
    }
    
    function queryPledge(address account) public view returns(uint256) {
        return pledge[account];
    }
}