...
contract LuumInDapp{
    ...
    function ComprarProducto (uint256 _id, uint _Cantidad) public {
        ...
        {  
            ...
            // MapPurchases.push(purchases (_nameProduct, MappingProductos[_id].price, _Cantidad, CostoTotal, MappingProductos[_id].cycle, MappingProductos[_id].percAttMin, MappingProductos[_id].percAttMax, block.timestamp));
            // MappingPurchases[msg.sender] = purchases (_nameProduct, MappingProductos[_id].price, _Cantidad, CostoTotal, MappingProductos[_id].cycle, MappingProductos[_id].percAttMin, MappingProductos[_id].percAttMax, block.timestamp);
            ...
            buyer.contratos.push(Contrato (idContract, _id, _nameProduct, MappingProductos[_id].percAttMin, MappingProductos[_id].percAttMax, _Cantidad, MappingProductos[_id].price, CostoTotal, msg.sender, true, block.timestamp));
            buyer.MapContracts.push(idContract);

            // emit purchaseDone(_nameProduct, CostoTotal, msg.sender);
    ...
    function OtorgarRendimiento(uint256 _idContract, uint _porcentajeRendimiento) public Unicamente(msg.sender) {
        ...
        if (true) {
            ...
            require (MappingProductos[IdProducto].percAttMin <= _porcentajeRendimiento, "Este porcentaje es inferior al minimo requerido");
            require (MappingProductos[IdProducto].percAttMax >= _porcentajeRendimiento, "Este porcentaje es superior al porcentaje maximo permitido");
    ...
    // function getPurchase(address userAddress) external view returns (purchases memory) {
    //     return MappingPurchases[userAddress];
    // }

    // function getPurchses() external view returns (purchases [] memory) {
    //     return MapPurchases;
    // }
    ...
    // function HistorialDeCompras() public view returns (cliente[] memory) {
    //     return MapClients[msg.sender];
    // }
