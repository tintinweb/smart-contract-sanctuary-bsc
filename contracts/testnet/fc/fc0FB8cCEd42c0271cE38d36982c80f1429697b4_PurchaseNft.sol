...
contract PurchaseNft is Ownable, ExchangeDomain {
    ...
    constructor (ERC20TransferProxy _erc20TransferProxy, NFT721 _nft, address beneficiary) {
    ...
    function purchase(Quality _quality, AssetType assetType, address addressToken)  payable public lockPurchase {        
        uint price = _salesSettings[_quality].price;
        Group _group = _whitelists[msg.sender];
        uint indexCurrent = _salesSettings[_quality].totalSold + _salesSettings[_quality].startTokenId;
        ...
        if ((_group == Group.A || _group == Group.B) && _whitelistSettings[_group].endTime >= block.timestamp && _whitelistSettings[_group].startTime < block.timestamp ) {
            bool isDiscount = _whitelistSettings[_group].discounted;
        ...
        if (price > 0) {
            transfer(_beneficiary, assetType, price, addressToken);
        }        
        nft.purchaseMint(indexCurrent, fees, indexCurrent.toString());
    ...
    modifier lockPurchase() {
        require(isLooked == false, "Purchase is Lock");
        _;
    }
