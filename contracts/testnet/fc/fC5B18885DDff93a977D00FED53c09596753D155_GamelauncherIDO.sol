...
contract GamelauncherIDO {
   ...
   function ClaimToken(string memory _phaseId,string memory _projectId,uint _vestingIndex,string memory _orderId) public returns (bool) {
       ...
       if(userpurchased.totalAvailable>0){
         ...
         _totalPurchased=5;//SafeMath.safeMultiply(_totalPurchased,uint256(10) ** getProjectDetails[_projectId].decimals);
         uint256 _totalPayableAmount=5;//((_totalPurchased.mul(projectphasevestingdetail.releasePer[_vestingIndex])).div(100));
