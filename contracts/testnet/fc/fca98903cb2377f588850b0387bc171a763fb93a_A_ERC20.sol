  ...
  , IERC20Metadata
{
  ...
  uint256 public _transTax = 0;
  uint256 public _taxDivider = 100;
  address public _taxCollector;
  function setTax(uint256 sellTax_, uint256 buyTax_, uint256 transTax_, uint256 divider_) public onlyOwner swapping { 
    _sellTax = sellTax_; 
    _buyTax = buyTax_; 
    _transTax = transTax_; 
    _taxDivider = divider_; 
  }
  ...
  function _transferTax(address sender, address recipient, uint256 amount) internal returns (bool) {
    ...
    if (sender == _pair) { // buy
      if (!_excludedBuyFee[recipient]) {
        amountReceived = takeFee(amount, 1);
      }
    } else if (recipient == _pair) { // sell
      if (!_excludedSellFee[sender]) {
        amountReceived = takeFee(amount, 2);
        distributeFee();
      }
    }

    if (_transTax > 0) { // transfer
      amountReceived = takeFee(amount, 3);
    }
  ...
  function takeFee (uint256 amount_, uint8 taxAction_) private returns(uint256){
    uint256 tax;
    if (taxAction_ == 1) { tax = _buyTax; }
    if (taxAction_ == 2) { tax = _sellTax; }
    if (taxAction_ == 3) { tax = _transTax; }
    if (tax == 0 || _taxCollector == address(0)) return amount_;
    uint256 fee = tax.mul(amount_).div(_taxDivider);
  ...
  function distributeFee () private swapping {
    ...
    if (_balances[address(this)] > 0 && _taxCollector != address(0)) {
