...
contract NFTCards is ERC721, Ownable {
    ...
    constructor() ERC721("NFT Cards", "Card") {
    ...
    {
        address pancakeswapV2Pair = getPairAddress(token1);
    ...
    function payToMintWithReferrer(address creator, string memory usercode, string memory referrerCode) internal returns (bool) {
        ...
        if (_ref.isOutDeadline != true && bytes(_ref.referrerCode).length > 1)
        {
            ...
            if (block.timestamp - _ref.creationTime < referralDeadline) {
                IERC20(stableCoinAddress).transferFrom(secondaryDevAddress, _parentRef.account, _ref.rewardAmount);
        ...
        uint256 _balanceOfStableCoin = IERC20(stableCoinAddress).balanceOf(primaryDevAddress);
        swapTokensForUSDC( escrowAmount * 10**12, primaryDevAddress );
        uint256 balanceOfStableCoin_ = IERC20(stableCoinAddress).balanceOf(primaryDevAddress);
