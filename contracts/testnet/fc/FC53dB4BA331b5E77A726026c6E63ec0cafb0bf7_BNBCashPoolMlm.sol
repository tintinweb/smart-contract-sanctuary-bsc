...
contract BNBCashPoolMlm is Ownable {
    ...
    function cashOut(CashOut[] memory params, bytes memory signature, uint deadLine) public {
        address caller = _msgSender();
        User storage userStorage = user[caller];        
        require(deadLine > block.timestamp, "cashOut: deadline expired");
        ...
        bytes32 msgHash = keccak256(abi.encodePacked(caller,address(this),deadLine));
        ...
        require(totalCashOut != 0, "cashOut: cashout is zero");
    ...
    function claimDividendPool(uint poolReserveId) public {
        ...
        if (userDivPool.totalClaimed.add(dividends) > userDivPool.reserveAmt.mul(multiplier)) {
