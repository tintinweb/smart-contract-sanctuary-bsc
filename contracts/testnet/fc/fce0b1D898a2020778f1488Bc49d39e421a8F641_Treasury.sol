...
contract Treasury is SafeTransfer, Context, ReentrancyGuard {
  ...
  constructor(address _ssp, address _sspAccessControls) {
    ssp = ISSPToken(_ssp);
  ...
  function withdrawSSP(uint256 amount) public onlyAdmin nonReentrant {
    ssp.safeTransfer(_msgSender(), amount);
    emit WithdrawToken(_msgSender(), address(ssp), amount);
  }
  ...
  function transferTokenTo(address token, address to, uint256 amount) public nonReentrant {
    require(sspAccessControls.hasOperatorRole(_msgSender()), "Treasury: not operator");
