...
interface ITreasury is IEpoch {
    ...
    function getDarkgLockedBalance() external view returns (uint256);

    function getDarkgCirculatingSupply() external view returns (uint256);
...
contract ProtocolStats is OwnableUpgradeSafe {
    ...
    function addresses() external view returns (
        address darkgAddress, address skygAddress, address lightgAddress, address wcroAddress, address pairDarkgDarkAddress, address pairSkygDarkAddress,
        ...
        pairSkygDarkAddress = pairSkygDark;
    ...
    function tokenStats() external view returns (
        ...
        }
        {
            ...
            skygWcroLpPrice = _priceChecker.getLpPriceToUsd(pairSkygDark);
        }
    ...
    function boardroomStat() external view returns (
        ...
        if (_treasurySharedPercent > 0) {
            IBoardroom _boardroom = IBoardroom(_treasury.boardroom());
            share = _boardroom.share();
            sharePrice = _priceChecker.getTokenPriceToUsd(share);
            totalShare = _boardroom.totalSupply();
            ...
            stakeFee = _boardroom.stakeFee();
            withdrawFee = _boardroom.withdrawFee();
        }
    }
