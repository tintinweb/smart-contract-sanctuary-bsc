...
contract LabradoWallet is Ownable, IERC721Receiver {
    ...
    event NFTWithdrewByOwner(address indexed to, uint256 tokenId);
    ...
    function withdrawNFTByOwner(address _to, uint256 _tokenId)
        public
        onlyOwner
    {
        address owner = IERC721(nft).ownerOf(_tokenId);
        require(owner == address(this), "This token does not belong to wallet");
        IERC721(nft).safeTransferFrom(address(this), _to, _tokenId);
        emit NFTWithdrewByOwner(_to, _tokenId);
    }
