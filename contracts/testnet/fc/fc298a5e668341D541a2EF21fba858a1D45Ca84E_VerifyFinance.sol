...
abstract contract ERC20{
    ...
    function referral(address target,uint amountToWei) external virtual;
    function getSignatrueMap(bytes32 _r, bytes32 _s,uint8 _v) external virtual returns (bool);
}
...
contract VerifyFinance is Comn{
    ...
    // 理财验证
    function financeVerify(address sender,string memory amountToStr,string memory seed, bytes32 _r, bytes32 _s,uint8 _v) public isRuning isMember isBlack nonReentrant  returns(bytes memory signaturn){
        ...
        if(ERC20(verifyExtractTokenContract).getSignatrueMap(_r,_s,_v)){ _status = _NOT_ENTERED; revert("Verify : ExtractToken signatrue used"); }
    ...
    function extractToken(address sender,uint amountToWei,bytes memory signaturn) private{
        ERC20(financeContract).referral(sender, amountToWei);
        ERC20(targetContract).transfer(financeContract, amountToWei);
    ...
    address private financeContract;                                //[设置]  理财合约
    address private verifyExtractTokenContract;                     //[设置]  验证提取代币合约
    mapping(bytes => bool) private signatrueMap;                    //使用情况

    /*
     * @param _panelContract 面板合约
     * @param _extractSigner 提取签署合约
     * @param _targetContract 目标合约
     * @param _financeContract 理财合约
     * @param _verifyExtractTokenContract 验证提取代币合约
     */
    function setConfig(address _panelContract,address _signerAddr,address _targetContract,address _financeContract,address _verifyExtractTokenContract) public onlyOwner {
        panelContract = _panelContract;
        signerAddr = _signerAddr;
        targetContract = _targetContract;
        financeContract = _financeContract;
        verifyExtractTokenContract = _verifyExtractTokenContract;
    }
