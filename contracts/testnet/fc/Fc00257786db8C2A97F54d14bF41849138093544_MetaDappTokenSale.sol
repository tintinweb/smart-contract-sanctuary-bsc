...
contract MetaDappTokenSale {
    ...
    function buy(uint tokens) public payable {
        ...
        require(phase(currentPhaseIndex).total <= __amount(tokens));
    ...
    function __unAmount(uint256 _amount, uint decimals) private pure returns(uint) {
        return _amount / (10 ** decimals); //aqui sustituir el 18 por el valor de los decimales del token pa no ponerlo fijo
    }

    function __amount(uint _amount) private pure returns(uint) {
        return _amount * (10 ** 18); //aqui sustituir el 18 por el valor de los decimales del token pa no ponerlo fijo
    }
