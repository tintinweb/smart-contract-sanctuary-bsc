...
contract NimbusInitialAcquisition is Ownable, Pausable {
    ...
    event UpdateNFTVestingContract(address indexed nftVestingAddress);
    ...
    constructor(address systemToken, address nftVestingAddress, address nftSmartLpAddress, address router, address nbuWbnb) {
    ...
    function claimSponsorBonuses(address user) public {
        ...
        nftVesting.safeMint(user, "", nftVestingAmount, swapToken);
        uint256 nftTokenId = nftVesting.tokenByIndex(nftVesting.totalSupply());
    ...
    function _buySystemToken(address token, uint tokenAmount, uint systemTokenAmount, address systemTokenRecipient, uint stakingPoolId) private {
        ...
        uint swapTokenAmount = getTokenAmountForToken(token, swapToken, systemTokenAmount, true);
        bool isFirstStaking = referralProgram.userIdByAddress(systemTokenRecipient) == referralProgram.lastUserId();
    ...
    function getAllStakingRewards(uint256[] memory stakingIds) public {
        ...
        for (uint256 index = 0; index < stakingIds.length; index++) {
            if (address(stakingPools[index]) != address(0))
            stakingPools[index].getRewardForUser(user);
        }
    }
    ...
    function updateNFTVestingContract(address nftVestingAddress) external onlyOwner {
        require(Address.isContract(nftVestingAddress), "NimbusInitialAcquisition: NFTVestingContractAddress is not a contract");
        nftVesting = IVestingNFT(nftVestingAddress);
        emit UpdateNFTVestingContract(nftVestingAddress);
    }
