    ...
    ERC20FlashMint
{
    ...
    uint256 public maximumHoldable;
    uint256 public maximumTransferrable;

    uint256 public _liquidityFee;
    uint256 public _marketingFee;
    uint256 public _reflectionFee;
    uint256 public _totalTransferFee;
        ...
        ERC20Permit("TestOgasa")
    {
        ...
        _marketingFee = 0;
        _reflectionFee = 0;
        _totalTransferFee = _liquidityFee + _marketingFee;

        // Calculate Reflection
        _tTotal = 100000000 * 10**decimals();
        _reflectionTotal = (MAX - (MAX % _tTotal));

        // Wallet limits
        maximumHoldable = _tTotal;
        maximumTransferrable = _tTotal;

        // Transfer token supply to owner wallet
        _rOwned[owner()] = _reflectionTotal;

        // Wallet that are excluded from holding limits
        _isLimitExempt[owner()] = true;
        _isLimitExempt[address(this)] = true;
        _isLimitExempt[BURN_WALLET] = true;
        ...
        _isExcluded[BURN_WALLET] = true;
        _isExcluded[address(this)] = true;

        // Push excluded wallets to array
        _excluded.push(BURN_WALLET);
    ...
    bool public noFeeOnTransfer = true;
    ...
    // Events
    event UpdatedMappingForLimitExempt(
        address walletAddress,
        bool isLimitExempt
    );

    event UpdatedMappingForExcludedFromFee(
        address walletAddress,
    ...
    event UpdatedWalletLimit(
        uint256 maximumTransferrable,
        uint256 maximumHoldable
    );
    ...
    event UpdatedNoFeeOnTransfer(bool isFeeOnTransfer);
    ...
    function transferOwnership(address newOwner) public override onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is 0x00");
    ...
    function tokenFromReflection(uint256 _rAmount)
        ...
        returns (uint256)
    {
        require(_rAmount <= _reflectionTotal, "Must be < total reflections");
        uint256 currentRate = _getRate();
        return _rAmount / currentRate;
    }

    // Excludes wallet from transaction and holding limits - Default false
    function mapWalletLimitExemption(address walletAddress, bool isLimitExempt)
        external
        onlyOwner
    {
        _isLimitExempt[walletAddress] = isLimitExempt;
        emit UpdatedMappingForLimitExempt(walletAddress, isLimitExempt);
    }

    // Excludes wallet from transaction and holding limits - Default false
    function mapWalletExcludeFee(address walletAddress, bool isFeeExclude)
        external
        onlyOwner
    {
        _isExcludedFromFee[walletAddress] = isFeeExclude;
        emit UpdatedMappingForExcludedFromFee(walletAddress, isFeeExclude);
    }
    ...
    */
    ...
    function reflectionsExcludeWallet(address account) external onlyOwner {
    ...
    function reflectionsIncludeWallet(address account) external onlyOwner {
    ...
    function updateMarketingWallet(address payable newWallet)
        ...
        onlyOwner
    {
        require(newWallet != address(0), "Can not be zero address");
        emit UpdatedMarketingWallet(_marketingWallet, newWallet);
    ...
    function updateLiquidityWallet(address newWallet) external onlyOwner {
        ...
        emit UpdatedLiquidityWallet(_liquidityWallet, newWallet);
    ...
    function setLimitsForWallets(
        ...
        maximumTransferrable = maxTokensPerTransaction * 10**decimals();
        maximumHoldable = maxTotalTokensPerWallet * 10**decimals();

        emit UpdatedWalletLimit(maximumTransferrable, maximumHoldable);
    }
    ...
    function setFees(
        ...
        require(
            marketingFee + liquidityFee + reflectionFee <= 10,
            "Fees too high"
        );
        ...
        _totalTransferFee = _marketingFee + _liquidityFee;

        emit UpdatedFees(marketingFee, liquidityFee, reflectionFee);
    }

    // When sending tokens to another wallet (not buying or selling) if noFeeToTransfer is true there will be no fee (default = true)
    function setTransfersWithoutFees(bool isFeeOnTransfer) external onlyOwner {
        noFeeOnTransfer = isFeeOnTransfer;
        emit UpdatedNoFeeOnTransfer(isFeeOnTransfer);
    }
    ...
    function _beforeTokenTransfer(
        ...
        // Require !maxWalletAmount
        require(balanceOf(from) != amount, "Amount is wallet max");
        // Wallet Limit
        if (!_isLimitExempt[to] && from != owner()) {
            uint256 heldTokens = balanceOf(to);
            require(
                (heldTokens + amount) <= maximumHoldable,
                "Over wallet max"
            );
        }
        ...
        if (!_isLimitExempt[to] || !_isLimitExempt[from]) {
            require(amount <= maximumTransferrable, "Over transaction limit");
        }
    ...
    function _takeReflect(uint256 _tReflect, uint256 _rReflect) private {
        _reflectionTotal = _reflectionTotal - _rReflect;
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            ...
            ) return (_reflectionTotal, _tTotal);
            rSupply = rSupply - _rOwned[_excluded[i]];
            tSupply = tSupply - _tOwned[_excluded[i]];
        }
        if (rSupply < _reflectionTotal / _tTotal)
            return (_reflectionTotal, _tTotal);
        return (rSupply, tSupply);
    }
    ...
    */
    ...
    function _transfer(
        ...
        require(sender != address(0), "ERC20: transfer from 0x00 address");
        require(recipient != address(0), "ERC20: transfer to 0x00 address");
        ...
        if (!takeFee) {
            ...
            tFeeTotal = 0; // Marketing, Liquidity and Contract Fee
        } else {
            tReflect = (tAmount * _reflectionFee) / 100;
            tFeeTotal = (tAmount * _totalTransferFee) / 100;
        }
        ...
        rSwapFeeTotal = tFeeTotal * rfi;
        tTransferAmount = tAmount - (tReflect + tFeeTotal);
        rTransferAmount = rAmount - (rReflect + rSwapFeeTotal);

        // transfer tokens based on RFI status of sender and recipient
