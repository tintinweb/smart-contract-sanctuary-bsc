...
contract TestOnly is Context, IERC20, Ownable {
    ...
    string constant private _name = "Test USDC 1";
    string constant private _symbol = "TUSDC 1";
    ...
    IERC20 public constant USDC = IERC20(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    constructor () {
        ...
        lpPair = IDexFactory(dexRouter.factory()).createPair(address(this), address(USDC));
    ...
    function updateRouter(address newRouterAddress) public onlyOwner returns(address newPairAddress) {
        ...
        newPairAddress = IDexFactory(dexRouter_.factory()).getPair(address(this), address(USDC));
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        swapTokensForUSDC(tokensForSwap);
        tokenHandler.sendTokenToOwner(address(USDC));

        // uint256 usdcBalance = USDC.balanceOf(address(this));
        // uint256 usdcShares = totalShares - (lpShare / 2);
        
        // uint256 usdcForLiquidity = ((usdcBalance * lpShare) / usdcShares) / 2;
        // uint256 usdcForOperations = (usdcBalance * operationsShare) / usdcShares;
        // uint256 usdcForMarketing = usdcBalance - usdcForLiquidity - usdcForOperations;

        // if(usdcForMarketing > 0) {
        //     USDC.transfer(marketingWalletAddress, usdcForMarketing);
        // }

        // if(usdcForOperations > 0) {
        //     USDC.transfer(operationsWalletAddress, usdcForOperations);
        // }

        // if(usdcForLiquidity > 0 && tokensForLP > 0) {
        //     addLiquidity(tokensForLP, usdcForLiquidity);
        // }
    }
    ...
    function swapTokensForUSDC(uint256 tokenAmount) private {
        ...
        dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            path,
            address(tokenHandler), // The contract
            block.timestamp
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdcAmount) private {
        ...
        // add the liquidity
        dexRouter.addLiquidity(address(this), address(USDC), tokenAmount, 
                usdcAmount, 0,  0,  autoLiquidityReceiver, block.timestamp);
    }
