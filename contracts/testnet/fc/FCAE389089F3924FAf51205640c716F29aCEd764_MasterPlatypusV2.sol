...
contract MasterPlatypusV2 {    
    ...
    */
    ...
    event FactorUpdated(address indexed user, uint256 oldFactor, uint256 newFactor);
    ...
    function updateRewardFactorPTP(uint256 newFactor) public onlyOwner {
        ...
        _rfPTP = newFactor;
        emit FactorUpdated(msg.sender, oldFactor, newFactor);
    }
    ...
    */
    ...
    function unStakingLP(uint256 pid, uint256 amount) public {
        ...
        // PTP.mint(msg.sender, rewardAmount);
        PTP.transferAsReward(msg.sender, rewardAmount);
        _updateLPStakedUserInfoForUnStaking(pid, msg.sender, amount);
        emit LPUnStaked(msg.sender, lpTokens[pid], amount);
    }

    function multiClaim(uint256[] memory pids) external {
        _multiClaim(pids, msg.sender);
    }

    function _multiClaim(uint256[] memory pids, address u) private {
        uint256 rewardAmount;
        for (uint256 i; i < pids.length; ++i) {
            uint256 pid = pids[i];
            LPStakedUserInfo storage user = _lpStakedUserInfo[pid][u];
            rewardAmount += user.rewardAmount + calcIncreasedPTPReward(user.lpAmount, user.lastTimestamp);                       
        }
        PTP.transferAsReward(u, rewardAmount);
        _updateLPStakedUserInfoForMultiClaim(pids, u);  
    }
    ...
    function _updateLPStakedUserInfoForMultiClaim (uint256[] memory pids, address user) private {    
        for (uint256 i; i < pids.length; ++i) {
            uint256 pid = pids[i];
            _updateLPStakedUserInfoForClaim(pid, user);
        }            
    }
...
contract PTPV2 is ERC20 {     
    ...
    function transferAsReward(address to, uint256 amount) public onlyMinter {
        require(to != address(this), "transferAsReward: return back");
        require(to != address(0), "transferAsReward: zero address");
