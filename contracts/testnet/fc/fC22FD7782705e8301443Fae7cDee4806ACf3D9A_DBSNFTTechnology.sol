...
contract DBSNFTTechnology  is Ownable{
    ...
    constructor(address _dbsnft, IERC20 _erc20, address _pveAddress){
        nftAddress = ERC1155Tradable(_dbsnft);
    ...
    ERC1155Tradable nftAddress;
    // tokens holder address
    address private pveAddress;
    uint256 constant sevenDays = 7*24*3600;
    Ship[] copperShip;
    Ship[] silverShip;
    Ship[] goldShip;
    ...
    uint256 private castCost;
    mapping(uint256 => uint256) internal NftToQuality;
    mapping(uint256 => uint256) internal IDToIndex;
    mapping(address => uint256) internal CastShip;
    // event
    event CastFinalized();
    event newCast(address _caster, uint256 _Id);
    event newRecast(address _caster, uint256[] _ids, uint256 _id);
    event newRecruitCopper(address _operator, uint256 _id, uint256 _destid);
    event newRecruitDest(address _operator, uint256[] _ids, uint256 _destid);
    event newDecompose(address _operator, uint256 _id, uint256[] _destcids, uint256[] _counts);

    /**
     * @return true if the crowdsale is finalized, false otherwise.
     */
    function IsCastfinalized() public view returns (bool) {
        return _finalized;
    }
     ...
     */
    function finalizeCast() public onlyOwner {
    ...
    function Cast() public  returns (uint256){
        require(IsCastfinalized(), "cast finalized");
        require(block.timestamp - CastShip[msg.sender] >= sevenDays, "cast cool down time require 7days");
        // Transfer the token
        token.safeTransferFrom(msg.sender, pveAddress, castCost*(10**uint256(18)));
        CastShip[msg.sender] = block.timestamp;
        uint256 index = _pickShipRandomClass(uint256(NFTQuality.NFTCopper));
        nftAddress.mint(msg.sender, copperShip[index].id, 1, "");
        emit newCast(msg.sender, copperShip[index].id);
        return copperShip[index].id;
    }

    function getCasterBeginTime() public view returns (uint256){
        return CastShip[msg.sender];
    }

    function Recast(uint256[] memory _ids) public  {
        require(_ids.length == 5, "DBSNFTTechnology: Recast _ids length error");
        uint256 quality = NftToQuality[_ids[0]];
        ...
        if(quality == uint256(NFTQuality.NFTCopper)){
            uint256 camp = copperShip[IDToIndex[_ids[0]]].camp;
            for (uint256 i = 1; i < _ids.length; i++) {
                require(quality == NftToQuality[_ids[i]], "DBSNFTTechnology: NFTCopper quality error");
                require(camp == copperShip[IDToIndex[_ids[i]]].camp, "DBSNFTTechnology: NFTCopper camp error");
                values[i] = 1;
            }
            require(IDToIndex[_ids[0]] < copperShip.length);
            nftAddress.burnBatch(msg.sender, _ids, values);
            uint256[] memory cids;
            uint256 count;
            ...
            nftAddress.mint(msg.sender, cids[cindex], 1, "");

            emit newRecast(msg.sender, _ids, cids[cindex]);
        }else if(quality == uint256(NFTQuality.NFTSilver)){
            uint256 camp = silverShip[IDToIndex[_ids[0]]].camp;
            for (uint256 i = 1; i < _ids.length; i++) {
                require(quality == NftToQuality[_ids[i]], "DBSNFTTechnology: NFTSilver quality error");
                require(camp == silverShip[IDToIndex[_ids[i]]].camp, "DBSNFTTechnology: NFTSilver camp error");
                values[i] = 1;
            }
            require(IDToIndex[_ids[0]] < silverShip.length, "DBSNFTTechnology: NFTSilver silverShip error");
            nftAddress.burnBatch(msg.sender, _ids, values);
            uint256[] memory sids;
            uint256 count;
            ...
            nftAddress.mint(msg.sender, sids[sindex], 1, "");

            emit newRecast(msg.sender, _ids, sids[sindex]);
        }
    }
    ...
    function RecruitCopper(uint256 _id) public  {
        uint256 quality = NftToQuality[_id];
        require(quality == uint256(NFTQuality.NFTCopper), "DBSNFTTechnology: RecruitCopper _quality error");
        
        nftAddress.burn(msg.sender, _id, 1);
        uint256 index = _pickSkillRandomClass(quality);
        nftAddress.mint(msg.sender, copperSkill[index], 1, "");

        emit newRecruitCopper(msg.sender, _id, copperSkill[index]);
    }

    function RecruitDest(uint256[] memory _ids) public  {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(quality == NftToQuality[_ids[i]]);
            values[i] = 1;
        }

        if(quality == uint256(NFTQuality.NFTCopper)){
            nftAddress.burnBatch(msg.sender, _ids, values);
            nftAddress.mint(msg.sender, copperShip[IDToIndex[_ids[0]]].skill, 1, "");

            emit newRecruitDest(msg.sender, _ids, copperShip[IDToIndex[_ids[0]]].skill);
        }else if(quality == uint256(NFTQuality.NFTSilver)){
            nftAddress.burnBatch(msg.sender, _ids, values);
            nftAddress.mint(msg.sender, silverShip[IDToIndex[_ids[0]]].skill, 1, "");

            emit newRecruitDest(msg.sender, _ids, copperShip[IDToIndex[_ids[0]]].skill);
        }
    }

    function Decompose(uint256 _id) public  {
        uint256 quality = NftToQuality[_id];
        ...
        if(quality == uint256(NFTQuality.NFTSilver)){
            ...
            uint256[] memory cids;
            uint256 count;
            (cids, count) = GetSameShipCamp(copperShip[IDToIndex[_id]].camp, uint256(NFTQuality.NFTCopper));
            ...
            nftAddress.batchMint(msg.sender, destcids, counts, "");
            emit newDecompose(msg.sender, _id, destcids, counts);
        }else if(quality == uint256(NFTQuality.NFTGold)){
            nftAddress.burn(msg.sender, _id, 1);

            uint256[] memory cids;
            uint256 count;
            (cids, count) = GetSameShipCamp(copperShip[IDToIndex[_id]].camp, uint256(NFTQuality.NFTSilver));
    ...
    */
    ...
    function modifyCastCost(uint256 _cost) public onlyOwner(){
        require(_cost > 0 , "DBSNFTTechnology: modify Cast Cost 0");
        castCost = _cost;
    }
}
