...
contract FundFactory is Initializable, UUPSUpgradeable, OwnableUpgradeable, IFundFactory {
    ...
    function newFund(FundInfo calldata fundInfo) public returns(address) {
        address manager = msg.sender;

        interaction.newFund(fundInfo.fundId, fundInfo.fundName, fundInfo.investPeriod, 100, 100, manager);
        fees.newFund(fundInfo.fundId, fundInfo.performanceFee, fundInfo.subscriptionFee, fundInfo.managementFee);

        emit FundCreated(manager, fundInfo.fundId, fundInfo.fundName,
            fundInfo.performanceFee, fundInfo.subscriptionFee, fundInfo.managementFee,
            fundInfo.investPeriod
        );
        return fundInfo.fundId;
    }
}
...
interface IFundFactory {
    ...
    function newFund(FundInfo calldata fundInfo) external returns(address);
}
