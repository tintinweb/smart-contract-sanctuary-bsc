...
abstract contract Multisig {
    ...
    function submitMint(uint256 _amount)
        external
    {
        ...
        emit SubmitMint(msg.sender, txIndex, address(this), _amount);
    }
    ...
    function submitUpdateMaxsupply(uint256 _amount)
        external
    {
    ...
    function submitUpdateMinter(address _minter)
        external
        isOwnerOrAuthorizer
    {
        uint256 txIndex = transactions.length;
        transactions.push(
            Transaction({
                transactionType: TransactionType.UpdateMinter,
                to: _minter,
                from: address(0),
                amount: 0,
                isAuth: false,
        ...
        emit SubmitUpdateMinter(msg.sender, txIndex, _minter);
    }
    event SubmitUpdateMinter(address indexed account, uint256 indexed txIndex, address minter);

    function submitUpdateCapper(address _cap)
        external
        isOwnerOrAuthorizer
    {
        uint256 txIndex = transactions.length;
        transactions.push(
            Transaction({
                transactionType: TransactionType.UpdateCapper,
                to: _cap,
                from: address(0),
                amount: 0,
                isAuth: false,
        ...
        emit SubmitUpdateCapper(msg.sender, txIndex, _cap);
    }
    event SubmitUpdateCapper(address indexed account, uint256 indexed txIndex, address capper);
        ...
        notExecuted(_txIndex)        
    {
        ...
        if (transaction.numConfirmations >= numConfirmationsRequired) {
            if ((transaction.isOwnerConfirmationRequired && isConfirmed[_txIndex][owner()])
                ...
                } else if (transaction.transactionType == TransactionType.UpdateMinter) {
                    emit UpdateMinter(minter, transaction.to);
                    minter = transaction.to;
                    transaction.executed = true;
                ...
                } else if (transaction.transactionType == TransactionType.UpdateCapper) {
                    emit UpdateCapper(cap, transaction.to);
                    cap = transaction.to;
                    transaction.executed = true;
                }
    ...
    event UpdateCapper(address oldCapper, address newCapper);
...
contract LGTEST is IBEP20, Multisig, Context {
  using SafeMath for uint256;
