...
contract BeeSkyv2 is Ownable {
  ...
  address public GLOVERNANCE = address(0);
  
  uint256 public INITIAL_PRICE = 100000000000000;
  uint256 public CHANGERATE_PRICE = 400000;
  ...
  mapping(uint256 => address) public refferallink;
  ...
  constructor() Ownable(msg.sender) {
    ...
    fee.refer[5] = 5;
    fee.withdraw = 50;
    Package memory newpack1 = Package( 50, 40);
    Package memory newpack2 = Package( 500, 450);
    Package memory newpack3 = Package( 1000, 1000);
    Package memory newpack4 = Package( 5000, 5500);
    Package memory newpack5 = Package( 10000, 12000);
    Package memory newpack6 = Package( 30000, 39000);
  ...
  function newPackage(uint256[] memory _data) public onlyOwner {
    require(_data.length == 2,"revert by arguments length");
    Package memory newpack = Package( _data[0], _data[1]);
  ...
  function getBuyPrice(address checker) external view returns (uint256) {
    ...
    if(isAdmin[checker]){
      ...
      buyrate = 1300000000000000000;
    }
    IERC20 token = IERC20(GLOVERNANCE);
    uint256 currentsupply = token.circularSupply();
    return buySQRT(buyrate,INITIAL_PRICE,CHANGERATE_PRICE,currentsupply);
  }
