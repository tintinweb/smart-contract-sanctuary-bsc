...
contract MarketplaceSecondary is Ownable, IERC1155Receiver {
    ...
    struct Bid {
        address bidder; // address that makes the bid
        uint price; // bid price
        uint timestamp; // timestamp of the bid
    }
    ...
    mapping (uint => Bid[]) public bidsByIndex;
    ...
    event Claim(address indexed buyer, address indexed collection, uint tokenId, uint price, address paymentToken, bool isERC721);
    event NewBid(address indexed bidder, uint amount, address indexed paymentToken, address indexed collection);
     ...
     * @param _minBidPriceStep Minimum amount the price of a bid has to change from the previous one
     * @dev set marketplaceFee and maxFee
     */
    constructor (address _treasury, uint _marketplaceFee, uint _maxFee, uint _minBidPriceStep){
        marketplaceFee = _marketplaceFee;
        maxFee = _maxFee;
        minimumBidPriceIncrease = _minBidPriceStep;
     ...
     * @param _baseBid Base price for the auction. If 0, auction is diabled for this item
    ...
    function sell721(address _collection, uint _tokenId, uint _price,
        uint _baseBid, uint _sellStart, uint _sellEnd, address _paymentToken) external {
     ...
     * @param _index Index of the sell order
     * @param _price New bid
     */
    function bid721(uint _index, uint _price) external payable {
        require (_index < nextSellId, "SecondaryMarketplace: Invalid index");

        Sell storage sell = sellOrdersById[_index];
        require (sell.active && sell.baseBid > 0, "SecondaryMarketplace: Not in auction");
        require (block.timestamp >= sell.start && block.timestamp < sell.end, "SecondaryMarketplace: Not in sale");

        (uint lastBidPrice, address lastBidder) = getLastBid(_index);
        require (_price >= lastBidPrice, "SecondaryMarketplace: Price too low");

        // Check price increase
        if (_price > sell.baseBid) {
            require (_price - lastBidPrice >= minimumBidPriceIncrease, "SecondaryMarketplace: Price increase too low");
        }

        // Refund old bidder
        if (lastBidder != address(0)) {
            processPaymentTo(sell.paymentToken, lastBidder, lastBidPrice);
        }

        // Process payment
        processPaymentFrom(sell.paymentToken, msg.sender, _price);
        ...
        bidsByIndex[_index].push(bid);

        emit NewBid(msg.sender, _price, sell.paymentToken, sell.collection);
    }

    /**
     * @notice Claim a NFT after auction and sale ended
     * @param _index Index of the sell order
     */
    function claim721(uint _index) external {
        require (_index < nextSellId, "SecondaryMarketplace: Invalid index");

        Sell storage sell = sellOrdersById[_index];
        require (block.timestamp >= sell.end, "SecondaryMarketplace: Sale not ended");
        require (sell.active, "SecondaryMarketplace: Already claimed or sold");

        (uint lastBid, address bidder) = getLastBid(_index);
        require (bidder == msg.sender, "SecondaryMarketplace: Not bidder");

        // Check royalties with ERC2981
        uint sellerAmount = lastBid;
        ...
        uint fee = lastBid * marketplaceFee / feePrecision;
        ...
        emit BuyOrder(msg.sender, sell.collection, sell.tokenId, lastBid, sell.paymentToken, true);
    }
    ...
    function buy721(uint _index) external payable {
        ...
        // Check if the auction price is bigger than sale price
        (uint lastBid, address bidder) = getLastBid(_index);
        require (lastBid < sell.price, "SecondaryMarketplace: Auction price exceeds sale price");

        // Refund last bidder
        if (bidder != address(0)) {
            processPaymentTo(sell.paymentToken, bidder, lastBid);
        }
     ...
     * @param _paymentToken Payment Token used (if 0x000... BNB will be used)
     */
    function sell1155(address _collection, uint _tokenId, uint _amount, uint _price, uint _baseBid, uint _sellStart, uint _sellEnd, address _paymentToken) external {
     ...
     * @param _price New bid
     */
    function bid1155(uint _index, uint _price) external payable {
        require (_index < nextSellId, "SecondaryMarketplace: Invalid index");

        Sell storage sell = sellOrdersById[_index];
        require (sell.active && sell.baseBid > 0, "SecondaryMarketplace: Not in auction");
        require (block.timestamp >= sell.start && block.timestamp < sell.end, "SecondaryMarketplace: Not in sale period");
     ...
     * @param _index Index of the sell order
     */
    function claim1155(uint _index) external {
        require (_index < nextSellId, "SecondaryMarketplace: Invalid index");
        ...
        // Pay the seller
        processPaymentTo(sell.paymentToken, sell.owner, lastBid - fee);
        ...
        emit BuyOrder(msg.sender, sell.collection, sell.tokenId, lastBid, sell.paymentToken, false);
    }
    ...
    /**
     * @notice Returns the list of bids for a specific order
     * @param _index Order index
     */
    function getBidsByIndex(uint _index) external view returns (Bid[] memory) {
        return bidsByIndex[_index];
    }
    ...
    // Returns last bid price and address of the bidder
    function getLastBid(uint _index) public view returns (uint, address) {
        uint baseBid = sellOrdersById[_index].baseBid;

        if (bidsByIndex[_index].length > 0) {
            uint lastBidIndex = bidsByIndex[_index].length - 1;
            Bid memory bid = bidsByIndex[_index][lastBidIndex];

            return (bid.price, bid.bidder);
        }

        return (baseBid, address(0));
    }
}
