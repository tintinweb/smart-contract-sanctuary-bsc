...
contract Exchange4JID is Ownable, IERC721Receiver {
    struct Info {
        uint256 price;
        address owner;
    }
    ...
    function sellNFT(uint256 _tokenID, uint256 _NFTPrice) public {
        ...
        if (!levelPermission) {
            require(NFTContract.identityLevel(_tokenID) > 0, "Exchange4JID: NFT level is normal.");
        }
    ...
    function setLevelPermission(bool _levelPermission) public onlyOwner {
        levelPermission = _levelPermission;
    }

    function setExchange(IERC721Enumerable _NFTContract, address _tokenAddress) public onlyOwner {
        NFTContract = _NFTContract;
        tokenAddress = _tokenAddress;
    }
}