...
contract KnightFamily is KnightHelper {
    ...
    event RequestMarry(uint _IDknightRequest, uint _IDknightResponse, address _from, address _to, uint amountGift);
    event ApprovalMarry(uint _IDknightRequest, uint _IDknightResponse, bool _resoult);
    event DestroyMarry(uint _IDknightRequest, uint _IDknightResponse);
    event TriggerTired(uint _knightID, uint _timeOut);

    modifier acceptedMarry(uint _IDknightRequest, uint _IDknightResponse) {
        require(registerMarry[_IDknightRequest][_IDknightResponse], "Invalid marriage consent");
        _;
    }
    ...
    {
        require(!registerMarry[_IDknightRequest][_IDknightResponse] &&  !registerMarry[_IDknightResponse][_IDknightRequest], "You have approved this marriage!");
        require(!knightMarried[_IDknightRequest] && !knightMarried[_IDknightResponse], "Knight is married!");
        if(_resoult) {
            registerMarry[_IDknightRequest][_IDknightResponse] = true;
            registerMarry[_IDknightResponse][_IDknightRequest] = true;
            knightMarried[_IDknightRequest] = true;
            knightMarried[_IDknightResponse] = true;
            uint moneyWedding = amountGift[ownerOf(_IDknightRequest)][ownerOf(_IDknightResponse)];
            if(moneyWedding >= minAmountgift) {
                payable(ownerOf(_IDknightResponse)).transfer(amountGift[ownerOf(_IDknightRequest)][ownerOf(_IDknightResponse)] - weddingExpenses);
            }
            delete amountGift[ownerOf(_IDknightRequest)][ownerOf(_IDknightResponse)];
        } else {
    ...
    function destroyMarry(uint _IDknightRequest, uint _IDknightResponse) 
        external  
    {
        require(ownerOf(_IDknightRequest) == msg.sender || ownerOf(_IDknightResponse) == msg.sender,"Invalid owner of knight");
        if(ownerOf(_IDknightRequest) == _msgSender() || ownerOf(_IDknightResponse) == _msgSender())
        {
            if(registerMarry[_IDknightRequest][_IDknightResponse]){
                _destroyMarry(_IDknightRequest, _IDknightResponse, false);
            } else {
                _destroyMarry(_IDknightRequest, _IDknightResponse, true);
            }
        }  
        emit DestroyMarry(_IDknightRequest, _IDknightResponse);
    }

    function _destroyMarry(uint _IDknightRequest, uint _IDknightResponse, bool _repay) 
        internal  
    {
        delete registerMarry[_IDknightRequest][_IDknightResponse];
        delete registerMarry[_IDknightResponse][_IDknightRequest];    
        delete knightMarried[_IDknightRequest];
        delete knightMarried[_IDknightResponse];  

        if (_repay) {
            payable(ownerOf(_IDknightRequest)).transfer(amountGift[ownerOf(_IDknightRequest)][ownerOf(_IDknightResponse)] - weddingExpenses);
            delete amountGift[ownerOf(_IDknightRequest)][ownerOf(_IDknightResponse)];
        }
    }

    function requestMarry(uint _IDknightRequest, uint _IDknightResponse) 
        external payable
        onlyOwnerOfKnight(_IDknightRequest)
    {
        require(msg.value >= minAmountgift, "Gift quantity less than 0.006 ether");
        require(!knightMarried[_IDknightRequest] && !knightMarried[_IDknightResponse], "Knight is married!");
        registerMarry[_IDknightRequest][_IDknightResponse] = false;
        amountGift[ownerOf(_IDknightRequest)][ ownerOf(_IDknightResponse)] = msg.value;
        emit RequestMarry(_IDknightRequest, _IDknightResponse, ownerOf(_IDknightRequest),  ownerOf(_IDknightResponse),  msg.value);
    }

    function interCourseKnight(uint _IDknightRequest, uint _IDknightResponse) 
        external 
        aboveLevel(1, _IDknightRequest) 
        aboveLevel(1, _IDknightResponse) 
        acceptedMarry(_IDknightRequest, _IDknightResponse) 
    {
        Knight storage myKnight = listOfKnight[_IDknightRequest];
        Knight storage loverKnight = listOfKnight[_IDknightResponse];
        require(_isReadySex(myKnight) && _isReadySex(loverKnight) ,"Knight can't fight yet");
        _reproductionKnight(myKnight, loverKnight, _IDknightRequest, _IDknightResponse);
    }
        ...
        internal 
    {
        ...
        if(randWhoParent <= 5) 
        {
            _createKnight("BabyKnight", newDna, uint8(randGender), avgLevel, ownerOf(_IDknightRequest));
        } else  {
            _createKnight("BabyKnight", newDna, uint8(randGender), avgLevel, ownerOf(_IDknightResponse));
        }
...
contract KnightFactory is KnightToken {
    ...
    function getInitKnight(address _owner) external view returns(bool) {
...
contract KnightToken is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("KnightCrypto", "KNIT") {}
