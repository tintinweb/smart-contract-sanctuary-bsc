...
contract DAIVault is Initializable, ReentrancyGuard, Pausable {
    ...
    */
    ...
    function deposit(address user, uint256 amount, bool isBUSD) external nonReentrant _completePause whenNotPaused() {
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "DAIvault: address is Blacklisted");
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "DAIVault: you can't interact in same block");
        }
    ...
    function claimReobaseReward(address user) public nonReentrant _completePause whenNotPaused {
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "DAIVault: you can't interact in same block");
            require(!_isContract(msg.sender),"DAIVault: No external contract interaction");
        }
        _claimRebaseReward(user);
    }
    ...
    */
    ...
    function _withdraw(address user, uint256 _amount, address sendTo) nonReentrant _completePause internal {
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "DAIVault: address is Blacklisted");
        require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender) || (sendTo == msg.sender && user == msg.sender),"DAIVault: No external contract interaction");
    ...
    //             "DAIVault: The difference b/w prev and current is greater than or less than the slippage");
