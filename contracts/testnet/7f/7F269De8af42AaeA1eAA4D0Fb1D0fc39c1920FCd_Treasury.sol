...
contract Treasury is ContractGuard {
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getEverestPrice() > hermesPriceCeiling) ? 0 : getHermesCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    // oracle
    function getEverestPrice() public view returns (uint256 hermesPrice) {
    ...
    function getBurnableHermesLeft() public view returns (uint256 _burnableHermesLeft) {
        uint256 _hermesPrice = getEverestPrice();
    ...
    function buyBonds(uint256 _hermesAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 hermesPrice = getEverestPrice();
    ...
    function _sendToOlympus(uint256 _amount) internal {
        ...
        if (team1FundSharedPercent > 0) {
            ...
            emit TeamFundFunded(block.timestamp, _team1FundSharedAmount);
        }
        ...
        emit OlympusFunded(block.timestamp, _amount);
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        _updateHermesPrice();
        previousEpochHermesPrice = getEverestPrice();
...
interface IOlympus {
    ...
    function getEverestPrice() external view returns (uint256);
