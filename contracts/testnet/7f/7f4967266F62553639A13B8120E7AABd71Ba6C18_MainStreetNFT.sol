...
interface IMainstStaker {
    ...
    function addHolder(address holderAddress) external;
    function removeHolder(address holderAddress) external;
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function setStakerAddress(address stakerAddress) internal virtual onlyOwner {
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        ...
        IMainstStaker(STAKER_ADDRESS).removeHolder(from);
        IMainstStaker(STAKER_ADDRESS).addHolder(to);
...
contract MainStreetNFT is ERC721, ReentrancyGuard{
//    using SafeMath for uint256;
    using Strings for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    uint256 public nftPrice; //0.4 BNB
    ...
    constructor(uint _maxSupply) ERC721("MAINSTREET", "MAINST") {
    ...
    function setStakingContractAddress(address payable _stakingContractAddress) public onlyOwner{
        ERC721.setStakerAddress(_stakingContractAddress);
    ...
    */
    ...
    function mintNFT() public payable nonReentrant() {
        ...
        require(msg.value >= nftPrice, "Not enough bnb balance");
        IMainstStaker(stakingContract).addHolder(msg.sender);
        IMainstStaker(stakingContract).deposit{value: msg.value}();
        _tokenIds.increment();
    ...
    function mintFor(address _address) public onlyOwner() {
        require(bytes(baseURI).length > 0, 'Base URI is not set.');
        require(stakingContract != address(0), 'Staking contract address not set.');
        require(saleIsActive, "Sale must be active to mint nft");
        require((totalSupply() + 1) <= MAX_SUPPLY, "Purchase would exceed max supply of NFTs");
