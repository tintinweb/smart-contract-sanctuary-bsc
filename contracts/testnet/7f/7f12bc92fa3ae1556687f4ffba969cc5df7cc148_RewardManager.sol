...
//    rewards[_user][dayPassed].push(Reward(_mission, userShare, farmShareAmount, referralShareAmount, landLordShareAmount, _heroIds));
  ...
  function syncPendingReward(address _user, uint[] calldata _dayToClaims, uint[][] calldata _fotaPrices) external onlyMainAdmin {
    uint dayToClaim;
    for (uint dayIndex = 0; dayIndex < _dayToClaims.length; dayIndex++) {
      dayToClaim = _dayToClaims[dayIndex];
      require(!claimMarker[_user][dayToClaim], "RewardManager: user has claimed this day reward");
      require(rewards[_user][dayToClaim].length == _fotaPrices[dayIndex].length, "RewardManager: price data invalid");
      for (uint gameIndex = 0; gameIndex < _fotaPrices[dayIndex].length; gameIndex++) {
        rewards[_user][dayToClaim][gameIndex].userAmount = rewards[_user][dayToClaim][gameIndex].userAmount * 1000 / _fotaPrices[dayIndex][gameIndex];
        rewards[_user][dayToClaim][gameIndex].farmShareAmount = rewards[_user][dayToClaim][gameIndex].farmShareAmount * 1000 / _fotaPrices[dayIndex][gameIndex];
        rewards[_user][dayToClaim][gameIndex].referralShareAmount = rewards[_user][dayToClaim][gameIndex].referralShareAmount * 1000 / _fotaPrices[dayIndex][gameIndex];
        rewards[_user][dayToClaim][gameIndex].landLordShareAmount = rewards[_user][dayToClaim][gameIndex].landLordShareAmount * 1000 / _fotaPrices[dayIndex][gameIndex];
        userDailyPending[_user][dayToClaim] += rewards[_user][dayToClaim][gameIndex].userAmount;
      }
    }
  }
}
