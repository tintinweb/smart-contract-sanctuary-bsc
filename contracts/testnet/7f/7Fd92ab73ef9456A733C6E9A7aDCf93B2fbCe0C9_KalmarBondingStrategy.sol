...
contract KalmarBondingStrategy is ReentrancyGuard, Ownable, Pausable {
    ...
    // Oracle kalm price
    uint256 public kalmPriceOracle;
    // admin
    address admin;
    // oracle setter
    ...
    function usdPrice() external pure returns (uint256) {
    ...
    function updateKalmPriceOracle(uint256 _price) external onlyOracleSetter {
      kalmPriceOracle = _price;
    }

    /* ========== RESTRICTED FUNCTIONS ========== */
    function _usdTokenPrice() internal pure returns (uint256) {
    ...
    function _bondPrice() internal view returns (uint256) {
      uint256 length = bondingEmission.length;
      uint256 priceDiscount = kalmPriceOracle.mul(bondingEmission[length-1].discount).div(DISCOUNT_FACTOR);
      uint256 bondPrice = kalmPriceOracle.sub(priceDiscount);
      return bondPrice;
    }
    ...
    function _getLpPrice() internal view returns(uint256)
    {
      ...
      uint256 totalKalmPrice = kalmReserve.mul(kalmPriceOracle);
    ...
    function addOracleSetter(address _setter) public onlyAdmin
    {
        oracleSetter[_setter] = true;
        emit AddOracleSetter(_setter);
    }

    function removeOracleSetter(address _setter) public onlyAdmin
    {
