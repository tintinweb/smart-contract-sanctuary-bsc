...
// <=================================== INTERNAL FUNCTION ===================================>
    ...
    function _unboxDetail(
        ...
        if(giftType == ItemType.Box) {
            console.log("boxNft typeBox: %s", boxNft[0].typeBox);
            console.logBytes32(boxNft[0].hashData);
            console.log("boxNft typeBox: %s", boxNft[0].cid);
            hashData = keccak256(abi.encodePacked(boxNft[0].typeBox, boxNft[0].hashData, boxNft[0].cid));
            (, bytes32 boxData) = boxContract.boxInformation(boxId);
            require(hashData == boxData, "Data is invalid");
            console.logBytes32(hashData);
            console.logBytes32(boxData);
