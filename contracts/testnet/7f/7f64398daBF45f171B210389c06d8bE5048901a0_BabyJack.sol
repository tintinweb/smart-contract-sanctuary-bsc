...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public dividendToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; // BTCB 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c
...
contract BabyJack is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BabyJackDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BabyJack", "BJCK") {
    	...
    	dividendTracker = new BabyJackDividendTracker();

    	buyBackWallet = 0x39786c7F9F93CA4D998D93BBa1151042A4060ED9;
    ...
    receive() external payable {
  	...
  	function setMaxSellTransaction(uint256 maxTxn) external onlyOwner {
        require(maxTxn >= 1500000000, "BabyJack: Max sell transaction must be large 1500000000");
  	    maxSellTransactionAmount = maxTxn * (10**18);
  	}
  	
  	function setMaxWalletToken(uint256 maxToken) external onlyOwner {
        require(maxToken >= 1500000000, "BabyJack: Max token must be large 1500000000");
  	    maxWalletToken = maxToken * (10**18);
  	}
  	
  	function setSellTransactionMultiplier(uint256 multiplier) external onlyOwner {
  	    require(sellFeeIncreaseFactor >= 100 && sellFeeIncreaseFactor <= 200, "BabyJack: Sell transaction multipler must be between 100 (1x) and 200 (2x)");
  	    sellFeeIncreaseFactor = multiplier;
  	}
  	
  	function setMarketingDivisor(uint256 divisor) external onlyOwner {
  	    require(marketingDivisor >= 0 && marketingDivisor <= 100, "BabyJack: Marketing divisor must be between 0 (0%) and 100 (100%)");
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "BabyJack: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyJack: The dividend tracker already has that address");

        BabyJackDividendTracker newDividendTracker = BabyJackDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyJack: The new dividend tracker must be owned by the BabyJack token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "BabyJack: Dividend reward tax must be between 0 and 10");
        dividendRewardsFee = newFee;
    }
    
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 12, "BabyJack: Marketing tax fee must be between 0 and 12");
        marketingFee = newFee;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyJack: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyJack: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyJack: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyJack: Automated market maker pair is already set to that value");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "BabyJack: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyJack: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyJack: Cannot update gasForProcessing to same value");
...
contract BabyJackDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BabyJack_Dividend_Tracker", "BabyJack_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 20000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "BabyJack_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "BabyJack_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyJack contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyJack_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BabyJack_Dividend_Tracker: Cannot update claimWait to same value");
