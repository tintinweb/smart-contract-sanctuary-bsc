    ...
    ERC721HolderUpgradeable
{
    ...
    function setOfferItem(
        OfferItem memory _offerItem
    ) external onlyRole(GATEWAY_ROLE) {
        offerItems[_offerItem.itemId] = _offerItem;
    }

    function getOfferItem(
        uint256 _id
    ) external view returns (OfferItem memory) {
        return offerItems[_id];
    }
}