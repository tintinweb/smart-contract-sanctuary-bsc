...
contract BlockChatUpgradeable2 is IBlockChatUpgradeable2, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function createMessageToRecipient(bytes32 recipientHash, string memory content) external override {
        messageLength++;
        messageMap[messageLength] = Message(
            keccak256(abi.encodePacked(msg.sender, recipientHash, content, block.timestamp)),
            block.number
        );
        recipientMessageListMap[recipientHash].push(messageLength);
        emit MessageCreatedToRecipient(messageLength, msg.sender, recipientHash, content, block.timestamp);
    }
...
interface IBlockChatUpgradeable2 {
    ...
    event MessageCreatedToRecipient(
        uint256 indexed messageId,
        address indexed sender,
        bytes32 recipientHash,
        string content,
    ...
    function createMessageToRecipient(bytes32 recipientHash, string memory content) external;
