...
contract TokenContract is Context, IBEP20, Ownable {
    string private _name = "testT2"; // token name
    string private _symbol = "TT2"; // token symbol
    ...
    IdexRouter public dexRouter; // dex ruoter address
    address public dexPair; // LP token address
    address public address1; // address to collect Fee1
    address public address2; // address to collect Fee2
    address public address3; // address to collect Fee3
    ...
    bool public sellLimitValid; // should be true to turn on sell count validation
    bool public priceLimitValid; // should be true to turn on price drop validation
    bool public priceSwitch; // to get the price from Dex
    bool public contractState = true; // owner can pause the contract.
    ...
    uint256 public fee1 = 20; // 2% will go to the fee address 1
    uint256 public fee2 = 20; // 2% will go to the fee address 2
    uint256 public fee3 = 20; // 2% will go to the fee address 3
    ...
    function setWhiteList(address account, bool _state) public onlyOwner {
        _whiteList[account] = _state;
    }

    function setGreyList(address account, bool _state) public onlyOwner {
        _greyList[account] = _state;
    }

    function setBlackList(address account, bool _state) public onlyOwner {
    ...
    function setSellCoolDownDuration(
        bool _state,
        uint256 _duration,
        uint256 _count
    ) external onlyOwner {
    ...
    function setPriceLimit(bool _state, uint256 _declinePercent)
        external
        onlyOwner
    {
        priceLimitValid = _state;
        declinedPricePercent = _declinePercent;
    }

    function switchPriceState(bool _state) external onlyOwner {
        priceSwitch = _state;
    }
    ...
    function setdexRouter(IdexRouter _dexRouter) external onlyOwner {
        dexRouter = _dexRouter;
    }
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        uint256 percentage = (tAmount * (lpFee + (fee1) + (fee2) + (fee3))) /
            (divider);
        return percentage;
    }
    ...
    function getCurrentPrice() public view returns (uint256) {
        ...
        uint256 Wbnb;
        if (priceSwitch) {
            (token, Wbnb, ) = IdexPair(dexPair).getReserves();
        } else {
            (Wbnb, token, ) = IdexPair(dexPair).getReserves();
        }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            require(contractState, "NT: Contract paused by owner");
        }
            ...
            if (block.timestamp < lastSellTime + (sellCoolDownDuration)) {
                if (sellLimitValid) {
                    require(
                        ...
                        "NT: wait for selling cool down"
                    );
                    sellCount++;
                }
                if (priceLimitValid) {
                    uint256 minPriceLimit = lastClosingPrice -
                        ((lastClosingPrice * declinedPricePercent) / divider);
                    require(
                        getCurrentPrice() > minPriceLimit,
                        "NT: token price droped more than limit"
                    );
                }
            } else {
                sellCount = 0;
    ...
    function _transferWithFee(
        ...
        } else if (balanceOf(sender) > maxHoldAmount) {
            transferAmount -= (tAmount * (additionalFee)) / divider;
            _takeAdditionalFee(sender, tAmount);
        }
