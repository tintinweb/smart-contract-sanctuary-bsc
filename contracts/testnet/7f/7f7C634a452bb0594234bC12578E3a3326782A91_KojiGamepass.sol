...
contract KojiGamepass is ERC721URIStorage, ERC165, Ownable, ReentrancyGuard {
    ...
    function viewHashes(address _address, string memory _holder) public pure returns (bytes32, bytes32) {
        ...
        bytes32 first = keccak256(bytes(AddresstoString(_address)));

        bytes32 second = keccak256(bytes(StringtoString(_holder)));
    ...
    function viewBytes(address _address, string memory _holder) external pure returns (bytes memory, bytes memory) {
        return (bytes(AddresstoString(_address)), bytes(StringtoString(_holder)));
    }

    function viewABIPacked(address _address, string memory _holder) external pure returns (string memory, string memory) {
        return (AddresstoString(_address), StringtoString(_holder));
    }
    ...
    */
    ...
    function StringtoString(string memory _string) public pure returns(string memory) {
