...
contract ArkarusTestFee is Context, IBEP20, Ownable {
  ...
  address public feeReciveAddress;
  address public pancakeLPAddress;

  constructor() public {
    _name = "ArkarusTestFee";
    _symbol = "AKSTESTFEE";
    _decimals = 18;
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) override external returns (bool) {
    if(_msgSender() == pancakeLPAddress) {
      if(buyFee > 0) {
        _transfer(_msgSender(), feeReciveAddress, amount * buyFee / 100);
  ...
  function transferFrom(address sender, address recipient, uint256 amount) override external returns (bool) {
    if(recipient == pancakeLPAddress) {
      if(sellFee > 0) {
        _transfer(sender, feeReciveAddress, amount * sellFee / 100);
        _transfer(sender, recipient, amount * (100 - sellFee)/100);
      }
      else {
        _transfer(_msgSender(), recipient, amount);
      }
  ...
  function setFeeReciveAddress(address _feeReciveAddress) external onlyOwner {
    feeReciveAddress = _feeReciveAddress;
  }
