...
contract ICO is Initializable{
    ...
    function initialize (ICOparam memory _data, uint[] calldata _time, uint[] calldata _percent) external initializer{
        icoInfo = _data;
    ...
    function setVesting(uint[] calldata _time, uint[] calldata percentage) internal{
        for(uint i=0; i< time.length; i++){
            _setVesting(_time[i], percentage[i]);
        }
    }
    uint[] time;
    uint[] percent;
