...
contract Arcadian is BEP20Burnable, Ownable{
	...
	function updateFee(uint256 _txFee,uint256 _liquidityFee,uint256 _charityFee) onlyOwner() public{
        _TAX_FEE = _txFee* 100; 
        _LIQUIDITY_FEE = _liquidityFee * 100;
		_COMPANY_FEE = _charityFee* 100;
		ORIG_TAX_FEE = _TAX_FEE;
		ORIG_LIQUIDITY_FEE = _LIQUIDITY_FEE;
		ORIG_COMPANY_FEE = _COMPANY_FEE;
	}
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(!inTransfer){
            ...
            if (sender == uniswapV2Pair && !_isExcluded[sender] || recipient == uniswapV2Pair && !_isExcluded[sender]) {
                _transferBothToFee(sender, recipient, amount);
            } else {
                removeAllFee();
    ...
    function _transferBothToFee(address sender, address recipient, uint256 tAmount) private {
        (uint256 tFee, uint256 tLiquidity, uint256 tCompany) = _getValuesFeeT(tAmount);
        uint256 tTransferAmount = getTTransferAmount(tAmount, tFee, tLiquidity, tCompany);
        _reflectFee(tAmount);
        super._transfer(sender, address(this), tFee);
        super._transfer(sender, LiquidAddress, tLiquidity);
        super._transfer(sender, CompanyAddress, tCompany);
    ...
    function _reflectFee(uint256 tAmount) private {
        ...
        _tTotal = _tTotal.sub(tFee).sub(tLiquidity).sub(tCompany);
        emit FeeTransaction(tFee, tLiquidity, tCompany);
    }

    function _getValuesFeeT(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        return (tFee, tLiquidity, tCompany);
    }
    
    function getTTransferAmount(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tCompany) private pure returns (uint256) {
        return tAmount.sub(tFee).sub(tLiquidity).sub(tCompany);
    }

    function _getRValues(uint256 tFee) private view returns (uint256) {
        uint256 currentRate =  _getRate();
    ...
    function _swapTokensForBNB(uint256 tokenAmount, address toAdress) private {
        address[] memory path = new address[](2);
        path[0] = toAdress;
        path[1] = uniswapV2Router.WETH();

        _approve(toAdress, address(uniswapV2Router), tokenAmount);
