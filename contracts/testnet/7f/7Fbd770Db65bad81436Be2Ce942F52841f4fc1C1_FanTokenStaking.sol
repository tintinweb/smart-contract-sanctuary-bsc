    ...
    TokenSaver
{
    ...
    //uint32 public constant monthInSeconds = 60;
   ...
   */
    function deposit(address _depositToken, uint256 _amount, uint8 _stakingPeriodMonth, address _receiver) external lock {
        ...
        uint256 claimPeriod = (_endPeriod - startPeriod) / monthInSeconds;
   ...
   */
    function claimAndWithdraw(uint256 _depositId) public lock {
    ...
    */
    function bathClimeAndWithdraw() external {
        ...
        for (uint256 i = 0; i < depositsOf[_msgSender()].length; i++) {
            if (block.timestamp >= depositsOf[_msgSender()][i].end
                ...
                totalAmount += depositsOf[_msgSender()][i].amount;
            }
        }

        require(totalAmount >= minimumDeposit, "FanTokenStaking.bathClimeAndWithdraw: no deposits for the clime");

        for (uint256 i = 0; i < depositsOf[_msgSender()].length; i++) {
            if (block.timestamp >= depositsOf[_msgSender()][i].end && depositsOf[_msgSender()][i].amount > minimumDeposit) {
                claimAndWithdraw(i);
            }
        }
    }
    ...
    function distributeRewords(uint256 _startPeriod, uint256 _endPeriod) external {
        ...
        require(rewardToken.balanceOf(_msgSender()) >= totalAmount, "FanTokenStaking.addRewords: Insufficient balance");
    ...
    function getCurrentPeriod() public view returns(uint256) {
        return (block.timestamp - startPeriod) / monthInSeconds;
    }
}
