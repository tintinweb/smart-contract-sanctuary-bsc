...
// The address of the player and => the user info
   ...
   constructor (IERC20 tkn, address _platformFeeAdd, address _adminFeeAdd) public {
       tokenAdd = tkn;
       token =  IERC20(tkn);
       plaformFeeAdd = _platformFeeAdd;
       adminFeeAdd = _adminFeeAdd;
   }
   function() public payable {}
    ...
    function setNewFeeAddress(address newPlatformfeeAdd, address newAdminFeeAdd) public onlyOwner {
        plaformFeeAdd = newPlatformfeeAdd;
        adminFeeAdd =  newAdminFeeAdd;
    }
    ...
    function allocatePrizes(uint256 _gameId, uint16 teamWinner) public onlyAgent {
        Game storage game = gameInfo[_gameId];
        ...
        for(i = 0; i < count; i++){
            ...
            rewards = (bets*(fee *(totalBets / totalBetsOne))) / 10000;
            _pendingBalance[add] += rewards;
            }
           
        }
        platformFee = (((totalBets-((totalBets*fee)/10000))*1000)/10000)*6;
        adminFee = (((totalBets-((totalBets*fee)/10000))*1000)/10000)*4;

        token.transfer(plaformFeeAdd, platformFee);
        token.transfer(adminFeeAdd, adminFee);
        for(i=0; i < players.length; i++){
            add = players[i];
        ...
        for( i = 0; i < count; i++){
            ...
            rewards = (bets*(fee *(totalBets / totalBetsTwo))) / 10000;
            _pendingBalance[add] += rewards;
            }
