...
contract EverETHLPMaster is Ownable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEVTPerShare += (reward * 1e12) / lpSupply; 
        }
        return (user.amount * accEVTPerShare) / 1e12 - user.rewardDebt; 
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accEVTPerShare = (pool.accEVTPerShare + reward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = (user.amount * pool.accEVTPerShare) / 1e12 - user.rewardDebt; 
            if (pending > 0) {
        ...
        user.rewardDebt = (user.amount * pool.accEVTPerShare) / 1e12;
