...
contract Swap is Ownable {
    ...
    IERC20Mintable public _dm3Token;//9
    IERC20Mintable public _iotxToken;//99
    uint256 public rateIotxDm3 = 100;

    constructor(address dm3, address iotx) {
        _dm3Token = IERC20Mintable(dm3);
        _iotxToken = IERC20Mintable(iotx);
    }

    function swap(uint256 amount) public {
        require(amount > 0 , "amount must be greater than zero");
        _dm3Token.safeTransferFrom(msg.sender, address(this), amount);
        _iotxToken.safeTransfer(msg.sender, amount / rateIotxDm3);
    }
