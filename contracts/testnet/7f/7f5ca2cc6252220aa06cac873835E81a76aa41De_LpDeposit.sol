...
contract LpDeposit is Ownable {
    ...
    address public fuel = 0x7D08F8Df5A5adCA84c7b01d0163785F61b3b0324;
    address public usdtDaosPair = 0x7Ab357Ff5E01149923115377D47A7A47edd9D7a5;
    ...
    uint256 public totalBonusedAmount;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 effect = (pool.rewardPercent * _amount) / 100;
    ...
    function withdraw(
        ...
        if (pool.lockSecond == 0) {
            ...
            //分红业务逻辑
            uint256 bonusPedding = effect * accBonus / 1e18 - bonusCurrentDepositInfos[msg.sender].debt;
            bonusCurrentDepositInfos[msg.sender].reward += bonusPedding;
            bonusCurrentDepositInfos[msg.sender].debt = (bonusCurrentDepositInfos[msg.sender].amount - _amount) * accBonus / 1e18;
            bonusCurrentDepositInfos[msg.sender].amount -= _amount;
            bonusCurrentDepositInfos[msg.sender].effect -= effect;
            bonusWithdrawAbles[msg.sender] += bonusPedding;
        } else {
            ...
            uint256 bonusPedding = peddingForBonus(msg.sender, _pid, _index);
            bonusDepositInfo.reward += bonusPedding;
    ...
    function harvestBatch() public {
        ...
        if (totalPedding > 0) {
            IDaos(daos).mint(msg.sender, totalPedding);
        }
    }
    ...
    //充值
    function bonusRescue(address _addr, uint256 _amount) public {
        require(_amount > 0, "Cannot be zero");
        IERC20(usdtToken).transferFrom(_addr, address(this), _amount);
        totalBonusAmount += _amount;
    }

    //分红
    function award() public {
        require(totalEffectAmount !=0, "Cannot Award");
    ...
    function harvestForBonus() public {
        PoolInfo[] memory poolInfosMemory = poolInfos;
        uint256 totalBonusPedding = 0;
        for (uint256 i = 0; i < poolInfosMemory.length; i++) { 
            if (poolInfosMemory[i].lockSecond == 0) { 
                 //分红收割业务逻辑
                uint256 currentPedding = bonusCurrentDepositInfos[msg.sender].amount * accBonus / 1e18 - bonusCurrentDepositInfos[msg.sender].debt;
                bonusCurrentDepositInfos[msg.sender].debt += currentPedding;
                bonusCurrentDepositInfos[msg.sender].reward += currentPedding;
                totalBonusPedding += currentPedding;
            } else {
                ...
                for(uint256 k = 0; k < bonusDepositInfoList.length; k++) {
                    ...
                    uint256 _bonusDepositPending = peddingForBonus(msg.sender, i, k);
                    bonusDepositInfoList[k].debt += _bonusDepositPending;
                    bonusDepositInfoList[k].reward += _bonusDepositPending;
                    totalBonusPedding += _bonusDepositPending;
                }
            }
        }

        if(bonusWithdrawAbles[msg.sender] > 0) {
            totalBonusPedding += bonusWithdrawAbles[msg.sender];
            bonusWithdrawAbles[msg.sender] = 0;
        }

        if(totalBonusPedding > 0) {
            uint256 burnAmount = totalBonusPedding / 100;
            IFuel(fuel).burnFrom(msg.sender, burnAmount);
            IERC20(usdtToken).safeTransfer(msg.sender, totalBonusPedding);
        }
    }

    function peddingForBonus(address _addr, uint256 _pid, uint256 _index) public view returns(uint256) {
    ...
    //查询用户所有质押分红收益
    function peddingForAllBonus(address _addr) public view returns(uint256) {
        PoolInfo[] memory poolInfosMemory = poolInfos;
        uint256 totalBonusPedding = 0;
        for (uint256 i = 0; i < poolInfosMemory.length; i++) { 
            if (poolInfosMemory[i].lockSecond == 0) { 
                uint256 currentPedding = bonusCurrentDepositInfos[_addr].amount * accBonus / 1e18 - bonusCurrentDepositInfos[_addr].debt;
                totalBonusPedding += currentPedding;
            } else {
                BonusDepositInfo[] storage bonusDepositInfoList = bonusDepositInfos[msg.sender][i];
                for(uint256 k = 0; k < bonusDepositInfoList.length; k++) {
                    ...
                    uint256 _bonusDepositPending = peddingForBonus(_addr, i, k);
                    totalBonusPedding += _bonusDepositPending;
                }
            }
        }
        return totalBonusPedding;
    }
}
