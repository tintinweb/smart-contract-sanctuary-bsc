...
contract IDO is Auth, EIP712Upgradeable {
  ...
  event FreeBought(uint indexed eventId, address indexed buyer, uint amount, uint price, USDCurrency usdCurrency, uint timestamp);
  event WhitelistBought(uint indexed eventId, address indexed buyer, uint amount, uint price, USDCurrency usdCurrency, uint timestamp);
  ...
  function freeBuy(uint _eventId, uint _amount, USDCurrency _usdCurrency) external eventExist(_eventId) {
    ...
    emit FreeBought(_eventId, msg.sender, _amount, eventDetail.freeUser.price, _usdCurrency, block.timestamp);
  }
  ...
  function whitelistBuy(bytes calldata _signature, uint _eventId, uint _totalToken, uint _price, USDCurrency _usdCurrency) external eventExist(_eventId) {
    ...
    emit WhitelistBought(_eventId, msg.sender, _totalToken, _price, _usdCurrency, block.timestamp);
  }
