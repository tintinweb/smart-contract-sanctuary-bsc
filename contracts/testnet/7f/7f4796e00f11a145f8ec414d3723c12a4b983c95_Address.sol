...
Welcome to Gameplace

Telegram: t.me/GameplaceStore
Website: gameplace.finance | gameplace.store

Crypto-Fueled Gaming Store
Discover Gameplace : the first gaming store linked to your crypto wallet. Made for gamers, powered by blockchain.
...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xe3A4a23B54e1ebb052E94F091f458a4847C97291;
...
contract Gameplace is Context, IERC20, Ownable {
    ...
    address public _RewardWalletAddress = 0x07b0deCf3F64C807075dEB2E892356d18365d37E;
    uint256 public constant MAX = ~uint256(0);
    uint256 public _tTotal = 1000000000 * 10**18;
    uint256 public _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _tFeeTotal;
    string public _name = "Gameplace";
    string public _symbol = "GMP";
    uint8 private _decimals = 18;
    uint256 public _taxFee = 20;
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _RewardFee = 30;
    uint256 private _previousDevelopmentFee = _RewardFee;
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _taxFee = taxFee;
        require(_taxFee.add(_RewardFee).add(_liquidityFee) <= 250, 'Tax Fee too high!');
    }
    function setRewardFeePercent(uint256 developmentFee) external onlyOwner() {
        _RewardFee = developmentFee;
        require(_RewardFee.add(_taxFee).add(_liquidityFee) <= 250, 'Reward Fee too high!');
    }
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
        require(_liquidityFee.add(_taxFee).add(_RewardFee) <= 250, 'Liquidity Fee too high!');
    }
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
        uint256 currentRate =  _getRate();
        uint256 rDevelopment = tDevelopment.mul(currentRate);
        _rOwned[_RewardWalletAddress] = _rOwned[_RewardWalletAddress].add(rDevelopment);
        if(_isExcluded[_RewardWalletAddress])
            _tOwned[_RewardWalletAddress] = _tOwned[_RewardWalletAddress].add(tDevelopment);
    }
    ...
    function removeAllFee() private {
        ...
        _previousDevelopmentFee = _RewardFee;
    ...
    function restoreAllFee() private {
        ...
        _RewardFee = _previousDevelopmentFee;
