...
contract GymLevelPool is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    modifier userQualified(address _user) {
        require(
            users[_user].level != 0,
            "GymLevelPool: user not qualified for any pool"
        );
        _;
    }
    ...
    function initialize(
        ...
        poolLevelDividers[13] = 6;
        poolLevelDividers[14] = 3;
        poolLevelDividers[15] = 2;
     ...
     *         If user level in range [13, 15] - add to level pool
     *         otherwise just skip.
    ...
    /**
     * @notice Try to activate user in GymLevelPool
     *         Reverts if provided wallet not in pool levels range [13, 15]
     * @param _wallet user wallet address
     */
    function activateUser(address _wallet) external {
        uint256 userLevel = IGymMLM(gymMLM).getUserCurrentLevel(_wallet);

        require(
            _isValidLevel(userLevel),
            "GymLevelPool: this address cannot be activated"
        );

        require(!_isUserQualified(_wallet), "GymLevelPool: already in pool");

        _updateUserQualification(_wallet, userLevel);
    }
     ...
     *
     * @param _poolId id of level pool (13, 14, 15)
     */
     ...
     *
     *         goldWalletsChunk   - lvl13 - 1/6 of total daily pool
     *         platinWalletsChunk - lvl14 - 1/3 of total daily pool
     *         blackWalletsChunk  - lvl15 - 1/2 of total daily pool
     *
     *         Wallet reward distribution in pools:
     *
     *         goldWalletReward   - lvl13 - goldWalletsChunk / count(lvl13 + lvl14 + lvl15)
     *         platinWalletReward - lvl14 - goldWalletsChunk / count(lvl15 + lvl15)
     *         blackWalletReward  - lvl15 - goldWalletsChunk / count(lvl15)
     *
    ...
    function distributeRewards() external onlyWhitelisted {
        ...
        for (uint256 i = 0; i < poolLevels.length; ++i) {
            ...
            PendingRewardsPerLevel
                memory levelRewards = _pendingRewardsPerLevel(
                    amountToDistribute,
                    level
                );
             ...
             */
            totalDistributed +=
                levelRewards.walletPendingReward *
                walletsInPool;
     ...
     *         level 13 pool will also include wallets from level 14 and 15
     *         level 14 pool will include wallets from level 15
        ...
        returns (QualifiedCandidates memory)
    {
        return
            QualifiedCandidates({
                totalGold: poolWallets[13].length(),
                totalPlatin: poolWallets[14].length(),
                totalBlack: poolWallets[15].length()
            });
    }

    /**
     * @notice Get number of unique qualified candidates for each pool level
     *         level 13 pool will contain only candidates qualified to level 13
     *         level 14 pool will contain only candidates qualified to level 14
     *         same for level 15
     * @return QualifiedCandidates struct with total candidates per each pool
     */
    function getUniqueCandidatesPerLevel()
        external
        view
        returns (UniqueCandidates memory)
    {
        uint256 goldWallets = poolWallets[13].length() -
            poolWallets[14].length();
        uint256 platinWallets = poolWallets[14].length() -
        ...
        return
            UniqueCandidates({
                ...
                blackLevel: poolWallets[15].length()
            });
    }
    ...
    function getUserUnclaimedRewards(address wallet)
        external
        view
        returns (uint256)
    {
    ...
    function getUserTotalClaimedRewards(address wallet)
        external
        view
        returns (uint256)
    {
     ...
     * @param _level level (pool) id - 13, 14, 15
     ...
     */
    function _userPendingRewards(address _user)
        internal
        view
