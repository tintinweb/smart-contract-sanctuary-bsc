...
/**
 * @title DesirePredictionV2
 */
contract DesirePredictionV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public latestClaimedLockedRewardEpoch; // epoch number 
    ...
    /**
     * @notice get locked reward
     */
    function claimLockedReward() external nonReentrant notContract onlyAdmin {
        uint256 reward = _getLockedReward();
        latestClaimedLockedRewardEpoch = currentEpoch + 1;
        if (reward > 0) {
            _safeTransferBNB(address(adminAddress), reward);
        }
    }
     ...
     * @notice get locked reward
     */
    function getLockedReward() external view returns (uint256 reward)  {
        return _getLockedReward();
    }

    function _getLockedReward()
        private
        view
        returns (
            uint256 reward
    ...
    {
        for (uint256 i = latestClaimedLockedRewardEpoch; i < currentEpoch + 1; i++) {
            Round memory round = rounds[i];
            if(round.oracleCalled) {
                // BULL WINS
                if(round.closePrice > round.lockPrice) {
                    // check if the house wins
                    if(round.bullAmount == 0) {
                        reward = reward + round.rewardAmount;
                    } 
                }
