...
contract AlpacaWorker is Worker, ReentrancyGuard
{
    ...
    function Open(uint256 baseAmount) public onlyManager onlyEOA() 
    {
        ...
        uint256 loanAmount;
        {
            ...
            loanAmount = baseAmount.mul(usdReserve).div(bnbReserve);
        }
        bytes memory baseBytes = abi.encode(baseAmount);
        bytes memory minReceivedBytes = abi.encode(minReceived);
        bytes memory data = abi.encode(AddStrategy, baseBytes, minReceivedBytes);
    ...
    function SetFarmingVault(address farmingVault) public onlyOperator
    {
        require(farmingVault != address(0), "Operatorable: new FarmingVault is the zero address");
        FarmingVault = farmingVault;
    }
    function SetBaseToken(address baseToken) public onlyOperator
    {
        require(baseToken != address(0), "Operatorable: new BaseToken is the zero address");
        BaseToken = baseToken;
    }
    function SetBaseVault(address newBaseVault) public onlyOperator
    {
        require(newBaseVault != address(0), "Operatorable: new BaseToken is the zero address");
        BaseVault = newBaseVault;
    }
    function SetBalaceTokens(address[] calldata balanceTokens) public onlyOperator
    {
        BalanceTokens = balanceTokens;
    }
    function SetRemoveStrat(address removeStrat) public onlyOperator
    
    {
        RemoveStrategy = removeStrat;
    }
    function SetAddStrat(address addStrat) public onlyOperator
    {
        AddStrategy = addStrat;
    }
    function SetPancakeswapWorker(address pancakeswapWorker) public onlyOperator
    {
