...
interface NFTInterface {   //new add
...
contract Holder is Ownable,Initializable {
    ...
    // modifier checkHasNft() {   //is have nft
    // require(NFTInterface().balanceOf(msg.sender)>0,"Not an NFT owner");
    // _;
    // }
    ...
    event Lock(address indexed user, address indexed tokenAddr, uint256 indexed value, uint256 time,  uint256 orderId);
    event LockETH(address indexed user,uint256 indexed value, uint256 time, uint256 orderId);
    event UnLock(address indexed user, address indexed tokenAddr, uint256 indexed value, uint256 orderId);
    event UnLockETH(address indexed user,uint256 indexed value, uint256 orderId);
    ...
    function lock(address _tokenAddr, uint256 _amount, uint256 _seconds)  external returns (bool){
    ...
    function lockEth(address _tokenAddr,uint256 _seconds) internal  returns(bool) {
        ...
        uint oid = userOrders[msg.sender].length;
        ...
        emit LockETH(msg.sender, _amount, _seconds, oid);
        return true;
    ...
    function lockERC20(address _tokenAddr,uint256 _amount,uint256 _seconds) internal returns(bool) {
        ...
        emit Lock(msg.sender, _tokenAddr, _amount, _seconds, oid);
        return true;
    ...
    function unlock(uint _index) external returns (bool){
        ...
        if(order.tokenAddr == address(0)){
            ...
            emit UnLockETH(msg.sender,order.lockAmount, _index);
        }else{
