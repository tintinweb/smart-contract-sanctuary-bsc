...
/**
 * @dev OGStringByteHelper Library String <> Bytes32
 */
library OGStringBytes32Helper {
...
contract OGAgentBook is OGOperators {
    ...
    using OGStringBytes32Helper for string;
    ...
    mapping (bytes32 => uint256) private aNamexaID_;     
    ...
    public
    {
        ...
        aIDxAgent_[1].name = "official";
        aIDxAgent_[1].laff = 0;
        aNamexaID_["official"] = 1;
        aID_ = 1;
    }

    function enrollNewAgent(address _agentAddr, string memory _agentName, uint256 _affiliateID)
    external
    onlyOwnerOrOperator needConfigured isActivated returns (uint256) {
        require(_agentAddr != address(0x0), "Cannot be zero address");
        bytes32 _name = _agentName.stringToBytes32();
        require(_name != bytes32(0), "A name must be set");
        require(aNamexaID_[_name] != 0, "Name already taken");
        if (_affiliateID < 1) {
            _affiliateID = 1;
        } else {
        ...
        aIDxAgent_[aID_].name = _name;
        aIDxAgent_[aID_].laff = _affiliateID;
        aNamexaID_[_name] = aID_;
        emit onEnrollAgent(aID_, _name, _agentAddr, _affiliateID, now);
    }
    ...
    function getAgentNameByID(uint256 _agentID) external view needConfigured onlyAuthorized returns (bytes32) {
        require(_agentID > 0 && _agentID <= aID_, "Agent doesnt exist");
        return aIDxAgent_[_agentID].name;
    }

    function getAgentAddrByID(uint256 _agentID) external view needConfigured onlyAuthorized returns (address) {
        require(_agentID > 0 && _agentID <= aID_, "Agent doesnt exist");
        return aIDxAgent_[_agentID].addr;
    }

    function getAgentAffiliateIDByID(uint256 _agentID) external view needConfigured onlyAuthorized returns (uint256) {
        require(_agentID > 0 && _agentID <= aID_, "Agent doesnt exist");
        return aIDxAgent_[_agentID].laff;
    }

    function getAgentIDByAddr(address _agentAddr) external view needConfigured onlyAuthorized returns (uint256) {
        require(_agentAddr != address(0x0), "Agent doesnt exist");
        return addrxaID_[_agentAddr];
    }

    function getAgentEnableByID(uint256 _agentID) external view needConfigured onlyAuthorized returns (bool) {
        require(_agentID > 0 && _agentID <= aID_, "Agent doesnt exist");
        return aAddrxEnable_[aIDxAgent_[_agentID].addr];
    }

    function getAgentEnableByAddr(address _agentAddr) external view needConfigured onlyAuthorized returns (bool) {
        require(_agentAddr != address(0x0), "Agent doesnt exist");
