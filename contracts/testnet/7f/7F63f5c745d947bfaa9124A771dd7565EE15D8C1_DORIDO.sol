...
import "../interfaces/INDCPackage.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract DORIDO is SafeOwnable {
    using SafeERC20 for IERC20;

    IERC20 public dor;
    INDCPackage public nftPackage;
    ...
    constructor(address _dor, address _nftPackage) {
        dor = IERC20(_dor);
        nftPackage = INDCPackage(_nftPackage);
    ...
    function shop(uint256 amount) external {
        require(isAllowCollect, "not isAllowCollect");
        require(amount >= tokenAmount, "tokenAmount min");
        dor.safeTransferFrom(msg.sender, hole, amount);

        nftPackage.mint(msg.sender, 1);
        emit Shop(msg.sender, amount);
    }
}
