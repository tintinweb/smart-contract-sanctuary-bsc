...
contract ZaiFighting is Ownable, Pausable {
    ...
    function initFighting(
        ...
        ZaiStruct.ScholarDatas memory scholarDatas = IDelegate(gameAddresses.getDelegateZaiAddress()).gotDelegationForZai(_zaiId);
        ...
        _toReturn = _updateCounterWinLoss(z,_zaiId,_toReturn,scholarDatas);
    ...
    function _updateCounterWinLoss(
        ...
        require(IStats(gameAddresses.getZaiStatsAddress()).updateCounterWinLoss(_zaiId,_toReturn[0],_toReturn, IRank), "Stat issue");
        ...
        if(_toReturn[1] < _toReturn[2]){
            ...
            if(_dayWinCounter[_zaiId] < bzaiRewardCountPerDay[z.metadata.state]){
                ...
                if(
                    ...
                    require(IPayments(paymentAddress).rewardPlayer(_scholarDatas.delegateDatas.ownerAddress,_bzaiWon));
    ...
    function _paySchoolarAndOwner(ZaiStruct.ScholarDatas memory _scholarDatas,uint256 _zaiId, uint256 _reward) internal returns(uint256){   
        uint256 _date = block.timestamp;
        uint256 _ownerReward;
        address _ownerAddress;
        uint256 _scholarReward;
        ...
        if(_scholarDatas.delegateDatas.scholarAddress != address(0x0)){
            ...
            require(payment.rewardPlayer(_scholarDatas.guildeDatas.platformAddress, _reward * _scholarDatas.guildeDatas.percentagePlatformFees / 100));
        ...
        require(payment.rewardPlayer(_ownerAddress,_ownerReward));
        require(payment.rewardPlayer(_scholarAddress,_scholarReward));

        return _scholarReward;
