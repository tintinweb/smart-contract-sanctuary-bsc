...
interface IRewardPool {
    ...
    function increRewardRate(uint256 _increReward) external;
}
contract BB is ERC20Burnable, Operator {
    using SafeMath for uint256;

    /* ========== STATE VARIABLES ========== */

    bool public increChefLockEnabled = true;
    ...
    event TaxAdded(address indexed account, address rewardPool, uint256 amount);
    ...
    /**
     * @notice Constructs the DEER-Peg Token ERC-20 contract.
     */
    constructor(
        string memory name_,
        ...
        _mint(msg.sender,24000000000 ether);
    }
    ...
    function toggleIncreChefLockEnabled() external onlyOwner {
        increChefLockEnabled = !increChefLockEnabled;
    }
    ...
    function setRewardPool(address _rewardPool) external onlyOwner {
        require(_rewardPool != address(0), "zero");
        rewardPool = _rewardPool;
        _isExcludedFromFee[rewardPool] = true;
        _isExcludedToFee[rewardPool] = true;
    }
    ...
    function _transfer(
        ...
        require(sender != address(0), "TDEER: transfer to the zero address");
        require(
            recipient != address(0),
            "TDEER: transfer to the zero address"
        );
            ...
            if (_tokenPrice > 0) {
                {
                    ...
                    if (_taxRate > 0) {
                        ...
                        super._transfer(sender, _rewardPool, _taxAmount);
                        _amount = _amount.sub(_taxAmount);
                        _totalTaxAdded = _totalTaxAdded.add(_taxAmount);
                        if (increChefLockEnabled) {
                            IRewardPool(_rewardPool).increRewardRate(_taxAmount);
                        }
                        emit TaxAdded(sender, _rewardPool, _taxAmount);
                    }
                }
                {
