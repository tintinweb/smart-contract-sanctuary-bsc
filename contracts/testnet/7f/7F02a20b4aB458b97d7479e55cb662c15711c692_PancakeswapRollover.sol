...
contract PancakeswapRollover is IUniswapV2Rollover, IPancakeCallee, BaseRollover {
  ...
  function rolloverLoan(
    ...
    _validateRollover(sourceLoanCore, contracts.targetLoanCore, loanTerms, newLoanTerms, loanData);
...
abstract contract BaseRollover is IRollover, Ownable {
  ...
  function _validateRollover(
    ...
    require(sourceLoanCore.borrowerNote().ownerOf(loanData.borrowerNoteId) == msg.sender, "caller not borrower");
    ...
    require(loanData.dueDate >= block.timestamp, "Overdue loan");
  }
