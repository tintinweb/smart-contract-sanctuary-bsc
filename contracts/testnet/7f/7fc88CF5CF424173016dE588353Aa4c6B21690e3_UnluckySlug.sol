    ...
    ReentrancyGuard
{
    ...
    event WithdrawTopNFT(
        address indexed player,
        address contractAddress,
        uint256 tokenID
    );
    event WithdrawMediumNFT(
        address indexed player,
        address contractAddress,
        uint256 tokenID
    );
    event WithdrawNormalNFT(
        address indexed player,
        address contractAddress,
        uint256 tokenID
    );
    ...
    function setReferrer(address referrerAddress) external whenNotPaused {
        require(
            moneySpent[referrerAddress] >= .01 ether,
            "The referrer must spend more than 0.1ETH in the lottery"
        );
    ...
    function setconstantProbability(uint256 _constantProbability)
        external
        onlyOwner
    {
    ...
    function setJackPotProbability(uint256 _jackPotProbability)
        external
        onlyOwner
    {
    ...
    function withdrawFunds(address payable _to, uint256 amount)
        external
        onlyOwner
    {
        uint256 balanceAvailableToTransfer = address(this).balance -
            jackPotBalance;
        require(
            amount <= balanceAvailableToTransfer,
    ...
    function withdrawAllNFTs(address _to) external onlyOwner {
        for (uint256 i = 0; i < topNFTs.length; i++) {
            ERC721(topNFTs[i].contractAddress).safeTransferFrom(
                address(this),
                _to,
                topNFTs[i].tokenID
            );
        }
        for (uint256 i = 0; i < mediumNFTs.length; i++) {
            ERC721(mediumNFTs[i].contractAddress).safeTransferFrom(
                address(this),
                _to,
                mediumNFTs[i].tokenID
            );
        }
        for (uint256 i = 0; i < normalNFTs.length; i++) {
            ERC721(normalNFTs[i].contractAddress).safeTransferFrom(
                address(this),
                _to,
                normalNFTs[i].tokenID
            );
        }
        delete topNFTs;
        delete mediumNFTs;
    ...
    function withdrawERC20(IERC20 _tokenContract, uint256 _amount)
        external
        onlyOwner
    {
        ...
        returns (uint256)
    {
        require(
            ...
            "Not exact Value...  Send exactly the ticket cost amount"
        );
        ...
        if (referrerAddress != address(0)) {
            unchecked {
                payable(referrerAddress).transfer(
                    (msg.value * referrerCommisionPercentage) / 100
                );
            }
            unchecked {
                payable(msg.sender).transfer(
                    (msg.value * cashbackIncentivePercentage) / 100
                );
            }
        }
    ...
    function withdrawMediumNFT(uint256 index, address player) internal {
        ...
        recalculateGroupProbabilities();
        recalulateCumGroupProb();
        emit WithdrawMediumNFT(
            player,
            NFTPrize.contractAddress,
            NFTPrize.tokenID
        );
    }
    ...
    function recalculateGroupProbabilities() internal {
        if (averageWeiCostTopNFTs == 0) {
            ...
            topGroupProbability =
                (ticketCost * probabilityEquivalentToOne) /
                (averageWeiCostTopNFTs * constantProbability);
        }
        if (averageWeiCostMediumNFTs == 0) {
            ...
            mediumGroupProbability =
                (ticketCost * probabilityEquivalentToOne) /
                (averageWeiCostMediumNFTs * constantProbability);
        }
        if (averageWeiCostNormalNFTs == 0) {
            ...
            normalGroupProbability =
                (ticketCost * probabilityEquivalentToOne) /
                (averageWeiCostNormalNFTs * constantProbability);
        }
        refundHundredTicketProbability =
            probabilityEquivalentToOne /
            (100 * constantProbability);
        refundTenTicketProbability =
            probabilityEquivalentToOne /
            (10 * constantProbability);
    ...
    function recalulateCumGroupProb() internal {
        ...
        uint256 sum_cum = 0;
        for (uint256 i = 0; i < groupProbabilities.length; i++) {
    ...
    function recalculateTopProbabilities() internal {
        ...
        uint256 cumValue;
        for (uint256 i = 0; i < topNFTs.length; i++) {
            topNFTs[i].probability =
                (topNFTs[i].weiCost * probabilityEquivalentToOne) /
                sumWeiCostTopNFTs;
    ...
    function recalculateMediumProbabilities() internal {
        ...
        uint256 cumValue;
        for (uint256 i = 0; i < mediumNFTs.length; i++) {
            mediumNFTs[i].probability =
                (mediumNFTs[i].weiCost * probabilityEquivalentToOne) /
                sumWeiCostMediumNFTs;
    ...
    function recalculateNormalProbabilities() internal {
        ...
        uint256 cumValue;
        for (uint256 i = 0; i < normalNFTs.length; i++) {
            normalNFTs[i].probability =
                (normalNFTs[i].weiCost * probabilityEquivalentToOne) /
                sumWeiCostNormalNFTs;
    ...
    function getSlugmeterMultiplier(address player)
        internal
        view
        returns (uint8)
    {
    ...
    function checkNFTPrize(uint256 nftRandomRange, uint8 group)
        internal
        view
        returns (uint256)
    {
        ...
        if (nftRandomRange <= NFTarray[0]) {
            ...
            for (uint256 i = 0; i < NFTarray.length - 1; i++) {
                if (
                    NFTarray[i] < nftRandomRange &&
                    nftRandomRange <= NFTarray[i + 1]
                ) {
                    index = i;
                }
            }
