 ...
 * @title WERKToken.
 */
contract WERKToken is ERC20, Ownable {
    uint256 public fee = 25;
    uint256 public denom = 10000;
    uint256 public maxFeeAmount = 24577e18;

    address public wallet;

    event NewFee(uint256 oldFee, uint256 newFee);
    event NewMaxFeeAmount(uint256 oldFeeMaxAmount, uint256 newMaxFeeAmount);
    ...
    /// @notice The function sets the new max fee amount.
    /// @param newMaxFeeAmount_ The fee value.
    /// @return The bool value.
    function _setMaxFeeAmount(uint256 newMaxFeeAmount_)
        external
        onlyOwner
        returns (bool)
    {
        require(
            newMaxFeeAmount_ < maxFeeAmount,
            "FeeToken::_setMaxFeeAmount: The newMaxFeeAmount_ must be less than the current max fee"
        );

        uint256 oldMaxFeeAmount = maxFeeAmount;
        maxFeeAmount = newMaxFeeAmount_;

        emit NewMaxFeeAmount(oldMaxFeeAmount, maxFeeAmount);
