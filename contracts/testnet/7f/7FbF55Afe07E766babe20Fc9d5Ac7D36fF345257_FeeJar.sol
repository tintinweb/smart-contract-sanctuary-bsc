...
contract FeeJar is AccessControlUpgradeable {
    ...
    event NetworFeeCollectorSet(address newCollector, address oldNetworkFeeCollector);
    ...
    function initialize(
        ...
        // fees validation
        require(
            _networkFee <= _maxPercentage && _lpFee <= _maxPercentage && _supportFee <= _maxPercentage,
    ...
    {
        if (networkFee > 0) {
            networkFeeAmount = (totalFee * networkFee) / maxPercentage;
        }
        if (lpFee > 0) {
    ...
    {
        (uint256 networkFeeAmount, uint256 supportFeeAmount, uint256 lpFeeAmount) = getFeeAmount(address(this).balance);
        address supportFeeCollector;

        if (networkFee > 0) {
            (bool networkFeeSuccess, ) = networkFeeCollector.call{ value: networkFeeAmount }("");
            require(networkFeeSuccess, "Swap Fee: Could not collect network fee");
        }
