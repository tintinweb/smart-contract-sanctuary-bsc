...
contract SoccerBusdV2 is Ownable , ReentrancyGuard {
    ...
    function getROI() public view returns(uint256){
        ...
        if((userWithdrawCount[userAddress] == 1)){
            return roi - 1;
        }else if((userWithdrawCount[userAddress] == 2)){
            return roi - 1;
        }else if((userWithdrawCount[userAddress] == 3)){
            return roi - 1;
        }else if((userWithdrawCount[userAddress] >= 4)){
            if(roi <= min_roi){
                return min_roi;
            }else{
                return roi - 1;
