...
contract Sale is ERC721Holder, ERC1155Holder, Ownable, Initializable {
    ...
    function _setControllers(address[] memory _accounts, bool isAdd) private {
        for (uint256 i; i < _accounts.length; i++) {
            require(_accounts[i] != address(0), "account is zero address");
            controllers[_accounts[i]] = isAdd;
        }
    }
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) idValid(_saleId) {
        ...
        if (!_project.isCreatedByAdmin) {
            ...
            emit BidSingleProjectUser(_msgSender(), _saleId, sale.tokenId, approval.percent, profitShare, remainingProfit);
        } 
        else {
    ...
    function bidMulti(uint256 _saleId, uint256 _amount) external payable onlyWinner(_saleId) idValid(_saleId) {
        ...
        if (!_project.isCreatedByAdmin) {
            ...
            payable(project.getSuperAdmin()).transfer(profitShare);
            payable(_project.manager).transfer(remainingProfit);
            emit BidMultiProjectUser(_msgSender(), _saleId, sale.tokenId, _amount, approval.percent, profitShare, remainingProfit);
        } 
        else {
            payable(project.getSuperAdmin()).transfer(msg.value);
