    ...
    UUPSUpgradeable
{
    ...
    /**
     * @param _forWalletAddress Address for which the random number is generated
     * @param _forSmartContractAddress Smart Contract that is requesting the random number
     */
    struct Caller {
        address _forWalletAddress;
        address _forSmartContractAddress;
    }
    /**
    ...
    event CallerEvent(address _sc, address _acc, uint256 requestId);
        ...
        internal
    {
        ...
        uint32 _callbackGasLimit = 25000 * _amountNumbers;
        ...
        emit RNRequested(
            chainlinkRequestForAccount[requestId]._forSmartContractAddress,
            chainlinkRequestForAccount[requestId]._forWalletAddress,
            requestId
        );
    }
        ...
        override
    {
        ...
        address _acc = chainlinkRequestForAccount[requestId]._forWalletAddress;
        address _sc = chainlinkRequestForAccount[requestId]
            ._forSmartContractAddress;

        // mark the request as completed
        isRequestOngoing[_acc] = false;

        // store the random number(s) (in array form) for the account
        // randomNumbers[_acc] = randomWords;
        // randomNumbers[_acc].push(randomWords[0]);
        // randomNumbers[_acc].push(randomWords[1]);

        emit RandomNumberGenerated(_sc, _acc, randomWords);
        // return;
        // llamar al smart contract que pidio el random number
        (bool success, ) = address(_sc).call(
            abi.encodeWithSignature(
                "fulfillRandomness(address,uint256[])",
                _acc,
                randomWords
            )
        );
