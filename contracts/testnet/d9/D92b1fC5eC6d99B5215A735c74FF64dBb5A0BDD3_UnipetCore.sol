...
import "./access/IUnipetAccessManager.sol";
import "./CoreFee.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract UnipetCore is ERC721, Ownable, ReentrancyGuard {
    uint256 public currentId;
    IUnipetAccessManager unipetAccessManager;
    CoreFee coreFee;
    ...
    constructor(IUnipetAccessManager _unipetAccessManager, CoreFee _coreFee)
        ERC721("Unipet721", "Unipet721")
    {
        require(
            address(_unipetAccessManager) != address(0),
            "Error: address(0)"
        );
        require(address(_coreFee) != address(0), "Error: address(0)");
        unipetAccessManager = _unipetAccessManager;
        coreFee = _coreFee;
        currentId = 1;
    }

    function setCategories(uint256[] memory _categories, bool[] memory _valids)
        external
        onlyOwner
    {
        require(_categories.length == _valids.length, "Error: input invalid");
        for (uint256 i = 0; i < _categories.length; i++) {
            categories[_categories[i]] = _valids[i];
        }
    }

    function setUnipetAccessManager(IUnipetAccessManager _unipetAccessManager)
        external
        onlyOwner
    {
        require(
    ...
    function born(
        ...
        require(
            unipetAccessManager.isBornAllowed(_msgSender(), _gene),
            "Error: 8u7y Not have born permisison"
        );
        require(categories[_category], "Error: category invalid");
        uint256 _nftId = currentId;

        _mint(_toAddress, _nftId);

        cores[_nftId].category = _category;
        cores[_nftId].gene = _gene;
        cores[_nftId].bornAt = block.timestamp;

        //charge fee
        coreFee.chargeBornFee(_toAddress, _nftId, _category, _gene);
    ...
    function borns(
        ...
        require(
            _toAddresses.length == _categories.length,
            "Error: input invalid"
        );
        require(_toAddresses.length == _genes.length, "Error: input invalid");

        uint256[] memory ids = new uint256[](_toAddresses.length);

        for (uint256 i = 0; i < _toAddresses.length; i++) {
            uint256 id = born(_toAddresses[i],_categories[i], _genes[i]);
            ids[i] = id;
        }
        return ids;
    }
    ...
    {
        require(
            unipetAccessManager.isEvolveAllowed(
                _msgSender(),
                _newGene,
                _nftIds
            ),
        ...
        require(_nftIds.length > 0, "Error: input invalid");

        uint256 category = cores[_nftIds[0]].category;
        require(categories[category], "Error: category invalid");

        for (uint8 i = 0; i < _nftIds.length; i++) {
            require(ownerOf(_nftIds[i]) != address(0), "Error: NFT id invalid");
            if (i != 0)
                require(
                    category == cores[_nftIds[i]].category,
        ...
        cores[_newNftId] = Core(category, _nftIds, _newGene, block.timestamp);

        for (uint8 i = 0; i < _nftIds.length; i++) delete cores[_nftIds[i]];
    ...
    function breed(
        ...
        require(
            unipetAccessManager.isBreedAllowed(_msgSender(), _nftId1, _nftId2),
        ...
        uint256 category = cores[_nftId1].category;
        require(categories[category], "Error: category invalid");
        require(
            category == cores[_nftId2].category,
            "Error: category invalid"
        );
        ...
        cores[_nftId].category = category;
        cores[_nftId].parents.push(_nftId1);
        cores[_nftId].parents.push(_nftId2);
        cores[_nftId].gene = _gene;
        cores[_nftId].bornAt = block.timestamp;
    ...
    function destroy(uint256 _nftId) external nonReentrant {
        require(
            unipetAccessManager.isDestroyAllowed(_msgSender(), _nftId),
        ...
        )
    {
        return (
            ownerOf(_nftId),
            cores[_nftId].category,
            cores[_nftId].parents,
            cores[_nftId].gene,
            cores[_nftId].bornAt
        );
    }
...
contract CoreFee is Ownable {
    ...
    modifier onlyUnipet() {
        require(_msgSender() == unipetCore, "Error: Not SEVCore");
        _;
    }

    function setUnipetCore(address _unipetCore) external onlyOwner {
        require(_unipetCore != address(0), "Error: address(0)");
        unipetCore = _unipetCore;
    }
