...
contract DeuxStake is Context, Ownable {
    ...
    function _addStakeToStakeholders(uint256 index, Stake memory _userStake)
        internal
        returns (uint256)
    {
        stakeholders[index].address_stakes.push();
        uint256 stakeIndex = stakeholders[index].address_stakes.length - 1;
        stakeholders[index].address_stakes[stakeIndex] = _userStake;
        return stakeIndex;
    }

    function _addUserStakeId(address staker) internal returns (uint256) {
        userStakeIds[staker].push();
        uint256 userStakeIndex = userStakeIds[staker].length - 1;
        userStakeIds[staker].push(userStakeIndex);
        return userStakeIndex;
    }
    ...
    function _stake(uint256 _amount, uint256 _rate) internal returns (uint256) {
        ...
        _addStakeToStakeholders(
            index,
            Stake(_id, _msgSender(), stakeAmount, timestamp, rewardRatio, _rate)
        );

        _addUserStakeId(_msgSender());
