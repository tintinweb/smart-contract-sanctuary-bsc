...
contract OP_SWAP {
    ...
    constructor(
        ...
        uniswapV2Router02 = uniswap_addr;
    ...
    /*
     * @dev: swap received ETH/ERC20 funds.
     *
     */
    function swap(uint256 _amountTokenIn, address _tokenSource) public {
        //check destination chainId
        require(
            _tokenSource == stableToken || _tokenSource == opv,
            "Token incorrect"
        );

        address[] memory paths = new address[](2);
        paths[0] = _tokenSource == stableToken ? stableToken : opv;
        paths[1] = _tokenSource == stableToken ? opv : stableToken;

        IERC20(paths[0]).safeTransferFrom(
            msg.sender,
            address(this),
            _amountTokenIn
        );
    ...
    function getEst(uint256 _amountTokenIn, address _tokenSource) public view returns (uint256){
        address[] memory paths = new address[](2);
        paths[0] = _tokenSource == stableToken ? stableToken : opv;
        paths[1] = _tokenSource == stableToken ? opv : stableToken;
        return 
            _getExactTokenForToken(_amountTokenIn, paths);
    }
