...
//contract BatchAuction {
  ...
  */
  ...
  function get1(address _user, uint256 _amount) external view returns (uint256)  {
  ...
  function _withdrawableAmountAfterDeposit(address _user, uint256 _amount) internal view returns (uint256)  {
    ...
    uint256 max_withdraw_update = _withdrawableAmountPerCommitmentsAmount(max_withdraw);
   ...
   */
    ...
    require(COMMIT_CURRENCY != address(0), "INVALID_COMMIT_CURRENCY");
    uint256 max_withdraw = _withdrawableAmountPerCommitmentsAmount(MAX_COMMITMENTS[_user]);
    return max_withdraw - ACCUMULATED_WITHDRAW[_user];
  }

   /**
   * @dev Calculate Withdrawable Amount based on Commitments Amount
   */
    function _withdrawableAmountPerCommitmentsAmount(uint256 _amount) internal view returns (uint256) {
    require(_amount >= 0, "INVALID_AMOUNT");
