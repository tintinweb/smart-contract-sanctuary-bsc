...
contract Tinad is Context, IERC20, Ownable {
    ...
    mapping (address => bool) excludeFromMaxWallet;

    mapping (address => bool) _isBot;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 999991996118 * (10**8);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    
    string private _name = "Tinad";
    ...
    uint256 public totalFees = _taxFee.add(_liquidityFee).add(_marketingFee).add(_burnFee);
    ...
    uint256 public swapTokensAtAmount = 100000000 * 10**8;
    ...
    bool public transactionCooldownEnabled = true;
    uint8 public cooldownTimerInterval = 60;
    mapping (address => uint) private cooldownTimer;

    // Anti Whale
    bool public enableAntiwhale = true;
    uint256 private maxTransferAmountRate = 500;
    ...
    constructor() {
        ...
        marketingToken = 0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47;
        ...
        excludeFromMaxWallet[deadAddress] = true;
    ...
    function cooldownEnabled(bool _status, uint8 _interval) public onlyOwner {
        require(_interval <= 300);
        transactionCooldownEnabled = _status;
    ...
    function _transfer(
        ...
        if (transactionCooldownEnabled){
            if (!isTimelockExempt[from]) {
                require(cooldownTimer[from] < block.timestamp,"Please wait for 1min between two transactions");
                cooldownTimer[from] = block.timestamp + cooldownTimerInterval;
            }
            if (!isTimelockExempt[to]) {
                require(cooldownTimer[to] < block.timestamp,"Please wait for 1min between two transactions");
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance.mul(_marketingFee).div(totalFees);
            uint256 burnTokens = contractTokenBalance.mul(_burnFee).div(totalFees);

            uint256 liquidityTokens = contractTokenBalance.mul(_liquidityFee).div(totalFees);
            uint256 liqHalf = liquidityTokens.div(2);
            uint256 liqOtherHalf = liquidityTokens.sub(liqHalf);

            uint256 tokensToSwap = liqHalf.add(marketingTokens);

            uint256 initialBalance = address(this).balance;
            swapTokensForBNB(tokensToSwap);
            uint256 newBalance = address(this).balance.sub(initialBalance);
            
            uint256 markBNB = newBalance.mul(marketingTokens).div(tokensToSwap);
            uint256 liqBNB = newBalance.mul(liqHalf).div(tokensToSwap);

            addLiquidity(liqOtherHalf, liqBNB);
            emit SwapAndLiquify(liqHalf, liqBNB, liqOtherHalf);

            if (markBNB > 0) {
                swapTokensForMarketingToken(markBNB, marketingWallet, marketingToken);
            }

            if (_burnFee > 0) {
                _rOwned[address(this)] = _rOwned[address(this)].sub(burnTokens);
    ...
    function swapTokensForMarketingToken(uint256 _tokenAmount, address _recipient, address _marketingToken) private {
        if(_marketingToken != address(0)) {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = _marketingToken;
    ...
    function setBotFees() private {
        ...
        _previousMarketingFee = _marketingFee;
        _previousBurnFee = _burnFee;
        _burnFee = _previousBurnFee * _botIncreaseFee;
    ...
    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {
        require(maxWallPercent_base1000 > 0);
        maxWalletToken = (_tTotal * maxWallPercent_base1000 ) / 1000;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private 
    {
        ...
        if (!excludeFromMaxWallet[recipient]) {
            uint256 balanceToBe = balanceOf(recipient);
            require((balanceToBe.add(amount)) <= maxWalletToken, "You cannot have more tokens");
        }
    ...
    function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
        _burnFee = burnFee;
        totalFees = _taxFee.add(_marketingFee).add(_liquidityFee).add(_burnFee);
    ...
    function setswapTokensAtAmount(uint256 newAmt) external onlyOwner() {
        swapTokensAtAmount = newAmt*10**8;
    }
