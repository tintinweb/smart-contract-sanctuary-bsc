...
contract PumpkinTaxOracle is Ownable {
    ...
    constructor(
        ...
        pumpkin = IERC20(_pumpkin);
        wftm = IERC20(_wftm);
        pair = _pair;
    }

    function consult(address _token) external view returns (uint144 amountOut) {
        require(_token == address(pumpkin), "token needs to be pumpkin");
        uint256 pumpkinBalance = pumpkin.balanceOf(pair);
        uint256 wftmBalance = wftm.balanceOf(pair);
        return uint144(pumpkinBalance.div(wftmBalance));
    }

    function setPumpkin(address _pumpkin) external onlyOwner {
        require(_pumpkin != address(0), "pumpkin address cannot be 0");
        pumpkin = IERC20(_pumpkin);
    }
