...
// public
    ...
    function spinWheel() external returns(uint8) {
        require(m_user_turn_count[_msgSender()] > 0, "You don't have any ticket!");
        address senderAddress = _msgSender();

        uint randomNum = _random();
        uint8 gameResult = _handleWheelResult(senderAddress, 7); // _handleWheelResult(senderAddress, randomNum);
...
// private
    function _handleWheelResult(address user, uint256 wheel_index) private returns(uint8) {
        if(wheel_index == 1 || wheel_index == 7) {
            m_user_win_balance[user] += WinReward * BLA_DECIMALS;
            m_user_turn_count[user] -= 1;
            return 7;
        } 
        else if(wheel_index == 2 || wheel_index == 5 || wheel_index == 2 ) {
            return 3;
        }
        else {
            m_user_turn_count[user] -= 1;
