...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    constructor(
        ...
        _bnb_Address = _uniswapV2Router.WETH();
        _TokenReserv = IUniswapV2Pair(uniswapV2Pair);
        ...
        returns (uint256)
    {
        ...
        if (address(this) == _TokenReserv.token0()) {
            (uint256 ResTOKEN, uint256 ResBNB, ) = _TokenReserv.getReserves();
            uint256 pricebnb = ResBNB.div(ResTOKEN);
            return (amount.mul(_DECIMALFACTOR)).div(pricebnb); // return amount of BNB needed to Transaction
        } else {
            (uint256 ResBNB, uint256 ResTOKEN, ) = _TokenReserv.getReserves();
            uint256 pricebnb = ResBNB.div(ResTOKEN);
            return (amount.mul(_DECIMALFACTOR)).div(pricebnb); // return amount of BNB needed to Transaction
        }
    }
