...
/**
 * @title PrivateSale
 * @dev PrivateSale is a base contract for managing a token crowdsale,
...
contract PrivateSale is Vesting {
    ...
    struct PresaleStatus {
        bool OPENED_TO_CLAIM; // final flag required to end a presale and enable withdrawls
        bool READY_TO_CLAIM; // last flag required to end a presale and enable withdrawls
    ...
    struct TokenVesting {
        ...
        uint256 VESTING_START_DATE; // starting time of the vesting in seconds
    }
    ...
    /**
     * @dev Event for presale initialized
     */
    event PresaleInitialized();

    /**
     * @dev Event for vesting initialized
     */
    ...
    /**
     * @dev Event for claims opened
     * @param time timestamp of cliams opened date
     */
    event ClaimsOpened(uint256 time);
    ...
    constructor(address token_) Vesting(token_) {
        require(token_ != address(0x0), "PrivateSale: invalid token address!");
    ...
    function init(
        ...
        require(presaleStatus() != 0, "PrivateSale: presale is already active!"); // QUED
        ...
        INFO.PRESALE_IN_BNB = address(baseToken_) == address(WETH);
    ...
    function updateTime(uint256 _startTime, uint256 _endTime)
        ...
        onlyOwner
    {
        require(presaleStatus() == 0, "PrivateSale: presale is already started!");
        INFO.START_TIME = _startTime;
        INFO.END_TIME = _endTime;
    }
    ...
    /**
     * @dev get crowdsale info with baseToken address, saleToken address, INFO, STATUS and presaleStatus()
     */
    function getInfo()
        public
        view
        returns (
            ...
            IERC20Ext,
            PresaleInfo memory,
            PresaleStatus memory,
            uint256
        )
    {
        return (_baseToken, _saleToken, INFO, STATUS, presaleStatus());
    }
    ...
    function presaleStatus() public view returns (uint256) {
        ...
        if (
            ...
            (STATUS.TOTAL_BASE_COLLECTED < INFO.SOFTCAP)
        ) {
            return 3; // FAILED - softcap not met by end block
        }

        if (STATUS.TOTAL_BASE_COLLECTED >= INFO.HARDCAP) {
        ...
        if (
            ...
            (block.number <= INFO.END_TIME)
        ) {
            return 1; // ACTIVE - deposits enabled
        }

        return 0; // QUED - awaiting start time
    }
    ...
    function userDeposit(uint256 _amount) external payable nonReentrant {
        ...
        require(presaleStatus() == 1, "PrivateSale: presale is not active!"); // ACTIVE
        ...
        require(tokensSold > 0, "PrivateSale: zero tokens!");
    ...
    function userWithdrawTokens() external nonReentrant {
        require(
            STATUS.OPENED_TO_CLAIM,
            "PrivateSale: awaiting for claims opening!"
        );

        BuyerInfo storage buyer = BUYERS[_msgSender()];
        uint256 tokensRemainingDenominator = STATUS.TOTAL_TOKENS_SOLD -
            STATUS.TOTAL_TOKENS_WITHDRAWN;
        uint256 tokensOwed = (_saleToken.balanceOf(address(this)) *
            buyer.tokensOwed) / tokensRemainingDenominator;
        require(tokensOwed > 0, "PrivateSale: nothing to withdraw!");
        ...
        if (INFO.TOKEN_VESTING) {
            ...
            createVestingSchedule({
                _name: "PrivateSale Vesting",
                _beneficiary: _msgSender(),
                _start: VESTING.VESTING_START_DATE,
    ...
    function userWithdrawBaseTokens() external nonReentrant {
        require(presaleStatus() == 3, "PrivateSale: not failed!"); // FAILED

        BuyerInfo storage buyer = BUYERS[_msgSender()];
        require(buyer.baseDeposited > 0, "PrivateSale: nothing to withdraw!");
    ...
    /**
     * @dev on presale failure. allows the owner to withdraw the tokens they sent for presale & initial liquidity.
     * @notice onlyOwner method
     */
    ...
    function forceFailByOwner() external onlyOwner {
        require(!STATUS.FORCE_FAILED, "PrivateSale: sale is force failed!");
        STATUS.FORCE_FAILED = true;
    }

    /**
     * @dev on presale success, this is the last step to end the presale, lock liquidity and enable withdrawls of the sale token.
    ...
    function finishPreSale() external onlyOwner {
        require(STATUS.READY_TO_CLAIM, "PrivateSale: presale is already finished!");
        require(presaleStatus() == 2, "PrivateSale: sale is not success!");
        ...
        STATUS.READY_TO_CLAIM = true;
        STATUS.PRESALE_END_DATE = block.timestamp;

        emit SaleFinished(STATUS.TOTAL_BASE_COLLECTED, STATUS.TOTAL_TOKENS_SOLD);
    }

    /**
     * @dev on presale success, this is the final step to end the presale, opens token claims for contributors.
     * @notice onlyOwner method
     */
    function openClaim() external onlyOwner {
        require(!STATUS.READY_TO_CLAIM, "PrivateSale: presale is not ready to claim!");
        require(STATUS.OPENED_TO_CLAIM, "PrivateSale: presale is already opened to claim!");

        STATUS.OPENED_TO_CLAIM = true;
        VESTING.VESTING_START_DATE = block.timestamp;

        emit ClaimsOpened(VESTING.VESTING_START_DATE);
    }
