...
contract KNTC is Ownable, IERC20 {
    ...
    uint256 private _fee = 25;
    address private _feeAddress;
    uint256 private _upAmount = 1000;
    uint256 private _upBase = 10000;
    bool private isSwap;
    address private _USDTAddress = 0x07FF944b000390Ce4bfEd9adfa1EdBDE62902857;
    ...
    constructor(){
        ...
        _pancakeRouter = IPancakeRouter(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
    ...
    function isMinter(address account) public view onlyOwner returns (bool) {
    ...
    function _transferSwap(address from, address to, uint256 amount) internal virtual{
        require(_pancakeAddress != address(0),"Not swap");
        if(isMinter(from)){
    ...
    function swap(uint256 amount) public virtual returns (bool) {
        ...
        uint256 realAmount = amount.mul(uint256(100).sub(_fee)).div(100);
        _transfer(msg.sender,_feeAddress,fee);
        _swapKNTCForUSDT(msg.sender,realAmount);
        return true;
    }

    function _swapKNTCForUSDT (address to,uint256 amount) private returns (uint){
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _USDTAddress;
        _approve(address(this), address(_pancakeRouter), amount);
        _pancakeRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount,0,path,to,block.timestamp);
        // uint[] memory amounts=_pancakeRouter.getAmountsOut(_pancakeAddress,amount,path);
        // uint256 ra = amounts[1];
        return 0;
    }
