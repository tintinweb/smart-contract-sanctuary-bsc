...
contract LavevelICO is Ownable {
  ...
  uint256 public constant START = 1645294844; // Mar 26, 2018 @ 12:00 EST
  uint256 public constant DAYS = 1; // 45 Day
  
  uint256 public constant initialTokens = 500000000 * 10**18; // Initial number of tokens available
   ...
   **/
  function idoFinal() onlyOwner public {
    // Transfer tokens back to owner
   // require(isActive());
    uint256 balance = token.balanceOf(this);
    assert(balance > 0);
    token.transfer(0x0000000000000000000000000000000000000000, balance);
  }
}