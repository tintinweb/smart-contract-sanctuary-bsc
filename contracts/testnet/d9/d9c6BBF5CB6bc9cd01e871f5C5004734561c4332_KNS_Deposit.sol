...
contract KNS_Deposit is OwnableUpgradeable {
    ...
    function stageShare(uint reward, uint totalReward,uint timestamp, bytes32 r, bytes32 s, uint8 v) external onlyEOA {
        require(block.timestamp <= timestamp,'out of time');
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, reward, totalReward,timestamp));
        address a = ecrecover(hash, v, r, s);
        require(a == banker, 'not banker');
    ...
    function claimDynamic(
        ...
        require(block.timestamp <= timestamp,'out of time');
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, directReward, totalDirectReward, secondReward, totalSecondReward,timestamp));
        address a = ecrecover(hash, v, r, s);
        require(a == banker, 'not banker');
