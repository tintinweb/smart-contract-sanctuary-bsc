...
// BullTestToken with Governance.
contract BullTestToken is BEP20("tBullionFX", "tBULL") {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "BullTest::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "BullTest::delegateBySig: invalid nonce"
        );
        require(now <= expiry, "BullTest::delegateBySig: signature expired");
        ...
        returns (uint256)
    {
        require(
            ...
            "BullTest::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying BullTests (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "BullTest::_writeCheckpoint: block number exceeds 32 bits"
        );
...
contract SyrupBar is BEP20("BullionFX SyrupBar Token", "BSYRUP") {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    // The BullTest TOKEN!
    BullTestToken public bull;

    constructor(BullTestToken _bull) public {
        bull = _bull;
    }
