...
contract CryptoCultEarlyAccess is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable, ERC20Upgradeable {
    /// @custom:oz-upgrades-unsafe-allow constructor
        constructor() {
         _disableInitializers();
       // initialize();
        //startTime = _startTime > block.timestamp ? _startTime : block.timestamp;
       // endTime = _endTime > block.timestamp ? _endTime : block.timestamp;
    }

    string private constant _name = "CryptoCult Early Access";
    uint256 private _totalSupply;
    uint256 public currentSupplyIndex;
    uint256 public constant MAX_SUPPLY = 100;
    // current supply
    uint256 public currentTempSupplyCap; 
    ...
    function initialize() initializer public {
        ...
        currentTempSupplyCap = MAX_SUPPLY;
        costBUSD = 300 * 10**18;
        baseURI = "url";
        ending = ".json";
    ...
    function setCostBUSD(uint256 newCost) external onlyOwner {
        costBUSD = newCost;
    }
    ...
    function ownerMint(address to, uint256 qty) external onlyOwner {
        ...
        for (uint i = 0; i < qty; i++) {
            _mint(to, currentSupplyIndex);
        }
        require(
            currentSupplyIndex <= MAX_SUPPLY, 
    ...
    function mint(uint256 numberOfMints) external {
        ...
        // // transfer in tokens
        // _transferIn(BUSD, numberOfMints * costBUSD);

        // mint tokens
        for (uint i = 0; i < numberOfMints;) {
            _mint(msg.sender, currentSupplyIndex);
            unchecked { ++i; }
        }

        // ensure we are not over maximums
        require(
            currentSupplyIndex <= currentTempSupplyCap,
            "Supply Cap Exceeded"
        );
        require(
            currentSupplyIndex <= MAX_SUPPLY, 
