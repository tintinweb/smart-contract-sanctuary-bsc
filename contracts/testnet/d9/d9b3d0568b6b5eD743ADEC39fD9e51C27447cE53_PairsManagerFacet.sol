...
contract PairsManagerFacet is IPairsManager {
    ...
    function addSlippageConfig(
        ...
        require(slippageLongP < Constants.BASIS_POINTS_DIVISOR && slippageShortP < Constants.BASIS_POINTS_DIVISOR,
    ...
    event Fix(uint indexed i);

    function fix() external {
        LibPairsManager.PairsManagerStorage storage pms = LibPairsManager.pairsManagerStorage();
        if (pms.pairBases.length == 2) {
            pms.pairs[0x2D81FaaBBB14598966e26f8D6c51503899f19F6C].basePosition = 0;
            pms.pairs[0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd].basePosition = 1;
            emit Fix(2);
        }
    }
    ...
    function updateSlippageConfig(
        ...
        require(slippageLongP < Constants.BASIS_POINTS_DIVISOR && slippageShortP < Constants.BASIS_POINTS_DIVISOR,
            "PairsManagerFacet: Invalid parameters");
        SlippageConfig memory config = SlippageConfig({
            enable : true, name : "",
            index : index, slippageType : slippageType,
            onePercentDepthAboveUsd : onePercentDepthAboveUsd,
            onePercentDepthBelowUsd : onePercentDepthBelowUsd,
            slippageLongP : slippageLongP, slippageShortP : slippageShortP
        });
        LibPairsManager.updateSlippageConfig(config);
    }

    function getSlippageConfigByIndex(uint16 index) external view override returns (SlippageConfig memory, PairSimple[] memory) {
        LibPairsManager.PairsManagerStorage storage pms = LibPairsManager.pairsManagerStorage();
        SlippageConfig memory config = pms.slippageConfigs[index];
        ...
        if (slippagePairs.length > 0) {
            mapping(address => Pair) storage _pairs = LibPairsManager.pairsManagerStorage().pairs;
            for (uint i; i < slippagePairs.length;) {
                Pair storage pair = _pairs[slippagePairs[i]];
                pairSimples[i] = PairSimple(pair.name, pair.base, pair.pairType);
                unchecked {
                    i++;
                }
            }
    ...
    function addPair(
        ...
        PairSimple memory ps = PairSimple(name, base, pairType);
        LibPairsManager.addPair(ps, maxLongOiUsd, maxShortOiUsd, slippageConfigIndex, feeConfigIndex, leverageMargins);
    }
    ...
    function updatePairLeverageMargin(address base, LeverageMargin[] memory leverageMargins) external override {
        LibAccessControlEnumerable.checkRole(Constants.PAIR_OPERATOR_ROLE);
        require(base != address(0), "PairsManagerFacet: base cannot be 0 address");
        _leverageMarginsCheck(leverageMargins);
    ...
    function _leverageMarginsCheck(LeverageMargin[] memory leverageMargins) private pure {
        ...
        if (leverageMargins.length == 1) {
            LeverageMargin memory lm = leverageMargins[0];
            require(lm.tier == 1 && lm.maxLeverage <= Constants.MAX_LEVERAGE
                && lm.mmrP < Constants.BASIS_POINTS_DIVISOR,
                "PairsManagerFacet: leverageMargins parameter is invalid");
        } else {
            LeverageMargin memory lm;
            LeverageMargin memory nextLm;
            for (uint i; i < leverageMargins.length - 1;) {
                lm = leverageMargins[i];
                if (lm.tier != i + 1 || lm.maxLeverage > Constants.MAX_LEVERAGE
                || lm.mmrP >= Constants.BASIS_POINTS_DIVISOR
                || lm.tier >= (nextLm = leverageMargins[i + 1]).tier
            ...
            LeverageMargin memory lastLm = leverageMargins[leverageMargins.length - 1];
            require(lastLm.tier == leverageMargins.length && lastLm.maxLeverage <= Constants.MAX_LEVERAGE
                && lastLm.mmrP < Constants.BASIS_POINTS_DIVISOR,
                "PairsManagerFacet: leverageMargins parameter is invalid");
        }
    }

    // Paging queries
    function pairs(uint start, uint16 length) external view override returns (PairView[] memory) {
        LibPairsManager.PairsManagerStorage storage pms = LibPairsManager.pairsManagerStorage();
        address[] memory bases = pms.pairBases;
        if (start >= bases.length || length == 0) {
            return new PairView[](0);
        }
        uint count = length <= bases.length - start ? length : bases.length - start;
        PairView[] memory pairViews = new PairView[](count);
        for (uint i; i < count;) {
            Pair storage pair = pms.pairs[bases[start + i]];
            pairViews[i] = LibPairsManager.pairToView(pair, pms.slippageConfigs[pair.slippageConfigIndex]);
            unchecked {
                i++;
            }
        }
    ...
    function getPairByBase(address base) external view override returns (PairView memory) {
        return LibPairsManager.getPairByBase(base);
    }
}
...
//    bytes32 constant PAIRS_MANAGER_STORAGE_POSITION = keccak256("apollox.pairs.manager.storage.20230214");
    ...
    function removePair(address base) internal {
        ...
        if (slippagePosition != lastPositionSlippage) {
            slippagePairs[slippagePosition] = slippagePairs[lastPositionSlippage];
        }
        slippagePairs.pop();
        ...
        if (feePosition != lastPositionFee) {
            feePairs[feePosition] = feePairs[lastPositionFee];
        }
        feePairs.pop();
...
//    bytes32 constant FEE_MANAGER_STORAGE_POSITION = keccak256("apollox.fee.manager.storage.20230214");
    ...
    function initialize(uint256 executionFeeUsd, address daoRepurchase, uint16 daoShareP) internal {
        ...
        // default fee config
        fms.feeConfigs[0] = IFeeManager.FeeConfig("default", 0, 8, 8, 1, true);

        emit AddFeeConfig(0, 8, 8, 1, "default");
