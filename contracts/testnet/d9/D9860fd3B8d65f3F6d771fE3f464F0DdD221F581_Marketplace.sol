...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    function buy(Detail calldata detail, bytes calldata signature) external nonReentrant {
        ...
        require(
            isSignatureValid(signature, keccak256(abi.encode(detail)), detail.seller),
            "signature error"
        );        

        /**
         * @notice Transfers bundle price from buyer to seller.
         */
        Price memory price = detail.price;
        IERC20(price.tokenAddress).transferFrom(msg.sender, detail.seller, price.amount);

        /**
         * @notice Transfers bundle from seller to buyer.
         */
        for (uint i = 0; i < detail.bundle.length; i++) {
            require(isTokenSupported[detail.bundle[i].tokenAddress], "token address not supported");
    ...
    /**
     * @notice Adds or removes tokens from the supported tokens by the marketplace.
     * @param token Token address to update support.
     * @param value Boolean value of whether to add or remove the token.
     */
    function supportToken(address token, bool value) external onlyOwner {
        isTokenSupported[token] = value;
        emit SupportTokenUpdated(token, value);
    }

    event Buy(address indexed buyer, Detail detail, bytes signature);
    event SupportTokenUpdated(address indexed token, bool value);
}
