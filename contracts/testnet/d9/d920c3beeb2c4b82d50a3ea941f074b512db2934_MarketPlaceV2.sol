...
contract MarketPlaceV2 is MarketPlaceV2Auth, EIP712Upgradeable, PausableUpgradeable {
  ...
  event HeroSummoned(address indexed owner, address usingRight, uint tokenId, uint timestamp);
  ...
  event OrderTaken(uint indexed id, address indexed taker, uint[] tokenIds);
  event OrderRevoked(uint[] ids, uint timestamp);
  event OrderRevokedByOwner(uint id, uint timestamp);
  event PrestigeHeroSummoned(address indexed owner, uint tokenId, uint timestamp);
  ...
  function makeOrder(uint[] calldata _tokenIds, uint _takerShare, uint _duration) external whenNotPaused {
    ...
    emit OrderCreated(orderCounter, msg.sender, _tokenIds, _takerShare, _duration, orderTrustFee, block.timestamp);
  }
  ...
  function takeOrder(uint _orderId) external whenNotPaused {
    _validateUser();
    _takeFund();
    Order storage order = orders[_orderId];
    require(order.maker != msg.sender, "EatherTrading: self trading");
    require(order.maker != address(0) && order.taker == address(0), "EatherTrading: order invalid");
  ...
  function summonShardHero(uint16 _classId, bytes memory _signature, uint _timestamp) external whenNotPaused {
    ...
    emit HeroSummoned(address(this), msg.sender, tokenId, block.timestamp);
  }
  ...
  function summonHero(uint16 _classId, uint _eatherId, bytes memory _signature, uint _timestamp) external whenNotPaused {
    ...
    emit HeroSummoned(msg.sender, msg.sender, tokenId, block.timestamp);
  }
  ...
  function summonPrestigeHero(uint16 _classId, uint _eatherId, bytes memory _signature, uint _timestamp) external whenNotPaused {
    rewardManager.summonPrestigeHero(msg.sender, heroPrice);
    _validateSignature(_classId, _timestamp, _signature);
    _validateEatherRightAndBurn(_eatherId);
    uint tokenId = heroNft.mintHero(msg.sender, _classId, heroPrice, 0);
    emit PrestigeHeroSummoned(msg.sender, tokenId, block.timestamp);
  }
  ...
  function ownerRevokeOrder(uint _orderId, bytes memory _signature, uint _timestamp) external {
    ...
    require(fotaToken.balanceOf(address(this)) >= order.orderTrustFee, "MarketPlaceV2: contract insufficient balance");
    fotaToken.transfer(order.maker, order.orderTrustFee);
  ...
  function revokeOrders(uint[] calldata _orderIds) external onlyPVE {
    ...
    for (uint i = 0; i < _orderIds.length; i++) {
      ...
      require(fotaToken.balanceOf(address(this)) >= order.orderTrustFee, "MarketPlaceV2: contract insufficient balance");
      fotaToken.transfer(order.taker, order.orderTrustFee);
      heroNft.updateLockedFromMKPStatus(order.tokenIds, false);
  ...
  function setContracts(address _rewardManager, address _heroNft, address _itemNft, address _gamePVE, address _fotaToken) external onlyMainAdmin {
    rewardManager = IRewardManager(_rewardManager);
    heroNft = IGameNFT(_heroNft);
    itemNft = IGameNFT(_itemNft);
    gamePVE = _gamePVE;
  ...
  function _takeFund() private {
    require(fotaToken.allowance(msg.sender, address(this)) >= orderTrustFee, "MarketPlaceV2: please approve fota first");
    require(fotaToken.balanceOf(msg.sender) >= orderTrustFee, "MarketPlaceV2: insufficient balance");
    require(fotaToken.transferFrom(msg.sender, address(this), orderTrustFee), "MarketPlaceV2: transfer fota failed");
  }

  function _validateHeroRight(uint[] calldata _tokenIds) private view {
