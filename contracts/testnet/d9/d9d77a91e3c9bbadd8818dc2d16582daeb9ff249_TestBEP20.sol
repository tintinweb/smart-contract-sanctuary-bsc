...
abstract contract BEP20GlobalData is BEP20{
	string internal constant _name = "BootyPootin";
	string internal constant _symbol = "BOP";
...
abstract contract DEX is Ownable, BEP20Modifier{
	...
	function swapBack(
		...
		bypassTax = true;
		amounts = DEXLibrary.getAmountsOut(router.factory(), amountIn, path);
		internalTransfer(msg.sender, adr_pair, amounts[0]);
        _swap(amounts, path, adr_router);
        IWETH(router.WETH()).withdraw(amounts[amounts.length - 1]);
        internalTransferETH(adr_team, amounts[amounts.length - 1]);
		bypassTax = false;
	}
	function _swap(uint[] memory amounts, address[] memory path, address to_) internal {
        for (uint i; i < path.length - 1; i++) {
