...
contract Kryptoon is ERC721, Pausable, Ownable, ReentrancyGuard {
    ...
    IERC20 public BUSD = IERC20(0xc055c69012D363Fd2B48D3D817022Fbb736dD169);
...
*/
    ...
    function _mintToken(
        ...
        uint256 capsuleReward = SafeMath.div(_value, 2);
...
*/
        ...
        returns (bool)
    {
        ...
        if (miner == address(0)) return false;
        require(
        ...
        if (readyToLaunch(nft.owner, nft.idToken)) {
            ...
            accounts[nft.owner].reward += nft.capsuleReward;
            BUSD.safeTransfer(nft.owner, nft.capsuleReward);
            erasePlan(nft.idToken);
        }
    ...
    function spreadOwnerToPromoter(CapsuleTypes storage token)
        ...
        returns (bool)
    {
        if (token.owner == address(0)) return false;
        ...
        for (uint256 i = 0; i < accounts[promoter].myCapsules.length; i++) {
            if (
                ...
                assert(
                    equals(_token, allCapsules[_token.idToken]) &&
                ...
                if (readyToLaunch(_token.owner, _token.idToken)) {
                    ...
                    accounts[promoter].reward += _token.capsuleReward;

                    BUSD.safeTransfer(_token.owner, _token.capsuleReward);
    ...
    function calculateContractDebt() external view returns (uint256) {
        ...
        for (i = 0; i < _tokenIdCounter.current(); i++) {
            if (allCapsules[i].isActive)
                acumDebt = acumDebt.add(allCapsules[i].capsuleReward);
        }
        return acumDebt;
    }
    ...
    */
    ...
    function readyToLaunch(address ad, uint256 tokenID)
        ...
        returns (bool)
    {
        if (ad == address(0) || allCapsules[tokenID].owner == address(0))
            return false;
    ...
    function checkMyActiveCapsules(address ad) private view returns (uint256) {
        ...
        for (uint256 i = 0; i < accounts[ad].myCapsules.length; i++)
            if (accounts[ad].myCapsules[i].isActive) count++;
        return count;
    }

    function getCapsules(address addr)
        public
        view
        returns (CapsuleTypes[] memory)
    {
        return accounts[addr].myCapsules;
    }
