...
contract DAO is Context, IBEP20, Ownable {
  ...
  uint256 public PROFIT_PER_SHARE = 0;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(SHARE_ON_CREATED[sender]==0) { SHARE_ON_CREATED[sender] = PROFIT_PER_SHARE; }
    if(SHARE_ON_CREATED[recipient]==0) { SHARE_ON_CREATED[recipient] = PROFIT_PER_SHARE; }
  ...
  function deposit(uint256 _amount) public onlyOwner {
      ...
      IBEP20(PROFIT_TOKEN).transferFrom(address(msg.sender), address(this), _amount);
...
contract DAOVoting is Ownable, DAO  {
  ...
  uint256 public PROPOSING_RIGHTS = 100;
  uint256 public VOTING_RIGHTS = 100;
  ...
  event Votes(address indexed user, uint256 indexed _proposal, uint256 _votes);
   ...
   modifier onlyProposers() {
    require(balanceOf(_msgSender()) >= (totalSupply().mul(PROPOSING_RIGHTS).div(10000)) , "You Don't Owe the Proposing Rights !");
    _;
  }

  modifier onlyVoters() {
    require(balanceOf(_msgSender()) >=  VOTING_RIGHTS  , "You Don't Owe the Proposing Rights !");
    _;
  }

  function setProposerRightPercent(uint256 _percent) public onlyOwner{
    PROPOSING_RIGHTS = _percent;
  }
  ...
  function closeProposal(uint256 _proposal) public onlyProposers{
    ...
    require(prop.createdBy == msg.sender, "Proposal : You are not the Creator !");
    prop.isActive = false;
  ...
  function claim() public {
    ...
    uint256 reward = (share.mul(profitShare).div(1e12)).sub(user.lastRewardedAmount);
    if(reward > IBEP20(PROFIT_TOKEN).balanceOf(msg.sender)) { reward = IBEP20(PROFIT_TOKEN).balanceOf(msg.sender); }
    IBEP20(PROFIT_TOKEN).transfer(msg.sender,reward);
