...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    function shuffle(uint256[] memory _lotteryTickets, uint256[] memory numbers) internal pure returns(uint256[] memory) {
        ...
        for (uint256 k = 0; k < numbers.length; k++) {
            for (uint256 i = 0; i < _lotteryTickets.length; i++) {
                uint256 n = i + numbers[k] % (_lotteryTickets.length - i);
        ...
        nonReentrant
    {
        ...
        uint256[] memory numbers = randomGenerator.expand(number, _lotteries[_lotteryId].prizes.length);
        ...
        for (uint256 i = 0; i < _lotteries[_lotteryId].prizes.length; i++) {
            ...
            // uint256 winnerIndex = numbers[i] % lotteryTickets.length - 1;
    ...
    function alreadyInWinnersMap(uint256 _lotteryId, uint256 number) internal view returns(bool) {
        uint256 start = _lotteries[_lotteryId].lastWinnerId + 1;
        for (uint256 i = start; i < currentWinnerId; i++) {
            if(_winners[i].ticket == number) return true;
        }
        return false;
    }
