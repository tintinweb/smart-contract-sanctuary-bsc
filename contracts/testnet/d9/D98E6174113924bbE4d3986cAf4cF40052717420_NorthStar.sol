...
contract NorthStar {
    ...
    function makeChefs(address ref) public requireKitchenOpen {
        ...
        claimedDishes[msg.sender] = claimedDishes[msg.sender] * (compRate ** compMulitplier);
    ...
    function getPendingDishes(address adr) public view returns(uint256) {
        ...
        uint256 divider = (block.timestamp - depositTime[adr]) > 15552000 ? 2 : 1;
        return min(CHEF_COST_IN_Dishes, (block.timestamp - lastDishesToChefsConversion[adr])) / divider * addressChefs[adr];
    }
