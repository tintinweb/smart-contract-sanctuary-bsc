...
contract FarmContractTestVersion is Initializable, OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _miningTokenContract, address _dividendTokenContract, address _inviteContract) initializer public {
        ...
        setLockupTime(600);
        setRedemptionGracePeriod(300);
        setOneStarUserPledgeThreshold(50 * 10 ** 18); 
        setTwoStarUserPledgeThreshold(100 * 10 ** 18);
        setThreeStarUserPledgeThreshold(150 * 10 ** 18);
    ...
    function pendingMiningReward(uint256 _pid, address _user) public view returns (uint256 totalMiningPending) {
    ...
    function pendingDividendReward(uint256 _pid, address _user) public view returns (uint256 totalDividendPending) {
    ...
    function pendingReferralReward(uint256 _pid, address _user) public view returns(uint256 totalReferralBonus){
        ...
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 referralBonus = (pendingMiningReward(_pid, myInviteeAddress[i]).add(mintAmount[_pid][_user])).mul(refferalRewardPercent[_pid][_user]).div(rateDenominator);
    ...
    function pendingNodeReward(uint256 _pid, address _user) public view returns(uint256 totalNodeBonus){
        address[] memory myInviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 nodeBonus = (pendingMiningReward(_pid, myInviteeAddress[i]).add(mintAmount[_pid][_user])).mul(nodeRewardPercent[_pid][_user]).div(rateDenominator);
    ...
    function claimReferralReward(uint256 _pid) public {
        uint256 pendingReward = pendingReferralReward(_pid, _msgSender());
        require(pendingReward > 0, "You have no referral rewards to claim");
        safeMiningTokenTransfer(_msgSender(), pendingReward);
        userClaimedReferralRewards[_pid][_msgSender()] = userClaimedReferralRewards[_pid][_msgSender()].add(pendingReward);
    }

    function claimNodeReward(uint256 _pid) public {
        uint256 pendingReward = pendingNodeReward(_pid, _msgSender());
        require(pendingReward > 0, "You have no node rewards to claim");
        safeMiningTokenTransfer(_msgSender(), pendingReward);
        userClaimedNodeRewards[_pid][_msgSender()] = userClaimedNodeRewards[_pid][_msgSender()].add(pendingReward);
    }
    ...
    function getTotalRedeemableLPAmount(uint256 _pid, address user) public view returns(uint256 totalRedeemableLPAmount) {
    ...
    function getUSDTAmountTransferToLPAmount(uint256 _pid, uint256 _amount) public view returns(uint256) {
    ...
    function decisionLevel(uint256 _pid) public {
    ...
    function claim(uint256 _pid) public whenNotPaused nonReentrant {
        require(enableContract == true,"The contract has not started");
    ...
    function closeAllRedeemedDepositInfo(uint256 _pid) public {
        for(uint256 i = 0; i < userDepositInfo[_pid][_msgSender()].length; i++) {
    ...
    function closeAllOngoingDepositInfo(uint256 _pid) public {
        for(uint256 i = 0; i < userDepositInfo[_pid][_msgSender()].length; i++) {
            if(userDepositInfo[_pid][_msgSender()][i].onGoing) {
                userDepositInfo[_pid][_msgSender()][i].withdrawTime = block.timestamp;
                userDepositInfo[_pid][_msgSender()][i].onGoing = false;
    ...
    function getUserAllDepositInfo(uint256 pid, address addr) public view returns (DepositInfo[] memory) {
