...
contract Presale is Ownable {
  ...
  function isExtraWhitelisted(address _user) public view returns (bool) {
    return extraWhitelisted[_user];
  }

  function getClaimable(address _user) public view returns (bool) {
    return openClaim == true && claimed[msg.sender] == false && boughtTokens[_user] && saleStartTime.add(privateSalePeriod) < block.timestamp;
  }
