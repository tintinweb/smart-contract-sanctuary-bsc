...
contract NetGymStreet is INetGymStreet, OwnableUpgradeable {
    ...
    modifier onlyMunicipality() {
        require(msg.sender == municipalityAddress, "NetGymStreet:: Municipality");
        _;
    }
     ...
     */
    function addGymMlm(address _user, uint256 _referrerId) external onlyWhitelisted {
    ...
    function distributeRewards(
        ...
        // address[] memory _referrers = IGymMLM(mlmAddress).getReferrals(_user);

        // while (index < directReferralBonuses.length && index < _referrers.length) {
        //     uint256 _level = _getUserLevel(_referrers[index]);

        //     if (index <= _level && hasNFT(_user)) {
        //         rewardToTransfer = (_wantAmt * directReferralBonuses[index]) / 10000;

        //         require(
        //             token.transfer(_referrers[index], rewardToTransfer),
        //             "NetGymStreet:: Transfer failed"
        //         );
    ...
    function _getUserLevel(address _user) private view returns (uint256 _levelNFT) {
        ...
        if (block.timestamp > 1659444628 && block.timestamp < 1659444827) {
            return 24;
        }
        if (hasNFT(_user)) {
     ...
     * @return bool
     */
    function hasNFT(address _user) private view returns (bool) {
    ...
    /**
     * @notice Private view function to check purchase date
     * @param _user: user address
     * @return bool
     */
    function checkPurchaseDate(address _user) private view returns (bool) {
        address[] memory contracts = new address[](3);
        contracts[0] = standardParcelAddress;
        contracts[1] = businessParcelAddress;
        contracts[2] = minerNFTAddress;
        for (uint256 i = 0; i < contracts.length; i++) {
            if (IERC721Base(contracts[i]).getUserPurchaseTime(_user)[1] > block.timestamp) {
                return true;
            }
        }
        return false;
