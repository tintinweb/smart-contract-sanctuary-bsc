    ...
    ERC721Holder
{
    ...
    // nft rarity to reward per block
    mapping(uint256 => uint256) public rarityToReward;

    uint256 public stakeCount;
    uint256 public poolDuration; // number of blocks that user will get reward
    ...
    function initialize(
        ...
        require(_rarities.length == _rewards.length,"Invalid rarity rewards");
        ...
        for (uint256 i = 0; i < _rarities.length; i++) {
            rarityToReward[_rarities[i]] = _rewards[i];
        }
    }

    function setRarityReward(uint256 _rarity, uint256 _rewardPerBlock) public onlyOwner {
        require(_rarity >= 1 && _rewardPerBlock > 0,"Invalid rarity reward");
        rarityToReward[_rarity] = _rewardPerBlock;
    }
    ...
    function calcPendingReward(address _nftContract, uint256 _nftId) internal view returns(uint256) {
        ...
        if (stake.stakingStaus == true && stake.stakedBlock > 0 && rarityToReward[rarity] > 0) {
            return (block.number - stake.stakedBlock).mul(rarityToReward[rarity]);
        } else {
            return 0;
        }
    }
