...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'cf6e59c39d56039ffd3e8261b823fb2341fdbea4d81f695a24cd36e211876e88' // init code hash
            ))));
...
contract PancakeRouter {
    ...
    function getSwapOutUsdt(address tokenA, address tokenB, uint256 coinNum) external view returns (uint256, uint256) {
        address pair = PancakeLibrary.pairFor(factory, tokenA, tokenB);
        (uint256 getCoinNum, uint256 nowPrice) = IPancakePair(pair).getSwapOutUsdt(tokenA, coinNum);
        return (getCoinNum, nowPrice);
    }
