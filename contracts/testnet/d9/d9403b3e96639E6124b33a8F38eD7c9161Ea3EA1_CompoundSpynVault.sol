...
contract CompoundSpynVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function deposit(uint256 amount, address referrer) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (amountToken > 0) {
            uint256 depositFeeAmount = amountToken.mul(depositFee).div(10000);
            if (depositFeeAmount > 0) {
                token1.transfer(treasury, depositFeeAmount);
            }
            uint256 feeTaken = payDepositReferralCommission(msg.sender, amountToken, address(token1));
            amountToken = amountToken.sub(feeTaken).sub(depositFeeAmount);
        }
        ...
        require(token1.balanceOf(address(this)) >= amountLeft, "invalid token1 balance");
        require(spyn.balanceOf(address(this)) >= spynOut, "invalid spyn balance");
    ...
    function depositTokenPair(
        ...
        if (amountToken1Final > 0) {
            uint256 depositFeeAmount = amountToken1Final.mul(depositFee).div(10000);
            if (depositFeeAmount > 0) {
                token1.transfer(treasury, depositFeeAmount);
            }
            uint256 feeTaken = payDepositReferralCommission(msg.sender, amountToken1Final, address(token1));
            amountToken1Final = amountToken1Final.sub(feeTaken).sub(depositFeeAmount);
        }

        if (amountSpynReceived > 0) {
            uint256 depositFeeAmount = amountSpynReceived.mul(depositFee).div(10000);
            if (depositFeeAmount > 0) {
                spyn.transfer(treasury, depositFeeAmount);
            }
            uint256 feeTaken = payDepositReferralCommission(msg.sender, amountSpynReceived, address(spyn));
            amountSpynReceived = amountSpynReceived.sub(feeTaken).sub(depositFeeAmount);
        }
    ...
    function depositLP(uint256 amount, address referrer) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (amountPairReceived > 0) {
            uint256 depositFeeAmount = amountPairReceived.mul(depositFee).div(10000);
            if (depositFeeAmount > 0) {
                pair.transfer(treasury, depositFeeAmount);
            }
            uint256 feeTaken = payDepositReferralCommission(msg.sender, amountPairReceived, address(pair));
            amountPairReceived = amountPairReceived.sub(feeTaken).sub(depositFeeAmount);
        }
    ...
    function withdraw(uint256 amount, bool isReceiveToken1) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
    ...
    function harvest(bool isReceiveToken1) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (reward > 0) {
            ...
            if(isReceiveToken1) {
                ...
                uint256 harvestFeeAmount = amountToken1.mul(harvestFee).div(10000);
                if (harvestFeeAmount > 0) {
                    token1.transfer(treasury, harvestFeeAmount);
                }
                uint256 feeTaken = payHarvestReferralCommission(msg.sender, amountToken1, address(token1));
                amountToken1 = amountToken1.sub(feeTaken).sub(harvestFeeAmount);
                ...
                uint256 harvestFeeAmount = amountSpyn.mul(harvestFee).div(10000);
                if (harvestFeeAmount > 0) {
                    spyn.transfer(treasury, harvestFeeAmount);
                }
                uint256 feeTaken = payHarvestReferralCommission(msg.sender, amountSpyn, address(spyn));
                amountSpyn = amountSpyn.sub(feeTaken).sub(harvestFeeAmount);
    ...
    function payHarvestReferralCommission(address _user, uint256 _pending, address _token) internal returns(uint256 feeTaken) {
    ...
    // Pay referral commission to the referrer who referred this user.
    function payDepositReferralCommission(address _user, uint256 _depositedAmount, address _token) internal returns (uint256 feeTaken) {
        if (address(spynReferral) != address(0)) {
