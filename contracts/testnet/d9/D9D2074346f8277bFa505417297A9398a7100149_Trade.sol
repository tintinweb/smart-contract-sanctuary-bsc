...
contract Trade is AccessControl {
    ...
    uint256 baseBuyerFeePermile;
    uint256 baseSellerFeePermile;
    ...
    // buyer platformFee
    uint256 public buyerFeePermille;
    //seller platformFee
    ...
    mapping(address => mapping(string => mapping(uint256 => PaymentFee))) private paymentAssets;

    mapping(address => bool) internal paymentAddressStatus; 

    mapping(string => bool) internal categoryStatus; 

    mapping(address => uint256) private buyerFees;

    mapping(address => uint256) private sellerFees;
    ...
    constructor(
        ...
        baseBuyerFeePermile = _buyerFee;
        baseSellerFeePermile = _sellerFee;
    ...
    function buyerServiceFee() external view virtual returns (uint256) {
    ...
    */
    ...
    function setBuyerServiceFee(uint256 _buyerFee)
        external
        onlyRole("ADMIN_ROLE")
        returns (bool)
    {
    ...
    */
    ...
    function setSellerServiceFee(uint256 _sellerFee)
        external
        onlyRole("ADMIN_ROLE")
        returns (bool)
    {
    ...
    */
    ...
    function addPaymentDetails(address payementToken, string memory category, PaymentFee[] memory paymentDetails) external onlyRole("ADMIN_ROLE") {
    ...
    function setBuyerFeeForToken(address tokenAddress, uint256 fee) external returns(bool) {
        buyerFees[tokenAddress] = fee;
        emit BuyerFee(fee);
        return true;
    }

    function setSellerFeeForToken(address tokenAddress, uint256 fee) external returns(bool) {
        sellerFees[tokenAddress] = fee;
        emit SellerFee(fee);
        return true;
    }
    ...
    */
    ...
    function getFees(
        ...
        uint256 royaltyFee;
        uint256 assetFee;
        string memory category = ICategory(order.nftAddress).getCategory(order.tokenId);
        (uint256 _buyerFee, uint256 _sellerFee) = paymentAddressStatus[order.erc20Address] && categoryStatus[category] ? txFees(order.erc20Address, category, order.seller, order.buyer) : (baseBuyerFeePermile, baseSellerFeePermile) ;
    ...
    */
    ...
    function txFees(address erc20Address, string memory category, address sender, address buyer) internal view returns(uint256 buyFee, uint256 sellFee) {
        ...
        for(uint8 i = 0; i < 4; i++) {
            if(buyerBalance >= paymentAssets[erc20Address][category][i].minimumAmount && buyerBalance <= paymentAssets[erc20Address][category][i].maximumAmount) {
                buyFee = buyerFees[erc20Address] - buyerFees[erc20Address] * paymentAssets[erc20Address][category][i].buyerFeePermileOffer / 100;
            }
            if(sellerBalance >= paymentAssets[erc20Address][category][i].minimumAmount && sellerBalance <= paymentAssets[erc20Address][category][i].maximumAmount) {
                sellFee = sellerFees[erc20Address] - sellerFees[erc20Address] * paymentAssets[erc20Address][category][i].sellerFeePermileOffer / 100;
            }
        }
