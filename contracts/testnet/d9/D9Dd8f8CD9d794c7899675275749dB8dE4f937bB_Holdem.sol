...
contract Holdem is Pausable, SellTime, IChips, Random, ERC1155{
    ...
    mapping (uint => uint) private _chipsIndex;
    mapping (uint => uint) private _chipsPurchaseAmount;
    //Contracts
    ...
    function getChipsIndex() private returns(uint){
        ...
        if(randomNumber > (permitSellAmount.sub(_chipsPurchaseAmount[1]))){
            _chipsPurchaseAmount[2] = _chipsPurchaseAmount[2].sub(1);
            return 2;
        }else{
            _chipsPurchaseAmount[1] = _chipsPurchaseAmount[1].sub(1);
            return 1;
        }
    }
    ...
    function buyChipsBatchByUsdt(
        ...
        indexs[0] = 1;
        indexs[1] = 2;
        mintBatch(msg.sender, indexs, chipsAmount);
    }
...
//uri
    ...
    function tokenId2Chips(uint tokenId) public view returns(uint){
        return _chipsIndex[tokenId];
    }
...
//_mint
    ...
    function mintBatch(address recipient, uint[] memory chipsIndex, uint[] memory amount) public onlyOwner{
...
//onlyOwner
    ...
    function setJpgChips(uint amount) external onlyOwner{
        _chipsPurchaseAmount[1] = amount;
    }

    function setGIFChips(uint amount) external onlyOwner{
        _chipsPurchaseAmount[2] = amount;
    }
    ...
    function setNewPermitSell(
        ...
        _chipsPurchaseAmount[1] = _jpgChips;
        _chipsPurchaseAmount[2] = _GIFChips;
    }
