...
contract FixedStaking is Ownable {
    ...
    function unStake(uint256 _amount) external {
        ...
        launchpad(launchpadAddress).transferIDOTokenToFixed(
            address(this),
            _user,
        ...
        IDO(address(idoToken)).burn(
            address(this),
            (initialAmount * (_amount / 1000000000000000000))
        );
    ...
    function receivedIDOToken(address _user, uint256 _amount) internal {
        idoTransfer[_user] = idoTransfer[_user] - _amount;
        idoBalance[_user] = idoBalance[_user] + _amount;
    }
