...
contract YetiGrinder is IERC20 {
  ...
  uint256 private constant startingSupply = 3_000_000_000;
  string private constant _name = "YETI Chain";
  ...
  uint256 public constant maxBuyTaxes = 100;
  uint256 public constant maxSellTaxes = 100;
  ...
  function setInitializers(
    address aInitializer,
    address cInitializer
  ) external onlyOwner {
  ...
  function setDividendExcluded(address account, bool enabled) public onlyOwner {
    require(account != address(this) && account != lpPair && account != DEAD);
  ...
  function setTaxes(uint16 buyFee, uint16 sellFee) external onlyOwner {
    ...
    require(
      buyFee <= maxBuyTaxes && sellFee <= maxSellTaxes,
  ...
  function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
    ...
    if (
      IERC20_BUSD.allowance(address(this), address(cashier)) !=
      type(uint256).max
    ) {
    ...
    uint256 marketingBalance = amtBalance -
      (rewardsBalance + developmentBalance);
    ...
    if (ratios.development > 0) {
      try
        IERC20_BUSD.transfer(_taxWallets.development, developmentBalance)
      {} catch {}
    }
  }
