...
contract MyNFT is ChargeFee, ERC721Full, BusinessRole, MinterRole, Lockable {
    ...
    function create(address user) public onlyMinter returns (uint256) {
    ...
    function multipleMint(address to, uint256 numItems) public onlyMinter {
        for (uint256 j = 0; j < numItems; j++) {
            create(to);
        }
    }

    function multipleMintAccounts(address[] memory tos, uint256[] memory numItems) public onlyMinter {
    ...
    function getUser(uint256 tokenId) public virtual returns (UserInfo memory) {
