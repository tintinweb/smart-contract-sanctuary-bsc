...
contract AzuroV2MockCore {
    ...
    // A simplified Azuro V2 Condition just for testing purposes
    struct CreatedCondition {
        uint256[2] odds;
        uint256[2] outcomes;
        uint256 timestamp;
        bytes32 ipfsHash;
    ...
    // An Azuro V2 Condition. Required by 'shift' endpoint
    struct Condition {
    ...
    mapping(uint256 => CreatedCondition) public createdConditions;
    ...
    function createCondition(
        ...
        CreatedCondition memory createdCondition;
        createdCondition.odds[0] = _odds[0];
        createdCondition.odds[1] = _odds[1];
        createdCondition.outcomes[0] = _outcomes[0];
        createdCondition.outcomes[1] = _outcomes[1];
        createdCondition.timestamp = _timestamp;
        createdCondition.ipfsHash = _ipfsHash;

        createdConditions[_cryptoGameId] = createdCondition;

        // NB: a real <Condition> with 'timestamp' is required for testing the 'shift' endpoint, which calls
        // 'getCondition(cryptoGameId)' and compares the timestamps.
        Condition memory condition;
        condition.timestamp = _timestamp;

        conditions[_cryptoGameId] = condition;
    }
    ...
    function deleteCreatedCondition(uint256 _cryptoGameId) external onlyMaintainer {
        ...
        delete conditions[_cryptoGameId];
    }
