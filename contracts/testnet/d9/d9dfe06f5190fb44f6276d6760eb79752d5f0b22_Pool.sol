...
contract Pool is Initializable, PoolStorage, OwnableUpgradeable, ReentrancyGuardUpgradeable, IPool {
    ...
    function reinit(address _oracle) external reinitializer(4) {
    ...
    function _getPoolValue(bool _max) internal view returns (uint256 sum) {
        ...
        for (uint256 i = 0; i < allTranches.length;) {
            sum += _getTrancheValue(allTranches[i], _max);
            unchecked {
                ++i;
            }
        }
    ...
    function _getTrancheValue(address _tranche, bool max) internal view returns (uint256 sum) {
        ...
        for (uint256 i = 0; i < allAssets.length;) {
            ...
            uint256 price = _getPrice(token, max);
    ...
    function _getPrice(address _token, Side _side, bool _isIncrease) internal view returns (uint256) {
        return _isIncrease && _side == Side.LONG ? oracle.getPrice(_token, true) : oracle.getPrice(_token, false);
    }
