...
contract Claim is Ownable {
    ...
    address public dynamicAddr = 0xA2B0964fE9761054FDca6c035e9de5C58d19199B;
    ...
    function permitClaim(address msgSender,address contractAddr,string memory funcName,uint256 _claimId,uint256 amount,uint256 claimType,uint256 deadline, uint8 v, bytes32 r, bytes32 s) private {
        require(block.timestamp <= deadline, "EXPIRED");
        uint256 tempNonce = nonces[msgSender]; 
        nonces[msgSender] = nonces[msgSender].add(1); 
        bytes32 message = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(msgSender, contractAddr, funcName, _claimId,amount, claimType,deadline, tempNonce))));
        address recoveredAddress = ecrecover(message, v, r, s);
        require(recoveredAddress != address(0) && recoveredAddress == signAddress, 'INVALID_SIGNATURE');
    }
   
    // claimType 1:static 2:dynamic
    function claim( uint256 _claimId,uint256 amount, uint256 claimType,uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        require(claimId[_claimId] == 0,"claim id have been used");
        require(claimType == 1 || claimType == 2,"wrong claimType");
        claimId[_claimId] = 1;
        permitClaim(msg.sender, address(this),"claim",_claimId,amount,claimType,deadline, v, r, s);
        if (claimType == 1){
            safeTransfer(basicToken, msg.sender, amount); 
        }
        else{
            uint256 dynamicFee = (amount.mul(dynamicFeeRate)).div(10000);
            safeTransfer(basicToken, dynamicAddr, dynamicFee); 
            safeTransfer(basicToken, msg.sender, amount.sub(dynamicFee)); 
        }
