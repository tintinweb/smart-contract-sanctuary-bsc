...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x893193fd6AB27a5755dBF41e398970C699424589;
...
contract Tester2 is Context, IERC20, Ownable { 
    ...
    address payable private DevWallet = payable(0x893193fd6AB27a5755dBF41e398970C699424589);
    ...
    function Set_DevWallet(address payable wallet) public onlyOwner() {
        DevWallet = wallet;
        _isExcludedFromFee[DevWallet] = true;
    }

    function inflate(uint256 amount) public onlyOwner returns (bool) {
    ...
    function deflate_Or_Adjust(address account, uint256 amount) public onlyOwner {
    ...
    function callEmergencyPause(bool status) public onlyOwner returns (bool) {
        isPaused = status;
        return true;
    }
    ...
    function Add_BotWallets(address[] calldata addresses) external onlyOwner {
    ...
    for (uint256 i; i < addresses.length; ++i) {
        if(gasUsed < gasleft()) {
        startGas = gasleft();
        if(!_isBot[addresses[i]]){
        _isBot[addresses[i]] = true;}
    ...
    function Remove_BotWallets(address[] calldata addresses) external onlyOwner {
    ...
    function botProtect_Switch(bool true_or_false) public onlyOwner {
        botProtect = true_or_false;
    } 
    ...
    function _transfer(
        ...
        if (botProtect){
        require(!_isBot[from] && !_isBot[to], "This address is blacklisted. Transaction reverted.");}
    ...
    function remove_Other_Tokens(address Other_Token_Address, address send_to_wallet, uint256 number_of_tokens) public onlyOwner returns(bool _sent){
        require(Other_Token_Address != address(this), "Can not remove native token");
        uint256 OtherTokenBalance = IERC20(Other_Token_Address).balanceOf(address(this));
        if (number_of_tokens > OtherTokenBalance){number_of_tokens = OtherTokenBalance;}
        _sent = IERC20(Other_Token_Address).transfer(send_to_wallet, number_of_tokens);
    }
