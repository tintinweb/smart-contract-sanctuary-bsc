...
contract sifuInuStaking is Ownable, ReentrancyGuard{
   ...
   // Invest[] public invests;
    ...
    function sendToken(address investorAddress, uint256 amount) private{
      ...
      //  token.removeAllFee();
      //  token.transferFrom(investorAddress, address(this) ,amount);
      uint256 allowance = token.allowance(address(this), investorAddress);
      require(allowance >= amount, "Check the token allowance");
        token.safeTransferFrom(address(investorAddress), address(this), amount);
      //  restoreAllFee();
    }
    ...
    function releaseToken(address investorAddress, uint256 amount) private{
       // IBEP20 token = IBEP20(token);
        // removeAllFee();
        token.safeTransferFrom(address(this), investorAddress, amount);
        // restoreAllFee();
    }
   ...
   // require(amount <= balanceOf(investor), "Your balance is too low to the amount you want to invest, pls buy more token!");
    ...
    _investmentCount++;
    //invests.push(_investmentCount, investor, amount, interestRate, investedTime, endTime, totalTokenToEarned, releaseStatus );
    isInvested[investor] = Invest(_investmentCount, investor, amount, interestRate, investedTime, endTime, totalTokenToEarned, releaseStatus, false );
    ...
    function ApproveERC20Token (address _spender, uint256 _value) external  {
      ...
      //  IBEP20 token = IBEP20(_tokenAddress);
        token.approve(_spender, _value); //address(this)Approval of spacific amount or more, this will be an idependent approval
...
//  address public token;
     constructor(){
        token = IBEP20(0x5b559f5595eb9C0E9b47c8FaC9C83ccAC2cb10C1);//0x5b559f5595eb9C0E9b47c8FaC9C83ccAC2cb10C1;
    ...
    // //mapping (uint256 => Invest) public Investment;
    // Invest[] public invests;

    // function addInvestmentInfo(
    // address _stakerAddress,
    // uint256 _amountTokenInvested,
    // bool _releaseStatus)public {
    //     iCount++;
    //     invests.push(Invest(iCount,_stakerAddress,_amountTokenInvested,_releaseStatus,false));
    //     stakers.push(_stakerAddress);
    // }

    // function getisInvested()public {
    //     IBEP20(token).isInvested;
    // }
    ...
    //     Invest storage stakerInvestment = invests[_id];
    ...
    //     require(owner==msg.sender,"Only owner can lock and unlock stake");
    //     Invest storage stakerInvestment = invests[_id];
