...
contract HoloSeed is ERC721A,Ownable{
    IHoloBasic Basic;

    uint256 Max_mint = 10;
    uint256 public Max_Total = 1310;

    uint256 Price_HS = 149;

    string public URL;
    
    constructor(address _HoloBasic) ERC721A("HoloSeed","HPS",Max_mint,Max_Total){
        Basic=IHoloBasic(address(_HoloBasic));
    URL="https://bafybeie7pjotv6yjo5cptijuxwpilvk4hsc7qlvv6qsrn2ll5uq3w2jxfq.ipfs.w3s.link/json/";
    _safeMint(Basic.get_G(), 4);
    _safeMint(Basic.get_V(), 4);
    _safeMint(Basic.get_I(), 2);
    }

    function Set_Price_HS(uint256 newPrice_HS) public onlyOwner {
        Price_HS=newPrice_HS;
    }

    function Mint(address _token) public {
        address sender = msg.sender;
    require(totalSupply()+Max_mint<=Max_Total);
    require(balanceOf(sender)==0);

    uint256 _value = Price_HS*(10^Basic.getdecimalsallowedTokens(_token));

    Basic.distributionSeed(_token, _value, sender);
    _safeMint(sender, 1);
  } 
...
function _baseURI() internal view virtual override returns (string memory) {
  ...
  function update_URL(string memory new_URL)  public onlyOwner{
    ...
    bool success = IBEP20(_token).transferFrom(address(this),msg.sender, _amount);
