...
contract UniswapV2Pair is UniswapV2ERC20, IUniswapV2Pair {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IUniswapV2Factory(factory).feeToAddress();
        uint feeToPercent = IUniswapV2Factory(factory).feeToPercent();
        feeOn = feeTo != address(0) && feeToPercent > 0;
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    uint numerator = totalSupply.mul(rootK.sub(rootKLast)).mul(feeToPercent);
                    uint denominator = rootK.mul(uint(100).sub(feeToPercent)).add(rootKLast.mul(feeToPercent));
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) override external lock {
        ...
        uint balance0Adjusted = balance0.mul(10000).sub(amount0In.mul(feeBP));
        uint balance1Adjusted = balance1.mul(10000).sub(amount1In.mul(feeBP));
