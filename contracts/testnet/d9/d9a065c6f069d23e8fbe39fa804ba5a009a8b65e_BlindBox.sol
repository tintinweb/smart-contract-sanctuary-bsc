...
contract  BlindBox is  Ownable, ReentrancyGuard  {
    ...
    constructor() public {
        acceptCurrency = IERC20(0xAed1A0f7D249B8dE575259364e0AC756bd4b5AEb);
        ...
        _materialIdToPrice[1] = 1 * 1e18;
        _materialIdToPrice[2] = 1 * 1e18;
        _materialIdToPrice[3] = 1 * 1e18;
        _materialIdToPrice[4] = 1 * 1e18;
        _materialIdToPrice[5] = 1 * 1e18;
    }
    ...
    function buyBox(uint256 materialId, uint256 amounts) public nonReentrant {
        ...
        emit BuyBox(_msgSender(), materialId, amounts);
    }

    function randNumber(uint256 materialId) public returns (uint256){
        nonce ++;
        uint result = SafeMath.add(_materialIdBaseAmount[materialId] , (uint(keccak256(abi.encodePacked(now, block.difficulty, msg.sender, nonce)))  %  _materialIdRandomRange[materialId]));
