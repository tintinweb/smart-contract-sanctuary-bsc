...
contract DutchAuction is Initializable, ReentrancyGuard, Ownable {
    ...
    function initialize(
        ...
        require(_treasury != address(0), "DutchAuction: wallet is the zero address");
        ...
        IERC20(auctionToken).safeTransferFrom(_funder, address(this), _totalTokens);
    ...
    function tokensClaimable(address _user) public view returns (uint256 claimerCommitment) {
        ...
        claimerCommitment = commitments[_user] * totalTokens / commitmentsTotal;
    ...
    function withdrawTokens(address beneficiary) public nonReentrant {
        if (auctionSuccessful()) {
            ...
            uint256 _claimableAmount = tokensClaimable(beneficiary);
            require(_claimableAmount > 0, "DutchAuction: No tokens to claim");
            claimed[beneficiary] = claimed[beneficiary] + _claimableAmount;
            _safeTransferToken(auctionToken, beneficiary, _claimableAmount);
        } else {
            ...
            _safeTransferToken(payToken, beneficiary, fundsCommitted);
        }
    }
