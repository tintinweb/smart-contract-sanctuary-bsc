...
contract LPChefV2 is Ownable {
    ...
    address public oasToken;
    address public micUsdtPair;
    address public micMebPair;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 _micWorth = calcMicWorth2(_amount);
    ...
    function harvest(uint256 _pid) public {
        ...
        if (pool.lockSecond == 0) {
            ...
            if (_pending != 0) {
                userCurrentInfos[msg.sender].totalAward = userCurrentInfos[msg.sender].totalAward.add(_pending);
                IOASToken(oasToken).mintByChef(msg.sender, _pending);
            }
        } else {
            ...
            if (totalPedding != 0) {
                IOASToken(oasToken).mintByChef(msg.sender, totalPedding);
            }
        }
    }
    ...
    function calcMicWorth2(uint256 lpAmount) public view returns (uint256) {
        uint256 micMebLpTotalSupply  = IERC20(micMebPair).totalSupply();
        (uint112 _micMebReserve0, uint112 _micMebReserve1,) = IPancakePair(micMebPair).getReserves();
        uint256 micAmount = lpAmount.mul(uint256(_micMebReserve1)).div(micMebLpTotalSupply);

        (uint112 _micUsdtReserve0, uint112 _micUsdtReserve1,) = IPancakePair(micUsdtPair).getReserves();
        uint256 micPrice = uint256(_micUsdtReserve0).mul(1e18).div(_micUsdtReserve1);
        return micAmount.mul(micPrice).div(1e18);
    }
