...
contract DeSwapAggregator is Ownable {
    ...
    constructor(uint256 _fee, address payable _feeWallet) public {
        ...
        locker
    {
        ...
        uint256 amt = afterFee(msg.value);
    ...
    function swapTokensForTokens(
        ...
        uint256 amt = afterFee(_amountIn);

        inToken.safeTransferFrom(msg.sender, address(this), _amountIn);

        uint256 allowance = inToken.allowance(address(this), router);

        if (allowance < amt) {
            inToken.approve(router, uint256(-1));
        }
    ...
    function getReturn(
        ...
        IUniswapV2Router r = IUniswapV2Router(router);
        uint256 amt = afterFee(amountIn);
        return r.getAmountsOut(amt, path);
    }
    ...
    function getInput(
        ...
        return r.getAmountsIn(amt, path);
    }
