    ...
    ReentrancyGuard
{
    ...
    function setChainlink(
        MediaEyeOrders.Chainlink memory _chainlink
    ) external {
        require(
            hasRole(ROLE_ADMIN, msg.sender) ||
                hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
    ...
    function _sellClaim(
        ...
        // get split info for collection
        MediaEyeOrders.Split memory split;
        if (IERC165(_auction.nfts[0].nftTokenAddress).supportsInterface(type(IMediaEyeSplitInfo).interfaceId)) {
            split = IMediaEyeSplitInfo(_auction.nfts[0].nftTokenAddress).getSplitInfo();
            if (split.royaltyBasisPoint > 0) {
                payoutToCreator = (_auctionSignature.price * split.royaltyBasisPoint) / 10000;
            }
        }
        else if (IERC165(_auction.nfts[0].nftTokenAddress).supportsInterface(type(IERC2981).interfaceId)) {
            (royaltyArtist, payoutToCreator) = IERC2981(_auction.nfts[0].nftTokenAddress)
                .royaltyInfo(_auction.nfts[0].nftTokenId, _auctionSignature.price);
            split.royaltyArtist = payable(royaltyArtist);
        }

        // payout to Charity/sellers
        uint256 remainingPayout = (_auctionSignature.price - payoutToTreasury - payoutToCreator);
        if (
