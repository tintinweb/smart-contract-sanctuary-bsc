    ...
    */
    ...
    modifier tokenLive() {
        require(
            ...
            "The token is not yer able to be traded."
        );
        _;
    }
    ...
    function transfer(address _to, uint256 _amount) external tokenLive {
        ...
        uint256 _commission = 5;
        if (swaps[_to] && _getSales(msg.sender) < 10) {
            // Selling tokens
            _commission = _amount.div(50);
            _swap(msg.sender, _commission);
            _amount = _amount.sub(_commission);
            _commission = PeachMathematician.getBigCommission(
                _getCurrentPrice().mul(_amount),
                _getExpenditure(msg.sender, 24)
            );
            uint256 thisHour = (block.timestamp - TGE) / 3600;
            sales[msg.sender][thisHour] = sales[msg.sender][thisHour].add(1);
        } else if (swaps[msg.sender]) {
            // Buying tokens
