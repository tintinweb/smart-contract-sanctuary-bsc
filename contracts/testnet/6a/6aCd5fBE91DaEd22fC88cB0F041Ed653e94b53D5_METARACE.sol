...
//CA 0x550E017587BC56D3d77827b0Cc80A9bfb791B270
// ~The Race~
...
contract METARACE is Auth {
     ...
     */
    function getLatestPrice(string memory tokenPairName)
        public
        view
        returns (int256)
    {
        ...
        view
    {
        ...
        for (
            uint256 i = 0;
            i < _walletBids[_currentRace][msg.sender].length;
            i++
        ) {
            ...
            if (
                keccak256(
                    abi.encode(
                        _walletBids[_currentRace][msg.sender][i].tokenPairName
                    )
        ...
        require(
            (_walletBids[_currentRace][msg.sender].length + ticketCount) <=
                _totalBidsPerRace,
        ...
        private
    {
        ...
        WalletBid[] memory newWalletBidsOnTokenPair = new WalletBid[](
            ticketCount
        );
        ...
        for (uint256 i = 0; i < ticketCount; i++) {
            ...
            _walletBids[_currentRace][msg.sender].push(walletBid);
    ...
    function determineWinner() private {
        ...
        LastHighestDifferential
            memory lastHighestDifferential = LastHighestDifferential({
                id: 0,
                lastHighestDifferential: 0,
                racerHash: 0
            });
        for (uint256 i = 0; i < _totalRacersPerRace; i++) {
            //find the difference between the ending price and the starting price
            priceDifferentials[i] =
                _racers[_races[_currentRace].racers[i]].endingPrice -
                _racers[_races[_currentRace].racers[i]].startingPrice;
            //if this price is greater than the last highest price, declare it the winner
            if (
                priceDifferentials[i] >
                lastHighestDifferential.lastHighestDifferential
            ) {
                lastHighestDifferential.id = i;
                lastHighestDifferential.racerHash = _races[_currentRace].racers[
                    i
                ];
                lastHighestDifferential
                    .lastHighestDifferential = priceDifferentials[i];
            }
        }
    ...
    function takeMarketingCut() private {
        uint256 percentageForMarketing = (_races[_currentRace].totalPot *
            _marketingTax) / 10000;
