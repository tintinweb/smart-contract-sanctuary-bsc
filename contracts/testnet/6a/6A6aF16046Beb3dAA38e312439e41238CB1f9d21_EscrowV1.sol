...
contract EscrowV1{
    ...
    function newEscrow(
        ...
        if(txToken == IERC20(0x0000000000000000000000000000000000000000)){
            require(msg.value == amount,"Pass same  val");
    ...
    function refund(uint256 _escrowId) public onlySeller(_escrowId) {
        ...
        localTransfer(txToken,idToEscrow[_escrowId].buyer,idToEscrow[_escrowId].amount + idToEscrow[_escrowId].fee);
    ...
    function claimAdminFee(uint256 _version) public onlyOwnerOrAdmin {
        if(_version == currentVersion){
            ...
            localTransfer(txToken,owner,adminCut[_version]);
        }
        else{
            //versionedTxToken[_version].transfer(msg.sender,adminCut[_version]);
            localTransfer(versionedTxToken[_version],owner,adminCut[_version]);
        }
        adminCut[_version]= 0;
    }

    function claimBrokerCut(uint256 _version) public {
