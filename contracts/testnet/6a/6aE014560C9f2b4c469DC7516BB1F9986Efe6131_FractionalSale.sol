...
// File: contracts/ERC20Fractional.sol
...
// File: contracts/FractionalSale.sol
...
contract FractionalSale {
    ...
    event Fractionalized(uint256 itemId, uint256 fractionsCreated, uint256 frTime);

    modifier onlyTokenOwner(uint256 itemId) {
        require(msg.sender == _market.getItemOwner(itemId), "FractionalSale: must be token owner");
        _;
    }

    function setMarketplaceContract(address marketplaceAddr) external {
        require(msg.sender == _market.owner(), "FractionalSale: mp owner only");
    ...
    /// @param _totalShares Total amount of fractional tokens to create
    ...
    function fractionalize(
        ...
        require(_totalShares > 0, "FractionalSale: invalid input");
        require(_idToFractionInfo[itemId].frToken == address(0), "FractionalSale: already fractionalized");
        ...
        emit Fractionalized(itemId, _totalShares, block.timestamp);
    }
