...
contract MainToken is ERC20BurnableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    rewardDistributorInterface public rewardDistributor;
    ...
    function initialize() public initializer {
        ...
        marketingReceiver = owner_;
        rewardReceiver = owner_;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if(!isDividendExempt[sender]){ try rewardDistributor.setShare(sender, _balances[sender]) {} catch {} }
        if(!isDividendExempt[recipient]){ try rewardDistributor.setShare(recipient, _balances[recipient]) {} catch {} }
    ...
    function swapBack() internal swapping {
        ...
        payable(DEAD).transfer(amountToBurn);
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 totalBNBFee = totalFee.sub(burnFee).sub(swapLiquidityFee.div(2));
            ...
            payable(rewardReceiver).transfer(amountBNBRewards);
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        if(exempt){
            rewardDistributor.setShare(holder, 0);
        }else{
            rewardDistributor.setShare(holder, _balances[holder]);
        }
    }
    function setIsFeeExempt(address _wallet, bool _exempt) external onlyOwner {
        isFeeExempt[_wallet] = _exempt;
    }
    function setIsFreezeExempt(address _wallet, bool _exempt) external onlyOwner {
        isFreezeExempt[_wallet] = _exempt;
    }
    function setIsBlacklisted(address _wallet, bool _exempt) external onlyOwner {
        isBlacklisted[_wallet] = _exempt;
    }
    function setIsTxLimitExempt(address _wallet, bool _exempt) external onlyOwner {
        isTxLimitExempt[_wallet] = _exempt;
    }
    function setFees(uint256 _rewardFee, uint256 _marketingFee, uint256 _liquidityFee, uint256 _burnFee, uint256 _feeDenominator) external onlyOwner {
        rewardFee = _rewardFee;
        marketingFee = _marketingFee;
        burnFee = _burnFee;
        liquidityFee = _liquidityFee;
        totalFee = _rewardFee.add(_marketingFee).add(_burnFee).add(_liquidityFee);
        feeDenominator = _feeDenominator;
        uint256 effectiveTax = totalFee.mul(100).div(feeDenominator);
        require(effectiveTax <= 20, "Taxes are set too high");
    }
    function setFeeReceivers(address _liquidityReceiver, address _marketingReceiver, address _rewardReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        marketingReceiver = _marketingReceiver;
        rewardReceiver = _rewardReceiver;
        rewardDistributor = rewardDistributorInterface(rewardReceiver);
    }
    ...
    function clearStuckBNB(address wallet) external onlyOwner {
