...
contract SwapToBurnBSCTestnet is SwapToBurnBase {
    constructor(
        uint256 _burnFee,
        uint256 _teamFee,
    ...
    function LUFFY() public pure override returns (address) {
        return 0x523CA978c5712EB010c8E6699d381bd9D0d07A9c;
    }

    function UNISWAP_V2_ROUTER() internal pure override returns (IUniswapV2Router02) {
        return IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    }

    function UNISWAP_FACTORY() internal pure override returns (IUniswapV2Factory) {
        return IUniswapV2Factory(0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F);
    }
}
...
abstract contract SwapToBurnBase is Ownable {
    ...
    function swapTokenForToken(
        ...
        uint256 amountInSub = _realAmountIn - burnAmount - teamAmount;
        ...
        if (IERC20(tokenIn).allowance(address(this), teamWalletAddress) != type(uint256).max) {
            _approveForTeamWallet(tokenIn);
        }
        IERC20(tokenIn).safeTransferFrom(address(this), teamWalletAddress, teamAmount);
    }
    ...
    function swapTokenForExactToken(
        ...
        uint256[] memory amounts = UNISWAP_V2_ROUTER().swapTokensForExactTokens(
            _amountOut,
            adjustedAmountIn - adjustedBurnFee - adjustedTeamFee,
            _path,
            msg.sender,
            block.timestamp
        );
        ...
        uint256 refundAmount = adjustedAmountIn - realAmountIn - burnAmount - teamAmount;
        if (refundAmount > 0) {
    ...
    function swapTokenForETH(
        ...
        if (tokenIn == WETH) {
            ...
            uint256 amountInSub = adjustedAmountIn - burnAmount - teamAmount;
            UNISWAP_V2_ROUTER().swapExactTokensForETHSupportingFeeOnTransferTokens(
                amountInSub,
                _amountOutMin,
                _path,
        ...
        validate(_path)
    {
        ...
        if (tokenOut == WETH) {
            ...
            uint256 amountInSub = amountIn - burnAmount - teamAmount;
        ...
        returns (uint256)
    {
        ...
        uint256 amountInSub = _amountIn - burnAmount - teamAmount;
    ...
    function _approveForTeamWallet(address token) internal {
        (bool success, ) = token.call(
            abi.encodeWithSignature(
                "approve(address,uint256)",
                teamWalletAddress,
