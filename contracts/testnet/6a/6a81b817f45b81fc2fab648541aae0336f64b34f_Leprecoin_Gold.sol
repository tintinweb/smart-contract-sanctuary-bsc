...
contract Leprecoin_Gold {
    using SafeMath for uint256;

    /** base parameters **/
    uint256 public GOLDS_TO_HIRE_1MINERS = 1440000;
    uint256 public REFERRAL = 70;
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public TAX = 9;
    uint256 public MKT = 12;
    uint256 public MARKET_GOLDS_DIVISOR = 2;

    uint256 public MIN_INVEST_LIMIT = 1 * 1e18; /** 0.01 BNB  **/
    ...
    function hireMoreFarmers(bool isCompound) public {
        ...
        if(isCompound) {
            uint256 dailyCompoundBonus = getDailyCompoundBonus(msg.sender, goldsForCompound);
            goldsForCompound = goldsForCompound.add(dailyCompoundBonus);
        ...
        user.miners = user.miners.add(goldsForCompound.div(GOLDS_TO_HIRE_1MINERS));
        user.claimedGolds = 0;
    ...
    function sellCrops() public{
        ...
        if(user.dailyCompoundBonus < COMPOUND_FOR_NO_TAX_WITHDRAWAL){
            ...
            goldValue = goldValue.sub(goldValue.mul(WITHDRAWAL_TAX).div(PERCENTS_DIVIDER));
        }else{
        ...
        user.claimedGolds = 0;  
        user.lastHatch = block.timestamp;
        marketGolds = marketGolds.add(hasGolds.div(MARKET_GOLDS_DIVISOR));
        
        if(getBalance() < goldValue) {
            goldValue = getBalance();
        }

        uint256 goldsPayout = goldValue.sub(payFees(goldValue));
    ...
    function hireFarmers(address ref) public payable{
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                ...
                users[upline].referralGoldRewards = users[upline].referralGoldRewards.add(refRewards);
                totalRefBonus = totalRefBonus.add(refRewards);
            }
        }

        uint256 goldsPayout = payFees(msg.value);
        totalStaked = totalStaked.add(msg.value.sub(goldsPayout));
        totalDeposits = totalDeposits.add(1);
        hireMoreFarmers(false);
    }

    function payFees(uint256 goldValue) internal returns(uint256){
        uint256 tax = goldValue.mul(TAX).div(PERCENTS_DIVIDER);
        uint256 mktng = goldValue.mul(MKT).div(PERCENTS_DIVIDER);
        dev1.transfer(tax);
        dev2.transfer(tax);
        dev3.transfer(tax);
        prtnr1.transfer(tax);
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _miners,
     uint256 _claimedGolds, uint256 _lastHatch, address _referrer, uint256 _referrals,
	 uint256 _totalWithdrawn, uint256 _referralGoldRewards, uint256 _dailyCompoundBonus, uint256 _farmerCompoundCount, uint256 _lastWithdrawTime) {
    ...
    function getAvailableEarnings(address _adr) public view returns(uint256) {
        uint256 userGolds = users[_adr].claimedGolds.add(getGoldsSinceLastHatch(_adr));
    ...
    function calculateGoldSell(uint256 golds) public view returns(uint256){
        return calculateTrade(golds, marketGolds, getBalance());
    }
    ...
    function calculateGoldBuySimple(uint256 eth) public view returns(uint256){
        return calculateGoldBuy(eth, getBalance());
    }

    /** How many miners and golds per day user will recieve based on BNB deposit **/
    function getGoldsYield(uint256 amount) public view returns(uint256,uint256) {
        uint256 goldsAmount = calculateGoldBuy(amount , getBalance().add(amount).sub(amount));
    ...
    function calculateGoldSellForYield(uint256 golds,uint256 amount) public view returns(uint256){
        return calculateTrade(golds,marketGolds, getBalance().add(amount));
    }
    ...
    function getMyGolds() public view returns(uint256){
        return users[msg.sender].claimedGolds.add(getGoldsSinceLastHatch(msg.sender));
    }
