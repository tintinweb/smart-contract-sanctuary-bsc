...
contract TicketSystemCD is Ownable, ReentrancyGuard, Pausable {
    ...
    event TicketBetted(address indexed user, uint256 numofTicket);
    event TicketWon(address indexed recipient, uint256 numofTicket);
    ...
    constructor() {
        ticketPrice = 1 * 10**18;
        teamPercentage = ticketPrice / 10;
        rewardPoolPercentage = ticketPrice / 40;
        burnPercentage = ticketPrice / 40;
     ...
     */
    function purchaseTicket(uint256 numOfTicket)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        uint256 teamAmount = numOfTicket * teamPercentage;
        uint256 rewardPoolAmount = numOfTicket * rewardPoolPercentage;
        uint256 burnAmount = numOfTicket * burnPercentage;

        amount = ticketAmount + teamAmount + rewardPoolAmount + burnAmount;

        bool success = token.transferFrom(_msgSender(), address(this), amount);
        require(success, "Purchase Ticket: Purchased Failed");

        bool teamTransfer = token.transfer(teamAddress, teamAmount);
        require(teamTransfer, "Team trasnfer failed");

        bool rewardPoolTrasnfer = token.transfer(rewardPool, rewardPoolAmount);
        require(rewardPoolTrasnfer, "RewardPool transfer failed");

        bool burnTrasnfer = token.burn(burnAmount);
        require(burnTrasnfer, "Burn failed");

        ticketBalance[_msgSender()] += numOfTicket;

        emit FeesTransfered(teamAmount, rewardPoolAmount, burnAmount);
     ...
     * @param numOfTicket to select quantity of tickets to purchase
     */
    function returnTicket(uint256 numOfTicket)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        require(balance >= ticketAmount, "Not enough balance in the contract");

        ticketBalance[_msgSender()] -= numOfTicket;

        bool success = token.transfer(_msgSender(), ticketAmount);
        require(success, "Return Ticket: Return Failed");

        emit TicketReturned(_msgSender(), numOfTicket, ticketAmount);
    }
     ...
     * @param numOfTicket to select quantity of tickets to purchase
     */
    function betTicket(uint256 numOfTicket)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        emit TicketBetted(_msgSender(), numOfTicket);
    }

    /**
     * @dev Function to Purchase Tickets
     * @param recipient  address that will receive tickets
     * @param numOfTicket to select quantity of tickets to purchase
     */
    function wonTicket(address recipient, uint256 numOfTicket)
        external
        onlyAdmin
        whenNotPaused
        nonReentrant
    {
        ...
        ticketBalance[recipient] += numOfTicket;

        emit TicketWon(recipient, numOfTicket);
    }
    ...
    /**
     * @dev Function to set the new token address that is used Purchasing tickets
     * @param tokenAdd The new token address
     */
    function seTokenAddress(address tokenAdd) external onlyOwner {
    ...
    /**
     * @dev Function to set token amount that will be burned.
    ...
    function setRewardAddress(address newRewardPoolAddress) external onlyOwner {
        require(newRewardPoolAddress != address(0), "Invalid address");
