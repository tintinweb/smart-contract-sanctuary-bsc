    ...
    KeeperCompatibleInterface
{
    ...
    uint256 public iterationNumber = 2;
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            ...
            userTotalRewardsGenerated[account] += earned(account);
        }
        _;
    }
        ...
        updateReward(msg.sender)
    {
        ...
        token.safeTransferFrom(msg.sender, feeRecipient, _feeAmount);
    ...
    function autoCompound() public {
        uint256 usersLeft = walletList.length - autoCompoundIndex;
        uint256 start = autoCompoundIndex;
        uint256 iter;
        if (usersLeft > iterationNumber) {
            iterationActive = true;
            autoCompoundIndex = autoCompoundIndex + iterationNumber;
        ...
        for (uint256 i = start; i < start + iter; i++) {
            _autoCompound(walletList[i]);
        }
    }

    function _autoCompound(address _staker) internal updateReward(_staker) {
        uint256 userReward = earned(_staker);
        if (userReward > 0) {
            rewards[_staker] = 0;
            totalSupply = totalSupply.add(userReward);
            _balances[_staker] = _balances[_staker].add(userReward);
    ...
    function getTotalRewardsGenerated(address _address) external view override returns(uint256) {
        return userTotalRewardsGenerated[_address] + earned(_address);
    }

    function getWalletsBalances(uint256 _start, uint256 _end)
        external
        view
...
interface IStaking {
    ...
    /**
     * @dev Returns the total amount of rewards earned and claimed by `_address` combined.
     * @param _address Address to check.
     */
