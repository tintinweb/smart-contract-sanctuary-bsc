...
contract EBep20UUPS is EBep20V2, UUPSProxiable {
 ...
 * @author Evry.Finance
 */
contract EBep20V2 is EToken, EBep20Interface{
    /**
    ...
    function mint(uint mintAmount) external override returns (uint) {
        ...
        if (_enableToDeposit && _onlyDepositWhiteList) {
            if(!depositAccounts[msg.sender]){
                return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
            }
        }
...
contract UUPSProxiable {
    ...
    function updateCodeAddress(address newAddress) internal {
        require(
            bytes32(0xc5f16f0fcc639fa48a6947836d9850f504798523bf8c9a3a87d5876cf622bcf7) == UUPSProxiable(newAddress).proxiableUUID(),
            "Not compatible"
        );
...
abstract contract EToken is ETokenInterface, Exponential, TokenErrorReporter {
    ...
    function enableTokenToDeposit(bool _enable) external returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }

        _enableToDeposit = _enable;
        emit EnableTokenToDeposit(_enableToDeposit);

        return uint(Error.NO_ERROR);
    }

    event EnableTokenToDeposit(
        bool _enableToDeposit
    );

    function enableTokenToBorrow(bool _enable) external returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }

        _enableToBorrow = _enable;
        emit EnableTokenToBorrow(_enableToBorrow);

        return uint(Error.NO_ERROR);
    }

    event EnableTokenToBorrow(
        bool _enableToDeposit
    );

    function setOnlyDepositWhiteList(bool _enable) external returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }

        _onlyDepositWhiteList = _enable;
        emit SetOnlyDepositWhiteList(_onlyDepositWhiteList);

        return uint(Error.NO_ERROR);
    }

    event SetOnlyDepositWhiteList(
        bool _onlyDepositWhiteList
    );
    ...
    function setDepositAccountsAllowed(address _toWhitelist, bool _state) external returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }

        depositAccounts[_toWhitelist] = _state;
        emit DepositAccountsSet(_toWhitelist, _state);
    }

    event DepositAccountsSet(
        address whitelisted,
        bool state
    );
}
...
contract ETokenStorage {
    ...
    bool internal _enableToDeposit;
    bool internal _enableToBorrow;
    bool internal _onlyDepositWhiteList;
    mapping(address => bool) internal depositAccounts;
