...
contract LotteryContract is Ownable {
    ...
    function adminGiveFreeTickets(uint256 _lotteryId, uint256[] memory _ticketNumber, address _receipent) public onlyOwner{
        ...
        require(lotteries[_lotteryId].status == Status.Open, "Lottery ended");
        require(block.timestamp < lotteries[_lotteryId].endTime, "Either lottery is not open or Lottery is ended");
    ...
    function changeRewardPer(uint256 _lotteryId, uint256[7] memory _per) public onlyOwner {
        require(lotteries[_lotteryId].startTime > block.timestamp, "Lottery started");
    ...
    function addFunds(uint256 _lotteryId, uint256 _amount) public onlyOwner {
        require(lotteries[_lotteryId].status == Status.Open, "Lottery Drawned");
    ...
    function withdrawRemainingFunds(uint256 _lotteryId) public onlyOwner {
        ...
        IERC20(lotteries[_lotteryId].token).transfer(msg.sender, ((lotteries[_lotteryId].amount + lotteries[_lotteryId].addFunds)*80/100)-totalReward);
    }
    ...
    function drawnLottery(uint256 _lotteryId) public onlyOwner { 
        ...
        IERC20(lotteries[_lotteryId].token).transfer(msg.sender, ((lotteries[_lotteryId].amount + lotteries[_lotteryId].addFunds)*20/100));  // burn 20% token
    ...
    function viewTicketIdRewards(uint256 _ticketId, uint256 _lotteryId) public view returns(uint256, uint256){
        ...
        if(j>0) {
            ...
            uint256 rewards = (((lotteries[_lotteryId].amount + lotteries[_lotteryId].addFunds)*80/100) * lotteries[_lotteryId].rewardAmountPerBracket[j - 1] / 100) / winner[j-1]; // burn 20% token
            return (j, rewards);
        }
        return (j, 0);
    }
    ...
    function viewCountWinnersInLottery(uint256 _lotteryId) public view returns (uint256[] memory, uint256[] memory){      
        ...
        for (uint256 i = 0; i < 6; i++) {
            ...
            rewardPerBracket[i] = (((lotteries[_lotteryId].amount + lotteries[_lotteryId].addFunds)*80/100) * lotteries[_lotteryId].rewardAmountPerBracket[i] / 100);
