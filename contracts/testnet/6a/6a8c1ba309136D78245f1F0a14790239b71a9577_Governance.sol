...
contract Governance is Ownable {
    ...
    constructor(
        ...
        CRONA = IERC20(_crona);
        veCRONA = IVotingEscrow(_veCrona);
    }
    ...
    // Adjusts _owner's votes according to latest _owner's veCRONA balance
    function poke(address _owner) public {
        ...
        uint256 _weight = veCRONA.balanceOf(_owner, block.timestamp);
    ...
    // Add LP tokens
    function setLPTokens(address[] memory _pairs) external onlyOwner {
        for (uint256 i = 0; i < _pairs.length; i ++) {
            address token = _pairs[i];
            bool needPush = lpTokens[token] == false;
            lpTokens[token] = true;
            if (needPush) {
                _tokens.push(token);
            }
