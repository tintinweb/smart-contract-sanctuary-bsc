...
interface IPancakeRouter {
    function addLiquidityBNB(address token,uint256 amountTokenDesired,uint256 amountTokenMin,
        uint256 amountBNBMin,address to,uint256 deadline)external payable returns (
        uint256 amountToken,uint256 amountBNB,uint256 liquidity);
}
contract SafuInvestmentsPresale {
    using SafeMath for uint256;
    IPancakeRouter private constant PancakeFactory =
    IPancakeRouter(address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1));
    ...
    function setAddressInfo(address _presaleCreator,address _tokenAddress,address _unsoldTokensDumpAddress
    ...
    function setGeneralInfo(uint256 _totalTokens,uint256 _tokenPriceInWei,uint256 _hardCapInWei,
        uint256 _softCapInWei,uint256 _maxInvestInWei,uint256 _minInvestInWei,
        uint256 _openTime,uint256 _closeTime) external onlySafuFactory {
    ...
    function setPancakeswapInfo(uint256 _uniListingPriceInWei,uint256 _uniLiquidityAddingTime,
        uint256 _uniLPTokensLockDurationInDays,uint256 _uniLiquidityPercentageAllocation
        ) external onlySafuFactory {
    ...
    function setStringInfo(bytes32 _saleTitle,bytes32 _linkTelegram,bytes32 _linkDiscord,
        bytes32 _linkTwitter,bytes32 _linkWebsite) external onlyPresaleCreatorOrSafuFactory {
    ...
    function setSafuInfo(address _safuLiqLockAddress,uint256 _safuDevFeePercentage,
        uint256 _safuMinDevFeeInWei,uint256 _safuId) external onlySafuDev {
    ...
    function addwhitelistedAddresses(address[] calldata _whitelistedAddresses)external
    ...
    function addLiquidityAndLockLPTokens() external presaleIsNotCancelled {
        ...
        uint256 liqPoolBnbAmount = finalTotalCollectedWei.mul(uniLiquidityPercentageAllocation).div(100);
        uint256 liqPoolTokenAmount = liqPoolBnbAmount.mul(1e18).div(uniListingPriceInWei);
        token.approve(address(PancakeFactory), liqPoolTokenAmount);
        PancakeFactory.addLiquidityBNB{value : liqPoolBnbAmount}(
            address(token),
            liqPoolTokenAmount,
            0,
            0,
    ...
    function claimTokens()external whitelistedAddressOnly presaleIsNotCancelled
    investorOnly notYetClaimedOrRefunded{
