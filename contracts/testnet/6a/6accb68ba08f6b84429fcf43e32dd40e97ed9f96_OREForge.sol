...
contract OREForge is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable {
    ...
    mapping(uint256 => Metadata) private token_id;
    ...
    function initialize() initializer public {
        __ERC1155_init("https://oreshop.mypinata.cloud/ipfs/{id}");
        __Ownable_init();
        __Pausable_init();
    ...
    function getNFTFee() public view returns (uint256) {
        return NFTFee;
    }
    ...
    function getTransactionTokenAddress(string memory _type) public view returns (address) {
        return tokentype[_type];
    }
    function addTransactionTokenAddress(string memory _type, address tokenAddress) public onlyOwner {
    ...
    function mint(uint256 id, uint256 value, uint256 royal, uint256 nooftoken, string memory ipfsname, string memory ipfsimage, string memory ipfsmetadata ) public {
    ...
    function _saleToken(
        ...
        uint256 val = pBEPent(amount, NFTFee).add(amount);
        if(keccak256(abi.encodePacked((bidtoken))) == keccak256(abi.encodePacked(("BNB")))){   
            ...
            (uint256 _adminfee, uint256 roy, uint256 netamount) = calc(amount, _royal[tokenId], NFTFee);
