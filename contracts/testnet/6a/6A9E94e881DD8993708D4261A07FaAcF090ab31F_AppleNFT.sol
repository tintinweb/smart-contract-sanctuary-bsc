...
contract AppleNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public commonTime=1657555200;
    uint256 public discountCost;
    uint256 public maxSupply;
    // uint256 public maxMintAmount = 2;
    uint256 public startNum;
    uint256 public onwerNum=0;
    ...
    mapping(uint256 => uint256) public whiteListMintNumList;
    uint256 public whiteListNum;
    mapping(uint256 => address) public highWhiteList;
    mapping(uint256 => uint256) public highWhiteListMintNumList;
    mapping(uint256 => uint256) public highMintNumList;
    ...
    constructor(
        ...
        _setHighWhiteList(_highWhiteList);
    ...
    //    require(balanceOf(msg.sender)+_mintAmount <=maxMintAmount, "Only 2 mint per account");
        ...
        if(highWhiteList[_id]==address(msg.sender)){
            require(block.timestamp > whiteListTime,"Haven't started!");
            if(msg.value>0){
                uint256 highMintNum= highMintNumList[_id]+_mintAmount;
                if(highMintNum<3){
                    _goToMint(discountCost,_mintAmount);
                    highMintNumList[_id]=highMintNum;
                }else{
                    _goToMint(cost,_mintAmount);
                }
            }else{
                uint256 highWhiteListMintNum= highWhiteListMintNumList[_id]+_mintAmount;
                require(highWhiteListMintNum<2,"Only 1 mint per account!");
                startNum=startNum+1;
                if(specialIds[startNum]){
                    startNum=startNum+1;
                }
                _safeMint(msg.sender, startNum);
                highWhiteListMintNumList[_id]=highWhiteListMintNum; 
            }             
         }else if(whiteList[_id]==address(msg.sender)){
             require(block.timestamp > commonTime,"Haven't started!");
             uint256 mintNum= whiteListMintNumList[_id]+_mintAmount;
             if(mintNum<3){
                _goToMint(discountCost,_mintAmount);
                whiteListMintNumList[_id]=mintNum;
             }else{
                _goToMint(cost,_mintAmount);
             }
         }else{
    ...
    function _setHighWhiteList(address[] memory _highWhiteList) private {
   ...
   function setSpecialIds(uint256[] memory _specialIds) public onlyOwner {
    ...
    function setCommonTime(uint256  _commonTime) public onlyOwner {
    ...
    function _goToMint(uint256 _cost,uint256 _mintAmount) private {
            ...
            _payRoyality(_cost * _mintAmount);
    ...
    function onwerMint(uint256 _mintAmount) public onlyOwner {
            require(onwerNum+_mintAmount <= 2000,"No more than 2000!");
            for (uint256 i = 0; i <_mintAmount; i++) {
                onwerNum=onwerNum+1;
               
                _safeMint(msg.sender, onwerNum);
            }
    }
}