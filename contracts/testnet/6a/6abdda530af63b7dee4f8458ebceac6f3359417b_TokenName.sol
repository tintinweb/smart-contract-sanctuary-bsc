...
contract TokenName is Context, IBEP20 { 
    ...
    address payable public Wallet_Marketing = payable(0x26Eecc0e768d9A74201572ccAa68d403c3021a3A); 
    address payable private Wallet_Dev = payable(0x26Eecc0e768d9A74201572ccAa68d403c3021a3A);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 10**15 * 10**_decimals;
    string private constant _name = "Token Name"; 
    string private constant _symbol = unicode"TokenTicker"; 
    ...
    constructor () {
        _owner = 0x26Eecc0e768d9A74201572ccAa68d403c3021a3A;
    ...
    function _transfer(
		...
		if(to == owner() || to == Wallet_Dev|| to == address(pancakeswapV2Router)){
		} else {
			 if (to != owner() &&
				to != Wallet_Burn &&
				to != address(this) &&
        ...
        if(txCount >= swapTrigger && !inSwapAndLiquify && from != pancakeswapV2Pair && swapAndLiquifyEnabled){  
