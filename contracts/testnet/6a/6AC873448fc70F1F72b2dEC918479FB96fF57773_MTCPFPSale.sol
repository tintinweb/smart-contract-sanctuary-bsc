...
contract MTCPFPSale is math{
    ...
    function addWhitelist(address _wAddrs) internal returns(bool) {
        ...
        whitelistInfos[_wAddrs]._AngelQuota += whitelistInfos[_wAddrs]._AngelQuota.add(700 * 10 ** uint(18));
        return true;
    }
    ...
    function isWhitelist(address _Addrs) public view returns (bool) {
		...
		if(whitelistInfos[_Addrs]._AngelQuota == 0 && whitelistInfos[_Addrs]._buyAmount == 0){
            _isWhitelist = true;
		}
        return _isWhitelist;
    }
