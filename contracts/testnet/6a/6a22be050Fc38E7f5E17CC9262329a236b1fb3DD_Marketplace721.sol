...
contract Marketplace721 is Ownable, ReentrancyGuard {
    ...
    event ListingCanseled(uint256 indexed id);
    ...
    constructor(uint256 gracePeriod_) Ownable() {
    ...
    function buyNative(uint256 listingId) external payable nonReentrant {
    ...
    function bidNative(uint256 listingId) external payable nonReentrant {
        require(address(listings[listingId].currency) == address(0), "Listing isn't for ETH");
    ...
    function cancel(uint256 listingId) external {
        ...
        emit ListingCanseled(listingId);
    }
    ...
    function claimCollectible(uint256 listingId) external nonReentrant {
        ...
        IERC721(listings[listingId].collection).transferFrom(
            address(this),
            target,
    ...
    function _buy(uint256 listingId) private {
        ...
        listings[listingId].lastBidder = msg.sender;
        listings[listingId].claimed = true;
        IERC721(listings[listingId].collection).transferFrom(address(this), msg.sender, listings[listingId].tokenId);
