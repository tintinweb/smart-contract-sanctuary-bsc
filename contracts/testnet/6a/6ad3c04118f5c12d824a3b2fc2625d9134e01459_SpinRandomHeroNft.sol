...
contract SpinRandomHeroNft is Auth, PausableUpgradeable {
  ...
  event SpinBonus(address user, uint collectionId, uint16[] heroClassIds, uint[] heroPices, uint spinPrice, uint timestamp);
  ...
  function spin(uint _collectionId, IMarketPlace.PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    emit SpinBonus(msg.sender, _collectionId, classIds, heroPrices, amount, block.timestamp);
  }
  ...
  function _giveReward(uint _collectionId) private returns (uint16[] memory classIds, uint[] memory heroPrices) {
    Collection storage collection = collections[_collectionId];
    ...
    uint index = 0;
    uint[] memory picked = new uint[](collection.rates.length);

    for (uint i = 0; i < totalPicks; i++) {
      index = i % collection.rates.length;
      if (!_isInArray(index, picked, totalHeroAssigned)) {
        if (_genRandomRate(collection.rates[index] + i) <= collection.rates[index]) {
          tempHeroClassIds[totalHeroAssigned] = collection.heroClassIds[index];
          tempHeroPrices[totalHeroAssigned] = collection.heroPrices[index];
          totalHeroAssigned++;
    ...
    if (totalHeroAssigned < collection.numberHeroReceive) {
      for (uint i = totalHeroAssigned; i < collection.numberHeroReceive; i++) {
        ...
        for (uint j = 0; j < collection.rates.length; j ++) {
          ...
          if (!_isInArray(j, picked, totalHeroAssigned)) {
            tempHeroClassIds[totalHeroAssigned] = collection.heroClassIds[j];
            tempHeroPrices[totalHeroAssigned] = collection.heroPrices[j];
            totalHeroAssigned++;
          }
        }
      }
