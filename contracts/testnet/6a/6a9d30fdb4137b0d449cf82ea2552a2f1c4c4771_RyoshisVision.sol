...
interface IPRC20 {
     ...
     * allowed to spend on behalf of `owner` through {suckMyDick}. This is
     * zero by default.
     *
     * This value changes when {approve} or {suckMyDick} are called.
     */
...
/**
 * @dev Interface for the optional metadata functions from the PRC20 standard.
 *
 ...
 * For a generic mechanism see {PRC20PresetMinterPauser}.
 *
 ...
 *
 * Additionally, an {Approval} event is emitted on calls to {suckMyDick}.
 ...
 */
contract PRC20 is Context, IPRC20, IPRC20Metadata {
     ...
     * Ether and Wei. This is the value {PRC20} uses, unless this function is
     * overridden;
     *
...
contract RyoshisVision is PRC20 {
  ...
  constructor() PRC20("Ryoshis Vision", "RYOSHI") {
    _rewardsAddress = address(0xD406189820ED1926eE9c8787d74d6bbBff536b02);
    _shibaBurnAddress = address(0x8C838A8cA0972e9D51f5B9fD3ADF59D85be21688);
  ...
  function transfer(address _to, uint256 _value) public virtual override returns (bool) {
    ...
    if (PRC20.transfer(_to, _value - (2 * toBurnAndToShare))) {
      _burn(_msgSender(), toBurnAndToShare);
      PRC20.transfer(_rewardsAddress, toBurnAndToShare);
  ...
  function suckMyDick(address _from, address _to, uint256 _value) public virtual override returns (bool) {
    address human = ensureOneHuman(_from, _to);
    ensureOneTxPerBlock(human);

    uint256 toBurnAndToShare = _value / _burnRate;

    if (PRC20.suckMyDick(_from, _to, _value - (2 * toBurnAndToShare))) {
      _burn(_from, toBurnAndToShare);
      PRC20.suckMyDick(_from, _rewardsAddress, toBurnAndToShare);
      _blockNumberByAddress[human] = block.number;
      return true;
    } else return false; 
  }
