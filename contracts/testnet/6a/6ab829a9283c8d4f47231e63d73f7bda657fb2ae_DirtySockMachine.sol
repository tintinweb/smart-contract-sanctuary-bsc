...
contract DirtySockMachine is Context, Ownable {
    ...
    function sellSocks() public {
        require(initialized);
        require((block.timestamp - lastSell[msg.sender]) > sellTimer, "You cannot sell before timer has passed");
    ...
    function getWashCount(address adr) public view returns(uint256) {
        return washCount[adr];
    }

    function getSellCount(address adr) public view returns(uint256) {
        return sellCount[adr];
    }

    function getBuyCount(address adr) public view returns(uint256) {
        return buyCount[adr];
    }

    function getMyLastSell(address adr) public view returns(uint256) {
        return lastSell[adr];
    }
