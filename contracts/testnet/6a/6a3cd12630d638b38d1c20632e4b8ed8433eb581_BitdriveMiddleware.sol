...
contract BitdriveMiddleware is ContextUpgradeable, OwnableUpgradeable {
    ...
    function swapExactTokensForETHMiddleware(
        ...
        if (inputType == 1) {
            ...
            tokenContract.approve(adminAddress, calculation.adminfee);
            tokenContract.transfer(adminAddress, calculation.adminfee);
    ...
    function swapExactTokensForETHMiddleware1(
        address router,
        IBEP20 _tokenContract,
        address to,
        uint256 deadline,
    ...
    function swapExactTokensForETHMiddleware2(
        address router,
        IBEP20 _tokenContract,
        address to,
        uint256 deadline,
        ...
        if (inputType == 1) {
            ...
            //     inputAmount
            // );
            test2 = calculation.adminfee;
            tokenContract.approve(adminAddress, calculation.adminfee);
            tokenContract.transfer(adminAddress, calculation.adminfee);
            test3 = calculation.amountIn;
            IUniswapV2Router02 uniswapV2Router = IUniswapV2Router02(router);
            // amounts = uniswapV2Router.swapExactTokensForETH(
            //     calculation.amountIn,
            //     calculation.outputAmount,
    ...
    function swapExactTokensForTokensMiddleware(
        ...
        //tokenContract.transferFrom(msg.sender, address(this), inputAmount);
        TransferHelper.safeTransferFrom(
            inputToken,
            msg.sender,
            address(this),
    ...
    function bitdriveSettings(
        ...
        if (_changetype == 1) {
            require(_fee > 0 && sitefee > _fee, "Invalid fee");
            bitdrivefee = _fee;
        }
        if (_changetype == 2) {
    ...
    function withdrawsafeBNB(address payable _address, uint256 _amount) public {
        require(msg.sender == adminAddress, "FORBIDDEN");
