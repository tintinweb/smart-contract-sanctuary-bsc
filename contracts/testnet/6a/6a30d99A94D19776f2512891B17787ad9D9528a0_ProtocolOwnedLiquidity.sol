 ...
 */
        ...
        onlyRole(SWAP_ROLE)
    {
        ...
        uint256 bYSLPrice;
        console.log("what percent",(IERC20(Admin.bYSL()).balanceOf(user) / 400));
        if (byslAmount > (IERC20(Admin.bYSL()).balanceOf(user) / 400)) {
            bYSLPrice = IBYSL(Admin.bYSL()).backedPrice();
            console.log(bYSLPrice,"bYSLPrice backed Price");
            ...
            console.log(IERC20(Admin.BUSD()).balanceOf(Admin.POL()),"IERC20(Admin.BUSD()).balanceOf(Admin.POL())");
            uint256 slipageRate = (((bYSLPrice * byslAmount) /
                (IERC20(Admin.BUSD()).balanceOf(Admin.Treasury()) +
                    IERC20(Admin.BUSD()).balanceOf(Admin.POL()))) * 10000); //1000 is cofficient for slipage %
            console.log(bYSLPrice,"bYSLPrice backed Price 2");
            console.log(slipageRate,"slipageRate");
            console.log( (10000 - (slipageRate / 10 ** 18))," (100 - (slipageRate / 1000 ** 18)) * 100");
            uint256 deductSlipage = (10000 - (slipageRate / 10 ** 18)); //100 is cofficient
            console.log(deductSlipage,"deductSlipage");
            // uint256 BUSDForSwap = (((bYSLPrice * byslAmount) *
            //     (deductSlipage / 100)) / 100);
            uint256 BUSDForSwap = (bYSLPrice * deductSlipage) / 10000;
                console.log(BUSDForSwap,"BUSDForSwap");
                console.log(IBYSL(Admin.bYSL()).backedPrice(),"IBYSL(Admin.bYSL()).backedPrice()");
                console.log((BUSDForSwap / byslAmount),"(BUSDForSwap / byslAmount)");
            if (
                IBYSL(Admin.bYSL()).backedPrice() < (BUSDForSwap / byslAmount)
            ) {

                withdrawBUSD(BUSDForSwap / 10 ** 18, msg.sender);
                IBYSL(Admin.bYSL()).burn(address(this), byslAmount);
            } else {
...
*/
    function withdrawBUSD(uint256 amount, address user) internal {
        ...
        console.log("before withdraw",IERC20(Admin.BUSD()).balanceOf(address(this)));
        IERC20(Admin.BUSD()).transfer(user, amount);
        console.log("after withdraw",IERC20(Admin.BUSD()).balanceOf(address(this)));
