...
contract KunciNFTOffering is Ownable, ReentrancyGuard, Pausable {
    ...
    IKunciNFT public immutable kunciNFTCollection;

    uint256 public collectionId;    //Ids of Collection
    address public stashAddress;    //Address stash

    address public platform;        // Address of platfrom
    address public artist;          // Address of artist

    uint256 public platformFee;     //10000 = 100%
    uint256 public artistFee;       //10000 = 100%

    uint256 public collectionNftCount;  //Count of NFT Collection
    uint256 public nftSoldCount;        //Count of sold nft  
    uint256 public nftSoldAmount;       //Amount of nft was sold
    uint256 public buyLimit;            //Limit to buy
    uint256 pendingPayment; //Ensure that this address has enough balance when claimNFT is called

    mapping (uint256 => NftId) public nftList;          // List of the nft
    mapping (address => uint256) public buyPerAccount;  // collection was buyed per Account
    ...
    function collectionName() external view returns(string memory) {
        return kunciNFTCollection.name();
    }
    ...
    ) 
    {
        ...
        require(_collectionAddr != address(0), "Collection doesn't exists");

        kunciToken = IERC20(_kunciAddress);
        kunciNFTCollection = IKunciNFT(_collectionAddr);
    ...
    /// @notice update when posible
    /// @param _platform       platform of nft
    /// @param _artistAddress  artist of the nft
    ...
    /// @param _nftIds               IDs nft
    /// @param _isNftOfferingAuction is nft being offering on auction
    /// @param _prices               nft prices  
    /// @param _offeringStartTimes   start the offering time   
    /// @param _offeringEndTimes     ended time of offer
    ...
    ) external onlyOwner 
    {
        ...
        for (uint256 i; i < _nftIds.length;) {
            ...
            nft.start = (_start > block.timestamp ? _start : block.timestamp);
            nft.end = _end;
    ...
    /// @notice Set fee do not exceed 10%
    /// @param _platformFee Fee from the platfrom
    /// @param _artistFee   Fee from the artist
    function setFee(
        uint256 _platformFee,
        uint256 _artistFee
    ) external onlyOwner
    ...
    function withdrawKunci(bool _force) external onlyOwner
    {
        kunciToken.transfer(
            msg.sender, 
            kunciToken.balanceOf(address(this)) - (_force ? 0 : pendingPayment)
            );
    }

    /// @notice for managing the nft offering 
    /// @param _nftIds              IDs nft
    /// @param _isOfferingActives   true if the offering was active
    ...
    ) external onlyOwner
    {
        ...
        for (uint256 i; i < _nftIds.length;) {
            ...
            if(_isOfferingActives[i])
                require(kunciNFTCollection.balanceOf(stashAddress, _id) != 0, "NFT id is not existed");
    ...
    /// @notice for managing the nft prices 
    /// @param _nftIds       IDs nft
    /// @param _newPrices    new nft prices to sell  
    ...
    /// @notice for purchase buyer order
    /// @param _nftId   IDs nft
    ...
    whenNotPaused
    {
        ...
        require(nft.isNftOffering, "NFT isn't listed");
    ...
    whenNotPaused
    {
        ...
        for (uint256 i; i < _ids.length;) {
            ...
            require(nft.isNftOffering, "NFT isn't available");
            require(!nft.isNftAuction, "NFT is not fixed price");
    ...
    /// @notice for buyer to buy bidding prices
    /// @param _nftId   id's of nft
    ...
    whenNotPaused
    {
        ...
        unchecked{pendingPayment += (_amount - (_lastBuyer == address(0) ? 0 : _lastPrice));}
    ...
    /// @notice claimed nft to only the winner 
    /// @param _nftId   IDs nft 
    ...
    /// @notice for closing the auction offer
    /// @param _nftIds  IDs nft 
    function closeOfferingAuction(uint256[] calldata _nftIds) external onlyOwner 
    {
        for (uint256 i; i < _nftIds.length;) {
            ...
            if (_bidder != address(0)) {
                ...
                emit WinBid(_amount, _id, _bidder, address(kunciNFTCollection));
            }
