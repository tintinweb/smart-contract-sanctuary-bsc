...
//вернуть рееэнтранси гард?

/**
 * @title TokenManagement, used for approve once for all MF Token operations
 */
contract TokenManagement is AccessControl {
    ...
    bytes32 public constant MFCONTRACTS_ROLE = keccak256("MFCONTRACTS_ROLE");

    event boughtBooster(
        address indexed buyer,
        uint256 indexed boosterType,
        uint256 amount
    );

    /**
     * @dev Creates a token management contract.
    ...
    /**
     * @dev Set MF contracts addresses.
     * @param _address contract address
     */
    function setMFContractAddress(address _address)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _grantRole(MFCONTRACTS_ROLE, _address);
    }

    /**
     * @notice Receive MF Tokens from user. Used only by MF contracts
     * @param _user user address
     * @param _amount amount of MF Tokens
    ...
    function receiveTokensFromUser(address _user, uint256 _amount)
        external
        onlyRole(MFCONTRACTS_ROLE)
    {
        token.transferFrom(_user, address(this), _amount);
    }

    /**
     * @notice Receive MF Tokens from current user. Used by user to send tokens to the Token management balance
     * @param _amount amount of MF Tokens
     */
    ...
    /**
     * @notice Send MF Tokens to user. Used only by MF contracts
     * @param _user user address
     * @param _amount amount of MF Tokens
     */
    ...
    /**
     * @notice Send MF Tokens to user. Used by user to claim specified by server amount. vrs signature needed
     * @param _user user address
     * @param _amount amount of MF Tokens
     * @param _salt random hash. Protection against replay attacks.
     * @param vrs vrs signature from server
    ...
    /**
     * @notice Send MF Tokens to treasury contract balance. Used only by MF contracts
     * @param _amount amount of MF Tokens
     */
    ...
    /**
     * @notice Buy experience booster
     * @param _type type of booster
     * @param _amount amount of MF Tokens
     * @param _salt random hash. Protection against replay attacks.
     * @param vrs vrs signature from server
     */
    function buyBooster(
        ...
        token.transferFrom(msg.sender, address(this), _amount);
        emit boughtBooster(msg.sender, _type, _amount);
    }
}
...
/**
 * @title MFNFT (MetaFighter NFT contract)
 */
contract MFNFT is ERC721Enumerable, Ownable {
    ...
    /**
     * @dev Creates a MFNFT contract.
     * @param token_ address of the IERC20/BEP20 token contract
     * @param tokenManagement_ address of the token management contract (for approve once all operations with MF tokens)
     */
    constructor(address token_, address tokenManagement_)
        ERC721("Meta Fighter NFT", "MFNFT")
    {
        require(
            token_ != address(0) && tokenManagement_ != address(0x0),
            "Incorrect contracts addresses!"
        );
        _token = IERC20(token_);
        _tokenManagement = TokenManagement(tokenManagement_);
    }
     ...
     */
    function setMaxCharacterId(uint256 maxCharacterId_) public onlyOwner {
    ...
    function stakeMint(address _to, uint256 _characterId) external {
        ...
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(_to, tokenId);
        emit Minted(_to, _characterId, tokenId);
    }

    /**
     * @notice Mint for users for MF Tokens
     * @param _characterId character identifier
     * @param _salt random hash. Protection against replay attacks.
     * @param vrs signature
     */
    function safeMint(
        ...
        _tokenManagement.receiveTokensFromUser(to, _characterPrice);
        _tokenManagement.sendTokensToTreasury(_characterPrice);

        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(to, tokenId);
        emit Minted(to, _characterId, tokenId);
    }
}