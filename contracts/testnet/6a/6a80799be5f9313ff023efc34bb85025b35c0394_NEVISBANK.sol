...
contract NEVISBANK  is ERC20,Ownable  {
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private  {
    ...
    function rewards(address userAddress, uint256 amount) public onlyOwner() {
     ...
     function rewards_nevis(address userAddress, uint256 amount) public onlyOwner() {

        require(getContractTokenBalance() > amount,"Low Balance");
        nevis_token.transfer(userAddress,amount);
    ...
    function setPairaddress(address pair_address) public {
          if(msg.sender==commissionWallet){
                Pair = pair_address;
          }
    }

     function setRouteraddress(address router_address) public {
          if(msg.sender==commissionWallet){
