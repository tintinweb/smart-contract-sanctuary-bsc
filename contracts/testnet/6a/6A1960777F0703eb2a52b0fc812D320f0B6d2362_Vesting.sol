...
contract Vesting is Initializable, AccessControl {
    ...
    function updateVestingPoints(uint128[2][] memory _vestingPoints)
        external
    ...
    function setTokenSaleContract(address _address)
        external
        validation(_address)
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function ascendingSort(uint128[2][] memory arr)
    internal
    pure
    returns (uint128[2][] memory, uint256)
    {
...
interface IAdmin is IAccessControl {
    ...
    function descendingSort(uint128[2][] memory arr) external pure returns (uint128[2][] memory, uint256);
...
interface ITokenSale {
    ...
    struct Params {
        ...
        uint64 privateStart;
        uint64 privateEnd;
        uint128[2][] vestingPoints; // Percentage base is 1000
    }
     ...
     * @param _priceFeed for priIAdmince aggregator contract.
     */
    function initialize(
    ...
    function updateVestingPoints(uint128[2][] memory _vestingPoints)external;
    // function getVestingPoints() external returns(uint128[2][] memory);
    // function setBUSD(address) external;
    ...
    // fuction removeOtherERC20Tokens(address) external;
    ...
    // event ClaimHistory(address indexed user, uint256 amount, uint256 change);
}
