...
import "../../listing/listing-strategies/fixed-rate/IFixedRateListingController.sol";
...
import "../../tax/tax-strategies/fixed-rate/IFixedRateTaxController.sol";
    ...
    ERC20RewardDistributorStorage
{
    ...
    function distributeExternalReward(
        ...
        Listings.Listing memory listing = IListingManager(_metahub.getContract(Contracts.LISTING_MANAGER)).listingInfo(
            listingId
        );
        ...
        if (listingTerms.strategyId == ListingStrategies.FIXED_RATE_WITH_REWARD) {
            ...
            ) = _retrieveRewardPercentages(agreement, listingTerms);

            rentalExternalERC20RewardFees = _calculateExternalRewardBasedFees(
                token,
                rewardAmount,
            ...
            rentalExternalERC20RewardFees = _calculateExternalRewardForFixedRate(token, rewardAmount);
        }
    ...
    function _retrieveRewardPercentages(
        ...
        )
    {
        ...
        (, listerRewardPercentage) = IFixedRateWithRewardListingController(
            listingStrategyRegistry.listingController(listingTerms.strategyId)
        ).decodeStrategyParams(listingTerms);

        IFixedRateWithRewardTaxController fixedRateWithRewardTaxController = IFixedRateWithRewardTaxController(
            listingStrategyRegistry.listingTaxController(listingTerms.strategyId)
        );

        (, universeRewardTaxPercentage) = IFixedRateWithRewardTaxController(fixedRateWithRewardTaxController)
            .decodeStrategyParams(universeTaxTerms);

        (, protocolRewardTaxPercentage) = IFixedRateWithRewardTaxController(fixedRateWithRewardTaxController)
            .decodeStrategyParams(protocolTaxTerms);
    }
    ...
    function _calculateExternalRewardBasedFees(
        address token,
        uint256 rewardAmount,
        uint16 listerRewardPercentage,
        uint16 universeRewardTaxPercentage,
    ...
    function _calculateExternalRewardForFixedRate(address token, uint256 rewardAmount)
        internal
        pure
