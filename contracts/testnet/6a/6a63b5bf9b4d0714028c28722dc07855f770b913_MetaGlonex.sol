...
contract MetaGlonex is Context, IERC20, Ownable {
    ...
    string private _name = "MetaGlonex";
    string private _symbol = "MTGX";
    ...
    uint256 public _burnsellFee ;
     uint256 public _charitysellFee;
    ...
    address payable public charityWallet = payable(0xF28BCAD7510A7a9F485063c893d5BBD357800C9B);
    address payable  public buybackWallet = payable(0x45B046f8976dD3be23F81Eb1708eAF828F9450f4);
    ...
    uint256 public numTokensSellToAddToLiquidity = 90000 * 10**18;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _charityFee==0 && _burnFee==0 && _buybackfee==0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 combineFees = _liquidityFee.add(_charityFee).add(_buybackfee); // charity + dev + liquidity fees
        
        
        
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(combineFees); 
        uint256 tokendev = contractTokenBalance.mul(_buybackfee).div(combineFees);
        
        uint256 tokensForcharity = contractTokenBalance.sub(tokensForLiquidity).sub(tokendev);
        uint256 tokensFordev = contractTokenBalance.sub(tokensForLiquidity).sub(tokensForcharity);
        ...
        transferToAddressETH(charityWallet, transferredMBalance);
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tdev = tAmount.div(100).mul(_buybackfee);
    ...
    function setTaxFeePercent(uint256 taxFee,uint256 liquidityFee,uint256 charityFee,uint256 burnFee, uint buybackfee) external onlyOwner() {
        _taxFee = taxFee;
        _pretaxfee = _taxFee;
        _liquidityFee =  liquidityFee;
        _preliquidityfee = _liquidityFee;
        _charityFee = charityFee;
        _precharityfee = _charityFee;
    ...
    function setsellTaxFeePercent(uint256 taxsellFee,uint256 liquiditysellFee,uint256 charitysellFee,uint256 burnsellFee, uint256 buybackSellFee) external onlyOwner() {
