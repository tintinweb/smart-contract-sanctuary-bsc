...
contract BKCFINANCE3 is OwnableUpgradeable {
    ...
    function calculateRelease(uint slot_) public view returns (uint) {
        ...
        if (release.amount != 0) {
            ...
            total = user.stakeAmount;
            _day = releaseTime[getDay(slot_)];
        }

        uint start = user.stakeTime + user.lockTime;
        uint end = start + _day;
    ...
    function firstUnStake(uint slot_) internal {
        UserInfo storage user = userInfo[msg.sender][slot_];
        ...
        releasePoolInfo[msg.sender][slot_] = ReleasePoolInfo({
            ...
            // startTime: user.stakeTime + user.lockTime,
            lastTime: 0,
            claimedAmount: 0
        });
    }
    ...
    function unStake(uint slot_) public onlyEOA {
        ...
        require(user.stakeAmount > 0 || release.amount > 0, "no amount");
        require(
    ...
    function checkReward(
        ...
        public
        view
        returns (bool istake, uint[3] memory times, uint[5] memory amounts)
    {
        uint slot;
        for (uint i = 0; i < slotLength; i++) {
            if (userInfo[msg.sender][i].stakeAmount != 0) {
                slot = i;
                break;
            }
        }
        UserInfo storage user = userInfo[msg.sender][slot];
        ReleasePoolInfo storage release = releasePoolInfo[msg.sender][slot];

        (uint _toClaim, ) = calculateReward(slot, user_);

        uint _releaseTime;
        uint _stakeAmount;
        uint _claimed;
        if (release.amount != 0) {
            _stakeAmount = release.amount;
            _releaseTime = release.day;
            _claimed = release.claimedAmount;
        } else {
            _stakeAmount = user.stakeAmount;
            _releaseTime = releaseTime[getDay(slot)];
        }
        uint _toReleaseAmount = calculateRelease(slot);

        uint start = user.stakeTime + user.lockTime;
        uint end;
        if (start != 0) {
            end = start + _releaseTime;
        }

        return (
            isStake[user_],
            [start, end, _releaseTime],
            [slot, _toClaim, _stakeAmount, _toReleaseAmount, _claimed]
        );
    }
}
