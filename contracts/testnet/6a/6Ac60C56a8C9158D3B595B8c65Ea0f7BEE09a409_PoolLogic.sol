...
contract PoolLogic is ReentrancyGuardUpgradeable {
    ...
    function previewWithdrawal(bool isStablecoin, uint256 _unitOut)
        external
        view
            ...
            if (withdrawnAmount > 0) {
                ...
                if (isStablecoin) {
                    ...
                    if (!IPoolManagerLogic(poolManagerLogic).isDepositAsset(asset)) {
                        ...
                        uint256[] memory amountOuts = IPancakeRouter02(swapRouter).getAmountsOut(withdrawnAmount, path);
    ...
    function withdraw(bool isStablecoin, uint256 _unitOut) external nonReentrant {
        require(_unitOut > 0, "require number of unit");
    ...
    function swap(
        ...
        _swap(_tokenIn, _tokenOut, _amountIn, _amountOutMin, _getPathSwap(_tokenIn, _tokenOut), address(this));
    }
    ...
    function _swapForWithdrawal(
        ...
        uint256[] memory amountOutmin = IPancakeRouter02(swapRouter).getAmountsOut(_amountIn, path);
        if (amountOutmin.length <= 0) {
            return;
        }

        _swap(_tokenIn, _tokenOut, _amountIn, amountOutmin[0], path, to);
    }
    ...
    function _swap(
        ...
        uint256 allowance = IERC20Upgradeable(_tokenIn).allowance(address(this), address(swapRouter));
        if (allowance < _amountIn) {
            if (allowance > 0) {
                IERC20Upgradeable(_tokenIn).safeApprove(address(swapRouter), 0);
            }
            IERC20Upgradeable(_tokenIn).safeApprove(address(swapRouter), _amountIn);
        }

        IPancakeRouter02(swapRouter).swapExactTokensForTokens(_amountIn, _amountOutMin, path, to, block.timestamp);

        emit Swap(_tokenIn, _tokenOut, _amountIn, to);
    }
