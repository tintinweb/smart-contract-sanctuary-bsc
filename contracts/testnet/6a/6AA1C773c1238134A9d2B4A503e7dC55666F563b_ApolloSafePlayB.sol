...
// File contracts/ApolloSafePlay/ApolloSafePlayB.sol
...
/**
 * Apollo safe play token with burning
 */
...
contract ApolloSafePlayB is Context, IERC20Metadata, Ownable {
    // Token parameters
    string private NAME;
    ...
    address public platformOwner = 0x73B8B91E038F3C5Fb5f67C913F06D1d716ceF9A4;
    ...
    constructor(
        ...
        require(
            (_txFeeRates.burning + _txFeeRates.buyBack) <= MAX_TX_FEES,
    ...
    function setFeeRates(
        ...
        uint256 buyFees = _buyFeeRates.burning;
        uint256 sellFees = _sellFeeRates.burning;
    ...
    function _tokenTransfer(
        ...
        unchecked {
            _balances[sender] = balanceOf(sender) - (amount - values.tBurning);
        }

        _balances[recipient] = _balances[recipient] + values.tTransferAmount;
        if (takeFee && (isBuy || isSell)) {
            _takeBurn(sender, values.tBurning);
     ...
     */
    function _takeBurn(address sender, uint256 tBurning) private {
        totalFeesPaid.burning += tBurning;

        _burn(sender, tBurning);
    }
