...
contract UniswapV2Pair is UniswapV2ERC20 {
    ...
    function _update(uint256 balance0, uint256 balance1, uint256 _reserve0, uint256 _reserve1, uint256 _aReserve0, uint256 _aReserve1) private {
        ...
        if(aFactor > 10000){
            assert(_aReserve0 >= _reserve0 && _aReserve1 >= _reserve1);
    ...
    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external lock {
        ...
        //uint256 balance1;
        uint256[2] memory aReserve = [aReserve0, aReserve1];
        //uint256 _aReserve1 = aReserve1;
        { // scope for _token{0,1}, avoids stack too deep errors
            ...
            if (aFactor > 10000) {
                aReserve[0] = uint256(aReserve0.add(balance[0]).sub(_reserve0));
                aReserve[1] = uint256(aReserve1.add(balance[1]).sub(_reserve1));
            }
        }
        uint256 amount0In = balance[0] > _reserve0 - amount0Out ? balance[0] - (_reserve0 - amount0Out) : 0;
        uint256 amount1In = balance[1] > _reserve1 - amount1Out ? balance[1] - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'UniswapV2: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint256 balance0Adjusted = aReserve[0].mul(1000).sub(amount0In.mul(3));
        uint256 balance1Adjusted = aReserve[1].mul(1000).sub(amount1In.mul(3));
        require(balance0Adjusted.mul(balance1Adjusted) >= uint256(_reserve0).mul(_reserve1).mul(1000**2), 'UniswapV2: K');
        }

        _update(balance[0], balance[1], _reserve0, _reserve1, aReserve[0], aReserve[1]);
    ...
    function sync() external lock {
        _update(IERC20(token0).balanceOf(address(this)), IERC20(token1).balanceOf(address(this)), reserve0, reserve1, aReserve0, aReserve1);
    }
}
