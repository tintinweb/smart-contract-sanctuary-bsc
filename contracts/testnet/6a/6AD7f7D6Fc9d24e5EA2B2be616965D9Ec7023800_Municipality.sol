...
interface ISignatureValidatorUM {
...
import "./interfaces/ISignatureValidatorUM.sol";
...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public signatureValidatorUMAddress;
    ...
    ///@notice Set Signature Validator for United Metas Address
    function setSignatureValidatorUMAddress(address _signatureValidatorUMAddress) external onlyOwner {
        signatureValidatorUMAddress = _signatureValidatorUMAddress;
    }
     ...
     */
    function seedUsersBalance(address[] memory _users, uint256[] memory _parcelsAmount, uint256[] memory _minersAmount, uint256[] memory _upgradesAmount) external onlyOwner {
        for (uint256 i; i < _users.length; i++) {
            usersMintableNFTAmounts[_users[i]].parcels = _parcelsAmount[i];
            usersMintableNFTAmounts[_users[i]].miners = _minersAmount[i];
            usersMintableNFTAmounts[_users[i]].upgrades = _upgradesAmount[i];
            // emit UsersBalanceUpdated(_referrers[i], _users[i]);
        }
    } 
    //TODO: Add requirment that the current user can seed only once
    function seedUserNFTBalance(SeededDataSignature memory _seedingSignature) external {
        require(ISignatureValidatorUM(signatureValidatorUMAddress).verifySigner(_seedingSignature), "Municipality: Not authorized signer");
    ...
    function upgradeStandardParcelsGroup(uint256[] memory _parcelIds, uint8 _purchaseType) external onlySaleActive {
        ...
        if(totalUpgradePrice > 0) {
            ...
            if(_purchaseType == PURCHASE_TYPE_BUSD_METAVIE) {
                metavieAmount = busdAmount * 1 ether / _getMetaviePriceInBUSD() / 10;
    ...
    function upgradeAllMiners(uint8 _purchaseType) external onlySaleActive {
        ...
        lastPurchase.dollarValue += amount;
        _lastPurchaseDateUpdate(msg.sender);
        uint256[] memory upgradedMiners = new uint256[](miners.length);
        uint256[] memory levels = new uint256[](miners.length);
        ...
        for(uint i; i < miners.length; i++){
            ...
            if(minerLevel < 9){
                ...
                upgradedMiners[currentIndex] = miners[i];
                levels[currentIndex] = minerLevel+1;
    ...
    function attachMinerToParcel(uint256 _parcelId, uint256 _minerId) external {
        ...
        require(msg.sender == IERC721Base(minerV1NFTAddress).ownerOf(_minerId), "Municipality: You are not authorized for this action");
        ...
        require(minerToParcelMapping[_minerId] == 0,"Municipality: Miner is already attached");
    ...
    function detachMinerFromParcel(uint256 _parcelId, uint256 _minerId) external {
        ...
        for(uint i; i < parcelToMinersMapping[_parcelId].length; i++) {
            if(parcelToMinersMapping[_parcelId][i] == _minerId) {
                parcelToMinersMapping[_parcelId][i] = parcelToMinersMapping[_parcelId][parcelToMinersMapping[_parcelId].length-1];
                parcelToMinersMapping[_parcelId].pop();
                minerToParcelMapping[_minerId] = 0;
                IMining(miningAddress).decreaseHashrate(msg.sender,IMinerNFT(minerV1NFTAddress).hashrate(_minerId));
                return;
            }
        }
    ...
    function updateMinersOnParcel(uint256 _parcelId, uint256[] memory _newMiners) external {
        require(msg.sender == IERC721Base(standardParcelNFTAddress).ownerOf(_parcelId),"Municipality: You are not authorized for this action");
        uint256 parcelSlots = IParcelInterface(standardParcelNFTAddress).upgradedParcelsMapping(_parcelId) ? upgradedParcelSlotsCount: standardParcelSlotsCount;
        ...
        for(uint i; i < _oldMiners.length; i++) {
            if(!_arrayContains(_newMiners, _oldMiners[i])) {
                _oldMiners[i] = _oldMiners[_oldMiners.length-1];
                _oldMiners.pop();
        ...
        for(uint i; i < _newMiners.length; i++) {
            if(minerToParcelMapping[_newMiners[i]] != _parcelId){
                ...
                newHashrate += IMinerNFT(minerV1NFTAddress).hashrate(_newMiners[i]);
            }
        }
    ...
    function updateAttachedMiners(uint256[] memory _minersToAttach, uint256[] memory _minersToDetach) external {
        ...
        for(uint i; i < _minersToDetach.length; i++) {
            ...
            for(uint j; j < parcelToMinersMapping[parcel].length; j++) {
                if(parcelToMinersMapping[parcel][j] == _minersToDetach[i]) {
                    parcelToMinersMapping[parcel][j] = parcelToMinersMapping[parcel][parcelToMinersMapping[parcel].length-1];
        ...
        for(uint i; i < parcels.length; i++) {
            ...
            while(parcelToMinersMapping[parcels[i]].length < slots) {
                ...
                parcelToMinersMapping[parcels[i]].push(_minersToAttach[minerId]);
        ...
        require(minerId == _minersToAttach.length,"Municipality: Not enough slots");
    } 

    function autoAttach() external {
        uint256[] memory userMiners =  IERC721Base(minerV1NFTAddress).tokensOf(msg.sender);
        uint256[] memory userParcels = IERC721Base(standardParcelNFTAddress).tokensOf(msg.sender);
        require(userMiners.length > 0, "Municiplaity: Not enough miners");
        require(userParcels.length > 0, "Municiplaity: Not enough slots");
        uint256 parcelFreeSlots;
        uint256 iteratorOnMinersArray;
        for(uint256 i; i < userParcels.length && iteratorOnMinersArray < userMiners.length; ++i) {
            parcelFreeSlots = IParcelInterface(standardParcelNFTAddress).upgradedParcelsMapping(userParcels[i]) ? upgradedParcelSlotsCount: standardParcelSlotsCount;
            parcelFreeSlots -= parcelToMinersMapping[userParcels[i]].length;
            uint256 iter;
            for(uint256 j = iteratorOnMinersArray; j < userMiners.length && iter < parcelFreeSlots; ++j) {
                if(minerToParcelMapping[userMiners[j]] == 0) {
                    parcelToMinersMapping[userParcels[i]].push(userMiners[j]);
                    minerToParcelMapping[userMiners[j]] = userParcels[i];
                    IMining(miningAddress).increaseHashrate(msg.sender, IMinerNFT(minerV1NFTAddress).hashrate(userMiners[j]));
                    ++iter;
                }
                ++iteratorOnMinersArray;
            }
        }
    }
    ...
    function getMinersToDetach(address _user) external view returns(uint256[] memory) {
        uint256[] memory miners = IERC721Base(minerV1NFTAddress).tokensOf(_user);
        uint256 userSlots = _getUserSlots(_user);
        uint256 finalMinersAmount = userSlots < miners.length ? userSlots : miners.length;
        uint256[] memory newMiners = _getTopMiners(_user, finalMinersAmount);
        uint256[] memory minersToDetach = new uint256[](userSlots);
        uint256 minersCount;
        for(uint256 i; i < miners.length; i++) {
            if(minerToParcelMapping[miners[i]] != 0 && !_arrayContains(newMiners, miners[i])){
                minersToDetach[minersCount] = miners[i];
                minersCount++;
            }
        }
    ...
    function getMinersToAtach(address _user) external view returns(uint256[] memory) {
        uint256 userSlots = _getUserSlots(_user);
        uint256 userMinersAmount = IERC721Base(minerV1NFTAddress).balanceOf(_user);
        uint256 finalMinersAmount = userSlots < userMinersAmount ? userSlots : userMinersAmount;
        uint256[] memory newMiners = _getTopMiners(_user, finalMinersAmount);
        uint256[] memory minersToAtach = new uint256[](userSlots);
        uint256 minersCount;
        for(uint256 i; i < newMiners.length; i++) {
            if(minerToParcelMapping[newMiners[i]] == 0){
                minersToAtach[minersCount] = newMiners[i];
                minersCount++;
            }
        }
    ...
    function _getTopMiners(address _user, uint256 _count) private view returns(uint256[] memory) {
        uint256[] memory topMiners = new uint[](_count);
        uint256[] memory userMiners = IERC721Base(minerV1NFTAddress).tokensOf(_user);
        ...
        for(uint i = 1; i < userMiners.length; i++) {
            ...
            if(minerLevel <= IMinerNFT(minerV1NFTAddress).minerIdToLevelMapping(topMiners[minersCount - 1])) {
                ...
                if(minersCount == _count) {
                    ...
                    topMiners[minersCount] = topMiners[minersCount - 1];
                while(current != 0 && IMinerNFT(minerV1NFTAddress).minerIdToLevelMapping(topMiners[current - 1]) < minerLevel) {
    ...
    function _getPriceForSuperBundle(uint8 _bundleType, uint8 _purchaseType) private view returns(uint256[2] memory) {
        ...
        if(_purchaseType == PURCHASE_TYPE_BUSD_METAVIE) {
            ...
            busdAndMetavie[1] = discountedPrice * 1 ether / _getMetaviePriceInBUSD() / 10;
        }
    ...
    function arrayContains(uint256[] memory _miners, uint256 _miner) public pure returns(bool) {
        return _arrayContains(_miners, _miner);
    }
