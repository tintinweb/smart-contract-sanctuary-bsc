...
contract Reserve is Ownable, Initializable {
    ...
    function initialize(address tokenAdd, uint allocation, uint subsequentClaims, uint releaseClaims, uint vestingPeriod, uint cliffs, uint totalNumberOfVestings) external initializer {
        //require(vestingPeriod > 0, "Reserve : vestingPeriod > 0");
        //require(subsequentClaims > 0,"Reserve : subsequentClaims > 0");
    ...
    function addDistributionWallet( address[] memory account, uint[] memory amount, uint[] memory startTime) external onlyOwner {
        require(account.length < 30,"Reserve : length < 30");
    ...
    function claim() external {
       ...
       // require((user_.totalClaimed == 0) && ((user_.lastClaim + claimPeriod ) <= block.timestamp), "Reserve : lastClaim < block.timestamp");
