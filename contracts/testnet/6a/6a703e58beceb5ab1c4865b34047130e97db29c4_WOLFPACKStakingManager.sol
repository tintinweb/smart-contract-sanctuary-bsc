...
contract WOLFPACKStakingManager is Ownable, ReentrancyGuard {
    ...
    // reward balance will be the balanceOf(address(this)) in BUSD.
    // uint vault;

    //address rewardCurr = payable(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee); // testnet BUSD
    //IERC20 rewCurr = IERC20(rewardCurr);

    //address wolfpack = payable(0x3445B126fA960C30E7FbF6d3d32d53c410e07399); // testnet WPACK
    //IERC20 WPACK = IERC20(wolfpack);

    //uint public rewBalance = rewCurr.balanceOf(address(this));
    ...
    function contractRewardBalance() external view returns (uint contractRewBalance) {
        return REWARD.balanceOf(address(this));
    }

    function stake(uint amount) external {
        require(amount > 0, "stake: invalid input");

    }

    // 1.5 = uint 1500000000000000000 (for 18 decimal point tokens e.g. BUSD)

    // get stake 

    function getStake(address staker) public view returns(uint amt) {
        
    }

    // withdraw stake

    function withdrawStake(uint amount) external nonReentrant {
