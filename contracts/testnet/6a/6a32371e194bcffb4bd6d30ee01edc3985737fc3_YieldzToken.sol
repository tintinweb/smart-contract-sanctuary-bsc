...
contract YieldzToken is ERC20, Ownable {
    ...
    function createBond(uint256 period) external {
        ...
        if (period == 10) {
            rate_ = tenBondTotal * 10**DECIMALS / tenBondPrice / 10 days;
            IERC20(usdtAddress).transferFrom(msg.sender, treasury, tenBondTotal * 10**DECIMALS);
        } else {
            rate_ = twentyBondTotal * 10**DECIMALS / twentyBondPrice / 20 days;
            IERC20(usdtAddress).transferFrom(msg.sender, treasury, twentyBondTotal * 10**DECIMALS);
        }
