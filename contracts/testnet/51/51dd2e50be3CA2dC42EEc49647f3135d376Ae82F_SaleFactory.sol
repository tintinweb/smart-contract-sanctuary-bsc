...
interface iSaleMaster {
    function _getCouponDiscount(address _address, uint256 _couponNumber) external view returns (uint256 _couponDiscount);
    function _getCouponActive(address _address, uint256 _couponNumber) external view returns (uint256 _couponActive);
    function _useCoupon(address _address, uint256 _couponNumber) external ;
    ...
    function launchFee(uint256 _saleType) external view returns (uint256 _feeAmount);
...
contract SaleFactory {
  ...
  function initSaleFactory(address _saleMasterAddress) public onlyOwner {
  ...
  function setFeeTo(address feeReceivingAddress) external onlyOwner {
    feeAddress = feeReceivingAddress;
  }

  function refundExcessiveFee(uint256 _option) internal {
    uint256 refund = msg.value - _saleMaster.launchFee(_option);
    if (refund > 0) {
...
interface iSaleMaster {
    function _getBlacklist(address _address, address _contract) external view returns (uint256 is_blacklisted);
    function _createLock(uint8 _round, address _token, uint256 _token_amount, uint256 _weth_amount, uint256 _start, uint256 _end) external payable;
    function _getSaleVault(address _sale_address) external returns(address);
    function adminAddress() external returns(address);
    function feeAddress() external returns(address);
    function convert() external returns(uint256);
    function launchFee() external returns(uint256);
}
...
contract Sale is ReentrancyGuard {
    ...
    constructor(
        ...
        launchFee = _saleMaster.launchFee();
    ...
    function finalizeSale (uint8 _round) public nonReentrant IsSaleEnded(_round) {
        ...
        if(liquidityAmount > 0) {
            ERC20(sale_details.sale_token).transfer(address(_saleMaster._getSaleVault(address(this))), liquidityToken);
