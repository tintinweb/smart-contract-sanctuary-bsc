...
contract CarbitPairStake is Ownable,ReentrancyGuard,Pausable {
    ...
    constructor (IERC20 _stake,IERC20 _busd) {
         ...
         planDetails[1].percent = 15e18;
         planDetails[2].percent = 40e18;
         planDetails[3].percent = 60e18;

         planDetails[1].duration = 182 days + 12 hours;
         planDetails[2].duration = 365 days;
         planDetails[3].duration = 730 days;

         planDetails[1].claimCount = 2;
         planDetails[2].claimCount = 4;
         planDetails[3].claimCount = 4;
    }
    ...
    function harvest (uint256 _id) public nonReentrant whenNotPaused {
        userDetails storage user = investDetails[msg.sender][_id];
        plan storage plans = planDetails[user.userInPlan];
        require(user.depositTime > 0 && user.tokenAmount > 0,"Invalid user");
        require(block.timestamp >= user.lastclaim + (plans.duration/plans.claimCount),"No time to claim");
        require(planDetails[user.userInPlan].claimCount > user.userClaimCount,"claim already reached its time to unstake");
        uint reward = user.busdAmount*(user.stakePercent/plans.claimCount)/100e18;
        reward = ((block.timestamp - user.lastclaim) / 1 days) * reward;
        
        if (user.userInPlan != 3) {
        reward = reward/2;
