...
contract FiveElements is Permission {
    ...
    bytes public _AnewHash;
    function _getRandomNumber(address player) public returns (bytes memory){
        _updateRamdomSeed();
        _Bhash =  keccak256(abi.encodePacked(
            randomSeed, block.timestamp, (block.number - 1), player
        ));
        for(uint8 i = 0; i < 31; i++){
            _AnewHash[i] = bytes1(_Bhash[i]);
        }
        // _newhash = _AnewHash;
        return _AnewHash;
    }

    function _getRandomNumber2(address player) public returns (bytes memory){
        _updateRamdomSeed();
        _Bhash = sha256(abi.encodePacked(
            randomSeed, block.timestamp, (block.number - 1), player
        ));
        for(uint8 i = 0; i < 31; i++){
            _AnewHash[i] = bytes1(abi.encodePacked(_AnewHash, _Bhash[i]));
        }
        // _newhash = _AnewHash;
        return _AnewHash;
    }
