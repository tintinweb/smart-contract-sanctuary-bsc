...
// File vesting_mbh/tokenVestingAngelWheel.sol
...
contract TokenVestingAngelWheel is Ownable {
   ...
   * @param _beneficiarys address of the beneficiary to whom vested tokens are transferred
   ...
   */
    ...
    constructor(
        ...
        for (uint256 index = 0; index < _beneficiarys.length; index++) {
            require(_beneficiarys[index] != address(0));
        }
        
        require(_cliff <= _duration);

        beneficiarys = _beneficiarys;
   ...
   */
    ...
    function release(IERC20 _token) public {
        ...
        for (uint256 index = 0; index < share; index++) {
            _token.safeTransfer(beneficiarys[index], unreleased.div(share));
        }
   ...
   */
    function vestedAmount(IERC20 _token) public view returns (uint256) {
        ...
        if (block.timestamp < cliff) {
            ...
            uint256 period = duration.div(count);

            return totalBalance.div(count).mul(block.timestamp.sub(start)).div(period);
