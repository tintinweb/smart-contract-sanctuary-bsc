...
contract AvionCollection is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    function mint() external {
        ...
        require(currentSupply < TOTAL_SUPPLY, "AC: total supply exceeded");
        require(balanceOf(msg.sender) == 0, "AC: maximum amount of tokens per wallet exceeded");
        require(LINK.balanceOf(address(this)) >= CHAINLINK_FEE, "AC: not enough LINK");
        uint256 price = getPrice();
        distributeTokens(msg.sender, price);
    }

    function mintAtWhitelistPresalePeriod(bytes32[] calldata merkleProof) external {
    ...
    function mintAtPublicPresalePeriod() external {
    ...
    function mintRandom(uint256 _randomNumber, address _reciepient) internal {
        uint256 range = TOTAL_SUPPLY - currentSupply;
        uint256 index = _randomNumber % range + 1;
        mintInternal(index, _reciepient);
    }
