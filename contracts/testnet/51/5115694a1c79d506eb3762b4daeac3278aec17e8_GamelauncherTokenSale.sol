...
contract GamelauncherTokenSale {
    ...
    function ParticipateIDO(string memory _phaseId,string memory _projectId,string memory _orderId,uint256 _numberOfTokens) public payable returns (bool) {
       ...
       uint256 scaledNumberOfTokens = SafeMath.safeMultiply(_numberOfTokens,uint256(10) ** 0);
    ...
    function ClaimToken(string memory _phaseId,string memory _projectId,uint _vestingIndex,string memory _orderId) public returns (bool) {
       ...
       if(userpurchased.totalAvailable>0) {
         ...
         uint256 _payableTokenAmount=SafeMath.safeMultiply(_totalPayableAmount,uint256(10) ** getProjectDetails[_projectId].decimals);
         if(userpurchased.totalAvailable>=_totalPayableAmount)
         {
            ...
            getProjectDetails[_projectId].tokenContract.transfer(msg.sender, _payableTokenAmount);
         }
       }
       return true;
    }

    function RefundBNB(string memory _phaseId,string memory _projectId,string memory _orderId) public returns (bool) {
