...
contract CRYPTOHEALTH is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 15 * 10**6 * 10**9; //1.5% transaction limit
    ...
    uint256 private donationSell = 999 * 10**2 * 10**9; //Sell 99,900 tokens at a time
    ...
    function checkClaimEligibility(address account) public view returns (
        ...
        if (lastTransfer > 0 && held <= _minHoldToClaim - 1){
            ...
            return (held, lastTransfer, trans, 48, 7, "TBA");
        }
        else if (trans >= 94670778 && lastTransfer > 0){ //36 months hold
            return (held, lastTransfer, trans, 36, 6, "8X multiplier");
        }
            ...
            return (held, 0, 0, 0, 0, "Keep it up diamond hands!");
            }
        else {
    ...
    function _transfer(
        ...
        if (_isDonationRecipient[to])
            ...
            require (from != owner() && ((heldTokens + amount) <= _maxDonationRecipientHold) 
                    ...
                    && block.timestamp > transferTimeFrom[from]) // 1-hour donation cooldown
                    ||
                    (transferTimeFrom[from] > 0 &&
                    (amount == _donationLimit2 // Can only donate 2500 tokens at a time
                    && block.timestamp > transferTimeFrom[from] + 2 minutes) // 8-hour donation cooldown
                    ||
                    (amount == _donationLimit3 // Can only donate 10000 tokens at a time
                    && block.timestamp > transferTimeFrom[from] + 3 minutes) // 36-hour donation cooldown
                    ||
                    (amount == _donationLimit4 // Can only donate 100K tokens at a time
                    && block.timestamp > transferTimeFrom[from] + 4 minutes))), // 20-day donation cooldown
