...
contract ObitalPresaleBUSD is ReentrancyGuard, Ownable {
    ...
    uint256 public constant TOKEN_PRICE = 2500 * 10**16;
    ...
    uint256 public constant CONTRIBUTION_MAX = 30000 ether;
    ...
    function withdraw() external nonReentrant {
        ...
        require(
            funders[_msgSender()].amount > 0 &&
                funders[_msgSender()].amount > funders[_msgSender()].claimed_amount &&
        ...
        if (status == PresaleStatuses.Finished) {
            if (funders[_msgSender()].status == FunderStatus.Invested) {
                ...
                funders[_msgSender()].claimed_amount += amount;
                funders[_msgSender()].first_claim_time = block.timestamp;
                funders[_msgSender()].status = FunderStatus.Claimed;
                ...
                require(
                    funders[_msgSender()].first_claim_time + CLAIM_PERIOD > block.timestamp,
                    "Can claim after week of first claimed"
                );
                uint256 period_claim_count = (block.timestamp - funders[_msgSender()].first_claim_time) / CLAIM_PERIOD;
