...
contract BoxNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable{
    ...
    function initByOwner(ShoeNFT _shoeNFT,address _boxShop, UtilsVRF _utilsVRF,address _boxMarketPlace, address _operator) public  onlyOwner {
        shoeNFT = _shoeNFT;
        boxShop=_boxShop;
...
function openBox(uint256 tokenId) external whenNotPaused returns (uint256 shoeId){
        ...
        uint256 shoeType = box.boxType;
        uint256 [] memory randomMulti = utilsVRF.randomMulti(minQuality[rare],maxQuality[rare],4);
  ...
  function setUtilsVRF(UtilsVRF _utilsVRF) public onlyOwner{
    utilsVRF = _utilsVRF;
  }  
...
contract BoxShop is OwnableUpgradeable, PausableUpgradeable  {
    BoxNFT public boxNFT;
    IBEP20 public bep20; //USDT
    using MathUpgradeable for uint256;
    using CountersUpgradeable for CountersUpgradeable.Counter;
    
    struct BoxSaleConfig {
        uint256 id;
        uint256 boxType;
        uint256 rare;
        uint256 price;
    ...
    mapping(uint256  => BoxSaleConfig) public boxSaleConfigs;
    mapping(uint256 => CountersUpgradeable.Counter) public totalBuys;
    uint256 public numOfConfigs;
    
    function initialize() public initializer {
        boxSaleConfigs[1].id=1;
        boxSaleConfigs[1].boxType = 1;
        boxSaleConfigs[1].rare = 1;              
        boxSaleConfigs[1].price = 200*10**18;
        boxSaleConfigs[1].isActive =  true;
        boxSaleConfigs[1].limit=10000;

        boxSaleConfigs[2].id=2;
        boxSaleConfigs[2].boxType = 2;
        boxSaleConfigs[2].rare = 1;              
        boxSaleConfigs[2].price = 250*10**18;
        boxSaleConfigs[2].isActive =  true;
        boxSaleConfigs[2].limit=10000;

        boxSaleConfigs[3].id=3;
        boxSaleConfigs[3].boxType = 3;
        boxSaleConfigs[3].rare = 1;              
        boxSaleConfigs[3].price = 300*10**18;
        boxSaleConfigs[3].isActive =  true;
        boxSaleConfigs[3].limit=10000;
        numOfConfigs=3;
        __Ownable_init();
    }
     ...
     */
    function initByOwner(BoxNFT _boxNFT, IBEP20 _bep20) public onlyOwner {
        boxNFT = _boxNFT;
        bep20=_bep20;
    }
event buyBoxEvent(uint256 boxId);
function buyBox(uint256 _id, uint256 _bep20) external  whenNotPaused returns (uint256) {
        require(_bep20 != 0,'value must be greater than 0');
        require(_bep20 == boxSaleConfigs[_id].price,'value must be equal price config');
        require(boxSaleConfigs[_id].isActive==true,'Shop not active');
        require(totalBuys[_id].current() <= boxSaleConfigs[_id].limit,'Shop not active');
        
        totalBuys[_id].increment();
        bep20.approve(address(this), _bep20);
        bep20.transferFrom(msg.sender, address(this), _bep20);
        uint256 id = boxNFT.buyBox(msg.sender,boxSaleConfigs[_id].rare,boxSaleConfigs[_id].boxType);
        emit buyBoxEvent(id);
        return id;
    }

  function getBoxSaleConfigs() external view returns (BoxSaleConfig[] memory ) {
        uint range = numOfConfigs;
        uint i=1;
        uint index=0;
        uint x=0;
        for(i; i <= range; i++){
          if(boxSaleConfigs[i].rare !=0){
            index++;
          }
        }
        BoxSaleConfig[] memory result = new BoxSaleConfig[](index);
        i=1;
        for(i; i <= range; i++){
          if(boxSaleConfigs[i].rare !=0){
            result[x] = boxSaleConfigs[i];
            x++;
          }
        }
        return result;
  ...
  function getConfig(uint256 _id )public view returns(uint256 _boxType,
  uint256 _rare, 
  uint256 _price, 
  bool _isActive, 
  uint256 _limit, 
   uint256 _totalBuys ){
      _boxType = boxSaleConfigs[_id].boxType;
      _rare = boxSaleConfigs[_id].rare;
      _price = boxSaleConfigs[_id].price;
      _isActive = boxSaleConfigs[_id].isActive;
      _limit = boxSaleConfigs[_id].limit;
      _totalBuys=totalBuys[_id].current();

  }

  function setConfig(uint256 _id,uint256 _boxType, uint256 _rare,uint256 _price, bool _isActive,uint256 _limit) public onlyOwner{
      boxSaleConfigs[_id].id = _id;
      boxSaleConfigs[_id].boxType = _boxType;
      boxSaleConfigs[_id].price = _price;
      boxSaleConfigs[_id].rare = _rare;
      boxSaleConfigs[_id].isActive = _isActive;
      boxSaleConfigs[_id].limit = _limit;
  }
...
contract UtilsVRF is OwnableUpgradeable, PausableUpgradeable  {
    uint256 randomNumber;
    uint256 privateNumber;
   ...
   using SafeMathUpgradeable for uint256;
    ...
    event randomIndexEvent(uint256 result, uint256 number);
    function randomMulti(uint256 from, uint256 to, uint32 multi) public whenNotPaused  returns (uint256[] memory) {
        require(to > from, "Not correct input");
        vRFv2Consumer.requestRandomWords();
        uint256 [] memory randomValue = vRFv2Consumer.getRandom();
        uint256[] memory result = new uint256[](multi);
       result[0] = from + ((randomValue[0]+ privateNumber)% (to-from+1));
       result[1] = from + ((randomValue[1]+ privateNumber/2)  % (to-from+1));
       result[2] = from + ((randomValue[2]+ privateNumber-5) % (to-from+1));
       result[3] = from + ((randomValue[3]+ privateNumber-7) % (to-from+1));
 ...
 function initByOwner(VRFv2Consumer _vRFv2Consumer) public  onlyOwner {
...
contract VRFv2Consumer is VRFConsumerBaseV2 {
    ...
    // mapping(uint256 => uint256[]) private _arrRandomWords;
    // mapping(uint256 => uint256[]) private _arrRandomWordsCounter;
    ...
    modifier onlyOperator() {
        require(operators[msg.sender] == true||msg.sender == s_owner);
        _;
    }
    ...
    function fulfillRandomWords(
        ...
        // uint256 i=0;
        // for(i; i < randomWords.length; i++){
        //   _arrRandomWords[requestId][i]=randomWords[i];
        // }
    ...
    //     _arrRandomWords[requestId]=randomWords;
    //     _index.increment();
    //     uint256 newIndex = _index.current();
    //     _arrRandomWordsCounter[newIndex]=randomWords;
    // }

    // function getArrRandomWords(uint256 requestId) public view  returns (uint256[] memory ) {
    //     return _arrRandomWords[requestId];
    // }

    // function getArrRandomWordsCounter(uint256 index) public view  returns (uint256[] memory ) {
    //     return _arrRandomWordsCounter[index];
    // }
}
