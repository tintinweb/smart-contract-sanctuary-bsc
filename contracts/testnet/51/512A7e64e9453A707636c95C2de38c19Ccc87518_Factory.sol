...
contract TestCall is  Ownable {
    ...
    modifier onlyFactory() {
        require(_msgSender() == factory, "Ownable: caller is not the owner");
        _;
    }
    ...
    function getTest3 (string memory test) public pure returns(bytes memory){
...
contract Factory is OwnableUpgradeable, DivestorUpgradeable {
    ...
    function initialize(uint value) public initializer {
        __Ownable_init_unchained();
    }

    uint count;
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        uint256 offset = count;
        // require(tokenA != tokenB, 'Pancake: IDENTICAL_ADDRESSES');
        // (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        // require(token0 != address(0), 'Pancake: ZERO_ADDRESS');
        // require(getPair[token0][token1] == address(0), 'Pancake: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(TestCall).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(count));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), add(offset, 0))
        }
        count++;
