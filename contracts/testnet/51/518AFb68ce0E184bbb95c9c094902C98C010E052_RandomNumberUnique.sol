...
contract RandomNumberUnique is VRFConsumerBase, ConfirmedOwner(msg.sender) {
    ...
    mapping(address => uint256) address_token_id;
    ...
    function setCommitters(address[] memory _committers) public onlyOwner {
    ...
    function generateWinner() public onlyOwner {
        ...
        for (uint256 i = 0; i < token_ids.length; i++) {
            uint256 randomNum = uint256(keccak256(abi.encode(randomResult, i)));
            uint256 winner = (randomNum % 10) % committers.length;
            uint256 winner_nft = (randomNum % 1000) % token_ids.length;
            winner_addresses.push(committers[winner]);
            address_token_id[committers[winner]] = token_ids[winner_nft];
            committers[winner] = committers[committers.length - 1];
            committers.pop();
    ...
    function getCommiters() public view returns (address[] memory) {
        return committers;
    }
    ...
    function getWinnerToken(address winner_address)
        ...
        returns (uint256)
    {
        return address_token_id[winner_address];
    }
}
