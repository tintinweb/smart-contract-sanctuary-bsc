...
contract BUSDBuffet is IBEP20, TEGAuth {
    ...
    function swapBack(address _sender, address _recipient, uint256 _amount) internal swapping {
        ...
        uint256 swapLiquidityFee = liquifyEnabled ? tax.liquidity : 0;
    ...
    function setTaxs(uint16 _transferType, uint256 _totalFee, uint256 _liquidityFee, uint256 _reflectionFee, uint256 _buybackFee, uint256 _feeDenominator) external authorizedFor(Permission.AdjustContractVariables) {
        ...
        require(_totalFee<=6000, "Total Fee shouldn't be bigger than 6000!");
    ...
    function setTradable(bool _enabled) external authorizedFor(Permission.AdjustContractVariables) {
        tradableLock = _enabled;
    }
    ...
    function initOwner() internal authorizedFor(Permission.AdjustContractVariables) {
        ...
        setIsAllExempt(DEAD, true);
        setIsAllExempt(pancakeV2BNBPair, true);
        setIsAllExempt(ZERO, true);
    ...
    function getDistributorAddress()  public view authorizedFor(Permission.AdjustContractVariables) returns (address) {
