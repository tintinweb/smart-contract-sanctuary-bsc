...
contract Test {
    ...
    constructor(address ownerAddress, IBEP20 _GWD)  
    {
        owner = ownerAddress;       
        GWD=_GWD;
        // for investment base
    ...
    function start_staking_other(address user, uint256 walletUsedBusd, uint256 walletUsedToken, address referrer, uint8 position) public 
    {
        require(stakingOn,"Staking Stopped.");
        require(position==1 || position==2,"Invalid Position.");
        if(!isUserExists(user))
	    {
	        registration(user, referrer,position);   
	    }
        ...
        uint8 _programId=getProgramId(users[user].totalStakingBusd+walletUsedBusd);
