...
contract UnlimitedStaking is IStaking, Ownable, ReentrancyGuard, Pausable {
    ...
    function recoverNonLockedRewardTokens() external override onlyOwner {
        uint256 nonLockedTokens = rewardsToken.balanceOf(address(this)).sub(
            rewardTokensLocked
        );
        ...
        updateReward(_msgSender())
    {
        ...
        usersInfo[_msgSender()].staked = usersInfo[_msgSender()].staked.add(
            _amount
        );
    ...
    function earned(address _account) public view override returns (uint256) {
        uint256 rewardsDifference = rewardPerToken().sub(
            usersInfo[_account].rewardPerTokenPaid
        );
        uint256 newlyAccumulated = usersInfo[_account]
            .staked
    ...
    function _getFutureRewardTokens() internal view returns (uint256) {
        uint256 blocksLeft = _calculateBlocksLeft(
    ...
    function _updateCooldown(uint256 _amount) internal {
        if (cooldownPeriod > 0) {
            uint256 timePassed = block.timestamp.sub(
                usersInfo[_msgSender()].lastUpdate
            );
            ...
            if (timePassed >= cooldownPeriod) {
                ...
                usersInfo[_msgSender()].locked = locked.add(_amount).sub(
                    locked.mul(share).div(PRECISION)
                );
            }
