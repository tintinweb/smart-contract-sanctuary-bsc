...
contract CyclingBoxEvent is ReentrancyGuard, Ownable {
    ...
    function buyBox(
        ...
        if (roundIsPrivate[_roundId]) {
            if (!_verify(_leaf(buyer,_userTotalAmount), proof)) {
                if (!backupWhiteList[_roundId][buyer]) {
                    revert("Buyer is not in the white list");
                }
            }
        }

        if (roundLimitBox[_roundId] > 0) {
            require(
        ...
        require(msg.value >= payWei, "Insufficient token");
    ...
    function _leaf(address account,uint8 _userTotalAmount) private pure returns (bytes32) {
        return keccak256(abi.encodePacked(account, _userTotalAmount));
    }
}
