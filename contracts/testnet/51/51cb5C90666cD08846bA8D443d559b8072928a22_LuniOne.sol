...
interface ILuniStake {
    function getUserStakes(address user) external view returns (uint256, uint256[] memory);
}

interface IFeeRecipient {
    function forwardTokens(address sender, address tokenAdd, uint256 amount) external returns(bool);
}

contract LuniOne is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, VRFConsumerBaseV2{
    ...
    ILuniStake public LuniStake;


    // Minters
    mapping(uint256 => address) public minters;

    address public v1LuniOne;
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomValues) internal override {
    ...
    function setMintWithBNBAllowed(bool state) external onlyOwner{
        mintWithBnbAllowed = state;
    }

    function _transfer(address from,address to, uint256 tokenId) internal override{
        require(balanceOf(to) + 1 + numberOfStakes(to) <= MAX_NFT_PER_USER, "You are exceeding MAX_NFT_PER_USER");
        super._transfer(from,to,tokenId);
    }

    function reservedTransferFrom(address from,address to, uint256 tokenId) external{
        require(msg.sender == address(LuniStake), "Only LuniStake can use reservedTransfer");
        require(ownerOf(tokenId) == from, "Only owner can transfer");
        super._transfer(from,to,tokenId);
    }

    function numberOfStakes(address user) internal view returns (uint256){
        (uint256 count, ) = LuniStake.getUserStakes(user);
        return count;
    }
    ...
    function mintWithBNB(uint256 numberOfNfts) external payable nonReentrant{
        ...
        require(balanceOf(msg.sender) + numberOfNfts + numberOfStakes(msg.sender) <= MAX_NFT_PER_USER, "You are exceeding MAX_NFT_PER_USER");
        ...
        payable(feeRecipient).sendValue(price);
    ...
    function mintWithLUNI(uint256 numberOfNfts) external nonReentrant{
        ...
        freeNft[msg.sender] -= numberOfNfts - _calculateNftToBePayed(numberOfNfts);
        if(price > 0) {
            require(IFeeRecipient(feeRecipient).forwardTokens(msg.sender, LUNI, price), "Error during forwarding");
        }
        requestRandomValues(uint32(numberOfNfts), msg.sender);
    }
    ...
    function bulkMint(uint256[] memory _ids, address[] memory recipients) external onlyOwner{
        require(_ids.length == recipients.length, "Length of ids and recipients must be equal");
        uint256 size = _ids.length;
        for(uint256 i; i < size; i++){
            require(minters[_ids[i]] == address(0), "ID already minted");
            uint256 tokenId = _popPendingAtIndex(_ids[i]);
            _totalSupply += 1;
            minters[tokenId] = ILuniOne(v1LuniOne).minters(tokenId);
    ...
    function setV1LuniOne(address _v1LuniOne) external onlyOwner{
        v1LuniOne = _v1LuniOne;
    }

    function setLuniStake(address _luniStake) external onlyOwner{
        LuniStake = ILuniStake(_luniStake);
    }
    ...
    function setFeeRecipient(address newAddress) external {
        require(msg.sender == admin, "Only admin can change fee recipient");
