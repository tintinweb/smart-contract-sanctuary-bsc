...
contract Mission99 is Ownable{
    ...
    mapping(uint256 => uint256[]) poolIncomeSlab;
    mapping(uint256 => uint256[]) withWallet;
    mapping(uint256 => uint256[]) topupWallet;
    mapping(uint256 => address[]) poolParentAvailable;
    mapping(uint256 => uint256) poolParentNo;
    mapping(uint256 => uint256) poolDirect;
    ...
    mapping(uint256 => mapping(address => PoolInfo)) poolInfo;
    ...
    function _autoPoolEntry(uint256 _poolNo, address _user) private{
        ...
        if(poolParentAvailable[_poolNo].length>0){
            ...
            poolParentAvailable[_poolNo].push(_user);
            poolInfo[_poolNo][_user].entryNo = 1;//poolParentAvailable[_poolNo].length.add(1);  
        }
