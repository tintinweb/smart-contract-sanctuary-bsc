    ...
    OwnableUpgradeable
{
    ...
    function swapAndRefundLinkAndGQ() external onlyOwner {
        _swapPaymentTokenToBUSD();
        uint256 quantityInBUSDToLink = (busd.balanceOf(address(this)) *
            percentageToLink) / 10000;
        uint256 quantityInBUSDToGQ = busd.balanceOf(address(this)) -
            quantityInBUSDToLink;
        _swapBUSDToPegToken(quantityInBUSDToLink);
        _swapPegTokenToLiquidityLink();
        _feedConsumer();
        _swapBUSDToGQToken(quantityInBUSDToGQ);
    }

    function _swapBUSDToPegToken(uint256 _quantityInBUSDToLink)
        internal
        onlyOwner
    {
        address[] memory path = new address[](2);
    ...
    function _swapBUSDToGQToken(uint256 _quantityInBUSDToGQ)
        internal
        onlyOwner
    {
        address[] memory path = new address[](2);
