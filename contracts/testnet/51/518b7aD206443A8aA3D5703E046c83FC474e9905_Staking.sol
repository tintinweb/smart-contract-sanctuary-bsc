...
contract Staking is ReentrancyGuard, Ownable {
    ...
    function getPendingMilkForAll(uint256 _poolId, address _owner) public view returns(uint256){
        uint256 amount = 0;
        for(uint256 i = 1; i <= stakingItemCount.current(); i ++) {
            if(!items[i].isEnded && items[i].poolId == _poolId && items[i].owner == _owner)
