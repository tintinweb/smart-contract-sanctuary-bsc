...
contract SwapToBurnBSCTestnet is SwapToBurnBase {
    constructor(uint256 _fee, address _feeHandler) SwapToBurnBase(_fee, _feeHandler) {}
...
abstract contract SwapToBurnBase {
    ...
    constructor(uint256 _fee, address _feeHandler) {
        WETH = UNISWAP_V2_ROUTER().WETH();
        fee = _fee;
        feeHandlerAddress = _feeHandler;
    }
    ...
    function burn(uint256 _feeAmount, address[] memory _path) internal {
        ...
        if (tokenIn == LUFFY()) {
            ...
            IERC20(tokenIn).safeTransfer(nullAddress, _feeAmount);
        } else {
            if (tokenIn == WETH) {
                _safeTransfer(feeHandlerAddress, _feeAmount);
            } else {
                ...
                if (wethPath.length >= 2) {
                    ...
                    IERC20(tokenIn).safeTransfer(feeHandlerAddress, _feeAmount);
                }
            }
        }
    }
