...
contract FleaNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 public commonRate = 1;
    uint256 public rareRate = 2500e18;
    uint256 public legendaryRate = 2500e18;
    ...
    function userMint(uint256 qty, uint256 amount) external {
        // This is where the user mints the NFT
        // Note: The currency we will accept is the BUSD Token

        require(qty > 0, "Qty must be greater than 0.");
        require(
            amount * qty >= commonRate * qty,
            "Not enough amount to purchase NFTs."
        );

        ERC20TokenToBeAccepted.transfer(address(this), amount);
