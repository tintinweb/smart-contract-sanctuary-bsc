...
contract Campaign is Whitelist {    
    ...
    function initialize(
        ...
        softCap = _settings[2];
        hardCap = _settings[3];
        mintType = _stringSettings[1];
        mintToken = _addresses[1];
        campaignId = _stringSettings[0];
        
        for (uint256 i = 0; i < _nftTiers.length; i++) {
            ERC721JumpStart nft = new ERC721JumpStart(_nftTiers[i].tokenName,
                _nftTiers[i].tokenSymbol);         
                   
            nft.initialize([_nftTiers[i].tokenName,
                _nftTiers[i].tokenSymbol,
    ...
    function getNfts() public view returns (address[] memory a) {
        return campaignNfts;
    }
}
...
contract ERC721JumpStart is ERC721A, Whitelist {
    ...
    function initialize(
        ...
        // [3] = mintType, [4] = nftUID
        string[5] memory _stringSettings,
        // [0] = quantity, [1] = mintPrice, [2] = priceDiscount,
        //[3] = priceDiscountEndDate, [4] = mintPerAddress,
        //[5] = mintPerTransaction, [6] = ticketsPerMint, [7] = campaignFee
        uint256[8] memory _intSettings,
        address _mintToken,
