...
contract TokenDistributor is Ownable, Pausable, ReentrancyGuard {
  ...
  modifier onlyAdmin() {
    require(metaspacecyAccessControls.hasAdminRole(_msgSender()), "TD: only admin");
    _;
  }
  ...
  function releaseToken(uint256 ticketSupply, uint256 expiry) public onlyAdmin whenNotPaused {
    ...
    uint256 totalTokens = metaspacecyToken.balanceOf(address(this));
    for (uint256 i = 0; i < _parties.length(); i++) {
  ...
  function fullFillReserveAndTreasury() public onlyAdmin whenPaused {
    ...
    uint256 missingInReserve = IShares(reserve).maxBalanceMST() - IShares(reserve).currentBalanceMST();
    if (missingInReserve > 0) {
      _distributeTokens(reserve, missingInReserve);
    }

    uint256 missingInTreasury = IShares(treasury).maxBalanceMST() - IShares(treasury).currentBalanceMST();
  ...
  function _updateTokenDistributor() private {
    uint256 nonCirculatingSupply = metaspacecyToken.maxTotalSupply() - metaspacecyToken.totalSupply();
    metaspacecyToken.mint(address(this), nonCirculatingSupply);
  }
  ...
  function _updateTicketCard(
    ...
    metaspacecyTicketCard.create(_initialOwner, _initialSupply, _expiry, _data);
  }

  function _distributeTokens(address _party, uint256 _amount) internal nonReentrant {
    require(_amount > 0, "TD: zero amount");
    metaspacecyToken.safeTransfer(_party, _amount);
