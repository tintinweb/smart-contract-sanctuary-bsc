...
contract KING is ERC20Interface, Owned {
    ...
    address public lionPool = address(0x96eD0b21d024b82A430386A3A1477324f25f0143);
    address public supe_token = address(0x0);

    address public WETH = address(0x1e33833a035069f42d68d1f53b341643de1c018d);
    address public LION = address(0xc9178b746228fd1d03fe712d7f516ee2b3e74a66);
    ...
    // Constructor
    // ------------------------------------------------------------------------
    constructor(address _LION, string _symbol, string _name) public {
        LION = _LION;
        symbol = _symbol;
        name = _name;
        decimals = 18;
    ...
    function initialize(address _router, address _devPool, address _gaur_dao, address _treasury) external onlyOwner{
        router = IUniswapV2Router02(_router);
        swapFactory = IUniswapV2Factory(router.factory());
        devPool = _devPool;
        gaur_dao = _gaur_dao;
        treasury = _treasury;
        emit Initialize(_router, address(0x0), swapFactory);
    }
    function initialize2(address _LION, string _symbol, string _name) external onlyOwner{
        LION = _LION;
        symbol = _symbol;
        name = _name;
    }
    ...
    function _swapAndLiquify(uint256 tokens) private lockTheSwap {
        if(tokens == 0) return;
        address pair = swapFactory.getPair(address(this), LION);
        if(extra_enable && IERC20(address(this)).balanceOf(pair).div(10) < tokens){
        ...
        uint256 initialBalance = IERC20(address(LION)).balanceOf(lionPool);//address(this).balance;

        // swap tokens for ETH
        if(_switch0) _swapTokensForLION(half, lionPool); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = IERC20(address(LION)).balanceOf(lionPool).sub(initialBalance);

        if(newBalance > 0){
            IERC20(address(LION)).transferFrom(lionPool, address(this), newBalance);
        }
    ...
    function _swapTokensForLION(uint256 tokenAmount, address to) internal {
        IERC20(address(this)).approve(address(router), 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
