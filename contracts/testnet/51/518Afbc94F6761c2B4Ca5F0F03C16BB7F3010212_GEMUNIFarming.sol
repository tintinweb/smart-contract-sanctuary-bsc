...
contract GEMUNIFarming is PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, IGEMUNIFarming {
    ...
    function setTokenLock(address _tokenLock) public onlyOwner {
        require(_tokenLock != address(0), "Farm: invalid address");
    ...
    function claimReward(uint256 _pid) external override validPool(_pid) {
        PoolInfoToken storage pool = poolInfoTokens[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        IGEMUNIVesting(tokenLock).lock(_pid, address(pool.rewardToken), msg.sender, currentReward, vestingTime[_pid]);
    ...
    function unlockReward(uint256 _pid, uint[] memory indexes) external override validPool(_pid) {
        for(uint i = 0; i < indexes.length; i++) {
            IGEMUNIVesting(tokenLock).unlock(_pid, msg.sender, i);
        }
    }
...
interface IGEMUNIVesting {
    function unlock(uint pid, address _addr, uint256 index) external;
    ...
    function getLockInfo(uint pid, address _user)
        external
        view
        returns (
    ...
    function getLockInfoByIndexes(uint pid, address _addr, uint256[] memory _indexes)
        external
        view
        returns (
            bool[] memory isWithdrawns,
    ...
    function getLockInfoLength(uint pid, address _addr)
        external
        view
        returns (uint256);
}
...
interface IGEMUNIFarming {
    ...
    function unlockReward(uint256 _pid, uint[] memory indexes) external;
}
