...
contract ELPToken is ERC20Upgradeable, OwnableUpgradeable, KeeperCompatibleInterface, ChainlinkClientUpgradable {
    ...
    function buyToken(uint256 _elpAmt) public {
        ...
        emit BuyToken(msg.sender, _currentNav, _elpAmt, block.timestamp);
    ...
    function sellToken(uint256 _elpAmt) public {
        ...
        IERC20Upgradeable(efunToken).safeTransferFrom(
            address(poolAddress),
            msg.sender,
            (_efunAmt * (10000 - sellFee)) / 10000
        );
        IERC20Upgradeable(efunToken).safeTransferFrom(address(poolAddress), feeCollector, (_efunAmt * sellFee) / 10000);
        emit SellToken(msg.sender, _currentNav, _elpAmt, sellFee, block.timestamp);
    ...
    event BuyToken(address user, uint256 nav, uint256 amount, uint256 timestamp);

    event SellToken(address user, uint256 nav, uint256 amount, uint256 fee, uint256 timestamp);
}
