...
import { LucksLogic } from "./libraries/LucksLogic.sol";
...
contract LucksExecutor is ILucksExecutor, ReentrancyGuardUpgradeable, OwnableUpgradeable {    
    ...
    function reCreateTask(uint256 taskId, TaskItem memory item, TaskExt memory ext) external payable override nonReentrant {
        ...
        LucksLogic.checkReCreateTask(taskId, tasks, userState, item, ext);       
    ...
    */
    ...
    {                
        ...
        Ticket memory ticket = LucksLogic.findWinnerTicket(tickets, ticketIds, taskId, finalNo);    
...
library LucksLogic {
    ...
    function checkReCreateTask(       
        ...
        require(task.seller == msg.sender, "Invalid auth"); // only seller
        ...
        require(canReCreate, "Invalid state");
      
        // checking inputs
        require(task.nftChainId == item.nftChainId, "Invalid nftChainId"); // action must start from NFTChain   
        require(task.nftContract == item.nftContract, "Invalid nftContract");
        require(keccak256(abi.encodePacked(task.tokenIds)) == keccak256(abi.encodePacked(item.tokenIds)), "Invalid tokenIds");
        require(keccak256(abi.encodePacked(task.tokenAmounts)) == keccak256(abi.encodePacked(item.tokenAmounts)), "Invalid tokenAmounts");
        require(task.seller == item.seller, "Invalid seller");
        require(task.depositId == item.depositId, "Invalid seller");
