...
contract ProjectToken is ERC20, Ownable {
    ...
    *
    * inputs:
    *   _amount - number of project tokens you want to return in exchange of BUSD
    *
    ...
    */
    function buyBack(uint256 _amount) external {
        ...
        require(balanceOf(msg.sender) >= _amount, "buyBack: You don't own enough project tokens!");

        bool transferSuccess = transfer(projectWallet, _amount);
        require(transferSuccess, "buyBack: Failed to send back token!");

        bool sendBUSDSuccess = busdAddress.transferFrom(projectWallet, msg.sender, (_amount/totalSupply() * projectEarned));
