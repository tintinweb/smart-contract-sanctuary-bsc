...
// File: RunTogetherPoolProposal.sol
...
contract RunTogetherPoolProposal is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount, address _user)
        ...
        isValidBlock
    {   
        require(_user == msg.sender || callerMap[msg.sender], "Invalid user");
        ...
        returns (uint256 _totalGain)
    {
        ...
        if (block.number - user.stakeBlock > minBlock) {
            ...
            if (isCompound) {
                _totalGain = compound(userAmount, rateReward, totalBlock).sub(userAmount);
            ...
            ABDKMath64x64.mulu(
                pow(
                    ABDKMath64x64.add(
                        ...
                        ABDKMath64x64.divu(ratio, blockPerYear * baseRate)
                    ),
                    totalBlock
                ),
                principal
    ...
    function setCompound(bool _status) external onlyOwner {
        isCompound = _status;
    }
    ...
    function setCallerMap(address[] calldata _callerMap) external onlyOwner {
        for (uint256 index = 0; index < _callerMap.length; index++) {
            callerMap[_callerMap[index]] = true;
        }
    }

    function removeCallerMap(address[] calldata _callerMap) external onlyOwner {
        for (uint256 index = 0; index < _callerMap.length; index++) {
            callerMap[_callerMap[index]] = false;
        }
    }
