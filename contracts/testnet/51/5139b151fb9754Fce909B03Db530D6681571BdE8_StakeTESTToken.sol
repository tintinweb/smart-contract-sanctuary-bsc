...
contract StakeTESTToken is Ownable {
    ...
    constructor(address _tokenAddr, uint128 _maxStakable) {
        ...
        token = IBEP20(_tokenAddr);//for staking plus referral reward
        maxTokensStakable = _maxStakable;
        rewardLifetime = 3 days;//12 minutes;//365 days;// 1 year
        nextRewardLifetime = 6 days; //730 days;// 2 years
        currentDuration = [90 days,180 days, 365 days];//[3 minutes, 6 minutes, 12 minutes];//[3 days,6 days,12 days];//[90 days,180 days, 365 days]
        currentAPR = [800,2000,4500]; //8%,20%,45%
        nextAPR = [350,800,1800];//3.5%,8%,18%
        referralTESTRewardFees = [250,500,750];//2.5%,5%,7.5%
        skipInitialClaimDuration = 360 minutes;//1 minutes;//30 days
    ...
    function stake(address referrer, uint128 _amount, uint32 duration) external returns(uint32 stakedCounter) {//duration in seconds
        ...
        require(_amount >=50 * 10**18 && _amount <= 10000 * 10**18,"Invalid Amount Entered");
        require(duration == currentDuration[0] || duration == currentDuration[1] || duration == currentDuration[2], "Invalid Duration Entered");//3 months,6 months,12 months//3 minutes//6 minutes//12 minutes
        ...
        if (block.timestamp < rewardStartTime + rewardLifetime){
            ...
            if (duration == currentDuration[0] && _amount >=50 * 10**18 && _amount <=500 * 10**18){//90 days = 6 minutes // 1 month = 2 minutes
                currApr = currentAPR[0];
                flag = 0;//Zero
            } else if (duration == currentDuration[1] && _amount >=100 * 10**18 && _amount <=5000 * 10**18){//180 days = 12 minutes
                currApr = currentAPR[1];
                flag = 1;
            } else if (duration == currentDuration[2] && _amount >=500 * 10**18 && _amount <=10000 * 10**18){//365 days = 24 minutes
        ...
        // referral rewards given only for first rewardTime period - 12 minutes/12 months
        if (block.timestamp < rewardStartTime + rewardLifetime){

            if (flag == 0){
                testReward = (_amount * referralTESTRewardFees[0])/10000;
            } else if (flag == 1){
                testReward = (_amount * referralTESTRewardFees[1])/10000;
            } else if (flag == 2){
                testReward = (_amount * referralTESTRewardFees[2])/10000;
            }

            require(
            fixedRewardsAvailable >= testReward,
            "Insufficient Fixed Rewards available"
            );

            token.safeTransfer(referrer, testReward);
            fixedRewardsAvailable -= testReward;
        }
