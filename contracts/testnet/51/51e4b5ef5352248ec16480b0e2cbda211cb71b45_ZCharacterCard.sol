...
contract ZCharacterCard is ERC721Upgradeable, OwnableUpgradeable {
    ...
    event createNFTForCard(uint256 cardId, uint256 tokenId);
    function initialize(string memory name_, string memory symbol_) public initializer {
        __ERC721_init(name_, symbol_);
        __Ownable_init();
        domain = "https://stg-api-herozst.zstudio.io/character-card/";
    }
    ...
    function mint(uint256 cardId) external returns (uint256){
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _mint(_msgSender(), newTokenId);
        emit createNFTForCard(cardId, newTokenId);
        return newTokenId;
    }
