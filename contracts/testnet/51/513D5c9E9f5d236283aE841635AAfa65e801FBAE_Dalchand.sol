...
contract Dalchand {
    ...
    function registration(address payable refAddress)public payable{
       ...
       function regUser(address payable userAddress) internal{
        ...
        users[userAddress].activeLevelPlanB=1;
        userActiveId[userAddress]=lastIDCount[_roundid];
               
        emit buyLevelEvent(userAddress, 1, now,LEVEL_PRICE[1],_roundid);
        
        emit binaryData(userAddress,lastIDCount[_roundid],lastFreeParent[_roundid],1,userAddressByID[_roundid][lastFreeParent[_roundid]],_roundid);
    ...
    function paymentForUp1(uint up,uint _level,uint _roundid) internal
    {
        ...
        {
            emit Hold(receiver,_level,LEVEL_PRICE[_level]);
            //emit hold
        }
    }
