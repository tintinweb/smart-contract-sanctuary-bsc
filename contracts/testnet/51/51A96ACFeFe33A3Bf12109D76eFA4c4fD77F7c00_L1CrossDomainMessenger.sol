...
import { IUpgradeAuthorizer } from "./interfaces/IUpgradeAuthorizer.sol";
    ...
    Lib_AddressResolverUpgradeable
{
    ...
    IUpgradeAuthorizer public upgradeAuthorizer;
    ...
    function initialize(address _libAddressManager) public initializer {
        ...
        __AddressResolver_init_unchained(_libAddressManager);
    }

    function setUpgradeAuthorizer(IUpgradeAuthorizer _upgradeAuthorizer) external onlyOwner {
        upgradeAuthorizer = _upgradeAuthorizer;
    }
    ...
    override
    {
        require(address(upgradeAuthorizer) != address(0), "L1CrossDomainMessenger: No authorizer set");
        upgradeAuthorizer.revertIfNotPermitted(newImplementation);
    }
}
...
abstract contract Lib_AddressResolverUpgradeable is Initializable {
    ...
    function __AddressResolver_init(address _libAddressManager) internal onlyInitializing {
        __AddressResolver_init_unchained(_libAddressManager);
    }

    function __AddressResolver_init_unchained(address _libAddressManager) internal onlyInitializing {
...
interface IUpgradeAuthorizer {
    function revertIfNotPermitted(address newImplementation) external;
}
