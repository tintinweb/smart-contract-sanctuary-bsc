...
contract F1RACE {
	...
	constructor(address payable wallet, address payable _developer , address payable _ref, address payable _adminWallet) public {
    ...
    function launchproject( uint gasFee) external {
        require(msg.sender==adminWallet || msg.sender == refWallet,'Permission denied');
        if (gasFee > 0) {
