...
contract AmsToUpSwap is Ownable {
    ...
    address public sellupradmin;//接收售卖up时10%的usdt，回流给管理员，由管理员统一操作
    ...
    function init(address _usdtJia, address _UP, address _amsswap, address _sellupradmin,
        uint256[] memory _upfan, uint256 _buyamsr, uint256 _huiliuams) public onlyOwner() {
        usdtJia = _usdtJia;
        UP = _UP;
        amsswap = _amsswap;
        sellupradmin = _sellupradmin;
    ...
    function sellUp(uint256 _amount) external {
        ...
        //取10假u，取10%兑换真u给管理员，由管理员手动回购ams
        uint256 _xhu = (usdtnum * buyamsr / 100);
        INewAms(amsswap).upSellUsdtAndAms(_xhu, 1, sellupradmin);
