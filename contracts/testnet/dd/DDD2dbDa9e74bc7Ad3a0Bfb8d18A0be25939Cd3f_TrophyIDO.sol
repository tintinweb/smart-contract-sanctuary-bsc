...
contract TrophyIDO is Ownable, ReentrancyGuard {
    ...
    function setTime(uint256 _startTime, uint256 _endTime, uint256 _vestingTime) external onlyOwner {
        startTime = _startTime;
        endTime = _endTime;
        vestingTime = _vestingTime;
    }
    ...
    function purchase() external payable occurring validAmount(msg.value) underHardCap {
    ...
    function calcTotalTokensRequired() public view returns (uint256) {
        return
            hardCap.mul(1 ether).div(price).add(
                hardCap.mul(lpPercent).div(RATE_PRECISION_FACTOR).mul(1 ether).div(listingPrice)
            );
    }
    ...
    function calcCurrentTokensRequired() public view returns (uint256) {
        return
            currentCap.mul(1 ether).div(price).add(
                currentCap.mul(lpPercent).div(RATE_PRECISION_FACTOR).mul(1 ether).div(listingPrice)
            );
    }

    function calcPurchasedTokenAmount(address purchaser) public view returns (uint256) {
        return purchaseDetails[purchaser].amount.mul(1 ether).div(price);
    }
    ...
    function calcClaimableTokenAmount(address purchaser) public view returns (uint256) {
        ...
        if (purchaseDetail.isClaimed || status != SaleStatus.FINALIZED) {
            return 0;
        }
        return purchaseDetail.amount.mul(1 ether).div(price);
    }
    ...
    function finalize(address _to) external onlyOwner {
        ...
        // for liquidity
        uint256 ethLiqAmount = currentCap.mul(lpPercent).div(RATE_PRECISION_FACTOR);
        uint256 tokenLiqAmount = ethLiqAmount.mul(1 ether).div(listingPrice);

        token.approve(address(ROUTER), tokenLiqAmount);

        ROUTER.addLiquidityETH{value: ethLiqAmount}(address(token), tokenLiqAmount, 0, 0, _to, block.timestamp);
    ...
    function refund() external whenCanceled nonReentrant {
        PurchaseDetail memory purchaseDetail = purchaseDetails[msg.sender];
        require(purchaseDetail.amount > 0 && !purchaseDetail.isClaimed, "INVALID_ACTION");
    ...
    function collectEthDust(address _to) external onlyOwner {
        uint256 ethDust = payable(address(this)).balance;
        payable(_to).transfer(ethDust);
    }

    function collectTokenDust(address _token, address _to) external onlyOwner {
