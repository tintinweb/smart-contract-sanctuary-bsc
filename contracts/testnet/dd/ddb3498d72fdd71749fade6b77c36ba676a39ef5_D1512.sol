...
contract D1512 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    D1512DividendTracker public dividendTracker;
    ...
    constructor() ERC20("D1512", "$D15") {
        ...
        dividendTracker = new D1512DividendTracker(
            payable(this),
            router,
...
contract D1512DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        D1512Contract = D1512(mainContract);
    ...
    function _transfer(
        ...
        require(false, "D1512_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "D1512_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main D1512 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(D1512Contract);

            uint256 prevBalance = D1512Contract.balanceOf(address(this));
            ...
            {
                uint256 received = D1512Contract.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    D1512Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
