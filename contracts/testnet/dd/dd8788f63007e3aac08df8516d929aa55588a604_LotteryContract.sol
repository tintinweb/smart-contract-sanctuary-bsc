...
contract LotteryContract is VRFConsumerBase, Ownable {
    ...
    function declareWinner(uint256 _lotteryId, uint256 _participants, uint256 _noOfWinner)
        external
        onlyOwner
    {
        require(
        ...
        uint256 total = 0;
        if (totalParticipants <= lottery.noOfWinner) {
            lottery.noOfWinner = totalParticipants;
            for (uint256 i = 0; i < lottery.noOfWinner; i++) {
                lottery.winners.push(lottery.participants[i]);
                checkWinner[_lotteryId][lottery.participants[i]] = true;
            }
            lottery.isFinished = true;
            lottery.noOfWinnerDeclared = totalParticipants;
        } else {
            uint256 newLength = totalParticipants - lottery.noOfWinnerDeclared;
            if ((lottery.noOfWinner - lottery.noOfWinnerDeclared) < _noOfWinner) {
                _noOfWinner = lottery.noOfWinner - lottery.noOfWinnerDeclared;
            }
            for (uint256 i = 0; i < _participants; i++) {
                total += lottery.tickets[i];
            }

            for (uint256 k = 0; k < _noOfWinner; k++) {
                ...
                for (uint256 i = 0; i < _participants; i++) {
                    ...
                    if (sum >= winnerIndex) {
                        ...
                        checkWinner[_lotteryId][lottery.participants[i]] = true;
                        total -= lottery.tickets[i];
                        lottery.participants[i] = lottery.participants[newLength - 1];
                        lottery.tickets[i] = lottery.tickets[newLength - 1];
                        newLength--;
                        _participants--;
                        break;
                    }
            ...
            lottery.noOfWinnerDeclared = lottery.noOfWinnerDeclared + _noOfWinner;
            if (lottery.noOfWinner == lottery.noOfWinnerDeclared) {
                lottery.isFinished = true;
            }
        }
    }
