...
contract Market is Ownable, ReentrancyGuard {
    ...
    uint256 public EPOCH = 1652659200;
    uint256 public SELL_RATE_BP = 500;
    IBurnableERC20 public caviar;
    IERC20 public busd;

    constructor(IBurnableERC20 _caviar, IERC20 _busd){
        caviar = _caviar;
        busd = _busd;
    }
    ...
    function currentPrice() public view returns (uint){
        ...
        return calculateValue(history, 1 ether);
    }
    ...
    function _update(uint numDays) private {
        if(numDays > 0){
            ...
            for (uint256 i = 0; i < numDays; i++) {
                simulateDay(history);
                priceHistories.push(history); //TODO CHECK IF COPY INTO STORAGE?
            }
    ...
    function stampHistory() private view returns (PriceHistory memory){
        ...
        totalSupply : caviar.totalSupply(),
    ...
    function simulateDay(PriceHistory memory history) private view {
        if (history.totalStaked > 0) {
            ...
            history.totalSupply -= soldAmount;
