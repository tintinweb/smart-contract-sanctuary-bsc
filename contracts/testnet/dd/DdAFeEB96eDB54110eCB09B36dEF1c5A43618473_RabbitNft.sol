...
contract RabbitNft is Ownable {
    ...
    mapping(uint256 => NftConfig) private NftDetail;
    mapping (uint256 => uint256) private nftNum;

    event BuyNft(address indexed _user,address indexed _currency,uint256 _id, uint256 _num, uint256 _value);

    function buyNft(uint256 id,uint256 num)  external {
        require(id>0&&num>0,"NFT: id num error !");
        NftConfig storage detail =NftDetail[id];
        require(detail.status==0&&detail.id==id&&detail.amount>0,"NFT: Id not open !");
        require(nftNum[id]+num<=detail.num,"NFT: num error !");
        uint256 totalValue=detail.amount.mul(num);
        IERC20(detail.currency).safeTransferFrom(msg.sender,acceptAddress,totalValue);
        nftNum[id]=nftNum[id]+num;
        emit BuyNft(msg.sender,detail.currency,id,num,totalValue);
    }

    function setNft(uint256 id,uint256 value,address currency,uint256 num,uint256 status) external onlyOwner {
        NftDetail[id]=NftConfig(id,value,currency,num,status);
    }
    ...
    function claim(address currency,uint256 _amount) public { 
        ...
        uint256 fee=0;
        fee =SafeMath.div(SafeMath.mul(_amount,config.fee),1000);
