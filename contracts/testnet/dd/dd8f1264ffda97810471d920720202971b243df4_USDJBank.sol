...
abstract contract USDJView is USDJBankStorage {
    ...
    function getMaxWithdrawAmount(address collateral, address user) external view returns (uint256 maxAmount) {
        DataTypes.UserInfo storage userInfo = userInfo[user];
        uint256 USDJBorrow = userInfo.t0BorrowBalance.decimalMul(getTRate());
...
contract USDJBank is IUSDJBank, USDJOperation, USDJView, USDJMulticall {
        ...
        operatorAccount(from)
    {
        ...
        require(_isAccountSafe(user, getTRate()), USDJErrors.AFTER_BORROW_ACCOUNT_IS_NOT_SAFE);
    }
        ...
        nonFlashLoanReentrant
    {
        ...
        require(_isAccountSafe(user, getTRate()), USDJErrors.AFTER_FLASHLOAN_ACCOUNT_IS_NOT_SAFE);
        ...
        internal
    {
        ...
        if (isDepositToJOJO) {
            IERC20(USDJ).approve(address(JOJODealer), tAmount);
