...
/**
 _ __  _ ___ _ __  _ _ _______   __
| |  \| | __| |  \| | |_   _\ `v' /
| | | ' | _|| | | ' | | | |  `. .' 
|_|_|\__|_| |_|_|\__|_| |_|   !_!  

This is the first Web 3.0 Token to utilize smart contracts to execute
options trading on the Binance Smart Chain. Our project will revolutionize
Web 3.0 by allowing holders to connect their wallet to perform options
trading with the Infinity Token.

Website: https://www.infinitybsc.io/                                                                                      
Twitter: https://twitter.com/1InfinityTrades                                                                                     
Telegram: https://t.me/InfinityTokenBSC 
*/
...
library Address{
 ...
 contract Infinity is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000 * 10**8 * (10 ** _decimals); // 100.000.000
    ...
    uint256 private buytotaltaxes;
    uint256 private selltotaltaxes;
    //nais!
    address public marketingdevelopmentWallet = 0x388BC733dff6AAe25A4Ff247173910737b77Ccbc;
    ...
    Taxes public taxes = Taxes(0, 6, 2);
    Taxes public sellTaxes = Taxes(0, 6, 2);
    ...
    function setTaxes(uint256 _reflection, uint256 _marketing, uint256 _liquidity) public onlyOwner {
        buytotaltaxes = _reflection + _marketing + _liquidity;
        require(buytotaltaxes <= 25, "Must keep taxes at 25% or less");
        taxes = Taxes(_reflection,_marketing,_liquidity);
        emit FeesChanged();
    }

    function setSellTaxes(uint256 _reflection, uint256 _marketing, uint256 _liquidity) public onlyOwner {
        selltotaltaxes = _reflection + _marketing + _liquidity;
        require(selltotaltaxes <= 25, "Must keep taxes at 25% or less");
        sellTaxes = Taxes(_reflection,_marketing,_liquidity);
        emit FeesChanged();
    }

    function _reflectRfi(uint256 rReflection, uint256 tReflection) private {
        _rTotal -=rReflection;
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tReflection-s.tMarketing-s.tLiquidity;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rReflection-rMarketing-rLiquidity;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rReflection > 0 || s.tReflection > 0) _reflectRfi(s.rReflection, s.tReflection);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(marketingdevelopmentWallet).sendValue(marketingAmt);
        }
    }
    ...
    function updateMarketingWallet(address newWallet) external onlyOwner{
        marketingdevelopmentWallet = newWallet;
    }
