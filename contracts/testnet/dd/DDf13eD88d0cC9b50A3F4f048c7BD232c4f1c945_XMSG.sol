...
contract XMSG is Context, IERC20, Ownable {
    ...
    event CreditBuy(address buyer,uint256 usd,uint256 amount);
    ...
    function buyCredits(uint256 _amountIn, uint256 usd, bool withToken) external payable{ // 
        ...
        if(withToken){
            ...
            amount = uint256(bnbPrice).mul(bnbTotalOut);
            ...
            path[1] = address(this);
            int bnbPrice = useManualPrice?latestBnbPrice:getLatestBNBPrice();
            amount = uint256(bnbPrice).mul(msg.value);
            ...
            _xMsgWallet.transfer(msg.value);
        } 
        emit CreditBuy(_msgSender(),usd,amount);
    ...
    function manualsend() external {
        require(_msgSender() == _marketingWallet || _msgSender() == owner() || _msgSender() == _xMsgWallet);
        uint256 contractETHBalance = address(this).balance;
