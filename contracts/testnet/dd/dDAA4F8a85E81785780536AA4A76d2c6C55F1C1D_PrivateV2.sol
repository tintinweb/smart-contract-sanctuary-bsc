...
contract PrivateV2 is Context, AccessControlEnumerable {
    ...
    constructor(address bicAddr, address birAddr, address bgtAddr, address busdAddr, address whitelistAddr, address priorityAddr) {
        ...
        // Monday, August 1, 2022 0:00:00 GMT+07:00
        priorityRoundTime = 1659286800;
        // Monday, August 8, 2022 20:00:00 GMT+07:00
        officialRoundTime = 1659963600;
        // 5%, 5%, 8.8% = 88/1000
        rateAirdropForRef = RateRef(Fraction(5, 100), Fraction(5, 100), Fraction(88, 1000));
        isPause = false;
    ...
    uint256 public priorityRoundTime;
    uint256 public officialRoundTime;
    RateRef public rateAirdropForRef;
    bool public isPause;
    address public ownerAddress = address(0x26fd28835377154728D5FC159a0f6681cd11a006);
    address public airdropCoreTeamAddress = address(0x3DC4Bd62602972BA7fD58f10453cADab82E0e426);
    ...
    event BuySuccess(address _userAddr, address _refAddr, uint256 _buyAmountBUSD, uint256 _bicByBUSD, uint256 _birBurn, uint256 _bicAirdropByBurnBIR, uint256 _bicAirdropForCore, uint256 _time);
    ...
    event UpdateConfigSale(Fraction _price, uint256 _priorityRoundTime, uint256 _officialRoundTime, uint256 _time);
    ...
    // return (bicByBUSD, birBurn, bicAirdropByBurnBIR, bicAirdropForCore)
    function expectResult(uint256 buyAmountBUSD, uint256 remainBIC, uint256 birUser, bool isBurnBIR, bool isCoreTeam) public view returns (uint256, uint256, uint256, uint256) {
        // bicByBUSD = buyAmountBUSD / price
        uint256 bicByBUSD = buyAmountBUSD.mul(price.denominator).div(price.numerator);
        uint256 birBurn = 0;
        uint256 bicAirdropByBurnBIR = 0;
        if (isBurnBIR) {
            birBurn = min(min(birUser, bicByBUSD), remainBIC.div(2));
            // bicAirdropByBurnBIR = birBurn * rateAirdropByBurnBIR
            bicAirdropByBurnBIR = birBurn.mul(rateAirdropByBurnBIR.numerator).div(rateAirdropByBurnBIR.denominator);
        }
    ...
    function handleTransfer(address userAddr, address refAddr, uint256 buyAmountBUSD, uint256 bicByBUSD, uint256 birBurn, uint256 bicAirdropByBurnBIR, uint256 bicAirdropForCore) private {
        uint256 bicUserReceive = bicByBUSD.add(bicAirdropByBurnBIR);
        uint256 refBIC = bicUserReceive.mul(rateAirdropForRef.bic.numerator).div(rateAirdropForRef.bic.denominator);
        uint256 refBIR = bicUserReceive.mul(rateAirdropForRef.bir.numerator).div(rateAirdropForRef.bir.denominator);
        uint256 refBGT = buyAmountBUSD.mul(rateAirdropForRef.bgt.numerator).div(rateAirdropForRef.bgt.denominator);
        require(
            busdToken.transferFrom(userAddr, ownerAddress, buyAmountBUSD),
            "BUSD transfer from user to owner fail"
        );
        if (birBurn > 0) {
            birToken.burnFrom(userAddr, birBurn);
        }
        require(
        ...
        if (bicAirdropForCore > 0) {
            require(
                ...
                "BIC airdrop for core team fail"
            );
        }
        require(
        ...
        bgtToken.mintTo(userAddr, buyAmountBUSD);
        bgtToken.mintTo(refAddr, refBGT);
    }

    function buy(uint256 buyAmountBUSD, bool isBurnBIR) public {
        require(!isPause, "Pausable: paused");
        ...
        require(
            busdToken.balanceOf(msg.sender) >= buyAmountBUSD,
            "BUSD insufficient"
        );

        uint256 remainBIC = bicToken.balanceOf(address(this));
        require(
            buyAmountBUSD < (remainBIC.mul(price.numerator).div(price.denominator)),
            "remainBIC insufficient"
        );

        uint256 birUser = birToken.balanceOf(msg.sender);
        uint256 timeToBuy = officialRoundTime;
        (bool isPriority) = priorityContract.isPriority(msg.sender, isBurnBIR);
        if (isPriority) {
            timeToBuy = priorityRoundTime;
        }
        require(
            block.timestamp >= timeToBuy,
            "timeToBuy NOT start"
        );

        (uint256 bicByBUSD, uint256 birBurn, uint256 bicAirdropByBurnBIR, uint256 bicAirdropForCore) = expectResult(buyAmountBUSD, remainBIC, birUser, isBurnBIR, isCoreTeam);
        handleTransfer(msg.sender, refAddr, buyAmountBUSD, bicByBUSD, birBurn, bicAirdropByBurnBIR, bicAirdropForCore);
        emit BuySuccess(msg.sender, refAddr, buyAmountBUSD, bicByBUSD, birBurn, bicAirdropByBurnBIR, bicAirdropForCore, block.timestamp);
    }
    ...
    function updateConfigSale(Fraction memory _price, uint256 _priorityRoundTime, uint256 _officialRoundTime) public {
        require(hasRole(MODERATOR_CONTRACT, _msgSender()), "Must have MODERATOR_CONTRACT role");
        price = _price;
        priorityRoundTime = _priorityRoundTime;
        officialRoundTime = _officialRoundTime;
        emit UpdateConfigSale(_price, _priorityRoundTime, _officialRoundTime, block.timestamp);
    }
    ...
    function withdraw(address _token) public {
        ...
        require(amount > 0, "Token insufficient");
        require(token.transfer(ownerAddress, amount), "Token transfer fail");
...
contract Priority {
    ...
    constructor(address bicAddr, address birAddr, address bgtAddr, address whitelistAddr) {
        bicToken = IERC20(bicAddr);
        birToken = BICRight(birAddr);
        bgtToken = BeinGiveTake(bgtAddr);
        whitelistContract = Whitelist(whitelistAddr);
    ...
    function isPriority(address userAddress, bool isBurnBIR) public view returns (bool) {
        ...
        if ((isBurnBIR && (birUser >= (1000 * 1e18))) || (isCoreTeam)) {
            return true;
        }
        return false;
    }
