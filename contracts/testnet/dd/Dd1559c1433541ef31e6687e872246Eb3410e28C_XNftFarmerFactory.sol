...
interface IXPilot is IERC721Enumerable {
    function mint(address player, uint64 kv, uint64 level, string memory genes, string memory tokenURI) external returns (uint256);
    function mintBatch(address player, uint256[] memory kvs, uint256[] memory levels, string[] memory genes, string[] memory tokenURIs) external returns (uint256[] memory);
    ...
    function updateToken(uint256 tokenId, uint64 kv, uint64 level, string memory genes, string memory tokenURI) external;
    function updateLevel(uint256 tokenId, uint64 level, string memory tokenURI) external;
...
contract XNftPower is Ownable {
    ...
    constructor(IXPilot _xPilot, IXShip _xShip) {
        xPilot = _xPilot;
        xShip = _xShip;
    }
