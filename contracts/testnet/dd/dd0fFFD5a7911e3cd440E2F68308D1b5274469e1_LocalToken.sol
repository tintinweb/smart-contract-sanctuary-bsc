...
contract LocalToken is BEP20("Token", "B20") {
...
contract Staking_pool is Ownable {
    ...
    function pendingAmount( address _user, uint256 index) public view returns (uint256) {
        uint256 _pid=0;
        UserInfo storage user = userInfo[_pid][_user];
        uint256 pending;
        if (user.amount > 0) {
    ...
    function userDoneDays( address _user, uint256 index) public view returns (uint256 doneDays){
        uint256 _pid=0;
        UserInfo storage user = userInfo[_pid][_user];
        UserRewardInfo storage rewardInfo = user.userRewardInfo[index];
        doneDays= block.timestamp.sub(rewardInfo.depositAt);
        doneDays= doneDays.div(1 days);
        return doneDays;
    }

    //Shows for how many days users staked the Lp tokens
    function userStakedDays(address _user, uint256 index) public view returns (uint256 stakedDays){
        uint256 _pid=0;
        UserInfo storage user = userInfo[_pid][_user];
        stakedDays= user.userRewardInfo[index].canHarvestTimestamp.sub(user.userRewardInfo[index].depositAt);
