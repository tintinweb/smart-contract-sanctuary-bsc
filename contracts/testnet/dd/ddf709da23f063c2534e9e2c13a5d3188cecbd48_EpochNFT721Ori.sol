    ...
    BEP721BurnableUpgradeable
{
    ...
    uint256 public discountAdminBNB;
    uint256 public discountAdminToekn;
    uint256 public minimumTokenLimit;

    uint256 private serviceValue;
    uint256 private escServiceValue;
    ...
    function initialize(address _whitelistAdmin) public initializer {   
        __Ownable_init();
        BEP721Upgradeable.__BEP721_init("EpochNFT", "EpochNFT");
        _tid = 1;
    ...
    function serviceFunctionEsc(uint256 _escServiceValue) public onlyOwner {
        escServiceValue = _escServiceValue;
    }

    function getServiceFeeEsc() public view returns (uint256) {
        return escServiceValue;
    }
    ...
    function mint(
        ...
        isStakeable[tokenId] = false;
        if( whiteListuserArray[msg.sender] == true && isStakable) {
            isStakeable[tokenId] = isStakable;
        }
    ...
    function updateStakeoption(bool _stakeStatus, uint256 tokenId) public {
        require(_exists(tokenId), "Token not Exist");
        require(balances[tokenId][msg.sender] > 0, "Not a owner");
        isStaked[tokenId] = _stakeStatus;
    }
    ...
    function setdiscountAdminBNB(uint256 _discountAdminBNB) public onlyOwner {
        discountAdminBNB = _discountAdminBNB;
    }

    function setdiscountAdminToekn(uint256 _discountAdminToekn) public onlyOwner {
        discountAdminToekn = _discountAdminToekn;
    }

    function setminimumTokenLimit(uint256 _minimumTokenLimit) public onlyOwner {
        minimumTokenLimit = _minimumTokenLimit;
    }
    ...
    function salewithToken(
        ...
        require(!isStaked[tokenId], "Unstake the token");
    ...
    function newtokenasbid(
        ...
        if(minimumTokenLimit != 0 && minimumTokenLimit < userBalance && whitelistAdmin == _creator[tokenId]  && balances[tokenId][whitelistAdmin] > 0) {
            amount = amount.sub(pBEPent(amount, discountAdminToekn));
        } else {
             require(
    ...
    function _acceptBId(
        ...
        IBEP20Upgradeable t = IBEP20Upgradeable(tokentype[tokenAss]);
        uint256 userBalance = t.balanceOf(from);
    ...
    function _saleToken(
        ...
        if(minimumTokenLimit != 0 && minimumTokenLimit < userBalance && whitelistAdmin == _creator[tokenId] && balances[tokenId][whitelistAdmin] > 0) {
            amount = amount.sub(pBEPent(amount, discountAdminBNB));
        } else {
            require(
            amount == order_place[from][tokenId].price &&
                order_place[from][tokenId].price > 0,
