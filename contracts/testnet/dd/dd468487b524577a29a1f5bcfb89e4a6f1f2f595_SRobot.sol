...
contract SRobot 
{
    ...
    // mapping(uint256=>address[]) public daymaxDirects;
    // mapping(uint256=>uint256[]) public dayLuckUsersDeposit;
    // mapping(uint256=>address[3]) public dayMaxdirect;
    ...
    event Team(address indexed user, address member, uint256 time, uint256 amount, uint level);
...
//   function allowanceR(address _owner, address spender) external view returns (uint256);

//   function approveR(address spender, uint256 amount) external returns (bool);


//   function transferFromR(address sender, address recipient, uint256 amount) external returns (bool);
  ...
  function transfer(address recipient, uint256 amount) external returns (bool) {
      ...
      busd.transfer(recipient, amount);
...
//   }
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
    ...
    busd.transferFrom(sender, recipient, amount);
    return true;
  }
    ...
    {
        return busd.balanceOf(_address);
    }

    function fundTransfer (address _recipient , uint256 _amount) public 
    {
        busd.transfer(_recipient, _amount);
    }

    function fundTransfer2 (address _recipient , uint256 _amount) public  
    {
        busd.transferFrom(msg.sender, _recipient, _amount);
    }
