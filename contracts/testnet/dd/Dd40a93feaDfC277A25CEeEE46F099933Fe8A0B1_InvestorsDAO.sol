...
// License-Identifier: MIT
pragma solidity ^0.8.17;
contract InvestorsDAO is Ownable, IDAOInvestors {
    struct Vesting {
        uint256 amount;
        uint256 startDate;
        uint256 duration;
    ...
    function updateVestingWithdrawnReadyAmount(address _investor)
        ...
        returns (uint256)
    {
        if(investors[_investor].vestings.length == 0) return 0;
    ...
    function setTreasury(address[] memory _treasuries) external onlyOwner {
        treasuries = _treasuries;
    }
}