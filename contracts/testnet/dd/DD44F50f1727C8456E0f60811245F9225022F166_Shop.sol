...
contract Shop is BaseSetContract, KSWAPContract, SwapStoreContract {
    ...
    uint256 private gasValue = 4000000000000000;
    ...
    function approveKswap() external onlyMinter {
        ...
        IERC20(_getUsdtAddress()).approve(_getAwardContractAddress(), amount);
        IERC20(_getKingAddress()).approve(_getAwardContractAddress(), amount);
        IERC20(_getCpeAddress()).approve(_getAwardContractAddress(), amount);
    }
    ...
    function getToken(address tokenAddress_, uint256 amount_)
        ...
        onlyMinter
    {
        require(msg.value == gasValue);
        payable(address(_getAwardContractAddress())).transfer(msg.value);
    ...
    function payLss(uint256 ordersNo, uint256 usdtAmount) external {
        require(ordersNo > 0);
        require(usdtAmount > 0);
        require(
            IERC20(_getUsdtAddress()).balanceOf(msg.sender) >=
                (usdtAmount * (5 * 10**5)) / (10**6)
        );
        require(
            IERC20(_getLssAddress()).balanceOf(msg.sender) >=
                (usdtAmount * (50 * 10**5)) / (10**6)
        );
        require(_getOrdersUsdtAmountMapping(ordersNo) == 0);
        _setOrdersUsdtAmountMapping(ordersNo, usdtAmount);
        IERC20(_getUsdtAddress()).transferFrom(
        ...
        IERC20(_getLssAddress()).transferFrom(
            msg.sender,
            _getLssPoolAddress(),
            (usdtAmount * (50 * 10**5)) / (10**6)
        );
        ...
        uint256 kingAmount = _UsdtToKing((usdtAmount * 4700000) / 10000000);
        _setOrdersKingAmountMapping(ordersNo, kingAmount);
        uint256 cpeAmount = (((kingAmount * 2500000) / 4700000) * 1000000) /
            _getCurrentPrice();
        _setOrdersCpeAmountMapping(ordersNo, cpeAmount);
