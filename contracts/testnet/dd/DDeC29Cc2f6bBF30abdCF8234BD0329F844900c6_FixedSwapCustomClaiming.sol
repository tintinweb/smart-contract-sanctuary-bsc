 ...
 *Submitted for verification at Etherscan.io on 2021-02-15
*/
contract FixedSwapCustomClaiming is Pausable, Whitelist {
    using SafeMath for uint256;
    uint256 increment = 0;
 ...
 //   uint256 public secondUnlockDate;
  ...
  function setUnlockDates(uint _startDate, uint _endDate, uint256 _distributionDate, uint _lastUnlockDate) public onlyOwner {
       // require(lastUnlockDate == 0, "already set");

        require(_startDate < _endDate && _endDate < _distributionDate && _distributionDate < _lastUnlockDate, "invalid input");
     ...
     function getLocked(uint256 purchase_id) public view returns(uint) {
        if(block.timestamp > lastUnlockDate) {
            ...
            uint256 totalDuration = lastUnlockDate - distributionDate;

            // 1600
            uint256 locked =  purchases[purchase_id].amount.mul(80).div(100).mul(diff).div(totalDuration); 
            if (locked < 0) {
                locked = 0;
            }
            // 2000
            uint256 tge = purchases[purchase_id].amount.mul(20).div(100);
            // 10000 - 2000 - (8000 - 1600)
            return purchases[purchase_id].amount - tge - (purchases[purchase_id].amount.mul(80).div(100) - locked);
        }
    ...
    function addPurchaseInfo(uint256[] calldata _amounts, uint256[] calldata _tokenAmounts, address[] calldata _purchasers) external onlyOwner {
            for (uint256 i=0;i<_amounts.length;i++) {
                uint256 amt = _amounts[i];
                uint256 tokenAmt = _tokenAmounts[i];
                ...
                /* Create new purchase */
                Purchase memory purchase = Purchase(amt, amt, purchaser, tokenAmt, block.timestamp, false, false);
                purchases[purchase_id] = purchase;
                purchaseIds.push(purchase_id);
                myPurchases[purchaser].push(purchase_id);
                buyers.push(purchaser);
                tokensAllocated = tokensAllocated.add(amt);
                emit PurchaseEvent(amt, purchaser, block.timestamp);
            }
    }

    /* Action Functions */
    function swap(uint256 _amount, uint256 _currencyTokenAmount) external whenNotPaused isSaleOpen onlyWhitelisted {
