...
//  uint constant secondInADay = 86400; // 24 * 60 * 60
  ...
  function withdraw() external {
    ...
    totalFotaDeposited -= farmers[msg.sender].fotaDeposited;
    totalLPDeposited -= farmers[msg.sender].lpDeposited;
    farmers[msg.sender].fotaDeposited = 0;
    farmers[msg.sender].lpDeposited = 0;
    uint fundAmount = 0;
    if (dayPassed >= rewardingDays) {
      for (uint i = 1; i < rewardingDays; i++) {
        uint index = dayPassed - i;
        if (farmers[msg.sender].pendingRewards[index] > 0) {
          fundAmount += farmers[msg.sender].pendingRewards[index];
          farmers[msg.sender].pendingRewards[index] = 0;
        }
      }
    } else {
      for (uint i = 0; i < rewardingDays; i++) {
        if (farmers[msg.sender].pendingRewards[i] > 0) {
          fundAmount += farmers[msg.sender].pendingRewards[i];
          farmers[msg.sender].pendingRewards[i] = 0;
        }
      }
    }
    _fundFOTA(fundAmount, dayPassed);
    if (checkinTracker[msg.sender][dayPassed]) {
      dailyCheckinPoint[dayPassed] -= farmers[msg.sender].point;
    }
    _syncDailyTotalPoint(dayPassed);
    dailyTotalPoint[dayPassed] -= farmers[msg.sender].point;
  ...
  function getProfitRate() external view returns (uint) {
    return getTotalRewarded() / dailyTotalPoint[getDaysPassed()] / 1e18;
  }
  ...
  function _checkin(uint _dayPassed) private {
    ...
    _syncDailyTotalPoint(_dayPassed);
    if (dailyTotalPoint[_dayPassed] == 0) {
      return;
    }
    uint reward = farmers[msg.sender].point * dailyReward[_dayPassed] / dailyTotalPoint[_dayPassed];
    if (checkinTracker[msg.sender][_dayPassed]) {
      _queueReward(_dayPassed, reward);
      emit CheckedIn(msg.sender, _dayPassed, reward, block.timestamp);
    } else {
      emit Missed(msg.sender, _dayPassed, reward);
    }
    if (_dayPassed > rewardingDays + 1) {
      _checkMissedClaim(_dayPassed);
    }
  }

  function _queueReward(uint _dayPassed, uint _reward) private {
    dailyCheckedIn[_dayPassed] += _reward;
    farmers[msg.sender].pendingRewards[_dayPassed] = _reward;
  }

  function _checkMissedClaim(uint _dayPassed) private {
    uint i = rewardingDays + 1;
    uint index = _dayPassed - i;
    uint fundAmount;
    while (farmers[msg.sender].pendingRewards[index] > 0) {
      fundAmount += farmers[msg.sender].pendingRewards[index];
      farmers[msg.sender].pendingRewards[index] = 0;
      if (i < _dayPassed) {
        index = _dayPassed - ++i;
    ...
    _fundFOTA(fundAmount, _dayPassed);
  }
  ...
  function _checkClaim(uint _dayPassed) private {
    ...
    totalEarned += reward;
    emit Claimed(msg.sender, _dayPassed, reward, block.timestamp);
  }
