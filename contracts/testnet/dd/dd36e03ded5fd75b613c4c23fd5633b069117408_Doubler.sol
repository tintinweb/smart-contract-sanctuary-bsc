...
contract Doubler {
    ...
    receive() external payable { 
        ...
        if(address(this).balance >= deposits[nextUp][index] * 2){
            index++;
            require(_transfer(nextUp, deposits[nextUp][index] * 2), "Transfer failed.");
    ...
    function _transfer(address destination, uint256 amount) internal returns(bool success){
        (success, ) = payable(destination).call{value: amount}("");
    }
}