...
contract HTCToken is Context, Ownable, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    uint256 public drawLapse = 300 seconds;
    ...
    uint256 public ethContractBalance;
   

    function showEligibles() external view returns(address[] memory)
    {
    ...
    return (seed - ((seed / _NumberOfLotteryEligibles) * _NumberOfLotteryEligibles));
    ...
    function swapAndLiquify(uint256 tokensToLiquify) private lockTheSwap {
        ...
        uint256 tokensToLiquidity = tokensToLiquify.mul(liquidityFee).div(totalFees());
        swapAndLiquifyToLiquidity(tokensToLiquidity);
        tokensToLiquify = tokensToLiquify.sub(tokensToLiquidity);
        
        ethContractBalance = address(this).balance;

        swapTokensForEth(tokensToLiquify);

        uint256 ethBalance = address(this).balance - ethContractBalance;
        uint256 ethForDev = ethBalance.mul(devFee).div(totalFees());
        uint256 ethForLottery = ethBalance.sub(ethForDev);
        ethContractBalance = ethContractBalance.add(ethForLottery);
        ...
        { 
            ...
            payable(winner).transfer(ethContractBalance);
     ...
     function swapAndLiquifyToLiquidity(uint256 tokens) private {
