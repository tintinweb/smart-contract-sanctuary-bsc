...
contract VenusAdapter is ILendingAdapter, ExponentialNoError {
    ...
    function deposit(address platformToken, uint amount) external override {
        ...
        if (address(asset) == NATIVE_TOKEN_ADDRESS) {
            ...
            if (asset.allowance(address(this), address(token)) == 0)
    ...
    function harvestReward() external override { 
    ...
    function accountDetailInfo(address account) external view override returns(AccountDetailInfo[] memory result) {
        ...
        for(uint i = 0; i < vTokens.length; i++) {
            ...
            // sumDeposit += oraclePrice * aTokenBalance
            result[i].depositValue = mul_ScalarTruncateAddUInt(vars.oraclePrice, vars.aTokenBalance, vars.sumDeposit);

            // sumCollateral += tokensToDenom * aTokenBalance
            result[i].collateral = mul_ScalarTruncateAddUInt(vars.tokensToDenom, vars.aTokenBalance, vars.sumCollateral);

            // sumBorrowPlusEffects += oraclePrice * borrowBalance
            result[i].borrowValue = mul_ScalarTruncateAddUInt(vars.oraclePrice, vars.borrowBalance, vars.sumBorrow);
        }
    }
