...
contract ClaimV2 is AccessControl {
    ...
    function claimOption2() external {
        require(!paused, "Please wait until the pool is open");
        require(startTime < block.timestamp, "has not started yet");
        uint256 amountPeriod = getPeriodByAccount(msg.sender);
    ...
    function getPeriodByAccount(address _account) public view returns(uint256){
        ...
        if(_timeStamp > startTime){
            ...
            amountAvaiable = amountPeriod.sub(withdrawnAmount[address(_account)]);
        }
    ...
    function safeBurn(uint256 _amount) public {
        ...
        tokenHeroes.transfer(address(0), amount);
    }
