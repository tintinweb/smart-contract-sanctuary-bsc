...
abstract contract SSV2Base is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "Sunshield V2";
	string private constant SYMBOL = "SSV2";
	...
	uint256 private constant _totalTokens = 777777 * 10**DECIMALS;	//777,777.00 total supply
	...
	// TRANSACTION LIMIT
	uint256 private _transactionLimit = _totalTokens / 1000; //777.777 The amount of tokens that can be sold at once
	...
	address private _salaryWallet = 0x1176b05f6E73c84Ab75A82f5e4411f24188FcbAc;
	...
	function calculateFeeRate(address sender, address recipient) private view returns(uint256) {
		...
		if (applyFees) {
			(bool ssv2Flag, uint256 ssv2BuyFee, uint256 ssv2SellFee) = onBeforeCalculateFeeRate();
	...
	function onBeforeCalculateFeeRate() internal virtual view returns (bool, uint256, uint256) {
		return (false, 0, 0);
	}
	...
	function executeSwap(uint256 amount) private {
		...
		uint256 tokensReservedForSalary = amount - tokensReservedForLiquidity - tokensReservedForBuyback - tokensReservedForMarketing - tokensReservedForReward;
		...
		uint256 tokensToSwap = tokensReservedForReward + tokensToSwapForLiquidity + tokensReservedForBuyback + tokensReservedForMarketing + tokensReservedForSalary;
		...
		uint256 bnbToBeSendToSalary = bnbSwapped * tokensReservedForSalary / tokensToSwap;
	...
	function swapBNBForCustomeTokens(address token, uint256 bnbAmount, address to) internal returns(bool) {
	...
	function setBuyFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 salaryFee) public onlyOwner {
	...
	function setSellFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 salaryFee) public onlyOwner {
	...
	function adjustTaxes(bool isBuyFromLp, bool isSelltoLp) private {
		...
		_poolFee = _liquidityFee + _rewardFee + _buybackFee + _marketingFee + _salaryFee;
	}
...
contract SSV2 is SSV2Base {
	...
	uint256 private _happyHourStartTimestamp;
	CustomTaxPeriod private _ssv21 = CustomTaxPeriod('ssv21', 3600,1,5,4,10,1,1,2,4,0,5);
	CustomTaxPeriod private _ssv22 = CustomTaxPeriod('ssv22', 3600,2,2,5,10,1,1,3,4,1,3);
	...
	constructor (address routerAddress, address pinkAntiBot_) SSV2Base(routerAddress, pinkAntiBot_) {
	...
	function onActivated() internal override {
		...
		setMinRewardBalance(777777 * 10**decimals() / 1000);  //At least 777.777 tokens are required to be eligible for rewards
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		if(!isTradingEnabled) {
			require(!isPancakeSwapPair(recipient), "SSV2: Trading is currently disabled.");
			require(!isPancakeSwapPair(sender), "SSV2: Trading is currently disabled.");
		}
	...
	function claimRewardToken(address user, uint256 bnbAmount) private returns (bool) {
		...
		bool success = swapBNBForCustomeTokens(claimRewardAsToken(user), bnbAmount, user);
		if (!success) {
			return false;
		}
    ...
    function bnbRewardClaimedAsSSV2(address byAddress) public view returns (uint256) {
	...
	function blacklistAccount(address account) public onlyOwner {
		require(!_isBlacklisted[account], "SSV2: Account is already blacklisted");
	...
	function claimRewardAsToken(address ofAddress) public view returns(address) {
	...
	function setClaimRewardAsToken(address token) public {
		require(token != address(0), "Cannot use the zero address as reward address");
		require(token != _claimRewardAsTokens[msg.sender], "SSV2: Cannot update claimToken to same value");
		IPancakeRouter02 pancakeswapV2RouterAddress = IPancakeRouter02(pancakeSwapRouterAddress());
		address pairAddress = IPancakeFactory(pancakeswapV2RouterAddress.factory()).getPair(token, pancakeswapV2RouterAddress.WETH());
		require(pairAddress != address(0), "Cannot use this address as reward address because this address is not added on pancakeswap");
		_claimRewardAsTokens[msg.sender] = token;
	}

	function setClaimRewardAsTokenAndPercentage(address token, uint256 percentage) public {
	...
	function isInHappyHour() external view returns (bool) {
		uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _happyHourStartTimestamp  ? _tradingPausedTimestamp : _getNow();
		uint256 totalHappyHour = _ssv21.timeInPeriod + _ssv22.timeInPeriod;
		uint256 timeSinceSsv2 = currentTimestamp - _happyHourStartTimestamp;
		if(timeSinceSsv2 < totalHappyHour) {
				return true;
		} else {
				return false;
		}
	...
	function setHappyHour1SellFees(uint256 liquidityFee,uint256 rewardFee,uint256 buybackFee,uint256 marketingFee,uint256 salaryFee) public onlyOwner {
		_setCustomSellTaxPeriod(_ssv21,liquidityFee, rewardFee,buybackFee,marketingFee,salaryFee);
	}

	function setHappyHour2SellFees(uint256 liquidityFee,uint256 rewardFee,uint256 buybackFee,uint256 marketingFee,uint256 salaryFee) public onlyOwner {
		_setCustomSellTaxPeriod(_ssv22,liquidityFee, rewardFee,buybackFee,marketingFee,salaryFee);
	}

	function setHappyHour1BuyFees(uint256 liquidityFee,uint256 rewardFee,uint256 buybackFee,uint256 marketingFee,uint256 salaryFee) public onlyOwner {
		_setCustomBuyTaxPeriod(_ssv21,liquidityFee, rewardFee,buybackFee,marketingFee,salaryFee);
	}

	function setHappyHour2BuyFees(uint256 liquidityFee,uint256 rewardFee,uint256 buybackFee,uint256 marketingFee,uint256 salaryFee) public onlyOwner {
		_setCustomBuyTaxPeriod(_ssv22,liquidityFee, rewardFee,buybackFee,marketingFee,salaryFee);
	}
	...
	function _setCustomSellTaxPeriod(CustomTaxPeriod storage map,
		...
		if (map.liquidityFeeOnSell != liquidityFee) {
				emit CustomTaxPeriodChange(liquidityFee, map.liquidityFeeOnSell, 'liquidityFee', map.periodName);
				map.liquidityFeeOnSell = liquidityFee;
		}
		if (map.rewardFeeOnSell != rewardFee) {
				emit CustomTaxPeriodChange(rewardFee, map.rewardFeeOnSell, 'marketingFee', map.periodName);
				map.rewardFeeOnSell = rewardFee;
		}
		if (map.buybackFeeOnSell != buybackFee) {
				emit CustomTaxPeriodChange(buybackFee, map.buybackFeeOnSell, 'buyBackFee', map.periodName);
				map.buybackFeeOnSell = buybackFee;
		}
		if (map.marketingFeeOnSell != marketingFee) {
				emit CustomTaxPeriodChange(marketingFee, map.marketingFeeOnSell, 'marketingFee', map.periodName);
				map.marketingFeeOnSell = marketingFee;
		}
		if (map.salaryFeeOnSell != salaryFee) {
				emit CustomTaxPeriodChange(salaryFee, map.salaryFeeOnSell, 'salaryFee', map.periodName);
				map.salaryFeeOnSell = salaryFee;
		}
	}
	...
	function _setCustomBuyTaxPeriod(CustomTaxPeriod storage map,
		...
		if (map.liquidityFeeOnBuy != liquidityFee) {
				emit CustomTaxPeriodChange(liquidityFee, map.liquidityFeeOnBuy, 'liquidityFee', map.periodName);
				map.liquidityFeeOnBuy = liquidityFee;
		}
		if (map.rewardFeeOnBuy != rewardFee) {
				emit CustomTaxPeriodChange(rewardFee, map.rewardFeeOnBuy, 'marketingFee', map.periodName);
				map.rewardFeeOnBuy = rewardFee;
		}
		if (map.buybackFeeOnBuy != buybackFee) {
				emit CustomTaxPeriodChange(buybackFee, map.buybackFeeOnBuy, 'buyBackFee', map.periodName);
				map.buybackFeeOnBuy = buybackFee;
		}
		if (map.marketingFeeOnBuy != marketingFee) {
				emit CustomTaxPeriodChange(marketingFee, map.marketingFeeOnBuy, 'marketingFee', map.periodName);
				map.marketingFeeOnBuy = marketingFee;
		}
		if (map.salaryFeeOnBuy != salaryFee) {
				emit CustomTaxPeriodChange(salaryFee, map.salaryFeeOnBuy, 'salaryFee', map.periodName);
				map.salaryFeeOnBuy = salaryFee;
		}
	}

	function getHappyHour1BuyFees() external view returns (uint256, uint256, uint256,uint256,uint256){
        return (_ssv21.liquidityFeeOnBuy,_ssv21.rewardFeeOnBuy, _ssv21.buybackFeeOnBuy, _ssv21.marketingFeeOnBuy, _ssv22.salaryFeeOnBuy);
	}

	function getHappyHour1SellFees() external view returns (uint256, uint256, uint256,uint256,uint256){
        return (_ssv21.liquidityFeeOnSell,_ssv21.rewardFeeOnSell, _ssv21.buybackFeeOnSell, _ssv21.marketingFeeOnSell, _ssv22.salaryFeeOnSell);
	}

	function getHappyHour2BuyFees() external view returns (uint256, uint256, uint256,uint256,uint256){
        return (_ssv22.liquidityFeeOnBuy,_ssv22.rewardFeeOnBuy, _ssv22.buybackFeeOnBuy, _ssv22.marketingFeeOnBuy, _ssv22.salaryFeeOnBuy);
	}

	function getHappyHour2SellFees() external view returns (uint256, uint256, uint256,uint256,uint256){
        return (_ssv22.liquidityFeeOnSell,_ssv22.rewardFeeOnSell, _ssv22.buybackFeeOnSell, _ssv22.marketingFeeOnSell, _ssv22.salaryFeeOnSell);
	}

	function _isInHappyHour1() internal view returns (bool) {
		uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _happyHourStartTimestamp  ? _tradingPausedTimestamp : _getNow();
		uint256 timeSinceSsv2 = currentTimestamp - _happyHourStartTimestamp;
		if(timeSinceSsv2 < _ssv21.timeInPeriod) {
				return true;
		} else {
				return false;
		}
	...
	function _isInHappyHour2() internal view returns (bool) {
		uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _happyHourStartTimestamp  ? _tradingPausedTimestamp : _getNow();
		uint256 timeSinceSsv2 = currentTimestamp - _happyHourStartTimestamp;
		if(timeSinceSsv2 > _ssv21.timeInPeriod && timeSinceSsv2 < _ssv21.timeInPeriod + _ssv22.timeInPeriod) {
				return true;
		} else {
				return false;
		}
	...
	function onBeforeCalculateFeeRate() internal override view returns (bool ssv2Flag, uint256 ssv2BuyFee, uint256 ssv2SellFee) {
		if(this.isInHappyHour()) {
			ssv2Flag = true;
			if(_isInHappyHour1()) {
				ssv2BuyFee = _ssv21.liquidityFeeOnBuy + _ssv21.rewardFeeOnBuy + _ssv21.buybackFeeOnBuy + _ssv21.marketingFeeOnBuy + _ssv21.salaryFeeOnBuy;
				ssv2SellFee = _ssv21.liquidityFeeOnSell + _ssv21.rewardFeeOnSell + _ssv21.buybackFeeOnSell + _ssv21.marketingFeeOnSell + _ssv21.salaryFeeOnSell;
			}
			else if (_isInHappyHour2()) {
 				ssv2SellFee = _ssv22.liquidityFeeOnBuy + _ssv22.rewardFeeOnBuy + _ssv22.buybackFeeOnBuy + _ssv22.marketingFeeOnBuy + _ssv22.salaryFeeOnBuy;
				ssv2BuyFee = _ssv22.liquidityFeeOnSell + _ssv22.rewardFeeOnSell + _ssv22.buybackFeeOnSell + _ssv22.marketingFeeOnSell + _ssv22.salaryFeeOnSell;
			}
			return (ssv2Flag, ssv2BuyFee, ssv2SellFee);
		}
