...
// File contracts/AssetMinter.sol
...
contract AssetMinter is Ownable, Pausable, ReentrancyGuard {
	...
	function calculateFee(address user, uint256 inputAmount, address output) public view returns (uint256) {
		if (fee == address(0)) {
			return 0;
		}
		return IFee(fee).calculate(user, inputToken, inputAmount, output);
	}
	...
	function fulfill(uint256 id, uint256 pricePerUnit) external onlyRelayer {
		...
		emit Fulfill(id, order.sender, msg.sender, order.asset, order.assetAmount, pricePerUnit, refundAmount);
	}
...
// File contracts/AssetTokenManager.sol
...
contract AssetTokenManager is Ownable, Pausable {
	...
	function mint(address token, uint256 amount) external onlyMinter onlyAsset(token) whenNotPaused {
		IAssetToken(token).mint(msg.sender, amount);
		emit Mint(msg.sender, token, amount);
	}

	function burn(address token, uint256 amount) external onlyMinter onlyAsset(token) whenNotPaused {
		IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
		IAssetToken(token).burn(amount);
		emit Burn(msg.sender, token, amount);
	}

	function deployAssetToken(string memory name, string memory symbol) external onlyOwner {
...
contract Fee is Ownable {
	...
	function calculate(address user, address input, uint256 inputAmount, address output) external view returns (uint256) {
