...
contract AdvisorVesting is ReentrancyGuard, Ownable {
    ...
    function schedule(address _advisor, uint _period) public view returns (
        uint date,
        uint amount,
        uint dateClaimed,
        Status status
    ) {
        date = settings.tgeTimestamp == 0 ? 0 : _periodToDate(_advisor, _period);
        amount = _amountByPeriod(_period, _advisor);
        dateClaimed = dateRealeased[_advisor][_period];
        if (isClaimed[_advisor][_period] == true) {
            status = Status.Claimed;
        } else {
            if (settings.tgeTimestamp == 0){
                status = Status.Pending;
            } else {
                if (_periodToDate(_advisor, _period) > block.timestamp) {
                status = Status.Pending;
                } else if (_periodToDate(_advisor, _period) <= block.timestamp) {
    ...
    function _amountByPeriod(uint _period, address _advisor) private view returns (uint tokenAmount) {
        if (_period == 1) {
            tokenAmount = (advisorsAllocation[_advisor] / 100) * settings.tokenPercentUnlockOnTge;
        } else {
            tokenAmount = (advisorsAllocation[_advisor] - ((advisorsAllocation[_advisor] / 100) * settings.tokenPercentUnlockOnTge)) / (settings.numberOfEmmissions - 1);
        }
    }
}
