...
abstract contract Token is IERC20, Ownable {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _ms + _buyLPDividendFee + _ws + _sellLPDividendFee + _sellLPFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _ws + _sellLPDividendFee + _sellLPFee;
            } else {
                require(tAmount <= maxTXAmount);
                swapFee = _ms + _buyLPDividendFee;
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = fistBalance * (_ms + _ws) * 2 / swapFee;
        uint firstfundamount = fundAmount / 2;
        uint secfundamount = fundAmount - firstfundamount;
        address secFundAddress = 0xeDb1EF3a969BF8DeDB525C3d1a1974B12b2005c1;//   营销
    ...
    function setms(uint256 s) external onlyOwner {
        _ms = s;
    }
    uint256 public startAddLPBlock;

    function setws(uint256 s) external onlyOwner {
        _ws = s;
    }
...
contract nice is Token {
    constructor() Token(
    
        address(0xB6BA90af76D139AB3170c7df0139636dB6120F7e), //路由
    
        address(0xEdA5dA0050e21e9E34fadb1075986Af1370c7BDb),  //u
        "kkk",
        "kkk",
        18,  //别改喽
        ...
        address(0xeDb1EF3a969BF8DeDB525C3d1a1974B12b2005c1),//fund 

        address(0x5F97FC69fF36EA3e679112A177Dbf761Ec418888)//dev 
    ){
