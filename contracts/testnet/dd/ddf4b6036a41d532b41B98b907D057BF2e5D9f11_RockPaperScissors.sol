...
abstract contract GameContext is Ownable {
    ...
    event ContextUpdate(uint256 _newIndex);
}
...
abstract contract Balance {
    ...
    function withraw(uint256 amount) external payable withrawMod(amount) {
        balances[msg.sender] -= amount;
        payable(msg.sender).transfer(amount);
        emit Withraw(msg.sender, amount);
    }

    modifier withrawMod(uint256 amount) {
        require(amount > 0, "ZERO");
        require(balances[msg.sender] >= amount, "INSUFFICIENT");
        _;
    }

    event Withraw(address indexed _adr, uint256 _amount);
}
...
abstract contract Referral {
    ...
    event NewReferral(address indexed _adr);
}
