...
import "./CarsDividendTracker.sol";
import "../utils/ITokenInitialize.sol";

//import "../utils/Utils.sol";

contract CarsRace is ERC20Upgradeable, OwnableUpgradeable, ITokenInitialize {
    ...
    CarsDividendTracker public dividendTracker;
    ...
    event TransferLog(address add, string log);
    ...
    constructor() {
        implementation = address(new CarsDividendTracker());
    }
    ...
    function initialize(
        ...
        require(owner() != addrs[3], "Wallets are same");
        __ERC20_init(name_, symbol_);
        __Ownable_init();
        gasForProcessing = 400000;
        inSwapAndLiquify = false;
        txCount = 0;
        swapTrigger = 3;
        rewardToken = addrs[1];
        ...
        require(totalFees <= 100, "Fees 100%");
        swapTokensAtAmount = totalSupply_.mul(2).div(10**6); // 0.002%

        dividendTracker = CarsDividendTracker(payable(Clones.clone(implementation)));
        ...
        */
        _mint(owner(), totalSupply_ * (10**uint256(decimals())));
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        CarsDividendTracker newDividendTracker = CarsDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Must be owned by the token contract");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Already set to value");
    ...
    function _transfer(
        ...
        //emit TransferLog(from, "Transfer start");
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            emit TransferLog(from, "Take fees 0");
            takeFee = false;
        }
        emit TransferLog(from, "Take fees");
        ...
        emit TransferLog(from, "Transferring");
    ...
    function swapAndSendToFee(uint256 tokens) internal {
        ...
        //Utils.swapTokensForCake(uniswapV2Router, rewardToken, tokens);
    ...
    function swapTokensForEth(uint256 tokenAmount) internal {
        ...
        //Utils.swapTokensForEth(uniswapV2Router, tokenAmount);
        // make the swap
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        //Utils.addLiquidity(address(uniswapV2Router), owner(), tokenAmount, ethAmount);
    }
...
contract CarsDividendPayingToken is
    ERC20Upgradeable,
    OwnableUpgradeable,
...
contract CarsDividendTracker is OwnableUpgradeable, CarsDividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function initialize(address rewardToken_, uint256 minimumTokenBalanceForDividends_)
        ...
        initializer
    {
        CarsDividendPayingToken.__DividendPayingToken_init(
            rewardToken_,
            "DIVIDEND_TRACKER",
            "DIVIDEND_TRACKER"
        );
    ...
    function withdrawDividend() public pure override {
        require(false, "Dividend_Tracker: Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Dividend_Tracker: Update between 1 and 24 hours"
        );
        require(
...
interface ITokenInitialize {
    function initialize(
        address[4] memory addrs, // [0] = owner, [1] = rewardToken, [2] = router, [3] = marketing wallet, [4] = anti bot
        string memory name_,
        string memory symbol_,
        uint256 totalSupply_,
        uint256 tokenRewardsFee_,
