...
contract Presale is Ownable {
    ...
    uint256[] public rLockinPeriod = [0,1638433800,1638435600,1638437400];
    ...
    function buyToken() public payable returns (address) {
        ...
        if (totalBNBAmount > hardcap) isPresaleOpen = false;
        return wallet;
    }
    ...
    {
        uint256 rangeOrBracket = 0;
        for (uint256 i = 0; i <= priceBrackets.length; i++) {
            if (amount.div(1**18) < priceBrackets[i]) {
                rangeOrBracket = i;
            }
        }
        return rLockinPeriod[rangeOrBracket];
    }
}
