...
contract TaskManager is Validatable, ITaskManager, ReentrancyGuardUpgradeable {
     ...
     */
    function initialize(IAdmin _admin, IProject project_)
        public
        initializer
        notZeroAddress(address(_admin))
        notZeroAddress(address(project_))
    {
    ...
    function createTask(
        ...
        require(
            budget <= IClaimPool(project.getClaimPoolOf(collection)).getReward(collection),
        ...
        IClaimPool(project.getClaimPoolOf(collection)).addBudgetUse(collection, budget);
    ...
    function updateTask(
        ...
        if (taskInfo.collection != collection) {
            ...
            IClaimPool(project.getClaimPoolOf(taskInfo.collection)).reduceBudgetUse(
                taskInfo.collection,
                taskInfo.budget
            );
            IClaimPool(project.getClaimPoolOf(collection)).addBudgetUse(collection, budget);
            ...
            if (budget > taskInfo.budget) {
                require(
                    budget - taskInfo.budget <= IClaimPool(project.getClaimPoolOf(collection)).getReward(collection),
                    "Over collection budget"
                );

                IClaimPool(project.getClaimPoolOf(collection)).addBudgetUse(collection, budget - taskInfo.budget);
            } else {
                IClaimPool(project.getClaimPoolOf(collection)).reduceBudgetUse(collection, taskInfo.budget - budget);
            }
        }
    ...
    function cancelTask(uint256 taskId) external onlyProjectOwner(tasks[taskId].collection) {
        ...
        IClaimPool(project.getClaimPoolOf(taskInfo.collection)).reduceBudgetUse(taskInfo.collection, taskInfo.budget);
        //slither-disable-next-line unused-return
        _taskOfCollection[taskInfo.collection].remove(taskId);
    ...
    function completeTask(
        ...
        IClaimPool(project.getClaimPoolOf(taskInfo.collection)).transferToReward(taskInfo.collection, spentBudget);
