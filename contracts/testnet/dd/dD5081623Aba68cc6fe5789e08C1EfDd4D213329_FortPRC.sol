...
contract FortPRC is FortPRCToken {
    ...
    function calcHashValueWithBlockHash(uint diceIndex, bytes32 blockHash) external view returns (uint hashValue, uint gain) {
        Dice memory dice = _dices[diceIndex];
        hashValue = uint(blockHash);
        if (hashValue > 0) {
            hashValue = uint(keccak256(abi.encodePacked(hashValue, diceIndex)));
            if (hashValue % uint(dice.m) < TIMES_BASE) {
                gain = uint(1) * uint(dice.m) * 1 ether / TIMES_BASE;
            }
        }
    }

    function calcHashValue(uint diceIndex) external view returns (uint hashValue, uint gain) {
        Dice memory dice = _dices[diceIndex];
        uint hashBlock = uint(dice.openBlock) + OPEN_BLOCK_SPAN;
        hashValue = uint(blockhash(hashBlock));
        if (hashValue > 0) {
            hashValue = uint(keccak256(abi.encodePacked(hashValue, diceIndex)));
            if (hashValue % uint(dice.m) < TIMES_BASE) {
                gain = uint(1) * uint(dice.m) * 1 ether / TIMES_BASE;
