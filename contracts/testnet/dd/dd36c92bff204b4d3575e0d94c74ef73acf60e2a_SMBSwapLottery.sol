...
interface ISMBSwapLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in SELF per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in SELF token
     * @dev Callable by operator
     */
...
/** @title SMBSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract SMBSwapLottery is ReentrancyGuard, ISMBSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInSELF = 50 ether;
    uint256 public minPriceTicketInSELF = 0.005 ether;
     ...
     * @param _SELFTokenAddress: address of the SELF token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _SELFTokenAddress, address _randomGeneratorAddress) {
        SELFToken = IERC20(_SELFTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of SELF to this contract
        uint256 amountSELFToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInSELF,
            _ticketNumbers.length
        );

        // Transfer SELF tokens to this contract
        SELFToken.safeTransferFrom(address(msg.sender), address(this), amountSELFToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInSELF += amountSELFToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInSELFToTransfer
        uint256 rewardInSELFToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInSELFToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        SELFToken.safeTransfer(msg.sender, rewardInSELFToTransfer);

        emit TicketsClaim(msg.sender, rewardInSELFToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInSELF) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in SELF for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].SELFPerBracket[j] =
                ...
                // A. No SELF to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].SELFPerBracket[j] = 0;
        ...
        uint256 add_amount = (_lotteries[_lotteryId].amountCollectedInSELF - amountToShareToWinners);

        if (_autoInjection) 
        {
            pendingInjectionNextLottery = amountToWithdrawToTreasury / 2;
            amountToWithdrawToTreasury = amountToWithdrawToTreasury / 2;
            
            amountToWithdrawToTreasury += add_amount / 2;
            pendingInjectionNextLottery += add_amount / 2;
        }
        else
        {
            amountToWithdrawToTreasury += add_amount;
        }
 
        // Transfer SELF to treasury address
        SELFToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        SELFToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInSELF += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInSELF >= minPriceTicketInSELF) && (_priceTicketInSELF <= maxPriceTicketInSELF),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInSELF: _priceTicketInSELF,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            SELFPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInSELF: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(SELFToken), "Cannot be SELF token");
    ...
    /**
     * @notice Set SELF price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInSELF: minimum price of a ticket in SELF
     * @param _maxPriceTicketInSELF: maximum price of a ticket in SELF
     */
    function setMinAndMaxTicketPriceInSELF(uint256 _minPriceTicketInSELF, uint256 _maxPriceTicketInSELF)
        external
        onlyOwner
    {
        require(_minPriceTicketInSELF <= _maxPriceTicketInSELF, "minPrice must be < maxPrice");

        minPriceTicketInSELF = _minPriceTicketInSELF;
        maxPriceTicketInSELF = _maxPriceTicketInSELF;
    }
     ...
     * @param _priceTicket price of a ticket (in SELF)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].SELFPerBracket[_bracket];
        } else {
            return 0;
        }
    }
