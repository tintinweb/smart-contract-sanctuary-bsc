...
contract BullForcePVT is Ownable,ICanMint {
  ...
  mapping(address => prop) public donor;
   ...
   function setToken(IBEP20 addr) external onlyOwner{
    ...
    function _claimRFP () internal {
       ...
       function migratePVTForDividend(address _msgsender,uint256 _bnb,uint flag) external onlyOwner {
           storeValue(_msgsender,_bnb,flag);
      
       }

         function storeValue (address _msgSender, uint256 _bnb,uint256 flag) internal {
            ...
            if(flag ==0){
                donor[_msgSender].bnb =donor[_msgSender].bnb.sub(_bnb);
            }
            donor[_msgSender].exist =true;
