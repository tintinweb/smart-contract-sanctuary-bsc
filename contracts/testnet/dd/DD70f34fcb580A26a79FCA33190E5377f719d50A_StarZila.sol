...
contract StarZila is ReflectionEvents, RebaseMethods, BEP20("TestZila", "TTZ"), Ownable {
    ...
    constructor(address _treasury) {
        ...
        // reflectionTracker.excludeFromReflections(owner());
    ...
    function _setDexAddresses(IRouter02 _router, bool value) private {
        ...
        if(value) {
            ...
            _isExcludedFromAmountLimit[address(_router)] = true;
            _isExcludedFromPeriodLimit[address(_router)] = true;
    ...
    function _setAMMAddresses(address _pair, bool value) private {
        ...
        if(value) {
            ...
            _isExcludedFromPeriodLimit[address(_pair)] = true;
        }
    }
    ...
    function swapAndDistributeBNB(uint256 tokens) public {
    ...
    function _transferFrom( address sender, address recipient, uint256 amount ) internal returns (bool) {
        ...
        if(takeFeeInTrade) {
            ...
            if (_ammPairs[recipient]) {
                ...
                fees = amount.mul(sellReflectionFee).div(100);
            } else {
                accReflectionFee = accReflectionFee.add(amount.mul(reflectionFee).div(100));
                fees = amount.mul(reflectionFee).div(100);
            }
            amount = amount.sub(fees);
            _basicTransfer(sender, address(this), fees);
    ...
    function withdrawAllToTreasury() external isSwapping onlyOwner {
        ...
        require( amountToSwap > 0," No token deposited in token contract");
        _basicTransfer(address(this), treasuryReceiver, amountToSwap);
    }

    function shouldTakeFee(address from, address to) internal view returns (bool) {
        return 
            (_ammPairs[from] || _ammPairs[to]) &&
            !_isFeeExempt[from];
    }
...
contract ReflectionTracker is ReflectionPayingToken, Ownable {
    ...
    uint256 public claimWait = 1 seconds;
