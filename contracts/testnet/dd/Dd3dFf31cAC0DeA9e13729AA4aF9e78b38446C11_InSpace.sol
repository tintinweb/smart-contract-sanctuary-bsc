...
contract InSpace is IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x032c287B354be78C296166dFCaEd5F211A5bF667)
    });
    ...
    uint256 private _maxTxAmount = 10;
...
//===============================================================================================================
    ...
    function setRatios(uint16 reflection, uint16 buyback, uint16 marketing) external onlyOwner {
    ...
    function setMaxTxPercent(uint256 percentInHundreds) external onlyOwner {
        require(percentInHundreds >= 10, "Max Transaction amt must be above 0.1% of total supply.");
        _maxTxAmount = percentInHundreds;
    }

    function getMaxTX() external view returns (uint256) {
        return ((_tTotal * _maxTxAmount) / masterTaxDivisor) / (10**_decimals);
    }
        ...
        if (_hasLimits(from, to)) {
            ...
            if (buy || sell){
                if (!_isExcludedFromLimits[from] && !_isExcludedFromLimits[to]) {
                    require(amount <= (_maxTxAmount * getCirculatingSupply()) / masterTaxDivisor, "Transfer amount exceeds the maxTxAmount.");
                }
            }
        }
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if (ratios.totalSwap == 0 || contractTokenBalance == 0) {
            return;
        }
