...
contract Exchange is OrderState, TransferSafe {
    ...
    function bidExchange(Order calldata order, Sig calldata sig,Sig calldata buyerSig, Sig calldata buyerFeeSig, address buyer, uint256 buyingAmount, uint256 royaltyFee, address payable royaltyReceipt)  external {
        require(( block.timestamp <= order.expiryTime ), "signature expired" );
        require(buyingAmount >= order.buying , "b3uyingAmount invalid"); 
