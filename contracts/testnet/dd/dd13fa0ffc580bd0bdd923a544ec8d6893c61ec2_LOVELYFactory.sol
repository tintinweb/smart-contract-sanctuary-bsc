...
contract LOVELYTokenList {
    ...
    function add(address _token, address _validationToken, uint _validationAmount) public {

        require(owner == msg.sender, "LOVELY DEX: NOT_OWNER");

        // Cannot add the token twice
        require(slots[_token].token == address(0x0), 'LOVELY DEX: EXISTS');

        // Save requirements for token validation
        slots[_token].token = _validationToken;
        slots[_token].amount = _validationAmount;
    ...
    function feeAt(uint i) public view returns (uint256) {
        return slots[addresses[i]].amount;
    }

    function tokenAt(uint i) public view returns (address) {
        return slots[addresses[i]].token;
    }
    ...
    function validate(address _token) public {
    ...
    function validated(address _token) public view returns (bool) {
        ...
        require(slots[_token].token != address(0x0), 'LOVELY DEX: NOT_EXISTS');
