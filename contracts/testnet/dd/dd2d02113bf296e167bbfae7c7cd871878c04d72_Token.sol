...
contract Token is ERC20, Ownable {
    ...
    uint256 public timeGap = 30 minutes; // time interval in which autoLP is triggred if SwapTokenAtAmount is reached
    ...
    constructor() ERC20("TOKEN", "TKN") {
        ...
        */
        ...
        maxTxAmount = totalSupply().mul(1).div(1000);
        swapTokensAtAmount = totalSupply().mul(1).div(10000);

        dailyLimit = 250 * 10**3 * 10**18;
    }
    ...
    function blacklistBot (address _bot, bool value) external onlyOwner {
    ...
    function setSellFee(
        ...
        totalSellFee = liquidity + burn;
        require (totalSellFee < 300, "max sell fees should be less than 30%");
    }
  ...
  // function to claim other bep20 tokens and BNB
    ...
    function _transfer(
        ...
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(isBlacklisted[from] == false, "ERC20: Sender is Blacklisted");
        require(isBlacklisted[to] == false, "ERC20: Receiver is Blacklisted");
        ...
        if (takeFee) {
            require(amount <= maxTxAmount, "Amount exceeds Per Transaction limit");

            if (!automatedMarketMakerPairs[to]) {
                require(
                    balanceOf(to) + amount <= maxWalletAmount,
                    "Balance exceeds Antiwhale limit"
                );
            }
