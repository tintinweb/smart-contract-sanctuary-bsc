...
contract StakingService is PausableByOwner, RecoverableByOwner {
    ...
    constructor(
		...
		state.totalTokenSupply = 0;
		state.previousTokenSupply = 0;
    ...
    function updateHistoricalRewardRate() public {
        uint256 currentTokenSupply = state.totalTokenSupply - state.previousTokenSupply;
        if (currentTokenSupply == 0) return;
		state.previousTokenSupply = state.totalTokenSupply;
