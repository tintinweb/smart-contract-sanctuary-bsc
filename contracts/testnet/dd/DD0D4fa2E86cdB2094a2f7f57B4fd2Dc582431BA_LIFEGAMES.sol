 ...
 * @title LIFEGAMES
 * @author Paulo
 * @notice Smart Staking - Reflection: 0.5%   (standard)
 *         Liquidez: 1%                       (send tokens to wallat address)
 *         BuyBack: 1%                        (send tokens to external wallet and decide if burn or not)
 *         Reserves: 0.5%                     (send tokens to external wallet address)
 */
...
contract LIFEGAMES is ERC20Upgradeable {
    ...
    uint256 internal _gTotal; // Ghost Total, for supply and burn purpouses
    ...
    function initialize() public initializer {
        ...
        // used for enable / disable inject liquidity automatically
        autoLiquidityTokenPriceThreshold = 0;
        ...
        // Rewards
        addresses[4] = 0x7FE850239D59b0AA8923963874948E68B6833E21;
        // Equipo
        addresses[5] = 0xd235eD438FB2D6Bd428F5AEdF67bc8AB03bcFB96;
        // Liquidez
        addresses[6] = 0x26dF456853F3EFa617a2fE392c525ba0784670D6;
        // Advisors
        addresses[7] = 0x23DA0303B3540dD51ED8EdB73c3B57b0d123E9A0;
        // Pre-venta
        addresses[8] = 0x09f33F64aAADf6A02956C9732b25d42DD9c2d4bC;
        // BUSD BuyBackBurn
        ...
        _isExcludedFromFee[addresses[5]] = true;
        _isExcludedFromFee[addresses[6]] = true;
        _isExcludedFromFee[addresses[7]] = true;
        _isExcludedFromFee[addresses[8]] = true;

        // excluded from rewards
        _isExcludedFromRewards[addresses[2]] = true;
        _isExcludedFromRewards[addresses[3]] = true;
        _isExcludedFromRewards[addresses[4]] = true;
        _isExcludedFromRewards[addresses[5]] = true;
        _isExcludedFromRewards[addresses[6]] = true;
        _isExcludedFromRewards[addresses[7]] = true;
        _isExcludedFromRewards[addresses[8]] = true;
        ...
        setBusdBuyBurnAddress(addresses[8]);
    ...
    function _mint(address account, uint256 amount) internal override virtual {
        ...
        uint _maxN = type(uint256).max;
        _tTotal += amount;
        _gTotal = _tTotal;
        _rTotal = (_maxN - (_maxN % _tTotal));
    ...
    /**
     * @notice This function is used to update the AutoLiquidityToken Threshold
    ...
    /**
     * @notice This function is updating the value of the variable autoAddLiquidityEnabled
     * @param newValue New value of the variable
     */
    ...
    /**
     * @notice This function is updating the value of the variable autoAddLiquidityThreshold
     * @param newValue New value of the variable
     */
    ...
    /// @notice Function inherited from BEP20
    function totalSupply() public view virtual override returns (uint256) {return _gTotal;}
    ...
    function setNewRouter(address newRouter, address busd) public onlyOwner() {
        ...
        address getPair = IFactoryV2(_newRouter.factory()).getPair(address(busd), address(this));
    ...
    function _transfer(
        ...
        if (transferDelayEnabled && block.timestamp < (initialDelayTime + totalDelayTime)) {
            ...
            if (from != _owner && to != address(dexRouter) && to != address(lpPair)) {
     ...
     */
     ...
     /*
          ...
          // todo revisar con transfer no fee, transfer both excluded 
    function _burnNoFee(address sender, uint256 amount) private {

        _gTotal -= amount;
        _transferNoFee(sender, address(0), amount);
    ...
    */
    ...
    function _takeFees(address sender, FeeValues memory values) private {
        _takeFee(sender, values.tLiquidity + values.tBuyBurn + values.tReserve, address(this));
    }
    ...
    */
    ...
    function sendToPools() internal {
        uint256 contractBusdBalance =  IERC20(busdAddress).balanceOf(address(this));
        
        // calculate percentage to bsud reserver and manual buyback and burn 
        uint256 busdTokensToliquidityAmount = (contractBusdBalance * _ratios.liquidityRatio) / (_ratios.total);
        uint256 busdTokensToBuyBurnAmount = (contractBusdBalance * _ratios.buyBurnRatio) / (_ratios.total);
        uint256 busdTokensbusdReserveAmount = (contractBusdBalance * _ratios.busdReserveRatio) / (_ratios.total);
        
        IERC20(busdAddress).transfer(busdForLiquidityAddress, busdTokensToliquidityAmount);
        IERC20(busdAddress).transfer(busdBuyBurnAddress, busdTokensToBuyBurnAmount);
        IERC20(busdAddress).transfer(busdReserveAddress, busdTokensbusdReserveAmount);
    }
    ...
    /**
     * @notice Returns the price of a token in the token/busd pair
     * @param amount Amount of tokens to get price for
     * @return uint256 - price in busd
     */
    ...
    function burn(uint256 amount) public {
        ...
        require(amount <= balanceOf(msg.sender), "Burn amount should be less than account balance");
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        // add all extra fees
        uint256 tTransferFee = tValues.tLiquidity + tValues.tBuyBurn + tValues.tReserve;
    ...
    function _getTValues(uint256 tAmount) private view returns (tFeeValues memory) {
        ...
        tValues.tTransferAmount = tAmount - tValues.tFee - tValues.tLiquidity - tValues.tBuyBurn - tValues.tReserve;
        return tValues;
    }
    ...
    /// @notice Used to addLiquidity to the pair address
