...
contract FarmContract is ERC20, Ownable{
    ...
    bool inited;
    uint256 rebates;//返佣率
    uint256 rebatesLevel;//返佣级别
    IERC20 mainToken;//主质押代币
    ...
    function Claim() external{
        ...
        Reward = _staking.staking.add(Reward);
        IERC20 token = IERC20(mainToken);
    ...
    function callRebatesLevel(uint256 reward,address from) private{
        ...
        for(uint i = 0; i < rebatesLevel; i++){
            ...
            send = send.sub(half);
            token.transferFrom(address(this),lastAccount, send);
        ...
        if (!_excludedFees[from] && !_excludedFees[to]) {
            ...
            if (pairs[from]) {//buy  from==uniswapPair
                ...
                if(Inflation<90){//膨胀
                    ...
                    //销毁流通量
                    super._transfer(from, deadWallet, fees.sub(1));
                }
