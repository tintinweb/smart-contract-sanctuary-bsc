...
contract marketPlace {
    ...
    function buyNft(uint256 _id) public payable {
        ...
        IERC20 tkn = IERC20(listingMap[_id].erc20Tkn);
        uint _erctkn = listingMap[_id].price;
        if (listingMap[_id].erc20Tkn != address(0)) {
            require(
                _erctkn == listingMap[_id].price,
            ...
            // _price = msg.value;
            require(
                msg.value >= listingMap[_id].price,
                "your price is not correct"
            );
    ...
    function finalizeAuction(uint256 _id) public payable {
        ...
        // uint256 value;
        payable(aucInfo[_id].seller).transfer(aucInfo[_id].highestPayableBid);
        ...
        tkn.transfer(aucInfo[_id].seller, 
        aucInfo[_id].highestPayableBid);
    ...
    // function finalizedAuction(uint256 _id) external  {
    //         // require(_id < aucInfo.length, "Invalid auction index");
    //         // require(!isOpen(_id), "Auction is still open");
    //         // Auction storage auction = auctionData[_id];
    //         require(msg.sender==aucInfo[_id].seller ||msg.sender== aucInfo[_id].highestBidder,"NFT can be claimed  by the current bid owner or ouction creator");

    //         IERC721 nftToken = IERC721(aucInfo[_id].token_address);
    //         if(aucInfo[_id].token_address == address(0))
    //         {
    //         nftToken.transferFrom( address(this), aucInfo[_id].highestBidder, aucInfo[_id].id);
    //     //  if(auction.addressErc20Token ==address(0)){
         
    //         IERC20 paymentToken = IERC20(aucInfo[_id].Erc20);
    //          paymentToken.transfer(aucInfo[_id].seller, aucInfo[_id].highestPayableBid);
    ...
    //         // emit NFTClaimed(_id, auction.nftId, msg.sender);
    //     }
}