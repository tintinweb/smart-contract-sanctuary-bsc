...
Buy tax
Tax: 10% (slippage 11%)
4% Payouts in $BTCB, $ETH, $BUSD, $MATIC, $ADA, $BNB & $ULT, more added later
3% Auto buyback 
1% Gas Refunds 
1% Marketing in $Bnb
0.6% admin 
...
Payouts
→ all the sellers tax is given back in ULTple tokens → no $ULT price dump from selling the sell tax
Out of the seling tax:
50% holders in $BTCB, $ETH, $ADA, $MATIC, $BNB, $BUSD & $ULT
...
interface DAPP {
    ...
    function ChooseULTplePayoutTokens(uint32 Token1,uint32 Slice1,uint32 Token2,uint32 Slice2,
...
// Ultimate Network contract

contract Ultimate is ERC20, ReentrancyGuard {
    string public constant symbol = "ULT"; //ULT
    string public constant name = "Ultimate"; //Ultimate Network
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(address(this), "ULT"); //this contract token
  ...
  function UpdateRegister(address recipient, bool ExcludedfromTax) private
   {
        ...
        _AutoPayoutEnabled[recipient] = true; // only human holders get payed auto rewards as default
         ...
         _RewardTokens[recipient].Slice1 = 25; //25% ULT
...
function checkRewards(address addr) public view returns (uint256)
{
   return _earnings[addr];
        ...
        if(LotteryPot > 0)
        {
             if(random(100) < 5) //won - buyamount ULTplied 1/20 probability
