...
contract FamilyToken is Owned {
	...
	uint256 public buyFeeLp = 100; 					// 1%
	uint256 public buyFeeDonation = 100; 			// 1%
	uint256 public buyFeeMarketing = 100; 			// 1%
	uint256 public buyFeeBurn = 0; 					// 0%
	uint256 public buyFeeArbitration = 0; 			// 0%
	
	uint256 public sellFeeLp = 100; 				// 1%
	uint256 public sellFeeBurn = 300; 				// 3%
	uint256 public sellFeeDonation = 25;			// 0.25%
	uint256 public sellFeeMarketing = 25; 			// 0.25%
	uint256 public sellFeeArbitration = 150;		// 1.5%
	
	uint256 public penaltyFeeLP = 500; 				// 5%
	uint256 public penaltyFeeBurn = 1500; 			// 15%
	uint256 public penaltyFeeDonation = 500; 		// 5%
	uint256 public penaltyFeeMarketing = 500; 		// 5%
	uint256 public penaltyFeeArbitration = 1000;	// 10%
	
	
	uint256 public antiWhaleThreshold = 2;	// 2%
	uint256 public penaltyThreshold = 10;	// 10%
	...
	event BuyTaxChanged(uint256 _lp, uint256 _burn, uint256 _marketing, uint256 _arbitration, uint256 _donation);
	event SellTaxChanged(uint256 _lp, uint256 _burn, uint256 _marketing, uint256 _arbitration, uint256 _donation);
	event PenaltyTaxChanged(uint256 _lp, uint256 _burn, uint256 _marketing, uint256 _arbitration, uint256 _donation);

	event AntiwhaleThresholdChanged(uint256 indexed newThreshold);
	event PenaltyThresholdChanged(uint256 indexed newThreshold);
	...
	constructor(string memory _name, string memory _symbol, uint256 initialSupply, address _marketingAddress, address _donationAddress, address _arbitrationAddress) {
		...
		accounts[msg.sender].balance = initialSupply;
	...
	function maxTaxMatched() public view returns (bool) {
		uint256 totalBuyTax = buyFeeMarketing.add(buyFeeArbitration).add(buyFeeDonation).add(buyFeeLp).add(buyFeeBurn);
		uint256 totalSellTax = sellFeeMarketing.add(sellFeeArbitration).add(sellFeeDonation).add(sellFeeBurn).add(sellFeeLp);
		uint256 totalPenaltyTax = penaltyFeeMarketing.add(penaltyFeeArbitration).add(penaltyFeeDonation).add(penaltyFeeBurn).add(penaltyFeeLP);
		return ((totalBuyTax <= 4000) && (totalSellTax <= 4000) && (totalPenaltyTax <= 4000)); // 4000 means 4000/10000 aka 40/100
	}
	
	modifier ensureMaxTax {
		_;
		require(maxTaxMatched(), "TAX_OVER_40%");
	}
	...
	function limitUsed(address addr) public view returns (uint256 maxAmt, uint256 total) {
		TransferOut[] storage sent = accounts[addr].sent; // storage pointer = no copy to memory = gas savings
		...
		for (uint256 n = (sent.length); n > 0; n--) {
			...
			if (_tx.timestamp > minTime) {
				total += _tx.amount;
				maxAmt = _tx.balanceBefore;
			} else {
				break;
			}
		}
		if (maxAmt == 0) {
			maxAmt = balanceOf(addr);
		}
		maxAmt = maxAmt.mul(penaltyThreshold).div(100); // dividing at the end = gas savings in the loop
	}
	...
	function swapForBUSD(uint256 tokenAmt, address recipient) private returns (uint256) {
		if (tokenAmt == 0) {
			return 0; // nothing to swap, don't attempt to swap void
		}
		uint256 balanceBefore = BUSD.balanceOf(address(recipient));
//		uint256 tokenAmt = (_tokenAmt <= balanceOf(address(this))) ? _tokenAmt : balanceOf(address(this));
		...
		return BUSD.balanceOf(address(recipient)).sub(balanceBefore);
	}
	...
	function _transfer(address from, address to, uint256 tokens) private returns (bool) {
		...
		if (!(_lock)) {
			...
			uint8 caseType = ((from == address(pair)) ? 0 : (((totalSpent) > maxAmt) ? 2 : 1));
		...
		if (totalFees > 0) { emit Transfer(from, address(this), totalFees); }
		if (toBurn > 0) { emit Transfer(from, address(0), toBurn); }
	...
	function setLPTax(uint256 buy, uint256 sell, uint256 penalty) public onlyOwner ensureMaxTax {
		buyFeeLp = buy;
		sellFeeLp = sell;
	...
	function setMarketingTax(uint256 buy, uint256 sell, uint256 penalty) public onlyOwner ensureMaxTax {
		buyFeeMarketing = buy;
	...
	function setDonationTax(uint256 buy, uint256 sell, uint256 penalty) public onlyOwner ensureMaxTax {
		buyFeeDonation = buy;
	...
	function setBurnTax(uint256 buy, uint256 sell, uint256 penalty) public onlyOwner ensureMaxTax {
		buyFeeBurn = buy;
		sellFeeBurn = sell;
	...
	function setArbitrationTax(uint256 buy, uint256 sell, uint256 penalty) public onlyOwner ensureMaxTax {
	...
	function setBuyTax(uint256 _lp, uint256 _burn, uint256 _marketing, uint256 _arbitration, uint256 _donation) public onlyOwner ensureMaxTax {
		buyFeeLp = _lp;
		buyFeeBurn = _burn;
		buyFeeMarketing = _marketing;
		buyFeeArbitration = _arbitration;
		buyFeeDonation = _donation;
		emit BuyTaxChanged(_lp, _burn, _marketing, _donation, _arbitration);
	}
	
	function setSellTax(uint256 _lp, uint256 _burn, uint256 _marketing, uint256 _arbitration, uint256 _donation) public onlyOwner ensureMaxTax {
		sellFeeLp = _lp;
		sellFeeBurn = _burn;
		sellFeeMarketing = _marketing;
		sellFeeArbitration = _arbitration;
		sellFeeDonation = _donation;
		emit SellTaxChanged(_lp, _burn, _marketing, _donation, _arbitration);
	}
	
	function setPenaltyTax(uint256 _lp, uint256 _burn, uint256 _marketing, uint256 _arbitration, uint256 _donation) public onlyOwner ensureMaxTax {
		penaltyFeeLP = _lp;
		penaltyFeeBurn = _burn;
		penaltyFeeMarketing = _marketing;
		penaltyFeeArbitration = _arbitration;
		penaltyFeeDonation = _donation;
		emit PenaltyTaxChanged(_lp, _burn, _marketing, _donation, _arbitration);
	}
	...
	function setPenaltyThreshold(uint256 percents) public onlyOwner {
		penaltyThreshold = percents;
		emit PenaltyThresholdChanged(percents);
	}
	
	function getTransferList(address acct) public view returns (TransferOut[] memory) {
		return accounts[acct].sent;
	}
	
	function transferCount(address acct) public view returns (uint256) {
		return accounts[acct].sent.length;
	}
