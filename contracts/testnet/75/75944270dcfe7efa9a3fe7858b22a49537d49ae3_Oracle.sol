...
contract Oracle {
    ...
    function getTokenPrice0(address pairAddress, uint amount) public view returns (uint256 price_) {
        ...
        return((amount*Res0)/Res1); // Returns amount of token0 needed to buy token1 (returns token)
    }
    ...
    function getBusdToBnbToTokenPrice2(address tokenBnbPairAddress, address bnbBusdPairAddress, uint amount) 
        public view returns (uint256 bnbPrice_, uint256 tokenPrice_) {
        uint amountPriceAsBnb = getTokenPrice1(bnbBusdPairAddress, amount);
        uint tokenPriceAsToken = getTokenPrice1(tokenBnbPairAddress, amountPriceAsBnb);
        return (amountPriceAsBnb, tokenPriceAsToken);
    }
    function getBusdToBnbToTokenPrice3(address tokenBnbPairAddress, address bnbBusdPairAddress, uint amount) 
        public view returns (uint256 bnbPrice_, uint256 tokenPrice_) {
        uint amountPriceAsBnb = getTokenPrice0(bnbBusdPairAddress, amount);
        uint tokenPriceAsToken = getTokenPrice1(tokenBnbPairAddress, amountPriceAsBnb);
        return (amountPriceAsBnb, tokenPriceAsToken);
    }
