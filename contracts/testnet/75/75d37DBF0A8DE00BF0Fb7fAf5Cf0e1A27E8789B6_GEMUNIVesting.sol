...
contract GEMUNIVesting is  PermissionGroup, IGEMUNIVesting {
    ...
    mapping(address => mapping(uint => VestingInfo)) public vestingInfo;
    mapping(address => mapping(uint => mapping(address => LockInfo))) public lockInfo;


    event Lock(address locker, uint pid, address token, address user, uint256 amount, uint unlockAt);
    event Unlock(address locker, uint pid, address token, address user, uint256 amount);

    event SetVestingInfo(address locker, uint pid, uint limitVesting, uint vestingTime);
    event EmergencyWithdraw(address token, uint _amount, address admin, address _to);
    event SetFarming(address newFarming);
    event SetStaking(address newStaking);

    constructor(address _farming, address _staking) {
        farming = _farming;
        staking = _staking;
    }

    function setVestingTime(address locker, uint pid, uint _vestingTime, uint _limit) public onlyOperator {
        require (locker == farming || locker == staking, "Lock: invalid locker");
        if( locker == farming ) {
            require(pid < IGEMUNIFarming(farming).poolLength(), "Lock: invalid pid");
        } else {
            require(pid < IGEMUNIStaking(staking).poolLength(), "Lock: invalid pid");
        }
        require(_vestingTime > 0, "Farm: invalid time");
        require(_limit > 0, "Lock: invalid limit");
        
        VestingInfo storage vestInfo = vestingInfo[locker][pid];
        vestInfo.vestingTime = _vestingTime;
        vestInfo.limitVesting = _limit;

        emit SetVestingInfo(locker, pid, _vestingTime, _limit);
    }

    function setFarming(address _farming) public onlyOwner {
        require(_farming != address(0), "Lock: invalid address");
        farming = _farming;
        emit SetFarming(_farming);
    }

    function setStaking(address _staking) public onlyOwner {
        require(_staking != address(0), "Lock: invalid address");
        staking = _staking;
        emit SetStaking(_staking);
    }

    function unlock(address locker, uint pid) public override {
        
        LockInfo storage lockInfo = lockInfo[locker][pid][msg.sender];
        VestingInfo memory vestInfo = vestingInfo[locker][pid];

        require (lockInfo.amount > 0, "Lock: not have locked reward");
        require ( block.timestamp >= lockInfo.deadline, "Lock: not unlockable yet");
        
        uint unlockAmount = lockInfo.amount < vestInfo.limitVesting ? lockInfo.amount : vestInfo.limitVesting;

        IERC20(lockInfo.token).safeTransfer(msg.sender, unlockAmount);

        lockInfo.amount -= unlockAmount;

        lockInfo.deadline = lockInfo.amount == 0 ? 0 : lockInfo.deadline + vestInfo.vestingTime;

        if (lockInfo.amount > 0) {
            emit Lock(locker, pid, lockInfo.token, msg.sender, lockInfo.amount, lockInfo.deadline);
        }
        emit Unlock(locker, pid, lockInfo.token, msg.sender, lockInfo.amount);
    }
    ...
    function lock(
        ...
        if (_amount > 0) {
            ...
            LockInfo storage lockInfo = lockInfo[locker][pid][_addr];
            VestingInfo memory vestInfo = vestingInfo[locker][pid];

            lockInfo.token = _token;
            lockInfo.amount += _amount;
            lockInfo.deadline = block.timestamp + vestInfo.vestingTime;

            emit Lock(locker, pid, _token, _addr, lockInfo.amount, lockInfo.deadline);
        }
    }

    function emergencyWithdraw (address token, uint _amount, address _to) external onlyOwner {
        require(_amount <= IERC20(token).balanceOf(msg.sender), "Lock: not enough balance");
        IERC20(token).safeTransfer(_to, _amount);
        emit EmergencyWithdraw(token, _amount, msg.sender, _to);
    }
