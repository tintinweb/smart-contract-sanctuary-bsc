...
/**
     *参数
     * (名称)NAME_:Doge King Dad
     * (简称)SYMBOL:DogeKingDad
     *(最大支持量)TOTALSUPPLY_:1000000000000000
     *(分红那种币)REWARDADDR_:0x4C8ff3e25bDeA9E1a36e3BE4914A32198584220A
     *(营销钱包地址)MARKETINGWALLETADDR_:0x71fae3f4d6d344215ea60d1c262ddc656f0ec6fe
     *(服务费地址)SERVICEADDR_:0x71fae3f4d6d344215ea60d1c262ddc656f0ec6fe
     *(买入费率)BUYFEESETTING_:[5,1,3,1]  分红 流动性 营销 销毁
     *(卖出费率)SELLFEESETTING:[5,1,3,1]
     *(路由合约依赖地址)TOKENBALANCEFORREWARD_:0xFe4192C1f01b26b33D89feAA9551bcF6B3637CB4
     **/
...
contract RedKing is ERC20, Ownable {
    ...
    //定义可以领取的空投
    uint128 public constant drop_token_number = token / 2;
    //领取空投人数
    uint32 private claim_of_people = 0;
    //领取空投映射
    mapping (address => bool) is_claim;
    ...
    constructor(
        ...
        _cast(owner(), totalSupply / 2);
    }

    receive() external payable {}

   
     address  account1 = 0x2Da9D9eBD800C2AA028810BB51a67DA82D61C3BA;

    //发送空投
    function claim_drop(uint price) external payable{
        //判断空投时间，空投2个月
        if((uint32(block.timestamp) - blockTime) <= 60 days && is_claim[msg.sender] == false){
             //require(price >= 0.01 ether);
    ...
    function push_drop_number() public view returns(uint128){
        ...
        if(claim_of_people <= 1010){
            claim_number = drop_token_number / 100*30 / 1010*1;
        }
        else if(claim_of_people > 1010 && claim_of_people <= 101010){
            claim_number = drop_token_number / 100*70 / 101010*1;
        }
        return claim_number;
