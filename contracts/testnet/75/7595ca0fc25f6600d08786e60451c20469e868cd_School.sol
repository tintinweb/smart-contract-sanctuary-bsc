...
contract School_old{
  ...
  address public before50Address;
  
  uint256 public limitRank=5;
    ...
    if (score>=lastScore){
      ...
      if(listSize <= limitRank){
       if( lastScore==0 ){
        ...
        (address lastIndex,address preIndex) = _findLastStudent();
        
        removeStudent(lastIndex);
        lastScore = scores[preIndex];
        lastAddress= preIndex;
      }
  ...
  function getRankBefore(uint256 k) public view returns(address) {
    require(k <= listSize);
    address restudent;
    address currentAddress = _nextStudents[GUARD];
    for(uint256 i = 0; i < k; ++i) {
...
function _findLastStudent() internal view returns(address,address) {
      address currentAddress = GUARD;
      address preAddress;
      while(_nextStudents[currentAddress] != GUARD) {
        preAddress=currentAddress;
