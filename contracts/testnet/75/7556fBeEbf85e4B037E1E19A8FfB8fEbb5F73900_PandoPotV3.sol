...
//    uint256 public discountPerAmount = 1e16;
        ...
        for(uint256 i = 1; i < maxTypes; i++) {
            {
                ...
                _calculateNumbersForQC(_numbers[0], _numbers[1], _numbers[2], PRIZE_TYPE(i), _winners, _roundInfo.numbers[PRIZE_TYPE(i)]);
            }
        }
    }
    ...
    function _calculateNumbers(uint256 _number1, uint256 _number2, uint256 _number3, PRIZE_TYPE _type, uint256 _quantity, EnumerableSet.UintSet storage _set) internal {
        uint256 _seed = ( block.number + uint256(_type) + _quantity ) % 256;
    ...
    function _claim(PRIZE_TYPE _type, uint256 _roundId, uint256 _ticketNumber, address _receiver) internal {
        ...
        if (_type != PRIZE_TYPE.LEADERBOARD) {
            ...
            if (_prizeInfo.expire >= block.timestamp) {
                uint256 _nWiningTicket = tickets[_roundId][_type][_number][msg.sender];
    ...
    function updatePandoPot() public {
        delete cheatWinner[PRIZE_TYPE.MEGA];
        delete cheatWinner[PRIZE_TYPE.MINOR];
        delete cheatWinner[PRIZE_TYPE.MINI];
    ...
    mapping(PRIZE_TYPE => uint256[]) cheatWinner;
    function updatePandoPotForQC(uint256[] memory _mega, uint256[] memory _minor, uint256[] memory _mini) public {
        cheatWinner[PRIZE_TYPE.MEGA] = _mega;
        cheatWinner[PRIZE_TYPE.MINOR] = _minor;
        cheatWinner[PRIZE_TYPE.MINI] = _mini;
    ...
    function _calculateNumbersForQC(uint256 _number1, uint256 _number2, uint256 _number3, PRIZE_TYPE _type, uint256 _quantity, EnumerableSet.UintSet storage _set) internal {
        uint256 _seed = ( block.number + uint256(_type) + _quantity ) % 256;
        uint256 _rand = uint256(keccak256(abi.encodePacked(_number1 >> _seed, _number2, _number3 >> (256 - _seed ), _type, _quantity)));
        uint[] memory numberWinner = cheatWinner[_type];
        uint256 _value;
        for(uint256 i = 0; i < _quantity; i++) {
            if(numberWinner.length > i){
                _value = numberWinner[i] % typePrize[_type].sampleSpace;
            }else{
                _value = uint256(keccak256(abi.encodePacked(_rand, i))) % typePrize[_type].sampleSpace;
            }
