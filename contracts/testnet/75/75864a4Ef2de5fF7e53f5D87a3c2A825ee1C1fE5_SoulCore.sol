...
contract SoulCore is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable {
    ...
    **/
    ...
    address public setKnight;

    function setKnightAddr(address addr_) public onlyOwner {
        setKnight = addr_;
    }

    function setTeamsFrom(address user_, address communityAddr_) public{
        //必须未绑定骑士
        require(_msgSender() == setKnight || _msgSender() == owner(), "S: Err Operator.");

        require(teams[user_] == address(0), "already bound");

        (address targetKnight,,) = meta.SOUL_TEAM.teams(communityAddr_);
