...
contract HODLSpeacialPool is Ownable, ERC721Holder {
    using SafeMath for uint256;
    ...
    //Caution testnet
    IPancakeRouter private pancakeRouter = IPancakeRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    address private BUSDaddress = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    function getUserInfo(address _user) public view returns(
        ...
        uint256 currentRate = poolInfo.lpToken.getRate();
    ...
    function pendingReward(address _user) external view returns (uint256) {
        UserInfo memory user = userInfo[_user];
        user.amount = user.amount * user.rate / poolInfo.lpToken.getRate();
        uint256 accHandPerShare = poolInfo.accHandPerShare;
    ...
    function updatePool() private {
        ...
        uint256 multiplier = getMultiplier(poolInfo.lastRewardBlock, block.number);
        uint256 handReward = multiplier.mul(handPerBlock);
        poolInfo.accHandPerShare = poolInfo.accHandPerShare.add(handReward.mul(1e12).div(lpSupply));
        poolInfo.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public payable {
        ...
        if (user.amount > 0) {
            ...
            user.earnedReward = user.amount.mul(poolInfo.accHandPerShare).div(1e12).add(user.earnedReward).sub(user.rewardDebt);
        }

        poolInfo.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rate = poolInfo.lpToken.getRate();
       
        user.rewardDebt = user.amount.mul(poolInfo.accHandPerShare).div(1e12);        
    ...
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        uint256 pending = user.amount.mul(poolInfo.accHandPerShare).div(1e12).add(user.earnedReward).sub(user.rewardDebt);
        ...
        user.rewardDebt = user.amount.mul(poolInfo.accHandPerShare).div(1e12);  // error when withdraw
    ...
    //For testing
    function getRestBNB() public view returns(uint256) {
