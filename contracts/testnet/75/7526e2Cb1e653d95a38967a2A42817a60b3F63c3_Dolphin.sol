...
contract DividendPayingToken is ERC20, Ownable {
  ...
  address public Divtoken;

  uint256 internal magnifiedDividendPerShareDivtoken;
  mapping(address => int256) internal magnifiedDividendCorrectionsDivtoken;
  mapping(address => uint256) internal withdrawnDividendsDivtoken;
  uint256 public totalDividendsDistributedDivtoken;

  constructor(string memory _name, string memory _symbol,  address _Divtoken) ERC20(_name, _symbol) {
        Divtoken = _Divtoken;
  }

  function distributeCAKEDividends( uint256 Divtoken_amount) public onlyOwner{
    require(totalSupply() > 0);
    if (Divtoken_amount > 0) {
      magnifiedDividendPerShareDivtoken = magnifiedDividendPerShareDivtoken.add(
        (Divtoken_amount).mul(magnitude) / totalSupply()
      );
      totalDividendsDistributedDivtoken = totalDividendsDistributedDivtoken.add(Divtoken_amount);
    }
  }
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (bool) {
    uint256 _withdrawableDividendDivtoken = withdrawableDividendOfDivtoken(user);
    if (_withdrawableDividendDivtoken > 0) {
      withdrawnDividendsDivtoken[user] = withdrawnDividendsDivtoken[user].add(_withdrawableDividendDivtoken);
      bool success = IERC20(Divtoken).transfer(user, _withdrawableDividendDivtoken);
      if(!success) {
        withdrawnDividendsDivtoken[user] = withdrawnDividendsDivtoken[user].sub(_withdrawableDividendDivtoken);
      }
    }

    return  _withdrawableDividendDivtoken > 0;
  }
  ...
  function dividendOfDivtoken(address _owner) public view returns(uint256) {
    return withdrawableDividendOfDivtoken(_owner);
  }

  /// @notice View the amount of dividend in wei that an address can withdraw.
  /// @param _owner The address of a token holder.
  /// @return The amount of dividend in wei that `_owner` can withdraw.
  function withdrawableDividendOfDivtoken(address _owner) public view returns(uint256) {
    return accumulativeDividendOfDivtoken(_owner).sub(withdrawnDividendsDivtoken[_owner]);
  }
  ...
  function withdrawnDividendOfDivtoken(address _owner) public view returns(uint256) {
    return withdrawnDividendsDivtoken[_owner];
  }
  ...
  /// @return The amount of dividend in wei that `_owner` has earned in total.
    ...
    function accumulativeDividendOfDivtoken(address _owner) public view returns(uint256) {
    return magnifiedDividendPerShareDivtoken.mul(balanceOf(_owner)).toInt256Safe()
      .add(magnifiedDividendCorrectionsDivtoken[_owner]).toUint256Safe() / magnitude;
  }
  ...
  function _transfer(address from, address to, uint256 value) internal virtual override {
    ...
    int256 _magCorrectionDivtoken = magnifiedDividendPerShareDivtoken.mul(value).toInt256Safe();
    magnifiedDividendCorrectionsDivtoken[from] = magnifiedDividendCorrectionsDivtoken[from].add(_magCorrectionDivtoken);
    magnifiedDividendCorrectionsDivtoken[to] = magnifiedDividendCorrectionsDivtoken[to].sub(_magCorrectionDivtoken);
  ...
  function _creation(address account, uint256 value) internal override {
    ...
    magnifiedDividendCorrectionsDivtoken[account] = magnifiedDividendCorrectionsDivtoken[account]
      .sub( (magnifiedDividendPerShareDivtoken.mul(value)).toInt256Safe() );
  }
  ...
  function _burn(address account, uint256 value) internal override {
    ...
    magnifiedDividendCorrectionsDivtoken[account] = magnifiedDividendCorrectionsDivtoken[account]
      .add( (magnifiedDividendPerShareDivtoken.mul(value)).toInt256Safe() );
  }
...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor( address _Divtoken, uint256 _minimumTokenBalanceForDividends) DividendPayingToken("Dividen_Tracker", "Dividen_Tracker",  _Divtoken) {
        claimWait = 3600;
...
contract Dolphin is ERC20, Ownable {
    ...
    string private name_ = "Dolphin";
    string private symbol_ = "Dph";
    uint256 private totalSupply_ = 1000000 * (10**9);
    uint256 private swapTokensAtAmount = 500*10**9;

    uint256 public DivtokenBonusFee = 4;
    uint256 public liquidityFee = 2;
    uint256 public marketingFee = 1;
    uint256 public deadFee = 1;
    uint256 public inviterFee = 3;
    uint256 private transferDivtokenFee = 0;
    uint256 private transferMarketingFee = 0;

    uint256 public maxTxAmount = 10000 * (10**9);
    uint256 public maxHolderAmount = 30000 * (10**9);
    uint256 public intervalTime = 60;
    uint256 public minimumTokenBalanceForDividends = 5000 * (10**9);

    address public Divtoken = 0x55d398326f99059fF775485246999027B3197955; //USDT
    address private ROUTE = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; 
    ...
    constructor(address marketingWalletAddr_) payable ERC20(name_, symbol_)  {
        ...
        dividendTracker = new TokenDividendTracker(Divtoken, minimumTokenBalanceForDividends);
        ...
        isWalletLimitExempt[address(bindAddress)] = true;
        
        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[marketingWalletAddress] = true;
        isTxLimitExempt[address(this)] = true;
        isTxLimitExempt[address(bindAddress)] = true;
    ...
    function setDivtokenBonusFee(uint256 amount) public onlyOwner {
        DivtokenBonusFee = amount;
    }
    ...
    function setTransferDivtokenFee(uint256 amount) public onlyOwner {
        transferDivtokenFee = amount;
    }
    ...
    function _transfer(
       ...
       if(!isTxLimitExempt[from] && !isTxLimitExempt[to]) {                                      
        ...
        if( !swapping && !automatedMarketMakerPairs[from] && from != owner() && to != owner() ) {
            swapping = true;
            if( AmountDivtokenBonusCurrent >= swapTokensAtAmount && dividendTracker.totalSupply() > 0) swapAndSendDividends();
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                _InviterFee  = amount.mul(inviterFee).div(100);
                _DivtokenFee = amount.mul(DivtokenBonusFee).div(100);
                lastBuyTime[to] = block.timestamp;
                finalAmount = finalAmount.sub(_LiquidityFee).sub(_MarketingFee).sub(_DeadFee).sub(_InviterFee).sub(_DivtokenFee);
                ...
                _DeadFee = amount.mul(deadFee).div(100);
                _InviterFee  = amount.mul(inviterFee).div(100);
                finalAmount = finalAmount.sub(_LiquidityFee).sub(_DivtokenFee).sub(_MarketingFee).sub(_InviterFee).sub(_DeadFee);
            }else{
                _DivtokenFee = amount.mul(transferDivtokenFee).div(100);
                _MarketingFee = amount.mul(transferMarketingFee).div(100);
                finalAmount = finalAmount.sub(_DivtokenFee).sub(_MarketingFee);
            ...
            if(_DivtokenFee > 0){
                super._transfer(from, address(this), _DivtokenFee);
                AmountDivtokenBonusCurrent = AmountDivtokenBonusCurrent.add(_DivtokenFee);
            }
        }
        ...
        if(checkWalletLimit && !isWalletLimitExempt[to])
                require(balanceOf(to).add(finalAmount) <= maxHolderAmount);
    ...
    function swapAndSendDividends() private{
        swapTokensForCake(AmountDivtokenBonusCurrent, Divtoken);
        AmountDivtokenBonusCurrent = 0;
        uint256 Divtoken_AMOUNT = IERC20(Divtoken).balanceOf(address(this));
        bool Divtoken_success = IERC20(Divtoken).transfer(address(dividendTracker), Divtoken_AMOUNT);
        if ( Divtoken_success) {
            dividendTracker.distributeCAKEDividends(Divtoken_AMOUNT);
        }
    }
