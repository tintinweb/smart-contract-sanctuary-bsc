...
contract NFTMint is ERC4907, Ownable, ReentrancyGuard {
    //Needed imports
    ...
    mapping(uint256 => string) public tokenMetaHash;
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        if(_exists(_tokenId)) {
    		return string(abi.encodePacked(baseURI, tokenMetaHash[_tokenId]));
        } else {
            return "";
        }
    }
    ...
    function CreateToken(string memory _hash) public payable Mintable returns (uint256){
        tokens.increment();
        MintedPerAddress[msg.sender] += 1;
        _safeMint(msg.sender, tokenId());
        tokenMetaHash[tokenId()] = _hash;
        return tokenId();
    }
    ...
    // widraw the fund from this contract (collecting the fees)
    function widrawFund() external onlyOwner nonReentrant {
