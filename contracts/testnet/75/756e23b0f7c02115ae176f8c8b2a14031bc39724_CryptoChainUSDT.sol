...
contract CryptoChainUSDT {
	...
	event RefBonusMiss(address parentaddress,uint256 reason);
...
//        require((user.deposits.length==0) || (user.deposits[user.deposits.length-1].amount <= tokenAmount),"Amount Should be greater than previous amount");
		...
		if (user.referrer == address(0)) 
		{
			...
			for (uint256 i = 0; i < 10; i++) {
				if ((upline != address(0))&& (BASE_CONDITION_AMOUNT[i] <= user.directbusiness) && (BASE_CONDITION_LEVEL[i] <= users[upline].levels[0]))
				{
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 10; i++) {
				...
				{
					emit RefBonusMiss(upline,0);
					break;
				}
				else if(BASE_CONDITION_AMOUNT[i] > user.directbusiness)
				{
					emit RefBonusMiss(upline,1);
				}
				else if(BASE_CONDITION_LEVEL[i] > users[upline].levels[0])
				{
					emit RefBonusMiss(upline,2);
				}
			}
		}
