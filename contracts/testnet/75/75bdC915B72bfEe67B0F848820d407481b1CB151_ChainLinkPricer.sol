...
contract ChainLinkPricer {
    ...
    function next(address pricer, uint256 chainId) public view returns (uint80) {
        return AggregatorProxy(pricer).next(chainId);
    }

    function prev(address pricer, uint256 chainId) public view returns (uint80) {
        return AggregatorProxy(pricer).prev(chainId);
    }
