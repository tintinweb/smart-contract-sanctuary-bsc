...
contract BasicToken is ERC20, ERC20Burnable, Pausable, Ownable, ReentrancyGuard {
    ...
    whenNotPaused
    {
        ...
        if (
            ...
            && sender != owner()                // is sender not the owner?
            && recipient != owner()             // is sender not the owner?
            ...
            for (uint8 i = 0; i < feeRecipientAddresses.length; i++) {// Note uint8: we only support 256 feeRecipients
                ...
                if (feeRecipient.exists) {
                    ...
                    if (fee > 0) {// Fee can be 0 when amount is smaller than 10000 wei
                        ...
                        // Buy-back - pay fees in BNB; same logic as the block above, but 
            ...
            if (liquidityFee > 0) {
                ...
                // Deposit in liquidity pool
                // First check if we have reached the minimum amount of tokens to deposit
                uint256 tokenBalance = balanceOf(address(this));

                if (
                    tokenBalance >= minLPDeposit    // enough tokens to start adding liquidity
                    // TODO - wrong. this is a buy, and fees should apply
                    && sender != address(pancakePair) // this is not during a pancake.removeLiquidity
                ) {
                    addLiquidity(tokenBalance);
                }
            }
        }
    ...
    function addLiquidity(uint256 amount) private doNotApplyFees {
        ...
        pancakeRouter.addLiquidityETH{value: bnbAmount}(
            address(this), (amount - half), 0, 0, owner(),
            block.timestamp
        );
    }

    // TODO optional suggestion
    // Code a function that allows a fee recipient to withdraw the amountToSwapDeposit
    // Then feeRecipients can withdraw without the owner which increases trust
    // It will also save the owner on gas fees
    // Iterates through the feeRecipient deposits, swaps token using the Pancake Router, then pays BNBs to the recipient address
