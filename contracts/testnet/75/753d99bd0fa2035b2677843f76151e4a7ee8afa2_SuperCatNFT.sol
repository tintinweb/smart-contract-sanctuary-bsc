...
contract SuperCatNFT is ERC721 {
    ...
    uint256 public constant NFT_TotalSupply = 8760000;
    
    uint256 public NFT_Id = 0;
    uint256 public UserMintNFT_Id = 8760000;
    ...
    event OfficalMint(address indexed origin, address indexed owner,string iphshash, uint256 TokenId);
    ...
    constructor() ERC721("Chan Kwong Yan", "SC1") {
    ...
    function setOfficeAddress(address _OfficalAddress,address _PromoteAddress,address _funder) public onlyDev {
    ...
    function mintinternal(address origin, address to, string  memory ipfsHash,uint256 stampFee,bool isOffcial) internal {
        if(isOffcial){
            NFT_Id++;
    ...
    function officalMint(address _to, string memory _hash) external returns(uint256){//officalMint
        require(devOwner[msg.sender]==true,' not owner');
        require(paused == false, "offical mint is paused");
        mintinternal(address(this),_to,_hash,50,true);
        emit OfficalMint(address(this),_to, _hash, NFT_Id);
        return NFT_Id;
    }
