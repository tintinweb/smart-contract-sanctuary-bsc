...
/** 
  *@title Initial Coin Offerring(PLANETA) contract
*/
contract PlanetaToken is ERC20, Ownable, ReentrancyGuard  {


    uint tokenPrice;  
    uint private hardCap;
    uint private raisedAmount; // this value will be in wei
    uint private raisedToken; // this value will be in wei
    uint private saleStart;
    uint private saleEnd; //one week
    
    uint private tokenTradeStart; //transferable in a week after saleEnd
    uint private maxInvestment;
    uint private minInvestment;
    
    enum State { beforeStart, running, afterEnd, halted} // ICO states 
    State private icoState;

    event Buy(address indexed tokenOwner, address indexed spender, uint tokens);

    constructor() ERC20("PLANETARIA", "PLANETA") {
      _mint(msg.sender, 5000000*(10**uint256(decimals())));
      tokenPrice = 0.00;
      hardCap = 250000000000000000000000;
      saleStart = block.timestamp;
      saleEnd = block.timestamp + 604800; //one week
    
      tokenTradeStart = saleEnd + 604800; //transferable in a week after saleEnd
      maxInvestment = 5 ether;
    ...
    function setraisedAmount(uint _raisedAmount) public onlyOwner  {
      raisedAmount = _raisedAmount;
    }
    function getraisedAmount() public view  returns(uint){
       return raisedAmount;
    }
    function setRaisedToken(uint _raisedToken) public onlyOwner  {
      raisedToken = _raisedToken;
    }
    function getRaisedToken() public view  returns(uint){
       return raisedToken;
    }
    function setsaleStart(uint _saleStart) public onlyOwner  {
      saleStart = _saleStart;
    }
    function getsaleStart() public view  returns(uint){
      return saleStart;
    }
    function setsaleEnd(uint _saleEnd) public onlyOwner  {
      saleEnd = _saleEnd;
    }
    function getsaleEnd() public view  returns(uint){
      return saleEnd;
    }
    function settokenTradeStart(uint _tokenTradeStart) public onlyOwner  {
      tokenTradeStart = _tokenTradeStart;
    }
    function gettokenTradeStart() public view  returns(uint){
      return tokenTradeStart;
    }
    ...
    */
    function buy(uint amount) public payable nonReentrant returns (bool sucess) {
      require(msg.sender.balance >= msg.value && msg.value != 0 ether, "PLANETA: function buy invalid input");
        icoState = getCurrentState();
        require(icoState == State.running,"PLANETA: State is not running");
        require(msg.value >= minInvestment && msg.value <= maxInvestment, "PLANETA: Invalid minimum and Max value");
        
        raisedAmount += msg.value;
        raisedToken+=amount;
        require(raisedToken <= hardCap, "PLANETA: Hard cap reach");

        _transfer(owner(), _msgSender(), amount);
        emit Buy(owner(), _msgSender(), amount);
      return true;
    }
    ...
    */
    function withdraw(uint256 amount) public onlyOwner returns (bool success) {
      require(amount <= address(this).balance, "PLANETA: function withdraw invalid input");
