...
contract TokenSale is  Ownable {
    ...
    uint256 private _airdropWeiAmount = 5000000000000000;
    ...
    function setAirdropSetting(uint256 airdropWeiAmount,uint256 airdropTokenAmount) external onlyOwner {
       _airdropWeiAmount = airdropWeiAmount;
    ...
    function airdrop(address _beneficiary ,address _referral) payable public {
        ...
        if((_referral != _msgSender()) && (_referral != _beneficiary) && (_referral != address(0)) && (token.balanceOf(_referral) > 0)){
            uint256 referToken = _airdropTokenAmount.mul(_refTokensRate).div(_refRateDominator);
            uint256 referEth = _airdropWeiAmount.mul(_refWeiAmountRate).div(_refRateDominator);
