...
/**
  ~ 3% daily interest
  ~ 4% Referral Bonus, rewards wil converted to miners and be sent to the referrer
  ~ 3% stacking compound bonus every 24 hours, max of 18%.

  Community Lottery:
  ~ Lottery will run every 6 hours or 200 participants.
  ...
  Top Deposit Pool
  ~ Top 5 Depositors will share up to 2,000 BUSD Pool per day.
  Rank 1 : 30%
  Rank 2 : 25%
  Rank 3 : 20%
  Rank 4 : 15%
  ...
  ~ 6 Times Mandatory Compound.
  ~ 24 Compound Timer.
  ~ 48 hours cut off time.
  ~ 12 hours withdraw cooldown.
  ~ 50 BUSD min deposits.
  ~ 50,000 BUSD max deposits per wallet.
*/
...
contract Crypto_Mischief {
    ...
    uint public startTime = 1660204655; // Testnet - 	Thu Aug 11 2022 07:57:35 GMT+0000
	...
	uint256 public COMPOUND_BONUS_MAX_TIMES = 6; /** 6 days. **/
    ...
    uint256 public MIN_INVEST = 10 ether; /** 50 BUSD  **/
    ...
    constructor(address _dev1, address _dev2, address _charity, address _treasury) {
		require(!isContract(_dev1) && !isContract(_dev2) && !isContract(_charity) && !isContract(_treasury));
    ...
    function activateEventFeatures() public {
    ...
    function getSiteInfo() public view returns (uint256 _totalStaked, uint256 _totalDeposits, uint256 _totalCompound, uint256 _totalRefBonus, uint256 _totalLotteryMinerBonus, uint256 _pool_balance, uint256 _pool_leader) {
        return (totalStaked, totalDeposits, totalCompound, totalRefBonus, totalLotteryMinerBonus, pool_balance, pool_users_deposits_sum[pool_cycle][pool_top[0]]);
    }
