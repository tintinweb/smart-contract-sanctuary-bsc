...
// File @openzeppelin/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.[email protected]
...
contract BombArmory is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for ERC20Upgradeable;

    IToken public lp_BOMBBTCB;
    address erctoken;
    ...
    uint256 public MARKET_EGGS_DIVISOR; // 50%
    uint256 public MARKET_EGGS_DIVISOR_SELL; // 100%

    uint256 public MIN_INVEST_LIMIT; /** 10 BUSD  **/
    uint256 public WALLET_DEPOSIT_LIMIT; /** 10000 BUSD  **/

    uint256 public COMPOUND_BONUS; /** 2.5% **/
    uint256 public COMPOUND_BONUS_MAX_TIMES; /** 10 times / 5 days. **/
    uint256 public COMPOUND_STEP; /** every 12 hours. **/

    uint256 public WITHDRAWAL_TAX;
    uint256 public COMPOUND_FOR_NO_TAX_WITHDRAWAL; // compound days, for no tax withdrawal.
    ...
    uint256 public WITHDRAW_COOLDOWN; /** 4 hours  **/
    ...
    function initialize() public initializer {
        ...
        prize = msg.sender;
        erctoken = 0xf3aeCEAb84E23EAcA00B60EdB1E7a2F929473a7f; /** BUSD Testnet **/
        // erctoken = 0x84392649eb0bC1c1532F2180E58Bae4E1dAbd8D6; /** BUSD Mainnet **/
        EGGS_TO_HIRE_1MINERS = 2592000;
        PERCENTS_DIVIDER = 1000;
        REFERRAL = 80;
        TAX = 100;

        MARKET_EGGS_DIVISOR = 2; // 50%
        MARKET_EGGS_DIVISOR_SELL = 1; // 100%

        MIN_INVEST_LIMIT = 1 * 1e18; /** 10 BUSD  **/
        WALLET_DEPOSIT_LIMIT = 1000 * 1e18; /** 10000 BUSD  **/

        COMPOUND_BONUS = 25; /** 2.5% **/
        COMPOUND_BONUS_MAX_TIMES = 10; /** 10 times / 5 days. **/
        COMPOUND_STEP = 12 * 60 * 60; /** every 12 hours. **/

        WITHDRAWAL_TAX = 400;
        COMPOUND_FOR_NO_TAX_WITHDRAWAL = 4; // compound days, for no tax withdrawal.
        CUTOFF_STEP = 48 * 60 * 60; /** 48 hours  **/
        WITHDRAW_COOLDOWN = 4 * 60 * 60; /** 4 hours  **/
        PSN = 10000;
        PSNH = 5000;
        lp_BOMBBTCB = IToken(erctoken);
    }
    ...
    function sellEggs() public {
        ...
        lp_BOMBBTCB.transfer(msg.sender, eggsPayout);
        user.totalWithdrawn = user.totalWithdrawn.add(eggsPayout);
        totalWithdrawn = totalWithdrawn.add(eggsPayout);
    }

    function buyEggs(address ref, uint256 amount) public {
        _buyEggs(ref, amount, msg.sender);
    }
    ...
    function initializeArmory(uint256 amount) public onlyOwner {
        if (!contractStarted) {
    ...
    function getBalance() public view returns (uint256) {
        return lp_BOMBBTCB.balanceOf(address(this));
    }
    ...
    function CHANGE_PRIZE(address value) external onlyOwner {
        prize = value;
    }
    ...
    function SET_MIN_INVEST_LIMIT(uint256 value) external onlyOwner {
    ...
    function _buyEggs(
        ...
        lp_BOMBBTCB.transferFrom(address(msg.sender), address(this), amount);
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                ...
                lp_BOMBBTCB.transfer(upline, refRewards);
