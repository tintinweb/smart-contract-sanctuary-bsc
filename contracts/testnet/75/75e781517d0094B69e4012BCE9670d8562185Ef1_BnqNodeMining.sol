...
contract BnqNodeMining is OwnableUpgradeable {
    ...
    address public BNQSHARE;
    address public BNQSTAKE;
    address public PAIR;
    address public USDT;
    address public BNQ;
    address public SP;
    ...
    // uint[3] internal linkGen;
    uint[3] internal linkPer;
    // uint[12] internal nodeGear;
    // uint[12] internal nodeOut;
    struct NodeInfo {
        uint nodeGear;
        uint nodeOut;
    }
    ...
    mapping(address => uint) public claimedBNQAmount;
    ...
    function setV5(address[] calldata adrrs_, bool b) public onlyOwner {
        for (uint i; i < adrrs_.length; i++) {
            ...
            v5User[adrrs_[i]].debt = v5Info.debt;
        }
    }
    ...
    function claimLink() external onlyEOA returns (uint) {
        ...
        if (rew > 0) {
            ...
            info.linkReward += rew;
            claimedBNQAmount[msg.sender] += bnqOut;
            linkToClaim[msg.sender] = 0;
    ...
    function _processV5(uint amount_) internal {
        uint diff;
        if (v5Info.v5Number > 0) {
            diff = ((amount_ * 10) / v5Info.v5Number) / 100;
        }
    ...
    function calculateV5(
        ...
        if (v5User[user_].v5) {
            rew = v5Info.debt - v5User[user_].debt;
    ...
    function _calculateRew(
        ...
        if (info.buyAmount == 0) {
            ...
            uint rate = info.buyAmount / 100 / 86400;
            rew =
    ...
    function buyNode(address inv_, uint lv_) external onlyEOA {
        ...
        if (info.invitor == address(0)) {
            ...
            (, uint _t, uint _c) = checkOldAmount(msg.sender);
            if (_t > _c) {
                userInfos[msg.sender].oldQuota = _t - _c;
            }
            nodeTotal += 1;
        }
        // old user
        ...
        if (info.buyAmount == 0) {
            ...
            TOTALQUOTA -= _total;
            TOTALQUOTA += info.thisQuota + info.oldQuota;
        }
        //pancake & burn
    ...
    function claimDynamicRew(
        ...
        if (isV5 && !v5User[msg.sender].v5) {
            ...
            v5User[msg.sender].debt = v5Info.debt;
        }
        // reward
        uint rew = rew_;
        uint bnqOut = getOut(1, rew);
    ...
    function checkOne(
        ...
        _linkList[0] = linkToClaim[addr];

        _linkList[1] = info.linkReward;
        ...
        _nodeList[3] = claimedBNQAmount[addr];
