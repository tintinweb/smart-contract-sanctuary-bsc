...
contract EMETMysteryBoxS1 is ERC721Holder, Ownable, RandomGenerator {
    ...
    struct UserInfo {
        ...
        uint lockTimeSynthetic;
        uint syntheticReward;
    }
    ...
    function initialize(address token_, address nft_) public onlyOwner {
        ...
        // starNFT = IStarNFT(0x927a7f35587BC7E59991CCCdd2D4aDd1f0e7bc66);
        // BVBox = IBox721(0x9fF3230bF5b38E8b909181f947c5d41FfDdB9440);
    ...
    function claim() public returns(uint) {
        ...
        uint temp;
        uint perDirect = (block.timestamp - user.lastClaimTime) * 1e10 / user.lockTimeDirect;
        uint perSynthesis = (block.timestamp - user.lastClaimTime) * 1e10 / user.lockTimeSynthetic;

        temp = (perDirect * user.directReward + perSynthesis * user.syntheticReward) / 1e10;
    ...
    // ----------------------------------  direct claim  ---------------------------------------
    event DirectClaim(address indexed user, uint indexed cardId, uint indexed tokenId);
    function directGetIt(uint tokenId_) public returns(bool) {
        ...
        user.directReward += pending;
        user.lockTimeDirect = directLockTime;

        _process(tokenId_);
        emit DirectClaim(msg.sender, cardId, tokenId_);
        return true;
    }
    ...
    function synthesis(uint[] memory tokenIds_) public returns(bool) {
        ...
        user.lockTimeSynthetic = tempTime;
        user.syntheticReward += pending;
