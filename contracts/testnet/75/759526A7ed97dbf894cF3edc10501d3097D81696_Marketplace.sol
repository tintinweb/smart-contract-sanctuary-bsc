    ...
    IERC721ReceiverUpgradeable
{
    ...
    mapping(address => NFTRoyalty) public royalties;

    /// @dev 10_000 = 100%
    uint64 public constant PERCENTAGE_FRACTION = 10_000;
    /// @dev 100/10_000 = 1%
    ...
    /// @dev Check whether listing specified by listingId is a valid one
    ...
    //////////////////////////////////////////////////////////////*/ 
    /**
     * @notice Listing nft (ERC20 && ERC1155)
     * @param _collection   Address of NFT
     * @param _tokenId      ID of NFT
     * @param _price        listed price
     * @param _token        Address of payment token
     * @param _quantity     Amount of NFT (available for ERC1155) 
     * note _amount is 1 whether NFT is ERC721
     */
    function createMarketItem(
        ...
        uint256 _quantity
    ) external nonReentrant validQuantity(_quantity) {
        require(_price != 0, "Marketplace_CreateMarketItem: Zero Price");

        address seller = _msgSender();
        uint tokenType = _getTokenType(_collection);
        
        currentListingIndex.increment();
        uint listingId = currentListingIndex.current();

        if(tokenType == 721){
            require(_quantity == 1, "Marketplace_CreateMarketItem: ERC721 has only 1 quantity");
        ...
        tokenToId[_tokenId][_collection] = listingId;
     ...
     * @param _listingId        Id of order
     * @param _quantityToBuy    Quanity to buy of NFT (available for NFT)
     */
    function createMarketSale(
        uint256 _listingId,
        uint256 _quantityToBuy
    ) external payable nonReentrant validQuantity(_quantityToBuy) onlyValidListing(_listingId)
    {
        MarketItem storage selectedListing = idToMarketItem[_listingId];
        address buyer = _msgSender();
        
        uint256 totalTxValue = selectedListing.price * _quantityToBuy;

        _processRoyaltyPayment(
            buyer,
        ...
        if (selectedListing.quantity - _quantityToBuy == 0) {
    ...
    /**
     * @notice delist an collection in marketplace
     * @param _listingId    ID of market item
     */
    function delistMarketItem(uint256 _listingId) external nonReentrant onlyListingCreator(_listingId) onlyValidListing(_listingId) {
        address sender = _msgSender();
        MarketItem storage selectedListing = idToMarketItem[_listingId];
        selectedListing.seller = address(0);
        selectedListing.price = 0;
        
        totalListings.decrement();
        
        _processAssetTransfer(_listingId, sender, selectedListing.quantity);
    ...
    function _processAssetTransfer(
        ...
        address _to,
        uint256 _quantityTransfer
    ) internal {
        MarketItem storage selectedListing = idToMarketItem[_selectedListingId];
        require(_quantityTransfer < selectedListing.quantity, "Marketplace: Invalid quantity");

        selectedListing.quantity -= _quantityTransfer;
    ...
    function _processRoyaltyPayment(
        address _buyer,
        address _seller,
        ...
        uint256 marketplaceCut = (_totalValue * marketplaceFee) / PERCENTAGE_FRACTION;
        ...
        _safeTransferFrom(_currency ,_buyer, address(this), marketplaceCut);
        _safeTransferFrom(_currency, _buyer, _seller, _totalValue - marketplaceCut - secondaryRoyalties);
    }
    ...
    function setRoyaltyPercent(
        ...
        NFTRoyalty memory royalty = NFTRoyalty(_owner, _fee);
        royalties[_addressCollection] = royalty;
    }
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        uint256 itemCount = currentListingIndex.current();
        uint256 length = totalListings.current() - totalSold.current();
    ...
    function fetchMarketItem(
        ...
        uint256 listingId = tokenToId[_tokenId][_nftContract];
        return idToMarketItem[listingId];
    }

    function getTokenType(address _assetContract) external view returns(string memory tokenType){
        uint256 _tokenType = _getTokenType(_assetContract);
        if(_tokenType == 721) {
            tokenType =  "ERC_721";
        } else {
            tokenType = "ERC_1155";
