...
/**
 * @title DogWarGame
 * https://shibarmybsc.com/
 */
contract DogWarGame is Ownable, Recoverable, Pausable, ReentrancyGuard {
    ...
    mapping(uint256 => bool) private _claimedRounds;

    event GameStarted(address indexed account, uint256 roundId);
    event Claimed(address indexed account, uint256 roundId, uint256 amount);
    ...
    function _getNextRoundId() internal returns (uint256) {
        _roundId.increment();
    ...
    function _verifySignature(
        ...
        bytes32 digest = ECDSA.toEthSignedMessageHash(keccak256(abi.encode(sender, roundId, amount)));

        (address recovered, ECDSA.RecoverError error) = ECDSA.tryRecover(digest, signature);
        if (error == ECDSA.RecoverError.NoError && recovered == authorizer) {
            return true;
        }
    ...
    function startGame() external whenNotPaused {
        uint256 roundId = _getNextRoundId();
        _userRounds[msg.sender] = roundId; //  discard all unclaimed rounds by user

        paymentToken.safeTransferFrom(msg.sender, address(this), paymentAmount);
        emit GameStarted(msg.sender, roundId);
    }

    function claim(bytes memory signature) external nonReentrant whenNotPaused {
        uint256 roundId = _userRounds[msg.sender];

        require(roundId != 0, "Round not started");
        require(!_claimedRounds[roundId], "Round already claimed");
        require(_verifySignature(msg.sender, roundId, rewardAmount, signature), "Invalid signature");

        _claimedRounds[roundId] = true;
        _userRounds[msg.sender] = 0;

        rewardToken.safeTransfer(msg.sender, rewardAmount);
        emit Claimed(msg.sender, roundId, rewardAmount);
    }
    ...
    function recoverToken(address _token) public override onlyOwner {
        require(_token != address(paymentToken), "Cannot recover payment token");
        super.recoverToken(_token);
    }
...
abstract contract Recoverable is Ownable, IRecoverable {
     ...
     */
    function recoverNonFungibleToken(address _token, uint256 _tokenId) public virtual onlyOwner {
     ...
     * @dev Callable by owner
     */
    function recoverToken(address _token) public virtual onlyOwner {
    ...
    function recoverEth(address payable _to) public virtual onlyOwner {
