...
contract NetGymStreet is OwnableUpgradeable {
    ...
    function setMinerNFTAddressKB(address _minerNFTAddressKB)  external onlyOwner {
        minerNFTAddressKB = _minerNFTAddressKB;
    }
    
    function setStandardParcelNFTAddressKB(address _standardParcelAddressKB)  external onlyOwner {
        standardParcelAddressKB = _standardParcelAddressKB;
    }

    function setMunicipalityAddressKBAddressKB(address _municipalityAddressKB)  external onlyOwner {
...
//        return userLevel[_user];
   ...
   function _getUserPurchasedAmount(address _user) private view returns (uint256) {
        uint256 Gs = IMunicipality(municipalityAddress).userToPurchasedAmountMapping(_user);
        uint256 Kb = IMunicipality(municipalityAddressKB).userToPurchasedAmountMapping(_user);
        return Gs >= Kb ? Gs : Kb;
    }
   function _isUserHasMinPurchase(address _user) private view returns (bool) {
    ...
    function seedUserMlmLevel(address _user, uint256 _level, bool _isNewPurchaseDate) external onlyWhiteListedContracts {
        ...
        if(_isNewPurchaseDate) {
            IMunicipality(municipalityAddress).lastPurchaseData(_user).lastPurchaseDate = block.timestamp;
        }
    }
