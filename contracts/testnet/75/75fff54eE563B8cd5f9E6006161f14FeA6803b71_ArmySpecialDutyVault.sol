...
import "./DutyVault.sol";

import "./ArmyMintingStation.sol";
import "./DutyProfile.sol";

/**
 * @title ArmySpecialDutyVault.
 * @notice It is a contract for users to mint Duty Vault collectible.
 */
contract ArmySpecialDutyVault is Ownable {
    using SafeMath for uint256;

    ArmyMintingStation public armyMintingStation;
    DutyVault public dutyVault;
    DutyProfile public dutyProfile;

    uint8 public constant armyId = 16;

    // Collectible-related.
    uint256 public endBlock;
    uint256 public thresholdTimestamp;

    // DutySwap Profile related.
    ...
    constructor(
        address _dutyVault,
        address _armyMintingStation,
        address _dutyProfile,
        uint256 _endBlock,
        ...
        dutyVault = DutyVault(_dutyVault);
        armyMintingStation = ArmyMintingStation(_armyMintingStation);
        dutyProfile = DutyProfile(_dutyProfile);
        endBlock = _endBlock;
    ...
    /**
     * @notice Mint a NFT from the ArmyMintingStation contract.
    ...
    function mintNFT() external {
        ...
        bool isUserActive;
        (, , , , , isUserActive) = dutyProfile.getUserProfile(msg.sender);
        ...
        uint256 tokenId = armyMintingStation.mintCollectible(msg.sender, tokenURI, armyId);

        // Increase point on DutySwap profile, for a given campaignId.
        dutyProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);

        emit ArmyMint(msg.sender, tokenId, armyId);
    }

    /**
     * @notice Change the campaignId for DutySwap Profile.
    ...
    /**
     * @notice Change the number of points for DutySwap Profile.
     * @dev Only callable by owner.
     */
    ...
    function _canClaim(address _userAddress) internal view returns (bool) {
        if (hasClaimed[_userAddress]) {
            ...
            if (!dutyProfile.getUserStatus(_userAddress)) {
                return false;
            } else {
                uint256 lastDepositedTime;
                (, lastDepositedTime, , ) = dutyVault.userInfo(_userAddress);
