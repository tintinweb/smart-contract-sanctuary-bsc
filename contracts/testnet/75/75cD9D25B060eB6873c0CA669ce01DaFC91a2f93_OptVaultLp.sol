...
contract OptVaultLp is AccessControl, Initializable {
    ...
    address public tokenSwap;//tokenswap contract address
    address public Treasury;//treasury address
    address public LPSwap;//Lpswap contract address
    ...
    mapping(uint32 => uint32) public multiplierLevel; // level => deduction value of deposit (for level 7 it is 50%). User must pass value by multiplying it with 10
    ...
    @param _Admin.MasterChef() Admin.MasterChef()  address
...
/**
    ...
    @param isReceipt. Reciept as a bool.
     */
    function deposit(address user, uint amountLp,uint32 _level, bool isReceipt) external {
        ...
        IERC20(token).approve(address(LPSwap), (amountLp*multiplierLevel[_level])/2000);
        uint busdBefore = IERC20(BUSD).balanceOf(address(this));
        ILpSwap(LPSwap).LPToToken(BUSD,token,(amountLp*multiplierLevel[_level])/2000,address(this));
        ...
        mintRecieptToken = (amountLp * (multiplierLevel[_level]/10))/exchangeRatio;
        INTT(recieptToken).mint(address(this), mintRecieptToken);
        UserReciept[user] += mintRecieptToken;

        // fetch price of bysl
        path[1] = address(bYSL);
        INTT(BShare).mint(user, (amountLp - (amountLp * (multiplierLevel[_level]/1000)))/ (IBYSL(bYSL).protocolPrice() * 100));
        IERC20(token).approve(Admin.MasterChef(),(amountLp -((amountLp*multiplierLevel[_level])/2000)));
        // depositing amount by subtracting the percentage of which is being used for add liquidity of USDY BUSD pair.
        IPancakeMaster(Admin.MasterChef()).deposit(id, (amountLp -((amountLp*multiplierLevel[_level])/2000)));
    ...
    function rewardStateForDeposit(address user,uint amountLp,uint32 _level) internal{
    ...
    */
    ...
    /**
    @dev withdrawAcc function for getting APY from Admin.Masterchef().
    ...
    */
    ...
    /**
    @dev Setter Function for TeamAddress
    @param _teamAddress ,team address as parameter.
    */
