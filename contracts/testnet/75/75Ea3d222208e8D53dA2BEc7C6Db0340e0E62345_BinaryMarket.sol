    ...
    ReentrancyGuard
{
    ...
    function initialize(string memory _uri, address payable _token, address payable _proxy) public {
        _setURI(_uri);
        token = IERC20(_token);
        proxy = Proxy(_proxy);
    ...
    function recoverFunds(uint256 _questionId) external nonReentrant onlyProxyOwner {
        Question storage question = questions[_questionId];
    ...
    modifier onlyProxyOwner() {
        require(proxy.callerAddress() == proxy.owner(), "Caller is not the proxy owner");
        _;
    }
}
