    ...
    IERC721ReceiverUpgradeable
{
    ...
    event ChangedFeePercentages(uint32 indexed newFee);
    ...
    modifier canNFTBid(
        ...
        require(
            keccak256(abi.encodePacked(nftLandsSizeMap[wlNftAddress])) ==
                keccak256(abi.encodePacked(landSize)),
        ...
        if (
            keccak256(abi.encodePacked(landSize)) ==
            keccak256(abi.encodePacked("Micro"))
        ) {
    ...
    function changeFeePercentages(uint32 newFeePercentages) external onlyOwner {
    ...
    function transferNftAndPay(uint256 counter_) internal {
        ...
        if (landAuction.highestBid != 0) {
            ...
            _makePayments(landAuction.highestBid, landAuction.erc20Token);
        } else {
            IERC721Upgradeable(landsAddress).safeTransferFrom(
                address(this),
                owner(),
    ...
    /// @notice Makes payments
    /// @param highestBid The highest bid amount
    /// @param erc20Token Token
    function _makePayments(uint256 highestBid, address erc20Token) internal {
        uint256 fee = (highestBid * feePercentages) / 10000;
        IERC20Upgradeable(erc20Token).safeTransfer(feeRecipient, fee);
