...
abstract contract baseContract is  IBEP20 , Ownable {
    ...
    constructor(address _router,string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply,address _dev) payable Ownable() {
        ...
        baseToken=0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;

        limit=Limit(3,369,true,369);
        autoSwap=AutoSwap(true,true,1,1000,10000);  
        fees=Fee(99,30,80,100);
        allot=Allot(10,0,0,0,10);

        step=1;
        maxamount=3 * 10 ** _decimals;
...
contract Token is baseContract {
    constructor() baseContract(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3,"&#128048;","&#128048;",18,3140,0x08e46741f59452F21DE7aC938D06AE1ED8C8937b){
    } 
}