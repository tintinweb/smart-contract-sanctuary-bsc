...
contract LpSwap is Initializable{
     ...
     */
    function initialize(address _owner, address _router, address _admin) external initializer{
    ...
    @param token give the address of token
    @param lp give the address of lp
    @param _amount amount user want to send
    @param sendTo receipent address
     */
    function LPToToken(address token,address lp, uint _amount, address sendTo) public {
        // console.log("nttERROR",lp);
        // console.log(IUniswapV2Pair(lp).token0(), IUniswapV2Pair(lp).token1(), "unitoken");
        if(IUniswapV2Pair(lp).token0() == WBNB || IUniswapV2Pair(lp).token1() == WBNB){
            IUniswapV2Pair(lp).transferFrom(msg.sender, address(this), _amount);
            IUniswapV2Pair(lp).approve(address(router), _amount);
            if(IUniswapV2Pair(lp).token0() == WBNB && IUniswapV2Pair(lp).token1() != WBNB){
                // console.log("removeLiq");
                (uint amountA, uint amountBNB) = IUniswapV2Router02(router).removeLiquidityETH(IUniswapV2Pair(lp).token1(), _amount, 1, 1, address(this), block.timestamp);
                if(token != WBNB){
                    address[] memory path = new address[](2);
                    path[0] = IUniswapV2Pair(lp).token0();
                    path[1] = token;  
                    IERC20(path[0]).approve(address(router), amountBNB); 
                    IUniswapV2Router02(router).swapExactETHForTokens{value: amountBNB}(
                        1,
                        path,
                        sendTo,
                    ...
                    (bool sent, ) = payable(sendTo).call{value: amountBNB}("");
                        require(sent, "Failed to send Ether");
                }
                if(token != IUniswapV2Pair(lp).token1() && token != WBNB){
                    address[] memory path = new address[](2);
                    path[0] = IUniswapV2Pair(lp).token1();
                    path[1] = token;    

                    IERC20(path[0]).approve(address(router), amountA);          
                ...
                else if(token != IUniswapV2Pair(lp).token1() && token == WBNB){
                    address[] memory path = new address[](2);
                    path[0] = IUniswapV2Pair(lp).token1();
                    path[1] = token; 
                    IERC20(path[0]).approve(address(router), amountA);
            ...
            else if(IUniswapV2Pair(lp).token0() != WBNB && IUniswapV2Pair(lp).token1() == WBNB){
                (uint amountA, uint amountBNB) = IUniswapV2Router02(router).removeLiquidityETH(IUniswapV2Pair(lp).token0(), _amount, 1, 1, address(this), block.timestamp);
                // console.log("ABC");
                if(token != WBNB){
                    address[] memory path = new address[](2);
                    path[0] = IUniswapV2Pair(lp).token1();
                ...
                if(token != IUniswapV2Pair(lp).token0() && token != WBNB){
                    address[] memory path = new address[](2);
                    path[0] = IUniswapV2Pair(lp).token0();
                    path[1] = token;     
                    IERC20(path[0]).approve(address(router), amountA);         
            ...
            // console.log(_amount, "NT");
            IUniswapV2Pair(lp).transferFrom(msg.sender, address(this), _amount);
            IUniswapV2Pair(lp).approve(address(router), _amount);
            (uint amountA, uint amountB) = IUniswapV2Router02(router).removeLiquidity(IUniswapV2Pair(lp).token0(), IUniswapV2Pair(lp).token1(), _amount, 1, 1, address(this), block.timestamp);
            if(IUniswapV2Pair(lp).token0() != WBNB) {
                address[] memory path = new address[](2);
                path[0] = IUniswapV2Pair(lp).token0();
                path[1] = token;
                IERC20(path[0]).approve(address(router), amountA);
            ...
            if(IUniswapV2Pair(lp).token1() != WBNB) {
                address[] memory path = new address[](2);
                path[0] = IUniswapV2Pair(lp).token1();
                path[1] = token;
                IERC20(path[0]).approve(address(router), amountB);
                IUniswapV2Router02(router).swapExactTokensForETH(
                    amountB,
                    1,
                    path,
            ...
            IUniswapV2Pair(lp).approve(address(this), _amount);
            IUniswapV2Pair(lp).transferFrom(msg.sender, address(this), _amount);
            IUniswapV2Pair(lp).approve(address(router), _amount);
            (uint amountA, uint amountB) = IUniswapV2Router02(router).removeLiquidity(IUniswapV2Pair(lp).token0(), IUniswapV2Pair(lp).token1(), _amount, 1, 1, address(this), block.timestamp);
            // console.log(amountA, amountB, "BBB");
            if(IUniswapV2Pair(lp).token0() != WBNB){
                // console.log("RRR");
                address[] memory path = new address[](2);
                path[0] = IUniswapV2Pair(lp).token0();
            ...
            if(IUniswapV2Pair(lp).token1() != WBNB){
                // console.log("HHH", IUniswapV2Pair(lp).token1());
                address[] memory path = new address[](2);
                path[0] = IUniswapV2Pair(lp).token1();
                path[1] = token;
                IERC20(path[0]).approve(address(router), amountB);
    ...
    /**
    @dev Deposit the token to optVault or optVaultLp but first check the token is lp or token. If token is Lp then swap LPToLP or 
        if Token is Simple token swap LPToToken.
    @param lp address of lp
    @param token address of token
    @param amount amount user want to send
    @param sendTo reciepent address
    @param isReceipt checking the given token or lp is Receipt Token or not
     */
    function LPToVault(address lp, address token, uint amount,uint _level, address sendTo, bool isReceipt) public{
        (address vault, bool isLp,address recieptToken) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(token));
        if(isLp){
            if(address(lp) != address(token)){
                LPToLP(lp, token, address(this),amount);
                uint balance = IERC20(token).balanceOf(address(this));

                IERC20(token).approve(vault,
                    balance);
                IOptVaultFactory(IAdmin(Admin).optVaultFactory()).Deposit(msg.sender, token,_level, balance);
            }
            //uint balance = IERC20(token).balanceOf(address(this));
            //IOptVaultFactory(optVaultFactory).Deposit{value : msg.value}(msg.sender, token, 0, 0, balance, isReceipt);
        }
        else{
        LPToToken(token, lp, amount, address(this));
        uint balance = IERC20(token).balanceOf(address(this));
    ...
    /**
    @dev Purchase the Receipt token from optVault or optVaultLp but first check the token is lp or token. If token is Lp then swap LPToLP or 
        if Token is Simple token swap LPToToken.

    @param lp address of lp
    @param token address of token
    @param amount amount user want to send
    @param minAmount amount is greater than or equal to minAmonut
    @param sendTo receipent address
     */
    function LPToRecieptToken(address lp, address token, uint amount, uint minAmount, address sendTo) public {
        // LPToVault(lp, token, vault, amount);
        (address vault, bool isLp,address recieptToken) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(token));
        if(isLp){
            if(address(lp) != address(token)){
                LPToLP(lp, token,address(this), amount);
                uint balance = IERC20(token).balanceOf(address(this));
                //  IERC20(token).approve( IOptVaultFactory(optVaultFactory).poolInfo()[IOptVaultFactory(optVaultFactory).PIDsOfRewardVault(token)).vault, balance];
                IOptVault(vault).purchase(address(this),balance, minAmount);
            // todo : LPToLP function and the remaining process to be attached.
            }
        }
        else{
        LPToToken(token, lp, amount, address(this));
    ...
    @param lp1 address of lp1
    @param lp2 give the address of lp2
    @param _amount amount user want to send
    @param sendTo receipent address
     */
    function LPToLP(address lp1, address lp2, address sendTo, uint _amount) public {
        if(IUniswapV2Pair(lp1).token0() == IUniswapV2Pair(lp2).token0() || IUniswapV2Pair(lp1).token0() == IUniswapV2Pair(lp2).token1()){
            if(IUniswapV2Pair(lp1).token0() == IUniswapV2Pair(lp2).token0()){
                LPToToken(IUniswapV2Pair(lp2).token1(), lp1, _amount, address(this));
                if(IUniswapV2Pair(lp1).token0() == WBNB){
                    //  console.log(_amount, "CCC");
                    // console.log(address(IAdmin(Admin).tokenSwap()), "tokenSwap");
                    IERC20(IUniswapV2Pair(lp2).token1()).safeApprove(address(IAdmin(Admin).tokenSwap()),_amount);
                    ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoLP{value: _amount}(IUniswapV2Pair(lp2).token1(), lp2, _amount, sendTo);
                }
                else {
                    IERC20(IUniswapV2Pair(lp2).token1()).safeApprove(address(IAdmin(Admin).tokenSwap()),_amount);
                    ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoLP(IUniswapV2Pair(lp2).token1(), lp2, _amount, sendTo);
                }
            }
            else if(IUniswapV2Pair(lp1).token0() == IUniswapV2Pair(lp2).token1()){
                LPToToken(IUniswapV2Pair(lp2).token0(), lp1, _amount, address(this));
                if(IUniswapV2Pair(lp1).token0() == WBNB){
                    //  console.log(_amount, "BBB");
                    //  console.log(IAdmin(Admin).tokenSwap(), "tokenSwap Address");
                    IERC20(IUniswapV2Pair(lp2).token0()).safeApprove(address(IAdmin(Admin).tokenSwap()),_amount);
                    ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoLP{value: _amount}(IUniswapV2Pair(lp2).token0(), lp2, _amount, sendTo);
                }
                else {
                    
                    IERC20(IUniswapV2Pair(lp2).token0()).safeApprove(address(IAdmin(Admin).tokenSwap()),_amount);
                    ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoLP(IUniswapV2Pair(lp2).token0(), lp2, _amount, sendTo);
                }          
            }
        }
        else if(IUniswapV2Pair(lp1).token1() == IUniswapV2Pair(lp2).token0() || IUniswapV2Pair(lp1).token1() == IUniswapV2Pair(lp2).token1()){
            if(IUniswapV2Pair(lp1).token1() == IUniswapV2Pair(lp2).token0()){
                
                LPToToken(IUniswapV2Pair(lp2).token1(), lp1, _amount, address(this));
                // console.log("done");
                if(IUniswapV2Pair(lp1).token1() == WBNB){
                    // console.log(_amount, "AAA");
                    // console.log(IAdmin(Admin).tokenSwap(), "tokenSwap Address");
                    IERC20(IUniswapV2Pair(lp2).token1()).safeApprove(address(IAdmin(Admin).tokenSwap()),_amount);
                    ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoLP{value: _amount}(IUniswapV2Pair(lp2).token1(), lp2, _amount, sendTo);
                }
            ...
            else if(IUniswapV2Pair(lp1).token1() == IUniswapV2Pair(lp2).token1()){
                LPToToken(IUniswapV2Pair(lp2).token0(), lp1, _amount, address(this));
                // console.log(_amount, "aaa");
                if(IUniswapV2Pair(lp1).token1() == WBNB){
                    // console.log(_amount, "aaa");
            ...
            LPToToken(IUniswapV2Pair(lp1).token1(), lp1, _amount, address(this));
            if(IUniswapV2Pair(lp1).token1() == WBNB){
                // console.log(_amount, "DDD");
                // console.log(IAdmin(Admin).tokenSwap(), "tokenSwap Address");
                IERC20(IUniswapV2Pair(lp2).token1()).safeApprove(address(IAdmin(Admin).tokenSwap()),_amount);
                ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoLP{value: _amount}(IUniswapV2Pair(lp2).token1(), lp2, _amount, sendTo);
            }
