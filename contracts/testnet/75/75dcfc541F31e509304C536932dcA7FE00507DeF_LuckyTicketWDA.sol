...
// File: contracts\LuckyTicket\LuckyTicketWDA.sol
...
contract LuckyTicketWDA is Ownable, ReentrancyGuard, VRFConsumerBase {
    ...
    uint256[] allTicketSoldPerPeriods;
    ...
    // player address => purchase date
    mapping(address => uint32[]) ownerTicket;
    // index ticket => player address
    mapping(uint256 => address) public ownerOfTicket;
    // player address => quantity
    ...
    function cronRollingWinningTicket() external isPeriod {
        _setTicket100000WDA();
        _setTicket5000WDA();
    ...
    function _setTicket5000WDA() internal {
        ...
        if (totalTicketSoldPerPeriod >= 100) {
            getRandomNumber();
            // random number ticket for 1% user reward 5000 WDA
            uint256 _winnerCount = totalTicketSoldPerPeriod / 100;
        ...
        for (uint256 i = 0; i < totalTicketSoldPerPeriod; i++) {
            ...
            if (i == _winningTicketIndex) {
                ...
                if (_rewardAmount == 5000) {
                    ownerWinningTicket5000WDA[_addressOwnerOfTicket] += 1;
                } else {
                    ownerWinningTicket100000WDA[_addressOwnerOfTicket] += 1;
                }
                emit TicketWin(_addressOwnerOfTicket, _reward, i);
            } else {
                ownerTicketReward[_addressOwnerOfTicket].push(
                    allTicketSoldPerPeriods[i] + 5 days
                );
            }
        }
    }

    function resetTicketEachDay() external  {
        ticketSoldPerDay = 0;
    }

    function resetAllTicket() external  isPeriod {
        for (uint24 i = 0; i < totalTicketSoldPerPeriod; i++) {
            address addressOwnerOfTicket = ownerOfTicket[i];
            ownerTicket[addressOwnerOfTicket] = new uint32[](0);
            delete ownerOfTicket[i];
        }
        allTicketSoldPerPeriods = new uint32[](0);
    ...
    function buyTicket(uint8 _quantity) external payable nonReentrant {
        uint32[] storage _ownerTicket = ownerTicket[msg.sender];
        // check maximum ticket allow of each user
        require(
            _ownerTicket.length + _quantity <= _ownerTicketMaxCount,
        ...
        for (uint8 i = 0; i < _quantity; i++) {
            _ownerTicket.push(uint32(block.timestamp));
            // store all user ticket
            allTicketSoldPerPeriods.push(block.timestamp);
            // store address owner of ticket
            ownerOfTicket[totalTicketSoldPerPeriod] = msg.sender;
            totalTicketSoldPerPeriod++;
        }
        ticketSoldPerDay = ticketSoldPerDay + uint24(_quantity);
    ...
    function claimCoin(uint8 _ticketType) external nonReentrant {
        ...
        if (_ticketType == 0) {
            require(
                ownerWinningTicket5000WDA[msg.sender] > 0,
                "Not have enough ticket to claim"
            );
            IERC20(_token).transfer(
                msg.sender,
                ownerWinningTicket5000WDA[msg.sender] * 5000
            );
            ownerWinningTicket5000WDA[msg.sender] = 0;
        } else {
            require(
                ownerWinningTicket100000WDA[msg.sender] > 0,
                "Not have enough ticket to claim"
            );
            IERC20(_token).transfer(
                msg.sender,
                ownerWinningTicket100000WDA[msg.sender] * 100000
            );
            ownerWinningTicket100000WDA[msg.sender] = 0;
        }
    }
    ...
    function claimReward() external nonReentrant {
        uint256 availableCount = 0;
        uint256 _restTicket = 0;
        uint256[] memory _newPlayerTicketReward;
        for (uint256 i = 0; i < ownerTicketReward[msg.sender].length; i++) {
            if (ownerTicketReward[msg.sender][i] <= block.timestamp) {
                availableCount++;
            } else {
                _newPlayerTicketReward[_restTicket] = ownerTicketReward[
                    msg.sender
                ][i];
                _restTicket++;
            }
        }
        require(availableCount > 0, "Do not have any ticket reward to claim");
        ownerTicketReward[msg.sender] = _newPlayerTicketReward;
        IERC20(_token).transfer(msg.sender, availableCount * _ticketCost);
        ownerWinningTicket100000WDA[msg.sender] = 0;
    }
