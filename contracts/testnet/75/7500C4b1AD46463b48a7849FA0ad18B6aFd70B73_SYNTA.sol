...
contract SYNTA is IERC20, Ownable, Pausable {
    ...
    uint private _totalSupply = 100_000_000e18;
    ...
    function transferFrom(address sender, address recipient, uint amount) external override whenNotPaused returns (bool) {
        ...
        uint256 toApprove = 0;
        unchecked { toApprove = currentAllowance - amount; }
        _approve(sender, msg.sender, toApprove);
    ...
    function decreaseAllowance(address spender, uint subtractedValue) external returns (bool) {
        ...
        uint256 toApprove = 0;
        unchecked { toApprove = currentAllowance - subtractedValue; }
        _approve(msg.sender, spender, toApprove);
    ...
    function unvest() external whenNotPaused returns (uint unvested) {
        ...
        for (uint i = 1; i <= _vestingNonces[msg.sender]; ) {
            ...
            toUnvest = toUnvest - _unvestedAmounts[msg.sender][i];
            unvested = unvested + toUnvest;
            _unvestedAmounts[msg.sender][i] = totalUnvestedForNonce;
            unchecked { ++i; }
        }
        _unfrozenBalances[msg.sender] = _unfrozenBalances[msg.sender] + unvested;
    ...
    function burnTokens(uint amount) external onlyOwner returns (bool success) {
        ...
        unchecked { _unfrozenBalances[owner()] = ownerBalance - amount; }
        _totalSupply = _totalSupply - amount;
    ...
    function balanceOf(address account) external view override returns (uint) {
        ...
        for (uint i = 1; i <= _vestingNonces[account]; ) {
    ...
    function availableForUnvesting(address user) external view returns (uint unvestAmount) {
        ...
        for (uint i = 1; i <= _vestingNonces[user]; ) {
            ...
            toUnvest = toUnvest - _unvestedAmounts[user][i];
            unvestAmount = unvestAmount + toUnvest;
            unchecked { ++i; }
        }
    }
    ...
    function _transfer(address sender, address recipient, uint amount) private {
        ...
        unchecked { _unfrozenBalances[sender] = senderAvailableBalance - amount; }
        _unfrozenBalances[recipient] = _unfrozenBalances[recipient] + amount;
    ...
    function _vest(address user, uint amount, uint vestType, uint vestingReleaseStart, uint vestingReleaseSecondPeriod) private {
        ...
        _unfrozenBalances[owner()] = _unfrozenBalances[owner()] - amount;
