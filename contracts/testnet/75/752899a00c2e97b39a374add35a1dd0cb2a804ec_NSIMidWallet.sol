...
contract NSIMidWallet is AccessControl {
    ...
    bool private Depositpaused = true;
    ...
    function Trade_Token(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin) public onlyAdmin{
        ...
        uint256[] memory amounts = IUniswapV2Router02(uniswapV2Router).swapExactTokensForTokens(_amountIn, _amountOutMin, path, _to, deadline);
        uint256 amount1 = amounts[1];
        uint256 amount2 = amounts[2];
        paused == false;

        if (_tokenIn == WETH || _tokenOut == WETH) {
            emit Resultant_Token(owner, amount1, _tokenOut);
        }else {

            emit Resultant_Token(owner, amount2, _tokenOut);
    ...
    function ownerWithdraw(uint256 _amount, address _tokenAddr) public onlyOwner {
        ...
        require(Withdrawpaused == false, "Admin hasn't enabled you to withdraw Tokens.");
