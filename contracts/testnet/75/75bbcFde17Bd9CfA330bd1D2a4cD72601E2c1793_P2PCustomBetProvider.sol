...
contract P2PCustomBetProvider is TokenProcessing, CustomProcessor {
    ...
    function refundCustomBet(uint betId, address client) external {
        ...
        require(mainTokenToRefund > 0 || alterTokenToRefund > 0, "P2PCustomBetProvider: refund - nothing");
    ...
    function cancelCustomJoin(uint betId, uint joinIdRef) external {
        ...
        (uint mainTokenToRefund, uint alterTokenToRefund) = cancelCustomBet(matchingInfo[betId], clientJoin);
...
abstract contract CustomProcessor {
    ...
    function cancelCustomBet(CustomDTOs.CustomMatchingInfo storage info, CustomDTOs.JoinCustomBetClient storage joinClient) internal returns (uint, uint) {
