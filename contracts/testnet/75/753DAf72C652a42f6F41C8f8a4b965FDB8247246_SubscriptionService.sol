 ...
 * @author Tim Loh
 */
interface ISubscriptionService is
    IAccessControl,
    IAdminPrivileges,
    IAdminWallet
{
    ...
    function consumeSubscriptionPackage(
        address account
    ) external returns (bool isConsumed);

    function erc20PurchaseSubscriptionPackage() external;

    function nativePurchaseSubscriptionPackage() external payable;
...
library TransferErc20 {
    ...
    function transferTokensFromContractToAccount(
        ...
        require(
            tokenDecimals <= TOKEN_MAX_DECIMALS,
            "TransferErc20: token decimals"
        );
...
import "./libraries/TransferErc20.sol";
 ...
 * @author Tim Loh
 */
contract SubscriptionService is
    Pausable,
    AdminPrivileges,
    AdminWallet,
    ISubscriptionService
{
    ...
    struct AvailableSubscriptionPackage {
        uint256 expiryDays; // number of days expire from date of purchase
        uint256 numberOfContracts; // number of contracts can deploy
        address paymentTokenAddress;
        uint256 paymentTokenDecimals;
        uint256 priceInPaymentTokenWei;
        bool isInitialized;
    ...
    struct PurchasedSubscriptionPackage {
        uint256 expiryTimestamp;
        uint256 numberOfContractsDeployed;
        uint256 numberOfDeployableContracts;
    }

    bytes32 public constant CONTRACT_USAGE_ROLE =
        keccak256("CONTRACT_USAGE_ROLE");
    ...
    mapping(address => AvailableSubscriptionPackage)
        private _availableSubscriptionPackages;
    mapping(address => uint256) private _paymentsWei;
    mapping(address => PurchasedSubscriptionPackage)
        private _purchasedSubscriptionPackages;
    ...
    function consumeSubscriptionPackage(
        ...
        virtual
        override
        onlyRole(CONTRACT_USAGE_ROLE)
        whenNotPaused
        returns (bool isConsumed)
    {
        require(account != address(0), "SubSvc: zero account");

        PurchasedSubscriptionPackage
            memory purchasedPackage = _purchasedSubscriptionPackages[account];
        require(purchasedPackage.expiryTimestamp > 0, "SubSvc: no expiry");
        require(
            purchasedPackage.numberOfDeployableContracts > 0,
            "SubSvc: no purchase"
        );
        require(
            block.timestamp < purchasedPackage.expiryTimestamp,
            "SubSvc: expired"
        );
        require(
            ++purchasedPackage.numberOfContractsDeployed <=
                purchasedPackage.numberOfDeployableContracts,
            "SubSvc: exceeded"
        );

        _purchasedSubscriptionPackages[account]
            .numberOfContractsDeployed = purchasedPackage
            .numberOfContractsDeployed;

        isConsumed = true;

        emit SubscriptionPackageConsumed(
            msg.sender,
            account,
            purchasedPackage.numberOfContractsDeployed,
            purchasedPackage.numberOfDeployableContracts,
            purchasedPackage.expiryTimestamp
        );
    }
     ...
     * @inheritdoc ISubscriptionService
     */
    function erc20PurchaseSubscriptionPackage()
        external
        virtual
        override
        whenNotPaused
    {
        AvailableSubscriptionPackage
            memory availablePackage = _getAvailableSubscriptionPackage(
                msg.sender
            );
        require(
            availablePackage.paymentTokenAddress != address(0),
            "SubSvc: native payment token"
        );

        _updatePurchaseState(msg.sender, availablePackage, true);

        IERC20(availablePackage.paymentTokenAddress)
            .transferTokensFromSenderToAccount(
                availablePackage.paymentTokenDecimals,
                availablePackage.priceInPaymentTokenWei,
                adminWallet()
            );
    }
     ...
     * @inheritdoc ISubscriptionService
     */
    function nativePurchaseSubscriptionPackage()
        external
        payable
        virtual
        override
        whenNotPaused
    {
        ...
        require(
            availablePackage.paymentTokenAddress == address(0),
            "SubSvc: ERC20 payment token"
        );
        require(
            msg.value >= availablePackage.priceInPaymentTokenWei,
            "SubSvc: insufficient payment"
        );
    ...
    function setAvailableSubscriptionPackage(
        ...
        require(
            ...
            "SubSvc: zero purchaser address"
        );
        require(expiryDays > 0, "SubSvc: zero expiry days");
        require(numberOfContracts > 0, "SubSvc: zero contracts");
        require(
            paymentTokenAddress != address(0) ||
                paymentTokenDecimals == TOKEN_MAX_DECIMALS,
            "SubSvc: invalid token"
        );
        require(
            paymentTokenDecimals <= TOKEN_MAX_DECIMALS,
            "SubSvc: token decimals"
        );

        AvailableSubscriptionPackage
            memory availablePackage = AvailableSubscriptionPackage({
                expiryDays: expiryDays,
                numberOfContracts: numberOfContracts,
                paymentTokenAddress: paymentTokenAddress,
                paymentTokenDecimals: paymentTokenDecimals,
                priceInPaymentTokenWei: priceInPaymentTokenWei,
                isInitialized: true
            });

        if (priceInPaymentTokenWei == 0) {
            _updatePurchaseState(purchaserAddress, availablePackage, false);
        } else {
            _availableSubscriptionPackages[purchaserAddress] = availablePackage;
        }

        emit AvailableSubscriptionPackageSet(
            msg.sender,
            purchaserAddress,
            paymentTokenAddress,
            paymentTokenDecimals,
            priceInPaymentTokenWei,
            expiryDays,
            numberOfContracts
        );
    }
        ...
        )
    {
        ...
        AvailableSubscriptionPackage
            memory availablePackage = _availableSubscriptionPackages[account];
        expiryDays = availablePackage.expiryDays;
        numberOfContracts = availablePackage.numberOfContracts;
        paymentTokenAddress = availablePackage.paymentTokenAddress;
        paymentTokenDecimals = availablePackage.paymentTokenDecimals;
        priceInPaymentTokenWei = availablePackage.priceInPaymentTokenWei;
        isInitialized = availablePackage.isInitialized;
    }
        ...
        )
    {
        ...
        PurchasedSubscriptionPackage
            memory purchasedPackage = _purchasedSubscriptionPackages[account];
        expiryTimestamp = purchasedPackage.expiryTimestamp;
        numberOfContractsDeployed = purchasedPackage.numberOfContractsDeployed;
        numberOfDeployableContracts = purchasedPackage
            .numberOfDeployableContracts;
    }
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(AccessControl, ISubscriptionService)
        returns (bool)
    {
        return
            interfaceId == type(ISubscriptionService).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    function _updatePurchaseState(
        address purchaser,
        AvailableSubscriptionPackage memory availablePackage,
        bool resetAvailableSubscriptionPackage
    ) internal virtual {
        require(purchaser != address(0), "SubSvc: zero purchaser");

        PurchasedSubscriptionPackage
            memory purchasedPackage = _purchasedSubscriptionPackages[purchaser];

        purchasedPackage.numberOfDeployableContracts = block.timestamp >=
            purchasedPackage.expiryTimestamp
            ? purchasedPackage.numberOfContractsDeployed +
                availablePackage.numberOfContracts
            : purchasedPackage.numberOfDeployableContracts +
                availablePackage.numberOfContracts;

        uint256 expiryTimestamp = _calculateExpiryTimestamp(
            availablePackage.expiryDays,
            block.timestamp
        );
        if (expiryTimestamp > purchasedPackage.expiryTimestamp) {
            purchasedPackage.expiryTimestamp = expiryTimestamp;
        }

        _purchasedSubscriptionPackages[purchaser]
            .numberOfDeployableContracts = purchasedPackage
            .numberOfDeployableContracts;
        _purchasedSubscriptionPackages[purchaser]
            .expiryTimestamp = purchasedPackage.expiryTimestamp;
        _paymentsWei[availablePackage.paymentTokenAddress] += availablePackage
            .priceInPaymentTokenWei;

        if (
            resetAvailableSubscriptionPackage ||
            _availableSubscriptionPackages[purchaser].isInitialized
        ) {
            _availableSubscriptionPackages[
                purchaser
            ] = AvailableSubscriptionPackage({
                expiryDays: 0,
                numberOfContracts: 0,
                ...
                priceInPaymentTokenWei: 0,
                isInitialized: false
            });
        }

        emit SubscriptionPackagePurchased(
            purchaser,
            adminWallet(),
            availablePackage.paymentTokenAddress,
            availablePackage.paymentTokenDecimals,
            availablePackage.priceInPaymentTokenWei,
            availablePackage.numberOfContracts,
            availablePackage.expiryDays,
    ...
    function _getAvailableSubscriptionPackage(
        address account
    )
        internal
        view
        virtual
        returns (AvailableSubscriptionPackage memory availablePackage)
    {
        availablePackage = _availableSubscriptionPackages[account];
        require(availablePackage.isInitialized, "SubSvc: unavailable");
        require(availablePackage.expiryDays > 0, "SubSvc: zero expiry days");
        require(
            availablePackage.numberOfContracts > 0,
            "SubSvc: zero contracts"
        );
        require(
            availablePackage.priceInPaymentTokenWei > 0,
            "SubSvc: zero price"
        );
        require(
            availablePackage.paymentTokenAddress != address(0) ||
                availablePackage.paymentTokenDecimals == TOKEN_MAX_DECIMALS,
            "SubSvc: invalid token"
        );
        require(
            availablePackage.paymentTokenDecimals <= TOKEN_MAX_DECIMALS,
    ...
    /**
     * @dev Returns the calculated timestamp when the subscription expires given the expiry in days and purchase timestamp
     * @param expiryDays The expiry in days
     * @param purchaseTimestamp The purchse timestamp as seconds since unix epoch
     * @return calculatedExpiryTimestamp The timestamp as seconds since unix epoch when the subscription expires
     */
    function _calculateExpiryTimestamp(
        uint256 expiryDays,
        uint256 purchaseTimestamp
    ) internal pure virtual returns (uint256 calculatedExpiryTimestamp) {
