...
contract BlindBox is AccessController, ReentrancyGuard {
    ...
    function buy(address nft, uint256 quantity) public payable validBox(nft) {
        (
            IERC721 sellingNft,
            uint256 listingPrice
        ) = getBox(nft);
        ...
        require(msg.value >= listingPrice * quantity, "Not enough BNB to cover item price.");
        
        uint256[] memory tokenIds = new uint[](quantity);
        for (uint256 index = 0; index < quantity; index++) {
            uint256 tokenId = IERC721Enumerable(nft).tokenOfOwnerByIndex(address(this), index);
