   ...
   /*初始化*/
    ...
    function UpdateEarning(address _address, uint8 _weight) private {
        ...
        if(roundId != userReward[_address].markerRound){
            ...
            if(data.school.listSize < maxRank){
                userReward[_address].markerUsdt = resD[roundId].resBalanceUsdt;
                userReward[_address].makerErc20 = resD[roundId].resBalanceErc20;
            }else{
    ...
    function Earning() payable public {
        ...
        if(inData[roundId].school.listSize >= maxRank)
        {
            ...
            userReward[msg.sender].markerUsdt = resD[roundId].resBalanceUsdt; 
            userReward[msg.sender].makerErc20 = resD[roundId].resBalanceErc20;        
        }   
