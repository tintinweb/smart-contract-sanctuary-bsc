...
/** 
    ...
    🖌 5 Unique Features:
      ...
      🖊  Horse Race - Miner Betting Feature runs every 8 hours. (Bet on any of the 5 horses and get the chance to win based on the fixed odds.)
...
contract ryker_busd {
    ...
    function invest(address ref,uint256 amount) public nonReentrant {
        ...
        if(!contractStarted){
            ...
            if(LOTTERY_ACTIVATED) {
                if(getCurTimeTest().sub(LOTTERY_START_TIME) >= lotteryTimeStep || currentLotteryParticipants >= 200 || currentPot >= maxIncentiveBalance) {
                    drawLotteryWinner();
                }
            ...
            if(LAST_DEPOSIT_ACTIVATED) {
                if(getCurTimeTest().sub(LAST_DEPOSIT_START_TIME) >= lastDepositTimeStep && currentLastDepositPot > 0 && potentialLastDepositWinner != address(0)) {
    ...
    function chooseWinners() external {
        ...
        if(LOTTERY_ACTIVATED && (getCurTimeTest().sub(LOTTERY_START_TIME) >= lotteryTimeStep || currentLotteryParticipants >= 200 || currentPot >= maxIncentiveBalance)) {
            drawLotteryWinner();
        }
        ...
        if(LAST_DEPOSIT_ACTIVATED && getCurTimeTest().sub(LAST_DEPOSIT_START_TIME) >= lastDepositTimeStep && currentLastDepositPot > 0 && potentialLastDepositWinner != address(0)) {
            drawLastDepositWinner();
        }
        ...
        if(currentLotteryParticipants > 0){
            ...
            for(uint256 i = 0; i < currentLotteryParticipants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    emit LotteryEvent(currentLotteryRound, winnerAddress, minerRewards, currentLotteryParticipants, currentTotalLotteryTickets, getCurTimeTest());
