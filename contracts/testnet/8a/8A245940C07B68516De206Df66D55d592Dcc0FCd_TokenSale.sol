...
contract TokenSale is Initializable, ITokenSale {
        ...
        initializer
    {
        ...
        busd = IERC20D(0xa513E6E4b8f2a923D98304ec87F64353C4D5C853);
    }
    ...
    function _processPrivate(address _sender, uint256 _amount) internal {
        ...
        require(add > 0, "No allocation left");
        if (t == 7) {
            if (!s.free) {
                s.free = true;
        ...
        s.amount += add;
        // require(add > 0, "more than allocation");
...
contract Staking is IStaking, Initializable {
    ...
    function stake(uint256 _level, uint256 _amount) external payable {
        ...
        if (s.lock > 1 && _getHighestTier(msg.sender) > 3 && IAirdrops(admin.airdrop()).isAirdropClaimed(msg.sender) == false){
            console.log("-");
            IAirdrops(admin.airdrop()).claimAirdrop(msg.sender);
        }
