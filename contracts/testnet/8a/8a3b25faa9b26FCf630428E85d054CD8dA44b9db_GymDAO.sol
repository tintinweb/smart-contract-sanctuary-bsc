...
contract GymDAO is OwnableUpgradeable {

    uint256 public votersTotalWeight;
    ...
    function initialize(address _singlePoolAddress) external initializer {
    ...
    function openProposals(Proposal[] calldata _proposals) external onlyOwner {
        for (uint i = 0; i < _proposals.length; i++) {
            proposals.push(Proposal({
                name: _proposals[i].name,
                voteCount: 0,
                delegate: _proposals[i].delegate
            }));
        }
    }
    ...
    function winningProposal() external onlyOwner returns (uint256) {
        ...
        return winnerProposal + 1;
    }

    function _cleanPrevResult() private {
        for (uint256 i = 0; i < proposals.length; i++) {
            delete proposals[i];
        }
    }

    function _giveRightToVote(address _voter) private {
        Voter storage voter = voters[_voter];
        require(!voter.voted, "The voter already voted.");
        require(voter.weight == 0);
        uint256 voterGGymnet = IGymSinglePool(singlePoolAddress).getUserInfo(_voter).totalGGYMNET;
        require(voterGGymnet > 0, "GymDAO: The voter has no shares in SinglePool");
        uint256 totalGGymnetInPoolLocked = IGymSinglePool(singlePoolAddress).totalGGymnetInPoolLocked();
        voter.weight = (voterGGymnet * 1e18) / totalGGymnetInPoolLocked; // weight in 1e18
    }
}
