...
contract COTEST is Context, IERC20, Ownable {
  using SafeMath for uint256;
  using Address for address;

  string private _name = "COTEST";
  ...
  uint256 private _tTotal = 10_000 * (10**DECIMALS);
  ...
  constructor() {
    ...
    //0.25 percent of the starting supply
    _previousVolume += (_tTotal * 25) / 10000;
    ...
    //exclude pair from getting reflections
  ...
  function _tokenTransfer(
    ...
    if (takeFee) {
      ...
      if (recipient == uniswapV2Pair) {
        ...
        if (userSellVolume[sender].amount > (_previousVolume * 5) / 100) {
          setHarpoonFee();
        }
      }
