...
contract USDTT is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(!_isExcludedFromFees[from] || !_isExcludedFromFees[to]){
            if(to == address(uniswapV2Pair)){
                // sell
                // usdtt -> usdt  usdtt 增加 ， usdt 减少
                // IERC20Upgradeable y = IERC20Upgradeable(_usdt);
                // uint256 u = y.balanceOf(address(uniswapV2Pair));
                // uint256 f = balanceOf(address(uniswapV2Pair));
                // uint256 a = f - u;
                // _burn(address(uniswapV2Pair), 8888);
            }else if(from == address(uniswapV2Pair)){
                // buy 
                // usdt -> usdtt  usdt增加，usdtt减少
                IERC20Upgradeable y = IERC20Upgradeable(_usdt);
                uint256 u = y.balanceOf(address(uniswapV2Pair));
                uint256 f = balanceOf(address(uniswapV2Pair));
                require(u > f, " u < f");
    ...
    //         IERC20Upgradeable y = IERC20Upgradeable(_usdt);
    //         return y.balanceOf(address(uniswapV2Pair));
    //     }
