...
contract CeresReward is CeresBase, ICeresReward, Ownable {
    ...
    mapping(address => RewardDuration) rewardDurations;
    ...
    function getDefaultRewardDuration() public view returns (RewardDuration memory) {
        return rewardDurations[address(0)];
    }
    ...
    function applyReward() external override onlyStakings returns (uint256 _bonus) {
        ...
        require(factory.isStakingRewards(staking.token()), "This staking can not apply rewards now.");

        uint256 stakingValue = staking.value();
        require(stakingValue >= minStakingValue, "Staking value is not enough to apply reward!");

        RewardDuration memory _rewardDuration = rewardDurations[address(0)];
        if (_rewardDuration.amount == 0)
            _rewardDuration = getDefaultRewardDuration();

        _bonus = 0;
        if (_rewardDuration.amount > 0) {

            uint256 transferCRSAmount = _rewardDuration.amount + applicantCRSBonus;
            require(crs.balanceOf(address(this)) >= transferCRSAmount, "Balance is not enough for applying reward.");
            crs.transfer(msg.sender, transferCRSAmount);
            staking.notifyReward(_rewardDuration.amount, _rewardDuration.duration);
            ...
            amountApplied[msg.sender] += _rewardDuration.amount;
            emit RewardApplied(msg.sender, _rewardDuration.amount, _bonus);
        }
    }
    ...
    function setRewardDuration(address _staking, uint256 _amount, uint256 _duration) public onlyOwner {
        require(_amount > 0, "Reward amount must be bigger than zero!");
        require(_duration > 0, "Reward duration must be bigger than zero!");

        rewardDurations[_staking].amount = _amount;
        rewardDurations[_staking].duration = _duration;
    }

    function setDefaultRewardDuration(uint256 _amount, uint256 _duration) external onlyOwner {
        setRewardDuration(address(0), _amount, _duration);
    }
