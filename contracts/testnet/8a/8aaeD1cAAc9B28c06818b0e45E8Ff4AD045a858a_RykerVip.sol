...
contract RykerVip {
    ...
    uint256 public LOTTERY_STEP = 8 * 60 * 60; /** every 8 hours. **/
    ...
    function sellCrops() public{
        ...
        if(getCurTime().sub(LOTTERY_START_TIME) >= LOTTERY_STEP || participants >= MAX_LOTTERY_PARTICIPANTS){
            chooseWinner();
        }
    }
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
    function chooseWinner() public {
       require(((getCurTime().sub(LOTTERY_START_TIME) >= LOTTERY_STEP) || participants >= MAX_LOTTERY_PARTICIPANTS),
        ...
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    userLotteryMinerRewards[winnerAddress] = userLotteryMinerRewards[winnerAddress].add(eggsReward);
