...
contract ConvexNode is ERC20, Ownable, PaymentSplitter {
    using SafeMath for uint256;

    INODERewardManagement public node1RewardManager;
    INODERewardManagement public node2RewardManager;
    INODERewardManagement public node3RewardManager;
    ...
    uint256 public rewardsFee; // Reward fee
    uint256 public liquidityPoolFee; // Liquidity pool fee
    uint256 public futurFee; // Futur fee
    uint256 public totalFees; // Total fees
    uint256 public cashoutFee; // Cashout fee
    uint256 public referalReward; // Refferal reward percentage
    bool public isReferRewardClaimable; // Is refferal reward claimable
    ...
    mapping(address => NodeFee) public _userNodeFees;
    ...
    constructor(
        ...
        uint256 swapAmount,
        address uniV2Router
    ) ERC20("Convex Finance", "$CON") PaymentSplitter(payees, shares) {
    ...
    function setNodeManagement(address[3] memory nodeManagement)
        external
        onlyOwner
    {
        node1RewardManager = INODERewardManagement(nodeManagement[0]);
        node2RewardManager = INODERewardManagement(nodeManagement[1]);
        node3RewardManager = INODERewardManagement(nodeManagement[2]);
    }
    ...
    function createNodeWithTokens(
        ...
        if (referrer != address(0)) {
            ReferReward storage _referReward = _userReferReward[referrer];
            _referReward.claimable += nodePrice.mul(referalReward).div(
                PERCENT_DIVIDER
            );
        }
    }

    function cashoutReward(uint256 blocktime, uint256 nodeIndex) public {
        INODERewardManagement nodeRewardManager = getNodeRewardManager(
            nodeIndex
        );
    ...
    function cashoutAll(uint256 nodeIndex) public {
        INODERewardManagement nodeRewardManager = getNodeRewardManager(
            nodeIndex
        );
        address sender = _msgSender();
    ...
    function claimReferReward() external {
        ...
        address sender = _msgSender();
        require(sender != address(0), "CLAIM:  Invalid address");
        ...
        super._transfer(address(this), sender, _referReward.claimable);
        ...
        payable
    {
        ...
        nodeRewardManager.payNodeMaintenanceFee(msg.sender, creationTime);
    }
        ...
        returns (INODERewardManagement)
    {
        ...
        if (index == 0) {
            return node1RewardManager;
        } else if (index == 1) {
            return node2RewardManager;
        } else {
            return node3RewardManager;
        }
    }
    ...
    function updateRewardClaimStatus(bool _status) external onlyOwner {
    ...
    {
        node1 = node1RewardManager._getNodeNumberOf(account);
        node2 = node2RewardManager._getNodeNumberOf(account);
        node3 = node3RewardManager._getNodeNumberOf(account);
    }
    ...
    {
        node1Time = node1RewardManager._getNodesCreationTime(account);
        node2Time = node2RewardManager._getNodesCreationTime(account);
        node3Time = node3RewardManager._getNodesCreationTime(account);
    }
    ...
    {
        node1Reward = node1RewardManager._getRewardAmountOf(account);
        node2Reward = node2RewardManager._getRewardAmountOf(account);
        node3Reward = node3RewardManager._getRewardAmountOf(account);
    }

    function changeNodePrice(uint256[3] memory newNodePrice) public onlyOwner {
        node1RewardManager._changeNodePrice(newNodePrice[0]);
        node2RewardManager._changeNodePrice(newNodePrice[1]);
        node3RewardManager._changeNodePrice(newNodePrice[2]);
    }
    ...
    {
        node1Price = node1RewardManager.nodePrice();
        node2Price = node2RewardManager.nodePrice();
        node3Price = node3RewardManager.nodePrice();
    }

    function changeRewardPerNode(uint256[3] memory newPrice) public onlyOwner {
        node1RewardManager._changeRewardPerNode(newPrice[0]);
        node2RewardManager._changeRewardPerNode(newPrice[1]);
        node3RewardManager._changeRewardPerNode(newPrice[2]);
    }
    ...
    {
        node1Reward = node1RewardManager.rewardPerNode();
        node2Reward = node2RewardManager.rewardPerNode();
        node3Reward = node3RewardManager.rewardPerNode();
    }

    function changeClaimTime(uint256[3] memory newTime) public onlyOwner {
        node1RewardManager._changeClaimTime(newTime[0]);
        node2RewardManager._changeClaimTime(newTime[1]);
        node3RewardManager._changeClaimTime(newTime[2]);
    }
    ...
    {
        node1Time = node1RewardManager.claimTime();
        node2Time = node2RewardManager.claimTime();
        node3Time = node3RewardManager.claimTime();
    }

    function changeAutoDistri(bool[3] memory newMode) public onlyOwner {
        node1RewardManager._changeAutoDistri(newMode[0]);
        node2RewardManager._changeAutoDistri(newMode[1]);
        node3RewardManager._changeAutoDistri(newMode[2]);
    }
    ...
    {
        node1Mode = node1RewardManager.autoDistri();
        node2Mode = node2RewardManager.autoDistri();
        node3Mode = node3RewardManager.autoDistri();
    }

    function changeGasDistri(uint256[3] memory newGasDistri) public onlyOwner {
        node1RewardManager._changeGasDistri(newGasDistri[0]);
        node2RewardManager._changeGasDistri(newGasDistri[1]);
        node3RewardManager._changeGasDistri(newGasDistri[2]);
    }
    ...
    {
        node1Gas = node1RewardManager.gasForDistribution();
        node2Gas = node2RewardManager.gasForDistribution();
        node3Gas = node3RewardManager.gasForDistribution();
    }
    ...
    function getDistriCount()
        ...
        view
        returns (
            uint256 node1GasCount,
            uint256 node2GasCount,
            uint256 node3GasCount
        )
    {
        node1GasCount = node1RewardManager.lastDistributionCount();
        node2GasCount = node2RewardManager.lastDistributionCount();
        node3GasCount = node3RewardManager.lastDistributionCount();
    }

    function distributeRewards(uint256 nodeIndex)
        public
        onlyOwner
        returns (
            uint256,
    ...
    function publiDistriRewards(uint256 nodeIndex) public {
        INODERewardManagement nodeRewardManager = getNodeRewardManager(
            nodeIndex
        );
    ...
    function getTotalStakedReward(uint256 nodeIndex)
        public
        view
        returns (uint256)
    {
...
interface INODERewardManagement {
    ...
    function autoDistri() external view returns (bool);

    function gasForDistribution() external view returns (uint256);

    function lastDistributionCount() external view returns (uint256);
