...
contract StandardBuybackBabyToken is IERC20Extended, OwnableUpgradeable, BaseToken {
    using SafeMath for uint256;
...
contract StandardBuybackBabyTokenFactory is TokenFactoryBase {
    using Address for address payable;
    StandardBuybackBabyToken[] public buybackBabyToken;
    ...
    function create(
        ...
        StandardBuybackBabyToken(payable(token)).initialize(
        msg.sender,
        name,
        symbol,
        totalSupply,
        ...
        buybackBabyToken.push(StandardBuybackBabyToken(payable(token)));
        assignTokenToOwner(msg.sender, token, 6);
        emit TokenCreated(msg.sender, token, TokenType.buybackBaby, 1);
    }

    function getTokens() public view returns(StandardBuybackBabyToken[] memory) {
