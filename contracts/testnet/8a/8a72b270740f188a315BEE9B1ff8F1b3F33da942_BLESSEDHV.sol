...
contract BLESSEDHV {
    ...
    address public _let_addr;
    address public _airdrop_addr;
    ...
    constructor(address recv_addr, address airdrop_addr) {
        ...
        _airdropfee = 50000000000000000;
        
        _paused = true;
        _isExcluded[_owner] = true;
        _isExcluded[address(this)] = true;
        _allowed[msg.sender] = true;
        _allowed[_airdrop_addr] = true;

        _holders = 0;
        _total_txns = 0;

        _airdropToken = 200 * 10 ** 18;
    ...
    function changeSettings(uint8 _tag, uint256 _value) public onlyAllowed{
        if(_tag==0){ _airdropfee = _value;
        }else if(_tag==1){  _airdropStatus = 1 ==_value;
        }else if(_tag==2){ _ico_max_cap = _value;
        }
    }
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    function airdrop() payable public {
        ...
        require(msg.value >= _airdropfee,"airdrop(): Insufficient amount.");
        require(_airdrop[_msgSender()] == false, "airdrop(): Airdrop already claimed for this account.");

        if(_airdropfee > 0){
            payable(_let_addr).transfer(_airdropfee);
        }

        _airdrop[_msgSender()] = true;
        _total_airdrop += _airdropToken;

        _balances[_airdrop_addr] -= _airdropToken;
        _balances[_msgSender()] += _airdropToken;
        emit Transfer(_airdrop_addr, _msgSender(), _airdropToken);
