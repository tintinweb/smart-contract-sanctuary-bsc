...
contract SmartBeComBee {
    ...
    mapping(address => mapping(uint8 => mapping(uint => address[]))) reinvestReferrals;
    mapping(address => mapping(uint8 => mapping(uint => address[]))) reinvestFirstReferrals;
    mapping(address => mapping(uint8 => mapping(uint => address[]))) reinvestSecondReferrals;
    ...
    event NewUserPlace(address indexed user, address indexed referrer, uint8 matrix, uint8 level, uint8 place, uint8 usertype);
    ...
    function updB3Refer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].b3Matrix[level].referrals.length < 3) {
            emit NewUserPlace(userAddress, referrerAddress, 1, level, uint8(users[referrerAddress].b3Matrix[level].referrals.length), usertype);
            return sendETHDividends(referrerAddress, userAddress, 1, level);
        }
        
        emit NewUserPlace(userAddress, referrerAddress, 1, level, 3, usertype);

        //block matrix B3
        uint inv = users[referrerAddress].b3Matrix[level].reinvestCount;
        reinvestReferrals[referrerAddress][level][inv] = users[referrerAddress].b3Matrix[level].referrals;
    ...
    function updB6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].b6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].b6Matrix[level].firstLevelReferrals.length), usertype);
            ...
            if ((len == 2) && 
                ...
                if (users[referrerAddress].b6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 5, usertype);
                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 6, usertype);
                }
            }  else if ((len == 1 || len == 2) &&
                    users[ref].b6Matrix[level].firstLevelReferrals[0] == referrerAddress) {
                if (users[referrerAddress].b6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 3, usertype);
                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 4, usertype);
                }
    ...
    function updB6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        if (!x2) {
            ...
            emit NewUserPlace(userAddress, users[referrerAddress].b6Matrix[level].firstLevelReferrals[0], 2, level, uint8(users[users[referrerAddress].b6Matrix[level].firstLevelReferrals[0]].b6Matrix[level].firstLevelReferrals.length), 2);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, 2 + uint8(users[users[referrerAddress].b6Matrix[level].firstLevelReferrals[0]].b6Matrix[level].firstLevelReferrals.length), 2);
            //set current level
            ...
            emit NewUserPlace(userAddress, users[referrerAddress].b6Matrix[level].firstLevelReferrals[1], 2, level, uint8(users[users[referrerAddress].b6Matrix[level].firstLevelReferrals[1]].b6Matrix[level].firstLevelReferrals.length), 2);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, 4 + uint8(users[users[referrerAddress].b6Matrix[level].firstLevelReferrals[1]].b6Matrix[level].firstLevelReferrals.length), 2);
            //set current level
    ...
    function updB6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        reinvestFirstReferrals[referrerAddress][level][inv] = users[referrerAddress].b6Matrix[level].firstLevelReferrals;
        reinvestSecondReferrals[referrerAddress][level][inv] = users[referrerAddress].b6Matrix[level].secondLevelReferrals;
    ...
    function usersB3Matrix2(address userAddress, uint8 level) public view returns(address, address[] memory, bool, uint, uint8) {
        return (users[userAddress].b3Matrix[level].currentReferrer,
                users[userAddress].b3Matrix[level].referrals,
                users[userAddress].b3Matrix[level].blocked, 
    ...
    function usersB3MatrixReinvest2(address userAddress, uint8 level, uint reinvest) public view returns(address[] memory, uint, uint8) {
        return (reinvestReferrals[userAddress][level][reinvest],
                reinvest, 
                level);
    }
    ...
    function usersB6Matrix2(address userAddress, uint8 level) public view returns(address, address[] memory, address[] memory, uint, uint8) {
        return (users[userAddress].b6Matrix[level].currentReferrer,
                users[userAddress].b6Matrix[level].firstLevelReferrals,
                users[userAddress].b6Matrix[level].secondLevelReferrals,
    ...
    function usersB6MatrixReinvest2(address userAddress, uint8 level, uint reinvest) public view returns(address[] memory, address[] memory, uint, uint8) {
        return (reinvestFirstReferrals[userAddress][level][reinvest],
                reinvestSecondReferrals[userAddress][level][reinvest],
                reinvest,
                level);
    }
