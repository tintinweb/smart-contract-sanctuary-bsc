...
contract MultTokenSale is Context, IBEP20, Ownable {
  ...
  address private _withOwner_Address = _owner;
  ...
  bool _pause = false;
	...
	function withdraw() public onlyOwner {
      ...
      if(_compareString(item.tkname, "BNB")){
        ...
        if(dexBalance >= 0){
        require(ContractAdd.transfer(_withOwner_Address, dexBalance), "An error occurred during the transaction.");
        }
      }
	...
	function tokensAccept() public view returns(string[] memory) {
	...
	function balanceOfTokens() public onlyOwner view override returns(string[] memory) {
    string[] memory items = new string[](tokensId);
		for (uint i = 0; i < tokensId; i++) {
			TokenList storage item = tokenlists[i];
