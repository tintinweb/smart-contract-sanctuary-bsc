...
contract Token is IERC20, Ownable, Pausable {
  ...
  uint256 private _collectedFeeThreshold = DEFAULT_GONSWAP_THRESHOLD;
  ...
  uint256 public athPriceDeltaPercent = 1;
  ...
  function setCollectedFeeThreshold(uint256 amount) external onlyOwner {
    _collectedFeeThreshold = amount.mul(_gonsPerFragment);
  }
  ...
  function checkCollectedFeeThreshold() external view returns (uint256) {
    return _collectedFeeThreshold.div(_gonsPerFragment);
  }
  ...
  function _getDailySellLimitAmountInternal(address _address) internal view returns (uint256) {
    uint256 factor = _getDailySellLimitFactor(_address);
    uint256 bal = _gonBalances[_address];
    // IERC20(address(this)).balanceOf(_address);
  ...
  function getDailySellLimitAmount(address _address) public view returns (uint256) {
    return _getDailySellLimitAmountInternal(_address).div(_gonsPerFragment);
  }
  ...
  function _checkDailySellLimitAndUpdateSaleHistory(address sender, uint256 gonAmount) private {
    ...
    if (timeElapsed < 1 days) {
      ...
      uint256 limitAmount = _getDailySellLimitAmountInternal(sender);
  ...
  function _swapBack() internal swapping {
    ...
    uint256 balanceBUSDBefore = IERC20(BUSD).balanceOf(treasury);
    ...
    path[0] = address(this);
    path[1] = BUSD;
    // this contract can't receive BUSD so it delegates received BUSD to treasury
    router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountToSwap, 0, path, treasury, block.timestamp);

    uint256 balanceBUSDAfter = IERC20(BUSD).balanceOf(treasury);
    ...
    // transfer BUSD amount to add liquidity from the treasury
    IERC20(BUSD).transferFrom(autoLiquidityReceiver, address(this), amountBUSDLiquidity);
    ...
    uint256 remainBUSD = IERC20(BUSD).balanceOf(address(this));
    if (remainBUSD > 0) {
      IERC20(BUSD).transfer(treasury, remainBUSD);
    }
  }
  ...
  function _shouldSwapBack() private view returns (bool) {
    return autoAddLiquidity && !_inSwap && msg.sender != pair && _gonBalances[address(this)] >= _collectedFeeThreshold;
  }

  function _shouldAddLiquidity() private view returns (bool) {
    return autoAddLiquidity && !_inSwap && msg.sender != pair && _gonBalances[autoLiquidityReceiver] >= _collectedFeeThreshold;
  }

  function _shouldCollectFees() private view returns (bool) {
    return autoCollectFees && !_inSwap && msg.sender != pair && _gonBalances[address(this)] >= _collectedFeeThreshold;
  }
  ...
  function setAthDeltaPercent(uint256 percent) external onlyOwner {
    require(percent > 0 && percent < 100, "INVALID_PERCENT");
  }
  ...
  function _estimateNegativeRebaseSupply() private view returns (uint256) {
    ...
    // https://github.com/Uniswap/v2-periphery/blob/0335e8f7e1bd1e8d8329fd300aea2ef2f36dd19f/contracts/libraries/UniswapV2Library.sol#L53
    uint256 expectedAmountOut = athPrice.add(athPrice.mul(athPriceDeltaPercent).div(100));
    ...
    uint256 expectedReserveIn = expectedDenominator.sub(amountInWithFee).div(1000);
