...
contract Dripto is Context, IERC20, Ownable {
    ...
    address payable public maintenanceWalletAddress = payable(0xd9Cb687EccC712A045eE3Af1542A4b057d8611C1); // Maintenance Address
    address payable public rewardsWalletAddress = payable(0xF2e913F5BFa727a745d88903FFA9949990A5Af8c); // Rewards Address
    address payable public buybackWalletAddress = payable(0xff8cC92700744ebF425fac18d546d020bd9C8459); // Buyback Address
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(bnbForMarketing > 0)
            transferToAddressBNB(marketingWalletAddress, bnbForMarketing);

        if(bnbForMaintenance > 0)
            transferToAddressBNB(maintenanceWalletAddress, bnbForMaintenance);

        if(bnbForRewards > 0)
            transferToAddressBNB(rewardsWalletAddress, bnbForRewards);

        if(bnbForBuyback > 0)
            transferToAddressBNB(buybackWalletAddress, bnbForBuyback);
