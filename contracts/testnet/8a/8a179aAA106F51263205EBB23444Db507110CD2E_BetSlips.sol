...
contract BetSlips is IBetSlips, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function debit(
        ...
        require(
            IERC20Upgradeable(token).balanceOf(player) >= wagerAmount,
            "Insufficient balance"
        );

        require(
            IERC20Upgradeable(token).allowance(player, address(this)) >= wagerAmount,
            "Insufficient allowance"
        );

        IERC20Upgradeable(token).transferFrom(player, address(this), wagerAmount);
    }
    ...
    function placeBetSlip(
        ...
        balance.amountBefore = IERC20Upgradeable(token).balanceOf(player) + wagerAmount;
        balance.amountAfter = IERC20Upgradeable(token).balanceOf(player);
        balance.decimals = ERC20(token).decimals();
    ...
    function completeBet(
        ...
        if (returnAmount > 0) {
            IERC20Upgradeable(betSlip.token).transfer(
                betSlip.player,
        ...
        Balance memory balance;
        balance.amountBefore = IERC20Upgradeable(betSlip.token).balanceOf(betSlip.player) - returnAmount;
        balance.amountAfter = IERC20Upgradeable(betSlip.token).balanceOf(betSlip.player);
        balance.decimals = ERC20(betSlip.token).decimals();
    ...
    {
        require(
            amount <= IERC20Upgradeable(tokenERC20).balanceOf(address(this)),
            "ERC20: amount is greater than currentBalance"
        );

        IERC20Upgradeable(tokenERC20).transfer(owner, amount);
    }
}