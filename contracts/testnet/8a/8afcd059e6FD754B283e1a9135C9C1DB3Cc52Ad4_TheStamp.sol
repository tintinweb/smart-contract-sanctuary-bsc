...
contract TheStamp is Context, Ownable {
    ...
    function networkInvitePayout(address addr, uint256 amount) public isInitialized {   
        ...
        if(user.networkSponsor != address(0)) {
            ...
            uint256 finalInviteBonus = inviteBonus.div(2); //1%

            //2% for referee -- 1% will go to direct deposit, 1% will be transfered to referee wallet.
            user.totalDirectDeposits     += finalInviteBonus; // invested
            user.totalDepositAmount      += finalInviteBonus;  // invested + compounded
            usersBonus[addr].inviteBonus += inviteBonus; //statistics record.
            token.transfer(addr, finalInviteBonus); 
            
            //2% for referrer -- 1% will go to direct deposit, 1% will be transfered to referrer wallet.
            networkSponsor.totalDirectDeposits += finalInviteBonus; //invested
            networkSponsor.totalDepositAmount  += finalInviteBonus; //invested + compounded
            usersBonus[sponsor].inviteBonus    += inviteBonus; //statistics record.
            token.transfer(sponsor, finalInviteBonus); 
    ...
    function withdraw() public isInitialized {
        ...
        if(payoutPostTax > 0 && user.totalDepositPayouts < maxPayout){ // avoid reverts
            //yieldPercentage will be deducted 0.5% every withdraw, starts at 2% else, if daily yield is less than 2% yield it goes back to 1.5%
...
/**     
    ...
    Features:
    Retroactive variable daily yield starts at 1.5% up to 3.5%:

    - Compound action every 24 hours will give the investor an additional 0.1% for their current yield which can increase up to 3.5%
        - Compound is not mandatory, but compounding will increase the investors daily yield.
        - There is a 3% sustainability tax every compound in which that 3% will stay in the contract. This makes the platform have a deflationary behavior.
        - User's are free to compound at any time, but compound bonuses will only take effect when a compound was done after 24 hours of last compound action.

    - Every withdraw/sell action, will have a 0.5% deduction to the user's current yield.
        - if the user's current daily yield is between 1.5% to 2.0%, Then when the user sell/withdraw their dividends the daily yield will go back to 1.5%
        - if the user's current daily yield is between 1.6% to 3.5%, then yield will decrease 0.5% (i.e user current yield is 3.5%, after sell/withdraw yield will decrease to 3.0%)
    
    See Breakdown Below:
    user's current yield % | user's yield % after withdraw | yield % deduction
    1.5%                     1.5%                            0.0%
    1.6%                     1.5%                            0.1% 
    1.7%                     1.5%                            0.2%
    1.8%                     1.5%                            0.3% 
    ...
    - Users can compound up to 5x max of his real deposited amount(ie. real deposited amount = 100 ether, max compound amount = 500 ether, max payout = 1,750 ether)
    ...
    - Variable Withdraw Tax:
        ...
        Compound Count is within 8 to 14 Days: 8%
        Compound Count is within 15 to 21 Days: 6%
        Compound Count is within 22 to 28 Days: 4%
        Compound Count is within 29 to 35 Days: 4%
        Compound Count is 35 Days and higher: 0%

    Consecutive Compound Count/Days | Sustainability tax 
     1 to  7 Days                     10%
     8 to 14 Days                      8%
    14 to 21 Days                      6%
    22 to 28 Days                      4%
    ...
    * Every sell/withdraw the compound count will be set back to 0/10% Sustainability Tax.  

    2% Referral Bonus and 2% Referee Cashback Bonus (4% total invite bonus):
    
    - 2% for referee -- 1% will go to direct deposit, 1% will be transfered to referee wallet.
    - 2% for referrer -- 1% will go to direct deposit, 1% will be transfered to referrer wallet.

    Network Sponsorship:
    - If an investor used a referral link that is not part in to the network, the protocol will automatically setup a network leader on the investor behalf.
    - Referral links will only work if the network invite address is already part in the network society.
    - Once successfully deposited in the protocol, investors can start creating their own network by inviting new investors under their own invite link.

    Max reward accumulation of 48 Hours.
    - If no action is done within 48 hours, reward accumulation will stop and accumulation of rewards will only reset if a deposit, withdraw or compound action is done.
    ...
    Daily Top Investors:
    - The winning address will be awarded with 5% of his deposited amount(i.e deposited amount is 1,000 ether - less the deposit tax(10%) = 900 ether, reward will be 45 ether). 
    - Reward will be directly sent to the winning address after the event draw. 
    - Event can run every 1 day to 7 days depending on the project owners setting.
    ...
    - The pot for each round has a 2,500 ether limit. 
    - Event can run every 1 hour to 24 hours depending on the project owners setting.
    
    Netwrok Airdrop(Development in-progress).
    ...
    Auto Compound Feature(Development in-progress):
