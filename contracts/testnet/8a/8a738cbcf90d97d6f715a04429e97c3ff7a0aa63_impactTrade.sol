...
contract impactTrade is Initializable, OwnableUpgradeable {
    ...
    {
        return (serviceValue, sellervalue, royalty);
    }
    ...
    function _saleToken(
        ...
        if (
            ...
            require(msg.value == ldatas[6], "Insufficient Balance");
            ...
            require(
                ldatas[2] >= ldatas[6].div(10**ldatas[1]),
                "Insufficient Balance"
            );
            (ldatas[3], ldatas[4], ldatas[5]) = calc(
                ids[1],
            ...
            require(
                ldatas[2] >=
                    (ldatas[3].add(ldatas[4].add(ldatas[5]))).div(
                        10**ldatas[1]
                    ),
                "Insufficient Balance"
            );
    ...
    function _acceptBId(
        ...
        require(
            ldatas[2] >= ldatas[6].add(ids[1]).div(10**ldatas[1]) &&
                ldatas[6] > 0,
            "Insufficient Balance"
        );
        (ldatas[3], ldatas[4], ldatas[5]) = calc(
        ...
        require(
            ldatas[2] >=
                (ldatas[3].add(ldatas[4].add(ldatas[5]))).div(10**ldatas[1]),
            "Insufficient Balance"
        );
        if (ldatas[3] != 0) {
    ...
    //     // _cRoyalty.collectionId = id_;
    //     // _cRoyalty.colletionRoyalty = _royalty;
    //     // uint totalper = 0;
    //     // for(uint i = 0; i < _splitpercentage.length; i++){
    //     //     totalper = totalper.add(_splitpercentage[i]);
    //     //     _cRoyalty.spliters[i] = splitaddress[i];
    //     //     _cRoyalty.splitpercentage[i] = _splitpercentage[i];
    //     // }
    //     // require(totalper == 100000000000000000000, "Percentage Mismatch");
    //     cRoyaltyDetails[id_] = _setRoyalty(
    //         _splitpercentage,
    //         splitaddress,
    //         _royalty,
    //         _tid
    //     );
    //     emit ColletionId(id_, _royalty);
    // }
    ...
    function _setRoyalty(
        ...
        uint256 _royalty,
        uint256 tokenId
    ) internal pure returns (collectionRoyalty memory) {
        require(
        ...
        _cRoyalty.colletionRoyalty = _royalty;
        uint256 totalper = 0;
        for (uint256 i = 0; i < _splitpercentage.length; i++) {
            totalper = totalper.add(_splitpercentage[i]);
            _cRoyalty.spliters[i] = splitaddress[i];
            _cRoyalty.splitpercentage[i] = _splitpercentage[i];
        }
    ...
    function minting(
        ...
        //     keccak256(abi.encodePacked(("Impact")))
        // ) {
        //     cRoyaltyDetails[id_] = _setRoyalty(
        //         _splitpercentage,
        //         splitaddress,
    ...
    function batchMinting(
        ...
        _tid = _tid.add(size);
        if (nftType == 721) {
            ImpactCollective721(impact721).mintBatch(
                ipfsmetadata,
                from,
                to,
                _tid,
            ...
            ImpactCollective1155(impact1155).mintBatch(
                ipfsmetadata,
                from,
                to,
                supply,
    ...
    function setRoyalty(uint256 roy) public onlyOwner {
        royalty = roy;
    }
}