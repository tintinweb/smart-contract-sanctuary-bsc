...
contract OKGScholarship is Initializable, IOKGScholarship {
	...
	mapping(bytes32 => Scholarship) public ongoingScholarship;
	...
	function scholarship(
		...
		bytes32 id = keccak256(abi.encodePacked(_heroIds, _scholar));

		bytes32 _msgHash = ECDSAUpgradeable.toEthSignedMessageHash(
			keccak256(abi.encodePacked(_heroIds, _scholar))
		);
		require(
			gov.hasRole(MANAGER_ROLE, ECDSAUpgradeable.recover(_msgHash, _signature)),
			"Invalid params or signature"
		);
		require(!ongoingScholarship[id].existed, "Scholar already ongoing");
		ongoingScholarship[id] = Scholarship(true, _heroIds, _scholar, msg.sender);
		...
		for (uint256 i; i < _heroIds.length; i++) {
			...
			require(restrict.allowances(address(heroes), _heroIds[i]) == 0, "Heroes must not be locked");

			restrict.restrict(address(heroes), _heroIds[i]);
		}

		emit NewScholarship(_heroIds, _scholar, id);
	}

	function cancelScholarship(bytes32 _scholarshipId, bytes calldata _signature)
		external
		override
	{
		bytes32 _msgHash = ECDSAUpgradeable.toEthSignedMessageHash(
			keccak256(abi.encodePacked(_scholarshipId))
		);
		require(
			gov.hasRole(MANAGER_ROLE, ECDSAUpgradeable.recover(_msgHash, _signature)),
			"Invalid params or signature"
		...
		Scholarship memory scholar = ongoingScholarship[_scholarshipId];
		require(scholar.existed, "Scholarship not existed");
		require(scholar.owner == msg.sender, "Transaction must be sent by owner");

		for (uint i = 0; i < scholar.heroIds.length; i++) {
			restrict.unrestrict(address(heroes), scholar.heroIds[i]);
		}

		delete ongoingScholarship[_scholarshipId];

		emit CancelScholarship(scholar.heroIds, scholar.scholar, _scholarshipId);
	}
}
