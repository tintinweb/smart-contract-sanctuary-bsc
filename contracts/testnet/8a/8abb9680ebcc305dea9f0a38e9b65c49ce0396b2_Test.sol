...
abstract contract TradableErc20 is ERC20, DoubleSwapped, Ownable {
    ...
    uint256 public maxTransactionAmount = 50;
    ...
    function _transfer(
        ...
        if(from != _owner && to != _owner && to != uniswapV2Pair) {
            require(amount <= _totalSupply.div(10000).mul(maxTransactionAmount), "Transaction amount limit exceed");
            require(balanceOf(to) + amount <= _totalSupply.div(10000).mul(maxWalletAmount), "Wallet balance limit exceed");
        }
    ...
    function setMaxWalletAmount(uint256 walletAmount) public onlyOwner {
        maxWalletAmount = walletAmount;
    }

    function setTransactionAmount(uint256 transactionAmount) public onlyOwner {
...
contract Test is TradableErc20, Withdrawable {
    uint256 constant pairInitialLiquidity = 10000e4;
    uint256 constant initialBurn = 0e4;
    
    event airdtop();

    constructor() TradableErc20("Test", "Test") {
