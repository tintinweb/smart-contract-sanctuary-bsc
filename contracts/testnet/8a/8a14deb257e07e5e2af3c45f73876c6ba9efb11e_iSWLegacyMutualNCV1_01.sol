...
iS.StudioWorks Legacy Mutual Bank Decentralized 
 Buy Locked Savings Bond, Earn Interest, Cash Out for Native Network Coins. 
  ...
  100000 BNB max investment per wallet.
...
contract iSWLegacyMutualNCV1_01 is BankSettingsFunctions {
    ...
    event NewInvestor(address invAddr, uint256 invAmount, uint256 amtBonds, uint256 bondValue, uint256 leXCValue, uint256 prtnrshpInt, uint256 TVL);
    ...
    //fetches latest BUSD/BNB price
        //BUSD-BNB MAINNET CA: 0x1B96B92314C44b159149f7E0303511fB2Fc4774f
        //BUSD-BNB TESTNET CA: 0x85EcDcdd01EbE0BfD0Aba74B81Ca6d7F4A53582b
    //function getLatestPrice() internal view returns (uint256) {
    //    PancakePair fetch = PancakePair(0x85EcDcdd01EbE0BfD0Aba74B81Ca6d7F4A53582b);
    //    (uint112 prBNB, uint112 prBUSD, uint32 _blockTimestampLast) = fetch.getReserves();
    //    return uint256(prBUSD / prBNB);
    //}
    ...
    function buyBondsWithCoins(address ref) external payable {
        ...
        if(user.referrer != address(0) && user.initialInvestmentAmt == 0) {
            ...
            refRewardsBonds = refRewardsLeXC.div(calculateIntRate(user.referrer));
            ...
            users[user.referrer].refRewardsReceivedTotal += refRewardsBonds;
            //emit RefRewardsSent(msg.sender, user.referrer, refRewardsBonds);
            emit RefRewardsReceived(user.referrer, msg.sender, refRewardsBonds);
    ...
    function internalCompFunction(address adr, uint256 amtComp, bool isBWC, bool isAuto) internal {
        ...
        if(isBWC){
            ...
            if(user.initialInvestmentAmt == 0) {
                ...
                userinfo[adr].initialLeXCValue = calculateLeXCWithdraw(1);
                userinfo[adr].initialBondValue = calculateLeXCWithdraw(1 * calculateIntRate(adr));
                userinfo[adr].initialInvestmentTime = block.timestamp;
                user.lastWithdrawTime = block.timestamp;
                userinfo[adr].lastBonusCompTime = block.timestamp;
                emit NewInvestor(adr, user.initialInvestmentAmt, user.Bonds, userinfo[adr].initialBondValue, userinfo[adr].initialLeXCValue, getPartnershipBonusBPS(adr), getBalance());
            }
