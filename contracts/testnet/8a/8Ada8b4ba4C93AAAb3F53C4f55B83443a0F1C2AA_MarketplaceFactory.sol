...
contract MarketplaceFactory is Initializable, UUPSUpgradeable, OwnableUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Create721A(address token);
     ...
     * @param _erc721ABeacon, address of the beacon implementation.
     * @param _erc721ABeaconProxy, address of the beacon implementation.
     * @param _erc1155Beacon, address of the beacon implementation.
     * @param _erc1155BeaconProxy, address of the beacon implementation.
     */
    function initialize(address operator, address _erc721Beacon, address _erc721BeaconProxy,address _erc721ABeacon, address _erc721ABeaconProxy, address _erc1155Beacon, address _erc1155BeaconProxy) public initializer {
        ...
        require(_erc721Beacon != address(0) && _erc1155Beacon!= address(0) && _erc721ABeacon != address(0), "Factory: Beacon address cant be zero address");
        require(_erc721BeaconProxy != address(0) && _erc1155BeaconProxy!= address(0) && _erc721ABeaconProxy != address(0), "Factory: Beacon Proxy address cant be zero address");
        ...
        erc721ABeacon = _erc721ABeacon;
        erc1155Beacon = _erc1155Beacon;
        ERC721 = _erc721BeaconProxy;
        ERC721A = _erc721ABeaconProxy;
    ...
    /**
     * @dev Creates an ERC721A token by the caller with OPERATOR_ROLE.
     *
     * @param _tokenName.
     * @param _tokenSymbol.
     * @param _baseUri.
     * @param _salt.
     * 
     * Emits a {Create721A} event indicating the token address.
     */
    function createERC721AToken(string memory _tokenName, string memory _tokenSymbol, string memory _baseUri, uint _salt) 
        external 
        onlyRole(OPERATOR_ROLE) 
        whenNotPaused 
        nonReentrant 
    {
        address beaconProxy = deployProxy(getERC721AData(_tokenName, _tokenSymbol), _salt, tokenType.ERC721A);
        EKTA_ERC721A token = EKTA_ERC721A(address(beaconProxy));
        emit Create721A(address(token));
     ...
     */
    function updateBeacon(address _erc721Beacon, address _erc721ABeacon, address _erc1155Beacon) external onlyRole(UPDATER_ROLE) {
        require(_erc721Beacon != address(0) && _erc721ABeacon != address(0) && _erc1155Beacon != address(0), "Factory: Cant be zero address");
        erc721Beacon = _erc721Beacon;
        erc721ABeacon = _erc721ABeacon;
        erc1155Beacon = _erc1155Beacon;
    }
     ...
     */
    function updateBeaconProxy(address _erc721BeaconProxy, address _erc721ABeaconProxy, address _erc1155BeaconProxy) external onlyRole(UPDATER_ROLE) {
        require(_erc721BeaconProxy != address(0) && _erc721ABeaconProxy != address(0) && _erc1155BeaconProxy != address(0), "Factory: Cant be zero address");
        ERC721 = _erc721BeaconProxy;
        ERC721A = _erc721ABeaconProxy;
        ERC1155 = _erc1155BeaconProxy;
    }
     ...
     * @param _symbol.
     */
    function getERC721AData(string memory _name, string memory _symbol) 
        view 
        internal 
        returns(bytes memory) 
    {
        return abi.encodeWithSelector(EKTA_ERC721A(address(ERC721A)).initialize.selector, _name, _symbol);
    }
    ...
    // returns address of the contract with provided arguments
    function getERC721AAddress(string memory _name, string memory _symbol, uint _salt)
        public
        view
        returns (address)
    {
        bytes memory bytecode = getCreationBytecode(getERC721AData(_name, _symbol), tokenType.ERC721A);
    ...
    function getCreationBytecode(bytes memory _data, tokenType _tokenType) 
        ...
        returns (bytes memory) 
    {
        if(keccak256(abi.encodePacked(_tokenType)) == keccak256(abi.encodePacked(tokenType.ERC721A))){
            return abi.encodePacked(
                type(EKTABeaconProxy).creationCode,  abi.encode(erc721ABeacon) , _data
            );
        }
        return abi.encodePacked(
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function updateEnableWhitelisting(bool enable)
        ...
        nonReentrant
    {
        require(enableWhitelisting != enable, "ERC721A: Already in same status");
    ...
    function includeInWhitelist(address account)
        ...
        nonReentrant
    {
        require(account != address(0), "ERC721A: Account cant be zero address");
        require(
            !whitelisted[account],
            "ERC721A: Account is already whitelisted"
        );
    ...
    {
        require(account != address(0), "ERC721A: Account cant be zero address");
        require(whitelisted[account], "ERC721A: Account is not whitelisted");
