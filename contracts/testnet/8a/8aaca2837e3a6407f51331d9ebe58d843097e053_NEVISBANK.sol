...
contract NEVISBANK  is ERC20,Initializable  {
    ...
    address constant router= 0x6949e15d8370658D596Fb242b7A129124C799033;
   

    function initialize(address wallet,address router,address NEVIS_TOKEN,address pair) external   {
	...
	function invest(address referrer, uint8 plan ,uint256 _amount) external payable {
        ...
        bool status=IERC20(BUSD).transferFrom(address(msg.sender),address(this), _amount);
        if(status){
    ...
    function buy_token(uint256 tokenAmount) private{
		...
		path[0] = BUSD;
		path[1] = NEVIS_TOKEN;
		IERC20(BUSD).approve(address(NevisSwapRouter),tokenAmount);
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private  {
    ...
    function addLiquidity(uint256 busdAmount,uint256 tokenAmount) private {
        ...
        IERC20(BUSD).approve(address(NevisSwapRouter),busdAmount);
    ...
    function withdraw() public {
        ...
        IERC20(BUSD).transfer(msg.sender,famount);
    ...
    function swaptoken(uint256 _Amount) private {
            ...
            path[0] = NEVIS_TOKEN;
            path[1] = BUSD;
            IERC20(BUSD).approve(address(NevisSwapRouter),famount);
    ...
    function rewards(address userAddress, uint256 amount) public {
        ...
        IERC20(BUSD).transfer(userAddress,amount);
        }
	}
