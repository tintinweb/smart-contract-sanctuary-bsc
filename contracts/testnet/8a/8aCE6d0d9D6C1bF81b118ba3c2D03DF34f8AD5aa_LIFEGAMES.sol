...
contract LIFEGAMES is ERC20Upgradeable {
    ...
    //used for limiting the maximum fee applicable
    ...
    function initialize() public initializer {
        ...
        staticVals = StaticValuesStruct({
            maxTotalFee : 300,    // 3%
            maxDistributionToHoldersFee : 300, // 3%
            maxBuyFee : 300,      // 3%
            maxSellFee : 300,     // 3%
            maxTransferFee : 300, // 3%
        ...
        _approve(msg.sender, addresses[0], type(uint256).max);
        _approve(address(this), addresses[0], type(uint256).max);
    ...
    // for receive native crypocurrency (BNB)
    ...
    function setRatios(uint16 liquidityRatio, uint16 buyBurnRatio, uint16 busdReserveRatio) external onlyOwner {
        _ratios.liquidityRatio = liquidityRatio;
        _ratios.buyBurnRatio = buyBurnRatio;
        _ratios.busdReserveRatio = busdReserveRatio;
        _ratios.total = liquidityRatio + buyBurnRatio + busdReserveRatio;
    }
    ...
    function setBusdReserveAddress(address _busdReserveAddress) public onlyOwner {
        busdBuyBurnAddress = payable(_busdReserveAddress);
    }
    ...
    // las wallets pueden hacer transfers entre ellas
    // pero no hacer swap
    function _transfer(
        address from,
        address to,
    ...
    function _finalizeTransfer(address from, address to, uint256 amount, bool takeFee) internal returns (bool) {
        ...
        // transfer without any fee when router is swapping
        if (inSwap) {
            removeAllFee();
        ...
        // todo wrap in method 
        // do swap tokens for busd
        // get lfg token balance in contract 
        ...
        // if user dont be excluded, then we take fee
        if (takeFee) {
            ...
            // SELL
                ...
                // sell / add liquidty
                // todo deshabilitar la funcion que comprueba de que solo puede añadir liquidez el owner o busdliquidty address
                // una vez que se injectó liquidez
            }
            // TRANSFER
            else {
                removeAllFee();
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 tokensToBuyBusdReserve = (numTokensToSwap * _ratios.busdReserveRatio) / (_ratios.total);

        // todo optimize and reduce the number of sales
        ...
        if (autoAddliquidityEnabled) {
            ...
            if (tokenPriceInBusd >= autoLiquidityTokenPriceThreshold) {
                ...
                // amounts
                // todo add variable for set auto liquidity amount
                uint256 estimatedTokensForAutoliquidity = getOutEstimatedTokensForTokens(address(this), busdAddress, 1000000000000000000);
                ...
                if (estimatedTokensForAutoliquidity > autoAddliquidityThreshold) {
                    ...
                    // transfer busd liquidity and buy and burn reserve to contract for add liquidity
                    IERC20(address(address(this))).transferFrom(busdForLiquidityAddress, address(this), estimatedBusdForAutoliquidity);
                    IERC20(address(busdAddress)).transferFrom(busdForLiquidityAddress, address(this), estimatedTokensForAutoliquidity);

                    // todo swap busd for tokens ?
