...
pragma solidity ^0.8.2;
 ...
 contract AImedisMeta is ERC721,ERC721Enumerable, ERC721URIStorage,AccessControl, Ownable,ERC721Burnable  {
   ...
   address public USDT = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    ...
    uint256 public USDAmount = 2500000000000000000000;
    ...
    mapping (uint256 => uint256) public Aimxprice;
    ...
    event PriceUpdate(address indexed owner, uint256 oldPrice, uint256 newPrice,  uint256 oldtokenPrice, uint256 newtokenPrice,uint256 nftID);

    event NftListStatus(address indexed owner, uint256 nftID, bool isListed);
    

    constructor(string memory _Name,string memory _Symbol) ERC721(_Name,_Symbol)  {
         _mintingCurrency = ERC20(0xA678E3Bc7F81304A9dedd47cB8651aaf7b7d36a5) ; //SET AIMX Currency for Token
         _contractOwner = payable(msg.sender);
         uniswapRouter = IUniswapV2Router02(UNISWAP_ROUTER_ADDRESS);
    }

        function setUSDAmount(uint256 _newUSDAmount) onlyOwner external {
        USDAmount = _newUSDAmount;
        }
    ...
    function mint(string memory _tokenURI, address _toAddress, uint256 _price,uint256 _aimxPrice) public payable returns (uint256) {
        
        _tokenIdCounter.increment();
        uint256 _tokenId = _tokenIdCounter.current();
        price[_tokenId] = _price;
        Aimxprice[_tokenId]= _aimxPrice;
   ...
   function getEstimatedBNBforUSD() public view returns (uint[] memory) {
     return uniswapRouter.getAmountsOut(USDAmount, getpathforBNBUSDPrice());
   }
    ...
    path[1] = address(_mintingCurrency); 
    return path;
  }
  


     function buywithAimx(uint256 _id, uint256 _aimxPrice) external payable {
        _validate(_id);

        address _previousOwner = ownerOf(_id);
        address _newOwner = msg.sender;
        require(_aimxPrice >= getEstimatedBNBforAIMX()[1],"Please Enter Exact Amount to buy");  
    ...
    function mintBatch(address _toAddress, uint256 numberofNFT,string calldata _tokenURI,uint256 _price) public   {
    ...
    function updatePrice(uint256 _tokenId, uint256 _price, uint256 _aimxPrice) public returns (bool) {
        uint256 oldPrice = price[_tokenId];
        uint256 oldtokenPrice = Aimxprice[_tokenId];
        require(msg.sender == ownerOf(_tokenId), "Error, you are not the owner");
        price[_tokenId] = _price;
        Aimxprice[_tokenId]= _aimxPrice;

        emit PriceUpdate(msg.sender, oldPrice, _price,oldtokenPrice,_aimxPrice, _tokenId);
        return true;
    }
