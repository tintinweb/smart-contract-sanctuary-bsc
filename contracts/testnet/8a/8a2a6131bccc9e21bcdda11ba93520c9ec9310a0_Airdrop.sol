...
contract Airdrop {
   ...
   */
  function doAirdrop(IERC20 token, address[] calldata addresses, uint256 values) external returns (uint256) {
    uint256 i = 0;

    while (i < addresses.length) {
      token.safeTransferFrom(msg.sender, addresses[i], values);
      i += 1;
    }
