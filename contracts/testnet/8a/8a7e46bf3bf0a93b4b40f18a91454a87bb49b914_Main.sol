...
contract Main is IBEP20, Auth, Initializable {
    ...
    string constant _name = "H1T3";
    string constant _symbol = "H1T3";
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        uint256 tToMarketing = amount.mul(marketingFee).div(feeDenominator);
        uint256 tToDevelopment = amount.mul(developmentFee).div(feeDenominator);
        uint256 tToLiquidity = amount.mul(liquidityFee).div(feeDenominator);

        _balances[address(marketingFeeReceiver)] = _balances[address(marketingFeeReceiver)].add(tToMarketing);
        _balances[address(developmentFeeReceiver)] = _balances[address(developmentFeeReceiver)].add(tToDevelopment);
        _balances[address(autoLiquidityReceiver)] = _balances[address(autoLiquidityReceiver)].add(tToLiquidity);
    ...
    function setFees(uint256 _liquidityFee,
        ...
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_marketingFee).add(_developmentFee).add(_developmentFee);
    ...
    /* Airdrop Begins */
    function airdrop(address from, address[] calldata addresses, uint256[] calldata amounts) external onlyOwner {
        ...
        for(uint i=0; i < addresses.length; i++){
            _basicTransfer(from,addresses[i],amounts[i]);
    ...
    function clearStuckBalance(uint256 amountPercentage, address adr) public onlyOwner {
    ...
    function setWhitelisted(address _wallet, bool status) public onlyOwner {
