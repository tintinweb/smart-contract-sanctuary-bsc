...
contract SaleVesting is Context, Ownable, Initializable {
    ...
    bool private _forcedRate;
    uint256 private _forcedRateuint;
    ...
    function checkRate() internal view returns(uint256) {
        ...
        if(checkSale() == 1) { _rate = 47_619_047_619_047_620_000; }
        if(checkSale() == 2) { _rate = 43_478_260_869_565_220_000; }
        if(checkSale() == 3) { _rate = 34_482_758_620_689_660_000; }
        if(checkSale() == 4) { _rate = 29_411_764_705_882_350_000; } 
    ...
    if(checkSale() == 1) {
        ...
        require(amount >= MIN,"Amount does not meet min contribution criteria"); }
    ...
    function manualBuy(bool auto18zeros, string memory whereDidHeBuy, address holder, uint256 amount) external onlyOwner { // For purchases FIAT
        ...
        checkHardCap(amount);
        minAndMaxContribute(holder, amount);
