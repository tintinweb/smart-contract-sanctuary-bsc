...
contract NFTStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public farmerROI = 833e16;
    uint256 public farmerPlusROI = 2222e16;
    ...
    constructor (address _farmerNFT, address _farmerNFTPlus, address _mgm) public{
        farmerNFT = IERC721(_farmerNFT);
        farmerNFTPlus = IERC721(_farmerNFTPlus);
        mgm = IBEP20(_mgm);
    }

    function depositFarmer(uint256 _tokenId, address _erc721) public nonReentrant{
        require(_erc721 == address(farmerNFT) || _erc721 == address(farmerNFTPlus), "Not supported erc721");
        ...
        deposits[msg.sender].push(Deposit(msg.sender,_tokenId,_erc721,now,block.number,true));
        depositLength[msg.sender] = depositLength[msg.sender]+1;
        
    }

    function withdrawFarmer(uint256 depositIndex) public nonReentrant{
        ...
        mgm.transfer(msg.sender, amountToWithdraw);
    ...
    function _distributeRewards(uint256 depositIndex,address account) public view returns(uint256 amountOut){
        ...
        if(userDeposit.lastWithdrawBlock < block.number){
            ...
            uint256 rewards = farmerROI;
            if(userDeposit.tokenContract == address(farmerPlusROI)){
                rewards = farmerPlusROI;
            }
            amountOut = rewardsBlock.mul(rewards).div(dailyBlock);
    ...
    function adjustFarmerROI(uint256 _newAmount) public onlyOwner{
        farmerROI = _newAmount;
    }

    function adjustFarmerPlusROI(uint256 _newAmount) public onlyOwner{
        farmerPlusROI = _newAmount;
    }
