...
contract PancakeStableSwapFactory is Ownable {
     ...
     */
    constructor(IPancakeStableSwapLPFactory _LPFactory) {
    ...
    function createSwapPair(
        ...
        // create LP Token
        address LP = LPFactory.createSwapLP(coins, swapContract);
        PancakeStableSwap(swapContract).initialize(coins, _A, _fee, _admin_fee, msg.sender, LP);
...
contract PancakeStableSwap is Ownable, ReentrancyGuard {
    ...
    function add_liquidity(uint256[N_COINS] memory amounts, uint256 min_mint_amount) external payable nonReentrant {
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            ...
            if (coin == BNB_ADDRESS) {
                require(amount == msg.value, "Inconsistent quantity");
            } else if (amount > 0) {
    ...
    function exchange(
        ...
        if (jAddress == BNB_ADDRESS) {
            _safeTransferBNB(msg.sender, dy);
        } else {
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            if (amounts[i] > 0) {
                if (coins[i] == BNB_ADDRESS) {
                    _safeTransferBNB(msg.sender, amounts[i]);
                } else {
    ...
    function withdraw_admin_fees() external onlyOwner {
        for (uint256 i = 0; i < N_COINS; i++) {
            if (coins[i] == BNB_ADDRESS) {
                uint256 value = address(this).balance - balances[i];
                if (value > 0) {
                    _safeTransferBNB(address(this), value);
                }
            } else {
...
interface IPancakeStableSwapLPFactory {
    function createSwapLP(address[2] memory _coins, address _minter) external returns (address);
}
