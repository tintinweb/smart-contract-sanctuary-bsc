...
contract CarsRace is ERC20Upgradeable, OwnableUpgradeable, ITokenInitialize {
    ...
    function initialize(
        ...
        */
        _mint(addrs[0], totalSupply_ * (10**uint256(decimals())));
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        Utils.addLiquidity(address(uniswapV2Router), address(0), tokenAmount, ethAmount);
    }
...
library Utils {
    ...
    function swapTokensForEth(IUniswapV2Router02 pancakeRouter, uint256 tokenAmount) public {
    ...
    function calcReward(
        ...
        for (i = 2; i <= precision; i += 2) {
            ...
            calcExponent = i == exponent
                ? 0
                : (calcExponent * (exponent - i) * (exponent - i - 1)) / (i + 1) / (i + 2);
        }
    ...
    {
        (tTransferAmount, tFee, tLiquidity) = _getTValues(tAmount, _taxFee, _liquidityFee);
        (rAmount, rTransferAmount, rFee) = _getRValues(tAmount, tFee, tLiquidity, currentRate);
