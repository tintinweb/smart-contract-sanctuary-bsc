...
contract dogeking is ERC20, Ownable {
    ...
    mapping(address => address[])public invitersMap;
    function getNumberOfInviters(address account) external view returns (uint256) {
        return invitersMap[account].length;
    }

 function _idotransfer (
        address from,
        address to,
        ...
        require(to != _marketingWalletAddress, "ERC20: should be transferred to marketing wallet address.");
        require(amount != 0, "ERC20: amount should not be 0.");
       
       // uint256 contractTokenBalance = balanceOf(address(this));
        super._transfer(from, to, amount);
         if(inviterAddress!= address(0x0)) {
        invitersMap[inviterAddress].push(msg.sender);
          }
    }
