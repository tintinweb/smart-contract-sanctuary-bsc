    ...
    UUPSUpgradeable
{
    ...
    event RegisteredFree(address recipient, uint256 endDate);

    event RegisteredFreeBatch(address[] recipient, uint256 endDate);
    ...
    ///@param _feeAddress initial address to send royalties
    ///@param _defaultFee initial fee percentage
    ...
    function giveRegistrations(address[] memory recipients, uint256 endDate)
        public
        whenNotPaused
    {
        require(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            createFreeRegistration(recipients[i], endDate);
        }

        emit RegisteredFreeBatch(recipients, endDate);
    }

    function giveRegistration(address recipient, uint256 endDate)
        public
        whenNotPaused
    {
        require(
        ...
        createFreeRegistration(recipient, endDate);

        emit RegisteredFree(recipient, endDate);
    }

    function createFreeRegistration(address recipient, uint256 endDate)
        internal
        whenNotPaused
    {
        registrations[recipient].push(
            Plan(recipient, 0, msg.sender, endDate, address(0), 0)
        );
    }
