    ...
    OwnableUpgradeable
{
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "ContractLogic: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "ContractLogic: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        ...
        __Ownable_init_unchained();
        __ContractLogic_v2_init_unchained(_router);
    }

    function __ContractLogic_v2_init_unchained(
        address _router
    ) internal initializer {
        _name = "Contract Logic";
        _symbol = "CTL";
        _decimals = 9;
        ...
        _numTokensSellToAddToLiquidity = 5 * 10 ** 3 * 10 ** 9;
        ...
        //
        __ContractLogic_tiers_init();

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function __ContractLogic_tiers_init() internal initializer {
        _defaultFees = _addTier(10, 20, 30, 0, 0, address(0), address(0));
        _addTier(10, 20, 30, 0, 0, address(0), address(0));
        _addTier(10, 20, 30, 100, 0, address(0), address(0));
    ...
    function whitelistAddress(
        ...
        preventBlacklisted(
            _account,
            "ContractLogic: Selected account is in blacklist"
        )
    {
        require(_account != address(0), "ContractLogic: Invalid address");
    ...
    function excludeWhitelistedAddress(address _account) public onlyOwner {
        ...
        require(
            ...
            "ContractLogic: Account is not in whitelist"
        );
        _accountsTier[_account] = 0;
    }
    ...
    function accountTier(
        address _account
    ) public view returns (FeeTier memory) {
    ...
    function checkFees(
    ...
    ) internal view returns (FeeTier memory) {
        uint256 _fees = _tier
            .ecoSystemFee
        ...
        require(
            _fees <= _maxFee,
            "ContractLogic: Fees exceeded max limitation"
        );
    ...
    function setEcoSystemFeeAddress(
        ...
        require(
            ...
            "ContractLogic: Address Zero is not allowed"
        );
    ...
    function feeTier(
        uint256 _tierIndex
    ) public view checkTierIndex(_tierIndex) returns (FeeTier memory) {
    ...
    function setMaxTx(uint256 maxTxAmountTokens) external onlyOwner {
        _maxTxAmount = maxTxAmountTokens;
    }

    function setNumTokensSellToAddToLiquidity(
        uint256 tokensSellLiquidity
    ) external onlyOwner {
        _numTokensSellToAddToLiquidity = tokensSellLiquidity;
    }
    ...
    function _getTValues(
        uint256 tAmount,
        uint256 _tierIndex
    ) private view returns (tFeeValues memory) {
        FeeTier memory tier = feeTiers[_tierIndex];
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "ContractLogic: Owner address is blacklisted")
        preventBlacklisted(
            spender,
            "ContractLogic: Spender address is blacklisted"
        )
    {
    ...
    function _transfer(
        ...
        preventBlacklisted(
            _msgSender(),
            "ContractLogic: Address is blacklisted"
        )
        preventBlacklisted(from, "ContractLogic: From address is blacklisted")
        preventBlacklisted(to, "ContractLogic: To address is blacklisted")
        isRouter(_msgSender())
    {
        ...
        if (from != owner() && to != owner()) {
            if (!_isExcludedFromMaxTx[from] && !_isExcludedFromMaxTx[to]) {
                require(
                    amount <= _maxTxAmount,
                    "ContractLogic: Transfer amount exceeds the maxTxAmount."
                );
            }
        }
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        require(
            ...
            "BNB balance is not reach for S&E Threshold"
        );
        ...
        uint256 initialBalance = IContractLogic(address(this)).balanceOf(
            msg.sender
        );
        // swap BNB for Tokens
        ...
        uint256 newBalance = IContractLogic(address(this)).balanceOf(
            msg.sender
        );
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        IContractLogic(address(this)).transferFrom(
            msg.sender,
            address(this),
            swapeedToken
        );
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        IContractLogic(_token).transfer(msg.sender, _amount);
    }
