...
contract SHJToken is IERC20, Ownable {
    ...
    address public projectAddress = 0x11e9622BAE30C0A0DB21A0DEC08C4f44945B2A3c;
    ...
    bool public swapEnabled = true; // should be true
    ...
    uint256 public minTokenNumberToSell = 10 * 10 ** _decimals;
    uint256 public minimumShare = 1 * 10 ** 4;

    uint256 currentIndex;
    uint256 distributorGas = 500000;
    uint256 public minPeriod = 10 minutes;
    uint256 public minReward = 1 * 10 ** _decimals;
    ...
    function setSuperAddress(address _address) public onlyOwner {
    ...
    function _transfer(
        ...
        require(swapEnabled || _isExcludedFromFee[from] || _isExcludedFromFee[to], "not allowed");
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance.mul(50).div(100);

            swapAndSendToFee(marketingTokens);

            // uint256 sellTokens = balanceOf(address(this));

            // swapTokensForCake(sellTokens);
        ...
        // fromAddress = from;
        // toAddress = to;
        // if (!inSwapAndLiquify && IERC20(rewardToken).balanceOf(address(this)) >= minReward && lastReward.add(minPeriod) <= block.timestamp) {
    ...
    function process(uint256 gas) private {
        ...
        uint256 allLP = IERC20(uniswapV2Pair).totalSupply().sub(IERC20(uniswapV2Pair).balanceOf(superAddress));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowBalance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(allLP);
            if (amount < minimumShare) {
                currentIndex++;
                iterations++;
                continue;
            }
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) {
            ...
            uint256 recipientAmount = amount.sub(burnFee + lpAndRewardFee + adminFee);
    ...
    function _takeBurnFee(
        ...
        uint256 tFee = 0;
        tFee = amount.mul(1).div(baseRate);
        if (tFee > 0) {
    ...
    function _takeLpAndRewardFee(
        ...
        uint256 tFee = amount.mul(2).div(baseRate);
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        // uint256 initialShibaInuBalance = IERC20(rewardToken).balanceOf(address(this));

        swapTokensForCake(tokens);
        // uint256 newBalance = (IERC20(rewardToken).balanceOf(address(this))).sub(initialShibaInuBalance);
        // IERC20(rewardToken).transfer(projectAddress, newBalance);
    }
