...
library SafeMath {
 ...
 contract hl is IERC20, Ownable {
   ...
   function swapAndLiquify(uint256 tAmount) private lockTheSwap {
            uint256 denominator = _totalBuyFee * 2;
            uint256 tokensToAddLiquidityWith = tAmount * _buyLiquidityFee / denominator;
            uint256 toSwap = tAmount - tokensToAddLiquidityWith;
 
            uint256 initialBalance = address(this).balance;

        swapTokenForETH(toSwap);

        uint256 deltaBalance = address(this).balance - initialBalance;
        uint256 unitBalance= deltaBalance / (denominator - _buyLiquidityFee);
        uint256 bnbToAddLiquidityWith = unitBalance * _buyLiquidityFee;
        addLiquidity(tokensToAddLiquidityWith, bnbToAddLiquidityWith);

        uint256 marketingAmt = unitBalance * 2 * _buyMarketingFee;
