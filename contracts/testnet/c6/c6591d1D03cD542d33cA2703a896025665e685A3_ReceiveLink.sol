...
contract TRC21 is ITRC21 {
    ...
    // Token name
    string public _name = 'ReceiveLink';

    // Token symbol
    string public _symbol = 'ReceiveLink';
...
contract ReceiveLink is TRC21, Ownable {
    using SafeMath for uint256;
    ...
    constructor () public {}
    function sendTRC21s(uint256 _num, uint256[] _ids, address _trc21, uint256 _amount, uint256 _start, uint256 _end) public {
    ...
    function validateTime(uint256 _type, uint256 _id) public view returns(bool) {
        bool validate;
        if(_type == 1) {
            validate = now >= sendTrc21s[_id].start && now <= sendTrc21s[_id].end;

        }
        else if(_type == 2) {
            validate = now >= tomos[_id].start && now <= tomos[_id].end;
...
//            require(sendTrc21s[_id].taker == msg.sender);
            ...
            require(now > sendTrc21s[_id].end, 'This time not available !');
            status = 2;
        }
    ...
    function sendTomos(uint256 _num, uint256[] _ids, uint256 _amount, uint256 _start, uint256 _end) public payable{
...
//            require(tomos[_id].taker == msg.sender);
            ...
            require(now > tomos[_id].end, 'This time not available !');
            status = 2;
        }
        msg.sender.transfer(tomos[_id].amount);
        tomos[_id].amount = 0;
        tomos[_id].status = status;
        tomos[_id].taker = msg.sender;
        emit RequestTomo(_id, msg.sender);
    }
    function sendTRC721s(uint256[] _ids, address _trc721, uint256[] _tokenIds, uint256 _start, uint256 _end) public {
...
//            require(sendTrc721s[_id].taker == msg.sender);
    ...
    function resetLink(uint256 _id, uint256 _type) public onlyManager {
        ...
        // _type == 3 => sendTrc721s
        if(_type == 1) {
            tomos[_id].maker.transfer(tomos[_id].amount);
            tomos[_id].amount = 0;
            tomos[_id].status = 2;
        } else if(_type == 2) {
            ITRC21 trc21 = ITRC21(sendTrc21s[_id].trc21);
