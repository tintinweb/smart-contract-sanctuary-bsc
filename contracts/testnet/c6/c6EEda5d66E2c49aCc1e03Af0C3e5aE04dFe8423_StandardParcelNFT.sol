    ...
    ReentrancyGuardUpgradeable
{
    ...
    function testingThis(address _user) external view returns(uint256, bool) {
    ...
    tests = this.tokensOf(_user);
        ...
        upgradedParcelsMapping[tests[1]];
    return  (tests.length,upgradedParcelsMapping[tests[1]]);//(tests, upgradedParcelsMapping[this.tokensOf(_user)[1]]);
    // else(upgradedParcesIds[i]) {
    //         for (uint j = i; i < tokensOf.length-1; j++){
    //         upgradedParcesIds[j] = upgradedParcesIds[j+1];
    //         }
    }
    ...
    function getUpgradedParcelIds(address _user) external view returns(uint256[] memory) {
       ...
       uint256[] memory upgradedParcesIds = new uint256[](length);
