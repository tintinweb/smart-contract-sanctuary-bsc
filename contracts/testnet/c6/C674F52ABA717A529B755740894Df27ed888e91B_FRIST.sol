...
library Address {
    ...
    function sendValue(address payable recipient, uint256 amount) internal {
        /*确认支付地址的余额是否大于amount  不满足条件的话提示余额不足*/
        require(address(this).balance >= amount, "Address: insufficient balance");
        /*条件成立则 将amount存入支付地址中*/
...
contract FRIST is Ownable, IERC20, IERC20Metadata { 
    /*
     *  将SafeMath绑定到uint256类型
     *  将Address绑定到address类型
     */
     ...
     *  _currprice     当前币价
     *  _defaultPrice  默认币价
     *  _balances      余额    用地址引用记录将他定义到一个整数
     *  _allowances    津贴    嵌套的引用地址
     *  isStatus       状态    地址引用记录定义为布尔
     *  airtokenlist   
     *  hvaeToken      
     */
