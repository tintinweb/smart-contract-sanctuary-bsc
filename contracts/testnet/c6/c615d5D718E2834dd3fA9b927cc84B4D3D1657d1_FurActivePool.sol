...
/*
 * The Active Pool holds the FUR collateral and FURUSD debt (but not FURUSD tokens) for all active vaults.
 *
 * When a vault is liquidated, it's FUR and FURUSD debt are transferred from the Active Pool, to either the
 * Stability Pool, the Default Pool, or both, depending on the liquidation conditions.
 *
 */
contract FurActivePool is BaseContract, IFurActivePool {
    ...
    event ActivePoolFURBalanceUpdated(uint _FUR);
    ...
    function sendFUR(address _account, uint _amount) external override whenNotPaused {
        _requireCallerIsBOorVaultMorSP();
        furToken.safeTransfer(_account, _amount);
        
        uint256 FUR = getFUR();
        emit ActivePoolFURBalanceUpdated(FUR);
        emit FURSent(_account, _amount);
    }
