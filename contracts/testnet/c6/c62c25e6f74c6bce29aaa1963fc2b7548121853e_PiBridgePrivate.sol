...
contract PiBridgePrivate is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;

    // Wallet
    address public devWallet;
    mapping(address => bool) public paidPrivateMap;
    ...
    StagePrivate[] public stageList;

    // Event
    event EventUserClaim(address user, uint256 amount, uint256 time);

    constructor(address _devWallet, address _claimToken) {
    ...
    function setClaimTime(
        ...
        for (uint256 index = 0; index < _startTimeList.length; index++) {
            StagePrivate memory stageObject;
    ...
    function setPaidPrivateMap(address[] calldata userList) public onlyOwner {
        for (uint256 index = 0; index < userList.length; index++) {
            paidPrivateMap[userList[index]] = true;
            vestingMap[userList[index]] = 0;
            totalOfPaid++;
        }
    }

    function removePaidPrivateMap(address[] calldata userList) public onlyOwner {
        for (uint256 index = 0; index < userList.length; index++) {
            paidPrivateMap[userList[index]] = false;
        }
    }
    ...
    function setTotalOfPaid(uint256 _totalOfPaid) public onlyOwner {
        totalOfPaid = _totalOfPaid;
    }
    ...
    function claimPrivate() external nonReentrant isRun {
        ...
        uint256 amountClaim = currentPercentVesting
            .mul(tokenPerSlot)
            .div(deminomator);
            
        claimToken.safeTransfer(msg.sender, amountClaim);

        vestingMap[msg.sender] += 1;
