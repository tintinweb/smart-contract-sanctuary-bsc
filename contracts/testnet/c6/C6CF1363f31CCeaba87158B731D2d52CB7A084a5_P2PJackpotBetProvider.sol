...
contract P2PJackpotBetProvider is TokenProcessing, JackpotProcessor {
    ...
    function createJackpotBet(JackpotDTOs.CreateJackpotRequest calldata createRequest) external onlyCompany returns (uint) {
        ...
        require(previousJackpot.expirationTime < block.timestamp, "P2PJackpotBetProvider.createJackpotBet: active jackpot found");
    ...
    function joinJackpotBet(JackpotDTOs.JoinJackpotRequest calldata joinRequest) public {
        require(jackpotBets[joinRequest.betId].lockTime >= block.timestamp, "P2PJackpotBetProvider.joinJackpotBet: lock time");
        clientBets[msg.sender].push(joinRequest.betId);
        ...
        JackpotDTOs.JoinJackpotBetClientList storage clientBetList = clientInfo[msg.sender][joinRequest.betId];
        ...
        JackpotDTOs.JackpotMatchingInfo storage info = matchingInfo[joinRequest.betId];
...
abstract contract TokenProcessing is CompanyVault, AlternativeTokenHelper, FeeConfiguration {
    ...
    function takeFee(uint amount, address targetAddress, bool isAlternative) external onlyOwner {
        IERC20 token;
        if (isAlternative) {
