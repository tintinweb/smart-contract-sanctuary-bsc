...
contract StrigoiGame is Ownable {
    using SafeERC20 for IERC20;

    address[] public Admins;
    mapping(address => bool) public AdminByAddr;
    IStrigoi721 public nftContract;
    uint[] levels = [39,119,239,399,599,839]; // max BP per level; 20*(level^2)-(20*level)
    ...
    constructor(address nftContract_) {
        nftContract = IStrigoi721(nftContract_);
    }

    function setNftContract(address nftContract_) external onlyOwner {
        require(nftContract_ != address(0), "nftContract_ = 0");
        nftContract = IStrigoi721(nftContract_);
    }

    function getLevel(uint creatureNftId) public view returns(uint, uint){
        uint bp = (nftContract.getNftInfo(creatureNftId)).propertyNum1;
        for(uint x = 0; x < levels.length; x++) {
            if(bp < levels[x]) {
                return (x+1, levels[x]+1-bp);
            }
        }
        return (1, levels[0]+1);
    }

    function getNftClassAndLevelCountForUser(address user, uint nftClass, uint level) external returns(uint) {
        uint result;
        uint bal = IStrigoi721(nftContract).balanceOf(user);
        for (uint x = 0; x < bal; x++) {
            uint id = IStrigoi721(nftContract).tokenOfOwnerByIndex(user, x);
            (uint currentLevel, uint nextLevel) = getLevel(id);
            if (IStrigoi721(nftContract).getNftClass(id) == nftClass && currentLevel == level) result++; 
        }
        return result;
    }
    ...
    function isNftClassAndLevel(uint id, uint nftClass, uint level) external returns(bool) {
        (uint currentLevel, uint nextLevel) = getLevel(id);
        return IStrigoi721(nftContract).getNftClass(nftClass) == nftClass && currentLevel == level;
    }   
    ...
    function getNftTokenIdForUserForNftType(address user, uint nftType) external returns(uint) {
        ...
        for (uint x = 0; x < bal; x++) {
            uint id = IStrigoi721(nftContract).tokenOfOwnerByIndex(user, x);
            if (IStrigoi721(nftContract).getNftType(id) == nftType) {
                return id;
            }
        }
        return result;        
