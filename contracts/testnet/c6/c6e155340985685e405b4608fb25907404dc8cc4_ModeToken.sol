...
contract ModeToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address public lpPoolAddress;
    string private _name = "Mode Token 4";
    string private _symbol = "MODE4";
            ...
            if(lpPoolAddress==recipient){
                ...
                if(!whiteList[sender]){
                  uint256 burnFee = amount.mul(_sellBurnFee).div(100);
                    if(burnFee>0) {
                        _balances[devAddress] = _balances[devAddress].add(burnFee);
                        emit Transfer(sender, devAddress, burnFee);
                ...
                if(!whiteList[recipient]){
                    uint256 burnFee= amount.mul(_buyBurnFee).div(100);
                    if(burnFee>0) {
                        _balances[devAddress] = _balances[devAddress].add(burnFee);
                        emit Transfer(sender, devAddress, burnFee);
                        realRecipientAmount=realRecipientAmount.sub(burnFee);
    ...
    function setFee(uint256 _sellBurn,uint256 _buyBurn) public  onlyOwner {
