...
contract EvocLaunchpad is Ownable, ReentrancyGuard {
    ...
    {
        User_Config storage item = PartnerRegister[launchId].UserConfig[account];
        return (
            item.balance,
            item.maxAmount,
            item.referAddress,
    ...
    {
        uint256[] memory partnerIds = new uint256[](partnerId_+1);
        for (uint256 i; i < partnerId_ + 1; i++) {
            if (
                address(PartnerRegister[i]._contractAdd) ==
    ...
    function buyWithToken(
        ...
        uint256 tkAmount = item.PartnerValues._launchPriceRate.mul(_tokenAmount);
        require(tokenAmount > 0  || tokenAmount > item.PartnerValues._totalRemaining, "Insufficient amount for this transaction");
        ...
        if (item.PartnerRefer._referActive) {
            ...
            uint256 referReward = (tokenAmount *
                item.PartnerRefer._referBonusPercent).div(100);
    ...
    function createPartner(
        ...
        PartnerRegister[partnerId_]
            .PartnerValues
            ._listingPriceRate = _listingPriceRate;
        PartnerRegister[partnerId_]
            .PartnerValues
            ._launchPriceRate = _launchPriceRate;
        ...
        PartnerRegister[partnerId_]
            .PartnerStatus
            ._liquidityPercentage = _liquidityPercentage;
    ...
    function _receiveTokensPatner(
        ...
        uint256 tokensAmount = (
            (_hardCap.div(_launchPriceRate)).add(LiquifyBalance)
        ).mul(_DECIMALFACTOR);
        require(
            ...
            for (uint256 i = 0; i < item._paymentType.length; i++) {
                ...
                if (balance > 0) {
                    if (_compareString(item._paymentType[i], "BNB")) {
                        ...
                        uint256 dexBalance = ContractAdd.balanceOf(
                            address(this)
                        );
                        require(
    ...
    function claimTokens(uint256 PartnerId) public nonReentrant isPausable {
        require(launchStatus(PartnerId) == 2, "Launchpad Release not yet completed");
    ...
    function claimFunds(uint256 PartnerId) public nonReentrant isPausable {
        address wallet = msg.sender;
        Partner_register storage item = PartnerRegister[PartnerId];
        require(launchStatus(PartnerId) == 0, "Launchpad is Active");
        require(
            !item.PartnerStatus._status,
            "The project has not yet been released to claim."
        );
    ...
    function swapAndLiquify(uint256 PartnerId) private lockTheSwap {
        Partner_register storage item = PartnerRegister[PartnerId];

        // Number of tokens sold divided by the percentage that will be added to liquidity.
        uint256 tokenAmount = item
            .PartnerValues
            ._totalTokenSold
    ...
    function swapTokensForBNB(uint256 PartnerId, uint256 tokenAmount) private {
    ...
    function launchStatus(uint256 PartnerId) public view returns (uint256) {
        Partner_register storage item = PartnerRegister[PartnerId];
        uint256 softCap = (item.PartnerValues._softCap.div(item.PartnerValues._launchPriceRate)).mul(_DECIMALFACTOR);
        uint256 hardCap = (item.PartnerValues._hardCap.div(item.PartnerValues._launchPriceRate)).mul(_DECIMALFACTOR);
        ...
        returns (address PairAdress)
    {
        ...
        if (tokenPair == address(0)) {
            tokenPair = IUniswapV2Factory(_uniswapV2Router.factory())
