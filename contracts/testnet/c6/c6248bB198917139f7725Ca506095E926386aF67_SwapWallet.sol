...
contract SwapWallet is ReentrancyGuard {
    ...
    SwapWalletFactory public immutable factory;
    uint256 private immutable safeMinGas;
    ...
    constructor(address owner_, SwapWalletFactory factory_) { 
...
contract SwapWalletFactory is ReentrancyGuard{
    using SafeERC20 for IERC20;
    ...
    constructor(address owner_, address WETH_, uint applyGasLimit_, uint applyGasInterval_) {
    ...
    function createWallet(address walletOwner) external onlyOwner returns (address) {
        ...
        SwapWallet wallet = new SwapWallet(walletOwner, this);
    ...
    function addWallet(address payable wallet, address walletOwner) external onlyOwner {
        ...
        require(walletContract.factory() == this, "SwapWalletFactory: wallet is not created from this factory");
    ...
    function withdraw(address assetAddress_, uint256 amount_, address toAddress_) external nonReentrant {
        ...
        if (assetAddress_ == address(0)) {
            ...
            uint256 assetBalance = IERC20(assetAddress_).balanceOf(address(this));
            require(assetBalance >= amount_, "SwapWalletFactory: not enough balance");
            IERC20(assetAddress_).safeTransfer(toAddress_, amount_);
            emit WithdrawHappened(assetAddress_, amount_, toAddress_);
        }
    }
