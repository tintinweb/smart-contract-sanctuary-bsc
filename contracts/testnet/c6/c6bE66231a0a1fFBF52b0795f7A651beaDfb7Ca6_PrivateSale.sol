...
contract PrivateSale is Auth {
  ...
  function withdrawPerMonth() external {
    ...
    require(currentAmount < totalAmount, 'PrivateSale:withdrawPerMonth - Monthly vesting finished');
    if (amountToReceivePerMonth >= totalAmount.sub(currentAmount))
      amountToReceivePerMonth = totalAmount.sub(currentAmount);

    require(amountToReceivePerMonth > 0, 'no receivable amount');

    uint256 notReceivedMamps = _claimMamp(amountToReceivePerMonth);

    investorInfos[msg.sender]._cliff = investorInfos[msg.sender]._cliff.add(cliff);
  ...
  function setBuyMinAmount(uint256 _amount) external onlyOwner {
    ...
    require(_amount < max_buy_amount_estimate, "can't exceed max amount");
    min_buy_amount_estimate = _amount;
  }

  function setBuyMaxAmount(uint256 _amount) external onlyOwner {
    require(_amount > 0, "can't be zero");
    require(_amount > min_buy_amount_estimate, "can't be less than min amount");
