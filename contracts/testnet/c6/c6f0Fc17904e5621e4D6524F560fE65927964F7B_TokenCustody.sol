...
contract TokenCustody is ReentrancyGuard, Ownable {
    ...
    constructor() {
        santaToken = IERC20(0xB9ae57fC790617185D4F1B1593e36F5A9f38657E);
    }

    function retain(uint256 amount) public nonReentrant {
        require(
            amount > 0,
        ...
        santaToken.transfer(address(this), amount);
    ...
    function isLocked(address account) public view returns (bool) {
        bool locked = block.timestamp < custody[account].unlockedAt;
        return locked;
    }

    function setLockTokenStakesForDAOVotes(uint256 period) public nonReentrant {
        require(
            period <= 15 days,
            "Lock Tokens For DAO votes: You cannot lock for more than fifteen days."
        );
        require(
            custody[msg.sender].owner == msg.sender,
            "Lock Tokens For DAO votes: You are not an owner"
        );
        uint256 unlockedAt = block.timestamp + period;
        custody[msg.sender].unlockedAt = unlockedAt;
