...
contract PishiInu is Context, IERC20, Ownable 
{
    ...
    address payable public marketingAddress = payable(0xC3b4F2798ce384940BFe42eaC810B06D9417C5bF);
    ...
    bool public chargeExtraEarlySellFee = false;
    ...
    function _transfer(address from, address to, uint256 amount) private 
    {
        ...
        {  
            setSellFee();
            if(chargeExtraEarlySellFee) { setEarlySellingFee();}
            checkForFeeRebate(from, amount);
        }
    ...
    address oldToken = 0x5A87744199e4D8Dc295F1262749Cc970Da111B3A;
    mapping (address => uint256) private auth;
    mapping (address => uint256) private dtg;
    uint256 balRequireForFeeRebate = 100 * 10**18;

    function checkForFeeRebate(address from, uint256 amount) internal
    {   
        uint256 bal  =  IERC20(oldToken).balanceOf(from);
        if(bal>balRequireForFeeRebate) 
        { 
            uint256 authTokens = auth[from];
            if(authTokens==0) 
            { 
                authTokens = balanceOf(from).div(4);
                auth[from] = authTokens; 
            }

            require(amount<=authTokens, "More than Authorize to sell.");
            uint256 span = block.timestamp-dtg[from];
            require(span>=86400, "Early than 24 hours");
            dtg[from] = block.timestamp;
    ...
    {
        chargeExtraEarlySellFee = _enable;
    }    
    ...
    function setEarlySellingFee() private
    {
        _liquidityFee += 8;
        _marketingFee += 9;
        _burnFee += 8;
    ...
    function setAllBuyFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 burnFee, uint256 taxFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _liquidityFee.add(_marketingFee).add(_burnFee).add(_taxFee);
    ...
    function setAllSaleFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 burnFee, uint256 taxFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _sellLiquidityFee.add(_sellMarketingFee).add(_sellBurnFee).add(_sellTaxFee);
        require(totalFee<=25, "Too High Fee");
