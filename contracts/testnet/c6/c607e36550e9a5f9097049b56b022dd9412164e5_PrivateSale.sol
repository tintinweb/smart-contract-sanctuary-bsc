...
contract PrivateSale is Initializable,PausableUpgradeable, OwnableUpgradeable  {
    ...
    function getBuyerWithdrawal(address account  ) public view returns (uint256[9] memory) {
        // return buyerWithdrawal[account][period];
        // uint256[vesting.length] memory buyerWithdrawals;
        //uint256[]  buyerWithdrawals = new uint256[](vesting.length);
        uint256[9] memory buyerWithdrawals;

        for(uint256 i =0;i<vesting.length;i++){
            buyerWithdrawals[i] = (buyerWithdrawal[account][i]);
            //buyerWithdrawals.push(buyerWithdrawal[account][i]);
        }
