...
contract CoinFlip is Ownable, ReentrancyGuard {
    ...
    function autoStartSession() internal {
        ...
        startSession(block.timestamp + defaultLength, defaultMaxBet, defaultMinBet, defaultDevFee);
    }
    ...
    function closeSession(uint256 _sessionId) external nonReentrant {
        ...
        require(_sessions[_sessionId].headsCount > 0 && _sessions[_sessionId].tailsCount > 0 , "Not enough people have bet!");
    ...
    function calculatePayout(address _address, uint256 _sessionId) external view returns (uint256) {
        ...
        if (_sessions[_sessionId].status != Status.Claimable || 
            ...
            HasClaimed[_address][_sessionId] || 
