...
contract FreeDao is ERC20Detailed, Pausable, Ownable, IFreeDao {
    ...
    mapping(address => address[]) public underlings;
    mapping(address => uint8) public referralCount;
    // address msgsender => referral => true/false  check relation 
    mapping(address => mapping(address => bool)) public relationCheck;
    ...
    function payToken() public override view returns (address) {
        return address(usdt);
    }

    // todo 
    function setDaoReward(uint256 _amount) override external {}
    ...
    // todo debug check, can not delete childrens!!
    function betray(address target) public onlyOperator {
        uint8 count = getReferralCount(target);
        require(count > 0, "no relation");
    ...
    function getReferralCount(address target) public view returns(uint8){
        return referralCount[target];
    }

    function setReferral(address _origin, address _referral) public override onlyOperator {
        _setReferral(_origin, _referral);
    }
    ...
    function _setReferral(address _origin, address _referral) private {
        ...
        // origin has no relation
        require(getReferralCount(_origin) == 0, "already has relation");
        require(!checkRealtion(_origin, _referral), "exist relation");
        ...
        // get referral's relation then set to origin
        uint8 count = getReferralCount(_referral);
        if (count > 0) {
            ...
            for (uint256 i = 0; i < count; i++) {
                ...
                if (uperReferral != address(0)) {
                    require(
                        !checkRealtion(_origin, uperReferral),
    ...
    function checkRealtion(address _origin, address _referral)
        public
        view
        returns (bool)
    {
        return relationCheck[_origin][_referral] || relationCheck[_referral][_origin];
    }
    ...
    function _transfer(
        ...
        uint256 referralFee;
        // if to without parent, from become to's parent
        if (!singles[from] && referralCount[to] == 0 && !checkRealtion(from, to)) {
