...
contract BullForce is IBEP20,Ownable{
    ...
    constructor() 
    
     {
      //  router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); //mainent
       ...
       _mint(owner(), 1000000000 ether,26740000 ether,100000000 ether);
    ...
    function _mint(address account, uint256 maximumSupply_, uint256 totalSupply_,uint256 halivingDivider_) virtual internal{
         ...
         require(account != address(0), 'BEP20: mint to the zero address');
          ...
          _maximumSupply = _maximumSupply.add(maximumSupply_);
            ...
            _balances[account] = _balances[account].add(totalSupply_);
              ...
              halvingDivider = halivingDivider_;
    ...
    function _transferFrom(
        ...
        if (!isDividendExempt[sender]) {
            ...
            .add(_stakebalances[sender])
            .add(apyStakeTracker[sender].totalActiveStakedAmount)
            .add(PVTShare[recipient])) {} catch {}
        }

        if (!isDividendExempt[recipient]) {
            try
                dividendTracker.setShare(recipient, _balances[recipient]
                .add(_stakebalances[recipient])
                .add(apyStakeTracker[recipient].totalActiveStakedAmount)
                .add(PVTShare[recipient])) {} catch {}
        }
    ...
    function setDistributorGas(uint256 gas) external onlyOwner {
     ...
     function startConsensus(address _contractToVote,address[20] memory voters) public onlyOwner{
        ...
        for(uint x =0; x<voters.length;x++){
         ...
         require(voters[x] !=address(0),"BFBC: Address Zero not allowed");
     ...
     function _mintReward(address to, uint256 _amount,uint256 fee) internal returns(uint256){
                      
           uint256 amount_fee = _amount.sub(fee);
       
           uint256  fee_half= fee.div(2);

             burnTax(address(this),fee_half);

             _balances[marketingFeeReceiver] = _balances[marketingFeeReceiver].add(fee_half);
              emit Transfer(address(this),marketingFeeReceiver,fee_half);
            

             _balances[to] = _balances[to].add(amount_fee);
             _totalSupply = _totalSupply.add(amount_fee);           
              emit Transfer(address(this),to,amount_fee);
          
             _halving = (_totalSupply/halvingDivider).add(1);
               
                if(stakeProfile[to].exists){
                 stakeProfile[to].totalYield = stakeProfile[to].totalYield.add(amount_fee);
             }
       ...
       function mineReward(address to,uint256 amount, bool isFee) override public{
        ...
        require(totalSupply().add(amount) <= maximumSupply(), "BFBC: `totalSupply` Exceeded");
    ...
    struct StakeTracker{
        uint256 totalActiveStakedAmount;
    ...
    mapping(address => mapping(uint256 => APYStakeProfile)) public Staker;
    mapping(address=>StakeTracker) public apyStakeTracker;
    address public apyStakeHoldder = 0x783071c98dDf4ad067E73EbFf8DfDd6011fB54f6;

   
    function apyStake (uint256 amount, uint256 _stakeType) public {       
        
          _basicTransfer(_msgSender(), apyStakeHoldder, amount);
    
           if(!Staker[_msgSender()][_stakeType].exists){

            Staker[_msgSender()][_stakeType] = APYStakeProfile({exists:true,unlockTime:0,amount:0,apy:0 });
            apyStakeTracker[_msgSender()] = StakeTracker({totalStakedAmount:0,totalUnstakedAmount:0,totalYield:0,totalActiveStakedAmount:0});
       ...
       if(_stakeType==1){
           Staker[_msgSender()][_stakeType].unlockTime=block.timestamp + 30 days;
           Staker[_msgSender()][_stakeType].apy=2;       
       }
      if(_stakeType==2){
          Staker[_msgSender()][_stakeType].unlockTime=block.timestamp + 90 days;
          Staker[_msgSender()][_stakeType].apy=8;        
          }
       if(_stakeType==3){
           Staker[_msgSender()][_stakeType].unlockTime=block.timestamp + 180 days;
       Staker[_msgSender()][_stakeType].apy=17;
       }
       if(_stakeType==4){
           Staker[_msgSender()][_stakeType].unlockTime=block.timestamp + 270 days;
       Staker[_msgSender()][_stakeType].apy=27;
       }
       if(_stakeType==5){
           Staker[_msgSender()][_stakeType].unlockTime=block.timestamp + 360 days;
           Staker[_msgSender()][_stakeType].apy=36;
       }
           
        Staker[_msgSender()][_stakeType].amount=amount;
        apyStakeTracker[_msgSender()].totalStakedAmount = apyStakeTracker[_msgSender()].totalStakedAmount.add(amount); 
        apyStakeTracker[_msgSender()].totalActiveStakedAmount = apyStakeTracker[_msgSender()].totalActiveStakedAmount.add(amount);    
        
                 

    }

    function unStakeApy(uint256 _stakeType) public {      
        require(Staker[_msgSender()][_stakeType].amount > 0,"Balance is Zero");

            uint256 amount = Staker[_msgSender()][_stakeType].amount;
            uint256 yield =0;
        
        if(Staker[_msgSender()][_stakeType].unlockTime<block.timestamp){
           amount =amount.add(yield);
           yield =amount.mul(Staker[_msgSender()][_stakeType].apy).div(100);
            _mintReward(_msgSender(),yield,0); 
        
        }

         _basicTransfer(apyStakeHoldder,_msgSender(), amount);
        Staker[_msgSender()][_stakeType].amount =0;
        apyStakeTracker[_msgSender()].totalUnstakedAmount = apyStakeTracker[_msgSender()].totalUnstakedAmount.add(amount);
        apyStakeTracker[_msgSender()].totalYield = apyStakeTracker[_msgSender()].totalYield.add(yield);
        apyStakeTracker[_msgSender()].totalActiveStakedAmount = apyStakeTracker[_msgSender()].totalActiveStakedAmount.sub(amount);         
  
    }
       
 
    function pvtDividend(address buyer, uint256 amount) public override {
        require(ICanMint(_msgSender()).isCanMint(),"You cannot use this function");
                 PVTShare[buyer] = amount;       
    }
