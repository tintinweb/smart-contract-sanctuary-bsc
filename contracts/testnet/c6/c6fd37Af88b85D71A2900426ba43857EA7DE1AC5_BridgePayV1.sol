...
contract BridgePayV1 is Ownable, ReentrancyGuard, Pausable {
    using SafeMath for uint256;

    uint256 public bridgeTax = 1;
    ...
    function getBridgePartners()
        public
        view
        returns (bridgePayPartner[] memory)
    {
        bridgePayPartner[] memory _bridgePayPartners2 = new bridgePayPartner[](
            partnerCount
        );
        for (uint256 i = 0; i < partnerCount; i++) {
            bridgePayPartner storage _bridgePayPartners1 = bridgePayPartners[i];
            _bridgePayPartners2[i] = _bridgePayPartners1;
        }
        return _bridgePayPartners2;
    }
    ...
    function sendToPartner(uint256 _amount, uint256 _iPartner) public {
        require(
            bridgePayPartners[_iPartner].partnerWallet != address(0),
        ...
        uint256 tax = _amount.mul(bridgeTax).div(100);
    ...
    function updateTaxInfo(address _address, uint256 _tax) external onlyOwner {
        require(_tax <= 10, "Bridge tax cannot be more than 10%");
        bridgeTax = _tax;
        taxAddress = _address;
    }
    ...
    function moveBEP20(address _token, address _to) external onlyOwner {
        require(
    ...
    function moveBNB(address _to) external onlyOwner {
