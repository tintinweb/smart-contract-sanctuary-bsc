...
contract IpfsIdStorage is Ownable {
    ...
    function getStatesLengths(uint256 _season) external view returns(uint256 bronzeLength,uint256 silverLength,uint256 goldLength,uint256 platinumLength ){
        return (
    ...
    function getNftId(uint256 _season, uint256 _state, uint256 _index) external view returns(uint256){
        if(_state == 0){
    ...
    function getMintedGods(uint256 _season) external view returns(uint256[] memory){
    ...
    function getNextIpfsId(uint256 _state, uint256 _nftId) external onlyAuth returns(uint256 newIpfsId){
        if(_state == 0){
        ...
        if(
            _state == 3 &&
            block.timestamp >= seasonCreationTimestamp[_currentSeason] + 180 days &&
            _mintedGods[_currentSeason].length < 7){
            _nonce = IOracle(gameAddresses.getOracleAddress()).getRandom(keccak256(abi.encodePacked(_currentSeason,block.timestamp,_nonce)));
            if(block.timestamp >= seasonCreationTimestamp[_currentSeason] + 180 days + ((_nonce % 30) * 1 days)){
                newIpfsId = _getGodId();
            }
        } 
        if(newIpfsId == 0){
        ...
        emit ZaiMinted(_state == 0 ? "bronze" : _state == 1 ? "silver" : _state == 2 ? "gold" : "platinum" , _nftId, block.timestamp);
    ...
    function _getRandomId(uint256 _state) internal returns(uint256){
        ...
        _nonce = 
            ...
            .getRandom(keccak256(abi.encodePacked(_currentSeason,block.timestamp,_nonce)));

        uint256 index = _nonce % (EnumerableSet.length(_freeIds[_currentSeason][_state]));
