...
contract TA is IERC20, IERC20Metadata, Ownable, MiningToken {
    ...
    function _tokenTransferBuy(
        ...
        _tokenTransfer(from, daoAddress, fee3);
    }
    ...
    function _tokenTransferFees(
        ...
        _tokenTransfer(from, BURN_ADDRESS, fee1);
        if (nftAddress != address(0)) {
            _tokenTransfer(from, nftAddress, fee2);
            INotifyRewardAmount(nftAddress).notifyRewardAmount(fee2);
        } else {
            _tokenTransfer(from, devAddress, fee2);
        }
...
contract TokenSeller is Ownable, UserRegistry {
    ...
    constructor(IERC20 _usdt, MintableERC721 _player, MintableERC721 _judge, address _receiveAddr) {
        usdt = _usdt;
        player = _player;
        judge = _judge;
    ...
    function becomeFans(address recommender_) external {
        ...
        _updateGroup(msg.sender, receiveAmount, 7);
    ...
    function claimPlayer(uint256 counts) external {
        ...
        require(childrenLength(msg.sender) > users[msg.sender].playerClaimCounts, "no chance");
    ...
    function _updateGroup(address account, uint256 amount, uint8 step) private {
        ...
        _updateGroup(account, amount, step - 1);
    }
}
