...
contract FrontRunner {
    ...
    function bestPath(address token, uint256 value) public view returns(address[] memory best) {
        ...
        address[] memory pathBNB; //= [uniswapRouter.WETH(), token];
        address[] memory pathUSDT; // = [uniswapRouter.WETH(), usdt, token];
        address[] memory pathBUSD; // = [uniswapRouter.WETH(), busd, token];
