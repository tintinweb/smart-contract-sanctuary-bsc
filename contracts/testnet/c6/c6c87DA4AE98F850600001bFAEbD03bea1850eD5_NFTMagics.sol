    ...
    OwnableUpgradeable
{
    ...
    uint256 public bnbPrice = 2100000000000000;
    uint256 public ayraPrice = 20000000000000000000;
    uint256 public ithdPrice = 29000000000000000000;
    uint256 public bnbPriceWithHasTag = 54000000000000000;
    uint256 public ayraPriceWithHasTag = 60000000000000000000;
    uint256 public ithdPriceWithHasTag = 88000000000000000000;

    mapping(uint256 => address) magicToOwner;
    mapping(address => uint256) ownermagicCount;
    ...
    function mint(
        ...
        if (_mintToken == BNB) {
            if (_hasTag) _mintPrice = bnbPriceWithHasTag;
            else _mintPrice = bnbPrice;
        }
        if (_mintToken == ITHD) {
            if (_hasTag) _mintPrice = ithdPriceWithHasTag;
            else _mintPrice = ithdPrice;
        }
        if (_mintToken == AYRA) {
        ...
        uint256 royaltyFee = (_mintPrice * royaltyPercent) / DENOMINATOR;
        uint256 ownerFee = (_mintPrice * ownerPercent) / DENOMINATOR;
        uint256 dev1Fee = (_mintPrice * dev1Percent) / DENOMINATOR;
        ...
        for (uint256 k = 0; k < _amount; k++) {
          ...
          for (uint256 i = 0; i < tokenId; i++) {
            address ownerAddress = magicToOwner[i + 1];
            rewardsAmount[ownerAddress] += royaltyFee / tokenId;
          }
          _tokenIds.increment();

          uint256 newItemId = _tokenIds.current();
          _safeMint(msg.sender, newItemId);
          magicToOwner[newItemId] = msg.sender;
        ...
        if (_mintToken == BNB) {
          payable(owner()).transfer(ownerFee * _amount);
          payable(dev1Address).transfer(dev1Fee * _amount);
          payable(msg.sender).transfer(msg.value - _mintPrice * _amount);
        } else {
          ERC20Upgradeable(_mintToken).transferFrom(msg.sender, address(this), (_mintPrice - ownerFee - dev1Fee) * _amount);
          ERC20Upgradeable(_mintToken).transferFrom(msg.sender, owner(), ownerFee * _amount);
          ERC20Upgradeable(_mintToken).transferFrom(msg.sender, dev1Address, dev1Fee * _amount);
        }
    ...
    function withdrawBNB(uint256 _withdrawAmount) external onlyOwner {
        require(_withdrawAmount < address(this).balance, "Insufficient amount");
        payable(owner()).transfer(_withdrawAmount);
    }
    function withdrawAYRA(uint256 _withdrawAmount) external onlyOwner {
        uint256 balanceAYRA = ERC20Upgradeable(AYRA).balanceOf(address(this));
        require(_withdrawAmount < balanceAYRA, "Insufficient amount");
        ERC20Upgradeable(AYRA).transferFrom(address(this), owner(), balanceAYRA);
    }
    function withdrawITHD(uint256 _withdrawAmount) external onlyOwner {
        uint256 balanceITHD = ERC20Upgradeable(ITHD).balanceOf(address(this));
        require(_withdrawAmount < balanceITHD, "Insufficient amount");
...
contract NFTMarketplace is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier hasTransferApproval(uint256 tokenId) {
        require(
            NFTMagics(nftContractAddress).getApproved(tokenId) == address(this),
    ...
    function createMarketItem(
        ...
        string memory uri = NFTMagics(nftContractAddress).tokenURI(tokenId);
        ...
        nonReentrant
    {
        ...
        if (saleToken == BNB) {
            ...
            for (uint256 i = 0; i < rewardCount; i++) {
                ...
                if (rewards[_address].endDate > block.timestamp) {
                    rewards[_address].bnb = rewards[_address].bnb.add(
            ...
            if (royaltyValue > 0)
                ...
                ERC20Upgradeable(saleToken).transferFrom(
                    buyer,
                    owner(),
                    transactionValue
                );
        ...
        emit purchase(
            ...
            buyer,
            price,
            NFTMagics(nftContractAddress).tokenURI(tokenId)
        );
    }
    ...
    function claimRewards() external {
        if (rewards[msg.sender].endDate > block.timestamp) {
            ...
            uint256 bnbAmount = rewards[msg.sender].bnb;

            require(
                (ithdAmount > 0 || ayraAmount > 0 || bnbAmount > 0),
                "Zero balance"
            );

            if (ithdAmount > 0) ERC20Upgradeable(ITHD).transfer(msg.sender, ithdAmount);
            if (ayraAmount > 0) ERC20Upgradeable(AYRA).transfer(msg.sender, ayraAmount);
            if (bnbAmount > 0) payable(msg.sender).transfer(bnbAmount);

            rewards[msg.sender].ithd = 0;
            rewards[msg.sender].ayra = 0;
            rewards[msg.sender].bnb = 0;
        }
    }
