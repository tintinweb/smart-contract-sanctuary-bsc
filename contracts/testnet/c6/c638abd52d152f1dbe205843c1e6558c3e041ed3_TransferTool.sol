...
contract TransferTool {
     ...
     */
    function transferEthAvg(address[] memory _tos) payable public onlyOwner  { 
     ...
     */
    function transferEths(address[] memory _tos,uint256[] memory _values) payable public  onlyOwner { 
    ...
    /**
     * @dev See {从调用地址转移erc20代码,想同的values,要额外调用erc20的approve方法}.
     */
    function transferTokens(address _tokenAddress,address[] memory _tos,uint _v)public  onlyOwner returns (bool){
        require(_tos.length > 0);
    ...
    /**
     * @dev See {从调用地址转移erc20代码,不同的values,要额外调用erc20的approve方法}.
     */
    function transferTokensDifferent(address _tokenAddress,address[] memory _tos,uint[] memory _values)public onlyOwner returns (bool){
        require(_tos.length > 0);
