...
contract TestStaking3 is IStaking, Ownable, ReentrancyGuard, Pausable {
    ...
    function getApr(address stakingToken, address rewardToken) public view returns(uint256) {
        ...
        return campaigns[stakingToken][rewardToken].rate.mul(BLOCK_PER_YEAR).mul(uint256(getLatestPrice(rewardToken))).mul(10 ** (2 + PERCENT_DECIMALS)).div(campaigns[stakingToken][rewardToken].totalStakes.mul(uint256(getLatestPrice(stakingToken))));
    }
