    ...
    OwnableUpgradeable
{
    ...
    function createMetaStocksFranchiseUsingUnclaimedBNB(
        ...
        emit CreateFranchise(msg.sender, companyId, 0, 1);
    }
    ...
    function createMetaStocksFranchiseUsingBNB(
        ...
        companyFranchises[companyId][0] += _number;
        companyFranchisesCounter[companyId] += _number;

        for (uint256 index = franchisesNumber; index < _number; index++) {
            franchisesLastClaimDates[companyId][index] = block.timestamp;
        }

        franchiseContinents[_continentId] += _number;
        totalFranchises += _number;
        metaStocksFranchise.mint(_to, 0, _number, "0x0");
        emit CreateFranchise(msg.sender, companyId, 0, _number);
    }
