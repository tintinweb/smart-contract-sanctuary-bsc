...
interface ICOMM {
//    function getCommAmount(address _user, uint _totalBill) external view returns (uint _commAmount);
...
contract SellBox is Ownable {
    ...
    mapping(address => ChildBuyBox) public childBuyBox; // user => info
    ...
    constructor(IRefferal _refer, IERC1155 _box, IERC20 _tokenBuy, address _ceo) {
        refer = _refer;
        box = _box;
        tokenBuy = _tokenBuy;
        ceo = _ceo;
    ...
    function buy(uint _tokenId, uint _amount) external {
        ...
        uint totalBill = prices[_tokenId] * _amount;
//        uint comm = commTreasury.getCommAmount(_msgSender(), totalBill);
        uint comm = totalBill * 35 / 100;

        box.safeTransferFrom(ceo, _msgSender(), _tokenId, _amount, '0x');
        remains[_tokenId] -= _amount;

        address _refferBy;
        (,_refferBy,,,,) = refer.userInfos(_msgSender());
        if(!childBuyBox[_refferBy].isChildBuy[_msgSender()]) {
            childBuyBox[_refferBy].isChildBuy[_msgSender()] = true;
            childBuyBox[_refferBy].totalChildBuy++;
        }

        if(refer.isReferrer(_msgSender())) {
            tokenBuy.transferFrom(_msgSender(), ceo, totalBill - comm);
            tokenBuy.transferFrom(_msgSender(), address(commTreasury), comm);
            commTreasury.handleComm(_msgSender(), totalBill, tokenBuy);
        } else {
            tokenBuy.transferFrom(_msgSender(), ceo, totalBill);
        }
