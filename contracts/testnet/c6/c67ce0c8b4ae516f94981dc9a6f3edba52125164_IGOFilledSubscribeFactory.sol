...
contract IGOFilledSubscribe is Ownable, ReentrancyGuard {
    ...
    function getSubscribePayAmount(uint256 _subscribeAmount) public view returns (uint256) {
        ...
        uint256 _dividend = _payTokenTotal.mul(_subscribeAmount).mul(100000);
        uint256 _res = _dividend.div(_subscribeTokenTotal).div(100000);
    ...
    function pledge() public nonReentrant onlyWhiteList {
        ...
        IERC20(pledgeToken).transferFrom(msg.sender, address(this), pledgeTokenAmount);
        ...
        if ( checkPreemptionIsGoing() ) {
            ...
            if ( _payAmount > 0 ) {
                ...
                if ( payToken == BASEADDRESS ) {
                    ...
                    IERC20(payToken).transferFrom(msg.sender, feeTo, _payAmount);
                }
            }
    ...
    function claimSubscribe() public nonReentrant {
        ...
        if( _claimAmount > 0 ){
            ...
            IERC20(subscribeToken).transfer(msg.sender, _claimAmount);
        }
        ...
        if ( checkOutputIsEnd() && checkUserPledgeStatus(msg.sender) && (checkUserPledgeClaimStatus(msg.sender) == false) ){
            ...
            IERC20(pledgeToken).transfer(msg.sender, _userPledgeAmount);
        }
