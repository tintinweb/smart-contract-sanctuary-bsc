...
contract BuddyDao is Ownable, Pausable, ReentrancyGuard, SafeTransfer, AutomationCompatibleInterface {
    ...
    // auto white 自动化白名单，chainlink调用和自己调用
    mapping(address => bool) public autoWhiteList;
    ...
    modifier onlyWhitelist(address caller) {
        require(isWhitelist(caller), "Whitelist: caller is not the whitelist");
        _;
    }

    function isWhitelist(address _userAddress) public view returns (bool) {
        return autoWhiteList[_userAddress];
    }
    ...
    constructor (uint256 _serviceFee, address _serviceFeeAddress, uint256 _maxFixedRate) {
        ...
        autoWhiteList[msg.sender] = true;
    }

    // 设置白名单权限
    function setWhitelist(address _whiteAddress) external onlyOwner {
        require(_whiteAddress != address(0) , "whiteAddress must be a positive number");
        if (!autoWhiteList[_whiteAddress]){
            autoWhiteList[_whiteAddress] = true;
        }
    }
    ...
    // chainlink logic onlyWhitelist(msg.sender)
