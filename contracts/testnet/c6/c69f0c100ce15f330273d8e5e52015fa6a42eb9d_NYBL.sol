...
contract NYBL is Initializable, ERC1155Upgradeable, AccessControlUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable {
    ...
    function initialize(uint256 amountLimit_,string memory name_) initializer public {
    ...
    function setURI(string memory newuri) public onlyRole(PROJECT_ROLE) {
        _setURI(newuri);
    }

    function updateAmountLimit(uint256 amount) public onlyRole(PROJECT_ROLE) {
        amountLimit=amount;
    }

    function pause() public onlyRole(PROJECT_ROLE) {
        _pause();
    }

    function unpause() public onlyRole(PROJECT_ROLE) {
        _unpause();
    }

    function updateIdToAmount(uint256 id,uint256 amount) public onlyRole(PROJECT_ROLE) {
        idToAmount[id]=amount;
    }

    function updateIdToAmountBatch(uint256[] memory ids, uint256[] memory amounts) public onlyRole(PROJECT_ROLE) {
        require(ids.length==amounts.length, "Length of ids must be equal to amounts");
    ...
    {
        if(idToAmount[id]>0){
            require(totalSupply(id)<idToAmount[id], "Exceed the limit of amount");
        }else{
            require(totalSupply(id)<amountLimit, "Exceed the limit of amount");
        }
