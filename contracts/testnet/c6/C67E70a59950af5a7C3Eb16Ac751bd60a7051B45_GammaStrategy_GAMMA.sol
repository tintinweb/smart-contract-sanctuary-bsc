...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(address _userAddress, uint256 _wantAmt)
        ...
        returns (uint256){
        //to remove warning
        require(_userAddress == gammaInfinityVault,"User address is not gammaInfinityVault");

        //tranfer assets from user account to this contract
        ...
        //local variable to store how much share of user is added into current
        uint256 sharesAdded;

        //calculating new shares to be added into the existing one
    ...
    function withdraw(address _userAddress, uint256 _wantAmt)
        ...
        //amount given to withdraw should be greater than 0
        require(_wantAmt > 0, "_wantAmt <= 0");

        //local variable to store how much shares is removed
        uint256 sharesRemoved = _wantAmt.mul(sharesTotal).div(wantLockedTotal);
        if (sharesRemoved > sharesTotal) {
            sharesRemoved = sharesTotal;
        }

        //deduct user withdraw amount shares from total no. of shares
    ...
    function earn() public virtual nonReentrant whenNotPaused {
        ...
        _earnedAmt = distributeFees(_earnedAmt);

        if (_earnedAmt > 0) {
            
            Gtoken(wantAddress).mint(_earnedAmt);
    ...
    function setGammaInfinityAddress(address _gammaInfinityAddress)
        public
        virtual
        onlyAllowGov
    {
        gammaInfinityVault = _gammaInfinityAddress;
    }
...
contract GammaStrategy_GAMMA is StratX2 {
    ...
    constructor(
        ...
        IERC20(earnedAddress).approve(wantAddress,uint(-1));
    }
