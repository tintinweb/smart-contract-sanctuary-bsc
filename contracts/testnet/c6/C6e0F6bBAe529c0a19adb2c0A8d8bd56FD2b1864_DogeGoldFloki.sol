...
contract DogeGoldFloki is Context, IERC20, Ownable {
    ...
    string private _name     = "Doge Gold Floki";
    string private _symbol   = "$DGF";  
    ...
    event ChangedBuyFees(uint256 taxFee, uint256 marketingFee);
    event ChangedSellFees(uint256 taxFee, uint256 marketingFee);
    ...
    { 
        ...
        totalBuyFees = marketingFeeonBuy + taxFeeonBuy;
        totalSellFees = marketingFeeonSell + taxFeeonSell;
...
//---------- Tax and Transfer ----------//
    ...
    function setBuyFeesPercent(uint256 _taxFeeonBuy, uint256 _marketingFeeonBuy) external onlyOwner {
        taxFeeonBuy = _taxFeeonBuy;
        marketingFeeonBuy = _marketingFeeonBuy;
        totalBuyFees = _taxFeeonBuy + _marketingFeeonBuy;
        require(totalBuyFees + totalSellFees <= 25, "Total fees cannot be more than 25%");
        emit ChangedBuyFees(_taxFeeonBuy, _marketingFeeonBuy);
    }

    function setSellFeesPercent(uint256 _taxFeeonSell, uint256 _marketingFeeonSell) external onlyOwner {
        taxFeeonSell = _taxFeeonSell;
        marketingFeeonSell = _marketingFeeonSell;
        totalSellFees = _taxFeeonSell + _marketingFeeonSell;
        require(totalBuyFees + totalSellFees <= 25, "Total fees cannot be more than 25%");
        emit ChangedSellFees(_taxFeeonSell, _marketingFeeonSell);
    }

    function setBuyFees() private {
        if(_marketingFee==marketingFeeonBuy && _taxFee == taxFeeonBuy){
            return;
        }
        
        _taxFee = marketingFeeonBuy;
        _marketingFee = taxFeeonBuy;
    }

    function setSellFees() private {
        if(_marketingFee == marketingFeeonSell && _taxFee == taxFeeonSell){
            return;
        }
        
        _taxFee = marketingFeeonSell;
        _marketingFee = taxFeeonSell;
    }

    function setResetFees() private {
        if(_marketingFee == 0 && _taxFee == 0){
            return;
        }
