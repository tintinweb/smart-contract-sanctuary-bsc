...
contract WealthMountainLiquid is Ownable, ReentrancyGuard {
    ...
    uint256 public AverageLiqStakeValue; //CHANGE: Added this global, use CMD+F to follow

    uint256 public AllTimeLiqStakeCount; //CHANGE: Added this global, use CMD+F to follow

    uint256 public AllTimeSumOfStakes; //CHANGE: Added this global, use CMD+F to follow
    ...
    uint256 public LiqProfitCount; // All time count of distributions to Liquid Pool
    ...
    // Global globals
    uint256 public AllTimeUsers; //CHANGE: Updated a name here from GlobalUserCount
    ...
    function StakeLiq(uint256 stakeamt) public {
        ...
        BUSD.safeTransferFrom(address(this), InvWallet, liqf);
        ...
        AllTimeLiqStakeCount += 1; // This will only increase to help calculate our average
        AllTimeSumOfStakes += adjustedamt; // This will only increase to help calculate our average

        LiqPoolTotal += adjustedamt; // This is a real-time value that should adjust with unstakes
        user.totalStaked += adjustedamt; // This is a real-time value that should adjust with unstakes
        user.numOfStakesActive += 1; // This is a real-time value that should adjust with unstakes
        user.stakeCount += 1; // This will only increase, it's an all time count of user stakes
        addDailyValues();
    }
    ...
    event UserStake(uint stakeID, uint256 amount, uint256 stakeTime, bool withdrawn);

    function DisplayUserStakeInfo(address user) public view returns(LiqStake [] memory LiqStakeList) {
        LiqUser storage user = LiqUserKey[user];
    ...
    function calcdivProf(address calcaddy) public view returns (uint256) {
        ...
        uint256 damt; // CHANGE: Should we use this or remove it? Not sure...
    ...
    function donationRecalc(uint256 amtq) pure internal returns (uint256 doamt){ // CHANGE: from public to pure internal
    ...
    function returnAverageStakeValue() public view returns (uint256 avg) {
        uint256 average = AllTimeSumOfStakes / AllTimeLiqStakeCount;
        return average;
    }
