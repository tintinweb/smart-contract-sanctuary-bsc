...
contract NetGymFarming is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function speedStake(
        ...
        if (_tokenAmount > 0) {
            IERC20Upgradeable token = IERC20Upgradeable(lpToken.token0());

            if (lpToken.token0() == router.WETH()) {
                token = IERC20Upgradeable(lpToken.token1());
            }
        ...
        IWETH(wbnbAddress).deposit{value : (bnbAmount * buyBackComission) / 100}();
        IERC20Upgradeable(wbnbAddress).safeTransfer(buyBack, (bnbAmount * buyBackComission) / 100);
     ...
     */
    function deposit(uint256 _pid, uint256 _amount, address _to) public nonReentrant poolExists(_pid) {
        updatePool(_pid);
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            IGymMLM(relationship).distributeRewards(_amount, address(rewardToken), msg.sender);
