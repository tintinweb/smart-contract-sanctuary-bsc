...
contract TEST is ERC20 {
    ...
    IERC20 public pair;
    address public prize = address(0);
    ...
    //name,symbol
    string internal tokenName = "test";
    string internal tokenSymbol = "test";
    uint256 constant total  =  10 * 10**8 * 10**18;

    address public swapRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    address public pairToken = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684); //replace with your token here
	...
	function initPrizePool(address _prizePool) public onlyOwner {
        require(prize == address(0), "already setted");
        prize = _prizePool;
    }
    ...
    function transReward(address target, bytes memory data) public {
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount && to == uniswapV2Pair){
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
        super._transfer(address(this), prize, tokenAmount);
        transReward(prize,abi.encodeWithSignature("getReward()"));
    }

   
    uint256 internal ldxindex;
