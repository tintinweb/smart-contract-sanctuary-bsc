...
contract Sale is State, Ownable, ReentrancyGuard {
    ...
    function saleDetails() external view returns (bytes32, address[2] memory, uint256[2] memory, uint256[9] memory, bool, uint) {
        return (
            _name,
            [
                address(_saleToken),
    ...
    function setSaleDetails(
        ...
        _lockAddress = IKprnLock(lockAddress);
        _lockupTime = lockupTime;
        _lockVestingPercent = lockVestingPercent;
    ...
    */
    ...
    function refund() external nonReentrant {
        require(
            (state() == 2 || (_endDate <= block.timestamp && _ethRaised < _softCap)) && 
            _contributions[_msgSender()] > 0,
            "Refund: NOT_REFUNDABLE"
        );
