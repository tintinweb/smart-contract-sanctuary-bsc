...
contract TokenFarm is Ownable {
    ...
    address public usdcAddress= 0x6042d8a0CB887f20F0152B21C610e9a17D1A301b;
    ...
    function claimableRewards (uint256 _pid, address _user) public view returns (uint256) {
        IBEP20 usdcToken = IBEP20 (usdcAddress);
        IBEP20 token = IBEP20 (poolInfo[_pid].tokenAddress);
        uint256 currentAmount = usdcToken.balanceOf(address(this));
        uint256 totalUsdc = currentAmount.add(totalRewardsClaimed);
        uint256 totalTokens = token.balanceOf(address(this));
        uint256 userBalance = userInfo[_pid][_user].amount;
        uint256 userTotalRewardShare = totalUsdc.mul(userBalance).div(totalTokens);

        if (userTotalRewardShare > userInfo[_pid][_user].rewardClaimed)
            return userTotalRewardShare.sub(userInfo[_pid][_user].rewardClaimed);
        else
            return 0;
    ...
    function claimTokens (uint256 _pid) public lock returns (bool) {
        ...
        uint256 _claimableRewards = claimableRewards(_pid, msg.sender);
        require (_claimableRewards > 0, "You don't have any claimable rewards");
        totalRewardsClaimed = totalRewardsClaimed.add(_claimableRewards);
        userInfo[_pid][msg.sender].rewardClaimed = userInfo[_pid][msg.sender].rewardClaimed.add(_claimableRewards);

        IBEP20 usdcToken = IBEP20 (usdcAddress);
        bool success = usdcToken.transfer(msg.sender, _claimableRewards);
        require(success, "Transfer failed. Please approve the token");
        return true;
    }
    ...
    function unstakeTokens (uint256 _pid) public {
        ...
        if (!poolInfo[_pid].poolActive) claimTokens(_pid);
