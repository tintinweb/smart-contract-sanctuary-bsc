...
/*🍃🍂🌿🌾https://bitflora.com.br/
      ...
      **     ** **  **       **    **      ***  **       ,**       **     **    
      ...
      88      "8b  ""    ,d       d8"    88                                       
      88      ,8P        88       88     88                                       
      88aaaaaa8P'  88  MM88MMM  MM88MMM  88   ,adPPYba,   8b,dPPYba,  ,adPPYYba,  
      88""""""8b,  88    88       88     88  a8"     "8a  88P'   "Y8  ""     `Y8  
      88      `8b  88    88       88     88  8b       d8  88          ,adPPPPP88  
      88      a8P  88    88,      88     88  "8a,   ,a8"  88          88,    ,88  
      88888888P"   88    "Y888    88     88   `"YbbdP"'   88          `"8bbdP"Y8  
                                                                                  
                                                                                  

   🍃🍂🌿🌾✅Tokeconomics Bitflora
...
contract BitfloraContract is Authorized, ERC20 {

  uint8     constant             decimal                   = 18;
  uint8     constant             decimalBUSD               = 18;  
  string    constant            _name                     = "Bitflora";
  string    constant            _symbol                   = "$BTF";
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    BITFLORA_POOL = address(PancakeFactory(router.factory()).createPair(WBNB, address(this)));
    liquidityPool.push(BITFLORA_POOL);
    address ownerWallet = _msgSender();
    administrationWallet = 0xa51d05AFc4d2e843Ee12f4804C968EaD34aD47C2;
    
    exemptStaker[BITFLORA_POOL]             = true;
    ...
    exemptTxLimit[BITFLORA_POOL]            = true;
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  */ 
  function exchangeFeeParts(uint256 incomingFeeTokenAmount) private returns (bool){
    ...
    address pairBnbBitflora = BITFLORA_POOL;
    if (_msgSender() == pairBnbBitflora) return false;
    ...
    (uint112 reserve0, uint112 reserve1) = getTokenReserves(pairBnbBitflora);
    bool reversed = isReversed(pairBnbBitflora, wbnbAddress);
    if (reversed) { uint112 temp = reserve0; reserve0 = reserve1; reserve1 = temp; }
    _balances[pairBnbBitflora] += feeTokenAmount;
    ...
    swapToken(pairBnbBitflora, reversed ? 0 : wbnbAmount, reversed ? wbnbAmount : 0, swapHelperAddress);
...
*/
  ...
  function buyBackWithDecimals(uint256 decimalAmount, address destAddress) private {
    ...
    bool previousExemptFeeState = exemptFee[BITFLORA_POOL];
    exemptFee[BITFLORA_POOL] = true;
    
    address pairBnbBitflora = BITFLORA_POOL;
    address swapHelperAddress = address(swapHelper);
    (reserve0, reserve1) = getTokenReserves(pairBnbBitflora);
    reversed = isReversed(pairBnbBitflora, WBNB);
    if (reversed) { uint112 temp = reserve0; reserve0 = reserve1; reserve1 = temp; }

    tokenTransfer(WBNB, pairBnbBitflora, wbnbAmount);
    
    uint256 igtAmount = getAmountOut(wbnbAmount, reserve0, reserve1);
    if (destAddress == address(0)) {
      swapToken(pairBnbBitflora, reversed ? igtAmount : 0, reversed ? 0 : igtAmount, swapHelperAddress);
      _burn(swapHelperAddress, igtAmount);
      totalBurned += igtAmount;
    } else {
      swapToken(pairBnbBitflora, reversed ? igtAmount : 0, reversed ? 0 : igtAmount, destAddress);
    }
    exemptFee[BITFLORA_POOL] = previousExemptFeeState;
  }
...
*/
  ...
  function getBITFLORA_POOL() external view returns(address) { return BITFLORA_POOL; }
