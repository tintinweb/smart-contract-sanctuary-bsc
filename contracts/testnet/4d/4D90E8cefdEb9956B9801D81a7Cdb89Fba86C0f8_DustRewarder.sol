...
// This contract based on SushiSwapRewarder
...
interface IMasterChefV2 {
    ...
    function poolInfo(uint256 pid) external view returns (MasterChefV2.PoolInfo memory);
}

contract DustRewarder is IRewarder, BoringOwnable {
    ...
    event LogInit(IERC20 indexed rewardToken, address owner, uint256 rewardPerBlock, IERC20 indexed masterToken);

    constructor(address _MASTERCHEF_V2, IERC20 _rewardToken, uint256 _rewardPerBlock, IERC20 _masterToken) public {
        MASTERCHEF_V2 = _MASTERCHEF_V2;
        rewardToken = _rewardToken;
        rewardPerBlock = _rewardPerBlock;
        masterToken = _masterToken;
        emit LogInit(rewardToken, owner, rewardPerBlock, masterToken);
    }

    function onRewarded(uint256 pid, address _user, address to, uint256, uint256 lpTokenAmount) external override onlyMCV2 {
        require(IMasterChefV2(MASTERCHEF_V2).lpToken(pid) == masterToken, "Mismatch masterToken.");
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256 pending) {
        ...
        uint256 lpSupply = IMasterChefV2(MASTERCHEF_V2).poolInfo(_pid).totalDeposited;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 time = block.number.sub(pool.lastRewardBlock);
            uint256 reward = time.mul(rewardPerBlock);
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            uint256 lpSupply = IMasterChefV2(MASTERCHEF_V2).poolInfo(pid).totalDeposited;

            if (lpSupply > 0) {
                uint256 time = block.number.sub(pool.lastRewardBlock);
                uint256 reward = time.mul(rewardPerBlock);
                pool.accToken1PerShare = pool.accToken1PerShare.add((reward.mul(ACC_TOKEN_PRECISION) / lpSupply).to128());
            }
