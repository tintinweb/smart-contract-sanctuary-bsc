...
import {PredictionKeeper, IPrediction} from '../PredictionKeeper.sol';
import {IMockAggregatorV3} from '../interfaces/IMockAggregatorV3.sol';

contract MockPredictionKeeper is PredictionKeeper {

    constructor(address _predictionContract) PredictionKeeper(_predictionContract) {
    }

    function _executeRound() internal override {
        IMockAggregatorV3 oracle = IMockAggregatorV3(IPrediction(PredictionContract).oracle());
        int256 price = int256(uint256(blockhash(block.number - 1)) % 1000);
...
contract PredictionKeeper is KeeperCompatibleInterface, Ownable, Pausable {
    ...
    function _executeRound() internal virtual {
