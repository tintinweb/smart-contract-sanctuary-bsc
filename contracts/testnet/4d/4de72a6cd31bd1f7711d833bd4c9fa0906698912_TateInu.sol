...
Tokenomics
Buy Fee Total 8%
BUSD Rewards 4% | Liquidity 1% | Buyback & Burn 1% | Marketing & Development 1% | Affordable Housing 1%

Sell Fee Total 13%
BUSD Rewards 8% | Liquidity 2% | Buyback & Burn 2% | Marketing & Development 1% | Affordable Housing 2%
*/

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.13; // latest version
...
contract DividendDistributor {
    ...
    IERC20 BUSDReward = IERC20(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47); // Mainnet
...
abstract contract Taxable is Owned, Tradable {
    ...
    Tradable(symbol, name, tokenDistribution) {
        ...
        _totalBuyTaxes = buyTaxes.AffordableHousingFee + buyTaxes.rewardsFee + buyTaxes.marketingFee + buyTaxes.BuybackBurnFee + buyTaxes.liqFee;
        _totalSellTaxes = sellTaxes.AffordableHousingFee + sellTaxes.rewardsFee + sellTaxes.marketingFee + sellTaxes.BuybackBurnFee + sellTaxes.liqFee;
        _maxFees = maxFees;
    ...
    function setAffordableHousingAddress(address payable newAffordableHousingAddress) external onlyOwner() {
    ...
    function setBuyFees(uint8 newAffordableHousingBuyFee, uint8 newRewardsBuyFee, uint8 newMarketingBuyFee, uint8 newBuybackBurnBuyFee, uint8 newLiqBuyFee) external onlyOwner {
        uint8 newTotalBuyFees = newAffordableHousingBuyFee + newRewardsBuyFee + newMarketingBuyFee + newBuybackBurnBuyFee + newLiqBuyFee;
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(newAffordableHousingBuyFee <= _maxAffordableHousingFee, "Cannot set AffordableHousing fee higher than max");
        require(newTotalBuyFees <= _maxFees, "Cannot set total buy fees higher than max");

        _buyTaxes = Taxes({ AffordableHousingFee: newAffordableHousingBuyFee, rewardsFee: newRewardsBuyFee, marketingFee: newMarketingBuyFee,
            BuybackBurnFee: newBuybackBurnBuyFee, liqFee: newLiqBuyFee });
        _totalBuyTaxes = newTotalBuyFees;
    }

    function setSellFees(uint8 newAffordableHousingSellFee, uint8 newRewardsSellFee, uint8 newMarketingSellFee, uint8 newBuybackBurnSellFee, uint8 newLiqSellFee) external onlyOwner {
        uint8 newTotalSellFees = newAffordableHousingSellFee + newRewardsSellFee + newMarketingSellFee + newBuybackBurnSellFee + newLiqSellFee;
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(newAffordableHousingSellFee <= _maxAffordableHousingFee, "Cannot set AffordableHousing fee higher than max");
        require(newTotalSellFees <= _maxFees, "Cannot set total sell fees higher than max");

        _sellTaxes = Taxes({ AffordableHousingFee: newAffordableHousingSellFee, rewardsFee: newRewardsSellFee, marketingFee: newMarketingSellFee,
    ...
    function calculateTotalFees(uint256 amount, uint8 txType) private returns (uint256) {
        uint256 AffordableHousingTokens = (txType == BUYTX) ? amount.mul(_buyTaxes.AffordableHousingFee).div(100) : amount.mul(_sellTaxes.AffordableHousingFee).div(100);
        ...
        _AffordableHousingTokensCollected = _AffordableHousingTokensCollected.add(AffordableHousingTokens);
        ...
        return AffordableHousingTokens.add(rewardsTokens).add(marketingTokens).add(BuybackBurnTokens).add(liqTokens);
    }

    function swapCollectedFeesForFunding() private lockTheSwap {
        uint256 totalCollected = _AffordableHousingTokensCollected.add(_marketingTokensCollected).add(_liqTokensCollected)
        ...
        uint256 totalAmountToSwap = _AffordableHousingTokensCollected.add(_rewardsTokensCollected).add(_marketingTokensCollected)
        ...
        uint256 AffordableHousingFunds = newFunds.sub(liqFunds).sub(marketingFunds).sub(rewardsFunds).sub(BuybackBurnFunds);

        addLiquidity(otherHalfLiq, liqFunds);
        (bool sent, bytes memory data) = _AffordableHousingAddress.call{value: AffordableHousingFunds}("");
...
library Address {
    ...
    //Name of Contract & Change token decimals
contract TateInu is Context, Owned, Taxable {
	using SafeMath for uint256;
	using Address for address;

    string private _Bname = "TateInu";
    string private _Bsymbol = "TATE";
    // 18 Decimals
    ...
    address payable _BAffordableHousingAddress = payable(address(0x19D0607781585de4e28ee808796DA7efdfa97554));
    address payable _BmarketingAddress = payable(address(0x90C0709435A4DCf8f30c6ba958A7F7B3A77E5565));
    address payable _BBuybackBurnAddress = payable(address(0x33Fa48F8A3f476831d50fE7114B3b561f997FB04));
    ...
    constructor () 
    Owned(_msgSender())
    Taxable(_Bsymbol, _Bname, _BtokenDistribution, _BAffordableHousingAddress, _BmarketingAddress, _BBuybackBurnAddress,
            Taxes({ AffordableHousingFee: 1, rewardsFee: 2, marketingFee: 32, BuybackBurnFee: 3, liqFee: 61 }), 
            Taxes({ AffordableHousingFee: 1, rewardsFee: 2, marketingFee: 32, BuybackBurnFee: 3, liqFee: 61 }), 
            _BmaxFees, _BmaxAffordableHousingFee, _BliquifyThreshhold) {
