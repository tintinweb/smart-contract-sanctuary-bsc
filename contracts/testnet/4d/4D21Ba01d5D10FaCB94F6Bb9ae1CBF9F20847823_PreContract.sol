...
interface IInvest {
   ...
   * @dev Returns the amount of tokens in existence.
   */
  function invest(address owner, address referrer, uint8 plan) external;
}
...
contract PreContract {
  ...
  constructor(address _base, address _baseToken, address _investContract) public {
    base = _base;
  ...
  function invest(address referrer, uint8 plan) public {
    ...
    IInvest(investContract).invest(msg.sender, referrer, plan);
