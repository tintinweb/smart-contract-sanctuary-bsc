...
contract TheTestShop is Ownable, ERC721AQueryable, PaymentSplitter {
    ...
    // Payable services
    uint public coinsPrice = 0.005 ether;
    uint public randomRunePrice = 0.008 ether;
    uint public magicCookiePrice = 0.002 ether;
    uint public resScrollPrice = 0.008 ether;
    ...
    constructor(address[] memory _team, uint[] memory _teamShares) ERC721A("Test Shop", "TestSHOP") PaymentSplitter(_team, _teamShares){
        // TO DELETE ----------
        balances[0x5B38Da6a701c568545dCfcB03FcB875f56beddC4] = 10000;
        balances[0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2] = 10000;
        setApprovalForAll(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, true);
        setApprovalForAll(0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2, true);
        
        // Set Items
        setItem(randomRuneID, "Random Rune", "Gives random stats when used.", 150, "Unusual", "Rune", "ipfs://rune", 0, 0, 0, 0, 0);
        setItem(magicCookieID, "Magic Cookie", "Heals 50% of Test HP.", 50, "Common", "Cookie", "ipfs://cookie", 0, 0, 0, 0, 0);
        setItem(reviveScrollID, "Resurrection Scroll", "Revive your Super Test.", 150, "Common", "Scroll", "ipfs://scroll", 0, 0, 0, 0, 0);
        setItem(3, "Dague", "Stabby Stabby.", 25, "Common", "Weapon", "ipfs://dague", 0, 0, 0, 20, 5);
        setItem(4, "Sword", "Stabby STABBY.", 50, "Unusual", "Weapon", "ipfs://sword", 0, 0, 0, 25, 8);
        setItem(5, "Nut", "Nutty Nutty.", 10, "Common", "Consumable", "ipfs://nut", 5, 0, 0, 0, 0);
        setItem(6, "Wooden Armor", "Woody", 10, "Common", "Armor", "ipfs://armor", 0, 1, 2, 3, 4);
        setItem(7, "Wooden Collar", "Collar", 10, "Common", "Collar", "ipfs://armor", 0, 1, 2, 3, 4);
    ...
    function setRing(address ring) public onlyOwner{
        allowedContract = ring;
        setApprovalForAll(ring, true);
    }
    ...
    function getCoins() external payable{
        if(msg.value < coinsPrice) revert("Not enough funds");
        balances[msg.sender] += coinsAmountToSell;
    }

    function uniqueRewards(address[] memory owners) external onlyOwner {
        // Loop for each owner, give 4 runes, 2 magic cookies, 2 resurrection scrolls
        for (uint i = 0; i < owners.length; i++) {
            balances[owners[i]] += 200;
    ...
    function getRandomRune(address owner) external payable{
        if(msg.value < randomRunePrice) revert("Not enough funds");
        tokenToItemIds[_nextTokenId()] = randomRuneID;
        _mint(owner, 1);
    }

    function getMagicCookie(address owner) external payable{
        if(msg.value < magicCookiePrice) revert("Not enough funds");
        tokenToItemIds[_nextTokenId()] = magicCookieID;
        _mint(owner, 1);
    }

    function getResurrectionScroll(address owner) external payable{
        if(msg.value < resScrollPrice) revert("Not enough funds");
        tokenToItemIds[_nextTokenId()] = reviveScrollID;
        _mint(owner, 1);
    }
    ...
    function buyItem(uint _quantity, uint _itemId) external  {
        ...
        setApprovalForAll(address(allowedContract), true);
