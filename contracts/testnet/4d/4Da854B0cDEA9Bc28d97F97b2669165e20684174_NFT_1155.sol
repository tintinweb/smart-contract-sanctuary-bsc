...
contract NFT_1155 is ERC1155, ERC1155URIStorage, ERC1155Supply, Ownable {
    ...
    // Mapping just for NFT from token ID to owner address
    mapping(uint256 => address) private _owners;

    // Implementation of incremental token ID
    ...
    function isNotFungible( uint256 id ) public view returns (bool) {
        require( ERC1155Supply.exists( id ), "NFT1155: is Non Fungible query for nonexistent token" );
        return bool( ERC1155Supply.totalSupply( id ) == 1 );
    }

    function convertToNonFungible( address owner, uint256 id ) public {
        require( ERC1155Supply.exists( id ), "NFT1155: convert query for nonexistent token" );
        require( ! isNotFungible( id ), "NFT1155: this token is already Non Fungible" );
        require( ERC1155.balanceOf( owner, id ) > 0, "NFT1155: convert query from non owner" );

        uint256 idToMint = _tokenIdCounter.current() + 1;

        burn( owner, id, 1);
        mint( owner, idToMint, 1 );
    }

    function totalSupply( uint256 id ) public view virtual override returns (uint256) {
        require( ERC1155Supply.exists( id ), "NFT1155: supply query for nonexistent token" );
        return ERC1155Supply.totalSupply( id );
    }

    function balanceOf( address account, uint256 id ) public view virtual override returns (uint256) {
        require( ERC1155Supply.exists( id ), "NFT1155: balance query for nonexistent token" );
    ...
    function mintBatch( address to, uint256[] memory tokenIDs, uint256[] memory amounts ) public onlyOwner {
        require( amounts.length >= 1 , "NFT1155: amounts length must grater than or equal to 1" );
    ...
    function burnBatch( address from, uint256[] memory tokenIDs, uint256[] memory amounts ) public onlyOwner {
        require( tokenIDs.length == amounts.length, "NFT1155: tokenIDs and amounts length must be equals" );
        ERC1155._burnBatch( from, tokenIDs, amounts );
    }

    function transfer( address from, address to, uint256 id, uint256 amount) public virtual {
        require( from == _msgSender() || ERC1155.isApprovedForAll(from, _msgSender()), "NFT1155: caller is not owner nor approved" );
        ERC1155._safeTransferFrom(from, to, id, amount, new bytes(0));
    }

    function transferBatch( address from, address to, uint256[] memory ids, uint256[] memory amounts) public virtual {
        require( from == _msgSender() || ERC1155.isApprovedForAll(from, _msgSender()), "NFT1155: caller is not owner nor approved" );
        ERC1155._safeBatchTransferFrom(from, to, ids, amounts, new bytes(0));
    }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            bool isNonFungible = amounts[i] == 1;
            ...
            if ( from == address(0) ) {
                ...
                require( _tokenIdCounter.current() == ids[i], "NFT1155: id/s must be consistent for minting" );
            ...
            if ( to == address(0) ){
                ...
                // Reset token URI
                if ( ERC1155Supply.totalSupply( ids[i] ) == 0 ) ERC1155URIStorage._setURI( ids[i], "" );
            }
        }
    }
