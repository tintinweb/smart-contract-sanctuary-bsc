...
contract HashratePool is Ownable {
    ...
    // mapping (address => Hashrate[]) public ownerStake;
    
    // mapping (address => Hashrate) public hashrates;
    // address[] public ownerStake;
    ...
    //总质押记录列表
    mapping (uint256 => StakeRecord) stakeTimeStamp;
    uint256[] stakeTimeStampList;
    ...
    function HashrateTotal() external view returns (uint256 _total) {
        ...
        for (uint i = 0; i < hashrates.length; i++) {
            _total += NFTHelpADV2.getRealHashrate(hashrates[i].staker);
        }
    }
    ...
    function recordStake() internal {
        ...
        if (stakeTimeStamp[_now].lastAirDrop != lastAirDrop) stakeTimeStamp[_now].lastAirDrop = lastAirDrop;
    }
    ...
    function openBox(uint32 _num) external lock returns(uint256[] memory _tks, uint256[] memory _qus, uint256[] memory _hrs, uint256[] memory _tys) {
        ...
        (_qus,_tys, _useIdx, _hrs,)  = YiSetting.calcOpenBox(_num);
