...
contract Milez is Pausable, AccessControl {
    ...
    function setPLAYERZ(IPLAYERZ PLAYERZ) external onlyRole(GAME_ADMIN) {_playerz = IPLAYERZ(PLAYERZ);}
    function getPLAYERZ() external view returns(address playerzAddress) {playerzAddress = address(_playerz);}
    function setTICKETZ(ITICKETZ TICKETZ) external onlyRole(GAME_ADMIN) {_ticketz = ITICKETZ(TICKETZ);}
    function getTICKETZ() external view returns(address ticketzAddress) {ticketzAddress = address(_ticketz);}
    ...
    external
    {
        ...
        //uint256 playerTier = _playerz.getPlayerFlyerTier(playerId);
        //require(playerTier < pkgId, "Current tier is higher.");
        _spendPlayerMilez(playerId, tier.pkgPrice);
