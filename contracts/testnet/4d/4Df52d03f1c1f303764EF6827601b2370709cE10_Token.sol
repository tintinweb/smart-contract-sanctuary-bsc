...
import "./Officer.sol"; //{ IOfficer } from
...
abstract contract TokenData {
	...
	function _getStatusToken(address _token) internal view returns (uint8 _status) {
	...
	function _addUpdateToken(address _token, uint8 _status) internal {
		if (_isToken(_token)) {
			// Update exits token
...
interface IToken {
	function getToken(address token) external view returns (TokenInfo memory tokeninfo);

	function getAvailableToken() external view returns (TokenInfo[] memory tokeninfo);
...
contract Token is TokenData, IToken {
	...
	function getToken(address token) external view onlyAccepted returns (TokenInfo memory tokeninfo) {
		require(_isAvailableToken(token), "Token: not supported or available");
		tokeninfo = _getToken(token);
	}

	// Dashboard view
	function getAvailableToken() external view onlyAccepted returns (TokenInfo[] memory tokeninfo) {
	...
	function tokenDeposit(address token, uint256 amount) external virtual onlyAccepted {
	...
	function tokenWithdraw(address token, uint256 amount) external virtual onlyAccepted {
		require(amount > 0, "Token: amount need more than 0");
		require(_isAvailableToken(token), "Token: not available");
	...
	function addUpdateToken(ERC20 token, uint8 status) external virtual onlyAccepted {
	...
	function Monitoring() external view virtual onlyAccepted returns (TokenInfo[] memory tokeninfos) {
		require(officer.isMonitoring(msg.sender), "Token: caller is not Monitoring");
		tokeninfos = _getToken();
		//...
	}

	function Bridging() external view virtual onlyAccepted returns (TokenInfo[] memory tokeninfos) {
...
interface IOfficer {
	function isAvailableOfficer(address officer) external view returns (bool);

	function getOfficerStatus() external view returns (uint256 status);

	function isAccepted(address caller) external view returns (bool);

	function getAcceptedRole(address caller) external view returns (uint256 role);

	function isGovernment(address caller) external view returns (bool);

	function isMonitoring(address caller) external view returns (bool);

	function isBridging(address caller) external view returns (bool);
}
