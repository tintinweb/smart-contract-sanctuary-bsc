...
contract BondRouter is Ownable, BondStruct, Pausable, ReentrancyGuard {
    ...
    event CreateNewBatch(uint256 _batchId, BatchConfig _config, BackedBond[] _backedBond, uint256[] _prices);
    ...
    function _executeHarvest(uint256 _batchId, uint256[] memory _ids, address _to) internal {
        ...
        if (IERC20(_info.currency).balanceOf(address(this)) >= _amountSent) {
            _payment(IERC20(_info.currency), _to, _amountSent);
        } else {
            _lock(bondNFTAddress, _ids);
    ...
    function createNewBatch(BatchConfig memory _config, BackedBond[] memory _backedBond, uint256[] memory _prices) external onlyOperator {
        batchId ++;
        ...
        emit CreateNewBatch(batchId, _config, _backedBond, batchInfo[batchId].bondPrice.values());
    }
