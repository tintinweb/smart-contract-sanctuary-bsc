...
contract Staking is Ownable, ReentrancyGuard {
    ...
    IERC20 public token;
    IERC20 public USDT = IERC20(0x8DFb025853e10184036ec64cAEE471cB6EC3949c);
    ...
    function pendingReward(address _user) public view returns (uint256, uint256) {
        ...
        if (block.timestamp > lastRewardTime && totalStakedAmount != 0) {
            ...
            uint256 adjustedEthPerShare = accEthPerShare.add(amountEthForReward.mul(PRECISION_FACTOR).div(totalStakedAmount));
...
contract TokenContract is ERC20, Ownable {
    ...
    uint256 public _ethReflectionFee = 5;
    uint256 public _stakingFee = 8;
    ...
    function updateTaxFees(uint256 _liquid, uint256 _ethReflection, uint256 _staking, uint256 _marketing) public onlyOwner {
        require(_liquid + _ethReflection + _staking + _marketing <= 20, "Total tax must less then 20");
    ...
    function _transfer(
        ...
        if(takeFee) {
        	uint256 fees = amount.mul(_liquidityFee.add(_ethReflectionFee).add(_stakingFee).add(_marketingFee)).div(100);
    ...
    function swapAndSend() private lockTheSwap {
        ...
        uint256 amountForMarketing = contractTokenBalance.sub(amountForLiquidity).sub(amountForEthReflection).sub(amountForStaking);
        ...
        // swap
        uint256 swapAmount = half.add(amountForEthReflection).add(amountForStaking).add(amountForMarketing);
        ...
        uint256 ethMarketing = ethBalance.sub(ethLiquid).sub(ethReflection).sub(ethStaking);
