...
contract Bizforce is Context, IERC20, Ownable,Initializable {   
    ...
    uint256 private _maxHoldingLimits =10000000000000000000000000000;
    ...
    uint256 [4] private _developmentTax  = [0,0,0,0];
   ...
   function pauseTransaction() onlyOwner private {
        paused = true;
        emit Pause();
   }

    /* Smart Contract Owner Can Unpause The Token Transaction if token previously paused */
    function unpauseTransaction() onlyOwner private {
        paused = false;
        emit Unpause();
    }
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance,uint TaxType) private lockTheSwap {
        ...
        //swap tokens for ETH
        swapTokensForEth(forMarketing); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
       ...
       uint256 _aggregateTax=_marketingTax[TaxType];
        ...
        _totalMarketingCollected=0;
       emit SwapAndLiquify(FullExp, Balance,_aggregateTax);
