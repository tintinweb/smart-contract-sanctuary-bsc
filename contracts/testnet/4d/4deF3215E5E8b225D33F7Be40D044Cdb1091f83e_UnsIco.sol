...
contract UnsIco is AccessControl {
    ...
    function buyUnsWithStableToken(
        ...
        require(_icoTokens[address(token)], "!_icoToken");
    ...
    /**
     * @notice Function to withdraw ERC20
    ...
    function withdrawETH() external onlyRole(ICO_ADMIN_ROLE) {
        ...
        if (ethBalance > 0) {
            require(payable(_msgSender()).send(ethBalance), "!recoverETH");
        }
    }
    ...
    function _usnForStable(address token, uint256 amount) public view returns (uint256) {
        require(_icoTokens[token], "!_icoToken");
    ...
    function _usnForBnb(uint256 amount) public view returns(uint256){
    ...
    function _usnForUsd(uint256 usdAmount) internal view returns (uint256) {
        return (usdAmount * (100)) / _unsPrice;
    }
