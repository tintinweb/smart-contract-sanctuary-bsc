...
import "./interfaces/IMarketDistribution.sol";
import "./interfaces/IMarketGeneration.sol";
import "./utils/TokensRecoverable.sol";
...
contract MarketDistribution is TokensRecoverable, IMarketDistribution {
    ...
    uint256 public recoveryDate;

    uint256 public baseAmountForLiquidity;
    uint256 public rootedAmountForLiquidity;
    ...
    function init(
        ...
        uint256 _vestingDuration) public ownerOnly()
    {
        rootedToken = _rootedToken;
        ...
        vestingDuration = (_vestingDuration * 1 minutes);
    }
    ...
    function completeSetup() public ownerOnly() {
        ...
        eliteToken.approve(address(uniswapV2Router), type(uint128).max);
        rootedToken.approve(address(uniswapV2Router), type(uint128).max);
        baseToken.safeApprove(address(uniswapV2Router), type(uint128).max);
        baseToken.safeApprove(address(eliteToken), type(uint128).max);
        rootedBaseLP.approve(address(uniswapV2Router), type(uint128).max);
        rootedEliteLP.approve(address(uniswapV2Router), type(uint128).max);
    }

    function setLiquidityController(address _liquidityController) public ownerOnly() {
        liquidityController = _liquidityController;
    }

    // baseToken = CRO
    function distribute(uint256 _baseAmountForLiquidity, uint256 _rootedAmountForLiquidity,
        uint256 _lockTimeInSeconds, uint256 _maxTransactionAmount, uint16 _additionalDumpTaxRate,
        uint256 _dumpTaxRateDurationInSeconds) public override {
        require (msg.sender == address(marketGeneration), "Unauthorized");
        require (!distributionComplete, "Distribution complete");

        baseAmountForLiquidity = _baseAmountForLiquidity;
        rootedAmountForLiquidity = _rootedAmountForLiquidity;

        vestingPeriodStartTime = block.timestamp;
        vestingPeriodEndTime = block.timestamp + (vestingDuration * 1 minutes);
        // Extra day added after the vesting period end completely..
        recoveryDate = ((block.timestamp + (vestingDuration * 1 minutes)) + 1 minutes);
        distributionComplete = true;

        baseToken.safeTransferFrom(msg.sender, address(this), baseToken.balanceOf(address(marketGeneration)));

        RootedTransferGate gate = RootedTransferGate(address(rootedToken.transferGate()));

        gate.setUnrestricted(true);

        //rootedToken.addAddressToWhitelist(address(this));
        gate.setDumpTax(_additionalDumpTaxRate, _dumpTaxRateDurationInSeconds);
        //rootedToken.initAntiSnipe(_lockTimeInSeconds, _maxTransactionAmount);
        rootedToken.unpause();
        rootedToken.mint(totalSupply);

        createRootedEliteLiquidity(_baseAmountForLiquidity);

        eliteToken.sweepFloor(address(this));
        eliteToken.depositTokens(baseToken.balanceOf(address(this)));

        // CRO
        baseToken.transfer(liquidityController, _baseAmountForLiquidity);
        baseToken.transfer(projectAddress, baseToken.balanceOf(address(this)));
    ...
    function createRootedEliteLiquidity(uint256 _baseAmountForLiquidity) private {
        eliteToken.depositTokens(_baseAmountForLiquidity);

        uint256 eliteTokenAmountForLiquidity = (eliteToken.balanceOf(address(this)) * (rootedAmountForLiquidity / totalSupply));

        uniswapV2Router.addLiquidity(address(eliteToken), address(rootedToken),
        eliteTokenAmountForLiquidity, rootedAmountForLiquidity, 0, 0, address(this), block.timestamp);
        eliteToken.withdrawTokens(baseToken.balanceOf(address(eliteToken)));
    }

    function createRootedBaseLiquidity() private {
        uniswapV2Router.addLiquidity(address(baseToken), address(rootedToken), baseAmountForLiquidity,
            rootedAmountForLiquidity, 0, 0, liquidityController, block.timestamp);
    ...
    function getTotalClaim(address account) public view returns (uint256) {
        return  marketGeneration.getTotalClaim(account);
    }
    ...
    function claim(address account) public override {
        ...
        require (totalClaim[account] > 0, "No available tokens");
    ...
    function getClaimInfo(address account) public view returns (uint256, uint256, uint256) {
        if (totalClaim[account] == 0){
            return (0, 0 , getTotalClaim(account));
        } else {
            uint256 share = totalClaim[account];
            uint256 endTime = vestingPeriodEndTime > block.timestamp ? block.timestamp : vestingPeriodEndTime;

            uint256 _remaining = remainingClaim[account];
            uint256 _claimed = totalClaim[account] - remainingClaim[account];

            if(claimTime[account] < endTime) {
                return (_claimed, 0, _remaining);
            }

            uint256 claimStartTime = claimTime[account] == 0 ? vestingPeriodStartTime : claimTime[account];
            uint256 _available = (endTime.sub(claimStartTime)).mul(share).div(vestingDuration);

            return (_claimed, _available, _remaining);
        }
    }

    function getRootedBaseLP() public view returns (IUniswapV2Pair) {
        return rootedBaseLP;
    }

    function getRootedEliteLP() public view returns (IUniswapV2Pair) {
        return rootedEliteLP;
    }
...
import "./../interfaces/ILiquidityLockedERC20.sol";
import "./../utils/AntiSnipeWhitelist.sol";

abstract contract LiquidityLockedERC20 is GatedERC20, ILiquidityLockedERC20, AntiSnipeWhitelist, Pausable
{
    mapping (IUniswapV2Pair => bool) public liquidityPairLocked;
    mapping (address => bool) public liquidityController;
    mapping (address => bool) public unPauseController;

    uint256 public antiSnipePeriod;
    ...
    modifier unpauseControllerOnly()
    {
        require (msg.sender == owner || unPauseController[msg.sender], "Not an owner or unpause controller");
        _;
    }

    constructor(string memory _name, string memory _symbol, uint8 _decimals)
        GatedERC20(_name, _symbol, _decimals)
    {
    ...
    function setUnPauseController(address _unPauseController, bool _canControl) public ownerOnly()
    {
        unPauseController[_unPauseController] = _canControl;
    }


    function balanceOf(address account) public override(IERC20, StandardERC20) view returns (uint256)
    {
    ...
    function transfer(address recipient, uint256 amount) public virtual override(IERC20, StandardERC20) returns (bool)
    {
        //if lock is active AND recipient is not whitelisted, recipient can't buy
        if (block.timestamp < antiSnipePeriod) {
            require(whitelist[recipient], "Account not whitelisted to buy in the first minute(s) of launch");
            require(amount <= maxTransactionAmount, "Whitelisted account cannot buy more than allowed in the first minute(s) of launch");
        }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override(IERC20, StandardERC20) returns (bool)
    {
    ...
    function initAntiSnipe(uint256 antiSnipePeriodInSeconds, uint256 maxAmountToBuyDuringThisPeriod) public whenPaused ownerOnly() {
        require(antiSnipePeriod == 0, "AntiSnipe Already activated at the launch of the project!");
        // This code is called only one time at the launch of the project
        // to allow only whitelisted people to buy in the first seconds defined
        // with a maximum amount of tokens allowed.
        maxTransactionAmount = maxAmountToBuyDuringThisPeriod;
        antiSnipePeriod = block.timestamp + (antiSnipePeriodInSeconds * 1 seconds);
        super._unpause();
    }

    function unpause() public whenPaused unpauseControllerOnly() {
        super._unpause();
    }

    function pause() public whenNotPaused ownerOnly() {
        super._pause();
    }
...
import "./../interfaces/ITransferGate.sol";
import "./../interfaces/IGatedERC20.sol";

abstract contract GatedERC20 is StandardERC20, AccessControl, TokensRecoverable, IGatedERC20
{
    using SafeMath for uint256;
    ...
    constructor(string memory _name, string memory _symbol, uint8 _decimals) StandardERC20(_name, _symbol, _decimals)
    {
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override
    {
        ...
        if (address(_transferGate) != address(0)) 
        {
            ...
            if (fees > 0)
            {
                _balances[splitter] = _balances[splitter].add(fees);
...
contract BlackListProtection is Ownable {
    ...
    function setBlackListed(address _account, bool _blackListed) public onlyOwner
    {
        if (_blackListed)
        {
            blackListedAddresses[_account] = _blackListed;
            emit BlackListedAddressAdded(_account);
        }
        else
        {
            delete blackListedAddresses[_account];
            emit BlackListedAddressRemoved(_account);
        }
    }

    function checkBlackListed(address _account) public view returns (bool){
        if(blackListedAddresses[_account]) {
            return true;
        }
        return false;
    }
...
contract AntiSnipeWhitelist is Context {
    ...
    event ASWOwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    constructor () {
        address msgSender = _msgSender();
        _owner = msgSender;
        emit ASWOwnershipTransferred(address(0), msgSender);
    }
...
interface IWrappedERC20 is IERC20Metadata, IWrappedERC20Events
{
    function wrappedToken() external view returns (IERC20Metadata);
...
interface IMarketGeneration {
    ...
    function getTotalClaim(address) external view returns (uint256);
}
...
interface IMarketDistribution {
    ...
    function distribute(uint256 _baseAmountForLiquidity, uint256 _rootedAndEliteAmountForLiquidity, uint256 _lockTimeInSeconds, uint256 _maxTransactionAmount, uint16 _additionalDumpTaxRate, uint256 _dumpTaxRateDurationInSeconds) external;
...
import "./interfaces/ILiquidityLockedERC20.sol";
import "./interfaces/ITransferGate.sol";
import "./utils/BlackListProtection.sol";
import "./utils/TokensRecoverable.sol";
...
{   
    ...
    IUniswapV2Router02 immutable internal pancakeRouter;
    ...
    BlackListProtection public blackListProtection;
   
    uint16 public dumpTaxStartRate; 
    uint256 public dumpTaxDurationInSeconds;
    uint256 public dumpTaxEndTimestamp;

    constructor(ILiquidityLockedERC20 _rootedToken, IUniswapV2Router02 _pancakeRouter)
    {
        rootedToken = _rootedToken;
        pancakeRouter = _pancakeRouter;
        pancakeFactory = IUniswapV2Factory(_pancakeRouter.factory());
    }
    ...
    function setFeeController(address feeController, bool allow) public ownerOnly()
    {
    ...
    function setBlackListProtection(BlackListProtection _blackListProtection) public ownerOnly()
    {
        blackListProtection = _blackListProtection;
    }
    ...
    function setDumpTax(uint16 startTaxRate, uint256 durationInSeconds) public
    {
        ...
        dumpTaxEndTimestamp = block.timestamp + (durationInSeconds * 1 seconds);
    }
    ...
    function handleTransfer(address, address from, address to, uint256 amount) public virtual override returns (uint256)
    {
        ...
        if (blackListProtection.checkBlackListed(from) || blackListProtection.checkBlackListed(to))
        {
            return amount;
        }
...
import "./utils/LiquidityLockedERC20.sol";
...
contract RootedToken is LiquidityLockedERC20 {
    ...
    constructor(string memory _name, string memory _symbol, uint8 _decimals)
        LiquidityLockedERC20(_name, _symbol, _decimals)
    {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _pause();
    }

    function setMinter(address _marketGenerationDistributionAddress) public ownerOnly() {
        _setupRole(MINTER_ROLE, _marketGenerationDistributionAddress);
    }
