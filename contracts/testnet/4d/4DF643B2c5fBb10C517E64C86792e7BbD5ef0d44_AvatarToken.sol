...
contract AvatarToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellLpRewardFee = 3;
    ...
    function setEnableBuy(bool _enable) external onlyOwner{
        enableBuy = _enable;
    }
    ...
    function _transfer(
        ...
        require(enableBuy || from != uniswapV2Pair || isExcludedFromFees(to),"Buy not enable");
        ...
        if( canSwap &&
            ...
            if(AmountLpRewardFee > 0){
                _swapTokenForUsdt(AmountLpRewardFee);
                AmountLpRewardFee = 0;
            }
            swapping = false;
        }
        ...
        if(takeFee && from != uniswapV2Pair) {
            amount =  _takeAllFee(from, amount,sellLpRewardFee);
        }
    ...
    function _takeAllFee(address from,uint256 amount,uint256 lpRewardFee) private returns(uint256 amountAfter) {
        amountAfter = amount;
