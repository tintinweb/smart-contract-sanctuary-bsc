...
contract FeatHealth is Ownable {
    ...
    event Challenge(uint256 indexed challengeId, uint256 rewardPercentage);
    ...
    uint256 public completeChallengeCount = 2;
    ...
    struct DepositerInfo {
        ...
        uint8 todayCompletedChallengeCount;
        uint256 challengeLimitTime;
    }

    mapping(address => DepositerInfo) private depositerInfo;
    ...
    modifier onlyPossibleChallenge() {
        require(
            (depositerInfo[msg.sender].todayCompletedChallengeCount < completeChallengeCount
    ...
    function deposit(uint256 _amount) public onlyDepositer(false) {
        ...
        depositerInfo[msg.sender] = DepositerInfo({
            ...
            isLockedUp:false,
            suspensionTime:0,
            todayCompletedChallengeCount:0,
    ...
    function withdraw() public onlyExpiredSuspensionTime() {
        IBEP20 token = IBEP20(featTokenAddress);
        uint256 tokenAmount = depositerInfo[msg.sender].amount;
        token.transfer(msg.sender, tokenAmount);
        depositerInfo[msg.sender].isDeposited = false;
        depositerInfo[msg.sender].isLockedUp = false;
     ...
     */
    function setCompleteChallengeCount(uint256 _count) public onlyOwner() {
        completeChallengeCount = _count;
    }
     ...
     */
    function createChallenge(uint256 _rewardPercentage) public onlyOwner() {
        ...
        emit Challenge(newChallengeId, _rewardPercentage);
    }

    /**
     * Handle update challenge
     */
    function updateChallenge(uint256 _challengeId, uint256 _rewardPercentage, bool _isEnable) public onlyOwner() {
        challengeInfo[_challengeId].rewardPercentage = _rewardPercentage;
        challengeInfo[_challengeId].isEnable = _isEnable;
    }
    ...
    {
        depositerInfo[msg.sender].todayCompletedChallengeCount.add(1);
        if(depositerInfo[msg.sender].todayCompletedChallengeCount == 1){
            depositerInfo[msg.sender].challengeLimitTime = block.timestamp + 1 days;
        } else if(depositerInfo[msg.sender].todayCompletedChallengeCount > completeChallengeCount){
            depositerInfo[msg.sender].todayCompletedChallengeCount = 0;
            depositerInfo[msg.sender].challengeLimitTime = 0;
        }
        uint256 userTokenAmount = depositerInfo[msg.sender].amount;
        uint256 rewardTokenAmount = userTokenAmount.div(10000).mul(challengeInfo[_challengeId].rewardPercentage);
        depositerInfo[msg.sender].amount.add(rewardTokenAmount);
    ...
    function getChallengeList() public view returns (ChallengeInfo[] memory) {
        ...
        uint currentIndex = 0;
        ChallengeInfo[] memory challenges = new ChallengeInfo[](totalChallengeCount);
        for (uint i = 0; i < totalChallengeCount; i++) {
            uint challengeId = i + 1;
            if(challengeInfo[challengeId].isEnable){
                ChallengeInfo storage currentChallenge = challengeInfo[challengeId];
    ...
    /**
     * Handle get deposit info
     */
    function getDepositInfo(address _userAddress) public view onlyUser(_userAddress) returns (DepositerInfo memory) {
        return depositerInfo[_userAddress];
    }
}
