...
import {IBluechipsBooster} from "./interfaces/IBluechipsBooster.sol";
import {IDAOGovernorBooster} from "./interfaces/IDAOGovernorBooster.sol";
import {IMembershipPassBooth} from "./interfaces/IMembershipPassBooth.sol";
import {IRoyaltyDistributionHub} from "./interfaces/IRoyaltyDistributionHub.sol";
import {ITerminal, ImmutablePassTier, AuctionedPass, Metadata} from "./interfaces/ITerminal.sol";
import {IFundingCycles, FundingCycleParameter, FundingCycleProperties, FundingCycleState, FundingCycleMod} from "./interfaces/IFundingCycles.sol";

contract Terminal is ITerminal, Initializable {
  ...
  IRoyaltyDistributionHub public royaltyDistributionHub;

  IDAOGovernorBooster public daoGovernorBooster;

  IBluechipsBooster public bluechipsBooster;

  IConfigStore public configStore;

  mapping(FundingCycleMod => ILaunchMod) public launchMods;
  ...
  function initialize(
    ...
    IRoyaltyDistributionHub _royaltyDistributionHub,
    ...
    if (
      ...
      _royaltyDistributionHub == IRoyaltyDistributionHub(address(0)) ||
    ...
    royaltyDistributionHub = _royaltyDistributionHub;
   ...
   * @param _encodedParam The encoded param for launch
   */
  function createDao(
    ...
    uint256 _projectId = projects.create(msg.sender, _metadata.handle);
    _setupProject(_projectId, _metadata, _tiers);

    for (uint256 i; i < _auctionedPass.length; i++) {
      if (_auctionedPass[i].id != i) revert BadTierId();

      if (
        (_auctionedPass[i].allocateAmount + _auctionedPass[i].reservedAmount) !=
        _tiers[i].tierCapacity
      ) {
    ...
    // Launch immediately
    _launch(_projectId, _auctionedPass, _params, _encodedParam);
  }
   ...
   * @param _auctionedPass The auctioned pass information
   * @param _params The parameters for funding cycle
   * @param _encodedParam The encoded param for launch
   */
  function createNewFundingCycle(
    ...
    uint256 _latestFundingCycleId = fundingCycles.latestIdFundingProject(_projectId);
    if (fundingCycles.getFundingCycleState(_latestFundingCycleId) != FundingCycleState.Expired)
      revert FundingCycleActived();
    ...
    for (uint256 i = 0; i < _auctionedPass.length; i++) {
      ...
      if (
        (_auctionedPass[i].allocateAmount + _auctionedPass[i].reservedAmount) !=
        fundingCycles.getAutionedPass(_latestFundingCycleId, i).reservedAmount
      ) {
        revert InsufficientBalance();
      }
    }
   ...
   */
  function setPausedFundingCycleProject(uint256 _projectId, bool _paused) external override {
    if (msg.sender != projects.ownerOf(_projectId) && msg.sender != configStore.superAdmin())
      revert UnAuthorized();

    fundingCycles.setPauseFundingCycle(_projectId, _paused);
  }

  /**
   * @notice
   * Introduce new launch mod
   *
   * @param _launchMod The new launch mod type
   * @param _launchModInst The launch mod contract instance
   */
  function registerLaunchMod(FundingCycleMod _launchMod, ILaunchMod _launchModInst)
    external
    onlyAdmin
  {
    if (launchMods[_launchMod] != ILaunchMod(address(0))) revert NoZero();

    launchMods[_launchMod] = _launchModInst;

    emit LaunchModChanged(_launchMod, address(_launchModInst), true);
  }

  /**
   * @notice
   * Deprecate a launch mod
   *
   * @param _launchMod The launch mod type
   */
  function removeLaunchMod(FundingCycleMod _launchMod) external onlyAdmin {
    if (launchMods[_launchMod] == ILaunchMod(address(0))) revert ZeroAddress();

    emit LaunchModChanged(_launchMod, address(launchMods[_launchMod]), false);

    delete launchMods[_launchMod];
  }
  ...
  function _setupProject(
    ...
    for (uint256 i; i < _tiers.length; i++) {
      if (_tiers[i].multiplier == 0 || _tiers[i].tierCapacity == 0) {
        revert NoZero();
      }
    ...
    address _royalty = royaltyDistributionHub.registerDistributor(_projectId);
    address _membershipPass = membershipPassBooth.issue(
      _projectId,
      _royalty,
      _tierFee,
      _tierCapacity,
    ...
    daoGovernorBooster.createGovernor(_projectId, _multipliers, _membershipPass);
  ...
  /**
   * @notice
   * Launch a rasing mode and start funding cycle
   *
   * @param _projectId The project id of the dao
   * @param _auctionedPass Auctioned pass information
   * @param _params The parameters for Funding Cycle
  ...
  function _launch(
    ...
    if (launchMods[_params.launchMode] == ILaunchMod(address(0))) revert ZeroAddress();

    FundingCycleProperties memory _fundingCycleProperty = fundingCycles.configure(
      _projectId,
      _params,
      _auctionedPass
    );
    launchMods[_params.launchMode].launch(_projectId, _fundingCycleProperty.id, _encodedParam);
  }
}
