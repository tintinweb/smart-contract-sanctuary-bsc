...
contract SmartGameStock is SmartSecure {
  ...
  constructor(uint256 salt) {
    owner = _msgSender();
    SMART_CARS = address((new SmartStockToken){salt: bytes32(salt)}("SmartCarStock", "STC"));
    SMART_ROBOTS = address(
      (new SmartStockToken){salt: bytes32(salt)}("SmartRobotStock", "STR")
    );
  }
