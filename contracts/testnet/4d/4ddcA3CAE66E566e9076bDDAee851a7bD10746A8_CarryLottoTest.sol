...
contract CarryLottoTest is ICarryOver, Ownable, ReentrancyGuard {
    ...
    function openResult(
        ...
        if (isWin_) {
            ...
            __transferCarroOverRemainAsset(seri_);
        }
    ...
    function closeSeri(uint256 seri_) external override {
        _checkOwner();
        Seri memory seri = _series[seri_];
                    ...
                    keccak256(
                        abi.encodePacked(
                            ...
                            keccak256(
                                abi.encodePacked(
                                    timestamp_,
                                    abi.encode(seri_, winners_, buyTickets_, totalTicket_, assets_, turn_)
                                )
                            )
                        )
                    ),
    ...
    function takePrizeExpired(uint256 seri_) external override {
        _checkOwner();
        Seri memory seri = _series[seri_];
    ...
    function configAffiliate(address[] calldata shareAddresses_, uint256[] calldata sharePercents_) external {
        _checkOwner();
    ...
    function _buyTransfer(
        ...
        __transferFrom(asset_, sender, address(this), assetAmt_, msg.value);
        __transfer(asset_, cfg.postAddr, assetAmt_ - postAmt_);
        uint256 shareAmount;
        for (uint256 i; i < length; ) {
            shareAmount = (postAmt_ * _sharePercents[i]) / 1e6;
    ...
    function __takePrize(
        ...
        for (uint256 j; j < coLength; ) {
            takeAssetInitialAmt = (_initialPrizes[seri_][j] * buyTickets_) / winTickets_;
    ...
    function __transferCarroOverRemainAsset(uint256 seri_) private {
        INormalLotto _normalLotto = _config.normalLotto;
        uint256 length = _initialAssets[seri_].length;
        for (uint256 i; i < length; ) {
