...
contract NFTExchange is Upgradable {
    ...
    * set the address of the super admin
    ...
    function getListOfMintedNFTsByGroupID(string memory _groupId) public view returns (MintedNFT[] memory) {
        ...
        for(uint256 i = 0; i < mintedAmount[_groupId].length; i++) {
            tokenId = mintedAmount[_groupId][i];
            addr = PEM721(pem721Address).ownerOfTokenID(tokenId);
            if(addr == address(0)) {
                length -= 1;
            }
        }
...
contract PEM721 is Ownable, ERC721URIStorage {
    ...
    function ownerOfTokenID(uint256 _tokenId) public view returns (address) {
        address owner = _ownerOfTokenID[_tokenId];
        return owner;
    }
