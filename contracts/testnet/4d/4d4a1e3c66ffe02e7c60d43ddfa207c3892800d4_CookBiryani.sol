...
contract CookBiryani is Context, Ownable  {
    ...
    uint256 private constant FEE55 = 5500;
    ...
    function sell() public {
        ...
        uint256 fee55 = getFee55(pointsValue);
        ...
        if(block.timestamp < deadline[msg.sender]){
           pointsValue =  pointsValue-fee55-fee;
        } else {
            pointsValue = pointsValue-fee;
        }
        
        deadline[msg.sender] = block.timestamp + 7 days;
        payable(msg.sender).transfer(pointsValue);
    }
    ...
    function getFee55(uint256 amount) private pure returns(uint256) {
        return amount * FEE55 / 10000;
    }
