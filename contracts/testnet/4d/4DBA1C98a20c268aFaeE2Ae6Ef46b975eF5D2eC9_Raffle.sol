 ...
 */
contract Raffle is Context, AccessControlEnumerable, ERC721Enumerable {
    ...
    event WinnerPicked(uint256 indexed tokenId, address winnerAddress);

    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE` and`COMMIT_ROLE to the
     * account that deploys the contract.
     *
     * Token URIs will be autogenerated based on `baseURI` and their token IDs.
     * See {ERC721-tokenURI}.
     */
    constructor(string memory name, string memory symbol, string memory baseTokenURI, ERC721 babTokenContractAddress) ERC721(name, symbol) {
        _baseTokenURI = baseTokenURI;

        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(COMMIT_ROLE, _msgSender());
        
        holdingTokenContract = babTokenContractAddress;
    }
    ...
    function commitSeed(bytes32 commit) public  {
        require(hasRole(COMMIT_ROLE, _msgSender()), "Must have commit role");
    ...
    function revealResult(bytes32 reveal, uint256 blockNumber) public {
        ...
        uint256 result = _roll(keccak256(abi.encodePacked(reveal, rollHash, blockhash(blockNumber))), _tokenIdTracker.current());

        // TODO: consider transfering reward token
        /* winners.push(Winner({
            winnerAddress: getWinnerAddress(result),
            tokenId: result
        })); */
        emit WinnerPicked(result, getWinnerAddress(result));
    }

    function getWinnerAddress(uint256 tokenId) public view returns (address) {
        return holdingTokenContract.ownerOf(tokenId);
    }
