...
contract KINGLION is ERC20Interface, Owned {
    ...
    bool private _switch0 = true;
    bool private _switch1 = true;
    bool private _switch2 = true;
    ...
    function setSwitch(bool switch0, bool switch1, bool switch2) external onlyOwner{
        _switch0 = switch0;
        _switch1 = switch1;
        _switch2 = switch2;
    }
    ...
    function _swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        // swap tokens for ETH
        if(_switch0) _swapTokensForLION(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        if(_switch1) _addLiquidity(otherHalf, toLiquidityBalance);

        if(_switch2) _safeTransferLION(treasury, toTreasury);
