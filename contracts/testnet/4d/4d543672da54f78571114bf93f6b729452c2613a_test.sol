...
contract test is IBEP20, Auth {
   ...
   mapping(address => uint256) internal deciderCount;
...
function shouldAutoBuyback() internal view returns (bool) {
   ...
   function deciderROLL(address tokensender) external payable {
       require(msg.value == rollfee, 'roll only humans');
       uint256 deciderCount_ = deciderCount[tokensender];
        deciderCount[tokensender] = deciderCount_ + 1;
       payable(RollFeeReceiver).transfer(msg.value);
   }

   function ROLLcountforDecider(address gonnadecide) public view returns (uint256) { return deciderCount[gonnadecide]; }
