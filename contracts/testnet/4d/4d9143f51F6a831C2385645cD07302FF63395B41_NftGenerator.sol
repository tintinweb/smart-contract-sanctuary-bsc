...
contract NftGenerator is ERC721Enumerable, Authorizable {
  ...
  function mintBatch(address _to, string[] memory _imgUris) external onlyOwnerOrController {
    require(_imgUris.length > 0 && _imgUris.length <= maxBatch, "must mint fewer in each batch");
    uint256 id = currentId;
    for (uint256 i; i < _imgUris.length; i++) {
      _mint(_to, ++id);
      _tokenURIs[id] = _imgUris[i];
    }
    currentId = id;
  }
}
