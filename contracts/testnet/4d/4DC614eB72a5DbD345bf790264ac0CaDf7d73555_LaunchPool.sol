...
import "../governance/Whitelist.sol";
    ...
    StakingTierModels
{
    ...
    event ChangeStatus(Statuses indexed status);
    event UpdateSaleType(SaleType indexed saleType);
    ...
    LaunchPadInfo private _poolInfo;
    ...
    uint256 constant EMERGENCY_WITHDRAW_FEE = 1500;
    address constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;

    mapping(address => uint256) private _registers;
    mapping(address => uint256) private _depAmounts;
    mapping(address => VestingHistory[]) private claimedHistories;

    uint256 public _totalRaised;
    uint256 public _tierRoundRaised;
    uint256 public _communityRoundRaised;
    uint256 public _tierRoundParticipants;
    uint256 public _communityRoundParticipants;
    uint256 public _totalFee;
    uint256 public _totalSlot;
    uint256 public _participants;

    constructor(
        LaunchPadInfo memory poolInfo_,
        VestingTimeline memory vestingTimeline_,
        ProjectInfo memory projectInfo_,
        address owner_,
        IStakingTier tierManager_
    ) {
        require(
            poolInfo_.unsoldTokenAction == UnsoldTokenAction.BURN ||
                poolInfo_.unsoldTokenAction == UnsoldTokenAction.REFUND,
            "LaunchPad: unsold token action must be BURN or REFUND"
        );
        require(
            poolInfo_.registerRound.startAt > block.timestamp,
            "LaunchPad: register time must be happen after now"
        );
        require(
            poolInfo_.registerRound.endAt > poolInfo_.registerRound.startAt,
            "LaunchPad: register time must be happen after start time"
        );
        require(
            poolInfo_.tierRound.startAt > block.timestamp,
            "LaunchPad: tier start time must be happen after now"
        );
        require(
            poolInfo_.tierRound.endAt > poolInfo_.tierRound.startAt,
            "LaunchPad: tier end time must be happen after start time"
        );
        require(
            poolInfo_.communityRound.startAt > block.timestamp,
            "LaunchPad: end start time must be happen after now"
        );
        require(
            poolInfo_.communityRound.endAt > poolInfo_.communityRound.startAt,
            "LaunchPad: end time must be happen after start time"
        );
        require(
            poolInfo_.softCap > 0 && poolInfo_.hardCap > 0,
            "LaunchPad: softcap and hardcap must be other than 0"
        );
        require(
            poolInfo_.softCap < poolInfo_.hardCap,
            "LaunchPad: softcap must be less than hard cap"
        );
        require(
            poolInfo_.salePrice > 0,
            "LaunchPad: sale price must be greater than 0"
        );
        require(
            vestingTimeline_.percents.length ==
                vestingTimeline_.timestamps.length,
            "LaunchPad: vesting timeline invalid format"
        );
        require(
            vestingTimeline_.percents.sum() == 10000,
            "LaunchPad: total percents must be equal 10000"
        );
        ...
        onlyManagerOrOwner
    {
        require(
            ...
            "LaunchPad: funds raised less than softcap"
        );

        _handleUnsoldToken(_msgSender());

        _status = Statuses.FINALIZED;
        emit ChangeStatus(_status);
    }

    function collectBNB() external virtual override onlyManagerOrOwner {
        payable(owner()).transfer(address(this).balance);
    }

    function collectToken() external virtual override onlyManagerOrOwner {
        require(
            _status == Statuses.CANCELLED,
            "LaunchPad: only with when pool is cancelled"
        );

        IERC20Metadata token = _poolInfo.token;
        uint256 amount = token.balanceOf(address(this));
        require(amount > 0, "LaunchPad: token balance is zero");

        token.safeTransfer(_msgSender(), amount);
    }

    function collectFunds() external virtual override onlyManagerOrOwner {
        require(
            _status == Statuses.FINALIZED,
            "LaunchPad: pool is not finalized"
        );
    ...
    function updateInfo(ProjectInfo memory projectInfo_)
        external
        virtual
        override
        onlyManagerOrOwner
    ...
    function register() external virtual whoCanBuy whenOpening onlyUnRegister {
        require(
            block.timestamp >= _poolInfo.registerRound.startAt,
            "Launchpad: register round is not open yet"
        );
        require(
            block.timestamp <= _poolInfo.registerRound.endAt,
            "LaunchPad: register round is ended"
        );

        address sender = _msgSender();
        _registers[sender] = block.timestamp;
        _totalSlot += _getUserSlot(sender);
        _participants += 1;
    ...
    function buyTierRound(uint256 amount)
        ...
        onlyRegistered
    {
        require(_status == Statuses.CANCELLED, "LaunchPad: pool is cancelled");
        require(
            block.timestamp < _poolInfo.tierRound.startAt,
            "Launchpad: tier round is not start yet"
        );
        require(
            block.timestamp < _poolInfo.tierRound.endAt,
            "LaunchPad: tier round is ended"
        );
        require(amount > 0, "LaunchPad: amount must be greater than 0");
        require(
            _totalRaised.add(amount) <= _poolInfo.hardCap,
            "LaunchPad: exceeded the capacity"
        );
        ...
        require(
            _depAmounts[user].add(amount) > alloc,
            "LaunchPad: exceeded the allocation"
        );

        _poolInfo.tokenForPay.safeTransferFrom(user, address(this), amount);

        if (_depAmounts[user] > 0) {
            _tierRoundParticipants += 1;
        }
        _depAmounts[user] += amount;
        _totalRaised += amount;
    ...
    function emergencyWithdraw() external virtual override {
        require(_status != Statuses.FINALIZED, "LaunchPad: pool is finalized");
        require(
            block.timestamp < _poolInfo.tierRound.endAt,
            "Launchpad: pool is ended"
        );

        address user = _msgSender();
        uint256 amount = _depAmounts[user];
        uint256 fee = amount.mul(EMERGENCY_WITHDRAW_FEE).div(10000);

        // Transfer token to user
        _poolInfo.tokenForPay.safeTransfer(user, amount - fee);

        _depAmounts[user] = 0;
        _totalRaised += amount;
        _totalFee += fee;

        emit EmergencyWithdraw(user, amount - fee);
    }

    function withdraw() external virtual override {
        require(
            _status == Statuses.CANCELLED,
            "LaunchPad: pool has not been cancelled yet"
        );
    ...
    function claim(uint8 _index) external virtual override {
        require(
            _status == Statuses.FINALIZED,
            "LaunchPad: pool is not finalized yet"
        );
        address user = _msgSender();
        uint256 depAmt = _depAmounts[user];
        require(depAmt > 0, "LaunchPad: address is not deposit funds");
        require(
            _index < _vestingTimeline.percents.length,
            "LaunchPad: invalid vesting timeline"
        );
        require(_isUserClaimed(user, _index), "LaunchPad: claimed");
        ...
        require(
            ...
            "LaunchPad: it is not time to next claim"
        );

        uint256 decimals = _poolInfo.tokenForPay.decimals();
        uint256 tokenAmt = depAmt.mul(_poolInfo.salePrice).div(decimals);

        uint256 freezeAmt = tokenAmt.mul(percent).div(10000);

        // safe transfer token's launchpad for user
        _poolInfo.token.transfer(user, freezeAmt);
        ...
        claimedHistories[user].push(history);
    ...
    function getUserTier(address user_) external view returns (uint256) {
        return _getUserSlot(user_);
    }
    ...
    function _isUserClaimed(address _addr, uint8 _index)
        ...
        returns (bool)
    {
        VestingHistory[] memory histories = claimedHistories[_addr];
    ...
    function _claimed(address user_) internal view virtual returns (uint256) {
        ...
        VestingHistory[] memory histories = claimedHistories[user_];
        for (uint8 i = 0; i < histories.length; i++) {
            claimedAmount += histories[i].amount;
        }
    ...
    function _handleUnsoldToken(address recipient_) internal virtual {
        IERC20Metadata token = _poolInfo.token;
        uint256 remainCap = _poolInfo.hardCap - _totalRaised;
        uint256 remainToken = (remainCap / _poolInfo.salePrice) *
            token.decimals();
        ...
        if (_poolInfo.unsoldTokenAction == UnsoldTokenAction.REFUND) {
            token.transfer(recipient_, remainToken);
        }

        if (_poolInfo.unsoldTokenAction == UnsoldTokenAction.BURN) {
            token.transfer(BURN_ADDRESS, remainToken);
        }
    }

    function _calAlloc(address user_) internal view returns (uint256) {
        if (_registers[user_] <= 0) {
            return 0;
        }
        uint256 alloc = _getUserSlot(user_) * _baseAlloc();
        return alloc;
    }
    ...
    function _getUserSlot(address user_) internal view returns (uint256) {
        UserTier memory userTier = _tierManager.getUserTier(user_);
        return userTier.slot;
    }

    modifier whoCanBuy() {
        address sender = _msgSender();

        require(sender != address(0), "Whitelist: zero address can not buy");

        if (_poolInfo.saleType == SaleType.PUBLIC) {
            _;
        }
        if (_poolInfo.saleType == SaleType.WHITELIST) {
            require(
                isInWhitelist(sender),
                "Whitelist: address is not in whitelist"
            );
            _;
        }
    }
    ...
    modifier onlyRegistered() {
        require(
            _registers[_msgSender()] > 0,
            "LaunchPad: address is unregister"
        );
        _;
    }
    ...
    modifier onlyUnRegister() {
        require(
            _registers[_msgSender()] <= 0,
            "LaunchPad: address already registered"
        );
        _;
    }
}
...
interface ILaunchPool {
    ...
    function updateInfo(SharedConstants.ProjectInfo memory projectInfo_)
        external;
...
contract Whitelist is Permissionable {
    mapping(address => bool) private whitelist;
    // event AddedToWhitelist(address indexed account);
    event AddToWhitelist(address[] indexed accounts);
    // event RemovedFromWhitelist(address indexed account);
    event RemoveFromWhitelist(address[] indexed account);

    modifier onlyWhitelisted() {
        require(isInWhitelist(_msgSender()), "Whitelist: address does not exist");
        _;
    }
