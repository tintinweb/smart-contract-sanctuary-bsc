...
contract GNSPriceAggregatorV6_2Bypass is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    ...
    function getPrice(
        ...
        */
        ...
        for(uint i = 0; i < nodes.length; i ++){
            orderIdByRequest[bytes32(block.timestamp + i)] = orderId;
            priceRequests.push(bytes32(block.timestamp + i));
        }
    ...
    function callFulfill() public {
        fulfill(priceRequests[request] ,165000000000000);
        request += 1;
    }
    ...
    function fulfill(
        ...
        if(answers.length == minAnswers){
            ...
            console.log("DENTRO DE 3a RESPUESTA");
        }
        emit PriceReceived(
            requestId,
            orderId,
    ...
    function linkFee(uint pairIndex, uint leveragedPosDai) public view returns(uint){
        ...
        * leveragedPosDai * 1e8 / 67340000000 /* uint(linkPriceUsd)*/ / PRECISION / 100;
    }
