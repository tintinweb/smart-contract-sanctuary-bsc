...
contract TicketCollection is ERC721AUpgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializerERC721A initializer {
        __ERC721A_init("CryptonaireTicket", "CTicket");
        __Ownable_init();
    }
    ...
    function setLotteryAddress(address _Lottery) external onlyOwner {
        require(!init, "Lottery Address can only be set once!");
        init = true;
        Lottery = _Lottery;
    }
    ...
    function mint(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            _tokenURIs[currentTokenId] = _tokenURI[i];
            currentTokenId++;
        }
    }
    ...
    function _afterTokenTransfers(
        ...
        if (quantity == 1 && from != address(0)) {
            ICryptonaireLottery lottery = ICryptonaireLottery(Lottery);
            address oldTicketOwner = (lottery.viewTicket(startTokenId)).owner;
            if (oldTicketOwner != address(0)) {
                require(from == oldTicketOwner, "Invalid ticket owner");
                lottery.updateTicketOwner(to, startTokenId);
            }
        }
    }
}
