...
import "./LibChegbeStorage.sol";
import "./LibChegbe.sol";
...
contract PremiumPlanFacet is PremiumBase {
    ...
    function initialize(address _pairAddress, address _treasury, address _baseToken) external protectedCall {
        LibChegbe.initialize(_pairAddress, _treasury, _baseToken);
    }

    function getUpgradeFeeInToken() external view returns (uint256) {
        LibChegbeStorage.ChegbeStorage storage es = LibChegbeStorage.chegbeStorage();
    ...
    function register(
        ...
        LibChegbeStorage.ChegbeStorage storage es = LibChegbeStorage.chegbeStorage();
        require(es.live, "NA");
        require(_referralID > 0 && _referralID <= es.lastID, "INVALID_REFERRALID");
        require(_uplineID == 0 || accountIsInPremium(_uplineID), "INVALID_UPLINEID");

        LibChegbe.register(_address, _referralID, _uplineID);
    }

    function joinArmy(uint256 userID, uint256 random) external noReentry {
        LibChegbeStorage.ChegbeStorage storage es = LibChegbeStorage.chegbeStorage();
        require(es.live, "NA");
        require(IERC20(es.baseToken).balanceOf(msg.sender) >= amountFromDollar(es.upgradeFee), "INS_BAL");

        LibChegbeStorage.User storage user = es.users[userID];

        IERC20(es.baseToken).transferFrom(msg.sender, address(this), amountFromDollar(es.upgradeFee));

        uint256 referralCommission = es.upgradeFee.div(es.referralCommission).div(es.percentageDivisor);
        emit Payout(user.referralID, userID, referralCommission, 1, "Referral payment");
        sendPayout(es.userAddresses[user.referralID], referralCommission);
    ...
    function upgradeReplacementAccount(
        ...
        sendPayout(referralEarner, referralCommission);
    ...
    function setMaxtraversalDept(uint256 dept) external protectedCall {
        LibChegbeStorage.chegbeStorage().traversalDept = dept;
    }
    ...
    function isAccountIInPremium(uint256 userID) external view returns (bool) {
        return userID == 1 || LibChegbeStorage.chegbeStorage().users[userID].premiumLevel > 0;
    }
    ...
    function premiumCounter() external view returns (uint256) {
        return LibChegbeStorage.chegbeStorage().premiumCounter;
    }
    ...
    function getAccountsCount(address addr) external view returns (uint256) {
        return LibChegbeStorage.chegbeStorage().userAccounts[addr].length;
    }

    function getFirstAccountID(address addr) external view returns (uint256) {
        LibChegbeStorage.ChegbeStorage storage es = LibChegbeStorage.chegbeStorage();
        if(es.userAccounts[addr].length == 0) {
            return 0;
        }
        return es.userAccounts[addr][0];
    }

    function getLastAccountID(address addr) external view returns (uint256) {
        LibChegbeStorage.ChegbeStorage storage es = LibChegbeStorage.chegbeStorage();
        if(es.userAccounts[addr].length == 0) {
            return 0;
        }
        return es.userAccounts[addr][es.userAccounts[addr].length-1];
    }
    ...
    function lastID() external view returns (uint256) {
        return LibChegbeStorage.chegbeStorage().lastID;
    }

    function userAddresses(uint256 userID) external view returns (address) {
        return LibChegbeStorage.chegbeStorage().userAddresses[userID];
    }

    function upgradeFee() external view returns (uint256) {
        return LibChegbeStorage.chegbeStorage().upgradeFee;
    }

    function totalPayout() external view returns (uint256) {
        return LibChegbeStorage.chegbeStorage().totalPayout;
    }

    function lpPair() external view returns (address) {
        return address(LibChegbeStorage.chegbeStorage().pairAddress);
    }

    function live() external view returns (bool) {
        return LibChegbeStorage.chegbeStorage().live;
    }

    function setLpPair(address oracle) external protectedCall {
        LibChegbeStorage.ChegbeStorage storage es = LibChegbeStorage.chegbeStorage();
        es.pairAddress = IPancakePair(oracle);
    }
    ...
    function changeWallet(uint256 userID, address newWallet) public {
        require(LibChegbeStorage.chegbeStorage().userAddresses[userID] == msg.sender, "NA");
        LibChegbeStorage.chegbeStorage().userAddresses[userID] = newWallet;
...
import "./ChegbeBase.sol";
import "./LibChegbeStorage.sol";
import "../shared/Access/CallProtection.sol";
import "hardhat/console.sol";

contract PremiumBase is ChegbeBase, CallProtection, ReentryProtection {
    using SafeMath for uint256;
    ...
    event Payout(uint256 indexed userID, uint256 indexed fromID, uint256 amount, uint256 level, string description);
    ...
    function sendMatrixPayout(uint256 fromID, uint256 level) internal returns (uint256) {
        ...
        sendPayout(es.userAddresses[beneficiary], es.levelConfigurations[level].perDropEarning);
        emit Payout(beneficiary, fromID, es.levelConfigurations[level].perDropEarning, level, "Matrix payout");
    ...
    function _getMatrixPayoutCount(uint256 userID, uint256 level) internal view returns (uint256) {
        ...
        input[0] = userID;
        uint256[16] memory ids = blockDownlines(input, level, LibChegbeStorage.chegbeStorage().levelConfigurations[level].paymentGeneration);
        uint256 count;
        for (uint256 i = 0; i < 16; i++) {
            if (ids[i] == 0) {
                continue;
            }
            if (LibChegbeStorage.chegbeStorage().users[ids[i]].premiumLevel >= level) {
                count += 1;
            }
        }
        return count;
    ...
    function moveToNextLevel(uint256 userID, uint256 random) internal {
        ...
        if (pendingPayoutCount > 0) {
            ...
            sendPayout(es.userAddresses[userID], pendingAmount);
        }
...
library LibChegbeStorage {
    bytes32 constant CHEGBE_STORAGE_POSITION = keccak256("Chegbe.storage.location");

    struct ChegbeStorage {
        bool live;
        uint256 activationFee;
        uint256 upgradeFee;
        uint256 referralCommission;
        uint256 percentageDivisor;
        IPancakePair pairAddress;
        address treasury;
        address baseToken;
        uint256 totalPayout;
        uint256 lastID;
    ...
    function chegbeStorage() internal pure returns (ChegbeStorage storage es) {
        bytes32 position = CHEGBE_STORAGE_POSITION;
        assembly {
            es.slot := position
        }
    }
...
library LibChegbe {
    ...
    function initialize(address _pairAddress, address _treasury, address _baseToken) internal {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        LibChegbeStorage.ChegbeStorage storage es = LibChegbeStorage.chegbeStorage();
        require(msg.sender == ds.contractOwner, "Must own the contract.");

        es.upgradeFee = 100 * 1e18;
        es.referralCommission = 100;
        es.percentageDivisor = 1000;
        es.traversalDept = 10;

        es.pairAddress = IPancakePair(_pairAddress);
        es.treasury = _treasury;
        es.baseToken = _baseToken;
    ...
    function register(
        ...
        if (referralID > 0) {
            es.users[referralID].referrals.push(es.lastID);
        }
    }
    ...
    function buildPremiumConfig() private {
        LibChegbeStorage.ChegbeStorage storage es = LibChegbeStorage.chegbeStorage();
        es.levelConfigurations[1] = LibChegbeStorage.LevelConfig(5 * 1e18, 1, 2);
        es.levelConfigurations[2] = LibChegbeStorage.LevelConfig(75 * 1e17, 2, 4);
        es.levelConfigurations[3] = LibChegbeStorage.LevelConfig(225 * 1e17, 3, 8);
        es.levelConfigurations[4] = LibChegbeStorage.LevelConfig(135 * 1e18, 2, 4);
        es.levelConfigurations[5] = LibChegbeStorage.LevelConfig(1620 * 1e18, 2, 4);
...
contract ChegbeBase {
    ...
    function amountFromDollar(uint256 dollarAmount) internal view returns (uint256 tokenAmount) {
        // IERC20Metadata token0 = IERC20Metadata(LibChegbeStorage.chegbeStorage().pairAddress.token0());
        // IERC20Metadata token1 = IERC20Metadata(LibChegbeStorage.chegbeStorage().pairAddress.token1());

        (uint256 r0, uint256 r1,) = LibChegbeStorage.chegbeStorage().pairAddress.getReserves();
        r1 = r1.div(1e10);
