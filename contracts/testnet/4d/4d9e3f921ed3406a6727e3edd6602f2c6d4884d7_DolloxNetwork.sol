...
function transfer(address to, uint256 tokens) public override returns (bool) {
    ...
    uint256 gasRequired = 21000;
    require(gasleft() >= gasRequired, "Not enough gas to execute transaction");
    uint256 gasBefore = gasleft();
    _balances[msg.sender] = _balances[msg.sender].sub(tokens);
    _balances[to] = _balances[to].add(tokens);
    emit Transfer(msg.sender, to, tokens);
    uint256 gasSpent = gasBefore - gasleft();
    require(gasSpent <= gasRequired, "Gas limit exceeded");
    return true;
}
...
function approveAndCall(address spender, uint256 tokens, bytes memory data) public onlyAuthorized returns (bool success) {
    require(_balances[msg.sender] >= tokens, "Insufficient balance");
    uint256 gasRequired = 40000;
    require(gasleft() >= gasRequired, "Not enough gas to execute transaction");
    uint256 gasBefore = gasleft();
    _allowed[msg.sender][spender] = _allowed[msg.sender][spender].add(tokens);
    emit Approval(msg.sender, spender, tokens);
