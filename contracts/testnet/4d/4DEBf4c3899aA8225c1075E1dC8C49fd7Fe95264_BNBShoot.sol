...
contract BNBShoot is Ownable{
    ...
    uint256 public thisRoundRate = 70;
...
// ***user basic function***
    ...
    function buyShoots(address ref) public payable{
        ...
        roundToReward[roundNum] = roundToReward[roundNum] + msg.value * (fomoRewardRate/100) * (thisRoundRate/100);
        roundToReward[roundNum+1] = roundToReward[roundNum] + msg.value * (fomoRewardRate/100) * ((100 - thisRoundRate)/100);
...
// ***user fomo function***
    ...
    function updateRoundNum() public {
        ...
        if(block.timestamp > nextRoundtime){
            ...
            //本轮总奖励 * 投入地址TOP奖励比例 = 本轮TOP总奖励(总奖励的60%给投入TOP)
