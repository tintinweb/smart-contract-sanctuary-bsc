...
interface IGetdoneDisputation {
    function create(uint256 merchantId, uint256 appId, uint256 contractId, address customer, uint256 price, address erc20) external payable;
}
...
contract GetdoneEscrow is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, AccessControlEnumerableUpgradeable, GetdoneApp{
    ...
    event DisputationUpdated(address disputation, address sender);
    ...
    function updateDisputation(address payable addr) 
        public
        onlyAdmin
    {
        disputationAddress = addr;
        emit DisputationUpdated(addr, _msgSender());
    }
    ...
    function createDisputation(uint256 merchantId, uint256 appId, uint256 contractId, address customer) 
        ...
        nonReentrant
    {
        require(disputationAddress != address(0), "GetdoneEscrow: Disputation Address is invalid");
        require(validApp(merchantId, appId), "GetdoneEscrow: App is invalid");
        address sender = _msgSender();
        ContractInfo storage contractInfo = contractInfos[merchantId][appId][customer][contractId];
        ...
        IGetdoneDisputation(disputationAddress).create(merchantId, appId, contractId, customer, amountRefund, contractInfo.erc20);

        if (amountRefund > 0 && amountRefund <= contractBalance) {
            contractInfo.refund = contractInfo.refund + amountRefund;
            if (contractInfo.erc20 == address(0)) {
                payable(disputationAddress).transfer(amountRefund);
            } else {
                // IERC20Upgradeable(contractInfo.erc20).approve(_sender, amountRefund);
                IERC20Upgradeable(contractInfo.erc20).safeTransfer(disputationAddress, amountRefund);
            }
        }
        ...
        emit ContractDisputationSent(merchantId, appId, contractId, amountRefund, _msgSender(), sender, contractInfo.talent == sender, contractInfo.customer == sender);
    }
