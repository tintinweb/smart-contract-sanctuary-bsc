...
contract DogeFuu is ERC20Detailed, Ownable {
    ...
    uint256 public limitSellRateDiscount = 20;
    uint256 public secondLimitSell = 900;
    ...
    function _transferFrom(
        ...
        address traderAddr = recipient == pair ? sender : recipient;
        if (recipient == pair || isTransfer) {
            require(amount <= maxSellTransactionAmount, "Error amount");
            uint256 blkTime = block.timestamp;
            uint256 txLimitRate = mgmController.getMgmParent(traderAddr) ==
                address(0)
                ? limitSellRate
            ...
            if (
                blkTime >
                tradeHistory[traderAddr].lastTradeTime + secondLimitSell
            ) {
                uint256 limitAmount = balanceOf(traderAddr).mul(txLimitRate).div(
                    feeDenominator
                );
                require(
                    amount <= limitAmount,
                ...
                tradeHistory[traderAddr].lastTradeTime = blkTime;
                tradeHistory[traderAddr].amount = amount;
                tradeHistory[traderAddr].limitAmount = limitAmount;
            } else {
                require(
                    tradeHistory[traderAddr].amount.add(amount) <=
                        tradeHistory[traderAddr].limitAmount,
                    "ERR: Can't sell more than limit rate in One day"
                );
                tradeHistory[traderAddr].amount = tradeHistory[traderAddr]
                    .amount
                    .add(amount);
            }
        }
