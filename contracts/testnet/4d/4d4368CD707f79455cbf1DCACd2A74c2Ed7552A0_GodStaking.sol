...
contract GodStaking is Ownable, ReentrancyGuard {
    ...
    constructor() {
        token = IERC20(0xc328Bc63e99D61EF3701eF95AAFa380A033Fb3d6);
        poolInfo.push(PoolInfo(130,   8, block.timestamp, 15 minutes,  60 seconds, 0, 0, 2000, 1000, true));
        poolInfo.push(PoolInfo(323,  10, block.timestamp, 15 minutes, 120 seconds, 0, 0, 2000, 1000, true));
        poolInfo.push(PoolInfo(584,  12, block.timestamp, 15 minutes, 180 seconds, 0, 0, 2000, 1000, true));
        poolInfo.push(PoolInfo(1500, 15, block.timestamp, 15 minutes, 360 seconds, 0, 0, 2000, 1000, true));
    }
    ...
    function setFees(uint256 _poolId, uint emFee) external onlyOwner {
        ...
        require(emFee <= 2000, "EmergencyWithdrawFee should be <= 20");
        pool.emergencyWithdrawFee = emFee;
    }

    function poolStatus(uint256 _pid,bool _isOpen) external onlyAuthorized{
    ...
    function getRewardDepth() external view returns(uint256) {
        PoolInfo[] storage pool = poolInfo;
