...
contract OrbedNFT is Initializable, ERC721Upgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  */
    ...
    onlyRole(TRANSACTION_ROLE)  
  {
    ...
    //Check if the user can mint tokens.
    _canUserMint(to);

    //Private Members
    uint256 tokensMinted = 0; 
    uint256[] memory tokensIdsMinted;
    bool result = false;
    string memory message = "";
    console.log("Tokens Requested is %s", tokenIds.length);

    //Iterate through the tokens provided as a parameter.
    for (uint256 i = 0; i < tokenIds.length; i++)
    {
      // Make sure the user can mint this token.
      (result, message) = _canUserMintToken(to);
      console.log("canUserMint Result: TokenId %s Result: %s, '%s'", tokenIds[i], result, message);
      ...
      if (result)
      {
        ...
        console.log("mintInternal Return Results: TokenId: %s Result: mintResult value: %s, mintMessage: '%s'", tokenIds[i], mintResult, mintMessage);
        if (mintResult) 
        {
          tokensMinted++;
          tokensIdsMinted[i] = tokenIds[i];
          console.log("TokenId %s minted.", tokenIds[i]);
        } 
        else 
        {
    ...
    if (tokensMinted < tokenIds.length)
    {
      ...
      message = "Refunding the difference";
    }

    // If we weren't able to mint any NFTs, tell them why.
    require(tokensMinted > 0, message);
    emit MintCompleted(to, tokensIdsMinted, message);
  }
  ...
  * @dev Uses several different conditions to determine if minting is possible.
  * @param to - The wallet address attempting to mint. 
  ...
  */
  ...
  {
      //Check if the user has a balance to mint one token
  ...
  /**
  * @notice Conducts a mint for a given token (tokenId) for a given 
  * @param to - Account where the NFT will be minted to.
  * @param tokenId - Token Id to Mint.
  */
  function _mintInternal(address to, uint256 tokenId) private returns(bool successResult, string memory message) {

        (bool success, bytes memory returnData) = address(this).call(
                abi.encodeWithSignature("ERC721Upgradeable._safeMint(address,uint256)",to,tokenId)
        );

        console.log("MintInternal Success Result: '%s'", success);
        if (success)
        {
        ...
        else 
        {
          ...
          {
            string memory returnMessage = abi.decode(returnData, (string));
            console.log("returnData Result: '%s'", returnMessage);
          }
          //TODO: try to decode the returnData, if possible.
          return (false, "The token could not be minted.");
        }
  }
