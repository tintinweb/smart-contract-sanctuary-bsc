...
contract ERC1155Tradable is ERC1155, ERC1155MintBurn, ERC1155Metadata, Ownable, MinterRole, WhitelistAdminRole {
	...
	uint256 internal _currentTypeID = 0;
  mapping(uint256 => uint256) internal _ids;
  ...
  function addWhitelistAdmin(address account) public onlyOwner {
    ...
    if (bytes(_uri).length > 0) {
      return string(abi.encodePacked(baseMetadataURI, _uri));
    }
    return string(abi.encodePacked(baseMetadataURI, _uint2str(_id), ".json"));
	}
    ...
    if (_currentTypeID > 0) {
      for (uint256 i = 1; i <= _currentTypeID; i++) {
        emit URI(uri(_ids[i]), _ids[i]);
      }
    }
	}
	 ...
	 * @param _initialOwner Optional address of the first owner of the token
	 * @param _initialSupply Optional amount to supply the first owner
	 * @param _id Optional ID of this token type
    ...
    require(!_exists(_id), "ERC1155Tradable#create: EXISTENT_TOKEN");
    require(_initialOwner != address(0), "ERC1155Tradable#create: INVALID_RECIPIENT");

		uint256 _typeID = _getNextTypeID();
		_incrementTypeID();
    _ids[_typeID] = _id;
    ...
    if (bytes(_uri).length > 0) {
      emit URI(string(abi.encodePacked(baseMetadataURI, _uri)), _id);
    }
    _uris[_id] = _uri;
  ...
  function burn(address _from, uint256 _id, uint256 _quantity) public {
    ...
    require((msg.sender == _from) || isApprovedForAll(_from, msg.sender), "ERC1155Tradable#burn: caller is not owner nor approved");
	...
	/**
	 * @dev calculates the next token ID based on value of _currentTypeID
	 * @return uint256 for the next token ID
	 */
	function _getNextTypeID() private view returns (uint256) {
		return _currentTypeID.add(1);
	}

	/**
	 * @dev increments the value of _currentTypeID
	 */
	function _incrementTypeID() private {
		_currentTypeID++;
	}
}

contract SYMR is ERC1155Tradable {
	constructor() public ERC1155Tradable("System Master", "SYMR") {
		_setBaseMetadataURI("https://nft-upload-jp.s3.ap-northeast-1.amazonaws.com/");
	}
