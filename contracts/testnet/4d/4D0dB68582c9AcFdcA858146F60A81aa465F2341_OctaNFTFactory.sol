...
contract OctaNFTFactory is Ownable, AccessControl {

    string constant public dapp = "OctaNFTCollections";
    ...
    /// @notice Octa token contract address;
    address public octaToken;
    ...
    */
    ...
    function updateOctaToken(address _octaToken) external onlyOwner {
        octaToken = _octaToken;
    }
    ...
    /// @notice Method for enabling/disabling whitelisting
    function togglWhitelisting() external onlyOwner {
        isWhitelistingEnabled = !isWhitelistingEnabled;
    }

    /// @notice Method for whitelisting an address
    /// @param  _addresses Address to whitelist or blacklist
    /// @param  _enabled Boolean to whitelist or blacklist
    function updateWhitelisting(address[] calldata _addresses, bool _enabled)
        external
        onlyOwner
    {
    ...
    /// @notice Method for adding minters
    /// @dev Only admin can call
    /// @param  _minters Address to add as minter
    ...
    /// @notice Method for removing minters
    /// @dev Only admin can call
    /// @param  _minters Address to remove as minter
    ...
    /// @notice Method for getting latest OctaNFT contract
    /// @return Address of latest NFT contract
...
contract OctaNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard, EIP712 {
    ...
    ) ERC721(_name, _symbol) EIP712(dapp, version) ReentrancyGuard() {
