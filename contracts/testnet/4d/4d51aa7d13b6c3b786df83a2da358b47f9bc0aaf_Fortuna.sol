...
contract Fortuna {
    ...
    uint256 private constant dayPerCycle = 40 minutes; // 15 days
    ...
    uint256 private constant capLimit = 2;
    ...
    //event Deposit1(uint capAmount, uint256 totalRevenue);
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 1000e18){
            ...
            }else if(total >= 2000e18 && user.teamNum >= 5 && maxTeam >= 1500e18 && otherTeam >= 1500e18){ // team 60+25k+25k
                levelNow = 4; //A4 comm from  L5-L6
            }else if(user.teamNum >= 3 && maxTeam >= 1000e18 && otherTeam >= 1000e18){ // team 30+10k_10k
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        //emit Deposit1(capAmount, user.totalRevenue);
        
        user.totalFreezed = user.totalFreezed.add(_amount);

        uint256 capAmount = user.capital.mul(capLimit);
        // remove capp
        if(user.totalRevenue < capAmount){
            user.isCapped = false;
        }
        ...
        //check for 3x capp
        if(user.totalRevenue != 0 && user.totalRevenue >= capAmount){
            ...
            user.capital = user.capital.add(_amount);
            user.isCapped = true;
            user.totalFreezed = 0;
        } else {
            orderInfos[_user].push(OrderInfo(
            ...
            if(upline != address(0)){
                ...
                if(i > 6){
                    if(userInfo[upline].level > 4){
                        ...
                        if(userInfo[upline].isCapped == false){
