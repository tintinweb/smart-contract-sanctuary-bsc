...
contract StakeUpFarm is Ownable, ReentrancyGuard {
    ...
    IERC20 public BUSD = IERC20(0x5FbDB2315678afecb367f032d93F642f64180aa3); // $BUSD
    IERC20 public SUF = IERC20(0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512); // $SUF
    IERC721 public NFTx125 =
        IERC721(0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0); // $NFT @todo: change to NFT
    IERC721 public NFTx150 =
        IERC721(0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0); // $NFT @todo: change to NFT
    IERC721 public NFTx175 =
        IERC721(0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0); // $NFT @todo: change to NFT
    IERC721 public NFTx2 = IERC721(0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0); // $NFT @todo: change to NFT

    // Configurables
    uint256 public BUSDdaily = 6; // Daily reward of 6%
    uint256 public SUFdaily = 12; // Daily reward of 12%
    uint256 public minDeposit = 50 ether; // 50 BUSD
    ...
    address public feeWallet = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // @todo: change to real wallet
    ...
    function buyNFT(uint256 _option) public nonReentrant {
        ...
        if (_option == 1) {
            ...
            NFTx125.safeTransferFrom(address(this), msg.sender, 1); //@todo: change to real ID
            investor[msg.sender].NFTx = 125;
        } else if (_option == 2) {
            BUSD.safeTransferFrom(msg.sender, address(this), 175 ether);
            NFTx150.safeTransferFrom(address(this), msg.sender, ID); //@todo: change to real ID
            investor[msg.sender].NFTx = 150;
        } else if (_option == 3) {
            BUSD.safeTransferFrom(msg.sender, address(this), 250 ether);
            NFTx175.safeTransferFrom(address(this), msg.sender, ID); //@todo: change to real ID
            investor[msg.sender].NFTx = 175;
        } else if (_option == 4) {
            BUSD.safeTransferFrom(msg.sender, address(this), 350 ether);
            NFTx2.safeTransferFrom(address(this), msg.sender, ID); //@todo: change to real ID
