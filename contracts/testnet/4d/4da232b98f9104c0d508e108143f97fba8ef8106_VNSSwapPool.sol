...
contract VNSSwapPool is Ownable, ReentrancyGuard, IERC721Receiver {
    address public vnsToken;
    address public vnsNFT;
    bool public isPoolOpen = true;
    uint256 public PRICE = 50000 * 1e18;
    ...
    constructor(address token, address nft) {
        setPoolData(token, nft);
    }

    function setPoolData(address token, address nft) public onlyOwner {
        vnsToken = token;
        vnsNFT = nft;
    }
    ...
    function redeem(uint256 _ownerTokenId) external nonReentrant poolOpen {
        ...
        if (_vnsNFT.ownerOf(_ownerTokenId) != msg.sender) {
            revert NFTOwnerNotMatch(
                _ownerTokenId,
                msg.sender,
                _vnsNFT.ownerOf(_ownerTokenId)
            );
        }
    ...
    function buy() external nonReentrant poolOpen {
        ...
        uint256 id= IVNSNFT(vnsNFT).mintTo(msg.sender, 1);
    ...
    function withdrawNFT(uint256[] calldata targetTokens, address recipient)
        external
        onlyOwner
    {
        IERC721 _vnsNFT = IERC721(vnsNFT);
