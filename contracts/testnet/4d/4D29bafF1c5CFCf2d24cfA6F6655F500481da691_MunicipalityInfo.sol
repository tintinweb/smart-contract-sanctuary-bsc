...
contract MunicipalityInfo is OwnableUpgradeable, ReentrancyGuardUpgradeable {
   ...
   function setNetGymStreetAddress(address _netGymStreetAddress) external onlyOwner {
    ...
    function getBundles(uint8 _paymentType) external view returns (uint256[8][17] memory bundleStats) {
        for (uint8 i = 0; i < 17; i++) {
            if (i < 13) {
                (uint256[2] memory discountPrice, uint256[2] memory price) = IMunicipality(municipalityAddress).getPriceForBundle(i, _paymentType); ///TODO hard code purchase type BUSD 
                bundleStats[i] = [i, IMunicipality(municipalityAddress).newBundles(i).parcelsAmount, 
                IMunicipality(municipalityAddress).newBundles(i).minersAmount, 0, discountPrice[0], discountPrice[1], price[0], price[1]];
            } else {
                (uint256[2] memory discountPrice, uint256[2] memory price) = IMunicipality(municipalityAddress).getPriceForSuperBundle(i, _paymentType);
                ...
                IMunicipality(municipalityAddress).superBundlesInfos(i-13).upgradesAmount, discountPrice[0],discountPrice[1], price[0], price[1]];
            }    
        }
    }
    ...
    function getNFTPurchaseExpirationDate(address _user) external view returns(uint256) {
        if(IMunicipality(municipalityAddress).lastPurchaseData(_user).expirationDate > (INetGymStreet(netGymStreetAddress).lastPurchaseDateERC(_user) + 30 days)) {
            return IMunicipality(municipalityAddress).lastPurchaseData(_user).expirationDate;
        } else {
...
interface IMunicipality {
    ...
    function getPriceForBundle(uint8 _bundleType, uint8 _paymentType) external view returns(uint256[2] memory, uint256[2] memory);
    function getPriceForSuperBundle(uint8 _bundleType, uint8 _paymentType) external view returns(uint256[2] memory, uint256[2] memory);
}
...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint8 private constant PAYMENT_TYPE_BUSD = 10;
    uint8 private constant PAYMENT_TYPE_BUSD_GYMNET = 20;
    ...
    bool public GymnetSaleActivated;
    ...
    function getPriceForPurchaseParcels (address _user, uint256 _parcelsCount, uint8 _paymentType) external view returns(uint256[2] memory, uint256[2] memory, uint256) {
        return _getPriceForPurchaseParcels (_user, _parcelsCount, _paymentType);
    }

    /// @notice Get price for mintMiners and purchaseMiners for Gymnet use purposes
    /// returns priceForMinting, priceForSinglePurchase, rawParcelsPrice
    function getPriceForPurchaseMiners(address _user, uint256 _minersCount, uint8 _paymentType) external view returns(uint256[2] memory, uint256[2] memory, uint256) {
        return _getPriceForPurchaseMiners(_user, _minersCount, _paymentType);
    }
    ...
    function setGymnetSaleActive(bool _gymnetSaleActivation) external onlyOwner {
        GymnetSaleActivated = _gymnetSaleActivation;
    }
    
    function setAddressForGymPrice(address[3] memory _tokenAdrresses) external onlyOwner {
        wbnbAddress = _tokenAdrresses[0];
        gymnetAddress = _tokenAdrresses[1];
        routerAddress = _tokenAdrresses[2];
    }

    function mintParcels(ParcelsMintSignature calldata _mintingSignature, uint256 _referrerId, uint8 _paymentType)
        external
        onlySaleActive
        notZeroAddress
    {
        ...
        _paymentCheck(_paymentType);
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (uint256[2] memory busdGymnetForMinting,,) = _getPriceForPurchaseParcels(msg.sender, parcelsLength, _paymentType);
        if(busdGymnetForMinting[0] > 0) {
            if( busdGymnetForMinting[1] > 0) {
               _transferToContractGym(busdGymnetForMinting[1]);
            }
            _transferToContract(busdGymnetForMinting[0]);
            IAmountsDistributor(amountsDistributorAddress).distributeAmounts(busdGymnetForMinting[0], OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
            userToPurchasedAmountMapping[msg.sender] += _sumBusdGymInBusd(busdGymnetForMinting);
            _updateAdditionLevel(msg.sender);
            lastPurchaseData[msg.sender].dollarValue += _sumBusdGymInBusd(busdGymnetForMinting);
        ...
        if (busdGymnetForMinting[0] > 0) {
            emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCEL, parcelIds, busdGymnetForMinting[0]);
        }
    }

    function mintMiners(uint256 _count, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress returns(uint256, uint256)
    {
        require(_count > 0, "Municipality: Can not mint 0 miners");
        _paymentCheck(_paymentType);
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (uint256[2] memory busdGymnetForMinting,,) = _getPriceForPurchaseMiners(msg.sender, _count, _paymentType);
        if(busdGymnetForMinting[0] > 0) {
            if( busdGymnetForMinting[1] > 0) {
               _transferToContractGym(busdGymnetForMinting[1]);
            }
            ...
            lastPurchaseData[msg.sender].dollarValue += _sumBusdGymInBusd(busdGymnetForMinting);
            _lastPurchaseDateUpdate(msg.sender);
            IAmountsDistributor(amountsDistributorAddress).distributeAmounts(busdGymnetForMinting[0], OPERATION_TYPE_MINT_MINERS, msg.sender);
        ...
        if (busdGymnetForMinting[0] > 0) {
            emit PurchaseMade(msg.sender, PURCHASE_TYPE_MINER, minerIds, busdGymnetForMinting[0]);
        }
        return (firstMinerId, count);
    }

    ///@notice purchase Parcels balance (without minting) from Gymnet Side
    function purchaseParcels(uint256 _parcelsAmount, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress {
        _paymentCheck(_paymentType);
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (,uint256[2] memory busdGymnetForSingleProd, ) = _getPriceForPurchaseParcels(msg.sender, _parcelsAmount, _paymentType);
        if( busdGymnetForSingleProd[1] > 0) {
            _transferToContractGym(busdGymnetForSingleProd[1]);
        }
        _transferToContract(busdGymnetForSingleProd[0]);
        userToPurchasedAmountMapping[msg.sender] += _sumBusdGymInBusd(busdGymnetForSingleProd);
        _updateAdditionLevel(msg.sender);
        lastPurchaseData[msg.sender].dollarValue += _sumBusdGymInBusd(busdGymnetForSingleProd);
        _lastPurchaseDateUpdate(msg.sender);
        IAmountsDistributor(amountsDistributorAddress).distributeAmounts(busdGymnetForSingleProd[0], OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
        usersMintableNFTAmounts[msg.sender].parcels += _parcelsAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCELS, nftIds, busdGymnetForSingleProd[0]);
    }

    ///@notice purchase Miners balance (without minting) from Gymnet Side
    function purchaseMiners(uint256 _minersCount, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress {
        _paymentCheck(_paymentType);
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (,uint256[2] memory busdGymnetForSingleProd,) = _getPriceForPurchaseMiners(msg.sender, _minersCount, _paymentType);
        if( busdGymnetForSingleProd[1] > 0) {
            _transferToContractGym(busdGymnetForSingleProd[1]);
        }
        _transferToContract(busdGymnetForSingleProd[0]);
        ...
        lastPurchaseData[msg.sender].dollarValue += _sumBusdGymInBusd(busdGymnetForSingleProd);
        _lastPurchaseDateUpdate(msg.sender);
        IAmountsDistributor(amountsDistributorAddress).distributeAmounts(busdGymnetForSingleProd[0], OPERATION_TYPE_MINT_MINERS, msg.sender);
        usersMintableNFTAmounts[msg.sender].miners += _minersCount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_MINERS, nftIds, _sumBusdGymInBusd(busdGymnetForSingleProd));
    }

    function purchaseBasicBundle(uint8 _bundleType, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress
    {
        ...
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (uint256[2] memory bundlePurchasePrice, ) = _getPriceForBundle(_bundleType, _paymentType);
        if(bundlePurchasePrice[1] > 0) {
            _transferToContractGym(bundlePurchasePrice[1]);
        }        
        _transferToContract(bundlePurchasePrice[0]);
        userToPurchasedAmountMapping[msg.sender] += _sumBusdGymInBusd(bundlePurchasePrice);
        _updateAdditionLevel(msg.sender);
        lastPurchaseData[msg.sender].dollarValue += _sumBusdGymInBusd(bundlePurchasePrice);
        _lastPurchaseDateUpdate(msg.sender);
        IAmountsDistributor(amountsDistributorAddress).distributeAmounts(bundlePurchasePrice[0], OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
        ...
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_BUNDLE, nftIds, bundlePurchasePrice[0]);
        emit BundlePurchased(msg.sender, _bundleType);
    }

    function purchaseSuperBundle(uint8 _bundleType, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress
    {
        _validateSuperBundleType(_bundleType);
        _paymentCheck(_paymentType);
        SuperBundleInfo memory bundle = superBundlesInfos[_bundleType - BUNDLE_TYPE_SUPER_1];
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        (uint256[2] memory bundlePurchasePrice,) = _getPriceForSuperBundle(_bundleType, _paymentType);
        if(bundlePurchasePrice[1] > 0) {
            _transferToContractGym(bundlePurchasePrice[1]); 
        }
        _transferToContract(bundlePurchasePrice[0]);
        ...
        usersMintableNFTAmounts[msg.sender].miners += bundle.minersAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_SUPER_BUNDLE, nftIds, bundlePurchasePrice[0]);
        emit SuperBundlePurchased(msg.sender, _bundleType);
    }

    function purchaseParcelsBundle(uint8 _bundleType, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress
    {
        require(basicBundleActivated, "Municipality: Basic bundle is deactivated");
        _paymentCheck(_paymentType);
        ...
        usersMintableNFTAmounts[msg.sender].parcels += bundle.parcelsAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCELS_BUNDLE, nftIds, bundlePurchasePrice[0]);
        emit BundlePurchased(msg.sender, _bundleType);
    }

    function purchaseMinersBundle(uint8 _bundleType, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress {
        require(basicBundleActivated, "Municipality: Basic bundle is deactivated");
        _paymentCheck(_paymentType);
        ...
        lastPurchaseData[msg.sender].dollarValue += _sumBusdGymInBusd(bundlePurchasePrice);
        _lastPurchaseDateUpdate(msg.sender);
        IAmountsDistributor(amountsDistributorAddress).distributeAmounts(bundlePurchasePrice[0], OPERATION_TYPE_MINT_MINERS, msg.sender);
        usersMintableNFTAmounts[msg.sender].miners += bundle.minersAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_MINERS_BUNDLE, nftIds, bundlePurchasePrice[0]);
        emit BundlePurchased(msg.sender, _bundleType);
    }
    ...
    function getParcelsUpgradePrice(uint256 numParcels, address _user, uint8 _paymentType) external view returns(uint256[2] memory) {
        ...
        return _priceInBusdGym(_totalUpgradePrice, _paymentType);
    }
    
    function upgradeStandardParcelsGroup(uint256[] memory _parcelIds, uint8 _paymentType) external onlySaleActive {
        ...
        uint256[2] memory totalUpgradePriceBusdGym = _priceInBusdGym(_totalUpgradePrice, _paymentType);
        if(totalUpgradePriceBusdGym[0] > 0) {
            if(totalUpgradePriceBusdGym[1] > 0) {
                _transferToContractGym(totalUpgradePriceBusdGym[1]);
            }
            _transferToContract(totalUpgradePriceBusdGym[0]);
            IAmountsDistributor(amountsDistributorAddress).distributeAmounts(totalUpgradePriceBusdGym[0], OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
            userToPurchasedAmountMapping[msg.sender] += _sumBusdGymInBusd(totalUpgradePriceBusdGym);
            _updateAdditionLevel(msg.sender);
            lastPurchaseData[msg.sender].dollarValue += _sumBusdGymInBusd(totalUpgradePriceBusdGym);
            _lastPurchaseDateUpdate(msg.sender);
        }
        IParcelInterface(standardParcelNFTAddress).upgradeParcels(_parcelIds);
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_UPGRADE_STANDARD_PARCELS_GROUP, nftIds, totalUpgradePriceBusdGym[0]);
    }

    function upgradeStandardParcel(uint256 _parcelId, uint8 _paymentType) external onlySaleActive {
        require(
        ...
        uint256[2] memory upgradePriceBusdGym = _priceInBusdGym(_upgradePrice, _paymentType);
        if(usersMintableNFTAmounts[msg.sender].upgrades > 0) {
            usersMintableNFTAmounts[msg.sender].upgrades--;
        } else {
            _upgradePrice = upgradePrice;
            if(upgradePriceBusdGym[1] > 0) {
                _transferToContractGym(upgradePriceBusdGym[1]);
            } 
            _transferToContract(upgradePriceBusdGym[0]);
            IAmountsDistributor(amountsDistributorAddress).distributeAmounts(upgradePriceBusdGym[0], OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
            userToPurchasedAmountMapping[msg.sender] += _sumBusdGymInBusd(upgradePriceBusdGym);
            _updateAdditionLevel(msg.sender);
            lastPurchaseData[msg.sender].dollarValue += _sumBusdGymInBusd(upgradePriceBusdGym);
            _lastPurchaseDateUpdate(msg.sender);
        }
        IParcelInterface(standardParcelNFTAddress).upgradeParcel(_parcelId);
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_UPGRADE_STANDARD_PARCEL, nftIds, upgradePriceBusdGym[0]);
    }

    function getPriceForSuperBundle(uint8 _bundleType, uint8 _paymentType) external view returns(uint256[2] memory, uint256[2] memory) {
        return _getPriceForSuperBundle(_bundleType, _paymentType);
    }

    function getPriceForBundle(uint8 _bundleType, uint8 _paymentType) external view returns(uint256[2] memory, uint256[2] memory) {
        return _getPriceForBundle(_bundleType, _paymentType);
    }
    ...
    function _getPriceForPurchaseParcels (address _user, uint256 _parcelsCount, uint8 _paymentType) private view returns(uint256[2] memory, uint256[2] memory, uint256) {
        ...
        uint256[2] memory busdGymnetForMinting =  _priceInBusdGym(priceForMinting, _paymentType);
        uint256[2] memory busdGymnetForSingleProd = _priceInBusdGym(priceForSinglePurchase, _paymentType);
        return (busdGymnetForMinting, busdGymnetForSingleProd, rawParcelsPrice); /// TODO: currently rawPrices are not used and due to that are in BUSD
    }
    ...
    function _getPriceForPurchaseMiners(address _user, uint256 _minersCount, uint8 _paymentType) private view returns(uint256[2] memory, uint256[2] memory, uint256) {
        ...
        uint256[2] memory busdGymnetForMinting = _priceInBusdGym(priceForMinting, _paymentType);
        uint256[2] memory busdGymnetForSingleProd = _priceInBusdGym(priceForSinglePurchase, _paymentType);
        return (busdGymnetForMinting, busdGymnetForSingleProd, rawMinersPrice); /// TODO: currently rawPrices are not used and due to that are in BUSD
    }
    ...
    function _getPriceForSuperBundle(uint8 _bundleType, uint8 _paymentType) private view returns(uint256[2] memory, uint256[2] memory) {
        _validateSuperBundleType(_bundleType);
        SuperBundleInfo memory bundle = superBundlesInfos[_bundleType- BUNDLE_TYPE_SUPER_1];
        ...
        uint256[2] memory bundlePriceInBusdGym = _priceInBusdGym(bundlePrice, _paymentType);
        uint256[2] memory discountedPriceBusdGymnet = _priceInBusdGym(discountedPrice, _paymentType);
        return (discountedPriceBusdGymnet, bundlePriceInBusdGym);
    }

    function _getPriceForBundle (uint8 _bundleType, uint8 _paymentType) private view returns(uint256[2] memory, uint256[2] memory) {
        BundleInfo memory bundle = newBundles[_bundleType];
        (uint256 parcelPrice, ) = _getPriceForParcels(bundle.parcelsAmount);
        uint256 bundlePrice = parcelPrice + bundle.minersAmount * minerPrice;
        uint256 discountedPrice = _discountPrice(bundlePrice, bundle.discountPct);
    ...
    /// @notice Transfers the given Gymnet amount to distributor contract
    function _transferToContractGym(uint256 _amount) private {
        IERC20Upgradeable(gymnetAddress).safeTransferFrom(
            address(msg.sender),
            address(amountsDistributorAddress),
            _amount
        );
    ...
    function _paymentCheck(uint8 _paymentType) private view {
            if(!GymnetSaleActivated) {
            require(_paymentType == PAYMENT_TYPE_BUSD, "Municipality: GaymnetSale is not active");
        }
    }    

    ///@notice function for dual token payment 50% busd 50 GymNet
    function _priceInBusdGym(uint256 _price, uint8 _paymentType) private view returns(uint256[2] memory) {
        uint256[2] memory totalPriceBusdGym = [_price, 0];
        if(_paymentType == PAYMENT_TYPE_BUSD_GYMNET) {
            totalPriceBusdGym[0] = _price / 2;
            totalPriceBusdGym[1] = (_price * 1e18) / _getGymnetPriceInBUSD() / 2;
        }
        return totalPriceBusdGym;
    }       

    /// @notice function returning sum of array elements for dual payment
    function _sumBusdGymInBusd(uint256[2] memory _array) private view returns(uint256) {
        return _array[0] + _array[1] * _getGymnetPriceInBUSD();
    }

    function _getGymnetPriceInBUSD() private view returns (uint256) {
        address[] memory path = new address[](2);

        path[0] = gymnetAddress;
        path[1] = wbnbAddress;
        uint256[] memory gymnetPriceInWBNB = IPancakeRouter02(routerAddress).getAmountsOut(
            1000000000000000000,
            path
        );
        path[0] = wbnbAddress;
        ...
        return  gymnetPriceInWBNB[1] * wbnbPriceInBUSD[1];
    }
}
