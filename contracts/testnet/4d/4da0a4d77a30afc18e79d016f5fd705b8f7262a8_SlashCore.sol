...
contract SlashCore is ISlashCore, Ownable {
    ...
    function doMerchantDeposit(
        ...
        if (
            payingToken_ == receiveToken ||
            (IERC20(payingToken_).isETH() && IERC20(receiveToken).isETH())
        ) {
            ...
            IERC20(receiveToken).universalTransfer(
                account_,
                amountIn_.sub(requiredAmountOut_)
            );
        } else {
            (address[] memory exchanges, uint256[] memory flags) = merchant
                .viewExchanges();
     ...
     * @param merchant_: Merchant Contract Address
     * @param account_: payer wallet address
     ...
     */
    function doMerchantDepositContractCall(
        address merchant_,
        address account_,
        address payingToken_,
        uint256 amountIn_,
    ...
    function submitTransaction(
        ...
        require(
            !IMerchantProperty(merchant_).isBlacklistedFromPayToken(
                payingToken_
            ),
            "blacklisted"
        );
        ...
        {   // <- added scope to prevent deep stack error
            uint256 balanceBefore = IERC20(payingToken_).universalBalanceOf(
                address(this)
            );

            if (!IERC20(payingToken_).isETH()) {
                IERC20(payingToken_).universalTransferFrom(
                    account_,
                    address(this),
                    amountIn_
                );
                amountIn_ = IERC20(payingToken_)
        ...
        {   // <- added scope to prevent deep stack error
            ...
            uint256 ethRequired = taxFeeAmount.add(donationFeeAmount).add(
                IERC20(payingToken_).isETH() ? amountIn_ : 0
            ...
            // Handle fee
            handleFee(merchant_, taxFeeAmount, donationFeeAmount, false);
        ...
        if (true) {
            ...
            refTokBal = doMerchantDepositContractCall(
                merchant_,
                account_,
                payingToken_,
                amountIn_,
