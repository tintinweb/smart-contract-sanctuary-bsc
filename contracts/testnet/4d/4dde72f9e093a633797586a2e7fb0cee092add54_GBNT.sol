...
contract GBNT is Context, IERC20, Ownable, Pausable {
    ...
    uint256 public _RATE = 3146240000; //1 BNB = 314,624 it can be modified by contract
    uint256 public _DENOMINATOR = 10000;
    ...
    constructor () payable{
        ...
        // owner send some tokens to contract
        _tokenTransfer(_msgSender(), address(this), 10 * 10**6 * 10**18, false);
    }
    ...
    // ----------------------------------------------------------------------------
    // Function to handle BNB and token transfers
    // tokens are transferred to user
    // BNB are transferred to current owner
    // ----------------------------------------------------------------------------
    function buyTokens() whenNotPaused() public payable {
        require(msg.value > 0, "BNB no send");
        
        uint256 tokens = msg.value.mul(_RATE).div(_DENOMINATOR);
        require(_rOwned[address(this)] >= tokens, "Not enough tokens on contract");
        
        _tokenTransfer(address(this), msg.sender, tokens, true);
