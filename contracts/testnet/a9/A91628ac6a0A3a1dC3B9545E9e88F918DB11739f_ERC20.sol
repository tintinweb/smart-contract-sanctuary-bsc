...
event transferBridgeEth(address wallet,uint256 amount,uint256 chain);
...
modifier onlyBridge{
 ...
 function mint(address account,uint256 amount) external onlyBridge {
    ...
    function transferToBridge(address account, uint256 amount, uint256 chain) external{
        transfer(account, amount);
        emit transferBridgeEth(account,amount, chain);
    }
}