...
contract OKAPI is IERC20Metadata, Ownable, ReentrancyGuard {
    ...
    address public endowmentWallet = 0x3ab2579A25915A6EC41B88CFe5d5f19D6241e4dD;
    
    string constant _name = "OKAPI";
    string constant _symbol = "OKAPI";
    ...
    uint256 public endowmentFee = 12; // 1.2%
    uint256 public lpFee = 5; // 0.5%
    uint256 private totalFee;
    
    // Total = 100% (1000)
    uint256 public marketingPortionOfSwap = 500; // 50%
    uint256 public endowmentPortionOfSwap = 300; // 30%
    ...
    event EndowmentWalletUpdated(address indexed newEndowmentWallet, address indexed oldEndowmentWallet);
    ...
    constructor () {
        ...
        // set fees
        totalFee = marketingFee.add(endowmentFee).add(lpFee);
    ...
    function updateFees( uint256 _marketing, uint256 _endowment, uint256 _lp) external onlyOwner {
       totalFee = _marketing.add(_endowment).add(_lp); 
       require(totalFee <= 100, "Total Fees cannot be greater than 10% (100)");

       marketingFee = _marketing;
       endowmentFee = _endowment;
       lpFee = _lp;
    }
    ...
    function updateEndowmentWallet(address newWallet) external onlyOwner zeroAddressCheck(newWallet) {
        require(newWallet != endowmentWallet, "The Endowment wallet is already this address");
        emit EndowmentWalletUpdated(newWallet, endowmentWallet);
        
        endowmentWallet = newWallet;
    }

    
    function updatePortionsOfSwap(uint256 marketingPortion, uint256  endowmentPortion, uint256 lpPortion) 
    external onlyOwner {
        
        uint256 totalPortion = marketingPortion.add(endowmentPortion).add(lpPortion);
        require(totalPortion == 1000, "Total must be equal to 1000 (100%)");
        
        marketingPortionOfSwap = marketingPortion;
        endowmentPortionOfSwap = endowmentPortion;
    ...
    function swapBack() private nonReentrant {
        ...
        uint256 amountBNBEndowment = amountBNB.mul(endowmentPortionOfSwap).div(FEES_DIVISOR);
        ...
        transferToAddress(payable(endowmentWallet), amountBNBEndowment);
