...
contract PositionBondFactory is BondFee, ReentrancyGuard, Ownable {
    ...
    function issueBond(
        ...
        require(
            positionBondFactoryStorage.bondSymbolExist(bondInformation.bondSymbol) == false,
            "bondSymbolExist"
        );
        ...
        if (assetInformation.underlyingAssetType == 0) {
            ...
            assetInformation.collateralAmount = IERC20(assetInformation.underlyingAsset).balanceOf(address(positionBond));
        }
        ...
        if (assetInformation.underlyingAssetType == 1) {
            ...
            assetInformation.collateralAmount = positionBondFunction.getParValue(assetInformation.nftIds);
        }
        ...
        positionBondFactoryStorage.pushNewBond(msg.sender, address(positionBond), bondInformation.bondSymbol);


        emit BondCreated(bondInformation, assetInformation, address(positionBond));
    ...
    function updatePositionBondFactoryStorage(address positionBondFactoryStorage_)
        public
        onlyOwner
    {
        positionBondFactoryStorage = IPositionBondFactoryStorage(positionBondFactoryStorage_);
    }
...
interface IPositionBondFactoryStorage {
    ...
    function pushNewBond(address issuer_ ,address newBond_, string memory symbol_) external;
}
...
contract PositionBondFactoryStorage is Ownable {
    ...
    function pushNewBond(address issuer_ ,address newBond_, string memory symbol_) external onlyBondFactory {
    ...
    ReentrancyGuard
{
    ...
    function isNotReachMaxLoanRatio() public view returns (bool) {
        uint256 bondAmountInPhase = BondStatusLending.getStatus() == Status.Pending
            ? bondSupply
            : totalSupply();
        return
    ...
    ) public payable onlyIssuer onlyOnSaleOrActive nonReentrant {
        ...
        nonReentrant
    {
        ...
        require(totalSoldAmountClaimed <= totalSupply() * getIssuePrice(), "!claim");
    ...
    function liquidate() external nonReentrant {
        ...
        if (getUnderlyingAssetType() == 1) {
            ...
            (address underlyingAsset, ) = UnderlyingAssetLending
                .underlyingAsset();
            address tokenMapped =  positionBondFunction.getTokenMapped(underlyingAsset);
            uint256 balanceBefore = IERC20(tokenMapped).balanceOf(address(this));
            UnderlyingAssetLending.decompose(
                posiNftFactory,
                positionBondFunction.getTokenMapped(underlyingAsset)
            );
            UnderlyingAssetLending.updateUnderlyingAmount(IERC20(tokenMapped).balanceOf(address(this)) - balanceBefore);
        }
    ...
    {
        return (BondMath.calculateBondAmountWithFixPrice(
            amount,
...
//        initUnderlyingAmount = underlyingAmount_;
    ...
    function _addCollateral(
        ...
        if (underlyingAssetType == 0) {
            ...
            amountAdded = IERC20(_underlyingAsset).balanceOf(
                address(this)
            ) - balanceBefore;
