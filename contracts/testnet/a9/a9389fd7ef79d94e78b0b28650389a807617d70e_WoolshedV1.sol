...
contract WoolshedV1 is OwnableUpgradeable, ContractGuard {
    ...
    mapping(address => bool) private _isInfinitePlayer;
    ...
    // Set infinite player - used to skip adding anything to the claimed value counter
    function setInfinitePlayer(address _addr, bool _selection) public onlyOwner {
        _isInfinitePlayer[_addr] = _selection;
    }
    ...
    function _refPayout(address _addr, uint256 _amount) internal {
        ...
        for (uint8 i = 0; i < ref_depth; i++) {
            ...
            uint256 _refBonus = ratesController.getRefBonus(i); // 5 for most tiers
    ...
    function _claim_out(address _addr) internal {
        ...
        taxVault.withdraw(address(woolToken), realizedPayout);
    ...
    function infinitePlayer(address _addr) public view returns (bool) {
        return (_isInfinitePlayer[_addr]);
    }
