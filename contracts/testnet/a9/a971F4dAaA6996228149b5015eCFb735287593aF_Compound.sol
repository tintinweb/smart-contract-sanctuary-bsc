...
contract Compound is OwnableUpgradeable{
    ...
    function _compoundCattle(uint tokenId, uint target) internal{
        require(stable.isStable(tokenId),'not in stable');
        require(!stable.isStable(target),'target must in wallet');
    ...
    function refreshDeadTime(uint tokenId) internal{
        uint life = cattle.getLife(tokenId);
        uint star = cattle.getStar(tokenId);
        uint limit = cattle.starLimit(star);
        uint newLife = life * 100 / limit;
        uint newDeadTime = block.timestamp + (35 days * newLife / 10000);
        uint oldDeadTime = cattle.deadTime(tokenId);
        cattle.addDeadTime(tokenId,newDeadTime - oldDeadTime);
    }
}
