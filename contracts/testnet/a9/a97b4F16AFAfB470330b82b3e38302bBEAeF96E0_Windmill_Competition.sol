...
contract Windmill_Competition is Base, Math, Adressable, AccessControl, With_DAORole, With_TradeRole,
    ...
    function setPercentPWRRewardMint(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        require(denominator>0, "Windmill_Competition: Denominator cannot be null");
    ...
    function getUserDetails(uint256 cycleId, address addr) external view returns(UserDetails memory){
        require(cycleId <= currentCycle, "Windmill_Competition: Cycle not found");
    ...
    function getOneProfitUser(uint256 cycleId, uint256 userId) external view returns(address){
        ...
        require(userId < cycle.users.length, "Windmill_Competition: User not found");
