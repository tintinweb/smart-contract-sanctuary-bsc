...
contract EuphoriaBetting is Ownable {
    ...
    event MatchAddition(uint256[] matches, Odds[] odds);
    ...
    function makeBet(LibBet.Bet memory bet) external {
        require(
            matches[bet.matchId].isAvailable,
    ...
    function finishMatch(
        ...
        require(
            matches[matchId].isAvailable == false,
            "Match must be closed before finishing"
        );
    ...
    function openMatches(uint256[] memory _matches, Odds[] memory odds)
        external
        onlyOwner
    {
        require(
            _matches.length == odds.length,
            "Length of matches and odds must be equal"
        );
        for (uint256 i; i < _matches.length; i++) {
            Match storage currentMatch = matches[_matches[i]];
            currentMatch.isAvailable = true;
            currentMatch.odds = odds[i];
        }

        emit MatchAddition(_matches, odds);
    }

    function closeMatches(uint256[] memory _matches) external onlyOwner {
        for (uint256 i; i < _matches.length; i++) {
            matches[_matches[i]].isAvailable = false;
        }

        emit MatchRemoval(_matches);
    }

    function cancelMatches(uint256[] memory _matches, Reward[] memory rewards)
        external
        onlyOwner
    {
        require(_matches.length > 0, "Length of matches must not be zero");
        for (uint256 i; i < _matches.length; i++) {
            matches[_matches[i]].isAvailable = false;
        }
