...
contract GrainAggregatorProxy is IAggregatorInterface {
    ...
    uint256 private MULTIPLE = 1e8;

    address public grain;
    address public wbnb;

    address public wbnbGrainPair;

    address public priceChecker;

    uint256 public minimumReserve = 0.1 ether;

    constructor(address _grain, address _wbnb, address _wbnbGrainPair, address _priceChecker) public {
        grain = _grain;
        wbnb = _wbnb;
        wbnbGrainPair = _wbnbGrainPair;
    ...
    function latestAnswer() external override view returns (int256) {
        ...
        (uint256 _grainRes, uint256 _wbnbRes) = getReserves(grain, wbnb, wbnbGrainPair);
        if (_grainRes < minimumReserve) return 0;
        return int256(_priceChecker.getTokenPriceToUsd(wbnb).mul(_wbnbRes).mul(MULTIPLE).div(_grainRes).div(1e18));
    }
