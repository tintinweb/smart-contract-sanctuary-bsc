...
// A fork of Reflect.Finance, EverestCoin and SafeMoon
...
contract FluffyPuppyToken is Context, IERC20, Ownable {
    ...
    mapping (address => TransferDetails[]) transferHistory;
    ...
    uint256 private sniperBlockDelta = 120;
    ...
    */
    ...
    string private _name = 'Fluffy Puppy Token';
    string private _symbol = 'FLUF';

    uint256 public reflectionFeePercentage = 0;
    uint256 public liquidityFeePercentage = 0;
    uint256 public devFeePercentage = 0;
    uint256 public burnFeePercentage = 0;

    uint256 public txTimeLimit = 60 * 60 * 24;
    uint256 public txLimit = 1000 * 1000 * 10**18;

    uint256 private numTokensSellToAddToLiquidity = 500 * 1000 * 10**18;
    ...
    */
    ...
    function includeAccountToTxLimitFromAddress(address account) external onlyOwner() {
        ...
        for (uint256 i = 0; i < _excludedFromTxLimitFromAddresses.length; i++) {
    ...
    function setFeePercentage(FeeType _feeType, uint256 _feePercentage) public onlyOwner {
        ...
        } else if (_feeType == FeeType.LIQUIDITY) {
            liquidityFeePercentage = _feePercentage;
    ...
    function shouldLimitTransfer(address sender, uint256 amount) public view returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        transferHistory[sender].push(transferDetails);
    ...
    function _getRInputs(
        ...
        return RInputs(tAmount, reflectionFee, liquidityFee, devFee, burnFee, currentRate);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (TValues memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(reflectionFee).sub(liquidityFee).sub(devFee).sub(burnFee);
        return TValues(tTransferAmount, reflectionFee, liquidityFee, devFee, burnFee);
    }
    ...
    function _getRValues(RInputs memory rInputs) private pure returns (RValues memory) {
        ...
        uint256 rTransferAmount = rAmount.sub(rReflectionFee).sub(rLiquidityFee).sub(rDevFee).sub(rBurnFee);
        return RValues(rAmount, rTransferAmount, rReflectionFee, rLiquidityFee, rDevFee, rBurnFee);
    }
    ...
    function txAmountSinceTimeLimit(address _from) private view returns (uint256) {
        uint256 amount = 0;

        for (uint256 i = 0; i < transferHistory[_from].length; i++) {
