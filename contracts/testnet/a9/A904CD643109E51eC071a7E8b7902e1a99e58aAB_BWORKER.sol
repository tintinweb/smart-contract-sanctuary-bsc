...
interface IBWORKER_GAME {
    function isBWorkerGame() external pure returns (bool);

    function unlockBalance(address account) external view returns (uint256);
...
contract BWORKER is Context, IERC20, IERC20Metadata, Ownable, IBWORKER {
    ...
    mapping(address => bool) private _gameContract;
    uint256 public maxSell = 100;
    ...
    address public addLiquidityReceiver =
        0x678944e4d708Ad6393965e065c9f3Fa67B383456;
    //todo: remove
    //address public BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public BUSD = 0x3eDf6A6fc783e533d67908fB0eC0620F5852Abc9;
    ...
    function _autoClaimNftReward(address account) internal {
        ...
        autoClaimEpoch[account] = lastMintEpoch;
        _lockBalances[account] = _lockBalances[account].add(_nftRewardAmount);
    ...
    function _transfer(
        ...
        } else if (_gameContract[sender]) {
            _gameWithdraw(sender, recipient, amount);
        } else {
    ...
    function _gameWithdraw(
        ...
        IBWORKER_GAME gameContract = IBWORKER_GAME(sender);
        _unLockBalances[sender] = _unLockBalances[sender].sub(amount);

        uint256 unlockGameBalance = gameContract.unlockBalance(recipient);
        if (unlockGameBalance >= amount) {
            _unLockBalances[recipient] = _unLockBalances[recipient].add(amount);
        } else {
            uint256 lockGameBalance = amount.sub(unlockGameBalance);
            _unLockBalances[recipient] = _unLockBalances[recipient].add(
                unlockGameBalance
            );
            _lockBalances[recipient] = _lockBalances[recipient].add(
                lockGameBalance
            );
        }
    }
    ...
    function _presaleClaim(
        ...
        _autoConvertNFTAfterClaim(recipient, numOfNFT);
    ...
    function _autoConvertNFTAfterClaim(address recipient, uint256 numOfNFT)
        internal
    {
