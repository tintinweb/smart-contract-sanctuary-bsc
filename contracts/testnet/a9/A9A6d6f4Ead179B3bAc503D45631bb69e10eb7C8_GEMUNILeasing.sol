...
contract GEMUNILeasing is PermissionGroupUpgradeable, IGEMUNILeasing{
    ...
    function registerLeasing(address nftToken, uint[] memory tokenIds, uint duration, uint minDuration, uint maxDuration, uint[] memory pricePerWeek, uint mortgageAmount, uint rentalPrice) external override {
        ...
        for(uint i = 0; i < tokenIds.length; i++) {
            LeasingInfo storage leasing = leasingInfo[nftToken][tokenIds[i]];
            LeasingGuildInfo storage leasingGuild = leasingGuildInfo[nftToken][tokenIds[i]];
            RenterInfo memory renter = renterInfo[nftToken][tokenIds[i]];
            ...
            require(minDuration > 0 && maxDuration > minDuration, "Leasing: invalid duration");
    ...
    function setNumberPassesOfGameGuild(uint _minPasses) external onlyOperator {
        require (_minPasses > 0, "invalid min");
    ...
    function rentMore(address nftToken, uint tokenId, uint amount) external override {
        ...
        if (msg.sender == renter.renter && guild.gameGuild != address(0)) {
            ...
            uint endLendingMore = renter.startLending + (renter.duration + caculatedDuration) * weekPerSecond;
            require(
                endLendingMore <= guild.startLending + guild.duration * weekPerSecond &&
                renter.duration + caculatedDuration <= guild.maxDuration,
                "Leasing: invalid amount"
            );
            ...
            lender = treasury;
            if (msg.sender == guild.gameGuild) {
                require(guild.duration + caculatedDuration <= leasing.maxDuration, "Leasing: invalid amount");
                guild.duration += caculatedDuration;
            } else {
                require(renter.duration + caculatedDuration <= leasing.maxDuration, "Leasing: invalid amount");
                renter.duration += caculatedDuration;
            }
        ...
        emit RentMore(nftToken, tokenId, geniPrice, caculatedDuration, lender , msg.sender);
    }
                    ...
                    if (nftToken == geniPass) {
                        ...
                        if (IERC721(nftToken).ownerOf(tokenId) == msg.sender) {
                            if (IERC721(nftToken).getApproved(tokenId) == address(this) || IERC721(nftToken).isApprovedForAll(msg.sender, address(this))) {
                                ...
                                geni.transfer(msg.sender, guild.mortgageAmount);
                            } else {
    ...
    function checkTransferNFT(address nftToken, uint tokenId, uint amount, address renter, address lender, bool isTransferWallet) internal {
        if (IERC721(nftToken).ownerOf(tokenId) == renter) {
            if (IERC721(nftToken).getApproved(tokenId) == address(this) || IERC721(nftToken).isApprovedForAll(renter, address(this))) {
                transferNFT(nftToken, tokenId, renter, address(this), isTransferWallet);
...
interface IGEMUNILeasing {
    ...
    event PutOnLeasing(address nftToken, uint[] indexed tokenIds, uint minDuration, uint maxDuration, uint[] pricePerweek, address lender, uint mortgageAmount);
    ...
    event UpdateLeasing(address nftToken, uint tokenId, uint newMinDuration, uint newMaxDuration, uint newPricePerWeek, bool newTransferWallet);
