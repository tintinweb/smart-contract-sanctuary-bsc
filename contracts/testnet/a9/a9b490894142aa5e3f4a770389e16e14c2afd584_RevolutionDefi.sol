...
contract RevolutionDefi is IERC20, Ownable {
    ...
    function clearStuckBalance(IERC20 token, address to, uint256 _amount) public onlyOwner {
        uint256 revolt = token.balanceOf(address(this));
        require(_amount <= revolt);
        token.transfer(to, _amount);
        emit TransferSent(msg.sender, to, _amount);
    }
