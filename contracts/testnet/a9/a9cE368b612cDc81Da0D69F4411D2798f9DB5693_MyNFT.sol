...
contract MyNFT is TRC21, ERC721Full("SPHEREyellow", "NFT4"), Ownable {
    ...
    TRC21 public feeToken = TRC21(0x96F4cf573Fc8074483511A9aeb1F8FeBbB191F96);
    constructor () public {
        _name = 'SPHEREyellow';
        _symbol = 'NFT4';
        _decimals = 0;
    ...
    function register(address user, uint _numItems, address _creator) onlyManager public {
        for (uint j = 0; j < _numItems ; j++) {
    ...
    function create(address user, address _creator) onlyManager public returns(uint){
        _mint(user, ++tokenIdd);
        metadata[tokenIdd] = meta(_creator);
        emit _register(user, tokenIdd);
        return tokenIdd;
    }
    ...
    function transfer(address to, uint256 value) public returns (bool) {
        require(feeToken.transferFrom(msg.sender, owner(), minFee()));
        require(!userBlocks[msg.sender] && !userBlocks[to]);
        return super.transfer(to, value);
    }
    function transferFrom(address from, address to, uint256 tokenId) public {
        require(isBusiness() || feeToken.transferFrom(from, owner(), minFee()));
    ...
    function setMinFee(uint256 value) public onlyManager{
        _changeMinFee(value);
    }
}