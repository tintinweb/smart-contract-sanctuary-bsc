...
contract YFDNode is OwnableUpgradeable, UUPSUpgradeable {
    ...
    uint256 public usenum;

    uint256 public price;

    EnumerableSetUpgradeable.AddressSet private useaddrs;
    ...
    function initialize(
        ...
        __Ownable_init();
        reset(_wallet, _usdt, _maxnum, _price);
    }

    function buyNode() public {
        require(usenum < maxnum, "buy over limit");
        require(!useaddrs.contains(msg.sender), "msg sender bought");
        usenum++;
        useaddrs.add(msg.sender);
        IERC20(usdt).transferFrom(msg.sender, wallet, price);
        emit BuyNode(msg.sender, wallet, price, block.timestamp);
    }

    function getNodeAddrs() public view returns (address[] memory) {
        return useaddrs.values();
    }
