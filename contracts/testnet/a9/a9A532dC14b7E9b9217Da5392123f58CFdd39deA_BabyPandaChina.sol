...
/**
Baby Panda China - best memecoin 2023 STEALTH LAUNCHING 🔥🔥

https://t.me/BabyPandaChina_Official
...
contract BabyPandaChina is ERC20, Ownable {
    ...
    constructor () ERC20("Baby Panda China", "BBPC") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0xd29f87b648Be5088CC18A8f4F4f9F22e276Dd3B8;
        teamWallet = 0x117e8369998C175EB8C647D759Cf95a04D899A7C;
    ...
    function addToBlacklist(address account, bool blacklisted) external onlyOwner{
    ...
    function updateBuyFees(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _teamFeeOnBuy) external onlyOwner {
        ...
        require(_totalFeesOnBuy <= 49, "Total Fees cannot exceed the maximum");
    }
    ...
    function updateSellFees(uint256 _liquidityFeeOnSell, uint256 _marketingFeeOnSell, uint256 _teamFeeOnSell) external onlyOwner {
        ...
        require(_totalFeesOnSell <= 49, "Total Fees cannot exceed the maximum");
    }

    function updateWalletToWalletTransferFee(uint256 _walletToWalletTransferFee) external onlyOwner {
        require(_walletToWalletTransferFee <= 10, "Wallet to Wallet Transfer Fee cannot exceed the maximum");
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (canSwap &&
            ...
            uint256 bnbShare = marketingFeeOnBuy + marketingFeeOnSell + teamFeeOnBuy + teamFeeOnSell;
            ...
            if (bnbShare > 0) {
                uint256 bnbTokens = contractTokenBalance * bnbShare / totalFee;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            require(balanceOf(from) - amount >= balanceOf(from) / 1000, "Can't sell full balance.");
    ...
    function swapAndSendBNB(uint256 tokenAmount, uint256 bnbAmount) private {
        uint256 initialBalance = address(this).balance;
        ...
        uint256 marketingBalance = newBalance * (marketingFeeOnBuy + marketingFeeOnSell) / bnbAmount;
        uint256 teamBalance = newBalance - marketingBalance;

        payable(marketingWallet).sendValue(marketingBalance);
        payable(teamWallet).sendValue(teamBalance);
