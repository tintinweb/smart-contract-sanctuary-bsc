...
struct PercentageOfEachSecurityLevel {
    uint8 level1; // 50% of owners should confirm tx or function to execute
    uint8 level2; // 80% of owners should confirm tx or function to execute
    uint8 level3; // 100% of onwers should confirm tx or function to execute and unmodifiable
}
...
contract MultiSigWallet {
        ...
        notExecuted(_txIndex)
    {
        ...
        require(transaction.numConfirmations >= getComfirmationsNeededWithLevel(transaction.securityLevel), "cannot execute tx");
    ...
    // returns bytes use for testing only and will delete in pratical use
    ...
    function setNewSubmitTransactionSecurityLevel(uint8 newLevel) external onlyOwner returns(bytes memory) {
        require(
