...
contract NFTHoodie is NFTBase {
    constructor(
        ...
        NFTBase(
            "GENESiS HOODIE 11/22",
            "PHY11",
            baseUri_,
            startDate_,
            priceUsd_,
            rewardManagerAddress_,
            relayerAddress_,
            feedAddress_
        )
    {}
...
abstract contract NFTBase is ERC721A, ERC721AQueryable, Ownable, PriceManager, ValidatorRelayer {
    ...
    event NFTUpgraded(address user, uint256 id, NFT nft);
    ...
    modifier inMintTime() {
        if (block.timestamp < startDate && block.timestamp > endDate()) revert NotInTime();
        _;
    }
    ...
    function upgrade(OrderItem[] memory orderItems) public payable inMintTime {
        // 1% Slippage
    ...
    function upgradeToken(OrderItem[] memory orderItems, address token) public {
        ...
        if (IERC20(token).allowance(address(this), address(rewardManager)) < totalPrice) {
            IERC20(token).approve(address(rewardManager), 2**256 - 1);
        }
    ...
    function _upgradeNFT(address to, OrderItem memory item) internal {
        if (item.upgradeId == 0) {
            _safeMint(to, 1);
            nftById[_totalMinted()] = item.nft;
            upgradedById[_totalMinted()] = true;
            emit NFTUpgraded(to, _totalMinted(), item.nft);
        } else {
            ...
            countFreemint[to] -= 1;
            emit NFTUpgraded(to, item.upgradeId, item.nft);
        }
    }
...
abstract contract PriceManager is Ownable {
    ...
    function getTokenPrice(address token, uint256 price) public {
        _upgradePrice[token] = price;
    }
}
