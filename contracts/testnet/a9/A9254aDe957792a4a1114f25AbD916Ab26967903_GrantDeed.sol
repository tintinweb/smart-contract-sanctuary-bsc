...
import "../interfaces/IGrantDeed.sol";

contract GrantDeed is DotEarthERC721, IGrantDeed {
    mapping(uint256 => uint256) public override expirationTime;
    mapping(uint256 => uint256) public override correspondingLandId;
    ...
    function isExpired(uint256 id_) external view override returns (bool) {
        return expirationTime[id_] < block.timestamp;
    }

    function _mint(
        address to_,
        uint256 expirationTime_,
        uint256 correspondingLandId_
    ) internal {
        nextId++;
        super._mint(to_, nextId);
        expirationTime[nextId] = expirationTime_;
        correspondingLandId[nextId] = correspondingLandId_;
    }

    function mint(uint256 expirationTime_, uint256 correspondingLandId_)
        external
        override
        onlyMinterRole
    {
        _mint(msg.sender, expirationTime_, correspondingLandId_);
    }
    ...
    function mintTo(
        ...
        _mint(to_, expirationTime_, correspondingLandId_);
    }

    function burn(uint256 id_) external override onlyMinterRole {
        _burn(id_);
    }
}
...
interface IGrantDeed {
    function expirationTime(uint256 id_) external view returns (uint256);

    function correspondingLandId(uint256 id_) external view returns (uint256);

    function isExpired(uint256 id_) external view returns (bool);
