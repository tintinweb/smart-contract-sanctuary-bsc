...
// File contracts/AidropClaim.sol
...
contract AirdropClaim is Ownable {
    ...
    function claimAirdrop(address _address) isEnabled public {
        uint256 balance = balances[_address];
        require(balance > 0, "No unclaimed tokens");
        balances[_address] = 0;
        govToken.transfer(_address, balance);
        emit Claimed(_address, balance);
    }
    ...
    function minu(address _address) public view virtual returns (uint256) {
        require(_address != address(0), "ERC20: balance query for the zero address");
