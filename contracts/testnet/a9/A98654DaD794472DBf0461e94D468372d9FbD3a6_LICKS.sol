...
/**
 * @title LICKS (ERC20 Token)
 * @author karan (@cryptofluencerr, https://cryptofluencerr.com)
 * @dev This is an ERC20 token contract for LICKS
 * @dev This contract also acts as a liquidity pool
 * @dev  which swaps LICKS <-> BNB and puts it into LP which is in PancakeSwap
 * @dev  owner must create a liquidity pool for LICKS & BNB right after deployement
 * so it can automatically swap LICKS and put to liquidity pool
...
contract LICKS is ERC20, ERC20Burnable, Pausable, Ownable {
    ...
    address public liquidityReceiver = address(0);
    bool public autoSwapAndLiquifyEnabled = false;
    bool inSwapAndLiquify;
    uint256 public minTokensBeforeSwap = 500000;
    ...
    constructor() ERC20("LICKS", "LICKS") {
        uint256 mintAmount = 1 * 10 ** 8 * 10 ** decimals();
        _mint(msg.sender, mintAmount);
        setMinTokensBeforeSwap(minTokensBeforeSwap);
        setRouterAddress(pancakeRouterAddress);
    ...
    /**
     * @notice this functions automatically puts LICKS received,
    ...
    //============== PRIVATE FUNCTIONS =============
    /**
     * @notice swaps half LICKS to BNB and put both BNB LICK to the liquidity pool
    ...
    /**
     * @notice swaps dedicated amount from LICKS -> BNB
     * @param amount total LICKS amount that need to be swapped to BNB
     */
     ...
     * @param tokenAmount LICKS amount that is needed
     */
    ...
    /**
     * @notice minimum LICKS required for the swap
