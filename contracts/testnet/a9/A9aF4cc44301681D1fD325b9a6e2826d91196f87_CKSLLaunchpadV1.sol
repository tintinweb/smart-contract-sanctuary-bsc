...
contract CKSLLaunchpadV1 is Context, Ownable {
  ...
  */
  ...
  function ape(uint256 padid,uint256 amount) external returns (bool) {
    ...
    IERC20(pad[padid].tokencommit).transferFrom(msg.sender,receiver,amount.mul(675).div(1000));
    IERC20(pad[padid].tokencommit).transferFrom(msg.sender,treasury,amount.mul(300).div(1000));
    IERC20(pad[padid].tokencommit).transferFrom(msg.sender,reserve,amount.mul(25).div(1000));
