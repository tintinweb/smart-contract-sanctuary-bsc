...
contract BitBotFriends is Ownable, ERC721, NonblockingReceiver {
    using Strings for uint256;
    ...
    uint256 nextTokenId = 6001;
    uint256 MAX_MINT_SUPPLY = 7000;
    ...
    constructor(string memory baseURI_, address _layerZeroEndpoint)
        ERC721("BitBot Friends", "BB Friends")
    {
        _owner = msg.sender;
    ...
    /** 
     @dev You can mint up to 5 NFTs with a single address.
     Beware that to not mint to much or the transaction might fail
     mint is free, but donations are also accepted
    **/ 
    function mint(uint256 _mintAmount) external payable {
        require(
            _mintAmount <= maxMintAmount,
            "Mint Amount exceeds the Maximum Allowed Mint Amount of 5"
        );
        require(_mintAmount > 0, "Mint Amount needs to be bigger than 0");
        require(
            nextTokenId + _mintAmount - 1 <= MAX_MINT_SUPPLY,
