...
contract LvlStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _LVL, address _llpRewardDistributor) external initializer {
        require(_LVL != address(0), "LvlStaking:initialize: invalid Lvl");
        require(_llpRewardDistributor != address(0), "LvlStaking:initialize: invalid reward distributor");
        __Ownable_init();
    ...
    function stake(address _to, uint256 _amount) external nonReentrant {
        require(_amount > 0, "LvlStaking::stake: amount > 0");
        ...
        LVL.safeTransferFrom(sender, address(this), _amount);
        emit Staked(sender, _to, _amount);
    }

    function unstake(address _to, uint256 _amount) external nonReentrant {
        require(_amount > 0, "LvlStaking::unstake: amount > 0");
        address sender = msg.sender;
        UserInfo memory _userInfo = userInfo[sender];
        require(_userInfo.amount >= _amount, "LvlStaking::unstake: unstake amount > staked amount");
        updateRewards();
        _userInfo.amount -= _amount;
        _userInfo.rewardDebt -= int256((_amount * accRewardPerShare) / ACC_REWARD_PRECISION);
    ...
    function claimRewardsToSingleToken(address _to, address _rewardTokenOut, uint256 _minAmountOut)
        ...
        nonReentrant
    {
        require(_rewardTokenOut != address(0), "LvlStaking::claimRewards: Invalid address!");
        updateRewards();
        address sender = msg.sender;
        UserInfo memory user = userInfo[sender];
        int256 _accumulatedReward = int256((user.amount * accRewardPerShare) / ACC_REWARD_PRECISION);
    ...
    function setLlpRewardDistributor(address _llpRewardDistributor) external onlyOwner {
        require(_llpRewardDistributor != address(0), "LvlStaking::setLlpRewardDistributor: Invalid address");
        llpRewardDistributor = ILlpRewardDistributor(_llpRewardDistributor);
