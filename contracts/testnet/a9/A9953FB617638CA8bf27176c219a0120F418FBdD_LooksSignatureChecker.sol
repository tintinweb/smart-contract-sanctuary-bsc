 ...
 */
contract  LooksSignatureChecker {
    /**
    ...
    function test( bytes32 hash, bytes32 domainSeparator)public view returns (bytes32){

        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, hash));
        return (digest);
    }

    function test2( bytes32 hash,
        bytes32 domainSeparator)public  returns (bytes32){
        string memory a = string(abi.encodePacked("\x19\x01", domainSeparator, hash));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, hash));
        emit TestEvent(digest,a);
        return (digest);
    }
}