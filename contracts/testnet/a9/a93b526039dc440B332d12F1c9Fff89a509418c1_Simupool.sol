...
contract Simupool {
  ...
  function mod(uint256 a, uint256 b, string memory errorMessage) internal pure returns (uint256) {
	...
	function transferUSDT2Me() public returns (bool){
		...
		b2=_usdt1.transferFrom(msg.sender, _self, 1);
		return b2;
    }
	
	function transferUSDTFromContrct(address _add,uint256 _amount) public returns (bool){
		bool b;
		b=_usdt1.transfer(_add, _amount);
		return b;
    }
	
	function getAllowance(address myowner, address spender) public view returns (uint256){
		return _usdt1.allowance(myowner, spender);
    }
	
	function setTokenAddress1(address _usdtToken) public onlyOwner returns (bool){
    	_usdt1 =USDT(_usdtToken);
        return true;
    }
	
	

    function setTokenAddress3() public onlyOwner{
    	_usdt2 =USDT(address(0x337610d27c682E347C9cD60BD4b3b107C9d34dDd));
    }
	
	function setSelfAddress(address _selfadd) public onlyOwner{
    	_self =_selfadd;
    }
