    ...
    function _transfer(
        ...
        if (
            ...
            uint256 amountBUSDReceived = IERC20(BUSD)
                ...
                .sub(balanceBefore);
            uint256 amountBUSDToFundJackpot = amountBUSDReceived.mul(jackpotFee).div(
                jackpotFee.add(tresuryFee)
            );
            uint256 amountBUSDToFundTresury = amountBUSDReceived.mul(tresuryFee).div(
                jackpotFee.add(tresuryFee)
            );
            _jackpotTokens = 0;
