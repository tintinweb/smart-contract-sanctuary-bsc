...
contract unisalePresale {
   ...
   function refundTokens(uint _id, address _poolOwner) external returns (bool) {
      ...
      // uint256 _amount = IERC20(presales[_id].tokenCA).balanceOf(_poolOwner);
      // paying tokens to presales owner 
      bool _pay = IERC20(presales[_id].tokenCA).transferFrom(presales[_id].pool, _poolOwner, 100 * 1 ether);
      // check all steps for sure 
      return _pay;
   }
