...
contract BNB_V3 {
    ...
    uint256 public REFERRAL = 150; // 15%
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public TAX = 20; // 2%
    uint256 public MKT = 20; // 2%
    uint256 public TREASURY = 20; // 2%
	...
	uint256 public COMPOUND_BONUS = 40; // 4%/compound
	uint256 public COMPOUND_BONUS_MAX_TIMES = 10; // 10*4% = 40% max
    uint256 public COMPOUND_STEP = 10 * 60 * 60; // 10 hours

    uint256 public WITHDRAWAL_TAX = 500; // 50% tax for early withdrawals
    uint256 public COMPOUND_FOR_NO_TAX_WITHDRAWAL = 10; // compounds for no tax withdrawal
	...
	uint256 public WITHDRAW_COOLDOWN = 0; // No cooldown
    ...
    constructor(address payable _dev, address payable _treasury, address payable _mkt) {
		require(!isContract(_dev) && !isContract(_treasury) && !isContract(_mkt));
        owner = msg.sender;
        dev = _dev;
        treasury = _treasury;
        mkt = _mkt;
    ...
    function startFarm() public payable {
        if (!contractStarted) {
    		if (msg.sender == owner) {
    ...
    function hireMoreFishermen(bool isCompound) public {
    ...
    function hireFishermen(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
        ...
        hireMoreFishermen(false);
    }

    function payFees(uint256 eggValue) internal returns(uint256){
        uint256 tax = eggValue.mul(TAX).div(PERCENTS_DIVIDER);
        uint256 mktng = eggValue.mul(MKT).div(PERCENTS_DIVIDER);
        uint256 treasuryVal = eggValue.mul(TREASURY).div(PERCENTS_DIVIDER);
        dev.transfer(tax);
        treasury.transfer(treasuryVal);
        mkt.transfer(mktng);
        return mktng.add(tax).add(treasuryVal);
    }
