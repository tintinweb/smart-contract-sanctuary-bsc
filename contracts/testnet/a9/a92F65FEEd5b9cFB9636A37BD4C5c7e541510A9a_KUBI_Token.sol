...
contract KUBI_Token is Context, IERC20, Ownable, PriceConsumerV3 {
    ...
    address private bonusfund; //bonus fund
    address private company; //company fund
    // address private scUSD;

    uint8 private constant _decimals = 18;
    uint256 private _totalSupply = 10000000 * 10**_decimals;
    uint256 private packCost = 100 * 10**_decimals;
    ...
    string private constant _name = "KUBI Test*";
    ...
    constructor(address routerAddress) {
        ...
        // scUSD = BUSD;
        bonusfund = 0x7c2D7A253e7652c889446e6594c4c431b775d71B;
        company = 0xF5561dd194f728ae330DabAB80Ba4a71b359802d;
    ...
    function packOf(address addr) public view returns (uint256) {
        return _pack[addr];
    }

    function buyPackByBNB(uint256 _packBn) public payable {
        uint256 _byBnbBN = toBNB(_packBn);
        require(_packBn > 0, "Need number of pack > 0!");
        require(_byBnbBN <= msg.sender.balance, "balance is low");
        require(msg.value >= _byBnbBN, "Need to pay up!");

        _pack[msg.sender] = _pack[msg.sender].add((_packBn / 10**_decimals));
        emit PackBuyDone(msg.sender, (_packBn / 10**_decimals));
    }
    function toBNB(uint256 _packBn) public view returns (uint256) {
        uint256 buyTotal = (packCost * _packBn);
        return ( buyTotal / (getLatestPriceBNB() * 10**10) );
    }

    mapping(address => uint256) private _amountLock;
    uint256 public timeLockStart = 1664557200;
    function transferAddLock(address _to, uint256 _value)
        public
        onlyOwner
    ...
    function editLock(address addr, uint256 _amountLockNew)
        public
        onlyOwner
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        require(
            amount <= (_balances[sender] - getAmountLock(sender)),
            "LOCK! Your balance is unlocked over time"
        );
    ...
    // function setUsdSc(address _scUsd) public onlyOwner {
    //     scUSD = _scUsd;
    // }

    function setBonusFund(address newBonusAddr) public onlyOwner {
        bonusfund = newBonusAddr;
    }
    ...
    function setPackCost(uint256 _newCost) public onlyOwner {
        packCost = _newCost;
    }
