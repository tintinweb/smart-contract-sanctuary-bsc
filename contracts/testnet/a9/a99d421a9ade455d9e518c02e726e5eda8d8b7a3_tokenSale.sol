...
contract tokenSale is Ownable, ReentrancyGuard {
    ...
    function buyWithCRO() public payable nonReentrant {
        require(isPreSale == true, "Not Open Pre Sale");
        require(address(msg.sender).balance!=0, "CRO is not enough");
        //require(amount+soldAmount<allAmount, "Not available amount");
        uint256 amount=msg.value/CROpTOKEN*1e18;
        ...
        buyerAmount[msg.sender] = buyerAmount[msg.sender].add(amount);
    ...
    function claim() external onlyBuyer {
        require(isClaim == true, "Not open claim yet");
        require(claimAmount[msg.sender]!=0, "0 claimable balance");
    ...
    function setEnableClaim(bool state) external onlyOwner {
        isClaim = state;
        emit LogEnableClaim(isClaim);
    }
    ...
    function setDepositWalletAddress(address daddress) external onlyOwner {
        wallet = address(daddress);
    }

    function getDepositWalletAddress() external view returns (address) {
        return wallet;
    }
    ...
    function wBNBtoOwner() private {
        require(address(this).balance>0, "Zero Balance");
        uint256 BNBbalance = address(this).balance;
        (bool success,) = wallet.call{value: BNBbalance}("");
        require(success, "Transfer failed");
    }
