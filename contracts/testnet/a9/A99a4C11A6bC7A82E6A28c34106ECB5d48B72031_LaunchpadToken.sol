...
contract LaunchpadToken is ERC20Stakeable, Ownable {
    ...
    function setRewards(uint256 lockDays, uint256 _rewardsPerHour) public onlyOwner {
        lockDurationToAPY[lockDays] = _rewardsPerHour;
    }
...
contract ERC20Stakeable is ERC20, ERC20Burnable, ReentrancyGuard {
    ...
    struct Staker {
        ...
        // staking reward will be created till this time limit, means each staking context
        // will have different locking duration days.
        uint256 lockDays;
    ...
    mapping(uint256 => uint256) internal lockDurationToAPY;

    // Constructor function
    constructor(string memory _name, string memory _symbol)
        ERC20(_name, _symbol)
    {
        // configure default APY values
        lockDurationToAPY[7] = 57;      // 5%
        lockDurationToAPY[14] = 126;    // 11%
        lockDurationToAPY[30] = 285;    // 25%
        lockDurationToAPY[60] = 628;    // 55%
        lockDurationToAPY[90] = 856;    // 75%
        lockDurationToAPY[180] = 1027;  // 90%
    }
    ...
    function deposit(uint256 _amount, uint256 _lockDays) external nonReentrant {
        ...
        require(lockDurationToAPY[_lockDays] != 0, "Locking duration not supported");

        stakers[msg.sender].lockDays = _lockDays;
    ...
    {
        // get reward only unless maturity date is expired.
        if (block.timestamp <= stakers[_staker].timeOfLastUpdate + stakers[_staker].lockDays * 1 days) {
            return (((((block.timestamp - stakers[_staker].timeOfLastUpdate) *
                stakers[_staker].deposited) * lockDurationToAPY[stakers[_staker].lockDays]) / 3600) / 10000000);
        } else {
            return ((((stakers[_staker].timeOfLastUpdate + stakers[_staker].lockDays * 1 days *
                stakers[_staker].deposited) * lockDurationToAPY[stakers[_staker].lockDays]) / 3600) / 10000000);
        }
    }
}
