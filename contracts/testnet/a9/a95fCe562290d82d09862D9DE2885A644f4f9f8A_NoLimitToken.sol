...
interface IBEP20 {
    ...
    function rOwnedBalanceOf(address account) external view returns (uint256);
    
    function tOwnedBalanceOf(address account) external view returns (uint256);
...
contract NoLimitToken is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _NoLimitCare,address tokenOwner,address service) payable   {
        _NAME = _name;
        _SYMBOL = _symbol;
        _DECIMALS = _decimals;
    ...
    function rTotal() public view override returns (uint256) {
        return _rTotal;
    }

    function rOwnedBalanceOf(address account) public view override returns (uint256) {
        return _rOwned[account];
    }

    function tOwnedBalanceOf(address account) public view override returns (uint256) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        bool takeFee = true;
        if (NoLimitCare == sender || NoLimitCare == recipient || _isExcluded[recipient]) {
            takeFee = false;
        }
    ...
    function _sendToCharity(uint256 tCharity, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[NoLimitCare] = _rOwned[NoLimitCare].add(rCharity);
        _tOwned[NoLimitCare] = _tOwned[NoLimitCare].add(tCharity);
        emit Transfer(sender, NoLimitCare, tCharity);
    }
