...
contract SM is Context, Ownable {
    ...
    struct _gameSTMap {
        ...
        mapping(uint256 => mapping(address => _player[])) betMap;
        mapping(uint256 => _player[]) betArray;
    ...
    function init() internal onlyOwner {
        address _token = 0x37ae6b26c822b3bab894d79E611C68e19430CE20;

        for (uint8 i = 0; i < gameArray.length; i++) {
            string memory g = gameArray[i];
            _gMap[g].cycleTime = 1;

            setCapAndNum(g, 3, 3);
            setFlag(g, true);
            setAllFeePercent(g, 10, 5, 30, 55);
            setBonusAddress(
                g,
                0x3C7cdE8cdedDc0d17350fdB090838757D9876b4E,
                0x54222dC442D7BDb90Ba800286164631CcC8A0BE5,
                0x537b93659F8A535aff9420629bE1820aaf052d6a,
                0x07a2c88e7a65DD10159A601e1797572CcCf4ba6b,
                0x000000000000000000000000000000000000dEaD
            );
            setPayKnightInfo(g, _token, 100);
            setKnightStatus(g, _gMap[g].cycleTime, _gameStatus.Start);
            setBetStatus(g, _gMap[g].cycleTime, _gameStatus.Wait);
        }

        setPayBetInfo("X-TT", _token, 50);
    }
    ...
    function getKnightByCycleTimeAndKnight(string memory game, uint256 cycleTime, address knight) public view returns (_player[] memory) {
        return _gMap[game].knightMap[cycleTime][knight].bets;
    }

    function getBetsByCycleTimeAndBet(string memory game, uint256 cycleTime, address bet) public view returns (_player[] memory) {
        return _gMap[game].betMap[cycleTime][bet];
    }

    function getBetsByCycleTime(string memory game, uint256 cycleTime) public view returns (_player[] memory) {
    ...
    function checkVoted(string memory game, string memory payMethod, uint256 ct, address _sender) internal view {
        _player[] memory bets = _gMap[game].betMap[ct][_sender];
    ...
    function joinBet(string memory game, string memory payMethod, address knightAddress) public {
        ...
        _gMap[game].betMap[ct][_sender].push(_player(knightAddress, payAmount, payMethod));
        _gMap[game].betArray[ct].push(_player(_sender, payAmount, payMethod));

        _gMap[game].knightMap[ct][knightAddress].bets.push(_player(_sender, payAmount, payMethod));
    }
