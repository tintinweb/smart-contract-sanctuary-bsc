...
contract Game is Ownable {
    ...
    function getHeroNftsByOwner(address _owner) external view returns(uint256[] memory) {
    ...
    function getHouseNftsByOwner(address _owner) external view returns(uint256[] memory) {
       uint256 count = _honseNFT.balanceOf(_owner);
       ...
       uint256 ownedTokenIndex = 0;
       while (ownedTokenIndex < count && currentTokenId < 30000) {
           if (_honseNFT.ownerOf(currentTokenId) == _owner) {
               result[ownedTokenIndex] = currentTokenId;
               ++ownedTokenIndex;
           }
           ++currentTokenId;
