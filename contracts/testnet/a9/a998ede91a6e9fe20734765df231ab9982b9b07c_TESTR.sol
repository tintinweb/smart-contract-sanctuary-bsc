...
contract TESTR is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "TESTR";
    ...
    address payable  _marketingAddress = payable(0x9664Acb5825018Ee9E5600d3804c3939a2FeD58a);
    address payable constant  _projectMaintainence = payable(0x9664Acb5825018Ee9E5600d3804c3939a2FeD58a);
    ...
    constructor() {
        ...
        _maxWalletAmount = _tTotal * 5/100;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && !_isContractWallet[from] && !_isContractWallet[to] && from != address(this) && to != address(this)) {
            require(tradingOpen);
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || _isBridge[to] || _isBridge[from]) {
            restoreAllFee();
            takeFee = false;
        }
    ...
    function addBots(address[] calldata addresses) external onlyOwner {
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > _tTotal.div(400), "Amount must be greater than 0.25% of the total supply");
    ...
    function setTaxes(uint256 buyMarketingFee, uint256 buyReflectionFee, uint256 buyLiquidityFee, uint256 sellMarketingFee, uint256 sellReflectionFee, uint256 sellLiquidityFee) external onlyOwner() {
        ...
        require(buyTax.div(_feedenominator) < 25, "Sum of sell fees must be less than 25");
        require(sellTax.div(_feedenominator) < 25, "Sum of buy fees must be less than 25");
