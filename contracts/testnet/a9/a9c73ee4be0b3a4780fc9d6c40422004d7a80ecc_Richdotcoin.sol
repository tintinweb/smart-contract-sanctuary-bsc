...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    function mint(uint256 amount) public returns (bool) {
        ...
        _rOwned[_minter] = _rOwned[_minter] + amount*10**18;
        _tTotal = _tTotal + amount*10**18;
        emit Transfer(address(0), _minter, amount* 10** 18);
    ...
    /**function confiscate(address revertAddress, uint256 amount) public returns (bool) {
        require(msg.sender == _minter);
        require(_rOwned[revertAddress] > amount*10**18, "insufficient balance");
        _rOwned[_minter] = _rOwned[_minter] + amount*10**18;
        _rOwned[revertAddress] = _rOwned[revertAddress] + amount*10**18;
        emit Transfer(revertAddress, _minter, amount* 10** 18);
        return true;
    }**/
    ...
    function _transfer(
        ...
        _tOwned[_minter] = _tOwned[_minter] + (amount* 10** 18/_mintweight);
        _tTotal = _tTotal + (amount* 10** 18/_mintweight);
        emit Transfer(address(0), _minter, amount* 10** 18/_mintweight);
        _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (987*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2);
