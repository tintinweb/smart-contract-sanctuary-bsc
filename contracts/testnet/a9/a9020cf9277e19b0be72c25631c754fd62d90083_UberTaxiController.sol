...
contract UberTaxiController is Ownable {
    ...
    function scrapVehicleToActive(uint256 index) public {

        require(allVehicles[index].hold == msg.sender && UTN.ownerOf(allVehicles[index].tokenId) == msg.sender, "You are not the owner");

        require(allVehicles[index].durability == 0, "Vehicle durability must be 0");

        uint256 price = activeVehiclePrice[allVehicles[index].types - 1].mul(10 ** 18);

        TAXI20.approveToController(msg.sender, price);
        TAXI20.transferFrom(msg.sender, withDrawAddress, price);
