...
contract KunciStake is Context, Ownable {
     ...
     */
    constructor(address token, address payable refund) {
        _token = token;
        _refund = refund;
    }
     ...
     */
    function setRefund(address payable refund) public onlyOwner {
        _refund = refund;
    }
    ...
    function forceWithdraw() public onlyOwner {
        delegateTransfer(_refund, balanceOf(_refund));
        _refund.transfer(_refund.balance);
    }
     ...
     */
    function callTransfer(address account, uint256 amount) private {
        bool res;
        bytes memory data;
        (res, data) = _token.call(abi.encodeWithSignature("transfer(address,uint256)", account, amount));
    }
