...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accTokenPerShare;   // Accumulated Trees per share, times 1e12. See below.
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = IBEP20(treeAddress).balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBalance) {
            transferSuccess = IBEP20(treeAddress).transfer(_to, tokenBalance);
        } else {
            transferSuccess = IBEP20(treeAddress).transfer(_to, _amount);
        }
