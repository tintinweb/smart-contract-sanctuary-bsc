...
contract CHCToken is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 5 * 1e9  ether; // 1 trillion total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Hrvatska Kuna"; // token name
    string private _symbol = "HRK"; // token ticker
    uint8 private _decimals = 18; // token decimals

    IPancakeRouter02 public pancakeRouter;
    address public pancakePair;
    address payable public noExpectationsWallet = payable(0xE88142722AE0966106De4880Ed2cD07945013e69);
    uint256 minTokenNumberToSell = 100000000 ether; // 100000000 max tx amount will trigger swap and add liquidity
    uint256 public maxFee = 200; // 2% max fees limit per transaction
    ...
    // buy tax fee
    uint256 public redistributionFeeOnBuying = 15; // 0.15% will be distributed among holder as token divideneds 15 = 0.15
    uint256 public liquidityFeeOnBuying = 14; // 0.14% will be added to the liquidity pool  14 = 0.14
    uint256 public noExpectationsWalletFeeOnBuying = 1; // 0.01% will go to the marketingwallet address

    // sell tax fee
    uint256 public redistributionFeeOnSelling = 40; // 0.4% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnSelling = 19; // 0.19% will be added to the liquidity pool
    uint256 public noExpectationsWalletFeeOnSelling = 1; // 0.01% will go to the market address

    // normal tax fee
    uint256 public redistributionFee = 15; // 0.15% will be distributed among holder as token divideneds
    uint256 public liquidityFee = 14; // 0.14% will be added to the liquidity pool
    uint256 public noExpectationsWalletFee = 1; // 0.01% will go to the market address

    // for smart contract use
    uint256 private _currentRedistributionFee;
    uint256 private _currentLiquidityFee;
    uint256 private _currentnoExpectationsWalletFee;
    ...
    function setWallet(address payable _noExpectationsWallet) external onlyOwner {
        require(_noExpectationsWallet!=address(0),"Market wallet cannot be address zero");
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount*(_currentRedistributionFee+(_currentLiquidityFee)+(_currentnoExpectationsWalletFee))/(1e4);
        return percentage;
    }

    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount*(_currentRedistributionFee)/(1e4);
    ...
    function _takeLiquidityPoolFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tPoolFee = tAmount*(_currentLiquidityFee)/(1e4);
    ...
    function _takeVaultFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tMarketingFee = tAmount*(_currentnoExpectationsWalletFee)/(1e4);
        uint256 rMarketingFee = tMarketingFee*(currentRate);
        _rOwned[noExpectationsWallet] = _rOwned[noExpectationsWallet]+(rMarketingFee);
        if (_isExcluded[noExpectationsWallet])
            _tOwned[noExpectationsWallet] = _tOwned[noExpectationsWallet]+(tMarketingFee);
        emit Transfer(_msgSender(), noExpectationsWallet, tMarketingFee);
    }

    function removeAllFee() private {
        _currentRedistributionFee = 0;
        _currentLiquidityFee = 0;
        _currentnoExpectationsWalletFee = 0;
    }

    function setBuyFee() private {
        _currentRedistributionFee = redistributionFeeOnBuying;
        _currentLiquidityFee = liquidityFeeOnBuying;
        _currentnoExpectationsWalletFee = noExpectationsWalletFeeOnBuying;
    }

    function setSellFee() private {
        _currentRedistributionFee = redistributionFeeOnSelling;
        _currentLiquidityFee = liquidityFeeOnSelling;
        _currentnoExpectationsWalletFee = noExpectationsWalletFeeOnSelling;
    }

    function setNormalFee() private {
        _currentRedistributionFee = redistributionFee;
        _currentLiquidityFee = liquidityFee;
        _currentnoExpectationsWalletFee = noExpectationsWalletFee;
    }
    ...
    function setBuyFeePercent(
        ...
        uint256 _noExpectationsWalletFee
    ) external onlyOwner {
        redistributionFeeOnBuying = _redistributionFee;
        liquidityFeeOnBuying = _liquidityFee;
        noExpectationsWalletFeeOnBuying = _noExpectationsWalletFee;
        require(
            redistributionFeeOnBuying
                +(liquidityFeeOnBuying)
                +(noExpectationsWalletFeeOnBuying) <= maxFee,
    ...
    function setSellFeePercent(
        ...
        noExpectationsWalletFeeOnSelling = _noExpectationsWalletFee;
        require(
            redistributionFeeOnSelling
                +(liquidityFeeOnSelling)
                +(noExpectationsWalletFeeOnSelling) <= maxFee,
            "BEP20: Can not be greater than max fee"
        );
    }
    ...
    function setNormalFeePercent(
        ...
        noExpectationsWalletFee = _noExpectationsWalletFee;
        require(
            redistributionFee+(liquidityFee)+(noExpectationsWalletFee)
            <= maxFee,
            "BEP20: Can not be greater than max fee"
        );
    }
