...
contract TokenSale is Ownable {
    ...
    IERC20 public token = IERC20(0xa9C5bC97636b902C7aEA9B6bd54671BD21efe091);
    ...
    function getCorrespondingTokens(uint256 _amount,uint256 _type) public view returns(uint256){
        ...
        uint256 recievingHVAM = (_amount * currPrice / price);
        return recievingHVAM;
    }
    ...
    function buyTokens(
        ...
        if (_type == 2) {
            ...
            depositToken.safeTransferFrom(
                msg.sender,
                investor,
                (amountToPay * investorDividend / 100)
            );
            depositToken.safeTransferFrom(
                msg.sender,
                admin,
                (amountToPay * ownerDividend / 100)
            );
            depositToken.safeTransferFrom(
                msg.sender,
                dev,
                (amountToPay * devDividend / 100)
            );
        }
   ...
   if(usersAddresses[_referrer]!=address(0))
        token.safeTransfer(usersAddresses[_referrer], _amount * refBonus / 100);
