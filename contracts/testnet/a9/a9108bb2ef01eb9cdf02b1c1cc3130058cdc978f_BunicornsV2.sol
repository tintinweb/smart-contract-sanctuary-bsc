...
contract BunicornsV2 is IBunicornsV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function getDetailEnhancedCounters(uint256 id) public view noFreshLookup(id) returns(uint8, uint8, uint8) {
        EnhancementCounters memory counters = enhancementCounters[id];
        return (counters.bronzeEnhanced, counters.sliverEnhanced, counters.goldEnhanced);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked("https://buni-stg-api.sotatek.works/buni-nft/bunicorns/", tokenId.toString()));
    }
    ...
    function unlockTransfer(uint256 tokenId) external {
        lastTransferTimestamp[tokenId] = block.timestamp.sub(TRANSFER_COOLDOWN_IN_SECONDS);
    }
}
