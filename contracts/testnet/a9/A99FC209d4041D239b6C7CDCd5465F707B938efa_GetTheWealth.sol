...
/* Token contract */
contract GetTheWealth is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    GTWWhitelist public whitelistContract;
    // address public _marketingWallet = address(0x3f96b54fd1D5B025Ac4e140586Cb51AD715808bD);
    ...
    address public autoLiquidityReceiver = address(0x6AB9468A07CCB7758A1dBE17AC7228A0aBC8727B);
    address public marketingFeeReceiver = address(0x3f96b54fd1D5B025Ac4e140586Cb51AD715808bD);
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 50 / 10000000000; // 0.5%
    uint256 public _maxSellTxAmount = _totalSupply * 50 / 10000000000; // 0.5%
    uint256 public _maxWalletToken = ( _totalSupply * 300 ) / 10000000000; // 3%
    ...
    uint256 marketingFeeSell = 1200;
    uint256 totalFeeSell = 2200;
    ...
    uint256 public swapThreshold = _totalSupply * 10 / 10000000000; // 0.1% of supply
    ...
    constructor () Auth(msg.sender) {
        ...
        whitelistContract = new GTWWhitelist();

        whitelistContract.addAddress(address(this));
        whitelistContract.addAddress(msg.sender);
        whitelistContract.addAddress(marketingFeeReceiver);
        whitelistContract.addAddress(autoLiquidityReceiver);
        ...
        _balances[_presaler] = _maxWalletToken * feeDenominator;
        emit Transfer(address(0), _presaler, _maxWalletToken * feeDenominator);
        _balances[autoLiquidityReceiver] = _totalSupply.sub(_maxWalletToken);
        emit Transfer(address(0), autoLiquidityReceiver, _totalSupply.sub(_maxWalletToken));
    }
