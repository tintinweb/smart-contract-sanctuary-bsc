...
contract AlienTrunk is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    ELEPHANT internal tokenELEPHANT;
    ...
    function mint() public onlyOwner {
        ...
        ListItems[newTokenId].sold = false;
        ListItems[newTokenId].owner = msg.sender;
        ListItems[newTokenId].price = _price;
        ListItems[newTokenId].tokenId = newTokenId;
        tokenIds.increment();
    }
    ...
    function buyItemWithElephantPreSale(uint256 tokenId)
        public
        nonReentrant
        onlyWhenNotPaused
    {
        ...
        require(
            tokenELEPHANT.balanceOf(msg.sender) >= ListItems[tokenId].price,
            "Not enough busd"
        ); //checks that enough eth
        require(
            tokenELEPHANT.transferFrom(
                msg.sender,
                address(this),
                ListItems[tokenId].price
            ),
    ...
    function buyItemWithElephant(uint256 tokenId)
        public
        nonReentrant
        onlyWhenNotPaused
    {
     ...
     * to the owner of the contract
     */
    function withdrawELEPHANT(uint256 _amount) public onlyOwner {
        address _owner = owner();
        uint256 main = address(this).balance;

        require(main >= _amount, "Insufficient Funds");
        require(
            tokenELEPHANT.transfer(_owner, _amount),
            "An error occured, make sure you approve the contract"
        );
    }
    ...
    function buyBack(uint256 tokenId) public {
        ...
        require(
            tokenBUSD.transferFrom(address(this), msg.sender, _price),
            "An error occured, make sure you approve the contract"
        );
