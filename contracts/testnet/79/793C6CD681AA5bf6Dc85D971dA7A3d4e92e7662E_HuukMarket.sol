    ...
    UUPSUpgradeable
{
    ...
    function __HuukMarket_init() internal initializer {
    ...
    function getRefData(address _user) private view returns (address payable) {
        address payable userRef = IHuukReferral(referralContract).getReferral(_user);
        return userRef;
    }
    ...
    function createOrder(
        ...
        uint256 balance;
        if (isERC721) {
            balance = (IERC721Upgradeable(_tokenAddress).ownerOf(_tokenId) == msg.sender) ? 1 : 0;
            ...
            balance = IERC1155Upgradeable(_tokenAddress).balanceOf(msg.sender, _tokenId);
            require(balance >= _quantity, 'Insufficient-token-balance');
            IERC1155Upgradeable(_tokenAddress).safeTransferFrom(msg.sender, address(this), _tokenId, _quantity, '0x');
        }
    ...
    function buy(
        ...
        if (_paymentToken == address(0) && msg.value > 0) {
            ...
            IERC20Upgradeable(_paymentToken).safeTransferFrom(msg.sender, address(this), exactPaymentAmount);
        }
    ...
    function createBid(
        ...
        uint256 _price, // price of 1 nft
        uint256 _expTime
    ) external whenNotPaused payable returns (uint256 _bidId) {
        require(_quantity > 0, 'Invalid-quantity');
        require(paymentMethod[_paymentToken], 'Payment-method-does-not-support');
        Bid memory newBid;
        ...
        returns (bool)
    {
        ...
        if (bid.paymentToken != address(0)) {
            IERC20Upgradeable(bid.paymentToken).safeTransferFrom(bid.bidder, address(this), exactPaymentAmount);
        }
    ...
    function updateOrder(
        ...
        if (_quantity > order.quantity && !order.isERC721) {
            IERC1155Upgradeable(order.tokenAddress).safeTransferFrom(msg.sender, address(this), order.tokenId, _quantity.sub(order.quantity), '0x');
    ...
    function _updateOrder(
        ...
        if (order.isERC721) {
            ...
            IERC1155Upgradeable(order.tokenAddress).safeTransferFrom(address(this), _buyer, order.tokenId, _quantity, abi.encodePacked(keccak256('onERC1155Received(address,address,uint256,uint256,bytes)')));
        }
        ...
        if (isHuukNFTs[order.tokenAddress]) {
            ...
            if (amountFirstSale[order.tokenAddress][order.tokenId] > 0 && (creator == order.owner)) {
                ...
                if (order.owner == creator) {
                    ...
                    IProfitEstimator(profitSender).profitToCreator(
                        order.tokenAddress,
                        _paymentToken,
                        order.tokenId,
                        _quantity,
