...
contract NFTSale is Ownable, IERC721Receiver, ReentrancyGuard, EIP712Base, BaseRelayRecipient {
    ...
    function transferWithTreasury(
        ...
        seed.safeTransferFrom(_from, _to, _amount - treasuryFee);
    }
     ...
     */
    function sellNFTBundle(uint256[] calldata _nftIds, uint256 price,uint256 expiryDuraation) external nonReentrant {
        ...
        uint256 expirytime=block.timestamp+expiryDuraation;

        Listing memory listing = Listing(listingId, _nftIds.length, _nftIds, price, _msgSender(), true, false,expiryDuraation,expirytime);
    ...
    function cancelListingBundle(uint256 _listingId) external nonReentrant {
        ...
        require(block.timestamp<listings[_listingId].expiryTime,"Listing already expired");
    ...
    function purchaseNFT(uint256 _listingId, uint256 _amount) external nonReentrant {
        ...
        require(listings[_listingId].expiryTime >block.timestamp,"Listing is expired");
