...
contract OgasaPredictionV3 is Ownable, Pausable, ReentrancyGuard {
    struct Round {
        uint256 epoch;
        uint256 bullAmount;
        uint256 bearAmount;
    ...
    uint256 internal constant minimumRewardRate = 90; // Minimum reward rate 90%
    uint256 public intervalSeconds = 180;
    ...
    event BufferAndIntervalSecondsUpdated(
    ...
    function _safeLockRound(
        ...
        require(
            ...
            "Can only lock round after lock timestamp"
        );
        require(
            block.timestamp <= Rounds[epoch].closeTimestamp,
            "Can only lock before end timestamp"
        );
    ...
    function changePriceSource(string memory _priceSource)
        external
        onlyOwner
    {
        require(
            bytes(_priceSource).length > 0,
            "Price source can not be empty"
        );
    ...
    function resume() public onlyOwnerOrOperator whenPaused {
        startOnce = false;
        lockOnce = false;
        _unpause();
    ...
    function roundLock(int256 price, uint32 timestamp)
        external
        onlyOwnerOrOperator
        whenNotPaused
    {
    ...
    function roundExecute(int256 price, uint32 timestamp)
        external
        onlyOwnerOrOperator
        whenNotPaused
    {
        ...
        require(
            Rounds[currentEpoch - 2].closeTimestamp != 0,
            "Can only start round after round n-2 has ended"
        );
        require(
            block.timestamp >= Rounds[currentEpoch - 2].closeTimestamp,
    ...
    function setRewardRate(uint256 _rewardRate) external onlyOwner {
        require(
            _rewardRate >= minimumRewardRate,
            "Reward rate can't be lower than minimum reward rate"
        );
        ...
        notContract
    {
        ...
        require(
            Bets[epoch][msg.sender].amount == 0,
        ...
        notContract
    {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            if (Rounds[epochs[i]].closed) {
                ...
                addedReward =
                    (Bets[epochs[i]][msg.sender].amount * round.rewardAmount) /
