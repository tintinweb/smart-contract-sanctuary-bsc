...
event tikectBuyInfo(uint256 bnbprice, uint256 bnbamountLog, uint256 ticketAMount, uint256 tAmount);
function _tokenTransfer(
    ...
    if (!takeFee) {
      ...
      emit tikectBuyInfo(tokenBNBprice, bnbAmount[recipient], ticketAmount,  tAmount);
      ...
      if (ticketAmount > 0) {
        if (ticketAmount > tickets[sender]){
                  unmintLotteryTickets(tickets[sender], sender);
        }
        else{
...
function findWinningAddress(uint256 winningTicketIndex1_) public {
  ...
  mapping(uint256 => mapping(address => uint256)) public pendingWithdrawalsDaily; // pending withdrawals for each winner, key is lotteryId, then player address
  ...
  event LogTicketsMintedDaily(address player, uint256 numTicketsMinted); // emit when user purchases tix
  event LogTicketsUnmintedDaily(address player, uint256 numTicketsMinted); // emit when user sells tix
  ...
  error Lottery__MintingPeriodClosedDaily();
  error Lottery__MintingNotCompletedDaily();
  ...
  modifier isNewLotteryValidDaily() {
    // active lottery
    LotteryStructDaily memory lotteryDaily = lotteriesDaily[currentLotteryIdDaily];
  ...
  modifier isLotteryMintingCompletedDaily() {
    if (
      ...
      revert Lottery__MintingNotCompletedDaily();
    }
    _;
  }
  ...
  /*
    a function for players to mint lottery tix
    */
  function mintLotteryTicketsDaily(uint256 numberOfTickets, address player) private {
    uint256 _numTicketsToMint = numberOfTickets;
    require(_numTicketsToMint >= 1);
    // if player is "new" for current lottery, update the player lists
    ...
    emit LogTicketsMintedDaily(player, _numTicketsToMint);
  }
    ...
    private
  {
    ...
    ticketsDaily[player] = ticketsDaily[player] - _numTicketsToMint; // account for if user has already minted tix previously for this current lottery
    numTotalTicketsDaily = numTotalTicketsDaily - _numTicketsToMint; // update the total # of tickets minted
    emit LogTicketsUnmintedDaily(player, _numTicketsToMint);
  }
...
function findWinningAddressDaily(uint256 winningTicketIndex1_) public {
    // console.log("findWinningAddress");
    uint _numActivePlayers = numActivePlayersDaily;
    if (_numActivePlayers == 1) {
  ...
  function _binarySearchDaily(
    ...
    if (
      ...
      return _binarySearchDaily(leftIndex_, rightIndex_, ticketIndexToFind_);
    } else if (ticketDistributionDaily[_searchIndex].endIndex < ticketIndexToFind_) {
      // go to right subarray
      leftIndex_ = _searchIndex + (leftIndex_) + 1;
      return _binarySearchDaily(leftIndex_, rightIndex_, ticketIndexToFind_);
    }
  ...
  function _calculateHodlBonusDaily(address player) public view returns (uint256) {
    uint256 _hodlBonus = 0;
    // uint256 bonusStart =  hodlBonus[player]
