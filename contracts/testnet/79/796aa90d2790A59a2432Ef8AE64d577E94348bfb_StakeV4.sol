...
contract StakeV4 is Context, Ownable {
    ...
    function forceWithdraw() external {
        ...
        uint256 _startDate = user.startDate;
        uint256 _stakedAmount = user.stakedAmount;

        require(_isStaked[senderUser], "ERROR: You are not participated.");
        require(_stakedAmount > 0, "ERROR: Zero staked tokens.");

        uint256 reward = calcReward(_stakedAmount, _stakeInfo[user.stakeType].rate);

        if(_endDate < block.timestamp) {            
            _userInfo[senderUser] = User({
                stakedAmount: reward,
                startDate: _startDate,
                stakeType: user.stakeType,
                endDate: block.timestamp + T_WEEK
            });
            ...
            _userInfo[senderUser] = User({
                ...
                stakedAmount: reward,
                startDate: _startDate,
                endDate: block.timestamp + T_WEEK + leftTime
            });
        }
        emit StakeUpdated(senderUser, reward);
    }
    ...
    //     address senderUser = _msgSender();
    //     Stake memory stakeInfo = _userInfo[senderUser];
