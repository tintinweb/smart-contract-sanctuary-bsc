...
contract TAP is Ownable {
    ...
    function getID(address) public view returns (uint256[] memory) {
        return InvestorAddress[msg.sender];
    }
    ...
    function buy(uint256 amount) public payable {
        require(
            ...
            "Need to exact amount of ether"
        );
        InvestorId++;
        InvestorAddress[msg.sender].push(InvestorId);
        balances[msg.sender] += amount;
        MyToken.transferPrice(owner(), msg.sender, amount);
        IDD[InvestorId] = (amount * 1);
        tokenPrice = tokenPrice + (amount * 1);
    }

    function sell(uint256 amount, uint256 id) public {
        balances[msg.sender] -= amount;
        payable(msg.sender).transfer(amount * (tokenPrice - IDD[id]));
