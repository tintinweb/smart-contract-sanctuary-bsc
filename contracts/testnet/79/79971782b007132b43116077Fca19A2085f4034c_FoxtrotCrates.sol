...
abstract contract CrateBundle is ICrateBundle {
	...
	function getUriOfBundle(uint256 _crateId) public view returns (string memory) {
	...
	function getCrateTypeOfBundle(uint256 _crateId) public view returns (CrateType) {
		require(
			...
			"FXDC: Not exist rarity on crate"
		);
	...
	function getProbabilityOfCrate(uint256 _crateId) external view returns (uint256[][] memory) {
		...
		)
	{
		...
		Token[] memory commonItems = new Token[](disembledRarities[0]);
		Token[] memory uncommonItems = new Token[](disembledRarities[1]);
		Token[] memory rareItems = new Token[](disembledRarities[2]);
		Token[] memory epicItems = new Token[](disembledRarities[3]);
		Token[] memory legendaryItems = new Token[](disembledRarities[4]);
		uint256[] memory arrCountWrapper = new uint256[](5);

		for (uint256 i = 0; i < _totalItems; i += 1) {
			Token memory item = crate[_crateId].tokens[i];
			Rarity rarity = item.rarity;

			if (rarity == Rarity.COMMON) {
				commonItems[arrCountWrapper[0]] = item;
				arrCountWrapper[0] += 1;
			} else if (rarity == Rarity.UNCOMMON) {
				uncommonItems[arrCountWrapper[1]] = item;
				arrCountWrapper[1] += 1;
			} else if (rarity == Rarity.RARE) {
				rareItems[arrCountWrapper[2]] = item;
				arrCountWrapper[2] += 1;
			} else if (rarity == Rarity.EPIC) {
				epicItems[arrCountWrapper[3]] = item;
				arrCountWrapper[3] += 1;
			} else if (rarity == Rarity.LEGENDARY) {
				legendaryItems[arrCountWrapper[4]] = item;
				arrCountWrapper[4] += 1;
			}
		}
		itemData[0] = commonItems;
		itemData[1] = uncommonItems;
		itemData[2] = rareItems;
		itemData[3] = epicItems;
		itemData[4] = legendaryItems;
		return itemData;
	}
	...
	) public view virtual returns (Rarity) {
	...
	function _createBundle(
		...
		require(crate[_crateId].count == 0, "id exists");
	...
	function _saveProbabilitiesAtCreation(uint256 _crateId, uint256[][] memory _probabilities)
		internal
	{
		if (_probabilities[0].length > 0 && _probabilities[1].length > 0) {
	...
	function _updateTokenInBundle(
		...
		require(_index < crate[_crateId].count, "index DNE");
		_checkTokenType(_tokenToBind);
	...
	function _deleteBundle(uint256 _crateId) internal {
		for (uint256 i = 0; i < crate[_crateId].count; i += 1) {
			delete crate[_crateId].tokens[i];
		}
...
// Helper interfaces
import { IWETH } from "contracts/base/interfaces/IWETH.sol";
...
{
	address private constant CHAIN_NATIVE_TOKEN =
	...
	function _releaseTokens(address _recipient, uint256 _idForContent)
		internal
	{
...
import "contracts/base/random/interface/IRandomManager.sol";
	...
	TokenStore
{
	...
	/// @notice Pack Creation by Rarity and Probabilities
	function createCrateByRarity(
		...
		onlyRole(minterRole)
		nonReentrant
		returns (uint256 crateId, uint256 packTotalSupply)
	{
		require(_items.length > 0, "FXDCrates: No items");

		crateId = nextTokenIdToMint;
		nextTokenIdToMint += 1;
		packTotalSupply = _totalCrates;

		packInfo[crateId].openStartTimestamp = _openStartTimestamp;
		packInfo[crateId].amountDistributedPerOpen = _rewardsPerCrate;
		packInfo[crateId].crateType = CrateType.RARITY_CRATE;

		_mint(_msgSender(), crateId, packTotalSupply, "");
		_storeTokens(
			_msgSender(),
			_items,
			_crateUri,
	...
	function openPackAndClaimRewards(uint256 _crateId, uint256 _amountToOpen)
		external
		returns (uint256)
	{
	...
	function openPack(uint256 _crateId, uint256 _amountToOpen) external returns (uint256) {
		uint32 requestedRandoms = 2;
		return
			_requestOpenPack(
				CrateType.RARITY_CRATE,
		...
		override
	{
		...
		require(info.randomWords.length == 0, "FXDC: VRF Already fulfilled");
		...
		if (info.openOnFulfillRandomness) {
			if (info.crateType == CrateType.RARITY_CRATE) {
				try
					FoxtrotCrates(payable(address(this))).sendRewardsIndirect(info.opener)
				{} catch {}
			}
		}
	}
	...
	function _requestOpenPack(
		...
		require(isTrustedForwarder(msg.sender) || opener == tx.origin, "FXDC: Invalid !EOA");
		require(openerToReqId[opener] == 0, "FXDC: VRF Fail");
		require(
			_amountToOpen > 0 && balanceOf(opener, _crateId) >= _amountToOpen,
			"FXDC: Out of Balance"
		);
		require(
			packInfo[_crateId].openStartTimestamp <= block.timestamp,
			"FXDC: Crate is not available"
		);
	...
	// TODO: Add batch crate open
	function openRarityCrate(address opener) internal {
		require(canCollectRewards(opener), "!ActiveReq");
		uint256 requestId = openerToReqId[opener];

		RequestInfo memory info = requestInfo[requestId];
		uint256 crateId = info.crateId;

		PackInfo memory pack = packInfo[crateId];

		uint256[] memory rarityOfCratePacked = _calculateRarities(crateId);
		uint256 totalItemsInCrate = getTokenCountOfBundle(crateId);

		Rarity[] memory pickedRarities;
		for (uint256 i = 0; i < pack.amountDistributedPerOpen; i++) {
			pickedRarities[i] = _pickRarityByProbability(
				crateId,
				uint256(keccak256(abi.encode(info.randomWords[0], i))) % 100 ether,
				0
			);
		}
		...
		Token[][] memory items = _filterRaritiesAndClassifiedIt(
			crateId,
			rarityOfCratePacked,
			totalItemsInCrate
		);

		Token[] memory itemsToSendToOpener;
		for (uint256 i = 0; i < pack.amountDistributedPerOpen; i++) {
			uint256 random = uint256(keccak256(abi.encode(info.randomWords[1], i))) % 100 ether;
			uint256 raritySelector = 0/* uint256(pickedRarities[i]) */;
			itemsToSendToOpener[i] = items[raritySelector][random % items[raritySelector].length];
		}
		...
		_transferTokenBatch(address(this), opener, itemsToSendToOpener);

		emit CrateOpened(info.crateId, opener, info.amountToOpen, itemsToSendToOpener);
	}

	function sendRewardsIndirect(address _opener) external {
		require(msg.sender == address(this), "FXDC: Only contract can interact");
		uint256 requestId = openerToReqId[_opener];
		RequestInfo memory info = requestInfo[requestId];

		if (info.crateType == CrateType.RARITY_CRATE) {
			openRarityCrate(_opener);
		}
	}
	...
	// function createBundleByRarityWithSecureItems()
	// function createBundleByPosibilities

	/**
	 * Opens a bundle that pre-exists, burn the pack and use VRF to send the objects to the user
	 */

	/* function openRarityCrate(uint256 _requestId, uint256[] memory random)
        internal
    {
        //require(random.length > 6, 'FXDCrate: Random incomplete');
        // execution path A
        uint256 crateId = crateIdSaveByRequestId[_requestId];
        //address sender = requestToSender[_requestId];

        uint256 _items = CrateManager.crate[crateId].count;

        // Calculate rarity values
        uint256[] memory arrCount = new uint256[](5);

        for (uint256 i = 0; i < _items; i += 1) {
            uint256 rarity = uint256(
                CrateManager.crate[crateId].items[i].itemRarity.rarity
            );
            if (rarity == uint256(Rarity.COMMON)) {
                arrCount[0] += 1;
            } else if (rarity == uint256(Rarity.UNCOMMON)) {
                arrCount[1] += 1;
            } else if (rarity == uint256(Rarity.RARE)) {
                arrCount[2] += 1;
            } else if (rarity == uint256(Rarity.EPIC)) {
                arrCount[3] += 1;
            } else if (rarity == uint256(Rarity.LEGENDARY)) {
                arrCount[4] += 1;
            }
        }
        Item[] memory commonItem = new Item[](arrCount[0]);
        Item[] memory uncommonItem = new Item[](arrCount[1]);
        Item[] memory rareItem = new Item[](arrCount[2]);
        Item[] memory epicItem = new Item[](arrCount[3]);
        Item[] memory legendaryItem = new Item[](arrCount[4]);
        uint256[] memory arrCountWrapper = new uint256[](5);

        for (uint256 i = 0; i < _items; i += 1) {
            Item memory item = crate[crateId].items[i];
            uint256 rarity = uint256(item.itemRarity.rarity);

            if (rarity == uint256(Rarity.COMMON)) {
                commonItem[arrCountWrapper[0]] = item;
                arrCountWrapper[0] += 1;
            } else if (rarity == uint256(Rarity.UNCOMMON)) {
                uncommonItem[arrCountWrapper[1]] = item;
                arrCountWrapper[1] += 1;
            } else if (rarity == uint256(Rarity.RARE)) {
                rareItem[arrCountWrapper[2]] = item;
                arrCountWrapper[2] += 1;
            } else if (rarity == uint256(Rarity.EPIC)) {
                epicItem[arrCountWrapper[3]] = item;
                arrCountWrapper[3] += 1;
            } else if (rarity == uint256(Rarity.LEGENDARY)) {
                legendaryItem[arrCountWrapper[4]] = item;
                arrCountWrapper[4] += 1;
            }
        }

        //Pick and fill array with the picked rarities
        uint256[] memory rarityPicked = new uint256[](_items);
        for (
            uint256 i = 0;
            i < CrateManager.crate[crateId].maxRewardPerCrate;
            i += 1
        ) {
            Rarity itemRarity = _pickItemByRarity(
                crateId,
                random[i] % 100 ether,
                0
            );
            rarityPicked[i] = uint256(Rarity(itemRarity));
        }
        console.log(
            "Length: %s",
            commonItem[random[4] % commonItem.length].tokenId,
            rarityPicked[1],
            rarityPicked[2]
        );

        sendItems(crateId, random, commonItem);

        //emit CrateOpenedWith(rarityPicked);
    } */

	/* function sendItems(
        uint256 crateId,
        uint256[] memory random,
        Item[] memory commonItem
    ) internal {
        for (
            uint256 i = 0;
            i < CrateManager.crate[crateId].maxRewardPerCrate;
            ...
            IERC1155(CrateManager.crate[crateId].items[0].assetContract)
                .safeTransferFrom(
                    address(this),
                    requestToSender[crateId],
                    commonItem[random[4] % commonItem.length].tokenId,
                    1,
                    ""
                );
	...
	//FIRST CALL
	/**
	 * Flow: RequestRandomNumber -> FulfillRandomWords ->
	 */
