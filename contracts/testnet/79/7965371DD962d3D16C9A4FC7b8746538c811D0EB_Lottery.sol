...
contract Lottery is ChainlinkHelper, Ownable, Math {
    ...
    event FinishDrop(uint256 indexed timestamp, address indexed user, uint256 dropId);
    event GiveTickets(
        uint256 indexed timestamp,
        address indexed user,
        uint256 dropId,
    ...
    event OwnerFactoryNFT(
        uint256 indexed timestamp,
        address indexed user,
        address newOwner
    );
    event OwnerNFT(uint256 indexed timestamp, address indexed user, address newOwner);
    ...
    function claimTickets() external {
        // TODO: do claim tickets with whitelist and staked in staking
    }
    ...
    function buyTickets(
        ...
        uint256 ticketPrice = getPrice(dropId, token, drop.settings.ticketsPrices);
    ...
    ) public view dropExists(dropId) returns (uint256 ticketPrice) {
        Drop memory drop = drops[dropId];

        if (token == address(nativeToken)) ticketPrice = prices.native;
        else if (token == address(stableToken)) ticketPrice = prices.stable;
        else if (token == drop.partnerToken) ticketPrice = prices.partner;
        else revert();
    }
    ...
    function claimReward(uint256 dropId, address token) external isFinished(dropId) {
        require(hasReward(dropId, msg.sender));

        Drop storage drop = drops[dropId];
        uint256 nftPrice = getPrice(dropId, token, drop.settings.nftsPrices);
    ...
    {
        Drop memory drop = drops[dropId];
        uint256 tickets_ = tickets[dropId][user];
        ...
        onlyOwner
    {
        ...
        emit RequestFinishDrop(block.timestamp, msg.sender, dropId, requestId);
    }
    ...
    function _finishDrop(uint256 dropId, uint256 randomness) internal {
        ...
        emit FinishDrop(block.timestamp, msg.sender, dropId);
    }
    ...
    function _giveTickets(
        ...
        address user,
        uint256 amount
    ) internal dropExists(dropId) {
        tickets[dropId][user] += amount;
        emit GiveTickets(block.timestamp, user, dropId, amount);
    }
    ...
    function setNativeToken(address nativeToken_) external onlyOwner {
        nativeToken = IERC20(nativeToken_);
        emit SetNativeToken(block.timestamp, msg.sender, nativeToken_);
    }
    ...
    function setStableToken(address stableToken_) external onlyOwner {
        stableToken = IERC20(stableToken_);
        emit SetStableToken(block.timestamp, msg.sender, stableToken_);
    }
    ...
    function newOwnerFactoryNFT(address newOwner) external onlyOwner {
        ...
        emit OwnerFactoryNFT(block.timestamp, msg.sender, newOwner);
    }
    ...
    function newOwnerNFT(NFT nft, address newOwner) external onlyOwner {
        ...
        emit OwnerNFT(block.timestamp, msg.sender, newOwner);
    }
}
