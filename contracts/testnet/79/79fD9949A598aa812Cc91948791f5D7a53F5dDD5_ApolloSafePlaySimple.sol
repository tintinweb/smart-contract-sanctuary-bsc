...
// File contracts/ApolloSafePlayMultiCurrency/ApolloSafePlaySimple.sol
...
/**
 * Apollo safe play token with only buyBack and without any other taxes
 */
...
contract ApolloSafePlaySimple is Context, IERC20Metadata, Ownable {
    // Token parameters
    string private NAME;
    ...
    function _getValues(
        ...
        if (!takeFee || (!isBuy && !isSell)) {
            ...
            values.tTransferAmount = tAmount - values.tBuyBack;
        } else if (isSell) {
    ...
    function _takeBuyBack(uint256 tBuyBack) private {
        totalFeesPaid += tBuyBack;
        pendingToPay += tBuyBack;
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        if (pendingToPay > 0) {
            if (block.chainid == 56 || block.chainid == 97) {
                swapTokensForETH(pendingToPay, address(this));
                pendingToPay = balanceOf(address(this));
            } else {
                _transfer(address(this), buyBackAddress, pendingToPay);
