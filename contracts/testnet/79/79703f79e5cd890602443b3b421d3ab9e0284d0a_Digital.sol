...
interface IGNFT {
    function storeInfo(address _to, string memory uri) external payable;
}
...
contract Digital is IGNFT, ERC721Enumerable, ERC721URIStorage, IERC2981, Ownable {
    using Strings for uint256;
    ...
    constructor(address _owner) ERC721("DIGITAL_FORENSIC_FRAMEWORK", "DFF")  {
    ...
    function storeInfo(address _to, string memory uri) override external payable  {
        require(totalSupply() + 1 <= maxSupply, "NFT: out of stock");
        require(_to != address(0), "NFT: invalid address");
        require(msg.value != 0, "NFT: Value should not be zero");
    ...
    function tokensOfOwner(address _owner) public view onlyOwner returns (uint256[] memory) {
    ...
    function exists(uint256 _id) external view onlyOwner returns (bool) {
        return _exists(_id);
    }
