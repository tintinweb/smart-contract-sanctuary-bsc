...
contract ConvexNode is ERC20, Ownable, PaymentSplitter {
    ...
    uint256 public nodeMaintenanceFee; // Node maintenance fee
    ...
    modifier isFeePaid(address _user) {
        NodeFee storage _nodeMaintenanceFee = _userNodeFees[_user];
        require(
            block.timestamp < _nodeMaintenanceFee.nextPaymentDate,
    ...
    function setNodeMaintenanaceFee(uint256 _fee) external onlyOwner {
        nodeMaintenanceFee = _fee;
    }
    ...
    function createNodeWithTokens(
        ...
        NodeFee storage _nodeMaintenanceFee = _userNodeFees[msg.sender];
        if (_nodeMaintenanceFee.lastPaidDate == 0) {
            _nodeMaintenanceFee.lastPaidDate = block.timestamp;
            _nodeMaintenanceFee.nextPaymentDate = block.timestamp + 30 days;
        }
    }

    function cashoutReward(uint256 blocktime, uint256 nodeIndex)
        public
        isFeePaid(msg.sender)
    {
        INODERewardManagement nodeRewardManager = getNodeRewardManager(
            nodeIndex
        );
    ...
    function cashoutAll(uint256 nodeIndex) public isFeePaid(msg.sender) {
        INODERewardManagement nodeRewardManager = getNodeRewardManager(
            nodeIndex
        );
        address sender = _msgSender();
    ...
    function payNodeMaintenanceFee() public payable {
        NodeFee storage nodeFee = _userNodeFees[msg.sender];
        require(
            nodeFee.nextPaymentDate != 0 &&
                block.timestamp >= nodeFee.nextPaymentDate,
            "PAYFEE: fee already paid"
        );
        uint256 feeAmount = calculateNodeFee(msg.sender);
        require(msg.value >= feeAmount, "PAYFEE: Not enough to pay fee");
        nodeFee.lastPaidDate = block.timestamp;
        nodeFee.nextPaymentDate += block.timestamp;
        nodeFee.lastPaidFee = msg.value;
    }

    function calculateNodeFee(address _user) public view returns (uint256) {
        NodeFee storage nodeFee = _userNodeFees[msg.sender];

        uint256 duration = (block.timestamp - nodeFee.lastPaidDate) / 30 days;

        (uint256 node1, uint256 node2, uint256 node3) = getNodeNumberOf(_user);
        uint256 _fee = ((node1 + node2 + node3) * nodeMaintenanceFee).div(
            10e18
        );
        return _fee * duration;
    }
    ...
    function updatePriceFee(AggregatorV3Interface _feed) external onlyOwner {
        priceFeed = _feed;
    }
