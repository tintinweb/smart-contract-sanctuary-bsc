...
interface IYiBoxSetting {
    function calcOpenBox() external returns (uint256,uint256, uint32,uint256);
...
contract HashratePool is Ownable {
    ...
    function openBox(uint256 _num) external lock returns(uint256[] memory _tks, uint8[] memory _lvs, uint32[] memory _hrs, uint16[] memory _tys) {
        ...
        for (uint i = 0; i < _num; i++) {
            uint256 _tarToken;
            (uint256 _lv, uint256 _ty, uint32 _idx,) = IYiBoxSetting(YiSetting).calcOpenBox();
            uint32 _hr = uint32(_lv);
            if (_lv <= 3) {
                _tarToken = 1 + (_lv << 16) + (_ty << 32);
                IYiBoxHeroNFT(HeroAddress).safeTransferFrom(address(this), _msgSender(), _tarToken, 1, "");
            } else {
                _tarToken = (_lv * 10 ** 20) + (_ty * (10 ** 16)) + _idx;
                IYiBoxNFT(NFTToken).transferFromInternal(mainPool, _msgSender(), _tarToken);
                 (, , _hr, ,) = IYiBoxNFT(NFTToken).tokenBase(_tarToken);
