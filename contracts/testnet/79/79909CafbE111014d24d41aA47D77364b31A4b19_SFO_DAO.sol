...
contract SFO_DAO is Ownable {
    ...
    function daoEdit(
        ...
        require(councliAt(daoAt, msg.sender) != noneExist, "forbidden");
    ...
    function daoCouncliList(uint16 daoAt) public view returns (Councli[] memory) {
    ...
    struct Proposal {
        ...
        // string name;0
        // string desc;1
        // uint8 content;0 // 10:普通提案;20:重要提案;30:核心提案
        // uint8 label;1 // 10:决策相关;20:发展相关;30:决策相关
        // uint256 reward;2
        // uint32 startTime;3
        // uint32 endTime;4
        string[] name_desc;
    ...
    mapping(uint16 => mapping(uint16 => Vote[])) private proposalVotes; // todo:查看?建议使用length:offset+size
    ...
    function prosalList(uint16 daoAt) public view returns (Proposal[] memory) {
    ...
    function userDaoList(address addr) public view returns (bool[] memory) {
        ...
        if (lp) {
            ...
            proposalVotes[daoAt][proposalAt].push(
                Vote({
                    ...
                    tokenAmount: (totalAmount * lpPrice) / 1e18,
                    options: options,
                    amounts: amounts,
                    status: true
                })
    ...
    function userVoteRecordQuery(address addr) public view returns (uint256[] memory) {
