...
contract Fusion is OwnableUpgradeable, ERC721Holder {
    ...
    uint256 constant GROWTH_CAP = 3000000;

    Seedmon public seedmon;
    event Factor(uint256 factor);
    ...
    function testMintSeedmon(uint256 mainTokenId, uint256 materialTokenId, address to) external {  
        ...
        uint256 star = mainUniqueProfile.numberOfFused.mul(TOTAL_ALLOCATION);
        if (star == 0) {
            star = 100;
        }
        ...
        uint256 factor = GROWTH_CAP.mul(star.div(400)).add(100).div(sumOfPendingBonusStat).div(TOTAL_ALLOCATION);

        SeedmonAttribute.Stat memory finalStat;
        finalStat.vitality = mainStat.vitality.add(pendingStat.vitality.mul(factor).div(TOTAL_ALLOCATION));
        finalStat.strength = mainStat.strength.add(pendingStat.strength.mul(factor).div(TOTAL_ALLOCATION));
        finalStat.smart = mainStat.smart.add(pendingStat.smart.mul(factor).div(TOTAL_ALLOCATION));
        finalStat.speed = mainStat.speed.add(pendingStat.speed.mul(factor).div(TOTAL_ALLOCATION));
        finalStat.dexterity = mainStat.dexterity.add(pendingStat.dexterity.mul(factor).div(TOTAL_ALLOCATION));
        finalStat.luck = mainStat.luck.add(pendingStat.luck.mul(factor).div(TOTAL_ALLOCATION));
