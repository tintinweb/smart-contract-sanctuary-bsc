 ...
 * 
 *  - Users stake Pancakeswap LP Tokens to receive WBNB and KNB Tokens as Rewards
 * 
 ...
 * 
 *  - Users deposit Set (Predecided) pancakeswap LP Tokens and get a share of the farm
 * 
 ...
 *      - KNB-WBNB
 *      - KNB-WBTC-WBNB (assumes appropriate liquidity is available in WBTC-WBNB pair)
 *      - KNB-USDT-WBNB (assumes appropriate liquidity is available in USDT-WBNB pair)
 *      - KNB-USDC-WBNB (assumes appropriate liquidity is available in USDC-WBNB pair)
 * 
 *  - Any swap may not have a price impact on KNB price of more than approx ~2.49% for the related KNB pair
 *      KNB-WBNB swap may not have a price impact of more than ~2.49% on KNB price in KNB-WBNB pair
 *      KNB-WBTC-WBNB swap may not have a price impact of more than ~2.49% on KNB price in KNB-WBTC pair
 *      KNB-USDT-WBNB swap may not have a price impact of more than ~2.49% on KNB price in KNB-USDT pair
 *      KNB-USDC-WBNB swap may not have a price impact of more than ~2.49% on KNB price in KNB-USDC pair
 * 
 *  - After the swap,converted WBNB is distributed to stakers at pro-rata basis, according to their share of the staking pool
 *    on the moment when the WBNB distribution is done. And remaining KNB is added to the amount to be distributed or burnt.
 *    The remaining KNB are also attempted to be swapped to WBNB in the next swap if the price impact is ~2.49% or less
 * 
 ...
 * 
 *  - Users may claim their pending WBNB and KNB anytime
 * 
 ...
 * 
 *  - Owner may transfer out WBNB and KNB Tokens from this contract once `adminClaimableTime` is reached
 * 
 *  - CONTRACT VARIABLES must be changed to appropriate values before live deployment
 */
contract FarmKNB is Ownable {
    ...
    // these contracts (and pancakeswap pair & router) are "trusted" 
    // and checked to not contain re-entrancy pattern
    // to safely avoid checks-effects-interactions where needed to simplify logic
    address public constant trustedDepositTokenAddress = 0x68E10BbA2c27F7aB07e3D65646d6f4bc2DB02ad9; //PancakePair contract address   bscscan testnet (testnet address)
    address public constant trustedRewardTokenAddress = 0x1BEb56FEcD26c1Ecf4BfEBbE5112D16CD4414331; // KNB token bscscan testnet address
    ...
    IpancakeswapV2Router02 public pancakeswapRouterV2;
    ...
    constructor(address[] memory swapPath) public {
        ...
        pancakeswapRouterV2 = IpancakeswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //PancakeRouter Address (Testnet address)
        pancakeswapV2Pair = IpancakeswapV2Pair(trustedDepositTokenAddress);
        SWAP_PATH = swapPath;
    }
    ...
    function updateAccount(address account) private {
        ...
        if (pendingDivsEth > 0) {
            require(Token(pancakeswapRouterV2.WETH()).transfer(account, pendingDivsEth), "Could not transfer WETH!");
    ...
    // view function to check last updated WBNB pending rewards
    ...
    // private function to distribute WBNB rewards
    ...
    function doSwap() private {
        ...
        require(Token(trustedRewardTokenAddress).approve(address(pancakeswapRouterV2), _tokensToBeSwapped), 'approve failed!');
    
        uint oldWBNBBalance = Token(pancakeswapRouterV2.WETH()).balanceOf(address(this));
                
        uint amountOutMin;
        
        uint estimatedAmountOut = pancakeswapRouterV2.getAmountsOut(_tokensToBeSwapped, SWAP_PATH)[SWAP_PATH.length.sub(1)];
        amountOutMin = estimatedAmountOut.mul(uint(100e2).sub(SLIPPAGE_TOLERANCE_X_100)).div(100e2);
        
        pancakeswapRouterV2.swapExactTokensForTokens(_tokensToBeSwapped, amountOutMin, SWAP_PATH, address(this), block.timestamp);
    
        uint newWBNBBalance = Token(pancakeswapRouterV2.WETH()).balanceOf(address(this));
        uint WBNBReceived = newWBNBBalance.sub(oldWBNBBalance);
        require(WBNBReceived >= amountOutMin, "Invalid SWAP!");
        
        if (WBNBReceived > 0) {
            distributeDivsEth(WBNBReceived);    
        }
    ...
    function transferAnyERC20Token(address _tokenAddr, address _to, uint _amount) public onlyOwner {
        ...
        require((_tokenAddr != trustedRewardTokenAddress && _tokenAddr != pancakeswapRouterV2.WETH()) || (now > adminClaimableTime), "Admin cannot Transfer out Reward Tokens or WBNB Yet!");
