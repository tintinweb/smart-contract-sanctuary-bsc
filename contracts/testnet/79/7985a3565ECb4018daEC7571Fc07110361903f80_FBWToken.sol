...
contract FBWToken is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 public _operationFee = 25;
    uint256 public _buyHandlingFee = 10;
    uint256 public _sellHandlingFee = 20;

    uint256 public _totalTaxIfBuying = _marketingFee.add(_teamFee).add(_operationFee).add(_buyHandlingFee);
    uint256 public _totalTaxIfSelling = _marketingFee.add(_teamFee).add(_operationFee).add(_sellHandlingFee);
    ...
    address public marketingWalletAddress = 0x0d2a2D872211f83772F27F028d41D101Cac735e9;
    address public teamWalletAddress = 0x0fBB5A560fd138F93593A341ec5425C44778e02d;
    address public operationWalletAddress = 0xd58e98758e7BE5FDF5e4e824260Cf325b904605d;
    IWrap public wrap = IWrap(0x4F047Fd93176ca53e8Dc78aECd5a30D81b19A5b0);
    IERC20 public usdt = IERC20(0x7b956fd8C75e5C5c002995ee7fbe3F023E65C174);
    IPancakeRouter01 public PancakeRouter01 = IPancakeRouter01(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);

    mapping (address => bool) public isMarketPair; 

    constructor () ERC20("FBWToken", "FBWT", 18, 1000000000 * 10 ** 18) {
    ...
    function _transfer(address sender, address recipient, uint amount) internal override {
        ...
        if (!freeOfCharge) {
            ...
            if (currentBalance > 0 && !swaping && sender != pancakePair) {
                swaping = true; 
                if(_buyFeeAmount > 0) {
                    _swapTokensForUsdt(_buyFeeAmount.mul(_marketingFee).div(_totalTaxIfBuying), marketingWalletAddress);
                    _swapTokensForUsdt(_buyFeeAmount.mul(_teamFee).div(_totalTaxIfBuying), teamWalletAddress);
                    _swapTokensForUsdt(_buyFeeAmount.mul(_operationFee).div(_totalTaxIfBuying), operationWalletAddress);
                }
                if(_sellFeeAmount > 0) {
                    _swapTokensForUsdt(_sellFeeAmount.mul(_marketingFee).div(_totalTaxIfSelling), marketingWalletAddress);
                    _swapTokensForUsdt(_sellFeeAmount.mul(_teamFee).div(_totalTaxIfSelling), teamWalletAddress);
                    _swapTokensForUsdt(_sellFeeAmount.mul(_operationFee).div(_totalTaxIfSelling), operationWalletAddress);
                }
                _buyFeeAmount = 0;
                _sellFeeAmount = 0;
                swaping = false;
