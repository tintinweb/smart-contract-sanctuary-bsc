...
contract UniswapAnchoredView is AggregatorValidatorInterface, UniswapConfig, Ownable {
    ...
    bytes32 constant ethHash = keccak256(abi.encodePacked("BNB"));
    bytes32 constant usdHash = keccak256(abi.encodePacked("BUSD"));
    ...
    constructor(uint anchorToleranceMantissa_,
        ...
        for (uint i = 0; i < configs.length; i++) {
            ...
            require(config.baseUnit > 0, "baseUnit must > 0");
            address uniswapMarket = config.uniswapMarket;
            if (config.priceSource == PriceSource.REPORTER) {
                require(uniswapMarket != address(0), "uniswapMarket==0");
                require(config.reporter != address(0), "reported==0");
    ...
    function priceInternal(TokenConfig memory config) internal view returns (uint) {
        ...
        if (config.priceSource == PriceSource.FIXED_ETH) {
            ...
            require(usdPerEth > 0, "ETH price == 0");
    ...
    function calculateAnchorPriceFromEthPrice(TokenConfig memory config) internal returns (uint anchorPrice) {
        ...
        if (config.priceSource == PriceSource.FIXED_ETH)
            return mul(ethPrice, config.fixedPrice) / ethBaseUnit;      // config.fixedPrice holds a fixed-point number with scaling factor 10**18 for FIXED_ETH

        //require(config.priceSource == PriceSource.REPORTER, "only reporter prices get posted");
        address baseToken = config.isUniswapReversed ? IUniswapV2Pair(config.uniswapMarket).token0() : IUniswapV2Pair(config.uniswapMarket).token1();
        uint conversionFactor = calculateAnchorPriceFromEthPrice(getTokenConfigByUnderlying(baseToken));
        return fetchAnchorPrice(config.symbolHash, config, conversionFactor);
    }
    ...
    function convertReportedPrice(TokenConfig memory config, int256 reportedPrice) internal pure returns (uint256) {
        require(reportedPrice >= 0, "Reported price cannot < 0");
    ...
    function fetchAnchorPrice(bytes32 symbolHash, TokenConfig memory config, uint conversionFactor) internal virtual returns (uint) {
        ...
        require(block.timestamp > oldTimestamp, "now < before");
