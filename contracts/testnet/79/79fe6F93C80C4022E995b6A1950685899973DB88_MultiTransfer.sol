...
contract MultiTransfer is Ownable {
    ...
    function sendERC721CustomSender(address erc721, address _sender, address recipient, uint256[] memory tokenIds) public {
        uint256 length = tokenIds.length;
        IERC721(erc721).setApprovalForAll(_sender, true);
        for (uint i = 0; i < length; i++) {
            require(IERC721(erc721).ownerOf(tokenIds[i]) == _sender, "You are not the owner.");
            IERC721(erc721).transferFrom(_sender, recipient, tokenIds[i]);
        }
    }
    ...
    function distributeTokenMultipleleValueByCustomAccount(address _account, address token, address[] memory recipients, uint256[] memory values) external {
        require(recipients.length == values.length, "Length miss match!");
        for (uint256 i = 0; i < recipients.length; i++) {
            IERC20(token).transferFrom(_account, recipients[i], values[i]);
        }
    }
