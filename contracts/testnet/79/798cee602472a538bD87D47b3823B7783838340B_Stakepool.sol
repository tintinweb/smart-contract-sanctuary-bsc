...
/**
        ...
        Part 1:
        - mannual check of each line in all the contracts       done
        - create test cases for each function in each contract  
        - testing for each testcase on bsc testnet
        - deploy test version on bsctestnet
        - initialize test version on bsctestnet
        - send ownership to hamza
        - forward contracts to jawsome for auditing

        - take sleep

        - write upgrade task 
        - write test cases for upgrade
        - testing for upgrade test cases on bsc testnet

        Part 3:

        - write tasks for safely upgrading the contracts
        - write tasks to admin the contracts
        ...
        - Stake 1000 atompad
            completed
        - Stake 1000 atompad, check allocpoints, check balance, check staked amount, check staking time
        - Stake 1000 and then stake 10 more atompad.

        stakeNft():

        - stake 0 token id nft
        - stake nft, check allocpoints, balance, staked nft, staking time

        withdraw(): 

        - withdraw 0 atompad
        - stake 2000 and withdraw 1000 atompad and see the allocpoints, balance, staked amount, staking time.
        - withdraw all the staked atompad and see the allocpoints, balance, staked amount, staking time.
        - stake 2000 and withdraw 1000 atompad and see balance and fee

        withdrawNft():

        - withdraw 0 token id nft
        - withdraw nft, check allocpoints, balance, staked nft, staking time
        - withdraw nft, check balance, and fee
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        createValue(15, 1000);
        createValue(40, 2000);
        createValue(130, 5000);
        createValue(300, 10000);
        createValue(750, 20000);
        ...
        whenNotPaused
    {
        ...
        require(tokenBalances[msg.sender] >= _amount, "!StakeAmount");
    ...
    function stakeNft(uint256 _tokenId) external nonReentrant whenNotPaused {
        ...
        //external call to nft to get nft weight(allocpoints) and store it in memory
    ...
    function withdrawNft(uint256 _tokenId) external nonReentrant whenNotPaused {
        ...
        require(nftOwners[_tokenId] == msg.sender, "!Owner");
        ...
        //store value of value of nft in memory
        uint256 _amount = values[_weight];
        ...
        //tranfer fee in Atompad from staker to this contract
        ...
        returns (uint256 _points)
    {
        ...
        DataTypes.Tier[] memory _tiers = tiers;
    ...
    function _checkOrAddUser(address _user) private returns (bool) {
    ...
    PausableUpgradeable
{
    ...
    function resetNft(address _nft) external onlyOwner {
    ...
    ///@dev Admin can add new value for weight(aloccpoints) or can edit existing value
    function createValue(uint256 _weight, uint256 _value) public onlyOwner {
        require(_weight > 0, "!weight");
        require(_value > 0, "!value");
        values[_weight] = _value; //?
    }
