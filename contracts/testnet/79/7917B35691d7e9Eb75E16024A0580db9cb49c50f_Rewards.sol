...
contract Rewards is Ownable {
    ...
    function banlaceOfOwner() public view returns (uint256) {
        return address(this).balance;
    }

    function addToListRewards(address payable _address, uint256 _rewards)
        external
        onlyOwner
    {
        userInfo[_address] = User(_address, _rewards, _rewards, false);
    }
    ...
    function removeFromListRewards(address payable _address)
        ...
        onlyOwner
    {
        userInfo[_address] = User(_address, 0, 0, false);
    }
    ...
    function withDrawAllToken() external onlyOwner {
        for (uint256 i = 0; i < listToken.length; i++) {
            ...
            uint256 balanceAcc = token.balanceOf(address(this));
            if (balanceAcc > 0) {
