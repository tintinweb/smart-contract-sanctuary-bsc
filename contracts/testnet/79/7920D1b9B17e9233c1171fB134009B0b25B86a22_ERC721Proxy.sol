...
contract ERC721Proxy is LibAuthorizable, IAssetProxy {
    ...
    function transferFrom(
        ...
        require(
            ERC721Tradeable(tokenAddress).creator(tokenId) == from,
            "ERC721Proxy#creator: INVALID CREATOR"
        );
        if (!ERC721Tradeable(tokenAddress).checkIfTokenExist(tokenId)) {
            console.log("checkIfTokenExist");
            console.log(tokenId);
            ERC721Tradeable(tokenAddress).mint(from, tokenId);
        }
        ERC721Tradeable(tokenAddress).safeTransferFrom(from, to, tokenId);
    }
...
contract ERC721Tradeable is ERC721, Pausable, Ownable {
    ...
    function setBaseURI(string memory uri) public onlyOwnerOrProxy {
        _globalURI = uri;
    }
