...
contract TraderDAO is Ownable{
    ...
    event TestProfitSharingAmount(uint indexed _amount);
    ...
    function VerifyPayout(uint _userId, uint _initialAmount, uint _finalAmount, uint _balanceAmount) external view returns(bool){
        if(_initialAmount != UserStakeAmount[_userId]){
            if(_initialAmount > UserStakeAmount[_userId]){
                return false;
            } 
            if(_balanceAmount + _initialAmount != UserStakeAmount[_userId]){
                return false;
            }
        }
       
        if(_initialAmount < _finalAmount){
            uint profit = _finalAmount - UserStakeAmount[_userId];
            if((_finalAmount - profit) ==_initialAmount){
                return true;
            }
        }
        else{
            uint loss = UserStakeAmount[_userId] - _finalAmount; 
            if((_finalAmount + loss) == _initialAmount){
                return true;
            }
        }
    ...
    function DistributeReward(uint[] memory _userIds, uint[] memory _initialAmounts, uint[] memory _finalAmounts) external onlyOwner{
        ...
        for(uint i = 0; i< _userIds.length; i++) {
            ...
            if(_initialAmount < _finalAmount){
                ...
                USDT.safeTransfer(PlatformAccount, profitsharing);
                _finalAmount = _finalAmount - profitsharing;
            }
            UserRewardAmount[_userId] = UserRewardAmount[_userId] + _finalAmount;

            if(UserReinvestStatus[_userAddress] == true){
                Insert(_userAddress, _finalAmount);
            }
            else{
