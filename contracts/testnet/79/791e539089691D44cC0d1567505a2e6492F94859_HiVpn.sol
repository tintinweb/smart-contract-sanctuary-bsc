...
contract HiVpn is IHiVpn, Secure {
  ...
  function findPlan(uint256 value) public view override returns (uint8) {
    uint256 moreValue = value.mul(TOLERANCE_UP).div(1000);
    uint256 lessValue = value.mul(1000).div(TOLERANCE_DOWN);
...
abstract contract Secure {
  ...
  uint128 public TOLERANCE_UP = 1010;
  uint128 public TOLERANCE_DOWN = 1010;
  ...
  function changeToleranceUp(uint128 tolerance) external onlyOwner {
    TOLERANCE_UP = tolerance;
  }

  function changeToleranceDown(uint128 tolerance) external onlyOwner {
    TOLERANCE_DOWN = tolerance;
  }
