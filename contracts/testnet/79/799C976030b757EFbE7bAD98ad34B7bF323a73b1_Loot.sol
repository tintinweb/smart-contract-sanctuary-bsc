...
import "../lib /openzeppelin-contracts-master/contracts/access/AccessControl.sol";
import "../lib /openzeppelin-contracts-master/contracts/token/ERC721/ERC721.sol";

contract Loot is ERC721, AccessControl, ICollectible{
    ...
    uint256 public constant MAX_TOTAL_SUPPLY = 100000;

    constructor(address _admin) ERC721("Domeprotocol.io Loot", "LOOT") {
        _setupRole(DEFAULT_ADMIN_ROLE, _admin);
        _setupRole(MINTER_ROLE, _admin);
    }

    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "Loot: Caller is not a Minter");
        _;
    }
    ...
    function mintTo(address _to, bytes32 _hash) public override onlyMinter{
        require (lastMintedTokenId < MAX_TOTAL_SUPPLY, "Dome Loot;: Max amount of tokens reached");
    ...
    function getMetadataIPFSHash(uint256 _tokenId) public override view returns(string memory){
        require(_exists(_tokenId), "Loot: URI query for nonexistent token");
    ...
    */
    function tokenURI(uint256 _tokenId) public view virtual override(ERC721, ICollectible) returns (string memory) {
        require(_exists(_tokenId), "Domeprotocol.io Loot: URI query for nonexistent token");
...
import "../lib /openzeppelin-contracts-master/contracts/token/ERC721/IERC721.sol";
