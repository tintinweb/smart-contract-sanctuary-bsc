...
contract StakingV2 is Ownable, ReentrancyGuard {
    ...
    function restore(uint256 _pid, address _user) internal{
        UserInfo storage info = userInfo[_pid][_user];
        uint256 amount;
        uint256 rewards;
        uint256 timestamp;
        (amount, rewards, timestamp) = g_oldContract.userInfo(_pid, _user);
        info.amount = amount;
    ...
    function getUserInfo(uint256 _pid, address _user) external view returns(uint256, uint256, uint256){ 
        ...
        return(info.amount, info.rewardDebt, info.timestamp);
    }
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if(!user.isRestored ){
            ...
            uint256 timestamp;
            (amount, rewards, timestamp) = g_oldContract.userInfo(_pid, _user);
            return amount * pool.apy * (block.timestamp - timestamp) / 3153600000 + rewards;
        }
    ...
    function withdraw(uint256 _pid) external nonReentrant{
        ...
        uint256 rewards = user.amount * pool.apy * (block.timestamp - user.timestamp) / 3153600000 + user.rewardDebt;
        uint256 withdrawAmount = user.amount + rewards;
        crace.safeTransfer(address(msg.sender), withdrawAmount);
        user.rewardDebt = 0;
        user.timestamp = block.timestamp;
        user.amount = 0;
        pool.stakedAmount = pool.stakedAmount - withdrawAmount;
