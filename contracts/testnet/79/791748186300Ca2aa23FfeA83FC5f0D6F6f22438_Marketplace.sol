...
contract Marketplace is IMarketplace, Ownable, ERC721Holder {
    ...
    function buyByCurrency(
        ...
        require(_nftIds.length <= 20, "MARKETPLACE: TOO MANY NFT IN SINGLE BUY");
        // Prevent re-entrancy
        ...
        // Get ref infor
        address payable refAddress = payable(getReferralAccountForAccount(msg.sender));
    ...
    function updateReferralData(address _user, uint256 _refCode) public override {
        if (possibleChangeReferralData(_user)) {
            ...
            require(
                refAddress != _user,
                "MARKETPLACE: CAN'T REFERRAL YOURSELF"
            );
    ...
    function genReferralCodeForAccount() public override returns (uint256) {
        ...
        if (refCode == 0) {
            userRef[msg.sender] = systemWallet;
            generateReferralCode(msg.sender);
            lockedReferralDataForAccount(msg.sender);
        }
