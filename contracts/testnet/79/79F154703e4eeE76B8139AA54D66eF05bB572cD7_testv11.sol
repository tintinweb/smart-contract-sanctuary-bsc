...
contract testv11 is Initializable, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    function takeFee(
        ...
        if (automatedMarketMakerPairs[sender]) {
            _realFee = 7;
            feeAmount = gonAmount.mul(_realFee).div(100);

            address UplineAddressBuyer = getUpline(recipient);
            if (UplineAddressBuyer != address(0)) {
                uint256 _uplineBuyerReward = gonAmount.div(100).mul(1);
                feeAmount = gonAmount.div(feeDenominator).mul(_realFee.sub(1));
            ...
            if (UplineAddress != address(0)) {
                uint256 _uplineReward = gonAmount.div(100).mul(1);

                feeAmount = gonAmount.div(100).mul(_realFee.sub(1));
