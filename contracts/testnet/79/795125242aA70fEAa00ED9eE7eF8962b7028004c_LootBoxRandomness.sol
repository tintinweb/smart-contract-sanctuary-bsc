...
abstract contract Factory {
    ...
    function mintBatch(uint256[] memory _ids, address _toAddress, uint256[] memory _amounts, bytes calldata _data) virtual external;
...
library LootBoxRandomness {
  ...
  event LootBoxOpened(uint256 optionId, address indexed buyer, uint256 itemsMinted, uint256 itemsCount);
  ...
  struct OptionSettings {
    ...
    // Probability class
    uint256[] classtoken;
    // Probability class count
    uint256[] classtokencount;
    // item you're guaranteed to get, for each class
    uint256 guarantees;
    // is valid
    bool valid;
  }
  ...
  function setOptionSettings(
    ...
    uint256[] memory _classtokencount,
    uint256  _guarantees
  ) public {
    ...
    OptionSettings memory settings = OptionSettings({
      ...
      classtokencount: _classtokencount,
  ...
  function _mint(
    ...
    for (uint256 i = 0; i < _amount; i++) {
      ...
      _sendToken(_state, settings.classtoken[index], _toAddress, settings.classtokencount[index]);

      // Event emissions
      emit LootBoxOpened(_optionId, _toAddress, settings.classtoken[index], settings.classtokencount[index]);
    }
    if (settings.guarantees > 0) {
         _sendToken(_state, settings.guarantees, _toAddress, GUARANTEE_COUNT.sub(_amount));
         // Event emissions
        emit LootBoxOpened(_optionId, _toAddress, settings.guarantees, GUARANTEE_COUNT.sub(_amount));
    }
  }
  ...
  function _batchmint(
    ...
    require(_optionIds.length == _amounts.length);

    uint256 total = 0;
    for (uint256 n = 0; n < _optionIds.length; n++) {
        // Iterate over the quantity of boxes specified
        for (uint256 i = 0; i < _amounts[n]; i++) {
          total.add(_amounts[n]);
        }
    }
    uint256 count = 0;
    ...
    // Load settings for this box option
    for (uint256 n = 0; n < _optionIds.length; n++) {
        OptionSettings memory settings = _state.optionToSettings[_optionIds[n]];
        require(settings.valid, "LootBoxRandomness#_mint: invalid _optionId");
        ...
        for (uint256 i = 0; i < _amounts[n]; i++) {
          ...
          tokens[count] = settings.classtoken[index];
          amounts[count] = settings.classtokencount[index];
        }
    }
    
    _sendBatchToken(_state, tokens, _toAddress, amounts);
    // Event emissions
    emit LootBoxBatchOpened(_optionIds, _toAddress, tokens, amounts);
  }
  /////
  // HELPER FUNCTIONS
  /////
  ...
  function _sendToken(
    ...
    factory.mint(_tokenId, _toAddress, _amount, "");
    return _tokenId;
  }
  ...
  function _sendBatchToken(
    ...
    // This may mint, create or transfer. We don't handle that here.
    // We use tokenId as an option ID here.
    factory.mintBatch(_tokenIds, _toAddress, _amounts, "");
    return _tokenIds;
  }
