...
contract Ballot {
    ...
    function openProposalsSubmissionForNextPoll() external {
        require(chairPerson == msg.sender, "Only a chair can open proposals acceptance");
        acceptProposals = true;
    }

    // chair stop accepting proposals
    function closeProposalsSubmissionForNextPoll() external {
        require(chairPerson == msg.sender, "Only a chair can close proposals acceptance");
        acceptProposals = false;
    }

    // Append new proposal to the to the end of `proposals`.
    function submitProposal(bytes32 title, bytes32 description, address account) external {
        require(acceptProposals == true, "Poll is currently not accepting proposals");
    ...
    function vote(uint256 proposalId) external {
        ...
        require(daoToken.balanceOf(msg.sender) > 0, "You have not right to vote due to your insuffient KOLO token balance");
        require(proposalId <= _proposalId, "The proposal identifier is unknown");

        uint256 balance = daoToken.balanceOf(msg.sender);
        uint256 voteWeight = balance - votersWeightUsed[msg.sender];

        require(voteWeight > 0, "You've already voted or you have now new right to vote");

        // mapping(int256 => mapping(uint256 => Proposal)) public pollHistory;
        Proposal memory candidate = pollHistory[pollId][proposalId];
        candidate.voteCount.add(voteWeight);
        pollHistory[pollId][proposalId] = candidate;

        votersWeightUsed[msg.sender] = balance;
    }
    ...
    function getPollProposal(uint256 _pollId, uint256 _proposaId) external view returns (bytes32, bytes32, uint256, address, address) {

        Proposal memory proposal = pollHistory[_pollId][_proposaId];
        return (proposal.title, proposal.description, proposal.voteCount, proposal.account, proposal.owner);
    }

    function declarePollWinner(uint256 _pollId, uint256 _proposaId) external {
        require(chairPerson == msg.sender, "Only the chair can declare a poll winner");
        pollWinner[_pollId] = _proposaId;
    }

    function getPollWinner(uint256 _pollId) external view returns (bytes32, bytes32, uint256, address, address){
