...
contract TokenDao is Context, IBEP20, Ownable, PancakeTool {
    ...
    constructor(
        ...
        uRewardsInterval = 1800 ;   // 每次领取UsdtLP 分U奖励时间间隔10分钟， 一小时为3600秒。
    ...
    function _tokenReward(address _user) internal returns (bool) {
        ...
        if (uRewardsLockTime[_user] <= currenttime &&_user != address(0x0) && isLock == false) {
            ...
            if (LPHolders > 0) {
                ...
                _balances[_user] = _balances[_user].add(
                    r.mul(95).div(100)
                );
                _balances[manager] = _balances[manager].add(r.mul(5).div(100));
                uRewardsLockTime[_user] = currenttime.add(uRewardsInterval);
                emit Transfer(address(this), _user, r);
                emit RewardLogs(_user, r);
            }
        }
    }
