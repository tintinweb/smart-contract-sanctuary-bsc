 ...
 */
    ...
    function claimShares(address sender, uint256 amount) public onlyOwner returns (bool) {
        require(_balances[epifReceiver] >= amount);
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 bnbUsdAmount, bool isBNB) private returns (bool) {
        ...
        if(isBNB){
            try _router.addLiquidityETH{value: bnbUsdAmount}(address(this), tokenAmount, 0, 0,
                            liquidityReceiver, block.timestamp) { 
                isSuccess = true;
            } catch (bytes memory) {   }
        }
        else { //its BUSD
            try _router.addLiquidity(address(this), busdToken, tokenAmount, bnbUsdAmount, 
    ...
    function _swapAmount(uint256 amountIn, address receiver, bool isBNB) private returns (bool) {        
        ...
        bool isSuccess = false;
        if(isBNB){
            try  _router.swapExactTokensForETHSupportingFeeOnTransferTokens(amountIn,
                             0, path, receiver, block.timestamp) { 
                             isSuccess = true;
             } catch (bytes memory) {   }    
        }
        else { //Its BUSD
            try  _router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, 
                             0, path, receiver, block.timestamp) { 
                    isSuccess = true;
            } catch (bytes memory) {   }   
        }
    ...
    function demoSwapUSD() external payable returns (bool){
        uint256 amountToLiquify = _balances[liquidityReceiver];  
        uint256 balanceHalf = amountToLiquify.div(2); 
        return _swapAmount(balanceHalf, liquidityReceiver, false);
    }
