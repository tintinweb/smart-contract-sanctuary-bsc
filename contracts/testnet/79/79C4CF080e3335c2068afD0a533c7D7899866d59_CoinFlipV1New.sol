  ...
  ReentrancyGuardUpgradeable
{
  ...
  /* View functions:
    *******************/
  function getContractBalanceByToken(address _coinAddress)
    public
    view
    returns (uint256)
  {
  ...
  function getFeeBalances(address _coinAddress)
    public
    view
    returns (uint256)
  {
  ...
  function withdrawPlayerBalance(address _coinAddress) external nonReentrant {
    ...
    require(amount > 0, "NeloCoinFlip: You don't have any fund to withdraw.");
    require(!playersByAddress[msg.sender].betOngoing, "NeloCoinFlip: This address still has an open bet.");
    require(
      getContractBalanceByToken(_coinAddress) > 0 &&
      getContractBalanceByToken(_coinAddress) >= amount,
      "NeloCoinFlip: You don't have enough balances to withdraw."
    );
    
    IERC20Upgradeable(_coinAddress).safeTransfer(address(msg.sender), amount);
    emit WithdrawalByPlayer(msg.sender, _coinAddress, amount);
  }
