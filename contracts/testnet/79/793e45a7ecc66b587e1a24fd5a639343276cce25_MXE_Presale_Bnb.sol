...
contract MXE_Presale_Bnb is ReentrancyGuard, Context, Ownable {
    ...
    constructor (uint256 rate,uint256 rateUsdt, address wallet, IERC20 token, IERC20 usdttoken)  {
    ...
    function buyTokensUsdt(uint256 amount) public nonReentrant icoActive{
        uint256 weiAmount = amount;
        ...
        uint256 tokens = _getTokenAmountUsdt(weiAmount);
        _preValidatePurchase(msg.sender, weiAmount);
        _weiRaised = _weiRaised + weiAmount;
        purchasedTokens += tokens;
    ...
    function _getTokenAmountUsdt(uint256 weiAmount) internal view returns (uint256) {
        return (weiAmount * _rateUsdt)/10**_tokenDecimals;
    }
