...
contract SpacePad is Pausable, Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        if (_referrer == address(0)) {
            busdContract.safeIncreaseAllowance(msg.sender, _amount);
            busdContract.safeTransferFrom(msg.sender, address(this), _amount);
        } else {
            ...
            busdContract.safeTransferFrom(msg.sender, _referrer, fee);
    ...
    function withdraw(address _recipient) external whenNotPaused onlyOwner {
        ...
        uint256 balance = busdContract.balanceOf(address(this));
        busdContract.safeTransfer(_recipient, balance);
