    ...
    SpotHouseStorage
{
    ...
    function _withdrawBNB(
        ...
        console.log("[SM][_withdrawBNB] address(this)1: ", IWBNB(WBNB).balanceOf(address(this)));
        assert(IWBNB(WBNB).transferFrom(pairManagerAddress, address(this), _amount));
        console.log("[SM][_withdrawBNB] address(this)2: ", IWBNB(WBNB).balanceOf(address(this)), address(this).balance);
        IWBNB(WBNB).withdraw(_amount);


        console.log("[SM][_withdrawBNB] balance1 BNB: ", address(this).balance);

        payable(_trader).transfer(_amount);
        console.log("[SM][_withdrawBNB] balance _trader BNB: ", _trader.balance);
//        TransferHelper.safeTransferETH(_trader, _amount);
