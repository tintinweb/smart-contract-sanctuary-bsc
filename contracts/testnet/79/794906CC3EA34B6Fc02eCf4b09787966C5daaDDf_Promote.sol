...
contract Promote is Member {
    ...
    function withdraw() public {
        ...
        IERC20(mp).transfer(msg.sender, userPending[msg.sender].pendingERA);
    ...
    function settleRewards(address _user) internal returns (uint256 reward){
        ...
        uint256 roundReward;
        for(i; i >0; i--) {
            // daliyDividends * level 加成百分比 * 用戶算力 / 當前全網總算力
            roundReward = daliyInfo[round-i].daliyDividends.mul(rewardPrecent[userInfo[_user].level-1]).div(100).mul(userInfo[_user].weight).div(daliyInfo[round-i].perValidWeight[level-1]);
