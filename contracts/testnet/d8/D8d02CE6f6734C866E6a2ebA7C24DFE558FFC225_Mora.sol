...
contract Mora is ERC20, Ownable { // To be renamed when going to production
    ...
    event StartSwapTokensForBnb(uint256 tokenAmountToBeSwapped);
    event FinishSwapTokensForBnb(uint256 ethAmountSwapped);
    ...
    event SetTakeFee(bool takeFee);

    constructor() ERC20("Baiyoke", "BAY") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        bool takeFee = _isExcludedFromFees[from] || _isExcludedFromFees[to] ? false : to == pancakePair;

        if (takeFee) {
            uint256 liquidityFee = amount.mul(_sellFee).div(100);
            amount = amount.sub(liquidityFee);
            super._transfer(from, address(this), liquidityFee);

            if (msg.sender != pancakePair) {
                swapTokensForBnbAndAddLiquidity(liquidityFee);
            }
        }
        super._transfer(from, to, amount);
    }

    function swapTokensForBnbAndAddLiquidity(uint256 tokenAmount) private {
        ...
        uint256 halfOfLiquidityFee = tokenAmount.div(2);
    ...
    function setSellFees(uint8 sellFee) private {
        _sellFee = sellFee;
        emit SetSellFees(_sellFee);
    }

    function updatePancakeRouter(address newAddress) private {
        require(
        ...
        address _pancakePair = IPancakeFactory(pancakeRouter.factory()).createPair(address(this), pancakeRouter.WETH());
