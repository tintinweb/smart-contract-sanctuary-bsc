...
contract Anubit is Context, IERC20, Ownable {
    ...
    //####################################### REMOvE BEFOR LAUNCH #######################################################################################
    event AutoSwapCall(uint256 _swapAmount);
    ...
    function _swapForBNB(uint256 amount) internal {
      if(amount > 0) {

        uint256 amountSwapForWeth = amount; //total amount token for liquify

        IUniswapV2Router02 router = IUniswapV2Router02(uniswapPair);
