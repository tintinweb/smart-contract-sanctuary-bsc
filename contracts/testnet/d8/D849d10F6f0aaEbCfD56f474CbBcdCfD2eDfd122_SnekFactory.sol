...
contract SnekFactory is IFactory {
  ...
  function setPauser(address _pauser) external {
    require(msg.sender == pauser, "SnekFactory: Not pauser");
    pendingPauser = _pauser;
  }

  function acceptPauser() external {
    require(msg.sender == pendingPauser, "SnekFactory: Not pending pauser");
  ...
  function setSwapFee(address pair, uint value) external {
    require(msg.sender == pauser, "SnekFactory: Not pauser");
    SnekPair(pair).setSwapFee(value);
  }

  function pairCodeHash() external pure override returns (bytes32) {
    return keccak256(type(SnekPair).creationCode);
  }
  ...
  external override returns (address pair) {
    require(tokenA != tokenB, 'SnekFactory: IDENTICAL_ADDRESSES');
    (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    require(token0 != address(0), 'SnekFactory: ZERO_ADDRESS');
    require(getPair[token0][token1][stable] == address(0), 'SnekFactory: PAIR_EXISTS');
    // notice salt includes stable as well, 3 parameters
    bytes32 salt = keccak256(abi.encodePacked(token0, token1, stable));
    (_temp0, _temp1, _temp) = (token0, token1, stable);
    pair = address(new SnekPair{salt : salt}());
...
contract SnekPair is IERC20, IPair, Reentrancy {
  using SafeERC20 for IERC20;
  ...
  function mint(address to) external lock override returns (uint liquidity) {
    ...
    require(liquidity > 0, 'SnekPair: INSUFFICIENT_LIQUIDITY_MINTED');
    _mint(to, liquidity);
  ...
  function burn(address to) external lock override returns (uint amount0, uint amount1) {
    ...
    require(amount0 > 0 && amount1 > 0, 'SnekPair: INSUFFICIENT_LIQUIDITY_BURNED');
  ...
  function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
    require(!IFactory(factory).isPaused(), "SnekPair: PAUSE");
    require(amount0Out > 0 || amount1Out > 0, 'SnekPair: INSUFFICIENT_OUTPUT_AMOUNT');
    (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
    require(amount0Out < _reserve0 && amount1Out < _reserve1, 'SnekPair: INSUFFICIENT_LIQUIDITY');
    uint _balance0;
    uint _balance1;
    {// scope for _token{0,1}, avoids stack too deep errors
      (address _token0, address _token1) = (token0, token1);
      require(to != _token0 && to != _token1, 'SnekPair: INVALID_TO');
    ...
    require(amount0In > 0 || amount1In > 0, 'SnekPair: INSUFFICIENT_INPUT_AMOUNT');
    {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
      ...
      require(_k(_balance0, _balance1) >= _k(_reserve0, _reserve1), 'SnekPair: K');
    }
  ...
  function approve(address spender, uint amount) external override returns (bool) {
    require(spender != address(0), "SnekPair: Approve to the zero address");
  ...
  function permit(
    ...
    require(deadline >= block.timestamp, 'SnekPair: EXPIRED');
    ...
    require(recoveredAddress != address(0) && recoveredAddress == owner, 'SnekPair: INVALID_SIGNATURE');
  ...
  function transferFrom(address src, address dst, uint amount) external override returns (bool) {
    ...
    if (spender != src && spenderAllowance != type(uint).max) {
      require(spenderAllowance >= amount, "SnekPair: Insufficient allowance");
    unchecked {
  ...
  function _transferTokens(address src, address dst, uint amount) internal {
    require(dst != address(0), "SnekPair: Transfer to the zero address");
    ...
    require(srcBalance >= amount, "SnekPair: Transfer amount exceeds balance");
  unchecked {
