...
function Deposit(uint _amountStacked) public{
    timed[msg.sender]=block.timestamp;
    ...
    deposit[msg.sender]+=_amountStacked;
...
function DailyEarning() public view returns(uint dailyearn){

   
    require(_pct>=5 && _pct<=10,"amount decided should be between .5 and 1");
    require(deposit[msg.sender] !=0,"deposit something please");

    if(booster[msg.sender].length!=0){
        for(uint i=0;i<booster[msg.sender].length;i++){
            if(booster[msg.sender][i].boosterTime+70>=block.timestamp){
               dailyearn+=(deposit[msg.sender]* _pct * (block.timestamp-timed[msg.sender]))/10000+(deposit[msg.sender]* booster[msg.sender][i].boosterPct * (block.timestamp-booster[msg.sender][i].boosterTime))/10000;
            }
            else{
                dailyearn=(deposit[msg.sender]* _pct * (block.timestamp-timed[msg.sender]))/10000;
...
function RewardWith() public{
    require(block.timestamp>=timed[msg.sender]+70,"call function after 70 sec");
    rewards[msg.sender]= DailyEarning();
...
function compound() public {
    require(block.timestamp>=timed[msg.sender]+70,"call function after 70 sec");
    rewards[msg.sender]=DailyEarning();
    deposit[msg.sender]+=rewards[msg.sender];
...
function unstake() public {
    require(block.timestamp>=timed[msg.sender]+70,"call function after 70 sec");
    timed[msg.sender]=block.timestamp;
    BSI.transfer(msg.sender,(deposit[msg.sender]*20)/100);
    deposit[msg.sender]-=(deposit[msg.sender]*20)/100;
    rewards[msg.sender]=0;
}
