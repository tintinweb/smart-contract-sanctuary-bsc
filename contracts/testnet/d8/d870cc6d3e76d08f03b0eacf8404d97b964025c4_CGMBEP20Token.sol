...
/**
 * @dev Implementation of the {CGM} interface.
 *
 * This implementation is agnostic to the way tokens are created. This means
 * that a supply mechanism has to be added in a derived contract using {_mint}.
 * For a generic mechanism see {CGMPresetMinterPauser}.
 *
 ...
 * allowances. See {CGM-approve}.
 */
 contract CGMBEP20Token is Initializable, IBEP20 {

    using SafeMath for uint256;

    // user https://eth-converter.com/
    uint256 private _totalSupply; // --> 10,000,000,000,000,000,000,000,000,000 (WEI) => 10B Tokens (ether)
    ...
    modifier whenStakeIsEnabled {
        require(enableStake, "CGM: can only be called when staking is snabled.");
        _;
    }

    modifier onlyOwner {
        require(msg.sender == _board.owner, "CGM: can only be called by the owner.");
        _;
    }

    modifier onlyStakeRewardTokenContract {
        require(msg.sender == address(csr), "CGM: only stake reward token can call this.");
        _;
    }
     ...
     */
     constructor(){
      ...
      //  uint256 private _tTotal = 1000000000 * 10**6 * 10**9;
       _totalSupply = 10000000000000000000000000000; // --> 10,000,000,000,000,000,000,000,000,000 (WEI) => 10B Tokens (in ETH)
    ...
    //    totalSupply = 1000000000000000000000000000; // --> 1,000,000,000,000,000,000,000,000,000 (WEI) => 10B Tokens
    //    tokenName = _name;
    //    tokenSymbol = _symbol;
    //    tokenDecimals = 18;
    ...
    */
     ...
     * Ether and Wei. This is the value {CGM} uses, unless {_setupDecimals} is
     * called.
     *
     * NOTE: This information is only used for _display_ purposes: it in
     * no way affects any of the arithmetic of the contract, including
     * {ICGM-balanceOf} and {ICGM-transfer}.
     */
     ...
     */
     function internalTransfer(address _s, address _r, uint256 amt) internal {
       ...
       require(_s != address(0), "CGM: transfer must be done from zero address");
       require(_r != address(0), "CGM: transfer must be done from zero address");
       require(getBalance(_s) >= amt, "insufficient funds");

       _board.parties[_s].balance = _board.parties[_s].balance.sub(amt, "CGM: transfer amount exceeds balance");
     ...
     */
     function internalApprove(address _o, address _s, uint256 amt) internal virtual {
      ...
      //  require(_o != address(0), "CGM: approval must be done from zero address");
       require(_s != address(0), "CGM: approval must be done from zero address");
     ...
     * required by the EIP. See the note at the beginning of {CGM};
     *
     * Requirements:
     ...
     */
     function transferFrom(address sender, address recipient, uint256 amount) external virtual returns(bool){
       ...
       // approve transfer
       internalApprove(sender, msg.sender, _board.parties[sender].allowance[msg.sender].sub(amount, "CGM: transfer amount exceeds allowance"));
    ...
    function decreaseAllowance(address spender, uint256 value) external virtual returns (bool) {
        internalApprove(msg.sender, spender, _board.parties[msg.sender].allowance[spender].sub(value, "CGM: decreased allowance below zero"));
        return true;
    }
    ...
    function internalBurn(address account, uint256 amount ) internal virtual {
      ...
      require(account != address(0), "CGM: ");

      _board.parties[account].balance = _board.parties[account].balance.sub(amount, "CGM: amount exceeds balance");
    ...
    */
    ...
    function setCsrParty(address _csrp) external virtual onlyOwner {
        require(csrp != address(0x0),"CGM: address can not be 0x0");
        csrp = _csrp;
    }
    ...
    function internalStake(uint256 amount, uint package) internal {
        ...
        uint stakes = _board.parties[msg.sender].stakes.length; // this is the stakeid. Always [current no of stakes] - 1
    ...
    function unStake(uint id) external virtual {
        ...
        require(_board.parties[msg.sender].staked > 0 && _board.parties[msg.sender].stakes.length > id && id >= 0, "CGM: stake not found");

        StakeData memory pStake = _board.parties[msg.sender].stakes[id];

        require(pStake.redeemed == false, "CGM: you can't redeem stake twice");
        ...
        require(currentDays > 0,"CGM: same day unstake not allowed");
        ...
        if(currentDays < pStake.duration){
            ...
            if(burnTax){
                ...
                require(csr.transfer(csrp, reward_tax),"CGM: CSR Transfer failed");
            }
        ...
        require(csr.transfer(msg.sender, reward),"CGM: CSR Transfer failed ");
    ...
    function getPartyStake(uint256 stakeId, address sender) external view returns (uint256 id, string memory package, uint256 returnPerDay, uint duration, uint256 staked,  uint256 timestamp,
        bool redeemed) {
        require(getStakesOf(sender) > 0 && getStakesOf(sender) > stakeId && stakeId >= 0, "CGM: stake not found");

        StakeData memory pStake = _board.parties[sender].stakes[stakeId];

        return (pStake.id, pStake.package, pStake.intPerDay, pStake.duration, pStake.totalStaked, pStake.timestamp, pStake.redeemed);
    }

    function getPartyStakeReward(uint256 stakeId, uint256 atTimestamp, address sender) external view returns (uint256) {
    ...
    function setStakeTaxDetails(uint256 stake_tax,uint256 reward_tax) external virtual onlyOwner {
        ...
        require(stake_tax > 0 && reward_tax > 0, "CGM: unstake tax percent must be greater than zero");
