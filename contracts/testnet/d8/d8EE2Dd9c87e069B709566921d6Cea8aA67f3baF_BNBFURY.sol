...
contract BNBFURY {
	...
	uint256 public					INVEST_MIN_AMOUNT				= 0.001 ether;			// 0.01 BNB
	uint[] public					REFERRAL_PERCENTS				= [70, 30, 20, 10, 5];	// 7% 3% 2% 1% 0.5%
	uint256 public					PROJECT_FEE						= 10;					// project fee 5% of deposit
	uint256 public					DEV_FEE							= 20;					// project fee 5% of deposit
	uint256 public					MARKETING_FEE					= 30;					// marketing fee 10% of deposit
	...
	constructor(address payable _walletProject, address payable _walletDev, address payable _walletMarketing) {
    ...
    function SET_INVEST_MIN_AMOUNT(uint256 value) external {
        require(msg.sender == WALLET_PROJECT, "Admin use only");
        INVEST_MIN_AMOUNT = value * 1 ether;
    }
    ...
    function CHANGE_DEV(address value) external {
        require(msg.sender == WALLET_DEV, "Dev use only.");
        WALLET_DEV = payable(value);
    }

    function CHANGE_MARKETING(address value) external {
        require(msg.sender == WALLET_MARKETING, "Marketing use only.");
        WALLET_MARKETING = payable(value);
    }

    function SET_REFERRAL_PERCENTS(uint value) external {
        require(msg.sender == WALLET_PROJECT, "Admin use only.");
        REFERRAL_PERCENTS = [value];
    }
