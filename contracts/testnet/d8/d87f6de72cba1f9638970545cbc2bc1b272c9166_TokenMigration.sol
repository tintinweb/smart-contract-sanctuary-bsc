...
contract TokenMigration is
    Initializable,
    PausableUpgradeable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ERC20Burnable public tokenV1;
    ERC20Burnable public tokenV2;
    ...
    function initialize(
        ...
        tokenV1 = ERC20Burnable(tokenV1_);
        tokenV2 = ERC20Burnable(tokenV2_);
    ...
    function migrateAll() external whenNotPaused {
    ...
    function migrate(uint256 amount) external whenNotPaused {
        ...
        tokenV1.transferFrom(_msgSender(), address(this), amount);
        tokenV2.transfer(_msgSender(), amount);

        // extra 10% bonus, hooray!
        if (!_isExcludedFromBonus[_msgSender()]) {
            tokenV2.transferFrom(bonusFromAddress, _msgSender(), amount / 10);
        }
    }
    ...
    function burnTokenV1(uint256 amount) public onlyOwner {
        uint256 balance = tokenV1.balanceOf(address(this));
        require(balance >= amount, "Migration: burn amount exceeds balance");
        tokenV1.burn(amount);
    }

    function burnAllTokenV1() public onlyOwner {
        uint256 balance = tokenV1.balanceOf(address(this));
        require(balance > 0, "Migration: no tokens to burn");
        tokenV1.burn(balance);
    }
