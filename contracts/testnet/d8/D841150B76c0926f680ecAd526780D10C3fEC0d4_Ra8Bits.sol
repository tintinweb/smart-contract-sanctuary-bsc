...
contract Ra8Bits is Context, Ownable, ERC1155Holder {
    ...
    uint256 public breedingTime = 1 days;
    ...
    function rejectSuperLike(
        ...
        require(
            IERC20(ratBitTokens).transfer(fromOwnerAddress,  isSuperliked[superLikeNFTId])
        );
        isSuperliked[superLikeNFTId] = 0;
        emit SuperLikeResponse(
            fromNftId,
    ...
    function withdrawalNft(uint256 _breedId) public {
        ...
        require(details.ownerOfParentA == _msgSender(), "Breeding started cant withrawal NFT");
        require(
        ...
        if (details.ownerOfParentA == _msgSender()) {
            ...
            emit WithdrawalNft(_breedId, _msgSender(), details.parentA,block.timestamp);
        } 
    }
    ...
    function claimNft(uint256 _breedId) public {
        ...
        if (details.ownerOfParentA == _msgSender()) {
            ...
            // }
            emit ClaimNft(_breedId, _msgSender(), details.parentA,block.timestamp);
        } else {
            sendNftToUser(
                claimDetails[_breedId].parentAClaimed,
            ...
            // }
            emit ClaimNft(_breedId, _msgSender(), details.parentB,block.timestamp);
        }
    }
