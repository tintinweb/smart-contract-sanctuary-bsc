...
library SmartMatrix {
	...
	function SelectAllChildrenNodeIDInNodeTest(
		...
		uint8 _Matrix,
		uint64 _Length
	) internal view returns (uint256 _NodeCount, uint256[] memory _Test, uint256 _TestLength) {
		uint64 _MEMORY_LENGTH = type(uint64).max;
...
contract Tuktu is TuktuBase, ITuktu {
	...
	{
		// uint256[] memory _NodeIDs = new uint256[](_Length);
		(_NodeCount, _Test, _TestLength) = SmartMatrix.SelectAllChildrenNodeIDInNodeTest(
			Nodes[_NodeID],
			Nodes,
			_Matrix,
			_Length
		);
		_MaxUint64 = type(uint64).max;
	}
