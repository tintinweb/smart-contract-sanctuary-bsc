...
contract TESTE is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 TOMB-WETH LP -> TOMB pool
    uint256 public constant INITIAL_TOMB_POOL_DISTRIBUTION = 100000 ether;
    ...
    uint256 public sellFee = 1000; // see fee is 10%
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("TESTE", "TESTE") {
    ...
    function setFeeAddress(address _feeAddress) external {
        require(msg.sender == feeCollectorAddress, "!fee");
        require(_feeAddress != address(0), "zero");
        feeCollectorAddress = _feeAddress;
    }
    
    function setSellFeePercentage(uint256 _value) external {
        require(msg.sender == feeCollectorAddress, "!fee");
    ...
    function transferFrom(
        ...
        if (!inSwap && sender != pair && takeFee && canSwap && sender != owner()) {
            //uint256 sellFeeAmount = amount.div(10000).mul(sellFee);
            //_transfer(sender, address(this), sellFeeAmount);
        ...
        if (currentTaxRate == 0 || excludedAddresses[sender]) {
            if( recipient == pair && takeFee && !inSwap){
                uint256 sellFeeAmount = amount.div(10000).mul(sellFee);
                _transfer(sender, address(this), sellFeeAmount);
                amount = amount.sub(sellFeeAmount);
                _transfer(sender, recipient, amount);
    ...
    function distributeReward(
        ...
        require(_loonPool != address(0), "!_loonPool");
        rewardPoolDistributed = true;
        // _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_loonPool, INITIAL_TOMB_POOL_DISTRIBUTION);
