...
contract BA2Token is IERC20, Ownable {
    ...
    address private projectAddress = 0x26367D10C4d64AA5C0D431C4B647F4e8b4194FaA;
    
    address public  marketAddr = 0x26367D10C4d64AA5C0D431C4B647F4e8b4194FaA;
    address public  dividendAddr =  0x26367D10C4d64AA5C0D431C4B647F4e8b4194FaA;
    address public  agentAddr =  0x26367D10C4d64AA5C0D431C4B647F4e8b4194FaA;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function setLPFeefenhong(uint amount) public onlyOwner {
        LPFeefenhong = amount;
    }
    ...
    function process(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
         ...
         if( amount < 1 * 10**16) {
             if (amount == 0) {
                 quitShare(shareholders[currentIndex]); 
    ...
    uint currentIndex2=0;
    function process2(uint256 gas, address nm) private {

        uint256 shareholderCount = shareholders.length;
        if (shareholders.length>0) {
            shareholderCount = shareholders.length-1;
        }
       ...
       // uint256 nowbanance = _tOwned[address(this)];
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
         ...
         // uint256 amount = nowbanance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(IERC20(uniswapV2Pair).totalSupply());
            if (nm!= address(0) && nm!=shareholders[currentIndex2]) {
                uint256 amount = IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex2]);

                if (amount ==0) {
                    quitShare(shareholders[currentIndex2]);   
                }
                gasUsed = gasUsed.add(gasLeft.sub(gasleft()));
                gasLeft = gasleft();
                if (amount ==0) {
    ...
    function _takeburnFee(
        ...
        if((_tTotal.sub(_tFeeTotal) ) <= 100000000 * 10**18)_burnFee = 0;
    ...
    function _transferSellStandard(
        ...
        _takeTransfer(sender,dividendAddr, tAmount,_dividendFee);
    ...
    function setMinPeriodDividend(uint256 value) public onlyOwner{
         minPeriodDividend = value ;
    }

    function getdividendKeyleng() public view returns  (uint256){
         return dividendKey.length;
    }
    ...
    function setAddrs(address _marketAddr,address _dividendAddr,address _agentAddr) onlyOwner  public  {
    ...
    function getDividendAddr() private {
        if(_tOwned[dividendAddr] > 0 && divdendTime.add(minPeriodDividend) <= block.timestamp) {
            delete dividendKey;
            uint256 dividendCount = dividendKeys.length;
            if(dividendCount == 0)return;
            uint256 nowbanance = _tOwned[dividendAddr];
            
            if(dividendCount>100) dividendCount= 100;
            for(uint256 i = 0; i< dividendCount;i++){
                if(_tOwned[dividendKeys[i]]>10000000*10**18){
                        dividendKey.push(dividendKeys[i]);
                }
            }
           
            uint256 dividendAmount =  nowbanance.div(dividendKey.length);
            
            for(uint256 i = 0; i<dividendKey.length;i++){
                _tOwned[dividendAddr] = _tOwned[dividendAddr].sub(dividendAmount);
                _tOwned[dividendKey[i]] = _tOwned[dividendKey[i]].add(dividendAmount);
                emit Transfer(dividendAddr,  dividendKey[i], dividendAmount);
            }
            divdendTime = block.timestamp;
        }
