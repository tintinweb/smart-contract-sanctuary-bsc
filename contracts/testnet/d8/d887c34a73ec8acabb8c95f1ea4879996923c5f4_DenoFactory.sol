...
contract DenoFactory is Context, Ownable {
    ...
    constructor(address address_) {
        _tokenAddress = address_; // in Ropsten
    ...
    function tokenBalanceOf(address account_) public view returns (uint256) {
        return iToken.balanceOf(account_);
    }

    function depositToken(uint256 amount_) public {
        iToken.approve(address(this), amount_);
        iToken.transferFrom(_msgSender(), address(this), amount_);
        _balances[_msgSender()] = _balances[_msgSender()].add(amount_);
    }

    function sendToken(uint256 amount_, address recipient_) public {
        require(_balances[_msgSender()] >= amount_, "ERC20: insufficient balance");
        iToken.approve(address(this), amount_);
        iToken.transferFrom(address(this), recipient_, amount_);
    }

    function spendToken(uint256 amount_, address recipient_) public {
