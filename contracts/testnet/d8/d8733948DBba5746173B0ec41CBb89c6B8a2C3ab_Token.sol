...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
	...
	function _withdrawDividendOfUser(address user) internal returns (uint256) {
		...
		if (_withdrawableDividend > 0) {
			...
			(bool success, bytes memory data) = usdt.call(abi.encodeWithSelector(0xa9059cbb, user, _withdrawableDividend));
...
contract yydsDividendTracker is DividendPayingToken, Ownable {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("yydsDividendTracker", "yyds_DividendTracker") {
		claimWait = 0;
	...
	function setyyds(address _yyds) external onlyOwner{
		yyds = _yyds;
	}

	function _transfer(address, address, uint256) internal pure override {
		require(false, "yydsDividendTracker: No transfers allowed");
	}

	function withdrawDividend() public pure override {
		require(false, "yydsDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main yyds contract.");
	}
...
contract Token is ERC20, Ownable {
	...
	address public usdt = 0x51A6088FA137D19a2D5CCb21458Ce7E5cB5ecA8c;   // bsctest
	
	address public wrap;

	bool private swapping;

	yydsDividendTracker public dividendTracker;
	address public marketingWallet = 0x8fc60904101Ba1d022Da6899824e0b7A17804168;	
	address public burnAddress = 0x000000000000000000000000000000000000dEaD;

	uint256 public maxTransactionAmount;   
	uint256 public maxHoldingAmount;   
	uint256 public swapTokensAtAmount = 2 * 10 ** uint256(decimals());   
	...
	constructor() ERC20("yyds", "yyds") {
		dividendTracker = new yydsDividendTracker();
		...
		dividendTracker.setyyds(address(this));
		...
		setWrap(0xB0b56a1e15588f0308BcB1094E538Eaeb6c34d2F);  // bsctestnet
	

		_mint(msg.sender, 1122 *  (10**decimals()));
	...
	function updatePancakeRouter(address newAddress) public onlyOwner {
		require(newAddress != address(pancakeRouter), "yyds: The router already has that address");
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "yyds: The dividend tracker already has that address");

		yydsDividendTracker newDividendTracker = yydsDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "yyds: The new dividend tracker must be owned by the yyds token contract");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "yyds: Account is already the value of 'excluded'");
	...
	function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromPeriodLimit[account] != excluded, "yyds: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTxLimit[account] != excluded, "yyds: Account is already the value of 'excluded'");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "yyds: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "yyds: Cannot update gasForProcessing to same value");
	...
	function _transfer(
		...
		// 	require(getAccountPeriodSellVolume(from).add(amount) <= balanceOf(from).mul(99).div(100), "Sell limit!");
		// }
		// if (!_isExcludedFromPeriodLimit[from]) {
		...
		if(takeFee) {
			...
			if (from == pancakePair) {
                ...
                require(amount <= 3 * (10**decimals()), "max 3 tokens per purchase");
	...
	function swapAndDistributeUsdt(uint256 tokens) public {
		...
		if(success) {
			...
			emit SwapAndSendTo(balance, "DIVIDENDS");
