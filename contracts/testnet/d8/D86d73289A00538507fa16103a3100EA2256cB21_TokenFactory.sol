...
contract TokenFactory is NonblockingLzApp {
    ...
    function mintToken(
        ...
        bytes memory payload = _getReqPayload(1, coll, URI, tokenOwner, mintPrice, creator, asset);
    ...
    function _nonblockingLzReceive(
        ...
        (uint256 act, address rec, uint256 paid, address asset, address cre, address coll, string memory URI) = abi.decode(_payload, (uint256, address, uint256, address, address, address, string));
        URI;
        ...
        if (isReq) {
            _onMintReq(_payload, srcId);
        }
    }

    function _onMintReq(bytes memory _payload, uint16 chId) private {
        (uint256 act, address rec, uint256 paid, address asset, address cre, address coll, string memory URI) = abi.decode(_payload, (uint256, address, uint256, address, address, address, string));
        act;
        ...
        try nft.mint(rec, URI) {
            emit Minted(coll, rec);
            _resAction(cre, price, asset, cre, chId);
        } catch {
            emit NewRefund(coll, rec);
            _resAction(rec, price, asset, cre, chId);
        }
    }
    ...
    function _resAction(address addr, uint256 price, address asset, address cre, uint16 chId) private {
        _lzAction(_getResPayload(2, addr, price, asset, cre), chId, 0);
    }
    ...
    function estimateFees(string memory chSym, address coll, string memory URI, address rec, uint256 price, address cre, uint gas, address asset) external view returns (uint) {
        bytes memory payload = _getReqPayload(1, coll, URI, rec, price, cre, asset);
    ...
    function _getReqPayload(uint256 act, address coll, string memory URI, address rec, uint256 price, address cre, address asset) private pure returns (bytes memory) {
        return abi.encode(act, rec, price, asset, cre, coll, URI);
    }

    function _getResPayload(uint256 act, address rec, uint256 price, address asset, address cre) private pure returns (bytes memory) {
        return abi.encode(act, rec, price, asset, cre, address(0), "");
    }
