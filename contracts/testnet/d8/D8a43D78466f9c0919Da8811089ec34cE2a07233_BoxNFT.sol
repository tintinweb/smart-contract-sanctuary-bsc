...
function openBox(uint256 tokenId) external whenNotPaused returns (uint256 characterId,uint256 diceId,uint256 randomCharacter,uint256 randomDice){
  ...
  /**
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        address sender=this.ownerOf(tokenId);
        Box memory box = boxes[sender][tokenId];
        string memory rankText="d";
        if(box.rank==2){
          rankText="c";
        }else if(box.rank==3){
          rankText="b";
        ...
        string memory baseURI ="https://static-cdn.billionaireplus.io/metadata/box/";
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, rankText,".png")) : "";
    }    
}
