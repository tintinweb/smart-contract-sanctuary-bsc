...
contract KronosStaking is Ownable {
        ...
        returns (bool)
    {
        ...
        warmupInfo[_recipient] = Claim({
            ...
            gons: info.gons.add(IMemo(sKronos).gonsForBalance(_amount)),
    ...
    /**
        @notice retrieve sKronos from warmup
    ...
    function claim(address _recipient) public {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            IWarmup(warmupContract).retrieve(
                _recipient,
                IMemo(sKronos).balanceForGons(info.gons)
            );
        }
    }

    /**
        @notice forfeit sKronos in warmup and retrieve Kronos
     */
    ...
    function rebase() public {
        if (epoch.endTime <= uint32(block.timestamp)) {
            IMemo(sKronos).rebase(epoch.distribute, epoch.number);
            ...
            uint256 staked = IMemo(sKronos).circulatingSupply();
