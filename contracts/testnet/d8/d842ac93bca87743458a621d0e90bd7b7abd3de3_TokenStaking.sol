...
contract TokenStaking is Ownable {
    ...
    function transferTokens() public onlyOwner {
        token.transfer(address(this), getContractBalacne());
    }
    ...
    function setWallet(address _treasury, address _devWallet, address _marketingWallet) public onlyOwner {
        require(
            _treasury != address(0),
        ...
        require(
            ...
            "Marketing address is the zero address."
        );
    ...
    function invest(uint256 _amount, address refferAddress) public {
        ...
        if (
            users.timeForCalculateReward + users.expireTime >
            block.timestamp
        ) {
            users.expireTime =
                (users.expireTime + users.timeForCalculateReward) -
