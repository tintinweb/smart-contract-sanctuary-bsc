...
interface IDividendDistributor {
    ...
    function setNewDividend(address account)external;

    function updateMaxNoOfTokensToGetReward(uint256 newAmount)external;
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public maxHodlingvalueToGetReward;
    ...
    function setNewDividend(address account)
        external
        override
        onlyToken
    {
    ...
    function updateMaxNoOfTokensToGetReward(uint256 amount)external onlyToken{
        maxHodlingvalueToGetReward = amount;
    }
        ...
        returns (bool)
    {
        return
            ...
            shares[shareholder].amount >= maxHodlingvalueToGetReward;
    }
...
contract TrustShades is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Trust Shades";
    ...
    uint256 public _maxNoOfTokensToGetReward = 100_000 * (10 **_decimals);
    ...
    uint256 internal reflectionFee = 5;
    uint256 internal marketingFee = 4;
    uint256 internal totalFee = 10;

    uint256 internal sellliquidityFee = 1;
    uint256 internal sellreflectionFee = 10;
    ...
    address public marketingFeeReceiver =
        0xa996e25bFc3FEE6066F5b038F1E55FDebF044D32;
    ...
    constructor() Auth(address(msg.sender)) {
        ...
        distributor.updateMaxNoOfTokensToGetReward(_maxNoOfTokensToGetReward);
    ...
    function _transferFrom(
        ...
        require(!_isBlackList[sender] && !_isBlackList[recipient],"You are blacklisted you can't make transaction's");
        if (inSwap) {
    ...
    function isWalletLimitExempt(address account) external view returns (bool) {
        return isWalletHoldingLimitExempt[account];
    }
    ...
    function _isExemptFromDividend(address account)external view returns(bool){
    ...
    function setNewDividend(address newDividendAddress)external authorized{
        distributor.setNewDividend(newDividendAddress);
        RWRD = newDividendAddress;
    }

    function UpdateMaxHoldingToGetReward(uint256 amount)external authorized{
        _maxNoOfTokensToGetReward = amount * (10 ** _decimals);
        distributor.updateMaxNoOfTokensToGetReward(_maxNoOfTokensToGetReward);
    }
    ...
    function setBuyFees(
        ...
        totalFee = _buyliquidityFee.add(_buyreflectionFee).add(
            _buymarketingFee
        );
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator / 5, "You can't set more than 20%");
    ...
    function setSellFees(
        ...
        totalFee = _SellliquidityFee.add(_SellreflectionFee).add(
            _SellmarketingFee
        );
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator / 5, "You can't set more than 20%");
