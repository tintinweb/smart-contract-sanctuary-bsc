    ...
    ReentrancyGuardUpgradeable
{
    ...
    event OfferCancelled(uint256 id, uint256 amount, uint256 remaining, address caller);
    ...
    function createCollection(
        ...
        IAlturaNFTV2(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function makeOffer(
        ...
        require(_price > 0, "invalid price");
        require(_expire > 0 && _expire > block.timestamp, "invalid expire");
    ...
    function acceptOffer(uint256 _id, uint256 _amount) external payable nonReentrant {
        ...
        Item memory item = items[offer.item_id];
        require(item.bValid, "invalid Item id");
        ...
        items[offer.item_id].balance = items[offer.item_id].balance.sub(_amount);
        items[offer.item_id].totalSold = items[offer.item_id].totalSold.add(_amount);
        ...
        emit Swapped(offer.owner, item.owner, offer.item_id, offer.collection, offer.token_id, _amount);
        emit OfferMatched(
            _id,
            offer.item_id,
            _amount,
    ...
    function cancelOffer(uint256 _id, uint256 _amount) external {
        ...
        emit OfferCancelled(_id, _amount, remaining, msg.sender);
    }
