...
contract NoFaceCoin is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    uint256 private totalFees = RewardTokenRewardsFee.add(liquidityFee).add(marketingFee);

    address public _marketingWalletAddress = 0x3069C1c2B16935C5cD33f07Ab725a513f1aEC7EE;
    ...
    constructor() BEP20("NoFaceCoin", "NFC") 
    {
    ...
    function setRwardTokenAddress(address _rewardToken) external onlyOwner
    {
    ...
    external onlyOwner 
    {
        RewardTokenRewardsFee = _rewardFee;
    ...
    {
        require(pair != uniswapV2Pair, "The PanRewardTokenSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function swapAndSendDividendsAndFee(uint256 tokens) private
    {
        ...
        uint256 _totalFee = RewardTokenRewardsFee.add(marketingFee);
        uint256 newBalance = IBEP20(RewardToken).balanceOf(address(this));
        uint256 dividends = newBalance.mul(RewardTokenRewardsFee).div(_totalFee);
        uint256 forMarketing = newBalance.sub(dividends);
        bool success = IBEP20(RewardToken).transfer(address(dividendTracker), dividends);
        IBEP20(RewardToken).transfer(_marketingWalletAddress, forMarketing);
        if (success) 
        {
            dividendTracker.distributeRewardTokenDividends(dividends);
            emit SendDividends(tokens.mul(RewardTokenRewardsFee).div(_totalFee), dividends);
        }
    }
...
contract RewardDividendTracker is Ownable, DividendPayingToken 
{
    ...
    function withdrawDividend() public pure override {
        require(false, "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Reward contract.");
    }
