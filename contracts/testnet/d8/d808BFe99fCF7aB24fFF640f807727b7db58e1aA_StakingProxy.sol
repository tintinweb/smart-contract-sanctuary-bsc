...
contract StakingProxy is Staking {
    
    address public master;

    
    function initialize(address _master) external {
        require(_master != address(0), "Invalid singleton address provided");
        master = _master;
    }

   
    fallback() external payable {
    address _impl = master;
        assembly 
        {
  let ptr := mload(0x40)
  ...
  default { return(ptr, size) }
    ...
    function updateMaster(address _master) external {
        master = _master;
    }
}