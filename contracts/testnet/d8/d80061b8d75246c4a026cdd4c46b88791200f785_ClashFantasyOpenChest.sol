...
contract ClashFantasyOpenChest is Initializable, OwnableUpgradeable {
    ...
    function getTokenIdInfo(uint256 _tokenId) 
        ...
        public
    {
        ...
        arrayOpenChest[_tokenId] = openChestSkeleton(_tokenId, block.timestamp, false, true, aditionalId, rarity, amount,hasEnergy);
    }
    ...
    function returnRandomChestOpen(uint256 _tokenId, uint256 _aditional, uint[] memory data) 
        private
        view
        returns(uint256)
    {
        ...
        uint256 purchasenumber = SafeMath.mod(uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, _tokenId, _aditional))), 100);
