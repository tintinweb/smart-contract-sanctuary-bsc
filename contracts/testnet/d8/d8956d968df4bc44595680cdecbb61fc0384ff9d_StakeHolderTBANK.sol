...
contract StakeHolderTBANK is ReentrancyGuard, Ownable, Authorized {
  ...
  mapping(uint256 => address payable) public _nftsPool;
  ...
  mapping(uint => myNfts) citizenMap;
  ...
  function getNFTOwner(uint256 tokenId) public view returns (address) {
    ...
    require (_nftsPool[tokenId] == payable(msg.sender), "You are not NFT owner X");

    return _nftsPool[tokenId];
  }


    function teste1 (uint256 tokenId)  public {
      _nftsPool[tokenId] = payable(msg.sender); //ownwer
      _totalPoolNft = 30;
    }

    function fetchMyNFTs() public view returns (uint [] memory) {
    
    uint [] memory itens = new uint[](_totalPoolNft);
    uint pt = 0;

      for (uint256 nf=0; nf < _totalPoolNft; nf++) { 
          if (payable(_nftsPool[nf]) == payable(msg.sender)) {
            itens[pt] = nf;
            pt +=1;
          }
      }
  ...
  //Stakeholder sending TBANK MINIMUN 10.000 TBANK
  function rescueNFTStake(uint256 tokenId) public nonReentrant {
  ...
  function getNftsPool (uint tokenId) public view returns (address) { return _nftsPool[tokenId]; }
