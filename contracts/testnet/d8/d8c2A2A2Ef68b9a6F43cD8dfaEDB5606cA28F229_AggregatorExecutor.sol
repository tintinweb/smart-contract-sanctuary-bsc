...
contract AggregatorExecutor is Ownable {
    ...
    //=========== external functions ============//
    function swap(SwapStep[] calldata _swaps, address _receiver) external payable {
        address _tokenIn = _swaps[0].fromToken;
        if(_tokenIn == BNB) {
            WBNB.deposit{value: _swaps[0].amountIn}();
        }
        IERC20(_tokenIn).safeTransferFrom(msg.sender, address(this), _swaps[0].amountIn);
