...
contract NDBcoinV4 is IBEP20, Auth {
    ...
    function _transferFrom(
        ...
        if (treasuryWallet[sender]==true)    
        {
            ...
            (referrer,referralCommissionRate,joinCommissionRate) = NDBReferral.getReferrer(recipient); // to is address of NDB buyer
            if (referrer!= address(0) && referralCommissionRate>0){
                ...
                if (joinCommissionRate > 0 && isFirstPurchase[recipient]==false)
                {
                    uint256 joinCommissionAmount = amountReceived.mul(joinCommissionRate*100).div(10000);
                    _balances[sender] = _balances[sender].sub(
                        joinCommissionAmount,
                        "Insufficient Balance"
                    );
                    isFirstPurchase[recipient] =true;
                    _balances[recipient] = _balances[recipient].add(joinCommissionAmount);
                    emit Transfer(sender, recipient, joinCommissionAmount);
                }               
                
                NDBReferral.recordReferralCommission(referrer,recipient,commissionAmount);
