...
import "./interfaces/IMangaNft.sol";
import "./interfaces/IMangaNftRandomizer.sol";

contract IDO is InvestorManager {
    using Address for address;
    IERC20 public manToken;
    IMangaNft public nftToken;
    uint256 public TOKEN_UNIT = 1e9;
    address public globalWallet;

    constructor(IERC20 _manToken, IMangaNft _nftToken) {
        manToken = _manToken;
        nftToken = _nftToken;
        globalWallet = owner();
    ...
    function setPayoutEachClaim(uint256 _PAYOUT_EACH_CLAIM) public onlyRole(ADMIN_ROLE) {
        PAYOUT_EACH_CLAIM = _PAYOUT_EACH_CLAIM;
    }

    uint256 public CLAIM_FEE = 0.00002 ether;

    function setClaimFee(uint256 _CLAIM_FEE) public onlyRole(ADMIN_ROLE) {
        CLAIM_FEE = _CLAIM_FEE;
    }

    function setGlobalWallet(address _globalWallet) public onlyRole(ADMIN_ROLE) {
    ...
    function claim(address presenterAddress) public payable virtual {
        ...
        payable(globalWallet).transfer(CLAIM_FEE);
    ...
    function unclaim(address account) public onlyRole(ADMIN_ROLE) {
    ...
    function payWithCommission(address receiver, uint256 value) internal {
        ...
        for (uint256 index = 0; index < payments.length; index++) {
            ...
            manToken.transfer(payment.receiver, payment.value);
    ...
    uint256 public MAN_PLUS_CASE_AMOUNT = 1 * 1e9;

    function isAbleToPayManPlusNft() public view returns (bool) {
        return manToken.balanceOf(address(this)) >= MAN_PLUS_CASE_AMOUNT && MAN_PLUS_CASE_AMOUNT > 0;
    }

    function setManPlusNftcaseReward(uint256 _MAN_PLUS_CASE_AMOUNT) public onlyRole(ADMIN_ROLE) {
        MAN_PLUS_CASE_AMOUNT = _MAN_PLUS_CASE_AMOUNT;
    }

    function payManPlusNft() internal {
        manToken.transfer(_msgSender(), MAN_PLUS_CASE_AMOUNT);
        nftToken.safeMintObo(_msgSender());
    }

}

contract Types {
    enum RewardType {NONE, ERC1155, COIN, USDT, MAN, MON, NOTHING, MAN_PLUS_NFT}
}

contract Randomizer is Ownable,
AccessControlEnumerable, ERC1155Holder, ERC721Holder {
    ...
    mapping(address => Types.RewardType) public specialGuesses;
    bool public isAllowClaimCase;
    ...
    function setAddressClaims(address[] memory _address, Types.RewardType _type) public onlyRole(ADMIN_ROLE) {
        uint256 length = _address.length;
        for (uint256 index = 0; index < length; index++) {
            specialGuesses[_address[index]] = _type;
        }
    }

    function setAllowClaimCase(bool _isAllowClaimCase) public onlyRole(ADMIN_ROLE) {
        isAllowClaimCase = _isAllowClaimCase;
    }

    function setRates(uint256 _ERC1155_RATE, uint256 _MAN_RATE, uint256 _COIN_RATE, uint256 _USDT_RATE, uint256 _MON_RATE) public onlyRole(ADMIN_ROLE) {
    ...
    function get(address investor) public view returns (Types.RewardType) {
        uint256 randomIn100 = getRandomIn100(block.timestamp, _msgSender());
        if (specialGuesses[_msgSender()] != Types.RewardType.NONE && isAllowClaimCase) {
            return specialGuesses[_msgSender()];
        }
...
contract IDOWithReward is IDO {
    ...
    constructor(IERC20 _man, IERC20 _usdt, IERC20 _mon, IERC1155 _erc1155ForReward, Randomizer _randomizer, IMangaNft _nft) IDO(_man, _nft) {
    ...
    function setRandomizer(Randomizer _randomizer) public onlyRole(ADMIN_ROLE) {
        randomizer = _randomizer;
    }

    event Reward(Types.RewardType rewardType);

    function claim(address presenterAddress) public payable override virtual {
        super.claim(presenterAddress);

        Types.RewardType randomRewardType = randomizer.get(address(0));
    ...
    uint256 public COIN_REWARD_AMOUNT = 0.00002 ether;

    function isAbleToPayCoin() public view returns (bool) {
        return address(this).balance >= COIN_REWARD_AMOUNT;
    }

    function setCoinRewardAmount(uint256 _COIN_REWARD_AMOUNT) public onlyRole(ADMIN_ROLE) {
    ...
    function setUsdtRewardAmount(uint256 _USDT_REWARD_AMOUNT) public onlyRole(ADMIN_ROLE) {
    ...
    function payMon() internal {
        mon.transfer(_msgSender(), MON_REWARD_AMOUNT);
...
contract IDORanking is IDOWithReward {
    ...
    constructor(IERC20 _man, IERC20 _usdt, IERC20 _mon, IERC1155 _erc1155ForReward, Randomizer _randomizer, IMangaNft _nft)
    IDOWithReward(_man, _usdt, _mon, _erc1155ForReward, _randomizer, _nft) {}

    function claim(address presenterAddress) public payable override virtual {
        super.claim(presenterAddress);
        if (presenterAddress == address(0) || presenterAddress == _msgSender()) return;
        if (refCounts[presenterAddress] == 0) userWallets.push(presenterAddress);
        refCounts[presenterAddress] += 1;
    }

    function getRanking(uint256 fromIndex, uint256 toIndex) public view returns (Ranking[] memory) {
        uint256 length = userWallets.length;
        Ranking[] memory emptyResponse = new Ranking[](0);
        ...
        Ranking[] memory result = new Ranking[](normalizedToIndex - fromIndex + 1);
        for (uint256 index = fromIndex; index <= normalizedToIndex; index++) {
            result[index - fromIndex] = Ranking(userWallets[index], refCounts[userWallets[index]]);
        }
        return result;
    }

    function getTotalRankingCount() public view returns (uint256) {
...
contract InvestorManager is Ownable, AccessControlEnumerable, ERC1155Holder, ERC721Holder {
  bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
  ...
  function createInvestor(address investorAddress, address presenterAddress) internal {
    ...
    investorIds.push(investorAddress);
    emit CreateInvestor(investorAddress, presenterAddress);
  }

  // WARNING: public -> internal
  function createNormalUser(address investorAddress, address presenterAddress) public virtual {
  ...
  function getInvestors(uint256 fromIndex, uint256 toIndex) public view returns (Investor[] memory) {
...
interface IMangaNft is IERC721, IERC721Burnable {
  function setTokenURI(uint256 tokenId, string memory _tokenURI) external;
  function safeMintObo(address to) external;
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.9;

interface IMangaNftRandomizer {
    function getRandomEggletType() external pure returns (uint256);
}