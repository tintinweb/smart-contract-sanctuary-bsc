...
import {GroupOrder} from "../common/Structs.sol";
...
contract CancelHandler is Upgradable {
    ...
    /**
     * @dev Handle logic of cancel function
     * @param _data (0) groupId, (1) unit price
     * @param _addr (0) seller, (1) signer, (2) tokenAddress
     * @param _str (0) internalTxID
     * @param _signatures (0) groupOrderSignature    
     */
    function cancelSaleOrder(
    ...
    ) public reentrancyGuard onlyAdmins notBlocked {
        
        require(signer == _addr[1], "CancelHandler: Only signer signature");
        require(isAdmin(msg.sender), "CancelHandler: Only super admin or admin");
        ...
        GroupOrder memory groupOrder = GroupOrder(
            _data[0],
            _data[1],
            _addr[0],
            _addr[2]
        ...
        require(
            SignatureUtils(signatureUtilsAddress).verifyGroupOrder(
                groupOrder,
                _signatures[0],
                _addr[1]
            ),
    ...
    /**
     * @dev Handle logic of cancel function
     * @param _data (0) saleOrderId, (1) unit price, (2) royaltyPercentage
     * @param _ids list of NFT ids
     * @param _addr (0) seller, (1) signer, (2) tokenAddress
     * @param _str (0) internalTxID
    ...
    function cancelResaleOrder(
        ...
        string[] memory _str,
        bytes[] memory _signatures
    ) public reentrancyGuard notBlocked{
        ...
        require(
            !invalidSaleOrder[_signatures[0]],
            "CancelHandler: Resale Order was canceled"
        );

        uint256 idsLength = _ids.length;
        for (uint256 i = 0; i < idsLength; i++) {
            require(isOwnerOf(_ids, msg.sender), "CancelHandler: Caller is not owner of the NFTs");
        }


        BundleSaleOrder memory bundleSaleOrder = BundleSaleOrder(
            _ids,
            _data[0],
            _data[1],
            _addr[0],
        ...
        require(
            SignatureUtils(signatureUtilsAddress).verifyBundleSaleOrder(
                bundleSaleOrder,
                _signatures[0],
                _addr[1]
            ),
