...
contract Lottery is Ownable {
    ...
    function buy(
        ...
        uint256 shareStakeAmount;
        uint256 sharePurchaseAmount; 
        uint256 shareAffiliateAmount; 
        uint256 takeTokenAmount; 
        {
            if(_seriType == 1) {
                ...
                shareAffiliateAmount = assetAmount.mul(share2AffiliateCO).div(1000000);
                takeTokenAmount = assetAmount.mul(share2OperatorCO).div(1000000);
            }
        }
        ...
        uint256 assetRemain;
        {
            assetRemain = assetAmount.sub(postRemain).sub(shareAffiliateAmount).sub(takeTokenAmount);
            if(_seriType == 1) assetRemain = assetRemain.sub(shareStakeAmount).sub(sharePurchaseAmount);
        }        
