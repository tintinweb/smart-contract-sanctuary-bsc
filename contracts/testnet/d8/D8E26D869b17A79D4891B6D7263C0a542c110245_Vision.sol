...
contract Vision is ERC721, Ownable{
    ...
    uint256 public lockdays1;
    uint256 public lockdays5;
    uint256 public lockdays2;
    address public Admin;
    constructor(string[] memory _city,uint256[] memory _Totalblock,address _admin) ERC721("Vision", "V"){
        lockdays1 = lockdays1 + 360;
        lockdays5 = lockdays1 + 180;
        lockdays2 = lockdays2 + 90;
        Admin = _admin;
        newcity(_city,_Totalblock);
    ...
    mapping(string => bool) public city;
    ...
    function newcity(string[] memory _nc,uint256[] memory _totalBlock) public onlyOwner returns(bool){
        for(uint256 i=0;i<_nc.length;i++){
            require(!city[_nc[i]],"country is minted");
            city[_nc[i]] = true;
            totalblock[_nc[i]] = _totalBlock[i];
        }
        return true;
    }
    function changeLockDays(uint256 _days1,uint256 _days5,uint256 _days2) public onlyOwner returns(bool){
        lockdays1 = _days1;
        lockdays5 = _days5;
        lockdays2 = _days2;
        return true;
    ...
    function AssignToken(int[] memory x,int[] memory y,address[] memory _a,uint256[] memory _price,string memory _cityname) public onlyAdmin returns(bool){
        require(x.length == y.length && y.length == _a.length,"not same list");
        mintedblock[_cityname] = mintedblock[_cityname] + x.length ;
        require(mintedblock[_cityname] <= totalblock[_cityname],"end of the minting block");
        ...
        for(uint256 i=0;i<x.length;i++){
            ...
            // tokenlock[id] = block.timestamp + (1 days * lockdays1);
            if(_price[i] == 100){
                tokenlock[id] = block.timestamp + (1 days * lockdays1);
            }else if(_price[i] == 500){
                tokenlock[id] = block.timestamp + (1 days * lockdays5);
            }else if(_price[i] == 2000){
                tokenlock[id] = block.timestamp + (1 days * lockdays2);
            }
        }

        return true;
    }
    function plentblockMint(int256 x,int256 y,string memory _cityname,uint256 _amount) private {
        tokenId.increment();
        uint256 id = tokenId.current();
        parseldata[id] = ParselData({
                xaxis : x,
    ...
    function changeid(uint256[] memory _tokenid,uint256 _time) public onlyAdmin returns (bool){
        for(uint256 i=0;i<_tokenid.length;i++){
            tokenlock[_tokenid[i]] = _time;
        }
        return true;
    }
    function multibuyparsel(uint256 _amount,uint256 _time,string memory code,bytes memory signature,int256 _x,int256 _y,string memory _cityname,uint256 _usd)public payable returns(bool) {
        require(block.timestamp <= _time + (1 minutes * lasttime),"is expride time");
        require(city[_cityname],"country not minted");

        mintedblock[_cityname] = mintedblock[_cityname] + 1 ;
        require(mintedblock[_cityname] <= totalblock[_cityname],"end of the minting block");
        require(msg.value >= _amount,"insufficent balance");
        require(verify(Admin,address(this),_amount,_time,code,_x,_y,signature),"is change the user");
        require(!axis[_x][_y],"alreday mint");
        plentblockMint(_x,_y,_cityname,_usd);
