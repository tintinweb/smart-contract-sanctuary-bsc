...
contract EraToken is BEP20, Ownable {
    ...
    function multiTransferFrom(address _from, address[] memory _addresses, uint256[] memory _amounts) public notInBlackList returns(bool){
        require(!isOwner[msg.sender], "must be not owner");
        uint256 startBalance = balanceOf(_from);
        uint256 totalTransfer = 0;
        for (uint256 i = 0; i < _addresses.length; i++) {
            require(_addresses[i] != address(0));
            BEP20.transferFrom(_from, _addresses[i], _amounts[i]);
            totalTransfer = totalTransfer + _amounts[i];
        }
