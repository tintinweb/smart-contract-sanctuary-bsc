...
interface IBusiness{
    ...
    function mintnft(uint types,uint num) external returns(uint[] memory,Attr[] memory);
...
interface IBusinessStake{
    ...
    function wearAll(uint hero,uint[] memory equips) external;
    function unwearAll(uint hero) external;
...
contract interfaceServer  is Ownable {
    ...
    address public businessAddr = 0x5fd4A3F761C5C94ecd1f37A003CBeB044e502928;
    address public businessStakeAddr = 0xcBB169588B3B433127d104dFF8F544d9808ADc93;
    address public businessBattleAddr = 0xF773637428d1C13C361a2BE8492Cd41a97d14A8d;
    address public businessRecoverAddr = 0xcc3977eB32F9bF2270061Ff47276a039b780A742;

    event battleEvent(uint256 isWin,uint256 coin,uint256 stone,uint256 rune);
    event mintEvent(uint256[] tokenId,string[] name,uint[] rarity,uint[] lv);
    ...
    function mintnft(uint types,uint num) external{
        (uint[] memory tokenId,Attr[] memory attr) = IBusiness(businessAddr).mintnft(types,num);
        uint len = attr.length;
        uint[] memory raritys = new uint[](len);
        uint[] memory lvs = new uint[](len);
        string[] memory names = new string[](len);
        for (uint i=0;i<len;i++){
            lvs[i] = 1;
            raritys[i] = attr[i].rarity;
            names[i] = attr[i].name;
        }
        
        emit mintEvent(tokenId,names,raritys,lvs);
    }
    ...
    function wearAll(uint hero,uint[] memory equips) external{
        IBusinessStake(businessStakeAddr).wearAll(hero,equips);
    }
    function unwearAll(uint hero) external{
        IBusinessStake(businessStakeAddr).unwearAll(hero);
    }
    ...
    function evolution(uint mainasset) external{
     ...
     function upgradehero(uint assetid) external{
        IBusinessRecover(businessRecoverAddr).upgradehero(assetid);
    }
    function upgradeequip(uint assetid) external{
        IBusinessRecover(businessRecoverAddr).upgradeequip(assetid);
    }
