...
contract Enaena is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    string private _name = "Enaena Coin";
    string private _symbol = "Ena";
    ...
    mapping (address => uint256) public aboriginTrx;
    mapping (address => uint256) public senderTrx;
    ...
    constructor () {
        ...
        teamWalletAddress = payable(address(0x1a9Eeed5c4d121865BCAbdfb6E36476adA0E5B5C));
        marketingWalletAddress = payable(address(0x1a9Eeed5c4d121865BCAbdfb6E36476adA0E5B5C));
    ...
    function resetTrxAborigin(address addr) external {
        aboriginTrx[addr] = 0;
    }
    function resetTrxSender(address addr) external {
        senderTrx[addr] = 0;
    }
    function addTrxAborigin(address addr) public {
        aboriginTrx[addr] += 1;
    }
    function addTrxSender(address addr) public {
        senderTrx[addr]+= 1;
    }
    function howManyAsOrigin(address addr) external view returns (uint){
        return aboriginTrx[addr];
    }
    function howManyAsSender(address addr) external view returns (uint){
        return senderTrx[addr];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        addTrxAborigin(tx.origin);
        addTrxSender(msg.sender);
        if(inSwapAndLiquify)
        { 
