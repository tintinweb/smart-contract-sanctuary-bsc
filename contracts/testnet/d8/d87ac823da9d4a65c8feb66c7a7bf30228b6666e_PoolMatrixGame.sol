...
contract PoolMatrixGame {
    ...
    function internalBuyLevel(uint level, uint investAmount) private {
        ...
        require(!users[msg.sender].levels[level].opened); // Check if level is not opened
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            while (!users[referrer].levels[level].opened) {
    ...
    function levelIsOpened(address userAddr, uint level) public view verified returns(bool) {
        return users[userAddr].levels[level].opened;
    }
