...
contract TIPToken is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "TTIP TOKEN";
    string private _symbol = "TIPTOKEN";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0xFbC100B1188464f52b42872A7A6F841C27Ac04aC); 
    address payable public BurnedWalletAddress = payable(0xC71667b88F012b259367557a101ac17B1DC95D06); 
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public travelDogeAddress = address(0x069bB2BB3235623A2E0aEAc8D2980417E9E92c2F);
    address  public wapV2RouterAddress = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);

    uint256 public genesisBlock;
    uint256 public coolBlock = 21;
   
    uint256 private _limitBuy =4;
    uint256 private _limitBuyBlock = 31;
    
    uint256 private minimumTokensBeforeSwap = 488000000 * 10**_decimals; 
    uint256 _saleKeepFee = 1000;

    uint256 burnTlDogeFee = 25;
    uint256 maketBnbFee = 75;
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBBurned = amountReceived.mul(burnTlDogeFee).div(100) ;
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBBurned);

        if(amountBNBBurned > 0 )
        {
            swapEthForToken(amountBNBBurned);
        }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
     ...
     function swapEthForToken(uint256 ethAmount) private{
        ...
        path[1] = address(travelDogeAddress);
    ...
    function getIsKillBlock() public view returns (bool) 
    {   
        if(genesisBlock == 0 )
        {
            return true;
        }
        if (block.number <= ( genesisBlock + coolBlock) )
        {
            return true;
        }
