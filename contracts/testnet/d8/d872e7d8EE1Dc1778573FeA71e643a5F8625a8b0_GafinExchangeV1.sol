...
contract ExchangeOrdersHolderV1 {
    ...
    function add(ExchangeDomainV1.Order calldata order) external {
        ...
        orders[key] = OrderParams(
            ...
            order.buying,
            FeesParams(
                order.sellerFee.beneficiaryFee,
                order.sellerFee.refFee,
                order.sellerFee.royaltyFee,
                order.sellerFee.royaltyWallet
            )
        );
    }
        ...
        returns (bool)
    {
        ...
        return
            ...
            params.sellerFee.beneficiaryFee == order.sellerFee.beneficiaryFee &&
            params.sellerFee.refFee == order.sellerFee.refFee &&
            params.sellerFee.royaltyFee == order.sellerFee.royaltyFee &&
            params.sellerFee.royaltyWallet == order.sellerFee.royaltyWallet;
    }
...
contract GafinExchangeV1 is Ownable, ExchangeDomainV1 {
    using SafeMath for uint256;
    using UintLibrary for uint256;
    ...
    function exchange(
        ...
        uint256 paying = order.buying.mul(buyOrder.amount).div(order.selling);
        ...
        if (order.key.buyAsset.assetType == AssetType.ETH) {
            validateEthTransfer(
                paying,
                buyOrder.buyerFee.beneficiaryFee +
                    buyOrder.buyerFee.refFee +
                    buyOrder.buyerFee.royaltyFee
            );
        }
        ...
        uint256 beneficiaryFee = order.sellerFee.beneficiaryFee;
        uint256 refFee = order.sellerFee.refFee;
        if (buyOrder.refWallet == address(0x0) || buyOrder.refWallet == buyer) {
            beneficiaryFee += refFee;
            refFee = 0;
        }
        FeeAddress[] memory sellerFeeAddresses = new FeeAddress[](3);
        // fee for selling
        sellerFeeAddresses[0] = FeeAddress(beneficiary, beneficiaryFee);
        // fee for ref
        sellerFeeAddresses[1] = FeeAddress(buyOrder.refWallet, refFee);
        // fee for royalty
        sellerFeeAddresses[2] = FeeAddress(
            order.sellerFee.royaltyWallet,
            order.sellerFee.royaltyFee
        );

        beneficiaryFee = buyOrder.buyerFee.beneficiaryFee;
        refFee = buyOrder.buyerFee.refFee;
        if (buyOrder.refWallet == address(0x0) || buyOrder.refWallet == buyer) {
            beneficiaryFee += refFee;
            refFee = 0;
        }
        FeeAddress[] memory buyerFeeAddresses = new FeeAddress[](3);
        // fee for buying
        buyerFeeAddresses[0] = FeeAddress(
            beneficiary,
            buyOrder.buyerFee.beneficiaryFee
        );
        // fee for ref
        buyerFeeAddresses[1] = FeeAddress(buyOrder.refWallet, refFee);
        // fee for royalty
        buyerFeeAddresses[2] = FeeAddress(
            buyOrder.buyerFee.royaltyWallet,
            buyOrder.buyerFee.royaltyFee
        );
        ...
        emitBuy(order, buyOrder.amount, buyer);
    }
    ...
    function validateBuyerFeeSig(
        Order memory order,
        Fees memory buyerFee,
        address refWallet,
        Sig memory sig
    ) internal view {
        require(
            prepareBuyerFeeMessage(order, buyerFee, refWallet).recover(
                sig.v,
                sig.r,
                sig.s
            ) == buyerFeeSigner,
    ...
    function prepareBuyerFeeMessage(
        ...
        return keccak256(abi.encode(order, fee, refWallet)).toString();
    }
    ...
    function transferWithFeesPossibility(
        ...
        Transfer memory transferObj,
        FeeAddress[] memory sellerFee,
        FeeAddress[] memory buyerFee,
        Asset memory secondType
    ) internal {
        if (!transferObj.hasFee) {
            transfer(
                firstType,
                transferObj.value,
                transferObj.from,
    ...
    function transferFeeToBeneficiary(
        ...
        address from,
        uint256 total,
        FeeAddress[] memory sellerFeeAddresses,
        FeeAddress[] memory buyerFeeAddresses
    ) internal returns (uint256) {
        uint256 restValue = total;

        for (uint8 i = 0; i < sellerFeeAddresses.length; i++) {
            (uint256 tempRestValue, uint256 tempFeeValue) = subFeeInBp(
                restValue,
                total,
                sellerFeeAddresses[i].fee
            );
            restValue = tempRestValue;
            ...
            if (tempFeeValue > 0) {
                transfer(
                    ...
                    tempFeeValue,
                    from,
                    sellerFeeAddresses[i].wallet
                );
            }
        }

        for (uint8 i = 0; i < buyerFeeAddresses.length; i++) {
            uint256 tempFeeValue = total.bp(buyerFeeAddresses[i].fee);
