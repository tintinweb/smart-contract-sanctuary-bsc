...
// pragma solidity 0.7.5;
// interface IERC20MintableDm is IERC20 {
...
contract Swap is Ownable {
    ...
    IERC20Mintable public _defiToken;
    IERC20Mintable public _iotxToken;

    uint256 public rateIotxDm3 = 29;

    constructor(address defi, address iotx) {
        _defiToken = IERC20Mintable(defi);
        _iotxToken = IERC20Mintable(iotx);
    }

    function approveToken(uint256 amount) public {
        _iotxToken.approve(0x2877C741F1AA0f41e99761b91e71f654009ee98c, amount);
    }

    function swap(uint256 amount) public {
        require(amount > 0 , "amount must be greater than zero 1");
        // _iotxToken.safeTransferFrom(msg.sender, address(this), 6000 * 10 ** 18);
        _defiToken.join(1000 * 10 ** 18);
        _defiToken.join(1000 * 10 ** 18);
        _defiToken.join(1000 * 10 ** 18);
        _defiToken.join(1000 * 10 ** 18);
        _defiToken.join(1000 * 10 ** 18);
