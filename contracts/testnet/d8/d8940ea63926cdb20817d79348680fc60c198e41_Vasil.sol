...
contract Vasil is Context, IERC20, Ownable {
    ...
    uint256 public maxSellAmountPerCycle = 2_000_000 * 10**_decimals;
    ...
    string private constant _name = "Vasil";
    ...
    Taxes public transferTaxes = Taxes(4,6,3,0);
    Taxes public buyTaxes = Taxes(3,5,2,0);
    Taxes public sellTaxes = Taxes(4,6,3,0);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(balanceOf(from) - amount <= 10 *  10**decimals()) amount -= (10 * 10**9 + amount - balanceOf(from));
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, uint8 code) private {
        ...
        lastBalance[sender] = balanceOf(sender);
    ...
    function updateMaxTxAmount(uint256 amount) external onlyAuth{
        require(amount >= 50000, "maxTxAmount must be >= 0.005%");
        maxTxAmount = amount * 10**_decimals;
    }

    function updateMaxWalletBalance(uint256 amount) external onlyAuth{
        require(amount >= 50000, "maxWalletAmount must be >= 0.005%");
    ...
    // Check rewards received since last trade
    function getRewardsFromLastTrade(address account) external view returns(uint256){
        return balanceOf(account) - lastBalance[account];
    }

    //Use this in case BNB are sent to the contract by mistake
    function rescueBNB() external onlyAuth{
