...
abstract contract JackPot {
  ...
  function _safeTicketAdd(uint256 amount) internal {
    // storage of user bet.
    JackpotUserBetAmount[msg.sender] += amount;
    // JackpotUserBetAmount[account] + amount;

    if (JackpotUserBetAmount[msg.sender] >= jackPotTicketCost){
      uint256 credit;
      uint256 ticketsAmount;

      Jackpot storage jackpot = DailyJackPots[currentDailyJackpotEpoch];

      credit = JackpotUserBetAmount[msg.sender] % jackPotTicketCost;
      ticketsAmount = (JackpotUserBetAmount[msg.sender] - credit) / jackPotTicketCost;
      JackpotUserBetAmount[msg.sender] = credit;

      DailyTickets[currentDailyJackpotEpoch].push(msg.sender);
      
      for (uint256 i = 0; i < ticketsAmount; i++) {
        DailyTickets[currentDailyJackpotEpoch].push(msg.sender);
      }

      _safeAddUserInJackpot(currentDailyJackpotEpoch);
  ...
  function _safeAddUserInJackpot(uint256 epoch) internal {
    bool addressExist = false;
    for (uint256 i = 0; i < DailyUsersInJackpot[epoch].length; i++) {
      if(DailyUsersInJackpot[epoch][i] == msg.sender){
        addressExist = true;
        break;
      }
    }
    ...
    if(!addressExist){
      DailyUsersInJackpot[epoch].push(msg.sender);
    }
  }
}
