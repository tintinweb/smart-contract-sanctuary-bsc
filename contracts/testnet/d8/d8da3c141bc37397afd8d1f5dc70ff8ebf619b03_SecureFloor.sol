...
contract SecureFloor is Ownable {
    ...
    function buyBond(uint256 classId, uint256 nonceId, uint256 payAmount) external payable {
        ...
        require(block.timestamp >= p.startDate && block.timestamp < p.endDate, "IBO closed");
        ...
        if (p.token != token0) (reserve0, reserve1) = (reserve1, reserve0);
        ...
        if (p.pairToken == address(1)) {
            ...
            (,,bondAmounts) = IRouter(derexRouter).addLiquidityETH{value: payAmount}(p.token, tokenAmount, 0, 0, msg.sender, block.timestamp);
            ...
            (,,bondAmounts) = IRouter(derexRouter).addLiquidity(p.token, p.pairToken, tokenAmount, payAmount, 0, 0, msg.sender, block.timestamp);
