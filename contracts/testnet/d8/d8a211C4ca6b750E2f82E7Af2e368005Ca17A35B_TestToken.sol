...
contract TestToken is Context, IERC20, Values {
    error FeeCollectReentrancy();
    error ApproveFailed();
    ...
    modifier collectFees {
        if(feesCollecting){
            revert FeeCollectReentrancy();
        }
        feesCollecting = true;
        _;
        feesCollecting = false;
    }

    constructor (address _investmentAddress, address _marketingAddress, address _buybackAddress, address _teamAddress) Values(_investmentAddress, _marketingAddress, _buybackAddress, _teamAddress) {
    ...
    function handleCollectedFees() public onlyOwner {
        ...
        for(uint i = 0; i < _fee.fees.length; i++){
            ...
            _transfer(address(this), _fee.fees[i].to, transferAmount);
        }
    }
...
contract Values is Ownable {
    ...
    constructor(address _investmentAddress, address _marketingAddress, address _buybackAddress, address _teamAddress){
