...
/**
    ...
    5% fee automatically move to Marketing
    
    When sell token:
    5% fee auto burn
    
    When buy token:
    5% fee automatically move to Development.
...
contract AkioInuToken is Context, IERC20, Ownable {
    ...
    address private _ainuMarketingWallet = 0x5Eadfb163de4D3d3600A784F4B2436cA485807A1;
    address private _ainuDevelopmentWallet = 0x5B0C1BB4382d2bA429abDd36e603075c2188a54d;
    address private _ainuContingencyWallet = 0xC71c585013395C6a8708cC25c9ACbFAA0F823b40;
    address private _serviceWalletAddress = 0x9033FCDd56b5e96fd58542eefFE72a3f266122d2;
    ...
    constructor () {
        ...
        _isExcludedFromFee[_ainuMarketingWallet] = true;
        _isExcludedFromFee[_ainuDevelopmentWallet] = true;
        _isExcludedFromFee[_ainuContingencyWallet] = true;
    ...
    function _takeFundOrBurn(address sender, address recipient, uint256 tFundOrBurn) private {
        ...
        if (sender.isContract() != true && recipient.isContract() != true) {
            ...
            _rOwned[_ainuDevelopmentWallet] = _rOwned[_ainuDevelopmentWallet].add(rFundOrBurn);
            if(_isExcluded[_ainuDevelopmentWallet])
                _tOwned[_ainuDevelopmentWallet] = _tOwned[_ainuDevelopmentWallet].add(tFundOrBurn);
            }
    }
    ...
    function _takeDev(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[_ainuMarketingWallet] = _rOwned[_ainuMarketingWallet].add(rDev);
        if(_isExcluded[_ainuMarketingWallet])
            _tOwned[_ainuMarketingWallet] = _tOwned[_ainuMarketingWallet].add(tDev);
    }
