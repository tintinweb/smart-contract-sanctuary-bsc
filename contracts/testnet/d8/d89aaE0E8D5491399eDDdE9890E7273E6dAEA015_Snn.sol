...
contract Snn is ERC20, Ownable {
    ...
    uint256 private constant _tTotal = 2000000000 * (10 ** 18); // 100000000 tokens
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "SNN";
    string private constant _symbol = "SNN";
    uint8 private constant _decimals = 18;

    uint256 public _taxFee = 0;       //3%分红本币
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 0; //3%流动性手续费
    ...
    address public marketingAddress = 0xc4b11Ea790602Cd5FB744825f8D59469B112e8e1; // Marketing Address
    ...
    IERC20 public usdToken = IERC20(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47);
    uint256 public _backFee = 4; //回购钱包
    address payable public backAddress = payable(0xBda5fc5A5f6C24a73dA53c54107213A82c3d3099);
    ...
    uint256 private _maxTxAmount = 25000000 * 10**18;
    uint256 private _minimumTokenBalance = 5000000 * 10**18;
    ...
    function swapAndLiquidity(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalTokensToSwap = _marketingFee + _backFee;
        ...
        uint256 ethForMarketing = newBalance.mul(_marketingFee).div(totalTokensToSwap);
        uint256 ethForLiquidity = newBalance.sub(ethForMarketing);

        swapUsdAndSendToken(ethForMarketing);
    ...
    function swapUsdAndSendToken(uint256 bnbBalance) private lockTheSwap {
        uint256 initialBalance = usdToken.balanceOf(address(this));
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(usdToken);

        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: bnbBalance}(
            0,
            path,
            address(this),
            block.timestamp
        ...
        uint256 tokenReceived = usdToken.balanceOf(address(this)).sub(initialBalance);
        usdToken.transfer(backAddress, tokenReceived);
    }
    ...
    function setConfigFeeInfo(uint256 taxFee, uint256 liquidityFee, uint256 burnFee, uint256 marketingFee) external onlyOwner() {
        _taxFee = taxFee;
