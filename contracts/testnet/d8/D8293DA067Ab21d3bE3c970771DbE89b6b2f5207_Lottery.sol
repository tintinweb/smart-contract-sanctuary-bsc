...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function reset() external onlyAdmin {
        ...
        winery.transferFrom(address(adminAddress), burnAddress, toBurn);
    ...
    function drawing() external onlyAdmin {
        ...
        winningNumbers[0]= random();

        
        winningNumbers[1]=random();

        
        winningNumbers[2]=random();

        
        winningNumbers[3] = random();
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        winery.transferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _lotteryId) external {
        require(msg.sender == lotteryBuyInfo[_lotteryId].user, "not from owner");
        ...
        if(reward>0) {
            ...
            //winery.transferFrom(address(this), address(msg.sender), reward);
        }
    ...
    function  multiClaim(uint256[] memory _tickets) external {
        ...
        for (uint i = 0; i < _tickets.length; i++) {
            require (msg.sender == lotteryBuyInfo[_tickets[i]].user, "not from owner");
    ...
    function getUserHistory(address member, uint256 _issueIndex) public view returns(uint256[] memory) {
    ...
    function random() private view returns (uint8) {
       // Generate a random number between 1 and 100:
        uint randNonce = 0;
        uint random = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, randNonce))) % maxNumber;
        randNonce++;
        uint random2 = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, randNonce))) % maxNumber;
