...
contract NexusGalaxy is ERC20, Ownable {
    ...
    constructor(uint256 initialSupply, address _outTaxToken, address _router, address _backToWalletAddress, address _teamAddress, address _marketingAddress, address _developmentAddress, uint[] memory _taxes) ERC20("Nexus Galaxy", "NXS") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(!_inTaxDistribution && to != address(0) && from != address(0) && !noTaxAddressesTo[to] && !noTaxAddressesFrom[from] && toggleTaxes){ 
    ...
    function releaseTaxes(uint releaseAmount) public lockWhileDistribution onlyOwner {
    ...
    function withdrawTokens(address tokensAddrs, address to,  uint amount) public lockWhileDistribution onlyOwner {
