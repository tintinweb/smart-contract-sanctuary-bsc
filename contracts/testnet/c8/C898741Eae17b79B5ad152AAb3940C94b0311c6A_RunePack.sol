...
contract RunePack is ERC721, Ownable {
    ...
    IProxy public proxy;
    event Pack(address owner, uint256 packId, uint256 timestamp, bytes32 packData);
    event UnPack(address owner, uint256 packId, uint256 timestamp);
    ...
    function pack(bytes32 data) public {
        ...
        emit Pack(msg.sender, _tokenIdCounter.current(), block.timestamp, data);
        _tokenIdCounter.increment();
    }
    ...
    function unpack(uint256 packId) public {
        ...
        emit UnPack(msg.sender, packId, block.timestamp);
        _burn(packId);
    }
}
