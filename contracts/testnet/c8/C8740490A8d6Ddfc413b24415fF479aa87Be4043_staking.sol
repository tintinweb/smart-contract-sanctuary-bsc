...
contract staking is Ownable {
    ...
    uint256 public totalInvetedBNB;

    uint256 public totalInvetedBUSD;
    ...
    function invest(uint256 investment,address reffer) public payable {
        ...
        if(msg.value==0 && investment>0){
            ...
            users.deposites.push(depoite(amount, block.timestamp,true));
            users.checkPointBUSD=block.timestamp;
            totalInvetedBUSD=totalInvetedBUSD.add(amount);
        }else{
             amount=msg.value;
            uint256 tax=amount.mul(depoiteTax).div(divider);
            payable(treasury).transfer(tax);
            users.deposites.push(depoite(amount, block.timestamp,false));
            users.checkPointBNB=block.timestamp;
            totalInvetedBNB=totalInvetedBNB.add(amount);
        }
    ...
    function withdrawRewardBNB()public {
        ...
        payable(msg.sender).transfer(totalRewards.sub(tax));
