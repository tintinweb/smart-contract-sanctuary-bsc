...
interface IBEP20 {
 ...
 contract RewardHunter is ERC1155, Ownable, Unbox {
  ...
  mapping(address => NftUsers) private getId;
  ...
  struct NftUsers {
    ...
    function _UnboxUser(address _usr) internal returns(uint256) {
      ...
      NftUsers storage getIds = getId[msg.sender];
    ...
    function newIdView(address _usr) public view returns(uint256) {
      NftUsers memory getIds = getId[_usr];
      return getIds.tokenId;
    }

    function newIdViewStorage(address _usr) public view returns(uint256) {
      NftUsers storage getIds = getId[_usr];
      return getIds.tokenId;
    }
    ...
    function _safeTransferFrom(uint _tokenId, address _receiver) virtual public {
          NftUsers storage ids = idToMarketItem[_tokenId];
