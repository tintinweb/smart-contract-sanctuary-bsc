...
contract Lotto is Ownable {
    ...
    mapping ( uint256 => mapping ( uint256 => mapping (address => uint256))) userNumberBet; // round id => bet number => user => amount
    mapping ( uint256 => mapping ( uint256 => uint256)) numberBet; // round id => bet number => amount
    mapping ( uint256 => mapping ( address => uint256)) netBetted; // round id => user => amount
