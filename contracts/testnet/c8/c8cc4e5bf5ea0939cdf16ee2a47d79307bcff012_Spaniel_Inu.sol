 ...
 * COIN
 */
contract Spaniel_Inu is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    // Addresses
    address private   MarketingAddress = 0xBf289CA73C2620EB63B4639d2321d24bB9A94d7A;
    address private  DevelopmentWallet = 0xBf289CA73C2620EB63B4639d2321d24bB9A94d7A ; 
    address private constant ROUTER_ADDRESS = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;

    // Token name
    string private _name = "Spaniel Inu";
    ...
    uint256 private Maxwalletsize = 15000000000000000 ;
    ...
    uint256 private changeFeeDays ; 
    uint256 private stratTime ; 
    ...
    function getAllBuyFees() public view returns (uint256 _liquidityFeeBuy, uint256 _MarketingFeeBuy, uint256 _rewardFeeBuy, uint256 _devFeeBuy){
        return (liquidityFeeBuy, MarketingFeeBuy, rewardFeeBuy, devFeeBuy);
    }

    // Get all the sell fees
    function getAllSellFees() public view returns (uint256 _liquidityFeeSell, uint256 _MarketingFeeSell, uint256 _rewardFeeSell, uint256 _devFeeSell){
        return (liquidityFeeSell, MarketingFeeSell, rewardFeeSell, devFeeSell);
    }
    ...
    */
    ...
    function setBuyFeesInPercentage(uint256 _liquidityFeeBuy, uint256 _MarketingFeeBuy, uint256 _rewardFeeBuy, uint256 _devFeeBuy) external onlyOwner {
        liquidityFeeBuy = _liquidityFeeBuy;
        MarketingFeeBuy = _MarketingFeeBuy;
        rewardFeeBuy = _rewardFeeBuy;
        devFeeBuy = _devFeeBuy;
        totalFeeBuy = _liquidityFeeBuy.add(_MarketingFeeBuy).add(_rewardFeeBuy).add(_devFeeBuy);
    }

    // Sets all the sell taxes, needs to be lower than the max
    function setSellFeesInPercentage(uint256 _liquidityFeeSell, uint256 _MarketingFeeSell, uint256 _rewardFeeSell, uint256 _devFeeSell) external onlyOwner {
        liquidityFeeSell = _liquidityFeeSell;
        MarketingFeeSell = _MarketingFeeSell;
        rewardFeeSell = _rewardFeeSell;
        devFeeSell = _devFeeSell;
        totalFeeSell = _liquidityFeeSell.add(_MarketingFeeSell).add(_rewardFeeSell).add(_devFeeSell);
    }
    // Enable trading 
    ...
    function getRdev( uint256 tdev , uint256 currentRate) private pure returns( uint256){
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        ...
        if (_isExcludedFromReward[owner()]) _tOwned[owner()] = _tOwned[owner()].add(tLiquidity);
    }
    ...
    function removeAllFee() private {
        if (_rewardFee == 0 && _MarketingFee == 0 && _liquidityFee == 0 && _devFee == 0) return;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
         ...
         if (block.timestamp - stratTime > changeFeeDays * 1 days) {
            liquidityFeeSell = 2;
            MarketingFeeSell = 21;
            devFeeSell = 0;
            rewardFeeSell = 2;
       ...
       // Set the buy or sell fees
        ...
        if (from != uniswapV2Pair && tradingEnabled && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
    ...
    function _transferDevelopmetn( uint256 tDev) private {
    uint256 currentRate = _getRate() ; 
    uint256 rdev = tDev.mul(currentRate) ; 
    _rOwned[DevelopmentWallet] = _rOwned[DevelopmentWallet].add(rdev) ; 
    if (_isExcludedFromReward[DevelopmentWallet]) _tOwned[DevelopmentWallet] = _tOwned[DevelopmentWallet].add(tDev);
    }
    function _transferFromExcluded(
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rHODLrFee, uint256 tTransferAmount, uint256 tHODLrFee, uint256 tMarketing, uint256 tLiquidity) =
            _getValues(tAmount);
        (uint256 devFee) = calculateTax(tAmount) ;  
        (uint256 rdevFee) = getRdev( devFee , _getRate()) ; 

        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] += (rTransferAmount.sub(rdevFee));
        _takeLiquidity(tLiquidity);
        _transferMarketing(tMarketing);
        _HODLrFee(rHODLrFee, tHODLrFee);
        _transferDevelopmetn( devFee); 
        emit TransferBurn(sender, MarketingAddress , tMarketing);
    ...
    function _transferToExcluded(
        ...
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] += (tTransferAmount.sub(devFee));
        _rOwned[recipient] += (rTransferAmount.sub(rdevFee));
        _takeLiquidity(tLiquidity);
        _transferMarketing(tMarketing);
        _HODLrFee(rHODLrFee, tHODLrFee);
