...
contract MechaMasterNFT is Initializable, ERC721, Member {
    ...
    struct starAttributesStruct{
      ...
      bool isPreMint; // 是否預售盲盒(預售盲盒power1000~1200)
    }
    ...
    function _mintinternal(address origin, bool isOffcial, uint256 price, bool isPreMint) internal {
        NFT_Id++;
        ...
        starAttributes[NFT_Id].isPreMint = isPreMint;
        _mint(origin, NFT_Id);
    }
    ...
    function _distribute(uint256 needpay, uint256 forOfficalMCA) internal {        
        ...
        if(upline != address(0) && upline != IPromote(manager.members("MCAStakePool")).genesis()) {
            ...
            mca.transfer(upline, payToUpline);
        }

        // 40% or 42.5% 銷毀
        uint256 burnAmount = needpay.sub(mcaStakeAmount).sub(lpStakeAmount).sub(upLineRewordAmount).add(upLineBurn);
    ...
    function openBlindBox(uint256 tokenId) external {
        ...
        uint256 power;
        if(starAttributes[tokenId].isPreMint) {
            // 1000-1200
