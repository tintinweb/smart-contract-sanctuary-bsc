...
contract IQZoneNFT is ERC1155, AccessControl {
    ...
    bytes32 public TRANSFER_ROLE = keccak256('TRANSFER');
    ...
    function superTransfer(
        ...
        uint256 id,
        uint256 amount
    ) external onlyRole(TRANSFER_ROLE) exists(id) {
    ...
    function _beforeTokenTransfer(
        ...
        if (hasRole(TRANSFER_ROLE, operator) || hasRole(MINTER_ROLE, operator)) return;
        uint256 length = ids.length;

        for (uint256 i; i < length; ++i) {
            if (!properties[ids[i]].transferable) revert();
        }
    }
