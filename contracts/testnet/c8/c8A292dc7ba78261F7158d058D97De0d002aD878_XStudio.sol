...
contract XStudio is IERC20, Ownable {
    ...
    string constant _name = "Test V8";
    string constant _symbol = "$Test8";
    ...
    uint256 public buyUtilityFee = 1;
    ...
    uint256 public sellUtilityFee = 1;
    uint256 public sellTotalFees = 8;

    uint256 marketingSwap = 3;
    uint256 liquiditySwap = 3;
    uint256 devSwap = 1;
    uint256 utilitySwap = 1;
    uint256 totalSwap = 8;

    address public marketingFeeReceiver =
        0xd7F79B7DE736eC5f8E026D23D2C46953847E1418;
    address public utilityWallet = 0x264BdC5dFF8261dEEEC18cea0E6d84550B85d67E;
    address public devFeeReceiver = 0xe8b05ac164BD10EC27ae5B621D0F3b6b0dA11AD9;
    ...
    constructor() {
        ...
        address deployer = 0x50A420299Cd066C3D5e2CC4082D463E4AB6B14b8;
        ...
        _balances[deployer] = _totalSupply/2;
         _balances[msg.sender] = _totalSupply/2;
        emit Transfer(address(0), deployer, _totalSupply/2);
    ...
    function updateBuyFees(
        ...
        buyDevFee = dev;
        buyUtilityFee = utility;
        buyTotalFees = utility.add(marketing).add(liquidity).add(dev);
        require(buyTotalFees <= 20, "Total Fees can not be grater than 20%");
    }
    ...
    function updateSellFees(
        ...
        sellDevFee = dev;
        sellUtilityFee = utility;
        sellTotalFees = utility.add(marketing).add(liquidity).add(dev);
        require(sellTotalFees <= 20, "Total Fees can not be grater than 20%");
    }
    ...
    function updateSwapAmounts(
        ...
        devSwap = dev;
        utilitySwap = utility;
        totalSwap = utility.add(marketing).add(liquidity).add(dev);
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 swappedFees = totalSwap.sub(liquiditySwap);

        uint256 marketingFeeBnb = swappedBnbAmount.mul(marketingSwap).div(
            swappedFees
        );
        uint256 devFeeBnb = swappedBnbAmount.mul(devSwap).div(swappedFees);

        uint256 utilityBnb = swappedBnbAmount.sub(marketingFeeBnb).sub(
            devFeeBnb
        );
        // calculate reward bnb amount
        if (utilityBnb > 0) {
            (bool utilityBnbSuccess, ) = payable(utilityWallet).call{
                value: utilityBnb,
                gas: 30000
            }("");
            utilityBnbSuccess = false;
        }
    ...
    function changeUtilityWallet(address newWallet) external onlyOwner {
