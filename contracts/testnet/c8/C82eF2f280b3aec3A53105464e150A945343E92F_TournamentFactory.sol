...
contract Tournament is AccessControl {
    ...
    */
    ...
    function setTokens(address[] calldata _tokens) external {
        ...
        require(_tokens.length >0, "TOURNAMENT::ATLEAST ONE TOKEN MUST BE ADDED");
        require(block.timestamp < registrationStartTime || registrationStartTime == 0, "TOURNAMENT::UNABLE TO SET TOKENS THIS TIME");
        _validateRemovedTokens(_tokens);
        tokens =  _tokens;
    }

    function _validateRemovedTokens(address[] calldata _tokensNew) private view {
        for(uint i = 0; i < tokens.length; i++){
            bool isRemove = true;
            for(uint j = 0; j < _tokensNew.length; j++){
            ...
            if(isRemove){
                require(totalTokenSponsored[tokens[i]] <= 0, "UNABLE TO REMOVE TOKEN. THERE IS STILL EXISTING BALANCE.");
            }
        }
    }
    ...
    function setRegistrationFee(address _registrationToken, uint256 _registrationFeeAmount, uint256 _registrationStartTime, uint256 _registrationEndTime) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || hasRole(ORGANIZER_ROLE, _msgSender()), "TOURNAMENT:: NO_PERMISSION");
        require(_registrationStartTime > 0, "TOURNAMENT::REGISTRATION START TIME MUST BE SET");
        require(_registrationStartTime < _registrationEndTime, "TOURNAMENT::REGISTRATION START CANNOT BE GREATER THAN THE END TIME");
    ...
    function removeSponsorFromPrizePool(address _token) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || _msgSender() == ITournamentFactory(factory).getpWallet(), "TOURNAMENT:: NO_PERMISSION");
        require(_msgSender() == tx.origin, "TOURNAMENT:: CANNOT CALL FROM CONTRACT");
