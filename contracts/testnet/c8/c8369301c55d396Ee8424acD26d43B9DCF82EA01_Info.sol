...
contract Info is OwnableUpgradeable{
    ...
    function cowInfoes(uint tokenId) external view returns(uint[21] memory info1,bool[3] memory info2, uint[2] memory parents){
    ...
    function compoundInfo(uint tokenId, uint targetId) external view returns(uint[5] memory info){
        ...
        info[3] = cattle.deadTime(targetId) - block.timestamp;
