...
contract NFTFractional is Upgradable {
    ...
    */
    ...
    function fractionalizeNFT(
        ...
        string memory _symbol) external onlyAdmins notEmpty(_name) notEmpty(_symbol) {
            ...
            uint256 _supply = _totalSupply * 10 ** ERC20Token(tokenERC20).decimals();
            ERC20Token(tokenERC20).mintERC20(address(this), _supply);
            FNFTInfo memory fNFTInfo = FNFTInfo(_tokenId,_supply, _supply, address(this), _token, _symbol, _name);
            fnftInfos[_tokenId] = fNFTInfo;
            emit FractionalizeNFT(_token, address(tokenERC20), address(this), _supply, _tokenId ,_symbol, _name);
    }
    ...
    */
    function createFNFTPool(address[] memory _addrs, uint256[] memory _datas, uint256[] memory _configs, uint256[] memory _tiers) external onlyAdmins {
        ...
        require(_datas[2] <=  fnft.totalSupply, "NFTFractional: Pool balance must less then total supply");
        FNFTPool memory pool = FNFTPool(_addrs[0], _addrs[1], _datas[0], _datas[1], _datas[2] * 1e8, _datas[3], _datas[4], _configs, _tiers);
