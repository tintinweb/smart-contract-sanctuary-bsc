...
contract DividendPayingToken is BEP20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable USDT = address(0x59b159c0D8fc4c0e2AA0978f838d494CB6A9756E); //USDT
...
contract DaddyDollar is BEP20, LockToken {
    ...
    uint256 public extraSaleFee = 10;
    
    uint256 public totalFees = USDTRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee).add(buybackFee);

    address payable public _marketingWalletAddress = payable(0xEF28EcB7f1372C802369B0FBAD1c1f15177e8dD0);
    address payable public _developerWalletAddress = payable(0x2D1232a4E1E2F546Be152E96CbA41eEe5E92294f);
    ...
    {
        require(_percentage<=40, "Cannot set more than 40%");
    ...
    function setUSDTRewardsFee(uint256 value) external onlyOwner{
        ...
        totalFees = USDTRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee).add(buybackFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            swapping = true;
            checkForBuyBack();
            uint256 swapTokens = contractTokenBalance.mul(totalFees.sub(USDTRewardsFee)).div(totalFees);
