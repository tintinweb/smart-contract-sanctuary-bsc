...
contract ShadowFiNFTStaking is ERC721Holder, Ownable  {
    ...
    struct UserInfo {
        ...
        uint256 totalRedeemed; // Total redeemed rewards from pid.
    ...
    function getUserInfo(uint256 _pid, address _account) public view returns(uint256 amount, uint256 rewardDebt, uint256 totalRedeemed, uint256[] memory stakedIds) {
        UserInfo storage user = userInfo[_pid][_account];
        uint256[] memory ids = userStakedNFTs(_pid, _account);
        return (
            user.amount,
            user.rewardDebt,
            user.totalRedeemed,
