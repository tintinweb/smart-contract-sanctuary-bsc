...
contract InvestmentsPresale {
    ...
    function setGeneralInfo(uint256 _totalTokens,uint256 _tokenPriceInWei,uint256 _hardCapInWei,
        ...
        uint256 _openTime,uint256 _closeTime) external onlyFactory {
    ...
    function setStringInfo(bytes32 _saleTitle,bytes32 _linkTelegram,bytes32 _linkDiscord,
        bytes32 _linkTwitter,bytes32 _linkWebsite) external onlyPresaleCreatorOrFactory {
    ...
    function setInfo(address _LiqLockAddress,uint256 _DevFeePercentage,
        uint256 _MinDevFeeInWei,uint256 _Id) external onlyDev {
    ...
    function claimTokensinvestors()external whitelistedAddressOnly presaleIsNotCancelled
    ...
    function getRefundBNB()external whitelistedAddressOnly investorOnly notYetClaimedOrRefunded{
        if (!presaleCancelled) {
            require(block.timestamp >= openTime, "Not yet opened");
