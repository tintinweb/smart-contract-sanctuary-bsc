...
contract LuckyRoll {
    ...
    ERC20 MIUXToken = ERC20(0x68ea36CdAc1c9Df17f4fE4B824663Fd946E65ff9);
    
    /// *** Constants section
    uint256 constant BASE_UNIT = 10 ** MIUXToken.decimals(); // 10 ** 18
    ...
    uint constant HOUSE_EDGE_MINIMUM_AMOUNT = BASE_UNIT * 45/100000; //  0.00045 MIUXToken

    // Bets lower than this amount do not participate in jackpot rolls (and are
    // not deducted JACKPOT_FEE).
    uint constant MIN_JACKPOT_BET = BASE_UNIT * 1/10; //0.1 MIUXToken

    // Chance to win jackpot (currently 0.1%) and fee deducted into jackpot fund.
    uint constant JACKPOT_MODULO = 1000;
    uint constant JACKPOT_FEE = BASE_UNIT * 1/1000; //0.001 MIUXToken

    // There is minimum and maximum bets.
    uint constant MIN_BET = BASE_UNIT * 1 /100; // 0.01 MIUXToken
    uint constant MAX_AMOUNT =  BASE_UNIT * 300000; // 300000 MIUXToken
    ...
    // 4 and 6; for games with modulos higher than threshold (LuckyRoll), a simple
    ...
    // threshold. On rare occasions LuckyRoll croupier may fail to invoke
    ...
    address public beneficiary_ = 0xA1463a024007b447bf50b2BC9a43a37b838787bE;
    ...
    function increaseJackpot(uint increaseAmount) external onlyOwner {
        require (increaseAmount <= MIUXToken.balanceOf(this), "Increase amount larger than balance.");
        require (jackpotSize + lockedInBets + increaseAmount <= MIUXToken.balanceOf(this), "Not enough funds.");
        jackpotSize += uint128(increaseAmount);
    }

    // Funds withdrawal to cover costs of LuckyRoll operation.
    function withdrawFunds(uint withdrawAmount) external onlyOwner {
        require (withdrawAmount <= MIUXToken.balanceOf(this), "Increase amount larger than balance.");
        require (jackpotSize + lockedInBets + withdrawAmount <= MIUXToken.balanceOf(this), "Not enough funds.");
    ...
    function kill() external onlyOwner {
        ...
        require(MIUXToken.transfer(beneficiary_,MIUXToken.balanceOf(address(this)))," send out all token failure");
    ...
    //                    by the LuckyRoll croupier bot in the settleBet transaction. Supplying
    ...
    function placeBet(uint betMask, uint modulo, uint commitLastBlock, uint commit, bytes32 r, bytes32 s,uint amount) external {
        ...
        require(MIUXToken.transferFrom(msg.sender,this,amount));//need user approve
	...
	bytes32 withPrefix = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32",keccak256(abi.encodePacked(uint40(commitLastBlock), commit))));
        require (secretSigner == ecrecover(withPrefix, 27, r, s), "ECDSA signature is not valid.");
        ...
        require (jackpotSize + lockedInBets <= MIUXToken.balanceOf(this), "Cannot afford to lose this bet.");
    ...
    // is additionally asserted to prevent changing the bet outcomes on Binance Smart Chain reorgs.
    ...
    // in a situation like this, just contact the LuckyRoll support, however nothing
    ...
    function sendFunds(address beneficiary, uint amount, uint successLogAmount) private {
        if (MIUXToken.transfer(beneficiary,amount)) {//beneficiary.call.value(amount)()
