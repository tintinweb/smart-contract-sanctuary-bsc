...
// File: contracts/RacingSnailsContract.sol
...
contract RacingSnails is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Pausable, Ownable {
    ...
    uint256 public maxMintable = 1690;

    uint256 public mintPrice = 30 ether;

    uint256 public whitelistPrice = 15 ether;
    ...
    constructor() ERC721("Racing Snails", "SNAILS") {
    ...
    function mint(uint256 quantity) external payable whenNotPaused {
        require((whitelistSale || salesAvailable), "Mint don't open yet");
        if(whitelistSale) {
            bool inWhitelist = whitelist[msg.sender];
            require(inWhitelist == true, "You don't have whitelist spot");
            uint256 amountMint = _mintCount.current();
            require(amountMint < maxMintable && ((amountMint + 1) < maxMintable), "Mint limit exceeded!");
            uint256 totalPrice = whitelistPrice * 1;
            require(msg.value >= totalPrice, "Invalid amount for whitelist!");
        ...
        if(salesAvailable) {
            ...
            require(msg.value >= totalPrice, "Invalid amount for mint!");
    ...
    function setWhitelistAddress(address[] calldata account) public onlyOwner {
        require(whitelistSale, "Whitelist Sales isn't public yet!");
    ...
    function internalMint(address to, uint256 tokenId) internal {
        require(tokenId <= maxMintable, "Token limit exceeded!");
        _safeMint(to, tokenId);
        _tokenIds.increment();
    }
