...
contract DexBot is Initializable {
    ...
    function depositBUSD(uint256 _amount) public {
        ...
        IERC20(BusdAddresss).safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function updateQueenReward(uint256 _plan, uint256 _percentageReward) public {
    ...
    function buyPlan(address referrerAddress, uint256 plan) public {
        ...
        user.isUser = true;
        user.lastStaked = 0;
        parent.totalRefrerRewards += (DirectRefferalPercentage * price[plan]) / 100;
    ...
    function stake(uint256 amount, uint256 _pid) public {
        ...
        if (block.timestamp > userstake.lastStaked && userstake.amount > 0) {
            uint256 multiplier = getMultiplier(userstake.lastStaked, block.timestamp);
            uint256 perMonthsec = 86400 * pool.duration;
            if(!pool.isMine){
                uint256 pendingReward   = (userstake.amount * multiplier * pool.rewardPercent)/(perMonthsec * 100000);
   ...
   function set(
    ...
    function withdrawStakeRewards(uint8 _pid) public {   
       ...
       require(userstake.amount > 0, "amount must be greate than zero");
        // require(
        //     block.timestamp > userstake.lastStaked + (_pid * 30 days),
    ...
    function distributeEarnings(address _user,uint cnt,uint _pid) internal {
        UserInfo storage user = userData[msg.sender];
        StakePoolInfo storage pool = poolInfo[_pid];   
