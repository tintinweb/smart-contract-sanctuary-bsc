...
name: public(String[60])
symbol: public(String[35])
...
@external
def batchMint(_to: address, _tokenId: DynArray[uint256, MAX_SIZE]) -> bool:
    """
    @dev Function to mint tokens
         Throws if `msg.sender` is not the minter.
         Throws if `_to` is zero address.
    ...
    assert msg.sender != empty(address)
    assert _to != empty(address)
    assert len(_tokenId) != 0
    
    for _id in _tokenId:
        self._addTokenTo(_to, _id)
        self.totalSupply += 1
        log Transfer(empty(address), _to, _id)
