...
contract FeeDistributor is Ownable {
    ...
    constructor(address _token, address _router, address _swapper) {
        token = IERC20(_token);
    ...
    function getFeeSet() external view returns (uint[] memory, uint[] memory) {
        ...
        for (uint i = 5; i < reservedSellFees.length + 5; i++) {
            sellFeeSet[i] = reservedSellFees[i];
        }
    ...
    function transferFee(bool _sell) external payable {
        require (msg.sender == tefiSwapper, "!swapper");
        if (_sell) {
    ...
    function _distribute() internal {
        ...
        uint dividendFee = collectedBuyFees.mul(dividendBuyFee).div(_totalBuyFee) + collectedSellFees.mul(dividendSellFee).div(_totalSellFee);
        uint reflectFee = collectedBuyFees.mul(reflectBuyFee).div(_totalBuyFee) + collectedSellFees.mul(reflectSellFee).div(_totalSellFee);
        uint marketingFee = collectedBuyFees.mul(marketingBuyFee).div(_totalBuyFee) + collectedSellFees.mul(marketingSellFee).div(_totalSellFee);
        uint[] memory reservedFees = new uint[](reservedBuyFees.length);
        for (uint i = 0; i < reservedBuyFees.length; i++) {
            reservedFees[i] = collectedBuyFees.mul(reservedBuyFees[i]).div(_totalBuyFee) + collectedSellFees.mul(reservedSellFees[i]).div(_totalSellFee);
        }
    ...
    function _dividend(uint _amount) internal {}

    function _reflect(uint _amount) internal {}
    ...
    function addReservedFee(uint _fee, address _wallet) external onlyTokenOwner {
        reservedBuyFees.push(_fee);
        reservedSellFees.push(_fee);
    ...
    receive() external payable {
        if (msg.sender != address(router)) {
            require (false, "TefiSwapper: ISN'T ALLOWED DIRECT SENDING");
        }
    }
}