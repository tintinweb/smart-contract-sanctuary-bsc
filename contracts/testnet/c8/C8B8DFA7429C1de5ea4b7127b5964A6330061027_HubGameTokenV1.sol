...
contract CommonUtil{
    ...
    public view returns (LockItem memory){
        ...
        uint256 _releaseTime = _releaseDate + (_ciff * periodInSecond);
        return LockItem({
...
contract HubGameTokenV1 is BEP20, Ownable, CommonUtil {
    ...
    address[] memory _signers, uint256[] memory _gpePercents, uint256[] memory _numOfPeriods, uint256[] memory _ciffs) 
    public onlyAdmin returns (address gameAddress){
        ...
        require(_signers.length > 0 && _wallets.length == _percents.length 
                && _gpePercents.length == _numOfPeriods.length && _numOfPeriods.length == _ciffs.length, "mismatch locked param");
    ...
    function transactionConfirm(uint256 _txId) public{
        ...
        //emit CheckExecuteTransaction(_from, _to, availableBalance(_from), _value, _canExecute);
        if(_canExecute && availableBalance(_from) >= _value){
            ...
            if(_params.length > 0 && _params[0] > 0){
                ...
                emit event_lockWallet(_from, _to, item.lockedAmount, item.releaseTime, _params[2]);
            }
