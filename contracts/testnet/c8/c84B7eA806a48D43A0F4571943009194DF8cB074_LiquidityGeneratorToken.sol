    ...
    AccessControlUpgradeable
{
    ...
    bytes32 public constant LIQUIDITY_GENERATOR_ROLE =
        keccak256("LIQUIDITY_GENERATOR_ROLE");

    event EthFromContractTransferred(address toAddress, uint256 amount);
    ...
    /**
    @param _reflectionFee - reflection if any %
    @param _liquidityFee - liquidityFee if any%
    @param _liquidityFeeToSell - liquidity fee on sell %
    @param _marketingFee - marketing Fee if any %
    @param _marketingFeeToSell - marketing Fee on sell %
    @param _marketingAddress - update marketing fee address 
    @param _charityFee - charity fee if any %
    @param _charityFeeToSell - charity fee on sell %
    @param _charityAddress - update charityAddress
    @param _transactionLimit - limit per transaction
     */
    function setValues(
     ...
     */
    function updateTransactionLimit(uint256 limitInPercentage)
        external
        onlyOwner
    {
     ...
     */
    function updateMarketingAddress(address marketingAddress_)
        external
        onlyOwner
    {
        require(
            marketingAddress_ != address(0),
            "Marketing address cant be zero address"
        );
    ...
    function updateCharityAddress(address charityAddress_) external onlyOwner {
        require(
            ...
            "Charity address cant be zero address"
        );
     ...
     */
    function updateLiquidityFeeToSell(uint256 _liquidityFeeToSell)
        external
        onlyOwner
    {
        liquidityFeeToSell = _liquidityFeeToSell;
     ...
     */
    function updateMarketingFeeToSell(uint256 _marketingFeeToSell)
        external
        onlyOwner
    {
        marketingFeeToSell = _marketingFeeToSell;
     ...
     */
    function updateCharityFeeToSell(uint256 _charityFeeToSell)
        external
        onlyOwner
    {
        charityFeeToSell = _charityFeeToSell;
        ...
        onlyOwner
    {
        require(
            ...
            "Bridge: Address cant be zero address"
        );
    ...
    function withdrawBnbFromContract(address user, uint256 amount)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (enableMaxTransactionAmount && recipient == uniswapV2Pair)
            require(
                amount <= calculateMaxTransactionAmount(),
                "Sell is above transaction limit"
            );
        ...
        if (
            enableFee &&
            (!_isExcludedFromDexFee[sender] &&
                !_isExcludedFromDexFee[recipient]) &&
    ...
    function _transferStandard(
        ...
        (
            ...
            uint256 rCharity,
            uint256 rLiquidity
        ) = getRValues(tAmount, tReflection, tMarketing, tCharity, tLiquidity);
    ...
    function takeReflectionFee(uint256 rReflection, uint256 tReflection)
        internal
    {
    ...
    function _swapAndLiquify(address from) internal {
        if (
            from != uniswapV2Pair &&
            (liquidityFeeBalance >= liquidityFeeToSell ||
                marketingFeeBalance >= marketingFeeToSell ||
                charityFeeBalance >= charityFeeToSell)
        ) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance, address account)
        internal
    {
