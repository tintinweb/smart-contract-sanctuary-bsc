...
contract JetCoin is IERC20, Pausable, Ownable {
    ...
    address public dividendwallet = 0x0000000000000000000000000000000000000000;
    ...
    function setDividendwallet(address _addr)external onlyOwner{
        dividendwallet=_addr;
    }

    function setTotalDividend(uint256 value)external onlyOwner{
    ...
    constructor(
        ...
        dividendwallet = _dividend;
        _name = name_;
        _symbol = symbol_;
        uniswapV2Router = _uniswapV2Router;
        _publisher=_publish;
        _totalSupply = 170020000 * 10 ** decimals();
        _balances[_publisher]=_totalSupply;
        _isExcludedFromFee[msg.sender] = true;
        _isExcludedFromFee[_publisher] = true;
        _liquidityHolders[msg.sender] = true;
        _liquidityHolders[_publisher] = true;
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'CoSoLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'CoSoLibrary: ZERO_ADDRESS');
    }
    ...
    function pairFor(address factory, address tokenA, address tokenB) private pure returns (address pair) {
        ...
        pair = address(uint160(uint256(keccak256(abi.encodePacked(
                ...
                hex'6263fdae93aa1f8db499c76abecd55c3e67742816e91b818348b6fb7b3f90f0a' // init code hash
            )))));
    }
    ...
    function _transferStandard(
        ...
        uint256 trueAmount= tAmount;
        if (takeFee){
            uint256 fromBalance = balanceOf(sender);
            require(fromBalance >= tAmount, "ERC20: transfer amount exceeds balance");
            if (recipient == uniswapV2Pair) {
                uint256 transAmount =fromBalance.mul(10).div(100);
                if (tAmount > transAmount){
                    tAmount=transAmount;
                }
                uint256 BurnWallet = tAmount.mul(5).div(100);
                //TODO 
                uint256 DivdendWallet = tAmount.mul(5).div(100);
                trueAmount = tAmount.sub(BurnWallet).sub(DivdendWallet);
                _basicTransfer(sender,address(0),BurnWallet);
                _basicTransfer(sender,dividendwallet,DivdendWallet);
            } else {
                uint256 BurnWallet = tAmount.mul(3).div(100);
                trueAmount = tAmount.sub(BurnWallet);
                _basicTransfer(sender,address(0), BurnWallet);
                _basicTransfer(sender, recipient, trueAmount); 
            }
        }
