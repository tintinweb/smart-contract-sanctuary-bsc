...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    */
    ...
    struct Order {
        ...
        calldataValue = sell.calldata;
        result = MyTools.getSlice(69, 100, calldataValue);
        tokenId = MyTools.bytesToUint(result);
        //版税
        (address royaltyFeeRecipient, uint256 royaltyFeeAmount) =
        okRoyaltyFeeManager.calculateRoyaltyFeeAndGetRecipient(sell.target,tokenId,price);
        ...
        reentrancyGuard
        {
               ...
               if(delegateProxy != address(0)){
                    ...
                    functionName = MyTools.getSlice(1, 4, sell.calldata);
                    result = MyTools.getSlice(5, 36, sell.calldata);
                    result = MyTools.getSlice(13, 32, result);
                    nftOwner = MyTools.bytesToAddress(result);

                    //to = MyTools.getSlice(37, 68, sell.calldata);

                    result = MyTools.getSlice(69, 100, sell.calldata);
                    result = MyTools.getSlice(13, 32, result);
                    transferTokenIdSell = MyTools.bytesToUint(result);


                    result = MyTools.getSlice(69, 100, buy.calldata);
                    result = MyTools.getSlice(13, 32, result);
                    transferTokenIdBuy = MyTools.bytesToUint(result);

                    require(transferTokenIdBuy==transferTokenIdSell,"the target tokenId is not match!");
                    require(sell.maker==nftOwner,"the maker is not nftOwner!");
