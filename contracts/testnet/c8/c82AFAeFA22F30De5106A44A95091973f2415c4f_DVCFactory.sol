...
interface IProtectedMarketplace {
    ...
    function createOrder(
        NftInfo memory _nftInfo,    // NFT contract address and token id
        uint256 _tokenPrice,    // Fix price or start price of auction
        ProtectionArgs memory _protectionArgs,
    ...
    function claimDownsideProtectionAmountByVToken(uint256[] memory _orderIds) external returns (bool);
    ...
    function createOffer(
        NftInfo memory _nftInfo,    // NFT token address and token id
        uint256 _tokenPrice,    // Fix price or start price of auction
        bool _useDepository,
...
contract DVC is ERC20, Ownable{
    ...
    uint256 public reimburesLimit;

    VBep20Interface public vBNB;
    ...
    struct SellNFTCriteria {
        ...
        bool useDepository; // use depository for downside protection = true, no = false
        uint64 protectionRate;  // minimum percentage of downside protection (with 2 decimals)
        bool isFixedProtection; // false -> soldTime + protectionTime, true -> fix date
        bool useMLR;    // use MLR = 1, no = 0
    ...
    constructor(
        ...
        reimburesLimit = _reimburseLimit;
    }
    ...
    function withdraw( uint256 _lpamount ) external {
        ...
        if ( address(this).balance < amount ) {
            claimUnderlyingAsset(vBNB.balanceOf(address(this)));
            if ( address(this).balance < amount ) {
     ...
     * @param _useDepository    use depository for downside protection = true, no = false
     * @param _protectionRate   minimum percentage of downside protection (with 2 decimals)
     ...
     */
    function checkBuyCriteriaStatus(uint256 _orderID) internal view returns(bool) {
    ...
    function sellNftOnCriteria( uint256 _orderID ) public returns ( bool ) {
        ...
        IProtectedMarketplace.NftInfo memory nftInfo = IProtectedMarketplace.NftInfo(ipo.tokenAddress, ipo.nftTokenId);
        IProtectedMarketplace.ProtectionArgs memory args = IProtectedMarketplace.ProtectionArgs(sellNFT.useDepository, sellNFT.protectionRate, sellNFT.isFixedProtection, newProtectionTime);
        
        marketplace.createOrder( nftInfo, sellPrice, args, sellNFT.isAuction, endSellOrderTime );
    ...
    function getFundsFromDownsideProtectionAsBuyer( uint256 _orderId, bool _byVToken ) public returns ( bool ) {
        ...
        if (_byVToken) {
            marketplace.claimDownsideProtectionAmountByVToken(_orderIds);
        } else {
            marketplace.claimDownsideProtectionAmount( _orderIds );
        }

        uint256 _gasLeft1 = gasleft();
        uint256 _gasFee = (_gasLeft - _gasLeft1) * _gasPrice;
        if (_gasFee > reimburesLimit) _gasFee = reimburesLimit;
        payable(msg.sender).transfer(_gasFee);
        
        return true;

    }

    function getFundsFromDownsideProtectionAsSeller( uint256 _orderId, bool _byVToken ) public returns ( bool ) {
    ...
    function claimUnderlyingAsset( uint256 _tokenAmount ) public returns ( bool ) {
        require(vBNB.balanceOf(address(this)) >= _tokenAmount, "Insufficient token amount");
        uint256 error = vBNB.redeem(_tokenAmount);
...
// Buy or Sell via MLR contract
    ...
    // DVC can call this function as buyer or seller
    function getFundsFromDownsideProtectionFromMLR(
        address _nftContract,
        uint256 _tokenID,
        uint256 _orderID
    ) public returns ( bool ) {
...
contract DVCFactory {
    ...
    function createDVC(
        ...
        require(
            _protectedMarketplace != address(0) &&
            _mlrFactory != address(0) &&
            _vBNB != address(0),
            "NA"
        );

        dvc = address(
            new DVC(_uniqueID, _vBNB, _protectedMarketplace, _mlrFactory, _isCloseEnded, _reimburseLimit)
        );
