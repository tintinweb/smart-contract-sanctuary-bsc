...
contract MinerPublicBuilding is Distributor, Ownable, IMinerPublicBuilding {
    ...
    function mintMiner() external payable returns(uint tokenId){
        require(
            msg.value == minerMintingPriceInBNB,
            "BrokerPublicBuilding: Invalid money amount"
        );

        return minerNFTContract.mintNFTTo(msg.sender);
    }

    function setMinerNFTContract(
        IActivatableNFT _minerNFTContract
        ) external
        onlyOwner
    {
...
interface IMinerPublicBuilding {
    ...
    function setMinerMintingPrice(
        uint256 _minerMintingPriceInBNB 
        ) external;
