...
contract deployer is OwnableUpgradeable {
    ...
    uint256 public bnbFeePercentage;
    uint256 public insuranceFund;

    address public tokenLocker;
    uint256 public minimumLPLockTime;

    mapping(address => bool) public whitelistedRouters;
    mapping(address => bool) public whitelistedTokens;

    event nePoolCreated(address _poolAddress, address _owner);
    event AddNewWhitelistedRouter(address routerAddress);
    event RemoveWhitelistedRouter(address routerAddress);

    constructor() {}

    function initialize() public initializer {
        feeAmount = 1 * 10**17;
        tokenFeePercentage = 200;
        bnbFeePercentage = 100;
        insuranceFund = 25;
        tokenLocker = 0x9A4A2f141480B225437179B64DEc2bF6A1408bD3;
        minimumLPLockTime = 30 days;
    }
    ...
    function createNewPool(
        ...
        require(
            whitelistedRouters[_addresses[2]],
            "You can not use this router"
        );
        require(
            _poolDetails[8] >= minimumLPLockTime,
            "LP Lock time should grater than 30 days"
        );

        require(
            whitelistedTokens[_addresses[1]],
            "Your token is not whitelisted,Please contact the admin"
        );
        ...
        uint256 bnbForLp = _poolDetails[2].mul(_poolDetails[7]).div(100);

        uint256 tokensToLp = _poolDetails[9].mul(bnbForLp).div(10**18);
        ...
        IERC20(_addresses[0]).transferFrom(
            address(msg.sender),
            address(newPool),
            totelRequireTokens
        );
    ...
    function whiteListRouters(address _routerAddress) external onlyOwner {
        require(!whitelistedRouters[_routerAddress], "Its Already Whitelisted");
        whitelistedRouters[_routerAddress] = true;

        emit AddNewWhitelistedRouter(_routerAddress);
    }

    function removeRouters(address _routerAddress) external onlyOwner {
        require(
            whitelistedRouters[_routerAddress],
            "This is not a whitelisted router"
        );

        whitelistedRouters[_routerAddress] = false;

        emit RemoveWhitelistedRouter(_routerAddress);
    }

    function whiteListTokens(address _tokenAddress) external onlyOwner {
        require(!whitelistedTokens[_tokenAddress], "Its Already Whitelisted");
...
contract Launchpad is Ownable, ReentrancyGuard {
    ...
    **/
    ...
    event leavePool(address investor, uint256 participateAmount);
    ...
    constructor(
        ...
        tokenAddress = IERC20(_addresses[0]);
        ...
        router = IUniswapV2Router02(_addresses[2]);

        tokenLocker = ITokenLocker(locker);

        remainingTokens = totalTokenAmount;

        collectingToken = IERC20(_addresses[1]);
    ...
    function participateToSale(uint256 _amount) public nonReentrant {
        syncPool();
        ...
        require(
            shares[msg.sender].participateAmount.add(_amount) <=
                maximumParticipate,
            "You Already reached maximum participate amount"
        );
        require(
            _amount >= minimumParticipate &&
                _amount <= maximumParticipate &&
                _amount <= remainingBNB,
            "Your Amount is not in minimum and maximum range"
        );
        uint256 tokensToUser = tokensPerbnb.mul(_amount).div(10**18);
        require(
        ...
        collectingToken.transferFrom(msg.sender, address(this), _amount);

        filledBNB = filledBNB.add(_amount);
        remainingBNB = hardCap.sub(filledBNB);

        shares[msg.sender].participateAmount = shares[msg.sender]
            .participateAmount
            .add(_amount);
        ...
        emit newParticipate(msg.sender, _amount, tokensToUser);
    }

    function exitPresale() public nonReentrant {
        syncPool();
        require(poolStatus == 3, "Pool not canceled");
        require(
            shares[msg.sender].participateAmount > 0,
            "You do not have bnb in the pool"
        );

        collectingToken.transfer(
            msg.sender,
            shares[msg.sender].participateAmount
        );

        remainingTokens = remainingTokens.add(shares[msg.sender].tokensAmount);
        filledBNB = filledBNB.sub(shares[msg.sender].participateAmount);
        remainingBNB = hardCap.sub(filledBNB);

        emit leavePool(msg.sender, shares[msg.sender].participateAmount);
    ...
    function sendBnb(address _investor) internal {
        if (shares[_investor].participateAmount > 0) {
            remainingTokens = remainingTokens.add(
                shares[_investor].tokensAmount
            );
            filledBNB = filledBNB.sub(shares[_investor].participateAmount);
            remainingBNB = hardCap.sub(filledBNB);

            emit leavePool(_investor, shares[_investor].participateAmount);

            shares[_investor].participateAmount = 0;
    ...
    function addLiquidity(
        ...
        pair = IUniswapV2Factory(router.factory()).createPair(
            address(collectingToken),
            address(this)
        );

        tokenAddress.approve(address(router), tokenAmount);
        collectingToken.approve(address(router), tokenAmount);
    ...
    function claimRewards() public nonReentrant {
        ...
        if (rewardsToSend > 0) {
            ...
            totalClaimedRewards += rewardsToSend;
