...
contract BatchTransferAssets is AccessControl {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;
    IERC721 public ASSET; 
    function batchTransfer(address _tokenAssets, address _receive, uint256 amountAssets) public {
        ASSET = IERC721(_tokenAssets);
        uint256 balance = ASSET.balanceOf(address(msg.sender));
        require(balance >= amountAssets, "Please check balance assets");
        for(uint256 i = 0; i < amountAssets; i++){
            uint256 tokenId = ASSET.tokenOfOwnerByIndex(msg.sender, 0);
            ASSET.safeTransferFrom(msg.sender, address(_receive), tokenId);
        }
    }
}