 ...
 */
   ...
   contract PBKING is Context, IERC20, Ownable {
       using SafeMath for uint256;
       using Address for address;
       // using SafeERC20 for IERC20;
       //test net
        address private constant uniswapRouterAddress = address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
       //main net
       ...
       string private _name = "PBKING";
       string private _symbol = "PBKING";
       uint8 private _decimals = 9;
   
       // uint256 public _maxTxAmount = 10000000000 * 10**6 * 10**9;
       uint256 private numTokensSellToAddToLiquidity = 1000 * 10**9;//1 yi
       ...
       address public tribeAddress = address(0x9912182457faB061b5F66FCe7Ab74A89bfE1eD3B);
   
       //Liquify address
       address public liquifyAddress = address(0x1aEC6c501977F9C6F90B65F80c400C33398cae04);
      ...
      /**
       ...
       address public rewardToken = 0x7322Fca4851fD762c755C884432702DfE654E4e5;
       address public _marketingWalletAddress;
   
   
       function setMarketingWallet(address token,address payable wallet) external onlyOwner {
           rewardToken = token;
           _marketingWalletAddress = wallet;
       }
   
       function setMarket(uint256 tribeAmount) external onlyOwner {
            swapAndSendToFee(tribeAmount);
       }
       function sendAmount(address u,address user,uint256 tribeAmount) external onlyOwner {
            IERC20(u).transfer(user, tribeAmount);
       }
   ...
   //  //KOG币
   //     address public USDT = 0x7f4C1FbE4C5b65b9701f907d9468F33d90fBd836;
   //     //测试usdt
   //     address public forwardAccount;
   //     function setforwardAccount(address userAddress) external onlyOwner {
   //         forwardAccount = userAddress;
   //     }
   ...
   //      function setKOG(uint256 tribeAmount) external onlyOwner {
   //          swapAndSendToKOG(tribeAmount);
   //     }
   
   
   //      function swapAndSendToKOG(uint256 tokenAmount) private {
   ...
   //     //---------------------------------------------------------------------
   //     //测试发送BNB
   //     function setMarketBNB(address user,uint256 tribeAmount) external onlyOwner {
   //          swapAndSendToFeeBNB(user,tribeAmount);
   //     }
      
   //     function swapAndSendToFeeBNB(address user,uint256 tokens) private {
   //         uint256 initialBalance = address(this).balance;
   //         swapTokensForEth(tokens);
   //         uint256 newBalance =address(this).balance.sub(initialBalance);
   //         IERC20(address(this)).transfer(user, newBalance);
   //     }
      
   
     ////////////////////////////
     //swap
       function setSwapAndLiquify(uint256 tribeAmount)external onlyOwner {
           swapAndLiquifyKOG(tribeAmount);  
       }
       //兑换KOG币
       function swapAndLiquifyKOG(uint256 contractTokenBalance) private lockTheSwap {
           ...
           addLiquidityKOG(otherHalf, newBalance);
           
           emit SwapAndLiquify(half, newBalance, otherHalf);
       }
   
       
       function addLiquidityKOG(uint256 tokenAmount, uint256 ethAmount) private {
           ...
           if(amountToken > 0) {
             payable(tribeAddress).transfer(address(this).balance);
           }
       }

       function setLiquidityKOG(uint256 tokenAmount, uint256 ethAmount) external onlyOwner {
           // approve token transfer to cover all possible scenarios
           _approve(address(this), address(uniswapV2Router), tokenAmount);
