...
contract VRTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("VRT_Dividend_Tracker", "VRT_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "VRT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "VRT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main VRT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "VRT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "VRT_Dividend_Tracker: Cannot update claimWait to same value");
...
contract VRT is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    VRTDividendTracker public dividendTracker;

    mapping(address => bool) public _isBlacklisted;

    uint256 public swapTokensAtAmount = 100000000 * (10**18);
    uint256 private maxSellTransactionAmount = 500000000 * (10**18);
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _buybackFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(buybackFee); // total fee transfer and buy

        require(totalFees <= 25, "the fees can't be set more then 25%");
    }
    ...
    function setbuybackWallet(address payable _newbuybackWallet) public onlyOwner {
        buybackWallet = _newbuybackWallet;
    }

    constructor() ERC20("Vrtrinity", "VRT") {
        BNBRewardsFee = 3;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        buybackFee = 6;
        buybackWallet = payable(0xE2Ca0d05d3903Ac114caD267Da814F587FFD97Ab);

        totalFees = BNBRewardsFee.add(liquidityFee).add(buybackFee); // total fee transfer and buy

    	dividendTracker = new VRTDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "VRT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "VRT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "VRT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "VRT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "VRT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "VRT: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        require(!_isBlacklisted[from] && !_isBlacklisted[to], "Blaclisted Address");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(buybackFee);
        buybackWallet.transfer(marketingAmount);
