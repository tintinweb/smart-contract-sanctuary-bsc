...
contract Bond is IBond{
    ...
    function LpToSig(address _pair, uint256 lpAmount) public view returns (uint256) {
        ...
        uint256 balanceOf0 = IBEP20(token0).balanceOf(_pair);
        uint256 lpValueInToken0 = balanceOf0.mul(lpAmount).div( IBEP20(_pair).totalSupply()).mul(2);
        if(token0 == bep20TokenAddress)
        {
            return lpValueInToken0;
        }
        address[] memory path = new address[](2);
        path[0] = token0;
        path[1] = bep20TokenAddress;
        return IDEXRouter(router).getAmountsOut(lpValueInToken0, path)[1];
    }
    ...
    function SIGtoLP(address _pair, uint256 sigAmount) public view returns (uint256 k_) {
        ...
        k_ = balanceOf1.div(totalTokenWorth).mul(IBEP20(_pair).totalSupply()).div(2);
    }
