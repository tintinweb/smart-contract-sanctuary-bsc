...
library TransferHelper {
    ...
    function safeTransferXDC(address to, uint256 value) internal {
        ...
        require(
            success,
            "TransferHelper::safeTransferXDC: XDC transfer failed"
        );
    }
}
...
interface IGrootRouter02 is IGrootRouter01 {
    function removeLiquidityXDCSupportingFeeOnTransferTokens(
        ...
        address to,
        uint256 deadline
    ) external returns (uint256 amountXdc);
...
library GrootLibrary {
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"2433b8f6cd206e6dda00220531c8f0b57af70a7ec689fcde6fd04985955daf84" // init code hash
                        )
                    )
                )
            )
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IGrootPair(
    ...
    function quote(
        ...
        require(
            ...
            "GrootLibrary: INSUFFICIENT_LIQUIDITY"
        );
...
// File: contracts/GrootRouter.sol
contract GrootRouter is IGrootRouter02 {
    ...
    constructor(address _factory, address _WXDC) {
        factory = _factory;
        WTOKEN = _WXDC;
    }

    receive() external payable {
        assert(msg.sender == WTOKEN); // only accept XDC via fallback from the WTOKEN contract
    }
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = GrootLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = GrootLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "GrootRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = GrootLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "GrootRouter: INSUFFICIENT_A_AMOUNT"
                );
        ...
        )
    {
        ...
        address pair = GrootLibrary.pairFor(factory, token, WTOKEN);
        TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
        IWXDC(WTOKEN).deposit{value: amountXDC}();
        assert(IWXDC(WTOKEN).transfer(pair, amountXDC));
        liquidity = IGrootPair(pair).mint(to);
        // refund dust eth, if any
        if (msg.value > amountXDC)
            TransferHelper.safeTransferXDC(msg.sender, msg.value - amountXDC);
    }
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        (uint256 amount0, uint256 amount1) = IGrootPair(pair).burn(to);
    ...
    function removeLiquidityXDC(
        ...
        returns (uint256 amountToken, uint256 amountXDC)
    {
        ...
        IWXDC(WTOKEN).withdraw(amountXDC);
        ...
        returns (uint256 amountToken, uint256 amountXDC)
    {
        ...
        (amountToken, amountXDC) = removeLiquidityXDC(
            token,
            liquidity,
            amountTokenMin,
            amountXDCMin,
    ...
    function removeLiquidityXDCSupportingFeeOnTransferTokens(
        ...
        address to,
        uint256 deadline
    ) public virtual override ensure(deadline) returns (uint256 amountXDC) {
        ...
        TransferHelper.safeTransfer(
            token,
            to,
            IXDC20(token).balanceOf(address(this))
        );
        IWXDC(WTOKEN).withdraw(amountXDC);
        TransferHelper.safeTransferXDC(to, amountXDC);
    }
    ...
    function removeLiquidityXDCWithPermitSupportingFeeOnTransferTokens(
        ...
        bytes32 r,
        bytes32 s
    ) external virtual override returns (uint256 amountXDC) {
        address pair = GrootLibrary.pairFor(factory, token, WTOKEN);
        uint256 value = approveMax ? type(uint256).max : liquidity;
        IGrootPair(pair).permit(
            msg.sender,
        ...
        returns (uint256[] memory amounts)
    {
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            GrootLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(
            ...
            "GrootRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
    ...
    function swapExactXDCForTokens(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[0] == WTOKEN, "GrootRouter: INVALID_PATH");
        amounts = GrootLibrary.getAmountsOut(factory, msg.value, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "GrootRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        IWXDC(WTOKEN).deposit{value: amounts[0]}();
        assert(
            IWXDC(WTOKEN).transfer(
                GrootLibrary.pairFor(factory, path[0], path[1]),
                amounts[0]
            )
        );
    ...
    function swapTokensForExactXDC(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[path.length - 1] == WTOKEN, "GrootRouter: INVALID_PATH");
        amounts = GrootLibrary.getAmountsIn(factory, amountOut, path);
        require(
            amounts[0] <= amountInMax,
            "GrootRouter: EXCESSIVE_INPUT_AMOUNT"
        ...
        IWXDC(WTOKEN).withdraw(amounts[amounts.length - 1]);
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (msg.value > amounts[0])
            TransferHelper.safeTransferXDC(msg.sender, msg.value - amounts[0]);
    }
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IGrootPair pair = IGrootPair(
                GrootLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                (uint256 reserveInput, uint256 reserveOutput) = input == token0
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);
                amountInput = IXDC20(input).balanceOf(address(pair)).sub(
                    reserveInput
                );
                amountOutput = GrootLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint256 balanceBefore = IXDC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IXDC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >=
                amountOutMin,
            "GrootRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
    ...
    function swapExactXDCForTokensSupportingFeeOnTransferTokens(
        ...
        IWXDC(WTOKEN).deposit{value: amountIn}();
        assert(
            IWXDC(WTOKEN).transfer(
                GrootLibrary.pairFor(factory, path[0], path[1]),
                amountIn
    ...
    function swapExactTokensForXDCSupportingFeeOnTransferTokens(
        ...
        uint256 amountOut = IXDC20(WTOKEN).balanceOf(address(this));
        require(
            amountOut >= amountOutMin,
            "GrootRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        IWXDC(WTOKEN).withdraw(amountOut);
