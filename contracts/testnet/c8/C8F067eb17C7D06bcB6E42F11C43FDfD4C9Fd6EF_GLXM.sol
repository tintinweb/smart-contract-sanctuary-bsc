...
contract GLXM is Context, AdminRole {
...
//    uint256 _TIME_SIX_MONTH = _TIME_ONE_MONTH * 6;
    ...
    */
    ...
    /**
     * @dev Return GLX balance locked in contract GLXM
     */
    function getGLXLockedInContract() public view returns (uint256) {
        return GLX(_glxContractAddress).balanceOf(address(this));
    }
    ...
    */
    function deliverToAccountWithRate10(address to_, uint256 amount_) public onlyAdmin returns (bool) {
        require(_balances[to_] == 0, "GLXM: user have already token locked");

        uint256 tokenLocked = getGLXLockedInContract();
        require((tokenLocked - _totalSupply + 1) > amount_, "GLXM: Balance enough of token locked");
    ...
    */
    function unlockSwap(address unlockAddress_) public onlyAdmin returns(bool) {
        require(_addressLockInfo[unlockAddress_].lockedByAdmin, "GLXM: Address amount is not locked by admin");
    ...
    /**
    * @dev Swap GLXM unlock in caller address to GLX caller address
    *
    * Emit `GLXMSwapped`
    *
    * Requirements
    ...
    */
    function swapGLXMtoGLX() public returns(bool) {
        require(_balances[_msgSender()] > 0, "GLXM: balance of sender is 0");

        LockInfo memory lockInfoAddress = _addressLockInfo[_msgSender()];
        require(block.timestamp > lockInfoAddress.timestampLock, "GLXM: token is already locked");
        require(!lockInfoAddress.lockedByAdmin, "GLXM: swap is already locked by admin");
        uint256 tokenToUnlock = getTokenUnlock(_msgSender());

        GLX(_glxContractAddress).transfer(_msgSender(), tokenToUnlock);
        ...
        emit GLXMSwapped(_msgSender(), tokenToUnlock);
...
contract GLX is Context, IERC20, Ownable {
    ...
    string private _name = "Galaxy Token";
