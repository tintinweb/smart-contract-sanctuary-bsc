...
contract HeedPlayStaking is Ownable, SignVerify, Pausable, ReentrancyGuard{
    ...
    uint256 public minWithdrawAmount = 100000000000000000000;
    uint256 public multiplier = 5;

    uint256 private constant timeStemp = 1 minutes;
 ...
 ////////////////  after testing it will changes //////////////////  
    ...
    constructor()
    {
        LSToken = IERC20(0x5D6dFB6bd14e6A2815C206F39A00fEbe1680B566);
    ...
    function _deposit(address _user, uint256 _tokenAmount, uint256 cardTime) private {
        ...
        require(_tokenAmount >= minDeposit, "less than minimum Amount!");
    ...
    function UpdateROIInfo(address users, uint256 levels) public view returns (uint256) 
    {
        uint256 _amounts;
        uint256 totalday;
        uint256 reward;
    ...
    function getRewards(address _user) public view 
    returns(uint256 _reward_100,uint256 _reward_200,uint256 _reward_400,uint256 _reward_600)
    {
    ...
    function withdraw(uint256 withdrwa_cardTime) external 
    {
        ...
        require(totalReward >= minWithdrawAmount, "Insufficient funds for withdrawal!");
        ...
        require(calwithdrwable > 0, "No rewards available for withdrawal!");
        LSToken.transfer(msg.sender,  calwithdrwable);
    }

    function checkWithdrawDirectsandROI(address _user) public view returns (uint256, uint256, uint256)
    {   
        uint256 remainingValue;
        uint256 totalDepositMultiple;
        uint256 flushAmount;

        UserInfo storage user = userInfo[_user];
        totalDepositMultiple = user.totalDepositAmount.mul(multiplier);

        uint256 TotalROI = UpdateROI(_user);

        RewardInfo storage upRewards = rewardInfo[_user];

        remainingValue = upRewards.directs.add(TotalROI).add(upRewards.total_Rewards);

        if(remainingValue >= totalDepositMultiple )
        {
            flushAmount = remainingValue.sub(totalDepositMultiple);
            remainingValue = totalDepositMultiple;
        }
        remainingValue = remainingValue.sub(upRewards.total_Rewards);
        
        return (TotalROI, remainingValue,flushAmount);
    }
    
    function withdrawDirectsandROI() external
    {   

        (uint256 _TotalROI, uint256 _remainingValue,uint256 _flushedAmount) = checkWithdrawDirectsandROI(msg.sender);
        rewardReceived[msg.sender] += _TotalROI;
        getFlushAmount[msg.sender] +=_flushedAmount;
        RewardInfo storage upRewards = rewardInfo[msg.sender];
        require(_remainingValue > 0, "Insufficient referral rewards!");
    ...
    function UpdateRewards(address users) private view 
    returns(uint256 _reward_100,uint256 _reward_200, uint256 _reward_400,uint256 _reward_600 ) {
        uint256 _amounts;
        uint256 totalday;
        uint256 reward;
        ...
        return (((reward_100).div(1e18)),((reward_200).div(1e18)),((reward_400).div(1e18)),((reward_600).div(1e18)));
    }
    ...
    function unstakeAmount( address User_, uint256 Unstake_cardTime) public view returns (bool, uint256)
    {
        bool unstakeCheck;
        uint256 amount;
