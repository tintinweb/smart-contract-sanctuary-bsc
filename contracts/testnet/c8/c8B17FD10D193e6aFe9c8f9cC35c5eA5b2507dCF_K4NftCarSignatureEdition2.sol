...
contract K4NftCarSignatureEdition2 is ERC721, Ownable, ReentrancyGuard {
    ...
    address payable private hotWalletAddress;
    ...
    event EtherTransfered(address _from, address _to, uint256 indexed amount);
    ...
    function mintHotWallet(
        ...
        require(tokenId.length == quantity, "Invalid parameter");
        require(
            hotWalletAddress != address(0),
            "HotWalletAddress cannot be 0"
        );
        for (uint256 i = 0; i < quantity; i++) {
            if (tokenId[i] <= NFTTOTALSUPPLY && !_exists(tokenId[i])) {
        ...
        emit EtherTransfered(msg.sender, hotWalletAddress, msg.value);
        hotWalletAddress.transfer(msg.value);
    }
    ...
    function setHotWalletAddres(address payable newHotWalletAddress)
        public
        onlyOwner
    {
        require(newHotWalletAddress != address(0), "Address cannot be zero");
        hotWalletAddress = newHotWalletAddress;
    }

    function getHotWalletAddress() public view onlyOwner returns (address) {
