...
contract FridgePurchase is Ownable {

   using SafeMath for uint256;
    mapping(uint256 => FridgePurchaseItem) public fridgePurchase;
    mapping(address => uint256[]) public fridgePurchaseIds;


    IERC20 usdtContractAddress;
    IERC20 mkingContractAddress;
    ...
    function getFridgePurchases(address purchaser) external view returns(FridgePurchaseItem[] memory) {
        uint256[] memory ids = fridgePurchaseIds[purchaser];
        FridgePurchaseItem[] memory items = new FridgePurchaseItem[](ids.length);

        for (uint256 i=0;i<ids.length;i++) {
            items[i] = fridgePurchase[i];
        }

        return items;
    }

 function setPricing(uint256 pricing) external onlyOwner {
        price = pricing;
    }

    function setUsdtContractAddress(IERC20 currency) external onlyOwner {
        usdtContractAddress = currency;
    }

    function setMkingContractAddress(IERC20 currency) external onlyOwner {
        mkingContractAddress = currency;
    }

    function setCompleted(uint256 orderId) external onlyOwner {
        FridgePurchaseItem storage item = fridgePurchase[orderId];
        require (item.status == 0, "Invalid status");
    ...
    function cancel(uint256 orderId) external onlyOwner {
        FridgePurchaseItem storage item = fridgePurchase[orderId];
        require (item.status == 0, "Invalid status");
        require(item.purchaser == msg.sender, "Must be sender");

        uint256 refundPrice = price.mul(90).div(100);

        IERC20(usdtContractAddress).transfer(
            address(this),
            refundPrice
        );

        IERC20(mkingContractAddress).transfer(
            address(this),
            refundPrice
        );
  ...
  function setMembershipNft(address _membershipNft) external onlyOwner {
        membershipNft = Membership(_membershipNft);
    }
     ...
     function stake(uint256 level) external {
       ...
       // require(mintCount[msg.sender][membership] < maximumMintCount, "Reached max mint count");
        ...
        IERC20(mkingContractAddress).transferFrom(
            _msgSender(),
            address(this),
            price
        );
        ...
        fridgePurchase[currentOrderId] =  FridgePurchaseItem({
            orderId: currentOrderId, 
            status: 0,
            purchaser: msg.sender
        });

        fridgePurchaseIds[msg.sender].push(currentOrderId);
