...
contract ReignToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function updateYield(address user, uint256 newTier) external  {
        uint256 tokenBalance = _gonBalances[user].div(getYield());
    ...
    function getNewRebaseToken(uint256 amount) external view returns (uint256 _amount) {
        return amount * rewardYields[0] / rewardYieldDenominator;
    }
    ...
    function getNewRebaseStakedToken(uint256 amount) external view returns (uint256[] memory _initial) {
        uint256[] memory nextRebase = new uint256[](stakingTypeCount);
        for (uint256 i = 0; i < stakingTypeCount; i++) {
            nextRebase[i] = totalStaked[i].div(_yields[i + 1]) * rewardYields[i + 1] / rewardYieldDenominator;
        }
        return nextRebase;
    }
