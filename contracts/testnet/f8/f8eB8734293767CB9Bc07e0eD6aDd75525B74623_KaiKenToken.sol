...
contract KaiKenToken is ERC20, Ownable {
    ...
    uint16 public sellDistribution;
    uint16 public holdDistribution;
    mapping(address => bool) public taxExcluded;
    address public taxRecipient;
    uint256 public sellThreshold;
    uint256 public sellTokenAccumulated;
    address public sellDex;
    address public tokenToSellFor;
    ...
    function _beforeTokenTransfer(
        ...
        if (sellTokenAccumulated >= sellThreshold) {
            sellTokenAccumulated = 0;
            _sell();
        }
    }
    ...
    function _afterTokenTransfer(
        ...
        _transfer(to, taxRecipient, (tax * holdDistribution) / 100);
        _transfer(to, address(this), (tax * sellDistribution) / 100);
        sellTokenAccumulated += (tax * sellDistribution) / 100;
    }

    function _sell() internal {
        uint256 amount = balanceOf(address(this));
        _approve(address(this), sellDex, amount);

        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = tokenToSellFor;
    ...
    function setTaxDistribution(uint16 sellDistribution_, uint16 holdDistribution_)
        external
        onlyOwner
    {
        require(sellDistribution_ + holdDistribution_ == 100, '< 100%');
        sellDistribution = sellDistribution_;
        holdDistribution = holdDistribution_;
    }
    ...
    function setSellThreshold(uint256 threshold) external onlyOwner {
        sellThreshold = threshold;
    }

    function setSellDex(address addr) external onlyOwner {
        require(addr != address(0) && addr != address(this), 'bad dex');
        sellDex = addr;
    }

    function setTokenToSellFor(address addr) external onlyOwner {
        require(addr != address(0) && addr != address(this), 'bad token');
        tokenToSellFor = addr;
    }
    ...
    constructor(
        ...
        uint256 sellDistribution_,
        uint256 holdDistribution_,
        uint256 sellThreshold_,
        address taxRecipient_,
        address sellDex_,
        address tokenToSellFor_
    ) ERC20('KaiKen', 'KKF') {
        require(buyTax_ < 100, 'buy tax too high');
        require(sellTax_ < 100, 'sell tax too high');
        require(transferTax_ < 100, 'transfer tax too high');
        require(
            sellDistribution_ + holdDistribution_ == 100,
            'sum of distributions not 100%'
        );
        require(
            taxRecipient_ != address(0) && taxRecipient_ != address(this),
            'bad recipient'
        );
        require(sellDex_ != address(0) && sellDex_ != address(this), 'bad dex');
        require(
            tokenToSellFor_ != address(0) && tokenToSellFor_ != address(this),
            'bad token'
        );
        ...
        sellDistribution = uint16(sellDistribution_);
        holdDistribution = uint16(holdDistribution_);
        sellThreshold = sellThreshold_;
        taxRecipient = taxRecipient_;
        sellDex = sellDex_;
        tokenToSellFor = tokenToSellFor_;
