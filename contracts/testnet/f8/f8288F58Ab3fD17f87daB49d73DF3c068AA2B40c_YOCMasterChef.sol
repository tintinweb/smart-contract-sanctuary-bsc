...
contract YOCMasterChef is Ownable, ReentrancyGuard {
    ...
    event AddPool(uint256 indexed pid, uint256 allocPoint, IERC20 indexed lpToken, bool isYocPool);
    event SetPool(uint256 indexed pid, uint256 allocPoint);
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accYocPerShare);
    ...
    function pendingYOC(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 yocReward = multiplier.mul(MASTERCHEF_YOC_PER_BLOCK).mul(pool.allocPoint).div(
                totalAllocPoint
            );
            accYocPerShare = accYocPerShare.add(yocReward.mul(ACC_YOC_PRECISION).div(lpSupply));
        }

        return user.amount.mul(accYocPerShare).div(ACC_YOC_PRECISION).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accYocPerShare = pool.accYocPerShare.add((yocReward.mul(ACC_YOC_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accYocPerShare);
        }
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.isYocPool && lockedPeriod < MIN_PERIOD) {
                uint256 _withdrawFee = _amount.mul(WITHDRAW_FEE).div(PERCENT_PRECISION);
        ...
        poolInfo[_pid].totalShare = poolInfo[_pid].totalShare.sub(
            _amount
        );
    ...
    function settlePendingYOC(
        ...
        UserInfo memory user = userInfo[_pid][_user];
        uint256 accYOC = user.amount.mul(poolInfo[_pid].accYocPerShare).div(ACC_YOC_PRECISION);
