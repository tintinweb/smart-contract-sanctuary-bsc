...
contract NodeDeployer is Ownable {
    event AdminTokenRecovery(address indexed tokenRecovered, uint256 amount);
    event NewNodeContract(address indexed nodeAddress);
    ...
    function deployNode(
        ...
        bytes memory bytecode = type(Node).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_price, _expiration, _rewardPerNodePerSecond, _batchLimit, _startTime));
        address nodeAddress;

        assembly {
            nodeAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        Node(nodeAddress).initialize(
            _grona,
            _crona,
            _gronaTreasury,
            _price,
            _expiration,
            _rewardPerNodePerSecond,
            _userLimit,
            _batchLimit,
            _startTime
        );

        emit NewNodeContract(nodeAddress);
    }
...
contract Node is Ownable {
    ...
    function initialize(IERC20 _grona, IERC20 _crona, IGronaTreasury _gronaTreasury,
        uint256 _price, uint256 _expiration, uint256 _rewardPerNodePerSecond,
    ...
    function buy(uint256 nodes) external onlyStart {
        uint _batchSold = batchSold.add(nodes);
        ...
        if (idx == 0 || userLocks[msg.sender][idx - 1].expireTime < expireTime) {
            userLocks[msg.sender].push(LockedNode({amount : nodes, expireTime : expireTime, lastRewardTime : block.timestamp}));
        } else {
        ...
        for (uint i; i < idx; i++) {
            ...
            if (_lockedNode.lastRewardTime < _lockedNode.expireTime) {
                if (_lockedNode.expireTime > block.timestamp) {
                    totalReward = totalReward.add(block.timestamp.sub(_lockedNode.lastRewardTime).mul(_lockedNode.amount).mul(rewardPerNodePerSecond));
                    userLocks[msg.sender][i].lastRewardTime = block.timestamp;
                } else {
                    totalReward = totalReward.add(_lockedNode.expireTime.sub(_lockedNode.lastRewardTime).mul(_lockedNode.amount).mul(rewardPerNodePerSecond));
                    zeroNode++;
    ...
    function pending(address user) external view returns (uint256){
        ...
        for (uint i; i < idx; i++) {
            ...
            if (_lockedNode.lastRewardTime < _lockedNode.expireTime) {
                if (_lockedNode.expireTime > block.timestamp) {
                    totalPending = totalPending.add(block.timestamp.sub(_lockedNode.lastRewardTime).mul(_lockedNode.amount).mul(rewardPerNodePerSecond));
                } else {
                    totalPending = totalPending.add(_lockedNode.expireTime.sub(_lockedNode.lastRewardTime).mul(_lockedNode.amount).mul(rewardPerNodePerSecond));
                }
            }
        }
