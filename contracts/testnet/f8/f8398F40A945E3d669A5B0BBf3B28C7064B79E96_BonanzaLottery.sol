...
contract BonanzaLottery is ILottery, ReentrancyGuard, Ownable {
    ...
    function buyTickets(
        ...
        if (ref != 0 && !referralProgram.hasReferrer(msg.sender)) {
    ...
    function injectFunds() external override onlyOwnerOrInjector {
        require(MIN_JP_PRIZE > lotteries[currentLotteryId].jpTreasury, "Treasury is enough funds");
        require(
            currentLotteryId == 0 || lotteries[currentLotteryId].status == Status.Claimable,
            "Lottery is not claimable"
        );
        uint256 _amount = MIN_JP_PRIZE - lotteries[currentLotteryId].jpTreasury;
        ...
        lotteries[currentLotteryId].escrowCredit += depositAmount;
    ...
    function withdrawEscrowFunds(address receiver, uint256 amount) external onlyOwner {
        require(receiver != address(0), "receiver address is zero");
        require(
            lotteries[currentLotteryId].status == Status.Open,
            "only withdraw when lottery open"
        );

        if (amount > lotteries[currentLotteryId].escrowBalance) {
            amount = lotteries[currentLotteryId].escrowBalance;
        }
        require(amount >= 0, "withdraw amount is 0");

        lotteries[currentLotteryId].escrowBalance -= amount;
        currency.transfer(receiver, amount);

        emit WithdrawnEscrowFunds(currentLotteryId, receiver, amount);
    }
