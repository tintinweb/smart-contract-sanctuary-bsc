...
// File contracts/marketplace/DutchAuctionMP.sol
...
contract DutchAuctionMP is AccessControl, Pausable, ERC721Holder  {
    ...
    */
    function bid(uint256 _tokenId, address _NFTContractAddress) external payable {
        // Get a reference to the auction struct
        Auction storage _auction = auctions[_NFTContractAddress][_tokenId];
        ...
        _removeAuction(_tokenId, _NFTContractAddress);

        // then transfer the NFT to the user
        IERC721(_NFTContractAddress).safeTransferFrom(address(this), msg.sender, _tokenId);

        if(_price > 0) {
            uint256 _auctioneerCut = _price / 10;  
            uint256 _sellerCut = _price - _auctioneerCut;
            
            payable(_seller).transfer(_sellerCut);
        }
    ...
    {
        Auction storage _auction = auctions[_contractAddress][_tokenId];
        require(_isOnAuction(_auction), "Auction not currently Active.");
        return (
        _auction.seller,
    ...
    */
        ...
        canBeStoredWith64Bits(_duration)
    {
        ...
        require(_owns(_seller, _tokenId, _contractAddress), "The user does not own the specified NFT");
        _escrow(_seller, _tokenId, _contractAddress);
    ...
    function cancelAuction(uint256 _tokenId, address _contractAddress) external {
        Auction storage _auction = auctions[_contractAddress][_tokenId];
        require(_isOnAuction(_auction), "Auction not active");
        require(msg.sender == _auction.seller, "The user does not own the specified NFT");
        _cancelAuction(_tokenId, _auction.seller, _contractAddress);
    }
    ...
    /// @param _contractAddress - address of contract token is on
        ...
        internal
    {
        ...
        auctions[_contractAddress][_tokenId] = _auction;
    ...
    /// @param _tokenId - ID of NFT on auction.
    function _removeAuction(uint256 _tokenId, address _contractAddress) internal {
        delete auctions[_contractAddress][_tokenId];
    }

    /// @dev Cancels an auction unconditionally.
    function _cancelAuction(uint256 _tokenId, address _seller, address _contractAddress) internal {
        _removeAuction(_tokenId, _contractAddress);
        IERC721(_contractAddress).safeTransferFrom(address(this), _seller, _tokenId);
    ...
    function _escrow(address _owner, uint256 _tokenId, address _contractAddress) internal {
        // It will throw if transfer fails
        IERC721(_contractAddress).safeTransferFrom(_owner, address(this), _tokenId);
    }
}