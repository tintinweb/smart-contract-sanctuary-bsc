...
contract TestStaking3 is IStaking, Ownable, ReentrancyGuard, Pausable {
    ...
    function rewardPerToken(address stakingToken, address rewardToken) public view override returns (uint256) {
        ...
        uint256 eBlock = block.number < campaigns[stakingToken][rewardToken].finishBlock ? block.number : campaigns[stakingToken][rewardToken].finishBlock;
        return
        ...
        updateReward(msg.sender, stakingToken, rewardToken)
    {
        ...
        if (campaign.totalStakes == 0) {
            uint256 blockPeriod = campaign.finishBlock.sub(block.number);
            _calculateRate(stakingToken, rewardToken, getRemainingReward(stakingToken, rewardToken), blockPeriod);
        ...
        updateReward(address(0), stakingToken, rewardToken)
    {
        ...
        Campaign storage campaign = campaigns[stakingToken][rewardToken];
        uint256 blockPeriod = finishPeriod.sub(block.timestamp).div(BLOCK_TIME);
