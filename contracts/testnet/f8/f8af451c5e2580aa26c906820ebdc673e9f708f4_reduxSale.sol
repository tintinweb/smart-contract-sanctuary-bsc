...
contract reduxSale is Ownable,PriceContract{
    ...
    mapping(address => uint256) public refferedEarning;

    constructor() public{
    }
    
    fallback() external  {
        purchaseTokensWithBNB(address(0));
    }
    
    
    
    function purchaseTokensWithBNB(address payable referral) payable public{   // with BNB
        uint256 amount = msg.value;       
        require(saleActive == true, "Sale not started yet!");

        if(refferedBy[msg.sender] == address(0) && referral != msg.sender){
            refferedBy[msg.sender] = referral;
        }
        if(refferedBy[msg.sender] != address(0)){
            uint256 reffPercent = SafeMath.div(amount,10); // 10 percent
            referral =  address(uint160(refferedBy[msg.sender]));
            referral.transfer(reffPercent);        
            uint256 refBnbToUsd = calculateUsd(reffPercent); // convert bnb to usd
            refferedEarning[referral] = refferedEarning[referral] + refBnbToUsd;
        }
    ...
    function purchaseTokensWithStableCoin(uint256 coinType, uint256 amount, address referral) public {
        ...
        if(refferedBy[msg.sender] != address(0)){
            uint256 reffPercent = SafeMath.div(amount,10); // 10 percent
            if(coinType == 1){
                busd.transferFrom(msg.sender, address(this), reffPercent);
            }
            else{
                usdt.transferFrom(msg.sender, address(this), reffPercent);
            }
            refferedEarning[referral] = refferedEarning[referral] + reffPercent;
        }
