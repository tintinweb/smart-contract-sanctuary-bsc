...
contract MRHDPairSetUp{
    ...
    address public lpweighteddividendaddress;//LP加权分红地址
    address public contributionweighteddividendaddress;//贡献值加权分红地址
    address public initialinvitationaddress;//初始邀请地址
    address[] public users;//用户数组
    ...
    mapping(address => mapping(address => uint256[2])) public addressfluidity;//LP：token地址 => 用户地址 => token,usdt
    mapping(address => address) public invitationrelationship;//邀请关系：地址 => 上级地址
    mapping(address => mapping(address => uint256)) public usercontribution;//用户贡献值：token => 用户地址 => 贡献值
    mapping(address => uint256) public contributionsum;//总贡献值：token => 总贡献值
    mapping(address => uint256) public lpsum;//总LP：token => 总LP
    ...
    modifier isBind() {
        require(invitationrelationship[msg.sender] != address(0), "Please bind the invitation relationship first");
        _;
    }
...
contract MRHDPair is MRHDPairSetUp{
    constructor(address _usdtaddress, uint256 _withdrawalfee,
        address _withdrawalfeeaddress, address _lpweighteddividendaddress,
        address _contributionweighteddividendaddress, address _initialinvitationaddress) {
        owner = msg.sender;
        usdtaddress = _usdtaddress;
        withdrawalfee = _withdrawalfee;
        withdrawalfeeaddress = _withdrawalfeeaddress;
        lpweighteddividendaddress = _lpweighteddividendaddress;
        contributionweighteddividendaddress = _contributionweighteddividendaddress;
        initialinvitationaddress = _initialinvitationaddress;
    }

    //贡献值加权算法,a：奖金总额，b：token总贡献值，
    function contributionWeightingAlgorithm(address _token,uint256 a,uint256 b) internal{
        for(uint256 i = 0;i<users.length;i++){
            uint256 d = usercontribution[_token][users[i]];
            if(d > 0){
                uint256 e = d / b * a;
                uint256 f = tokenconfiguration[_token][15] * e;
                IERC20(_token).transferFrom(contributionweighteddividendaddress, msg.sender, e);
                if(usercontribution[_token][users[i]] > f * 10){
                    usercontribution[_token][users[i]] = usercontribution[_token][users[i]] - f * 10;
                }else{
                    usercontribution[_token][users[i]] = 0;
                }
                if(contributionsum[_token] > f * 10){
                    contributionsum[_token] = contributionsum[_token] - f * 10;
                }else{
                    contributionsum[_token] = 0;
                }
            }
        }
    }
    ...
    //贡献值加权分红
    function gxzjq(address _token) public{
        require(IERC20(_token).balanceOf(contributionweighteddividendaddress) >= 1, "Invite: 0001"); // 分佣总金额大于1
        contributionWeightingAlgorithm(_token,IERC20(_token).balanceOf(contributionweighteddividendaddress),contributionsum[_token]);
    }

    

    //LP加权算法,a：token奖金总额，b：token总LP，
    function LPWeightingAlgorithm(address _token,uint256 a,uint256 b) internal{
        for(uint256 i = 0;i<users.length;i++){
            uint256 d = addressfluidity[_token][users[i]][0];
            if(d > 0){
                uint256 e = d / b * a;
                IERC20(_token).transferFrom(contributionweighteddividendaddress, msg.sender, e);
            }
    ...
    //LP加权分红
    function lpjq(address _token) public{
        require(IERC20(_token).balanceOf(lpweighteddividendaddress) >= 1, "Invite: 0001"); // 分佣总金额大于1
        contributionWeightingAlgorithm(_token,IERC20(_token).balanceOf(lpweighteddividendaddress),lpsum[_token]);
    }

    //更新贡献值
    function updateContribution(address _token,address myAddress,uint256 u) internal{
        //更新自己贡献值
        usercontribution[_token][myAddress] = usercontribution[_token][myAddress] + u * 10;
        contributionsum[_token] = contributionsum[_token] + u * 10;
        //更新1代贡献值
        myAddress = invitationrelationship[myAddress];
        if(myAddress != address(0) && myAddress != initialinvitationaddress){
            usercontribution[_token][myAddress] = usercontribution[_token][myAddress] + u * 10 / 2;
            contributionsum[_token] = contributionsum[_token] + u * 10 / 2;
        }
        //更新2代贡献值
        myAddress = invitationrelationship[myAddress];
        if(myAddress != address(0) && myAddress != initialinvitationaddress){
            usercontribution[_token][myAddress] = usercontribution[_token][myAddress] + u * 10 / (2 ** 2);
            contributionsum[_token] = contributionsum[_token] + u * 10 / (2 ** 2);
        }
        //更新3代贡献值
        myAddress = invitationrelationship[myAddress];
        if(myAddress != address(0) && myAddress != initialinvitationaddress){
            usercontribution[_token][myAddress] = usercontribution[_token][myAddress] + u * 10 / (2 ** 3);
            contributionsum[_token] = contributionsum[_token] + u * 10 / (2 ** 3);
        }
        //更新4代贡献值
        myAddress = invitationrelationship[myAddress];
        if(myAddress != address(0) && myAddress != initialinvitationaddress){
            usercontribution[_token][myAddress] = usercontribution[_token][myAddress] + u * 10 / (2 ** 4);
            contributionsum[_token] = contributionsum[_token] + u * 10 / (2 ** 4);
        }
        //更新5代贡献值
        myAddress = invitationrelationship[myAddress];
        if(myAddress != address(0) && myAddress != initialinvitationaddress){
            usercontribution[_token][myAddress] = usercontribution[_token][myAddress] + u * 10 / (2 ** 5);
            contributionsum[_token] = contributionsum[_token] + u * 10 / (2 ** 5);
        }
        //更新6代贡献值
        myAddress = invitationrelationship[myAddress];
        if(myAddress != address(0) && myAddress != initialinvitationaddress){
            usercontribution[_token][myAddress] = usercontribution[_token][myAddress] + u * 10 / (2 ** 6);
            contributionsum[_token] = contributionsum[_token] + u * 10 / (2 ** 6);
        }

    }

    //绑定邀请
    function blind(address  parentAddress) public{
        require(parentAddress != address(0), "Invite: 0001"); // 不允许上级地址为0地址
        // require(invitationrelationship[msg.sender] != address(0), "Invite: 0004");// 不允许重复绑定
        require(parentAddress != msg.sender, "Invite: 0002");// 不允许自己的上级是自己
        // 验证要绑定的上级是否有上级，只有有上级的用户，才能被绑定为上级（firstAddress除外）。如果没有此验证，那么就可以随意拿一个地址绑定成上级了
        require(invitationrelationship[parentAddress] != address(0) || parentAddress == initialinvitationaddress, "Invite: 0003");
        invitationrelationship[msg.sender] = parentAddress;
        users.push(msg.sender);
    }

    //添加交易对
    function addPair(address _token, uint256[15] memory nums) public isOwner isBind{
        tokenconfiguration[_token] = nums;
        tokenconfiguration[_token][6] = 0;
    ...
    //获取交易对
    function getPair(address _token) public view returns (uint256 a, uint256 b, uint256[17] memory c){
        return (IERC20(_token).balanceOf(address(this)),IERC20(usdtaddress).balanceOf(address(this)),tokenconfiguration[_token]);
    }

    //添加流动性
    function addLiquidity(address _token, uint256 amount) public isBind{
        ...
        uint256 a = amount * tokenconfiguration[_token][15];
        uint256 usdtamount = a * 10 ** 18 / tokenconfiguration[_token][5];
        IERC20(usdtaddress).transferFrom(msg.sender, address(this), usdtamount);
        tokenconfiguration[_token][6] = tokenconfiguration[_token][6] + a;
        addressfluidity[_token][msg.sender][0] = addressfluidity[_token][msg.sender][0] + amount;
        addressfluidity[_token][msg.sender][1] = addressfluidity[_token][msg.sender][1] + a;
        lpsum[_token] = lpsum[_token] + amount;
    }

    //提取流动性
    function extractLiquidity(address _token, uint256 amount) public isBind{
        require(amount > uint256(0),"Add quantity is greater than 0");
        require(addressfluidity[_token][msg.sender][0] > amount,"Withdrawal amount exceeds liquidity balance");
        uint256 a = addressfluidity[_token][msg.sender][1] * (amount / addressfluidity[_token][msg.sender][0]);
        uint256 usdtamount = a  * 10 ** 18 / tokenconfiguration[_token][5];
        uint256 sxf_token = amount * withdrawalfee / 100;
        uint256 sxf_udst = usdtamount * withdrawalfee / 100;
        IERC20(_token).transfer(msg.sender, amount - sxf_token);
        IERC20(usdtaddress).transfer(msg.sender, usdtamount - sxf_udst);
        IERC20(_token).transfer(withdrawalfeeaddress, sxf_token);
        IERC20(usdtaddress).transfer(withdrawalfeeaddress, sxf_udst);
        tokenconfiguration[_token][6] = tokenconfiguration[_token][6] - a;
        addressfluidity[_token][msg.sender][0] = addressfluidity[_token][msg.sender][0] - amount;
        addressfluidity[_token][msg.sender][1] = addressfluidity[_token][msg.sender][1] - a;
        lpsum[_token] = lpsum[_token] - amount;
    ...
    function swap(address _token, bool status_b, bool status_t,uint256 _amount) public payable isBind{
        ...
        if(status_b){
            ...
            if(status_t){
                ...
                IERC20(usdtaddress).transferFrom(msg.sender, address(this), m * 10 ** 18 / tokenconfiguration[_token][5]);
                IERC20(_token).transfer(msg.sender,_amount);
                updateContribution(_token,msg.sender,m / tokenconfiguration[_token][5]);
            }else{
                (m,n) = algorithmSellNumber(tokenconfiguration[_token],_amount,0);
                require(tokenconfiguration[_token][6] >= m,"001");
                IERC20(_token).transferFrom(msg.sender, address(this), _amount);
                IERC20(usdtaddress).transfer(msg.sender,m * 10 ** 18 / tokenconfiguration[_token][5]);
                updateContribution(_token,msg.sender,m / tokenconfiguration[_token][5]);
            }
        }else{
            ...
            if(status_t){
                ...
                IERC20(usdtaddress).transferFrom(msg.sender, address(this), _amount * 10 ** 18 / tokenconfiguration[_token][5]);
                IERC20(_token).transfer(msg.sender,m);
                updateContribution(_token,msg.sender,_amount / tokenconfiguration[_token][5]);
            }else{
                ...
                IERC20(usdtaddress).transfer(msg.sender,_amount * 10 ** 18 / tokenconfiguration[_token][5]);
                updateContribution(_token,msg.sender,_amount / tokenconfiguration[_token][5]);
            }
        }
