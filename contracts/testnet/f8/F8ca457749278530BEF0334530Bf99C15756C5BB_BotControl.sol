...
contract BotControl {
    ...
    uint public cooldownDecreasePerMinute; // minute
    ...
    constructor(address pairAddress_) {
        ...
        setTierPeriods(1, 0);
        setTierPeriods(2, 0);
        setTierPeriods(3, 180);
        setTierPeriods(0, 360);
        setOnlyTiers(true);
        ...
        setBuySellCooldown(20,30);


        setHasConstantIncreaseMaxBuySell();
        setTokenAmountIncreasePerSecond(1*1e18);

        setHasConstantDecreaseBuySellCooldown();
        setCooldownDecreasePerMinute(1);

        
        setBPActiveDuration(10*60);
    }


    function resetBP() public onlyAdmin {
        tierStartTime = 0;
    }

    function setPairAddress(address pairAddress_) public onlyAdmin {
        pairAddress = pairAddress_;
        isWhitelistContract[pairAddress_] = true;
    }
    ...
    function setCooldownDecreasePerMinute(uint _cooldownDecreasePerMinute) public onlyAdmin { // input of 1 means decrease 1 seconds per minute
        require(hasConstantDecreaseBuySellCooldown, "BP: constant buy and sell cooldown is not turned on.");
        cooldownDecreasePerMinute = _cooldownDecreasePerMinute * 1 minutes;
    }
    ...
    function protect (
        ...
        if (hasConstantDecreaseBuySellCooldown && hasBuySellCooldown) {
            calculatedBuyCooldown = buyCooldown - ( (block.timestamp - tierStartTime) / 60 * cooldownDecreasePerMinute );
            calculatedSellCooldown = sellCooldown - ( (block.timestamp - tierStartTime) / 60 * cooldownDecreasePerMinute );
        }
