...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function getBuyableTokenCount() public view returns (uint256) {
        uint256 currentNativeBalance = address(msg.sender).balance;
        currentNativeBalance /= gasPrice;
        uint256 mintAmount = (currentNativeBalance - startFee) / unitFee;
    ...
    function getNativeBalance(address _addr) public view returns (uint256) {
        return address(_addr).balance;
    }

    function getNativeBalance1() public view returns (address, uint256) {
        return (
            msg.sender,
