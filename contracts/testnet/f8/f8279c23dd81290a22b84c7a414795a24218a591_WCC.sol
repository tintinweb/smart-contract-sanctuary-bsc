...
contract WCC is Context, IBEP20, Ownable {
    ...
    string private _name = "WorldCup Coins";
    ...
    function addSniper(address account) external onlyOwner {
        require(!_isSniper[account], "Account is a recorded sniper.");
    ...
    function setMarketAddress(address market) public onlyOwner {
        require(_market != market, "_market is this value.");
        _market = market;
    }

    function setTakeTax(bool value) public onlyOwner {
        require(_takeTax != value, "_takeTax is this value.");
        _takeTax = value;
    }
    ...
    function setRewardsFeePercent(uint256 buyRewardsFee, uint256 sellRewardsFee)
        external
        onlyOwner
    {
    ...
    function setBurnFeePercent(uint256 buyBurnFee, uint256 sellBurnFee)
        external
        onlyOwner
    {
    ...
    function setMarketFeePercent(uint256 buyMarketFee, uint256 sellMarketFee)
        external
        onlyOwner
    {
        ...
        )
    {
        ...
        ) = _getRValues(tAmount, tRewards, tMarket, tBurn, _getRate());
        return (
            rAmount,
            rTransferAmount,
            rRewards,
    ...
    {
        uint256 rewardsFee = calculateRewardsFee(tAmount);
        uint256 marketFee = calculateMarketFee(tAmount);
        uint256 burnFee = calculateBurnFee(tAmount);

        uint256 tTransferAmount = tAmount.sub(rewardsFee).sub(marketFee).sub(
            burnFee
        );
        return (tTransferAmount, rewardsFee, marketFee, burnFee);
    }
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rRewards).sub(rMarket).sub(rBurn);
        return (rAmount, rTransferAmount, rRewards, rMarket, rBurn);
    }
    ...
    function calculateRewardsFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
    ...
    function withdrawBNBMarket() private {
        payable(_market).transfer(address(this).balance);
    }
    ...
    function _takeMarket(address sender, uint256 tMarket) private {
        ...
        emit Transfer(sender, address(this), tMarket);
    }

    function _takeBurn(address sender, uint256 tBurn) private {
        uint256 currentRate = _getRate();
        uint256 rBurn = tBurn.mul(currentRate);
        _rOwned[_burn] = _rOwned[_burn].add(rBurn);
        if (_isExcluded[address(this)]) {
            _tOwned[_burn] = _tOwned[_burn].add(tBurn);
        }
        emit Transfer(sender, _burn, tBurn);
    }

    function removeAllFee() private {
        if (_burnFee == 0 && _marketFee == 0 && _rewardsFee == 0) return;
