...
contract ElectSmartContract is Ownable  {
    ...
    uint public candidateCount; //Total # of candidates
    uint public voterCount; //Total # of user voted
    address public deductAddress; //Address to send tokens to for vote
    address private voteToken;
    uint private cycleCount;
    phase voteStart; 

    mapping(uint => candidate) private totalCandidate;

    constructor(){
        candidateCount = 0;
        voterCount = 0;
        cycleCount = 0;
        deductAddress = 0xdb4D508855BD1420eED27Adf68B56EbE3745968a;
        voteToken = 0x5944F2F4af55167Bc57Fa2130be6c2212C6d474b;
    ...
    function addVote(uint _idCount) public {
        ...
        require(IERC20(voteToken).balanceOf(msg.sender) != 0, "Invalid balance");
        require(hasVoted[msg.sender].cycleVoted != cycleCount, "You have already voted");

        IERC20(voteToken).transferFrom(msg.sender, deductAddress, 1);
    ...
    function cancelVote() public {
        ...
        IERC20(voteToken).transfer(msg.sender, 1);
