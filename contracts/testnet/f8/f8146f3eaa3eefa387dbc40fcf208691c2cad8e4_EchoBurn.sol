...
contract EchoBurn is Context, IERC20, Ownable {
    ...
    string public constant _name ="EchoBurn";
    string public constant _symbol = "ECHOBURN";
    ...
    uint256 public AmountForBurn;

    IUniswapV2Router02 public pcsV2Router;
    address public pcsV2Pair;

    address public MarketingWallet = address(0xfe77b26ee502bDe44385Ca304005baC425EdD010);
    address public DeveloperWallet = address(0x7FDbecb1C08e53b4781cFd92Ae82915dbEd51CF7);
    ...
    constructor ()  {       
        ...
        buyFee.setMarketingFee = 40;
        buyFee.setDevelopmentFee = 30;
        buyFee.setBurnFee = 10;

        sellFee.setTaxFee = 20;
        sellFee.setLiquidityFee = 20;
        sellFee.setMarketingFee = 40;
        sellFee.setDevelopmentFee = 30;
        sellFee.setBurnFee = 10;
    ...
    function calculateLiquidityFee(uint256 _amount) private returns (uint256) {
        ...
        AmountForBurn += _amount.mul(_BurnFee).div(10**3);

        return _amount.mul(_liquidityFee + _MarketingFee + _DevelopmentFee + _BurnFee).div(
            10**3
        );
    }
  
    function removeAllFee() private {
        uint subtotal = _taxFee.add(_DevelopmentFee).add(_liquidityFee).add(_MarketingFee).add(_BurnFee);
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        if(AmountForBurn > 0) sendToBurn(AmountForBurn);
    }

    function sendToBurn(uint _token) private {
        _tokenTransferNoFee(address(this),address(dead),_token);
        AmountForBurn = AmountForBurn.sub(_token);
    }
