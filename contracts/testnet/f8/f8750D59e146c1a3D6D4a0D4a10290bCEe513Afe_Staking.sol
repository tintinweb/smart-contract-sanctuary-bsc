...
contract Staking is AccessControl {
    ...
    uint256[4] public defaultPackages = [1000 ether, 2000 ether, 5000 ether, 10000 ether];
    ...
    uint256 public timePeriod = 25 * 30 days;
    ...
    constructor(){
        ...
        USDT = IERC20(0xa1bFd8aBCE6699B62f6510Fb686eEfc1C089340C);
        DEXO = IERC20(0x27D6D8b845b0a06410C7C8123b259651934a9a48);
        oneSD = IERC20(0x90D44D04E7e208e0ea43765c26De90dddb65e297);
        oneXD = IERC20(0x6fE8f7646776Ce79a681727a6615A4D856Ef7Ae5);
    ...
    modifier stakingStarted {
        ...
        uint256 shinyBalance = oneSD.balanceOf(address(this));
    ...
    function stake(uint256 amount, address _address, uint256 tokenId) public stakingStarted {
        ...
        if (tokenId == 1) {
            ...
            investedDexoAmount = getRewardByUSDT(amount);
            token = DEXO;
        }
        if (tokenId == 2) {
            token = oneSD;
        ...
        if (enableTreasury) {
            token.transferFrom(msg.sender, treasuryAddress, tokenId == 1 ? investedDexoAmount : amount);
        } else {
            token.transferFrom(msg.sender, address(this), tokenId == 1 ? investedDexoAmount : amount);
        }
    ...
    function unStake(uint256 stakeTime, address user, bool urgentUnstake) public onlyAdmin {
        ...
        IERC20 token;
        if (users[user][stakeIndex].tokenId == 0) { token = USDT; }
        if (users[user][stakeIndex].tokenId == 1) { token = DEXO; }
        if (users[user][stakeIndex].tokenId == 2) { token = oneSD; }

        token.transfer(user, users[user][stakeIndex].tokenId == 1 ? (users[user][stakeIndex].dexoAmount * (100 - penaltyPercent)) / 100 : (users[user][stakeIndex].amount * (100 - penaltyPercent)) / 100);
    ...
    function setShinyToken(IERC20 newShinyToken) public onlyAdmin {
        oneSD = newShinyToken;
    }
