...
contract MetaCrowdV2 is AccessControlEnumerable {
    ...
    function setProjectParams(uint256 regionIndex, uint256 round, uint16 state) external onlyRole(OPERATOR_ROLE){
    ...
    function fund(address addr, uint256 regionIndex, uint256 amount) public {
        require(recordMetaPoint(addr), "no meta point account");
        require(amount > 0, "zero amount error");
        require(accounts[addr].lastFundTime + fundInterval < block.timestamp, "funding time limited");
