...
contract KojiMarket is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function removeMarketItem(uint256 tokenId) external nonReentrant {
      ...
      require(idToMarketItem[tokenId].owner == address(msg.sender), "Only item owner can perform this operation");
    ...
    function fetchMyNFTs() public view returns (MarketItem[] memory) {
      ...
      if(soldtokens.length > 0) {
        ...
        for (uint i = 0; i < soldtokens.length; i++) {
            if (idToMarketItem[soldtokens[i]].owner == address(msg.sender)) {
            itemCount++;
            }
        }
    ...
    function fetchItemsListed() public view returns (MarketItem[] memory) {
      ...
      if(totalItemCount > 0) {
        ...
        for (uint i = 0; i < totalItemCount; i++) {
            if (idToMarketItem[heldtokens[i]].seller == address(msg.sender)) {
            itemCount++;
            }
        }
