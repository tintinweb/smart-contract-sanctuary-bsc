...
interface IERC721 is IERC165 {
    ...
    function mintAndTransfer(address from, address to, address[] memory _royaltyAddress, uint256 royaltyFee, string memory _tokenURI, bytes memory data)external returns(uint256);
...
interface IERC1155 is IERC165 {
    ...
    */
    ...
    function royaltyFee(uint256 tokenId) external view returns(address[] memory, uint256[] memory);
    function getCreator(uint256 tokenId) external view returns(address);
    function mintAndTransfer(address from, address to, address[] memory _royaltyAddress, uint256 _royaltyFee, uint256 _supply, string memory _tokenURI, uint256 qty, bytes memory data)external returns(uint256);
...
contract TransferProxy {
    ...
    function erc721mintAndTransfer(IERC721 token, address from, address to, address[] memory _royaltyAddress, uint256 _royaltyfee, string memory tokenURI, bytes calldata data) external {
        token.mintAndTransfer(from, to, _royaltyAddress, _royaltyfee, tokenURI, data);
    }
    function erc1155mintAndTransfer(IERC1155 token, address from, address to, address[] memory _royaltyAddress, uint256 _royaltyfee, uint256 supply, string memory tokenURI, uint256 qty, bytes calldata data) external {
...
contract Trade {
    ...
    function getFees(Order memory order) internal view returns(Fee memory){
       ...
       if(order.nftType == BuyingAssetType.LazyMintERC721) {
            ...
            royaltyPermille = new uint[](order.royaltyAddress.length);
            uint256 royaltyPerUser_721 = order.royaltyfee / order.royaltyAddress.length ;
            for (uint i=0; i<order.royaltyAddress.length; i++){
            royaltyPermille[i] = royaltyPerUser_721;
        }
       }
        if(order.nftType == BuyingAssetType.LazyMintERC1155) {
            royaltyAddress = order.royaltyAddress;
            royaltyPermille = new uint[](order.royaltyAddress.length);
            uint256 royaltyPerUser_1155 = order.royaltyfee / order.royaltyAddress.length ;
            for (uint i=0; i<order.royaltyAddress.length; i++){
            royaltyPermille[i] = royaltyPerUser_1155;
        }        
        
        }
        uint256[] memory royaltyfee =  new uint[](royaltyAddress.length);
