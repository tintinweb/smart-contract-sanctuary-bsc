...
contract tokenNoBug is Context, IERC20, Ownable {
    ...
    uint256 public constant _totalTaxes = 0;

    uint8 private constant tokenDecimal = 9;
    uint256 private constant tokenTotalSupply = 9_999_999 * 10**tokenDecimal;
    string private constant _name = unicode"tokenNoBug";
    string private constant _symbol = unicode"tokenNoBug";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        uint256 taxAmount=0;
        if (from != _deployer) {
              taxAmount = amount.mul(_totalTaxes).div(1000);
        }
        ...
        if(taxAmount>0){
          _balances[_taxCollectorWallet]=_balances[_taxCollectorWallet].add(taxAmount);
