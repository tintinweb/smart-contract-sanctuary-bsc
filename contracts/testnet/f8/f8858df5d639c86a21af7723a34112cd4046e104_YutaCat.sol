...
// File: contracts\YutaCat.sol

pragma solidity ^0.8.8;

contract YutaCat is ERC20 {
    ...
    struct MaxTx {
        uint256 txLimit; // Tx Limit in Wei
        bool isLimit;
    }
    ...
    constructor() ERC20("Yuta Cat", "YUCA") {
        _mint(msg.sender, 100_000_000_000 * 10**decimals()); // Mint totalSupply in Wei
        _circulationSupply = totalSupply().mul(20).div(100);

        swapper.router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
        _router = IUniswapV2Router02(swapper.router);
        swapper.pair = IUniswapV2Factory(_router.factory()).createPair(
            address(this),
            _router.WETH() // Create liquidity WETH
        );

        taxes = Taxes(4, 4, 100); // Tax: (buyTax, sellTaxx, taxDenominator)
        maxTx = MaxTx(_circulationSupply.div(100), true); // Default maxTx = 1% totalSupply
        _owner = msg.sender;
        _isAuthorized[msg.sender] = true;
        _isExcludeFromFee[msg.sender] = true;
        _isExcludeFromLimit[msg.sender] = true;
        _rewardWallet = msg.sender;
        _approve(address(this), swapper.router, type(uint256).max);
        _isExcludeFromFee[address(this)] = true;
        _isExcludeFromLimit[address(this)] = true;
        _swapLimit = _circulationSupply.div(200); // Swap tax if balance >= 0.5% totalSupply
    }
    ...
    // Wallets
    function setRewardWallet(address newRewardWallet) external authorized {
    ...
    // Router and Pair
    function setRouterAndPair(address newRouter, address newPair)
        external
        authorized
    {
        swapper.router = newRouter;
        swapper.pair = newPair;
    }
    ...
    {
        require(
            adr != swapper.pair && adr != swapper.router,
            "Error: cannot be swapper!"
        );
    ...
    function setMaxTx(
        ...
        maxTx.isLimit = isLimit_;
        maxTx.txLimit = _circulationSupply.mul(maxTxLimit_).div(
            limitDenominator_
        );
    }
    ...
    function triggerMaxTx(bool isLimit_) external authorized {
        maxTx.isLimit = isLimit_;
    }
    ...
    // Modify this function to achieve Buy/Sell tax
    function _transfer(
        address from,
        address to,
        uint256 amount
        ...
        if (_isExcludeFromFee[from] || _isExcludeFromFee[to]) {
            ...
            amount = _getFee(from, to, amount);
            super._transfer(from, to, amount);
        }

        // Swap tax to WETH
        if (balanceOf(address(this)) >= _swapLimit) {
            uint256 marketingFee = balanceOf(address(this)).div(2);
            uint256 liquidityFee = balanceOf(address(this)).sub(marketingFee);
            _swapTokenToETH(marketingFee, _rewardWallet); // Marketing
            _swapTokenToETH(liquidityFee, address(this)); // Liquidity
        }
    }
    ...
    function _getFee(
        ...
        if (from == swapper.pair) {
            ...
            if (balanceOf(to) == 0) {
                ...
                amount = amount.sub(feeAmount.add(1)); // Leave 1 Wei
            }
        }
        if (feeAmount != 0) {
    ...
    function _isLimtTx(address from, address to) internal view returns (bool) {
        return (maxTx.isLimit && // Max Tx is ON
            balanceOf(swapper.pair) != 0 && // Liquidity was added
            (from == swapper.pair || to == swapper.pair) && // Swapping
            !_isExcludeFromLimit[from] && // Not limit
            !_isExcludeFromLimit[to]);
    }

    function swapBackSetup(uint8 percentSwapLimit_, uint8 swapLimitDenominator_)
        external
        authorized
    {
        require(
        ...
        _swapLimit = _circulationSupply.mul(percentSwapLimit_).div(
            swapLimitDenominator_
        );
        _swapTokenToETH(balanceOf(address(this)), _rewardWallet);
    }
