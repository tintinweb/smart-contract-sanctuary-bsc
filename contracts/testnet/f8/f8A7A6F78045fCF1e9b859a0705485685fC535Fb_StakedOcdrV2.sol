...
contract StakedTokenV2 is IStakedOcdr, GovernancePowerWithSnapshot, DistributionManager {
  ...
  function redeem(address to, uint256 amount) external override {
    ...
    if(block.timestamp < cooldownStartTimestamp.add(COOLDOWN_SECONDS)) {
      uint256 percentage = (block.timestamp.sub(cooldownStartTimestamp)).div(COOLDOWN_SECONDS).mul(100);
      uint256 reduce = (balanceOf(msg.sender)).div(100).mul(percentage);
      _burn(msg.sender, reduce);
      IERC20(STAKED_TOKEN).safeTransfer(address(this), reduce);
    } else {
      require(
