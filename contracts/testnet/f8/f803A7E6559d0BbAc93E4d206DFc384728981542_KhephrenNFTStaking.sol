...
contract KhephrenNFTStaking is IKhephrenNFTStaking{
    ...
    */
    ...
    function setPlatformPrice(uint256 _price) onlyOwner(msg.sender) external{
        price = _price;
    }
    ...
    */
    ...
    function createProject(address _nftAddress) payable external virtual override returns (address){
        require(msg.value == price, "ERC20: Invalid amount");
        address _clone = Clones.clone(projectImplementation);
        IProject721(_clone).setProjectDetails(_nftAddress, msg.sender);
    ...
    function getAllProject() view external virtual override returns(address[] memory){
        return mapProject721[msg.sender];
    }

    function setProjectAddress(address projectAddress) external{
        IProject721(projectAddress).setSetterAddress(address(this));
    }
