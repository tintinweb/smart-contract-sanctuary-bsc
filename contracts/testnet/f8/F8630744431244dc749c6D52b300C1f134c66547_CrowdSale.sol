...
//        stableCoinsLPs[stableCoins[2]] = StableCoinLP(WETH_USDT_Pair, tokenIndex2, 6);
    ...
    function getETHPrice() public pure returns (uint256 price) {
        price = 1566281972090847560152;
...
//        price = price / stableCoins.length;
    ...
    function userDeposit (uint256 _amount, address _baseToken) external payable nonReentrant {
        ...
        uint256 tokensSold = isETH
        ? amount_in.mul(ethPrice).mul(10 ** PRICE_DECIMALS).div(INFO.tokenPrice).div(10 ** 18)
        : amount_in.mul(10 ** PRICE_DECIMALS).div(INFO.tokenPrice).div(10 ** uint256(IERC20(_baseToken).decimals()));
        ...
        if(isETH) {
            ...
            buyer.depositedValue = buyer.depositedValue.add(amount_in.mul(ethPrice));
            STATUS.totalETHCollected = STATUS.totalETHCollected.add(amount_in);
            STATUS.totalETHValueCollected = STATUS.totalETHValueCollected.add(amount_in.mul(ethPrice));
