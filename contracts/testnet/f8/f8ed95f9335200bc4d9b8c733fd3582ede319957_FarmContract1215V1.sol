...
contract FarmContract1215V1 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(uint256 => mapping(address => uint256)) public deservedMintedAmount;
    mapping(uint256 => mapping(address => mapping(address => uint256))) public claimedRefferalReward;
    ...
    function pendingCumulatedReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalPendingReward) {
        uint256 accMiningTokenPerShare = poolInfo[_pid].accMiningTokenPerShare;
        uint256 lpSupply = poolInfo[_pid].lpToken.balanceOf(address(this));
        if (block.number > poolInfo[_pid].lastUpdateBlock && lpSupply != 0) {
            uint256 blockInterval = block.number.sub(poolInfo[_pid].lastUpdateBlock);
        ...
        for(uint256 i = 0; i < userDepositInfo[_pid][_user].length; i++) {
            if(userDepositInfo[_pid][_user][i].onGoing) {
                ...
                uint256 recordAccMiningTokenPerShare = currentAccMiningTokenPerShare[_pid][userDepositInfo[_pid][_user][i].depositBlock];
                uint256 actualAccMiningTokenPerShare = accMiningTokenPerShare.sub(recordAccMiningTokenPerShare);
                uint256 pendingReward = amount.mul(actualAccMiningTokenPerShare).div(1e12).sub(userDepositInfo[_pid][_user][i].cumulatedRewardDebt);
                totalPendingReward = totalPendingReward.add(pendingReward);
            }
        }
    }

    function updateDeservedCumulatedReward(uint256 _pid, address _user) internal {
        for(uint256 i = 0; i < userDepositInfo[_pid][_user].length; i++) {
            if(userDepositInfo[_pid][_user][i].onGoing) {
                uint256 amount = userDepositInfo[_pid][_user][i].amount;
                uint256 recordAccMiningTokenPerShare = currentAccMiningTokenPerShare[_pid][userDepositInfo[_pid][_user][i].depositBlock];
                ...
                uint256 pendingReward = amount.mul(actualAccMiningTokenPerShare).div(1e12).sub(userDepositInfo[_pid][_user][i].cumulatedRewardDebt);
                userDepositInfo[_pid][_user][i].cumulatedRewardDebt = userDepositInfo[_pid][_user][i].cumulatedRewardDebt.add(pendingReward);
                deservedMintedAmount[_pid][_user] = deservedMintedAmount[_pid][_user].add(pendingReward);
            }
        }
    }

    function getDeservedRefferalReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 PendingReward) {
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            uint256 pending = pendingCumulatedReward(_pid, InviteeAddress[i]).add(deservedMintedAmount[_pid][InviteeAddress[i]]).sub(claimedRefferalReward[_pid][InviteeAddress[i]][_user]);
            PendingReward = PendingReward.add(pending);
        }
    }

    function getDeservedNodeReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 PendingReward) {
        if(getLevel(_pid, _user) <= 1)  return 0;
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            if(getLevel(_pid, InviteeAddress[i]) < getLevel(_pid, _user)) {
                uint256 pending = pendingCumulatedReward(_pid, InviteeAddress[i]).add(deservedMintedAmount[_pid][InviteeAddress[i]]).sub(claimedNodeReward[_pid][InviteeAddress[i]][_user]);
                PendingReward = PendingReward.add(pending);
            }
        }
    }

    function getAllPendingRefferalReward(address _user) public view returns (uint256 totalPendingRefferal) {
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            uint256 pending = getDeservedRefferalReward(pid, _user).mul(getRefferalRewardPercent(pid, _user)).div(rateDenominator);
            totalPendingRefferal = totalPendingRefferal.add(pending);
        }
    }

    function getAllPendingNodeReward(address _user) public view returns (uint256 totalPendingNode) {
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            uint256 pending = getDeservedNodeReward(pid, _user).mul(getNodeRewardPercent(pid, _user)).div(rateDenominator);
            totalPendingNode = totalPendingNode.add(pending);
        }
    }
    ...
    function claimRefferalReward(uint256 _pid) internal returns (uint256 PendingReward) {
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            updateDeservedCumulatedReward(_pid, InviteeAddress[i]);
            PendingReward = pendingCumulatedReward(_pid, InviteeAddress[i]).add(deservedMintedAmount[_pid][InviteeAddress[i]]);
            PendingReward = PendingReward.sub(claimedRefferalReward[_pid][InviteeAddress[i]][_msgSender()]);
            claimedRefferalReward[_pid][InviteeAddress[i]][_msgSender()] = claimedRefferalReward[_pid][InviteeAddress[i]][_msgSender()].add(PendingReward);
        }
    }
    ...
    function claimNodeReward(uint256 _pid) internal returns (uint256 PendingReward) {
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            if(getLevel(_pid, InviteeAddress[i]) < getLevel(_pid, _msgSender())) {
                updateDeservedCumulatedReward(_pid, InviteeAddress[i]);
                PendingReward = pendingCumulatedReward(_pid, InviteeAddress[i]).add(deservedMintedAmount[_pid][InviteeAddress[i]]);
                PendingReward = PendingReward.sub(claimedNodeReward[_pid][InviteeAddress[i]][_msgSender()]);
                claimedNodeReward[_pid][InviteeAddress[i]][_msgSender()] = claimedNodeReward[_pid][InviteeAddress[i]][_msgSender()].add(PendingReward);
            }
        }
    }
    ...
    function claimAllRefferalReward() public returns(uint256 totalRefferalReward) {
        ...
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            updatePool(pid);
            uint256 pending = claimRefferalReward(pid);
            pending = pending.mul(getRefferalRewardPercent(pid, _msgSender())).div(rateDenominator);
    ...
    function claimAllNodeReward() public returns(uint256 totalNodeReward) {
        ...
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            updatePool(pid);
            uint256 pending = claimNodeReward(pid);
            pending = pending.mul(getRefferalRewardPercent(pid, _msgSender())).div(rateDenominator);
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePid(_pid) {
        ...
        updatePool(_pid);
        updateDeservedCumulatedReward(_pid, _msgSender());
