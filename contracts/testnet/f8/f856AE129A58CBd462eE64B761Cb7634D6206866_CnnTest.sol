...
contract CnnTest is ERC20, Ownable {
    ...
    // function _addLiquidityAuto(uint amount_) internal {

    //     address[] memory path = new address[](2);
    //     path[0] = address();
    //     path[1] = address();
        
    //     (uint re0, uint re1) = _getReserves(path[0], path[1]);
    //     router.addLiquidity(path[0], path[1], amount_,  amount_ * re1 / re0 , 0, 0, address(this), block.timestamp + 720);

    //     router.swapExactTokensForTokens(amount_, 0, path, address(this), block.timestamp + 720);
    // }

        // function _getReserves(address t0_, address t1_) public view returns(uint, uint) {
    //     require(t0_ != address(0) && t1_ != address(0), 'a 0');
    //     address pair = factory.getPair(t0_, t1_);
