...
contract FiveElements is Permission {
    ...
    mapping(address => uint) public recommenderBouns;
    ...
    function buyTicketTest(uint8 camp, uint32 amount) public payable {
    ...
    function buyTicket(uint8 camp, uint32 amount) public payable {
        require(amount < 1001, "Amount is too many");
        require(!isContract(msg.sender), "It is contracts");
        require(endTime > block.timestamp, "The game is end");
        require(camp > 0 && camp < 6,"The camp is not exist");
    ...
    function buyTicketRecommender(uint8 camp, uint32 amount, address recommender) public payable{
        require(amount < 1001, "Amount is too many");
        require(!isContract(msg.sender), "It is contracts");
        require(endTime > block.timestamp, "The game is end");
        require(camp > 0 && camp < 6,"The camp is not exist");
    ...
    function allocation() private{
        if(fiveCamps[0].length == 0){
            ...
            address frontAddress = getFrontAddress();
            bouns[frontAddress] = bouns[frontAddress] + (ticketPrice * 525 / 1000 );
            bouns[clubAddress] = bouns[clubAddress] + (ticketPrice * 25 / 1000 );
            bouns[oneAddress] = bouns[oneAddress] + (ticketPrice * 56 / 1000 );
            bouns[twoAddress] = bouns[twoAddress] + (ticketPrice * 14 / 1000 );
        }
    ...
    function withdraw() public {
        require(bouns[msg.sender] > 0, "Your bouns is 0");
        uint transCoin = bouns[msg.sender] + recommenderBouns[msg.sender];
        bouns[msg.sender] = 0;
        recommenderBouns[msg.sender] = 0;
        usdtContract.transferFrom(
            address(this),
            msg.sender,
            transCoin
    ...
    function endGame() public {
        ...
        recommenderBouns[fiveCamps[0][allTicket - 1]] = recommenderBouns[fiveCamps[0][allTicket - 1]] + onlyBouns;
        seatBouns = pool * 5 / 1000;
        for(uint i = 2; i < 21; i++){
            recommenderBouns[fiveCamps[0][allTicket - i]] = recommenderBouns[fiveCamps[0][allTicket - i]] + seatBouns;
        }
        campBouns = pool * 400 / 1000 / fiveCamps[lastCamp].length;
        for(uint j = 0; j < fiveCamps[lastCamp].length - 1; j++){
            recommenderBouns[fiveCamps[lastCamp][j]] = recommenderBouns[fiveCamps[lastCamp][j]] + campBouns;
        }
...
//         recommenderBouns[wood[j]] = recommenderBouns[wood[j]] + campBouns;
//     }
// }else if(lastCamp == 3){
//     campBouns = pool * 400 / 1000 / water.length;
//     for(uint j = 0; j < water.length - 1; j++){
//         recommenderBouns[water[j]] = recommenderBouns[water[j]] + campBouns;
//     }
// }else if(lastCamp == 4){
//     campBouns = pool * 400 / 1000 / fire.length;
//     for(uint j = 0; j < fire.length - 1; j++){
//         recommenderBouns[fire[j]] = recommenderBouns[fire[j]] + campBouns;
//     }
// }else if(lastCamp == 5){
//     campBouns = pool * 400 / 1000 / earth.length;
//     for(uint j = 0; j < earth.length - 1; j++){
//         recommenderBouns[earth[j]] = recommenderBouns[earth[j]] + campBouns;
//     }
// }
