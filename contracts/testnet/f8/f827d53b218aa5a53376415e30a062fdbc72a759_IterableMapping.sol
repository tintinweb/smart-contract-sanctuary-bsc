...
contract T is ERC20, Ownable {
    ...
    address public marketingWallet = 0xB5e81d89eA99A609B5C6575a5D9Bb2f24Cc8A9Ef;
    address public BuybackWallet = 0x0b2e68B2Fc3332677CFdaDa1510E752306664eAe;
    ...
    IERC20 public ADA = IERC20(0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47);
    IERC20 public BUSD = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    uint256 public busdDividendRewardsFee = 5;

    uint256 public adaDividendRewardsFee = 3;

    uint256 public marketingFee = 4;
    uint256 public buybackFee = 1;

    uint256 public totalFees = busdDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee).add(buybackFee);
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "T: The marketing wallet is already this address");
    ...
    function updateBuybackWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != BuybackWallet, "T: The Buyback wallet is already this address");
    ...
    function updatebusdTokenDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdTokenDividendTracker), "T: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdTokenDividendTracker =  BUSDDividendTracker(payable(newAddress));
 
        require(newbusdTokenDividendTracker.owner() == address(this), "T: The new dividend tracker must be owned by the T token contract");
    ...
    function updateRc1DividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(adaDividendTracker), "T: The dividend tracker already has that address");
 
        ADADividendTracker newadaDividendTracker = ADADividendTracker(payable(newAddress));
 
        require(newadaDividendTracker.owner() == address(this), "T: The new dividend tracker must be owned by the T token contract");
    ...
    function updateBusdDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = busdDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee).add(buybackFee);
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "T: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "T: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "T: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "T: Cannot update gasForProcessing to same value");
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		...
		emit ProcessedadaDividendTracker(adaIterations, adaClaims, adaLastProcessedIndex, false, gas, msg.sender);
    }
    ...
    function _transfer(
        ...
        if (canSwap &&
            ...
            swapping = true;
                ...
                uint256 swapTokensBuyback = contractTokenBalance.div(totalFees).mul(buybackFee);
                uint256 initialBalanceBuyback = address(this).balance;
                swapTokensForBNB(swapTokensBuyback);
                uint256 BuybackPortion = address(this).balance.sub(initialBalanceBuyback);
                 address(BuybackWallet).call{value: BuybackPortion}("");
       ...
       try busdTokenDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        ...
        if(!swapping) {
	    	...
	    	try adaDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedadaDividendTracker(iterations, claims, lastProcessedIndex, true, gas, msg.sender);
	    	}
	    	catch {
    ...
    function swapTokensForADA(uint256 _tokenAmount) private {
    ...
    function swapAndSendADADividends(uint256 tokens) private {
        ...
        uint256 adaTokens = IERC20(ADA).balanceOf(address(this));
        transferDividends( address(adaDividendTracker), adaDividendTracker, adaTokens, ADA);
    }
...
contract ADADividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("T_ADA_Dividend_Tracker", "T_ADA_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "T_ADA_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "T_ADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main T contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "T_ADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "T_ADA_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BUSDDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("T_BUSD_Dividend_Tracker", "T_BUSD_Dividend_Tracker", 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "T_BUSD_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "T_BUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main T contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "T_BUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "T_BUSD_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
