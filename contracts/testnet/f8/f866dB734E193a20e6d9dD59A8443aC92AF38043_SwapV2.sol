...
contract SwapV2 is BaseContract
{
    ...
    function _buy(address buyer_, address payment_, uint256 amount_) internal returns (uint256)
    {
        ...
        // Swap USDC for FUR.
        usdc.approve(address(liquidityManager), _usdcAmount_ - _usdcTax_);
        liquidityManager.swapUsdcForToken(address(this), _usdcAmount_ - _usdcTax_, 0);
    ...
    function _buyUsdc(address payment_, uint256 amount_, address buyer_) internal returns (uint256)
    {
        ...
        _path_[0] = address(_payment_);
        _path_[1] = address(usdc);
        _payment_.approve(address(router), _payment_.balanceOf(address(this)) - _paymentBalance_);
    ...
    function sell(uint256 amount_) external whenNotPaused
    {
        ...
        uint256 _usdcBalance_ = usdc.balanceOf(address(this));
        // Swap USDC for FUR.
        fur.approve(address(liquidityManager), _furReceived_ - _furTax_);
        liquidityManager.swapTokenForUsdc(address(this), _furReceived_ - _furTax_, 0);
