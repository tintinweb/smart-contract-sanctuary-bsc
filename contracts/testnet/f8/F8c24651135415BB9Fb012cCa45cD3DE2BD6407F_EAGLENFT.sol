...
contract EAGLENFT is ERC721 {
    ...
    function initEagleToken(address _EagleToken) external {
        ...
        require(_msgSender() == MarketingWallet,"you not MarketingWallet");
    ...
    function sellNFT(uint256 _tokenId, uint256 _price) external {
        ...
        require(
            block.timestamp - NFTConfig[_tokenId].creationTime > 1 minutes,
            "Your NFT casting has not lasted more than a month!!!"
        );
        uint256 receiveNum = (block.timestamp -
            NFTConfig[_tokenId].creationTime) / 1 minutes;
        if (receiveNum > 12) {
            receiveNum = 12;
        }
        require(
    ...
    function getReceive(uint256 tokenId) external view returns (bool) {
        uint256 receiveNum = (block.timestamp -
            NFTConfig[tokenId].creationTime) / 1 minutes;
        if (receiveNum > 12) {
            receiveNum = 12;
        }
