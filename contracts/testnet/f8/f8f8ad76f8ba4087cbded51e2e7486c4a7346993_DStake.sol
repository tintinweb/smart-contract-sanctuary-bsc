...
contract DStake is IERC721Receiver, Ownable {
  ...
  function upgradeStake(uint8 tierId) public whenNotPaused {
    ...
    require(tierId > userStake.tierId, "should upgrade to the higher levels");
    ...
    if (userStake.nftDiscount == 1) {
      ...
      diffCostInToken1 = diffCostInToken1.sub(
        diffCostInToken1.mul(pairs[userStake.pairId].nftPercent).div(10000)
      );
    } else if (userStake.nftDiscount == 2) {
      // used on token2 cost
      diffCostInToken2 = diffCostInToken2.sub(
        diffCostInToken2.mul(pairs[userStake.pairId].nftPercent).div(10000)
      );
    }
  ...
  function unStake() public {
    ...
    if (userStake.token1Amount > 0) {
      IERC20(userPair.token1).transfer(msg.sender, userStake.token1Amount);
    }
    if (userStake.token2Amount > 0) {
      IERC20(userPair.token2).transfer(msg.sender, userStake.token2Amount);
    }
    if (userStake.nft != address(0)) {
      require(
        IERC721(userStake.nft).ownerOf(userStake.nftId) == address(this),
        "!owner"
      );
  ...
  function forceUnstake() public {
    ...
    Pair memory userPair = pairs[userStake.pairId];
    require(userStake.staked, "!staked");
    require(userPair.earlyUnstake, "EarlyUnstake is not allowed for this pair");
    require(
      userStake.startTime.add(userPair.lockPeriod) > block.timestamp,
      "can't unstake"
    );
