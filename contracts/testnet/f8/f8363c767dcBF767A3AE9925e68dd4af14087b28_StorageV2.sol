...
import "./Libs/ILogicContract.sol";
...
contract StorageV2 is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function withdraw(uint256 amount, address token) external isUsedToken(token) whenNotPaused {
        ...
        if (amountExp18 > tokenBalance[token]) {
            uint256 requireReturnAmount = amount - (tokenBalance[token] / (10**(18 - decimals)));
            LogicContract(logicContract).returnToken(requireReturnAmount, token);
            interestFee();
            IERC20Upgradeable(token).safeTransferFrom(logicContract, address(this), requireReturnAmount);
