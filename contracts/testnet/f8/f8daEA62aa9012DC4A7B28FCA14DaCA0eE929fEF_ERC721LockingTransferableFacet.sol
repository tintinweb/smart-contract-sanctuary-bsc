...
import "./ERC721TransferableFacet.sol";
import "../interfaces/IERC5058Facet.sol";
import "../libraries/LibERC5058.sol";
import "@openzeppelin/contracts/token/ERC721/IERC721Receiver.sol";
import "@openzeppelin/contracts/utils/Address.sol";
import {LibMeta} from "../../../../libraries/diamond/LibMeta.sol";
import {AppStorage} from "../AppStorage.sol";

contract ERC721LockingTransferableFacet is
    ERC721TransferableFacet,
    IERC5058Facet
{
    using Address for address;

    function lockerOf(uint256 tokenId) public view returns (address) {
        require(isLocked(tokenId), "ERC5058: token is not locked");
        return LibERC5058.lockerOf(tokenId);
    }

    function _isApprovedToLockOrOwner(address spender, uint256 tokenId)
        internal
        view
        virtual
        returns (bool)
    ...
    function lock(uint256 tokenId, uint256 expired) external {
        ...
        require(!isLocked(tokenId), "ERC5058: token is already locked");
        require(
            _isApprovedToLockOrOwner(LibMeta.msgSender(), tokenId),
            "ERC5058: not allowed to lock"
        );
        address sender = LibMeta.msgSender();
        LibERC5058.lock(tokenId, expired, sender);
        emit Locked(sender, _ownerOf(tokenId), tokenId, expired);
    }
    ...
    function unlock(uint256 tokenId) external {
        ...
        address sender = LibMeta.msgSender();
        require(
            lockerOf(tokenId) == LibMeta.msgSender(),
            "ERC5058: not allowed to unlock"
        );
        LibERC5058.unlock(tokenId);
        emit Unlocked(sender, _ownerOf(tokenId), tokenId);
    }
    ...
    function lockApprove(address to, uint256 tokenId) external {
        ...
        // solhint-disable-next-line reason-string
        require(
            LibMeta.msgSender() == owner || isLockApprovedForAll(owner, sender),
            "ERC5058: not allowed to lock approve"
        );
        LibERC5058.lockApprove(to, tokenId);
        emit LockApproval(owner, to, tokenId);
    }

    function setLockApprovalForAll(address operator, bool approved) external {
        address sender = LibMeta.msgSender();
        require(sender != operator, "ERC5058: lock approve to caller");
        LibERC5058.setLockApprovalForAll(sender, operator, approved);
        emit LockApprovalForAll(sender, operator, approved);
    }

    function getLockApproved(uint256 tokenId) public view returns (address) {
        _requireMinted(tokenId);
        return LibERC5058.getLockApproved(tokenId);
    }
    ...
    function isLockApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        return LibERC5058.isLockApprovedForAll(owner, operator);
    }

    function isLocked(uint256 tokenId) public view returns (bool) {
        // solhint-disable-next-line not-rely-on-time
        return LibERC5058.lockExpiredTime(tokenId) >= block.timestamp;
    }

    function lockExpiredTime(uint256 tokenId) external view returns (uint256) {
        return LibERC5058.lockExpiredTime(tokenId);
    }
    ...
    function _transfer(
        ...
        require(!isLocked(tokenId_), "ERC5058: token is locked");
        // solhint-disable-next-line reason-string
        require(
