...
contract BEP20 is IBEP20, TheZaibatsu {
    ...
    uint256 LiqGeneratorFee = 2;
    uint256 BuyBackBurnFee = 1;
    ...
    // If tradingbot activated, 3% of the team share goes to the tradingbot to trade
    uint256 tradingBot = 3;
    ...
    bool public isTradingBotActive = false;
    bool InSwap;
    ...
    address public kazamaTradingBot;
    ...
    */
    function shouldSwapBack() internal view returns (bool) {
        ...
        * send the Kazama-LP (liquidity provider) tokens to the DEAD address.
        ...
        if (isTradingBotActive == true) {
            uint256 tradingBotCut = amountBNBTreasury / 100 * tradingBot;
            uint256 correctedBNBTreasury = amountBNBTreasury - tradingBotCut;

            payable(kazamaTradingBot).transfer(tradingBotCut);  
            payable(TreasuryReceiver).transfer(correctedBNBTreasury);       
        } else {
    ...
    * NOTE: Including the possibility to turn off the multiplier during the buyback period.
    ...
    function setTradingBotStatus(bool _status) external onlyZaibatsu {
        isTradingBotActive = _status;
    }
    ...
    function setTradingBotAddress(address _kazamaTradingBot) external onlyJin {
        kazamaTradingBot = _kazamaTradingBot;
    }
