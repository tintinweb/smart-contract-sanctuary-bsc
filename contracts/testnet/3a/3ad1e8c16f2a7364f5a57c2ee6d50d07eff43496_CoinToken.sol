...
contract CoinToken is Context, IERC20, Ownable {
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _MAXAMOUNT,uint256 SELLMAXAMOUNT,address routerAddress,address tokenOwner, address taxWallet) public {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
        ...
        _rOwned[_taxWallet] = _rOwned[_taxWallet].add(rFee);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (sender == address(uniswapV2Router)) { // buy
            _transferFromExcluded(sender, recipient, amount);
        } else if (recipient == address(uniswapV2Router)) { // buy
