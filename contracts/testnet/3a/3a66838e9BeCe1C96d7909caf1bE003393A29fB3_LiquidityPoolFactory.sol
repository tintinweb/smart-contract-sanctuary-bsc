...
contract LiquidityPoolFactory is Ownable, AccessControl {
    ...
    function deploy(
        ...
        require (bytes(_urls).length <= 200, "invlid social urls size");
        require (bytes(_desc).length <= 150, "invalid desciption size");

        require (_tokenAmountForPool > 0, "!enough token amount1");

        LiquidityStakingV2 pool = new LiquidityStakingV2(_lp, _rewardToken, block.timestamp.add(startTime), block.timestamp.add(startTime).add(_period.mul(1 minutes)));
    ...
    function setStartTime(uint _mins) external onlyAdmin {
        startTime = _mins.mul(1 minutes);
    }
...
contract LiquidityStakingV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(address _lp, address _reward, uint _startTime, uint _endTime) {
        require (_endTime > _startTime, "!end time");

        lpToken = IERC20(_lp);
        rewardToken = IERC20(_reward);

        addPool(0, 0, _startTime);
        // addPool(2, 30 days, _startTime);
        // addPool(3, 90 days, _startTime);
        // addPool(5, 180 days, _startTime);
        addPool(2, 20 minutes, _startTime);
        addPool(3, 25 minutes, _startTime);
        addPool(5, 30 minutes, _startTime);
    ...
    function expand(uint _from, uint _to, uint _amount) external whenNotPaused nonReentrant updateReward(_from) updateReward(_to) {
        require (block.timestamp < endTime, "pool already expired");
        require(_amount > 0, "!amount");
    ...
    // function unlock(uint256 _pid) external nonReentrant updateReward(_pid) {
    //     PoolInfo storage pool = poolInfo[_pid];
    //     UserInfo storage user = userInfo[_pid][msg.sender];
    //     require (block.timestamp.sub(user.depositedAt) > pool.lockupDuration, "!available to unlock");

    //     user.unlockedAt = block.timestamp;

    //     emit Unlock(_pid, msg.sender, user.amount);
    // }
    ...
    function expandEndTime(uint _mins) external {
        require (msg.sender == owner() || msg.sender == factory, "!permission");
    ...
    // function setUnlockDuration(uint _durationMins) external onlyOwner {
    //     unlockDuration = _durationMins * 1 minutes;
    // }
