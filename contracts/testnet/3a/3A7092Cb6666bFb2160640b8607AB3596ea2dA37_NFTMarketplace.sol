...
contract NFToken is ERC721, SupportsInterface {
    ...
    modifier canOperate(uint256 _tokenId) {
        ...
        require(
            tokenOwner == msg.sender || ownerToOperators[tokenOwner][msg.sender],NOT_OWNER_OR_OPERATOR);
        _;
    }
...
contract NFTokenMetadata is NFToken, ERC721Metadata {
    ...
    function tokenURI(uint256 _tokenId)external view override validNFToken(_tokenId) returns (string memory){
...
contract NFTMarketplace is NFTokenMetadata, Ownable {
    ...
    address payable _owner = payable(0xC5C65074064e5283C9D40403208Af004c8960b95);
        // Array with all token ids, used for enumeration
    uint256[] private _allTokens;
    constructor(){}
    function mint(string memory _nftName,string memory _nftSymbol,address _to,uint256 _tokenId,string calldata _uri) external payable{
        nftName = _nftName;
        nftSymbol=_nftSymbol;
        // Check that the number of tokens requested wouldn't exceed what's left.
        require(totalSupply().add(maxMint) <= MAX_TOKENS, "Minting would exceed max. supply");
