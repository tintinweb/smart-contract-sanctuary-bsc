...
contract INFLIV is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
	...
	PartnersInvestorsPool partnersInvestorsPool;
	...
	BountyBonusesPool bountyBonusesPool;
	...
	ListingsLiquidityPool listingsLiquidityPool;
	...
	GovernanceFeeShare governanceFeeShare;
	...
	function setStakingPoolAddress(address _stakingPoolAddress) public onlyOwner {
	    require(_stakingPoolAddress != address(0), "ERC20: Zero address");
        stakingPoolAddress = _stakingPoolAddress;
    }
	
	function setFarmPoolAddress(address _farmPoolAddress) public onlyOwner {
	    require(_farmPoolAddress != address(0), "ERC20: Zero address");
        farmPoolAddress = _farmPoolAddress;
    }
	
	function setInsurancePoolAddress(address _insurancePoolAddress) public onlyOwner {
	    require(_insurancePoolAddress != address(0), "ERC20: Zero address");
        insurancePoolAddress = _insurancePoolAddress;
    }
	
	function setBountyPoolAddress(address _bountyPoolAddress) public onlyOwner {
	    require(_bountyPoolAddress != address(0), "ERC20: Zero address");
        bountyPoolAddress = _bountyPoolAddress;
    }
	
	function mintPartnersInvestorsPoolTokens(address to, uint256 amount) public onlyOwner {
	   uint256 available = availablePartnersInvestorsPoolTokens();
	   require(available >= amount, "ERC20: Amount is not available for mint");
	...
	function mintBountyBonusesPoolTokens(address to, uint256 amount) public {
	   uint256 available = availableBountyBonusesPoolTokens();
	   require(available >= amount, "ERC20: Amount is not available for mint");
	   require(msg.sender == bountyPoolAddress || msg.sender == owner(), "ERC20: Caller is not correct");
	...
	function mintTreasuryPoolTokens(address to, uint256 amount) public onlyOwner {
	...
	function mintInsurancePoolTokens(address to, uint256 amount) public onlyOwner {
	   uint256 available = availableInsurancePoolTokens();
	   require(available >= amount, "ERC20: Amount is not available for mint");
	   require(msg.sender == insurancePoolAddress, "ERC20: Caller is not correct");
	...
	function mintCharityPoolTokens(address to, uint256 amount) public onlyOwner {
	...
	function mintReservePoolTokens(address to, uint256 amount) public onlyOwner {
	...
	function addTeamMember(address account, uint256 share) public onlyOwner() {
        require(!teamMember[account], "Member is already added");
        teamMember[account] = true;
		teamShare[account] = share;
        teamList.push(account);
    }
	
    function removeTeamMember(address account) external onlyOwner() {
        require(teamMember[account], "Account is already removed");
        for (uint256 i = 0; i < teamList.length; i++) {
            if (teamList[i] == account) {
                teamList[i] = teamList[teamList.length - 1];
                teamMember[account] = false;
                teamList.pop();
                break;
            }
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20Upgradeable){      
		...
		if(block.timestamp >= teamPool.lastClaimTime + teamPool.claimInterval) {
		   uint256 TAmount = availableTeamPoolTokens();
		   if(TAmount > 0 && teamList.length > 0) 
		   {
				for (uint256 i = 0; i < teamList.length; i++) {
					address teamAddress = teamList[i];
					uint256 share = teamShare[teamList[i]];
					uint256 amountToMint = TAmount * share / 10000;
					
					super._mint(teamAddress, amountToMint);
				}
				teamPool.lastClaimTime = block.timestamp;
		...
		if(block.timestamp >= communityPool.lastClaimTime + communityPool.claimInterval) {
		    uint256 CPAmount = availableCommunityPoolTokens();
			if(CPAmount > 0) 
			{
			    uint256 stakingPoolShare = CPAmount * 30 / 100;
				uint256 farmPoolShare = CPAmount - stakingPoolShare;
				
				super._mint(stakingPoolAddress, stakingPoolShare);
				super._mint(farmPoolAddress, farmPoolShare);
	...
	function _distributeFee(uint256 amount) internal{
	    ...
	    uint256 reservePoolShare  = amount - communityPoolShare - liquidityPoolShare - treasuryPoolShare - bountyBonusesPoolShare - insurancePoolShare - charityPoolShare - teamPoolShare;
