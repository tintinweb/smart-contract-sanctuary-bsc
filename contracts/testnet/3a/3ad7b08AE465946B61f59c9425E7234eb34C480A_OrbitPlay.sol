...
contract OrbitPlay is Ownable {
    ...
    event DiceRollResult(address bidder, uint256 betAmount, uint256 betNumber, uint256 winNumber, uint256 returned);
    ...
    function placeDiceRollBet(uint256 amount, uint256 diceNumber) public returns (uint256, uint256){
		require(
            dicePlayable,
        ...
        emit DiceRollResult(msg.sender, amount, diceNumber, destiny, userInfo.returningAmount);

        return (destiny, userInfo.returningAmount);
	}

    function updatePlayInfo(uint256 paidOut, uint256 burnt) private {
    ...
    function diceRandom() private view returns (uint) {       	
        uint rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, msg.sender)));
        rnd = rnd % 6 + 1;
        return rnd;
    }
