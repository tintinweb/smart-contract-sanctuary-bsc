...
contract BASBridge is Context, Ownable, ReentrancyGuard {
    ...
    IERC20 public BAS = IERC20(_tokenAddress);
    ...
    event DepositBAS(address indexed account, uint256 amounts, uint256 timestamp);
    event WithDrawBAS(address indexed account, uint256 orderId, uint256 amounts, uint256 timestamp);


    constructor (address basAddress, address validator) public {
        _tokenAddress = basAddress;
    ...
    // BAS approve first
    function depositBAS(uint256 amounts) public nonReentrant {
        require(amounts > 0, "can not deposit 0 amounts");

        _depositAmounts[_msgSender()] = _depositAmounts[_msgSender()].add(amounts);
        BAS.safeTransferFrom(_msgSender(), address(this), amounts);

        emit DepositBAS(_msgSender(), amounts, block.timestamp);
    }

    function withdrawBAS(Order memory order, bytes memory signature) public nonReentrant {
        ...
        require(order.tokenAddress == _tokenAddress, "not BAS address");
        ...
        BAS.safeTransfer(_msgSender(), order.amounts);

        emit WithDrawBAS(_msgSender(), order.orderId, order.amounts, block.timestamp);
    }
