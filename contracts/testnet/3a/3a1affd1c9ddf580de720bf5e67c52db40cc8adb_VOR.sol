...
contract VOR is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress =
        ...
        //0x3C7989A630d2c8B79fd7BCF3C56823411E17BAc1-----> pool address
    address public immutable deadAddress =
        0x000000000000000000000000000000000000dEaD;
    mapping(address => uint256) private _rOwned;
    mapping(address => uint256) private _tOwned;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private isLpHelpers;
    mapping(address => bool) private isTeamMembers;
    ...
    string private _symbol = "VOR-Test";
    ...
    uint256 public startTimeForLp;
    uint256 public lockPeriodForTeam;
    uint256 public lockPeriodForWhale;
    ...
    function setLpHelpers(address[] memory lpHelpers)external  onlyOwner{
        for(uint256 i=0;i<lpHelpers.length;i++){
            isLpHelpers[lpHelpers[i]]=true;
        }
    }
    function setTeamMembers(address[] memory teamMembers)external  onlyOwner{
        for(uint256 i=0;i<teamMembers.length;i++){
            isTeamMembers[teamMembers[i]]=true;
        }
    }
    ...
    function setLpTime() external onlyOwner{
        startTimeForLp=block.timestamp;
    }
    function setLockPeriodForTeam(uint256 period) external onlyOwner{
        lockPeriodForTeam=period;        
    }
    function setLockPeriodForWhale(uint256 period) external onlyOwner{
        lockPeriodForWhale=period;        
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            if (to == uniswapV2Pair) {
                ...
                uint256 canSaleTimeForTeam=startTimeForLp+lockPeriodForTeam;
                uint256 canSaleTimeForWhale=startTimeForLp+lockPeriodForWhale;
                if(isLpHelpers[from]){
                    require(block.timestamp>=canSaleTimeForWhale,"LiquidityHelpers can't sell during for a period of time.");
                }
                if(isTeamMembers[from]){
                    require(block.timestamp>=canSaleTimeForTeam,"TeamMembers can't sell during for a period of time.");
                }
