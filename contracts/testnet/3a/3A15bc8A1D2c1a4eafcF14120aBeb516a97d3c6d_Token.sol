...
contract Token is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20 {
    ...
    string private constant _name = "Dilitium";
    string private constant _symbol = "DILTH";
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensToAddLiquidityWith = contractTokenBalance.mul(_liquidity).div(_liquidityFee);
        ...
        uint256 BNBToAddLiquidityWith = deltaBalance.mul(_liquidity).div(_liquidityFee);
        ...
        uint256 developerFee = remainingBalance.mul(_developer).div(_liquidityFee);
        uint256 ownersFee = remainingBalance.mul(_owners).div(_liquidityFee);
        uint256 marketingFee = remainingBalance.mul(_marketing).div(_liquidityFee);
        uint256 charityFee = remainingBalance.mul(_charity).div(_liquidityFee);
        uint256 bountyFee = remainingBalance.mul(_bounty).div(_liquidityFee);
        ...
        //get bounty as ANB
        _tokenTransfer(address(this),_bountyAddress,bountyFee,false);
    ...
    */
    function _setFees(uint256 liq, uint256 refl) external onlyOwner() {
        require((liq + refl) <= 400, "Fee is too high!");
        _liquidityFee = liq;
        _reflectionFee = refl;
