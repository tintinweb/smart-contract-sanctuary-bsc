...
contract VoteContract is Ownable, Pausable, ReentrancyGuard {
    ...
    event onVoteOptionAdded(uint8 _optionId, string _description, uint256 _timestamp);
    ...
    // Add vote option
    function addVoteOption(string memory _description) public onlyOwner() whenPaused() {

        // There cannot be any votes cast already
        require(voteToken.balanceOf(address(this)) == 0, "VoteToken: TOKENS_IN_CONTRACT");

        // Add another option to the count
        totalOptions++;

        // Record the option description
        _options[totalOptions] = _description;

        // Tell the network
        emit onVoteOptionAdded(totalOptions, _description, block.timestamp);
    }

    // Start the vote
    function startVote(uint256 _voteStartTime, uint256 _voteDuration) public onlyOwner() whenPaused() ifLockedEquals(false) {
        require(totalOptions > 0, "VoteContract: NO_OPTIONS_SET");
        ...
        voteFinishTime = (block.timestamp + voteDuration);
    ...
    // Conclude the vote
    function finishVote() public onlyOwner() whenNotPaused() ifLockedEquals(true) {
    ...
    function recoverTokens(bool _sendToPool) public onlyOwner() whenPaused() {
        ...
        if (_sendToPool) {
            ...
            stakingPool.notifyRewardAmount(tokenAmount);
        } else {
    ...
    // Set the staking pool for vote earning
    function setPool(address _pool) public onlyOwner() {
        stakingPool = StakingRewards(_pool);
    }
...
// StakingRewards.sol
// Users stake xSH33P in this contract (5% fee in and out), which they can earn vSH33P from.
contract StakingRewards is ReentrancyGuard, Pausable, Whitelist {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    function setRewardsDuration(uint256 _rewardsDuration) external onlyOwner {
        require(block.timestamp > periodFinish, "STAKING_ALREADY_ACTIVE");
