...
contract OlympusMigrator is Auth {
	...
	mapping (address => uint112) public reserve0Deposit;
	mapping (address => uint112) public reserve1Deposit;
	...
	function deposit(uint256 amount) external {
		...
		if (reserve0Deposit[msg.sender] > 0) {
			(reserve0Deposit[msg.sender] + reserve0) / 2;
		} else {
			reserve0Deposit[msg.sender] = reserve0;
		}
		if (reserve1Deposit[msg.sender] > 0) {
			(reserve1Deposit[msg.sender] + reserve1) / 2;
		} else {
			reserve1Deposit[msg.sender] = reserve1;
		}
	...
	function getReservesOnDeposit(address a) external view returns (uint112 reserve0, uint112 reserve1) {
		return (reserve0Deposit[a], reserve1Deposit[a]);
	}
}