...
contract Marketing is Ownable, Initializable {
    event InitializedOn( 
        address account, 
        uint alloc, 
        uint time
    ...
    function initialize(address[2] memory addresses, uint alloc, uint subsequentClaims, uint timestamp) external initializer {
        require(timestamp > 0, "Distributor : timestamp > 0");
        require(alloc > 0,"Distributor : alloc > 0");
        require(subsequentClaims > 0,"Distributor : subsequentClaims > 0");

        
        token = IBEP20(addresses[0]);
        allocation = alloc;
        claimPeriod = timestamp;
        subsequentClaim = subsequentClaims;

        IBEP20(addresses[0]).transferFrom(msg.sender, address(this), alloc);
        user[addresses[1]] = userStruct(alloc,0,block.timestamp,block.timestamp);
        total[0] += alloc;
