...
Ownable
{
  ...
  function buyNFT1(uint256 _amount, uint256 _coin) payable public {
    ...
    nft1.mint(address(msg.sender), _amount);
    emit Buy(address(msg.sender), _amount);
  }

  function setOpenBox(uint _flag, uint256 _amount1, uint256 _p1, uint256 _amount2, uint256 _p2, uint256 _amount3, uint256 _p3, uint256 _amount4, uint256 _p4, uint256 _p5, uint256 _p6) public onlyOwner {
    ...
    openCfgMap[_flag].push(_amount1);
    openCfgMap[_flag].push(_p1);
    openCfgMap[_flag].push(_amount2);
    openCfgMap[_flag].push(_p2);
    openCfgMap[_flag].push(_amount3);
    openCfgMap[_flag].push(_p3);
    openCfgMap[_flag].push(_amount4);
    openCfgMap[_flag].push(_p4);
    openCfgMap[_flag].push(_p5);
    openCfgMap[_flag].push(_p6);
  }

  function setNfts(uint256 _p1, uint256 _p2, uint256 _p3, uint256 _p4, uint256 _p5, uint256 _p6, uint256 _p7, uint256 _p8, uint256 _p9) public onlyOwner {
    nftsCfg = [_p1, _p2, _p3, _p4, _p5, _p6, _p7, _p8, _p9];
  }

  function openBox(uint256 _flag) public payable {
    ...
    require(cfg.length == 10, "openBox: open not set");
    if (p < cfg[1]) {
      //0.1 three
      IERC20(msg.sender).transfer(address(this), cfg[0]);
    } else if (p >= cfg[1] && p < cfg[3]) {
      //0.5 three
      IERC20(msg.sender).transfer(address(this), cfg[2]);
    } else if (p >= cfg[3] && p < cfg[5]) {
      //1 three
      IERC20(msg.sender).transfer(address(this), cfg[4]);
    } else if (p >= cfg[5] && p < cfg[7]) {
      //2 three
      IERC20(msg.sender).transfer(address(this), cfg[6]);
    } else if (p >= cfg[7] && p < cfg[8]) {
      //segment
      require(nftsCfg.length > 0, "openBox: nfts not set");
      uint256 p2 = uint256(keccak256(abi.encodePacked(msg.sender, _flag, block.timestamp, block.number))) % 100;
      if (p2 < nftsCfg[1]) {
        nfts.mint(address(msg.sender), 0);
      } else if (p2 >= nftsCfg[1] && p2 < nftsCfg[2]) {
        nfts.mint(address(msg.sender), 1);
      } else if (p2 >= nftsCfg[2] && p2 < nftsCfg[3]) {
        nfts.mint(address(msg.sender), 2);
      } else if (p2 >= nftsCfg[3] && p2 < nftsCfg[4]) {
        nfts.mint(address(msg.sender), 3);
      } else if (p2 >= nftsCfg[4] && p2 < nftsCfg[5]) {
        nfts.mint(address(msg.sender), 4);
      } else if (p2 >= nftsCfg[5] && p2 < nftsCfg[6]) {
        nfts.mint(address(msg.sender), 5);
      } else if (p2 >= nftsCfg[6] && p2 < nftsCfg[7]) {
        nfts.mint(address(msg.sender), 6);
      } else if (p2 >= nftsCfg[7] && p2 < nftsCfg[8]) {
        nfts.mint(address(msg.sender), 7);
      } else if (p2 >= nftsCfg[8] && p2 < nftsCfg[9]) {
        nfts.mint(address(msg.sender), 8);
      }
    } else if (p >= cfg[8] && p < cfg[9]) {
      //nftx
  ...
  function compound() payable public {
    ...
    if (allBurned) {
      uint256 _type = uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, idArr[1], idArr[3], idArr[5], idArr[7]))) % 300;
