...
contract BigBaby is ERC20, Ownable {
    ...
    uint256 public totalBuyFee = buyRewardFee + buyMarketingFee;
    uint256 public totalSellFee= sellRewardFee + sellMarketingFee;

    uint256 public rewardShare = 50;
    uint256 public marketingShare = 25;
    uint256 public buyBackShare = 25;
    ...
    event FeesBuyUpdated(uint256 buyRewardFee, uint256 buyMarketingFee);
    event FeesSellUpdated(uint256 sellRewardFee, uint256 sellMarketingFee);
    ...
    constructor() payable ERC20("BigBaby", "BBY") {
        require(totalBuyFee <= 25 && totalSellFee <= 25, "Fees must be less than 25%");
        require(maxTransactionRateBuy >= 1 && maxTransactionRateSell >= 1, "Max transfer rates must be greater than 0.1%");

    	dividendTracker = new DividendTracker(10**18 , rewardToken);//minimum dist 1 BUSD
        ...
        swapTokensAtAmount = 1_000_000_000 * (10 ** 18) / 5000;
    ...
    function updateBuyFees(uint256 _buyRewardFee,uint256 _buyMarketingFee) external onlyOwner {
        require(_buyRewardFee + _buyMarketingFee <= 20, "Fees must be less than 20%");
        buyRewardFee = _buyRewardFee;
        buyMarketingFee = _buyMarketingFee;
        totalBuyFee = buyRewardFee + buyMarketingFee;
        emit FeesBuyUpdated(buyRewardFee, buyMarketingFee);
    }

    function updateSellFees(uint256 _sellRewardFee,uint256 _sellMarketingFee) external onlyOwner {
        require(_sellRewardFee + _sellMarketingFee <= 20, "Fees must be less than 20%");
        sellRewardFee = _sellRewardFee;
        sellMarketingFee = _sellMarketingFee;
        totalSellFee = sellRewardFee + sellMarketingFee;
        emit FeesSellUpdated(sellRewardFee, sellMarketingFee);
    }
    ...
    function updateShare(uint256 _rewardShare, uint256 _marketingShare, uint256 _buyBackShare) external onlyOwner{
        require(_rewardShare + _marketingShare + _buyBackShare == 100,"Sum Of All Share Values should be 100");
    ...
    function manualBuyBack(uint256 amount) external onlyOwner {      
        buyBack(amount * 10**15); // i.e. "1" would equal 0.001 BNB
    } 
