...
interface IBird {
    function balanceBird(address _user) external view returns(uint256);
...
contract BugCoin is ERC20, ERC20Burnable,ReentrancyGuard, Ownable {
    ...
    function setBird(address _address) external onlyOwner {
        bird = IBird(_address);
    ...
    function removeCouncillor(address _councillor) public onlyOwner {
        ...
        // updated_amount = (balance(user) * rewardAmount * delta / 86400) + amount * initial rate
    function updateRewardOnMint(address _user, uint256 _amount) external {
        ...
        uint256 balanceBird = bird.balanceBird(_user);

        if(balanceBird >= mintAmount){
            balanceBird = mintAmount;
        }

        //update reward count is this is not their first mint
        if (timerUser > 0) {
            rewards[_user] = rewards[_user] + (balanceBird * rewardAmount * ((time - timerUser) / 86400));
    ...
    // called on transfers
    function updateBouns(address _from, address _to, uint256 _tokenId) external {
        require(councillors[msg.sender], "Unauthorized");
        require(_tokenId <= totalSupply() , "Max supply reached");

        rewards[_from] -= MINT_BONUS;
        rewards[_to] += MINT_BONUS;   
    }
    ...
    function updateReward(address _from, address _to, uint256 _tokenId) external {
        ...
        uint256 fromBalanceBird = bird.balanceBird(_from);
        uint256 toBalanceBird = bird.balanceBird(_to);

            if(fromBalanceBird >= mintAmount){
            fromBalanceBird = mintAmount;
            }

            if(toBalanceBird >= mintAmount){
            toBalanceBird = mintAmount;
            }

            if (timerFrom > 0) {
                rewards[_from] +=  fromBalanceBird * rewardAmount * ((time - timerFrom) / 86400);
            ...
            if (timerTo > 0) {
                rewards[_to] +=   toBalanceBird * rewardAmount * ((time - timerTo) / 86400);
  ...
  function burnFrom(address account, uint256 amount) public override {
    ...
    function getTotalClaimable(address _user) external view returns(uint256) {
        ...
        uint256 pending = balanceBird* rewardAmount * ((time - lastUpdate[_user]) / 86400);
        return rewards[_user] + pending;
    }

    
    function getTotalBounse(address _user) external view returns(uint256) {
