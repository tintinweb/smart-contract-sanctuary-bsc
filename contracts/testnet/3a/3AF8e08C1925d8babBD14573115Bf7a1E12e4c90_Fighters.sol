    ...
    ERC721EnumerableUpgradeable
{
    ...
    uint8 public constant MAX_USAGE        = 3;
    ...
    function mint(address _to) external override onlyAdmin {
        ...
        fighters[tokenId].useCounter = MAX_USAGE;
    ...
    function _beforeTokenTransfer(
        ...
        if (to != address(0) && from != to) {
            // Each user can only held 2 NFTs
            require(balanceOf(to) < 2, "Only 2 NFT per user");

            // Only transferred after consume all membership
            if (from != address(0)) {
                require(
