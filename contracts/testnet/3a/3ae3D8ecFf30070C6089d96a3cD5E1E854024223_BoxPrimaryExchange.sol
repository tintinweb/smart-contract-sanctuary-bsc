...
import "./IBoxPrimaryExchange.sol";

// 盲盒代币公售、白名单售卖
contract BoxPrimaryExchange is IBoxPrimaryExchange, Caller, Fallback {
    ...
    EnumerableSet.AddressSet private orderListToken;
    mapping(address => OrderInfo) private orderList;
        ...
        returns (OrderInfo memory)
    {
        ...
        info.stockAmount = IERC20(token).balanceOf(config.getMintBoxNFT());
        return info;
    }

    function getOrderLengthList() external view override returns (uint256) {
        return orderListToken.lengthAddressSet();
    }

    function getOrderTokenList(uint256 index)
        external
        view
        override
        returns (address)
    {
        return orderListToken.atAddressSet(index);
    }

    function getOrderList(address token)
        external
        view
        override
        returns (OrderInfo memory)
    {
        OrderInfo memory info = orderList[token];
        info.stockAmount = IERC20(token).balanceOf(config.getMintBoxNFT());
        return info;
    }
    ...
    function make(
        ...
        emit MakeEvent(
            0, //白名单业务
            token, //盲盒代币合约
            maxTakeAmount, //单地址购买上线数量
            IERC20(token).balanceOf(config.getMintBoxNFT()), //盲盒代币总库存数量
            totalAmount, //盲盒代币总售卖数量
            quoteToken, //报价代币合约
            quotePrice, //报价代币价格
            startBlock, //生效开始区块
            stopBlock //生效结束区块
    ...
    // 盲盒代币普通购买
    function take(address token, uint256 amount) external payable override {
        ...
        require(
            0 < amount &&
                amount <= IERC20(token).balanceOf(config.getMintBoxNFT()),
            "1129"
        );
        ...
        TransferV1.transferFromFlush(
            ...
            amount.mul(orderInfo.quotePrice, "1178"),
            "1113",
            "1114"
        );
        ...
        emit TakeEvent(
            ...
            amount,
            orderInfo.quoteToken,
            amount.mul(orderInfo.quotePrice, "1163"),
            msg.sender,
            block.timestamp
        );
    }
    ...
    //     uint256 stopBlock //生效结束区块
    // ) external override {
    //     require(startBlock < stopBlock && block.number < stopBlock, "1094");
    //     orderListToken.addAddressSet(token);
    //     orderList[token] = OrderInfo(
    //         list,
    //         token,
    //         maxTakeAmount,
    //         0,
    ...
    // // 盲盒代币白名单购买
    // function takeList(address token, uint256 amount) external payable override {
    //     OrderInfo storage orderInfo = orderList[token];
    //     require(
    //         orderInfo.startBlock < block.number &&
    //             block.number < orderInfo.stopBlock,
    //         "1189"
    ...
    //     );
    //     require(
    //         config.getList().getStateV2(orderInfo.list, msg.sender),
    //         "1096"
    //     );
    //     IApproveProxy approveProxy = config.getApproveProxy();
    //     TransferV1.transferFromFlush(
    ...
    //         msg.sender,
    //         config.getBoxSaleFee(),
    //         amount.mul(orderInfo.quotePrice, "1118"),
    //         "1117",
    //         "1116"
    //     );
    //     approveProxy.transferFromERC20(
    ...
    //         amount,
    //         orderInfo.quoteToken,
    //         amount.mul(orderInfo.quotePrice, "1167"),
    //         msg.sender,
    //         block.timestamp
    //     );
    // }
    ...
    // // 盲盒代币白名单取消售卖
    // function cancelList(address token) external override onlyCaller {
    //     orderListToken.removeAddressSet(token);
    //     delete order[token];
    // }
}
...
// 盲盒公售
interface IBoxPrimaryExchange {
    event MakeEvent(
        uint16 list, //白名单业务
        address indexed token, //盲盒代币合约
        uint256 maxTakeAmount, //单地址购买上线数量
        uint256 indexed stockAmount, //盲盒代币总库存数量
        uint256 totalAmount, //盲盒代币总售卖数量
    ...
    event TakeEvent(
        uint16 list, //白名单业务
        address indexed token, //盲盒代币合约地址
    ...
    struct OrderInfo {
        uint16 list; //白名单业务
        address token; //盲盒代币合约地址
        uint256 maxTakeAmount; //单地址购买上限数量
        uint256 stockAmount; //盲盒代币总库存数量
        uint256 totalAmount; //盲盒代币总售卖数量
    ...
    function getOrderLengthList() external view returns (uint256);

    function getOrderTokenList(uint256 index) external view returns (address);
