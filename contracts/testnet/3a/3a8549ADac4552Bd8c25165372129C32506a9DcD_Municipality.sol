...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _lastPurchaseDateUpdate(address _user) private {
        ...
        if (IERC721Base(businessParcelNFTAddress).getUserPurchaseTime(_user)[0] > _lastDate) {
            _lastDate = IERC721Base(businessParcelNFTAddress).getUserPurchaseTime(_user)[0];
        } 
