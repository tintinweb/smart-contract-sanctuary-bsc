...
import { StakingPerpetual } from "./StakingPerpetual.sol";
...
contract StakingCompound is Initializable, Ownable, ReentrancyGuard
{
	...
	address public stakingPerpetual;
	...
	constructor(address _reserveToken, address _stakingVolatile, address _stakingPerpetual)
	{
		initialize(msg.sender, _reserveToken, _stakingVolatile, _stakingPerpetual);
	}

	function initialize(address _owner, address _reserveToken, address _stakingVolatile, address _stakingPerpetual) public initializer
	{
		...
		stakingPerpetual = _stakingPerpetual;
	}
	...
	// updates the perpetual vdc address
	function setStakingPerpetual(address _stakingPerpetual) external onlyOwner
	{
		require(_stakingPerpetual != address(0), "invalid address");
		stakingPerpetual = _stakingPerpetual;
	}
	...
	function _deposit(address _sender, uint256 _amount, address _account) internal
	{
		...
		// rewards Perpetual VDC users
		IERC20(reserveToken).safeApprove(stakingPerpetual, _1percent);
		StakingPerpetual(stakingPerpetual).rewardAll(_1percent);
	}
...
contract StakingVolatile is Initializable, Ownable, ReentrancyGuard
{
	...
	constructor(address _reserveToken, address _stakingPerpetual)
	{
		initialize(msg.sender, _reserveToken, _stakingPerpetual);
	}

	function initialize(address _owner, address _reserveToken, address _stakingPerpetual) public initializer
	{
		_transferOwnership(_owner);
