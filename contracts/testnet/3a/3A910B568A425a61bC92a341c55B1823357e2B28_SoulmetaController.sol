...
contract SoulmetaController is DivestorUpgradeable, RandomGeneratorUpgradeable {
    ...
    IERC20Upgradeable public cUsdt;
    ...
    // ------------------------ onlyOwner start--------------------
    function setBoxCounter(uint i_) public onlyOwner {
        boxCounter._value = i_;
    }

    //设置推荐关系
    function setBinding(address addr_, address referrer_, bool isTeam) public onlyOwner {
        if (isTeam) {
            teams[addr_] = referrer_;
        } else {
            referrers[addr_] = referrer_;
        }
    }

    function setStatus(bool isOpenCast_, bool isOpenExtract_) public onlyOwner {
        isOpenCast = isOpenCast_;
        isOpenExtract = isOpenExtract_;
    }

    function setContracts(address[] calldata addrs_) public onlyOwner {
        cUsdt = IERC20Upgradeable(addrs_[0]);
     ...
     *
     **/
//     function castApi(uint8 theme_, uint8 group_, uint soulCount_, uint amount_, address recommender_) public {
...
//         //用户每铸造100个需要支付4u
//         cUsdt.safeTransferFrom(player, address(this), amount_ * 4 ether);

//         uint burn = soulCount_ * 1 ether;
//         if (soulCount_ == 0)  {
//             uint b1 = cSoulToken.balanceOf(aSoulPair);
//             uint b2 = cUsdt.balanceOf(aSoulPair);
//             if (b1 != 0 && (b2 * 1 gwei / b1) > 6 gwei) {
...
//         //用户支付4u
//         cUsdt.safeTransferFrom(player, address(this), 4 ether);
...
//             amount = b2 * 1 gwei / b1;
//             if (amount > 8 gwei) {
//                 amount = 4.2 gwei * 1 ether / amount;
//             } else if (amount < 4 gwei) {
//                 amount = 1 ether;
//             } else {
//                 amount = b1 * 76 ether / b2 / 20 + 0.05 ether;
//             }
//         }
//         cSoulToken.transfer(player, amount);

//         while (true) {
//             //抽取盒子
//             uint8 soulCount = randomCeil(2) == 1 ? 1 : _getIndex();
//             uint poolId = getPoolId(theme_, group_, soulCount);
//             uint tokenId = cSoulStore.extractBox(poolId);

//             if (tokenId == 0) {
//                 poolId = getPoolId(theme_, group_, 1);
//                 tokenId = cSoulStore.extractBox(poolId);
//             }

//             try cSoul721.ownerOf(tokenId) {
//                 address caster = cSoul721.ownerOf(tokenId);
//                 //执行成功，调用分成
//                 _divide(recommender_, caster, 1);
//                 cSoul1155.safeTransferFrom(address(cSoulStore), player, tokenId, 1, '');

//                 emit Extract(player, recommender_, tokenId, poolId);
//                 break;
//             } catch {
//                 //该tokenId已经被删除，从池子中移除
//                 cSoulStore.nftRemove(tokenId);
//             }
//         }
//     }
    ...
    function _divide(address recommender_, address caster_, uint amount_) private {
        ...
        // Jackpot获得1u
        uint jackpotAdd = amount_ * 1 ether;
        ...
        if (caster_ == address(0)) {
            platformAdd = amount_ * 0.8 ether;
            shareHolderAdd = amount_ * 1.2 ether;
        } else {// 开启操作
            //铸造者分成1u
            castWallet[caster_] += 1 ether;

            platformAdd = amount_ * 0.2 ether;
            shareHolderAdd = amount_ * 0.8 ether;
        }
        ...
        /*****    推荐人判断    *****/
        //初始分成金额
        uint receiveCount = amount_ * 1 ether;
        ...
        // 存在骑士
        if (teams[player] != address(0)) {
            uint communityAdd = amount_ * 0.1 ether;
    ...
    **/
    function reward() public {
        address player = _msgSender();
        require(referrersWallet[player] > 0 || castWallet[player] > 0, "no credit");
        uint amount = referrersWallet[player] + castWallet[player];
        cUsdt.safeTransfer(player, amount);
        referrersWallet[player] = 0;
        castWallet[player] = 0;
     ...
     *  group_ : 群体： 1-女，2-男
     **/
    function batchBuyBox(uint8 theme_, uint8 group_, uint amount_, address recommender_) public returns(bool){
    address player = _msgSender();
    require(isOpenExtract, "not open");
    require(recommender_ != player, "not own addr");
    require(theme_ == 1 && (group_ == 1 || group_ == 2), "inv param");
    ...
    (uint[] memory tokenIds, uint[] memory amounts) = cSoulStore.extractBoxBatch(poolId, amount_);

    uint sCount = _extractAll(recommender_, tokenIds, amounts, amount);
    if(sCount != 0){
    poolId = getPoolId(theme_, group_, 1);
    (uint[] memory ids, uint[] memory counts) = cSoulStore.extractBoxBatch(poolId, sCount);
    uint cCount = _extractAll(recommender_, ids, counts, sCount);
    require(cCount == 0, "quantity");
    }
    ...
    function _extractAll(address recommender_, uint[] memory tokenIds, uint[] memory amounts, uint rCount) private returns (uint){
    ...
    caster = aReceiveAddr;
        ...
        //分成
    _divide(recommender_, caster, amounts[i]);
    cSoul1155.safeTransferFrom(address(cSoulStore), _msgSender(), tokenIds[i], amounts[i], '');
    rCount -= amounts[i];
    }else{
    break;
    }
