...
contract OptVault is AccessControl, Initializable {
    ...
    // address public bYSLVault; // bYSL Vault address
    address public masterChef; //Apeswap masterChef address
    // address public BUSD; // address of BUSD
    IAdmin public  Admin; 
    // address public factoryOptVault; //address of Factory optimization vault
    address public teamAddress; // address of team address wallet
    // address public treasuryAddress; // treasury contract address
    // address public Admin.YSL(); // address of Admin.YSL() token
    // address public xYSL; // address of xYSL token
    // address public USDy; // address of USDy token
    // address public bYSL; // address of bYSL token
    // address public Bshare; // address of Bshare token address
    // address public temporaryHolding; // temporaryholding contract address
    // address public YSLVault; // YSLVault address
    // address public xYSLVault; //xYSLVault address
    // address public Admin.vaultSwap();
    ...
    uint totalStaked;
    uint S;
    uint[] _setOptimizationTaxFEE = new  uint[](3);
    ...
    function deposit(address user,uint amount, bool isReceipt) external {
        ...
        if(isReceipt){
            ...
            IPancakeMaster(masterChef).withdraw(id, amount);
            console.log("balance before deposit", IPancakeMaster(masterChef).userInfo(id, address(this)).amount);
        ...
        path[1] = Admin.BUSD();
        token.approve(router, ((amount*25)/100));
        uint256 convertedBUSD = IUniswapV2Router02(router).swapExactTokensForTokens(((amount*25)/100), 0, path, address(this), block.timestamp+1000)[1];
        path[0] = Admin.BUSD();
        path[1] = Admin.USDy();
        ...
        if(IERC20(recieptToken).totalSupply() == 0){
            mintRecieptToken = (amount * 50)/exchangeRatio;
        }
        else{
            exchangeRatio = (IPancakeMaster(masterChef).userInfo(id, user).amount * 100)/IERC20(recieptToken).totalSupply();
            mintRecieptToken = ((amount * 50))/exchangeRatio;
        }
        ...
        uint protocolPriceBYSL = IBYSL(Admin.bYSL()).protocolPrice();
        byslPrice = protocolPriceBYSL * 100;
        uint mintedBYSL = (amount * 50)/ byslPrice;
        INTT(Admin.BShare()).mint(user, mintedBYSL);
        IERC20(token).approve(masterChef,(amount *75)/100);

        IPancakeMaster(masterChef).deposit(id, (amount *75)/100);
        ...
        stakedAmount[user] += (amount *75)/100;
        totalStaked += (amount *75)/100;
        share[user] = S;
    }
    ...
    function withdraw(address user, bool isReciept, uint _amount) external{
        ...
        console.log("user balance",UserReciept[user]);
        ...
        if(msg.sender == Admin.vaultSwap()){
        require(UserReciept[user] > 0,"OptVault: You need to first deposit");
        console.log("user balance",UserReciept[user]);
        require(_amount <= UserReciept[user],"OptVault: Invalid Amount");
        (,uint balance) = rewardState(user, _amount);
        if(isReciept){
            IERC20(recieptToken).safeTransfer(Admin.vaultSwap(),_amount);
        }else{
            IPancakeMaster(masterChef).withdraw(id, balance);
            IERC20(token).safeTransfer(Admin.vaultSwap(), balance);
        }
        INTT(recieptToken).burn(address(this), _amount);
        UserReciept[user] -= _amount;
            }
    ...
    function purchase(address user, uint amount, uint minAmount) external returns(uint){
        ...
        // console.log("exchangeRatio in purchase",exchangeRatio);
    ...
    function sell(address user, uint amount, uint minAmount) external returns(uint){
        // console.log("amount of user",IERC20(recieptToken).balanceOf(user));
        // console.log("amount of contract",IERC20(recieptToken).balanceOf(address(this)));
        // console.log("address of opt vault contract",address(this));
        // console.log("user",user);
        // console.log("recieptToken",recieptToken);
        // console.log(IERC20(recieptToken).allowance(user,address(this)));
        // IERC20(recieptToken).approve(address(this),amount);
    ...
    function collectOptimizationTax() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        // console.log("convertedYSL",convertedYSL);
        path[1] = Admin.xYSL();
    ...
    function optimizationReward(address user, uint optMultiplier) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        console.log(IUSDY(Admin.USDy()).allowance(address(Admin.USDy()), address(this)));
        console.log(IUSDY(Admin.USDy()).balanceOf(address(this)));
    ...
    function setOptimizationTaxFEE(uint[] calldata getOptimizationTaxFEE) external onlyRole(SETTER_ROLE){
        ...
        // uint totalAmount;
        for(uint i=0;i<3;i++){
            _setOptimizationTaxFEE[i]=getOptimizationTaxFEE[i];
            _setOptimizationTaxFEE[i];
        }
    }
    // function setTemporaryHoldingContract(address _temporaryHolding) external onlyRole(SETTER_ROLE){
    ...
    // function setTreasuryAddress(address _treasuryAddress) external onlyRole(SETTER_ROLE)  {
    //     require(_treasuryAddress != address(0), 'OptVault: treasury address can not null');
    //     treasuryAddress = _treasuryAddress;
    // }

    // function setUSDyAddress( address _USDy) external onlyRole(SETTER_ROLE)  {
    //     require(_USDy != address(0), 'OptVault: USDy address can not null');
    //     USDy = _USDy;

    // }

    // function setbYSLAddress( address _bYSL, address _BShare) external onlyRole(SETTER_ROLE) {
    //     require(_bYSL != address(0), 'OptVault: bYSL address can not null');
    //     bYSL = _bYSL;
    //     Bshare = _BShare;
    // }
    ...
    // function setYSL(address _YSL) external onlyRole(SETTER_ROLE){
    //     require(_YSL != address(0));
    //     Admin.YSL() = _YSL;
    // }

    // function setYSLAndxYSLVault(address _YSLVault, address _xYSLVault) external onlyRole(SETTER_ROLE) {
    //     require(_YSLVault != address(0), 'OptVault: YSLVault address can not null');
    //     require(_xYSLVault != address(0), 'OptVault: xYSLVault address can not null');
    //     YSLVault = _YSLVault;
...
interface IOptVaultFactory{
    ...
    enum LiqStatus {
     ...
     function add( IERC20 _token, address _strat,string memory _name,string memory _symbol) external;
     function Deposit(address user,address _token,uint256 _amount,bool isReceipt) external payable;
