...
contract LukyLeprechaun is Context, IERC20, Ownable {
    ...
    uint8 public miniJackpotWinners = 12;
    ...
    function getMiniJackpot() public view returns(address[] memory, address[] memory) {
        return (lastBuyers,lastWinners);
    }
    ...
    function awardJackpot() private lockTheSwap {
        ...
        uint256 miniJackpot = bnbEquivalent(miniJackPotAmount * miniJackpotWinners);
        if(_pendingJackpotBalance >= miniJackpot){
            for(uint8 i = 0; i < miniJackpotWinners; i++){
                payable(lastBuyers[i]).transfer(miniJackpot.div(miniJackpotWinners));
    ...
    function setBuyer(address user) private {
        ...
        if(lastBuyers[0] == address(0)){
            lastBuyers[0] = user;
            return;
        }

        for(uint8 i = 0; i < miniJackpotWinners; i++){
            if(lastBuyers[i] == user){
                return;
            }else{
                lastBuyers[i] = lastBuyers[i+1];
            }

            if(lastBuyers[i+1] == address(0)){
                lastBuyers[i+1] = user;
                return;
            }
        }

        lastBuyers[miniJackpotWinners] = user;
    }
