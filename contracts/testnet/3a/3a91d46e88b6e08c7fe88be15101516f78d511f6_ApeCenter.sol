...
contract ApeCenter is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ApeCenterDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ApeCenter", "APC") {
        marketingWallet = payable(0x79010D7638c018d726d8304A4942B0f907D4402a);
        devWallet = payable(0x44C562c7Cd82f583090527380BF3726a75dCC1F7);
        address router = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; // testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 
        ...
        dividendTracker = new ApeCenterDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56, // eth(bsc): 0x2170Ed0880ac9A755fd29B2688956BD959F933F8
            "ApeCenterTRACKER",
            "APCTRACKER"
        );
...
contract ApeCenterDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    ApeCenter public ApeCenterContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ApeCenterContract = ApeCenter(mainContract);
    ...
    function _transfer(
        ...
        require(false, "ApeCenter_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "ApeCenter_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ApeCenter contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ApeCenterContract);

            uint256 prevBalance = ApeCenterContract.balanceOf(address(this));
            ...
            {
                uint256 received = ApeCenterContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ApeCenterContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
