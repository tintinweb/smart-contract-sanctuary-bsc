...
contract rigelScheduleAlarm is Context {
    ...
    function swapTokens(uint256 _id, address _user, bytes32 _hash, bytes32 r,  bytes32 vs) external {
        ...
        userData memory _userData = Data[_user][_id]; 
        address rUser = recover_r_vs(_userData.caller, _hash, r, vs);
    ...
    function recover_WithSignature(
        address caller,
        bytes32 hash,
        bytes32 signature
    ) public pure returns (address) {
        bytes32 getHash = hash.toEthSignedMessageHash();
        address recoveredAddress = getHash.recover(hash, signature);
        require(recoveredAddress != address(0) && recoveredAddress == caller, 'Rigels Exchange: INVALID_SIGNATURE');
        return recoveredAddress;
    }
