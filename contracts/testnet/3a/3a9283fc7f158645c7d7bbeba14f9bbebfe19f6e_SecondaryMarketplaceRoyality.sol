...
import "./interface/IMarketplaceWhitelist.sol";
import "@openzeppelin/contracts/token/common/ERC2981.sol";
contract SecondaryMarketplaceRoyality is Ownable, IERC1155Receiver {
     ...
     */
    function sell721(address _collection, uint _tokenId, uint _price, address _paymentToken) external {
    ...
    function buy721(uint _index) external payable {
        ...
        Sell storage sell = sellOrdersById[_index];
        require (sell.active, "SecondaryMarketplace: Not in sale");
     ...
     * @param _paymentToken Payment Token used (if 0x000... BNB will be used)
     */
    function sell1155(address _collection, uint _tokenId, uint _amount, uint _price, address _paymentToken) external {
    ...
    function buy1155(uint _index) external payable {
        ...
        if (sell.paymentToken == address(0)) {
            processPaymentBNB(sell.price, sell.owner);
        } else {
            processPayment(sell.paymentToken, sell.price, sell.owner);
        }
    ...
    // INTERNAL METHODS
    function processPaymentBNB(uint _price, address _seller) internal {
        require (msg.value >= _price, "SecondaryMarketplace: Not enough funds");

        // Fees
        uint platformFee = _price * marketplaceFee / 1000;
    ...
    function processPayment(address _token, uint _price, address _seller) internal {
        ...
        // // Fees
        token.safeTransfer(_seller, _price);
    }
