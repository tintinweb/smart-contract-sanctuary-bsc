...
contract MinerToken is Ownable {
    ...
    function deposit(uint256 option, uint256 amount) external {
        ...
        require(startMiner, "NO_OPEN");
        require(_products[option].dctAmount > 0 && amount >= 1, "PARAMETER_ERR");
        uint256 approveAmountDCT = IERC20(dct).allowance(sender, address(this));

        uint256 dctAmount = _products[option].dctAmount.mul(amount);

        require(approveAmountDCT >= dctAmount, "INSUFFICIENT_FUNDS");
        ...
        totalComputPower += _products[option].power.mul(amount);
        _user[sender] = block.timestamp;
        _balance[sender] = _balance[sender].add(_products[option].power.mul(amount));
    ...
    function withdrawLinearRelease() public {
        ...
        require(dctBalance > totalBurnDCTAmount, "INSUFFICIENT_FUNDS");
        dctBalance = dctBalance.sub(totalBurnDCTAmount);
        require(totalComputPower >= minRewardPower, "NO_OPEN_MINER"); // 最小算力限制
        require(block.timestamp > _user[sender], "ALREADY_RECEIVED");
        ...
        require(rewardAmount > 0, "INSUFFICIENT_FUNDS");
    ...
    // 开启挖矿
    function setStartMiner(bool start_) external onlyOwner {
        startMiner = start_;
    }
    ...
    function setAddProduct(uint256 option, uint256 dctAmount, uint256 power) external onlyOwner {
        require(_products[option].dctAmount == 0, "PRODUCT_EXIST");
    ...
    function withdrawDCT() external onlyOwner {
        ...
        require(balance > totalBurnDCTAmount, "INSUFFICIENT_FUNDS");
