...
/**
 * #In-Dex-Sale($IDSale) Token
 *
 * Features:
 * Total supply: 1,000,000,000
 * 5% rate automatically added to the liquidity pool to be blocked forever on sale;
 * 5% automatic distribution fee for all holders;
 * 3% fee automatically distributed to NFT Wallet on buy;
 * 5% fee automatically distributed to NFT Wallet on sell;
 * The rest of the accumulated BNBs will be sent to NFT Wallet
 *
 * 20% of the supply is burned at the start;
 * 5% of the supply will go to developers;
 * 1.5% of the supply will go to MARKET Wallet;
 * 2% of the supply will go to a donation portfolio
 *
 * NFT Wallet   : 0xe43A226fC507d5A084571177cDB73FD45A9937c6
 * MARKET Wallet: 0x6db52E2F2EFe5E6e0c0AE402Bc756c41D6a8Ee8F
 * 
 * Website official: https://in-dex-sale.ml
 * Telegram group  : https://t.me/in-dex-sale
 * Twitter profile : https://twitter.com/In-Dex-Sale
 */
 ...
 */
contract InDexSaleToken is Context, IBEP20, Ownable {
	...
	address payable public constant nftWallet = payable(0x6a9640227debF43C222DAA35Ca6049d45A25D640);
    address public constant marketingWallet   = 0x6a9640227debF43C222DAA35Ca6049d45A25D640;

    string private constant _name          = "InDexSale";
    string private constant _symbol        = "IDSale";
	...
	uint256 public buyNftFee   = 3; // 3% fee to distribute for NFT Wallet on buy
	uint256 public sellNftFee  = 5; // 5% fee to distribute for NFT Wallet on sell
    ...
    uint256 private _tNftTokenTotal;
	uint256 private _tNftBnbTotal;
    ...
    receive() external payable {
        if (msg.value > 0) {
			...
			_tNftBnbTotal = _tNftBnbTotal.add(msg.value);
    ...
    function totalNFTs() external view returns (uint256, uint256) {
        return (_tNftTokenTotal, _tNftBnbTotal);
    }
	...
	function reflectionFromToken(uint256 tAmount, bool deductTransferRewardFee) external view returns (uint256) {
        ...
        if (!deductTransferRewardFee) {
            ...
            _rv.rTransferAmount = 0; _rv.rRewardFee = 0; _rv.rLiquidityFee = 0; _rv.rNftFee = 0;
            _tv.tTransferAmount = 0; _tv.tRewardFee = 0; _tv.tLiquidityFee = 0; _tv.tNftFee = 0;
			return _rv.rAmount;
        } else {
            (rValues memory _rv, tValues memory _tv) = _getValues(tAmount);
            _rv.rAmount         = 0; _rv.rRewardFee = 0; _rv.rLiquidityFee = 0; _rv.rNftFee = 0;
            _tv.tTransferAmount = 0; _tv.tRewardFee = 0; _tv.tLiquidityFee = 0; _tv.tNftFee = 0;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (overMinTokenBalance && !_inSwapTokens &&
            ...
            // swap Tokens for BNB and transfer BNBs to NFT Wallet 
            _swapAndtransferBnbToNftWallet(contractTokenBalance);
        }
    ...
    function _swapAndtransferBnbToNftWallet(uint256 contractTokenBalance) private lockTheSwap {
	    // swap tokens for BNB
        _swapTokensForBnb(contractTokenBalance);

        // how much BNB did we just swap into?
        if (address(this).balance >= 100000000 gwei) { // Min 0.1 BNB
			// takes the BNBs and sends them to the NFT wallet
            _transferBnbNft(nftWallet, address(this).balance);
        }
    }
    ...
    function _transferBnbNft(address payable recipient, uint256 amount) private {
        recipient.transfer(amount);
		_tNftBnbTotal = _tNftBnbTotal.add(amount);
		emit BnbFromLP(recipient, amount);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        _transferNftWallet(sender, _rv.rNftFee, _tv.tNftFee);
	...
	function _removeAllFee() private {
        if (_rewardFee == 0 && _liquidityFee == 0 && _nftFee == 0) return;
	...
	function _getValues(uint256 tAmount) private view returns (rValues memory, tValues memory) {
        ...
        rValues memory _rv = _getRValues(tAmount, _tv.tRewardFee, _tv.tLiquidityFee, _tv.tNftFee, _getRate());
        return (_rv, _tv);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (tValues memory) {
        ...
        _tv.tTransferAmount = tAmount.sub(_tv.tRewardFee).sub(_tv.tLiquidityFee).sub(_tv.tNftFee);
        return _tv;
    }

    function _getRValues(uint256 tAmount, uint256 tRewardFee, uint256 tLiquidityFee, uint256 tNftFee, uint256 currentRate) private pure returns (rValues memory) {
        rValues memory _rv;
        ...
        _rv.rTransferAmount = _rv.rAmount.sub(_rv.rRewardFee).sub(_rv.rLiquidityFee).sub(_rv.rNftFee);
        return _rv;
    }
    ...
    function _transferNftWallet(address sender, uint256 rNftFee, uint256 tNftFee) private {
        if (tNftFee > 0) {
            ...
            _tNftTokenTotal = _tNftTokenTotal.add(tNftFee); 
            _tFeeTotal      = _tFeeTotal.add(tNftFee);
        }
    }
