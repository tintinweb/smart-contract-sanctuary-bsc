...
//FOOTBALL FLOKI LETS PLAY BALL ME AND PAPA ELON ARE READY TO PUT SOME KIDS IN SORTS LETS HAVE SOME FUN AND DO GOOD
// T.ME/FOOTBALLFLOKIOFFICIAL
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
 ...
 contract FOOTBALLFLOKI is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint8 private _decimals = 9;

    string private _name = "FOOTBALL FLOKI";                  
 ...
 //COLLECT TAX IN TOKENS WHEN SWAP LIQUIFY SET TO FALSE THIS IS FOR SLOW PERIODS AND COMMUNITY BURNS
    uint256 public defaultFundingFee = 0;
    uint256 public _fundingFee = defaultFundingFee;
    uint256 private _previousFundingFee  =_fundingFee;

    uint256 public  _fundingFee4Buyers = 8;
    uint256 private _previousFundingFee4Buyers = _fundingFee4Buyers;

    uint256 public _fundingFee4Sellers = 9;
    uint256 private _previousFundingFee4Sellers = _fundingFee4Sellers;

    uint256 public defaultBurnFee = 0;
    uint256 public _burnFee = defaultBurnFee;
    ...
    //Anti Bot and Anti Jeet AKA Whale
    mapping(address => uint256) private _holderLastTransferTimestamp; // to hold last Transfers temporarily during launch
    bool public transferDelayEnabled = true;


    // Responsible for collecting token fees when swap liquify auto sells are turned off we will collect these fees in tokens

    address  public CharityAddress = 0xc50CAfB1A3cb6730257472ec207003A4A5E82c0d; // Charity Wallet wallet address
    address  public MarketingAddress = 0x0525676D822D151B290b04429CaaEA97828D335D; // Marketing Wallet
    address  public Team1Address = 0x4D09bF6435Fd9bb12Afbeef25fA98cD2F18af474; //  Team1 Tax 
    address  public Team2Address = 0xb5F17fd56Fe9f17E2E1BE216410E232D3bC476d9; //  Team2 Tax 
    address  public Team3Address = 0x4D09bF6435Fd9bb12Afbeef25fA98cD2F18af474; //  Team13Tax 
    


    uint256[] public _liquidityFeeSplit = [10,90];                                       // % division between swap+liquify & wallets
    uint256 public numTokensSellToAddToLiquidity = _tTotal * 3 / 1000;                   // contract balance to trigger swap & liquify + wallet transfer.
    //address public pancakeRouterAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // Pancake  Version 2 address                         
    address public pancakeRouterAddress = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; // Pancake TEST Router Version 2 address
    uint[] public _percentages = [30,30,30,10,0,0,0,0];                                  // % of liquidity fee split to wallet(s)
    address payable[] public _wallets = [                                                // wallet(s) to receive liquidity fee split
        payable(0x4D09bF6435Fd9bb12Afbeef25fA98cD2F18af474),                             // Team 1
        payable(0x0525676D822D151B290b04429CaaEA97828D335D),                             // Marketing wallet
        payable(0xb5F17fd56Fe9f17E2E1BE216410E232D3bC476d9),                             // Team 2
        payable(0x1bE5643ED94166E44BcA4c179148809e13ce0149),                             // Charity wallet
        payable(0xc50CAfB1A3cb6730257472ec207003A4A5E82c0d)
    ...
    modifier lockTheSwap {
     ...
     constructor() IBEP20 () {
        ...
        _setAutomatedMarketMakerPair(address(uniswapV2Pair), true);
         ...
         maxTransactionAmount = _tTotal * 3 / 100;// 3% maxTransactionAmount
  ...
  function setLiquidityFee4Buyers(uint256 liquidityFee4Buyers) external onlyOwner() {
     ...
     function setDefaultBurnFee(uint256 burnFee) external onlyOwner() {
        require(burnFee <= 20, "Fee too high!");
        defaultBurnFee = burnFee;
    }
    ...
    function setDefaultFundingFee(uint256 fundingFee) external onlyOwner() {
        require(fundingFee <= 20, "Fee too high!");
        defaultFundingFee = fundingFee;
    }
   ...
   function excludeFromMaxWallet(address account) public onlyOwner {
    ...
    function updateMaxTxAmount(uint256 newNum) external onlyOwner {
     require(newNum >= (_tTotal * 5 /  1000)/1e18, "Cannot set maxTransactionAmount lower than 0.5%");
   ...
   function isExcludedFromMaxWallet(address account) public view returns(bool) {
    ...
    function setFees(address sender , address recipient) private {
        if (sender == uniswapV2Pair && swapAndLiquifyEnabled )  {
            _taxFee = _taxFee4Buyers;
            _liquidityFee = _liquidityFee4Buyers;
        } else if (recipient == uniswapV2Pair && swapAndLiquifyEnabled) {
            _taxFee = _taxFee4Sellers;
            _liquidityFee = _liquidityFee4Sellers;
        }else if (recipient == uniswapV2Pair && !swapAndLiquifyEnabled) {
            _taxFee = _taxFee4Sellers;
            _fundingFee = _fundingFee4Sellers;
        } else if (sender == uniswapV2Pair && !swapAndLiquifyEnabled)  {
            _taxFee = _taxFee4Buyers;
            _fundingFee = _fundingFee4Buyers;
            _liquidityFee = defaultLiquidityFee;

        } else {
            _taxFee = defaultTaxFee;
            _fundingFee = defaultFundingFee;
            _liquidityFee = defaultLiquidityFee;
        }
    }
  ...
  function calculateTaxFee(uint256 _amount) private view returns (uint256) {
    ...
    function setCharityAddressWallet(address newWallet) external onlyOwner() {
        CharityAddress = newWallet;
    }
    
    function setMarketingAddressWallet(address newWallet) external onlyOwner() {
        MarketingAddress = newWallet;
    }
    
    function setTeam1AddressWallet(address newWallet) external onlyOwner() {
        Team1Address = newWallet;
    }
    
    function setTeam2AddressWallet(address newWallet) external onlyOwner() {
        Team2Address = newWallet;
    }
    function setTeam3AddressWallet(address newWallet) external onlyOwner() {
        Team3Address = newWallet;
    }
  ...
  function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _fundingFee == 0) return;
  ...
  function restoreAllFee() private {
    ...
    function _goLive() external onlyOwner() {
        require(!isTokenLive, "SBF is already live");
        isTokenLive = true;
    }
    ...
    function _transfer(
        ...
        require(_isExcludedFromFees[from] || _isExcludedFromFees[to] || isTokenLive, "SBF is not live yet");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        uint256 burnAmt = (_burnFee);
        uint256 fundingAmt = 10;
        ...
        _transferStandard(sender, Team1Address, fundingPiece.mul(4)); // change to 4 
        _transferStandard(sender, Team2Address, fundingPiece.mul(4)); // Marketing Wallet         
        _transferStandard(sender, Team3Address, fundingPiece.mul(4)); // Marketing Wallet         
        _transferStandard(sender, MarketingAddress, fundingPiece.mul(4)); // Team Wallet 
