...
contract MarketPlace is Ownable {
    ...
    function getMyProposals(address _user) external view returns(OfferDatas[] memory myProposals){
        uint256 _length = _getMyProposalsActiveNumber(_user);
        uint256 _start = _myProposals[_user].length - 1;
        uint256 _index;

        for(uint256 i = _start ; i >= _start - _length ; ){
                myProposals[_index] = _offers[_myProposals[_user][i].offerId];
                unchecked{ -- i ;}
                unchecked{ ++ _index ;}
            }
        return myProposals;
