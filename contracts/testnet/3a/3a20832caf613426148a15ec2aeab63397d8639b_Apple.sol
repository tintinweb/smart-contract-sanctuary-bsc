...
contract Apple {
    ...
    // 高度 - 静态单位算力收益
    mapping (uint256 => HashInfo) private _staticUintProfitMap;
    // 高度 - 动态单位算力收益
    mapping (uint256 => HashInfo) private _dynamicUintProfitMap;
    // 领取记录
    mapping (address => TakeInfo[]) private _extractRecord;
    // 开挖时间
    uint256 private _startMineTime;
    // 是否产出
    bool private _mineSwitch;
    // 开始产出时的高度
    uint256 private _startMineHeight;
    // 全网总静态算力
    uint256 private _netTotalStaticHashRate;
    // 全网总动态算力
    uint256 private _netTotalDynamicHashRate;
    // 地址 - 上级地址
    ...
    // 地址在启动产出前最新的记录算力的高度
    mapping (address => uint256) private _addressLastRecordHeightBeforeStartMineMap;
    // 最后计算静态和动态单位算力收益的高度
    uint256 private _lastCalculateBlockHeight;
    bool private _hasRecord = false;
    // 购买空间的BUSD收款地址
    address private _spaceReceiveAddress;
    // 空间单价[BUSD]
    uint256 private _spacePrice;
    // 已销毁APPLE数量 - 购买空间
    uint256 private _buyDestroyedApple;
    // 已销毁APPLE数量 - 出局销毁
    uint256 private _outDestroyedApple;
    // 发布者
    address private _owner;
    // 总上级地址
    address private _totalParentAddress;
    // 地址购买空间上限
    uint256 private _buySpaceBuyMax = 5000;
    // 地址购买空间上限
    uint256 private _destroyAppleTotal = 10000;
    // 地址购买空间上限
    ...
    constructor (address appleContractAddress, uint256 appleContractDecimal, address busdContractAddress, uint256 busdContractDecimal) public {
        ...
        _destroyAppleTotal = _destroyAppleTotal.mul(uint256(10).power(_appleContractDecimal));
    ...
    function startMine() public onlyOwner {
        if (_mineSwitch) return;
        _mineSwitch = true;
        _startMineHeight = getBlockHeight();

        _staticUintProfitMap[_startMineHeight].hash = calculateProfitPerUnit(_startMineHeight, TYPE_STATIC);
        _staticUintProfitMap[_startMineHeight].record = true;
        _dynamicUintProfitMap[_startMineHeight].hash = calculateProfitPerUnit(_startMineHeight, TYPE_DYNAMIC);
        _dynamicUintProfitMap[_startMineHeight].record = true;
    }
    ...
    function refreshDynamicHashRateByBuy(address user, uint256 blockHeight) internal {
        ...
        for (uint i = 0; i < maxCanGetGenD; i++) {
            ...
            setAddressStaticHashRateAtHeight(parent, blockHeight, _addressTotalStaticHashRateMap[parent]);
            if (!_mineSwitch) {
                _addressLastRecordHeightBeforeStartMineMap[parent] = blockHeight;
            }
            // 全网总动态算力更新
    ...
    // 开挖高度
    function getStartMineHeight() public view returns (bool, uint256) {
        return (_mineSwitch, _startMineHeight);
    }
    ...
    function refreshSpacePrice(uint256 destroyApple) internal {
        // 已销毁APPLE数量增加
        _buyDestroyedApple = _buyDestroyedApple.add(destroyApple);
        // 锁定价格了，不再走这个逻辑了
        if (_spacePrice >= uint256(10).power(_busdContractDecimal)) return;
        // 加多少次0.01BUSD
        uint256 time = _buyDestroyedApple.div(uint256(10000).mul(uint256(10).power(_appleContractDecimal)));
        // 最高不超过0.4BUSD
        if (time >= 10) {
            _spacePrice = uint256(10).power(_busdContractDecimal);
        } else {
    ...
    function activeAddress(address user, uint256 qty) public {
        ...
        require(user != msg.sender, "can not active self");
        require(user != address(0), "can not active address(0)");
        require(user != _totalParentAddress, "can not active total parent address");
        // 转账
        ...
        if (currentBlockHeight >= 1) {
            uint256 start = _lastCalculateBlockHeight.add(1);
            if (!_hasRecord) {
                start = 0;
            }
            if (start < currentBlockHeight) {
                for (uint256 i = start; i < currentBlockHeight; i++) {
                    _staticUintProfitMap[i].hash = calculateProfitPerUnit(i, TYPE_STATIC);
                    _staticUintProfitMap[i].record = true;
                    _dynamicUintProfitMap[i].hash = calculateProfitPerUnit(i, TYPE_DYNAMIC);
                    _dynamicUintProfitMap[i].record = true;
                }
            }
        }
        // 被邀请地址所有上级刷新动态算力
        ...
        // 当前高度的单位收益
        _staticUintProfitMap[currentBlockHeight].hash = calculateProfitPerUnit(currentBlockHeight, TYPE_STATIC);
        _staticUintProfitMap[currentBlockHeight].record = true;
        _dynamicUintProfitMap[currentBlockHeight].hash = calculateProfitPerUnit(currentBlockHeight, TYPE_DYNAMIC);
        _dynamicUintProfitMap[currentBlockHeight].record = true;
        // 最后计算收益的高度
    ...
    // 总销毁概览[购买空间销毁数量 + 出局销毁总量 + 销毁记录条数]
    function destroySummary() public view returns (uint256, uint256, uint256) {
        return (_buyDestroyedApple, _outDestroyedApple, destroyCount());
    }

    function reachMax(address user, uint256 qty) internal view returns (bool) {
        uint256 l = _addressSpaceMap[user].length;
        uint256 t = 0;
        for (uint256 i = 0; i < l; i++) {
            t = t.add(_addressSpaceMap[user][i].spaceAmount);
        }
        return t.add(qty) > _buySpaceBuyMax;
    }

    // 是否执行销毁
    function doDestroy(uint256 amount) internal view returns (bool) {
        return _buyDestroyedApple.add(_outDestroyedApple).add(amount) <= _destroyAppleTotal;
    }

    // 购买空间
    function purchaseSpace(uint256 qty) public {
        // 购买空间总价
        uint256 totalBusd = getSpacePrice().mul(qty);
        require(!reachMax(msg.sender, qty), "reach max");
        // 转账
    ...
    function calculateUnitProfit(address user, uint256 qty, uint256 currentBlockHeight) internal {
        ...
        setAddressDynamicHashRateAtHeight(user, currentBlockHeight, _addressTotalDynamicHashRateMap[user]);
        if (!_mineSwitch) {
            _addressLastRecordHeightBeforeStartMineMap[user] = currentBlockHeight;
        }
    ...
    function getLastCalculateBlockHeight() public view returns (uint256) {
        return _lastCalculateBlockHeight;
    }

    function getNetTotalStaticHashRate() public view returns (uint256) {
        return _netTotalStaticHashRate;
    }

    function getNetTotalDynamicHashRate() public view returns (uint256) {
        return _netTotalDynamicHashRate;
    }
    ...
    function syncHeight(uint256 heightNum) public {
        ...
        for (uint256 i = _lastCalculateBlockHeight.add(1); i <= limit; i++) {
            ...
            if (_netTotalDynamicHashRate > 0) {
                ...
                _dynamicUintProfitMap[i].hash = 0;
            }
            _dynamicUintProfitMap[i].record = true;
        }
    ...
    // 全网数据[全网静态算力， 个人静态算力，个人总动态算力]
    function getNetData() public view returns (uint256, uint256, uint256) {
        return (_netTotalStaticHashRate, _addressTotalStaticHashRateMap[msg.sender], _addressTotalDynamicHashRateMap[msg.sender]);
    }
    ...
    function takeProfit() public {
        ...
        // 算力3倍价值的U
        uint256 maxTake = addressTotalStaticHashRate.mul(3).mul(uint256(10).power(_busdContractDecimal));
        // 超3倍
        if (pendingProfit.mul(applePrice).div(uint256(10).power(_appleContractDecimal)) >= maxTake) {
            clearStaticHashRate = true;
            // 3倍下的APPLE数量
            uint256 maxAppleAmount = maxTake.mul(uint256(10).power(_appleContractDecimal)).div(applePrice);
        ...
        // 多余的APPLE销毁
        if (destroyAmount > 0 && doDestroy(destroyAmount)) {
            require(TransferHelper.burn(_appleContractAddress, destroyAmount), "asset insufficient2");
            // 出局销毁总量添加
            _outDestroyedApple = _outDestroyedApple.add(destroyAmount);
            // 添加销毁记录
        ...
        // 扣减算力。刷新动态算力
        takeToSubHashRate(clearStaticHashRate, msg.sender, currentBlockHeight, takeAmount, applePrice);
        // 刷新上次领取收益的高度
    ...
    function takeToSubHashRate(bool clearStaticHashRate, address user, uint256 currentBlockHeight, uint256 takeAmount,
        uint256 applePrice) internal {
        // 地址本次扣减的静态算力
        ...
        if (clearStaticHashRate) {
            ...
            // 降了之后的算力
            needSubHashRate = takeAmount.mul(applePrice).div(uint256(10).power(_appleContractDecimal.add(_busdContractDecimal))).div(3);
            if (_addressTotalStaticHashRateMap[user].sub(needSubHashRate) < 1) {
                needSubHashRate = _addressTotalStaticHashRateMap[user];
            }
        }
        // 全网总静态算力扣减
    ...
    function getStaticUnitProfitByHeight(uint256 blockHeight) public view returns (bool, uint256) {
        return (_staticUintProfitMap[blockHeight].record, _staticUintProfitMap[blockHeight].hash);
    }

    function getDynamicUnitProfitByHeight(uint256 blockHeight) public view returns (bool, uint256) {
        return (_dynamicUintProfitMap[blockHeight].record, _dynamicUintProfitMap[blockHeight].hash);
    }
    ...
    function getStartCalProfitBlockHeight(address user) public view returns (uint256) {
        if (_mineSwitch) {
            ...
            return l < _startMineHeight ? _startMineHeight : l;
        } else {
            return uint256(10).power(25);
        }
    }
    ...
    // 设置购买空间上限
    function setBuySpaceMax(uint256 buySpaceBuyMax) public onlyOwner {
        require(buySpaceBuyMax > 0, "need rather than 0");
        _buySpaceBuyMax = buySpaceBuyMax;
    }

    // 根据区块高度得到产出量
    function getTotalGenByHeight(uint256 blockHeight) public view returns (uint256) {
        if (!_mineSwitch) return 0;
        uint256 d = getHeightCycle(blockHeight);
        return uint256(5200).sub(d.mul(310)).mul(uint256(10).power(_appleContractDecimal));
    }
    ...
    // 高度抓取第几个轮回
    function getHeightCycle(uint256 blockHeight) internal view returns (uint256) {
        if (!_mineSwitch) return 0;
        if (blockHeight < _startMineHeight) return 0;
        uint256 d = blockHeight.sub(_startMineHeight).div(90);
        return d > 10 ? 10 : d;
    }
    ...
    function getPendingProfit(address user, uint256 currentBlockHeight) internal view returns (uint256) {
        ...
        // 静态单位算力收益
        uint256 staticUnitProfit = _staticUintProfitMap[startBlock].hash;
        uint256 staticUnitProfitCycle = getHeightCycle(startBlock);
        // 此高度的静态算力
        uint256 s = _addressHeightStaticHashRateMap[user][startBlock].hash;
        if (!_addressHeightStaticHashRateMap[user][startBlock].record) {
            uint256 h = _addressLastRecordHeightBeforeStartMineMap[user];
            staticUnitProfitCycle = getHeightCycle(h);
            s = _addressHeightStaticHashRateMap[user][h].hash;
            staticUnitProfit = _staticUintProfitMap[h].hash;
        }

        // 动态单位算力收益
        uint256 dynamicUnitProfit = _dynamicUintProfitMap[startBlock].hash;
        uint256 dynamicUnitProfitCycle = getHeightCycle(startBlock);
        // 此高度的动态算力
        uint256 d = _addressHeightDynamicHashRateMap[user][startBlock].hash;
        if (!_addressHeightDynamicHashRateMap[user][startBlock].record) {
            uint256 h = _addressLastRecordHeightBeforeStartMineMap[user];
            dynamicUnitProfitCycle = getHeightCycle(h);
            d = _addressHeightDynamicHashRateMap[user][h].hash;
            dynamicUnitProfit = _dynamicUintProfitMap[h].hash;
        }
        ...
        for (uint256 i = startBlock; i < currentBlockHeight; i++) {
            ...
            if (_staticUintProfitMap[i].record) {
                staticUnitProfit = _staticUintProfitMap[i].hash;
                staticUnitProfitCycle = getHeightCycle(i);
            } else if (getHeightCycle(i) != staticUnitProfitCycle) {
                // 非同一个轮回，需要重新算
                staticUnitProfit = calculateProfitPerUnit(i, TYPE_STATIC);
                staticUnitProfitCycle = getHeightCycle(i);
            }
            // 如果没有，则沿用前一次的，因为没有静态算力变动
            tS = tS.add(staticUnitProfit.mul(s));

            // 这个高度确实记录了静态算力，直接用
            if (_addressHeightDynamicHashRateMap[user][i].record) {
                d = _addressHeightDynamicHashRateMap[user][i].hash;
            }
            if (_dynamicUintProfitMap[i].record) {
                dynamicUnitProfit = _dynamicUintProfitMap[i].hash;
                dynamicUnitProfitCycle = getHeightCycle(i);
            } else if (getHeightCycle(i) != staticUnitProfitCycle) {
                // 非同一个轮回，需要重新算
                dynamicUnitProfit = calculateProfitPerUnit(i, TYPE_DYNAMIC);
                dynamicUnitProfitCycle = getHeightCycle(i);
            }
            // 如果没有，则沿用前一次的，因为没有静态算力变动
            tD = tD.add(dynamicUnitProfit.mul(d));
        }
        return tS.add(tD);
    }
