...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ARKB3TEST3: The router already has that address");
  ...
  constructor() {
    ...
    ammIsWhiteListed[address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3)] = true; // ApeSwap router
  }
...
contract ARKB3TEST3 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("ARKB3TEST3", "ARKB3", 18) {
    ...
    receive() external payable {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "ARKB3TEST3: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "ARKB3TEST3: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ARKB3TEST3: The new dividend tracker must be owned by the ARKB3TEST3 token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "ARKB3TEST3: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "ARKB3TEST3: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ARKB3TEST3: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "ARKB3TEST3: The operations wallet is already this address");
    ...
    function updateFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _teamFee) external onlyOwner {
        ...
        require(totalFees <= 40, "Must keep fees at 20% or less");
    }

    // changes the gas reserve for processing dividend distribution
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ARKB3TEST3: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ARKB3TEST3: Cannot update gasForProcessing to same value");
    ...
    function updateSellPenalty(uint256 sellFactor) external onlyOwner {
        require(sellFactor >= 100 && sellFactor <= 350, "sellFactor must be between 100 and 350");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "ARKB3TEST3: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "ARKB3TEST3: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(!isBlacklistedToken(rewardTokenAddress), "ARKB3TEST3: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "ARKB3TEST3: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "ARKB3TEST3: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "ARKB3TEST3: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(!isBlacklistedToken(rewardTokenAddress), "ARKB3TEST3: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "ARKB3TEST3: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        uint256 userBNBDividends = getRawBNBDividends(msg.sender);
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "ARKB3TEST3: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ARKB3TEST3: Automated market maker pair is already set to that value");
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        if (success) {
            emit SwapAndSendTo(rewardsFee, amountBNBReflection, "Rewards");
            rewardsFee = 0;
        }
        ...
        if (success) {
            emit SwapAndSendTo(operationsFee, amountBNBOperations, "Operations");
            operationsFee = 0;
        }        
        ...
        if (success) {
            emit SwapAndSendTo(teamFee, amountBNBTeam, "Team");
            teamFee = 0;
        }
...
contract DividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() pure external override {
        require(false, "ARKB3TEST3_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ARKB3TEST3 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 86400, "ARKB3TEST3_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ARKB3TEST3_Dividend_Tracker: Cannot update claimWait to same value");
