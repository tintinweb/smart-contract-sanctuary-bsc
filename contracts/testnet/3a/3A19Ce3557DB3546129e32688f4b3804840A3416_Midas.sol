...
import "./interfaces/midas/IMidasTokenGeneratorManager.sol";
import "./managers/midas/MidasMultinetworkRouterManager.sol";

contract Midas is Initializable {
    MidasMultinetworkRouterManager public midasMultinetworkRouterManager;
    AggregatorV3Interface internal priceFeed;
    IMidasTokenGeneratorManager midasTokenGeneratorManager;
    ...
    function initialize() public initializer {
        ...
        midasMultinetworkRouterManager = MidasMultinetworkRouterManager(
            0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        );

        midasTokenGeneratorManager = IMidasTokenGeneratorManager(
            0xd8B821D610D000EA86f87ba947bD5fC0628d133e
        );
    ...
    function createNewToken(
        ...
        midasTokenGeneratorManager.createNewToken(
            tokenOwner,
            tokenName,
            tokenSymbol,
            supply
        ...
        returns (uint256)
    {
        ...
        if (availableChainlinkNet[network]) {
            nativeTokenPrice = uint256(getLatestPrice());
        } else {
            // get price from router pool
            nativeTokenPrice = estimatedTokensForTokens(
                midasMultinetworkRouterManager.getNativeTokenAddress(97),
                busdAddress,
                1 ether
            );
        }
    ...
    */
    ...
    function estimatedTokensForTokens(
        ...
        return
            midasMultinetworkRouterManager.getDexRouter().getAmountsOut(
                amount,
    ...
    function swapBnbForUSD(uint256 _amount) private {
        ...
        uint256 amountOutIn = midasMultinetworkRouterManager
            .getDexRouter()
            .getAmountsOut(_amount, path)[1];

        // make the swap
        midasMultinetworkRouterManager.getDexRouter().swapExactETHForTokens{
            value: _amount
    ...
    {
        address[] memory path = new address[](2);
        path[0] = midasMultinetworkRouterManager.getNativeTokenAddress(97);
        path[1] = busdAddress;

        IERC20(tokenAddress).approve(
            address(midasMultinetworkRouterManager.getDexRouterAddress()),
            tokenAmount
        );
...
interface IMidasTokenGeneratorManager {
    function createNewToken(
        address tokenOwner,
        string memory tokenName,
        string memory tokenSymbol,
