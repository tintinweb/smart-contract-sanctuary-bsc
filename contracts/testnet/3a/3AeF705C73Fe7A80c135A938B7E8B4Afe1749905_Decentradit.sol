...
contract Decentradit {
    ...
    function voteUp(bytes32 _postId, uint8 _reputationAdded) external {
      ...
      // if ( postList[num].upVotes >= postResult[num].minVotes) {
         if ( postList[num].upVotes >= postResult[_postId].minVotes) {
          postResult[_postId].pass = true ;
       }
        emit Voted(
            _postId,
            _contributor,
   ...
   function Result( bytes32 _postId ) public view returns (uint vote , bool finish , bool _postdelete){
      vote = postResult[ _postId].minVotes ; 
      _postdelete = postResult[ _postId].postDelete;
    if (postRegistry[_postId].upVotes < postResult[ _postId].minVotes){
