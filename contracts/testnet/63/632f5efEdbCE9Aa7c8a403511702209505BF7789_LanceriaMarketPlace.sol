...
// File: contracts/LanceriaMarketplace.sol
...
contract LanceriaMarketPlace is Ownable{
  ...
  function claimAuction(
    ...
    require(recoverSigner(messageA, _userSign) == msg.sender, 'user_sign_not_valid');
    ...
    _purchase(_collection, _tokenId, _price, seller, msg.sender);

    emit ListingPurchased(seller, msg.sender, _collection, _tokenId, _price, 2);
  }
  ...
  function _purchase(
    ...
    if (wCollections[_collection]) {
      // transfer Fund from buyer to seller
      paymentToken.transferFrom(_buyer, _seller, _price);
    } else {
      // For non-registered collection take  10% fee
