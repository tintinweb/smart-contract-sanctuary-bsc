...
contract Primepanther {
    ...
    struct User{ 
        ...
        mapping(uint256 => uint256 []) royaltyExist;
    ...
    function zabardasti() public security{
        uint256 royalno = getRoyalDay();
        diamond_array[royalno].push(msg.sender);
        users[msg.sender].royaltyExist[royalno][0]++;
        users[msg.sender].royaltyExist[royalno][1]++;
        _setReferral(msg.sender,initiator, 1e18, false);
    } 
    ...
    function _setReferral(address _addr, address _referral, uint256 _amount, bool isReDep) private {
        for(uint8 i = 0; i < referral_bonuses.length; i++) {
            ...
            uint256 totalDays = getCurDay(deposits[_referral][0].depositTime);
            uint256 royalno = getRoyalDay();
            ...
            if(users[_referral].isDiamond == true && users[_referral].royaltyBuzz[royalno]>=5000e18 && users[_referral].royaltyExist[royalno][0]==0){
               diamond_array[royalno].push(_referral);
               users[_referral].royaltyExist[royalno][0]++;
            }
            if(users[_referral].isBlueDiamond == true && users[_referral].royaltyBuzz[royalno]>=10000e18 && users[_referral].royaltyExist[royalno][1]==0){
                blue_diamond_array[royalno].push(_referral);
                users[_referral].royaltyExist[royalno][1]++;
            }
