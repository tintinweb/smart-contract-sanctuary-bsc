...
contract Authenticator is Ownable {
    ...
    function register(string memory name, bytes memory signature) external {
        address signer = getSigner(name, signature);
    ...
    function login(string memory name, bytes memory signature)
        external
        view
        returns (bool)
    {
        address signer = getSigner(name, signature);
        require(signer == msg.sender, "Authenticator: unauthorized signer");
        require(isRegistered[msg.sender], "Authenticator: user not registered");
        return isRegistered[msg.sender];
    }

    function getSigner(string memory name, bytes memory signature)
        internal
        view
        returns (address)
    {
        bytes32 hash = keccak256(
            abi.encodePacked(msg.sender, name, address(this))
        );
