...
contract UI {
    ...
    uint256 private constant oneWarriorArraySize = 27;
...
// *** SET ***
    ...
    function Initialize_All_Game(address eternalStorageAddr_, address[] memory values) external {
        ...
        */
        ...
        eternalStorage.SET_GLOBAL_DATA_UINT256("MAX_HERO_PERSONA", 3,
                                               "MAX_HERO_RARITY", 4,
                                               "MAX_ENEMY_PERSONA", 3,
                                               "WARRIOR_MINT_COST_ORACLE", 155 * 10 ** 18); // 155 USD referred
        eternalStorage.SET_GLOBAL_DATA_UINT256("WARRIOR_MINT_COST_STABLE", 15500 * 10 ** 18, // 0.01 referred
        ...
        eternalStorage.SET_GLOBAL_DATA_UINT256("ETH_WARRIOR_MINT_FEE", 0, // 4 * 10 ** 15 = 0.004 BNB (1.6 USD)
                                               ...
                                               "SWAP_FEE_ON_WITHDRAW", 50, // 50% of the fee will be swapped
                                               "WITHDRAW_COOLDOWN", 2 * 3600); // Wait 2 hours for every withdraw action
        eternalStorage.SET_GLOBAL_DATA_UINT256("REFERENCE_FEE", 5); // 5% withdraw fee (2.5% for user, 2.5% for owner)
    ...
    */
    function S_WARRIOR_MINT_COST_ORACLE_AND_STABLE(uint256 oracleValue_, uint256 stableValue_) external onlyOwner{
    ...
    // GLOBAL DATA ADDRESS
    /*
    function S_GLOBAL_DATA_ADDRESS(string[] calldata key_, address[] calldata value_) external onlyOwner {
        if (key_.length > 0 && key_.length == value_.length) {
            for (uint256 i = 0; i < key_.length; ++i){
...
// *** GET EXTERNAL IN-GAME ***
    ...
    */
    /*
    function EX_StatusUint256() external view returns (uint256[] memory _status) {
        return _EX_StatusUint256();
    }
    */
    function _EX_StatusUint256() private view returns (uint256[] memory) {
        uint256[] memory status = new uint256[](20);   
        ...
        if (eternalStorage.GET_GLOBAL_DATA_BOOL("ORACLE_ENABLED")) // Busd Amount -> Converts to Bnb Amount -> Converts to Aow Amount
            ...
            status[18] = status[8];
        status[19] = eternalStorage.GET_GLOBAL_DATA_UINT256("GAME_VERSION");
        return status;
    }
    ...
    */
    /*
    function EX_StatusAddress() external view returns (address[] memory _status) {
        return _EX_StatusAddress();
    }
    */
    function _EX_StatusAddress(address addr_) private view returns (address[] memory) {
        address[] memory status = new address[](13);   
        ...
        (status[8], status[9], status[10], status[11]) = eternalStorage.GET_GLOBAL_DATA_ADDRESS("MINT_ADDRESS",                                                                                       
                                                                                                ...
                                                                                                "BURN_ADDRESS",
                                                                                                "ROUTER_ADDRESS");      
        status[12] = eternalStorage.GET_WORD_DATA_ADDRESS(_stringToBytes32(string(abi.encodePacked(addr_))), 0, 0);                                                                                                                                         
        return (status);
    }
    ...
    */
    /*
    function EX_StatusBool() external view returns (bool[] memory _status) {
        return _EX_StatusBool();
    }
    */
    ...
    */
    /*
    function EX_WarriorDetails(uint256 warriorId_) external view returns (uint256[] memory _values) {
        return _EX_WarriorDetails(warriorId_);
    }
    */
    /*
    function EX_AllWarriorDetails(address addr_) external view returns (uint256[] memory _values) {
        return _EX_AllWarriorDetails(IWarrior721(eternalStorage.GET_GLOBAL_DATA_ADDRESS("WARRIOR721_ADDRESS")).G_AllOwnerTokenIds(addr_));
    }
    */
    /*
    function EX_AllWarriorDetails(uint256[] memory warriorIds_) external view returns (uint256[] memory _values) {
        return _EX_AllWarriorDetails(warriorIds_);
    }
    */
    ...
    function _EX_WarriorDetails(uint256 warriorId_) private view returns (uint256[] memory) {
        ...
        values[24] = warriorId_;
        (values[20], values[21]) = eternalStorage.GET_GLOBAL_DATA_UINT256("ONE_FIGHT_COST", "MAX_FIGHT_LIMIT");
        ...
        (values[7], values[22], values[23], values[25]) = eternalStorage.GET_NFT_DATA_UINT256(warriorId_, 0, 13, 5, 6, 10);
        (values[26]) = eternalStorage.GET_NFT_DATA_UINT256(warriorId_, 0, 0);
        if (values[7] > 0) {
            (values[4] , values[5], values[6],) = eternalStorage.GET_NFT_DATA_UINT256(warriorId_, 0, 7, 8, 9, 99);
        }
        values[8] = ((values[2] / 10) + 1) * 1000;
        if (block.timestamp > values[25]) values[9] = block.timestamp - values[25];
        values[10] = values[21] * values[20];

        if (values[25] + values[20] < block.timestamp) {
            if ((block.timestamp - values[25]) / values[20] >= values[21]) values[11] = values[21];
            else values[11] = (block.timestamp - values[25]) / values[20];
        }
    ...
    */
    /*
    function EX_WalletDetailsUint256(address addr_) external view returns (uint256[] memory _values) {
        return _EX_WalletDetailsUint256(addr_);
    }
    */
    function _EX_WalletDetailsUint256(address addr_) private view returns (uint256[] memory) {
        ...
        values[2] = eternalStorage.GET_WALLET_DATA_UINT256(addr_, 0, 1);
        (values[3], values[4]) = eternalStorage.GET_GLOBAL_DATA_UINT256("WITHDRAW_COIN_FEE", "WITHDRAW_FEE_DAILY_DROP");
        
        uint256 withdrawTimeDifference = block.timestamp - values[2];
        values[5] = values[3];
        values[6] = eternalStorage.GET_WALLET_DATA_UINT256(addr_, 0, 0);
        values[7] = values[6];
        values[8] = eternalStorage.GET_GLOBAL_DATA_UINT256("WITHDRAW_COOLDOWN");
        ...
        if (withdrawTimeDifference >= 86400) {
            ...
            uint256 totalFeeReduction = withdrawTimeDifference * values[4];
    ...
    function EX_AllLists(address addr_, uint256 maxIndex) external view returns (bool[] memory lists_) {
        return _EX_AllLists(addr_, maxIndex);
    }
    */
    ...
    /*
    function EX_AOWToBUSDPrice(uint256 amount) external view returns (uint256 aowToBUSDPrice, uint256 busdToAOWPrice) {
        (address tokenBnbPairAddress, address busdBnbPairAddress) = eternalStorage.GET_GLOBAL_DATA_ADDRESS("AOW_PAIR_ADDRESS", "BUSD_BNB_PAIR_ADDRESS");
        ...
        return (getTokenToUSDPrice(tokenBnbPairAddress, busdBnbPairAddress, amount), getUSDToTokenPrice(tokenBnbPairAddress, busdBnbPairAddress, amount));
    }
    */
}