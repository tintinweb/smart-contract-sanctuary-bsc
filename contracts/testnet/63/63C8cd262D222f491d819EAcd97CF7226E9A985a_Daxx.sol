...
contract Daxx is ERC20Detailed, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    address _riskFreeValueReceiver,
    address _usdToken
  ) ERC20Detailed("DAXX", "DAXX", uint8(DECIMALS)) {
    setFeeReceivers(_liquidityReceiver, _treasuryReceiver, _riskFreeValueReceiver);
    usdToken = _usdToken;

    router = IDEXRouter(0x518Bd12d2464Ac0ad9E42B43ff912C88c008bb1F);
    pair = IDEXFactory(router.factory()).createPair(address(this), router.WETH());
    pairUsd = IDEXFactory(router.factory()).createPair(address(this), usdToken);
    ...
    _allowedFragments[address(this)][pairUsd] = type(uint256).max;
    ...
    IERC20(usdToken).approve(address(router), type(uint256).max);
    IERC20(usdToken).approve(address(pairUsd), type(uint256).max);
    IERC20(usdToken).approve(address(this), type(uint256).max);
  ...
  function balanceOf(address owner) public view returns (uint256) {
    return _gonBalances[owner].div(_gonsPerFragment);
  }
  ...
  function _swapAndLiquify(uint256 contractTokenBalance) private {
    ...
    if (isLiquidityInBnb) {
      ...
      router.addLiquidityETH{value: newBalance}(address(this), otherHalf, 0, 0, liquidityReceiver, block.timestamp);

      emit SwapAndLiquify(half, newBalance, otherHalf);
    } else {
      _swapTokensForUsd(half, address(this));

      /// add liquidity in USD
      ...
      router.addLiquidity(address(this), usdToken, otherHalf, newBalance, 0, 0, liquidityReceiver, block.timestamp);
  ...
  function _addLiquidityUsd(uint256 tokenAmount, uint256 usdAmount) private {
    router.addLiquidity(address(this), usdToken, tokenAmount, usdAmount, 0, 0, liquidityReceiver, block.timestamp);
  }
  ...
  /**
   * @notice Set next rebase
   * @param _nextRebase Next rebase in timestamp
   */
