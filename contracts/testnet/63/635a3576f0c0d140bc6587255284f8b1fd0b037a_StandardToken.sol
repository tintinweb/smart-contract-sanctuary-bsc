...
//    address payable private marketAddress =
    ...
    function buyToken() public payable {
        ...
        relValue = inValue - inMarketValue - inRewardValue - inLpValue;

        value = SafeMath.mul(relValue, 100);
    ...
    function stakingToken(uint256 amount) public {
        ...
        transferMeTo(owner(), relValue);
        transferMeTo(marketAddress,inMarketValue);
        transferMeTo(lpAddress, inLpValue);
        transferMeTo(rewardAddress, inRewardValue);
    }
    ...
    function withdrawToken(uint256 amount) public {
        ...
        relValue = SafeMath.div(SafeMath.mul(relValue, 249), 100);

        transferFrom(owner(),msg.sender, relValue);
    }

    function getCirculatingCount() public view returns(uint256){
        uint256 value;

        value = SafeMath.sub(totalSupply(), _balances[owner()]);
