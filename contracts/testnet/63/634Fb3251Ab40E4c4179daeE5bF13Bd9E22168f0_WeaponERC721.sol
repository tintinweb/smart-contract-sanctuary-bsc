    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    mapping(uint8 => RangesByTier) public rangesMap;
    ...
    function initialize(
        ...
        rangesMap[1] = RangesByTier({minRange: 1, maxRange: 5});
        rangesMap[2] = RangesByTier({minRange: 6, maxRange: 10});
        rangesMap[3] = RangesByTier({minRange: 11, maxRange: 15});
        rangesMap[4] = RangesByTier({minRange: 16, maxRange: 20});
        rangesMap[5] = RangesByTier({minRange: 21, maxRange: 25});
        rangesMap[6] = RangesByTier({minRange: 26, maxRange: 30});

        baseURIByTier[1] = "https://bamboodefi.mypinata.cloud/ipfs/QmbvmPhQvp86ZGdRfkVCQejFowUfdrTU8tWtikaEKbxL46/";
        baseURIByTier[2] = "https://bamboodefi.mypinata.cloud/ipfs/QmRqUFViKQSWBAodDDZ9MMtwwVqwLqKbRAYaQLjHJ4En2i/";
        baseURIByTier[3] = "https://bamboodefi.mypinata.cloud/ipfs/QmbPwA6dzXEK26ED26qyjy5RmnbaE6XXbKaiwjwNipvLbb/";
        baseURIByTier[4] = "https://bamboodefi.mypinata.cloud/ipfs/QmRXNUK43FKUjSCcCP9vJDaAjx1UiQHA7ptt3L36t7qpi4/";
    }
    ...
    function randomMint(address to, uint8 _tier) public onlyRole(MINTER_ROLE) {
        _tokenIdCounter.increment();
    ...
    function fulfillRandomWords(
        ...
        RangesByTier memory range = rangesMap[requestData.tier];
        string memory _tokenURI = string(
            abi.encodePacked(
                baseURIByTier[requestData.tier],
                "_",
                StringsUpgradeable.toString(
                    (randomWords[0] % range.maxRange) + range.minRange
                ),
                ".json"
            )
        );
