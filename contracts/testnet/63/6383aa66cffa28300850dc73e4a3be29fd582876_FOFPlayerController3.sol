...
contract FOFPlayerController3 is Ownable {
    ...
    mapping(uint256 => uint256) public sellTokenIdToIndexs;
    ...
    function sellMyNFT(uint256 tokenId, uint256 price) public {
        ...
        if(sellTokenIdToIndexs[tokenId] > 0){
            sellTokenIds[sellTokenIdToIndexs[tokenId]] = tokenId;
        }else{

            sellTokenIdToIndexs[tokenId] = sellTokenIds.length;
    ...
    function cancelSellNFT(uint256 tokenId) public {
        ...
        delete sellTokenIds[sellTokenIdToIndexs[tokenId]];
    ...
    function getSellsByType(uint256 NFTType) public view returns(uint256[] memory sellNFTIDs, uint256[] memory prices, address[] memory owners) {
        ...
        for(i = 0; i < sellTokenIds.length; i++){
            if(FPN.NFTTypes(sellTokenIds[i]) == NFTType){
