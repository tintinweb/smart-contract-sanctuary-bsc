...
contract HardEvaluation is IDFYHardEvaluation, BaseContract {
    ...
    function createNftToken(
        ...
        require(bytes(nftCID).length > 0, "0"); // Invalid nftCID
    ...
    function evaluationAndCreateNFT(
        ...
        string memory evaluationCID,
        string memory nftCID
    ) external onlyEvaluator whenNotPaused {
        require(bytes(nftCID).length > 0, "0"); // Invalid nftCID
        ...
        if (collectionStandard == CollectionStandard.NFT_HARD_721) {
            tokenId = IDFYHard721(collectionAddress).safeMint(owner, nftCID);
        }
        ...
        emit EvaluationAndCreateNFTEvent(
            collectionAddress,
            amount,
            currency,
            price,
...
interface IDFYHardEvaluation is BaseInterface {
    ...
    event EvaluationAndCreateNFTEvent(
        address collectionAddress,
        uint256 amount,
        address currency,
        uint256 price,
