...
contract IDOSale is AccessControl, Pausable, ReentrancyGuard {
    ...
    // user address => purchased airdrop token amount
    mapping(address => uint256) public purchasedAirdropAmounts;
    // user address => claimed token amount
    mapping(address => uint256) public claimedAmounts;
    // user address => claimed airdrop token amount
    mapping(address => uint256) public claimedAirdropAmounts;
    // user address => claimed promotion data
    mapping(address => uint256) public promotionData;
    // IDO token price
    ...
    // The cap amount each user can purchase Airdrop up to
    uint256 public purchaseAirdropCap;
    // The total purchased amount
    uint256 public totalPurchasedAmount;
    // The total airdrop purchased amount
    uint256 public totalPurchasedAirdropAmount;
    ...
    event PurchaseAirdropCapChanged(uint256 purchaseAirdropCap);
    ...
    event PurchasedAirdrop(address indexed sender, uint256 amount);
    event ClaimedAirdrop(address indexed sender, uint256 amount);
    ...
    constructor(
        ...
        uint256 _purchaseAirdropCap,
        uint256 _startTime,
        uint256 _endTime
    ) {
        require(_ido != address(0), "IDOSale: IDO_ADDRESS_INVALID");
        require(_purchaseToken != address(0), "IDOSale: PURCHASE_TOKEN_ADDRESS_INVALID");
        require(_idoPrice > 0, "IDOSale: TOKEN_PRICE_INVALID");
        require(_purchaseCap > 0, "IDOSale: PURCHASE_CAP_INVALID");
        require(_startTime < _endTime, "IDOSale: TIMESTAMP_INVALID");
        ...
        purchaseAirdropCap = _purchaseAirdropCap;
     ...
     * @dev Set purchase cap for each user
     */
    function setPurchaseAirdropCap(uint256 _purchaseAirdropCap) external onlyOwner {
        purchaseAirdropCap = _purchaseAirdropCap;

        emit PurchaseAirdropCapChanged(_purchaseAirdropCap);
    }
     ...
     */
    function purchase(uint256 amount, address promotionAddress) external nonReentrant whenNotPaused {
        ...
        require(amount <= purchaseCap, "IDOSale: PURCHASE_CAP_EXCEEDED");
        require(purchasedAmounts[_msgSender()] == 0, "IDOSale: You purchase IDO token already");
        require(promotionAddress != _msgSender(), "Promotion Address can't be same as Your address");
        ...
        promotionData[promotionAddress] += 1;
    ...
    /**
     * @dev Purchase Airdrop token
     * Only whitelisted users can purchase within `airdropCap` amount
     */
    function purchaseAirdrop(uint256 amount) external nonReentrant whenNotPaused {
        require(startTime <= block.timestamp, "IDOSale: SALE_NOT_STARTED");
        require(amount > 0, "IDOSale: PURCHASE_AMOUNT_INVALID");
        require(purchasedAirdropAmounts[_msgSender()] + amount <= purchaseAirdropCap, "IDOSale: PURCHASE_CAP_EXCEEDED");
        uint256 idoBalance = ido.balanceOf(address(this));
        require(totalPurchasedAirdropAmount + amount <= idoBalance, "IDOSale: INSUFFICIENT_SELL_BALANCE");
        uint256 purchaseTokenAmount = amount * idoPrice / (10 ** 18);
        require(purchaseTokenAmount <= purchaseToken.balanceOf(_msgSender()), "IDOSale: INSUFFICIENT_FUNDS");

        purchasedAirdropAmounts[_msgSender()] += amount;
        totalPurchasedAirdropAmount += amount;
        purchaseToken.safeTransferFrom(_msgSender(), address(this), purchaseTokenAmount);
        emit PurchasedAirdrop(_msgSender(), amount);
        
        claimedAirdropAmounts[_msgSender()] += amount;
        ido.safeTransfer(_msgSender(), amount);

        emit ClaimedAirdrop(_msgSender(), amount);
    }
    ...
    /**
     * @dev Return PromotionData 
     */
    function getPromotionData(address userAddress) external view returns (uint256) {
        require(userAddress != address(0), "Address: ADDRESS_INVALID");

        return promotionData[userAddress];
    }
