...
contract AuraStakingProgram  {
    ...
    function start_staking(uint256 walletUsedBusd,uint256 _programId,address referrer) public 
    {
        ...
        if(users[msg.sender].totalStakingBusd<100*1e18 && (users[msg.sender].totalStakingBusd+_busdAmount)>100*1e18)
        {
            users[users[msg.sender].referrer].qualifiedDirect+=1;
        }
        users[users[msg.sender].referrer].directInvestment+=_busdAmount;
    ...
    function isQualifiedForLevel(address _user) public view returns (bool) 
    {
        ...
        if(users[_user].qualifiedDirect>=3 && users[_user].directInvestment>=1000e18)
        return true;
        else return false;
    }

    function directMember(address _user) public view returns (address[] memory) 
    {
