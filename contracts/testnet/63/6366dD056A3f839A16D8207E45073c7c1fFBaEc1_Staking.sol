...
contract Staking is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(length == _rewardsPerSecond.length, "INVALID_SCHEDULE");
    unchecked {
    ...
    function deposit(
        ...
        if (user.amount > 0) {
            ...
            if (_pendingReward > 0) {
                require(IRewardManager(rewardManager).mint(msg.sender, _pendingReward, true), "TOKEN_MINT_FAILED");
    ...
    function emergencyWithdraw(uint256 pid) external {
        ...
        // decrease pool balance
        poolInfo[pid].tokenBalance -= user.amount;
    ...
    {
        uint256 length = _startTimeOffset.length;
        require(length == _rewardsPerSecond.length && length == emissionSchedule.length, "INVALID_SCHEDULE");
    unchecked {
        for (uint256 i = length - 1; i + 1 != 0; i--) {
            emissionSchedule[length - i - 1] = EmissionPoint({
    ...
    function flushLostToken(uint256 pid) external onlyOwner {
