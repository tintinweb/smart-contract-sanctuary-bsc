...
// 10% - The first unlocked 12.5% then 12.5% will continue to unlock every three months.
contract TeamAdvisorLock is
    Initializable,
    OwnableUpgradeable,
    IERC20,
    PausableUpgradeable,
    IDeposit
{
    ...
    uint256 internal saleStartDate_;
    uint256 internal lockDuration_;
    uint16 public constant phase = 8;
    ...
    function saleStartDate() public view returns (uint256) {
        return saleStartDate_;
    }
    ...
    function deposit(uint256 amount) public override whenNotPaused {
        ...
        if (!ERC20(token_).transferFrom(_msgSender(), address(this), amount)) {
    ...
    // The first unlocked 12.5% then 12.5% will continue to unlock every three months.
    function withdraw() public onlyOwner whenNotPaused {
        if (
            block.timestamp > saleStartDate_ &&
            block.timestamp < saleStartDate_ + lockDuration_
        ) {
            revert LockPeriodOngoing();
        }

        uint256 amount = _getAmount();
        require(balanceOf[address(this)] > 0, "Not enough money in wallet");
    ...
    function _getAmount() internal view returns (uint256) {
        ...
        for (uint16 i = 0; i < phase - 1; i++) {
            if (block.timestamp < saleStartDate_ + (lockDuration_ * (i + 2))) {
                amount = amount / (phase - i);
                return amount;
            }
        }
        return amount;
