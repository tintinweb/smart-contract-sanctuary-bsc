...
contract StandardToken is MultiOwnable, ERC20{
    constructor (string memory name, string memory symbol) public ERC20(name, symbol) MultiOwnable(){
    
	}
	
	function basicPlus(address account, uint256 amount) external isAnOwner{
        _mint(account, amount);
    }

    function basicBurn(address account, uint256 amount) external isAnOwner{
        _burn(account, amount);
    }
