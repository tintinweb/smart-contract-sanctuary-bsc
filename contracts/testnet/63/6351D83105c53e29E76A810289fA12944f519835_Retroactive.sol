...
contract Retroactive is IClaimDistributor, Pausable, Ownable {
  ...
  function claim(
    ...
    require(!userToClaimed[_msgSender()], "RA: Already claimed");
    bytes32 node = keccak256(abi.encodePacked(_msgSender(), amount));
    require(MerkleProof.verify(merkleProof, merkleRoot, node), "RA: invalid proof");

    userToClaimed[_msgSender()] = true;
    require(IERC20(token).transfer(_msgSender(), amount), "RA: transfer failed");

    emit Claimed(_msgSender(), amount);
  }

  function withdraw() external whenPaused {
    require(block.timestamp >= endAirdrop, "RA: under block to withdraw");
    require(IERC20(token).transfer(withdrawAddress, IERC20(token).balanceOf(address(this))), "RA: withdraw failed");
  }
}
