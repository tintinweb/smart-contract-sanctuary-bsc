...
Ownable
{
  ...
  function openBox(uint256 _flag) public payable {
    ...
    if (p < cfg[1]) {
      //0.1 three
      IERC20(three).transfer(address(this), cfg[0]);
    } else if (p >= cfg[1] && p < cfg[3]) {
      //0.5 three
      IERC20(three).transfer(address(this), cfg[2]);
    } else if (p >= cfg[3] && p < cfg[5]) {
      //1 three
      IERC20(three).transfer(address(this), cfg[4]);
    } else if (p >= cfg[5] && p < cfg[7]) {
      //2 three
      IERC20(three).transfer(address(this), cfg[6]);
