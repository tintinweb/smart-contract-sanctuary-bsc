...
contract BamiMarketPlace is Ownable, ReentrancyGuard, Pausable {
  ...
  function sellItem(address _nftAddress, uint256 _nftId, uint256 _price, address _currency) external nonReentrant validOpenMarket validAddress(_currency) {
    ...
    counter.increment();
    // Emit event
    emit EventNewSales(saleItem.saleId, saleItem.nftId, saleItem.priceListing, saleItem.isSold, saleItem.seller, address(0), currency, nftAddress);
  ...
  function getNftList() public view returns(uint256 nftId)
  {
    SaleItem memory saleItem;
    for(uint256 index; index < saleItems.length; index++)
    {
      if(saleItem.seller == msg.sender)
      {
        nftId = saleItem.nftId;
        return nftId;
      }
    }
  }
