...
contract Test is Context, IERC20, Ownable {
    ...
    // it will contain balance of Reward Token
    mapping (address => uint256) public _rOwned;
    // it will contain balance of actual Token
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    //Blacklisted Address cant receive reward tokens
    mapping (address => bool) private _isExcluded;
    // list of blackListed addresses cant receive reward tokens 
    ...
    constructor (
        ...
        uint256 _DECIMALS, 
        uint256 _supply, // 1 Quadrillion
        uint256 _txFee,
        uint256 _lpFee,
        uint256 _DexFee,
        address routerAddress,
        address feeaddress,
        address tokenOwner) payable {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
        ...
        _maxTxAmount = (_tTotal * 5 / 1000) * 10 ** _decimals; // 5 Trillion
        numTokensSellToAddToLiquidity = (_tTotal * 5 / 10000) * 10 ** _decimals; // 500 Billion
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate =  _getRate(); // _getRate() = _rTotal /_tTotal
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidity); // add rLiquidity to this contract

        if(_isExcluded[address(this)]) // if this contract is blacklisted = ineligibel for reflective token 
    ...
    function _transfer(
        ...
        uint _contractTokenBalance,
        uint _numTokensSellToAddToLiquidity,
        bool _overMinTokenBalance
        ...
        _overMinTokenBalance = overMinTokenBalance; 
    }
...
//-------------------Testing--------------//
    ...
    function currentRate_And_Balance() external view returns(uint256 currentRate_,uint256 contractTokenBalance_, uint256 rTotal_,uint _rDivided_t ){
        currentRate_ = _getRate();
        contractTokenBalance_ = balanceOf(address(this));
        rTotal_ = _rTotal;
         _rDivided_t = _rTotal.div(_tTotal);

        
    }

    function condition2_getCurrentSupply()external view returns(uint256, uint256){
    ...
    function condition3_getCurrentSupply()external view returns(uint256, uint256){
