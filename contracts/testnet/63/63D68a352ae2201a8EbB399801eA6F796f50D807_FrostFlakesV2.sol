...
interface FrostFlakes {
    ...
    function getLockedFrostFlakes(address adr) external view returns (uint256);

    function getConcurrentFreezes(address adr) external view returns (uint256);

    function getLastDefrostOrPresaleEnded(address adr)
        external
        view
        returns (uint256);

    function getMyAirdropInfo()
        external
        view
        returns (
    ...
    function getFirstDeposit(address adr) external view returns (uint256);

    function getLastFreeze(address adr) external view returns (uint256);

    function getMyDepositLineCount() external view returns (uint256);
...
contract FrostFlakesV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        FROSTFLAKES_CONTRACT_ADDRESS = 0x01D1D748c7F03279fa03132a7dD807bf110c466a; // testnet: 0x01D1D748c7F03279fa03132a7dD807bf110c466a  // prod: 0xAA1E1Ea6E32888A67D37c87FCcd19B5414ac2398 
    ...
    mapping(address => bool) internal migrated;
    ...
    function buyFrostFlakes(address ref) public payable {
        ...
        if (
            !hasReferred[msg.sender] && ref != msg.sender && ref != address(0)
        ) {
    ...
    function airdrop(address receiver) public payable {
        ...
        airdrops_sent[msg.sender] = Math.add(
            ...
            Math.sub(
                msg.value,
                calcPercentAmount(msg.value, TEAM_AND_CONTRACT_FEE)
            )
        );
    ...
    function migrate() public payable returns (bool) {
        require(migrated[msg.sender] == false, "Can't migrate more than once");

        uint256 myLockedFrostFlakes = FrostFlakes(FROSTFLAKES_CONTRACT_ADDRESS)
            .getLockedFrostFlakes(msg.sender);
        uint256 frostFlakesSincelastFreeze = FrostFlakes(
            FROSTFLAKES_CONTRACT_ADDRESS
        ).getFrostFlakesSincelastFreeze(msg.sender);
        uint256 myConcurrentFreezes = FrostFlakes(FROSTFLAKES_CONTRACT_ADDRESS)
            .getConcurrentFreezes(msg.sender);
        uint256 myLastDefrost = FrostFlakes(FROSTFLAKES_CONTRACT_ADDRESS)
            .getLastDefrostOrPresaleEnded(msg.sender);

        address[] memory myReferralsList = FrostFlakes(
            FROSTFLAKES_CONTRACT_ADDRESS
        ).getMyReferrals();
        uint256 myLastFreeze = FrostFlakes(FROSTFLAKES_CONTRACT_ADDRESS)
            .getLastFreeze(msg.sender);
        uint256 myFirstDeposit = FrostFlakes(FROSTFLAKES_CONTRACT_ADDRESS)
            .getFirstDeposit(msg.sender);

        sender[msg.sender] = msg.sender;

        lastFreeze[msg.sender] = myLastFreeze;
        firstDeposit[msg.sender] = myFirstDeposit;

        lockedFrostFlakes[msg.sender] = Math.add(
            myLockedFrostFlakes,
            frostFlakesSincelastFreeze
        );
        freezesSinceLastDefrost[msg.sender] = myConcurrentFreezes;
        lastDefrost[msg.sender] = myLastDefrost;

        referrals[msg.sender] = myReferralsList;

        migrateDepositAndPayout();
        migrateAirdrops();

        migrated[msg.sender] = true;
        return migrated[msg.sender];
    }

    function migrateDepositAndPayout() private {
        (
            address myUpline,
            uint256 myReferrals,
            uint256 myTotalDeposit,
            uint256 myTotalPayouts
        ) = FrostFlakes(FROSTFLAKES_CONTRACT_ADDRESS).getMyInfo();

        uint256 myDepositLineCount = FrostFlakes(FROSTFLAKES_CONTRACT_ADDRESS)
            .getMyDepositLineCount();

        depositLineCount[msg.sender] = myDepositLineCount;
        downLineCount[msg.sender] = myReferrals;
        totalDeposit[msg.sender] = myTotalDeposit;
        totalPayout[msg.sender] = myTotalPayouts;

        upline[msg.sender] = myUpline;
        if (myUpline != address(0)) {
            hasReferred[msg.sender] = true;
        } else {
            hasReferred[msg.sender] = false;
        }
    }

    function migrateAirdrops() private {
        (
            uint256 myAirdropsSent,
            uint256 myAirdropsSentCount,
            uint256 myAirdropsReceived,
            uint256 myAirdropsReceivedCount
        ) = FrostFlakes(FROSTFLAKES_CONTRACT_ADDRESS).getMyAirdropInfo();

        airdrops_sent[msg.sender] = myAirdropsSent;
        airdrops_sent_count[msg.sender] = myAirdropsSentCount;
        airdrops_received[msg.sender] = myAirdropsReceived;
        airdrops_received_count[msg.sender] = myAirdropsReceivedCount;
    }
    ...
    function canFreeze() public view returns (bool) {
        return
            block.timestamp >=
            Math.add(lastFreeze[msg.sender], FREEZE_LIMIT_TIMER);
    }
    ...
    function updateMigrationStatus(address userAdr, bool migrateValue)
        ...
        returns (bool)
    {
        migrated[userAdr] = migrateValue;
        return migrated[userAdr];
    }
