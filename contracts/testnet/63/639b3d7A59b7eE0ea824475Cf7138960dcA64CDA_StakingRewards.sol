...
contract StakingRewards is  ReentrancyGuardUpgradeable,OwnableUpgradeable {
    ...
    address public community = 0xA12bBA3dD8c0dAbD9e556D57b2805d13182b2E62 ;
    ...
    function stake(uint256 amount) external  nonReentrant updateReward(msg.sender) {
        ...
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
         ...
         //_timedStake.push(TimedStake({amount: msg.value, stakeTimes: block.timestamp}));
        _timedStake.stakes[block.timestamp] = amount;
        _timedStake.stakeTimes.push(block.timestamp); 
        _timedStake.stIndex.push();
         ...
         for (uint8 index = 0; index < _timedStake.stakeTimes.length; index++) {
           if (_total > 0){
              uint256 key = _timedStake.stakeTimes[index];
               if (block.timestamp.sub(key) >holdingTime){
