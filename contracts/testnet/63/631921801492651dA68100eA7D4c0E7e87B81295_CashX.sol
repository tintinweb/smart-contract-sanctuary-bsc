...
contract CashX {
  ...
  function sig2hash(string memory _msg, bytes memory _sig) private pure returns (bytes32) {
    bytes32 r;
    bytes32 s;
    uint8 v;
    assembly {
  ...
  function withdraw(bytes calldata _sig, address payable _recipient, uint256 _gas) external payable nonReentrant {
    bytes32 _hash = sig2hash("withdraw", _sig);
    require(locked[_hash], "The commitment has not been locked");
    uint256 fee = useFixedFee ? amountFee : denomination * amountFee / 10000;
    if(msg.sender==_recipient)
      _gas = 0;
    if(token==address(0)) {
      require(msg.value == 0, "Message value is supposed to be zero for ETH instance");
      (bool success, ) = _recipient.call{ value: denomination - fee - _gas }("");
