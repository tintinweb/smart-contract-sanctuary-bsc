...
contract TokenLock {
    ...
    constructor(
        ...
        // check inputs
        require(_unlockTimestampsLength == _unlockRates.length, "INVALID_ARRAY_LENGTH");

        require(_unlockTimestampsLength > 0, "EMPTY_ARRAY");
    ...
    unchecked {
        for (uint256 i = _unlockTimestampsLength - 1; i + 1 != 0; i--) {
    ...
    function lock(address[] calldata recipients, uint256[] calldata amounts) external {
        ...
        require(recipientsLength > 0, "EMPTY_ARRAY");

        require(recipientsLength == amounts.length, "INVALID_ARRAY_LENGTHS");
