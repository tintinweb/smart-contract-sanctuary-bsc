...
import "./interfaces/IGTPTokenFactory.sol";
import "./interfaces/IVCCToken.sol";

/** @title Swap Contract  */
contract Swap is Ownable, ERC1155Holder, ERC721Holder {
    IGTPToken GTP;
    IVCCToken public VCC;
    IGTPTokenFactory public ERC1155Factory;
    ...
    */
    ...
    function setGTPTokenFactory(address _tokenAddress) external onlyOwner {
        require(_tokenAddress != address(0), "Cannot be zero address");
        ERC1155Factory = IGTPTokenFactory(_tokenAddress);
    }

    /** @dev Responsible for swap b/w VCC and GTP + burn the received the GTP
     * @param pId Project ID for the corresponding Project token Address
     * @param tokenId Token ID w.r.t the assigned Project ID
    ...
    function swap(
        ...
        emit SwapSucess(pId, tokenId, _msgSender(), startIndex, quantity);
    }
}
