...
contract PistonBuy is Initializable, ERC20Upgradeable, OwnableUpgradeable {
	...
	function swapBUSDForTokens(uint256 busdAmount) public {
			...
			// get BUSD from user
			IERC20Upgradeable(BUSD).approve(address(this), busdAmount);
	...
	/* getters */
	function getBalances() public view  returns (uint256, uint256, uint256) { // get PSTN/BUSD/BNB balances
		return (pistonToken.balanceOf(address(this)), IERC20Upgradeable(BUSD).balanceOf(address(this)), address(this).balance);
	}
	
	function getPairBalance() public view  returns (uint256, uint256) { // get PSTN/BUSD balances of exchange
