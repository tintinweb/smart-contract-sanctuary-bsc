...
contract BlockStarLPStaking is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => mapping( uint256 => uint256)) public PooltotalStake;
    mapping(address => mapping( uint256 => uint256)) public PooltotalWithdraw;
    mapping(address => mapping( uint256 => uint256)) public PooltotalRewardsDistribution;
   ...
   function initialize() public initializer {
    ...
    function editPool(
        ...
        for(uint i=0;i < pooldata[_poolAddress].length;i++){
            if(pooldata[_poolAddress][i].lockupDuration == _lockupDuration){
                pooldata[_poolAddress][i].totalReward = _totalReward;
                pooldata[_poolAddress][i].status = _status;
    ...
    function deposit(
        ...
        orders[++latestOrderId] = OrderInfo(
            ...
            _id,
            block.timestamp,
            pool.endTime > block.timestamp ? pool.endTime : pool.endTime.add(pool.lockupDuration.mul(baseTime)),
            0,
            false
        );

        totalStake = totalStake.add(_amount);
        PooltotalStake[_poolAddress][_id] = PooltotalStake[_poolAddress][_id].add(_amount);
    ...
    function _calculateReward(address _poolAddress , uint256 _id) internal returns(uint256 [] memory percentage_Arr ,  address [] memory user_Adderss , uint256 [] memory total_Reward , uint256 [] memory order_Ids , uint256 extra_Amount){
        uint256 total_Stake = PooltotalStake[_poolAddress][_id];
        ...
        for(uint i=1; i <= latestOrderId ; i++){
            if( !orders[i].claimed && orders[i].starttime < pool.endTime && orders[i].starttime > pool.startTime  && orders[i].amount > 0){
                arrIndex++;
            }
        }

        uint [] memory percentageArr = new uint[](arrIndex);
        address [] memory userAdderss = new address[](arrIndex);
        uint [] memory totalReward = new uint[](arrIndex);
        uint [] memory order_IdsArr = new uint[](arrIndex);


        uint arrIndexI = 0;
        for(uint i=1; i <= latestOrderId ; i++){
            if( !orders[i].claimed && orders[i].starttime < pool.endTime && orders[i].starttime > pool.startTime  && orders[i].amount > 0){
                uint256 percentage = orders[i].amount.mul(10000).div(total_Stake); 
                uint256 userTotalReward = pool.totalReward.mul(percentage).div(10000);
                uint256 userTotalTime = pool.endTime.sub(orders[i].starttime);
                ...
                extraAmount += userTotalReward.mul(10000 - userTimePer).div(10000);
                percentageArr[arrIndexI] = percentage;
                userAdderss[arrIndexI] = orders[i].beneficiary;
                totalReward[arrIndexI] = userFinalReward;
                order_IdsArr[arrIndexI] = i;
    ...
    function withdraw(uint256 _orderId) public nonReentrant{
        ...
        PooltotalStake[poolAddress][id] = PooltotalStake[poolAddress][id].sub(amount);
    ...
    function distributeReward(address _poolAddress , uint256 _id) public nonReentrant {
        PoolInfo storage pool = pooldata[_poolAddress][_id];
        require(pool.lockupDuration > 0 && pool.endTime > 0 && pool.poolAddress != address(0) , "Pool data not found or pool not added with Id!!");
        ...
        for(uint i=0; i < order_Ids.length ; i++){
            ...
            orders[order_Ids[i]].starttime = pool.endTime;
            orders[order_Ids[i]].endtime = pool.endTime.add(pool.lockupDuration.mul(baseTime));

            PooltotalRewardsDistribution[_poolAddress][_id] += userReward;
