...
pragma solidity ^0.8.0;
contract SALERA_TOKEN is ERC20, Ownable {
    ...
    constructor() ERC20("SALERA_TOKEN", "SRA") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are KIA_FINANCE!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are KIA_FINANCE!!!
    sTot ++;
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function LPKIA_FINANCELocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPKIA_FINANCELocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
   ...
   /*************************************/
    ...
    function buyTokens1() payable public {
        uint amount = (msg.value * buyPrice) / (10**18) ;                 // calculates the amount
     ...
     */
    function sellTokens1(uint256 amount) public {
