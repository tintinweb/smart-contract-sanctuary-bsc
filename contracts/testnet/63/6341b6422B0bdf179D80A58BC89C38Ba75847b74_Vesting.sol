...
contract Vesting is Ownable, ReentrancyGuard {
	...
	event CreatedVestingSchedule(address sender, address account, uint256 startTime, uint256 vestingPeriod, uint256 vestingCliff, uint256 releaseInterval, uint256 amount);
	 ...
	 * @param listOfReleaseInterval release interval in days
	...
	function addVestingSchedules(
		...
		uint256[] memory listOfReleaseInterval,
		...
		require(
			...
			listOfRecipient.length == listOfVestingPeriod.length &&
				...
				listOfRecipient.length == listOfReleaseInterval.length &&
	 ...
	 * @param _releaseInterval release interval in days
	...
	function createVestingSchedule(
		...
		VestingSchedule memory vestingSchedule = VestingSchedule(
			...
			_releaseInterval * MINUTES_IN_DAY * 60,
			_startTime,
			_amount,
			0,
			false
		...
		emit CreatedVestingSchedule(msg.sender, _recipient, _startTime, _vestingPeriod, _vestingCliff, _releaseInterval, _amount);
	}
	...
	function _claimTo(address to) internal {
		...
		require(
			block.timestamp > vestingSchedulesById[id].startTime,
			"Vesting is not started."
		);

		require(
			block.timestamp >= vestingSchedulesById[id].startTime + vestingSchedulesById[id].vestingCliff,
			"Vesting is not unlocked."
		);

		require(
			block.timestamp >= vestingSchedulesById[id].startTime + vestingSchedulesById[id].vestingPeriod || block.timestamp >= vestingSchedulesById[id].lastClaimedTime + vestingSchedulesById[id].releaseInterval,
			"Cannot claim before passing release interval."
		);

		uint256 _amount = computeClaimableAmount(id);

		require(_token.balanceOf(address(this)) > _amount, "Not enough remained token on contract");
		...
		uint256 numberOfIntervals = (block.timestamp - vestingSchedulesById[id].startTime - ((block.timestamp - vestingSchedulesById[id].startTime) % vestingSchedulesById[id].releaseInterval)) / vestingSchedulesById[id].releaseInterval;
		vestingSchedulesById[id].lastClaimedTime += numberOfIntervals * vestingSchedulesById[id].releaseInterval;
		...
		returns (uint256 _amount)
	{
		if (vestingSchedulesById[id].isEnded) {
			...
			if (block.timestamp < vestingSchedulesById[id].startTime) {
				_amount = 0;
			} else if (
				block.timestamp >=
				vestingSchedulesById[id].startTime +
				...
				if (
					block.timestamp >=
					vestingSchedulesById[id].startTime + vestingSchedulesById[id].vestingCliff
				) {
					uint256 numberOfIntervals = (block.timestamp - vestingSchedulesById[id].startTime - ((block.timestamp - vestingSchedulesById[id].startTime) % vestingSchedulesById[id].releaseInterval)) / vestingSchedulesById[id].releaseInterval;

					_amount =
						(vestingSchedulesById[id].allocatedAmount *
							vestingSchedulesById[id].releaseInterval) /
						vestingSchedulesById[id].vestingPeriod * numberOfIntervals -
						vestingSchedulesById[id].vestedAmount;
				} else {
					_amount = 0;
				}
	 ...
	 */
	function rescueWithdraw(uint256 _amount)
		public
		onlyOwner
		nonReentrant
	{
	...
	function getStartTime(uint256 id) public view returns (uint256) {
		return vestingSchedulesById[id].startTime;
	}
