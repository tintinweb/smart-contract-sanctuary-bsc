...
contract DreamHorseToken is Context, IBEP20, Ownable {
    ...
    mapping (uint256 => uint256) public bullNFTTokenId;

    mapping (uint256 => uint256) public hammerFixTime;

    mapping (uint256 => Pledge) public tokenIdToPledges;

    mapping (uint256 => uint256) public pledgeTypesToRate;

    uint256 public hammerFixcircleTime = 1 days;

    uint256 public hammerBasePrice = 100 * 10 ** 18;

    // uint256 public bullNFTPrice = 500 * 10 ** 18;

    // uint256 public founderNFTPrice = 1000 * 10 ** 18;
    ...
    string private _name = "DreamHorse Token";
    ...
    address public collapsePoolAddr = 0xEd703D611df2fef3D0c626B1f688edFeeA8CfcF7;

    address public repoPoolAddr = 0xEd703D611df2fef3D0c626B1f688edFeeA8CfcF7;
    
    uint256 public _buyCastingPoolFee = 5;
    uint256 private _previousBuyCastingPoolFee = _buyCastingPoolFee;

    uint256 public _buyOperationFee = 2;
    uint256 private _previousBuyOperationFee = _buyOperationFee;

    uint256 public _buyCollapsePoolFee = 2;
    uint256 private _previousBuyCollapsePoolFee = _buyCollapsePoolFee;


    uint256 public _sellCastingPoolFee = 5;
    uint256 private _previousSellCastingPoolFee = _sellCastingPoolFee;

    uint256 public _sellOperationFee = 2;
    uint256 private _previousSellOperationFee = _sellOperationFee;

    uint256 public _sellCollapsePoolFee = 4;
    uint256 private _previousSellCollapsePoolFee = _sellCollapsePoolFee;

    uint256 public _sellRepoPoolFee = 3;
    uint256 private _previousSellRepoPoolFee = _sellRepoPoolFee;

    
    // uint256 public _liquidityFee = 2;
    // uint256 private _previousLiquidityFee = _liquidityFee;

    // uint256 public _transferFee = 12;
    ...
    event UpdateBuyFees(uint256 buyCastingPoolFee, uint256 buyOperationFee, uint256 buyCollapsePoolFee);
    // event UpdateBuyFees(uint256 bnbFee, uint256 liquidityFee, uint256 transferFee);
    event UpdateSellFees(uint256 sellCastingPoolFee, uint256 sellOperationFee, uint256 sellCollapsePoolFee, uint256 sellRepoPoolFee);
    ...
    constructor () {
        ...
        TRN = TaurenNFT(0x4A2A8D38c5081b51A7693200AE6a13F2b2557933);

        pledgeTypesToRate[1] = 100;
        pledgeTypesToRate[2] = 300;
        pledgeTypesToRate[3] = 500;
        pledgeTypesToRate[4] = 500;
    }

    function setPledgeTypesToRate(uint256 index, uint256 rate) public onlyOwner {
        pledgeTypesToRate[index] = rate;
    }
    ...
    function buyHammer(uint256 _bullPeriodIndex) public {

        require(hammerOwner[_bullPeriodIndex].length < 16, "This NFT has been unlocked");
        ...
        require(hammerFixTime[_bullPeriodIndex] + 1 days > block.timestamp, "You can't buy your own hammer");

        uint256 price = hammerBasePrice.mul(11 ** (hammerOwner[_bullPeriodIndex].length - 1)).div(10 ** (hammerOwner[_bullPeriodIndex].length - 1));

        ERC20(BUSD).transferFrom(msg.sender, preOwner, price);

        hammerOwner[_bullPeriodIndex].push(msg.sender);

        uint256 tokenId = bullNFTTokenId[_bullPeriodIndex];
        ...
        // uint256 amount = tokenOfPair.mul(price).div(BUSDOfPair * 100);

        uint256 amount = BUSDEqualToken(price);

        transferFrom(msg.sender, deadWallet, amount * 10 ** 18);

        if(hammerOwner[_bullPeriodIndex].length >= 16){
    ...
    function getPledgeKOGAmount(uint256 tokenId) public view returns(uint256){
        ...
        uint256 NFTTypes = TRN.NFTIDToTypes(tokenId);

        uint256 KOGAmount = BUSDEqualToToken(NFTPrice[NFTTypes - 1]);

        return KOGAmount;
    }

    function sellTaurenNFT(uint256 _bullPeriodIndex) public {

        uint256 tokenId = bullNFTTokenId[_bullPeriodIndex];

        // require(TRN.ownerOf(tokenId) == msg.sender, "You are not the owner");

        require(hammerOwner[_bullPeriodIndex].length >= 16, "This NFT has not been unlocked");
        ...
        TRN.transferFrom(msg.sender, bullNFTHolder, tokenId);

        ERC20(BUSD).transferFrom(castingPoolAddr, msg.sender, NFTPrice[1]);
    }
    ...
    function pledgeNFT(uint256 types, uint256 tokenId) public {
        ...
        uint256 tokenAmount = BUSDEqualToToken(NFTPrice[NFTTypes - 1]);

        if(types == 1){
            tokenIdToPledges[tokenId] = Pledge(tokenId,types.add(NFTTypes).sub(1).mul(10),block.timestamp);
        }
    ...
    function quitPledgeNFT(uint256 tokenId) public {
        ...
        require(tokenIdToPledges[tokenId].joinTime > 0, "Your card is not pledged");
    ...
    function getPledgeProfit(uint256 tokenId) public view returns(uint256){
        ...
        uint256 profit = tokenAmount.mul(pledgeTypesToRate[tokenIdToPledges[tokenId].types]).mul(pledgeDays).div(365);
    ...
    function setBuyFees(uint256 buyCastingPoolFee, uint256 buyOperationFee, uint256 buyCollapsePoolFee) external onlyOwner() {
        require(_buyCastingPoolFee != buyCastingPoolFee || _buyOperationFee != buyOperationFee || _buyCollapsePoolFee != buyCollapsePoolFee);

        if(_buyCastingPoolFee != buyCastingPoolFee){
            _buyCastingPoolFee = buyCastingPoolFee;
        }

        if(_buyOperationFee != buyOperationFee){
            _buyOperationFee = buyOperationFee;
        }

        if(_buyCollapsePoolFee != buyCollapsePoolFee){
            _buyCollapsePoolFee = buyCollapsePoolFee;
        }
        
        emit UpdateBuyFees(buyCastingPoolFee, buyOperationFee, buyCollapsePoolFee);
    }

    function setSellFees(uint256 sellCastingPoolFee, uint256 sellOperationFee, uint256 sellCollapsePoolFee, uint256 sellRepoPoolFee) external onlyOwner() {
        require(_sellCastingPoolFee != sellCastingPoolFee || _sellOperationFee != sellOperationFee || _sellCollapsePoolFee != sellCollapsePoolFee || _sellRepoPoolFee != sellRepoPoolFee);

        if(_sellCastingPoolFee != sellCastingPoolFee){
            _sellCastingPoolFee = sellCastingPoolFee;
        }

        if(_sellOperationFee != sellOperationFee){
            _sellOperationFee = sellOperationFee;
        }

        if(_sellCollapsePoolFee != sellCollapsePoolFee){
            _sellCollapsePoolFee = sellCollapsePoolFee;
        }

        if(_sellRepoPoolFee != sellRepoPoolFee){
            _sellRepoPoolFee = sellRepoPoolFee;
        }
        
        emit UpdateSellFees(sellCastingPoolFee, sellOperationFee, sellCollapsePoolFee, sellRepoPoolFee);
    }
    ...
    function _getBuyFeeValues(uint256 tAmount) private view returns (uint256 buyCastingPoolFee, uint256 buyOperationFee, uint256 buyCollapsePoolFee, uint256 tTransferAmount) {
        buyCastingPoolFee = tAmount.mul(_buyCastingPoolFee).div(10**2);
        buyOperationFee = tAmount.mul(_buyOperationFee).div(10**2);
        buyCollapsePoolFee = tAmount.mul(_buyCollapsePoolFee).div(10**2);

        // uint256 fee = tAmount.mul(_BNBFee + _liquidityFee).div(10**2);
        tTransferAmount = tAmount.sub(buyCastingPoolFee).sub(buyOperationFee).sub(buyCollapsePoolFee);
        // return tTransferAmount;
    }

    function _getSellFeeValues(uint256 tAmount) private view returns (uint256 sellCastingPoolFee, uint256 sellOperationFee, uint256 sellCollapsePoolFee, uint256 sellRepoPoolFee, uint256 tTransferAmount) {
        sellCastingPoolFee = tAmount.mul(_sellCastingPoolFee).div(10**2);
        sellOperationFee = tAmount.mul(_sellOperationFee).div(10**2);
        sellCollapsePoolFee = tAmount.mul(_sellCollapsePoolFee).div(10**2);
        sellRepoPoolFee = tAmount.mul(_sellRepoPoolFee).div(10**2);

        // uint256 fee = tAmount.mul(_BNBFee + _liquidityFee).div(10**2);
        tTransferAmount = tAmount.sub(sellCastingPoolFee).sub(sellOperationFee).sub(sellCollapsePoolFee).sub(sellRepoPoolFee);
        // return tTransferAmount;
    }
    ...
    //     if(_buyCastingPoolFee == 0 && _buyOperationFee == 0 && _buyCollapsePoolFee == 0) return;
    ...
    function _transfer(
        ...
        if(
            ...
            if(tokenOfPair > 0 && BUSDOfPair > 0 && amount >= tokenOfPair.mul(100).div(BUSDOfPair)){
                bullUsers[bullPeriodIndex].push(from);
                if(bullUsers[bullPeriodIndex].length >= periodUserNum){
                    activateBullNFT();
                }
            }
    ...
    function activateBullNFT() private {
        ...
        bullNFTTokenId[bullPeriodIndex] = tokenId;
    ...
    function swapAndCharge(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 liquidBalance = tokenBalance.mul(_buyCastingPoolFee).div(0 + 5).div(2);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, uint256 takeFee) private {
        ...
        if(takeFee == 1){
            (uint256 buyCastingPoolFee, uint256 buyOperationFee, uint256 buyCollapsePoolFee, uint256 tTransferAmount) = _getBuyFeeValues(amount);
            _balances[sender] = _balances[sender].sub(amount);
            _balances[recipient] = _balances[recipient].add(tTransferAmount);
            _balances[castingPoolAddr] = _balances[castingPoolAddr].add(buyCastingPoolFee);
            _balances[operationAddr] = _balances[operationAddr].add(buyOperationFee);
            _balances[collapsePoolAddr] = _balances[collapsePoolAddr].add(buyCollapsePoolFee);
        ...
        if(takeFee == 2){
            (uint256 sellCastingPoolFee, uint256 sellOperationFee, uint256 sellCollapsePoolFee, uint256 sellRepoPoolFee, uint256 tTransferAmount) = _getSellFeeValues(amount);
            _balances[sender] = _balances[sender].sub(amount);
            _balances[recipient] = _balances[recipient].add(tTransferAmount);
            _balances[castingPoolAddr] = _balances[castingPoolAddr].add(sellCastingPoolFee);
            _balances[operationAddr] = _balances[operationAddr].add(sellOperationFee);
            _balances[collapsePoolAddr] = _balances[collapsePoolAddr].add(sellCollapsePoolFee);
            _balances[repoPoolAddr] = _balances[repoPoolAddr].add(sellRepoPoolFee);
