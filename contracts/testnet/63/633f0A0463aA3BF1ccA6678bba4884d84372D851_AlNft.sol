...
contract AlNft is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    function setMaxSupply(uint256 _mx) public onlyOwner {
      maxSupply = _mx;
    }
    ...
    {
      uint8 id = 1;
      require(totalClaimableNft() <= maxSupply, "[ALNFT] All NFTs are minted!");
      require(pause == false, "[ALNFT] Temporarily paused to mint!");
      uint256 cost = mintPrices[stage];
      require(usdt.balanceOf(msg.sender) >= cost, "[ALNFT] Your balance is insufficient balance to mint NFT!");
    ...
    function _beforeTokenTransfer(
        ...
        for(uint i = 0; i < ids.length; i ++) {
          ...
          if (to != address(0)) {
            ...
            pendings[to][id].count += amounts[i];
          }
        }
    }
