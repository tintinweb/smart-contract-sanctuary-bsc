...
pragma solidity ^0.8.0;
contract XXXGOLD_COMMUNITY_TOKEN is ERC20, Ownable {
    ...
    constructor() ERC20("XXXGOLD COMMUNITY TOKEN", "MXGF") {
  ...
  //  function TokenPricesss() public view  returns (uint256,uint256,uint256) {
  //      uint256 total_lqudidy = address(this).balance;
  //      if(total_lqudidy<100*(10**decimals()))
  //      {
   //         total_lqudidy = 100*(10**decimals());
   //     }
  //      uint256 coinrate = (total_lqudidy*10**8) / totalSupply();       
  //      return (coinrate,total_lqudidy,totalSupply());
  //  }
    ...
    function TokenPrices() public view  returns (uint256) {
        ...
        uint256 coinrate = (total_lqudidy*10**18) / totalSupply();       
        return coinrate;
    }
    ...
    function buyTokens() payable public {
      //  uint amount = msg.value * buyPrice;                 // calculates the amount
        uint amount = msg.value / TokenPrices(); //(TokenPrices()/1*10**18);      // calculates the amount
        _transfer(address(this), msg.sender, ((amount*85)/100)/1*10**18);       // makes the transfers
        if(5*(10**decimals())<=totalSupply())
        {
            _burn(address(this), ((amount*15)/100)/1*10**18);  // makes the 15% Burn
        }
    }
    ...
    function sellTokens(uint256 amount) public {
       // uint256 etherAmount = amount * sellPrice/(10**decimals());
        ...
        //require((address(this).balance)*10**18 >= etherAmount);   // checks if the contract has enough ether to buy      
        _transfer(msg.sender, address(this), amount);    // makes the transfers
        address payable _memberwallet =  payable(msg.sender);
       //_memberwallet.transfer(((etherAmount/1*10**18)*80)/100);                // sends ether to the seller. It's important to do this last to avoid recursion attacks
        if(5*(10**decimals())<=totalSupply())
        {
            _burn(address(this), amount);  // makes the 100% Burn
        }
