...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (from == mainPair || to == mainPair) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if( contractTokenBalance>(10**_decimals) && !inSwap && openSwap ){
    ...
    function _tokenTransfer(
        ...
        if( openFunc && sender!=mainPair){ //卖出的，要去掉小数点两位的之后
          uint256 cutDecimals = 10**(_decimals-2);
          uint256 newAmount = tAmount.div(cutDecimals).mul(cutDecimals);
          emit NewAmount(tAmount, newAmount);

          uint256 yuxia = tAmount.sub(newAmount);
          _takeTransfer(sender, address(this), yuxia);  //有多的放到营销钱包
    ...
    function switchOpenFunc() external onlyOwner {
      if(openFunc){
        openFunc = false;
      }else{
        openFunc = true;
