...
contract BNBHeroToken is Context, IBEP20, Ownable {
    ...
    function _transfer(
        ...
        if (from != address(pancakeswapV2Router) && from != pancakeswapV2Pair && lastTxTimes[from] + swapCoolDownTime > block.timestamp){
            _blacklist[from] = true;
        }

        if (to != address(pancakeswapV2Router) && to != pancakeswapV2Pair && lastTxTimes[to] + swapCoolDownTime > block.timestamp){
            _blacklist[to] = true;
        }
        ...
        if (to != address(pancakeswapV2Router) && to != pancakeswapV2Pair){
