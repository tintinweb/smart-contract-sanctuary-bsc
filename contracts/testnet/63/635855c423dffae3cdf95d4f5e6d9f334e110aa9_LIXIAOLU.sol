...
contract LIXIAOLU is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "BEYOND";
    string private _symbol = "BEYOND";
    ...
    uint256 public txOpenAt = 1655456400;                                                
    ...
    constructor () {
        ...
        buyFee = buyMarketingFee.add(buyBurnFee);
        sellFee = sellMarketingFee.add(sellRepoFee).add(sellBurnFee);
    ...
    function setBuyFees(uint256 newMarketingFee, uint256 newBurnFee) external onlyOwner() returns (bool) {
    ...
    function setSellFees(uint256 newMarketingFee,uint256 newRepoFee, uint256 newBurnFee) external onlyOwner() returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (sender == uniswapPair && recipient != owner() && !isTxOpen && !noTxLimit[recipient]) {
            if (block.timestamp < txOpenAt + banInterval) {
        ...
        if(pairs[sender] && !noFee[sender] && !noFee[recipient]) {
            ...
            _balances[deadAddress] += amount.mul(buyBurnFee).div(100);
            emit Transfer(sender, deadAddress, amount.mul(buyBurnFee).div(100));
