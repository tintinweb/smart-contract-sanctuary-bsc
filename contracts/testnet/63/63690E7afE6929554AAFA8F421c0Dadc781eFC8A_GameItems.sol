...
contract GameItems is ERC1155 {
    ...
    function Claim(uint256 nftId,uint256 seed,address user,uint8 v,bytes32 r,bytes32 s) external
    {
        require(isClaimed[msg.sender][nftId]==false,"Allready claimed");
        // bytes32 hash = keccak256(abi.encode(nftId,seed,user));
         string memory hash = keccak256(abi.encode(nftId,seed,user)).toString();
        address signer = hash.recover(v, r, s);
        require(signer == authAddress, "Invalid signature");
        _mint(msg.sender, nftId,1,"0x");
        isClaimed[msg.sender][nftId] = true;
    }

    function GetSigner(uint256 nftId,uint256 seed,address user,uint8 v,bytes32 r,bytes32 s) external view returns(address)
    {
        require(isClaimed[msg.sender][nftId]==false,"Allready claimed");
        // bytes32 hash = keccak256(abi.encode(nftId,seed,user));
         string memory hash = keccak256(abi.encode(nftId,seed,user)).toString();
    ...
    function GetHash(uint256 nftId,uint256 seed,address user) external view returns(string memory)
    {
        require(isClaimed[msg.sender][nftId]==false,"Allready claimed");
        // bytes32 hash = keccak256(abi.encode(nftId,seed,user));
         string memory hash = keccak256(abi.encode(nftId,seed,user)).toString();
