...
interface OtherGameContractsInterface {
    ...
    function play(uint8 _bet, address player, uint256 _value, uint256 txIdentifier) external payable;
}
...
interface ReferralsContractInterface {
    ...
    function addCalculatedFatherFee(address _childReferral, uint256 _amount, uint256 txIdentifier)
        external
        payable;
}
    ...
    ReentrancyGuardUpgradeable
{
        ...
        nonReentrant
    {
        ...
        uint256 preSeed = uint256(keccak256(abi.encode(keyHash, game.gameAddress, _newNonce)));
        uint256 txIdentifier = uint256(keccak256(abi.encode(keyHash, preSeed)));
        sendFeesAndReferrals((msg.value * fee) / 10_000, msg.value, msg.sender, txIdentifier);
        otherContract.play{value: newAmount}(_bet, msg.sender, msg.value, txIdentifier);
    ...
    function sendFeesAndReferrals(uint256 _amount, uint256 _sentAmount, address _caller, uint256 _txIdentifier)
        internal
    {   
        ...
        if (_referralFeeAmount != 0) {
            ...
            }(_caller, _sentAmount, _txIdentifier);
