...
contract nft_trade is ERC721Holder, ERC1155Holder, Ownable, Pausable {
    ...
    function takeOffer(uint256 _offerId) external whenNotPaused {
        ...
        if (_fee > 0) {
            ...
            minter(feeReceiverContract).payReward(fee,offers[_offerId].assetId);
        }

        uint _type = minter(feeReceiverContract).tokenType(offers[_offerId].assetId);
        poolInterface(poolContractAddress).removeToken(_type);
