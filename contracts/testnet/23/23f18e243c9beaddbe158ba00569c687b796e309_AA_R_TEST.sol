...
contract AA_R_TEST is Context, IERC20 { 
 ...
 //   _excluded.push(address(this)); // XXXX 
    ...
    */
    ...
    function Processing_Remove_Tokens(address Token_Address, uint256 Percent_To_Remove) external onlyOwner returns(bool _sent){
            ...
            require (Token_Address != address(this), "You can not remove native token, must be processed via SwapAndLiquify");

            // Can not purge the liquidity token!
            require (Token_Address != uniswapV2Pair, "You can not remove the liquidity token");
  ...
  uint256 public Liquidity_Unlock_Time;
    // XX needs only owner!
    function Liquidity_Set_Lock() external {
    ...
    function Liquidity_Lock_Days_Remaining() external view returns(uint256 Days_Remaining)  {
 ...
 function Liquidity_Tokens_Total() external view returns(uint256) {
        return IERC20(uniswapV2Pair).totalSupply();
    }



 function Liquidity_Tokens_on_Contract() external view returns(uint256) {
        return IERC20(uniswapV2Pair).balanceOf(address(this));
    }


 function Liquidity_Percent_on_Contract() external view returns(uint256) {


        uint256 OnContract = IERC20(uniswapV2Pair).balanceOf(address(this));
        uint256 inTotal =  IERC20(uniswapV2Pair).totalSupply();

        return (inTotal * OnContract / 100);
