...
contract AirdropSystem is Whitelist, Pausable {
    ...
    string public teamNameDefault;
    string public teamImageBaseURI;
    ...
    constructor (address _woolToken, address _woolshed) {
        ...
        teamNameDefault = "DegenTeam";
        teamImageBaseURI = "https://degenprotocol.io/assets/img/logo.png";
    }
    ...
    function teamDataOf(address _addr) public view returns (string memory teamName, string memory teamImage, uint256 createDate) {
        teamName = _users[_addr].name;
        teamImage = _users[_addr].image;
        createDate = _users[_addr].created;
    }
    ...
    // Update the name of the sender
    function setName(string memory _name) public returns (bool) {
        return _setNameOf(msg.sender, _name);
    }

    // Update the image of the sender
    function setImage(string memory _url) public returns (bool) {
        return _setImageOf(msg.sender, _url);
    }
    ...
    // Reset Name (in case of requirement)
    function resetName(address _user) public onlyWhitelisted() returns (bool _success) {
        return _setNameOf(_user, teamNameDefault);
    }

    // Reset Image (in case of requirement)
    function resetImage(address _user) public onlyWhitelisted() returns (bool _success) {
        return _setImageOf(_user, teamImageBaseURI);
    }
    ...
    // Unban an address from the airdrop system
    ...
    function _teamAirdrop(address[] memory recipients_, uint256 amount_, uint256 minBalance_, uint256 maxBalance_) internal returns (bool _success) {
        ...
        for(uint256 i = 0; i < recipients_.length; i ++) {
            ...
            bool _isEligible = checkEligibility(_recipient, amount_, minBalance_, maxBalance_);
    ...
    function _setImageOf(address _team, string memory _url) internal returns (bool _success) {
        _users[_team].image = _url;
        return true;
    }

    function _setNameOf(address _team, string memory _name) internal returns (bool _success) {
        _users[_team].name = _name;
        return true;
    }
}