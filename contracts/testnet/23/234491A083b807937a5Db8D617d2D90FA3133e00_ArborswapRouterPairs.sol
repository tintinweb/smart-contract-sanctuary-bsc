...
contract ArborswapRouterPairs is IArborswapRouterPairs, Ownable {
    ...
    modifier validFactory(address factory) {
        require(hasFactory(factory), 'ArborswapRouterPairs: INVALID_FACTORY');
        _;
    }

    constructor(address _aggregator) {
        require(_aggregator != address(0), "ArborswapRouterPairs: ZERO_ADDRESS");
    ...
    {
        return ArborswapLibrary.pairFor(factory, getInitHash(factory), tokenA, tokenB);
    }
    function getReserves(address factory, address tokenA, address tokenB) 
        external view override returns (uint256 reserveA, uint256 reserveB)
    {
        return ArborswapLibrary.getReserves(factory, getInitHash(factory), tokenA, tokenB);
    }
    ...
    {
        return ArborswapLibrary.getAmountOut(feeAggregator, tokenIn, false, amountIn, reserveIn, reserveOut, getLPFee(factory));
    }
    ...
    {
        return ArborswapLibrary.getAmountOut(feeAggregator, tokenIn, feePayed, amountIn, reserveIn, reserveOut, getLPFee(factory));
    }
    ...
    {
        return ArborswapLibrary.getAmountIn(feeAggregator, tokenOut, false, amountOut, reserveIn, reserveOut, getLPFee(factory));
    }
    ...
    {
        return ArborswapLibrary.getAmountIn(feeAggregator, tokenOut, feePayed, amountOut, reserveIn, reserveOut, getLPFee(factory));
    }
        ...
        external view override returns (uint256[] memory amounts, uint256 feeAmount, address feeToken) 
    {
        ...
        return ArborswapLibrary.getAmountsOut(feeAggregator, _factories, hashes, amountIn, path, fees);
    }
    function getAmountsIn(address[] calldata _factories, uint256 amountOut, address[] calldata path)
        external view override returns (uint256[] memory amounts, uint256 feeAmount, address feeToken) 
    {
        (bytes32[] memory hashes, uint256[] memory fees) = getInitHashesAndFees(_factories);
        return ArborswapLibrary.getAmountsIn(feeAggregator, _factories, hashes, amountOut, path, fees);
    }
    ...
    function setFeeAggregator(address aggregator) external override onlyMultiSig {
        require(aggregator != address(0), "ArborswapRouterPairs: FEE_AGGREGATOR_NO_ADDRESS");
    ...
    function setFactory(address _factory, bytes32 initHash) external override onlyMultiSig returns (bool) {
        require(_factory != address(0), "ArborswapRouterPairs: FACTORY_NO_ADDRESS");
