...
contract WoolShed is Ownable, ReentrancyGuard {
    ...
    ILocker private tokenLocker; // Token Locker
    ...
    event onHeartbeat(address indexed addr, uint256 timestamp);
    ...
    function checkin() public {
        ...
        emit onCheckIn(_addr, custody[_addr].last_checkin);
    }
    ...
    function airdrop(address _to, uint256 _amount) external {
        ...
        require(gameToken.transferFrom( _addr, address(fundVault), _amount),
    ...
    function payoutOf(address _addr) public view returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
        ...
        // Get payout rate as calculated by TokenLocker values and holdings
        uint256 userPayoutRate = tokenLocker.payoutRateOf(msg.sender);

        if(users[_addr].payouts < max_payout) {

            //Using 1e18 we capture all significant digits when calculating available divs
            share = users[_addr].deposits.mul(userPayoutRate * 1e18).div(100e18).div(24 hours); //divide the profit by payout rate and seconds in the day
    ...
    function _heart(address _addr) internal {
        ...
        emit onHeartbeat(_addr, custody[_addr].last_heartbeat);
    }
    ...
    function isTeamBalanceCovered(address _addr, uint8 _level) private view returns (bool) {
        ...
        return tokenLocker.teamBalanceLevel(_addr) >= _level;
    }
    ...
    function isRateBalanceCovered(address _addr, uint8 _level) private view returns (bool) {
        ...
        return tokenLocker.rateBalanceLevel(_addr) >= _level;
    }
}