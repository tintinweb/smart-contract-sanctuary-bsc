...
contract SportFantansyPrediction is Ownable{
    ...
    event MatchCreated(uint256 matchId, uint256 timeStart);
    ...
    function getAllMatchs() external view returns(Match[] memory)  {
        ...
        for(i; i < matchCount; i++){
            ...
            allMatchs[i] = Match(index,matchId,isEnded,startTime,result);
        }
    ...
    function getCurrentMatchs() external view returns(Match[] memory)  {
        ...
        for(i; i < matchCurrentCount; i++){
            ...
            currentMatchs[i] = Match(index,matchId,isEnded,startTime,result);
        }
    ...
    function getEndedMatchs() external view returns(Match[] memory){
        ...
        for(i; i < matchEndedCount; i++){
            ...
            endedMatchs[i] = Match(index,matchId,isEnded,startTime,result);
        }
    ...
    function addMatch(uint256 _timeStart) public onlyOwner{
        require(_timeStart > block.timestamp, "time error");
        ...
        emit MatchCreated(matchId,_timeStart);
    }
