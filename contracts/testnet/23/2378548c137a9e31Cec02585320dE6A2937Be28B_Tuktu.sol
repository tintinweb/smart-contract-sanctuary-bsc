...
library SmartMatrix {
	...
	function InitNewNodeToMatrixes(Node storage _Node, mapping(uint256 => SmartMatrix.Node) storage _Nodes) internal {
		uint8 Unilevel = 0; // Unilevel matrix (Sun)
		uint8 Binary = 1; // Binary marix (Tow leg)
		uint8 Ternary = 2; // Ternary matrix (Three leg)
	...
	function UpdateSponsorLevel(Node storage _Node, mapping(uint256 => SmartMatrix.Node) storage _Nodes) internal {
		uint8 Unilevel = 0; // Unilevel matrix (Sponsor matrix)
	...
	function SelectAllChildrenNodeIDInNode(
		...
		_NodeIDs[0] = _Node.NodeID;
		uint256[] memory temp1 = _NodeIDs;
	...
	function SelectAllChildrenNodeIDInNodeReturnTreeviewToFrontend(
		...
		mapping(uint256 => SmartMatrix.Node) storage _Nodes,
		uint8 _Matrix
	) internal view returns (TreeView[] memory _TernaryTreeview) {
		(uint256[] memory NodeIDs, uint256 NodeCount) = SelectAllChildrenNodeIDInNode(_Node, _Nodes, _Matrix);
		for (uint256 i = 0; i < NodeCount; i++) {
			_TernaryTreeview[i] = TreeView({
...
abstract contract TuktuBase {
	...
	struct Account {
		uint256 AccountID;
		string Affiliate; // For share link. User can modify
	...
	mapping(string => uint256) Affiliates; // To AccountID. Affiliates is used as AccountID
	...
	function _GetNewAffiliate() internal view returns (string memory Affiliate) {
		uint i;
		while (true) {
			Affiliate = string(
				abi.encode(bytes10(keccak256(abi.encodePacked(msg.sender, block.difficulty, block.timestamp, i++))))
			);
	...
	function _ChangeAffiliate(uint256 _AccountID, string memory _NewAffiliate) internal {
		string memory oldaffiliate = Accounts[_AccountID].Affiliate;
		require(
	...
	function _SelectAllChildrenNodesOfAccountOnMatrix(
		...
		if (Nodes[_AccountID].NodeID == _AccountID)
...
contract Tuktu is TuktuBase, ITuktu {
	...
	function Registration(string memory _Affiliate) public {
		Affiliates[_Affiliate] == Accounts[Affiliates[_Affiliate]].AccountID
			? _registration(msg.sender, Affiliates[_Affiliate])
			: _registration(msg.sender);
	}

	function Registration(uint256 _SponsorID) public {
		Accounts[_SponsorID].AccountID == _SponsorID ? _registration(msg.sender, _SponsorID) : _registration(msg.sender);
	}
	...
	function Registration(address _AddressOfNewAccount, uint256 _SponsorID) public {
		...
		Accounts[_SponsorID].AccountID == _SponsorID
	...
	function ChangeAffiliate(uint256 _AccountID, string memory _NewAffiliate) public onlyAccountOwner(_AccountID) {
		_ChangeAffiliate(_AccountID, _NewAffiliate);
	}
