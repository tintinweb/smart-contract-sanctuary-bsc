...
contract bitcityzLaunchpool is Ownable {
    ...
    function _giveTicket(UserInfo memory user, PoolInfo memory pool, uint timeSpend) internal {
        uint n = timeSpend / pool.lockingTime;
        for(uint i = 0; i < n; i++) {
    ...
    function unstake(uint _pid) external {
        ...
        if(timeSpend < pool.lockingTime) {
            ...
            _giveTicket(user, pool, timeSpend);
        }

        pool.lockingToken.transfer(_msgSender(), user.amount - fee);
        user.amount = 0;
        user.startTime += timeSpend % pool.lockingTime;
