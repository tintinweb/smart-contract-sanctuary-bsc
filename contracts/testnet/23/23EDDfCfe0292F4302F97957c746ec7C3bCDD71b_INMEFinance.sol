...
contract INMEFinance is IERC20 {
    ...
    string constant private _name = "INME Finance";
    string constant private _symbol = "INMER";
    ...
    struct TaxWallets {
        ...
        address payable competition;
        address liquidity;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xA3F339d08f361b8b557494330A218F78B7EF5E09),
        competition: payable(0xBd305d03044093208956bF2641b103133Df8c342),
        liquidity: 0x8BB98dE3Fd2E30E1af7f3626C528C8192a9db4a0
        });
    ...
    bool rewardsLoadBuy = true;
    bool rewardsLoadSell = true;
    bool rewardsLoadTransfer = true;
...
//===============================================================================================================
    ...
    function setInitializers(address aInitializer, address cInitializer) external onlyOwner {
        ...
        _isExcludedFromFees[cInitializer];
    }
    ...
    function setWallets(address payable marketing, address payable liquidity, address payable competition) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.liquidity = payable(liquidity);
        _taxWallets.competition = payable(competition);
    }
    ...
    function setRatios(uint16 rewards, uint16 liquidity, uint16 marketing, uint16 competition) public onlyOwner {
        _ratios.rewards = rewards;
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.competition = competition;
        _ratios.total = liquidity + marketing + competition;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.total + _ratios.rewards <= total, "Cannot exceed sum of buy and sell fees.");
    }
    ...
    function processRewards(address from, address to, bool other) internal {
    ...
    function takeTaxes(address from, uint256 amount, bool buy, bool sell, bool other) internal returns (uint256) {
        ...
        uint256 rewardsAmount = (feeAmount * _ratios.rewards) / (_ratios.rewards + _ratios.total);
        uint256 swapAmt = feeAmount - rewardsAmount;

        if (rewardsAmount > 0) {
            _tOwned[address(reflector)] += rewardsAmount;
            try reflector.load(rewardsAmount) {} catch {}
            emit Transfer(from, address(reflector), rewardsAmount);
        }
    ...
    function contractSwap(uint256 contractTokenBalance) internal swapping {
        ...
        bool success;
        uint256 competitionBalance = (amtBalance * ratios.competition) / ratios.total;
        uint256 marketingBalance = amtBalance - (competitionBalance);

        if(ratios.marketing > 0){
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: 35000}("");
        }
        if(ratios.competition > 0){
            (success,) = _taxWallets.competition.call{value: competitionBalance, gas: 35000}("");
        }
    }

    function manualLoad(uint256 tokenAmount) external {
        address user = msg.sender;
        uint256 amount = tokenAmount*10**_decimals;
        require(balanceOf(msg.sender) >= amount, "You do not have enough tokens to load this.");
        _basicTransfer(msg.sender, address(reflector), amount);
        if (!_isExcludedFromDividends[user]) {
            try reflector.tally(user, _tOwned[user]) {} catch {}
        }
