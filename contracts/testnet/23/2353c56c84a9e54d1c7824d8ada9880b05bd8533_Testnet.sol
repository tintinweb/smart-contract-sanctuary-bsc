...
contract Testnet is Context, IERC20, Ownable {
    ...
    address payable public feeAddress = payable(0x32cf981E1F97809F0c4b158d97b74A0764dfa4bA); 
    // address payable public usdtAddress = payable(0x55d398326f99059fF775485246999027B3197955);
    address payable public usdtAddress = payable(0x337610d27c682E347C9cD60BD4b3b107C9d34dDd);
    ...
    string private constant _name = "TESNET";
    string private constant _symbol = "TESTNET";
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) 
        {
            if (to == uniswapV2Pair) 
            {
                if (overMinimumTokenBalance) 
                {
                    ...
                    uint256 marketingAndDevelopmentToken = contractTokenBalance.mul(_sellMarketingFee).div(_sellMarketingFee);
                    // Token Swapping in BNB for usdt
                    swapTokens(marketingAndDevelopmentToken);    
                    // Fetch balance of the token swapped for BNB for usdt
                    uint256 bnbBalusdt = address(this).balance;
                    // Swap BNB for USDT and send to All Address
                    SwapAndLiquifyBNBtoUsdt(bnbBalusdt);
    ...
    function swapBNBForTokensToUsdt(uint256 amount) private lockTheSwap{
        // Generate the uniswap pair path of token -> WBNB
        address[] memory path = new address[](2);
    ...
    function manualSwapAndLiquifyBNBtoUsdt(uint256 bnbLiquifyAmount) external lockTheSwap onlyOwner {
        SwapAndLiquifyBNBtoUsdt(bnbLiquifyAmount);
    }
    ...
    function SwapAndLiquifyBNBtoUsdt(uint256 bnbLiquifyAmount) private lockTheSwap {
        // swap BNB for Tokens
        swapBNBForTokensToUsdt(bnbLiquifyAmount); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered
        // how much Tokens did we just swap into?
        uint256 newTokenBalance = IERC20(usdtAddress).balanceOf(address(this));

        uint256 marketingFunds = newTokenBalance.mul(_sellMarketingFee).div(_sellMarketingFee);

        require(IERC20(usdtAddress).transfer(feeAddress,marketingFunds));
