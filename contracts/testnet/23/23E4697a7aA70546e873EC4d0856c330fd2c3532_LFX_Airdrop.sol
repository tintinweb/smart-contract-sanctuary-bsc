...
contract LFX_Airdrop is Ownable, ReentrancyGuard {
    ILFX token;
    ...
    function initializeAirDrop(
        ...
        require(
            _airdropToken >= amountPerAddress,
            "amount should be greater than per user"
        );
    ...
    function claimToken() public onlyActive {
        ...
        require(
            token.balanceOf(address(this)) >= amountPerAddress,
    ...
    function updateTokenAdress(ILFX newTokenAdres) public onlyOwner {
        require(msg.sender == _owner, "only owner can call");
        token = newTokenAdres;
    }
