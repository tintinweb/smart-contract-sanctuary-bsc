    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        // init
        pancakeSwapRouter02Address = _routerAddress;

        IPancakeRouter02 _pancakeswapV2Router = IPancakeRouter02(
            _routerAddress
        );
        pancakeSwapV2Router = _pancakeswapV2Router;
        address WBNB = _pancakeswapV2Router.WETH();
        pancakeSwapPair = address(
            IPancakeFactory(_pancakeswapV2Router.factory()).createPair(
                WBNB,
                _rftx
            )
        );
    ...
    function setPancakeSwapRouter02Address(address _address) public onlyOwner {
        require(
            _address != address(pancakeSwapRouter02Address),
            "RFTX: already has that address"
        );
    ...
    function _addLiquidity(Tax memory buyTax, Tax memory sellTax, Tax memory tax, uint256 tokenAmount) internal {
        ...
        uint256 liquidityRFTX = (buyTax.liquidity + sellTax.liquidity + tax.liquidity)
            .mul(tokenAmount)
            .div(totalPerc);

        //swapTokensForEth
        uint256 liquidityBNB = swapTokensForEth(rematicAddress, liquidityRFTX.div(2));
        uint256 liquidityToken = liquidityRFTX.sub(liquidityRFTX.div(2));

        // approve token transfer to cover all possible scenarios
        IERC20(rematicAddress).approve(
            pancakeSwapRouter02Address,
            liquidityToken
        );
    ...
    function setDefaultTokenTracker(address _address) public onlyOwner {
        require(
            ...
            "RFTX Admin: The defaultTokenTracker already has that address"
        );
    ...
    function excludeContractAddressesFromDividendTracker() public onlyOwner {
        ...
        IDefaultTracker(defaultTokenTracker)
            ._excludeFromDividendsByAdminContract(pancakeSwapRouter02Address);
        IDefaultTracker(defaultTokenTracker)
            ._excludeFromDividendsByAdminContract(pancakeSwapPair);
    ...
    function setFour01programForDefaultTokenTracker() public onlyOwner {
        IDefaultTracker(defaultTokenTracker).setFour01Programe(
            four01program,
            four01TeamWalletAddress
        );
    }
    ...
    function _getRFTXAmountForBNB(Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal pure returns(uint256 bnbRFTX){
        bnbRFTX = buyTax.pension
        + sellTax.pension
        + tax.pension
        + buyTax.legal
        + sellTax.legal
    ...
    function _distributeBNB(Tax memory buyTax, Tax memory sellTax, Tax memory tax, uint256 tokenAmount) internal {
            ...
            uint256 totalPerc =_getTotalPercentage(buyTax, sellTax, tax);
            uint256 bnbPec = _getRFTXAmountForBNB(buyTax, sellTax, tax);
            if(bnbPec > 0){
                uint256 bnbRFTX = tokenAmount.mul(bnbPec).div(totalPerc);
                uint256 bnbAmount = swapTokensForEth(rematicAddress, bnbRFTX);

                //send some rewardBNB to Pension wallet
                uint256 pAmount = bnbAmount * (buyTax.pension + sellTax.pension + tax.pension) / bnbRFTX;
                _sendBNBToPensionWallet(pAmount);

                //send some rewardBNB to Team wallet
                uint256 tAmount = bnbAmount * (buyTax.team + sellTax.team + tax.team) / bnbRFTX;
    ...
    function _distibuteREWARD (Tax memory buyTax, Tax memory sellTax, Tax memory tax, uint256 tokenAmount) internal {
        ...
        uint256 pRewardRFTX = buyTax.partition + sellTax.partition +tax.partition + buyTax.k401 + sellTax.k401 + tax.k401;
