...
contract DeoniCrowsale1 is Crowdsale,Ownable {
    ...
    constructor(
        ...
        uint256 maxCap,    // Max Contributions
        uint256 rate_,    // rate in TKNbits
        address payable wallet,
        uint256 cap,
        ERC20 token
    )
        Crowdsale(rate_, wallet, token, cap)
    {
        _minCap = minCap;
        _maxCap = maxCap;
        _rate = rate_;
    ...
    // function buyTokens(address beneficiary) public virtual override nonReentrant payable {
    ...
    // function weiRaised() public view virtual override returns (uint256) {
    //     return _weiRaised;
    // }
    ...
    function _preValidatePurchase(address _beneficiary, uint256 _weiAmount) internal override {
        ...
        require(_newContribution >= _minCap && _newContribution <= _maxCap);
    ...
    function setCrowdsaleStage(uint _stage, uint256 rate_) public onlyOwner {
...
abstract contract Crowdsale is Context, ReentrancyGuard {
    ...
    constructor (uint256 rate_, address payable wallet_, ERC20 token_, uint256 cap_) {
