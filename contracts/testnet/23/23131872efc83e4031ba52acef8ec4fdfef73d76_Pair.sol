...
contract Pair is Ownable{
    ...
    function swap(uint amountIn, address[] memory path, address to) external lock {
        require(swapSwitch,"Pair:NOT_OPENED");
        ...
        //buy
            ...
            userBuyAmount[to] = userBuyAmount[to].add(amountOut);
            require( userBuyAmount[to] <= maxBuyAmount,"Pairs: AMOUNT_MAX");
         ...
         //sell
            ...
            if(userBuyAmount[msg.sender] < amountIn){
                userBuyAmount[msg.sender] = 0;
            }else{
            userBuyAmount[msg.sender] = userBuyAmount[msg.sender].sub(amountIn);
            }
            uint256 userBal = IERC20(token1).balanceOf(to);
            require( amountIn <= userBal.mul(maxSellRate).div(10000),"Pairs: AMOUNT_MAX");
		}
    ...
    function setSwapSwitch(bool _swapSwitch)public onlyOwner{
        swapSwitch = _swapSwitch;
    }

    function setBuyAndSell(uint256 _maxBuyAmount, uint256 _maxSellRate)public onlyOwner{
        maxBuyAmount = _maxBuyAmount;
        maxSellRate = _maxSellRate;
    }
