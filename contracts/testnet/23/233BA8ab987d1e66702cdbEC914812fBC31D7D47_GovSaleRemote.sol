...
contract GovSaleRemote is
    ...
    function getPayload(
        ...
        payload = abi.encode(address(this), targetSale, invoiceIndex, _amountIn, _buyer);
    }
    ...
    function buyToken(uint128 _amountIn) external payable whenNotPaused nonReentrant {
        ...
        if(feeIn > feeNeeded) {
            (bool success1,) = payable(sender).call{value: feeIn - fees}("");
            require(success1, "fail#1");
        }

        payment.safeTransferFrom(sender, address(this), _amountIn + feeCharged);

        (bool success2,) = factory.crossChainManagerRemote().call{value: feeNeeded}(
            abi.encodeWithSelector(ICrossChainManagerRemote.buyToken.selector, getPayload(_amountIn, sender), getAdapterParams(), tax)
        );
        require(success2, "fail#2");
    }
    ...
    function receiveCall(bytes calldata _payload) external {
        ...
        (
            ...
            uint128 bought,
            bool chance
        ) = abi.decode(_payload, (address, address, uint32, uint256, uint128, uint128, bool));
