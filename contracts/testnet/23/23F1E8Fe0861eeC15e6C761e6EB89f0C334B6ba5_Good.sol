...
contract Good is Ownable, ReentrancyGuard {
    ...
    address public constant DEVELOPER =
        0xccD42484785ef5Df06713e457a0f6B5D56d44FE2;
    ...
    event addressBonus(address indexed lastAddressBonus, uint256 amountBonus);

    
    IERC20 public immutable token;

    constructor() {
        address tokenAddress = 0xA2f7BeCA9E5ea15fd33be37e098B91c426908563;
        ...
        alreadyInit
    {
        ...
        emit depUser(
            lastID,
            msg.sender,
            users[msg.sender].amountDeposit,
            _amount
        );
    }
    ...
    function bonusTransfer() internal {
        if (lastTimeDeposit + 1 hours < block.timestamp) {
            token.transfer(lastAddressDeposit, bonus);

            emit addressBonus(lastAddressDeposit, bonus);
    ...
    function manualTransfer(address _user) external noReentrant alreadyInit {
        uint256 timestampUser = users[_user].timestamp;
        require(timestampUser != 0, "No deposit");
        require(
            timestampUser + 14 days < block.timestamp,
            "14 days have not yet passed"
        );
        
        uint256 depositUser = users[_user].deposit;
        token.transfer(_user, depositUser);

        
        delete users[_user].deposit;
    }
