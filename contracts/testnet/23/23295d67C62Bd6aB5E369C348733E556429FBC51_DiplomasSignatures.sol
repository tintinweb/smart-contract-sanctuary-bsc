...
contract DiplomasSignatures {
    ...
    function getSignedStatus(uint layer, uint index) public view returns(bool) {
        return _signed[layer][index];
    }

    function getAddressOfSigner(uint layer, uint index) public view returns(address) {
        return _addresses[layer][index];
    }

    function getSignerNumber(uint layer) public view returns (uint) {
        return _signed[layer].length;
    }
