...
contract GoRunnerIDO is ERC20, ERC20Pausable, Ownable, ReentrancyGuard {
    ...
    */
    function buy() payable nonReentrant onlyEOA external returns (bool success) {
        ...
        require(totalContributedAmount + msg.value <= 1 ether, 'GoRunnerIDO: max contribution amount 100 BNB reached');
        ...
        require(user.contributedAmount+msg.value<=1, "GoRunnerIDO: max contribution amount 1 BNB per user reached'");
        if(!user.contributed){
    ...
    function setUnlockTimes(uint256 claimTimes_) whenPaused public onlyOwner {
        claimTimes = claimTimes_;
    }

    function updateUserClaimable(address sender, bool data) internal {
        UserInfo storage user = contributors[sender];
        if(claimTimes == 1){
            user.firstClaimable = data;
        }else if(claimTimes == 2){
            user.secondClaimable = data;
        }
    }
