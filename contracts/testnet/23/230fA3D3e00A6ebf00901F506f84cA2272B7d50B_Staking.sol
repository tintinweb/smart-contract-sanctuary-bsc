...
/**
    ...
    function claimReward(uint256 campaignId) external {
        require(userAwardedCollections[msg.sender][campaignId].length > 0, "no rewards");
    ...
    function getClaimableTickets() external view returns(uint256) {
        return totalUserXTickets[msg.sender][latestCampaignId];
    }
    ...
    function _getClaimableXTickets(address _stakeOwner, uint256 _stakeId) private returns(uint256) {
        ...
        uint256 newTickets = totalUserXTickets[msg.sender][latestCampaignId] == 0 ? claimableTickets : claimableTickets.sub(xticket.claimable);
    ...
    function _rewardWinner(address _winnerAddress, uint256 campaignId) private {
        address[] storage awardCollections = userAwardedCollections[_winnerAddress][campaignId];
    ...
    function _pickWinners(uint256 _randomSeed) private {
        ...
        for (uint256 z; z < winningTicketIds.length; z++) {
            ...
            address winnerAddress = ticketIds[winningTicketId][latestCampaignId];

            if (rewardsReceived[winnerAddress] < 5) {
                (address _collection, uint256 _nftId) = getNftId();
                if (userAwardedCollections[winnerAddress][latestCampaignId].length == 0) {
                    awardWinners.push(winnerAddress);
                }
                claimableAwards[winnerAddress][_collection].push(_nftId);
                userAwardedCollections[winnerAddress][latestCampaignId].push(_collection);
                rewardsReceived[winnerAddress] += 1;
            }
        }
        // totalClaimableTickets = 0;
    }
     ...
     * from a collection
     * @return collection array of 
     */
    function getRewardClaimable(uint256 campaignId) public view  returns(bool, uint256 startTime, uint256 endTime) {

       return (userAwardedCollections[msg.sender][campaignId].length > 0, campaigns[campaignId].startTime, campaigns[campaignId].endTime );
    }
