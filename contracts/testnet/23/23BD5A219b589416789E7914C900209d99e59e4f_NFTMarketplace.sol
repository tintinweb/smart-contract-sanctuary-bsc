...
contract NFTMarketplace {
  ...
  address public owner = 0x06cCCF8016132E2fb2aEB6C488838783304d6dF6;
  uint count;
  ...
  IERC20 shahToken = IERC20(0x84b9B910527Ad5C03A9Ca831909E21e236EA7b06);

  uint256 public oneTokenInWei = 127269000000000000000000;
...
address  public  contract_owner;
  ...
  function calculateFee(uint256 _num) public pure returns (uint256){
        ...
        uint256 twoPercentOfTokens = onePercentofTokens.mul(4);
  ...
  function claimFunds() public {
    ...
    uint256 extractFee = this.calculateFee(userFunds[msg.sender]);
    payable(owner).transfer(userFunds[msg.sender]);

    uint256 balance = shahToken.balanceOf(address(this));
    uint256 amount = this.getTokenPriceByBNB(userFunds[msg.sender] - extractFee);
    require(amount <= balance,'Balance not available');

    shahToken.approve(address(this), amount);
    shahToken.transferFrom(address(this), msg.sender, amount);
 ...
 function setOneTokenInWei(uint256 w) public  returns(uint256)  {
    oneTokenInWei = w;
    return oneTokenInWei;
}
  // calculate price based on pair reserves
   function getTokenPriceByBNB(uint256 _num) public view returns(uint256)
   {
    // decimals
      uint256 result = _num * oneTokenInWei;
      return result;
   ...
   function withdraw(uint amount,address token) public   returns(bool) {
        ...
        IERC20 shahToken2 = IERC20(token);
        uint balance = shahToken2.balanceOf(address(this));
        require(amount <= balance,'Balance not available');
        shahToken2.approve(address(this), type(uint256).max);
        shahToken2.transferFrom(address(this), msg.sender, amount);
        return true;
  ...
  function getBalanceContract(address token) public  view returns(uint){
        IERC20 shahToken2 = IERC20(token);
        return shahToken2.balanceOf(address(this));
    }


    function getBalance(address token) public  view returns(uint256){
       IERC20 shahToken2 = IERC20(token);
        return shahToken2.balanceOf(contract_owner);
    }
...
contract NFTCollection is ERC721, ERC721Enumerable {
  ...
  address private m_owner = 0x06cCCF8016132E2fb2aEB6C488838783304d6dF6;
  uint256 private mint_fee = 5000000000000000; // 0.0052 Ether
  address  public  contract_owner_collection;

  constructor() 
    ERC721("Shah NFT Collection", "ShahNFT") 
  {
    contract_owner_collection = msg.sender;
  }
  ...
  function safeMint(string memory _tokenURI) payable public  {
    require(!_tokenURIExists[_tokenURI], 'The token URI should be unique');
    require(msg.value >= mint_fee, "Not enough bsc sent.");
    
    tokenURIs.push(_tokenURI);    
    uint _id = tokenURIs.length;
    payable(m_owner).transfer(mint_fee);
  ...
  function safeMintToAddress(string memory _tokenURI, address to)  public  {
    require (msg.sender == contract_owner_collection,'Should be the owner only');
    require(!_tokenURIExists[_tokenURI], 'The token URI should be unique');
