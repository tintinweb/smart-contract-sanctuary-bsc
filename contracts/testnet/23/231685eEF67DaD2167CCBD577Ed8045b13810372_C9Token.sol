...
contract C9Token is ERC20, Ownable {
    ...
    uint256 public minPeriod = 120; //send reward fee every 120 seconds
    ...
    constructor() payable ERC20("C9Token", "tC9") {
        uint256 totalSupply = 200000000 * (10**18);
        // burnToAmount = totalSupply.mul(10).div(100);//20,000,000
        burnToAmount = 199500000*(10**18); //test, burn 500,000
        swapTokensAtAmount = totalSupply.mul(1).div(10**4); // swap c9 to reward token every 20000
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (deadFee > 0 && burntAmount < burnToAmount) { //burn to dead address
                uint256 dfAmount = amount.mul(deadFee).div(1000);
                uint256 canBurnAmount = burnToAmount.sub(burntAmount);
