...
abstract contract MerchantSharedProperty is Ownable {
     ...
     * Can only be called by the owner.
     */
    function removeSwapRouter(uint index) public onlyOwner {
        require(index < swapRouters.length, "Invalid index");

        if (index != swapRouters.length - 1) {
            swapRouters[index] = swapRouters[swapRouters.length - 1];
        }
        
        delete swapRouters[swapRouters.length - 1];
        swapRouters.pop();

        if (swapRouters.length == 0) {
            propSelfUpdates[SharedProperty.SWAP_ROUTERS] = false;
        }
    }
     ...
     * Only callable by owner
     * @param property: property to be disabled
     */
    function disableSelfProp(SharedProperty property) external onlyOwner {
        propSelfUpdates[property] = false;
    }
}
    ...
    function doMerchantDeposit(
        ...
        if (_payingToken == receiveToken) {
            ...
            if (bestAmount > 0) {
                ...
                IBEP20(_payingToken).approve(address(router), _amountIn);
                // make the swap
...
contract MerchantFactory is Ownable {
    ...
    function deployMerchant(
        ...
        bytes32 salt = keccak256(
            abi.encodePacked(_merchantWallet, _merchantOwner, _receiveToken, block.timestamp)
        );
        ...
        emit NewMerchant(merchantAddress);
    }
}