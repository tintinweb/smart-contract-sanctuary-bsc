...
contract BSTTRADENFT is IBidNFT, ERC721Holder, Ownable, Pausable {
    ...
    function buyTokenTo(uint256 _tokenId, address _to) public override whenNotPaused {
        ...
        address get_owner = nft.ownerOf(_tokenId);
        nft.safeTransferFrom(get_owner, _to, _tokenId);
    ...
    function getAsks(uint256 _tokenId) public view returns (AskEntry[] memory) {
        AskEntry[] memory asks = new AskEntry[](_asksMap.length());
        
        (uint256 tokenId, uint256 price) = _asksMap.at(_tokenId);
        asks[_tokenId] = AskEntry({tokenId: tokenId, price: price});
    ...
    function sellTokenTo(uint256 _tokenId, address _to) public override whenNotPaused {
        ...
        nft.safeTransferFrom(_owner, _to, _tokenId);
