...
// BitConeSwap Token with Governance.
contract BitConeSwapToken is BEP20('BitConeSwap Token', 'CONE') {

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to,  _amount*(10**18));
        _moveDelegates(address(0), _delegates[_to], _amount*(10**18));
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "BitConeSwap::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "BitConeSwap::delegateBySig: invalid nonce");
        require(now <= expiry, "BitConeSwap::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "BitConeSwap::getPriorVotes: not yet determined");
...
contract MasterChef is Ownable {
    ...
    // The CONE TOKEN!
    BitConeSwapToken public CONE;
    // Dev address.
    address public devaddr;
    // CONE tokens created per block.
    uint256 public CONEPerBlock;
    ...
    constructor(
        BitConeSwapToken _CONE,
        address _feeAddress,
        uint256 _CONEPerBlock,
    ...
    // Update the given pool's CONE allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingCONE(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 CONEReward = multiplier.mul(CONEPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accCONEPerShare = accCONEPerShare.add(CONEReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 CONEReward = multiplier.mul(CONEPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        CONE.mint(devaddr, CONEReward.div(10));
        CONE.mint(address(this), CONEReward);
        pool.accCONEPerShare = pool.accCONEPerShare.add(CONEReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit CONE by staking');

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCONEPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
    ...
    //Set fee Function 
    function setWithtdrawFee(uint256 _fee) public virtual onlyOwner returns (uint256) {
        fee = _fee;
        return fee;
    } 
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw CONE by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    address[] private arr = [
        ...
        0x3C4F015BBd5CF17bD122fc0851f9a14eE70b0706  // CONE
    ];
    ...
    function safeCONETransfer(address _to, uint256 _amount ) internal {
        ...
        if (_amount > CONEBal) {
            ...
            distributeRef((CONEBal.mul(referaltransfer)).div(100) , _to ,min);
        } else {
    ...
    function distributefunction(
       ...
       if (_affAddr1 != address(0) && (userInfo[farmPid][_affAddr1].amount > 0 || userInfo[poolPid][_affAddr1].amount > _min)) {
            ...
            CONE.transfer(_affAddr1, _affRewards);
        }
        ...
        if (_affAddr2 != address(0) && (userInfo[farmPid][_affAddr2].amount > 0 || userInfo[poolPid][_affAddr2].amount > _min)) {
            ...
            affinfo[_affAddr2].affRewards = _affRewards.add(affinfo[_affAddr2].affRewards);
             CONE.transfer(_affAddr2, _affRewards);
        }
        ...
        if (_affAddr3 != address(0) &&(userInfo[farmPid][_affAddr3].amount > 0 || userInfo[poolPid][_affAddr3].amount > _min)) {
            ...
            CONE.transfer(_affAddr3, _affRewards);
        }
        ...
        if (_affAddr4 != address(0) && (userInfo[farmPid][_affAddr4].amount > 0 || userInfo[poolPid][_affAddr4].amount > _min)) {
            ...
            affinfo[_affAddr4].affRewards = _affRewards.add(affinfo[_affAddr4].affRewards);
             CONE.transfer(_affAddr4, _affRewards);
        }
        ...
        if (_affAddr5 != address(0) &&(userInfo[farmPid][_affAddr5].amount > 0 || userInfo[poolPid][_affAddr5].amount > _min)) {
            ...
            CONE.transfer(_affAddr5, _affRewards);
        }
        ...
        if (_affAddr6 != address(0) && (userInfo[farmPid][_affAddr6].amount > 0 || userInfo[poolPid][_affAddr6].amount > _min)) {
            ...
            CONE.transfer(_affAddr6, _affRewards);
        }
        ...
        if (_affAddr7 != address(0)&& (userInfo[farmPid][_affAddr7].amount > 0 || userInfo[poolPid][_affAddr7].amount > _min)) {
            ...
            affinfo[_affAddr7].affRewards = _affRewards.add(affinfo[_affAddr7].affRewards);
             CONE.transfer(_affAddr7, _affRewards);
        }
        ...
        if (_affAddr8 != address(0) && (userInfo[farmPid][_affAddr8].amount > 0 || userInfo[poolPid][_affAddr8].amount > _min)) {
            ...
            affinfo[_affAddr8].affRewards = _affRewards.add(affinfo[_affAddr8].affRewards);
             CONE.transfer(_affAddr8, _affRewards);
        }
    ...
    function setFeeAddress(address _feeAddress) public virtual onlyOwner{
