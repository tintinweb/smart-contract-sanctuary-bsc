...
contract Auction is Ownable, Pausable {
    ...
    mapping(IERC721=> mapping(uint256 => bool)) tokens;
    ...
    function auction(IERC721 token,uint256 _tokenId) public payable time{
        require(tokens[token][_tokenId], "Sale is not started for this token id");
        require(
            msg.value >= salePrice,
    ...
    function transferNFT(IERC721 tokenAddress, uint256 _tokenId) external{
        require(block.timestamp >= endTime, "sale is not completed");
        address TokenHolder = Token.ownerOf(_tokenId);
        require(tokens[tokenAddress][_tokenId] == true, "Check the token address and token id");
        require(TokenHolder == msg.sender, "Only token holder transfer the token");
        Token.safeTransferFrom(TokenHolder, _maxBidPriceHolder, tokenId);
    ...
    function setAuction(
        ...
        Token = tokenAddress;
        require(Token.ownerOf(_tokenId) == msg.sender, "Only owner can set Auction");
        tokens[tokenAddress][_tokenId] = true;
