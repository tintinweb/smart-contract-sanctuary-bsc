...
contract AustralianFarm is Ownable{
    ...
    // 用户土地编号
    mapping(address => uint256) public userLandNOs;
    // 用户土地
    ...
    // 开盲盒
	function openBlindBox() public returns(uint256){
    ...
    function putLand(uint256 landNO) public{
        ...
        // 该土地已经存在
        require(userLands[msg.sender][landNO].NO == 0,"The land already exists");

        uint256 tokenId = _putInTheBackpack(msg.sender,BackpackType.Land,false,0);

        userLandNOs[msg.sender] ++;
    ...
    function putBuliding(uint256 landNO,BackpackType backpackType)public{
        ...
        userLands[msg.sender][landNO].bulidings[bulidingNO - 1].bulidingType = BulidingType(_backpackType2MainType(backpackType));

        userLands[msg.sender][landNO].bulidings[bulidingNO - 1].tokenId = tokenId;

        userLands[msg.sender][landNO].bulidings[bulidingNO - 1].isBulided = true;

        if(backpackType == BackpackType.FarmLand){

            userLands[msg.sender][landNO].bulidings[bulidingNO - 1].crops[0] = Crop(1,CropType.Corn,false,0,0,0);
        }
    }

    // 土地放进背包
    function putLandInBackpack(uint256 landNO) public {

        Land memory land = userLands[msg.sender][landNO];
        ...
        for(uint256 i = 0 ; i < land.bulidings.length ; i++){
            ...
            // 土地被占用
            require(land.bulidings[i].NO == 0,"Land occupied");
        }

        _putInTheBackpack(msg.sender,BackpackType.Land,true,land.tokenId);

        delete userLands[msg.sender][landNO];
    }

    // 建筑放入背包
    function putBulidingInBackpack(uint256 landNO,uint256 bulidingNO) public {
        ...
        for(uint256 i = 0 ; i < buliding.crops.length ; i++){
            ...
            // 建筑被占用
            require(buliding.crops[i].NO == 0,"buliding occupied");
        }

        delete userLands[msg.sender][landNO].bulidings[bulidingNO - 1];
    ...
    // 作物放入背包
    function putCropInBackpack(uint256 landNO,uint256 bulidingNO,uint256 cropNO)public{
        
        Crop memory crop = userLands[msg.sender][landNO].bulidings[bulidingNO - 1].crops[cropNO - 1];

        // 作物不存在
        require(crop.NO != 0,"The crop not found");
        ...
        delete userLands[msg.sender][landNO].bulidings[bulidingNO - 1].crops[cropNO - 1];
    ...
    // 建造+,土地编号，建筑编号
    function bulid(uint256 landNO,uint256 bulidingNO) public{
        ...
        userLands[msg.sender][landNO].bulidings[bulidingNO - 1].clicks ++;

        userLands[msg.sender][landNO].bulidings[bulidingNO - 1].lastBulidingTime = block.timestamp;

        if(bulidingRules[uint256(buliding.bulidingType)].clicks == userLands[msg.sender][landNO].bulidings[bulidingNO - 1].clicks){
        ...
        if(buliding.bulidingType == BulidingType.FarmLand){
           ...
           userLands[msg.sender][landNO].bulidings[bulidingNO - 1].crops[0] = Crop(1,CropType.Corn,false,0,block.timestamp,0);
        }
    }
    ...
    function buyNFTs(uint256 saleIndex,uint256 amount) public {
        ...
        KG20.approveToController(msg.sender, NFTGoods[saleIndex].price);
    ...
    // 土地上的建筑,土地编号
    function myBulidings(uint256 landNO) public view returns(Bulid[3] memory){

        return userLands[msg.sender][landNO].bulidings;
    }

    // 建筑上的作物
    function myCrops(uint256 landNO,uint256 bulidingNO) public view returns(Crop[] memory){
        
        return userLands[msg.sender][landNO].bulidings[bulidingNO - 1].crops;
    }
