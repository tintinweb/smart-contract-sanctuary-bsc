    ...
    FixedRateTaxControllerStorage
{
        ...
        )
    {
        ...
        // decoding params
        uint16 baseUniverseTaxRate = decodeStrategyParams(universeTaxTerms);
        // decoding params
        uint16 baseProtocolTaxRate = decodeStrategyParams(protocolTaxTerms);
        // calculating tax
    ...
    function decodeStrategyParams(ITaxTermsRegistry.TaxTerms memory terms)
        public
        pure
        compatibleStrategy(terms.strategyId)
...
abstract contract TaxController is ITaxController, ERC165 {
    ...
    modifier compatibleStrategy(bytes4 checkedStrategyId) {
        if (checkedStrategyId != strategyId()) revert TaxStrategyMismatch(checkedStrategyId, strategyId());
        _;
    }
...
library TaxStrategies {
    /**
     * @dev Is thrown when invalid Tax Strategy id was passed for validation.
     */
    error InvalidTaxStrategyId();
