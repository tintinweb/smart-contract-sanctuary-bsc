...
contract VaultRouter is Ownable, ReentrancyGuard
{
    ...
    event TokenAddressUpdated(address indexed previousTokenAddress, address indexed newTokenAddress, uint256 indexed poolId);
    event WorkerAddressUpdated(address indexed previousWorkerAddress, address indexed newWorkerAddress, uint256 indexed poolId);
    ...
    function Deposit(uint256 amount, uint256 poolId) public onlyRegisteredUsers nonReentrant
    {
        UpdateShareVal(poolId);
        IShareToken shareToken = IShareToken(ShareTokens[poolId]);
        uint256 shares = shareToken.AmountToShare(amount);
        Fairlaunch.Deposit(shares, poolId, _msgSender());
        address baseToken = IAlpacaWorker(Workers[poolId]).BaseToken();
        TransferHelper.safeTransferFrom(baseToken, _msgSender(), Workers[poolId], amount);
        shareToken.mint(_msgSender(),shares);
        Fairlaunch.UpdateWorkerInfo(amount, poolId, true);
    }
    function WithDraw(uint256 share, uint256 poolId) public onlyRegisteredUsers nonReentrant
    {
        UpdateShareVal(poolId);
        IShareToken shareToken = IShareToken(ShareTokens[poolId]);
        Fairlaunch.Withdraw(share, poolId, _msgSender());
        address workerAddress = Workers[poolId];
        ...
        Fairlaunch.UpdateWorkerInfo(amount, poolId, false);
    }
    function ForcedWithdraw(uint256 share, uint256 poolId) public onlyRegisteredUsers nonReentrant
    {
        UpdateShareVal(poolId);
        IShareToken shareToken = IShareToken(ShareTokens[poolId]);
        address workerAddress = Workers[poolId];
    ...
    function EmergencyWithdrawToken(address token, address to, uint256 amount, uint256 poolId) public onlyOwner
    {
        address workerAddress = Workers[poolId];
        IAlpacaWorker(workerAddress).ApproveToken(token, address(this), amount);
        Operatable(workerAddress).emergencyWithdraw(token, to,  amount);
        IAlpacaWorker(workerAddress).ApproveToken(token, address(this), uint256(0));
    }
    function UpdateShareVal(uint256 poolId) public {
        address workerAddress = Workers[poolId];
        address shareTokenAddress = ShareTokens[poolId];
    ...
    function SetFairLaunch(address fairLaunch) public onlyOwner {
        require(fairLaunch != address(0), "Operatorable: new tokenAddress is the zero address");
        Fairlaunch = IFairLaunch(fairLaunch);
    }
    function SetShareTokenAddress(address newTokenAddress, uint256 poolId) public onlyOwner {
        require(newTokenAddress != address(0), "Operatorable: new tokenAddress is the zero address");
        _updateTokenAddress(newTokenAddress, poolId);
    }
    function SetWorkerAddress(address newWorkerAddress, uint256 poolId) public onlyOwner {
        require(newWorkerAddress != address(0), "Operatorable: new workerAddress is the zero address");
        _updateWorkerAddress(newWorkerAddress, poolId);
    }
    ...
    function UpdateWorkerOperator(address newOperator, uint256 poolId) public onlyOwner{
        Operatable(Workers[poolId]).updateOperator(newOperator);
    }
    function SetWorkerManager(address newManager, uint256 poolId) public onlyOwner{
        IAlpacaWorker(Workers[poolId]).SetManagerAddress(newManager);
    }
    function TransferWorkerOwnership(address newOwner, uint256 poolId) public onlyOwner{
        require(newOwner != address(0), "VaultRouter: new owner is address zero." );
        Operatable(Workers[poolId]).transferOwnership(newOwner);
    }
    function TransferTokenOwnership(address newOwner, uint256 poolId) public onlyOwner{
        require(newOwner != address(0), "VaultRouter: new owner is address zero." );
        Operatable(ShareTokens[poolId]).transferOwnership(newOwner);
    }
     /**
     * @dev Update tokenAddress of the contract
     * Internal function without access restriction.
     */
    function _updateTokenAddress(address newTokenAddress, uint256 poolId) internal{
        address previousTokenAddress = ShareTokens[poolId];
        ShareTokens[poolId] = newTokenAddress;
        emit TokenAddressUpdated(previousTokenAddress, ShareTokens[poolId], poolId);
    }
     ...
     * Internal function without access restriction.
     */
    function _updateWorkerAddress(address newWorkerAddress, uint256 poolId) internal{

        address previousWorkerAddress = Workers[poolId];
        Workers[poolId] = newWorkerAddress;
        emit TokenAddressUpdated(previousWorkerAddress, Workers[poolId], poolId);
    }
...
interface IFairLaunch {
  ...
  function Deposit(uint256 amount, uint256 workerId, address from) external;
  function Withdraw(uint256 amount, uint256 workerId, address to) external;
