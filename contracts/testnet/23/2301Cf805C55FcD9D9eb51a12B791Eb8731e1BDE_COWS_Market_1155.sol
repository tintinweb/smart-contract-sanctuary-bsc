...
contract COWS_Market_1155 is ReentrancyGuard {
    ...
    */
    ...
    function setTokenSale(
        ...
        // Pageging Index
        saleIndex.saleIds.push(saleList.length - 1);
        saleIndex.saleIdIndex[saleList.length - 1] = saleIndex.saleIds.length;
        //=> IDsale => Index => idList
        ...
        onlySaleOwner(_saleId) ifNotPaused
    {
        ...
        //=> IDsale => Index => idList    
        // Pageging Index
        uint256 _index = saleIndex.saleIdIndex[sale.tokenId] - 1 ;
        saleIndex.saleIds[_index] = saleIndex.saleIds[
            saleIndex.saleIds.length - 1
        ];
        saleIndex.saleIdIndex[saleIndex.saleIds.length - 1] = _index + 1;
        saleIndex.saleIdIndex[sale.tokenId]=0;
        saleIndex.saleIds.pop();
        delete saleIndex.saleIdIndex[sale.tokenId];
    ...
    */
    ...
    function getAllSales(uint offset, uint limit) external view returns (SaleInfo[] memory _nftItems, uint nextOffset, uint total) {
        ...
        return (saleList, offset + limit, totalRows);
    }
