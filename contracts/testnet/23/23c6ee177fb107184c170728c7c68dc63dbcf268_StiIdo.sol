...
contract StiIdo  {
    ...
    function viewInfo(address account_) public view returns(uint mode, uint buyAmount, uint target, address idoToken, string memory name, uint totalSupply,  bool isW, uint[7] memory info, uint[4] memory userData){
        mode = meta.mode;
...
contract IDOFactory is OwnableUpgradeable {
    ...
    function create(address idoToken) public returns(address ido) {
        bytes memory bytecode = type(StiIdo).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(idoToken));
        assembly {
            ido := create2(0, add(bytecode, 32), mload(bytecode), salt)
    ...
    function create1(address idoToken) public returns(address ido) {
        bytes memory bytecode = type(StiIdo).creationCode;
