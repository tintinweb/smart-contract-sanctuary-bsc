...
contract LOKENDRA_ROI {
   ...
   function getNextReward() public view returns (uint) {
    ...
    function Registration(uint _referrerID, uint256 _amount) public {
        ...
        totalFreeze[msg.sender] = totalFreeze[msg.sender] + tokenReward/20;
...
function topUp(uint256 _amount) public{
    ...
    // Get Token Price 
    /**
     * @dev tokenPrice find token Price it means at a time of deployment token price will be 25 USDT
     * so usdt have 6 decimal. 1 usdt = 1000000 wei 
     * at start token price = 25000000
     * After one day 1 token = 25250000 and so on.
     */
    function tokenPrice()public view returns(uint256){
          uint256 numDays = (block.timestamp - startTime);
    if (numDays>1){
      return 25e18 + (numDays*6e18);  
    }else if(numDays<1){
      return 1e18 ;
    }
    }
