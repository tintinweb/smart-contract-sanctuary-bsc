...
contract NFTMasterBlindBoxNew is Ownable, ReentrancyGuard {
    ...
    // set white list
    function setWhiteList(uint256 _boxIndex, address[] memory _addressList) public onlyAdmin nonReentrant {
        require(_boxIndex > 0, "BOX_INDEX_ERROR");
        require(_addressList.length > 0, "NONEMPTY_ADDRESS_LIST");
        ...
        BlindBoxExtraInfo storage _blindBoxExtraInfo = BlindBoxExtraList[_boxIndex];
        _blindBoxExtraInfo.whiteList = _addressList;
    }

    // add white list
    function addWhiteList(uint256 _boxIndex, address[] memory _addressList) public onlyAdmin nonReentrant {
        require(_boxIndex > 0, "BOX_INDEX_ERROR");
        require(_addressList.length > 0, "NONEMPTY_ADDRESS_LIST");
        ...
        for (uint256 _dd = 0; _dd < _addressList.length; _dd++) {
            ...
            _blindBoxExtraInfo.whiteList.push(_addressList[_dd]);
        }
    }

    // get white list
    function getWhiteList(uint256 _boxIndex) public view returns (address[] memory){
        require(_boxIndex > 0, "BOX_INDEX_ERROR");

        BlindBoxExtraInfo memory _blindBoxExtraInfo = BlindBoxExtraList[_boxIndex];
        return (_blindBoxExtraInfo.whiteList);
    }
    ...
    function isPassingWhitelist(uint256 _boxIndex, address account) public view returns (bool){
        BlindBoxExtraInfo memory _blindBoxExtraInfo = BlindBoxExtraList[_boxIndex];
        // check white list
        uint256 _whiteListLength = _blindBoxExtraInfo.whiteList.length;
        if (_whiteListLength > 0) {
            bool _isInWhiteList;
            for (uint256 _i = 0; _i < _whiteListLength; _i++) {
                if (_blindBoxExtraInfo.whiteList[_i] == account) {
                    _isInWhiteList = true;
                    break;
                }
            }
    ...
    function blindPurchase(uint256 _boxIndex, uint256 _num) public payable nonReentrant {
        ...
        // check white list
        require(isPassingWhitelist(_boxIndex, msg.sender), "NOT_IN_WHITE_LIST");
