...
contract Member is Ownable{
    ...
    struct User{
        ...
        uint claimed;
        bool inCycle;
        //        EnumerableSet.AddressSet recommended;
        address invitor;
        address user;
        uint node;
    }
    ...
    //    EnumerableSet.AddressSet   level2;
    //    EnumerableSet.AddressSet   level3;
    //    EnumerableSet.AddressSet  level4;
    ...
    function initLpPool(address bank_, address pair_, address BYZ_, uint startTime_,address liuidityPool_,address usdt_) public onlyOwner{
        ...
        //        pancakePair=IPancakePair(pancakePair_);
        bank = bank_;
   ...
   /* function setStarTime(uint time_) public onlyOwner{
    ...
    function setnode(address []  memory addr_,uint node_) public onlyOwner {
        for (uint256 i = 0; i <  addr_.length; ++i) {
            userInfo[addr_[i]].node=node_;
            levels[node_-1].remove(addr_[i]);
            levels[node_].add(addr_[i]);
        }
    }
    ...
    function checkRe(address user) public view returns(uint){
        ...
        if (_user.lastTime < invalidTime){
            ...
            //            p = _user.power;
            //            x = p * Acc / nowPower;
            //            _re = (token.checkMemberTotalTotal() - lastTotal) * x;
    ...
    function addInvitor(address addr_) public returns(address _invitor) {
        ...
        userInfo[msg.sender].LpAmount=getUserLpAmount(msg.sender);
//        userInfo[addr_].recommended.add(msg.sender);
...
//    function levelOf(uint level_,uint256 startIndex, uint256 endIndex) view external returns (address[] memory addr){
//        uint256 len= levels[level_].length();
//        if (len == 0) {
//            return addr;
//        }
//        if (endIndex == 0 || endIndex > len) {
...
//        address[] memory result = new address[](endIndex - startIndex);
//        for (uint256 i = startIndex; i != endIndex; ++i) {
//            result[i - startIndex] =  levels[level_].at(i);
//        }
//        return result;
//    }
    ...
    function upgradeUser(address[] memory addr_) public check returns (bool upgrade){
        ...
        if (recommendedAddr==5){
            ...
            if (recommended[msg.sender].contains(addr_[0])&&recommended[msg.sender].contains(addr_[1])&&recommended[msg.sender].contains(addr_[2])){
                if (userInfo[addr_[0]].node==userInfo[addr_[1]].node&&userInfo[addr_[0]].node==userInfo[addr_[2]].node){
                    if (userInfo[addr_[0]].node>=1&&userInfo[addr_[0]].node<4&&userInfo[addr_[0]].node==userInfo[msg.sender].node){
                        userInfo[msg.sender].node+=1;
                        if (userInfo[msg.sender].node==2){
                            levels[1].remove(msg.sender);
                            levels[2].add(msg.sender);
                        }else if(userInfo[msg.sender].node==3){
                            levels[2].remove(msg.sender);
                            levels[3].add(msg.sender);
                        }else if(userInfo[msg.sender].node==4){
                            levels[3].remove(msg.sender);
                            levels[4].add(msg.sender);
                        }
                        emit UpgradeUser(msg.sender,userInfo[msg.sender].node);
                        return true;
                    }else{
