    ...
    ReentrancyGuardUpgradeable
{
    ...
    function onlyManager() internal view {
        require(msg.sender == managerOwner || authorized[msg.sender]);
    }
    ...
    function getNonBMValues() public view returns (uint256) {
        ...
        for (uint256 i; i < nonBMNamesAddress.length; i++) {
            if (nonBMNamesMapping[nonBMNamesAddress[i]]) {
        ...
        returns (uint256 finalSwapOutput)
    {
        ...
        if (managerUnit > 0 && copytrading) {
            TransferHelper.safeTransferBNB(managerToCopy, managerUnit);
        } else if (managerUnit > 0 && !copytrading) {
    ...
    function updateCopyTrading() public returns (bool) {
        ...
        uint256[] memory targetWeight_t = new uint256[](
            fundToCopy.getTargetWeightsAddress().length
        );

        toAddresses = fundToCopy.getTargetNamesAddress();

        targetWeight_t = fundToCopy.getTargetWeightsAddress();

        uint256 totalValueB4 = _getFundValues();
        _createTargetNames(toAddresses, targetWeight_t);
    ...
    function setDCA(bool result) public returns (bool) {
        onlyManager();
        dca = result;
        return true;
    }
    ...
    /// @dev pause fund
    function setPause(bool result) public {
        onlyManager();
    ...
    function createTargetNames(
        ...
        require(!copytrading, "CopyActive");
        ...
        returns (uint256)
    {
        ...
        factory.mint(msg.sender, mintQty, EMPTY_DATA);
        ...
        returns (uint256)
    {
        ...
        if (msg.sender != address(this)) {
            factory.burn(msg.sender, tradeParams.amount);
        }
    ...
    function emergencyRedeem(uint256 redeemUnit) external payable {
        uint256 totalValueB4 = _getFundValues();

        uint256 redeemratio = redeemUnit.mul(1e18).div(totalSupply());
        require(redeemratio > 0, "MustBeZero");
        factory.burn(msg.sender, redeemUnit);
        uint256 totalBaseBal = address(this).balance;
        uint256 totalOutput = redeemratio.mul(totalBaseBal).div(1e18);
        TransferHelper.safeTransferBNB(msg.sender, totalOutput);
    ...
    function _getNewFundUnits(
        ...
        uint256 totalUnitAfter = totalValueAfter.mul(totalSupply_t).div(
            totalFundB4
        );

        require(
            totalSupply_t < totalUnitAfter,
            "Total Supply Must Be Less TotalUnitAfter"
        );

        uint256 mintUnit = totalUnitAfter.sub(totalSupply_t);
    ...
    {
        require(totalfundvalue > 0, "Fund Value Must Be Greater Than 0");
        destRebQty = 0;
    ...
    function _createTargetNames(
        ...
        uint256 totalWeight;
        for (uint256 i; i < targetWeight_t.length; i++) {
            totalWeight += i;
        }
        ...
        for (uint256 i; i < toAddresses.length; i++) {
            targetWeight[toAddresses[i]] = targetWeight_t[i];
