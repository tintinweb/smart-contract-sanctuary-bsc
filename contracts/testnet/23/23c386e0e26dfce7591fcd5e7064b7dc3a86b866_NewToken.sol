...
contract NewToken is Context, IBEP20, Ownable {
    ...
    uint256 public burnFeeOnBuying = 30; // 3% will be added to the development address
    ...
    uint256 public burnFeeOnSelling = 30; // 3% will be added to the zero address
    ...
    constructor() {
        ...
        isExcludedFromMaxHolding[burnWallet] = true;
    ...
    function setBuyFeePercent(
        ...
        require(
            _burnFee.add(_lpFee) <= percentDivider.mul(3).div(10),
    ...
    function totalBuyFeePerTx(uint256 amount) public view returns (uint256) {
        uint256 fee = amount
            .mul(
                burnFeeOnBuying.add(liquidityFeeOnBuying)
            )
            .div(percentDivider);
        return fee;
    }
    ...
    function distributeAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 bnbFormarket = deltaBalance.mul(burnFeeCounter).div(
                tokenAmountToBeSwapped
            );
            uint256 bnbForBurn = deltaBalance.sub(bnbToBeAddedToLiquidity).sub(
                bnbFormarket
            );
            ...
            if (bnbForBurn > 0)  burnWallet.transfer(bnbForBurn);
