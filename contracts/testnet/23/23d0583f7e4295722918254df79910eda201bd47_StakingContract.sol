...
contract StakingContract {
    ...
    event infoCreatorReferal(uint256 battleId, address referalGetter, address ReferalSender, uint256 referalAmount, uint256 atTime);
	event infoJoinerReferal(uint256 battleId, address referalGetter, address ReferalSender, uint256 referalAmount, uint256 atTime);
    ...
    function JoinBattle(uint256 _amount, uint256 _battleId, address _joinerReferalPerson) public {
        ...
        emit infoCreatorReferal(
            _battleId,
			battle.creatorReferalPerson,
			battle.creator,
			creatorReferalAmount,
			creatorReferalTime
		);

		emit infoJoinerReferal(
            _battleId,
