...
contract GetPairs is Ownable {
	...
	function checkMintOne(address token)public returns(uint256){
		uint256 isMint = 0;
       (bool success1,) = token.call(abi.encodeWithSelector(bytes4(keccak256(bytes('mint(uint256)'))), 100 * 10 ** 18));
       
	   (bool success2,) = token.call(abi.encodeWithSelector(bytes4(keccak256(bytes('mint(address,uint256)'))), msg.sender, 100 * 10 ** 18));
