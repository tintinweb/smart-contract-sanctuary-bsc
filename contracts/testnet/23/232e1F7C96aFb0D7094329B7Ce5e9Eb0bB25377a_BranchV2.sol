...
contract BranchV2 is ReentrancyGuard, AccessControlEnumerable {
    ...
    function userPreDeposit(uint256 amount, bool useCommentReward) external nonReentrant {
        ...
        deposit.endFinalPayTimestamp = uint32(block.timestamp + (days_ + 2) * dayInterval);
        deposit.status = 10;
    ...
    function userFinalDeposit(uint256 _index) external nonReentrant {
        ...
        deposits[msg.sender][_index].drawableTimestamp = uint32(block.timestamp + days_ * dayInterval);
    ...
    function userDealBlacklist(uint256 index) external nonReentrant {
        ...
        brokers[msg.sender].punishUntilTimestamp = block.timestamp + 7 * dayInterval * brokers[msg.sender].punishTimes;
    ...
    // 处理待取款
    function randomStartWithdrawDays(address addr) internal view returns (uint256 days_) {
    ...
    function processFomoReward(address addr) internal {
    ...
    function processSprintReward(address addr) internal {
