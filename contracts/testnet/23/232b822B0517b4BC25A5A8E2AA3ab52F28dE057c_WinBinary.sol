...
contract WinBinary is ERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        uint256[4] memory feeSettings, // rewards, liquidity, jackpot, burn
        ...
        totalFees = tokenRewardsFee.add(liquidityFee).add(jackpotFee);
        require(totalFees <= 2500, "WINB: Total fee is over 25%");
        swapTokensAtAmount = 100 ether; // Swap at 100 DNG
    ...
    function setLiquidityFee(uint256 value) external onlyOwner {
        liquidityFee = value;
        totalFees = liquidityFee.add(tokenRewardsFee).add(jackpotFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }

    function setJackpotFee(uint256 value) external onlyOwner {
        jackpotFee = value;
        totalFees = jackpotFee.add(liquidityFee).add(tokenRewardsFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }

    function setBurnFee(uint256 value) external onlyOwner {
        burnFee = value;
        uint256 totalSellFees = burnFee.add(liquidityFee).add(tokenRewardsFee).add(tokenRewardsFee);
    ...
    function getBUSDValue(uint256 amount) public view returns (uint256) {
        ...
        path[2] = BUSD;
        uint256 value = uniswapV2Router.getAmountsOut(amount, path)[2];
        return value;
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        require(
            ...
            "WINB: The new dividend tracker must be owned by the DNG token contract"
        );
    ...
    function _transfer(
        ...
        if (
            ...
            swapTokensForTokens(jackpotTokens, BUSD);

            uint256 amountBUSDToFundJackpot = IERC20(BUSD)
        ...
        if (takeFee) {
            if (automatedMarketMakerPairs[to]) {
                uint256 sellTotalFees = liquidityFee
                    .add(jackpotFee)
                    .add(tokenRewardsFee)
                    .add(burnFee);
                uint256 totalSellFeesTokens = amount.mul(sellTotalFees).div(
                    FEE_DENOMINATOR
                );
                _jackpotTokens = _jackpotTokens.add(
                    totalSellFeesTokens.mul(jackpotFee).div(sellTotalFees)
                );
                uint256 burnTokens = totalSellFeesTokens.mul(burnFee).div(sellTotalFees);
                super._burn(from, burnTokens);
                amount = amount.sub(totalSellFeesTokens);
                super._transfer(
                    from,
                    address(this),
                    totalSellFeesTokens.sub(burnTokens)
                );
            } else {
                uint256 fees = amount.mul(totalFees).div(FEE_DENOMINATOR);
                _jackpotTokens = _jackpotTokens.add(
                    fees.mul(jackpotFee).div(totalFees)
                );
    ...
    function swapTokensForTokens(uint256 tokenAmount, address tokenOut) public {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = BUSD;
        ...
        uint256 wethBalanceBefore = IERC20(uniswapV2Router.WETH()).balanceOf(address(this));
        ...
        uint256 wethSwapAmount = IERC20(uniswapV2Router.WETH()).balanceOf(address(this)).sub(wethBalanceBefore);
        /// swap to BUSD
        address[] memory path1 = new address[](2);
        path1[0] = uniswapV2Router.WETH();
        path1[1] = tokenOut;
        IERC20(uniswapV2Router.WETH()).approve(address(uniswapV2Router), wethSwapAmount);
        uniswapV2Router.swapExactTokensForTokens(
            wethSwapAmount,
            0,
            path1,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 busdAmount) public {
...
contract WinBinaryTokenDividendTracker is
    Initializable,
    OwnableUpgradeable,
    DividendPayingToken
{
