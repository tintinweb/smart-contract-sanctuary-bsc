...
contract Growwin is Context, IERC20, Ownable {
    ...
    address public immutable _liquiditypool;
    address public constant _burn = address(0xdead);

    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 3000000000000000000000000000;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Grow Win";
    string private constant _symbol = "GWC";
    uint8 private constant _decimals = 18;

    uint256 public _taxFee = 200; // 3% redistributed among hodlers
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _burnFee = 100; // 2% burn fee
    uint256 private _previousBurnFee = _burnFee;

    uint256 public marketingAndDevelopment = 100; // 2% Marketing and Development
    uint256 public liquiditypools = 100;
    uint256 private _previousMarketingAndDev = marketingAndDevelopment;
    uint256 private _previousliquiditypool = liquiditypools;

    uint256 public _maxTxAmount = 3000000000000000000000000000;
    uint256 public _maxWalletToken = 3000000000000000000000000000;
    ...
    event LiquidityPoolFeeChange(uint256 oldValue, uint256 newValue);
    ...
    constructor(address marketingAndDev,address liquiditypool) {
        require(
    ...
    function setMaxTxPercent(uint256 newValue) external onlyOwner {
        require(
            newValue <= 3000000000000000000000000000,
    ...
    function setLiquidityFeePercent(uint256 newValue) external onlyOwner {
      //require(
        //    newValue <= 1,
        //    "Liquidity poo fee must be less than or equal to  1"
       // );
        _previousliquiditypool = newValue;
        liquiditypools = newValue;
        emit MarketAndDevFeeChange(_previousliquiditypool, newValue);
    }
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(liquiditypools).div(10000);
    }

    function removeAllFee() private {
        if (_taxFee == 0 && _burnFee == 0 && marketingAndDevelopment == 0 && liquiditypools == 0)
            return;
        _taxFee = 0;
        _burnFee = 0;
    ...
    function restoreAllFee() private {
        ...
        liquiditypools = _previousliquiditypool;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (_isExcluded[_liquiditypool]) {
                _tOwned[_liquiditypool] = _tOwned[_liquiditypool].add(
                    calculateLiquidityFee(amount)
                );
            }
            _rOwned[_liquiditypool] = _rOwned[_liquiditypool].add(
                calculateLiquidityFee(amount).mul(_getRate())
            );
