...
contract ReflectionToken is Context, IERC20, Ownable {
    ...
    uint256 private WHOLE_TOKENS = 5_000_000_000; //5_000_000_000
    uint256 private _initialSimpleTotal = WHOLE_TOKENS * (10**_decimals); // 5 billion tokens
    uint256 private _simpleTokenTotal = _initialSimpleTotal;
    uint256 private MAX_UINT = type(uint256).max;
    uint256 private _initialReflectionPool = MAX_UINT; // - _simpleTokenTotal; // (MAX_UINT - (MAX_UINT % _simpleTokenTotal));//type(uint256).max;
    uint256 private _reflectionPoolTotal = _initialReflectionPool;
    uint16 private _taxRate = 300; // Tax rate, represented as hundredths of a percent, e.g 100 = 1%
    ...
    event ExcludeAccount(address excludedAccount);
    event IncludeAccount(address includedAccount);
    ...
    */
        ...
        returns (uint256)
    {
        ...
        return
            _tokenFromReflection(_reflectionPoolBalances[account], _getRate());
    }
    ...
    */
    ...
    function reflectionFromToken(uint256 amount) public view returns (uint256) {
        return _reflectionFromToken(amount, _getRate());
    }
    ...
    function excludeAccount(address account) external onlyOwner {
        ...
        if (_reflectionPoolBalances[account] > 0) {
            uint256 simpleTokenAmount = _tokenFromReflection(
                _reflectionPoolBalances[account],
                _getRate()
            );
            _simpleTokenBalances[account] = simpleTokenAmount;
            //_simpleTokensHeld += simpleTokenAmount;
            //_reflectionPoolTotal -= _reflectionPoolBalances[account];
    ...
    function includeAccount(address account) external onlyOwner {
        ...
        if (currentSimpleTokenBalance > 0) {
            uint256 balanceToRestore = _reflectionFromToken(
                currentSimpleTokenBalance,
                _getRate()
            );
            _reflectionPoolBalances[account] = balanceToRestore; //Restore the reflection balance
            //_reflectionPoolTotal += balanceToRestore;
            //_simpleTokensHeld += currentSimpleTokenBalance;
        ...
        returns (uint256)
    {
        ...
        //require(tokenFee == tokenAmount / 100, "Fee calculation error");
        return tokenFee;
    }

    // Get the current exchange rate: reflections per token
    function _getRate() public view returns (uint256) {
        (uint256 reflections, uint256 tokens) = _getCurrentSupply();
        uint256 rate = reflections / tokens;
        return rate;
    }
    ...
    function _getCurrentSupply() public view returns (uint256, uint256) {
        uint256 reflectionSupply = _reflectionPoolTotal;
        uint256 tokenSupply = _initialSimpleTotal;
        // Start with the total supply, and subtract the tokens owned by excluded accounts
        for (uint256 i = 0; i < _excluded.length; i++) {
            address excludedAccount = _excluded[i];
            if (
                _reflectionPoolBalances[excludedAccount] > reflectionSupply ||
                _simpleTokenBalances[excludedAccount] > tokenSupply
            ) return (_reflectionPoolTotal, _initialSimpleTotal);
            reflectionSupply -= _reflectionPoolBalances[excludedAccount];
            tokenSupply -= _simpleTokenBalances[excludedAccount];
        }
        if (reflectionSupply < _reflectionPoolTotal / _initialSimpleTotal)
            return (_reflectionPoolTotal, _initialSimpleTotal);
