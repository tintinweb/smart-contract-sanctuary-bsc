...
contract A1_BOOSTER {
    ...
    uint256[20] private level5Percents = [200, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50 ,50, 50 ,50, 50];
    ...
    function withdraw() external {
     ...
     if(withdrawable+FlushRecords[msg.sender].totalFlush <= userInfo[msg.sender].totalreward)
        {
          Flush =withdrawable;
      
        FlushRecords[msg.sender].latetFlush=Flush;
        FlushRecords[msg.sender].totalFlush+=Flush;
        
        
         }
        else
        {
             Flush = userInfo[msg.sender].totalreward.sub(FlushRecords[msg.sender].totalFlush);
             FlushRecords[msg.sender].latetFlush=Flush;
             FlushRecords[msg.sender].totalFlush+=Flush;
        }
      
        usdt.transfer(msg.sender, FlushRecords[msg.sender].latetFlush);
