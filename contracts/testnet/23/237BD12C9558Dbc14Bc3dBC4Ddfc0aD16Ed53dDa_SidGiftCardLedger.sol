...
contract SidGiftCardRegistrar is ERC1155, ISidGiftCardRegistrar, Ownable {
    ...
    function register(
        ...
        uint256 id,
        uint256 amount
    ) external onlyController returns (uint256, uint256) {
...
contract SidGiftCardVoucher is Ownable{
    ...
    uint256 public constant THREE_DIGIT_VOUCHER_VALUE = 6.5 * (10**20);
    uint256 public constant FOUR_DIGIT_VOUCHER_VALUE = 1.6 * (10**20);
    uint256 public constant FIVE_DIGIT_VOUCHER_VALUE = 5 * (10**18);

    mapping(uint256 => uint256) public customizedVoucherValue;

    function addCustomizedVoucher(uint256 tokenId, uint256 price) public onlyOwner{
        customizedVoucherValue[tokenId] = price;
    }
    
    function totalValue(uint256[] memory ids, uint256[] memory amounts) public view returns (uint256) {
        uint256 total = 0;
        for (uint256 i = 0; i < ids.length; i++) {
            if (ids[i] == THREE_DIGIT_VOUCHER) {
                total += THREE_DIGIT_VOUCHER_VALUE * amounts[i];
            } else if (ids[i] == FOUR_DIGIT_VOUCHER) {
                total += FOUR_DIGIT_VOUCHER_VALUE * amounts[i];
            } else if (ids[i] == FIVE_DIGIT_VOUCHER) {
                total += FIVE_DIGIT_VOUCHER_VALUE * amounts[i];
            } else {
                total += customizedVoucherValue[ids[i]] * amounts[i];
            }
        }
        return total;
    }
