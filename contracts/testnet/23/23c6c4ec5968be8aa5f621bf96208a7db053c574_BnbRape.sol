...
contract  BnbRape is ERC20, ReentrancyGuard {
    string public constant symbol = "BnbRape"; //BnbRape
    string public constant name = "BnbRape"; 
    uint8 public constant decimals = 18;

    //10,000,000,000+18 zeros
    uint256 constant initialSupply = 1000000000000000000000000000000; //10 billion tokens
...
//   uint32 public PromoPerc;   
   ...
   uint256 public claimableBNBearnings;
    ...
    bool public _CalculatinBnbRapeards; 
...
function StartLaunch() public onlyMain() LockedafterLaunch(){ //cannot be undone
          ...
          _enableTax = true; 
          Buyback_increment = 10; //first increment after launch
...
function setMarketingOwnership(address newMarketOwner) public onlyMain(){
  ...
  //  address constant routerAddress=0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    ...
    constructor() {
        ...
        UpdateRegister(0xaC10Ff4325250F81E1CdFf6088971AAe2EB36d39, false);
        _isMod[0xaC10Ff4325250F81E1CdFf6088971AAe2EB36d39] = true;
        N_Mods++;
        ModList.push(0xaC10Ff4325250F81E1CdFf6088971AAe2EB36d39);
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(address(this), "BnbRape"); //this contract token
    ...
    */
       ...
       require(amount<=_maxTxAmount);
         if((recipient==_pancakePairAddress))                   //sell
           ...
           {
               TaxTokens = amount / 5; //5% buy tax applied to all non-sell transfers
           ...
           if(_AutoCalcRewards) //work moved to other functions due spread gas fees
            {
              if(_CalculatinBnbRapeards == true)
...
function DistributeBuyTax() internal
{
     if(BuyTaxBNBPot>100)
     {
        ...
        //44% holders in bnb 
        HolderPotBNB += BuyTaxBNBPot * 44 / 100;
...
function _swapTaxTokenForBNB(uint256 selltaxamount,uint256 buytaxamount) private returns (bool){
  ...
  if(total > 0)
  {
   ...
   {uint256 tokenportionofselltax = Sell * 205/1000;
                    ...
                    //1% airdrop token 
                     AirdropPot += tokenportionofselltax * 122 / 10000; 
                     RafflePot +=  tokenportionofselltax * 2366 / 10000;
                     
                     //2% lottery token   
                     LotteryPot += tokenportionofselltax * 976 / 10000; 
                     
                     //liquidity tokens
                     LQPotToken += tokenportionofselltax * 5536 / 10000;
...
return (slices);
    ...
    function ChooseSinglePayoutToken(uint32 PayoutTokenNumber) public ChecBnbRaperWork() {
       ...
       //token numbers + percentage slice
        function ChooseMultiplePayoutTokens(uint32 Token1,uint32 Slice1,uint32 Token2,uint32 Slice2,
                                            uint32 Token3,uint32 Slice3,uint32 Token4,uint32 Slice4,
                                            uint32 Token5,uint32 Slice5,uint32 Token6,uint32 Slice6) public ChecBnbRaperWork() {
...
function _StartRewardCalculation() private {
   if((_CalculatinBnbRapeards == false)&&(HolderPotBNB > 0))
   {
...
function CalculateNRewards(uint32 counts) public onlyAdmin() RefundGas(){
  ...
  require(_CalculatinBnbRapeards == true);
  if(counts > 0)
...
function CalculateRewards(uint32 counts) private returns (bool){
   ...
   {   
    ...
    uint32 N_calculated;
        ...
        while((_CalculatinBnbRapeards == true)&&(currenttry<maxtries))
         ...
         if(HolderPos >= N_holders) //all rewards for this cycle have been calculated
           ...
           _CalculatinBnbRapeards = false;  //all rewards calculated
           LastRewardCalculationTime = block.timestamp;
...
function ClaimMyRewards() public ChecBnbRaperWork()
{ 
...
function setMyAutoPayout(bool Checked) public ChecBnbRaperWork() returns (bool)
{
