...
contract SoulmetaController is DivestorUpgradeable, RandomGeneratorUpgradeable {
    ...
    address public aPropCore;           //道具逻辑合约
    ...
    function castApi(uint8 theme_, uint8 group_, uint soulCount_, uint amount_, address recommender_) public {
        ...
        require(soulCount_ == 1 || (soulCount_ > 0 && soulCount_ % 5 == 0), "soul err");

        //用户每铸造100个需要支付4u
        cUsdt.safeTransferFrom(player, address(this), amount_ * 4 ether);

        //消耗soul的数量
        cSoulToken.burnFrom(player, soulCount_ * 1 ether);

        //        uint b1 = cSoulToken.balanceOf(aSoulPair);
        //        uint b2 = cUsdt.balanceOf(aSoulPair);
        //        uint amount = b1 * 76 ether / amount / 20  + 0.05 ether;
        //        cSoulToken.transfer(player, amount);

        //触发分成规则
        _divide(recommender_, address(0), amount_);

        uint soulCount = (soulCount_ >= 5 && soulCount_ <= 20) ? (soulCount_ / 5 + 1) : 1;

        //组装插入数组中的索引
        uint poolId = getPoolId(theme_, group_, uint8(soulCount));
   ...
   *  @params
     ...
     *  amount_ : 数量
     **/
    function _divide(address recommender_, address caster_, uint amount_) public {
    require(_msgSender() == aPropCore || _msgSender() == owner() || _msgSender() == address(this) , "S: Err Operator.");
    address player = _msgSender();
        // 份数
    ...
    **/
    ...
    function setReferrersFrom(address user_, address communityAddr_) public {
        require(_msgSender() == aPropCore || _msgSender() == aSoulAirdrop || _msgSender() == owner() , "S: Err Operator.");
