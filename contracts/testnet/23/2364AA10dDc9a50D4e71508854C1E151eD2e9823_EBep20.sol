...
/**
 * @title Evry.Finance's EBep20 Contract
 * @notice ETokens which wrap an EIP-20 underlying
 * @author Evry.Finance
 */
contract EBep20 is EToken, EBep20Interface {
    /**
     ...
     * @param depositAllowed_ BEP-20 allow to deposit of this token
     * @param borrowAllowed_ BEP-20 allow to borrow of this token
     * @param onlyWhiteListAllowToDeposit_ BEP-20 allow to deposit for only the whitelist account of this token
     * @param whiteListDepositAccount_ BEP-20 set whitelist of the deposit account of this token
     */
    function initialize (address underlying_,
                ...
                uint8 decimals_,
                bool depositAllowed_,
                bool borrowAllowed_,
                bool onlyWhiteListAllowToDeposit_,
                address whiteListDepositAccount_) external {

        require(msg.sender == admin, "only admin may initialize the market");
        require(accrualBlockNumber == 0 && borrowIndex == 0, "market may only be initialized once");

        // EToken initialize does the bulk of the work
        super.init(comptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_, depositAllowed_, borrowAllowed_, onlyWhiteListAllowToDeposit_, whiteListDepositAccount_);
     ...
     */
    function mint(uint mintAmount, bool activateCollateral) external override returns (uint) {
        (uint err,) = mintInternal(mintAmount, activateCollateral);
        return err;
    }

    /**
     * @notice Sender supplies assets into the market and receiver receives eTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver the account which is receiving the eTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
...
/**
 * @title Evry.Finance's EToken Contract
...
abstract contract EToken is ETokenInterface, Exponential, TokenErrorReporter {
     ...
     * @param depositAllowed_ EIP-20 allow to deposit of this token
     * @param borrowAllowed_ EIP-20 allow to borrow of this token
     * @param onlyWhiteListAllowToDeposit_ EIP-20 allow to deposit for only the whitelist account of this token
     * @param whiteListDepositAccount_ EIP-20 set whitelist of the deposit account of this token
     */
    function init(ComptrollerInterface comptroller_,
                        ...
                        bool borrowAllowed_,
                        bool onlyWhiteListAllowToDeposit_,
                        address whiteListDepositAccount_) internal {
        require(msg.sender == admin, "only admin may initialize the market");
        require(accrualBlockNumber == 0 && borrowIndex == 0, "market may only be initialized once");
        ...
        onlyWhiteListAllowToDeposit = onlyWhiteListAllowToDeposit_;
        if(onlyWhiteListAllowToDeposit){
            depositWhiteListAccounts[whiteListDepositAccount_] = true;
        }
     ...
     */
    function mintInternal(uint mintAmount, bool activateCollateral) internal nonReentrant returns (uint, uint) {
        ...
        return mintFresh(msg.sender, mintAmount, activateCollateral);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintFresh(address minter, uint mintAmount, bool activateCollateral) internal returns (uint, uint) {
         ...
         *  Note: The eToken must handle variations between BEP-20 and BNB underlying.
        ...
        /* Add to EToken to market if required */
        if (activateCollateral) {
            uint activateErr = comptroller.addToMarketByEToken(address(this), minter);
            require(activateErr == 0, "ACTIVATE_COLLATERAL_FAILED");
        }
     ...
     * @notice Sender supplies assets into the market and receiver receives eTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver The address of the account which is receiving the eTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    ...
    /**
     * @notice Payer supplies assets into the market and receiver receives eTokens in exchange
     * @dev Assumes interest has already been accrued up to the current block
     * @param payer The address of the account which is paying the underlying token
     * @param receiver The address of the account which is receiving eToken
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintBehalfFresh(address payer, address receiver, uint mintAmount) internal returns (uint, uint) {
        ...
        /*
         * We calculate the new total supply of eTokens and receiver token balance, checking for overflow:
         *  totalSupplyNew = totalSupply + mintTokens
    ...
    function liquidateBorrowInternal(address borrower, uint repayAmount, ETokenInterface eTokenCollateral) internal nonReentrant returns (uint, uint) {
        // Only allow calls from liquidator contract
        require(msg.sender == comptroller.getLiquidatorContract(), "call not from liquidator contract");
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, ETokenInterface eTokenCollateral) internal returns (uint, uint) {
        ...
        uint allowed = comptroller.liquidateBorrowAllowed(address(this), address(eTokenCollateral), liquidator, borrower, repayAmount);
        if (allowed != 0) {
        ...
        (uint amountSeizeError, uint seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(eTokenCollateral), actualRepayAmount);
        ...
        /* We call the defense hook */
        comptroller.liquidateBorrowVerify(address(this), address(eTokenCollateral), liquidator, borrower, actualRepayAmount, seizeTokens);
    ...
    /**
     * @notice updates the depositAllowed
     * @dev Admin function to update the depositAllowed
     * @param _allow the new depositAllowed to use
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setDepositAllowed(bool _allow) public override returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_DEPOSIT_ALLOWED_OWNER_CHECK);
        }

        depositAllowed = _allow;
        emit AllowTokenToDeposit(depositAllowed);

        return uint(Error.NO_ERROR);
    }

    /**
     * @notice updates the borrowAllowed
     * @dev Admin function to update the borrowAllowed
     * @param _allow the new borrowAllowed to use
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setBorrowAllowed(bool _allow) public override returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_BORROW_ALLOWED_OWNER_CHECK);
        }

        borrowAllowed = _allow;
        emit AllowTokenToBorrow(borrowAllowed);

        return uint(Error.NO_ERROR);
    }

    /**
     * @notice updates the onlyWhiteListAllowToDeposit
     * @dev Admin function to update the onlyWhiteListAllowToDeposit
     * @param _allow the new onlyWhiteListAllowToDeposit to use
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setOnlyWhiteListAllowToDeposit(bool _allow) public override returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_ONLY_WHITELIST_ALLOW_TO_DEPOSIT_OWNER_CHECK);
        }

        onlyWhiteListAllowToDeposit = _allow;
        emit AllowOnlyWhiteListToDeposit(onlyWhiteListAllowToDeposit);

        return uint(Error.NO_ERROR);
    }

    /** 
     * @dev Sets the whitelist contract allowed to deposit
     *
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setDepositWhiteListAccounts(address _toWhitelist, bool _state) public override returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_DEPOSIT_WHITELIST_ACCOUNTS_OWNER_CHECK);
        }

        depositWhiteListAccounts[_toWhitelist] = _state;
        emit DepositWhiteListAccountsSet(_toWhitelist, _state);
...
abstract contract ComptrollerInterfaceG1 {
    ...
    function getLiquidatorContract() external view virtual returns (address);
}
...
contract ETokenAdminStorage {
    ...
    */
    ...
    /**
    * @notice Active brains of EBep20/EBNB
    */
    address public tokenImplementation;

    /**
    * @notice Pending brains of EBep20/EBNB
    */
    address public pendingTokenImplementation;
}
...
contract ETokenInterfaceStorage is ETokenStorage {
    /**
...
abstract contract ETokenInterface is ETokenInterfaceStorage{
...
contract ComptrollerErrorReporter {
    ...
    enum FailureInfo {
        ...
        SET_LIQUIDATOR_CONTRACT_OWNER_CHECK,
...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        SET_ETOKEN_IMPLEMENTATION_OWNER_CHECK,
