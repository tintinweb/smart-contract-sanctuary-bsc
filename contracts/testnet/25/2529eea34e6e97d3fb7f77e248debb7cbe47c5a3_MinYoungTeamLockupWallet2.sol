 ...
 */
contract MinYoungTeamLockupWallet2  is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    event Allocated(address indexed member, uint256 indexed amount, uint256 indexed releaseTimestamp, uint256 duration);
    ...
    function allocation(address to, uint256 amount, uint256 releaseTimestamp, uint256 duration) public onlyOwner {

        require(duration > 0, "vesting duration must be greater than zero");
        require(releaseTimestamp > 0, "release timestamp must be greater than zero");
        ...
        _allocationInfos[to].push(
            AllocationInfo(amount, releaseTimestamp, duration)
        );

        emit Allocated(to, amount, releaseTimestamp, duration);
    }

    function multipleAllocation(address[] memory to, uint256[] memory amount, uint256[] memory releaseTimestamp, uint256[] memory duration) public onlyOwner {
        require(to.length == amount.length && amount.length == releaseTimestamp.length && releaseTimestamp.length == duration.length, "data invalid");
        for(uint i = 0; i < releaseTimestamp.length; i++) {
            require(duration[i] > 0, "vesting duration must be greater than zero");
        }
        for(uint i = 0; i < releaseTimestamp.length; i++) {
            require(releaseTimestamp[i] > 0, "release timestamp must be greater than zero");
        }
        ...
        for(uint i = 0; i < amount.length; i++) {
            _allocationInfos[to[i]].push(
                AllocationInfo(amount[i], releaseTimestamp[i], duration[i])
            );
            emit Allocated(to[i], amount[i], releaseTimestamp[i], duration[i]);
        }
    }
    ...
    function releaseBalance(address member) public view returns(uint256) {
        ...
        for(uint i = 0; i < allocs.length; i++) {
            if(_getTimestamp() < allocs[i].releaseTimestamp) {
                totalAmount += 0;
            } else if (_getTimestamp() >= allocs[i].releaseTimestamp + allocs[i].duration) {
                totalAmount += allocs[i].amount;
            } else {
                totalAmount += (allocs[i].amount * (_getTimestamp() - allocs[i].releaseTimestamp)) / allocs[i].duration;
            }
        }
        return totalAmount;
    }
    ...
    function withdrawFromMember() public nonReentrant returns(bool) {
