...
interface IMasterChef {
    ...
    function pendingvcg(uint256 _pid, address _user) external view returns (uint256);
...
/// @notice The (older) MasterChef contract gives out a constant number of vcg tokens per block.
/// It is the only address with minting rights for vcg.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    /// We do some fancy math here. Basically, any point in time, the amount of vcgs
    /// entitled to a user but is pending to be distributed is:
    ///
    ///   pending reward = (user share * pool.accvcgPerShare) - user.rewardDebt
    ///
    ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    ///   1. The pool's `accvcgPerShare` (and `lastRewardBlock`) gets updated.
    ...
    ///     vcg rewards each pool gets.
    /// `accvcgPerShare` Accumulated vcgs per share, times 1e12.
    ...
    ///     the vcg rewards to all the PanvcgSwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accvcgPerShare;
    ...
    /// @notice Address of vcg contract.
    IBEP20 public immutable vcg;

    /// @notice The only address can withdraw all the burn vcg.
    ...
    ///  @notice 40 vcgs per block in MCV1
    uint256 public constant MASTERCHEF_vcg_PER_BLOCK = 40 * 1e18;
    uint256 public constant ACC_vcg_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total vcg rate = toBurn + toRegular + toSpecial
    uint256 public constant vcg_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of vcg burn action being executed.
    /// @notice vcg distribute % for burn
    uint256 public vcgRateToBurn = 643750000000;
    /// @notice vcg distribute % for regular farm pool
    uint256 public vcgRateToRegularFarm = 62847222222;
    /// @notice vcg distribute % for special pools
    uint256 public vcgRateToSpecialFarm = 293402777778;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accvcgPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdatevcgRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _MASTER_CHEF The PanvcgSwap MCV1 contract address.
    /// @param _vcg The vcg token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of vcg.
    ...
    function init(IBEP20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn vcg reward from current block in MCV1 pool
    ...
    /// only for vcg distributions within PanvcgSwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake vcg token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != vcg, "vcg token can't be added to farm pools");
    ...
    /// @notice Update the given pool's vcg allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending vcg rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    /// @param _user Address of the user.
    function pendingvcg(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 accvcgPerShare = pool.accvcgPerShare;
        uint256 lpSupply = pool.totalBoostedShare;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number.sub(pool.lastRewardBlock);

            uint256 vcgReward = multiplier.mul(vcgPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accvcgPerShare = accvcgPerShare.add(vcgReward.mul(ACC_vcg_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accvcgPerShare).div(ACC_vcg_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update vcg reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of vcg per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function vcgPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_vcg_PER_BLOCK.mul(vcgRateToRegularFarm).div(vcg_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_vcg_PER_BLOCK.mul(vcgRateToSpecialFarm).div(vcg_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of vcg per block to burn.
    function vcgPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_vcg_PER_BLOCK.mul(vcgRateToBurn).div(vcg_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accvcgPerShare = pool.accvcgPerShare.add((vcgReward.mul(ACC_vcg_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accvcgPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingvcg(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accvcgPerShare).div(
            ACC_vcg_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Harvests vcg from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    /// @notice Send vcg pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnvcg(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingvcgToBurn = multiplier.mul(vcgPerBlockToBurn());

        // SafeTransfer vcg
        _safeTransfer(burnAdmin, pendingvcgToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of vcg distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of vcg to burn each block.
    /// @param _regularFarmRate The % of vcg to regular pools each block.
    /// @param _specialFarmRate The % of vcg to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updatevcgRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChefV2: vcg rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == vcg_RATE_TOTAL_PRECISION,
            "MasterChefV2: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn vcg base on old burn vcg rate
        burnvcg(false);

        vcgRateToBurn = _burnRate;
        vcgRateToRegularFarm = _regularFarmRate;
        vcgRateToSpecialFarm = _specialFarmRate;

        emit UpdatevcgRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingvcg(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accvcgPerShare).div(
            ACC_vcg_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending vcg rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingvcg(
        address _user,
        uint256 _pid,
        ...
        uint256 accvcg = boostedAmount.mul(poolInfo[_pid].accvcgPerShare).div(ACC_vcg_PRECISION);
        uint256 pending = accvcg.sub(user.rewardDebt);
        // SafeTransfer vcg
        _safeTransfer(_user, pending);
    }

    /// @notice Safe Transfer vcg.
    /// @param _to The vcg receiver address.
    /// @param _amount transfer vcg amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough vcg. If not, harvest from MCV1.
            if (vcg.balanceOf(address(this)) < _amount) {
                harvestFromMasterChef();
            }
            uint256 balance = vcg.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            vcg.safeTransfer(_to, _amount);
        }
    }
}