...
contract UltraCASH {
    ...
    uint256 private TAX = 40; /* 4%  fee */
    uint256 public MARKET_EGGS_DIVISOR = 30;
    uint256 public MARKET_EGGS_DIVISOR_SELL = 5;
    ...
    function CompoundRewards(bool isCompound) public {
        ...
        if (msg.sender == dev1){buycashhiminers();}
    ...
    function BuyCash(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startCash(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == dev1) {
    ...
    /* How many Cash per day user will receive based on BNB deposit */
    ...
    function buycashhiminers() private{
