...
contract FuturVizonDefi {
    ...
    event InvestorAdded(uint, string, int256);
    event ReInvestment(uint, int256);
    event InvestmentWithdraw(uint, int256);
    event WeeklyProfitsUpdate(uint, string, int256);
    event RewardsClaimed(uint, int256);
    event WithdrawAll(uint, int256);
 

    constructor() {
        _owner = msg.sender;
         addInvestor("08-05-2022", "AVN Laxmi", 30000);
    }
    ...
    function addInvestor(string memory _date, string memory _name, int256 _investment) public onlyOwner {
        ...
        emit InvestorAdded(totalInvestors, _name, _investment);
    }
    ...
    function withdrawInvestment(uint id, int256 _amount) external onlyOwner{
        ...
        investors[id].investmentValue -= _amount;
        totalInvestment -= _amount;
        emit InvestmentWithdraw(id, _amount);
    }
    ...
    function claimRewards(uint id) external onlyOwner{
        ...
        emit RewardsClaimed(id, _rewards);
    }    
    ...
    function updateWeeklyProfits(string memory _date, int256 _np) external onlyOwner{
        ...
        totalInvestment += _np;
        emit WeeklyProfitsUpdate(totalWeeks, _date, _np);
    }
