...
contract MasterStore is ERC2771Context, AccessControl {
    bytes32 public constant EMP_ROLE = keccak256("EMP_ROLE");
    bytes32 public constant PARTNER_ROLE = keccak256("PARTNER_ROLE");
    string public constant NAME = "MasterStore";
    Token public token;
    NFT public nft;
    ...
    modifier staffOnly() {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) ||
                ...
                hasRole(PARTNER_ROLE, _msgSender()),
            "ERROR: staff only"
        );
        _;
    }
    ...
    modifier canClaim(uint256 tokenId) {
        (uint256 pid, uint256 redeemedAt, , , bool isRedeemed, ) = nft
            .nftCouponInfo(tokenId);

        (, , , uint256 expDays, , , , , , ) = nft.rewardInfo(pid);
        require(
            block.timestamp <= redeemedAt + (expDays * 1 days),
            "ERROR: NFT Coupon expired."
        );
    ...
    function configContract(address _token, address _nft)
        public
    ...
    function topUpByTransfer(address to, uint256 amount) public staffOnly {
        token.transfer(to, amount);
    }

    function topUp(address to, uint256 amount) public staffOnly {
    ...
    function burnNftCoupon(uint256 tokenId)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        nft.burn(tokenId);
    ...
    function _redeem(uint256 _pid, bool byTransfer) private {
        ...
        require(
            ...
            "ERROR: Not during coupon redemption period."
        );
        require(
        ...
        if (byTransfer) {
            token.transferFrom(_msgSender(), address(this), requiredPoints);
        } else {
    ...
    function redeemByTranfer(uint256 _pid) public {
        _redeem(_pid, true);
    }

    function redeemByBurn(uint256 _pid) public {
        _redeem(_pid, false);
    }

    function _claim(uint256 tokenId, address owner) private canClaim(tokenId) {
        (uint256 pid, , , , , ) = nft.nftCouponInfo(tokenId);
        nft.burn(tokenId);
        emit Claimed(pid, tokenId, _msgSender(), owner);
    }
    ...
    function claimBySig(
        ...
        bytes32 structHash = keccak256(
            abi.encode(DELEGATION_TYPEHASH, owner, tokenId, nonces[owner]++)
        );
...
contract Token is ERC20, ERC2771Context, AccessControl, Pausable {
...
contract NFT is ERC721URIStorage, ERC2771Context, ERC721Enumerable, AccessControl, Pausable {
    ...
    function updateReward(
        ...
        require(
            _rewardInfo.frozen == false,
            "ERROR: Frozen rewards cannot be edited."
        );
        _rewardInfo.expDays = _expDays;
        ...
        _rewardInfo.startAt = _startAt;
        _rewardInfo.endAt = _endAt;
        _rewardInfo.frozen = _frozen;
        emit RewardChanged(
            _pid,
            _startAt,
            _endAt,
            _rewardInfo.redeemed,
            _expDays,
            _requiredPoints,
            _tokenURI,
            _limit,
    ...
    {
        uint256 pid = nftCouponInfo[tokenId].pid;
        RewardInfo memory _rewardInfo = rewardInfo[pid];
        return _rewardInfo.tokenURI;
    }
