...
contract Marketplace is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event SystemFeeUpdated(uint256 percent);
    event TreasuryUpdated(address treasury);
    event Erc20WhitelistUpdated(address erc20, bool status);

    event AskCreated(address seller, uint256 price, uint256 tokenId);
    event AskCanceled(address seller, uint256 price, uint256 tokenId);
    event TokenSold(address buyer, address seller, uint256 price, uint256 tokenId);
    event Payout(uint256 tokenId, uint256 systemFeePayment, uint256 sellerPayment);
    ...
    function initialize(address _erc20, address _erc721)
        public
        initializer
    {
        __Ownable_init();
    ...
    function setSystemFee(uint256 percent)
        public
        onlyOwner
    {
    ...
    function setTreasury(address addr)
        ...
        onlyOwner
    {
        require(addr != address(0), "Marketplace: address is invalid");
    ...
    function setSalePrice(uint256 tokenId, uint256 price)
        public
        whenNotPaused
        nonReentrant
    {
        ...
        emit AskCreated(msgSender, price, tokenId);
    }

    function cancelSalePrice(uint256 tokenId)
        public
        whenNotPaused
        nonReentrant
    {
        ...
        emit AskCanceled(msgSender, info.price, tokenId);
        ...
        nonReentrant
    {
        ...
        _payout(tokenId, info.price, info.seller);

        emit TokenSold(msgSender, info.seller, info.price, tokenId);

        delete asks[erc721][tokenId];
    }

    function _payout(uint256 tokenId, uint256 price, address seller)
        internal
    {
        ...
        if (erc20 == address(0)) {
            if (systemFeePayment > 0) {
                payable(treasury).transfer(systemFeePayment);
            }
            ...
            if (systemFeePayment > 0) {
                IERC20(erc20).safeTransfer(treasury, systemFeePayment);
            }
        ...
        emit Payout(tokenId, systemFeePayment, sellerPayment);
    }
