...
contract IFOBitdriveProxy is ReentrancyGuard, Initializable {
  ...
  function round3Deposit(uint256 _amount) public {
    ...
    uint totalPrice = _amount.mul(tokenPrice);
    uint cal = totalPrice.mul(instantpercentage).div(100);
    uint balance = totalPrice.sub(cal);

    uint256 transferReward= cal.mul(1e18);
