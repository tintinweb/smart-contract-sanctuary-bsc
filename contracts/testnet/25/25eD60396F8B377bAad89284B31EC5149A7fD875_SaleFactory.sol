...
import {Sale} from "./contracts/Sale_Final.sol";
...
interface iIncentive {
    function findWinnerBase(uint256 _round) external payable;
    function findWinnerToken(uint256 _round) external;
...
contract Sale is ReentrancyGuard {
    ...
    iSaleMaster internal _saleMaster;
    iSaleVault internal _saleVault;
    iIncentive internal _incentive;
    ...
    address internal saleFactoryAddress;
    address internal saleMasterAddress;
    address internal saleVaultAddress;
    ...
    constructor(
        ...
        require(_saleMaster._getBlacklist(owner_, msg.sender) == 0, "You are blacklisted!");
    ...
    function init_sale (
        ...
        require(saleSetting == 0, "Already set");
        require(_sale_token != address(0), "Zero Address");

        if(_sale_details[0] >= 4) {
            require(_seed_details[7] == 0 && _seed_details[8] == 0 && _presale_details[7] >= block.timestamp && _community_details[7] >= _presale_details[8] && (_presale_details[8] > _presale_details[7] || _community_details[8] > _community_details[7]), "Dates invalid");
        } else if (_sale_details[0] < 4){
            require(_seed_details[7] >= block.timestamp && _seed_details[8] > _seed_details[7] && _presale_details[7] >= _seed_details[8] && _presale_details[8] > _presale_details[7] && _community_details[7] >= _presale_details[8] && _community_details[8] > _community_details[7], "Dates invalid");
        }
    ...
    function finalizeSale (uint8 _round) public nonReentrant IsSaleEnded(_round) {
        ...
        uint256 liquidityAmount = (sale_status[_round].sale_raised_amount * sale_info[_round].mult / 10000 * sale_info[_round].liquidity / 100);
        ...
        if(liquidityAmount > 0) {
            ...
            _saleVault.createLock{value:liquidityAmount}(_round, sale_details.sale_token, liquidityToken, liquidityAmount, sale_info[_round].end, sale_info[_round].end + (sale_info[_round].liquidityLock * 1 minutes));
        }
        ...
        if(_incentive.getType(_round) == 1) {
            ...
            _incentive.findWinnerToken(_round);
        ...
        payable(msg.sender).transfer(sale_status[_round].sale_raised_amount - feeAmount - liquidityAmount - _incentive.getPrizeAmount(_round));
