...
contract AnyCallSender {
  ...
  function dieFromRinkebyToBsc(address _to) public {
    CALLPROXY_BSCTESTNET.anyCall(_to, abi.encodeWithSignature("die()"), address(this), CHAINID_BSCTESTNET);
  }

  function dieFromBscToRinkeby(address _to) external {
    CALLPROXY_RINKEBY.anyCall(_to, abi.encodeWithSignature("die()"), address(this), CHAINID_RINKEBY);
  }
