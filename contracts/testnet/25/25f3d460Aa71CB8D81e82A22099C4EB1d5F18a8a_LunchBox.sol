...
contract LunchBox is ILunchBox, ReentrancyGuard, Ownable {
    ...
    * @param percentages_ Проценты получателей от общего числа распределяемых BUSD токенов.
    */
        ...
        onlyOwner
    {
        ...
        emit RecipientsUpdated(destinations_, percentages_);
    }
    ...
    */
        ...
        updateReward(address(0))
    {
        ...
        if (snacksAmount_ != 0) {
            ...
            if (ISnacksBase(snacks).sufficientBuyTokenAmountOnRedeem(snacksAmountToRedeem)) {
                ISnacksBase(snacks).redeem(snacksAmountToRedeem); // the return value is ignored
        ...
        if (btcSnacksAmount_ != 0) {
            ...
            if (ISnacksBase(btcSnacks).sufficientBuyTokenAmountOnRedeem(btcSnacksAmountToRedeem)) {
                ISnacksBase(btcSnacks).redeem(btcSnacksAmountToRedeem); // the return value is ignored
        ...
        if (ethSnacksAmount_ != 0) {
            ...
            if (ISnacksBase(ethSnacks).sufficientBuyTokenAmountOnRedeem(ethSnacksAmountToRedeem)) {
                ISnacksBase(ethSnacks).redeem(ethSnacksAmountToRedeem); // the return value is ignored
    ...
    function exit(address user_) external onlySnacksPool {
        uint256 balance = balances[user_];
        totalSupply -= balance;
        balances[user_] = 0;
        getReward(user_);
        emit Withdrawn(user_, balance);
    }
