...
contract DogeApiV2 {
    ...
    function unstakeIds(address owner)
        public
        view
    ...
    // count total unstaked doges & sheeps 's olny ids by group
    function getUnStakeTokenIds(address owner)
        public
        view
        returns (uint256[] memory sheeps, uint256[] memory doges)
    {
        (
            uint256 amount,
            uint256 sheepCount,
            uint256 dogeCount
        ) = getUnStakeCount(owner);
    ...
    // page query unstakes details by page
    function unstakeDetails(
        address owner,
        uint256 start,
        uint256 size
    ) public view returns (Detail[] memory details) {
        uint256 amount = doge.balanceOf(owner);
        if (amount == 0 || amount <= start * size) {
            return details;
        }
        if (size == 0) size = 20;
        if (amount < size) {} else if (amount < (start + 1) * size) {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            details[i] = Detail({tokenId: _id, tokenURI: doge.tokenURI(_id)});
        }
    }

    // query staked sheeps&doges 's tokenId
    function stakedtokenIds(address owner)
        public
        view
        returns (uint256[] memory doges, uint256[] memory sheeps)
    {
        (
            uint256 amount,
            uint256 sheepCount,
            uint256 dogeCount
        ) = getStakedCount(owner);
        sheeps = new uint256[](sheepCount);
        doges = new uint256[](dogeCount);
        uint256 sheepIndex;
        uint256 dogeIndex;
    ...
    function stakedDogeIds(address owner)
        public
        view
        returns (uint256[] memory ids)
    {
    ...
    function stakedSheepIds(address owner)
        public
        view
        returns (uint256[] memory ids)
    {
    ...
    // query staked dogeDetails by page
    function stakedDogeDetails(
        address owner,
        uint256 start,
        uint256 size
    ) public view returns (Detail[] memory details) {
        ...
        details = new Detail[](amount);
        for (uint256 i = 0; i < amount; i++) {
            uint256 _id = dogeIds[(start * size) + i];
            details[i] = Detail({tokenId: _id, tokenURI: doge.tokenURI(_id)});
        }
    }

    // query staked SheepDetails by page
    function stakedSheepDetails(
        address owner,
        uint256 start,
        uint256 size
    ) public view returns (Detail[] memory details) {
    ...
    function getStakedCount(address owner)
        public
        view
        returns (
            uint256 amount,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < amount; i++) {
            if (isSheep(barn.tokenOfOwnerByIndex(owner, i))) sheepCount++;
            else dogeCount++;
        }
    }

    // count total unstaked doges & sheeps
    function getUnStakeCount(address owner)
        public
        view
        returns (
            uint256 amount,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < amount; i++) {
            if (isSheep(doge.tokenOfOwnerByIndex(owner, i))) sheepCount++;
            else dogeCount++;
        }
    }

    // Count staked doges & sheeps by page.  amount = sheepCount + dogeCount
    function getStakedCountPages(
        address owner,
        uint256 start,
        uint256 size
    )
    ...
    // mluti transfer doges & sheeps to target address
    function multiTransferTokens(address target, uint256[] memory tokenIds)
        external
    {
        require(target != address(0x0), "not to burn!");
        require(
            doge.balanceOf(msg.sender) >= tokenIds.length,
            "balance not enough"
        );
    ...
    // common query wool wating shearing
    function pendingWool(uint256 tokenId) public view returns (uint256) {
        require(
            doge.ownerOf(tokenId) == address(barn),
            "only query for staked doge&sheep "
        );
        if (isSheep(tokenId)) {
            return pendingSheepClaim(tokenId);
        } else {
            return pendingDogeClaim(tokenId);
        }
    }

    // just for caluating , exclude tax & burn & stolean
    function pendingSheepClaim(uint256 tokenId)
        public
        view
        returns (uint256 owed)
    {
        IBarn.Stake memory s = barn.barn(tokenId);
        if (barn.totalWoolEarned() < barn.MAXIMUM_GLOBAL_WOOL()) {
            owed =
                ((block.timestamp - s.value) * barn.DAILY_WOOL_RATE()) /
                1 days;
        } else if (s.value > barn.lastClaimTimestamp()) {
            owed = 0; // $WOOL production stopped already
        } else {
            owed =
                ((barn.lastClaimTimestamp() - s.value) *
                    barn.DAILY_WOOL_RATE()) /
                1 days; // stop earning additional $WOOL if it's all been earned
        }
    }

    // just for caluating , change with totalAlpha staked
    function pendingDogeClaim(uint256 tokenId)
        public
        view
        returns (uint256 owed)
    {
        ...
        IBarn.Stake memory s = barn.pack(alpha, index);
        owed = (alpha) * (barn.woolPerAlpha() - s.value);
    }

    function getStakeDoge(uint256 tokenId)
        public
        view
        returns (IBarn.Stake memory s)
    {
        uint256 alpha = _alphaForDoge(tokenId);
        uint256 index = barn.packIndices(tokenId);
