...
import {IDasunPair} from "./interfaces/IDasunPair.sol";
import {IDasunRouter02} from "./interfaces/IDasunRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by DasunSwap teams.
 */
contract DasunZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // DasunRouter interface
    IDasunRouter02 public dasunRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from DasunSwap)
     ...
     * @param _dasunRouter: address of the DasunRouter
    ...
    constructor(
        ...
        dasunRouter = IDasunRouter02(_dasunRouter);
    ...
    {
        address token0 = IDasunPair(_lpToken).token0();
        address token1 = IDasunPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IDasunPair(_lpToken).getReserves();
    ...
    {
        require(
            _token0ToZap == IDasunPair(_lpToken).token0() || _token0ToZap == IDasunPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IDasunPair(_lpToken).token0() || _token1ToZap == IDasunPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IDasunPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IDasunPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IDasunPair(_lpToken).totalSupply();
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IDasunPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "DasunRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "DasunRouter: INSUFFICIENT_B_AMOUNT");
...
import "./IDasunRouter01.sol";

interface IDasunRouter02 is IDasunRouter01 {
