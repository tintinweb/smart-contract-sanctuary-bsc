...
contract TokenFarm {
    ...
    struct TransactionInfo {
        ...
        uint256 amount; // How many tokens the user has provided.
        uint256 time;
        uint256 action; // 0 - stake / 1 - claim profit / 2 - claim staking
    }
    ...
    constructor(IERC20 _stakeToken) {
        ...
        // Init packages
        packages[1] = Package(6, 3, true);
        lockups[1] =  [5, 10, 15, 25, 35, 45, 60, 80, 100];

        packages[2] = Package(24, 6, true);
        lockups[2] =  [5, 10, 25, 40, 65, 100];

        packages[3] = Package(60, 12, true);
        lockups[3] =  [100];
    ...
    function getTransactionByProfileId(uint256 _profileId) public view returns(TransactionInfo[] memory) {
        return transactions[_profileId];
    }
    ...
    function stake(uint _amount, uint256 _packageId) public payable {
        ...
        profile.vestingEnd = block.timestamp + packages[_packageId].vestingTime * 5 minutes;
        ...
        trans.id = profileId;
        trans.user = msg.sender;
        trans.amount = _amount;
        trans.time = block.timestamp;
        trans.action = 0;
        transactions[profileId].push(trans);
    ...
    function claimProfit(uint256 _profileId) public {
        ...
        trans.id = _profileId;
        trans.user = msg.sender;
        trans.amount = remainProfit;
        trans.time = block.timestamp;
        trans.action = 1;
        transactions[_profileId].push(trans);
    ...
    function getCurrentStakeUnlock(uint256 _profileId) public view returns(uint256) {
        ...
        for(uint i = length - 1; i >= 0; i--){
            uint256 limitWithdrawTime = info.vestingStart + (i + 1) * 5 minutes;
