...
library Uint256Array {
	...
	function TrimRight(uint256[] memory _Array) internal pure returns (uint256[] memory _Return) {
		require(_Array.length < type(uint64).max, "Array: out of index range");
		uint256 count;
		for (uint256 i = 0; i < _Array.length; i++) {
			if (_Array[i] != 0) count++;
			else break;
...
library SmartMatrix {
	...
	function SelectAllChildrenNodeIDInNodeTest(
		...
		uint8 _Matrix,
		uint64 _Length
	) internal view returns (uint256 _NodeCount, uint256[] memory _Test, uint256 _TestLength, uint64 _MEMORY_LENGTH) {
		_MEMORY_LENGTH = type(uint64).max - 1; //_Length < type(uint64).max - 1 ? _Length : type(uint64).max - 1;
		uint256[] memory _NodeIDs = new uint256[](_MEMORY_LENGTH);
		uint256[] memory a = new uint256[](_MEMORY_LENGTH);
...
contract Tuktu is TuktuBase, ITuktu {
	...
	function TestSelectAllNode(
		...
		returns (
			...
			uint256 _TestLength,
			uint256 _MaxUint64,
			uint64 _MEMORY_LENGTH
		)
	{
		// uint256[] memory _NodeIDs = new uint256[](_Length);
		(_NodeCount, _Test, _TestLength, _MEMORY_LENGTH) = SmartMatrix.SelectAllChildrenNodeIDInNodeTest(
			Nodes[_NodeID],
			Nodes,
			_Matrix,
			_Length
