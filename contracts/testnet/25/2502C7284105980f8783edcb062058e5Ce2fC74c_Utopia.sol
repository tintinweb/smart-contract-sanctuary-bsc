...
contract Utopia is  Ownable {
    ...
    uint8[7] public grade_requirements = [0,3,8,6,5,3,2];
    mapping(address => uint256) public IDCARD;//身份id
    mapping(address => address[]) public recommended_address; //直推地址
    bool public pause;//暂停铸币
//    uint8[8] public rewardLadder = [5,20,10,10,5,5,3,2];//奖励阶梯 [0,7]
    uint8[8] public rewardLadder = [2,1,1,1,1,1,1,1];//奖励阶梯 [0,7]
    uint256 public totalBonus;//总奖金
    uint256 public historytotalBonus;//上次总奖金
    ...
    mapping(address=>uint256) public historyCommission;//个人直推奖励
    mapping(address=>uint256) public historyBonus;//个人领取奖励
    address[] public allUserAddress;//全部用户
    address[] public allJoinUserAddress;//全部用户
    address public newUserAddress;//最新用户
    uint256 public todayUtopiaUserNum;//今日乌托邦用户数量
    UtopiaInsurance public utopiaInsurance;
    address public operatingFund;//运作基金
    uint256 public lastTime;//最终结算时间
    bool public autoStatisticsReward;
    event AutoSR(bool flag);

    constructor(address utopiaToken_, address utopiaNFT_, address admin_, address utopiaRalationship_, address utopiaInsurance_, address operatingFund_) {
    ...
    function getAllUserAddressNum() public view returns(uint256){
        ...
        return allUserAddress.length > totalNftCount ? allUserAddress.length : totalNftCount;
    }
    ...
    function join() public {
        ...
        require(IDCARD[msg.sender] == 0, "[Utopia]you cant rejoin!");
        address recommender_address = getRecommender(msg.sender);//上级地址
        uint256 join_price = getMintPrices();
        require(join_price > 0, "[Utopia]Mint is not supported!");
        uint256 amount;
        if(utopiaToken_usdt){
            require(utopiaToken.balanceOf(msg.sender) >= join_price, "[Utopia]Your balance is insufficient balance to mint NFT!");
            ...
            utopiaToken.transfer(address(utopiaInsurance),amount);
            amount = (join_price*20)/100;
            utopiaToken.transfer(operatingFund,amount);
    ...
    function statisticsReward() public {
        bool autoSR = (msg.sender == owner() || msg.sender == admin) || (nowTime() > lastTime && lastTime != 0 && autoStatisticsReward);
        if(autoSR){
            uint256 totalNftCount = utopiaNFT.totalSupply();
            if(totalBonus > 0 && totalNftCount > 0){
                ...
                if(rAmount > 0){
                    if(utopiaToken_usdt){
                        utopiaToken.transfer(address(utopiaInsurance),rAmount);
                    }else{
                        usdt.transfer(address(utopiaInsurance),rAmount);
                    }
                }
