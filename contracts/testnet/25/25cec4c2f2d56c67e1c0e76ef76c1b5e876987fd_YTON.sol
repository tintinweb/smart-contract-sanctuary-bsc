...
contract YTON is Ownable{
	...
	//增加其他速率 （NFT 链游等）
	function setOtherFee (address addr,uint256 otherFee) public onlyMarket returns (uint256) {
	...
	returns(uint256,uint256,uint256,uint256,uint256,uint256,uint256)
	{
		...
		//生态池
		(uint256 daoHold,uint256 daoRate) = getUserDaoHoldAndRate(_userAddress);
		//当前可提取
		uint256 Available = getUserAvailable(_userAddress);
		//当前静态可提取
		(uint256 totalDividends,) = getUserDividends(_userAddress);
		//当前利率
		uint256 userPercentRate = getUserPercentRate(_userAddress);

		return (totalDep,totalWit,daoHold,daoRate,Available,totalDividends,userPercentRate);
	}
}
