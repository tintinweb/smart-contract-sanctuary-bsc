...
contract PrivateSale is Ownable, Vestable {
    ...
    function purchaseFromBnb(uint _saleIndex, uint256 _lastPrice, bytes memory signature) 
        public  
        payable
        throwIfPriceImpactTooHigh(_lastPrice)
    ...
    function purchaseFromQuill(uint256 _quillAmount) 
        external
    {
        require(msg.sender != address(0), "wallet is the zero address");
        require(_quillAmount > 0, 'QUT amount is zero');
        require(tokenPurchased[msg.sender].add(_quillAmount) <= purchaseLimit, "Individual cap exceeded");

        quill.transferFrom(msg.sender, owner(), _quillAmount);

        addVest(_quillAmount);
        emit TokensPurchased(msg.sender, _quillAmount);
    }

    function _distributeToken(uint256 _saleIndex, uint256 _token, address _sender) internal virtual {
        addVest(_saleIndex, _token);
    ...
    modifier throwIfPriceImpactTooHigh(uint _lastPrice) {
        uint256 latestPrice = getLatestPrice().toUint256(); 
        uint _priceImpact = latestPrice > _lastPrice ? latestPrice - _lastPrice : _lastPrice - latestPrice;
...
contract Vestable {
    ...
    mapping (uint256 => address) public vestToOwner;
    mapping (address => mapping(uint256 => bool)) public hasVest;
    ...
    function scheduleByVest(uint256 _index) public view returns (uint[] memory) {
    ...
    function getSchedule(uint32 _date, uint256 _index) public view returns (
        ...
        return (
            _getCurrentPeriod(_gmtMidnight(vest.dateStarted),  _date),
            _date,
            percentage,
            amount,
            dateClaimed,
    ...
    {
        Vest storage vest = vests[_index];
        uint withCliff = _gmtMidnight(vest.dateStarted) + CLIFF_IN_MINUTES;
        uint vestEndDate = withCliff + (CLIFF_IN_MINUTES * TOTAL_PERIOD); 
        return (
            vest.saleIndex,
            _index,
            vest.amount, 
    ...
    function addVest(uint256 _amount) internal {
        ...
        // hasVest[msg.sender][_index] = true;
    ...
    function claimVest(uint256 _index, uint32 _period) external {
        require(vestToOwner[_index] == msg.sender, 'Caller is not the vestor');
        require(_period > 0 && _period <= TOTAL_PERIOD, 'Invalid period');
        Vest storage vest = vests[_index];
        uint32 withCliff = _gmtMidnight(vest.dateStarted) + CLIFF_IN_MINUTES;
        uint32 releaseDate = withCliff + (CLIFF_IN_MINUTES * _period); 

        require(vest.amount != vest.claimedAmount, 'Already claimed');
        require(block.timestamp >= releaseDate, 'Function cannot call now');
        (
            ,,
            uint8 percentage,
            uint amount,,
            Status status
        ) = getSchedule(releaseDate, _index);
        ...
        isClaimed[_index][releaseDate] = true;
        claimedDate[_index][releaseDate] = block.timestamp;
    ...
    function _getCurrentPeriod(uint32 dateStartedGmtMidnight, uint _date) public view returns (uint256 currentPeriod) {
        uint256 timePassed = (_date - (dateStartedGmtMidnight + CLIFF_IN_MINUTES)) / 60;
        currentPeriod = timePassed / MINUTES_PER_PERIOD;
        if (currentPeriod > TOTAL_PERIOD ) { 
            currentPeriod = TOTAL_PERIOD; 
        } // prevents tokensToClaim to exceed its 100% value
    }
}
