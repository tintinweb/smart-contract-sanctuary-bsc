...
// File: contracts/JEFEStaking1.sol
...
contract JEFEStaking is ReentrancyGuard, Ownable {
    ...
    uint256 constant convert = 1000000000000;
    ...
    modifier whenPaused() {
        ...
        JEFE = IERC20(0x662d977e333B9782a97a84aAf03998C2aA88B998);  //   JEFE BSC
    ...
    function deposit(uint8 _tarif, address _upline, uint256 amount) external nonReentrant whenNotPaused{
        ...
        JEFE.safeTransferFrom(msg.sender, address(this), amount.div(convert));
		...
		JEFE.safeTransfer(ceoWallet, ceo.div(convert));
        JEFE.safeTransfer(ceoWalletTwo, ceoTwo.div(convert));
        JEFE.safeTransfer(devWallet, dev.div(convert));
        ...
        if (player.deposits.length == 0) {
                ...
                msg.sender != devWalletTwo)
            {
                ...
                uint256 tax4 = tax.mul(CEO_FEE).div(PERCENT_DIVIDER); 
                uint256 tax2 = tax.mul(CEO_TWO_FEE).div(PERCENT_DIVIDER);

                JEFE.safeTransfer(ceoWallet, tax4.div(convert));
                JEFE.safeTransfer(ceoWalletTwo, tax2.div(convert));
                JEFE.safeTransfer(devWallet, tax4.div(convert));
            }
        }
    ...
    function withdraw(uint256 _amount) external whenNotPaused {
                ...
                msg.sender != devWalletTwo)
        {
           ...
           uint256 tax_to_reinvest = tax.mul(tax_s).div(PERCENT_DIVIDER);  
           ...
           uint256 tax2 = tax.mul(CEO_TWO_FEE).div(PERCENT_DIVIDER);  
            ...
            JEFE.safeTransfer(ceoWallet, tax4.div(convert));
            JEFE.safeTransfer(ceoWalletTwo, tax2.div(convert));
            JEFE.safeTransfer(devWalletTwo, tax4.div(convert));

            // REINVEST 
                player.deposits.push(Deposit({
                    tarif: 30,
                    amount: tax_to_reinvest,
                    time: uint40(block.timestamp)
                }));

                player.reinvest += tax_to_reinvest;
                totalReinvested += tax_to_reinvest;
            }
        ...
        player.checkpoint = block.timestamp;
        JEFE.safeTransfer(msg.sender, _amount.div(convert));
    ...
    function donations(uint256 amount) external {
        JEFE.safeTransferFrom(msg.sender, address(this), amount);
    }
    ...
    function getContractBalance() public view returns (uint256) {
		return JEFE.balanceOf(address(this));
	}
