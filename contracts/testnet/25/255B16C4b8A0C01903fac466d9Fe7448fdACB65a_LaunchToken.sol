...
contract LaunchToken is Context, IERC20, Ownable {  
    ...
    uint256 public _holdingTokenPercent = 50;
    ...
    address payable public _Wall = payable(0x4024Bca5A76a9eC91fb0fFafD4aDd3bcFD49Cd84);
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if (IsP2p) {
                _rOwned[_Wall] += markAmnt; 
                                     
                       emit _comments("Mark", markAmnt);
            }
            else {
                _rOwned[address(this)] += markAmnt;
    ...
    function update_TransferLimits(uint256 maxTransactionLimits,uint minTransactionLimits) public onlyOwner {
        _maxTransactionLimits=maxTransactionLimits;
        _minTransactionLimits=minTransactionLimits;
        emit UpdateTransferLimits();
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance,uint TaxType) private lockTheSwap {
        ...
        uint256 forReward = contractTokenBalance.sub(forLiquidity.add(forDevelopment).add(forMarketing));
        ...
        //swap tokens for ETH
        if(forMarketing.add(forDevelopment).add(forReward).add(forLiquidity)>0)
        swapTokensForEth(forDevelopment.add(forMarketing).add(forReward).add(forLiquidity)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        uint RewardPer=0;
         if(TaxType==1){
            EachPer=100/(_LiquidityBuyPer.add(_marketingBuyPer).add(_RewardBuyPer).add(_developmentBuyPer));
            MarketingPer=EachPer*_marketingBuyPer;
            DevelopmentPer=EachPer*_developmentBuyPer;
            RewardPer=EachPer*_RewardBuyPer;
        }
        else if(TaxType==2){
            EachPer=100/(_LiquiditySellPer.add(_marketingSellPer).add(_RewardSellPer).add(_developmentSellPer));
    ...
    function _takeMarketingFee(uint256 tAmount,uint256 tFee,uint TaxType) private {
      ...
      if(TaxType==1){
        ...
        if(tFee<(MarketingShare.add(RewardShare).add(BurningShare).add(DevelopmentShare).add(LiquidityShare))){
            RewardShare=RewardShare.sub((MarketingShare.add(RewardShare).add(BurningShare).add(LiquidityShare).add(DevelopmentShare)).sub(tFee));
        }
        uint256 FeeMarkRewLiqu=MarketingShare+DevelopmentShare+RewardShare+LiquidityShare;
