...
contract NFTMagics is ERC721URIStorage, Ownable {
  ...
  // address payable public _walletAddress = "0xAAb63577527A1cE1725DCFD108528975fF6e1a37";
  ...
  uint256[] public ithdPriceWithHasTag = [
    ...
    3290000000000000000000,
    5980000000000000000000,
    11960000000000000000000
  ];
  ...
  function mint(
    ...
    if (_hastag) {
      _price = bnbPriceWithHasTag[count];
    } else {
      _price = bnbPrice[count];
    }

    if (_paytoken == ITHD) {
      if (_hastag) {
        _price = ithdPriceWithHasTag[count];
      } else {
        _price = ithdPrice[count];
      }
    }
    if (_paytoken == ARYA) {
      if (_hastag) {
        _price = ayraPriceWithHasTag[count];
      } else {
        _price = ayraPrice[count];
      }
    }

    if (_paytoken == WBNB) {
      require(msg.value >= _price, 'mint price is not correct');
      _contractOwner.transfer(_price);
    } else {
      require(
        IERC20(_paytoken).transferFrom(msg.sender, _contractOwner, _price),
    ...
    uint256[] memory tokensId = new uint256[](length);
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      approve(marketplace, newItemId); //grant transaction permission to marketplace
      ...
      tokensId[i] = newItemId;
      
      if(_sale_price > 0) {
        NFTMarketplace(marketplace).putItemForSale(newItemId, _sale_price);
      }
    }

    emit MintedNFT(msg.sender, tokensId);
...
contract NFTMarketplace {
  ...
  // function updateItemForSalePrice(uint256 id, uint256 price)
  //   external 
  //   returns (uint256){
  //     uint256 tokenId = itemsForSale[id].tokenId;
  //     require(!activeItems[tokenId], "Item is already up for sale");
  //     require(token.ownerOf(tokenId) == msg.sender, "Sender does not own the item");
  //     itemsForSale[id].price = price;

  //     emit updateItemSalePrice(id, price);
  //     return id;
  // }
