...
// pepperbird.finance
// 5/4/2022
// forked https://github.com/gr3yc4t/ERC20-Staking-Machine/blob/master/contracts/Staking.sol
...
contract PBIRDStaking is Ownable, ReentrancyGuard {
  ...
  function getAllAccount() external view onlyOwner returns (address[] memory) {
   ...
   */
  function getPotentialWithdrawAmount() external view onlyOwner returns (uint256) {
   ...
   */
  function stakeToken(uint256 _amount, address _referralAddress) external nonReentrant {
   ...
   */
  function withdrawFromPot(uint256 _amount) public nonReentrant returns (bool) {
    if (_amount > pot) {
      emit PotExhausted();
      return false;
    }
    ...
    )
  {
    ...
    return (selectedStake.deposit_amount, selectedStake.returned, selectedStake.stake_creation_time, myReferral, amountToWithdraw, penalty);
  }
   ...
   */
  function calculateRewardReferral(address customer) public view returns (uint256) {
    uint256 lowestStake;
    uint256 lowStakeID;
    ...
    uint256 currentReward = lowestStake.mul(_REFERALL_REWARD).mul(periods).div(100000);

    uint256 alreadyWithdrawed = account_referral[customer].referralAlreadyWithdrawed;
  ...
  function calculateTotalRewardReferral(address _account) public view returns (uint256) {
   ...
   */
  function getLowestStake(address customer) public view returns (uint256, uint256) {
  ...
  function calculatePenalty(uint256 _amountStaked) private pure returns (uint256) {
