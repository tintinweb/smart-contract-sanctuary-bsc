...
contract BranchV2 is ReentrancyGuard {
    ...
    // 存款配置
    bool public startFinalPaySwitch; // 缴纳尾款开关
    ...
    function userPreDeposit(uint256 amount, bool useCommentReward) external nonReentrant {
        ...
        require(block.timestamp - deposits[msg.sender][deposits[msg.sender].length - 1].createTimestamp >= depositInterval[accounts[msg.sender].level], "deposit interval limit of your level.");
    ...
    function userFinalDeposit(uint256 _index) external nonReentrant {
        require(startFinalPaySwitch, "startFinalPaySwitch off");
    ...
    function allocAmount(uint256 amount) private {
        ...
        // 处理重启池
        IERC20(usdt).safeTransfer(fuelRescue, (amount * 5) / 1000); // 处理援助池
    }
    ...
    function userCheckBlacklist(address addr) internal view returns (bool result) {
        ...
        for (uint256 i = 0; i < deposits_.length; i++) {
            if (deposits_[i].status == 20 && block.timestamp > deposits_[i].endFinalPayTimestamp) {
                return true;
            }
        }
    ...
    function userDealBlacklist(uint256 index) external {
        ...
        require(deposit.status == 20, "deposit status error.");
    ...
    // 生成{0-max}的随机数
    function createRandom(uint256 max) internal view returns (uint256) {
