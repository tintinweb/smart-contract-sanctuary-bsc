...
contract SwiftBit is Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(address partner) external payable whenStarted nonReentrant {
        ...
        if (stake[_msgSender()].percentage == 0) {
            ...
            stake[_msgSender()].percentage = 7;
            stake[_msgSender()].partner = partner;
        }
        emit StakeChanged(_msgSender(), stake[_msgSender()].partner, stake[_msgSender()].stake);
    }

    function withdrawOrReinvest(bool fromPartners, bool reinvest, uint256 amount) external whenStarted whenNotPaused nonReentrant {
        require(amount > 0, "Zero amount");
        uint256 pending = pendingReward(_msgSender());
        stake[_msgSender()].timestamp = block.timestamp;
        stake[_msgSender()].notWithdrawn += pending;
        _traverseTree(stake[_msgSender()].partner, pending);
        if (reinvest) {
            if (fromPartners) {
                require(amount <= stake[_msgSender()].fromPartners, "Balance too low");
                stake[_msgSender()].fromPartners -= amount;
            }
            else {
            ...
            payable(_msgSender()).transfer(amount - pending);
        }
    }
    ...
    function _traverseTree(address account, uint256 value) private {
        if (value != 0) {
            for (uint8 i; i < 5; i++) {
                ...
                stake[account].fromPartners += ((value * PERCENTAGES[i]) / 100);
