...
library SafeMath {
    ...
    //Guards Against Integer Overflows
    function safeDivison(uint256 a, uint256 b) internal pure returns (uint256) {
        if (a == 0) {
            return 0;
        } else {
            uint256 c = a / b;
...
contract GamelauncherIDOLaunchpad {
    ...
    function addProjectPhase(string memory _phaseId,string memory _projectId,uint _startUTCDateTime,uint _endUTCDateTime,uint256 _tokenPrice,uint256 _tokenSaleCap,uint256 _minimumBuyCap,uint256 _maximumBuyCap,bool _phaseStatus) public onlyOwner() {
        ...
        // projectphasedetail.numberOfTokens=_numberOfTokens;
    ...
    function getEstimatedTokenNeedInBNB(string memory _projectId,string memory _phaseId,uint256 _bnbworth) public view returns(uint256 _numberOfTokens){ 
        if (_bnbworth == 0) {
           ...
           uint256 _tokenprice=getProjectPhaseDetails[_phaseId].tokenPrice;
            uint256 safeDivison=uint256(10) ** getProjectDetails[_projectId].decimals;
            return SafeMath.safeDivison(_bnbworth, _tokenprice)/safeDivison;
        }
    }
