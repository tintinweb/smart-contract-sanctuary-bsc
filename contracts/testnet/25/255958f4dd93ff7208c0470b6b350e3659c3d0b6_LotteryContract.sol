...
contract LotteryContract is Ownable {
    ...
    event PurchaseTicket(uint256 indexed lotteryId, address indexed buyer, uint256 price, uint256 totalTickets, uint256[] ticketId, uint256 time);
    ...
    function buyTickets(uint256 _lotteryId, uint256[] memory _ticketNumber, uint256 _amount) public {
        ...
        emit PurchaseTicket(_lotteryId, msg.sender, amount, _ticketNumber.length, ticketIds, block.timestamp);            
    }
    ...
    function claimTickets(uint256 _lotteryId, uint256[] calldata _ticketIds, uint256[] calldata _brackets) external{
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            (,uint256 rewardForTicketId) = viewTicketIdRewards(_lotteryId, thisTicketId);
    ...
    function viewTicketIdRewards(uint256 _ticketId, uint256 _lotteryId) public view returns(uint256, uint256){
        ...
        for (uint256 i = 1; i <= 6; i++) {
            if(lotteryTicket[_ticketId].number % (10**(i)) == winningNumber % (10**(i))){
                j=i;
            }
        }
        if(j == 0){
    ...
    function viewCountWinnersInLottery(uint256 _lotteryId)external view returns (uint256[] memory){      
