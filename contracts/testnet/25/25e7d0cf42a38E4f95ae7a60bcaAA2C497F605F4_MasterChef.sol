...
contract MasterChef is Ownable {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(pending > 0) {
            _safeBountieTransfer(msg.sender, pending);
            emit Transfer(msg.sender, pending);
        }
        if(_amount > 0) {
    ...
    function leaveStaking(uint256 _amount) public {
        ...
        if(_amount > 0) {
            uint256 penalty = _calculatePenalty(msg.sender, _amount);
            uint256 totalWithdraw = user.amount.sub(penalty);
            // Send the penalty to bountie wallet
            user.amount = user.amount.sub(_amount);
            pool.lpToken.safeTransfer(devaddr, penalty);
            emit Transfer(devaddr, penalty);
    ...
    function calculatePenalty(uint256 _amount) external view returns (uint256) {
        UserInfo storage user = userInfo[0][msg.sender];
        require(user.amount <= _amount, "unable to calculate penalty more than your stake");
        return _calculatePenalty(msg.sender, _amount);
    }
...
contract BountieHunter is ERC20, Ownable {
    ...
    constructor() public ERC20 ("BOUNTIEIO", "BOUNTIE")
    {
