...
contract Implementation is Upgradeable {
    ...
    function setSaleAdmin(address _account) public notZeroAddress(_account) onlySuperAdmin notUsers(_account) {
        require(!blackList[_account] && !saleAdminList[_account] && !creatorAdmins[_account], "Implementation: Account was not set as sale admin");
    ...
    function setSaleAdmins(address[] memory _account) public onlySuperAdmin {
        ...
        for(uint256 i = 0; i < _account.length; i++ ){
            if(!saleAdminList[_account[i]] && _account[i] != address(0) && !isUser[_account[i]] && !blackList[_account[i]] && !creatorAdmins[_account[i]]) {
    ...
    function setCreatorAdmin(address _account) public notZeroAddress(_account) onlySuperAdmin notUsers(_account) {
        require(!blackList[_account] && !creatorAdmins[_account] && !saleAdminList[_account], "Implementation: Account was not set as creator admin");
    ...
    function setCreatorAdmins(address[] memory _account) public onlySuperAdmin {
        ...
        for(uint256 i = 0; i < _account.length; i++ ){
            if(!creatorAdmins[_account[i]] && _account[i] != address(0) 
            && !isUser[_account[i]] && !blackList[_account[i]] && !saleAdminList[_account[i]] 
            ) {
...
library HashUtils {
        ...
        returns (bytes32 hash)
    {
        ...
        index = index.writeUint256(_rentRequest.rentTime);
