...
contract Token is IERC20, Ownable, Pausable {
  ...
  // 1.5 billion
  uint256 private constant INITIAL_FRAGMENTS_SUPPLY = (10**9 + 5 * 10**8) * ONE_UNIT;
  ...
  uint256 public constant MAX_HOLDING_PERCENT_SELL_LIMIT_APPLIED = 9;
  ...
  // Sell limit
  uint32 public sellLimitDenominator = 10000;

  // 3rd party contracts
  IDEXRouter public router;
  IPriceOracle public priceOracle;

  // A transfer with an amount above 99% percent of a holder's balance will not pay for transfer fee
  uint256 public minPercentNoTransferFee = 99;
  uint256 public minFeeAmountToCollect = TOTAL_GONS / 1000;
  uint256 public minAmountToAddLiquidity = TOTAL_GONS / 1000;
  address public pair;
  // all time high price
  ...
  function _shouldApplyTransferFee(address sender, uint256 gonAmount) private view returns (bool) {
    ...
    uint256 balance = _gonBalances[sender];

    uint256 percent = gonAmount.mul(100).div(balance);
    if (percent >= minPercentNoTransferFee) {
      return false;
    }
  ...
  function calculateSellLimitFactor(uint256 holdingPercent) public view returns (uint256) {
    ...
    // y = -0.11x + 1.11
    // <=> 100y = 111 - 11x
    uint256 percentApplied = holdingPercent > MAX_HOLDING_PERCENT_SELL_LIMIT_APPLIED ? MAX_HOLDING_PERCENT_SELL_LIMIT_APPLIED : holdingPercent;
    uint256 sellLimitFactor = (111 - 11 * percentApplied);
    return sellLimitFactor;
  }

  function getDailySellLimitFactor(address _address) public view returns (uint256) {
    ...
    uint256 sellLimitFactor = calculateSellLimitFactor(holdingPercent);
   ...
   * If the contract can swapback, execute swapBack
   */
  function _rebase(uint256 twap) private {
    RebaseType rebaseType = RebaseType.POSITIVE;
    // triggerNegativeRebasePrice = athPrice - negativeFromAthPercent * athPrice
  ...
  function _takeFee(
    ...
    // burn tokens
    _gonBalances[DEAD] = _gonBalances[DEAD].add(feeAmount.div(feeDenominator).mul(burnFeePercent));
    console.log("burn");
    _gonBalances[address(this)] = _gonBalances[address(this)].add(feeAmount.div(feeDenominator).mul(treasuryFeePercent));
    console.log("to treasury");
    _gonBalances[autoLiquidityReceiver] = _gonBalances[autoLiquidityReceiver].add(feeAmount.div(feeDenominator).mul(autoLiquidityFeePercent));
    console.log("to liquidity");
  ...
  function _collectFeesAndSendToTreasury() internal swapping {
  ...
  function _checkDailySellLimitAndUpdateSaleHistory(address sender, uint256 amount) private {
    ...
    if (timeElapsed < 1 days) {
      ...
      uint256 factor = getDailySellLimitFactor(sender);
      uint256 dailySellLimitAmount = _totalSupply.mul(factor).div(sellLimitDenominator);
      require(amount <= dailySellLimitAmount, "EXCEEDS_DAILY_SELL_LIMIT");
      history.lastSoldTimestamp = block.timestamp;
      history.lastDailySellLimitAmount = dailySellLimitAmount;
      history.totalSoldAmountLast24h = amount;
    }
  }
  ...
  function _transferFrom(
    ...
    uint256 gonAmount = amount.mul(_gonsPerFragment);
    uint256 gonAmountToRecipient = shouldTakeFee(sender, recipient) ? _takeFee(sender, recipient, gonAmount) : gonAmount;
    console.log("gonBalances[sender]");
  ...
  function _shouldRebase() public view returns (bool) {
    return initRebaseStartTime != 0 && block.timestamp >= (lastRebasedTime + rebaseFrequency);
  }

  function _shouldAddLiquidity() internal view returns (bool) {
    return autoAddLiquidity && !_inSwap && msg.sender != pair && _gonBalances[address(this)] >= minAmountToAddLiquidity;
  }
  ...
  function checkMinAmountToAddLiquidity() external view returns (uint256) {
    return minAmountToAddLiquidity.div(_gonsPerFragment);
  }

  function checkMinFeeAmountToCollect() external view returns (uint256) {
    return minFeeAmountToCollect.div(_gonsPerFragment);
  }
  ...
  function setInitRebaseStartTime() external onlyOwner {
    uint256 currentTime = block.timestamp;
    initRebaseStartTime = currentTime;
  ...
  function _shouldCollectFees() private view returns (bool) {
    return autoCollectFees && !automatedMarketMakerPairs[msg.sender] && !_inSwap && _gonBalances[address(this)] >= minFeeAmountToCollect;
  }
  ...
  function setFeeSplit(
    ...
    require(autoLiquidityFeePercent + treasuryPercent + burnPercent == 100, "INVALID_FEE_SPLIT");
    autoLiquidityFeePercent = autoLiquidityFeePercent;
  ...
  // TODO: setDynamicDailySellLimitBrackets
