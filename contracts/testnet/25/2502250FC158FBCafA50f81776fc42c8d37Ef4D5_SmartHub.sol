...
//  █████ █████ █████ █████ █████ █████ █████ █████ █████ █████ █████ █████ █████
//
//  SmartHub - a modern smart-contract for safe and fast earning on BSC.
//
//  Daily rate: 1%
//  APR: 365%
//
...
contract SmartHub {
    ...
    uint256 constant internal LAUNCH_TIME = 0;
    ...
    address constant internal FEE_ADDRESS = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
    uint8 constant internal DEPOSIT_FEE = 15;
    uint8 constant internal WITHDRAW_FEE = 10;

    address constant internal DEFAULT_REFERRER = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
    ...
    function invest(address referrer) external payable nonReentrant onlyLaunched {
        ...
        uint256 new_deposit = users[msg.sender].deposit + (msg.value - fee);

        require(new_deposit <= MAX_DEPOSIT, "Deposit must be less than maximal deposit");
    ...
    function reinvest(uint256 amount) external nonReentrant onlyRegistered onlyLaunched {
        _chargeBalance(msg.sender);
        ...
        uint256 new_deposit = users[msg.sender].deposit + amount;
        require(new_deposit <= MAX_DEPOSIT, "Deposit must be less than maximal deposit");
    ...
    function withdraw(uint256 amount) external nonReentrant onlyRegistered onlyLaunched {
        _chargeBalance(msg.sender);
    ...
    function claim(uint256 amount) external nonReentrant onlyRegistered onlyLaunched {
        _chargeBalance(msg.sender);
