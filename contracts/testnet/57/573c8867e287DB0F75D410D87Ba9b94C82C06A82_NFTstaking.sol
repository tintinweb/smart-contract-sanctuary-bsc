...
contract NFTstaking is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public price; // tokenType => price
    mapping(uint256 => uint256) public TokenType; // id => tokenType
    ...
    modifier updateUserList {
        _;
        if (getUserAmount(msg.sender) > 0 || userInfo[msg.sender].pendingRewards > 0) _checkOrAddUser(msg.sender);
    ...
    modifier updateReward(bool withdrawal) {
        ...
        if (totalSupply > 0) {
            ...
            if(Math.min(block.timestamp, endTime) > lastUpdateTime) {
                multiplier = Math.min(block.timestamp, endTime).sub(lastUpdateTime);
            }
        ...
        uint256 pending = penaltyFlag ? 0 : userAmount.mul(accPerShare).div(1e12).sub(user.rewardDebt);
        user.pendingRewards = user.pendingRewards.add(pending);
        
        _;
        
        user.rewardDebt = userAmount.mul(accPerShare).div(1e12);
    ...
    function setTokenType(uint256[] memory _tokenIds, uint256 _type) external onlyOwner {
        for(uint i = 0; i < _tokenIds.length; i ++) {
            TokenType[_tokenIds[i]] = _type;
        }
    }

    function updateRewardToken(IDigitalaxNFT _parentNFT) external onlyOwner {
        parentNFT = _parentNFT;
    }
    ...
    function getUserAmount(address _user) internal view returns (uint256 amount) {
        UserInfo storage user = userInfo[_user];

        for (uint i = 0; i < user.tokenIds.length; i++) {
           amount += price[TokenType[user.tokenIds[i]]];
        }
    }

    function stakeAll() external updateReward(false) {  
    ...
    function stake(uint256 _tokenId) external updateReward(false) {
        require(parentNFT.ownerOf(_tokenId) == msg.sender, "only owner can stake!");
        _stake(_tokenId);
    }

    function _stake(uint256 _tokenId) internal nonReentrant whenNotPaused updateUserList {
        require (block.timestamp < endTime, "expired");
        
        UserInfo storage user = userInfo[msg.sender];
        uint256 amount = price[TokenType[_tokenId]];
    ...
    function unstakeAll() external updateReward(true) {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function unstake(uint256 _tokenId) external updateReward(true) {
        require(isExistTokenId(_tokenId) == true, "only owner can unstake!");
        _unstake(_tokenId);
    }

    function _unstake(uint256 _tokenId) internal nonReentrant updateUserList {
        UserInfo storage user = userInfo[msg.sender];
    ...
    function claimable(address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        if (user.tokenIds.length == 0) return user.pendingRewards;
        ...
        uint256 amount = getUserAmount(_user);
    ...
    function emergencyWithdraw() external onlyOwner {  
        parentNFT.setApprovalForAll(msg.sender, true);
    }
