...
import './ERC721A.sol';
import './Proxy/CollectionStorage.sol';
...
function editRound(uint256 _roundId, uint256 price, address seller, uint256 perPurchaseLimit, uint256 userPurchaseLimit, bool isPublic, uint256 startTime, uint256 endTime, uint256 maxSupply) onlyOwner public {
  require(_roundId < roundId.current(), "invalid roundId");
  require(roundInfo[_roundId].supply.current() == 0 || block.timestamp > roundInfo[_roundId].endTime, "can not edit");
  require( startTime < endTime, "invalid time");
  require(price > 0, "invalid price");
  
  roundInfo[_roundId] =  Round(startTime, endTime, price, seller, isPublic, perPurchaseLimit, maxSupply,userPurchaseLimit, roundInfo[_roundId].supply);
  emit AddRound(_roundId, price, seller, perPurchaseLimit, userPurchaseLimit, isPublic, startTime, endTime, maxSupply);
}
