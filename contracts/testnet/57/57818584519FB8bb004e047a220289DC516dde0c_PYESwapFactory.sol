...
contract PYESwapPair is IPYESwapPair, PYESwapERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, uint amount0Fee, uint amount1Fee, address to, bytes calldata data) external lock {
        ...
        uint balance0Adjusted = balance0.mul(10000).sub(amount0In.mul(adminFee0));
        uint balance1Adjusted = balance1.mul(10000).sub(amount1In.mul(adminFee1));
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'PYESwap: K');
        }
