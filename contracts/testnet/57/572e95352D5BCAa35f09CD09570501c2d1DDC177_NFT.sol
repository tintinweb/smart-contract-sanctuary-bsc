...
contract NFT is ERC721, Ownable {
    ...
    mapping (uint => address) public mintToken;
    mapping (uint => string) public hasTag;
    mapping (uint => bool) public listedMap;

    uint[] public bnbPrice = [2100000000000000,5200000000000000,10000000000000000,20000000000000000];
    uint[] public ayraPrice = [20000000000000000000,100000000000000000000,200000000000000000000,400000000000000000000];
    uint[] public ithdPrice = [29000000000000000000,149000000000000000000,299000000000000000000,598000000000000000000];
    uint[] public bnbPriceWithHasTag = [5400000000000000,11800000000000000,23000000000000000,46000000000000000];
    uint[] public ayraPriceWithHasTag = [60000000000000000000,220000000000000000000,400000000000000000000,800000000000000000000];
    uint[] public ithdPriceWithHasTag = [88000000000000000000,329000000000000000000,598000000000000000000,1196000000000000000000];
    ...
    function mintWithOutHas(string memory _tokenURI, address _toAddress, uint count , address _paytoken, bool _putOnSale) public payable returns (bool) {
        uint _price = bnbPrice[count];
        if(_paytoken == ITHD){
        ...
        if(_paytoken == WBNB){
            ...
            require(IERC20(_paytoken).transferFrom(msg.sender, _contractOwner , _price),"You don't have enough funds");
        }
        uint length = 1;
        if(count==1){
            length=5;
    ...
    function mintWithHas(string memory _tokenURI, address _toAddress, uint count , address _paytoken ,string memory _hash, bool _putOnSale) public payable returns (bool) {
        uint _price = bnbPriceWithHasTag[count];
        if(_paytoken == ITHD){
