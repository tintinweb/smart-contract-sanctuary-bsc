...
contract TDAOToken is IERC20, Ownable {
    ...
    uint256 private minTotal = 10000 * 1e18;

    uint256 private destroyTotal;

    uint256 private percent = 1000;

    uint256 private SELLMAX = 90;

    address public _marketAddress =
        address(0x07BFA99cC84B7B4AdA7aEc8a24B6b315eB28E2d9);
    ...
    function getMaxSell(uint256 tAmount, address sender)
        public
        view
        returns (bool)
    {
        return tAmount <= IERC20(this).balanceOf(sender).div(100).mul(SELLMAX);
    }
    ...
    function setSellMax(uint256 _sellmax) public onlyOwner {
        SELLMAX = _sellmax;
    }
    ...
    function _transfer(
        ...
        if (sender == uniswapV2Pair) {
            ...
            if (!_isExcludedFromFee[recipient]) {
                require(
                    tAmount <=
                        IERC20(this).balanceOf(sender).div(100).mul(SELLMAX),
                    "sell must limit lower 90 percent"
                );
            }
    ...
    function _tokenBuyTransferFee(
        ...
        if (fee) {
            ...
            uint256 ratesum = _takeInviterFee(
                sender,
                recipient,
                tAmount,
                currentRate
            ...
            uint256 destroyfee = tAmount.div(percent).mul(
                _inviterFee.sub(ratesum)
            );

            if (_tTotal.sub(destroyTotal).sub(destroyfee) > minTotal) {
                _takeTransfer(sender, _destroyAddress, destroyfee, currentRate);

                destroyTotal = destroyTotal.add(destroyfee);

                allFee = allFee + _inviterFee;
            } else {
                allFee = allFee + ratesum;
            }

            _rOwned[recipient] = _rOwned[recipient].add(
                rAmount.div(percent).mul(percent - allFee)
            );
            emit Transfer(
                sender,
                recipient,
                tAmount.div(percent).mul(percent - allFee)
            );
        } else {
            _rOwned[recipient] = _rOwned[recipient].add(rAmount);
            emit Transfer(sender, recipient, tAmount);
    ...
    function _tokenSellTransferFee(
        ...
        if (fee) {
            ...
            _takeTransfer(
                sender,
                uniswapV2Pair,
                tAmount.div(percent).mul(_liquidityFee),
                currentRate
            );

            allFee = allFee + _liquidityFee;

            uint256 destroyfee = tAmount.div(percent).mul(_destroyFee);
