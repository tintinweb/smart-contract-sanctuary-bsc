...
uint256 public costLobby2 =30000000000000000;
...
function LobbyTier2(uint256 IdTrack, uint256 _ID) public payable {
require(msg.value >= costLobby2+costLobby2/10,"Incorrect ticket price");
require(NFT.ownerOf(_ID) == msg.sender,"You are not the owner" );
require( MetaTire[_ID] == 2,"Your token has the wrong Tier 2" );
...
(bool success, ) = Commission.call{value: costLobby2/10 }("");
...
if(WTier1[w] == ArrTier1[a]){
    ...
    if(w==0){
        Prize( _BasePlayer[_IdTrack][a+1].Address, costLobby2*LobbyAmount/2);
    }
    if(w==1){
        Prize( _BasePlayer[_IdTrack][a+1].Address, costLobby2*LobbyAmount/100*35);
    }
    if(w==2){
        Prize( _BasePlayer[_IdTrack][a+1].Address, costLobby2*LobbyAmount/100*15);
    }
WTier1[w]=888888;
ArrTier1[a] =444444;
}}}}
...
function setcostLobby2(uint256 _costLobby2) public onlyOwner {
costLobby2 = _costLobby2;
}
}
