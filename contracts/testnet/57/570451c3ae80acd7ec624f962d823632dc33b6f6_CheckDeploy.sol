...
contract CheckDeploy is Context, IERC20, Ownable {  
    ...
    string private _name = "resteXYz";
    string private _symbol = "XYS";
    ...
    function RewrdDistri(uint256 contractTokenBalance) private  {
        ...
        emit comments ("Balance", Balance.div(10*10**18));
        uint256 SplitBNBBalance = Balance.div(_marketingPer.add(_RewardPer));
        emit comments ("RDSplitBNBBalance", SplitBNBBalance.div(10*10**18));
        ...
        emit comments ("RDRewardBNB", RewardBNB.div(10*10**18));
        marketingwallet.transfer(MarketingBNB);
        _rewardPool=_rewardPool.add(RewardBNB);
        emit comments ("RDReward Pool", _rewardPool.div(10*10**18));
    ...
    function _takeMarketingFee(uint256 tAmount,uint256 tFee) private {
        ...
        emit comments ("_tOwned[address(this)]", _tOwned[address(this)]);
        _tOwned[address(this)] = _tOwned[address(this)].add(contractTransferBalance);
        emit comments ("after adding more balance",_tOwned[address(this)]);
        _totalRewardCollected=_totalRewardCollected.add(RewardShare);
        emit comments ("RewardsCOllected", _totalRewardCollected);
        _totalMarketingCollected=_totalMarketingCollected.add(MarketingShare);
        emit comments ("MarketingRewardCollected",_totalMarketingCollected);
      }
