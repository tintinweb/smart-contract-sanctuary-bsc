...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => bool)) public onOrders;
    ...
    function getActiveOrderByCollectionAndTokenId(
        address _collection,
        uint256 _tokenId
    ) external view returns (Order memory) {
        Order memory result = Order(
            0,
            address(0),
            0,
            payable(address(0)),
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            if (
                (orders[i].collection == _collection) &&
                (orders[i].tokenId == _tokenId) &&
                (orders[i].isActive)
            ) return orders[i];
        }
        return result;
    ...
    function _createOrder(
        ...
        uint256 price,
        address currency
    ) internal onlyItemOwner(collection, tokenId) {
        require(acceptableCollections[collection], "Not acceptable collection");
        require(
            !onOrders[collection][tokenId],
        ...
        onOrders[collection][tokenId] = true;
        activeOrderCount++;
    ...
    function _cancelOrder(uint256 id) internal {
        ...
        onOrders[currentOrder.collection][currentOrder.tokenId] = false;
        activeOrderCount--;
    ...
    function buyOrder(uint256 id) external payable {
        Order storage order = orders[id];

        if (order.currency == address(0)) {
            require(msg.value >= order.price, "insufficient fund");
            (order.maker).transfer(msg.value);
        } else {
            require(
                IERC20(order.collection).allowance(msg.sender, address(this)) >=
                    order.price,
                "insufficient fund"
            );
            IERC20(order.collection).transferFrom(
                msg.sender,
                order.maker,
                order.price
            );
        ...
        onOrders[order.collection][order.tokenId] = false;
        activeOrderCount--;
        emit OrderDone(id);
    }
