...
contract GBC_Project{
    ...
    address public web_addr = 0x0c3a41A4A23A4C20c01be609441601bb525b0Bb4;// 平台地址
    ...
    constructor (address _addr)  public {
        ...
        P_Config.total = 120*1*10**decimals;// 150 USDT
        P_Config.standard = 12*1*10**decimals;// 20 USDT
        P_Config.sponsors = 0;
        P_Config.fee = 0;
        P_Config._amount = 0;
    }
    ...
    function invest(uint256 usdt_num) public returns (bool) {
        ...
        USDT.transferFrom(msg.sender, address(this), usdt_num);// USDT 扣款
        user_invest[msg.sender] = user_invest[msg.sender].add(usdt_num);
        sponsors_list[P_Config.sponsors] = msg.sender;// 投資人清單
        P_Config._amount = P_Config._amount.add(usdt_num);// 募資金額++
    ...
    function withdraw() public onlyOwner{
        ...
        if(P_Config._amount >= P_Config.standard)//已達低標
        {
            if(P_Config.fee >0 )
            {
                uint256 new_fee = (contract_balance*P_Config.fee)/100;// 給平台
                uint256 new_b = contract_balance.sub(new_fee); // 給專案發起人

                USDT.transfer(project_owner, new_b);
                USDT.transfer(web_addr, new_fee);
                P_Config._amount = 0; // 募資金額歸0
        ...
        else
        {
            ...
            for(uint i = 0; i < P_Config.sponsors; i++) 
            {
                ...
                if(invest_amount >0)
                {
                    ...
                    P_Config._amount = P_Config._amount.sub(invest_amount); // 減去退回的金額
                }
            }
        }
    }
