...
contract ProductFactory is FactoryERC721, Ownable, AccessControl {
    ...
    function whitelistAddresses(
        ...
        uint256[] memory eligibleQuanties
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 lootboxMinted;
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            if (
                eligibleLootboxMintQuantity[_addresses[i]].add(
                    eligibleQuanties[i]
                ) >
                maxMintQuantity ||
            ...
            eligibleLootboxMintQuantity[
                _addresses[i]
            ] = eligibleLootboxMintQuantity[_addresses[i]].add(
                eligibleQuanties[i]
            );
            eligibleProductMintQuantity[
                _addresses[i]
            ] = eligibleProductMintQuantity[_addresses[i]].add(
                eligibleQuanties[i]
            );
        }
    }

    function whitelistAddress(address _address, uint256 _eligibleQuantity)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        require(
            eligibleLootboxMintQuantity[_address].add(_eligibleQuantity) <=
                maxMintQuantity,
            "Exceeded max mint quantity"
        );
        require(
            totalLootboxMinted[_address].add(_eligibleQuantity) <=
                maxMintQuantity,
            "Reached max mint quantity"
        );
        eligibleLootboxMintQuantity[_address] = eligibleLootboxMintQuantity[
            _address
        ].add(_eligibleQuantity);
        eligibleProductMintQuantity[_address] = eligibleProductMintQuantity[
            _address
        ].add(_eligibleQuantity);
    }
    ...
    function batchMint(uint256 _optionId, address[] memory _toAddresses)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function changeMaxMintQuantity(uint256 _quantiy)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function mintLootbox(address _toAddress, uint256 quantity) public {
    ...
    function mintAll() public {
        for (uint256 i = 0; i < _addressBook[0].length; i++) {
            if (eligibleLootboxMintQuantity[_addressBook[0][i]] > 0) {
                mintLootbox(
                    _addressBook[0][i],
                    eligibleLootboxMintQuantity[_addressBook[0][i]]
                );
            }
        }
    }
    ...
    function mint(uint256 _optionId, address _toAddress) public override {
        ...
        if (_optionId == SINGLE_PRODUCT_OPTION) {
            ...
            eligibleProductMintQuantity[
                _toAddress
            ] = eligibleProductMintQuantity[_toAddress].sub(1);
            ...
            totalLootboxMinted[_toAddress] = totalLootboxMinted[_toAddress].add(
                1
            );
            eligibleLootboxMintQuantity[
                _toAddress
            ] = eligibleLootboxMintQuantity[_toAddress].sub(1);
    ...
    function _checkAllowMintLootBox(address _toAddress, uint256 quantity)
        internal
        view
    {
        require(
            eligibleLootboxMintQuantity[_toAddress] > 0,
            "Address is not eligible to mint lootbox"
        );
        require(
            quantity <= eligibleLootboxMintQuantity[_toAddress],
            "Mint quantity exceeded eligible quantity"
        );
        require(
            totalLootboxMinted[_toAddress].add(quantity) <= maxMintQuantity,
            "Exceeded max mint quantity"
        );
    }

    function _allowMintLootBox(address _toAddress, uint256 quantity)
        internal
        view
        returns (bool)
    {
    ...
    function _allowMintProduct(address _toAddress, uint256 quantity)
        internal
        view
        returns (bool)
    {
    ...
    function productBalanceOf(address _address)
        internal
        view
        returns (uint256)
    {
    ...
    function lootboxBalanceOf(address _address)
        internal
        view
        returns (uint256)
    {
    ...
    function lootboxAddress() public view returns (address) {
    ...
    function updateProduct(address _nftAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        nftAddress = _nftAddress;
...
contract Product is ERC721Tradable, ERC721Votes {
    ...
    constructor() ERC721Tradable("GSMC", "GSMC") {
    ...
    function baseTokenURI() public pure override returns (string memory) {
        return
            "ipfs://bafybeieoqjvnvnskct3gjejnv2q77li3g6xqzz2dkp2rlgluymxb7c2nii/";
    }
    ...
    function transferFrom(
        ...
        require(
            balanceOf(to) < productFactory.maxMintQuantity(),
            "Receiver has reached maximum quantity allowed"
        );
        _transfer(from, to, tokenId);
    }
    ...
    function updateFactory(ProductFactory _productFactory) public onlyOwner {
        productFactory = _productFactory;
        _setupRole(DEFAULT_ADMIN_ROLE, address(_productFactory));
    }

    function updateCurrentBaseTokenURI(string memory _baseTokenURI)
        public
        onlyOwner
    {
    ...
    function listOwnerTokens(address owner)
        public
        view
    ...
    function firstOwnerTokensOf(address owner)
        public
        view
        returns (uint256[] memory)
    {
