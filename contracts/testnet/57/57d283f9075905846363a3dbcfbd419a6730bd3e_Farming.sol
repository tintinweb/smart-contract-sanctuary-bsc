...
contract Farming is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    constructor() initializer {}
     ...
     function initialize() public initializer {
      ...
      function beforeDeposit(uint256 _pid, uint256 _amount) internal view {
         require(poolInfo.length < _pid, "Pool id is not found");
      ...
      function claimMyReward(uint256 _pid) external {
         ...
         require(poolInfo.length > _pid, "Pood not exist");
         require(block.timestamp > user.depositStartTime,"");
    ...
    function withdrawRewardTokenFromPool(uint256 _pid, uint256 _amount) external {
      (uint256 reward,) = calculateReward(_pid, msg.sender);
      require(reward  >= _amount,"Insufficient Reward");
