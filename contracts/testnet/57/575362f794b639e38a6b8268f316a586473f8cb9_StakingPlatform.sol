...
contract StakingPlatform is Ownable {
    ...
    function getLockPeriod(address addr) private view returns (uint256) {
        ...
        if ( _stakerInfor[msg.sender]._mode == 0 ) { lockPeriod =  _lockupTime[0]; }
        else if ( _stakerInfor[msg.sender]._mode == 1 ) { lockPeriod =  _lockupTime[1]; }
        else if ( _stakerInfor[msg.sender]._mode == 2 ) { lockPeriod =  _lockupTime[2]; }
        return lockPeriod;
    }
