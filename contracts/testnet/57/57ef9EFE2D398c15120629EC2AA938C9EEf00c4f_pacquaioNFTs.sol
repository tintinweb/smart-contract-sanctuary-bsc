...
contract pacquaioNFTs is Ownable, ERC721A, ReentrancyGuard {
    // // metadata URI
    ...
    uint256 public MAX_PUBSALE_SIZE_PER_ACCOUNT = 10;
    uint256 public MAX_TEAMSALE_SIZE_PER_ACCOUNT = 10;
    uint256 public MAX_WHITELISTSALE_SIZE_PER_ACCOUNT = 10;
    uint256 public MAX_PRESALE_SIZE_PER_ACCOUNT = 10;
    ...
    constructor() ERC721A("Pacquaio NFTs", "PQN", MAX_BATCH_SIZE, MAX_TOKENS) {
    ...
    function setBaseURI(string calldata baseURI) external onlyOwner {
        ...
        emit NewURI(baseURI, msg.sender);
    }
    ...
    function teamMint(uint256 _quantity) external payable isMintable(_quantity) {
        ...
        require(
          teamMinted[msg.sender] + _quantity <= MAX_TEAMSALE_SIZE_PER_ACCOUNT,
    ...
    function whitelistMint(uint _quantity, bytes32[] calldata _proof) 
        ...
        require(
          whitelistMinted[msg.sender] + _quantity <= MAX_WHITELISTSALE_SIZE_PER_ACCOUNT,
          "can not mint this many"
        );
    ...
    function preSaleMint(uint _quantity) 
        ...
        require(
          preSaleMinted[msg.sender] + _quantity <= MAX_PRESALE_SIZE_PER_ACCOUNT,
          "can not mint this many"
        );
    ...
    function publicSaleMint(uint256 _quantity)
        ...
        require(
          pubSaleMinted[msg.sender] + _quantity <= MAX_PUBSALE_SIZE_PER_ACCOUNT,
          "can not mint this many"
        );
        require(
    ...
    function getCurrentMintablePriceOfAccount(address _account, bytes32[] calldata _proof) 
        public view returns (uint256) {
        if (teams[_account] && 
            teamMinted[_account] < MAX_TEAMSALE_SIZE_PER_ACCOUNT)
            return teamMintPrice;

        if (isWhiteListed(_account, _proof) && 
            whitelistMinted[_account] < MAX_WHITELISTSALE_SIZE_PER_ACCOUNT)
