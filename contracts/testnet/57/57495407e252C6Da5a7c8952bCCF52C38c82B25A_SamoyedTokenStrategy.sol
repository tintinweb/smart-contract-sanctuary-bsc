...
contract SamoyedTokenStrategy is StratManager, FeeManager, GasThrottler, ReentrancyGuard {
    ...
    constructor(
        ...
        address _gasPrice,
        address _lottoPot
    ) StratManager(_router, _keeper, _strategist, _feeRecipient, _lottoPot) GasThrottler(_gasPrice) {
        want = IERC20(_want);
        masterchef = IMasterChef(_masterchef);
        wbnb = IERC20(_wbnb);

        wantToWbnbRoute = [_want, _wbnb];
        _giveAllowances();
    }
    ...
    function chargeFees() internal {
        ...
        uint256 amountToWbnb = want.balanceOf(address(this)).mul(35).div(1000);
        ...
        // send 28% of fee to caller address
        uint256 callFeeAmount = wbnbBalance.mul(callFee).div(MAX_FEE);
        wbnb.safeTransfer(tx.origin, callFeeAmount);

        // send 58% of fee to platform reserve
        uint256 feeAmount = wbnbBalance.mul(samoyedFee).div(MAX_FEE);
        wbnb.safeTransfer(feeRecipient, feeAmount);

        // send 14% of fee to strategist address
        uint256 strategistFee = wbnbBalance.mul(STRATEGIST_FEE).div(MAX_FEE);
        wbnb.safeTransfer(strategist, strategistFee);
    }

    // Store 50% of output to lotto pot
    function _collectLottoPot() internal {
        // Store 50% from reward token to Lotto Pot
        uint256 amountToWbnb = want.balanceOf(address(this)).mul(lottoPotRatio).div(MAX_POT_RATIO);
        ...
        uint256 wbnbBalance = wbnb.balanceOf(address(this));
        IVaultLotto(lottoPot).deposit(wbnbBalance);
    }
    ...
    function _giveAllowances() internal {
        ...
        want.safeApprove(router, type(uint256).max);

        wbnb.safeApprove(lottoPot, 0);
        wbnb.safeApprove(lottoPot, type(uint256).max);
    }

    function _removeAllowances() internal {
        want.safeApprove(address(masterchef), 0);
        want.safeApprove(router, 0);
        wbnb.safeApprove(lottoPot, 0);
    }
}
...
abstract contract FeeManager is StratManager {
    uint256 public constant STRATEGIST_FEE = 140;
    uint256 public constant MAX_FEE = 1000;
    uint256 public constant MAX_CALL_FEE = 280;
    ...
    uint256 public callFee = 280;
    uint256 public samoyedFee = MAX_FEE - STRATEGIST_FEE - callFee;

    uint256 public lottoPotRatio = 5000;
    uint256 public MAX_POT_RATIO = 10000;

    function setCallFee(uint256 _fee) external onlyManager {
        require(_fee <= MAX_CALL_FEE, "!cap");

        callFee = _fee;
        samoyedFee = MAX_FEE - STRATEGIST_FEE - callFee;
    }
    ...
    function setLottoPotRatio(uint256 _ratio) external onlyManager {
        require(lottoPotRatio <= MAX_POT_RATIO, "!ratio");

        lottoPotRatio = _ratio;
    }
}
...
abstract contract StratManager is Ownable, Pausable {
    ...
    address public lottoPot;

    event onSetLottoPot(address newLottoPot);
    ...
    constructor(
        ...
        require(_lottoPot != address(0), "Invalid lottoPot.");
    ...
    /**
     * @dev Updates lotto pot address.
     * @param _lottoPot new lotto pot address.
     */
    function setLottoPot(address _lottoPot) external onlyOwner {
        require(_lottoPot != address(0), "Invalid lottoPot.");
        lottoPot = _lottoPot;

        emit onSetLottoPot(lottoPot);
    }
