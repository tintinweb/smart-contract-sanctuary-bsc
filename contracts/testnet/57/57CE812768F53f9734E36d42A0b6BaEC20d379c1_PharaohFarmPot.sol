...
function getWithdrawalTaxRev(address userAddress) public view returns(uint256){ 
        uint256 initial = user_data[userAddress].initialDeposit;
        uint256 _totalSupply = CAFToken.balanceOf(address(this));
        uint256 balancePercentage = initial.mul(100).div(_totalSupply);
    ...
    function claimRewards() public {
        ...
        // uint256 _totalSupply = CAFToken.balanceOf(address(this));
        // uint256 initial = user_data[msg.sender].initialDeposit;
        uint256 withdrawTax = getWithdrawalTaxRev(msg.sender);
        ...
        if (myRew > myPayout) {
            CAFToken.safeTransfer(
                msg.sender,
                myPayout - ((myPayout *( withdrawTax + 10)) / 100)- claimed
            );
            ...
            CAFToken.safeTransfer(
                msg.sender,
             (   myPayout - claimed)- (((myPayout - claimed)  * (withdrawTax + 10)) / 100) 
