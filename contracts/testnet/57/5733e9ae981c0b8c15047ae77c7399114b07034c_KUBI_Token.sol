...
contract KUBI_Token is Context, IERC20, Ownable, PriceConsumerV3 {
    ...
    function buyPackByBNB(uint256 packNum) public payable {
        require(packNum > 0, "Need number of pack > 0!");
        require(_toBNB(packNum) <= msg.sender.balance, "balance is low");
        require(msg.value >= _toBNB(packNum), "Need to pay up!");
        _pack[msg.sender] = _pack[msg.sender].add(packNum);
        emit PackBuyDone(msg.sender, packNum);
    }

    function _toBNB(uint256 packNum) internal virtual returns (uint256) {
        return (packNum / getLatestPriceBNB()) * 10**10;
    }
