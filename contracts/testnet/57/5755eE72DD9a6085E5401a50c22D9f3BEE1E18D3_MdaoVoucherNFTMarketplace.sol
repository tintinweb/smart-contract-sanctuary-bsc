...
interface IMdaoVoucherNFTMarketplace {
    ...
    event ListItem(uint256 itemId, uint256 nftId, uint256 price, address seller, PaymentType payType, address payToken);
    event EditItem(uint256 itemId, uint256 price, address payToken);
...
contract MdaoVoucherNFTMarketplace is IMdaoVoucherNFTMarketplace, Ownable {
    ...
    modifier checkNFT(address _nftContract) {
        require(nftContracts[_nftContract], "Collection doesn't supported");
        _;
    }

    modifier checkAvailable(uint256 _itemId) {
        require(marketItems[_itemId].available, "Nft doesn't available");
        _;
    }
    ...
    function listItem(address _nftContract, uint256 _nftId, uint256 _price, PaymentType _payType, IERC20 _payToken) external checkNFT(_nftContract) {
        ...
        emit ListItem(marketItems.length - 1, _nftId, _price, msg.sender, _payType, address(_payToken));
    }

    function editItem(uint256 _itemId, uint256 _price, IERC20 _payToken) external checkAvailable(_itemId) checkSeller(_itemId) {
        marketItems[_itemId].price = _price;
        marketItems[_itemId].payToken = _payToken;

        emit EditItem(_itemId, _price, address(_payToken));
    }

    function removeItem(uint256 _itemId) external checkAvailable(_itemId) checkSeller(_itemId) {
    ...
    function buyItem(uint256 _itemId) external payable checkAvailable(_itemId) {
    ...
    function payToken(uint256 _itemId) internal {
        ...
        marketItems[_itemId].payToken.safeTransferFrom(msg.sender, marketItems[_itemId].seller, msg.value - feeAmount);
    }
