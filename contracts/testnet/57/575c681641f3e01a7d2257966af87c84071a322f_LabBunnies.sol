...
interface IResearchLab {
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256 availableUnlockedRewards, uint256 availableLockedRewards, uint256[22] memory rewardsByEra, uint256 adjVested);
...
contract LabBunnies is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    string[] private types = ["White Fur", "Black Fur", "Bloody", "Ghost", "Zombie"];
    string[] private eyes = ["Regular", "Angry", "Bloodshot", "Cat Eyes", "Eye Sockets"];
    string[] private mouths = ["Regular", "Foaming", "Bloody", "Vampire Teeth", "Jawless"];
    string[] private heads = ["Regular", "Witch Hat", "Knife", "Nails", "Exposed Brain"];
    ...
    uint8[] private claimingPower = [25, 35, 60, 100, 250];

    IResearchLab researchLab;
    IERC20 public erc20;
    ...
    constructor(address _ecto, address _researchLab)
    ...
    function random(address sender, string memory _tokenId, string memory _attr) private view returns(uint256){
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, sender, _tokenId, _attr)));
    }
    ...
    function randomOne(uint256 _tokenId, address sender) internal view returns (Attributes memory) {
        ...
        string memory __types = string(abi.encodePacked(types[0], types[1], types[2], types[3], types[4]));
        string memory __eyes = string(abi.encodePacked(eyes[0], eyes[1], eyes[2], eyes[3], eyes[4]));
        string memory __mouths = string(abi.encodePacked(mouths[0], mouths[1], mouths[2], mouths[3], mouths[4]));
        string memory __heads = string(abi.encodePacked(heads[0], heads[1], heads[2], heads[3], heads[4]));
        randomAttr.types = usew(mintingOdds,random(sender, _tokenId.toString(), __types)%1000);
        randomAttr.eyes = usew(mintingOdds,random(sender, _tokenId.toString(), __eyes)%1000);
        randomAttr.mouth = usew(mintingOdds,random(sender, _tokenId.toString(), __mouths)%1000);
        randomAttr.head = usew(headsOdds,random(sender, _tokenId.toString(), __heads)%1000);
        return randomAttr;
    }
    ...
    function mint(uint256 _quantity) public {
        require(
            isPaused == false,
             "Sale is not active at the moment"
        );
        require(
        ...
        if(minters[msg.sender].twoWeekMints >= maxPerMintCooldown){
            ...
            if(block.timestamp.sub(minters[msg.sender].timeLastMint) >= mintCooldown){
        ...
        uint256 mintsLeft = maxPerMintCooldown.sub(minters[msg.sender].twoWeekMints);

        // Quantity minted in this tx must be less or equal to the mints left
        require(
            _quantity <= mintsLeft,
            "Quantity exceeds mint limit"
        );

        erc20.approve(address(this), price.mul(_quantity));
        for (uint256 i = 0; i < _quantity; i++) {
            _safeMint(msg.sender, totalsupply());
            bunnies[totalSupply()].generation = 0;
            bunnies[totalSupply()].claimsLeft = claimsPerGen[0];
            bunnies[totalSupply()].attributes = randomOne(totalSupply(), msg.sender);
            bunnies[totalSupply()].attributes.image = "none";
            tokenId++;
        ...
        erc20.burnFrom(msg.sender, price.mul(_quantity));
    }
    ...
    function breeding(uint256 _tokenid1, uint256 _tokenid2) public {
        ...
        require(
            bunnies[_tokenid1].generation == bunnies[_tokenid2].generation, "They must be from the same generation"
        );
        require(
            bunnies[_tokenid1].generation != 4, "Gen4 cannot breed"
        );
        if(block.timestamp.sub(bunnies[_tokenid1].timeLastClaim) >= claimCooldown){
            bunnies[_tokenid1].onBreedCoolDown = false;
        }
        if(block.timestamp.sub(bunnies[_tokenid2].timeLastClaim) >= claimCooldown){
            bunnies[_tokenid2].onBreedCoolDown = false;
        }
        require(
            bunnies[_tokenid1].onBreedCoolDown == false && bunnies[_tokenid2].onBreedCoolDown == false, "Gen4 cannot mint"
        );
        require(
            bunnies[_tokenid1].totalBreeds < breedsPerGen[bunnies[_tokenid1].generation], "Exceeds total breeds"
        );
        require(
            bunnies[_tokenid2].totalBreeds < breedsPerGen[bunnies[_tokenid2].generation], "Exceeds total breeds"
        );
        ...
        erc20.approve(address(this), breedingPrice);
        _safeMint(msg.sender, totalsupply());
        bunnies[_tokenid1].totalBreeds++;
        bunnies[_tokenid2].totalBreeds++;
        bunnies[totalSupply()].generation = bunnies[_tokenid1].generation++;
        bunnies[totalSupply()].claimsLeft = claimsPerGen[bunnies[_tokenid1].generation];
        bunnies[totalSupply()].attributes = randomOne(totalSupply(), msg.sender);
        bunnies[totalSupply()].attributes.image = "none";
        tokenId++;
        erc20.burnFrom(msg.sender, breedingPrice);
    }
    ...
    function claim(uint256 _poolId, uint256 bunnyId) public nonReentrant {
        ...
        if(block.timestamp.sub(bunnies[bunnyId].timeLastClaim) >= claimCooldown){
        ...
        //uint256 pendingRewards = researchLab.pendingRewardsCalc(_poolId, msg.sender, block.number);
        uint256 bunnyClaimingPower = claimingPower[bunnies[bunnyId].attributes.types]+claimingPower[bunnies[bunnyId].attributes.eyes]+claimingPower[bunnies[bunnyId].attributes.mouth]+claimingPower[bunnies[bunnyId].attributes.head];
        uint256 lockedRewards;
        (, lockedRewards, , ) = researchLab.pendingRewards(_poolId, msg.sender);
        uint256 claimableAmount = lockedRewards.mul(bunnyClaimingPower).div(1000);
        // Sets cooldown
        ...
        // Claims the reward :)
        researchLab.safeLockedTransfer(msg.sender, _poolId, claimableAmount);
    }
    ...
    function recharge(uint256 _amount, uint256 bunnyId) public {
        ...
        require(
            bunnies[bunnyId].claimsLeft <= claimsPerGen[bunnies[bunnyId].generation],
            "You must have claims left to recharge"
        );

        // Adds claims per carrot
        for(uint256 i = 0; i < _amount; i++){
            bunnies[bunnyId].claimsLeft += claimsPerCarrotPercent.mul(claimsPerGen[bunnies[bunnyId].generation]).div(1000);
        }

        if(bunnies[bunnyId].claimsLeft > claimsPerGen[bunnies[bunnyId].generation]){
            bunnies[bunnyId].claimsLeft = claimsPerGen[bunnies[bunnyId].generation];
        }
        ...
        erc20.burnFrom(msg.sender, rechargePrice.mul(_amount));
    }
    ...
    function tokenURI(uint256 _tokenId) override public view returns (string memory) {
        return string(abi.encodePacked('data:application/json;base64,', Base64.encode(bytes(string(abi.encodePacked('{"name": "Lab Bunny #',_tokenId.toString(), '", "attributes": [{"trait_type":"Type", "value":"', types[bunnies[_tokenId].attributes.types], '"},{"trait_type":"Eyes", "value":"', eyes[bunnies[_tokenId].attributes.eyes], '"},{"trait_type":"Mouth", "value":"', mouths[bunnies[_tokenId].attributes.mouth], '"},{"trait_type":"Head", "value":"', heads[bunnies[_tokenId].attributes.head], '"}], "image":"',bunnies[_tokenId].attributes.image,'"}'))))));
        //return string(bytes(string(abi.encodePacked('{"name": "Lab Bunny #',_tokenId.toString(), '", "attributes": [{"trait_type":"Type", "value":"', types[bunnies[_tokenId].attributes.types], '"},{"trait_type":"Eyes", "value":"', eyes[bunnies[_tokenId].attributes.eyes], '"},{"trait_type":"Mouth", "value":"', mouths[bunnies[_tokenId].attributes.mouth], '"},{"trait_type":"Head", "value":"', heads[bunnies[_tokenId].attributes.head], '"}], "image":"',bunnies[_tokenId].attributes.image,'"}'))));
    }
