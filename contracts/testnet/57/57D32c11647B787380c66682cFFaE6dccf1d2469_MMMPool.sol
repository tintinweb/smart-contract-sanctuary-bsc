...
contract MMMPool is Context, Initializable {
    ...
    function isFCFSSecond() external view returns(bool) {
        return _isFCFSSecond;
    }
    function startFCFSSecond() external onlyOwner{
        _isFCFSSecond = true;
    }
    function startFCFS() external onlyOwner{
        ...
        _fcfsTime = block.timestamp;
        _state = 2;
        emit StartFCFS(_activePosCount, roundActiveNum, block.timestamp);
    }
    ...
    function fcfsTime() external view returns(uint256) {
        return _fcfsTime;
    }
    ...
    function fcfsSlot(address who) external view returns(uint256) {
        return _fcfsAmount[who];
    }

    function _joinPoolFCFS() internal {
        if(_type == 0) {
    ...
    function joinFCFS() external {
        require(_activePosCount < _endPos.sub(_startPos).add(1), "No slot to join");
        uint256 roundActiveNum = _endPos.sub(_startPos).add(1);
        require(_state == 2, "Not in FCFS");
        require(_fcfsAmount[msg.sender] < 2, "you join much FCFS");
        require(_activePosCount < roundActiveNum, "no slot");
        if(!_isFCFSSecond && !_isJoinedPool[msg.sender])
        ...
        emit JoinFCFS(msg.sender, _activePosCount, roundActiveNum, block.timestamp);
    }
    ...
    //only for debug
    function userSlots(address who) external view returns(uint256[] memory) {
    ...
    function getUsers(uint startPos, uint endPos) external view returns(address[] memory) {
        address[] memory users = new address[](endPos.sub(startPos).add(1));
        for(uint i = startPos; i<= endPos; i++) {
            users[i] = _posAddress[i];
        }
        return users;
    }
    ...
    event JoinFCFS(address who, uint256 activeCount, uint256 roundNum, uint256 time);
    ...
    event StartFCFS(uint256 activePosCount, uint256 roundActiveNum, uint256 time);
