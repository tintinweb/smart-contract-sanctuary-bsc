...
contract OpenGateMarketV3 is ERC721Holder, Ownable, Pausable, RoyaltiesLib {
    ...
    function cancelSellToken(address _nftAddress, uint256 _tokenId) public whenNotPaused {
        ...
        revokeBidByNft(_nftAddress, _tokenId);
    }
    ...
    function revokeBidByNft(address _nftAddress, uint256 _tokenId) private {
        // 撤回其余出价
        BidEntry[] memory bids = getBids(_nftAddress, _tokenId);
    ...
    // 撤回某一用户的出价
    function revokeBid(BidEntry memory be) private {
        address payable _address = payable(be.bidder);

        if (be.coin == address(0)) {
            _address.transfer(be.price);
        } else {
            quoteErc20 = IERC20(be.coin);
            quoteErc20.transfer(address(_address), be.price);
        }

        cancelBid(be.tokenAddress, be.tokenId, address(_address));
        emit CancelBidToken(address(_address), be.tokenAddress, be.tokenId);
    }
