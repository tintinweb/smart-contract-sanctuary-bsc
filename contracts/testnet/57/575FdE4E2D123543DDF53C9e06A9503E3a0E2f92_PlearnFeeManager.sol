...
contract PlearnFeeManager is Ownable {
    ...
    function processAllFee(bool ignoreError) external onlyOwner {
        address[] memory pairs = _getPairsForProcessFee();
        ...
        for (uint256 i = 0; i < pairs.length; ++i) {
            ...
            if (burnAmount > 0) {
                ...
                (uint amountAMin, uint amountBMin) = _getLiquidityTokenMinAmount(reserve0, reserve1, burnAmount, totalSupply);
                sendLP(pair);
    ...
    function _getPairsForProcessFee() public view returns (address[] memory pairs) {
    ...
    function _getLiquidityTokenMinAmount(uint _reserve0, uint _reserve1, uint _lpBalance, uint _totalSupply) public view returns (uint _amountAMin, uint _amountBMin) {
