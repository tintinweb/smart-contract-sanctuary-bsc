...
contract Dao is ReentrancyGuard, ERC20, ChainlinkClient, ConfirmedOwner {
    ...
    function request(uint256 _proposalId) external nonReentrant returns (bytes32 requestId) {
        Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);

        req.add('get', appendString(apiUrl, toAsciiString(address(this)), Strings.toString(_proposalId)));    // need change with chainlink vars
        req.add('path', 'id');
        req.add('path', 'status'); 
     ...
     */
    function fulfill(bytes32 _requestId, uint _id, bool _status) public recordChainlinkFulfillment(_requestId) {
        emit RequestQuorumStatus(_requestId, _id, _status);
        proposalPreQuorum[_id] = _status;
    }
