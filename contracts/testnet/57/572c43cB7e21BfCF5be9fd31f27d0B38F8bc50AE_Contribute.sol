...
contract Contribute is Context,Auth{
  ...
  function claimAll() external noReentrant() returns (bool) {
    ...
    while(i<thismounth())
    {
      ...
      (
        ...
        secshared_address_1[i]==msg.sender ||
        secshared_address_2[i]==msg.sender ||
        secshared_address_3[i]==msg.sender ||
        secshared_address_4[i]==msg.sender
      )
      {
  ...
  function getunclaim() external view returns (uint256) {
    ...
    while(i<thismounth())
    {
      ...
      {
        result = result.add(getclaimamount(msg.sender,i));
      }
      i++;
    }
    return result;
  ...
  function claimtopshared(address account,uint256 month) internal {
    ...
    require(
      topshared_address[month]==account ||
      secshared_address_1[month]==account ||
      secshared_address_2[month]==account ||
      secshared_address_3[month]==account ||
      secshared_address_4[month]==account
    );
