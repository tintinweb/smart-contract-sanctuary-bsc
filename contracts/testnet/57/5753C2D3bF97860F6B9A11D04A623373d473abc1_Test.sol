...
contract Test is DateTime{
    ...
    function addEntry(uint percent, uint startTime) external {
        uint16 year = getYear(startTime);
        uint8 month = getMonth(startTime);
        for(uint i=0; i<=1000;i+=percent){
            entries.push(toTimestamp(year,month,1));
            month++;
