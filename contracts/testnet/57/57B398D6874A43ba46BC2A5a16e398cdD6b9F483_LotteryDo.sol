    ...
    function setWinners(uint32 ID_) private {
        ...
        for (uint i=0;i<BlockID[ID_].valuePercantage.length;i++){
            ...
            for (uint a=0;a<winnersAmount;a++){
                ...
                BlockID[ID_].surplus-=prizeValue;
        ...
        function refundSurplus(uint32 ID_) private {
            uint surplusPerTicket = BlockID[ID_].surplus/BlockID[ID_].ticketsBought;
            for(uint32 i=1;i<BlockID[ID_].ticketsBought+1;i++){
                UserID[BlockID[ID_].ticketsOwner[i]].balance+=surplusPerTicket;
                BlockID[ID_].surplus-=surplusPerTicket;
        }
        ...
        emit RefundSurplus (ID_,BlockID[ID_].ticketsBought,surplusPerTicket);
    }
