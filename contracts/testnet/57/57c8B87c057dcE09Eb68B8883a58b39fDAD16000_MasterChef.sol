...
contract MasterChef is Ownable {
    ...
    function add(IERC20 _lpToken, uint256 _apr, uint256 _withdrawLockPeriod) public onlyOwner {
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256) {
        ...
        return user.amount.mul(rewardRate).mul(block.timestamp - user.lastDepositTime).div(1 days).mul(pool.apr).div(100);
    }
    ...
    function withdraw(uint256 _pid, bool _unstake) public {
        ...
        uint256 pending = pendingToken(_pid, msg.sender);
