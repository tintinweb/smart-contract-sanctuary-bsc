...
contract BNB1Daily is Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    // For checking msg.sender balanceOf Drip //
    IERC20 DripToken;

    uint256 public requiredripamount = 1; // Amount of DripNetwork Token msg.sender needs to write contract functions.
    uint256 public liquidityfee = 1000; // 1000 : 10 %. 10000 : 100 % --> Added to pool.
    uint256 public rewardPeriod = 1 days; // The rate you receive 1.5% apr.
    uint256 public withdrawPeriod = 4 weeks; // TimeLock period before user can withdraw funds.
    uint256 public apr = 100; // 100 : 1.0 %. 10000 : 100 %.
    ...
    constructor(address _DripAddress) {
        DripToken = IERC20(_DripAddress);
    }

    // deposit funds by user, add pool
    function deposit() external payable {
        require(msg.value > 0, "you can deposit more than 0 BNB");
        require(DripToken.balanceOf(msg.sender) >= requiredripamount );

        uint256 _id = _getNextDepositID();
        _incrementDepositID();

        uint256 depositFee = (msg.value * liquidityfee).div(percentRate);
