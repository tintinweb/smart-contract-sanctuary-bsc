...
contract CollectionsRepository is ICollectionsRepository {
     ...
     */
    function setCollectionFactory(address factory) external {
        require(msg.sender == _owner && _collectionFactory == address(0));
    ...
    function setTokenFactory(address factory) external {
        require(msg.sender == _owner && _tokenFactory == address(0));
...
contract OmniERC721 is IOmniERC721, ERC721URIStorage {
    ...
    constructor(
        ...
        if (params.metadataURIs.length > 0) _setMetadataURIs(params.metadataURIs);
        emit Created(address(this), creator);
    }
    ...
    function mint(address owner) override external {
        ...
        if (metadataURIs.length > 0) _setTokenURI(tokenIds, metadataURIs[tokenIds - 1]);
    }
    ...
    function _setDates(uint256 from, uint256 to) internal {
        if (from > 0) {
            require(from <= (block.timestamp + 92 days + 1 hours) && from >= (block.timestamp - 1 days));
            dropFrom = from;
        }
        if (to > 0) {
            require(to > from && to <= (from + 8 days));
            dropTo = to;
        }
    }

    /**
     * @notice Validates and sets metadata URIs of the tokens that belong to this collection.
     *
     * @param _metadataURIs Metadata URIs of the ERC721 tokens.
     */
    function _setMetadataURIs(string[] memory _metadataURIs) internal {
        uint256 _totalSupply = _metadataURIs.length;
        require(_totalSupply <= 10000);
        metadataURIs = _metadataURIs;
