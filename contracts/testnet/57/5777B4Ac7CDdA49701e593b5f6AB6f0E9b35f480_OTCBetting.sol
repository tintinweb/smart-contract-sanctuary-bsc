...
contract OTCBetting is Ownable {
    ...
    //Developer wallets
    address devWallet1 = 0x37782ffc9FD944f713BEd2642C788218422A8c00;
    address devWallet2 = 0x37782ffc9FD944f713BEd2642C788218422A8c00;
    uint revenue;
    ...
    function addNewHistory(address player, uint amount, uint action) internal {
        ...
        emit NewHistory(player, amount, block.timestamp, action);
    }
    ...
    function calculateLuckyMan(uint roomNumber) internal view returns(uint) {
        ...
        for(i = 0; i < 3; i ++) {
            ...
            t += status[player].chance[roomNumber];
        }
        uint ticker = block.timestamp % t;
        t = 0;
        for(i = 0; i < 3; i ++) {
            address player = rooms[roomNumber].players[i];
