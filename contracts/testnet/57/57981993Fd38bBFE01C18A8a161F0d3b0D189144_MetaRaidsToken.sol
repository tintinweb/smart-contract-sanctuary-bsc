...
contract MetaRaidsToken is Context, IERC20, Ownable {
    ...
    string private _name = "MetaRaids";
    ...
    address public _marketingWallet =
        payable(address(0x4912C2deE0E27c2dA0B81f1fca2C35E922a56da4));
    ...
    function setBuy() private {
        uint256 reduction = ((block.timestamp - tradingStartTime) / 4 weeks) *
            10;
        if (reduction >= 100) {
            removeAllFee();
        } else {
            _taxFee = buyFee.taxFee - uint16(reduction/2);
            _marketingFee = buyFee.marketingFee - uint16(reduction/2);
        }
    }
    ...
    function setSell() private {
        ...
        if (reduction >= 100) {
            ...
            _taxFee = sellFee.taxFee - uint16(reduction/2);
            _marketingFee = sellFee.marketingFee - uint16(reduction/2);
        }
    }
