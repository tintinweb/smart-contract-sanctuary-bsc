...
contract WATTS_bsc is AccessControl, Ownable, ERC20 {

    mapping (address => bool) public isWhitelisted;

    bool public isTransferBlocked = true;
    bool public isBlockRemoved;
    ...
    mapping(address => uint256) public userClaimNonce; // safeguard against reusing proofs attack
    ...
    constructor() ERC20("WATTS", "$WATTS") Ownable() {
    ...
    /** CLAIMING */
    function claim(uint256 claimAmount, uint256 nonce, bytes32[] memory proof) external {
        address recipient = msg.sender;
        require(proof.length > 0 && claimAmount != 0, "WRONG DATA PASSED");

        bytes32 leaf = keccak256(abi.encodePacked(recipient, claimAmount, nonce));

        require(nonce == userClaimNonce[recipient], "INCORRECT NONCE");
        require(MerkleProof.verify(proof, merkleRoot, leaf), "INVALID PROOF");

        userClaimNonce[recipient]++;
        _mint(recipient, claimAmount);

        emit ClaimedReward(recipient, claimAmount, block.timestamp);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (isTransferBlocked && from != address(0) && to != address(0)) {
            require(isWhitelisted[from] || isWhitelisted[to] || hasRole(MINTER_ROLE, msg.sender), "TOKEN TRANSFER BLOCKED");
        }
    }
    ...
    function addToWhitelist(address[] memory users) external onlyRole(ADMIN_ROLE) {
        require(users.length > 0, "ZERO ARRAY PASSED");
        for (uint i = 0; i < users.length; i++) {
            isWhitelisted[users[i]] = true;
        }
    }

    function deleteFromWhitelist(address[] memory users) external onlyRole(ADMIN_ROLE) {
        require(users.length > 0, "ZERO ARRAY PASSED");
        for (uint i = 0; i < users.length; i++) {
    ...
    function setMerkleRoot(bytes32 newmerkleRoot) external onlyOwner {
        ...
        emit setMerkleRootEvent(newmerkleRoot);
    }

    function toggleTransferStatus() external onlyOwner {
        require(!isBlockRemoved, "TRANSFER STATUS FINALIZED");
        isTransferBlocked = !isTransferBlocked;
    }

    function finalizeTransfer() external onlyOwner {
        require(!isBlockRemoved, "TRANSFER STATUS FINALIZED");
