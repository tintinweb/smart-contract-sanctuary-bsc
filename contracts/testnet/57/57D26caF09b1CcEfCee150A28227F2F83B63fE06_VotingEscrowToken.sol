...
/**
 * @title Voting escrow token for ChainHop. Based on xALPACA which is in turn based on Curve DAO.
 * Ported to Solidity from Vyper: https://github.com/curvefi/curve-dao-contracts/blob/master/contracts/VotingEscrow.vy
 */
contract VotingEscrowToken is ReentrancyGuard {
    ...
    event Withdrawn(address indexed locker, uint256 value, uint256 timestamp);
    event SupplyChanged(uint256 previousSupply, uint256 supply);
    ...
    /// @dev MAX_LOCK 208 weeks - 1 seconds
    uint256 public constant MAX_LOCK = (208 * WEEK) - 1;
    uint256 public constant MULTIPLIER = 10**18;

    /// @dev Token to be locked (CNO)
    address public token;
    /// @dev Total supply of CNO that is locked
    ...
    /// @dev Mapping (user => epoch) to keep track of which epoch user is at
    mapping(address => uint256) public userPointEpoch;
    /// @dev Mapping (round off timestamp to week => slopeDelta) to keep track of slope changes over epoch
    mapping(uint256 => int128) public slopeChanges;

    /// @notice ERC20 compatible variables
    string public name;
    string public symbol;
    uint8 public decimals;

    /**
     * @notice Initialize veCNO
     * @param _token The address of CNO token
     */
    ...
    /**
     * @notice Return the balance of veCNO at a given "_blockNumber"
     * @param _user The address to get a balance of veCNO
     * @param _blockNumber The specific block number that you want to check the balance of veCNO
     */
    ...
    function _checkpoint(
        ...
        if (_address != address(0)) {
            ...
            if (_newLocked.end != 0) {
                ...
                if (_newLocked.end == _prevLocked.end) {
                    ...
                    // Possibly when user deposited more CNO to his locker
        ...
        if (block.timestamp > _lastPoint.timestamp) {
            ...
            // Possibility when epoch = 0 or _blockSlope hasn't get updated in this block
            _blockSlope =
        ...
        if (_address != address(0)) {
            ...
            // We subtract _newSlopeDelta from `_newLocked.end`
     ...
     * @dev This will crate a new lock and deposit CNO to veCNO Vault
     * @param _amount the amount that user wishes to deposit
     * @param _unlockTime the timestamp when CNO get unlocked, it will be
     ...
     * @dev This function is used for deposit to created lock. Not for extend lock time.
    ...
    function depositFor(address _for, uint256 _amount) external nonReentrant {
        ...
        require(_lock.amount > 0, "lock not exist");
    ...
    /**
     * @notice Internal function to perform deposit and lock CNO for a user
     * @param _for The address to be locked and received veCNO
     * @param _amount The amount to deposit
     * @param _unlockTime New time to unlock CNO. Pass 0 if no change.
    ...
    function _depositFor(
        ...
        emit Deposited(_for, _amount, _newLocked.end, _actionType, block.timestamp);
        emit SupplyChanged(_supplyBefore, supply);
    }
    ...
    /**
     * @notice Do Binary Search to find the most recent user point history preceding block
     ...
     * @param _amount The amount of CNO to be added to the lock
     */
    function increaseLockAmount(uint256 _amount) external onlyEOA nonReentrant {
        ...
        require(_amount > 0, "bad _amount");
        require(_lock.amount > 0, "lock not exists");
        require(_lock.end > block.timestamp, "lock expired. please withdraw");
    ...
    /**
     * @notice Calculate total supply of veCNO (voting power)
     */
    function totalSupply() external view returns (uint256) {
    ...
    /**
     * @notice Calculate total supply of veCNO at specific block
    ...
    /**
     * @notice Calculate total supply of veCNO (voting power) at some point in the past
    ...
    /**
     * @notice Withdraw all CNO when lock has expired.
     */
    function withdraw() external nonReentrant {
        LockedBalance memory _lock = locks[msg.sender];

        require(block.timestamp >= _lock.end, "lock not expired");
        ...
        // _prevLock can have either block.timestamp >= _lock.end or zero end
