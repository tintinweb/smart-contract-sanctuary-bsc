...
contract Promote is Initializable, Member {
    ...
    modifier onlyPool{
        require(msg.sender == address(manager.members("NFTStakePool")), "err1"); // this function can only called by pool address!
        _;
    }
    ...
    function initialize(IERC20 _mca, IERC20 _usdt, IUniswapV2Pair _pair, address genesis) public initializer {
        __initializeMember();
        init();
        mca = _mca;
        usdt = _usdt;
    ...
    function update(uint256 amount) external validSender {
        if(block.timestamp >= roundTime[round] + 24 hours) {
            ...
            if (round > 0) {
                ...
                daliyInfo[round].totalDown2GenWeight = daliyInfo[round -1].totalDown2GenWeight;
