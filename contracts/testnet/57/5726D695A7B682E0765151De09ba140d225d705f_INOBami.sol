...
contract INOBami is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    mapping(address => bool) private isPaid;
    mapping(address => uint256) private userTotalPrice;
    ...
    function BuyNft(uint256 purchaseQuantity) public {
        ...
        userTotalPrice[msg.sender] = NftQuantityPerUser[msg.sender] * priceNft;
        amountNftToDraw-=purchaseQuantity;
        userIsBought[msg.sender] = true;
        
        emit BuyNftComplete(purchaseQuantity, msg.sender, userTotalPrice[msg.sender], amountNftToDraw);
    }

    //payment to claim
    function NftPayment() public{
        require(userIsBought[msg.sender] == true, "You must buy first");
        require(priorWhiteList[msg.sender]==true, "You are not in white list");
        require(isPaid[msg.sender]==false, "You have already paid");
        require(IERC20(tokenAddress).balanceOf(msg.sender)>=userTotalPrice[msg.sender], "Not enough balance to pay");
        IERC20(tokenAddress).approve(msg.sender,userTotalPrice[msg.sender]);
        IERC20(tokenAddress).safeTransferFrom(msg.sender,devWallet,userTotalPrice[msg.sender]);
    ...
    function priorClaimNft() external nonReentrant{
        ...
        require(block.timestamp <= endTimeClaim,"Claim time ended");
        require(isClaimed[msg.sender]==false, "You have claimed");
        require(isPaid[msg.sender]==true, "You should pay first");

        for(uint256 index = 0; index < NftQuantityPerUser[msg.sender]; index++)
        {
