...
contract QubeStake is Pausable {
    ...
    Roles.Role private _stakeAdmins;
    ...
    modifier onlyStakeAdmin() {
        require(isStakeAdmin(_msgSender()), "AdminRole: caller does not have the Admin role");
        _;
    }

    function isStakeAdmin(address account) public view returns (bool) {
        return _stakeAdmins.has(account);
    }
    ...
    function _addAdmin(address account) internal {
        _stakeAdmins.add(account);
    }

    function _removeAdmin(address account) internal {
        _stakeAdmins.remove(account);
    }
    ...
    function roiUpdate(uint256 newRoi) public onlyStakeAdmin {
        rewardRoi = newRoi;
    }

    function timeUpdate(uint256 _startTime,uint256 _endTime) public onlyStakeAdmin{
        startTime = _startTime;
        endTime =_endTime;
    }
