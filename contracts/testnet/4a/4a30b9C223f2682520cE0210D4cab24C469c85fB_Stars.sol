    ...
    ERC20VotesUpgradeable
{
    ...
    function bigBang(
        ...
        taxPercent = 90; // 90% of difference
        taxOn = tax;
    ...
    function buyRadiance(uint256 amount) internal returns (bool success) {
    ...
    function tryToBurnForGlow(uint256 amount) internal returns (bool success) {
        ...
        if (canBuy > 0) {
            ...
            // console.log("Max Buyable", maxBuyable);
            ...
            // console.log("Stars Glow Balance", glowBalance);

            IBasisAsset(glow).transfer(msg.sender, glowBalance);
            // uint256 userGlowBalance = IBasisAsset(glow).balanceOf(msg.sender);
            // console.log("User Glow Balance", userGlowBalance);
            emit BurnForGlow(msg.sender, maxBuyable, glowBalance);
        }
        if (amount > 0) {
            IBasisAsset(radiance).burn(amount);
            // uint256 newBalance = IBasisAsset(radiance).balanceOf(address(this));
