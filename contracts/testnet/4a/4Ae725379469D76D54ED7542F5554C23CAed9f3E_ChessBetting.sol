...
contract ChessBetting {
    ...
    function getWinCount() public view returns (uint) {
        uint count = 0;
    ...
    function getLostCount() public view returns (uint) {
        uint count = 0;
        for (uint i = 0; i < gameIds.length; i++) {
            string memory gameId = gameIds[i];
            if (games[gameIds[i]].player1 == msg.sender || games[gameId].player2 == msg.sender) {
                if (games[gameId].winner != msg.sender && games[gameId].winner != address(0)) {
                    count++;
                }
            }
        }
    ...
    function getLostAmount() public view returns (uint256) {
        uint amount = 0;
        for (uint i = 0; i < gameIds.length; i++) {
            string memory gameId = gameIds[i];
            if (games[gameIds[i]].player1 == msg.sender || games[gameId].player2 == msg.sender) {
    ...
    function setDraw(string memory _gameId) public {
        ...
        claims[msg.sender] = prize / 2;
        burnAmount = burnAmount / 4;
