...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function mintChiToken(address _addr, uint256 amount, uint256 flag) public {
        ...
        uint256 mintAmount = (currentNativeBalance - startFee) / unitFee;
        preMintableAmount = mintAmount;

        if (flag == 1) {
            IChiToken(chiAddress).mint(amount);
        } 
        else {
            IChiToken(chiAddress).mint(preMintableAmount);
        }
    }

    function setGasPriceAndBaseFee(uint256 _gasPrice, uint256 _baseFee) public {
        gasPrice = _gasPrice;
        baseGasFee = _baseFee;
    }
