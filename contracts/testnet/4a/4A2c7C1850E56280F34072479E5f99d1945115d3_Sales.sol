 ...
 */
contract Sales is ProxyBaseStorage, Ownable, SalesStorage{
...
modifier isValid() {
  ...
  _;
    ...
    function setVIPPASS(uint256 vipPassId, uint256 price, uint256 limit) public onlyOwner {
    ...
    function placeOrder(productDetail[] memory _productDetail, uint256 totalAmount ) payable public {
        require(status == 1, "sales not opended yet");
        uint256 amount = 0;
        uint256 price = 0;
        orderId++;
        for (uint256 index = 0; index < _productDetail.length; index++) {
            productDetail memory tempProduct = _productDetail[index];
            amount += tempProduct.amount;
            price += ( _vip[tempProduct.vipPassId].price * tempProduct.amount);
            order[orderId].productCount[tempProduct.vipPassId] = amount;
            userBought[msg.sender][tempProduct.vipPassId] += amount;
        }

        require(amount == totalAmount, "amount mismatch");
         uint256 depositAmount = msg.value;
        require(price <= depositAmount, "108");
        userOrders[msg.sender].push(orderId);
        order[orderId].buyer = msg.sender;
        order[orderId].amount = amount;
        payable(seller).call{value: price}("");
        emit PlaceOrder(_productDetail, orderId, totalAmount, price, msg.sender, seller);
    }
  ...
  // events
  event PlaceOrder(productDetail[] _productDetail, uint256 indexed orderId, uint256 totalAmount, uint256 totalPrice, address buyer, address seller);
}
