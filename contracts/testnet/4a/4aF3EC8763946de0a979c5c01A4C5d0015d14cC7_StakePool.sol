...
contract StakePool is IERC721Receiver {
    ...
    mapping(address => uint256[]) private desUserStakes;
    mapping(address => uint256) private numberOfDesUserStakes;
    ...
    function getTokenUserDesStakes(address user) external view returns (uint256[] memory tokenList) {

        uint256 _numberOfUserStakes = numberOfDesUserStakes[user];

        if (_numberOfUserStakes > 0) {
            tokenList = new uint256[](_numberOfUserStakes);

            uint256[] memory allTokenStaked = desUserStakes[user];
            uint256 index = 0;
            uint256 len = allTokenStaked.length;
    ...
    function mineDestroy(uint256 tokenId) external {
        ...
        numberOfUserStakes[msg.sender] -= 1;
        numberOfUserMineStakes[msg.sender] -= 1;
        desUserStakes[msg.sender].push(tokenId);
        numberOfDesUserStakes[msg.sender] += 1;
