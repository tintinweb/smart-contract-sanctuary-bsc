...
contract sShibDosh is IsHUG, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgPAW public gPAW; // additional staked supply (governance token)
    ...
    function setgPAW(address _gPAW) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gPAW) == address(0), "gPAW:  gPAW already set");
        require(_gPAW != address(0), "gPAW:  gPAW is not a valid contract");
        gPAW = IgPAW(_gPAW);
    }
    ...
    /**
        @notice increases rBONE supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sHUG of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sHUG
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sHUG: insufficient balance");
    }
    ...
    // toG converts an sHUG balance to gPAW terms. gPAW is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gPAW.balanceTo(amount);
    }

    // fromG converts a gPAW balance to sHUG terms. sHUG is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gPAW.balanceFrom(amount);
    }

    // Staking contract holds excess sHUG
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gPAW.balanceFrom(IERC20(address(gPAW)).totalSupply())).add(
