...
contract ArborSwapLock is IArborSwapLock, Ownable{
    ...
    constructor(address payable _feeAddr, uint256 _feeNormal, uint256 _feeVesting){
        feeAddr = _feeAddr;
    ...
    function lock(
        ...
        require(msg.value >= feeNormalLock, "Should be more or equal than current fee");
        id = _createLock(
            owner,
            token,
            isLpToken,
    ...
    function vestingLock(
        ...
        require(msg.value >= feeVestingLock, "Should be more or equal than current fee");
        id = _createLock(
            owner,
            token,
            isLpToken,
    ...
    function multipleVestingLock(
        ...
        require(msg.sender.balance >= feeVestingLock * owners.length, "Incuficient balance");
        require(msg.value >= feeVestingLock * owners.length, "Should be more or equal than current fee * number of locks");
        feeAddr.transfer(msg.value);
        return
