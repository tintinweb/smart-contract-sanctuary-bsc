...
contract Landian is ERC721, ERC721Enumerable, ReEntrancyGuard, Withdraw {
    ...
    /// @dev buy land
    function buyLand(uint256[] memory _lands)
        external
        noReentrant
        returns (uint256[] memory _NolandsMint)
    {
        /// @dev valid
        ...
        uint256 _amountTokens = _landsAmount * price;
        ...
        uint256[] memory _landsMint;

        /// @dev Mint the tokens to sender
        for (uint i = 0; i < _lands.length; i++) {
            /// @dev valid is the token is not already owned
            if (!_exists(_lands[i])) {
                _mint(_msgSender(), _lands[i]);
            } else {
                tokenReturn++;
                _landsMint[i] = _lands[i];
            }
        }

        /// @dev return the lands that can't be minted
        uint returnLand = tokenReturn * price;
        IERC20(addressLnda).transfer(_msgSender(), returnLand);
...
contract Withdraw is Ownable {
    ...
    address public addressLnda = address(0); // Address of the Lnda contract
    uint public price = 0; // Price of the token in wei
