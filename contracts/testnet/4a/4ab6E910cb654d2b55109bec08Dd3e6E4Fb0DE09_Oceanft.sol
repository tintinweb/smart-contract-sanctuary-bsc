...
contract Oceanft is ERC721Enumerable, TrustCaller {
    ...
    function tokenInfo(uint256 tokenId) external view returns (string memory collection, string memory rank, string memory uri){  
        require(_exists(tokenId), "token does not exist");
        return (tokenIdToCollection[tokenId], tokenIdToRank[tokenId], collectionRankURIMap[tokenIdToCollection[tokenId]][tokenIdToRank[tokenId]]);
    }

    function burn(uint256 tokenId) external {
        require(_exists(tokenId), "token does not exist");
        require(ERC721.ownerOf(tokenId) == _msgSender(), "burn token that is not owner");
        _burn(tokenId);
