...
contract Market is Ownable, IMarket {
    ...
    modifier canBuyToken(uint256 tokenId) {
       require(tokenSellingStatuses[tokenId], "Token has not been sold");
       _;
    }

    modifier canCancelToken(uint256 orderId) {
        Order memory order = orders[orderId];
        require(tokenSellingStatuses[order.tokenId], "Token has not been sold");
    ...
    function sellToken(uint256 tokenId, uint256 price, uint32 expireTime) override NFTContractMustSet tokenApproved(tokenId) canSellToken(tokenId) public {
    ...
    function buyToken(uint256 orderId) NFTContractMustSet override public payable {
        ...
        require(tokenSellingStatuses[order.tokenId], "Token has not beed sold");
        _execBuyToken(order.tokenId, orderId);
    }

    function _execBuyToken(uint256 tokenId, uint256 orderId) public payable tokenApproved(tokenId) canBuyToken(tokenId) {
    ...
    function cancelSellToken(uint256 orderId) public canCancelToken(orderId) {
        Order memory order = orders[orderId];
