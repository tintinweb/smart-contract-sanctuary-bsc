...
//import {IPancakeRouter01} from './resources/IPancakeRouter01.sol';
...
//import {IPancakeRouter02} from './resources/IPancakeRouter02.sol';
...
//import {IPancakePair} from './resources/IPancakePair.sol';
...
//import {IPancakePair} from './resources/TransferHelper.sol';
...
contract SwapPancakeswap is Ownable {
    ...
    */
    function tradeExactBNBForToken_pairMethod(address pairAddress, address receiver, uint256 deadlineOffset) external payable onlyOwner ensureOffset(deadlineOffset){
        ...
        require(IPancakePair(pairAddress).token0()==address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd) || IPancakePair(pairAddress).token1()==address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd), 'PAIR_DOES_NOT_HAVE_WETH');
    ...
    */
    function tradeBNBForExactToken_pairMethod(uint amountOut, address pairAddress, address receiver, uint256 deadlineOffset) external payable onlyOwner ensureOffset(deadlineOffset){
        ...
        // contract is a bit peculiar.
        // The way to deposit ETH(BNB) into the WETH contract is by the deposit function:
        IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).deposit{value: amountIn}();
        assert(IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).transfer(address(pairAddress), amountIn));
        // There is no need to approve (outside of this contract, i.e. bscscan) that this input token 0xc47e3819d527fD16de13dAfCF34F4FE50821665e can be spent by this contract SwapPancakeswap.
        ...
        if (msg.value > amountIn){
            TransferHelper.safeTransferETH(receiver, msg.value - amountIn);
        }
    }
    ...
    */
    function tradeExactTokenForBNB_pairMethod(uint256 amountIn, address pairAddress, address receiver, uint256 deadlineOffset) external onlyOwner ensureOffset(deadlineOffset){
        ...
        // Now I can perform the swap using the pair contract. but the peculiarity of this swap is that the receiver is going to be this contract SwapPancakeswap address and not the owner(sender/wallet)
        //(uint amount0Out, uint amount1Out) = inputIsToken0 ? (uint(0), amountOut) : (amountOut, uint(0));
        if(IPancakePair(pairAddress).token0()!=address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd)){
            IPancakePair(pairAddress).swap(uint(0), amountOut, /*owner()*//*msg.sender*/address(this), new bytes(0)); // receiver should be the the owner(sender/wallet)... but if a concatenation of pair swaps is performed, then the receiver should be the next pair address in the "path" and just the last swap would have the owner in the receiver parameter
        }else{
            IPancakePair(pairAddress).swap(amountOut, uint(0), /*owner()*//*msg.sender*/address(this), new bytes(0)); // receiver should be the the owner(sender/wallet)... but if a concatenation of pair swaps is performed, then the receiver should be the next pair address in the "path" and just the last swap would have the owner in the receiver parameter
        }

        require(IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).balanceOf(address(this)) > 0, 'INSUFFICIENT_WETH_AMOUNT_CONTRACT');
        require(IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).balanceOf(address(this)) >= amountOut, 'AMOUNTOUT_HIGHER_THAN_WETH_BALANCE');
        //require(IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).balanceOf(msg.sender) > 0, 'INSUFFICIENT_WETH_AMOUNT_ADDRESS');
        //require(IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).balanceOf(msg.sender) >= amountOut, 'AMOUNTOUT_HIGHER_THAN_WETH_BALANCE');

        // Approve that input token IERC20(path[0]) can be spent by the Router
        IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).approve(
            address(0xF855E52ecc8b3b795Ac289f85F6Fd7A99883492b),
            type(uint256).max //amountIn // type(uint256).max
        ); 
        ...
        IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).withdraw(amountOut);
        TransferHelper.safeTransferETH(receiver, amountOut);
        
    }

    function withdraw(uint wad) public {
        IWETH(address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd)).withdraw(wad);
    }
