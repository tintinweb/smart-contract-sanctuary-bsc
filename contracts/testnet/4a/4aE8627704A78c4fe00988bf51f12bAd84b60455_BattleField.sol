...
contract BattleField is VRFConsumerBase, ITrait, Ownable {
    ...
    uint256 public BOOST_BONUS = 10;
    uint256 public SKIP_COST = 10 ** 18;
    uint256 public BOOST_COST = 10 ** 18;
    ...
    event PlayerBoosted(address indexed _from, uint256 indexed _id, uint256 indexed _remain);
    event ItemBought(address indexed _from, uint256 indexed _type);
    ...
    function startChooseEnemy(uint256 tokenID) public {
        require(shibament.ownerOf(tokenID) == msg.sender, "You are not the owner of this nft");
    ...
    function getChosenEnemy(bytes32 requestId, uint256 random) internal {
        uint256 tokenID = randomRequest[requestId];
        setEnemy(characters[tokenID].owner, tokenID, random);
    }

    function setEnemy(address owner, uint256 tokenID, uint256 random) internal {
        ...
        emit EnemySelected(owner, tokenID, characters[tokenID].enemyID);
    }

    function skipEnemy(uint256 tokenID) external {
        require(shibament.ownerOf(tokenID) == msg.sender, "You are not the owner of this nft");
        require(characters[tokenID].state == STATES.PICKED, "Enemy not even choosed yet");
        address user = characters[tokenID].owner;
        require(players[user].skipChance > 0, "You have no chance to skip");
        players[user].skipChance -= 1;
        uint256 random = characters[tokenID].random / 100;
        setEnemy(msg.sender, tokenID, random);
    }

    function boostPlayer(uint256 tokenID) external {
        require(shibament.ownerOf(tokenID) == msg.sender, "You are not the owner of this nft");
        address user = characters[tokenID].owner;
        require(players[user].boostChance > 0, "You have no boost");
        require(characters[tokenID].isBoosted == false, "A potion is already in use");
        players[user].boostChance -= 1;
        characters[tokenID].power = characters[tokenID].power * (100 + BOOST_BONUS) / 100;
        characters[tokenID].isBoosted = true;
        emit PlayerBoosted(msg.sender, tokenID, players[user].boostChance);
    }
    ...
    function startFight(uint256 tokenID) external {
        ...
        if(characters[tokenID].isBoosted == true) {
            characters[tokenID].power = characters[tokenID].power * 100 / (100 + BOOST_BONUS);
            characters[tokenID].isBoosted = false;
        }
    }

    function buyItem(uint256 itemType) external {
        uint256 balance = shibatoken.balanceOf(msg.sender);
        uint currentTime = block.timestamp;
        if(itemType == 0) {
            require(currentTime - players[msg.sender].skippedTime >= 86400, "Time not reached");
            require(balance >= SKIP_COST, "Not enough money to buy item");
            shibatoken.transferFrom(msg.sender, address(this), SKIP_COST);
            players[msg.sender].skipChance += 3;
            players[msg.sender].skippedTime = currentTime;
        } else if(itemType == 1) {
            require(currentTime - players[msg.sender].boostedTime >= 86400, "Time not reached");
            require(balance >= BOOST_COST, "Not enough money to buy item");
            shibatoken.transferFrom(msg.sender, address(this), BOOST_COST);
            players[msg.sender].boostChance ++;
            players[msg.sender].boostedTime = currentTime;
        }
        emit ItemBought(msg.sender, itemType);
    }

    function calcWinPercent(uint256 tokenID) public view returns (uint256) {
        uint256 enemyID = characters[tokenID].enemyID;
    ...
    function getBattlePower(uint256 power, ELEMENT_TYPES element, ELEMENT_TYPES enemyElement) internal view returns(uint256) {
