...
contract GetdoneEscrow is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, AccessControlEnumerableUpgradeable, GetdoneApp{
    ...
    bytes32 public constant DISPUTATION_ROLE = keccak256("DISPUTATION_ROLE");
    ...
    event ContractDisputationSent(uint256 merchantId, uint256 appId, uint256 contractId, uint256 amount, address sender, address owner, bool isTalentVoted, bool isCustomerVoted);
        ...
        initializer
    {
        ...
        // _setupRole(DISTRIBUTOR_ROLE, sender);
        // _setupRole(DISPUTATION_ROLE, sender);
    ...
    function isDisputation(uint256 merchantId, uint256 appId, uint256 contractId, address customer)
        ...
        returns (bool)
    {
        return contractDisputations[merchantId][appId][customer][contractId].dispute == true;
    }
    ...
    function createMilestone(uint256 merchantId, uint256 appId, uint256 contractId, uint256 milestoneId, address customer, address talent, uint256 price, address erc20, uint256 orderId) 
        public
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        if (customer != address(0) && hasRole(DISTRIBUTOR_ROLE, _msgSender())) {
            _customer = customer;
        }
        ...
        if (amountPay > 0) {
            if (erc20 == address(0)) {
                ...
                uint256 a = amountPay;
                IERC20Upgradeable(erc20).safeTransferFrom(_customer, address(this), a);
            }
        }
        ...
        if (contractInfo.init == false) {
            ...
            contractInfo.milestoneIds.push(milestoneId);
        } else {
            contractInfo.status = CONTRACT_STATUS_PROCESSING;
            if (milestone.milestoneId == 0) {
                contractInfo.milestoneIds.push(milestoneId);
            }
            contractInfo.price = contractInfo.price + price;
        ...
        nonReentrant
    {
        ...
        emit MilestoneRequestPaidLess(merchantId, appId, contractId, milestoneId, contractInfo.talent, contractInfo.customer, milestone.payless, contractInfo.erc20, milestone.status);
    }
    ...
    function depositContract(uint256 merchantId, uint256 appId, uint256 contractId, address customer, address talent, uint256 price, address erc20, uint256 orderId) 
        public
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        if (erc20 == address(0)) {
            ...
            IERC20Upgradeable(erc20).safeTransferFrom(_customer, address(this), price);
        }
    ...
    function createDisputation(uint256 merchantId, uint256 appId, uint256 contractId, address customer, address owner) 
        external
        payable
        whenNotPaused
        nonReentrant
        returns (uint256)
    {
        require(hasRole(DISPUTATION_ROLE, _msgSender()), "GetdoneEscrow: caller is not the Distripution");
        require(validApp(merchantId, appId), "GetdoneEscrow: App is invalid");
        address _sender = _msgSender();
        ContractInfo storage contractInfo = contractInfos[merchantId][appId][customer][contractId];
        ContractDisputation storage contractDisputation = contractDisputations[merchantId][appId][customer][contractId];
        
        require((!contractDisputation.dispute), "GetdoneEscrow: contract cannot vote disputation");
        ...
        require((contractInfo.customer == owner || contractInfo.talent == owner), "GetdoneEscrow: can not change contract if sender has not made one");
        ...
        if (amountRefund > 0 && amountRefund <= contractBalance) {
            contractInfo.refund = contractInfo.refund + amountRefund;
            if (contractInfo.erc20 == address(0)) {
                payable(_sender).transfer(amountRefund);
            } else {
                // IERC20Upgradeable(contractInfo.erc20).approve(_sender, amountRefund);
                IERC20Upgradeable(contractInfo.erc20).safeTransfer(_sender, amountRefund);
            }
        }

        contractDisputation.contractId = contractId;
        contractDisputation.owner = owner;
        contractDisputation.dispute = true;
        contractDisputation.amount = amountRefund;
        contractInfo.isEndCustomer = true;
        contractInfo.isEndTalent = true;
        contractInfo.deposit = 0;
        contractInfo.refund = contractInfo.refund + amountRefund;

        emit ContractDisputationSent(merchantId, appId, contractId, amountRefund, _sender, owner, contractInfo.talent == owner, contractInfo.customer == owner);
        ...
        internal
    {
        ...
        if (erc20 == address(0)) {
            if (fee > 0) {
                payable(treasuryWallet(merchantId, appId)).transfer(fee);
            }
            ...
            if (fee > 0) {
                IERC20Upgradeable(erc20).safeTransfer(treasuryWallet(merchantId, appId), fee);
            }
            if (amount > 0) {
