...
contract INFLIVStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable{
	...
	function deposit(uint256 amount, uint256 plan) external nonReentrant{
	...
	function withdrawReward(uint256 plan) external nonReentrant{
		if(mapUserInfo[msg.sender][plan].amount > 0) 
		{
            uint256 pending = pendingReward(msg.sender, plan);
			if(pending > 0) 
	...
	function withdraw(uint256 plan) external nonReentrant{
	    if(mapUserInfo[msg.sender][plan].amount > 0) 
		{
            uint256 pending = pendingReward(msg.sender, plan);
			uint256 governanceTax = pending * governanceFee / 10000;
			...
			if(mapUserInfo[msg.sender][plan].startTime + stakingPeriod[plan] >= block.timestamp) 
			{
				...
				IGovernance(stakedToken).distributeFee(penalty);
