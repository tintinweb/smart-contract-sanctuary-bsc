...
import "./interfaces/INFTCapability.sol";

contract DragonaireNFTCore is
    Ownable,
    ...
    ERC721Pausable
{
    ...
    function increaseCap(uint256 amount_) public onlyRole(MANAGER_ROLE) {
    ...
    function updateBaseTokenURI(string memory baseTokenURI_) public onlyRole(MANAGER_ROLE) {
        _baseTokenURI = baseTokenURI_;
    }

    function mint(address to_) public onlyRole(MANAGER_ROLE) returns (uint256) {
     ...
     * @dev Batch mint
     */
    function mintBatch(address to_, uint256 amount_) public onlyRole(MANAGER_ROLE) {
     ...
     */
    function supportsInterface(bytes4 interfaceId_) public view virtual override(IERC165, AccessControlEnumerable, ERC721, ERC721Enumerable) returns (bool) {
    ...
    function updateTokenMetaData(uint256 tokenId_, uint256 artifacts_) external onlyRole(MANAGER_ROLE) {
    ...
    function updateTokenMetaData(uint256 tokenId_, uint256 artifacts_, bytes32 dna_) external onlyRole(MANAGER_ROLE) {
        require(_exists(tokenId_));
    ...
    function updateCapability(uint256 tokenId, uint256 capability) public onlyRole(MANAGER_ROLE) {
        require(_exists(tokenId));
        _items[tokenId].capability = capability;
    }
    ...
    function updateCapabilityInBatch(CapabilityUpdateRequest[] calldata requests) external onlyRole(MANAGER_ROLE) {
        for (uint256 i = 0; i < requests.length; i++) {
            updateCapability(requests[i].tokenId, requests[i].capability);
        }
    }

    function capabilityOf(uint256 tokenId) external view override returns (uint256) {
        return _items[tokenId].capability;
    }
}
...
interface INFTCapability is IERC721 {

    /// Retrieve capability of a token, returns 0 if the token does not exist.
    function capabilityOf(uint256 tokenId) external view returns (uint256);
}
...
interface IItemFactory {
    ...
    function addItem(
        uint256 boxType_, uint256 groupIndex_, uint256 itemType_, uint256 itemId_
    ) external;
