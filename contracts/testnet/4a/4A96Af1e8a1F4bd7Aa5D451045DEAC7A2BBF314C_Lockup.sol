...
interface IERC721 is IERC165 {
    ...
    function createToken(address recipient, string memory uri) external returns (uint256);
}
...
contract Lockup is Ownable {
    ...
    constructor () {
        ...
        NFToken = IERC721Metadata(0xA23239A7d6e04B730902C9b19f66621AC063A998);      // // NFT token address
        treasureWallet = 0xF0b6C436dd743DaAd19Fd1e87CBe86EEd9F122Df;
        rewardWallet = 0xe829d447711b5ef456693A27855637B8C6E9168B;
    ...
    function unStakeAll(address user) public {
    ...
    function _dealWithIrreversibleAmount(uint256 amount, string memory name) private {
        ...
        // generate NFT
        uint256 tokenId = IERC721Metadata(NFToken).createToken(_msgSender(), "https://gateway.pinata.cloud/ipfs/QmaFxL15oSodfwnpJ5exy3sHN6zb6v8wiCxhdL99Lj75Ak");
        bytes32 key = _string2byte32(name); 
        StakeInfo storage info = stakedUserList[key];
        // save NFT id
        info.NFTId = tokenId;
    ...
    function unclaimedAllRewards(address user) public view returns(uint256, uint256[] memory) {
        uint256 reward = 0;
        uint256[] memory resVal = new uint256[](userInfoList[user].length);
        for (uint256 i = 0; i < userInfoList[user].length; i++) {
            StakeInfo memory info = stakedUserList[userInfoList[user][i]];
            resVal[i] = calculateReward(info.name);
            reward += resVal[i];
        }
        return (reward, resVal);
    }

    function getLastClaimedTime(string memory name) public view returns(uint256) {
        require(isExistStakeId(name), "This id doesn't exist!");
        StakeInfo memory stakeInfo = stakedUserList[_string2byte32(name)];
        return stakeInfo.lastClaimed;
    }
    ...
    function getLastClaimedTimeList(address user) public view returns(uint256[] memory) {
        ...
        for (uint256 i = 0; i < userInfoList[user].length; i++) {
            StakeInfo memory info = stakedUserList[userInfoList[user][i]];
            resVal[i] = getLastClaimedTime(info.name);
        }
        return resVal;
    }
