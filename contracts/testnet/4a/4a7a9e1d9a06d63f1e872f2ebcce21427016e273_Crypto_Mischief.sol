...
contract Crypto_Mischief {
    ...
    address erctoken = 0xc46CCBE42Afdf64cc4DA7e56DCd60eE9bF1B743B; // Testnet BUSD
    ...
    constructor(address _dev1, address _dev2, address _charity, address _treasury, address _genesis, uint _startTime) {
    ...
    function buyEggs(address ref, uint256 amount) public nonReentrant {
        if(block.timestamp < startTime && msg.sender != genesis){
            ...
            if(totalDeposits < 1 && msg.sender != genesis){ //genesis will only hoard and fund the contract but won't count as investment.
            ...
            if(user.initialDeposit < 1 && msg.sender != genesis){ //new user! add count for new deposits only for precise record of data. genesis funding will not be counted as deposit.
                totalDeposits++; 
            }
