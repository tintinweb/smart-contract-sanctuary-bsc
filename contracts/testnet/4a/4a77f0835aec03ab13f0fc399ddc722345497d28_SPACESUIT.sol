...
TELEGRAM: https://t.me/spacesuitbsc
...
contract SPACESUIT is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "SPACESUIT";
    string private _symbol = "SS BSC";
    ...
    uint256 _buyBackFee; 
    uint256 public _buyBackFeeForBuying = 20;  
    uint256 public _buyBackFeeForSelling = 30;  
    
    uint256 _marketFee;
    uint256 public _marketFeeForBuying = 30; 
    uint256 public _marketFeeForSelling = 50;  
    
    uint256 _teamFee;
    uint256 public _teamFeeForBuying = 20;  
    uint256 public _teamFeeForSelling = 20;
    ...
    uint256 public sellCoolDownDuration = 30 seconds;
    ...
    function setAllBuyFees(uint256 _bliquidityFee, uint256 _bbuyBackFee, uint256 _bmarketFee,  uint256 _bteamFee) external onlyOwner {
        _liquidityFeeForBuying = _bliquidityFee;
        _buyBackFeeForBuying = _bbuyBackFee;
        _marketFeeForBuying = _bmarketFee;
        _teamFeeForBuying = _bteamFee;
    }

    function setSllSellFees(uint256 _sliquidityFee, uint256 _sbuyBackFee, uint256 _smarketFee,  uint256 _steamFee) external onlyOwner {
        _liquidityFeeForSelling = _sliquidityFee;
        _buyBackFeeForSelling = _sbuyBackFee;
        _marketFeeForSelling = _smarketFee;
        _teamFeeForSelling = _steamFee;
    }
    ...
    function enableCoolDown() external onlyOwner {
        buyCoolDownDuration = 30 seconds;
    }
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_liquidityFee).add(_buyBackFee).add(_marketFee).add(_teamFee)).div(1e3);
        return percentage;
    }
    ...
    function _takeAllFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tFee = tAmount.mul(_liquidityFee.add(_buyBackFee).add(_marketFee).add(_teamFee)).div(1e3);
    ...
    function manualSend() external onlyOwner {
        ...
        uint256 dvPercent = _teamFee.mul(1e4).div(totalPercent);

        // buybcak fee transfer
        buyBackWallet.transfer(deltaBalance.mul(bbPercent).div(totalPercent));
        // teamfee transfer
        teamWallet.transfer(deltaBalance.mul(dvPercent).div(totalPercent));
        // market fee transfer
