...
import "../interface/IChainLinkOracleSportDataMultiMatches.sol";
    ...
    IRegularCompetitionContract
{
    ...
    uint256 public constant decimalOfResult = 10000;
    ...
    function requestData() external onlyOpen {
        ...
        string[] memory comeptitionId = new string[](1);
        comeptitionId[0] = competition.competitionId;

        string memory queryString = getQueryString();

        requestID = IChainLinkOracleSportDataMultiMatches(oracle).requestData(
            comeptitionId,
            competition.sportTypeAlias,
            queryString
        );
    ...
    function _getResult() internal view returns (bytes32 _key, bool _success) {
        ...
        (uint256[][] memory _result, ) = IChainLinkOracleSportDataMultiMatches(
            oracle
        ).getData(requestID);

        uint256[] memory result = _result[0];
        if (result.length != betOptions.length) return (bytes32(0), false);
    ...
    function getQueryString() public view returns (string memory) {
...
interface IRegularCompetitionContractMultiMatches is
    Metadata,
    IRegularCompetitionContract
{
    struct CompetitionMulti {
    ...
    function setCompetition(
        CompetitionMulti memory _competition,
        address _sportManager
    ) external;

    function setBetOptions(MultiBet[] memory _betOptions) external;
}
...
interface ICompetitionPool {
    ...
    event UpdateAdmin(address _oldAdminAddress, address _newAdminAddress);
    ...
    event UpdateAirdropToken(address _airdropToken);
...
pragma solidity ^0.8.0;
import "../metadata/Metadata.sol";
import "../interface/IRegularCompetitionContractMultiMatches.sol";

interface IChainLinkOracleSportDataMultiMatches {
