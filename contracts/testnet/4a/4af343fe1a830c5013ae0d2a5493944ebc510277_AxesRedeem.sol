...
contract AxesRedeem is ReentrancyGuard, ERC721Holder, AccessControl {
    ...
    constructor() {
        ...
        contract721 = 0x99a0B81E5b62903e06944c78dD7B85eDfBf99A1A;
        running = true;
    }
    ...
    function redeem(address _from, uint256[] memory _ids) external nonReentrant returns (uint256 _redeemId) {
        ...
        redeems[redeemId].owner = _from;
        redeems[redeemId].time = block.timestamp;
        for (uint i; i < _ids.length; i++) {
            IERC721(contract721).safeTransferFrom(_from, address(this), _ids[i]);
            redeems[redeemId].burnedIds.push(_ids[i]);
        ...
        emit Redeem(redeemId, _from, redeems[redeemId].burnedIds);
        return redeemId;
    }

    function setRedeemNewIds(uint256 _id, uint256[] memory newIds) external returns (bool) {
        ...
        //require(redeems[_id].newIds.length == 0, "New ids already set for this redeem.");
        redeems[_id].newIds = newIds;
        return true;
    }

    function redeemIdInfo(uint256 _id) external view returns (address burnedOwner, uint256[] memory burnedIds, uint256[] memory newIds, uint256 time) {
        require(redeems[_id].burnedIds.length > 0, "Nonexistent redeem ID.");
