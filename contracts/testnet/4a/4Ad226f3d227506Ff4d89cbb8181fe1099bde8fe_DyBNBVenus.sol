...
abstract contract DyToken is ERC20, Ownable {
    ...
    function _deposit(uint256 amountUnderlying_) internal {
        ...
        if (_totalDeposit.mul(_totalSupply) == 0) {
            ...
            depositAverageRate[_msgSender()] = (_totalDeposit.add(amountUnderlying_)).mul(1e18).div(_totalToken);
        }
    ...
    function profitOfUnderlying(address lender_) public view returns (uint256) {
        uint256 _userBalance = balanceOf(lender_);
        if (_userBalance == 0) {
            return 0;
        }
        uint256 _depositAmount = _userBalance.mul(depositAverageRate[lender_]).div(1e18);
        uint256 _currentAmount = _userBalance.mul(totalDeposits()).div(totalSupply());
        return _currentAmount.sub(_depositAmount);
    }
