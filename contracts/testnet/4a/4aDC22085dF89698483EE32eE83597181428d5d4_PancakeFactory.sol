...
contract PancakePair is IPancakePair {
    ...
    function swap(address tokenA, address tokenB, uint256 coinNum, address to) external lock {
        ...
        if (tokenA == usdtAddress) {
            ...
            uint256 decimalA = IERC20(tokenA).decimals();
            uint256 decimalB = IERC20(tokenB).decimals();
            uint256 realNum = subFee(coinNum);

            uint256 getCoinNum = ((floatPrice - 2 * nowPrice + Math.sqrt((2*nowPrice-floatPrice)*(2*nowPrice-floatPrice) + 8 * floatPrice * realNum)) / (2 * floatPrice)) * (10 ** decimalB);
            nowPrice = nowPrice + getCoinNum * floatPrice/(10**decimalA)/(10**decimalB);
    ...
    function getSwapOut(address tokenA, address tokenB, uint256 coinNum) public view returns (uint256, uint256) {
        ...
        if (tokenA == usdtAddress) {
            ...
            uint256 getCoinNum = ((floatPrice - 2 * nowPrice + Math.sqrt((2*nowPrice-floatPrice)*(2*nowPrice-floatPrice) + 8 * floatPrice * realNum)) / (2 * floatPrice)) * (10 ** decimalB);
            uint256 _nowPrice = nowPrice + getCoinNum * floatPrice/(10**decimalA)/(10**decimalB);
