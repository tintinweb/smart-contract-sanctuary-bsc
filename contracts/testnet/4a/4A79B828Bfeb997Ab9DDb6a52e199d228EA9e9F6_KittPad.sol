...
contract KittPad is IKittPad, Ownable {
  ...
  function stakeNFT(uint256[] memory nftTokenIds) external {
    ...
    totalNftStaked.add(nftTokenIds.length);
  ...
  function unstakeNFT(uint256[] memory nftTokenIds) external {
    ...
    totalNftStaked.sub(nftTokenIds.length);
  }
  ...
  function distributeRewardForNft(address shareholder) internal {
    if (shares[shareholder].nftBoostTokenIds.length == 0) return;
    ...
    earnedRewards = earnedRewards.sub(rewardsExcluded);
  ...
  function distributeReward(
    ...
    rewards[shareholder].totalRealised[rewardsToken] = rewards[shareholder]
      ...
      .add(amount);
    rewards[shareholder].totalExcluded[
      rewardsToken
    ] = getCumulativeRewardsToken(shares[shareholder].amount, rewardsToken);
