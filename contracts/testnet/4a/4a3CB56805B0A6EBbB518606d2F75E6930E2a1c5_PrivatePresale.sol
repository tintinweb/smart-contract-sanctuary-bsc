 ...
 * Development : Digichain Technology
...
contract PrivatePresale is Ownable {
    address public GMX  = 0x209e0166740d76306D9Ff0881a39dCc41fD4067e;
    ...
    function allocate(uint256 _allocateAmount) launchStatus(false) external onlyOwner{
        IERC20(GMX).transferFrom(msg.sender, address(this), _allocateAmount);
    ...
    function allocatedAmount() public view returns(uint256 amount){
        return IERC20(GMX).balanceOf(address(this));
    }
    ...
    function finish() launchStatus(true) external onlyOwner{
        ...
        IERC20(GMX).transfer(owner(), IERC20(GMX).balanceOf(address(this)));
        emit presaleFinished();
    }
    
    function buyBybusd(uint256 _amout) launchStatus(true) external{
        ...
        IERC20(BUSD).transferFrom(msg.sender, address(this), _amout);
        IERC20(GMX).transfer(msg.sender, receivedTokens);
