...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    event NewRewardToken(IERC20Metadata rewardToken);
    ...
    */
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        bool underLock = isUnderLock(msg.sender);
        ...
        if (_amount > 0) {
            ...
            if(underLock){
                uint256 amountPenalty = amountToTransfer * lockPenalty / 100;
                stakedToken.safeTransfer(address(rewardAddress), amountPenalty);
                amountToTransfer = _amount - amountPenalty;
            }
     ...
     */
    function updateRewardToken(IERC20Metadata _rewardToken) external onlyOwner {
        rewardToken = _rewardToken;
    ...
    function isUnderLock(address _user) public view returns (bool) {
        UserInfo storage user = userInfo[_user];
        return block.timestamp < (user.lastStakedTime + lockDays * 1 days);
    }
...
contract SmartChefFactory is Ownable {
     ...
     */
    function deployPool(
        IERC20Metadata _stakedToken, //0x4c563d1d4bb131c388c21da5680c31489035e3f7 
        IERC20Metadata _rewardToken, //0x4c563d1d4bb131c388c21da5680c31489035e3f7
        uint256 _rewardPerBlock, // 10000000000
        uint256 _lockDays,
        uint256 _lockPenalty, // 10
        address _rewardAddress, //0x8e8924e90e3cf5a96cf47de3bac3169863493f5f
        address _admin
    ) external onlyOwner {
