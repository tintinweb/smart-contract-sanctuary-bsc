...
contract QWKM is IERC20, IERC20Metadata, Ownable {
    ...
    mapping(address => uint256) public _nextTime;

    uint public rate = 3; // 3%

    uint public time = 60*1; //60 * 60 * 24 * 30; // 30 days
    ...
    function _transfer(
        ...
        if (_isExcluded[from] != true && _isExcluded[to] != true) {
            ...
            require(block.timestamp > _nextTime[from], string(abi.encodePacked("Next transfer out time ", _nextTime[from].toString())));

            uint _max = _total[from].mul(rate).div(100);

            require(amount <= _max, string(abi.encodePacked("The maximum number of transfers that can be made this time ", _max.toString())));

          
           _nextTime[from] = block.timestamp + time;
