...
contract provideHelp is Context, Ownable {
    ...
    uint256 public currentGetHelp = 1;
    ...
    event clearGetHelpEvent(address indexed caller,uint256 contractBalance,uint256 paidAmount, address user);
    ...
    constructor()  {
        for (uint256 i = 0; i < 10; i++) {
            ...
            _currentUserID++;
            users[ownerWallets[i]].totalHelp.push(HelpPoolStruct({amount: amounts[i],paidAmount: amounts[i],isPaid: true,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp}));
        }
    }


    // pay 20% amount  
    function provideHelpFunction(uint256 amount ) public payable {
        ...
        users[_msgSender()].totalHelp.push(HelpPoolStruct({amount: amount,paidAmount: msg.value,isPaid: false,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp}));
        clearGetHelp();
    ...
    function completeHelp(uint256 helpPoolId) public payable{
        ...
        require( users[_msgSender()].totalHelp[helpPoolId].time.add(waitForPartnerTime)>block.timestamp, 'time over user block');
    ...
    function percentage(uint256 price, uint256 per) internal pure returns (uint256) {
     ...
     function clearGetHelp() internal{
        if(!getHelp[currentGetHelp].isPaid && getHelp[currentGetHelp].amount !=0 && address(this).balance != 0)
        {
            if((getHelp[currentGetHelp].amount-getHelp[currentGetHelp].paidAmount)<=address(this).balance)
            {
                payable(getHelp[currentGetHelp].userAddress).transfer(getHelp[currentGetHelp].amount-getHelp[currentGetHelp].paidAmount);
                getHelp[currentGetHelp].paidAmount=getHelp[currentGetHelp].paidAmount+(getHelp[currentGetHelp].amount-getHelp[currentGetHelp].paidAmount);
                getHelp[currentGetHelp].isPaid=true;
                currentGetHelp++;
                emit clearGetHelpEvent(_msgSender(),address(this).balance,getHelp[currentGetHelp].amount-getHelp[currentGetHelp].paidAmount,getHelp[currentGetHelp].userAddress);

            }
            else
            {
                payable(getHelp[currentGetHelp].userAddress).transfer(address(this).balance);
                getHelp[currentGetHelp].paidAmount=getHelp[currentGetHelp].paidAmount+address(this).balance;
                emit clearGetHelpEvent(_msgSender(),address(this).balance,address(this).balance,getHelp[currentGetHelp].userAddress);
            }
           
           users[_msgSender()].totalWithdrawalAmount=users[_msgSender()].totalWithdrawalAmount+msg.value;
        }
     }
...
function getUserInfo(address _addr)
        ...
        )
    {
        ...
        // for (uint256 i = 0; i < users[_addr].totalHelp.length; i++) {
        //     HelpPoolStruct storage dep = user.totalHelp[i];
        // }
