...
contract OraculaGames is Context, Ownable {
    ...
    function getAllBetsInEvent(uint256 _eventId) external view returns (address[] memory players, uint256[] memory outcomeIds, uint256[] memory betAmounts, uint256[] memory timestamps) {
        uint256 counter0;
        for (uint i; i < allPlayers.length; i++) {
    ...
    function getReverseOutcome(uint256 _eventId, uint256 _outcomeId) external view returns (uint256) {
    ...
    function transferBet(uint256 _eventId, address from, address to) external onlyBetMarket {
        ...
        require(playerInfo[_eventId][from].totalBet > 0);

        PlayerInfo storage _from = playerInfo[_eventId][from];

        uint256 betAmount = _from.totalBet;
        ...
        if (!addedToAllPlayers[to]) {
            allPlayers.push(to);
            addedToAllPlayers[to] = true;
        }
        _to.inGame = true;
        _to.totalBet = betAmount;
        _to.betAmount.push(betAmount);
        _to.timestamp.push(block.timestamp);
        _to.outcome = outcome;
