...
contract Lottery is Ownable {
    ...
    uint256 public numberWinners=3;
    string[] public tasksList;
    mapping(string => bool) isTaskSelected;
    ...
    function startLottery() public onlyOwner {
        ...
        tasksList = new string[](0);
        winners = new string[](0);
    }

    function addPlayers(string[] memory tasks) public {
    ...
    function declareWinner() public onlyOwner{
        ...
        for(uint256 i=0; i<numberWinners; i++) {
            bool flag = false;
            while(!flag) {
                uint256 index = i + uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp))) % (tasksList.length - i);
                if(isTaskSelected[tasksList[index]] == false)
                {
                    flag = true;
                    winners.push(tasksList[index]);
                    isTaskSelected[tasksList[index]]=true;                    
                }
            }
        }
    }

    function returnWinnersList() public view returns(string[] memory) {
        return winners;
    }
    ...
    function resetLottery() public onlyOwner {
        ...
        for(uint256 i = 0 ; i < winners.length; i++) {
            isTaskSelected[winners[i]]=false;
        }
        tasksList = new string[](0);
        winners = new string[](0);
    }
