...
contract PongHeroes is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
      ...
      function _tokenTransfer(address sender, address recipient, uint256 tAmount) private {
        ( uint256 sellerTransferAmount,uint256 buyerTransferAmount, uint256 totalTax) = _getValues(tAmount);
        // 10
        // 102 sender
        // 95  
        ...
        require(senderBalance >= sellerTransferAmount, "ERC20: transfer amount exceeds balance");
        unchecked {
            _balances[sender] = senderBalance - sellerTransferAmount;
        }
        lastTrans[sender] = block.timestamp;
        lastTrans[recipient] = block.timestamp;
        _balances[recipient] += buyerTransferAmount;
        ////

         emit Transfer(sender, sender, totalTax);
    }

    function _getValues(uint256 tAmount) private view returns (uint256, uint256,uint256) {
        (uint256 sellerTransferAmount,uint256 buyerTransferAmount, uint256 totalTax) = _getTValues(tAmount);
        return ( sellerTransferAmount,buyerTransferAmount, totalTax );
        }
   function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 buyFee = calculateSellTaxFee(tAmount);
        uint256 sellFee = calculateBuyTaxFee(tAmount);

        uint256 totalTax = buyFee.add(sellFee); 
        uint256 sellerTransferAmount = tAmount.add(sellFee); // 10 + 2%
        uint256 buyerTransferAmount = tAmount.sub(buyFee);
        return (sellerTransferAmount,buyerTransferAmount, totalTax);
    }
