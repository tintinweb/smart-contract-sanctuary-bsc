...
contract BEP20TokenA is Context, IBEP20, Ownable, PancakeTool {
    ...
    address public walletM;

    uint8 public _backFee = 4;
    uint8 public _mFee = 1;
    uint256 public _bFee = 1;
    uint256 public _endTotalSupply;
    ...
    constructor(
        TokenData.tokenInfo memory info,
        address owner
    ) {
        _name = info.name;
        _symbol = info.symbol;
        _decimals = info.decimals;
        _totalSupply = info.totalSupply;
        _endTotalSupply = info.endTotal;
        _balances[owner] = _totalSupply;
        _backFee = info.backFee;
        _mFee = info.mFee;
        _bFee = info.backFee;
        pancakeRouter = info.router;
        tokenPair = info.pair;
        tokenSize = 10 ** _decimals;
        swapMin = info.min * tokenSize;

        _distBack = new TokenDistributor(tokenPair);

        initIRouter(pancakeRouter, tokenPair);
        ...
        IBEP20(tokenPair).approve(pancakeRouter, ~uint256(0));
    ...
    function _transfer(
        ...
        if (sender != PancakePair && !swapLocking) {
            swapAndbackflow();
        }
        ...
        if (swapLocking) {} else {
            ...
            if (dutyFree[sender]) {
                payTaxes = false;
            } else if (sender == PancakePair && dutyFree[recipient]) {
                payTaxes = false;
            }

            if (payTaxes) {
                uint256 burnFee = amount.div(100).mul(_bFee);
                uint256 totalFee = amount.div(100).mul(_backFee + _mFee);
                if (totalFee > 0) {
                ...
                if (_totalSupply > _endTotalSupply && burnFee > 0) {
    ...
    function changeDutyFree(address account, bool isDuty)
        public
        onlyOwner
        returns (bool)
    {
        dutyFree[account] = isDuty;
        return true;
    }

    function swapAndbackflow() private lockTheSwap {
        uint256 balance = _balances[address(this)];
        if (_balances[PancakePair] / 100 < balance)
            balance = _balances[PancakePair] / 100;
        if (balance >= swapMin) {
            uint256 half = balance.div(_backFee + _mFee).mul(_mFee);
            uint256 otherHalf = balance.sub(half);
            swapTokensForTokens(half, tokenPair, walletM);
            backflow(otherHalf);
        }
    }
    ...
    function backflow(uint256 amount) private {
        ...
        uint256 oldBalance = IBEP20(tokenPair).balanceOf(address(this));

        swapTokensForTokens(half, tokenPair, address(_distBack));

        IBEP20(tokenPair).transferFrom(
            address(_distBack),
            address(this),
            IBEP20(tokenPair).balanceOf(address(_distBack))
        );

        uint256 riseBalance = IBEP20(tokenPair).balanceOf(address(this)) -
            oldBalance;
        addLiquidity(address(this), tokenPair, otherHalf, riseBalance);
    }

    function changeSwapMin(uint256 min) public onlyOwner returns (bool) {
