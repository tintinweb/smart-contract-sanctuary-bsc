...
contract SmartChef is Ownable, ReentrancyGuard {
    ...
    struct Share {
        ...
        uint256 totalExcludedMF;
        uint256 totalRealisedMF; 
    }

    // Dev address.
    address public devaddr;
    //address public rewardDistributor;
    address public moonshotMechanism;
...
// ----------------- BEGIN WBNB Variables -----------
    ...
    uint256 public unallocatedBountys;
    uint256 public totalShares;
    uint256 public totalBountys;
    uint256 public totalDistributed;
    uint256 public bountysPerShare;
    uint256 public bountysPerShareAccuracyFactor = 10 ** 36; // Keeping same accuracy factor in the MF Token Variables

// ----------------- BEGIN MF Token Variables -----------

    uint256 public unallocatedMFBountys;
    uint256 public totalBountysMF;
    uint256 public totalDistributedMF;
    uint256 public bountysPerShareMF;
    uint256 public totalStakedTokens;

// ----------------- END MF Token Variables -----------    
    ...
    function initialize(
        ...
        devaddr = _admin;
        moonshotMechanism = _moonshotMechanism;
    ...
    modifier onlyMoonshotMechanism {
        require(msg.sender == moonshotMechanism);
        _;
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && totalStakedTokens != 0) {
            ...
            uint256 fuelReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
            accTokenPerShare.add(fuelReward.mul(PRECISION_FACTOR).div(totalStakedTokens));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 fuelReward = multiplier.mul(rewardPerBlock);
        rewardToken.mint(devaddr, fuelReward.mul(devfee).div(10000));
        rewardToken.mint(address(this), fuelReward);
        accTokenPerShare = accTokenPerShare.add(fuelReward.mul(PRECISION_FACTOR).div(totalStakedTokens));
        lastRewardBlock = block.number;
    }
    ...
    function setShare(address keeper, uint256 amount) internal {
        ...
        shares[keeper].totalExcludedMF = getCumulativeBountysMF(shares[keeper].amount);
    }

    function depositWBNBToStakingContract(uint256 _amount) external onlyToken {
        if (totalShares == 0) {unallocatedBountys = unallocatedBountys.add(_amount); return; } 
        
        if (unallocatedBountys > 0) {
            uint256 amount = _amount.add(unallocatedBountys);
            ...
            unallocatedBountys = 0;
        } else {
            totalBountys = totalBountys.add(_amount);
            bountysPerShare = bountysPerShare.add(bountysPerShareAccuracyFactor.mul(_amount).div(totalShares));
        }    
    }
    ...
    function setNewMoonshot(address _newMoonshot) public onlyOwner {
        moonshotMechanism = _newMoonshot;
    }

    // function setNewDistributor(address _newDistributor) public onlyOwner {
    //     rewardDistributor = _newDistributor;
    // }

// ------------------- BEGIN MF TOKEN FUNCTIONS ---------------

    function depositMFToStakingContract(uint256 _amountMF) external onlyMoonshotMechanism {
        if (totalShares == 0) {unallocatedMFBountys = unallocatedMFBountys.add(_amountMF); return; } 
        
        if (unallocatedMFBountys > 0) {
            uint256 amount = _amountMF.add(unallocatedMFBountys);
            totalBountysMF = totalBountysMF.add(amount);
            bountysPerShareMF = bountysPerShareMF.add(bountysPerShareAccuracyFactor.mul(amount).div(totalShares));
            unallocatedMFBountys = 0;
        } else {
            totalBountysMF = totalBountysMF.add(_amountMF);
            bountysPerShareMF = bountysPerShareMF.add(bountysPerShareAccuracyFactor.mul(_amountMF).div(totalShares));
        }   
    }

    function getCumulativeBountysMF(uint256 share) internal view returns (uint256) {
        return share.mul(bountysPerShareMF).div(bountysPerShareAccuracyFactor);
    }

    function getUnpaidEarningsMF(address keeper) public view returns (uint256) {
        if(shares[keeper].amount == 0){ return 0; }

        uint256 keeperTotalBountysMF = getCumulativeBountysMF(shares[keeper].amount);
        uint256 keeperTotalExcludedMF = shares[keeper].totalExcludedMF;

        if(keeperTotalBountysMF <= keeperTotalExcludedMF){ return 0; }

        return keeperTotalBountysMF.sub(keeperTotalExcludedMF);
    }

    function distributeBountyMF(address keeper) internal {
        if(shares[keeper].amount == 0){ return; }

        uint256 amount = getUnpaidEarningsMF(keeper);
        if(amount > 0){
            totalDistributedMF = totalDistributedMF.add(amount);
            IBEP20(stakedToken).transfer(keeper, amount);
            keeperClaims[keeper] = block.timestamp;
            shares[keeper].totalRealisedMF = shares[keeper].totalRealisedMF.add(amount);
            shares[keeper].totalExcludedMF = getCumulativeBountysMF(shares[keeper].amount);
        }
    }

    function claimBountyMF() external {
        distributeBountyMF(msg.sender);
    }
    
    // function depositMF(uint256 _amountMF) external onlyOwner {
    //     uint256 balanceMFBefore = IERC20(address(stakedToken)).balanceOf(address(this));

    //     IBEP20(stakedToken).transferFrom(address(msg.sender), address(this), _amountMF);

    //     uint256 amount = IERC20(address(stakedToken)).balanceOf(address(this)).sub(balanceMFBefore);
