...
contract Staking is ReentrancyGuard {
    ...
    function getYearlyRateOfReturn(address account)
        public
        view
        returns (uint256)
    {
        uint256 currentBalance = s_balances[account];
        uint256 earnedThisYear = earned(account);
        uint256 totalEarned = earnedThisYear + currentBalance;
        // calculate the number of seconds in a year
        uint256 secondsInAYear = 365 * 24 * 60 * 60;
        // calculate the rate of return
        uint256 rateOfReturn = (totalEarned / currentBalance) **
            (secondsInAYear / (block.timestamp - s_lastUpdateTime)) -
            1;
        return rateOfReturn;
    }
