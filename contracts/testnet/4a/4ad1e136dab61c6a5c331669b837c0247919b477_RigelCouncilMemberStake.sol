...
////import "./IRigelDecentralizedP2PSystem.sol";
////import "./IERC20.sol";
////import "./IERC1155.sol";
////import "./Badge.sol";
...
contract RigelCouncilMemberStake is ERC1155Holder, Badge {    
    ...
    function looseRGPBadge(uint256 amount) external returns(bytes memory) {
        ...
        uint256 s_Aamount = userInfo.amount;
        ...
        emit EarnBadge(s_rank, _msgSender(), s_Aamount, userInfo.time);
        return s_rank;
    }
    ...
    function earnNFTBadge(bytes calldata _rank, uint256 tokenID) external returns(bytes memory, bytes memory) {
        ...
        bytes memory s_rank = tradeMine.otherRank;
        MerchantStore memory store = MerchantStore(_msgSender(), tokenAddr, _rank, s_rank, tokenID, tradeMine.maxAmountOnDisputeToJoin,0, block.timestamp);
    ...
    function looseNFTBadge() external lock returns(bytes memory) {
        ...
        userInfo.time = block.timestamp;
        userInfo.maxAmountToJoin = 0;
        IERC1155(userInfo.NFT).safeTransferFrom(address(this), _msgSender(), s_tokenID, 1, "");
        userInfo.NFT = address(0);
        userInfo.TokenID = 0;
        emit looseBadge(s_rank, _msgSender(), block.timestamp);
        return s_rank;
    }
