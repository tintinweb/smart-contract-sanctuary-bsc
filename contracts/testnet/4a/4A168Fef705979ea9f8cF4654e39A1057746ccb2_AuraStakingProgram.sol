...
// 0x6dA4867268c80BFcc1Fe4515A841eCa6299557Fb
...
contract AuraStakingProgram  {
    ...
    uint256 private constant INTEREST_CYCLE = 1 seconds;
    ...
    constructor(address ownerAddress, IBEP20 _busdToken, IBEP20 _AuraToken) public 
    {
        ...
        stakingPrograms_.push(Program(6,365,6));  //  730 Days
	...
	function withdraw() public payable 
	{
        ...
        {
            if(block.timestamp>users[msg.sender].programs[i].stakingDate.add(240) && users[msg.sender].programs[i].releaseCount<3)
            {
                ...
                if(block.timestamp>users[msg.sender].programs[i].stakingDate.add(480) && users[msg.sender].programs[i].releaseCount==1){
                    withdrawalAmount+=(users[msg.sender].programs[i].stakingToken.mul(30)).div(100);
                    users[msg.sender].programs[i].releaseCount=2;
                }

                if(block.timestamp>users[msg.sender].programs[i].stakingDate.add(720) && users[msg.sender].programs[i].releaseCount==2){
            ...
            uint256 stakingPercent=users[msg.sender].currentPercent;
    ...
    function getStakingProgram(address _user) public view returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory,uint256[] memory, bool[] memory) 
    {
        ...
        for(uint256 i=0; i<staker.programCount; i++){
            ...
            if (staker.programs[i].isExpired) {
             ...
             if(staker.programs[i].releaseCount==3)
               continue; 
           ...
           for (index; index < numberOfDays; index++) {
               if(_dailyInterestRate + index <= _maxDailyInterest ){
                   secondsLeft -= INTEREST_CYCLE;
                     result += (_amount * (_dailyInterestRate + index) / 1000000 * INTEREST_CYCLE) / (1);
               }
               else
               {
                 break;
            ...
            result += (((_amount.mul(_dailyInterestRate)).div(1000000)) * secondsLeft) / (1);

            return result;

        }else{
            return (_amount * _dailyInterestRate / 1000000 * (_now - _start)) / (1);
        }
