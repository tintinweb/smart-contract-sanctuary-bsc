...
contract Airdrops is Initializable {
        ...
        onlyOperator
    {
       ...
       function addAirdrop(address token, string calldata _URL, string calldata _fromDetail, 
                        address[] calldata users, uint[] calldata amounts) external payable onlyOperator{
        ...
        if(token != address(1)){
            IERC20D(token).safeTransferFrom(msg.sender, address(this), totalAmount);
        }else{
    ...
    function claimAirdrop() external{
        ...
        uint end = userAirdropDetails[msg.sender].airdrops.length;
        for(; i < end; i++){
            if(airdropsDetails[i+1].token != address(1)){
                IERC20D(airdropsDetails[i+1].token).safeTransfer(msg.sender,userAirdropDetails[msg.sender].airdropAmount[i+1]);
            }else{
                (bool sent, ) = payable(msg.sender).call{value: userAirdropDetails[msg.sender].airdropAmount[i+1]}("");
            require(sent, "Failed to send BNB");
            }
            emit ClaimedAirdrop(airdropsDetails[i+1].token, msg.sender,userAirdropDetails[msg.sender].airdropAmount[i+1]);
        }
