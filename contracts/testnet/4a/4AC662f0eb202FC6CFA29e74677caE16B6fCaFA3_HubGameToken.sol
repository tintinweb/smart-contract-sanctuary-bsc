...
contract CommonUtil{
    ...
    uint256 public periodInSecond = 2628000; //month
    ...
    function getLockItem(uint256 _amount, uint256 _nextReleaseTime, uint256 _numOfPeriod, uint256 _tgeAmount) 
...
contract HubController is CommonUtil, Ownable{    
    ...
    uint256[] memory _tgePercents, uint256[] memory _numOfPeriods, uint256[] memory _ciffs) {  
        ...
        if(_releaseDate > 0){
            listingDate = _releaseDate;
        }
        for (uint256 i = 0; i < _wallets.length; i++) {
            uint256 _amount = _supplyTokenWei  * _percents[i]/100;
            uint256 _tgeAmount = 0;
            if(listingDate <= block.timestamp) _tgeAmount = _amount * _tgePercents[i]/100;
            uint256 _nextReleaseTime = listingDate + (_ciffs[i] * periodInSecond);

            LockItem memory item = getLockItem(_amount, _nextReleaseTime, _numOfPeriods[i], _tgeAmount);
    ...
    function getLockedAmount(address lockedAddress) public view returns(uint256) {
        ...
        if(item.tgeAmount > 0){
            if(listingDate <= block.timestamp){
                item.lockedAmount = item.lockedAmount - item.tgeAmount;
                item.tgeAmount = 0;
            }
        }
...
contract HubGameToken is BEP20, Ownable, CommonUtil {
    ...
    function getControllers(string memory _gameCode) public view onlyAdmin returns (address){
    ...
    function signerAdd(string memory _gameCode, address[] memory _signers) public requireDeployed(_gameCode){
        HubController(gameControllers[_gameCode]).addSigner(_signers);
    }

    function signerIndexOf(string memory _gameCode, address _signer)  public view requireDeployed(_gameCode) returns(uint256){
        return HubController(gameControllers[_gameCode]).indexOfSigner(_signer);
    }

    function signerRemove(string memory _gameCode, uint256 index) public requireDeployed(_gameCode) returns (bool){
    ...
    function _verifyGameCode(string memory _gameCode) internal view returns (address _contract){
        if(keccak256(abi.encodePacked(_gameCode)) == keccak256(abi.encodePacked(""))){
            _gameCode = systemWallets[msg.sender];
        }
    ...
    function getConfirmations(uint256 _txId) public view returns (address[] memory) {
        if(_totalMinted > 0){
            address _contract = _verifyGameCode(signerWallets[msg.sender]);
    ...
    function transactionConfirm(uint256 _txId) public{
        ...
        if(_canExecute && availableBalance(_from) >= _value){
            ...
            if(_params.length > 0 && _params[0] > 0){
                ...
                LockItem memory item = getLockItem(_value, _params[0], _params[1], 0);
