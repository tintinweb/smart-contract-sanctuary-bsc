...
contract xYSLBUSDVault is Initializable, ReentrancyGuard, Pausable, IEvents {
    ...
    */
    ...
    function deposit(address user, uint amount,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused _completePause{
        ...
        if(_isContract(msg.sender)){
            require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender),"xYSLBUSDVault: No external contract interaction");
        }
        ...
        if (IERC20(Admin.xYSL()).balanceOf(address(this)) >= xYSLRequire) {
            ...
            _convertBUSDToLP(amount * percentxYSL/100, (reservexYSL * (amount * percentxYSL/100)) / reservesBUSD, BUSD, Admin.xYSL(), address(xyslBUSD));
        }
        else{
            percentxYSL = 15;
            ...
            if(IERC20(Admin.xYSL()).balanceOf(address(this)) <= (reservexYSL * amount * percentxYSL)/(reservesBUSD * 100)){
                    _convertBUSDToLP(reservesBUSD * IERC20(Admin.xYSL()).balanceOf(address(this)) / reservexYSL, 
                    IERC20(Admin.xYSL()).balanceOf(address(this)),
                    BUSD, Admin.xYSL(), address(xyslBUSD));
        ...
        exchangeRatio = IERC20(Admin.USDy()).balanceOf(address(this)) * (10 ** 18)/(IERC20(xyslBUSD_A).totalSupply());
    ...
    */
    ...
    //     withdraw(msg.sender, UserDeposit[msg.sender], msg.sender);
    // }


    function setRouter(address _router) external _isAdmin{
        require(_router != address(0),"xYSLBUSDVault: invalid data");
        router = _router;
    }
    ...
    function setxyslBUSD(address _xyslBUSD) external _isAdmin{
        require(_xyslBUSD != address(0),"xYSL-BUSD: invalid data");
        xyslBUSD = _xyslBUSD;
    }
    ...
    */
    function claimRebaseReward(address user) public nonReentrant whenNotPaused _completePause{
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "xYSLBUSDVault: you can't interact in same block");
            require(!_isContract(msg.sender),"xYSLBUSDVault: No external contract interaction");
        }
        _claimRebaseReward(user);
    }
    ...
    */
    ...
    function _claimRebaseReward(address user) internal {
        ...
        require(reward > 0,"xYSLBUSDVault: Please claim after rebase");
        }
        if(Admin.PhoenixNFT() != address(0) && IReceipt(Admin.PhoenixNFT()).balanceOf(user) >= 1){
