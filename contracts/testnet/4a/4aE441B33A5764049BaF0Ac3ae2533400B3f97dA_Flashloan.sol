...
contract Flashloan is Ownable {
        ...
        )
    {
        ...
        uint256 amountRequired = router.getAmountsIn(borrowAmount, path)[0];

        if (path[0] == token0) {
            debtToken = token0;
            borrowToken = token1;
            amount1Out = amountRequired;
        } else {
            debtToken = token1;
            borrowToken = token0;
            amount0Out = amountRequired;
        }
        ...
        IERC20(debtToken).transfer(address(pair), amountRequired);
    }
    ...
    function uniswapV2Call(
        ...
        IERC20(info.borrowedToken).transfer(
            info.targetPool,
            borrowedAmount
        );
