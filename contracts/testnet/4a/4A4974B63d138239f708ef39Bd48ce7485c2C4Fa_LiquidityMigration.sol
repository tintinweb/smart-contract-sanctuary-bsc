...
contract LiquidityMigration is Context, Ownable, ReentrancyGuard {
  ...
  uint256 public amountTokenDesired;

  IRouter private Router;

  event migration(uint256 LPspended, uint256 LPrecived);

  constructor (address _tokenV1, address _tokenV2, address _lpAddress, address _router) {
      v1Address = _tokenV1;
      v2Address = _tokenV2;
      lpAddress = _lpAddress;
      router = _router;
      Router = IRouter(_router);
      IERC20(_lpAddress).approve(address(Router), type(uint256).max);
  ...
  function migrate(uint256 _lpTokens) public nonReentrant {
      ...
      if (amountEthRecived > amountEthStaked) {
          uint256 diffEth = amountEthRecived - amountEthStaked;
          payable(_msgSender()).transfer(diffEth);
        }

      if (amountTokenRecived > amountTokenStaked) {
          uint256 diffToken = amountTokenRecived - amountTokenStaked;
          IERC20(v2Address).transfer(_msgSender(), diffToken);
        }
  ...
  function changeAmountDesired(uint256 newAmount) public onlyOwner {
    amountTokenDesired = newAmount;
  }
  ...
  function changeAddress(
      ...
      address _newV2, 
      address _newLp,
      address _newRouter) public onlyOwner {
    v1Address = _newV1;
    v2Address = _newV2;
    lpAddress = _newLp;
    router = _newRouter;
  ...
  function getBalance(address user) public view returns(uint256) {
    return(IERC20(lpAddress).balanceOf(user));
  }
}
