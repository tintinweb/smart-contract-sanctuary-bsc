...
// File: Bridge/contracts/Bridge.sol
...
contract CallistoBridge is Ownable {
    ...
    event Claim(address indexed originalToken, uint256 originalChainID, address indexed token, address indexed to, uint256 value, bytes32 txId, uint256 fromChainId);
    ...
    event ClaimToContract(address indexed originalToken, uint256 originalChainID, address indexed token, address indexed to, uint256 value, bytes32 txId, uint256 fromChainId, address toContract);
    event AddToken(address indexed token, uint256 chainID, uint256 decimals, string name, string symbol);

    // run only once from proxy
    function initialize(address newOwner, address _system, address _tokenImplementation) external {
        require(newOwner != address(0) && _owner == address(0) && _system != address(0)); // run only once
        _owner = newOwner;
        emit OwnershipTransferred(address(0), msg.sender);
        system = _system;
    ...
    */
    ...
    modifier onlySystem() {
        require(owner() == msg.sender || system == msg.sender, "caller is not the owner or system");
        _;
    }
    ...
    function getToken(address originalToken, uint256 originalChainID) external view returns(Token memory) {
        return addedTokens[keccak256(abi.encodePacked(originalToken, originalChainID))];
    }

    // return Token structure for native token address (token from native chain)
    function getToken(address nativeToken) external view returns(Token memory) { 
        ...
        external 
    {
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(token, chainID, decimals, name, symbol));
        ...
        address wrappedToken = Clones.cloneDeterministic(tokenImplementation, bytes32(uint256(uint160(token))));
        IBEP20TokenCloned(wrappedToken).initialize(address(this), _name, _symbol, uint8(decimals));
        addedTokens[key].token = token;
    ...
    // add MUST authority for specific token
    function addTokenMustAuthority(
        address token,      // original token address
        uint256 chainID,    // original token chain ID
        ...
        onlySystem 
    {
        ...
        require(wrappedToken != address(0) && IBEP20TokenCloned(wrappedToken).getOwner() == address(this), "is not our wrapped token");
        ...
        notFrozen
    {
        ...
        (address originalToken, uint256 originalChainID) = _deposit(token, value);
        emit BridgeToContract(originalToken, originalChainID, token, receiver, value, toChainId, toContract, data);
    }
        ...
        notFrozen
    {
        ...
        bytes32 key = keccak256(abi.encodePacked(originalToken, originalChainID));
        require(addedTokens[key].token != address(0), "token not exist");
        isTxProcessed[fromChainId][txId] = true;
        // Check signature
        {
        address must = addedTokens[key].authority;
        bytes32 messageHash = keccak256(abi.encodePacked(originalToken, originalChainID, to, value, txId, fromChainId, block.chainid, address(this), toContract, data));
        ...
        emit ClaimToContract(originalToken, originalChainID, token, to, value, txId, fromChainId, toContract);
        }
    }
        ...
        notFrozen
    {
        require(receiver != address(0), "Incorrect receiver address");
        (address originalToken, uint256 originalChainID) = _deposit(token, value);
        emit Deposit(originalToken, originalChainID, token, receiver, value, toChainId);
    }
    ...
    function _deposit(
        ...
        internal 
        returns(address originalToken, uint256 originalChainID)
    {
        ...
        notFrozen
    {
        ...
        {
        address must = addedTokens[key].authority;
        bytes32 messageHash = keccak256(abi.encodePacked(originalToken, originalChainID, to, value, txId, fromChainId, block.chainid, address(this)));
        checkSignatures(must, messageHash, sig);
        }
        address token = addedTokens[key].wrappedToken;
        if (token != address(0)) {
        ...
        emit Claim(originalToken, originalChainID, token, to, value, txId, fromChainId);
    }

    // Signature methods
    function checkSignatures(address must, bytes32 messageHash, bytes[] memory sig) internal view {
        ...
        require(must == address(0) && required == address(0), "The required authorities didn't sign");
    }
