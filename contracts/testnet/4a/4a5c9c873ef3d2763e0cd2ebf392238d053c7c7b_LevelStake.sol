...
contract LevelStake is Initializable, OwnableUpgradeable, ILevelStake {
    ...
    uint256 public constant UNSTAKE_WINDOWN = 10 minutes;
    ...
    function claimRewards(address _to) external {
        ...
        if (_pendingReward != 0) {
            safeRewardTransfer(_to, _pendingReward);
    ...
    // Safe transfer function, just in case if rounding error causes pool to not have enough LVLs.
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        uint256 lgoBalance = LGO.balanceOf(address(lgoReserve));
        if (_amount > lgoBalance) {
            lgoReserve.requestTransfer(_to, lgoBalance);
        } else {
            lgoReserve.requestTransfer(_to, _amount);
        }
    }
