...
contract NEWTESTZBNB is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _getHero(uint256 _heroId, bool calcSpaceship) internal view returns (HeroLibrary.Hero memory) {
        ...
        if (hero.arrivalTime > block.timestamp) {
            ...
            return HeroLibrary.Hero(21, 5, 1000, 0, 0, block.timestamp, _heroId, hero.arrivalTime - block.timestamp, 1, false, block.timestamp);
        }
...
//NEWFUNC
    ...
    function decreaseRB(uint[] calldata tokensId, uint decreasePercent, uint minDecreaseLevel, address user) external onlyRole(RB_SETTER_ROLE) returns(uint decreaseAmount) {
        ...
        for(uint i = 0; i < tokensId.length; i++){
            if(_heroes[tokensId[i]].level <= minDecreaseLevel) continue;
            require(ownerOf(tokensId[i]) == user, "Not owner");
            uint currentRB = _heroes[tokensId[i]].stamina;
            finalRB[i] = currentRB * decreasePercent / 1e12;
            _heroes[tokensId[i]].stamina = finalRB[i];
    ...
    function decreaseRBView(uint[] calldata tokensId, uint decreasePercent, uint minDecreaseLevel) external view returns(uint decreaseAmount) {
        ...
        for(uint i = 0; i < tokensId.length; i++){
            if(_heroes[tokensId[i]].level <= minDecreaseLevel) continue;
            decreaseAmount += _heroes[tokensId[i]].stamina - _heroes[tokensId[i]].stamina * decreasePercent / 1e12;
        }
    }
    ...
    function mint(address to) public onlyRole(TOKEN_MINTER_ROLE) nonReentrant returns(uint256) {
        ...
        _heroes.push(HeroLibrary.Hero(heroNames[hero], heroTypes[hero], 1000, attacks, armors, staminaTimestamp, tokenId, block.timestamp + heroArrivalTime, 1, false, block.timestamp));
    ...
    function launchpadMint(
        ...
        _heroes[tokenId].stamina = robiBoost;
        _heroes[tokenId].createTimestamp = block.timestamp;
        _heroes[tokenId].level = level;
        _safeMint(to, tokenId);
    }

    // function levelUp(uint[] calldata tokenId) public nonReentrant {
    //     require(tokenId.length <= MAX_ARRAY_LENGTH_PER_REQUEST, "Array length gt max");
    //     uint currentLevel = _heroes[tokenId[0]].level;
    ...
    //         Hero memory token = _heroes[tokenId[i]]; //safe gas
