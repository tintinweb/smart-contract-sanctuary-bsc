...
contract TheNetworkSociety is Context, Ownable{
    ...
    uint256 private constant maxRewardAccumulationStep = 172800; //2 days
    uint256 private constant maxCompoundMultiplier = 5; //5x of real investment.

    uint256 private constant referralBonus = 20; //2% for referrer and referee total of 4% per deposit.
    ...
    uint256 private constant depositMaximum = 10000 * (10 ** 18); 
    uint256 private constant airdropMinimum = 10 * (10 ** 18);
    uint256 private constant maxRewardsPool = 2000 * (10 ** 18);
    
    uint256 private timeStep = 86400;  //1 day
    uint256 private eventTimeStep = 86400; //1 day
    ...
    //auto compound contract address
    ...
    function updateTimeForTesting(uint256 eventTime, uint256 dayStep) external onlyOwner {
    ...
    //invest from the owner address to setup initial leader address.
    function invest(uint256 amount) external onlyOwner {
        _invest(msg.sender, amount);
    }

    //normal invest function. sponsor is the owner by default if there is no sponsor.
    ...
    function _invest(address _addr, uint256 _amount) private {
        ...
        if(amountToCompound > 0 && users[_addr].totalDepositAmount.add(_amount) < this.maxCompoundOf(users[_addr].totalDirectDeposits)) {
            ...
            if(block.timestamp.sub(users[_addr].checkpoint) >= timeStep) {
        ...
        if(users[_addr].sponsor != address(0)) {
            ...
            uint256 refBonus = _amount.mul(referralBonus).div(1000);
            //check if the total deposit of the network leader is within max compound amount. no need to check referee since its new deposit.
    ...
    function lastDepositEntry(address userAddress, uint256 amount) private {
        ...
        uint256 share = amount.mul(50).div(1000); //5%
    ...
    function poolReferralDeposit(address _addr, uint256 _amount) private {
	    ...
	    uint256 pool_amount = _amount.mul(50).div(1000); //5%
    ...
    function _drawTopReferrerPool() private {
        if(topReferrerEnabled && topReferrerLastDrawAction.add(eventTimeStep) > block.timestamp) {
            ...
            for(uint8 i = 0; i < topReferrerPoolBonuses.length; i++) {
                ...
                userBonusStatistics[topReferrerPool[i]].topReferrerBonus += win; //only for statistics
    ...
    function _drawTopDepositPool() private {
        if(topDepositEnabled && topDepositLastDrawAction.add(eventTimeStep) > block.timestamp) {
    ...
    function compound(address addr) public initialized {
        ...
        if(users[addr].checkpoint.add(timeStep) >= block.timestamp) {
    ...
    function setSponsor(address _addr, address _sponsor) private {
        if(this.checkSponsorValid(_addr, _sponsor)) {
            ...
            users[_sponsor].networkInvites++; //record total invites of network leader
        }
    }
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
        ...
        if(users[_addr].totalDepositPayouts < max_payout) {
            uint256 actionTime = users[_addr].checkpoint < block.timestamp.sub(maxRewardAccumulationStep) ? block.timestamp.sub(maxRewardAccumulationStep) : users[_addr].checkpoint;
            payout = (users[_addr].totalDepositAmount.mul(users[_addr].userYieldPercentage).div(1000))
            ...
            sustainability_fee = payout.mul(sustainTax).div(1000);
    ...
    function payoutToCompound(address _addr) view external returns(uint256 finalPayout) {
        ...
        if(users[_addr].totalDepositPayouts < max_payout) {   
            uint256 actionTime = users[_addr].checkpoint < block.timestamp.sub(maxRewardAccumulationStep) ? block.timestamp.sub(maxRewardAccumulationStep) : users[_addr].checkpoint;
            uint256 payout = (users[_addr].totalDepositAmount.mul(users[_addr].userYieldPercentage).div(1000))
                    .mul(block.timestamp.sub(actionTime)).div(timeStep);
            finalPayout = payout.sub(payout.mul(compoundTax).div(1000));   
        }            
    }

    function maxPayoutOf(uint256 _amount) pure external returns(uint256) {
        return _amount.mul(userMaxPayout).div(1000);
    }

    function maxCompoundOf(uint256 _amount) pure external returns(uint256) {
    ...
    //airdrop to anyone invested in TNS.
    function airdrop(address _to, uint256 _amount) external {
        ...
        uint256 payout = _amount.sub(payTax(_amount));
    ...
    //airdrop from your team. networkId is selected by default
    function networkAirdrop(uint256 networkId, bool excludeOwner, uint256 _amount) external {
        address _addr = msg.sender;
        require(airdropEnabled, "Airdrop not Enabled.");
        require(contractInitialized, "Contract not initialized.");
        require(_amount >= airdropMinimum, "Mininum airdrop amount not met.");
        require(networks[networkId].owner != address(0), "Network not found.");
        require(networks[networkId].owner == msg.sender, "NetworkId is not from the sender.");
        ...
        if(excludeOwner == true){
            memberDivider--; //less 1 in the length to remove owner address.
        }
    ...
    function payTax(uint256 amount) internal returns(uint256) {
        ...
        uint256 projTax = amount.mul(projectTax).div(1000);
        uint256 mktTax = amount.mul(marketTax).div(1000);
    ...
    //External functions. 
    function userAutoInfo(address _addr) view external returns(uint256 checkpoint, uint256 totalDepositAmount) { //checking for auto compound execution.
