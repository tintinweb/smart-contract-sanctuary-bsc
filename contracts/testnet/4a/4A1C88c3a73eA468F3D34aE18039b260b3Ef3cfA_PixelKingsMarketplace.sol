...
contract PixelKingsMarketplace is Ownable {
    ...
    function updateHeroUri(string memory _hero, string memory _uri) external onlyOwner {
        heroUri[_hero] = _uri;
        emit UpdateHero(_hero, _uri);
    }
    
    function addNewHero(Class _class, string memory _hero, string memory _uri) external onlyOwner {
