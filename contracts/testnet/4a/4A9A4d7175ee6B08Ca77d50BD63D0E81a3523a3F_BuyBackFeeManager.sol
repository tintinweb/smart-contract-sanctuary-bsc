...
contract BuyBackFeeManager is AccessControl {
    ...
    /*
     * Function unwraps LP tokens in batches of 100.
        ...
        external
    {
        ...
        uint256 _endingIndex = _startingIndex + 100;
        ...
        for (
            ...
            if (liquidity > 0) {
                ...
                address tokenA;
                try IBabyDogeV2Pair(_lpTokenToUnwrap[current])
                ...
                try IBabyDogeV2Router02(router).removeLiquidity(
                    tokenA,
                    tokenB,
                    liquidity,
                    0,
    ...
    function swapTokens(address[] storage path) internal {
        ...
        try IBabyDogeV2Router02(router).getAmountsOut(amountIn, path)
        returns (uint256[] memory amounts) {
            address to = address(this);
            uint256 deadline = block.timestamp + 120; //2 minutes to complete transaction
            try IBabyDogeV2Router02(router)
    ...
    function swapToBuyBackAndBurn() external {
        ...
        if (buyBackCoin0 != address(0)) {
            ...
            try IBabyDogeV2Router02(router).swapExactTokensForTokens(
                _buyBackAmount0,
                amountOutMin[1],
                path,
