...
interface IMetahub is IProtocolConfigManager, IPaymentManager, IRentingManager, IAssetManager {}
...
import "../listing/IStrategyOverrideConfigRegistry.sol";
...
library Rentings {
     ...
     * @param strategyConfigId Strategy Config ID. Points to listing / taxation strategy override and its data.
     */
    struct Params {
        uint256 listingId;
        address warper;
        ...
        // review need for this parameter
        uint256 strategyConfigId;
    }
    ...
    function validateRentingParams(
        ...
        IStrategyOverrideConfigRegistry overrideStrategyRegistry,
        ...
        // Validate from the override strategy perspective
        overrideStrategyRegistry.checkRegisteredOverrideConfig(params.strategyConfigId);
        IStrategyOverrideConfigRegistry.ListingStrategyOverrideParams
            memory listingStrategyParams = IStrategyOverrideConfigRegistry.ListingStrategyOverrideParams({
                configId: params.strategyConfigId,
                listingGroupId: listing.groupId,
                universeId: warper.universeId,
                warperAddress: params.warper
            });
        // check that strategy override config works for listing group
        overrideStrategyRegistry.checkListerOverrideConfigExistance(listingStrategyParams);
        // read override strategy config from registry
        IStrategyOverrideConfigRegistry.StrategyOverrideConfig memory overrideConfig = overrideStrategyRegistry
            .listerOverrideConfig(listingStrategyParams);
        // find listing tax strategy id for listing strategy id from override config
        bytes4 taxStrategyId = listingStrategyRegistry.listingTaxStrategyIdForListingStrategy(overrideConfig.strategyId);
        IStrategyOverrideConfigRegistry.ListingTaxStrategyOverrideParams
            memory listingTaxStrategyParams = IStrategyOverrideConfigRegistry.ListingTaxStrategyOverrideParams({
                listingTaxStrategyId: taxStrategyId,
                universeId: warper.universeId,
                warperAddress: params.warper
            });
        // check that strategy override config works for universe
        overrideStrategyRegistry.checkUniverseOverrideConfigExistance(listingTaxStrategyParams);
        // check that strategy override config works for protocol
        overrideStrategyRegistry.checkProtocolOverrideConfigExistance(listingTaxStrategyParams);
    }
    ...
    function calculateRentalFees(
        ...
        IListingManager listingRegistry,
        IWarperManager warperManager,
        IListingStrategyRegistry listingStrategyRegistry,
        IStrategyOverrideConfigRegistry overrideStrategyRegistry
    ) external view returns (RentalFees memory fees) {
        // Resolve listing info
        Listings.Listing memory listing = listingRegistry.listingInfo(rentingParams.listingId);
        // Resolve warper to get universeId from it
        Warpers.Warper memory warper = warperManager.warperInfo(rentingParams.warper);
        // Compose ListingStrategyOverrideParams for getting lister override config
        IStrategyOverrideConfigRegistry.ListingStrategyOverrideParams
            memory listerStrategyOverrideParams = IStrategyOverrideConfigRegistry.ListingStrategyOverrideParams({
                configId: rentingParams.strategyConfigId,
                listingGroupId: listing.groupId,
                universeId: warper.universeId,
                warperAddress: rentingParams.warper
            });
        // Reading Lister Listing StrategyOverrideConfig from SOCR
        IStrategyOverrideConfigRegistry.StrategyOverrideConfig memory strategyOverrideConfig = overrideStrategyRegistry
            .listerOverrideConfig(listerStrategyOverrideParams);
        // Resolve listing controller to calculate lister fee based on selected listing strategy.
        address listingControllerAddress = listingStrategyRegistry.listingController(strategyOverrideConfig.strategyId);
        // Resolving all fees using single call to ListingController.calculateRentalFee(...)
        (fees.total, fees.listerBaseFee, fees.universeBaseFee, fees.protocolFee) = IListingController(
            listingControllerAddress
        ).calculateRentalFee(listerStrategyOverrideParams, rentingParams);
...
interface IUniverseRegistry {
     ...
     */
    function universe(uint256 universeId) external view returns (string memory name);
...
import "../asset/Assets.sol";
import "./IStrategyOverrideConfigRegistry.sol";
...
library Listings {
    ...
    /*
     * @dev Taxation strategy identifiers to be used across the system:
     */
    bytes4 public constant FIXED_RATE_TAX = bytes4(keccak256("FIXED_RATE_TAX"));
    bytes4 public constant FIXED_RATE_TAX_WITH_REWARD_RATE_TAX =
        bytes4(keccak256("FIXED_RATE_TAX_WITH_REWARD_RATE_TAX"));
     ...
     * @param config Listing strategy override config.
     */
    struct Params {
        uint256 groupId;
        IStrategyOverrideConfigRegistry.StrategyOverrideConfig config;
    }
    ...
    function register(
        ...
        IStrategyOverrideConfigRegistry overrideStrategyRegistry,
        IStrategyOverrideConfigRegistry.StrategyOverrideConfig calldata config
    )
        external
        returns (
            uint256 listingId,
            uint256 listingGroupId,
            uint256 strategyOverrideConfigId
        )
    {
        ...
        // Registering listing strategy override config for lister on global level
        strategyOverrideConfigId = overrideStrategyRegistry.registerListerGlobalLevelOverrideConfig(
            listingGroupId,
            config
        );
    ...
    /**
     * @dev Checks if the provided `account` is listing group owner address.
     * @param listingGroupId Listing group ID.
     * @param account The account to check ownership for.
     */
    function isListingGroupOwner(
        Registry storage self,
        uint256 listingGroupId,
        address account
    ) external view returns (bool) {
        return self.listingGroups[listingGroupId].owner == account;
    }
...
interface IListingManager {
    ...
    /**
     * @dev Check if the given account is the owner of the listing group.
     * @param listingGroupId Listing Group ID.
     * @param account The users account to checked for the ownership on the listing group.
     * @return True if the account is the owner of the listing group.
     */
    function isListingGroupOwner(uint256 listingGroupId, address account) external view returns (bool);
     ...
     * @param account The account to check ownership for.
     */
    function checkListingGroupOwner(uint256 listingGroupId, address account) external view;
...
import "./IListingTaxController.sol";
...
interface IListingStrategyRegistry {
    ...
    /**
     * @dev Thrown when listing tax controller does not implement the required interface.
     */
    error InvalidListingTaxControllerInterface();
    ...
    /**
     * @dev Thrown when the listing tax cannot be processed by the specific
     *      controller due to the listing tax strategy ID mismatch.
     * @param provided Provided listing tax strategy ID.
     * @param required Required listing tax strategy ID.
     */
    error ListingTaxStrategyMismatch(bytes4 provided, bytes4 required);
    ...
    /**
     * @dev Thrown upon attempting to register a listing tax strategy twice.
     * @param listingTaxStrategyId Duplicate taxation strategy ID.
     */
    error ListingTaxStrategyIsAlreadyRegistered(bytes4 listingTaxStrategyId);
    ...
    /**
     * @dev Thrown upon attempting to work with unregistered listing tax strategy.
     * @param listingTaxStrategyId Taxation strategy ID.
     */
    error UnregisteredListingTaxStrategy(bytes4 listingTaxStrategyId);
    ...
    /**
     * @dev Emitted when the new listing tax strategy is registered.
     * @param listingTaxStrategyId Taxation strategy ID.
     * @param controller Controller address.
     */
    event ListingTaxStrategyRegistered(bytes4 indexed listingTaxStrategyId, address indexed controller);
    ...
    /**
     * @dev Emitted when the listing tax strategy controller is changed.
     * @param listingTaxStrategyId Taxation strategy ID.
     * @param newController Controller address.
     */
    event ListingTaxStrategyControllerChanged(bytes4 indexed listingTaxStrategyId, address indexed newController);
    ...
    /**
     * @dev Listing tax strategy information.
     * @param controller Listing tax controller address.
     */
    struct ListingTaxStrategyConfig {
        address controller;
    }
     ...
     */
    function registerListingStrategy(
        bytes4 listingStrategyId,
        bytes4 listingTaxStrategyId,
        ListingStrategyConfig calldata config
    ) external;

    /**
     * @dev Registers new listing tax strategy.
     * @param listingTaxStrategyId ListingTax strategy ID.
     * @param config Taxation strategy configuration.
     */
    function registerListingTaxStrategy(bytes4 listingTaxStrategyId, ListingTaxStrategyConfig calldata config) external;
    ...
    /**
     * @dev Sets listing tax strategy controller.
     * @param listingTaxStrategyId Listing tax strategy ID.
     * @param controller Taxation controller address.
     */
    function setListingTaxController(bytes4 listingTaxStrategyId, address controller) external;
    ...
    /**
     * @dev Returns listing tax strategy controller.
     * @param listingTaxStrategyId Listing strategy ID.
     * @return Taxation controller address.
     */
    function listingTaxController(bytes4 listingTaxStrategyId) external view returns (address);
    ...
    /**
     * @dev Returns listing tax strategy configuration.
     * @param listingTaxStrategyId Listing tax strategy ID.
     * @return Listing tax strategy information.
     */
    function listingTaxStrategy(bytes4 listingTaxStrategyId) external view returns (ListingTaxStrategyConfig memory);

    /**
     * @dev Returns listing tax strategy ID for listing strategy.
     * @param listingStrategyId Listing strategy ID.
     * @return listingTaxStrategyId Listing tax strategy ID.
     */
    function listingTaxStrategyIdForListingStrategy(bytes4 listingStrategyId)
        external
        view
        returns (bytes4 listingTaxStrategyId);

    /**
     * @dev Returns listing tax strategy ID and config for listing strategy.
     * @param listingStrategyId Listing strategy ID.
     * @return listingTaxStrategyId Listing tax strategy ID.
     * @return listingTaxConfig Listing tax strategy config.
     */
    function listingTaxStrategyForListingStrategy(bytes4 listingStrategyId)
        external
        view
        returns (bytes4 listingTaxStrategyId, ListingTaxStrategyConfig memory listingTaxConfig);

    /**
     * @dev Reverts if listing strategy is not supported.
     * @param listingStrategyId Listing strategy ID.
     */
    function checkSupportedListingStrategy(bytes4 listingStrategyId) external view;
    ...
    /**
     * @dev Checks listing tax strategy registration.
     * @param listingTaxStrategyId Listing tax strategy ID.
     */
    function isRegisteredListingTaxStrategy(bytes4 listingTaxStrategyId) external view returns (bool);
    ...
    /**
     * @dev Reverts if listing tax strategy is not registered.
     * @param listingTaxStrategyId Listing tax strategy ID.
     */
    function checkRegisteredListingTaxStrategy(bytes4 listingTaxStrategyId) external view;
}
...
interface IStrategyOverrideConfigRegistry {
    /**
     * @dev Thrown upon attempting to work with unregistered override strategy config.
     */
    error UnregisteredOverrideConfig(uint256 configId);

    /**
     * @dev Thrown upon attempting to work with lister (listing group) with missing overrides.
     */
    error MissingOverrideConfigsForLister(uint256 listingGroupId);

    /**
     * @dev Thrown upon attempting to work with lister (listing group) with missing overrides.
     */
    error MissingOverrideConfigsForUniverse(uint256 universeId, bytes4 listingTaxStrategyId);

    /**
     * @dev Thrown upon attempting to work with lister (listing group) with missing overrides.
     */
    error MissingOverrideConfigsForProtocol(bytes4 listingTaxStrategyId);

    /**
     * @dev Emitted when the new override strategy is registered.
     * @param configId Listing strategy override config ID.
     * @param strategyId Listing strategy ID.
     * @param strategyData Listing strategy data.
     */
    event StrategyOverrideConfigRegistered(uint256 indexed configId, bytes4 indexed strategyId, bytes strategyData);

    /**
     * @dev Emitted when the global level lister's override strategy is registered.
     * @param listingGroupId Listing group ID.
     * @param configId Listing strategy override config ID.
     */
    event ListerGlobalLevelOverrideConfigRegistered(uint256 listingGroupId, uint256 indexed configId);

    /**
     * @dev Emitted when the universe level lister's override strategy is registered.
     * @param listingGroupId Listing group ID.
     * @param universeId Universe ID.
     * @param configId Listing strategy override config ID.
     */
    event ListerUniverseLevelOverrideConfigRegistered(
        uint256 listingGroupId,
        uint256 universeId,
    ...
    /**
     * @dev Emitted when the warper level lister's override strategy is registered.
     * @param listingGroupId Listing group ID.
     * @param warperAddress Address of the warper.
     * @param configId Listing strategy override config ID.
     */
    event ListerWarperLevelOverrideConfigRegistered(
        uint256 listingGroupId,
    ...
    /**
     * @dev Emitted when the global level universe's override strategy is registered.
     * @param universeId Universe ID.
     * @param configId Listing strategy override config ID.
     */
    event UniverseGlobalLevelOverrideConfigRegistered(uint256 universeId, uint256 indexed configId);

    /**
     * @dev Emitted when the warper level universe's override strategy is registered.
     * @param universeId Universe ID.
     * @param warperAddress Address of the warper.
     * @param configId Listing strategy override config ID.
     */
    event UniverseWarperLevelOverrideConfigRegistered(
        uint256 universeId,
        address indexed warperAddress,
        uint256 indexed configId
    );
    ...
    /**
     * @dev Emitted when the global level protocol's override strategy is registered.
     * @param configId Listing strategy override config ID.
     */
    event ProtocolGlobalLevelOverrideConfigRegistered(uint256 indexed configId);

    /**
     * @dev Emitted when the universe level protocol's override strategy is registered.
     * @param universeId Universe ID.
     * @param configId Listing strategy override config ID.
     */
    event ProtocolUniverseLevelOverrideConfigRegistered(uint256 universeId, uint256 indexed configId);

    /**
     * @dev Emitted when the warper level protocol's override strategy is registered.
     * @param warperAddress Address of the warper.
     * @param configId Listing strategy override config ID.
     */
    event ProtocolWarperLevelOverrideConfigRegistered(address indexed warperAddress, uint256 indexed configId);

    /**
     * @dev Listing strategy config information.
     * @param strategyId Listing / Listing Tax strategy config ID.
     * @param strategyData Listing strategy ID.
     */
    struct StrategyOverrideConfig {
        bytes4 strategyId;
        bytes strategyData;
    }

    /**
     * @dev Listing strategy config parameters.
     * @param configId StrategyOverrideConfig ID.
     * @param listingGroupId Listing group ID.
     * @param universeId Universe ID.
     * @param warperAddress Address of the warper.
     */
    struct ListingStrategyOverrideParams {
        uint256 configId;
        uint256 listingGroupId;
        uint256 universeId;
        address warperAddress;
     ...
     * @param universeId Universe ID.
     * @param warperAddress Address of the warper.
     * @param listingTaxStrategyId ListingTaxStrategy ID.
     */
    struct ListingTaxStrategyOverrideParams {
        bytes4 listingTaxStrategyId;
        uint256 universeId;
        address warperAddress;
    }
    ...
    /**
     * @dev Registers global lister's strategy config.
     * @param listingGroupId The ID of the listing group.
     * @param config The listing strategy override config.
     * @return configId The newly created listing strategy override config ID.
     */
    function registerListerGlobalLevelOverrideConfig(uint256 listingGroupId, StrategyOverrideConfig calldata config)
        external
        returns (uint256 configId);

    /**
     * @dev Registers (overrides) universe level lister's strategy config.
     * @param listingGroupId The ID of the listing group.
     * @param universeId The ID of the universe.
     * @param config The listing strategy config.
     * @return configId The newly created listing strategy config ID.
     */
    function registerListerUniverseLevelOverrideConfig(
        uint256 listingGroupId,
        uint256 universeId,
        StrategyOverrideConfig calldata config
    ) external returns (uint256 configId);

    /**
     * @dev Registers (overrides) warper level lister's strategy config.
     * @param listingGroupId The ID of the listing group.
     * @param warperAddress The address of the warper.
     * @param config The listing strategy config.
     * @return configId The newly created listing strategy config ID.
     */
    function registerListerWarperLevelOverrideConfig(
        uint256 listingGroupId,
        address warperAddress,
        StrategyOverrideConfig calldata config
    ) external returns (uint256 configId);
    ...
    /**
     * @dev Registers global universe's strategy config.
     * @param universeId The ID of the universe.
     * @param config The listing strategy config.
     * @return configId The newly created listing strategy config ID.
     */
    function registerUniverseGlobalLevelOverrideConfig(uint256 universeId, StrategyOverrideConfig calldata config)
        external
        returns (uint256 configId);

    /**
     * @dev Registers (overrides) warper level universe's strategy config.
     * @param universeId The ID of the universe.
     * @param warperAddress The address of the warper.
     * @param config The listing strategy config.
     * @return configId The newly created listing strategy config ID.
     */
    function registerUniverseWarperLevelOverrideConfig(
        uint256 universeId,
        address warperAddress,
        StrategyOverrideConfig calldata config
    ) external returns (uint256 configId);
    ...
    /**
     * @dev Registers global protocol's strategy config.
     * @param config The listing strategy config.
     * @return configId The newly created listing strategy config ID.
     */
    function registerProtocolGlobalLevelOverrideConfig(StrategyOverrideConfig calldata config)
        external
        returns (uint256 configId);

    /**
     * @dev Registers (overrides) universe level protocol's strategy config.
     * @param universeId The ID of the universe.
     * @param config The listing strategy config.
     * @return configId The newly created listing strategy config ID.
     */
    function registerProtocolUniverseLevelOverrideConfig(uint256 universeId, StrategyOverrideConfig calldata config)
        external
        returns (uint256 configId);

    /**
     * @dev Registers (overrides) warper level protocol's strategy config.
     * @param warperAddress The address of the warper.
     * @param config The listing strategy config.
     * @return configId The newly created listing strategy config ID.
     */
    function registerProtocolWarperLevelOverrideConfig(address warperAddress, StrategyOverrideConfig calldata config)
        external
        returns (uint256 configId);

    /**
     * @dev Returns strategy override config.
     * @param configId The strategy override config ID.
     * @return Listing strategy override config.
     */
    function overrideConfig(uint256 configId) external view returns (StrategyOverrideConfig memory);

    /**
     * @dev Returns lister's listing strategy override config.
     * @param params The listing strategy override params.
     * @return Listing strategy override config.
     */
    function listerOverrideConfig(ListingStrategyOverrideParams memory params)
        external
        view
        returns (StrategyOverrideConfig memory);

    /**
     * @dev Returns universe's listing strategy override config.
     * @param params The listing strategy override params.
     * @return Listing tax strategy override config.
     */
    function universeOverrideConfig(ListingTaxStrategyOverrideParams memory params)
        external
        view
        returns (StrategyOverrideConfig memory);

    /**
     * @dev Returns protocol's listing strategy override config.
     * @param params The listing strategy override params.
     * @return Listing tax strategy override config.
     */
    function protocolOverrideConfig(ListingTaxStrategyOverrideParams memory params)
        external
        view
        returns (StrategyOverrideConfig memory);

    /**
     * @dev Checks registration of StrategyOverrideConfig.
     * @param configId StrategyOverrideConfig ID.
     * @return Boolean that is positive in case of existance
     */
    function isRegisteredOverrideConfig(uint256 configId) external view returns (bool);

    /**
     * @dev Checks registration of StrategyOverrideConfig.
     *      Reverts with UnregisteredOverrideConfig() in case OverrideStrategyConfig does not registered.
     * @param configId StrategyOverrideConfig ID.
     */
    function checkRegisteredOverrideConfig(uint256 configId) external view;

    /**
     * @dev Checks registration of StrategyOverrideConfig for lister on global, universe and warper levels.
     *      Reverts in case of absence of StrategyOverrideConfig on all levels.
     * @param params ListingStrategyOverrideParams specific params.
     */
    function checkListerOverrideConfigExistance(ListingStrategyOverrideParams memory params) external view;

    /**
     * @dev Checks registration of StrategyOverrideConfig for universe on global and warper levels.
     *      Reverts in case of absence of StrategyOverrideConfig on all levels.
     * @param params ListingTaxStrategyOverrideParams specific params.
     */
    function checkUniverseOverrideConfigExistance(ListingTaxStrategyOverrideParams memory params) external view;

    /**
     * @dev Checks registration of StrategyOverrideConfig for protocol on global, universe and warper levels.
     *      Reverts in case of absence of StrategyOverrideConfig on all levels.
     * @param params ListingTaxStrategyOverrideParams specific params.
     */
    function checkProtocolOverrideConfigExistance(ListingTaxStrategyOverrideParams memory params) external view;
}
...
interface IListingController is IERC165 {
     ...
     * @param listingStrategyParams Listing strategy override params.
     ...
     * @return universeBaseFee Universe fee (lister base fee * universeBaseTax / 100%).
     * @return protocolBaseFee Protocol fee (lister base fee * protocolBaseTax / 100%).
     */
    function calculateRentalFee(
        IStrategyOverrideConfigRegistry.ListingStrategyOverrideParams calldata listingStrategyParams,
        Rentings.Params calldata rentingParams
    )
        external
        view
...
interface IListingTaxController is IERC165 {
    /**
     ...
     * @param listingTaxOverrideParams Listing tax strategy override params.
     * @param rentingParams Renting params.
    ...
    function calculateRentalTax(
        IStrategyOverrideConfigRegistry.ListingTaxStrategyOverrideParams calldata listingTaxOverrideParams,
        Rentings.Params calldata rentingParams,
        uint256 taxableAmount
    ) external view returns (uint256 universeBaseTax, uint256 protocolBaseTax);
}
