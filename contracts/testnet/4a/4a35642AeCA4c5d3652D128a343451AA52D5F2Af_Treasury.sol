...
contract Treasury is SafeTransfer, Context, ReentrancyGuard {
  ...
  modifier onlyAdmin() {
    require(sspAccessControls.hasAdminRole(_msgSender()), "Treasury: only admin");
    _;
  }

  constructor(address _sspAccessControls) {
  ...
  function withdrawToken(address token, uint256 amount) public onlyAdmin nonReentrant {
    safeTokenTransfer(token, payable(_msgSender()), amount);
    emit WithdrawToken(_msgSender(), token, amount);
  }

  function transferTokenTo(address token, address to, uint256 amount) public nonReentrant {
    require(sspAccessControls.hasTreasuryRole(_msgSender()), "Treasury: not treasury member");
    safeTokenTransfer(token, payable(to), amount);
