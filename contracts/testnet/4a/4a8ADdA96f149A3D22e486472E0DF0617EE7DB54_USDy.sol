...
contract USDy is ERC20,AccessControl {
    ...
    */
    ...
    function _transfer(
        ...
        require(restrictTransfer[sender] != block.number,"USDy: you can't interact in same block");
        console.log("Sender address", sender);
        console.log("router1", Admin.ApeswapRouter());
        ...
        if(isContract(sender) || isContract(recipient)){
            ...
            "USDy: No external contract interact with USDy");
        }
        if(sender == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.WBNB()) || recipient == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.WBNB())){
            address user = sender == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.WBNB()) ? recipient : sender;
            ...
            IERC20(Admin.BUSD()).transfer(Admin.temporaryHolding(), (convertedBUSD * USDy_Tax_Allocation[0])/USDy_Tax);
            IERC20(Admin.BUSD()).transfer(Admin.Treasury(), ((convertedBUSD * USDy_Tax_Allocation[1])/USDy_Tax));
            ...
            uint256 prevAmount = balanceOf(IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.WBNB())); 
            console.log("prevAmount",prevAmount);
            super._transfer(sender, recipient, amount);
            uint256 currentAmount = balanceOf(IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.WBNB()));   
            ...
            require(transactionTimeLimit[sender] + lockTransactionTime <= block.timestamp,"USDy: transactionTimeLimit is greater than current time");
                console.log('Enterrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrrr1');
                uint taxAmount = (amount * USDy_Tax)/10000;
                super._transfer(sender, address(this), taxAmount);
                //if(IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(balanceOf(address(this)),path)[2] >= USDy_ThresholdLimit){
                    IERC20(address(this)).approve(Admin.ApeswapRouter(), taxAmount); //BSC Testnet pancake router address
                    uint convertedBUSD = IUniswapV2Router02(Admin.ApeswapRouter()).swapExactTokensForTokens( 
                        taxAmount,
                        0,
     ...
     */
    function setPriceImpactProtection(uint value) public onlyRole(DEFAULT_ADMIN_ROLE) {
        priceImpactProtection = value;
    }
    /**
    setter function for Lock Transaction Time
    */
    function setLockTransactionTime(uint time) public onlyRole(DEFAULT_ADMIN_ROLE) {
