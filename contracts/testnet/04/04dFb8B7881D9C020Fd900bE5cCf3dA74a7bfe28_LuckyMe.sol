...
contract LuckyMe is Ownable {
   ...
   */
  uint8 PriLevels;
  mapping(uint8 => mapping(uint8 => uint256)) public PrizeRefLevels;
  ...
  ********************************************************/
  ...
  constructor(address _DAI) {
    ...
    GameEntryFee[2] = 100 ether;
    GameEntryFee[3] = 250 ether;
    GameEntryFee[4] = 500 ether;
    GameEntryFee[5] = 1000 ether;
    GameEntryFee[6] = 10000 ether;
     ...
     */
    PriLevels = 5;
    PrizeRefLevels[0][0] = 22.85 ether;
    PrizeRefLevels[0][1] = 8.55 ether;
    PrizeRefLevels[0][2] = 5.7 ether;
    PrizeRefLevels[0][3] = 2.85 ether;
    PrizeRefLevels[0][4] = 2.85 ether;
    //             Total = 42.8 ether

    PrizeRefLevels[1][0] = 11.4 ether;
    PrizeRefLevels[1][1] = 4.3 ether;
    PrizeRefLevels[1][2] = 2.85 ether;
    PrizeRefLevels[1][3] = 1.45 ether;
    PrizeRefLevels[1][4] = 1.45 ether;
    //             Total = 21.45 ether

    PrizeRefLevels[2][0] = 5.7 ether;
    PrizeRefLevels[2][1] = 2.15 ether;
    PrizeRefLevels[2][2] = 1.4 ether;
    PrizeRefLevels[2][3] = 0.75 ether;
    PrizeRefLevels[2][4] = 0.75 ether;
    //             Total = 10.75 ether
  }
  ...
  ********************************************************/
  ...
  function register(
    ...
    if (!isUserExists(User[_ref].Address)) _ref = 1;
    ...
    uplineMemRef(_ref, _amount);
  }

  function renew(uint256 _id) public noReentrant {
    user memory _user = User[_id];

    require(_user.Id != 0, "User not exists");

    uint256 _amount = MembershipPlan[_user.Plan];
    DAI.safeTransferFrom(msg.sender, address(this), _amount);

    _user.TimeToRenew = block.timestamp + Year;

    uplineMemRef(_user.Ref, _amount);
  }
  ...
  function enterGame(
    ...
    require(
      _user.TimeToRenew > block.timestamp || _id == 1,
      "It's Time To Renew"
    );
    ...
    uplinePurRef(_user.Ref, _amount);
    _Game.TotalPrizeAmount += Percentage(_amount, 85);

    if (_Game.AllNumbers.length == 99) {
      for (uint8 i = 0; i < TotalPrizes; i++) {
        uint8 _luckyNumber = generateRandomNumber(i) % 99;
  ...
  function withdrawPrizes(uint8 _game, uint256 _gameId) public noReentrant {
    ...
    for (uint8 i = 0; i < 3; i++) {
      game storage _Game = Game[_game][_gameId];
      user memory _User = User[_Game.UserId[_Game.Winners[i]]];

      DAI.safeTransferFrom(
        address(this),
        _User.Address,
        Percentage(_amount, Prizes[i])
      );
      uplinePriRef(_User.Ref, i);
  ...
  ********************************************************/
  ...
  function uplineMemRef(uint256 _ref, uint256 _amount) private {
    uint8 j;
    while (j < RefLevels) {
      user memory _user = User[_ref];
      if (_user.TimeToRenew > block.timestamp || _ref == 1) {
        uint256 _Percentage = Percentage(_amount, MembershipRefLevels[j]);

        DAI.safeTransfer(_user.Address, _Percentage);
        _user.TotalReferralRewards = _Percentage;
  ...
  function uplinePurRef(uint256 _ref, uint256 _amount) private {
    uint8 j;
    while (j < PurLevels) {
      user memory _user = User[_ref];

      if (
        (_user.Plan == 1 && _user.TimeToRenew > block.timestamp) || _ref == 1
      ) {
        uint256 _Percentage = Percentage(_amount, PurchaseRefLevels[j]);

        DAI.safeTransfer(_user.Address, _Percentage);
        _user.TotalPicksAmount = _Percentage;
  ...
  function uplinePriRef(uint256 _ref, uint8 _i) private {
    uint8 j;
    while (j < PriLevels) {
      user memory _user = User[_ref];
      ...
      if (
        ...
        uint256 _amount = PrizeRefLevels[_i][j];

        DAI.safeTransfer(_user.Address, _amount);
        _user.TotalPrizesAmount = _amount;
  ...
  ********************************************************/
  ...
  function generateRandomNumber(uint8 _number) private view returns (uint8) {
    return
      uint8(
        uint256(
          keccak256(
            abi.encodePacked(block.timestamp, block.prevrandao, _number)
          )
        )
      );
  }
  ...
  ********************************************************/
  ...
  function gameDetails(
    ...
    uint8 _from,
    uint256 _length
  ) public view returns (currentGame[] memory) {
    uint256 _GameId = GameIds[_game].current();
    if (_GameId < _length) _length = _GameId;
    currentGame[] memory _currentGame = new currentGame[](_length);

    for (uint8 i = _from; i < _length; i++) {
      game storage _Game = Game[_game][i];

      _currentGame[i] = currentGame(
        i,
        _Game.GameOver,
        GameEntryFee[_game],
        _Game.Winners,
  ...
  function userDetail(address _user) public view returns (user memory _User) {
    _User = User[UserId[_user]];
  }
}
