...
contract Coin77 is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Coin 77";
    string private _symbol = "C77";
...
/// Excludes addresses for max sell
...
/// Sets max sell at start change to private
    uint256 public _maxSellAmount = 30 * 10**3 * 10**9 * 10**9;

/// function to set max sell must be marketing wallet
    function setMaxSell(uint256 maxTx) public {
        if (_msgSender() == marketingWalletAddress) {
            _maxSellAmount = maxTx;
        }
    }

/// is the mint function
...
/// Do I need to change uniswapPair? Owner doesnt need to be under maxSell or any excludedAccount or the uniswapPair
            if(sender != owner() && recipient != owner() && sender != uniswapPair && !excludedAccount) {
