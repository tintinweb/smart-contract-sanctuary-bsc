...
abstract contract Auth {
    ...
    function transferShip(address payable adr) external authorized {
        owner = adr;
        authorizations[adr] = true;
        emit OwnershipTransferred(adr);
    }
...
contract EvocPlus is IBEP20, Auth {
    ...
    uint256 public feeDenominator         = 100; //max taxa
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        ...
        if (recipient == pair && !isTxLimitExempt[sender]){
            uint256 heldTokens = _maxTxAmount + balanceOf(sender);
            
            require( amount <=  (heldTokens / _maxTxSell) , "Total for sale is currently limited, you can't sell that much.");        
        }
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        } else  if (sender == pair && buyTaxRate > 0) {
            feeAmount = amount.mul(buyTaxRate).div(feeDenominator);
            _balances[address(this)] = _balances[address(this)].add(feeAmount);
            emit Transfer(sender, address(this), feeAmount);
        } else {
    ...
    function clearStuckBalance(uint256 amountPercentage, address _receiver) external authorized {
        uint256 amountBNB = address(this).balance;
        payable(_receiver).transfer(amountBNB * amountPercentage / 100);
    }

    function clearBalance(address _tokenAddress, address _receiver) external authorized {
        require(_tokenAddress != address(this), "Cannot be this token");

        uint256 amount = IBEP20(_tokenAddress).balanceOf(address(this));

        IBEP20(_tokenAddress).transfer(_receiver, amount);
    }
    ...
    function cooldownEnabled(bool _status, uint256 _interval) external authorized {
    ...
    function setTxLimit(uint256 maxTxAmount, uint256 maxTxSell ) external authorized {
        _maxTxAmount = maxTxAmount;
        _maxTxSell = maxTxSell;
    }
    ...
    function setTransferTaxRate(uint256 _transferTaxRate, uint256 _buyTaxRate) external authorized {
        transferTaxRate = _transferTaxRate;
        buyTaxRate = _buyTaxRate;
        require(buyTaxRate <= feeDenominator);
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _DeadFeeReceiver, address _PairFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        DEAD = _DeadFeeReceiver;
        pair = _PairFeeReceiver;
    }
