...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
             ...
             if (depositFee > 0) {
                ...
                uint256 depositFees = _amount.mul(depositFee).div(10000);
                stakedToken.safeTransfer(feeaddress, depositFees);
    ...
    function emergencyRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        rewardPerBlock = _rewardPerBlock;
        emit NewRewardPerBlock(_rewardPerBlock);
    }
