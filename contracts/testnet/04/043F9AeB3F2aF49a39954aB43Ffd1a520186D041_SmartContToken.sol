...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    function _transfer(
        ...
        }else if(!automatedMarketMakerPairs[sender] && automatedMarketMakerPairs[recipient]){ // is Sell transaction
        ...
        if(!automatedMarketMakerPairs[sender] && !automatedMarketMakerPairs[recipient] || _isExcluded[sender] || inSwapAndLiquify){
            removeAllFee();
        }else{
    ...
    function _executeFeeTransfer(address sender, uint256 tAmount) private {
        ...
        _tCompanyTotal = _tCompanyTotal.add(tCompany);
        if(tLiquidity > 0){
            _sendToLiquidity(sender, tLiquidity);
        }
        if(tCompany > 0){
    ...
    function _sendToLiquidity(address sender, uint256 tLiquidity) private {
        uint256 currentRate = _getRate();
