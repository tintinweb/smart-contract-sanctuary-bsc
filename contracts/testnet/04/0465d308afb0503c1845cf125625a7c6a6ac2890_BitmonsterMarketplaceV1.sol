...
contract BitmonsterMarketplaceV1 is
    ...
    PausableUpgradeable
{
        ...
        onlyExistItem(marketId)
    {
        ...
        if (itemData._itemType == TokenType.ERC1155) {
            IERC1155(itemData._item).unlock(
                itemData._owner,
                itemData._tokenId,
                itemData._amount
            );
        } else if (itemData._itemType == TokenType.ERC721) {
            IERC721(itemData._item).lock(itemData._tokenId, false);
        }
    ...
    {
        items[marketId]._available = false;
        if (items[marketId]._itemType == TokenType.ERC1155) {
            IERC1155(items[marketId]._item).unlock(
                items[marketId]._owner,
                items[marketId]._tokenId,
                items[marketId]._amount
            );
        } else if (items[marketId]._itemType == TokenType.ERC721) {
            IERC721(items[marketId]._item).lock(
                items[marketId]._tokenId,
                false
            );
        }
