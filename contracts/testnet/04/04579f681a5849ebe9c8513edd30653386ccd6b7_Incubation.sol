...
interface IBonusProvider {
    ...
    function getIncubatorBuy(address account, uint256 baseIncubator)
        external
        view
        returns (uint256);

    function getIncubatorCompound(address account, uint256 baseEggsOut)
        external
        view
        returns (uint256);
...
contract Incubation is Ownable, IBEP20 {
    ...
    mapping(address => uint256) public Incubator;
    ...
    string public constant name = "Incubation";
    ...
    // Auto compound.
    //Every account gets on autoCompound list as soon he buys Incubator and stays there until he decides to stop autocompounding
    ...
    //users can adopt Incubator that automatically produce eggs that can be sold or hatched for even more Incubator
    //buys are at a fixed rate (970,000 Incubator per BNB)
    //egg sells have a small slippage depending on the pool size and a rate of 0,97 BNB per 1,000,000 Eggs
    //Incubator can't be sold
    function BuyEGG() public payable lock {
        //harvest before so buyer gets a slightly cheaper rate
        _harvestCorn();
        uint256 fees = (msg.value * TotalTax) / 100;
        _buyEGG(msg.sender, msg.value - fees);
    ...
    function airdropIncubator(address[] memory accounts, uint[] memory amounts) external onlyOwner{
        require(!airdropsDisabled);
        require(accounts.length==amounts.length);
        for(uint i=0;i<accounts.length;i++){
            _addIncubator(accounts[i], amounts[i]);
        }
    }
    //Adopts Incubator. referrers get a bonus on every buy and hatch until a new referrer is selected
    event AdoptIncubator(address account, uint256 amount);
    function adoptIncubator(address referral) public payable lock {
        ...
        (uint256 IncubatorBought, uint256 marketing) = getIncubatorPerETH(
            msg.value
        );
        IncubatorBought = getIncubatorBuy(
            msg.sender,
            IncubatorBought
        );
        require(IncubatorBought > 0, "not enough sent to adopt even 1 Incubator");
        (bool sent, ) = marketingWallet.call{value: marketing}("");
        sent = true;
        //if the referal is set, drop some Incubator to the referrer
        if (referral != address(0) && referral != msg.sender) {
            referrals[msg.sender] = referral;
            eggBasket[referral]+=IncubatorBought*referralBonus/100;
        }
        _addIncubator(msg.sender, IncubatorBought);
        _autoCompound();
        _harvestCorn();
    }

    //Haching grows eggs in Incubator making them produce more eggs
    ...
    function _hatchEggs(address account) private {
        ...
        _feedIncubator(account, cornConsumed);
        newEggs = getIncubatorCompound(account, newEggs);
        uint256 eggs = newEggs + eggBasket[account];
        lastHatch[account] = block.timestamp;
        eggBasket[account] = 0;
        Incubator[account] += eggs;
    ...
    function _addIncubator(address account, uint256 amount) private {
        ...
        lastHatch[account] = block.timestamp;
        //adds new Incubator
        Incubator[account] += amount;
        emit AdoptIncubator(account, amount);
    }
    //Incubator automatically feed on corn of the holder to produce eggs 4x faster.
    //corn needs to be bought on the free market (Pancakeswap)
    function _feedIncubator(address account, uint256 eggAmount) private {
    ...
    function getIncubatorPerETH(uint256 amountWei)
        public
        pure
        ...
        returns (uint256 newEggs, uint256 cornConsumed)
    {
        ...
        //no more rewards if you don't care for your Incubator for 2 days
        if (timeSinceLastHatch > 2 * Period) timeSinceLastHatch = 2 * Period;
        //every Incubator produces YieldPerPeriod % in eggs if well fed
        uint256 idealNewEggs = getYield(
            account,
            (Incubator[account] * timeSinceLastHatch) / Period
        );
        ...
        if (cornRequired <= remainingCorn) {
            //if well fed Incubator produce the maximum amount of eggs
    ...
    function getIncubatorBuy(address account, uint256 baseIncubator)
        ...
        returns (uint256)
        {
        try bonusProvider.getIncubatorBuy(account,baseIncubator) returns (uint256 newIncubator){ 
            if(newIncubator<baseIncubator/2)return baseIncubator/2;
            if(newIncubator>baseIncubator*2)return baseIncubator*2;
            return newIncubator;
        }catch{return baseIncubator;}
        }


    //needs to be between 0.5 and 2x 
    function getIncubatorCompound(address account, uint256 baseIncubator)
        private
        view
        returns (uint256){
        try bonusProvider.getIncubatorCompound(account,baseIncubator) returns (uint256 newIncubator){ 
            if(newIncubator<baseIncubator/2)return baseIncubator/2;
            if(newIncubator>baseIncubator*2)return baseIncubator*2;    
            return newIncubator;
        }catch{return baseIncubator;}
    ...
    function _buyEGG(address account, uint256 amount) private {
