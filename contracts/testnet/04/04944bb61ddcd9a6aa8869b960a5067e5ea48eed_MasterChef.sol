...
contract TestToken is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0xD0594d6D2892927d66e64de63ee1c81A5F3105fd;
    ...
    // Min amount to liquify. (default 1 TEST)
    ...
    // The operator can call all functions except of minting TestToken
    ...
    modifier blackList(address sender, address recipent) {
        if (maxHolding() > 0) {
            if (_includeToBlackList[sender] == true || _includeToBlackList[recipent] == true) {
                require(_includeToBlackList[sender] == false,"TEST::blackList: You have been blacklisted as a bot (SENDER)");
                require(_includeToBlackList[recipent] == false,"TEST::blackList: You have been blacklisted as a bot (RECIPENT)");
            }
        }
        _;
    }
     ...
     */
    constructor() public BEP20("TestToken", "TEST") {
        ...
        _includeToBlackList[burnAddr] = false;
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the TEST -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "TEST::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "TEST::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "TEST::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "TEST::updateSwapRouter: Invalid pair address.");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Tests to distribute per second.
        uint256 lastRewardSecond; // Last second that Tests distribution occurs.
        uint256 accTestPerShare; // Accumulated Tests per share, times 1e18. See below.
        uint16 depositFeeBP; // Withdrawal fee in basis points
        uint256 lpSupply;
    }
    ...
    // TEST tokens created per second.
    uint256 public TestPerSecond = 0.1 ether;
    // Fee address
    address public feeAddress;
    // Max emission rate.
    uint256 public constant MAX_EMISSION_RATE = 15 ether;

    uint256 constant MAX_TEST_SUPPLY = 600000 ether;
    ...
    // The block timestamp when TestToken mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 TestPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    // Update the given pool's TestToken allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending TestToken on frontend.
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 testReward = multiplier
                .mul(TestPerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            uint256 devReward = testReward.div(10);
            uint256 totalRewards = testToken.totalSupply().add(devReward).add(
                testReward
            );
                if (totalRewards > MAX_TEST_SUPPLY) { 
                testReward = MAX_TEST_SUPPLY.sub(testToken.totalSupply());
            }
            accTestPerShare = accTestPerShare.add(
                testReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalRewards <= MAX_TEST_SUPPLY) {  // NEEDS CHECKING HERE
            // mint as normal as not at maxSupply
            testToken.mint(devaddr, testReward.div(10));
            testToken.mint(address(this), testReward);
        } else {
            // mint the difference only to MC, update testReward
            testReward = MAX_TEST_SUPPLY.sub(testToken.totalSupply());
            testToken.mint(address(this), testReward);
        }

        if (testReward != 0) {
            // only calculate and update if testReward is non 0
            pool.accTestPerShare = pool.accTestPerShare.add(
                testReward.mul(1e18).div(pool.lpSupply)
            );
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                safeTestTokenTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) { 
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accTestPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeTestTokenTransfer(msg.sender, pending);
    ...
    // Safe TestToken transfer function, just in case if rounding error causes pool to not have enough TestTokens.
    function safeTestTokenTransfer(address _to, uint256 _amount) internal {
        uint256 testTokenBal = testToken.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > testTokenBal) {
            transferSuccess = testToken.transfer(_to, testTokenBal);
        } else {
            transferSuccess = testToken.transfer(_to, _amount);
        }
        require(transferSuccess, "safeTestTokenTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _TestPerSecond) external onlyOwner {
        require(_TestPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        TestPerSecond = _TestPerSecond;
        emit UpdateEmissionRate(msg.sender, _TestPerSecond);
    }
