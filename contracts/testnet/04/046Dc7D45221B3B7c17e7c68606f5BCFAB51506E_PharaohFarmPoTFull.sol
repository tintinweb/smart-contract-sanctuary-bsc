...
contract PharaohFarmPoTFull is Ownable, ReentrancyGuard{
    ...
    // }
        ...
        1000;
emit Reffered(msg.sender,referAddress,refRew);
        if (available + claimed+ refRew <= max && referAddress != firstAddress) {
        emit ReferralRewarded(msg.sender,referAddress, refRew);
            user_data[referAddress].referralRewards +=refRew;
                        ...
                        user_data[referAddress].referralRewards += gib;
                        emit ReferralRewarded(msg.sender,referAddress, refRew);
...
emit Reffered(msg.sender,uplineUserAddress,rew);
...
emit Reffered(msg.sender,uplineUserAddress,rew);
 ...
 if (interest + claimed > maximumInterest) {
    ...
    function doubleUp() external nonReentrant {
        ...
        }else if( max - (claimed + available )>0 && oldRef != firstAddress){
                        uint256 gib = max   - (claimed+ available) ;
                        if(gib > _refRew){
                            gib= _refRew;
                        }
                        user_data[oldRef].referralRewards += gib;
                        emit ReferralRewarded(msg.sender,oldRef, _refRew);
                        reinvestRefer(oldRef, finalAmount);
