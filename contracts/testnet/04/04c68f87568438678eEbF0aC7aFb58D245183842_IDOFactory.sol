...
contract IDOFactory is Ownable {
    ...
    mapping(uint256 => address) public idoIdToIDOContract;
    mapping(address => uint256[]) public ownerToIDOs;
    ...
    function createIDO(
        ...
        uint256 idoIDToAssign = idoIdTracker.current();
        ...
        idoIdToIDOContract[idoIDToAssign] = address(ido);
        ownerToIDOs[msg.sender].push(idoIDToAssign);


        emit IDOCreated(idoIDToAssign, address(ido));

        return idoIDToAssign;
    }
     ...
     * @param _idoId: Index of the IDO
     * @param _finalizer: Address of finalizer
     */
    function finalizeIDO(uint256 _idoId, address _finalizer) external onlyOwner  {
        ...
        IDO(idoIdToIDOContract[_idoId]).finalize(owner(), _finalizer, feePercent, feeRecipient);
    }
     ...
     * @param _idoId: Index of the IDO
     */
    function emergencyRefund(uint256 _idoId) external onlyOwner {
        IDO(idoIdToIDOContract[_idoId]).emergencyRefund();
    }
