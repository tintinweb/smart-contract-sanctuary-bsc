...
contract GoRunnerIDO is ERC20, ERC20Pausable, Ownable, ReentrancyGuard {
    ...
    */
    function buy() payable nonReentrant onlyEOA external returns (bool success) {
        require(block.timestamp>=1648863900 && block.timestamp<=1648864200, "GoRunnerIDO: not in IDO time"); //////////要改时间
        require(totalContributedAmount + msg.value <= 2 ether, 'GoRunnerIDO: max contribution amount 100 BNB reached'); //////////// 要改回 100
        require(msg.value >= 0.1 ether && msg.value <= 1 ether, 'GoRunnerIDO: need to be between 0.1 and 1 BNB');
        require(msg.value*10%(1 ether) == 0, 'GoRunnerIDO: need to be rounded');
        UserInfo storage user = contributors[msg.sender];
        require(user.contributedAmount+msg.value<=1 ether, "GoRunnerIDO: max contribution amount 1 BNB per user reached'");
        if(!user.contributed){
    ...
    function claim() whenNotPaused onlyEOA external returns (bool success) {
        require(isContributor(msg.sender), "GoRunnerIDO: never contributed");
        require(isUserClaimable(msg.sender), "GoRunnerIDO: already withdrawn");
        require(totalClaimableAmount != 0, "GoRunnerIDO: not supplied");
        UserInfo storage user = contributors[msg.sender];
