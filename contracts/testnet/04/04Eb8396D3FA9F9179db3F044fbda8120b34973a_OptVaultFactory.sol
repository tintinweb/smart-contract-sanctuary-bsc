...
import "./interfaces/IBYSLVault.sol";
import "./interfaces/IYSLVault.sol";
import "./interfaces/IxYSLVault.sol";
import "./interfaces/IUSDyVault.sol";
import "./interfaces/IBshareVault.sol";
import "./interfaces/IYSLBUSDVault.sol";
import "./interfaces/IxYSLBUSDVault.sol";
import "./interfaces/IUSDyBUSDVault.sol";
import "./interfaces/IBshareBUSDVault.sol";
import "./interfaces/INTT.sol";
import "./interfaces/IPhoenixNft.sol";
import "./nft/PhoenixNft.sol";
import "@openzeppelin/contracts/proxy/Clones.sol";
import "./interfaces/IOpt1155.sol";
import "hardhat/console.sol";

contract OptVaultFactory is AccessControl, Initializable {
    ...
    // modifier isActiveUntill(uint256 _pid) {
    ...
    */
    function add(
        ...
        // require(_strat != address(0), "OptVaultFactory: Zero address strategy");
        // require(
        //     address(_token) == address(IOptVault(_strat).vaultToken()),
        //     "OptVaultFactory: Incorrect underlying"
        // );
        address instance = Clones.clone(Admin.masterNTT());
        if(isLpToken){
        address StratLp = Clones.clone(OptVaultLp);
        PoolInfo memory pool = PoolInfo(_token,StratLp,instance,true,isLpToken,false,6000);
        poolInfo.push(pool);
        IOptVaultLp(StratLp).initialize(poolInfo.length,  msg.sender, address(_token), _want, masterChef, address(Admin));
        IOptVaultLp(StratLp).setreciept(Admin.masterNTT());
        INTT(instance).initialize(StratLp, _name, _symbol);
        }
        else 
        {
        address Strat = Clones.clone(OptVault);
        PoolInfo memory pool = PoolInfo(_token,Strat,instance,true,isLpToken,false,6000);
        poolInfo.push(pool);
        IOptVault(Strat).initialize(poolInfo.length, msg.sender, address(_token), _want, masterChef,address(Admin));
        IOptVault(Strat).setreciept(instance);
        INTT(instance).initialize(Strat, _name, _symbol);

        // console.log("masterntt",instance);
        }
        totalAllocPoint += poolInfo[poolInfo.length-1].multiplier;
        PIDsOfRewardVault[address(_token)] = poolInfo.length-1;
        IOpt1155(IAdmin(Admin).Opt1155()).createNFTForVault(poolInfo.length, _name);
        // IPancakeMaster(masterChef).add(poolInfo[poolInfo.length-1].multiplier,IBEP20(address(_token)),true);
        // console.log("PIDsOfRewardVault[_token]",PIDsOfRewardVault[address(_token)]);
        return (instance);
    }
    function addCustomVaults(IERC20 _token,address vaultAddress,address receiptToken,string memory _name,string memory _symbol ) external onlyRole(DEFAULT_ADMIN_ROLE){
        if(vaultAddress == address(Admin.YSLVault())){
        IYSLVault(Admin.YSLVault()).initialize(address(Admin),owner,Admin.ApeswapRouter());
        PoolInfo memory pool = PoolInfo(IERC20(Admin.YSL()),Admin.YSLVault(),receiptToken,true,false,true,0);
        poolInfo.push(pool);
        }
        else{
            if(vaultAddress == address(Admin.xYSLVault())){
            IxYSLVault(Admin.xYSLVault()).initialize(address(Admin),owner,Admin.ApeswapRouter());
            PoolInfo memory pool = PoolInfo(IERC20(Admin.xYSL()),Admin.xYSLVault(),receiptToken,true,false,true,0);
            poolInfo.push(pool);
            }
            else{
                if(vaultAddress == address(Admin.USDyVault())){
                IUSDyVault(Admin.USDyVault()).initialize(address(Admin),owner,Admin.ApeswapRouter());
                PoolInfo memory pool = PoolInfo(IERC20(Admin.USDy()),Admin.USDyVault(),receiptToken,true,false,true,0);
                poolInfo.push(pool);
                }
                else{
                    if(vaultAddress == address(Admin.BShareVault())){
                    IBshareVault(Admin.BShareVault()).initialize(address(Admin),owner,Admin.ApeswapRouter());
                    PoolInfo memory pool = PoolInfo(IERC20(Admin.BShare()),Admin.BShareVault(),receiptToken,true,false,true,0);
                    poolInfo.push(pool);
                    }
                }
            }
    ...
    function addCustomLPVaults(IERC20 _token,address vaultAddress,address receiptToken,string memory _name,string memory _symbol ) external onlyRole(DEFAULT_ADMIN_ROLE){
        if(vaultAddress == address(Admin.YSLBUSDVault())){
        IYSLBUSDVault(Admin.YSLBUSDVault()).initialize(address(Admin),owner,address(_token),Admin.ApeswapRouter());
        PoolInfo memory pool = PoolInfo(_token,Admin.YSLBUSDVault(),receiptToken,true,false,true,0);
        poolInfo.push(pool);
        }
        else{
            if(vaultAddress == address(Admin.xYSLBUSDVault())){
            IxYSLBUSDVault(Admin.xYSLBUSDVault()).initialize(address(Admin),owner,address(_token),Admin.ApeswapRouter());
            PoolInfo memory pool = PoolInfo(_token,Admin.xYSLBUSDVault(),receiptToken,true,false,true,0);
            poolInfo.push(pool);
            }
            else{
                if(vaultAddress == address(Admin.USDyBUSDVault())){
                IUSDyBUSDVault(Admin.USDyBUSDVault()).initialize(address(Admin),owner,address(_token),Admin.ApeswapRouter());
                PoolInfo memory pool = PoolInfo(_token,Admin.USDyBUSDVault(),receiptToken,true,false,true,0);
                poolInfo.push(pool);
                }
                else{
                    if(vaultAddress == address(Admin.BShareBUSDVault())){
                    IBshareBUSDVault(Admin.BShareBUSDVault()).initialize(address(Admin),owner,address(_token),Admin.ApeswapRouter());
                    PoolInfo memory pool = PoolInfo(_token,Admin.BShareBUSDVault(),receiptToken,true,false,true,0);
                    poolInfo.push(pool);
                    }
                }
            }
     ...
     */
    function setMultiplier(uint256 pid, uint256 _number)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_number > 0, "OptVaultFactory: Invalid Amount");
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if (poolInfo[id].status) {
            ...
            totalAllocPoint -= poolInfo[id].multiplier;
        } else {
    ...
    function Deposit(address user,address _token,uint256 _amount,bool isReceipt)
        external payable
        isActive(PIDsOfRewardVault[_token])
    {
        require(
            _amount >=
                INTT(poolInfo[PIDsOfRewardVault[_token]].recieptInstance)
                    .balanceOf(user),
        ...
        // IERC20(_token).approve(poolInfo[PIDsOfRewardVault[_token]].vault, _amount);
        if(poolInfo[PIDsOfRewardVault[_token]].isLp){
        IOptVaultLp(poolInfo[PIDsOfRewardVault[_token]].vault).deposit{value : msg.value}(user,_amount,isReceipt);
        }
        else{
        IOptVault(poolInfo[PIDsOfRewardVault[_token]].vault).deposit(user,_amount,isReceipt);
        }
    ...
    function withdraw(address user,address _token,bool isReceipt,uint _recieptAmount) external  {
        ...
        if(poolInfo[PIDsOfRewardVault[_token]].isLp){
        IOptVaultLp(poolInfo[PIDsOfRewardVault[_token]].vault).withdraw(user, isReceipt, _recieptAmount);
        }
        else{
        IOptVault(poolInfo[PIDsOfRewardVault[_token]].vault).withdraw(user,isReceipt,_recieptAmount);
        }
        IOpt1155(IAdmin(Admin).Opt1155()).burn(user, PIDsOfRewardVault[_token], 1);
        userInfo[PIDsOfRewardVault[_token]][user].NFT = nftStatus(0);
    }

    function calculateAPR() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            liqStatusValue == LiqStatus.SWAP_WANT_TO_BUSD,
        ...
        for (uint256 id = 0; id < poolInfo.length; id++) {
            if (poolInfo[id].status) {
                if(poolInfo[id].isLp){
                IOptVaultLp(poolInfo[id].vault).swapWantToBUSD();
                }
                else{
     ...
     */
    function calculateControllerFee() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            liqStatusValue == LiqStatus.CONTROLLER_FEE,
        ...
        for (uint256 id = 0; id < poolInfo.length; id++) {
            if (poolInfo[id].status) {
                ...
                if(poolInfo[id].isLp){
                IOptVaultLp(poolInfo[id].vault).deductControllerFee(amount);
                }
                else{
    ...
    function optimization() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            liqStatusValue == LiqStatus.OPTIMIZATION_TAX,
        ...
        for (uint256 id = 0; id < poolInfo.length; id++) {
            if (poolInfo[id].status) {
                // USDy.mint(distributor, BUSD.balanceOf(poolInfo[id].vault));
                IUSDY(IAdmin(Admin).USDy()).mint(Admin.temporaryHolding(), IERC20(IAdmin(Admin).BUSD()).balanceOf(poolInfo[id].vault));
                if(poolInfo[id].isLp){
                IOptVaultLp(poolInfo[id].vault).collectOptimizationTax();
                }
                else{
    ...
    function optimizationRewards(address _token)
        external
    {
        uint256 id = PIDsOfRewardVault[_token];
        if (
            poolInfo[id].status &&
            block.timestamp <= userInfo[id][msg.sender].time + 30 days &&
            userInfo[id][msg.sender].phoenixNFTStatus
        ) {
            uint256 modulus = (block.timestamp -
                userInfo[id][msg.sender].time) / epochTime;
            if(poolInfo[id].isLp){
            IOptVaultLp(poolInfo[id].vault).optimizationReward(msg.sender,
                ((poolInfo[id].multiplier +
                    ((poolInfo[id].multiplier) * 25) /
                    100) * (80**modulus)) / (100**modulus)
                );
            }
            else{
             IOptVault(poolInfo[id].vault).optimizationReward(msg.sender,
                ((poolInfo[id].multiplier +
                    ((poolInfo[id].multiplier) * 25) /
                    100) * (80**modulus)) / (100**modulus)
                );
            ...
            if (
                ...
                IOptVault(poolInfo[id].vault).optimizationReward(msg.sender,
                    (poolInfo[id].multiplier * (80**modulus)) / (100**modulus)
                );
                    }
            } else {
                if (
                    ...
                    uint256 modulus = (block.timestamp -
                        userInfo[id][msg.sender].time) / epochTime;
                    // console.log((block.timestamp - userInfo[id][msg.sender].time),block.timestamp,
                    //     userInfo[id][msg.sender].time,epochTime);
                    uint256 epochLimit = 3 * 30;
                    // console.log("modulus",modulus);
                    // console.log("multiplier",poolInfo[id].multiplier);
                    // console.log("(80**epochLimit) = ",(80**epochLimit));
                    // console.log("numerator",(poolInfo[id].multiplier * (80**epochLimit)));
                    // console.log("denomenator",(100**epochLimit));
                    // console.log("optMul",(poolInfo[id].multiplier * (80**epochLimit)) /
                    //         (100**epochLimit));
                            if(poolInfo[id].isLp){
                                 IOptVaultLp(poolInfo[id].vault).optimizationReward(msg.sender,
                                                ((poolInfo[id].multiplier +
                    ...
                    IOptVault(poolInfo[id].vault).optimizationReward(msg.sender,
                        (poolInfo[id].multiplier * (80**epochLimit)) /
                            (100**epochLimit)
                    );
...
interface IOptVault {
    function initialize(uint256 _id, address _owner,  address _token, address _want, address _masterChef, address _Admin) external  ;
...
interface IOptVaultLp {
    ...
    function ssetTokenSwap(address _tokenSwap) external;
    function optimizationReward(address user, uint optMultiplier) external;
    function deductControllerFee(uint fee) external;
    function collectOptimizationTax() external;
