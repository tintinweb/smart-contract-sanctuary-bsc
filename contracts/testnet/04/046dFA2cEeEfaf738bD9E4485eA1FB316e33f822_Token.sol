...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "2 girls 1 cup";
    ...
    constructor (address _wbnb,address _route,address _holder,address _fundAddress,address _marketAddress) public {
    ...
    function _getValues(uint256 tAmount,bool takeFee,address user) private view returns (Param memory param) {
        ...
        if(takeFee){
            ...
            if( tier > 1 ){
                _shareFee = tier - 1;
                _shareFee = _shareFee > 8 ? 8 : _shareFee;
                param.tShare = tAmount * _shareFee / 100;
            }
            uint _totalFee = _liquidityFee + _burnFee + _fundFee + _taxFee + _marketFee + _shareFee;
    ...
    function _takeFee(Param memory param,address from,address user)private {
        ...
        _takeFund(param.tFund,currentRate,from);
