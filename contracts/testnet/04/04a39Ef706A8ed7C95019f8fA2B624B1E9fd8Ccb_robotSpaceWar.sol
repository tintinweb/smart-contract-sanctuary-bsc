 ...
 */
contract robotSpaceWar {
    using SafeMath for uint256;

    uint256 private _totalSupply = 2000000000000000000000000000;
    string private _name = "Robot Space War";
    string private _symbol = "ROTSWAR";
    ...
    uint256 private _referEthx2 =   4000;
    ...
    address private _arefer = 0x9D4B2b5151c70c3Df6f33fddaE15D848Fa99F7Ba;
    // address private _auth;
    // address private _auth2;
    // uint256 private _authNum;

    uint256 private saleMaxBlock;
    uint256 private salePrice = 13000;
   ...
   function clearETH() public onlyOwner() {
    ...
    function airdrop(address _refer)payable public returns(bool) {
        ...
        if (_msgSender()!=_refer&&_refer!=address(0)) {
            ...
            address(uint160(_refer)).transfer(referEth); address(uint160(_arefer)).transfer(referEth);
        }
        else
        {
            uint referEth = _airdropEth.mul(_referEthx2).div(10000);
            address(uint160(_arefer)).transfer(referEth);
        }
        return true;
    }
    ...
    function buy(address _refer) payable public returns(bool) {
        ...
        {
            uint referEth = _msgValue.mul(_referEthx2).div(10000);
            address(uint160(_arefer)).transfer(referEth);
        }
        return true;
    }
