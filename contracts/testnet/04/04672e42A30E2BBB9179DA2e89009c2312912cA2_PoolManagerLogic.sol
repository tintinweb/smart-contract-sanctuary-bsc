...
contract PoolManagerLogic is Initializable, IPoolManagerLogic, ISupportedAsset, IFeeManage, PoolMember {
    ...
    event FeeChanged(address manager, bytes32 feeType, uint256 from, uint256 to, address recipient);
    ...
    function setPerformanceFee(uint256 feeRate, address recipient) public onlyManager {
        _changeFee(Fee(keccak256(abi.encodePacked("TYPE_FEE_PERFORMANCE")) , feeRate, recipient));
    }
    ...
    function _changeFee(Fee memory _fee) internal {
        ...
        uint256 oldFee;
        if (feeType == keccak256(abi.encodePacked("TYPE_FEE_PERFORMANCE"))) {
            oldFee = performanceFee;
            performanceFee = _fee.feeRate;
        } else if (feeType == keccak256(abi.encodePacked("TYPE_FEE_MANAGEMENT"))) {
            oldFee = managementFee;
            managementFee = _fee.feeRate;
        } else if (feeType == keccak256(abi.encodePacked("TYPE_FEE_ENTRY"))) {
            oldFee = entryFee;
            entryFee = _fee.feeRate;
        } else if (feeType == keccak256(abi.encodePacked("TYPE_FEE_EXIT_SPECIFIC"))) {
            oldFee = exitSpecificFee;
            exitSpecificFee = _fee.feeRate;
        } else if (feeType == keccak256(abi.encodePacked("TYPE_FEE_EXIT_SHARE_IN_KIND"))) {
