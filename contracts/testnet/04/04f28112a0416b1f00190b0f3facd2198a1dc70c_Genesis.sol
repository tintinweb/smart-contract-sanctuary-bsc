...
import { IGenesis, IStriveNFT } from "./interfaces/IGenesis.sol";
...
contract Genesis is ERC721EnumerableUpgradeable, AccessControlEnumerableUpgradeable, IGenesis {
	using SafeERC20Upgradeable for IERC20Upgradeable;
	using ECDSAUpgradeable for bytes;
	using StringsUpgradeable for uint256;

    uint256 private constant FIAT_DECIMALS = 1 ether;

	bytes32 public constant METADATA_MANAGER_ROLE = keccak256("METADATA_MANAGER");
	bytes32 public constant WHITELIST_SIGNER_ROLE = keccak256("WHITELIST_SIGNER");
	bytes32 public constant MINTING_MANAGER_ROLE = keccak256("MINTING_MANAGER");
	bytes32 public constant DEFAULT_APPROVED_ROLE = keccak256("DEFAULT_APPROVED");

	uint256 public constant MASTER_TOKEN_ZERO = 0;
	uint256 public constant START_ID = 0;
	uint256 public constant REFERRAL_LAYERS = 3;
	...
	function initialize(
		...
		uint256 mintingFeeInUSD
	)
		external override initializer
	{
		__ERC721_init("STRIVE FOR GREATNESS", "STRV");
		__AccessControlEnumerable_init();
		require(mintingFeeInUSD >= 10, "minting fee too low");
		...
		_mintingFee = uint120(mintingFeeInUSD * FIAT_DECIMALS);
	}
   ...
   // External user functions
		...
		external override 
	{
		...
		(bool isValidReferral, address[REFERRAL_LAYERS] memory referrers) = _getReferral(msg.sender, referral);
        ...
        _processPayment(IERC20Upgradeable(currency), _mintingFee * amount, referrers);
		...
		external override 
	{
		...
		emit RequestByToken(tokenId, keys, values);
	}

 // external AC Functions
	function allocate(
		address recipient, 
		uint256 amount,
		...
		external override onlyRole(DEFAULT_ADMIN_ROLE) 
	{
		if(currency == address(0)) {
			...
			IERC20Upgradeable(currency).safeTransfer(recipient, amount);
		}
	}
		...
		external override onlyRole(METADATA_MANAGER_ROLE) 
	{
		...
		emit MetadataChanged(_unrevealedMetadata, _revealedMetadata, _revealedUntil);
	}
	...
	{
		require(mintingFee_ >= 10 * FIAT_DECIMALS, "minting fee too low");
 ...
 // INTERNAL Non view
	...
	function _mintRangeWithReferrerToken(
		...
		// we save the referrer token for each token minted and also push the tokens minted into the referrer tokens´s array
		uint16[] storage inviteesByToken = _getTokenInStorage(referrerToken).firstGenChildren;
		// if referrerToken == 0, we dont have to save the referrer token explicitly
		if(referrerToken == MASTER_TOKEN_ZERO) {
			while(toBeMintedNext < totalSupplyAfter) {
				_mint(recipient, toBeMintedNext);
				inviteesByToken.push(uint16(toBeMintedNext));

				emit Minted(msg.sender, toBeMintedNext, referrerToken);
			...
			while(toBeMintedNext < totalSupplyAfter) {
				_mint(recipient, toBeMintedNext);
				inviteesByToken.push(uint16(toBeMintedNext));
				_getTokenInStorage(toBeMintedNext).referrerToken = referrerToken;
		...
		_totalSupply = uint32(totalSupplyAfter);
	}
	...
	function _processPayment(
		...
		internal 
	{
        if(currency != USDT && currency != USDC) revert OnlyUSDTAndUSDC(address(USDT), address(USDC));

		currency.safeTransferFrom(msg.sender, address(this), feeInUSD);

		for(uint256 i = 0; i < referrers.length; i++) {
			if(referrers[i] == address(0)) break;
			currency.safeTransfer(referrers[i], _calculateReferralAmount(feeInUSD, i));
		}
    }
 ...
 // VIEW
  // INTERNAL VIEW and PUBLIC VIEW
   // minting logic internal helpers
	...
	{
		isValid = _verifyReferralSignature(referee, referral);

		// if the invitee is invited by Strive (MASTER_TOKEN_ZERO) the referrers are empty
		if(referral.tokenId == MASTER_TOKEN_ZERO || !isValid) return (isValid, referrers);

		// now populate the referrers[] up to REFERRAL_LAYERS referrers upstream
		for(uint256 i = 0; i < REFERRAL_LAYERS; i++) {
			referrers[i] = ownerOf(referral.tokenId);
			referral.tokenId = _getTokenInStorage(referral.tokenId).referrerToken;

			if(referral.tokenId == MASTER_TOKEN_ZERO) break;
		}
	}
   ...
   // parent contract overrides
	function isApprovedForAll(
		address owner, 
		address operator
	) 
		public view virtual override(IERC721Upgradeable, ERC721Upgradeable)
		returns (bool)
	{
	...
	function hasRole(bytes32 role, address account) 
		public view override(IAccessControlUpgradeable, AccessControlUpgradeable) 
		returns (bool) 
	{
  ...
  // EXTERNAL VIEW
   // Token Queries
	...
	function getReferralRoute(uint256 tokenId, uint256 length) external view override returns(uint256[] memory) {
		uint256[] memory route = new uint256[](length);
	...
	function getDirectDescendants(uint256 tokenId) external view override returns(uint16[] memory) {
		return _getTokenInStorage(tokenId).firstGenChildren;
	}

	function processReferral(
        address referee,
		Referral calldata referral
    ) external view returns(bool isValid, address[REFERRAL_LAYERS] memory referrers) {
		return _getReferral(referee, referral);
	}
   ...
   // other contract state queries:
	...
	function totalSupply() public view override(ERC721EnumerableUpgradeable, IStriveNFT) returns(uint256) {
		return _totalSupply;
	}

	function getMintingFeeInUSD() external view override returns(uint256) {
		return uint256(_mintingFee) / FIAT_DECIMALS;
	}
	...
	function verifyReferralSignature(
		...
		return _verifyReferralSignature(referee, referral);
	}
...
interface IStriveNFT {
   ...
   // EVENTS
    ...
    event RequestByToken(uint256 indexed tokenId, bytes32[] indexed key, bytes[] value);
    event Minted(address indexed minter, uint256 mintedTokenId, uint256 indexed referrerTokenId);
   ...
   // VIEW EXTERNAL
    ...
    /// @return totalSupply amount of NFTs minted - amount of NFTs burned
	function totalSupply() external view returns(uint256);

    /// @return mintingFeeInCents US dollar cent value of the minting fee
    ...
    function verifyReferralSignature(address referee, Referral calldata referral) external view returns(bool);
    ...
    /// @return isValid signals if the signature is a valid strive referral signature
    /// @return referrer current owners of the referrer token and optionally 
    ///                  the owners of the next 2 referrer token upstream
    /// @dev this method is gas-efficient and also called from the 2nd NFT Collection
    function processReferral(
        address referee,
        Referral calldata referral
    ) external view returns(bool isValid, address[3] memory referrer);

    function START_ID() external view returns(uint256 smallestTokenId);
  ...
  // Access Control Functions: 
   // ADMIN:
	...
	function retrieve(address currency, address recipient, uint256 amount) external;
   ...
   // MINTING_MANAGER:
    /// @param mintingFee_ in units of USDC: 1_000_000 => 1 USD6
    /// @dev reverts on input less than 10 USD (10**7)
	function setMintingFee(uint120 mintingFee_) external;
...
import { IStriveNFT } from "./IStriveNFT.sol";


interface IGenesis is IStriveNFT {
    ...
    /// @return tokenIds all the tokenIds directly referred by `tokenId`
    function getDirectDescendants(uint256 tokenId) external view returns(uint16[] memory);
}
