...
contract Staking is Initializable, OwnableUpgradeable {
    ...
    ERC20Upgradeable public gems;
    ISToken public sGEMS;
    ...
    function updateConfiguration(address _gems, address _sGEMS, address _treasury, uint256 _minimumLimitToStake) public onlyOwner {
        gems = ERC20Upgradeable(_gems);
        sGEMS = ISToken(_sGEMS);
        treasury = ITreasury(_treasury);
        minimumLimitToStake=_minimumLimitToStake*10**gems.decimals();
    ...
    function stake(uint256 amount) public {
        uint256 originSTokenAmount = sGEMS.balanceOf(msg.sender);
        ...
        sGEMS.safeTransfer(msg.sender,_tBalanceAdded);
    ...
    function unstake(uint256 amount) public {
        ...
        uint256 balanceBefore = sGEMS.balanceOf(address(this));
        sGEMS.safeTransferFrom(msg.sender, address(this), amount);
        uint256 _tBalanceAdded = sGEMS.balanceOf(address(this)).sub(
            balanceBefore
        );
        gems.safeTransfer(msg.sender,_tBalanceAdded);
    ...
    function rebase() public returns (uint256) {
        ...
        bool succeed=sGEMS.distributeRewards(_tBalanceAdded);
        if(succeed){
    ...
    function takeFeeForSTokenTransaction(uint256 amount) external {
        uint256 balanceBefore = sGEMS.balanceOf(address(this));
        sGEMS.safeTransferFrom(msg.sender, address(this), amount);
        uint256 _tBalanceAdded = sGEMS.balanceOf(address(this)).sub(
            balanceBefore
        );
        gems.approve(address(treasury), _tBalanceAdded);
        treasury.depositReward(_tBalanceAdded);
    }
