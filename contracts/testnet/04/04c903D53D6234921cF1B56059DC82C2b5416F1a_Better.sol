...
contract Better is VRFConsumerBaseV2, ConfirmedOwner {
  ...
  event NewBet(uint256 gameId, address indexed sender, uint256 amount);
  ...
  function tossCoin(uint256 bet) public payable {
    ...
    games[gameId] = Game(gameId, bet, msg.value, 0, 0, payable(msg.sender));
  ...
  function processResult(uint256 random) internal {
    ...
    for (uint256 i = lastGameId; i < gameId; i++) {
      ...
      if ((random >= half && games[i].bet == 1) || (random < half && games[i].bet == 0)) {
        ...
        winAmount -= (winAmount * feePercent) / 100;
