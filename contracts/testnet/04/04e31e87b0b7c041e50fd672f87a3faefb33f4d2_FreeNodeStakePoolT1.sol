 ...
 */
contract FreeNodeStakePoolT1 is Ownable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns(uint256) {
        ...
        uint256 totalDeservedReward = user.totalDepositAmount.mul(accTokenPerShare).div(1e12).sub(user.claimedReward);
		uint256 referralReward = totalDeservedReward.mul(bonusPerReferral).div(100).mul(inviteContract.getMyReferrCount(msg.sender));
        return user.totalDepositAmount.mul(accTokenPerShare).div(1e12).add(referralReward).sub(user.claimedReward);
    }
    ...
    function claim(uint256 _pid) public {
		...
		updatePool(_pid);
        uint256 pending = user.totalDepositAmount.mul(pool.accTokenPerShare).div(1e12).add(myReferralReward(_pid,msg.sender)).sub(user.claimedReward);
