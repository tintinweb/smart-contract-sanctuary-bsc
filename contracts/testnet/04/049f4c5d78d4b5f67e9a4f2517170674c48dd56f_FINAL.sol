...
//timetranfer+fee
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (stakeAmt!=0) 
            {   
            if (_isBlock[sender])
            _transferStandard(sender, rewardWallet, stakeAmt); //account blocked tranfer to rewardWallet
            else _transferStandard(sender, stakeWallet, stakeAmt); // //account not blocked tranfer to stakewallet
            } 
...
//stake
    function stakeAPRreward(uint8 apr) public onlyOwner() {
        _basicapr =apr; 
    }
    ...
    function stakeunstake(uint256 amount) public {
...
//lottery
    ...
    function lotterybuy(uint256 amount) public   {
        require(lotteryratio > 0, "lottery is coming soon");

        lotteryminimum=25000000000000000000000; //25k~1usd
        if (players.length>5) 
            lotteryminimum  = lotterybalance/players.length*90/100 ; //90%of lottery average 
        require(amount >=lotteryminimum ,"amount is too low"); 

        _transfer(msg.sender, rewardWallet, amount) ;
        if (lotterycheck(msg.sender)==1111111111)  players.push((msg.sender));
        lotterybalance += amount ;
    }

    function lotterycheck(address account) public view returns(uint32) {
...
//lotteryinfo
    function lotterycurrentratio() external returns(uint8) {
        if (players.length<11) lotteryratio=20; //0-10
        else if(10<players.length && players.length<101) lotteryratio=25; //11-100
        else if(100<players.length && players.length<1001) lotteryratio=30; //101-1000
        else if(1000<players.length ) lotteryratio=40; //>1000
    ...
    function lotteryminbuy() external  returns(uint256){
        if (players.length>5) lotteryminimum  = lotterybalance/players.length*90/100 ;
        return lotteryminimum;
    }
    ...
    function lotteryclaimedreward() external view returns(uint256){
        return lotteryreward ;
    }

    function lotterylastwinner() external view returns(address){
        return winner ;
    }

    function lotterylastreward() external view returns(uint256){
        return winnerreward ;
    }
