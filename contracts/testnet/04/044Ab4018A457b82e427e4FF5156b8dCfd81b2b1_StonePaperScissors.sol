...
contract StonePaperScissors is Ownable {
    ...
    **/
    function createRoom(uint256 size, Choice choice, address token, uint256 tokenAmount) external payable {
        require(size > 0, "SPS: Size should be a positive number");
        ...
        if(token == address(this)){
            require(msg.value == betInfoList[size].price, "SPS: You have to bet money to create a room");
        }else{
    ...
    function betGame(uint256 roomId, Choice choice, uint256 tokenAmount) external payable {
        ...
        if(address(this) == _roomInfo.token){
            require(_roomInfo.price == msg.value, "SPS: There is not enough price");
        }else{
    ...
    function harvest(uint256 roomId) external {
        ...
        if(_roomInfo.token == address(this)){
            ...
            _totalMoney = _roomInfo.size * _roomInfo.tokenAmount;
        }
    ...
    function unstake(uint256 roomId) external {
        ...
        if(_roomInfo.token == address(this)){
            ...
            require(IERC20(_roomInfo.token).balanceOf(address(this)) > _roomInfo.tokenAmount,
                "SPS: There is not enough token to unstake in game");
            require(IERC20(_roomInfo.token).transfer(msg.sender, _roomInfo.tokenAmount),
                "SPS: There is an error to unstake token in game");  
        }

        _playerInfo.checked = true;

        emit Unstake(roomId, msg.sender, _roomInfo.token, _roomInfo.tokenAmount);
