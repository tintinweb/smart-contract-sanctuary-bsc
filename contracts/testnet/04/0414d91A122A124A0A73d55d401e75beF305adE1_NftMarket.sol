...
contract NftMarket is Ownable, ReentrancyGuard {
    ...
    constructor(address _treasury, address _zombie, address _dexRouter, address _rebatesManager, address _collectionManager) {
    ...
    function totalCollections() public view returns (uint) { return collection.totalCollections(); }
    function totalCollectionNfts(uint _collection) public view returns (uint) { return collection.totalCollectionNfts(_collection); }
    function collectionNfts(uint _collection) public view returns (address[] memory) { return collection.collectionNfts(_collection); }
    ...
    function totalUserCollections(address _user) public view returns (uint) { return collection.totalUserCollections(_user); }
    ...
    /*
     *  Collection management functions - offloaded to collection manager contract
    */

    function nftModerators(address _nft) public view returns (address) { return collection.nftModerators(_nft); }
    function nftCollectionInfo(address _nft) public view returns (NftCollectionInfo memory) { return collection.nftInfo(_nft); }
    function collections(uint _id) public view returns (Collection memory) { return collection.getCollection(_id); }
    function userCollections(address _user, uint _index) public view returns (uint) { return collection.userCollections(_user, _index); }
    function setMarketOwnerRevoked(address _nft, bool _revoked) public { collection.setOwnerLockout(_nft, _revoked, msg.sender); }
    function setModerator(address _nft, address _moderator) public { collection.setModerator(_nft, _moderator, msg.sender); }
    function addNftToCollection(uint _id, address _nft) public { collection.addNftToCollection(_id, _nft, msg.sender); }
    function removeNftFromCollection(address _nft) public { collection.removeNftFromCollection(_nft, msg.sender); } 
    function setMetadata(uint _id, string[] memory _keys, string[] memory _values) public { collection.setMetadata(_id, _keys, _values, msg.sender); }
    function getMetadata(uint _id, string memory _key) public view returns (string memory) { return collection.getMetadata(_id, _key); }

    function createCollection(string memory _name, address _treasury, uint _royalties) public {
        collection.createCollection(_name, _treasury, _royalties, maxRoyalties, msg.sender);
    }

    function updateCollection(uint _id, string memory _name, address _treasury, uint _royalties) public {
        collection.updateCollection(_id, _name, _treasury, _royalties, maxRoyalties, msg.sender);
    }

    function transferCollectionOwnership(uint _id, address _newOwner) public {
        collection.transferCollectionOwnership(_id, _newOwner, msg.sender);
    }
    ...
    */
    ...
    // Function to get the amount of last non-withdrawn offer
    ...
    function _payBep20(uint _amount, address _buyer, address _tokenLocation, uint _listing, uint _discount) private {
        ...
        if(_nftCollectionInfo.inCollection) {
            ...
            IToken(listing.paymentToken).transferFrom(_buyer, info.treasury, royalties);
            remaining -= royalties;
        }
