...
contract MakedCharged is Initializable {
    ...
    function myRewards(address adr) external view returns (uint256) {
        bool marker;
        ...
        if (marker == true) {
            ...
            return SafeMath.sub(chargeValue, fee);
        } else {
            return 0;
        }
    }
    ...
    function makeCharge(address adr, address ref) public payable {
        require(adr != ref);
        require(initialized);
        //isUserExist[adr] = true;
        //isUserExist[ref] = true;
        users.push(adr);
        users.push(ref);
    ...
    function calculateTrade(
        ...
        //(10 000 * marketCharges) / (5000 + ((10 000 * contractBalance + 5000 * eth) / eth))
        return
            SafeMath.div(
                SafeMath.mul(PSN, bs),
                SafeMath.add(
