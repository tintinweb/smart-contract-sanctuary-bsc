...
contract Safezone is ISafezone, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  function _cleanSafezoneBuyerWhenUnstake(uint16[] calldata tokenIds, bool unstake) internal {
        ...
        if (unstake) {
            Stake memory stake = safezone[tokenIds[0]];

            delete tokenStaking;
            for (uint j = 0; j < safezoneBuyer[stake.owner].length; j++) {
                bool find = false;    
                for (uint k = 0; k < tokenIds.length; k++) {
                    
                    if (tokenIds[k] == safezoneBuyer[stake.owner][j]) {
                        find = true;
                    }
                }
    
                if (find == false) {
                    tokenStaking.push(safezoneBuyer[stake.owner][j]);
                }
            }

            safezoneBuyer[stake.owner] = tokenStaking;
