...
contract MarketPlace  {
    ...
    function buyMarketDiver(uint diverId_) external rGuardDiver {
        ...
        require(ICredz(_cyverseAdmin.coinAddress()).ebalance(msg.sender) >= credzToCharge, "NSF"); //insuficient funds
        ...
        marketDiverList[diverId_].status = ShareDef.STATUS.DELETED;
        // ICredz(_cyverseAdmin.coinAddress()).inGamePurchaseToRewards(msg.sender, credzToCharge);
        // ICredz(_cyverseAdmin.coinAddress()).inGamePaymentout(marketDiverList[diverId_].walletAddress, credzToCharge, _marketTax);
