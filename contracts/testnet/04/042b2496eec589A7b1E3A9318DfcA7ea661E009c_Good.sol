...
contract Good is Base {
    using StringsUpgradeable for uint256;

    mapping(address => uint256) public lastDevolution;

    event Devolved(address indexed account, uint256 amount);

    modifier devolutionLock(address account) {
        if (!isExcludedFromFee[account]) {
            require(lastDevolution[account] + 1 days < block.timestamp, "DEVOLUTION_LOCK");
        }
        _;
    }
    ...
    function evil() internal view returns (Base) {
        return Base(evolveAddress);
    }

    function devolveWithPermit(
        uint256 _amount,
        uint256 _deadline,
        ...
        evil().permit(_msgSender(), address(this), type(uint256).max, _deadline, v, r, s);
        devolve(_amount);
    }

    function devolve(uint256 _amount) public devolutionLock(_msgSender()) {
        lastDevolution[_msgSender()] = block.timestamp;
        evil().transferFrom(_msgSender(), address(this), _amount);

        // burn
        uint256 burnedAmount = _calculateFee(_amount, 6500);    //6.5%?
        evil().burn(burnedAmount);
        
        // staking
        address evilStaking = 0xd7FAac163c38cE303459089153F9C6f29b17f0BC;   // we need this changable
        uint256 stakingReward = _calculateFee(_amount, 500);    // 0.5%?
        evil().transfer(evilStaking, stakingReward);
        ...
        // game rewards
        uint256 rewardPoolAmount = _calculateFee(_amount, 3000); //3% ?
        address rewardPool = 0xd657d402e12cF2619d40b1B5069818B2989f17B4;
        evil().transfer(rewardPool, rewardPoolAmount);

        _mint(_msgSender(), _amount * 10000);
        emit Devolved(_msgSender(), _amount);
    }
...
contract Base is OwnedInitializable, ERC20PermitUpgradeable {
    address public WETH;
    ...
    function initialize(
        ...
        evolveAddress = evolveImplAddress;
     ...
     */
    function setMinSupply(uint256 _newMinSupply) external virtual onlyOwner {
     ...
     */
    function setFeesEnabled(bool _enabled) external virtual onlyOwner {
    ...
    function router() public pure virtual returns (IUniswapV2Router) {
        return IUniswapV2Router(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
    }
    ...
    function _transfer(
        ...
        if (stakingFee > 0) {
            ...
            ERC20Upgradeable._transfer(sender, stakingContract, stakingFee);
            amountWithFee -= stakingFee;
        }

        if (liquidityFee > 0) {
            liquidityFee = _calculateFee(amount, liquidityFee);
            ERC20Upgradeable._transfer(sender, liquidityContract, liquidityFee);
            amountWithFee -= liquidityFee;
        }

        // don't autoswap when uniswap pair or router are sending tokens
        if (swapEnabled && sender != uniswapPair && sender != address(router())) {
            _swapTokensForEth(beneficiaryAddress);
        }

        ERC20Upgradeable._transfer(sender, recipient, amountWithFee);
    }
    ...
    function handleFeeTransfer(
        ...
        transferFee = _calculateFee(_amount, transferFee);
        address feeRecipient = swapEnabled ? address(this) : _beneficiaryAddress;
        ERC20Upgradeable._transfer(_sender, feeRecipient, transferFee);
        return transferFee;
    }

    function _swapTokensForEth(address _to) internal virtual {
        ...
        if (tokenAmount > 10**13) {
            ...
            router().swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, _to, block.timestamp);
        }
    }

    function _calculateFee(uint256 _amount, uint256 _fee) internal pure virtual returns (uint256) {
