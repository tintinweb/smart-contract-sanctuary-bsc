...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _transUsdtAmount = 20 * 10 ** 18;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 firstAmount = inviteAmount * 67 / 100;
    ...
    function _bindInvitor(address account, address invitor) private {
        address invitor_2 = _inviter[invitor];
        require(invitor_2 != address(0) || invitor_2 == fundAddress, "Parent address is not bound");
    ...
    function donate() external {
        ...
        uint256 balance = IERC20(_usdt).balanceOf(account);
        ...
        if (address(0) != invitor) {
            ...
            if(secondAmount > 0){
                if (address(0) != invitor_2) {
                    IERC20(usdt).transfer(invitor_2, secondAmount);
                }else{
                    firstAmount += secondAmount;
                }
            }
            IERC20(usdt).transfer(invitor, firstAmount);
        }
        IERC20(usdt).transferFrom(account, address(this), transUsdtAmount);
    ...
    function claimAirdrop() external {
        ...
        require(_claimed[account] < _transUsdtAddr[account], "claimed");
...
contract Token is AbsToken {
    ...
    //USDT
        ...
        1000000,
    //Fund
        address(0x46256e12dc725A8542395CBae7a5E80E5532362A),
    //Receive
        address(0xb7ef288F8B422AeD51FF21415Ef1D283B39FE8D8)
    ){
