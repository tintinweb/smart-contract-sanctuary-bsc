...
/**
 * @title Panrp1Swap Farms Auctions.
...
contract FarmAuction is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable rp1Token;
    ...
    /**
     * @notice Constructor
     * @param _rp1Token: address of the $rp1 token
     ...
     * @param _amount: amount of the bid in $rp1 token
    ...
    function bid(uint256 _amount) external nonReentrant {
        ...
        rp1Token.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function claimAuction(uint256 _auctionId) external nonReentrant {
        ...
        rp1Token.safeTransfer(address(msg.sender), claimableAmount);
    ...
    /**
     * @notice Close an auction round, and store $rp1 leaderboard threshold
     * @param _bidLimit: minimal $rp1 committed to be a winner
     * @dev Callable by operator
     */
    ...
    /**
     * @notice Claim $rp1 for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(rp1Token), "Recover: Cannot be rp1 token");
