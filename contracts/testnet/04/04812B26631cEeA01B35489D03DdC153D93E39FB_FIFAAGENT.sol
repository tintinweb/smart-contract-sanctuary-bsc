//SPDX-License-Identifier: Unlicense
    ...
    contract FIFAAGENT is Ownable {
        ...
        event PlaceGuess(address player, uint256 matchIndex,uint256 guessScore, uint256 guessAmount, address agent, uint256 level, uint256 room);
        event Claim(address player, uint256 matchIndex, address agent);
        ...
        mapping(uint256=>mapping(uint256=>Score)) public scoreInfo;
        ...
        */
        function placeGuess(uint256 _matchUpIndex,uint256 _guessScore, uint256 _guessAmount, address _agent, uint256 _level) external {
            ...
            require((agentInfo[_agent][_level][_matchUpIndex].level == 1 && _guessAmount>=(1000e18)) ||
                ...
                (agentInfo[_agent][_level][_matchUpIndex].level == 5 && _guessAmount>=(10000000e18)), "invalid level, guessAmount, or agent");
            require(agentInfo[_agent][_level][_matchUpIndex].hasCreated == true, "room was not created by admin");
            uint256 _room = agentInfo[_agent][_level][_matchUpIndex].roomNum;
            Guess storage guessStorage = guessInfo[msg.sender][_room][_guessScore];
            ...
            scoreInfo[_room][_guessScore].scoreTotal +=_guessAmount;
            userInfo[msg.sender][_room].personalMatchTotal += _guessAmount;
            agentInfo[_agent][_level][_matchUpIndex].matchTotalPool += _guessAmount;
            scoreInfo[_room][_guessScore].hasGuessed = true;
            guessStorage.hasUserGuessed = true;
            emit PlaceGuess(msg.sender, _matchUpIndex, _guessScore, _guessAmount, _agent, _level, _room);
        }
        ...
        */
        ...
        function claim(uint256 _matchIndex, address _agent, uint256 _level) external{
            ...
            require(!userInfo[msg.sender][_room].hasUserClaimed, "Amount has been claimed");
            ...
            if (matchStatus == MatchStatus.DELAYED || matchStatus == MatchStatus.CANCELLED){
                IERC20(tokenAddress).safeTransfer(msg.sender, (userInfo[msg.sender][_room].personalMatchTotal * refundInterest) / 1000);
                devFee = ((userInfo[msg.sender][_room].personalMatchTotal * devRefundInterest ) / 1000);
            }

            else if ((!scoreInfo[_room][resultScore].hasGuessed && matchStatus == MatchStatus.COMPLETED) || matchStatus == MatchStatus.DELAYED || matchStatus == MatchStatus.CANCELLED){
                IERC20(tokenAddress).safeTransfer(msg.sender, (userInfo[msg.sender][_room].personalMatchTotal * rewardInterest) / 1000);
                uint256 personalMatchTotal = userInfo[msg.sender][_room].personalMatchTotal;
            ...
            else if (guessInfo[msg.sender][_room][resultScore].hasUserGuessed && matchStatus == MatchStatus.COMPLETED) {
                Guess storage guessStorage = guessInfo[msg.sender][_room][resultScore];
                uint256 scoreTotal = scoreInfo[_room][resultScore].scoreTotal;
                uint256 totalPool = agentInfo[_agent][_level][_matchIndex].matchTotalPool;
            ...
            userInfo[msg.sender][_room].hasUserClaimed = true;

            emit Claim(msg.sender, _matchIndex, _agent);
        }

        function viewGuessStatus(address _address, uint256 _matchIndex, address _agent, uint256 _level) public view returns (string memory) {
            uint256 resultScore = resultList[_matchIndex];
            uint256 _room = agentInfo[_agent][_level][_matchIndex].roomNum;
            if (matchUps[_matchIndex].matchStatus != MatchStatus.COMPLETED){
                return "PENDING";
            }
            if (!scoreInfo[_room][resultScore].hasGuessed){
                return "REFUND";
            }
            if (guessInfo[_address][_room][resultScore].hasUserGuessed) {
                return "WIN";
            }
            return "LOSE";
        }
        ...
        function getUserInfo(uint256 _matchIndex, address _agent, uint256 _level) public view returns (User memory){
            uint256 _room = agentInfo[_agent][_level][_matchIndex].roomNum;
            return userInfo[msg.sender][_room];
        }

        function getGuess(uint256 _matchUpIndex, uint256 result, address _agent, uint256 _level) public view returns (Guess memory){
            uint256 _room = agentInfo[_agent][_level][_matchUpIndex].roomNum;
            return guessInfo[msg.sender][_room][result];
        }
