...
contract EBT_Token_Presale is Ownable {
    ...
    uint256 PhaseOne = 8500000000000; //(100M to 75M) 1 EBT = 0.0000085 BNB
    uint256 PhaseTwo = 10200000000000; //(75M to 25M) 1 EBT = 0.0000102 BNB
    uint256 PhaseThree = 11900000000000; //(<25M) 1 EBT = 0.0000119 BNB
    ...
    function joinPresale(address _ref) public payable {
        ...
        if (contractBalance > PhaseOneLimit) {
            require(msg.value >= PhaseOne,"-> 1 EBT = 0.0000085 BNB!!");
            PhaseChecker = 1;
        }
        else if (contractBalance > PhaseTwoLimit) {
            require(msg.value >= PhaseTwo,"-> 1 EBT = 0.0000102 BNB!!");
            PhaseChecker = 2;
        }
        else {
            require(msg.value > PhaseThree,"-> 1 EBT = 0.0000119 BNB!!");
            PhaseChecker = 3;
        }
