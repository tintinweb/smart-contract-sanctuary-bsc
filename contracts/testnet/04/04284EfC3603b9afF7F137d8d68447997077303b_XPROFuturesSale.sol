...
contract XPROFuturesSale is ReentrancyGuard, Ownable {
     ...
     */
    function createFuturesSale(uint256 _startTime, uint256 _endTime, uint256 _price, uint256 _tokensToSellAmount, uint256 _baseDecimals) external onlyOwner {
        require(
        ...
        emit FuturesSaleCreated(futuressaleId, _tokensToSellAmount, _startTime, _endTime);
    }
     ...
     */
    function changeSaleTimes(uint256 _id, uint256 _startTime, uint256 _endTime) external checkFuturesSaleId(_id) onlyOwner {
        require(_startTime > 0 || _endTime > 0, "Invalid parameters");
        if (_startTime > 0) {
            require(
                block.timestamp < futuressale[_id].startTime,
                "Sale already started"
            );
            require(block.timestamp < _startTime, "Sale time in past");
            uint256 prevValue = futuressale[_id].startTime;
            futuressale[_id].startTime = _startTime;
        ...
        if (_endTime > 0) {
            require(
                block.timestamp < futuressale[_id].endTime,
                "Sale already ended"
            );
            require(_endTime > futuressale[_id].startTime, "Invalid endTime");
            uint256 prevValue = futuressale[_id].endTime;
            futuressale[_id].endTime = _endTime;
            emit FuturesSaleUpdated(
                bytes32("END"),
                prevValue,
                _endTime,
     ...
     */
    function changePrice(uint256 _id, uint256 _newPrice) external checkFuturesSaleId(_id) onlyOwner
    {
        require(_newPrice > 0, "Zero price");
        uint256 prevValue = futuressale[_id].price;
        futuressale[_id].price = _newPrice;
        emit FuturesSaleUpdated(
            bytes32("PRICE"),
            prevValue,
            _newPrice,
     ...
     */
    function buyToken(uint256 _id, uint256 amount, uint256 period) external payable checkFuturesSaleId(_id) checkSaleState(_id, amount) nonReentrant returns (bool)
    {
