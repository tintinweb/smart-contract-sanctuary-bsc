...
function stake_user(uint256 _amount)
   ...
   up2date(msg.sender);
        ...
        iToken.approve(msg.sender, _amount);
        iToken.approve(address(this), _amount);


require(iToken.allowance(msg.sender, address(this)) >= 10000, "insufficient allowance amount");
...
//     }
    ...
    // function get_Rewards_user_old(address _account) public returns (uint256) {
  ...
  rewards[_account] = rewards[_account] + (( time_s * ( _balances[_account] * (rewardRateX / 100))) / 31556926);
