...
contract elonMegaCoin is OwnableUpgradeable {
    ...
    IERC20 public BUSD;
    IERC20 public ELONMC;
    using SafeMath for uint256;
    address token_BUSD;
    address token_ELONMC;

    bool public started;
    uint256 public totalInvested_BUSD;
    uint256 public maxUserDeposits;
    uint256 public elonmcTokenPrice;
    uint256 spots;
    ...
    */
    ...
    function initialize() external initializer {
        ...
        token_BUSD = address(0xDDB43ebc3F34947C104A747a6150F4BbAA78a5eB); // test BUSD
    ...
    function claim() external {
        ...
        require(elonmcTokenPrice > 0, "token price not set");
        require(claimed[msg.sender] == 0, "nothing to claim");

        uint256 myDeposits = deposits[address(msg.sender)];
        uint256 myTokenAmount = elonmcTokenPrice.mul(myDeposits).div(1 ether);

        claimed[msg.sender] = myTokenAmount;

        ELONMC.transfer(msg.sender, myTokenAmount);
    }

    function getClaimActive() public view returns(bool) {
        return elonmcTokenPrice > 0;
    }
