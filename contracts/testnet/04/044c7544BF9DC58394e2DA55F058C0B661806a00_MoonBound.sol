...
contract MoonBound is Context, IERC20, Ownable, Pausable {
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return
            _tTotal.sub(balanceOf(deadAddress)).sub(
    ...
    function getFeePercent(uint256 taxFee) public pure returns (uint256) {
        require(taxFee > 0, "ERC20: tax fee must be greater than 0");
    ...
    function excludeFromReward(address account) public onlyOwner {
        require(
            !_isExcluded[account],
            "MoonBound: Account is already excluded"
        );
        require(
            account != address(this),
            "MoonBound: Cannot exclude contract from reward"
        );
    ...
    function includeInReward(address account) external onlyOwner {
        ...
        require(
            account != address(this) && account != uniswapV2Pair,
            "MoonBound: Cannot include contract from reward"
        );
    ...
    function setSwapThreshold(uint256 threshold) external onlyOwner {
        ...
        emit MinTokensBeforeSwapUpdated(threshold);
    }
    ...
    function setMarketingTaxModifier(uint256 marketingTaxModifier)
        external
        onlyOwner
    {
    ...
    function setLiquidityTaxModifier(uint256 liquidityTaxModifier)
        external
        onlyOwner
    {
    ...
    /**
     * @dev Claim taxes made from transactions and stored on contract for redistribution,
     * continues marketing / airdrops, and possible buy backs. All txns are recorded on chain.
     *
     */
    function claimTaxes(
        address _devWallet,
        ...
        require(
            ...
            "MoonBound: Cannot claim taxes if balance is zero"
        );
        require(
            tokenAmount > 0,
            "MoonBound: Cannot claim taxes if token amount is zero"
        );
        require(
            bnbAmount > 0,
            "MoonBound: Cannot claim taxes if bnb amount is zero"
        );
        require(
            _devWallet != address(this),
            "MoonBound: Cannot claim taxes if dev wallet is same as contract"
        );

        // Transfer Tokens
        transferFrom(address(this), _devWallet, tokenAmount);
    ...
    function swapAndLiquify() private lockTheSwap {
        uint256 totalFee = getFeePercent(_taxFee);
        uint256 dynamicLiquidityFee = isOverLiquified(
            targetLiquidity,
            targetLiquidityDenominator
        )
            ? 0
            : getFeePercent(_liquidityFee);
        uint256 amountToLiquify = numTokensSellToAddToLiquidity
            .mul(dynamicLiquidityFee)
            .div(totalFee)
            .div(2);
        uint256 amountToSwap = numTokensSellToAddToLiquidity.sub(
            amountToLiquify
        );
        ...
        uint256 amountBNBLiquidity = newBalance.mul(dynamicLiquidityFee).div(
            totalFee
        );
        uint256 amountBNBReflection = newBalance
            .mul(getFeePercent(_rewardFee))
            .div(totalFee);
        uint256 amountBNBMarketing = newBalance
            .mul(getFeePercent(_marketingFee))
            .div(totalFee);
    ...
    function addLiquidityManually(uint256 tokenAmount)
        public
        payable
        onlyOwner
    {
    ...
    function _transferStandard(
        ...
        (uint256 tTransferAmount, uint256 tFee) = _getValues(
            tAmount,
            isSelling,
            takeFee
        );
        ...
        if (!_isExcluded[recipient]) {
            try
                rewardDistributor.setStock(recipient, _rOwned[recipient])
            {} catch {}
        }
