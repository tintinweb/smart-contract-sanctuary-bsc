...
contract NFTFractional is Upgradable, ERC20Upgradeable {
    ...
    */
    function setAdmin(address _address, bool _value) external onlyController {
        require(_address != address(0), "admin is not the zero address");
    ...
    */
    ...
    function setBaseURI(string memory _baseURI) external onlyController notEmpty(_baseURI) {
        require(nftToken != address(0), "Address is not the zero address");
    ...
    function minNFT(
        ...
        require(bytes(NFTToken(nftToken).baseURI()).length > 0, "baseURI not empty");
    ...
    function fractionalizeNFT(
        ...
        string memory _symbol) external onlyAdmins notEmpty(_name) notEmpty(_symbol) {
            require(_curator != address(0), "Address is not the zero address");
            require(_totalSupply > 0, "Total supply not greater than zero");
            __ERC20_init(_name, _symbol);
            uint256 _supply = _totalSupply * 10 ** decimals();
            _mint(_curator, _supply);
            TokenVaultInfo memory tokenVaultInfo = TokenVaultInfo(_id,_supply, _supply, _curator, _symbol, _name);
            fractionalNFTs[_tokenId] = tokenVaultInfo;
            emit FractionalizeNFT(_token, _curator, _supply, _symbol, _name);
    }

    function createFNFTPool() external onlyAdmins {
...
contract Upgradable {
    ...
    mapping(uint256 => TokenVaultInfo) public fractionalNFTs;
    ...
    modifier notEmpty(string memory _value) {
        require(bytes(_value).length > 0, "Not Empty");
        _;
    }

    modifier onlyAdmins() {
        require(msg.sender == controller || adminList[msg.sender], "Only controller and admins");
        _;
    }
