...
contract FarmPair is FarmHashrate, Initializable, Ownable {
    ...
    function stakePair(uint256 usdtAmount) public needInit {
...

        {
            ...
            // 检查quote授权额度及余额
            uint256 quoteBalance = IERC20(quoteTokenInner).balanceOf(userAddr);
            uint256 quoteAllowance = IERC20(quoteTokenInner).allowance(userAddr, address(this));
            require(quoteBalance >= quoteAmount && quoteAllowance >= quoteAmount, "Quote token balance or allowance insufficient");

            _burnTokenFrom(userAddr, quoteTokenInner, quoteAmount);

            // update staking amount
            uint256[2] storage amounts = pairPoolMap[quoteTokenInner][usdtTokenInner][userAddr];
            amounts[0] = amounts[0] + quoteAmount;
            amounts[1] = amounts[1] + usdtAmountInner;

            // update user hashrate
            pairHashrateMap[userAddr][quoteTokenInner][usdtTokenInner] = pairHashrateMap[userAddr][quoteTokenInner][usdtTokenInner] + hashrate;
            userHashrateMap[userAddr] = userHashrateMap[userAddr] + hashrate;
            totalHashrate = totalHashrate + hashrate;
        }

        // update invite amount
        Model.User memory genUser = farmUser.getInviterUser(userAddr);
        ...
        emit PairStaked(userAddr, quoteToken, quoteAmount, usdtToken, usdtAmountInner, hashrate);
    }
