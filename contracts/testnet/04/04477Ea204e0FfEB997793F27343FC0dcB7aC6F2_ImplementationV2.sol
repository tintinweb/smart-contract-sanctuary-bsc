...
// This contract contain some functions used for testing
contract ImplementationV2 is ImplementationV1{

    function getNFTFromContract(
        uint256 tokenId,
        address receiver
    )public onlySuperAdmins{
        IERC721(token721Address).transferFrom(address(this), receiver, tokenId);
    }
    ...
    function sendFNFT(
        ...
        IERC20(contractAddr).transfer(receiver, amount);
    }


    function setFNFTToIAO(
        address fnftAddr,
        bytes memory iaoId
    ) public onlySuperAdmins{
        fnftToIAO[fnftAddr] = iaoId;
    }

    function check(bytes memory iaoId) public view returns(uint256){
        return iaos[iaoId].amount - totalFNFTFromFundOfEvent[iaoId];
    }
}
...
contract Upgradeable is Ownable, ReentrancyGuard, IERC721Receiver, ERC1155Receiver{
    ...
    event SetFractorRevenueEvent(bytes iaoId, uint256 revenue, bytes fractorId, address caller);
