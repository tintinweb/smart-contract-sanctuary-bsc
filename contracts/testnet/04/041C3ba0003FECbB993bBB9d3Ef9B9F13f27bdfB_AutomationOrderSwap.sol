...
contract AutomationOrderSwap is AutomationCompatible, Ownable {
    ...
    function _validateOrder(
        ...
        if (_path.length == 2) {
            uint256 amountOut = _getAmountOut(_path[0], _path[1], _amountIn);
            return amountOut >= _minOut;
        }
        if (_path.length == 3) {
            uint256 midOut = _getAmountOut(_path[0], _path[1], _amountIn);
            uint256 amountOut = _getAmountOut(_path[1], _path[2], midOut);
            return amountOut >= _minOut;
        }
        ...
        returns (bool, bytes memory)
    {
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            if (_validateOrder(path, amountIn, minOut)) {
