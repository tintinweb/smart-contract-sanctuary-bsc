...
/// @dev Except for `payoutValue()` & `bet()`, all other methods are expected to be called by the Factory.
contract Round {
    ...
    constructor (address _creator, string memory pairName, uint256 _round_id, uint startTime, uint timeToBet, uint roundDuration, uint256 _minBetValue, uint256 _referralFees, uint256 _creatorFees, uint256 _roundFees) {
        ...
        vault         = BankVault(0x6aa09c6a0145190838AeAbD0dda9CBd4D049700c);
    }
    ...
    /// @notice Cancel the round, trigerring a refund tot he Vault
    /// @dev Only callable by the Factory
    function cancel() public {
        require(msg.sender==owner, "Forbidden");
        bool condition = (started==false && block.timestamp > round_start+30) || 
                         (ended==false && block.timestamp > round_end+30) || 
        ...
        for (i=0;i<betCount;i++) {
            refundAddrs[i] = bets[i].wallet;
            refundAmounts[i] = bets[i].sent;
        }
        // Cancel the escrow, get back the referral fees
        vault.cancelEscrow(round_id);

        // Refund the users
        vault.creditMany{value: address(this).balance}(refundAddrs, refundAmounts, "Refunds", round_id_str);
    }
...
contract RoundFactory {
    ...
    function cancel(uint256 roundId) public {
        ...
        emit roundCanceled(roundId, msg.sender);
    }
