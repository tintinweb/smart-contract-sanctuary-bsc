...
contract FrostFlakesV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        previousContractOwnerAddress = payable(
            0x067277B8fFE9623799b7F2b2D83593E191684993
        );
        TEAM_ADDRESS = 0x6466F8b39F4702d4676a2125Bf381d07753AE2f4;
    ...
    address payable internal previousContractOwnerAddress;
        ...
        returns (uint256)
    {
        ...
        uint256 rewardFactor = Math.pow(102, daysStarted); // 102^2 = 10404
        uint256 maxTvlAfterRewards = Math.div(
            Math.mul(rewardFactor, lockedFrostFlakes[adr]), // 10404 * 30000000 = 312120000000 / (10^(2*2(daysStarted))) = 31212000
        ...
        uint256 maxRewards = Math.mul(
            ...
            100000
        ); // 31212000 - 30000000 = 1212000 * 100000 = 121200000000
        uint256 rewardsPerSecond = Math.div( // 121200000000 / (86400 * 2(daysStarted)) = 701388
            maxRewards,
            Math.min(
        ...
        uint256 currentRewards = Math.mul(rewardsPerSecond, secondsPassed); // 7013 * (2 * 86400) = 121199846400
        currentRewards = Math.div(currentRewards, 100000); // 121199846400 / 1000 = 1211998
    ...
    function massAirdrop() public payable {
        ...
        for (uint256 i = 0; i < referrals[msg.sender].length; i++) {
            handleAirdrop(referrals[msg.sender][i], sharedAmount);
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 minTvlEligibleForFee = 500000000000000000; // 0.5 BNB
        ...
        uint256 fee = Math.div(feeLevel, 5); // 0.2% of TVL
        return fee;
    }
    ...
    function requestMigration() public payable {
        if (migrationRequested[msg.sender] == false) {
            previousContractOwnerAddress.transfer(3000000000000000); // 0.003 BNB
        }
    ...
    function canReRequestMigration() public view returns (bool) {
        return
            migrationRequested[msg.sender] == true &&
            block.timestamp > Math.add(lastMigrationRequest[msg.sender], 3599);
    }
    ...
    function migrateUserInfo(
        ...
        uint256 frostFlakesSincelastFreeze,
        ...
        lockedFrostFlakes[user] = Math.add(
            myLockedFrostFlakes,
            frostFlakesSincelastFreeze
        );
        ...
        returns (DetailedReferral[] memory myReferrals)
    {
        ...
        for (uint256 i = 0; i < referrals[adr].length; i++) {
            ...
            result[i] = DetailedReferral(
                ...
                userName[refAddress],
                userInfoMigrated[refAddress] && userDataMigrated[refAddress]
            );
        }
        ...
        returns (uint256)
    {
        ...
        uint256 secondsPassed = Math.min(
            maxFrostFlakes,
            Math.sub(block.timestamp, lastFreeze[adr])
        );
    ...
    function calcFrostFlakesReward(
        ...
        uint256 rewardsPerDay = calcPercentAmount(
            ...
            dailyReward
        ); //  Math.div(Math.mul(Math.mul(29675377, 1000),2),100); = 593507540
        uint256 rewardsPerSecond = Math.div(rewardsPerDay, SECONDS_PER_DAY); // 593507540 / 86400 = 6869
        uint256 frostFlakes = Math.mul(rewardsPerSecond, secondsPassed); // 6869 * 108000 = 741852000
        frostFlakes = Math.div(frostFlakes, 100000); // 741852000 / 1000 = 741852
        return frostFlakes;
    }
}
