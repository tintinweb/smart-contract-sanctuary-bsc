...
contract Ownable is Context {
    ...
    function unlock() public virtual {
        ...
        require(block.timestamp > _lockTime , "Time not reached");
...
contract MISAD is Context, IERC20, Ownable {
    ...
    uint256 public numWETHSellToBuyback = 10**16; //0.01

    IUniswapV2Router02 public uniswapV2Router;

    address public uniswapV2Pair;
    address constant WETH = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public _treasureAddress = 0x31dBcFe9a338e31edc96e6Fb9C8f0319AF8ff98f;
    ...
    function setNumWETHSellToBuyback(uint256 _numWETH) external onlyOwner() {
        require(_numWETH >= 0, "Negative WETH value");
        numWETHSellToBuyback = _numWETH;
    }

    // To avoid any ETH getting stuck in the contract, if liquidity is 
    // disabled this is the only way.
    function withdrawETH() external onlyOwner {
        require(address(this).balance > 0, "There is no WETH balance");
        payable(msg.sender).transfer(address(this).balance);
    }

    // To avoid any ERC20 token getting stuck in the contract
    function withdrawERC20(IERC20 token) external onlyOwner {
        require(token.balanceOf(address(this)) > 0, "There is no ERC20 token balance");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (initialBalance >= numWETHSellToBuyback)
        {
