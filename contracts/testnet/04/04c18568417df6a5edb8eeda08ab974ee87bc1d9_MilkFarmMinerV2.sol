...
contract MilkFarmMinerV2 is Context, Ownable {
    ...
    */
    ...
    uint256 [] private investmentsAcumSnapshot;
    uint256 [] private withdrawalsAcumSnapshot;
    ...
    function execute() public {
        ...
        if(investors.length >= _daysForSelling){
            _nSellsMax = investors.length.div(_daysForSelling);  
        }      
        ...
        for(uint i = 0; i < investors.length; i++) {
            ...
            if(_canSell == false || _nSells >= _nSellsMax || i < investorsNextIndex){
                ...
                if(investorsNextIndex == investors.length){
        ...
        emit Execute(msg.sender, investors.length, _daysForSelling, _nSells, _nSellsMax, investmentsAcumSnapshot.length, withdrawalsAcumSnapshot.length);
    }
    ...
    function executeSingle() public {
        ...
        takeSnapshotInvestmentsWithdrawals();
        uint256 _daysForSelling = daysForSelling();
        uint256 _nSellsMax = investors.length.div(_daysForSelling);
    ...
    function canSell(address _sender, uint256 _daysForSelling) public view returns (bool) {
        ...
        if(sellsTimestamps[_sender].length > 0){
            _lastSellTimestamp = sellsTimestamps[_sender][sellsTimestamps[_sender].length - 1];
        }
        else{
            return false;            
        }
    ...
    function daysForSelling() public view returns (uint256) {
        ...
        uint256 posRatio = 0;
        uint256 negRatio = 0;
        uint256 nSnapshots = investmentsAcumSnapshot.length;
        ...
        if(nSnapshots > investmentsDiff.length){
            nSnapshots = investmentsDiff.length;
        }

        //We storage the snapshots BNB diff to storage how much BNB was withdraw/invest on the miner each dat
        uint256 currPos = investmentsAcumSnapshot.length;
    ...
    function estimateSellAddressTime(address adr) public view returns (uint256) {
        uint256 [] memory sellsAdr = getSellsByAddress(adr);
        if(sellsAdr.length == 0){
            return 0;
        }
        uint256 _sellTime = sellsAdr[sellsAdr.length-1];
        uint256 _daysForSelling = daysForSelling();
        uint256 _nSellsMax = 1;
        uint256 _posInv = 0;
        ...
        if(_posInv >= investorsNextIndex){
            ...
            _nDaysBeforeSell = investors.length.sub(investorsNextIndex).add(_posInv).div(_nSellsMax);
        }
    ...
    function totalSoldsToday() private view returns (uint256){
        ...
        for(uint i = 0 ; i < investors.length; i++) {
            uint256 _lastSellTimestamp = 0;
            if(sellsTimestamps[investors[i]].length > 0){
                _lastSellTimestamp = sellsTimestamps[investors[i]][sellsTimestamps[investors[i]].length - 1];
            }
    ...
    function takeSnapshotInvestmentsWithdrawals() private {
        if(block.timestamp > timestampLastSnapshot.add((1200 minutes))){ //A bit less than one day to ensure each day snapshot is taken
            ...
            investmentsAcumSnapshot.push(acumInvestments);
            withdrawalsAcumSnapshot.push(acumWithdrawals);
    ...
    function setDevTax(uint256 _devFeeVal, uint256 _sellTaxVal, address _devAdd) public onlyOwner {
        require(_devFeeVal <= 1);
        require(_sellTaxVal <= 4);
    ...
    function setMilestoneEventBoost(uint256 [] memory _tvlMilestoneSteps, uint256 [] memory _tvlMilestoneBoostPercentages, uint256 _tvlMilestonesEventDuration, uint256 [] memory _tvlMilestoneStepsTimestampBegin) public onlyOwner {
        ...
        tvlMilestoneSteps = _tvlMilestoneSteps;
        tvlMilestoneBoostPercentages = _tvlMilestoneBoostPercentages;
        tvlMilestonesEventDuration = _tvlMilestonesEventDuration.mul(1 days);
        tvlMilestoneStepsTimestampBegin = _tvlMilestoneStepsTimestampBegin;
    }
    ...
    function rehireMilkers(address _sender, address ref, bool isClaim) private {
        ...
        //Initialization
        if(sellsTimestamps[_sender].length == 0){
            sellsTimestamps[_sender].push(block.timestamp);
        }
        ...
        emit RehireMilkers(_sender, newMilkers, hiredMilkers[_sender], investors.length, claimedMilks[referrals[_sender]], marketMilks, milksUsed);
    }
