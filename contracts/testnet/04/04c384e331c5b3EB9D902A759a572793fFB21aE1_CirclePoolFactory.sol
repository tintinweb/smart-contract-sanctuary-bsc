...
contract CirclePoolFactory is OwnableUpgradeable {
    struct Fee {
        uint256 tokenFee;
        uint256 nativeFee;
    }
    ...
    function createSale(
        ...
        checkfees();
        uint8 decimals = IERC20Info(presale.token).decimals();
        uint256 totalToken = _feesCount(
            decimals,
            presale.rate,
            presale.liquidityListingRate,
            presale.hardCap,
            presale.liquidityPercent,
            fee.tokenFee,
            presale.listingType
        );
        ...
        _transferFromEnsureExactAmount(presale.token, pair, totalToken);
    ...
    function createPrivateSale(
        ...
        checkPrivateSalefees();
        IPoolManager(poolManager).addPoolFactory(pair);
        IPoolManager(poolManager).registerPrivatePool(
            pair,
            presale.tokenName,
            presale.governance,
            version
        ...
        emit PrivatePoolCreated(pair, presale);
    }
    ...
    function initalizeFairClone(
        ...
        fairFees();
        address ethAddress = IUniswapV2Router01(presale.router).WETH();
        if(presale.paymentToken != address(0)) {
            ethAddress = presale.paymentToken;
        }
        address factoryAddress = IUniswapV2Router01(presale.router).factory();
        address getPair = IUniswapV2Factory(factoryAddress).getPair(
            ethAddress,
            presale.token
        );
    ...
    function createFairSale(
        ...
        address governance = presale.governance;
        _safeTransferFromEnsureExactAmount(
            token,
            address(msg.sender),
            address(this),
        ...
        emit FairPoolCreated(pair, presale);
    }
    ...
    function fairFees() internal {
        require(
            msg.value >= fairmasterPrice,
            "Payable Amount is less than required !!"
        );
    }

    function checkPrivateSalefees() internal {
        require(
            msg.value >= privatemasterPrice,
            "Payable Amount is less than required !!"
        );
    }
    ...
    function _feesCount(
        ...
        uint256 totalToken = (((_rate * _hardcap) / 10**decimals)).add(
            _listingType == 0 ? (((_hardcap * _Lrate) / 10**decimals) * _liquidityPercent) / 100 : 0
        );
        uint256 totalFees = (((((_rate * _hardcap) / 10**decimals)) * _fees) / 100);
