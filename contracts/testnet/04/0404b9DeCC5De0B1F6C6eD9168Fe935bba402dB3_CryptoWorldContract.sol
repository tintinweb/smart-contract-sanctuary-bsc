...
contract CryptoWorldContract is Ownable,  ReentrancyGuard {
    ...
    constructor() ReentrancyGuard() {
        bonusLevel[0] = 7; //1º nível 7%
        bonusLevel[1] = 4; //2º nível 4%
        bonusLevel[2] = 3; //3º nível 3%
        bonusLevel[3] = 3; //4º nível 3%
        bonusLevel[4] = 3; //5º nível 3%
        bonusLevel[5] = 2; //6º nível 2%
        bonusLevel[6] = 2; //7º nível 2%
        bonusLevel[7] = 2; //8º nível 2%
        bonusLevel[8] = 1; //9º nível 1%
        bonusLevel[9] = 1; //10º nível 1%
        bonusLevel[10] = 1; //11º nível 1%
        bonusLevel[11] = 1; //12º nível 1%

        //Setar a primeira carteira da rede no envio do contrato;
        _addStartSystem();
        minValue = 0.05 ether;
        levelMaximo = 12;
    ...
    function adicionarSaldoJogos() nonReentrant public payable
    {
        ...
        //console.log("address(this): ", address(this));
        //console.log("donationValue: ", msg.value);
        // _sendMoney(address(this), msg.value);
        //console.log("saldo antes: ", accMapping[_msgSender()].valueReservedForGames);
        accMapping[_msgSender()].valueReservedForGames = msg.value;
        //console.log("saldo depois: ", accMapping[_msgSender()].valueReservedForGames);
    }
    ...
    function _withdraw(uint256 withdrawValue) private 
    {
        ...
        //console.log("totalSacado: ", totalSacado);
        //console.log("valorTotalRecebivel: ", valorTotalRecebivel);

        //tem o valor na conta
    ...
    function _valorLiquidoAReceber() private returns(uint256)
    {
        ...
        //console.log("bonusFidelidade: ", bonusFidelidade);

        // //limitador = % bonus fidelidade + 200% do valor depositado - 2% jogos
        uint256 valorTotalRecebivelReturn = _valorTotalRecebivelSemTaxas(valorDepositado, bonusFidelidade);//1040

        //console.log("valorTotalRecebivelReturn: ", valorTotalRecebivelReturn);

        // uint256 diasPassadosDesdeUltimoSaque = _diferencaEntreDias(block.timestamp, accMapping[msgSender].ultimoSaque);//50


        // uint valorDiarioAtigindo = (valorTotalRecebivelReturn / 100) * diasPassadosDesdeUltimoSaque;//520

        // return valorDiarioAtigindo;
        ...
        //valor reservado para jogos
        // accMapping[_msgSender()].valueReservedForGames = valorReservadoParaJogos;
        //console.log("valorAReceber: ", valorAReceber);
        return valorAReceber - valorReservadoParaJogos;

    }

    //Valor total recebivel considerando 200% + bonus fidelidade sem descontos de taxas
    ...
    function setRenewAccount() nonReentrant external payable 
    {
        ...
        //console.log("totalAcumuladoAReceber", totalAcumuladoAReceber);
        //console.log("valueReservedForGames", valueReservedForGames);
        //console.log("ganhoDiario", ganhoDiario);
        //console.log("tetoTotal", tetoTotal);
        //console.log("valorTotalRecebivelSemTaxas", valorTotalRecebivelSemTaxas);
    ...
    function _distribuiComissoes(address _indicator) private 
    {
        for(uint16 i = 0; i < bonusLevel.length; i++)
        {
            if(accMapping[_indicator].numeroDeIndicados >= i+1)
            {
                ...
                //verificar se não é melhor mandar a taxa de 6% diretamente para variavel e fazer retirarda junto com carteiras aliados
    ...
    {
        //console.log("totalHistoricoContrato: ", totalHistoricoContrato);
        //console.log("address(this).balance: ", address(this).balance);
        //console.log("contractSecurityLock: ", contractSecurityLock);
        //console.log("(totalHistoricoContrato  / 100) * 50", (totalHistoricoContrato  / 100) * 50);
    ...
    {
        return accMapping[_address];
    }

    function contrato() external view returns(uint256) 
    {
        //console.log("Total historico: ", totalHistoricoContrato);
        //console.log("Total comissoes dos 3s: ", totalBalanceInAliados);
        //console.log("carteira expansao: ", address(0x14723A09ACff6D2A60DcdF7aA4AFf308FDDC160C).balance);
