...
contract SynthMixPoolV1 is Admin {
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        interval = stakingDetails[userAddress].startTime[stakeId] > 0
            ? endOfProfit.sub(stakingDetails[userAddress].startTime[stakeId])
            : endOfProfit.sub(block.timestamp);

        uint256[2] memory stakeData;
        stakeData[0] = (stakedAmount > 0 ? stakedAmount : 0);
        stakeData[1] = (totalStake > 0 ? totalStake : 0);
    ...
    function _UserRewardCalculation(
        ...
        address referrerMain = stakingDetails[userAddress].referrer[stakeId] !=
            address(0)
            ? stakingDetails[userAddress].referrer[stakeId]
            : address(0);

        address tokenMain = stakingDetails[userAddress].tokenAddress[stakeId] !=
            address(0)
            ? stakingDetails[userAddress].tokenAddress[stakeId]
            : address(0);
        ...
        tokensEarned[0] = rewardsEarned > 0 ? rewardsEarned : 0;
        tokensRefEarned[0] = refEarned > 0 ? refEarned : 0;
        ...
        while (i < intervalDays.length) {
            if (noOfDays[0] >= intervalDays[i]) {
                ...
                tokensEarned[i] = rewardsEarned > 0 ? rewardsEarned : 0;
                tokensRefEarned[i] = refEarned > 0 ? refEarned : 0;
