...
contract TownToken is BEP20Burnable, Ownable{
    ...
    function _addLiquidity(uint256 tokenLiquid) private {
        uint256 half = tokenLiquid.div(2);
        uint256 otherHalf = tokenLiquid.sub(half);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        uint256 currentRate =  _getRate();
        (uint256 tFee, uint256 tBurn, uint256 tLiquid) =  _getFee(tAmount);
        uint256 rFee = _getValuesR(tAmount);
        uint256 tTransferAmount = getTTransferAmount(tAmount, tFee, tBurn, tLiquid);

        if(tFee > 0) super._transfer(sender, FeeAddress, tFee);
        if(tLiquid > 0) _addLiquidity(tLiquid);
    ...
    function _getValuesR(uint256 tAmount) private view returns (uint256) {
        uint256 currentRate =  _getRate();
