...
contract Econ721Signature is EconBaseContract, Econ721Enumerable, ERC721HolderUpgradeable {
    ...
    event Withdrawn(bytes32 messageHash, address user, address contractAddress, uint256 tokenId);
    event WithdrawFailed(bytes32 messageHash, address user, address contractAddress, uint256 tokenId);
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(address(this), _contractAddress, _tokenId, _to, _expiryTime));
    }
    ...
    function withdraw(
        ...
        bytes32 messageHash = getMessageHash(_contractAddress, _tokenId, _msgSender(), _expiryTime);
        ...
        try IERC721(_contractAddress).safeTransferFrom(address(this), _msgSender(), _tokenId) {
            ...
            emit Withdrawn(messageHash, _msgSender(), _contractAddress, _tokenId);
        } catch {
            emit WithdrawFailed(messageHash, _msgSender(), _contractAddress, _tokenId);
        }
    }
