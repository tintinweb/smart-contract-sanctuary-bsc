...
contract BoostToken is ERC20Detailed, Ownable {
    ...
    bool public _rebasePairTokens = false;
    ...
    uint256 public sellThreshold = 10000 * 10**DECIMALS;

    uint256 public _rebasePeriod = 15 minutes;
    uint256 public _rebaseStartedTime;
    uint256 public _lastRebasedTime;
    uint256 public _lastAddLiquidityTime;
    uint256 public _addLiqPeriod = 15 minutes;
    ...
    constructor() ERC20Detailed("BoostToken1", "$Boost1", uint8(DECIMALS)) Ownable() {
        ...
        aeternaTreasuryAddress = 0xd9Cb687EccC712A045eE3Af1542A4b057d8611C1; 
        aeternaDefenceAddress = 0xff8cC92700744ebF425fac18d546d020bd9C8459;
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, 
        address _aeternaTreasuryAddress,
        address _aeternaDefenceAddress
    ) external onlyOwner {
    ...
    function setLiquidityAddPeriod(uint256 liqPeriod) external onlyOwner {
        _addLiqPeriod = liqPeriod;
    }
    ...
    function rebase() internal {
        if ( inSwap ) return;
        uint256 rebaseRate;
        uint256 deltaTimeFromInit = block.timestamp - _rebaseStartedTime;
        uint256 deltaTime = block.timestamp - _lastRebasedTime;
        uint256 times = deltaTime.div(_rebasePeriod);
        uint256 epoch = times.mul(15);
        uint256 pairBalanceBefore = _gonBalances[pairAddress].div(_gonsPerFragment);
        uint256 daysOfYear = (365 days);

        if (deltaTimeFromInit <= daysOfYear) {
            rebaseRate = 2170;
        } else if (deltaTimeFromInit <= daysOfYear.mul(2)) {
            rebaseRate = 211;
        } else if (deltaTimeFromInit <= daysOfYear.mul(8)) {
            rebaseRate = 14;
        } else {
            rebaseRate = 2;
        }
        ...
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        _lastRebasedTime = _lastRebasedTime.add(times.mul(_rebasePeriod));
    ...
    function _transferFrom(
        ...
        if (inSwap) {
            ...
            if (hasFee && shouldRebase()) {
                rebase();
            }

            if (hasFee && shouldAddLiquidity()) {
                addLiquidity();
            }

            if (hasFee && shouldSwapBack()) {
                swapBack();
            }
    ...
    function takeFee(
        ...
        if(feeAmount > 0) {
            uint256 forLiq = feeAmount.div(_totalFee).mul(liquidityFee).div(2);
            uint256 forOthers = feeAmount.sub(forLiq);
        
            if(forLiq > 0) {
                _gonBalances[autoLiquidityReceiver] = _gonBalances[autoLiquidityReceiver].add(forLiq);
            }

            if(forOthers > 0) {
                _gonBalances[address(this)] = _gonBalances[address(this)].add(forOthers);
            }
    ...
    //     if (amountToLiquify > 0 && bnbReceived > 0) {
    //         router.addLiquidityETH{value: bnbReceived}(
    //             address(this),
    //             amountToLiquify,
    //             0,
    //             0,
    ...
    function addLiquidity() internal swapping {
        ...
        uint256 bnbAvailable = address(this).balance;

        if (autoLiquidityAmount > 0 && bnbAvailable > 0) {
            router.addLiquidityETH{value: bnbAvailable}(
                address(this),
                autoLiquidityAmount,
                0,
                0,
    ...
    function swapBack() internal swapping {
        ...
        bool success;
        uint256 bnbReceived = (address(this).balance).sub(bnbBefore);
        uint256 nonLPShares = aeternaTreasuryFee.add(aeternaDefenceFee);
        uint256 noOfShares = nonLPShares.add(liquidityFee.div(2));

        uint256 bnbForLp = bnbReceived.mul(liquidityFee.div(2)).div(noOfShares);
        uint256 bnbForTreasury = bnbReceived.sub(bnbForLp).mul(aeternaTreasuryFee).div(nonLPShares);
        uint256 bnbForDefence = bnbReceived.sub(bnbForLp).sub(bnbForTreasury);
    ...
    function shouldRebase() internal view returns (bool) {
        return _autoRebase && (_totalSupply < MAX_SUPPLY) &&
            msg.sender != pairAddress  && !inSwap &&
            block.timestamp >= (_lastRebasedTime + _rebasePeriod);
    }

    function shouldAddLiquidity() internal view returns (bool) {
        return _autoAddLiquidity && !inSwap && 
            msg.sender != pairAddress &&
            block.timestamp >= (_lastAddLiquidityTime + _addLiqPeriod);
    }
