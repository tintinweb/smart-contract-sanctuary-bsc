...
contract PyeVesting is Ownable, ReentrancyGuard {
    ...
    uint256 public immutable startTime; // beginning of 30 day vesting window (block #)
    ...
    constructor(address _PYE, address _Apple, address _Cherry) {
    ...
    function getPYEAllotment() public view returns (uint256) {
        return Allotments[msg.sender].allotedPYE;
    }
    ...
    function getTotalPYEAllotments() public view returns (uint256) {
    ...
    function setPYE(address _PYE) public onlyOwner {
        PYE = _PYE;
    }

    function setApple(address _Apple) public onlyOwner {
        Apple = _Apple;
    }

    function setCherry(address _Cherry) public onlyOwner {
        Cherry = _Cherry;
    }
    ...
    function claimTokens() external nonReentrant notContract() {
        require(getElapsedTime() > 10 minutes , "You have not waited the 30-day cliff period!");
        ...
        uint256 withdrawableApple = ((weightedAllotment.mul(claimableApple)).div(accuracyFactor)).sub(Allotments[msg.sender].claimedApple);
        uint256 withdrawableCherry = ((weightedAllotment.mul(claimableCherry)).div(accuracyFactor)).sub(Allotments[msg.sender].claimedCherry);

        uint256 withdrawablePYE;

        if (getElapsedTime() >= 19 minutes) {
            withdrawablePYE = available;
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 18 minutes && getElapsedTime() < 19 minutes) {
            withdrawablePYE = (9 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 17 minutes && getElapsedTime() < 18 minutes) {
            withdrawablePYE = (8 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 16 minutes && getElapsedTime() < 17 minutes) {
            withdrawablePYE = (7 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 15 minutes && getElapsedTime() < 16 minutes) {
            withdrawablePYE = (6 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 14 minutes && getElapsedTime() < 15 minutes) {
            withdrawablePYE = (5 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 13 minutes && getElapsedTime() < 14 minutes) {
            withdrawablePYE = (4 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 12 minutes && getElapsedTime() < 13 minutes) {
            withdrawablePYE = (3 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 11 minutes && getElapsedTime() < 12 minutes) {
            withdrawablePYE = (2 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 10 minutes && getElapsedTime() < 11 minutes) {
    ...
    function calculateWithdrawableAmounts(address _address) external view returns (uint256, uint256, uint256) {
        ...
        uint256 weightedAllotment = (original.mul(accuracyFactor)).div(totalAllotments);
        uint256 withdrawableApple = ((weightedAllotment.mul(claimableApple)).div(accuracyFactor)).sub(Allotments[_address].claimedApple);
        uint256 withdrawableCherry = ((weightedAllotment.mul(claimableCherry)).div(accuracyFactor)).sub(Allotments[_address].claimedCherry);

        uint256 withdrawablePYE;

        if (getElapsedTime() >= 19 minutes) {withdrawablePYE = available;
        } else if (getElapsedTime() >= 18 minutes && getElapsedTime() < 19 minutes) {withdrawablePYE = (9 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 17 minutes && getElapsedTime() < 18 minutes) {withdrawablePYE = (8 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 16 minutes && getElapsedTime() < 17 minutes) {withdrawablePYE = (7 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 15 minutes && getElapsedTime() < 16 minutes) {withdrawablePYE = (6 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 14 minutes && getElapsedTime() < 15 minutes) {withdrawablePYE = (5 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 13 minutes && getElapsedTime() < 14 minutes) {withdrawablePYE = (4 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 12 minutes && getElapsedTime() < 13 minutes) {withdrawablePYE = (3 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 11 minutes && getElapsedTime() < 12 minutes) {withdrawablePYE = (2 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 10 minutes && getElapsedTime() < 11 minutes) {withdrawablePYE = tenPercent.sub(withdrawn);
