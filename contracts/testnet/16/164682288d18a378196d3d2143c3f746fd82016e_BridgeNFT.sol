...
contract BridgeNFT is OwnableUpgradeable {
    ...
    function setDestNftAddr(
        address _srcNft,
        uint256[] memory _dstChIds,
        address[] memory _dstNfts
    ) external onlyAdmin {
        require(_dstChIds.length == _dstNfts.length, "length mismatch");

        for (uint256 i = 0; i < _dstChIds.length; ) {
    ...
    function delDestNftAddr(address _srcNft, uint256 _dstChId)
        external
        onlyAdmin
    {
    ...
    function setTxFee(uint256[] memory _chids, uint256[] memory _fees)
        external
        onlyAdmin
    {
        require(_chids.length == _fees.length, "length mismatch");
        for (uint256 i = 0; i < _chids.length; ) {
    ...
    function bridge(
        ...
        require(
            IBridgeNFT(_srcNft).checkBridge(_tokenId),
            "tokenid not bridge"
        );
        require(
            msg.sender == IBridgeNFT(_srcNft).ownerOf(_tokenId),
            "not token owner"
        );
        address dstNft_ = checkAddr(_srcNft, _dstChId);
        require(msg.value >= destTxFee[_dstChId], "insufficient fee");
        if (origNFT[_srcNft]) {
            IBridgeNFT(_srcNft).transferFrom(
                msg.sender,
                address(this),
                _tokenId
            );
    ...
    function checkAddr(address _srcNft, uint256 _dstChid)
        internal
        view
        returns (address dstNft_)
    {
