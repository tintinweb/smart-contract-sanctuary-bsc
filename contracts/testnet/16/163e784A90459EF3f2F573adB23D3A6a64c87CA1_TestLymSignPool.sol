...
contract TestLymSignPool is OwnableUpgradeable, DivestorUpgradeable {
    ...
    function setTm(address account, uint lastRestTm_, uint lastClaimTm_, uint staticHp_) external onlyOwner {
        userInfo[account].lastReSetTm = lastRestTm_;
        userInfo[account].lastClaimTm = lastClaimTm_;
        userInfo[account].staticHP = staticHp_;
    }
    ...
    function addStaticHP() external onlyOrigin onlyOpen {
        ...
        uint256 deadLine1 = userInfo[msg.sender].staticHP + 3600 * 6;
    ...
    function getStaticReward(address account_) public view returns (uint256) {
        ...
        //     return 0;
        // }
        uint deadLine1 = userInfo[account_].lastReSetTm + 3600 * 6;
        if (deadLine > deadLine1) {
            deadLine = deadLine1;
        }
    ...
    // function loadInfo(address[] calldata accounts_, uint[] calldata infos) external onlyOwner {}
}
