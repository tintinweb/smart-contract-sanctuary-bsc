...
contract MarketPlace is ERC1155, ReentrancyGuard, Ownable {
    ...
    uint256 fee = 1000;
    address orignalContract = 0x1314B0a78C150146b093E477BE31c60222bceE63;
    ...
    function buyItem(address payable _seller, address _tokenAddress, uint256 _id, uint256 _amount) public payable {
        ...
        uint _price = saleInfo.price + fee;
        uint totalPrice = _price * _amount;
        require(msg.value >= totalPrice, "MarketPlace: not enought Aeon to cover total price.");
        saleInfo.amount = saleInfo.amount - _amount;

        IERC20 paymentToken = IERC20(orignalContract);
        uint256 funds = paymentToken.allowance(msg.sender, address(orignalContract));
        require(funds >= totalPrice, "not enough funds");

        paymentToken.transferFrom(msg.sender, orignalContract, fee);
        paymentToken.transferFrom(msg.sender, _seller, saleInfo.price);
