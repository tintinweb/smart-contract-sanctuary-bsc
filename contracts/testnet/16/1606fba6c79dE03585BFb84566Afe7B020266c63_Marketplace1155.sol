...
contract Marketplace1155 is ERC1155HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event ListingBought(uint256 indexed id, address indexed buyer, address indexed seller, uint256 tokenAmount, uint256 price);

    event ListingBid(uint256 indexed id, address indexed bidder, address indexed seller, uint256 tokenAmount, uint256 amount, uint256 timeEnd);

    event ListingCanceled(uint256 indexed id, address indexed canceler, uint256 amount);

    event PendingWithdrawn(uint256 indexed listingId, address indexed claimer, uint256 amount);

    event CollectibleClaimed(uint256 indexed listingId, address indexed seller, address indexed target, uint256 tokenAmount, uint256 amount);
    ...
    event Boosted(uint256 indexed listingId, uint256 amount);
    ...
    function initialize(address boost_, uint256 gracePeriod_, uint256 _fee) public initializer {
        ...
        for (uint256 i = 1; i <= _lastListingId; i++) {
            if ( listings[i].claimed != true && listings[i].collection == collection ) {
                if ( listings[i].listingType == ListingType.FixedPrice ) {
                    amount += listings[i].minimalBid * listings[i].tokenAmount;
                }
    ...
    function createListing(
        ...
        IERC1155Upgradeable(collection).safeTransferFrom(_msgSender(), address(this), tokenId, tokenAmount, new bytes(0));
    ...
    function buy(uint256 listingId, uint256 amount) external nonReentrant {
        ...
        listings[listingId].currency.safeTransferFrom(
            _msgSender(),
            address(this),
            listings[listingId].minimalBid.mul(amount)
        );

        _buy(listingId, amount);
    }

    function buyNative(uint256 listingId, uint256 amount) external payable nonReentrant {
        require(address(listings[listingId].currency) == address(0), "Listing isn't for ETH");
        require(msg.value >= listings[listingId].minimalBid.mul(amount), "Not enough ETH passed");
    ...
    function bidNative(uint256 listingId, uint256 amount) external payable nonReentrant {
        require(address(listings[listingId].currency) == address(0), "Listing isn't for ETH");

        uint256 totalBid = msg.value + listings[listingId].pendingAmounts[_msgSender()];
        listings[listingId].pendingAmounts[_msgSender()] = 0;
    ...
    function cancel(uint256 listingId) external {
        ...
        emit ListingCanceled(listingId, _msgSender(), listings[listingId].tokenAmount);
    }
    ...
    function claimCollectible(uint256 listingId) external nonReentrant {
        ...
        emit CollectibleClaimed(listingId, listings[listingId].seller, target, listings[listingId].tokenAmount, listings[listingId].lastBid);
    }
    ...
    function boost(uint256 listingId_, uint256 amount_) external {
        ...
        emit Boosted(listingId_, amount_);
    }
    ...
    function adminSetFee(uint256 _fee) external onlyOwner {
        fee = _fee;

        emit FeeChanged(fee);
    }

    function adminWithdraw(address currency, address target, uint256 amount) external onlyOwner {
        require(currencies[address(currency)].enabled, "This currency is not allowed");

        if (currency == address(0)) {
            payable(target).send(amount);
        } else {
    ...
    function _buy(uint256 listingId_, uint256 amount_) private {
        ...
        if (listings[listingId_].tokenAmount == 0) {
            listings[listingId_].lastBidder = _msgSender();
        ...
        emit ListingBought(listingId_, _msgSender(), listings[listingId_].seller, amount_, listings[listingId_].minimalBid.mul(amount_));
    }
    ...
    function _bid(uint256 listingId, uint256 amount) private {
        ...
        emit ListingBid(listingId, _msgSender(), listings[listingId].seller, listings[listingId].tokenAmount, amount, listings[listingId].timeEnd);
    }
    ...
    function _distributeValue(uint256 listingId) private {
        uint256 price = listings[listingId].lastBid - (listings[listingId].lastBid / DENOMINATOR * fee);
        address royaltyReceiver;
        uint256 royaltyAmount;

        if(listings[listingId].collection.supportsInterface(type(IERC2981Upgradeable).interfaceId)) {
            (royaltyReceiver, royaltyAmount) = IERC2981Upgradeable(listings[listingId].collection).royaltyInfo(listings[listingId].tokenId, price);
        }
        ...
        _transferCurrency(listingId, listings[listingId].seller, price - royaltyAmount);
    }
