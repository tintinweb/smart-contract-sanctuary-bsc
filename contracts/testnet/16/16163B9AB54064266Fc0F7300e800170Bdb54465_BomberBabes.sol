...
// @dev: defi_guru
// BomberBabes PROTOCOL COPYRIGHT (C) 2022
...
contract BomberBabes is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = 'BomberBabes';
    string public _symbol = '$BABES';
    ...
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(insuranceFundFee).add(vaultFee);
    uint256 public feeDenominator = 1000;
    uint256 public rebaseInterval = 60 minutes;
    uint256 public apyPerInterval = 1000;
    ...
    constructor(
        ...
        address _vault
    ) ERC20Detailed('BomberBabes', '$BABES', uint8(DECIMALS)) Ownable() {
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit < (365 days)) {
            rebaseRate = apyPerInterval;
        } else if (deltaTimeFromInit >= (365 days)) {
            rebaseRate = apyPerInterval / 10;
        } else if (deltaTimeFromInit >= ((15 * 365 days) / 10)) {
            rebaseRate = apyPerInterval / 100;
        } else if (deltaTimeFromInit >= (7 * 365 days)) {
            rebaseRate = apyPerInterval / 1000;
        }
    ...
    function takeFee(
        ...
        _gonBalances[vault] = _gonBalances[vault].add(gonAmount.div(feeDenominator).mul(vaultFee));
