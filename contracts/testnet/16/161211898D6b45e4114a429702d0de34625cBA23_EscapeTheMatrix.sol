...
//------------------ReferralSystem------------------//
    ...
    function getAllReferrers() external view returns(Referral[] memory){
        uint256 length = _referrers.length();

        Referral[] memory list = new Referral[](length);
    ...
    function getReferrerByIndex(uint256 index) external view returns(Referral memory){
    ...
    function getReferrerByAddress(address _user) external view returns(Referral memory){
        return Referral(
    ...
    function getReferrerByCode(bytes32 _code) external view returns(Referral memory){
        return Referral(
    ...
    function getReferrerBetween(uint256 start, uint256 end) external view returns(Referral[] memory){
    ...
    function generateMyReferral(string memory _referredCode) external{
        ...
        require(addressToReferrals[address(msg.sender)].referrerCode != _result, "This code already used by you.");
    ...
    function setReferrerToSystem(string memory _referredCode) external{
        ...
        require(addressToReferrals[address(msg.sender)].referrerCode != _result, "Can not use own referrer code.");
        require(referralCodestoAddress[_result] != address(0), "Invalid referrer code.");
...
//------------------Transfer------------------//
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (from == uniswapV2Pair){
            ...
            require(fetchReferrerBalance(receiver) >= referralWalletLimit, "Error: Insufficient referrer balance!");
        }
