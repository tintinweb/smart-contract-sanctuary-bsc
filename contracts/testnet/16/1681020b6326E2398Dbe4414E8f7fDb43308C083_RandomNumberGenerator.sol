...
import './interfaces/IPlearnLottery.sol';
...
contract RandomNumberGenerator is VRFConsumerBase, IRandomNumberGenerator, Ownable {
    ...
    function getRandomNumber() external override {
        require(msg.sender == plearnLottery, 'Only PlearnLottery');
    ...
    /**
     * @notice Set the address for the PLearnLottery
     * @param _plearnLottery: address of the Plearn lottery
     */
    function setLotteryAddress(address _plearnLottery) external onlyOwner {
        plearnLottery = _plearnLottery;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        latestLotteryId = IPlearnLottery(plearnLottery).viewCurrentLotteryId();
    }
}
...
interface IPlearnLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in Plearn per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in Plearn token
     * @dev Callable by operator
     */
