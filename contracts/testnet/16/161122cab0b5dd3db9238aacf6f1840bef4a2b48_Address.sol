...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(uint256 minBalance, uint256 minDividend)
    ...
    function updateMinimumDividendForAutoClaim(uint256 _newMinimumDividend)
        external
        onlyOwner
    {
        require(
...
contract DGC is ERC20, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint256 public swapTax;

    uint256 public marketingShares;
    uint256 public charityShares;
    uint256 public team1Shares;
    uint256 public team2Shares;
    uint256 public team3Shares;

    address public marketingWallet = 0xCdD7a5Ebea2EeA145509e1af3d7A6Fe3f258b18A;
    address public charityWallet = 0xCdD7a5Ebea2EeA145509e1af3d7A6Fe3f258b18A;
    address public team1Wallet = 0xCdD7a5Ebea2EeA145509e1af3d7A6Fe3f258b18A;
    address public team2Wallet = 0xCdD7a5Ebea2EeA145509e1af3d7A6Fe3f258b18A;
    address public team3Wallet = 0xCdD7a5Ebea2EeA145509e1af3d7A6Fe3f258b18A;
    ...
    event UpdateCharityWallet(address indexed charityWallet);
    event UpdateTeam1Wallet(address indexed team1Wallet);
    event UpdateTeam2Wallet(address indexed team2Wallet);
    event UpdateTeam3Wallet(address indexed team3Wallet);
    event UpdateTaxShares(
        uint256 marketingShares,
        uint256 charityShares,
        uint256 team1Shares,
        uint256 team2Shares,
        uint256 team3Shares
    );
    ...
    constructor() ERC20("DogeClaus", "DGC") {
        ...
        swapTokensAtAmount = (totalSupply() * 3) / 1000; // 0.3% of totalSupply
    ...
    function setCharityWallet(address _charityWallet) external onlyOwner {
        ...
        require(
            ...
            "Charity wallet cannot be the zero address"
        );

        charityWallet = _charityWallet;
        emit UpdateCharityWallet(_charityWallet);
    }

    function setTeam1Wallet(address _team1Wallet) external onlyOwner {
        require(
            _team1Wallet != team1Wallet,
            "Team 1 wallet is already that address"
        );
        require(
            _team1Wallet != address(0),
            "Team 1 wallet cannot be the zero address"
        );

        team1Wallet = _team1Wallet;
        emit UpdateMarketingWallet(_team1Wallet);
    }

    function setTeam2Wallet(address _team2Wallet) external onlyOwner {
        require(
            _team2Wallet != team2Wallet,
            "Team 2 wallet is already that address"
        );
        require(
            _team2Wallet != address(0),
            "Team 2 wallet cannot be the zero address"
        );

        team2Wallet = _team2Wallet;
        emit UpdateMarketingWallet(_team2Wallet);
    }

    function setTeam3Wallet(address _team3Wallet) external onlyOwner {
        require(
            _team3Wallet != team3Wallet,
            "Team 3 wallet is already that address"
        );
        require(
            _team3Wallet != address(0),
            "Team 3 wallet cannot be the zero address"
        );

        team3Wallet = _team3Wallet;
        emit UpdateMarketingWallet(_team3Wallet);
    }

    function setTaxShare(
        uint256 _marketingShares,
        uint256 _charityShares,
        uint256 _team1Shares,
        uint256 _team2Shares,
        uint256 _team3Shares
    ) external onlyOwner {
        uint256 totalShares = _marketingShares +
            _charityShares +
            _team1Shares +
            _team2Shares +
            _team3Shares;

        require(totalShares == 100, "Share must be 100%");

        marketingShares = _marketingShares;
        charityShares = _charityShares;
        team1Shares = _team1Shares;
        team2Shares = _team2Shares;
        team3Shares = _team3Shares;

        emit UpdateTaxShares(
            _marketingShares,
            _charityShares,
            _team1Shares,
            _team2Shares,
    ...
    function swapBack() internal inSwap {
        ...
        uint256 feeShares = (newBalance * 2) / 3;

        uint256 rewardShares = newBalance - feeShares;

        if (feeShares > 0) {
            uint256 toMarketing = (feeShares * marketingShares) / 100;
            uint256 toCharity = (feeShares * charityShares) / 100;
            uint256 toTeam1 = (feeShares * team1Shares) / 100;
            uint256 toTeam2 = (feeShares * team2Shares) / 100;
            uint256 toTeam3 = feeShares -
                toMarketing -
                toCharity -
                toTeam1 -
                toTeam2;
            ...
            if (toCharity > 0) {
                sendBNB(charityWallet, toCharity);
            }

            if (toTeam1 > 0) {
                sendBNB(team1Wallet, toTeam1);
            }

            if (toTeam2 > 0) {
                sendBNB(team2Wallet, toTeam2);
            }

            if (toTeam3 > 0) {
                sendBNB(team3Wallet, toTeam3);
            }
        }

        if (rewardShares > 0) {
            sendBNB(address(dividendTracker), rewardShares);
            dividendTracker.distributeDividends(rewardShares);
            emit SendDividends(rewardShares);
        }
    }
    ...
    function updateMinimumDividendForAutoClaim(uint256 newMinimumBalance)
        external
        onlyOwner
    {
