...
contract PlanetFarm is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize() public initializer(){
        ...
        gammaInfinityVaultAddress = 0xe19759af64a2170d0685E2D05306a1B6cc996c82;
    ...
    function pendingGAMMAAllPools(address _user) external view returns (uint256) {
        ...
        for(uint i = 0; i < poolLen; ++i){
            console.log("see inside pendingGAMMAAllPools");
            console.logUint(usersPoolList[_user][i]);
            console.logUint(_pendingGAMMA(usersPoolList[_user][i], _user));
            totalPendingGamma = totalPendingGamma + _pendingGAMMA(usersPoolList[_user][i], _user);
        }
        console.logUint(usersClaimableGamma[_user]);
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        console.log("usersClaimableGamma");
        console.logUint(usersClaimableGamma[_msgSender()]);
        console.log("see what is pending here: ");
        console.log(pending);
        ...
        if (_wantAmt > 0) {
            ...
            console.log("comes inside wantAmt");
    ...
    function updateFactor(address _user, uint256 _newiGammaBalance) external {
        ...
        for (uint256 pid; pid < len; ++pid) {
            ...
            console.log("see usersClaimableGamma inside updateFactor");
            console.logUint(usersClaimableGamma[_msgSender()]);
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        console.log("see usersClaimableGamma inside withdraw");
        console.logUint(usersClaimableGamma[_msgSender()]);
    ...
    function claimAllPoolsPendingGamma() external nonReentrant {
        ...
        for(uint256 i = 0; i < length; ++i) {
            ...
            if(user.shares > 0){
                uint256 pending = (user.shares*pool.accGAMMAPerShare + user.factor*pool.accGAMMMAPerFactorPerShare)/1e12 - user.rewardDebt;
                usersClaimableGamma[_msgSender()] = usersClaimableGamma[_msgSender()] + pending;
                user.rewardDebt = (user.shares*pool.accGAMMAPerShare + user.factor*pool.accGAMMMAPerFactorPerShare);
            }
        ...
        console.logAddress(_msgSender());
        console.log("see totalPending here: ");
        console.logUint(totalPending);
