...
contract GoatPluCoin{
  ...
  event Airdropped(address indexed _userAddress, uint256 _amount);
    ...
    constructor(address ownerAddress,IBEP20 _BUSD,IBEP20 _GPC) public
    {
        owner = ownerAddress;  
        BUSD = _BUSD;
        GPC = _GPC;
	...
	function BuyForm(string memory investorId,uint investment,string memory pool) public payable
	{
	  //require(msg.value>=investment*1e18);
		BUSD.transferFrom(msg.sender ,owner,investment*1e18);
		emit Reinvestment( investorId,msg.value,msg.sender,pool);
	}
  function Staking(string memory investorId,uint investment,string memory pool) public payable
	{
	  //require(msg.value>=investment*1e18);
		GPC.transferFrom(msg.sender ,owner,investment*1e18);
		emit Reinvestment( investorId,msg.value,msg.sender,pool);
	}
