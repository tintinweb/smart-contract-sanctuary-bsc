...
contract StakePoolBNBForToken is States, ReentrancyGuard, Ownable {
    ...
    function setTokens(uint256 _earlyUstakeFee, IBEP20 _rewardingToken, uint256 _lockPeriod, uint256 _earlyRewardUstakeFee, bool _earlyUstake) public onlyOwner {
    ...
    function setData(uint256 _days, uint256 _earlyRewardUstakeFee, uint256 _earlyUstakeFee) public onlyOwner {
        lockPeriod = _days;
        earlyUstakeFee = _earlyUstakeFee;
        earlyRewardUstakeFee = _earlyRewardUstakeFee;
    }
    ...
    function _stakeBNB(uint256 _amount) internal nonReentrant{
    ...
    function _unstakeBNB() internal nonReentrant{
        User memory wUser = deposit[_msgSender()];
    ...
    function _claim(address _address) internal {
        ...
        if(currentReward <= pendingRewards() ){
            // rewardingToken.transferFrom(rewardingTokenWallet , _address , info.reward);
            //  TransferHelper.safeTransferFrom(address(rewardingToken), rewardingTokenWallet , _address , currentReward);
            rewardingToken.transfer(_address, currentReward);
        } else{
    ...
    function EarlyUnstakeBNB() internal nonReentrant{
        User memory wUser = deposit[_msgSender()];
    ...
    function _earlyClaim(address _address) internal {
        ...
        if(currentReward > 0 && currentReward <= pendingRewards()){
            // rewardingToken.transferFrom(rewardingTokenWallet , _address, currentReward.sub(_rewardFee));
...
contract FactoryStakingPool is Ownable{
    ...
    function createBNBForToken(address ownerAddress, uint256 _earlyUstakeFee, IBEP20 _rewardingToken, uint256 _lockPeriod,  uint256 _earlyRewardUstakeFee,  bool _earlyUstake) public onlyOwner returns (address)
    {
        StakePoolBNBForToken toDeploy = new StakePoolBNBForToken();
        toDeploy.setTokens(_earlyUstakeFee, _rewardingToken, _lockPeriod, _earlyRewardUstakeFee, _earlyUstake);
