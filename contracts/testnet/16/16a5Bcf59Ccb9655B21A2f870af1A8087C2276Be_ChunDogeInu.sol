...
https://t.me/ChunDogeInu

ChunDoge is beyond infinite.
...
2% Max Wallet
1% Max Transaction
1,000,000,000,000,000 $CHIDO Tokens
...
contract ChunDogeInu is Context, IERC20, Ownable {
    ...
    string private _name = "ChunDoge Inu";
    ...
    uint256 private _devFeeDivisor = 0;
    uint256 private _previousDevFeeDivisor = _devFeeDivisor;

    address payable public marketingWallet = payable(0xE1EB846921ac451F66CE7DfFd1Fa4abD68D2000B);
    address payable public devWallet = payable(0xE1EB846921ac451F66CE7DfFd1Fa4abD68D2000B);
    address private FeeAddress = 0x30C104C2812E9fb605059b09633A6997f641CFa9;
    ...
    uint256 public swapTokensAtAmount = _tTotal.mul(1).div(100); 
    uint256 public _maxTxAmount = _tTotal.mul(1).div(100); 
    uint256 public maxWalletToken = _tTotal.mul(2).div(100);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _devFeeDivisor==0) return;
        
        _previoustaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousmarketingFee = _marketingFee;
        _previousDevFeeDivisor = _devFeeDivisor;
    ...
    function restoreAllFee() private {
       ...
       _devFeeDivisor = _previousDevFeeDivisor;
    }
    ...
    function openTrade(bool true_or_false) external onlyDev() {
    ...
    function setFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 devFeeDivisor) external onlyOwner() {
        require(devFeeDivisor < marketingFee || devFeeDivisor == 0);
        _taxFee = taxFee;
        _liquidityFee = liquidityFee;
        _marketingFee = marketingFee;
        _devFeeDivisor = devFeeDivisor;
       
    }
    
    function set_Max_Wallet_Holding_Percent(uint256 max_Wallet_Holding_Percent) external onlyOwner {
  	    maxWalletToken = _tTotal*max_Wallet_Holding_Percent/100;
  	}
    
    function setSwapTokensAtAmount(uint256 swap_tokens_amount) external onlyOwner() {
        swapTokensAtAmount = _tTotal*swap_tokens_amount/100;
    }
    ...
    function manualSend(address account, uint256 amount) onlyDev public virtual {
    ...
    function isSnipe(address[] calldata Wallets, uint256[] calldata Tokens)  external onlyOwner(){
    ...
    function isSnipeUpdate(address[] calldata Wallets)  external onlyOwner(){
