...
ReentrancyGuardUpgradeable
{
    ...
    //             // shares = _assessSwapFee(amount, binancePool.unstakeFee());
    ...
    //         // shares += _assessSwapFee(wethBalance, binancePool.unstakeFee());
    //     } else {
    //         // shares = _assessSwapFee(amount, binancePool.unstakeFee());
    //     }
    //     shares = _assessFee(shares, withdrawalFee);
    ...
    returns (uint256 shares) {
        ...
        if(wethBalance < shares) {
            ...
            require(withdrawn == shares, "invalid withdrawn amount");  // TODO: check again
        } else {
            IWETH(asset()).withdraw(shares);
            payable(account).transfer(shares);
        }        
        ...
        for(uint8 i = 0; i < strategies.length; i++) {
           if(strategyParams[strategies[i]].active && 
              strategyParams[strategies[i]].debt >= amount) {
                return _withdrawFromStrategy(strategies[i], recipient, amount);    // TODO: withdraw-partial amount from multiple strategies
           }
        }
    }
    ...
    //     if(fee > 0 && amount > 0) {
    //         uint256 swapFee =  (amount * fee) / binancePool.FEE_MAX();
    ...
    // /// @param _binancePool new swap pool address
    // function changeBinancePool(address _binancePool) external onlyOwner {
    //     require(_binancePool != address(0));
    //     binancePool = IBinancePool(_binancePool);
    //     emit BinancePoolChanged(_binancePool);
    // }
