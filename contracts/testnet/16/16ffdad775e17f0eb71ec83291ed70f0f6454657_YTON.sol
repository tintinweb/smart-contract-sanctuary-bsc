...
contract YTON is Ownable{
	...
	//用户信息 上级  动态  静态  直推人数
	function getUserInfo(address userAddress) public view returns(address,uint256,uint256,uint256){
		User storage user = users[userAddress];

		return (user.referrer,user.bonus,user.staticBonus,user.totalReferrer);
	}
