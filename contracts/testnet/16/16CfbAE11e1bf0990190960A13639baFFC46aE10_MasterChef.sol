...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingTech(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            uint256 techReward = multiplier.mul(pool.fixedApr).mul(1e12).div(BLOCKS_PER_YEAR.mul(100));
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 techReward = multiplier.mul(pool.fixedApr).mul(pool.totalAmount).mul(1e12).div(
            BLOCKS_PER_YEAR.mul(100)
        );
    ...
    function deposit(
        ...
        if (_amount > 0) {
            ...
            if (address(referral) != address(0)) {
                ...
                if (referrer != address(0)) {
                    referral.addTotalFund(referrer, _amount, 0);
            ...
            if (address(pool.lpToken) == address(tech)) {
        ...
        if (pool.lockingPeriod >= 365 days && _amount >= minAmountBonusNFT) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(referral) != address(0) && referral.getReferrer(msg.sender) != address(0)) {
                referral.reduceTotalFund(referral.getReferrer(msg.sender), _amount, 0);
            }
        }
        user.rewardDebt = user.amount.mul(pool.accTechPerShare).div(1e12);
    ...
    function withdrawInvestment(uint256 _pid) public nonReentrant {
        ...
        if (_totalWithdrawalAmount > 0 && _totalWithdrawalAmount <= user.amount) {
            ...
            if (address(referral) != address(0) && referral.getReferrer(msg.sender) != address(0)) {
                referral.reduceTotalFund(referral.getReferrer(msg.sender), _totalWithdrawalAmount, 0);
            }
        }
        user.rewardDebt = user.amount.mul(pool.accTechPerShare).div(1e12);
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(referral) != address(0) && referral.getReferrer(msg.sender) != address(0)) {
            referral.reduceTotalFund(referral.getReferrer(msg.sender), amount, 0);
        }
    ...
    function mintBunny(address _to) internal {
        uint256 _tokenId = randomBunnyId(10);
            ...
            if (address(referral) != address(0)) {
                ...
                if (commissionAmount > 0 && _referrer != address(0)) {
                    ...
                    if (totalFund > minRequimentRef.mul(depth)) {
                        ...
                        payReferralCommission(_referrer, _amount, depth.add(1), commissionAmount, totalFund);
                    }
                }
            }
        }
    ...
    function payDirectCommission(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        uint256 directCommissionAmount = _amount.mul(pool.directCommission);
        ...
        if (vipBranch) {
            ...
            if (upperVip != address(0)) {
                tech.mint(upperVip, directCommissionAmount.mul(2).div(1e3));
                referral.recordReferralCommission(upperVip, directCommissionAmount.mul(2).div(1e3));
            }
        }
    }
