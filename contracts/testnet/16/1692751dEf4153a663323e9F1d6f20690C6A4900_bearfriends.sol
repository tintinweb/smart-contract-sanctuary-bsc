...
contract MerkleWhitelist is Ownable {
  ...
  bytes32 public wl2WhitelistMerkleRoot;
...
contract bearfriends is Ownable, ERC721A, MerkleWhitelist, ReentrancyGuard {
    ...
    uint256 public MAIN_PRICE = 0.0001 ether;

    address private team1 = 0x1D04D136a61b1b2Cc2d172229AbC76C5b468f23E;
    address private team2 = 0xf6B7759C18e1a7F683385b02ee86b5C17a9f9174;

    uint256 public TEAM1_AMOUNT = 1;
    uint256 public TEAM2_AMOUNT = 55;
    uint256 public WL1_AMOUNT = 3;
    uint256 public WL2_AMOUNT = 5;
...
1666323780
    */
    ...
    uint256 public WL1_START_TIME = 1666323540;
    uint256 public WL1_END_TIME = 1666323660;

    uint256 public WL2_START_TIME = 1666323660;
    uint256 public WL2_END_TIME = 1666323780;

    uint256 public MAIN_START_TIME = 1666323780;
    uint256 public MAIN_END_TIME = 1666323780;


    uint16 public WL1_MINTED;
    uint16 public WL2_MINTED;
    uint16 public MAIN_MINTED;


    uint16 public WL1_LIMIT=1;
    uint16 public WL2_LIMIT=1;
    uint16 public MAIN_LIMIT=5;
    ...
    string public UNREVEALED_URI = "https://data.bearfriends.net/bear/box.json";
    ...
    function wl1Info(address user) public view returns (uint256,uint256,uint256,uint256,uint256,uint256) {
       return  (WL1_AMOUNT,WL1_MINTED,0,WL1_START_TIME,WL1_END_TIME,WL1_WALLET_CAP[user]);
    }

    function wl2Info(address user) public view returns (uint256,uint256,uint256,uint256,uint256,uint256) {
       return  (WL2_AMOUNT,WL2_MINTED,0,WL2_START_TIME,WL2_END_TIME,WL2_WALLET_CAP[user]);
    }

    function mainInfo(address user) public view returns (uint256,uint256,uint256,uint256,uint256,uint256) {
       return  (mainAmount(),MAIN_MINTED,MAIN_PRICE,0,MAIN_END_TIME,MAIN_WALLET_CAP[user]);
    }

    function mainAmount() public view returns (uint256) {
       return  ALL_AMOUNT - TEAM1_AMOUNT - TEAM2_AMOUNT - WL1_MINTED - WL2_MINTED - MAIN_MINTED;
    }

  
    function mintWL1(uint8 quantity,bytes32[] memory proof)
        public
        onlyWl1Whitelist(proof)
    {
        ...
        require(WL1_WALLET_CAP[msg.sender] + quantity <= WL1_LIMIT, "Purchase would exceed max number of metacards per wallet."); 
        require(WL1_MINTED + quantity <= WL1_AMOUNT,"Max supply for WL1 reached!");
        require(totalSupply() + quantity <= ALL_AMOUNT,"reached max supply");

        WL1_MINTED = WL1_MINTED + quantity;

        WL1_WALLET_CAP[msg.sender] += quantity;

        _safeMint(msg.sender, quantity);
    }

    function mintWL2(uint8 quantity,bytes32[] memory proof)
        public
       onlyWl2Whitelist(proof)
    {
        ...
        require(WL2_WALLET_CAP[msg.sender] + quantity <= WL2_LIMIT, "Purchase would exceed max number of metacards per wallet."); 
        require(WL2_MINTED + quantity <= WL2_AMOUNT,"Max supply for WL2 reached!");
        require(totalSupply() + quantity <= ALL_AMOUNT,"reached max supply");

         if (WL2_MINTED + quantity == WL2_AMOUNT){
            _isWL2Active = false;
            _isMainActive = true;
            STATE = 3;
        }

        WL2_MINTED = WL2_MINTED + quantity;

        WL2_WALLET_CAP[msg.sender] += quantity;

        _safeMint(msg.sender, quantity);

    }

     function mintMain(uint8 quantity)
        public
        payable
    {
        require(quantity > 0, "Must mint at least 1 token.");
   ...
   function withdraw() public onlyOwner nonReentrant {
    ...
    function flipAllState(bool isWL1Active,bool isWL2Active,bool isMainActive,uint256 _st) external onlyOwner {
