...
contract CRYPTOHEALTH is Context, IERC20, Ownable {
    ...
    address private Wallet_Team = 0x2aF06333382CfC4189CF749C5b4a163fbE7F5672;
    ...
    uint256 private _teamSellStart = 1749556800; //To be updated to 1749556800 (June 10, 2025 12:00:00 GMT )for Final Contract
    ...
    uint256 private donationTransferMin = 99 * 10**3 * 10**9; //Sell 99K at a time
    ...
    constructor() payable {
        ...
        // Test Wallets
        _isTeamWallet[Wallet_Team] = true;
    ...
    function _transfer(
        ...
        if (_isSniper[from])
            {
            require (amount <= _sniperSellLimit // Can only sell at most 50,000 tokens at a time
                    && block.timestamp > transferTime[from] + 7 days, // Can only transfer once every 7 days
                    "Account tagged as Sniper, sell amount and time limits in place."
            );
        ...
        if (_isTeamWallet[from])
            ...
            require (_isExcluded[to] // Can only sell or burn, cannot transfer to others
                    && amount <= _teamSellLimit // Can only sell at most 500K tokens at a time
                    && block.timestamp >= _teamSellStart // Can only sell after predetermined period
                    && block.timestamp >= transferTime[from] + 14 days, // Can only transfer twice a month
                    "Team Wallet has transer limitations."
            );
            transferTime[from] = block.timestamp;
        }
        ...
        if (_isEmploymentWallet[from]) 
            ...
            require (to == Wallet_Treasury || to == uniswapV2Pair // Can only sell or transfer to Treasury
                    ...
                    && block.timestamp > transferTime[from] + 14 days, // Can only sell twice a month
        ...
        if (_isDonationRecipient[to] && from != owner())
            ...
            require ((heldTokens + amount) <= _maxDonationRecipientHold 
                    ...
                    && block.timestamp > transferTime[to] + 24 hours, // Can only donate every 24 hours
                    "Donation Recipient Wallets can only receive 10K tokens from a holder every 24 hours. "
            );
        ...
        if (_isDonationRecipient[from] && to != owner()) 
            ...
            require ((amount == donationTransferMax && _isEmploymentWallet[to]) // Send 250K tokens to Employment Wallet
                    || (amount == donationTransferMin && to == uniswapV2Pair) // Sell 99K tokens at a time
                    || (amount == burnTransferAmt && to == _burnWallet), // Burn 1K tokens at a time
