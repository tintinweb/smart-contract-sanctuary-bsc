...
contract StakeContract is Ownable {
    ...
    event Rent(address indexed user, uint256 amount);
    ...
    //remove updatePool
    function rent(uint256 _amount) public {
        // require(endBlock > lastRewardBlock, "Staking pool ended");
        require(_amount >= minimum_deposit, "Cannot rent less than minimum");
        ...
        emit Rent(msg.sender, _amount);
    }
    
    //remove updatePool
    function withdraw(address _toAddr, uint256 _amount) public onlyOwner{
        // require(block.number > _unlockblock[msg.sender], "Unlock block not reached");
        require(_stakedBalance[_toAddr] > 0, "Account does not have a balance rented");
        require(_amount > 0, "Cannot withdraw Zero token");
        ...
        staketoken.safeTransfer(address(_toAddr), _amount);
        emit Withdraw(_toAddr, _amount);
    }
