...
contract Utils is OwnableUpgradeable, PausableUpgradeable  {
    ...
    function initialize() public initializer {
       randomNumber=10005500;
       max=99993992;
      __Ownable_init();
    }

function randomMulti(uint256 from, uint256 to) public view whenNotPaused  returns (uint256[] memory) {
        require(to > from, "Not correct input");
        uint256 [] memory randomValue =  new uint256[](4);
        randomValue[0] = random(from,to);
        randomValue[1] = random1(from,to);
        randomValue[2] = random2(from,to) ;
        randomValue[3] = random3(from,to) ;
        uint256[] memory result = new uint256[](4);
       result[0] = from + ((randomValue[0]+ max)% (to-from+1));
       result[1] = from + ((randomValue[1]+ max/2)  % (to-from+1));
       result[2] = from + ((randomValue[2]+ max-5) % (to-from+1));
       result[3] = from + ((randomValue[3]+ max-7) % (to-from+1));
        
        return result;
    }
    function random(uint256 from, uint256 to) public whenNotPaused view returns (uint256 number) {
        require(to > from, "Not correct input");
        uint256 tmp3  = uint(keccak256(abi.encodePacked(block.timestamp,randomNumber + max,msg.sender))) % max;
     ...
     function random1(uint256 from, uint256 to) public whenNotPaused view returns (uint256 number) {
        require(to > from, "Not correct input");
        uint256 tmp3  = uint(keccak256(abi.encodePacked(block.timestamp,randomNumber+randomNumber,msg.sender)))% max;
        uint256 tmp4= uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randomNumber))) % randomNumber;
        number = from + ((tmp3 + tmp4 )  % (to-from+1));
    }
    ...
    function random2(uint256 from, uint256 to) public whenNotPaused view returns (uint256 number) {
        require(to > from, "Not correct input");
        uint256 tmp3  = uint(keccak256(abi.encodePacked(block.timestamp,randomNumber,msg.sender))) % (max+randomNumber);
        uint256 tmp4= uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randomNumber))) % randomNumber;
        number = from + ((tmp3 + tmp4 )  % (to-from+1));
    }
