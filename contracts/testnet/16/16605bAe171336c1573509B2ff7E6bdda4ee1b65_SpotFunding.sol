    ...
    OwnableUpgradeable
{
    ...
    modifier onlyExistSpotManager(address _spotManager) {
        require(
    ...
    modifier onlyNotExistSpotManager(address _spotManager) {
        require(
            !_isExistSpotManager(_spotManager),
    ...
    function addSpotFundingAndImportAsset(address _spotManager)
        public
        override
        onlyNotExistSpotManager(_spotManager)
        onlyOwner
    {
        require(_spotManager != address(0), Errors.VL_EMPTY_ADDRESS);

        ISpotManager spotManager = ISpotManager(_spotManager);
        IERC20 _quoteAsset = spotManager.getQuoteAsset();
        IERC20 _baseAsset = spotManager.getBaseAsset();
    ...
    function addSpotFunding(
        ...
        address _quoteAsset,
        address _baseAsset
    ) public override onlyNotExistSpotManager(_spotManager) onlyOwner {
        require(
    ...
    function deposit(
        ...
        public
        override
        onlyExistSpotManager(_spotManager)
        onlyCounterParty
    ...
    function withdraw(
        ...
        Asset asset,
        uint256 amount
    ) public override onlyExistSpotManager(_spotManager) onlyCounterParty {
...
interface ISpotFunding {
    ...
    function addSpotFundingAndImportAsset(address _spotManager) external;
    ...
    function ownerDeposit(
        address _spotManager,
        SpotFundingStorage.Asset _asset,
        uint256 _amount
    ) external;
...
library TickPosition {
        ...
        internal
    {
        ...
        unchecked {
            ...
            if (
                self.orderQueue[index].size ==
                self.orderQueue[index].partialFilled
            ) index++;
