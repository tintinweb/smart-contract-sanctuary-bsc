...
// File: contracts/SafeAffinity.sol
...
contract SafeAffinity is IERC20, Context, Ownable {
    ...
    // debugging events
    event transferEvent(address recipient, uint256 amount);
    event transferFromEvent(address sender, address recipient, uint256 amount);
    event takeFeeEvent(address receiver, uint256 amount);
    event inSwapEvent(bool isSwap);
    event shouldTakeFeeEvent(address sender);
    event swapBackEvent();
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        emit shouldTakeFeeEvent(recipient);
    ...
    function takeFee(address receiver, uint256 amount) internal returns (uint256) {
        ...
        emit takeFeeEvent( receiver,  amount);
