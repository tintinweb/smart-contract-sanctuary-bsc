...
contract DogepadStaking {
    ...
    function unstake(uint256 index) public {
        ...
        if(block.timestamp< stakersRecord[msg.sender][index].staketime +Duration[stakersRecord[msg.sender][index].plan]){
            uint256 duration = block.timestamp - stakersRecord[msg.sender][index].staketime;
            claimableReward = duration *  stakersRecord[msg.sender][index].persecondreward;
        } else {
    ...
    function getStakeInfo(address _user, uint256 index) external view returns(
        uint256 unlockTime,
        uint256 amount, 
        uint256 reward
    ) {
        ...
        uint256 claimableReward;
        if(block.timestamp< stakersRecord[_user][index].staketime +Duration[stakersRecord[_user][index].plan]){
            uint256 duration = block.timestamp - stakersRecord[_user][index].staketime;
            claimableReward = duration *  stakersRecord[_user][index].persecondreward;
        } else {
            claimableReward = stakersRecord[_user][index].reward;
        }

        return (
            stakersRecord[_user][index].staketime + Duration[stakersRecord[_user][index].plan],
