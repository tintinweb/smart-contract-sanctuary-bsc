...
contract Escrow is AccessControl {
  ...
  mapping(address => mapping(uint256 => Deposit)) public deposit;
  ...
  function setPaymentToken(address _token) public onlyAdmin {
        paymentToken = IERC20(_token);
    }

  function makeDeposit(uint256 _amount) external returns (uint256) {
    IERC20 token = IERC20(paymentToken);
    countDeposits = countDeposits + 1;
    _grantRole(COBIE, msg.sender);
    
    token.safeTransferFrom(payable(msg.sender), address(this), _amount);
    totalBalance += _amount;
    deposit[msg.sender][countDeposits].amount = _amount;
    deposit[msg.sender][countDeposits].withdraw = true;
    deposit[msg.sender][countDeposits].locked = true;
  ...
  function releaseToken(address _tx_id) public payable onlyRole(COBIE) returns (bool) {
    if (balances[msg.sender][_tx_id].locked) {
      balances[msg.sender][_tx_id].locked = false;
    }
    // require(
    //   balances[msg.sender][_tx_id].locked == false,
    ...
    IERC20 token = IERC20(paymentToken);
    token.safeTransfer(balances[msg.sender][_tx_id].buyer, balances[msg.sender][_tx_id].amount);
    ...
    returns (bool)
  {
    ...
    token.transfer(
      balances[_seller][_tx_id].buyer,
      balances[msg.sender][_tx_id].amount
    );
