...
contract DCBNFTStaking is Initializable, OwnableUpgradeable, IStaking, IERC721ReceiverUpgradeable {
  ...
  event Stake(address indexed user, uint32[] indexed ids, uint256 time);
  ...
  function getPools() external view returns (PoolInfo[] memory pools) {
    ...
    for (uint256 i = 0; i < poolExt.length; i++) {
      ...
      pools[i].header = poolExt[i].header;
  ...
  function users(uint256 _pid, address _user) external view returns (User memory) {
    return (userExt[_pid][_user].common);
  }

  function poolInfo(uint256 _pid) external view returns (Pool memory) {
    return (poolExt[_pid].common);
  }
