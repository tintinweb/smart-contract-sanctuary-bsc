...
contract EVMStaking is Ownable {
    using SafeMath for uint256;
    ...
    IBEP20 public immutable evmToken;
    uint256 public lastRewardBlock;
    uint256 public accEVMPerShare;
    ...
    function depositReward(uint256 _amount) external onlyOwner {
        evmToken.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function updateStatus() private {
        ...
        if (_rewardBalance < reward) {
            accEVMPerShare = accEVMPerShare.add(_rewardBalance.mul(1e12).div(totalStakedAmount));
            _rewardBalance = 0;
        } else {
            _rewardBalance = _rewardBalance.sub(reward);
            accEVMPerShare = accEVMPerShare.add(reward.mul(1e12).div(totalStakedAmount));
        }
        lastRewardBlock = block.number;
    }
    ...
    function stake(uint256 _amount) public {
        ...
        updateStatus();
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(accEVMPerShare).div(1e12).sub(user.rewardDebt);
            user.pendingAmount = user.pendingAmount.add(pending);
        }
        evmToken.safeTransferFrom(msg.sender, address(this), _amount);
        totalStakedAmount = totalStakedAmount.add(_amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(accEVMPerShare).div(1e12);
    ...
    function unStake(uint256 _amount) public {
        ...
        updateStatus();
        uint256 pending = user.amount.mul(accEVMPerShare).div(1e12).sub(user.rewardDebt);
        if (evmToken.balanceOf(address(this)) < pending) {
            pending = evmToken.balanceOf(address(this));
        }
        user.pendingAmount = user.pendingAmount.add(pending);
        ...
        evmToken.safeTransfer(msg.sender, amount);
        evmToken.safeTransfer(feeWallet, unStakeFee);
    ...
    function harvest(bool reStake) public {
        ...
        if (reStake) {
            ...
            if (evmToken.balanceOf(address(this)) < amount) {
                amount = evmToken.balanceOf(address(this));
            }
            evmToken.safeTransfer(msg.sender, amount);

            if (evmToken.balanceOf(address(this)) < _harvestFee) {
                _harvestFee = evmToken.balanceOf(address(this));
            }
            evmToken.safeTransfer(feeWallet, _harvestFee);
