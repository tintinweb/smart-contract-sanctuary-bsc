...
contract SmartCashCapital is Context, Ownable {
    ...
    constructor(address dev, address part,uint256 launcTime) {
    ...
    function stakeStablecoins(uint256 amount, address ref) payable public {
    ...
    function withdrawDivs() public returns (uint256 withdrawAmount){
        User storage user = UsersKey[msg.sender];
        withdrawAmount = calcdiv(msg.sender);
    ...
    function withdrawInitial(uint256 key) public {
        User storage user = UsersKey[msg.sender];
    ...
    function calcdiv(address key) public view returns (uint256 totalWithdrawable) {
        User storage user = UsersKey[key];	
        ...
        for (uint256 i = 0; i < user.depoList.length; i++){	
            ...
            if (elapsedTime <= PercsKey[10].daysInSeconds){
                ...
                revert("Cannot calculate user dividends.");
            }
        }
    }

    function userInfo() view external returns (DepositList [] memory depoList){
        return(UsersKey[msg.sender].depoList);
    }
