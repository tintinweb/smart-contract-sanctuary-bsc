...
// using 0.6.6 to match imports

import "@uniswap/v2-core/contracts/interfaces/IUniswapV2Pair.sol";
import "@uniswap/lib/contracts/libraries/FixedPoint.sol";
import "../libraries/PancakeOracleLibrary.sol";
import "../libraries/PancakeLibrary.sol";
import "../interfaces/IPCSPriceOracle.sol";

/// @title PCSPriceOracle
...
contract PCSPriceOracle is IPCSPriceOracle {
    ...
    constructor(IUniswapV2Pair _pair) public {
        ...
        require(reserve0 != 0 && reserve1 != 0, "PCSPriceOracle: NO_RESERVES"); // ensure that there's liquidity in the pair
    }

    /**
    @notice updates current price average
    */
    function update() external override {
        ...
        require(timeElapsed >= PERIOD, "PCSPriceOracle: PERIOD_NOT_ELAPSED");
    ...
    @dev note this will always return 0 before update has been called successfully for the first time.
    */
    function consult(address token, uint256 amountIn)
        external
        view
        override
        returns (uint256 amountOut, uint32 updatedAt)
    {
        if (token == token0) {
            return (
                price0Average.mul(amountIn).decode144(),
                blockTimestampLast
            );
        } else {
            require(token == token1, "PCSPriceOracle: INVALID_TOKEN");
            return (
