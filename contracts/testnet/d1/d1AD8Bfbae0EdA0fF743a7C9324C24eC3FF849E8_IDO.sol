...
import "../token/WMCToken.sol";
import "../interface/INFTCard.sol";
...
contract IDO is Ownable, ReentrancyGuard {
    ...
    WMCToken public wmcToken;
    IERC20 public USDT;
    INFTCard public nftCard;
    IAccount public user;
    ...
    mapping(address => uint256) public inviteIdoAmount;
    mapping(address => uint256) public idoTime;
    ...
    constructor(
        ...
        wmcToken = WMCToken(_token);
        USDT = IERC20(_usdt);
        wallet = _wallet;

        price = 100 * 10**18;
        tokenAmount = 400 * 10**18;
        startTime = _startTime;
        endIme = _startTime.add(11 * 86400);
    }

    function setAccount(address _user) public onlyOwner {
        user = IAccount(_user);
    }

    function setCard(address _card) public onlyOwner {
        nftCard = INFTCard(_card);
    }
    ...
    function setPriceAmount(uint256 _amount) public onlyOwner {
        price = _amount;
    }
    ...
    function ido() external nonReentrant {
        ...
        require(user.getSuperior(msg.sender) != address(0), "not superior");
        require(!isUserIdo[msg.sender], "not ido");
        isUserIdo[msg.sender] = true;

        address superior = user.getSuperior(msg.sender);
        inviteIdoCount[superior] = inviteIdoCount[superior].add(1);
        inviteIdoAmount[superior] = inviteIdoAmount[superior].add(price);
        idoTime[msg.sender] = block.timestamp;

        USDT.transferFrom(msg.sender, wallet, price);
        ...
        if(inviteIdoCount[superior] >= 3) {
            nftCard.mint(superior);
        }
        wmcToken.mint(address(this), _value);
    }
    ...
    function withdraw() external nonReentrant {
        ...
        wmcToken.transfer(msg.sender, uint256(available));
    }
        ...
        returns (UserInfo[] memory)
    {
        ...
        if (page > 0) {
            ...
            address[] memory list = user.getInviteList(account);
            ...
            for (uint256 i = 0; i < size; i++) {
                ...
                info[i].amount = inviteIdoAmount[info[i].account];
                info[i].createTime = idoTime[info[i].account];
            }
        }
        return info;
    }
...
contract WMCToken is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function setReal(address _real) public onlyOwner {
        realAddr = _real;
    }
    ...
    modifier onlyMinner() {
        require(isMinner(msg.sender), "WMCToken: Only minner");
        _;
    }
    ...
    function _transfer(
        ...
        require(to != address(0), "WMCToken: zero address");
        require(balanceOf[from] >= amount, "WMCToken: balance not enough");
    ...
    function transferFee(address from, uint256 tokenAmount)
        internal
        returns (uint256)
    {
        uint256 returnAmountB = tokenAmount;
        if (fundAddr != address(0)) {
            uint256 fee = tokenAmount.mul(2).div(100);
            balanceOf[fundAddr] += fee;
            emit Transfer(from, fundAddr, fee);
            returnAmountB = returnAmountB.sub(fee);
        }
        if (realAddr != address(0)) {
            uint256 fee = tokenAmount.mul(3).div(100);
            balanceOf[realAddr] += fee;
            emit Transfer(from, realAddr, fee);
            returnAmountB = returnAmountB.sub(fee);
        }
        if (nodeAddr != address(0)) {
            uint256 fee = tokenAmount.mul(5).div(100);
            balanceOf[nodeAddr] += fee;
            emit Transfer(from, nodeAddr, fee);
            returnAmountB = returnAmountB.sub(fee);
        }
    ...
    function approve(address spender, uint256 amount)
        ...
        returns (bool)
    {
        require(spender != address(0), "WMCToken: zero address");
