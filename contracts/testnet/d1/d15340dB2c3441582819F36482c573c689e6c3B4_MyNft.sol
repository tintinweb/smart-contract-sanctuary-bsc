...
contract MyNft is ERC721Enumerable, Ownable {
    ...
    function allMaps() external view returns (uint[] memory) {
        uint[] memory temp = new uint[](19600);

        for (uint i = 0; i < 19600; i++) {
            temp[i] = myMap[i];
        }
    ...
    function allMapsTest() external view returns (uint[] memory) {
        uint[] memory temp2 = new uint[](3);
