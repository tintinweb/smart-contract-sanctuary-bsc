 ...
 */
    ...
    // function depositBUSDToLP(address user,uint amount,address lp) public onlyRole(BUSDTOLPCALL) returns(uint){
    //     IERC20(Admin.BUSD()).safeTransferFrom(msg.sender, address(this), amount);
    //     address[] memory path = new address[](2);
    //     path[0] = Admin.BUSD();
    //     path[1] = IUniswapV2Pair(lp).token0() == Admin.BUSD() ? IUniswapV2Pair(lp).token1() : IUniswapV2Pair(lp).token0();
    //     uint convertedToken = IUniswapV2Router02(router).getAmountsOut(amount, path)[1];
    //     if(IERC20(path[1]).balanceOf(address(this)) >= convertedToken){
    //     IERC20(path[0]).safeApprove(router, IERC20(path[0]).balanceOf(address(this)));
    //     IERC20(path[1]).safeApprove(router, IERC20(path[1]).balanceOf(address(this)));
    //     IUniswapV2Router02(router).addLiquidity(
    //         path[0],
    //         path[1],
    ...
    //     else{
    //         IERC20(Admin.BUSD()).safeApprove(router,IERC20(Admin.BUSD()).balanceOf(address(this)));
    //         uint _amount = IUniswapV2Router02(router).swapExactTokensForTokens((IERC20(Admin.BUSD()).balanceOf(address(this)) / 2),1,path,address(this),block.timestamp +1000)[path.length - 1];
    //         IERC20(path[0]).safeApprove(router, IERC20(path[0]).balanceOf(address(this)));
    //         IERC20(path[1]).safeApprove(router, IERC20(path[1]).balanceOf(address(this)));
    //         IUniswapV2Router02(router).addLiquidity(
    //         path[0],
    ...
    //     }
    //     IERC20(lp).safeTransfer(Admin.Treasury(), IUniswapV2Pair(lp).balanceOf(address(this)));
