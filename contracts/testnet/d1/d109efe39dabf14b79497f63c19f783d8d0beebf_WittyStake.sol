...
contract WittyStake {
    ...
    mapping (address => mapping(uint8 => bool))public incentiveEligible;
    ...
    function stake(uint _plan,uint _amount,address _mainRef,address[] memory _ref,uint[] memory _commissionAmt,uint _flag) public isLock isContractCheck(msg.sender) {
       ...
       uint count = investCount[msg.sender][_plan];
        ...
        if (_flag == 1) {
            ...
            incentiveEligible[msg.sender][users[msg.sender].incentiveCount] = true;
        }
        else if (_flag == 0) {
        ...
        users[msg.sender].totalStaked += depAmount[_plan];
        users[msg.sender].plan =_plan;
    ...
    function checkIncentive(address _user)public view returns(uint) {
        require(_user != address(0),"Invalid params");
        if (block.timestamp <= users[_user].creditTime.add(30 days)) {
            return users[_user].incentiveAmount;
        }
        else return 0;
    }
          
    function incentiveDistribute(address _user,uint amount)public onlyOwner {
        user storage _users = users[_user];
        require(_user!= address(0) && amount > 0,"invalid params");
        require(_users.incentiveCount == 0 || incentiveEligible[_user][_users.incentiveCount],"Not eligible");
        _users.incentiveAmount = _users.incentiveAmount.add(amount);
        _users.creditTime = block.timestamp;
        _users.incentiveCount++;
    }
