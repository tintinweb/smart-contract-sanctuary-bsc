...
contract BSC_Swap {
    ...
    constructor(address _feeReceiver1, address _feeReceiver2) public {
    ...
    function depositBySplit(uint256 _amount) external {
        require(_amount >= 50e6 && _amount.mod(50e6) == 0, "amount err");
    ...
    function distributePoolRewards() public {
        if(block.timestamp > lastDistribute.add(1 days)){
    ...
    function _calCurStaticRewards(address _user) private view returns(uint256, uint256) {
        ...
        uint256 splitAmt = totalRewards.mul(3000).div(10000);
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        require(_amount >= 50e6, "less than min");
        require(_amount.mod(50e6) == 0 && _amount >= 50e6, "mod err");
        ...
        uint256 addFreeze = (orderInfos[_user].length.div(2)).mul(1 days);
        if(addFreeze > 45 days){
            addFreeze = 45 days;
        }
        uint256 unfreezeTime = block.timestamp.add(15 days).add(addFreeze);
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(block.timestamp > order.unfreeze  && order.isUnfreezed == false && _amount >= order.amount){
                ...
                uint256 staticReward = order.amount.mul(150).mul(15 days).div(1 days).div(10000);
                if(isFreezeReward){
    ...
    function _distributeTopPool(uint256 _dayNow) private {
        ...
        for(uint256 i = 0; i < 3; i++){
            ...
            if(userAddr != address(0)){
                uint256 reward = topPool.mul(rates[i]).div(10000);
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        uint256 luck = _amount.mul(50).div(10000);
        luckPool = luckPool.add(luck);
        uint256 star = _amount.mul(30).div(10000);
        starPool = starPool.add(star);
        uint256 top = _amount.mul(20).div(10000);
            ...
            if(upline != address(0)){
                ...
                if(i > 0 && i < 5 && userInfo[upline].level > 3){
                    if(upRewards.level4Freezed > 0){
                        uint256 level4Reward = newAmount.mul(level4Percents[i - 1]).div(10000);
                ...
                if(i >= 5 && userInfo[upline].level > 4){
                    if(upRewards.level5Left > 0){
                        uint256 level5Reward = newAmount.mul(level5Percents[i - 5]).div(10000);
    ...
    function _setFreezeReward(uint256 _bal) private {
        for(uint256 i = balDown.length; i > 0; i--){
            if(balStatus[balDown[i - 1]]){
                uint256 maxDown = balDown[i - 1].mul(balDownRate[i - 1]).div(10000);
