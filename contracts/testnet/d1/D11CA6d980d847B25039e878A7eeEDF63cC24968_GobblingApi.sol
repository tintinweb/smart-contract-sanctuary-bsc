...
contract GobblingApi is Erc721API {
    ...
    function getGblLevels(IGobbling gbl, uint256[] memory tokenIds) public view returns (uint64[] memory) {
        uint64[] memory levels = new uint64[](tokenIds.length);
        for (uint idx = 0; idx < tokenIds.length; idx ++) {
            levels[idx] = gbl.tokenLevel(tokenIds[idx]);
        }
        return levels;
    }

    function getGblInfosWithIds(IGobbling gbl, uint256[] memory tokenIds) public view returns (IGobbling.GobblingInfo[] memory) {
...
contract Erc721API is API {
    ...
    function getTokenIds(ERC721Enumerable erc721, uint256 from, uint256 to) public view returns (uint256[] memory ids, uint256 total) {
    ...
    function getTokenIdsForOwner(ERC721Enumerable erc721, address user, uint256 from, uint256 to) public view returns (uint256[] memory ids, uint256 balance) {
    ...
    function getTokenURIs(ERC721Enumerable erc721, uint256[] calldata tokenIds) public view returns (string[] memory list) {
    ...
    function Token721Enumerate(ERC721Enumerable erc721, uint256 from, uint256 to) public view returns (Token721[] memory tokenList, uint256 total) {
        total = erc721.totalSupply();
        if (total < to) to = total;
        if (from >= to) {
    ...
    function Token721EnumerateForOwner(ERC721Enumerable erc721, address owner, uint256 from, uint256 to) public view returns (Token721[] memory tokenList, uint256 balance) {
...
interface IGobbling is IERC721Enumerable, IERC4907 {
    ...
    function updateLevel(uint256 tokenId, uint64 level, string memory newUri) external;
    function updateLevels(uint256[] memory tokenIds, uint64[] memory levels, string[] memory newUris) external;
