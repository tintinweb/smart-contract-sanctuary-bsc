...
contract RewardsWinningFound is Ownable, ReentrancyGuard {
    ...
    uint256 public hourlyBlockQuantity = 1800;
    ...
    function updateAddresses() external onlyOwner {
        ...
        pvpRewardAddress = gameAddresses.getRewardsPvPAddress();
        rankingRewardAddress = gameAddresses.getChallengeRewardsAddress();
    }

    // balancer function, is necessary to balancer pools if a pool drop to much
    // only can use it after 6 months lock
    function balancerToPvpReward() external onlyOwner {
        require(pvpRewardAddress != address(0x0),"Not setted");
        require(block.timestamp >= unlockBalancerTimestamp, "too soon");
        require(BZAI.transfer(pvpRewardAddress, BZAI.balanceOf(address(this)) / 10));
    }

    function balancerToRankingReward() external onlyOwner {
        require(rankingRewardAddress != address(0x0),"Not setted");
        require(block.timestamp >= unlockBalancerTimestamp, "too soon");
        require(BZAI.transfer(rankingRewardAddress, BZAI.balanceOf(address(this)) / 10));
    }

    function setHourlyBlockQuantity(uint256 _blocks) external onlyOwner {
        hourlyBlockQuantity = _blocks;
    }
    ...
    function setBonusMult(uint256 _bonus) external onlyOwner {
        require(_bonus >= 10 && _bonus <= 12, "bonus multiplicator not match");
        bonusMult = _bonus;
    }
    ...
    {
        if (!reserveFinished) {
            if (block.number >= (lastBlockUpdated + hourlyBlockQuantity)) {
                lastBlockUpdated = block.number;
                reserveFinished = IReserve.updateRewards();
            }
        }
        ...
        if(bonus){
            _toSend = _toSend * bonusMult / 10;
        }

        require(BZAI.transfer(gameAddresses.getPaymentsAddress(), _toSend));
        return _toSend;
    }
}
