...
contract SassmetaItem1155 is Ownable, ERC1155Enumerable {
    using Address for address;
    using Strings for uint256;


    event MergeNFT(address indexed renter, address indexed user, uint256 id, uint256[] tokenId);
    ...
    constructor() ERC1155("https://www.sina.com/") {
        _name = "YUANHUA META";
        _symbol = "YHN";
        myBaseURI = "https://www.baidu.com/";
    }
    ...
    function mintBatch2(address to_, uint256  startId_, uint256 amount_) public returns (bool) {
        ...
        uint256[] memory ids_ = new uint256[](amount_);

        uint256[] memory amounts_ = new uint256[](amount_);

        for (uint i = 0; i < amount_; i++) {
            
            id = startId_++;
            itemInfoes[id].tokenId = id;

            itemInfoes[id].currentAmount += 1;
    ...
    function burn(address account, uint256 id, uint256 value) public override {
        require(burners[_msgSender()], "S: not burner's calling");
    ...
    function safeBatchTransferFrom2(
        address from,
        address to,
        uint256[] memory ids,
        uint256[] memory amounts
    ...
    function mergeNFT(address renter, address user, uint256 id, uint256[] memory tokenIds) public {
        ...
        for(uint256 i = 0; i < 10 ; i++){
            ...
            if(tokenIds[i] != 0){
                ...
                require(balanceOf(user, tokenIds[i]) > 0, "User does not own the fragment");
        ...
        // super._setApprovalForAll(user, msg.sender, true);

        super._burnBatch(user, tokenIds, amounts);

        // _mint(user, id, 1, "");

        safeTransferFrom(renter,user,id,1,"");

        // super._setApprovalForAll(user, msg.sender, false);

        emit MergeNFT(renter, user, id, tokenIds);
    }
