...
import "../../externalContract/openzeppelin/IERC20Metadata.sol";
import "../../externalContract/openzeppelin/AggregatorV2V3Interface.sol";
import "hardhat/console.sol";

contract PriceFeeds_BSC is Ownable {
    ...
    uint256 WEI_PERCENT_PRECISION = 10**20;
    ...
    function checkPriceDisagreement(
        ...
        rate = (rate * WEI_PRECISION) / precision;
        ...
        if (spreadValue != 0) {
            spreadValue = (spreadValue * WEI_PERCENT_PRECISION) / sourceToDestSwapRate;

            require(spreadValue <= maxSlippage, "PriceFeed/price-disagreement");
        }
    }
    ...
    function getFastGasPrice(address payToken) external view returns (uint256) {
        uint256 gasPrice = _getFastGasPrice() * WEI_PRECISION * WEI_PRECISION;
        if (payToken != wethTokenAddress && payToken != address(0)) {
            require(!globalPricingPaused, "PriceFeed/pricing-is-paused");
            (uint256 rate, uint256 precision) = _queryRate(wethTokenAddress, payToken);
            gasPrice = (gasPrice * rate) / precision;
        }
        return gasPrice;
    }
    ...
    function setPriceFeed(address[] calldata tokens, address[] calldata feeds) external onlyOwner {
    ...
    function setDecimals(IERC20Metadata[] calldata tokens) external onlyOwner {
        for (uint256 i = 0; i < tokens.length; i++) {
    ...
    function _queryRateCall(address token) internal view returns (uint256 rate) {
        ...
        require(rate != 0 && (rate >> 128) == 0, "PriceFeed/price-error");
    }
    ...
    function _getFastGasPrice() internal pure returns (uint256 gasPrice) {
        return 10e9;
    }
}
