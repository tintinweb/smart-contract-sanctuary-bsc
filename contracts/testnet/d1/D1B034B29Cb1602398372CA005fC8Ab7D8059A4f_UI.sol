...
contract UI is AccessControl, ReentrancyGuard, Pausable {
    ...
    function NFTaddAirdrops(
        address[] calldata _accounts,
        uint256[] calldata _values
    ) external whenNotPaused nonReentrant onlyRole(MARKETING) {
        INFT(nft).addAirdrops(_accounts, _values);
    }

    function NFTaddWhiteList(
        address[] calldata _accounts,
        uint256[] calldata _values
    ) external whenNotPaused nonReentrant onlyRole(MARKETING) {
    ...
    function NFTsetPaused(bool _paused)
        external
        whenNotPaused
        nonReentrant
        onlyRole(ADMIN)
    ...
    function NFTsetMINT_PRICE(uint256 _price)
        external
        whenNotPaused
        nonReentrant
        onlyRole(ADMIN)
    ...
    function NFTsetWhiteList(bool _OnlyWhiteList)
        external
        whenNotPaused
        nonReentrant
        onlyRole(ADMIN)
    ...
    function STACKINGsetPaused(bool _paused)
        external
        whenNotPaused
        nonReentrant
        onlyRole(ADMIN)
    ...
    function STACKINGsetMINIMUM_TO_EXIT(uint256 _MINIMUM_TO_EXIT)
        external
        whenNotPaused
        nonReentrant
        onlyRole(ADMIN)
    ...
    function STACKINGsetDAILY_token_RATE(uint256 _DAILY_token_RATE)
        external
        whenNotPaused
        nonReentrant
        onlyRole(ADMIN)
    ...
    function STACKINGsetToken_CLAIM_TAX_PERCENTAGE(
        uint256 _token_CLAIM_TAX_PERCENTAGE
    ) external whenNotPaused nonReentrant onlyRole(ADMIN) {
        IStaking(staking).setToken_CLAIM_TAX_PERCENTAGE(
            _token_CLAIM_TAX_PERCENTAGE
        );
    }

    function STACKINGsetProbabilityToLoseEvryThing(
        uint256[] calldata _probabilityToLoseEvryThing
    ) external whenNotPaused nonReentrant onlyRole(ADMIN) {
        IStaking(staking).setProbabilityToLoseEvryThing(
            _probabilityToLoseEvryThing
        );
    }

    function STACKINGsetClaiming(bool _canClaim)
        external
        whenNotPaused
        nonReentrant
        onlyRole(ADMIN)
