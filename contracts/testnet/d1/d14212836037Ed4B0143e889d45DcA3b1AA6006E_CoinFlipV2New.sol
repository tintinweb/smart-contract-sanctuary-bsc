  ...
  ReentrancyGuardUpgradeable
{
  ...
  function betByToken(
    ...
    feeBalances[_coinAddress] += getBetFee(_betAmount);

    uint256 totalAmount = getBetAmountWithFee(_betAmount);

    IERC20Upgradeable(_coinAddress).transferFrom(
      msg.sender,
      address(this),
      totalAmount
    );
  ...
  function getBetAmountWithFee(uint256 _betAmount)
    public
    view
    returns (uint256)
  {
  ...
  function withdrawFee(
      ...
      address _receiver,
      uint64 _rate
  ) external onlyOwner nonReentrant rateInputCondition(_rate) {
    require(
      getContractBalanceByToken(_coinAddress) > 0 &&
        getContractBalanceByToken(_coinAddress) >=
        feeBalances[_coinAddress],
      "NeloCoinFlip: You don't have enough balances to withdraw."
    );

    uint256 amount = feeBalances[_coinAddress].mul(_rate).div(1e8);
  ...
  function withdrawPlayerBalance(address _coinAddress, uint256 _amount) external nonReentrant {
    require(playersByAddress[msg.sender].balances[_coinAddress] > 0, "NeloCoinFlip: You don't have any fund to withdraw.");
  ...
  function payoutToken(address _coinAddress, uint256 _amount)
    external
    nonReentrant
    onlyOwner
  {
    IERC20Upgradeable(_coinAddress).safeTransfer(
      address(msg.sender),
      _amount
    );
  }
...
abstract contract VRFConsumerBaseV2Upgradable {
    ...
    function __initializeVRFConsumerBase(address _vrfCoordinator) internal {
