...
contract StakingStream is ERC20, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256[]) public usersStakings;

    /// @dev Events
    event Staked(uint256 stakeId);
    event Unstaked(uint256 stakeId, uint128 amount);
    ...
    function stake(StakeData memory _stakeData) external nonReentrant {
        ...
        uint128 profitValue = calculateProfitValue(_stakeData.amount, _stakeData.percentage);
        uint128 mintAmount = _stakeData.amount+profitValue;
        _mint(msg.sender, mintAmount);
        totalShares += mintAmount;
        usersStakings[msg.sender].push(_stakingId);
     ...
     */
    function calculateProfitValue(uint128 total, uint128 percent) pure private returns(uint128) {
        uint128 division = total * percent;
        uint128 percentValue = division / 10000;
        return percentValue;
    }
