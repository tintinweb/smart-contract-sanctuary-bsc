...
contract Presale is Ownable {
    ...
    struct UserInfo {
        ...
        uint256[] earned; //earn calculate earch buy
        uint256[] refRaised; //total ref raised
        RefInfo[] refLogs; //log ref of him
        uint256 lastClaim; //time
        uint256[] withdrawed; //token withdraw
    }

    mapping(address => uint) private _luckynumber;
    //usser balance
    ...
    //percent add if whitelist
    uint256 private denominator = 10000;

    //Claim:
    address public rewardWallet;
    uint256 public releaseTime = 1600000000;
    uint256 public apr = 150;
    mapping(uint256 => VestingInfo) private _vesting;
    ...
    function importBalances(uint pid, bytes[] calldata amounts) external onlyOwner{
        address buyer;
        uint amount;
        for(uint i=0; i< amounts.length; i++){
            (buyer, amount) = abi.decode(amounts[i], (address, uint));
            //totalHold = totalHold.sub(wallets[buyer].amount).add(amount);
            _balances[buyer].locked[pid] = amount;
            _balances[buyer].withdrawed[pid] = 0;
            _balances[buyer].lastClaim = 0;
        }
    }
    ...
    function setToken(address xtoken) external onlyOwner {
        require(xtoken != address(0), "token is 0");
        _token = IERC20(xtoken);
    }
    ...
    function balanceOf(uint pid, address account) public view returns (uint256) {
        return _balances[account].locked[pid] - _balances[account].withdrawed[pid];
    }
    ...
    // function _preValidatePurchase(uint256 pid, address beneficiary, uint256 weiAmount) internal view virtual onlySaleOpen(pid) {
    ...
    // }
    event Debug(address indexed addr, uint256 weiAmount);
    function _preValidatePurchase(uint256 pid, address beneficiary, uint256 weiAmount) internal virtual onlySaleOpen(pid) {
        ...
        emit Debug(beneficiary, weiAmount);
    }
    ...
    function buyToken(uint256 pid, address refaddr, uint ln) public payable {
        ...
        _luckynumber[msg.sender] = ln;
        //POOL
        uint256 tokenAmount = weiAmount.mul(_pools[pid].price);
        _pools[pid].raised = _pools[pid].raised.add(weiAmount);
        _pools[pid].locked = _pools[pid].locked.add(tokenAmount);
        //USER
        _balances[beneficiary].locked[pid] = _balances[beneficiary].locked[pid].add(tokenAmount);
        //REF USER:
        if (refaddr != address(0)) {

            //total ref:
            _balances[refaddr].refRaised[pid] = _balances[refaddr].refRaised[pid].add(weiAmount);
            _balances[refaddr].refLogs.push(RefInfo(pid, msg.sender, weiAmount));
            _balances[refaddr].earned[pid] = _balances[refaddr].refRaised[pid].mul(_pools[pid].price).mul(_pools[pid].earnReferral).div(denominator);
        }
        emit BuyToken(beneficiary, weiAmount, tokenAmount, pid, _pools[pid].locked, _pools[pid].price, _pools[pid].hardCap);
    }

    //Claim locked token:
    function amountCanClaim(uint pid, address holder) public view returns(uint){
        if(_balances[holder].locked[pid] == 0){
            return 0;
        }
        //before release time
        ...
        for (uint256 i = 0; i < vestingLength - 1; i++) {
            if(block.timestamp >= releaseTime.add(_vesting[i].openingTime) && block.timestamp < releaseTime.add(_vesting[i + 1].openingTime)){
                return _balances[holder].locked[pid].mul(_vesting[i].percent).div(denominator).sub(_balances[holder].withdrawed[pid]);
            }
        }
        // release all
        if(block.timestamp >= releaseTime.add(_vesting[vestingLength - 1].openingTime)){
            return _balances[holder].locked[pid].sub(_balances[holder].withdrawed[pid]);
        }
        return 0;
    }

    function rewardAmount(uint pid, address holder) public view returns(uint){
        uint256 amount = _balances[holder].locked[pid].sub(_balances[holder].withdrawed[pid]);
        if(amount == 0){
            return 0;
        }
        uint timeElapsed;
        if(_balances[holder].lastClaim != 0){
            timeElapsed = block.timestamp - _balances[holder].lastClaim;
        }else{
    ...
    function claim(uint pid) external{
        ...
        require(_balances[msg.sender].locked[pid] > 0, "Your balance is 0");
        uint256 amount = amountCanClaim(pid, msg.sender);
        if(amount > 0){
            require(_token.balanceOf(address(this)) >= amount, "Not enough token");
            // reward stake
            uint reward = rewardAmount(pid, msg.sender);
            if(reward > 0){
                _token.transferFrom(rewardWallet, msg.sender, reward);
                _balances[msg.sender].lastClaim = block.timestamp;
            }
            // release
            _token.transfer(msg.sender, amount);
            //totalHold = totalHold.sub(amount);
            _balances[msg.sender].withdrawed[pid] = _balances[msg.sender].withdrawed[pid].add(amount);
            emit ClaimToken(msg.sender, amount);
        }
    }

    function claimRef(uint pid) external{
        require(block.timestamp >= releaseTime, "Wait for release time");
        require(_balances[msg.sender].locked[pid] > 0, "Your balance is 0");
        uint256 amount = amountCanClaim(pid, msg.sender);
        if(amount > 0){
    ...
    function luckyNumber(address _wallet) public view returns(uint) {
        return _luckynumber[_wallet];
    }
