...
contract YHToken is Ownable, ERC20 {
    ...
    address private _walletD;
    address private _walletE;
    address private _walletBurn;
    address _walletConsensusFunds;
    address _walletSuperNode;
    // liquidity
    ...
    modifier onlyOperatable() {
        require(
            (msg.sender == owner() || operators[msg.sender]),
            "[YHTOKEN] You are not an operator"
        );
        _;
    }
    ...
    function init() public onlyOwner {
        setupLP();
        _totalBuyFee = 800; // buy fee  : 6%
        _totalSellFee = 800; // sell fee : 15%
        _feeRate = 10000;
        numTokensToAddToLiquidity = 1*(10**_decimals); // 100 ether
        ...
        _walletD = 0x2659A32AAbc44256337b324041969f06b943b477;
        _walletE = 0x7DEfe04FEEd55f771226627f1c7550Ca89742D91;
        _walletBurn = 0x000000000000000000000000000000000000dEaD;

        _walletConsensusFunds = 0x85048aae2FCc6877cA379e2dfDD61ea208Fa076C;
        _walletSuperNode = 0x85048aae2FCc6877cA379e2dfDD61ea208Fa076C;
    ...
    constructor(
        ...
        _decimals = d_;
        uint256 oneEther = 10**d_;
        _mint(owner(), 10000000000 * oneEther);
        init();
    }
    ...
    function setupLP() public onlyOwner {
        ...
        if (block.chainid == 56) {
          ...
          usdt = IERC20(0xdADdAfF5b17e6d9731BeE20bf51B7Cd835801282);
        }
    ...
    function setTradeEnable(bool flag) public onlyOperatable {
        tradeEnable = flag;
    }
    ...
    function _transferWithBuyFee(
        ...
        uint256 amountA = (feeAmount * 300) / _totalBuyFee;
        uint256 amountB = (feeAmount * 200) / _totalBuyFee;
        uint256 amountC = (feeAmount * 150) / _totalBuyFee;
        uint256 amountD = (feeAmount * 100) / _totalBuyFee;
        uint256 amountE = (feeAmount * 50) / _totalBuyFee;

        _transfer(from, _walletA, amountA);
        _transfer(from, _walletB, amountB);
        _transfer(from, _walletC, amountC);
        _transfer(from, _walletD, amountD);
        _transfer(from, _walletE, amountE);
        _transfer(from, to, tAmount);
    }

    function _transferWithSellFee(
        address from,
        address to,
        uint256 amount
    ) internal returns (bool) {
        ...
        uint256 tAmount = amount - feeAmount;
        uint256 amountConsensus = (feeAmount * 200) / _totalSellFee;
        uint256 amountBurn = (feeAmount * 200) / _totalSellFee;
        uint256 amountSuperNode = (feeAmount * 200) / _totalSellFee;
        uint256 amountLP = (feeAmount * 200) / _totalSellFee;

        _transfer(from, _walletConsensusFunds, amountConsensus);
        _transfer(from, _walletBurn, amountBurn);
        _transfer(from, _walletSuperNode, amountSuperNode);
        _transfer(from, address(this), amountLP);
    ...
    function _transferFrom(
        ...
        if (from == pair || to == pair) { // trading
            require(
                (tradeEnable == true ||
                    operators[from] ||
                    operators[to]),
                "[YHTOKEN] Not allowed to trade now!"
            );
        }

        // auto liquidity
        if (!inSwap && from != pair) autoLP();
