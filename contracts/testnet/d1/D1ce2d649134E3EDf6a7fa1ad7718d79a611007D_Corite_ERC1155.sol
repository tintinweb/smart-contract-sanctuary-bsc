...
contract Corite_ERC1155 is ERC1155Supply, AccessControl{
    ...
    function createCampaign(address _owner, uint _supplyCap, uint _toBackersCap) external isCREATE_CLOSE_HANDLER() returns(uint){
        require(_supplyCap > 0 && _toBackersCap > 0, "Cannot create campaign with 0 shares or 0 allowed to sell");
        require(_supplyCap >= _toBackersCap , "Supply much be greater or equal to max allowed");
