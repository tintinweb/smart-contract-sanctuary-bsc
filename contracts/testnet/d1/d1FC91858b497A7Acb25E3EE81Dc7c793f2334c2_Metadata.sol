...
contract Metadata{
    ...
    uint256 public maintenanceCost =  10 * 10 ** 18; // Price in busd
    ...
    constructor(Busd _busdAddress, ArmyNFT _armynftAddress){
        owner = msg.sender;
    ...
    function setCost(uint _busdCost) external{
        require(msg.sender == owner, 'You must be the owner to run this.');
        maintenanceCost = _busdCost;
    }
    ...
    function maintenanceNFT(address _address, uint _collection, uint _busdAmount, uint256 _tokenId) external {
        require(_collection >= 1 && _collection <= 3, 'Invalid Collection type');
        require(_busdAmount >= maintenanceCost, 'You must provide enough BUSD for the Maintenance NFT');
