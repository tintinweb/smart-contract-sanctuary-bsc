...
contract BC420 is Ownable,IERC20 {
    ...
    function approve(address spender, uint256 amount) public override returns (bool success) {
        require(balances[tx.origin]>= amount && amount > 0,"Balances is not enough"); // Check if the sender has enough
    ...
    function transferFrom(address _spenderOwner, address _recipient, uint256 _amount) public override returns (bool) {
        require(allowed[_spenderOwner][tx.origin] >= _amount,"Amount is more then approve amount"); // Check allowance amount
        ...
        _approve(_spenderOwner,tx.origin,allowed[_spenderOwner][tx.origin].sub(_amount,"ERC20: transfer amount exceeds allowance"));
