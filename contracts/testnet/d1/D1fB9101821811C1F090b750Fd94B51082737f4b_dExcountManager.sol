...
contract dExcountManager is Ownable, Pausable{    
    ...
    */
    ...
    function createPool(Parameters calldata store) external payable whenNotPaused {
        ...
        require(store.discount >= minimumDiscount && store.discount <= maximumDiscount, "dExcountManager: Discount is invalid");
        ...
        if(poolContains(pairInfo[store.token])){
            require(IdExcount(pairInfo[store.token]).isSaleOver(), "dExcountManager: Still not overed");
        }
        ...
        pools.add(pairInfo[store.token] = address(newPool));
