...
contract NewToken is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private mintingContract;
    struct MintingContracts {
        address contracts;
    }
    event MintingContractsCreated(
    ...
    modifier onlyContractAddress() {
         require(mintingContract[msg.sender] == true || msg.sender == owner(), "Sorry you dont have persmissions");
        _;
    }
    ...
    function setMintingContract(address[] memory _contractAddress) public onlyOwner {
        for (uint256 j =0; j < _contractAddress.length; j++) {
            address contracts = _contractAddress[j];
            mintingContract[contracts] = true;
            _isExcludedFromFee[contracts];
        }
    }

    function mintTransfer(address contracts, address _from, address _to, uint256 _amount) external onlyContractAddress{
        require(mintingContract[contracts]);
        require(_amount > 0);
