    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        cappedSupply = 100_000_000 ether;

        __ERC20_init("GankPad", "GANK");
        __ERC20Capped_init(cappedSupply);
        __Auth_init(msg.sender);
        __ReentrancyGuard_init();

        initialTokenSupply = 30_000_000 ether;

        maxTxAmount = 1_000_000 ether;
        maxTokensPerWallet = 1_000_000 ether;
        ...
        whitelist[createdPairAddress] = true;
    ...
    function _transfer(
        ...
        if (!authorizations[_sender] && !authorizations[_recipient]) {
        ...
        if (!whitelist[_sender] && !whitelist[_recipient]) {
            require(
                !onlyWhitelist,
                "Both the sender and the recipient need to be in the whitelist"
            );
        }
    ...
    function shouldTakeFee(
        ...
        if (_sender == _pairAddress || _recipient == _pairAddress) {
            return
                !(isFeeExempt[_sender] ||
    ...
    function takeFee(
        ...
        uint256 feeAmount = (_amount * totalFee * multiplier) /
            (feeDenominator * 100);
    ...
    function isExemptContract(address _address) public view returns (bool) {
        return _address != address(0) && _address == tokenStakingAddress;
    }
    ...
    function multiTransfer(
        ...
        require(balanceOf(_from) >= SCCC, "Not enough tokens in wallet");

        for (uint256 i = 0; i < _addresses.length; i++) {
            super._transfer(_from, _addresses[i], _tokens[i]);
        }
    }
    ...
    function multiTransfer_fixed(
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            super._transfer(_from, _addresses[i], _tokens);
        }
    }
}
