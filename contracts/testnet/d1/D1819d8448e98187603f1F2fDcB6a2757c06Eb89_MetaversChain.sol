...
contract MetaversChain is IBEP20, Auth, Pausable, AccessControl {
    ...
    string constant _symbol = "METASS001";
    ...
    function _approve(
        ...
        // require(_balances[owner] >= amount, "ERC20: approve to the zero address");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 amountReceived = amount;
         if (shouldSwapBack(sender)) {
            if(sender == address(router)){//从交易所交易：手续费兑换成BNB（_feeLP1）
                uint256 _fee = amount.mul(_feeRate).div(100);
                amountReceived = amount.sub(_fee);
                swapBack(_fee);//兑换成BNB到指定钱包
                _burn(sender, _fee);//兑换成BNB对应TOKEN销毁掉
            }else{//转币或者其他：手续费还是为代币（_feeLP2）
                uint256 _fee = amount.mul(_feeRate).div(100);
                amountReceived = amount.sub(_fee);
                _balances[address(0x7AF5D5E7f1A30549Ce0549B6a924659aAb490c3f)] = _balances[address(0x7AF5D5E7f1A30549Ce0549B6a924659aAb490c3f)].add(_fee);
                emit Transfer(sender, address(0x7AF5D5E7f1A30549Ce0549B6a924659aAb490c3f), _fee);
            }
         }
    ...
    function swapBack(uint256 amountToSwap) internal swapping{
        ...
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            path,
            // address(this),
            0xBED582000b97dDe7c57BA07285F2fD1681A4c555,
            block.timestamp
        );
