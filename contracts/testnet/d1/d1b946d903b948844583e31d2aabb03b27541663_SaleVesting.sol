...
contract SaleVesting is Ownable {
	...
	/* the duration of the Sale vesting */
	...
	/* the whitelist of the Sale vesting */
	...
	function _vestedAmount(address addr) private view returns (uint256) {
		...
		// vesting during 8 months
		} else {
			uint256 initialAmount = balanceOfINERY[addr].mul(20).div(100); // 20% after TGE time
			uint256 monthAmount = balanceOfINERY[addr].mul(10).div(100); // monthly 10% Release
			uint256 monthCount = block.timestamp.sub(tgeTime).div(30 days);
			return initialAmount.add(monthAmount.mul(monthCount));
		}
	}
