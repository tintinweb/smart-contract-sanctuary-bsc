...
contract Token is TokenData, IToken {
	...
	function getAvailableToken(address token) external onlyAccepted returns (TokenInfo memory tokeninfo) {
		require(_isAvailableToken(token), "Token: not supported or available");
		tokeninfo = _getToken(token);
	}

	// Dashboard view
	function getAvailableToken() external onlyAccepted returns (TokenInfo[] memory tokeninfo) {
	...
	function tokenDeposit(address token, uint256 amount) external onlyAccepted {
	...
	function tokenWithdraw(address token, uint256 amount) external onlyAccepted {
		require(amount > 0, "Token: amount need more than 0");
		require(_isAvailableToken(token), "Token: not available");
	...
	function addUpdateToken(ERC20 token, uint8 status) external onlyAccepted {
	...
	function getToken4Monitoring() external onlyAccepted returns (TokenInfo[] memory tokeninfos) {
		require(officer.isMonitoring(msg.sender), "Token: caller is not Monitoring");
		tokeninfos = _getToken();
		//...
	}

	function getToken4Bridging() external onlyAccepted returns (TokenInfo[] memory tokeninfos) {
