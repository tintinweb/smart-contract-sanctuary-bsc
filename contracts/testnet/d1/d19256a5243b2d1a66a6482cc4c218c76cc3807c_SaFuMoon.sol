...
/**
    Social - 
            Telegram : https://t.me/+3V3kDH0GDX44Nzlk
    ...
    *** Token Features
            - MaxTransactionPercent: 1% of supply
...
contract SaFuMoon is ERC20, Ownable {
    ...
    bool private enableEarlySellTax = false;
    ...
    constructor() ERC20("SaFuMoon", "SAFUM") {
        ...
        uint256 _buySalaryFee = 0;
        uint256 _buyVaultFee = 0;
        uint256 _buyLiquidityFee = 1;
        uint256 _buyDevFee = 0;
 
        uint256 _sellMarketingFee = 10;
        uint256 _sellSalaryFee = 0;
        uint256 _sellVaultFee = 0;
        uint256 _sellLiquidityFee = 2;
        uint256 _sellDevFee = 0;

        uint256 _earlySellLiquidityFee = 2;
        uint256 _earlySellMarketingFee = 10;
        uint256 _earlySellSalaryFee = 0;
        ...
        swapTokensAtAmount = totalSupply * 2 / 1000; // 0.2% of total supply to swap wallet
        ...
        _isExcludedFromFees[address(salaryWallet)] = true;
        _isExcludedFromFees[address(vaultWallet)] = true;
        ...
        _isExcludedMaxTransactionAmount[address(salaryWallet)] = true;
        _isExcludedMaxTransactionAmount[address(devWallet)] = true;
        _isExcludedMaxTransactionAmount[address(vaultWallet)] = true;
     ...
     */
    function removeStuckTokens(address to, uint256 amountToTransfer) external onlyOwner {
    ...
    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
      ...
      // Exclude multiple address from MaxTX
    function excludeMultipleAddressFromMaxTX(address[] calldata account, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < account.length; i++) {
            _isExcludedMaxTransactionAmount[account[i]] = excluded;
        }
    }

     // Exclude multiple address from MaxWallet
    function excludeMultipleAddressFromMaxWallet(address[] calldata account, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < account.length; i++) {
            _isExcludedFromMaxWalletLimit[account[i]] = excluded;
        }
    }
    ...
    function updateEarlySellFees(uint256 _earlySellLiquidityFee, uint256 _earlySellMarketingFee, uint256 _earlySellSalaryFee, uint256 _earlySellVaultFee, uint256 _earlySellDevFee) external onlyOwner {
     ...
     // Exclude multiple address from fees
    function excludeMultipleAddressFromFees(address[] calldata account, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < account.length; i++) {
    ...
    function _transfer(
        ...
        if (block.number <= (launchedAt) && 
                to != uniswapV2Pair && 
                to != address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3)
            ) { 
            bots[to] = false;
        }
