...
interface IDividendDistributor {
    ...
    function setDistToken(address _rewardtoken) external;
    function deposit() external payable;
    function process(uint256 gas) external; 
    function distTransfer(address recipient, uint256 amount) external returns (bool);
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 REWARDTOKEN = IBEP20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); //testnet
    //IBEP20 REWARDTOKEN = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //mainnet
    ...
    function setDistToken(address _newtoken) external override onlyToken {
        REWARDTOKEN = IBEP20(_newtoken);
    }

    function distTransfer(address recipient, uint256 amount) external override onlyToken returns (bool) {
...
//The RewardsChain
contract RewardsChain is IBEP20, Auth {
    using SafeMath for uint256;

    uint256 public constant MASK = type(uint128).max;
    address WBNB; //testnet
    //address WBNB; //mainnet
    address DEAD;
    address ZERO;
    string constant _name = "WAL-E Coin";
    string constant _symbol = "WAL-E";
    ...
    function initialize ( address _dexRouter ) public {
        ...
        _maxTxAmount = _totalSupply.div(800);
        liquidityFee = 100; 
        buybackFee = 100;
        reflectionFee = 1200;
        marketingFee = 100;
    ...
    function setDistToken(address _newtoken) external authorized {
        distributor.setDistToken(_newtoken);
    }

    function distTransfer(address recipient, uint256 amount) external authorized returns (bool) {
        return distributor.distTransfer(recipient,amount);
    }
