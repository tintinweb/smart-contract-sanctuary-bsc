...
interface IMarket {
    ...
    function getIsListed(uint256 _tokenId) external view returns (bool);
}
...
contract BCNFTStaking is Context, Ownable {
    ...
    mapping (address => uint256[]) public tokensForStaked;
    ...
    function stake(uint256 _period) external {
        ...
        uint256 tokenAmount = getTokenCount(msg.sender);
        uint256[] memory _stakedIds = new uint256[](tokenAmount);
        uint256[] memory ownIds = iBCNFT.getTokensOfOwner(msg.sender);
        for(uint256 i = 0; i < ownIds.length; i++) {
            bool _checked = checkListed(ownIds[i]);
            if(!_checked) {
                _stakedIds[i] = ownIds[i];
                iBCNFT.transferFrom(msg.sender, address(this), ownIds[i]);
            }
        }
        require(_stakedIds.length > 1, "Your NFTs can not be staked");

        stakers[msg.sender].tokenIds = _stakedIds;
    ...
    function isStakable (address owner) public returns(bool) {
        ...
        uint256[] memory levels = getAccountLevelsInfo(owner);
    ...
    function getAccountLevel(address _owner) public view returns(uint256) {
        return iBCNFT.checkAccountLevel(_owner);
    }
    ...
    function claimReward() external returns(bool) {
        ...
        uint256[] memory _stakedIds = stakers[msg.sender].tokenIds;
        for(uint256 i = 0; i < _stakedIds.length; i++) {
            iBCNFT.transferFrom(address(this), msg.sender, _stakedIds[i]);
        }
        uint256[] memory empty = new uint256[](0);
        stakers[msg.sender].isStaking = false;
        stakers[msg.sender].tokenIds = empty;
    ...
    function setRewardPercentage(address _owner, uint256 _period) public view returns (uint256) {
        ...
        uint256 _adminRecoveryBalance = adminRecoveryAddress.balance;
    ...
    function checkListed(uint256 _id) public view returns (bool) {
        return iMarket.getIsListed(_id);
    }
}
