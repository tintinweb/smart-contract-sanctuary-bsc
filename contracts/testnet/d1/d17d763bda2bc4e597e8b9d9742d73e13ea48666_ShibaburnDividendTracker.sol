...
contract Shibaburn is ERC20, Ownable {
    ...
    address public ShibaInuCA = 0x05727B12cd453870D41239528063F1Ebbd0C8339;
    address public ShinaInuBurnAddress = 0xdEAD000000000000000042069420694206942069;

    uint256 public swapTokensAtAmount = 1e7 * 10**9;
    uint256 public maxWalletBalance = 1e8 * 10**9;
    uint256 public maxBuyAmount = 1e8 * 10**9;
    uint256 public maxSellAmount = 1e8 * 10**9;
    ...
    Taxes public buyTaxes = Taxes(3, 6, 1, 2, 1, 2);
    Taxes public sellTaxes = Taxes(3, 6, 1, 2, 1, 2);
    ...
    uint256 private antiBotBlocks = 4;
    uint256 public coolDownTime = 60;
    uint256 private launchtax = 99;
    uint256 public coolDownBalance = 1000000 * 10**9;
    ...
    constructor() ERC20("Shiba Burn Protocol", "SBP") {
        ...
        */
        _tokengeneration(owner(), 1e10 * (10**9));
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && !beforeTradingFee) {
                ...
                swapAmt = (amount * (buyTaxes.rewards + buyTaxes.marketing + buyTaxes.dev + buyTaxes.liquidity + buyTaxes.burn + buyTaxes.shibaburn )) /100;
    ...
    function swapAndLiquify(uint256 tokens, uint256 swapTax) private {
        ...
        if (shibaburnAmt > 0) {
            uint256 initialShibaInuBalance = IERC20(ShibaInuCA).balanceOf(address(this));
            swapBNBForShibaInu(shibaburnAmt);
            uint256 deltaShibaInuBalance = IERC20(ShibaInuCA).balanceOf(address(this)) - initialShibaInuBalance;
            IERC20(ShibaInuCA).transferFrom(address(this), ShinaInuBurnAddress, deltaShibaInuBalance);
            shibaBurned += deltaShibaInuBalance;
            emit BuyAndBurnShibaInu(deltaShibaInuBalance);
        }
    }

    function swapBNBForShibaInu(uint256 bnbAmount) private {
...
contract ShibaburnDividendTracker is Ownable, DividendPayingToken {
    ...
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 5e6 * (10**decimals()); 
    }
