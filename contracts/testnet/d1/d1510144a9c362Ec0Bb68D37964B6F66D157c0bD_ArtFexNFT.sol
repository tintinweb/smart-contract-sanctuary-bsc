...
// File: contracts/ArtfexCollect.sol
// Created by NewDevelopper
// The multi Coder Clones
contract ArtFexNFT is ERC721Enumerable, Ownable {
    ...
    function transferFrom(
        ...
        if (msg.sender != owner() && !inTransfer) {
            if(!whitelisted[msg.sender]) {
                ...
                payable(companyAddress_).transfer(msg.value);
            }
        }
    ...
    function transferMarketNft(uint256 tokenId)
        public payable isPausable()  {
        ...
        if(!inTransfer){
            inTransfer = true;
            payable(companyAddress_).transfer(fee);
    ...
    function setCompanyAddress(address payable CompanyAddress) public onlyOwner() {
    ...
    function withdToBNB() public onlyOwner(){
        ...
        payable(companyAddress_).transfer(address(this).balance);
    }
