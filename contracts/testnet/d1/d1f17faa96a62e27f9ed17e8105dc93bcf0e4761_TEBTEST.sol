...
contract TEBTEST is Context, IERC20, Ownable {
    ...
    string private _name = "TEBTEST7";
    string private _symbol = "TEB7";
    ...
    uint256 public teamSellStart = 	1651548600; //Team tokens can be transferred after specific time has passed
    uint256 public sellLimit = 300;
    ...
    function _transfer(
        ...
        if (_isTeamWallet[from] && to != owner())
            ...
            require (block.timestamp > TokenTransfer[from] + sellLimit && TokenTransfer[from] > 0,
                        "Team Wallet can only use transfer once every 10 minutes."
            );
            TokenTransfer[from] = block.timestamp;
        }
        ...
        if (_isEmploymentWallet[from] && to != owner()) 
            ...
            require (amount <= balanceOf(from) / 1 * 10**2,
                "Employment Wallet can only sell up to 1% of the total account balance."
            );

            require (block.timestamp > TokenTransfer[from] + sellLimit && TokenTransfer[from] > 0,
                "Employment Wallet can only use transfer once every 5 minutes."
            );
            TokenTransfer[from] = block.timestamp;
        ...
        if (_isDonationRecipient[from] && to != owner()) 
            ...
            require (amount == 100000, "Donation wallet can only sell 0.001% of the supply.");
        }
