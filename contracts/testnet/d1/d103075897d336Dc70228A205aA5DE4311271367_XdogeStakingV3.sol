...
contract XdogeStakingV3 is Ownable {
    ...
    function claim() external noContractsAllowed {
        ...
        if (amount > 0) {
            ...
            require(
                Token(TRUSTED_TOKEN_ADDRESS).transfer(msg.sender, amount),
                "Could not transfer earned tokens."
            );
    ...
    function updateMarketingAddress(address newAddress) external onlyOwner {
        require(
            newAddress != address(marketingAddress),
            "Marketer Address already has that address"
        );
