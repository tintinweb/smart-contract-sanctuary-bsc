...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGEN }

    address public immutable GEN;
    ...
    address public sGEN;
    uint public sGENQueue; // Delays change to sGEN address
    ...
    /**
        @notice allow approved address to deposit an asset for GEN
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint GEN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( GEN ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn GEN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IGenERC20( GEN ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sGEN ).balanceOf( msg.sender ); // Can only borrow against sGEN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GEN
        @param _amount uint
     */
    function repayDebtWithGEN( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IGenERC20( GEN ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, GEN, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( GEN ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( GEN ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns GEN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match GEN decimals
            value_ = _amount.mul( 10 ** IERC20( GEN ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SGEN ) { // 9
            sGENQueue = block.number.add( blocksNeededForQueue );
        } else return false;
