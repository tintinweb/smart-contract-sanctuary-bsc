...
contract ExchangeFactory {
    ...
    function createPair(address baseToken, address qouteToken) external returns (address) {
        ...
        // (address token0, address token1) = baseToken < qouteToken ? (baseToken, tokenB) : (tokenB, tokenA);
        require(baseToken != address(0) && qouteToken != address(0), 'Exchange: ZERO_ADDRESS');
        require(getPair[baseToken][qouteToken] == address(0), 'Exchange: PAIR_EXISTS'); // single check is sufficient
        ...
        getPair[qouteToken][baseToken] = address(pair); // populate mapping in the reverse direction
