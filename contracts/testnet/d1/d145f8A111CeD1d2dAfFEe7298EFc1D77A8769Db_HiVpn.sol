...
contract HiVpn is IHiVpn, Secure {
  ...
  // Deposit function
  function buyVpn(address referrer, uint256 plan) public payable override {
...
interface IHiVpn {
  ...
  function buyVpn(address referrer, uint256 plan) external payable;
...
abstract contract Secure {
  ...
  function changePrice(uint8 index, uint32 plan) external onlyOwner {
    PLAN[index] = plan;
  }
