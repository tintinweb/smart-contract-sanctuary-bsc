...
contract AfntyMigrator is Pausable, Ownable {
    ...
    event migrated(uint legacyTokenBalance, uint newTokenBalance, address walletAddr);
    ...
    function deposit(uint amount) external onlyOwner {
      newToken.transfer(address(this), amount);
    ...
    function migrate() external {
      ...
      // TODO TRANSFER NEW TOKEN
      // if (newTokenBalance > legacyTokenBalance) {
      //   uint convertedAmount = legacyTokenBalance * swapRate / swapRateDenominator;
      //   newToken.transfer(msg.sender, convertedAmount);
      // }

      emit migrated(legacyTokenBalance, newTokenBalance, msg.sender);
    } 
}
