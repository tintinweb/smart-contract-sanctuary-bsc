...
contract IFOInitializable is ICronaIFO, ReentrancyGuard, Ownable {
    ...
    // 1 HOUR
    uint256 constant public MAX_CLAIM_DELAY_INTERVAL = 3600;
    ...
    // Time interval between endTime and the time when user can claim token
    uint256 public claimDelayInterval;
    ...
    function harvestPool(uint8 _pid) external override nonReentrant notContract {
        ...
        require(block.timestamp > endTime + claimDelayInterval, "Harvest: Too early");
    ...
    /**
    * @notice It allows the admin to update claimDelayInterval
    * @param _claimDelayInterval: the new time interval
    * @dev This function is only callable by admin.
    */
    function setClaimDelayInterval(uint256 _claimDelayInterval) external onlyOwner {
        require(_claimDelayInterval < MAX_CLAIM_DELAY_INTERVAL, "Operations: Too late");
        claimDelayInterval = _claimDelayInterval;
    }
