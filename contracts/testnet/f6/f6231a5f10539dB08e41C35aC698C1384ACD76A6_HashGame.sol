...
contract HashGame is ReentrancyGuard {
    ...
    function processMvccGame(address _userAddress, bool _result) external onlyOperator {
        ...
        if(_result) {    
            ...
            if(isValidPremium(_userAddress)) 
                distributeMvccPremiumRewards(_userInfo.gameMode , _userAddress);
    ...
    function processMvcrGame(address _userAddress, bool _result) external onlyOperator {
        ...
        if(_result) {    
            ...
            if(isValidPremium(_userAddress)) 
                distributeMvcrPremiumRewards(_userInfo.gameMode, _userAddress);
    ...
    function distributeMvccPremiumRewards(uint256 _gameMode, address _userAddress) internal nonReentrant {
        ...
        emit DistributeMvccPremiumRewards(_gameMode, _userAddress, _amount);
    }

    function distributeMvcrPremiumRewards(uint256 _gameMode, address _userAddress) internal nonReentrant {
        uint256 _amount = getPremiumReimbursementByIndex(_gameMode);

        // Mint MVCR rewards 
        IMVCR.mint(_userAddress, _amount);
        
        emit DistributeMvcrPremiumRewards(_gameMode, _userAddress, _amount);
    }
    ...
    event DistributeMvccPremiumRewards(uint256 gid, address userAddress, uint256 amount);
    event DistributeMvcrPremiumRewards(uint256 gid, address userAddress, uint256 amount);
