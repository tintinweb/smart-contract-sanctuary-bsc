...
contract SC1 is Ownable {
    ...
    address public _EABCDADDRESS = 0x792ae2F4Dc646D6fbfe291fC4280daCCB729b5A2; //testnet
    ...
    address public _SC2ADDRESS = 0x326d5Cb2a7485B89b285b8DA5A5f0ca6c40A402b;
    ...
    function transferBUSD(address dest, uint256 amount) public onlyOwner{
        uint256 balance = IBEP20(_BUSDADDRESS).balanceOf(address(this));
        require(balance > amount, "Insufficient Amount");
        IBEP20(_BUSDADDRESS).transfer(dest, amount);
    }
    ...
    function buyOrder(uint256 amount) public{
        ...
        IEABCD(_EABCDADDRESS).mint(msg.sender, amount * (100 * 1000 - feeForBuyer) / 1000 / 100 / priceFromOracle);
        swapBUSDForTHALES(amount * feeForBuyer / 1000 / 100 / 2);
        IBEP20(_BUSDADDRESS).transfer(_SC2ADDRESS, amount * feeForBuyer / 1000 / 100 / 2);
    ...
    function swapBUSDForTHALES(uint256 amount) internal{
        ...
        IPancakeRouter02(_PANCAKEROUTER).swapExactTokensForETHSupportingFeeOnTransferTokens(
            amount,
    ...
    function airDropTHALES(uint256 amount) public onlyOwner{
        require(amount > IBEP20(_THALESADDRESS).balanceOf(address(this)), "Insufficient amount");
    ...
    function _getFeeForBuyer(address buyer) public view returns(uint256){
        ...
        uint256 fee = plan_A_Fee;
        if(balance > plan_A_Tokens && balance <= plan_B_Tokens)
            fee = plan_B_Fee;
        if(balance > plan_B_Tokens && balance <= plan_C_Tokens)
            fee = plan_C_Fee;
        if(balance > plan_C_Tokens)
            fee = plan_D_Fee;
        return fee;
    }
