...
contract FmToken is ERC20, Ownable{  
    ...
    mapping (address => bool) public _isExcludedLock;// Balance lock end time
    // address
    ...
    constructor() ERC20("Feminist", "F"){
        ...
        _isExcludedLock[0x1119A68C59FC5cffe3696D463f274273BE817c1c] = true;
        ...
        lockTimeType[0x1119A68C59FC5cffe3696D463f274273BE817c1c] = 1;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        // 從鎖倉帳戶轉出
        if(_isExcludedLock[msg.sender]){
            if(lockBalanceOfEndTime[recipient] == 0){
                if(lockTimeType[msg.sender] == 4){
                    lockBalanceOfEndTime[recipient] = block.timestamp.add(balanceLockTime_three_half);
                }
                else if(lockTimeType[msg.sender] == 3){
                    lockBalanceOfEndTime[recipient] = block.timestamp.add(balanceLockTime_two_half);
                }
                else if(lockTimeType[msg.sender] == 2){
                    lockBalanceOfEndTime[recipient] = block.timestamp.add(balanceLockTime_one_half);
                }
                else if(lockTimeType[msg.sender] == 1){
                    lockBalanceOfEndTime[recipient] = block.timestamp.add(balanceLockTime_half);
                }
                _LockBalanceOf[recipient] = _LockBalanceOf[recipient].add(amount);
            }
            else if(lockBalanceOfEndTime[recipient] < block.timestamp){
                _LockBalanceOf[recipient] = _LockBalanceOf[recipient].add(amount);
            }
        ...
        // 非鎖倉帳戶轉出
            ...
            // 鎖倉時間未過
            if(lockBalanceOfEndTime[msg.sender] > block.timestamp && lockBalanceOfEndTime[msg.sender] != 0){
                // 檢查未鎖倉金額是否足夠轉出
                require(balanceOf(msg.sender).sub(_LockBalanceOf[msg.sender]) >= amount,"ERC20: Insufficient balance");
            }
        }
