...
contract ParalaxExchange is ExtensionPercent, EIP712, AccessControl {
    ...
    event LimitOrderP2P(SignerData signerData1, SignerData signerData2);
    event RemoveLimitOrderP2P(bytes sign);
    event LevelOrderDCA(OrderDCA order, uint256 amountIn);
    event TimeMultiplierDCA(TimeMultiplier tm, uint256 amountIn);

    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    bytes32 public constant SIGNER_DATA =
        keccak256(
            "SignerData(address account,address baseCurrency,address quoteCurrency,uint256 amount,uint256 price,uint256 deadline)"
        );
    ...
    mapping(bytes => SignerData) private _signerDatas;
    mapping(bytes => OrderDCA) private _ordersDCA;
    mapping(bytes => TimeMultiplier) private _timeMultipliers;
    mapping(bytes => uint256) private _deadlinesDCA;
    mapping(bytes => bool) private _limitOrdersDEX;
    mapping(bytes => ProcessingDCA) private _processingDCA;

    constructor(
        address adapter,
        address treasure,
        uint256 delta
    ) EIP712("ParalaxExchange", "1") {
    ...
    function limitOrderP2P(
        ...
        // transfering assets between the signers
        ...
        emit LimitOrderP2P(signerData1, signerData2);
    }
    ...
    function limitOrderDEX(
        ...
        require(_verify(sign, signerData), "Sign Error");
        require(signerData.deadline >= block.timestamp, "Deadline expired");
        require(!_limitOrdersDEX[sign], "Already executed");
    ...
    /**
     * @dev Initialize DCA order, proceed the level of the DCA order
     */
    function initOrderDCA(bytes calldata sign, Order memory order) public {
        require(_verify(order, sign), "Sign Error");

        require(_validateOrderDCA(order.dca), "Order Error");
        require(_validatePriceDCA(order.dca), "Price Error");
        require(_validateInitDCA(sign), "Init Error");
    ...
    /**
     * @dev Get initialized order from state, proceed the level of the DCA order
     */
    function dca(bytes calldata sign) public {
        OrderDCA memory order = _ordersDCA[sign];
        ProcessingDCA memory procDCA = _processingDCA[sign];

        require(order.volume != 0 && _deadlinesDCA[sign] != 0, "Init Error");
        require(procDCA.doneTM + procDCA.done < order.volume, "Order Error");
        require(block.timestamp <= _deadlinesDCA[sign], "Expiry Error");
    ...
    function initTimeMultiplier(
        ...
        require(_validatePriceDCA(order.dca), "Price Error");
        require(_validateInitDCA(sign), "Init Error");
        require(_validateTMDCA(order), "TM Error");
        require(_timeMultipliers[sign].amount == 0, "TM: Init Error");
    ...
    function dcaTM(bytes calldata sign) external {
        ...
        if (procDCA.doneTM == tm.amount && tm.amount != 0) {
            ...
            OrderDCA memory order = _ordersDCA[sign];
            uint256 deadline = _deadlinesDCA[sign];

            require(order.volume != 0 && deadline != 0, "Init Error");
            require(procDCA.done < order.volume, "Order Error");
            require(procDCA.doneTM < tm.amount, "TM: Order Error");
            require(block.timestamp <= deadline, "Expiry Error");
            require(_validatePriceDCA(order), "Price Error");
    ...
    function _validateInitDCA(bytes memory sign) internal view returns (bool) {
        return _deadlinesDCA[sign] == 0 && _ordersDCA[sign].price == 0;
    }
    ...
    function _getQuotePrice(
        ...
        uint256[] memory amounts = getAmountsOut(path, newAmountIn);
    ...
    function _levelTM(
        ...
        if (procDCA.doneTM == 0 || (procDCA.doneTM != 0 && order.scale == 0)) {
            ...
            (amount, ) = addPercentage(procDCA.scaleAmount, order.scale);
        }

        if (tm.amount - procDCA.doneTM <= amount) {
        ...
        emit TimeMultiplierDCA(tm, amountIn);
    }
    ...
    function _validateData(
        ...
        require(_verify(sign1, signerData1), "Sign1 Error");
        require(_verify(sign2, signerData2), "Sign2 Error");
    ...
    function verify(
        ...
        return _verify(signature, signerData);
    }
