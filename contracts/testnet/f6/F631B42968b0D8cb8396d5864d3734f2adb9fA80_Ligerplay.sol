...
contract Ligerplay is IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        treasuryReceiver: payable(0xC726Ac99258552bFa2AbaABA47126a8335400299)
    });
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) public virtual onlyOwner {
        require(tokenAddress != address(this), "cannot claim the same token");
    ...
    function setSellLimit(uint _sellLimit) external onlyOwner {
		require (_sellLimit >= 30, "min 30% x Day");
        SellLimit = _sellLimit;
    }

    function setTwentyFourhours(uint256 _time) external onlyOwner {
        require(_time <= 86400, "no more than 1 day");
        TwentyFourhours = _time;
    }

    function setTaxes(uint16 buyFee, uint16 sellFee, uint16 transferFee) external onlyOwner {
        require(buyFee + sellFee + transferFee <= maxRoundtripTax, "Cannot exceed roundtrip maximum.");
    ...
    function setMaxSellTransaction(uint256 _maxTxn) external onlyOwner {
        require (_maxTxn >= _tTotal / 1000, "min 0,1%");
    ...
    function enableTrading(bool _tradingEnabled) public onlyOwner {
        require(!tradingEnabled, "Owner cannot pause trading");
