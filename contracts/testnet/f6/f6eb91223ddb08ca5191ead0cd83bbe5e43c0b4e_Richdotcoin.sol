...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    modifier updateReward(address _account) {
        ...
        if (_account != address(0)) {
            treasuryeOf[_account] = _treasuryAmount[_account] * treasuryRATE/100 * duration;
        }
    ...
    function CheckTreasuryPerformance(address _account) public view returns (uint256) {
        return _treasuryAmount[_account] * treasuryRATE/100 * (block.timestamp - startAt[_account]);
    }

    function withdraw(uint _amount) external updateReward(msg.sender) {
        require(_amount <= treasuryClaim[msg.sender] && _amount > 0, "amount = 0");
