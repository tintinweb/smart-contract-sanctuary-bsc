...
abstract contract OfficerData {
	...
	function _getOfficerStatus(address _officer) public view returns (uint256 officerStatus) {
	...
	/** --------------------------------------------------------------------------- */
	function _getacceptedRole(address _destination, address _caller) public view returns (uint256 _acceptedRole) {
	...
	function _isGovernment(address _destination, address _caller) public view returns (bool) {
		return _getacceptedRole(_destination, _caller) == 369000001; // XXXYYYZZZ
	}

	function _isMonitoring(address _destination, address _caller) public view returns (bool) {
		return _getacceptedRole(_destination, _caller) == 369000002;
	}

	function _isBridging(address _destination, address _caller) public view returns (bool) {
	...
	function _setOfficerStatus(address _officer, uint256 _status) public {
		require(_officer != address(0), "Officer: cannot zero address");
...
interface IOfficer {
	...
	function setOfficerStatus(address officer, uint256 status) external;

	function addOfficer(address _officer) external;
...
contract Officer is OfficerData, IOfficer {
	...
	function addOfficer(address _officer) public virtual OnlyAvailableOfficer onlyGovernment{
		//... Set status anh role ... import data
	}

	function getOfficer4Monitoring()
		public
		view
		virtual
