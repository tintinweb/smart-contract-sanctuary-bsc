...
contract Kinja is  ERC20 , Ownable { 
    ...
    // = IFactory(pancakeRouter.factory()).createPair(
    //         address(this),
    //         pancakeRouter.WETH()
    //     );


    constructor(uint256 initialSupply, address _stackingAddress) ERC20("Kinja Token", "KNJ") { 
   ...
   function burnToken(address _from , uint256 _amount) internal returns(uint256){
    ...
    function transfer(address to, uint256 amount) public virtual override  isPanCakeSwap(msg.sender,to,amount) returns (bool) {
     ...
     function setRouterAddress(address newRouter) external payable onlyOwner {
    ...
    function stacking(uint256 _amount) public {
        transferFrom(msg.sender,stackingAddress, _amount);
    }
