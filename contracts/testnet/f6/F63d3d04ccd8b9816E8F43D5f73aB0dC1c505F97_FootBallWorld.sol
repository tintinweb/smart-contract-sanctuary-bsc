...
contract FootBallWorld is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private constant _TIMELOCK = 0; //自己填写
    ...
    constructor() ERC20("FootBallWorld", "FBW")  {
    ...
    function _transfer(
        ...
        require(from != address(0), "ERC20: transfer from the zero address");  //发送地址不能为空
        require(to != address(0), "ERC20: transfer to the zero address");      //接收地址不能为空
        require(!_isEnemy[from] && !_isEnemy[to], 'Enemy address');            //发送和接收地址不能为黑名单
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                BFee = amount.mul(buyLiquidityFee).div(100);
                AmountLiquidityFee += BFee;
                fees = BFee;
            }
            if(automatedMarketMakerPairs[to]){
                SFee = amount.mul(sellLiquidityFee).div(100);
                fees = SFee;
            }
            amount = amount.sub(fees);
            if(BFee > 0) super._transfer(from, deadWallet, BFee);
            super._transfer(from, address(this), fees.sub(SFee));
        }
