...
contract MountSapphire is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    mapping(uint256 => uint256) public blucamonPower; // tokenID => Power
    ...
    function setRewardToken(address _tokenAddress) public onlyOwner {
        require(_tokenAddress != address(0), "Address shouldn't be 0");
    ...
    function disableTokenReward() public onlyOwner {
        require(rewardToken.enabled, "Reward token is disabled");
        rewardToken.enabled = false;
        emit DisableRewardToken();
    }

    function enableTokenReward() public onlyOwner {
        require(!rewardToken.enabled, "Reward token is enabled");
        rewardToken.enabled = true;
        ...
        returns (address, uint256)
    {
        ...
        return (rewardToken.tokenAddress, reward);
    }
    ...
    function _validateBlucamons(address _sender, uint256 _sentBlucamonsCount)
        internal
        view
    {
        require(
            userInfo[_sender].blucamonTokenIds.length + _sentBlucamonsCount <=
    ...
    function stake(uint256[] memory _blucamonsTokenIds) external {
        _validateBlucamons(msg.sender, _blucamonsTokenIds.length);
