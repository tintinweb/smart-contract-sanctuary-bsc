...
library Whitelist {
    ...
    function verifySignatureWhenMergeDNFT(
        address _signer,
        string memory _stringToVerify,
        bytes memory _signature
    ) internal pure returns (bool) {
        bytes32 messageHash = getMessageHashWhenMergeDNFT(_signer, _stringToVerify);
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
        address signerAddress = getSignerAddress(ethSignedMessageHash, _signature);
        return signerAddress == _signer;
    }
    ...
    function getMessageHashWhenMergeDNFT(address _signer, string memory _stringToVerify)
        internal
        pure
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(_signer, _stringToVerify));
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function deployNewSalePhase(
        ...
        if (_salePhase == 4) {
            claimableTime = _endTime + SEVEN_DAYS;
        }
        ...
        nonReentrant
    {
        ...
        if (totalRescued >= unSoldToken) revert ALL_TOKENS_ARE_SOLD_OR_RESCUED();
        if (block.timestamp < claimableTime) revert NOT_YET_RESCUE_TIME();
        ...
        if (_isUsingBNB) {
            uint256 rescuePriceInBNB = convertBUSDToBNB(rescuePrice);
    ...
    function temporaryMerge(
        ...
        if (!verifyMergeSignature(_tokenIds, _timestamp, _sessionId, _signature)) revert INVALID_SIGNATURE();
        ...
        BUSDToken.transferFrom(msg.sender, treasuryAddress, mergeTax);

        uint256 totalTokens = _tokenIds.length;
        for (uint256 i = 0; i < totalTokens; i++) {
            if (ownerOf(_tokenIds[i]) != msg.sender) revert NOT_OWNER_OF_TOKEN();
    ...
    function _claimable() private view {
        if (block.timestamp < claimableTime) revert NOT_CLAIMABLE_TIME();
    }
    ...
    function verifyMergeSignature(
        ...
        string memory mergeSet = MergeHelper.generateMergeSet(_tokenIds);
        string memory stringToVerify = string(abi.encodePacked(mergeSet, _timestamp.toString(), _sessionId));
        return (Whitelist.verifySignatureWhenMergeDNFT(signer, stringToVerify, _signature));
    }

    function lockToken(uint256 _tokenId) private {
        if (ownerOf(_tokenId) != msg.sender) revert NOT_OWNER_OF_TOKEN();
        if (isLocked[_tokenId]) revert TOKEN_IS_LOCKED();
        isLocked[_tokenId] = true;
    }

    function unlockToken(uint256 _tokenId) private {
        if (ownerOf(_tokenId) != msg.sender) revert NOT_OWNER_OF_TOKEN();
        if (!isLocked[_tokenId]) revert TOKEN_IS_NOT_LOCKED();
    ...
    function safeTransferFrom(
        ...
        if (isTokenLocked(tokenId)) revert TOKEN_IS_LOCKED();
        uint256 taxAmount = getMintTaxAmount(launchPrice);
        if (BUSDToken.balanceOf(from) < taxAmount) revert INSUFFICIENT_BUSD();
