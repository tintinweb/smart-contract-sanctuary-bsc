...
contract TokenTool is Context, IERC20, Ownable {
	...
	mapping (address => address) public isRecommenders;
	...
	function recommender(address account) public view returns (address) {
        return isRecommenders[account];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
			...
			if(!isRecommender[recipient]){
				isRecommender[recipient]=true;
				isRecommenders[recipient]=sender;
			}
            return true;
        }
    }
