...
//减产机制 0-1.5年 10000 ，1.5-3年 8000 ，3-4.5年 5.12，4.5后4.096 
  function Reduceproduction()public view  returns(uint256)  {
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        uint256 splitStaticReward;
        uint256 trueStaticReward;
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(block.timestamp > order.unfreeze  && order.isUnfreezed == false && _amount >= order.amount){
                ...
                _removeInvalidDeposit(_user, order.amount);
                 staticReward = order.amount.mul(dayRewardPercents).mul(dayPerCycle).div(timeStep).div(baseDivider).mul(Reduceproduction()).div(10000);
                 splitStaticReward = staticReward.mul(freezeIncomePercents).div(baseDivider);
                 trueStaticReward = staticReward.sub(splitStaticReward);
