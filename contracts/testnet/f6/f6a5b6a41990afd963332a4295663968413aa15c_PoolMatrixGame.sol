...
contract PoolMatrixGame {
    ...
    uint private globalRefreshCounter;
    ...
    receive() external payable {
        ...
        if (users[msg.sender].regDate == 0) {
            internalRegister(rootWallet1, restOfAmount);
    ...
    function internalRegister(address referrer, uint investAmount) private {
        ...
        //users[msg.sender].changesCounter = 0;
    ...
    function internalBuyLevel(uint level, uint investAmount) private {
        ...
        require(users[msg.sender].levels[level].opened == 0); // Check if level is not opened
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            while (users[referrer].levels[level].opened == 0) {
                users[referrer].levels[level].missedProfit += value;
                users[referrer].changesCounter++;
        ...
        users[msg.sender].levels[level].opened = 1;
        users[msg.sender].levels[level].missedProfit = 0;
        users[msg.sender].changesCounter++;
    ...
    function register(address referrer) public payable {
        uint restOfAmount = msg.value;
        internalRegister(referrer, restOfAmount);
        restOfAmount -= REGISTRATION_PRICE;
    ...
    function detectUserChanges(address userAddr) public view verified returns(uint) {
        uint changes = globalRefreshCounter + users[userAddr].changesCounter;
        if (state == 1)
            changes += 9999;
        return changes;
    }
    ...
    function getSlots(address userAddr) public view verified returns(int[] memory slots, uint[] memory sizes, uint[] memory partnerBonuses, uint[] memory poolProfits, uint[] memory missedProfits, uint contractState) {
        ...
        for (uint level = 0; level < LEVELS_COUNT; level++) {
            ...
            if (users[userAddr].levels[level].opened == 0) {
                if (users[userAddr].levels[level].payouts == 0)
    ...
    function levelIsOpened(address userAddr, uint level) public view verified returns(bool) {
        return users[userAddr].levels[level].opened > 0;
    }
    ...
    function getImplementation() public view verified returns (address) {
