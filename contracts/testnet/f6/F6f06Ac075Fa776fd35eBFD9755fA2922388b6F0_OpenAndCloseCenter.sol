...
contract OpenAndCloseCenter is Ownable {
    ...
    mapping(uint256 => uint256) public lockedInTrainingCenterID; // LP locked in this CENTER instance
    uint256 public trainingCenterPrice = 1000000 * 1E18;

    mapping(uint256 => uint256) public laboratoryMaturityTime;
    mapping(uint256 => uint256) public lockedInLaboID; // LP locked in this labo instance
        ...
        returns (string memory)
    {
        ...
        if (trainingCenterMaturityTime[_tokenId] == 0) {
            if (_tokenId <= ITraining(gameAddresses.getTrainingNFTAddress()).getPreMintNumber()) {
        ...
        returns (string memory)
    {
        ...
        if (laboratoryMaturityTime[_tokenId] == 0) {
            if (_tokenId <= ILaboratory(gameAddresses.getLaboratoryNFTAddress())
                .getPreMintNumber()) {
                result = housesStates[2];
            } else {
                result = housesStates[0];
