...
contract Team is Ownable {
	...
	uint256 public limits;//邀请奖励设置门槛
	event setNums(uint num1,uint num2);
    ...
    constructor() {
        usdtToken = IERC20(0xb063D2b564d555637Ef5b8D1A21f96E08973Ef04);
    ...
    function setStake(address _adr) external onlyOwner{
        stake = StakingRewards(_adr);//挖矿合约地址
    }
    ...
    function bulid(address superior) external{
        ...
        emit relations(msg.sender,superior);//关系log
    }
    ...
    function setBalance(address _adr,uint _num) external onlyWhite{
        ...
        if(relation[_adr] != address(0) && stake.balanceOfs(relation[_adr]) >= limits){
            balanceOf[relation[_adr]] += num1;
        }else{
            balanceOf[owner()] += num1;
        }

        if(relation[relation[_adr]] != address(0) && stake.balanceOfs(relation[_adr]) >= limits){
            balanceOf[relation[relation[_adr]]] += num2;
        }else{
            balanceOf[owner()] += num2;
        }
		emit setNums( num1, num2);
