...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    uint256 private constant HEARTS_PER_HEX = 10 ** uint256(8); // 1e8
    ...
    uint256 internal constant LAUNCH_TIME = 1665582069;
    ...
    bytes32 internal constant MERKLE_TREE_ROOT = 0x72de99952a33796f445a432bb3783469bc1fc4287f2dbc531bd601c856904976;
    ...
    function _currentDay()
        ...
        returns (uint256)
    {
        return (block.timestamp - LAUNCH_TIME) / 120;
    }
        ...
        returns (uint256)
    {
        ...
        */
        ...
        return (payout * g._claimedBtcAddrCount / CLAIMABLE_BTC_ADDR_COUNT) + (payout * g._claimedSatoshisTotal / CLAIMABLE_SATOSHIS_TOTAL);
    }
...
contract StakeableToken is GlobalsAndUtility {
     ...
     */
    function stakeStart(uint256 newStakedHearts, uint256 newStakedDays, string memory newStakeName)
        external
    {
        GlobalsCache memory g;
        GlobalsCache memory gSnapshot;
        ...
        _stakeStart(g, newStakedHearts, newStakedDays, newStakeName, false);
        ...
        external
    {
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "not fully served");
        ...
        emit StakeGoodAccounting( // (auto-generated event)
            uint256(uint40(block.timestamp))
                | (uint256(uint72(st._stakedHearts)) << 40)
    ...
    function stakeTransfer (uint256 stakeIndex, address transferToAddress) external {
        // GlobalsCache memory g;
        // GlobalsCache memory gSnapshot;
        // _globalsLoad(g, gSnapshot);
    ...
    // function stakeRecords(address stakerAddr)
    //     external
    //     view
    //     returns (StakeStore[] memory records)
    // {
    //     return stakeLists[stakerAddr];
    // }
        ...
        internal
    {
        ...
        uint256 newStakeShares = (newStakedHearts + (_stakeStartBonusHearts(newStakedHearts, newStakedDays))) * SHARE_RATE_SCALE / g._shareRate;
        ...
        private
    {
        if (stakeReturn > st._stakedHearts) {
            ...
            */
            ...
            uint256 newShareRate = (stakeReturn + (_stakeStartBonusHearts(stakeReturn, st._stakedDays))) * SHARE_RATE_SCALE / st._stakeShares;
...
contract SignatureVerification  is StakeableToken{
        ...
        returns (bool)
    {
        ...
        */
        ...
        return  ecrecover(messageHash, v, r, s) == pubKeyToEthAddress(pubKeyX, pubKeyY);
    }
        ...
        returns (bool)
    {
        ...
        return _merkleProofIsValid(keccak256(abi.encodePacked(btcAddr, rawSatoshis)), proof);
    }
...
contract UTXORedeemableToken is SignatureVerification {
        ...
        returns (uint256)
    {
        ...
        /* Sanity check */
        require(rawSatoshis <= MAX_BTC_ADDR_BALANCE_SATOSHIS, "CHK: rawSatoshis");

        /* Enforce the minimum stake time for the auto-stake from this claim */
        require(autoStakeDays >= MIN_AUTO_STAKE_DAYS, "autoStakeDays lower than min");
     ...
     * @param autoStakeDays Number of days to auto-stake, subject to minimum auto-stake days
     * @param referrerAddr Eth address of referring user (optional; 0x0 for no referrer)
     *  Total number of Hearts credited, if successful
     */
        ...
        returns (uint256 totalClaimedHearts)
    {
        ...
        require(g._currentDay >= CLAIM_PHASE_START_DAY, "phase not yet started");
...
contract TransformableToken is UTXORedeemableToken {
    ...
    function xfLobbyExit(uint256 enterDay) external {
        ...
        do {
            ...
            if (referrerAddr == address(0)) {
                /* No referrer */
                _emitXfLobbyExit(enterDay, headIndex, xfAmount, referrerAddr,0);
            } else {
    ...
    function xfLobbyDailySupply(uint256 beginDay, uint256 endDay)
        external
        view
        returns (uint256[] memory list)
    {
        ...
        do {
            if (src == 0) {
                list[dst++] = WAAS_LOBBY_SEED_HEARTS;
            }
    
           if (src >= CLAIM_PHASE_START_DAY) {
                if (g._dailyDataCount >= src) {
                    uint256 unclaimed = dailyData[src]
                        .dayUnclaimedSatoshisTotal;
                    list[dst++] =
                        (unclaimed * HEARTS_PER_SATOSHI) /
                        CLAIM_PHASE_DAYS;
                } else {
                uint256 unclaimed = dailyData[g._dailyDataCount].dayUnclaimedSatoshisTotal;
                if(unclaimed == 0){
                unclaimed = FULL_SATOSHIS_TOTAL;
                }
                list[dst++] =  (unclaimed * HEARTS_PER_SATOSHI) / CLAIM_PHASE_DAYS;
