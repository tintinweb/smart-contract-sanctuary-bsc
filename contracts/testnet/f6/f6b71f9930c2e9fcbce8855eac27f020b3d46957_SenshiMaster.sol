...
// Kazama Senshi token with Governance.
contract KazamaSenshi is BEP20('Kazama Senshi', 'KAZAMA') {
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "KAZAMA::delegateBySig: signature expired");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying KAZAMA (not scaled);
...
contract SenshiPower is BEP20('Senshi Power', 'SENSHI') {
    ...
    function safeKazamaTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorSenshi {
    function migrate(IBEP20 token) external returns (IBEP20);
}

contract SenshiMaster is Ownable {
    using SafeMath for uint256;
    ...
    // Update the given pool's KAZAMA allocation point. Can only be called by the owner.
    ...
    function setMigrator(IMigratorSenshi _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    // Deposit LP tokens to SenshiMaster for KAZAMA allocation.
    ...
    // Withdraw LP tokens from SenshiMaster.
    ...
    function enterStaking(uint256 _amount) public {
        ...
        senshi.mint(msg.sender, _amount);
    ...
    function dev(address _devaddr) public {
        require(msg.sender == devaddr, "You aren't the Senshi Master ..");
        devaddr = _devaddr;
    }
}