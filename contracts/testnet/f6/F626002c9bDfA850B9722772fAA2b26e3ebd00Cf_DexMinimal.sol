...
contract DexMinimal{
    ...
    function buy(uint _order_id,string memory ticker,Category side) external TokenCheck(ticker)  {
        Order storage order=orderBook[ticker][uint(side)][_order_id];
        uint256 amount=order.amount;
        uint256 price=order.price;
        require(amount<=balancelist[order.creator][ticker],"currency BALANCE IS LOW ");
        TokenManager manager=TokenManager(managerAddress);
        require(IERC20(manager.getTokenAddress(string('DAI'))).transferFrom(msg.sender,order.creator,amount*price),"cant transfer dai");
    ...
    function sell(uint _order_id,string memory ticker,Category side) external TokenCheck(ticker)  {
        Order storage order=orderBook[ticker][uint(side)][_order_id];
        uint256 amount=order.amount;
        uint256 price=order.price;
        TokenManager manager=TokenManager(managerAddress);
        withdraw(order.amount*order.price,'DAI');
        // require(IERC20(manager.getTokenAddress(string('DAI'))).transfer(msg.sender,amount*price),"cant send dai");
        ...
        int256 index=indexOf(orderBook[ticker][uint(side)],_order_id);
        if(index!=-1){
