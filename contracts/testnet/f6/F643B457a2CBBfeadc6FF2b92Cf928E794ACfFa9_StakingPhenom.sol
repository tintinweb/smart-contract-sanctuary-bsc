...
contract StakingPhenom{
    ...
    function transferTokenToStaking(uint256 amount, uint256 period) public {
        ...
        holders[msg.sender].push(Staking(amount, StakingType.TOKEN, StakingPeriods[period], block.timestamp, periodDays, false));
    }

    function transferFrozenToStaking(uint256 period) public {
        require(msg.sender != address(0), "User with zero address");
        require(StakingPeriods[period] != 0, "There is no such staking period");
    ...
    function delivery(
        ...
        if(getSales() == 1){
            ...
            holders[user].push(Staking(amount, StakingType.USDT, 0, 0, 0, true));
        }
	}

    function withdrawPercents(address user, StakingType token) public{
        for(uint i = 0; i < holders[user].length; i++){
            if(holders[user][i].staking == token){
                withdrawOneStakingPercents(user, i);
            }
        }
    }

    function withdrawOneStakingPercents(address user, uint256 stakingNum) public{
        require(user != address(0), "Withdraw user the zero address");
        require(stakingNum < holders[user].length, "Staking does not exist");
        uint256 amount = 0;
        ...
        if(!holders[user][stakingNum].frozen){
            if(holders[user][stakingNum].stakingTimestamp != 0){
                (uint256 days_, uint256 remains) = getDate(user, stakingNum);
                if(days_ > 0){
                    holders[user][stakingNum].stakingTimestamp = timestamp - remains;
                    if(holders[user][stakingNum].daysRemains > days_){
                        holders[user][stakingNum].daysRemains -= days_;
                        if(holders[user][stakingNum].staking == StakingType.TOKEN){
                            amount += (holders[user][stakingNum].amount * holders[user][stakingNum].StakingPeriod / 100) / 100 * days_;
                        }else if(holders[user][stakingNum].staking == StakingType.USDT){
                            amountUSDT += (holders[user][stakingNum].amount * holders[user][stakingNum].StakingPeriod / 100) / 100 * days_;
                        }
                    }else{
                        if(holders[user][stakingNum].staking == StakingType.TOKEN){
                            amount += (holders[user][stakingNum].amount * holders[user][stakingNum].StakingPeriod / 100) / 100 * holders[user][stakingNum].daysRemains;
                            bAmount += holders[user][stakingNum].amount;
                        }else if(holders[user][stakingNum].staking == StakingType.USDT){
                            amountUSDT += (holders[user][stakingNum].amount * holders[user][stakingNum].StakingPeriod / 100) / 100 * holders[user][stakingNum].daysRemains;
                            bAmountUSDT += holders[user][stakingNum].amount;
                        }
                        holders[user][stakingNum] = holders[user][holders[user].length - 1];
                        holders[user].pop();
                    }
                }
            }
        ...
        if(amount > 0){
            ...
            if(holders[user][stakingNum].StakingPeriod == 12){
                allStakingAmount += holders[user][stakingNum].amount;
            }
        ...
        if(amountUSDT > 0){
            ...
            for(uint256 i = 0; i < holders[user].length; i++){
                if(holders[user][i].StakingPeriod == 12){
                    allStakingAmount += holders[user][i].amount;
                }
            }
        ...
        user,nAmount,allStakingAmount));
        require(success,"depositPools call FAIL");
    }

    //////////////////////////////////////

    function reinvestStakingPercents(address user, StakingType token) public{
        for(uint i = 0; i < holders[user].length; i++){
            if(holders[user][i].staking == token){
                reinvestOneStakingPercents(user, i);
            }
        }
    }

    function reinvestOneStakingPercents(address user, uint256 stakingNum) public{
        require(user != address(0), "Reinvest user the zero address");
        require(stakingNum < holders[user].length, "Staking does not exist");
        uint256 amount = 0;
        uint256 amountUSDT = 0;
        uint256 timestamp = block.timestamp;
        ...
        if(!holders[user][stakingNum].frozen){
            if(holders[user][stakingNum].stakingTimestamp != 0){
                (uint256 days_, uint256 remains) = getDate(user, stakingNum);
                require(holders[user][stakingNum].daysRemains > days_, "The staking period is over");
                if(days_ > 0){
                    holders[user][stakingNum].stakingTimestamp = timestamp - remains;
                    if(holders[user][stakingNum].daysRemains > days_){
                        holders[user][stakingNum].daysRemains -= days_;
        ...
        if(amount > 0){
            ...
            holders[user][stakingNum].amount += amount * 75 / 100;
        ...
        if(amountUSDT > 0){
            holders[user][stakingNum].amount += amountUSDT;
        }
    }
