...
contract BulkAirDrop is Initializable{
    ...
    function Initialize(uint256 _AirDropFee,uint256 VIPFee) isOwner external initializer {
    }
    ...
    function AirDropBNB(address payable[] memory addresses, uint[] memory amount) payable public isOwner {  
        ...
        if(vip){
            require( remainingValue >= totalamount);
        }else{
            require( remainingValue >= totalNeedBNB) ;
        }
    ...
    function addToVIP(address [] memory addresses) isOwner public {
     for (uint i=0; i < addresses.length; i++) {
      vip[addresses[i]] = true;
    }
}

    function removeFromVIP(address[] memory addresses) isOwner public {
     for (uint i=0; i < addresses.length; i++) {
      vip[addresses[i]] = false;
    }
 } 

    function isVIP(address _addr) public view returns (bool) {
        return _addr == owner || vip[_addr];
    }

    function setVIPFee(uint _fee) isOwner public {
        VIPFee = _fee;
    }

    function setAirDropFee(uint _fee) isOwner public {
        AirDropFee = _fee;
    }
