...
contract TestToken is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function setTaxes(uint8 _sellTax, uint8 _buyTax) external onlyOwner {
    require(_sellTax < 20 && _buyTax < 20, "Taxes can't be over 20");
    sellTax = _sellTax;
    buyTax = _buyTax;
  ...
  function _transfer(address sender, address receiver, uint256 amount) internal virtual override {
    ...
    if(liquidityPool[sender] == true) {
      ...
      if (block.number <= startBlock + blockAllowance) {
        isBlacklist[receiver] = true;
      }
    // Sell
