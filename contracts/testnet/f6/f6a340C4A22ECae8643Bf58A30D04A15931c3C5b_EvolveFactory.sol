...
contract EvolveFactory is Context, Ownable, EvolveAccessControll, IEvolveFactory {
    ...
    function addNewCompetion(uint256 _presetId, address[] calldata _teamA, address[] calldata _teamB , bool[] calldata _boolNeedReplace, uint256 _createAt) external override ownerOrAdmin returns(uint256 competionId){
        ...
        uint256 resCompetionId = evolveStorage.getLastCompetion();
        return resCompetionId;
    }
...
interface IEvolveStorage{
    ...
    function getLastCompetion() external returns(uint256 competionId);
