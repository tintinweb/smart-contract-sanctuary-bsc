...
contract BANK is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = "Bankero Finance";
    string public _symbol = "$BANK";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY =
        512 * 10**3 * 10**DECIMALS;

    uint256 public liquidityFee = 40;
    uint256 public treasuryFee = 25;
    uint256 public bankInsuranceFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(bankInsuranceFundFee).add(firePitFee);
    ...
    address public bankInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed(_name, _symbol, uint8(DECIMALS)) Ownable() {
        ...
        bankInsuranceFundReceiver = 0xCE0d4763b7bf9f59A412c4f7272E2eE1e47E9Aa2;
        firePit = 0x3d88f1ABB1583ab9038747DD0214831CC87c9a74;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(
                _treasuryFee.add(bankInsuranceFundFee)
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(bankInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(bankInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(bankInsuranceFundFee).div(
                treasuryFee.add(bankInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function setFeeReceivers(
        ...
        address _bankInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        bankInsuranceFundReceiver = _bankInsuranceFundReceiver;
        firePit = _firePit;
    }
    ...
    function changeFees(
        ...
        uint256 _bankInsuranceFundFee,
        uint256 _sellFee,
        uint256 _firePitFee
    ) external onlyOwner {
        liquidityFee = _liquidityFee;
        treasuryFee = _treasuryFee;
        bankInsuranceFundFee = _bankInsuranceFundFee;
        sellFee = _sellFee;
        firePitFee = _firePitFee;
        totalFee = _liquidityFee
            .add(_treasuryFee)
            .add(_bankInsuranceFundFee)
            .add(_firePitFee);
    }
