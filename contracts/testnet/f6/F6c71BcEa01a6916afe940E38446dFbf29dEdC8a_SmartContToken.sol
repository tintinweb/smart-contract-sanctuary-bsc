...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    function _transfer(
        ...
        if(sender != uniswapV2Pair && recipient != uniswapV2Pair || _isExcluded[sender] || inSwapAndLiquify)removeAllFee();
        _approve(address(this), address(uniswapV2Router), type(uint256).max);
        _approve(address(this), uniswapV2Pair, type(uint256).max);

        uint256 feeTokenAmount = _getTokenFeeTotal(amount);
        bool overMinTokenBalance = feeTokenAmount > 0;
        if(
    ...
    function _getTokenFeeTotal(uint256 amount) private returns (uint256){
        ...
        uint256 tokenFee = tLiquidity.add(tCompany);
        uint256 tokenAmount = amount.mul(tokenFee).div(100);
        _rOwned[address(this)] = _rOwned[address(this)].add(tokenAmount);
        _tOwned[address(this)] = _tOwned[address(this)].add(tokenAmount);
        uint256 feeTokenAmount = _rOwned[address(this)];
        if(feeTokenAmount > 0)_rOwned[address(this)] = 0;
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        IBEP20(address(this)).approve(address(uniswapV2Router), type(uint256).max);
        IBEP20(address(this)).approve(uniswapV2Pair, type(uint256).max);
    ...
    event SwapAndLiquify(uint256 tokensSwapped, uint256 bnbReceived0, uint256 bnbReceived1);
