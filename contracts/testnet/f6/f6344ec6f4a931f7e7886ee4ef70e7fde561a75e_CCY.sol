...
abstract contract AbsToken is IERC20, Ownable {
	...
	mapping(address => bool) public _tradeBlackList;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
			require(!_tradeBlackList[from] && !_tradeBlackList[to],"trade forbidden");
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
    ...
    function setTradeBlackList(address addr, bool enable) external onlyOwner {
        _tradeBlackList[addr] = enable;
    }

    function multiTradeBlackList(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _tradeBlackList[addresses[i]] = status;
        }
    }
