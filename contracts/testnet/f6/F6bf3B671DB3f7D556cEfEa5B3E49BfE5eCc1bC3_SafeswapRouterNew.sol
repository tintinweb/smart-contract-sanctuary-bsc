...
// File: contracts/SafeswapRouterNew.sol
...
contract SafeswapRouterNew is ISafeswapRouter02 {
    using SafeMath for uint;

    address public immutable override factory;
    address public immutable override WETH;
    // beginnig of STP info
    struct tokenInfo {
        bool exists;
        string tokenName;
        address tokenAddress;
        address feesAddress;
        uint expectedTransfer;
        uint expectedBuy;
        uint expectedSell;
        uint actualTransfer;
        uint actualBuy;
    ...
    event Here(bool Here);
    event amountInBeforeD(uint amountInQ);
    event beforeDeductionEvent(uint beforeD);
    event amountInAfterD(uint amountInDQ);
    ...
    function registerToken
        ...
        public
        {
            nameToInfo[tokenAddress].exists = true;
            ...
            nameToInfo[tokenAddress].expectedTransfer = expectedTransfer;
            nameToInfo[tokenAddress].expectedBuy = expectedBuy;
            nameToInfo[tokenAddress].expectedSell = expectedSell;
            nameToInfo[tokenAddress].actualTransfer = actualTransfer;
    ...
    function swapExactTokensForTokens(
        ...
        if(nameToInfo[path[0]].exists == true && (nameToInfo[path[0]].actualSell != nameToInfo[path[0]].expectedSell)) {
        ...
        if(nameToInfo[path[1]].exists == true && (nameToInfo[path[1]].actualBuy != nameToInfo[path[1]].expectedBuy)) {
            ...
            amounts[amounts.length-1] = amountOut;
            /*TransferHelper.safeTransferFrom(
            path[1], msg.sender, nameToInfo[path[1]].feesAddress, deduction
            );*/
        }
        //require(amounts[amounts.length - 1] >= amountOutMin, 'SafeswapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
    ...
    event EExists(bool isExists);
    event EactualSell (uint actualSell);
    event EexpectedSell (uint expectedSell);
