...
contract AuctionTime is Ownable, Random{
    ...
    uint public auctionStartTime = 1645531200;// 2022/1/30 20:00
...
contract MomoBearAuction is Pausable, AuctionTime, IMomoBear{
    ...
    function getPriceForUSDT() public view returns(uint){
        uint newtime = now - dutchAuctionTime;
        uint add = newtime / addTime;

        uint newPrice = _priceForUSDT + addPrice*add;
        return newPrice;
    }
    ...
    function winTheBidByUsdt(
        ...
        require(totalAuctionAmount() < 10, "Auction has ended");
...
//_winTheBid
    ...
    function _winTheBid(uint lastPrice) private{
        uint heroIndex = 905;
        uint allAmount = 10 - bearAuctionAmount() - specialAuctionAmount();
        uint randomNumber = (_getRandomNumber() % allAmount).add(1);
        if(randomNumber > (9 - bearAuctionAmount())){
            heroIndex = 906;
        ...
        totalAuction = totalAuction + lastPrice;
    }
...
//onlyOwner
    ...
    function setAddPrice(uint amount) external onlyOwner {
        addPrice = amount;
    }

    function setAddTime(uint _time) external onlyOwner {
        addTime = _time;
    }
