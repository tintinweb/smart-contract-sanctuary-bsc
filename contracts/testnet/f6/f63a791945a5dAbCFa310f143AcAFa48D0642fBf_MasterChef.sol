...
contract MasterChef is Ownable {
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, uint16 _withdrawFeeBP) public onlyOwner {
    ...
    //Set fee Function 
    // function withdrawFee(uint256 _pid,uint16 _withdrawFee) public virtual onlyOwner returns (uint16) {
    //     fee = _withdrawFee;
    //     return fee;
    // } 

    // function seewithdrawFee(uint256 _pid) external view returns (uint256){
    //     return withdrawFee;
    // } 
        ...
        updatePool(_pid);
         ...
         if(_amount > 0) {
         	if(pool.withdrawFeeBP > 0){
                ...
                user.amount = _amount.sub(withdrawFee);
