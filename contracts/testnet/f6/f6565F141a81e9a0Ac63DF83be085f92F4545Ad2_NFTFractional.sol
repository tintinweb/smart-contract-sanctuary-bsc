...
contract NFTFractional is Upgradable {
    ...
    */
    function createRewardPool(address _addr, uint256[] memory _datas) external onlyAdmins {
        ...
        // check reward pool exist
        require(rewardPools[_datas[0]].rewardToken == address(0), "NFTFractional: Reward Pool does not exist");
        RewardPool memory pool = rewardPools[_datas[0]];
    ...
    */
    function withdrawFund(address _tokenAddress, address _account, uint256 _amount, uint256 _poolId) external onlyAdmins {
        ...
        require(IERC20(_tokenAddress).balanceOf(address(this)) - stakingBalances[_tokenAddress] >= _amount && pool.availableBalance >= _amount, "NFTFractional: Pool not has enough balance");
