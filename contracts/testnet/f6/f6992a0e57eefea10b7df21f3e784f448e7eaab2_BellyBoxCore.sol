...
contract BellyBoxCore is Initializable, AccessControlUpgradeable, IBellyBoxCore{
    ...
    // Each type total sales
    mapping(uint256 => uint256) private typeSales;
    // Each type total max sales 1-24000 2-18000 3-10000
    mapping(uint256 => uint256) private typeMaxSales;
    ...
    function initialize(address _ingredientERC1155Address, address _equipmentERC1155Address, address chiCoinAddress, address cswAddress, address _seedBiulder, address _busdToken) public initializer{
        ...
        busdToken = IERC20Token(_busdToken);
        ...
        typeArray = [1, 2, 3];
        typeMaxSales[1] = 24000;
        typeMaxSales[2] = 18000;
        typeMaxSales[3] = 10000;
    }
    ...
    function createBellyBox(
        ...
        SafeERC20.safeTransferFrom(cswToken, _msgSender(), address(this), cswAmount);
        _openBox(boxId, bellyBoxType, _name, _describe);
    }
    ...
    function createBellyBoxByBUSD(
        ...
        require(bellyBoxes[bellyBoxType].opened <= bellyBoxes[bellyBoxType].supply, "The belly box exceeds the upper limit");
        SafeERC20.safeTransferFrom(busdToken, _msgSender(), address(this), busdAmount);
        _openBox(boxId, bellyBoxType, _name, _describe);
    }

    function _openBox(uint256 boxId,
        uint8 bellyBoxType,
        string calldata _name,
        string calldata _describe) private {
        uint length = typeArray.length;
        require(length > 0, "Ingredient: All belly box sales out");
        bellyBoxes[bellyBoxType].opened ++;

        uint256 seed = seedBiulder.getRandomSeedUsingHash(_msgSender(), bytes32(uint256(bellyBoxes[bellyBoxType].opened)));
        if(bellyBoxType == 3) {
            bellyBoxType = uint8(typeArray[Random.createRandom(1, length, seed, block.timestamp) - 1]);
        }
        if(++ typeSales[bellyBoxType] >= typeMaxSales[bellyBoxType]){
        ...
        if (bellyBoxType == 1) {
            ingredientERC1155.createIngredient(_msgSender(), boxId, _name, _describe, seed);
        }else if (bellyBoxType == 2) {
            equipmentERC1155.createEquipment(_msgSender(), boxId, _name, _describe, seed);
        }else if (bellyBoxType == 3) {
            SafeERC20.safeTransfer(chiCoin, _msgSender(), bellyBoxChiAmount);
            emit GiveChiToken( boxId, _msgSender(), bellyBoxChiAmount);
        }
    }
...
interface IEquipmentERC1155 {
    ...
    event EquipmentCreated(uint8 childType, uint256 indexed id, address indexed owner, uint256 boxId);
    
    function createEquipment(address recipient, uint256 boxId, string memory _name, string memory _describe, uint256 seed) external;
