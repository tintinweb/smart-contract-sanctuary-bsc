...
contract StandardTokenFactoryNew is ReentrancyGuard, Ownable {
...
//    refundExcessiveFee();
    StandardToken newToken = new StandardToken(
      name, symbol, decimals, totalSupply, feeTo, flatFee
    );
//    payable(address(newToken)).sendValue(flatFee);
//    assignTokenToOwner(msg.sender, address(newToken), 0);
