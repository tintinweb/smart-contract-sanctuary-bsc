...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  event GameFinished(address indexed user, uint mission, uint userReward, uint currentEnergy, bool dailyQuestCompleted, uint dayPassed, bytes signature, uint fotaPrice);
  ...
  function finishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature, uint _timestamp) external {
    ...
    emit GameFinished(msg.sender, _mission, userReward, energy, dailyQuestCompleted, dayPassed, _signature, fotaPricer.fotaPrice());
  }
  ...
  function _giveReward(uint _mission, bool _firstCompleteMission, uint[] calldata _heroIds) private returns (uint, bool) {
    ...
    (userReward, dailyQuestCompleted) = rewardManager.addPVEReward(_mission, msg.sender, reward, _heroIds);
