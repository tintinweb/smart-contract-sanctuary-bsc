...
//This is a simple erc20 token transfer
contract ERC721CrossChain {
    ...
    function transferToken(
        ...
        zKSendApi.zkSendWithPayload(payload);
    }
    ...
    function completeTransfer(
        ...
        string memory uri,
        address to,
        uint16 toChain) = abi.decode(payload, (address, uint16, string, string, uint256, string, address, uint16));

        // TODO you can verify emitterChainId and emitterAddress
        require(toChain == chainId, "invalid target chain");
        require(!completedTransfers[hash], "transfer already completed");
        completedTransfers[hash] = true;

        IERC721 transferToken;
        if (tokenChain == chainId) {
            transferToken = IERC721(tokenAddress);
        } else {
            address wrapped = wrappedAssets[tokenChain][tokenAddress];

            // If the wrapped asset does not exist yet, create it
            if (wrapped == address(0)) {
                wrapped = _createWrapped(tokenChain, tokenAddress, name, symbol);
            }
        ...
        if (tokenChain != chainId) {
            // mint wrapped asset
            NFTImplementation(address(transferToken)).mint(to, tokenID, uri);
        } else {
            transferToken.safeTransferFrom(address(this), to, tokenID);
        }
    ...
    function _createWrapped(uint16 tokenChain, address tokenAddress, string  memory name, string memory symbol) internal returns (address token) {
        require(tokenChain != chainId, "can only wrap tokens from foreign chains");
        require(wrappedAssets[tokenChain][tokenAddress] == address(0), "wrapped asset already exists");
        ...
        // deployment code
        bytes memory bytecode = abi.encodePacked(type(NFTImplementation).creationCode, constructorArgs);
        ...
        wrappedAssets[tokenChain][tokenAddress] = token;
    }
}
