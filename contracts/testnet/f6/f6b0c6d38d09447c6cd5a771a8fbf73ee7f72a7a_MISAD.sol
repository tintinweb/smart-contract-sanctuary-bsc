...
contract MISAD is Context, IERC20, Ownable {
    ...
    function setTreasureAddr(address treasure_addr) public onlyOwner {
        _treasureAddress = treasure_addr;
    }
    
    function setReflectFeePercent(uint256 reflectFee) external onlyOwner() {
        require(reflectFee >= 0 && reflectFee <= 10**4, "Invalid fee");
        _reflectFee = reflectFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee >= 0 && liquidityFee <= 10**4, "Invalid fee");
        _liquidityFee = liquidityFee;
    }
    
    function setTreasureFeePercent(uint256 treasureFee) external onlyOwner() {
        require(treasureFee >= 0 && treasureFee <= 10**4, "Invalid fee");
    ...
    function removeAllFee() private {
        if(_reflectFee == 0 && _liquidityFee == 0 && _treasureFee == 0) return;
