...
contract MainPool is Ownable {
    ...
    //设置燃烧费率
    function setBurnFee(uint32 _burnFee) external onlyOwner {
        burnFee = _burnFee;
    }

    //获取燃烧费率
    function getBurnFee() external view returns(uint32) {
        return burnFee;
    }

    //设置收益费率
    function setIncomeFee(uint32 _incomeFee) external onlyOwner {
        incomeFee = _incomeFee;
    }

    //获取收益费率
    function getIncomeFee() external view returns(uint32) {
        return incomeFee;
    }
    ...
    function getBurnPool () external returns(address) {
        return burnPool;
    }

    function setIncomePool (address _incomePool) external onlyOwner {
    ...
    //计算手续费并返回  burn income
    function _handlefee(uint256 _amount) internal returns(uint256, uint256) {
        require(_amount > 1000," Insufficient balance, At least 1000 ...");
        uint256 iBurn = _amount / 100 * burnFee;
        uint256 iIncome = _amount / 100 * incomeFee;
        return (iBurn, iIncome);
    }

    //计算手续费
    function Handlefee(address _sender, uint256 _amount) external returns(uint256, uint256) {
        return _handlefee(_amount);
    }
}