...
library Counters {
  ...
  function reset(Counter storage counter) internal {
    counter._value = 100000; // Starting value set at 100000
  }
}
...
import "./PentasNFT.sol";
import "./MarketplaceCustodial.sol";

contract MarketplaceV5 is
  Initializable,
  ContextUpgradeable,
  OwnableUpgradeable
{
  ...
  function setSalePrice(uint256 tokenId, uint256 price) external {
    ...
    // Emit event
    emit PriceUpdated(tokenId, _pentasNFT.ownerOf(tokenId), price);
  }
  ...
  function makeOffer(uint256 tokenId) external payable {
    ...
    if (currentBidder == address(0)) {
      ...
      MarketplaceCustodial _custodialContract = MarketplaceCustodial(
        _custodial
      );
  ...
  function cancelOffer(uint256 tokenId) external {
    ...
    MarketplaceCustodial _custodialContract = MarketplaceCustodial(_custodial);
   ...
   */
  function tokenBurn(uint256 tokenId) external {
    // Only allow call from Pentas ERC-721 contract
    require(
      _msgSender() == address(_pentasNFT),
  ...
  function acceptOffer(uint256 tokenId) external {
    ...
    uint256 netRoyalty;
    address minter;
    (minter, netRoyalty) = _pentasNFT.royaltyInfo(tokenId, currentOffer);
    ...
    _pentasNFT.safeTransferFrom(_msgSender(), buyer, tokenId);
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./Counters.sol";
import "./MarketplaceV5.sol";
import "./IMarketplace.sol";

contract PentasNFTV3 is
    Initializable,
    ERC721Upgradeable,
    ERC721EnumerableUpgradeable,
    ERC721URIStorageUpgradeable,
    ...
    OwnableUpgradeable
{
     ...
     * @dev Marketplace contract
     */
    MarketplaceV5 private _marketplace; // Not in use. Need to remain here due to contract upgrade.
    ...
    function safeMint(string memory ipfsCID, uint32 royalty) external {
        require(
            royalty <= 90000,
            "Pentas: Royalty should not be more than 90%"
        );
    ...
    function approveMarket(uint256 tokenId, uint256 price) external {
        ...
        // Call marketplace contract to set selling price
        PentasMarketplace(_marketplaceAddress).setSalePrice(tokenId, price);
    }
    ...
    function burn(uint256 tokenId) public override {
        ...
        require(
            ...
            "Pentas: Sender is not the token owner"
        );

        // Cancel offer if exist
        PentasMarketplace(_marketplaceAddress).tokenBurn(tokenId);
