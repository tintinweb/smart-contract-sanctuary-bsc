...
contract ROI {
    ...
    function deposit(uint256 _amount, address _upline) public payable {
        ...
        collect(msg.sender);
        if (investorsMap[_upline].deposits.length == 0) {
            require(
		...
		for (uint256 i = 0; i < investor.deposits.length; i++) {
            DepositStruct storage dep = investor.deposits[i];
					...
					if (from < to) {
                        ...
                        if(dep.withdrawn.add(dividends)>dep.amount.mul(MAX_LIMIT).div(percentDivider)){
                            dividends =  dep.amount.mul(MAX_LIMIT).div(percentDivider).sub(dep.withdrawn);
                        }
    ...
    function getAvailabel(address _addr)  external view returns (uint256)
    {
        ...
        uint256 profit = investor.totalInvested.mul(MAX_LIMIT).div(percentDivider);
        if(amount.add(investor.totalWithdrawn) > profit) {
            amount = profit.sub(investor.totalWithdrawn);
        }
        return amount;
