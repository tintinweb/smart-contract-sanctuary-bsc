...
contract presale {
    ...
    function buyTokenbnb() public payable {
        ...
        SNOW.transferFrom(owner, msg.sender, numberOfTokens);
    ...
    function buyTokenbtc(uint256 amount) public {
        require(
            block.timestamp >= preSaleStartTime &&
                block.timestamp < preSaleEndTime,
            "PRESALE: PreSale time not met"
    ...
    function buyTokeneth(uint256 amount) public {
        require(
            block.timestamp >= preSaleStartTime &&
                block.timestamp < preSaleEndTime,
            "PRESALE: PreSale time not met"
    ...
    function bnbToToken(uint256 _amount) public view returns (uint256) {
        ...
        return numberOfTokens * (10**(SNOW.decimals()))/1e8;
