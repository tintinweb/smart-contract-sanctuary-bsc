...
contract Presale is ReentrancyGuard {
    ...
    Counters.Counter public _purchasedPackListId ;
    ...
    PurchasedPack[]  public   purchasedPackList ;
    ...
    {
        string memory tempCode = substring( Strings.toString( deterministicPseudoRandomDNA( 0 , walletAddress ) ) , 0, 18   );
    ...
    {
        codes[code] = InfluencerData(discount,assignment,wallet);
    }
    ...
    */
    ...
    {
        require( price >= 10000000000000000 ,"Monto Erroneo" );
    ...
    */
    ...
    public payable nonReentrant returns (PurchasedPack[] memory)
    {
        ...
        for (uint8 i = 0; i < packetList[packetId].treasureNftPerPackage ; i++)
        {
            ...
            uint256 dna = deterministicPseudoRandomDNA(  block.number + _purchasedPackListId.current() + tokenId , msg.sender  );
            
            uint8 rarity = 1;
            if(i==0){
                rarity = packetList[packetId].rarity;
            }else{
                rarity = getRarityByDna(packetList[packetId].rarityChance, packetList[packetId].rarity, _getDNASection( dna, i ) )  ;
            }

            created[ i ] =  PurchasedPack( tokenId ,rarity, dna ,  msg.sender)   ;  
            purchasedPackList.push( created[ i ]  ) ;  
            _purchasedPackListId.increment();
        }
    ...
    {   
        ...
        if(dnaSegment <= probability){
            ...
            return (rarity -1 >= 0)? rarity -1:0  ;
        }else{
            return (rarity -2 >= 0)? rarity -2:0  ;
        }
    }
