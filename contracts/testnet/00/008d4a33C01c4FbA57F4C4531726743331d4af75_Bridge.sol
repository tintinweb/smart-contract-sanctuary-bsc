...
contract Bridge is BscBank, OwnableUpgradeable, Validator, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    event ChangeFeeConfig(uint256 _index, uint256 _fee);
    event SetFeeForAddress(address _user, uint256 _index);
    ...
    function changeBurnAddress(address _burn) public isTimeLock {
        burnAddress = _burn;
        emit ChangeBurnAddress(_burn);
    }

    function addFeeConfig(uint256 _fee) public isTimeLock {
        feeConfig.push(_fee);
    }

    function changeFeeConfig(uint256 _index, uint256 _fee) public isTimeLock {
        require(_index < feeConfig.length - 1, "NOTOK");
        feeConfig[_index] = _fee;
        emit ChangeFeeConfig(_index, _fee);
    }

    function setFeeForAddress(address _user, uint256 _index) public isTimeLock {
        fees[_user] = _index;
        emit SetFeeForAddress(_user, _index);
    }
    ...
    function _verify(bytes32 _digest, bytes memory _signature) public view returns (bool) {
        console.log(operator);
        console.log(ECDSA.recover(_digest, _signature));
        return operator == ECDSA.recover(_digest, _signature);
    }
    ...
    function burn(
        ...
        // BEP20 deposit
            ...
            BridgeToken(_token).transferFrom(msg.sender, burnAddress, _amount);
    ...
    function mint(
        ...
        if (_tokenAddress == address(0)) {
            require(burnAddress.balance >= _amount, "Insufficient balance.");
        } else {
            require(BridgeToken(_tokenAddress).balanceOf(burnAddress) >= _amount, "Insufficient ERC20 balance.");
        }

        uint256 _feeIndex = fees[_recipient];
        uint256 _feePercent = feeConfig[_feeIndex];
        uint256 _fee = (_amount * _feePercent) / BONE;
        mintFunds(_recipient, payable(feeReceiver), _tokenAddress, _amount, _fee, _interchainTX);
    }
    ...
    function emergencyWithdraw(
        ...
        bytes32 hash = getWithdrawHash(_tokenAddress, _amount);
        require(_verify(hash, _signature), "Invalid signature");
        // Check if it is BNB
        if (_tokenAddress == address(0)) {
    ...
    function refund(
        ...
        // Check if it is BNB
        if (_tokenAddress == address(0)) {
            require(burnAddress.balance >= _amount, "Insufficient BNB for delivery.");
        } else {
...
contract BscBank {
    ...
    function mintFunds(
        ...
        if (_token == address(0)) {
            ...
            BridgeToken(_token).transfer(_recipient, _amount - _fee);
            BridgeToken(_token).transfer(_owner, _fee);
        }
