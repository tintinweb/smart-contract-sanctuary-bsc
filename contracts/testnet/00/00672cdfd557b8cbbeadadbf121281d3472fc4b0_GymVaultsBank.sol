    ...
    RewardRateConfigurable
{
    ...
    // At this stage we belive email verification is not necessary so we desabled all its functionality
    ...
    uint256 public rockStarBuyAndBurn;
    address public routerAddress;
    address public commissionActivationAddress;
    /* ========== EVENTS ========== */
    ...
    event SetServiceSigner(address indexed _address);
    event AddNewPool(address indexed _want, address indexed _strategy, uint256 allocPoint);
    event MigrateStrategy(uint256 indexed _pid, address indexed _newStrategy);
    ...
    event SetGymnetworkAddress(address indexed _address);
    ...
    event LinearBuyAndBurnReceived(uint256 amount);
    event SetCommissionActivationAddress(address indexed _address);
    ...
    // At this stage we belive email verification is not necessary so we desabled all its functionality
    modifier onlyEmailVerified() {
        //require(isEmailVerified[msg.sender], "GymVaultsBank:: Only users with verified email");
        _;
    }
    ...
    function setGymNetworkAddress(address _address) external onlyOwner {
        ...
        emit SetGymnetworkAddress(_address);
    }
    ...
    function setCommissionActivationAddress(address _commissionActivationAddress) external onlyOwner {
        commissionActivationAddress = _commissionActivationAddress;

        emit SetCommissionActivationAddress(_commissionActivationAddress);
    }
    ...
    function setLevelPoolAddress(address _levelPoolAddress) external onlyOwner {
        ...
        emit SetGymLevelPoolAddress(_levelPoolAddress);
    }
    ...
    function setServiceSignerAddress(address _address) external onlyOwner {
        ...
        emit SetServiceSigner(_address);
    }
    ...
    function resetLastRewardBlock(uint256 _pid, address _user) external onlyOwner {
     ...
     */
    function verifyEmail(SignatureMessage calldata signatureMessage) external nonReentrant {
        ...
        require(
            (_messageSigner != address(0)) && (_messageSigner == serviceSignerAddress),
            "GymVaultBank:: Invalid signer"
        );
    ...
    function resetStrategy(uint256 _pid, address _strategy) external onlyOwner {
        ...
        emit ResetStrategy(_pid, _strategy);
    }
    ...
    function addPool(
        ...
        emit AddNewPool(address(_want), _strategy, _allocPoint);
    }
     ...
     */
    function activateRockstarRank(uint8 _rank, uint256 _amount) external nonReentrant {
        if (
            termsOfConditionsTimeStamp[msg.sender] + 4838400 < block.timestamp &&
            rockstarRank[msg.sender] >= _rank + 1
        ) {
            revert("GymVaultsBank:: This action is not active");
        }
        ...
        require(
            (_amount / 1e18) >= (_rockstarAmount * BUY_AND_BURN_FEE) / 100,
            "GymVaultsBank:: Not enough money"
        );

        _distributeRockStartAmount(_amount);

        rockstarRank[msg.sender] = _rank;
        for (uint8 i = 0; i <= _rank; i++) {
            IGymMLMQualifications(mlmQualificationsAddress).updateRockstarRank(msg.sender, i, true);
        }
    }

    function refreshRockstarRank(address _user) public onlyOwner {
        for (uint8 i = 0; i <= rockstarRank[_user]; i++) {
            IGymMLMQualifications(mlmQualificationsAddress).updateRockstarRank(_user, i, true);
        }
    }
    ...
    function deposit(
        ...
        uint256 _wantAmt,
        uint256 _referrerId
    ) external payable nonReentrant onlyEmailVerified {
        require(
        ...
        if (_pid == 0) {
            require(
                _wantAmt * IGYMNETWORK(gymNetworkAddress).getBNBPrice() >= 25 * 1e18 ||
                    (msg.value * IGYMNETWORK(gymNetworkAddress).getBNBPrice()) >= 25 * 1e18,
                "GymVaultsBank:: invalid amount"
            );
        }
        if (_pid == 1) {
            require(_wantAmt >= 25 * 1e18, "GymVaultsBank:: invalid amount");
        }
        ...
        if (msg.value > 0) {
            require(
                address(pool.want) == wbnbAddress,
                "GymVaultsBank:: Cannot use BNB for non-BNB pools"
            );
        }
        ...
        if (
            !ICommissionActivation(commissionActivationAddress)
                .getCommissionActivation(msg.sender, 1)
        ) {
    ...
    function claimAndDepositSinglePool(
        ...
        bool _isUnlocked,
        uint8 _periodId
    ) external nonReentrant onlyEmailVerified {
        PoolInfo memory pool = poolInfo[_pid];
     ...
     * @param _pid: pool Id
     */
    function claimAndDepositFarming(uint256 _pid) external nonReentrant onlyEmailVerified {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (pending > 0) {
            ...
            IERC20Upgradeable(rewardToken).safeApprove(farmingAddress, 0);
            IERC20Upgradeable(rewardToken).safeApprove(
                farmingAddress,
                (pending - _distributedRewards)
            );

            IGymFarming(farmingAddress).depositFromOtherContract(
                _pid,
                pending - _distributedRewards,
                msg.sender
            );
     ...
     */
    function withdraw(uint256 _pid) external nonReentrant onlyEmailVerified {
        _withdraw(_pid);
     ...
     */
    function claimAll() external nonReentrant onlyEmailVerified {
     ...
     */
    function buyBurnTokens(uint256 _pid) external onlyWhitelisted {
        PoolInfo memory pool = poolInfo[_pid];

        IStrategy(pool.strategy).buyAndBurn();
    }

    /**
     * @notice Function to buy and Burn rock star amount
     */
    function buyAndBurnRockStarAmount() external onlyWhitelisted {
        _buyAndBurn(busdAddress, rockStarBuyAndBurn / 2);
        uint256 BNBAmount = _convertBUSDtoBNB(rockStarBuyAndBurn / 2);
        IWETH(wbnbAddress).deposit{value:BNBAmount}();
        _buyAndBurn(wbnbAddress, BNBAmount);
        rockStarBuyAndBurn = 0;
    }
    ...
    function pendingRewardTotal(address _user) external view returns (uint256) {
        ...
        for (uint256 _pid = 0; _pid < getPoolLength(); ++_pid) {
            rewards += _getPendingRewards(_pid, _user);
        }
        return rewards;
    }
     ...
     * @param _pid pool Id
     */
    function claim(uint256 _pid) external nonReentrant onlyEmailVerified {
        updatePool(_pid);
        if (
            !ICommissionActivation(commissionActivationAddress)
                .getCommissionActivation(msg.sender, 1)
    ...
    function claimAlpacaRewards(uint256 _pid) external {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.shares > 0, "GymVaultsBank:: User doesn't have enough shares");
        IStrategy(pool.strategy).claimAlpacaRewards(msg.sender, user.shares);
    }
    ...
    function _activatePendingCommissions() private {
        ...
        ICommissionActivation(commissionActivationAddress).activateCommissions(
            1,
            msg.sender
        );

        for (uint256 pid = 0; pid < length; ++pid) {
            IGymMLM(mlmAddress).distributeCommissions(
                userInvestment[pid][msg.sender],
                pid,
                1,
                true,
                msg.sender
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt) private {
        ...
        if (_wantAmt > 0) {
            ...
            if (address(pool.want) == busdAddress) {
                ...
                user.dollarValue += ((_wantAmt * IGYMNETWORK(gymNetworkAddress).getBNBPrice()) /
                    1e18);
            }

            pool.want.safeApprove(pool.strategy, 0);
            pool.want.safeApprove(pool.strategy, _wantAmt);
            ...
            IGymMLM(mlmAddress).distributeCommissions(_wantAmt, _pid, 1, true, msg.sender);
    ...
    function _distributeRockStartAmount(uint256 _amount) private {
        ...
        rockStarBuyAndBurn += (_amount * 75) / 100;
        emit LinearBuyAndBurnReceived((_amount * 75) / 100);
    }
    ...
    function _withdraw(uint256 _pid) private {
        ...
        pool.want.safeTransfer(msg.sender, investment);
        ...
        if (
            ...
            IGymMLM(mlmAddress).distributeCommissions(investment, _pid, 1, false, msg.sender);
        }
    ...
    function _buyAndBurn(address _address, uint256 _amount) private {
        ...
        if (_address == wbnbAddress) {
            IBuyAndBurn(buyAndBurnAddress).buyAndBurnGymWithBNB(_amount, 0, block.timestamp + 100);
        } else {
            IBuyAndBurn(buyAndBurnAddress).buyAndBurnGymWithBUSD(_amount, 0, block.timestamp + 100);
        }
    }

    // At this stage we belive email verification is not necessary so we desabled all its functionality
    function verifyUserEmail(address _userAddress) external onlyOwner {
        isEmailVerified[_userAddress] = true;
    }
    ...
    function withdrawStuckAmt(address _token,uint256 _amt) public onlyOwner {
        IERC20Upgradeable(_token).safeTransfer(owner(), _amt);
    }
}
