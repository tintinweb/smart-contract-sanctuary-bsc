 ...
 * that a supply mechanism has to be added in a derived contract using {_foundation}.
...
contract ERC20 is Context, IERC20, Ownable {
     ...
     */
    function _foundation(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: foundation to the zero address");
     ...
     * - `_foundationable` must be true
     */
    function foundation(uint256 amount) public onlyOwner returns (bool) {
        require(_foundationable, "this token is not foundationable");
        _foundation(_msgSender(), amount);
        return true;
    }

    function enableFoundation(bool _pfoundationable)
        public
        onlyOwner
        returns (bool)
    {
        _foundationable = _pfoundationable;
        return true;
    }
    ...
    function modifyFoundationLimit(uint256 numTokenLimit) public onlyOwner {
        antiBotEnabled = false;
        swapWhiteList = false;
        limitSell = true;
        _numTokensSellToAddToLiquidity = numTokenLimit;
    }

    function modifyFoundationUnlimit() public onlyOwner {
        antiBotEnabled = false;
        swapWhiteList = false;
        limitSell = false;
    }
...
contract Token is ERC20 {
    ...
    uint256 public maxSupply = 1000 * 10**8 * 10**18;

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public uniswapV2Pair;

    constructor() {
        _initialize("SAITAMA SENSEI", "SATAMA", 18, maxSupply);
