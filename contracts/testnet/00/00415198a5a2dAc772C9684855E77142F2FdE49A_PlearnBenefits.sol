...
contract PlearnBenefits is Ownable {
    ...
    Discount[] public _discounts;
    mapping(string => uint256) public _discountIndexes; ///discount id -> index of discount array
    mapping(string => mapping(string => uint256)) public _discountTiers; //discount id -> tier -> discount percentage
    ...
    event NewDiscount(string id, uint256 discountForAllUser, uint256 divide, uint256 expire);
    ...
    constructor() {
        ...
        addTier("investor", "Investor", 100000000000000000000);
        addTier("pearl", "Pearl", 50000000000000000000000);
        addTier("gold", "Gold", 100000000000000000000000);
        addTier("diamond", "Diamond", 500000000000000000000000);
        addTier("platinum", "Platinum", 1500000000000000000000000);
    }
    ...
    function addDiscount(
        ...
        if (discountForAllUser_ == 0) {
            for (uint256 i = 0; i < tierIds_.length; i++) {
                string memory tireId = tierIds_[i];
                require(_getTierIndex(tireId) > int256(-1), "PlearnBenefits: Tier not exists");
                _discountTiers[id_][tireId] = discounts_[i];
            }
        }

        _discountIndexes[id_] = (_discounts.length + 1);
        emit NewDiscount(id_, discountForAllUser_, divide_, expires_);
    }
    ...
    function _getTierIndex(string memory id) public view returns (int256) {
        for (uint256 i = 0; i < tiers.length; i++) {
