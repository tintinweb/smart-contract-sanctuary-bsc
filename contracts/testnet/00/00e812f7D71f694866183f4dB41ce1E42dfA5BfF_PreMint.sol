...
contract PreMint is PirateWarAccessControl {
    uint256 public _premintAmount = 0.01 ether;
     ...
     */
    function mintNewCharacters(uint256 qty) public whenNotPaused {
        // Check busd balance
        uint256 tokenBalance = IERC20(_tokenContract).balanceOf(address(this));
        uint256 tokenAmountToPay = qty * _premintAmount;
        ...
        IPirateWarCharacter(_nftContract).summonNewPirates(msg.sender, qty);
    } 
     ...
     */
    function setPremintAmount(uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(amount > 0, "Amount must be greater than 0");

        _premintAmount = amount;
    }

    /**
     * @dev Set the NFT contract address
     */
    function setNFTContract(address nftContract) public onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setPremintingTime(uint256 startTime, uint256 endTime) public onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function collectSales() public onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 tokenBalance = IERC20(_tokenContract).balanceOf(address(this));
...
contract PirateWarAccessControl is AccessControl, Pausable {
...
interface IPirateWarCharacter {
    function getUserPirates()
        external
        view
        returns (uint256[] memory);

    function summonNewPirate(address to) external;
    function summonNewPirates(address to, uint256 count) external;
}
