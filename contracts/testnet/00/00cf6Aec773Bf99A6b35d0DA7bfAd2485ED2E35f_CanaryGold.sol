...
contract CanaryGold is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = 'Canary Gold';
    string private _symbol = 'CGLD';
    ...
    uint256 internal _tokenTotal = 200_000_000e18;
    ...
    uint256 public _reflectionFee = 700; // 7%
    uint256 public _burnFee = 300; // 3%
    ...
    uint256 public maxTxAmount = _tokenTotal.div(10000); // 0.01%
    ...
    function reflectionFromToken(uint256 tokenAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            ...
            return tokenAmount.sub(tokenAmount.mul(_reflectionFee).div(10**_feeDecimal + 2)).mul(_getReflectionRate());
        }
    }
    ...
    function _transfer(
        ...
        require(amount <= maxTxAmount || isWhitelisted[sender] || isWhitelisted[recipient], 'Transfer Limit Exceeded');

        uint256 transferAmount = amount;
        uint256 rate = _getReflectionRate();

        if (isFeeActive && !isWhitelisted[sender] && !isWhitelisted[recipient]) {
    ...
    function collectFee(
        ...
        //@dev tax fee
        if (_reflectionFee != 0) {
            uint256 taxFee = amount.mul(_reflectionFee).div(10**(_feeDecimal + 2));
    ...
    function setWhitelistAccount(address account, bool value) external onlyOwner {
