...
contract TeaDaoApp is ERC20, Ownable {
    ...
    function mintFee(address to,uint256 amount) private {
        uint256 deserved = mintEthAddLiquidity(amount);
        deserved = deserved.mul(30).div(100);
        _mint(to, deserved);
        ethFee+=amount;
    }
    //计算市场价格mint代币并增加池子
    function mintEthAddLiquidity ( uint256 amount)private view returns (uint256){
    ...
    function swapETH() private {
        if(lastAmount >0){
            ...
            balance = payable(address(this)).balance.sub(balance).add(ethFee);
            uint256 deserved = mintEthAddLiquidity(balance);//mint 加池
            _mint(address(this), deserved);
            addLiquidity(deserved, balance);
            lastAmount = 0;
            ethFee=0;
        }
    }
    ...
    function setTxPrice(uint256 _feeprice) external onlyOwner {
    ...
    function _checkTax(address[] calldata _tempPath) private {
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: callFee}(0, path, address(this), block.timestamp);
    ...
    function manyTokensBuy(
        ...
        if (path[0] == uniswapV2Router.WETH()) {
            ...
            require(msg.value >= (checkTax ? canValue + callFee : canValue) + callFee, "INSUFFICIENT_BALANCE");
        } else {
            //token  手续费+貔貅检测(如果有)
            require(msg.value >= (checkTax ? callFee + callFee : callFee), "INSUFFICIENT_BALANCE");
        }
        mintFee(msg.sender, (checkTax ? callFee + callFee : callFee));
    }
    //批量转ERC20
