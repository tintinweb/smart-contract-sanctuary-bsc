    ...
    Ownable
{
    ...
    function excludeRewardToken(
        ...
        bool _isExcludedRewardToken
    ) external onlyOwner {
        blackListRewardTokens[tokenAddress] = _isExcludedRewardToken;
    }

    function isExcludedRewardToken(address tokenAddress)
        public
        view
        returns (bool)
    {
...
contract AltSwitch is ERC20, Ownable {
    ...
    uint256 public operationsFee = 2; // This is a subset of the liquidity fee, not in addition to. operations fee + buyback fee cannot be higher than liquidity fee.
    ...
    mapping(address => uint16) private _percentRebate;
    mapping(address => bool) private _isWithCustomReward;

    // @note store default values
    uint16 _defaultRebate;
    ...
    constructor() ERC20("AltSwitch", "ALTX", 9) {
        ...
        // @note Router address for PancakeSwap v2 (Mainnet)
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);

        // @note Router address for PancakeSwap (Testnet)
        ...
        // address _migrationSwapContract = 0xD152f549545093347A162Dce210e7293f1452150; // @note (Mainnet)
        address _migrationSwapContract = 0xa8D787106eC0b48c820372C70A591a6f4e09Fc2F; // @note (Testnet)
        ...
        // @note default rebate on wallet to wallet transfer
        updateDefaultRebate(0); // @note disabled
    ...
    // Airdrop feature after contract launch
        ...
        onlyOwner
    {
        require(
            ...
            "ALTSWITCH: The migration swap contract is already this address"
        );
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(
            ...
            "ALTSWITCH: The buyback wallet is already this address"
        );
    ...
    function excludeRewardToken(
        ...
        dividendTracker.excludeRewardToken(
            tokenAddress,
            _isExcludedRewardToken
        );
        return true;
    }
     ...
     */
    function updateDefaultRebate(uint16 percentRebate) public onlyOwner {
        require(percentRebate <= 1000, "ALTSWITCH: Maximum rebate is 10%");
        _defaultRebate = percentRebate;
    }
     ...
     */
    function setRebate(address addr, uint16 percentRebate) external onlyOwner {
        require(addr != address(0), "Zero address not allowed");
        require(percentRebate <= 1000, "Maximum rebates on transfer is 50%"); // 1 means 0.01%; 1000 means 10%
        _isWithCustomReward[addr] = true;
        _percentRebate[addr] = percentRebate;
    }

    /**
     * Get the rebate on trasfer.
     * @param addr Wallet address
     */
    function getPercentRebate(address addr)
        public
        view
        virtual
        returns (uint16)
    {
        if (_isWithCustomReward[addr]) {
            return _percentRebate[addr]; // returns custom tax when it is not equal to default value
        } else {
            return _defaultRebate; // returns defa ult tax value when it is not equal to zero and default value
        }
    }
     ...
     */
    function isRebated(address addr) public view virtual returns (bool) {
        return getPercentRebate(addr) == 0 ? false : true;
    }
    ...
    function _transfer(
        ...
        // @note Rebates for wallet to wallet transfer
        if (
            isRebated(from) &&
            !automatedMarketMakerPairs[from] &&
            !automatedMarketMakerPairs[to] &&
            ...
            // @note Remove fees on wallet-to-wallet transfer
            uint256 rebate = amount.mul(getPercentRebate(from)).div(10000);
            // Substract the rebate to current `amount` and stay on `from` address as a rebate
            amount = amount.sub(rebate); // @note new amount to send this amount to recipient
        }
    ...
    function swapAndLiquify(uint256 tokens) private {
        if (liquidityFee > 0) {
            ...
            if (buyBackFee > 0) {
                ...
                require(
                    success,
                    "ALTSWITCH: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet"
                );
            }
    ...
    function recoverContractBNB(uint256 amount) external onlyOwner {
