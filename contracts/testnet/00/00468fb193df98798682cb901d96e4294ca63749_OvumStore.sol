...
import "./IDOWithReward.sol";

contract OvumStore is ERC1155PresetMinterPauserUpgradeable, TimeSkippable, InvestorManager {
  using SafeMathUpgradeable for uint256;
  using CountersUpgradeable for CountersUpgradeable.Counter;
  CountersUpgradeable.Counter private packageCounter;
  CountersUpgradeable.Counter private ovumCounter;
  bytes32 public constant GATEWAY_ROLE = keccak256("GATEWAY_ROLE");
  string public name;
  string public symbol;

  mapping(uint8 => Package) public idToPackage;
  mapping(address => BoughtPackage[]) public boughtPackages;
  mapping(uint256 => uint256) private _totalSupply;

  uint8 public ovumLimit;
  uint8 public daysOnMonth;
  uint8 ovumPerDay;
  uint8 ovumLevel;
  uint8 boxQuantityLimit;
  uint8 boxTokenLimitId;
  uint256 defaultPrice;
  uint256 defaultLimit;
  IERC20 public token;

  function addPackageToStore(uint8 id, uint256 price, uint256 quantity) internal onlyInitializing {
    idToPackage[id] = Package(id, ovumLevel, ovumPerDay, price, quantity);
  }
  ...
  function initialize(IERC20 _token) public initializer {
    ...
    for (uint8 id = 1; id <= boxTokenLimitId; id++) {
      addPackageToStore(id, defaultPrice, defaultLimit);
    }
    token = _token;
    initTimeSkip();
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155PresetMinterPauserUpgradeable, InvestorManager) returns (bool) {
    return
  ...
  event BuyPackage(uint8 id, uint256 price, uint256 quantity);
  ...
  * @dev Total amount of tokens in with a given id.
  ...
  function mintFromBoughtPackages(address from) internal {
    if (from == address(0)) return;
    BoughtPackage[] memory _boughtPackages = boughtPackages[from];
    for (uint256 index = 0; index < _boughtPackages.length; index++) {
      BoughtPackage memory boughtPackage = _boughtPackages[index];
      uint256 amount = getNow().sub(boughtPackage.startTime).div(1 days);
      if (amount > daysOnMonth) amount = daysOnMonth;
      if (amount.sub(boughtPackage.latestMint) > 0) {
        uint256 mintBalance = amount.sub(boughtPackage.latestMint);
        _mint(from, boughtPackage.tokenId, mintBalance, "");
        boughtPackages[from][index].latestMint += mintBalance;
      }
    }
  }

  function updatePackage(uint8 id, uint256 price, uint256 quantity) external onlyRole(ADMIN_ROLE) {
    require(id > 0 && id <= boxTokenLimitId, "Id is invalid");
    idToPackage[id].quantity = quantity;
    idToPackage[id].price = price;
  }

  function buyMultiplePackages(uint8 id, uint8 quantity, address presenterAddress) external {
    require(quantity > 0, "Quantity is invalid");
    for (uint256 count = 1; count <= quantity; count++) {
      buyPackage(id, presenterAddress);
    }
  }

  function buyPackage(uint8 id, address presenterAddress) public {
    require(id > 0 && id <= boxTokenLimitId, "Id is invalid");
    require(getOvumByBoughtPackages(_msgSender()).add(1) <= boxQuantityLimit, "The number of boxes cannot exceed the limit");
    Package memory storePackage = idToPackage[id];
    ...
    idToPackage[id].quantity--;

    packageCounter.increment();
    uint256 _boughtId = packageCounter.current();
    uint256 mintedQuantity = 0;
    boughtPackages[_msgSender()].push(BoughtPackage(_boughtId, id, mintedQuantity, getNow()));
    emit BuyPackage(storePackage.id, storePackage.price, storePackage.quantity);
  }

  function balanceOf(address account, uint256 id) public view override returns (uint256) {
    if (id > ovumLimit) return super.balanceOf(account, id);
    uint256 balance = super.balanceOf(account, id);
    uint256 availableBalance = getAvailableBalance(account, id);
    return balance.add(availableBalance);
  }

  function getAvailableBalance(address account, uint256 id) internal view returns (uint256) {
    uint256 availableBalance = 0;
    BoughtPackage[] memory _boughtPackages = boughtPackages[account];
    for (uint256 index = 0; index < _boughtPackages.length; index++) {
      BoughtPackage memory boughtPackage = _boughtPackages[index];
      if (boughtPackage.tokenId != id) continue;
      uint256 amount = getNow().sub(boughtPackage.startTime).div(1 days);
      if (amount > daysOnMonth) amount = daysOnMonth;
      if (amount <= boughtPackage.latestMint) continue;
      availableBalance += amount.sub(boughtPackage.latestMint);
    }
    return availableBalance;
  }

  function getOvumByBoughtPackages(address account) public view returns (uint256) {
    return boughtPackages[account].length;
  }
}
...
WithdrawUpgradeable, AccessControlEnumerableUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
  ...
  function _InvestorManager() public virtual initializer {
...
contract IDO is InvestorManager {
  ...
  uint256 public TOKEN_UNIT;
  uint256 public PAYOUT_EACH_CLAIM;
  uint256 public CLAIM_FEE;
  uint256 public totalPayout;
  uint256[] public rates;
  mapping(address => bool) public claimed;

  function _IDO(IERC20 _myToken) public initializer {
    _InvestorManager();
    TOKEN_UNIT = 1e9;
    PAYOUT_EACH_CLAIM = 1000 * TOKEN_UNIT;
    CLAIM_FEE = 0.00002 ether;
    totalPayout = 0;
    rates = [100, 15, 5, 5];
...
contract IDOWithReward is IDO {
  ...
  uint256 public erc1155Quantity;
  ...
  function _IDOWithReward(IERC20 _man, IERC20 _usdt, IERC20 _mon, IERC1155 _erc1155ForReward, Randomizer _randomizer) public initializer {
    _IDO(_man);
    ...
    erc1155TokenId = 0;
    erc1155Quantity = 1;
    COIN_REWARD_AMOUNT = 0.00002 ether;
    MAN_REWARD_AMOUNT = 25000 * 5 * 1e9;
    USDT_REWARD_AMOUNT = 50 * 1000000;
    MON_REWARD_AMOUNT = 25000 * 1e18;
  }
...
contract IDORanking is IDOWithReward {
  ...
  function _IDORanking(IERC20 _man, IERC20 _usdt, IERC20 _mon, IERC1155 _erc1155ForReward, Randomizer _randomizer) public initializer {
    _IDOWithReward(_man, _usdt, _mon, _erc1155ForReward, _randomizer);
  }
