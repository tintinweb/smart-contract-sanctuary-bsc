...
contract Elen is IElen, ReflectToken {
    ...
    uint256 public constant initialTotalSupply = 10_000_000_000 * 10 ** 18;
     ...
     */
    constructor(string memory name, string memory symbol) public ReflectToken(name, symbol, initialTotalSupply) {
    ...
    function withdraw() public onlyOwner {
        ...
        super._transfer(address(this), withdrawAddress, contractTokenBalance);
    }
    ...
    function _swapAndLiquifyOrCollect(uint256 contractTokenBalance) private lockTheSwap {
        _transfer(address(this), address(0x000aADbe0Bb60199134D99E63EB8ce815515D0De), contractTokenBalance);
...
abstract contract ReflectToken is Context, IERC20, Ownable {
    ...
    event LogToast(uint256 value, string msg);
    ...
    function transferFrom(
        ...
        _approve(
            sender,
            _msgSender(),
            _allowances[sender][_msgSender()].sub(amount, 'ReflectToken: transfer amount exceeds allowance')
        );
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external virtual returns (bool) {
        _approve(
            _msgSender(),
            spender,
            _allowances[_msgSender()][spender].sub(subtractedValue, 'ReflectToken: decreased allowance below zero')
        );
        return true;
    }
    ...
    function _transfer(
        ...
        uint256 max_limit = _tTotal.div(100);

        emit LogToast(max_limit, 'max_limit');
        emit LogToast(balance, 'balanceOf');
        emit LogToast(_tTotal, '_tTotal');

        require((balance + tAmount) <= max_limit, 'You are not allow to transafer more token ');

        (
            uint256 rAmount,
            uint256 rTransferAmount,
            uint256 rFee,
            uint256 rAccumulation,
            uint256 tTransferAmount,
            uint256 tFee,
            uint256 tAccumulation
        ...
        )
    {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rAccumulation) = _getRValues(
            tAmount,
            tFee,
            tAccumulation
        );
