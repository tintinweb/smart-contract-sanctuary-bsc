...
contract TestToken is IERC20, Ownable {
    ...
    // Router (Test Net) : 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    // Tax Fee           : 500
    // Liquidity Fee     : 300
    // Development Fee   : 400
    // Owner Address     : 0xaf60F8eea938e2A8e7109442F43EDc560473157A
    constructor(
        string memory name_,
        ...
        _developmentAddress = 0xB5a3C4430AB273B1312E436A4592AddeD0c7F985;
    ...
    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(
            _taxFee + _liquidityFee + _developmentFee <= 10**4 / 4,
        ...
        )
    {
        ...
        return (tTransferAmount, tFee, tLiquidity, tdevelopmentFee);
    }
    ...
    function _takedevelopmentFee(uint256 tCharity) private {
        if (tCharity > 0) {
            uint256 currentRate = _getRate();
            uint256 rCharity = tCharity.mul(currentRate);
            _rOwned[_developmentAddress] = _rOwned[_developmentAddress].add(rCharity);
            if (_isExcluded[_developmentAddress])
                _tOwned[_developmentAddress] = _tOwned[_developmentAddress].add(
                    tCharity
                );
            emit Transfer(_msgSender(), _developmentAddress, tCharity);
        }
    }
