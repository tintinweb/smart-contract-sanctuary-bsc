...
contract NitroLeague is INitroLeague, Context, Ownable, TokenWithdrawer {
    ...
    constructor(address game_, address treasuryWallet, address raceEventFactory_, address raceFactory_) {
        _game = game_;
    ...
    /// @param title as string to store event title.
    /// @param uri as string to hold race event title.
    /// @return address of new RaceEvent.
    function createRaceEvent(string calldata raceEventID, uint256 raceEventType, string memory title, string memory uri) external override returns(address) {
        ...
        IRaceEvent raceEvent = IRaceEvent(raceEventFactory.newRaceEvent(
            address(this), raceEventID, raceEventType, title, uri
        ));
...
contract RaceEvent is IRaceEvent, Context, Ownable, RewardManager, TokenWithdrawer {
    ...
    /// @param raceEventId_ as string for unique race event id.
    /// @param raceEventType_ as number for enum RaceEvent.RaceEventType.
    /// @param title_ as string to store event title.
    /// @param uri_ as string location of metadata.
    constructor(address nitroLeague_, string memory raceEventId_, uint256 raceEventType_, string memory title_, string memory uri_) {
        nitroLeague = INitroLeague(nitroLeague_);
        raceFactory = IRaceFactory(nitroLeague.getRaceFactory());
        transferOwnership(tx.origin);

        raceEventID = raceEventId_;
        title = title_;
        uri = uri_;
    ...
    function endRaceEvent(address payable[] memory results_) external override {
        ...
        require(
            _msgSender() == nitroLeague.getGame()
            || _msgSender() == owner(),
    ...
    /// @param raceTitle as string name of race.
    /// @param uri_ as string location of metadata.
