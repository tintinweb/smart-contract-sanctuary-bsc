...
contract DexGoRentAndKm is Ownable {
    ...
    function rent(uint256 _tokenId) public payable {
        ...
        require(!IDexGoStorage(storageContract).getInAppPurchaseBlackListWallet(msg.sender) && !IDexGoStorage(storageContract).getInAppPurchaseBlackListTokenId(_tokenId), "wallet or tokenId blacklisted");
        payable(IERC721(IDexGoStorage(storageContract).getNftContract()).ownerOf(_tokenId)).transfer(fixedAmountOwner);

        Address.sendValue(payable(IDexGoStorage(storageContract).getNftContract()), fixedAmountProject);
        IDexGoNFT(IDexGoStorage(storageContract).getNftContract()).distributeMoney(msg.sender, fixedAmountProject);
    ...
    function setKmForId(uint256 tokenId, uint256 km) public {
        require(msg.sender == IDexGoStorage(storageContract).getNftContract() || msg.sender == IDexGoStorage(storageContract).getGameServer() || msg.sender == owner(),'only admin accounts can change km');
