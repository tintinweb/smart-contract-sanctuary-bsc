...
contract PoolLogic is ReentrancyGuard {
    ...
    bool public disableWithdrawal;
    address[] public depositedToken;

    mapping(address => uint256) public balances;
    mapping(address => uint256) public indexOfInvestor;
    mapping(address => mapping(address => uint256)) public tokens;

    event Deposit(address from, uint256 value);
    event DepositToken(address token, address from, uint256 value);
    ...
    constructor(address _router, address _WBNB, address[] memory _tokenIn) {
        manager = msg.sender;
    ...
    function depositToken(address _tokenIn, uint _amountIn) external {
        TransferHelper.safeTransferFrom(_tokenIn, msg.sender, address(this), _amountIn);
        tokens[_tokenIn][msg.sender] += _amountIn;
        ...
        emit DepositToken(_tokenIn, msg.sender, _amountIn);
    }
    ...
    //     remove(indexOfInvestor[msg.sender] - 1);
    //     delete indexOfInvestor[msg.sender];
    ...
    function withdrawToken(address _tokenOut) external nonReentrant {
        require(!disableWithdrawal, "cannot withdraw");

        uint amount = tokens[_tokenOut][msg.sender];
        require(amount > 0 , "Balance insufficient");
        tokens[_tokenOut][msg.sender] = 0;

        remove(indexOfInvestor[msg.sender] - 1);
        delete indexOfInvestor[msg.sender];

        TransferHelper.safeApprove(_tokenOut, address(this), amount);

        emit WithdrawToken(_tokenOut, msg.sender, amount);
    }
    ...
    function remove(uint256 index) private {
        ...
        if (investors[index] == investors[investors.length-1]) {
            isLastIndex = true;
        }
        ...
        if (!isLastIndex) {
            indexOfInvestor[investors[investors.length-1]] = index + 1;
        }
    }
    ...
    function swapBNBToTokens(address _tokenOut,uint256 _amountIn, uint256 _amountOutMin) external onlyManager {
        ...
        emit Swap(WBNB, _tokenOut, _amountIn);
    }

    function swapExactTokensForTokens(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin) external {
