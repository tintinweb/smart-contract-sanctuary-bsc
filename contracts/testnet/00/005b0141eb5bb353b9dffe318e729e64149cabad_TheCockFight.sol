...
// REFERRALS
abstract contract CockFightReferrals
{
...
contract TheCockFight is Ownable, ReentrancyGuard {
    ...
    function safeclaim(uint256[] calldata roundIDs) internal{
     ...
     function claimed(uint256[] calldata roundIDs) external nonReentrant {
        safeclaim(roundIDs);
     }

    function claimedReferred(uint256[] calldata roundIDs, address referrer) external nonReentrant {
    ...
    function SetReferralsContract(address _referralsContract) external onlyAgent 
    {
        Referrals = CockFightReferrals(_referralsContract);
    }

    function SetReferralsRate(uint256 _rate) external onlyAgent 
    {
        referralRate = _rate;
    }
