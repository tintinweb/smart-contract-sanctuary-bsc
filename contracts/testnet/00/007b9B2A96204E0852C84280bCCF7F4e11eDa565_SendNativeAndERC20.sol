...
contract SendNativeAndERC20 is Ownable {
    ...
    event LogDistributeETH(
        address indexed from,
        uint256 amounts,
        address[] list,
        uint256 value
    );
    event LogDistributeERC20(
        address indexed from,
        address token,
        uint256 amounts,
        address[] list,
    ...
    function distributeETH(address[] calldata accounts) external payable {
        uint256 value = (msg.value - accounts.length) / accounts.length;
        for (uint256 i = 0; i < accounts.length; i++) {
            (bool success, ) = accounts[i].call{value: value}(new bytes(0));
            require(success, "TransferHelper: ETH_TRANSFER_FAILED");
        }
        emit LogDistributeETH(msg.sender, msg.value, accounts, value);
    }
    ...
    function distributeERC20(
        ...
        uint256 value = (amounts - accounts.length) / accounts.length;
        for (uint256 i = 0; i < accounts.length; i++) {
            token.transferFrom(msg.sender, accounts[i], value);
        }

        emit LogDistributeERC20(msg.sender, address(token), amounts, accounts, value);
    }
