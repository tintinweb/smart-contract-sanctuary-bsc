...
import "./interfaces/ITreasuryDeposit.sol";
...
/// [Team & Advisors]        : 17.4% ➔ Direct unlock 1.45%, then unlock 1.45% more every three months.
/// [Marketing]              : 10%
/// -----------------------------------------------------------------------------------------------------------
/// [Token Sale]             : 28.4%
/// ●------------[Seed]      : 10.6% ➔ Unlock 1.325% every three months.
    ...
    ITreasuryDeposit
{
    ...
    uint256 internal constant LIQUIDITY_MAX_TOTAL_AMOUNT = 57500000e18; // 23%
    uint256 internal constant FOUNDATION_MAX_TOTAL_AMOUNT = 53000000e18; // 21.2%
    uint256 internal constant MARKETING_MAX_TOTAL_AMOUNT = 25000000e18; // 10%
    uint256 internal constant TOKEN_SALE_MAX_TOTAL_AMOUNT = 71000000e18; // 28.4%
    ...
    uint256 VESTION_MAX_TOTAL_AMOUNT = 43500000e18;
    uint256 VESTION_AMOUNT = 3625000e18;
    ...
    constructor() ERC20("Lebo", "LEBO") {
        ...
        /// Team and Advisor
        uint256 _teamCliff = 8 hours;
        team_ = _msgSender();

        _addPhase(team_, creationTime_, false);
        _addPhase(team_, creationTime_ + (_teamCliff * 1), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 2), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 3), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 4), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 5), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 6), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 7), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 8), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 9), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 10), false);
        _addPhase(team_, creationTime_ + (_teamCliff * 11), false);
        /// Direct unlock 2.175% for team and advisor
        _unlockTeam(team_, VESTION_AMOUNT, 0);
    }
    ...
    function unLockLiquidity(uint256 amount) public onlyOwner {
        ...
        if (totalAmount > LIQUIDITY_MAX_TOTAL_AMOUNT) {
    ...
    function unLockMarketing(uint256 amount) public onlyOwner {
        ...
        if (totalAmount > MARKETING_MAX_TOTAL_AMOUNT) {
            revert ExceedsBalance();
        }
    ...
    function liquidityDeposit(address from, uint256 amount)
        external
        override
        returns (bool)
    {
        require(treasury_ != address(0), "Treasury is zero address");
        liquidityTotalUnlocked_ += amount;
        if (!transferFrom(from, treasury_, amount)) {
            revert TransferFailed();
        }
        return true;
    }

    function foundationDeposit(address from, uint256 amount)
        external
        override
        returns (bool)
    {
    ...
    function marketingDeposit(address from, uint256 amount)
        external
        override
        returns (bool)
    {
    ...
    function tokenSaleDeposit(address from, uint256 amount)
        external
        override
        returns (bool)
    {
    ...
    function unLockTeam(address recever, uint8 phase)
        public
        onlyOwner
        whenNotPaused
    {
