...
contract Lend is IERC721Receiver {
  ...
  function getUsersCollateralToken(address _veNftAddress, uint256 _nftId) public view returns (uint256) {
  ...
  function lend(
    ...
    IToken(supportedAddress2[_veNftAddress]).transferFrom(msg.sender, depositAddress, fee);
  ...
  function repay(
    ...
    value = (IVeNft(_veNftAddress).getAmount(_nftId).mul(userInfo[_veNftAddress][_nftId].lockPercentage));
    uint256 interest = (value.mul(block.number.sub(userInfo[_veNftAddress][_nftId].lockPeriod)).mul(interestRatePerBlock));
    IChee(supportedAddress1[_veNftAddress]).burnFrom(msg.sender, _cheeAmount);
    IToken(supportedAddress2[_veNftAddress]).transferFrom(msg.sender, depositAddress, interest);
  ...
  function liquidate(
    ...
    require(userInfo[_veNftAddress][_nftId].amount == _cheeAmount, "Enter Correct Amount");
    value = (IVeNft(_veNftAddress).getAmount(_nftId).mul(userInfo[_veNftAddress][_nftId].lockPercentage)).div(100);
    uint256 interest = (value.mul(block.number.sub(userInfo[_veNftAddress][_nftId].lockPeriod.add(graceTime))).mul(interestRatePerBlock)).div(100);
    IChee(supportedAddress1[_veNftAddress]).burnFrom(msg.sender, _cheeAmount);
    IToken(supportedAddress2[_veNftAddress]).transferFrom(msg.sender, depositAddress, interest);
    IVeNft(_veNftAddress).safeTransferFrom(address(this), msg.sender, _nftId);
  }
