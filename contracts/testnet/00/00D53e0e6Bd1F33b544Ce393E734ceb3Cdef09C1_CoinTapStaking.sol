...
interface ICoinTapTier {
    ...
    function updateUserInfo(address account, uint256 amount, uint256 slot, bool unstake) external;
}
...
contract StakingModel {
    struct StakingPackage {
        string name;
        uint256 duration;
        uint256 apy; // Example 1000 -> 10%; 500 -> 5%; 2550 -> 25,5%
    }
...
contract CoinTapStaking is OwnableUpgradeable, StakingModel {
    ...
    uint256 public emergencyUnstakeFee = 2500; // 25%
    ...
    function initialize(ERC20 _cointap, ICoinTapTier _tier) public initializer {
        ...
        // Add default staking packages
        stakingPackages.push(StakingPackage("Testing", 300, 500));
        stakingPackages.push(StakingPackage("7 Days", 604800, 0));
        stakingPackages.push(StakingPackage("30 Days", 2592000, 500));
        stakingPackages.push(StakingPackage("60 Days", 604800, 700));
        stakingPackages.push(StakingPackage("90 Days", 7776000, 1000));
        stakingPackages.push(StakingPackage("180 Days", 15552000, 2000));
    
        // Add default levels
        levels.push(Level("Level 1", 100 * tokenDecimals, 1));
        levels.push(Level("Level 2", 500 * tokenDecimals, 2));
        levels.push(Level("Level 3", 1000 * tokenDecimals, 5));
        levels.push(Level("Level 4", 2000 * tokenDecimals, 5));
     ...
     */
    function updatePackages(StakingPackage[] memory newPkgs)
        external
        onlyOwner
    {
        delete stakingPackages;
        for (uint256 i = 0; i < newPkgs.length; i++) {
            require(newPkgs[i].apy > 0, "APY must be greater than 0");
            require(
                newPkgs[i].apy <= 10000,
                "APY must be less than 10000"
            );
        ...
        emit StakingPackageChange(newPkgs);
    }
    ...
    function updateEmergencyUnstakeFee(uint256 _emergencyUnstakeFee)
        ...
        onlyOwner
    {
        require(_emergencyUnstakeFee <= 10000, "Percent of fee invalid (10000)");
     ...
     * @param _pid uint256 index of staking package
     */
    function _stake(uint256 _amount, uint256 _pid)
        internal
        returns (uint256 amount, uint256 duration)
    {
        ...
        Tier.updateUserInfo(user, amount, slot, false);
    ...
    function emergencyUnstake() external {
        ...
        uint256 total = userInfo.amount.mul(10000 - emergencyUnstakeFee).div(10000);
        CoinTap.transfer(user, total);
    ...
    function _unstake() internal {
        ...
        Tier.updateUserInfo(user, 0, slot, false);

        emit Unstake(user, total);
    }

    function stake(uint256 _amount, uint256 _pid)
        external
        returns (uint256 amount, uint256 duration)
    {
        return _stake(_amount, _pid);
    }

    function stakeAndSyncData(uint256 _amount, uint256 _pid, ICoinTapTier.StakingRecord[] memory records)
        external
        returns (uint256 amount, uint256 duration)
    {
        // Sync staking recorda to tier
        Tier.syncTier(records);
        return _stake(_amount, _pid);
    }
    ...
    function unStakeAndSyncData(ICoinTapTier.StakingRecord[] memory records) external {
        // Sync staking recorda to tier
        Tier.syncTier(records);
        _unstake();
    }
    ...
    /**
     * @dev Estimate staking rewards at current moment
     */
    ...
    function _calInterestAmount(
        ...
        return pending.mul(_apy).div(10000);
    }
