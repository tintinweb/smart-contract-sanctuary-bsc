...
contract MarketPlaceRadikals is ReentrancyGuard, Ownable {
  ...
  /// @param _itemId id of item that is in the marketPlace and has not been sold yet	
	function purchaseItem(uint _itemId) external payable nonReentrant {
		...
		rdkToken.transferFrom(msg.sender, address(this), _sellerAmount);
		rdkToken.transfer(item.seller, _sellerAmount);
		rdkToken.transferFrom(msg.sender, feeAccount, item.price - _sellerAmount);
  ...
  /// @param _riderId id of rider to be burnt
	function burnRider(uint _riderId) external {
		...
		rdkToken.transferFrom(feeAccount, msg.sender, priceOracleInstance.getUsdtToToken(625));
