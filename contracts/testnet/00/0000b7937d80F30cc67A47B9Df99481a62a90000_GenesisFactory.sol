...
contract GenesisFactory is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  // deployer => BillsInfo
  mapping(address => BillInfo) public billsInfo;

  address public billImplementation;
  address public billNftImplementation;
  address public treasuryImplementation;

  event Deploy(address bill, address nft, address treasury);

  function initialize(
    address _billImplementation,
    address _billNftImplementation,
    address _treasuryImplementation
  ) public initializer {
    __Ownable_init();
    transferOwnership(msg.sender);

    billImplementation = _billImplementation;
    billNftImplementation = _billNftImplementation;
    treasuryImplementation = _treasuryImplementation;
  }

  function deploy(
    BillParams calldata _billParams,
    BillNftParams calldata _billNftParams,
    TreasuryParams calldata _treasuryParams
  ) external {
    ERC1967Proxy treasuryProxy = _deployTreasuryProxy(_treasuryParams);
    ERC1967Proxy nftProxy = _deployNftProxy(_billNftParams);
    ERC1967Proxy billProxy = _deployBillProxy(_billParams, address(treasuryProxy), address(nftProxy));

    _registerBill(address(billProxy), address(nftProxy), address(treasuryProxy));

    emit Deploy(address(billProxy), address(nftProxy), address(treasuryProxy));
  }

  // TODO: should approve bill contract to call deposit function
  function _deployTreasuryProxy(TreasuryParams calldata _treasuryParams) internal returns (ERC1967Proxy) {
    ERC1967Proxy treasuryProxy = _deployProxy(
      treasuryImplementation,
      abi.encodeWithSelector(
        Treasury(address(0)).initialize.selector,
        _treasuryParams.owner,
  ...
  function _deployNftProxy(BillNftParams calldata _billNftParams) internal returns (ERC1967Proxy) {
    ERC1967Proxy nftProxy = _deployProxy(
      billNftImplementation,
      abi.encodeWithSelector(
        BillNft(address(0)).initialize.selector,
        _billNftParams.name,
        _billNftParams.symbol,
        _billNftParams.baseTokenURI,
        _billNftParams.factory,
        _billNftParams.owner,
        owner()
      )
    );
  ...
  function _deployBillProxy(
    BillParams calldata _billParams,
    address _treasuryProxy,
    address _nftProxy
  ) internal returns (ERC1967Proxy) {
    ERC1967Proxy billProxy = _deployProxy(
      billImplementation,
      abi.encodeWithSelector(
        GenesisBill(address(0)).initialize.selector,
        [
          _treasuryProxy,
          _billParams.principalToken,
          _billParams.treasury,
          _billParams.subsidyRouter,
          _billParams.DAO,
          _nftProxy,
          _billParams.initialOwner
        ],
        _billParams.tierCeilings,
        _billParams.fees,
        false,
        owner()
      )
  ...
  function _deployProxy(address _implementationAddress, bytes memory _params) internal returns (ERC1967Proxy) {
    return new ERC1967Proxy(_implementationAddress, _params);
  }

  function _registerBill(address billProxy, address nftProxy, address treasuryProxy) internal {
    billsInfo[msg.sender] = BillInfo({
      bill: billProxy,
      nft: nftProxy,
  ...
  function setBillImplementation(address _billImplementation) external onlyOwner {
    billImplementation = _billImplementation;
  }

  function setBillNftImplementation(address _billNftImplementation) external onlyOwner {
    billNftImplementation = _billNftImplementation;
  }
...
import "./interfaces/IGenesisBill.sol";
 ...
 *
 */
contract BillNft is IVestingNFT, AccessControlEnumerableUpgradeable, ERC721EnumerableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
     ...
     */
    function mint(address to, address billAddress) external onlyMinter returns (uint256 newTokenId) {
        return _mintInternal(to, billAddress);
    }

    function mintMany(uint256 amount, address to, address billAddress) external onlyMinter {
        for(uint i = 0; i < amount; i++){
            _mintInternal(to, billAddress);
        }
    }

    function _mintInternal(address to, address billAddress) internal  returns (uint256 newTokenId) {
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) internal virtual {
        require(hasRole(URI_SETTER_ROLE, _msgSender()), "BillNft: Only uri setter role");
    ...
    function claim(uint256 tokenId) public {
        ...
        IGenesisBill bill = IGenesisBill(billAddresses[tokenId]);
        bill.redeem(tokenId);
    }
    ...
    function vestedPayout(uint256 tokenId) external view returns (uint256 payout) {
        IGenesisBill bill = IGenesisBill(billAddresses[tokenId]);
        payout = bill.pendingPayoutFor(tokenId);
    }
    ...
    function pendingPayout(uint256 tokenId) external view returns (uint256 payout) {
        payout = IGenesisBill(billAddresses[tokenId]).billInfo(tokenId).payout;
    }
    ...
    function pendingVesting(uint256 tokenId) external view returns (uint256 vestingSeconds) {
        IGenesisBill bill = IGenesisBill(billAddresses[tokenId]);
        vestingSeconds = bill.pendingVesting(tokenId);
    }
    ...
    function payoutToken(uint256 tokenId) external view returns (address tokenAddress) {
        tokenAddress = IGenesisBill(billAddresses[tokenId]).payoutToken();
    }
...
contract Treasury is Initializable, PolicyUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    /* ======== DEPENDENCIES ======== */
