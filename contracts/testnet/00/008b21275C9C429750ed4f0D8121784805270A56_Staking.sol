...
contract Staking is Ownable, Pausable {
    ...
    IERC20 tokenReward = IERC20(0x2F6c6cE689C0231919713c5556213cF3895Fed3B);
    ...
    function deposit(uint256 _pid, uint256 _amount) public whenNotPaused {
        ...
        require(_amount.mul(package.rateUSD).div(1e18) >= package.min, "Staking: token deposit too small");
        require(user.amount.add(_amount).mul(package.rateUSD).div(1e18) <= package.max, "Staking: limit staking this package");
    ...
    function pendingReward(uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 apyYear = user.amount.mul(package.rateUSD).div(1e18).mul(package.apr).div(100);
    ...
    function add(IERC20 _token, uint256 _rate, uint256 _apr, uint256 _min, uint256 _max) public onlyOwner {
        packageInfo.push(
            Package(_token, _rate, _apr, _min, _max, true)
        );
    }

    function set(uint256 _pid, IERC20 _token, uint256 _rate, uint256 _apr, uint256 _min, uint256 _max, bool _enable) public onlyOwner {
