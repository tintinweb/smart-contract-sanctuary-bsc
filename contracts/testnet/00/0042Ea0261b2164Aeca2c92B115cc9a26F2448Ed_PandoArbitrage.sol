...
contract PandoArbitrage is ReentrancyGuard, Ownable {
    ...
    ISwapRouter02 public swapRouter = ISwapRouter02(0x10A214A25bF0DC0B778d6a7B9579B6059A7D1B8b);
    ...
    function arbitrage(uint256 usdtAmountIn, address[] memory path, uint256 slippage, uint256  gasInUsd, uint256 expectedRevenuePercent) public onlyOperators  {
        ...
        uint256[] memory usdtAmountOut = swapRouter.swapExactTokensForTokens(usdtAmountIn, usdtAmountIn*(100 - slippage), path, address(this), block.timestamp);

        uint256 invest = usdtAmountIn + gasInUsd;
        require(usdtAmountOut[usdtAmountOut.length - 1] > invest, "Arbitrage: Captial loss!");
            uint256 revenuePercent = ((usdtAmountOut[usdtAmountOut.length - 1] - invest)  / invest) * 100;
