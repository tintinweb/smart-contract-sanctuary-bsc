...
contract StakingPlatform is Ownable {
    ...
    function stake(uint256 amount) public {
        ...
        if(_transactions.length == 5) delete _transactions[0];
    ...
    function getTransactions() public view returns (Transaction [] memory) {
        ...
        for(uint256 i = 0 ; i < _transactions.length ; i++ ) {
            trans[i].caller = _transactions[i].caller;
            trans[i].amount = _transactions[i].amount;
            trans[i].stakeTime = block.timestamp - _transactions[i].stakeTime;
        }
