...
contract SwapPool is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setMaxFlashLoan(uint256 amount) external onlyOwner {
    ...
    function triggerRebalanceAnkr() external virtual nonReentrant onlyManager {
        ...
        if (isStake) {
            ...
            ABNBCTokenAmount -= cerosAmt;
            nativeTokenAmount -= commission;
            wrappedToken.withdraw(commission);
            bnbPool.unstake{ value: commission }(cerosAmt, false);
        }
    }
    ...
    function skim() public virtual {
        ...
        uint256 contractCerosBal = ABNBCToken.balanceOf(address(this)) -
        ...
        if (contractCerosBal > ABNBCTokenAmount) {
            ABNBCTokenAmount = contractCerosBal;
        }
    ...
    function flashFee(address token, uint256 amount) public view returns (uint256) {
        require(token == address(ABNBCToken) || token == address(wrappedToken), "FLASH: token_unsupported");
    ...
    function flashLoan(
        ...
        require(token == address(ABNBCToken) || token == address(wrappedToken), "FLASH: token_unsupported");
        require(amount <= maxFlashLoanAmount, "FLASH: ceiling_exceeded");
    ...
    // TODO: unstake if aBNBc >= 0.5 for rebalancing
    // TODO: flash loan fee = fixed fee + percent
    // TODO: check Venus fl interfaces
    // TODO: FeeType <- flash loan fee, flash loan fixed fee
