...
import {
    BSCFeeCollectorInterface,
    WrappedInterface
} from "./interfaces/BSCFeeCollectorInterface.sol";

/**
 * @title   BSCFeeCollector
 * @author  OpenSea Protocol Team
 * @notice  BSCFeeCollector is a contract that is used as an
 ...
 *          and transferring wrapped tokens.
 */
contract BSCFeeCollector is BaseFeeCollector, BSCFeeCollectorInterface {
    constructor() BaseFeeCollector() {}

    /**
     * @notice Unwraps and withdraws the given amount of wrapped tokens from the
    ...
    function unwrapAndWithdraw(
        ...
        (bool result, bytes memory data) = wrappedTokenContract.call(
            abi.encodeWithSelector(WrappedInterface.withdraw.selector, amount)
        );
...
/**
 * @title   BSCFeeCollectorInterface
 * @author  OpenSea Protocol Team
 * @notice  BSCFeeCollectorInterface contains all external function
 *          interfaces for the fee collector implementation.
 */
interface BSCFeeCollectorInterface {
    /**
     * @notice Unwraps and withdraws the given amount of wrapped tokens from the
     *         provided contract address. Requires the caller to have the
     *         operator role, and the withdrawal wallet to be in the
