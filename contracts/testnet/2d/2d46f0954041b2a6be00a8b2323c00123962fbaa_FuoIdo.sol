...
contract FuoIdo is Ownable {
    ...
    IBEP20 public futToken = IBEP20(0x0C8CC9Fb600C87a3fC2E4b0b609F5a0f28475A68);
    ...
    function pledgeJoin(uint256 usdtAmount,uint256 pledgeType) public {
        require(usdtAmount > 0 && msg.sender != masterAddr,"params error");
        ...
        if(pledgeType == 1){
            ...
            require(pledgeMap[msg.sender].lockAmount >= futAmount,"lockAmount not enough");
    ...
    function pledgeTake() public {
        ...
        if(pledgeMap[msg.sender].takeValue + usdtValue >= pledgeMap[msg.sender].myPower * 4){
    ...
    function getUserPledgeProfit(address user) public view returns (uint256,uint256) {
        ...
        if(pledgeData.myPower <= 0 || diffTime < 3600 || pledgeMap[msg.sender].takeValue >= pledgeMap[user].myPower * 4){
            return (0,0);
        }
        ...
        uint256 usdtValue = realMyPower * rateTimes * 2 / (24 * 100);
        if(pledgeMap[msg.sender].takeValue + usdtValue >= pledgeMap[user].myPower * 4){
            usdtValue = pledgeMap[user].myPower * 4 - pledgeMap[msg.sender].takeValue;
        }
