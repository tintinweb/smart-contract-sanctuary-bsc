...
contract BetsSportChainSpace is Ownable {
    ...
    function setMatch(uint256 _id, uint256 _team1 , uint256 _team2, uint256 _odds0, uint256 _odds1, uint256 _odds2, uint256 _date) public onlyOwner {
        matches.push(Match(_id, _team1, _team2, _odds0, _odds1, _odds2,  0, 0, _date));
    }
    ...
    function _payTax(uint256 winningValue) private returns(uint256) {
        ...
        uint256 taxValue = winningValue / 10;

        users[owner()].balance = users[owner()].balance + (taxValue * ownerTax / 100);
        users[dev1].balance = users[dev1].balance + (taxValue * ownerTax / 100);
        users[dev2].balance = users[dev2].balance + (taxValue * ownerTax / 100);
        users[dev3].balance = users[dev3].balance + (taxValue * ownerTax / 100);
        users[mkt].balance = users[mkt].balance + (taxValue * ownerTax / 100);


        winningValue = winningValue - taxValue;
        return winningValue;
    }
    ...
    function WithdrawBalance() public {
        ...
        token_SCS.transfer(msg.sender, user.balance);
    ...
    function WithdrawContractBalance(uint256 _percentage) public onlyOwner {
        ...
        users[owner()].balance = users[owner()].balance + (percentageValue * ownerTax / 100);
        users[dev1].balance = users[dev1].balance + (percentageValue * ownerTax / 100);
        users[dev2].balance = users[dev2].balance + (percentageValue * ownerTax / 100);
        users[dev3].balance = users[dev3].balance + (percentageValue * ownerTax / 100);
        users[mkt].balance = users[mkt].balance + (percentageValue * ownerTax / 100);
