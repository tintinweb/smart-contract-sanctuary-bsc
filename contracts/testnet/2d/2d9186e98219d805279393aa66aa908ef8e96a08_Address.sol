...
contract SkullNFTTest is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x5e0690c273C8e90184189c57c8261c929Cf8DF2f;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "SkullNFTTest: minting has not started yet");
    require(_mintAmount > 0, "SkullNFTTest: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "SkullNFTTest: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "SkullNFTTest: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "SkullNFTTest: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "SkullNFTTest: zero address");
    require(_amount > 0, "SkullNFTTest you have to mint at least one");
    require(lastSupply != 0,  "SkullNFTTest: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"PixarWorldcup: URI query for nonexistent token");
