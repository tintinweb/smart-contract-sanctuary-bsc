...
// File: contracts/tos-1/IERC721Batch.sol
...
// File: contracts/tos-1/Address.sol
...
// File: contracts/tos-1/IERC721Receiver.sol
...
// File: contracts/tos-1/IERC165.sol
...
// File: contracts/tos-1/ERC165.sol
...
// File: contracts/tos-1/IERC721.sol
...
// File: contracts/tos-1/IERC721Enumerable.sol
...
// File: contracts/tos-1/IERC721Metadata.sol
...
// File: contracts/tos-1/Context.sol
...
// File: contracts/tos-1/ERC721T.sol
...
// File: contracts/tos-1/ERC721EnumerableT.sol
...
// File: contracts/tos-1/Ownable.sol
...
// File: contracts/tos-1/Delegated.sol
...
contract xiaozhi is ERC721EnumerableT, Delegated, ReentrancyGuard {
  ...
  /**
    ...
    mapping(address => Staker) public stakers;//质押者

    // Mapping from token ID to owner address
    mapping(uint256 => address) public originalStakeOwner;//原始权益所有者

     // @notice event emitted when a user has staked a token
    event Staked(address owner, uint256 tokenId);//质押事件

    // @notice event emitted when a user has unstaked a token
    event Unstaked(address owner, uint256 tokenId);//赎回事件

  bool public revealed = false;
  string public notRevealedUri = "ipfs://QmXZGWhQp3CQc3svnDhUuf4yTtoQBuqumHeC33jmEG69Dd/hidden.json";//盲盒封面

  uint public MAX_SUPPLY   = 8888;//最大供应量
  uint public PRICE        = 0.15 ether;
  uint public MAX_QTY = 1;//每次最大的mint 数量
  
  Moon[] public moons;//token列表
  ...
  bool public isMintActive = false;//mint状态


  bool public isStakeActive   = false;//质押状态

  mapping(address => uint256 ) private _mintcount;//计数
  uint256 private  Max_Mint_Count = 5;//最大的min次数
  ...
  string private _tokenURIPrefix;//盲盒的url
  string private _tokenURISuffix =  ".json";//盲盒的url的类型
...
/**
  ...
  */
  function balanceOf(address account) public view override returns (uint) {//余额
...
/**
  ...
  */
  function isOwnerOf( address account, uint[] calldata tokenIds ) external view override returns( bool ){//是否是所有者
    for(uint i; i < tokenIds.length; ++i ){
...
/**
  ...
  *
  */
  function ownerOf( uint tokenId ) public override view returns( address owner_ ){//所有者
...
/**
  ...
  * Use along with {totalSupply} to enumerate all tokens.
  */
  function tokenByIndex(uint index) external view override returns (uint) { //代币索引标记
...
/**
  ...
  * Use along with {balanceOf} to enumerate all of ``owner``'s tokens.
  */
  function tokenOfOwnerByIndex(address owner, uint index) public view override returns (uint tokenId) {//索引的所有者令牌
    uint count;
  ...
  * @dev Returns the Uniform Resource Identifier (URI) for `tokenId` token.
  */
  function tokenURI(uint tokenId) external view override returns (string memory) {//token的url
...
/**
  * @dev Returns the total amount of tokens stored by the contract.
  */
  function totalSupply() public view override returns( uint totalSupply_ ){//总供应
    return moons.length;
  }
...
/**
  ...
  */
  function walletOfOwner( address account ) external view override returns( uint[] memory ){//钱包所有者
    uint quantity = balanceOf( account );
    uint[] memory wallet = new uint[]( quantity );
    for( uint i; i < quantity; ++i ){
        wallet[i] = tokenOfOwnerByIndex( account, i );
...
/**
  ...
  */
  function setRevealState(bool reveal_) external onlyDelegates {//设置显示状态,盲盒开
      revealed = reveal_;
  }
  ...
  * @dev Owner sets the Staking contract address.
  */
  function setUnrevealURI(string calldata notRevealUri_) external onlyDelegates {//设置取消显示，盲盒关
  ...
  */
  function mint( uint quantity ) public {
    require(isMintActive == true,"MOON: Minting needs to be enabled.");
      ...
      require(quantity <= MAX_QTY, "MOON:Quantity must be less than or equal MAX_QTY");//每次的mint的数量
...
/**
  ...
  */
  function getBalanceofContract() external view returns (uint256) {//获取合约余额
...
/**
  ...
  */
  function withdraw(uint256 amount_) public onlyOwner {//提取
  ...
  */
  function team_mint(uint[] calldata quantity, address[] calldata recipient) external onlyDelegates{//团队
...
/**
  ...
  */
  function setMintingActive(bool mintActive_) external onlyDelegates {//设置mint激活
...
/**
  ...
  */
  function setBaseURI(string calldata prefix, string calldata suffix) external onlyDelegates{//设置url
...
/**
  ...
  */
  function setMaxSupply(uint maxSupply) external onlyDelegates{//设置最大供应
...
/**
  ...
  */
  function setMaxQty(uint maxQty) external onlyDelegates{//设置mint最大数量
...
/**
    ...
    */
  function updateMoontype(bool moonType, uint[] calldata tokenIds ) external onlyDelegates {//更新类型
...
/**
  ...
  */
  function getMoonType(uint[] calldata tokenIds) external view returns(bool[] memory moonTypes) {//获取类型
...
/**
  ...
  */
  function _transfer(address from, address to, uint tokenId) internal override {//交易
  ...
  /**
    ...
    */
    function getStakedTokens(address _user) public view returns (uint256[] memory tokenIds) {//获取质押token
...
/**
  ...
  */
    function stake( uint[] calldata tokenIds ) external {//质押
...
/**
  ...
  */
    function unStake( uint[] calldata tokenIds ) external {//取消质押
        require( isStakeActive, "MOON: Staking is not active" );
  ...
  /**
    ...
    */
    function setStakeActive( bool isActive_ ) external onlyDelegates {//开启质押
    ...
    *   @dev usable only if _array contains unique elements only
     */
    function _removeElement(address _user, uint256 _element) internal {//赎回的时候移除元素
        Staker storage staker = stakers[_user];
    ...
    function setRevertMintCount()  external onlyDelegates  {//重置mint为0
