...
contract Forsage2 is SmartMatrixForsageBasic {
  ...
  constructor(
    ...
    users[_ownerAddress].referrer = address(0);
    users[_ownerAddress].partnersCount = uint256(0);
  ...
  function updateX6(
    ...
    if (!x2) {
      ...
      //set current level
      users[userAddress].x6Matrix[level].currentReferrer = users[referrerAddress]
        .x6Matrix[level]
  ...
  function updateX6ReferrerSecondLevel(
    ...
    address[] memory x6 = users[users[referrerAddress].x6Matrix[level].currentReferrer]
      .x6Matrix[level]
      .firstLevelReferrals;
  ...
  function findFreeX3Referrer(address userAddress, uint8 level)
    public
    view
    returns (address referrerAddress)
  {
    while (true) {
  ...
  function findEthReceiver(
    ...
    uint8 matrix,
    uint8 level
  ) private returns (address receiverAddress, bool isExtraDividendsBool) {
  ...
  function withdrawLostTokens(address tokenAddress) public onlyContractOwner {
    ...
    if (tokenAddress == address(0)) {
      payable(address(uint160(multisig))).transfer(address(this).balance);
    } else {
