...
contract ERC_721_token {
    ...
    function safeOwnerOf(uint256 tokenId)
        internal
        view
...
contract DEVSNFTs is ERC_721_token {
    ...
    constructor(RewardToken _RewardingTokenAddr, string memory _URI)
        ERC_721_token("DEVSNFTs", "Devs", 50)
    {
        require(
    ...
    function currentBalanceOf() public view returns (uint8 toReturn) {
        User memory tempInfo = userInfo[msg.sender];

        toReturn += (tempInfo.diamond +
            tempInfo.silver +
            tempInfo.gold +
            tempInfo.platinum);
    }
    ...
    function _mint() internal returns (uint256 TokenID) {
        ...
        if (idtoMint <= 15) {
            userInfo[msg.sender].silver += 1;
        } else if (idtoMint <= 30) {
            userInfo[msg.sender].gold += 1;
        } else {
            userInfo[msg.sender].platinum += 1;
        }
        noOfCurrentlyMinted += 1;
        return TokenID;
    }

    function mintDEV(bytes32[] calldata _merkleProof)
        public
        returns (uint256 TokenIDminted)
    {
        require(currentBalanceOf() < 4, "You have already minted 4 NFTS!");
        require(noOfCurrentlyMinted < 50, "All the Tokens have been minted");
        State currentState = mintingCurrentState;
        require(currentState != State.Paused, "PreSale Hasn't Started");
        if (currentState == State.PreSale) {
            require(
                whitelistMint(_merkleProof) == true,
                "You are not Whitelisted"
            );

            // Require Staking Tokens 50
            require(
                RewardingTokenAddr.balanceOf(msg.sender) >= 50*10**RewardingTokenAddr.decimals(),
                "You require 50 staking tokens to Participate in PRE SALE"
            );
            require(
                RewardingTokenAddr.transferFrom(msg.sender, address(this), 50 *10**RewardingTokenAddr.decimals()),
                "Error in Transfering Rewarding Token"
            );
        } else {
            // Require Staking Tokens 100
            require(
                RewardingTokenAddr.balanceOf(msg.sender) >= 100 *10**RewardingTokenAddr.decimals(),
                "You require 100 staking tokens to Participate in PUBLIC SALE"
            );
            require(
                RewardingTokenAddr.transferFrom(msg.sender, address(this), 100 *10**RewardingTokenAddr.decimals()),
                "Error in Transfering Rewarding Token"
            );
        }
        TokenIDminted = _mint();
    ...
    function mint2nftDEV(bytes32[] calldata _merkleProof)
        public
        returns (uint256 TokenID1, uint256 TokenID2)
    {
        ...
        if (currentState == State.PreSale) {
            ...
            // Require Staking Tokens 100
            require(
                RewardingTokenAddr.balanceOf(msg.sender) >= 200 *10**RewardingTokenAddr.decimals(),
                "You require 200 staking tokens to Participate in PUBLIC SALE & mint 2 NFTS"
            );
            require(
                RewardingTokenAddr.transferFrom(msg.sender, address(this), 200 *10**RewardingTokenAddr.decimals()),
                "Error in Transfering Rewarding Token"
            );
        }
        ...
        TokenID1 = _mint();
        TokenID2 = _mint();
        noOfCurrentlyMinted += 2;
    }

    function mintDiamondNFT(bytes32[] calldata _merkleProof)
        public
        returns (uint256)
    {
        require(currentBalanceOf() < 4, "You have already minted 4 NFTS!");
    ...
    function CurrentlyOwnedTokens1(address user)
        public
        view
        returns (uint256[4] memory ownedTokens)
    {
...
// User Transfer 4 tokens to your address than You can Still Mint 4 Tokens
// Fix Minting Issue with whitelisting