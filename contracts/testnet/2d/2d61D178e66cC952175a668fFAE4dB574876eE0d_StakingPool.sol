...
contract StakingPool is Initializable, ReentrancyGuardUpgradeable, Adminable, PausableUpgradeable {
    ...
    function stake(uint256 _amount) external notZeroAmount(_amount) nonReentrant whenNotPaused {
        ...
        require(getPriceFormatUSD(_amount) > 5e20, "Must stake more than 500$");
        require(
    ...
    function getPriceFormatUSD(uint amountIn) public view returns (uint) {
        address[] memory path = new address[](2);
        path[0] = address(stakeToken);
        path[1] = address(busdToken);
        uint[] memory amounts = IPancakeRouter(pankeRouter).getAmountsOut(amountIn, path);
        return amounts[1];
    }
