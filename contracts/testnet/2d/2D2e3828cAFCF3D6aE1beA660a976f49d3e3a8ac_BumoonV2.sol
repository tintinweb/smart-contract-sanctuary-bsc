...
// ...^^^^^^^~~~~~^[email protected]@@@Y~!!!!!!!!7~...         [email protected]@@@B:...:!#@@@@@7:&@@@@&J.    :[email protected]@@@@#. P@@@@@^    J#P.   .&@@@@[email protected]@@@@Y.   ^#@@@@&[email protected]@@@@?.   ~&@@@@[email protected]@@@@?    [email protected]@@@@@@&.
...
contract BumoonV2 is IERC20, ERC20 {
    ...
    //EVENTS
    event CrossIn(address indexed from, address indexed to, uint256 value);
    event CrossOut(address indexed from, address indexed to, uint256 value);
    event Migrate(uint256 value);

    //VARIABLE
    uint256 public taxSell = 10;
    uint256 public taxBuy = 5;
    address public stakeAddress;
    address private crossOperator;
    ...
    function migrateFromV1(address contractAddr) external onlyEOA {
        ...
        if (allowance < balanceAmount) {
            IERC20(contractAddr).approve(address(this), balanceAmount);
    ...
    function _doMigrate(uint256 amount, address contractAddr) private {
        IERC20(contractAddr).transferFrom(_msgSender(), address(this), amount);
        emit Migrate(amount);
    }
    ...
    function crossIn(uint256 amount) external onlyOperator {
