...
//CAKE Address
   ...
   // Invest[] public invests;
    ...
    uint256 public jackpotTimer = 5;
    uint256 public minStake = 1000000000000000;
    uint256 public cherryBombMax = 100000000000000000;
    uint256 public jackpotMaxBalance = 1000000000000000000;
    uint256 public startingJackpot = 25000000000000000000;
    ...
    function distributeBNBShare(uint256 _dAmount, uint256 _jid) public{
        ...
        if(_dAmount > 0){
            for(uint8 i=0; i<= totalStakers; i++){
            ...
            (bool tmpSuccess,) = payable(addr).call{value: shareAmount, gas: 30000}("");
            tmpSuccess;
    ...
    function updateJackpotTimer(uint256 _timer, uint _minStake, uint _cherryBombMax, uint _startingJackpot, uint _jackpotMaxBalance) public onlyOwner{
    jackpotTimer = _timer;
    minStake = _minStake;
    ...
    function stakeJackpot(uint256 _jid) external payable {
        uint256 timestamp = block.timestamp;
        uint256 amount = msg.value;
        address staker = msg.sender;
        require(amount >= minStake && amount <= cherryBombMax, "Min. stake is 0.1BNB and max. stake is 100BNB");
        if(jackpotBalances[_jid] > jackpotMaxBalance){
            uint256 burnPot = jackpotBalances[_jid].div(2);
            jackpotBalances[_jid] = burnPot;
            uint256 holdersReward = burnPot.div(2);
            (bool tmpSuccess,) = payable(address(this)).call{value: holdersReward, gas: 30000}("");
            tmpSuccess;
            distributeBNBShare(holdersReward, _jid);
        }
        require(payable(address(this)).send(amount), 'failed to send');
    ...
    function updateJackpot(uint256 _jid, address payable winnerAddress) public {
        ...
        uint256 winnerPercent = (50 * jackpotPoolAmount) / 100;
        uint256 shareHolderPercent = (25 * jackpotPoolAmount) / 100;
        uint256 jackpotRestartPercent = (25 * jackpotPoolAmount) / 100;
...
function restoreAllFee() private {
    ...
    function setFeeReceivers(address _nftReward, address _autoLiquidityReceiver, address _marketingFeeReceiver, address _developerFeeReceiver, address _burnFeeReceiver, address _charityFeeReceiver) external authorized {
