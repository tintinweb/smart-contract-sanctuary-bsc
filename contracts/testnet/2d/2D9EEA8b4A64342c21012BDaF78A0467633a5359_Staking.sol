...
contract Staking {
    ...
    function initialize() external {
        ...
        packages[0].apr = 100;
        packages[0].duration = 1; // MUST CHANGE IT TO 30
        packages[0].id = 0;
        packages[1].apr = 138;
        packages[1].duration = 2; // MUST CHANGE IT TO 90
        packages[1].id = 1;
        packages[2].apr = 220;
        packages[2].duration = 3; // MUST CHANGE IT TO 180
        packages[2].id = 2;
        packages[3].apr = 5;
    ...
    function _calculateEarned(uint256 stakingId, bool isTotal)
        internal
        view
        returns (uint256)
    {
        ...
        if (ownerStaking.duration == 0) {
            ...
            return
                ownerStaking.amount +
                ((ownerStaking.amount * finalApr * stakedTimeClaim) /
                    100 /
                    12 /
                    30); // tiền lãi theo ngày * số ngày
        } else {
            uint256 profit = ((ownerStaking.duration * ownerStaking.amount * finalApr) /
                    100 /
                    30 /
                    12); // tiền lãi theo ngày * số ngày
            return isTotal ? ownerStaking.amount + profit : profit; 
        }
    }
    ...
    function claim(uint256 _stakingId) external {
        ...
        if (stakeDuration != 0) {
            require(
                block.timestamp >=
                    (lastTimeCheck + (stakeDuration * unitToSecond)),
                "Staking locked"
            );
        }
