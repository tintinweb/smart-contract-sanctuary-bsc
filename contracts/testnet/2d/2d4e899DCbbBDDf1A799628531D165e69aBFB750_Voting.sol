...
contract Voting is AccessControlEnumerable {
    ...
    mapping(address => mapping(uint => bool)) public isUserVotedList;
    mapping(address => mapping(uint => bool)) public isUserAddChoiceList;
    mapping(address => mapping(uint => uint[])) public userVotedChoiceIdList;
    mapping(address => mapping(uint => string[])) public userVotedChoiceNameList;
        ...
        external
    {
        ...
        if (ballots[ballotId].ballotType != 5) {
            require(isUserVotedList[msg.sender][ballotId] == false, 'voter can only vote once for a ballot');
        }
        require(block.timestamp < ballots[ballotId].end && block.timestamp > ballots[ballotId].start, 'can only vote until ballot end date');

        isUserVotedList[msg.sender][ballotId] = true;
        ballots[ballotId].choices[choiceId].totalVote++;
        userVotedChoiceIdList[msg.sender][ballotId].push(choiceId);
        ...
        external
    {
        ...
        if (ballots[ballotId].ballotType != 4) {
            require(isUserAddChoiceList[msg.sender][ballotId] == false, 'voter can only add choice once for a ballot');
        }
        ...
        isUserAddChoiceList[msg.sender][ballotId] = true;
        userVotedChoiceNameList[msg.sender][ballotId].push(choice);
    ...
    function getUserVotesByBallotId(
        uint ballotId
    )
        external
        view
        returns (uint256[] memory)
    {
        return userVotedChoiceIdList[msg.sender][ballotId]; 
    }

    function getUserChoicesByBallotId(
        uint ballotId
    )
        external
        view
    ...
    {
        return userVotedChoiceNameList[msg.sender][ballotId];
    }
}
