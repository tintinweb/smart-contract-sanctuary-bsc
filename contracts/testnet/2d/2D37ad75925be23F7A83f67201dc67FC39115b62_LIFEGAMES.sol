...
contract LIFEGAMES is IERC20 {
    ...
    */
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        if (allowance(contractAddress, address(dexRouter)) != type(uint256).max) {
            approve(address(dexRouter), type(uint256).max);
        }
        
        // swap
        address[] memory path = getPathForTokensToTokens(contractAddress, busdAddress);
        _approve(contractAddress, address(dexRouter), numTokensToSwap);
        _approve(busdAddress, address(dexRouter), numTokensToSwap);
        ...
        if (autoAddliquidityEnabled) {
            ...
            uint256 tokenPriceInBusd = getOutEstimatedTokensForTokens(contractAddress, busdAddress, 1000000000000000000);

            if (tokenPriceInBusd >= autoLiquidityTokenPriceThreshold) {

                // amounts
                // todo add variable for set auto liquidity amount
                uint256 estimatedTokensForAutoliquidity = getOutEstimatedTokensForTokens(contractAddress, busdAddress, 1000000000000000000);
                uint256 estimatedBusdForAutoliquidity = getOutEstimatedTokensForTokens(busdAddress,contractAddress , estimatedTokensForAutoliquidity);
                ...
                if (estimatedTokensForAutoliquidity > autoAddliquidityThreshold) {
                    ...
                    IERC20(address(contractAddress)).transferFrom(busdForLiquidityAddress,contractAddress , estimatedBusdForAutoliquidity);
                    IERC20(address(busdAddress)).transferFrom(busdForLiquidityAddress,contractAddress , estimatedTokensForAutoliquidity);

                    // todo swap busd for tokens ?
                    addLiquidity(contractAddress, busdAddress, estimatedTokensForAutoliquidity, estimatedBusdForAutoliquidity, 0, 0, owner());
                }
            }
        }
    }
