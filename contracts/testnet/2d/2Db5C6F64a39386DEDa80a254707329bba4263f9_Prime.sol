...
interface IPrime {
  ...
  /*********************BSC-VIADUCT************************* */
  /********************************************************* */
  function bridgeBNB() external payable returns (bool);

  function bridgeBUSD(uint256 amount) external returns (bool);
}
...
contract Prime is ReentrancyGuard, IPrime {
  ...
  function bridgeBNB() public payable override returns (bool) {
    return IBscViaDuct(ADDRESSES_PROVIDER.getBscViaDuctContract()).bridgeBnb{ value: msg.value }(msg.sender);
  }

  function bridgeBUSD(uint256 amount) public override returns (bool) {
    return IBscViaDuct(ADDRESSES_PROVIDER.getBscViaDuctContract()).bridgeBusd(amount, msg.sender);
  }
}
