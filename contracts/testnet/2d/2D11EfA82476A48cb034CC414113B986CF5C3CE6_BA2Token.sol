...
contract BA2Token is IERC20, Ownable {
    ...
    uint256 private _previousluckyFee;

    uint256 public _inviterFee = 700;
    uint256 private _previousInviterFee;
    
    uint256 private _tTotal = 2000 * 10**4 * 10**9;
    ...
    constructor(address _marketAddress, address _luckyAddress) {
    ...
    function setLuckyAddress(address newLuckyAddress) public onlyOwner {
        luckyAddress = newLuckyAddress;
    }
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair || from == uniswapV2Pair || from == address(uniswapV2Router)) {
            takeFee = true;
        }
