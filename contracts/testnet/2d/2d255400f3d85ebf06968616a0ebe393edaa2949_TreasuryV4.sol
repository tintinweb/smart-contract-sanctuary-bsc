...
contract TreasuryV4 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    event OwnerCashReservesChanged(address indexed caller, uint cache, uint buffer, uint reserve0, uint reserve1);
    event OwnerPoolStateChanged(address indexed caller, address poolAddress, uint lpAmount);
    event OwnerConfigurationChanged(address indexed caller);
    ...
    function set_maximum_buffer(uint _buffer) public onlyOwner {
        ...
        emit OwnerConfigurationChanged(msg.sender);
    }
