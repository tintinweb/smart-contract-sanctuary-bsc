...
contract ERC721NativeMintableFacet is IERC721MintableFacet {
    ...
    function moveTo(address to, uint256 tokenId) public virtual override {
        OmniteAccessControl oac = LibDiamond
            .systemContext()
            .omniteAccessControl();

        address sender = LibMeta.msgSender();
        require(oac.hasRole(oac.BRIDGE_ROLE(), sender), "Only bridge can call");

        if (LibERC721.isOwner(s.diamondAddress, s.diamondAddress, tokenId)) {
    ...
    function mintTo(address to, uint256 tokenId) public virtual override {
        ...
        address sender = LibMeta.msgSender();
        bool hasBridgeRole = oac.hasRole(oac.BRIDGE_ROLE(), sender);
        bool isOwner = LibDiamond.owner() == sender;

        // solhint-disable-next-line reason-string
        require(
            hasBridgeRole || isOwner,
            "Only owner of collection or bridge can mint"
        );
