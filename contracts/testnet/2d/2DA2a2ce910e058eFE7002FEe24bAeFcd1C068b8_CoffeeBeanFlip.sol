...
contract CoffeeBeanFlip is VRFConsumerBaseV2, Context, Ownable, Pausable {
    ...
    mapping(bytes32 => uint256) private claimedBeans;
    mapping(bytes32 => uint256) private lastRoastTime;
    mapping(address => bytes32) private referral;
    mapping(uint256 => address) private flipRequests;
    mapping(uint256 => uint256) private flipRewardRates;
    mapping(uint256 => uint8) private flipRewardCoins; // 0: nothing, 1: head, 2: tail

    mapping(uint256 => uint256[]) public rewardChances; // 0: win, 1: miss, 2: lose
    mapping(address => uint8) public lastFlipRes; // 3: win, 2: miss, 1: lose
    ...
    event Roasted(address indexed player);

    event Sold(address indexed player);

    event Bought(address indexed player);
    ...
    constructor(
        ...
        // DOUBLE SHOT chance: 45% win, 5% miss, 50% lose
        rewardChances[2] = [45, 5, 50];

        // TRIPLE SHOT chance: 30% win, 5% mis, 65% lose
        rewardChances[3] = [30, 5, 65];

        // QUAD SHOT chance: 22% win, 5% mis, 73% lose
        rewardChances[3] = [22, 5, 73];
    }
    ...
    function getUserHash(address user) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(user));
    }

    function roastBeans(bytes32 ref) public whenNotPaused {
        if (ref == getUserHash(msg.sender)) {
            ref = bytes32(0);
        }
        if (referral[msg.sender] == bytes32(0) && referral[msg.sender] != ref) {
        ...
        claimedBeans[getUserHash(msg.sender)] = 0;
        lastRoastTime[getUserHash(msg.sender)] = block.timestamp;
    ...
    function buyBeans(bytes32 ref) public payable whenNotPaused {
        ...
        claimedBeans[getUserHash(msg.sender)] += beansBought;
    ...
    function getMyBeans(address adr) public view returns (uint256) {
        return claimedBeans[getUserHash(adr)] + getBeansSincelastRoastTime(adr);
    }
        ...
        returns (uint256)
    {
        uint256 secondsPassed = min(
            BEANS_PER_MINER,
            block.timestamp - lastRoastTime[getUserHash(adr)]
        );
        ...
        override
    {
        ...
        if (rand < chances[0]) {
            lastFlipRes[user] = 3; // win
            lastRoastTime[getUserHash(user)] = block.timestamp;
            claimedBeans[getUserHash(user)] =
                hasBeans *
                flipRewardRates[requestId]; // X shot
        } else if (rand < chances[0] + chances[1]) {
            lastFlipRes[user] = 2; // miss
        } else {
            lastFlipRes[user] = 1; // lose
            lastRoastTime[getUserHash(user)] = block.timestamp;
            claimedBeans[getUserHash(user)] = 0; // empty
        ...
        emit Flipped(
            user,
            flipRewardRates[requestId],
            flipRewardCoins[requestId],
            lastFlipRes[user]
        );
    }
}
