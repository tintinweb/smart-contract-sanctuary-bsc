...
contract DTM {
  ...
  function Reduceproduction()public view  returns(uint256)  {
    ...
    function _deposit(address _userAddr, uint256 _amount, bool _isLuckable) private {
        ...
        if(user.count==1){
           require(_amount>=100&&_amount<=1000,"100-1000 for the first ");//100-1000
        }
        if(user.count==2){ 
           require(_amount>=100&&_amount<=1500,"100-1500 for the first ");//100-1000
        }
        if(user.count==3){ // 100-2000
           require(_amount>=100&&_amount<=2000,"100-2000 for the first ");//100-1000
        }
