...
contract Referral is ReferralFetchers {
    ...
    function setMaximumReward(
        uint256 _maximumReward
    ) external onlyOwner {
        if (
            _maximumReward <= 0 
        ) {
...
abstract contract ReferralEvents is ReferralDeclaration {
    event AddedAsAFriend(address friendAddress, address customerAddress);
    ...
    event MaximumRewardSet(uint256 _maximumReward);
}
