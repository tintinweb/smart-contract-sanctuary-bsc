...
contract BitaXSeat {
    ...
    function distributeReferral(address direct, uint256 _bitax, bool isReg) internal{
        for(uint8 i=0; i<referral.length; i++){
            ...
            if(isReg==true){
                ...
                else if(i == 2 && users[users[direct].refer].levelTeam[2] >= 200){ // Minimum 200 members at level 3
                    users[users[direct].refer].dividend[2]+=_bitax.mul(users[users[direct].refer].levelTeam[2]).mul(dividend).div(100);
                }
                else if(i == 3 && users[users[direct].refer].levelTeam[3] >= 500){ // Minimum 500 members at level 4
                    users[users[direct].refer].dividend[3]+=_bitax.mul(users[users[direct].refer].levelTeam[3]).mul(dividend).div(100);
                }
                else if(i == 4 && users[users[direct].refer].levelTeam[4] >= 1000){ // Minimum 1000 members at level 5
    ...
    function buy(uint256 _busd) public security{
        ...
        require(user.stakedTokens<=tokenLimit && _busd<=tokenLimit-user.stakedTokens,"Maximum token limit exceeds.");
        if(topInvestors<=50){
            if(user.isTopInvestor == false){
                user.isTopInvestor = true;
            }
            topInvestors++;
        }
