    ...
    PausableUpgradeable
{
    ...
    function claimAlpacaRewards(address _user, uint256 _userShares) external {
        ...
        if (earnedAmt > 0) {
            if (controllerFee > 0) {
                _distributeFee = (earnedAmt * controllerFee) / controllerFeeMax;
                IERC20Upgradeable(alpacaTokenAddress).safeTransfer(owner(), _distributeFee);
                emit DistributeFee(alpacaTokenAddress, _distributeFee, owner());
                earnedAmt = earnedAmt - _distributeFee;
            }
        }
