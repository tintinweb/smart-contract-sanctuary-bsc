...
contract TheNetworkAutoCompound is Context, Ownable , ReentrancyGuard {
    ...
    constructor(address payable _executor) {
		require(!Address.isContract(_executor));
        executor = _executor;
    }
    ...
    function withdrawAndDisableAutoCompound() external nonReentrant {
        ...
        if(this.getContractBalance() < payout) {
            payout = this.getContractBalance();
        }
    ...
    function depositAndEnableAutoCompound(uint256 compoundTimesInDays) payable external nonReentrant {
        ...
        if(autoCompoundMap[_addr].isAutoCompound == true) 
        revert("User already has an existing auto-compound. This needs to be finished before doing another auto-compound cycle.");
    ...
    function execAutoCompoundAddresses() external onlyOwner {
        ...
        this.execAutoCompound(nextAddressesForAutoCompound());
    ...
    function execAutoCompound(address[] memory _addresses) external onlyOwner {
        ...
        for(uint256 i = 0; i < iterations && i < _addresses.length; i++){
            ...
            if( _addr != address(0)){
                ...
                autoCompoundMap[_addr].lastActionTime = this.getCurrentTime();
        ...
        if(this.getContractBalance() < gasFeeRefund) {
            gasFeeRefund = this.getContractBalance();
        }
    ...
    function nextAddressesForAutoCompound() internal returns (address[] memory _addresses) {
        ...
        for(uint256 i = 0; i < iterations; i++){
            ...
            if(    _addr != address(0)     
                ...
                if(this.getCurrentTime().sub(checkpoint) >= compoundInterval){
    ...
    function getAddresses(uint256 startIndex, uint256 limit) view external returns (address[] memory _addresses) {
    ...
    function calculateGasFeeEstimate(uint256 _days) view external returns (uint256) {
        return _days.mul(averageGasFee);
    }

    function isCompoundExecutionPossible(address _addr, uint256 _days) view external returns (bool isExecute) {
    ...
    function updateAvgGasFees(uint256 _value)  external onlyOwner {
        require(_value <= 0.002 ether, "Required fee not met");
        averageGasFee = _value;
    }

    function updateBatchSize(uint256 _value) external onlyOwner {
        iterations = _value;
    }

    function resetExecutionIndex() external onlyOwner {
        currentIndex = 0;
    }

    function updateExecutor(address _value) external onlyOwner {
        executor = payable(_value);
    }
  
    function setMainContractAddress(address addr) external onlyOwner {
