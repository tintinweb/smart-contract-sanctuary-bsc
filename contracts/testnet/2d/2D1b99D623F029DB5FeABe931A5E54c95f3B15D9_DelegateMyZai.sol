...
contract DelegateMyZai is Ownable {
    ...
    // same "pagination" concept but for list all Zai delegated to a scholar 
    ...
    function openMyZaiToDelegation(
        ...
        address _scholar) external onlyZaiOwner(_zaiId) {
            require(_contractDuration >= 3 && _contractDuration <= 30, "min 3 days, max 30 days");
            ...
            if(_scholar != address(0x0)){
                ...
                m.contractEnd = block.timestamp + (_contractDuration * 1 days);
                m.lastScholarPlayed = block.timestamp;
                m.scholarAddress = _scholar;
                EnumerableSet.add(scholarDelegated[_scholar],_zaiId);
            }
