...
import "./access/ISEVAccessManager.sol";
import "./CoreFee.sol";
import "./security/ReentrancyGuard.sol";
import "./ERC/ERC20/SafeBEP20.sol";

contract SEVHeroCore is ERC721, Ownable, ReentrancyGuard {
    using SafeBEP20 for IBEP20;

    uint256 public currentId;
    ISEVAccessManager SEVAccessManager;
    CoreFee coreFee;
    ...
    constructor(
        ISEVAccessManager _SEVAccessManager,
        CoreFee _coreFee,
        uint256 _breedMax
    ) ERC721("spacemarvel.io", "SEV721") {
        require(address(_SEVAccessManager) != address(0), "Error: address(0)");
        require(address(_coreFee) != address(0), "Error: address(0)");
        SEVAccessManager = _SEVAccessManager;
        coreFee = _coreFee;
        breedMax = _breedMax;
        currentId = 1;
    }

    function setSEVAccessManager(ISEVAccessManager _SEVAccessManager)
        external
        onlyOwner
    {
        require(address(_SEVAccessManager) != address(0), "Error: address(0)");
    ...
    function born(address _toAddress, uint256 _gene) public nonReentrant {
        require(
            SEVAccessManager.isBornAllowed(_msgSender(), _gene),
    ...
    function evolve(uint256[] memory _nftIds, uint256 _newGene) external nonReentrant returns (uint256){
        require(
            SEVAccessManager.isEvolveAllowed(_msgSender(), _newGene, _nftIds),
    ...
    function breed(
        ...
        require(
            SEVAccessManager.isBreedAllowed(_msgSender(), _nftId1, _nftId2),
    ...
    function destroy(uint256 _nftId) external nonReentrant {
        require(
            SEVAccessManager.isDestroyAllowed(_msgSender(), _nftId),
...
contract CoreFee is Ownable {
    ...
    modifier onlySEV() {
        require(_msgSender() == myMasterWarCore, "Error: Not SEVCore");
        _;
    }

    function setSEVCore(address _myMasterWarCore) external onlyOwner {
