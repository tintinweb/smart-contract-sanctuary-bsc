...
contract Custodian is Authorization, Pausable, ReentrancyGuard {
    ...
    mapping(IERC20 => mapping(address => uint256)) public locked;
    mapping(bytes32 => mapping(uint16 => bool)) public refunded;
    mapping(IERC20 => bool) public allowedTokens;

    event Lock(address indexed from, IERC20 indexed token, uint256 amount, uint256 newBalance);
    ...
    function lock(IERC20 token, uint256 amount) external whenNotPaused nonReentrant {
        ...
        uint256 newBalance = locked[token][msg.sender] + amount;
        token.safeTransferFrom(msg.sender, address(this), amount);
        locked[token][msg.sender] = newBalance;
        emit Lock(msg.sender, token, amount, newBalance);
    }
