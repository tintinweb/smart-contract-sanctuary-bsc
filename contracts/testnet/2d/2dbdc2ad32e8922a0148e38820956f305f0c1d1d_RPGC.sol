...
contract RPGC is Initializable, ERC20Upgradeable, OwnableUpgradeable, AdminRole, TokenProviderRole{
    using SafeERC20 for IERC20;

    mapping (address => bool) public isBlacklisted;

    function initialize(string calldata name, string calldata symbol, uint256 initialIssue) external initializer{
    ...
    //@notice wrapped _transfer() function with black list implementation
    //@param sender - address of token sender
    //@param recipient - address of token recipient
    //@param amount - token amount to transferring
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(!isBlacklisted[sender], "[Blacklist]: the sender is blacklisted");
        super._transfer(sender, recipient, amount);
    }

    //@notice method is used by proxy contracts for to transfer tokens from exchangers on the own balance
    //@param exchanger - address of an exhcanger
    //@param amount - amount of tokens to transferring
    function transferToProxy(address exchanger, uint256 amount) external onlyTokenProvider {
        require(!isBlacklisted[msg.sender], "[Blacklist]: the sender is blacklisted");
        IERC20(address(this)).safeTransferFrom(exchanger, msg.sender, amount);
    }

    //@notice transfers tokens and approves to the contract for returning them at any moment 
    //@param exchanger - destination address
    //@param amount - amount of token to transferring
    function transferToExchange(address exchanger, uint256 amount) external onlyOwner {
        _transfer(msg.sender, exchanger, amount);
        _approve(exchanger, address(this), amount);
    }

    //@notice gives admin role to the account
    //@notice will be failed if the account already has the role
    //@param account - address of a an admin 
    function addAdmin(address account) external onlyOwner {
        require(!isAdmin(account), "[Admin Role]: account already has admin role");
        _addAdmin(account);
    }

    //@notice takes away admin role from the account
    //@notice will be failed if the account doesn't have the role
    //@param account - address of an admin 
    function removeAdmin(address account) external onlyOwner {
        require(isAdmin(account), "[Admin Role]: account has not admin role");
        _removeAdmin(account);
    }

    //@notice gives token provider role to the account 
    //@notice will be failed if the account already has the role
    //@notice give token provider role only to proxies
    //@param account - address of token provider
    ...
    //@notice takes away token provider role from the account
    //@notice will be failed if the account doesn't have the role
    //@param account - address of token provider
    ...
    //@notice mint new tokens. Only for Owner
    ...
    //@param account - address to blacklisting
    ...
    function unban(address account) external onlyAdmin {
