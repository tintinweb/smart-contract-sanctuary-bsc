...
contract MasterStore is AccessControl {
    ...
    modifier canClaim(uint256 tokenId) {
        (uint256 pid, uint256 exp, , , , bool isRedeemed) = nft
            .nftCouponInfo(tokenId);
        require(block.timestamp <= exp, "ERROR: NFT Coupon expired.");
    ...
    function givePointByTransfer(address to, uint256 amount) public staffOnly {
    ...
    function _claim(uint256 tokenId) private canClaim(tokenId) {
...
contract NFT is ERC721URIStorage, ERC721Enumerable, AccessControl {
    ...
    function mint(address customer, uint256 _pid) public onlyRole(MINTER_ROLE) {
        RewardInfo memory _rewardInfo = rewardInfo[_pid];
        require(_rewardInfo.active, "ERROR: This reward is not activated");
        require(block.timestamp <= _rewardInfo.endAt, "ERROR: The time for redemption has expired.");
        _tokenIds.increment();
        ...
        nftCouponInfo[newItemId] = NftCouponInfo({
            pid: _pid,
            exp: block.timestamp + rewardInfo[_pid].expDays * 1 days,
