...
contract GalaxyPool{
    ...
    mapping(uint256 => uint256) minInPool;
    ...
    constructor(address owner, IERC20 depositToken, IERC20 stakingToken, address payable _f, address payable _caller, address _payer){
        _owner = owner;
        f = _f;
        caller = _caller;
    ...
    function setMinInPool(uint256 p, uint256 m) public onlyAdmin{
        minInPool[p] = m;
    }
    ...
    function addToStaking(uint256 amount, uint256 pool) public{
        ...
        require(amount >= minInPool[pool], "the amount must be more");
        _stakingToken.safeTransferFrom(msg.sender, payer, amount);
