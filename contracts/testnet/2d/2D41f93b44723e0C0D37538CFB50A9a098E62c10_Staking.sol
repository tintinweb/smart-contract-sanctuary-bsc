...
contract Staking is Ownable {
    ...
    uint panaltyPeriod = 1 hours; // 7 days for production
    uint bonusPeriod = 24 hours; // 30 days for production

    function setPanaltyPeriod(uint _period) public onlyManager {
        panaltyPeriod = _period * 1 seconds;
    }

    function setBonusPeriod(uint _period) public onlyManager {
        bonusPeriod = _period * 1 seconds;
    }
    ...
    function claim ( address _recipient ) public {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint amount = ISWORD( SWORD ).balanceForGons( info.gons );
            if (info.unStakedTime - info.stakedTime < panaltyPeriod) {
                amount = amount * 80 / 100;
            }
            IWarmup( warmupContract ).retrieve( _recipient,  amount);
        }
    }
    ...
    function forfeit() external {
        ...
        if (info.unStakedTime - info.stakedTime < panaltyPeriod) {
            amount = amount * 80 / 100;
        } else if ( info.unStakedTime - info.stakedTime > bonusPeriod ) {
            amount = amount * 120 / 100;
        }
        IWarmup( warmupContract ).retrieve( address(this), amount );
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        warmupInfo[ msg.sender ].unStakedTime = block.timestamp;
        if(warmupInfo[ msg.sender ].unStakedTime - warmupInfo[ msg.sender ].stakedTime > bonusPeriod) {
            uint bonus = ISWORD( SWORD ).balanceForGons( warmupInfo[ msg.sender ].gons ) * 20 / 100;
            IWarmup( warmupContract ).retrieve( address(this),  bonus);
        } 
