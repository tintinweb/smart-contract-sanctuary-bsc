...
abstract contract BatchReveal is IBatchReveal, VRFConsumerBaseV2Upgradeable, Initializable {
    ...
    function initializeBatchReveal(uint256 _revealBatchSize, uint256 _collectionSize) internal onlyInitializing {
...
contract Descriptor is OwnableUpgradeable, PausableUpgradeable, BatchReveal, IDescriptor {
    ...
    /// @param _donkeysBatchRevealSize Number of tokens to reveal in each batch
    /// @param _donkeysCollectionSize Total number of tokens in the collection
    ...
    function initialize(
        ...
        initializeBatchReveal(_donkeysBatchRevealSize, _donkeysCollectionSize);
    ...
    /// @param _vrfCoordinator Address of VRF consumer
    /// @param _keyHash Keyhash of the gas lane wanted
    /// @param _subscriptionId Chainlink subscription ID
    /// @param _callbackGasLimit Max gas used by the coordinator callback
    function setVRF(address _vrfCoordinator, bytes32 _keyHash, uint64 _subscriptionId, uint32 _callbackGasLimit)
        external
        onlyOwner
    {
        if (!isChainWithVRF) {
