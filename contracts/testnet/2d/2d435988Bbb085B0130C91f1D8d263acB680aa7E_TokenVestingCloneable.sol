 ...
 */
contract TokenVestingCloneable is AccessControl {
    ...
    uint96 public constant PERCENTAGE = 10000; // means 100
    ...
    function init(
        ...
        for (uint256 i = 0; i < _claimTime.length; i++) {
            ...
            Vesting memory vesting = Vesting(claimTime, claimPercentage);
        ...
        _grantRole(BLACKLISTER_ROLE, contractOwner);
    ...
    function claim() external notStopped notBlacklisted {
        ...
        if (availableAmountToClaim == 0) revert NoTokenToClaim();
        if (availableAmountToClaim > contractBalance) revert ContractDoesNotHaveEnoughFunds();

        _beneficiaryClaimedAmount[msg.sender] += availableAmountToClaim;
        claimedTokenAmount += availableAmountToClaim;

        token.safeTransfer(msg.sender, availableAmountToClaim);

        emit Claim(msg.sender, availableAmountToClaim);
    }
    ...
    function getBeneficiaryTotalClaimAmount(address beneficiary) external view returns (uint256 amount) {
        amount = _beneficiaryTotalClaimAmount[beneficiary];
    }

    function getBeneficiaryClaimedAmount(address beneficiary) external view returns (uint256 amount) {
        amount = _beneficiaryClaimedAmount[beneficiary];
    }

    function getVestings() external view returns (Vesting[] memory vestings) {
        vestings = _vestings;
    }
