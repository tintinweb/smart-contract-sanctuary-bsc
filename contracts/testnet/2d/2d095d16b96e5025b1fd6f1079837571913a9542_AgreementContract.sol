...
contract AgreementContract is Ownable {
    ...
    mapping(uint256 => mapping(Role => bool)) public claimed;
    ...
    function CreateContract(
        ...
        require(
            _agrement.price == totalMile_Price,
            "milestone price is same as contract price"
        );
    ...
    function Approve(
        ...
        // escrowAmount=agreementDetails[agreementID].price.div(getLatestPrice());
    ...
    function depositFunds(uint256 _agreementID,uint256 _milesID) public payable {
        ...
        require(msg.value == getMilestoneBNBAmount(_agreementID,_milesID), "Incorrect amount");
        escrowAmount = escrowAmount.add(msg.value);
    ...
    function approveMilestone(
        ...
        // require(milestoneDetails[_milesID].validated == true, "It must be validated");
        ...
        for (uint256 i = 0; i < agreementDetails[_agreementID].milstones.length; i++) {
            if (_milesID == agreementDetails[_agreementID].milstones[i]) {
                ...
                if(status == true) {
                    ...
                    escrowAmount = escrowAmount.sub(amount);
    ...
    function getMilestoneBNBAmount(uint256 agreementID, uint256 _mileID) public view returns(uint256){   
    ...
    function convertUSDtoBNB(uint256 amount) public view returns(uint256){
        uint256 _amount = amount.mul(1e18).div(getLatestPrice());
        return _amount;
    }

    // Used to validated contact(with respect to contract id) by validator(validator id).
    // function ValidateContract(uint256 agreementID,string memory validaterid) public {
    //     agreementDetails[agreementID].validated = true;
    //     // for (
    //     //     uint256 i = 0;
    //     //     i < agreementDetails[agreementID].validatersID.length;
    //     //     i++
    //     // ) {
    //     //     if (validaterid == agreementDetails[agreementID].validatersID[i]) {
    //     //         agreementDetails[agreementID].validated = true;
    //     //     }
    //     // }
    // }
    ...
    function ValidateMilestone(
        ...
        // ) {
        //     if (validaterid == agreementDetails[agreementID].validatersID[i]) {
        //         milestoneDetails[agreementID][milestoneID].validated = true;
        //     }
        // }
    }
    ...
    function raiseDispute(uint256 agreementID, uint256 milesid, Role role) public {
        require(fundTransfered[agreementID][milesid] != true,"Payment already transfered");         
        if (role == Role.employee) {
            ...
            claimed[agreementID][role] = true;

        } else if (role == Role.employer) {
            require(milestoneDetails[agreementID][milesid].Approve != true, "Check your Approval status");
            require(escrowAmount >= milestoneDetails[agreementID][milesid].Mile_Price, "Funds not escrowed");
            claimed[agreementID][role] = true;
    ...
    function disputeResolve(uint256 agreementID, uint256 milesID, Role role, uint256 amount, address receiver) public onlyOwner{
        require(claimed[agreementID][role] == true, "No dispute raised");
