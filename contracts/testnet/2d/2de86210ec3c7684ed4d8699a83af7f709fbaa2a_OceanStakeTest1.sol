...
contract OceanStakeTest1 {
    ...
    //multiplier for ether parameters when using a token or a different coin than ethereum (bnb 10x,avax 100x,matic 1000x etc.)
    uint256 public etherMultipliedBy = 1; //(MIN_REF MIN_INVEST MAX_INVEST BALANCE_STEP)
    ...
    uint256 public MIN_REF = 0.05 ether; //min amount of investment needed to withdraw ref rewards
    ...
    constructor(
        ...
        if (_multiplyEtherBy > 0) {
            //i.e. x1 for ethereum, x10 for bnb, x100 for avax, x3000 for busd/usdt/usdc
            etherMultipliedBy = _multiplyEtherBy;
        }

        if (etherMultipliedBy > 1) {
        ...
        nonReentrant
    {
        ...
        if (user.invested == 0) {
            ...
            userCount++;
            refInc = 1;
            emit NewInvestor(addr, user.upline, block.timestamp);
        }
