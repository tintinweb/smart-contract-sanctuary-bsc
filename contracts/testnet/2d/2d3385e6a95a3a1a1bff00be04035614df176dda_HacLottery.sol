...
// File @openzeppelin/contracts/security/Pausable[email protected]
...
contract HacLottery is VRFConsumerBaseV2, Ownable, Pausable {
    VRFCoordinatorV2Interface COORDINATOR;
    uint64 s_subscriptionId = 1560;
    ...
    address public tokenAddress = 0xd5A8Fe9Ff205ad178936a472f94a2BC10013923e;
    IERC20 token;
    ...
    uint256 public maxTicketsPerPlayer = 1400;
    uint256 public maxTicketsPerTx = 1000;
    ...
    function enter(uint256 tickets) external whenNotPaused {
        ...
        token.transferFrom(msg.sender, address(this), tickets * entryFee);
    }
   ...
   /* function payoutLottery() external {
        ...
        uint256 entryIdOfWinner = s_randomWords % entryCounter;
    ...
    function reset() external onlyOwner {
        endTimes[lotteryID] = 0;
        lotteryID++;
        players = new address[](0);
        _pause();
    }

     function setLotteryParameters(uint256 _newEntryFee, uint256 _newMaxTicketPerPlayer, uint256 _newMaxTicketPerTx) external onlyOwner {
        entryFee = _newEntryFee;
        maxTicketsPerPlayer = _newMaxTicketPerPlayer;
        maxTicketsPerTx = _newMaxTicketPerTx;
    }

    function setLotteryToken(address _newToken) external onlyOwner {
        tokenAddress = _newToken;
    }

    function setDuration(uint _newDuration) external onlyOwner {
        duration = _newDuration;
        _pause();
    }

    function wiwthdrawErc20(address _address, uint256 _amount) external payable onlyOwner {
