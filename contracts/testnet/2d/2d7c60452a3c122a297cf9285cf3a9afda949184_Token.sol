...
contract Token is Context, Ownable, IERC20 {
    ...
    address private _teamWallet = 0xd98F0A2Fd778ad56eF960f045ed41d980a92145F;
    address private _marketingWallet =
        0x55Fd8c8736Cb44E0EAe8F37f6C8ddBA3EB9EeC41;
    ...
    uint256 private constant AMMLimit = (_tTotal / (10**2)) / (10**4);
    //when liquidity is low, this needs to be true to avoid liquidate error because of high slippage
    bool private liquididateByLimit = true;
    ...
    function circulatingSupply() public view returns (uint256 _cSupply) {
     ...
     */
    function changeTaxStatus(bool status) public onlyOwner {
        _taxEnabled = status;
    }
     ...
     * @dev Enable tax after IDO
     */
    function setLiquidifyByLimit(bool status) public {
        require(_msgSender() == _teamWallet, "Permission: UNAUTHORIZED");
        liquididateByLimit = status;
    }

    /**
     * @dev Allow marketing wallet to claim initial fund of 5% of total supply after IDO
     */
    function marketingClaimInitialShare() public validClaim onlyOwner {
        _transferWithoutFee(owner(), _marketingWallet, (_tTotal * 5) / 100);
    }
    ...
    function changeTeamWallet(address wallet) public {
        address sender = _msgSender();
        require(
            _isTeam(sender),
        ...
        address currentTeamWallet = _teamWallet;
        _teamWallet = wallet;
        // Move all token from current team wallet to the new one
        _tOwned[_teamWallet] = _tOwned[currentTeamWallet];
        _tOwned[currentTeamWallet] = 0;
        // Exclude new team wallet from receiving rewards
        _isExcludedFromFee[_teamWallet] = true;
        _excludeAccount(_teamWallet);
    }
    ...
    /**
     * @dev Only use if there's a new version of PancakeSwap Router
     */
        ...
        returns (address newPairAddress)
    {
        require(
            _isTeam(_msgSender()),
            "Permission: Address doesnot have permission"
        );
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            newRouterAddress
        );
    ...
    function _excludeAccount(address account) private {
        ...
        else _tOwned[account] = tokenFromReflection(_rOwned[account]);
        ...
        )
    {
        ...
        (rAmount, rTransferAmount, rTotalFee) = _getRValues(
            tAmount,
            tTotalFee,
            _getRate()
        );
    ...
    function _isTeam(address wallet) internal view returns (bool) {
        return wallet == _teamWallet;
    }
    ...
    function _takeTeamFee(uint256 rTotalFee, uint256 tTotalFee) private {
        ...
        _rOwned[_teamWallet] = _rOwned[_teamWallet] + rTeam;
        _tOwned[_teamWallet] = _tOwned[_teamWallet] + tTeam;
    }
    ...
    function _checkExceededAMMLimit() private {
        if (!inSwapAndLiquify && _swapAndLiquifyEnabled) {
            ...
            if (overMaxTokenBalance) {
                if (liquididateByLimit) swapAndLiquify(AMMLimit);
                else swapAndLiquify(contractTokenBalance);
            }
        }
    }
