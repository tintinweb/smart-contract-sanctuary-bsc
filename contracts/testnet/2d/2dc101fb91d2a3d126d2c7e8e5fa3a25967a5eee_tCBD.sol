...
contract tCBD is Context,IERC20Metadata,Ownable {
   ...
   //event payFeeEV(address indexed topc, address from, address to, uint256 feeCBD);
    ...
    function _transfer(
        ...
        uint256  feeCBD = buyFeeRate.div(totalFeeRate).mul(payCBD); 
        totalFee += feeCBD;
        totalBurn += burnCBD;
        ...
        emit burnFeeEV(lpContractAddress,address(this),burn,burnCBD);  
        emit Transfer(address(this),burn,burnCBD);
        
        emit intotTreasury(lpContractAddress,address(this),treasury,feeCBD);
        emit Transfer(address(this),burn,realCBD);
        }
        ...
        //卖 94% cbd 给 lp ,5% cbd 手续费 ，1%销毁  
         emit intoContractEV(lpContractAddress,msg.sender,recipient,feeCBD);
         emit Transfer(msg.sender,recipient,totalCBD.sub(burnCBD).sub(feeCBD));
