...
contract BUSDSTAKER {
    ...
    uint256 public stake_period = 30 days;
    ...
    function renivest() public valid_user(msg.sender) {
        ...
        total_staked = total_staked + (calculaterewards(msg.sender));
        ...
        if (calculaterewards(msg.sender) > 0) {
            BUSD.transferFrom(
                ...
                calculaterewards(msg.sender) -
                    ((calculaterewards(msg.sender) * dev_fee_percentage) / percent_divider)
            );
            BUSD.transferFrom(
                reward_address,
                dev_fee_address,
        ...
        plan[msg.sender].amount =
            plan[msg.sender].amount +
            (calculaterewards(msg.sender) -
                ((calculaterewards(msg.sender) * tax) / percent_divider));
        uint256 _stake_percent = getpercent();
    ...
    function withdrawrefrewards() external valid_user(msg.sender) {
        require(reffral_rewards[msg.sender] > 0, "no claimable amount available");
        BUSD.transfer(msg.sender, reffral_rewards[msg.sender]);
