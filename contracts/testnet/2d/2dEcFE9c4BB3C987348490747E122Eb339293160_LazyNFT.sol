    ...
    function recover(SigningInfo memory signing) public view returns (address) {
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "SigningInfo(address nftContractAddress,uint256 tokenId,uint256 amount,uint256 price,uint256 startTime,uint256 endTime,address paymentToken)"
                    ),
        ...
        address signer = ECDSA.recover(digest, signing.signature);
        return signer;
    }
