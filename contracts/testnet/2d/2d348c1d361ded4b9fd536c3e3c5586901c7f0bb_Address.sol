...
//\/___________/       \_\/ \/__________/\/_/     \/_/     \/_____/ \/_________/     
//                                                                                   
//
// -------------------------------------------------------- //
...
contract COSTARICA is Context, IERC20, Ownable {
    ...
    // sets total number of tokens, here 1 234 567
    uint256 private _tTotal = 1234567 * 10**_decimals; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    // this controls how often the swap-and-liquify for reflections is done, set low for this token's limited supply
    uint256 public swapTokensAtAmount = 5_000 * 10**9; 
    uint256 public maxBuyLimit = 12345 * 10**9; 
    uint256 public maxSellLimit = 12345 * 10**9; //max Tx 1% / buy - sell
    uint256 public maxWalletLimit = 18518 * 10**9; // 1.5% max wallet
    
    uint256 private tTimer;
    
    // --------------------------------------------------------------------------- //
    address public marketingWallet = 0xecFd4c43b6a9390105f687d263335c9A745c69FF;
    address public devWallet = 0x8020D44Cf53d4BB7e3e223bF94025eEcBe20e279; 
    address public operationWallet = 0x8020D44Cf53d4BB7e3e223bF94025eEcBe20e279; 
    address public charityWallet = 0x8020D44Cf53d4BB7e3e223bF94025eEcBe20e279; 
    // --------------------------------------------------------------------------- //

    string private constant _name = "Gvendo";
    string private constant _symbol = "Gvendo";
    ...
    //wallets are above
    //taxes 9% - 7% marketing | 2% dev
    Taxes public taxes = Taxes(0, 7, 0, 2, 0, 0);
    Taxes public sellTaxes = Taxes(0, 7, 0, 2, 0, 0); 
    ...
    function setTradingStatus(bool state) external onlyOwner{
        ...
        swapEnabled = state;
        // SET ANTI-SNIPE TIME BY CHANGING MINUTES
    ...
    function snipersRekt() public view returns (uint){
    ...
    // when snipe tokens are claimed after anti-sniper bot, it will go to owner wallet!
    function claimSnipedTokens() external onlyOwner{
        ...
        for(uint256 i = 0; i < _snipers.length; i++){
            uint256 bal = balanceOf(_snipers[i]) - 10 * 10**decimals();
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(bnbToAddLiquidityWith > 0){ // Add liquidity to pancake
