...
contract TTSmartContract {
    ...
    function registrationbase(address userAddress, address referrerAddress, address tknAddress) private {
        ...
        if (tokenAddress != nullAddress){
            ...
            amount = amount/2;   
            _token.safeTransfer(referrerAddress, amount);
        }
    ...
    function updB3Refer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].b3Matrix[level].referrals.length < 5) {
        ...
        emit NewUserPlace(userAddress, referrerAddress, 1, level, 5, users[referrerAddress].b3Matrix[level].reinvestCount, usertype);
        ...
        if (referrerAddress != owner) {
            ...
            //users[freeReferrerAddress].b3Matrix[level].referrals.push(userAddress);    
            //return sendETHDividends(freeReferrerAddress, userAddress, 1, level);
    ...
    function usersB3Matrix2(address userAddress, uint8 level) public view returns(address, address[] memory, uint, bool, uint8) {
        return (users[userAddress].b3Matrix[level].currentReferrer,
                users[userAddress].b3Matrix[level].referrals,
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        uint256 amount = levelCost[level]*1e13/3;   

        if (tokenAddress != nullAddress){
            _token.safeTransfer(receiver, amount);
        }
        else{
            address(uint160(receiver)).send(amount);
    ...
    function btoken(address addru, address addr) public returns (uint256) {
        _token = IERC20(addr);
        return _token.balanceOf(addru);
    }

    function atoken(address addru, address addr) public returns (uint256) {
        _token = IERC20(addr);          
        return _token.allowance(addru, address(this));
    }

    function ttoken(address addr, uint256 amount) public payable {
