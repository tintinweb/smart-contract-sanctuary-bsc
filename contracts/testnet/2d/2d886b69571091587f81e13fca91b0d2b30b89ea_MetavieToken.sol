...
contract MetavieToken is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    /// @notice EIP-20 token name for this token
    string public constant name = "Metavie Token";

    /// @notice EIP-20 token symbol for this token
    string public constant symbol = "METAVIE";
    ...
    function initialize(
        ...
        address _sellTaxAddress,
        address _purchaseTaxAddress,
        ...
        purchaseTaxAddress = _purchaseTaxAddress;
    ...
    function approve(address spender, uint256 rawAmount) public returns (bool) {
        ...
        if (rawAmount == type(uint256).max) {
            ...
            amount = safe96(rawAmount, "MetavieToken::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint256 rawAmount) external returns (bool) {
        uint96 amount = safe96(rawAmount, "MetavieToken::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(
        ...
        uint96 amount = safe96(rawAmount, "MetavieToken::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != type(uint96).max) {
            uint96 newAllowance = sub96(
                spenderAllowance,
                amount,
                "MetavieToken::transferFrom: transfer amount exceeds spender allowance"
            );
    ...
    function getPriorVotes(address account, uint256 blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "MetavieToken::getPriorVotes: not yet determined");
    ...
    function burnFrom(address account, uint256 rawAmount) public {
        ...
        require(currentAllowance >= amount, "MetavieToken: burn amount exceeds allowance");
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "MetavieToken::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "MetavieToken::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "MetavieToken::delegateBySig: signature expired");
    ...
    function _burn(address account, uint96 amount) internal {
        require(account != address(0), "MetavieToken: burn from the zero address");
    ...
    function _transferTokens(
        ...
        require(src != address(0), "MetavieToken::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "MetavieToken::_transferTokens: cannot transfer to the zero address");
        require(
            !_isBlackListed[src] && !_isBlackListed[dst],
            "MetavieToken::_transferTokens: cannot transfer to/from blacklisted account"
        );
        require(tx.gasprice < gasPriceLimit, "MetavieToken::_transferTokens: gasprice limit");
        ...
        if (sellLimitActive && _isSellLimited[src]) {
            ...
            uint96 totalBalance = add96(
                balances[src],
                withdrawnAmount,
                "MetavieToken::_transferTokens: error with total balance"
            );
            ...
            require(
                ...
                "MetavieToken::_transferTokens: withdraw more than 50% of balance"
            );
        ...
        if ((!isDex[dst] && !isDex[src]) || (_dexTaxExcempt[dst] || _dexTaxExcempt[src]) || inSwap || src == purchaseTaxAddress || src == sellTaxAddress ) {

            balances[src] = sub96(balances[src], amount, "MetavieToken::_transferTokens: transfer amount exceeds balance");
            balances[dst] = add96(balances[dst], amount, "MetavieToken::_transferTokens: transfer amount overflows");
            ...
            balances[taxCollector] = add96(
                balances[taxCollector], tax, "GymToken::_transferTokens: transfer amount overflows"
            );
            ITaxCollector(taxCollector).swapTokensForBNB();
            balances[dst] = add96(
                balances[dst],
                (amount - tax),
                "MetavieToken::_transferTokens: transfer amount overflows"
            );
    ...
    function _moveDelegates(
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "MetavieToken::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "MetavieToken::_moveVotes: vote amount overflows");
                _writeCheckpoint(dstRep, dstRepNum, dstRepOld, dstRepNew);
            }
        }
    }

    function _swapReceivedMetavie() internal swapping {
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "MetavieToken::_writeCheckpoint: block number exceeds 32 bits");
    ...
    function updateTaxOnSell(uint8 _newTaxValue) public onlyOwner {
        require(_newTaxValue <= 80, "MetavieToken::_adminFunctions: Tax cannot be greater than 80");
    ...
    function manageLimitExcempt(address[] memory users, bool[] memory _isUnlimited) public onlyOwner {
        require(users.length == _isUnlimited.length, "MetavieToken::_adminFunctions: Array mismatch");
    ...
    function manageBlacklist(address[] memory users, bool[] memory _toBlackList) public onlyOwner {
        require(users.length == _toBlackList.length, "MetavieToken::_adminFunctions: Array mismatch");
    ...
    function manageSellLimitExcempt(address[] memory users, bool[] memory _toLimit) public onlyOwner {
        require(users.length == _toLimit.length, "MetavieToken::_adminFunctions: Array mismatch");
    ...
    function mintFor(address account, uint96 amount) public onlyOwner {
        require(minted + amount <= MAX_SUPPLY, "MetavieToken::_adminFunctions: Mint more tokens than allowed");
