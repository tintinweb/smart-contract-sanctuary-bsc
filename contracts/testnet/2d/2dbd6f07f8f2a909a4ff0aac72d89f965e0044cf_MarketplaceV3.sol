...
contract MarketplaceV3 is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  function executeOrderWithBusd(uint256 orderId) external whenNotPaused {
    ...
    if (ownerCutPerMillion > 0) {
      ...
      // Transfer share amount for marketplace Owner
      if (vault == address(0)) {
        acceptedToken.burnFrom(address(this), saleShareAmount);
        emit FeeBurned(saleShareAmount, orderId);
      } else {
        acceptedToken.transferFrom(address(this), vault, saleShareAmount);
      }
    }

    // Transfer sale amount to seller
    acceptedToken.transferFrom(address(this), seller, price - saleShareAmount);
  ...
  function _buyMHT(uint256 _mhtAmountOut) public whenNotPaused returns (uint256[] memory amounts) {
    ...
    busd.transferFrom(address(this), owner, swapFee);
