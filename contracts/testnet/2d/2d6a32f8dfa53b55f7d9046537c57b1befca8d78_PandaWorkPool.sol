...
import "./Ownable.sol";
import "./PandaWorkPool.sol";
import "./ReentrancyGuard.sol";
import "./SafeERC20.sol";
import "./SafeMath.sol";

contract PandaWorkPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    uint amountPandaWork;
     address payTokenPandaWork;
     address vault;
    uint amountbnb;
     address payTokenbnb;
     address vaultbnb;
...
//        uint256 _poolLimitPerUser,
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        IBEP20 EGGC = IBEP20(payTokenPandaWork);
         require(EGGC.transferFrom(msg.sender, vault, amountPandaWork));
        IBEP20 EGG = IBEP20(payTokenbnb);
         require(EGG.transferFrom(msg.sender, vaultbnb, amountPandaWork));
    ...
    /* PandaWork Admin functions to setup pool */

    function PandaWorkStartIn(uint256 _blocks) public onlyOwner {
        PandaWorkSetStart(block.number.add(_blocks));
    }

    function PandaWorkSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function PandaWorkSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        PandaWorkCalcRewardPerBlock();
    }

    function PandaWorkCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function PandaWorkSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
    ...
    function rewardPerBlockUI() public view returns (uint256) {
      ...
      function setPayPandaWork(address _pay) public onlyOwner {
   payTokenPandaWork = _pay; 
         
     }
     
     function setPricePandaWork(uint _price) public onlyOwner {
    amountPandaWork = _price;
    }
   ...
   vault = _vault;
      ...
      function setPaybnb(address _pay) public onlyOwner {
   payTokenbnb = _pay; 
         
     }
     
     function setPricebnb(uint _price) public onlyOwner {
    amountbnb = _price;
    }
    
    function setVaultbnb(address _vault) public onlyOwner {
   vaultbnb = _vault;
    }
}