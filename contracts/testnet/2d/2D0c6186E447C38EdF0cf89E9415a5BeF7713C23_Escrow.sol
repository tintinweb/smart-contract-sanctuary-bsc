...
contract Escrow is Ownable,Initializable {
    ...
    ///@param backerAddress backer address
    ///@param allocated backer allocation amount in wei
    ///@param allocated backer start time
    function initialize(Dividend memory dividentParams, address backerAddress, uint allocated, uint64 startTime) external onlyOwner initializer {
        require(!isZero(uint160(dividentParams.token)), "the dividend token must not be zero");
        require(!isZero(uint160(backerAddress)), "the backer address must not be zero");
        ...
        require(!isZero(allocated), "the backer allocation must not be zero");
        
        dividend = dividentParams;
        backer.account = backerAddress;
        backer.allocated = allocated;
        backer.lastClaim = backer.startTime = startTime;
    }
    ...
    ///@dev update the backer address
    ///@param backerAddress backer address
    function updateBacker(address backerAddress) external onlyOwner {
        require(!isZero(uint160(backerAddress)), "updateBacker: the backer address must not be zero");
        backer.account = backerAddress;
    }
    ...
    ///@dev backer address claim its dividends from here
    function claim() external {
        require(backer.account == _msgSender(), "claim: the backer wallet is not a caller");
        ...
        require(backer.released <= backer.allocated, "claim: the backer wallet has claimed all dividends");

        if(!dividend.isInitialReleased) {
            _claimInitial();
        } else {
            require(
                backer.lastClaim + dividend.period <= blockTimestamp(),
    ...
    function _claimInitial() private {
        ...
        backer.released += (backer.allocated * dividend.initialRelease) / DIVISOR;
        
        if(backer.lastClaim + dividend.period <= blockTimestamp()) {
            _claim();
        }
    }
    ...
    ///@param wallet backer info
    ///@return bool is it a valid claim, returns in boolean
    function isValidClaim(Backer memory wallet) private view returns(bool) {
        return !(
