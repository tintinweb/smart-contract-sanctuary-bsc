...
contract Orchard is TransferOwnable {
    ...
    function toOrchard(Convoy.ConvoyEntry memory convoy, Convoy.OrchardEntry memory orchard, uint256 orchardLevel) public view returns (Convoy.OrchardEntry memory) {
        ...
        require(convoy.damage <= 80, 'There is equipment in the Convoy that needs to be repaired');
    ...
    function checkReward(Convoy.ConvoyEntry memory convoy, Convoy.OrchardEntry memory orchard) public returns (Convoy.OrchardEntry memory){
        ...
        if(number < selectOrchard.success){
				
            //success
            orchard.isSuccess = 1;
            orchard.balance = selectOrchard.reward;
            orchard.rewards += selectOrchard.reward;
