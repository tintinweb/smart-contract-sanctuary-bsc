...
contract KGTDAO_Router {
    ...
    mapping(address=> pledgeLog[]) private userPledgeLog;
    pledgeLog[] private allPledgeLog;
    ...
    function myPledgeLog(address _addr)public view returns(pledgeLog[] memory){
        ...
        return userPledgeLog[_address];
    }
    function getAllPledgeLog()public view returns(pledgeLog[] memory){
        require(msg.sender == owner);
        return allPledgeLog;
    }
    function getAllVIPLog()public view returns(vipLog[] memory){
        require(msg.sender == owner);
        return all_log_vipBuy;
    }
    function getAllUser()public view returns(user[] memory){
        return getTeam_all[KGTDAO];
    }
    ...
    function pledge(uint _kgtValue,address _token,uint _tokenValue,address _referrer) public payable {
        ...
        while(referrerAddress != address(0x0)){
            ...
            if(isNewUser){
                ...
                if(referrerAddress == allUsers[msg.sender]._referrer){
                    getTeam_1[referrerAddress].push(allUsers[msg.sender]);
                }
                getTeam_all[referrerAddress].push(allUsers[msg.sender]);
            }
            referrerAddress = allUsers[referrerAddress]._referrer;
        }
        pledgeLog memory _log = pledgeLog(_kgtValue,0,_tokenValue,_token,block.timestamp,_weight_get);
        userPledgeLog[msg.sender].push(_log);
        allPledgeLog.push(_log);
    ...
    function upgradeVIP(uint payKGT,uint16 level_set)external payable{
        ...
        all_log_vipBuy.push(vipLog(msg.sender,level_set,weight_get,payKGT,payUSD,block.timestamp,(level_set == level_last + 1) ? 0 : 1));
