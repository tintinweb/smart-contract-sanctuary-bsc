...
// SPDX-License-Identifier: MIT
    ...
    abstract contract ReentrancyGuard {
        ...
        contract BnbYielding is ReentrancyGuard{
            ...
            uint256 private MAX_DEPOSIT_BONUS_STEP_TIME = 5 days;
            uint256 public USER_DEPOSITS_STEP = 10 ether; //once 10bnb deposits
            uint256 private DEPOSIT_BONUS_PERCENT = 10 * 1e15; //1% 
            uint256 private PRC_PARTN = 100; //1% 
            uint256 private PRC_BONUS_HOLDER = 10; //1%
            uint256 private REINVEST_PERCENT = 100;
            uint8 private TimeReInvest = 24; //24 days

            uint256 public VIP_TARIF = 20; // 20 days hold
            uint256 public VIP_TARIF_PRICE = 20 * 1e18; // access with 20bnb
            uint256 private VIP_WITHDRAW_TIME_STEP = 1728000; // 20 days hold
            uint256 private VIP_FeeWithdraw = 300; // 30% of fee for withdraw user vip

            uint256 private TaxWithdrawWhale = 450; //45%
            uint256 private TaxDepositWhale = 400; //40%
            uint256 private MAX_TaxWITHDRAW_COUNT = 1;

            uint8   constant BONUS_LINES_COUNT = 5;
            uint16  PERCENT_DIVIDER = 1000;
            ...
            constructor(address payable _marketing_cost,address payable _secure_wallet,uint256 timestamp) {
                ...
                uint256 tarifPercent = 175;
                uint256 accumulator = 5;
        ...
        function investorInfo(address _addr) view external returns(uint256 for_withdraw, uint256 total_invested, uint256 total_withdrawn, uint256 total_match_bonus, uint256[BONUS_LINES_COUNT] memory structure, uint256 _checkpoint) {
            ...
            function contractInfo() view external returns(uint256 _invested, uint256 _withdrawn, uint256 _match_bonus, uint256 _secure_pool, uint256 _initUNIX, uint256 _totalInvestors) {
                return (invested, withdrawn, match_bonus, secure_pool, initUNIX, totalInvestors);
            }
