...
contract NftIdo is Configurable, IERC721ReceiverUpgradeable {
    using AddressUpgradeable for address;
    using SafeERC20 for IERC20;
    
    address public nft;   //sell
    address public ercToken;// buy with erc20 token
    uint public begin;
    uint public end;
    uint public price;


    mapping (address => uint ) public buyNums;  
    
    mapping (uint => uint ) public ids; //index =>tokenId
    uint public inIndex;
    uint public soldIndex;



    function __NftIdo_init(address governor_,address _nft,address _ercToken,uint _begin,uint _end,uint _price) public initializer {
	    __Governable_init_unchained(governor_);
        __NftIdo_init_unchained(_nft,_ercToken,_begin,_end,_price);
	}
    
    function __NftIdo_init_unchained(address _nft,address _ercToken,uint _begin,uint _end,uint _price) public governance  {
        nft = _nft;
        ercToken = _ercToken;
        begin = _begin;
        end = _end;
    ...
    function setNftErc20AndPrice(address _nft,address _ercToken,uint _price) public governance{
        nft = _nft;
        ercToken = _ercToken;
        price = _price;
	}
    ...
    function buy() public payable {
        ...
        bool isEth = (ercToken==address(0)?true:false);
        if (isEth){
            require(msg.value>=price*num,"Insufficient amount");
            if (msg.value>price*num){
               (bool success,) = payable(msg.sender).call{value:(msg.value-price*num)}("");
               require(success,"send bnb faild");
            }
        }
        else{
            IERC20(ercToken).safeTransferFrom(msg.sender, address(this), price*num);
        }
        uint cur = soldIndex;
        require(cur+num<=inIndex,"Insufficient nft");
        uint tokenId;
        soldIndex += num;
        for (uint i=0;i<num;i++){
            ...
            emit Buy(msg.sender,nft,tokenId,ercToken,price);    
        }
    }
    event Buy(address indexed account, address nft,uint tokenId,address ercToken,uint price);
    

    function withdrawNft(address to,uint max) virtual  public governance{
        uint cur = soldIndex;
        uint realNum = max;
        if (cur+max>inIndex)
            realNum = inIndex-cur;
        uint tokenId;
        soldIndex += realNum;
        for (uint i=0;i<realNum;i++){
            cur++;
            tokenId = ids[cur];
            IERC721Upgradeable(nft).safeTransferFrom(address(this),to,tokenId);  
        }
        emit WithdrawNft(to,nft,realNum);
    }
    event WithdrawNft(address indexed to, address nft,uint num);

   function withdrawErc20(address to) virtual  public governance{
       if (ercToken!=address(0)){
            uint balance = IERC20(ercToken).balanceOf(address(this));
            IERC20(ercToken).safeTransfer(to, balance);
        }else{
