...
contract FOE is ERC20, Ownable {
    ...
    address public marketingWallet = 0x5A680AEAf0177Ea26f9082F937E8A645954b3de2;
    address public charityWallet = 0x5A680AEAf0177Ea26f9082F937E8A645954b3de2;
    ...
    event MarketingWalletUpdated(address previousWallet, address newWallet);
    event CharityWalletUpdated(address previousWallet, address newWallet);

    constructor () ERC20("FOE", "FOE") {


        // exclude from paying fees and max wallet limit
    ...
    function _transfer(
        ...
        if(!excludedAccount && (automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to])) {
            ...
            if(marketingFee > 0) {
                marketingTax = amount.mul(marketingFee).div(100);
                super._transfer(from, marketingWallet, marketingTax); 
            }

            if(charityFee > 0) {
                charityTax = amount.mul(charityFee).div(100);
                super._transfer(from, charityWallet, charityTax); 
            }

            amount = amount.sub(marketingTax.add(charityTax));
        }
    ...
    function setFees(uint256 _marketingFee, uint256 _charityFee) external onlyOwner {
        require(_marketingFee.add(_charityFee) <= 5, "Fees too hugh");
    ...
    function setMarketingWallet(address _newWallet) external onlyOwner {
        require(_newWallet != address(0), "marketing wallet can't be set as dead address");
        require(_newWallet != marketingWallet, "Marketing wallet already has that address");
      ...
      function setCharityWallet(address _newWallet) external onlyOwner {
        require(_newWallet != address(0), "charity wallet can't be set as dead address");
        require(_newWallet != charityWallet, "Charity wallet already has that address");
        address previousWallet = charityWallet;
  	    charityWallet = _newWallet;
        emit CharityWalletUpdated(
            previousWallet,
            _newWallet
        );
  	}
