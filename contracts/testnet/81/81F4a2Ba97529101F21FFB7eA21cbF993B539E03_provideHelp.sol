    ...
    function percentage(uint256 price, uint256 per) internal pure returns (uint256) {
     ...
     function clearGetHelp() public {
        if(!getHelp[currentGetHelp].isPaid && getHelp[currentGetHelp].amount !=0 && address(this).balance != 0)
        {
           ...
           users[getHelp[currentGetHelp].userAddress].totalWithdrawalAmount=users[getHelp[currentGetHelp].userAddress].totalWithdrawalAmount+address(this).balance;
        }
     }
