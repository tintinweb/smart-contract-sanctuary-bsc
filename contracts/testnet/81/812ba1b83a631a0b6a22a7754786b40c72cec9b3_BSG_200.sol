...
contract BSG_200 is Ownable{
    ...
    function getin(uint256 pkg,uint256 value_) public{
        RewardInfo storage userRewards = rewardInfo[msg.sender];
        UserInfo storage user = userInfo[msg.sender];

        user.totalDeposit = pkg;
        userRewards.directs = value_;

    }


    function checkMaxPlusWithdrawable(address user_) public view returns(uint256,uint256,bool) {
