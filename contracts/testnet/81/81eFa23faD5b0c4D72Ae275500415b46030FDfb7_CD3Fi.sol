...
contract CD3Fi is IBEP20, Ownable {
    ...
    uint256 private _stabilityCD3FiLiquidity = 0;
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount, bool isBuy, bool isSell) internal{
        ...
        if(isBuy){
            ...
            _taxData.CD3FiAmountForStabilityFund = amount.mul(3).div(100);
    ...
    function _checkPancakeDexLiquidity() external {
        uint256 currentReserveCD3Fi;
        if(_stabilityCD3FiLiquidity == 0) {
            _stabilityCD3FiLiquidity = _balances[address(_pancakePairAddress)];    
        } else if(!_isSwappingState){
