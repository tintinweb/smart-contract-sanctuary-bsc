...
uint256 public time=24*60*60; 
 ...
 constructor() ERC20("uroboros","urbrs") {
     	_mint(msg.sender,1000000*10**decimals());
 }


function minutesOrDays() external onlyOwner {
    if (time==24*60*60)
        time=60;
    else 
        time=24*60*60;
...
function getRewardsAmount (address user) public view returns (uint256 rewards) {
         if (burnedTokensTime[user]>0 && burnedTokensAmount[user]>100 )
            return ((block.timestamp-burnedTokensTime[user])/(time)*(burnedTokensAmount[user]/100));
         else 
            return 0;
}
