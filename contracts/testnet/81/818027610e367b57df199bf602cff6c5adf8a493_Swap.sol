...
contract Swap is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        rematicFinanceAddress = _tokenAddress;
        pancakeSwapPair = owner();

    }

    function _authorizeUpgrade(address newImplementaion) internal override onlyOwner {}

    function setPairAddress(address newAdress) public onlyOwner {
        pancakeSwapPair = newAdress;
    }

    function sellTokensForETH(uint256 tokenAmount) public onlyOwner {

        IERC20(rematicFinanceAddress).approve(address(pancakeSwapV2Router), tokenAmount);
    ...
    function faucetTokensToPair(uint256 amount) public onlyOwner {
        IRematic(rematicFinanceAddress).transfer(pancakeSwapPair, amount);
    }
