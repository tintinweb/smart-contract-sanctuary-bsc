...
contract Vault is Initializable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function swapBUSDToPegTokenAndGetLink() external onlyOwner {
        ...
        paymentToken.approve(address(router), busd.balanceOf(address(this)));
        paymentToken.approve(pairAddress, busd.balanceOf(address(this)));
        router.swapExactTokensForTokens(
            busd.balanceOf(address(this)),
            minAmounts[1],
            path,
        ...
        pegSwapContract.swap(
            pegToken.balanceOf(address(this)) /
                pegToken.balanceOf(address(this)),
            address(pegToken),
            address(liquidityLink)
        );
        emit SwapPegTokenToLiquidityLink(minAmounts[1], block.timestamp);
        */
    }
}
