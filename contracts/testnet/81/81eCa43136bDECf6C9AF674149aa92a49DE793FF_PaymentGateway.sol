...
contract PaymentGateway is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event ProductPayed(address indexed user, bytes32 product_code, bytes32 coupon_code, bytes32 referrer_code, uint256 total_charged);
    ...
    mapping(address => bytes32) public users_referrer_code;
    mapping(address => mapping(bytes32 => bool)) public users_coupon_used;

    mapping(bytes32 => Referrer) public referrer_data;
    mapping(bytes32 => Product) public product_data;
    mapping(bytes32 => Coupon) public coupon_data;

    bytes32[] public refererrer_code_list;
    bytes32[] public product_code_list;
    bytes32[] public coupon_code_list;
    ...
    function referrerCreate(address user, bytes32 code, uint8 cut_percent) public onlyOwner {
        ...
        bytes32 user_code = users_referrer_code[user];
    ...
    function referrerSetActive(address user, bool active) public onlyOwner {
        bytes32 code = users_referrer_code[user];
        Referrer storage referrer = referrer_data[code];
    ...
    function productCreate(bytes32 code, uint256 price) public onlyOwner {
    ...
    function productSetActive(bytes32 code, bool active) public onlyOwner {
        Product storage product = product_data[code];
    ...
    function productSetPrice(bytes32 code, uint256 price) public onlyOwner {
        Product storage product = product_data[code];
    ...
    function couponCreate(bytes32 code, uint8 discount_percent, bytes32[] memory product_list) public onlyOwner {
        ...
        for (uint i=0; i < product_list.length; i++) {
            bytes32 product_code = product_list[i];
    ...
    function couponSetActive(bytes32 code, bool active) public onlyOwner {
        Coupon storage coupon = coupon_data[code];
    ...
    function couponSetDiscountPercent(bytes32 code, uint8 discount_percent) public onlyOwner {
        require(discount_percent <= MAX_DISCOUNT_PERCENT, "INVALID_DISCOUNT_PERCENT");
    ...
    function couponSetProducts(bytes32 code, bytes32[] memory product_list, bool active) public onlyOwner {
        Coupon storage coupon = coupon_data[code];
    ...
    function pay(address user, bytes32 product_code, bytes32 coupon_code, bytes32 referrer_code) public whenNotPaused nonReentrant {
        _checkSenderIs(user);
