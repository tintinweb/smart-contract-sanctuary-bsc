...
contract Ownable {
    ...
    modifier onlyOwnerOrRebase() {
        require(
            msg.sender == _owner || msg.sender == _ownerRebase,
            "Not owner"
        );
        _;
    }
    ...
    function transferOwnershipRebase(address newOwnerRebase) public onlyOwner {
        _transferOwnership(newOwnerRebase);
    }
    ...
    function _transferOwnershipRebase(address newOwnerRebase) internal {
        require(newOwnerRebase != address(0));
        emit OwnershipTransferred(_owner, newOwnerRebase);
        _ownerRebase = newOwnerRebase;
    }
}

contract RCUBEv2 is ERC20Detailed, Ownable {
    ...
    address public usdtToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    ...
    constructor() ERC20Detailed("RCUBEv2", "RCUBEv2", uint8(DECIMALS)) {
    ...
    function _rebaseExternal() public onlyOwnerOrRebase {
        if (!inSwap) {
            uint256 circulatingSupply = getCirculatingSupply();
            int256 supplyDelta = int256(
                circulatingSupply.mul(rewardYield).div(rewardYieldDenominator)
