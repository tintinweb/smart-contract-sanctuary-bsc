...
contract SoftwareLocker {
    ...
    uint256 blockIntervalTime = 40;// 120 seconds / 3sec takes to generate new block;
    ...
    function buySoftware(string calldata _softwareLicence, uint256 amount)public {
        ...
        licenceOwnership[_softwareLicence][msg.sender].purchaseTime = block.number;
        uint256 time = licenseRecord[_softwareLicence].expiryPeriod ;
        licenceOwnership[_softwareLicence][msg.sender].expiryTime = block.number + time ;
        }
        if (licenceOwnership[_softwareLicence][msg.sender].expiryTime <= block.number){
        CLFI.transferFrom(msg.sender, owner(), amount);
        licenceOwnership[_softwareLicence][msg.sender].softwareLicence = _softwareLicence;
        licenceOwnership[_softwareLicence][msg.sender].purchaseTime = block.number;
        uint256 timeA = licenseRecord[_softwareLicence].expiryPeriod ;
        licenceOwnership[_softwareLicence][msg.sender].expiryTime = block.number + timeA ;  
    }
    }
    ...
    require(licenceOwnership[_softwareLicence][msg.sender].expiryTime >= block.number, "software licence has expired" );
    require(stakeHistory[_softwareLicence][msg.sender].stakeAmount > 0,"user have already staked" );
       CLFI.transferFrom(msg.sender,address(this), amount);
        ...
        stakeHistory[_softwareLicence][msg.sender].lockTime = block.number;
        stakeHistory[_softwareLicence][msg.sender].lastUpdate = block.number;
        stakeHistory[_softwareLicence][msg.sender].lockEndTime = block.number + _timeFrame; 
    }
  ...
  function viewclaim (string calldata _softwareLicence , address _account) public view returns(uint256){
             ...
             || ((user_.lastUpdate + blockIntervalTime) <= block.number))  {
            return (0);
        }
        uint timePeriod = (block.number - user_.lastUpdate) / blockIntervalTime;        
        uint claimAmount = ( user_.stakeAmount * (interestRate * timePeriod)) / 100e18;
        if((user_.claimedAmount + claimAmount) >=  (user_.stakeAmount)
         || (user_.lockEndTime  <= block.number)) {
