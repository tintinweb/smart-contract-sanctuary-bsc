...
contract MWallet  {
    ...
    function depositTokens(uint _amount) public checkAllowance(NSI, _amount)   {
        ...
        Details[msg.sender].withdrawPaused = false;
        userDetailsHistory.push(userDetails(msg.sender,_amount,currentNSI,currentPrice,block.timestamp,true, false));
    ...
    function updateUserDetails(address user,  uint256 TokensDeposited,
        uint256 depositPrice, uint256 totalPrice, uint256 depositTime, bool hasDeposited, bool withdrawPaused
        ) external {
        ...
        userDetailsHistory.push(userDetails(user ,TokensDeposited,depositPrice,totalPrice,depositTime,hasDeposited, withdrawPaused));       
    }
    ...
    function WithdrawProfits(uint256 _amount) public  {
        ...
        balances[msg.sender] -= _amount; 
        Details[msg.sender].TokensDeposited -= _amount;
        Details[msg.sender].depositPrice = getCurrentNSIPrice();
        Details[msg.sender].totalPrice =getTokenDepositLimit() ;
        Details[msg.sender].depositTime = block.timestamp;
        userDetailsHistory.push(userDetails(msg.sender,_amount,getCurrentNSIPrice(),getTokenDepositLimit(),block.timestamp,true, false));
        emit Withdraw(msg.sender, _amount, NSI);
        }
    ...
    function getUserHistory(address user) public view returns (userDetails[] memory){
        userDetails[] memory elements = new userDetails[](0);
        uint256 counter = 0;
