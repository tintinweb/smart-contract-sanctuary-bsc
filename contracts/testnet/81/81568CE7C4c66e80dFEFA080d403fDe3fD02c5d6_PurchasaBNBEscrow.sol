...
contract PurchasaBNBEscrow is Ownable {
    ...
    mapping (address => BEPTokenList) public tokens;
    mapping (address => bool) public tokenslist;

    Escrow[] public userOrders; //create array from struct
    
    BEPTokenList[] public tokensArray;


    function addNewCategory(uint256 escrowt,string memory name,uint tcommission,uint bnbcommission) public onlyOwner {
        ...
        newCategory.bnbcommission = bnbcommission;
    }

    function EditCategory(uint256 escrowt,string memory name,uint tcommission,uint bnbcommission) public onlyOwner {
        ...
        categoryList[name].commission = tcommission;
         categoryList[name].bnbcommission = bnbcommission;
    }
    ...
    function addNewToken(address tokenAddress,string memory tokenName,string memory tokenSymbol,uint tcommission) public onlyOwner {
        ...
        BEPTokenList storage newTokenList = tokens[tokenAddress];
        newTokenList.tokenAddress = IBEP20(tokenAddress);
        ...
        BEPTokenList memory tokenData = BEPTokenList(IBEP20(tokenAddress),tokenName,tokenSymbol,tokenAddress,tcommission);
    ...
    function lockBNBToken(string memory orderId,Orders[] memory orders, address reciever,string memory currency,string memory userId) payable public {
        ...
        newOrder.items.push(Items(currentTime,currentTime + categoryList[orders[i].categoryName].escrowtime,(((orders[i].amount * orders[i].quantity) / 100) * categoryList[orders[i].categoryName].bnbcommission),(((orders[i].amount * orders[i].quantity) / 100) * categoryList[orders[i].categoryName].bnbcommission),false,orders[i].amount * orders[i].quantity,orders[i].serveId,orders[i].quantity,orders[i].categoryName,(orders[i].amount * orders[i].quantity),categoryList[orders[i].categoryName]));
    ...
    function payoutBNB(string memory currency) public {
    ...
    function lockBEPToken(string memory orderId,Orders[] memory orders, address tokenAddress,uint256 amount, address reciever,string memory currency,string memory userId) public {
    ...
    function payoutBEP(address tokenAddress,string memory currency) public {
        require(tokenslist[tokenAddress] == true, "We are not providing services for this token");
        require(keccak256(abi.encodePacked(tokens[tokenAddress].tokenSymbol)) == keccak256(abi.encodePacked(currency)), "We are not providing services for this currency");
        uint256 totalAmount = 0;
        uint256 currentTime = block.timestamp;
    ...
    function withdrawalBEPCommission(address tokenAddress,string memory currency) public onlyOwner {
        require(tokenslist[tokenAddress] == true, "We are not providing services for this token");
        require(keccak256(abi.encodePacked(tokens[tokenAddress].tokenSymbol)) == keccak256(abi.encodePacked(currency)), "We are not providing services for this currency");
        uint256 totalAmount = 0;
        uint256 currentTime = block.timestamp;
    ...
    function withdrawalBNBCommission(string memory currency) public onlyOwner {
        uint256 totalAmount = 0;
        uint256 currentTime = block.timestamp;
        for(uint i=0; i < userOrders.length; i++){
            if(keccak256(abi.encodePacked(userOrders[i].currency)) == keccak256(abi.encodePacked(currency)) && userOrders[i].isDispute == false) {
    ...
    function disputeBNBAction(string memory orderId,address refundAddress) payable public onlyOwner {
        uint256 amount = 0;
    ...
    function getBEPBalance(address tokenAddress,address merchantAddress,string memory currency) public view returns (uint256) {
        require(tokenslist[tokenAddress] == true, "We are not providing services for this token");
        require(keccak256(abi.encodePacked(tokens[tokenAddress].tokenSymbol)) == keccak256(abi.encodePacked(currency)), "We are not providing services for this currency");
        uint256 totalAmount = 0;
        for(uint i=0; i < userOrders.length; i++){
    ...
    function getBEPCommissionBalance(address tokenAddress,string memory currency) public view onlyOwner returns (uint256) {
        require(tokenslist[tokenAddress] == true, "We are not providing services for this token");
        require(keccak256(abi.encodePacked(tokens[tokenAddress].tokenSymbol)) == keccak256(abi.encodePacked(currency)), "We are not providing services for this currency");
        uint256 totalAmount = 0;
        uint256 currentTime = block.timestamp;
    ...
    function getBNBCommissionBalance(string memory currency) public view onlyOwner returns (uint256) {
        uint256 totalAmount = 0;
        uint256 currentTime = block.timestamp;
        for(uint i=0; i < userOrders.length; i++){
            if(keccak256(abi.encodePacked(userOrders[i].currency)) == keccak256(abi.encodePacked(currency)) && userOrders[i].isDispute == false) {
    ...
    function getTokens() public view returns (BEPTokenList[] memory) {
       return tokensArray;
    }
