...
library Address{
 ...
 contract ZILLA is IERC20, Context, Ownable {
    ...
    string private constant _name = "ZILLADZILLA";

    uint256 public ZILLA_max_BuyLimit = 20000000000 * 10**9;
    uint256 public ZILLA_max_SellLimit = 20000000000 * 10**9;

    address private ZILLA_Wallet = 0x55D43e3B14de193bE082AA2956Ea123343B0b7F5;
    address private ZILLA_Team_Wallet = 0x55D43e3B14de193bE082AA2956Ea123343B0b7F5;

    uint256 public ZILLA_max_WalletLimit = 30000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[ZILLA_Wallet] = true;
        _isExcludedFromFee[ZILLA_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[ZILLA_Wallet] = true;
        allowedTransfer[ZILLA_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= ZILLA_max_BuyLimit, "You are exceeding ZILLA_max_BuyLimit");
            require(balanceOf(to) + amount <= ZILLA_max_WalletLimit, "You are exceeding ZILLA_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= ZILLA_max_SellLimit, "You are exceeding ZILLA_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= ZILLA_max_WalletLimit, "You are exceeding ZILLA_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(ZILLA_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(ZILLA_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function ZILLA_Update_MarketingWallet(address newWallet) external onlyOwner{
        ZILLA_Wallet = newWallet;
    }
    
    function ZILLA_Update_TeamWallet(address newWallet) external onlyOwner{
        ZILLA_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        ZILLA_max_BuyLimit = maxBuy * 10**decimals();
        ZILLA_max_SellLimit = maxSell * 10**decimals();
    }
    
    function ZILLA_Max_Walletlimit(uint256 amount) external onlyOwner{
        ZILLA_max_WalletLimit = amount * 10**decimals();
    }
