...
contract BOXCoin is Context, IERC20, Auth {
  ...
  address public usdtreceiver = 0x5Fa039de61906DcF550b012Aeb8B9BF7Bf918603;
...
function setnumTokensSellToAddToLiquidity(uint _numTokensSellToAddToLiquidity)public onlyOwner returns(bool){
  ...
  function setusdtreceiver(address _usdtreceiver)public onlyOwner returns(bool){
  ...
  function setPair(address _pair,address _pair_usdt,address _pair_wbnb) public onlyOwner returns (bool) {
   ...
   */
    ...
    if (
(!iswhitelist) && IERC20(rewardToken).balanceOf(usdtreceiver) >=  (ispricebelow30()?150e18:70e18)
    ) {
...
if(iswhitelist){
    ...
    if(_balances[address(this)]>=numTokensSellToAddToLiquidity){
