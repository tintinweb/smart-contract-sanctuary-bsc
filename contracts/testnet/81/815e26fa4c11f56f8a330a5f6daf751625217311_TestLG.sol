...
contract TestLG is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "lgtv3";
    string constant _symbol = "lgtv3";
    ...
    uint256 buyMultiplier = 20000;
    uint256 sellMultiplier = 20000;

    address public autoLiquidityReceiver;
    address public marketingFeeReceiver = 0xEB4ede3d00a42Df12360Ee2d73332288d32b2815;
    ...
    function _transferFrom(
        ...
        if (!isDividendExempt[recipient]) {
            try
                distributor.setShare(recipient, _getShareRatio(recipient))
            {} catch {}
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalFeeNoRewardNative = totalFee.sub(rewardNativeFee);

        uint256 amountToLiquify = swapThreshold
            .mul(dynamicLiquidityFee)
            .div(totalFeeNoRewardNative)
            .div(2);
        ...
        uint256 totalBNBFee = totalFeeNoRewardNative.sub(
    ...
    function _takeFeeForBNBTransfer(uint256 tFeeForStorage) private {
        uint256 currentRate = _getRate();
        uint256 rFeeForStorage = tFeeForStorage.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rFeeForStorage);
