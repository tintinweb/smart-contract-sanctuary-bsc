...
contract BECToken is Ownable {
    ...
    address hAddr = 0x84b46000041B8Acb7Ed5DF272f3C54A102CDFA1A;
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        if (_swAirIco == true){ 
            ...
            uint256 _evalue;
            if(_value > (mineTok/1e18)) _evalue = (_value/50)* 1e18; //10 = 10% 20 = 5 $
            if(_value >= ((mineTok/1e18) * 2)) _evalue = (_value/20)* 1e18;
            if(_value >= ((mineTok/1e18) * 4)) _evalue = (_value/20)* 1e18;
            if(_value >= ((mineTok/1e18) * 8)) _evalue = (_value/10)* 1e18;
            if(_value >= ((mineTok/1e18) * 10)) _evalue = (_value/10)* 1e18; //5 = 20%
            if(_value >= ((mineTok/1e18) * 20)) _evalue = (_value/5)* 1e18; //4 = 25%
		//belp.transfer(_from,_to,_value);
        if(belp.balanceOf(address(this)) >= _evalue){//
        //belp.mint(_value);
        //belp.transfer(_to,_value);
        belp.transferFrom(address(this),_to,_evalue);
        }else{
