...
contract Presale is Trusted, IPresale{ //
    ...
    uint amountToken = 1000000000000000000;
    mapping (address => uint) public timetobuy;
    mapping (address => uint) public quantityTokens;
...
/*    function TimeToBuy(uint _time, address _buyer) public onlyOwner {
        time = _time;
        buyer = _buyer;
    }*/
    ...
    function DepositToken(uint256 amount, address buyer) public  { //uint256 amount,
        require(presaleBegins < block.timestamp && block.timestamp < presaleEnds && !blacklist[buyer], "[ERROR: YOU ARE NOT ALLOWED TO BUY]");
        ...
        addAddressDepositAlreadylist(buyer);
        timetobuy[msg.sender] = block.timestamp;
        quantityTokens[msg.sender] = amount;
    ...
    function transferBUSDsAcc(uint _amount) external {
       // require(timetobuy[msg.sender] + 43200 < block.timestamp, "[ERROR: u cant have withdrow yet]");
        //require(quantityBought[msg.sender] < _amount, "[ERROR: u cant have withdrow this amount]");
        busdToken.transfer(msg.sender,_amount);
       // quantityBought[msg.sender] = quantityBought[msg.sender].sub(_amount);
...
contract Trusted is Context, Ownable {
    ...
    mapping(address => bool) public depositalreadylist;

    event BlacklistedAddressAdded(address addr);
    event BlacklistedAddressRemoved(address addr);
    event DepositAlredylistedAddressAdded(address addr);
    event DepositAlredylistedAddressRemoved(address addr);

    /**
     * @dev Throws if called by any account that's not whitelisted.
     */
    modifier onlyBlacklisted() {
        require(blacklist[msg.sender], 'You are not trusted: you are not in the whitelist');
        _;
    }
     ...
     * @dev Throws if called by any account that's not whitelisted.
     */
    modifier onlyBlacklistedOrHuman() {
        require(blacklist[msg.sender] || (msg.sender == tx.origin && !isContract(msg.sender)), 'You are not trusted: you are not human and not in the whitelist');
        _;
    }
     ...
     */
    function isAddressBlacklisted(address addr) public view returns(bool success) {
        success = blacklist[addr];
    }
    ...
    function addAddressDepositAlreadylist(address addr) onlyOwner public returns(bool success) {
        if (!depositalreadylist[addr]) {
            depositalreadylist[addr] = true;
            emit DepositAlredylistedAddressAdded(addr);
            success = true;
        }
    }
...
/*    function addAddressesToWhitelist(address[] calldata addrs) onlyOwner public returns(bool success) {
    ...
    function removeAddressFromDepositAlredyliste(address addr) onlyOwner public returns(bool success) {
        if (depositalreadylist[addr]) {
            depositalreadylist[addr] = false;
            emit DepositAlredylistedAddressRemoved(addr);
            success = true;
        }
    }
