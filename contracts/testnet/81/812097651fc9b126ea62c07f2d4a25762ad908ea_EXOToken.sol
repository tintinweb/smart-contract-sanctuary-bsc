    ...
    AccessControlUpgradeable
{
        ...
        returns (uint256)
    {
        ...
        uint8 tier = IStakingReward(STAKING_REWARD).getTier(account);
        return
            votes == 0
                ? 0
                : ((uint256(_getVoteWeightPerEXO(tier)) / 100) * votes) / 1e18;
    }
    ...
    function _afterTokenTransfer(
        ...
        if (tier > 0) {
            ...
            if (
                balance < uint256(minimumAmount[tier]) * 1e18 &&
                stakingCount < 1
