    ...
    Withdraw
{
    ...
    function buy(string memory _ticket) external checkInTime() whenNotPaused(){
        statistics.addAmount(_ticket, 1);
        _BuyerPayUsdt(ticketPrice);
        uint tokenId = _mintToken(_msgSender(), "clubMTicket.json");
        emit Buy(_msgSender(), tokenId, block.timestamp);
    }

    function batchBuy(string memory _ticket, uint amount) external checkInTime() whenNotPaused(){
        statistics.addAmount(_ticket, amount);
    ...
    function setLimiteds(string memory item, uint amount) external onlyRole(MANAGER_ROLE){
...
library StatisticsLimited{
    ...
    function addAmount(
        ...
        require(data._items.contains(bytesName), 
