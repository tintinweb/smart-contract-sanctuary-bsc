...
interface IbridgePool {
     ...
     function withdraw(address poolAddress , uint256 amount) external;
     function pools(address poolAddress) external  returns (pool memory);
}
...
contract Farm is Ownable, ReentrancyGuard {
    ...
    constructor(IERC20 _erc20, uint256 _rewardPerBlock, uint256 _startBlock, address _feeAddress , IbridgePool _bridgePool) public {
        erc20 = _erc20;
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        IbridgePool.pool memory poolasset = bridgePool.pools(address(pool.lpToken));
         if(bridgePool.validPool(address(pool.lpToken)) && poolasset.wrappedAsset != address(0)){
          uint256 balanceBefore = IERC20(poolasset.wrappedAsset).balanceOf(address(this));
          uint256 lpPoolBalance = pool.lpToken.balanceOf(address(this));
          pool.lpToken.safeApprove(address(bridgePool) ,lpPoolBalance );
          bridgePool.deposit(address(pool.lpToken) , lpPoolBalance); 
          uint256 balanceAfter = IERC20(poolasset.wrappedAsset).balanceOf(address(this));
          require(balanceAfter.sub(balanceBefore) >= lpPoolBalance , "Invalid amount");
        }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(!(pool.lpToken.balanceOf(address(this)) >= _amount)){
          ...
          IERC20(poolasset.wrappedAsset).safeApprove(address(bridgePool) ,  _amount);
          bridgePool.withdraw(address(pool.lpToken) , _amount);  
          uint256 balanceAfter = pool.lpToken.balanceOf(address(this));
          require(balanceAfter.sub(balanceBefore) >= _amount , "Invalid amount");
        }
        }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if(!(pool.lpToken.balanceOf(address(this)) >= user.amount)){
          ...
          uint256 balanceBefore = pool.lpToken.balanceOf(address(this));
          IERC20(poolasset.wrappedAsset).safeApprove(address(bridgePool) ,  user.amount);
          bridgePool.withdraw(address(pool.lpToken) , user.amount);  
          uint256 balanceAfter = pool.lpToken.balanceOf(address(this));
          require(balanceAfter.sub(balanceBefore) >= user.amount , "Invalid amount");
        }
        }
