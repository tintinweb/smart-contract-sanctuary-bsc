...
contract TokenVesting is Ownable {
  ...
  modifier onlyBeneficiary() {
    require(msg.sender == beneficiary, "Caller is not the staking beneficiary");
    _;
  }
   ...
   */
  function release(ERC20 token) onlyBeneficiary public {
...
// File: contracts/TokenVestingPool.sol
...
contract GSPreseedRound is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;
    ...
    // Investor cap
    uint256 public investorMinCap = 100000000000000000; // 0.1 bnb
    uint256 public investorHardCap = 50000000000000000000; // 50 bnb
    // Crowdsale time
    uint256 public openingTime;
    uint256 public closingTime;
    // Total Wei Cap
    uint256 public cap;
    //MerkleProof Whitelist
    bytes32 public root;
    ...
    uint256 public constant CROWDSALE_ESCROW_CLIFF = 60 days;
    uint256 public constant CROWDSALE_ESCROW_DURATION = 120 days;
  ...
  function capReached() public view returns (bool) {
     ...
     */
    function buyTokens(bytes32[] calldata proof, address _beneficiary)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        require(
            _newPayment >= investorMinCap && _newPayment <= investorHardCap,
            "Below Minimum cap or exceed Maximum cap"
        );
        require(
    ...
    function distributeTokens() public onlyOwner nonReentrant {
        require(
            tokenDistributionComplete == false,
            "Token distribution already completed."
        );
    ...
    function getTokenSaleVestedContract(address beneficiary)
        public
        view
        returns (address)
    {
        require(
            tokenDistributionComplete == true,
            "Tokens have not been distributed yet"
        );

        address[] memory addressTokenSaleEscrow = tokenSaleEscrow
            .getDistributionContracts(beneficiary);
        return addressTokenSaleEscrow[0];
    }

    function getTokenSaleVestedFunds(address beneficiary)
        public
        view
        returns (uint256)
    {
        ...
        address addressTokenSaleEscrow = getTokenSaleVestedContract(
            beneficiary
        );
    ...
    //function to withdraw remaining token
    function withdrawTokenBalance () public onlyOwner {
