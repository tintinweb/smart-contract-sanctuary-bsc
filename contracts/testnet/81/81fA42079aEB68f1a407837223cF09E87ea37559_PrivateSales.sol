...
import "./PrivateSalesVesting.sol";
...
contract PrivateSales is Ownable {
    ...
    // 1 coin price 10**8 decimals
    uint256 public coinPriceInUSD = 10000000;

    // BNB / USD price contract address
    ...
    function setReleaseTime(uint64 releaseTime_) external onlyOwner {
        releaseTime = releaseTime_;
    }

    /// @dev setter of priceFeed contract
    /// @param priceFeedAddress_ address of price feed contract BNB/USD
    ...
    /// @dev view function to get latest prive for BNB to USD
    ...
    /// @dev setting coinPrice variable
    /// @param coinPriceInUSD_ new coin price
    function setCoinPrice(uint256 coinPriceInUSD_) external onlyOwner {
        coinPriceInUSD = coinPriceInUSD_;
    }
    ...
    function _createVestWallet(
        ...
        uint64 durationSeconds = durationDays * 1 days;

        // creating vesting wallet contract
        PrivateSalesVesting vestingWallet = new PrivateSalesVesting(beneficiaryAddress, durationSeconds, address(token));
    ...
    function buy() external payable {
        require(msg.value > 0, "Metahunter: Zero amount");
        uint256 coins = (msg.value * uint256(getLatestBNBPrice())) / coinPriceInUSD;
        _createVestWallet(msg.sender, 360 days, coins);
    }

    function withdraw() external {
        (bool success,) = payable(msg.sender).call{value : address(this).balance}("");
        require(success, "Metahunter: unsuccessful withdraw");
    }
}
...
interface IPrivateSales{
    function releaseTime() external view returns (uint64);
}
 ...
 */
contract PrivateSalesVesting is Context {
    ...
    address private immutable _salesContract;
    /**
    ...
    constructor(
        ...
        require(duration_ > 0, "VestingWallet: duration is zero");
        require(token_ != address(0), "VestingWallet: beneficiary is zero address");
        _beneficiary = beneficiary_;
        _duration = duration_;
        _token = token_;
        _salesContract = msg.sender;
    }
    ...
    function start() public view virtual returns (uint256) {
        return IPrivateSales(_salesContract).releaseTime();
    }
    ...
    function vestedAmount(uint64 timestamp) public view virtual returns (uint256) {
        require(start()>0, "VestingWallet: start is not defined");
