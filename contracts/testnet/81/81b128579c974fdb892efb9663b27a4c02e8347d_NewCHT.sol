...
contract NewCHT is ERC20, Ownable {
    ...
    address public devWallet = 0x352eB27F9F13cb1369F96CBDf1BA831728Cd3812;
    ...
    constructor(address routerAdd) ERC20("Testing", "TESTING") {
        _tokengeneration(msg.sender, 1e9 * 10**decimals());
        exemptFee[msg.sender] = true;

        IRouter _router = IRouter(routerAdd);
        // Create a pancake pair for this new token
        address _pair = IFactory(_router.factory()).createPair(address(this), busd);
    ...
    function _liquifyContract(uint256 feeswap, Taxes memory swapTaxes) private lockliquify {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            swapTokensForBUSD(toSwap);

            uint256 deltaBalance = IERC20(busd).balanceOf(address(this)) - initialBalance;
            uint256 unitBalance = deltaBalance / (denominator - swapTaxes.liquidity);
            uint256 busdToAddLiquidityWith = unitBalance * swapTaxes.liquidity;
            ...
            if (marketingAmt > 0) {
                IERC20(busd).transfer(marketingWallet, marketingAmt);
            }

            uint256 buybackAmt = unitBalance * 2 * swapTaxes.buyback;
            if (buybackAmt > 0) {
                IERC20(busd).transfer(buybackWallet, buybackAmt);
            }

            uint256 devAmt = unitBalance * 2 * swapTaxes.dev;
            if (devAmt > 0) {
                IERC20(busd).transfer(devWallet, devAmt);
            }
