...
contract TEBTEST is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private sellToken;
    ...
    string private _name = "TEBTESTFINAL";
    string private _symbol = "TEBFIN";
    ...
    //Team Wallets
    address private Wallet_Team1 = 0x2aF06333382CfC4189CF749C5b4a163fbE7F5672;
    ...
    uint256 private teamSellStart = 	1651644420; //Team tokens can be transferred after specific time has passed
    ...
    constructor() payable {
        ...
        // Team Wallets
        _isTeamWallet[Wallet_Team1] = true;
    ...
    // Maximum transfer fee canot not be set over 35%
    ...
    function setTransferFee(uint16 liq, uint16 develop, uint16 treasury, uint16 tax) external onlyOwner {
        require(liq + develop + treasury + tax <= 35, "Fees are capped at 35%"); 
    ...
    function _transfer(
        ...
        if (_isTeamWallet[from] && to != owner())
            ...
            require (block.timestamp > sellToken[from] + 2 minutes,
                        "Team Wallet can only transfer once every 2 minutes."
            );

            sellToken[from] = block.timestamp;
        ...
        if (_isEmploymentWallet[from] && to != owner()) 
            ...
            require (block.timestamp > sellToken[from] + 14 days,
                "Employment Wallet can only transfer once every 15th and 30th of the month."
            );
