 ...
 */
contract CityBox is Context, IBEP20, Ownable, ReentrancyGuard {
     ...
     * construction.
     */
    string private constant _NAME = "CityBox";
    string private constant _SYMBOL = "CBox";
    ...
    address public constant BURN_ADDRESS = 0x0b2B7e7D31F6bc1F286434AFfaD3c58e92D8aE4D; 
    ...
    address public developmentWallet = 0x0b2B7e7D31F6bc1F286434AFfaD3c58e92D8aE4D;
    uint256 private _previousDevelopmentFee = _developmentFee;


    uint256 public _marketingFee = 1;
    address public marketingWallet = 0x0b2B7e7D31F6bc1F286434AFfaD3c58e92D8aE4D;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Calculate burn amount marketing and development amount
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(developmentAmt).sub(marketingAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(developmentAmt).sub(marketingAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(developmentAmt).sub(marketingAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(developmentAmt).sub(marketingAmt)));
        }
        
        //Temporarily remove fees to transfer to burn address marketing and development wallet
        _taxFee = 0;
        _liquidityFee = 0;

        //Send transfers to burn address , marketing and development wallet
