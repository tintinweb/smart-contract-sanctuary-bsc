    ...
    IPool
{
    ...
    function setPTPV2(MARKET newP) external onlyOwner {
    ...
    function swapProc(
        ...
        uint256 baseAPR1 = _masterPlatypus.baseAPR(0); // 10**18 times (fUSDT)
        ...
        if (amount.amount3 > 0) {
            (actualToAmount.actualToAmount31, ) = _innerSwap(
                token.token3,
                token.token1,
                (amount.amount3 * baseAPR1) / sumBaseAPR,
                0,
                to
            );
    ...
    function autoStaking(AmountsParam memory lpAmounts, uint256 marketAmount)
        private
    {
        if (marketAmount > 0)
    ...
    function depositAuto(
        ...
        autoStaking(liquidities, marketAmount);
    }
...
contract MasterPlatypusV3 {    
    ...
    */
    ...
    address private _owner;
    MARKET public PTP;
    veMARKET public VePTP;

    address private fUSDT_LP = address(0x3b2E43D2e525178c2ea7c688A5707957b97dD78A);
    address private DAI_LP = address(0x134fC1Cf0a2966B0431b79EA371741042E56e023);
    address private USDC_LP = address(0x0e4866F1858C366903efca829dBD95FbAd630248);
    ...
    */
    ...
    function stakingLPFromOther (address from, uint256 pid, uint256 amount) public { 
        require(address(PTP) != address(0), "stakingLP: PTP does not set");        
    ...
    function stakingPTPFromOther (address from, uint256 amount) public { 
        require(address(VePTP) != address(0), "stakingPTP: VePTP does not set");        
        require(IERC20(PTP).balanceOf(from) >= amount, "stakingPTP: insufficient amount");
    ...
    function updatePTP(MARKET newPtp) public onlyOwner {
    ...
    function updateVePTP(veMARKET newVePtp) public onlyOwner {
