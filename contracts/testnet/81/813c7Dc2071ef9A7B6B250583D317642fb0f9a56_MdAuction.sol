...
/**
  1) function openAuction:
     - mint a new NFT
     - everybody will place bid to gain this gift
  2) function closeAuction:
     - when auction closed, transfer nft to the highest bidderdata
  3) function placeBid:
     - put an entranceFee, it will be a fee for us to pay fee for run autotask 
     - when a new bidder comes, the token (NFT token) will be transfered to the previous bidder
*/
contract MdAuction is Ownable, IERC721Receiver {
    // MdNFT intarface (so MdNFT can be called)
    enum AuctionState {
        OPENED, 
        EXPIRED,
        CLOSED
    ...
    uint256 public auctionLength; // duration of auction
    IERC20 standartToken;
    InterfaceMdNFT standardNFT;
    mapping (address => uint256) tokenBalances;
    uint256 public durationAuction = 20 * 60; // 20min
    uint256 balances;
    ...
    function openAuction(string memory _uri) public onlyOwner {
        // Condition: contractState = CLOSED
    ...
    function closeAuction() public onlyOwner {
        // Condition: contractState = EXPIRED
        ...
        if (auctions[auctionLength - 1].highestBid > 0) {
            // if anyone bid for this session, the highestBid will be greater than 0
            standardNFT.safeTransferFrom(address(this), auctions[auctionLength - 1].highestBidder, auctions[auctionLength - 1].nftId);
        } else {
            standardNFT.burn(auctions[auctionLength - 1].nftId); // no one wants the token so we burn it
        }
