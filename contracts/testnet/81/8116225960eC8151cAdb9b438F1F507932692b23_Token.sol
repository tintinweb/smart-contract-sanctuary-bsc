...
contract Token is ERC20Permit, Ownable {
    /**
     * @dev Constructor that split token between 2 address:
     * @param _treasury Main tokens reserve, behind a multisig (3 out of 5);
     * @param _distributor Address used for token distribution via user's claim;
     */
    constructor(address _treasury, address _distributor) ERC20Permit("Token") ERC20("Token", "TOK") {
        // _mint(msg.sender, 777000000 * (10**uint256(decimals())));
        _mint(_treasury, 477_000_000 * 10**decimals());
        _mint(_distributor, 300_000_000 * 10**decimals());
