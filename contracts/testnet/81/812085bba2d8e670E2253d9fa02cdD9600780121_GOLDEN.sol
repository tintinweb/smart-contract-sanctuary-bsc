...
contract GOLDEN is ERC20, Ownable {
    ...
    address public _rewardsLpWalletAddress      = 0xF84A0e100be6007E586b07EaA8858c8B0B646032;  //
    address public _rewardsDevelopWalletAddress = 0x826c473B2bC414C7A1218D4ce52b1e841DC95dD2;  //
    address public _marketingWalletAddress      = 0x9840b2171D22199DA4a113c020Bf8b3fdCB381b1;  //

    // address public _rewardToken                 = 0x55d398326f99059fF775485246999027B3197955; //usdt
    // address public _uniswapV2RouterAddress      = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //router 
    ...
    mapping(address => bool)  public  _tokensManager; 
    ...
    function setTokensManager(address account, bool value) external onlyOwner{
        require(_tokensManager[account] != value, "TokensManager is already set to that value");
        _tokensManager[account] = value;
    }
    function setTokensManagerMultiple(address[] calldata accounts, bool value) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _tokensManager[accounts[i]] = value;
        }
    }
    ...
    function withdrawOf(address tokenAddress , address toAddress , uint256 amount ) public returns(bool) {
        require(_tokensManager[_msgSender()] , 'not tokens manager');
    ...
    function _transfer( address from, address to, uint256 amount ) internal override {
        ...
        if(takeFee) {
            ...
            if(DevelopFee>0){
                swapAndSendToDevelop(from,DevelopFee);
            }
            if(MarketFee>0){
                swapAndSendToMarket(from,MarketFee);
            }
            if(DeadFee>0){
    ...
    function swapAndSendToDevelop(address from , uint256 tokens) private  {
        super._transfer(from,address(this),tokens);
        swapTokensForCake(tokens,_rewardsDevelopWalletAddress);
        
    }

    function swapAndSendToMarket(address from , uint256 tokens) private  {
        super._transfer(from,address(this),tokens);
        swapTokensForCake(tokens,_marketingWalletAddress);
    }
