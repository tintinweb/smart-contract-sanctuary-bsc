...
/* ---------------------------------- Fees ---------------------------------- */
	...
	uint256 public _collateralFee = 20;
	// Used to cache fee when removing fee temporarily.
...
abstract contract RFI is IERC20, Ownable, Tokenomics, Pancake {
	...
	function setTaxBuy(uint256 percent)external onlyOwner{
		taxBuy = percent;
	}
	//Set tax sale on percent
	function setTaxSell(uint256 percent)external onlyOwner{
		taxSell = percent;
	}
	...
	function _getTValues(
		...
		tV.tDistribution = calculateDistributionFee(amountAfterTax, 1);
	...
	function _getRValues(
		...
		uint256 fees = rV.rFee + rFee + rLiquidity + rBuyback + rDistribution + rCollateral;
...
abstract contract Expensify is Ownable, Helpers, Tokenomics, Pancake, TxPolice {
	...
	// Will keep tabs on how much BNB from the balance belongs for Tax.
	uint256 public bnbAccumulatedForTax;
	...
	*/
	...
	function sellForTax(
		...
		bnbAccumulatedForTax = bnbAccumulatedForTax.add(bnbReceived);
		// Reset the accumulator.
		accumulatedForTax = 0;
		emit SoldTokensForTax(tokenAmount, bnbReceived);
	}
