...
contract DexGo is Ownable,IERC721Receiver {
    ...
    */
    ...
    function registerApprovalForPathCompleted(uint256 shoesTokenId, uint256 pathTokenId, string memory socialPostURL) public payable nonReentrant {
        ...
        IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, IDexGoStorage(storageContract).getFixedApprovalAmount(), false, false);
    ...
    function _distributeApprovePath(
        ...
        if (rentable) {
            ...
            if (borrowerValueUSDT > 0) IERC20(IDexGoStorage(storageContract).getUSDT()).transfer(borrower, borrowerValueUSDT);
            ...
            if (shoesOwnerValueUSDT > 0) IERC20(IDexGoStorage(storageContract).getUSDT()).transfer(IERC721(IDexGoStorage(storageContract).getNftContract()).ownerOf(shoesTokenId), shoesOwnerValueUSDT);
        ...
        for(uint256 x=0;x<approvalsPathCompletedStack.length;x++) {
            if (approvalsPathCompletedStack[x].pathTokenId == pathTokenId && approvalsPathCompletedStack[x].shoesTokenId == shoesTokenId) {
              // match approval stack
                ...
                (rewardForPathCompletedResult, rewardForPathCompletedResultUSDT) = rewardForPathCompleted(shoesTokenId, pathTokenId, completedResultInPercents);
                ...
                IERC20(IDexGoStorage(storageContract).getUSDT()).transfer(IERC721(IDexGoStorage(storageContract).getNftContract()).ownerOf(pathTokenId), pathOwnerValueUSDT);
