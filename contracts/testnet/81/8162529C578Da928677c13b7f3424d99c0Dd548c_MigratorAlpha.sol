...
contract MigratorAlpha is GiftPack {
    ERC20 public oldFtContract;
    ERC20 public newFtContract;
    IGiftPack public oldNftContract;
    ...
    constructor(
        ...
        oldNftContract = IGiftPack(_oldNft);
        oldFtContractAddress = _oldFt;
        newFtContractAddress = _newFt;
    }

    function migrateAlphaData() public {
        ...
        // --- Zombie ---
        zombieCount = oldNftContract.tokenCount();

        // for (uint i = 0; i < zombieCount; i++) {
        //     zombiesKeys.push(i + 1);
        //     (
        //         uint id,
        //         string memory name,
        //         uint dna,
        ...
        //         string memory rarity,
        //         uint exp
        //     ) = oldNftContract.zombies(i);
        //     zombiesV1[zombiesKeys[i]] = ZombieV1(
        //         id,
        //         name,
        //         dna,
        //         uint16(level),
...
contract GiftPack is ZombieAttack {
    ...
    event OpenStarterPack(address indexed owner, ZombieV1[] zombies);
...
contract ZombieAttack is ZombieHelper {
    ...
    {
        ZombieV1 storage myZombie = zombiesV1[_zombieId];
        ZombieV1 storage enemyZombie = zombiesV1[_targetId];
    ...
    function _attackByTurn(
        ZombieV1 storage attackZombie,
        ZombieV1 storage defenseZombie,
        uint currentHP
    ...
    function _updateZombie(
        ZombieV1 storage winZombie,
        ZombieV1 storage lossZombie,
        uint winnerExp,
        uint loserExp
    ) internal {
...
contract ZombieHelper is ZombieFeeding {
        ...
        returns (uint)
    {
        ...
        for (uint index = 0; index < zombieCount; index += 1) {
            if (_owner == ownerOf(zombiesV1[zombiesKeys[index]].id)) {
                count += 1;
            }
        }
    ...
    function changeZombieName(uint _zombieId, string memory _name) external {
        ...
        zombiesV1[_zombieId].name = _name;
...
contract ZombieFeeding is ZombieFactory {
    ...
    function breedZombie(
        ...
        ZombieV1 storage father = zombiesV1[_fatherId];
        ZombieV1 storage mother = zombiesV1[_motherId];
...
contract ZombieFactory is ZombieBase {
    ...
    function _createZombie(
    ...
    ) internal returns (ZombieV1 memory) {
        tokenCount++;
        zombieCount++;
        Sex sex = randomSex();
        uint id = tokenCount;
        ...
        ZombieV1 memory zombieV1 = ZombieV1(
            id,
            _realName,
            _dna,
            1,
    ...
    function _createManyDnas(uint count) internal returns (ZombieV1[] memory) {
        uint i;
        Dna[] memory dnas = new Dna[](count);
        ZombieV1[] memory zombiesV1 = new ZombieV1[](count);
        for (i = 0; i < count; i += 1) {
    ...
    function openDna(uint _dnaId) public returns (ZombieV1 memory) {
        require(
        ...
        ZombieV1 memory zombieV1 = _createZombie("", dna.dna, zombieRarity);
...
contract ZombieBase is DnaBase {
    ...
    uint[] public zombiesKeys; // Contain keys of the zombiesV1 mapping (for iterating)
    mapping(uint => Zombie) public zombies;
    mapping(uint => ZombieV1) public zombiesV1;
    ...
    function getZombieOf(address _owner)
        external
        view
        returns (ZombieV1[] memory)
    {
        ...
        ZombieV1[] memory ownerZombies = new ZombieV1[](ownerZombieCount);
        uint ownerZombieIndex = 0;
        for (uint i = 0; i < zombieCount; i += 1) {
            if (ownerOf(zombiesKeys[i]) == _owner) {
                ownerZombies[ownerZombieIndex] = zombiesV1[zombiesKeys[i]];
                ownerZombieIndex += 1;
            }
        }
    ...
    function findRandomZombie(uint _zombieId) internal view returns (uint) {
        ...
        // Get all possible zombiesV1 to battle (zombie not of the current owner and not dnas)
        for (uint i = 0; i < zombieCount; i++) {
            if (_owner != ownerOf(zombiesV1[zombiesKeys[i]].id)) {
                result[counter] = zombiesV1[zombiesKeys[i]].id;
                counter++;
            }
        }
        ...
        return zombiesV1[zombiesKeys[zombieCount - 1]].id;
    }

    function internalLevelUp(uint _zombieId) internal {
        while (
            zombiesV1[_zombieId].level < LVL_MAX &&
            zombiesV1[_zombieId].exp >= EXP_UP_LEVEL[zombiesV1[_zombieId].level - 1]
        ) {
            // Add 1 level
            zombiesV1[_zombieId].level = zombiesV1[_zombieId].level.add(1);

            // Increase random stats with random amount
            uint totalGrowPoint = rarityToGrowStat[zombiesV1[_zombieId].rarity];
            zombiesV1[_zombieId].combatPower = zombiesV1[_zombieId].combatPower.add(
            ...
            while (true) {
                ...
                while (totalGrowPoint > 0) {
                    ...
                    if (
                        ...
                        zombiesV1[_zombieId].healthPoint = zombiesV1[_zombieId]
                            .healthPoint
                        ...
                        zombiesV1[_zombieId].attack = zombiesV1[_zombieId]
                            .attack
                            .add(uint16(randomStatAmount));
                    } else if (
                        keccak256(abi.encodePacked(randomStat)) ==
                        ...
                        zombiesV1[_zombieId].defense = zombiesV1[_zombieId]
                            .defense
                            .add(uint16(randomStatAmount));
                    } else if (
                        keccak256(abi.encodePacked(randomStat)) ==
                        ...
                        zombiesV1[_zombieId].criticalRate = zombiesV1[_zombieId]
                            .criticalRate
                            .add(uint16(randomStatAmount));
                    } else if (
                        keccak256(abi.encodePacked(randomStat)) ==
                        ...
                        zombiesV1[_zombieId].criticalDamage = zombiesV1[_zombieId]
                            .criticalDamage
                            .add(uint16(randomStatAmount));
                    } else if (
                        keccak256(abi.encodePacked(randomStat)) ==
                        ...
                        zombiesV1[_zombieId].speed = zombiesV1[_zombieId].speed.add(
