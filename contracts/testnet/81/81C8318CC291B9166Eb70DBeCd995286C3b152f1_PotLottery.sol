...
contract PotLottery is Initializable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _owner) public initializer {
        ...
        busdAddr = 0x6DE71f139556bf8F5cBe39232053EAfE9fCBcF50;
    ...
    */
    function UpdatePrice() public nonReentrant {
        if (participantCount == 0) {
            for (uint256 index = 0; index < tokenWhiteListNames.length; index++) {
                Token memory potTokenInfo = tokenWhiteList[tokenWhiteListNames[index]];
                (uint256 Res0, uint256 Res1) = _getTokenReserves(potTokenInfo.tokenAddress, busdAddr);
                IPRC20 busdToken = IPRC20(busdAddr);
                uint256 res1 = Res1 * (10**potTokenInfo.tokenDecimal);
                uint256 price = res1 / Res0;

                updateTokenUsdValue(tokenWhiteListNames[index], (price * 10**10) / 10**busdToken.decimals());
            }
        }
    }
    ...
    */
    ...
    function swapAccumulatedFees() external validBNBP nonReentrant {
        ...
        _distributeToTokenomicsPools(bnbSwapAmounts[1] + busdSwapAmounts[1]);
    }
