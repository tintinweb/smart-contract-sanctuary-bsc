...
contract Lottery is ILottery, Ownable, ReentrancyGuard{
    ...
    constructor(address _rollerTokenAddr) public {
    ...
    function injectFirstPrize(address[] calldata dst, uint256 amount) public nonReentrant notContract {
        rollerToken.safeTransferFrom(address(msg.sender), address(this), amount);
    ...
    function claimLottery() external nonReentrant notContract{
        ...
        if(amount > 0) {
            ...
            rollerToken.safeTransfer(msg.sender, amount);
    ...
    function getRollerLevel(address user) external override view returns (uint256){
        return lotteryInfo[user].pendingAmount;
    }

    // set the roller level.
    function setRollerLevel(address _RollerLevel) external onlyOwner {
        require(_RollerLevel != address(0), "Zero");
        RollerLevel = IRollerLevel(_RollerLevel);
        emit SetRollerLevel(block.number, _RollerLevel);
    }
}
