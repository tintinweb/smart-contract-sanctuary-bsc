...
contract MinterToken is Context,Ownable,IERC20 {
    ...
    */
    ...
    function Minter(uint256 amountUsdt,uint256 day) public returns(bool) {
        ...
        uint unlockedTime = block.timestamp + 3600*24*0;
        uint256 interest = reward(amountUsdt,day);
        interest = interest.mul(day);
        ...
        emit Transfer(address(this),sender,interest);
        return true;
    }
    ...
    function reward(uint usdtamount,uint day) private  view returns (uint){
        uint256 rate = getlv(day);
        return usdtamount.mul(2).mul(rate).div(100).div(365);
    ...
    function withdaw(uint id) public nonReentrant {
        ...
        tokenAmount = interest.add(tokenAmount);
        IERC20(ToToken).transferFrom(address(this),sender,tokenAmount);
        ...
        persons[sender].pop();
        emit Transfer(sender,address(this),interest);
    }

    function within(uint id) private  {
        address sender = _msgSender();
        uint index = getIndexForId(id);
        uint unlockedTime = persons[sender][index].unlockedTime;
        require(block.timestamp >=unlockedTime,'loclked');
    ...
    function withdawAll() public nonReentrant {
        address sender = _msgSender();
        uint leng = persons[sender].length;
        for(uint i=0;i<leng;i++){
            if(persons[sender][i].unlockedTime<block.timestamp){
                within(persons[sender][i].id);
            }
        }
    }
