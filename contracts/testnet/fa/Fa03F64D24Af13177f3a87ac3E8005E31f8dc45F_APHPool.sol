...
contract APHPool is PoolBase, PoolSetting, PoolEvent, PoolToken {
    ...
    function activateRank(uint256 nftId) external nonReentrant returns (uint8 newRank) {
        ...
        newRank = _activateRank(_NFTOwner(nftId), nftId);
    }
        ...
        )
    {
        ...
        (mintedP, mintedItp, mintedIfp) = _deposit(_NFTOwner(nftId), nftId, depositAmount);
    }
    ...
    {
        nftId = Membership(membershipAddress).usableTokenId(nftId);
        WithdrawResult memory result = _withdraw(_NFTOwner(nftId), nftId, withdrawAmount);
    ...
    {
        nftId = Membership(membershipAddress).usableTokenId(nftId);
        return _claimAllInterest(_NFTOwner(nftId), nftId);
    }
    ...
    {
        nftId = Membership(membershipAddress).usableTokenId(nftId);
        WithdrawResult memory result = _claimTokenInterest(_NFTOwner(nftId), nftId, claimAmount);
        ...
        returns (WithdrawResult memory)
    {
        nftId = Membership(membershipAddress).usableTokenId(nftId);
        //:TODO transfer forw
        return _claimForwInterest(_NFTOwner(nftId), nftId, claimAmount);
    }
