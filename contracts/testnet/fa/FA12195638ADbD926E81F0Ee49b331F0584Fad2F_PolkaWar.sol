...
contract PolkaWar is Ownable, ReentrancyGuard {
    ...
    // get game players
    function getGamePlayers(uint256 pid) public view returns (address[] memory) {
        return pools[pid - 1].players;        
    }
    ...
    function draw(uint256 _pid) external onlyOwner {
        ...
        polkaWarToken.transfer(getGamePlayers(poolIndex)[0], refund);
        polkaWarToken.transfer(getGamePlayers(poolIndex)[1], refund);
    ...
    function claimAward(uint256 _pid) external nonReentrant {
        ...
        // check game status
        require(pools[poolIndex].state == GameState.Finished, "no valid time");
        require(getGamePlayers(poolIndex)[0] == msg.sender || getGamePlayers(poolIndex)[1] == msg.sender, "player not found");
