...
contract Pray is ERC2981, ERC721A, Ownable {
    ...
    string public notRevealedURI= "https://praynft.s3.amazonaws.com/pray_metadata.json";
    ...
    //LAUNCHPAD
    uint256 public LAUNCH_MAX_SUPPLY = 9500;        // max launch supply
    ...
    function mint(uint256 amount) external payable {
        ...
        uint256 requirePay = publicSalePrice * amount;
    ...
    /************************************* onlyOwner ********************/
    function devmint(address to, uint32 amount) public onlyOwner {
        require(to != address(0), "address invalid");
        require(amount + totalSupply() <= maxSupplyAmount, "REACHED MAX SUPPLY AMOUNT");
        _safeMint(to, amount);
    }

    function setMaxLaunchpadSupply(uint256 _new) external onlyOwner {
        LAUNCH_MAX_SUPPLY = _new;
    }
    ...
    function multidevmint(address [] calldata tos, uint32 [] calldata amounts) external onlyOwner {
        ...
        for(uint l = 0; l < tos.length; l ++) {
            require(tos[l] != address(0), "address invalid");
    ...
    function withdraw(address treasury) external onlyOwner {
        require(treasury != address(0), "treasury invalid");
        uint256 balance = address(this).balance;

        (bool success1, ) = payable(treasury).call{value: balance}("");
    ...
    function burnMany(uint256 [] memory tokenIds) external onlyOwner {
