...
contract ContributorVesting is ReentrancyGuard, Ownable {
    ...
    function schedule(address _contributor, uint _period) public view returns (
        ...
        amount = _amountByPeriod(_period, _contributor);
    ...
    function _periodToDate(address _contributor, uint _period) private view returns (uint date) {
        if (_period == 1) {
            if (customTimeStart[_contributor] != 0) {
                customTimeStart[_contributor];
            } else {
                date = settings.tgeTimestamp;
            }
        } else {
            if (customTimeStart[_contributor] != 0) {
                date = customTimeStart[_contributor] + settings.cliffInSeconds + (settings.periodDuration * (_period - 2));
            } else {
    ...
    function _amountByPeriod(uint _period, address _contributor) internal view returns (uint tokenAmount) {
        if (_period == 1) {
            tokenAmount = (contributorsAllocation[_contributor] / 100) * settings.tokenPercentUnlockOnTge;
        } else {
            tokenAmount = (contributorsAllocation[_contributor] - ((contributorsAllocation[_contributor] / 100) * settings.tokenPercentUnlockOnTge)) / (settings.numberOfEmmissions - 1);
        }
    }
}
