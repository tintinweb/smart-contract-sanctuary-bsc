...
contract BunBunXX is Context, IERC20, Ownable {
    ...
    uint256 public _maxWalletToken = _tTotal.div(100); // 0.25%
    ...
    string private constant _name = "BunBunXX2";
    string private constant _symbol = "BUXX";
    uint8 private constant _decimals = 9;

    address payable private _developmentAddress =
        payable(0x7373f0c3AAd71C224776c9e14d3dae4442272413);
    address payable private _marketingAddress =
        payable(0xcc78D278058CC4340133cfA9e3090f63dd083cD9);
    ...
    function setIsTxLimitExempt(address wallet, bool exempt) external onlyDev {
        isTxLimitExempt[wallet] = exempt;
    }

    function setIsWalletLimitExempt(address wallet, bool exempt) external onlyDev {
        isWalletLimitExempt[wallet] = exempt;
    }    
    ...
    function setTxLimit(uint256 amount) public onlyDev {
        _maxTxAmount = amount;
    }

    function checkMaxWalletToken(address addr) internal view {
        require(balanceOf(addr)<= _maxWalletToken || isWalletLimitExempt[addr], "maxWallet Limit Exceeded");
    }

    function setMaxWallet(uint256 maxWall) public onlyDev {
        _maxWalletToken = maxWall;
    }
}