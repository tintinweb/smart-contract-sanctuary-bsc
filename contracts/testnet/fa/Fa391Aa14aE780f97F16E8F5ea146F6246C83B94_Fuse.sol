...
contract Fuse is OwnableUpgradeable, ERC721Holder {
    ...
    uint256 constant BASIS = 10**4;
    uint256 constant BONUS_STAT = 4000;// 40% basis points;
    uint256 constant GROWTH_CAP = 30000;
    uint256 constant MAGIC_NUMBER = 1 * SCALE;

    Seedmon public seedmon;
    event SeedmonFused(uint256 vitality, uint256 strength, uint256 smart, uint256 speed, uint256 dexterity, uint256 luck);
    event SeedmonCloned(bytes32 name);
    ...
    function cloneSeedmon(uint256 tokenId, address to) external {
        (bytes32 name,SeedmonAttribute.UniqueProfile memory uniqueProfile, SeedmonAttribute.Stat memory stat) = seedmon.tokenInfo(tokenId);
        
        seedmon.mint(to, name, uniqueProfile, stat);
        seedmon.mint(to, name, uniqueProfile, stat);
        emit SeedmonCloned(name);
    }
    function fuse(uint256 mainTokenId, uint256 materialTokenId, address to) external {  
        
        SeedmonAttribute.Stat memory pendingStat;
        (bytes32 mainName,SeedmonAttribute.UniqueProfile memory mainUniqueProfile, SeedmonAttribute.Stat memory mainStat) = seedmon.tokenInfo(mainTokenId);
        { //avoding stack too deep
            (bytes32 materialName,SeedmonAttribute.UniqueProfile memory materialUniqueProfile, SeedmonAttribute.Stat memory materialStat) = seedmon.tokenInfo(materialTokenId);
            
            require(mainName == materialName, "Fuse: Material Mon must be the same type with Main Mon");
            ...
            require(mainTokenId != materialTokenId, "Fuse: Invalid Material Mon");

            
            pendingStat.vitality = materialStat.vitality.mul(BONUS_STAT);
            pendingStat.strength = materialStat.strength.mul(BONUS_STAT);
            pendingStat.smart = materialStat.smart.mul(BONUS_STAT);
            pendingStat.speed = materialStat.speed.mul(BONUS_STAT);
            pendingStat.dexterity = materialStat.dexterity.mul(BONUS_STAT);
            pendingStat.luck = materialStat.luck.mul(BONUS_STAT);
        }
        uint256 star = mainUniqueProfile.numberOfFused.mul(SCALE);
        if (star == 0) {
            star = 1 * SCALE; 
        }
        ...
        sumOfPendingBonusStat = sumOfPendingBonusStat.div(BASIS);
        uint256 factor = GROWTH_CAP.mul(star.div(4).add(MAGIC_NUMBER)).div(sumOfPendingBonusStat);
        SeedmonAttribute.Stat memory finalStat;
        
        //If (Over Stat Penalty < 1) -> Each Pending Stat * Over Stat Penalty
        if (factor / SCALE < 1) {
            finalStat.vitality = mainStat.vitality.add((pendingStat.vitality.div(BASIS).mul(factor.div(SCALE))));
            finalStat.strength = mainStat.strength.add((pendingStat.strength.div(BASIS).mul(factor.div(SCALE))));
            finalStat.smart = mainStat.smart.add((pendingStat.smart.div(BASIS).mul(factor.div(SCALE))));
            finalStat.speed = mainStat.speed.add((pendingStat.speed.div(BASIS).mul(factor.div(SCALE))));
            finalStat.dexterity = mainStat.dexterity.add((pendingStat.dexterity.div(BASIS).mul(factor.div(SCALE))));
            finalStat.luck = mainStat.luck.add((pendingStat.luck.div(BASIS).mul(factor.div(SCALE))));
        } else {
            finalStat.vitality = mainStat.vitality.add(pendingStat.vitality.div(BASIS));
            finalStat.strength = mainStat.strength.add(pendingStat.strength.div(BASIS));
            finalStat.smart = mainStat.smart.add(pendingStat.smart.div(BASIS));
            finalStat.speed = mainStat.speed.add(pendingStat.speed.div(BASIS));
            finalStat.dexterity = mainStat.dexterity.add(pendingStat.dexterity.div(BASIS));
            finalStat.luck = mainStat.luck.add(pendingStat.luck.div(BASIS));
        }

        mainUniqueProfile.numberOfFused = star.div(SCALE) + 1;
        
        seedmon.safeTransferFrom(msg.sender, address(this), mainTokenId);
        seedmon.safeTransferFrom(msg.sender, address(this), materialTokenId);
        seedmon.burn(mainTokenId);
        seedmon.burn(materialTokenId);
        seedmon.mint(to, mainName, mainUniqueProfile, finalStat);
        
        emit SeedmonFused(finalStat.vitality, finalStat.strength, finalStat.smart, finalStat.speed, finalStat.dexterity, finalStat.luck);
    }
