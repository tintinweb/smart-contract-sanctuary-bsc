...
contract EtermonNFTStaking is WhitelistedRole, Pausable, EnableTokensClaimable, LockStaking, ERC721Holder {

    event StakingBegin(address admin, uint256 stakingFunds);
    event Staked(address indexed from, address indexed to, uint64 indexed stakeIndex, uint256 tokenId);
    event Unstaked(address indexed from, address indexed withdrawer, uint256 tokenId, uint256 reward, uint64 indexed stakeIndex);
    event UnstakeAll(address indexed from, address indexed withdrawer, uint256 reward, uint8 totalStake);
    ...
    mapping(uint256 => uint8) public tokenDepositIndex;
    ...
    constructor (
        ...
        uint256 poolLimit_,
        uint8 nftType_,
        uint256 rewardsPerTokenPoints_,
        ...
        rewardsPerTokenPoints = rewardsPerTokenPoints_;
    ...
    function startStaking() public whenNotPaused whenStakingLocked onlyWhitelistAdmin {
        require(IERC20(etmToken).balanceOf(address(this)) >= rewardPool, "EtermonNFTStaking: not enough staking rewards");
        stakingFunds = rewardPool;
        _unlockstaking();
        emit StakingBegin(_msgSender(), stakingFunds);
    }

    function stake(uint256[] memory depositTokenIds) public whenNotPaused whenNotStakingLocked {
        require(_userDepositTotal[_msgSender()] + depositTokenIds.length >= minQuantity, "EtermonNFTStaking: The minimum of nft amount staking capacity not met");
        require(_userDepositTotal[_msgSender()] + depositTokenIds.length <= maxQuantity, "EtermonNFTStaking: The maximum of nft amount staking capacity exceeded");
        require(totalDeposited + depositTokenIds.length <= poolLimit, "EtermonNFTStaking: Contract staking capacity exceeded");

        _userDepositTotal[_msgSender()] += depositTokenIds.length;
        totalDeposited += depositTokenIds.length;

        for (uint8 i = 0; i < depositTokenIds.length; i++) {
            _stake(_msgSender(), depositTokenIds[i]);
        }
    }
    ...
    function _stake(
        ...
        private
    {
        (,bytes memory result) = nft.call{value:0}(abi.encodeWithSignature("getBoxTypeBaseTokenId(uint256)",_tokenId));
        uint8 type_ = abi.decode(result,(uint8));
        require(type_ == nftType, "EtermonNFTStaking: Type of your NFT is invalid in this staking pool");
        _userDeposits[_msgSender()].push(userDeposit({
            tokenId: _tokenId,
            depositTime: block.timestamp
        }));
        tokenOwner[_tokenId] = _user;
        tokenDepositIndex[_tokenId] = uint8(_userDeposits[_msgSender()].length - 1);
        IERC721(nft).safeTransferFrom(
            _user,
            address(this),
            _tokenId
        );
        emit Staked(_msgSender(), address(this), tokenDepositIndex[_tokenId], _tokenId);
    }

    function unStakeAll() public whenNotPaused whenTokensClaimableEnabled {
        require(_userDepositTotal[_msgSender()] > 0, "EtermonNFTStaking: Nothing to withdraw");
        uint256 totalReward = getAllReward(_msgSender(), block.timestamp);
        uint8 totalStake = uint8(_userDepositTotal[_msgSender()]);
        _userDepositTotal[_msgSender()] = 0;
        stakingFunds -= totalReward;
        for (uint64 i = 0; i < uint64(_userDeposits[_msgSender()].length); i++) {
            if (_userDeposits[_msgSender()][i].depositTime > 0) {
                delete _userDeposits[_msgSender()][i];
                require(tokenOwner[_userDeposits[_msgSender()][i].tokenId] == _msgSender(), "EtermonNFTStaking: You are not staker");
                _unstake(_msgSender(), _userDeposits[_msgSender()][i].tokenId);
            }
        }
        SafeERC20.safeTransfer(IERC20(etmToken), _msgSender(), totalReward);
        emit UnstakeAll(address(this), _msgSender(), totalReward, totalStake);
    }

    function unStakeBatch(uint256[] memory tokenIds) public whenNotPaused whenTokensClaimableEnabled {
        for (uint8 i = 0; i < tokenIds.length; i++) {
            uint8 index = tokenDepositIndex[tokenIds[i]];
            uint256 depositTime =  _userDeposits[_msgSender()][index].depositTime;
            require(depositTime > 0, "EtermonNFTStaking: Nothing to withdraw at this index");
            require(tokenOwner[tokenIds[i]] == _msgSender(), "EtermonNFTStaking: You are not staker");
            uint256 reward = getReward(_msgSender(), block.timestamp, index);
            _userDepositTotal[_msgSender()] -= 1;
            stakingFunds -= reward;
            ...
            emit Unstaked(address(this), _msgSender(), tokenIds[i] , reward, index);
        }
    }

    function unStake(uint256 tokenId) public whenNotPaused whenTokensClaimableEnabled {
        uint8 index = tokenDepositIndex[tokenId];
        uint256 depositTime =  _userDeposits[_msgSender()][index].depositTime;
        require(depositTime > 0, "EtermonNFTStaking: Nothing to withdraw at this index");
        require(tokenOwner[tokenId] == _msgSender(), "EtermonNFTStaking: You are not staker");
        uint256 reward = getReward(_msgSender(), block.timestamp, index);
        _userDepositTotal[_msgSender()] -= 1;
        stakingFunds -= reward;
        ...
        delete _userDeposits[_msgSender()][index];
        SafeERC20.safeTransfer(IERC20(etmToken), _msgSender(), reward);
        emit Unstaked(address(this), _msgSender(), tokenId, reward, index);
    }

    function unStakeBatchByIndex(uint64[] memory indexes) public whenNotPaused whenTokensClaimableEnabled {
        for (uint8 i = 0; i < indexes.length; i++) {
            uint256 depositTime =  _userDeposits[_msgSender()][indexes[i]].depositTime;
            uint256 tokenId = _userDeposits[_msgSender()][indexes[i]].tokenId;
            require(depositTime > 0, "EtermonNFTStaking: Nothing to withdraw at this index");
            require(tokenOwner[tokenId] == _msgSender(), "EtermonNFTStaking: You are not staker");
            ...
            SafeERC20.safeTransfer(IERC20(etmToken), _msgSender(), reward);
            emit Unstaked(address(this), _msgSender(), tokenId , reward, indexes[i]);
        }
    }

    function unStakeByIndex(uint64 index) public whenNotPaused whenTokensClaimableEnabled {
        uint256 depositTime =  _userDeposits[_msgSender()][index].depositTime;
        uint256 tokenId = _userDeposits[_msgSender()][index].tokenId;
        require(depositTime > 0, "EtermonNFTStaking: Nothing to withdraw at this index");
        require(tokenOwner[tokenId] == _msgSender(), "EtermonNFTStaking: You are not staker");
        uint256 reward = getReward(_msgSender(), block.timestamp, index);
        _userDepositTotal[_msgSender()] -= 1;
    ...
    function _unstake(
        ...
        private 
    {
        delete tokenDepositIndex[_tokenId];
    ...
    function addReward() public whenNotPaused whenNotStakingLocked onlyWhitelistAdmin {
        uint256 rewardAdded = IERC20(etmToken).balanceOf(address(this)) - stakingFunds;
        require(rewardAdded > 0, "EtermonNFTStaking: no rewards added");
    ...
    function revokeReward(uint256 amount) public whenNotPaused whenNotStakingLocked onlyWhitelistAdmin {
        require(IERC20(etmToken).balanceOf(address(this)) >= amount, "EtermonNFTStaking: Amount too high");
        rewardPool -= amount;
        stakingFunds -= amount;
    ...
    function getAllReward(address account, uint256 timestamp) public view returns(uint256) {
        uint256 totalReward;
        for (uint64 i = 0; i < uint64(_userDeposits[_msgSender()].length); i++) {
            if (_userDeposits[_msgSender()][i].depositTime > 0) totalReward += getReward(account, timestamp, i);
        }
        return totalReward;
    }

    function getRewardBatch(address account, uint256 timestamp, uint64[] memory indexes) public view returns(uint256) {
        uint256 totalReward;
        for (uint8 i = 0; i < uint8(indexes.length); i++) {
            totalReward += getReward(account, timestamp, indexes[i]);
        }
        return totalReward;
    }

    function getReward(address account, uint256 timestamp, uint64 index) public view returns(uint256) {
        (, uint256 userDepositTime) = getUserDepositAtIndex(account, index);
        uint256 period = timestamp - userDepositTime;
        if (period >= 60*DAY_IN_SECONDS) {
            return rewardsPerTokenPoints*pointMultiplier*APR_DAY60_MULTIPLIER*60*DAY_IN_SECONDS/(YEAR_IN_SECONDS*100);
        } else if (period >= 30*DAY_IN_SECONDS && period < 60*DAY_IN_SECONDS) {
            return rewardsPerTokenPoints*pointMultiplier*APR_DAY30_MULTIPLIER*30*DAY_IN_SECONDS/(YEAR_IN_SECONDS*100);
        } else if (period >= 15*DAY_IN_SECONDS && period < 30*DAY_IN_SECONDS) {
            return rewardsPerTokenPoints*pointMultiplier*APR_DAY15_MULTIPLIER*15*DAY_IN_SECONDS/(YEAR_IN_SECONDS*100);
        } else return 0;
    }
    ...
    function getUserDepositAtIndex(address account, uint64 index) public view returns(uint256 tokenId, uint256 depositTime) {
        return (_userDeposits[account][index].tokenId, _userDeposits[account][index].depositTime);
    }
}