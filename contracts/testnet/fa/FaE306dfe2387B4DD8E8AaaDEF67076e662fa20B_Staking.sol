...
/**
    ...
    function userClaimable(address stakeOwner, uint256 stakeId) public view returns(uint256) {
        ...
        uint256 stakedDays;
        if(campaigns[latestCampaignId].endTime == 0 || campaigns[latestCampaignId].endTime < block.timestamp)
        stakedDays = 0;
        else
        (, stakedDays) = _getUserStakedTime(usrStake);
        // uint256 alreadyClaimed = xticket.claimTime == 0 ? 0 : block.timestamp.sub( xticket.claimTime);
        // alreadyClaimed = alreadyClaimed.div(10 minutes);
        // stakedDays = stakedDays.sub(alreadyClaimed);
        ...
        uint256 newTickets = claimableTickets.sub(xticket.claimed).sub(xticket.claimable);
        return newTickets;
    }
    ...
    function claimReward() external {
        require(userAwardedCollections[msg.sender].length > 0, "no rewards");
     ...
     * @return totalUserXTickets
     */
    function getClaimableTickets() external view returns(uint256) {
        return totalUserXTickets[msg.sender];
    }
    ...
    function rewardLootBox() external onlyOwner campaignEnded {
        ...
        //uint256 _randomSeed = 982173982173192873;
    ...
    /**
     * @dev ADMIN METHOD: Add/Update VRFConsumer
     * @param _consumerAddress Address of the vrfConsumer contract
     */
    ...
    function _getUserStakedTime(Stake memory usrStake) private view returns (uint256 unixStakedTime, uint256 stakedDays) {
        ...
        if(campaigns[latestCampaignId].startTime > usrStake.stakedAt)
        {
            stakedTime = campaigns[latestCampaignId].startTime;
        }
        if(block.timestamp > campaigns[latestCampaignId].endTime &&  campaigns[latestCampaignId].endTime != 0){
            nowTime  = campaigns[latestCampaignId].endTime;
        }
        unixStakedTime = nowTime.sub(stakedTime);
        stakedDays = unixStakedTime.div(5 minutes);
    ...
    function _getClaimableXTickets(address _stakeOwner, uint256 _stakeId) private returns(uint256) {
        ...
        for (uint256 i; i < newTickets; i++) {
            ...
            ticketIds[ticketId] = _stakeOwner;
        }

        xticket.claimable = claimableTickets.sub(xticket.claimed);
        if(xticket.claimTime == 0){
            xticket.claimTime = usrStake.stakedAt;
        }
        xticket.claimTime = xticket.claimTime.add(stakedDays.mul(60));
    ...
    function _rewardWinner(address _winnerAddress) private {
        address[] storage awardCollections = userAwardedCollections[_winnerAddress];

        for (uint256 j; j < awardCollections.length; j++) {
            ...
            for (uint256 k = nftsWon.length; k > 0; k--) {
                ...
                emit Reward(_winnerAddress, nftId);
            }

            // remove collection address from userAwardedCollections if all nfts claimed from it 
            if (nftsWon.length == 0) {
                if (awardCollections.length > 1) {
                    awardCollections[j] = awardCollections[awardCollections.length.sub(1)];
                    awardCollections.pop();
                } else {
                    awardCollections.pop();
                }
    ...
    function _pickWinners(uint256 _randomSeed) private {
        ...
        uint256[] memory winningTicketIds = _getRandomNumbers(_randomSeed, N_REWARD_WINNER, totalClaimableTickets);

        // Loop over all winning tickets and add reward for winner
        for (uint256 z; z < winningTicketIds.length; z++) {
            uint256 winningTicketId = winningTicketIds[z];
            address winnerAddress = ticketIds[winningTicketId];
...
abstract contract GlobalsAndUtils {
    ...
    modifier onlyVRFConsumer {
        require(msg.sender == VRFV2CONSUMER_ADDRESS, 'Only allowed to be called by VRFConsumer contract');
        _;
    }
    ...
    uint256 internal constant CLAIM_X_TICKET_DURATION = 10;
    uint256 internal constant CLAIM_X_TICKET_UNIT= 1 minutes;
    ...
    mapping(uint256 => address) internal ticketIds;
    ...
    /// @notice Chainlink's VRFConsumer for getting random seeds
