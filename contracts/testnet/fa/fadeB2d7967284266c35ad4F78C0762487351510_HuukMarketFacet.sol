...
contract HuukMarketFacet is IHuukMarketFacet {
        ...
        returns (bool)
    {
        ...
        if (_receiver != address(0)) {
            ...
            LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
            LibStruct.Order memory order = ds.orders[orderId];
            if (ds.totalBuyQuantityInUSDT[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] >= _amount) {
                ds.totalBuyQuantityInUSDT[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] -= _amount;
            }
...
//      require(msg.sender == _order.representative, "representative");
    ...
    function _match(
        ...
        if (ds.isHuukNFTs[order.tokenAddress]) {
            ...
            if (ds.totalBuyQuantityInUSDT[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] >= _quantity) {
                ds.totalBuyQuantityInUSDT[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] -= _quantity;
            }
            if (order.owner == creator) {
                ...
                uint256 amountToCreator = IProfitEstimator(ds.profitSender).profitToCreator(
                    ...
                    _quantity,
                    _price,
                    ds.averageBuyCashInUSDT[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))]
                );
    ...
    function _updateOrderAfterMatch(
        ...
        ds.averageBuyCashInUSDT[_buyer][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] =
            (ds.averageBuyCashInUSDT[_buyer][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))]
            * ds.totalBuyQuantityInUSDT[_buyer][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))]
            + estimateUSDT(
                _paymentToken,
                _price
            ) * _quantity) * ZOOM_FEE /
            (ds.totalBuyQuantityInUSDT[_buyer][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))]+ _quantity) / ZOOM_FEE
        ;
        ds.totalBuyQuantityInUSDT[_buyer][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] += _quantity;
        return true;
    }
    ...
    function amountToCreator(uint256 _orderId, address _paymentToken, uint256 _quantity, uint256 _price)
    external
    override
    returns (uint256 amountCreator)
    {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        LibStruct.Order memory order = ds.orders[_orderId];
        amountCreator = IProfitEstimator(ds.profitSender).profitToCreator(
            order.tokenAddress,
            _paymentToken,
            order.tokenId,
            _quantity,
...
//        return ds.averageBuyCashInUSDT[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))];
    }
...
library LibError {
    ...
    function revert(bytes memory errorData) public pure {
        assembly {
...
library LibDiamond {
    ...
    struct DiamondStorage {
        ...
        uint256 totalOrders;
        uint256 totalBids;
        mapping(uint256 => bool) executedAcceptBid;
        mapping(uint256 => bool) executedOrder;

        mapping(uint256 => LibStruct.Order) orders;
        mapping(bytes32 => uint256) orderID;
        mapping(uint256 => LibStruct.Bid) bids;
        ...
        uint256 xCreator;
        uint256 yRefRate; // 50%
        uint256 zProfitToCreator; // 10% profit
        uint256 discountForBuyer;
        uint256 discountForHuuk; // discount for user who made payment in huuk
        mapping(address => bool) paymentMethod;
        mapping(address => bool) isHuukNFTs;
        mapping(address => bool) isOperator;
        mapping(address => bool) isHuukNFTPremiums;
        mapping(address => mapping(bytes32 => uint256)) averageBuyCashInUSDT;
        mapping(address => mapping(bytes32 => uint256)) totalBuyQuantityInUSDT;
    }
    ...
    function whenPaused() internal view {
        require(LibDiamond.paused(), "Pausable: not paused");
    }
    ...
    function enforceIsOperator() internal view {
        require(diamondStorage().isOperator[msg.sender], "operator");
    }
    ...
    function _EIP712_init(string memory name, string memory version) internal {
        ...
        bytes32 hashedVersion = keccak256(bytes(version));
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        ds._TYPE_HASH = keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)");
    ...
    function _setSystemFee(
        ...
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        ds.xUser = _xUser;
        ds.xCreator = _xCreator;
        ds.yRefRate = _yRefRate;
        ds.zProfitToCreator = _zProfitToCreator;
        ds.discountForBuyer = _discountForBuyer;
        ds.discountForHuuk = _discountForHuuk;
        ds.xPremium = _xPremium;
    }
