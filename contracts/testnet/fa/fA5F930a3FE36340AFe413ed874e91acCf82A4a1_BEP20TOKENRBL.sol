...
contract BEP20TOKENRBL is Context, IBEP20, Ownable {
    ...
    constructor() {
        ...
        _addresses[4]=0xadA37191D9396a28588B711652377dD9d0b25A8F;//4加池子以及预售
        ...
        //                            环保大使
        //                              NFT1%
        _percents=[2,5,5,3,85,2,2,2,5,4,1];
        //分配代币
    ...
    //修改swap开启时间
    function changSwapTime(uint256 time) external onlyOwner{
        _swapOpenTime=time;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(_swapNowStatus==true){
            ...
            if(_pairAddress[sender]==true&&_whites[recipient]==false){
                require(_swapOpenTime<=block.timestamp,"The current time is not open");
                //买
                ...
                uint[] memory amounts2=uniswapV2Router.getAmountsOut(amount.mul(_percents[9]).div(100),path);
                //先给合约本身再执行兑换
                ...
                //兑换营销
                uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount.mul(_percents[9]).div(100),amounts2[1].mul(100-_feeMax).div(100),path,_addresses[9],block.timestamp);
