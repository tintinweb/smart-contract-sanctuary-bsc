...
contract OpenBiSea is Ownable  {
    ...
    uint256 public initialPriceInt = 88800000000000000;
    ...
    constructor (uint256 _initialBalance,address _usdContract,address _tokenOBS,uint256 _networkId) {
        tokenOBS = _tokenOBS;
        initialBalance = _initialBalance;
        usdContract = _usdContract;
        if (_networkId == 1) initialPriceInt = 14145132743363000; // ETH mainnet
        if (_networkId == 42) initialPriceInt = 14145132743363000;// ETH kovan
        if (_networkId == 56) {
            tokensaleTotalSold = 429212543849625700000;
            initialPriceInt = 88800000000000000;// BCS mainnet
        }
        if (_networkId == 97) {
            tokensaleTotalSold = 429212543849625700000;
            initialPriceInt = 88800000000000000;   // BCS testnet
        }
        if (_networkId == 80001) initialPriceInt = 607578947368421000;// Matic testnet
        if (_networkId == 137) initialPriceInt = 607578947368421000;// Matic mainnet
        if (_networkId == 1313161554) initialPriceInt = 14145132743363000;// Aurora mainnet
        if (_networkId == 1313161555) initialPriceInt = 14145132743363000;// Aurora testnet
        if (_networkId == 43114) initialPriceInt = 428689655172413952;// AVAX mainnet
        if (_networkId == 43113) initialPriceInt = 428689655172413952;// AVAX testnet
    }
    ...
    function setInitialPriceInt(uint256 _initialPriceInt) public onlyOwner {
        initialPriceInt = _initialPriceInt;
    }
    ...
    function purchaseTokensQuantityFor(uint256 amount) public view returns (uint256,uint256) {
        ...
        uint256 newPrice = initialPriceInt.mul(initialBalance).div(delta);
    ...
    function whitelistContractCreator(address _contractNFT) public payable {
        require(msg.value >= initialPriceInt.mul(auctionCreationFeeMultiplier), "OpenBiSea: you must send minimal amount or more");
        (bool success, ) = owner().call{value:msg.value}("");
        require(success, "OpenBiSea: Transfer failed.");
    ...
    function whitelistContractCreatorTokens(address _contractNFT) public {
        ...
        totalIncome = totalIncome.add(initialPriceInt.mul(amount).div(10 ** uint256(18)));
        IOpenBiSeaAuction(auction).whitelistContractCreator(_contractNFT,initialPriceInt.mul(amount));
    }
    ...
    function bidToken(
        ...
        require(restrictedTokens[token] != true, "OpenBiSea: token is restricted for auctions");
        IERC20(token).transferFrom(msg.sender,address(this),_bidAmount);
        bool isWin;
        uint256 amountTransferBack;
        address auctionLatestBidderOrSeller;
        address tokenSaved;
        (isWin, amountTransferBack, auctionLatestBidderOrSeller, tokenSaved) = IOpenBiSeaAuction(auction).bid( _contractNFT, _tokenId, _bidAmount, _isERC1155, msg.sender, token);
        require(token == tokenSaved, "OpenBiSea: auction use another token");

        if (isWin) {
            uint256 depositFee = _bidAmount.mul(platformFeePercent).div(100);
            uint256 depositFeeReferrer = _bidAmount.mul(referralPercent).div(100);
    ...
    function bid(
        ...
        address auctionLatestBidderOrSeller;
        address token;
        (isWin,amountTransferBack,auctionLatestBidderOrSeller,token) = IOpenBiSeaAuction(auction).bid( _contractNFT, _tokenId, msg.value, _isERC1155, msg.sender, address (0x0) );
        ...
        if (isWin) {
            ...
            if (_referrals[msg.sender] != address(0) || referral != address(0)) {
                ...
                (bool success, ) = owner().call{value:depositFee}("");
                require(success, "OpenBiSea: Transfer failed.");
            }
            (bool success, ) = auctionLatestBidderOrSeller.call{value:totalSellerAmount}("");
            require(success, "OpenBiSea: Transfer failed.");
        } else {
            if (amountTransferBack > 0) {
                (bool success, ) = auctionLatestBidderOrSeller.call{value:amountTransferBack}("");
                require(success, "OpenBiSea: Transfer failed.");
            }
        }
    }
    ...
    function claimFreeTokens() public returns (bool) {
        ...
        uint8 decimals;
        (priceMainToUSD,decimals) = _oracleContract.getLatestPrice();
