...
import "./interfaces/IGAkita.sol";

contract AkitaAirdrop is Ownable, ReentrancyGuard {
    ...
    function dropTokens(
        ...
        uint256[] memory _no_of_vestings
    ) public onlyOwner returns (bool) {
        for (uint256 i = 0; i < _recipients.length; i++) {
            require(_recipients[i] != address(0));
            // require(whitelist.contains(_recipients[i]), "Not whitelist");
        }
        for (uint256 i = 0; i < _recipients.length; i++) {
            require(
                IGAkita(token).mint(address(this), _amount[i]),
                "mint fail"
            );

            lockToken(_amount[i], _recipients[i], _no_of_vestings[i]);
        }
        return true;
    }
    ...
    function lockToken(
        ...
        require(_no_of_vesting <= 3, "Max number of vesting is 3");
        uint256 amountLocked = _amount;
        uint256 amountPerVesting = amountLocked.div(_no_of_vesting);
        uint256 lockDate = block.timestamp;
        uint256 unlockPeriodPerVesting = 1 days;
        uint256 lastUnlockDate = lockDate;
        for (uint256 i = 0; i < _no_of_vesting; i++) {
            TokenLock memory token_lock;
            token_lock.lockDate = lockDate;
            token_lock.amount = amountPerVesting;
            token_lock.unlockDate = lastUnlockDate.add(
                (i + 1).mul(unlockPeriodPerVesting)
            );
            lastUnlockDate = token_lock.unlockDate;
    ...
    function unlockToken(
        ...
        if (vaultBalance < _amount) {
            require(
                IGAkita(token).mint(address(this), _amount.sub(vaultBalance)),
                "mint fail"
            );
        }
