...
contract KGTDAO_Router {
    ...
    function upgradeVIP(uint256 lockKGT,uint16 level)public payable{
       
        require(level > 0 && level <= 6,'level error');
        ...
        require(level > level_last,'level > level_last');
        
        require(lockKGT > 0,'lock KGT error');

        uint payKGT = estimateVIPpayValue(level_last, level);

        require(payKGT <= lockKGT,'payKGT <= lockKGT');
        (uint _0,uint _1) = (lockKGT,payKGT);
        require((_0 - _1) / payKGT < 5 ,'out 5%');
        IERC20(KGTDAO).transferFrom(msg.sender, deadAddr, lockKGT);
        uint payUSD = vip_usd[level] - vip_usd[level_last];
        allVip[msg.sender]._level = level;
        allVip[msg.sender]._costKGT += lockKGT;
        allVip[msg.sender]._costUSD += payUSD;
        allVip[msg.sender]._isValid = true;

        emit UpgradeVIP(msg.sender,level,level_last,payUSD,lockKGT);
