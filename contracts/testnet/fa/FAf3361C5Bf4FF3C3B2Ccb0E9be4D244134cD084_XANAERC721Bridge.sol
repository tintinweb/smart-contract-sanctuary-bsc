...
contract XANAERC721Bridge {
    ...
    event Deposit(string depositType, address owner, uint256 nftId, uint256 sourceChainId, address sourceCollection, uint256 targetChainId, address targetCollection);
    ...
    function sourceDeposit(address _sourceCollection, uint256 _nftId) public payable {
        ...
        bridgeData[_sourceCollection][_nftId] = depositData(data._AChainId, data._BChainId, data._BCollection, true, false);
        ...
        emit Deposit('sourceDeposit', msg.sender, _nftId, data._AChainId, data._ACollection, data._BChainId, data._BCollection);
    }
    ...
    function targetDeposit(address _targetCollection, uint256 _nftId) public payable {
        ...
        bridgeData[data._ACollection][_nftId]._deposited = false;
        bridgeData[data._ACollection][_nftId]._released = true;
        ...
        emit Deposit('targetDeposit', msg.sender, _nftId, data._AChainId, data._ACollection, data._BChainId, data._BCollection);
    }
    ...
    function mintNft(address _owner, address _targetCollection, uint256 _nftId) public onlyOwner {
