...
contract ProjectReward is AccessControl, IAirdropReward, Initializable {
    ...
    uint256 public airdropBNBamount;
    ...
    function isUserEligibleForAirdrop() public view returns (bool) {
        return (userAirdropDetails[msg.sender].claimedIndex <
            userAirdropDetails[msg.sender].airdrops.length);
    }
    ...
    function claimAirdrop() external {
        ...
        uint256 end = airdropCount > i + 100 ? i + 100 : airdropCount;
        for (; i < end; i++) {
            if (userAirdropDetails[msg.sender].airdropAmount[i + 1] > 0) {
                if (airdropsDetails[i + 1].token != address(1)) {
                    ...
                    airdropBNBamount -= userAirdropDetails[msg.sender]
                        .airdropAmount[i + 1];
                }
                emit ClaimedAirdrop(
    ...
    /**
     @dev Transfers extra BNB to wallet
     */
    function takeLockedBNB() external onlyAdmin {
        require(address(this).balance > airdropBNBamount, "No BNB amount");
        uint256 BNBValue = address(this).balance - airdropBNBamount;
        (bool sent, ) = payable(wallet).call{value: BNBValue}("");
        require(sent, "Failed to send BNB");
    }
