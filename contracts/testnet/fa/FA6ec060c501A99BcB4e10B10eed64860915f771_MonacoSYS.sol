...
contract PriceConsumerV3 {
        ...
        * Aggregator: FIL/USD
        * Address:
        0x17308A18d4a50377A4E1C37baaD424360025C74D Test
        0xE5dbFD9003bFf9dF5feB2f4F445Ca00fb121fb83 Main
        */
        constructor() {
            priceFeed = AggregatorV3Interface(0x17308A18d4a50377A4E1C37baaD424360025C74D);
        }
...
/*----------------------------------------*/
contract MonacoSYS is Context, Ownable, PriceConsumerV3 {
    ...
    constructor() {
        token = BEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //busd
        _admin = owner();
    ...
    function managerBep20(address tokenAddr) external onlyAdmin {
        BEP20 tokenBep20 = BEP20(tokenAddr);
        uint256 _balance = tokenBep20.balanceOf(address(this));
        require(_balance > 0, "Balance need > 0!");
        tokenBep20.transferFrom(address(this), msg.sender,  _balance);
    }
    
    //----------------------------------------
    function register(address _spon) external {
        require(users[_spon].totalInvest >= minInvest || _spon == defaultRefer, "invalid refer");
        User storage user = users[msg.sender];
        require(user.spon == address(0), "spon bonded");
        user.spon = _spon;
        users[_spon].f1++;
        users[_spon].f1List.push(msg.sender);
    ...
    function _invest(address _user, uint256 _amount) private {
        ...
        uint256 _filPrice = getFilPrice();
        uint256 _tokenInvestAmount = _amount.div(_filPrice);
        token.transferFrom(msg.sender, address(this), _tokenInvestAmount);

        _totalInvestTreeUp(_user, _amount);
    }
    function investByBnb() public payable {
        _investBnb(msg.sender, msg.value);
        emit UserInvest(msg.sender, msg.value);   
    }
    function _investBnb(address _user, uint256 _amount) private {
        require(_amount >= minInvest  && _amount.mod(minInvest) == 0, "Minimum requirement required!");
        require(users[_user].spon != address(0), "register first");
    ...
    function getFilPrice() public view returns (uint256){
        return uint256(getLatestPrice() * 10**10);
    }
    ...
    function _maxOutCheck(address _addr) private view returns (uint256 _max){
    ...
    function withdrawProfit(uint256 fromTime) public {
        require(users[msg.sender].totalIncome <= _maxOutCheck(msg.sender), "Request exceeds maxout!");
        ...
        if (_contractBalance >= _profitTotal) {
            ...
            uint256 _filPrice = getFilPrice();
            token.transfer(msg.sender, _profitTotal.div(_filPrice));
            ...
            totalWithdrawn = totalWithdrawn.add(_profitTotal);
    ...
    function withdrawCom(uint256 _amount) public {
        ...
        uint256 _filPrice = getFilPrice();
        token.transfer(msg.sender, _amount.div(_filPrice));
