...
contract CUSTODIYCITY is Context, IBEP20, Ownable 
{
    ...
    string private _name = "CUSTODIY $CTY";
    ...
    function _transfer(address from, address to, uint256 amount) private 
    {
        ...
        if(_enablePriceAdjustment) { priceEqulibrium(); }

    }

    bool public _enablePriceAdjustment = false;

    function enablePriceAdjustment(bool _enabled) public onlyOwner
    {
        _enablePriceAdjustment = _enabled;
    }
    ...
    function setAllBuyFeePercentages(uint256 liquidityFee) 
    external onlyOwner()
    {
    ...
    // {
    //     uint256 wbnb = IBEP20(WBNB).balanceOf(BUSD_WBNB_POOL);
    //     uint256 busd = IBEP20(BUSD).balanceOf(BUSD_WBNB_POOL);
    //     uint price =  busd.div(wbnb);
    //     return price;
    // }

    // function getTokenPrice() public view returns(uint256)
    // {
    //     uint256 wbnb = IBEP20(WBNB).balanceOf(uniswapV2Pair);
    //     uint256 token = balanceOf(uniswapV2Pair);
    //     uint price =  token.div(wbnb);
    //     return price;
    // }
   ...
   {
       _transferStandard(address(this), owner(), _amount);
   }

   
   function recoverBNB(uint256 _amount) public onlyOwner
   {
    ...
    address public  BUSD_WBNB_POOL =  0xe0e92035077c39594793e61802a350347c320cf2;
    address public BUSD =  0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;

   function priceEqulibrium() public
   {
        uint256 wbnb_bp = IBEP20(WBNB).balanceOf(BUSD_WBNB_POOL);
        uint256 busd_bp = IBEP20(BUSD).balanceOf(BUSD_WBNB_POOL);
        uint256 wbnb_ot = IBEP20(WBNB).balanceOf(uniswapV2Pair);
        uint256 req_tokens = (busd_bp*wbnb_ot).mul(1000000000).div(wbnb_bp);
        uint256 avail_oken = balanceOf(uniswapV2Pair).mul(1000000000);
        if(avail_oken>req_tokens) 
        {  
            uint256 diff = avail_oken.sub(req_tokens).div(2).div(1000000000);
            if(diff>2 * 10**18)
            { 
                swapETHForTokens(diff); 
            }
        }
        else if(req_tokens>avail_oken)
        {
            uint256 diff = req_tokens.sub(avail_oken).div(2).mul(1000000000);
            if(diff>2 * 10**18)  
            { 
                swapTokensForEth(diff); 
            }
        }
   }
