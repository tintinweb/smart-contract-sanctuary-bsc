    ...
    OwnableUpgradeable
{
    ...
    event rewardClaimedSuccess(uint256 claimedAmount);
    event rewardClaimedFailed(uint256 avilableBalance);
    ...
    modifier onlyOcean() {
        require(
            msg.sender == address(REWARD) ||
                msg.sender == _token ||
                msg.sender == address(stakeAddress) ||
                msg.sender == owner()
        );
        _;
    }
    ...
    function deposit(uint256 amount) external override onlyOcean {
        stakeAddress.stake(amount);

        lastStakedTime = block.timestamp;
    }

    function claimStaking() external {
        uint256 avilableBalance = stakeAddress.calculateUnclaimedReward(
            address(this)
        );

        if (
            avilableBalance > 1 ether &&
            ((lastClaimedTime + claimDuration) <= block.timestamp)
        ) {
            // initial balance

            uint256 claimableReward = stakeAddress.claimReward();

            totalDividends = totalDividends + claimableReward;
            dividendsPerShare =
                dividendsPerShare +
                ((dividendsPerShareAccuracyFactor * claimableReward) /
                    totalShares);

            emit rewardClaimedSuccess(avilableBalance);
        } else {
            emit rewardClaimedFailed(avilableBalance);
        }
