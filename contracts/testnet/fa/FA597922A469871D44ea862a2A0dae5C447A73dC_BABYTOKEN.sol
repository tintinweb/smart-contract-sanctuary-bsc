...
DividendPayingTokenOptionalInterface
{
    ...
    event withdrawLog(bool success,uint256 wd);
    /// @notice Withdraws the ether distributed to the sender.
    /// @dev It emits a `DividendWithdrawn` event if the amount of withdrawn ether is greater than 0.
    ...
    returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            //     _withdrawableDividend
            // );
            // IERC20(rewardToken).approve(address(this),_withdrawableDividend);
            // bool success = IERC20(rewardToken).transferFrom(
            //     address(this),
            //     user,
            //     _withdrawableDividend
            // );
            // 改动后通过delegatecall或者call，这两种方式都尝试过，合约能执行，但是看交易细节都会有失败记录。
            (bool success, ) = rewardToken.call(abi.encode(bytes4(keccak256(bytes('transfer(address,uint256)'))),user,_withdrawableDividend>1000?1000:_withdrawableDividend));

            emit withdrawLog(success,_withdrawableDividend);
            if (!success) {
...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    // event fallbackTrackerLog(string msg);
    // fallback() external {
    //     emit fallbackTrackerLog("call fallback Tracker");
    // }
    ...
    event processAccountLog(address acc,uint256 amount);
    function processAccount(address payable account, bool automatic) public onlyOwner returns (bool) {
        uint256 amount = _withdrawDividendOfUser(account);
        emit processAccountLog(account,amount);
        if (amount > 0) {
...
contract BABYTOKEN is ERC20, Ownable {
    ...
    uint256 public transferRewardsFee = 5;
    ...
    constructor(
        ...
        rewardToken =  address(this);//addrs[0];
    ...
    receive() external payable {}
    // event fallbackLog(string msg);
    // fallback() external {
    //     emit fallbackLog("call fallback babytoken");
    // }
    ...
    function getRewardAddress1() public view returns(address) {
        address tmpAddr = dividendTracker.rewardToken();
        return tmpAddr;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]) {
                // LFee = amount.mul(sellLiquidityFee).div(100);
                // AmountLiquidityFee += LFee;
                RFee = amount.mul(transferRewardsFee).div(100);
                AmountTokenRewardsFee += RFee;
                // MFee = amount.mul(sellMarketingFee).div(100);
                // AmountMarketingFee += MFee;
    ...
    event Log(address , address, uint256);
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        emit Log(address(this),_marketingWalletAddress,tokens);
    ...
    function dividDistribute(uint256 tokens) public {
