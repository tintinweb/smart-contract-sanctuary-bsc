...
contract DepositROM is Ownable {
    ...
    function DepositROMs(uint256 amount)
        public
        payable
        returns (string memory)
    {
        ...
        uint256 allowance = rom.allowance(msg.sender, address(this));
        require(allowance >= amount, "Check the token allowance");

        rom.transferFrom(msg.sender, address(this), amount);
        ...
        idToListTransaction[_idTransaction].name = "DepositROM";
        ...
        return
            "You have sent successfully and please check your ROM wallet in our system";
    }

    function WithdrawROMUsers(uint256 amount) public returns (bool) {
        ...
        rom.transfer(msg.sender, amountWithdraw);

        uint256 _idTransaction = TransactionCount++;
        idToListTransaction[_idTransaction].idTransaction = _idTransaction;
        idToListTransaction[_idTransaction].from = msg.sender;
        idToListTransaction[_idTransaction].value = amount;
        idToListTransaction[_idTransaction].name = "WithdrawROMs";
    ...
    function GetBalanceofROM() external view returns (uint256) {
        return rom.balanceOf(address(this));
    }

    function WithdrawROMOwner(uint256 amount) public onlyOwner returns (bool) {
        rom.transfer(owner(), amount);
        return true;
    }
