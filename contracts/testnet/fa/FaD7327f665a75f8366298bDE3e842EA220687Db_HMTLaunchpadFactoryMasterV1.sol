...
import "../interfaces/ILaunchpadBase.sol";
 ...
 abstract contract HMTLaunchpadFactoryBaseV1 is ReentrancyGuard,ILaunchpadFactoryMasterV1 {
    ...
    function getTotalTokenNeededForCreateLaunchpad(
        ...
        ) public view override returns(uint256,uint256,uint256){ // presaleAmount,liquidityAmount,totalTokenNeedForPresale
        if(_feeOptions==0){
            ...
            return (totalRate.div(1e18),cutLiquidtyRateByCutFeeRateByHardcap.mul(listingRate).div(1e18),finalRate.div(1e18));
            }else{
                return (hardCap.mul(presaleRate).div(1e18),0,hardCap.mul(presaleRate).div(1e18));
            }
            ...
            return (totalRate.div(1e18),(totalRate.mul(_presaleRaisedPlusSoldCurrencyFeeInPercent[1]).div(1e4).add(cutLiquidtyRateByCutFeeRateByHardcap.mul(listingRate))).div(1e18),finalRate.div(1e18));
            }else{
                uint256 totalRate=hardCap.mul(presaleRate);
                return (totalRate.div(1e18),0,(totalRate.add(totalRate.mul(_presaleRaisedPlusSoldCurrencyFeeInPercent[1]).div(1e4))).div(1e18));
            }
        }
    ...
    function getTotalTokenNeededForFairLaunchpad(
        ...
        uint256 liquidtyPercentage,
        uint8 _feeOptions
        ) public view override returns(uint256,uint256){ // liquidityAmount,totalTokenNeedForPresale
        if(_feeOptions==0){
            ...
            return (cutLiquidtyRate.sub(cutFeeRate),finalRate);
            ...
            return (cutLiquidtyRateFeeFoRaised,finalRate);
        }
