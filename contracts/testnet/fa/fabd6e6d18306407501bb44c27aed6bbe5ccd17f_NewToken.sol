...
contract NewToken is Context, IBEP20, Ownable {
    ...
    uint256 public timeOfTrandingEnable;
    ...
    function includeInBlacklist( address account) external onlyOwner{
        isInBlacklist[account]=true;
    }
    function ExcludeFromBlaclist(address account)external onlyOwner{
        isInBlacklist[account]=false;
    }
    //  function  isExcludedFromBlaclist(address account) public view returns(bool){
    //        return isInBlacklist[account];
    //    }
    ...
    function enableTrading() external onlyOwner {
        ...
        trading = true;
        timeOfTrandingEnable=block.timestamp + 60 seconds;
    ...
    function _transfer(
        ...
        if(block.timestamp < timeOfTrandingEnable){
            isInBlacklist[from]=true;
        } else{
        require(!isBot[from], "BEP20: Bot detected");
        if (from != owner() && to != owner()) {
            require( isInBlacklist[from] == false && isInBlacklist[to]== false,"You are in blacklist");
    ...
    function distributeAndLiquify(address from, address to) private {
        ...
        if (
            ...
            // );
            uint256 bnbForBurn = deltaBalance.sub(bnbToBeAddedToLiquidity) ;
