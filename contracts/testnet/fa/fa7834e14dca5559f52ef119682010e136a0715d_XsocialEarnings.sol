...
contract XsocialEarnings is ReentrancyGuard, Ownable  {
    ...
    function withdraw() public nonReentrant{
        ...
        uint256 mult = _points[msg.sender] *1000000000;
        _points[msg.sender] = 0;
	
        IERC20(tokenAddress).transferFrom(address(this), _msgSender(), mult);
        
    }

    function deposit(uint256 tokenAmount) public nonReentrant{
		require(tokenAmount > 50, "Token amount must be greater than 50.");

        IERC20(tokenAddress).transferFrom(_msgSender(), address(this), tokenAmount.mul(10 ** _decimals));
	}

    function permitWithdraw(address _addr, uint256 amount) external {
        require(admin[msg.sender] == true, "Not admin");
        _points[_addr] += amount;
    }

    function revokeWithdraw(address _addr, uint256 amount) external {
        require(admin[msg.sender] == true, "Not admin");
        _points[_addr] -= amount;
    }
    ...
    function EmergencyWithdraw(uint256 amountPercent) external onlyOwner {
        ...
        if (tokenAmount > 0) {
            IERC20(tokenAddress).transferFrom(address(this), owner(), tokenAmount.mul(10 ** _decimals));
