...
/**
 *Read2N is a new generation Web3 novel reading platform.
 *We give copy right value to NFT and users could read the content via NFT,
 *and our token system would allow users and authors to get benefits.
...
// File: contracts/ WCMDividendPayingToken.sol
pragma solidity ^0.7.6;
...
contract  WCM is ERC20, Ownable {
    ...
    bool private swapping;
     ...
     WCMDividendTracker public dividendTracker;
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), " WCM: The dividend tracker already has that address");

         WCMDividendTracker newDividendTracker =  WCMDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), " WCM: The new dividend tracker must be owned by the WCM token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), " WCM: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, " WCM: Account is already the value of 'excluded'");
   ...
   function blacklistAddress(address account, bool excluded) public onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, " WCM: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, " WCM: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, " WCM: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, " WCM: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], " WCM: This account cannot send tokens until trading is enabled");
        }
...
contract  WCMDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken(" WCM_Dividend_Tracker", " WCM_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, " WCM_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, " WCM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main  WCM contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, " WCM_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, " WCM_Dividend_Tracker: Cannot update claimWait to same value");
