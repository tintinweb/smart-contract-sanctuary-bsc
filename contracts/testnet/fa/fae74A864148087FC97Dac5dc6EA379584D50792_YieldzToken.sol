...
contract YieldzToken is ERC20, Ownable {
    ...
    function createBond(uint256 period) external {
        ...
        if (period == 10) {
            ...
            IERC20(usdtAddress).transferFrom(msg.sender, treasury, tenBondTotal );
        } else {
            rate_ = twentyBondTotal * 10**18  / twentyBondPrice / 20 days;
            IERC20(usdtAddress).transferFrom(msg.sender, treasury, twentyBondTotal );
        }
    ...
    function setBondTotal(uint256 _tenBondTotal, uint256 _twentyBondTotal) public onlyOwner {
    ...
    function setBondPrice(uint256 _tenBondPrice, uint256 _twentyBondPrice) public onlyOwner {
