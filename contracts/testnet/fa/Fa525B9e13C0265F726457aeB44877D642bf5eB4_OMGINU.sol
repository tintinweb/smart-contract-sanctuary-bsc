...
function totalSupply() external view returns (uint256);
    ...
    event  _checkcontbal(string  iscontgrtr);
...
contract OMGINU is Context, IERC20, Ownable {
    ...
    uint256 public  _tokenForLiquidity = 10000000;
  ...
  function sendToWallet(address payable wallet, uint256 amount) private {
   ...
   function funcSETBuyTax(uint256 liquidity, uint256 dev) external onlyOwner() 
     {
   ...
   function funcSectSelltax(uint256 liqTX,  uint256 devTax) external onlyOwner() 
     {
        _liqSelTx = liqTX;
         _dvSelTx=devTax;
         BuyOrSell=2;
      ...
      function setTokenForLiquidity(uint256 TokensLiq) external onlyOwner  {
       ...
       function CntBuyOrSell() public view returns  (uint256){
          return BuyOrSell;
        }
   ...
   function totalSupply() public view override returns (uint256) {
    ...
    function _transfer(
      ...
      uint256 _contractBalance=balanceOf(address(this));
        ...
        if (_contractBalance > _tokenForLiquidity)
        {
            _contractBalance =_tokenForLiquidity;
            emit _checkcontbal ("yes");
        }
    ...
    if (BuyOrSell==1)
        {
         ...
         if (_contractBalance >= _tokenForLiquidity &&  !swapping && from != uniswapV2Pair && from != owner() && to != owner()) {
            swapping = true;
            letLiqSomedata(_contractBalance);
            swapping = false;
        }
        _tOwned[from] -= amount;
         uint256 transferAmount = amount;
          if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to])
          {
                uint256 liquidityFee = amount.mul(_liqTx).div(100); 
                    ...
                    transferAmount=amount.sub(liquidityFee).sub(devfee);
         } 
          _tOwned[to] += transferAmount;
        emit Transfer(from, to, transferAmount);
        emit _displayTakeFee(takeFee, _contractBalance , _tokenForLiquidity );
        
      }
  function letLiqSomedata(uint256 _contractBalance) private 
  {
                  
           uint256 tokensDev = _contractBalance.mul(_dvTx).mul(100).div(2);
           uint256 tokensLP = _contractBalance.mul(_liqTx).mul(100).div(2);
           emit _getEventOFLiquify(_tOwned[address(this)],  tokensDev , tokensLP);
           uint256 balanceBeforeSwap = address(this).balance;
           emit _swapTokenForBNB(tokensLP.add(tokensDev));
           swapTokensForBNB(tokensLP.add(tokensDev));
           uint256 BNB_Total = address(this).balance.sub(balanceBeforeSwap);
           emit _BNBTotal(BNB_Total);
             uint256 DevPer = _dvTx.mul(100).div(_liqTx.add(_dvTx));
             uint256 BNB_D = BNB_Total.mul(DevPer).div(100);
             emit _split(DevPer , BNB_D);
             emit _addLiquidity(tokensLP, (BNB_Total.sub(BNB_D)));
             addLiquidity(tokensLP, (BNB_Total.sub(BNB_D)));
