...
contract HyperRunTokensVesting {
    ...
    modifier onlyWithdrawAddress () {
        require(whitelisted[msg.sender].exists == true, "This wallet is not whitelisted");
        _;
    }
    ...
    function sendTokens (uint256 tokensToSend) private {
        if (tokensToSend > 0) {
            ...
            whitelisted[msg.sender].totalClaimAmount.add(tokensToSend);
            // Sending allowed tokens amount
            hyperRunToken.transfer(msg.sender, tokensToSend);
            // Raising event
            emit Withdraw(msg.sender, tokensToSend, block.timestamp);
        }
    }
    ...
    function getTokensAmountAllowedToWithdraw (address wallet, uint256 tokensUnlockedPercentage) private view returns (uint256) {
        ...
        return totalTokensAllowedToWithdraw;
    }
