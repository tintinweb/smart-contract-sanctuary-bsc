...
function transfer(uint256 _nftId, address _target)
  ...
  function syncUpdateCharacter(uint256 from,uint256 to) public onlyOwner  {
    uint range=_tokenIds.current();
    require(to<=range,"Out of range data");
      for (uint i=from; i<=to; i++) {
        if(_exists(i)){
          address owner=ownerOf(i);
          // uint256 value = uint256(i*block.timestamp);
          // uint random = (value << 10)%100+1;
          // uint typeChar=7;
          // if(1<= random && random<=40){
          //     typeChar=1;
          // } else if (41 <= random && random <= 60) {
          //     typeChar=2;
          // } else if (61 <= random && random<=80) {
          //     typeChar=3;
          // }
          Character memory character= getCharacter(owner,i);
          characters[owner][i] = encode(Character(i,character.characterType,character.rank,character.star,character.exp,0));
          characterIndexToOwner[i]=owner;
        }else{
          characters[characterIndexToOwner[i]][i] = encode(Character(i,0,0,0,0,1));
