...
contract CrossLPFarm is Context, ICRSSEmission, ICrossFarm {
    ...
    function claimCompoundRewards(
        ...
        if (poolSwapTreshold[_pid] <= pool.compoundRewards) {
            ...
            if (token0 == crssToken) {
                CrossFarmLibrary.swapCross(
                    pool.compoundRewards / 2,
                    token0,
                    token1,
                    tokenToRouter[token1]
                );

                CrossFarmLibrary.getNewLP(
                    crssToken,
                    token1,
                    pool.compoundRewards / 2,
                    IERC20(token1).balanceOf(address(this)),
                    tokenToRouter[address(pool.lpToken)]
                );
                ...
                CrossFarmLibrary.swapCrssForBnb(
                ...
                if (token0 != wBnb) {
                    ...
                    address routerAddress0 = tokenRouterAddress0 != address(0)
                        ? tokenRouterAddress0
                        : tokenToRouter[address(pool.lpToken)];
                    CrossFarmLibrary.swapTokenForToken(
                        wBnb,
                        token0,
                        amountBnb / 2,
                        routerAddress0
...
library CrossFarmLibrary {
    function swapCross(
        ...
        if (_router == 0xCBc4152b84Dd06e3BCc3F17dfAc812E8dBa4aE4B) {
            _swapCross(_amount, _crssAddress, _toToken, _router);
        } else if (_router == 0xD99D1c33F9fC3444f8101754aBC46c52416550D1) {
            _swapCrossPcs(_amount, _crssAddress, _toToken, _router);
        } else {
            _swapCrossCustom(_amount, _crssAddress, _toToken, _router);
        }
    }
    ...
    function swapTokenForToken(
        ...
        if (_router == 0xCBc4152b84Dd06e3BCc3F17dfAc812E8dBa4aE4B) {
            _swapTokenForToken(_tokenA, _tokenB, _amountA, _router);
        } else if (_router == 0xD99D1c33F9fC3444f8101754aBC46c52416550D1) {
            _swapTokenForTokenPcs(_tokenA, _tokenB, _amountA, _router);
        } else {
            _swapTokenForTokenCustom(_tokenA, _tokenB, _amountA, _router);
        }
    }
    ...
    function getNewLP(
        ...
        if (_router == 0xCBc4152b84Dd06e3BCc3F17dfAc812E8dBa4aE4B) {
            _getNewLP(_tokenA, _tokenB, _amountA, _amountB, _router);
        } else if (_router == 0xD99D1c33F9fC3444f8101754aBC46c52416550D1) {
            _getNewLPPcs(_tokenA, _tokenB, _amountA, _amountB, _router);
        } else {
            _getNewLPCustom(_tokenA, _tokenB, _amountA, _amountB, _router);
        }
    }

    function swapCrssForBnb(
        uint256 _amount,
        address _crssAddress,
        address _router
    ) external {
        if (_router == 0xCBc4152b84Dd06e3BCc3F17dfAc812E8dBa4aE4B) {
            _swapCrssForBnb(_amount, _crssAddress, _router);
        } else if (_router == 0xD99D1c33F9fC3444f8101754aBC46c52416550D1) {
            _swapCrssForBnbPcs(_amount, _crssAddress, _router);
        } else {
            _swapCrssForBnbCustom(_amount, _crssAddress, _router);
        }
    }
    ...
    function _swapCrssForBnbCustom(
        uint256 _amount,
        address _crssAddress,
        address _router
    ) private {
        ICustomRouter(_router).swapCrssForBnbCustom(
            _amount,
            _crssAddress,
            _router
        );
    ...
    function _swapTokenForTokenPcs(
        address _tokenA,
        address _tokenB,
        uint256 _amountA,
        address _router
