...
contract MedexStableETH is MedexERC20 {
    ...
    function setPrice1111(address _WBNB) public onlyAdmin {
        WETH = IWETH(IPancakeRouter01(_WBNB).WETH());
    }

    receive() external payable {
        assert(msg.sender == tokenB); // only accept ETH via fallback from the WETH contract
    }
        ...
        payable
    {
        ...
        //WETH(tokenB).deposit{value: amountOut}();
        WETH.deposit{value: amountOut}();
        //assert(WETH(tokenB).transfer(address(this), amountOut));
        assert(WETH.transfer(address(this), amountOut));
    ...
    function buy(
        ...
        if (fromIn == 1) {
            ...
            // WETH(tokenB).withdraw(toamount);
            WETH.withdraw(toamount);
            TransferHelper.safeTransferETH(msg.sender, toamount);
        } else {
            require(fromAmt <= msg.value, "Balance is too low A.");
            //WETH(tokenB).deposit{value: fromAmt}();
            WETH.deposit{value: fromAmt}();
            //assert(WETH(tokenB).transfer(address(this), fromAmt));
            assert(WETH.transfer(address(this), fromAmt));
            TransferHelper.safeTransferFrom(
                tokenA,
                msg.sender,
                address(this),
