...
import "./SLOWToken.sol";

// MasterChef is the master of Slow. He can make Slow and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accSlowPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accSlowPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accSlowPerShare; // Accumulated POPSs per share, times 1e12. See below.
    }
    // The POPS TOKEN!
    SLOWToken public pops;
    ...
    function pendingSlow(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accSlowPerShare = pool.accSlowPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0
            ...
            accSlowPerShare = accSlowPerShare.add(
                popsReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accSlowPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accSlowPerShare = pool.accSlowPerShare.add(
            popsReward.mul(1e12).div(lpSupply)
        );
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accSlowPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            safeSlowTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accSlowPerShare).div(1e12);
    ...
    function safeSlowTransfer(address _to, uint256 _amount) internal {
...
contract SLOWToken is ERC20, Ownable {
