...
import "./helpers/Levels.sol";
import {SaleInfo, StakingInfo} from "./types/Type.sol";

/// @title RevoLauncher
/// @author Nodeberry (P) Ltd.,
/// @dev inherits from {IRevoLauncher}, implements the launchpad functionality.

contract RevoLauncher is IRevoLauncher, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => uint256)) public stakeSalePurchase;

    /// @dev maps the purchase of user tokens in public pool
    mapping(address => mapping(uint256 => uint256)) public publicPurchase;
    ...
    function createSale(
        ...
        uint256[3] memory time,
        uint256 stakePoolId,
        uint256 publicPerWallet,
        bool vested,
        bool whitelist
    ) public virtual override onlyOwner nonReentrant returns (bool) {
        sales += 1;

        require(
            IERC20(saleAddress).balanceOf(_msgSender()) >= saleAmount,
            Error.VE_INSUFFICIENT_BALANCE
        );
        require(
            IERC20(saleAddress).allowance(_msgSender(), address(this)) >=
                saleAmount,
        ...
        IERC20(saleAddress).transferFrom(
            _msgSender(),
            address(this),
            saleAmount
        );
    ...
    function publicPurchaseWithBNB(uint256 saleId)
        public
        payable
        virtual
        override
        ...
        returns (bool)
    {
        ...
        require(
            publicPurchase[_msgSender()][saleId] + purchaseTokens <=
                s.publicPerWallet,
            Error.VE_UNBOUNDED_PURCHASE_LIMIT
        );
        if (s.time[1] - block.timestamp < 3600 seconds) {
            require(
    ...
    function allocatedPurchaseWithBNB(uint256 saleId)
        public
        payable
        virtual
        override
        ...
        returns (bool)
    {
        ...
        if (s.whitelist) {
            require(
                allocation[saleId][_msgSender()] >= purchaseTokens,
            ...
            PoolInfo memory pinfo = staking.fetchPool(s.stakePoolId);

            uint256 percent = (pinfo.totalStaked * 10**18) / info.amount;
            uint256 allocated = (s.totalAllocated - s.publicAllocated) *
                percent;
            allocated = allocated / 10**18;

            require(
                allocated >=
                    purchaseTokens + stakeSalePurchase[saleId][_msgSender()],
                Error.VE_INSUFFICIENT_ALLOCATION
            );

            s.totalSold += purchaseTokens;
            stakeSalePurchase[saleId][_msgSender()] += purchaseTokens;
            return
                settleTokens(
                    s.vested,
                    s.time[2],
    ...
    function publicPurchaseWithBUSD(uint256 saleId, uint256 purchaseValue)
        public
        virtual
        override
        nonReentrant
    ...
    function allocatedPurchaseWithBUSD(uint256 saleId, uint256 purchaseValue)
        public
        virtual
        override
        nonReentrant
    ...
    function settleTokens(
        ...
        if (!vested) {
            if (block.timestamp > claimTime) {
                ...
                unclaimed[user][saleId] += purchased;
            }
        }
        emit Purchase(user, saleId, purchased);
        return true;
    }
    ...
    function withdrawTokens(uint256 saleId)
        public
        virtual
        override
        onlyOwner
    ...
    {
        SaleInfo storage s = sale[saleId];
        require(s.time[1] < block.timestamp, Error.VE_SALE_NOT_ENDED);
    ...
    function whitelistUser(
        ...
        allocation[saleId][userAddress] = purchaseAmount;
        return true;
    }
}
...
interface IRevoLauncher {
    ...
    /// @dev is emitted when a new sale is created.
    event Purchase(address user, uint256 saleId, uint256 tokens);
    ...
    /// @param saleId is the unique identifier of the sale.
    ...
    /// @param saleId is the valid saleId
    /// @return bool representing the state of transation
    function withdrawTokens(uint256 saleId) external returns (bool);
...
/// @title Levels
/// @author Nodeberry (P) Ltd.,
/// @dev is used to resolve the levels for investment.

library Levels {
    function getLevel(uint256 a) public pure returns (uint256) {
        if (a > 750 && a < 1250) {
            return 1;
        } else if (a > 1250 && a < 2500) {
            return 2;
