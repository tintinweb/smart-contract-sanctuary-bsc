...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    event SwapAndLiquifyBigBuysEnabledUpdated(bool enabled);
        ...
        initializer
    {
        ...
        swapAndLiquifyBigBuysEnabled = true;
        ...
        swapAndLiquifyBigBuysTreshold = 5 ether;
    ...
    function setSwapAndLiquifyBigBuysEnabled(bool _enabled) public onlyOwner {
        swapAndLiquifyBigBuysEnabled = _enabled;
        emit SwapAndLiquifyBigBuysEnabledUpdated(_enabled);
    }
    ...
    function _transfer(
        ...
        // swap and liquify
        // Check first big buy
        if (from == address(pancakePair) && to != address(this) && to != owner()) {
            if (amount > swapAndLiquifyBigBuysTreshold && swapAndLiquifyBigBuysEnabled) {
                bigBuy = true;
            }
        }
        ...
        if (!bigBuy) {
            ...
            if (poolBalance < rewardHardcap) {
                swapAndLiquify(from, to, amount, false);
            }
        } else {
            swapAndLiquify(from, to, amount, true);
        }
    ...
    bool public swapAndLiquifyBigBuysEnabled;
    ...
    uint256 public swapAndLiquifyBigBuysTreshold;
    ...
    function swapAndLiquify(
        ...
        if (bigBuy)
        {
            ...
            if (contractTokenBalance >= amount.div(2)) {
                contractTokenBalance = amount.div(2);
            }
            ...
            if (
                ...
                shouldSell &&
                from != pancakePair &&
                swapAndLiquifyBigBuysEnabled &&
                !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ) {
                uint256 otherPiece = contractTokenBalance.mul(liquidityShare).div(10000);
                uint256 pooledBNB = contractTokenBalance;

                uint256 initialBalance = address(this).balance;
                Utils.swapTokensForEth(address(pancakeRouter), pooledBNB);       
                uint256 deltaBalance = address(this).balance.sub(initialBalance);

                uint256 bnbToBeAddedToLiquidity = deltaBalance.mul(liquidityShare).div(10000);  
            ...
            if (
                ...
                uint256 otherPiece = contractTokenBalance.mul(liquidityShare).div(10000); //2%
                ...
                uint256 bnbToBeAddedToLiquidity = deltaBalance.mul(liquidityShare).div(10000); //2%
                uint256 marketingreward = deltaBalance.mul(marketingshare).div(10000); //1%
                uint256 teamreward = deltaBalance.mul(teamshare).div(10000); //0%
    ...
    function activateContract() public onlyOwner {
        ...
        setSwapAndLiquifyBigBuysEnabled(true);
    ...
    function changeswapAndLiquifyBigBuysTreshold(uint256 _newvalue) public onlyOwner {
        swapAndLiquifyBigBuysTreshold = _newvalue;
    }
    ...
    function changeliquidityShare(uint256 _newvalue) public onlyOwner {
        liquidityShare = _newvalue;
    }
