...
contract RigelPad_Oracula_BUSD {
    ...
    // 0xF5959ecB547AA5B1c464C52451CCA270231ad1b2
    // 0x3F7F18EE8473967aAdBae6F9765Ff3edCfE3aD68
    // 2000000000000000000
    ...
    constructor( 
        ...
        ) 
        {
            ...
            updateDistributionTime(_swapTokenTo, _distTime, _percent);
    ...
    function updateDistributionTime(
        ...
        uint256 dec = 10 ** IERC20(_swapTokenTo).decimals();
        if (_time.length == 1) {
   ...
   function updateInfoData(
    ...
    function lockFund(uint256 _tokenAmount) public checkLocked{
        ...
        data storage inData = getData[0];
        UserFunds storage user = userFunds[_msgSender()];
        ...
        require((user.lockedFunds + _tokenAmount) <= inData.maxLockForEachUser, "Cant exceed the max amount of token to lock for this User.");
        ...
        uint256 userDep = user.lockedFunds + _tokenAmount;
        user.lockedFunds = userDep;                
        uint256 rwd = (userDep / inData.price) * 1E18;
        user.rewards =  rwd ;
    ...
    function dist(uint256 _distPercent) internal {
        ...
        for(uint256 i = 0; i <= (userLength - 1); i++) {
            ...
            if (user.amountReceived == user.rewards) {
                delete(users[i]); // empty the users.length
            } else {
                // calculate amount to transfer to user in %
                uint256 dec = 10 ** IERC20(inData.swapTokenTo).decimals();
                uint256 _amt = ((user.rewards * (_distPercent * dec)) / 100E18);
                // update user locked funds
                user.amountReceived = user.amountReceived + _amt;
    ...
    function getOutPutAmount(uint256 _distPercent, uint256 amount) public view returns(uint256) {
        data memory inData = getData[0];
        uint256 dec = 10 ** IERC20(inData.swapTokenTo).decimals();
        uint256 rwd = (amount / inData.price) * 1E18;
        uint256 _amt = ((rwd * (_distPercent * dec)) / 100E18);
        return (_amt);
    }
