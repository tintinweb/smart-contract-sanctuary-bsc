...
contract lbtestw is VRFConsumerBase,ReentrancyGuard,Ownable {
	using SafeMath for uint256;
    ...
    // External Jackpot fill
    function fillJackpot(uint256 amount) external {
        require(msg.sender == dailyContract,"only luckblocks dailly lotteries can fill this contract.");

        prizeValueJackpot += amount;               
    }
    ...
    function autoSpin(address _caller, uint256[] calldata _nodes) public nonReentrant {
        ...
        require(block.timestamp > lastAutoSpin + 604800);

         uint256 botReward = prizeValueJackpot.mul(95).div(1000); // 9,50% of weekly jackpot goes to NFTNode Golden

         if(getJackpotinUSD() >= (minimumPot*5) && registeredsAccts.length >= 50){ 
            ...
            if(block.timestamp > lastAutoSpin + 605100 && reset == false){   // if no node activates in 5 minutes period queue resets
                ...
                reset == true;
            } else if(lbNodes.updateNodeInfoB(_caller,botReward,1) == true){
    ...
    //Register Tickets
    function registerTickets(uint256 quantity,address user,uint256 amountInUSD,uint256 amountInToken) external nonReentrant {
       ...
       if(usd.balanceOf(address(this)) >= amountInUSD){
        
        swapTokens(amountInToken);
        prizeValueJackpot += amountInToken;

       } else{
        
        uint256 tprice = getLatestPrice();

        uint256 newTokenAmount = ((usd.balanceOf(address(this)) * 10**18 / tprice) * 10**18) / 10**10;

        swapTokens(newTokenAmount);
        prizeValueJackpot += newTokenAmount;
    ...
    function getLastUserTicket(uint256 drawId,address userWallet) internal returns(uint256) {
    ...
    function ForfeitTicket(address user) external nonReentrant {
        ...
        if(clearUserTicket(user,requestCounter,ticketId)){
