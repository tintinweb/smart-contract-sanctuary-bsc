...
contract RunTogetherMarketplaceAdmin is Ownable, ReentrancyGuard, Pausable {
    ...
    function initNftsInMarket() external nonReentrant onlyOwner {
        uint256 _to = 0;
        if (maxNftInTransfer <= ERC721(runTogetherBoxNft).balanceOf(address(this))) {
            _to = maxNftInTransfer;
        } else {
            _to = ERC721(runTogetherBoxNft).balanceOf(address(this));
        }
        for (uint256 index = totalNfts; index < _to; index++) {
