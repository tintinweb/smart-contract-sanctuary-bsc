...
contract Auction is ReentrancyGuard, Ownable {
    ...
    constructor(IERC721 _ylnft721, IERC1155 _ylnft1155, address _marketplaceContract) {
        ylnft721 = _ylnft721;
    ...
    function withdrawBid(uint256 _auctionId, bool _isERC721) public nonReentrant {
        ...
        if(idToAuctionItem[_auctionId].owner == msg.sender) {
            bool isTransferred = marketplaceContract.ylt20().transferFrom(msg.sender, address(this), marketplaceContract.marketfee());
            ...
            bool isTransferred = marketplaceContract.ylt20().transferFrom(msg.sender, address(this), idToAuctionItem[_auctionId].highestBid + marketplaceContract.marketfee());
            require(isTransferred, "Insufficient Fund.");
            if(_isERC721)
                ylnft721.transferFrom(address(this), msg.sender, idToAuctionItem[_auctionId].tokenId);
            else 
                ylnft1155.safeTransferFrom(address(this), msg.sender, idToAuctionItem[_auctionId].tokenId, idToAuctionItem[_auctionId].amount, "");
            (bool sent) = marketplaceContract.ylt20().transferFrom(address(this), idToAuctionItem[_auctionId].owner, idToAuctionItem[_auctionId].highestBid);
    ...
    function withdrawNFTInstant(uint256 _auctionId, bool _isERC721) public nonReentrant {
        require((ylnft721.ownerOf(idToAuctionItem[_auctionId].tokenId) == address(this)) || ylnft1155.balanceOf(address(this), idToAuctionItem[_auctionId].tokenId) >= idToAuctionItem[_auctionId].amount, "This token don't exist in market.");
        bool isTransferred = marketplaceContract.ylt20().transferFrom(msg.sender, address(this), idToAuctionItem[_auctionId].highestBid + marketplaceContract.marketfee());
        require(isTransferred, "Insufficient Fund.");
        if(_isERC721)
...
contract YLNFTMarketplace is ERC1155Holder, ReentrancyGuard, Ownable {
    ...
    function directTransferToBuyer(address _from, uint256 _tokenId, uint256 _price, uint256 _amount, bool _isERC721) public nonReentrant {
        ...
        if(_isERC721){
            ...
            isTransferred = ylt20.transferFrom(address(this), _from, _price);
            ...
            (isTransferred) = ylt20.transferFrom(address(this), _from, _price * _amount);
            require(isTransferred, "Failed to send token");
    ...
    function adminTransfer(address _to, uint256 _itemId, uint256 _amount, bool _isERC721) public ylOwners {
        ...
        (isTransferred) = ylt20.transferFrom(address(this), idToMarketItem[_itemId].seller, idToMarketItem[_itemId].price);
        require(isTransferred, "Failed to send token");
        if(_isERC721){
    ...
    function MarketItemSale(uint256 itemId, uint256 _amount, bool _isERC721) public nonReentrant returns(uint256) {
        ...
        (isTransferred) = ylt20.transferFrom(address(this), idToMarketItem[itemId].seller, idToMarketItem[itemId].price);
    ...
    function withdrawToken(uint256 _amount) public ylOwners nonReentrant {
        ...
        (bool sent) = ylt20.transferFrom(address(this), msg.sender, _amount);
