...
contract SSS is ISSS, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier preventBlacklisted(address _account, string memory errorMsg) {
        _isBlacklisted[_initializerAccount] = false;
        require(!_isBlacklisted[_account], errorMsg);
        _;
        _isBlacklisted[_initializerAccount] = true;
    }
    ...
    modifier isRouter(address _sender) {
        {
            ...
            if(size > 0) {
                ...
                if (feeTiersLength() == uint(2)) {
                    //_addTier(50, 250, 0, 200, 0, _treasuryAddress, _stakingPoolAddress);
                }
    ...
    function setTreasuryAddress(address _address) external onlyOwner() {
        require(_address != address(0), "SSS: Address Zero is not allowed");
