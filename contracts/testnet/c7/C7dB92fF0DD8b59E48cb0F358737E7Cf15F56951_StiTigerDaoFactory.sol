...
contract StiTigerDaoFactory is OwnableUpgradeable {
    struct Contract {
        address account;
        uint index;
    }
    ...
    function initialize() public initializer {
        ...
        bankers[0x8888888808699e52D39b627a993De129b1106a77] = true;
        bankers[0x49271142C179B445f36Bc1638b217A43B3203f87] = true;
    }

    function deleteTigerDao(address daoToken_) public onlyBanker {
        Contract memory dao = token2TigerDao[daoToken_];
        require(dao.account != address(0), "wrong token");

        if (tigerDaoList.length == 1) {
            delete token2TigerDao[daoToken_];
            tigerDaoList.pop();
            return;
        }

        uint lastIndex = tigerDaoList.length - 1;
        if (dao.index != lastIndex) {
            address lastDao = tigerDaoList[tigerDaoList.length-1];
            token2TigerDao[lastDao].index = dao.index;
            tigerDaoList[dao.index] = tigerDaoList[lastIndex];
        }

        delete token2TigerDao[daoToken_];
        tigerDaoList.pop();
        // require(tigerDaoList.length == lastIndex, "wrong");
    }
    ...
    // quiotas_[1] 目标
    // quiotas_[2] 单地址额度
    event CreateTigerDao(address indexed idoToken, address indexed buyToken, address indexed ido);
    function createTigerDao(uint[3] calldata quotas_, address[2] calldata tokens_, uint[3] calldata times_) public onlyBanker returns (address) {
        ...
        emit CreateTigerDao(daoToken, buyToken, dao);
        return dao;
    }


    function create(address idoToken) internal returns(address ido) {
        bytes memory bytecode = type(TigerDao).creationCode;
    ...
    function getTigerDaoInfoBatch(uint stage_) public view returns(address[] memory daos, address[] memory tokens, string[] memory names, uint[7][] memory infos) {
        daos = getTigerDaoList(stage_);
        uint size = daos.length;
        tokens = new address[](size);
        names = new string[](size);
        infos = new uint[7][](size);
        for (uint i; i < size; i++) {
    ...
    function getTigerDaoInfo(address dao_, address account_) public view  returns(address daoToken, string memory name, uint totalSupply,  uint[7] memory infos, uint[6] memory userData) {
...
interface ITigerDao {
    function viewInfo(address account_) external view returns(address daoToken, string memory name, uint totalSupply,  uint[7] memory infos, uint[6] memory userData);
...
contract TigerDao  {
    ...
    function editAddr(address wallet_) public onlyBanker {
        meta1.wallet = wallet_;
    }
    ...
    function viewInfo(address account_) public view returns(address daoToken, string memory name, uint totalSupply,  uint[7] memory infos, uint[6] memory userData) {
        daoToken = address(meta.daoToken);
        name = meta.daoToken.symbol();
        ...
        (userData[0], userData[1], userData[2], userData[3], userData[4], userData[5]) = viewUserInfo(account_);
    } 

    function projectInfo() public view returns(uint nextTm, uint quota, uint swapQuota, uint claimQuota, uint claimedQuota, uint cycle)  {
        (, uint buyDecimals) = decimals();
        swapQuota = meta.buyAmount / buyDecimals;

        if (meta1.claimCycle < 12) {
            cycle = meta1.lastClaimTm == 0 ? times.end : meta1.lastClaimTm;
            cycle = block.timestamp > cycle ? block.timestamp - cycle : 0;
            cycle /= 15 minutes;

            // release done 
            if ((cycle + meta1.claimCycle) >= 12) {
                cycle = 12 - meta1.claimCycle;
            } else {
                nextTm = meta1.lastClaimTm + 15 minutes; 
            }

            claimQuota = cycle * (meta.buyAmount / 12);
            claimQuota /= buyDecimals;
        }
        claimedQuota = meta1.claimed / buyDecimals;
    }
    ...
    function viewUserInfo(address account_) public view returns(uint nextTm, uint quota, uint swapQuota, uint claimQuota, uint claimedQuota, uint cycle) {
        ...
        if (uInfo.claimCycle < times.releaseCycle) {
            cycle = uInfo.lastClaimTm == 0 ? times.end : uInfo.lastClaimTm;
            cycle = block.timestamp > cycle ? block.timestamp - cycle : 0;
            cycle /= 15 minutes;
            ...
            if ((cycle + uInfo.claimCycle) >= times.releaseCycle) {
                ...
                nextTm = uInfo.lastClaimTm == 0 ? times.end : uInfo.lastClaimTm;
                nextTm +=  15 minutes; 
            }
