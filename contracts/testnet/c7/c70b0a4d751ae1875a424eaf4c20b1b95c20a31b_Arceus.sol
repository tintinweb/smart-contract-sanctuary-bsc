...
contract Arceus is ERC20Detailed, Ownable {
    ...
    uint256 public CURRENT_APY = 400000; //Percentage /
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(arceusInsuranceFundFee).add(
            firePitFee
        );
    ...
    function rebase() internal {
        ...
        uint256 APY = CURRENT_APY;
        uint256 DAILY_RATE = APY.div(364);
        uint256 RATE_MINUTE = 56;
        uint256 EPOCH_RATE = DAILY_RATE.div(RATE_MINUTE);

        for (uint256 i = 0; i < times; i++) {
            _totalSupply = _totalSupply
                .mul((10**RATE_DECIMALS).add(EPOCH_RATE))
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(arceusInsuranceFundFee))
        );
    ...
    uint256 private percentageDenominator = 1000;
    uint256 private supplyLimitForRewards = 40;
    bool private allowRewrads = true;
    //when claimPercentage = 1 and percentageDenominator = 1000;
    // 50 * 1 / 1000 = 0.05 || 50 * 1 / 100 = 0.5
    ...
    function claimTokens() external returns (string memory State) {
        ...
        uint256 getRewardsSupply = MAX_SUPPLY.mul(supplyLimitForRewards).div(percentageDenominator);
    ...
    function nextClaimAmount(address account) public view returns (uint256) {
        ...
        uint256 Nvalue = Balance.mul(claimPercentage).div(percentageDenominator);
    ...
    function setClaimPerc(uint256 _percentage, uint256 _percentageDenominator) external onlyOwner(){
    ...
    function getAcountRewardEarning(address account) external view returns(uint256){
        return (_EarnedFromReward[account]);
    }

    function getRewradsState() view external returns(bool){
        return allowRewrads;
    }
    ...
    function _swapEnabled(bool _flag) external onlyOwner returns(bool success){
        swapEnabled = _flag;
        return true;
    }
    ...
    function manualRebase() external returns(bool success){
        if (shouldRebase()) {
           rebase();
        }
        return true;
    ...
    function getCurrentEpochRate() external view returns(uint256 _RATE, uint256 _MINUTE, uint256 _DAY_RATE, uint256 _CURRENT_APY){
        uint256 APY = CURRENT_APY;
        uint256 DAILY_RATE = APY.div(364);
        uint256 RATE_MINUTE = 56;
        uint256 EPOCH_RATE = DAILY_RATE.div(RATE_MINUTE);
        return (EPOCH_RATE, RATE_MINUTE, DAILY_RATE, APY);
    }

    function setNewAPY(uint256 _newAPY) external onlyOwner{
        CURRENT_APY = _newAPY;
    }
}