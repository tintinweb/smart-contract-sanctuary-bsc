...
contract HashForceCloudMining is Ownable {
    ...
    uint8 public amountOfMiningMachinesAvailable;
    uint8 public amountOfMiningMachinesTotal;
    ...
    mapping(address => mapping(uint256 => uint256[])) public accountDetail;
    ...
    function deposit(uint8 _contractDay, uint256 _price, uint8 _hashRateType) external
    {
        ...
        busdToken.transferFrom(msg.sender, address(this), totalPrice * 10**18);
        ...
        accountDetail[msg.sender][depositRoundForTheAccount[msg.sender]] = [_hashRateType, _contractDay, totalPrice, timeNow, timeEnd];

        emit Deposit(msg.sender, depositRoundForTheAccount[msg.sender], _contractDay, totalPrice, _hashRateType);
    }

    function withdraw(uint8 _round, uint256 _amount) external
    {
        ...
        require(busdToken.balanceOf(address(this)) >= _amount * 10**18, "This contract amount exceeds balance");

        busdToken.transfer(msg.sender, _amount * 10**18);
    ...
    function claimReward(uint8 _round, uint256 _amount) external
    {
        ...
        require(_amount > minimumClaim, "Not enough amount");
    ...
    function transferBusdTokenFromContract(address  _to, uint256 _amount) external 
    {
        ...
        busdToken.transfer(_to, _amount * 10**18);
    }
    ...
    function setAmountOfMiningMachinesTotal(uint8 _amountTotal, uint8 _amountAvailable) external onlyOwner
    {
    ...
    function setMinimumClaim(uint256 _minimumClaim) external onlyOwner
    {
