...
/**
 *Submitted for verification at BscScan.com on 2022-05-05-10
*/
...
contract MREUPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MREUP_Dividend_Tracker", "MREUP_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**9); //must hold 1000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MREUP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MREUP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MREUP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MREUP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "MREUP_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MREUP is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MREUPDividendTracker public dividendTracker;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        ...
        require(totalFees <= 10, "sell fee can not be more than 10%");
    }
    ...
    function setSellFee(uint256 _onSellbnbRewardFee, uint256 _onSellliquidityFee, uint256 _onSellMarketingFee) public onlyOwner {
        ...
        require(onSelltotalFees <= 10, "sell fee can not be more than 10%");
    }
    ...
    constructor() ERC20("Movil Reup", "MREUP") {
        BNBRewardsFee = 2;
        liquidityFee = 3;
        marketingFee = 3;

        onSellBNBRewardsFee = 2;
        onSellliquidityFee = 3;
        onSellmarketingFee = 3;

        marketingWallet = payable(0x923B16C39c8ce22d701D7aF283F1f28ba4c973Db);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee);

        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee);

    	dividendTracker = new MREUPDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MREUP: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MREUP: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MREUP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MREUP: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MREUP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MREUP: Cannot update gasForProcessing to same value");
