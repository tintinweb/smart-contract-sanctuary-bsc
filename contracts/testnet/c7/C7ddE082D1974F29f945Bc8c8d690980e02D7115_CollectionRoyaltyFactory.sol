...
contract CollectionRoyaltyFactory is Ownable {
    // events
    ...
    modifier onlyIfAuthorized {
        require(authorized[msg.sender],'Address Not Whitelisted');
        _;
    }
   ...
   constructor(){
    ...
    function authorizeAddress(address newUser) external
        onlyIfAuthorized
    ...
    function revokeAccess(address removeUser) external
        onlyIfAuthorized
    {
        require(removeUser != owner(),'Owner Address');
    ...
    */
    function deployCollectionRoyalty(address _owner, string memory _name, string memory _symbol, string memory _baseURI,
        uint96 _baseRoyalty, address _treasury) external onlyIfAuthorized {

        CollectionRoyalty collectionRoyalty = new CollectionRoyalty(_name, _symbol);
