...
contract Unisynths is Ownable, ReentrancyGuard {
    ...
    uint256 public sdrUSD = 57813 * 10**13;
    uint256 public sdrCNY = 109930 * 10**13;
    uint256 public sdrYEN = 134520 * 10**13;
    uint256 public sdrGBP = 8087 * 10**13;
    uint256 public sdrEUR = 37379 * 10**13;

    uint256 public goldWeight = 40;
    uint256 public btcWeight = 60;
    uint256 btcMonthPrice;
    uint256 xauMonthPrice; 
    uint256 public snapshotBold;
    uint256 percentageBTC = btcMonthPrice.mul(25).div(100);
    uint256 percentageXAU = xauMonthPrice.mul(25).div(100);
    ...
    event MintToken(address indexed user, string indexed token,uint256 amount, uint256 indexed price);
    event BurnToken(address indexed user, string indexed token,uint256 amount, uint256 indexed price);
   ...
   function bytes32ToString(bytes32 _bytes32) internal pure returns (string memory) {
     ...
     */
    function getLatestPrice(string memory _ticker) public view returns (uint256) {

        bytes32 ticker = stringToBytes32(_ticker);

        AggregatorV3Interface priceFeed = AggregatorV3Interface(tokens[ticker].priceFeed);
    ...
    function updateBold() internal {

        // define this month weights
        if(block.timestamp >= snapshotBold + 2628000){      // 1 month


            if(getLatestPrice("kBTC") > btcMonthPrice.add(percentageBTC)){
            ...
            } else if(getLatestPrice("kBTC") < btcMonthPrice.sub(percentageBTC)){
            ...
            } else if(getLatestPrice("kXAU") > xauMonthPrice.add(percentageXAU)){
            ...
            } else if(getLatestPrice("kXAU") < xauMonthPrice.sub(percentageXAU)){
        ...
        xauMonthPrice = getLatestPrice("kXAU");
        btcMonthPrice = getLatestPrice("kBTC");
        snapshotBold = block.timestamp;
    ...
    function getBoldPrice() public view returns (uint256) {
      
        string memory _kBTC = "kBTC";
        string memory _kXAU = "kXAU";

        uint256 btcprice = getLatestPrice(_kBTC); // kBTC
        uint256 goldprice = getLatestPrice(_kXAU);    // kXAU

        uint256 sharebtc = btcprice.mul(btcWeight).div(100);
        uint256 sharegold = goldprice.mul(goldWeight).div(100);
    ...
    function getSdrPrice() public view returns (uint256) {

        string memory _kCNY = "kCNY";
        string memory _kYEN = "kYEN";
        string memory _kGBP = "kGBP";
        string memory _kEUR = "kEUR";
    ...
    function getPrivPrice() public view returns (uint256) {

        string memory _kXMR = "kXMR";
        string memory _kZCASH = "kZCASH";
        string memory _kDASH = "kDASH";
        string memory _kLTC = "kLTC";

        uint256 xmrprice = getLatestPrice(_kXMR);
        uint256 zcprice = getLatestPrice(_kZCASH);   
        uint256 dashprice = getLatestPrice(_kDASH);   
        uint256 ltcprice = getLatestPrice(_kLTC); 

        uint256 xmramountprice = xmrprice.mul(40).div(100);
        uint256 zcamountprice = zcprice.mul(30).div(100);
        uint256 dashamountprice = dashprice.mul(20).div(100);
        uint256 ltcamountprice = ltcprice.mul(10).div(100);


        uint256 synthPrice = xmramountprice + zcamountprice +dashamountprice + ltcamountprice;

        return synthPrice;
    } 

    function getMuskPrice() public view returns (uint256) {

        string memory _kTSLA = "kTSLA";
        string memory _kDOGE = "kDOGE";
        string memory _kBTC = "kBTC";
        string memory _kETH = "kETH";

        uint256 tslaprice = getLatestPrice(_kTSLA);
        uint256 dogeprice = getLatestPrice(_kDOGE);   
        uint256 btcprice = getLatestPrice(_kBTC);   
        uint256 ethprice = getLatestPrice(_kETH); 

        uint256 tslaamountprice = tslaprice.mul(35).div(100);
        uint256 dogeamountprice = dogeprice.mul(35).div(100);
        uint256 btcamountprice = btcprice.mul(20).div(100);
        uint256 ethamountprice = ethprice.mul(10).div(100);


        uint256 synthPrice = tslaamountprice + dogeamountprice + btcamountprice + ethamountprice;
    ...
    function updateSustainFee() public {
        
        // For each 10% closer of collateral below 200% overcollaterization we grow fee 1% to burn synths with a total of 10% fee

        if(getTotalSupply() >= getTotalMinted().mul(190).div(100)){
            currentFee = 100;
        } else if (getTotalSupply() >= getTotalMinted().mul(180).div(100)){
            currentFee = 200;
        } else if (getTotalSupply() >= getTotalMinted().mul(170).div(100)){
            currentFee = 300;
        } else if (getTotalSupply() >= getTotalMinted().mul(160).div(100)){
            currentFee = 400;
        } else if (getTotalSupply() >= getTotalMinted().mul(150).div(100)){
            currentFee = 500;
        } else if (getTotalSupply() >= getTotalMinted().mul(140).div(100)){
            currentFee = 600;
        } else if (getTotalSupply() >= getTotalMinted().mul(130).div(100)){
            currentFee = 700;
        } else if (getTotalSupply() >= getTotalMinted().mul(120).div(100)){
            currentFee = 800;
        } else if (getTotalSupply() >= getTotalMinted().mul(110).div(100)){
            currentFee = 900;
        } else if (getTotalSupply() <= getTotalMinted()){ 
            currentFee = 1000;
        }
    ...
    function mintSynth(string memory _ticker,uint256 _amount) tokenExist(_ticker) nonReentrant public {
        bytes32 ticker = stringToBytes32(_ticker);
        ...
        string memory kBOLD = "kBOLD";
        string memory kSDR = "kSDR";
        string memory kPRIV = "kPRIV";
        string memory kMUSK = "kMUSK";
        ...
        if(usd.balanceOf(address(this)) > getTotalMinted().mul(5) && usd.balanceOf(address(this)) > 6000 * 10**18){
        ...
        if(keccak256(abi.encodePacked(_ticker)) == keccak256(abi.encodePacked(kBOLD))){
            
            synthPrice = getBoldPrice();

        } else if (keccak256(abi.encodePacked(_ticker)) == keccak256(abi.encodePacked(kSDR))){
            
            synthPrice = getSdrPrice();

        } else if (keccak256(abi.encodePacked(_ticker)) == keccak256(abi.encodePacked(kPRIV))){

             synthPrice = getPrivPrice();

        } else if (keccak256(abi.encodePacked(_ticker)) == keccak256(abi.encodePacked(kMUSK))){

            synthPrice = getMuskPrice();
        ...
        EIP20(tokens[ticker].tokenAddress).mint(msg.sender, amountMint);

        updateSustainFee();

        updateBold();

        emit MintToken(msg.sender, _ticker, amountMint, synthPrice);
    }
    
    function burnSynth(string memory _ticker,uint256 _amount) tokenExist(_ticker) nonReentrant public {
        bytes32 ticker = stringToBytes32(_ticker);
        require(EIP20(tokens[ticker].tokenAddress).balanceOf(msg.sender) >= _amount , "You don't have enough tokens to burn/sell.");      
        ...
        if(krstm.balanceOf(msg.sender) < 5 * 10**18){
            // charge sustainability fee
            uint256 chargedFee = amountToSend.mul(currentFee).div(10000); 
            amountToSend = amountToSend.sub(chargedFee);  
        }
        ...
        } else if(usd.balanceOf(address(this)) > getTotalMinted().mul(5) && usd.balanceOf(address(this)) > 6000 * 10**18){
        ...
        // burn synth
        EIP20(tokens[ticker].tokenAddress).burnFrom(msg.sender, _amount);
        
        updateSustainFee();

        updateBold();

        emit BurnToken(msg.sender, _ticker, _amount, synthPrice);
    ...
    /// @return wei balance of all tokens supply
        ...
        // Return reserve balance in usd
          Token[] memory _tokens = new Token[](tokenList.length);
            for (uint i = 0; i < tokenList.length; i++) {
                ...
                string memory convertedTicker = bytes32ToString(tokens[tokenList[i]].ticker);
                uint256 totalsupply = EIP20(tokens[tokenList[i]].tokenAddress).totalSupply();
                
                if(keccak256(abi.encodePacked(convertedTicker)) == keccak256(abi.encodePacked(kBOLD))){
                    
                    tokenprice = getBoldPrice();

                } else if (keccak256(abi.encodePacked(convertedTicker)) == keccak256(abi.encodePacked(kSDR))){
                                        
                    tokenprice = getSdrPrice();

                } else if (keccak256(abi.encodePacked(convertedTicker)) == keccak256(abi.encodePacked(kPRIV))){

                    tokenprice = getPrivPrice();

                } else if (keccak256(abi.encodePacked(convertedTicker)) == keccak256(abi.encodePacked(kMUSK))){

                    tokenprice = getMuskPrice();

                } else {

                    tokenprice = getLatestPrice(convertedTicker);
    ...
    function previewMintAmount(string memory _ticker, uint256 _amountUSD) public view returns (uint256) {      
    ...
    function previewReturnOnBurn(string memory _ticker, uint256 _amountTKN) public view returns (uint256) {
