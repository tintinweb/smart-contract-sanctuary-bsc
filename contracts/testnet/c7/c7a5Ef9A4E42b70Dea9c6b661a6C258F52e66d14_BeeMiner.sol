...
contract BeeMiner is Claimable {
    ...
    uint256 public totalWithdrawedTokens;
    ...
    function withdraw(uint256 amount) public {
        ...
        require(amount > 0, "You can't withdraw more than 180% of the total deposit + ambassador bonus");
        ...
        totalWithdrawedTokens = totalWithdrawedTokens.add(amount);
    ...
    function addLiquidity(uint256 bnbAmount, bool createLiquidity, int256 additionalPercent) external onlyOwner {
      uint256 amount = tokensAmount(bnbAmount);
      if (additionalPercent != 0) {
        amount = amount.mul(uint256(100 + additionalPercent)).div(100);
      }
    ...
    function availableHoneyToWithdraw(address user) external view returns (uint256) {
      Player memory player = players[user];
      return tokensAmount((player.totalDeposited.add(ambassadorBonus[msg.sender])).mul(18).div(10).sub(player.totalWithdrawed));
    }
