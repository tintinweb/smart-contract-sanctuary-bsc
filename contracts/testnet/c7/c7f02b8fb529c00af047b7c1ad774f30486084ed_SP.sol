...
contract SP is Context, IERC20, Ownable {
    ...
    string private _name = "SP ST";
    string private _symbol = "SP";
    uint8 private _decimals = 18;

    uint256 public _buyHolderFee = 0;
    uint256 public _buyNoonFee = 0;

    uint256 public _sellTaxFee = 0;
    uint256 public _sellNoonFee = 5;

    uint256 private _holderFee = _buyHolderFee;
    uint256 private _previousTaxFee = _holderFee;

    uint256 public _noonFee = _buyNoonFee;
    uint256 private _previousNoonFee = _noonFee;

    address public noonWallet = 0xfE97B8485a07a3ab14330631D75B237C4802CFD7;
    ...
    function changeNoonWallets(address wallet) public onlyOwner {
        noonWallet = wallet;
    }
    ...
    function excludeFromFoundation(address account) public onlyOwner {
    ...
    function includeInFoundation(address account) public onlyOwner {
    ...
    function setNoonPercentFee(uint256 buyNoonFee, uint256 sellNoonFee)
        external
        onlyOwner
    {
        _sellNoonFee = sellNoonFee;
        _buyNoonFee = buyNoonFee;
    }
    ...
    function _takeAdvertisement(uint256 tAdvertisement) private {
        uint256 currentRate = _getRate();
        uint256 rAdvertisement = tAdvertisement.mul(currentRate);
        _rOwned[noonWallet] = _rOwned[noonWallet].add(rAdvertisement);
        if (_isExcluded[noonWallet])
            _tOwned[noonWallet] = _tOwned[noonWallet].add(tAdvertisement);
    }
    ...
    function calculateAdvestisementFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(_noonFee).div(10**2);
    }
    ...
    function removeAllFee() private {
        if (_holderFee == 0 && _noonFee == 0 && _burnFee == 0) return;

        _previousTaxFee = _holderFee;
        _previousNoonFee = _noonFee;
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) {
            ...
            if (isBuy) {
                ...
                _noonFee = _buyNoonFee;
            } else if (isSell) {
                _holderFee = _sellTaxFee;
                _noonFee = _sellNoonFee;
            }
        }
