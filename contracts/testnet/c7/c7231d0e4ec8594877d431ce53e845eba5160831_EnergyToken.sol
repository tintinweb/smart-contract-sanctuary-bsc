...
contract EnergyToken is IBEP20, LockToken
{
    ...
    mapping(address => bool) private _excludeFromWhale;

    function excludeWalletsFromWhales() private 
    {
        _excludeFromWhale[owner()]=true;
        _excludeFromWhale[address(this)]=true;
        _excludeFromWhale[address(0)]=true;
    }
    ...
    private view
    {
        ...
        if(!_excludeFromWhale[from] && !_excludeFromWhale[to]) 
        { 
            require(newBalance <= _walletMaxHoldingLimit, "Exceeding max tokens limit in the wallet"); 
        } 
        if(from==poolPair && !_excludeFromWhale[to]) 
        { 
            require(newBalance <= _walletMaxHoldingLimit, "Exceeding max tokens limit in the wallet"); 
        } 
    }
    ...
    {
        _excludeFromWhale[_account] = _value; // true for excluded, false for not excluded. 
    }
