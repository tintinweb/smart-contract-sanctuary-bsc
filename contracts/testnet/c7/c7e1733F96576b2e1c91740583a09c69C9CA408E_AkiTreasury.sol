...
import "./AkiRewardSplitter.sol";
// import "hardhat/console.sol";

contract AkiTreasury is Ownable {
  struct RewardContractInfo {
    address addr;
    uint256 shares;
    address nftAddress;
  }
  RewardContractInfo[] public rewardContracts_;
  uint256 treasuryShares_;
  mapping(address => bool) isWhiteListed;

  event TreasuryShares(uint256 indexed shares);
  event RewardInfo(uint256 indexed idx, address indexed addr, uint256 shares, address indexed nftAddress);
  ...
  function setTreasuryShares(
    uint256 shares
  ) public onlyOwner {
    treasuryShares_ = shares; 
    emit TreasuryShares(shares);
  }
  ...
  function setRewardInfo(
    ...
    while (rewardContracts_.length < idx + 1) {
      rewardContracts_.push();
    }
    rewardContracts_[idx].addr = addr;
    rewardContracts_[idx].shares = shares;
    rewardContracts_[idx].nftAddress = nftAddress;
    emit RewardInfo(idx, addr, shares, nftAddress);
  }
  ...
  function addPayment(
    ...
    require(block.timestamp >= rewardStartTime && rewardEndTime > rewardStartTime, "misconfigured reward period");
    require(isWhiteListed[address(token)], "the token should be whitelisted");
    ...
    uint value = IERC20(token).balanceOf(address(this)) - balanceBefore;

    for (uint256 i = 0; i < rewardContracts_.length; i++) {
      if (rewardContracts_[i].shares != 0) {
        uint256 currentAmount = rewardContracts_[i].shares * value / totalShares;
        // console.log("show addr", rewardContracts_[i].shares, amount, totalShares);
        AkiRewardSplitter splitter = AkiRewardSplitter(rewardContracts_[i].addr);
        SafeERC20.safeApprove(token, rewardContracts_[i].addr, currentAmount);
        splitter.addPayment(token, currentAmount, rewardContracts_[i].nftAddress, rewardStartTime, rewardEndTime);
        totalSent += currentAmount;
      }
    }

    // remaining will be sent to the owner address
    SafeERC20.safeApprove(token, address(this), amount - totalSent);
    SafeERC20.safeTransferFrom(token, address(this), owner(), amount - totalSent);
    emit AddPayment(address(token), amount, rewardStartTime, rewardEndTime);
  }

  function setWhiteListToken(address token) public onlyOwner {
...
contract AkiRewardSplitter is Ownable {
  mapping(address => mapping(address => uint64)) public nftToPayeeToShares;
  PaymentEnvelope[] public envelopes;
  ...
  function setTreasuryAddress(address addr) public onlyOwner {
    require(address(0) != addr, "Cannot be zero address!");
  ...
  function setTokenLevelInfo(
    ...
    for (uint32 i = 0; i < payees.length; i++) {
      uint256 prev = nftToPayeeToShares[nftAddress][payees[i]];
      nftToPayeeToShares[nftAddress][payees[i]] = shares[i];
      uint256 delta = shares[i] - prev;

      for (uint32 envIdx = 0; envIdx < envelopes.length; envIdx++) {
        PaymentEnvelope storage env = envelopes[envIdx];
        if (address(env.nftAddress) != nftAddress) {
          continue;
        }
        uint256 currentShare = env.payeeToShares[payees[i]];
        if (currentShare != 0) {
          env.payeeToShares[payees[i]] += delta;
          env.totalShares += delta;
          // console.log(nftAddress);
          // console.log("show total share %d", env.totalShares);
        }
      }
    }
