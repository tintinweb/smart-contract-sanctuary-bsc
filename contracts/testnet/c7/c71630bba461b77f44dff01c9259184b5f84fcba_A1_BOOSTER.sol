...
contract A1_BOOSTER {
    ...
    uint256 private constant diamond = 100;
    uint256 private constant doubleDiamond = 100;
    ...
    function withdraw() external {
        ...
        if (a > userInfo[msg.sender].splitLimit )
      
        { 
              if(userInfo[msg.sender].splitLimit==0)
              {
                b=a;
                }
                else
           {   
               b=  (userInfo[msg.sender].splitLimit); 
           }  
        }
        else {
            b= a;       
        ...
        userRewards.split = userRewards.split.add(b);
    ...
    function getCurSplit(address _user) public view returns(uint256){
      ...
      uint a = (staticSplit).add(dynamicSplit).sub(rewardInfo[_user].splitDebt);
      
        
         if (a > userInfo[_user].splitLimit )
        { 
            if (userInfo[_user].splitLimit==0){
             rewardInfo[_user].split.add(a);  
            }
            else{
            rewardInfo[_user].split.add(userInfo[_user].splitLimit);   
            }      
        }
        else {
             rewardInfo[_user].split.add(a);        
        }
        return rewardInfo[_user].split;
    }
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        user.totalFreezed = user.totalFreezed.add(_amount);
         FlushRecords[msg.sender].remainingFlush=0;
        user.totalreward = _amount*2100/1000;
        user.splitLimit= _amount*90/100;
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        uint256 luck = _amount.mul(diamond).div(baseDivider);
        luckPool = luckPool.add(luck);
        uint256 star = _amount.mul(doubleDiamond).div(baseDivider);
