 ...
 */
contract JuniorStakingPoolTest is Ownable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 totalDeservedReward,uint256 referralBonus, uint256 finalyPendingReward) {
		...
		for (uint i = 0; i < transactionInfos.length; i++) {
			if(transactionInfos[i].lpToken == pool.lpToken && transactionInfos[i].user == _user){
				uint256 deservedReward = transactionInfos[i].amount.mul(getBoostMultiplier(i).mul(accTokenPerShare)).div(1e12).sub(transactionInfos[i].claimedReward);
   ...
   // 刷新指定池子的信息
    ...
    // 获取未领取的奖励代币
    function getPendingReward(uint256 _pid, address _user) private returns (uint256 totalDeservedReward, uint256 referralBonus, uint256 finalyPendingReward) {
        PoolInfo storage pool = poolInfo[_pid];
		updatePool(_pid);
        for (uint i = 0; i < transactionInfos.length; i++) {
            if(transactionInfos[i].lpToken == pool.lpToken && transactionInfos[i].user == _user){
				transactionInfos[i].deservedReward = transactionInfos[i].amount.mul(getBoostMultiplier(i).mul(pool.accTokenPerShare)).div(1e12).sub(transactionInfos[i].claimedReward);
                transactionInfos[i].claimedReward = transactionInfos[i].claimedReward.add(transactionInfos[i].deservedReward);
