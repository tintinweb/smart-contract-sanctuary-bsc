...
contract FOCCER is IERC20, Ownable {
    using Address for address;
    
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "FOCCER TOKEN";
    string constant _symbol = "FOCCER";
    ...
    uint256 FOCCERAIDFee = 6;
    uint256 liquidityFee = 1;
    uint256 totalFee = FOCCERAIDFee + liquidityFee;
    uint256 sellBias = 0;
    uint256 feeDenominator = 100;

    address payable public liquidityFeeReceiver = payable(0x70701D2a1daC773382EBa3a47210855a5deb9b73);
    address payable public FOCCERAIDFeeReceiver = payable(0x5C896dD79679bb7FD2D5a5d7f645E28c8d0d3Fb4);
    ...
    function swapBack(uint256 amount) internal swapping {
        ...
        uint256 amountBNBFOCCERAID = amountBNB - amountBNBLiquidity;
        
        if (amountBNBFOCCERAID > 0)
            FOCCERAIDFeeReceiver.transfer(amountBNBFOCCERAID);
        ...
        emit FundsDistributed(amountBNBFOCCERAID, amountBNBLiquidity, amountToLiquify);
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _FOCCERAIDFee, uint256 _sellBias, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        FOCCERAIDFee = _FOCCERAIDFee;
        totalFee = _FOCCERAIDFee + _liquidityFee;
        sellBias = _sellBias;
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator / 2);
    }

    function setFeeReceivers(address _liquidityFeeReceiver, address _FOCCERAIDFeeReceiver) external onlyOwner {
        liquidityFeeReceiver = payable(_liquidityFeeReceiver);
        FOCCERAIDFeeReceiver = payable(_FOCCERAIDFeeReceiver);
    }
    ...
    event FundsDistributed(uint256 FOCCERAIDBNB, uint256 liquidityBNB, uint256 liquidityTokens);
