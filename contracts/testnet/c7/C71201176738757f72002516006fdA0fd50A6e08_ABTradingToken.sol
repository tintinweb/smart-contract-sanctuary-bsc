...
contract ABTradingToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        sellTokenRewardsFee = sellTokenRewardsFee_;
        sellLiquidityFee = sellLiquidityFee_;

        require(buyLiquidityFee <= 25, "Total purchase cost exceeds 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee) <= 25, "Total sales expense exceeds 25%");
        ...
        uint256 tokenBalanceForDividendReward = tokenBalanceForReward_ * (10**18);
        dividendTracker = new TokenDividendTracker(rewardAddr_, tokenBalanceForDividendReward);
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                fees = amount.mul(buyLiquidityFee).div(100);
