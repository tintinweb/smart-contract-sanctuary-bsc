...
contract invite  is Ownable {
    ...
    event PrintUpdateValidInvite(address, uint, address, address, address, address);
  ...
  /*重启数据*/
   ...
   /*初始化*/
    ...
    function _UpdateValidInvite(InviteData storage data , address _address) private {
        ...
        oldRankAddress = data.school.getRankBefore(maxRank);
        
        //更新排行榜
        if(data.school._nextStudents[inviteAdress] == address(0)){
        ...
        //对比历史 50 150 是否有变换，有变化则结算掉出榜单数据数据
        if(oldRankAddress != address(0)){
            newTop1Address = data.school.getRankBefore(top1);
            newRankAddress = data.school.getRankBefore(maxRank);
        ...
        }else if(oldTop1Address != address(0)){
            newTop1Address = data.school.getRankBefore(top1);
            if(oldRankAddress != newRankAddress){
                UpdateEarning(oldRankAddress, LIMITWEIGHT);
            }
        ...
        if(selfRank > 0){
            ...
            }else if(selfRank <= maxRank){
                UpdateEarning(inviteAdress, MAXRANKWEIGHT);
            }
        }else{
            UpdateEarning(inviteAdress, LIMITWEIGHT);
        }
        emit PrintUpdateValidInvite(inviteAdress, selfRank, oldTop1Address, newTop1Address, oldRankAddress, newRankAddress);
    }
    ...
    function Earning() payable public payGas {
        //InviteData storage data = inData[roundId];
        ...
        userReward[msg.sender].takeUsdt = earnE.add(userReward[msg.sender].takeErc20);

        profit[usdtAddress] = earnU.add(profit[usdtAddress]);
        profit[xtokenAddress] = earnU.add(profit[xtokenAddress]);
    ...
    function countEarningV (address _address) public view returns (uint256, uint256, uint256, uint256){
        ...
        //计算收益
        (uint256 countU, uint256 countE) = _countEarningV(_address);
        totalU = userReward[_address].rewardUsdt.add(countU);
        totalE = userReward[_address].rewardErc20.add(countE);

        earnU = totalU.sub(userReward[_address].takeUsdt);
        earnE = totalE.sub(userReward[_address].takeErc20);
    ...
    function _countEarningV (address _address) private view returns (uint256, uint256){
        ...
        if(roundId != userReward[_address].markerRound){
            if(userReward[_address].weight != 0){
                countUsdt = data._resData.resBalanceUsdt
                    .sub(userReward[_address].markerUsdt)
                    .div(data._resData.allWeight)
                    .mul(userReward[_address].weight);
                countErc20 = data._resData.resBalanceErc20
                    .sub(userReward[_address].makerErc20)
                    .div(data._resData.allWeight)
                    .mul(userReward[_address].weight);
            }
        } 
        else if(userReward[_address].weight != 0 && data.school.listSize >= maxRank){
