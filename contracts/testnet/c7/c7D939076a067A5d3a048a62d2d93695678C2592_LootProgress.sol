...
contract LootProgress is Ownable{
    ...
    mapping (address =>uint256) _lastLootClaimable;
...
//===============================
    ...
    function unlockWeeklyLootForTest(uint256 _weekNb)external {
        _lastLootClaimable[msg.sender] = _weekNb;
        for(uint256 i =_weekNb ; i > 0 ; i--){
            _weeklyLootClaimed[msg.sender][i].claimable = true;
            _weeklyLootClaimed[msg.sender][i].claimed = false;
        }
    }
...
// TESTING Functions => to delete
    ...
    function _getUserProgress(address _user) internal view returns(uint256 lastActionTimestamp,uint256 dayOfWeek,uint256 weekNumber, uint256 lastLootClaimable){
        ...
        if(p.lastActionTimestamp == 0){
            ...
            return(p.lastActionTimestamp,p.dayOfWeek,p.weekNumber,_lastLootClaimable[_user]);
        }
        else if(_beginningDay - p.lastActionTimestamp <= 1 days){
            return(p.lastActionTimestamp,p.dayOfWeek,p.weekNumber,_lastLootClaimable[_user]);
        }
        else if(_beginningDay - p.lastActionTimestamp <= 7 days ){
            return(p.lastActionTimestamp,0,p.weekNumber,_lastLootClaimable[_user]);
        }
        else if(
                _beginningDay - p.lastActionTimestamp > 7 days &&
                _beginningDay - p.lastActionTimestamp < 14 days
                ){
            return(p.lastActionTimestamp,0,p.weekNumber > 1 ? p.weekNumber - 1 : 1,_lastLootClaimable[_user] );
        }
        else if(_beginningDay - p.lastActionTimestamp >= 14 days){
            return(p.lastActionTimestamp,0,1,_lastLootClaimable[_user]);
        }
            ...
            if(_beginningDay >= p.lastActionTimestamp){
                if(_beginningDay - p.lastActionTimestamp <= 1 days){
                    ...
                    if(p.dayOfWeek == 7){
                        ...
                        if(_lastLootClaimable[_user] < p.weekNumber){
                            _lastLootClaimable[_user] = p.weekNumber;
                        }
                        p.weekNumber += 1;
                        p.dayOfWeek = 1;
                    }
    ...
    function claimLoot(uint256 _weekNumber) external {
        ...
        if(_weekNumber > 1){
            require(_weeklyLootClaimed[msg.sender][_weekNumber - 1].claimed, "Previous week loot hasn't been claimed");
        }
        w.claimable = false;
        w.claimed = true;
    ...
    function _getPotionLoot(address _user, uint256 _numberOfPotions, uint256 _minLevel, uint256 _maxLevel) internal returns(uint256[4] memory potions){
        ...
        for(uint256 i = 0 ; i < _numberOfPotions ; i++){
            ...
            _potions[i] = I.mintMultiplePotion(_powers, _user);
        }
        return _potions;
    }
