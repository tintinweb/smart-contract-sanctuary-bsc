...
contract BuyHeroBirthEvent is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    using SafeMathUpgradeable for uint256;
    using BHeroDetails for BHeroDetails.Details;

    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    uint256 public constant LUCKY_WHEEL_NUM = 2;

    IBHeroToken public bHeroToken;
    IBHeroDesign public bHeroDesign;
    mapping (address => bool) public userUsedDiscount;

    event TokenCreateRequested(address to, uint256 block);

    function initialize(address bHeroTokenAddress, address bHeroDesignAddress ) public initializer {
        ...
        bHeroToken = IBHeroToken(bHeroTokenAddress);
        bHeroDesign = IBHeroDesign(bHeroDesignAddress);
    }
    ...
    function setBHeroToken(address bHeroTokenAddress) external onlyRole(MANAGER_ROLE) {
        bHeroToken = IBHeroToken(bHeroTokenAddress);
    }

    function setBHeroDesign(address bHeroDesignAddress) external onlyRole(MANAGER_ROLE){
        bHeroDesign = IBHeroDesign(bHeroDesignAddress);
    }

    function checkDiscountUser(address user) view public returns (bool) {
        return userUsedDiscount[user];
    }

    function getLuckyWheel(address user) view public returns (uint){
        if(userUsedDiscount[user]){
            return LUCKY_WHEEL_NUM;
        }
        return 0;
    }
    function buyHeroEvent(uint256 count, uint256 category) external {
        require(count == 15, "Number hero is not valid");
        require(category == 0 || category == 1, "Invalid category");

        address user = msg.sender;
        require(!userUsedDiscount[user], "User has used voucher");
        userUsedDiscount[user] = true;

        if (category == 0) {
            uint256 cost = bHeroDesign.getMintCostHeroS() * count;
            cost = (cost* 80)/(100);   //Discount 20%
            bHeroToken.buyHeroWithBcoin(user, cost);
        } else if (category == 1) {
            uint256 cost = bHeroDesign.getSenMintCostHeroS() * count;
            cost = (cost* 80)/(100);  //Discount 20%
            bHeroToken.buyHeroWithSen(user, cost);
        }

        // Check limit.
        require(bHeroToken.getTotalHeroByUser(user) + count <= bHeroDesign.getTokenLimit(), "User limit reached");
        //requestCreateToken
        uint256 isHeroS = 1;
        requestCreateToken(
            user,
