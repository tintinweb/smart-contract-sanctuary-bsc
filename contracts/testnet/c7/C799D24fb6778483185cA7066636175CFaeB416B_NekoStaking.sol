...
contract NekoStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public constant TIME_LINEAR = 90; // 3 months,
    ...
    function createPool(uint256 _timeStart , uint256 _timeEnd , uint256 _ninoReward , uint256 _limitTokenStake , uint256 _numberDayLock) public onlyOwner {
        uint256 rewardPerSecond = _ninoReward / (_timeEnd - _timeStart );
        poolInfo.push(PoolInfo({ timeStart : _timeStart, timeEnd : _timeEnd , totalNinoStake : 0 , rewardNinoPerSecond : rewardPerSecond , timeLock : _numberDayLock * PERIOD , limitTokenStake : _limitTokenStake}));
    }
    ...
    function stake(uint256 _pid, uint256 _amount) public whenNotPaused nonReentrant {
        ...
        uint256 newRewardPerSecond = (pool.rewardNinoPerSecond * (pool.timeEnd - pool.timeStart) - user.unclaimed )/ (pool.timeEnd - pool.timeStart) ;
        pool.rewardNinoPerSecond = newRewardPerSecond;
            
        if(pool.limitTokenStake > 0) {
            uint256 timeStartRelease = pool.timeStart + pool.timeLock ;
            _setAddressVesting(msg.sender , user.amount , timeStartRelease);
        }

        emit Deposit(msg.sender, _pid, _amount);
    }

    function pendingReward(uint256 _pid , address _user) public whenNotPaused view returns(uint256)  {
        ...
        if(user.lastDeposit <= 1 days && pool.limitTokenStake == 0) {
            return 0;
        }
        uint256 reward = ( time - user.lastDeposit ) * pool.rewardNinoPerSecond * user.amount / pool.totalNinoStake;
        return reward;
    }

    function getReward(uint256 _pid , address _user) public whenNotPaused view returns(uint256)  {
        UserInfo memory user = userInfo[_pid][_user];
    ...
    function unStake(uint256 _pid) public whenNotPaused {
        ...
        if(pool.limitTokenStake == 0){
            ...
            require(user.amount >= amountRelease, "Error");
    ...
    function _setAddressVesting(address _addr, uint256 _amount , uint256 _timeStart) private  {
