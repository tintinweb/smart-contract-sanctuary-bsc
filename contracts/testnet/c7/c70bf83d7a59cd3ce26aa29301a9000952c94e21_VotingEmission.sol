...
contract VotingEmission is IVotingEmission, OwnableUpgradeable, CoreUtility {
    ...
    uint256 public guardedLaunchStart;
    ...
    mapping(uint256 => mapping(address => uint256)) public weightsPerWeek; // timestamp (end of week) => address of pool => weight

    function initialize(address votingEscrow_, uint256 guardedLaunchStart_, uint256 lastTimestamp_) public initializer {
        __Ownable_init();
        ...
        initializeEmission(lastTimestamp_);
    }

    function initializeEmission(uint256 lastTimestamp_) public onlyOwner {
    ...
    function getPoolRelativeWeight(address pool, uint256 timestamp) external override returns (uint256) {
        ...
        if (timestamp < guardedLaunchStart) {
            return poolExisted(pool) ? 1e18 / size : 0;
        }
        ...
        _updatePoolWeight(weekTimestamp);
        return weightsPerWeek[weekTimestamp][pool];
    }

    function _updatePoolWeight(uint256 weekTimestamp) private {
    ...
    function _updateWeight() private {
        uint256 weekTimestamp = _endOfWeek(block.timestamp).sub(System.SYSTEM_INTERVAL);
        uint256 lastTimestamp_ = lastTimestamp; // gas saver
        require(weekTimestamp <= lastTimestamp_ + System.SYSTEM_INTERVAL, "Previous week is empty");
        if (weekTimestamp > lastTimestamp_) {
            lastTimestamp = lastTimestamp_ + System.SYSTEM_INTERVAL;
    ...
    ProxyUtility
{
    ...
    function increaseUnlockTime(uint256 unlockTime) external nonReentrant whenNotPaused {
        ...
        emit UnlockTimeIncreased(msg.sender, unlockTime,duration);
    }
    ...
    function _duration(uint256 unlockTime_, uint256 timestamp_) private view returns(uint256 duration_){
...
abstract contract CoreUtility {
    ...
    uint256 internal constant START_TIME = 1646326800;
