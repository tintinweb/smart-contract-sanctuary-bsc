            ...
            if(user.refs[0]>=requireDirect[i]){
               for(uint j =0;j<count;j++){
                address downline = refDetails[_userAddress][i+1][j];
                     uint downlineRoi = getUserDividends(downline);
                     uint income;
 ...
 function getLatestPrice() public view returns (int) {
  ...
  function getBoosterDataContract(address _user) private view returns(uint){
    ...
    function calaculateBosterRoi(address _user) private{
      ...
      if(totalBusiness >= users[_user].deposits[0].amountUSD.mul(10)){
  ...
  function progressReport(address _addr) view public returns( uint256 progress, uint max) {
        ...
        if(count >0){
            if(users[_addr].deposits[count-1].withdrwanUSD < max_payout) {
            uint payout = getUserDividends(_addr).add(getUserPassiveIncome(_addr)).add(getBoosterDataContract(_addr).add(users[_addr].deposits[count-1].withdrwanUSD));
