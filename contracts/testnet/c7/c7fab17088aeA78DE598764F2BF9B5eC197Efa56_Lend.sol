...
contract Lend is IERC721Receiver, ReentrancyGuard, AccessProtected {
    ...
    event ListingAdded(uint256 listingId,uint256[] _nftIds, uint256 _price, uint256 _duration ,address _lender, uint256 _timestamp,uint256 rentperiod);
    event ListingCancelled(uint256 indexed listingId,uint256[] indexed _nftIds, address indexed _lender, uint256 _timestamp);
    event Rented(
    ...
    event NftClaimed(uint256 indexed listingId,uint256[] indexed _nftIds, address indexed _lender);
    ...
    function lendNft(
        ...
        Listing memory listing = Listing(listingId,nftIds, price, block.timestamp + duration, _msgSender(), true, false,rentduration,0);
        listings[listingId] = listing;
       
        emit ListingAdded(listingId,nftIds, price,block.timestamp+duration, _msgSender(), block.timestamp,rentduration);
    }
    ...
    function cancelLendNft(uint256 listingId) external nonReentrant {
        ...
        for(uint i=0;i<listings[listingId].tokenIDs.length;i++){
        ...
        emit ListingCancelled(listingId,listings[listingId].tokenIDs, _msgSender(), block.timestamp);
    }
    ...
    function rentNft(uint256 listingId) external nonReentrant {
        ...
        // Transfer Game Token
        for(uint i=0;i<listings[listingId].tokenIDs.length;i++){
        nftGameToken.issueToken(msg.sender, listings[listingId].tokenIDs[i], nftAddress.tokenURI(listings[listingId].tokenIDs[i]));
        }
        emit Rented(listingId,listings[listingId].tokenIDs, seller, buyer, price, block.timestamp);
    }
    ...
    function claimNft(uint256 listingId) external nonReentrant {
        ...
        emit NftClaimed(listingId,listings[listingId].tokenIDs, listings[listingId].lender);
    }
