...
contract YOGBoxSale is Pausable, Ownable {
    /* ========== LIBs ========== */
    ...
    mapping(bytes => uint256) private sigBoughtBoxTypeMap;
    ...
    function getSigBoughtBoxType(bytes memory _sig)
        public
        view
        returns (uint256)
    {
        return sigBoughtBoxTypeMap[_sig];
    }
    ...
    function buxBoxWithSignature(
    ...
    ) external whenNotPaused checkTime {
        require(amount > 0 && nonce > 0 && boxType > 0 && buyNumber > 0);
        require(boxStockMap[boxType] > 0, "insufficient stock");
        require(buyNumber <= maxBuyAmount, "maxBuyAmount limit");
        require(sigBoughtBoxTypeMap[signature] == 0, "signature used");
        ...
        require(_signer == signer, "not signed by gameAdmin");
        ...
        sigBoughtBoxTypeMap[signature] = boxType;
        boxStockMap[boxType]--;

        emit BuyLog(msg.sender, signature, boxType, buyNumber, amount);
    }

    function devBuyBoxMock(address erc20Contract, uint256 amount)
        external
        onlyOwner
    {
        require(amount > 0);
