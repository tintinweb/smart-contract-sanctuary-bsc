...
contract Trade is AccessControl {
    ...
    //uint256 private mintingFeePermille;
    ...
    */
    ...
    function setMintingFee(uint256 _mintFee)
        external
        onlyRole("ADMIN_ROLE")
        returns (bool)
    {
    ...
    */
    ...
    function mint(address nftAddress, BuyingAssetType nftType, string memory tokenURI, uint96 fee, uint256 supply,
     Sign memory sign)
        external
        returns(bool)
    {
        ...
        if(mintingFee != 0) {
            transferProxy.erc20safeTransferFrom(IERC20(usdt), msg.sender, owner, mintingFee);
        }
        if(nftType == BuyingAssetType.ERC721) {
            transferProxy.erc721mint(IMint(nftAddress), msg.sender, tokenURI, fee);
        }
        if(nftType == BuyingAssetType.ERC1155) {
            transferProxy.erc1155mint(IMint(nftAddress), msg.sender, tokenURI, fee, supply);
        }
        return true;
    }
