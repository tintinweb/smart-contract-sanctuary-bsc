...
contract Staking is GlobalsAndUtils, Ownable, ReentrancyGuard, IERC721Receiver {
     ...
     */
    function stake(uint256 stakedAmount) external CampaignOnGoing {
    ...
    function unStake(uint256 _stakeId) external {
        ...
        if (penaltyAmount > 0) {
            stakingToken.transfer(address(this), penaltyAmount);
...
/**
     ...
     */
    function claimXTickets() external ClaimXTicketAllowed returns (uint256 newClaimedTickets) {
    ...
    function claimReward(uint256 campaignId)  external {
        ...
        _rewardWinner(msg.sender,campaignId);
    }
     ...
     * @return endTime
     */
    function getCurrentCampaignDetails() external view returns(uint256 campaignId, uint256 rewardCount, uint256 startTime, uint256 endTime, uint256 coolDown) {
    ...
    function getClaimableTickets() external view returns(uint256 newClaimedTickets) {
        ...
        if( totalUserXTickets[msg.sender][latestCampaignId] > 0){
            newClaimedTickets =  totalUserXTickets[msg.sender][latestCampaignId];
        }else {
            for (uint256 x=0; x < userStakeIds[msg.sender].length; x++) {
            newClaimedTickets += userClaimable(msg.sender, userStakeIds[msg.sender][x]);
        }
        }
     ...
     * @param _awardCollections Array of collections to give awards from
     */
    function startLootBox(uint256 startTime, uint256 endTime, uint256 rewardCount, address[] memory _awardCollections) external onlyOwner campaignEnded {
        uint256 totalNfts = 0;
        ...
        // set campaign collections
        campaignAwardCollections[latestCampaignId] = _awardCollections;
     ...
     */
    function rewardLootBox() external onlyOwner CalculateRewardAllowed {
     ...
     * @param _campaignId Id of the campaign to add collections to
     * @param _collections Array of collections to add to the campaign
     */
     ...
     * @param _campaignId Id of the campaign to remove the collection from
     * @param _collection Address of collection to remove from the campaign
     */
    function removeCollection(uint256 _campaignId, address _collection) external onlyOwner {
        ...
        for (uint256 m; m < awardCollections.length; m++) {
            if (awardCollections[m] == _collection) {
                if (awardCollections.length > 1) {
                    awardCollections[m] = awardCollections[awardCollections.length.sub(1)];
     ...
     * @param _campaignId Id of the campaign to add nftTokenIds to
     * @param _collectionAddress Address of collection to add the nftTokenIds to
     * @param _nftTokenIds Array of nftTokenIds to add against a collection
     */
    function addNftTokenIds(uint256 _campaignId, address _collectionAddress, uint256[] memory _nftTokenIds) external onlyOwner {
        // require(_campaignId <= latestStakeId, "campaign doesn't exist");

        uint256[] storage nftTokenIds =  NftTokenIds[_campaignId][_collectionAddress];
        for (uint256 m; m < _nftTokenIds.length; m++) {
            nftTokenIds.push(_nftTokenIds[m]);
        }
    }

    /**
     * @dev ADMIN METHOD: Withdraw NFTs that were not awarded to anyone in a campaign
     * @param _campaignId Id of the campaign to withdraw from
     * @param receiver Address of the user to transfer the nft to
     */
    function withdrawUnusedNfts(uint256 _campaignId, address receiver) external onlyOwner campaignEnded {
        ...
        for (uint256 f; f < awardCollections.length; f++) {
            ...
            if (nftIds.length > 0) {
                ...
                for (uint256 g = nftIds.length; g > 0; g--) {
                    rewardsToken.safeTransferFrom(address(this), receiver, nftIds[g-1]);
                    nftIds.pop();
                }
            }
        }
    ...
    /**
     * @dev ADMIN METHOD: Merge an old campaign's collections with the latest
     * @param _campaignId old campaignId
     */
    ...
    function _unStake(uint256 _stakeId) private {
        ...
        if (latestCampaignId > 0) {
            ...
            if (unixStakedTime < campaignEndTime && totalUserXTickets[msg.sender][latestCampaignId] > 0) {
                totalUserXTickets[msg.sender][latestCampaignId] -= xTickets[msg.sender][_stakeId].claimable;
    ...
    function _getUserStakedTime(Stake memory usrStake) private view returns (uint256 unixStakedTime) {
        ...
        unixStakedTime =  block.timestamp.sub(usrStake.stakedAt);
    }
    ...
    function _getClaimableXTickets(address _stakeOwner, uint256 _stakeId) private returns(uint256) {
        ...
        // // save ticket ids against user address
        // for (uint256 i; i < newTickets; i++) {
        //     uint256 ticketId = totalClaimableTickets + i;
        //     ticketIds[ticketId][latestCampaignId] = _stakeOwner;
        // }
     ...
     */
    function _rewardWinner(address _winnerAddress, uint256 campaignId) private {
       (uint256[] memory _winningTicketIds,,)= getRewardClaimable(latestCampaignId);
       require(_winningTicketIds.length > 0, "no reward");
    ...
    //    uint256[] storage _winningTicketIds = winningTicketIds[campaignId];
        for (uint256 j = _winningTicketIds.length - 1; j < _winningTicketIds.length; j--) {
           ...
           if(winningTicketId >= rangeStart && winningTicketId <= rangeEnd){
               if(rewardsReceived[_winnerAddress][campaignId] >= 5){
                ...
                rewardsToken = IERC721(ticketCollection[campaignId][winningTicketId]);
                rewardsToken.safeTransferFrom(address(this), _winnerAddress, ticketNFTID[campaignId][winningTicketId], "");
     ...
     * @param num total random numbers required
    ...
    function _pickWinners() private {
        ...
        if (totalClaimableTickets == 0) {
         ...
         winningTicketIds[latestCampaignId] = _getRandomNumbers(randomSeed, N_REWARD_WINNER, totalClaimableTickets);
        uint256[] memory _winningTicketIds = winningTicketIds[latestCampaignId];
        // Loop over all winning tickets and add reward for winner
        for (uint256 z; z < _winningTicketIds.length; z++) {
            uint256 winningTicketId = _winningTicketIds[z];
            (address _collection, uint256 _nftId) = getNftId();
            ticketCollection[latestCampaignId][winningTicketId] = _collection;
            ticketNFTID[latestCampaignId][winningTicketId] = _nftId;
        }
        // totalClaimableTickets = 0;
    }
     ...
     */
    function getRewardClaimable(uint256 campaignId) public view  returns(uint256[] memory userXticketsIds, uint256 startTime, uint256 endTime) {
       ...
       uint256[] memory _winningTicketIds = winningTicketIds[campaignId];
        for (uint256 j; j < _winningTicketIds.length; j++) {
            uint256 winningTicketId = _winningTicketIds[j];
           if(winningTicketId >= rangeStart && winningTicketId <= rangeEnd){
                userXticketsIds[j] = winningTicketId;
           }
        }
        return (userXticketsIds, campaigns[campaignId].startTime, campaigns[campaignId].endTime );
    }
    ...
    function getClaimableToken() public view returns(uint256 stakerPenaltyBonus) {
        stakerPenaltyBonus = userClaimedPenalty[msg.sender][latestCampaignId] ? 0 : _stakerPenaltyBonus(msg.sender);
    }
     ...
     */
    function claimPenaltyBonus() external nonReentrant  {
        require(userStakedAmount[msg.sender] > 0, "user has no stakes");
        uint256 stakerPenaltyBonus = _stakerPenaltyBonus(msg.sender);
        penaltyAccumulated -= stakerPenaltyBonus;

        stakingToken.transfer(msg.sender, stakerPenaltyBonus);
    }
     ...
     */
    function _stakerPenaltyBonus(address _staker) allowedClaim private view returns (uint256) {
        uint256 stakerBonusPenalty = 0;

        // staker percentage in total penaltyBonus
        uint256 stakerPercentage = userStakedAmount[_staker].mul(100).div(penaltyAccumulated);

        require(stakerPercentage > 0, "user doesn't have any percentage in penalty bonus");

        // calculate penaltyBonus percentages
        uint256 currentPenaltyAccumulated = penaltyAccumulated;
        ...
        (uint256 unixStakedTime) = _getUserStakedTime(stakes[msg.sender][userStakeIds[msg.sender][0]]);
        uint256 stakedDays = unixStakedTime.div(1 days);

        if (stakedDays >= 90) {
            stakerBonusPenalty += bonusTwentyPercent.mul(stakerPercentage).div(100);
        } else if (stakedDays >= 180) {
            stakerBonusPenalty += bonusTwentyFivePercent.mul(stakerPercentage).div(100);
        } else if (stakedDays >= 360) {
            stakerBonusPenalty += bonusTwentyFivePercent.mul(stakerPercentage).div(100);
        }

        stakerBonusPenalty += bonusThirtyPercent.mul(stakerPercentage).div(100);
...
abstract contract GlobalsAndUtils {
    ...
    modifier allowedClaim() {
         require(ON_COOLDOWN == true, "Campaign on going");
         require(userClaimedPenalty[msg.sender][latestCampaignId], "already claimed");
        _;
    }
    ...
    uint256 internal constant CLAIM_X_TICKET_DURATION= 20 minutes;
    ...
    mapping(address => mapping (uint256=> uint256)) totalUserXTickets;

    /// @notice Mapping to store ticket ids against owner address
    mapping(uint256 => mapping (uint256=> address)) internal ticketIds;
    ...
    mapping(address => mapping (uint256=>address[])) internal userAwardedCollections;
    ...
    mapping(uint256 => uint256[])winningTicketIds;
    /// @notice mapping for which id get which collection address and which nft;
    mapping (uint256 => mapping (uint256=> address)) ticketCollection;
    mapping (uint256 => mapping(uint256=> uint256)) ticketNFTID;
    ...
    mapping (address=> mapping(uint256 => XTicketRange)) userXTicketRange;
    
    uint256 internal randomSeed;

    /// @notice Variable to store total penalty accumulated in the contract
    uint256 internal penaltyAccumulated;
