...
contract Ticket1155Contract is ERC1155 {
    using EnumerableSet for EnumerableSet.UintSet;

    string public name;
    address public owner;
    uint256 public _typeCount;
    ...
    constructor(string memory _name, address _owner) ERC1155("") public {
        name = _name;
        owner = _owner;
    }
    ...
    function initTicket( string[] memory _types, uint256[] memory _supplies, uint256[] memory _prices ) public {
        ...
        for (uint256 i = 0; i < _types.length; i++) {
            ...
            _mint(msg.sender, _typeCount, _supplies[i], "0x");
            idOfUsers[msg.sender].add(_typeCount);
            _typeCount++;
...
contract Ticket1155Factory {
    mapping (address => address []) public eventMap;
    address[] public deployedEvents;

    event CreatedNFT1155Event (address eventAddress, address creator);

    function createEvent(
        string memory _initName
    ) external {
        address newEvent = address(new Ticket1155Contract(_initName, msg.sender));
        eventMap[msg.sender].push(newEvent);
        deployedEvents.push(newEvent);
        emit CreatedNFT1155Event(newEvent, msg.sender);
    }
    ...
    function getEventByUser (address sender) external view returns (address [] memory) {
        return eventMap[sender];
    }

    function eventOfUserLength(address sender) external view returns (uint256){
        return eventMap[sender].length;
    }

    function getLengthDeployedEvents () external view returns (uint256) {
