...
contract STTData is Ownable {
    ...
    function addTotalUserAmount(uint256 amount) public lock onlyController {
        totalUserAmount  = totalUserAmount.add(amount);
    }

    function addTodayNewUserAmount(uint256 amount, bool reset) public lock onlyController {
        if(reset){
    ...
    function addTotalRechargeAmount(uint256 amount) public lock onlyController {
        totalRechargeAmount = totalRechargeAmount.add(amount);
    }

    function addTodayRechargeAmount(uint256 amount, bool reset) public lock onlyController {
        if(reset){
    ...
    function addTotalDrawUsdtAmount(uint256 amount) public lock onlyController {
        totalDrawUsdtAmount = totalDrawUsdtAmount.add(amount);
    }

    function addTodayDrawUsdtAmount(uint256 amount, bool reset) public lock onlyController {
        if(reset){
    ...
    function addTotalUserTokenProfit(uint256 amount) public lock onlyController {
        totalUserTokenProfit = totalUserTokenProfit.add(amount);
    }

    function addTodayUserTokenProfit(uint256 amount, bool reset) public lock onlyController {
        if(reset){
    ...
    function addTotalBuyBlackHoleValue(uint256 amount) public lock onlyController {
        totalBuyBlackHoleValue = totalBuyBlackHoleValue.add(amount);
    }

    function addTodayBuyBlackHoleValue(uint256 amount, bool reset) public lock onlyController {
        if(reset){
    ...
    function addTotalTransferAmount(uint256 amount) public lock onlyController {
        totalTransferAmount = totalTransferAmount.add(amount);
    }

    function addTodayTransferAmount(uint256 amount, bool reset) public lock onlyController {
        if(reset){
...
contract STTQuery is Ownable {
    ...
    constructor() {
        STTD = STTData(payable(0xb7bfD8be7dfEaAD3a5b9814FF6e52c6437c0D60F));
    }
    ...
    function queryLastInvestRecordsTime() public view returns(uint256 investTime) {
        (,,,,investTime,,,,) = STTD.investRecords(STTD.queryAllTokenRecordsNum());
    }
    ...
    function checkBuyBlackHoleValue(address user, uint256 usdtAmount) public view returns (uint256){
        ...
        if(usdtAmount < STTD.buyBlackHoleValueLimit()) return 1;
        if(queryUserRecommender(user) == address(0)) return 2;
