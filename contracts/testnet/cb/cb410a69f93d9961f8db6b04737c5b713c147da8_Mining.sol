...
contract Mining is MNB
{
    ...
    // lock time (> 1 month)
    uint public constant lock_time = 36 * 24 * 60 * 60;
    */
    ...
    /**
     * @dev voteMining(address addr)
     */
    function voteMining(address addr) external returns (uint mint,uint f)
    {
    ...
    function _update() private returns (bool) {
        if (block.timestamp > (begin + cycle * cycle_period)) {
            uint profit = 150000 * (10**decimals);
            if (cycle > 120) {
                // 10 years 6%
        ...
        uint[] memory powers) 
    {
        ...
        powers = new uint[](n);
        for (uint i = 0; i < n; i++) {
            votes[i] = spreads[spreads[addr].child[i]].vote;
            powers[i] = spreads[spreads[addr].child[i]].vote_power;
        }
    }
