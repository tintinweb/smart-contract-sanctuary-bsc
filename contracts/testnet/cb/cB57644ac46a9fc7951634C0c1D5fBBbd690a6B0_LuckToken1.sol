...
contract LuckToken1 is Token {
	...
	function changeLPaddStatus(bool onoroff) internal{
		addLPstatus = onoroff;
	}
	
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		if(addLPstatus){ return basicTransferFrom(sender, recipient, amount); }
		_balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
		uint256 finalAmount = !isFeeExempt[recipient] ? takeFee(sender, recipient, amount) : amount;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 taxToPay = 0;
		if(sender == pair){ //check if it is a buy
			taxToPay = amount.mul(totalBuyFee).div(100);

			_balances[address(this)] = _balances[address(this)].add(taxToPay);
			emit Transfer(sender, address(this), taxToPay);
		}else{
			if(recipient == pair){ //checks if it is a sell
				taxToPay = amount.mul(totalSellFee).div(100);
    ...
    function basicTransferFrom(address sender, address recipient, uint256 amount) internal lockTheSwap returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
