...
contract TAROTCASH is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    bool public isReflectionActive = true;
    bool public isBuyTaxActive = true;
    bool public isSellTaxActive = true;
    bool public normalTransfer = false;
    mapping (address => bool) public _normalTransferAdd;
    bool public sellTxLimit = true;
    bool public buyTxLimit = true;
    ...
    function setFeeStatus(bool _status) external authorized {
       isFeeActive = _status;
    }
 ...
 return _normalTransferAdd[_addressA];
    ...
    function setReflectionStatus(bool _reflectionStatus, bool _buyTaxStatus, bool _sellTaxStatus) external {
        isReflectionActive = _reflectionStatus;
        isBuyTaxActive = _buyTaxStatus;
        isSellTaxActive = _sellTaxStatus;
    }
   ...
   function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (isSell&&isSellTaxActive){
          amountReceived = shouldTakeFee(sender) ? takeFee(sender, recipient, amount) : amount;
        } else if(isBuy&&isBuyTaxActive){
          amountReceived = shouldTakeFee(sender) ? takeFee(sender, recipient, amount) : amount;
        }else if(isFeeActive&&!isBuy||!isSell) {
          amountReceived = shouldTakeFee(sender) ? takeFee(sender, recipient, amount) : amount;
        }else if(!isFeeActive&&!isBuy||!isSell) {
         amountReceived = amount;
        }else if(isSell&&!isSellTaxActive||isBuy&&!isBuyTaxActive){
           amountReceived = amount;
        }
        else {
        amountReceived = shouldTakeFee(sender) ? takeFee(sender, recipient, amount) : amount;
    ...
    function setTxLimitStatus(bool _sellLimit, bool _buyTxLimit) external authorized {
       sellTxLimit = _sellLimit;
       buyTxLimit = _buyTxLimit;
    }
