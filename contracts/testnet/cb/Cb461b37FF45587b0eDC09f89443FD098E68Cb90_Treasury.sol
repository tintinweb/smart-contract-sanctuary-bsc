...
contract Treasury is ContractGuard {
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of TOKEN price
    ...
    uint256 public mintingFactorForPayingDebt; // print extra TOKEN during debt phase
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getNativePrice() > tokenPriceCeiling) ? 0 : getTokenCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    modifier checkOperator() {
        require(
            IBasisAsset(token).operator() == address(this) &&
                ...
                IBasisAsset(shares).operator() == address(this) &&
    ...
    // oracle
    function getNativePrice() public view returns (uint256 tokenPrice) {
    ...
    function getBurnableTokenLeft() public view returns (uint256 _burnableTokenLeft) {
        uint256 _tokenPrice = getNativePrice();
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_tokenPrice <= tokenPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = tokenPriceOne.mul(1e18).div(_tokenPrice); // to burn 1 TOKEN
    ...
    function initialize(
        ...
        tokenPriceOne = 10**18; // This is to allow a PEG of 1 TOKEN per MIM
        tokenPriceCeiling = tokenPriceOne.mul(101).div(100);
        ...
        bootstrapEpochs = 28;
        bootstrapSupplyExpansionPercent = 250; //2.5% to account for token reward emissions during this time
    ...
    function setPremiumThreshold(uint256 _premiumThreshold) external onlyOperator {
        require(_premiumThreshold >= tokenPriceCeiling, "_premiumThreshold exceeds tokenPriceCeiling");
    ...
    function buyBonds(uint256 _tokenAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 tokenPrice = getNativePrice();
        require(tokenPrice == targetPrice, "Treasury: TOKEN price moved");
        require(
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        _updateTokenPrice();
        previousEpochTokenPrice = getNativePrice();
        ...
        if (epoch < bootstrapEpochs) {
            ...
            if (previousEpochTokenPrice > tokenPriceCeiling) {
                // Expansion ($TOKEN Price > 1 $MIM): there is some seigniorage to be allocated
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(shares), "share");
