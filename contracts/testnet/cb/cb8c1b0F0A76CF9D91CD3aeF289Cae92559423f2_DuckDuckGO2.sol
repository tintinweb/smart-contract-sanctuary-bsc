 ...
 * t.me/duckduckgocoin
   ...
   - 4% of every transaction is converted to BNB and sent to the marketing wallet
...
contract DuckDuckGO2 is Context, IERC20, Ownable {
    ...
    address private _marketingWalletAddress = 0x0266CFfeD9a384b92CFbB4B3F66345AcB52cFBe0;
    address private _gamedevWalletAddress = 0x5291FEa76389422956BeF52953754978a83219fE;
    ...
    string private _name = "DuckDuckGO Coin2";
    string private _symbol = "DDG2";
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tDev, uint256 tLiquidity, uint256 tmarketing, uint256 tBurn) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity + tDev + tmarketing);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, fees.tFee, fees.tDev, fees.tLiquidity, fees.tmarketing, _getRate());
        return (rAmount, rTransferAmount, rFee, fees.tTransferAmount, fees.tFee, fees.tDev, fees.tLiquidity, fees.tmarketing, fees.tBurn);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (Fees memory) {
        ...
        fees.tmarketing = calculatemarketingFee(tAmount);
        fees.tBurn = calculateBurnFee(tAmount);
        fees.tTransferAmount = tAmount.sub(fees.tFee).sub(fees.tDev).sub(fees.tLiquidity).sub(fees.tmarketing).sub(fees.tBurn);
        return (fees);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tDev, uint256 tLiquidity, uint256 tmarketing, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rDev = tDev.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rmarketing = tmarketing.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rDev).sub(rLiquidity).sub(rmarketing);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingBNB = newBalance.div(_marketingFee + _devFee + _liquidityFee).mul(_marketingFee);
        uint256 devBNB = newBalance.div(_marketingFee + _devFee + _liquidityFee).mul(_devFee);
        uint256 liquidityBNB = newBalance - marketingBNB - devBNB;

        TransferBnbToExternalAddress(_marketingWalletAddress, newBalance.div(_marketingFee + _devFee + _liquidityFee).mul(_marketingFee));
        TransferBnbToExternalAddress(_gamedevWalletAddress, newBalance.div(_marketingFee + _devFee + _liquidityFee).mul(_devFee));
