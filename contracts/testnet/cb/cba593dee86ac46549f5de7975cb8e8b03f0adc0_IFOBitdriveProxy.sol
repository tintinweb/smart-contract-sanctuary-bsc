...
contract IFOBitdriveProxy is ReentrancyGuard, Initializable {
  ...
  function deposit(uint256 _amount) public {
    ...
    if(userInfo[msg.sender].amount<STAKING_PRICE[0]){
  ...
  function harvestReward() public nonReentrant {
    ...
    uint256 withdrawnamount = userInfo[msg.sender].withdrawnamount;
    uint256 available = perAmount.sub(withdrawnamount);
    require (available >0, 'unable to withdraw');
    offeringToken.safeTransfer(address(msg.sender), available);
    
  }

   function getuserAllocated(address _user) public view returns (uint256) {
       uint256 perAmount = tokenAllocated();
       uint256 currentTier = userInfo[_user].tier;
       require (currentTier>1 && currentTier<=4, 'not eligible');
       uint256 tieramount;
       if(currentTier==2){
            tieramount =(totalTier2Users.mul(perAmount).mul(POOL_WEIGHT[1])).div(10000);
       }else if(currentTier==3){
            tieramount =(totalTier3Users.mul(perAmount).mul(POOL_WEIGHT[2])).div(10000);
       }else if(currentTier==4){
            tieramount =(totalTier4Users.mul(perAmount).mul(POOL_WEIGHT[3])).div(10000);
       }
       return tieramount;
    }

  function tokenAllocated() public view returns (uint256) {
        uint256 allocate= (
            (totalTier2Users.mul(POOL_WEIGHT[0])).add(totalTier3Users.mul(POOL_WEIGHT[1]))
        .add(totalTier4Users.mul(POOL_WEIGHT[3])))*10000;
        uint256 perUser=offeringAmount/allocate;
        return perUser;
  }
