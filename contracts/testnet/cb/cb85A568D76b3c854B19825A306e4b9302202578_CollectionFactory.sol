...
contract Collection is ERC721Enumerable,ERC2981, Ownable {
   ...
   * @param _tokenURI suffix of tokenURI
   */
  function mint(address _to, string memory _tokenURI,address royaltyReceiver, uint96 feeNumerator) public onlyOwner {
   ...
   * @param _tokenURI suffix of tokenURI
   */
  function batchMint(address _to, string memory _tokenURI, uint _amount,address royaltyReceiver, uint96 feeNumerator) public onlyOwner {
    require(bytes(_tokenURI).length > 0, "Collection: tokenURI not valid");
    require(_amount > 0);
