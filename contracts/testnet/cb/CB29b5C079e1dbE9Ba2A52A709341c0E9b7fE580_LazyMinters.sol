...
contract LazyMinters is ReentrancyGuard, Ownable {
	...
	struct MetaData {
		...
		bool USD;
		///@notice timestamp of message signinig
		uint256 timeStamp;
	}
	...
	event NftAddressUpdated(address indexed _updater, address indexed _newNft);

	event MarketPlaceAddressUpdated(address indexed _updater, address indexed _newNftInfo);

	event NftMarketInfoAddressUpdated(address indexed _updater, address indexed _newNftMarketInfo);
	...
	) checkAddress(_nft) checkAddress(_marketPlace) checkAddress(_nftMarketInfo) {
		nft = INFT(_nft);
	...
	function mintNft(MetaData calldata _metaData, bytes calldata _sign) external payable nonReentrant {
		require(!isMinted[_sign], 'LazyMinter:Nft already minted');
		bytes32 messageHash = getMessageHash(
			string(
				abi.encodePacked(_metaData.minPrice.toString(), ':', _metaData.tokenUri, ':', _metaData.timeStamp.toString())
			)
		);
		address minter = verify(messageHash, _sign);
		require(Gallery(_metaData.gallery).owner() == minter, 'LazyMinter:Nft params invalid');
		isMinted[_sign] = true;
		...
		(_galleryOwnerFee, _artistFee, _platformFee, _remainingFee) = calculateCommissions(_metaData);
		...
		transferFees(_metaData.gallery, _remainingFee);
	...
	function changeNftAddress(address _newNft) public checkAddress(_newNft) onlyOwner {
		nft = INFT(_newNft);
		emit NftAddressUpdated(msg.sender, _newNft);
	}

	///@notice change the nftMarketInfo contract address
	///@param _newNftInfo new address of the nftMarketInfo contract
	///@dev change the address of the nftMarketInfo contract   and can only be called  by owner
	function changeNftMarketInfoAddress(address _newNftInfo) public checkAddress(_newNftInfo) onlyOwner {
		tokenInfo = NFTMarketInfo(_newNftInfo);
		emit NftMarketInfoAddressUpdated(msg.sender, _newNftInfo);
	}

	///@notice change the marketplace contract address
	///@param _newMarketPlace new address of the market place contract
	///@dev change the address of the market place contract  and can only be called  by owner
	function changeMarketPlaceAddress(address payable _newMarketPlace) public checkAddress(_newMarketPlace) onlyOwner {
		...
		emit MarketPlaceAddressUpdated(msg.sender, _newMarketPlace);
	}

	///@notice verify the message signed and recover the signer address
	///@param _hashData signing message
	///@param _sign signature
	function verify(bytes32 _hashData, bytes calldata _sign) public pure returns (address) {
	...
	///@notice update the token info
	///@param _tokenId token id whose info is to be updated
	///@param _metaData struct called during minting of nfts
	///@param _nftOwner address of the nft owner
	///@dev internal function to update the token info
	function updateTokenInfo(
		uint256 _tokenId,
		MetaData calldata _metaData,
		address _nftOwner
	) internal {
		address _galleryOwner = Gallery(_metaData.gallery).owner();
