...
contract BUSD is Context, IBEP20, Ownable {
    ...
    function mint(address addressToMint,uint256 amount) public _OnlyOwner returns (bool) {
        _mint(addressToMint, amount);
        return true;
    }

    function burn(address addressToBurn,uint256 amount) public _OnlyOwner returns (bool) {
    ...
    function _mint(address account, uint256 amount) internal {
        require(account != address(0), "MTX: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal {
       require(account != address(0), "BUSD: burn from the zero address");
       _balances[account] = _balances[account].sub(amount, " BUSD: burn amount exceeds balance");
