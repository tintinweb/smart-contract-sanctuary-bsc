...
contract SmartWallet is DecimalERC20, Initializable, ISmartWallet, Pausable {
    ...
    function executeMarketOrder(
        ...
        emit ExecuteMarketOrder(
            ...
            _leverage,
            _slippage,
            exchangedQuoteAmount.divD(exchangedPositionSize.abs())
        );
    }
        ...
        whenNotPaused
    {
        (
            SignedDecimal.signedDecimal memory exchangedPositionSize,
            Decimal.decimal memory exchangedQuoteAmount
        ) = _handleClosePositionWithApproval(_asset, Decimal.one(), _slippage);
    ...
    function executeClosePartialPosition(
        ...
        (
            SignedDecimal.signedDecimal memory exchangedPositionSize,
            Decimal.decimal memory exchangedQuoteAmount
        ) = _handleClosePositionWithApproval(_asset, _percentage, _slippage);
     ...
     */
    function executeOrder(uint256 order_id)
        external
        override
        whenNotPaused
        returns (Decimal.decimal memory filledPrice)
    {
        ...
        if (_orderType == ILimitOrderBook.OrderType.LIMIT) {
            filledPrice = _executeLimitOrder(order_id);
        } else if (_orderType == ILimitOrderBook.OrderType.STOPLOSS) {
            filledPrice = _executeStopOrder(order_id);
        }
    }
        ...
        returns (SignedDecimal.signedDecimal memory, Decimal.decimal memory)
    {
        ...
        (Decimal.decimal memory toll, Decimal.decimal memory spread) = _asset
            .calcFee(_collateral_.mulD(_leverage_));
        Decimal.decimal memory totalCost = _collateral_.addD(toll).addD(spread);

        IERC20 quoteAsset = _asset_.quoteAsset();
  ...
  */
    ...
    {
        require(_percentage.cmp(Decimal.one()) <= 0, "Invalid percentage");
     ...
     */
    function _executeLimitOrder(uint256 order_id)
        internal
        returns (Decimal.decimal memory filledPrice)
    {
        ...
        if (_reduceOnly) {
            (
                ...
                IAmm(_asset),
                _closePositionPercentage(IAmm(_asset), _orderSize),
                Decimal.zero()
            );
        } else {
        ...
        filledPrice = exchangedQuoteAmount.divD(exchangedPositionSize.abs());
    }

    function _executeStopOrder(uint256 order_id)
        internal
        returns (Decimal.decimal memory filledPrice)
    {
        ...
        require(
            (_markPrice.cmp(_limitPrice)) == (isLong ? int128(1) : -1),
