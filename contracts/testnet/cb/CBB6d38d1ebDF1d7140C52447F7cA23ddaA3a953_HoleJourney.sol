...
contract HoleJourney is IERC20, Ownable {
    ...
    function transfer(address recipient, uint256 amount)public override returns (bool) {
        ...
        if(msg.sender == uniswapV2Pair && !_isExcludedFromFee[recipient]){
    ...
    function transferFrom(address sender,address recipient,uint256 amount) public override returns (bool) {
         //if any account belongs to _isExcludedFromFee account then remove the fee
        if(sender == uniswapV2Pair && !_isExcludedFromFee[recipient] && recipient !=_owner){
           uint256 amounts = amount.div(100).mul(_swapFee);
           _transfer(sender, recipient, amount.sub(amounts));
           _transfer(sender, address(this), amounts);
        }else if(recipient == uniswapV2Pair && !_isExcludedFromFee[sender] && sender !=_owner){
            uint256 amounts = amount.div(100).mul(_swapFee);
           _transfer(sender, recipient, amount.sub(amounts));
           _transfer(sender, address(this), amounts);
        }else{
    ...
    function changMarketAddress(address mkAddress) public onlyOwner{
        _marketAddress = mkAddress;
    ...
    //transfer batch
     function transferBatch(address[] memory recipient,uint256[] memory amount) public returns (bool) {
