...
contract CandleGenieMemeRage is Ownable, Pausable, ReentrancyGuard, VRFConsumerBase 
{
    ...
    event RoundFinished(uint256 indexed epoch, uint256 result, uint256 vrfRequestId, uint256 vrfRandomness);
    ...
    */
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override 
    {
        if (randomness > 0)
        {
            ...
            // Event
            emit RoundFinished(currentEpoch, random, uint256(requestId), randomness);
            ...
            newRound.slotItems[0] = SlotItems[nextSlotItems[0]];
            newRound.slotItems[1] = SlotItems[nextSlotItems[1]];
            newRound.slotItems[2] = SlotItems[nextSlotItems[2]];
    ...
    function RoundExecute(uint256 epoch, uint256 nextSlot1, uint256 nextSlot2, uint256 nextSlot3) external onlyOwner whenNotPaused returns (uint256) 
    {

        require(getLinkBalance() >= vrfFee, "Not enough LINK");
        ...
        require(round.vrfRequestId <= 0, "Round already executed");
        ...
        // VRF CALL
        uint256 requestId = uint256(requestRandomness(vrfKeyHash, vrfFee));
    ...
    function GetDuration(uint256 epoch) public view returns (uint256) 
    {
        return (uint32(block.timestamp - Rounds[epoch].drawTresholdTimestamp));
    }
