...
contract FiveElements is Permission {
    ...
    function getFrontAddress() public view returns(address){
        return fiveCamps[0][fiveCamps[0].length / 2];
    }
    ...
    function buyTicket(uint8 camp, uint32 amount) public {
        ...
        require(endTime > block.timestamp, "The game is end");
    ...
    function luckFive() private {
        ...
        bool isTrue = (count > 7) ? true : false;
    ...
    function _getRandomNumber(address player) internal returns (bytes32){
        _updateRamdomSeed();
        bytes32 _hash =  sha256(abi.encodePacked(
            randomSeed, block.timestamp, block.number , player
        ));
        bytes32 newHash;
        for(uint8 i = 0; i < 31; i++){
            newHash = bytes32(abi.encodePacked(newHash, _hash[i]));
        }
        return newHash;
    }
    ...
    function setEndTime(uint _endTime) external onlyRole(MANAGER_ROLE){
        require(endTime < block.timestamp, "The game is not end");
        endTime = _endTime;
    }
