  ...
  ReentrancyGuardUpgradeable
{
  ...
  function payoutToken(address _coinAddress) external nonReentrant onlyOwner {
    uint256 balances = getContractBalanceByToken(_coinAddress);
    require(balances != 0, "NeloCoinFlip: No funds to withdraw");

    if (_coinAddress == address(0)) {
      require(_transfer(payable(msg.sender), balances), "NeloCoinFlip: Failed to payout token.");
      contractBalance = 0;
    } else {
      IERC20Upgradeable(_coinAddress).safeTransfer(address(msg.sender), balances); 
    }
  }
