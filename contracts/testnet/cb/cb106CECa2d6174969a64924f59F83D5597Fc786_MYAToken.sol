...
contract MYAToken is ERC20Permit, VaultOwned, Initializable {
  using SafeMath for uint256;
    ...
    uint256 public _initialSupply = 40000000 * (10**uint256(18));
    uint256 private PRBrandingIncentivesMarketingAmount =
        214000000 * (10**uint256(18));
    uint256 private TeamAdvisorsAmount = 200000000 * (10**uint256(18));
    uint256 private FoundationReserveAmount = 140000000 * (10**uint256(18));
    uint256 private SeedSaleAmount = 113000000 * (10**uint256(18));
    uint256 private StrategicSaleAmount = 50000000 * (10**uint256(18));
    uint256 private PromotionPartnershipsAmount = 3000000 * (10**uint256(18));
    uint256 private ListingLiquidityRewardsAmount =
        200000000 * (10**uint256(18));
    //uint256 private PreSaleAmount = 40000000;
    //uint256 private PublicSaleAmount = 40000000 * (10**uint256(18));

    address private PRBrandingIncentivesMarketing =
        0x5260b2a74d9673686BD357F06045ef910Ec88A64;
    address private TeamAdvisors = 0x73212472e3c1191481D63c9b65F3563866458493;
    address private FoundationReserve =
        0x7C936a6263Fe15581798C1c7ff12C09f054fFeD5;
    ...
    address private PromotionPartnerships =
        0x01e32A7DdAABB1466C8dd1D7754Bb386B2511303;
    address private ListingLiquidityRewards =
        0x222832a5D2C22Aa2005989B0b4a96981B051Aa61;

    //address private PreSale = 0x1aE0EA34a72D944a8C7603FfB3eC30a6669E454C;
    //address private publicSale = 0x0A098Eda01Ce92ff4A4CCb7A4fFFb5A43EBC70DC;

    constructor() ERC20("Miyamoto", "MYA", 18) {
    }

    function initialize() public onlyOwner initializer{
        _mint(msg.sender, _initialSupply);
        _mint(
            PRBrandingIncentivesMarketing,
            PRBrandingIncentivesMarketingAmount
        );
        _mint(TeamAdvisors, TeamAdvisorsAmount);
        _mint(FoundationReserve, FoundationReserveAmount);
        _mint(SeedSale, SeedSaleAmount);
        _mint(StrategicSale, StrategicSaleAmount);
        _mint(PromotionPartnerships, PromotionPartnershipsAmount);
        _mint(ListingLiquidityRewards, ListingLiquidityRewardsAmount);
        // _mint(PreSale, PreSaleAmount);
        // _mint(publicSale, PublicSaleAmount);
        _mint(address(this), 40000000 * (10**uint256(18)));
    }
    ...
    function _burnFrom(address account_, uint256 amount_) public virtual {
     ...
     modifier checkSaleValidations(address _userAddress, uint256 _value) {
        ...
        require(
            _value > saleMinimumAmount && _value < saleMaximumAmount,
            "Amount not appropriate"
        );
        require(
            _value.mul(tokenPrice) <= saleMaximumAmount,
            "Amount Exceeded Maximum Purchaseable Amount"
        );
        require(
            _value.mul(tokenPrice) >= saleMinimumAmount,
            "Amount is not sufficient for this round"
        );
        _;
    }
        ...
        returns (bool)
    {
        ...
        uint256 _amount = msg.value.mul(tokenPrice);
