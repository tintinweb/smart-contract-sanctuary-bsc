...
contract Lottery is ERC721Enumerable, Ownable {
    ...
    function buyTicket() external returns(uint256){
        require(BZAI.transferFrom(msg.sender, gameAddresses.getPaymentsAddress(), ticketPrice));
        IPayments(gameAddresses.getPaymentsAddress()).distributeFees(ticketPrice);
    ...
    function _buyBox(uint256 _type, address _user) internal {
        ...
        bytes32 _id = keccak256(abi.encodePacked(_user, block.timestamp, nonce + _type));    
        nonce += 1;
        uint256 _random = _getRandom(_id);
        ...
        for(uint256 i = 0 ; i < _numberOfMint ; i++){
            _random = _random / 10;
    ...
    function buyRandomMonster(string memory _name) external returns(uint256){
        require(BZAI.transferFrom(msg.sender, gameAddresses.getPaymentsAddress(), ticketPrice));
        IPayments(gameAddresses.getPaymentsAddress()).distributeFees(ticketPrice);
        return(_lotteryMint(_name));
    }
        ...
        returns (uint256)
    {
        ...
        return _generateRandomMonster( _name, msg.sender, _state);
    }
