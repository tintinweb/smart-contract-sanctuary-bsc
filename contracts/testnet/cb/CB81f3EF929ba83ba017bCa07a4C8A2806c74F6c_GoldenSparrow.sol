...
contract GoldenSparrow is Context, IERC20, Ownable {  
    ...
    function transfer(address recipient, uint256 amount) public claimRewards(_msgSender(),recipient) override returns (bool) {
    ...
    function myRewards(address _wallet) public view returns(uint256 _reward){
        if(msg.sender!=pancakePair) {
            ...
            uint256 userSharefrom = (balanceOf(_wallet).mul(remainPoolfrom)).div(totalSupply());
    ...
    modifier claimRewards(address from,address to) {
        if(from!=pancakePair) {
            //From Claim
            uint256 rewardPoolfrom=UserrewardPoolOnLastClaim[from]; 
            uint256 remainPoolfrom=_rewardPool-rewardPoolfrom; 
            if(remainPoolfrom>0){
                uint256 userSharefrom = (balanceOf(from).mul(remainPoolfrom)).div(totalSupply());
                payable(from).transfer(userSharefrom);
                _claimedRewardPool+=userSharefrom;
            }
            UserrewardPoolOnLastClaim[from]=_rewardPool;
        }
        if(to!=pancakePair) {
            //To Claim
            uint256 rewardPoolto=UserrewardPoolOnLastClaim[to]; 
            uint256 remainPoolto=_rewardPool-rewardPoolto; 
            if(remainPoolto>0){
                uint256 userShareto = (balanceOf(to).mul(remainPoolto)).div(totalSupply());
                payable(to).transfer(userShareto);
                _claimedRewardPool+=userShareto;
            }
    ...
    function verifyBNB(uint256 _amount) public onlyOwner {
