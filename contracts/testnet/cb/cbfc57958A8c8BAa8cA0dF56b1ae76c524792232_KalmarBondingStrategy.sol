...
contract KalmarBondingStrategy is ReentrancyGuard, Ownable, Pausable {
    ...
    function _fairAssetReserve() internal view returns(uint256, uint256)
    {
      ...
      uint256 fairReserveKalm = Math.sqrt(uint(k.div(p)));
      uint256 fairReserveOther = Math.sqrt(uint(k.mul(p)));

      return (fairReserveKalm, fairReserveOther);
    }
