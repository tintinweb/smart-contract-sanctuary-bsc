...
contract SparkLifeP2p is Ownable {
    ...
    */
    function offertEscrow(address sellerAddress, uint ID) public returns (bool) {
        ...
        if (sellerEscrowDatabase[sellerAddress][ID].type_escrow == 0) {
            ...
            // Depositar el token sps al contrato p2p
            require(spsToken.transferFrom(msg.sender, address(this), sellerEscrowDatabase[sellerAddress][ID].weiAmmount), "Can't deposit SPS");
        }
    ...
    */
    ...
    /**
        ...
        Si es llamada por el buyer, se debe de enviar el id del buyer y no del seller, es porque se debe de llamar desde el listado de buyer que tiene IDs diferentes a los de seller
        si es llamada por el seller, el id es el id del escrow del seller
    */
