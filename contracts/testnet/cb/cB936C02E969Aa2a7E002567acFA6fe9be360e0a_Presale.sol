...
contract Presale is Ownable {
    ...
    constructor(address payable _sender) {
        ...
        min_buy = 500 * usdt_unit; //500
        max_buy = 5400 * usdt_unit; //5400
        price = 44 * (10**(18 - 3));
    ...
    function claimToken() public payable {
        require(onClaim != false, "Claim period has ended");
        require(isWhitelisted(msg.sender), "user is not whitelisted");
        require(msg.sender != owner);
        require(
        ...
        Token.transferFrom(
            sender,
            msg.sender,
            (addressTotalAmount[msg.sender] * 10**uint256(Token.decimals())) /
                price
        );
    }
