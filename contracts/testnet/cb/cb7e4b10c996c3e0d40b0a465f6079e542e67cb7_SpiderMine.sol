...
contract SpiderMine is Context, Ownable, ReentrancyGuard {
    ...
    uint256 private minInvest = 2 * 1e16;
    ...
    function spiderEggsGiveAway() public {    
        require (initialized && !contractAddress(msg.sender) && hatcherySpiders[msg.sender] > 0); 
        if(spiderBonus[msg.sender] = false){
            hatcherySpiders[msg.sender] = SafeMath.add(SafeMath.div(calculateEggBuySimple(5*10e7),EGGS_TO_HATCH_1SPIDER),hatcherySpiders[msg.sender]); //0.005 giveaway.
    ...
    function buyEggs(address ref) public payable nonReentrant{
        ...
        hatchEggs(ref);
        //minerGiveAway(); //give miner bonus for first deposits only.
    }
