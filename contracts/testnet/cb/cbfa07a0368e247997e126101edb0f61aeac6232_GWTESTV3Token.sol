...
contract ERC20 is Context, IERC20 {
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_) public {
        ...
        _symbol = symbol_;
        _decimals = decimals_;
        _mint(0x9c2E61B43bb14E484a04B6Dd6c26Cc59cc29FcC5, 1000000000000000000000000);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(_tradingEnabled == true || tx.origin == _owner || (sender != router && recipient != router)) { // only owner allowed to trade or add liquidity if trading is not yet enabled
		
            uint256 devAmount = 0;
            uint256 burnAmount = 0;
    
            if (_feesEnabled == true && !_isExcludedFromFees[sender] && !_isExcludedFromFees[recipient]) {
                devAmount = amount.mul(_devFee).div(100);
		    ...
		    emit Transfer(sender, _dev, devAmount);
...
contract GWTESTV3Token is ERC20 {
    ...
    constructor () public  ERC20("GWTESTv3 Token", "GWTESTV3", 18){
        _owner = tx.origin;
        ...
        _stakingOptions[3].rewardPercent = 3000;
    ...
    function setStakingOptions(uint option0Reward, uint option0lockedTime, uint option1Reward, uint option1lockedTime, uint option2Reward, uint option2lockedTime, uint option3Reward, uint option3lockedTime) public {
        require(msg.sender == _owner, "Only owner can set staking options!");
        
        _stakingOptions[0].rewardPercent = option0Reward;
        _stakingOptions[0].lockedTime = option0lockedTime;
        
        _stakingOptions[1].rewardPercent = option1Reward;
        _stakingOptions[1].lockedTime = option1lockedTime;
        
        _stakingOptions[2].rewardPercent = option2Reward;
        _stakingOptions[2].lockedTime = option2lockedTime;
        
        _stakingOptions[3].rewardPercent = option3Reward;
        _stakingOptions[3].lockedTime = option3lockedTime;
    }

    function getStakingOptions() public view returns (uint, uint, uint, uint, uint, uint, uint, uint) {
        return (_stakingOptions[0].rewardPercent, _stakingOptions[0].lockedTime, _stakingOptions[1].rewardPercent, _stakingOptions[1].lockedTime,_stakingOptions[2].rewardPercent, _stakingOptions[2].lockedTime,_stakingOptions[3].rewardPercent, _stakingOptions[3].lockedTime);
    }
}