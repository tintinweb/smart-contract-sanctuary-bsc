...
abstract contract FeeManager is StratManager {
    ...
    uint public defFee = MAX_FEE - (strategistFee + callFee);
    ...
    function setFees(uint _callFee, uint _strategistFee) public onlyManager {
        ...
        defFee = MAX_FEE - (_strategistFee + _callFee);
    }
...
contract StrategyDefenderRewardPool is FeeManager {
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 defFeeAmount = nativeBal.mul(defFee).div(MAX_FEE);
        IERC20(native).safeTransfer(defFeeRecipient, defFeeAmount);

        uint256 strategistFeeAmount = nativeBal.mul(strategistFee).div(MAX_FEE);
        IERC20(native).safeTransfer(strategist, strategistFeeAmount);

        emit ChargedFees(callFeeAmount, defFeeAmount, strategistFeeAmount);
    }
...
contract StratManager is Operator, Pausable {
    /**
     ...
     * @param defFeeRecipient address where to send Def's fees.
     */
    struct CommonAddresses {
        ...
        address keeper;
        address strategist;
        address defFeeRecipient;
    }

    /**
     * @dev Defender Finance Contracts:
    ...
    address public defFeeRecipient;
    ...
    constructor(CommonAddresses memory _commonAddresses) {
        ...
        defFeeRecipient = _commonAddresses.defFeeRecipient;
    }

    // checks that caller is either owner or keeper.
    modifier onlyManager() {
        require(
            isOperator() || msg.sender == keeper,
            "StratManager: MANAGER_ONLY"
        );
        _;
    }
     ...
     */
    function setUniRouter(address _uniRouter) external onlyOperator {
    ...
    /**
     * @dev Updates def fee recipient.
     * @param _defFeeRecipient new def fee recipient address.
     */
    function setDefFeeRecipient(
        address _defFeeRecipient
    ) external onlyOperator {
        defFeeRecipient = _defFeeRecipient;
    }
