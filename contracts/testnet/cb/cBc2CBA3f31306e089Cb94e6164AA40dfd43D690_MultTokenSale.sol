...
contract MultTokenSale is Context, IBEP20, Ownable {
  ...
  function tokensAccept() external view returns(string[] memory) {
     ...
     */
  function getpriceBNB(uint amount) public view virtual returns (uint price) {
    uint256 resPrice = 0;
    if(getContract("BNB") == _bnbBusdPair.token0()){
      (uint256 ResBNB,,) = _bnbBusdPair.getReserves();
      uint256 totBusd = amount;
      resPrice = totBusd.div(ResBNB);
    }else{
      (,uint256 ResBNB,) = _bnbBusdPair.getReserves();
      uint256 totBusd = amount;
      resPrice = totBusd.div(ResBNB);
    }
    return(resPrice); // return amount of BNB needed to buy Box
  }
