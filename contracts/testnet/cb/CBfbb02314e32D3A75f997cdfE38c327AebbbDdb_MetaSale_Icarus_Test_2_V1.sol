...
interface IErc721Contract {
    ...
    function mintBotz2(uint256 count, address recepient) external;
}

contract MetaSale_Icarus_Test_2_V1 is AccessControl {
    ...
    uint256 public PRICE_BNB = .0001 ether; 
    uint256 public SUPPLY = 500;
    
    bool public isSaleClosed = false;

    //Purchase BNB
    function purchaseBNB(uint256 count) external payable {
        require(count > 0, "No token to mint");
        require(msg.sender != address(0), "Zero address");
        require(isSaleClosed == false, "NFT Sale Closed!");
        require(SUPPLY > 0, "Out of supply");
        require(msg.value == (PRICE_BNB * count), "Not enough BNB");
       
        IErc721Contract nftContract = IErc721Contract(_nftnContractAddress);
       
        nftContract.mintBotz2(count, msg.sender);
        SUPPLY = SUPPLY - count;
    }
    
    //Fetch Data
    function retSupply() external view returns(uint256 _supply){
        return SUPPLY;
    }
    //Setters
    function _setSaleStatus() external onlyRole(DEFAULT_ADMIN_ROLE){
        isSaleClosed = !isSaleClosed;
    }

    function _setPriceBNB(uint256 price) external onlyRole(DEFAULT_ADMIN_ROLE){
        PRICE_BNB = price;
    }

    function _setSupply(uint256 supply) external onlyRole(DEFAULT_ADMIN_ROLE){
        SUPPLY = supply;
    }
