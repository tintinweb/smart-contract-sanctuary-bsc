    ...
    function recordStake() internal returns (uint256 base) {
        ...
        if (stakeTimeStamp == 0) {
            ...
            if (block.timestamp > stakeTimeStamp) {
                ...
                base = YiBoxBase.GetLastStakeNode(YiBoxType.KeyStakeNode) + calcIncome(1,_delay,_laststake,lastAirDrop);
...
/*
    ...
    function _addstake(address _staker, uint256 _stakeNum) internal returns(uint256 _income) {
        require(_staker != address(0), "K2004: not a real user ... ");
        uint256 hasLP = queryLP(_staker);
        require(hasLP >= _stakeNum,"K2007: not enough LP ... ");

        _income = updateStaker(true, _staker, _stakeNum, block.timestamp);
...
*/
    ...
    function subStake(uint256 _stakeNum) external isOpen returns(uint256 _income) {
        ...
        KeyStake memory _ks = YiBoxBase.GetKeyStake(_staker);
        require(_ks.lastTime == 0, "K2014: Can't find staker ...");
        require(_ks.Amount >= _stakeNum, "K2015: Person stake Insufficient balance ...");
    ...
    function getOutLP() external lock {
        ...
        require(mainPool != address(0),"K2010: mainPool error ...");
        (uint256 hasLP,) = this.getLPOnStake();//queryLP(_staker);
        require(hasLP >= _ks.Amount,"K2013:  not enough LP ... ");
        
        uint256 _delay = block.timestamp - _ks.lastTime;
        uint256 _p = 0;
        uint256 _f = 0;
        (_f,_p) = fee(_ks.Amount, _delay);
        //将在本池质押的LP退还给用户
        aPancakePair.transfer(_staker, _p);
        aPancakePair.transfer(mainPool, _f);
    }
    ...
    function queryIncomeNow(KeyStake memory _ks) internal view returns(uint256 IncomeNow){
        ...
        if (block.timestamp <= stakeTimeStamp) {
            ...
            (,uint256 _laststake) = this.getLPOnStake();
            uint256 base = YiBoxBase.GetLastStakeNode(YiBoxType.KeyStakeNode) - YiBoxBase.GetParam(YiBoxType.KeyStakeNode,_ks.lastTime) + calcIncome(1, block.timestamp - stakeTimeStamp,_laststake,lastAirDrop);
            IncomeNow = base * _ks.Amount; 
        }
