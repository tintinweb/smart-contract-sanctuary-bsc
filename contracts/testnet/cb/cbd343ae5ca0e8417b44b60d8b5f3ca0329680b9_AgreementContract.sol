...
contract AgreementContract is Ownable {
    ...
    mapping(uint256 => milestone) public milestoneDetails;
    ...
    function CreateContract(
        ...
        for (uint256 i = 0; i < Mile_Price.length; i++) {
            ...
            milestoneDetails[i].id = i;
            milestoneDetails[i].taskName = _taskName[i];
            milestoneDetails[i].Priority =priority[i];
            milestoneDetails[i].due_Date = due_date[i];
    ...
    function shareOption(uint256 agreementID,uint256 _employerID)public{
    ...
    function Approve(
        ...
        require(agreementDetails[_agreementID].employerID == _employerID, "Only Employer");
    ...
    function depositFunds(uint256 _milesID, uint256 _agreementID) public payable {
        ...
        require(milestoneDetails[_milesID].Approve != true, "Milestone already delivered");
        require(msg.value == getMilestoneBNBAmount(_milesID), "Incorrect amount");
        // require(msg.value<=escrowAmount,"Amount is higher than escrowAmount");
    ...
    function deliverMilestone(
        ...
        require(agreementDetails[_agreementID].employeeID == _employeeID, "Only Employee");
        require(contractCancelled[_agreementID] != true, "Agreement cancelled");
        require(milestoneDetails[_milesID].Approve != true, "Milestone already accepted");

        for (uint256 i = 0; i < agreementDetails[_agreementID].milstones.length; i++) {
            if (_milesID == agreementDetails[_agreementID].milstones[i]) {
                milestoneDetails[_milesID].delivered = true;
                milestoneDetails[_milesID].submission_time = _submissionTime;
            }
        }
    }
    ...
    function approveMilestone(
        ...
        require(agreementDetails[_agreementID].employerID == _employerID, "Only Employer");
        require(contractCancelled[_agreementID] != true, "Agreement cancelled");
        require(milestoneDetails[_milesID].Approve != true, "Milestone already approved");
        require(milestoneDetails[_milesID].delivered == true, "Milestone not delivered");
        ...
        for (uint256 i = 0; i < agreementDetails[_agreementID].milstones.length; i++) {
            if (_milesID == agreementDetails[_agreementID].milstones[i]) {
                milestoneDetails[_milesID].Approve = status;
                
                if(status == true) {
                    uint amount = milestoneDetails[i].Mile_Price.mul(1e18).div(getLatestPrice());
    ...
    function getMilestoneBNBAmount(uint256 _mileID) public view returns(uint256){
        
        milestone storage mile = milestoneDetails[_mileID];
    ...
    function ValidateMilestone(
        ...
        require(milestoneDetails[milestoneID].delivered == true, "it must be delivered");
        require(milestoneDetails[milestoneID].Approve != true,"Milestone already accepted");
        ...
        for (
            ...
            if (validaterid == agreementDetails[agreementID].validatersID[i]) {
                milestoneDetails[milestoneID].validated = true;
            }
        }
    }
    ...
    function raiseDispute(uint256 milesid, uint256 agreementID, Role role) public {
        ...
        require(milestoneDetails[milesid].validated == true,"it must be validated");
        
        if (role == Role.employee) {
            require(milestoneDetails[milesid].delivered == true, "Milestone is not delivered");
            require(milestoneDetails[milesid].Approve != true, "Check your Approval status");
            claimed[agreementID][role] = true;

        } else if (role == Role.employer) {
            require(milestoneDetails[milesid].Approve != true, "Check your Approval status");
            require(escrowAmount >= milestoneDetails[milesid].Mile_Price, "Funds not escrowed");
            claimed[agreementID][role] = true;
