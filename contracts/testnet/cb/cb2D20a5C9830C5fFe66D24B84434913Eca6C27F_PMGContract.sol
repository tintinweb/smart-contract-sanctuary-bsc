...
contract PMGContract {
    ...
    function bidNFT(uint256 _auctionItemId,uint256 _price) public payable {
        ...
        if(auctionitem.cryptoToken == 0xDfb1211E2694193df5765d54350e1145FD2404A1){
            if(balance - freezeBalance< auctionitem.price){
                ...
                IStrategy(strategy[auctionitem.cryptoToken]).directDeposit{value:msg.value}(_price  ,msg.sender,1);
            }else{
    ...
    function getUserBalance(address _address , address _tokenContract) public view returns(uint256,uint256,uint256,uint256){
        (uint256 balance,uint256 freezeBalance,uint256 share,uint256 wanToShare) = IStrategy(strategy[_tokenContract]).userInfos(_address);
        return (balance,freezeBalance,share,wanToShare);
    }
