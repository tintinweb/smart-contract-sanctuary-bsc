...
contract CamelNFT is ERC721r, ReentrancyGuard {
    ...
    constructor(address _tokenAddress, string memory uri, uint _price) ERC721r("Al-Hejin","ALH",99){
     baseUri=uri;
    ...
    function setNftPrice(uint _price) onlyOwner public{
        uint oldPrice = price;
        price = _price;
...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function updateMaxSupply(uint MaxSupply) onlyOwner() public {
        _maxSupply = _maxSupply + MaxSupply;
        _numAvailableTokens = _numAvailableTokens + MaxSupply;
    }

    function getUserTokenIDS(address _user) public view returns(uint[] memory TokenIDS){
        uint[] memory user = new uint[](userTokenIDS[_user].length);
        for(uint i=0; i< userTokenIDS[_user].length; i++){
            user[i] = userTokenIDS[_user][i];
        }
        TokenIDS = user;
    }
