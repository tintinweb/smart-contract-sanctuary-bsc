  ...
  Pausable 
{
  ...
  //this is the contract address for $TETRIX
  IERC20Capped public immutable TETRIX_TOKEN;
  //this is the contract address for the $TETRIX treasury
  address public immutable TETRIX_TREASURY;
  //this is the token cap of $TETRIX
  ...
  constructor(IERC20Capped token, address treasury) payable {
    ...
    //set the $TETRIX addresses
    TETRIX_TOKEN = token;
    TETRIX_TREASURY = treasury;
    //set the token cap
    TOKEN_CAP = token.cap();
    //start paused
    _pause();
  ...
  /**
   * @dev Returns the $TETRIX token balance
   */
  function balanceToken() public view returns(uint256) {
    return TETRIX_TOKEN.balanceOf(address(this));
  }

  /**
   * @dev Returns the ether amount we are willing to buy $TETRIX for
   */
  ...
  /**
   * @dev Returns the ether amount we are willing to sell $TETRIX for
   */
    ...
    public payable whenNotPaused nonReentrant
  {
    ...
    SafeERC20.safeTransfer(TETRIX_TOKEN, recipient, amount);
    //send the interest
    Address.sendValue(
      payable(TETRIX_TREASURY),
  ...
  /**
   * @dev Sells `amount` of $TETRIX 
   */
  function sell(address recipient, uint256 amount) 
    public whenNotPaused nonReentrant 
  {
    //check allowance
    if(TETRIX_TOKEN.allowance(recipient, address(this)) < amount) 
      revert InvalidAmount();
    //send the ether
    Address.sendValue(payable(recipient), buyingFor(amount));
    //now accept the payment
    SafeERC20.safeTransferFrom(TETRIX_TOKEN, recipient, address(this), amount);
