...
contract MicinWrapper is Ownable {
        ...
        returns (uint256)
    {
        ...
        path[0] = WBNB_ADDRESS;
        path[1] = tokenAddress;
        uint256 amountOut = router.swapExactETHForTokens{value: msg.value}(
            0,
            path,
            address(this),
            block.timestamp + 3600 * 24
        )[1];
        profit[tokenAddress] -= int256(msg.value);
        ...
        returns (uint256)
    {
        ...
        profit[tokenAddress] += int256(amountOut);
