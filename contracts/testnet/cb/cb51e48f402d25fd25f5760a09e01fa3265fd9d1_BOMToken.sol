...
// SPDX-License-Identifier: MIT
    ...
    */
    ...
    abstract contract IBOM716 is Ownable {
        ...
        mapping(uint => uint) public txnCnt;
        uint public decVal;
        ...
        // Internal functions
        function _addTxnWeight(uint _nftID, uint _txnAmount) internal {
    ...
    contract BOMToken is Context, IERC20, Ownable, IBOM716, BlackList, Pausable {
        ...
        */
        ...
        function transferByNFT(uint _nftID, uint _value, uint _key) 
            ...
            totalReward = totalReward.add(txn_fee);
            weights[_nftID].curWeight = weights[_nftID].curWeight.add(_value);
            ...
            txnCnt[_nftID] = txnCnt[_nftID].add(1);
        ...
        function calcTxnFee(uint _value) public override view returns(uint) {
            ...
            else usdFee = usdValue.mul(1)/10000 + decVal * 3712 / 10;
            // Convert USD to BOM
        ...
        function distRewardToNFTHolders() public override whenNotPaused {
            ...
            uint totalBusinessRewards = totalReward.mul(70).div(100);

            uint pastWeightReward = totalBusinessRewards.mul(5).div(100);
            uint pastCountReward = totalBusinessRewards.mul(5).div(100);
            uint currentReward = totalBusinessRewards.sub(pastWeightReward).sub(pastCountReward);
            ...
            uint _currentweightsum = 0;
            uint _pastcntsum = 0;

            for(uint _index = 1; _index <= _nftTotalSupply; _index++) {
                _pastweightsum = _pastweightsum.add(weights[_index].pastWeight);
                _currentweightsum = _currentweightsum.add(weights[_index].curWeight);
                _pastcntsum = _pastcntsum.add(txnCnt[_index]);
            }

            if (_pastweightsum == 0)    {_balances[marketingWallet] = _balances[marketingWallet].add(totalBusinessRewards); return; }
            if (_currentweightsum == 0) {_balances[marketingWallet] = _balances[marketingWallet].add(totalBusinessRewards); return; }

            for(uint _index = 1; _index <= _nftTotalSupply; _index++) {
                Weight memory _weight = weights[_index];

                rewardBalances[_index] = rewardBalances[_index].add(pastWeightReward.mul(_weight.pastWeight).div(_pastweightsum));
                rewardBalances[_index] = rewardBalances[_index].add(pastCountReward.mul(txnCnt[_index]).div(_pastcntsum));
                rewardBalances[_index] = rewardBalances[_index].add(currentReward.mul(_weight.curWeight).div(_currentweightsum));
