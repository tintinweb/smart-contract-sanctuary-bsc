...
//The treasury is a step above a simple burn address 
...
//         //we should rebalance when we get more than target percentage of the supply in the treasury
...
//         //we should rebalance when we get more than 55% of the supply in the treasury
//         uint256 upperbound = token.totalSupply().mul(upperboundPercentage).div(100);
//         uint256 balance = token.balanceOf(address(this));
...
contract BirdCrypto is  IERC20 {
    ...
    */
    ...
    uint256 private numTokensSellToAddToLiquidity = 500000 ether; //500k 
    ...
    constructor () {
        ...
        //owner start out owning all the tokens 
        _rOwned[_owner] = _rTotal;
        emit Transfer(address(0), _owner, totalSupply);
    }

    function setRouter (address exchangeRouter) public onlyOwner {        
        require(exchangeRouter != address(0), "Exchange must be supplied");        

        //Core Setup
        IPancakeSwapRouter _swapRouter = IPancakeSwapRouter(exchangeRouter);
        // set the rest of the contract variables
        swapRouter = _swapRouter;

        // Create a pancakeswap pair for this new token
        // setTokenB(_swapRouter.WETH());
        // swapPair = IPancakeSwapFactory(_swapRouter.factory())
    ...
    function _setSwapPair() private{
        require(address(swapRouter) != address(0), "swapRouter must be supplied");   
    ...
    function setTokenB(address _tokenB) public onlyOwner{
        require(_tokenB != address(0), "tokenB must be supplied");           
        tokenB = _tokenB;
        _setSwapPair();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            swapTokensForTokenB(contractTokenBalance);
        }
    ...
    function swapForTokenBAndLiquify(uint256 amount) private lockTheSwap {
        // split the contract balance into halves
        uint256 half = amount.div(2);
        uint256 otherHalf = amount.sub(half);
        ...
        swapTokensForTokenB(half); // <- this breaks the tokenB -> HATE swap when swap+liquify is triggered

        // how much tokenB did we just swap into?
        uint256 tokenBAmount = IERC20(tokenB).balanceOf(address(this)).sub(initialBalance);

        // add liquidity to pancakeswap
        addTokenBLiquidity(otherHalf, tokenBAmount);
        
        emit SwapAndLiquify(half, tokenBAmount, otherHalf);
    }
    ...
    function swapTokensForTokenB(uint256 tokenAmount) private {
        // generate the pancakeswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = tokenB;
    ...
    function addTokenBLiquidity(uint256 tokenAmount, uint256 tokenBAmount) private {
