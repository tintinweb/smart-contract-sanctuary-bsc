...
contract WR is ERC20, Ownable {
    ...
    address public devWallet = 0x0b2e68B2Fc3332677CFdaDa1510E752306664eAe;
    ...
    uint256 public totalFees = busdDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee).add(devFee);
    ...
    constructor() ERC20("Wise Reflect", "WR") {
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "WR: The marketing wallet is already this address");
    ...
    function updateDevWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != devWallet, "WR: The dev wallet is already this address");
    ...
    function updatebusdTokenDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdTokenDividendTracker), "WR: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdTokenDividendTracker =  BUSDDividendTracker(payable(newAddress));
 
        require(newbusdTokenDividendTracker.owner() == address(this), "WR: The new dividend tracker must be owned by the WR token contract");
    ...
    function updateRc1DividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(adaDividendTracker), "WR: The dividend tracker already has that address");
 
        ADADividendTracker newadaDividendTracker = ADADividendTracker(payable(newAddress));
 
        require(newadaDividendTracker.owner() == address(this), "WR: The new dividend tracker must be owned by the WR token contract");
    ...
    function updateBusdDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = busdDividendRewardsFee.add(marketingFee).add(adaDividendRewardsFee).add(liquidityFee).add(devFee);
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "WR: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WR: The PanadaSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "WR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "WR: Cannot update gasForProcessing to same value");
...
contract ADADividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("RC2_ADA_Dividend_Tracker", "RC2_ADA_Dividend_Tracker", 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "RC2_ADA_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "RC2_ADA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "RC2_ADA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "RC2_ADA_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BUSDDividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() pure public override {
        require(false, "RC2_BUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WR contract.");
    }
