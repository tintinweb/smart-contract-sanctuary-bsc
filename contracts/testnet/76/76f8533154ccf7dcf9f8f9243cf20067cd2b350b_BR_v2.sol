...
// SPDX-License-Identifier: MIT
    ...
    interface BnBRevolucionInterface {
        ...
        function getSiteInfo(address _adr) external view returns (
    ...
    /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
        ...
        function GetSetSiteInfoOfOldContract(address _OldContract) external nonReentrant{
            require(msg.sender == owner, "Admin use only");
            uint256 _totalDeposits;
            uint256 _totalCompound; 
            uint256 _totalRefBonus; 
            uint256 _totalLotteryBonus;
            ( , _totalDeposits, _totalCompound, _totalRefBonus, _totalLotteryBonus) = bnbRevContractOld.getSiteInfo(_OldContract);
