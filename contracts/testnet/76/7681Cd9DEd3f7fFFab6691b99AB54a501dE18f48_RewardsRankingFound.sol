...
contract RewardsRankingFound is Ownable { 
 ...
 IAddresses public gameAddresses;
    ...
    uint256 public weeklyPercentageReward = 20; // 0,2%
    uint256 public dailyPercentageReward = 2;// 0,02%
    ...
    modifier onlyGame(){
        require(
            msg.sender == gameAddresses.getRankingContract()
            ,"only Rewards contract auth");
        _;
    }

    function setPercentages(uint256 _daily, uint256 _weekly) external onlyOwner{
        require(_daily <= 500 && _weekly <= 500 && _daily >= 1 && _weekly >= 1, "values forbiden");
    ...
    function _updateRewards() private returns(bool){
        IReserveForChalengeRewards I = IReserveForChalengeRewards(gameAddresses.getReserveChallengeAddress());

        if(_lastRewardUpdate == 0){
            _lastRewardUpdate = block.timestamp;
            return true;
        }else if(I.getRewardFinished()){
            reserveFinished = true;
        }else if(block.timestamp >= (_lastRewardUpdate + 30 days)){
            _lastRewardUpdate = block.timestamp;
            return I.updateRewards();
        }
        return true;

    }

    function getDailyRewards(address _rewardStoringAddress) external onlyGame returns(uint256){
        if(!reserveFinished){
            _updateRewards();
        }
        uint256 _toSend = BZAI.balanceOf(address(this)) * dailyPercentageReward  / 10000;
        require(BZAI.transfer(_rewardStoringAddress,_toSend));
        return _toSend;
    }

    function getWeeklyRewards(address _rewardStoringAddress) external onlyGame returns(uint256){
        if(!reserveFinished){
            _updateRewards();
        }
        uint256 _toSend = BZAI.balanceOf(address(this)) * weeklyPercentageReward / 10000;
        require(BZAI.transfer(_rewardStoringAddress,_toSend));
        return _toSend;
    }
