...
contract rev3alFarm is Ownable, ReentrancyGuard {
    ...
    constructor(IERC20 _rev3al, uint256 _rewardPerBlock) {
        rev3al = _rev3al;
    ...
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 rev3alReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTokenPerShare = accTokenPerShare.add(rev3alReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 rev3alReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(rev3alReward.mul(1e12).div(lpSupply));
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 rev3alBal = rev3al.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > rev3alBal) {
            transferSuccess = rev3al.transfer(_to, rev3alBal);
        } else {
            transferSuccess = rev3al.transfer(_to, _amount);
        }
    ...
    function withdrawTokens(IERC20 _tokenAddress) external onlyOwner {
