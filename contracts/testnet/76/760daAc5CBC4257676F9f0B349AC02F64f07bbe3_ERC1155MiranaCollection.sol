...
abstract contract MinimalMiranaBaseCollection is OwnableUpgradeable {
  ...
  function setLazyTransferProxyAddress(address _lazyTransferProxyAddress)
    external
    onlyOwner
  {
  ...
  RoyaltiesV2Impl
{
  ...
  function mint(address recipient, uint256 tokenId, string memory uri, uint256 amount) external onlyOwner {
    uint256 id;
    if (tokenId != 0) {
      id = tokenId;
    } else {
