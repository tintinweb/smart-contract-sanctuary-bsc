...
contract BitdriveLaunchpad is ReentrancyGuard, Initializable {
  ...
  function deposit(uint256 _amount) public {
    ...
    require (userInfo[msg.sender].tier==0, 'already purchased');

    uint256 transferAmt= _amount*1e18;
    uint256 balance = IBEP20(lpToken).balanceOf(msg.sender);
    require (balance >= transferAmt, 'insufficient balance');
  ...
  function harvestReward(uint256 _amount) public nonReentrant {
    ...
    require (currentTier>1, 'invalid tier');
    ...
    uint convertPrice = _amount.mul(tokenPrice);
    uint transferAmt= convertPrice*1e18;
  ...
  function tokenAllocatedPublicSale() public view returns (uint) {
        ...
        uint perUser=balanceAmount.div(totalUsers);
        perUser = perUser;
        return perUser;
  }
  ...
  function harvestPublicReward(uint256 _amount) public nonReentrant {
    ...
    uint convertPrice = _amount.mul(tokenPrice);
    uint256 transferAmt= convertPrice*1e18;
    offeringToken.safeTransfer(address(msg.sender), transferAmt);
  ...
  function round3Deposit(uint256 _amount) public {
    ...
    uint maxInvest=balanceAmount/tokenPrice;
    require (_amount<=maxInvest, 'invalid invest amount');
    require (_amount > 0, 'need _amount > 0');
    uint256 transferAmt= _amount.mul(1e18);
    lpToken.safeTransferFrom(address(msg.sender), address(this), transferAmt);

    uint cal = _amount.mul(instantpercentage).div(100);
    uint convertPrice = cal.mul(tokenPrice);
    
    uint256 transferReward= convertPrice.mul(1e18);
    offeringToken.safeTransfer(address(msg.sender), transferReward);
  ...
  function round3Harvest() public {
      ...
      userInfo[msg.sender].round3withdrawtime = block.timestamp;
      uint totalPrice = available.mul(tokenPrice);
      uint256 transferReward= totalPrice.mul(1e18);
      offeringToken.safeTransfer(address(msg.sender), transferReward);
  ...
  function availableMcar(address _user) public view returns (uint256) {
      uint diff = userInfo[_user].round3withdrawtime-block.timestamp;
      uint invest = userInfo[_user].round3invest;
      uint withdraw = userInfo[_user].round3withdraw;
      uint balance = invest-withdraw;
      
      uint256 available = 0;
      if( block.timestamp < userInfo[_user].round3withdrawtime && diff > PERIOD_LENGTH ){
            uint calcMcar = diff / PERIOD_LENGTH;
            if(calcMcar >= 4){
                available = balance;
            }else{
                available = uint256(calcMcar) * balance.mul(instantpercentage)/100;
            }
      }else{
          available = 0;
      }
  ...
  function availableMcarTest(address _user) public view returns (uint256,uint256,uint256,uint256,uint256,uint256) {
      uint diff = userInfo[_user].round3withdrawtime-block.timestamp;
    //   uint round3withdrawtime1 = userInfo[_user].round3withdrawtime;
    //   uint timestamp = block.timestamp;
    //   uint diff1 = round3withdrawtime1-timestamp;
      uint invest = userInfo[_user].round3invest;
      uint withdraw = userInfo[_user].round3withdraw;
      uint balance = invest-withdraw;
      ...
      return (diff,invest,withdraw,balance,calcMcar,available);
  }
