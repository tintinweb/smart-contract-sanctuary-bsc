...
contract BlocVaultVesting is Ownable{
    ...
    uint256 private TIME_UNIT = 1 minutes; // testing
    ...
    function initialize(IERC20 _token, address _reflectionToken, uint256 _duration) external onlyOwner {
    ...
    function vest(uint256 amount) external onlyActive {
        ...
        vestingInfos[msg.sender][lastIndex] = VestingInfo(
            ...
            duration,
            block.timestamp,
            block.timestamp.add(duration.mul(TIME_UNIT)),
            block.timestamp,
            0,
            0,
            0
        ...
        emit Vested(msg.sender, lastIndex, amount, duration);
    }

    function revoke(uint256 _vestId) external onlyActive {
    ...
    function release(uint256 _vestId) external onlyActive {
        VestingInfo storage _vest = vestingInfos[msg.sender][_vestId];
    ...
    function claimDividend(uint256 _vestId) external onlyActive {
        VestingInfo storage _vest = vestingInfos[msg.sender][_vestId];
        require(_vest.amount > 0 && _vest.status == 0, "Not available");
    ...
    function claimReward(uint256 _vestId) external onlyActive {
        VestingInfo storage _vest = vestingInfos[msg.sender][_vestId];
        require(_vest.amount > 0 && _vest.status == 0, "Not available");
        require(block.timestamp.sub(_vest.lastHarvestTime) < harvestCycle.mul(TIME_UNIT), "Cannot harvest in 7 days after last harvest");
    ...
    function calcReward(uint256 _amount, uint256 _lockedTime, uint256 _releaseTime, uint256 _rewardDebt) public view returns(uint256 reward) {
    ...
    function harvest() external onlyOwner {
        ...
        uint256 tokenAmt = vestingToken.balanceOf(address(this));
        uint256 reflectionAmt = (tokenAmt.sub(totalVested)).mul(accReflectionPerShare).div(PRECISION_FACTOR).sub(reflectionDebt);
        if(reflectionAmt > 0) {
            payable(msg.sender).transfer(reflectionAmt);
        } else {
            IERC20(reflectionToken).safeTransfer(msg.sender, reflectionAmt);
