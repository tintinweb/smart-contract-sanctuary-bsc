...
contract LimitTimeAuction is Member {
    ...
    mapping(address => uint256[]) public ownerTokensList;
    mapping(address => mapping(uint256=>uint256)) public ownerTokensListIndexs;
    uint256[] public makerOrdersList;  // 紀錄所有訂單 MakerOrdersIndexs[index]: tokenId
    mapping(uint256=>uint256) public makerOrdersListIndexs;

    mapping(uint256 => AuctionOrder) private makerOrders;
    uint256 public funderFee = 20; // 項目方2%
    uint256 public nftOwnerFee = 20; // 鑄造者2%
    uint256 public nftStakePoolFee = 45; // 回流NFT質押池4.5%
    uint256 public mcaStakePoolFee = 15; // 回流MCA質押池1.5%
    ...
    constructor(IERC20 _usdt, IERC20 _mca) {
        usdt = _usdt;
        mca = _mca;
    }

    function addOrder(address user, uint256 tokenId) internal {
        // 全部
        makerOrdersList.push(tokenId);
        makerOrdersListIndexs[tokenId] = makerOrdersList.length - 1;

        // 個人
        ownerTokensList[user].push(tokenId);
        ownerTokensListIndexs[user][tokenId] = ownerTokensList[user].length - 1;
    }

    function removeOrder(address user, uint256 tokenId) internal {
        // 全部
        uint256 index = makerOrdersListIndexs[tokenId];
        uint256 len = makerOrdersList.length - 1;
        if (index == len){
            makerOrdersList.pop(); 
        } else {
            makerOrdersListIndexs[makerOrdersList[len]] = index;
            makerOrdersList[index] = makerOrdersList[len];
            makerOrdersList.pop(); 
        }
        delete makerOrdersListIndexs[tokenId]; 


        // 個人
        uint256 index2 = ownerTokensListIndexs[user][tokenId];
        uint256 len2 = ownerTokensList[user].length - 1;
        if (index2 == len2) {
            ownerTokensList[user].pop();
        } else {
            ownerTokensListIndexs[user][ownerTokensList[user][len2]] = index2;
            ownerTokensList[user][index2] = ownerTokensList[user][len2];
            ownerTokensList[user].pop();
        }

        delete ownerTokensListIndexs[user][tokenId];
    }
    
    function balanceOfMakerOrdersOf() external view returns(uint256) {
        return makerOrdersList.length;
    }

    function makerOrdersOf(uint256 startIndex, uint256 endIndex)
        external view returns(uint256[] memory) {
                
        uint256[] storage tokens = makerOrdersList;
        if (endIndex == 0) {
            return tokens;
        }
    ...
    function balanceOfOwnerOrders(address owner) external view returns(uint256) {
        require(owner != address(0), "owner is zero address");
        return ownerTokensList[owner].length;
    }

    function ownerOrdersOf(address owner, uint256 startIndex, uint256 endIndex)
        external view returns(uint256[] memory) {

        require(owner != address(0), "owner is zero address");

        uint256[] storage tokens = ownerTokensList[owner];
        if (endIndex == 0) {
            return tokens;
        }
    ...
    function createAuction(uint256 tokenid, uint256 acutionPrice,uint256 auctionStartTime, uint256 auctionEndTime,uint8 payment) external {    //创建订单
        require(manager.members("nft") != address(0), "empty member nft address");
        require(INFT(manager.members("nft")).ownerOf(tokenid) == msg.sender, "It's not your nft");
        require(makerOrders[tokenid].maker == address(0), "Exists Auction!");
        require(payment == 2, "payment token type error");
        require(block.timestamp < auctionEndTime, "Auction end!");
    ...
    function cancelAuction(uint256 tokenid) public {     
        ...
        removeOrder(msg.sender, tokenid);
    ...
    function auctionEnd(uint256 tokenid) public payable {
        ...
        require(manager.members("nft") != address(0), "empty members nft address");
        require(manager.members("funder") != address(0), "empty members funder address");
        require(manager.members("NFTStakePool") != address(0), "empty members NFTStakePool address");
        require(manager.members("MCAStakePool") != address(0), "empty members MCAStakePool address");

        uint8 payment = makerOrders[tokenid].payment;
        uint256 auctionAmount = makerOrders[tokenid].auctionAmount;
        ...
        if(makerOrders[tokenid].isBid){
            ...
            //收取手续费
            uint256 tradeFee = auctionAmount.mul(funderFee).div(1000); // 項目方2% 
            uint256 stampFee = auctionAmount.mul(nftOwnerFee).div(1000); // 鑄造者2%
            uint256 nftStakeFee = auctionAmount.mul(nftStakePoolFee).div(1000); // 回流NFT質押池4.5%
            uint256 mcaStakeFee = auctionAmount.mul(mcaStakePoolFee).div(1000); // 回流MCA質押池1.5%
            uint256 sendAmount = auctionAmount.sub(tradeFee).sub(stampFee).sub(nftStakeFee).sub(mcaStakeFee);
            ...
            IERC20(token).transfer(manager.members("NFTStakePool"), nftStakeFee);
            IERC20(token).transfer(manager.members("MCAStakePool"), mcaStakeFee);
            IERC20(token).transfer(makerOrders[tokenid].maker, sendAmount);
        }
        else{       //流拍
            IERC721(manager.members("nft")).transferFrom(address(this), makerOrders[tokenid].maker, tokenid);
            makerOrders[tokenid].highestBidder = makerOrders[tokenid].maker;
        }
    ...
    function setFees(uint256 _funderFee, uint256 _nftOwnerFee, uint256 _nftStakePoolFee, uint256 _mcaStakePoolFee) external {
        require(manager.members("owner") != address(0), "empty members owner address");
        require(msg.sender == manager.members("owner"), "only owner");
        funderFee = _funderFee;
        nftOwnerFee = _nftOwnerFee;
        nftStakePoolFee = _nftStakePoolFee;
        mcaStakePoolFee = _mcaStakePoolFee;
    }
}
