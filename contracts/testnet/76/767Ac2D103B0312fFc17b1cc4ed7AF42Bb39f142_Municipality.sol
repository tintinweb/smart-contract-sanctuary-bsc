...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setMonths(uint64[6] memory _months) external onlyOwner {
        months = _months;
    }
    ...
    function rentParcel(uint256 _parcelId, address _user, uint256 _periodId) external {
        require(IERC721Base(standardParcelNFTAddress).userOf(_parcelId) == address(0), "The selected parcel is not free");// set that can be rentied if has no user
        ...
        IERC721Base(standardParcelNFTAddress).setUser(_parcelId, _user, block.timestamp + months[_periodId] * 2592000); // 1 month = 30 days
    ...
    function _transferToContract(uint256 _amount, address _owner) private {
        IERC20Upgradeable(busdAddress).safeTransferFrom(address(msg.sender),address(this), _amount);
        IERC20Upgradeable(busdAddress).safeTransferFrom(address(this), _owner, _amount * 4 / 10);
        IERC20Upgradeable(busdAddress).safeTransferFrom(address(this),address(companyWallet), _amount * 3 / 10);
        IERC20Upgradeable(busdAddress).safeTransferFrom(address(this),address(buyAndBurnWallet), _amount * 3 / 10);
    }
