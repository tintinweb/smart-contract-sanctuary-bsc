...
contract HOLIDAY_MEME_FAMILY is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "HOLIDAY MEME FAMILY";                  
    string private _symbol = "HMF";                                 
    ...
    uint256 public _taxFee4Buyers = 2;
    ...
    uint256 public numTokensSellToAddToLiquidity = _tTotal * 5 / 1000;                   // contract balance to trigger swap & liquify + wallet transfer.
    address public pancakeRouterAddress = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;    // BSC Router Version 2 address
    ...
    function setSendDIVInTx(bool _newStatus) external onlyOwner {
    ...
    function getNumberOfB20DividendTokenHolders() external view returns(uint256) {
     ...
     function changeTokenName(string memory newName , string memory newSymbol) external onlyOwner {
     _name = newName;
     _symbol = newSymbol;
    ...
    function setFee4SellerBuyers(uint256 taxFee4Sellers,uint256 taxFee4Buyers ,uint256 liquidityFee4Sellers ,uint256 liquidityFee4Buyers,uint256 liquidityFee ,uint256 taxFee ) external onlyOwner() {
   ...
   function isExcludedFromMaxWallet(address account) public view returns(bool) {
    ...
    function setFees(address sender , address recipient) private {
        if (sender == uniswapV2Pair) {
            ...
            a80DividendRewardsFee = a80DividendRewardsFee;
            b20DividendRewardsFee = b20DividendRewardsFee;
    ...
    function _transfer(
        ...
        require(_isExcludedFromFees[from] || _isExcludedFromFees[to] || isTokenLive, "HMF is not live yet");
        
        //when buy
       

     if (automatedMarketMakerPairs[from] && isTokenLive) {
       ...
       require((heldTokens + amount) <= _maxWallet,"Over wallet limit.");
        ...
        if(!inSwapAndLiquify && to != deadAddress) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for BNB
        swapTokensForBNB(liquifyHalves[0]); // <- this breaks the MATIC -> HMF swap when swap+liquify is triggered
    ...
    function swapTokensForDividendToken(uint256 _tokenAmount, address _recipient, address _dividendAddress) private {
     ...
     function swapAndSendA80Dividends(uint256 tokens) private {
        uint256 beforeAmount = IBEP20(a80DividendToken).balanceOf(address(a80DividendTracker));

        swapTokensForDividendToken(tokens, address(a80DividendTracker), a80DividendToken);

        uint256 a80Dividends = IBEP20(a80DividendToken).balanceOf(address(a80DividendTracker)).sub(beforeAmount);
    ...
    function swapAndSendB20Dividends(uint256 tokens) private {
        uint256 beforeAmount = IBEP20(b20DividendToken).balanceOf(address(b20DividendTracker));

        swapTokensForDividendToken(tokens, address(b20DividendTracker), b20DividendToken);

        uint256 b20Dividends = IBEP20(b20DividendToken).balanceOf(address(b20DividendTracker)).sub(beforeAmount);
