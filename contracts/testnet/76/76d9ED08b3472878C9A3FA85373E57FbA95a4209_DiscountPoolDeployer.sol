...
import "./interfaces/IDiscountPoolDeployer.sol";
import "./interfaces/IDiscountPool.sol";
import "./interfaces/IUniswapV2Pair.sol";
import "./interfaces/IUniswapV2Router02.sol";
import "./DiscountPool.sol";

contract DiscountPoolDeployer is Ownable, Pausable, AccessControl{    
    ...
    */
    ...
    function createPool(Parameters calldata store) external payable whenNotPaused {
        require(validateSellDate(store.startTime,store.endTime), "invalid time duration");
        ...
        require(store.claimDate.sub(store.endTime) >= minimumClaimedDate && store.claimDate.sub(store.endTime) <= maximumClaimedDate, "claim Date is invalid");
    ...
    function distributeOffer(
        ...
        if(platformSoldTokensFeeEnabled){
            IERC20(token).safeTransferFrom(user,owner(),offeringAmount.mul(platformSoldTokensFee).div(1e2));
        }
        pools.add(pairInfo[token] = pool);
        {            
...
contract DiscountPool is Ownable,ReentrancyGuard,Pausable,EIP712 {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address payable;
    ...
    constructor() EIP712("DiscountPool", "1") {}
    
    receive() external payable {}

    modifier onlyOperator() {
        require((msg.sender == poolAdmin) || 
                (msg.sender == DiscountMain.PlatformWalletAddress()), "unable to access");
        _;
    }
    ...
    */
    ...
    function initialize(
        ...
        DiscountMain = IDiscountPoolDeployer(_DisCountMain);
    ...
    function transferProjectOwnerShip(address newOwner) external returns (bool) {        
        ...
        require(msg.sender == poolAdmin, "unable to access");
     ...
     * - `status` true means enable and false means disable.
     */     
    function blockPool(bool status) public onlyOperator {
        isPoolBlock = status;
    }    
    ...
    function swap() external payable nonReentrant whenNotPaused returns (bool) {
        require(!isWhiteList , "only whitelist people can able to access");
    ...
    function swapWithPermit(
        ...
        require(DiscountMain.isSigner(signer), "ERC20Permit: invalid signature");

        return _swap(_msgSender(),msg.value);
    }

    function _swap(address user,uint256 amount) internal returns (bool){
        require(!isPoolBlock, "pool is blocked");
    ...
    function claim() external nonReentrant whenNotPaused onlyOperator returns (bool) {
        ...
        saleToken.safeTransfer(poolAdmin,saleToken.balanceOf(address(this)).sub(soldOutTokens.sub(redeemTokens)));
        claimState = true;
        return true;
    }
    ...
    function poolTimeUpdate(uint256 _startTime,uint256 _endTime,uint256 _claimDate) external onlyOperator{
        startTime = _startTime;
        endTime = _endTime;
        claimDate = _claimDate;
    }
