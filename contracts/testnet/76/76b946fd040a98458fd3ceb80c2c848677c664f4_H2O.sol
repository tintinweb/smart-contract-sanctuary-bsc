...
contract H2O is Context, IERC20, Ownable {
    ...
    address public devWallet = 0xaF2E5ea3be07022Ab512BF44CDBEdDFf3856a3DD ;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 walletAmt = amount.mul(_devFee).div(100);
        
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(walletAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(walletAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(walletAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(walletAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(walletAmt)));
        }
        ...
        _transferStandard(sender, devWallet, walletAmt);
