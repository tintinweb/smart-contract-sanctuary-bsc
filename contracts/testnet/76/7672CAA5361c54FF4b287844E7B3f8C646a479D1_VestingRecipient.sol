 ...
 */
contract VestingRecipient is AccessControl, ReentrancyGuard {
    ...
    function stake(uint256 _amount) external onlyRole(OPERATOR_ROLE) {
        IERC20(sesameToken).approve(address(distributor), _amount);
        distributor.deposit(_amount);
    }

    function unstake(uint256 _amount) external onlyRole(OPERATOR_ROLE) {
        distributor.withdraw(_amount);
    }

    function withdrawSesame(uint256 _amount)
        external
        nonReentrant
        onlyRole(OPERATOR_ROLE)
    {
        _withdraw(sesameToken, _amount);
    }

    function withdrawOtherToken(address _currency, uint256 _amount)
        external
        nonReentrant
        onlyRole(OPERATOR_ROLE)
    {
        require(_currency != sesameToken, "Owner: Cannot withdraw Sesame");
        _withdraw(_currency, _amount);
    }

    function _withdraw(address _currency, uint256 _amount) internal {
        uint256 balance = IERC20(_currency).balanceOf(address(this));
        require(_amount <= balance, "Insufficient");
        IERC20(_currency).safeTransfer(msg.sender, _amount);
        emit Withdraw(_currency, _amount);
    }
}
