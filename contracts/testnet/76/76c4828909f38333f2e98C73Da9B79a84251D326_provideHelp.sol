...
contract provideHelp is Context, Ownable {
    ...
    uint256 INVEST_MIN_AMOUNT=0.001 ether;
    ...
    constructor()  {
        for (uint256 i = 0; i < 10; i++) {
            ...
            _currentUserID++;
            getHelp[_getHelpCounter]=getHelpStruct({amount:amounts[i],userAddress:_msgSender(),userId:users[_msgSender()].userId,isPaid:false,paidAmount:0});
            _getHelpCounter++;
        }
    }
    ...
    function provideHelpFunction(uint256 amount ) public payable {
        ...
        require(msg.value == percentage(msg.value,20), "need 20 % amount");
        ...
        helpPool[_helpPoolId]=HelpPoolStruct({amount: amount,paidAmount: msg.value,isPaid: false,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp});
        _helpPoolId++;
    ...
    function completeHelp(uint256 helpPoolId) public payable{
        ...
        require(percentage(helpPool[_helpPoolId].amount,80) == msg.value, "need");
        helpPool[helpPoolId].paidAmount =  helpPool[helpPoolId].paidAmount+msg.value;
