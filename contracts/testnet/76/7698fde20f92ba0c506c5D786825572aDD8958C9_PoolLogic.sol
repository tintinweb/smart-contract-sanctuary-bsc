...
contract PoolLogic is ERC20ExtendedUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event RedeemFee(address to, uint256 amountUnit);
    event FeeMinted(uint256 feeType, address to, uint256 amountUnit);
    ...
    function deposit(address _tokenIn, uint256 _amountIn) public {
        require(_tokenIn == IPoolManagerLogic(poolManagerLogic).denominationAsset(), "invalid deposit");
        require(_amountIn > 0, "invalid unit");
        ...
        _mintFee(uint256(IFeeManage.FeeType.TYPE_FEE_ENTRY), IPoolFactory(factory).getProtocolFee(), numberOfUnit);
    ...
    function withdraw(bool _isStablecoin, uint256 _unitOut) public nonReentrant {
        require(_unitOut > 0, "invalid unit");

        uint256 numberOfUnit = balanceOf(msg.sender);
        ...
        uint256 protocolFee = IPoolFactory(factory).getProtocolFee();
        if (!_isStablecoin) {
            _mintFee(uint256(IFeeManage.FeeType.TYPE_FEE_EXIT_SHARE_IN_KIND), protocolFee, _unitOut);
        } else {
            _mintFee(uint256(IFeeManage.FeeType.TYPE_FEE_EXIT_SPECIFIC), protocolFee, _unitOut);
        }

        _withdraw(_isStablecoin, IPoolManagerLogic(poolManagerLogic).denominationAsset(), proportion, msg.sender);
    }
    ...
    function swap(
        ...
        require(ISupportedAsset(poolManagerLogic).isSupportedAsset(_tokenOut), "invalid asset");

        uint256 amountOut = SwapLib._swap(
            IPoolFactory(factory).pancakeswapRouter(),
            _tokenIn,
            _amountIn,
            _amountOutMin,
            _path,
    ...
    function calculatePerformanceFee() public {
        require(msg.sender == IPoolFactory(factory).getBot(), "not allow");
        require(IFeeManage(poolManagerLogic).performanceFee() != 0, "no fee");
        ...
        if (managerFee > 0) _mint(poolManagerLogic, managerFee);
        if (systemFee > 0) _mint(factory, systemFee);

        emit FeeMinted(uint256(IFeeManage.FeeType.TYPE_FEE_PERFORMANCE), manager, managerFee);
        emit FeeMinted(uint256(IFeeManage.FeeType.TYPE_FEE_PERFORMANCE), factory, systemFee);
    }

    /// @dev manager or  redeem fee
    function redeemFee(bool _isStablecoin) public nonReentrant {
        address shareOwner;
        address recipent;
        if (msg.sender == manager) {
            shareOwner = poolManagerLogic;
            recipent = IPoolManagerLogic(poolManagerLogic).getRecipientWithdrawal(
                uint256(IFeeManage.FeeType.TYPE_FEE_PERFORMANCE)
            );
        } else if (msg.sender == IPoolFactory(factory).getBot()) {
            shareOwner = factory;
            recipent = IPoolFactory(factory).getOwner();
        } else {
            revert("not allow");
        }
        uint256 numberOfUnit = balanceOf(shareOwner);

        require(numberOfUnit > 0, "no fee");

        _burn(shareOwner, numberOfUnit);
        _withdraw(
            _isStablecoin,
            IPoolManagerLogic(poolManagerLogic).denominationAsset(),
            numberOfUnit.mul(10**18).div(totalSupply()),
            recipent
        );

        emit RedeemFee(recipent, numberOfUnit);
    }
    ...
    function _mintFee(
        ...
        emit FeeMinted(feeType, factory, fee);
    }
    ...
    function _withdraw(
        ...
        WithdrawnAsset[] memory withdrawnAssets = new WithdrawnAsset[](_isStablecoin ? 1 : assetCount);

        for (uint256 i = 0; i < assetCount; i++) {
            address asset = assets[i];
            uint256 withdrawnAmount = _calculateNumberOfWithdrawnToken(address(this), asset, _proportion);
            if (withdrawnAmount > 0) {
                if (!_isStablecoin) {
                    IERC20Upgradeable(asset).transfer(_recipient, withdrawnAmount);
                } else {
                    if (_denominationAsset == asset) {
                        // transfer only denomination asset
                        IERC20Upgradeable(asset).transfer(_recipient, withdrawnAmount);
                    } else {
                        // swap to stable coin
                        withdrawnAmount = SwapLib._swapForWithdrawal(
                            IPoolFactory(factory).WBNB(),
                            IPoolFactory(factory).pancakeswapRouter(),
                            asset,
                            _denominationAsset,
                            withdrawnAmount,
                ...
                if (_isStablecoin) {
                    uint256 balanceBefore = withdrawnAssets[0].amount;
                    withdrawnAssets[0] = WithdrawnAsset({
                        asset: _denominationAsset,
                        amount: balanceBefore.add(withdrawnAmount)
                    });
                } else {
                    withdrawnAssets[index] = WithdrawnAsset({asset: asset, amount: withdrawnAmount});
                    index++;
                }
            }
        }
        ...
        emit Withdraw(withdrawnAssets, _recipient);
    }
    ...
    function _calculateNumberOfWithdrawnToken(
        ...
        withdrawnAmount = (IERC20Upgradeable(_asset).balanceOf(_pool)).mul(_proportion).div(10**18);
    }
}
...
library SwapLib {
    ...
    function _swapForWithdrawal(
        ...
        address[] memory path = _getPathSwap(_wbnb, _tokenIn, _tokenOut);
        uint256[] memory amountOutmin = IPancakeRouter02(_router).getAmountsOut(_amountIn, path);
        if (amountOutmin.length <= 0) {
            return 0;
        }

        return _swap(_router, _tokenIn, _amountIn, amountOutmin[0], path, _to);
    }
    ...
    function _swap(
        ...
        uint256 allowance = IERC20Upgradeable(_tokenIn).allowance(address(this), address(_router));
        if (allowance < _amountIn) {
            if (allowance > 0) {
                IERC20Upgradeable(_tokenIn).safeApprove(address(_router), 0);
            }
            IERC20Upgradeable(_tokenIn).safeApprove(address(_router), _amountIn);
        }

        uint256[] memory amounts = IPancakeRouter02(_router).swapExactTokensForTokens(_amountIn, _amountOutMin, _path, _to, block.timestamp);
        if (amounts.length == 0) {
            return 0;
        }
        return amounts[0];
