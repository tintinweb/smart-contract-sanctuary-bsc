...
contract ChainedX is Context, IERC20, Ownable {
    ...
    address public marketingAddress = 0x5aB9869069d3c32427Cb96178cbB4c783D00FBf2;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "ChainedX";
    string private constant _symbol = "CHDX";
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _burn, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        require(_rfi + _burn + _marketing + _liquidity + _buyback <= uint(10), "Max tax rate is 10");
        feeRates.rfi = _rfi;
        feeRates.burn = _burn;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _burn, uint256 _marketing, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        require(_rfi + _burn + _marketing + _liquidity + _buyback <= uint(10), "Max tax rate is 10");
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tOperations = tAmount*sellFeeRates.burn/100;
            ...
            s.tOperations = tAmount*feeRates.burn/100;
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 denominator= (feeRates.liquidity + feeRates.buyback + feeRates.burn) * 2;
        ...
        // Send BNB to burn address
        uint256 burnAmt = unitBalance * 2 * feeRates.burn;
        if(burnAmt > 0){
