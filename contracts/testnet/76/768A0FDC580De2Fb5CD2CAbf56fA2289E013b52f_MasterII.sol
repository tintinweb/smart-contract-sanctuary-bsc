...
contract MasterII is Ownable {
    ...
    uint public levelOne;
    uint public levelTwo;
    ...
    constructor(address cnnLP_, address ctnLP_, address usdt_)  {
        cnnLP = IPair(cnnLP_);
        ctnLP = IPair(ctnLP_);
        USDT = usdt_;
        devaddr = msg.sender;
    ...
    function deposit(uint256 pid_, uint256 amountIn_, address inv_) public {
        ...
        require(inv_ != msg.sender, 'cant invite yourself');
        require (pid_ != 0, 'deposit no good');
        PoolInfo storage pool = poolInfo[pid_];
    ...
    function claim(uint pid_) public {
        require (pid_ != 0, 'withdraw CAKE by unstaking');
        PoolInfo storage pool = poolInfo[pid_];
        UserInfo storage user = userInfo[pid_][msg.sender];
        ...
        pool.outPutToken.safeTransfer(address(msg.sender), _reward);
    ...
    function withdraw(uint256 pid_, uint256 amountOut_) public {
        ...
        claim(pid_);
        if (amountOut_ > 0 ){
            user.amount = user.amount -= amountOut_;
    ...
    function addPool(IPair lpToken_, IBEP20 outPutToken_, uint dailyOut_, bool withUpdate_) public onlyOwner {
        if (withUpdate_) {
            massUpdatePools();
        }
    ...
    function setPool(uint256 _pid, uint256 dailyOut_, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
