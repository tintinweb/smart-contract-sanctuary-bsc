...
// File contracts/firefundbinance.sol
...
contract FireFundBinance{
    ...
    uint256 constant launch = 1668146400;
    ...
    function stakeStablecoins(uint256 amtx, address ref) payable public {
        require(block.timestamp >= launch, "App not launch yet.");
        ...
        if (ref == 0x000000000000000000000000000000000000dEaD){
            user.refBonus += 0;
            USDC.safeTransfer(owner, refAmtx);
        } else {
    ...
    function withdrawDivs() public returns (uint256 withdrawAmount){
        ...
        uint256 moreThanOneDay = block.timestamp - 86400;
        require(user.lastWith < moreThanOneDay, "Less than 1 day"); 
    ...
    function withdrawInitial(uint256 keyy) public {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ 
            uint256 minusAmt = initialAmt.mul(FeesKey[10].feePercentage).div(percentdiv); 
          	...
          	uint256 dailyReturn = initialAmt.mul(3).div(percentdiv);
    ...
    function withDrawFromContract(uint256 amtx, address ref) public {
        ...
        require(block.timestamp > (devLastWithdrawal + 86400), "Less than 7 days");
        require(msg.sender == owner, "You are not the owner!");
        uint256 maxWithdrawal = minContractValue().mul(2);
        require(amtx < maxWithdrawal);
