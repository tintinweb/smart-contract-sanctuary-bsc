...
contract GetMintNft {
    ...
    function getNFTS() public view returns (IGetMintNft.EachNFT[] memory) {
        uint256 itemCount = 4;
        uint256 currentIndex = 0;
        IGetMintNft.EachNFT[] memory items = new IGetMintNft.EachNFT[](
            itemCount
        );
        for (uint256 i = 1; i <= 4; i++) {
            IGetMintNft.EachNFT memory nft = IGetMintNft.EachNFT(
                collectionContract.getNFT(uint8(i)).level,
                collectionContract.getNFT(uint8(i)).amount,
                collectionContract.getNFT(uint8(i)).metaData,
                collectionContract.getNFT(uint8(i)).tokenId
