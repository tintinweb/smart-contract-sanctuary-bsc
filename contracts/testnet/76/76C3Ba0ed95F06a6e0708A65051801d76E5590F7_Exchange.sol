...
contract Exchange is ExchangeDomain, Initializable, ERC2771ContextUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, FeeComp, RoyaltyComp, BidComp {
    ...
    function createAndSell(CreateAndSellRequest calldata createAndSellReq) public {
        if (createAndSellReq.isTimedAuction) {
            if(sellinfos[_msgSender()][createAndSellReq.asset.token][createAndSellReq.asset.tokenId].tokenAmount != 0) {
                return;
            }
        }
        ...
        payable
    {
        ...
        require((sellInfo.tokenAmount == bidReq.asset.tokenAmount),"Exchange: match tokenAmount faild");
    ...
    function rewardBid(RewardBidRequest calldata rewardBidReq) public {
        ...
        uint256 feeAmount = _payFee(address(this), sellInfo.currency, sellInfo.price);
        uint256 royaltyAmount = _payRoyalty(rewardBidReq.asset.token, rewardBidReq.asset.tokenId, address(this), sellInfo.currency, sellInfo.lastBid);
        ...
        if(sellInfo.currency == address(0)){
            ...
            IERC20Upgradeable erc20 =  IERC20Upgradeable(sellInfo.currency);
            require(erc20.transferFrom(address(this), rewardBidReq.seller, royaltyAmount), "BidComp: Bid with ERC20 token failed");
        }
    }
    ...
    function _swap(BuyRequest calldata buyReq) internal {
        ...
        uint256 feeAmount = _payFee(address(this), sellInfo.currency, sellInfo.price);
        uint256 royaltyAmount = _payRoyalty(buyReq.asset.token, buyReq.asset.tokenId, address(this), sellInfo.currency, sellInfo.price);
        uint256 remainingAmount = sellInfo.price - feeAmount - royaltyAmount;
        ...
        if(sellInfo.currency == address(0)){
            ...
            IERC20Upgradeable erc20 =  IERC20Upgradeable(sellInfo.currency);
            require(erc20.transferFrom(address(this), buyReq.seller, royaltyAmount), "Exchange: buy with ERC20 token failed");
        }
    }
}
