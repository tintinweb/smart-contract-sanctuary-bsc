...
contract PixelToken is Context, IERC20, Ownable, Pausable {
    ...
    string private _name = 'Pixel Tokens';
    ...
    modifier onlyTreasury() {
        require(_msgSender() == treasuryContract, "Only Treasury - PIXEL");
        _;
    }

    constructor () {
        address sender = _msgSender();
        uint256 amount =  BASE_MINT*1000; //10m
        _tTotal = amount;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance  - PIXEL"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "ERC20: decreased allowance below zero  - PIXEL"));
        return true;
    }
    ...
    function distributeAirdrop(address[] memory _receivers, uint _value) public onlyOwner {
        require(_isRegisterAirdropDistribution, "not registered - PIXEL");
        aidropDistributed = aidropDistributed.add(_receivers.length.mul(_value));
        require(aidropDistributed <= AIRDROP_AMOUNT, "exceeds max - PIXEL");
    ...
    function registerAirdropDistribution() public onlyOwner {
        require(!_isRegisterAirdropDistribution, "Already registered - PIXEL");
        uint256 _currentRate = _getRate();
    ...
    function setGenesisReward(uint256 amount) public onlyOwner {
        ...
        require(amount <= totalSupply().div(1e11), "set genesis reward: bad - PIXEL");
    ...
    function setTransferStatus(bool _isPaused) public {
        require(msg.sender == botKeeper, "Caller is not bot keeper - PIXEL");
        if(_isPaused){
            _pause();
        }else{
            _unpause();
    ...
    function donate(uint256 amount) public {
        address sender = _msgSender();
        require(!_isExcluded[sender], "Excluded addresses cannot call this function - PIXEL");
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        require(tAmount <= _tTotal, "Amount must be less than supply - PIXEL");
    ...
    function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
        require(rAmount <= _rTotal, "Amount must be less than total reflections - PIXEL");
        uint256 currentRate =  _getRate();
        return rAmount.div(currentRate);
    }

    function excludeAccount(address account) external onlyOwner() {
        require(!_isExcluded[account], "Account is already excluded - PIXEL");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "ERC20: approve from the zero address - PIXEL");
        require(spender != address(0), "ERC20: approve to the zero address - PIXEL");
    ...
    function _mint(address receiver, uint256 amount) private {
        require(amount % BASE_MINT == 0, "Cannot mint this amount - PIXEL");
    ...
    function _burn(uint256 amount) private {
        require(amount % BASE_MINT == 0, "Cannot burn this amount - PIXEL");
        address sender = _msgSender();
        uint256 _rate = _getRate();
    ...
    function _transfer(address sender, address recipient, uint256 amount) private whenNotPaused {
        require(sender != address(0), "ERC20: transfer from the zero address - PIXEL");
        require(recipient != address(0), "ERC20: transfer to the zero address - PIXEL");
        require(amount > 0, "Transfer amount must be greater than zero - PIXEL");
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount);
