...
contract BITMIC is StandardToken {
 ...
 ERC721Burnable
{
    ...
    function buy(uint256 _id, bool _present, address _to, uint256 _price) external payable {
        if(_present)
        {
        ...
        if(_present)
        {
            emit Purchase(_previousOwner, _to, price[_id], _id, tokenURI(_id));
        }else
        {
    ...
    function buy_Cash(uint256 _id, address _to, uint256 _price) external payable onlyOwner {
        require(_to != address(0) , "recever address empty!");
        require(_to != ownerOf(_id) , "recever address is owner!");
        ...
        emit Purchase(_previousOwner, _to, _price, _id, tokenURI(_id));
    }
    ...
    function buy_token(uint256 _id, bool _present, address _to, uint256 _price) external {
        if(_present)
        {
            require(_to != address(0) , "recever address empty!");
            require(_to != ownerOf(_id) , "recever address is owner!");
...
contract BITMIC_NFT is Market {
    ...
    event ImmediatelySell_Cash_Auction(address indexed owner, uint256 nftID, uint256 price);
    ...
    function mInt_Multi_Num(uint256 Count) public{   
        require(msg.sender == _Administrator || msg.sender == _owner);     
   ...
   //경매생성
    ...
    //즉시 판매 경매 생성 - 현금
    function immediatelySell_Cash_Auction(uint256 _tokenId, uint256 _price) public  {     
        require(_ownerOf(_tokenId) == msg.sender);//토큰 소유자인지 확인
        require(hasAuction(_tokenId) == false);
        require(listedMap[_tokenId] == false); // 즉시판매 진행중           
        listedMap[_tokenId] = true; 
        cashSell[_tokenId] = true; 
        emit ImmediatelySell_Cash_Auction(msg.sender, _tokenId, _price);  
    }
   ...
   //토큰입찰;
   function bid_Token(uint256 _tokenId, uint256 _price) external {
       _bid_token(_tokenId, _price);
   }
   ...
   function OpenBox()  external payable returns (uint256){   
       ...
       require( msg.value == _BOX_Open_Price,  "Price sent is not correct." );
