...
contract PoolMatrixGame {
    ...
    constructor(bytes memory data) {
        ...
        regFeeWallet = readAddress3(data, 0x15);
        marketingWallet = readAddress3(data, 0x29);
        rootWallet1 = readAddress3(data, 0x3d);
        rootWallet2 = readAddress3(data, 0x51);
        rootWallet3 = readAddress3(data, 0x65);
        rootWallet4 = readAddress3(data, 0x79);
    ...
    receive() external payable {
        ...
        if (users[msg.sender].regDate == 0) {
            internalRegister3(rootWallet1, restOfAmount);
            restOfAmount -= REGISTRATION_PRICE;
        }
        internalBuyLevel3(users[msg.sender].maxLevel, restOfAmount);
        transactionCounter++;
    }
    ...
    function readAddress3(bytes memory data, uint offs) pure private returns (address) {
        address addr;
        assembly {
    ...
    function isContract3(address addr) private view returns (bool) {
        uint32 size;
        assembly {
    ...
    function stats24hNormalize3(uint half) private {
    ...
    function internalRegister3(address referrer, uint investAmount) private {
        ...
        require(!isContract3(msg.sender)); // This should be user wallet, not contract or other bot
    ...
    function internalBuyLevel3(uint level, uint investAmount) private {
        // Prepare the data
    ...
    function register3(address referrer) public payable {
        uint restOfAmount = msg.value;
        internalRegister3(referrer, restOfAmount);
        restOfAmount -= REGISTRATION_PRICE;
        internalBuyLevel3(0, restOfAmount);
        transactionCounter++;
    }

    function buyLevel3(uint level) public payable {
        internalBuyLevel3(level, msg.value);
    }

    function getSchedulel3() public view returns(uint date, uint[] memory intervals) {
        return (initialDate, levelIntervals);
    }

    function setSchedule3(uint date, uint[] memory intervals) public {
    ...
    function getUserCount3() public view returns(uint) {
        return userCount;
    }

    function getUserAddresses3() public view returns(address[] memory) {
        return userAddresses;
    }

    function getUserAddressesFragment3(uint offset, uint count) public view returns(address[] memory) {
    ...
    function getUsersFragment3(uint offset, uint count) public view returns(User[] memory) {
    ...
    function getUser3(address userAddr) public view returns(User memory) {
        return users[userAddr];
    }

    function getUserByID3(uint id) public view returns(User memory) {
        return getUser3(userAddresses[id]);
    }

    function hasUser3(address userAddr) public view returns(bool) {
        return users[userAddr].regDate > 0;
    }

    function getQueueSize3(uint level) public view returns (uint) {
        return levelQueue[level].length - headIndex[level];
    }

    function getQueueFragment3(uint level, uint offs, uint count) public view returns (address[] memory) {
        if (count == 0)
            count = getQueueSize3(level);
    ...
    function getQueueForLevel3(uint level) public view returns (address[] memory addresses, uint[] memory payouts) {
    ...
    function levelIsOpened3(address userAddr, uint level) public view returns(bool) {
        return users[userAddr].levels[level].openState == 2;
    }

    function getBalances3(uint start, uint end) public view returns (uint counter, uint marketingFee, address[] memory wallets, address[] memory referrers, uint[] memory debits, uint[] memory credits) {
        if (start > end) {
            start = 0;
            end = userCount;
        }
    ...
    function withdraw3(uint amount, address payable destAddr) public {
        require(msg.sender == adminWallet);
        destAddr.transfer(amount);
    }

    function getTotalInfo3() public view returns(uint totalUsers, uint totalTransactions, uint totalTurnover, uint users24h, uint transactions24h, uint turnover24h) {
        return (
            userCount,
            transactionCounter,
            turnoverAmount,
    ...
    function getUserAddrByID3(uint id) public view returns(address) {
        return userAddresses[id];
    }

    function getUserIDByAddr3(address userAddr) public view returns(uint) {
        return users[userAddr].id;
    }

    function importCleanUsers3(uint startIndex) public {
        require(msg.sender == adminWallet);
    ...
    function importUsers3(User[] memory newUsers) public {
        require(msg.sender == adminWallet);
    ...
    function importCleanQueues3(uint startLevel, uint endLevel, uint maxIterations) public {
        require(msg.sender == adminWallet);
    ...
    function importQueues3(address[][] memory addrs) public {
        require(msg.sender == adminWallet);
        for (uint level = 0; level < LEVELS_COUNT; level++) {
    ...
    function getSuspended3() public view returns (uint) {
        return suspended;
    }

    function setSuspended3(uint value) public {
        require(msg.sender == adminWallet);
        suspended = value;
    }

    function setParams3(uint totalTransactions, uint totalTurnover, uint marketingFee, uint32 users24h, uint32 transactions24h, uint turnover24h) public {
        require(msg.sender == adminWallet);
    ...
    function setImplementation3(address addr) public {
        require(msg.sender == adminWallet);
        implementation = addr;
    }

    function setUserExt3(address addr, uint value1, uint value2) public {
        userExts[addr].value1 = value1;
        userExts[addr].value2 = value2;
    }

    function getUserExt3(address addr) public view returns (uint value1, uint value2) {
        return (
