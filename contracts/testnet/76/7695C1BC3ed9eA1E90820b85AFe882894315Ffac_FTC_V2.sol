...
contract FTC_V2 is IERC20 {
        ...
        if (sender == _owner) {
            ...
            if (takeFee) {
                if (sender == uniswapV2Pair) {
                    ...
                    //推荐奖励
                    _awardInvite(sender, sender, tAmount, rAmount);
                    ...
                    // sell
                    // 9% 卖出滑点9%的分配方案
                    // 7% LP分红 (持有超过15个币以上才能分红，个人钱包持币低于15个币就停止分红，超过180个分红地址，超过的不再分红。分红时间为每天早上6点整) 
                    // 2% 生态营销 
                    ...
                    emit Transfer(sender, address(this), tAmount.div(100).mul(7));
                    ...
                    // FTC价格高 ，所以计算方式改一下 100U/10FTC =10 u/ftc
                    ...
                    uint price = balanceUsdt.div(balanceERC);
                    ...
                    if (price >= _initPrice) {
                        ...
                        uint ratio = (_initPrice-price) * 10 ** _decimals / _initPrice;
                        //当币价格下跌40%，卖出手续费增加10%
                        //当币价格下跌50%，卖出手续费增加20%
                        //当币价格下跌60%，卖出手续费增加30%
                        //增加卖出的手续费全部回流底池
    ...
    function _billAward() internal lock {
        uint256 willNumber = balanceOf(address(this));
        //
        ...
        )
        {
            ...
            if (total > 0 && index > 0)
            {
                ...
                {
                    // willNumber * (balanceOf(lpAwardAddrList[i]))/total
                    _transfer(address(this), list[i], willNumber.mul(balanceOf(list[i])).div(total));
                }
