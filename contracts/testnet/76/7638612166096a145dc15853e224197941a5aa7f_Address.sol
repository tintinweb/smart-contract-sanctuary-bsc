...
library Address {
    ...
    function sendValue(address payable recipient, uint256 amount) internal {
        ...
        require(success, "Address: unable to send value, re revertedecipient may hav");
    }
    ...
    function functionCallWithValue(
        ...
        require(address(this).balance >= value, "Address: delegate balance for call");
...
contract YieldRobot is Ownable, ReentrancyGuard{
    ...
    address private devWallet;      //开发钱包
    address public BUSDContract;
    uint256 public _currentDepositID = 0;    //存款ID。

    uint256 public totalInvestors = 0;         //总人数
    ...
    function _getNextDepositID() private view returns (uint256) {    // 该方法只是返回了一个 存款人数ID  并没有实质改变 _currenDepositID 
        return _currentDepositID + 1;
    }

    function _incrementDepositID() private {         //增加一个存款ID
        _currentDepositID++;
    }

    function deposit(uint256 _amount, address _referrer) external {
        require(_amount > 10, "you can deposit more than 10 busd");
        ...
        uint256 lastRoiTime = block.timestamp - investors[msg.sender].lastCalculationDate;  //86400
        uint256 allClaimableAmount = (lastRoiTime *          
            investors[msg.sender].totalLocked *
            apr).div(percentRate * rewardPeriod);

        investors[msg.sender].claimableAmount = investors[msg.sender].claimableAmount.add(allClaimableAmount);  //claimableAmount 初始值为0.
    ...
    function setCoupon(CouponSigData calldata coupon, Sig calldata sig) external {
        ...
        uint256 lastRoiTime = block.timestamp - investors[msg.sender].lastCalculationDate;
        uint256 allClaimableAmount = (lastRoiTime *
            investors[msg.sender].totalLocked *              // lastRoiTime  = 86400s   totalLocked 是总存款金额  
            apr).div(percentRate * rewardPeriod);
