...
import "./IBoxExchange.sol";

// 盲盒代币公售、白名单售卖
contract BoxExchange is IBoxExchange, Caller, Fallback {
    ...
    function make(
        ...
        require(
            stockAmount <= IERC20(token).balanceOf(config.getMintBoxNFT()),
            "1111"
        );
    ...
    function take(address token, uint256 amount) external payable override {
        ...
        require(amount < orderInfo.stockAmount, "1129");
        ...
        orderInfo.stockAmount = orderInfo.stockAmount.sub(amount, "1115");
    ...
    function takeList(address token, uint256 amount) external payable override {
        ...
        require(amount < orderInfo.stockAmount, "1119");
        takeAmount[msg.sender][token] = takeAmount[msg.sender][token].add(
            amount,
            "1113"
        );
        ...
        orderInfo.stockAmount = orderInfo.stockAmount.sub(amount, "1130");
        IApproveProxy approveProxy = config.getApproveProxy();
        TransferV1.transferFromFlush(
            approveProxy,
            orderInfo.quoteToken,
