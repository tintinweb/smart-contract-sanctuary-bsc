...
contract RewardToken is ERC20, ERC721Holder, Ownable {
    ...
    uint256 public EMISSION_RATE = (SafeMath.div(1, 2) * 10 ** decimals()) / 2 days;
    ...
    mapping(address => TransferHistory) public history;
    ...
    function transfer(address to, uint256 amount) public override returns (bool){
        if (checkLimit(msg.sender)> 1 days) {
            history[msg.sender].amount = 0;
            history[msg.sender].lastTransferTime = uint40(block.timestamp);
        }
        uint256 sum = history[msg.sender].amount;
        require(sum<TRANSFER_WHITELISTED, "limit reached");
        history[msg.sender].amount = history[msg.sender].amount + amount;
        transfer(to, amount);
    }

    function checkLimit(address _user) internal view returns (uint40){
        return uint40(block.timestamp - history[_user].lastTransferTime);
    }
    ...
    function calculateTokens(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 totalReward =timeElapsed * EMISSION_RATE;
