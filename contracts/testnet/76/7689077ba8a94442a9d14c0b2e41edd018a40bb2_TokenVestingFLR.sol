 ...
 * @title TokenVesting
 */
contract TokenVestingFLR is Ownable, ReentrancyGuard {
    ...
    modifier onlyIfVestingScheduleExists(bytes32 vestingScheduleId) {
        require(
            ...
            "TokenVestingFLR: INVALID Vesting Schedule ID! no vesting schedule exists for that ID"
        );
        _;
    }
    ...
    modifier onlyIfBeneficiaryExists(address beneficiary) {
        require(
            ...
            "TokenVestingFLR: INVALID Beneficiary Address! no vesting schedule exists for that beneficiary"
        );
        _;
    }
    ...
    function getVestingScheduleByBeneficiaryAndIndex(
        ...
        require(
            ...
            "TokenVestingFLR: INVALID Vesting Schedule Index! no vesting schedule exists at this index for that beneficiary"
        );
        return
            _getVestingSchedule(
                _computeVestingScheduleIdForAddressAndIndex(beneficiary, index)
            );
    }
    ...
    function _getVestingSchedule(bytes32 vestingScheduleId)
        private
        view
    ...
    function createVestingSchedule(
        ...
        require(_amount > 0, "TokenVestingFLR: amount must be > 0");
        tokenAddress.transferFrom(_msgSender(), address(this), _amount);
        require(_duration > 0, "TokenVestingFLR: duration must be > 0");
        require(
            _slicePeriodSeconds >= 1,
            "TokenVestingFLR: slicePeriodSeconds must be >= 1"
        );
        ...
        onlyIfBeneficiaryExists(msg.sender)
    {
        ...
        uint256 vestingSchedulesCountByBeneficiary = holdersVestingCount[
            beneficiary
        ];
        ...
        do {
            vestingSchedule = vestingSchedules[
                _computeVestingScheduleIdForAddressAndIndex(beneficiary, i)
            ];
