...
contract EXPONA_RBCF {
    ...
    function LevelReward()public view returns(uint256){
         return REGESTRATION_FESS / 50;
    }
    function Level1Reward()public view returns(uint256){
         return REGESTRATION_FESS / 5;
    }
    function AutopoolReward() public view returns(uint256){
         return REGESTRATION_FESS / 10 / height;
    }

    function LevelRewardToken()public view returns(uint256){
         return REGESTRATION_FESS / tokenPrice() * 1e18 / 50;
    }
    function Level1RewardToken()public view returns(uint256){
         return REGESTRATION_FESS / tokenPrice() * 1e18 / 5;
    }
    

    function AutopoolRewardToken()public view returns(uint256){
         return REGESTRATION_FESS / tokenPrice() * 1e18 / 10 / height;
    }
    ...
    function Registration(uint _referrerID, uint _coreferrerID,uint256 _amount) public payable {
        ...
        require(users[userList[_coreferrerID]].referredUsers >=2,"Co-Referral-should-be-2");
           ...
           if (!isTokenAcceptance) {
            ...
            require(_amount == REGESTRATION_FESS / tokenPrice() * 1e18, "Incorrect Value");
    ...
    function heightPayment(
        ...
        if (
            ...
            if (!isTokenAcceptance) {
                if(_value > 0){
                sent = address(uint160(nextLevel)).send(AutopoolReward());
                }else{
                   sent = stableCoin.transfer(address(uint160(nextLevel)), AutopoolReward());
            ...
            if (users[userList[i]].batchPaid < batchSize) {
                if (!isTokenAcceptance) {
                    ...
                    sent = address(uint160(userList[i])).send(
                        ...
                        // Need to Add USDT
                        sent = stableCoin.transfer(address(uint160(userList[i])), AutopoolReward());
                    }
                } else {
                    sent = token.transfer(
                        // msg.sender,
    ...
    function payReferral(uint _level, address _user, uint _value) internal {
        ...
        if (_level > 1) {
            level_price_local = LevelReward();
            level_price_localtoken = LevelRewardToken();
        } else {
            level_price_local = Level1Reward();
            level_price_localtoken = Level1RewardToken();
        }
        if (!isTokenAcceptance) {
            if(_value>0){
      ...
      function paycoReferral(uint _level, address _user, uint _value) internal {
        ...
        users[referer].stageIncomeRecieved =
            users[referer].stageIncomeRecieved +
            1;
