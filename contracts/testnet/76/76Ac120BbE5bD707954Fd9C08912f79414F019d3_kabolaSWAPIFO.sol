...
contract kabolaSWAPIFO is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 kabolaAtLastUserAction; // keeps track of kabola deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }
    //IFO
    struct UserIFOInfo {
        // ifo valid period is current block between startblock and endblock
        uint256 lastActionBalance; // staked kabola numbers (not include compoud kabola) at last action
        uint256 lastValidActionBalance; // staked kabola numbers in ifo valid period
    ...
    IBEP20 public immutable token; // kabola token
    ...
    /**
     * @notice Constructor
     * @param _token: kabola token contract
    ...
    /**
     * @notice Deposits funds into the kabola Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in kabola)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.kabolaAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    function increaseAllowance() public onlyOwner {
        IBEP20(token).safeApprove(address(masterchef), type(uint256).max);
    }
     ...
     * @param _lastValidActionBalance: staked kabola number at last action.
     ...
     * @param _amount:the kabola amount that need be add or sub
    ...
    /**
     * @notice original Withdraws implementation from funds, the logic same as kabola Vault withdraw
    ...
    /**
     * @notice Reinvests kabola tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the kabola Vault
     */
     ...
     * @return Expected reward to collect in kabola
     */
    function calculateHarvestkabolaRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingkabola(0, address(this));
     ...
     * @return Returns total pending kabola rewards
     */
    function calculateTotalPendingkabolaRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingkabola(0, address(this));
