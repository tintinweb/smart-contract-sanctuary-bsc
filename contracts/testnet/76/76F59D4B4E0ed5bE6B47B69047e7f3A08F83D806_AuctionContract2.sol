...
import '../interfaces/IAuctionContract2.sol';
import '@openzeppelin/contracts/token/ERC721/utils/ERC721Holder.sol';

/*
Contract đấu giá các vật phẩm
Các lần đấu giá sẽ được cộng dồn với nhau xem ai là người cao nhất đến cuối cùng
*/

contract AuctionContract2 is Ownable, ERC721Holder, IAuctionContract2 {
  ...
  function bidAuction(uint256 _auctionOrderId, uint256 _bidIncrease)
    external
    override
    CheckAuctionOrder(_auctionOrderId)
  {
    ...
    require(
      token.balanceOf(msg.sender) >= _bidIncrease,
    ...
    require(
      fundsByBidder[msg.sender][_auctionOrderId] + _bidIncrease > _order.highestBid,
      'There is already a higher or equal bid'
    );

    fundsByBidder[msg.sender][_auctionOrderId] += _bidIncrease;
    _order.highestBidder = msg.sender;
    _order.highestBid = fundsByBidder[msg.sender][_auctionOrderId];

    token.transferFrom(msg.sender, recipient, _bidIncrease);
    emit BidAuctionEvent(msg.sender, _auctionOrderId, _bidIncrease, uint32(block.timestamp));
  }
