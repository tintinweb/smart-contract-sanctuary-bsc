...
contract LIFEGAMES is IERC20 {
    ...
    uint256 private gasPriceLimit = 70000000000; // 70 gWei / gWei -> Default 25
    ...
    address payable public tokensReserveAddress;

    bool public contractSwapEnabled = true;
    uint256 public swapThreshold = 50000000000000000000;
    uint256 public swapAmount = 25000000000000000000;
    bool inSwap;
    ...
    constructor (address[] memory addresses) {
        ...
        setTokensReserveAddress(addresses[4]);
    ...
    function setLiquidityAddress(address _busdReserveAddress) public onlyOwner {
        require(
            _busdReserveAddress != address(0),
            "_busdReserveAddress address cannot be 0"
        );
        busdReserveAddress = payable(_busdReserveAddress);
    }

    function setTokensReserveAddress(address _tokensReserveAddress)
    public
    onlyOwner
    {
        require(
            _tokensReserveAddress != address(0),
            "_tokensReserveAddress address cannot be 0"
        );
        tokensReserveAddress = payable(_tokensReserveAddress);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        bool takeReflectionFee = true;

        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            takeReflectionFee = false;
        }

        return _finalizeTransfer(from, to, amount, takeReflectionFee && takeFeeEnabled);
    }

    function _finalizeTransfer(address from, address to, uint256 amount, bool takeReflectionFee) internal returns (bool) {
...
/*
        ...
        */
        ...
        if (takeReflectionFee) {
            uint256 amountReceived = takeTaxes(from, to, amount);
        }

        _tokenTransfer(from, to, amount, takeReflectionFee);
        return true;
    }
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        // calculate percentage to bsud reserver and manual buyback and burn reserve
        ...
        if (amountToBuyBackAndBurn > 0) {
            removeAllFee();
            _tokenTransferNoFee(address(this), tokensReserveAddress, amountToBuyBackAndBurn);
            restoreAllFee();
        }

                // todo test auto liquidity
            if (autoAddLiquidity) {

                // amounts
                uint256 estimatedTokensForAutoLiquidity = getOutEstimatedTokensForTokens(address(this), busdAddress, 10000000000000000000);
                ...
                if (estimatedTokensForAutoLiquidity > autoAddLiquidityThreshold) {
                    ...
                    IERC20(address(busdAddress)).transferFrom(busdReserveAddress, address(this), estimatedBusdForAutoLiquidity);
                    IERC20(address(this)).transferFrom(tokensReserveAddress, address(this), estimatedTokensForAutoLiquidity);
