...
contract Company is Ownable, Initializable {
    ...
    function initialize(address tokenAdd, uint allocation, uint subsequentClaims, uint _claimPeriod) external initializer {
        ...
        require(_claimPeriod > 0, "company : claimPeriod > 0");
        require(subsequentClaims > 0,"company : subsequentClaims > 0");

        token = IBEP20(tokenAdd);
        claimPeriod = _claimPeriod;
        subsequentClaim = subsequentClaims;
        require(IBEP20(token).transferFrom(msg.sender, address(this), allocation), "company : TranferFrom failure");
    ...
    function claim() external {
        ...
        total[1] += claimAmount;
        require(token.transfer(msg.sender, claimAmount), "company : Tranfer failure");
        emit Claim(
            msg.sender,
            claimAmount
        );
