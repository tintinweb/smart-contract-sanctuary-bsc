...
contract LandBureau is ModuleBase, Lockable {
    ...
    function grouponLand(uint256 amount, address parent) external lock {
        ...
        require(amount >= needMUTAmount, "amount too small");
    ...
    event debug1(uint256 a, uint256 b, uint256 c);
    event debug2(uint256 a_tokenId, uint256 b_tokenId, uint256 c_tokenId);
    event debug3(address account, uint256 rewardShared);
    event debug4(address account, uint256 rewardLose);
    ...
    function _grouponLand(
        ...
        if (rd.inCount == group_number) {
            ...
            emit debug2(a_tokenId, b_tokenId, c_tokenId);
            ...
            for (uint32 i = 1; i <= group_number; ++i) {
                ...
                if (isParent) {
                    ...
                    emit debug3(parentAddress, rewardShared);
                }
                ...
                if (
                    ...
                    emit debug4(ud.account, refund10);
                }
            }
        }
    }
