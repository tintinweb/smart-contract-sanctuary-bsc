...
contract DAIDAIMintNFT is ERC721Enumerable, Ownable {
    ...
    // maxBatch 
    uint256 public constant maxBatch = 10000;
    ...
    uint256 public constant maxBatchMint = 10;
    bool private started;

    // Optional mapping for token URIs
    mapping (uint256 => string) private _tokenURIs;

    constructor(string memory name_, string memory symbol_, string memory baseURI_, uint256 totalCount_, uint256 price_) public ERC721(name_, symbol_) {
    ...
    function batchMint(uint256 _times) public onlyOwner{
        ...
        for(uint256 i=0; i< _times; i++){
            ...
            _setTokenURI(newId, uint2str(newId));
        }
    }

    function batchMintTo(address user, uint256 _times) public onlyOwner{
        require(_times >0 && _times <= maxBatch, "wrong batch number");
    ...
    function estimatePrice(uint256 _times) public view returns (uint256) {
    ...
    function mintNFT(uint256 _times) payable public {
        require(started, "not started");
        require(_times >0 && _times <= maxBatchMint, "wrong batch number");
        require(totalSupply() + _times < totalCount, "wake too much");
        require(msg.value == estimatePrice(_times), "value error");
     ...
     */
    function setNftName(uint8 _nftId, string memory _name) external onlyOwner {
