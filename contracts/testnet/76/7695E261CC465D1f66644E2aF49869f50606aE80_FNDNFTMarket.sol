    ...
    NFTMarketAuction
{
    ...
    modifier onlyValidAuctionConfig(uint256 reservePrice) {
        require(reservePrice >= 10000000000000, "NFTMarketReserveAuction: Reserve price must be at least 10000000000000 wei");
        _;
    }
    ...
    {   
        ...
        auctionIdToAuction[auctionId] = ReserveAuction(
            ...
            reservePrice,
            BidderInfo(payable(address(0)), 0, 0) // Struct where we set address bidder, bid, and fee amount
        );
    ...
    function finalizeReserveAuction(uint256 auctionId) public nonReentrant { 
        ...
        ownersPoolCounter[auction.nftContract][auction.tokenId]++; //increase Owners Pool 
    ...
    SendValueWithFallbackWithdraw
{
    ...
    uint256 private _foundationFeeBasisPoints;
        ...
        )
    {
        ...
        // SafeMath is not required when dividing by a constant value > 0.
        FeesInfo.foundationFee = price.mul(_foundationFeeBasisPoints) / BASIS_POINTS;
        FeesInfo.fanFee = price.mul(FanFeeBasisPoints) / BASIS_POINTS;
        FeesInfo.ownersFee = price.mul(ownersFeeBasicPoints) / BASIS_POINTS;
        FeesInfo.ownerRev = price.sub(FeesInfo.creatorSecondaryFee).sub(FeesInfo.fanFee).sub(FeesInfo.ownersFee).sub(FeesInfo.foundationFee);
    }
    ...
    function _distributeFunds(
        ...
        _sendValueWithFallbackWithdrawWithMediumGasLimit(   // send fee to creator (artist)
            creatorFeeTo,
            FeesInfo.creatorSecondaryFee
        );
        _sendValueWithFallbackWithdrawWithMediumGasLimit(   // send fee to owner
            ownerRevTo,
            FeesInfo.ownerRev
        );
          _sendValueWithFallbackWithdrawWithMediumGasLimit(   // send fee to foundation (platform)
            getFoundationTreasury(),
            FeesInfo.foundationFee
        );
    }
     /**
     * @dev Allows to change the foundation fees.
     */

     function _calculateFoundationFees( uint256 amount) public view returns(uint256){
        require(
            _foundationFeeBasisPoints <= 500,
            "NFTMarketFees: foundation Fees > 5%"
        );
        return (amount.mul(_foundationFeeBasisPoints) / BASIS_POINTS);
     }

     /**
     * @dev Allows to change the creator fees.
     */
    ...
    function _updateMarketFees(
        ...
        _foundationFeeBasisPoints = foundationFeeBasisPoints;
