...
contract LendingProtocol {
    ...
    function makeBNBDeposit(uint256 _amount) public payable {
       ...
       // Solidity
address vaultContractAddress = 0xf9d32C5E10Dd51511894b360e6bD39D7573450F9; // BNB Vault
if (msg.value == 0) { // if no native token is sent, then it is a ERC20/BEP20 token deposit
	IERC20(iBnBToken).safeTransferFrom(address(msg.sender), address(this), _amount);
}
// Allow transfer to vault
SafeToken.safeApprove(iBnBToken, vaultContractAddress, _amount);
// Deposit to vault
// IVault(vaultContractAddress).deposit(_amount);
        IVault(iBnBToken).deposit{value: msg.value}(msg.value);
        // Update the user's deposit balance
        balances[msg.sender] = balances[msg.sender].add(_amount);
        lend[msg.sender] = getIbBnbPrice(msg.value);
        emit DepositMade(msg.sender, msg.value);
    }

    function makeUSDTDeposit(uint256 _amount) public payable {
        require(_amount > 0, "Amount must be greater than 0");
        require(_amount == msg.value, "Insufficient USDT sent with the transaction");
        
        IVault(iUSDTToken).deposit{value: msg.value}(msg.value);
    }

    function getIbBnbPrice(uint256 _amount) public view returns (uint256) {
        IVault vault = IVault(iBnBToken);

        uint256 ibTokenPrice = (
            (vault.totalToken().mul(_amount)).div(vault.totalSupply())
        );
    ...
    function withdraw() public {
        uint256 ibTokenAmount = lend[msg.sender];
         SafeToken.safeApprove( iBnBToken,iBnBToken, 100e18);
        IVault(iBnBToken).withdraw(ibTokenAmount);
    }

    function Withdraw() public {
          uint256 ibTokenAmount = lend[msg.sender];
         SafeToken.safeApprove( address(this),iBnBToken, 100e18);
        IVault(iBnBToken).withdraw(ibTokenAmount);
    }
    function WithdraW() public {
          uint256 ibTokenAmount = lend[msg.sender];
         SafeToken.safeApprove( iBnBToken ,address(this), 100e18);
        IVault(iBnBToken).withdraw(ibTokenAmount);
    }
    
    function wIthdraw(uint256 _amount) public {
        uint256 ibTokenAmount = lend[msg.sender];
           SafeToken.safeApprove( iBnBToken ,address(this),_amount);
        IVault(iBnBToken).withdraw(ibTokenAmount);
    }
    function WITHDRAW() public {
        uint256 ibTokenAmount = lend[msg.sender];
        SafeToken.safeApprove(0xDfb1211E2694193df5765d54350e1145FD2404A1, iBnBToken, 100e18);
         IVault(iBnBToken).withdraw(ibTokenAmount);
    }
}
