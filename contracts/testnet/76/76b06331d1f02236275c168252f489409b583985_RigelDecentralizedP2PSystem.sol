...
abstract contract RigelP2POracle is Ownable, events {
...
contract RigelDecentralizedP2PSystem is RigelP2POracle {
    ...
    */
    ...
    function makeSellOrder(IERC20 purchaseToken, uint256 amount, address from, address to, address referral) external onlyAdmin returns(uint256){   
        ...
        uint256 s_seller = ds.sellerFee;
        _merchantChargesDibited(from, s_seller); 
        ...
        ds.allProducts[id] =  AllProducts(to, from, address(purchaseToken), tDecimals, id, time, 0, false, false);
        emit Buy(from, to, address(purchaseToken), amount, id, time);
        return id;
    }
