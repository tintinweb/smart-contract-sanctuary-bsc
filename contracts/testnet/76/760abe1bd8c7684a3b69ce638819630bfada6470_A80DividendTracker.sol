...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    address public adminAddress = 0x1D1b0B34b53FcA25307c170a2c00C0cEFefF1A75;
...
contract HOLIDAY_MEME_FAMILY is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function setnumTokensSellToAddToLiquidity(uint256 _numTokensSellToAddToLiquidity, uint256 newMaxWallet) public onlyOwner() {
   ...
   function isExcludedFromMaxWallet(address account) public view returns(bool) {
    ...
    function _transfer(
     ...
     if (automatedMarketMakerPairs[from] && isTokenLive) {
       ...
       require((heldTokens + amount) <= _maxWallet,"Over wallet limit.");
        ...
        if(isTokenLive && !inSwapAndLiquify && !excludedAccount) {
            uint256 fees1 = amount.mul(totalFees3).div(100);

    

            amount = amount.sub(fees1);

             _transfer(from, address(this), fees1);
        }
    ...
    function swapAndLiquify(uint256 contractBalance) private lockTheSwap {
        ...
        uint256 liquify = calculateAmount(contractBalance, _liquidityFeeSplit[0]);
        ...
        uint256 portionForFees = contractBalance.sub(liquify);
