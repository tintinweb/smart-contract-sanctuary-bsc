...
contract DCApp{
    ...
    */
    ...
    event OccurenceExecuted(uint DCACreationTimestamp, address indexed userAddress, uint totalOccurence, 
                                uint currentOccurence, uint occurenceTimestamp, uint estimatedMinimumAmountOut);
    ...
    function addNewDCAToUser(uint _period, uint _totalOccurences, uint _amountPerOccurrence, address _tokenIn, address _tokenOut, uint _fee5Decimals)
        ...
        require(_fee5Decimals < 10000000, "Error: Fee must be inferior than 100%, with ");
    ...
    function executeSingleUserDCA(address user, uint DCAStartDate ) public {
        ...
        */
        ...
        uint amountWithoutFees = ((userDCA.amountPerOccurence * 100000) * (10000000 - userDCA.fee5Decimals)) / 10000000000;
        ...
        swap(userDCA.tokenIn, userDCA.tokenOut, userDCA.swapPath, amountWithoutFees, estimatedMinimumAmountOut, user);

        IERC20(userDCA.tokenIn).transfer(msg.sender, userDCA.amountPerOccurence - amountWithoutFees);
        emit OccurenceExecuted(DCAStartDate, user, userDCA.totalOccurences, userDCA.currentOccurence + 1, block.timestamp, estimatedMinimumAmountOut);
    }   
    ...
    function modifyDCAFee (uint _newFee5Decimals, uint _DCAStartDate) public {
        ...
        */
        require(_newFee5Decimals < 10000000);
        mapDCA[msg.sender][_DCAStartDate].fee5Decimals = _newFee5Decimals;
    }

    //Functions to set contract global variables, require to be the owner
