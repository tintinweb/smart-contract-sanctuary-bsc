...
contract Solarity is Initializable, ERC20Upgradeable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  */
  event TransferFeesChanged(uint256 marketingFee, uint256 liquidityFee, uint256 operationsFee, uint256 updatedTimestamp);
  ...
  */
  event BuyFeesChanged(uint256 marketingFee, uint256 liquidityFee, uint256 operationsFee, uint256 updatedTimestamp);
  ...
  */
  event SellFeesChanged(uint256 marketingFee, uint256 liquidityFee, uint256 operationsFee, uint256 updatedTimestamp);
  ...
  */
  function updateTransferFees(uint256 marketingFee, uint256 liquidityFee, uint256 operationsFee) public whenNotPaused onlySystemRoles
  {
    // SHEPARD/SENSEI: Validate that none of the fee settings are above 1000 individually.
    require(marketingFee <= 1000 && liquidityFee <= 1000 && operationsFee <= 1000, "One of the fees passed is greater 1000 (100%). Individual fee rates cannot exceed 100%.");
    // SHEPARD/SENSEI: Validate that the total sum of all fee values are not above or below 1000.
    require(marketingFee + liquidityFee + operationsFee <= 1000, "The sum of all fees is greater than 1000. Combined fees must not exceed 100%.");
    ...
    emit TransferFeesChanged(marketingFee, liquidityFee, operationsFee, block.timestamp);
  }
  ...
  * @param operationsFee    - Operations Fee.
  */
  function updateBuyFees(uint256 marketingFee, uint256 liquidityFee, uint256 operationsFee) public whenNotPaused onlySystemRoles
  {
    // SHEPARD/SENSEI: Validate that none of the fee settings are above 1000 individually.
    require(marketingFee <= 1000 && liquidityFee <= 1000 && operationsFee<= 1000, "One of the fees passed is greater 1000 (100%). Individual fee rates cannot exceed 100%.");
    // SHEPARD/SENSEI: Validate that the total sum of all fee values are not above or below 1000.
    ...
    emit BuyFeesChanged(marketingFee, liquidityFee, operationsFee, block.timestamp);
  }
  ...
  * @param operationsFee    - Operations Fee.
  */
  function updateSellFees(uint256 marketingFee, uint256 liquidityFee, uint256 operationsFee) public whenNotPaused onlySystemRoles
  {
    // SHEPARD/SENSEI: Validate that none of the fee settings are above 1000 individually.
    require(marketingFee <= 1000 && liquidityFee <= 1000 && operationsFee <= 1000, "One of the fees passed is greater 1000 (100%). Individual fee rates cannot exceed 100%.");
    // SHEPARD/SENSEI: Validate that the total sum of all fee values are not above or below 1000.
    ...
    emit SellFeesChanged(marketingFee, liquidityFee, operationsFee, block.timestamp);
  }
  ...
  */
  ...
  /**
  * @notice Updates the current Admin Wallet.
  * @dev The Admin Wallet can be change and therefore the public member must be configurable.
  *      This wallet can only be updated by the DEFAULT_ADMIN_ROLE role.
  *      If you want to add an administrator role to a wallet, please use the grantRole() function instead.
  * @param newWalletAddress - The new wallet the Admin wallet will represent.
  */
  function updateAdminWalletAddress(address newWalletAddress) external whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(adminWalletAddress != newWalletAddress , "The new wallet address cannot be equivalent current wallet address.");
    // SHEPARD: Immediately deny the old wallet and add the new wallet to all bypass and internal lists. 
    _bypassAllFees(adminWalletAddress, false);
    updateInternalList(adminWalletAddress, false);
    _bypassAllFees(newWalletAddress, true);
    updateInternalList(newWalletAddress, true);
    
    // SHEPARD: When updating the Admin Wallet, you first have to assign a new admin. Therefore, the first call must be an
    // grantRole() to the new wallet. Otherwise, you will loose complete access to the contract.
    grantRole(DEFAULT_ADMIN_ROLE, newWalletAddress);
    revokeRole(DEFAULT_ADMIN_ROLE, adminWalletAddress);

    // SHEPARD: Replace the public member with the new wallet value. 
    adminWalletAddress = newWalletAddress;
  }
