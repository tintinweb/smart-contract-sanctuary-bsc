...
contract Staking is ReentrancyGuard {
    ...
    function stakeToken(uint256 _amount) external payable nonReentrant {
        ...
        require(tokenStake.balanceOf(msg.sender) > _amount, "Balance must be greater than amount of staking!");
    ...
    function claimToken() external payable onlyStaker nonReentrant {
        require(block.timestamp > stakeInfos[msg.sender].endTS, "Not enough time to claim reward token");
        uint256 reward = (stakeInfos[msg.sender].amount * rewardRate * timeLock) / (100 * 31536000);
        // 31536000 means the total seconds per year
        require(totalRewardPool >= reward, "Not enough reward token in pool to claim");
        require(totalStake >= stakeInfos[msg.sender].amount, "Not enough staking token in pool to claim");
        SafeERC20.safeTransfer(tokenReward, msg.sender, reward);
        SafeERC20.safeTransfer(tokenStake, msg.sender, stakeInfos[msg.sender].amount);
        stakeInfos[msg.sender].amount = 0;
        totalRewardPool -= reward;
        totalStake -= stakeInfos[msg.sender].amount;
