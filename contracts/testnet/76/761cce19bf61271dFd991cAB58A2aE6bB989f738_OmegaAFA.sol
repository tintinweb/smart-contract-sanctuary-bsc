...
contract OmegaAFA is Ownable, ERC20Permit {
    ...
    uint256 public liquiAfaFeeRate = 30000; //3%  only for sell
    uint256 public liquiOmaFeeRate = 30000; //3%  only for buy
    uint256 public daoFeeRate = 20000; //2% dao fee.only for buy
    uint256 public firePitFeeRate = 20000; //2% only for sell
    ...
    ) ERC20('Omega AlfaDao', 'OMA', 9) ERC20Permit('Omega AlfaDao') {
    ...
    function takeFee(
        ...
        if (isBuy) {
            ...
            amount = amount.sub(daoFee).sub(liquiOmaFee);
        }
        ...
        //     amount = amount.sub(firePitFee).sub(liquiAfaFee);
        // }
    ...
    function _transferFrom(
        ...
        if (shouldAddLiquiAfa()) {
            addLiquiAfa();
        }

        if (shouldAddLiquiOma()) {
            addLiquiOma();
        }
    ...
    function addLiquiAfa() internal swapping {
        if (liquiAfaReceive > 0) {
            IAlfaDao(autoLiquiReceiver).addLiquiAfa(liquiAfaReceive);
            liquiAfaReceive = 0;
            _lastAddLiquiTimeAfa = block.timestamp;
        }
    }

    function addLiquiOma() internal swapping {
        if (liquiOmaReceive > 0) {
            IAlfaDao(autoLiquiReceiver).addLiquiOma(liquiOmaReceive);
            liquiOmaReceive = 0;
    ...
    function shouldAddLiquiAfa() internal view returns (bool) {
        return _autoAddLiquidity && !inSwap && msg.sender != pair && _lastAddLiquiTimeAfa > 0 && block.timestamp >= (_lastAddLiquiTimeAfa + autoLiquidityInterval);
    }

    function shouldAddLiquiOma() internal view returns (bool) {
        return _autoAddLiquidity && !inSwap && msg.sender != pair && _lastAddLiquiTimeOma > 0 && block.timestamp >= (_lastAddLiquiTimeOma + autoLiquidityInterval);
    }
