...
contract SantaGift is ERC20, ERC20Burnable, Ownable {
    // ADDRESSESS -------------------------------------------------------------------------------------------
    ...
    uint256 autoAddLiquidityPercent;
    uint256 buybackPercent;
    uint256 burnPercent;
    uint256 taxDivisor; // divisor | 0.0001 max presition fee
    uint256 maxWalletAmount; // max balance amount (Anti-whale)
    uint256 maxTransactionAmount; // max transaction amount (Anti-whale)
    uint256 buybackThreshold; // buyback tokens limit
    ...
    // CONSTRUCTOR ------------------------------------------------------------------------------------------
    constructor() ERC20("SantaGift", "SGT") {
        // mint tokens to deployer
        _mint(msg.sender, 1000000 ether);

        maxWalletAmount = 1000000 ether;
        maxTransactionAmount = 1000000 ether;

        // marketing address
        w1Address = 0x27446e20F489cF6a8AE1c38143a6fDF2bfdE6d6c;
        w2Address = 0x5057Fd1ec4f4CbB4E07780139c531D8ACf07bbAe;

        // default fees
        // 0% on BUY
        // 8% on SELL
        // 0% on Transfer
        _feesRates = Fees({buyFee: 0, sellFee: 800, transferFee: 0});
        ...
        // contract do swap when have 100 tokens balance
        swapThreshold = 100 ether;

        w1AddressPercent = 2000; //20%
        w2AddressPercent = 2000; //20%
        burnPercent = 2000; //20%
        autoAddLiquidityPercent = 2000; //20%
        buybackPercent = 2000; //20%
    ...
    function setTaxes(
        ...
        require(
            buyFee + sellFee + transferFee <= 800, // 8% max
    ...
    function _transfer(
        ...
        if (contractMustSwap(from, to)) {
            ...
            // send tokens to team wallet
            super.transfer(
                w1Address,
                calculatePercentage(tokenBalance, w1AddressPercent)
            );
            super.transfer(
                w2Address,
                calculatePercentage(tokenBalance, w2AddressPercent)
            );

            // inject liquidity
            autoAddLiquidity(
                calculatePercentage(tokenBalance, autoAddLiquidityPercent)
            );

            // sell tokens for buyback reserve
            swapTokensForBNB(calculatePercentage(tokenBalance, buybackPercent));
            ...
            // burn tokens
            super.burn(calculatePercentage(tokenBalance, burnPercent));
        }
    ...
    function calculatePercentage(uint256 amount, uint256 percentage)
        ...
        returns (uint256)
    {
        return (amount * percentage) / taxDivisor;
    }
    ...
    function calcBuySellTransferFee(
        ...
        if (totalFeePercent > 0) {
            feeAmount = calculatePercentage(amount, totalFeePercent);
        }
    ...
    function autoAddLiquidity(uint256 tokenAmount) public {
        ...
        // swap tokens for ETH
        swapTokensForBNB(halfTokens); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newEthBalance = address(this).balance - initialBalance;
    ...
    function blackListAddress(address _addr, bool _value)
        public
        virtual
        onlyOwner
    {
    ...
    function setBuybackThreshold(uint256 value) public virtual onlyOwner {
        buybackThreshold = value;
    }
