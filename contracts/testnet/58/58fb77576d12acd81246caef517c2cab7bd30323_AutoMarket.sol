...
contract AutoMarket is LPSwapSupport {
    ...
    event WithdrawBusd(address indexed spender, uint256 amount);
    ...
    function withdrawBusd() external authorized{
        require(users[_msgSender()] != 0, "User: no busd to withdraw");
        uint256 busdBalance = BUSD.balanceOf(address(this));
        uint256 amount = busdBalance.mul(users[_msgSender()]).div(totalAmount);

        BUSD.transfer(_msgSender(), amount);

        totalAmount = totalAmount.sub(users[_msgSender()]);
        users[_msgSender()] = 0;
        
        emit WithdrawBusd(_msgSender(), amount);
    }    
}
