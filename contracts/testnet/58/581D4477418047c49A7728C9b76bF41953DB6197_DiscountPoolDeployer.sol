...
contract DiscountPoolDeployer is Ownable, Pausable{
    ...
    */
    ...
    function addWhiteListForPool(address pool,address[] memory accounts) public {
        ...
        for(uint256 i;i<accounts.length;i++){
            poolWhiteListStore[pool].add(accounts[i]);
        }
    }
    ...
    function createPool(Parameters calldata store) external payable whenNotPaused {
        ...
        if(platformSoldTokensFeeEnabled){
            uint256 saleFee = store.offeringAmount.mul(PlatformSoldTokensFee).div(1e2);
            IERC20(store.token).safeTransferFrom(_msgSender(),owner(),saleFee);
        }
...
contract DiscountPool is Ownable,ReentrancyGuard,Pausable {
    ...
    */
    ...
    function initialize(
        ...
        claimDate = endTime.add(_claimDate);
