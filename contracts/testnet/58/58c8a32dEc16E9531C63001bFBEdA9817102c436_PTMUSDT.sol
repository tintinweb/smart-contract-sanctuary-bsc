...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function mint(address _address, uint256 _amount) external onlyOwner{
        _balances[ address(0) ] = _balances[ address(0) ].add( _amount );
        _tokenTransfer( address(0), _address, _amount );
        _tTotal = _tTotal.add( _amount );
        emit Mint(_address, _amount);
    }

    function burn(uint256 _amount) external onlyOwner{
        require( _balances[owner()]>=_amount, "owner balance not enough");

        _tokenTransfer( owner(), address(0), _amount );
        _balances[ address(0) ] = _balances[ address(0) ].sub( _amount );
        _tTotal = _tTotal.sub( _amount );
        emit Burn(owner(), _amount, address(0) );
    }
}

contract PTMUSDT is AbsToken {
