...
import "../nft/SarmetCard.sol";
import "../referral/Referral.sol";
import "../interfaces/ISarmatBoxIDO.sol";

contract SMTCMintable is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    uint256 public price = 199 * 10**18;
    uint256 public referralFee = 100 * 10**18;
    address public vault;
    address public referralVault;
    IERC20 public USDT;
    SarmetCard public SMTCard;
    ISarmatBoxIDO public boxIDO;
    ...
    function setNFT(SarmetCard _SMTCard) external onlyOwner {
        SMTCard = _SMTCard;
    }

    function setSarmatBoxIDO(ISarmatBoxIDO _ido) external onlyOwner {
        boxIDO = _ido;
    }
    ...
    function setPrice(uint256 _price) external onlyOwner {
        require(
            _price > referralFee,
            "unit price cannot be less than the referral fee"
        );
        price = _price;
    }

    function setReferralFee(uint256 _fee) external onlyOwner {
        require(_fee < price, "referral fee cannot be greater than unit price");
        referralFee = _fee;
    }
    ...
    function setReferralVault(address _vault) external onlyOwner {
        referralVault = _vault;
    }

    function subscribe() external {
        require(!isPayable[msg.sender], "already subscribed");
    ...
    function collect() external {
        require(isTaskCompleted(msg.sender), "task not completed");
        isCollect[msg.sender] = true;
        SMTCard.mintSarmetCard(msg.sender, 1);

        address referrer = referral.referrers(msg.sender);
        if (referrer != address(0) && SMTCard.balanceOf(referrer) > 0) {
            USDT.safeTransferFrom(referralVault, referrer, referralFee);
            referralReward[referrer] = referralReward[referrer].add(
                referralFee
            );
        }
    }

    function isTaskCompleted(address account) public view returns (bool) {
        if (address(boxIDO) == address(0) || address(SMTCard) == address(0)) {
            return false;
        }
        if (isCollect[account]) {
            return false;
        }
        return
            isPayable[account] &&
            boxIDO.numberOfsuccessfulReferrals(account) > 10;
    }
}
...
contract SarmetCard is ERC721("Sarmet Card", "SMTC"), Ownable {
    using SafeMath for uint256;
    ...
    function _beforeTokenTransfer(
        ...
        if (address(dispatchHandle) != address(0)) {
            ...
            if (from != address(0)) {
                dispatchHandle.withdraw(from, power);
            }
            dispatchHandle.stake(to, power);
        }
    }
}
...
contract Referral is IReferral, Ownable {
    uint256 public constant MAX_LIMIT = 300;
    mapping(address => address) internal _referrers;
    mapping(address => address[]) internal _recommended;

    constructor(address _initReferrer) {
        _referrers[_initReferrer] = address(this);
        _recommended[address(this)].push(_initReferrer);
        emit SetReferrer(_initReferrer, address(this));
    }
    ...
    function setReferrer(address _referrer) public virtual override {
        ...
        require(
            _recommended[_referrer].length < MAX_LIMIT,
            "recommended limit"
        );
