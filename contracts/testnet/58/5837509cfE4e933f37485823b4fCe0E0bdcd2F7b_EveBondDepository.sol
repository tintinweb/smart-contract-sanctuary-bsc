...
contract EveBondDepository is Ownable {
    ...
    struct Bond {
        uint256 payout; // EVE remaining to be paid
    ...
    function deposit(
        ...
        // uint profit = _amount.mul( 5 ).div( 100 );
        IERC20(principle).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(principle).approve(address(treasury), _amount);
        ITreasury(treasury).deposit(_amount, principle, _maxPrice);
        // total debt is increased
        totalDebt = totalDebt.add(_amount);
        // depositor info is stored
        bondInfo[_depositor] = Bond({ payout: bondInfo[_depositor].payout.add(_maxPrice), vesting: terms.vestingTerm, lastTime: uint32(block.timestamp), pricePaid: priceInUSD });

        // indexed events are emitted
        emit BondCreated(_amount, _maxPrice, block.timestamp.add(terms.vestingTerm), priceInUSD);
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(Eve).totalSupply();
    ...
    function getPrinciple() public view returns(uint) {
        return IERC20(principle).balanceOf(address(this));
    }

    function getEVE() public view returns(uint) {
        return IERC20(Eve).balanceOf(address(this));
    }
}