...
contract BearToken is Context, IERC20, Ownable {
    ...
    uint8 public constant maxLiqFee = 5;
    uint8 public constant maxTaxFee = 5;
    uint8 public constant maxBurnFee = 5;
    uint8 public constant maxMarketingFee = 5;
    uint8 public constant maxBuybackFee = 5;
    ...
    //address public testnet = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    ...
    event burnWalletUpdate(uint8 indexed _buyBackFeeUpdate, uint8 indexed _marketingFeeUpdate);
    ...
    constructor (address tokenOwner,string memory tokenName,
        ...
        address payable _marketingWallet, address payable _treasuryWallet
        )  {
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 burnFee, uint8 marketingFee, uint8 buybackFee) external onlyOwner() {
        require(taxFee >= 0 && taxFee <=maxTaxFee,"TF err"); //Sets max tax fee at 5
        require(liquidityFee >= 0 && liquidityFee <=maxLiqFee,"LF err"); //Sets max liquidity fee at 5
        require(burnFee >= 0 && burnFee <=maxBurnFee,"BF err"); //Sets max burn fee at 5
        require(marketingFee >= 0 && marketingFee <=maxMarketingFee,"WF err"); //Sets max wallet fee at 5
        require(buybackFee >= 0 && buybackFee <=maxBuybackFee,"BBF err"); //Sets max buyback fee at 5
        _taxFee = taxFee;
        ...
        emit burnWalletUpdate(buybackFee, marketingFee);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(_buybackFee != 0){
            ...
            _tokenTransferNoFee(address(this), treasuryWallet, spentAmount);
            totSpentAmount = totSpentAmount + spentAmount;
        }
