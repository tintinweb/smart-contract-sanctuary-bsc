...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    address public immutable RWRD;
    
    mapping(address => int256) internal magnifiedDividendCorrections;
    mapping(address => uint256) internal withdrawnDividends;

    constructor(string memory _name, string memory _symbol, address _RWRD) ERC20(_name, _symbol) { 
        RWRD = _RWRD;
    }
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(uint256 minBalance, address _RWRD) DividendPayingToken("Reward Tracker", "DividendTracker", _RWRD) {
    	claimWait = 3600;
...
contract BabyToken is ERC20, Ownable, BaseToken {
    ...
    function swapAndSendDividends(uint256 amount) private{
        ...
        uint256 balanceRWRD = IERC20(RWRD).balanceOf(address(this));
        bool success = IERC20(RWRD).transfer(address(dividendTracker), balanceRWRD);

        if (success) {
            dividendTracker.distributeDividends(balanceRWRD);
            emit SendDividends(balanceRWRD);
        }
    }
