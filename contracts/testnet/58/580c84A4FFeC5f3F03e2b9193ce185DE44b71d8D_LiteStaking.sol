...
contract LiteStaking is StakeInfo {
    address public immutable upgradeAddress; // 升级合约
    address public immutable tokenAddress; // token合约地址
    address public owner; // 合约所有者

    uint256 public bounsDay = 30; //20 d
    uint256 public constant bounsDuration = 600; // 奖励周期 86400
    uint256 public unstakeDuration = 600; // 活期解压周期 86400
    uint256 public autoDelay = 3 * bounsDuration; // 过期3个分红周期(3d),自动延期

    bool public isStaking = false; // 是否开启质押
    bool public closeUnstake = false; // 关闭的池子允许直接解押
    ...
    function _updatePool(uint256 pid) private {
        if (
            ...
            uint256 _reward = (lastBounsToken * poolInfo[pid].allocPoint) /
                totalAllocPoint;
        ...
        returns (uint256)
    {
        ...
        if (_user.stakedOf > 0) {
            ...
            uint256 _epoch_day = block.timestamp / bounsDuration;
            if (_epoch_day > lastBounsEpoch && poolInfo[pid].allocPoint > 0) {
