...
contract AladMiner is Ownable {
    ...
    function deposit(uint256 amount) external {
        ...
        for (uint256 ind = 1; ind <= 20; ind++) {
            ...
            acc.teamCapital = acc.teamCapital.add(
                amount.mul(1e18).div(getp1(address(token)))
            );
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 cWithdraw = acc.selfExpectedRewards.mul(r).mul(getp1(address(token))).div(
            1e18
        );
        ...
        return
            accRes >= cWithdraw
                ? (cWithdraw, cWithdraw.mul(getp1(address(token))).div(1e18))
                : (accRes, accRes.mul(getp1(address(token))).div(1e18));
    }
        ...
        return
            acc.getDynamicRewards <= accRes
                ? (
                    ...
                    acc.getDynamicRewards.mul(getp1(address(token))).div(1e18)
                )
                : (accRes, accRes.mul(getp1(address(token))).div(1e18));
    }
