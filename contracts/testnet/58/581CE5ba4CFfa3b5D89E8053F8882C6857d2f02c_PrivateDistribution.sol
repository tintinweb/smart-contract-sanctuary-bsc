...
contract PrivateDistribution is Ownable {
    ...
    modifier onlyICO() {
        require(ico != address(0), "Ask Admin to link ICO");
        require(
            msg.sender == ico,
            "addInvestor role is not assigned to this address"
        );
        _;
    }
    ...
    function investorExists(address _investor) public view returns (bool){
        return investorsInfo[_investor].exists;
    }

    function investorAllotedTokens(address _investor)
        public
        view
        returns (uint256)
    {
        return investorsInfo[_investor].tokensAllotment;
    }
    ...
    function addInvestor(address _investor, uint256 _tokensAllotment)
        external
        onlyICO
    {
        ...
        totalAllocatedAmount = totalAllocatedAmount + _tokensAllotment;
        emit InvestorAdded(_investor, _msgSender(), _tokensAllotment);
    }

    function removeInvestor(address _investor) external onlyICO {
        uint256 tokenAmount = investorsInfo[_investor].tokensAllotment;
        delete investorsInfo[_investor];
        totalAllocatedAmount -= tokenAmount;
        emit InvestorRemoved(_investor, _msgSender(), tokenAmount);
    }

    // 5% on listing and rest monthly distribution from day 31 for 17 months (18 months)
    function withdrawTokens() external onlyInvestor initialized {
        ...
        timeToken.safeTransfer(_msgSender(), tokensAvailable);
    ...
    function setIco(address _ico) external onlyOwner {
        require(_ico != address(0), "Cammot be 0 address");
        ico = _ico;
    }
    ...
    {
        // 5% on listing and 18 months linear Vesting

        // 23,333,333,333.31 TimeToken assigned
        // 1,16,66,66,666.6655 tokens on TGE - 5% on TGE
        // 22,16,66,66,666.6445 tokens distributed for 18 months - 95% remaining
        // 22,16,66,66,666.6445/18 = 1,23,14,81,481.48025 tokens per month
        // 95/18 = 5.2777% every month released

        uint256 thirtyOneDays = initialTimestamp + 31 days;
        uint256 remainingDistroPercentage = 95;
        uint256 noOfRemaingMonths = 18;
        uint256 currentTimeStamp = block.timestamp;
        uint256 everyMonthReleasePercentage = (remainingDistroPercentage *
            (1e18)) / (noOfRemaingMonths);
        // Date difference in days - (endDate - startDate) / 60 / 60 / 24;
        uint256 noOfDays = (currentTimeStamp - (initialTimestamp)) /
            (60) /
            (60) /
            (24);
        uint256 monthsSinceInitial = noOfDays / (30);

        if (currentTimeStamp > initialTimestamp) {
            if (currentTimeStamp <= thirtyOneDays) {
                return uint256(5) * (1e18);
            } else if (
                currentTimeStamp > thirtyOneDays && monthsSinceInitial < 18
            ) {
                uint256 currentUnlockedPercentage = (monthsSinceInitial *
