...
contract PoolsSOW is Ownable, ReentrancyGuard {
    ...
    function currentRewardFor6912(uint pid, address user, bool isWD) public view returns(uint) {
        ...
        if(userClaimed[user][pid].length > 0) {
            ...
            if(claim.date > u.startTime / interestPeriod) spendDays = getDays() - claim.date;
        }
        if(!isWD) {
    ...
    function withdraw(uint pid) public nonReentrant {
        ...
        if(pid > 1 * interestPeriod) _claimRewardFor6912(pid, true);
    ...
    function _claimRewardFor6912(uint pid, bool isWd) internal {
        require(pid > 1 * interestPeriod, 'Pools::claimRewardFor6912: pid invalid');
