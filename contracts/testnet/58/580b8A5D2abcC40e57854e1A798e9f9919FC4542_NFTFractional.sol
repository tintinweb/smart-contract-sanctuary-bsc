...
contract NFTFractional is Upgradable {
    ...
    */
    function createFNFTPool(address[] memory _addrs, uint256[] memory _datas, uint256[] memory _configs, uint256[] memory _tiers) external onlyAdmins {
        ...
        // exchange rate > 0
        require(_datas[2] > 0, "NFTFractional: Exchange Rate must greater then zero");
        // pool balance > 0
        require(_datas[1] > 0, "NFTFractional: Pool balance must greater then zero");
        ...
        FNFTInfo memory fnft = fnftInfos[_datas[1]];
        // check fnt exists
        ...
        fnft.availableSupply = fnft.availableSupply - _datas[2];
        FNFTPool memory pool = FNFTPool(_addrs[0], _addrs[1], _datas[0], _datas[1], _datas[2], _datas[3], _datas[4], _configs, _tiers);
    ...
    */
    function createTieringPool(address _addr, uint256[] memory _datas) external onlyAdmins {
        ...
        TierPool memory pool;
        pool = tierPools[_datas[0]];
        if (pool.stakingToken == address(0)) {
            pool = TierPool(_addr, 0, 0, _datas[1], _datas[2]);
            tierPools[_datas[0]] = pool;
        } else {
    ...
    */
    function createRewardPool(address _addr, uint256[] memory _datas) external onlyAdmins {
        ...
        // poolOpenTime > purchase end time of f-nft pool
        require(_datas[3] > fnftPool.configs[3], "NFTFractional: Pool Open Time must greater than purchase end time of f-nft pool");
        // exchangeRate > 0
    ...
    * @param _datas poolId(0), amount(1), alloction(2)
    * @param _signature signature of user
    */
    function purchaseFNFT(uint256[] memory _datas, bytes memory _signature) external {
        require(
        ...
        uint256 purchasedFNFT = _datas[1] * pool.exchangeRate / 1e8;
        if (userInfo.alloction == 0) {
            userInfo.alloction = _datas[2];
        }
        require(purchasedFNFT + userInfo.purchased <= userInfo.alloction, "NFTFractional: Limit allocation");

        IERC20(pool.acceptToken).transfer(pool.receiveAddress, _datas[1]);
        IERC20(fnftInfo.tokenFNFT).transfer(msg.sender, purchasedFNFT);

        userInfo.purchased += _datas[1];
        emit PurchaseFNFT(_datas[0], userInfo.purchased, userInfo.alloction - userInfo.purchased, purchasedFNFT);
    }
    ...
    */
    function stakeTierPool(uint256[] memory _datas) external {
        ...
        IERC20(pool.stakingToken).transfer(address(this), _datas[1]);
    ...
    */
    function unStakeTierPool(uint256[] memory _datas) external {
        ...
        require(userInfo.unStakeLastTime + pool.withdrawDelayDuration + ONE_DAY_IN_SECONDS < block.timestamp, "NFTFractional: User is in withdraw delay duration");
...
interface ISignatureUtils {
    function getMessageHash(uint256 poolId,uint256 amount,address sender) external returns (bytes32);
    function getEthSignedMessageHash(bytes32 _messageHash) external returns (bytes32);
    function verify(uint256 poolId,uint256 amount,address sender,address signer,bytes memory signature) external returns (bool);
