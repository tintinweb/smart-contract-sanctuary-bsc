...
contract iSStudioWorksMoneyPrinterContract is OwnerAdminSettings {
    ...
    event BuyFeeChanged(uint256 fee, uint256 timestamp);
    event SellFeeChanged(uint256 fee, uint256 timestamp);
    ...
    constructor (
        ...
        uint256 feeBps_, // Applies to both Buy & Sell Fees. 1 = .1%, 1000 = 100%
    ...
    function initialize() external payable onlyOwner {
        ...
        (bool srvcFeePaid,) = payable(setupFeeReceiver).call{value: (msg.value), gas: transferGas}("");
    ...
    function buyMoney(address ref) external payable {
        ...
        if (isWhitelisted[msg.sender]) {
            ...
            moneyBought = SafeMath.sub(moneyBought,buyFee(moneyBought));
    ...
    function printMoney(address ref, uint256 amtMoney) external {
        require(initialized);
        require(amtMoney <= getMyMoney(msg.sender), "YOU CANNOT COMPOUND WITH MORE THAN WHAT YOU HOLD!");
    ...
    function sellMoney(uint256 amtMoney) public {
        require(initialized);
        ...
        uint256 fee = sellFee(moneyValue);
    ...
    function buyFee(uint256 amount) private view returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount,(maintenanceFeeBps + buyFeeBps)),1000);
    }

    function sellFee(uint256 amount) private view returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount,(maintenanceFeeBps + sellFeeBps)),1000);
    }
    ...
    function setMaintenanceFeeBps(uint256 feeBps) public onlyOwner {
    ...
    function setBuyFeeBps(uint256 feeBps) public onlyOwner {
        require(feeBps >= minFeeBps && feeBps <= maxFeeBps, "1 = .1%, 1000 = 100%, Fee provided is out of limits");
        buyFeeBps = feeBps;
        emit BuyFeeChanged(feeBps, block.timestamp);
    }

    function setSellFeeBps(uint256 feeBps) public onlyOwner {
        require(feeBps >= minFeeBps && feeBps <= maxFeeBps, "1 = .1%, 1000 = 100%, Fee provided is out of limits");
        sellFeeBps = feeBps;
        emit SellFeeChanged(feeBps, block.timestamp);
    }
