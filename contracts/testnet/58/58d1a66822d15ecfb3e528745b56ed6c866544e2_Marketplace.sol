...
contract Marketplace is AccessControl, Pausable, IERC721Receiver {
    ...
    /** Stale time */
    uint256 public  staleTime = 90 days;
  ...
  */
    ...
    /** @notice staleListing functions of contract.
    @dev Available only to admin.
  */
    function setStaleTime (uint256 staleT) external onlyRole(DEFAULT_ADMIN_ROLE){
        require (staleT > 20, "Stale time should be greator than 20 days");
        staleTime = staleT * 1 days;
    }
     ...
     * @param bidAmount Amount in Allowed tokens.
     */
        ...
        whenNotPaused
    {
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            // Return Allowed tokens to the last bidder
    ...
    function finishAuction(uint256[] memory orderId) external whenNotPaused {
        ...
        for (uint8 i = 0; i < len; i++) {
            ...
            if (numBids > 2) {
                ...
                // Return Allowed to the last bidder
                if (numBids > 0)
                    _transferTokens(
                        order.token,
                        address(0),
    ...
    /** @notice Exchanges Allowed tokens and Items between users.
     * @dev `payer` here is either `itemRecipient` or `address(0)`
     * which means that we should transfer Allowed from the contract.
     *
     * @param orderId Order ID.
     * @param itemContract Item contract address.
     * @param itemId Item ID.
     * @param price Item price in Allowed tokens.
     ...
     * @param amount Transfer amount in Allowed tokens.
     */
