...
contract escrow is Backend, OwnableUpgradeable {
    ...
    uint public TxFee;
    function initialize(address _cbs, address _cbr,uint _TxFee) public initializer {
        __Backend_init();
        __Ownable_init();
        TxFee=_TxFee;
        CBS = ICBS(_cbs);
    ...
    function adminRelease(BackendSigner memory whitelist) external{
     ...
     function raiseDispute(BackendSigner memory whitelist) external{
        require (getSigner(whitelist) == signer, '!Signer');
        require (msg.sender == owner(),'!Not A Owner');
        uint amount = mileStonePaymentAmount[whitelist.senderAddress][whitelist.slotId]
        [senderSlotToReceiver[whitelist.senderAddress][whitelist.slotId]]
        ...
        if (_spents[whitelist.senderAddress][whitelist.slotId] + amount == _deposits[whitelist.senderAddress][whitelist.slotId])
            delete _deposits[whitelist.senderAddress][whitelist.slotId];
        _spents[whitelist.senderAddress][whitelist.slotId] += amount;
        CBS.transfer(whitelist.senderAddress, amount * 1 ether);
        CBR.burnCBRFromPartnerControllers(whitelist.senderAddress, amount );
        emit ForcedWithdrawn(whitelist.senderAddress, amount);
    }
    ...
    function payTxFee() external {
        CBE.transfer(owner(),(TxFee/100)*1 ether);
    }
