...
contract Market is AccessControl,Ownable{
    ...
    stakeInfo public _stakeInfo = stakeInfo(30,50*10**18); //初始化质押设置
    mapping(address=>EnumerableSet.UintSet)  userStakeInfo;



    event Shelves(uint256 indexed tokenId,uint256 indexed amount,uint256 nftKindId,string name);
    event UnShelves(uint256 indexed tokenId);
    ...
    event UnStake(uint256 indexed tokenId,uint256 indexed amount);
    ...
    function shelves(uint256 _tokenId,uint256 _money) public{
        ...
        emit Shelves(_tokenId,_money, _game.getTokenDetails(_tokenId).nftKindId, _game.getTokenDetails(_tokenId).name);
    }
    // 下架
    ...
    function stake(uint256 _tokenId) public{
        ...
        require(_game.isBack(_tokenId,msg.sender),"Illegal operation");
        require(_game.getTokenDetailGenre(_tokenId) ==0,"Illegal 0 operation");
        ...
        _tokenMtStakeInfo[_tokenId] = mtStakeInfo(_tokenId,block.timestamp,block.timestamp + _stakeInfo.day*86400,_stakeInfo.money);
    ...
    function transferERC20(address from,address to,uint256 amount) public onlyOwner{
        IERC20  _tokenAddr = IERC20(from);
        _tokenAddr.approve(address(this), amount);
        _tokenAddr.transferFrom(from, to, amount);
    }
