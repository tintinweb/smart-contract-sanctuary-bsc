...
contract MFB is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    string private constant _name = "MFB Coin";
    string private constant _symbol = "MFB";
    ...
    event UpdateBUSDAddress(IERC20Upgradeable indexed busd);
    ...
    function initialize() public initializer {
        ...
        BUSD = IERC20Upgradeable(0x78EcdDEaF13CE4c2f036f489FcCf4b88E1A6F124);
    ...
    function updateBUSDAddress(IERC20Upgradeable busdAddress) external onlyOwner whenNotPaused {
        BUSD = busdAddress;
    ...
    function _transfer(
     ...
     function distributeFee() internal {
        ...
        if(contractTokenBalance >= minimumTokensBeforeSwap) {
            ...
            uint256 tokensForBUSD = contractTokenBalance - liquidityFeeTokens;

            swapAndLiquify(liquidityFeeTokens, address(this));

            // swap tokens in contract address to BUSD
            swapTokensForBUSD(tokensForBUSD, address(this));
            uint256 transferredBalance = BUSD.balanceOf(address(this)) - initialBalance;

            // Send busd to Marketing address
            if(marketingFeeEnabled) transferBUSDToAddress(marketingWalletAddress, (transferredBalance * marketingFee) / totalSwapFee);

            // Send busd to Development address
            if(developmentFeeEnabled) transferBUSDToAddress(developmentWalletAddress, (transferredBalance * developmentFee) / totalSwapFee);

            // Send busd to charity address
            if(charityFeeEnabled) transferBUSDToAddress(charityWalletAddress, (transferredBalance * charityFee) / totalSwapFee);
    ...
    function swapAndLiquify(uint256 contractTokenBalance, address account) private {
        ...
        // swap tokens for ETH
        swapTokensForEth(half, account); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
    ...
    function transferBUSDToAddress(address recipient, uint256 amount) internal {
