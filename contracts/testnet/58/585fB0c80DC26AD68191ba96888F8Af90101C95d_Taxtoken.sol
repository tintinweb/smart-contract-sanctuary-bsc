...
contract Taxtoken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "TAXTOKEN10.io";
    string private _symbol = "TAXTOKEN10";

    uint256 public _taxBurnPercentage = 200;
    uint256 public _taxRewardPercentage = 1000;
    ...
    constructor(address router_) {
        ...
        // setPair(f.createPair(address(this), router.WETH()), true);

        address[] memory feeRewardSwapPath = new address[](2);
        feeRewardSwapPath[0] = address(this);
        feeRewardSwapPath[1] = router.WETH();
    ...
    function _transfer(
        ...
        if (_pair[to] && !_feeExcluded[from]) {
            uint256 feeBurnAmount = 0;
            uint256 feeRewardAmount = 0;

            if (_taxBurnPercentage > 0) {
                feeBurnAmount = amount.mul(_taxBurnPercentage).div(10000);
                _totalSupply = _totalSupply.sub(feeBurnAmount);
                emit Transfer(from, address(0), feeBurnAmount);
            }

            if (_taxRewardPercentage > 0 && _rewardAddress != address(0)) {
                feeRewardAmount = amount.mul(_taxRewardPercentage).div(10000);
                if (_router != address(0)) {
                    _balances[address(this)] = _balances[address(this)].add(
                        feeRewardAmount
                    );
                    emit Transfer(from, address(this), feeRewardAmount);
                    ...
                    _balances[_rewardAddress] = _balances[_rewardAddress].add(
                        feeRewardAmount
                    );
                    emit Transfer(from, _rewardAddress, feeRewardAmount);
                }
            }
