...
contract Sneaker is BEP721, Ownable {
    ...
    mapping(uint256 => uint256) mintBoxPrice; // use for summon 
    mapping(uint256 => uint256) tokenBlocktime; // tokenID => block.timestamp
    mapping(uint256 => uint256) tokenMintTime; // tokenID => time minted
    ...
    constructor(address _MFG) BEP721("Sneaker Token", "SNEAKER") {
        ...
        // * ********************************* * //
        mintBoxPrice[1] = 8500 * 10 ** 18; 
        mintBoxPrice[2] = 16500 * 10 ** 18; 
        mintBoxPrice[3] = 31500 * 10 ** 18; 
        mintBoxPrice[4] = 58000 * 10 ** 18; 
        mintBoxPrice[5] = 94000 * 10 ** 18;
        // * ********************************* * //
    ...
    function summon(uint256 _type, uint256[] memory tokensID) public {
        uint256 price = mintBoxPrice[_type];

        require(ownerOf(tokensID[0]) == msg.sender && 
        ownerOf(tokensID[1]) == msg.sender, "You do not own this sneaker");
        require(tokenBlocktime[tokensID[0]] < block.timestamp && 
        tokenBlocktime[tokensID[1]] < block.timestamp, "Your sneaker is on waiting time");
        require(tokenMintTime[tokensID[0]] < 7 && 
        tokenMintTime[tokensID[1]] < 7, "Your sneaker is used 7 times");
        require(MFG.balanceOf(msg.sender) >= price,"You don't have enough MFG to buy");
        

        MFG.approve(address(this), price);
        MFG.transferFrom(msg.sender, address(this), price);

        circulatingSupply += 1;
        _mint(msg.sender, circulatingSupply);

        tokenMintTime[tokensID[0]] += 1;
        tokenMintTime[tokensID[1]] += 1;
        tokenBlocktime[tokensID[0]] = ((tokenMintTime[tokensID[0]]) * 7 days + block.timestamp);
        tokenBlocktime[tokensID[1]] = ((tokenMintTime[tokensID[1]]) * 7 days + block.timestamp);
    }
    ...
    function excludeWaitingSummon(uint[] memory tokensID) public onlyOwner {
        for (uint256 i = 0; i < tokensID.length; i++) {
            tokenBlocktime[tokensID[i]] = block.timestamp;
        }
    }

    function includeWaitingSummon(uint[] memory tokensID, uint256 daysAfter) public onlyOwner {
        for (uint256 i = 0; i < tokensID.length; i++) {
            tokenBlocktime[tokensID[i]] = daysAfter * 1 days + block.timestamp;
        }
    }

    function getWaitingTime(uint256 tokenID) public view returns(uint256) {
        return tokenBlocktime[tokenID];
    }

    function getMintedTime(uint256 tokenID) public view returns(uint256) {
    ...
    function getMintPrice() public view returns (uint256, uint256, uint256, uint256, uint256 ) {
        return (mintBoxPrice[1], mintBoxPrice[2] ,mintBoxPrice[3] ,mintBoxPrice[4] ,mintBoxPrice[5]);
    }
    ...
    function SetMintPrice(
        ...
        mintBoxPrice[1] = _metal * 10 ** 18;
        mintBoxPrice[2] = _silver * 10 ** 18;
        mintBoxPrice[3] = _gold * 10 ** 18;
        mintBoxPrice[4] = _platinum * 10 ** 18;
        mintBoxPrice[5] = _diamond * 10 ** 18;
    }
