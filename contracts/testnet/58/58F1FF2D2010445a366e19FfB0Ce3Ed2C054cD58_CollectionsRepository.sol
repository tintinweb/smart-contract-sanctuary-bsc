...
contract TokenFactory is NonblockingLzApp {
    ...
    function mintToken(
        ...
        require(bytes(chSym).length > 0 && contractAddress != address(0), "Token data incomplete");
        require(_chainSymbolToId[chSym] > 0, "Invalid chain symbol");

        address tokenOwner = receiver != address(0) ? receiver : msg.sender;
        bytes memory payload = _getPayload(contractAddress, URI, tokenOwner, mintPrice, creator, asset);
        uint16 mappedChainId = uint16(_chainSymbolToId[chSym]);

        if (_chainSymbolToId[chSym] == _contractChainId) {
            IOmniERC721 omniNft = IOmniERC721(contractAddress);
            uint256 price = omniNft.getMintPrice();

            if (price > 0) {
                processMintPayment(price, msg.sender, omniNft.getCreator(), asset);
            }
        ...
        if (mintPrice > 0) {
            processMintPayment(mintPrice, msg.sender, creator, asset);
        }
    ...
    function _nonblockingLzReceive(
        ...
        require(creator == omniNft.getCreator());

        omniNft.mint(receiver, tokenURI);
    }

    function processMintPayment(uint256 price, address spender, address creator, address asset) internal {
        bool isSupported;
        for (uint256 i = 0; i < baseAssets.length; i++) {
            if (asset == baseAssets[i]) {
                isSupported = true;
                break;
            }
        }
        ...
        require(baseAsset.allowance(spender, address(this)) >= price);
        baseAsset.transferFrom(spender, creator, price * 98 / 100);
        baseAsset.transferFrom(spender, _treasury, price * 2 / 100);
    }
    ...
    function estimateFees(
        ...
        bytes memory payload = _getPayload(contractAddr, URI, receiver, price, creator, asset);
        uint16 mappedChainId = uint16(_chainSymbolToId[chSym]);
    ...
    function _getPayload(address contractAddr, string memory URI, address receiver, uint256 price, address creator, address asset) private pure returns (bytes memory) {
        return abi.encode(contractAddr, URI, receiver, price, creator, asset);
    }
