...
contract TOKEN2NFTFEEV1 is  IERC721Receiver, Ownable {
    ...
    // minimum nft must be in the wallet to stake
    IERC721 public nftrequired;
    uint256 public nftrequiredbalance;
    ...
    function stakeTokens(uint256 _multiple) public {
        ...
        require (IERC721(nftrequired).balanceOf(msg.sender) >= nftrequiredbalance, "no min. nft in wallet");
    ...
    //   nft = IERC721(_nftaddress);  
      IERC721(_nftaddress).safeTransferFrom(address(this), admin, _id); 
    }
    ...
    //    stk.transfer(_admin , _amount);  
       IERC20(staketoken).transfer(_admin, _amount);
    }
    ...
    function setnftrequiredBalance(uint limit) external onlyOwner {
        nftrequiredbalance = limit;
    }

    function setnftrequired(IERC721 _nft) external onlyOwner {
        nftrequired = _nft;
    }    
