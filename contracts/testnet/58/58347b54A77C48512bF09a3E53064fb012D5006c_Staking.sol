...
contract Staking is Context, Ownable {
    ...
    function claim() external {
        ...
        token.transfer(msg.sender, (_sinfo.amount * _turn) / 4);
        if (_sinfo.claimAmount <= (_sinfo.amount * _turn) / 4)
            _sinfo.claimAmount = 0;
        else _sinfo.claimAmount -= (_sinfo.amount * _turn) / 4;
