...
contract StakeUIHelper is StakeUIHelperI {
    ...
    uint256 internal constant APY_PRECISION = 10000;
    /// @dev Value of the max percentage 100%
    ...
    /**
     * @dev get user's info about sanction's percent
     * @param cooldownSeconds General cooldown duration in seconds
     * @param cooldownStartTimestamp User's cooldown
     * @return Sanction's percent
     */
    function _getUserSanctions(uint256 cooldownSeconds, uint256 cooldownStartTimestamp)
        ...
        returns (uint256)
    {
        return MAX_PERCENTAGE.sub((block.timestamp.sub(cooldownStartTimestamp))
            .mul(MAX_PERCENTAGE).div(cooldownSeconds));
    }
}
