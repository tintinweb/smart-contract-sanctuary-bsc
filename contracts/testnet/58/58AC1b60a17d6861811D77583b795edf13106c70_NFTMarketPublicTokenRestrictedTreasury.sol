...
contract NFTMarketPublicTokenRestrictedTreasury is ReentrancyGuard, AccessControl, ERC721Holder {
    ...
    mapping(address => mapping(uint256 => address)) private _creators; // nftContract => tokenId => creator
    ...
    function setRoyalty(uint96 _royaly) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_royaly > MAX_FEE) revert HighFee({ fee: _royaly, maxFee: MAX_FEE });
        royalty = _royaly;
    }
    ...
    function listTradeItem(
        ...
        // if given nft has no creator then creator is the msg.sender
        if (_creators[nftContract][tokenId] == address(0)) _creators[nftContract][tokenId] = msg.sender;
    ...
    function buyTradeItem(uint256 tradeId) external payable nonReentrant {
        ...
        uint256 royaltyTransfer = _getRoyalty(tradeItem.price);
        uint256 sellerTransfer = tradeItem.price - (feeTransfer + royaltyTransfer);
        address royaltyOwner = _creators[tradeItem.nftContract][tradeItem.tokenId];

        if (isNative) {
            address[] memory fundOwners = new address[](2);
            fundOwners[0] = address(0);
            fundOwners[1] = royaltyOwner;

            uint256[] memory amounts = new uint256[](2);
            amounts[0] = feeTransfer;
            amounts[1] = royaltyTransfer;

            treasury.addFunds{ value: feeTransfer + royaltyTransfer }(fundOwners, address(0), amounts);

            payable(tradeItem.seller).transfer(sellerTransfer);
        } else {
            IERC20(tradeItem.tokenContract).safeTransferFrom(msg.sender, address(this), tradeItem.price);

            IERC20(tradeItem.tokenContract).safeTransfer(address(treasury), feeTransfer + royaltyTransfer);
            address[] memory fundOwners = new address[](2);
            fundOwners[0] = address(0);
            fundOwners[1] = royaltyOwner;
            ...
            treasury.addFunds(fundOwners, tradeItem.tokenContract, amounts);

            IERC20(tradeItem.tokenContract).safeTransfer(tradeItem.seller, sellerTransfer);
        }
    ...
    function bidAuctionItem(uint256 auctionId, uint256 bid) external payable nonReentrant {
        ...
        if (!isNative) IERC20(auctionItem.tokenContract).safeTransferFrom(msg.sender, address(this), bid);
    ...
    function finishAuction(uint256 auctionId) external nonReentrant {
        ...
        if (auctionItem.bidCount < auctionItem.parameters.minBidderCount) {
            ...
            uint256 royaltyTransfer = _getRoyalty(auctionItem.highestBid);
            uint256 sellerTransfer = auctionItem.highestBid - (feeTransfer + royaltyTransfer);
            address royaltyOwner = _creators[auctionItem.nftContract][auctionItem.tokenId];
            ...
            if (isNative) {
                ...
                payable(auctionItem.seller).transfer(sellerTransfer);
            } else {
                IERC20(auctionItem.tokenContract).safeTransfer(address(treasury), feeTransfer + royaltyTransfer);
                address[] memory fundOwners = new address[](2);
                fundOwners[0] = address(0);
                fundOwners[1] = royaltyOwner;
                ...
                treasury.addFunds(fundOwners, auctionItem.tokenContract, amounts);

                IERC20(auctionItem.tokenContract).safeTransfer(auctionItem.seller, sellerTransfer);
            }
    ...
    function fetchNftCreator(address nftContract, uint256 tokenId) external view returns (address creator) {
        creator = _creators[nftContract][tokenId];
    }
    ...
    function _getRoyalty(uint256 price) private view returns (uint256 calculatedRoyalty) {
        calculatedRoyalty = (price * royalty) / _FEE_DENOMINATOR;
    }
...
contract Treasury is AccessControl {
    ...
    mapping(address => mapping(address => uint256)) private _withdrawableAmount; // fundOwner => token => amount

    event Withdrawn(address indexed fundOwner, address indexed tokenContract, uint256 amount);
    ...
    function addFunds(
        address[] calldata fundOwners,
        address token,
        uint256[] calldata amounts
    ) external payable onlyRole(FUNDER_ROLE) {
        for (uint256 i = 0; i < fundOwners.length; i++) {
            address fundOwner = fundOwners[i];
            uint256 amount = amounts[i];
            _withdrawableAmount[fundOwner][token] += amount;
        }
    }

    function withdraw(address tokenContract) external {
        address fundOwner = msg.sender;
        if (hasRole(DEFAULT_ADMIN_ROLE, fundOwner)) fundOwner = address(0);

        uint256 amount = _withdrawableAmount[fundOwner][tokenContract];
        _withdrawableAmount[fundOwner][tokenContract] = 0;

        if (amount > 0) {
            if (tokenContract == address(0)) payable(msg.sender).transfer(amount);
            else IERC20(tokenContract).safeTransfer(msg.sender, amount);

            emit Withdrawn(msg.sender, tokenContract, amount);
        }
    }

    function getWithdrawableAmount(address fundOwner, address tokenContract) external view returns (uint256 amount) {
        amount = _withdrawableAmount[fundOwner][tokenContract];
    }
}
