...
contract GeneralNFTRewardUpgradeable is IPool,GovernanceUpgradeable, IMigratable {
    ...
    function __GeneralNFTRewardUpgradeable_init_unchained(
        ...
        _gegoToken = INFTSignatureUpgradeable(spynNftToken);
    ...
    function earnedInternal(address account) private view returns (uint256) {
        ...
        return reward.div(1e12).add(_rewards[account]);
    }
    ...
    */
    ...
    function getStakeInfo( uint256 gegoId ) public view returns (
        uint256 stakeRate, uint256 degoAmount, uint256 realAmount, uint256 expiringTime
    ){
        ...
        checkStart
    {
        ...
        (uint256 stakeRate, uint256 degoAmount, uint256 realAmount, uint256 expiringTime) = getStakeInfo(gegoId);
        require(expiringTime > block.timestamp, "The NFT card is expired");

        takeDepositFee(gegoId, realAmount);
        ...
        if(_nextHarvestUntil[msg.sender] == 0){
            _nextHarvestUntil[msg.sender] = block.timestamp.add(_harvestInterval);
        }
        ...
        checkStart
    {
        ...
        if(stakingDegoAmount > 0 && block.timestamp < expiringTime){
            uint256 stakeWeight = stakeRate.mul(stakingDegoAmount).div(_fixRateBase);
            _degoBalances[msg.sender] = _degoBalances[msg.sender].add(stakingDegoAmount);
    ...
    function takeDepositFee(uint256 gegoId, uint256 degoAmount) internal {
        ...
        // Reward 50%
        uint256 feeTaken = depositFee.add(referralFeeTaken).add(burnFee);
        require (feeTaken < degoAmount, "fee is greather than dego amount");
        _gegoFactory.takeFee(gegoId, degoAmount.sub(feeTaken), address(this), false);
    }
    ...
    // ((APY + 1) ^ (1 / timeCompounded) - 1) / (365 * 86400 / timesCompounded) * 1e12 = rewardRate
