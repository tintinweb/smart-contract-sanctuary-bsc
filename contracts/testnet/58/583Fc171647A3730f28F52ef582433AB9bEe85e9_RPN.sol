...
contract RPN {
    ...
    function claimRoyalty() public {
        ...
        uint256 calculatedRoyalty = calcRoyalty(msg.sender); 
        usdt.transfer(msg.sender, calculatedRoyalty);
        royaltyDebt += calculatedRoyalty;
    ...
    function calcRoyalty(address _user) public view returns(uint256) {
        ...
        uint256 _royalty;
        if (user.level == 6) {
            uint256 royaltyMinusDebt = royaltyPool.sub(royaltyDebt);
            _royalty = royaltyMinusDebt.div(level6Count);
        }
