...
contract DividendDistributor is IDividendDistributor {
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            GMT.approve(address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3), otherreward);
...
contract RunEarner is ERC20Detailed, Ownable {
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(gmtWbnbFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(gmtWbnbFee)
            ),
            gas: 30000
        }("");

        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(gmtWbnbFee).div(treasuryFee.add(gmtWbnbFee))
        } () {} catch {}        
    }
