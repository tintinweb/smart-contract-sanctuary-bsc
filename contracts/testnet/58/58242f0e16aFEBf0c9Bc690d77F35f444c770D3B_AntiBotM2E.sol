...
enum TokenType {
    standard,
    antiBotStandard,
    M2ELiquidityGenerator,
    antiBotM2ELiquidityGenerator,
    baby,
    antiBotBaby,
    buybackBaby,
    antiBotBuybackBaby
...
// import "gist-e69874396d7b947b26842130dcd73a7/Pausable.sol";
...
contract AntiBotM2E is IERC20, Ownable, BaseToken, Pausable {
    ...
    constructor(
        ...
        uint16 taxFeeBps_,
        uint16 LiquidityPoolFeeBps_,
        uint16 vaultFeeBps_,
        address pinkAntiBot_
    ) payable {
        require(taxFeeBps_ >= 0 && taxFeeBps_ <= 10**4, "Invalid tax fee");
        require(
            LiquidityPoolFeeBps_ >= 0 && LiquidityPoolFeeBps_ <= 10**4,
            "Invalid liquidity fee"
        );
        require(
            vaultFeeBps_ >= 0 && vaultFeeBps_ <= 10**4,
            "Invalid vault fee"
        );
        if (vaultAddress_ == address(0)) {
            require(
                vaultFeeBps_ == 0,
                "Cant set both vault address to address 0 and vault percent more than 0"
            );
        }
        require(
            taxFeeBps_ + LiquidityPoolFeeBps_ + vaultFeeBps_ <= 10**4,
        ...
        emit TokenCreated(
            owner(),
            address(this),
            TokenType.antiBotM2ELiquidityGenerator,
            VERSION
        );
    ...
    */
    ...
    /**
    * @dev Added function to check if a wallet is blacklisted
    */
    function getBlacklistCheck(address _account) public onlyOwner view returns (bool) {
    ...
    function setLiquidityPoolFeePercent(uint256 LiquidityPoolFeeBps)
        external
        onlyOwner
    {
        require(
            LiquidityPoolFeeBps >= 0 && LiquidityPoolFeeBps <= 10**4,
            "Invalid bps"
        );
        _LiquidityPoolFee = LiquidityPoolFeeBps;
    }
        ...
        )
    {
        ...
        uint256 tLiquidity = calculateLiquidityPoolFee(tAmount);
        uint256 tvaultFee = calculatevaultFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(
            tvaultFee
        );
        return (tTransferAmount, tFee, tLiquidity, tvaultFee);
    }
    ...
    function _takevaultFee(uint256 tvault) private {
        if (tvault > 0) {
            uint256 currentRate = _getRate();
            uint256 rvault = tvault.mul(currentRate);
            _rOwned[_vaultAddress] = _rOwned[_vaultAddress].add(rvault);
            if (_isExcluded[_vaultAddress])
                _tOwned[_vaultAddress] = _tOwned[_vaultAddress].add(
                    tvault
                );
            emit Transfer(_msgSender(), _vaultAddress, tvault);
        }
    }
    ...
    function calculateLiquidityPoolFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_LiquidityPoolFee).div(10**4);
    }
    ...
    function calculatevaultFee(uint256 _amount)
        ...
        returns (uint256)
    {
        if (_vaultAddress == address(0)) return 0;
        return _amount.mul(_vaultFee).div(10**4);
    }

    function removeAllFee() private {
        if (_taxFee == 0 && _LiquidityPoolFee == 0 && _vaultFee == 0) return;
    ...
    function _transfer(
        ...
        require(!_includeToBlackList[from], "This address is blacklisted"); 
        require(!_includeToBlackList[to], "This address is blacklisted");
