...
contract SwapToken is Ownable{
    ...
    function swapExactETHForTokens(
        ...
        path[0] = uniswap.WETH();
        path[1] = tokenToSwap;
        uint256 ammountInWithTax = msg.value - (msg.value * (swapETHToTokensFee / 100));
        uniswap.swapExactETHForTokens{value: ammountInWithTax}(amountOutMin, path, msg.sender, block.timestamp + (1000 * 60 *60 * 100));
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        path[1] = tokenToSwap;
        uint256 ammountInWithTax = msg.value - (msg.value * (swapETHToTokensFee / 100));
        uniswap.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ammountInWithTax}(amountOutMin, path, msg.sender, block.timestamp + (1000 * 60 *60 * 100));
    }
