    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getRate(
        uint256 tradeId,
        address purchaseToken
    ) public view returns (uint256 rate, uint256 decimals) {
        decimals = 1 ether;
        if (trades[tradeId].askedAsset == purchaseToken) {
            rate = trades[tradeId].askedRate;
        } else if (purchaseToken == address(WETH)) {
            address[] memory path = new address[](2);
            path[0] = pancakeRouter.WETH();
            path[1] = trades[tradeId].askedAsset;
            uint[] memory amountsBNBBusd = IPancakeRouter(pancakeRouter)
                .getAmountsOut(decimals, path);
            rate = (amountsBNBBusd[1] * trades[tradeId].askedRate) / decimals;
        } else {
            rate = 0;
        }
    }
    ...
    function getCurrentTokenAmount(
        ...
        (uint256 rate, uint256 decimals) = getRate(tradeId, purchaseToken);
        return (partialAmount * rate) / decimals;
    }
