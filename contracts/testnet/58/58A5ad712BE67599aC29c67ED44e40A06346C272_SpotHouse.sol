    ...
    SpotHouseStorage
{
    ...
    function openMarketOrderWithQuote(
        ...
        if (_side == Side.BUY) {
            ...
            (sizeOutQuote, baseAmount) = _pairManager
                .openMarketWithQuoteAsset(_quoteAmount, true, _trader);
            require(
            ...
            (sizeOutQuote, baseAmount) = _pairManager
                .openMarketWithQuoteAsset(_quoteAmount, false, _trader);
            require(
                sizeOutQuote == _quoteAmount,
                Errors.VL_NOT_ENOUGH_LIQUIDITY
            );
    ...
    function claimFee(IPairManager _pairManager, uint256 feeBase ,uint256 feeQuote ) external onlyOwner {
        SpotFactoryStorage.Pair memory _pairAddress = _getQuoteAndBase(
            _pairManager
        );
    ...
    function _openBuyLimitOrder(
        ...
        (uint256 quoteAmountTransferred, uint256 feeQuote) = _deposit(
            _pairManager,
            _trader,
            Asset.Quote,
            quoteAmount
    ...
    function _openSellLimitOrder(
...

        {
            ...
            // with token has RFI we need deposit first
            // and get real balance transferred
            (baseAmountTransferred, ) = _deposit(
                _pairManager,
                _trader,
                Asset.Base,
                uint128(_quantity)
    ...
    function setAbc(uint256 b) public {
        a = b;
        emit abc(a);
    }
}
