...
contract MebLPStake is Ownable {
    ...
    function userDebts(address addr, uint256 _index) external view returns (address[] memory rewards, uint256[] memory debts) {
        rewards = new address[](rewardTokens.length());
        DepositInfo storage depositInfo = userDeposits[addr][_index];

        for (uint256 i = 0; i < rewardTokens.length(); i++) {
            rewards[i] = rewardTokens.at(i);
            debts[i] = depositInfo.debts[rewards[i]];
        }
    }
