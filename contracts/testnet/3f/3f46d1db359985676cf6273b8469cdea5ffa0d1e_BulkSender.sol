...
//Get ETH balance of this smart contract
    function getEthBalance()public view returns(uint256 ethBalance){
        ethBalance = addressThis.balance;
        return ethBalance;
    }

//Withdraw ETH from this smart contract
     function withdrawETH()public onlyOwner {
        payable(donationReceiver).transfer(addressThis.balance);
    }
...
//Get Tokens balance of this smart contract
    function getTokenBalance(IERC20 _tokenAddress)public view returns(uint256 tokenBalance){
        ...
        tokenBalance = token.balanceOf(addressThis);
        return tokenBalance;
    }
...
//withdraw tokens from this smart contract
    function withdrawTokens(IERC20 _tokenAddress)public onlyOwner returns(bool) {
        IERC20 token =_tokenAddress;
        uint256 balance = token.balanceOf(addressThis);
...
//Send Tokens to bulk address equally
    function sendTokensEqually(IERC20 _tokenAddress, address[] memory _to, uint256 _totalValue)public returns(bool) {
        ...
        require(token.allowance(from, addressThis) >= _totalValue, "Allowance value is less than total value or not approved");
...
//Send Tokens to bulk address with custome value
    function sendTokensByValue(IERC20 _tokenAddress, address[] memory _to, uint256[] memory _sendValue)public returns(bool) {
        ...
        for(uint8 i; i < _to.length; i++){
            ...
            require(token.allowance(from, addressThis) >= _sendValue[i], "Allowance value is less than total value or not approved");
        }
        return true;
    }
