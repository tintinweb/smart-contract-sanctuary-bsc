...
contract OMNICOIN is BEP20('OMNICOIN', 'OMNICOIN') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "Omni::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "Omni::delegateBySig: invalid nonce");
        require(now <= expiry, "Omni::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "Omni::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying Omnis (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "Omni::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterOmni is the master of Omni 
// He can make Omni and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Omni is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MasterOmni is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Omnis
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Omnis to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Omnis distribution occurs.
        uint256 accOmniPerShare; // Accumulated Omnis per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 totalLp;            // Total Token in Pool
    }

    // The Omni TOKEN!
    OMNICOIN public Omni;
    ...
    // Omni tokens created per block.
    uint256 public OmniPerBlock;
    // Bonus muliplier for early Omni makers.
    ...
    // The block number when Omni mining starts.
    uint256 public startBlock;
    // Total Omni in Omni Pools (can be multiple pools)
    uint256 public totalOmniInPools = 0;
    ...
    //actual Omni left in MasterChef can be used in rewards, must excluding all in Omni pools
    //this function is for safety check 
    function remainRewards() public view returns (uint256) {
        return Omni.balanceOf(address(this)).sub(totalOmniInPools);
    }

    //All Omnis that are not in pools or masterchef reward stack
    function getCirculatingSupply() external view returns(uint256) {
        uint256 tSupply = Omni.totalSupply();
        uint256 OmniBalance = Omni.balanceOf(address(this));

        return tSupply.sub(OmniBalance);    
    }
    ...
    // Update the given pool's Omni allocation point. Can only be called by the owner.
    ...
    // View function to see pending Omnis on frontend.
    function pendingOmni(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 OmniReward = multiplier.mul(OmniPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            uint256 totalRewardFees = OmniReward.mul(1500).div(10000);
            uint256 OmniRewardUsers = OmniReward.sub(totalRewardFees);

            uint256 totalminted = Omni.totalMinted();
             
        if(totalminted >= 159000000000000000000000000){
         
            accOmniPerShare = accOmniPerShare;

            }else{
                accOmniPerShare = accOmniPerShare.add(OmniRewardUsers.mul(1e12).div(lpSupply));
            }
        ...
        // View function to see all locked Omnis on frontend.
        function lockedOmni() external view returns (uint256) {
            return totalOmniInPools;
        }
    ...
    function updatePool(uint256 _pid) public validatePool(_pid) {
        ...
        if(totalminted >= 159000000000000000000000000){
            ...
            if(currentDevFee > 0 || currentStaffFee > 0 || currentMarketingFee > 0){
             ...
             safeOmniTransfer(devaddr,currentDevFee);
             safeOmniTransfer(staffAddress,currentStaffFee);
             safeOmniTransfer(marktAddress,currentMarketingFee);

            }
  
            pool.accOmniPerShare = pool.accOmniPerShare;
            ...
            currentStaffFee = 0;
             ...
             Omni.mint(address(this),OmniReward);
            ...
            //1% dev fee
            currentDevFee = currentDevFee.add(OmniReward.mul(blockDevFee).div(10000));

            //4% staff fee
            currentStaffFee = currentStaffFee.add(OmniReward.mul(blockStaffFee).div(10000));
            
            //10% marketing fee
            currentMarketingFee = currentMarketingFee.add(OmniReward.div(10));
 
            pool.accOmniPerShare = pool.accOmniPerShare.add(OmniRewardUsers.mul(1e12).div(pool.totalLp));
            pool.lastRewardBlock = block.number;

   }         
        

    // Deposit LP tokens to MasterOmni for Omni allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
       // require(block.number >= startBlock, "MasterChef:: Can not deposit before farm start");
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accOmniPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                uint256 currentRewardBalance = remainRewards();
                if(currentRewardBalance > 0) {
                    if(pending > currentRewardBalance) {
                        safeOmniTransfer(msg.sender, currentRewardBalance);
                    } else {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if (address(pool.lpToken) == address(Omni)) {
                    totalOmniInPools = totalOmniInPools.add(_amount).sub(depositFee);   
                } 
                ...
                if (address(pool.lpToken) == address(Omni)) {
                    totalOmniInPools = totalOmniInPools.add(_amount);
                }
    ...
    // Withdraw LP tokens from MasterOmni.
    function withdraw(uint256 _pid, uint256 _amount) public validatePool(_pid) nonReentrant {
        ...
        if(_amount > 0) {
             ...
             if (address(pool.lpToken) == address(Omni)) {
                 ...
                 uint256 OmniBal = Omni.balanceOf(address(this));

                 require(_amount <= OmniBal,'withdraw: not good');    

                if(_amount >= totalOmniInPools){
                    totalOmniInPools = 0;
                }else{
                    require(totalOmniInPools >= _amount,'amount bigger than pool wut?');
                    totalOmniInPools = totalOmniInPools.sub(_amount);
                }  
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(Omni)) {
            ...
            require(amount <= OmniBal,'withdraw: not good'); 

            if(amount >= totalOmniInPools){
                totalOmniInPools = 0;
            }else{
                require(totalOmniInPools >= amount,'amount bigger than pool wut?');
                totalOmniInPools = totalOmniInPools.sub(amount);
            }  
    ...
    returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accOmniPerShare, uint16 depositFeeBP, uint256 totalLp) {
        return (address(poolInfo[_pid].lpToken),
             poolInfo[_pid].allocPoint,
             poolInfo[_pid].lastRewardBlock,
             poolInfo[_pid].accOmniPerShare,
             poolInfo[_pid].depositFeeBP,
             poolInfo[_pid].totalLp);
    }

    // Safe Omni transfer function, just in case if rounding error causes pool to not have enough Omnis.
    function safeOmniTransfer(address _to, uint256 _amount) internal {
        if(Omni.balanceOf(address(this)) > totalOmniInPools){
            //OmniBal = total Omni in MasterChef - total Omni in Omni pools, this will make sure that MasterChef never transfer rewards from deposited Omni pools
            uint256 OmniBal = Omni.balanceOf(address(this)).sub(totalOmniInPools);
            if (_amount >= OmniBal) {
                Omni.transfer(_to, OmniBal);
            } else if (_amount > 0) {
                Omni.transfer(_to, _amount);
            }
        }
    }
    ...
    function updateEmissionRate(uint256 _OmniPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, OmniPerBlock, _OmniPerBlock);
        OmniPerBlock = _OmniPerBlock;
    }
