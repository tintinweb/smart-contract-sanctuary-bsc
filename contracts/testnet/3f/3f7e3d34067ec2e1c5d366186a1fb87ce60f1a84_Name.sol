...
contract Name is IERC20, Ownable {
    ...
    function checkATH(address from, address to, uint256 amount) public {
        ...
        if (tmpAddr == address(this)){
            ...
            if (!_isExcludedFromFees[from] && to == uniswapV2Pair){
                ...
                emit LogReserve(res0, res1);
                result = res1 * 1e18 / res0;
                if (result < lastATH / 100 * (100 - priceImpactLimit)){
    ...
    function swapAndDistribute() private {
        if (address(this).balance > 0){
            (bool success,) = address(projectWalletAddress).call{value: address(this).balance}("");            
        }
        ...
        (bool success,) = address(projectWalletAddress).call{value: ethBalance}("");   
    }    
