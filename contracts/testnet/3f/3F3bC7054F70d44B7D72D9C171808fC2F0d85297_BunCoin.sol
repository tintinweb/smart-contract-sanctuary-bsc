...
contract BunCoin is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        for (uint i = 0; i < _percentagePerEntities.length; i++) {
                (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount, _percentagePerEntities[i] );
                _bunCoinOwned[_percentagePerEntities[i].addr] -= tTransferAmount;
                emit ReflectionSent(_percentagePerEntities[i].addr, tTransferAmount );
