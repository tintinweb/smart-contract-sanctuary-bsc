...
contract Treasury is Initializable, AccessControlUpgradeable {
    ...
    function claimableFee(address _token) public view returns (uint256) {
    ...
    function withdrawFee(address _token) external {
        require(hasRole(CONTROLLER_ROLE, _msgSender()), "Treasury::withdrawFee: caller is not the controller");
        require(_token != address(0), "Treasury::withdrawFee: zero address");
        uint256 feeReserve = claimableFee(_token);
        require(feeReserve > 0, "Treasury::withdrawFee: required fee reserve > 0");
        pool.withdrawFee(_token, address(this));

        emit FeeWithdrawn(_token);
    }

    function transfer(address _token, address _receiver, uint256 _amount) external {
        address spender = _msgSender();
        require(hasRole(SPENDER_ROLE, _msgSender()), "Treasury::transfer: caller is not the spender");
        require(_amount <= IERC20(_token).balanceOf(address(this)), "Treasury::transfer: insufficient balance");
    ...
    function transferETH(address _receiver, uint256 _amount) external {
        address spender = _msgSender();
        require(hasRole(SPENDER_ROLE, _msgSender()), "Treasury::transferETH: caller is not the spender");
        require(_amount <= address(this).balance, "Treasury::transferETH: insufficient balance");
    ...
    function _safeTransferETH(address to, uint256 amount) internal {
        ...
        require(success, "Treasury::_safeTransferETH: ETH transfer failed");
    }

    /* ========== EVENTS ========== */
    event FeeWithdrawn(address indexed _token);
