...
contract PYESwap_Base is IPYE, Context, Ownable {
    ...
    function buyAndLiquify(uint256 balanceWBNB) private lockTheSwap {
        
        uint256 amountToSwap = balanceWBNB.div(2);
        uint256 amountWBNBtoAdd = balanceWBNB.sub(amountToSwap);

        uint256 initialTokenBalance = address(this).balance;

    //    buyTokens(amountToSwap);

        uint256 amountTokenToAdd = address(this).balance.sub(initialTokenBalance);

        addLiquify(amountTokenToAdd, amountWBNBtoAdd);
    ...
    function addLiquify(uint256 amountTokenToAdd, uint256 amountWBNBtoAdd) internal {
