...
contract Dolphin is ERC20, Ownable {
    ...
    string private symbol_ = "DLP";
    uint256 private totalSupply_ = 1000000 * (10**9);
    uint256 private swapTokensAtAmount = totalSupply_.mul(2).div(10**6);
    ...
    uint256 private transferDivtokenFee = 5;
    uint256 private transferMarketingFee = 6;
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                finalAmount = finalAmount.sub(_LiquidityFee).sub(_MarketingFee).sub(_DeadFee).sub(_InviterFee);
                ...
                finalAmount = finalAmount.sub(_LiquidityFee).sub(_DivtokenFee).sub(_MarketingFee).sub(_InviterFee);
            }else{
