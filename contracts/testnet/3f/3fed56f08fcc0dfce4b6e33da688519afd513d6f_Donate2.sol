...
contract Donate2 is Ownable {
    ...
    UserRewards[] private userRewards;
    ...
    function buy(uint256 purchaseIndex, uint256 code) public {
        ...
        UserRewards memory map = UserRewards(msg.sender, _maxAmount, block.timestamp, 0, 0, 0);
        userRewards.push(map);
        rewardsMap[msg.sender] = map;
    ...
    function mining(uint256 price, uint256 ts) public {        
        ...
        for (uint256 i = 0; i < userRewards.length; i++) {
            UserRewards memory ur = userRewards[i];
            uint256 ds = DaysDiff(ur.timestamp, ts);
            if (ds > 60) {
                // remove
                ...
                if (dao != 0) {
                    userRewards[i].reward = userRewards[i].reward.add(dao);
                    userRewards[i].count = userRewards[i].count.add(1);

                    rewardsMap[ur.account].reward = userRewards[i].reward;
                    rewardsMap[ur.account].count = userRewards[i].count;
                    emit Mining(
                        ur.account,
        ...
        for (uint256 i = 0; i < removeRewardsIndex.length; i++) {
            for (uint j = removeRewardsIndex[i]; j < userRewards.length - 1; j++) {
                    userRewards[j] = userRewards[j + 1];
            }
            address account = userRewards[removeRewardsIndex[i]].account;
            emit MiningOut(
                account,
