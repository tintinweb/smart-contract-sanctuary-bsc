...
contract MetaPointV40 is Ownable {
    ...
    mapping(uint40 => uint40) public dailyCnts;
    ...
    mapping(uint40 => bool) private grwCreatorToday;
    address public opr;
    ...
    function up() public {
        ...
        lpdsBalance += (expend * allot[3]) / 100;
    ...
    mapping(address => mapping(uint40 => uint40)) public userDailyCnts;

    function today() public view returns (uint40) {
        return uint40(block.timestamp - (block.timestamp % (24 * 60 * 60)));
    }
    ...
    address metapoint = 0x087d1A4a8997025e52B9a4575471191A9caA9d5d;
    ...
    function mapping_accounts(address[] calldata addrs, uint40[] calldata rootSns) external {
        for (uint40 i = 0; i < addrs.length; i++) {
            AccountSrc memory accountSrc = IMetaPoint(metapoint).accounts(addrs[i]);
            accounts[addrs[i]] = Account({
                srd: accountSrc.srd, //
                srw: accountSrc.srw,
            ...
            (address[] memory addrs_, ) = IMetaPoint(metapoint).followList(addrs[i]);
            follows[addrs[i]] = addrs_;
        }
    }
}
