...
contract Three is Ownable{
	...
	uint256  public INVEST_MIN_AMOUNT = 10 ether;  // 最小质押USDT数量
    uint256  public WITH_MIN_AMOUNT = 10 ether;  // 最小质押USDT数量
	uint256  public BASE_PERCENT = 33;  // 3.3% 日产3.3%
	uint256  public BASE_FEE = 20; //2%提币手续费
	uint256[] public REFERRAL_PERCENTS = [200, 100];  //二代奖励
	...
	function invest(address referrer,uint256 investAmount) public returns (bool) {
        ...
        if (user.referrer != address(0)) {
            ...
            if(isActive(upline)){
                ...
                //没有质押过就只给上级 10% 
                uint256 amount = investAmount.mul(100).div(PERCENTS_DIVIDER);
                users[upline].bonus = users[upline].bonus.add(amount);
	...
	//用户信息
	function getUserInfo(address userAddress) public view returns(address,uint256,uint256,uint256,uint256,uint256){
		...
		return (user.referrer,user.bonus,team,TotalDeposits,TotalWithdrawn,Dividends);
	}
