...
contract EAGLE is ERC20, Ownable {
    ...
    function swapTokensForUsdt(uint256 tokenAmount,address account)
        public
    {
    ...
    function _transfer(
        ...
        uint256 trsAmount = amount;
            if (!whitelist[from] && to == address(pancakeSwapV2Pair) && !(from == address(this)&&to == address(pancakeSwapV2Pair))) {
                // 2.8% is Service Charge
                uint256 fee = (amount * 24) / 1000;
                uint256 backFlowFee = (amount * 4) / 1000;
                uint256 MmrketingWalletFee = backFlowFee;
                uint256 NFTRewrd = (amount * 20) / 1000;
                super._transfer(from, address(this), fee);
                super._transfer(from, MarketingWallet, fee);
                swapUsdtAndLiquify(backFlowFee);
                swapTokensForUsdt(NFTRewrd,address(this));
                uint256 usdtBalance = usdt.balanceOf(address(this));
                if (usdtBalance >= 500 * 10**18) {
                    usdtDistribute(usdtBalance);
                }
                trsAmount = amount - fee - MmrketingWalletFee;
            }
