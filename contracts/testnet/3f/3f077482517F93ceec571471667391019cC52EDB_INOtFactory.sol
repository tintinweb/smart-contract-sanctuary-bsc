...
interface IUserLevel {
    ...
    function setBonus(address _contract, uint[] memory _bonus) external;
...
import  "./InitialNFTOffering.sol";

contract INOtFactory {
    function createINO(
        uint _projectId,
        address _operator,
        address _userLevel
        ...
        _initialize(_projectId, _operator, _userLevel, _projectAddress);
        return _projectAddress;

    }

    function _calculateAddress(uint _projectId, address _operator) internal returns(address) {
        bytes memory bytecode = type(InitialNFTOffering).creationCode;
    ...
    function _initialize(
        ...
        bool check = InitialNFTOffering(_projectAddress).initialize(_projectId, _operator, _userLevel);
...
contract InitialNFTOffering is Initializable, ERC721Holder {
    ...
    // type => time lock
    mapping(uint256 => uint256) public lockingTime;

    // address token => id nft=> unlock time
    mapping(address => mapping(uint => uint)) public unlockTime;
    ...
    modifier checkSigner(bytes memory _signature) {
        bytes32 _hash = keccak256(abi.encodePacked(projectId, msg.sender, address(this))).toEthSignedMessageHash();
    ...
    event Setting(uint indexed projectId, bool indexed update);
    event RegisterINO(address indexed user, uint indexed projectId);
    ...
    function setting(
        ...
        require(_times.length == 3 && _times[0] > block.timestamp && _times[0] < _times[1], "InitialNFTOffering: time buying incorrect");
        require( _times[0] > _times[2] && _times[2] >= block.timestamp, "InitialNFTOffering: time register incorrect");
        if(_update) {
            require(block.timestamp < startTime, "Can not setting after starting");
        }
        if(_isLock) {
            require(_lockTime.length == _types, "InitialNFTOffering: length not equal _lockTime");
            for(uint i = 1; i <= _types; i++) {
                lockingTime[i] = _lockTime[i-1];
            }
        }
        ...
        if(!_update) {
            startTime = _times[0];
            endTime = _times[1];
            registerTime = _times[2];
        }
        emit Setting(projectId, _update);
        return true;
    }

    function registerINO(bytes memory _signature) external checkSigner(_signature) {
        ...
        emit RegisterINO(msg.sender, projectId);
    }


    function depositNFT(uint _typeId, uint[] memory _ids) external onlyInitializing onlyOperator checkTypeId(_typeId){
    ...
    // withdraw when lock
    function redeem(uint _typeId, uint[] memory _ids) external checkTypeId(_typeId) {
        address nft = typeAddress[_typeId];
        for(uint i = 0; i < _ids.length; i++) {
            uint _id = _ids[i];
            require(nftTypes[msg.sender][_typeId].contains(_id), "InitialNFTOffering: user not owner of token id");
            require(unlockTime[nft][_id] <= block.timestamp, "InitialNFTOffering: token id is locking");

            //remove token in list
            nftTypes[msg.sender][_typeId].remove(_id);
    ...
    //withdraw admin
    function withdrawAdmin(address _to) external onlyOperator afterSale {
        require(_to != address(0), "InitialNFTOffering: zero address" );
        currency.safeTransfer(_to, currency.balanceOf(address(this)));
    }

    function withdrawNFT(address _to, uint _typeId) external onlyOperator afterSale checkTypeId(_typeId) {
        require(_to != address(0), "InitialNFTOffering: zero address" );
        ...
        for(uint i = length; i > 0;) {
            ...
            nft.safeTransferFrom(address(this), _to, _id);
            typeNFTId[_typeId].remove(_id);
        }
    }

    function _safeLock(uint _typeId) internal {
        //get address of nft
        ...
        //add lock
        unlockTime[address(_nft)][id] = block.timestamp + lockingTime[_typeId];
    }
    ...
    function getAllocations(uint _typeId, address _user) internal view returns(uint) {
        ...
        if(userLevel != address(0)) {
            (uint _numerator, uint _denominator) = IUserLevel(userLevel).getBonus(_user, address(this));
            currentAllocation = currentAllocation * _numerator / _denominator;
        }
        return currentAllocation;
    }

    function verifySignature(bytes memory _signature, address _sampleAddress) external view returns (bool) {
        bytes32 _hash = keccak256(abi.encodePacked(projectId, _sampleAddress, address(this))).toEthSignedMessageHash();
