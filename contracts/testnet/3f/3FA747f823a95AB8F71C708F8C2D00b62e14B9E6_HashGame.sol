...
contract HashGame is ReentrancyGuard {
    ...
    function registerHashGame(uint256 _gid, bool _premium, bool _jackpot) external nonReentrant {
        ...
        // If user switch mode
        if(_userMvccGameInfo.gameMode != 0 && _userMvccGameInfo.gameMode != _gid) {
    ...
    event PurchasePremium(uint256 gid);
    ...
    event SetMarketPoolReceiver(address newAddress);
    event SetInsurancePoolReceiver(address newAddress);
    event SetSuperNodeReceiver(address newAddress);
    event SetLiquidityPoolReceiver(address newAddress);
    event SetMvccBonusReceiver(address newAddress);
    event SetPremiumAllocation(
        uint256 marketPoolPercentage, 
        uint256 insurancePoolPercentage,
        uint256 superNodePercentage,
        uint256 liquidityPoolPercentage,
        uint256 mvccBonusPercentage
    );
