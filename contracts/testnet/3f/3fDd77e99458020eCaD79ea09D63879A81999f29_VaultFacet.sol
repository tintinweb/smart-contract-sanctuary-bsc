...
contract VaultFacet is IVault {
    ...
    function addToken(Token memory token, uint16[] memory weights) external override {
        LibAccessControlEnumerable.checkRole(LibVault.OPERATOR_ROLE);
        require(token.tokenAddress != address(0), "VaultFacet: Token address can't be 0 address");
        LibVault.addToken(token, weights);
    }
...
library LibVault {
    ...
    function addToken(IVault.Token memory _token, uint16[] memory weights) internal {
        VaultStorage storage vs = vaultStorage();
        require(vs.tokens[_token.tokenAddress].weight == 0, "LibVault: Can't add token that already exists");
        require(_token.weight == weights[weights.length - 1], "");
        if (_token.dynamicFee && _token.taxBasisPoints <= _token.feeBasisPoints) {
            revert("");
        }

        AvailableToken storage at = vs.tokens[_token.tokenAddress];
        at.tokenAddress = _token.tokenAddress;
        at.tokenAddressPosition = uint32(vs.tokenAddresses.length);
        at.feeBasisPoints = _token.feeBasisPoints;
        at.taxBasisPoints = _token.taxBasisPoints;
        at.decimals = IERC20Metadata(_token.tokenAddress).decimals();
        at.stable = _token.stable;
        at.dynamicFee = _token.dynamicFee;

        vs.tokenAddresses.push(_token.tokenAddress);
        emit AddToken(at.tokenAddress, _token.weight, at.feeBasisPoints, at.taxBasisPoints, at.stable, at.dynamicFee);
...
interface IVault {
    ...
    function addToken(Token memory token, uint16[] memory weights) external;
