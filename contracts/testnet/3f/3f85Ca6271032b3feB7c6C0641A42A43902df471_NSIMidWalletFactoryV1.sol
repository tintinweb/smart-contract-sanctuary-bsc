...
contract NSIMidWalletV1 is AccessControl {
    ...
    function clearStuckToken(address tokenAddress, uint256 tokens) external onlyOwner returns (bool success) {
        ...
        require(disabled[tokenAddress] == false, "This token is disableb by the owner. Enable it again before withdrawing from the mid wallet.");
    ...
    function Trade_Token_Supporting_Fee(address _tokenIn, address _tokenOut, uint256 _amountIn, address[] memory path, uint256 _amountOutMin) public onlyAdmin{
        ...
        uint deadline = block.timestamp + 200;
        uint256[] memory amounts = IUniswapV2Router02(uniswapV2Router).getAmountsOut(_amountIn, path);
        uint256 amountOut = amounts[amounts.length - 1];
        require(amountOut >= _amountOutMin, "Received amount is less than the expected minimum amount");
        IUniswapV2Router02(uniswapV2Router).swapExactTokensForTokensSupportingFeeOnTransferTokens(_amountIn, amountOut, path, _to, deadline);
        paused == false;
        emit Resultant_Token(owner, amountOut, _tokenOut);
    }
...
// File: midwalletfactory.sol


pragma solidity ^0.8.0;


contract NSIMidWalletFactoryV1 is AccessControl{
    using Address for address;
    ...
    function createFor(address owner) public onlyAdmin returns(address created){
        ...
        address ret = address(new NSIMidWalletV1(
            owner
        ));
        owners[owner] = ret;
        return ret;
