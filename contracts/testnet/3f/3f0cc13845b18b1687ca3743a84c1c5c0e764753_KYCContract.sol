...
contract KYCContract is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /**
     * @dev all Merchants referrals count 
     * username=>referral count
     **/
    mapping(string=>uint256) allMerchantsReferralCount;
    //referral reward
    uint256 referralRewardValue;

    /**
     * @dev KYC Token Contracts
     * KYC Token for fees 
     **/
    IBDLT20 private BDLTToken;
    ...
    function initialize() initializer public {
        ...
        referralRewardValue=1*10**6; // 1 * decimals
        requestFees=1*10**6; // 1 * decimals
    }
    ...
    function getAccessAllowedByAdmin(
        string memory _fromMerchant,
        string memory _toUsername
    ) public view onlyOwner returns(bool){
        return accessAllowedByAdmin[_fromMerchant][_toUsername];
    }

    function getBDLTToken() public view onlyOwner returns(IBDLT20){
        return BDLTToken;
    }
    
    function setBDLTToken(IBDLT20 token) public onlyOwner{
        BDLTToken=token;
    }

    function getRequestFees() public view onlyOwner returns(uint256){
        return requestFees;
    }

    function setRequestFees(uint256 _fees) public onlyOwner{
        requestFees=_fees;
    }

    function getReferralRewardValue() public view onlyOwner returns(uint256){
        return referralRewardValue;
    }

    function setReferralRewardValue(uint256 _referralRewardValue) public onlyOwner{
        referralRewardValue=_referralRewardValue;
    }
    ...
    function updateHistoryData(
        ...
        string memory _toUsername
            ...
            for (uint i = accessHistory[_toUsername].length-1; i >= 0; i--) {
                // && accessHistory[_toUsername][i].toTimestamp==0
                if(compareStrings(accessHistory[_toUsername][i].fromUsername,_fromMerchant)){
                    index=i;
                    break;
                }
            }
    ...
    function payoutToken(address from,address to, uint256 amount) internal{
        // transfer token
        BDLTToken.transferFrom(from,to,amount);
    }
    
    function payoutToken(address to, uint256 amount) internal{
        // transfer token
        BDLTToken.transfer(to,amount);
    }
    ...
    function createCustomer(
        ...
        string memory _referralMerchant
        ) public{
        ...
        //add referral
        allMerchantsReferralCount[_referralMerchant]++;
    ...
    function merchantReferralRewardCount(
        string memory _username
        ) public view merchantExist(_username) validMerchants(_username) returns(uint256){
        
        return(allMerchantsReferralCount[_username]);
    }

    function claimMerchantReferralReward(
        string memory _username
        ) public merchantExist(_username) validMerchants(_username){
        // check for referral present
        require(allMerchantsReferralCount[_username]>0,"Do not have any referral");
        // transfer referral rewards
        payoutToken(allMerchants[_username].userWalletAddress,(referralRewardValue*allMerchantsReferralCount[_username]));
        allMerchantsReferralCount[_username]=0;
    }                           
    ...
    function sendAccessRequest(
        ...
        // Check token allowance for fees payment
        uint256 allowance=BDLTToken.allowance(msg.sender,address(this));
        require(allowance>=requestFees,"check token allowance");
        //pay fees
        payoutToken(msg.sender,address(this),requestFees);            
        

        // added request in merchant request list
        allRequestsData[_fromMerchant][_toUsername].timestamp=block.timestamp;
        allRequestsData[_fromMerchant][_toUsername].openTime=0;
        allRequestsData[_fromMerchant][_toUsername].closeTime=0;
    ...
    function rejectRequest(
        ...
        //revert paid fees
        payoutToken(allMerchants[_fromMerchant].userWalletAddress,requestFees);            
