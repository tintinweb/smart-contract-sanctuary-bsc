...
contract VIRALSPIN is IBEP20, Ownable {
    
    using SafeMath for uint256;

    string constant _name = "Viral Spin";
    ...
    function changeFees(uint256 newLiqFee, uint256 newDevFee, uint256 newMarketingFee, uint256 newBurnFee, uint256 newExtraSellFee) external onlyOwner {
        require((newLiqFee + newDevFee + newMarketingFee + newBurnFee) <= 10, "Taxes cannot be greater than 10%");
        require(newExtraSellFee <= 5, "Added sell tax cannot be greater than 10%");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isTxLimitExempt[sender] && !isTxLimitExempt[recipient]){
            ...
            if (recipient != pair) { require(amount <= _maxTxAmount,"Transfer amount exceeds the maxTxAmount"); }
        }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (burnFee > 0) {
            burnTokens = feeAmount.mul(burnFee).div(feeApplicable);
    ...
    function swapBack() internal lockTheSwap {
        ...
        if (amountBNBDev > 0) {
            (bool devSuccess, ) = payable(devWallet).call{
                value: amountBNBDev,
                gas: 30000
            }("");
            // only to supress warning msg
