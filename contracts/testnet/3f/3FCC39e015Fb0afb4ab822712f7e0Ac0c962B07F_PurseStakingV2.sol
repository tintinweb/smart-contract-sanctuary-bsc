...
contract PurseStakingV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    event purseTransfer(address indexed _from, address indexed _to, uint256 _value);
    ...
    function enter(uint256 purseAmount) external returns (bool success) {
        ...
        emit purseTransfer(msg.sender, address(this), purseAmount);
    ...
    function leave(uint256 xPurseAmount) external returns (bool success){  
        ...
        emit purseTransfer(address(this), msg.sender, purseAmount);
        safePurseTransfer(msg.sender, purseAmount);
        return true;
    }

    function safePurseTransfer(address recipient, uint256 amount) internal {
        uint256 purseBal = purseToken.balanceOf(address(this));
        if (amount > purseBal) {
            purseToken.transfer(recipient, purseBal);
        } else {
