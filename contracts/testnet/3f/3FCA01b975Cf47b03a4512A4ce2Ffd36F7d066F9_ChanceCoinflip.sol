...
contract ChanceCoinflip is Ownable {
    ...
    event pushGame(uint256 round);
    event removeGame(uint256 round);
    ...
    function pushRound(uint256 i) public {
        // Append to array
    ...
    function removeRound(uint256 i) public {
        uint j;
        while (j < gameList.length) {
            if (i == gameList[j]._round){
                delete gameList[j];
                emit removeGame(i);
                break;
            }
    ...
    function allotAmount(uint256 _round) private {
        ...
        if (inviter != BLACKHOLE_ADDR) {
            ...
            accountMap[inviter] += invite_amount;
        }
