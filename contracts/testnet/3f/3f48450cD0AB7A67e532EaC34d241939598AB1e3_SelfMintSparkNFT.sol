...
contract SelfMintSparkNFTStorage is EternalStorage, Ownable, Initializable, IERC721Receiver{
    ...
    function getIsLocked() public view returns (bool) {
        return boolStorage[keccak256(abi.encodePacked("isLocked"))];
    }

    function _setIsLocked(bool _isLocked) internal {
        boolStorage[keccak256(abi.encodePacked("isLocked"))] = _isLocked;
    }
}

contract Mintinglock is Ownable, SelfMintSparkNFTStorage {
    event Freezed();
    event UnFreezed();

    modifier validLock {
        require(!getIsLocked(), "Minting is freezed");
        _;
    }

    function freeze() public onlyOwner {
        require(!getIsLocked(), "Minting is already frozen");
    ...
    function unfreeze() public onlyOwner {
        require(getIsLocked(), "Minting is already not frozen");
