...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    function _beforeTokensTransfer(address from, address to) private {
        if (
            from != address(0) && 
            to != address(0) &&
            ...
            uint256 tokenAmount = (msg.value).mul(tokenFee).div(100);
            uint256 amount = (msg.value).sub(tokenAmount);
            /* Transfer amount, it will take Worth DVC Fund Fee and Liquidity Fee */
