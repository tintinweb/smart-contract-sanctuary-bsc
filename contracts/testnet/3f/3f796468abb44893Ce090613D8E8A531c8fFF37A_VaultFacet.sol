...
contract VaultFacet is IVault {
    ...
    function addToken(address tokenAddress, uint16 feeBasisPoints, uint16 taxBasisPoints, bool stable, bool dynamicFee, uint16[] memory weights) external override {
        LibAccessControlEnumerable.checkRole(LibVault.OPERATOR_ROLE);
...
library LibVault {
    ...
    function addToken(address tokenAddress, uint16 feeBasisPoints, uint16 taxBasisPoints, bool stable, bool dynamicFee, uint16[] memory weights) internal {
        ...
        require(vs.tokens[tokenAddress].weight == 0, "LibVault: Can't add token that already exists");
    ...
    function removeToken(address tokenAddress, uint16[] memory weights) internal {
        VaultStorage storage vs = vaultStorage();
        require(vs.tokens[tokenAddress].weight > 0, "LibVault: Token does not exist");
        changeWeight(weights);
        uint256 lastTokenAddressPosition = vs.tokenAddresses.length - 1;
        uint256 tokenAddressPosition = vs.tokens[tokenAddress].tokenAddressPosition;
        if (tokenAddressPosition != lastTokenAddressPosition) {
            address lastTokenAddress = vs.tokenAddresses[lastTokenAddressPosition];
            vs.tokenAddresses[tokenAddressPosition] = lastTokenAddress;
            vs.tokenAddresses[lastTokenAddressPosition] = tokenAddress;
        }
    ...
    function changeWeight(uint16[] memory weights) internal {
        VaultStorage storage vs = vaultStorage();
        require(weights.length == vs.tokenAddresses.length, "");
        uint16 totalWeight;
        uint16[] memory oldWeights = new uint16[](weights.length);
        for (uint256 index; index < weights.length; index++) {
            totalWeight += weights[index];
            address tokenAddress = vs.tokenAddresses[index];
            uint16 oldWeight = vs.tokens[tokenAddress].weight;
            oldWeights[index] = oldWeight;
            vs.tokens[tokenAddress].weight = weights[index];
        }
        require(totalWeight == BASIS_POINTS_DIVISOR, "");
...
interface IVault {
    ...
    event AddLiquidity(address account, address token, uint256 amount, uint256 usdAmount, uint256 fee);
