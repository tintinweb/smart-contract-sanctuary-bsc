...
ContextMixin
{
    ...
    uint constant public MIN_GRANT_REQUEST_DELAY = 28800; // 1 day in BSC
    ...
    constructor(
        ...
        address NFTRewards,
        address referrals
    ) public ERC20Capped(100*(10**6)*(10**18)) ERC20("BNB Synths", "BNBS") {

        // Token init
        _setupContractId("ChildMintableERC20");
        _setupDecimals(18);
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _initializeEIP712("BNB Synths");
        ...
        _setupRole(MINTER_ROLE, referrals);
    }
    ...
    // Used for the setup (by-passes the timelock the first time
    function setCallbackForTheFirstTime(address _callback) external only(DEFAULT_ADMIN_ROLE){
        _setupRole(MINTER_ROLE, _callback);
    }
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external only(DEFAULT_ADMIN_ROLE){
