...
contract BNBSpynVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function harvest(bool isReceiveBnb) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (reward > 0) {
            ...
            (uint256 amountSpyn,uint256 amountBNB) = removeLiquidity(lpNeeded);
            if(isReceiveBnb) {
                uint256 bnbAmount = swapSpynToBnb(amountSpyn).add(amountBNB);
                uint256 feeTaken = takeHarvestFee(msg.sender, bnbAmount, address(0));
                bnbAmount = bnbAmount.sub(feeTaken);
                payable(msg.sender).transfer(bnbAmount);
            }else{
                uint256 spynAmount = swapBnbToSpyn(amountBNB).add(amountSpyn);
                uint256 feeTaken = takeHarvestFee(msg.sender, spynAmount, address(spyn));
                spynAmount = spynAmount.sub(feeTaken);
                spyn.transfer(msg.sender, spynAmount);
            }
    ...
    function compound() external updateReward(address(0)) nonReentrant {
        ...
        if (_canCompound) {
            ...
            uint256 busdOut = swapSpynToBnb(spynAmountToSwap);
            (,, uint256 liquidityAmount) = _addLiquidity(rewardForPool.sub(spynAmountToSwap), busdOut);
            depositStakingPool(liquidityAmount);
    ...
    function takeHarvestFee(address _user, uint256 _pending, address _token) internal returns (uint256 feeTaken) {
        ...
        if (harvestFeeAmount > 0) {
            if (_token == address(0)) {
                payable(treasury).transfer(harvestFeeAmount);
            } else {
    ...
    function takeDepositFee(address _user, uint256 _depositedAmount, address _token) internal returns (uint256 feeTaken) {
        ...
        if (depositFeeAmount > 0) {
            if (_token == address(0)) {
                payable(treasury).transfer(depositFeeAmount);
            } else {
