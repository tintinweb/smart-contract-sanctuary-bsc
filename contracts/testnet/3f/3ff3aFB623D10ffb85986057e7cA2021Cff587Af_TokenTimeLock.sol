...
contract TokenTimeLock is Ownable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;

    uint256 public constant PERIOD = 86400; // 1 days,
    uint256 public constant TIME_LINEAR = 10; // 210 days,
    uint256 public constant START_TIME = 1649305858; // 7:30:00 GMT 9/2/2022
    address public NINO_TOKEN = 0x00738A3b92AcD2962202eE3370E91a068bcABC50;
    ...
    function setAddressVesting(address[] memory _addr, uint256[] memory _amount) external onlyOwner {
        for (uint256 i = 0; i < _addr.length; i++) {
            balanceOf[_addr[i]] = _amount[i];
            numberOfToken[_addr[i]] = _amount[i];
            nextRelease[_addr[i]] = START_TIME;
        }
    }
    ...
    function changeAddressWallet(address from, address to) external onlyOwner {
        // check address to not in the vesting
        require(countRelease[to] == 0 && balanceOf[to] == 0 && numberOfToken[to] == 0 && nextRelease[to] == 0, "address to invalid");
        //
        countRelease[to] = countRelease[from];
        countRelease[from] = 0;
        //
        balanceOf[to] = balanceOf[from];
        balanceOf[from] = 0;
        //
        numberOfToken[to] = numberOfToken[from];
        numberOfToken[from] = 0;
        //
        nextRelease[to] = nextRelease[from];
        nextRelease[from] = 0;
    }
