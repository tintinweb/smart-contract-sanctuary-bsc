...
contract Staking is ReentrancyGuard, Ownable, Runnable {
    ...
    uint256 public constant MAX_DIVISOR = 1000000000000; // 100%
    ...
    event ClaimInterest(address userAddress, uint256 tokenAmount, uint256 currentDeposited, uint256 fromTimestamp, uint256 toTimestamp);
    event WithdrawPrincipal(address userAddress, uint256 tokenAmount, uint256 currentPrincipal, uint256 fromTimestamp, uint256 toTimestamp);
    ...
    function staking(uint256 tokenAmount) external notContract whenRunning nonReentrant {
        ...
        uint256 balanceUser = stakingToken.balanceOf(msg.sender);
        require (balanceUser >= tokenAmount, "Not enough balance");
        ...
        if (user.currentDeposited == 0) {
            ...
            if (interestAmount > 0) {
                ...
                emit ClaimInterest(msg.sender, interestAmount, user.currentDeposited, user.lastClaimInterestTimestamp, block.timestamp);
            }
            if (lastTime) {
    ...
    function withdraw() external notContract whenRunning nonReentrant {
        ...
        emit WithdrawPrincipal(msg.sender, principalAmount, user.currentPrincipal, user.lastClaimPrincipalTimestamp, block.timestamp);
