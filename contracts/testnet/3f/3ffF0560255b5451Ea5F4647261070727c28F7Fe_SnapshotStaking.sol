...
contract SnapshotStaking is ISnapshotStaking, Ownable {
    ...
    constructor(
        ...
        _add(_lpToken, _rewardToken, _poolSize, _startTime, _endTime, _apr);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 reward;
        uint256 current = block.timestamp - DateTimeLib.DAY_IN_SECONDS;

        if (current > (user.lastRewardBlock - DateTimeLib.DAY_IN_SECONDS) && lpSupply != 0) {
    ...
    function stake(uint256 _pid, uint256 _amount) external _positive(_amount) {
        ...
        uint256 lpSupply = user.amount;
        // IF early staking occur
        // JUST increase staking amount and update user's lastRewardBlock if it's not set
        // NOT accumulated reward
        if(current <= pool.startTime && user.lastRewardBlock != pool.startTime){
            user.lastRewardBlock = pool.startTime;
        }
        else if(current > pool.startTime && lpSupply == 0) {
            user.lastRewardBlock = current;
        }
        else if(current > pool.startTime) {
            uint256 multiplier = getMultiplier(
                user.lastRewardBlock, 
                current, 
                pool.endTime
            );
            user.rewardPending += lpSupply * multiplier * pool.apr / (APR_DENOMINATION * 365);
            user.lastRewardBlock = current;
        }
        user.amount += _amount;
    ...
    function unStake(uint256 _pid, uint256 _amount) external _positive(_amount) {
        ...
        uint256 current = block.timestamp - DateTimeLib.DAY_IN_SECONDS;
        if(current > pool.startTime - DateTimeLib.DAY_IN_SECONDS) {
            // Reward not counting on the day withdrawal
            uint256 multiplier = getMultiplier(
                user.lastRewardBlock, 
                current, 
                pool.endTime
            ...
            uint256 lpSupply = user.amount;
            uint256 pending = user.rewardPending + lpSupply * multiplier * pool.apr / (APR_DENOMINATION * 365);
    ...
    function getMultiplier(uint256 _from, uint256 _to, uint256 _endTime)
        internal
        pure
        returns (uint256)
    {
    ...
    function emergencyWithdraw(uint256 _pid) external {
        ...
        pool.lpToken.safeTransfer(sender, lpSupply);
