...
contract Presale is Context  {
    ...
    function _safeApprove(IBEP20 token, uint256 amount) private {
        bool sent = token.approve(address(this), amount);
        require(sent, "Token approve failed");
    }

    function ApproveBTC(uint256 amount) public  {
        require(
            AllownceTIME() > 0,
            "Presale haven't start "
        );
        _safeApprove(BTC, amount);
    }
