...
contract OnChKycOrchestrator is OnChOwnableWithWhitelist {

    event KycCompleted(address indexed customerAddress_);

    event KycRevoked(address indexed customerAddress_);

    mapping(address => bool) internal _kycs;

    function completeKyc(address account) external whitelistedOnly returns (bool) {
        if (!_kycs[account]) {
            _kycs[account] = true;
            emit KycCompleted(account);
        }
        return true;
    }

    function revokeKyc(address account) external whitelistedOnly returns (bool) {
        if (_kycs[account]) {
            _kycs[account] = false;
