...
contract WhitelistedCrowdsale is Crowdsale, IWhitelistedCrowdsale {
    ...
    constructor(uint256 price_, uint256 raise_, address BUSD_, address USDT_, address USDC_, address RAX_, address pancakeRouter_, address factory_)
        Crowdsale(price_, raise_, BUSD_, USDT_, USDC_, RAX_, pancakeRouter_, factory_) {
    }
    ...
    function buy(address, uint256, uint256, address) external payable virtual override(ICrowdsale, Crowdsale) onlySalePeriod {
 ...
 * @title Crowdsale
 */
contract Crowdsale is ICrowdsale, Ownable, Pausable, ReentrancyGuard {
    ...
    function _execute(address vestingManager, address beneficiary, uint256[] memory amountRaxOuts) private {
        address[] memory allLevelsVestingWallets = _getAllLevelsVestingWallets(vestingManager, beneficiary);
    ...
    function _getAllLevelsVestingWallets(address vestingManager, address beneficiary) internal returns (address[] memory) {
        ...
        return allLevelsVestingWallets;
    }
}
