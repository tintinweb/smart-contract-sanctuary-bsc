...
contract MSDC is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2e9 *10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public _totalBurnt;

    string private _name = "METASTAKE CLASSIC";
    string private _symbol = "MSDC";
    ...
    //Transfer Fee
    uint256 public TransferTaxFee = 0;
    ...
    address payable public marketingWallet = payable(0x3612c6c6EAEbC60D485a59C5f0DfE67617124d05);
    ...
    uint256 public numTokensSellToAddToLiquidity = 2e6 *10**18;
    ...
    function _transfer(
        ...
        // Set sell fees
        else if(to == uniswapV2Pair || to.isContract() &&  from !=owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            _taxFee = _sellTaxFee;
    ...
    function setBuyFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        ...
        _buyBurnFee = burnFee;
        require(_buyTaxFee + _buyMarketingFee + _buyLiquidityFee + _buyBurnFee <= 15, "Buy Fee should be 11% or less");
    }
    ...
    function setSellFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        ...
        require( _sellTaxFee + _sellMarketingFee + _sellLiquidityFee + _sellBurnFee <= 15, "Sell Fee should be 11% or less");
    }

    function SetTransferFee(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner(){
        TransferTaxFee = taxFee;
        TransferMarketingFee = MarketingFee;
        TransferLiquidityFee = liquidityFee;
        TransferBurnFee = burnFee;

        require(TransferBurnFee <= 15, "Transfer burn Fee must be 5% or less then 5%");
    }
