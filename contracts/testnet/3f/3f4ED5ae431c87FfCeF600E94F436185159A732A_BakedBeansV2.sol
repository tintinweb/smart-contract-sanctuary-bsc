...
contract BakedBeansV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        MAX_WALLET_TVL_IN_BNB = 50000000000000000000; // 50 BNB
    ...
    uint256 private bonusRoundReward;
    bool private depositBonusEnabled;
    ...
    function getUser(address adr) public view returns (Baker memory) {
        return bakers[adr];
    }

    function getDepositBonusInfo()
        public
        view
        returns (bool enabled, uint256 bonus)
    {
        return (depositBonusEnabled, bonusRoundReward);
    }
    ...
    function toggleDepositBonus(bool toggled, uint256 bonus) public onlyCEO {
        if (bonus >= 10) {
            bonusRoundReward = 10;
        } else {
            bonusRoundReward = bonus;
        }
        depositBonusEnabled = toggled;
    }
    ...
    function buyBeans(address ref) public payable {
        ...
        if (depositBonusEnabled) {
            beansBought = Math.add(
                beansBought,
                percentFromAmount(beansBought, bonusRoundReward)
            );
        }
    ...
    function eat() public {
        ...
        if (
            ...
            beansInBnbBeforeFee = Math.sub(
                maxPayout(baker.adr),
                baker.totalPayout
            );
            baker.totalPayout = maxPayout(baker.adr);
        } else {
            baker.totalPayout = Math.add(baker.totalPayout, bnbToEat);
        }
    ...
    function dailyReward(address adr) public view returns (uint256) {
        uint256 referralsCount = bakers[adr].referrals.length;
