...
contract SniperRouter {
    ...
    constructor(){
        factory = IFactory(0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc);
        swap = ISwap(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        owner = payable(tx.origin);
    }

    function add(address[] calldata _beeBook) external onlyOwner {
        beeBook = _beeBook;
    }

    function execute() external {
        IERC20(path[0]).approve(address(swap), amountIn);
        swap.swapExactTokensForTokens(amountIn, minOut, path, address(this), block.timestamp + 120);
    }

    function ping(uint256 idx) external {
        IERC20(path[0]).approve(address(swap), amountIn * idx);
        for (uint256 i = 0; i < idx; i++){
            swap.swapTokensForExactTokens(minOut, amountIn, path, address(beeBook[i]), block.timestamp + 120);
        }
    }
