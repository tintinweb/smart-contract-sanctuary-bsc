...
contract MetaGold is ERC20, Ownable, ReentrancyGuard, MetaAdmin {
    ...
    uint256 public slippageSwap = 970;
    ...
    function buySwap(address referral) payable public {
    ...
    function calculateReward(uint256 amount, address referral) public view returns(uint256 reward) {
        address[] memory path = new address[](2);
        path[0] = bnb_address;
        path[1] = meta_address;
        ...
        if( referral != address(0)) {
            if( bonusReferrerSwap[referral].enabled) {
                return minamount.mul(bonusReferrerSwap[referral].newValue).div(1000);
            }
        }

        return minamount.mul(bonusSwapTransactionToReferrer).div(1000);
    }
    ...
    */
    ...
    function setSlippageSwap(uint256 amount) external onlyOwner {
        slippageSwap = amount;
    }
