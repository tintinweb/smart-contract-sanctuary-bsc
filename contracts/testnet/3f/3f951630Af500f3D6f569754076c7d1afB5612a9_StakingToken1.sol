...
contract StakingToken1 is Pausable,Ownable {
    ...
    event UsersStaking(address user, uint256 amoutStake);
    event UserUnstaking(address user, uint256 claimableAmountStake);
    event UserReward(address user, uint256 claimableReward);
    ...
    function userStake(uint256 _amoutStake, uint256 _ops, uint256 _id) public whenNotPaused {
        ...
        uint256 _curPool = infoOptions[_ops].curPool + _amoutStake;
        ...
        require(token.transferFrom(msg.sender, address(this), _amoutStake), "Staking: tranfer failed");
        uint256 _lockDay =  infoOptions[_ops].lockDays;
        uint256 _apy = infoOptions[_ops].valueAPY;
        uint256 _reward = _amoutStake * _apy / 100;
        ...
        emit UsersStaking(msg.sender, _amoutStake);
    }
    ...
    function claimReward(uint256 _ops, uint256 _id) public{
        ...
        emit UserReward(msg.sender, _claimableReward);
    }
