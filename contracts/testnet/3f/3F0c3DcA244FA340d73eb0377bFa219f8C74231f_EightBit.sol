...
contract DividendDistributor is IDividendDistributor {
    ...
    function distributeDividend(address shareholder, bool swapTo8Bit) internal {
        ...
        if(amount > 0){
            ...
            if(swapTo8Bit){
                uint256 _8Bit_beforeBalance = IERC20(_token).balanceOf(address(this));
                SwapTo8Bit(amount);
                IERC20(_token).transfer(shareholder, IERC20(_token).balanceOf(address(this)) - _8Bit_beforeBalance );
            }else{
    ...
    function SwapTo8Bit(uint256 btcAmount) internal {
        ...
        if(receivedTokens > 0){
            path[0] = IDEXRouter(router).WETH(); 
            path[1] = _token;
            IDEXRouter(router).swapExactETHForTokens{value : receivedTokens}(
                0,
                path,
                address(this),
                block.timestamp            
