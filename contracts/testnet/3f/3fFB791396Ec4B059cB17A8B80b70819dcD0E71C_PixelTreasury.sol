...
import "./IPixelToken.sol";
 
contract PixelTreasury is Ownable {

    address public pixelStakingManager;
    ...
    modifier onlyCounterParty {
        require(pixelStakingManager == msg.sender || insuranceFund == msg.sender, "not authorized - Pixel Treasury");
        _;
    }

    constructor(IPixelToken _pixel) {
        pixel = _pixel;
    }

    function myBalance() public view returns (uint256) {
        return pixel.balanceOf(address(this));
    }

    function mint(address recipient, uint256 amount) public onlyCounterParty {
        if(myBalance() < amount){
            pixel.mint(address(this), calulateMintAmount(amount));
        }
        pixel.treasuryTransfer(recipient, amount);
    }
    ...
    function setpixelStakingManager(address _newAddress) public onlyOwner {
        pixelStakingManager = _newAddress;
    }
    ...
    function setPixel(IPixelToken _newPixel) public onlyOwner {
        pixel = _newPixel;
    }
    ...
    function calulateMintAmount(uint256 amount) private view returns (uint256 amountToMint) {
        uint256 baseAmount = pixel.BASE_MINT();
        amountToMint = baseAmount*(amount/baseAmount+1);
        require(amountToMint < maxMintAmount, "Max exceed - Pixel Treasury");
    }
