...
interface IMarket {
    ...
    event SellToken(TokenSellingInfo tokenSellingInfo);
...
contract Market is Ownable, IMarket {
    ...
    function sellToken(uint256 tokenId, uint256 price, uint32 expireTime) override NFTContractMustSet tokenApproved(tokenId) public {
        ...
        require(!sellingInfo.isSelling, "Token already sold");
        tokens[tokenId] = TokenSellingInfo(msg.sender, price, block.timestamp + expireTime, true);
        NFTAddress.safeTransferFrom(msg.sender, owner(), tokenId);

        emit SellToken(tokens[tokenId]);
    }
    ...
    function buyToken(uint256 tokenId) NFTContractMustSet tokenApproved(tokenId) override public payable {
        TokenSellingInfo memory sellingInfo = tokens[tokenId];
        require(sellingInfo.price > 0, "Token has not been sold");
        require(msg.value >= sellingInfo.price, "You must pay greater or equal selling price");
        require(block.timestamp <= sellingInfo.expireTime, "This token is not available");
        require(sellingInfo.isSelling, "Token has not beed sold");
