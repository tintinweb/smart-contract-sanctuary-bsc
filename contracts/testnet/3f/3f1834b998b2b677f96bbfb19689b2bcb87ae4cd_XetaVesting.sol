  ...
  function releaseFunds() external onlyOwner {
    ...
    for (uint256 i = 0; i < userName.length; i++) {
        if(users[userName[i]].reward[emission] > 0){
            uint256 calculated = SafeMath.div(SafeMath.mul(users[userName[i]].reward[emission],users[userName[i]].maxSupply),10 ** decimals);
            ...
            // users[userName[i]].distributed = SafeMath.add(users[userName[i]].distributed,calculated);
        }
    }
    emission++;
