...
//  uint constant secondInADay = 86400; // 24 * 60 * 60
  uint constant secondInADay = 60 * 60; // TODO
  ...
  event CheckedIn(address indexed farmer, uint dayPassed, uint timestamp);
  ...
  function claim() external {
    require(farmers[msg.sender].point > 0, "FarmEarth: please join the farm first");
    uint dayPassed = _checkin();
    uint dateToClaim = dayPassed - 1;
    _syncDailyTotalPoint(dateToClaim);
    require(dailyTotalPoint[dateToClaim] > 0, "FarmEarth: please wait to tomorrow");
    if (checkin[msg.sender][dateToClaim]) {
      require(farmers[msg.sender].lastDateClaimed < dateToClaim, "FarmEarth: claimed");
      _claim(dateToClaim);
    } else {
      uint reward = farmers[msg.sender].point * dailyReward[dateToClaim] / dailyTotalPoint[dateToClaim];
  ...
  function withdraw() external {
    ...
    uint dateToClaim = getDaysPassed();
    _syncDailyTotalPoint(dateToClaim);
    dailyTotalPoint[dateToClaim] -= farmers[msg.sender].point;
  ...
  function getTodayReward() public view returns (uint) {
  ...
  function _syncDailyTotalPoint(uint _dateToClaim) private {
    if (dailyTotalPoint[_dateToClaim] == 0) {
      uint i = 1;
      while(i < _dateToClaim && dailyTotalPoint[_dateToClaim - i] == 0) {
        i += 1;
      }
      dailyTotalPoint[_dateToClaim] = dailyTotalPoint[_dateToClaim - i];
    }
  }
  ...
  function _claim(uint _dateToClaim) private {
    uint reward = farmers[msg.sender].point * dailyReward[_dateToClaim] / dailyTotalPoint[_dateToClaim];
    dailyClaimed[_dateToClaim] += reward;
    farmers[msg.sender].lastDateClaimed = _dateToClaim;
    farmers[msg.sender].totalEarned += reward;
    require(fotaToken.balanceOf(address(this)) >= reward, "FarmEarth: contract is insufficient balance");
...
//  function _getPointWhenDepositViaLP(uint _lpAmount) private view returns (uint) {
  ...
  function _takeFundLP(uint _amount) private {
    require(lpToken.allowance(msg.sender, address(this)) >= _amount, "FarmEarth: please approve LP token first");
    require(lpToken.balanceOf(msg.sender) >= _amount, "FarmEarth: insufficient balance");
    require(lpToken.transferFrom(msg.sender, address(this), _amount), "FarmEarth: transfer LP token failed");
  }
  ...
  function _checkin() private returns (uint) {
    uint dayPassed = getDaysPassed();
    if (!checkin[msg.sender][dayPassed]) {
      checkin[msg.sender][dayPassed] = true;
      dailyCheckinPoint[dayPassed] += farmers[msg.sender].point;
      emit CheckedIn(msg.sender, dayPassed, block.timestamp);
    }
    return dayPassed;
  }
