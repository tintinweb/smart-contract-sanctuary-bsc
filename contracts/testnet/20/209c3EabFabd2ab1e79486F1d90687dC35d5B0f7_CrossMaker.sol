...
contract CrossMaker is WiredMaker, IMaker, Ownable, SessionManager, LiquidityControl {
    ...
    function setToken(address _token) public override(WiredMaker) virtual canWire {
    ...
    function _addLiquidity( // Get amounts to transfer to the pair fee of fees.
        ...
        if (sessionParams.isOriginAction) { // Transform user-available amount to can-arrive-at-pair amount.
            if (tokenA == token) {
        ...
        if (sessionParams.isOriginAction) { // Transform must-arrive-at-pair amount to must-depart-from-user amount, then payFee.
            if (tokenA == token) {
                if (reserveA > 0 && reserveB > 0) { // Exempt, as _payFee may incur swap if there is a liquidity fee.
                    _payFee(msg.sender, _getOuterFeeAdded(amountA, rates), rates, true);
                }
            } else if (tokenB == token) {
                if (reserveA > 0 && reserveB > 0) { // Exempt, as _payFee may incur swap if there is a liquidity fee.
                    _payFee(msg.sender, _getOuterFeeAdded(amountB, rates), rates, true);
                }
            }
        }
        ...
        )
    {
        ...
        address pair = CrossLibrary.pairFor(factory, _token, WETH);
    ...
    function _removeLiquidity(
        ...
        if (sessionParams.isOriginAction) { // Transform must-arrive-at-pair amount to must-depart-from-user amount, then payFee.
            ...
            {
                if (tokenA == token) {
                    amountA -= _payFee(msg.sender, amountA, rates, false);
                } else if (tokenB == token) {
                    amountB -= _payFee(msg.sender, amountB, rates, false);
                }
            }
        }
        require(amountA >= amountAMin, sInsufficientA);
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransfer(_token, to, IERC20(token).balanceOf(address(this)));
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(to, amountETH);
    ...
    function getPair(address tokenA, address tokenB) external view override virtual returns (address pair) {
        return ICrossFactory(factory).getPair(tokenA, tokenB);
    }
}
...
abstract contract WiredMaker is IWiredMaker, WireBase {

    address public override token;
    address public override factory;

    string private constant sForbidden = "WiredMaker: Forbidded";
    string private constant sZeroAddress = "WiredMaker: Zero address";
    ...
    function informOfPair(address pair, address token0, address token1) public override(IWireBase, WireBase) virtual pairInformer {
        super.informOfPair(pair, token0, token1);
        IWiredToken(token).informOfPair(pair, token0, token1);
    }
...
uint256 constant NumberSessionTypes = 14;
