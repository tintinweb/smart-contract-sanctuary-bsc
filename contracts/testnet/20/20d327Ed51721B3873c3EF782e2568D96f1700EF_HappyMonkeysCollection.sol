...
contract HappyMonkeysCollection is Evolvable, ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, VRFConsumerBaseV2{
    ...
    mapping(uint256 => uint8) private nftLevel;

    mapping(uint8 => EvolutionLevel) public availableLevels;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        uint8 currentNftLevel = nftLevel[tokenId];
    ...
    function _transfer(address from,address to, uint256 tokenId) internal override{
        require(balanceOf(to) + 1 <= MAX_NFT_PER_USER, "You are exceeding MAX_NFT_PER_USER");
    ...
    function splitBalance(uint256 amount) internal {
        ...
        payable(admin).transfer(devShare);
    }
    ...
    function _reflectDividend(uint256 amount) internal {
        ...
        totalDividend = totalDividend + (amount / (totalSupply() + 1));
    }
    ...
    function addEvolvableAddress( address newAddress) external onlyOwner{
        require(isEvolvableAddress(newAddress) == false, "Evolvable: duplicated address");
        evolvableAddress.push(newAddress);
        emit NewEvolvableAddress(newAddress);
    }

    function removeEvolvableAddress( address newAddress) external onlyOwner{
        require(isEvolvableAddress(newAddress) == true, "Evolvable: address not found");
        (, uint8 index) = getEvolvableAddressIndex(newAddress);
        delete evolvableAddress[index];
        emit RemoveEvolvableAddress(newAddress);
    }

    function getEvolvableAddresses() external view returns(address[] memory) {
        return evolvableAddress;
    }

    function setNftLevel(uint256 tokenId, uint8 level) external onlyEvolvableAddress{
    ...
    function setNewEvolutionLevel(uint8 levelId, bool enable, string memory baseUri, string memory name) external onlyOwner {
...
abstract contract Evolvable is Context{
    address[] internal evolvableAddress;

    event NewEvolvableAddress(address newAddress);
    event RemoveEvolvableAddress(address newAddress);
    event NftEvolved(uint256 nftId, uint8 newLevel);

    function isEvolvableAddress(address addressToCheck) internal view returns(bool) {
        (bool found, ) = getEvolvableAddressIndex(addressToCheck);
        return found;
    }

    function getEvolvableAddressIndex(address addressToCheck) internal view returns(bool, uint8) {
        uint8 index = 0;
        bool found = false;
        for (uint8 i = 0; i < evolvableAddress.length && !found; i++) {
            if (evolvableAddress[i] == addressToCheck) {
                index = i;
                found = true;
            }
        }
    ...
    modifier onlyEvolvableAddress() {
        bool found = isEvolvableAddress(_msgSender());
        require(found == true, "Evolvable: caller is not an evolvable address");
        _;
    }
}
