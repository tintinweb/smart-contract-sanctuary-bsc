...
contract NewTokenV3 is IERC20, Ownable, Pausable {
    ...
    string constant public name = "NewToken";
    string constant public symbol = "NTK";
    uint constant public decimals = 18;
    uint256 private _totalSupply = 1 * 1E6 * 10**decimals;

    address public feeWallet;
    uint public feeMultiplier = 15;
    uint public feeDivider = 1000;

    address public stakingContract;
    uint public stakingMultiplier = 1;
    uint public stakingDivider = 100;
    ...
    IRouter public Router = IRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // testnet;
    ...
    constructor(address _feeWallet, address _stakingContract) {
        ...
        isExcludedFromTax[feeWallet] = true;
        isExcludedFromTax[stakingContract] = true;
    ...
    function setFeeWalletAddress(address _feeWallet) external onlyOwner {
        require(feeWallet != address(0), "Token : invalid address.");
        feeWallet = _feeWallet;
    }
    ...
    function setFeeWalletPercentage(uint _multiplier, uint _divider) external onlyOwner {
        require((100 * _multiplier / _divider) <= 5, "Token : transaction fee percentage exceeds 5.");
        feeMultiplier = _multiplier;
        feeDivider = _divider;
    }

    function setStakingContractPercentage(uint _multiplier, uint _divider) external onlyOwner {
        require((100 * _multiplier / _divider) <= 5, "Token : transaction fee percentage exceeds 5.");
        stakingMultiplier = _multiplier;
        stakingDivider = _divider;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private whenNotPaused {
        ...
        if (isExcludedFromTax[sender] || isExcludedFromTax[recipient]) {
            ...
            if(sender == address(Router) || recipient == address(Router)) {
                uint256 walletTaxAmount = amount * feeMultiplier / feeDivider;
                uint256 stakingTaxAmount = amount * stakingMultiplier / stakingDivider;

                receiveAmount = amount - walletTaxAmount - stakingTaxAmount;

                _balances[feeWallet] += walletTaxAmount;
                _balances[stakingContract] += stakingTaxAmount;
