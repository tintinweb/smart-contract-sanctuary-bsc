...
contract OpenBiSeaAuction is Ownable,IERC721Receiver,IERC1155Receiver {
    ...
    function getNFTsAuctionList(address contractNFT) public view returns (Auction[] memory) {
        Auction[] memory auctionList = new Auction[](contractsSenderPlusTokenIdList[contractNFT].length);
        for (uint i=0; i< contractsSenderPlusTokenIdList[contractNFT].length; i++) {
            auctionList[i] = contractsPlusTokenIdsAuction[contractsSenderPlusTokenIdList[contractNFT][i]];
        }
        return auctionList;
    }
    ...
    function createAuction(
        ...
        contractsTokenIdList[contractNFT].push(addressPlusTokenId(sender, tokenId));
