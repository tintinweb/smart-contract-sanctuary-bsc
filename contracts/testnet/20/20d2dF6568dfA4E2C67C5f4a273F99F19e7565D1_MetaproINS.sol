...
interface MetaproRoyalty {
    function getTeamMembers(uint256 _tokenId)
        external
        view
        returns (Royalty.RoyaltyTeamMember[] memory);

    function getTeam(uint256 _tokenId)
        external
        view
...
contract MetaproINS is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    // @dev: INS
    mapping(uint256 => Royalty.RoyaltyTeamMember[]) public insRoyaltyFees;

    // @dev: Tressury fee - interger value - example: 500 -> 5%
    uint256 public treasuryFee = 250; // 250 = 2.5%
    ...
    uint256[] private activeInsTokenIds;

    uint256[] private finishedInsTokenIds;
    ...
    function create(
        ...
        saveInsRoyaltyFees(currentInsId, tokenId);
        ...
        for (uint256 i = 0; i < activeInsTokenIds.length; ++i) {
            if (activeInsTokenIds[i] == tokenId) {
                tokenIdAlreadyexists = true;
            }
        }

        if (!tokenIdAlreadyexists) {
            activeInsTokenIds.push(tokenId);
        }
    ...
    function saveInsRoyaltyFees(uint256 _insId, uint256 _tokenId) private {
        Royalty.RoyaltyTeamMember[] memory teamMembers = metaproRoyalty
            .getTeamMembers(_tokenId);

        Royalty.RoyaltyTeamMember[] storage royaltyInsFees = insRoyaltyFees[
            _insId
        ];
    ...
    function deposit(
        ...
        INSDeposit memory walletDeposit = INSDeposit(msg.sender, amount);
        ...
        emit Deposit(configuration.tokenId, insId, msg.sender, amount);
    }
    ...
    function withdraw(uint256 insId, bytes memory _data) external nonReentrant {
        ...
        INSConfiguration storage insConfiguration = availableIns[insId];
        Referral.ReferralFees storage feesConfiguration = insReferralFees[
            insId
        ];

        bool canWidtraw = false;
        if (insConfiguration.operator == msg.sender) {
            canWidtraw = true;
        } else {
        ...
        require(block.number >= insConfiguration.endBlock, "INS: not ended");

        require(alreadyCompletedIns[insId] != true);

        uint256 tokenTransfered = 0;

        if (insConfiguration.minCap != 0) {
            if (
                insConfiguration.currentCapInBusd <
                insConfiguration.minCapInBusd
            ) {
                for (uint256 i = 0; i < deposits[insId].length; i++) {
                    busd.transfer(
        ...
        uint256 tokenBalance = insConfiguration.maxCap - tokenTransfered;

        if (tokenBalance != 0) {
            token.safeTransferFrom(
                address(this),
                insConfiguration.operator,
                insConfiguration.tokenId,
                tokenBalance,
                _data
            );
        }
        ...
        for (uint256 i = 0; i < activeInsTokenIds.length; i++) {
            if (activeInsTokenIds[i] == insConfiguration.tokenId) {
                delete activeInsTokenIds[i];
                finishedInsTokenIds.push(insConfiguration.tokenId);
            }
        }

        insConfiguration.valid = false;
        alreadyCompletedIns[insId] = true;
        emit INSComplete(insId, insConfiguration.tokenId);
    }
    ...
    function getActiveInsTokenIds() public view returns (uint256[] memory) {
        uint256 correctArraySize = 0;

        for (uint256 i = 0; i < activeInsTokenIds.length; i++) {
            if (activeInsTokenIds[i] != 0) {
                correctArraySize += 1;
            }
        }

        uint256[] memory activeIns = new uint256[](correctArraySize);
    ...
    function getFinishedInsTokenIds() public view returns (uint256[] memory) {
        return finishedInsTokenIds;
    }
    ...
    function sendFeesToRoyaltyTeamMembers(
        ...
        uint256 fee = 0;
        Royalty.RoyaltyTeamMember[] storage royaltyMembers = insRoyaltyFees[
            _insConfiguration.insId
        ];
    ...
    function sendTokenToDepositer(
        INSConfiguration memory _insConfiguration,
        Referral.ReferralFees memory _insFeesConfiguration,
        INSDeposit memory _insWalletDeposits,
        uint256 tokenAmount,
        bytes memory _data
    ) private returns (uint256) {
        uint256 depositAmount = _insWalletDeposits.amount;

        uint256 referralFee = sendFeesToReferrers(
            _insConfiguration,
            _insFeesConfiguration,
            _insWalletDeposits.amount,
            _insWalletDeposits.wallet
        );
        ...
        uint256 depositAmountWithFees = depositAmount - referralFee;

        return depositAmountWithFees;
    }

    function sendEarningsToOperator(
        INSConfiguration memory _insConfiguration,
        uint256 _walletDepositsWithRefFee,
        uint256 _treasuryFee,
        uint256 _royaltyFee
    ) private {
        busd.transfer(
            _insConfiguration.operator,
            _walletDepositsWithRefFee - _royaltyFee - _treasuryFee
        );
    }
    ...
    function processWithdraw(
        ...
        if (_insWalletDeposits.length == 0) {
            return 0;
        }

        uint256 tokenTransfered = 0;
        uint256 depositsWithReferralFee = 0;

        for (uint256 i = 0; i < _insWalletDeposits.length; i++) {
            INSDeposit memory singleWalletDeposit = _insWalletDeposits[i];
            uint256 tokenAmount = singleWalletDeposit.amount.div(
                _insConfiguration.pricePerToken
            );

            depositsWithReferralFee += sendTokenToDepositer(
                _insConfiguration,
                _insFeesConfiguration,
                singleWalletDeposit,
                tokenAmount,
        ...
        sendEarningsToOperator(
            _insConfiguration,
            depositsWithReferralFee - treasuryFeeAmount - royaltyFee,
            treasuryFee,
            royaltyFee
        );
