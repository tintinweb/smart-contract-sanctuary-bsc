...
// File contracts/StockDetails.sol


pragma solidity 0.8.17;


contract StockDetails is Ownable, Pausable {
    error Game__StockIdCanNotBeZero();
    event StockIdUpdated(uint256 stockId, string stockSymbol);
    /**
     * @notice Stock Name For This Stock ID  (stockID -> StockName) start
     * storing stockId's from 1, i.e when stockId = 0, that is not a valid stock
     */
    mapping(uint256 => string) public stockIdToSymbol;
    ...
    /**
     * @notice Update Stock Symbol
     * @dev Revert When stock ID is 0 , Revert if sender is not the owner , Emit event Of Updated Stock
     * @param _stockId stock ID of which name has to be update
     * @param _stockSymbol stock name which has to updated for guiven stock ID
     */
    function updateStock(uint256 _stockId, string memory _stockSymbol)
        public
        onlyOwner
    {
        if (_stockId == 0) {
            revert Game__StockIdCanNotBeZero();
        }
        stockIdToSymbol[_stockId] = _stockSymbol;
        emit StockIdUpdated(_stockId, _stockSymbol);
    }

    function _populateStockIds() private {
        stockIdToSymbol[1] = "GME";
        stockIdToSymbol[2] = "TSLA";
        stockIdToSymbol[3] = "AAPL";
        stockIdToSymbol[4] = "AMZN";
        stockIdToSymbol[5] = "NIO";
        stockIdToSymbol[6] = "AMC";
        stockIdToSymbol[7] = "MSFT";
        stockIdToSymbol[8] = "DIS";
        stockIdToSymbol[9] = "NFLX";
        stockIdToSymbol[10] = "META";
        stockIdToSymbol[11] = "NVDA";
        stockIdToSymbol[12] = "AMD";
        stockIdToSymbol[13] = "WMT";
        stockIdToSymbol[14] = "GOOG";
        stockIdToSymbol[15] = "PYPL";
        stockIdToSymbol[16] = "SQ";
        stockIdToSymbol[17] = "UBER";
        stockIdToSymbol[18] = "KO";
        stockIdToSymbol[19] = "PEP";
        stockIdToSymbol[20] = "COST";
        stockIdToSymbol[21] = "ROKU";
        stockIdToSymbol[22] = "NKE";
        stockIdToSymbol[23] = "SBUX";
        stockIdToSymbol[24] = "ETSY";
        stockIdToSymbol[25] = "TGT";
        stockIdToSymbol[26] = "MCD";
        stockIdToSymbol[27] = "TWTR";
        stockIdToSymbol[28] = "SNAP";
        stockIdToSymbol[29] = "BB";
        stockIdToSymbol[30] = "IXIC";
        stockIdToSymbol[31] = "DJI";
        stockIdToSymbol[32] = "GSPC";
        stockIdToSymbol[33] = "LOW";
        stockIdToSymbol[34] = "AAL";
        stockIdToSymbol[35] = "HD";
        stockIdToSymbol[36] = "VZ";
        stockIdToSymbol[37] = "T";
        stockIdToSymbol[38] = "CMCSA";
        stockIdToSymbol[39] = "DAL";
        stockIdToSymbol[40] = "DISH";
        stockIdToSymbol[41] = "SAVE";
        stockIdToSymbol[42] = "UAL";
        stockIdToSymbol[43] = "BA";
        stockIdToSymbol[44] = "JBLU";
        stockIdToSymbol[45] = "LUV";
        stockIdToSymbol[46] = "DELL";
        stockIdToSymbol[47] = "DPZ";
        stockIdToSymbol[48] = "PARA";
        stockIdToSymbol[49] = "XOM";
        stockIdToSymbol[50] = "CVX";
    }
}
...
contract Head2Head is Ownable, Pausable {
    ...
    error Head2Head__WrongStockIds(uint256 stockId1, uint256 stockId2);
    error Head2Head__StockIdNotInGame(uint256 gameId, uint256 stockId);
    ...
    struct Head2HeadGame {
        uint256[2] stockIds; // stock IDs listed in this game
        mapping(uint256 => bool) isStockInGame; // boolean check is stock is listed in this game
        ...
        mapping(uint256 => uint256) startGameStockPricesInWei; // stockId => price before game start in wei uints
        mapping(uint256 => uint256) endGameStockPricesInWei; // stockId => price after game start in wei uints
        uint256 winningStockId; // Stock ID of winning stock
        bool rewardsDistributed; // Is Reward Distributed in this game ?
        address payable[] betters; // address array of all the betters
        uint256 totalNumberOfBetters;
    ...
    StockDetails stockDetailsContract;
     ...
     */
    function placeBet(uint256 _gameId, uint256 _stockId)
        external
        payable
        whenNotPaused
    {
        ...
        } else if (!head2headGames[_gameId].isStockInGame[_stockId]) {
    ...
    function createGame(
        ...
        if (
            keccak256(abi.encodePacked(_stockSymbols[0])) ==
            keccak256(abi.encodePacked(emptyString)) ||
            keccak256(abi.encodePacked(_stockSymbols[1])) ==
            keccak256(abi.encodePacked(emptyString))
        ) {
            revert Head2Head__WrongStockIds(_stockIds[0], _stockIds[1]);
        }
        if (_stockIds[0] == 0 || _stockIds[1] == 0) {
            revert Head2Head__WrongStockIds(_stockIds[0], _stockIds[1]);
        }
    ...
    function rewardDistribution(uint256 _gameId) public onlyOwner {
        uint256 winningStockId = head2headGames[_gameId].winningStockId;
    ...
    function updateBetAmount(
        ...
        if (betAmountValueSent == 0) {
            // WHEN VALUE SENT IS ZERO MEANS DECREASE THE BET
            if (
     ...
     * @param _stockId stock ID
     */
    function cancelBet(uint256 _gameId, uint256 _stockId)
        external
        whenNotPaused
    {
        // SUFFICIENT CHECKS
    ...
    function emergencyWithdrawFunds(uint256 _gameId) external onlyOwner {
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            uint256 stockId = bets[_gameId][betters[i]].stockId;
    ...
    function getEndGameStockPriceOfStockId(uint256 _gameId, uint256 _stockId)
        public
        view
        returns (uint256)
    {
    ...
    function getStartGameStockPriceOfStockId(uint256 _gameId, uint256 _stockId)
        public
        view
        returns (uint256)
    {
    ...
    function getIsStockInGame(uint256 _gameId, uint256 _stockId)
        public
        view
        returns (bool)
    {
