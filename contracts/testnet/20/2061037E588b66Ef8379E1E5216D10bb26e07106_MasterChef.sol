...
interface IMigratorChef {
    // Perform LP token migration from legacy EnvoysSwap to EvtSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to EnvoysSwap LP tokens.
    // EvtSwap must mint EXACTLY the same amount of EvtSwap LP tokens or
    // else something bad will happen. Traditional EnvoysSwap does not
    // do that so be careful!
    function migrate(IBEP20 token) external returns (IBEP20);
}

// MasterChef is the master of Evt. He can make Evt and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once EVT is sufficiently
...
contract MasterChef is Ownable {
    ...
    uint256 public evtPerBlock;
    // Bonus muliplier for early evt makers.
    ...
    function pendingEvt(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accEvtPerShare = accEvtPerShare.add(evtReward.mul(1e12).div(lpSupply));
        }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit EVT by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw EVT by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake EVT tokens to MasterChef
    ...
    // Withdraw EVT tokens from STAKING.
