...
contract MasterPlatypusV3 {    
    ...
    */
    ...
    function _totalLPAmount() internal view returns (uint256) {
        ...
        for (uint i=0;i<lpTokens.length;i++) {
            totalLpAmounts += Asset(lpTokens[i]).totalSupply() / Asset(lpTokens[i]).decimals();
        }
    ...
    function _calcIncreasedBasePTPReward (uint256 pid, LPStakedUserInfo storage userinfo) private view returns (uint256) {
        if (block.timestamp - userinfo.lastTimestamp > 60 && Asset(lpTokens[pid]).totalSupply() > 0)
            return _rfBasePTP * userinfo.lpAmount * (block.timestamp - userinfo.lastTimestamp) * _coverageRatio(pid) * _totalLPAmount() / (lpTokens.length * Asset(lpTokens[pid]).totalSupply() / Asset(lpTokens[pid]).decimals() * 10**18 * 100000);
        else return 0;
    }
    ...
    function _updateLPStakedUserInfoForStaking (uint256 pid, address user, uint256 amount) private {
        if (isExistLPStakedUserInfo(pid, user)) {
            ...
            _lpStakedUserInfo[pid][user].lpAmount += amount / Asset(lpTokens[pid]).decimals();
        } else {
    ...
    */
    ...
    function unStakingLP(uint256 pid, uint256 amount) public {
        ...
        require(userinfo.lpAmount >= amount / Asset(lpTokens[pid]).decimals(), "unStakingLP: insufficient amount");
    ...
    function lpStakedInfo(uint256 pid, address user) public view returns (uint256 lpAmount, uint256 rewardAmount) {
        ...
        lpAmount = userinfo.lpAmount * Asset(lpTokens[pid]).decimals();
    }
    ...
    {        
        ...
        for (uint i = 0; i<lpTokens.length; i++) {   
            ...
            lpAmounts[i] = userinfo.lpAmount * Asset(lpTokens[i]).decimals();
    ...
    function baseAPR (uint256 pid) public view returns (uint256) {     
        if (Asset(lpTokens[pid]).totalSupply() > 0)   
            return _rfBasePTP * (365 * 24 * 60 * 60) * _coverageRatio(pid) * _totalLPAmount() * 100 * 10**18 / (lpTokens.length * Asset(lpTokens[pid]).totalSupply() / Asset(lpTokens[pid]).decimals() * 10**18 * 100000);
        else return 0;
    }
