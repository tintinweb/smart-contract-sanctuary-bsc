...
// File contracts/baby/Pr1712DividendTracker.sol
...
contract Pr1712DividendTracker is OwnableUpgradeable, DividendPayingToken {
  using SafeMath for uint256;
  using SafeMathInt for int256;
  ...
  function withdrawDividend() public pure override {
    require(
      false,
      "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Pr1712 contract."
    );
  }
...
// File contracts/Pr1712.sol

pragma solidity ^0.8.4;

contract Pr1712 is ERC20, Ownable, BaseToken {
  using SafeMath for uint256;
  ...
  Pr1712DividendTracker public dividendTracker;
  ...
  constructor(
    ...
    dividendTracker = Pr1712DividendTracker(payable(Clones.clone(addrs[3])));
  ...
  function updateDividendTracker(address newAddress) public onlyOwner {
    require(
      ...
      'Pr1712: The dividend tracker already has that address'
    );

    Pr1712DividendTracker newDividendTracker = Pr1712DividendTracker(
      payable(newAddress)
    );

    require(
      newDividendTracker.owner() == address(this),
      'Pr1712: The new dividend tracker must be owned by the Pr1712 token contract'
    );
  ...
  function updateUniswapV2Router(address newAddress) public onlyOwner {
    require(
      ...
      'Pr1712: The router already has that address'
    );
  ...
  function excludeFromFees(address account, bool excluded) public onlyOwner {
    require(
      ...
      "Pr1712: Account is already the value of 'excluded'"
    );
    _isExcludedFromFees[account] = excluded;
    ...
    onlyOwner
  {
    require(
      pair != uniswapV2Pair,
      'Pr1712: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs'
    );
  ...
  function _setAutomatedMarketMakerPair(address pair, bool value) private {
    require(
      ...
      'Pr1712: Automated market maker pair is already set to that value'
    );
    automatedMarketMakerPairs[pair] = value;
  ...
  function updateGasForProcessing(uint256 newValue) public onlyOwner {
    require(
      ...
      'Pr1712: gasForProcessing must be between 200,000 and 500,000'
    );
    require(
      newValue != gasForProcessing,
      'Pr1712: Cannot update gasForProcessing to same value'
    );
