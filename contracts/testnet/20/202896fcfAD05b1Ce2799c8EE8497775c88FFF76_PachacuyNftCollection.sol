    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        _addRarityToMap(1, _phenomenomList);
        _addRarityToMap(2, _mysticList);
        _addRarityToMap(3, _legendaryList);
    ...
    function purchaseNftWithBusdAndId(uint256 _id) public nonReentrant {
        ...
        uint256 priceForNftWithId = _calculatePriceByNftId(_id);
    ...
    function _calculatePriceByNftId(uint256 _id)
        internal
        view
        returns (uint256)
    {
        if (phenomenomMap[_id]) return phenomenoPrice;
        if (mysticMap[_id]) return mysticPrice;
        if (legendaryMap[_id]) return legendaryPrice;
        if (_id == 1403) return mandingoPrice;
        return explorerPrice;
    }
