...
contract TokenDao is Context, IBEP20, Ownable, PancakeTool {
    ...
    constructor(
        ...
        lottery.interval = 7200; 
        lottery.epochLength = 30;  
        lottery.rewardNum = 10;   
        _updateLottery(_current);

        lockRe.start = _current;
        lockRe.lockInterval = 3600;       
        lockRe.openInterval = 30;  
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        if (!Address.isContract(address(msg.sender)) && address(msg.sender) != owner() && !_excludes[msg.sender]) {
            require(
    ...
    returns (bool) {
        if (!address(sender).isContract() && Address.isContract(address(msg.sender)) && sender != owner() && !_excludes[sender]) {
            require(amount <= _balances[sender].mul(divBase.sub(mustRemain)).div(divBase));
        }
        if(!address(sender).isContract() && isPair[recipient]&& sender != owner() && !_excludes[sender]){
    ...
    internal {
        ...
        uint256 rLottery = 0;
        uint256 _cFee = 0;
        if(user != address(0) && user != owner() && !_excludes[user]){
