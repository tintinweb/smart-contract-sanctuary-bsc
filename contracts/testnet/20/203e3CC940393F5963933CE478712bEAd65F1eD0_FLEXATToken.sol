...
contract FLEXATToken is ERC20 {
    mapping(address => bool) public signers;
    uint pendingTransactionID = 1;
    uint OwnerpendingTransactionID = 0;
    uint public TransactionsCount = 0;
    uint public OwnerTransactionsCount = 0;
    uint[] public PendingTransactionList;
    uint[] public OwnerPendingTransactionList;
    ...
    function getAllVestingTransaction() public view onlyVestingWallet returns(uint[] memory) 
    {
    ...
    function checkVestingTransactionList(uint _id) internal view returns(bool)
    {
     ...
     function checkOwnerPendingTransactionList(uint _id) internal view returns(bool)
    {
        for(uint i = 0; i < OwnerPendingTransactionList.length; i++)
        {
            if(OwnerPendingTransactionList[i] == _id)
            {
                return true;
            }
        }
    ...
    function deleteExceededTimeVestingTransaction() internal
    {
        ...
        function deleteExceededTimeOwnerPendingTransaction() internal
    {
        for(uint i = 0; i < OwnerPendingTransactionList.length; i++)
        {
            bool check = checkOwnerPendingTransactionList(i);
            if(check && block.timestamp > OwnerTransactions[i].ExecuteDate + 300)
            {
                OwnerTransactions[i].status = "failed";
                OwnerPendingTransactionList[i]=0;
            }
        }
    }
      ...
      function approvePendingVestingTransaction(uint256 _id) onlySigner public
    {
        ...
        bool check = checkVestingTransactionList(_id);
            if(check)
            {
                if(block.timestamp < VestingTransactions[_id].ExecuteDate + 300 && keccak256(abi.encodePacked((VestingTransactions[_id].status))) == keccak256(abi.encodePacked(("pending"))))
                {
    ...
    function approvePendingOwnerTransactions() onlySigner public
    {
        for(uint i = 0; i < OwnerPendingTransactionList.length; i++)
        {
            uint id =  OwnerPendingTransactionList[i];
            bool check = checkOwnerPendingTransactionList(id);
            if(check)
            {
                if(keccak256(abi.encodePacked((VestingTransactions[id].status))) == keccak256(abi.encodePacked(("pending"))))
                {
                    approvePendingOwnerTransaction(id);
                }
            }
        }
    }

      function approvePendingOwnerTransaction(uint256 _id) onlySigner public
    {
       
        bool check = checkOwnerPendingTransactionList(_id);
            if(check)
            {
                if(block.timestamp < OwnerTransactions[_id].ExecuteDate + 300 && keccak256(abi.encodePacked((VestingTransactions[_id].status))) == keccak256(abi.encodePacked(("pending"))))
                {
                    OwnerTransactions[_id].status ="successful";
                     _transfer(owner, OwnerTransactions[_id].to, OwnerTransactions[_id].amount); 
    ...
    function Claim() public returns (bool)
    {
     ...
     function transfer(address recipient, uint amount) override public returns (bool) {
        if(msg.sender == owner){
        uint id =  OwnerTransactionsCount;
        OwnerTransactions[id] = OwnerTransaction(recipient, amount, "pending",block.timestamp);
        OwnerPendingTransactionList.push(id+1);
        OwnerpendingTransactionID = OwnerpendingTransactionID + 1;
        OwnerTransactionsCount = OwnerTransactionsCount +1;
        } 
        else {
             _transfer(_msgSender(), recipient, amount);
        }
