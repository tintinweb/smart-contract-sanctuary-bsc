...
// File contracts/PrivateIDO.sol


interface IMasterChef {
  function getRank(address user) external view returns (uint256 _userTier);
}

contract PrivateIDO is ReentrancyGuard, Initializable {
  ...
  mapping(uint256 => uint256) public multiplierToTier;

  // Allocation amount;
  //   uint256 public allocationLimit;
    ...
    initializer
  {
    ...
    for (uint256 i = 0; i < HARVEST_PERIODS; i++) {
      harvestReleaseTimestamps[i] = _startClaimingTime + (_vestingOffset * i);
    }

    multiplierToTier[4] = 550;
    multiplierToTier[3] = 400;
    multiplierToTier[2] = 175;
    multiplierToTier[1] = 100;
    multiplierToTier[0] = 0;
  }

  function setMultiplier(uint256 tierTitle, uint256 multiplier) public onlyOwner {
    multiplierToTier[tierTitle] = multiplier;
  }

  function isRegistered(address account) public pure returns (bool) {
    return true;
  }

  function addWhitelist(address[] memory _whitelist) external onlyOwner {
    for (uint256 i = 0; i < _whitelist.length; i++) {
      isWhitelist[_whitelist[i]] = true;
    }
  }

  function removeWhitelist(address[] memory _whitelist) external onlyOwner {
    for (uint256 i = 0; i < _whitelist.length; i++) {
      delete isWhitelist[_whitelist[i]];
    }
  }
  ...
  function updateStartClaimingTime(uint256 _startClaimingTime, uint256 _vestingOffset)
    external
    onlyOwner
  {
    startClaimingTime = _startClaimingTime;
  ...
  function setTreasuryDepositAddress(address _treasuryDepositAddress) public onlyOwner {
    treasuryDepositAddress = _treasuryDepositAddress;
  }

  // Update treasury withdraw address
  function setTreasuryWithdrawAddress(address _treasuryWithdrawAddress) public onlyOwner {
    treasuryWithdrawAddress = _treasuryWithdrawAddress;
  }
  ...
  function getLimitDeposit(address _user) public view returns (uint256) {
    uint256 limit = 0;
    uint256 userTier = masterChef.getRank(_user);
    uint256 allocationMultiplier = multiplierToTier[userTier];
    // console.log("Allocation multiplier ", allocationMultiplier);
    uint256 allocationLimit = (allocationMultiplier * baseAllocation) / 100;

    limit = allocationLimit - userInfo[_user].amount;
    uint256 remainAmount = raisingAmount - totalAmount;
    if (remainAmount < limit) limit = remainAmount;
  ...
  function deposit(uint256 _amount) external nonReentrant onlyActiveIDO {
    ...
    require(totalAmount + _amount <= raisingAmount, "totalAmount + _amount > raisingAmount");

    require(_amount <= getLimitDeposit(msg.sender), "Deposit amount is greater than the limit");

    raisingToken.safeTransferFrom(msg.sender, treasuryDepositAddress, _amount);
  ...
  function harvest(uint256 harvestPeriod) external nonReentrant {
    ...
    require(block.timestamp > harvestReleaseTimestamps[harvestPeriod], "not harvest time");
    require(userInfo[msg.sender].amount > 0, "have you participated?");
    require(!userInfo[msg.sender].claimed[harvestPeriod], "harvest for period already claimed");

    uint256 offeringTokenAmountPerPeriod = getOfferingAmountPerPeriod(msg.sender, harvestPeriod);
    ...
    returns (uint256 offeringTokenHarvest, uint256 offeringTokensVested)
  {
    ...
    for (uint256 i = 0; i < HARVEST_PERIODS; i++) {
      if (currentTime >= harvestReleaseTimestamps[i] && !userInfo[_user].claimed[i]) {
