    ...
    */
    contract Ownable is Context {
        ...
        */
        function _transfer(address sender, address recipient, uint256 amount) internal {
            ...
            if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]){
                if(sender == _lpPairAddress){
                    // purchase 10: 3 2 3 2ck
                    afterTax = amount - amount * _purchaseTax / 100;
                    tax = amount - afterTax;

                    (uint256 burnOSKAmount, uint256 marketAmount, uint256 sharePool, uint256 burnAmount) = calculateTaxFee(tax, 30, 20, 30);

                    _burnFrom(sender, burnAmount);
                    _balances[sender] = _balances[sender].sub(burnOSKAmount + marketAmount + sharePool , "BEP20: transfer amount exceeds balance");
                    ...
                    // sell & transfer 10: 4 2 2 2ck
                    afterTax = amount - amount * _sellTax / 100;
                    tax = amount - afterTax;
                    (uint256 burnOSKAmount, uint256 marketAmount, uint256 sharePool, uint256 burnAmount) = calculateTaxFee(tax, 40, 20, 20);
        ...
        function calculateTaxFee(uint256 taxAmount, uint256 oskBurnPercent, uint256 marketPercent, uint256 sharePoolPercent) internal pure returns(uint256, uint256, uint256, uint256){
            uint256 burnOSKAmount = taxAmount * oskBurnPercent / 100;
            uint256 marketAmount = taxAmount * marketPercent / 100;
            uint256 sharePool = taxAmount * sharePoolPercent / 100;
            uint256 burnAmount = taxAmount - burnOSKAmount - marketAmount - sharePool;
            return (burnOSKAmount, marketAmount, sharePool, burnAmount);
        }
        ...
        */
        ...
        function setTaxFreeAddress(address addr) external onlyAdmin{
            _isExcludedFromFee[addr] = true;
        }
        function removeTaxFreeAddress(address addr) external onlyAdmin{
