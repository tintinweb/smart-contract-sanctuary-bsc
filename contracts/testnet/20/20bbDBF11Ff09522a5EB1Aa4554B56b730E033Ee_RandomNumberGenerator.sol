...
interface IPBXLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in PBX per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in PBX token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInPbx: price of a ticket in PBX
...
contract RandomNumberGenerator is VRFConsumerBase, IRandomNumberGenerator, Ownable {
     ...
     * @param _seed: seed provided by the PBXLottery lottery
     */
    function getRandomNumber(uint256 _seed) external override {
        require(msg.sender == PBXLottery, "Only PBXLottery");
    ...
    /**
     * @notice Set the address for the PBXLottery
     * @param _PBXLottery: address of the PBXLottery lottery
     */
    function setLotteryAddress(address _PBXLottery) external onlyOwner {
        PBXLottery = _PBXLottery;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        latestLotteryId = IPBXLottery(PBXLottery).viewCurrentLotteryId();
    }
}