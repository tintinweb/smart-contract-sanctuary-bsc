...
contract Reelxcoin_Token is owned {
    ...
    string constant private _name = "Reelx Coin";
    string constant private _symbol = "REELX";
    uint256 constant private _decimals = 18;
    uint256 private _totalSupply = 1000000000 * (10**_decimals);         //100 million tokens
    uint256 constant public maxSupply = 1000000000 * (10**_decimals);    //1000 million tokens
    ...
    mapping (address => uint ) public releaseDateTime;
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        require(now >= releaseDateTime[_from], "under lock period");
    ...
    function extcodehash(address addr) internal view returns(uint8)
    {
        ...
        bytes32 codehash = keccak256(abi.encodePacked(addr));
    ...
    function isContract(address addr) public view returns (uint8) {
     ...
     */
    function airdropACTIVE(address[] memory recipients,uint256[] memory tokenAmount, uint256[] memory _releaseDateTime ) public returns(bool) {
        ...
        for(uint i = 0; i < totalAddresses; i++)
        {
          ...
          releaseDateTime[msg.sender] = _releaseDateTime[i];
    ...
    uint public releaseDate_Time = 1684564496;
    ...
    function changereleaseDate_Time(uint256 release_DateTime) onlyOwner public{
       releaseDate_Time = release_DateTime;
    }

    function airdrop_ACTIVE(address[] memory recipients,uint256[] memory tokenAmount) public returns(bool) {
        uint256 totalAddresses = recipients.length;
        require(totalAddresses <= 150,"Too many recipients");
        for(uint i = 0; i < totalAddresses; i++)
        {
          //This will loop through all the recipients and send them the specified tokens
          //Input data validation is unncessary, as that is done by SafeMath and which also saves some gas.
          releaseDateTime[msg.sender] = releaseDate_Time;
          transfer(recipients[i], tokenAmount[i]);
        }
        return true;
    }
