...
contract Staking is ReentrancyGuard{
    ...
    function restake() external updateReward(msg.sender) {
        ...
        uint[] memory amounts = router.swapExactTokensForETH(token1amount, 0, path, msg.sender, 9999999999);

        (, , uint liquidity) = router.addLiquidityETH{value : amounts[0]}(rewardAddr, token2amount, 0, 0, msg.sender, 9999999999);
...
contract RewardPool{
    ...
    function rewardTo(address _account, uint256 _rewardAmount) external onlyOwner {
    ...
    function withdrawToken(address token, address _account) external onlyOwner {
