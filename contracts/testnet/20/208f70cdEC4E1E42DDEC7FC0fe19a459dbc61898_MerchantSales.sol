...
interface IMerchantMessage {
    function queryMerchantSta(address _merchant) external view returns (bool);
}
...
contract MerchantSales  is Initializable,Ownable {
    ...
    modifier onlyFundManagementContract() {
        require(msg.sender == address(fundManagement), "MerchantSales: Call without permission");
        _;
    }
    ...
    function __MerchantSalesContract_init_unchained(address _file) internal initializer{
        ...
        address _MerchantMessage = file.queryStrAddr("MerchantMessage");
        address _InitAddr = file.queryStrAddr("InitAddr");
        require(
            _FundManagement != address(0x0) && 
            _CMMToken != address(0x0) && 
            _USDTToken != address(0x0) && 
            _InitAddr != address(0x0) &&
        ...
        merchantMessage = IMerchantMessage(_MerchantMessage);
        ...
        userRecommendID[_InitAddr] = _recommendIndex;
         recommendUser[_recommendIndex] = _InitAddr;
    }
    ...
    function buyGoods(address _merchant, uint256 _amount, uint256 _recommenderId) external { 
        ...
        require( merchantMessage.queryMerchantSta(_merchant),"The address is not a business address");
        ...
        require( usdtToken.transfer(_merchant, offsetAmount),"Token transfer failed");
        require( _burnToken(fee),"Token burn failed");
        userBasicMsg[_sender].staticComputingPower = userBasicMsg[_sender].staticComputingPower.add(_amount);
        userBasicMsg[_merchant].staticComputingPower = userBasicMsg[_merchant].staticComputingPower.add(fee);
        ...
        _updateRecommender(_sender, _recommenderId);
        calcRecommenderComputingPower(_sender,_amount, timestamp, fee/10);
    }

    function buyMember(uint256 _grade, uint256 _recommenderId) external { 
        ...
        if(_userBasicMsg.memberLevel == 0){
            ...
            uint256 _selfRecommenderId = _generateRecommenderId(_sender);
            require( _selfRecommenderId != _recommenderId,"The recommender cannot be yourself");
            address _recommender = _userBasicMsg.recommender;
            if(_recommender !=address(0x0))userBasicMsg[_recommender].recommendMemberNum++;
        }else {
        ...
        addComputingPower(_sender, _amount, timestamp);
        _updateRecommender(_sender, _recommenderId);
        calcRecommenderComputingPower(_sender,_amount, timestamp, 0);
    }
    
    function updateRecommender(address _sender, uint256 _recommenderId) external returns(bool){ //onlyFundManagementContract()
        _updateRecommender(_sender, _recommenderId);
        return true;
    }

    function _updateRecommender(address _sender, uint256 _recommenderId) internal { 
        ...
        if(_recommender == address(0x0)){
            ...
            require(_recommender != address(0x0),"Referrer cannot be empty");
    ...
    function _generateRecommenderId(address _sender) internal returns (uint256){
        ...
        if(_selfRecommenderId == 0){
            _selfRecommenderId = recommendIndex++;
            userRecommendID[_sender] = _selfRecommenderId;
            recommendUser[_selfRecommenderId] = _sender;
        }
    ...
    //计算过期会员奖励
    function handleMemberAward(uint256 _memberNum) external{
    ...
    //计算过期会员奖励
    function calcMemberAward(uint256 _memberNum) internal{
    ...
    function recordUserRecommenderMsg(address _recommender, address _sender, uint256 _amount, uint256 _recommendType) public{
        ...
        _userRecommenderMsg.indexRecommend[_recommendNum] = _sender;
        _userRecommenderMsg.recommendType[_recommendNum] = _recommendType;
    }

    function calcRecommenderComputingPower(address _sender, uint256 _amount, uint256 timestamp, uint256 fee) public { 
        ...
        while (recommender!= address(0x0)) {
            if(i == 1){
                percentAmount = percent*20;
                addComputingPower(recommender, percentAmount, timestamp);
                userBasicMsg[recommender].dynamicComputingPower = userBasicMsg[recommender].dynamicComputingPower.add(percentAmount);
                recordUserRecommenderMsg(recommender, _sender, percentAmount, 1);
                recommender = calcTeamComputingPower(recommender, 1, percentAmount, 1,timestamp);
                if(fee> 0 && merchantMessage.queryMerchantSta(_sender)){
                    userBasicMsg[recommender].staticComputingPower = userBasicMsg[recommender].staticComputingPower.add(fee);
                    addComputingPower(recommender, fee, timestamp);
                }
            }else if(i == 2){
                percentAmount = percent*10;
                addComputingPower(recommender, percentAmount, timestamp);
                userBasicMsg[recommender].dynamicComputingPower += percentAmount;
                recordUserRecommenderMsg(recommender, _sender, percentAmount, 2);
                recommender = calcTeamComputingPower(recommender, 1, percentAmount, 1,timestamp);
            }else if(i == 3){
                percentAmount = percent*10;
                recommender = calcTeamComputingPower(recommender, 1, percentAmount, 1,timestamp);
                ...
                recommender = calcTeamComputingPower(recommender, 2, percentAmount, 2,timestamp);
            }else if(i == 5){
                percentAmount = percent*5;
                recommender = calcTeamComputingPower(recommender, 2, percentAmount, 2,timestamp);
            }else if(i == 6){
                percentAmount = percent*3;
                recommender = calcTeamComputingPower(recommender, 3, percentAmount, 3,timestamp);
            }else if(i == 7){
                percentAmount = percent*2;
                recommender = calcTeamComputingPower(recommender, 3, percentAmount, 3,timestamp);
            }else if(i == 8){
                percentAmount = percent*2;
                recommender = calcTeamComputingPower(recommender, 4, percentAmount, 4,timestamp);
            }else if(i == 9){
                percentAmount = percent*5;
                recommender = calcTeamComputingPower(recommender, 4, percentAmount, 4,timestamp);
            }else if(i == 10){
                percentAmount = percent*10;
                recommender = calcTeamComputingPower(recommender, 5, percentAmount, 5,timestamp);
            }else if(i ==11){
                percentAmount = percent*15;
                recommender = calcTeamComputingPower(recommender, 5, percentAmount, 4,timestamp);
            }else if(i >11){
                percentAmount = percent*2;
                recommender = calcTeamComputingPower(recommender, 5, percentAmount, 4,timestamp);
            }
    ...
    function calcTeamComputingPower(
        address _recommender, 
        uint256 _memberLevel, 
        uint256 _percent, 
        uint256 _recommendMemberNum,
        uint256 timestamp
    ) internal returns(address){ 
        if(userBasicMsg[_recommender].expiration > block.timestamp && 
           userBasicMsg[_recommender].memberLevel >= _memberLevel && 
           userBasicMsg[_recommender].recommendMemberNum > _recommendMemberNum
           ){
            addComputingPower(_recommender, _percent, timestamp);
            userBasicMsg[_recommender].teamDynamicComputingPower = userBasicMsg[_recommender].teamDynamicComputingPower.add(_percent);
        }
        return userBasicMsg[_recommender].recommender;
    }
    ...
    function calcAwardData(address _sender, uint256 timestamp) public {   
        uint256 _rewardMsgNum = rewardMsgNum;
        if(_rewardMsgNum > 0){
    ...
    {   
        ...
        address _fundAddr = file.queryStrAddr("FundAddr");
        uint256 fee = _amount/50;
        uint256 offsetAmount = _amount.sub(fee);
        require( cmmToken.transfer(_fundAddr, fee),"Token transfer failed");
        require( cmmToken.transfer(_sender, offsetAmount),"Token transfer failed");
    }
