...
contract CA2Farm is Ownable{
    ...
    uint public Counter;
    uint public TotalReward = 10000000000000000;//0.01
    address public CA2LPFarm = 0xD63B3e41ebf39D2179503befC0beddF67b2436F8; //test
    ...
    event RewardPaidRestakeCA2LP(address indexed _address, uint indexed _amount);
    ...
    constructor(){
        ...
        // testnet
        PlatformAccount = 0x6d773046FE0BB82F6b3B19C225ff226059bc2a51; //test
        CA2Token = IERC20(0xCC037dF86c9CbFAd0c8B015c8BDE00acb298Ea03); //test
        CA2LPToken = IERC20(0xAd8CF0eA95bF0310C2D415ab8089e9Fb7551624b); //test
        BUSD = IERC20(0xb0FBeA8afe2b7c9B69c6aa79350d4F36E5f38E41); //test
        WBNB = IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); //test
    ...
    function Deposit(uint amount) public checkPhase{
        ...
        Insert(msg.sender, amount, false);
    }

    function OwnerDeposit(uint amount, address _userAddress) public checkPhase onlyOwner{
        CA2Token.safeTransferFrom(msg.sender, address(this), amount);
        InsertOwner(_userAddress, amount, false);
    }
    
    function Insert(address _userAddress, uint amount, bool _restake) internal{
        
        UserPoolStakeAmount[_userAddress][PoolId] = UserPoolStakeAmount[_userAddress][PoolId].add(amount);
        UserLastStakePool[_userAddress] = PoolId;
        PoolStaked[PoolId] = PoolStaked[PoolId].add(amount); 
        if(PoolFirstStake[_userAddress][PoolId] == false){
            PoolFirstStake[_userAddress][PoolId] = true;
            Counter = Counter + 1;
            PoolSubIdCounter[PoolId] = PoolSubIdCounter[PoolId] + 1;
            PoolSubIdUser[PoolId][PoolSubIdCounter[PoolId]] = _userAddress;
            UserPoolSubId[_userAddress][PoolId] = PoolSubIdCounter[PoolId];
        }
        if(!_restake){
            emit InsertUser(_userAddress, PoolId, amount);
        }
    }

    function InsertOwner(address _userAddress, uint amount, bool _restake) internal{
    ...
    function ExitOwner(address _userAddress) public checkPhase onlyOwner{
        require(UserPoolStakeAmount[_userAddress][PoolId] > 0, "No CA2Token to withdraw in this pool");
        CA2Token.safeTransfer(msg.sender, UserPoolStakeAmount[_userAddress][PoolId]);
        PoolStaked[PoolId] = PoolStaked[PoolId] - UserPoolStakeAmount[_userAddress][PoolId];
        emit WithdrawUser(_userAddress, PoolId, UserPoolStakeAmount[_userAddress][PoolId]);
        UserPoolStakeAmount[_userAddress][PoolId] = 0;
        if(Counter > 2){
            address lastUser = PoolSubIdUser[PoolId][PoolSubIdCounter[PoolId]];
            uint exitSubId = UserPoolSubId[_userAddress][PoolId];
            PoolSubIdUser[PoolId][PoolSubIdCounter[PoolId]] = address(0);
            UserPoolSubId[lastUser][PoolId] = 0;
            PoolSubIdUser[PoolId][exitSubId] = lastUser;
            UserPoolSubId[lastUser][PoolId] = exitSubId;
            ...
            PoolSubIdCounter[PoolId] = PoolSubIdCounter[PoolId] -1;
            PoolSubIdUser[PoolId][UserPoolSubId[_userAddress][PoolId]] = address(0);
            UserPoolSubId[_userAddress][PoolId] = 0;
        }
    
        PoolFirstStake[_userAddress][PoolId] = false;
        Counter = Counter - 1;
    }

    function OwnerDisributeReward(uint _poolId, uint _amount) external onlyOwner checkPhase(){
        ...
        if(RewardAmountCal[_poolId] == 0){
            RewardAmountCal[_poolId] = TotalReward.mul(1e8).div(PoolStaked[_poolId]); //test
        }

        for(uint i = 0; i <= _amount; i++){
            address user = PoolSubIdUser[_poolId][i];
            if(UserPoolStakeAmount[user][_poolId] > 0){
                UserRewards[user] = UserRewards[user].add(RewardAmountCal[_poolId].mul(UserPoolStakeAmount[user][_poolId]).div(1e8));
                emit RewardDistributed(_poolId, user, RewardAmountCal[_poolId].mul(UserPoolStakeAmount[user][_poolId]).div(1e8)); //test
            }
    ...
    function ReadyDistribute() view external onlyOwner returns(bool){
        if(PoolId == 1){
            if(PoolRewardDistributed[PoolId] == true){
                return false;
            }
            if(block.timestamp > PoolEndTime[PoolId]){
                return true;
            ...
            if(PoolRewardDistributed[PoolId.sub(1)] == true){
                return false;
            }
            if(block.timestamp > PoolEndTime[PoolId.sub(1)]){
                return true;
            }
        }
        return false;
    ...
    function Distribute() external onlyOwner checkPhase{
        uint _poolId = PoolId.sub(1);
        require(PoolLock[_poolId] == true, "Period not end");
        require(PoolRewardDistributed[_poolId] == false, "Pool reward distributed");
    ...
    function Claim() public checkPhase{
        require(UserRewards[msg.sender] > 0, "No reward to claim");
        BUSD.safeTransfer(msg.sender, UserRewards[msg.sender]);
        emit RewardPaid(msg.sender, UserRewards[msg.sender]);
        UserRewards[msg.sender] = 0;
        if(UserPoolStakeAmount[msg.sender][PoolId - 1] > 0){
            Insert(msg.sender, UserPoolStakeAmount[msg.sender][PoolId - 1], true);
            UserPoolStakeAmount[msg.sender][PoolId - 1] = 0;
        }
        else{
            Insert(msg.sender, UserPoolStakeAmount[msg.sender][UserLastStakePool[msg.sender]], true);
            UserPoolStakeAmount[msg.sender][UserLastStakePool[msg.sender]] = 0;
        }
        UserRewards[msg.sender] = 0;
    }

    function OwnerClaim(address _userAddress) public onlyOwner checkPhase{
        require(UserRewards[_userAddress] > 0, "No reward to claim");
        BUSD.safeTransfer(msg.sender, UserRewards[_userAddress]);
        emit RewardPaid(_userAddress, UserRewards[_userAddress]);
        UserRewards[_userAddress] = 0;
        if(UserPoolStakeAmount[_userAddress][PoolId - 1] > 0){
            Insert(_userAddress, UserPoolStakeAmount[_userAddress][PoolId - 1], true);
            UserPoolStakeAmount[_userAddress][PoolId - 1] = 0;
        }
        else{
            Insert(_userAddress, UserPoolStakeAmount[_userAddress][UserLastStakePool[_userAddress]], true);
            UserPoolStakeAmount[_userAddress][UserLastStakePool[_userAddress]] = 0;
        }
        UserRewards[_userAddress] = 0;
    }
    ...
    function Restake() external checkPhase{
        require(UserRewards[msg.sender] > 0, "No reward to claim");
        ...
        Insert(msg.sender, CA2UserReward, true);
    ...
    function OwnerRestake(address _userAddress) external onlyOwner checkPhase{
        require(UserRewards[_userAddress] > 0, "No reward to claim");
        uint balanceInContrat = CA2Token.balanceOf(address(this));
        convertPancake(UserRewards[_userAddress], address(BUSD), address(CA2Token));
        uint CA2UserReward = CA2Token.balanceOf(address(this)).sub(balanceInContrat);

        Insert(_userAddress, CA2UserReward, true);
        emit RewardPaidRestake(_userAddress, CA2UserReward);
        UserRewards[_userAddress] = 0;
        if(UserPoolStakeAmount[_userAddress][PoolId - 1] > 0){
            Insert(_userAddress, UserPoolStakeAmount[_userAddress][PoolId - 1], true);
            UserPoolStakeAmount[_userAddress][PoolId - 1] = 0;
    ...
    function RestakeCA2LP() external checkPhase{
        require(UserRewards[msg.sender] > 0, "No reward to claim");
        uint balanceInContrat = CA2Token.balanceOf(address(this));
        uint half = UserRewards[msg.sender].div(2);
        ...
        if (token0Amt > 0 && token1Amt > 0) {
                ...
                IERC20(CA2Token).safeApprove(pancakerouter, 115792089237316195423570985008687907853269984665640564039457584007913129639935);
                IERC20(WBNB).safeApprove(pancakerouter, 0);
                IERC20(WBNB).safeApprove(pancakerouter, 115792089237316195423570985008687907853269984665640564039457584007913129639935);
        ...
        emit RewardPaidRestakeCA2LP(msg.sender, CA2LPToken.balanceOf(address(this)));
        UserRewards[msg.sender] = 0;
        ...
        IERC20(CA2LPToken).safeApprove(CA2LPFarm, 115792089237316195423570985008687907853269984665640564039457584007913129639935);
        ICA2LPFarm(CA2LPFarm).stakeFarm(CA2LPToken.balanceOf(address(this)), msg.sender);
    }

    function OwnerRestakeCA2LP(address _userAddress) external onlyOwner checkPhase{
        require(UserRewards[_userAddress] > 0, "No reward to claim");
        uint balanceInContrat = CA2Token.balanceOf(address(this));
        uint half = UserRewards[_userAddress].div(2);
        convertPancake(half, address(BUSD), address(CA2Token));
        convertPancake(half, address(BUSD), address(WBNB));
        uint CA2UserReward = CA2Token.balanceOf(address(this)).sub(balanceInContrat);
        ...
        emit RewardPaidRestakeCA2LP(_userAddress, CA2LPToken.balanceOf(address(this)));
        UserRewards[_userAddress] = 0;
        ...
        IERC20(CA2LPToken).safeApprove(CA2LPFarm, 0);
        IERC20(CA2LPToken).safeApprove(CA2LPFarm, 115792089237316195423570985008687907853269984665640564039457584007913129639935);
        ICA2LPFarm(CA2LPFarm).stakeFarm(CA2LPToken.balanceOf(address(this)), _userAddress);
    }

   

    function convertPancake(uint _amount, address _tokenin, address _tokenout) internal { //test
    ...
    function setCA2LPFarm(address _CA2Token, address _CA2LPToken, address _CA2LPFarm) external onlyOwner{
