...
contract ABep20Interface is ABep20Storage {
    ...
    function _setStrategyVault(address newStrategyVault) public;
    function _transferLiquidityToStrategyVault(uint256 underlyingAmount) external;
}
...
contract AStrategyVaultStorage {
    /**
     * @notice Administrator for the strategy vault contract
     */
    address public strategyVaultAdmin;

    /**
     * @notice Strategy vault contract
     */
    address public strategyVault;
}

contract AStrategyVaultInterface is AStrategyVaultStorage {
    /**
     * @notice New Strategy Vault Admin
     */
    event NewStrategyVaultAdmin(address oldAdmin, address newAdmin);
    
    /**
     * @notice New Strategy Vault
     */
    event NewStrategyVault(address oldStrategyVault, address strategyVault);
    
    /**
     * @notice New Strategy Vault
     */
    event LiquidityToStrategyVaultTransfered(address strategyVault, uint256 amount);
}
 ...
 * @author Atlantis
 */
contract ABep20 is AToken, ABep20Interface, AStrategyVaultInterface {
    /**
    ...
    function _setStrategyVault(address newStrategyVault) public {
        require(msg.sender == admin, "only admin can set strategy vault");

        address oldStrategyVault = strategyVault;
        strategyVault = newStrategyVault;

        emit NewStrategyVault(oldStrategyVault, strategyVault);
    }

    function _transferLiquidityToStrategyVault(uint256 underlyingAmount) external {
        require(strategyVault != address(0), "Address cannot be Zero");
        require(msg.sender == strategyVault, "only strategy vault proxy can transfer funds");
        require(underlyingAmount > 0, "Withdraw amount must be non-zero");
        require(underlyingAmount <= getCashPrior(), "Withdraw amount exceeding the current balance");

        _transferToStrategyVaultInternal(strategyVault, underlying, underlyingAmount);
    }

    function _transferToStrategyVaultInternal(address strategyVault, address underlying, uint256 underlyingAmount) internal {
        uint error = redeemInternal(underlyingAmount);
        require(error == uint(Error.NO_ERROR), "Cannot tranfer liquidity to strategy vault");

        emit LiquidityToStrategyVaultTransfered(strategyVault, underlyingAmount);
    }
...
// File contracts/ABep20Delegate.sol
