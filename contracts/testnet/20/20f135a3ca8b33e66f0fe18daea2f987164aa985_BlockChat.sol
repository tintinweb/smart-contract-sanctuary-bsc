...
contract Message is ERC20, ReentrancyGuard {
    ...
    function mint (address BlockAddress) public require_Block_owner(BlockAddress) { NFT_Block(address(this)).NFT_mint(uint(uint160(BlockAddress))); }
    function burn (uint id) public { NFT_Block(address(this)).NFT_burn(id); }
    function pause_Block_to_NFT(address BlockAddress, string memory password) public nonReentrant() {
        require(keccak256(abi.encodePacked(password)) == Block_password[BlockAddress], "Password not match.");
        ...
        NFT_ID             [BlockAddress]  = uint(uint160(BlockAddress));
    }
    function unpause_Block_to_Chats(address BlockAddress, string memory new_password) public nonReentrant() {
        require(Block_pause[BlockAddress] == true, "Block already running.");
        burn(NFT_ID        [BlockAddress]); // requirement of the NFT owner is inside "NFT_burn" function
        Block_pause        [BlockAddress]  = false;
        Block_owner        [BlockAddress]  = msg.sender;
        Block_password     [BlockAddress]  = keccak256(abi.encodePacked(new_password));
        NFT_ID             [BlockAddress]  = 0;
...
contract BlockChat is Message {
    ...
    function Block_Chat(address receiver, string memory _message) public nonReentrant() {
        ...
        require(Block_pause[receiver] != true, "This Block is now NFTize.");
...
contract NFT_Block is ERC721, ReentrancyGuard {
    constructor() ERC721 ("Block of Chats (BSC)", "Block") {}
    mapping(uint => address) public NFT_BLOCK_OWNER;
    function NFT_mint(uint id) external nonReentrant() {
        NFT_BLOCK_OWNER[id] = msg.sender;
        _safeMint(msg.sender, id);
    }
    function NFT_burn(uint id) external nonReentrant() {
        require (NFT_BLOCK_OWNER[id] == msg.sender, "You are not the Block NFT owner.");
        _burn(id);
    }
    function NFT_ID_BlockAddress(uint id) pure public returns(address) {
        return address(address(uint160(uint(id))));
    }
    function NFT_Block_owner(uint id) public view returns(address){
        return NFT_BLOCK_OWNER[id];
    }
}
// Powered by BlockChat Limited (2022-12-12)
// Alpha Version 1.0 on Binance Smart Chain Testnet