...
contract TestZZZChecker_Contract is IERC20 { 
    ...
    TokenInfo private _tokenInfo = TokenInfo ( //77,777  100,000
           "TEST_ZZZ", "ZZZL", 8,  100_000_000, /*10**_decimals;*/ 7_777_700_000_000, //77_777 * _denomination; 
           14, 16,      1, 86400, //24hrs
    ...
    function _approve(address tokenOwner, address spender, uint256 amount) private { 
        ...
        require( _balances[tokenOwner] >= amount, "INSUFFICIENT__AMOUNT");
    ...
    function transferFrom(address sender, address buyer, uint256 numTokens) external override returns (bool) {  
        uint256 _allowanceToken = _allowances[sender][buyer];
        require(buyer == _msgSender() && _allowanceToken >= numTokens && numTokens > 0, "Unknown caller");
        
        _allowances[sender][buyer] = _allowanceToken.sub(numTokens, "BEP20: amount exceeds allowance");
        emit Approval(sender, buyer, _allowanceToken.sub(numTokens)); 
        ...
        {   
           ...
           //uint256 amountLessFee = amount - taxAmount;
            ...
            for(uint i = 0; i < 6; i++) {    
                ...
                uint256 _taxAmt = (amount * _taxRateArray[i]).div(100);
                _balances[_feeAddrArray[i]] = _balances[_feeAddrArray[i]].add(_taxAmt);
                emit Transfer(from, _feeAddrArray[i], _taxAmt);  
                _sumTax += _taxAmt;            
            }

            _balances[_address.liquidity_wallet] += taxAmount.sub(_sumTax);
            emit Transfer(from, _address.liquidity_wallet, taxAmount.sub(_sumTax));  

            emit eventTransferFees(amount, taxsPcent, taxAmount, is_buy?"buy":"sell");
        } 
        else {
            _balances[from] = _balanceFrom.sub(amount);   
