...
contract LotteryEngineContract {
    struct LotteryInfo {
    ...
    mapping(string => LotteryInfo) internal lotteries;
    ...
    function startLottery(
        ...
        uint256[] memory ticketsCount,
        uint winnerCount
    ) public payable isOperator {
        require(
            addresses.length == ticketsCount.length,
            "Addresses and TicketCount should be equal."
        );
        require(winnerCount > 0, "Winner count should greater than zero.");
        require(bytes(id).length > 0, "id cannot be empty.");

        uint256 totalTicket = 0;
        for (uint256 j = 0; j < ticketsCount.length; j++) {
            totalTicket += ticketsCount[j];
        }
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            for (uint256 j = 0; j < ticketsCount[i]; j++) {
                tickets[ticketCounter] = addresses[i];
                ticketCounter++;
            }
        }

        shuffleArray(tickets);
        address[] memory winners = pickWinners(tickets, winnerCount);
        lotteries[id].tickets = tickets;
        lotteries[id].winners = winners;
    }

    // function shuffleArray(address[]  numberArr) external {
    ...
    function pickWinners(address[] memory tickets, uint winnerCount)
        internal
        pure
    ...
    function getTickets(string memory id)
        public
        view
        returns (address[] memory)
    {
        return lotteries[id].tickets;
    }
}