...
import "./transferFee/MintingFee.sol";
...
contract FuseNFT is MintingFee {
    ...
    modifier sameSpecies(uint256[] memory tokenIds) {
        (uint8 rawRanking, uint8 rawLevel) = Metadata(rawContract).species(tokenIds[0]);
        for (uint256 i = 1; i < tokenIds.length; i++) {
            (uint8 ranking, uint8 level) = Metadata(rawContract).species(tokenIds[i]);
    ...
    function fuseNfts(uint256[] memory tokenIds) public validLength(tokenIds) sameSpecies(tokenIds) {
        require(tokenIds.length == threshold, "Insufficient-NFTs!");

        (uint8 rawRanking, uint8 rawLevel) = Metadata(rawContract).species(tokenIds[0]);
        uint8 newRanking = upgradeRanking(rawRanking);
        
        for (uint256 i = 0; i < tokenIds.length; i++) {
            // IERC721(rawContract).safeTransferFrom(msg.sender, address(this), tokenIds[i]);
            ERC721Burnable(rawContract).burn(tokenIds[i]);
        }
        feeToken.burn(msg.sender, feeAmount);
    ...
    function harvest(uint256 fusionId) public {
        ...
        user.fusionIds.pop();
        Metadata(rawContract).batchFusion(fusion.rank, fusion.level, 1);
    }
    ...
    function setRawContract(address _rawContract) public onlyRole(ADMIN_ROLE) {
        rawContract = _rawContract;
    }

    function setThreshHold(uint8 _threshold) public onlyRole(ADMIN_ROLE) {
...
import "../access/AccessControl.sol";
import "../token/ERC20/IERC20Burnable.sol";

contract MintingFee is AccessControl {
