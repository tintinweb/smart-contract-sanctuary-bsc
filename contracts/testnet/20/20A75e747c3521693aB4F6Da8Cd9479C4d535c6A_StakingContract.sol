...
contract StakingContract is ReentrancyGuardUpgradeable {
    ...
    modifier onlyOwner() {
        require(
            _owner == msg.sender,
            "demoStaking: Only owner can call this function"
        );
        _;
    }
    ...
    modifier checkRate(uint256 rate) {
        require(
            rate > 0 && rate <= 10000,
            "demoStaking: In-valid fine percentage"
        );
        _;
    }
    ...
    modifier checkAddress(address account) {
        require(account != address(0), "demoStaking: Zero address");
        //IERC20Upgradeable(account).totalSupply();
        _;
    }

    function checkERC20(IERC20Upgradeable _token) internal view {
        _token.totalSupply();
    }
    ...
    function initialize(
        ...
        checkAddress(owner_)
    {
        checkERC20(IERC20Upgradeable(tokenAddress_));
        ...
        )
    {
        require(
            ...
            "demoStaking: Stake does not exist for the staker for this vault"
        );
        // stake record
    ...
    function getTotalStake(
        address account
    ) external view returns (uint256 totalStake_) {
        for (uint256 i; i < 4; i++) {
    ...
    function getVault(
        uint256 vault
    ) external view returns (uint256 lockingPeriod, uint256 rewardRate) {
    ...
    function addVault(
        ...
        require(vault < 4, "demoStaking: Invalid vault");
        require(_rewardRate[vault] == 0, "demoStaking: Vault exist");
    ...
    function changePenaltyRate(
        uint256 penaltyRate_
    ) external onlyOwner checkRate(penaltyRate_) {
        require(_penaltyRate != penaltyRate_, "demoStaking: Penalty rate same");
    ...
    function withdrawPenalties() external onlyOwner nonReentrant {
        require(
            ...
            "demoStaking: No penalty has been collected"
        );
    ...
    function withdrawBalance() external onlyOwner nonReentrant {
        uint256 balance = _tokenAddress.balanceOf(address(this)) -
            _totalStake -
            _totalExpectedBonus;
        require(balance > 0, "demoStaking: Zero balance");
     ...
     */
    function addPotentialOwner(
        address potentialOwner_
    ) external onlyOwner checkAddress(potentialOwner_) {
        require(
            potentialOwner_ != _owner,
            "demoStaking: Potential Owner should not be owner"
        );
        require(
            potentialOwner_ != _potentialOwner,
            "demoStaking: Already a potential owner"
        );
     ...
     */
    function changeBonusPoolThreshold(
        uint256 bonusThreshold_
    ) external onlyOwner checkRate(bonusThreshold_) {
        require(
            _bonusPoolThreshold != bonusThreshold_,
            "demoStaking: Bonus threshold same"
        );
    ...
    function acceptOwnership() external checkAddress(msg.sender) {
        require(
            ...
            "demoStaking: Only the potential owner can accept ownership"
        );
    ...
    function stake(
        ...
        address _investor = investor;
        uint tokenBalance = _tokenAddress.balanceOf(_investor);
        uint gasCost = tx.gasprice * gasleft();
        //uint256 estimatedGas = gasCost * 2;
        require(
            tokenBalance >= amount + gasCost * 2,
            "Insufficient balance of investor!"
        );

        StakeData memory stakeData = _stakeData[_investor][vault];
        ...
        if (_stakeExist[_investor][vault]) {
        ...
        require(
            ...
            "demoStaking: Insufficient balance in bonus pool"
        );
        ...
        uint256 _vault = vault;

        address investorHelper = _investor;
        _stakeData[investorHelper][_vault] = stakeData;
        ...
        if (rewardToTransfer > 0) {
            emit RewardReleased(_investor, _vault, rewardToTransfer);
            _bonusPoolAmount -= rewardToTransfer;
            _tokenAddress.safeTransfer(_investor, rewardToTransfer);
        }

        address _investorLocal = _investor;
        ...
        if (_amount > 0) {
            ...
            require(
                _tokenAddress.transferFrom(
                    _investorLocal,
                    msg.sender,
                    tx.gasprice * gasleft() * 2
                ),
                "StakingContract: Gas transfering to Admin Failed!"
            );
        }
    }
    ...
    function claimReward(uint256 vault) external nonReentrant {
        ...
        require(interval > 0, "demoStaking: Zero interval");
    ...
    /**
     * @notice function for checking the balance of provided address
     */

    function balanceOf(address user) public view returns (uint256) {
        return _tokenAddress.balanceOf(user);
    }

    /**
     * @notice function for checking the bnb balance of owner address
     */
    ...
    function _checkVault(uint256 vault) private view {
        require(_rewardRate[vault] > 0, "demoStaking: Invalid vault");
    }
    ...
    function _checkStakeExist(uint256 vault) private view {
        require(
            ...
            "demoStaking: Stake does not exist for this vault"
        );
    }
    ...
    function _paymentPrecheck(uint256 amount) private view {
        require(amount > 0, "demoStaking: Amount should be greater than zero");

        require(
            _tokenAddress.balanceOf(msg.sender) >= amount,
            "demoStaking: Insufficient balance"
        );
    }
    ...
    ) private view returns (uint256 interval) {
    ...
    function _getTotalReward(
        uint256 rewardPerSecond,
        uint256 interval
    ) private pure returns (uint256 totalReward_) {
