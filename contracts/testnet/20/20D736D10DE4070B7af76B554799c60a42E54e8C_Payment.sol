...
contract Payment is Ownable {
    ...
    event PurchaseByToken(string affCode, uint256 amount, address token, uint256 purchaseTime);
    event PurchaseByCurrency(string affCode, uint256 amount, uint256 purchaseTime);
    event WithdrawContractToken(address token, address receiver);
    event WithdrawContractCurrency(address receiver);

    constructor() {
        adminAddress = msg.sender;
    }

    function purchaseByToken(string memory _affCode, address _token, uint256 _amount) external {
    ...
    function purchaseByCurrency(string memory _affCode) external payable {
    ...
    function withdrawContractToken(address token) external onlyOwner {
        ...
        emit WithdrawContractToken(token, adminAddress);
    }

    function withdrawContractCurrency() external onlyOwner {
        require(address(this).balance > 0, "Insufficient currency balance");
        payable(adminAddress).transfer(address(this).balance);

        emit WithdrawContractCurrency(adminAddress);
    }
}
