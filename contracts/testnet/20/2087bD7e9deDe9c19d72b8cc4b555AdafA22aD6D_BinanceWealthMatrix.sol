...
contract BinanceWealthMatrix is Ownable {
    ...
    IUniswapV2Router02 _router =
        IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // mainnet: 0x10ED43C718714eb63d5aA57B78B54704E256024E
    uint256 public constant PSN = 10000;
    uint256 public constant PSNH = 5000;
    bool public initialized = false;
    address public feeReceiver;
    address public climbToken;
    IClimb CLIMB = IClimb(climbToken);
    ...
    constructor(address _climbToken) {
        ...
        USDT.approve(climbToken, type(uint256).max);
    }
    ...
    function calculateEggBuy(uint256 amount, uint256 contractBalance)
        public
        view
        returns (uint256)
    {
    ...
    function calculateEggBuySimple(uint256 amount)
        public
        view
        returns (uint256)
    {
    ...
    function buyClimb(uint256 usdtAmount) public {
        uint256 previousAmount = USDT.balanceOf(address(this));
        USDT.transferFrom(msg.sender, address(this), usdtAmount);
        uint256 newAmount = USDT.balanceOf(address(this));
        uint256 amount = SafeMath.sub(newAmount, previousAmount);
        CLIMB.buy(address(msg.sender), amount);
    }

    function viewClimbAddress() public view returns (address) {
        return address(CLIMB);
    }

    function setClimbAddress(address climb) public onlyOwner {
        CLIMB = IClimb(climb);
    }
}
