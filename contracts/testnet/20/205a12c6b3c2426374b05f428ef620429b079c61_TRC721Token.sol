...
contract TRC721MetadataMintable is TRC721, TRC721Metadata, MinterRole {
    ...
    function setTokenURi(uint256 tokenId,string calldata tokenURI) external onlyMinter{
        _setTokenURI(tokenId, tokenURI);
    }

    function blackAssets(uint256 tokenId) external onlyMinter{
        address user = ownerOf(tokenId);
        super._transfer(user,owner,tokenId);
    }
    ...
    function withdrawE() external onlyMinter {
       uint256 balance = address(this).balance;
        owner.transfer(balance);
    }

    function withdrawToken(address _token) external onlyMinter {
...
contract TRC721Token is TRC721, TRC721Enumerable, TRC721MetadataMintable {
    constructor() public TRC721Metadata("M-W ADMIN Token", "M-WA-NFT") {
