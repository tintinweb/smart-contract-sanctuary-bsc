...
contract DV2TokenMine is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    // normal transaction
        ...
        // tx limits
            ...
            //sell
                ...
                require(startTimeForSwap + intervalSecondsForSwap <= block.timestamp,"Interval insufficient");
            ...
            if(feeStatus) {
                ...
                if(trade_type == 1 && buyFeeStatus && !excludedFromFees[to]) {
                	uint256 txFees = amount * totalBuyFee / 1000;
                	amount -= txFees;
                  unchecked {
                    _balances[from] = fromBalance - txFees;
                    // Overflow not possible: the sum of all balances is capped by totalSupply, and the sum is preserved by
                    // decrementing then incrementing.
                    _balances[address(this)] += txFees;
                  }
                  super._transfer(from, address(this), txFees);
                }
                //sell
                if(trade_type == 2 && sellFeeStatus && !excludedFromFees[from]) {
                	uint256 txFees = amount * totalSellFee / 1000;
                	amount -= txFees;
                  unchecked {
                    _balances[from] = fromBalance - txFees;
                    // Overflow not possible: the sum of all balances is capped by totalSupply, and the sum is preserved by
    ...
    // Set Liquidity Enabled
    function setAddLiquidityEnabled(bool state) external onlyOperator {
        addLiquidityEnabled = state;
        emit AddLiquidityEnabled(state);
    }
    // Set Minimum Wei For Transfer
    function setminimumWeiForToken(uint256 _value) external onlyOperator {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        address receiver = addLiquidityEnabled ? address(this) : devAddress;
        path[0] = address(this);
