...
contract KapexPresaleClaim {
  ...
  function getAvailableTokenToClaim(address _address) public view returns (uint256) {
    uint256 totalKapex = calculateBNBToPresaleToken(_address, getTotalBought(_address));
    return ((totalKapex * getTotalClaimPercentage()) / feeDenominator) - totalClaim[_address];
  }

  function getTotalClaimPercentage() private view returns (uint256) {
    uint256 currentClaimDate = getCurrentClaimDate();

    (, , uint256 day, uint256 hour, , ) = BokkyPooBahsDateTimeLibrary.timestampToDateTime(currentClaimDate);
    uint256 interval = BokkyPooBahsDateTimeLibrary.diffMonths(kapexPresale.getStartDateClaim(), currentClaimDate);
