...
contract MountSapphire is Pausable, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    constructor(
        ...
        require(_blucContract != address(0), "S_MTS_0103");
    ...
    function _updateTokenRewardPerDay() internal {
        ...
        for (uint256 index = 0; index < powerList.length; index++) {
            if (totalPower <= powerList[index]) {
                rewardToken.rewardPerDay = tokenPerDayList[index];
                return;
            }
        }
        rewardToken.rewardPerDay = tokenPerDayList[tokenPerDayList.length - 1];
    }
    ...
    function getRewardToken() external view returns (uint256) {
        return rewardToken.rewardPerDay;
    }
    ...
    function getPendingReward(address _user) external view returns (uint256) {
        ...
        if (
            isRewardEmpty ||
            totalPowerInPool == 0 ||
            userInfo[_user].finalPower == 0
        ) {
            return rewardDebt[_user];
        }
        uint256 userWeight = getUserWeight(_user);
        uint256 rewardPerSecond = rewardToken.rewardPerDay.div(SECONDS_PER_DAY);
        uint256 newReward = rewardPerSecond
            .mul(userWeight)
            .div(EXA_CONSTANT)
            .mul(_getDepositTime(_user));
        if (newReward > totalRewardSupply - totalDistributedReward) {
            newReward = totalRewardSupply - totalDistributedReward;
        }
        return newReward.add(rewardDebt[_user]);
    }
    ...
    function updateRewardDebtAllUser() public whenNotPaused {
    ...
    function _updateRewardDebt(address _user) internal {
        ...
        totalDistributedReward = totalDistributedReward.add(newReward);
        rewardDebt[_user] = rewardDebt[_user].add(newReward);
    ...
    function getUserWeight(address _user) public view returns (uint256) {
        uint256 power = userInfo[_user].finalPower * EXA_CONSTANT;
    ...
    function stake(uint256[] memory _blucamonTokenIds) external whenNotPaused {
        ...
        totalPowerInPool = totalPowerInPool.add(power);
        _updateTokenRewardPerDay();
        ...
        require(transferResult, "S_MTS_1200");
        ...
        require(transferResult, "S_MTS_1300");
        emit StakeTokens(
            msg.sender,
    ...
    function emergencyUnstake() external whenNotPaused {
        ...
        for (uint256 i = 0; i < blucamonTokenIds.length; i++) {
            ...
            totalPowerInPool = totalPowerInPool.sub(
                blucamonMapping[blucamonTokenIds[i]].power
            );
        }
        ...
        returns (uint256)
    {
        ...
        uint256[] memory rarities = new uint256[](7);
        uint256[] memory gods = new uint256[](6);
        ...
        for (uint256 i = 0; i < _blucamonTokenIds.length; i++) {
            ...
            gods[blucamon.god] = gods[blucamon.god].add(1);
            if (gods[blucamon.god] == 3) {
                totalPercent = totalPercent.add(50);
            } else if (gods[blucamon.god] == 5) {
                totalPercent = totalPercent.add(50);
            } else if (gods[blucamon.god] == 10) {
                totalPercent = totalPercent.add(1_00);
            }

            elements[blucamon.element1] = elements[blucamon.element1].add(1);
            ...
            elements[blucamon.element2] = elements[blucamon.element2].add(1);
            ...
            elements[blucamon.element3] = elements[blucamon.element3].add(1);
