...
import "./ITITIFUND.sol";
import "./IInvest.sol";


contract TITIFUND is Ownable, Pausable, ITITIFUND {
    ...
    constructor () {
        ...
        USDT=IERC20(0x219a0CD6263fE0BBff0f30ED0d7F99ddea426649);
        update();
    }

    uint256 Price_USDT=36900000;
    uint256 Fix = 10;
    uint256 Count;
    

    function Exchange_NFTW (uint256 ID_Collections, uint256 ID_NFTW) public {
        require(Count+1<=Fix);
        IERC721 NFTW;
        NFTW =IERC721(Coll.GetID_Collection(ID_Collections));
        require(NFTW.ownerOf(ID_NFTW)==msg.sender);
        require(Coll.Get_What_Collection(ID_Collections)==3);
        USDT.transfer(msg.sender, Price_USDT);
        NFTW.safeTransferFrom(msg.sender,Contr.getStock(), ID_NFTW, "");
        Count++;
    }

    function send_to_investment () public onlyOwner {
        require(IN.get_Invet_True()==true);
        uint256 Balance = USDT.balanceOf(address(this));
        uint256 _40p = Balance/100*40;
        USDT.transfer(Contr.getInvest(), _40p);
        IN.OFF_Invet_True();
    }

    //Set_____________________________________
    function set_Count () public onlyOwner {
        Count=0;
    }

    function set_Price_USDT (uint256 new_Price_USDT) public onlyOwner {
        Price_USDT=new_Price_USDT;
    }

    function set_Fix (uint256 new_Fix) public onlyOwner {
        Fix=new_Fix;
    }

    //Get_____________________________________
    function get_Price_USDT() public virtual override view  returns (uint256) {
        return Price_USDT;
    }
    function get_Fix () public virtual override view  returns (uint256){
        return Fix;
    }
    
    function left_to_stop() public virtual override view  returns (uint256){
    return Fix-Count;
    }
    ...
    function  update()  public virtual override{
        Coll=ICollections(Contr.getCollections());
        IN=IInvest(address(Contr.getInvest()));
    } 
}
...
interface ICollections {
    ...
    function getPrimary()  external view returns (bool);
...
interface ITITIFUND { 
    function get_Price_USDT() external view  returns (uint256);
    function get_Fix () external view  returns (uint256);
    function left_to_stop() external view  returns (uint256);
    function  update()  external;
}
...
interface IInvest {
    function get_Invet_True () external view returns (bool);
    function OFF_Invet_True () external;
}
