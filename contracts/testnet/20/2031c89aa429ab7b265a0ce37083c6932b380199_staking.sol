...
contract staking{
    ...
    function stake(uint256 _amount) external{
        ...
        stakes[msg.sender].remainingReward = stakes[msg.sender].amount.div(100).mul(20);
    }
    function calculateReward(address _addr) public view returns(uint256){
        ...
        if(TimeDiff >= totalTime){
            ...
            reward = stakes[_addr].amount.div(100).mul(1);
        }
        else if(TimeDiff < 4 seconds){
            reward = stakes[_addr].amount.div(100).mul(3);
        }
        else if(TimeDiff < 8 seconds){
            reward = stakes[_addr].amount.div(100).mul(7);
        }
        else if(TimeDiff < 13 seconds){
            reward = stakes[_addr].amount.div(100).mul(12);
        }    
        else{
            reward = stakes[_addr].amount.div(100).mul(15);
        }
        return reward;
       }
