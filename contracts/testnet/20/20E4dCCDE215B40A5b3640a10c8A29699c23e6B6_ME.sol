...
contract LastBuy {
    ...
    uint256 public minTokens = 10**9 * 100000;

    function setMinTokens(uint256 amount) external onlyTokenOwner {
        minTokens = amount;
    }

    function setTimespan(uint256 timespan) external onlyTokenOwner {
        Duration = timespan;
    }

    function Play(uint256 tokenAmount, address account) external onlyToken {
        if (getTimeLeft() == 0 && lastBuyer != address(0)) payoutWinner();
        if (tokenAmount < minTokens) return;
        if (lastBuyer == account) return;
    ...
    function getTimeSinceLastBuy() public view returns (uint256) {
        if (LastBuyTimestamp <= block.timestamp) return 0;
        return block.timestamp - LastBuyTimestamp;
    }

    function getTimeLeft() public view returns (uint256) {
        if (block.timestamp <= LastBuyTimestamp) return 0;
        uint256 timeSinceLastBuy = block.timestamp - LastBuyTimestamp;
        if (timeSinceLastBuy >= Duration) return 0;
        return Duration - timeSinceLastBuy;
    }
}

// File: SmartBuyback.sol

pragma solidity ^0.8.4;

contract SmartBuyback {
    address public Token;
    IDEXRouter public router;
    address[] pairPath;
        ...
        returns (uint256 amountIn)
    {
        ...
        if (Balance < 10**9) return 0;
        //Just a rough estimation
        amountIn = router.getAmountsOut(amountOut, pairPath)[1];
        amountIn = (amountIn * 10025) / 10000; //PancakeFee
        if (amountIn > Balance) {
            amountIn = Balance;
        }
    }
    ...
    constructor(address _router) {
        ...
        pairPath[0] = address(msg.sender);
        pairPath[1] = router.WETH();
    }
    ...
    function Buyback(uint256 TokenAmount) external onlyToken {
        uint256 amountIn = getAmountIn(TokenAmount);
        if (amountIn < 10**9) return;
...
contract ME is IBEP20, Ownable {
    ...
    //Token Info
    string private constant _name = "TE";
    string private constant _symbol = "Tearn";
    ...
    uint256 private _buybackTax = 200;
    uint256 private _marketingTax = 100;
    uint256 private _lastBuyTax = 200;
    ...
    constructor() {
        ...
        buyback = new SmartBuyback(PancakeRouter);
        lastBuy = new LastBuy();
    ...
    function _regularTransfer(
        ...
        if (
            ...
            if (AutoBuyback && recipient == _pancakePairAddress)
                _buyback(amount);
        }
        _transferTaxed(sender, recipient, amount, tax);
    }

    function Buyback(uint256 TokenAmount) external onlyTeam {
        _buyback(TokenAmount);
    }

    function _buyback(uint256 amount) private lockTheSwap {
        buyback.Buyback(amount);
    }
    ...
    function DisableAutoPayout(bool disabled) public onlyTeam {
    ...
    function ChangeAutoPayoutCount(uint256 count) public onlyTeam {
    ...
    function swapForAnyToken(uint256 amount, address _token)
        private
        returns (uint256 newAmount)
    {
        ...
        newAmount = IBEP20(_token).balanceOf(address(this));
    }
    ...
    function claimAnyToken(address token) external isInFunction {
        ...
        toBePaid[msg.sender] = 0;
        alreadyPaidShares[msg.sender] = profitPerShare * getShares(msg.sender);
        RewardsToken.transfer(msg.sender, swapForAnyToken(dividents, token));
    }

    bool public defaultAutoPayout = true;
    ...
    function SetAutoBuyback(bool flag) external onlyOwner {
        AutoBuyback = flag;
    }
        ...
        lockTheSwap
    {
        ...
        uint256 lastBuyBNB = (distributeBNB * _lastBuyTax) / totalBNBTax;
        uint256 stakingBNB = distributeBNB -
            marketingBNB -
            buybackBNB -
            lastBuyBNB;
        ...
        (sent, ) = address(lastBuy).call{value: lastBuyBNB}("");
    }
    ...
    function AddOrRemoveAMM(address AMMPairAddress, bool Add) public onlyTeam {
    ...
    function ChangeTeamWallet(address newTeamWallet) public {
    ...
    function SetLiquifyTreshold(uint8 TresholdPermille) public onlyTeam {
    ...
    function SetOverLiquifyTreshold(uint8 TresholdPermille) public onlyTeam {
    ...
    function SwitchSwapAndLiquify(bool disabled) public onlyTeam {
    ...
    function SetExcludedStatus(address account, bool excluded) public onlyTeam {
        require(
    ...
    function BurnContractToken(uint8 percent) public onlyTeam {
        require(percent <= 100);
    ...
    function WithdrawStrandedToken(address strandedToken) public onlyTeam {
        require(
