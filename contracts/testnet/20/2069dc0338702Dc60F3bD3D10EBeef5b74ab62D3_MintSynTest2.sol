...
// File: contracts/8/libraries/Math.sol
...
// File: contracts/8/Interfaces/IPriceOracle.sol
...
// File: contracts/8/Interfaces/IAssetConfig.sol
...
interface IAssetConfig {
    ...
    function getMinCollateralRatio(address asset)
        external
        view
        returns (uint256);
...
// File: contracts/8/Interfaces/ICollateralConfig.sol

pragma solidity ^0.8.9;

interface ICollateralConfig {
    function getAccepCollateral(address collateral)
        external
        view
        returns (bool);
...
// File: contracts/8/Interfaces/ILock.sol
...
// File: contracts/8/Interfaces/IPancakeRouter02.sol
...
// File: contracts/8/Interfaces/IPancakeFactory.sol
...
// File: contracts/8/Interfaces/IPancakePair.sol
...
// File: contracts/8/mocks/ERC20Mock.sol
...
// File: contracts/8/MintSynTest2.sol

pragma solidity ^0.8.9;

contract MintSynTest2 is Ownable {
    /// @notice Libraries
    ...
    function getHealth(uint256 positionId)
        public
        view
        returns (
            uint256 assetValueInCollateral,
        ...
        )
    {
        ...
        uint256 collateralPriceInAsset = (assetPrice.mul(1e18)).div(
            colleteralPrice
        );
        assetValueInCollateral = (pos.mintAmount.mul(collateralPriceInAsset))
            .div(1e18);
    ...
    function openPosition(
        ...
        require(
            collateralConfig.getAccepCollateral(collateral),
            "this collateral not allowed"
        );
        require(
            assetConfig
                .getMinCollateralRatio(asset)
                .mul(collateralConfig.getMultipier(collateral))
                .div(1e5) <= collateralRatio,
            "low collateral ratio than minimum"
        );
        IERC20(collateral).transferFrom(msg.sender, address(this), amount);
        (uint256 colleteralPrice, ) = twap.getPrice(collateral);
        (uint256 assetPrice, ) = twap.getPrice(asset);
        uint256 collateralPriceInAsset = (colleteralPrice.mul(1e18)).div(
            assetPrice
        );
        uint256 mintAmount = (
            (amount.mul(collateralPriceInAsset)).mul(100).div(collateralRatio)
        );
        if (mintAmount == 0) {
    ...
    function withdraw(uint256 positionId, uint256 withdrawAmount) external {
        ...
        uint256 assetValueInCollateral = assetAmount.mul(assetPrice).div(
            colleteralPrice
        );
        uint256 mulipier = collateralConfig.getMultipier(pos.collateral);
        if (
            (
                assetValueInCollateral.mul(mulipier).mul(
                    assetConfig.getMinCollateralRatio(pos.asset)
                )
            ).div(1e25) > collateralAmountAfterSub
        ) {
            revert(
    ...
    function mintAsset(uint256 positionId, uint256 mintAmount) external {
        Position storage pos = positions[positionId];
        require(!pos.closePosition, "position was closed");
        require(msg.sender == pos.owner, "not owner");
        ...
        uint256 assetValueInCollateral = newAssetAmount.mul(assetPrice).div(
            colleteralPrice
        );
        uint256 mulipier = collateralConfig.getMultipier(pos.collateral);
        if (
    ...
    function burnAsset(uint256 positionId, uint256 burnAmount) external {
        Position storage pos = positions[positionId];
        require(!pos.closePosition, "position was closed");
        require(msg.sender == pos.owner, "not owner");
        require(
            pos.mintAmount >= burnAmount,
        ...
        uint256 fee = burnAmount.mul(collateralPriceInAsset).mul(feeRate).div(
            1e22
        );
        if (fee > 0) {
        ...
        ERC20Mock(pos.asset).transferFrom(
            msg.sender,
            address(this),
            burnAmount
        );
    ...
    function auction(uint256 positionId, uint256 burnAmount) external {
        Position storage pos = positions[positionId];
        require(
            pos.mintAmount >= burnAmount,
            "Cannot liquidate more than the position amount"
        );
        (uint256 colleteralPrice, ) = twap.getPrice(pos.collateral);
        (uint256 assetPrice, ) = twap.getPrice(pos.asset);
        uint256 collateralPriceInAsset = (assetPrice.mul(1e18)).div(
        ...
        uint256 assetValueInCollateral = (
            pos.mintAmount.mul(collateralPriceInAsset)
        ).div(1e18);
        if (
            assetValueInCollateral
                .mul(collateralConfig.getMultipier(pos.collateral))
                .mul(assetConfig.getMinCollateralRatio(pos.asset))
                .div(1e25) < pos.collateralAmount
        ) {
            revert("Cannot liquidate a safely collateralized position");
        }
        IERC20(pos.asset).transferFrom(msg.sender, address(this), burnAmount);
        uint256 auctionDiscount = Math.min(
            assetConfig.getAuction(pos.asset).div(100),
            assetConfig.getMinCollateralRatio(pos.asset).div(100).sub(one)
        );
        uint256 discountPrice = collateralPriceInAsset.mul(1e18).div(
            ((one).sub(auctionDiscount))
        );

        uint256 assetValueInCollateralDiscount = pos
            .mintAmount
            .mul(discountPrice)
            .div(1e18);
        ...
        uint256 fee = liquidatedAmount
            .mul(collateralPriceInAsset)
            .mul(feeRate)
            .div(1e22);
        if (fee > 0) {
            IERC20(pos.collateral).transfer(collector, fee);
