...
contract launchpad is Ownable {
    ...
    uint256 public lockperiod = 7 minutes;
    ...
    function getClaim(address _user) public view returns(uint) {
        ...
        uint256 tempTier = getTier(msg.sender);
         if (tempTier != 0) {
    ...
    function getRewardValue(uint _amount) internal returns(uint256) {
        ...
        uint256 temp = per * 1e18;
        uint256 tempTier = getTier(msg.sender);
         if (tempTier != 0) {
             if (tempTier == 1) {temp = tierPercent1;}
             else if (tempTier == 2) {temp = tierPercent2;}
             else {temp = tierPercent3;}
         }
        uint _feePercent = temp;
