...
import "./interfaces/IEliteToken.sol";
...
contract VaultBooster is Whitelist {
    ...
    // Basic token handling for both Token and xToken
    IERC20 public token;
    ...
    // Elite Token interface, to stake Token into xToken
    IEliteToken public eToken;
    ...
    constructor(address _token, address _xToken, address _vault) {
        token = IERC20(_token);
        xToken = IERC20(_xToken);
        eToken = IEliteToken(_token);
        vault = IVault(_vault);
    ...
    function prepare() onlyWhitelisted() public returns (bool _success) {
        ...
        uint256 _amount = tokenBalance(address(token));

        totalTokensStaked += _amount;

        token.approve(_xToken, MAX_UINT);
        eToken.stake(_amount);
        
        emit onStakeRootToken(msg.sender, _xToken, _amount, block.timestamp);
        return true;
    }
    ...
    function boost() onlyWhitelisted() public returns (bool _success) {
        ...
        uint256 _amount = tokenBalance(address(xToken));

        totalRewardsDistributed += _amount;
        
        xToken.approve(_vault, MAX_UINT);
