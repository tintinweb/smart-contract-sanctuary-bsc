...
contract Exchange is Pausable , Ownable{
    ...
    address wallet;
    //ETH
    AggregatorV3Interface internal priceFeed = AggregatorV3Interface(0x143db3CEEfbdfe5631aDD3E50f7614B6ba708BA7);
    int public fetchedPrice = ((fetchLatestPrice()*10**18/31000000000000000000)*100);
    AOEX public token;
    ...
    constructor(address _wallet) {
        ...
        token = AOEX(0xAb4Ee282968a4c331a7c173E3AF5907297Cbc7fE);
    }
    ...
    function buyToken(uint _amount, address _exchange)  public  whenNotPaused {
        int goldPrice = ((fetchLatestPrice()*10**18/31000000000000000000)*100);
        exchangeToken = IERC20(_exchange);
        uint256 feeAmount = _amount.mul(fee).div(100 * 10 ** 18);
        uint256 amount = _amount - feeAmount;
        require(exchangeToken.balanceOf(msg.sender) >= _amount, "You don't have enough balance");
    ...
    function sellToken(uint _amount, address _exchange)  public  whenNotPaused  {
        require(token.balanceOf(msg.sender) >= _amount, "You don't have enough balance");
        int goldPrice = ((fetchLatestPrice()*10**18/31000000000000000000)*100);
        exchangeToken = IERC20(_exchange);
        uint totalexchangeToken = _amount * (uint(goldPrice)/10**18);
        require(exchangeToken.balanceOf(address(this)) >= totalexchangeToken, "contract don't have enough balance");
