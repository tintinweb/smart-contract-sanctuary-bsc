...
abstract contract SnacksBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	constructor (address routerAddress, address pinkAntiBot_) {
		...
		// Initialize PancakeSwap V2 router and Snacks <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
		setPancakeSwapRouter(routerAddress);

		// 1% liquidity fee, 7% reward fee, 1% buyback fee, 3% marketing fee, 2% salary fee, 2% additional sell fee
		setFees(1, 7, 1, 3, 2, 2);
	...
	function calculateFeeRate(address sender, address recipient) private view returns(uint256) {
		...
		if (applyFees) {
			(bool snackFlag, uint256 snackBuyFee, uint256 snackSellFee) = onBeforeCalculateFeeRate();
	...
	// This function swaps a {tokenAmount} of Snacks tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for Snacks -> WBNB
	...
	// Returns how many more $Snacks tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 salaryFee, uint8 additionalSellFee) public onlyOwner {
		...
		require(liquidityFee + rewardFee + buybackFee + marketingFee + salaryFee + additionalSellFee <= 30, "Total fees cannot exceed 30%");
...
contract Snacks is SnacksBase {
	...
	uint256 private _totalBNBAsSnacksClaimed; // The total number of BNB that was converted to Snacks and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsSnacksClaimed; // The amount of BNB converted to Snacks and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy Snacks automatically
	...
	bool private _reimburseAfterSnacksClaimFailure; // If true, and Snacks reward claim portion fails, the portion will be given as BNB instead
	...
	CustomTaxPeriod private _snack1 = CustomTaxPeriod('snack1', 3600,7,3,2,1,1,16);
	CustomTaxPeriod private _snack2 = CustomTaxPeriod('snack2', 3600,7,3,2,1,1,8);
	...
	constructor (address routerAddress, address pinkAntiBot_) SnacksBase(routerAddress, pinkAntiBot_) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterSnacksClaimFailure(true);
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough Snacks balance");
	...
	function doClaimReward(address user) private returns (bool) {
		...
		if (!claimRewardToken(user, claimBnbAsTokens)) {
			...
			if (_reimburseAfterSnacksClaimFailure) {
	...
	function claimRewardToken(address user, uint256 bnbAmount) private returns (bool) {
		...
		_bnbAsSnacksClaimed[user] += bnbAmount;
		_totalBNBAsSnacksClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $Snacks on a zero balance address:
		...
		// Depending on the % of $Snacks tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsSnacks(address byAddress) public view returns (uint256) {
		return _bnbAsSnacksClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsSnacks() public view returns (uint256) {
		return _totalBNBAsSnacksClaimed;
	}
	...
	function reimburseAfterSnacksClaimFailure() public view returns(bool) {
		return _reimburseAfterSnacksClaimFailure;
	}


	function setReimburseAfterSnacksClaimFailure(bool value) public onlyOwner {
		_reimburseAfterSnacksClaimFailure = value;
	}
	...
	function isInHappyHour() external view returns (bool) {
		...
		uint256 totalHappyHour = _snack1.timeInPeriod + _snack2.timeInPeriod;
		uint256 timeSinceSnack = currentTimestamp - _HappyHourStartTimestamp;
		if(timeSinceSnack < totalHappyHour) {
				return true;
		} else {
				return false;
		}
	...
	function setHappyHour1Fees(uint256 liquidityFee,uint256 rewardFee,uint256 buybackFee,uint256 marketingFee,uint256 salaryFee,uint256 additionalFee) public onlyOwner {
		_setCustomSellTaxPeriod(_snack1,liquidityFee, rewardFee,buybackFee,marketingFee,salaryFee,additionalFee);
	}

	function setHappyHour2Fees(uint256 liquidityFee,uint256 rewardFee,uint256 buybackFee,uint256 marketingFee,uint256 salaryFee,uint256 additionalFee) public onlyOwner {
		_setCustomSellTaxPeriod(_snack2,liquidityFee, rewardFee,buybackFee,marketingFee,salaryFee,additionalFee);
	}
	...
	function getHappyHour1Fees() external view returns (uint256, uint256, uint256,uint256,uint256,uint256){
        return (_snack1.liquidityFee,_snack1.rewardFee, _snack1.buybackFee, _snack1.marketingFee, _snack2.salaryFee, _snack1.additionalSellFee);
	}

	function getHappyHour2Fees() external view returns (uint256, uint256, uint256,uint256,uint256,uint256){
        return (_snack2.liquidityFee,_snack2.rewardFee, _snack2.buybackFee, _snack2.marketingFee, _snack2.salaryFee, _snack2.additionalSellFee);
	}

	function _isInHappyHour1() internal view returns (bool) {
		uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _HappyHourStartTimestamp  ? _tradingPausedTimestamp : _getNow();
		uint256 timeSinceSnack = currentTimestamp - _HappyHourStartTimestamp;
		if(timeSinceSnack < _snack1.timeInPeriod) {
				return true;
		} else {
				return false;
		}
	...
	function _isInHappyHour2() internal view returns (bool) {
			uint256 currentTimestamp = !isTradingEnabled && _tradingPausedTimestamp > _HappyHourStartTimestamp  ? _tradingPausedTimestamp : _getNow();
			uint256 timeSinceSnack = currentTimestamp - _HappyHourStartTimestamp;
			if(timeSinceSnack > _snack1.timeInPeriod && timeSinceSnack < _snack1.timeInPeriod + _snack2.timeInPeriod) {
					return true;
			} else {
					return false;
			}
	...
	function onBeforeCalculateFeeRate() internal override view returns (bool snackFlag, uint256 snackBuyFee, uint256 snackSellFee) {
		if(this.isInHappyHour()) {
			snackFlag = true;
			if(_isInHappyHour1()) {
				snackBuyFee = _snack1.liquidityFee + _snack1.rewardFee + _snack1.buybackFee + _snack1.marketingFee + _snack1.salaryFee;
				snackSellFee = _snack1.liquidityFee + _snack1.rewardFee + _snack1.buybackFee + _snack1.marketingFee + _snack1.salaryFee + _snack1.additionalSellFee;
			}
			else if (_isInHappyHour2()) {
 				snackSellFee = _snack2.liquidityFee + _snack2.rewardFee + _snack2.buybackFee + _snack2.marketingFee + _snack2.salaryFee + _snack2.additionalSellFee;
				snackBuyFee = _snack2.liquidityFee + _snack2.rewardFee + _snack2.buybackFee + _snack2.marketingFee + _snack2.salaryFee;
			}
			return (snackFlag, snackBuyFee, snackSellFee);
		}
