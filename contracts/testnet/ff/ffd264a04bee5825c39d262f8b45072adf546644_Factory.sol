...
contract Factory{
    ...
    mapping(address => bool) public mainPairToken;
    ...
    function createPair(address tokenA,address tokenB) public returns(address addr){
        require(tokenA != tokenB,'');
        require(tokenA != address(0x0) && tokenB != address(0x0),'ZERO_ADDRESS');
        ...
        Pair(addr).initialize(tokenA,tokenB);

        getPair[tokenA][tokenB] = addr;
        getPair[tokenB][tokenA] = addr; // 双向交易对
        
        allPairs.push(addr);

        emit PairCreated(tokenA, tokenB, addr);
