...
contract NaviUi is Ownable {
    ...
    function getPoolBalances(address account, IFarmGroup grp)
        ...
        returns (IFarmGroup.Deposit[] memory out)
    {
        uint256 length = getPoolBalancesLength(account, grp);
        out = new IFarmGroup.Deposit[](length);
        length = grp.getPairsLength();
        uint256 k = 0;
        for (uint32 i = 0; i < length; i++) {
            address pair = grp.getPairAddress(i);
            if (pair == address(0)) continue;
            uint256 balance = IERC777(pair).balanceOf(account);
            if (balance == 0) continue;
            out[k++] = IFarmGroup.Deposit(address(0), pair, balance);
        }
    }

    function getPoolBalancesLength(address account, IFarmGroup grp)
        public
        view
        returns (uint256 ret)
    {
        ...
        for (uint32 i = 0; i < length; i++) {
            address pair = grp.getPairAddress(i);
            if (pair == address(0)) continue;
            if (IERC777(pair).balanceOf(account) > 0) ret++;
        }
    }
    ...
    function getFarmBalances(address account, IFarmGroup grp)
        ...
        returns (IFarmGroup.Deposit[] memory out)
    {
        uint256 length = getFarmBalancesLength(account, grp);
        out = new IFarmGroup.Deposit[](length);
        ...
        for (uint32 i = 0; i < length; i++) {
            ...
            if (balance == 0) continue;
            out[k++] = IFarmGroup.Deposit(farm, address(0), balance);
        }
    }

    function getFarmBalancesLength(address account, IFarmGroup grp)
        public
        view
        returns (uint256 ret)
    {
        ...
        for (uint32 i = 0; i < length; i++) {
            address farm = grp.getFarm(i);
            if (farm == address(0)) continue;
            if (IERC777(farm).balanceOf(account) > 0) ret++;
        }
    }
