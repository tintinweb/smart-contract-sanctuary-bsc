...
contract MetaMinerPresaleTestNet is Context, Ownable {
  ...
  mapping (address => uint256) private _balancesRED;
  mapping (address => uint256) private _balancesWHITE;

  mapping (address => mapping (address => uint256)) private _allowances;

  uint256 private _pricepresale;
  uint256 private _timerpresale;
  uint256 private _timerpacksale;
  uint256 private _tokenpresale;
  uint256 private _idoClaimMax;
  uint256 private _idoClaimCooldown;

  uint256 private _deployTime;
  uint256 public _currentRedPacks;
  uint256 public _currentWhitePacks;

  address private _tokencontract;
  bool private _linkedcontract;
  bool private ispresale;
  bool private ispacksale;
  bool private finishPresale;
  bool public finishPacksale;

  constructor() public {

    _idoClaimCooldown = 60 * 60 * 24 * 15;
    _deployTime = block.timestamp;
    _currentRedPacks = 500;
    _currentWhitePacks = 100;
  ...
  function packtimerExpire() external view returns (uint256) {
    require( block.timestamp < _timerpacksale,"BEP20: packs sale timer expired");
    return _timerpacksale - block.timestamp; // in a second.
  }
  ...
  function balancesRedOf(address account) external view returns (uint256) {
    return _balancesRED[account];
  }

  function balancesWhiteOf(address account) external view returns (uint256) {
    return _balancesWHITE[account];
  }
  ...
  function buyredpack() public payable {
    require( ispacksale == true,"BEP20: packs sale not open yet");
    require( block.timestamp <= _timerpacksale,"BEP20: packs sale is out of date");
    require( msg.sender != address(0), "BEP20: error zero address");
    require( msg.value == 100000000000000000 wei,"BEP20: revert by bnb value");
    require( _currentRedPacks > 0,"BEP20: packs was sold out");
    _currentRedPacks = _currentRedPacks - 1;
    _balancesRED[msg.sender] = _balancesRED[msg.sender] + 1;
  }

  function buywhitepack() public payable {
    require( ispacksale == true,"BEP20: packs sale not open yet");
    require( block.timestamp <= _timerpacksale,"BEP20: packs sale is out of date");
    require( msg.sender != address(0), "BEP20: error zero address");
    require( msg.value == 250000000000000000 wei,"BEP20: revert by bnb value");
    require( _currentWhitePacks > 0,"BEP20: packs was sold out");
    _currentWhitePacks = _currentWhitePacks - 1;
    _balancesWHITE[msg.sender] = _balancesWHITE[msg.sender] + 1;
  }

  function transfer(address recipient, uint256 amount) external returns (bool) {
    if ( amount == 0 ) {
      require( _balancesRED[recipient] > 0,"BEP20: not enough pack");
      _balancesRED[recipient] = _balancesRED[recipient] - 1;
    } else {
      require( _balancesWHITE[recipient] > 0,"BEP20: not enough pack");
      _balancesWHITE[recipient] = _balancesWHITE[recipient] - 1;
    }
    return true;
  }
  ...
  function startPackSale(uint256 settimer) public onlyOwner returns (bool) {
    require( _linkedcontract == true,"BEP20: link token contract frist before use function");
    _timerpacksale = block.timestamp + settimer;
    ispacksale = true;
    return true;
  }
  ...
  function endpack() public onlyOwner returns (bool) {
    finishPacksale = true;
  }
