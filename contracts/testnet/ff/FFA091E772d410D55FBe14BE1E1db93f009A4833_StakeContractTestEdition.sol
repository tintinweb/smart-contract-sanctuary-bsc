 ...
 */
contract StakeContractTestEdition is Ownable {
   ...
   struct DepositInfo {
    ...
    uint256 public referralRateDenominator;
    ...
    constructor(
        ...
        setReferralRateDenominator(100);
    ...
    function myReferralReward(uint256 _pid,address _user) public view validatePoolByPid(_pid) returns(uint256 totalReferralBonus){
        address[] memory myInviteeAddress = IInviteContract(inviteContract).getMyInviteeAddress(_user);
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 referralBonus = (pendingReward(_pid,myInviteeAddress[i]).add(userInfo[_pid][myInviteeAddress[i]].mintAmount)).mul(bonusPerReferral).div(referralRateDenominator);
    ...
    function receiveMyReferralReward(uint256 _pid) public validatePoolByPid(_pid) {
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePoolByPid(_pid) {
        ...
        require(IInviteContract(inviteContract).getMyInviter(msg.sender) != address(0),"deposit: You have not bound an inviter yet");
    ...
    function claim(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        require(IInviteContract(inviteContract).getMyInviter(msg.sender) != address(0),"claim: You have not bound an inviter yet");
    ...
    function withdraw(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        require(IInviteContract(inviteContract).getMyInviter(msg.sender) != address(0),"withdraw: You have not bound an inviter yet");
    ...
    function setInviteContract(address _newInviteContract) public onlyOwner {
        inviteContract = _newInviteContract;
    }
