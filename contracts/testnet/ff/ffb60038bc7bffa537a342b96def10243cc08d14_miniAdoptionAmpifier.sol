...
contract miniAdoptionAmpifier is Ownable , ReentrancyGuard{
  ...
  mapping(bytes32 => bool) public executed;
    ...
    uint public DAY = 360;   // In Seconds 86400
    uint public YEAR = 4320; // Seconds in Year
    ...
    */
    event pointsDeposited(uint indexed _day, uint indexed _points, address indexed _user, uint nonce);
    /**
    ...
    */
    event PointsEarned(address indexed _user, uint indexed _amount, uint indexed _nonce);
...
//   ************** Reward Claim Pool **************
     ...
     */
    function depositPointsForReward(uint256 _points, Signature calldata _sign, uint256 nonce) public nonReentrant {
        
        bytes32 sigHash = keccak256(abi.encodePacked(nonce, _sign.v, _sign.r, _sign.s));
  ...
  function verifySignature(
    ...
    bytes32 messageHash = keccak256(abi.encodePacked(header, _to, _amount, _nonce));
