...
contract Swap is SwapGovernance {
     ...
     */
    constructor(address _feeCollector, address _1inchRouter) {
    ...
    function _swapToken1Inch(
        ...
        if (address(swapDescriptionObj.srcToken) == NATIVE_ADDRESS) {
            ...
            require(
                (msg.value).sub(feeAmount) >= swapDescriptionObj.amount,
            ...
            require(
                (amount).sub(feeAmount) >= swapDescriptionObj.amount,
                "Key Pair: Amount Not match with Swap Amount."
            );
            ...
            require(swapSrcToken.balanceOf(msg.sender) >= amount, "Key Pair: You have insufficient balance to swap");
     ...
     * @param _1inchData a 1inch data to call aggregate router to swap assets.
     */
    function swapTokens(bytes calldata _1inchData, uint256 amount) external payable virtual returns (uint256) {
...
contract SwapTest is Swap, SwapGovernanceTest {
    ...
    function swapTokens(bytes calldata _1inchData, uint256 amount) external payable override returns (uint) {
        ...
        if (address(swapDescriptionObj.dstToken) != NATIVE_ADDRESS) {
