...
contract TokenSwap is Ownable, Pausable, ReentrancyGuard, Whitelist {
    ...
    constructor(address _router, address _sheep, address _wool, address _lms, address _vault) {
        ...
        if (ISwapPair(factory.getPair(address(sheep), address(wool))) == ISwapPair(address(0))) {
            pair = ISwapPair(factory.createPair(address(sheep), address(wool)));
        } else {
            pair = ISwapPair(factory.getPair(address(sheep), address(wool)));
        }
    }
