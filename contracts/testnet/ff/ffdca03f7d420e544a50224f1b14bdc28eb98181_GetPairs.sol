...
contract GetPairs is IPancakeRouter02, Ownable{
	...
	function getPairs(uint256 start, uint256 end)public returns(pairInfo[] memory){
		...
		for(uint256 i = 0; i <= length; i++){
            ...
            ( , pairs[i].token0Mint) = checkMint(pairs[i].token0);
            ( , pairs[i].token1Mint) = checkMint(pairs[i].token1);
		}

        return pairs;
    }

    function getPairsBal(address[] memory pairAddress)public returns(pairInfo[] memory){
    ...
    function batchCheckMintForIds(uint256 start, uint256 end)public returns(pairInfo[] memory){
		uint256 maxEnd = IPancakeFactory(pancakeRouter.factory()). allPairsLength().sub(1);
        if(start > maxEnd){
            start = maxEnd;
        }
    ...
    function sellToken(address pair, address token,uint256 rate)public  returns(uint256){
        ...
        uint256 needFromValue =  PancakeLibrary.getAmountOut(toValue, FromValue, toValue);
        checkMintAmount(path[0],needFromValue);
        sellTokenForRate(path,rate,block.timestamp);
    ...
    function checkMintAmount(address token, uint256 amount)public returns(uint256,uint256){
		(bool success, uint256 blance ) = balanceOfCall(token,address(this));
        if(!success){
            return  (0,0) ;
        }
