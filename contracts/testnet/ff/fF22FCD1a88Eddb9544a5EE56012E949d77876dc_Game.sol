...
contract Game is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function getU(address user) public view returns (uint256[] memory) {
        return ownerToTokens[user].values();
    }

    function getUM(address user) public view returns (string[] memory) {
        string[] memory userMetadata = new string[](getU(user).length);
        for (uint256 i; i < getU(user).length; i++) {
            userMetadata[i] = tokenURI(getU(user)[i]);
        }
        return userMetadata;
    }
