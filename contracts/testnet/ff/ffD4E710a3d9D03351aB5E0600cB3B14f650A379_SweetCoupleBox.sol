...
import "./ISweetCoupleManager.sol";

contract SweetCoupleBox is ERC721Enumerable, Ownable, ReentrancyGuard {
    constructor(address _sweetCoupleBoxManagerAddress)
        ERC721("Sweet Couple Box", "DINOSCBOX")
    {
        sweetCoupleBoxManagerAddress = _sweetCoupleBoxManagerAddress;
        boxOpenableAt = block.timestamp;
        round3StartAt = block.timestamp;
        round3EndAt = block.timestamp + 10 days;
    ...
    address public sweetCoupleBoxManagerAddress;
    string public boxTokenURI;
    uint256 public boxPrice = 2999 * 1e18;
    ...
    function setBoxManagerAddress(address _sweetCoupleBoxManagerAddress)
        external
        onlyOwner
    {
        sweetCoupleBoxManagerAddress = _sweetCoupleBoxManagerAddress;
    ...
    function buyBox(uint256 _amount) external noContract nonReentrant {
        ...
        uint256 curentTotalBoughtBoxesByAddress = _amount.add(
        ...
        require(
            currentTimestamp >= round3StartAt &&
                currentTimestamp <= round3EndAt,
        ...
        addressToTotalBoughtBoxes[msg.sender] = _amount.add(
            addressToTotalBoughtBoxes[msg.sender]
        );
    ...
    function openBox(uint256 _boxId) external noContract nonReentrant {
        ...
        _burn(_boxId);
        (uint256 dinoGenes1, uint256 dinoGenes2) = ISweetCoupleManager(
            sweetCoupleBoxManagerAddress
        ).calculateRewards(_boxId, box.createdAtBlock);
        uint256 dinoId1 = 0;
        uint256 dinoId2 = 0;
        /// @dev Reward Dino
        uint128 dinoGender1 = uint128((block.timestamp % 2) + 1);
        uint128 dinoGender2 = uint128(
            ((block.timestamp + uint256(blockhash(block.number - 1))) % 2) + 1
        );

        dinoId1 = _transferDinoReward(dinoGenes1, dinoGender1, msg.sender);
        dinoId2 = _transferDinoReward(dinoGenes2, dinoGender2, msg.sender);

        emit BoxOpened(_boxId, msg.sender, dinoId1, dinoId2);
    }
}
...
interface ISweetCoupleManager {
    function calculateRewards(uint256 _boxId, uint256 _createdAtBlock)
        external
        view
        returns (uint256 dinoGenes1, uint256 dinoGenes2);
}
