...
contract AgeioPool is Ownable {
  ...
  function deposit(uint256 _pid, uint256 amount) external onlyPool(_pid) {
  ...
  function requestWithdraw(uint256 _pid, uint256 amount) external onlyPool(_pid) {
    ...
    withdrawalRequests[_msgSender()] = withdrawalRequests[_msgSender()].add(amount);

    emit RequestedWithdraw(_pid, _msgSender(), amount, block.timestamp);
  }
  event CancelDeposit(uint256 pid, address user, uint256 amount);
  function cancelDeposit(uint56 _pid) public onlyPool(_pid) {
    PoolInfo storage pool = poolInfo[_pid];
    require(pool.unstakingFromGNode == false, "CancelDeposit: Not allowed");
    require(withdrawalRequests[_msgSender()] >= 0, "CancelDeposit: Insufficient balance.");
    UserInfo storage user = userInfo[_pid][_msgSender()];
    pool.totalDeposited = pool.totalDeposited.add(withdrawalRequests[_msgSender()]);
    user.amount = user.amount.add(withdrawalRequests[_msgSender()]);
    // safeTransferTfuel(address(pool.tank), msg.value);
    emit CancelDeposit(_pid, _msgSender(), withdrawalRequests[_msgSender()]);
    delete withdrawalRequests[_msgSender()];
  }
  ...
  function withdraw(uint256 _pid) public onlyPool(_pid) {
    ...
    require(withdrawalRequests[_msgSender()] > 0, "Claim Withdraw: Insufficient Balance.");
    IERC20(token).safeTransfer(address(_msgSender()), withdrawalRequests[_msgSender()]);
    emit ClaimedWithdraw(_pid, _msgSender(), withdrawalRequests[_msgSender()]);
    delete withdrawalRequests[_msgSender()];
  }
