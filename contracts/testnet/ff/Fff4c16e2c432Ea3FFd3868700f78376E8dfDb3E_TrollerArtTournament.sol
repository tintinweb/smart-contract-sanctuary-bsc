...
contract TrollerArtTournament is Ownable {
    ...
    event RoundEnded(uint256 tournamentId, uint256[] roundWinners);
    ...
    function _setWinners(uint256 _tournamentId,string[] memory _nftName ,uint8[] memory _voteCount,string[] memory _hashKey,address[] memory _nftOwnerWallet) internal
    {
        ...
        if(tournaments[_tournamentId].topListed.length == 0){
        for(uint8 i=0; i<n; i++){
            ...
            nftData[_tournamentId].push(currentInfo);
             ...
             for (uint8 i = 0; i < n; i++) {
                tournaments[_tournamentId].topListed[i] = _voteCount[i];
                nftData[_tournamentId][i].nftName =  _nftName[i];
                nftData[_tournamentId][i].voteCount = _voteCount[i];
                nftData[_tournamentId][i].hashKey = _hashKey[i];
                nftData[_tournamentId][i].nftOwnerWallet = _nftOwnerWallet[i];
            }
        }
    }

    function endCurrentRound(string[] memory _nftName ,uint8[] memory _voteCount,string[] memory _hashKey,address[] memory _nftOwnerWallet) external{
        uint256 _tournamentId = tournamentIds.current();
        require(tournaments[_tournamentId].totalRounds > 1, "Tournament must not be reached the last round");
        _setWinners(_tournamentId, _nftName,_voteCount,_hashKey,_nftOwnerWallet);
        tournaments[_tournamentId].totalRounds--;
        emit RoundEnded(_tournamentId, tournaments[_tournamentId].topListed);
    }
    ...
    function endTournament(
        ...
        require(_tournamentId > 0 ,"Invalid tournamentId");
        require(tournaments[_tournamentId].topListed.length>=3, "Winner array length must be three");
        require(tournaments[_tournamentId].totalRounds == 1, "Tournament must be in the last round");
        tournaments[_tournamentId].totalRounds--;
