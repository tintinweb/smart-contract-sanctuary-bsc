...
contract QUINTpresale is Ownable {
    ...
    uint256 public tokenPriceFTM = 9000000000000000000; 
    uint256 public tokenDecimal = 18;
    uint256 public FTMDecimal = 18;
    uint256 public totalTransaction;
    uint256 public totalHardCap;
    uint256 public minContribution = 1e17;
    uint256 public maxContribution = 15e20;
    uint256 public hardCap = 220e21;
    ...
    event FTMDeposited(address indexed beneficiary, uint amount);
    ...
    /* This Function Will Airdrop FTM to Multiple Users */
    ...
    /* This will deposit FTM to Contract */
    function depositCrypto() payable public returns (bool){
        uint256 amount = msg.value;
        address userAddress = msg.sender;
        emit FTMDeposited(userAddress, amount);
        return true;
    }
    ...
    /* This function will accept FTM directly sent to the address */
    receive() payable external {
        ExchangeFTMforToken(msg.sender, msg.value);
    }

    /* This Function will exchange FTM to Token */    
    function ExchangeFTMforToken(address _addr, uint256 _amount) private {
        ...
        require(totalHardCap < hardCap, "FTM Hardcap Reached");
        ...
        FTMAmount = ((amount.mul(10 ** uint256(tokenDecimal)).div(tokenPriceFTM)).mul(10 ** uint256(tokenDecimal))).div(10 ** uint256(tokenDecimal));
        tokenExchanged[userAdd] += FTMAmount;
        
        emit FTMDeposited(msg.sender,msg.value);
    }


    /* This Function will exchange FTM to Token in Mannual Call */
    function ExchangeFTMforTokenMannual() public payable {
        uint256 amount = msg.value;
        address userAdd = msg.sender;
        uint256 FTMAmount = 0;
        balances[msg.sender] = balances[msg.sender].add(msg.value);
    ...
    /* View FTM Balance */
    function FTMBalance() public view returns (uint256){
