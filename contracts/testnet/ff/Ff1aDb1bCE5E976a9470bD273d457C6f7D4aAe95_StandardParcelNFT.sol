...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event NftForSale(uint256 indexed tokenId, uint256 price);
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function tokensOfOwner(uint256 _tokenId) external view returns (uint256[] memory) {
        address tokenOwner = _ownerOf(_tokenId);
        return _userTokens[tokenOwner];
    }
}