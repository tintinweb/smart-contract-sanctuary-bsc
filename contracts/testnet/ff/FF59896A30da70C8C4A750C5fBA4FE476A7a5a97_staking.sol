...
contract staking is Ownable, Pausable{
    ...
    function deposit(uint256 _tokenAmount) external whenNotPaused {
        ...
        tokenAddress.transferFrom(_msgSender(), address(this), _tokenAmount);
        emit Deposit(_msgSender(), user.stakeID, _tokenAmount, block.timestamp);
    }

    function claim(uint256 _stakeID) public whenNotPaused {
        ...
        uint256 rewardAmount = stakeTime * user.amount * perDay / 1e3 / 1e16 / 86400;
        
        tokenAddress.transfer(_msgSender(), rewardAmount);
    ...
    function withdraw(uint256 _stakeID) external whenNotPaused {
        ...
        claim(_stakeID);
        user.unstake = true;
        tokenAddress.transfer(_msgSender(), user.amount);
