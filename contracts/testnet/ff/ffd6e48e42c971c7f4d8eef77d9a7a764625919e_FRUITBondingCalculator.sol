...
contract FRUITBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(FRUIT) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(FRUIT), "not a FRUIT lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** FRUIT.decimals() ) ).div( getTotalValue( _pair ) );
    }
