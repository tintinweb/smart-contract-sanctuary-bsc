...
contract IdoProjectV2 is ReentrancyGuard, Initializable {
    ...
    function registerProject(bytes memory _signature, uint256 _slot, uint _userLevel, uint _pool) external onPoolSale checkSigner(_signature, _slot, _userLevel) {
        ...
        if(_pool == 1) {
            ...
            _execute(msg.sender, maxAllocation, _decimals, _pool);
        }
        // other
        if( _pool == 2) {
    ...
    function addWhiteList(address[] memory _users) external onlyOperator {
        ...
        require(_users.length + whiteList.length() <= totalWhiteList, "_users empty");
    ...
    function submitWinners(address[] memory _users) external onlyOperator {
        ...
        require(winners.length() + totalWhiteList + _users.length  <= totalSlot + allowListSubmitted, "Slot over flow");
        uint _decimals = Decimal(address(tokenSale)).decimals();
        for(uint i = 0; i < _users.length; i++ ){
            ...
            uint _decimals = Decimal(address(tokenSale)).decimals();
            _execute(_user, _allocation, _decimals, 2);
        }
    ...
    //=================== internal ======================//
    function _execute(address _user, uint _allocation, uint _decimals, uint _pool) internal {
        // set claim info
