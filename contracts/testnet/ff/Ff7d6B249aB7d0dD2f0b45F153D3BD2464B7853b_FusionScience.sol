...
contract FusionScience is Ownable {
    ...
    constructor() {
        dinoRarityToSuccessRate[1] = 6700;
        dinoRarityToSuccessRate[2] = 5000;
        dinoRarityToSuccessRate[3] = 3300;
        dinoRarityToSuccessRate[4] = 2000;
    }

    modifier onlyFusionLabContract {
        require(msg.sender == fusionLabContract, "Only fusion lab contract can call this function");
        _;
    }

    function setFusionLabContract(address _fusionLabContract) external onlyOwner {
    ...
    ) external onlyFusionLabContract view returns (bool _isSuccess, uint256 _dinoGenes) {
        uint256 rand = ((uint256(
            keccak256(abi.encodePacked(_dino1BornAt, _dino2Id))
        ) + _dino1Id) % 10000) + 1;
        uint256 rand2 = ((uint256(
            keccak256(abi.encodePacked(_dino2BornAt, _dino1Id))
        ) +
            _dino2Id +
            block.timestamp) % 10000) + 1;
