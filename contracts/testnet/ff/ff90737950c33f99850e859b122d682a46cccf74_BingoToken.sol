...
contract BingoToken is  Context, IERC20, Ownable{
    ...
    mapping (address => uint) public _excludeFee;//白名单功能 0未在名单，1在名单
    mapping (address => uint) public _banListMap;//黑名单功能 0正常，1拉黑
    mapping (address => uint) public _totalReceiveFeeMap;// 各个地址收的手续费map
    mapping (address => uint) public _feeRateMap;//手续费百分比
    mapping (address => uint) public _swapAddrMap;//交易所地址 0不是，1是

    address public _institutionalFinancingAddr = 0xc449689f8Ee9918Fc73a30544fa470C5af415040;//机构融资
    address public _operateAddr = 0x3868e220Daeff3727f76348F664A8B65739377B2;//早期运营

    address public _jackpotPoolAddr =  0xE7Ee995d4e9f4eb26D61940DB32510236EFfA8D0;//Jackpot池
    address public _clubRewardAddr = 0x850C72e95ba659773b31744A1b778d9B6A2e88Ca;//CLUB奖励（战队排名奖励）
    address public _deadAddr = 0x000000000000000000000000000000000000dEaD;//销毁地址
    address public _groupAddr = 0x6E15d12aa08085b9Fdd8F77123eFC7c23D9AB773;//技术团队归属地址
    address public _transferFeeReceiveAddr = 0xE7Ee995d4e9f4eb26D61940DB32510236EFfA8D0;//交易手续费接收地址

    address public _IDOAddr = 0x94bC1EAAfcd7e2b6cE6891854401073f4fCf40f2;//IDO

    bool _pauseTransfer = true;// 是否暂停交易
    ...
    constructor () {
        //发行量 10000000000
        _mint(_groupAddr, 600000000 ether);
        _mint(_institutionalFinancingAddr, 200000000 ether);
        _mint(_operateAddr, 200000000 ether);
        _mint(_jackpotPoolAddr, 5000000000 ether);
        _mint(_IDOAddr, 2000000000 ether);
//        _mint(_clubRewardAddr, 2000000000 ether); // todo 待打开
        _mint(_clubRewardAddr, 1000000000 ether);   // todo 待删除
        _mint(_msgSender(), 1000000000 ether); // todo 待删除
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        uint onePercent = amount.div(100);//百分之一
        ...
        if(_swapAddrMap[recipient] == 1 && _excludeFee[sender] != 1){
            ...
            totalFee = totalFee.add(currentFee);
        ...
        if(_excludeFee[sender] != 1 && _excludeFee[recipient] != 1){
            ...
            //记录总税费
            totalFee = totalFee.add(transferFee);
            //转账金额扣除税费
    ...
    //fee 归属运营团队
    function setOperateFee(uint fee) onlyOwner external returns (bool) {
        _feeRateMap[_operateAddr] = fee;
        return true;
    }
    //fee 转账
    ...
    function setOperateAddr(address addr) onlyOwner external returns (bool) {
        _operateAddr = addr;
        return true;
    }
