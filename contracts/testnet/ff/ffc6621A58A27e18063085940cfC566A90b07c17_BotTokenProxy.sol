...
contract BotTokenProxy {

    IDABot internal _bot;

    bytes32 public immutable IMPL_MODULE_ID;

    constructor(bytes32 implModuleId) {
        IMPL_MODULE_ID = implModuleId;
    }

    function _fallback() internal virtual {
        require(address(_bot) != address(0), "BotTokenProxy not initialized");
        _delegate(_getImpl(_bot));
    }
    ...
    function init(bytes calldata data) external payable {
        ...
        (IDABot bot,) = abi.decode(data, (IDABot, IRoboFiERC20));
        _delegate(_getImpl(bot));
    }

    function _getImpl(IDABot bot) private view returns (address impl) {
