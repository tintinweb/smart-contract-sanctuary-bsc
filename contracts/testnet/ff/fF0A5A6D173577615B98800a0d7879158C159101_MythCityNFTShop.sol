...
contract MythCityNFTShop {
    ...
    function addModShopItem(
        ...
        modShopItems[_modId] = modShopItem(_modImageId, _cost, _stat, nameId);
        emit modShopAdded(_modId, _modImageId, _cost, _stat, nameId);
    }
    ...
    function addEngramShopItem(
        ...
        emit engramShopAdded(_engramId, _tableAddress, _cost, _tier);
    }
    ...
    function buyMod(uint256 _modId) external payable {
        modShopItem memory tempItem = modShopItems[_modId];
        require(
            tempItem.itemCost > 0 && msg.value == tempItem.itemCost,
            "Send Exact Value"
        );
    ...
    function buyEngram(uint256 _engramId) external payable {
        engramShopItem memory tempItem = engramShopItems[_engramId];
        require(
            tempItem.itemCost > 0 && msg.value == tempItem.itemCost,
            "Send Exact Value"
        );
        MythEngrams tempEngram = MythEngrams(engramAddress);
        require(
            tempEngram.mint(
                msg.sender,
                tempItem.engramTier,
                tempItem.tableAddress
            ),
            "Failed to mint"
        );
    }

    function buyCosmetic(uint256 _cosmeticId) external payable {
    ...
    function buyEquipment(uint256 _equipmentId) external payable {
    ...
    function buyWeapon(uint256 _weaponId) external payable {
...
contract MythDegen is ERC721 {
    ...
    mapping(uint256 => defaultCosmetics) public degenDefaults; //Mapping from token Id to cosmetics struct
    ...
    event cosmeticChanged(uint256 degenId, uint256 layerType, uint256 itemId);

    event degenReGrade(uint256 degenId, uint256 coreScore, uint256 damageCap);
    ...
    function setModsAddress(address _address)
        external
        isWhitelisted
        returns (bool)
    {
        modsAddress = _address;
    }

    function setWeaponsAddress(address _address)
        external
        isWhitelisted
        returns (bool)
    {
        weaponsAddress = _address;
    }

    function setEquipmentAddress(address _address)
        external
        isWhitelisted
        returns (bool)
    {
    ...
    function equipCosmetic(uint256 _degenId, uint256 _cosmeticId) external {
        require(
        ...
        emit cosmeticChanged(_degenId, tempStats.layerType, _cosmeticId);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '",',
                        '"attributes": [{"trait_type": "Layer Type","display_type": "number", "value": ',
...
contract MythEngrams is ERC721 {
    ...
    event engramActivated(uint256 engramId, uint256 blockNumber);
    event engramResolved(uint256 engramId, uint256 numberRolled);
    ...
    function activateWeapon(
        ...
        tempStats.blockInitialized = block.number;
        ...
        emit engramActivated(_engramId, block.number);
    }
        ...
        external
    {
        ...
        require(blockNumberCount[_blockNumber] > 0, "Nothing to resolve");
        ...
        while (counter < resolveCountOfBlock) {
            ...
            currentEngram.numberRolled =
                ...
                100000;
            // resolve function to upgrade weapon
            if (currentEngram.typeOfItemToUpgrade == 1) {
                require(
                    upgradeWeapon(
                        currentEngram.engramTable,
                        currentEngram.idOfItemToUpgrade,
                        currentEngram.statOfItemToUpgrade,
                        _resolutionSeed
                    ),
                    "Weapon upgrade failed to change weapon contract data"
                );
                pendingWeaponIds[tempId] = false;
            } else if (currentEngram.typeOfItemToUpgrade == 2) {
                require(
                    upgradeMod(
                        currentEngram.engramTable,
                        currentEngram.idOfItemToUpgrade,
                    ...
                    "Weapon upgrade failed to change mod contract data"
                );
                pendingModIds[tempId] = false;
            } else if (currentEngram.typeOfItemToUpgrade == 3) {
                require(
                    upgradeEquipment(
                        currentEngram.engramTable,
                        currentEngram.idOfItemToUpgrade,
                    ...
                    "Weapon upgrade failed to change equipment contract data"
                );
                pendingEquipmentIds[tempId] = false;
            } else if (currentEngram.typeOfItemToUpgrade == 4) {
                require(
                    upgradeDegen(
                        currentEngram.engramTable,
                        currentEngram.idOfItemToUpgrade,
            ...
            emit engramResolved(tempId, currentEngram.numberRolled);
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '",',
                        '"attributes": [{"trait_type": "Engram Type", "value": ',
        ...
        returns (string memory)
    {
        ...
        string memory json = string(
            abi.encodePacked(
                ...
                abi.encodePacked(tempStats.engramTable),
                ...
                Base64.uint2str(tempStats.blockInitialized),
                '},{"trait_type":"Number Rolled","display_type": "number", "value":',
                Base64.uint2str(tempStats.numberRolled),
                '},{"trait_type":"Resolution Seed","display_type": "number", "value":',
                abi.encodePacked(tempStats.resolutionSeed),
                "}"
            )
        );
        return json;
    ...
    function upgradeWeapon(
        ...
        EngramTable tableContract = EngramTable(_tableAddress);
        uint256 rand = uint256(
            keccak256(abi.encodePacked(_resolutionSeed, _weaponId))
        ) % 100000;
        ...
        if (_upgradeStat == 1) {
            ...
            require(
                tempContract.regradeWeaponStat(_weaponId, newStatValue, 0),
                "Upgrade Failed"
            );
        } else {
            ...
            newStatValue = newStatValue / 10**18;
            require(
                tempContract.regradeWeaponStat(_weaponId, 0, newStatValue),
                "Upgrade Failed"
            );
        }
        return true;
    ...
    function upgradeDegen(
        ...
        MythDegen tempContract = MythDegen(degenAddress);
        MythDegen.stats memory tempDegenStats = tempContract.getStats(_degenId);
        EngramTable tableContract = EngramTable(_tableAddress);
        uint256 rand = uint256(
            keccak256(abi.encodePacked(_resolutionSeed, _degenId))
        ) % 100000;
        EngramTable.prize memory tempPrize = tableContract.getWinningPrize(
            uint128(rand)
        );
        ...
        if (_upgradeStat == 1) {
            ...
            newStatValue = newStatValue / 10**18;
            require(
                tempContract.reGradeDegen(_degenId, newStatValue, 0),
                "Upgrade Failed"
            );
        } else {
            ...
            newStatValue = newStatValue / 10**18;
            require(
                tempContract.reGradeDegen(_degenId, 0, newStatValue),
                "Upgrade Failed"
            );
        }
        return true;
    ...
    function upgradeMod(
        ...
        EngramTable tableContract = EngramTable(_tableAddress);
        uint256 rand = uint256(
            keccak256(abi.encodePacked(_resolutionSeed, _modId))
        ) % 100000;
        EngramTable.prize memory tempPrize = tableContract.getWinningPrize(
            uint128(rand)
        );
        ...
        newStatValue = newStatValue / 10**18;
        require(
            tempContract.regradeModStat(_modId, newStatValue),
            "Upgrade Failed"
        );
    ...
    function upgradeEquipment(
        ...
        MythCityEquipment.itemStat memory tempEquipmentStats = tempContract
            .getStats(_equipmentId);
        EngramTable tableContract = EngramTable(_tableAddress);
        uint256 rand = uint256(
            keccak256(abi.encodePacked(_resolutionSeed, _equipmentId))
        ) % 100000;
        EngramTable.prize memory tempPrize = tableContract.getWinningPrize(
            uint128(rand)
        );
        ...
        newStatValue = newStatValue / 10**18;
        require(
            tempContract.regradeEquipmentStat(_equipmentId, newStatValue),
            "Upgrade Failed"
        );
