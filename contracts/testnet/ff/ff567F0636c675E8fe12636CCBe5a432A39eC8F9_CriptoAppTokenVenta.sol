...
contract CriptoAppTokenVenta{
    ...
    event Venta (address _buyer, uint _amount);
    ...
    function buy(uint tokens) public payable{
        require(msg.value / phase(currentPhaseIndex).price == tokens, 'Error el valor no coincide');
        require(phase(currentPhaseIndex).total <= __amount(tokens), 'ERR0R Hay poco valance');
        require(token.balanceOf(address(this)) >= __amount(tokens), 'Agotado');
    ...
    function __tokens() public view returns(uint){ //identificando los tokens del usuario
