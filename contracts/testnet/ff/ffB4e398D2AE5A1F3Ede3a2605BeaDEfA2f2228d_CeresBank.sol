...
contract CeresBank is CeresBase, ICeresBank, Ownable {
    ...
    uint256 public override updateCooldown;
    uint256 public override ratioStep = 1500;

    uint256 public maxBurnedCRSAmount = 500000e18;
    uint256 public maxMintedASCAmount = 100000e18;
    uint256 public override minMintedASCAmount = 500e18;
    uint256 public minterCRSBonus;
    ...
    constructor (address _owner, address _factory, address _asc, address _crs, uint256 _updateCooldown, uint256 _bonus)
    Ownable(_owner) CeresBase(_factory){

        collateralRatio = 1e6;
        asc = ICeresCoin(_asc);
        crs = ICeresCoin(_crs);
        updateCooldown = _updateCooldown;
        minterCRSBonus = _bonus;
    }
    ...
    returns (ICeresBank.MintResult memory result){
        ...
        require(factory.isStakingMineable(collateral) == true, "CeresBank: Collateral is not available to mint in CeresBank");
        require(block.timestamp > nextUpdateTime[msg.sender], "CeresBank: Please wait for update cooldown!");
        require(collateralRatio > minCR, "CeresBank: Must be greater than minimal collateral ratio!");
        ...
        require(ascMintTotal <= maxMintedASCAmount, "CeresBank: Exceed maximum ASC to be minted");
        require(ascMintTotal >= minMintedASCAmount, "CeresBank: Must be greater than minMintedASC");
        ...
        require(crsBurnAmount <= maxBurnedCRSAmount, "CeresBank: Exceed maximum CRS to be burned");

        if (crsBurnAmount > 0) {
            require(ICeresStaking(stakingCRS).totalStaking() >= crsBurnAmount, "CeresBank: Balance of StakingCRS is not enough!");
    ...
    function setMaxASCRedeemablePrice(uint256 _maxASCRedeemablePrice) external onlyOwner {
