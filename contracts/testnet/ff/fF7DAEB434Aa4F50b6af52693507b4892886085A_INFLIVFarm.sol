...
contract INFLIVFarm is Initializable, OwnableUpgradeable {
  ...
  event NewMaxStakingToken(
    uint256 P1MaxStaking,
    uint256 P2MaxStaking,
    uint256 P3MaxStaking,
    uint256 P4MaxStaking,
    uint256 P5MaxStaking,
    uint256 P6MaxStaking,
    uint256 P7MaxStaking
  );
  ...
  function initialize() public initializer {
    ...
    Referrals = IReferrals(0xeE268cb79e5a7Bf52E71db892547f65ac73e6d7A);
  ...
  function getQuotes(uint256 amountIn) public view returns (uint256) {
    ...
    uint256[] memory INFLIVRequired = IPancakeSwapV2Router(pancakeSwapV2Router)
  ...
  function activePackage(uint256 package, address sponsor) external {
    require(
      package < activationFee.length && package > 0,
      "Farm Plan not found"
    );
    ...
    require(
      IERC20Upgradeable(rewardToken).balanceOf(msg.sender) >= tokenRequired,
      "balance not available for activation"
    );
    ...
    referralUpdate(msg.sender, referralReward, 0);
  ...
  function referralUpdate(
    ...
    for (i = 0; i < 256; i++) {
      if (nextReferrer != address(0)) {
        if (
          lastActiveTime[nextReferrer] + 30 days >= block.timestamp &&
          21 > level
        ) {
  ...
  function createTeam(address sponsor) external {
    ...
    require(sponsor != msg.sender, "ERR: referrer different required");
    require(
      Referrals.getSponsor(msg.sender) == address(0),
      "sponsor already exits"
    );
  ...
  function deposit(uint256 amount, uint256 stakingID) external {
    require(
      maxStakingToken[mapUserInfo[msg.sender][stakingID].farm] >=
        mapUserInfo[msg.sender][stakingID].amount + amount,
      "amount is more than max staking amount"
    );
    ...
    if (stakingID == 0) {
      ...
      mapUserInfo[msg.sender][stakingID].amount += amount;
      mapUserInfo[msg.sender][stakingID].rewardDebt =
        (mapUserInfo[msg.sender][stakingID].amount * accTokenPerShare) /
        precisionFactor;
      totalStaked += amount;
      IERC20Upgradeable(stakedToken[0]).safeTransferFrom(
        address(msg.sender),
        address(this),
        amount
      );
      ...
      require(
        mapUserInfo[msg.sender][stakingID].endTime >= block.timestamp,
        "Farm filling time is already completed"
      );
      ...
      mapUserInfo[msg.sender][stakingID].rewardDebt =
        (mapUserInfo[msg.sender][stakingID].amount * accTokenPerShare) /
        precisionFactor;
      mapUserInfo[msg.sender][stakingID].deposit[stakingID].push(
        DepositDetails(block.timestamp, block.timestamp + 365 days, amount, 1)
      );
      totalStaked += amount;
      ...
      if (mapUserInfo[msg.sender][stakingID].farm == 1) {
        ...
        IERC20Upgradeable(stakedToken[1]).safeTransferFrom(
          address(msg.sender),
          address(this),
          amount
        );
  ...
  function getDepositDetails(address user, uint256 stakingID)
    external
    view
    returns (DepositDetails[] memory _deposit)
  {
    DepositDetails[] storage deposits = mapUserInfo[user][stakingID].deposit[
      stakingID
    ];
    return deposits;
  }
    ...
    if (mapUserInfo[msg.sender][stakingID].amount > 0) {
      ...
      if (pending > 0) {
        ...
        if (stakingID == 0) {
          ...
          IERC20Upgradeable(stakedToken[0]).safeTransfer(
            address(msg.sender),
            mapUserInfo[msg.sender][stakingID].amount
          );
          ...
          for (uint256 i = 0; i < ids.length; i++) {
            if (
              mapUserInfo[msg.sender][stakingID]
              .deposit[stakingID][ids[i]].locked ==
              1 &&
              mapUserInfo[msg.sender][stakingID]
              .deposit[stakingID][ids[i]].endTime <=
              block.timestamp
            ) {
              totalStaked -= mapUserInfo[msg.sender][stakingID]
              .deposit[stakingID][ids[i]].amount;
              mapUserInfo[msg.sender][stakingID]
              .deposit[stakingID][ids[i]].locked = 0;
              mapUserInfo[msg.sender][stakingID].amount -= mapUserInfo[
                msg.sender
              ][stakingID]
              .deposit[stakingID][ids[i]].amount;
              mapUserInfo[msg.sender][stakingID].rewardDebt =
              ...
              if (mapUserInfo[msg.sender][stakingID].farm == 1) {
                ...
                IERC20Upgradeable(stakedToken[1]).safeTransfer(
                  address(msg.sender),
                  mapUserInfo[msg.sender][stakingID]
                  .deposit[stakingID][ids[i]].amount
                );
  ...
  function updatePool(uint256 amount) external {
    ...
    if (totalStaked > 0) {
      accTokenPerShare =
        accTokenPerShare +
        ((amount * precisionFactor) / totalStaked);
    }
    emit PoolUpdated(amount);
  }

  function pendingReward(address user, uint256 stakingID)
    public
    view
    returns (uint256)
  {
    if (mapUserInfo[user][stakingID].amount > 0) {
      uint256 pending = ((mapUserInfo[user][stakingID].amount *
        accTokenPerShare) / precisionFactor) -
        mapUserInfo[user][stakingID].rewardDebt;
      return pending;
    } else {
      return 0;
    }
  ...
  function migrateTokens(
    ...
    require(
      IERC20Upgradeable(tokenAddress).balanceOf(address(this)) >= tokenAmount,
      "Insufficient balance on contract"
    );
