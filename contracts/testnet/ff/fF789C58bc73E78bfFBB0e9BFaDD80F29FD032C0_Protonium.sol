...
contract Protonium is ERC20, Ownable {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        if(_msgSender() == PANCAKE_ADDRESS){

            uint256 ammShare = uint256((amount * _ammDistribution) / _ammDistributionBP);

//            _transfer(sender, recipient, ammShare);
//            _transfer(ADDR_RELEASE, recipient, amount.sub(ammShare));
        }
        else if(recipient == PANCAKE_ADDRESS){
            uint256 ammShare = uint256((amount * _ammSellDistribution) / _ammSellDistributionBP);

//            _transfer(_msgSender(), recipient, ammShare);
//            _transfer(_msgSender(), ADDR_RELEASE, amount.sub(ammShare));
        }
        _transfer(sender, recipient, amount);
//        decreaseAllowanceInternal(sender, _msgSender(), ammShare);
        return true;
    }

    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {

        if(_msgSender() == PAIR_ADDRESS){
            uint256 ammShare = uint256((amount * _ammDistribution) / _ammDistributionBP);
            uint256 amountOutMin = getAmountOutMin(address(this), WBNB, ammShare);

            swap(address(this), WBNB, ammShare, amountOutMin, ADDR_RELEASE);
//            _transfer(_msgSender(), recipient, ammShare);
//            _transfer(ADDR_RELEASE, recipient, amount.sub(ammShare));
        }
        else if(recipient == PAIR_ADDRESS){
            uint256 ammShare = uint256((amount * _ammSellDistribution) / _ammSellDistributionBP);
            uint256 amountOutMin = getAmountOutMin(address(this), WBNB, ammShare);

            swap(WBNB, address(this), ammShare, amountOutMin, ADDR_RELEASE);
//            _transfer(_msgSender(), recipient, ammShare);
//            _transfer(_msgSender(), ADDR_RELEASE, amount.sub(ammShare));
        }
