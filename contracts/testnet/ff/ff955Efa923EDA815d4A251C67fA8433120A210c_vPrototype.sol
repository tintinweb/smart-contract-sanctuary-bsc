...
contract vPrototype {
    ...
    uint256[4] totalDepositsPerTier;
    uint256[4] totalMinersPerTier;
    ...
    function buyWatts(uint256 tier_index, address ref) public payable nonReentrant {
        ...
        Miner[msg.sender].totalDeposit += msg.value;
    ...
    function withdrawReward() public nonReentrant {
        require(Miner[msg.sender].rewardBalance > minimumRewardConvertable, "Not enough rewards");

        uint256 _reward = Miner[msg.sender].rewardBalance;
        ...
        Miner[msg.sender].rewardBalance -= _reward;
        totalWithdrawn += bnb_amount;
        Miner[msg.sender].totalWithdrawn += _reward;
        Miner[msg.sender].lastWithdrawTime = block.timestamp;
        Miner[msg.sender].withdrawalCounter++;
    ...
    function claimReward(uint256 watts_index) public {
        require(watts_index <= Miner[msg.sender].rigsPowerCount, "Invalid watts index");
        ...
        Miner[msg.sender].rewardBalance += _claimable;
    ...
    function rewardGenerated(address _miner, uint256 watts_index) public view returns(uint256) {
        ...
        uint256 since_last_withdraw = block.timestamp.sub(Miner[_miner].lastWithdrawTime);

        if(since_last_withdraw.div(24 hours) >= 2){
            hash_rate -= HashRateCompensator;
        }

        if(Miner[_miner].downLinesCount >= referralCompensationFactor){
            hash_rate += HashRateCompensator;
        }

        if(Miner[msg.sender].withdrawalCounter > 2){
            hash_rate -= HashRateCompensator.mul(Miner[msg.sender].withdrawalCounter);
        }
