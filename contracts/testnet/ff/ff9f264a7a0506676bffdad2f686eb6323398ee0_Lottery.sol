...
interface ILottery {
     ...
     * @param _priceTicketInCake: price of a ticket 
...
contract Lottery is ReentrancyGuard, ILottery, Ownable {
        ...
        nonReentrant
    {
        ...
        // Calculate number of cake tokens to this contract
        ...
        uint256 balanceBefore = cakeToken.balanceOf(address(this));
        cakeToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountTokenToTransfer
        );
        amountTokenToTransfer = cakeToken.balanceOf(address(this)).sub(
            balanceBefore
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInCake = _lotteries[_lotteryId]
            .amountCollectedInCake
            .add(amountTokenToTransfer);
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInCakeToTransfer = rewardInCakeToTransfer.add(
                rewardForTicketId
            );
        }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        amountToWithdrawToTreasury = amountToWithdrawToTreasury
            .add(_lotteries[_lotteryId].amountCollectedInCake)
        ...
        onlyOwnerOrInjector
    {
        ...
        _amount = cakeToken.balanceOf(address(this)).sub(balanceBefore);
     ...
     * @dev Only callable by owner
     * @param _minPriceTicketInCake: minimum price of a ticket 
     * @param _maxPriceTicketInCake: maximum price of a ticket 
     */
