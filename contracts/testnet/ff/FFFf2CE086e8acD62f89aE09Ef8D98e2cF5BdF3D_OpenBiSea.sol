...
contract OpenBiSea is Ownable  {
    ...
    function claimFreeTokens() public returns (bool, uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tokensToPay;
        uint256 amountTotalForCustomer;
        uint256 percentOfSales;

        (tokensToPay, amountTotalForCustomer, percentOfSales,)= checkTokensForClaim(msg.sender,priceMainToUSD.div(10 ** uint256(decimals)));


        uint256 delta = initialBalance.sub(tokensaleTotalSold);
        uint256 newPriceOBS = initialPrice.mul(initialBalance).div(delta);
        uint256 incomeInOBSfromUser = amountTotalForCustomer.div(newPriceOBS);
        if (tokensToPay > incomeInOBSfromUser) tokensToPay = incomeInOBSfromUser / 100; // can't reward more than 1% of customer income
        ...
        emit ClaimFreeTokens(tokensToPay, msg.sender);//, amountTotalForCustomerUSDwei, incomeInOBSfromUser, percentOfSales, newPriceOBS, priceMainToUSD);
        return (result, amountTotalForCustomer, incomeInOBSfromUser, newPriceOBS, priceMainToUSD, tokensToPay);
    }
