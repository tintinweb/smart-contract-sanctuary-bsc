...
contract ApeSwapPoolManager is Ownable {
    ...
    function viewVotingPower(address userAddress, address stakingToken) external view returns (uint256) {
        uint256 stakedBalance;
        uint256 holdingBalance = IERC20(stakingToken).balanceOf(userAddress);
        address[] memory activePools = this.allActivePools();
        
        for (uint256 i = 0; i < activePools.length; i++) { 
            address pool = activePools[i];
            IRewardPool.UserInfo memory userInfo = IRewardPool(pool).userInfo(userAddress);
...
interface IRewardPool {
    ...
    function userInfo(address) external view returns (UserInfo memory userInfo);
}
