...
contract OpenBiSea is Ownable  {
    ...
    function claimFreeTokens() public returns (bool) {
        ...
        (tokensToPay, amountTotalUSDwei, percentOfSales,)= checkTokensForClaim(msg.sender,priceMainToUSD.div(10 ** uint256(decimals)));
        uint256 delta = initialBalance.sub(tokensaleTotalSold);
...
//        uint256 usdPriceOBS = newPriceOBS.mul(priceMainToUSD).div(10 ** decimals);
        uint256 incomeInOBSfromUser = amountTotalUSDwei.div(priceMainToUSD).div(newPriceOBS).mul(percentOfSales).div(10000);
        if (tokensToPay > incomeInOBSfromUser) tokensToPay = incomeInOBSfromUser.div(100).mul(1); // can't reward more than 1% of customer income
