...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterfaceG2, ComptrollerErrorReporter, ExponentialNoError {
    ...
    event DistributedSupplierVenus(EToken indexed eToken, address indexed supplier, uint venusDelta, uint venusSupplyIndex);

    /// @notice Emitted when XVS is distributed to a borrower
    event DistributedBorrowerVenus(EToken indexed eToken, address indexed borrower, uint venusDelta, uint venusBorrowIndex);
    ...
    function mintAllowed(address eToken, address minter, uint mintAmount) external onlyProtocolAllowed override returns (uint) {
        ...
        updateVenusSupplyIndex(eToken);
        distributeSupplierVenus(eToken, minter);
    ...
    function redeemAllowed(address eToken, address redeemer, uint redeemTokens) external override onlyProtocolAllowed returns (uint) {
        ...
        // Keep the flywheel moving
        updateVenusSupplyIndex(eToken);
        distributeSupplierVenus(eToken, redeemer);
    ...
    function borrowAllowed(address eToken, address borrower, uint borrowAmount) external override onlyProtocolAllowed returns (uint) {
        ...
        updateVenusBorrowIndex(eToken, borrowIndex);
        distributeBorrowerVenus(eToken, borrower, borrowIndex);
    ...
    function transferAllowed(address eToken, address src, address dst, uint transferTokens) external override onlyProtocolAllowed returns (uint) {
        ...
        // Keep the flywheel moving
        updateVenusSupplyIndex(eToken);
        distributeSupplierVenus(eToken, src);
        distributeSupplierVenus(eToken, dst);
     ...
     * @return (errorCode, number of vTokenCollateral tokens to be seized in a liquidation)
     */
    function liquidateVAICalculateSeizeTokens(address vTokenCollateral, uint actualRepayAmount) external view override returns (uint, uint) {
        /* Read oracle prices for borrowed and collateral markets */
    ...
    function setVenusSpeedInternal(EToken eToken, uint venusSpeed) internal {
        uint currentVenusSpeed = venusSpeeds[address(eToken)];
        if (currentVenusSpeed != 0) {
            ...
            updateVenusSupplyIndex(address(eToken));
            updateVenusBorrowIndex(address(eToken), borrowIndex);
        } else if (venusSpeed != 0) {
            // Add the XVS market
            Market storage market = markets[address(eToken)];
            require(market.isListed == true, "venus market is not listed");

            if (venusSupplyState[address(eToken)].index == 0 && venusSupplyState[address(eToken)].block == 0) {
                venusSupplyState[address(eToken)] = VenusMarketState({
        ...
        if (venusBorrowState[address(eToken)].index == 0 && venusBorrowState[address(eToken)].block == 0) {
                venusBorrowState[address(eToken)] = VenusMarketState({
                    index: venusInitialIndex,
                    block: safe32(getBlockNumber(), "block number exceeds 32 bits")
                });
            }
        ...
        if (currentVenusSpeed != venusSpeed) {
            venusSpeeds[address(eToken)] = venusSpeed;
            emit VenusSpeedUpdated(eToken, venusSpeed);
        }
    }
     ...
     */
    function updateVenusSupplyIndex(address eToken) internal {
        VenusMarketState storage supplyState = venusSupplyState[eToken];
        uint supplySpeed = venusSpeeds[eToken];
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            ...
            venusSupplyState[eToken] = VenusMarketState({
     ...
     */
    function updateVenusBorrowIndex(address eToken, Exp memory marketBorrowIndex) internal {
        VenusMarketState storage borrowState = venusBorrowState[eToken];
        uint borrowSpeed = venusSpeeds[eToken];
        uint blockNumber = getBlockNumber();
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            ...
            venusBorrowState[eToken] = VenusMarketState({
                index: safe224(index.mantissa, "new index overflows"),
                block: safe32(blockNumber, "block number overflows")
            });
        } else if (deltaBlocks > 0) {
     ...
     */
    function distributeSupplierVenus(address eToken, address supplier) internal {
        ...
        Double memory supplierIndex = Double({mantissa: venusSupplierIndex[eToken][supplier]});
        venusSupplierIndex[eToken][supplier] = supplyIndex.mantissa;
        ...
        emit DistributedSupplierVenus(EToken(eToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
     ...
     */
    function distributeBorrowerVenus(address eToken, address borrower, Exp memory marketBorrowIndex) internal {
        if (address(vaiVaultAddress) != address(0)) {
            releaseToVault();
        }
        ...
        Double memory borrowerIndex = Double({mantissa: venusBorrowerIndex[eToken][borrower]});
        venusBorrowerIndex[eToken][borrower] = borrowIndex.mantissa;
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            emit DistributedBorrowerVenus(EToken(eToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
     ...
     */
    function claimVenus(address holder, EToken[] memory vTokens) public {
     ...
     */
    function claimVenus(address[] memory holders, EToken[] memory vTokens, bool borrowers, bool suppliers) public {
        uint j;
        for (uint i = 0; i < vTokens.length; i++) {
            EToken eToken = vTokens[i];
            require(markets[address(eToken)].isListed, "not listed market");
            if (borrowers) {
                Exp memory borrowIndex = Exp({mantissa: eToken.borrowIndex()});
                updateVenusBorrowIndex(address(eToken), borrowIndex);
                for (j = 0; j < holders.length; j++) {
                    distributeBorrowerVenus(address(eToken), holders[j], borrowIndex);
            ...
            if (suppliers) {
                updateVenusSupplyIndex(address(eToken));
                for (j = 0; j < holders.length; j++) {
                    distributeSupplierVenus(address(eToken), holders[j]);
                    venusAccrued[holders[j]] = grantXVSInternal(holders[j], venusAccrued[holders[j]]);
                }
            }
        }
     ...
     * @param eToken The market whose XVS speed to update
     * @param venusSpeed New XVS speed for market
     */
    function _setVenusSpeed(EToken eToken, uint venusSpeed) public {
        require(adminOrInitializing(), "only admin can set venus speed");
        setVenusSpeedInternal(eToken, venusSpeed);
    }
...
contract XVS is Tokenlock {
    ...
    uint96 constant MAX_INT96 = 2**96 - 1;
    ...
    function transferFrom(address src, address dst, uint rawAmount) external validLock returns (bool) {
        ...
        if (spender != src && spenderAllowance != MAX_INT96) {
