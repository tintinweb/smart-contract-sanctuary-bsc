 ...
 */
contract BasicNodeStakeContractTEST3 is Ownable {
    ...
    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 referralBonus){
		if(inviteContract.getMyReferrCount(msg.sender) > 0) {
			referralBonus = pendingReward(_pid,_user).mul(bonusPerReferral).div(referralRateDenominator).mul(inviteContract.getMyReferrCount(msg.sender));
		}
    ...
    function judgmentThreshold(uint256 _pid) public view returns(bool isReached) {
        ...
        uint256 mylpTokenSupply = pool.lpToken.balanceOf(msg.sender);
