...
// File: contracts/asserts/FixedAsserts.sol
...
contract FixedAsserts is ERC721 {
    ...
    event Cast(uint256 tokenId, address initialOwner, address coffer, string receiptNum, address operator, address NFTAddr, uint256 NFTId, address tokenAddr, uint256 tokenAmount, uint256 timestamp);
    ...
    constructor(
        ...
        manager = newAssert.manager;
        owner = newAssert.owner;
        baseURI = newAssert.baseURI;
        priceRoot = newAssert.priceRoot;
    }
    ...
    pure
    returns (bytes32) {
        return keccak256(abi.encode(token.tokenId, token.feeToken, token.feeAmount));
    }
    ...
    */
    ...
    returns (bool) {
        ...
        if (token.feeToken == address(0)) {
            require(msg.value >= token.feeAmount, "Insufficient ETH fee");
            payable(address(coffer)).transfer(token.feeAmount);
            ...
            if (amount < token.feeAmount) {
                revert("Insufficient ERC20 approve");
            } else {
                bool success = ERC20Token.transferFrom(tx.origin, address(coffer), token.feeAmount);
                require(success, "ERC20 transferFrom fail");
                require(CofferIface(coffer).sendReceipt(receipt), "Receipt sending failed");
            }
        }

        prices[token.tokenId].priceToken = token.feeToken;
        prices[token.tokenId].priceAmount = token.feeAmount;

        emit Cast(token.tokenId, tx.origin, address(coffer), receipt.receiptNum, receipt.operator, receipt.NFTAddr, receipt.NFTId, receipt.tokenAddr, receipt.tokenAmount, receipt.timestamp);
        return true;
    }
    ...
    view
    returns (bytes32) {
        return keccak256(abi.encode(address(this), tokenId, feeToken, feeAmount));
    }
    ...
    returns (
        ...
        return (prices[tokenId].priceToken, prices[tokenId].priceAmount);
    }
    ...
    returns (bytes32 hash){
        bytes memory message = abi.encode(token.to, address(this), token.tokenId, token.feeToken, token.feeAmount);
...
// File: contracts/asserts/FixedAssertsFactory.sol
...
contract FixedAssertsFactory is Ownable {
    ...
    returns (address) {
        ...
        newAssert.priceRoot = priceRoot_;
        newAssert.coffer = coffer;
        newAssert.manager = manager;
        ...
        chargeFee.assert = address(asserts);
    ...
    returns (bool) {
        ...
        CofferIface.Receipt storage receipt;
        string memory receiptNum = string(abi.encodePacked(address(this).toHexString(), "#", uint256(uint160(chargeFee.assert)).toString()));
        receipt = receipts[receiptNum];
        receipt.receiptNum = receiptNum;
        receipt.operator = tx.origin;
        receipt.NFTAddr = address(this);
        receipt.NFTId = uint256(uint160(chargeFee.assert));
        ...
        emit NewAsserts(chargeFee.businessId, chargeFee.assert, address(coffer), receipt.receiptNum, receipt.operator, receipt.NFTAddr, receipt.NFTId, receipt.tokenAddr, receipt.tokenAmount, receipt.timestamp);
        return true;
    }
    ...
    view
    returns (bytes32 hash){
        bytes memory message = abi.encode(newAssert.name, newAssert.symbol, newAssert.baseURI, newAssert.totalSupply, newAssert.priceRoot, ChargeFee.feeToken, ChargeFee.feeAmount, msg.sender);
        hash = keccak256(message);
    }
