...
pragma solidity ^0.8.0;
pragma abicoder v2;
contract VARPredictionMarket is Ownable, Pausable, ReentrancyGuard {
    ...
    function betDown(
        ...
        require(
            verifyMessage(epoch, msg.sender, signature),
            "Invalid signature"
        );
    ...
    function claim(uint256[] calldata epochs) external nonReentrant {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            require(
                rounds[epochs[i]].status != Status.NotInitialized,
                "Round not started"
            );
            uint256 addedReward = 0;

            // Round valid, claim rewards
            if (rounds[epochs[i]].status == Status.Ended) {
                require(
    ...
    function refundable(
        ...
        Round memory round = rounds[epoch];
        return
            round.status != Status.NotInitialized &&
            round.status != Status.Ended &&
            (rounds[epoch + 1].status != Status.NotInitialized ||
                rounds[epoch + 2].status != Status.NotInitialized) &&
            !betInfo.claimed &&
            round.epoch < currentEpoch - 2 &&
            betInfo.amount != 0;
    }
    ...
    function _safeEndRound(uint256 epoch, int256 price) internal {
        require(
            rounds[epoch].status == Status.Locked,
    ...
    function _safeLockRound(uint256 epoch, int256 price) internal {
        require(
            rounds[epoch].status == Status.Started,
    ...
    function _safeStartRound(uint256 epoch) internal {
        ...
        require(
            rounds[epoch - 2].status == Status.Ended,
            "Can only start round after round n-2 has ended"
        );
        require(
            rounds[epoch].status == Status.NotInitialized,
            "Can only start round when not started"
        );
        _startRound(epoch);
    }
     ...
     * Round must have started and next round must have not started
     */
    ...
    /**
     * @notice Verify message signed by verify to check if user has right to bet
     */
    function verifyMessage(
        ...
        bytes32 dataHash = encodeData(epoch, user);
    ...
    function encodeData(
        ...
        return keccak256(abi.encode(id, epoch, user, address(this)));
    }
}