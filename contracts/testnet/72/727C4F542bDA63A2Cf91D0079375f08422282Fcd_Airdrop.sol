...
contract Airdrop is Token {
  ...
  event RewardToken(uint256 dropId, IBEP20 token, address claimer, uint256 amount);
  event StartAirdrop(address token, uint256 airdropamount, uint256 releasetime);
  modifier onlyadmin() {
    if (msg.sender == admin) {}
    _;
  }
  ...
  function startAirdrop(
    ...
    IBEP20(_token).transferFrom(msg.sender, address(this), _maxAirdropAmount);
    ...
    emit StartAirdrop(_token, _maxAirdropAmount, releasetime);
  }
  ...
  function RewardTokens() public {
    ...
    alldrops.push(Drops(currentdropId, token, msg.sender, amount, releasetime, false));
    _unlockedDropsNumber.increment();
    emit Claimtoken(currentdropId, token, msg.sender, amount, releasetime);
  }

  function withdrawTokens(IBEP20 _token, uint256 dropId) public {
    ...
    require(drop.releasetime < block.timestamp, "you must wait for unlock!");
