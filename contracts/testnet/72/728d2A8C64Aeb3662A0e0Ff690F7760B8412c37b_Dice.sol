...
contract Dice is Ownable, VRFConsumerBaseV2 {
    struct Fees {
        uint16 treasury;
        uint16 burn;
        uint16 swap;
    ...
    struct Game {
        uint8 direction; //0-under, 1-over
        uint8 rolledNumber;
        uint32 timestamp;
        address betToken;
        address player;
    ...
    constructor(uint64 _subscriptionId) VRFConsumerBaseV2(vrfCoordinator) {
        ...
        tokenInfo[0x77c21c770Db1156e271a3516F89380BA53D594FA] = Token({
            isActive: true,
    ...
    function playDice(uint8 _overOrUnder, address _token, uint256 _betAmount) external payable {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        Game storage user = games[game.player][totalPlayed[game.player]];

        uint256 randomNum = randomWords[0] % 101;
        if ((game.direction == 0 && randomNum <= rollUnder) || (game.direction == 1 && randomNum >= rollOver)) {
