...
contract MicroMachineManufacturingPlant is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable  {
    ...
    address payable public royaltyContract = payable(msg.sender);
    ...
    constructor(address _EngineEcosystemContractAddress ) ERC721("MicroMachineWarBots", "MMWarBot") {
        enginecontract = EngineEcosystemContract ( _EngineEcosystemContractAddress );
    ...
    function initEngineEcosystemContractAddress ( address _address ) public onlyOwner {
        require ( _address != address(0) , "No Zero Address");
        enginecontract = EngineEcosystemContract ( _address );
    }

    function setDepositandTokenIDandDataCost ( uint256 _deposits, uint256 _tokenid, uint256 _datacost ) public onlyOwner {
        deposits = _deposits;
        _tokenIds = _tokenid;
        dataCost = _datacost;
    }
   ...
   uint256 public dataCost; 
   function migrateMyNext ( ) public payable {
       require ( msg.value == dataCost , "AVAX required");
       RequestData _request = RequestData(enginecontract.returnAddress ( "RequestData"));
       (uint256 _type,  uint256 _id) = _request.getNext(msg.sender);
       RequestsCount++;
       Requests[RequestsCount]._type = _type;
       Requests[RequestsCount]._id = _id;
    ...
    function upgradePlant ( uint256 _plant ) public {
        ...
        ERC20 _nano = ERC20(enginecontract.returnAddress ( "NMAC"));
        _nano.transferFrom ( msg.sender , address(this), calculatePlantUpgradeCost( _plant, ManufacturingPlants[_plant]._level ));
        _nano.transfer( enginecontract.returnAddress ( "NanoReserve"), calculatePlantUpgradeCost( _plant, ManufacturingPlants[_plant]._level ));
    }
    ...
    function setUpgradeCostAndMaxPlantLevel(  uint8 _maxlevel, uint256 _cost ) public onlyEngine {
    ...
    function stakeMicroMachines( uint256 _units, uint256 _timeperiod , Location memory _location ) public {
        ...
        ERC20 _token = ERC20(enginecontract.returnAddress ( "MMAC"));
   ...
   function freezeToggle () public onlyOwner {
       freeze = !freeze;
   }
    ...
    modifier onlyEngine() {
        ...
        require ( enginecontract.isEngineContract(msg.sender), "Not an Engine Contract");
         _;
    }
