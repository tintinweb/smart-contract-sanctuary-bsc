...
abstract contract SeedSlippage {
    ...
    function sellSlippage(uint256 amountToWei, bool maxSlippageFlag) external virtual returns (address [] memory slippageAddress, uint256 [] memory slippageAmount);
}
...
contract SeedToken is IERC20, Ownable, Util {
    ...
    mapping(uint256 => mapping(address => uint256)) private protectPoolRecord;
    ...
    function _transfer(
        ...
        if(sender == swapRouter && !_isExcludedFromFee[recipient]) {
            ...
            // sell
            bool maxSlippageFlag = false;
            if(block.timestamp - lastPurchaseTime[sender] <= shortestTradingTime) {
                maxSlippageFlag = true;
            }

            uint256 slippageAmount = 0;
            (address [] memory slippageAddresses, uint256 [] memory slippageAmounts) = seedSlippage.sellSlippage(amount, maxSlippageFlag);
            for(uint8 i=0; i<slippageAddresses.length; i++) {
                if(slippageAddresses[i] != address(0)) {
                    slippageAmount = slippageAmount.add(slippageAmounts[i]);
                    _balances[slippageAddresses[i]] = _balances[slippageAddresses[i]].add(slippageAmounts[i]);
            ...
            uint256 seedToUsdtPrice = querySeedToUsdtPrice(); // unit256: wei
            uint256 protectAmount = usdtToken.balanceOf(address(this));
            uint256 overplusAmount = seedToUsdtPrice.mul(amount.sub(slippageAmount)).div(10 ** 18);
            if(protectAmount >= overplusAmount) {
                protectPoolRecord[block.number][sender] = overplusAmount;
    ...
    function getProtectPoolRecord(uint256 _number, address _address) public view returns(uint256 amountToWei) {
        return protectPoolRecord[_number][_address];
    }
    ...
    function queryUsdtToSeedPrice() public view returns (uint256) {
        uint256 reserveA = _balances[swapRouter];
        uint256 reserveB = usdtToken.balanceOf(swapRouter);
