...
contract Coin11 is Initializable, ContextUpgradeable, OwnableUpgradeable, BEP20 {
    ...
    address payable public liquidityTaxWallet;
    ...
    address payable public taxWallets;
    ...
    function initialize () external initializer
    {
        ...
        marketingTaxWallet = payable(getMarketingTaxWallet());
        liquidityTaxWallet = payable(getLiquidityTaxWallet());
        ...
        denominator = 100;
        taxStatus = true;
        swapThreshold = 10 wei; // The contract will only swap to ETH, once the fee tokens reach the specified threshold
        marketingTaxBuy = 5;
        liquidityTaxBuy = 5;
     ...
     */
    function handleTax(address from, address to, uint256 amount) public payable returns (uint256) {
        require(taxStatus, "Tax Status Inactive");
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            uint256 marketingBaseUnitBuy = amount.div(denominator).mul(marketingTaxBuy);
            uint256 liquidityBaseUnitBuy = amount.div(denominator).mul(liquidityTaxBuy);
            uint256 marketingBaseUnitSell = amount.div(denominator).mul(marketingTaxSell);
            uint256 liquidityBaseUnitSell = amount.div(denominator).mul(liquidityTaxSell);
            if(from == address(uniswapV2Pair)) {
                tax = marketingBaseUnitBuy + liquidityBaseUnitBuy;

                
                if(tax > 0) {
                    _transfer(from, marketingTaxWallet, marketingBaseUnitBuy);   
                }
                ...
                tax = marketingBaseUnitSell + liquidityBaseUnitSell; 
                
                if (tax > 0) {
                    _transfer(from, marketingTaxWallet, marketingBaseUnitSell);   
                }
            }
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                taxSum = liquidityBaseUnitBuy;

            }
            // SELL
            else if(to == address(uniswapV2Pair)) {
                taxSum = liquidityBaseUnitSell;
            ...
            if(ethValue >= swapThreshold) {
                ...
                (uint256 amountToken, uint256 amountETH, uint256 liquidity) = uniswapV2Router02.addLiquidityETH{value: liquidityETH}(
                    address(this),
                    liquidityToken,
                    0,
                    0,
                ...
                if(remainingTokens > 0) {
                    _transfer(address(this), marketingTaxWallet, remainingTokens);
                }
                
                
                (bool success, /* bytes memory data */) = payable(marketingTaxWallet).call{value: marketingETH}("");
                require(success, "receiver rejected ETH transfer");

                
                if(ethGained - (marketingETH + liquidityETH) > 0) {
                    (bool _success, /* bytes memory data */) = payable(marketingTaxWallet).call{value: ethGained - (marketingETH + liquidityETH)}("") ;
                require(_success, "receiver rejected ETH transfer");
    ...
    function _transfer(
        ...
        if(taxStatus) {
            calculated_tax = handleTax(sender, recipient, amount);   
            super._transfer(sender, recipient, calculated_tax);
        }
    ...
    /**
     * @dev Sets wallets for Marketing taxes.
     */
    function setMarketingTaxWallet(address payable marketing) public  onlyOwner {
        marketingTaxWallet = marketing;
    }

    /**
     * @dev Gets wallets for Marketing taxes.
     */
    function getMarketingTaxWallet() public view onlyOwner returns (address) {
        return marketingTaxWallet;
    }

    /**
     * @dev Sets wallets for Liquidity taxes.
     */
    function setLiquidityTaxWallet(address payable liquidity) public  onlyOwner {
        liquidityTaxWallet = liquidity;
    }

    /**
     * @dev Gets wallets for Liquidity taxes.
     */
    function getLiquidityTaxWallet() public view onlyOwner returns (address) {
        return liquidityTaxWallet;
    }
    ...
    function getUniswapV2Router02() public view returns (IUniswapV2Router02) {
