...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  ///     but keeping track of such data on-chain costs ROGh more than
...
contract ROGDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("ROG_Dividend_Tracker", "ROG_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100000 * (10**9); //must hold 100000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ROG_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ROG_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ROG contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ROG_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "ROG_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ROG is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    ROGDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    uint256 public maxBuyTransactionAmount = 100000000 * (10**9);
    uint256 public swapTokensAtAmount = 1000000 * (10**9);

    uint256 public BNBRewardFee;
    ...
    function setFee(uint256 _BNBRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _burnFee) public onlyOwner {
        ...
        totalFees = BNBRewardFee.add(liquidityFee).add(marketingFee).add(burnFee).add(burnFee); // total fee transfer and buy
    }
...
/**
TOKEN NAME Ride of Gamers
SYMBOL ROG
MAXIMUM SUPPLY 1,000,000,000 TOKEN
...
TRANSACTION TAX 12%
4% Burn
2% All token holders in BNB
5% Liquidity to pancakeswap
1% Marketing
*/
    constructor() ERC20("Ride of Gamers", "ROG") {
        ...
        marketingFee = 1;
        burnFee = 4;
        marketingWallet = payable(0x9bcBE4Af33C43E12c82d2fcD77DF6AF7F82174A2);

        totalFees = BNBRewardFee.add(liquidityFee).add(marketingFee).add(burnFee); // total fee transfer and buy

    	dividendTracker = new ROGDividendTracker();
 //   	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ROG: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ROG: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ROG: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ROG: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ROG: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ROG: Cannot update gasForProcessing to same value");
