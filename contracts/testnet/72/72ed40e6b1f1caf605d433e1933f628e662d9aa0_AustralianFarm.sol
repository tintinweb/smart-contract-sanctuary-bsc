...
contract AustralianFarm is Ownable{
    ...
    function putBuliding(uint256 landTokenId,BackpackType backpackType)public{
            ...
            if(num < cowshedsOnTheLandNum){
                ...
                if(num >= 1){        
                    ...
                    for(uint256 i=0; i<crops.length; i++){
                        ...
                        // 公牛母牛合养
                        if(userCrops[crops[i]].cropType != CropType.Cattle && crop.cropType != CropType.Cattle && userCrops[crops[i]].cropType != crop.cropType){
    ...
    function NFTSale(BackpackType backpackType,uint256 price,uint256 amount) public {
        ...
        for(uint256 i = 1; i <= userBackpacks[msg.sender][backpackType].count; i ++ ){
            ...
            if(count <= amount){
                ...
                bool set = _setNFTLock(tokenId, true) && farmNFT.setNFTLock(tokenId,true);
    ...
    function cancleNFTSale(uint256 saleIndex) public {
        ...
        for(uint256 i = 0; i < NFTGoods[saleIndex].surplusAmount; i ++ ){
            ...
            bool set = _setNFTLock(tokenId, false) && farmNFT.setNFTLock(tokenId,false);
    ...
    // 获取土地tokenid
    function getMyLandTokenIds()public view returns(uint256[] memory){
        return userLandTokenIds[msg.sender];
    }
    ...
    function _putInTheBackpack(address owner,BackpackType backpackType,bool isPut,uint256 tokenId) private returns(uint256){
       ...
       if(isPut){
           ...
           for(uint256 i = 1; i <= userBackpacks[owner][backpackType].count; i++){
               ...
               if(!isLock[userBackpacks[owner][backpackType].tokenIds[i - 1]]){
