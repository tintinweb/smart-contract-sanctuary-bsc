    ...
    Ownable
{
    ...
    constructor(
        ...
        ] = true; // PCS Testnet router @note disabled
    }
...
contract AltSwitch is ERC20, Ownable2Step {
    ...
    function _transfer(
        ...
        // @note Remove fees for wallet-to-wallet transfer
        ...
        // Wallet-to-wallet transfer rebate
        if (
            isRebated(from) &&
            !automatedMarketMakerPairs[from] &&
            !automatedMarketMakerPairs[to] &&
            ...
            // Substract rebate to `amount` and stay on `from` wallet address
            amount = amount.sub(rebate); // @note `from` address will only send this amount to recipient
        }
