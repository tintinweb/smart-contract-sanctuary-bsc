...
contract Anubit is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20 {
   ...
   function version() public pure virtual returns (string memory) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(_merchantSellFee[sender]){
            //set custom div
            customFees.bounty = customDiv[sender];
            customFees.liq = customDiv[sender];
            customFees.refl = customDiv[sender]; 
        }
    ...
    function customMerchantFees(
        ...
        for (uint256 i = 0; i < wallets.length; i++) {
            ...
            //if (_isMerchant[account]) continue;
