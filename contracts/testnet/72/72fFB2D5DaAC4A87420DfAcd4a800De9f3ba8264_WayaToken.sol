...
contract WayaToken is ERC20, BasicAccessControl {
    ...
    constructor (string  memory _tokenName, 
        ...
        _setupRole(TaskMaster,       _taskMaster);

        _setRoleAdmin(Contract_Manager, Contract_Manager);
        _setRoleAdmin(TaskMaster,       Contract_Manager);

    }

    /// @dev Creates `_amount` token to `_to`. Must only be called by the TaskMaster.
    function mint(address _to, uint256 _amount) public onlyRole(TaskMaster) {
        _mint(_to, _amount);
