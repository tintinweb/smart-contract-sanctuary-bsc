...
contract GsToken is Ownable {
    ...
    function getMarket(uint8 _id) internal view returns (Market memory) {
        if (_id == 0) {
            _id = 1;
        }
    ...
    function getUserLoadSize() public view returns (uint256) {
        return userLoads[msg.sender].length;
    }

    function getUserLoads() public view returns (UserLoad[] memory) {
