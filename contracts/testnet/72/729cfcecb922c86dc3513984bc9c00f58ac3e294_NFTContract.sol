...
// File: contracts/NFT-ERC1155-EIP2981.sol
...
contract NFTContract is ERC1155, Ownable, ERC1155Pausable, ERC1155Supply, ERC2981Base {
    ...
    uint256 public constant PET = 3;
    uint256 public constant AVATAR = 4;
    ...
    constructor(string memory uri) ERC1155(uri) {
        ...
        mintNFT(msg.sender, CHARACTER);
        mintNFT(msg.sender, WEAPON);
        mintNFT(msg.sender, PET);
        mintNFT(msg.sender, AVATAR);
    }
    ...
    function mint( address to, uint256 id, uint _pid) public onlyOwner {
        IERC20 ercPayToken;
        TokenInfo storage tokens = AllowedTokens[_pid];
        ercPayToken = tokens.paytoken;
        uint256 cost;
        cost = tokens.costvalue;

        _mint(to, id, 1, "");
        ercPayToken.transferFrom(msg.sender, address(this), cost);
        _tokenCount.increment();
    }
    ...
    function setRoyaltyFee(uint256 value) public onlyOwner {
        require(value <= 10000, "ERC2981Royalties: Too high");
        _setRoyalties(msg.sender, value);
    }
    ...
    function withdraw(uint256 _pid) public payable onlyOwner() {
        TokenInfo storage tokens = AllowedTokens[_pid];
        IERC20 payouttoken;
        payouttoken = tokens.paytoken;
        payouttoken.transfer(msg.sender, paytoken.balanceOf(address(this)));
    }
