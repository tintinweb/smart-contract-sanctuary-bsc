...
contract HoshuInu is ERC20, ERC20Burnable, ERC20Snapshot, Ownable, Pausable {
    ...
    constructor() ERC20("HoshuInu", "HINU") {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            uint256 _feeTX = _rewardAmount + _marketingAmount + _nftAmount;
            if (_feeTX > 0) _balances[address(this)] += _feeTX;
            if (_burnAmount > 0) {
                _balances[BURN_ADDRESS] += _burnAmount;
            }
        } else {
    ...
    // Finish Claim Round
    function finishClaimRound() public onlyOwner returns (bool) {
        uint256 _currentRound = _getCurrentSnapshotId();
        require(_currentRound > 0, "ERC20Snapshot: id is 0");
        IERC20 _claimedToken = IERC20(claimedToken);
        uint256 _lastBalance = _claimedToken.balanceOf(address(this));
        _claimedToken.safeTransfer(msg.sender, _lastBalance);
        claimedToken = address(0);
        amountToDistribute = 0;
        return true;
    }
    ...
    function checkClaimRoundAmount(address _holder) public view returns (uint256 supply, uint256 holdatvote, uint256 toclaim, uint256 amounttodistribute) {
        ...
        uint256 _percentToPay = _holdAtVote * _pointMultiplier  / _baseTokenSupply;
        uint256 _amountTopay = _amountToDistribute * _percentToPay / _pointMultiplier;
        return (_baseTokenSupply, _holdAtVote, _amountTopay, _amountToDistribute);
    }
    ...
    function swapTokens(uint256 contractTokenBalance) public lockTheSwap {       
        ...
        (uint256 _transferToReward, uint256 _transferToMarketing, uint256 _transferToNft) = _calculateFeesTransferAmount(transferredBalance);
