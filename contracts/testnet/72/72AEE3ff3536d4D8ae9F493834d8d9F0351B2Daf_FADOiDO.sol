...
// File: contracts/FADOiDO/FADOiDO.sol
...
contract FADOiDO {
    ...
    ERC20 public investToken; // Token that use to buy presale token
    ...
    mapping(address => bool) public claimedTGE; // if claimed = true, the token already claimed. Only allow claim when claim != true
    mapping(address => uint256) public lastClaimed; // last claimed timestamp of each investor
    mapping(address => uint256) public claimEndTime; // claim end time of each investor
    ...
    uint256 public vestingPeriod = 365 days; // period of time when vesting takes place
        ...
        presaleIsNotCancelled
    {
        ...
        require(_amount <= tokensLeft.mul(tokenPriceInWei));
        uint256 totalInvestmentInWei = investments[msg.sender].add(_amount);
        require(
        ...
        require(investToken.transferFrom(msg.sender, address(this), _amount), "Transfer investToken fail");

        if (investments[msg.sender] == 0) {
            totalInvestorsCount = totalInvestorsCount.add(1);
        }

        lastClaimed[msg.sender] = claimTime;
        if(block.timestamp > claimTime){
            lastClaimed[msg.sender] = block.timestamp;
        }
        claimEndTime[msg.sender] = lastClaimed[msg.sender].add(vestingPeriod);

        totalCollectedWei = totalCollectedWei.add(_amount);
        investments[msg.sender] = totalInvestmentInWei;
        tokensLeft = tokensLeft.sub(getTokenAmount(_amount));
    }
        ...
        onlyClaimAllowed
    {
        ...
        if(!claimedTGE[msg.sender]){
            claimedTGE[msg.sender] = true; // make sure only claim one time
            pending = pending.add(amount.mul(500).div(10000));
        }

        uint256 vestingAmount = amount.mul(9500).div(10000);
        if(claimEndTime[msg.sender] <= block.timestamp){
            pending = pending.add(vestingAmount.mul(claimEndTime[msg.sender].sub(lastClaimed[msg.sender])).div(vestingPeriod));
        }
        else{
            pending = pending.add(vestingAmount.mul(block.timestamp.sub(lastClaimed[msg.sender])).div(vestingPeriod));
        }
        ...
        onlyRefundAllowed
    {
        if (!presaleCancelled) {
            ...
            require(claimedTGE[msg.sender] != true, "Already claimed");
        }

        claimedTGE[msg.sender] = true; // make sure this goes first before transfer to prevent reentrancy
        uint256 investment = investments[msg.sender];
        uint256 presaleBalance = investToken.balanceOf(address(this));
        ...
        if (investment > 0) {
            investToken.transfer(msg.sender, investment);
        }
    }
    ...
    function collectFundsRaised() external onlyPoolOwner {
        ...
        if (investToken.balanceOf(address(this)) > 0) {
            uint256 balance = investToken.balanceOf(address(this));
            if (withdrawFee == 0) {
                investToken.transfer(devAddress, balance);
            } else {
                uint256 fee = balance.mul(withdrawFee).div(100000);
                investToken.transfer(devAddress, balance.sub(fee));
                investToken.transfer(feeAddress, fee);
            }
        }
    }
