...
contract HeroNFT is BaseNFT {
  ...
  function mintHeroesForTest(address[] calldata _owners, uint16[] calldata _classIds, uint _price, uint8 _level, uint _quantity, uint[3] calldata skills) onlyMintAdmin external {
    for(uint i = 0; i < _owners.length; i++) {
      for(uint j = 0; j < _classIds.length; j++) {
        for(uint k = 0; k < _quantity; k++) {
          _mintForTest(_owners[i], _classIds[j], _price, k, _level, skills);
        }
      }
    }
  }
  ...
  function _mintForTest(address _owner, uint16 _classId, uint _price, uint _index, uint8 _level, uint[3] calldata skills) private returns (uint) {
    require(_classId >= 1 && _classId <= countId, 'NFT: Invalid class');
    uint newId = _genNewId(_index);
    _mint(_owner, newId);
    heroes[newId].id = _classId;
    heroes[newId].level = _level;
    heroes[newId].experience = experienceCheckpoint[_level];
    heroes[newId].createdAt = block.timestamp;
    heroes[newId].ownPrice = _price;
    heroes[newId].skills = skills;
    return newId;
  }
