 ...
 */
contract BasicNodeStakeContract is Ownable {
    ...
    function add(IERC20 _lpToken,uint256 _allocPoint,uint256 _threshold,uint256 _thresholdRateDenominator) public onlyOwner nonDuplicated(_lpToken) {
    ...
    function set(uint256 _pid,IERC20 _newLpToken,uint256 _allocPoint,uint256 _threshold,uint256 _thresholdRateDenominator) public onlyOwner {
    ...
    function pendingReward(uint256 _pid, address _user) public view returns (uint256 totalPending) {
        ...
        if (block.number > pool.lastUpdateBlock && lpSupply != 0) {
            ...
            uint256 tokenReward = blockInterval.mul(tokenPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function getMyPendingReward(uint256 _pid, address _user) internal returns (uint256 totalPending) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accTokenPerShare = pool.accTokenPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
    ...
    function claim(uint256 _pid) public {
        ...
        updatePool(_pid);
        uint256 deservedReward = getMyPendingReward(_pid,msg.sender);
    ...
    function getAllowedWithdrawAmount(uint256 _pid, address _user) public view returns (uint256 amount) {
        for (uint256 i = 0; i < userInfo[_pid][_user].depositInfo.length; i++) {
                if (block.timestamp > (userInfo[_pid][_user].depositInfo[i].lastDepositTime + userInfo[_pid][_user].depositInfo[i].depositDays)) {
    ...
    function deleteWithdrawTX(uint256 _pid) private {
        for (uint256 i = 0; i < userInfo[_pid][msg.sender].depositInfo.length; i++) {
            if (block.timestamp > (userInfo[_pid][msg.sender].depositInfo[i].lastDepositTime + userInfo[_pid][msg.sender].depositInfo[i].depositDays)) {
                delete userInfo[_pid][msg.sender].depositInfo[i];
            }
        }
    }
