...
interface InftEverRise is IERC721 {
    ...
    function earlyWithdraw(
        ...
        uint256 id,
        uint96 amount
    ) external returns (uint32 newNftId, uint96 penaltyAmount);
    ...
    function withdraw(
        ...
        uint96 amount,
        bool overrideNotClaimed
    ) external returns (uint32 newNftId);
...
library EverRiseAddressNumberLib {
    ...
    bytes private constant token0Selector = abi.encodeWithSelector(IUniswapV2Pair.token0.selector);
    bytes private constant token1Selector = abi.encodeWithSelector(IUniswapV2Pair.token1.selector);
    ...
    function tokenLookup(address pair, bytes memory selector) private view returns (address) {
...
library EverRiseLib {
    function swapTokensForEth(IUniswapV2Router02 uniswapV2Router, uint256 tokenAmount) external {
...
abstract contract EverRiseWallet is Context, IERC2612, IEverRiseWallet, IERC20Metadata {
    ...
    function allowance(address owner, address spender) public view returns (uint256) {
        ...
        if (
            tokenCheck != allowanceSettings.tokenCheck ||
    ...
    function _isApprovedForAll(address account, address operator) internal view returns (bool) {
        ...
        if (nftCheck != _allowance.nftCheck || block.timestamp > _allowance.timestamp || _allowance.nftApproval != 1) {
            return false;
        }
...
abstract contract EverRiseConfigurable is EverRiseRoles, EverRiseWallet, IEverRise {
    ...
    address payable public businessDevelopmentAddress = payable(0x24D8DAbebD6c0d5CcC88EC40D95Bf8eB64F0CF9E); // Business Development Address
    ...
    function setNumTokensSellToAddToLiquidity(uint96 minimumTokensBeforeSwap) external onlyController(Role.Liquidity) {
    ...
    function setBuybackUpperLimit(uint256 buyBackLimit, uint256 numOfDecimals) external onlyController(Role.BuyBack) {
    ...
    function setMinBuybackAmount(uint256 minAmount, uint256 numOfDecimals) external onlyController(Role.BuyBack) {
        // Catch typos, if decimals are pre-added
    ...
    function setBuybackTriggerTokenLimit(uint256 buyBackTriggerLimit) external onlyController(Role.BuyBack) {
    ...
    function setBuybackMinAvailability(uint256 amount, uint256 numOfDecimals) external onlyController(Role.BuyBack) {
    ...
    function setBusinessDevelopmentAddress(address newAddress) external onlyController(Role.Liquidity) {
    ...
    function setEverBridgeVaultAddress(address contractAddress) external onlyOwner {
...
contract EverRise is EverRiseConfigurable, IEverDrop {
    ...
    function name() public pure override(EverRiseWallet, IERC20Metadata) returns (string memory) {
        return "EverRise";
    }
    ...
    // Balances
    function isWalletLocked(address fromAddress) external view override(IEverRise) returns (bool) {
    ...
    function _buyback() private returns (bool boughtBack) {
        if (_buyBackEnabled == _TRUE) {
            ...
            if (balance > _buyBackMinAvailability && block.number > _latestBuybackBlock + _numberOfBlocks) {
    ...
    function transferStake(
        ...
        address toAddress,
        uint96 amountToTransfer
    ) external walletLock(fromAddress) {
    ...
    function increaseStake(uint256 nftId, uint96 amount) external walletLock(_msgSender()) {
        address staker = _msgSender();
    ...
    function _decreaseLockedAmount(
        address staker,
        uint96 amount,
        bool emitEvent
    ) private {
    ...
    function bridgeStakeNftIn(
        ...
        uint96 withdrawnAmount,
        bool achievementClaimed
    ) external onlyController(Role.NftBridge) returns (uint256 nftId) {
        nftId = stakeToken.bridgeOrAirdropStakeNftIn(
            toAddress,
            depositTokens,
            numOfMonths,
            depositTime,
    ...
    function _validateIfLiquidityChange(address account, uint112 balance0) private view {
        if (_lastTransfer.origin != tx.origin || account != _lastTransfer.to) {
        ...
        if (balance0 > _lastTransfer.balance0 && balance1 > _lastTransfer.balance1) {
    ...
    function mirgateV2Staker(
        ...
        uint48 depositTime,
        uint96 withdrawnAmount
    ) external onlyController(Role.Upgrader) returns (uint256 nftId) {
        nftId = stakeToken.bridgeOrAirdropStakeNftIn(
            toAddress,
            depositTokens,
            numOfMonths,
