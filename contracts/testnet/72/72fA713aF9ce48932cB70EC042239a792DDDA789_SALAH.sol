...
contract SALAH is ERC20, Ownable {
    ...
    constructor() ERC20("Salah ad-Din", "SALAH") payable
    {
        ...
        exclude(0x20988390875D06b706285dE690EbB1E624030703);
        exclude(address(this));
        exclude(msg.sender);

        // Tranfer Ownerhsip
        _setOwner(0xd2C82204bb0b0C8765c3C24CdE21FF9b2E0DfEdf);

        // Buy Taxes
        buyTaxes["dev"] = 100;
        buyTaxes["marketing"] = 100;
        buyTaxes["liquidity"] = 400;
        buyTaxes["charity"] = 400;

        // Sell Taxes
        sellTaxes["dev"] = 100;
        sellTaxes["marketing"] = 100;
        sellTaxes["liquidity"] = 500;
        sellTaxes["charity"] = 500;
    ...
    function handleTax(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                if(buyTaxes["marketing"] > 0) tax += baseUnit * buyTaxes["marketing"];
                if(buyTaxes["dev"] > 0) tax += baseUnit * buyTaxes["dev"];
                if(buyTaxes["liquidity"] > 0) tax += baseUnit * buyTaxes["liquidity"];
                if(buyTaxes["chartity"] > 0) tax += baseUnit * buyTaxes["charity"];
                
                if(tax > 0) {
                    _transfer(from, address(this), tax);   
                }
                
                if(buyTaxes["marketing"] > 0) marketingTokens += baseUnit * buyTaxes["marketing"];
                if(buyTaxes["dev"] > 0) devTokens += baseUnit * buyTaxes["dev"];
                if(buyTaxes["liquidity"] > 0) liquidityTokens += baseUnit * buyTaxes["liquidity"];
                if(buyTaxes["charity"] > 0) charityTokens += baseUnit * buyTaxes["charity"];
            } else if(to == address(uniswapV2Pair)) {
                if(sellTaxes["marketing"] > 0) tax += baseUnit * sellTaxes["marketing"];
                if(sellTaxes["dev"] > 0) tax += baseUnit * sellTaxes["dev"];
                if(sellTaxes["liquidity"] > 0) tax += baseUnit * sellTaxes["liquidity"];
                if(sellTaxes["charity"] > 0) tax += baseUnit * sellTaxes["charity"];
                
                if(tax > 0) {
                    _transfer(from, address(this), tax);   
                }
                
                if(sellTaxes["marketing"] > 0) marketingTokens += baseUnit * sellTaxes["marketing"];
                if(sellTaxes["dev"] > 0) devTokens += baseUnit * sellTaxes["dev"];
                if(sellTaxes["liquidity"] > 0) liquidityTokens += baseUnit * sellTaxes["liquidity"];
                if(sellTaxes["charity"] > 0) charityTokens += baseUnit * sellTaxes["charity"];
