 ...
 *  - postRelayedCall
 */
contract GasRelayPaymaster is IPaymaster, OwnableUpgradeable {
    ...
    uint256 private constant DEPOSIT = 200000000000000000;
    ...
    function withdrawToPool() external {
        ...
        (bool sent, ) = payable(proxyPoolLogic).call{value: amount}("");
    ...
    /// @dev Helper to pull WBNB from the associated vault to top up to the max deposit in the relay hub
    function __depositMax() private {
        ...
        if (prevDeposit < DEPOSIT) {
            ...
            // pull wbnb from proxyPoolLogic to paymaster.
            // IGasRelayPaymasterDepositor(proxyPoolLogic)
            //     .autoDepositForGasRelayer(amount);
            IERC20Upgradeable(WETH_TOKEN).safeTransferFrom(
                proxyPoolLogic,
                address(this),
                amount
            );

            // withdraw wbnb to bnb.
            // IERC20Token(WETH_TOKEN).withdraw(amount);

            // // deposit in relay hub
