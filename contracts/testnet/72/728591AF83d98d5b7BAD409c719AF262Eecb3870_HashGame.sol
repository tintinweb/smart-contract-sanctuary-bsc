...
interface I_Jackpot {
    function wonJackpot(uint256 _gid, address _userAddress) external view returns(bool);
    function wonGrandJackpot(uint256 _gid, address _userAddress) external view returns(bool);
    function isValidGrandJackpot(uint256 _gid, address _userAddress) external view returns(bool);
}
...
contract HashGame is ReentrancyGuard {
    ...
    function joinDefaultGame(uint256 _amount) external nonReentrant {
        ...
        require(!_gameInfo.paused, "Game paused, please try again later");
    ...
    function joinMvccGame(uint256 _gid) external nonReentrant {
        ...
        require(canJoinGame(_gid, msg.sender), "Please claim your jackpot, or upgrade to grand jackpot");
    ...
    function processMvccGame(uint256 _gid, address _userAddress, bool _result) external onlyOperator {
        ...
        if(_result) {    
            ...
            if(isValidPremium(_gid, _userAddress)) 
    ...
    function purchasePremium(uint256 _gid) external nonReentrant {
        ...
        require(!isValidPremium(_gid, msg.sender), "Premium already purchased before");
        ...
        bool _status = MVCC.transferFrom(msg.sender, address(this), _gameInfo.premiumFee);
    ...
    function resetUserConsecutiveBurstCount(uint256 _gid, address _userAddress) external onlyOperator {
        UserInfo storage _userInfo = userMvccInfo[_gid][_userAddress];
        _userInfo.consecutiveBurstCount = 0;

        emit ResetUserConsecutiveBurstCount(_gid, _userAddress);
    }
    ...
    // GETTERS
    // ===================================================================
    function canJoinGame(uint256 _gid, address _userAddress) public view returns(bool) {
        bool _wonJackpot = IJackpot.wonJackpot(_gid, _userAddress);
        bool _wonGrandJackpot = IJackpot.wonGrandJackpot(_gid, _userAddress);
        bool _isValidGrandJackpot = IJackpot.isValidGrandJackpot(_gid, _userAddress);

        if(_wonGrandJackpot) {
            // Return false if user won grand jackpot
            return false;
        } else if(_wonJackpot && !_isValidGrandJackpot) {
            // Return false if user won jackpot and do not own a valid grand jackpot ticket
            return false;
        }
    ...
    function isValidPremium(uint256 _gid, address _userAddress) public view returns(bool) {
        UserInfo storage _userInfo = userMvccInfo[_gid][_userAddress];
    ...
    function getUserGameStatus(uint256 _gid, address _userAddress) external view returns(bool) {
        return userMvccInfo[_gid][_userAddress].gameActive;
    }

    function getUserCurrentGameRound(uint256 _gid, address _userAddress) external view returns(uint256) {
        return userMvccInfo[_gid][_userAddress].currentGameRound;
    }

    function getUserConsecutiveBurstCount(uint256 _gid, address _userAddress) external view returns(uint256) {
        return userMvccInfo[_gid][_userAddress].consecutiveBurstCount;
    }
    ...
    event ResetUserConsecutiveBurstCount(uint256 _gid, address _userAddress);
