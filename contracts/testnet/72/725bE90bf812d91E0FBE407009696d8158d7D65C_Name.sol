...
contract Name is Context, IERC20, Ownable {
    ...
    string private _name = "CroYachtClub";
    ...
    function checkBigWallet(address _wallet, uint256 _amount) public view returns(bool) {
        if(_wallet == charityWallet || _wallet == owner())
            return false;
        if(balanceOf(_wallet).add(_amount) >= totalSupply().mul(_maxAmount).div(100) )
            return true;
        else
            return false;
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(!checkBigWallet(recipient, amount), "bigwallet error");
