...
contract Presale is Ownable, Pausable {
    ...
    constructor( address _MBRK, address _treasuryAddress, uint256 _startingTime, uint256 _endingTime, uint256 _tokenPerBNB, address _BUSD, uint256 _tokenPerBUSD, address _signer ) {
        Mabrook = IBEP20(_MBRK); 
    ...
    function updateTreasuryAddress(address _newWallet) external onlyOwner {
        require(_newWallet != address(0x0),"Invalid address");
    ...
    function updateToken(address _newToken) external onlyOwner {
        require(_newToken != address(0x0),"Invalid address");
