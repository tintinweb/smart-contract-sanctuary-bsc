    ...
    function printUser(uint256 level, address id) public view returns(string memory) {
     ...
     function printUser2(uint256 level, address id) public view returns(string memory) {
         ...
         if (ProxyMappingInverse[id] != 0x0000000000000000000000000000000000000000) {
            ...
            } while(ProxyMappingInverse[id] != 0x0000000000000000000000000000000000000000);
        }
  ...
  emit Log("bonus slot found2");
    ...
    function join(uint256 level, address userId, address uplineId, bool needPay) private {
        ...
        if (ProxyMappingInverse[uplineId] != 0x0000000000000000000000000000000000000000) {
            ...
            } while(ProxyMappingInverse[uplineId] != 0x0000000000000000000000000000000000000000);
        }

        string memory logs1 = string(abi.encodePacked("yong slp with koko and join with upline: ", toAsciiString(uplineId)));
        emit Log(logs1);
...
if (ProxyMapping[originalUplineId]!= 0x0000000000000000000000000000000000000000) {
                 do {
            originalUplineId = ProxyMapping[originalUplineId];
            } while(ProxyMapping[originalUplineId] != 0x0000000000000000000000000000000000000000);
}
