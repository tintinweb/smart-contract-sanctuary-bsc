...
interface IProtectedMarketplace {
    ...
    function _proceedPayments(uint256 _orderId, uint256 _price, uint256 _protectionRate, address payable buyerAddress) external ;

    function _updateOrderDetails(uint256 _orderId, uint256 _downsideAmount, uint256 _protectionShares) external;
...
contract DVC is ERC20, Ownable{
    ...
    event BuyNFTOnCriteria( uint256 orderID, IProtectedMarketplace.OrderType orderType, uint256 buyValue, IProtectedMarketplace.BidStatus bidStatus, uint offerClosingTime );
    event SellNFTOnCriteria( uint256 orderID, bool isAuction, uint256 sellPrice, uint64 protectionRate, uint256 protectionTime );
    ...
    struct BuyNFTCriteria {
        ...
        bool isLootbox;         // Single = 0, Lootbox = 1
        ...
        uint64 protectionPeriod;// minimum required downside protection period (in seconds).
    }
    
    struct SellNFTCriteria {
        uint64 abovePriceRate;  // percentage that must added above the purchasing price to get sell price.
        bool isAuction;         // true if Auction else Fix sale
        bool isLootbox;         // Single = 0, Lootbox = 1
        uint256 amountNFT;      // pack this amount of NFT into lootbox (if isLootbox is true)
        uint64 protectionRate;  // minimum percentage of downside protection (with 2 decimals)
        uint256 offerTime; // Epoch time (in seconds) when auction will be closed (or fixed order expired)
    }

    BuyNFTCriteria buyNFT ;
    SellNFTCriteria sellNFT ;
    uint256 startNewSellOrderTime;
    uint256 endSellOrderTime;
    ...
    constructor( address _protectedMarketplace ) ERC20( "LPToken", "LPT" ){
     ...
     * @param _tokenId  NFT Token ID set buy user 
     * @param _nftAddress NFT address provided by user
     * @param _isAuction   Fixedpay = false or Auction = true
     * @param _isLootbox   Single = false or Lootbox = true
     ...
     * @param _protectionPeriod minimum required downside protection period (in seconds).
     */    
    ...
    function getBuyCriteria() external {
        emit GetBuyCriteria( buyNFT );
    }

    function getSellCriteria() external {
        emit GetSellCriteria( sellNFT );
    }
    ...
    function checkBuyCriteriaStatus(uint256 _orderID) internal returns(bool) {
        ...
        IProtectedMarketplace.Order memory ipo = marketplace.orders( _orderID );

        require( ipo.nftTokenId == buyNFT.tokenId || buyNFT.tokenId == 0, "Tokens Id are different" );
        require( ipo.tokenAddress == buyNFT.nftAddress || buyNFT.nftAddress == address(0),"NFT address are not matched" );
        require( ipo.tokenPrice.mul(buyNFT.auctionRate.add(10000)).div(10000) <= buyNFT.maxPrice,"Price is not matched" );
        require( ipo.protectionRate >= buyNFT.protectionRate,"Protection Rate is not valid" );
        require( ipo.protectionTime >= buyNFT.protectionPeriod,"Protection time is different" );
    ...
    function sellNftOnCriteria( uint256 _orderID ) public returns ( bool ) {
        ...
        require( startNewSellOrderTime == 0 || startNewSellOrderTime + sellNFT.offerTime < block.timestamp, "Sell order is still alive" );

        uint256 priceChange = ipo.tokenPrice.mul( sellNFT.abovePriceRate ).div( 100 );
        uint256 sellPrice = ipo.tokenPrice.add( priceChange );

        startNewSellOrderTime = block.timestamp;
        if ( endSellOrderTime == 0 ) {
            endSellOrderTime = startNewSellOrderTime + ipo.protectionTime - 86400;
        }

        require( startNewSellOrderTime + sellNFT.offerTime < endSellOrderTime, "Insufficient offer time" );
        uint256 newProtectionTime = endSellOrderTime - startNewSellOrderTime - sellNFT.offerTime;
        
        marketplace.createOrder( ipo.tokenAddress, ipo.nftTokenId, sellPrice, sellNFT.protectionRate, newProtectionTime, sellNFT.isAuction, sellNFT.offerTime );
        
        emit SellNFTOnCriteria( _orderID, sellNFT.isAuction, sellPrice, sellNFT.protectionRate, newProtectionTime );
        
        return true;
    
    }

    function getFundsFromDownsideProtection( uint256 _orderId ) public returns ( bool ) {

        IProtectedMarketplace.Order memory ipo = marketplace.orders( _orderId );

        require( ipo.buyerAddress == payable(address(this)), "This order should be done by buyer" );
        require( endSellOrderTime == 0 || block.timestamp < endSellOrderTime + 86400, "Protection time expired" );
