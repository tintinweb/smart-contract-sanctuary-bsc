...
contract Lotto is Ownable {
    ...
    event OnClaimReward (address _player, uint256 _amount);
    ...
    function claimReward () public {
        uint256 _number = getLatestLuckyNumber();
        uint256 _reward = checkLatestReward(_msgSender());
        require(_reward > 0, "Lotto: No reward to claim");
        IERC20(busd).transfer(_msgSender(), _reward);
        userNumberBet[previousRoundId][_number][_msgSender()] = 0;

        emit OnClaimReward(_msgSender(), _reward);
    }
    ...
    function getCurrentNetBetted (address _user) public view returns (uint256) {
        return netBetted[getCurrentRoundId()][_user];
    }

    function quoteWithdrawFee (uint256 _amount) public view returns (uint256) {
        return _amount * withdrawFeeBPS / 10000;
    }
    ...
    function withdraw () public {
        ...
        require(IERC20(busd).transfer(buyback, _withdrawFee),"Lotto: Transfer failed");
