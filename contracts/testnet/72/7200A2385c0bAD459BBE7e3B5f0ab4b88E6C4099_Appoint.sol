...
contract Appoint is Ownable {
    ...
    constructor(address _signAddress,address _basicToken,address _blackAddr,address _adminAddress) public {
    ...
    function permitDeposit(address msgSender,address contractAddr,string memory funcName,uint256 amount,uint256 deadline, uint8 v, bytes32 r, bytes32 s) private {
        require(block.timestamp <= deadline, "EXPIRED");
        uint256 tempNonce = nonces[msgSender]; 
        nonces[msgSender] = nonces[msgSender].add(1); 
    ...
    function deposit( string memory funcName,uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        require(amount > 0, ' Zero amount!');
        permitDeposit(msg.sender, address(this),funcName,amount,deadline, v, r, s);
    ...
    function burn( uint256 _burnId,address burnUser,uint256 amount) public onlyAdmin{
        ...
        burnId[_burnId] = 1;
        safeTransfer(basicToken, blackAddr, amount); 
