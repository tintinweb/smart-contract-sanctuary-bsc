...
error WrongBuyAmoutError();
error NotClaimableError();
error ZeroAmountError();
error ExeedsCapError();
error AddressZeroError();

contract CryptoStrikeICO is Ownable, PaymentSplitter {
    using SafeERC20 for IERC20;

    address public quoteToken;
    address public purchaseToken;
    uint256 public price = 3 * 1e16; // 0.03
    ...
    uint256 public cap = 100_000_000 * 1e18;
    ...
    constructor(
        ...
        address _quoteToken, address _purchaseToken
    ...
    function setState(bool _isOn, bool _canClaim) external onlyOwner {
        isOn = _isOn;
        canClaim = _canClaim;
    }
    ...
    function _buy(address recipient) private {
        ...
        if (amount < minBuyAmount) revert WrongBuyAmoutError();
        ...
        claimables[recipient] += quoteAmount;
    }
    ...
    function _buy(uint256 amount, address recipient) private {
        ...
        IERC20(purchaseToken).safeTransferFrom(_msgSender(), address(this), amount);
        splitPayment(purchaseToken);
    ...
    function claim(address recipient) public {
        if (!canClaim) revert NotClaimableError();
        if (recipient == address(0)) revert AddressZeroError();
        
        uint256 amount = claimables[msg.sender];
