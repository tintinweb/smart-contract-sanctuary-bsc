...
contract Subscribe is Base, ISubscribers {
    ...
    event subscriber(address indexed user, bool indexed isActive);
    ...
    function newSubscription(address _addr) external {
        require(users[_addr].untilTime == 0, "User already exists");
        users[_addr] = User(block.timestamp.add(trialDuration), false);
        emit subscriber(_addr, true);
    }
