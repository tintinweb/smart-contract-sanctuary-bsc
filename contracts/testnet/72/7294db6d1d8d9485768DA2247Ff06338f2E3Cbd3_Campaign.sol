...
contract Campaign is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    struct CampaignInfo {
        ...
        uint256 campaignType; // 0 - Default | 1 - Special Box
        uint256 start;
        uint256 end;
        bool active;
    ...
    uint256 public maxBuySpecialBox = 3;


    event OpenCampaign(address indexed creator, uint256 indexed campaignId, uint256[] mysteryBoxIds);
    event BuyMysteryBox(address indexed buyer, uint256 indexed mysteryBoxId, uint256 indexed quantity, uint256 campaignId, uint256 campaignType);
    ...
    event UpdateStartTime(address indexed sender, uint256 campaignId, uint256 start);
    event UpdateEndTime(address indexed sender, uint256 campaignId, uint256 end);
    ...
    function setAdmin(address _newAdmin) public onlyOwner {
        ...
        require(_newAdmin != admin, "No Need To Update");
        admin = _newAdmin;
    }
    ...
    function initialize(
        ...
        address _quoteErc20Address, // ATF token
        address _admin
    ) external initializer {
        require(_mysteryBoxNFT != address(0) && _mysteryBoxNFT != address(this), "Invalid Mystery Box Address!");
        require(address(_feeAddress) != address(0) && address(_feeAddress) != address(this), "Invalid Fee Address");
        require(_admin != address(0) && _admin != address(this), "Invalid Admin Address");
        if (_quoteErc20Key > 0) {
    ...
    function openCampaign(
        ...
        require(_campaignType == 0 || _campaignType == 1, 'Invalid Campaign Type');
        require(mysteryBoxNFT.isApprovedForAll(_msgSender(), address(this)) == true, 'Not Approve Box For Contract');
        ...
        info.campaignType = _campaignType;
        info.start = _start;
        info.end = _end;
        info.active = true;
    ...
    function buyMysteryBox(
        ...
        require(mysteryBoxNFT.balanceOf(address(this), _mysteryBoxId) >= _quantity, 'Out Of Stock - Can Not Buy Box Now!');

        if (campaign.campaignType == 1) {
            bytes32 message = prefixed(keccak256(abi.encodePacked(_msgSender(), address(this))));
            require(recoverSigner(message, sig) == admin, 'Wrong Signature');
            require(campaign.boughtBoxes[_msgSender()][_mysteryBoxId].add(_quantity) <= maxBuySpecialBox, "Max Buy Special Box");
        }
        ...
        if (campaign.quoteTokenKey == 0) {
            ...
            require(quoteErc20.balanceOf(_msgSender()) >= campaign.mysteryBoxInfo[_mysteryBoxId].price.mul(_quantity), 'Not Enough ATF To Buy');
...
//        require(campaign.campaignType == 0, 'invalid Campaign');
    ...
    function updateEndTime(uint256 campaignId, uint256 end) public onlyWhiteListUser {
        CampaignInfo storage campaign = campaigns[campaignId];
        require(campaign.end != end, "No Need To Update!");
        require(end > campaign.start, "Invalid End Time");
        campaign.end = end;
        emit UpdateEndTime(_msgSender(), campaignId, end);
    }

    function updateStartTime(uint256 campaignId, uint256 start) public onlyWhiteListUser {
        CampaignInfo storage campaign = campaigns[campaignId];
        require(campaign.start != start, "No Need To Update!");
        require(start < campaign.end, "Invalid Start Time");
        campaign.start = start;
        emit UpdateStartTime(_msgSender(), campaignId, start);
    }
