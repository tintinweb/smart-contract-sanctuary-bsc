...
contract FlameToken is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!swapping && routerExists) {
            if (pairs[_from]) {
                ...
                super._transfer(_from, address(this), liqAmount);
                super._burn(_from, burnAmount);
                swapAndLiquify(liqAmount);
            } else if (pairs[_to]) {
                // SELL
    ...
    function swapAndLiquify(uint256 amount) internal {
        ...
        super.approve(address(mainRouter), amount * 2);
        address[] memory path;
        path[0] = address(this);
