...
contract SendApi is Ownable {
    ...
    function zkSendWithPayload(bytes calldata payload)
        external
        payable
    ...
    function setZkHole(address zkHoleAddress) external onlyOwner {
        require(zkHoleAddress != address(zkHole), "Incorrect address");
        zkHole = IZKHole(zkHoleAddress);
    }
}
