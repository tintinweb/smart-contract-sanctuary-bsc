...
contract NewCryptoSabongI is Ownable, ReentrancyGuard {
   ...
   IERC20 token;
    ...
    function betWala(uint256 epoch, uint256 amount) public nonReentrant  {
        ...
        emit BetWala(msg.sender, epoch, amount);
    }
    ...
    function betMeron(uint256 epoch, uint256 amount) public nonReentrant {
        ...
        emit BetMeron(msg.sender, epoch, amount);
    }
    ...
    function executeRound(uint256 _gameResult) external onlyAgent {
    ...
    function genesisStartRound() external onlyAgent {
     ...
     function _calculateRewards(uint256 epoch, uint256 _gameResult) internal {
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) {
        BetInfo memory betInfo = ledger[epoch][user];
        Round memory round = rounds[epoch];
        if (round.gameResult == 0 ||round.gameResult == 1) {
            return false;
        }
        return
            !round.gameStatus &&
