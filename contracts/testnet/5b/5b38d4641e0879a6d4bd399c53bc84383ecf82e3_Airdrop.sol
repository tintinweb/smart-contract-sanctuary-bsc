...
contract Airdrop is Ownable{
    ...
    function setAirdropToken(address _airdropToken) public{
        airdropToken = IBEP20(_airdropToken);
    ...
    function airdrop(address[] calldata _recipients, uint256[] calldata _amount) public {
       sendDropFee();
        require(_recipients.length == _amount.length, "The number of recipients is not equal to the number of values");
        for (uint i = 0; i < _amount.length; i++) {
            airdropToken.transfer(_recipients[i], _amount[i]);
        }
