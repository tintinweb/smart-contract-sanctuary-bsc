...
contract CrosswiseVesting is ReentrancyGuard {
    ...
    function claimAllRewards(uint256 _pid) public nonReentrant {
        ...
        userWithdrawn[msg.sender][_pid].lpTokensWithdrawn += uint128(totalLp);
    ...
    function initiateFarmVestingInstance(
        ...
        uint256 rewardsMultiplier = rewardMultiplier[_vestingPeriod / oneMonth];
        ...
        if (_tokenAddress == crssAddress) {
            ...
            userVesting[msg.sender][_pid].crssVesting += uint128(_amount);
        } else {
            userVesting[msg.sender][_pid].lpTokensVesting += uint128(_amount);
        }
    ...
    function getUserVesting(address _user, uint256 _pid)
        public
        view
        returns (uint128 crssVesting, uint128 lpTokenVesting)
    {
        (crssVesting, lpTokenVesting) = (
            userVesting[_user][_pid].crssVesting,
            userVesting[_user][_pid].lpTokensVesting
        );
    }
    ...
    function getUserEarned(address _user, uint256 _pid)
        public
        view
        returns (uint128 crssVesting, uint128 lpTokensVesting)
    {
        (crssVesting, lpTokensVesting) = (
            userWithdrawn[_user][_pid].crssWithdrawn,
            userWithdrawn[_user][_pid].lpTokensWithdrawn
        );
    }
