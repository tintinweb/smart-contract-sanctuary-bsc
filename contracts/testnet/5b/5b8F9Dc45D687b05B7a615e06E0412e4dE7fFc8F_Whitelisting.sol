...
contract Whitelisting is WhitelistingDeveloper {
    ...
    event SuspendUser(address indexed wallet);

    event UpdateKYC(address indexed wallet, bool kycStatus);

    event UpdateFillerStatus(address indexed wallet, uint256 fillerExpiry);

    event RemoveUser(address indexed wallet);


    function addSymbol(string memory _symbol) public {
        require(!symbols[_symbol], "This symbol is already registered");
        symbols[_symbol] = true;
    }

    function checkSymbolExistance(string memory _symbol) public view returns(bool) {
    ...
    function addWhitelistedUser(
        ...
        newUser.fillerExpiry = _fillerExpiry;
        newUser.isActive = true;
        newUser.valid = true;
        userList.push(_wallet);
        emit AddWhitelistedUser(_wallet, _kycVerified, _fillerVerified, _userType);
    }

    function addUserInBulk(
        address[] memory _wallet,
        bool[] memory _kycVerified,
        bool[] memory _fillerVerified,
        string[] memory _userType,
        uint256[] memory _fillerExpiry)
        public onlyDeveloper {
        for(uint i = 0; i < _wallet.length; i++){
        UserWhitlisting storage newUser = whiteListedUsers[_wallet[i]];
        require(_wallet[i] != address(0), "Please provide valid address");
        require(newUser.wallet != _wallet[i], "Whitelisting: User is already added");
        newUser.wallet = _wallet[i];
        newUser.kycVerified = _kycVerified[i];
        newUser.fillerVerified = _fillerVerified[i];
        newUser.userType = _userType[i];
        newUser.fillerExpiry = _fillerExpiry[i];
        newUser.isActive = true;
        newUser.valid = true;
        userList.push(_wallet[i]);
    ...
    */
    ...
    function updateKYC(address _wallet, bool _kycStatus) public onlyDeveloper {
        ...
        emit UpdateKYC(_wallet , _kycStatus);
        ...
        onlyDeveloper
    {
        ...
        emit UpdateFillerStatus(_wallet, _fillerExpiry);
    }
...
contract WhitelistingDeveloper is Context {
    ...
    // developer events
    event AddDeveloper(address indexed wallet);

    event RemoveDeveloper(address indexed wallet);

    event AddDeveloperAdmin(
        address developer,
        address indexed developerAdmin,
        bytes32 role
    );

    event RemoveDeveloperAdmin(
        address developer,
        address indexed developerAdmin
    );
    event UpdateDeveloperAdminRole(
        address develper,
        address indexed develperAdmin,
        bytes32 role
    );
    ...
    event RemoveAgent(address indexed wallet);
    event SuspendAgent(address indexed wallet);
    event UpdateAgentCommission(address indexed wallet, uint256 commission);
    event UpdateLicenseExpiry(address indexed wallet , uint256 licenseExpiry);

    // contract events
    event AddWhitelistContract(address indexed contractAddress);
    event RemoveWhitelistContract(address indexed contractAddress);
    // admin events
    event AddAdmin(address indexed wallet);
    event RemoveAdmin(address indexed wallet);
    ...
    */
    function addDeveloper(address _wallet) public onlyAdmin {
        require(_wallet != address(0), "Please provide valid address");
        require(!developers[_wallet], "Developer already added");
        developers[_wallet] = true;
        emit AddDeveloper(_wallet);
    }
    ...
    */
        ...
        onlyDeveloper
    {
        ...
        emit AddDeveloperAdmin(_msgSender(), _developerAdmin, _role);
    }
    ...
    */
        ...
        onlyDeveloper
    {
        ...
        emit RemoveDeveloperAdmin(_msgSender(), _developerAdmin);
    }
    ...
    */
    ...
    function addAgent(
        ...
        emit AddAgent(_wallet, _licenseExpiry, _commission);
    }
        ...
        onlyDeveloper
    {
        ...
        emit UpdateLicenseExpiry(_wallet, _licenseExpiry);
    }
        ...
        onlyDeveloper
    {
        ...
        emit UpdateAgentCommission(_wallet, _commission);
    }
        ...
        onlyDeveloper
    {
        ...
        emit AddWhitelistContract(_contractAddress);
    }
        ...
        onlyDeveloper
    {
        ...
        for (uint256 i; i < contractList.length; i++) {
            if (contractList[i] == _contractAddress) {
                ...
                emit RemoveWhitelistContract(_contractAddress);
            }
        }
    }
