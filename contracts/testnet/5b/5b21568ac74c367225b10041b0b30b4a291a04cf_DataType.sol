...
contract LaunchpadProxyV9 is ILaunchpadProxy, Ownable, ReentrancyGuard {
    ...
    bytes4 internal constant PROXY_ID = 0x8733c8f2;

    // element 0x erc20 asset proxy
    address zeroExERC20AssetProxy = 0x30B9c33e70d84cD148F6ecf350d785402B316322; // bsc default mainnet address
    ...
    event ChangeSlotStartIdAndSaleQty(
        bytes4 proxyId,
        bytes4 indexed launchpadId,
        uint256 slotIdx,
        uint256 startId,
    ...
    modifier onlyCollaboratorOrOwner() {
        require(
            ...
            Errors.LPAD_ONLY_COLLABORATOR_OWNER
        );
        _;
    }
    ...
    function onlyLPADController(
        address msgSender,
        address controllerAdmin,
        DataType.CtlPermission ctlPermission,
        DataType.CtlPermission permission
    ) internal {
        if (msgSender == controllerAdmin) {
            require(
                uint8(ctlPermission) >= uint8(permission),
                Errors.LPAD_CONTROLLER_NO_PERMISSION
            );
        } else {
            require(
                owner() == msgSender || collaborator == msgSender,
                Errors.LPAD_ONLY_CONTROLLER_COLLABORATOR_OWNER
            );
        }
    }
    ...
    function setCheckAuthority(bool checkAuth)
        external
        onlyCollaboratorOrOwner
    {
        checkAuthority = checkAuth;
    ...
    // get launchpad info
    function getLaunchpadInfo(bytes4 launchpadId, uint256[] memory params)
        external
        view
        override
        returns (
    ...
    // set controller
    function setLaunchpadController(
        bytes4 launchpadId,
        address controller,
        DataType.CtlPermission perm
    ...
    function setLaunchpadAssetProxy(bytes4 launchpadId, address zxERC20Proxy)
        external
        onlyCollaboratorOrOwner
    {
    ...
    function setLaunchpadFeeParam(
        ...
        onlyLPADController(
            ...
            launchpads[launchpadId].ctlPermission,
            DataType.CtlPermission.P7
        ); // only controller
    ...
    function setLaunchpadEnableAndLocked(
        bytes4 launchpadId,
        bool enable,
        bool lock
    ) external {
    ...
    function launchpadBuy(
        ...
        if (checkAuthority) {
            require(
                ...
                Errors.LPAD_ONLY_AUTHORITIES_ADDRESS
            );
        } else {
        ...
        return
            Logic.processBuy(
                launchpads[launchpadId],
                launchpadVars[launchpadId].accountSlotStats[
                    DataType.genSlotAddressKey(sender, slotIdx)
                ],
                slotIdx,
                sender,
                quantity,
    ...
    function setSlotStartTimeAndFlags(
        ...
        DataType.setSlotStartTimeAndFlags(
            launchpads[launchpadId],
            slotIdx,
            saleStart,
            saleEnd,
    ...
    function setSlotSupplyParam(
        ...
        onlyLPADController(
            ...
            launchpads[launchpadId].controllerAdmin,
            launchpads[launchpadId].ctlPermission,
            DataType.CtlPermission.P2
        ); // only controller
        DataType.setSlotSupplyParam(
            launchpads[launchpadId],
            slotIdx,
            maxSupply,
            maxBuyQtyPerAccount,
    ...
    function setStartTokenIdAndSaleQuantity(
        ...
        onlyLPADController(
            ...
            launchpads[launchpadId].controllerAdmin,
            launchpads[launchpadId].ctlPermission,
            DataType.CtlPermission.P3
        ); // only controller
        require(!launchpads[launchpadId].lockParam, Errors.LPAD_PARAM_LOCKED); // only when not locked
        launchpads[launchpadId].slots[slotIdx].startTokenId = uint128(
            startTokenId
        );
        launchpads[launchpadId].slots[slotIdx].saleQuantity = uint32(
            saleQuantity
        );
        emit ChangeSlotStartIdAndSaleQty(
            PROXY_ID,
            launchpadId,
            slotIdx,
            startTokenId,
    ...
    function setBuyTokenAndPrice(
        ...
        onlyLPADController(
            ...
            launchpads[launchpadId].controllerAdmin,
            launchpads[launchpadId].ctlPermission,
            DataType.CtlPermission.P5
        ); // only controller
        require(!launchpads[launchpadId].lockParam, Errors.LPAD_PARAM_LOCKED); // only when not locked
        DataType.setSlotBuyTokenAndPrice(
            launchpads[launchpadId],
            slotIdx,
            buyToken,
            buyPrice
    ...
    // set target abi
    function setTargetContractAndABI(
        bytes4 launchpadId,
        uint256 slotIdx,
        address target,
        ...
        onlyLPADController(
            ...
            launchpads[launchpadId].controllerAdmin,
            launchpads[launchpadId].ctlPermission,
            DataType.CtlPermission.P4
        ); // only controller
        DataType.setTargetContractAndABI(
            launchpads[launchpadId],
            slotIdx,
            target,
            abiSelector
    ...
    // set whitelist mode
    function setWhitelistModeAndSigner(
        bytes4 launchpadId,
        uint256 slotIdx,
    ...
    function addOrRemoveSlotWhiteLists(
        ...
        onlyLPADController(
            _msgSender(),
            launchpad.controllerAdmin,
            launchpad.ctlPermission,
            DataType.CtlPermission.P0
        ); // only controller
        require(launchpad.id > 0, Errors.LPAD_INVALID_ID);
        require(
            wls.length == wln.length,
            Errors.LPAD_INPUT_ARRAY_LEN_NOT_MATCH
        );
        require(
            launchpad.slots[slotIdx].whiteListModel ==
        ...
        for (uint256 i = 0; i < wls.length; i++) {
            ...
            launchpadVars[launchpadId]
                .accountSlotStats[DataType.genSlotAddressKey(wls[i], slotIdx)]
    ...
    function isInWhiteList(
        ...
        if (offChainSign.length > 0) {
            require(
                wls.length == offChainMaxBuy.length &&
                    wls.length == offChainSign.length,
                Errors.LPAD_INPUT_ARRAY_LEN_NOT_MATCH
            );
            for (uint256 i = 0; i < wls.length; i++) {
                wln[i] = uint8(
                    LogicUtil.offChainSignCheck(
                        lpad,
                        wls[i],
                        slotIdx,
                        offChainMaxBuy[i],
            ...
            for (uint256 i = 0; i < wls.length; i++) {
                ...
                wln[i] = launchpadVars[launchpadId]
                    .accountSlotStats[
                        DataType.genSlotAddressKey(wls[i], slotIdx)
                    ]
                    .whiteListBuyNum;
            }
        }
    ...
    {
        return
            Logic.getAccountInfoInLaunchpad(
                launchpads[launchpadId],
                launchpadVars[launchpadId].accountSlotStats[
                    DataType.genSlotAddressKey(sender, slotIdx)
                ],
...
library DataType {
    uint256 internal constant MAX_REFERRAL_FEE_PERCENT = 5000; // 5000 means 50%, 10000 max
    uint256 internal constant MAX_PERCENT = 10000; // 10000 means 100%

    uint256 internal constant ABI_IDX_BUY_SELECTOR = 0; // example: bytes4(keccak256("safeMint(address,uint256)"))
    // buy param example:
    ...
    uint256 internal constant ABI_IDX_OPENBOX_SELECTOR = 2; // example: bytes4(keccak256("open(address,uint256)"))
    // open param example:
    ...
    uint256 internal constant ABI_IDX_OPENBOX_PARAM_TABLE = 3;
    uint256 internal constant ABI_IDX_MAX = 4;

    // flag to control buy or open flag
    uint256 internal constant LAUNCHPAD_BUY_FLAG = 0; // can call buy
    uint256 internal constant LAUNCHPAD_OPENBOX_FLAG = 1; // can call openbox
    uint256 internal constant LAUNCHPAD_CALL_BUY_FROM_CONTRACT_FLAG = 2; // support call buy from contract
    uint256 internal constant LAUNCHPAD_CALL_OPENBOX_FROM_CONTRACT_FLAG = 3; // support call openbox from contract
    uint256 internal constant LAUNCHPAD_OPENBOX_WHEN_SOLD_OUT = 4; // only when sold out can open the box
    uint256 internal constant LAUNCHPAD_FLAG_MAX = 5;

    // process buy additional flag
    uint256 internal constant BUY_ADDITIONAL_IDX_WL_MAX_BUY_NUM = 0; // whitelist max buy number
    uint256 internal constant BUY_ADDITIONAL_IDX_SIMULATION = 1; // simulation buy
    uint256 internal constant BUY_ADDITIONAL_IDX_REFERRAL = 2; // referral

    // role
    uint256 internal constant ROLE_LAUNCHPAD_FEE_RECEIPTS = 1; // fee receipt
    uint256 internal constant ROLE_LAUNCHPAD_CONTROLLER = 2; // launchpad controller
    uint256 internal constant ROLE_PROXY_OWNER = 4; // proxy admin
    uint256 internal constant ROLE_LAUNCHPAD_SIGNER = 8; // launchpad signer

    // simulation flag
    uint256 internal constant SIMULATION_NONE = 0; // no simulation
    uint256 internal constant SIMULATION_CHECK = 1; // check param
    uint256 internal constant SIMULATION_CHECK_REVERT = 2; // check param, then revert
    uint256 internal constant SIMULATION_CHECK_PROCESS_REVERT = 3; // check param & process, then revert
    uint256 internal constant SIMULATION_CHECK_SKIP_START_PROCESS_REVERT = 4; // escape check start time param, process, then revert
    uint256 internal constant SIMULATION_CHECK_SKIP_WHITELIST_PROCESS_REVERT =
        5; // escape check skip whitelist param, process, then revert
    uint256 internal constant SIMULATION_CHECK_SKIP_BALANCE_PROCESS_REVERT = 6; // escape check skip whitelist param, process, then revert
    uint256 internal constant SIMULATION_NO_CHECK_PROCESS_REVERT = 7; // escape check param, process, then revert
    ...
    // event
    event FeeReceiptChange(
        bytes4 indexed launchpadId,
        address[] feeReceipts,
        uint256[] fees,
        address operator
    );
    event SlotBuyTokenPriceChange(
        bytes4 indexed launchpadId,
        uint256 slotIdx,
        address token,
        uint256 price
    ...
    function addLaunchpad(
        ...
        newLpad.zeroExERC20AssetProxy = (
            zxERC20Proxy == address(0) ? address(this) : zxERC20Proxy
        );
    ...
    function setTargetContractAndABI(
        ...
        require(
            abiSelector.length == ABI_IDX_MAX,
            Errors.LPAD_SLOT_ABI_ARRAY_LEN
        );
    ...
    function setSlotStartTimeAndFlags(
        ...
        require(
            flags.length == LAUNCHPAD_FLAG_MAX,
            Errors.LPAD_SLOT_FLAGS_ARRAY_LEN
        );
    ...
    function setSlotBuyTokenAndPrice(
        Launchpad storage self,
        uint256 slotIdx,
        address token,
        uint256 price
    ...
    // set referral fee
    function setReferralFee(Launchpad storage self, uint256 referralFee)
        public
    {
        require(self.id > 0, Errors.LPAD_INVALID_ID);
        require(
            referralFee < MAX_REFERRAL_FEE_PERCENT,
            Errors.LPAD_REFERRAL_FEE_PCT_LIMIT
        );
        self.referralFeePct = uint16(referralFee);
    }

    // set fee, receipt
    function setFeeAndReceipt(
        Launchpad storage self,
        address[] memory feeReceipts,
        uint256[] memory fees
    ) public {
        ...
        require(
            feeReceipts.length == fees.length && fees.length > 0,
            Errors.LPAD_INPUT_ARRAY_LEN_NOT_MATCH
        );
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            require(
                isValidAddress(feeReceipts[i]),
                Errors.LPAD_RECEIPT_ADDRESS_INVALID
            ); // must invalid address
            self.feeReceipts.push(feeReceipts[i]);
            self.fees.push(uint16(fees[i]));
            feeMaxPct += fees[i];
        }
        require(
            feeMaxPct == DataType.MAX_PERCENT,
            Errors.LPAD_FEES_PERCENT_INVALID
        ); // must total 100%
    ...
    function genSlotAddressKey(address account, uint256 slotIdx)
        public
        pure
        returns (uint256)
    {
...
library Logic {
    bytes4 public constant zeroExERC20ProxyId = 0xf47261b0; // element 0x erc20 assetProxyId, 0xf47261b0
    ...
    function processBuy(
        ...
        // Simulation Buy
        uint256 simulationBuy = (additional.length >
            DataType.BUY_ADDITIONAL_IDX_SIMULATION)
            ? additional[DataType.BUY_ADDITIONAL_IDX_SIMULATION]
        ...
        if (simulationBuy < DataType.SIMULATION_NO_CHECK_PROCESS_REVERT) {
            string memory ret = LogicUtil.checkLaunchpadBuy(
                ...
                additional.length > DataType.BUY_ADDITIONAL_IDX_WL_MAX_BUY_NUM
                    ? additional[DataType.BUY_ADDITIONAL_IDX_WL_MAX_BUY_NUM]
    ...
    function processOpenBox(
        ...
        // check
        if (simulationBuy < DataType.SIMULATION_NO_CHECK_PROCESS_REVERT) {
            string memory ret = LogicUtil.checkLaunchpadOpenBox(
                lpad,
                slotIdx,
                sender,
                tokenAddr,
        ...
        )
    {
        ...
        //boolData[1]
        //boolData[2]
        boolData[3] = isWhiteListModel(
            lpadSlot.whiteListModel,
            lpadSlot.whiteListSaleStart,
            lpadSlot.saleStart
        );
        ...
        intData[3] = (lpadSlot.whiteListModel ==
            DataType.WhiteListModel.OFF_CHAIN_SIGN)
            ? (quantity >> 128)
            : uint256(accountStats.whiteListBuyNum);
        quantity = uint256(quantity & 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF); // low 128bit is the quantity want to buy

        if (boolData[3]) {
            intData[1] = (intData[3] > accountStats.totalBuyQty)
                ? (intData[3] - accountStats.totalBuyQty)
                : 0;
        } else {
            intData[1] =
                lpadSlot.maxBuyQtyPerAccount -
                accountStats.totalBuyQty;
        }
        ...
        if (sender != address(0)) {
            if (quantity > 0) {
                ...
                byteData[0] = bytes(
                    LogicUtil.checkLaunchpadBuy(
                        ...
                        DataType.SIMULATION_CHECK
                    )
                ); // simulation check

                // simulation open box check param
                byteData[1] = bytes(
                    LogicUtil.checkLaunchpadOpenBox(
                        lpad,
                        slotIdx,
                        sender,
            ...
            ) = LogicUtil.checkTokenBalanceAndApprove(
                lpad,
                slotIdx,
                sender,
                quantity
    ...
    function callLaunchpadBuy(
        ...
        bytes4 selector = lpadSlot.abiSelectorAndParam[
            DataType.ABI_IDX_BUY_SELECTOR
        ]; // bytes4(keccak256("safeMint(address,uint256)")),
        bytes4 paramTable = lpadSlot.abiSelectorAndParam[
            DataType.ABI_IDX_BUY_PARAM_TABLE
        ];
        // IERC721(token).safeTransferFrom(msg.sender, address(this), amount);
        // encode abi data
        ...
        if (paramTable == bytes4(0x00000000)) {
            ...
            proxyCallData = abi.encodeWithSelector(
                ...
                quantity,
                address(
                    uint160(additional[DataType.BUY_ADDITIONAL_IDX_REFERRAL])
                )
            );
        ...
        (bool didSucceed, bytes memory returnData) = lpadSlot
            .targetContract
            .call(proxyCallData);
        if (!didSucceed) {
            ...
            revert(
                string(
                    abi.encodePacked(
                        Errors.LPAD_SLOT_CALL_BUY_CONTRACT_FAILED,
                        Errors.LPAD_SEPARATOR,
                        returnData
                    )
                )
            );
    ...
    function callLaunchpadOpenBox(
        ...
        bytes4 selector = lpad.slots[slotIdx].abiSelectorAndParam[
            DataType.ABI_IDX_OPENBOX_SELECTOR
        ]; // bytes4(keccak256("open(uint256)")),
        bytes4 paramTable = lpad.slots[slotIdx].abiSelectorAndParam[
            DataType.ABI_IDX_OPENBOX_PARAM_TABLE
        ];
        ...
        (bool didSucceed, bytes memory returnData) = lpad
            .slots[slotIdx]
            .targetContract
            .call(proxyCallData);
        if (!didSucceed) {
            ...
            revert(
                string(
                    abi.encodePacked(
                        Errors.LPAD_SLOT_CALL_OPEN_CONTRACT_FAILED,
                        Errors.LPAD_SEPARATOR,
                        returnData
                    )
                )
    ...
    function transferFees(
        ...
        if (buyToken != address(0)) {
            ...
            ) = transferERC20FromZeroExProxy(
                    lpad.zeroExERC20AssetProxy,
                    buyToken,
                    sender,
                    address(this),
                    shouldPay
            ...
            require(
                ...
                string(
                    abi.encodePacked(
                        Errors.LPAD_TRANSFER_TO_LPAD_PROXY_FAIL,
                        Errors.LPAD_SEPARATOR,
                        returnData
                    )
                )
        ...
        if (lpad.referralFeePct > 0) {
            ...
            if (referral != address(0)) {
                uint256 referralFee = (shouldPay * lpad.referralFeePct) /
                    DataType.MAX_PERCENT;
                if (buyToken == address(0)) {
                    //ETH
        ...
        for (uint256 i = 0; i < lpad.fees.length; i++) {
            uint256 incomeFeeToTransfer = (incomeToReceipt * lpad.fees[i]) /
                DataType.MAX_PERCENT;
            if (buyToken == address(0)) {
                //ETH
    ...
    function transferERC20FromZeroExProxy(
        ...
        if (zeroExERC20Proxy == address(this)) {
            return (
                ...
                bytes(Errors.LPAD_SLOT_ERC20_TRANSFER_FAILED)
            );
        } else {
            ...
            return
                zeroExERC20Proxy.call(
                    abi.encodeWithSelector(
                        IAssetProxy(address(0)).transferFrom.selector,
                        abi.encodePacked(
                            zeroExERC20ProxyId,
                            uint256(uint160(erc20Token))
                        ),
                        from,
                        to,
                        amount
    ...
    function isValidERC20AssetProxy(address erc20Proxy)
        public
        view
        returns (bool)
    {
        return
            IAssetProxy(address(erc20Proxy)).getProxyId() == zeroExERC20ProxyId;
    }

    function checkAddLaunchpadSlot(DataType.LaunchpadSlot memory slot)
        external
        pure
    {
...
library LogicUtil {
    ...
    function checkLaunchpadBuy(
        ...
        if (lpadSlot.targetContract == address(0))
            return Errors.LPAD_SLOT_TARGET_CONTRACT_INVALID;

        // launchpad active check
        if (!lpadSlot.flags[DataType.LAUNCHPAD_BUY_FLAG])
            return Errors.LPAD_SLOT_BUY_DISABLE;

        // check support call from contract
        if (
            isContract(sender) &&
            !lpadSlot.flags[DataType.LAUNCHPAD_CALL_BUY_FROM_CONTRACT_FLAG]
        ) return Errors.LPAD_SLOT_BUY_FROM_CONTRACT_NOT_ALLOWED;

        // left supply check
        if ((quantity + lpadSlot.saleQuantity) > lpadSlot.maxSupply)
            return Errors.LPAD_SLOT_QTY_NOT_ENOUGH_TO_BUY;

        uint256 paymentNeeded = quantity * lpadSlot.price;

        if (
            simulateBuy != DataType.SIMULATION_CHECK_SKIP_BALANCE_PROCESS_REVERT
        ) {
            // check buy token
            if (lpadSlot.buyToken != address(0)) {
                // ERC20
                // balance check
                if (paymentNeeded > IERC20(lpadSlot.buyToken).balanceOf(sender))
                    return Errors.LPAD_SLOT_ERC20_BLC_NOT_ENOUGH;
                ...
                if (simulateBuy == DataType.SIMULATION_NONE) {
                    ...
                    if (
                        paymentNeeded >
                        IERC20(lpadSlot.buyToken).allowance(
                            sender,
                            lpad.zeroExERC20AssetProxy
                        )
                    ...
                    // pay value not need
                    if (msg.value > 0)
                        return Errors.LPAD_SLOT_PAY_VALUE_NOT_NEED;
                }
            } else {
                // balance check
                if (paymentNeeded > (sender.balance + msg.value))
                    return Errors.LPAD_SLOT_PAYMENT_NOT_ENOUGH;

                // not simulate, really call buy ..
                if (simulateBuy == DataType.SIMULATION_NONE) {
                    // ETH msg.value send check
                    if (paymentNeeded > msg.value)
                        return Errors.LPAD_SLOT_PAY_VALUE_NOT_ENOUGH;
                    // ETH msg.value upper need
                    if (msg.value > paymentNeeded)
                        return Errors.LPAD_SLOT_PAY_VALUE_UPPER_NEED;
                }
            }
        }

        // max buy number in one transcation limit
        if (quantity > lpadSlot.maxBuyNumOnce)
            return Errors.LPAD_SLOT_MAX_BUY_QTY_PER_TX_LIMIT;

        // one account max buy num check
        if ((quantity + accStats.totalBuyQty) > lpadSlot.maxBuyQtyPerAccount)
            return Errors.LPAD_SLOT_ACCOUNT_MAX_BUY_LIMIT;

        // account buy time cool down check
        if (block.timestamp - accStats.lastBuyTime < lpadSlot.buyInterval)
            return Errors.LPAD_SLOT_ACCOUNT_BUY_INTERVAL_LIMIT;

        // endTime check
        if (lpadSlot.saleEnd > 0 && block.timestamp > lpadSlot.saleEnd)
        ...
        if (lpadSlot.whiteListModel != DataType.WhiteListModel.NONE) {
            ...
            if (
                simulateBuy !=
                DataType.SIMULATION_CHECK_SKIP_START_PROCESS_REVERT
            ) {
                // public sale time check
                if (block.timestamp < lpadSlot.saleStart)
    ...
    function checkWhitelistBuy(
        ...
        if (
            simulateBuy ==
            DataType.SIMULATION_CHECK_SKIP_WHITELIST_PROCESS_REVERT
        ) {
            return Errors.OK;
        }

        if (lpadSlot.whiteListSaleStart != 0) {
            // in whitelist model, and set whiteListSaleStart
            // whitelist buy time has passed
            ...
            if (
                ...
                if (block.timestamp < lpadSlot.whiteListSaleStart)
        ...
        if (
            simulateBuy == DataType.SIMULATION_NONE &&
            lpadSlot.whiteListModel == DataType.WhiteListModel.OFF_CHAIN_SIGN
        ) {
            require(data.length > 0, Errors.LPAD_SLOT_ACCOUNT_NOT_IN_WHITELIST);
            require(
                data.length == 65,
                Errors.LPAD_INVALID_WHITELIST_SIGNATURE_LEN
            );
            maxWhitelistBuy = offChainSignCheck(
                lpad,
                sender,
                slotIdx,
                maxWhitelistBuy,
        ...
        if (maxWhitelistBuy == 0)
            return Errors.LPAD_SLOT_ACCOUNT_NOT_IN_WHITELIST;
    ...
    function checkLaunchpadOpenBox(
        ...
        // not support openbox
        if (lpadSlot.boxOpenStart == type(uint256).max)
            return Errors.LPAD_SLOT_OPENBOX_NOT_SUPPORT;

        // launchpad flag check
        if (!lpadSlot.flags[DataType.LAUNCHPAD_OPENBOX_FLAG])
            return Errors.LPAD_SLOT_OPENBOX_DISABLE;

        // check support open before sold out check
        if (
            lpadSlot.flags[DataType.LAUNCHPAD_OPENBOX_WHEN_SOLD_OUT] &&
            lpadSlot.maxSupply > lpadSlot.saleQuantity
        ) return Errors.LPAD_SLOT_ONLY_OPENBOX_WHEN_SOLD_OUT;

        // openbox time check
        if (block.timestamp < lpadSlot.boxOpenStart)
            return Errors.LPAD_SLOT_OPENBOX_TIME_INVALID;

        // check support call from contract
        if (
            isContract(sender) &&
            !lpadSlot.flags[DataType.LAUNCHPAD_CALL_OPENBOX_FROM_CONTRACT_FLAG]
        ) return Errors.LPAD_SLOT_OPENBOX_FROM_CONTRACT_NOT_ALLOWED;
        ...
        pure
    {
        ...
        // must valid address
        require(
            isValidAddress(slot.targetContract),
            Errors.LPAD_SLOT_TARGET_CONTRACT_INVALID
        );

        // ABI length check
        require(
            slot.abiSelectorAndParam.length == DataType.ABI_IDX_MAX,
            Errors.LPAD_SLOT_ABI_ARRAY_LEN
        );
        // buy selector check, not all project has a open selector
        require(
            slot.abiSelectorAndParam[DataType.ABI_IDX_BUY_SELECTOR] !=
                bytes4(0),
            Errors.LPAD_SLOT_ABI_BUY_SELECTOR_INVALID
        );

        // max buy quantity check
        require(
            (slot.maxBuyQtyPerAccount > 0) &&
                (slot.maxBuyQtyPerAccount <= slot.maxSupply),
            Errors.LPAD_SLOT_MAX_BUY_QTY_INVALID
        );
        ...
        require(
            slot.saleEnd == 0 || slot.saleEnd > slot.saleStart,
            Errors.LPAD_SLOT_SALE_END_TIME_INVALID
        );
        ...
        require(
            slot.flags.length == DataType.LAUNCHPAD_FLAG_MAX,
            Errors.LPAD_SLOT_FLAGS_ARRAY_LEN
        );
        require(
            !slot.flags[DataType.LAUNCHPAD_CALL_BUY_FROM_CONTRACT_FLAG],
            Errors.LPAD_SLOT_BUY_FROM_CONTRACT_NOT_ALLOWED
        );
        require(
            !slot.flags[DataType.LAUNCHPAD_CALL_OPENBOX_FROM_CONTRACT_FLAG],
            Errors.LPAD_SLOT_OPENBOX_FROM_CONTRACT_NOT_ALLOWED
        );
    }
    ...
    function offChainSignCheck(
        ...
        bytes32 ecHash = keccak256(
            abi.encodePacked(
                ...
                hashForWhitelist(account, lpad.id, slotIdx, maxBuyNum)
            )
        );
        for (uint256 i = 0; i < lpad.signers.length; i++) {
