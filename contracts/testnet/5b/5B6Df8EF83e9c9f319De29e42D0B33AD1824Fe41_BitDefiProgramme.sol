...
contract BitDefiProgramme {
    ...
    function packageInfo(uint256 _pkg) pure private  returns(uint8 p) {
        if(_pkg == 100e18){
            ...
            p=4;
        }else if(_pkg == 1500e18){
            p=5;
        }else if(_pkg == 3000e18){
            p=6;
        }else{
            p=0;
        }
    ...
    function _setb5(uint256 poolNo,address _referral,address _addr,uint256 _amount) private{
        ...
        if(poolNo==0){
            //referel
            players[_referral].incomeArray[0]+=_amount.mul(50).div(100);
            //1st upline
            players[up].incomeArray[1]+=_amount.mul(50).div(100);
            players[up].matrixArray[poolNo]+=_amount.mul(50).div(100);
        }else{
            //referel
            _setReferral(_referral,_amount);
            //1st upline
            if(players[up].b5_level[poolNo][0]>2){
                players[up].incomeArray[3]+=_amount.mul(50).div(100);
                players[up].matrixArray[poolNo]+=_amount.mul(50).div(100);
            }
    ...
    function staking(uint256 _bdf) public {
        ...
        //Creater
        players[owner].incomeArray[4]+=_bdf.mul(1).div(100);
    }
    ...
    function userDetails(address _addr) view external returns(uint256 dep,uint256 pw,uint256 rw,uint256 sw,uint256 roidays,uint256 myentry, uint256 [7] memory p) {
         for(uint8 i=0;i<7;i++){
    ...
    function teamDetails(address user) public view returns(uint256 [5] memory levelteam, uint256 [10] memory levelinc, uint256 [5] memory staketeam, uint256 [5] memory stakeinc){
