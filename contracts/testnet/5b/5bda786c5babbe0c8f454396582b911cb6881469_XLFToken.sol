...
contract XLFToken is IERC20,Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(_isExcludedFromFees[sender] || _isExcludedFromFees[recipient]){
			...
			if(uniswapV2Pair == sender || uniswapV2Pair == recipient){//买卖 9% 3 5 1 
                //3% 回流lp钱包
                uint _lpAmount = amount.div(100).mul(3);
                _transferToken(sender,address(this),_lpAmount);                
                //uint usdtAmount = swapTokensForOtherUsdt(_lpAmount.div(2)); //1.5%出售                
                //swapAddLiquidity(_lpAmount.div(2), usdtAmount); //加池子 1.5%代币+U
                //5% 持有用户分红U
                uint dividendsAmount = amount.div(100).mul(5);                
                _transferToken(sender,address(this),dividendsAmount); 
                //swapTokensForOtherUsdt(dividendsAmount);
                //_splitOtherToken();
                //1% 黑洞
				_transferToken(sender,_destroyAddress,amount.div(100).mul(1));              			
	...
	function swapTokensForOtherUsdt(uint256 tokenAmount) external returns(uint256) {
	...
	function swapAddLiquidity(uint256 tokenAmount,uint256 usdtAmount) external {
