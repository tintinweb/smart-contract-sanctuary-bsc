...
contract LyLevel is Initializable, OwnableUpgradeable, IERC20 {
    ...
    mapping(uint256 => BatchInfoV1) public v1Batches;
    ...
    function claimable(uint256 _batchId, address _account) public view returns (uint256) {
        if (_batchId > currentBatchId) {
            ...
            uint256 totalReward = _balances[_batchId][_account] * batches[_batchId].rewardPerShare;
            uint256 vestingDuration = v1Batches[_batchId].vestingDuration;
            if (vestingDuration == 0) {
                return (totalReward / PRECISION) - _rewards[_batchId][_account];
            } else {
                ...
                return (totalReward * duration / vestingDuration / PRECISION) - _rewards[_batchId][_account];
            }
        }
    }
    ...
    function allocate() external {
        ...
        BatchInfoV1 memory newV1Batch = BatchInfoV1({vestingDuration: _vestingDuration});
        v1Batches[currentBatchId] = newV1Batch;
