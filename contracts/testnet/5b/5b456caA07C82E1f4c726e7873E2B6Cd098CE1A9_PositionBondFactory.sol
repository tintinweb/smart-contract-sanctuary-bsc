...
import "../strategy/extensions/BondFee.sol";
import "./PositionBondFactoryStorage.sol";
import "../interfaces/IPositionBondFunction.sol";
import "@openzeppelin/contracts/proxy/Clones.sol";
import "hardhat/console.sol";


contract PositionBondFactory is BondFee, ReentrancyGuard, Ownable {
    IPositionBondFunction public positionBondFunction;
    address public positionAdmin;
    address public chainLinkPriceFeed;
    IPositionBondFactoryStorage public positionBondFactoryStorage;

    address public mommyBond;

    address public bondRouter;

    event BondCreated(
        PositionBondLending.BondInformation bondInformation,
        PositionBondLending.AssetInformation assetInformation,
        address bondAddress
    );

    modifier onlyAdmin() {
        require(msg.sender == positionAdmin, "positionAdmin");
        _;
    }
    ...
    constructor(
        ...
        positionAdmin = msg.sender;
        chainLinkPriceFeed = chainLinkPriceFeed_;
    }

    function setBondFee(uint256 bondFee_) public override(BondFee) onlyOwner {
        super.setBondFee(bondFee_);
    }
    ...
    function issueBond(
        ...
        require(
            positionBondFactoryStorage.bondSymbolExist(
...
//        require(
//            positionBondFunction.verifyRequire(
//                bondInformation,
//                assetInformation
//            ),
//            "require failed"
//        );
        ...
        address positionBond;

        bytes32 salt = keccak256(
            abi.encodePacked(issuer, address(this), block.timestamp)
        );

        positionBond = Clones.cloneDeterministic(
            mommyBond,
            salt
        );


        IPositionBondLending.BondSetup  memory bondSetup = IPositionBondLending.BondSetup({
            positionBondFunction: positionBondFunction,
            chainLinkPriceFeed: chainLinkPriceFeed,
            positionAdmin: positionAdmin,
            fee: bondFee,
            collateralAmount: assetInformation.collateralAmount,
            bondRouter: bondRouter
        });



        if (assetInformation.underlyingAssetType == 0) {
            IERC20(assetInformation.underlyingAsset).transferFrom(
                issuer,
                positionBond,
                assetInformation.collateralAmount
            );
            assetInformation.collateralAmount = IERC20(
                assetInformation.underlyingAsset
            ).balanceOf(positionBond);
        }

        if (assetInformation.underlyingAssetType == 1) {
            for (uint256 i = 0; i < assetInformation.nftIds.length; i++) {
                IERC721(assetInformation.underlyingAsset).safeTransferFrom(
                    issuer,
                    positionBond,
                    assetInformation.nftIds[i]
                );
            }
            assetInformation.collateralAmount = positionBondFunction
                .getParValue(assetInformation.nftIds);
        }


        IPositionBondLending(positionBond).bondInitialize(
            bondInformation,
            assetInformation,
            bondSetup,
            issuer
        ...
        console.log("positionBond: ", positionBond);


//        require(IPositionBondLending(positionBond).isNotReachMaxLoanRatio(), "!LTV");

        positionBondFactoryStorage.pushNewBond(
            msg.sender,
            positionBond,
            bondInformation.bondSymbol
        );
    ...
    function updatePositionBondFunction(address positionBondFunction_)
        public
        onlyOwner
    {
        positionBondFunction = IPositionBondFunction(positionBondFunction_);
    }

    function updatePositionBondFactoryStorage(
        address positionBondFactoryStorage_
    ) public onlyOwner {
        positionBondFactoryStorage = IPositionBondFactoryStorage(
            positionBondFactoryStorage_
        );
    }

    function updateChainLinkPriceFeed(address chainLinkPriceFeed_)
        public
        onlyOwner
    {
        chainLinkPriceFeed = chainLinkPriceFeed_;
    ...
    function updatePositionBondAdmin(address positionAdmin_) public onlyOwner {
        positionAdmin = positionAdmin_;
    }


    function updateMommyBond(address mommyBond_) public onlyOwner {
        mommyBond = mommyBond_;
    }

    function updateBondRouter(address router) public onlyOwner{
        bondRouter = router;
    }


    function at(address _address) internal view returns (bytes memory o_code) {
        assembly {
...
abstract contract BondFee {
    uint256 public bondFee;

    function setBondFee(uint256 bondFee_) public virtual {
        bondFee = bondFee_;
    }
}
...
interface IPositionBondFactoryStorage {
    function bondSymbolExist(string memory) external view returns (bool);
...
contract PositionBondFactoryStorage is Ownable {
    mapping(address => address[]) public positionBonds;
    mapping(string => bool) public bondSymbolExist;

    address public bondFactory;

    constructor(address bondFactory_) {
        bondFactory = bondFactory_;
    }

    modifier onlyBondFactory() {
        require(msg.sender == bondFactory, "!factory");
        _;
    }
    ...
    function pushNewBond(
        ...
        address newBond_,
        string memory symbol_
    ) external onlyBondFactory {
        positionBonds[issuer_].push(newBond_);
        bondSymbolExist[symbol_] = true;
    }

    function updateBondFactory(address bondFactory_) external onlyOwner {
        bondFactory = bondFactory_;
    }
}
    ...
    ReentrancyGuard
{
    ...
    function bondInitialize(
        ...
        fee = bondSetup.fee;
        UnderlyingAssetLending.updateUnderlyingAmount(bondSetup.collateralAmount);
        ...
        console.log("bondInformation.bondSupply: ", bondInformation.bondSupply);
    ...
    function isNotReachMaxLoanRatio() public view override returns (bool) {
        ...
        Status a =  BondStatusLending.getStatus();
        console.log("BondStatusLending.getStatus(): ",uint256(a));
        ...
        console.log("bondAmountInPhase: ", bondAmountInPhase);
        return
        ...
        nonReentrant
    {
        ...
        totalSoldAmountClaimed += amount;
        require(
            totalSoldAmountClaimed <= totalSupply() * getIssuePrice(),
            "!claim"
        );
        uint256 fee = calculateFee(amount);
        FaceAssetLending._transferOut(amount - fee, msg.sender);
        if (fee > 0) {
...
abstract contract FaceAssetLending {
    ...
    function initFaceAssetLending(
        ...
        console.log("faceAssetType: ", faceAssetType);
    ...
    function _transferOut(uint256 amount, address to) internal {
        if (faceAssetType == 0) {
            ...
            BondTransfer._transferOutEther(to, amount);
        }
    }
...
abstract contract LoanRatio {
    uint256 public liquidateRate = 8000;
    uint256 public maxLoanRatio = 6500;
    uint256 public twapInterval = 3 * 24 * 3600;
...
interface IPositionBondLending {
    ...
    function purchaseBondLending(uint256 amount, address recipient) external payable returns (uint256);
...
abstract contract BondStatusLending {
    ...
    function setDuration(uint64 duration_) internal {
        duration = Timestamp.wrap(duration_ * 1 minutes);
    }
}
