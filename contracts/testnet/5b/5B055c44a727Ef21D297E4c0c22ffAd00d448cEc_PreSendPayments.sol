...
contract PreSendPayments is AutomationCompatibleInterface, Initializable, AccessControlUpgradeable  {
    ...
    // Number of seconds in a year - used to determine if an affiliate has been registered for a year or more
    // and needs to reregister before receiving more fees from this contract.
    ...
    PreSendAffiliate public preSendAffiliate;
    ...
    uint256 public affiliateRegistrationTime = YEAR_IN_SECONDS;

    // Why 10 ** 24? - 10 ** 18 to convert to wei, divide by 100 since it's 2 cents, and then multiple by 10**8 since the aggregator returns the coin price in USD * 10**8.
    uint256 private aggregatorCoinPriceMult = 2 * (10 ** 24);

    // Subtract 10**5 from the fee amount in case the price of the native coin updated since the msg.value was calculated in the frontend.
    uint256 private aggregatorCoinPriceSub = 10 ** 5;
    ...
    */
    function setPreSendAffiliate(address payable _preSendAffiliateAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        preSendAffiliate = PreSendAffiliate(_preSendAffiliateAddress);
    ...
    */
    function _payPreSendFee(address user, address currency, uint256 amount, uint256 payment, uint256 currencyPrice, address affiliate) private {
        uint256 feeAmount = currencyPrice * amount / feeDivisor;

        // aggregator.latestRoundData returns the coin price in USD * 10**8 hence the 2 * 10**24 instead of 2 * 10**16
        // Formula is 0.02 (for 2 cents) / Matic price
        (, int256 coinPrice, , ,) = aggregator.latestRoundData();

        require(uint256(coinPrice) != 0, "Unable to fetch price of the native coin.");

        // aggregatorCoinPriceMult - Why 10 ** 24? - 10 ** 18 to convert to wei, divide by 100 since it's 2 cents, and then multiple by 10**8 since the aggregator returns the coin price in USD * 10**8.
        ...
        feeAmount += (aggregatorCoinPriceMult / uint256(coinPrice)) - aggregatorCoinPriceSub;

        require(payment >= feeAmount, "Payment not enough or too much to cover the fee of the transfer!");
        ...
        if (affiliate != address(0) && preSendAffiliate.affiliateToRegisteredTimestamp(affiliate) > block.timestamp - affiliateRegistrationTime) {
            // Step 1 - get affiliate percentage (5% or 10% by default, but more affiliate tiers can be added after deployment)
            uint256 affiliatePercentage = preSendAffiliate.affiliatePercentage(affiliate);

            if (affiliatePercentage > 0) {
                // Step 2 - Update the affiliate balance in the affiliate smart contract to what is was before + the 5% or 10% amount just calculated
                ...
                // Step 3 - send the affiliate fee (based on the affiliatePercentage value computed above) to the affiliate in the affiliate contract if they are registered
    ...
    */
    function payPreSendFee(address currency, uint256 amount, uint256 currencyPrice) external payable {
        _payPreSendFee(msg.sender, currency, amount, msg.value, currencyPrice, address(0));
    }
    ...
    @param affiliate The address of the affiliate wallet that the paying user is tied to
    */
    function payPreSendFee(address currency, uint256 amount, uint256 currencyPrice, address affiliate) external payable {
        _payPreSendFee(msg.sender, currency, amount, msg.value, currencyPrice, affiliate);
    }
...
contract PreSendAffiliate is Initializable, AccessControlUpgradeable {
    ...
    // Mapping to determine the affiliate percentage for each tier.
    // mapping(uint256 (tier) => uint256 (tier percentage as whole number))
    mapping(uint256 => uint256) public affiliateTierToPercentage;

    // Mapping to determine the tier for each affiliate.
    // If an address has a tier of 0, that means they are not an affiliate.
    mapping (address => uint256) public affiliateToTier;
    ...
    // Number tiers added to the contract. If this count is 2, for example,
    // then this contract currently has tier 1 and tier 2.
    // The affiliateTierToPercentage mapping can then be used to get the percentage for both tiers.
    uint256 public affiliateTiersCount;
    ...
    // Event to emit whenever an affiliate tier is created.
    event affiliateTierAdded(uint256 indexed affiliateTier, uint256 newTierPercentage);

    // Event to emit whenever an affiliate tier is updated.
    event affiliateTierUpdated(uint256 indexed affiliateTier, uint256 newTierPercentage);

    // Event to emit whenever an affiliate is added or removed.
    event affiliateAddedUpdatedOrRemoved(address indexed affiliate, uint256 affiliateTier);
    ...
    */
    function initialize(address _paymentsAddress) initializer external {
        ...
        affiliateTierToPercentage[1] = 5;
        affiliateTierToPercentage[2] = 10;
        affiliateTiersCount = 2;
    ...
    */
    function affiliateClaim() external {
        ...
        require(affiliateToTier[affiliate] > 0, "You are not an affiliate!");
    ...
    @param affiliate the address of the affiliate the percentage is being queried for
    */
    function affiliatePercentage(address affiliate) external view returns (uint256) {
        return affiliateTierToPercentage[affiliateToTier[affiliate]];
    }

    /**
    @dev Affiliate admin only function to add an affiliate based on an affiliate tier, update an affiliate tier, or remove an affiliate.
    @param affiliate the address of the affiliate
    @param tier the tier of the affiliate that determines the percentage of transfer fees they get. 0 for affiliate removal.
    */
    function addUpdateOrRemoveAffiliate(address affiliate, uint256 tier) external onlyRole(AFFILIATE_ADMIN) {
        require(affiliate != address(0), "Affiliate address cannot be the zero address.");
        require(tier == 0 || affiliateTierToPercentage[tier] > 0, "This affiliate tier does not exist or has been removed. To remove an affiliate, set their tier to 0. Otherwise, choose a tier already created.");
        affiliateToTier[affiliate] = tier;

        if (tier > 0) {
            affiliateToRegisteredTimestamp[affiliate] = block.timestamp;
        }

        emit affiliateAddedUpdatedOrRemoved(affiliate, tier);
    }

    /**
    @dev Affiliate admin only function to add a tier.
    @param tierPercentage the percentage for the tier being added or updated
    */
    function addTier(uint256 tierPercentage) external onlyRole(AFFILIATE_ADMIN) {
        require(tierPercentage <= 100, "You can't create a tier where the percentage is greater than 100!");
        affiliateTiersCount += 1;
        affiliateTierToPercentage[affiliateTiersCount] = tierPercentage;
        emit affiliateTierAdded(affiliateTiersCount, tierPercentage);
    }

    /**
    @dev Affiliate admin only function to update a tier.
    @param tier the index for the tier mapping that is being updated
    @param tierPercentage the percentage for the tier being updated
    */
    function updateTier(uint256 tier, uint256 tierPercentage) external onlyRole(AFFILIATE_ADMIN) {
        require(tier > 0 && tier <= affiliateTiersCount, "Invalid tier supplied. Must be greater than zero and less than or equal to the current number of tiers.");
        require(tierPercentage <= 100, "You can't update a tier by making the percentage greater than 100!");
        affiliateTierToPercentage[tier] = tierPercentage;
        emit affiliateTierUpdated(tier, tierPercentage);
    }
    ...
    * this function will take away affiliate funds and should only be used for very specific cases
