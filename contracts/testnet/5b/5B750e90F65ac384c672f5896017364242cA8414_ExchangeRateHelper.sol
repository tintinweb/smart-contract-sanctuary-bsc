...
// File contracts/ExchangeRateHelper.sol
...
contract ExchangeRateHelper is IExchangeRateHelper {
  ...
  function getExchangeRate(address _tokenAddress)
    public
    view
    override
    returns (uint256)
  {
    ...
    if (tokenPrice == 0) {
      tokenPrice = getRateFromOracle(tokenAddress);
    }
    if (tokenPrice == 0) {
      tokenPrice = getRateFromDex(tokenAddress);
    }
    return tokenPrice;
  }
  ...
  function getRateFromDex(address _tokenAddress) public view returns (uint256) {
    ...
    if (priceRecord.active) {
      ...
      uint256 baseTokenPrice = getExchangeRate(priceRecord.baseToken);
  ...
  function getRateFromOracle(address _tokenAddress)
    public
    view
    returns (uint256)
  {
    uint256 chainLinkPrice = getRateFromChainlink(_tokenAddress);
    return chainLinkPrice;
  }

  function getRateFromChainlink(address _tokenAddress)
    public
    view
    returns (uint256)
  {
  ...
  function setDexPriceRecord(
    ...
    uint256 baseTokenPrice = getExchangeRate(_baseToken);
