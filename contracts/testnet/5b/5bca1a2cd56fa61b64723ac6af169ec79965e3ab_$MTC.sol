...
contract $MTCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("$MTC_Dividend_Tracker", "$MTC_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "$MTC_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "$MTC_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main $MTC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "$MTC_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "$MTC_Dividend_Tracker: Cannot update claimWait to same value");
...
contract $MTC is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    $MTCDividendTracker public dividendTracker;
    ...
    function setSellFee(uint256 _onSellbnbRewardFee, uint256 _onSellliuidityFee, uint256 _onSellMarketingFee) public onlyOwner {
    ...
    constructor() ERC20("Meta currency", "$MTC") {
        BNBRewardsFee = 0;
        liquidityFee = 0;
        marketingFee = 0;

        onSellBNBRewardsFee = 4;
        onSellliquidityFee = 4;
    	...
    	dividendTracker = new $MTCDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "$MTC: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "$MTC: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "$MTC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$MTC: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$MTC: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$MTC: Cannot update gasForProcessing to same value");
