// contracts/VowLpExchangeBridge.sol
...
contract VowLpExchangeBridge is AccessControl {
    bytes32 public constant GOVERNOR_ROLE = keccak256("GOVERNOR_ROLE");

    event BridgeToken(address indexed token, address indexed dest, uint256 tokenAmount, uint256 bnbAmount);
    ...
    function bridge(
        ...
        if (tokenAmount > 0) {
            require(IERC20(token).transfer(destination, tokenAmount), "!token");
        }
        if (bnbAmount > 0) {
            require(payable(destination).send(bnbAmount), "!bnb");
        }

        emit BridgeToken(token, destination, tokenAmount, bnbAmount);
    }
