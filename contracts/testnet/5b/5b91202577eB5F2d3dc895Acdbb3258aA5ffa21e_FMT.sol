...
contract FuMeta is Context, IERC20, Ownable,ReentrancyGuard {
    ...
    function setContract(address _stake,address _dapp,address _pp,address _U)public onlyOwner{
        stakeAddress=_stake;
        dappAddress=_dapp;
        ppAddress=_pp;
        USDT = IERC20(_U);
    ...
    function safeRegisterUser(address _userAddress,address _inviter) external returns(bool) {
        require(msg.sender==stakeAddress||msg.sender==dappAddress||msg.sender==ppAddress);
    ...
    function _safeRegisterUser(address _userAddress,address _inviter) private{
        ...
        for(uint i=1;i<=8;i++){
            if(getUserData[_inviter].inviterAddr[0]!=address(0)){
                _newUser.inviterAddr[i]=getUserData[_inviter].inviterAddr[0];
            }else{
                break;
            }
        }
