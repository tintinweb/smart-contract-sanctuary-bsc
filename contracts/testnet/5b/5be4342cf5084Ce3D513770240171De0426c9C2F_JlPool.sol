...
contract JlPool  is Ownable {
    ...
    mapping (address => bool) private IdoOrder;

    function buyIdo(uint256 id, address currency, uint256 amount)  external {
        require(!IdoOrder[msg.sender], "IDO: An address can only participate once");  
        IERC20(currency).safeTransferFrom(msg.sender,acceptAddress,amount);
        emit BuyIdo(msg.sender,currency,id,amount);
    }

    function getIdoOrder(address _address) external view returns (bool) {
        return IdoOrder[_address];
    } 
