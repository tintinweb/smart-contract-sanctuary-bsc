...
contract YHIDO is OwnableUpgradeable {
  struct Partner {
    bool  bought;
    uint  inviteCount;
    bool  lv2Opened;
  ...
  function init(address yhToken_, address paymentToken_) public onlyOwner {
    ...
    setTargetInviteNumber(2);
  }

  function initialize(address yht, address pt) public initializer {
    __Ownable_init();
    init(yht, pt);
  }
  ...
  function setIDOTokens(address yh, address pt) public onlyOwner {
    ...
    yhClaimAmount100 = getAmountYhFromDex(idoPrice);
    yhClaimAmount300 = getAmountYhFromDex(idoPriceLv2);
  }
  ...
  function participateIDO100(address recommender) public onlyInIdo {
    ...
    if (investors[recommender].level == 0)
    ...
    if (recommender != address(0) && investors[recommender].level != 0) {
      paymentToken.transfer(recommender, (idoPrice*5)/100); // 5%
      address recommenderLv2 = referrals[recommender];
      if (recommenderLv2 != address(0)) {
        paymentToken.transfer(recommenderLv2, (idoPrice*3)/100); // 3%
      }
      investors[recommender].inviteCount ++;
      if (investors[recommender].level == 2) {
        investors[recommender].invitedCountInPartner ++;
        if (investors[recommender].inviteCount >= targetInviteNumber) {
          // refund ticket and reward NFT
          investors[recommender].level = 3;
          investors[recommender].invitedCountInPartner = 0;
          paymentToken.transfer(recommender, ticketPrice);
        }
      }
    }
    totalInvestors ++;
  ...
  function participateIDO300() public onlyInIdo {
    ...
    paymentToken.transferFrom(msg.sender, address(this), idoPriceLv2);
  ...
  function endIDO() public onlyOwner {
    ...
    uint256 yhForLiquidity = getAmountYhFromDex(usdtForLiquidity);
    ...
    emit EndIDO(usdtForLiquidity, remainUsdt);
  }
  ...
  function getAmountYhFromDex(uint256 usdtAmount) public view returns(uint256) {
