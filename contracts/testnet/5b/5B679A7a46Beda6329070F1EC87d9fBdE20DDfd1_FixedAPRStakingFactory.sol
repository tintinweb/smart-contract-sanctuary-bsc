...
contract FixedAPRStakingFactory is SherpaEarnFactory{
    ...
    function create (CreateFixedAPRRequest memory request) public atLeastAdmin nonReentrant returns (address)
    {
        ...
        FixedAPRStaking newFixedAPRStaking = new FixedAPRStaking(Structs.StakingParams(_projectId, request.stakingToken, request.minStakeAmount, request.unboundPeriodBlock, request.instantUnboundFeePercentage, address(adminAccess), request.feeAddress, request.commissionPercentage,request.payoutIntervalBlock),request.fixedAPRPercentage);
...
contract FixedAPRStaking is Staking{
    ...
    /// @notice withdraw , users can withdraw their unbindingAmount after unbindPeriodBlock
    function withdraw() public whenNotPaused nonReentrant userStaked(msg.sender) returns(bool){
        address user=msg.sender;
        StakeUser storage stakeUser=stakeUsersInfo[user];
        ...
        require(_claimRewards(user),"CF");
        IERC20 token=IERC20(stakingToken);
        token.safeTransfer(user,stakeUser.unbindingAmount);
    ...
    function getRewardDebt() public view returns(uint256 totalRewardInPool,uint256 totalStakingAmount,uint256 totalUnpaidReward){
        totalRewardInPool=IERC20(stakingToken).balanceOf(address(rewardPool));
        totalUnpaidReward=getAllUnpaidReward();
        return (totalRewardInPool,totalStakingAmount,totalUnpaidReward);
    }
...
abstract contract Staking is AccessControlEnumerable,ReentrancyGuard{
    ...
    uint256 public eachBlockSecond = 3;
    /// @notice block number for whole year
    uint256 public yearBlock=10512000;
    ...
    function setInstantUnboundFee(uint256 feePer) public atLeastProject{
...
abstract contract SherpaEarnFactory is ReentrancyGuard {
    ...
    internal
    {
        ...
        currentProjectIdIndex=project.projectID;
