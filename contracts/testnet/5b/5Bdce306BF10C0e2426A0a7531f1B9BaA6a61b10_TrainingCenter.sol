...
contract TrainingCenter is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) public _numberOfTrainingSpots;

    constructor() ERC721("Training_Center_NFT", "TNFT") {
        for(uint256 i = 0; i < 19 ; i++){
            _mintTrainingCenter(msg.sender);
        }
    }

    modifier onlyAuth() {
        require(
            gameAddresses.getTrainingCenterAddress() == msg.sender ||
            gameAddresses.getOpenAndCloseAddress() == msg.sender,
             "!Auth");
        _;
    }
    ...
    function mintTrainingCenter(address _to)
        ...
        returns (uint256)
    {
        return _mintTrainingCenter(_to);
    }

    function _mintTrainingCenter(address _to)
        internal
        returns (uint256)
    { 
        _tokenIds.increment();
    ...
    function addTrainingSpots(uint256 _tokenId, uint256 _amount) external onlyAuth returns(bool){
        _numberOfTrainingSpots[_tokenId] += _amount;
        return true;
    }
