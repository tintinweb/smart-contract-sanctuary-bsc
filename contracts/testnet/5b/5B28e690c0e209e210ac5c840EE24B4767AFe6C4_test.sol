...
contract HubbleXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("HubbleX_Dividend_Tracker", "HubbleX_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**6); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "HubbleX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "HubbleX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HubbleX contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HubbleX_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "HubbleX_Dividend_Tracker: Cannot update claimWait to same value");
...
contract test is ERC20, Ownable, SafeToken, LockToken {
    ...
    HubbleXDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1000000000000  * (10**6);
    ...
    constructor() ERC20("test", "test") {
        ...
        marketingFee = 5;
        marketingWallet = payable(0x553f351228FDc76894DDdDA3C61a87C96b136Fa7);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new HubbleXDividendTracker();
