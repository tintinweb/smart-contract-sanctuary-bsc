...
contract TestToken is Context, IERC20, Ownable {
    ...
    bool public swapMarketingWallet = false;
    // Anti bot
    ...
    function _tokenTransfer(
        ...
        if (swapMarketingWallet){
            uint256 initialBalance = address(this).balance;

        // swap tokens for ETH
            swapTokensForEth(marketingAmt);
        // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
            uint256 newBalance = address(this).balance.sub(initialBalance);
            _transferStandard(sender, marketingWallet, newBalance);
        }
    ...
    function updateBuyFees(
        ...
        require(
            _taxFee
                ...
                .add(_sellBurnFee)
                .add(_sellDevFee) <= 30,
            "Total fees can not grater than 30%"
        );
    }
    ...
    function setswapMarketingWallet(bool _enabled) public onlyOwner {
        swapMarketingWallet = _enabled;
    }
