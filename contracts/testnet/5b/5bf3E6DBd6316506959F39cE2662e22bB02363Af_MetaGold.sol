...
contract MetaGold is ERC20, Ownable, ReentrancyGuard, MetaAdmin {
    ...
    uint256 nextLockpayRebase = 0;
    uint256 lockpayTwentyFourHours = 86400;
    bool doLockpayRebase = false;
    ...
    uint256 public bonusSwapTransaction = 10; //1%
    ...
    function buySwap() payable public {
        ...
        uint256 reward = minamount.mul(bonusSwapTransaction).div(1000);
        IERC20(meta_address).transferFrom(
                        tokenWalletForSendingMETAfrom,
                        msg.sender,
                        reward
    ...
    function calculateReward(uint256 amount) public view returns(uint256 reward){
        address[] memory path = new address[](2);
        path[0] = bnb_address;
        path[1] = meta_address;
        uint256 minamount = uniswapV2Router.getAmountsOut(amount, path)[1];
        return minamount.mul(bonusSwapTransaction).div(1000);
    }
    ...
    */
    ...
    function setBonusSwapTransaction(uint256 value) external onlyOwner {
        require(value >= 0, "Bonus direct transaction must be greater than zero");
        bonusSwapTransaction = value;
    }
