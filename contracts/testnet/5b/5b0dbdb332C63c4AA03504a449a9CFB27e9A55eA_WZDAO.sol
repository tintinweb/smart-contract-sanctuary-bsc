...
contract WZDAO is ERC20Detailed, Ownable {
    ...
    function _transferFrom(
        ...
        if (_shouldBindParent(sender, recipient)) {
            referral.bindReferral(sender, recipient);
        }
        ...
        if (_shouldTakeFee(sender, recipient)) {
            if (sender != pair) {
                uint256 minHolderAmount = _gonsBalanceOf(sender).mul(
                    999
                ).div(1000);
    ...
    function _shouldBindParent(address _parent, address _user)
        private
        view
        returns (bool)
    {
        return
            referral.isBindReferral(_parent) &&
    ...
    function _takeFee(
        ...
        result = gonAmount;
        //LP
        uint256 lpFee = gonAmount.mul(recipient==pair?lpFeeRate():_lpFeeRate).div(100);
        ...
        if (gonCirculation > gonMinCirculation) {
            uint256 burnFee = gonAmount.mul(burnFeeRate).div(100).min(
        ...
        for (uint256 i = 0; i < referrals.length; i++) {
            ...
            uint256 reward = gonAmount.mul(referralFeeRates[i]).div(100);
            reward > 0 &&
                _basicTransfer(
                    sender,
        ...
        //营销钱包1
        uint256 marketingFee1 = gonAmount.mul(marketingFeeRate1).div(100);
        marketingFee1 > 0 &&
            _basicTransfer(sender, marketingWallet1, marketingFee1);
        result = result.sub(marketingFee1);

        //营销钱包2
        uint256 marketingFee2 = gonAmount.mul(marketingFeeRate2).div(100);
        marketingFee2 > 0 &&
