...
contract HolderToken is ERC20, ERC20Burnable, Ownable {
   ...
   function mint(uint256 amount) public onlyOwner() returns (bool) {
       ...
       require(_msgSender() != address(0), "ERC20: mint to the zero address");
        ...
        uint256 _mTotal = amount * 10 ** _decimals;

        removeAllFee();

        _mintPrivate(_msgSender(), _mTotal);

        restoreAllFee();



        //_tTotal = _tTotal + _mTotal;

        //_rTotal = (MAX - (MAX % _tTotal)) - _rFeeTotal;

/*

        removeAllFee();

//        _tOwned[_msgSender()] += _mTotal;
    ...
    function _mintPrivate(address account, uint256 amount) private {

        (, uint256 rTransferAmount, uint256 tTransferAmount, uint256 currentRate) = _getTransferValues(amount);



        _rOwned[account] = _rOwned[account] + rTransferAmount;   

        

        _reflectFee(amount, currentRate);
...
/*
    ...
    function burn(uint256 amount) public override {
        ...
        uint256 _bTotal = amount * 10 ** _decimals;

        _tTotal = _tTotal - _bTotal;
