...
contract RebasePair is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
        ...
        )
    {
        ...
        emit RebaseData(fx0, fx1, fxNT, reserve0USD, reserve1USD, averageUSD, rebaseFeeUSD);
        // dif0, dif1 - difference
        (uint256 dif0, uint256 dif1) = (reserve0USD > averageUSD)
            ? (reserve0USD - averageUSD, uint256(0))
            : (uint256(0), reserve1USD - averageUSD);

        if ((dif0 * 997) / 1000 > rebaseFeeUSD) {
