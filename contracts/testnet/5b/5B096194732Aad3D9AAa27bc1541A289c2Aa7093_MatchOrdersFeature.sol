...
library LibNativeOrdersStorage {
    ...
    struct Storage {
        ...
        // mapping for 1 FNFT, Stable token
        mapping(address => mapping(address => uint256)) feeSystemForOnePair;
    }
...
abstract contract NativeOrdersInfo is FixinEIP712, FixinTokenSpender {
        ...
        returns (LibNativeOrder.OrderInfo memory orderInfo)
    {
        ...
        _populateCommonOrderInfoFields(orderInfo, order.makerAmount, order.expiry, order.salt, minValidSalt);
    }
        ...
        returns (LibNativeOrder.OrderInfo memory orderInfo)
    {
        ...
        _populateCommonOrderInfoFields(orderInfo, amount, order.expiry, order.salt, minValidSalt);
    }
    ...
    function _populateCommonOrderInfoFields(
        ...
        // Get the filled and direct cancel state.
        {
            ...
            uint256 rawMakerTokenFilledAmount = stor.orderHashToFilledAmount[orderInfo.orderHash];
        ...
        returns (uint128 actualFillableTakerTokenAmount)
    {
        ...
        uint256 fillableMakerTokenAmount = LibMathV06.getPartialAmountFloor(
            uint256(params.orderTakerAmount - params.orderInfo.makerTokenFilledAmount),
...
import "../features/ManageTokenFeature.sol";
...
contract FullMigration {
    ...
    struct Features {
        ...
        ManageTokenFeature tokenmanager;
    }
    ...
    function _addFeatures(
        ...
        // MatchOrdersFeature
        {
            ...
            ownable.migrate(
                ...
                abi.encodeWithSelector(MatchOrdersFeature.migrate.selector),
                address(this)
            );
        }
        ...
        // LimitOrdersFeature
        {
            ...
            ownable.migrate(
                ...
                abi.encodeWithSelector(LimitOrderFeature.migrate.selector),
                address(this)
            );
        }

        {
            ...
            ownable.migrate(
                ...
                abi.encodeWithSelector(PancakeSwapFeature.migrate.selector),
                address(this)
            );
        }

        {
            // Register the feature.
            ownable.migrate(
                address(features.tokenmanager),
                abi.encodeWithSelector(ManageTokenFeature.migrate.selector),
                address(this)
            );
        }
    }
...
contract MatchOrdersFeature is IFeature, IMatchOrdersFeature, FixinCommon, FixinTokenSpender, NativeOrdersInfo {
    using LibBytesV06 for bytes;
    using LibSafeMathV06 for uint256;
    using LibSafeMathV06 for uint128;
    ...
    constructor(address zeroExAddress) public FixinCommon() NativeOrdersInfo(zeroExAddress) {
        // solhint-disable-next-line no-empty-blocks
    }
    ...
    function matchOrders(
        ...
        uint256 sellFee,
        uint256 buyFee
    ) external override onlyAdmin returns (LibNativeOrder.MatchedFillResults memory matchedFillResults) {
        return
            _matchOrders(sellOrder, buyOrder, sellSignature, buySignature, price, sellType, buyType, sellFee, buyFee);
    }

    function batchMatchOrders(LibNativeOrder.BatchMatches calldata batch) public override onlyAdmin {
        require(
            batch.sellOrder.length == batch.buyOrder.length &&
                ...
                batch.sellOrder.length == batch.sellFee.length &&
                batch.sellOrder.length == batch.buyFee.length,
    ...
    function _assertValidMatch(
        ...
        if (sellOrder.makerAmount.safeMul(buyOrder.makerAmount) < sellOrder.takerAmount.safeMul(buyOrder.takerAmount)) {
            LibRichErrors.rrevert(LibExchangeRichErrors.NegativeSpreadError(sellOrderHash, buyOrderHash));
        }
    }

    LibNativeOrder.LimitOrder _sellOrder;
    LibNativeOrder.LimitOrder _buyOrder;
    LibSignature.Signature _sellSignature;
    LibSignature.Signature _buySignature;
    ...
    function _matchOrders(
        ...
        uint256 sellFee,
        uint256 buyFee
    ) private returns (LibNativeOrder.MatchedFillResults memory matchedFillResults) {
        ...
        _buyOrder.makerToken = _sellOrder.takerToken;
        _buyOrder.takerToken = _sellOrder.makerToken;

        // Get sell & buy order info
        uint128 amountSellByType = sellType == LibNativeOrder.MATCH_AMOUNT
            ? sellOrder.makerAmount
            : sellOrder.takerAmount;
        uint128 amountBuyByType = buyType == LibNativeOrder.MATCH_AMOUNT
            ? _buyOrder.takerAmount
            : _buyOrder.makerAmount;
        LibNativeOrder.OrderInfo memory sellOrderInfo = getOrderInfo(_sellOrder, amountSellByType);
        LibNativeOrder.OrderInfo memory buyOrderInfo = getOrderInfo(_buyOrder, amountBuyByType);

        // Either our context is valid or we revert
        _assertFillableOrder(_sellOrder, sellOrderInfo, msg.sender, _sellSignature);
        _assertFillableOrder(_buyOrder, buyOrderInfo, msg.sender, _buySignature);
        ...
        matchedFillResults = calculateMatchedFillResults(sellOrder, buyOrder, matchInfo);

        // matchedFillResults = calculateMatchedFillResultsRevert(matchedFillResults);

        {
            uint256 recentAmountSell = sellType == LibNativeOrder.MATCH_AMOUNT
                ? matchedFillResults.makerAmountFinal
                : matchedFillResults.takerAmountFinal;
            uint256 recentAmountBuy = buyType == LibNativeOrder.MATCH_AMOUNT
                ? matchedFillResults.makerAmountFinal
                : matchedFillResults.takerAmountFinal;
    ...
    function getMatchOrderResult(
        ...
        uint256 sellFee,
        uint256 buyFee
    ) public view override returns (LibNativeOrder.MatchedFillResults memory matchedFillResults) {
        // Get sell & buy order info
        uint128 amountSellByType = sellType == LibNativeOrder.MATCH_AMOUNT
            ? sellOrder.makerAmount
            : sellOrder.takerAmount;
    ...
    function getOrderHashInfo(
        bytes32 orderHash
    ) public view override returns (uint256 filled, uint256 feeLock, uint256 locked) {
        filled = LibNativeOrdersStorage.getStorage().orderHashToFilledAmount[orderHash];

        feeLock = LibNativeOrdersStorage.getStorage().orderHashToFeeAmountRemaining[orderHash];

        locked = LibNativeOrdersStorage.getStorage().orderLocked[orderHash];
    }

    function getOrderInfo(
        LibNativeOrder.LimitOrder memory order,
        uint128 amount
    ) public view override returns (LibNativeOrder.OrderInfo memory orderInfo) {
    ...
    function _assertFillableOrder(
        ...
        if (hasLocked == 0) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.OrderStatusError(
                    orderInfo.orderHash,
        ...
        //         LibRichErrors.rrevert(
        //             LibExchangeRichErrors.ExchangeInvalidContextError(
    ...
    function calculateMatchedFillResults(
        ...
        LibNativeOrder.LimitOrder memory buyOrder,
        LibNativeOrder.MatchOrderInfoPlus memory matchInfo
    ) internal view returns (LibNativeOrder.MatchedFillResults memory matchedFillResults) {
        ...
        uint256 sellAmountRemaining = matchInfo.sellType == LibNativeOrder.MATCH_AMOUNT
            ? sellRemaining
            : sellRemaining.safeMul(decimal).safeDiv(matchInfo.price);

        uint256 buyAmountRemaining = matchInfo.buyType == LibNativeOrder.MATCH_AMOUNT
            ? buyRemaining
        ...
        if (sellAmountRemaining == buyAmountRemaining) {
            ...
            //fee
            matchedFillResults.sellFeePaid = LibNativeOrdersStorage.getStorage().orderHashToFeeAmountRemaining[
                matchInfo.sellOrderHash
            ];
            matchedFillResults.buyFeePaid = LibNativeOrdersStorage.getStorage().orderHashToFeeAmountRemaining[
                matchInfo.buyOrderHash
            ];

            matchedFillResults.realSellFee = sellAmountRemaining.safeMul(sellOrder.takerTokenFeeAmount).safeDiv(
                sellOrder.makerAmount
            );
            ...
            matchedFillResults.realSellFee = buyAmountRemaining.safeMul(sellOrder.takerTokenFeeAmount).safeDiv(
                sellOrder.makerAmount
            );
        ...
        if (matchInfo.sellType == LibNativeOrder.MATCH_TOTAL) {
            ...
            matchedFillResults.returnSellAmount = sellRemaining.safeMul(sellOrder.makerAmount).safeDiv(
                sellOrder.takerAmount
            );
            matchedFillResults.returnSellAmount = matchedFillResults
                .returnSellAmount
                .safeSub(matchedFillResults.makerAmountFinal)
                .safeSub(makerAmountRemainingSell);
            matchedFillResults.returnSellAmount = matchedFillResults
                .returnSellAmount
                .safeAdd(matchedFillResults.sellFeePaid)
                .safeSub(matchedFillResults.realSellFee);
        }

        if (matchInfo.buyType == LibNativeOrder.MATCH_AMOUNT) {
            // calculate for price not equal
            uint256 takerAmountRemainingBuy = buyRemaining
                .safeSub(matchedFillResults.makerAmountFinal)
                .safeMul(buyOrder.makerAmount)
                .safeDiv(buyOrder.takerAmount);

            // calculate the amount of taker token that - amount now - amount filled
            matchedFillResults.returnBuyAmount = buyRemaining
                .safeMul(buyOrder.makerAmount)
                .safeDiv(buyOrder.takerAmount)
                .safeSub(matchedFillResults.takerAmountFinal)
                .safeSub(takerAmountRemainingBuy);

            matchedFillResults.returnBuyAmount = matchedFillResults
                .returnBuyAmount
                .safeAdd(matchedFillResults.buyFeePaid)
                .safeSub(matchedFillResults.realBuyFee);
        }
    ...
    function calculateMatchedFillResultsWithFeeForTaker(
        LibNativeOrder.MatchedFillResults memory matchedFillResults,
        uint256 sellFee,
        uint256 buyFee
    ) internal view returns (LibNativeOrder.MatchedFillResults memory) {
        // matchedFillResults;

        // LibNativeOrder.MatchedFillResults memory results = matchedFillResults;

        // buy fee is not FNFT
        if (matchedFillResults.realBuyFee != 0) {
            uint256 returnExtenBuy = matchedFillResults.realBuyFee - buyFee;
            matchedFillResults.realBuyFee = buyFee;
            matchedFillResults.returnBuyAmount += returnExtenBuy;
        } else {
            // buyfee is FNFT
            matchedFillResults.makerAmountFinal -= buyFee;
        }

        if (matchedFillResults.realSellFee != 0) {
            uint256 returnExtenSell = matchedFillResults.realSellFee - sellFee;
            matchedFillResults.realSellFee = sellFee;
            matchedFillResults.returnSellAmount += returnExtenSell;
        } else {
            matchedFillResults.takerAmountFinal -= sellFee;
        }

        return matchedFillResults;
    }

    // function calculateMatchedFillResultsRevert(
    //     LibNativeOrder.MatchedFillResults memory matchedFillResults,
    //     uint256 sellFee,
    //     uint256 buyFee
    // ) internal view returns (LibNativeOrder.MatchedFillResults memory) {
    //     // buy fee is not FNFT
    //     if (matchedFillResults.realBuyFee + buyFee != 0) {
    //         uint256 returnExtenBuy = matchedFillResults.realBuyFee - buyFee;
    //         matchedFillResults.realBuyFee = buyFee;
    //         matchedFillResults.returnBuyAmount += returnExtenBuy;
    //     } else {
    ...
    function _settleMatchedOrders(
        ...
        matchedFillResults = calculateMatchedFillResultsWithFeeForTaker(matchedFillResults, sellFee, buyFee);
        {
            sendBalanceTo(buyOrder.makerToken, sellOrder.maker, matchedFillResults.takerAmountFinal);
        }

        {
            sendBalanceTo(sellOrder.makerToken, buyOrder.maker, matchedFillResults.makerAmountFinal);
        }

        {
            //fee for each order // does not send to feeRecipients save that to withdraw later
            // sendBalanceTo(buyOrder.makerToken, buyOrder.feeRecipient, matchedFillResults.realBuyFee);
            LibNativeOrdersStorage.getStorage().feeSystemForOnePair[address(buyOrder.makerToken)][
                address(buyOrder.takerToken)
            ] += matchedFillResults.realBuyFee;
        }

        {
            // sendBalanceTo(sellOrder.makerToken, sellOrder.feeRecipient, matchedFillResults.realSellFee);
            LibNativeOrdersStorage.getStorage().feeSystemForOnePair[address(sellOrder.makerToken)][
                    address(sellOrder.takerToken)
                ] += matchedFillResults.realSellFee;
        }

        if (matchedFillResults.returnSellAmount > 0) {
            sendBalanceTo(sellOrder.makerToken, sellOrder.maker, matchedFillResults.returnSellAmount);
        }

        if (matchedFillResults.returnBuyAmount > 0) {
            sendBalanceTo(buyOrder.makerToken, buyOrder.maker, matchedFillResults.returnBuyAmount);
        }
    }
    ...
    function _updateFilledState(
        ...
        LibNativeOrdersStorage.getStorage().orderHashToFilledAmount[orderHash] = LibNativeOrdersStorage
            .getStorage()
            .orderHashToFilledAmount[orderHash]
            .safeAdd(recentAmount);

        LibNativeOrdersStorage.getStorage().orderHashToFeeAmountRemaining[orderHash] = LibNativeOrdersStorage
            .getStorage()
            .orderHashToFeeAmountRemaining[orderHash]
            .safeSub(feePaid);

        LibNativeOrdersStorage.getStorage().orderLocked[orderHash] = LibNativeOrdersStorage
            .getStorage()
        ...
        emit Fill(orderHash, makerAmountFinal, takerAmountFinal, realFee);
    }

    function setDecimalPrice(uint256 _decimal) public override onlyAdmin {
        LibNativeOrdersStorage.getStorage().decimalPrice = _decimal;
    }

    function getDecimalPrice() public view override returns (uint256) {
        uint256 decimal = LibNativeOrdersStorage.getStorage().decimalPrice;
        if (decimal == 0) {
            decimal = 10 ** 10;
        }
...
contract LimitOrderFeature is IFeature, ILimitOrderFeature, FixinCommon, FixinTokenSpender, NativeOrdersInfo {
    using LibBytesV06 for bytes;
    using LibSafeMathV06 for uint256;
    using LibSafeMathV06 for uint128;
    ...
    function createLimitOrder(LibNativeOrder.LimitOrder calldata order, LibSignature.Signature calldata signature)
        public
        override
        returns (LibNativeOrder.OrderInfo memory orderInfo)
    {
        ...
        // If maker token is an FNFT -> fee == 0, taker token can be FNFT, or STABLETOKEN
        if (
            LibManageTokenStorage.getStorage().tokenTypeStorage[address(order.makerToken)] ==
            LibManageTokenStorage.TokenType.FNFT
        ) {
            require(
                LibManageTokenStorage.getStorage().tokenTypeStorage[address(order.takerToken)] ==
                    LibManageTokenStorage.TokenType.FNFT ||
                    LibManageTokenStorage.getStorage().tokenTypeStorage[address(order.takerToken)] ==
                    LibManageTokenStorage.TokenType.STABLETOKEN,
                "Can not create limit order with token that not registry before"
            );
        }
        // if makerToken == stable token -> takerToken should be FNFT
        else if (
            LibManageTokenStorage.getStorage().tokenTypeStorage[address(order.makerToken)] ==
            LibManageTokenStorage.TokenType.STABLETOKEN
        ) {
            require(
                LibManageTokenStorage.getStorage().tokenTypeStorage[address(order.takerToken)] ==
                    LibManageTokenStorage.TokenType.FNFT,
                "Should have at least one token FNFT"
            );
        }
        // all the cases not in above should revert
        else {
            revert("check your token");
        }

        orderInfo = getOrderInfo(order);
        // Either our context is valid or we revert
        _assertFillableOrder(order, orderInfo, takerAddress, signature);
        ...
        LibNativeOrdersStorage.getStorage().orderLocked[orderInfo.orderHash] = order.makerAmount.safeAdd(
            order.takerTokenFeeAmount
        );

        LibNativeOrdersStorage.getStorage().orderHashToFeeAmountRemaining[orderInfo.orderHash] = order
            .takerTokenFeeAmount;
    ...
    function getOrderLocked(bytes32 orderHash) public view override returns (uint256) {
        return LibNativeOrdersStorage.getStorage().orderLocked[orderHash];
    }

    function getFilledOrder(bytes32 orderHash) public view override returns (uint256) {
        return LibNativeOrdersStorage.getStorage().orderHashToFilledAmount[orderHash];
    }

    function cancelLimitOrderWithHash(
        bytes32 orderHash,
        IERC20TokenV06 token,
        address maker
    ) public override onlyAdmin {
        LibNativeOrdersStorage.Storage storage stor = LibNativeOrdersStorage.getStorage();
        sendBalanceTo(token, maker, stor.orderLocked[orderHash]);
    ...
    {
        // Compute the order hash and fetch the amount of takerAsset that has already been filled
        orderInfo = getLimitOrderInfoV2(order, order.makerAmount);
...
contract ManageTokenFeature is IFeature, IManageTokenFeature, FixinCommon, FixinTokenSpender {
    ...
    function withdrawFeeSystem(
        ...
        uint256 _gasFee = _amount.safeMul(LibManageTokenStorage.getStorage().feePercentageForWithdrawl).safeDiv(
            LibManageTokenStorage.getStorage().DECIMALS_PERCENTAGE
        );
        sendBalanceTo(_token, _to, _amount - _gasFee);

        emit SentUserFeeSystem(_to, address(_token), _amount - _gasFee, typeWithdraw, fractorId);

        address uniswapRouterV2 = LibManageTokenStorage.getStorage().uniswap;
        address wbnb = LibManageTokenStorage.getStorage().wbnb;

        address factory = IUniswapV2Router02(uniswapRouterV2).factory();
        address isPair = IUniswapV2Factory(factory).getPair(address(_token), wbnb);

        address _admin = IOwnableFeature(address(this)).admin();

        if (isPair == address(0)) {
            // send token to
            sendBalanceTo(_token, _admin, _gasFee);
        } else {
            address[] memory paths = new address[](2);
            paths[0] = address(_token);
            paths[1] = wbnb;

            _checkAndAdjustTokenAllowanceIfRequired(_token, _gasFee, uniswapRouterV2);

            IUniswapV2Router02(uniswapRouterV2).swapExactTokensForETH(
                _gasFee,
                0,
                paths,
                _admin,
...
interface IMatchOrdersFeature {
    event Fill(bytes32 indexed orderHash, uint256 makerAmountFinal, uint256 takerAmountFinal, uint256 feePaid);

    event TransferMatchDone(uint256 stt, address from, address to, uint256 amount);
    ...
    function matchOrders(
        ...
        uint256 sellTier,
        uint256 buyTier
    ) external returns (LibNativeOrder.MatchedFillResults memory matchedFillResults);

    function batchMatchOrders(LibNativeOrder.BatchMatches calldata batch) external;

    function getOrderHashInfo(bytes32 orderHash)
        external
        view
        returns (
            uint256 filled,
    ...
    function getMatchOrderResult(
        ...
        uint256 sellTier,
        uint256 buyTier
    ) external view returns (LibNativeOrder.MatchedFillResults memory matchedFillResults);

    function setDecimalPrice(uint256 _decimal) external;

    function getDecimalPrice() external view returns (uint256);
...
interface ILimitOrderFeature {
    ...
    event TransferDone(address maker, address matchOrderAddress);

    event LimitOrderCancelled(bytes32 orderHash, uint256 orderLocked);
