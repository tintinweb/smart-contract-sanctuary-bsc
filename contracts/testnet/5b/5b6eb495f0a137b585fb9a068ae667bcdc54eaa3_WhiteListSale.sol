...
contract WhiteListSale is Ownable{
     ...
     uint256 public maxContribe; // 5bnb
     ...
     uint256 public releaseBlock;//開始釋放的區塊
	 
	 bool public openWhiteList;//是否开启白名单
     
     struct userContribute{
         uint256 bnbAmount;
         uint256 tokenAmount;
         uint256 preRawToken;//每個區塊領取多少
         uint256 tegToken;//首次释放多少TOKEN
         uint256 claimTotal; //已經領取了多少
         uint256 lastClaimBlock; //最後一次執行領取的區塊
         bool hasClaim;
     }
    ...
    mapping(address=>bool) public whiteList;//第一輪白名單
    ...
    function init(
        ...
        preToken = 70000 * 10 ** 18;
     
        minContribe = 2 * 10**17; //1bnb
        maxContribe = 2 * 10**18; // 1bnb
     
     
        softHardBnb = 1 * 10 ** 18; //軟頂1 bnb 
        totalBnb = 500 * 10 ** 18; //300 bnb
    ...
    function doContribute() public payable {
        ...
        require(_userContribute.bnbAmount.add(buyAmount) <= maxContribe,"accumulated maximum 2.0.");
        ...
        //_userContribute.tegToken = _userContribute.tokenAmount.mul(20).div(100);//首次釋放 20%
        //_userContribute.preRawToken = _userContribute.tokenAmount.sub(_userContribute.tegToken).div(28800).div(60);//剩下的80%釋放2個月
