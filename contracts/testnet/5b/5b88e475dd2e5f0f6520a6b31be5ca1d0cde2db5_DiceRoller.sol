...
contract DiceRoller is VRFConsumerBaseV2, ReentrancyGuard {
  ...
  constructor() VRFConsumerBaseV2(vrfCoordinator) {
    ...
    owner = msg.sender;
    //Testnet Addresses
    VRFv2SubscriptionManagerAddress = 0xf5b556502E8cd726f16cd6E5AD68253aBC02EF8a;
  ...
  function fundChainlinkSubscription() public payable {
  ...
  function betUp(uint256 _amount) isUnpaused public payable{
    rollDice(1,_amount);
  }

  function betDown(uint256 _amount) isUnpaused public payable{
    rollDice(0,_amount);
  }
  ...
  function rollDice(uint16 _bet, uint256 _amount) internal {
    ...
    if(getVRFsubscriptionBalance() <= 1 ether){
      fundChainlinkSubscription();
    }
    // Will revert if subscription is not set and funded.
    require(getVRFsubscriptionBalance() >= 2000000000000000,"Insufficient Chainlink VRF subscription balance");
  ...
  function claimWinnings() public nonReentrant isUnpaused{
