...
contract test is IERC20, Context, Ownable, ReentrancyGuard {
    ...
    function _buy(uint256 minTokenOut, uint256 deadline) public nonReentrant ensure(deadline) payable returns (bool) {
        ...
        //remove the buy tax
        uint256 bnbAmount = isFeeExempt[msg.sender] ? msg.value : msg.value.mul(buyFee).div(divisor);
        
        // how much they should purchase?
        uint256 nShouldPurchase = _balances[address(this)].sub(liqConst.div(bnbAmount.add(liquidity)));

        // apply our spread fee
        uint256 tokensToSend = isFeeExempt[msg.sender] ? nShouldPurchase: nShouldPurchase.mul(spreadFee).div(divisor);
    ...
    function _sell(uint256 tokenAmount, uint256 deadline, uint256 minBNBOut) public nonReentrant ensure(deadline) payable returns (bool) {
        ...
        uint256 amountTax = amountBNB.mul(divisor.sub(sellFee)).div(divisor);
    ...
    function changeTaxDistribution(uint newTeamTax, uint newTreasuryTax) external onlyOwner {
        require( newTeamTax.add(newTreasuryTax) <= 100);
    ...
    function getTokenAmountOut(uint256 amountBNBIn) external view returns (uint256) {
        ...
        return amountBNBIn.mul(_balances[address(this)]).mul(spreadFee).div(liquidity).div(divisor);
    }
