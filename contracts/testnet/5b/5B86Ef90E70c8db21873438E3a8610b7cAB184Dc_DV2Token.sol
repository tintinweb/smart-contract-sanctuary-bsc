...
contract DV2Token is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    function updateMultiplier() public returns (bool allow) {
      if(isMultiEnabled){
        ...
        if(sellPrice >= sellprice && buyprice >= basePrice && sellPrice >= basePrice){
    ...
    function checkUserCanSold(address _user,uint256 _soldAmount) public view returns (bool) {
      ...
      uint256 sellprice = sellPriceNow();
      if(isMultiEnabled){
        if(_soldAmount < initialBalance.mul(minPercent).div(1000) && sellPrice >= sellprice){
          return true;
        }else{
          return false;
        }
