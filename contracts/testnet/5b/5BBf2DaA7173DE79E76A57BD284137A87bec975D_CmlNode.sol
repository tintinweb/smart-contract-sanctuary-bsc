...
contract CmlNode is ERC721Enumerable, Ownable, ControlledAccess{
    using Strings for uint256;
    ...
    function updateCmlAddress( address _cmlAddress) public onlyOwner{
        cml = _cmlAddress;
    }
    ...
    function mint(uint _type) public payable {
        ...
        if (msg.sender != owner()) {
            require(IERC20(cml).transferFrom(msg.sender, address(this), price), "Not enough funds");
            IERC20(cml).approve(address(uniswapV2Router), _tax);
            if (autosell) {
                swapTokensForEth(_tax);
            }
            IERC20(cml).transfer(rewardPool, price-_tax);
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IERC20(cml).approve(address(uniswapV2Router), tokenAmount);
