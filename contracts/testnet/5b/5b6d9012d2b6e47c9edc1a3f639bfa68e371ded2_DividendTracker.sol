    ...
    DividendPayingTokenOptionalInterface
{
    ...
    address public immutable CRK = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            IERC20(CRK).transfer(user, _withdrawableDividend);
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        public
        DividendPayingToken("DividendTracker", "DividendTracker")
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2500000000 * (10**18);
    }
...
contract CroKingToken is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmountMax = 1000000000000 * (10**18);
    ...
    address public buybackWallet = 0x8ACD539223B2E4b2b9096109C0637c99B13BA463;
    address public nftRewardsWallet = 0x3E0a34E74cF223348226C1a9aE66b6D735cdC1Fa;
    address public teamWallet = 0x3bEF52196aBAF96E628D7526a66E792e9449edb4;
    ...
    function initialize(address dividendTrackerAddress, address _newOwner) external onlyOwner {
        ...
        _dividendTracker.excludeFromDividends(_newOwner);
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (_isExcludedFromFees[from] || _isExcludedFromFees[to]), "Trading is disabled");
        ...
        if(takeFee) {
        	...
        	uint256 fees = amount.mul(totalFees).div(feeDenominator);

            if (_reducedFeesFor[from] || 
                _reducedFeesFor[to]) {
                fees -= fees.div(100).mul(reducedFeePercentage);
            } 
    ...
    function swapBack(uint256 _contractTokenBalance) internal {

        uint256 amountToLiquify = _contractTokenBalance.div(totalFees).mul(liquidityFee).div(2);
        uint256 amountToReflection = _contractTokenBalance.div(totalFees).mul(CRKRewardsFee);
        uint256 amountToSwap = _contractTokenBalance.sub(amountToLiquify).sub(amountToReflection);

        if(CRKRewardsFee > 0){
            swapAndSendDividends(amountToReflection);
        }
        ...
        uint256 totalCROFee = totalFees.sub(CRKRewardsFee).sub(liquidityFee.div(2));
        ...
        uint256 amountCroTeam = amountCRO.sub(amountCroLiquidity).sub(amountCroBuyback).sub(amountCroNftHolders);

        if(liquidityFee > 0){
           addLiquidity(amountToLiquify, amountCroLiquidity);
        }
        if(buybackFee > 0){
