...
contract WLSQToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public liquidityReceiveAddress;
    address public weth;
    ...
    constructor(
        ...
        LPtokenAddress = payable(LptokenAddr_);
    ...
    function takeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 Lptfee = amount.mul(foundationFee).div(100);
        if(Lptfee > 0) super._transfer(from, LPtokenAddress, Lptfee);
        amountAfter = amountAfter.sub(Lptfee);

        uint256 LFee = amount.mul(liquidityFee).div(100);
        AmountLiquidityFee += LFee;
        amountAfter = amountAfter.sub(LFee);

        uint256 LPFee = amount.mul(lpRewardFee-2).div(100);
...
//        emit SwapAndLiquify(half, newBalance, otherHalf);
        ...
        uint256 tokensForLP = tokens.mul(liquidityFee).div(_totalswapfee).div(2);
        uint256 tokensForSwap = tokens.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance - initialBalance;

        uint256 totalFee = _totalswapfee.sub(liquidityFee.div(2));
        
        uint256 amountLiquidity = amountReceived.mul(liquidityFee).div(totalFee).div(2);
        uint256 amountFoundation = amountReceived.mul(foundationFee).div(totalFee);
        uint256 amountMarketing = amountReceived.sub(amountLiquidity).sub(amountFoundation);
