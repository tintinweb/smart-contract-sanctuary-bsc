    ...
    PausableUpgradeable
{
    ...
    function setRewardDistributor(uint256 _poolId, address _rewardDistributor)
        external
        linearValidatePoolById(_poolId)
        onlyOwner
    {
        ...
        linearPoolInfo[_poolId].linearRewardDistributor = _rewardDistributor;
    }

    /**
     * @notice Add a new pool with different APR and conditions. Can only be called by the owner.
     * @param _linearAcceptedToken accepted token address
     * @param _linearRewardToken reward token address
     * @param _linearRewardDistributor reward distributor address
     ...
     * @param _exchangeRate the exchange rate of the pool.
    ...
    function linearSetPool(
        ...
        linearPoolInfo[_poolId].lockDuration = _lockDuration;
        ...
        linearValidatePoolById(_poolId)
    {
        require(
            treasury != address(0),
            "LinearStakingPool: treasury is not set"
        );
        ...
        linearValidatePoolById(_poolId)
    {
        ...
        stakingData.balance = 0;
        IERC20(linearPoolInfo[_poolId].linearRewardToken).safeTransfer(
            account,
            withdrawBalance
        );
