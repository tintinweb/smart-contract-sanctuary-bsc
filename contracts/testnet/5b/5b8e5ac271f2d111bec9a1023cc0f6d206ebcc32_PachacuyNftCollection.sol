    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        _grantRole(MINTER_ROLE, 0x77A45071c532b71Cb4c99126D9A98c1D86276D3A);
    }
    ...
    ////                   HELPER FUNCTIONS                    ////
    ///////////////////////////////////////////////////////////////
    function retrievePriceAndRace(uint256 _id) public view {}
    ...
    // Testing
    function addRarityToMapFix(uint256 _id, uint256[] memory _list)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _addRarityToMap(_id, _list);
    }

    // Testing
    function clean(uint256 _ix) public onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_ix == 1) {
            for (uint256 x = 0; x < 1999; x++) {
                delete phenomenomMap[x];
            }
            return;
        }
        if (_ix == 2) {
            for (uint256 y = 0; y < 1999; y++) {
                delete mysticMap[y];
            }
            return;
        }
        if (_ix == 3) {
            for (uint256 z = 0; z < 1999; z++) {
                delete legendaryMap[z];
            }
            return;
        }
    }
