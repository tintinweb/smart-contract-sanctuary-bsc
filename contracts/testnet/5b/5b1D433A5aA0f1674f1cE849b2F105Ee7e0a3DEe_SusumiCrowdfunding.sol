...
contract SusumiCrowdfunding is Ownable, ReentrancyGuard {
    ...
    address public USDTAddress; //binance
    ...
    mapping(address => uint128) public userTotalFunds; // Total funds raised by the user in USDT
    mapping(address => uint128) public userTotalDonations; // Total donation by the user in USDT
    ...
    mapping(uint128 => Campaign) public campaignInfo; // Mapping to the Campaign struct using index
    ...
    mapping(address => mapping(uint128 => User)) public userInfo; // Mapping to the User struct using user's address
    ...
    mapping(uint128 => VestingInfo) public vestingMapping; // Vesting Mapping
    ...
    mapping(uint128 => SusuValueInfo) public susuValueMapping; // Mapping to store Susu Value Info
    ...
    function initialize(
        ...
        defaultMaturityPeriod = 180 minutes;
        receiverAddress = _receiverAddress;
        receiverFeeSUSU = uint64(100 * (10**Token(tokenAddress).decimals())); // Fee in SUSU
    ...
    function setSusuValueInfo(
        ...
        SusuValueInfo memory newSusuInfo = SusuValueInfo({
        ...
        susuValueMapping[_userLevel] = newSusuInfo;
    }
    ...
    function addUpdateVestingInfo(
        ...
        VestingInfo memory vInfo = VestingInfo({
            maxSusuValue: _maxSusuValue,
            vestingPeriod: _vestingPeriod
        });
        vestingMapping[vestingCounter] = vInfo;
    }

    // Returns Vesting period index for the Susu Value
    function setVestingValue(uint128 _susuValue) internal view returns (uint8) {
    ...
    function createCampaign(uint128 _susuValue, uint256 _ID)
        external
        nonReentrant
        checkUserCampaignStatus
    {
        setLevelAndCheckValue(_susuValue);

        campaignInternal(
            _susuValue,
            susuValueMapping[userLevel[msg.sender]].minKeyValue,
            msg.sender,
            _ID
        );
    }
    ...
    function newCampaignInternal(
        ...
        Campaign memory newCampaign = Campaign({
            ...
            vestingIndex: setVestingValue(_SusuValue),
            expirationTime: 0,
        ...
        campaignReceiverCommission[totalCampaigns] = susuValueMapping[
            userLevel[msg.sender]
        ].receiverFee;

        // User's status is turned true since he creates 1 campaign
    ...
    function requestCustomCampaign(
        ...
        uint32 _slot,
        uint256 _ID
    ) external nonReentrant checkUserCampaignStatus {
        setLevelAndCheckValue(_susuValue);

        // Only level 2 or above user can request
        require(
            userLevel[msg.sender] >= 2,
            "User not applicable for this type of campaign"
        );

        SusuValueInfo storage sInfo = susuValueMapping[userLevel[msg.sender]];
        require(
        ...
        campaignInternal(_susuValue, _slot, msg.sender, _ID);
    }
    ...
    function createCampaignUsingClaim(
        ...
        User storage uInfo = userInfo[msg.sender][_campaignIndex];
        require(!uInfo.userClaimed, "Already Claimed");
        require(uInfo.tokenAmount > 0, "You have not donated.");

        uint128 _susuActivation = calculateSUSUActivation(
            _slot,
            sInfo.minKeyValue
        );
        ...
        price =
            price -
            uint32(
                (uint256(price * _pricePercentage)) / (uint256(10**8 * 100))
            );
        ...
        return
            uint32(
                (uint256(_amount * 10**8 * 100)) /
                    (uint256(Token(tokenAddress).balanceOf(address(this))))
            );
    }

    // Sets level of the user, if new and checks susuValue according to level
    function setLevelAndCheckValue(uint128 _susuValue) internal {
        ...
        require(
            _susuValue >= susuValueMapping[userLevel[msg.sender]].minSusuValue,
            "Minimum Value according to user level not met"
        );

        // Susu value cannot be more than what level allows
        require(
            _susuValue <= susuValueMapping[userLevel[msg.sender]].maxSusuValue,
        ...
        validateCampaign(_campaignIndex)
    {
        ...
        Campaign storage cInfo = campaignInfo[_campaignIndex];

        checkUSDTAmount(cInfo.slotValue);
        ...
        // USDT donated by the doner sent to the contract
        TransferHelper.safeTransferFrom(
            USDTAddress,
            msg.sender,
            address(this),
        ...
        validateCampaign(_campaignIndex)
    {
        ...
        require(msg.sender == cInfo.creator, "Not the campaign creator");
        ...
        view
    {
        ...
        require(
            _susuActivation <=
                Token(tokenAddress).allowance(_userAddress, address(this)),
            "You don't have enough allowance"
        );
        // creator needs to have SUSU tokens to create campiag
        require(
            _susuActivation <= Token(tokenAddress).balanceOf(msg.sender),
            "You don't have enough Token balance"
        );
    }

    // Checks if receiver fee and donation is valid
    function checkUSDTAmount(uint128 _usdtValue) internal view {
        require(
        ...
        require(
            (Token(USDTAddress).balanceOf(msg.sender)) >= _usdtValue,
            "Check your balance."
        );
    }
