...
contract BSG is Ownable{
    ...
    constructor()
    {
        BUSD = IERC20(0x18aAb96a31245042dE37839835D1cc47cE0FA38D);
    ...
    function checkMax(address user_) public view returns(uint256,uint256,uint256) {
        (uint256 staticReward, uint256 staticSplit) = _calCurStaticRewards(user_);
        (uint256 dynamicReward, uint256 dynamicSplit) = _calCurDynamicRewards(user_);
        ...
        uint256 totalAmount = userT.sub(userRewards.totalWithdrawls);

        uint256 totalSplit = totalAmount.mul(freezeIncomePercents).div(baseDivider);
        uint256 totalBusd = totalAmount.sub(totalSplit);

        return (userT,totalSplit,totalBusd);
