...
contract BAZSmartContract is Context, IERC20, Ownable {
    ...
    address private routerAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; //Pcs TESTNET
   ...
   function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
     ...
     if (to != pair && !_isExcludedFromMaxWalletSize[to] && !_isExcludedFromFee[to]) {
                require(amount + balanceOf(to) <= maxWalletSize, "Recipient exceeds max wallet size.");
        }
              
        uint256 contractTokenBalance = balanceOf(address(this));
        if(!swapping && swapEnabled && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !_isExcludedFromMaxWalletSize[from] && !_isExcludedFromMaxWalletSize[to] ){
