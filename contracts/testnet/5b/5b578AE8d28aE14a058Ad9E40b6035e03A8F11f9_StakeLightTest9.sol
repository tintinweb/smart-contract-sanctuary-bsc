...
payable(msg.sender).transfer(totalwithdrawTokenBNb);
  ...
  function getcurrentPASSIVEincome(address userAddress) public view returns (uint256){
    ...
    function getcurrentDividendsincome(address userAddress) public view returns (uint256){
	    User storage user = users[userAddress];
	    return (getUserDividends(userAddress).sub(user.totalRoiIncome));	    
	}
