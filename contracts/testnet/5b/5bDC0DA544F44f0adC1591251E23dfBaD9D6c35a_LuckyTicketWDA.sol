...
contract LuckyTicketWDA is ReentrancyGuard, VRFConsumerBase {
    ...
    function payWinner(
        ...
        if (proposalPercentage != 0) {
            uint256 proposalToWDA = (rewardAmount * proposalPercentage) / 100;
            if (action == 0) {
                reward -= proposalToWDA;
            } else {
                reward += proposalToWDA;
            }
        }
        ...
        if (rewardAmount == 5000) {
            ownerWinningTicket5000WDA[winner].push(
                TicketReward(reward, purchaseDate + lockTime)
            );
        } else {
            ticketType = 1;
        ...
        emit TicketWin(winner, reward, ticketType);
    }
    ...
    function claimCoin(uint8 _ticketType) external {
        ...
        WDAToken.transfer(msg.sender, _earned * 10**18);
    }
