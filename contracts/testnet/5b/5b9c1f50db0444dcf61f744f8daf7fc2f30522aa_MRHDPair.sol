...
contract MRHDPairSetUp{
    ...
    /**
    * token配置：token地址 => 初始价格，计量单位，幅动价格，幅动增减值，触发倍数，token数量，USDT数量，最大买入token，最小买入token，最大卖出token，最小卖出token，最大买入USDT，最小买入USDT,最大卖出USDT，最小卖出USDT,当前价格，初始幅动价格
    **/
    ...
    mapping(string => uint256) public addressfluidity;//地址、token流动性
...
contract MRHDPair is MRHDPairSetUp{
    constructor() {
        owner = msg.sender;
        usdtaddress = 0xd0b2794514F791f248B34186D5474f7ac0d5aedE;
    }
    ...
    //添加流动性
    function addLiquidity(address _token, address _tradpairaddress, uint256 amount) public {
        ...
        if(_token == usdtaddress){ //判断token或者usdt
            tokenconfiguration[_token][6] = tokenconfiguration[_token][6] + amount;
        }else {
            tokenconfiguration[_token][5] = tokenconfiguration[_token][6] + amount;
        }
        string memory key_1 = addressToString(msg.sender);
        string memory key_2 = addressToString(_tradpairaddress);
        string memory key_3 = addressToString(_token);
        string memory key = strConcat(strConcat(key_1,key_2),key_3);
        addressfluidity[key] = addressfluidity[key] + amount;
    }

    //提取流动性
    function extractLiquidity(address _token, address _tradpairaddress, uint256 amount) public {
        require(amount > uint256(0),"Add quantity is greater than 0");
        string memory key_1 = addressToString(msg.sender);
        string memory key_2 = addressToString(_tradpairaddress);
        string memory key_3 = addressToString(_token);
    ...
    function algorithmBuyNumber(uint256[17] memory _tokenconfiguration, uint256 _amount, uint256 usdt_sum) public pure returns (uint256 usdt_sum_n,uint256[17] memory _tokenconfiguration_n) {
        Tokenconfiguration memory t = Tokenconfiguration(_tokenconfiguration[0],_tokenconfiguration[15],_tokenconfiguration[1],_tokenconfiguration[2],_tokenconfiguration[3],_tokenconfiguration[4],_tokenconfiguration[16]);
        ...
        while(flang){
            ...
            if(_amount >= a){
                usdt_sum = usdt_sum + (t.price + (t.price + t.swingprice * (a - 1))) * a / 2;
                _amount = _amount - a;
                ...
                if(_amount != 0){
                    usdt_sum = usdt_sum + (t.price + (t.price + t.swingprice * (_amount - 1))) * _amount / 2;
    ...
    function usdtBuyNumber(uint256[17] memory _tokenconfiguration, uint256 _amount, uint256 _token_sum) public pure returns (uint256 token_sum_n,uint256[17] memory _tokenconfiguration_n) {
        ...
        while(flang){
            ...
            uint256 np = (t.price + (t.price + t.swingprice * (a - 1))) * a / 2;//计算消耗U
            if(_amount >= np){
    ...
    function swap(address _token, bool status_b, bool status_t,uint256 _amount) public payable {
        ...
        if(status_b){
            ...
            if(status_t){
                ...
                IERC20(usdtaddress).transfer(address(this),m);
            }else{
                (m,n) = algorithmSellNumber(tokenconfiguration[_token],_amount,0);
                IERC20(_token).transfer(msg.sender,m);
            }
        }else{
            if(status_t){
