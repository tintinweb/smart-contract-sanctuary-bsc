...
contract SquidGrowStaking is Ownable, ReentrancyGuard {
    ...
    // Info of each user that stakes NFT token
    ...
    // pooiId is attached?
    mapping(uint256 => bool) public isAttached;
    // On/Off Nft staking
    bool public isAbletoStakeNft = true;
    ...
    function add(IERC20 _lpToken, uint256 _allocPoint, uint16 _depositFeeBP) external onlyOwner {
        require(_depositFeeBP <= MAXIMUM_FEE_RATE, "depositFee mustn't be greater than 10%");
    ...
    // Add nft pool
    function addNft(IERC721Enumerable _nftToken, uint256 _boostAPR, uint256 _poolId) public {
        require(!isAttached[_poolId], "That pool is already attached");
        nftPoolInfo.push(NftPoolInfo({
            nftToken: _nftToken,
            boostAPR: _boostAPR,
            poolId: _poolId
        }));
        isAttached[_poolId] = true;
    }

    // Update the given nft pool's info
    function setNft(uint256 _nftPid, uint256 _boostAPR, uint256 _poolId) public {
        require(!isAttached[_poolId], "That pool is already attached");
        nftPoolInfo[_nftPid].boostAPR = _boostAPR;
        isAttached[nftPoolInfo[_nftPid].poolId] = false;
        nftPoolInfo[_nftPid].poolId = _poolId;
        isAttached[_poolId] = true;
    }
    ...
    function pendingSquidgrow(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 boostAPRPerNft;
        uint256 nftAmount;
        for (uint256 i = 0; i < nftPoolInfo.length; i++) {
            if (_pid == nftPoolInfo[i].poolId) {
                NftUserInfo memory nftUser = nftUserInfo[i][_user];
                boostAPRPerNft = nftPoolInfo[i].boostAPR;
                nftAmount = nftUser.nftAmount;
            }
        }
        ...
        uint256 basePending = user.amount.mul(accSquidgrowPerShare).div(1e19).sub(user.rewardDebt).add(user.prevReward);
        return basePending.mul(nftAmount.mul(boostAPRPerNft).add(10000)).div(10000);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        for (uint256 i = 0; i < nftPoolInfo.length; i++) {
            if (_pid == nftPoolInfo[i].poolId) {
                NftUserInfo memory nftUser = nftUserInfo[i][msg.sender];
                boostAPRPerNft = nftPoolInfo[i].boostAPR;
                nftAmount = nftUser.nftAmount;
            }
        }
        uint256 basePending = user.amount.mul(pool.accSquidgrowPerShare).div(1e19).sub(user.rewardDebt);
        uint256 pending = basePending.mul(nftAmount.mul(boostAPRPerNft).add(10000)).div(10000);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        require(_amount >= user.amount.mul(25).div(100), "Minimum withdraw amount should be greater than 25% of amount.");
        updatePool(_pid);
        uint256 boostAPRPerNft;
        uint256 nftAmount;
        uint256 nftPid;
        ...
        if (!isWhiteListedForWithdraw[msg.sender]) {
            ...
            safeSquidgrowTransfer(feeAddress, withdrawFeeReward);
            user.prevReward = user.prevReward.sub(withdrawFeeReward);
            uint256 withdrawFeeStaking = _amount.mul(withdrawFeeRate).div(10000);
    ...
    function depositNft(uint256 _nftPid, uint256 _nftAmount) public {
        require(isAbletoStakeNft, "NFT staking is turned off.");
        require(_nftAmount > 0, "deposit amount should be greater than 0");
        NftPoolInfo storage nftPool = nftPoolInfo[_nftPid];
        NftUserInfo storage nftUser = nftUserInfo[_nftPid][msg.sender];
        UserInfo storage user = userInfo[nftPool.poolId][msg.sender];
        require(user.amount > 0, "You should stake token to deposit NFT.");
        uint256 nftBalance = nftPool.nftToken.balanceOf(msg.sender);

        require(nftBalance >= _nftAmount, "You don't have enough Nft");
        for (uint256 i = 0; i < _nftAmount; i++) {
            uint256 tokenId = nftPool.nftToken.tokenByIndex(i);
            nftPool.nftToken.transferFrom(msg.sender, address(this), tokenId);
            nftUser.nftTokenIds.push(tokenId);
        }
        nftUser.nftAmount = nftUser.nftAmount.add(_nftAmount);
    }

    // Withdraw Nft Token from staking
    function withdrawNft(uint256 _nftPid) public {
        NftPoolInfo storage nftPool = nftPoolInfo[_nftPid];
        NftUserInfo storage nftUser = nftUserInfo[_nftPid][msg.sender];
        for (uint256 i = 0; i < nftUser.nftAmount; i++) {
            uint256 tokenId = nftUser.nftTokenIds[nftUser.nftAmount - i - 1];
            nftPool.nftToken.transferFrom(address(this), msg.sender, tokenId);
    ...
    function recoverUnsupportedToken(address _addr, uint256 _amount) external onlyOwner {
        ...
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            if (poolInfo[pid].lpToken == IERC20(_addr)) {
                totalDeposit = poolInfo[pid].lpSupply;
            }
        }
        ...
        if (_amount > 0 && _amount <= amt) {
            IERC20(_addr).safeTransfer(msg.sender, amt);
        }
    }
    ...
    function turnOnNftStaking() external onlyOwner {
        require(!isAbletoStakeNft, "already enable");
        isAbletoStakeNft = true;
    }

    // Turn off Nft staking
    function turnOffNftStaking() external onlyOwner {
        require(isAbletoStakeNft, "already disable");
        isAbletoStakeNft = false;
    }
