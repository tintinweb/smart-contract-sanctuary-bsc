...
contract USDTMINERV2 is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;

    IERC20Upgradeable public Usdt; //USDT token
    ...
    uint256 public restrictedBuyAmountUsdt;
    ...
    function store() public initializer {
        ...
        SpellSyrup = 750000;
        restrictedBuyAmountUsdt = 1000_000000000000000000; // 1000 USDT
        ...
        Usdt = IERC20Upgradeable(0x50f0cDd58588979f8268E6a14a7dc57cC42c3ec7);
    ...
    function sellSeas() public {
        ...
        IERC20Upgradeable(Usdt).transfer(developerWallet,developerFee);
        IERC20Upgradeable(Usdt).transfer(address(msg.sender), deductedAmount);

        trident[msg.sender]._SurfTime = block.timestamp;
        checker();
    }

    function UsdtRewards(address adr) public view returns (uint256) {
    ...
    function buySeas(address ref, uint256 amount) public {
        ...
        require(amount <= restrictedBuyAmountUsdt,"Error: Cann't Purchase More than 10K!!");
        require(!blacklisted[msg.sender],"Opps: Your Blacklisted by Security!!");
        ...
        uint256 balance = IERC20Upgradeable(Usdt).balanceOf(address(this));
        ...
        require(trident[msg.sender]._invested.add(amount) <= restrictedBuyAmountUsdt,"Error: Cann't Purchase More than 10K!!");
    ...
    function magicSurf(address _target) internal {
        ...
        IERC20Upgradeable(Usdt).transfer(developerWallet,developerFee);
        IERC20Upgradeable(Usdt).transfer(address(_target), deductedAmount);
    ...
    function getEstimatedSeas(uint _amount) public view returns (uint256) {
        uint _balance = IERC20Upgradeable(Usdt).balanceOf(address(this));
    ...
    function setUsdtInCan(uint256 _Usdt) public onlyOwner {
        Seas_TO_HATCH_1MINERS = _Usdt;
    }

    function setUsdt(address _adr) public onlyOwner {
        require(!initialized); //ones start cannot change token
        Usdt = IERC20Upgradeable(_adr);
    }
