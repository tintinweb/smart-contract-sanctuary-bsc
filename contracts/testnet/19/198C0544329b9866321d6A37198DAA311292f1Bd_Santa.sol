...
contract Santa is IERC20, Ownable {
    ...
    address public donationFeeReceiver;
    address public devAddress;

    // swap percentage
    uint256 public rewardSwap = 300;
    uint256 public marketingAndDAOSwap = 100;
    uint256 public donationSwap = 50;
    uint256 public devSwap = 50;
    uint256 public totalSwap = 500;
    ...
    uint256 public minSwapTime = 3600;
    uint256 public lastSwappedAt;

    bool public swapEnabled = true;
    uint256 public swapThreshold = (_totalSupply * 1) / 2000; // 0.5% of supply
    ...
    event ChangePinkAntiBot(bool status);
    event EnableTrading(bool status);
    event ChangeWalletDividendStatus(address holder, bool status);
    event ChangeWalletFeeStatus(address holder, bool status);
    event AddAuthorizedWallets(address holder, bool status);
    event ChangeSwapBackSettings(bool status, uint256 amount);
    event ExcludeFromMaxWalletLimit(address wallet, bool status);
    event ChangeDistributionCriteria(
        uint256 minPeriod,
        uint256 minDistribution
    );
    event MaxWalletAmountChange(uint256 amount);
    ...
    constructor() {
        ...
        donationFeeReceiver = 0x9c55657ED2DFAb5988281DD5204A91743713b902;
        devAddress = 0xeffE48E632C21482c462e92ad3625f0a099e3c22;
    ...
    function _transferFrom(
        ...
        if (!isMaxWalletExempt[recipient] && recipient != pair) {
            ...
            require(
                (currentBalance + amount) <= maxWalletAmount,
    ...
    function takeFee(address sender, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 feeToken = (amount * totalSwap) / 10000;
    ...
    function setEnableAntiBot(bool _enable) external onlyOwner {
        ...
        emit ChangePinkAntiBot(_enable);
    }
    ...
    function doContractSwap() internal swapping {
        ...
        swapTokensForTokens(contractTokenBalance, REWARD);

        uint256 swappedTokensAmount = IERC20(REWARD).balanceOf(address(this));

        uint256 busdForReward = (swappedTokensAmount * rewardSwap) / totalSwap;

        uint256 busdForMarketing = (swappedTokensAmount * marketingAndDAOSwap) /
            totalSwap;

        uint256 busdForDonation = (swappedTokensAmount * donationSwap) /
            totalSwap;

        uint256 busdForDev = swappedTokensAmount -
            busdForDonation -
            busdForMarketing -
            busdForReward;
        ...
        if (busdForMarketing > 0) {
            IERC20(REWARD).transfer(marketingFeeReceiver, busdForMarketing);
        }

        if (busdForDonation > 0) {
            IERC20(REWARD).transfer(donationFeeReceiver, busdForDonation);
        }

        if (busdForDev > 0) {
            IERC20(REWARD).transfer(devAddress, busdForDev);
        }
        ...
        onlyOwner
    {
        ...
        emit ChangeWalletDividendStatus(holder, exempt);
    }

    function setIsFeeExempt(address holder, bool exempt) external onlyOwner {
        isFeeExempt[holder] = exempt;

        emit ChangeWalletFeeStatus(holder, exempt);
    }
        ...
        onlyOwner
    {
        ...
        emit AddAuthorizedWallets(holder, exempt);
    }

    function setIsMaxWalletExempt(address _wallet, bool _status)
        external
        onlyOwner
    {
        isMaxWalletExempt[_wallet] = _status;
        emit ExcludeFromMaxWalletLimit(_wallet, _status);
    }

    function changeMaxWalletAmount(uint256 amount) external onlyOwner {
        require(amount >= 1000000000, "Amount should greater than 1%");
        maxWalletAmount = amount * 10**18;

        emit MaxWalletAmountChange(amount);
    }
        ...
        onlyOwner
    {
        ...
        emit ChangeSwapBackSettings(_enabled, _amount);
    }
