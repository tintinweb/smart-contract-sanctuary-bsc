...
contract Staking {
    ...
    function stakeToken(uint256 _amount, uint256 _timePeriod) public returns(uint256) {
        ...
        stakeDetails memory newStake = stakeDetails({
            ...
            staker: msg.sender,
            stakeAmount: _amount,
            stakeTimePeriod: _timePeriod,
            stakeTimeEnd: block.timestamp + _timePeriod * 1 days,
            unstaked: false
        });
    ...
    function unstake(uint256 _stakeID) public {
        ...
        require(block.timestamp > stakes[_stakeID].stakeTimePeriod);
        stakeIDs = stakeIDs + 1;
        ...
        uint256 unstakeAmount = stakes[_stakeID].stakeAmount.add(rewardAmount);
    ...
    function calculateReward(uint256 _stakeID) public view returns(uint256) {
        ...
        if(stakes[_stakeID].stakeTimePeriod == 7) {
            rewardPercentage = sevenDaysRewardPercentage;
        } else if(stakes[_stakeID].stakeTimePeriod == 15) {
            rewardPercentage = fifteenDaysRewardPercentage;
        } else if(stakes[_stakeID].stakeTimePeriod == 30) {
