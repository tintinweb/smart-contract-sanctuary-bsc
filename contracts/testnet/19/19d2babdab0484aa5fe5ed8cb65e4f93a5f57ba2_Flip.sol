...
abstract contract gamebalances {
    ...
    mapping(uint256 => uint256) private recevievedVRF;
    ...
    */
    ...
    function updatePlayerHistory(address playerAddress) internal returns (uint256) {
        ...
        for(uint256 i=0;i<playerPlaysHistory[playerAddress].length;i++) {
            if(playerPlaysHistory[playerAddress][i].outcome==0) {
                uint256 vrfResult = recevievedVRF[playerPlaysHistory[playerAddress][i].vrfID];
                    if(vrfResult>0) {
                        uint256 n = vrfResult%2==0 ? 1 : 2;
    ...
    function getReceivedVrfNumByVrfId(uint256 vrfID) public view returns (uint256) {
        return recevievedVRF[vrfID];
    }
    function saveVRFvalues(uint256 vrfID,uint256 vrfNum) internal {
        recevievedVRF[vrfID] = vrfNum;
    }
...
contract Flip  is Ownable, Cointoss, ReentrancyGuard {
    ...
    function fulfillRandomWords(uint256, /* requestId */  uint256[] memory randomWords ) internal override {
        ...
        //recevievedVRF[s_requestId] = randomWords[0];
        saveVRFvalues(s_requestId,randomWords[0]);
