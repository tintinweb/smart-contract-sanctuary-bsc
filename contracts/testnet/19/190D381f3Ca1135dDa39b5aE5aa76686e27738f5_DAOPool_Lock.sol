 ...
 */
 ...
 contract DAOPool_Lock is Context, Ownable {
     ...
     address private _Dev_address = 0x4a1fAC92414Ce682D32c6BD91c4853139384C0B1;
     ...
     function _RemovefromLocked(uint256 _amount) private {
         ...
         if(TokenLockedPerson[_msgSender()] == 0){
             lockedPerson -= 1;
         }
     }
    ...
    function ChangeTotalDepositValue(uint256 newTotalDeposit) external onlyOwner returns (uint256){
         require(newTotalDeposit >= 0, "Can't set Zero Value");
         return _ChangeTotalDepositValue(newTotalDeposit);
     }
 
     function _ChangeTotalDepositValue(uint256 _newTotalDeposit) private returns (uint256) {
         emit ChangeMaxTotalDeposit(MaxTotalDeposit, _newTotalDeposit);
         MaxTotalDeposit = _newTotalDeposit;
         return MaxTotalDeposit;
     }
     ...
     function _ChangeDevAddress(address _adrs) private returns(bool) {
      ...
      function ContractApprove(uint256 amount) public returns (bool)
	 {
		 return tokenx.approve(address(this), amount);
	 }
	 
     function getStatistic(address adrs) public view returns (string memory projectname, string memory symbol , uint256 decimals, uint256 totalSupply, uint256 totalLock, address token, address admin,  uint256 time, uint256 timeLock, uint256 tokenPerson, uint256 locktimePerson, uint256 allowance) 
	 {
		projectname = tokenx.name();               
		symbol = tokenx.symbol();                  
		decimals =	tokenx.decimals();              
		totalSupply	= tokenx.totalSupply(); 
        totalLock = lockedToken;      
		token =	token_address;                    
		admin =	owner();                          
		time =	getTime();                          
		timeLock =	getUnlockTime();               
		tokenPerson = TokencLockedCheckView(adrs);
		locktimePerson = TimeDifferential(adrs);
        allowance = tokenx.allowance(adrs, address(this));
     }
