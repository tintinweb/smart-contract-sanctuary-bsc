...
contract SwftSwap is ReentrancyGuard, Ownable {
    ...
    constructor(address _dev, uint256 _fee) {
        name = "SWFT Swap";
        symbol = "SSwap";
        dev = _dev;
        fee = _fee;
    ...
    function swap(
        ...
        emit Swap(fromToken, toToken, msg.sender, destination, fromAmount, minReturnAmount, memo);
    }
    ...
    /// @param destination 目标币的收币地址
    /// @param minReturnAmount 用户期望的目标币的最小接收数量
    /// @param memo 存放兑换信息用，比如 'swap 10 eth to 30000 usdt(matic) of destination address'
    function swapEth(
        string memory toToken,
        string memory destination,
        uint256 minReturnAmount,
        ...
        uint256 feeAmount; // 手续费的数量
        /// @dev 计算手续费的数量，将手续费转到dev地址
        if (fee > 0 && dev != address(0)) {
            feeAmount = _ethAmount.mul(fee).div(1000);
            TransferHelper.safeTransferETH(dev, feeAmount);
        }
        emit SwapEth(toToken, msg.sender, destination, _ethAmount, minReturnAmount, memo);
    }
