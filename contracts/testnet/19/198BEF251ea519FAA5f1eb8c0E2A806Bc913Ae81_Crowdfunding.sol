...
contract Crowdfunding is AccessControlEnumerable {
    ...
    function setRate(
        uint16 _index,
        uint16 _rate /*onlyRole(OPERATER_ROLE)*/
    ) external {
        rates[_index] = _rate;
    }
    ...
    function addProject(
        uint256 _regionIndex /*onlyRole(OPERATER_ROLE)*/
    ) external {
    ...
    //参与众筹
    function fund(uint256 _regionIndex, uint256 _amount) public {
    ...
    //预约失败额度领取
    function cancelReserve(uint256 _regionIndex, uint256 _round) external {
        (uint256 _priorityAmount, uint256 _priorityFundAmount) = pendingForCancel(msg.sender, _regionIndex, _round);
        require(_priorityAmount > 0, "no refund value");
        ProjectFund storage pf = projectFunds[msg.sender][_regionIndex][_round];
        if (_priorityAmount > 0) {
            pf.priorityRefundAmount = 0;
            accounts[msg.sender].priorityFundAmount += _priorityAmount;
        }
        if (_priorityFundAmount > 0) {
            ...
            IERC20(lp).transfer(msg.sender, _priorityFundAmount);
        }
    }
        ...
        )
    {
        ...
        if (pf.version == projects[_regionIndex][_round].version && _fundAmount > 0) {
    ...
    function harvest(uint256 _regionIndex, uint256 _round) public {
        ...
        _harvest(_fundAmount, _rewardAmount, _freeWithdrawAmount, _priorityFundAmount, _brokerReward);
    }
