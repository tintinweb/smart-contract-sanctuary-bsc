...
contract STRTLocking is Initializable, UUPSUpgradeable, OwnableUpgradeable, adminProxyManager {
    ...
    uint64 public apy_d2; // decimal 2
     ...
     */
    function lockDuration() public view virtual returns (uint64){
     ...
     */
    function pendingRewards(uint128 _lockIndex, address _member) public view virtual returns(uint128){
        if(!everLocked(_member)) return 0;

        Detail memory summary = summaries[_member][_lockIndex];

        uint64 startDate = summary.lastClaimed;
        if(startDate == 0) startDate = summary.start;

        uint64 endDate = uint64(block.timestamp);
     ...
     */
    function calcReward(uint128 _amount, uint64 _start, uint64 _end) public view virtual returns (uint128) {
        uint128 rewardPerSecond = ((((uint128(apy_d2) * _amount) / 12) / 30) / 86400) / 10000;
        return uint128(_end - _start) * rewardPerSecond;
    }
     ...
     */
    function getMembersLength() public view virtual returns(uint256){
     ...
     * @param _member Member address
     */
    function getUserLockedLength(address _member) public view virtual returns(uint256){
     ...
     * @param _member Member address
     */
    function getLockedTokensBeforeDate(address _member, uint64 _before) public view virtual returns (uint128 lockedTokens) {
     ...
     * @param _member Member address
     */
    function lock(uint128 _amount, address _member) public virtual isLockPaused {
     ...
     * @param _member Member address
     */
    function claim(uint128 _lockIndex, address _member) public virtual isLockPaused {
    ...
    /**
     * @dev Get & lock the rewards at specified lock index of member
     * @param _lockIndex Index of lock
     * @param _member Member address
     */
    function relock(uint128 _lockIndex, address _member) public virtual isLockPaused {
        uint128 reward = _claim(_lockIndex, _member);
     ...
     * @param _member Member address
     */
    function unlock(uint128 _lockIndex, address _member) public virtual isLockPaused {
        uint64 dateTimeNow = uint64(block.timestamp);
        Detail memory summary = summaries[_member][_lockIndex];

        require(everLocked(_member) && summary.rewardEnded == 0, "bad");
     ...
     * @param _member Member address
     */
    function unlockAndClaim(uint128 _lockIndex, address _member) public virtual isLockPaused {
        require(summaries[_member][_lockIndex].end <= uint64(block.timestamp), "bad");

        unlock(_lockIndex, _member);
    ...
    function _lock(uint128 _amount, address _member) internal virtual {
        ...
        summaries[_member].push(Detail(uint128(summaries[_member].length), _amount, dateTimeNow, dateTimeNow + lockPeriod, 0, 0));
     ...
     * @param _member Member address
     */
    function _claim(uint128 _lockIndex, address _member) internal virtual returns(uint128 reward) {
        uint64 dateTimeNow = uint64(block.timestamp);
    ...
    /**
     * @dev Delete lock when it was unlocked & claimed
     * @param _lockIndexToDelete Index of lock to delete
     * @param _member Member address
     */
    function _deleteLockIndex(uint128 _lockIndexToDelete, address _member) internal virtual {
        uint128 lockIndexToMove = uint128(summaries[_member].length - 1);
    ...
    /**
     * @dev Delete members when s/he had no running locked
     * @param _member Member address
     */
    function _deleteMember(address _member) internal virtual {
        require(!everLocked(_member), "bad");

        address memberToMove = members[members.length - 1];
        uint128 indexToDelete = memberIndex[_member];
    ...
    /**
     * @dev Update APY in percent 2 decimal
     * @param _apy_d2 Apy in percent 2 decimal (ex. 18% into 1800)
     */
    function updateApy(uint64 _apy_d2) public virtual onlyOwner {
        apy_d2 = _apy_d2;
    }
     ...
     */
    function updateMinLock(uint128 _minLock) public virtual onlyOwner {
        minLock = _minLock;
    }
     ...
     */
    function updatePenalty(uint64 _penalty_d2) public virtual onlyOwner {
        penalty_d2 = _penalty_d2;
    }
     ...
     */
    function updateFee(uint64 _fee_d2) public virtual onlyOwner {
        fee_d2 = _fee_d2;
    }
     ...
     */
    function updateLockDuration(uint64 _lockDurationInMonth) public virtual onlyOwner {
        lockPeriod = _lockDurationInMonth * 30 * 86400;
    }
     ...
     */
    function updateToken(address _token) public virtual onlyOwner {
        token = _token;
    }
     ...
     */
    function toggleLocked() public virtual onlyOwner {
...
contract adminProxyManager is Proxied {
  bool adminProxyManagerInitialized;

  function __adminProxyManager_init(address _sender) internal virtual {
    require(!adminProxyManagerInitialized, "initialized");

    assembly {
      sstore(0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103, _sender)
    }

    adminProxyManagerInitialized = true;
  }
  ...
  function transferProxyAdmin(address _newProxyAdmin) public virtual proxied{
