...
contract ElumntNFTMarketplace is Initializable, UUPSUpgradeable, OwnableUpgradeable, Verification {
    ...
    struct collectionAcceptOffer721Data {
        bytes signature;
        string encodeKey;
        uint256 nonce;
        address newOwner;
    ...
    struct collectionAcceptOffer1155Data {
        bytes signature;
        string encodeKey;
        uint256 nonce;
        address newOwner;
    ...
    function collectionAcceptOfferBid721(collectionAcceptOffer721Data memory _transferData) external payable onlyProxy {
        require(!seenNonces[msg.sender][_transferData.nonce], "Invalid request");
        seenNonces[msg.sender][_transferData.nonce] = true;
        require(verify(msg.sender, msg.sender, _transferData.amount, _transferData.encodeKey, _transferData.nonce, _transferData.signature), "invalid signature");
        single721Data[] memory nftsData = _transferData.nftsData;
        ...
        for(uint8 i = 0; i < nftsData.length; i++) {
            ...
            uint256 royaltyPercent;
            if(nftsData[i].royalty > 0 && _transferData.creator != msg.sender) {
            ...
            transfer721(nftsData[i].nft, msg.sender, _transferData.newOwner, nftsData[i].tokenId);
        }
        if(totalPlatformShare > 0) {
            transfer20(_transferData.newOwner, address(this), totalPlatformShare, _transferData.erc20token);
        }
        if(totalRoyalty > 0) {
            transfer20(_transferData.newOwner, _transferData.creator, totalRoyalty, _transferData.erc20token);
        }
        transfer20(_transferData.newOwner, msg.sender, amountToTransfer, _transferData.erc20token);
        emit CollectionOfferAccepted(tokenIds, msg.sender, _transferData.newOwner, creatorEarnings);
    }
    function collectionAcceptOffer1155(collectionAcceptOffer1155Data memory _transferData) external payable onlyProxy {
        require(!seenNonces[msg.sender][_transferData.nonce], "Invalid request");
        seenNonces[msg.sender][_transferData.nonce] = true;
        require(verify(msg.sender, msg.sender, _transferData.amount, _transferData.encodeKey, _transferData.nonce, _transferData.signature), "invalid signature");

        single1155Data[] memory nftsData = _transferData.nftsData;
        uint256 amountToTransfer;
        uint256 totalRoyalty;
        uint256 totalPlatformShare;
        uint256[] memory tokenIds;
        ...
        for(uint8 i = 0; i < nftsData.length; i++) {
            ...
            amountToTransfer += singleNFTAmount;
            transfer1155(nftsData[i].nft, msg.sender, _transferData.newOwner, nftsData[i].quantity, nftsData[i].tokenId);
        }
        if(totalPlatformShare > 0) {
            transfer20(_transferData.newOwner, address(this), totalPlatformShare, _transferData.erc20token);
        }
