...
contract ClaimTokenVesting is Ownable, Pausable, ReentrancyGuard {
    ...
    function claim(uint256 _amount) external nonReentrant whenNotPaused {
        ...
        require(_amount * (10 ** 18) <= amountUnclaimed + rewardDebt,"exceed tokens that can be received");

        uint256 claimedTo = user.claimedTo;

        if (amountUnclaimed > 0) {
            ...
            userInfo[_msgSender()].rewardDebt = amountUnclaimed + rewardDebt - (_amount * (10 ** 18));
