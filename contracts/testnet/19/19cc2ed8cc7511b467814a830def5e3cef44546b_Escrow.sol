...
contract Escrow {
   ...
   // Defining function modifier 'onlyBuyer'
    ...
    function placeOrder(address _token, address _seller, uint256 _amount, uint256 _taxAmount, uint256 _dealPeriodInDays) public {
        ...
        _dealIDs.increment();
        uint256 newDealId = _dealIDs.current();
        Deal memory deal = Deal(newDealId, msg.sender, _seller, _amount, _token, block.timestamp, block.timestamp + _dealPeriodInDays * 1 days, State.await_delivery);
