...
contract NOMANTOKENDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("NOMANTOKEN_Dividend_Tracker", "NOMANTOKEN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "NOMANTOKEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "NOMANTOKEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NOMANTOKEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "NOMANTOKEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "NOMANTOKEN_Dividend_Tracker: Cannot update claimWait to same value");
...
contract NOMANTOKEN is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    NOMANTOKENDividendTracker public dividendTracker;
...
WITH OPTION FOR BLACKLIST
*/
    constructor() ERC20("NOMAN TOKEN", "NOMAN") {
        BNBRewardsFee = 1;
        liquidityFee = 1;
        marketingFee = 1;
        devFee = 1;
        ...
        marketingWallet = payable(0x67f7fFF86b5AB08EeC870aFb5E72AF8113166A7B); // 4
        developmentWallet = payable(0x5Cea5Cc09E749f1f52eBA56d5F666B7CD770DD4A); // 5 

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSellDevFee);

    	dividendTracker = new NOMANTOKENDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "NOMANTOKEN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "NOMANTOKEN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NOMANTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NOMANTOKEN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NOMANTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NOMANTOKEN: Cannot update gasForProcessing to same value");
