...
contract Pool is Initializable, PoolStorage, OwnableUpgradeable, ReentrancyGuardUpgradeable, IPool {
    ...
    function initialize(
        ...
        _setMaintenanceMarginRatio(_maintainanceMargin);
        fee.daoFee = PRECISION;
    }

    function setOwner(address _add) external {
        _transferOwnership(_add);
    }
    ...
    function setMaintenanceMarginRatio(uint256 _maxLeverage) external onlyOwner {
        _setMaintenanceMarginRatio(_maxLeverage);
    }
    ...
    function _setMaintenanceMarginRatio(uint256 _ratio) internal {
        _validateMaxValue(_ratio, MAX_MAINTENANCE_MARGIN_RATIO);
        maintenanceMarginRatio = _ratio;
        emit MaintenanceMarginRatioChanged(_ratio);
    }
        ...
        returns (bool allowed)
    {
        ...
        return collateral.mul(PRECISION).lt(_position.size * maintenanceMarginRatio)
...
uint256 constant MAX_MAINTENANCE_MARGIN_RATIO = 5e8; // 5%
