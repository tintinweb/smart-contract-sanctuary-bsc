...
contract MasterChef is Ownable {
    ...
    function setAddress(address _devAddress, address _rewardAddress) external onlyOwner {
        devaddr = _devAddress;
    ...
    function getMultiplier(uint256 _from, uint256 _to, uint256 _lock) public pure returns (uint256) {
        ...
        if (delta >= _lock){
            return _lock.mul(1e12).div(365 days);
        }
        return delta.mul(1e12).div(365 days);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        RewardToken.safeTransferFrom(rewardaddr, devaddr, feeAmount);
        RewardToken.safeTransferFrom(rewardaddr, msg.sender, pending.sub(feeAmount));
    ...
    function setRewardAddr(address _rewardaddr) public onlyOwner {
        require(msg.sender != address(this), "rewardaddr: wut?");
        require(msg.sender != address(0), "rewardaddr: wut?");
