...
//   }
  ...
  function stakeNft(uint256[] calldata _idNftList) public nonReentrant isRun {
    ...
    //uint256[] memory nftList = new uint256[](_idNftList.length);  
  ...
  function unstakeToken(uint256 amount) public nonReentrant {
    require(blacklistMap[msg.sender] == false, "You are in Blacklist");
    require(amount <= nftIdList[msg.sender].length,"Amount exceeded");
    ...
    for (uint256 index = 0; index < amount; index++) {
      //userNftDetail storage nftDetail = userNftMap[msg.sender][userNft-index-1];
        // uint256 numberOfBlocks = unstakeBlock[msg.sender] - stakeBlock[msg.sender];
        // nftTotalGain += numberOfBlocks.mul(nftPrice).mul(amount);
        // Give back NFT
        IERC721(hesicNftAddress).transferFrom(address(this), msg.sender, nftIdList[msg.sender][userNft-index-1]);
        // for(uint256 jndex = 0; jndex < nftIdList[msg.sender].length; jndex++){
        //     nftIdList[msg.sender][jndex] = nftIdList[msg.sender][jndex+1];
        // }
        
        //delete nftIdList[msg.sender][userNft-index-1];
...
//     nftTotalGain = (block.number - stakeBlock[msg.sender]).mul(nftPrice).mul(amount).mul(benefitPercentPerYear).div(100).div(10368000);
//     return nftTotalGain;
//   }
