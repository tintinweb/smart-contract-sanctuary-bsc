...
contract Mining {
    ...
    function mint(
        ...
        emit Minted(mintedQuantity, msg.sender, _amount, _duration, _nftId);
        mintedQuantity++;
    }
    ...
    function _deleteMintingPackage(address account, uint256 mintedId) internal {
        if (vault[account][mintedId].contributeNFTId != 0) {
            CrownContract.stakeOrUnstake(
                vault[account][mintedId].contributeNFTId,
                false
            );
        }
        emit Unminted(mintedId, account, vault[account][mintedId].amount);
