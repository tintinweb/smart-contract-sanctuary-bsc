...
contract Trader is Ownable, Pausable, ITrader {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using UniversalERC20 for IERC20;

    address private dexRouterAddress;
    address private WETH;
    uint256 private constant TTL = 30 * 60; //30 minutes
    ...
    constructor(address _dexRouterAddress, address _weth) {
    ...
    function trySell(
        ...
        uint256 minPrice,
        uint256 maxPrice
    ) external payable override whenNotPaused returns (uint256 returnAmount) {
        emit CallSwap(
            dexRouterAddress,
            srcToken,
            dstToken,
        ...
        uint256 balanceBeforeDst = dstToken.universalBalanceOf(address(this));
        uint256 balanceBeforeSrc = srcToken.universalBalanceOf(address(this));

        srcToken.universalTransferFrom(msg.sender, address(this), amount);

        // 检查转入的数量是否正常
        uint256 balanceAfterSrc = srcToken.universalBalanceOf(address(this));
        ...
        require(
            balanceAfterSrc.sub(balanceBeforeSrc) == amount,
            "srcToken amount not enough"
        );

        //执行 dex swap 操作
        _executeSwapUniswapV2(srcToken, dstToken, amount);
        // 检查转入的数量是否正常
        uint256 balanceAfterDst = dstToken.universalBalanceOf(address(this));
        ...
        return balanceAfterDst.sub(balanceBeforeDst);
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    function _executeSwapUniswapV2(
        IERC20 srcToken,
        IERC20 dstToken,
        uint256 amount
    ) private returns (uint256 returnAmount) {
        // 如果 src 和 dst 一致直接抛出异常
        require(
            dstToken != srcToken,
            "dst token should should not same as src"
        );
        //授权并且获取swap路径
        address[] memory path = _prepareSwapUniswapV2(
            dstToken,
            srcToken,
            amount
        );
        ...
        if (srcToken.isETH()) {
            IUniswapV2Router02(dexRouterAddress)
                ...
                value: amount
            }(0, path, address(this), block.timestamp + TTL);
        } else if (dstToken.isETH()) {
            IUniswapV2Router02(dexRouterAddress)
    ...
    function _prepareSwapUniswapV2(
        IERC20 dstToken,
        IERC20 srcToken,
        uint256 amount
    ) private returns (address[] memory) {
        // 授权给dexRouter对合约内的币进行操作
        srcToken.universalApprove(dexRouterAddress, amount);
        address[] memory path = new address[](2);
        path[0] = srcToken.isETH() ? WETH : address(srcToken);
        path[1] = dstToken.isETH() ? WETH : address(dstToken);
        return path;
    }
}
