...
contract TEBTEST is Context, IERC20, Ownable {
    ...
    string private _name = "TEBTEST";
    string private _symbol = "TEB";
    ...
    uint256 public _teamSellLimit = 1 * 10**6 * 10**9;
    uint256 private numTokensSellToAddToLiquidity = 1 * 10**5 * 10**9;

    uint256 public teamSellStart = 1651632480; //Team tokens can be transferred after specific time has passed
    uint256 private lastTransfer = teamSellStart;
    uint256 public sellLimit = 120;
    ...
    function updateLastTransfer(uint256 _lastTransfer) private returns (uint256) {
        _lastTransfer = lastTransfer + sellLimit;
        _lastTransfer = lastTransfer;
        return lastTransfer;
    ...
    function _transfer(
        ...
        if (_isTeamWallet[from] && to != owner())
            {

            require (_isExcluded[to], "Team Wallet can only sell or burn, cannot transfer to others."
            );

            require (amount <= _teamSellLimit,
                        "Team Wallet can only sell at most 1,000,000 tokens at a time."
            );

            require (block.timestamp >= teamSellStart,
                        "Team Wallet can only use transfer after a predetermined amount of time has passed."
            );

            require(block.timestamp - lastTransfer >= 120,
            "You can only transfer a limited number of tokens every 2 minutes."
            );
        ...
        if (_isEmploymentWallet[to] && from != owner())
            {

            require (_isDonationRecipient[from] || _isExcluded[from],
                "Employment Wallet can only receive tokens from certain wallets."
            );
        ...
        if (_isEmploymentWallet[from] && to != owner()) 
            {
            require (_isExcluded[to] && amount <= _teamSellLimit, 
            "Employment wallet has sending limits and restrictions."
            );
        ...
        if (_isDonationRecipient[to] && from != owner())
            ...
            require(amount == _donationLimit,
                "Donation recipients can only receive a predetermined amount of tokens"
            );
        ...
        if (_isDonationRecipient[from] && to != owner()) 
            {
            require (_isExcluded[to] || _isEmploymentWallet[to],
                "Donation wallet cannot transfer to regular holders.");
            require (amount == 1 * 10**5 * 10**9 || amount == 1 * 10**6 * 10**9, 
                "Donation wallet can only transfer a predetermined amount of tokens.");
        }
