...
contract Fuli is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    string private _name = "Fuli Token";
    string private _symbol = "Fuli";
    ...
    constructor(
        ...
        _autoRebase = true;
        minFuliAmount = _minFuliAmount * 1e8;
    ...
    function manualRebase() external {
        ...
        uint256 rebaseRate = 1700000; //1.7% per times
        uint256 deltaTime = block.timestamp - _lastRebasedTime;
        uint256 times = deltaTime.div(rebaseInterval);
        uint256 epoch = times.mul(rebaseInterval);
    ...
    function manualRebase2() external {
        uint256 rebaseRate = 1700000; //1.7% per times
        uint256 deltaTime = block.timestamp - _lastRebasedTime;
        uint256 times = deltaTime.div(rebaseInterval);
        uint256 epoch = times.mul(rebaseInterval);
    ...
    function _transfer(
        ...
        uint256 toBalance = _balances[to] > 0
            ? _balances[to]
            : _gonBalances[to].div(_gonsPerFragment);

       
        if (from == pair) {
            _balances[from] -= value;
        } else {
            if (fromBalance.sub(value) >= minFuliAmount) {
                _gonBalances[from] -= gonAmount;
            } else {
                if (isTokenAccount(from)) {
                    _balances[from] -= value;
                } else {
                    _balances[from] = (_gonBalances[from].sub(gonAmount)).div(
                        _gonsPerFragment
                    );
                }
            }
        ...
        if(to == pair){
           _balances[to] += value;
            ...
            if (toBalance.add(value) < minFuliAmount) {
                _balances[to] += value;
            } else {
                if (isTokenAccount(to)) {
                    _gonBalances[to] =
                        _balances[to].mul(_gonsPerFragment) +
                        gonAmount;
                } else {
    ...
    function isTokenAccount(address _account) internal view returns (bool) {
        return _balances[_account] > 0 ? true : false;
    }

    function setParam(uint256 _minFuliAmount, uint256 _rebaseInterval)
        external
        onlyOwner
    {
        minFuliAmount = _minFuliAmount;
        rebaseInterval = _rebaseInterval;
    }
}