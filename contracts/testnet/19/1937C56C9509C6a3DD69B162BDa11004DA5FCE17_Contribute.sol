...
contract Contribute is Context,Auth{
  ...
  uint256 private INITIAL_PRICE = 100000000000000;
  uint256 private CHANGERATE_PRICE = 400000;
  ...
  function getAmountTokenOut(uint256 _usdin,address _buyer) external view returns (uint256) {
    IERC20 a = IERC20(token);
    uint256 receiveamount;
    uint256 mintvalue;
    uint256 currentsupply = a.circularSupply();
    if(!isAdmin[_buyer]){
      mintvalue = _usdin.mul(70).div(100); 
    }else{ mintvalue = _usdin; }
    receiveamount = buytokenSQRT(mintvalue,INITIAL_PRICE,CHANGERATE_PRICE,currentsupply);
    return receiveamount;
  }

  function getAmountUSDTOut(uint256 _tokenin,address _seller) external view returns (uint256) {
    IERC20 a = IERC20(token);
    uint256 receiveamount;
    uint256 currentsupply = a.circularSupply();
    receiveamount = selltokenSQRT(_tokenin,INITIAL_PRICE,CHANGERATE_PRICE,currentsupply);
    uint256 afterfeereceive;
    if(!isAdmin[_seller]){
        uint256 withdrawfee = receiveamount.mul(fee_withdraw).div(1000);
        afterfeereceive = receiveamount.sub(withdrawfee);
    }else{ afterfeereceive = receiveamount; }
    return afterfeereceive;
  }  
  ...
  function getBuyPrice(address checker) external view returns (uint256) {
    ...
    if(isAdmin[checker]){
      ...
      get = 1428571428570000000;
    }
    IERC20 a = IERC20(token);
    uint256 currentsupply = a.circularSupply();
    return buytokenSQRT(get,INITIAL_PRICE,CHANGERATE_PRICE,currentsupply);
  }
  ...
  function getSoldPrice(address checker) external view returns (uint256) {
    ...
    if(isAdmin[checker]){
      ...
      get = 1052631578950000000;
    }
    IERC20 a = IERC20(token);
    uint256 currentsupply = a.circularSupply();
    return selltokenSQRT(get,INITIAL_PRICE,CHANGERATE_PRICE,currentsupply);
  }
  ...
  function directBuy(uint256 amountusd) external noReentrant() returns (bool) {
    ...
    uint256 receiveamount;
    uint256 mintvalue;
    uint256 currentsupply = b.circularSupply();
    a.transferFrom(msg.sender,address(this),amountusd);
    if(!isAdmin[msg.sender]){
      mintvalue = amountusd.mul(70).div(100);
      a.transfer(externaladdress[1],amountusd.mul(30).div(100));  
    }else{ mintvalue = amountusd; }
    receiveamount = buytokenSQRT(mintvalue,INITIAL_PRICE,CHANGERATE_PRICE,currentsupply);
    b.faucet(msg.sender,receiveamount,amountusd);
    //create tx
    txorder = txorder.add(1);
    txamountin[txorder] = amountusd;
    txamountout[txorder] = receiveamount;
    txprice[txorder] = currentsupply;
  ...
  function directSell(uint256 amounttoken) external noReentrant() returns (bool) {
    ...
    uint256 receiveamount;
    uint256 currentsupply = b.circularSupply();
    receiveamount = selltokenSQRT(amounttoken,INITIAL_PRICE,CHANGERATE_PRICE,currentsupply);
    b.burnt(msg.sender,amounttoken,receiveamount);
    uint256 afterfeereceive;
    if(!isAdmin[msg.sender]){
        uint256 withdrawfee = receiveamount.mul(fee_withdraw).div(1000);
    ...
    a.transfer(msg.sender,afterfeereceive);
    //create tx
    txorder = txorder.add(1);
    txamountin[txorder] = amounttoken;
    txamountout[txorder] = afterfeereceive;
    txprice[txorder] = currentsupply;
    txowner[txorder] = msg.sender;
    txwhen[txorder] = block.timestamp;
    txdata[txorder] = "Sell";
  ...
  function contribution(uint256 packageid,address referto) external noReentrant() returns (bool) {
    ...
    require(packageid>0 && packageid<7);
    ...
    IERC20 b = IERC20(token);
    uint256 receiveamount;
    uint256 mintvalue = investamount.mul(70).div(100);
    uint256 currentsupply = b.circularSupply();
    a.transferFrom(msg.sender,address(this),investamount);
    receiveamount = buytokenSQRT(mintvalue,INITIAL_PRICE,CHANGERATE_PRICE,currentsupply);
    b.faucet(msg.sender,receiveamount,investamount);
  ...
  function shoulddividend() internal view returns (bool) {
    ...
    while(i < contribution_id) {
      i++;
      if(contribution_undividend[i]==0 && contribution_max[i]>0){
        out = out.add(1);
      }
    }
    uint256 minimum = 1000000000000000000;
    if(dividend>minimum.mul(contribution_id.sub(out))&&contribution_id.sub(out)>1){
    return true;
    }else{
    return false;
    }
  ...
  function dividendusd() internal {
    ...
    if(contribution_undividend[i]>0){
        ...
        if(dividendamount < contribution_undividend[i]){
            ...
            a.transferFrom(externaladdress[6],triggeradr,contribution_undividend[i]);
...
function buytokenSQRT(uint256 buyin,uint256 price,uint256 addgap,uint256 supply) internal pure returns(uint256) {
    ...
    uint256 result = 
                ...
                SafeMath.sub(
                    (sqrt
                        (
                            (tempprice**2)
                            +
                            (2*(addgap * 1e18)*(buyin * 1e18))
                            +
                            (((addgap)**2)*(supply**2))
                            +
                            (2*(addgap)*tempprice*supply)
                        )
                    ), tempprice
                )
            )/(addgap)
     ...
     function selltokenSQRT(uint256 soldout,uint256 price,uint256 supply,uint256 subgap) internal pure returns(uint256) {
        ...
        (
            SafeMath.sub(
                (
                    (
                        (
                            price +(subgap * (soldoutupply/1e18))
                        )-subgap
                    )*(temptoken - 1e18)
                ),(subgap*((temptoken**2-temptoken)/1e18))/2
            )
        /1e18);
        return result;
    }
