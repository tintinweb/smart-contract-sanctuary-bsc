...
contract NFTClaim is Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(address => uint256)) public nftBalanceList;
    ...
    event AddNftBalance(address _token, address _user, uint256 _num, uint256 _at);
    event AddNftBalanceMulti(address[] _token, address[] _user, uint256[] _num, uint256 _at);
    ...
    function addNftBalance(address _token, address _user, uint256 _num) external nonReentrant onlyAdmin {
        ...
        emit AddNftBalance(_token, _user, _num, block.timestamp);
    }

    function addNftBalanceMulti(address[] calldata _tokenArr, address[] calldata _userArr, uint256[] calldata _numArr) external nonReentrant onlyAdmin {
        require(_tokenArr.length > 0, "NONEMPTY_TOKEN_LIST");
        require((_tokenArr.length == _userArr.length) && (_tokenArr.length == _numArr.length), "INCONSISTENT_ARRAY");
        ...
        emit AddNftBalanceMulti(_tokenArr, _userArr, _numArr, block.timestamp);
    }
    ...
    function claimNft(address _token) public payable nonReentrant {
        ...
        uint256 nftNum = nftBalanceList[_token][msg.sender];
