...
contract ER is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    using Counters for Counters.Counter;
    using Strings for uint256;
    Counters.Counter private _tokenIds;

    constructor() ERC721A("ERName","ER") {}  // TODO zcy

    uint256 public maxSupply = 10000;
    // Whitelist Mint CONFIG
    uint256 public wlMintStartTime;
    uint256 public wlMintEndTime;
    uint256 public wlPrice = 0.001 ether;
    uint256 public maxMinted = 3;
    // Public Mint CONFIG
    uint256 public pMintStartTime;
    uint256 public pPrice = 0.0001 ether;
    // merkle config
    bytes32 public merkleRoot; // whitelist mint
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        return
            bytes(_baseURI()).length != 0
                ? string(abi.encodePacked(_baseURI(), _tokenId, suffixUri))
                : "";
    }
    ...
    function publicMint(uint256 _quantity) external payable callerIsUser nonReentrant {
        ...
        // verify mint gas
        require(msg.value >= pPrice * _quantity, "Insufficient gas fee");
        // verify mint max count
        require(totalSupply() + _quantity <= maxSupply);
        _safeMint(msg.sender, _quantity);
    }
