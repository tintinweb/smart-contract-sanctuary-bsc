...
contract UndeadToken is ERC20Upgradeable, OwnableUpgradeable, AccessControlUpgradeable, AntiShark {
  ...
  function emergencyTransfer(address _token) external onlyRole(_EMERGENCY_ROLE) {
    IERC20Upgradeable token = IERC20Upgradeable(_token);
    require(token.transfer(owner(), token.balanceOf(address(this))), "emergencyTransfer:01");

    emit EEmergencyTransfer(_token);
  }
