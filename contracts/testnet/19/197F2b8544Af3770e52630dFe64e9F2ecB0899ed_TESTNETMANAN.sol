...
// File: TESTNETMAN_flat.sol


// File: contracts/TESTNETMAN.sol
...
contract TESTNETMANAN is Context, IERC20, Ownable {
    ...
    address payable public _marketingWallet = payable(0x42a9194B9a5f405402f85860c95afA823EdB8307);
    ...
    bool private boostedTaxes = false;
    bool private boosted = false;
    ...
    function setTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner() {
        require(reflectFee + liquidityFee + marketingFee <= maximumTaxesPercent);
        _reflectFee = reflectFee;
        _liquidityFee = liquidityFee;
        _marketingFee = marketingFee;
    }

    function setBoostedTaxes(uint256 reflectFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner() {
        require(reflectFee + liquidityFee + marketingFee <= maximumTaxesPercent);
...
,        address to,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalBNBFees = _marketingFee.add(_liquidityFee);
        if (totalBNBFees == 0)
            return;
        ...
        uint256 marketingBalance = fromSwap.sub(liquidityBalance);
        ...
        transferEthOut(marketingBalance);
    }

    function transferEthOut(uint256 marketingBalance) internal {
