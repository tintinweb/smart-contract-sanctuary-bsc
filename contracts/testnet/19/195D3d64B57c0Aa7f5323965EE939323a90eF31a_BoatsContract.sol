...
contract BoatsContract is ERC721Enumerable, Ownable, Pausable {
    ...
    event MaxTokensUpdated(address caller, uint256 indexed newMaxTokens);
    ...
    {
        require(
            _nbTokens <= maxTokensPerMint,
            "Boats: You cannot mint more than maxTokensPerMint tokens at once!"
        );
    ...
    /// @notice updateMaxTokensPerMint, called only by owner
    /// @dev update the maximum number of tokens a user is allowed to mint at a time
    /// @param _newMaxTokens, new max tokens per mint
    function updateMaxTokensPerMint(uint256 _newMaxTokens) external onlyOwner {
        maxTokensPerMint = _newMaxTokens;
        emit MaxTokensUpdated(msg.sender, _newMaxTokens);
    }
    ...
    function safeOwnershipTransfer() external {
        ...
        require(proposedOwner != address(0), "Boats: proposed owner is the zero address");
