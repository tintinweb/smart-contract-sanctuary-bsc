...
contract DogeChampionsStakeProtocol is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable
{
    mapping(uint256 => uint256) tokenIdToStakeDate;


    uint256 public stakeRewardFactor;
    uint256 public stakeDurationFactor;
    ...
    function initialize() initializer public {
        ...
        stakeRewardFactor = 1;
        stakeDurationFactor = 604800; // one week
    }

    /*
     * @author stakes given NFT of the user
     */
    function stake(uint256 tokenId) public nonReentrant
    {
        require(dogeChampionsNFT.validatePlayability(msg.sender, tokenId), "Can't stake without owning given NFT.");
        require(tokenIdToStakeDate[tokenId] == 0, "Can't stake an NFT that is already staked.");

        tokenIdToStakeDate[tokenId] = block.timestamp;
        _pushToPlayableIds(msg.sender, tokenId);
    }

    /*
     * @author let's user collect claimable stake rewards and mints a random premium consumable
     */
    function collectStakeReward(uint256 tokenId) public nonReentrant
    {
        require(dogeChampionsNFT.validatePlayability(msg.sender, tokenId), "Can't collect stake rewards without owning given NFT.");
        require(tokenIdToStakeDate[tokenId] != 0, "Given NFT is not staked yet.");
        require(block.timestamp - tokenIdToStakeDate[tokenId] >= stakeDurationFactor, "Stake period is not over for given NFT.");

        tokenIdToStakeDate[tokenId] = 0;
        _removeFromPlayableIds(msg.sender, tokenId);
        dogeChampionsConsumable.rewardMint(msg.sender, (dogeChampionsNFT.getRarity(tokenId) + 1) * stakeRewardFactor, true);
    }
    ...
    /*
     * @author sets stake reward factor
     */
    function setStakeRewardFactor(uint256 factor) public onlyOwner
    {
        require(factor > 0, "Stake reward factor should be bigger than zero.");
        stakeRewardFactor = factor;
    }

    /*
     * @author sets stake duration factor
     */
    function setStakeDurationFactor(uint256 factor) public onlyOwner
    {
        require(factor > 0, "Stake duration factor should be bigger than zero.");
        stakeDurationFactor = factor;
    }

    function getUserStakeData(address walletAddress) public view returns(uint256[] memory stakedTokenIds, uint256[] memory stakeDates)
    {
        stakedTokenIds = userToStakedTokenIds[walletAddress];
        stakeDates = new uint256[](stakedTokenIds.length);

        for(uint256 i = 0; i < stakedTokenIds.length; i++)
        {
            stakeDates[i] = tokenIdToStakeDate[stakedTokenIds[i]];
        }

        return (stakedTokenIds, stakeDates);
    }
    ...
    {
        userToStakedTokenIds[walletAddress].push(tokenId);
    }
    ...
    {
        uint256[] memory playableIdArray = userToStakedTokenIds[walletAddress];
        ...
        userToStakedTokenIds[walletAddress] = newArray;
    }
}
