...
contract HolderRewarderDistributor is Structures {
    ...
    */
        ...
        if(!pendingTransactions.isEmpty()){
            ...
            while (share.rewardDate <= block.timestamp) {
                ...
                if(amount > 0) {
                    ...
                    if (isHolder[holder] == true && holdersToRewardsBalance[holder] == 0) {
                        removeHolder(holder);
                    } else if (holdersToRewardsBalance[holder] < int(UBRthreshold)) {
                        removeFromEligibleUBR(holder);
                    }
                }
    ...
    function processAllRewards() external {
        //require(block.timestamp >= lastDistribution + 1 days, "Cannot distribute two times in a day"); //TODO: uncomment for production
