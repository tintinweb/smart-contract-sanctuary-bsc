...
contract NFTHelp is Ownable {
    ...
    function tradeNFT(uint256 _GroupID) external lock haveNft haveHashpool haveBUSD haveSetting returns (uint256[] memory _tks) {
    ...
    function walletTop(uint _top) external view haveNft returns (address[] memory _wal, uint256[] memory _hash) {
