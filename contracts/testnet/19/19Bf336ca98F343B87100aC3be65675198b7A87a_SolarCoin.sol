...
contract SolarCoin is ERC20, Ownable ,Pausable {
...
constructor() ERC20("Solar Coin ", "SXP") {

        uint256 totalSupply = 1 * 1e15 * 1e9;
        
        swapTokensAtAmount = 1 * 1e4 * 1e9; // 10000 swap tokens amount
        maxTxn = totalSupply * 3 / 100; // 3% Max wallet

        rewardsBuyFee = 20; // 2% reward for holders
        marketingBuyFee = 50;
        liquidityBuyFee = 0;
         buyBurnFee=10; ////1% burn 
        totalBuyFees = rewardsBuyFee + marketingBuyFee + liquidityBuyFee+buyBurnFee;
        
        rewardsSellFee = 20;  // 2% reward for holders
        marketingSellFee = 50;
        liquiditySellFee = 0;
        sellBurnFee=10;
        totalSellFees = rewardsSellFee + marketingSellFee + liquiditySellFee+sellBurnFee;

   
    	
    	marketingWallet = address(0x6A96BB02A32B72e1C4Ea670aA2DCcD8bacF6DCEC); // set as marketing wallet
    ...
    function updateMaxWalletAmount(uint256 newNum) external onlyOwner{
        ...
        require(newNum > (totalSupply() * 1 / 100)/1e9, "Cannot set maxTxn lower than 1%");
        maxTxn = newNum * (10**9);
    }
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee ,uint256  _burnFee) external onlyOwner {
        ...
        liquidityBuyFee = _liquidityFee;
            buyBurnFee = _burnFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee+buyBurnFee;
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee,uint256 _burnFee) external onlyOwner {
        ...
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee+sellBurnFee;
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0){
                ...
                tokensForMarketing += fees * marketingSellFee / totalSellFees;
                 tokensForBurn += fees * sellBurnFee / totalSellFees;
            }
            ...
            // on buy
                ...
                tokensForBurn += fees * buyBurnFee / totalBuyFees;
            }
