...
contract LUNANAS is ERC20, Ownable {
    ...
    uint256 public lunapeBurnFeeOnBuy  = 1;
    uint256 public lunapeBurnFeeOnSell = 1;
    ...
    uint256 public bnbValueForLunapeBurn = 2e18;
    uint256 public accumulatedLunapeBurnBNB;
    ...
    address public lunape = 0x8a9424745056Eb399FD19a0EC26A14316684e274; //DAI
    ...
    constructor() ERC20("LUNANAS", "$LUNANAS") 
    {   
    ...
    function claimStuckTokens(address token) external onlyOwner {
        ...
        if (token == address(0x0)) {
            uint256 bnbBalance = address(this).balance - (accumulatedBuybackBNB + accumulatedLunapeBurnBNB);
    ...
    function updateBuyFees(
        ...
        uint256 _lunapeBurnFeeOnBuy
    ) external onlyOwner {
        require(
            (launchTime > 0 && launchTime + 3 minutes < block.timestamp) ||
            _rewardFeeOnBuy + _treasuryFeeOnBuy + _buybackFeeOnBuy + _lunapeBurnFeeOnBuy<= _totalFeesOnBuy,
        ...
        lunapeBurnFeeOnBuy = _lunapeBurnFeeOnBuy;

        _totalFeesOnBuy  = rewardFeeOnBuy + treasuryFeeOnBuy + buybackFeeOnBuy + lunapeBurnFeeOnBuy;

        emit UpdateBuyFees(rewardFeeOnBuy, treasuryFeeOnBuy, buybackFeeOnBuy, lunapeBurnFeeOnBuy);
    }
    ...
    function updateSellFees(
        ...
        uint256 _lunapeBurnFeeOnSell
    ) external onlyOwner {
        require(
            (launchTime > 0 && launchTime + 3 minutes < block.timestamp) ||
            _rewardFeeOnSell + _treasuryFeeOnSell + _buybackFeeOnSell + _lunapeBurnFeeOnSell <= _totalFeesOnSell,
            "Owner can not increase taxes in the first 14 days from listing"
        );
        require(
        ...
        lunapeBurnFeeOnSell = _lunapeBurnFeeOnSell;

        _totalFeesOnSell  = rewardFeeOnSell + treasuryFeeOnSell + buybackFeeOnSell + lunapeBurnFeeOnSell;

        emit UpdateSellFees(rewardFeeOnSell, treasuryFeeOnSell, buybackFeeOnSell, lunapeBurnFeeOnSell);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 lunapeBurnShare = lunapeBurnFeeOnBuy + lunapeBurnFeeOnSell;
            ...
            if (lunapeBurnShare > 0) {
                uint256 lunapeBurnBNB = (newBalance * lunapeBurnShare) / totalShares;
                accumulatedLunapeBurnBNB += lunapeBurnBNB;
                if (accumulatedLunapeBurnBNB > bnbValueForBuyBurn) {
                    if (address(this).balance >= accumulatedLunapeBurnBNB) {
                        lunapeBurn(accumulatedLunapeBurnBNB);
                    } else {
                        lunapeBurn(address(this).balance);
                    }
                    accumulatedLunapeBurnBNB = 0;
                }
            }
    ...
    function lunapeBurn(uint256 amount) internal {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = lunape;
