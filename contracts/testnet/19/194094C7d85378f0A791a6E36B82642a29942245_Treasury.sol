...
interface ILiquidityManager {
    function setCrystalToken(address _crystal, address _pair) external;

    function getPriceImpactCRSAmount(uint256 _priceImpact) external view returns (uint256);

    function getPriceImpactBusdAmount(uint256 _priceImpact) external view returns (uint256);

    function swapCRSToBusd(uint256 _crsAmount) external;

    function swapBusdToCRS(uint256 _busdAmount) external;
}
...
contract Treasury is ContractGuard, Operator {
    ...
    address[] public excludedFromTotalSupply = [
        address(0xe472642B93D9ABC837C094289C16D31B48485704) // GenesisPool
    ];
    ...
    uint256 public lmEnableThreshold;
    uint256[] public lmRange;
    uint256 public lmTradePrice;
    uint256 public lmPriceImpact;
    ...
    address public lmFund;
    uint256 public lmFundSharedPercent;
    ...
    event LMSFunded(uint256 timestamp, uint256 seigniorage);
    ...
    function toggleLMEnabled() external onlyOperator() returns (bool) {
    ...
    function initialize(
        ...
        lmEnableThreshold = 1.5e18;
        lmRange = [9.8e17, 9.9e17, 1.02e18, 2e18];
        lmTradePrice = 200 ether;
        lmPriceImpact = 50;
    ...
    function setLMEnableThreshold(uint256 _lmEnableThreshold) external onlyOperator {
        require(_lmEnableThreshold >= crystalPriceOne && _lmEnableThreshold <= crystalPriceOne.mul(500).div(100), "out of range");
        lmEnableThreshold = _lmEnableThreshold;
    }

    function setLMRange(uint256[] memory _lmRange) external onlyOperator {
        require(_lmRange.length == 4, "Liquidity Manager range should be contains 4 numbers");
        lmRange = _lmRange;
    }

    function setLMTradePrice(uint256 _tradePrice) external onlyOperator {
        require(_tradePrice > 0, "Liquidity Manager trade price should be non-zero value");
        lmTradePrice = _tradePrice;
    }

    function setLMPriceImpact(uint256 _priceImpact) external onlyOperator {
        require(_priceImpact > 0, "Liquidity Manager price impact should be non-zero value");
        require(_priceImpact < 10000, "Liquidity Manager price impact should be small than 100%");
        lmPriceImpact = _priceImpact;
    }
    ...
    function setExtraFunds(
        ...
        require(_lmFund != address(0), "LMS should be non-zero address");
        ...
        lmFund = _lmFund;
        lmFundSharedPercent = _lmFundSharedPercent;
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        ...
        uint256 _lmFundSharedAmount = 0;
        if (lmFundSharedPercent > 0) {
            _lmFundSharedAmount = _amount.mul(lmFundSharedPercent).div(10000);
            IERC20(crystal).transfer(lmFund, _lmFundSharedAmount);
            emit LMSFunded(block.timestamp, _lmFundSharedAmount);
        }
        ...
        _amount = _amount.sub(_treasuryFundSharedAmount).sub(_lmFundSharedAmount).sub(_lotteryFundSharedAmount);
    ...
    function _calculateMaxSupplyExpansionPercent(uint256 _crystalSupply) internal returns (uint256) {
        ...
        maxSupplyExpansionPercent = maxExpansionTiers[1];
        while (_crystalSupply < supply) {
            supply = supply.mul(125).div(100);
            maxSupplyExpansionPercent = maxSupplyExpansionPercent.mul(90).div(100);
        }
        return maxSupplyExpansionPercent;
    }

    function _performLiquidityManager(uint256 _crystalPrice) internal {
        uint256 amount = lmTradePrice.div(_crystalPrice);
        uint256 priceImpactAmount = 0;

        if (isLMEnabled) {
            if (_crystalPrice >= lmRange[0] && _crystalPrice < lmRange[1]) {
                priceImpactAmount = ILiquidityManager(liquidityManager).getPriceImpactBusdAmount(lmPriceImpact);
                if (amount > priceImpactAmount) {
                    ILiquidityManager(liquidityManager).swapBusdToCRS(priceImpactAmount);
                } else {
                    ILiquidityManager(liquidityManager).swapBusdToCRS(amount);
                }
            } else if(_crystalPrice >= lmRange[2] && _crystalPrice < lmRange[3]) {
                priceImpactAmount = ILiquidityManager(liquidityManager).getPriceImpactCRSAmount(lmPriceImpact);
                if (amount > priceImpactAmount) {
                    ILiquidityManager(liquidityManager).swapCRSToBusd(priceImpactAmount);
                } else {
                    ILiquidityManager(liquidityManager).swapCRSToBusd(amount);
                }
            }
        }
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs || isBoostEnabled) {
            ...
            if (previousEpochCRSPrice > crystalPriceOne) {
                ...
                uint256 _mse = _calculateMaxSupplyExpansionPercent(crystalSupply).div(10000).mul(crystalPriceOne);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
        ...
        // Liquidity Manager
        if (previousEpochCRSPrice > lmEnableThreshold) {
            isLMEnabled = true;
        }
        _performLiquidityManager(previousEpochCRSPrice);
    }
