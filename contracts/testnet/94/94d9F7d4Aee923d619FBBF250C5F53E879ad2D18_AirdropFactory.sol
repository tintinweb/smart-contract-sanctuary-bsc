...
interface IAirdropManager {
    ...
    event PoolForTokenCreated(address indexed token, address pool);
}


contract AirdropFactory is OwnableUpgradeable {
    ...
    event AirdropCreated(address indexed pool_address, address[3] _addrs, uint256[1] _saleInfo, string _poolDetails, uint256[3] _vestingInit);
    ...
    function createSale(
        ...
        require(
            msg.value >= masterPrice,
            "Payable Amount is less than required !!"
        );
        IAirdropManager(poolManager).addPoolFactory(pair);
        IAirdropManager(poolManager).registerPool(
            pair,
            _addrs[0],
            _addrs[2],
            version
        );
        emit AirdropCreated(pair, _addrs, _saleInfo, _poolDetails, _vestingInit);
    }
    ...
    function poolEmergencyWithdrawToken(
        ...
        IAirdrop(poolAddress).emergencyWithdrawToken(
            payaddress,
            tokenAddress,
            tokens
        );
    ...
    function poolEmergencyWithdraw(
        ...
        IAirdrop(poolAddress).emergencyWithdraw(to_, amount_);
    }
    ...
    onlyOwner
    {
        IAirdrop(poolAddress).setGovernance(_governance);
    }
}
...
contract AirdropMaster is OwnableUpgradeable, IAirdrop,ReentrancyGuard {
    ...
    bool public useWhitelisting = true;
    ...
    modifier inProgress() {
        ...
        require(block.timestamp >= startTime, "It's not time to buy");
        _;
    }
    ...
    function initialize(
        ...
        require(_saleInfo[0] >= block.timestamp, "Start time should be in the future");
    ...
    function addWhitelistedUsers(address[] memory users, uint256[] memory _allocations) external {
        require(_allocations.length == users.length, "INVALID INPUT");
        uint256 prevTotalCost = totalCost;
        for (uint256 i = 0; i < users.length; i++) {
            setWhitelist(users[i], true, _allocations[i]);
        }
        if(totalCost > prevTotalCost) {
            _safeTransferFromEnsureExactAmount(token, governance, address(this), (totalCost - prevTotalCost));
        }
        if(prevTotalCost > totalCost) {
            _transferFromEnsureExactAmount(token, governance, prevTotalCost - totalCost);
        }
        require(whitelistedUsers.length() <= MAX_ALLOCATIONS, "MAX_ALLOCATIONS Reached");
    }

    function addWhitelistedUser(address user, uint256 _allocation) external {
        uint256 prevTotalCost = totalCost;
        setWhitelist(user, true, _allocation);
        if(totalCost > prevTotalCost) {
            _safeTransferFromEnsureExactAmount(token, governance, address(this), (totalCost - prevTotalCost));
        }
        if(prevTotalCost > totalCost) {
    ...
    function removeWhitelistedUsers(address[] memory users) external {
        uint256 prevTotalCost = totalCost;
        for (uint256 i = 0; i < users.length; i++) {
            setWhitelist(users[i], false, allocations[users[i]]);
        }
        if(totalCost > prevTotalCost) {
            _safeTransferFromEnsureExactAmount(token, governance, address(this), (totalCost - prevTotalCost));
        }
    ...
    function removeWhitelistedUser(address user) external {
        uint256 prevTotalCost = totalCost;
        setWhitelist(user, false, allocations[user]);
        if(totalCost > prevTotalCost) {
            _safeTransferFromEnsureExactAmount(token, governance, address(this), (totalCost - prevTotalCost));
        }
        if(prevTotalCost > totalCost) {
    ...
    function setWhitelist(address user, bool whitelisted, uint256 _allocation) internal onlyOperator notInProgress {
        ...
        if (whitelisted) {
            if (!whitelistedUsers.contains(user)) {
                ...
                allocations[user] = _allocation;
                totalCost += _allocation;
            }
        } else {
            if (whitelistedUsers.contains(user)) {
    ...
    function getPoolInfo() external override view returns (address, address, uint8[] memory , uint256[] memory , string memory , string memory , string memory){
        ...
        state[1] = IERC20Info(token).decimals();
        info[0] = startTime;
        info[1] = tgeBps;
        info[2] = cycle;
        info[3] = cycleBps;
        info[4] = getNumberOfWhitelistedUsers();
        info[5] = totalClaimed;
        info[6] = totalCost;

        return (token, governance , state , info , IERC20Info(token).name() , IERC20Info(token).symbol() , poolDetails);
    }
    ...
    function getWhitelistedUsers(uint256 startIndex, uint256 endIndex) public view returns (address[] memory, uint256[] memory) {
        ...
        uint256[] memory whitelistedParticipantsAllocations = new uint256[](len);

        uint256 counter = 0;
        for (uint256 i = startIndex; i <= endIndex; i++) {
            whitelistedParticipantsPart[counter] = whitelistedUsers.at(i);
            whitelistedParticipantsAllocations[counter] = allocations[whitelistedUsers.at(i)];
            counter++;
        }
        return (whitelistedParticipantsPart, whitelistedParticipantsAllocations);
    }


    function claim() public nonReentrant inProgress onlyWhitelisted  {
        require(allocations[msg.sender] > 0, "Cant claim");
        uint256 volume = allocations[msg.sender];
        ...
        require(withdrawable > 0 , "No token available for claim!!");
    ...
    function changeStartAt(uint256 _startTime) external onlyOperator notInProgress {
        require (poolState == PoolState.inUse, "Pool was either finished or cancelled");
        startTime = _startTime;
        emit PoolUpdated(block.timestamp);
    }

    function cancel() external onlyOperator notInProgress {
        require (poolState == PoolState.inUse, "Pool was either finished or cancelled");
    ...
    function getUpdatedState() public view returns (uint256, uint256, uint8, string memory) {
        return (totalCost,totalClaimed, uint8(poolState), poolDetails);
    }

    function userAvalibleClaim(address _userAddress) public view returns (uint256){
        uint256 volume = allocations[_userAddress];
