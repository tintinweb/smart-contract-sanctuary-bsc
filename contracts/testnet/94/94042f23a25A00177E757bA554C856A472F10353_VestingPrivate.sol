...
contract VestingPrivate is Ownable {
    ...
    event OnLockSingle (address _addr, uint256 _amount);

    constructor(address _token, uint256 _tgeRelease){
        require(_token != address(0), "Wallet cannot be 0");
        token = _token;
        vestingPeriod = 1800;
        startClaim = block.timestamp + 300;
    ...
    function setVesting(
        ...
        vestingAmount[_addr] = _amount;
        fullAmount[_addr] = _amount;
        require(IERC20(token).transferFrom(_msgSender(), address(this), _amount), "Fail transfer");

        emit OnLockSingle(_addr, _amount);
    }
