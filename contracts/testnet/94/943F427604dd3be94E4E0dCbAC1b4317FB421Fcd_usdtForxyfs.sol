...
contract usdtForxyfs {
    ...
    function test2(uint256 amount)  public{
        TransferHelper.burn(_xyfsToken, amount);//xyfs 销毁
    }

    function test3(uint256 amount)  public{
        TransferHelper.safeTransferFrom(_seaToken,_poolToken,msg.sender, amount);//拿相应的sea
    }

    function test4(uint256 amount)  public{
        addressTotal[msg.sender] = addressTotal[msg.sender].add(amount);
        addExRecord(msg.sender,amount,amount,amount);
    }
