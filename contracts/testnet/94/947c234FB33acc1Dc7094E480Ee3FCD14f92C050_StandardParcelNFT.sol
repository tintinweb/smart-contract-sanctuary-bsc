    ...
    ReentrancyGuardUpgradeable
{
    ...
    function mint(address _user, uint256 _x, uint256 _y, uint256 _pt, uint256 _lt) public onlyMunicipality returns (uint256) {
        uint256 parcelId = _getParcelId(_x, _y, _pt, _lt);
        _mintFor(parcelId, _user);
        return parcelId;
    }

    function parcelExists(uint256 _x, uint256 _y, uint256 _pt, uint256 _lt) external view returns(bool) {
        return _parcelExists(_x, _y, _pt, _lt);
    }

    function getParcelId(uint256 _x, uint256 _y, uint256 _pt, uint256 _lt) external pure returns (uint256) {
        return _getParcelId(_x, _y, _pt, _lt);
    }
    ...
    // Private interface
    function _getParcelId(uint256 _x, uint256 _y, uint256 _pt, uint256 _lt) private pure returns (uint256) {
        uint256 token = _lt;
        token = (token << 8) | _pt;
        token = (token << 16) | _y;
        token = (token << 16) | _x;
    ...
    function _parcelExists(uint256 _x, uint256 _y, uint256 _pt, uint256 _lt) private view returns(bool) {
        uint256 parcelId = _getParcelId(_x, _y, _pt, _lt);
