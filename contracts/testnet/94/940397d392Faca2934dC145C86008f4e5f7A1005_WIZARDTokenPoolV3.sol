...
contract WIZARDTokenPoolV3 is Ownable, IERC721Receiver {
    ...
    function deposit(uint256 _amount) public {
        ...
        if (user.amount == 0 && _amount > 0) {
            if (nftToken.balanceOf(msg.sender) > 0 ) {
                 uint nftId = nftToken.tokenOfOwnerByIndex(msg.sender,0);
                 nftToken.safeTransferFrom(msg.sender, address(this), nftId);
                 user.nftStaked = nftId;
            }
            else {
                revert("deposit: no nftid found");
            }
        }
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if (user.amount == 0) {
                // transfer nft back to user wallet
                nftToken.safeTransferFrom(address(this), msg.sender, user.nftStaked);
                user.nftStaked = 0;
            }
