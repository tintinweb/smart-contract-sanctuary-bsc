...
contract GymMLM is OwnableUpgradeable {
    ...
    function distributeRewards(
        ...
        while (index < length && addressToId[userToReferrer[_user]] != 1) {
            ...
            uint256 userLevel  = getUserLevelInVault(referrer);
                if (index <= userLevel ) {
   ...
   function getUserLevelInVault(address _user) public view returns(uint256 userLevel) {
       userLevel = 0;
       uint256 levellimitSinglePool = firstDepositTimestamp[_user] != 0 && firstDepositTimestamp[_user] > singlePoolStartTimestamp ? 1 : 0;
