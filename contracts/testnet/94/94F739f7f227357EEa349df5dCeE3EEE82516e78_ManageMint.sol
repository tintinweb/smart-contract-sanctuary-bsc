...
contract ManageMint is AccessControl {
    ...
    function buyBox(address _collection, address _redeem,string memory _uri) public returns (uint256) {     
        require(!paused, "is paused");
        (address _token, uint256 _price) = INFTMistery(_collection).getPriceToken();
        address _owner = INFTMistery(_collection).getOwner();
        require(IERC20(_token).transferFrom(_redeem, _owner, _price), "Fail transfer");
        uint256 id = INFTMistery(_collection).redeem(_redeem, _uri);
        emit create(_redeem, id, _uri, "Buy Box");
        return id;
    }

    function minting(address _collection, address _redeem,string memory _uri) public returns (uint256) {
        require(!paused, "is paused");
    ...
    function openBox(address _collection, address _redeem,uint256 _idBox, string memory _uri) public returns (uint256) {
        require(!paused, "is paused");
        uint256 id = INFTMistery(_collection).openBox(_redeem, _idBox, _uri);
...
interface INFTMistery {
    ...
    function redeem(address _redeem,string memory _uri) external returns (uint256);

    function Minting(address _redeem,string memory _uri) external returns (uint256);

    function openBox(address _redeem,uint256 _idBox, string memory _uri) external returns(uint256);
