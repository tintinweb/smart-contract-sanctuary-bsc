...
contract BTSAuthRouter {
    ...
    function getUserTier(address queryAddress) public view returns (uint) {
        return userTier[queryAddress];
    }

    function getTierPrice(uint tierID) public view returns (uint) {
        return tierPrices[tierID];
    }

    function getTierDevFee(uint tierID) public view returns (uint) {
        return tierDevFees[tierID];
    }
