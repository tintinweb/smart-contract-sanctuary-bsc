...
import "../GuineaPigERC1155/IGuineaPigERC1155.sol";
    ...
    UUPSUpgradeable
{
    ...
    // GuineaPig ERC1155
    IGuineaPigERC1155 public guineaPigToken;
    ...
    function initialize(
        ...
        priceOne = 5 * 1e18; // 5 BUSD
        priceTwo = 10 * 1e18; // 10 BUSD
        priceThree = 15 * 1e18; // 15 BUSD
    ...
    function fulfillRandomness(
        ...
        Transaction memory _tx = ongoingTransaction[_msgSender()];

        if (_compareIfEqual(_tx.transactionType, "PURCHASE_GUINEA_PIG")) {
     ...
     */
    function purchaseGuineaPigWithBusd(uint8 _ix) external {
        // Valid index (1, 2 or 3)
        require(
            _ix == 1 || _ix == 2 || _ix == 3,
            "Marketplace: Index must be 1, 2 or 3"
        );

        // Verify that customer does not have an ongoing transaction
        require(
            !ongoingTransaction[_msgSender()].ongoing,
            "Marketplace: Customer cannot have multiple ongoing transactions."
        );

        uint256 price = _getGuineaPigPrice(_ix);
    ...
    function _finishPurchaseGuineaPig(
        ...
        (
            ...
            string memory _raceAndGender
                ...
                uint8(_randomNumbers[1] % 2)
            );

        bytes memory data = abi.encode(_account, _ix, block.number);

        // Mint Guinea Pigs
        guineaPigToken.mint(_account, _guineaPigId, 1, data);
        ...
        emit GuineaPigPurchase(_account, price, _guineaPigId, _raceAndGender);
    }
    ...
    function _getRaceGenderGuineaPig(
        ...
        if (_ix == 1) {
            likelihood = likelihoodRacePriceOne;
        } else if (_ix == 2) {
            likelihood = likelihoodRacePriceTwo;
        } else {
            likelihood = likelihoodRacePriceThree;
        }

        // race between 1 and 4
        uint8 i;
        for (i = 0; i < likelihood.length; i++) {
            if (_raceRN <= likelihood[i]) break;
        }
        i += 1; // 1, 2, 3, 4
        assert(i >= 1 && i <= 4);

        uint8 id = i + _genderRN * 4;
        assert(id >= 1 && i <= 8);

        string memory raceAndGender;
        if (id == 1) raceAndGender = "PERU MALE";
        else if (id == 2) raceAndGender = "INTI MALE";
        else if (id == 3) raceAndGender = "ANDINO MALE";
        else if (id == 4) raceAndGender = "SINTETICO MALE";
        else if (id == 5) raceAndGender = "PERU FEMALE";
        else if (id == 6) raceAndGender = "INTI FEMALE";
        else if (id == 7) raceAndGender = "ANDINO_FEMALE";
        else raceAndGender = "SINTETICO FEMALE";

        return (id, raceAndGender);
    }

    function _compareIfEqual(string memory a, string memory b)
        internal
        pure
        returns (bool)
    {
    ...
    function _getGuineaPigPrice(uint8 _ix) internal view returns (uint256) {
        if (_ix == 1) {
            return priceOne;
        } else if (_ix == 2) {
            return priceTwo;
    ...
    function setPriceForScheme(uint8 _ix, uint256 _newPrice)
        public
    ...
    function setGuineaPigErc1155Address(address _guineaPigToken)
        public
        onlyRole(GAME_MANAGER)
    {
        guineaPigToken = IGuineaPigERC1155(_guineaPigToken);
    }
