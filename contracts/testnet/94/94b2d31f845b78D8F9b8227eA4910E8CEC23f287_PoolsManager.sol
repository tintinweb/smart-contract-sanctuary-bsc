...
contract TimeLockPool is ITimeLockPool {
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            ...
            ((((lastTimeRewardApplicable() - lastUpdateTime) / rewardPeriod) *
                rewardRate *
                1e18) / totalFinalSupply);
    }

    function earned(address account) public view returns (uint256) {
        return
            ((balancesFinal[account] *
        ...
        updateReward(address(0))
    {
        if (block.timestamp >= periodFinish) {
            rewardRate = reward / (stakingPoolDuration / rewardPeriod);
        } else {
            uint256 remainingTime = periodFinish - block.timestamp;
            uint256 leftover = (remainingTime * rewardRate) / rewardPeriod;
            rewardRate = (reward + leftover) / (stakingPoolDuration / rewardPeriod);
        }
    ...
    function getTotalRewards(address account) external view returns (LockInfo[] memory) {
        LockInfo[] memory info = new LockInfo[](locks[account].length);

        for (uint256 i = 0; i < locks[account].length; i++) {
            Lock memory lock = locks[account][i];

            if (block.timestamp >= lock.end) {
                info[i] = LockInfo({lockId: i, amount: lock.amount, available: true});
            } else {
                info[i] = LockInfo({lockId: i, amount: lock.amount, available: false});
            }
        }
    ...
    function getLocksOf(address account) external view returns (Lock[] memory) {
        return locks[account];
    }

    function getLocksLengthOf(address account) external view returns (uint256) {
        return locks[account].length;
    }

    function claimRewardFromLock(uint256 lockId) public {
        require(locks[msg.sender].length > lockId, 'Lock doesnt exist');
        Lock memory lock = locks[msg.sender][lockId];

        require(block.timestamp >= lock.end, 'Too soon');

        locks[msg.sender][lockId] = locks[msg.sender][locks[msg.sender].length - 1];
    ...
    function createLock(address account, uint256 _amount) internal {
        locks[account].push(
            Lock({
                amount: _amount,
                start: block.timestamp,
