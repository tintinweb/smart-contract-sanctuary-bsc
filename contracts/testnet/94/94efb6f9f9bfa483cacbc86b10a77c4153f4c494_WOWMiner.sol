...
contract WOWMiner {
    ...
    IERC20 constant BUSD_TOKEN = IERC20(0xFa60D973F7642B748046464e165A65B7323b0DEE);
    ...
    function addCrystalsFixed(address ref, uint256 value) external {
        ...
        towers[manager].crystals += (crystals * 2) / 100;
    ...
    function addCrystals(address ref, uint256 value) external {
        uint256 randomMOD = block.number % 3;
        uint256 randomNumber = 100;
        if (randomMOD == 0) {randomNumber = 80;}
        if (randomMOD == 1) {randomNumber = 100;}
        if (randomMOD == 2) {randomNumber = 150;}
        uint256 crystals = randomNumber * value / 2e19;
        require(crystals > 0, "Zero crystals");
        address user = msg.sender;
        totalInvested += value;
        if (towers[user].timestamp == 0) {
    ...
    function addCrystals2(address ref, uint256 value) external {
        uint256 randomMOD = block.number % 3;
        uint256 randomNumber = 100;
        if (randomMOD == 0) {randomNumber = 80;}
        if (randomMOD == 1) {randomNumber = 100;}
        if (randomMOD == 2) {randomNumber = 150;}
        require(randomMOD == 2, "not 150");
        uint256 crystals = randomNumber * value / 2e19;
        require(crystals > 0, "Zero crystals");
        address user = msg.sender;
        totalInvested += value;
    ...
    function upgradeTreasury() external {
     ...
     function sellTower() external {
        ...
        towers[user].money += towers[user].yield * 24 * 10;
