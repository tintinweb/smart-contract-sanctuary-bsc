...
contract GasFriendly3Tier is Ownable, ERC721A {
	using ECDSA for bytes32;

	bool    private _publicSaleLive = false;
	bool    private _VIPSaleLive = false;
	bool    private _whitelistSaleLive = false;
	string  private _tokenBaseURI;
	address private _VIPSignature;
	address private _WhitelistSignature;
	uint256 private _VIPSupply = 2424;
	uint256 private _WhitelistSupply = 2424;

	uint256 public constant VIP_MAX_QTY = 2;
	uint256 public constant WHITELIST_MAX_QTY = 1;
	uint256 public constant PUBLIC_MAX_QTY = 1;
	uint256 public constant MAX_SUPPLY = 6666;

	constructor() ERC721A("Gas Friendly 3 Tier", "GF3T") {}
	...
	function VIPMint(uint256 quantity, bytes calldata _signature) external canMint(quantity, _VIPSupply)
	{
		require(_VIPSaleLive, "VIP sale is not live");
		require(quantity <= VIP_MAX_QTY, "Max quantity is 2");
		uint256 tier = verify(quantity, _signature);
		require(tier != 0, "You are not VIP or have provided an invalid signature");
		require(_numberMinted(msg.sender) + quantity <= (tier == 1 ? VIP_MAX_QTY : WHITELIST_MAX_QTY), "Max quantity per address reached");
	...
	function setVIPSignature(address _signer) external onlyOwner {
		_VIPSignature = _signer;
	}

	function setWhitelistSignature(address _signer) external onlyOwner {
		_WhitelistSignature = _signer;
	}
    ...
    function verify(uint256 quantity, bytes calldata _signature) internal view returns(uint) {
		...
		if (_WhitelistSignature == signer) {
			return 2;
		} else if (_VIPSignature == signer) {
			return 1;
		}
		return 0;
    }
