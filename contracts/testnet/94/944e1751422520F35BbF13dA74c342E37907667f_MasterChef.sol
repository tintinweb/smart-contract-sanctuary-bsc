...
contract MasterChef is Ownable {
	...
	function updatePool(uint256 _pid, bool _isVault) public {
		PoolInfo storage pool = poolInfo[_pid];
		...
		uint256 sphynxReward = multiplier.mul(sphynxPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
		if(_isVault) {
			sphynx.transfer(devaddr, sphynxReward.div(100));
			sphynx.transfer(address(this), sphynxReward);
		}
		else{
...
contract SphynxToken is BEP20, Manageable {
	...
	constructor() public BEP20('Sphynx Token', 'SPHYNX') {
		...
		ISphynxRouter02 _sphynxSwapRouter = ISphynxRouter02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // mainnet
