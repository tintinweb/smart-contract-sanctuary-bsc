...
contract Updated_final2c is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // View function to see if user can harvest CRANK.
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        require(token.balanceOf(address(msg.sender)) >= 100 * 1000 * 10 ** 18 , 'CRANK token balance is too low');
