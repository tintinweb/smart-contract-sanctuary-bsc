...
library LibOrder {
    ...
    bytes32 private constant ORDER_TYPEHASH = keccak256(
        "Order(address seller,address tokenAddress,uint256 tokenId)"
    );
    ...
    function _genHashKey(Order memory order) internal pure returns(bytes32) {
        bytes32 hashKey = keccak256(
            abi.encode(order.seller, order.tokenAddress, order.tokenId)
        );
        return hashKey;
    }
    ...
    function _genOrderHash(Order memory order) internal pure returns(bytes32) {
        return keccak256(
            abi.encode(ORDER_TYPEHASH, order.seller, order.tokenAddress, order.tokenId)
        );
    }
}
...
contract Exchange is Initializable, UUPSUpgradeable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function createOrder(LibOrder.Order memory order, bytes calldata signature) external {
