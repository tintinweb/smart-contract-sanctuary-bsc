...
contract Pool is Ownable, Pausable, ReentrancyGuard{
    ...
    event Withdraw(address indexed token, address to, uint256 amount);
    ...
    function _AddPackage(uint8 _num,uint256 _avaliableNum, uint256 _payoutAmount, uint256 _endTime) private {
        Packages[_num] = PackageDetails(_avaliableNum,_payoutAmount,_endTime,true);
        emit NewPackage(_num,_avaliableNum,_payoutAmount,_endTime,true);
    }

    function AddMultiPackage(uint8[] calldata _num, uint256[] calldata _avaliableNum, uint256[] calldata _payoutAmount, uint256[] calldata _endTime) external onlyAdminOrOperator {
        require((_num.length == _avaliableNum.length) && (_num.length == _payoutAmount.length) && (_num.length == _endTime.length), "Invalid parameters Input length");
        for(uint8 i = 0; i < _num.length ; i++ ){
             _AddPackage(_num[i],_avaliableNum[i], _payoutAmount[i],_endTime[i]);
        }   
    }
    ...
    function packageSold(address _token, address _buyer, uint8 _package) external onlyAdminOrOperator {
        ...
        transferToken(_token,_buyer,Packages[_package].PayoutAmount);
    }
