...
// SPDX-License-Identifier: MIT
    ...
    abstract contract ReentrancyGuard {
        ...
        contract BnbYielding is ReentrancyGuard{
            ...
            uint256 public MAX_TaxWITHDRAW_COUNT = 1;
            ...
            function withdraw() external NonBlackListed {
                ...
                if(vipinvestor.vip = true && vipinvestor.TimeForWithdraw < block.timestamp ){
                ...
                if(investor.bonus_deposit = true) {
                    investor.amount_bonus_deposit = DEPOSIT_BONUS_PERCENT/PERCENTS_DIVIDER ;
                ...
                uint256 amount = investor.dividends + (investor.dividends * investor.amount_bonus_deposit) + investor.match_bonus;
                ...
                if (vipinvestor.vip=true && amount > MAX_WITHDRAW) {
                    vipinvestor.statusFeeWithdraw = true;
                } else if (vipinvestor.statusFeeWithdraw = false && amount > MAX_WITHDRAW && !iswhitelist(msg.sender)) {
                    investor.dividends = amount - MAX_WITHDRAW;
                    amount = MAX_WITHDRAW;
                } else if (vipinvestor.statusFeeWithdraw = false && amount == MAX_WITHDRAW && investor.whithdrawCount >= MAX_TaxWITHDRAW_COUNT) {
                    investor.statusTaxWithdraw = true;
                } else if (iswhitelist(msg.sender)) {
                    investor.statusTaxWithdraw = false;
                } else if (vipinvestor.statusFeeWithdraw = true && vipinvestor.FeeWithdraw == 0) {
                    ...
                    amount = amount - amount_taxWithdrawVip;
                } else if (investor.statusTaxWithdraw = true) {
