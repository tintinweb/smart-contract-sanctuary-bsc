...
contract Moc is ERC20Detailed, Ownable {
    ...
    function _addLiquidityForMoc() internal swapping {
        ...
        uint256 initBalance = IERC20(usdtAddress).balanceOf(liqMocReceiver);

        swapTokensForUsdt(address(this), half, liqMocReceiver);

        uint256 newBalance = IERC20(usdtAddress).balanceOf(liqMocReceiver).sub(
            initBalance
        );
    ...
    function _swapBackEai() internal swapping {
        ...
        uint256 initBalance = IERC20(usdtAddress).balanceOf(backEaiReceiver);
        ...
        uint256 newBalance = IERC20(usdtAddress).balanceOf(backEaiReceiver).sub(
            initBalance
        );
    ...
    function shouldSwapBack() internal view returns (bool) {
        return
            _autoSwapBack &&
            msg.sender != mocPair &&
            block.timestamp >= (lastBackTime + addLiquidityInterval);
    }
