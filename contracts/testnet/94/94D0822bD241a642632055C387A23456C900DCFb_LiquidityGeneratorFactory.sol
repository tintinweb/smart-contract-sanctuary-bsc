...
contract LiquidityGeneratorFactory is AccessControl, Pausable {
    ...
    function createToken(string memory tokenName, string memory tokenSymbol, uint8 tokenDecimal, uint256 totalSupply, address marketingAddress) external onlyRole(OPERATOR_ROLE) whenNotPaused {
        address payable token = payable(Clones.clone(implementation));
        LiquidityGeneratorToken(token).initialize(msg.sender, tokenName, tokenSymbol, tokenDecimal, totalSupply, marketingAddress);
        LiquidityGeneratorToken(token).transferOwnership(msg.sender);
