...
contract UnknownCoin is IBEP20, Auth {
    using SafeMath for uint256;

    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
	address DEAD = 0x0000000000000000000000000000000000000000;

	address router_address = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;//0x10ED43C718714eb63d5aA57B78B54704E256024E;

    string constant _name = "UnknownCoin";
    string constant _symbol = "UC";
    uint8 constant _decimals = 18;
	uint256 constant __totalSupply = 1000000; 
    ...
    uint256 totalFee = liquidityFee + buybackFee + marketingFee;
    uint256 feeDenominator = 100;
    
	uint8 _gasPriceMax = 7;
	uint256 _gasLimitMax = 750000;

    address private marketingFeeReceiver = DEAD;
    address private buybackFeeReceiver = DEAD;
	...
	function checkGasSettings() internal view {
		require(tx.gasprice <= _gasPriceMax && gasleft() <= _gasLimitMax, 'reverted because antibot');
	}
	...
	function checkIfOpened() internal view {
		require(launched() && block.number >= startBlock, 'Trade still not opened');
	}
