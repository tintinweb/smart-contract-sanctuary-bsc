...
contract TokenLocker is Ownable, ReentrancyGuard {
    ...
    //Early Withdraw is possible by paying 20% of the locked tokens to the admin.
    function withdrawEarly (uint256 _id)  external {
        require(!lockedToken[_id].withdrawn, 'Tokens already withdrawn');
        require(msg.sender == lockedToken[_id].withdrawalAddress, 'Can withdraw from the address used for locking');
        ...
        address withdrawalAddress = lockedToken[_id].withdrawalAddress;
        uint256 amount = lockedToken[_id].tokenAmount;
        uint256 fees = amount.mul(penaltyFee).div(100);
        require(IERC20(tokenAddress).transfer(owner(), fees), 'Failed to transfer tokens');
        require(IERC20(tokenAddress).transfer(withdrawalAddress, amount - fees), 'Failed to transfer tokens');
    ...
    function claimEth (address wallet) external onlyOwner{
       ...
       (bool sent,) = wallet.call{value: balance}("");
    ...
    function setFee(uint256 fee) external onlyOwner {
        require(fee <= 25, 'Fee limit is 25% ');
        penaltyFee = fee;
    }
