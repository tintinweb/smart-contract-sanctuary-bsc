...
contract Staking is IStaking{
    ...
    uint constant public rewardInterval = 1 minutes;
    ...
    function stake(uint _value,uint256 ID_NFTO,uint256 MPN_) public {
        ...
        require(_value>=Min_Staking);
        require(MITI.balanceOf(sender) >= _value);
        require(MITI.transferFrom(sender, address(this), _value));

        address Adr_NFTO = M.getMPN_NFTO_True(MPN_, ID_NFTO);
        require(ID_NFTO==M.getMining_take_NFTO(Adr_NFTO));

        stakes[MPN_][sender] += _value;
        ...
        TotalStakeNFTO[MPN_][ID_NFTO]+= _value;
        MPNtotalStaked[MPN_]+= _value;
    ...
    function updateStake(uint _value,uint256 MPN_) public {
        ...
        uint256 ID_NFTO = Address_S_ID_NFTO[MPN_][sender];
        require(locked[MPN_][msg.sender]);
        require(MITI.transferFrom(msg.sender, address(this), _value));
        stakes[MPN_][msg.sender] += _value;
        totalStaked += _value;
        TotalStakeNFTO[MPN_][ID_NFTO]+= _value;
        MPNtotalStaked[MPN_]+= _value;
    }
