...
contract LockerFactory is Initializable {
    ...
    mapping(address => Locker) public lockersByAddress;
    ...
    function lock(Locker.SLockDescriptor calldata lockDescriptor) external {
        Locker locker = new Locker(lockDescriptor);
        ...
        if (factory == address(0)) {
            tokens.push(lockDescriptor.tokenAddress);
        } else {
            lpTokens.push(lockDescriptor.tokenAddress);
        }
        ...
        returns (AddressInfo memory)
    {
        ...
        if (factory != address(0)) {
            ...
            return
                AddressInfo({
                    ...
                    balance: IUniswapV2Pair(token).balanceOf(msg.sender)
                });
        }
        ...
        return
            AddressInfo({
                ...
                balance: IERC20Metadata(token).balanceOf(msg.sender)
            });
    }
    ...
    function getToken(address tokenAddress)
        public
        view
        returns (Token memory, Locker[] memory)
    {
        ...
        for (uint256 i = 0; i < lockerAddresses.length; ++i) {
            totalAmountLocked += lockerAddresses[0].getLocker().amount;
            currentAmountLocked +=
                lockerAddresses[0].getLocker().amount -
                lockerAddresses[0].getLocker().releasedAmount;
        }
    ...
    function getTokens(
        ...
        Token[] memory returnToken;
        address[] memory ntokens = lp ? lpTokens : tokens;

        if (token != address(0)) {

            if (lockersByToken[token].length == 0) return returnToken;

            (Token memory tokenInfo, ) = getToken(token);

            returnToken = new Token[](1);
            returnToken[0] = tokenInfo;
    ...
    function getLocker(address lockerAddress)
        ...
        returns (Token memory, Locker[] memory)
    {
        Locker locker = lockersByAddress[lockerAddress];
        require(address(locker) != address(0), "locker not found");

        return getToken(locker.getLocker().tokenAddress);
    }
...
contract Locker {
    ...
    uint16 public constant percentagePrecision = 10000;
    ...
    constructor(SLockDescriptor memory lockDescriptor) {
        ...
        uint256 actualAmount = safeTransfer(amount);
        ...
        if (vest) {
            ...
            require(
                vestPercentage > 0 && vestPercentage < percentagePrecision / 2,
        ...
        uint256 vestFullyUnlocksAt = vest
            ? unlockAt +
                ((vestPeriod *
                    ...
                    actualAmount *
                    vestPercentage) / percentagePrecision)
            : 0;
    ...
    function getInfo() external view returns (SLocker memory) {
        SLocker memory info = locker;

        if (locker.vest) {
            info.vestUnlockedPercentage = uint16(
                (block.timestamp - locker.unlockAt) * percentagePrecision / getDuration()
            );
        }
    ...
    function rename(string calldata name) external onlyOwner {
        string memory previousName = locker.name;
        locker.name = name;
        emit ChangedName(previousName, name);
    }

    function extendDuration(uint256 duration) external onlyOwner {
        require(
    ...
    function extendAmount(uint256 amount) external onlyOwner {
        require(
            locker.unlockAt > block.timestamp,
        ...
        uint256 actualAmount = safeTransfer(amount - previousAmount);

        require(
            actualAmount > previousAmount,
            "cannot extend duration once unlocked"
        );
    ...
    function safeTransfer(uint256 amount) public returns(uint256) {
        uint256 recipientBalanceBefore = IERC20Metadata(locker.tokenAddress)
            .balanceOf(address(this));

        IERC20Metadata(locker.tokenAddress).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );

        uint256 recipientBalance = IERC20Metadata(locker.tokenAddress)
            .balanceOf(address(this));
    ...
    function getClaimableAmount() external view returns (uint256) {
        return locker.amount - getUnlockedAmount();
    }

    function getUnlockedAmount() private view returns (uint256) {
        if (block.timestamp < locker.unlockAt) return 0;

        if (block.timestamp >= locker.vestFullyUnlocksAt) return locker.amount;

        return
            (locker.amount * (block.timestamp - locker.unlockAt)) /
            getDuration();
    }
    ...
    function claim() external onlyOwner {
        ...
        uint256 claimableAmount = locker.amount - unlockedAmount;

        require(claimableAmount > 0, "there are no claimable tokens");

        locker.releasedAmount += claimableAmount;

        if (locker.vest) {
            locker.vestClaimedPeriods =
                getDuration() /
                (locker.vestPeriod * getVestUnitPeriod(locker.vestUnit));

            locker.vestClaimedPercentage = uint16(
                unlockedAmount * percentagePrecision / locker.amount
            );

            locker.claimed = locker.vestClaimedPercentage >= percentagePrecision;
        } else {
            locker.claimed = true;
        }

        IERC20Metadata(locker.tokenAddress).safeTransfer(
            locker.owner,
            claimableAmount
        );
