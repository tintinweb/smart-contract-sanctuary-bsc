...
contract GovernorAlpha {
    ...
    string public constant name = "Chee Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public pure returns (uint) { return 600000e18; } // 600,000 = 2% of CHEE

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint) { return 300000e18; } // 300,000 = 1% of CHEE
    ...
    /// @notice The address of the Chee Protocol Timelock
    TimelockInterface public timelock;

    /// @notice The address of the Chee governance token
    CheeInterface public chee;
    ...
    constructor(address timelock_, address chee_, address guardian_) public {
        timelock = TimelockInterface(timelock_);
        chee = CheeInterface(chee_);
        guardian = guardian_;
    }

    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(chee.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || chee.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        uint96 votes = chee.getPriorVotes(voter, proposal.startBlock);
...
// File: contracts/Lens/CheeLens.sol
