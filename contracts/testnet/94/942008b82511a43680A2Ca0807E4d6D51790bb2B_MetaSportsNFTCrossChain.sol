...
contract MetaSportsNFTCrossChain is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    * @notice transfer into token
    ...
    function addCollection(
        ...
        emit CollectionNew(_collection, _creator, _whitelistChecker);
    }
    ...
    function modifyCollection(
        ...
        emit CollectionUpdate(_collection, _creator, _whitelistChecker);
    }
     ...
     */
    function recoverFungibleTokens(address _tokenAddress) external onlyOwner {
        uint256 amountToRecover = IERC20(_tokenAddress).balanceOf(address(this));
        require(amountToRecover != 0, "Operations: No token to recover");

        IERC20(_tokenAddress).safeTransfer(address(msg.sender), amountToRecover);

        emit TokenRecovery(_tokenAddress, amountToRecover);
    }
    ...
    function recoverNonFungibleToken(address _collection, uint256 _tokenId) external onlyOwner nonReentrant {
        require(!_ccTokenIds[_collection].contains(_tokenId), "Operations: NFT not recoverable");
        IERC721(_collection).safeTransferFrom(address(this), address(msg.sender), _tokenId);
