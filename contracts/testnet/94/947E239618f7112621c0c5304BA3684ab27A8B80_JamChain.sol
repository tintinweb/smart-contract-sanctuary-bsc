...
contract JamChain {
    ...
    address payable public insuranceAddr;
    ...
    constructor(uint256 _startTime, address payable _dev, address payable _insuranceAddr,address payable _treasuryAddr) public {
        owner = msg.sender;
        dev = _dev;
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        uint256 _devFee = _amount.div(20); //5%
        dev.transfer(_devFee);
        insuranceAddr.transfer(_devFee.div(2)); // 2.5%
        treasuryAddr.transfer(_devFee.div(2)); // 2.5%
    }
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = (users[_addr].deposit_amount * ((block.timestamp - users[_addr].deposit_time) / 1 days) / 100) * 3000/1000 - users[_addr].deposit_payouts;
