...
contract MiranaOrder is MiranaOrderBase, RaribleTransferManager {
  function initialize(
...
abstract contract MiranaOrderBase is
  Initializable,
  OwnableUpgradeable,
  AssetMatcher,
  TransferExecutor,
  ...
  ITransferManager
{
  ...
  function parseData(LibOrder.Order memory orderLeft)
    public
    pure
    returns (bytes4 dataType, bytes4 dataG)
  {
    dataType = orderLeft.dataType;
    dataG = LibOrderDataV1.V1;
  }

  function decodeparseData(LibOrder.Order memory orderLeft)
    public
    pure
    returns (LibOrderDataV1.DataV1 memory orderData)
  {
    orderData = abi.decode(orderLeft.data, (LibOrderDataV1.DataV1));
  }
  ...
  EIP712Upgradeable
{
  ...
  function __OrderValidator_init_unchained() internal initializer {
    __EIP712_init_unchained("MiranaOrder", "1");
  }
