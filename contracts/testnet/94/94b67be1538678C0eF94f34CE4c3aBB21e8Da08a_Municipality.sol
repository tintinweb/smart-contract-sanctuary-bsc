    ...
    function attachDetachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) external notZeroAddress {
        ...
        for (uint256 index; index < newMiners.length; index++) {
            ...
            if (!_isMinerInList(tokenId, oldMiners)) {
                ...
                uint256 endDate = IMining(miningAddress).getMinerElectricityEndDate(tokenId);
                if(endDate >= block.timestamp) {
                    require(userToElectricityVoucherAmountMapping[msg.sender] > 0, "Municipality::attachDetachMinersToParcel - Not enough vouchers to automatically apply on a miner");
                    userToElectricityVoucherAmountMapping[msg.sender]--;
                    IMining(miningAddress).applyVoucher(tokenId, 1);
                }
            }
        }
...
interface IMinerNFT {
    ...
    function changeTokenLocking(uint256 _tokenId, bool _lock) external;
}
