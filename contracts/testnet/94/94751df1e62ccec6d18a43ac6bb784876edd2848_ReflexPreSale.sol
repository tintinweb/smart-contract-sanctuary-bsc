...
contract ReflexPreSale is Crowdsale, Pausable, FinalizableCrowdsale {
    ...
    constructor(
      ...
      uint256 _feeInBnb,         // if round ends while add liquidity it will take this much percentage of bnb to owner wallet
    ...
    function finalization() virtual internal override{
      require(isListedOnDex, "Finalization: Not yet listed on dex"); 
  ...
  function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
    // add the liquidity
    token.approve(address(uniswapV2Pair), tokenAmount);
  ...
  function withdrawBNBFromContract(uint256 amount) external onlyOwner whenNotPaused {
    require(isListedOnDex, "Withdraw BNB: Not yet listed on dex");   
