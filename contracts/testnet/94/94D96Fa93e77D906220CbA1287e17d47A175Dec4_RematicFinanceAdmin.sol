...
contract RematicFinanceAdmin is UUPSUpgradeable, OwnableUpgradeable {
    ...
    uint256 public liquidityFeeRate1;
    uint256 public liquidityFeeRate2;
    ...
    address public rematicFinanceAddress;
    ...
    modifier onlyRematicFinace() {
        require(rematicFinanceAddress == address(msg.sender), "Message sender needs to be RematicFinance Contract");
        _;
    }
    ...
    function initialize(
        ...
        address _pancakeSwapPair = IPancakeFactory(_pancakeswapV2Router.factory()).createPair(address(this), _pancakeswapV2Router.WETH());

        pancakeSwapV2Router = _pancakeswapV2Router;
        pancakeSwapPair = _pancakeSwapPair;
        REWARD = _REWARD;

        liquidityFeeRate1 = 600; // out of 10000 : 6%
        liquidityFeeRate2 = 600;// out of 10000 : 6%
        pensionFeeRate = 200; // out of 10000 : 2%
        legalFeeRate = 1600; // out of 10000 : 16%
        teamFeeRate = 1600;// out of 10000 : 16%
        holdersSdtFeeRate = 2000; // out of 10000 : 20%
    ...
    // function startLiquidate(address tokenAddress) public onlyRematicFinace {
    function startLiquidate() public onlyTeamWallet {
        address tokenAddress = rematicFinanceAddress;
        uint256 activeLiquidateAmount = IERC20(tokenAddress).balanceOf(address(this));
        uint256 totalFee = liquidityFeeRate1 + liquidityFeeRate2 + pensionFeeRate + legalFeeRate + teamFeeRate + holdersSdtFeeRate + holdersPrtnFeeRate + four01FeeRate;
        uint256 liquidityRFTX = activeLiquidateAmount * liquidityFeeRate2 / totalFee;
        //swapTokensForEth
        uint256 rewardBNB = swapTokensForEth(tokenAddress, (activeLiquidateAmount - liquidityRFTX));
        console.log("rewardBNB ==> %s", rewardBNB);

        totalFee = totalFee - liquidityFeeRate1;

        //add liquidity
        uint256 liquidityBNB = rewardBNB * liquidityFeeRate2 / totalFee;
        addLiquidity(tokenAddress, liquidityRFTX, liquidityBNB);

        rewardBNB = rewardBNB - liquidityBNB;
        totalFee = totalFee - liquidityFeeRate2;

        //send some rewardBNB to Pension wallet
        uint256 pAmount = rewardBNB * pensionFeeRate / totalFee;
        _sendBNBToPensionWallet(pAmount);

        //send some rewardBNB to Team wallet
        uint256 tAmount = rewardBNB * teamFeeRate / totalFee;
        _sendBNBToTeamWallet(tAmount);

        uint256 lAmount = rewardBNB * legalFeeRate / totalFee;
        //send some rewardBNB to LegalWallet
        _sendBNBToLegalWallet(lAmount);

        //distrubute on Default Token Tracker (Holders Standard in diagram)
        uint256 hsAmount = rewardBNB * holdersSdtFeeRate / totalFee;
        _distributeRewardDividends(hsAmount);

        //send some rewardBNB to Holders partition
        uint256 hpAmount = rewardBNB * holdersPrtnFeeRate / totalFee;
        _sendBNBToHoldersPrtn(hpAmount);

        //send some rewardBNB to 401 program
        uint256 f01Amount = rewardBNB * four01FeeRate / totalFee;
        _sendBNBToFour01TeamWallet(f01Amount);
        

    }

    function swapTokensForREWARD(address tokenAddress, uint256 _amountIn) private returns( uint256 ){
        ...
        IERC20(tokenAddress).approve(pancakeSwapRouter02Address, _amountIn);

        uint256 initialBUSD = IERC20(path[1]).balanceOf(address(this));
        ...
        // after swaping
        uint256 newBUSD = IERC20(path[1]).balanceOf(address(this));
    ...
    function setLiquidityFeeRate(uint256 _liquidityFeeRate1, uint256 _liquidityFeeRate2) public onlyOwner {
        require(liquidityFeeRate1 != _liquidityFeeRate1, "already set value.");
        liquidityFeeRate1 = _liquidityFeeRate1;
        require(liquidityFeeRate2 != _liquidityFeeRate2, "already set value.");
        liquidityFeeRate2 = _liquidityFeeRate2;
    }
    ...
    function _sendBNBToPensionWallet(uint256 amount) internal {
        if(amount > 0 && isOnPensionFee ){
            console.log("Pensionwallet : %s", amount);
    ...
    function _sendBNBToLegalWallet(uint256 amount) internal {
        if(amount > 0 && isOnLegalFee){
            console.log("Legalwallet : %s", amount);
    ...
    function _sendBNBToTeamWallet(uint256 amount) internal {
        if(amount > 0 && isOnTeamFee){
            console.log("TeamWallet : %s", amount);
    ...
    function _distributeRewardDividends(uint256 amount) internal {
        ...
        if(amount > 0){
            (bool success, ) = address(defaultTokenTracker).call{value: amount}(new bytes(0));
            if(success){
                IDefaultTracker(defaultTokenTracker).distributeRewardDividends(amount);
            }
            try IDefaultTracker(defaultTokenTracker).process() returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
    ...
    function setBalanceForFrom(address payable account, uint256 newBalance, uint256 _txAmount) external onlyRematicFinace  {
        ...
        if(account != pancakeSwapPair){
            ...
            IHoldersPartition(holdersPrtn).recordTransactionHistory(account, _txAmount, true);
        }
        // uint256 numberofholders = IDefaultTracker(defaultTokenTracker).getNumberOfTokenHolders();
        // console.log("numberofholders ===> %s", numberofholders);
    }

    function setBalanceForTo(address payable account, uint256 newBalance, uint256 _txAmount) external onlyRematicFinace  {

        if(account != pancakeSwapPair){
            // buying
            // console.log("--------buying--------");
            IDefaultTracker(defaultTokenTracker).setBalance(account, newBalance);
            IHoldersPartition(holdersPrtn).recordTransactionHistory(account, _txAmount, false);
        }
    ...
    function setRematicFinance(address _address) public onlyOwner {
        require(_address != address(rematicFinanceAddress), "RFTX Admin: The rematicFinanceAddress already has that address");
        rematicFinanceAddress = _address;
    }

    function _sendBNBToHoldersPrtn(uint256 amount) internal {
        if(amount > 0){
            (bool success, ) = address(holdersPrtn).call{value: amount}(new bytes(0));
            if(success){
    ...
    function _sendBNBToFour01TeamWallet(uint256 amount) internal {
        if(amount > 0 && isOn401kFee){
            console.log("401TeamWallet : %s", amount);
            (bool success, ) = address(four01TeamWalletAddress).call{value: amount}(new bytes(0));
            if(!success){
    ...
    function excludeAddressFromHolders() public onlyOwner {
        IDefaultTracker(defaultTokenTracker).excludeFromDividends(defaultTokenTracker);
        IDefaultTracker(defaultTokenTracker).excludeFromDividends(address(this));
        IDefaultTracker(defaultTokenTracker).excludeFromDividends(owner());
        IDefaultTracker(defaultTokenTracker).excludeFromDividends(pancakeSwapRouter02Address);
        IDefaultTracker(defaultTokenTracker).excludeFromDividends(pancakeSwapPair);
        address burnWallet = IRematicFinance(rematicFinanceAddress).burnWallet();
        IDefaultTracker(defaultTokenTracker).excludeFromDividends(burnWallet);
    }

    function setBurnWallet(address _address) public onlyOwner {
        // burnWallet = _address;
        IRematicFinance(rematicFinanceAddress).setBurnWallet(_address);
    }
    function setStakingWallet(address _address) public onlyOwner {
        IRematicFinance(rematicFinanceAddress).setStakingWallet(_address);
    }
    function setTxFeeRate(uint256 _newValue) public onlyOwner {
        IRematicFinance(rematicFinanceAddress).setTxFeeRate(_newValue);
    }
    function setBurnFeeRate(uint256 _newValue) public onlyOwner {
        IRematicFinance(rematicFinanceAddress).setBurnFeeRate(_newValue);
    }
    function setStakingFeeRate(uint256 _newValue) public onlyOwner {
        IRematicFinance(rematicFinanceAddress).setStakingFeeRate(_newValue);
    }

    function setLiquidityFeePercentage(uint256 _newPercentage) public onlyOwner {
        liquidityFeeRate1 = _newPercentage / 2;
        liquidityFeeRate2 = _newPercentage - _newPercentage / 2;
    }

    function setPensionFeePercentage(uint256 _newPercentage) public onlyOwner {
        pensionFeeRate = _newPercentage;
    }

    function setLegalFeePercentage(uint256 _newPercentage) public onlyOwner {
        legalFeeRate = _newPercentage;
    }

    function setTeamFeePercentage(uint256 _newPercentage) public onlyOwner {
        teamFeeRate = _newPercentage;
    }

    function setHoldersSdtFeePercentage(uint256 _newPercentage) public onlyOwner {
        holdersSdtFeeRate = _newPercentage;
    }

    function setHoldersPrtnFeePercentage(uint256 _newPercentage) public onlyOwner {
        holdersPrtnFeeRate = _newPercentage;
    }

    function set401kFeePercentage(uint256 _newPercentage) public onlyOwner {
        four01FeeRate = _newPercentage;
    }
    ...
    function withraw401KReward(address _account) public onlyOwner {
        IFour01Programe(four01program).withraw401KReward(_account);
    }
    ...
    function setIsOnBurnFeeForRematicFinance(bool flag) public onlyOwner {
        IRematicFinance(rematicFinanceAddress).setIsOnBurnFee(flag);
    }

    function setIsOnStakingFeeForRematicFinance(bool flag) public onlyOwner {
        IRematicFinance(rematicFinanceAddress).setIsOnStakingFee(flag);
    }

    //--------development mode for testig purpose------------
    // function sellTokensForETH(uint256 tokenAmount) public {
    //     address[] memory path = new address[](2);
    //     require(path.length <= 2, "fail");
    //     path[0] = rematicFinanceAddress;
    //     path[1] = pancakeSwapV2Router.WETH();
    ...
    //     );
    // }
    //----------end of development mode for testig purpose------------
}
...
interface IRematicFinance is IERC20 {
