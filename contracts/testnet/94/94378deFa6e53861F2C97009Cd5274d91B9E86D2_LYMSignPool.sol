...
contract LYMSignPool is OwnableUpgradeable, DivestorUpgradeable {
    ...
    function claimDynamicReward() public {
        ...
        if (reward > userInfo[msg.sender].power) {
            uint256 amount = (userInfo[msg.sender].power * nodeInfo[userInfo[msg.sender].node].rate) / 100;
            addr.usdt.transferFrom(msg.sender, address(this), amount);
        }
