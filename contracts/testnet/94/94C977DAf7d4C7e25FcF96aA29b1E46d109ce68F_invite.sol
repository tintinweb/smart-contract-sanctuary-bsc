  ...
  /*重启数据*/
   ...
   /*初始化*/
    ...
    function _UpdateValidInvite(InviteData storage _data , address _address) private {
        ...
        if(_data.school.listSize >= top1){
            oldTop1Address = _data.school.getRankBefore(top1);
        }
        if(_data.school.listSize >= maxRank){
    ...
    function UpdateEarning(address _address, uint8 _weight) private {
        ...
        if(roundId != userReward[_address].markerRound){
            ...
            if(data.school.listSize < maxRank){
                userReward[_address].markerUsdt = data.resD.resBalanceUsdt;
                userReward[_address].makerErc20 = data.resD.resBalanceErc20;
            }else{
    ...
    function Earning() payable public {
        ...
        if(inData[roundId].school.listSize >= maxRank)
        {
            ...
            userReward[msg.sender].markerUsdt = inData[roundId].resD.resBalanceUsdt; 
            userReward[msg.sender].makerErc20 = inData[roundId].resD.resBalanceErc20;        
        }   
    ...
    function _countAddV (address _address, uint256 _balanceU, uint256 _balanceE) private view returns (uint256, uint256){
        ...
        if(roundId != accdata.markerRound){
            if(accdata.weight != 0){
                if(data.resD.resBalanceUsdt > 0 && data.resD.allWeight > 0){
                    countUsdt = data.resD.resBalanceUsdt
                        .sub(accdata.markerUsdt)
                        .div(data.resD.allWeight)
                        .mul(accdata.weight);
                }
                if(data.resD.resBalanceErc20 > 0 && data.resD.allWeight > 0){
                    countErc20 = data.resD.resBalanceErc20
    ...
    function Restart(uint256 _round) public OnlyPledged{
        require(inData[_round].resD.init != true,"round already restart");
        
        InviteData storage res = inData[roundId];
        /*更新上一轮数据*/
        res.resD.resBalanceUsdt = _getBalance(usdtAddress);
        res.resD.resBalanceErc20 = _getBalance(xtokenAddress);
        if(res.school.listSize < maxRank){
            res.resD.allWeight = res.school.listSize;
        }
        else{
            res.resD.allWeight = MUSTSUREWEIGHT;
        }
        /*初始化新一轮数据*/
        _Restart(_round, res.resD.resBalanceUsdt, res.resD.resBalanceErc20);
    }
    ...
    function _Restart(uint256 _round,uint256 _startBalanceUsdt,uint256 _startBalanceErc20) private {
        ...
        /*写入新一轮数据*/
        roundId = _round;
        inData[roundId].resD.init = true;
        inData[roundId].resD.startBalanceUsdt = _startBalanceUsdt;
        inData[roundId].resD.startBalanceErc20 = _startBalanceErc20;
