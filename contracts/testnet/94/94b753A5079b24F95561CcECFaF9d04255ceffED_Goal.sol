...
library EnumerableSet {
  ...
  function insert(AddressSet storage set_, uint256 index_, address valueToInsert_ ) internal returns ( bool ) {
    return _insert( set_._inner, index_, bytes32(uint256(uint160(valueToInsert_))) );
  }
...
contract VaultOwned is Ownable {
  ...
  function renounceVault() public onlyOwner() {
    _vault = address(0);
  }

  function transferVault( address newVault_ ) public onlyOwner() {
    require( newVault_ != address(0), "Vault: new vault is the zero address");
    _vault = newVault_;
  }
}

interface IHayyaRouter{
    function referParent(address parentAddress, address children) external;
}


contract Goal is ERC20Permit, VaultOwned {
    using SafeMath for uint256;
    constructor() ERC20("Hayya Goal", "$GOAL", 18) {}

    address public tokenHYA;
    address public hayyaRouter;

    function setTokenHYA(address _tokenHYA) external onlyOwner{
        tokenHYA = _tokenHYA;
    }

    function setHayyaRouter(address _hayyaRouter)external onlyOwner{
        hayyaRouter = _hayyaRouter;
    }
    ...
    function _transfer(
      ...
      if(balanceOf(recipient) == 0 && IERC20(tokenHYA).balanceOf(recipient) == 0 && amount > 1){
        IHayyaRouter(hayyaRouter).referParent(sender, recipient);
      }
