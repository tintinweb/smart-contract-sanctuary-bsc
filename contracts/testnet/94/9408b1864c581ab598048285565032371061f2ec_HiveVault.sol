...
contract HiveVault is ERC20, Ownable {
    uint256 public  stakingFeeOnBuy  = 0;
    uint256 public  stakingFeeOnSell = 4;
    ...
    address public stakingWallet = 0xF963f145cd74C4c63EbD6AB4867aA846D99e8f66;
    ...
    event UpdateBuyFees(uint256 stakingFeeOnBuy, uint256 marketingFeeOnBuy);
    event UpdateSellFees(uint256 stakingFeeOnSell, uint256 marketingFeeOnSell);
    ...
    constructor () ERC20("Hive Vault", "HIV") 
    {   
        safuOperator = msg.sender;
        transferOwnership(0xb31d2B1Cb48569a549CF65200292EFA8499C95ba);
    ...
    function updateBuyFees(uint256 _stakingFeeOnBuy, uint256 _marketingFeeOnBuy) external onlyOwner {
        require(
            _stakingFeeOnBuy + _marketingFeeOnBuy <= 5,
            "Buy fees must be 5% at max"
        );
        stakingFeeOnBuy = _stakingFeeOnBuy;
        marketingFeeOnBuy = _marketingFeeOnBuy;

        _totalFeesOnBuy   = stakingFeeOnBuy + marketingFeeOnBuy;
        emit UpdateBuyFees(_stakingFeeOnBuy, _marketingFeeOnBuy);
    }

    function updateSellFees(uint256 _stakingFeeOnSell, uint256 _marketingFeeOnSell) external onlyOwner {
        require(
            _stakingFeeOnSell + _marketingFeeOnSell <= 12,
            "Sell fees must be 12% at max"
        );
        stakingFeeOnSell = _stakingFeeOnSell;
        marketingFeeOnSell = _marketingFeeOnSell;
        _totalFeesOnSell   = stakingFeeOnSell + marketingFeeOnSell;
        emit UpdateSellFees(_stakingFeeOnSell, _marketingFeeOnSell);
    }

    function updateWalletToWalletFee(uint256 _walletToWalletFee) external onlyOwner{
        require(_walletToWalletFee <= 5, "Wallet to wallet fee must be 5% at max");
