...
contract BigexSeedSale is Ownable, Pausable {
	...
	mapping(address => uint256) userTotalPaymentBnb;

	address public otherInvestmentContract;

	bool public activeBuyBNB = true;
	bool public activeBuyToken = false;
	...
	function buyTokenBNB() public payable whenNotPaused {
		...
		require(minBuyBnb <= _paymentAmount && _paymentAmount <= maxBuyBnb, "SeedSale: min max buy is not valid");
		require(totalBuyBnb(msg.sender) + _paymentAmount <= maxBuyBnb, "SeedSale: limit buy token");

		// check balance payment token before buy token
		require(address(msg.sender).balance >= _paymentAmount, "SeedSale: balance not enough");

		uint256 totalToken = _paymentAmount.div(tokenPriceRateBnb) * 10 ** 18;

		// check balance token contract
		require(bigexToken.balanceOf(address(this)) >= totalToken, "SeedSale: contract not enough balance");

		// transfer bnb from user to contract
		payable(address(this)).transfer(_paymentAmount);
		...
		// update total payment amount
		userTotalPaymentBnb[msg.sender] = userTotalPaymentBnb[msg.sender].add(_paymentAmount);
