...
contract WinBigWin is Ownable, ReentrancyGuard {
    ...
    uint256 public totalReferralReward = 0; // total count of referral rewards
    ...
    event NewAgent(address indexed Agent, bool state);

    event RegisterReferral(address indexed referrer, address user);
    event ClaimReferralReward(address indexed referrer, uint256 reward);
   ...
   */
    function registerReferral(address referrer, address user) internal {
        ...
        emit RegisterReferral(referrer, user);
    }
   ...
   */
    ...
    */
    function deductReferralRate(address user, uint256 deduction) internal {
        ...
        totalReferralReward +=  round.totalAmountBet - (round.totalAmountBet - deduction);
    }
    ...
    */
    function claimReferralReward() external nonReentrant {
        ...
        RefInfo storage refI = refInfo[msg.sender];
        uint256 rewardAmount = 0;

        rewardAmount = refI.claimable;
        refI.claimable -= rewardAmount;
        totalReferralReward -= rewardAmount;

        tokenAddress.safeTransfer(address(msg.sender), rewardAmount);
        
        emit ClaimReferralReward(msg.sender, rewardAmount);
    }
