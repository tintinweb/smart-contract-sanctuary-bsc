...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  uint256 public minAmount = 666;
  uint256 public slippage = 990;
  ...
  address[] public tokens = [0xE5e544D5Cabe8Ac280e7365e9ccD4e884c843169, 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3];
  ...
  function setSlippage(uint256 _minAmount, uint256 _slippage) public onlyOwner {
  ...
  function addInitialLiquidity() external onlyOwner {
    ...
    router.addLiquidity(busdAddr, address(this), amount, amount * 1000, amount, amount * 1000, treasury, now + 60);
  }
  
  function getAmounts(address addr)  public view returns (uint256, uint256, uint256, uint256) {
    require(isValid(addr));
    IBEP20 STABLE = IBEP20(addr);
    uint256 amount = STABLE.balanceOf(address(this));
    (uint256 r0 , uint256 r1 ) = getReserves(addr);
    uint256 amount2 = amount * r1 / r0;
    uint256 ratio = 1000000 * r0 / r1;
    return (amount2, r0, r1, ratio);
  }
  
  function addLiquidity(uint256 index) public {
    address addr = tokens[index];
    require(isValid(addr));
    _addLiquidity(addr, minAmount, 1000);
  }

  function addLiquidity2(address addr, uint256 min, uint256 max) public onlyOwner {
    _addLiquidity(addr, min, max);
  }
  ...
  function _addLiquidity(address addr, uint256 min, uint256 max) internal {
    (uint256 r0 , uint256 r1 ) = getReserves(addr);
    require(r0 > 0 && r1 > 0);
    uint256 ratio = 1000000 * r0 / r1;
    if(ratio >= min && ratio <= max) {
      ...
      require(amountA > 0);
      uint256 amountB = amountA * r1 / r0;
      require(amountB > amountA);
      _mint(address(this), amountB);
      router.addLiquidity(addr, address(this), amountA, amountB, amountA * slippage / 1000 , amountB * slippage / 1000 , treasury, now + 60);
    } 
  }
  ...
  function mint(uint256 _amount, uint256 index) public {
    address addr = tokens[index];
    require(isValid(addr));
    require(_amount > 0);
    IBEP20 STABLE = IBEP20(addr);
