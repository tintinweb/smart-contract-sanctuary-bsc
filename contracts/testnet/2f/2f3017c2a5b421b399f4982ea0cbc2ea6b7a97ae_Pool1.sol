...
contract Pool1 is Ownable {
    ...
    constructor() {
        init(
            address(0x1b3b27e591A60994816B3b1F89f51D01F3E4F233),
            address(0xdAb5B23DC2955Eb3074BC8A0ba2f093A08739a32),
            0
        );
    }
 
    function init(address _token, address _pair, uint256 _r) public onlyOwner {
        token = IToken(_token);
    ...
    mapping(address => address[]) private recommendAddrList; // rec
    mapping(address => bool) private isRegister; // registered
    mapping(address => address) private rootNode; // root address
    mapping(address => address[]) private rootList; // root list
    mapping(address => uint256) private nodeStatus; // node type 0 not， 1 node， 2 real node
    address[] realNodes;
    ...
    function setNode(address account, uint256 status) public onlyOwner {
        nodeStatus[account] = status;
    }
    ...
    function getRootList(address account) external view returns(address[] memory) {
        return rootList[account];
    }
    ...
    function getNodeStatus(address account) external view returns (uint256) {
        return nodeStatus[account];
    }

    function getRealNodesLength() external view returns(uint256) {
        return realNodes.length;
    }

    function getRealNodesList(uint256 start, uint256 limit) external view returns(address[] memory) {
        uint256 length = realNodes.length;
        require(start <= length - 1 && start >= 0, "wrong data");
        require(limit >= start && length - start >= limit && limit <= 1000,  "wrong data");
        address[] memory list = new address[](limit);
        for (uint256 i = start; i < limit; i++) {
            list[i-start] = realNodes[i];
        }
        return list;
    }
    ...
    function upgradeToNode() public {
        ...
        (uint256 self_sum, ,) = _calculatedValue(msg.sender, 0);
        require(self_sum >= n_1000, "Must pledge and lock 1000 USDT");
        nodeStatus[msg.sender] = 1;
    ...
    function registerRootNode(address rootAccount) external {
        ...
        require(nodeStatus[rootAccount] > 0, "not a node");
        require(rootAccount != msg.sender, "same address");

        recommendAddrList[rootAccount].push(msg.sender);
    ...
    function calculatedValue(address account) external returns (uint256, uint256, uint256) {
        return _calculatedValue(account, 0);
    }

    // 计算用户锁仓总价值
    function _calculatedValue(address account, uint256 types) internal virtual returns (uint256 amount, uint256 token0Amount, uint256 token1Amount) {
        // 锁仓总LP
        uint256 self_LP = 0;
        if(types == 0)
            self_LP = lockLPSum[account]; 
        else 
            self_LP = normalLockLPSum[account];
    ...
    function _queryRequiredQuantity() internal virtual returns (uint256, bool) {
        ...
        for(uint256 i = 0; i < recommendAddrList[msg.sender].length; i++) {
            address addr = recommendAddrList[msg.sender][i];
            (uint256 s, ,) = _calculatedValue(addr, 0);
    ...
    function upgradeToRealNode() public returns (bool) {
        require(nodeStatus[msg.sender] > 0, "not a node");
        ...
        if(sum >= requiredQuantity && result) {
            nodeStatus[msg.sender] = 2;
            realNodes.push(msg.sender);
    ...
    mapping(address => bool) private isLocked; // 已经参与锁仓
    address[] private lockList; // 锁仓地址列表
    mapping(address => uint256) public userRewardPerTokenPaid; //已经支付
    mapping(address => uint256) public rewards; // 未支付奖励

    uint256[] private lockOrderSTL; // locked startTime list
    uint256[] private lockOrderTTL; // locked total list
    address[] private lockOrderAddr; // locked account list   
    ...
    mapping(address => uint256) public normal_userRewardPerTokenPaid; // 已经支付
    mapping(address => uint256) public normal_rewards; // 未支付奖励
    ...
    event GetNormalReward(address indexed account, uint256 amount);
    ...
    function lockLP(uint256 amount, uint256 types) external {
        require(amount > 0, "amount zero");
        uniswapV2Pair.transferFrom(msg.sender, address(this), amount);

        if (types == 0){
            lockOrderSTL.push(block.timestamp);
            lockOrderTTL.push(amount);
            lockOrderAddr.push(msg.sender);
    ...
    function unlockLP(uint256 startTime, uint256 types) external returns (bool) {
    ...
    function getLockLP(address account, uint256 types) external view returns (uint256[] memory){
        if(types == 0)
    ...
    function getLockLPStartTime(address account, uint256 types) external view returns (uint256[] memory){
        if(types == 0)
    ...
    function getLockLPSum(address account, uint256 types) external view returns (uint256){
        if(types == 0)
    ...
    function getLockList(uint256 types) public view returns (address[] memory) {
        if(types == 0)
            return lockList;
        else
    ...
    function getLockedOrderLength() external view returns(uint256) {
        return lockOrderSTL.length;
    }

    function getlockOrderSTLList(uint256 start, uint256 limit) external view returns(uint256[] memory) {
        uint256 length = lockOrderSTL.length;
        require(start <= length - 1 && start >= 0, "wrong data");
        require(limit >= start && length - start >= limit && limit <= 1000,  "wrong data");
        uint256[] memory list = new uint256[](limit);
        for (uint256 i = start; i < limit; i++) {
            list[i-start] = lockOrderSTL[i];
        }
        return list;
    }
    function getlockOrderTTLList(uint256 start, uint256 limit) external view returns(uint256[] memory) {
        uint256 length = lockOrderTTL.length;
        require(start <= length - 1 && start >= 0, "wrong data");
        require(limit >= start && length - start >= limit && limit <= 1000,  "wrong data");
        uint256[] memory list = new uint256[](limit);
        for (uint256 i = start; i < limit; i++) {
            list[i-start] = lockOrderTTL[i];
        }
        return list;
    }
    function getlockOrderAddrList(uint256 start, uint256 limit) external view returns(address[] memory) {
        uint256 length = lockOrderAddr.length;
        require(start <= length - 1 && start >= 0, "wrong data");
        require(limit >= start && length - start >= limit && limit <= 1000,  "wrong data");
        address[] memory list = new address[](limit);
        for (uint256 i = start; i < limit; i++) {
            list[i-start] = lockOrderAddr[i];
        }
        return list;
    }
    ...
    function lottery() public returns (bool) {
        ...
        for (uint256 i = 0; i < lockList.length; i++) {
            ...
            (uint256 LockValue,,) = _calculatedValue(account, 0);
        ...
        for (uint256 i = 0; i < NormalLockList.length; i++) {
            ...
            (uint256 nomarlLockValue,,) = _calculatedValue(account, 1);
        ...
        for (uint256 i = 0; i < LockUserAccount.length; i++) {
            ...
            rewards[account] = rewards[account].add(val);
        }
        ...
        for (uint256 i = 0; i < NormalLockUserAccount.length; i++) {
            ...
            normal_rewards[account] = normal_rewards[account].add(val);
        }
    ...
    function getReward() public {
        require(rewards[msg.sender] > 0, "Extractable to zero");
        address account = msg.sender;
        uint256 amount = rewards[account];
        
        token.transfer(account, amount);
        userRewardPerTokenPaid[account] = userRewardPerTokenPaid[account].add(amount);
        rewards[account] = 0;

        emit GetReward(account, amount);
    }

    function getNormalReward() public {
        require(normal_rewards[msg.sender] > 0, "Extractable to zero");
        address account = msg.sender;
        uint256 amount = normal_rewards[account];

        token.transfer(account, amount);
        normal_userRewardPerTokenPaid[account] = normal_userRewardPerTokenPaid[account].add(amount);
        normal_rewards[account] = 0;

        emit GetNormalReward(account, amount);
    }
