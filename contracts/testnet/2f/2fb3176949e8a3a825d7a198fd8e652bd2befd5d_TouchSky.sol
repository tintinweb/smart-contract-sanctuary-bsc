...
contract TouchSky {
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		if (user.referrer != address(0)) {
		 ...
		 address upline1 = user.autopoolreferrer;            
            for (uint256 i = 0; i < 10 && upline1!= 0x0000000000000000000000000000000000000000; i++) {
                // check direct child
                if(users[upline1].levels[0] >10){
                    token.transfer(upline1,5e18);
                    users[upline1].autopool.add(5e18);                   
                    break;
                } 
