...
contract SquadGame is SquadGameBasic {
    ...
    function registr(address userAddress, uint8 level) public payable {
        ...
        users[userAddress].activeMainLevels[level] = true;

        RegBuyLevel(userAddress, level);
        
        userIds[lastUserId] = userAddress;
        lastUserId++;        
    }

    function RegBuyLevel(address _userAddress, uint8 level) public payable{

        require(msg.sender==_userAddress,"Wrong sender");
        require(msg.value==levelPrice[level]+RegFee,"Wrong value");

        if (users[_userAddress].smartMatrix[level-1].blocked) {
            users[_userAddress].smartMatrix[level-1].blocked = false;
        }
    ...
    function registrationRef(address userAddress, address referrerAddress, uint8 level) public payable {
        require(!isUserExists(userAddress), "user exists");
    ...
    function paySmartReferrer(address userAddress, uint8 level) public returns (bool) {      
        ...
        {     
            (bool success, ) = (users[userAddress].firstLevelReferrals).call{value:firstPercent}('');      
            return success;
        }

        if(users[userAddress].secondLevelReferrals!=address(0))
        {
            (bool success, ) = (users[userAddress].secondLevelReferrals).call{value:secPercent}('');   
            return success;
        } 

        if(users[userAddress].thirdLevelReferrals!=address(0))          
        {
            (bool success, ) = (users[userAddress].thirdLevelReferrals).call{value:thirdPercent}('');   
            return success;
        }             
    ...
    function sendETHDividends(uint8 level) public returns (bool) {
        bool money_paid=false;
        uint256 i=0;
        while(!money_paid)
        {
            uint n_id = LevelPayments[level].Level_User_id[i];
            ...
            {     
                ...
                money_paid = true;
                return success;
            }else  if(users[prev_user].secPayment==false)
            {
                (bool success, ) = (prev_user).call{value:mainPercent}('');   
    ...
    function AregFee(uint8 level) public view returns (uint256) {
        return levelPrice[level]+RegFee;
    }
