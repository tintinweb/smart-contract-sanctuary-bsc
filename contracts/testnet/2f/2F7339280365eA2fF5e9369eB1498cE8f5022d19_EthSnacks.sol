...
contract EthSnacks is SnacksBase {
    ...
    function _beforeDistributeFee(
        ...
        if (feeEthSnacks_ > 0) {
            // % EthSnacks holders
            uint256 feeToHolders = (feeEthSnacks_ * feeBTCETHSnacksHolders) /
                100;
            ...
            if (totalEthHoldersBalance_ > 0) {
                uint256 holderListLength = _holderList.length();
                for (uint256 i = 0; i < holderListLength; i++) {
            ...
            if (feeSnacksHolders > 0) {
                IERC20(address(this)).transfer(
                    snacks,
                    (feeEthSnacks_ * feeSnacksHolders) / 100
                );
            }
        }
    }
