...
contract InitialNFTOffering is Initializable, ERC721Holder {
    ...
    //level milestone
    uint[] private milestones;

    mapping(uint => uint) private bonusMilestones;
    ...
    //emit Setting(projectId, _update, registerTime, startTime, endTime, admin, currency, totalTypes);
    ...
    event Setting(uint indexed projectId, bool indexed update, uint registerTime, uint startTime, uint endTime, address admin, address currency, uint totalTypes);
    ...
    function setting(
        ...
        emit Setting(projectId, _update, registerTime, startTime, endTime, admin, address(currency), totalTypes);
        return true;
    }
    ...
    function depositNFT(uint _typeId, uint[] memory _ids) external onlyOperator checkTypeId(_typeId){
