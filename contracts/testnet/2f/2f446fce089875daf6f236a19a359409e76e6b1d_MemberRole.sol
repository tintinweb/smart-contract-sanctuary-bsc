...
contract MemberRole {
    ...
    */
    ...
    event TypeChanged(address member, string newType, uint256 timestamp);
    ...
    */
    ...
    function changeRoleType(address _member, uint256 _roleType)
        public
        onlyOwner
    {
        require(_member != address(0), "Pleae provide a valid address.");
        ...
        require(
            keccak256(abi.encodePacked(userRole[_member].roleType)) != keccak256(abi.encodePacked(roleTypes[_roleType])),
            "The member is already in this role type"
        );
        userRole[_member].roleType = roleTypes[_roleType];

        emit TypeChanged(_member, roleTypes[_roleType], block.timestamp);
    }
