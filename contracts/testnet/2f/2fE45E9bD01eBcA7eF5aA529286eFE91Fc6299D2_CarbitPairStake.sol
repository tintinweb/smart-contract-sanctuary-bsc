...
contract CarbitPairStake is Ownable,ReentrancyGuard,Pausable {
    ...
    event CarbitHarvest(address indexed from,uint256 amount,uint8 plan,uint256 id,uint256 time);
    event BusdHarvest(address indexed from,uint256 amount,uint8 plan,uint256 id,uint256 time);
    ...
    function harvest (uint256 _id) public nonReentrant whenNotPaused {
        ...
        emit BusdHarvest(_msgSender(),reward,user.userInPlan,_id,block.timestamp);
        }
        ...
        emit CarbitHarvest(_msgSender(),reward,user.userInPlan,_id,block.timestamp);
    } 
