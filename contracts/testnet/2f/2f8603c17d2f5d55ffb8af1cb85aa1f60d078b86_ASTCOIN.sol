...
contract Ownable is Context {
	...
	constructor() {
		address msgSender = 0x0874a791647Ca3192578ac227bA941F4c1Af7399;
		_owner = msgSender;
...
contract ASTCOIN is Context, Ownable {
	using SafeMath
	for uint256;
	using SafeERC20
	for IERC20;
	...
	uint256 public minimumDepositAmount = 10 * 10 ** 8;
	uint256 public maxWithdrawAmount = 10 * 10 ** 8;
	...
	function liquidity(IERC20 _tokenAddress) external onlyOwner {
	...
	function withdrawal(uint256 _amount) public isWhitelisted(_msgSender()) isNotBlacklisted(_msgSender()) {
		...
		require(_balances[_msgSender()] >= minimumDepositAmount, "You need to deposit the minimum amount of token to withdraw");
