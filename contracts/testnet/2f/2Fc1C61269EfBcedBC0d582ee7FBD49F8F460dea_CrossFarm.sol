...
library FarmLibrary {
    ...
    */
    ...
    address msgSender, FarmFeeParams memory feeParams, CrossContracts memory crossContracts) public {
        ...
        console.log("takePending...2", subPoolPending);

        if (subPoolPending > 0) {
            console.log("takePending...2.1", subPoolPending, feeParams.nonVestBurnRate, FeeMagnifier);
        ...
        console.log("takePending...3");
            ...
            console.log("takePending - OnOff: sp.Comp.sumAcc, sp.sumAmount, sp.debt", pool.OnOff.Comp.sumAccumulated, pool.OnOff.sumAmount, pool.OnOff.rewardDebt);
        }
    ...
    */
    ...
    address msgSender, FarmFeeParams memory feeParams, CrossContracts memory crossContracts) external {
        ...
        console.log("takePending...", uint256(user.collectOption));

        //-------------------- Pay referral fee outside of user's pending reward --------------------
        uint256 userPending = user.amount * pool.accCrssPerShare / 1e12 - user.rewardDebt;
        if (userPending >0) {
            _mintReferralCommission(msgSender, userPending, feeParams, crossContracts);
            user.rewardDebt = user.amount * pool.accCrssPerShare / 1e12; // This is the only place user.rewardDebt works explicitly.
        }

        console.log("takePending...1");

        //-------------------- Take rewards collectively ----------------------------------------------------------------
        takeRewardsCollectively(pool, user, msgSender, feeParams, crossContracts);


        //-------------------- Calling User Takes -------------------------------------------------------------------------

        console.log("takePending...5");
        ...
        if (user.collectOption == CollectOption.OnOff && user.amount > 0) {
            ...
            console.log("takePending...6", userCompound);
    ...
    */
    ...
    function _addToSubPool(SubPool storage subPool, uint256 totalShare, uint256 newAmount) internal {
        console.log("addToSubPool...total, newAmount", totalShare, newAmount);
        subPool.sumAccumulated += newAmount;
        subPool.accPerShare += (newAmount * 1e12 / totalShare);
        console.log("addToSubPool...sumAccumulated, accPerShare", subPool.sumAccumulated, subPool.accPerShare);
    }
}
