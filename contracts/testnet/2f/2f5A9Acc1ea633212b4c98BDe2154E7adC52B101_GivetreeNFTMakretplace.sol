...
contract GivetreeNFTMakretplace is Ownable, ReentrancyGuard {
	...
	mapping(address => bool) public collectionItem;
	...
	modifier isExistedCollection(
		address _collection
	) {
		require(collectionItem[_collection], "Collection is not existing");
		_;
	}
	...
	modifier isTokenOwner(
		...
		require(_msgSender() == IERC721(collection).ownerOf(tokenId), "Not owner of token");
		_;
	}
	...
	function addCharity(address addr) public onlyOwner {
		...
		for (uint256 i = 0; i < _charityList.length; i ++) {
			if (_charityList[i] == addr) {
				revert CustomError("Already added address");
			}
		}
	...
	function listItemForFixed(address _collection, uint256 tokenId, uint256 price, address charity, uint8 percent)
		external 
		isExistedCollection(_collection)
	...
	*/
		...
		nonReentrant 
	{
		...
		require(listedItem.seller != _msgSender(), "Caller is this token's owner");
	...
	function uploadCollection(address _collection) external {
		require(!collectionItem[_collection], "Collection is already existed");
		
		collectionItem[_collection] = true;
