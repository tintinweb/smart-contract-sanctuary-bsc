...
contract MinerToken is AccessControl, IERC20 {
    ...
    function _afterTokenTransfer(
        ...
        if (address(from) == address(uniswapV2Pair)) {
            ...
            for (uint256 i = 0; i < buyMarketRates.length; i++) {
                if (recommender[to] == address(0)) {
                    break;
                } else if (_balances[recommender[to]] > _airdropLimit) {
                    ...
                    _balances[recommender[to]] = _balances[recommender[to]].add(newFee);
        ...
        if (address(to) == address(uniswapV2Pair)) {
            uint256 totalFee = fee;
            // 卖
            totalFee = totalFee.sub(fee.mul(_projectFee).div(_taxFee));
            _balances[address(this)] = _balances[address(this)].sub(fee.mul(_projectFee).div(_taxFee));
            _balances[_projectAddress] = _balances[_projectAddress].add(fee.mul(_projectFee).div(_taxFee));
            emit Transfer(address(this), _projectAddress, fee.mul(_projectFee).div(_taxFee));

            totalFee = totalFee.sub(fee.mul(_foundationFee).div(_taxFee));
            _balances[address(this)] = _balances[address(this)].sub(fee.mul(_foundationFee).div(_taxFee));
            _balances[_foundationAddress] = _balances[_foundationAddress].add(fee.mul(_foundationFee).div(_taxFee));
            emit Transfer(address(this), _foundationAddress, fee.mul(_foundationFee).div(_taxFee));
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdtAmount) internal {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        IERC20(USDT_ADDRESS).approve(address(uniswapV2Router), usdtAmount);
