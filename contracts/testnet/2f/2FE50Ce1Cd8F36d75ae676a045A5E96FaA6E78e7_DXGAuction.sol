...
// File contracts/DXG-Auction.sol
...
contract DXGAuction is Ownable {
    ...
    event Sell(address seller, uint256 price, uint256 royalty, uint256 tokenId);
    event Purchase(address buyer, address seller, uint256 price, uint256 tokenId);
     ...
     */
    function bulkSell(uint256[] memory tokenIds, uint256 price, uint256 royalty) external {
        for(uint i = 0; i < tokenIds.length; i++) {
            sell(tokenIds[i], price, royalty);
        }
    }
     ...
     */
    function sell(uint256 tokenId, uint256 price, uint256 royalty) public {
        ...
        emit Sell(msg.sender, price, royalty, tokenId);
    }
    ...
    function buy(uint256 tokenId) external {
        ...
        // TODO check if buyer has enough ERC20
     ...
     */
    function createAuction(uint256 tokenId, uint256 royalty, uint256 price, uint256 startTime, uint256 endTime) external {
    ...
    function bid(uint256 tokenId, uint256 price) external {
        ...
        require(paymentTokenAddress != address(0), "Payment token is not set");
        // TODO check if bidder has enough ERC20
    ...
    function cancelBid(uint256 tokenId) external {
        ...
        require(selling.seller != address(0), "Selling does not  exist");
        require(selling.sellingType == SELLING_TYPE.AUCTION, "This selling is not an auction");
