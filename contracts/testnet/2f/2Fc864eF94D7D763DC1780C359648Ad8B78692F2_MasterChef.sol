...
// will be transferred to a governance smart contract once BOW is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BOWs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BOWs to distribute per block.
        uint256 lastRewardBlock; // Last block number that BOWs distribution occurs.
        uint256 accPopsPerShare; // Accumulated BOWs per share, times 1e12. See below.
    }
    // The BOW TOKEN!
    BOWToken public bow;
    // Block number when bonus BOW period ends.
    uint256 public bonusEndBlock;
    // BOW tokens created per block.
    uint256 public bowPerBlock;
    // Bonus muliplier for early bow makers.
    ...
    // The block number when BOW mining starts.
    uint256 public startBlock;
    // The block number when BOW mining ends.
    ...
    // Update the given pool's BOW allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending BOWs on frontend.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0
            ...
            uint256 bowReward =
                multiplier.mul(bowPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accPopsPerShare = accPopsPerShare.add(
                bowReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        bow.mint(address(this), bowReward);
    ...
    // Deposit LP tokens to MasterChef for BOW allocation.
    ...
    // Safe bow transfer function, just in case if rounding error causes pool to not have enough BOWs.
    function safePopsTransfer(address _to, uint256 _amount) internal {
        uint256 bowBal = bow.balanceOf(address(this));
        if (_amount > bowBal) {
            bow.transfer(_to, bowBal);
        } else {
            bow.transfer(_to, _amount);
        }
    }
}
