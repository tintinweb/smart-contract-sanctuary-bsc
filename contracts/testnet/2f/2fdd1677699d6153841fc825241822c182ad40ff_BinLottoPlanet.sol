...
contract BinLottoPlanet is Context, Ownable {
  using SafeMath
  for uint256;
  ...
  function buyCountry(string memory _country, string memory _message) public payable {
    require(bytes(_message).length <= 200, "Message must be under 200 characters");
    if (countries[_country].price > 0) {
      ...
      require(msg.value == 0.001 ether, "Not enough BNB send");
