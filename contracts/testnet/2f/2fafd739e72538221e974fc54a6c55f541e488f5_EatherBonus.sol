...
contract EatherBonus is Auth, EIP712Upgradeable {
  ...
  uint private normalRate;
  bytes32 public rootHash;
  uint private whiteListRate;
  ...
  function getBonus(bytes calldata _signature, uint _timestamp, uint _quantity, bytes32[] calldata _path) external {
    ...
    bool success = _haveBonus(_signature, _isInWhiteList(_path));
    if (success) {
      _giveReward(_quantity);
    }
  ...
  function _haveBonus(bytes calldata _signature, bool _whiteList) private view returns (bool) {
    (uint r0, uint r1) = lpToken.getReserves();
    uint randomNumber = Math.genRandomNumber(string(_signature), r0 + r1);
    return randomNumber < (_whiteList ? whiteListRate : normalRate);
  }

  function _isInWhiteList(bytes32[] calldata _path) private view returns (bool) {
  ...
  function setWhiteListRate(uint _rate) onlyMainAdmin external {
    require(_rate >= 1 && _rate <= 100, "EatherBonus: invalid rate");
    whiteListRate = _rate;
  }

  function setNormalRate(uint _rate) onlyMainAdmin external {
    require(_rate >= 1 && _rate <= 100, "EatherBonus: invalid rate");
    normalRate = _rate;
  }
}
