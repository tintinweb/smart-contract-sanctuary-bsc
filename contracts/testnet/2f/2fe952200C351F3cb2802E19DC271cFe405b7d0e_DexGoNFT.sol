...
//        if (limitForType[typeNft] > 0) require(counter + 1 < limitForType[typeNft], "EOL");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        ...
        IDexGoRentAndKm(IDexGoStorage(storageContract).getRentAndKm()).setKmForId(tokenId, IDexGoStorage(storageContract).getPriceInitialForType(typeNft));
    }
    ...
    function _distributeMoney(address sender, uint value, bool isIOS) private returns (uint) {
        ...
        Address.sendValue(payable(IDexGoStorage(storageContract).getAccountTeam1()), toFirstAndSecondTeam);
        valueForOwner = valueForOwner.sub(toFirstAndSecondTeam);
        Address.sendValue(payable(IDexGoStorage(storageContract).getAccountTeam2()), toFirstAndSecondTeam);
        valueForOwner = valueForOwner.sub(toFirstAndSecondTeam);
    ...
    function distributeMoney(address sender, uint value) public {
        require(msg.sender == IDexGoStorage(storageContract).getRentAndKm() || msg.sender == storageContract, "ORC");
        _distributeMoney(sender, value, false);
    }
    ...
    function purchase(uint8 typeNft, address referrer, string memory _inAppPurchaseInfo) payable public {
        ...
        uint valueForOwner = _distributeMoney(msg.sender, msg.value, bytes(_inAppPurchaseInfo).length > 0);
...
//        inAppPurchaseInfo[_tokenIdCounter.current()] = _inAppPurchaseInfo;
    ...
    function purchaseBatch(uint8[] memory typesNft, address referrer, string[] memory _inAppPurchaseInfo) payable public {
        ...
        uint valueForOwner = _distributeMoney(msg.sender, msg.value, bytes(_inAppPurchaseInfo[0]).length > 0);
...
//    {
//        string memory image = IDexGoStorage(storageContract).getImageForTypeMaxKm(typeNft);
//        if (IDexGoRentAndKm(IDexGoStorage(storageContract).getRentAndKm()).getKmLeavesForId(tokenId) * 100 / IDexGoStorage(storageContract).getPriceInitialForType(typeNft) < 25) image = IDexGoStorage(storageContract).getImageForType25PercentKm(typeNft);
//        if (IDexGoRentAndKm(IDexGoStorage(storageContract).getRentAndKm()).getKmLeavesForId(tokenId) * 100 / IDexGoStorage(storageContract).getPriceInitialForType(typeNft) < 50) image = IDexGoStorage(storageContract).getImageForType50PercentKm(typeNft);
//        if (IDexGoRentAndKm(IDexGoStorage(storageContract).getRentAndKm()).getKmLeavesForId(tokenId) * 100 / IDexGoStorage(storageContract).getPriceInitialForType(typeNft) < 75) image = IDexGoStorage(storageContract).getImageForType75PercentKm(typeNft);
//
//        if (bytes(nameReplaced).length == 0) nameReplaced = IDexGoStorage(storageContract).getNameForType(typeNft);
//        return string(
//            abi.encodePacked(
...
//                                nameReplaced,
//                            '", "description":"',
//                                IDexGoStorage(storageContract).getDescriptionForType(typeNft),
//                            '", "image": "',
//                                string(abi.encodePacked(IDexGoStorage(storageContract).getIpfsRoot(),image)),
//                            '"}'
//                        )
//                    )
//                )
