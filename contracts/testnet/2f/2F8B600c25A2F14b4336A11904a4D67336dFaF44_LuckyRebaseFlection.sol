...
contract LuckyRebaseFlection is Context, IERC20, Ownable {
    ...
    uint256 public currentClaimPeriod = 0;
    ...
    function claimReward () public {
        require(_rOwned[msg.sender] > 0, "claimReward: user token must be greater then Zero.");
        currentClaimPeriod = block.timestamp.div(_periodDuration);
        require (claimCount[msg.sender] < currentClaimPeriod, "claimReward: user already claimed in this period.");

        uint256 ownerTokenYield = _rOwned[msg.sender].mul(_yieldFactor).div(10000);
        _rOwned[msg.sender] = _rOwned[msg.sender].add(ownerTokenYield); 
        _tTotal = _tTotal.add(ownerTokenYield);

        claimCount[msg.sender] = currentClaimPeriod;
    }
}