...
contract NFTFactory is AccessControl, Pausable, ReentrancyGuard, NativeMetaTransaction {
    string public constant name = "Playbux NFT Factory";
    uint256 public constant BLOCK_PER_DAY = 28000;
    bytes32 internal constant ADMIN_ROLE = keccak256("ADMIN_ROLE"); // Admin role is used to withdraw nft from the contract
    ...
    event Mint(address indexed _from, uint256 _value, uint256 _type, string _transactionId);
    ...
    function buyNFT(
        ...
        if (block.number - lastMint[_receiver] > BLOCK_PER_DAY) {
            require(receivedAmount <= mintLimitPerDay, "Minting limit exceeded");
            mintAmount[_receiver] = 0; // reset amount
        } else {
            require(mintAmount[_receiver] + receivedAmount <= mintLimitPerDay, "Minting limit per day is exceeded");
        }

        mintAmount[_receiver] += receivedAmount;
        lastMint[_receiver] = block.number;
        nft.mintTo(_receiver, _type);

        emit Mint(_receiver, receivedAmount, _type, _transactionId);
    }
