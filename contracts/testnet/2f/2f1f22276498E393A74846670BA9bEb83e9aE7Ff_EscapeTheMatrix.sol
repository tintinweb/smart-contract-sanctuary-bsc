...
contract EscapeTheMatrix is ERC20, Ownable {
    ...
    mapping(address => bytes32) public addressToRefferalCode;
    ...
    string public referredCode;

    IPancakeswapV2Pair public USDPair;
    IPancakeswapV2Pair public TOKENPair;

    address public USD = 0xD17760BaBcE5fb9149293DCc7D4875E2Ad1547fB;
    ...
    {   
        ...
        projectWallet = 0xfee2879FAAb37E5662BD5E9D2f211cFc897C7340;
...
//------------------RefferalSystem------------------//
    ...
    function userRefferalInfo(address _user) public view returns(bytes32,string memory,address,uint256) {
        Refferal memory refferalInformation = addressToRefferals[_user];
        return (refferalInformation.referrerCode,refferalInformation.referrerString,refferalInformation.user,refferalInformation.referralBalance);
    }
    ...
    function manageTop25(address sender, uint256 _amount) internal{
        ...
        if (top25ActiveUsers[sender] == false){
            if (top25Users.length < 25){
                top25Users.push(Top25User(sender, _amount));
    ...
    function setReferrer(string memory _referredCode) external{
        ...
        addressToRefferalCode[address(msg.sender)] = _result;

        addressToRefferals[address(msg.sender)].referrerCode = _result;

        addressToRefferals[address(msg.sender)].referrerString = _referredCode;
    }

    function referralWalletLimits(uint256 _referralWalletLimit) external onlyOwner {
        referralWalletLimit = _referralWalletLimit;
    }
...
//------------------Transfer------------------//
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (from == uniswapV2Pair){
            require(refferalCodestoAddress[addressToRefferalCode[to]] != address(0), "You must have a referrer to buy");
        ...
        if (_isExcludedFromFees[from] || 
            ...
            if(balanceOf(address(this)) > 0 ){
                ...
                addressToRefferals[to].referralBalance += referralTokens;
                manageTop25(to, addressToRefferals[to].referralBalance);
                super._transfer(address(this),addressToRefferals[to].user,referralTokens);
            }
