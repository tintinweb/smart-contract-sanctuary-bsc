...
contract Grade24 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, IGrade24 {
	...
	mapping(address => int256) public pendingTicketCount;
    mapping(address => int256) public pendingBreedCount;
    ...
    function initialize(IERC20Upgradeable _mbtc, address root, address authorizer ) public initializer {
	...
	function register(address myParent) public{
		...
		int256 _pendingTicketCount = pendingTicketCount[msg.sender];
		int256 _pendingBreedCount = pendingBreedCount[msg.sender];
		
		if (_pendingTicketCount > 0) {
			updateTicketP(msg.sender, _pendingTicketCount);
		}
		
		if (_pendingBreedCount > 0) {
			updateBreedP(msg.sender, _pendingBreedCount);
		}
		...
		if (fromMember.round > 0) {
			...
			if (fromMemberParent.round > 0) {
				...
				if (fromMemberGrandParent.round> 0) {
					...
					for(uint256 i=0; i< fromMemberGrandParent.children.length;i++) {
						...
						if (thisChild.round > 0) {
							...
							} else if (highestP1 > 0 && thisChild.directP1 == highestP1) { 
								deductLevel2P1 += thisChild.directP1;
							}
				...
				if (fromMemberGrandParent.round > 0) {
					...
					for(uint256 i=0; i< fromMemberGrandParent.children.length;i++) {
						...
						if (thisChild.round > 0) {
							...
							addLevel2P2 += thisChild.directP1;
