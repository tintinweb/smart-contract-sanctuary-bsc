...
contract YTON is Ownable{
	...
	uint256  public INVEST_MIN_AMOUNT = 50 ether;  // 最小质押USDT数量
    ...
    function updateLevel(address userAddress,uint256 amount) internal returns(bool){
        ...
        if(amount == 400 ether){
            user.userLevel = 3;
            user.staticBonus = amount.mul(STATIC_NUM[1]).div(PERCENTS_DIVIDER);
        }
        if(amount == 200 ether){
            user.userLevel = 2;
            user.staticBonus = amount.mul(STATIC_NUM[2]).div(PERCENTS_DIVIDER);
        }
        if(amount == 100 ether){
            user.userLevel = 1;
            user.staticBonus = amount.mul(STATIC_NUM[3]).div(PERCENTS_DIVIDER);
        }
        if(amount == 50 ether){
            user.staticBonus = amount.mul(STATIC_NUM[4]).div(PERCENTS_DIVIDER);
        }
        return true;
    }
