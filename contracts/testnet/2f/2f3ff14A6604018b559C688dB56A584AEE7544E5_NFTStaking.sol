...
contract NFTStaking is Ownable {
    ...
    mapping(uint256 => address) private raffleEntries;
    ...
    function setTokenStakeSettings(uint256 _minToken, uint256 _maxQty)
        external
        onlyOwner
    {
        require(_minToken > 0 && _maxQty > 0);
        minToken = _minToken;
        maxQty = _maxQty;
    }
    ...
    function setPrizeItem(
        ...
        require(
            _index <= prizeLength && _index != 0,
            "Durham_Inu_Staking: Invalid stake item"
        );
        if (_isTangible) {
            require(
                _ethReward == 0,
        ...
        prizeItems[_index].isTangible = _isTangible;
        prizeItems[_index].name = _name;
        prizeItems[_index].desc = _desc;
        prizeItems[_index].ethReward = _ethReward;
        prizeItems[_index].requiredPoints = _requiredPoints;
    }

    function setPrizeLength(uint256 _priceLength) external onlyOwner {
        prizeLength = _priceLength;
    }
    ...
    function getUserEntries(address _address) external view returns (uint256) {
        return currentStakeInfo[_address].raffleIds.length();
    }
    ...
    function _unStake(address _address) internal {
        ...
        for (uint256 x = 0; x < userRaffleCount; x++) {
            uint256 id = currentStakeInfo[_address].raffleIds.at(x);
    ...
    function claim(uint256 _prizeItem) external {
        require(
