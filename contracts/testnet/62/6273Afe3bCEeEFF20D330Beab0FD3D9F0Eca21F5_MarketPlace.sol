...
contract MarketPlace is Ownable, ReentrancyGuard, Pausable {
  ...
  // LiveTrade Membership List
  mapping(address => bool) public membershipNfts;
  ...
  constructor(
    ...
    uint256 _feeRate,
    address[] memory _membershipNfts,
    ...
    feeRate = _feeRate;
    for (uint256 index = 0; index < _membershipNfts.length; index++) {
      membershipNfts[_membershipNfts[index]] = true;
    }
  ...
  // Main function
  // Total of sale item
  function querySaleItemsByStatus(bool _isSold) public view returns (uint256[] memory) {
    uint256[] memory result = new uint256[] (saleItems.length);
    uint256 counterTmp = 0;
    for (uint256 index = 0; index < saleItems.length; index++) {
      SaleItem storage saleItem = saleItems[index];
      if (saleItem.isSold == _isSold) {
        result[counterTmp] = saleItem.saleId;
        counterTmp++;
      }
    }
    return result;
  ...
  // Set nft support trading
  function setSupportNft(address[] memory _newMembershipNfts ) external onlyOwner {
    for (uint256 index = 0; index < _newMembershipNfts.length; index++) {
      membershipNfts[_newMembershipNfts[index]] = true;
    }
  }

  // Set nft support trading
  function removeSupportNft(address[] memory _membershipNfts ) external onlyOwner {
    for (uint256 index = 0; index < _membershipNfts.length; index++) {
      membershipNfts[_membershipNfts[index]] = false;
    }
  }
  ...
  function sellItem(address _nftAddress, uint256 _nftId, uint256 _price, address _currency) external nonReentrant validOpenMarket validAddress(_currency) {
    ...
    // Check nftAddress in whitelist
    require(membershipNfts[_nftAddress]  == true, "Your NFT is not in membershipNfts");
    ...
    counter.increment();
    // Emit event
    emit EventNewSales(saleItem.saleId, _nftId, _price, saleItem.isSold, saleItem.seller, address(0), _currency, _nftAddress);
  ...
  function updateItemPrice(uint256 _saleIndex, uint256 _newPrice) external nonReentrant validOpenMarket validSaleItem(_saleIndex, msg.sender) {
    ...
    emit EventSellerUpdateNewPrice(_saleIndex, _newPrice);
  }
  ...
  function delistItem(uint256 _saleIndex) external nonReentrant validSaleItem(_saleIndex, msg.sender){
    ...
    emit EventDelistItem(_saleIndex, itemSeller);
  }
  ...
  function buyItem(uint256 _saleIndex) external nonReentrant validItemIsNotSold(_saleIndex) {
    ...
    emit EventTradingCompleted(_saleIndex, nftId, msg.sender, pricing);
  }
