    ...
    ReentrancyGuardUpgradeable
{
    ...
    function deposit(uint256 _amount) external {
        ...
        if (stakeInfos[msg.sender].amount == 50000e18) {
            uint256 _apr = 75;
            developerFee = 300;
            _deposit(_amount, _apr, developerFee);
        } else if (stakeInfos[msg.sender].amount == 500000e18) {
            uint256 _apr = 100;
            developerFee = 300;
            _deposit(_amount, _apr, developerFee);
        } else if (stakeInfos[msg.sender].amount == 2500000e18) {
            uint256 _apr = 125;
            developerFee = 250;
            _deposit(_amount, _apr, developerFee);
        } else if (stakeInfos[msg.sender].amount == 15000000e18) {
            uint256 _apr = 150;
            developerFee = 200;
            _deposit(_amount, _apr, developerFee);
        } else {
    ...
    function compoundAllReward() public nonReentrant {
        ...
        _deposit(amountToSend, apr, developerFee);
        totalReward = totalReward.add(amountToSend);
    }
    ...
    function getAllClaimableReward(address _investor)
        ...
        returns (uint256)
    {
        if (stakeInfos[_investor].amount == 50000e18) {
            uint256 _apr = 75;
            uint256 a = getallClaim(_investor, _apr);
            return a;
        } else if (stakeInfos[_investor].amount == 500000e18) {
            uint256 _apr = 100;
            uint256 a = getallClaim(_investor, _apr);
            return a;
        } else if (stakeInfos[_investor].amount == 2500000e18) {
            uint256 _apr = 125;
            uint256 a = getallClaim(_investor, _apr);
            return a;
        } else if (stakeInfos[_investor].amount == 15000000e18) {
            uint256 _apr = 150;
            uint256 a = getallClaim(_investor, _apr);
            return a;
        } else {
    ...
    function stakeMSTToken(uint256 stakeAmount) external payable {
        ...
        if (stakeInfos[msg.sender].amount + stakeAmount >= 50000e18) {
            // New APR
            uint256 _apr = 75;
            ...
            stakeInfos[msg.sender].amount + stakeAmount >= 500000e18 &&
            stakeInfos[msg.sender].amount + stakeAmount <= 2500000e18
        ) {
            uint256 _apr = 100;
            // get remaing time of withdrawal period
            uint256 totalTime = (investors[msg.sender].startTime +
            ...
            stakeInfos[msg.sender].amount + stakeAmount >= 2500000e18 &&
            stakeInfos[msg.sender].amount + stakeAmount <= 15000000e18
        ) {
            uint256 _apr = 125;
            // get remaing time of withdrawal period
            uint256 totalTime = (investors[msg.sender].startTime +
            ...
            // Assign Total Claim to Max Claim
            investors[msg.sender].maxClaimableAmount = totalClaim;
        } else if (stakeInfos[msg.sender].amount + stakeAmount >= 15000000e18) {
            uint256 _apr = 150;
            // get remaing time of withdrawal period
            uint256 totalTime = (investors[msg.sender].startTime +
                withdrawPeriod) - block.timestamp;
