...
interface IDaoMigration {
 ...
 updatePoolBUSD();
  ...
  function payreferralbank(uint _pid,uint _amount) internal{
    address refer= referralvault.getReferrer(msg.sender);
    require(refer!=address(0), 'invalid referrer');
    uint firstamount=_amount.mul(levelRate[0]).div(1000);
    userInfo[_pid][refer].referralpoint=userInfo[_pid][refer].referralpoint.add(firstamount);
    totalPoint=totalPoint.add(firstamount);
    address secondrefer=referralvault.getReferrer(refer);
    if(secondrefer!=address(0)){
      uint secondamount=_amount.mul(levelRate[1]).div(1000);
      userInfo[_pid][secondrefer].referralpoint=userInfo[_pid][secondrefer].referralpoint.add(secondamount);
      totalPoint=totalPoint.add(secondamount);
      address thirdrefer=referralvault.getReferrer(secondrefer);
      if(thirdrefer!=address(0)){
        uint thirdamount=_amount.mul(levelRate[0]).div(1000);
        userInfo[_pid][thirdrefer].referralpoint=userInfo[_pid][thirdrefer].referralpoint.add(thirdamount);
        totalPoint=totalPoint.add(thirdamount);
        checkend(_pid);
      }
    }
  }
  ...
  function payreferralDAO(uint _pid,uint _amount) internal{
    address refer= referralvault.getReferrer(msg.sender);
    require(refer!=address(0), 'invalid referrer');
    userInfo[_pid][refer].referralpoint=userInfo[_pid][refer].referralpoint.add(_amount).mul(referralRate[0]).div(1000);
    address secondrefer=referralvault.getReferrer(refer);
    if(secondrefer!=address(0)){
      userInfo[_pid][secondrefer].referralpoint=userInfo[_pid][secondrefer].referralpoint.add(_amount).mul(referralRate[1]).div(1000);
      address thirdrefer=referralvault.getReferrer(secondrefer);
      if(thirdrefer!=address(0)){
          userInfo[_pid][thirdrefer].referralpoint=userInfo[_pid][thirdrefer].referralpoint.add(_amount).mul(referralRate[2]).div(1000);
      }
    }
  }
  ...
  function pendingRewardDaorate(uint _pid, address _address) public view returns (uint256){
    ...
    return point.mul(510).div(86400).div(365).div(300).div(pricedcvc).div(decimals);
  }
   ...
   */
  function payReferralDAO(uint256 value,address token) internal returns(uint){
    ...
    uint256 remaining=0;
    uint256 buybackamount=value.mul(20).div(100);
      remaining=value.sub(totalReferal).sub(buybackamount);
      IBEP20(token).transfer( address(RewardPool),totalReferal);
      IBEP20(token).transfer(buyback,buybackamount);
  ...
  function payReferralBank(uint256 value,address token) internal returns(uint){
    // Account memory userAccount = accounts[msg.sender];
    address user=msg.sender;
    uint256 totalReferal;
     uint256 c ;
