...
// Lava Token
contract LavaToken is BEP20("Volcanic Token", "LAVA") {
...
contract LavaMasterchef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block timestamp when Lava mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 LavaPerSecond);
    event addPool(
        uint256 indexed pid,
        address lpToken,
        uint256 allocPoint,
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _LavaPerSecond,
        uint256 _startTime
    ) public {
        lava = _lava;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        LavaPerSecond = _LavaPerSecond;
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 lavaReward = multiplier
                .mul(LavaPerSecond)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accLavaPerShare = accLavaPerShare.add(
                lavaReward.mul(1e18).div(pool.lpSupply)
            );
        }
        return user.amount.mul(accLavaPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 devReward = lavaReward.div(10);
        uint256 totalRewards = lava.totalSupply().add(devReward).add(
            lavaReward
        );

        if (totalRewards <= lava.maxSupply()) {
            // mint as normal as not at maxSupply
            lava.mint(devaddr, lavaReward.div(10));
            lava.mint(address(this), lavaReward);
        } else {
            // mint the difference only to MC, update lavaReward
            lavaReward = lava.maxSupply().sub(lava.totalSupply());
            lava.mint(address(this), lavaReward);
        }

        if (lavaReward != 0) {
            // only calculate and update if lavaReward is non 0
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accLavaPerShare).div(1e18).sub(
            user.rewardDebt
        );
        if (pending > 0) {
            safeLavaTransfer(msg.sender, pending);
    ...
    function updateEmissionRate(uint256 _LavaPerSecond) external onlyOwner {
        require(_LavaPerSecond <= MAX_EMISSION_RATE, "Emission too high");
        massUpdatePools();
        LavaPerSecond = _LavaPerSecond;
        emit UpdateEmissionRate(msg.sender, _LavaPerSecond);
    }
