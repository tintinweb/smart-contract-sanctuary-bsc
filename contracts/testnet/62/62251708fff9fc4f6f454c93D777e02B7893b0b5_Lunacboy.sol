...
contract Lunacboy is Context, IERC20, Ownable {
    ...
    string private constant _name = "Luna Classic Boy";
    string private constant _symbol = "LUNCB";

    address private Lunc_Marketing_Wallet = 0x916347F913588E7de971074d3D7330954Fe579DC;
    address private Lunc_Team_Wallet = 0x916347F913588E7de971074d3D7330954Fe579DC;

    uint256 public Lunc_max_WalletLimit = 20000 * 10**9; //2%
    uint256 public swapTokensAtAmount = 3000 * 10**9; //0.3%
    uint256 public Luncboy_max_BuyLimit = 20000 * 10**9; // 2%
    uint256 public Luncboy_max_SellLimit = 20000 * 10**9; //2%
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Lunc_Marketing_Wallet] = true;
        _isExcludedFromFee[Lunc_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Lunc_Marketing_Wallet] = true;
        allowedTransfer[Lunc_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Luncboy_max_BuyLimit, "You are exceeding Lunc_max_BuyLimit");
            require(balanceOf(to) + amount <= Lunc_max_WalletLimit, "You are exceeding Lunc_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Luncboy_max_SellLimit, "You are exceeding Lunc_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Lunc_max_WalletLimit, "You are exceeding Lunc_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Lunc_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Lunc_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Luncboy_max_BuyLimit = maxBuy * 10**decimals();
        Luncboy_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Luncboy_Max_Walletlimit(uint256 amount) external onlyOwner{
        Lunc_max_WalletLimit = amount * 10**decimals();
    }
