...
contract KyotoLaunchpad {
    struct IDO {
        ...
        uint256 publicMaxAllocation; // Maximum amount of tokens that can be allocated to users
    ...
    function investInWhitelistPhase(
        ...
        require(
            _idos[idoID].publicInvestmentStartTimestamp > block.timestamp,
    ...
    function _validateIDOData(IDO calldata ido) private view {
        ...
        require(
            !(ido.publicSlots != 0 && ido.publicMaxAllocation == 0),
            "KyotoLaunchPad: Public max allocation cannot be 0"
        );
        require(
        ...
        require(!ido.isRewarded, "KyotoLaunchPad: IDO cannot be rewarded");
    }
    ...
    function _invest(string calldata idoID, uint256 _amount) private {
        ...
        // Check for max allocation for that tier
        require(
            _idos[idoID].publicMaxAllocation >=
            idoInvestment.publicTierTotalInvestment + _amount,
            "KyotoLaunchPad: Public max allocation is full"
        );
        idoInvestment.publicTierTotalInvestment += _amount;
