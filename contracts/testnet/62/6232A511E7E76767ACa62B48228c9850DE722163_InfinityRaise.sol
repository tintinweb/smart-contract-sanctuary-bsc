...
contract InfinityRaise is ERC20, ERC1363, ERC2612, ERC20Burnable, ERC20TokenRecover, IInfinityRaise {
    ...
    function addNewRouter(address _router, bool makeDefault) external override onlyOwner {
        ...
        if (makeDefault) {
            ...
            defaultPair = IDEXFactory(defaultDexRouter.factory()).createPair(address(this), busd);
            _setAutomatedMarketMakerPair(defaultPair, true);
        }
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        swapTokensForBUSD(half); // <- this breaks the BUSD -> IRAISE swap when swap+liquify is triggered
    ...
    function addLiquidity(uint256 tokenAmount, uint256 busdAmount) private {
        ...
        _approve(address(this), address(defaultDexRouter), tokenAmount);
        IERC20(busd).approve(address(defaultDexRouter), busdAmount);
