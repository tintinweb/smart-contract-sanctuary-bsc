...
contract NFTHelpAdv1 is Ownable {
    ...
    function HeroTop(uint _top) external view haveMainPool haveNft returns (address[] memory _wal, uint256[] memory _token, uint16[] memory _type, uint16[] memory _level, uint8[] memory _quality, uint32[] memory _hashrate) {
    ...
    function queryBytoken(uint256 _token) external view haveNft returns (address _owner, uint16 _level, uint8 _quality,uint32 _hashrate, uint8 _status, uint16 _type, uint16[] memory _Levels, uint256[] memory _UpTimes, uint32[] memory _HashRates) {
    ...
    function queryByOwner(address _inowner) external view haveNft returns (address[] memory _outowner, uint16[] memory _level, uint8[] memory _quality,uint32[] memory _hashrate, uint8[] memory _status, uint16[] memory _type) {    
        // uint256[] memory allTokens = IYiBoxNFT(NFTToken).getAllTokens();
        uint256[] memory allTokens = IYiBoxNFT(NFTToken).tokensOfOwner(_inowner);
