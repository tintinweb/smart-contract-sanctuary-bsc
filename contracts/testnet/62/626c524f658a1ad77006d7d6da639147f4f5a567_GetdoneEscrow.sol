...
contract GetdoneEscrow is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, AccessControlEnumerableUpgradeable, GetdoneApp{
    ...
    function depositContract(uint256 merchantId, uint256 appId, uint256 contractId, address talent, uint256 price, address erc20, uint256 orderId) 
        public
        payable
        whenNotPaused
        nonReentrant
    ...
    function createDisputation(uint256 merchantId, uint256 appId, uint256 contractId, address customer, address sender, address disputationBanker) 
        public
        whenNotPaused
        nonReentrant
        returns (uint256)
    {
        require(hasRole(DISPUTATION_ROLE, _msgSender()), "GetdoneEscrow: caller is not the disputation contract");
        require(validApp(merchantId, appId), "GetdoneEscrow: App is invalid");
        require(disputationBanker != address(0), "GetdoneEscrow: Disputation Banker Address is invalid");
