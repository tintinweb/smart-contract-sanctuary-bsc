    ...
    function calclulateReward(address _user) public view returns(uint256 ,uint256){
        ...
        for(uint256 i=0;i<users.deposites.length;i++){
            ...
            uint256 time=currenctTime.sub(users.deposites[i].checkPointBUSD);
                if(time<20 minutes){
                    totalRewardBUSD+=depositeAmount.mul(percentage[0]).div(divider).mul(time).div(1 minutes);
                }else if(time>=21 minutes && time<40 minutes){
                    totalRewardBUSD+=depositeAmount.mul(percentage[1]).div(divider).mul(time).div(1 minutes);
                }else if(time>=40 minutes){
                    totalRewardBUSD+=depositeAmount.mul(percentage[2]).div(divider).mul(time).div(1 minutes);
                }
            }else{
                uint256 time=currenctTime.sub(users.deposites[i].checkPointBNB);
                if(time<20 minutes){
                    totalRewardBNB+=depositeAmount.mul(percentage[0]).div(divider).mul(time).div(1 minutes);
                }else if(time>=21 minutes && time<40 minutes){
                    totalRewardBNB+=depositeAmount.mul(percentage[1]).div(divider).mul(time).div(1 minutes);
                }else if(time>=40 minutes){
                    totalRewardBNB+=depositeAmount.mul(percentage[2]).div(divider).mul(time).div(1 minutes);
                }
            }
        }
