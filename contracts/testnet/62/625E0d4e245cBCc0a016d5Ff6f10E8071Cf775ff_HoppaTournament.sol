...
contract HoppaTournament {
    ...
    address[MAX_PLAYERS] public players;
    ...
    function removePlayer(address player) external onlyGameMaker {
        uint index = findEntry( player );
        require( index < MAX_PLAYERS );

        if( stakedTokens[ player ] > 0 ) {
    ...
    function startGame(uint daysDuration) external onlyGameMaker {
    ...
    function selectWinner(address _winner) external onlyGameMaker {
        ...
        uint index = findEntry(_winner);
        require( index < MAX_PLAYERS, "Winner must have a ticket");
    ...
    function enterArena(uint256 amount) external {
        ...
        if( stakedTokens[ msg.sender ] == 0 ) {
            index = findFreeEntry();
            require( index < MAX_PLAYERS, "Sold out");
        }
        else {
            index = findEntry( msg.sender );
            require( index < MAX_PLAYERS, "All slots are full");
        }
    ...
    function leaveArena() external {
        ...
        uint index = findEntry( msg.sender );
    ...
    function findEntry( address player ) internal view returns (uint) {
        for (uint i = 0; i < players.length; i++) {
            if (players[i] == player) {
                return i;
            }
    ...
    function findFreeEntry() internal view returns (uint) {
        for (uint i = 0; i < players.length; i++) {
            if (players[i] == address(0)) {
                return i;
            }
