    ...
    
{
    ...
    address public dividendToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    function distributeDividendForBnb(uint256 amount) public payable  onlyOwner {
...
contract WIZEToken is ERC20, Ownable {
    ...
    uint256 public buyTotalFees = buyDividendRewardsFee + buytreasuryFee  + buyTeamFee + buyLiquidityFee  + buyMarketingFee + buyBurnFee;
    // sell fees
    ...
    uint256 public sellTotalFees = sellDividendRewardsFee +selltreasuryFee + sellTeamFee + sellLiquidityFee  + sellMarketingFee + sellBurnFee;
    ...
    uint256 private maxTransferAmountRate = 100;
    ...
    constructor(address _newOwner) ERC20("Wizefrog", "WIZE") {
        dividendTracker = new WIZETokenDividendTracker();
        
        teamWallet = 0x5Bfb7ED32DC7BaEC41892d0ed10Ab231d495fb67;
        treasurywallet = 0x4E14401C3857710467dcBa8BAFf6FA3bf44d5845;
        marketingWallet = 0x701641b954a05825AfFB945B954dFe1eE1886C81;
        ...
        */
        _mint(_newOwner,  5000000000 * (10**9));
    ...
    function setTradingIsEnabled() public onlyOwner {
        require(tradingIsEnabled == false, 'WIZE: The trading has already enabled');
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "WIZE: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "WIZE: The new dividend tracker must be owned by the WIZE token contract"
        );
    ...
    function updateBuyFees(
        ...
        buyTotalFees = reward
            ...
            .add(burn)
            .add(treasury);
        require(buyTotalFees <= 10, "max 10%");    
    }
    ...
    function updateSellFees(
        ...
        require(sellTotalFees <= 10, "max 10%");     
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "WIZE: The router already has that address"
        );
    ...
    function updateTeamAddress(address newAddress) public onlyOwner {
        require(teamWallet != newAddress, "Team wallet and new Addres should not same");
    ...
    function updateTreasuryAddress(address newAddress) public onlyOwner {
        require(treasurywallet != newAddress, "Team wallet and new Addres should not same");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "WIZE: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "WIZE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "WIZE: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "WIZE: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "WIZE: Cannot update gasForProcessing to same value"
        );
    ...
    function setMaxTransferAmountRate(uint256 _val) public onlyOwner {
        require(_val >= 10 , "Max Transaction limit cannot be lower than 0.1% of total supply"); 
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to] ) {
            ...
            if (!swapping && canSwap && buyTotalFees > 0) {
                ...
                if(buyBurnFee > 0) {
                  uint256 burnAmount = contractTokenBalance.mul(buyBurnFee).div(buyTotalFees); 
                  super._transfer(address(this), deadAddress, burnAmount);
                }  
            
                // swap and add liquidity
                if(buyLiquidityFee > 0 ) {
                   uint256 buyLiquidityAmount = contractTokenBalance.mul(buyLiquidityFee).div(buyTotalFees);
                   swapAndLiquify(buyLiquidityAmount);
                }
    ...
    function swapAndSendBnb(uint256 tokenAmount, uint256 totalFee) private {
        ...
        transferToWallet(
            payable(teamWallet),
            BNBAmount.div(totalFee).mul(buyTeamFee)
        );
        transferToWallet(
            payable(treasurywallet),
            BNBAmount.div(totalFee).mul(buytreasuryFee)
        );
        transferToWallet(
            payable(marketingWallet),
            BNBAmount.div(totalFee).mul(buyMarketingFee)
        );
    }
    ...
    function swapAndSendDividendsInBNB(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributeDividendForBnb(dividends);
            emit SendDividends(tokens, dividends);
        }
    }
...
contract WIZETokenDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("WIZE_Dividend_Tracker", "WIZE_Dividend_Tracker")
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 250000 * (10**9); //must hold 10000+ tokens
    }
    ...
    function _transfer(
        ...
        require(false, "WIZE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WIZE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WIZE contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "WIZE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "WIZE_Dividend_Tracker: Cannot update claimWait to same value"
        );
