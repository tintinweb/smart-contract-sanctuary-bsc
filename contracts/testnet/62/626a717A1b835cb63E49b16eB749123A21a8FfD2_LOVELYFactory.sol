...
contract LOVELYPairToken is UniswapV2Pair {
    ...
    function initializeValidated(address _token, uint _amount, uint _fee) external {
        require(msg.sender == factory, 'LOVELY SWAP: FORBIDDEN');
    ...
    function getReserves() public view returns (uint112 _reserve0, uint112 _reserve1, uint32 _blockTimestampLast) {
        require(0 == _validationTokenAmount, "LOVELY SWAP: NON_VALIDATED_PAIR");
    ...
    function validate() public {
        address feeTo = IUniswapV2Factory(factory).feeTo();
        require(feeTo != address(0), "LOVELY SWAP: NON_FEE_PAIR");
...
contract LOVELYFactory is UniswapV2Factory {
    ...
    // TODO: Block using usual "createPair"
    ...
    function createValidatedPair(address tokenA, address tokenB, address tokenC, uint validationAmount, uint fee) external returns (address pair) {
        ...
        require(getPair[token0][token1] == address(0), 'LOVELY SWAP: PAIR_EXISTS');
        ...
        ILOVELYPairToken(pair).initializeValidated(tokenC, validationAmount, fee);
        getPair[token0][token1] = pair;
