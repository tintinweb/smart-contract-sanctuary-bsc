    ...
    IERC721ReceiverUpgradeable
{
    ...
    /// @notice nftContractAddress => landSize of the WL
    mapping(address => string) public nftLandsSizeMap;

    /// @notice auctionBidPeriod => allowed or not
    mapping(uint256 => bool) public allowedBidPeriods;

    /// @notice weaponTier => valid or not
    mapping(uint8 => bool) public validWeaponTiers;

    /// @notice armorTier => valid or not
    mapping(uint8 => bool) public validArmorTiers;
    ...
    /// @notice the contract address of the Armor
    ...
    //============== MODIFIERS ==============
    /// @notice Checks if the NFT the user has is valid for the WL of the land
    /// @param landId The identifier of the land to buy
    /// @param wlNftAddress The contract address of the NFT for allowing the WL
    /// @param tokenId The identifier of the NFT for allowing the WL
    modifier canNFTBid(
        uint256 landId,
        address wlNftAddress,
        uint256 tokenId
    ) {
        string memory landSize = ILand(landsAddress).getLandSize(landId);
        require(
            IERC721Upgradeable(wlNftAddress).ownerOf(tokenId) == msg.sender,
            "Not owner of NFT for whitelist"
        );
        require(
            equal(nftLandsSizeMap[wlNftAddress], landSize),
            "Wrong nft for land size"
        );
        if (equal(landSize, "Micro")) {
            (uint8 tier, , , , , , ) = Weapon(wlNftAddress).weaponsInfoMap(
                tokenId
            );
            require(validWeaponTiers[tier], "Wrong weapon tier");
        } else if (wlNftAddress == armorAddress) {
            //Necessary because there are land vehicles and armors for the medium land size
            (uint8 tier, , , , , , ) = Armor(wlNftAddress).armorsInfoMap(
                tokenId
            );
            require(validArmorTiers[tier], "Wrong armor tier");
        }
        _;
    }
    ...
    function initialize(
        ...
        validWeaponTiers[1] = true;
        validWeaponTiers[2] = true;
        validArmorTiers[2] = true;

        allowedLandSizes["Micro"] = true;
        allowedLandSizes["Standard"] = true;
        allowedLandSizes["Macro"] = true;
        allowedLandSizes["Mega"] = true;
    }
    ...
    /// @param wlNftAddress The NFT address of the whitelist
    /// @param whitelistTokenId The token ID of the NFT that gives whitelist permission
    function makeBid(
        uint256 counter_,
        uint256 landId,
        uint256 tokenAmount,
        ...
        canNFTBid(landId, wlNftAddress, whitelistTokenId)
    {
    ...
    /// @dev LandSize 0 is for NANO lands, which don't have WL and are sold in another contract
    /// @param landSize Land size
    /// @param nftContractAddress Nft contract address to be set for the land size
    function changeNftLandSizeMap(
        string calldata landSize,
        address nftContractAddress
    ) external onlyOwner {
        require(allowedLandSizes[landSize], "Land size not allowed");
        require(nftContractAddress != address(0), "Wrong nftContractAddress");
        nftLandsSizeMap[nftContractAddress] = landSize;
        emit ChangedNftLandSizeMap(landSize, nftContractAddress);
    }
    ...
    /// @notice Change required weapon tiers for WL
    /// @param tier The tier of the weapon
    /// @param isValid Tells if the tier has access to the WL or not
    function changeValidWeaponTiers(
        uint8 tier,
        bool isValid
    ) external onlyOwner {
        validWeaponTiers[tier] = isValid;
        emit ChangedValidWeaponTiers(tier, isValid);
    }

    /// @notice Change required armor tiers for WL
    /// @param tier The tier of the armor
    /// @param isValid Tells if the tier has access to the WL or not
    function changeValidArmorTier(uint8 tier, bool isValid) external onlyOwner {
        validArmorTiers[tier] = isValid;
        emit ChangedValidArmorTiers(tier, isValid);
    }
