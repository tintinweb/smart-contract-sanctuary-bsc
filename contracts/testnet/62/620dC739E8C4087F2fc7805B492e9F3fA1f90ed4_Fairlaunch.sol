...
contract Fairlaunch is Ownable
{
    ...
    function _mintFee(uint256 workerId, uint256 time) internal {

        IWorkerConfig config = Workers[workerId].WorkerConfig;
        IShareToken(config.ShareToken()).mint(TreasuryAccount, config.PendingManagementFee());
    ...
    mapping(uint256 => mapping(address => UserInfo)) WorkerUsers;
    
    PoolInfo[] public Pools;
    mapping(uint256 => mapping(address => UserInfo)) PoolUsers;
    ...
    function SetParams(uint256 rewardRatio,address treasuryAccount) public onlyOwner
    {
        RewardRatio = rewardRatio;
        TreasuryAccount = treasuryAccount;
    }
    function SetWorker(uint256 workerId, address newWorkerConfig, address utilityToken) public onlyOwner
    {
        Workers[workerId].WorkerConfig = IWorkerConfig(newWorkerConfig);
    ...
    function AddWorker(address dev,address workerConfig,address utilityToken) public onlyOwner {
        Workers.push(WorkerInfo({
            ...
            AccRewardPerShare:0,
            Dev:dev,
            WorkerConfig: IWorkerConfig(workerConfig),
    ...
    function WorkerDeposit(uint256 amount, uint256 workerId, address from, uint256 time) public onlyRouter {
        _mintFee(workerId, time);
        WorkerInfo storage worker = Workers[workerId];
        UserInfo storage user = WorkerUsers[workerId][from];
        UpdateWorker(workerId);
        uint256 useramount = IShareToken(worker.WorkerConfig.ShareToken()).balanceOf(from);
        if (user.Amount > 0) {
    ...
    function WorkerWithdraw(uint256 amount, uint256 workerId, address to, uint256 time) public onlyRouter {
        _mintFee(workerId, time);
        WorkerInfo storage worker = Workers[workerId];
        UserInfo storage user = WorkerUsers[workerId][to];
        UpdateWorker(workerId);
        ClaimUtility(workerId, to);
        uint256 useramount = IShareToken(worker.WorkerConfig.ShareToken()).balanceOf(to);
    ...
    function ChangeShareToken_Test(uint256 workerId, uint256 amount, bool isMint)public
    {
        if(isMint)
        {
            IShareToken(Workers[workerId].WorkerConfig.ShareToken()).mint(owner(), amount);
        }
        else
        {
            IShareToken(Workers[workerId].WorkerConfig.ShareToken()).burn(owner(), amount);
        }
    }
    ...
    {
        uint256 rewardBalance = IShareToken(RewardToken).balanceOf(address(this));
        {
            uint256 length = Workers.length;
            for (uint256 workerId = 0; workerId < length; ++workerId) {
                UpdateWorker_Test(workerId,blockNumber, blockTime);
            }
        }
        rewardBalance = IShareToken(RewardToken).balanceOf(address(this)) - rewardBalance;
        PoolInfo storage pool = Pools[poolId];
        if ( blockNumber <= pool.LastRewardBlock) {
            return;
        }
        uint256 stakeTokenSupply = IShareToken(pool.StakeToken).balanceOf(address(this));
        if (stakeTokenSupply == 0) {
            pool.LastRewardBlock =  blockNumber;
            return;
        }
        pool.AccRewardPerShare = pool.AccRewardPerShare.add(rewardBalance.mul(ACC_REWARD_PRECISION).div(stakeTokenSupply));
        pool.LastRewardBlock =  blockNumber;
    ...
    function UpdateWorker(uint256 workerId) public
    {
        ...
        
        {
            ...
            if(block.timestamp > worker.LastRewardTime){
                worker.WorkerConfig.Add(curRate, period);
        ...
        {
            int256 mdf = worker.WorkerConfig.ManagerDiscount()[0];
...
{
    function Workers(uint256 poolId) external view returns(address);
    function ShareTokens(uint256 poolId) external view returns(address);
    function Deposit(uint256 amount, uint256 poolId) external;
    function WithDraw(uint256 share, uint256 poolId) external;
    function ForcedWithdraw(uint256 share, uint256 poolId) external;
    function SetasideBaseToken(uint256 amount, uint256 poolId) external;
}
