...
contract OKGHeroes is ERC721Enumerable {
	...
	event GiveBirth(address indexed to, uint256[] tokenIDs);

	constructor(address _gov, string memory _uri)
		ERC721("Okeenga Heroes Collection", "OKGH")
	{
    ...
    */
	...
	/**
		...
		@param	_tokenIDs			A list of `tokenId` that will be minted to `_receiver`
    */
	function breed(address _to, uint256[] calldata _tokenIDs)
		external
		isLocked
		onlyMinter
	{
		uint256 _size = _tokenIDs.length;
		for (uint256 i; i < _size; i++) _safeMint(_to, _tokenIDs[i]);

		emit GiveBirth(_to, _tokenIDs);
	}
