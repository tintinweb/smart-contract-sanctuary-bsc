...
contract TomatoSoupExchangeTreasury is Ownable {
    ...
    modifier notPaused() {
        require(!paused, "TomatoSoupExchangeTreasury: paused");
        _;
    }
    ...
    modifier onlyOperator() {
        require(
            msg.sender == operator,
            "TomatoSoupExchangeTreasury: only operator can call"
        );
        _;
    }
    ...
    function _transfer(
        ...
        if (isETH) {
            require(
                ...
                "TomatoSoupExchangeTreasury: not enough ether balance"
            );
            require(
                to.send(amount),
                " TomatoSoupExchangeTreasury: ether transfer failed"
            );
        } else {
            ...
            require(
                balance >= amount,
                "TomatoSoupExchangeTreasury: not enough currency balance"
            );
    ...
    function claim(bytes calldata message, bytes calldata signature)
        ...
        notPaused
    {
        address sourceAddress = source(message, signature);
        require(
            sourceAddress == truthHolder,
            "TomatoSoupExchangeTreasury: only accept truthHolder signed message"
        );
        ...
        require(
            claimHistory[id] == 0,
            "TomatoSoupExchangeTreasury: already claimed"
        );
        require(
            isETH || supportCurrency[currency],
            "TomatoSoupExchangeTreasury: currency not support"
        );
        require(
            block.timestamp < deadline,
            "TomatoSoupExchangeTreasury: already passed deadline"
        );
...
library Address {
    ...
    {
        return payable(address(account));
    }
