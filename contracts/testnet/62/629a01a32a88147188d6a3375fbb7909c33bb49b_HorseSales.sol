...
contract HorseSales is Initializable, OwnableUpgradeable {
    ...
    //token id => address
    mapping(uint256 => address) private horseToOwner;
    //store minted unique names
    mapping(string => bool) private horseNames;
    // //address => token id
    mapping(address => uint256) private ownerToHorse;
    //check for address whitelist
    mapping(address => bool) private whitelistedAddresses;
    //address => ticketCount
    mapping(address => uint256) private ticketQty;
    //address => per address ticket limit 2
    mapping(address => uint256) private ticketAddressLimit;
    ...
    function isValidPurchase(uint256 _amount, uint256 quantity) internal view {
        ...
        require(
            ticketAddressLimit[msg.sender] + quantity <= 2,
    ...
    function buyTicket(uint256 _amount, uint256 quantity) external payable {
        ...
        isValidPurchase(_amount, quantity);
    ...
    function _breedHorse(
        ...
        horseToOwner[newItemId] = msg.sender;
        assignHorse(newItemId, horse);
    }
    ...
    function assignHorse(uint256 newItemId, HasHorse.Horse memory horse)
        internal
    {
        hasHorseContract.setHorse(
            newItemId,
            horse
            // horse.name,
            // horse.endurance,
