    ...
    payable
    {
        ...
        if (listing.isErc721) {
            if (
              ...
              /**
              We will keep this contract always payable as this will allow us to extend our marketplace easily to accept native asset as well
              Since this contract is payable, We have to revert the money if user has revoked the token approval.
              */
                if (msg.value > 0) {
                    Address.sendValue(payable(msg.sender), msg.value);
                }
        ...
        /**
        If a seller lists NFT in X token, buyer can't buy it in Y token :)
        Lame documentation.
        */
...
/**
Revisiting the documentation again :
* @param erc20Address in createListing - address of the ERC20 token, which will be used for the payment. If native asset is used, this should be 0x0 address
*/
    ...
    function cancelListing(string memory listingId) public virtual {
        ...
        /**
        cashbackSum is always zero since we are not using Tatum cashback ERC721 or ERC1155
        */
