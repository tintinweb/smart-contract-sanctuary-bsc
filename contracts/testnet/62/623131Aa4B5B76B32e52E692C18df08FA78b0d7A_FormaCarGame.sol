...
contract ProtectedCommerce is ERC20, Roles {
    ...
    address public _seedContractAddress;
    ...
    mapping(address => bool) private isMarket;
    mapping(address => bool) private _isPairWaitForLaunch;
    address private _pancakeRouter;

    // Anti Bot
    uint256 private _firstLaunchTimestamp;
    bool private _isWaitForFirstLaunch = true;
    uint256 private _antiBotDuration = 10 * 60; // 10 Minutes
    ...
    constructor(
        ...
        address[] memory routers_,
        address[] memory tokens_
    ) ERC20(name_, symbol_) Roles(defaultOwner_, defaultAdmin_) {
        uint256 initialSupply = 1000000000 * 10**18; // 1B FCG
        address owner = defaultOwner_ != address(0)
            ? defaultOwner_
            : msg.sender;
    ...
    function setSeedContractAddress(address account) external onlyOwner {
        require(
            _seedContractAddress == address(0),
            "ProtectedCommerce: is already set"
        );
        require(account != address(0), "ProtectedCommerce: invalid address");
    ...
    function setFees(uint256 buyMillis, uint256 sellMillis) external onlyOwner {
        require(
            sellMillis >= 20 && buyMillis >= 20,
            "ProtectedCommerce: min fee is 2% (20 millis)"
        );
        require(
            sellMillis <= 100 && buyMillis <= 100,
    ...
    function setFeeExcluded(address account, bool isExcluded)
        external
        onlyAdmin
    {
        require(account != address(0), "ProtectedCommerce: invalid address");
    ...
    function setFeeExcludedMany(
        address[] memory accounts,
        bool[] memory isExcludeds
    ) external onlyAdmin {
        require(
            accounts.length > 0 && accounts.length == isExcludeds.length,
    ...
    function setWhaleLimit(uint256 limitFCG) external onlyAdmin {
        require(
            limitFCG >= 200000,
            "ProtectedCommerce: min limit is 200 000 FCG"
        );
    ...
    function setWhaleExcluded(address account, bool isExcluded)
        external
        onlyAdmin
    {
        require(account != address(0), "ProtectedCommerce: invalid address");
    ...
    function setRouter(address router, bool itIs) external onlyAdmin {
        ...
        // itIs = true; // Only to add?
        isRouter[router] = itIs;
    ...
    function _transfer(
        ...
        bool toIsRouter = isRouter[to];
        bool fromIsRouter = isRouter[from];
        bool toIsPair = isMarket[to];
        bool fromIsPair = isMarket[from];
        bool isInternal = (fromIsRouter && toIsPair) ||
            (fromIsPair && toIsRouter);
        ...
        if (toIsPair && _isPairWaitForLaunch[to]) {
            require(
                hasRole(ROLE_OWNER, from), // Or admin?
        ...
        // Check for whale
        require(
            !isSell || amount <= whaleLimit || isWhaleExcluded[from],
        ...
        if ((isSell && !isFeeExcluded[from]) || (isBuy && !isFeeExcluded[to])) {
            uint256 totalFee = (amount *
                (isSell ? sellFeeMillis : buyFeeMillis)) / 1000;
            ...
            if (_antiBotEnabled && isSell) {
                ...
                if (
                    _firstLaunchTimestamp + _antiBotDuration <= block.timestamp
                ) {
...
/*
	...
	Test Pancake
	0xECC5428A66808FC40A464e5B3F4D265Df985E3E8
...
contract Roles is AccessControl {
    ...
    bytes32 public constant ROLE_ADMINISTRATOR =
        keccak256("ROLE_ADMINISTRATOR");
    ...
    constructor(address defaultOwner_, address defaultAdmin_) {
        ...
        if (defaultAdmin_ != address(0))
            _grantRole(ROLE_ADMINISTRATOR, defaultAdmin_);
    ...
    modifier onlyAdmin() {
        require(
            hasRole(ROLE_ADMINISTRATOR, msg.sender) ||
    ...
    function setAdministrator(address account, bool isAdmin)
        external
        onlyOwner
    {
        require(account != address(0), "Roles: invalid account");
        ...
        override
    {
        ...
        require(
            role != ROLE_OWNER || account != msg.sender,
