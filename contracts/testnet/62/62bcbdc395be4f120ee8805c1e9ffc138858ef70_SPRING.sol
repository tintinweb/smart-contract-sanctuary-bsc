...
abstract contract RewardsToken is uniSwapTool, ERC20 {
    ...
    uint256 internal rewardThreshold = 0.05 ether;
...
contract SPRING is Ownable, ERC20, RewardsToken, ReentrancyGuard, System {
    address addressDEAD = 0x000000000000000000000000000000000000dEaD;
    mapping(address => bool) private isExcludeFee;
    bool public inSwap;
    address returnAddress;//回流地址
    address ethAddress;//营销地址
    address liquidAddress;
    uint256 swapThreshold = 0.2 ether;
    uint256 divBase = 1e3;
    event DistributeRewardToken(address user, uint256 rewardAmount);
    constructor(address _router, address[] memory _reward, address[] memory addrs) ERC20("SPRING", "SPRING", 3333000000000000000000) {
        ...
        initAddrs(addrs);
        _mint(liquidAddress, 3648 ether);
        _mint(owner(), 3018 ether);
    ...
    function handRewardFees(uint256 amount) private {
        rewardPoolTotal += amount * 6 / divBase;//百分之6分红
    }
    function handSwap() private returns (bool) {
        ...
        if (WETHAmount * amountDesire / TOKENAmount >= swapThreshold) {
            ...
            payable(returnAddress).transfer(address(this).balance/2);//百分之2回流
            payable(ethAddress).transfer(address(this).balance/2);//百分之2营销
            return true;
        }
        return false;
    }
