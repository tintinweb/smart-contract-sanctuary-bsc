...
interface IGAME is Game {
    ...
    function invite(address _account) external view returns (address);
}
...
contract BCKGAME is Game, IERC721Receiver {
    ...
    uint256 public baseUpgradeTime = 120; //基础升级时间 3600
    uint256 public epoch = 300; //周期 86400
    uint256 public seedEffective = 10 * epoch; //种子有效期 40days
    uint256 public rewardRate = 3; //奖励比例 level*8+35
    uint256 public jfRate = 2; //奖励比例 level*5/10+1

    address public usdtAddress = 0x47A01F129b9c95E63a50a6aa6cBaFDD96bEb4C6F; //usdt合约
    ...
    mapping(uint256 => bool) public nftUpgrade;
    ...
    function setRewardRate(uint256 _rewardRate, uint256 _JFrate)
        public
        onlyOwner
    {
    ...
    function upgradeGame(uint256 _tokenId) public {
        ...
        GameInfo memory newGame = gameInfo[_tokenId];
        require(newGame.id == 0 && !newGame.status, "game is already started");

        // 升级游戏信息
        _upgradeGameInfo(_tokenId);

        GameInfo memory game = gameInfo[_tokenId];
        if (game.id > 0 && game.master != address(0) && game.status) {
            ...
            upgradeInvite(game.master);
            totalUsers += 1;
            userInfo[game.master] = _tokenId;
            emit StartGame(game.master, _tokenId);
        }
    }

    function _upgradeGameInfo(uint256 _tokenId) public {
        GameInfo storage newGame = gameInfo[_tokenId];
        if (newGame.id > 0 || nftUpgrade[_tokenId]) {
            nftUpgrade[_tokenId] = true;
            return;
        }
    ...
    function upgradeInvite(address _account) public returns (bool) {
        address inviter_ = IGAME(upgradeGameAddress).invite(_account);
        if (invite[_account] == address(0) && inviter_ != address(0)) {
            invite[_account] = inviter_;
            inviteCount[inviter_] += 1;
            emit BindInviter(_account, inviter_);
            return true;
        }
        return false;
    }
    ...
    function startGame(uint256 _tokenId) public onlyLandOwner(_tokenId) {
        ...
        _upgradeGameInfo(_tokenId);
        upgradeInvite(msg.sender);

        bool isTemp = INFT(nftAddress).nftTemp(_tokenId);
        uint256 nftPrice = INFT(nftAddress).nftPrice(_tokenId);
        if (game.id == 0 && !isTemp && nftPrice > 0) {
            address invite1 = invite[msg.sender];
            if (invite1 != address(0)) {
                uint256 reward1 = (nftPrice * 20) / 100;
                ...
                if (invite2 != address(0)) {
                    uint256 reward2 = (nftPrice * 10) / 100;
                    TransferHelper.safeTransferFrom(
                        usdtAddress,
                        depositAddress,
                        invite2,
    ...
    // 偷菜
    function stealAll(uint256 _tokenId) public {
        ...
        for (uint8 i = 1; i <= game.lands; i++) {
            uint256 reward = rewardAmount(_tokenId, i);
            LandInfo storage land = landInfo[_tokenId][i];
            uint256 stealDay = stealInfo[myToken][_tokenId][i];
            if (reward > 0 && land.steal < 2 && land.nextHarvestAt > stealDay) {
                stealInfo[myToken][_tokenId][i] = block.timestamp;
                land.steal += 1;
                reward = (reward * 5) / 100;
                ...
                emit Steal(msg.sender, game.master, i, reward);
            }
        }
    }
    ...
    function setInviter(address inviter_) external virtual returns (bool) {
        bool isUpgrade = upgradeInvite(msg.sender);
        if (!isUpgrade) {
