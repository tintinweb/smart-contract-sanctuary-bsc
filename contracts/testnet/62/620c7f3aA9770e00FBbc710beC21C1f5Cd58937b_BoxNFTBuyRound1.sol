...
contract BoxNFTBuyRound1 is OwnableUpgradeable, PausableUpgradeable  {
    ...
    function initialize() public initializer {
        ...
        priceFlashSales[1]=38911;       // D Starter Box
        priceFlashSales[2]=129702;       // C Bronze Box
        priceFlashSales[3]=259403;       // B Silver Box
        priceFlashSales[4]=1000000000;          // A Golden Box
        priceFlashSales[5]=1000000000;         // S Diamond Box
        priceFlashSales[6]=1000000000;         // SS Legendary Box
        priceFlashSales[7]=1000000000;        // SSS Dragon Box
        priceFlashSales[8]=1000000000;        // SSSS Mystic Box

        priceSales[1]=46693;       // D Starter Box
        priceSales[2]=155642;       // C Bronze Box
        priceSales[3]=311284;       // B Silver Box
        priceSales[4]=1000000000;          // A Golden Box
        priceSales[5]=1000000000;         // S Diamond Box
        priceSales[6]=1000000000;         // SS Legendary Box
        priceSales[7]=1000000000;        // SSS Dragon Box
        priceSales[8]=1000000000;        // SSSS Mystic Box
     ...
     */
    function initByOwner(BoxNFT _boxNFT,StakingPool _stakingPool, IBEP20 _bep20,uint256 _startTime, uint256 _endTime,uint256 _startFlashSaleTime) public onlyOwner {
...
function buyBox(uint256 rank, uint256 _bep20) external  whenNotPaused returns (uint256) {
        uint256 current=block.timestamp;
        uint256 newBoxFlashSaleId=0;
        uint256 newBoxId=0;
        ...
        if(startFlashSaleTime <=current && current <= startTime){
          require(indexOfWhitelist[msg.sender] != 0,'You can not buy FlashSale');
          require(_bep20==priceFlashSales[rank],'Not equal priceFlashSales to buy');
          ...
          newBoxFlashSaleId = _totalBuyFlashSale.current();
          flashSaleBoxes[msg.sender][newBoxFlashSaleId] =Box(newBoxFlashSaleId,rank);
        ...
        // Normal Sale 
          ...
          require(_bep20==priceSales[rank],'Not equal priceSales to buy');
          require(_buyByRanks[rank].current()<=limits[rank]-1,'Not buy box, because maximum boxes are selled');
        ...
        bep20.approve(address(this), _bep20*10**18);
        bep20.transferFrom(msg.sender, address(this), _bep20*10**18);
