...
contract RidersFactory is VRFConsumerBaseV2, Ownable  
{
  ...
  function mint() external {
    ...
    for(uint i = 0; i < _randomWordsLength; i += 4) {
      ...
      (string memory currentBaseURI, string memory currentImageURI) = radikalStoreInstance.getRidersURIs(attributes.pizzaQuantity,_getTokenBaseURIIndex( (_randomWords[i + 1] % 1000) + 1 , radikalStoreInstance.getRidersBaseURIProbability(attributes.pizzaQuantity)));
      ...
      _mint(msg.sender, (_randomWords[i + 3]), motorBike, _tokenURI, _imageURI, attributes);      
    }
  ...
  function promotionalMint(uint _mintQuantity) external onlyOwner {
    ...
    IRadikalStore.RiderAttributes memory attributes  = radikalStoreInstance.getRiderAttributes(rangeResult);
    // Get the IPFs folder for the NFT being minted using the second random
    (string memory currentBaseURI, string memory currentImageURI) = radikalStoreInstance.getRidersURIs(attributes.pizzaQuantity, _getTokenBaseURIIndex( (uint(keccak256(abi.encodePacked(i,block.timestamp, 'Second Random'))) % 1000) + 1 , radikalStoreInstance.getRidersBaseURIProbability(attributes.pizzaQuantity)));
    // Gets the final IPFs URI of the image and the motorbike of the rider using the third random
    ...
    ridersInstance.mint(msg.sender, _tokenURI, _imageURI, riderAttributes); 
    }
  ...
  function _mint(address user, uint _randonNumber3, bytes12 _motorBike, string memory _tokenURI, string memory _imageURI, IRadikalStore.RiderAttributes memory attributes) internal {
    ...
    // Information is ready and sent to mint the NFT in the CollectionRiders contract
    ridersInstance.mint(user, _tokenURI, _imageURI, riderAttributes); 
  }
...
interface IRadikalStore {
	...
	function getRidersURIs(uint8 _nftRareness, uint _imageRareness) external view returns (string memory, string memory);
...
interface ICollectionRiders {
  ...
  function mint(address user, string memory _tokenURI, string memory _imageURI, RidersAttributes memory attributes) external;
