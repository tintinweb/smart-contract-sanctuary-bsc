...
contract SnailTravelers is Initializable, ERC721Upgradeable {
  ...
  function initialize(
    ...
    __ERC721_init("SnailTravelerss", "SNLT");
  ...
  modifier ensureAvailabilityFor() {
    require(1 + totalSupply <= maxSupply, "SOLD_OUT");
    _;
  }

  modifier validateEthPayment(uint256 price) {
    require(1 * price == msg.value, "WRONG_AMOUNT_PAYED");
    _;
  }

  modifier ensureMaxPerWallet() {
    require(
      1 + _walletMints[msg.sender] <= maxPerWallet,
  ...
  function safeMint(address sender_) internal {
    uint256 newTokenId = totalSupply + 1;
    totalSupply++;
    _safeMint(sender_, newTokenId);
    _walletMints[sender_] = 1;

    emit Minted(sender_, newTokenId, block.number);
  }
  ...
  function mintFromBreedSale(
    ...
    uint256 tokenId,
    uint256 price
  ) external payable validateEthPayment(price) {
    bytes32 digest = keccak256(
      abi.encode(CouponType.BreedSale, tokenId, price, parent1, parent2)
    );
    require(_isVerifiedCoupon(digest, coupon), "INVALID_COUPON");

    _safeMint(msg.sender, tokenId);
    emit Breeded(msg.sender, tokenId, parent1, parent2, price);

    uint256 parentsCut = (msg.value * 45) / 100;

    parent1.transfer(parentsCut);
    parent2.transfer(parentsCut);
  }
  ...
  function withdraw() external onlyOwner {
    ...
    _owner.transfer((balance * 90) / 100);
    _rewards.transfer((balance * 10) / 100);
  }
}