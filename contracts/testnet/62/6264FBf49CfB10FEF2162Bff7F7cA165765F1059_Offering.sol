...
contract Offering is IOffering, Caller {
    ...
    function makeOffering(
        ...
        require(0 < pledgeStart && pledgeStart < payStart && payStart < lockStart && lockStart < lockEnd, "111");
        require(address(0) != offeringToken && 0 < offeringAmount, "222");
        require(0 < pledgeLimit, "333");
    ...
    function pledge(uint256 offeringId, uint256 amount) external override {
        //判断状态
        Offering storage offering = offeringInfo[offeringId];
        ...
        //增加个人质押数量和总数量
        pledgeAmount[offeringId][msg.sender] = personPledgedAmount.add(amount, "1123");
    ...
    function unPledge(uint256 offeringId, uint256 unPledgedAmount) external override {
        //判断状态 质押期和锁仓期都可以解押
        Offering storage offering = offeringInfo[offeringId];
        ...
        //减去个人质押数量和总数量
        pledgeAmount[offeringId][msg.sender] = personPledgedAmount.sub(unPledgedAmount, "1123");
        offering.pledgedAmount = offering.pledgedAmount.sub(unPledgedAmount, "1123");
    ...
    //手动领取
    function receiveToken(uint256 offeringId) external override {
        //判断状态
        Offering storage offering = offeringInfo[offeringId];
        require(3 == offering.state, "1120");


        (uint256 personOfferingAmount, uint256 surplusPladgeAmount) = _getOfferingTokenAmount(offeringId, offering, msg.sender);
        //转账打新代币
        TransferV1.transferFromFlush(
            config.getApproveProxy(),
    ...
    //自动领取
    function autoReceiveToken(uint256 offeringId) external override {
        //判断状态
        Offering storage offering = offeringInfo[offeringId];
        require(3 == offering.state, "1120");
        ...
        for (uint8 i = 0; i < currentPledgeAddress.length; i++) {
            ...
            (personOfferingAmount, surplusPladgeAmount) = _getOfferingTokenAmount(offeringId, offering, add);
            //转账打新代币
            TransferV1.transferFromFlush(
                config.getApproveProxy(),
                offering.offeringToken,
    ...
    function _getOfferingTokenAmount(uint256 offeringId, Offering memory offering, address account) private returns (uint256, uint256){
        uint256 personOfferingAmount;
        uint256 surplusPladgeAmount;
        ...
        if(offering.expectAmount > offering.pledgedAmount) {
            //按照兑换比例计算
            personOfferingAmount = personPledgedAmount.mul(offering.exchangeRate, "");
            surplusPladgeAmount = 0;
        }else {
            //按照质押占比计算
            uint256 a = personPledgedAmount.mul(offering.offeringAmount, "");
            personOfferingAmount = a.div(offering.pledgedAmount, "");//个人得到的打新币数量

            uint256 c = personOfferingAmount.div(offering.exchangeRate, "");
            surplusPladgeAmount = personPledgedAmount - c;
    ...
    function getOffering(uint256 offeringId, address account) external view override returns (uint256[] memory, uint8, uint128, uint128, uint128, uint128){
        Offering storage offering = offeringInfo[offeringId];
        ...
        return (amountArray, offering.state, offering.pledgeStart, offering.payStart, offering.lockStart, offering.lockEnd);
    }

    function getOfferingV2(uint256 offeringId, address account) external view override returns (uint256, Offering memory){
        Offering memory offering = offeringInfo[offeringId];
        uint256 personPledgeAmount = pledgeAmount[offeringId][account];
    
        return (personPledgeAmount, offering);
    }
}
...
interface IOffering {
    ...
    function pledge(uint256 offeringId, uint256 amount) external;

    function unPledge(uint256 offeringId, uint256 unPledgedAmount) external;

    function receiveToken(uint256 offeringId) external;

    function autoReceiveToken(uint256 offeringId) external;

    function getOffering(uint256 offeringId, address account) external returns (uint256[] memory, uint8, uint128, uint128, uint128, uint128);
    
    function getOfferingV2(uint256 offeringId, address account) external returns (uint256 personPledgeAmount, Offering memory);
}
