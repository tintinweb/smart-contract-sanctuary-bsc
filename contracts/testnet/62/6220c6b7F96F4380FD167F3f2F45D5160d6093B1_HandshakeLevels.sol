...
contract HandshakeLevels is Ownable {
    ...
    constructor(address _dexGoNFT) {
        dexGoNFT = _dexGoNFT;
        percentPerLevelWei[0] = 0.2 ether;
        percentPerLevelWei[1] = 0.06 ether;
        percentPerLevelWei[2] = 0.06 ether;
        percentPerLevelWei[3] = 0.06 ether;
        percentPerLevelWei[4] = 0.06 ether;
        percentPerLevelWei[5] = 0.06 ether;
    }

    function digitsFrom(uint number) public view returns (uint8[] memory){
        uint returnNum = number;
        uint returnNumForFill = number;
        uint countDigits;
        while (returnNum > 0) {
            uint8 digit = uint8(returnNum % 10);
            returnNum = returnNum / 10;
            countDigits++;
        }

        uint8[] memory digits =  new uint8 [](countDigits);
        countDigits = 0;
        while (returnNumForFill > 0) {
            uint8 digit = uint8(returnNumForFill % 10);
            returnNumForFill = returnNumForFill / 10;
            digits[countDigits] = digit;
            countDigits++;
        }
    ...
    function setHandshake(address wallet, address referrer) public returns (uint,uint, bool, uint) {
        require(msg.sender == dexGoNFT || msg.sender == owner(),'HandshakeLevels: unauthorized sender');
    ...
    function getAllReferralsLine(address wallet) public view returns (address[] memory, uint) {
        address[] memory levels =  new address [](maxLevels);
        uint countLevels;
