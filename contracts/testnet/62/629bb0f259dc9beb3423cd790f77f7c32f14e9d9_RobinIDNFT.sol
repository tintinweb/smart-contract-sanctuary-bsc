    ...
    RobinNFTStorageV1
{
    ...
    function buyID(address buyer) public payable returns (uint256) {
        // 验资
        require(buyPrice > 0, "price NOT set");
        if (buyToken != IERC20(address(0))) {
            buyToken.transferFrom(_msgSender(), address(this), buyPrice);
        } else {
            require(msg.value >= buyPrice, "not enough msg.value to buy");
        }
    ...
    function buyIDAdmin(address buyer) external payable returns (uint256) {
        require(msg.sender == owner() || msg.sender == admin, "no auth");
        return _buyID(buyer);
    }

    function _buyID(address buyer) private returns (uint256) {
        ...
        emit BuyID(buyer, tokenId, address(buyToken), buyPrice);
        return tokenId;
    }
    ...
    function claimRSG(uint256[] memory ids) public {
        ...
        rsg.mint(_msgSender(), rsgClaim.bonus);
    }
    ...
    function setProperty(uint256 tokenId, uint256 property, uint256 times) public onlyOwner {
    ...
    function mint(uint256 level) public onlyOwner returns (uint256) {
        uint256 tokenId = _incTokenId();
    ...
    /// @notice mintMany mint many NFT token to msg.sender.
    /// @param to The owner mint for.
    /// @param from the start mint tokenId (included)
    /// @param end the end mint tokenId (not included)
    /// @param level the level of the minted tokens
    function mintMany(address to, uint from, uint end, uint level) external onlyOwner {
      for (uint i = from; i < end; i++) {
        _mint(to, i);
        attributesOf[i].level = level;
        attributesOf[i].ownedAt = uint64(block.timestamp);
        setNFTCreator(i, to);
      }
      nextTokenId = end;
    }
    ...
    /// @param resultLevel the minted token level
    ...
    function _synthesize(uint256[] memory from,
        ...
        require(propsKey.length == propsCount.length, "props should equal");
        require(rsg.balanceOf(sender) >= rsgAmt, "not enough RSG");
        require(rsg.balanceOf(sender) >= rsgcAmt, "not enough RSGC");
        ...
        uint256 tokenId = mint(resultLevel);
        NFTAttributes storage attr = attributesOf[tokenId];
