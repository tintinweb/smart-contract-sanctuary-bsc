...
contract ProviderToken is ERC20, Ownable {
    ...
    function mintTokens(uint256 _amount, address _wallet) external onlyOwner {
        // Calculate sqrt(_amount)
        uint256 tokensToMint = sqrt(_amount);

        // Mint tokens to msg.sender
        _mint(_wallet, tokensToMint);
    }
...
contract BabyAptosStakingPool is Ownable, ReentrancyGuard {
    ...
    // Track NFT boost
    mapping(address => uint256) nftBoost;

    // Track balance by address
    mapping(address => uint256) private stakedBalances;
    mapping(address => uint256) private boostedBalances;
    mapping(address => uint256) private earnedAmount;

    struct NFTMetadata {
        EnumerableSet.AddressSet stakedTiers;
        mapping(address => uint256) tokenIds;
        uint256 totalBoostRate;
    }

    // Track staked NFTS by user
    mapping(address => NFTMetadata) private stakedNFTs;
    ...
    */
    ...
    //     // TODO: Get All Staked NFTs for wallet

    //     // iterate through nfts
    //     // rewards = (nft_rate ** 18) * blocksPassed / 28800
    ...
    function getBoostRateForUser(address _user) external view returns(uint256) {
        return stakedNFTs[_user].totalBoostRate;
    }

    function getRateForNFT(address _contract) external view returns(uint256) {
        return (nftBoost[_contract]);
    }
    ...
    function addAcceptedNFT(address _contract, uint256 _stakingBoost) external onlyOwner {
        // Record metadata and NFT contract
        nftBoost[_contract] = _stakingBoost;
    ...
    function stakeNFT(address _contract, uint256 _tokenId) external {
        ...
        // Check user NFTs
        require(!stakedNFTs[msg.sender].stakedTiers.contains(_contract), "Already staked NFT tier!");

        // Transfer NFT
        IERC721(_contract).transferFrom(msg.sender, address(this), _tokenId);

        // Set boost to 100 for first time
        if (stakedNFTs[msg.sender].stakedTiers.length() == 0) {
            stakedNFTs[msg.sender].totalBoostRate = 100;
        }

        stakedNFTs[msg.sender].stakedTiers.add(_contract);
        stakedNFTs[msg.sender].tokenIds[_contract] = _tokenId;
        stakedNFTs[msg.sender].totalBoostRate = stakedNFTs[msg.sender].totalBoostRate.add(nftBoost[_contract]);
    }

    function withdrawNFT(address _contract) external {
        require(stakedNFTs[msg.sender].stakedTiers.contains(_contract), "No staked NFT!");

        // Transfer NFT
        IERC721(_contract).transferFrom(address(this), msg.sender, stakedNFTs[msg.sender].tokenIds[_contract]);

        stakedNFTs[msg.sender].stakedTiers.remove(_contract);

        stakedNFTs[msg.sender].totalBoostRate = stakedNFTs[msg.sender].totalBoostRate.sub(nftBoost[_contract]);
    }
    ...
    */
    ...
    function depositStake(uint256 _amount) external payable nonReentrant {
        ...
        // Mint providerTokens to msg.sender
        pBAPT.mintTokens(stakedTokens, msg.sender);
    }
    ...
    function withdrawStake(uint256 _amount) external nonReentrant {
        ...
        uint256 pBAPTValue = IERC20(tokenToBuy).balanceOf(address(this)).mul(_amount).div(pBAPT.totalSupply());
