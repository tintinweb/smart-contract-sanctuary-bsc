...
contract Upgradable {
    ...
    struct PoolInfo {
        ...
        uint256 typePool; // 0: pool tokenReward, 1: pool tokenReward and ERC721Reward, 2: pool tokenReward and ERC1155Reward
        uint256 lastPoolUpdateTime; // when call poolUpdate and changes initialFund
        uint256 lastInitialFund; // last Initial Fund
    }
}
...
contract StakingPool is Upgradable {
    ...
    */
    function stakeToken(
        ...
        // Update reward
        if(pool.lastPoolUpdateTime < pool.lastUpdateTime){
   ...
   */
    ...
    */
    function rewardPerToken(string memory poolId) public view returns (uint256) {
        ...
        if(pool.lastPoolUpdateTime < pool.lastUpdateTime){
            rewardPool = pool.initialFund * (currentTimestamp - pool.lastUpdateTime) * 1e20;
        } else {
            rewardPool = (pool.lastInitialFund * (pool.lastPoolUpdateTime - pool.lastUpdateTime)+
            pool.initialFund * (currentTimestamp - pool.lastPoolUpdateTime)) * 1e20;
        }
    ...
    */
    function claimReward721NFT (
        ...
        poolExist(poolId)
    {
        require(verify(addrs[0], addrs[1], addrs[2], uints[0], uints[1], signature), "Dont have NFT reward");
        ERC721(addrs[2]).safeTransferFrom(address(this), addrs[1], uints[0]);
        emit ClaimRewardNFT( addrs[2],address(this), addrs[1], uints[0], uints[1], poolId);
    }
    ...
    */
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_to, _tokenAddress, _tokenId, _amount));
    }
    ...
    */
    function verify(
        ...
        bytes32 messageHash = getMessageHash(_to, _tokenAddress, _tokenId, _amount);
   ...
   */
    function createPool(string[] memory strs, address[] memory addr, uint256[] memory data, uint256[] memory configs) external onlyAdmins {
        ...
        PoolInfo memory pool = PoolInfo(addr[0], addr[1], 0, 0, data[0], data[0], 0, 0, 0, 1, configs, typePool,configs[1],data[0]);
   ...
   */
        ...
        poolExist(strs[0])
    {
        ...
        if (newConfigs[2] != 0) {
            ...
            pool.lastPoolUpdateTime = block.timestamp;
            pool.lastInitialFund = pool.initialFund;
    ...
    /**
     * @dev Stop Pool, user cant stake anymore
     * @param poolId: poolID
    */
    function stopPool(string memory poolId) 
        external 
        onlyAdmins
        poolExist(poolId)
    {
     ...
     * @param strs: poolID
    */
    function updateMaxTVL(string[] memory strs) external onlyAdmins {
        for ( uint256 i = 0; i < strs.length; i++){
            string memory poolId = strs[i];
            PoolInfo storage pool = poolInfo[poolId];
    ...
    */
    function withDrawNFT721(address _tokenAddress, address _account, uint256 tokenId, string memory poolId)
        external
        onlyAdmins
        poolExist(poolId)
    {
        ...
        emit ClaimRewardNFT( _tokenAddress,address(this), _account, tokenId, 1, poolId);
    }
    ...
    */
    function withDrawNFT1155(address _tokenAddress, address _account, uint256 tokenId, uint256 amount, string memory poolId)
        external
        onlyAdmins
        poolExist(poolId)
    {
        ...
        emit ClaimRewardNFT( _tokenAddress,address(this), _account, tokenId, amount, poolId);
    }
     ...
     * @param tokenId: token ID
    */
    function depositNFT721(address _tokenAddress, uint256 tokenId, string memory poolId)
        external
        onlyAdmins
    {
        ERC721(_tokenAddress).safeTransferFrom(msg.sender, address(this), tokenId);

        emit ClaimRewardNFT( _tokenAddress,msg.sender,address(this), tokenId, 1, poolId);
    }
     ...
     * @param amount: amount
    */
    function depositNFT1155(address _tokenAddress, uint256 tokenId, uint256 amount, string memory poolId)
        external
        onlyAdmins
    {
        ERC1155(_tokenAddress).safeTransferFrom(msg.sender, address(this), tokenId, amount, "");

        emit ClaimRewardNFT( _tokenAddress,msg.sender, address(this), tokenId, amount, poolId);
    }
