...
contract AlchemyV1 is Ownable {
    ...
    function updateInterfaces() external {
        Del = IDelegate(gameAddresses.getAddressOf(AddressesInit.Addresses.DELEGATE));
        Zai = IZaiMeta(gameAddresses.getAddressOf(AddressesInit.Addresses.ZAI_META));
        Potions = IPotions(gameAddresses.getAddressOf(AddressesInit.Addresses.POTIONS_NFT));
        Chicken = IChicken(gameAddresses.getAddressOf(AddressesInit.Addresses.CHICKEN));
        Oracle = IOracle(gameAddresses.getAddressOf(AddressesInit.Addresses.ORACLE));
        emit InterfacesUpdated(address(Del), address(Zai), address(Potions), address(Chicken), address(Oracle));
    }
    ...
    function useAlchemy(
        ...
        if (_manaAdditional > 1000) {
            ...
            ((z.level / 10) + 1);
            if(_isManaMix){ // UPDATE AUDIT: fix by deleting "!"
