...
contract DripMaxi is ERC20, Ownable {
    ...
    uint256 public buybackFeeOnBuy   = 2;
    uint256 public buybackFeeOnSell  = 2;

    uint256 private _totalFeesOnBuy  = 5;
    uint256 private _totalFeesOnSell = 5;

    uint256 public walletToWalletTransferFee = 10;

    address public treasuryWallet = 0x3Ca8693CfccE2555535475ac86DC2e3721BE547f;
    ...
    address public rewardToken = 0x802050B036Fa8384521a40EB36E5ACfCE269D992; //! 0x20f663CEa80FaCE82ACDFA3aAE6862d246cE0333
    ...
    event UpdateBuyFees(uint256 rewardFeeOnBuy, uint256 treasuryFeeOnBuy, uint256 buybackFeeOnBuy);
    event UpdateSellFees(uint256 rewardFeeOnSell, uint256 treasuryFeeOnSell, uint256 buybackFeeOnSell);
    ...
    constructor() ERC20("DripMaxi", "$DRIPMAXI") 
    {   
        //transferOwnership(0xB6ad74094A9EB3c085169c9db53A64400769a9D3);

        _mint(owner(), 500_000 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 5000;

    	dividendTracker = new DividendTracker(500_000 / 50_000, rewardToken);
        ...
        pinkAntiBot = IPinkAntiBot(0xbb06F5C7689eA93d9DeACCf4aF8546C4Fe0Bf1E5); //0x8EFDb3b642eb2a20607ffe0A56CFefF6a95Df002
        ...
        bnbValueForBuyBurn = 3e10; //! 0.3 BNB
    ...
    function updateBuyFees(uint256 _rewardFeeOnBuy, uint256 _treasuryFeeOnBuy, uint256 _buybackFeeOnBuy) external onlyOwner {
        require(
            _rewardFeeOnBuy + _treasuryFeeOnBuy + _buybackFeeOnBuy <= 10,
        ...
        _totalFeesOnBuy  = rewardFeeOnBuy + treasuryFeeOnBuy + buybackFeeOnBuy ;

        emit UpdateBuyFees(rewardFeeOnBuy, treasuryFeeOnBuy, buybackFeeOnBuy);
    }

    function updateSellFees(uint256 _rewardFeeOnSell, uint256 _treasuryFeeOnSell, uint256 _buybackFeeOnSell) external onlyOwner {
        require(
            _rewardFeeOnSell + _treasuryFeeOnSell + _buybackFeeOnSell <= 10,
            "Fees cannot be more than 10%"
        );
        ...
        _totalFeesOnSell  = rewardFeeOnSell + treasuryFeeOnSell + buybackFeeOnSell;

        emit UpdateSellFees(rewardFeeOnSell, treasuryFeeOnSell, buybackFeeOnSell);
    }

    function updateWalletToWalletTransferFee(uint256 _walletToWalletTransferFee) external onlyOwner {
        require(_walletToWalletTransferFee <= 10, "Fees cannot be more than 10%");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 totalShares   = _totalFeesOnBuy + _totalFeesOnSell;
            ...
            if(treasuryShare > 0) {
                uint256 treasuryBNB = newBalance * treasuryShare / totalShares;
                payable(treasuryWallet).sendValue(treasuryBNB);
                emit SendTreasury(treasuryBNB);
            }

            if (buybackShare > 0) {
                uint256 buybackBNB = (newBalance * buybackShare) / totalShares;
            ...
            if(rewardShare > 0) {
                uint256 rewardBNB = newBalance * rewardShare / totalShares;
