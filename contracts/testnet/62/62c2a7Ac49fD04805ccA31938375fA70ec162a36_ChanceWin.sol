...
contract ChanceWin is Ownable, Pausable, ReentrancyGuard{
    ...
    event winnerWithdrawListLog(address indexed _Chance, address _addrWithdraw, uint256 amount, uint256[] tokenIDs);
    ...
    function withdrawByWinnerList(uint256[] calldata rs, address payable _to) public checkWinList(rs) whenNotPaused nonReentrant {
        ...
        (bool success, ) = _to.call{value: amount}("");
        require(success, "FailedToSend");
        emit winnerWithdrawListLog(address(this), msg.sender, amount, w_id);
    }
}
