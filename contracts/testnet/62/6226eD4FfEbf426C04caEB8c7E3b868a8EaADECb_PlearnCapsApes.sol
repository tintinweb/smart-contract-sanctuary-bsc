...
contract PlearnCapsApes is ERC721Enumerable, ERC721Pausable, ERC721Burnable, Ownable {
    ...
    constructor(uint256 maxSupply_) ERC721("Plearn Caps Apes", "PLNCAPAPE") {
    ...
    function setRound(ROUND round_) external onlyOwner {
        round = round_;
        emit RoundChanged(round_);
    }

    function setWhitelisterAddress(address addr) external onlyOwner {
        whitelisterAddress = addr;
    }

    function claimNFT() external returns (uint256 tokenId) {
        require(
            IWhitelist(whitelisterAddress).isWhitelist(msg.sender),
            "Plearn NFT: You need to be whitelisted to claim NFT"
        );
        require(round == ROUND.Private, "Plearn NFT: Round is not private round");
        require(!IWhitelist(whitelisterAddress).isClaimed(msg.sender), "Plearn NFT: You already claimed a NFT");
        require(totalSupply() < maxSupply, "Plearn NFT: Total supply reached");

        IWhitelist(whitelisterAddress).setClaimed(msg.sender);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        //TODO("For testnet")
        return "https://firebasestorage.googleapis.com/v0/b/nfts-9f99b.appspot.com/o/plearn-nft-airdrop%2Fjsons%2Fcap-ape.json?alt=media&token=a57068f0-53b4-418d-b3a6-58e2124b8999";
    }
...
interface IWhitelist {
    ...
    function isClaimed(address addr) external returns (bool);

    function setClaimed(address addr) external;
}
