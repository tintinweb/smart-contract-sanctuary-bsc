    ...
    IMarketplace
{
    ...
    function initialize(IRoleManager roleManager) external initializer {
        __EIP712_init_unchained("Marketplace", "1.0.0");
        __ERC721Holder_init();
        __ERC1155Holder_init();
        __ReentrancyGuard_init();
        __Marketplace_init_unchained(roleManager);
    }

    function __Marketplace_init_unchained(IRoleManager roleManager) internal onlyInitializing {
    ...
    function getCurrencyType(address currencyAddress) external view returns (bytes4) {
        return _availableCurrencies[currencyAddress];
    }
    ...
    function _validateSignature(bytes32 hash, bytes calldata signature) private view {
        ...
        require(_roleManager.accountHasRole(signer, LibRole.MARKETPLACE_PERMIT_SIGNER_ROLE), "Invalid signature");
    }
    ...
    function _payOffLastBidder(
        ...
        if (highestBid > 0 && highestBidder != address(0)) {
            _transfer(highestBidder, highestBid, currency, currencyType);
        }
    }

    function _transferMarketplaceFee(
        uint256 totalAmount,
        address currency,
        bytes4 currencyType
    ) private returns (uint256) {
        //        uint256 fee = (totalAmount * _marketplaceFee.value) / LibShare.SHARE_DIVISOR;
        //        _transfer(payable(_marketplaceFee.account), fee, currency, currencyType);
        //        return fee;
        return 0;
    }

    function _transferFromMarketplaceFee(
        address from,
        uint256 totalAmount,
        address currency,
        bytes4 currencyType
    ...
    function _transferRoyalties(
        ...
        if (collection.supportsInterface(type(IRoyalties).interfaceId)) {
            ...
            for (uint256 i = 0; i < royalties.length; i++) {
                ...
                _transfer(payable(royalties[i].account), value, currency, currencyType);
                transferred += value;
            }
        }
        return transferred;
    ...
    function _transferFromRoyalties(
        address from,
        address collection,
        uint256 tokenId,
        uint256 totalAmount,
    ...
    function _transferWithFees(
        ...
        bytes4 currencyType = _availableCurrencies[currency];
        uint256 transferred = _transferMarketplaceFee(totalAmount, currency, currencyType);
        transferred += _transferRoyalties(collection, tokenId, totalAmount, currency, currencyType);
        _transfer(recipient, totalAmount - transferred, currency, currencyType);
    }
    ...
    function _transferFromWithFees(
        ...
        bytes4 currencyType = _availableCurrencies[currency];
        uint256 transferred = _transferFromMarketplaceFee(from, totalAmount, currency, currencyType);
        transferred += _transferFromRoyalties(from, collection, tokenId, totalAmount, currency, currencyType);
