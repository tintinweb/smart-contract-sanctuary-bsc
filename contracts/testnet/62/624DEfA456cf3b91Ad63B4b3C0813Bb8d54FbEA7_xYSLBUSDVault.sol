 ...
 */
contract xYSLBUSDVault is AccessControl, Initializable {
    ...
    address public xYSLBUSD_A;
    IUniswapV2Pair xYSLBUSD;
    IAdmin public Admin;
    ...
    @param _xYSLBUSD, address of Pair.
    @param _router, router address.
    Admin.masterNTT(); address of Master NTT.
    Admin.xYSL(); address of xYSL token.
    ...
    function initialize(
        ...
        Admin = IAdmin(_admin); 
        xYSLBUSD = IUniswapV2Pair(_xYSLBUSD);
        startTime = block.timestamp;
        router = _router;
        xYSLBUSD_A = Clones.clone(Admin.masterNTT());
        INTT(xYSLBUSD_A).initialize(address(this), "xYSL-BUSD-A", "xYSL-BUSD-A");
    ...
    function deposit(address user, uint amountLp) external {
        ...
        INTT(xYSLBUSD_A).mint(address(this),(amountLp * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
        exchangeRatio = (IERC20(address(xYSLBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(xYSLBUSD_A).totalSupply());
    ...
    function withdraw(address user,bool isReciept, uint _amount) external{
        ...
        if(isReciept){
            IERC20(xYSLBUSD_A).safeTransfer(user,_amount);
        }
        else{
            uint balance = (_amount * exchangeRatio)/10 ** 18;
            IERC20(address(xYSLBUSD)).safeTransfer(user,balance);        
            INTT(xYSLBUSD_A).burn(address(this),balance);
        }
        UserDeposit[user] -= _amount;
        exchangeRatio = (IERC20(address(xYSLBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(xYSLBUSD_A).totalSupply());
    }
    /**
    @dev claimReward Function used for getting Reward.
    */
    function claimReward() external{
        require(UserDeposit[msg.sender] > 0, "xYSLBUSDVault: Deposit first");
        uint balance = (UserDeposit[msg.sender] * exchangeRatio)/(IERC20(address(xYSLBUSD)).balanceOf(address(this)) * 10 ** 18);
        IERC20(Admin.xYSL()).safeTransfer(msg.sender, balance);
    }
    /**
    ...
    */
    function purchase(address user, uint amount, uint minAmount) external returns(uint){
        IERC20(address(xYSLBUSD)).safeTransferFrom(user, address(this),amount);
        uint balance = purchaseOf(amount);
        require(balance > minAmount, "xYSLBUSDVault: Amount is less than the minAmount");
        INTT(xYSLBUSD_A).mint(user, balance);
        exchangeRatio = (IERC20(address(xYSLBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(xYSLBUSD_A).totalSupply());
        return(balance);
    }
    /**
    ...
    */
    function sell(address user, uint amount, uint minAmount) external returns(uint){
        INTT(xYSLBUSD_A).burn(user,amount);
