...
contract SpaceWorms is ERC721, Pausable, Ownable {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://spaceworms.app/nftdata/";
    }
    ...
    function safeMint(uint256 _amount) external whenNotPaused returns(uint256) {
    ...
    function safeMintNoCost() external onlyOwner returns(uint256) {
        _tokenIdCounter.increment();
        uint256 nftId = _tokenIdCounter.current();
        _safeMint(msg.sender, nftId);
        return _tokenIdCounter.current();
        ...
        override
    {
        ...
        require(transferable || (!transferable && from == owner()), "Transfers are paused");
