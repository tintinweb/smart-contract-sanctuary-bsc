...
// File: contracts/interfaces/IPbexLottery.sol

pragma solidity ^0.8.4;

interface IPbexLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in PBEX per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in PBEX token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInPbex: price of a ticket in PBEX
...
// File: contracts/PbexLottery.sol

pragma solidity 0.8.15;
pragma abicoder v2;

/** @title PbexSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PbexLottery is ReentrancyGuard, IPbexLottery, Ownable {
    ...
    uint256 public maxPriceTicketInPbex = 50 ether;
    uint256 public minPriceTicketInPbex = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInPbex;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] pbexPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInPbex;
        uint32 finalNumber;
    }
     ...
     * @param _pbexTokenAddress: address of the PBEX token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _pbexTokenAddress, address _randomGeneratorAddress) {
        pbexToken = IERC20(_pbexTokenAddress);
    ...
    function setEndTime(uint256 lotteryId,uint256 _endTime) external onlyOwner {
        _lotteries[lotteryId].endTime = _endTime;
    }
        ...
        nonReentrant
    {
        ...
        // Calculate number of PBEX to this contract
        uint256 amountPbexToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInPbex,
            _ticketNumbers.length
        );

        // Transfer pbex tokens to this contract
        pbexToken.safeTransferFrom(address(msg.sender), address(this), amountPbexToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInPbex += amountPbexToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInPbexToTransfer
        uint256 rewardInPbexToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInPbexToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        pbexToken.safeTransfer(msg.sender, rewardInPbexToTransfer);

        emit TicketsClaim(msg.sender, rewardInPbexToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPbex) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in PBEX for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].pbexPerBracket[j] =
                ...
                // A. No PBEX to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].pbexPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInPbex - amountToShareToWinners);

        // Transfer PBEX to treasury address
        pbexToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        pbexToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInPbex += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInPbex >= minPriceTicketInPbex) && (_priceTicketInPbex <= maxPriceTicketInPbex),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInPbex: _priceTicketInPbex,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            pbexPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInPbex: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(pbexToken), "Cannot be PBEX token");
    ...
    /**
     * @notice Set PBEX price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInPbex: minimum price of a ticket in PBEX
     * @param _maxPriceTicketInPbex: maximum price of a ticket in PBEX
     */
    function setMinAndMaxTicketPriceInPbex(uint256 _minPriceTicketInPbex, uint256 _maxPriceTicketInPbex)
        external
        onlyOwner
    {
        require(_minPriceTicketInPbex <= _maxPriceTicketInPbex, "minPrice must be < maxPrice");

        minPriceTicketInPbex = _minPriceTicketInPbex;
        maxPriceTicketInPbex = _maxPriceTicketInPbex;
    }
     ...
     * @param _priceTicket price of a ticket (in PBEX)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].pbexPerBracket[_bracket];
        } else {
            return 0;
        }
    }
