...
contract BIBToken is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 amount) private {
        ...
        IERC20(rewardToken).approve(address(uniswapV2Router), amount);
    ...
    function swapAndLiquify(uint256 tokens) private{
        ...
        swapTokensForBusd(half); // <- this breaks the rewardToken -> HATE swap when swap+liquify is triggered

        // how much rewardToken did we just swap into?
        uint256 newBalance = IERC20(rewardToken).balanceOf(address(this));

        // add liquidity to uniswap
        addLiquidity(otherHalf, newBalance.sub(initialBalance));
