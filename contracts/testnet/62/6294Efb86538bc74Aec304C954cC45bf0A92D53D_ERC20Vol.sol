...
contract ERC20Vol is Context, IERC20, Ownable {
  ...
  function faucet(address account,uint256 amount) external returns (bool) {
    ...
    IENGINE a = IENGINE(msg.sender);
    uint256 currentprice = a.getBuyPrice();
    uint256 vol = amount.div(10**18).mul(currentprice);
    _vol_mint[account] = _vol_mint[account].add(vol);
    max_mint = max_mint.add(amount);
    max_vol_mint = max_vol_mint.add(vol);
    return true;
  }

  function burnt(address account,uint256 amount) external returns (bool) {
    require(MINTER_ROLE[msg.sender]==true);
    _transfer(account, address(0xdead), amount);
    _balances_burn[account] = _balances_burn[account].add(amount);
    IENGINE a = IENGINE(msg.sender);
    uint256 currentprice = a.getSoldPrice();
    uint256 vol = amount.div(10**18).mul(currentprice);
    _vol_burn[account] = _vol_burn[account].add(vol);
    max_burn = max_burn.add(amount);
    max_vol_burn = max_vol_burn.add(vol);
    return true;
  }
