...
contract Staking is Ownable, ReentrancyGuard {      
    ...
    *   @dev Emitted when the subscription is stopped (400.000 tokens)
    ...
    uint256 private  INTEREST_1_VALUE; 
    uint256 private  INTEREST_2_VALUE; 
    uint256 private  INTEREST_3_VALUE; 
    uint256 private   INTEREST_AFFILIATE_VALUE; 
    uint256 private   INTEREST_STAKING_VALUE; 
    ...
    uint256 private constant _INTEREST_PERIOD = 1 days;    
    ...
    // 90 Days
    function setLevel1Interest(uint _interestRate1) external onlyOwner {
        INTEREST_1_VALUE = _interestRate1;
        
    }

    function getLevel1Interest() public view returns (uint) {
        
        return INTEREST_1_VALUE;
    }

    // 180Days
    function setLevel2Interest(uint _interestRate2) external onlyOwner {
        INTEREST_2_VALUE = _interestRate2;
            
     }

      function getLevel2Interest() public view returns (uint) {
        return INTEREST_2_VALUE;
    }

    // 365Days
     function setLevel3Interest(uint _interestRate3) external onlyOwner {
        INTEREST_3_VALUE = _interestRate3;
     }

      function getLevel3Interest() public view returns (uint) {
        return INTEREST_3_VALUE;
    }

     // referral reward
     function setreferralStakingInterest(uint _interestRateStaking) external onlyOwner {
            INTEREST_STAKING_VALUE = _interestRateStaking;
     }

      function referralStakingInterest() public view returns (uint) {
          
          return INTEREST_STAKING_VALUE;
    }

      // affiliate reward
     function setreferralAffiliateInterest(uint _interestRateAffiliate) external onlyOwner {
            INTEREST_AFFILIATE_VALUE = _interestRateAffiliate;
     }

     function getaffiliateStakingInterest() public view returns (uint) {
            return INTEREST_AFFILIATE_VALUE;
    }
    ...
    *   @param _referralAddress Address of the referer; 0x000...1 if no referer is provided
    ...
    function stakeToken(uint _amount, address _referralAddress, int _stakingClass) external nonReentrant {

        require(tokenAddress != address(0), "No contract set");

        require(_amount >= _MIN_STAKE_AMOUNT, "You must stake at least 1 tokens");
        ...
        newStake.stakingClass = _stakingClass;
    ...
    *   @param _stakeID The ID of the stake to be returned
    ...
    function returnTokens(uint _stakeID) external nonReentrant returns (bool){
        ...
        int checkClass = selectedStake.stakingClass;    

        if(checkClass == 1){
            require(calculatePeriods(_stakeID) >= 90, "Stake Duration not reached. You staked for 90 Days");
        }

        if(checkClass == 2){
            require(calculatePeriods(_stakeID) >= 180, "Stake Duration not reached. You staked for 180 Days");
        }

        if(checkClass == 3){
            require(calculatePeriods(_stakeID) >= 360, "Stake Duration not reached. You staked for 360 Days");
        }

        //  ****** This is just a test class ToDo: Remove in production *****
        if(checkClass == 0){
            require(calculatePeriods(_stakeID) >= 2, "Stake Duration not reached. You staked for 360 Days");
        }
    ...
    * @param _stakeID The ID of the stake of the caller
    ...
    function getCurrentStakeAmount(uint _stakeID) external view returns (uint256)  {
     ...
     function getCurrentStakeClass(uint _stakeID) external view returns (int)  {
        require(tokenAddress != address(0), "No contract set");

        return stake[msg.sender][_stakeID].stakingClass;
    }
    ...
    */
    function getStakeInfo(uint _stakeID) external view returns(uint, bool, uint, address, uint, uint,int){
        ...
        int stakeClass = selectedStake.stakingClass;
    ...
    *   @return The amount of referral reward related to the given customer
    ...
    function calculateRewardToWithdraw(uint _stakeID) public view returns (uint){
        ...
        //Periods for interest calculation
        // 0xeE52def4a2683E68ba8aEcDA8219004c4aF376DF
        // check the staking class to reward the interests
         uint interest_value;
            if(_stake.stakingClass == 1){
            interest_value = INTEREST_1_VALUE;
            }else if(_stake.stakingClass == 2){
                interest_value = INTEREST_2_VALUE;
            }else{
                interest_value = INTEREST_3_VALUE;
            }

            uint interest = amount_staked.mul(interest_value);
    ...
    function calculateCompoundInterest(uint _stakeID) external view returns (uint256){
        ...
        for(uint i = 0; i < periods; i++){
            ...
            period_interest = excepted_amount.mul(interest_value).div(100);
