...
contract Marketplace is Ownable, ERC721Holder, ERC1155Holder, IMarketplace {
  ...
  constructor(
    ...
    require(nftAddress != address(0) && recipient_ != address(0), 'NFTMarketplace: Address input is zero address');
    require(Address.isContract(nftAddress), 'You must input contract address');
...
contract RelipaNFT is ERC721Holder, ERC721Enumerable, Ownable, IRelipaNFT, AccessController {
  ...
  function transferNFT(
    ...
    address to,
    uint256 tokenId
  ) external override CheckAddress(from) CheckAddress(to) checkTokenId(tokenId) onlyOperator {
