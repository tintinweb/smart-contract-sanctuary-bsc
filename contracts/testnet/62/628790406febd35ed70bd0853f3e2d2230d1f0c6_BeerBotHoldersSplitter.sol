...
contract BeerBotHoldersSplitter is Ownable{
    ...
    // IER20 usdt; //try to define in constructor and not in each function
    ...
    function splitToHolders(address[] memory payees, uint16[] memory shares_) 
        ...
        onlyOwner        
        {
            ...
            uint256 payment = 0;
            uint256 usdtBaseValue = IERC20(usdtAddress).balanceOf(address(this));
            
            for(uint16 i = 0; i < payees.length; i++) {
                payment =  usdtBaseValue * shares_[i] / 10000;                
                // (bool sent, ) = payable(payees[i]).call{value: payment}("");                
                sendUSDT(payees[i], payment);
            }
        }
