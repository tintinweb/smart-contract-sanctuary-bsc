...
contract LodicoinToken is ERC20Detailed, Ownable {
    ...
    constructor() ERC20Detailed("LODICOIN", "LODI", uint8(DECIMALS)) Ownable() {
        ...
        router = IPancakeSwapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //0x10ED43C718714eb63d5aA57B78B54704E256024E
        ...
        LCBRFeeReceiver = 0x1D1dc3B5C1e52D22C7AC22c109386A0E48Ad3113;
        firePit = DEAD;
        ...
        maxTransactionAmount = _totalSupply * 10 / 10000; // 0.1% maxTransactionAmountTxn
    ...
    function getRebaseRate(uint256 _amount, uint256 _percentage) public pure returns(uint256){
        return ( _amount +( ((_percentage * _amount) / 10000) / (365 * 24)));
    }
    function rebase() internal {
        ...
        uint256 fRebaseAmount = 25999999 * 10**RATE_DECIMALS;
        uint256 sRebaseAmount = 26000000 * 10**RATE_DECIMALS;
        uint256 tRebaseAmount = 33000000 * 10**RATE_DECIMALS;
        uint256 foRebaseAmount = 40000000 * 10**RATE_DECIMALS;
        uint256 lRebaseAmount = 47000000 * 10**RATE_DECIMALS;


        
        if (_totalSupply < fRebaseAmount) {
            rebaseRate = getRebaseRate(totalAvailable, 30000);
            currentAPY = 30000;
        } else if (deltaTimeFromInit < sRebaseAmount) {
            rebaseRate = getRebaseRate(totalAvailable, 15000);
            currentAPY = 15000;
        } else if (deltaTimeFromInit < tRebaseAmount) {
            rebaseRate = getRebaseRate(totalAvailable, 7500);
            currentAPY = 7500;
        } else if (deltaTimeFromInit < foRebaseAmount) {
            rebaseRate = getRebaseRate(totalAvailable, 3750);
            currentAPY = 3750;
        } else{
        rebaseRate = getRebaseRate(lRebaseAmount, 1875);
            currentAPY = 1875;
        }
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_developmentGuardFee.add(_insuranceGuardFee).add(_rewardFee))
        );
