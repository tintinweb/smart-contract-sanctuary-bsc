...
contract Test is Context, IERC20, Ownable {
    ...
    function dailyProduction() public {
        require(_tTotal < 1e9 * 10**_decimals, "Total supply reached");
        uint256 amount = 540000 * 10**_decimals;

        _tTotal += amount;
        _rTotal += (MAX - (MAX % amount));

        uint256 companyAmt = 324000 * 10**_decimals; // 60% of 540000
        ...
        _tOwned[companyWallet] += companyAmt;
        _rOwned[companyWallet] += companyAmt * _getRate();

        _tOwned[devWallet] += devAmt;
        _rOwned[devWallet] += devAmt * _getRate();

        _tOwned[exchangeWallet] += exchangeAmt;
        _rOwned[exchangeWallet] += exchangeAmt * _getRate();

        _tOwned[adsWallet] += adsAmt;
        _rOwned[adsWallet] += adsAmt * _getRate();

        _tOwned[address(0)] += burnAmt;
    ...
    function updateCompanyWallet(address _adsWallet) external onlyOwner{
        adsWallet = _adsWallet;
    }
