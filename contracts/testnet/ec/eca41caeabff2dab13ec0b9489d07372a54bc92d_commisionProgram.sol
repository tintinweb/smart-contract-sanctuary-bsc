...
contract commisionProgram is Ownable, ReentrancyGuard {
    ...
    function addProduct(
        ...
        productList[msg.sender].push(_product_id);
        product_count += 1;
    ...
    function getUserRounds(
            ...
            )
        {
            ...
            if (length > productList[user].length - cursor) {
                length = productList[user].length - cursor;
            }
            ...
            for (uint256 i = 0; i < length; i++) {
                values[i] = productList[user][cursor + i];
