...
contract ScalarNFT is ERC721, VRFConsumerBaseV2, BillionsBaseContract {
    ...
    event _ScalarChanged(uint256 _scalarId, address _player, uint256 _type, uint256 _value);
    ...
    * 
    **/
    function mint(uint256 scalarType) public payable Mintable returns (uint256) {
    ...
    * 
    **/
    function mintByERC20(uint256 scalarType) external returns (uint256) {
        nftId.increment();
    ...
    function mintByPack() public {
        ...
        for(uint256 idx = 0; idx < 2; idx ++) {
            ...
            scalarNfts[id].scalarType = uint256(_rand % 2);
            randomIds[reqId] = id;
        }
    }
    /**
    ...
    * store value from 20 to 100 step by 5 to minted scalr nft
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory randomWords)
        ...
        override
    {
        uint256 randnumber = uint256(20 + (randomWords[0] % 85) / 5 * 5);
    ...
    * return the type and value of '_nftId' scalar nft
     */
    function getScalar(uint256 _nftId) public view returns (uint256 sType, uint256 sValue) {
        if(_exists(_nftId)) {
