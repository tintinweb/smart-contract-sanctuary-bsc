...
Ownable
{
  ...
  //nft price in usdt
  mapping(address => mapping(address => uint256)) public nftPriceMap;
  mapping(address => uint256) public nftCountMap;
  ...
  function setPrice(address _nft, address _token, uint256 _price) onlyOwner external {
    nftPriceMap[_nft][_token] = _price;
  }

  function setNftCountMap(address[] calldata _nfts, uint256[] calldata _counts) onlyOwner external {
    require(_nfts.length == _counts.length && _nfts.length > 0, "len not good");
    for (uint i = 0; i < _nfts.length; i++) {
      nftCountMap[_nfts[i]] = _counts[i];
    }
  }

  function setFee(address _feeToken, uint256 _fee) onlyOwner external {
    feeToken = _feeToken;
    fee = _fee;
  }

  //  function _sortToken(address tokanA, address tokenB) internal view returns (address token0, address token1) {
  //    (address token0, address token1) = tokanA < tokenB ? (tokanA, tokenB) : (tokenB, tokanA);
  //  }
  //
  //  function addLpMap(address token, address lp) onlyOwner external {
  //    require(ERC20(token).decimals() == 18);
  //    (address token0, address token1) = _sortToken(token, usdtAddress);
  //    lpMap[token0][token1] = lp;
  //  }
  ...
  function buyNFT(address _nft, address _token) public {
    require(nftCountMap[_nft] > 0, "buy: _nft amount not good");
    uint256 price = nftPriceMap[_nft][_token];
    require(price > 0, "buy: _nft price not good");
    if (_token == feeToken) {
      require(ERC20(_token).balanceOf(msg.sender) >= fee.add(price), "buy: balance not good");
    } else {
      require(ERC20(_token).balanceOf(msg.sender) >= price, "buy: balance not good");
    }
    ...
    NFTokenMetadataEnumerableMock(_nft).mint(address(msg.sender));
  }

  function buyNFTBatch(address _nft, address _token, uint256 _num) public {
  ...
  //  function getPrice(uint256 _times, address _token) public view returns (uint256) {
  //    require(_token != usdtAddress, "getPrice: invalid _token");
  //    (address token0, address token1) = _sortToken(_token, usdtAddress);
  //    address lp = lpMap[token0][token1];
  //    uint256 balanceToken = IERC20(_token).balanceOf(lp);
  //    uint256 balanceUsdt = IERC20(usdtAddress).balanceOf(lp);
  //    return balanceUsdt.mul(_times).div(balanceToken);
  //  }
