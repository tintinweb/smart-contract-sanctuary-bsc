...
//****************************************************************************//
 ...
 contract xMan is owned {
    ...
    {  
        ...
        if(stakeInfos[ref].length >= lastSTakeIndex)   tokenInterface(tokenAddress).transfer(ref, _amt/10);

        emit paidEv(0,ref, userInfos[ref].id, amount/10, now);

        for (uint i=0 ; i < 15; i++)
        {
            if(stakeInfos[ref].length >= lastSTakeIndex)   userInfos[ref].rewardEarned[i] += amount * levelRewardPercent[i] / 100000;
