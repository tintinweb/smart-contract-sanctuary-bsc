    ...
    IExenoFinanceNode
{
    ...
    function _collectInboundCorePayments(
        ...
        internal
    {
        if (_isCore(d.token[0]) && d.mode[0] != bytes32("i")) {
        ...
        internal
    {
        ...
        if (params.length > 0) {
            ...
            if (action == ACTION_DEBIT_REWARD) {
                ...
                ExenoFinance.emitDebit(d, id, token, amount);
            } else if (action == ACTION_DEBIT_OPTION) {
                (address token, uint256 amount) = abi.decode(args, (address, uint256));
                _verifyIdentifiers(d.method, ExenoFinance.METHOD_COLLATERAL);
                ExenoFinance.emitOption(d, token, amount);
            } else {
        ...
        internal
    {
        if (params.length > 0) {
            ...
            if (action == ACTION_CREDIT_REWARD) {
                ...
                ExenoFinance.emitCredit(d, token, amount);
    ...
    function _processSwap(
        ...
        internal
    {
        _verifyIdentifiers(d.method, ExenoFinance.METHOD_SWAP);
...
library ExenoFinance {
        ...
        external view
    {
        ...
        require(verifyTimeout(d.timeout[0]),
            "EFN: trxn outside of time-window");
        require(unprocessed || d.method == METHOD_SWAP,
            "EFN: debit a/y processed");
        if (d.method == METHOD_SWAP) {
            require(d.beneficiary[0] != address(this) || d.mode[0] == bytes32("i"),
                "EFN: mode mismatch");
        } else if (d.method == METHOD_STAKE_UNSTAKE) {
            ...
            require(d.method == ExenoFinance.METHOD_BURN_MINT
    ...
    /**
     * Publish an event while reducing its storage footprint to mimimum
     */
    function publish(
        Event e,
        Data memory d
