...
contract AS_Token is Context, IERC20, Ownable {
    ...
    uint256 public _maxSellRate = 99;                   //最大卖币比例 （public 可查询）
       

    //税费：总8%，持币分红0%；其他8%
    uint256 public _FeeTax = 0;     //持币分红（public 可查询）
    uint256 private _previousFeeTax = _FeeTax;
    uint256 public _FeeElse = 8;    //其他费用 （public 可查询）
    uint256 private _previousFeeElse = _FeeElse;

    //本程序目录这些地址都参与分红，可设置一个开关，这些地址是否参与分红    
    address public mainAddress;     //主地址    
     
    address public fundAddress;     //基金地址    （无邀请人的回到基金地址） 
    address public liquidAddress;   //回流地址     （增加LP池回流）
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint8 _DECIMALS, uint256 _SUPPLY, 
       ...
       address _mainAddress,address _fundAddress,address _liquidAddress) public {
        _name = _NAME;
        _symbol = _SYMBOL;
        _decimals = _DECIMALS;
    ...
    //设置分红手续费 setFeeTax，对应百分比整数，默认=0
    function setFeeTax(uint256 FeeTax) public onlyOwner {
        _FeeTax = FeeTax;
        _previousFeeTax = _FeeTax;
    }
    //设置其他手续费 setFeeElse，对应百分比整数，默认=8
    ...
    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "EGN: Transfer from the zero address");  //转出为非零地址
        require(to != address(0), "EGN: Transfer to the zero address");      //转入为非零地址        
        //require(amount > 0, "EGN: Transfer amount must be greater than zero");  //数量大于0,不要此句，用来激活空投

        //检查 是否为 冻结帐户 
        require(!_frozenAccount[from], "EGN: Transfer from Account is frozen");
        require(!_frozenAccount[to], "EGN: Transfer to Account is frozen");

        //如果接收地址 是合约地址 并且是交易对合约地址 ；是卖币
        if (isContract(to) && _isSwapPair[to]) {
            require(_SellBool,"EGN: Sell not allowed");
            if (_WhiteBool) { 
                require(_whiteAccount[from],"EGN: Sell account not in white list");  //如果只允许白名单交易
            }  
            require( _maxSellAmount <= 0 || amount <= _maxSellAmount, "EGN: Transfer amount exceeds the maxSellAmount."); //卖币最大量
            require(amount <= balanceOf(from) * _maxSellRate / 100, "EGN: Transfer amount exceeds the maxSellRate.");   //最多一次转比例（可反复） 
            //如果对卖出次数有限制 同时卖出地址不是白名单，则判断是否超卖次数
			if(_SellNum >0 && !_whiteAccount[from]){
				require(_onSellNum[from] <= _SellNum, "EGN: Number of sell exceeds"); //大于卖出次数                
            }    
        ...
        if (isContract(from) && _isSwapPair[from]) {   
            if (_WhiteBool) { require(_whiteAccount[to],"EGN: Buy account not in white list"); }
            else { require( _maxBuyAmount <= 0 || amount <= _maxBuyAmount, "EGN: Transfer amount exceeds the maxBuyAmount."); } //买币最大量
        } 
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        //买卖手续费8%
        _takeInviterFee(sender, recipient, tAmount); // 2.5% （动态奖分成拿5代共2.5%）
        _takeLiquidity(sender, tElseFee * 5 / 16); // 2.5% （回流2%）
        _takeBurn(sender, tElseFee * 3 / 8);   // 3% （销毁2%）
       
        _reflectFee(rTaxFee, tTaxFee);        //  （持币分红）
    }
    ...
    function _takeInviterFee(address sender, address recipient, uint256 tAmount) private {
        ...
        for (uint8 i = 0; i < inviteRate.length; i++) {
            ...
            cur = _inviter[cur];
            if (cur == address(0)) {
                cur = fundAddress;  //如果没有推荐地址，则转基金地址
            }
