...
contract PandoChestV2 is Ownable, Pausable {
    ...
    constructor (address _busd, address _pandoPool) {
        busd = _busd;
    ...
    function allocateMoreRewards(uint256 _allocationAmount, uint256 _allocateDay) external onlyOperator whenNotPaused{
        ...
        for(uint256 i = 1; i <= totalPool; i++){
            uint256 _amount = _allocationAmount * poolInfo[i].rate / ONE_HUNDRED_PERCENT;
            IPandoAssembly(poolInfo[i].pool).allocateMoreRewards( _amount, _allocateDay);
            emit RewardAllocated(poolInfo[i].pool, _amount, _allocateDay);
        }
    ...
    function manualAllocate(uint256 _allocationAmount, uint256 _allocateDay) external onlyOwner{
        require(IERC20(busd).balanceOf(address(this)) >= _allocationAmount, 'PandoChest: !balance');
        for(uint256 i = 1; i <= totalPool; i++){
            uint256 _amount = _allocationAmount * poolInfo[i].rate / ONE_HUNDRED_PERCENT;
            IPandoAssembly(poolInfo[i].pool).allocateMoreRewards(_amount, _allocateDay);
            emit RewardAllocated(poolInfo[i].pool, _amount, _allocateDay);
    ...
    function directAllocate(address _pool, uint256 _allocationAmount, uint256 _allocateDay) external onlyOwner{
        IPandoAssembly(_pool).allocateMoreRewards(_allocationAmount , _allocateDay);
        emit RewardAllocated(_pool, _allocationAmount, _allocateDay);
    }
    ...
    function setBUSD(address _busd) external onlyOwner {
        address old = _busd;
        busd = _busd;
        emit BUSDChanged(_busd, old);
    }

    function setPools(address[] memory _pools, uint256[] memory _rates) external onlyOwner {
        ...
        for(uint256 i = 0; i < _pools.length; i++){
            ...
            if(IERC20(busd).allowance(address(this), _pools[i]) == 0){
                IERC20(busd).safeApprove(_pools[i], type(uint256).max);
            }
            _totalRate += _rates[i];
        }

        require(_totalRate == ONE_HUNDRED_PERCENT,"PandoChest: !ONE_HUNDRED_PERCENT");
        emit PoolChanged(_pools,_rates);
    }

    event RewardAllocated(address _pool,uint256 _amount, uint256 _days);
    ...
    event PoolChanged(address[] pools, uint256[] rates);
    event PandoPoolChanged(address newPool, address oldPool);
    event BUSDChanged(address newBUSD, address oldBUSD);
}