...
contract NimbusInitialAcquisition is Initializable, NimbusInitialAcquisitionStorage {
    ...
    function getSwapSystemTokenAmountForToken(address token, uint tokenAmount) public view returns (uint) { 
        return getSwapRate(token, address(SYSTEM_TOKEN), tokenAmount);
    }

    function getSwapSystemTokenAmountForBnb(uint bnbAmount) public view returns (uint) { 
        return getSwapRate(swapRouter.NBU_WBNB(), address(SYSTEM_TOKEN), bnbAmount);
    }
    ...
    function getSwapRate(address tokenSrc, address tokenDest, uint tokenAmount) public view returns (uint256 resultSwap) { 
        if (tokenSrc == tokenDest) return tokenAmount;
        address[] memory path = new address[](2);
        ...
        uint256 maxSystemTokenImpact = getMaxSystemTokenImpact();
        uint256 swapPartBnb = getSwapSystemTokenAmountForToken(NBU_WBNB, maxSystemTokenImpact); // part to swap with price impact
        uint256 systemTokenAddPart = 0;

        if (resultSwap > maxSystemTokenImpact && tokenAmount > swapPartBnb && tokenDest == address(SYSTEM_TOKEN)) {
        ...
        return resultSwap + systemTokenAddPart;
    }

    function getMaxSystemTokenImpact() public view returns(uint256) {
        ...
        return (sqrt - 1997*p)*reserveIn/(2*997*p);
    }
    ...
    function _purchaseFromSwap(address token, uint tokenAmount) private returns(uint systemTokenAmount) {
        ...
        if (tokenAmount > maxSystemTokenImpact && tokenAmount > swapPartBnb) {
            uint256 fixedPriceSwap = tokenAmount - swapPartBnb; // part to swap with fixed price
            systemTokenAddPart = getSystemTokenAmountForBnb(fixedPriceSwap);  // to add NIMB
    ...
    function updateSystemToken(address newSystemToken, address newNimbProxy) external onlyOwner {
    ...
    function updateMaxPriceImpact(uint256 _maxPriceImpact) external onlyOwner {
