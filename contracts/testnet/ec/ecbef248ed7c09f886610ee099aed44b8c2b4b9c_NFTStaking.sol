...
contract NFTStaking is Ownable {
    ...
    function deposite(uint256 tokenId) public {
        ...
        require(
            ...
            "User can not hold this NFT"
        );
        stakedToken.transferFrom(msg.sender, address(this), tokenId);
        userInfo(tokenId);
    }

    function calclulateReward(address account, uint256 id)
        public
        view
        returns (uint256)
    {
        require(
            stakesOfUser[msg.sender][id].amount > 0,
            "There is no such stake!"
        );
        require(
            !stakesOfUser[msg.sender][id].isWithdrawn,
    ...
    function claim(uint256 id) public {
        ...
        stakedToken.transferFrom(
            address(this),
            msg.sender,
            stakesOfUser[msg.sender][id].tokenId
        );
    }

    constructor() {
        tokenPerSecond = 1000000000000; //0.033
        stakedToken = IERC721(0xf262A8bcF3E27053160375c9Ad8d3Af394D5825b);
        rewardToken = IBEP20(0x06Fb74Bb4cB3084Fe480057BC8255322804053a4);
    }
}