...
// IUniswap interface(IUniswap 接口) V2
// Factory
...
contract WHOToken is BEP20, permission{
    ...
    bool isSolidity; // 是否是合约本身调用
    bytes4 private constant SELECTOR = bytes4(keccak256(bytes('transfer(address,uint256)')));
    IWhoSql db;
    IUniswapV2Router02 router;
    // 初始化 constructor
    constructor(address permission_address,address usdt,address uniswapRouter,address _db,address _PoundageAddress) BEP20("WHO","WHO") {
        // USDT地址
        USDTAddress = usdt;
        // Router地址
        ...
        // 默认手续费地址不能为合约本身
        PoundageAddress = _PoundageAddress;
        // _mint(address(this), endPoolAmount);
        // 分配代币给某个用户
        _mint(msg.sender, endPoolAmount); // 底池添加用 (发到一个地址 后台进行调用)
        // 初始化生成UniswapPair流动池
        uniswapV2Pair = IUniswapV2Factory(router.factory()).createPair(address(this),usdt);
        // 设置数据库
        db = IWhoSql(_db);
        // 设置权限库
        setPermissioin(permission_address);
    }
    // @dev 设置Dao合约地址
    ...
    // @dev 设置数据库
    function setSql(address sql) public payable isAdmin(msg.sender){
        db = IWhoSql(sql);
    }
    
    function _safeTransfer(address token, address to, uint value) private {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(SELECTOR, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'WHOToken: transfer failed');
    }
    ...
    function transferFrom(
        ...
        if(!isSolidity){
            if(from == routerAddress|| to == uniswapV2Pair){
                ...
                // 是否大于20w
                if(_total>20 * 10**5 * 10 **18){
                    uint _newTotal = _total - 20 * 10**5 * 10 **18;
                    for(uint i = 0;i<users.length;i++){
                        // 分配收入
                        db.addSupernumeraryIncome(users[i],db.getUserLpWhoTotal(users[i], "WHO")/_newTotal);
                    }
                    // * _total / db.getUserLpWhoTotal(msg.sender, "WHO");
                }
                // 2%销毁
                _burn(address(this), amount / 100 * 2);
                // 2% 注入底池
                _transfer(address(this), daoAddress, amount / 100 * 2);
                // 官方底池添加金额
                db.addPoolAmount("WHO",amount / 100 * 2);
                //  1% 社区分红 固定平均分配（缺少分配记录）
                _transfer(address(this),daoAddress, amount / 100 * 1);
                WhoLib.partner[] memory part = db.getGeneralPartner();
                WhoLib.partner[] memory npart = new WhoLib.partner[](part.length);
                uint b = 0; // 总存在的人数
                ...
                for(uint c=0;c<npart.length;c++){
                    db.addSupernumeraryIncome(npart[c].user,(amount / 100 * 1)/b);
                }
                // 返回给to
    ...
    // 删除流动性 (删除全部)
    function removeLiquidityUser(uint liquidity) public payable {
        ...
        _safeTransfer(address(this),msg.sender,amountA * 97 / 100);
        _safeTransfer(USDTAddress,msg.sender,amountB * 97 / 100);
        _safeTransfer(address(this),PoundageAddress,amountA * 3 / 100);
        _safeTransfer(USDTAddress,PoundageAddress,amountB * 3 / 100);
    }
    // @dev 增加流动性方法 amountADesired 内部方法 使用前需要转入部分 USDT 到合约
    function addLiquidityUser(uint tokenAmount,uint USDTAmount) public payable{
        isSolidity = true;
        ...
        // 白名单和官方不收手续费
        if(!db.isTradingWhiteList(msg.sender)){
            if(!db.isOfficeAddress(msg.sender)){
                _approve(address(this), daoAddress, tokenAmount / 100 * 3);
                _transfer(address(this), daoAddress, tokenAmount / 100 * 3);
                _safeTransfer(USDTAddress,daoAddress,USDTAmount / 100 * 3);
                IERC20(address(this)).approve(routerAddress,tokenAmount / 100 * 97);
                IERC20(USDTAddress).approve(routerAddress,USDTAmount);
                db.setUserLPStartTime(msg.sender,block.timestamp);
                // 调用内部的添加流动池方法 零滑点
                router.addLiquidity(address(this), USDTAddress, tokenAmount / 100 * 97, USDTAmount / 100 * 97, 0, 0, msg.sender, block.timestamp);
                // 增加记录
                db.addLpRecord(msg.sender, WhoLib.LpUint(block.timestamp,tokenAmount/100*97,USDTAmount/100*97,"WHO"));
            }else{
                IERC20(address(this)).approve(routerAddress,tokenAmount);
                IERC20(USDTAddress).approve(routerAddress,USDTAmount);
                // 调用内部的添加流动池方法 零滑点
                router.addLiquidity(address(this), USDTAddress, tokenAmount, USDTAmount, 0, 0, msg.sender, block.timestamp);
                // 增加记录
                db.addLpRecord(msg.sender, WhoLib.LpUint(block.timestamp,tokenAmount,USDTAmount,"WHO"));
            }
        }
        // 增加用户（自动过滤）
    ...
    // @dev 代币与代币交换
    function swapTokensForExactTokens(uint amountOut,address[] memory tokens) public {
        isSolidity = true;
        // 转移tokens第一个代币到合约地址
        IERC20(tokens[0]).transferFrom(msg.sender, address(this), amountOut);
        uint remain;
        // 白名单和官方不走手续费
        if(!db.isTradingWhiteList(msg.sender)&&!db.isOfficeAddress(msg.sender)){
            distributionFee(msg.sender,amountOut);
            // 扣除手续费后剩余部分
        ...
        // 批准合约兑换
        IERC20(tokens[0]).approve(routerAddress, remain);
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(remain, 0, tokens,msg.sender, block.timestamp);
        isSolidity = false;
    }
    // 判断父级是否满足条件
    function fatherMeet(address user,uint amount) internal {
        uint fatherAmount = balanceOf(user);
        // 获取usdt与who在pair流动池的total
        (uint112 usdtTotal,uint112 whoTotal,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        // WHO价格 默认定死 usdt 为 1u
        // uint whoAmount = UniswapV2Library.getAmountOut(1, usdtTotal, whoTotal);
        uint whoAmount = (usdtTotal * 10 ** decimals())/whoTotal;
        // 质押挖矿的WHO数
        uint plgWho = db.getPlgTokenNum(user,"WHO");
        uint lpWhoTotal = db.getUserLpWhoTotal(user,"USDT");
        if(fatherAmount>=whoAmount * 500){ // 余额满足500U
        // 质押挖矿满足100WHO且Lp满足100U
            if(plgWho>100&&lpWhoTotal>100){
                // USDT是否满足数量
                _transfer(address(this),user, amount);
            }else{
                _transfer(address(this), daoAddress, amount);
            }
        }else{
            _transfer(address(this),daoAddress, amount);
        }
    ...
    function distributionFee(address user,uint amount) internal {
        // 获取所有父级 按顺序排序
        address[] memory father = db.getFathers(user);
        // 设置兑换金额(一次性)
        db.setExchange(user,amount);
        if(father.length>0){
            // 分配推荐人分红
            for(uint i = 0;i<father.length;i++){
                ...
                if(i>=0&&i<3){
                    // 1-3 分配 1%
                    fatherMeet(father[i],amount / 100 * 1);
                }else if(i>=3&&i<5){
                    // 4-5 分配 0.5%
                    fatherMeet(father[i],amount / 1000 * 5);
                }else if(i>=5&&i<10){
                    // 6-10 分配 0.2%
                    fatherMeet(father[i],amount / 1000 * 2);
                }
            }
        }else{
            // 无父级直接转移 5% 到dao
...
import "../../library/WhoLib.sol";
interface IWhoSql{
    function testBig(WhoLib.PoolType pType) external payable returns(uint);
    function setPledgeLookPool(WhoLib.PledgePoolBasicInfo memory plgPoolBasicInfo) external payable ;
    function getOnePledgeLockPool(WhoLib.PoolType pType) external view returns(WhoLib.PledgeLockPool memory plgLkP) ;
    function addPledgeUintToLockPool(WhoLib.PledgeUint memory plgUint, WhoLib.PoolType pType) external payable ;
    function getPledgeUints(WhoLib.PoolType pType) external view returns(WhoLib.PledgeUint[] memory) ;
    function setAcconAddPledgeUintRecords(address accon, WhoLib.PledgeUint memory plgUint, WhoLib.PoolType pType) external payable ;
    function getAcconAddPledgeUintRecords(address accon, WhoLib.PoolType pType) external view returns(WhoLib.PledgeUint[] memory) ;
    function getAcconAddPledgeUnitTotalAmount(address accon, WhoLib.PoolType pType) external view returns(uint) ;
    function setAcconSubPledgeUintRecords(address accon, WhoLib.PledgeUint memory plgUint, WhoLib.PoolType pType) external payable ;
    function getAcconSubPledgeUintRecords(address accon, WhoLib.PoolType pType) external view returns(WhoLib.PledgeUint[] memory) ;
    function getAcconSubPledgeUnitTotalAmount(address accon, WhoLib.PoolType pType) external view returns(uint) ;
    function setdWithdrawUintRecord(address accon, WhoLib.WithdrawUint memory wdUint) external payable ;
    function getWithdrawUintRecords(address accon) external view returns(WhoLib.WithdrawUint[] memory records);
    function setWhoPoolTotal(address accon, WhoLib.PoolType pType, uint256 totalAmount) external payable ;
    function getWhoPoolTotal(address accon, WhoLib.PoolType pType) external view returns(uint256) ;
    ...
    function getSeniorPartner() external view returns (WhoLib.partner[] memory _senior);
    function setGeneralPartner(address user) external payable;
    function deleteGeneralPartner(address user) external payable;
    function getGeneralPartner() external view returns (WhoLib.partner[] memory _general);
    ...
    function setExchange(address user,uint _amount) external payable;
    function getExchange(address user) external view returns (uint);
    function addExchangeRecord(address user,WhoLib.exChangeUint memory _exRecord) external payable;
    function getExchangeRecord(address user) external view returns (WhoLib.exChangeUint[] memory _exRecord);
    function addLpRecord(address user,WhoLib.LpUint memory _lpRecord) external payable;
    function addAddLpUser(address user) external payable;
    function getLpUser() external view returns (address[] memory user);
    function getLpRecord(address user) external view returns (WhoLib.LpUint[] memory _lpRecord);
    function getLpTotal(string memory _type) external view returns (uint);
    function setuserLpWhoTotal(address user,string memory _type,uint amount) external payable ;
    function clearUserLpWhoTotal(address user,string memory _type) external payable ;
    function getUserLpWhoTotal(address user,string memory _type) external view returns (uint);
    ...
    function addSupernumeraryIncome(address user,uint amount) external payable;
    function getSupernumeraryIncome(address user) external view returns (uint amount);
    function setCalculatePool(uint amount) external payable ;
    ...
    function getUserLPStartTime(address user) external view returns (WhoLib.timeUint[] memory time);
...
library WhoLib {
    ...
    enum UintStatus {Failed, Succeed, Pending} 
    ...
    struct WithdrawUint{
        ...
        string tokenName; // token 类型 USDT & WHO
    ...
    struct PledgePoolBasicInfo {
        ...
        uint256 outputRatioNumer; // 产量占比 15% 30% 分子 15/30
        uint256 outputRatioDeno; // 产量占比分母 100/100
        PoolType pType; // 矿池的类型
        // PoolStatus 
        // uint256 endTime; // 矿池销毁时间
    ...
    struct MarketStatistics {
        ...
        uint256 currentCurrencyValue; // 当前流通市值
        uint256 burnedTotalAmount; //当前销毁的总数量
        uint256 burnedTotalValue; // 当前销毁的总市值
    }
    ...
    struct LpUint{
        ...
        uint tokenAmount; // 添加的代币金额
        uint usdtAmount; // 添加的usdt金额
