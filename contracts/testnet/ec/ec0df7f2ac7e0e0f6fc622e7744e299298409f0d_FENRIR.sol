...
contract DividendDistributor is IDividendDistributor {
    ...
    function setDistributionCriteria(uint256 newMinPeriod, uint256 newMinDistribution) external override FenrirToken {
        minPeriod = newMinPeriod;
        minDistribution = newMinDistribution;
    }

    function setShare(address shareholder, uint256 amount) external override FenrirToken {
    ...
    function deposit() external payable override FenrirToken {
    ...
    function process(uint256 gas) external override FenrirToken {
    ...
    function claimDividend(address shareholder) external FenrirToken{
        distributeDividend(shareholder);
    }
    
    function rescueDividends(address to) external FenrirToken {
        RewardToken.transfer(to, RewardToken.balanceOf(address(this)));
    }
    
    function setRewardToken(address _rewardToken) external FenrirToken{
...
contract FENRIR is IBEP20, Auth {
        
    using SafeMath for uint256;

    string constant _name = "FENRIR";
