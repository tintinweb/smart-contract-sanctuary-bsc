...
contract NFTSimplifiedMinter is AccessControl {
    ...
    function mint(uint256[] calldata prices, MarketItems.Currency[] calldata currencies) external onlyRole(MINTER_ROLE) {
        IPrideNFT _token = IPrideNFT(token);
        for (uint256 i = 0; i < prices.length; i++) {
            _token.safeMint(holder);
        }
    }
