...
struct TokenInfo {
    uint8 decimals;
    address charity;
    uint256 maxTxBurnFee;
    uint256 maxTxCharityFee;
    uint256 burnFee;
    uint256 charityFee;
    uint256 maxTxAmount;
    uint256 firstDayMaxTxAmount;
}
...
contract TigerFortune is ERC20 {
    ...
    constructor(address _charity, uint256 _timestamp) ERC20(m_name, m_symbol) {
        ...
        INFO.charity = _charity;
        tradingEnabledTimestamp = _timestamp;
        m_totalSupply = m_totalSupply * (10**uint256(m_decimals));
        _isExcludedFromFees[_msgSender()] = true;
        _isExcludedFromFees[INFO.charity] = true;
        INFO.decimals = m_decimals;
        INFO.maxTxBurnFee = 60;
        INFO.maxTxCharityFee = 50;
        INFO.burnFee = 10;
        INFO.charityFee = 20;
        INFO.maxTxAmount = m_totalSupply.mul(10).div(10**3);
        INFO.firstDayMaxTxAmount = m_totalSupply.mul(5).div(10**3);
    ...
    function setCharityWallet(address account) external onlyOwner {
        address _privateCharity = INFO.charity;
        INFO.charity = account;
    ...
    function _transfer(
        ...
        uint256 _maxTxAmount = INFO.maxTxAmount;
        if (from != owner() && to != owner()) {
            if (block.timestamp <= tradingEnabledTimestamp + 1 days) {
                _maxTxAmount = INFO.firstDayMaxTxAmount;
                require(
                    amount <= _maxTxAmount,
        ...
        if (!_isExcludedFromFees[_msgSender()]) {
            uint256 BurnAmount = amount.mul(INFO.burnFee).div(10**3);
            uint256 charityAmount = amount.mul(INFO.charityFee).div(10**3);
            if (amount > _maxTxAmount) {
                BurnAmount = amount.mul(INFO.maxTxBurnFee).div(10**3);
                charityAmount = amount.mul(INFO.maxTxCharityFee).div(10**3);
            }
            uint256 trueAmount = amount.sub(BurnAmount).sub(charityAmount);
            if (BurnAmount > 0) super._transfer(from, m_deadWallet, BurnAmount);
            if (charityAmount > 0)
                super._transfer(from, INFO.charity, charityAmount);
