...
contract AggregatorExecutor is Ownable {
    ...
    ensure(_deadline)
    {
        ...
        address[] memory _paths = new address[](_length+1);
        for (uint256 index = 0; index < _length; index++) {
            _paths[index] = _steps[index].fromToken;
        }
        _paths[_length-1] = _steps[_length-1].toToken;
        checkSigner(_signature.data, _paths);
        //check input eth
    ...
    //=========== internal functions ============//
    function checkSigner(bytes memory _signature, address[] memory _path) internal{
        // require(!usedNonce.get(_nonce), "AggregatorExecutor: _nonce has been used");
        bytes32 _hash = keccak256(abi.encodePacked(address(this), _path)).toEthSignedMessageHash();
    ...
    function verifySignature(bytes memory _signature, address[] memory _path) external view returns(bool) {
        // require(!usedNonce.get(_nonce), "AggregatorExecutor: _nonce has been used");
        bytes32 _hash = keccak256(abi.encodePacked(address(this), _path)).toEthSignedMessageHash();
        address _signer = _hash.recover(_signature);
