...
contract SpurtStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The spurt TOKEN!
    IBEP20 public immutable spurtToken;
    ...
    event StakeSpurt(address indexed user, uint256 amount);
    event WithdrawSpurt(address indexed user, uint256 amount);
    event SpurtHarvest(address indexed user, uint256 amount);

    constructor(
    ) {
        spurtToken = IBEP20(0xdC763fEE65C445C46a8ff9Cc997F966B7DC44402);
        rewardToken = IBEP20(0xdC763fEE65C445C46a8ff9Cc997F966B7DC44402);
        rewardPerBlock = 1 * 1e18; // 1 per block
        startBlock = block.number;
        bonusEndBlock = 99999999;

        // staking pool
        poolInfo.push(PoolInfo({
            lpToken: IBEP20(0xdC763fEE65C445C46a8ff9Cc997F966B7DC44402), // Spurt
            allocPoint: 1000,
        ...
        transferOwnership(0xc72e05F49e4a1eabDf3174a8081686260f1cBf64);
    ...
    function stakeSpurt(uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[0];
        ...
        emit StakeSpurt(msg.sender, _amount);
    }

    // Withdraw Spurt from STAKING.
    function withdrawSpurt(uint256 _amount) public nonReentrant {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= _amount, "withdrawSpurt: attempting to withdraw too many tokens");
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accTokensPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            rewardToken.safeTransfer(address(msg.sender), pending);
        ...
        emit WithdrawSpurt(msg.sender, _amount);
    }

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function spurtHarvest() external nonReentrant {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        emit SpurtHarvest(msg.sender, user.amount);
    }
    ...
    function updateRewardPerBlock(uint256 _amount) external onlyOwner {
        require(_amount <= 50, "Cannot set higher than 50 spurt per Block");
