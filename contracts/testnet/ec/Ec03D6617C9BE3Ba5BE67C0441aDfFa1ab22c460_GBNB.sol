...
abstract contract GToken is EIP20Interface, ExponentialNoError, TokenErrorReporter, ReentrancyGuard {
    ...
    */
     ...
     * @return uint, the max amount that be reduced by the admin from the market
     */
    function estimateMaxReduceReserves() external view returns (uint) {
        return totalReserves - (discountLevel.totalDiscountGiven(address(this)) - totalDiscountReceived);
    }
