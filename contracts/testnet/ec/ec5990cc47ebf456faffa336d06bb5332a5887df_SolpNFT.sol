...
contract SolpNFT is ERC721, Ownable {
    ...
    uint256 public punksRemainingToAssign = 5000;

    constructor(string memory baseURI, address payable _firstOwner)
     ERC721("SolPunks", "SolP") {
    ...
    function mintNFT(uint256 amount) public payable {
        ...
        require(amount <= 5, "You cannot mint more than 5 Nfts per once");
        require(SafeMath.add(totalSupply(), amount) <= MAX_SUPPLY, "Exceeds maximum supply. Please try to mint less Nfts.");
        require(getNFTPrice(amount) == msg.value, "Amount of CET sent is not correct.");
