...
contract MpoFinance is OwnableUpgradeable {
    ...
    function checktoClaimBonus(address user_) public view returns (uint aa) {
        uint p = IMPOT(MPO).checkPhase();
        (bool b, , , , ) = IMPOT(MPO).buyTBonusInfo(p);
        if (b) {
            aa =
    ...
    function claim() public returns (uint rew) {
        ...
        (bool b, , , , ) = IMPOT(MPO).buyTBonusInfo(_phase);
        if (b) {
            rew = checktoClaimBonus(user_);
            if (rew > 0) {
                // uint _ba = IERC20(MPO).balanceOf(address(this));
