...
contract SummitKodaStaking is Ownable, ReentrancyGuard {
  ...
  mapping(address => uint256) public user; // user => amount
  ...
  function setStatus(address _user, uint256 _status) external {
  ...
  function setStatusBoost(uint256 _status, uint256 _boost) external {
  ...
  function claimPremium(address premiumToken) external {
    claimedTokenAmounts[premiumToken][msg.sender] = _claimPremium(premiumToken);
  }

  function _claimPremium(address premiumToken) private returns (uint256) {
    uint256 total = (ratings[premiumToken][msg.sender] * tokenPerRatingPoints[premiumToken]) /
      PREMIUM_PER_RATING_POINT_BASE;
    uint256 _claimedAmount = claimedTokenAmounts[premiumToken][msg.sender];
    ...
    tokensEarned[premiumToken][msg.sender] += rest;
  ...
  function deposit(
    ...
    _claimPremium(kodaToken);
    _claimPremium(kapexToken);
  ...
  function withdrawDeposit(uint256 depositId, uint256 amount) public {
    ...
    if (deposit.isWithdrawable) {
      ...
      claimedTokenAmounts[kodaToken][deposit.user] = _claimPremium(kodaToken);
      claimedTokenAmounts[kapexToken][deposit.user] = _claimPremium(kapexToken);
    }
    ...
    if (deposit.isWithdrawable) {
      ...
      if (claimedTokenAmounts[kodaToken][deposit.user] > amount) {
        claimedTokenAmounts[kodaToken][deposit.user] -= amount;
      } else {
        claimedTokenAmounts[kodaToken][deposit.user] = 0;
      }
    }
