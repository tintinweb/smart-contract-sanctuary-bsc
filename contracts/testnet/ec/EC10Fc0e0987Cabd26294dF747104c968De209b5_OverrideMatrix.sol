...
contract OverrideMatrix is AccessControl {
    ...
    function _addLocationsTo6(address _account, address _vertex, uint256 _grade) internal {
        if (accountInfoList[_vertex].matrix6Grade[_grade].upperLayer[0] == address(0) ||
            ...
            for (uint256 i = 0; i < 2; i++) {
                if (accountInfoList[_vertex].matrix6Grade[_grade].upperLayer[i] == address(0)) {
                    _set6Location(_vertex, _account, _grade, i);
                    address upper = accountInfoList[_vertex].matrix6Grade[_grade].vertex;
                    if (upper == address(0)) {
                        USDToken.transfer(noReferPlatform, currentPrice(_grade));
                        return;
                    } else {
                        if (accountInfoList[upper].matrix6Grade[_grade].upperLayer[0] == _vertex &&
                            (
                            accountInfoList[upper].matrix6Grade[_grade].lowerLayer[0] == address(0) ||
                            accountInfoList[upper].matrix6Grade[_grade].lowerLayer[1] == address(0)
                            )
                        ) {
                            if (i == 0) {
                                _set6Location(upper, _account, _grade, 2);
                                return;
                            }
                            if (i == 1) {
                                _set6Location(upper, _account, _grade, 3);
                                return;
                            }
                        }
                        if (accountInfoList[upper].matrix6Grade[_grade].upperLayer[1] == _vertex &&
                            (
                            accountInfoList[upper].matrix6Grade[_grade].lowerLayer[2] == address(0) ||
                            accountInfoList[upper].matrix6Grade[_grade].lowerLayer[3] == address(0)
                            )
                        ) {
                            if (i == 0) {
                                _set6Location(upper, _account, _grade, 4);
                                return;
                            }
                            if (i == 1) {
                                _set6Location(upper, _account, _grade, 5);
                                return;
                            }
                        }
                    }
    ...
    function _set6Location(address _setKey, address _setValue, uint256 _setGrade, uint256 _setLocation) internal {
        uint256 price = currentPrice(_setGrade);
        if (_setLocation == 0) {
        ...
        if (_setLocation == 2) {
            ...
            USDToken.transfer(_setKey, price);
            return;
        }
        if (_setLocation == 3) {
