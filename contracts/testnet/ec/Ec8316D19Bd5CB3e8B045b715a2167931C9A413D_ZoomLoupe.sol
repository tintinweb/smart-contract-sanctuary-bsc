...
import "./INFTWA.sol";
import "./IZoomLoupe.sol";


contract ZoomLoupe is Ownable, Pausable,IZoomLoupe{
    IContracts_TITIMITI Contr;
    IERC20 MITI;
    IERC20 USDT;
    IERC721 NFTWA;
    ITrue_chest TrueChest;
    ITake Take;
    IERC1155 Details;
    ICollections Coll;
    IColl_NFTWA_NFTA CNF;
    INFTWA NFTWA_get;

    uint256 Decimals =1000000;
    uint256 public NumColl_NFTWA;

    address public addressNFTWA;
    uint256 public ID_NFTWA;

    uint256 _MAXColl_NFTWA=3;

    event TransferSent(address _from, address _destAddr, uint _amount);

constructor() {
        Contr=IContracts_TITIMITI(0x998A99E482DFa7c436a39296B16C8d11e0beBFea);
        CNF=IColl_NFTWA_NFTA(0x36826fc7Ed3F414670175DD845a062754aC6333a);
        USDT = IERC20(0x5E10a91adb891831378A6A035f9B05382B45Aa15);
        update();
    }
    ...
    function Take_chest (uint256 num_Track, uint256 chest_num) public {
       address sender = msg.sender;
        uint256 _MITI = Take.Get_One_chest_MITI(num_Track,chest_num);
        uint256 _USDT = Take.Get_One_chest_USDT(num_Track, chest_num);

        uint256 procent_owner = TrueChest.Get_LevelPlayer_num(sender, num_Track, chest_num);
        uint256 procent_landlord =100-procent_owner;


        MITI.transfer(sender, _MITI*Decimals/100*procent_owner);
        USDT.transfer(sender, _USDT*Decimals/100*procent_owner);

        MITI.transfer(Contr.getLandLord(), _MITI*Decimals/100*procent_landlord);
        USDT.transfer(Contr.getLandLord(), _USDT*Decimals/100*procent_landlord);

         TrueChest.Take_chest(num_Track, chest_num,sender);

            if(chest_num>=4 && NumColl_NFTWA<=_MAXColl_NFTWA){
                addressNFTWA = Coll.GetID_Collection(ID_NFTWA);
                NFTWA_get=INFTWA(address(addressNFTWA));
                uint256 Total_NFTWA_=NFTWA_get.get_Total_NFTWA();
                ID_NFTWA = CNF.Get_NFTWA_num(NumColl_NFTWA);
                uint256 MAX_Col_NFTWA = Coll.Get_Max_id_Collection(ID_NFTWA);

                if (Total_NFTWA_+1>=MAX_Col_NFTWA) {
                    NFTWA_get.Mint_NFTW(sender);
                    NumColl_NFTWA++;
                }
                else if (NumColl_NFTWA<_MAXColl_NFTWA+1){
                    NFTWA_get.Mint_NFTW(sender);
                }
            }
    }

    function Take_detail() public {
        uint256 Summ = TrueChest.Get_detail(msg.sender);
        require(Summ>=1);
        Details.safeTransferFrom(Contr.getStock(),msg.sender, 30, Summ, "");
        TrueChest.take_detail(Summ,msg.sender);
    }
   

    function withdrawToken(address payable adr) public onlyOwner{
    uint256 balanceC = MITI.balanceOf(address (this));
       uint256 balanceT = USDT.balanceOf(address (this));
       MITI.transfer(adr,balanceC);
    ...
    function  update()  public virtual override{
        ...
        TrueChest =ITrue_chest(address(Contr.getTrue_chest()));
        Take = ITake(address(Contr.getTake()));
        Details = IERC1155 (address(Contr.getDetails()));
        Coll = ICollections(address(Contr.getCollections()));
  } 
...
interface INFTWA {
    function get_Max_NFTWA () external view  returns (uint256);
    function get_Total_NFTWA () external view  returns (uint256);
    function update_URL()  external;
    function Mint_NFTW (address adr ) external;
    function get_ID_Collection () external view returns (string memory);
}
