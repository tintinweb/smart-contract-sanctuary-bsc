...
contract ERC20SafePermit {

    IPermit2 public PERMIT2;

    constructor(address permit_) public {
        PERMIT2 = IPermit2(permit_);
    }
    
    function lockERC20Permit(
...
import "../../ERC20SafePermit.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

contract FeeHandlerWithOracle is IFeeHandler, AccessControl, ERC20SafePermit {
    address public _bridgeAddress;
    address public _feeHandlerRouterAddress;
    ...
    constructor(address bridgeAddress, address feeHandlerRouterAddress, address permit) ERC20SafePermit(permit) {
     ...
     */
    function setFeeProperties(uint16 feePercent) external onlyAdmin {
        _feePercent = feePercent;

        emit FeeHandlerSetFeeProperties(address(this), feePercent);
    }
    ...
    function collectFeePermit(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData, IPermit2.PermitTransferFrom memory permit, bytes memory signature) payable external onlyBridgeOrRouter {
        require(msg.value == 0, "collectFee: msg.value != 0");
        (uint256 fee, address tokenAddress) = _calculateFee(sender, fromDomainID, destinationDomainID, resourceID, depositData, feeData);
        ...
        lockERC20Permit(permit, signature, sender, address(this), fee);
        emit FeeCollected(sender, fromDomainID, destinationDomainID, resourceID, fee, tokenAddress);
    }
    ...
    function _calculateFee(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) internal view returns(uint256 fee, address tokenAddress) {
        ...
        // txCost = dstGasPrice * gasUsed * Token Effective Rate (rate of dest base currency to token)
        uint256 txCost = oracleMessage.dstGasPrice * oracleMessage.gasUsed * oracleMessage.ter / 1e18;
...
interface IFeeHandler {
    ...
    function collectFeePermit(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData, IPermit2.PermitTransferFrom memory permit, bytes memory signature) payable external;
