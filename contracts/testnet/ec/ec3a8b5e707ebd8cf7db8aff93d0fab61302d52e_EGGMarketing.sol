...
contract EGGMarketing is Ownable {
    ...
    function checkWhiteList(address[] memory whiteList) public virtual returns(bool){
        bool isHave = false;
        for(uint256 i = 0 ; i < whiteList.length; i ++){
            if(whiteList[i] == msg.sender && !freeMintAccount[msg.sender]){
                isHave = true;
                break;
            }
        }
    ...
    function getComputedHex(address[] memory whiteList) public virtual  onlyOwner returns(bytes32){
