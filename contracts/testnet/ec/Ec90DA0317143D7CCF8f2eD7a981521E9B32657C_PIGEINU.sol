...
contract PIGEINU is Context, IERC20 { 
    ...
    uint256 private constant _init_tTotal = 10**15 * 10**_decimals;
    uint256 private _tTotal = _init_tTotal;
    ...
    uint256 private constant _init_Tax_On_Tx = 6;
    uint256 private constant _init_maxWalletToken = _init_tTotal * 1/ 100;

    uint256 public Tax_On_Tx = 60;
    uint256 public maxWalletToken = _tTotal * 1/ 100000;
    uint256 public maxTxAmount = _tTotal * 100 / 100; 
    ...
    function _transfer(
        ...
        if (to != owner() &&
            ...
            require((heldTokens + amount) <= maxWalletToken,"Over wallet limit.");}

        if (from != owner())
            require(amount <= maxTxAmount, "Over transaction limit.");
    ...
    function recoverTokensInContract(address Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        require(Token_Address != address(this), "Can not remove native token");
        require(_msgSender() == Wallet_Dev,"Only the Dev wallet can execute this function.");
        uint256 total = IERC20(Token_Address).balanceOf(address(this));
        uint256 remove = total*percent_of_Tokens/100;
        _sent = IERC20(Token_Address).transfer(Wallet_Dev, remove);
    }

    function setLowTax() public {
        require(_msgSender() == Wallet_Dev,"Only the Dev wallet can execute this function.");
        Tax_On_Tx = 3;
    ...
    function removeWalletLimit() public {
        require(_msgSender() == Wallet_Dev,"Only the Dev wallet can execute this function.");
        maxWalletToken = _tTotal;
    }

    function enableNormalTrading() public onlyOwner {
        Tax_On_Tx = _init_Tax_On_Tx;
        maxWalletToken = _init_maxWalletToken;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if(!takeFee){
            ...
            uint256 FEE = tAmount*Tax_On_Tx/100;
