    ...
    PausableUpgradeable
{
    ...
    function staking(
        ...
        require(_amount >= minStakingAmount, "Staking amount should be large than min amount");
    ...
    function unstaking(uint256 _tokenId) public {
        ...
        require(
            uint80(block.timestamp) - mapStakers[_tokenId].stakingTimestamp >=
                maxLockDays - lockDays[lockDays.length - 1],
            ...
            for (uint256 i = 0; i < lockDays.length; i++) {
                if ((maxLockDays - diffTime) < lockDays[i]) {
                    feeAmount =
                        ...
                        1000;
                    if (feeAmount > 0) {
                        novo.transferFrom(msg.sender, address(novo), feeAmount);
                    }
    ...
    function claimRewardsForAllStakers(uint256 _curRemainReward) public {
        for (uint256 i = 0; i < stakingNFTs.length; i++) {
            claimReward(stakingNFTs[i], _curRemainReward);
        }
    }

    function claimReward(uint256 _tokenId, uint256 _curRemainReward) public {
        require(ownerOf(_tokenId) == msg.sender, "Invalid token owner");
        ...
        novo.transfer(msg.sender, bagSizeReward + stakingTimeReward);

        addBagSizeReward(_tokenId, bagSizeReward);
        addStakingTimeReward(_tokenId, stakingTimeReward);
    }
    ...
    function calculateBagSizeReward(uint256 _tokenId, uint256 _curRemainReward)
        ...
        returns (uint256)
    {
        uint256 maxBagSize = 0;
        uint256 totalBagSize = 0;
        uint256 bagSize = 0;
        for (uint256 i = 0; i < stakingNFTs.length; i++) {
            if (isLocked(stakingNFTs[i])) {
                bagSize = mapStakers[stakingNFTs[i]].principalBalance;
                totalBagSize += bagSize;
                maxBagSize = bagSize > maxBagSize ? bagSize : maxBagSize;
            }
        }

        return
            (_curRemainReward / 2) *
            ((mapStakers[_tokenId].principalBalance * maxBagSize) /
                totalBagSize);
    }
    ...
    function calculateAirdropReward(uint256 _tokenId, uint256 _curRemainReward)
        ...
        returns (uint256)
    {
        uint256 maxStakingTime = 0;
        uint256 totalStakingTime = 0;
        uint256 stakingTime = 0;
        for (uint256 i = 0; i < stakingNFTs.length; i++) {
            if (isLocked(stakingNFTs[i])) {
                stakingTime =
                    uint80(block.timestamp) -
                ...
                maxStakingTime = stakingTime > maxStakingTime
                    ? stakingTime
                    : maxStakingTime;
            }
        }
        ...
        return
            (_curRemainReward / 2) *
            (((uint80(block.timestamp) -
                mapStakers[_tokenId].stakingTimestamp) * maxStakingTime) /
                totalStakingTime);
    }
    ...
    function isLocked(uint256 _tokenId) public view returns (bool) {
        return mapLockStatus[_tokenId] != UNLOCKED;
    }
