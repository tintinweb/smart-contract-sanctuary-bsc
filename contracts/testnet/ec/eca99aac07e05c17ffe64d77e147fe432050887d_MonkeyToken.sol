...
interface iMonkeyToken {
     ...
     */
    // function convertOldMbyToNew(uint256 index, uint256 amount, bytes32[] calldata merkleProof) external;
...
contract MonkeyToken is iMonkeyToken, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "MonkeyToken: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "MonkeyToken: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __MonkeyToken_v2_init_unchained(_router);
    }

    function __MonkeyToken_v2_init_unchained(address _router) internal initializer {
        _name = "Monkey TOKEN";
        _symbol = "MBY";
        _decimals = 9;
        ...
        numTokensSellToAddToLiquidity = 500000 * 10**9;
        ...
        __MonkeyToken_tiers_init();

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function setNumTokensSellToAddToLiquidity(uint256 tokenAmountNoDecimals) external onlyOwner() {
        numTokensSellToAddToLiquidity = tokenAmountNoDecimals * 10**9;
    }

    function __MonkeyToken_tiers_init() internal initializer {
        _defaultFees = _addTier(0, 200, 200, 0, 0, address(0), address(0));
    ...
    preventBlacklisted(_account, "MonkeyToken: Selected account is in blacklist")
    {
        require(_account != address(0), "MonkeyToken: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "MonkeyToken: Invalid address");
        require(_accountsTier[_account] > 0, "MonkeyToken: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "MonkeyToken: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "MonkeyToken: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "MonkeyToken: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "MonkeyToken: Owner address is blacklisted")
    preventBlacklisted(spender, "MonkeyToken: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "MonkeyToken: Address is blacklisted")
    preventBlacklisted(from, "MonkeyToken: From address is blacklisted")
    preventBlacklisted(to, "MonkeyToken: To address is blacklisted")
    isRouter(_msgSender())
    {
    ...
    function setMigrationFromContractAddress(address _migration) public onlyOwner() {
    ...
    external
    preventBlacklisted(account, "MonkeyToken: Migrated account is blacklisted")
    override
    {
        require(migration != address(0), "MonkeyToken: Migration is not started");
        require(_msgSender() == migration, "MonkeyToken: Not Allowed");
    ...
    */
    ...
    /*
    // OBSOLETE AND OLD
    function convertOldMbyToNew(uint256 amount) external preventBlacklisted(_msgSender(), "MonkeyToken: Migrated account is blacklisted") override {
        // You have to APPROVE spending first, possibly through the MBY SWAP web platform.
        IBEP20 mbyOldToken = IBEP20(migration);
        require(mbyOldToken.allowance(_msgSender(), address(this)) >= amount, "MonkeyTokenV1: Allowance is less than amount");
        require(mbyOldToken.balanceOf(_msgSender()) >= amount, "MonkeyTokenV1: Balance is less than amount");
        mbyOldToken.transferFrom(_msgSender(), address(this), amount);
        _tokenTransfer(_initializerAccount, _msgSender(), amount.div(100000), 0, false);
    }
    */

    /* 
    function convertOldMbyToNew(uint256 index, uint256 amount, bytes32[] calldata merkleProof) external preventBlacklisted(_msgSender(), "MonkeyToken: Migrated account is blacklisted") override {
        bytes32 node = keccak256(abi.encodePacked(index, _msgSender(), amount));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), "MerkleDistributor: Invalid proof.");
        require(!_hasConvertedOldMby[_msgSender()], "MonkeyToken: You have already converted your tokens.");
        if (amount < 100000) {
            amount = 100000;
        }
        
        _hasConvertedOldMby[_msgSender()] = true;

/*        IBEP20 mbyOldToken = IBEP20(migration);
        uint256 oldMbyBalance = mbyOldToken.balanceOf(_msgSender());
        require(mbyOldToken.allowance(_msgSender(), address(this)) >= oldMbyBalance, "MonkeyTokenV1: Allowance is less than all.");
        mbyOldToken.transferFrom(_msgSender(), address(this), oldMbyBalance);
