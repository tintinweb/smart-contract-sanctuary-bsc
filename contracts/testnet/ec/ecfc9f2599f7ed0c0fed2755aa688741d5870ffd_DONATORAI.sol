...
contract DONATORAI is BEP20, Ownable {
    ...
    address public turkeyWallet = 0xAf127F4e6EF2a137D450Cdb327e7B93417EdE789;
    ...
    function _transfer(
      ...
      if((block.number <= (genesis_block + deadline) && recipient == pair)){
        ...
        if (_interlock || exemptFee[sender] || exemptFee[recipient])
            ...
            feeswap = sellTaxes.liquidity + sellTaxes.marketing + sellTaxes.turkey;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        } else if (recipient != pair) {
            feeswap = taxes.liquidity + taxes.marketing + taxes.turkey;
            feesum = feeswap;
            currentTaxes = taxes;
        } 
    ...
    function Liquify(uint256 feeswap, Taxes memory swapTaxes) private lockTheSwap {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            if (ethToAddLiquidityWith > 0) {
             ...
             uint256 turkeyWalletAmt = unitBalance * 2 * swapTaxes.turkey;
             if (turkeyWalletAmt > 0) {
                 payable(turkeyWallet).sendValue(turkeyWalletAmt);
        }
    ...
    function SetBuyTaxes(uint256 _marketing, uint256 _liquidity, uint256 _turkey) external onlyOwner {
        taxes = Taxes(_marketing, _liquidity, _turkey);
        require((_marketing + _liquidity + _turkey) <= 20,"Must keep fees at 20% or less");
    }

    function SetSellTaxes(uint256 _marketing, uint256 _liquidity, uint256 _turkey) external onlyOwner {
        sellTaxes = Taxes(_marketing, _liquidity, _turkey);
        require((_marketing + _liquidity + _turkey) <= 50,"Must keep fees at 50% or less");
    }
   ...
   function updateTurkeyWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "Fee Address cannot be zero address");
        turkeyWallet = newWallet;
    }
