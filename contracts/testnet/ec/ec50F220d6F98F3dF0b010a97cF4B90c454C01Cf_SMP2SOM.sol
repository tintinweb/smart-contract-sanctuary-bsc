...
contract SMP2SOM is Ownable{
    ...
    IERC20 public busd;
    address _busd = 0xa63b7047350b8d6173C308d86903c25A32255BD5;
    constructor() {
         busd = IERC20(_busd);
         withdrawLimit = 40 * 10 ** 18;
    }
   ...
   /**
    ...
    function Claim(uint BUSD_reward, address user ) public onlyOwner {
            require(userTime[user]== 0 || block.timestamp >= userTime[user] + day, "you can not withdraw before Cooldown");
            require(BUSD_reward<= withdrawLimit , "withdraw limit exceeded ");
            if(checkWhitelist(user)){
               if(userCount[user] + BUSD_reward <= somFee *10 ** ERC20(_busd).decimals()) {
                busd.transfer(owner(), BUSD_reward);
               }
               if(userCount[user] > somFee *10 ** ERC20(_busd).decimals()){
                busd.transfer(user, BUSD_reward);
               }
               if(userCount[user] <= somFee *10 ** ERC20(_busd).decimals() && userCount[user] + BUSD_reward >= somFee *10 ** ERC20(_busd).decimals()){
                uint256 feeLeft = somFee *10 ** ERC20(_busd).decimals() - userCount[user];
                busd.transfer(owner(), feeLeft);
                busd.transfer(user, BUSD_reward - feeLeft);
               }
            }
            if(!checkWhitelist(user)){
             busd.transfer(user, BUSD_reward);
            ...
            userCount[user] += BUSD_reward; 
            totalToken += BUSD_reward;
    }
