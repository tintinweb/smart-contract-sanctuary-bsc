    ...
    AccessControlUpgradeable
{
    ...
    function getPendingReward(uint256 _poolId, address _account, uint256 _stakedId) external view returns(uint256 pendingReward) {
        StakingData storage user = userStakingData[_poolId][_account];
        // UserStakingData storage userStaking = user.stakingDatas[_stakedId];
        // uint256 pendingReward = userStaking.pendingReward;
        // return pendingReward;
        return _getPendingReward(_poolId, _account, _stakedId) + user.stakingDatas[_stakedId].pendingReward;
    }
    ...
    function _getPendingReward(uint256 _poolId, address _account, uint256 _stakedId) private view returns(uint256 pendingReward) {
        ...
        uint256 pendingReward = (userStaking.balance * userStaking.APR * (block.timestamp + 2592000 - userStaking.stakeTime)) / (ONE_YEAR * 1e20);
        return pendingReward;
    }
