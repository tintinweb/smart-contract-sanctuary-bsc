    ...
    function getRewardInTHALESForOwner( address addr) internal view returns(uint256){
        ...
        for(uint i=0; i<stake_data[msg.sender].stakeElement.length; i++){
            if(stake_data[msg.sender].stakeElement[i].stakeType == 2)
                totalReward += stake_data[msg.sender].stakeElement[i].amount * stake_eABCD_amount * (block.timestamp - stake_data[msg.sender].stakeElement[i].dateStaked) * THALES_decimal / stake_eABCD_period / eABCD_decimal;
            if(stake_data[msg.sender].stakeElement[i].stakeType == 1)
                totalReward += stake_data[msg.sender].stakeElement[i].amount * stake_BUSD_amount * (block.timestamp - stake_data[msg.sender].stakeElement[i].dateStaked) * THALES_decimal / stake_BUSD_period / BUSD_decimal;
        }
        return totalReward;
    }
