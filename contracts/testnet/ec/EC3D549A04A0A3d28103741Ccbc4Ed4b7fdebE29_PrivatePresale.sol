...
contract PrivatePresale is Crowdsale {
    ...
    uint public totalWhitelistedUser;
    ...
    function getPurchasableAmount(address account, uint amount) public view returns (uint) {
        ...
        return amount - purchasedAmount[account];
    }
    ...
    function percentVested() public view returns ( uint percentVested_ ) {
        ...
        if(block.timestamp < closingTime)
            return 0;
        uint phaseCompleted = (block.timestamp.sub(closingTime)).div(phaseLength).add(4);
    ...
    function whitlelistMultiple (address[] memory accounts, bool whitelist ) external onlyOwner {
        uint256 count = accounts.length;
        totalWhitelistedUser = totalWhitelistedUser.add(count);
        for(uint256 i = 0; i< count; i++) {
            whitelisted[accounts[i]] = whitelist;
        }
    }

    function whitlistAccount(address account, bool whitelist) external onlyOwner {
