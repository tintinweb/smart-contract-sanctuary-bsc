...
contract CCL is Context, Ownable, ReentrancyGuard {
   ...
   IERC20 private WBNBToken = IERC20(address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd)); // WBNB CONTRACT TEST NET
    ...
    function _isSusAddress(address _addr) external view LTSHOLDER() returns (bool) { return isSuspected[_addr]; } // RETURNS SUSPECTED ADDRESSES
    ...
    function bnbFEE(uint newFee) external nonReentrant() onlyOwner() { 
        ...
        // JIMPO BIMBO
    function checkLTSAllowance() internal view { require(LTSToken.allowance(_msgSender(), address(this)) >= LTSCost, "Not enough LTS allowance");}
    function checkWBNBAllowance() internal view { require(WBNBToken.allowance(_msgSender(), address(this)) >= BNBCost, "Not enough BNB allowance");}
