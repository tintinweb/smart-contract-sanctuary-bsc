...
constructor(address _adminAddress)  {
        ...
        plans[0].HelpAmount = 500;
        plans[0].HelpAdminAmount = 100;
        ...
        plans[1].HelpAmount = 1000;
        plans[1].HelpAdminAmount = 200;
        ...
        plans[2].HelpAmount = 2500;
        plans[2].HelpAdminAmount = 500;
        ...
        plans[3].HelpAmount = 5000;
        plans[3].HelpAdminAmount = 1000;
...
function Donation(uint256 _planId, address _phAddress, uint256 _timestamp) public virtual;

function referralDonation(uint256 _planId,address _phAddress,uint256 _timestamp, address _referrer) public virtual; 
...
contract TronExpert is TronAbstract{
    ...
    struct Referral {
        address referrer;
        address[] referees;
        mapping(address => uint256[][]) referralHelps;    //address -> [][]plan  count
    ...
    function referralDonation(uint256 _planId,address _phAddress,uint256 _timestamp, address _referrer) public override {
        require(_planId < planLimit, "Sorry, Help Plan not found!!");
        require(_phAddress != address(0), "Sorry, Help address cannot be zero!!");
        require(_timestamp <= block.timestamp , "Sorry, Invalid timestamp!!");
        require(_referrer!=_phAddress, "You can't refer yourself");
        require(!isActive(_phAddress,_planId),"You are already active in this plan");
        
        if(referrals[_phAddress].referrer == address(0)) {
            referrals[_phAddress].referrer = _referrer;
            referrals[_referrer].referees.push(_phAddress);
        }
         Plan storage plan = plans[_planId];
        require( !plan.Concluded, "Plan has ended for this Help pool");
        
        uint256 phIndex = ProvideHelpIndex(_planId);
       
        if(plan.HelpHash[phIndex].gotHelp1 == address(0)){
            plan.HelpHash[phIndex].gotHelp1= _phAddress;
        }
        else{
            plan.HelpHash[phIndex].gotHelp2= _phAddress;
        }
       
        PH storage help = plans[_planId].HelpHash.push();
        help.HelpAddress = _phAddress;
        help.dateAt = _timestamp;
        plan.TotalHelpCount += 1;
        plan.TotalHelpAmount += plan.HelpAmount;
         
        helpLists[_phAddress][_planId].push(plan.HelpHash.length - 1);
        
        if(referrals[_phAddress].referrer != address(0)) {
            address _referrerr = referrals[_phAddress].referrer;
            if(referrals[_referrerr].referralHelps[_phAddress].length == 0){
            referrals[_referrerr].referralHelps[_phAddress].push();
            referrals[_referrerr].referralHelps[_phAddress].push();
            referrals[_referrerr].referralHelps[_phAddress].push();
            referrals[_referrerr].referralHelps[_phAddress].push();
            }
            referrals[_referrerr].referralHelps[_phAddress][_planId].push(plan.HelpHash.length - 1);
    ...
    function Donation(uint256 _planId,address _phAddress,uint256 _timestamp) public override {
    ...
    function getLevelReferralEarningsData(address _referrer, uint256 _level) public view returns(
        ...
        uint256[][][] memory _referralDonations;
        ...
        if(_level < referralLevels && _referrer != address(0)) {
            ...
            uint256[][][] memory _nextReferralDonations;
    ...
    function getSingleLevelReferralEarningsData(address _referrer, uint256 _level) public view returns(
        ...
        {     
            ...
            _referralHelps = new uint256[][][](_referees.length);
            uint256 count = MAXRLIMIT <= _referees.length ? MAXRLIMIT : _referees.length;
            for(uint256 i = 0; i < count; i++) {  
                _levels[i] = _level;
    ...
    function concatenateReferralDonations(uint256[][][] memory a1, uint256[][][] memory a2) internal pure returns(uint256[][][] memory) {
