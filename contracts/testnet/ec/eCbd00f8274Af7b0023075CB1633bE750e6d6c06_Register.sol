...
contract Register is Ownable {
    ...
    mapping(address => string) public refFromAdd;
    mapping(string => address) public addFromRef;
    ...
    function registerWithoutSponsor(string memory _ownRef) external payable {
        require(!_pause, "Register Paused!");
        require(msg.value >= 0.18 ether, "Insufficient funds.");
        require(addFromRef[_ownRef] == address(0), "Duplicated Referral Code.");
        require(keccak256(abi.encodePacked(refFromAdd[msg.sender])) == keccak256(abi.encodePacked("")), "Duplicated Address.");
        refFromAdd[msg.sender] = _ownRef;
        addFromRef[_ownRef] = msg.sender;
    }

    function registerWithSponsor(string memory _ownRef, string memory _sponsorRef) external payable {
        require(!_pause, "Register Paused!");
        require(msg.value >= 0.06 ether, "Insufficient funds.");
        require(addFromRef[_sponsorRef] != address(0), "Invalid Sponsor Address.");
        require(addFromRef[_ownRef] == address(0), "Duplicated Referral Code.");
        require(keccak256(abi.encodePacked(refFromAdd[msg.sender])) == keccak256(abi.encodePacked("")), "Duplicated Address.");

        address _sponsorAdd = addFromRef[_sponsorRef];
        refFromAdd[msg.sender] = _ownRef;
        addFromRef[_ownRef] = msg.sender;
    ...
    function getDownline(string memory _ownRef) public view returns(
                                        uint256 first_level, 
        ...
        address _ownAdd = addFromRef[_ownRef];
    ...
    function getGenealogicalTree(string memory _ownRef) public view returns(
        ...
        address _ownAdd = addFromRef[_ownRef];
        (, uint256 level2Cnt, uint256 level3Cnt, uint256 level4Cnt) = getDownline(_ownRef);
