...
contract HeroNFT is BaseNFT {
  ...
  function _beforeTokenTransfer(
    ...
    if (_to == address(0)) {
      ...
      if (!exclusives[_to]) {
        ...
        if (_from == address(0)) {
          point = 1;
          energyManager.updatePoint(_to, point);
        } else if (!reachMaxProfit(_tokenId)) {
          point = int(heroes[_tokenId].level);
          energyManager.updatePoint(_to, point);
        }
  ...
  function _mint(address _owner, uint16 _classId, uint _price, uint _index) private returns (uint) {
    ...
    fotaOwnPrices[newId] = _convertToFOTAOwnPrice(_price);
    return newId;
  }

  function _levelUp(uint _tokenId) private {
    heroes[_tokenId].level += 1;
    marketPlace.increaseCurrentRentedHeroCounter(_tokenId);
  ...
  function getOwnerTotalLevelThatNotReachMaxProfit(address _owner) external view returns(uint) {
    ...
    for(uint i = 0; i < ids.length; i++) {
      if (!reachMaxProfit(ids[i])) {
        totalLevel += uint(heroes[ids[i]].level);
      }
    }
    return totalLevel;
  }
  ...
  function updateOwnPrice(uint _tokenId, uint _ownPrice) override onlyMintAdmin external {
    ...
    uint tokenOwnPrice = _convertToFOTAOwnPrice(_ownPrice);
  ...
  function increaseTotalProfited(uint _tokenId, uint _amount) onlyMintAdmin external returns (uint) {
    ...
    uint profitAble = _amount;
    if (hero.totalProfited + _amount >= maxProfitable) {
      profitAble = maxProfitable - hero.totalProfited;
    }
    hero.totalProfited += profitAble;
    (uint fotaTotalProfited, uint fotaProfitAble) = _increaseTotalFOTAProfited(_tokenId, _amount, maxFOTAProfitable);
    emit TotalProfitedUpdated(_tokenId, hero.totalProfited, fotaTotalProfited);
  ...
  function skillUp(uint _tokenId, uint8 _index) external onlyUpgradingContract {
    ...
    _levelUp(_tokenId);
    if (!reachMaxProfit(_tokenId)) {
      energyManager.updatePoint(ownerOf(_tokenId), 1);
    }
  ...
  function _convertToFOTAOwnPrice(uint _ownPrice) private view returns (uint) {
    return _ownPrice * 1000 / fotaPricer.fotaPrice();
  }

  function _increaseTotalFOTAProfited(uint _tokenId, uint _amount, uint _maxFOTAProfitable) private returns (uint, uint) {
    uint fotaProfitable = _amount;
    if (heroTotalProfitedInFota[_tokenId] + _amount >= _maxFOTAProfitable) {
      fotaProfitable = _maxFOTAProfitable - heroTotalProfitedInFota[_tokenId];
    }
    heroTotalProfitedInFota[_tokenId] += _convertUsdToFota(fotaProfitable);
    return (heroTotalProfitedInFota[_tokenId], fotaProfitable);
  }

  function _convertUsdToFota(uint _amount) private returns (uint) {
