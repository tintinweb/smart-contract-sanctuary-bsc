...
contract Token is Context, IBEP20, Ownable, Pausable {
  ...
  function setBurnPercentage(uint256 per) public onlyOwner returns (uint256) {
      uint256 percent = per * 100;
      burnper = percent;
      return burnper;
  }
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(!frozenAccount[msg.sender]);
    uint256 amtburn = (uint(burnper) * amount) / 100;
