...
contract LotteryFactory is AccessControl {
    ...
    address aggregatorBTC = 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada; // munbai
    ...
    constructor(address _aggregatorBTC, address _aggregatorETH, address _currentAggregator) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        aggregatorBTC = _aggregatorBTC;
    ...
    function _mintLotteryTicket(address _owner, uint8 _amountTicket) internal {
        ...
        for (uint i = 0; i < _amountTicket; i++) {
            ...
            uint256 seed = Random.computerSeed(0, aggregatorBTC, aggregatorETH);
    ...
    */
     ...
     */
    function setAggregatorBtc(address _aggregator) external onlyRole(MINTER_ROLE) {
        aggregatorBTC = _aggregator;
    }
    ...
    function setTimeValid(uint256 _time) external onlyRole(MINTER_ROLE) {
        timeValid = _time;
        lotteryToday[_lotteryIdCounter.current()].timeValid = uint32(_time);
    }
