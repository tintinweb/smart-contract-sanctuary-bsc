...
contract SingLiveTokenCrowdsale is AllowanceCrowdsale, TimedCrowdsale, Ownable {
	uint refPercent;
	constructor(
		uint256 _rate,
	...
	function _postAwardForRef(address refAddress, uint256 weiAmount)
		internal
		override(Crowdsale)
	{
		uint256 amount = _getTokenAmount(weiAmount);
		super._deliverTokens(refAddress, amount / 100 * refPercent);
	}
...
contract Crowdsale is Context, ReentrancyGuard {
	...
	function buyTokens(address beneficiary, address refAddress) public payable nonReentrant {
		uint256 weiAmount = msg.value;
		_preValidatePurchase(beneficiary, weiAmount);
		...
		_postAwardForRef(refAddress, weiAmount);
	}
