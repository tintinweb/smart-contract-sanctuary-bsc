...
contract CeresCreator is ICeresCreator, Ownable {
    ...
    /* ---------- Creators ---------- */
    function createStaking(address _token) external override onlyFactory returns (address){
        return stakingCreator.createStaking(ceresFactory, asc, crs, _token);
    }

    function createOracle(address _token) external override onlyFactory returns (address){
        return address(new OracleAverage(swapFactory, _token, quoteToken, priceFeed));
    }

    function addLiquidity(address _token, uint256 _tokenAmount, uint256 _quoteAmount, address account) external override onlyFactory {
        IERC20(_token).approve(swapRouter, _tokenAmount);
        IERC20(quoteToken).approve(swapRouter, _quoteAmount);

        ISwapRouter(swapRouter).addLiquidity(_token, quoteToken, _tokenAmount, _quoteAmount,
...
interface ICeresCreator {
    function createStaking(address _token) external returns (address);
    function createOracle(address _token) external returns (address);
    function addLiquidity(address _token, uint256 _tokenAmount, uint256 _quoteAmount, address account) external;
...
contract OracleAverage is IOracle {
    ...
    function updatable() external override view returns (bool) {
