...
contract PrivateSale is Auth {
  ...
  function joinPool() public payable {
    ...
    investorInfos[msg.sender] = InvestorInfo(block.timestamp, 0, total, total, 0, 0);
  ...
  function getContractMampBalance() external view returns (uint256) {
    require(MambaContract.balanceOf(address(this)) > 0, 'PrivateSale: balance is 0');
    return MambaContract.balanceOf(address(this));
  }

  function getDepositedBNB() external view returns (uint256) {
    return depositedBNB;
  }
