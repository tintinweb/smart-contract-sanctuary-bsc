...
contract LLAW {
    ...
    constructor(
        ...
        //矿池合约地址
        _mint(_stakingPoolAddress, 2000_0000 * 10**18);
        //白名单
        _mint(_whitelistAddress, 20_0000 * 10**18);
        //流动性添加地址
        _mint(_liquidityAddress, 20_0000 * 10**18);
        //技术
        _mint(_devAddress, 5_0000 * 10**18);
        //运营
        _mint(_operatorAddress, 5_0000 * 10**18);
        //空投锁仓合约地址
        _mint(_airdropLockAddress, 50_0000 * 10**18);
    ...
    function _transfer(
        ...
        if (to == DEAD_ADDRESS) {
            ...
            if (block.timestamp > startTime + 1 hours) {
                ...
                if(IStakePool(stakingPoolAddress).updateReward(destroyAmount)){
                    emit UpdateReward(destroyAmount, block.timestamp);
                }
            }
        }
        if (to != pairAddress && from != pairAddress) {
            ...
            if (feeWhitelist[from]) {
                ...
                balanceOf[from] -= amount;
                balanceOf[DEAD_ADDRESS] += toBurn;
                emit Transfer(from, DEAD_ADDRESS, toBurn);
                ...
                if (
                    ...
                    inviter[from] != address(pairAddress) &&
                    inviter[from] != address(stakingPoolAddress)
                ) {
                    balanceOf[inviter[from]] += toInviterFee;
                } else {
                    balanceOf[DEAD_ADDRESS] += toInviterFee;
                    emit Transfer(from, DEAD_ADDRESS, toInviterFee);
                    inviter[to] = from;
                }
