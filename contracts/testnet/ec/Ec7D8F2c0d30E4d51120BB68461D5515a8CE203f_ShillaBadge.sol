...
contract ShillaBadge is ERC721, Ownable {
    ...
    uint8 public highestBadgeLevel;

    mapping(uint8 => Badge) badges;
    mapping(uint256 => uint8) private badgeLevelOf;
    ...
    constructor(address _token) ERC721("Shilla Army Badge", "ShillaBadge") {
        token = IShilla(_token);

        highestBadgeLevel = 1;
        badges[highestBadgeLevel].minShillaRequired = 10;
        badges[highestBadgeLevel].maxSupply = 1000;

        highestBadgeLevel = 2;
        badges[highestBadgeLevel].minShillaRequired = 100;
        badges[highestBadgeLevel].maxSupply = 100;

        highestBadgeLevel = 3;
        badges[highestBadgeLevel].minShillaRequired = 1000;
        badges[highestBadgeLevel].maxSupply = 10;
    }
    ...
    function mint(uint8 badgeLevel, address to) external returns (uint256 id) {
        ...
        //badgeOwnedBy[to] = id;
        emit BadgeMinted(id, to, badgeLevel);
    }
    ...
    function getUnAllocatedBadges() external view returns(uint8[] memory _badgeLevels, uint32[] memory counts) {
        uint8 j;
        for(uint8 i = 1; i <= highestBadgeLevel; i++) {
            j = i - 1;
            _badgeLevels[j] = i;
            counts[j] = badges[i].maxSupply - badges[i].totalMinted;
        }
    }
