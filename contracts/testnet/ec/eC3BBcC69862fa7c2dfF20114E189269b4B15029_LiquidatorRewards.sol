...
contract LiquidatorRewards is ILiquidatorRewards, Owned, MixinSystemSettings, ReentrancyGuard {
    ...
    function updateEntry(address account) public {
        ...
        ILiquidatorRewardsAggregator aggregator = liquidatorRewardsManager().getAggregator(collateralKey);
        aggregator.sendUpdateEntry(synthetixDebtShare().balanceOfCrossChain(account), account);
    }
    ...
    function notifyRewardAmount(uint256 reward) external onlySynthetix {
        ...
        ILiquidatorRewardsAggregator aggregator = liquidatorRewardsManager().getAggregator(collateralKey);
        aggregator.sendNotifyReward(reward.divideDecimal(sharesSupply));
    }
...
interface ILiquidatorRewardsManager {
    ...
    function getAggregator(bytes32 key) external view returns (ILiquidatorRewardsAggregator);

    function getAggregators() external view returns (bytes32[] memory);
...
interface ILiquidatorRewardsAggregator {
    function sendUpdateEntry(uint256 debtShare, address account) external;

    function sendNotifyReward(uint256 increasedShare) external;
...
interface IRewardEscrowV2Manager {
    ...
    function getAggregator(bytes32 key) external view returns (IRewardEscrowV2Aggregator);
...
interface IRewardEscrowV2Aggregator {
    function sendAppend(address account, uint256 amount) external;

    function sendVest(address account, uint256 amount) external;
