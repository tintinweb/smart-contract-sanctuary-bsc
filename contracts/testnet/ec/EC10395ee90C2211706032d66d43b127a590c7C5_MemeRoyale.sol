...
contract DividendDistributor is IDividendDistributor {
    ...
    function deposited(uint256 amount) external override onlyToken {
...
contract MemeRoyale is BEP20, Auth {
    ...
    uint256 public swapThreshold = TOTAL_SUPPLY.div(100000);
    ...
    constructor()
        // 0 = Router, 1 = Pink Anti Bot, 2 = Marketing
        Auth(msg.sender)
        BEP20(TOKEN_NAME, TOKEN_SYMBOL)
    {
        ...
        _feeExempt[address(distributor)] = true;

        _reflectionExempt[pair] = true;
        _reflectionExempt[address(this)] = true;
        _reflectionExempt[ADDR_DEAD] = true;
        _reflectionExempt[ADDR_ZERO] = true;
        _reflectionExempt[address(distributor)] = true;

        _maxWalletExempt[msg.sender] = true;
        _maxWalletExempt[address(this)] = true;
        _maxWalletExempt[ADDR_DEAD] = true;
        _maxWalletExempt[marketingWallet] = true;
        _maxWalletExempt[address(distributor)] = true;

        _maxSellTxSizeExempt[msg.sender] = true;
        _maxSellTxSizeExempt[address(this)] = true;
        _maxSellTxSizeExempt[ADDR_DEAD] = true;
        _maxSellTxSizeExempt[marketingWallet] = true;
        _maxSellTxSizeExempt[address(distributor)] = true;

        _sellCooldownExempt[msg.sender] = true;
        _sellCooldownExempt[address(this)] = true;
        _sellCooldownExempt[marketingWallet] = true;
        _sellCooldownExempt[address(distributor)] = true;
    ...
    function _swapBack() internal {
        ...
        // Deposit to distributor and tell it how much we added
        super._transfer(address(this), address(distributor), amountToReflect);
        try distributor.deposited(amountToReflect) {} catch {
            // Reclaim on failure
            super._transfer(
                address(distributor),
                address(this),
     ...
     */
    function setReflectionExempt(address holder, bool exempt)
        external
        authorized
    {
    ...
    function setDistributionSettings(
        uint256 nextGas,
        uint256 nextMinPeriod,
        uint256 nextMinDistribution
    ) external authorized {
    ...
    function setAmm(address amm, bool isMaker) external authorized {
        ...
        amms[amm] = isMaker;
        _reflectionExempt[amm] = isMaker;

        if (isMaker) {
            distributor.setShare(amm, 0);
        } else {
            distributor.setShare(amm, balanceOf(amm));
        }
    }
