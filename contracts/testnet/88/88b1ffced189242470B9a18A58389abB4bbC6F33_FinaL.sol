...
contract Stakeable {
  ...
  uint16 public _thirdStackTypeDuration   = 168;
...
contract FinaL is Context, IERC20, Ownable, Stakeable {
  ...
  function addressFrozenAmount(address sender) public view returns(uint256){
    if (_privateSaleAmount[sender] > 0) {
      uint256 quarter = (block.timestamp - airdropEndTime) / 7776000;
      if (quarter < 3) {
  ...
  // function setFrozenAmountForAddress(address sender) private{
  //   if ()
  // }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    //TODO: add _addressFrozenAmount fixer
