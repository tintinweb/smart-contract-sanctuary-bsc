...
contract RedeemGear is AccessControl {
    using SafeMath for uint;
    using SafeBEP20 for IBEP20;
    IERC721 public gearNFT;
    address public blacklist;
    address public gear;
    constructor( address _gearNFT, address _blacklist) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
		gearNFT = IERC721(_gearNFT); // Token Gear Assets
    ...
    function redeemGear(uint256[] memory tokenId) external {
        BlackList list = BlackList(blacklist);
        for(uint256 i = 0; i < tokenId.length; i++){
            require(list.getTokenId(gear,tokenId[i]) < block.timestamp, "TokenId in blacklist");
            gearNFT.burn(msg.sender, tokenId[i]);
            emit redeemgear(
                msg.sender,
                tokenId[i],
                block.timestamp
