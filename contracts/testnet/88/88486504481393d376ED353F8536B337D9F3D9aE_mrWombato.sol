...
/// @title mrWOM
/// @author Wombato Team
/// @notice mrWOM is a token minted when 1 wom is locked in wombato
contract mrWombato is Initializable, ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using SafeERC20 for IERC20;

    /* ============ State Variables ============ */

    address public sweep_recipient; // multisig treasury
    ...
    event mrWomMinted(address indexed user, uint256 amount);
    event WombatStakingSet(address indexed _wombatStaking);
    event WomConverted(uint256 _womAmount, uint256 _veWomAmount);
    event UpdateSweepRecipient(address indexed _proposed_recipient);
    ...
    function initialize(address _wombatStaking, address _wom, address _sweep_recipient) public initializer {
        __ERC20_init("mrWOM", "mrWOM");
        __Ownable_init();
        wombatStaking = _wombatStaking;
        wom = _wom;
        totalConverted = 0;
        totalAccumulated = 0;
        sweep_recipient = _sweep_recipient;
    ...
    function convert(uint256 _amount) whenNotPaused  external {
        _convert(_amount, true);
    }

    function set_sweep_recipient(address _proposed_recipient) external onlyOwner {
        sweep_recipient = _proposed_recipient;
        emit UpdateSweepRecipient(_proposed_recipient);
    }

    function sweep(address _token, uint256 _amount) external {
        require(msg.sender == sweep_recipient, "Only sweep recipient.");
        ...
        IERC20(_token).safeTransfer(sweep_recipient, amount);
            
    }

    /* ============ Internal Functions ============ */

    function _convert(uint256 _amount, bool _doConvert) whenNotPaused nonReentrant internal {
        if (_doConvert) {
        ...
        emit mrWomMinted(msg.sender, _amount);
    }
