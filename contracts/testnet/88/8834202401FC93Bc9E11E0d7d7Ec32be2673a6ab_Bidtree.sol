...
contract Bidtree is Ownable {
    ...
    /// Variables
    Counters.Counter _contributionsCounter; // Bids number counter
    Counters.Counter _refundsCounter; // Refunded bids number counter
    ...
    address private _router = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; // DEX router address
    ...
    mapping(uint => bool) private _gifted; // Mapping coffee number with is it gifted or not

    /// Structurs
    struct Bid {
        uint128 _price; // Bid price
        uint128 _to_fund; // USD amount sent to fund
        uint128 _btc_rate; // BTC rate when the bid was contributed
        uint128 _referrals_left; // Referrals left
    ...
    struct Contribution {
        ...
        Counters.Counter _bidsCounter; // User's bids counter
    }

    /// Constructor
    constructor(address usd, address bitcoin, uint waitingTime, uint depositAmount,
        uint percentageOwner, uint percentageMarketing, uint percentageLottery, uint percentageReferral, uint percentageFund, uint numDiscounts) {
        _usd = usd;
        _bitcoin = bitcoin;
    ...
    function getActiveReferrals(address account, uint bid) external view onlyOwner returns (uint){
        return _contributions[account]._bids[bid]._referrals_left;
    }
    /// @notice Getting count of account bids
    /// @return (uint)
    function getCountBid(address account) external view returns (uint){
        return _contributions[account]._bidsCounter.current();
    }
    ...
    /// @notice Check if number is a power of two
    ...
    function contribute(uint multi, bool base, address referral, uint bid) external {
        ...
        require(isPowerOfTwo(multi + 1) == true, "Multi+1 should be a power of two");
        require(multi < 1024, "Maximum size of the bid is 1024");
        Bid storage b_referral = _contributions[referral]._bids[bid];
        if (base == false) {
            require(b_referral._referrals_left > 0);
        }
        require(!locker);
        locker = true;
        ...
        _bankOwner += _toOwner;
        _payments[_contributionsCounter.current()] = msg.sender;
        _multi[_contributionsCounter.current()] = multi + 1;
        _gifted[_contributionsCounter.current()] = false;
        _contributionsCounter.increment();

        Contribution storage c_sender = _contributions[msg.sender];
        Bid storage b_sender = _contributions[msg.sender]._bids[c_sender._bidsCounter.current()];

        b_sender._referrals_left = uint128(multi + 1);
        ...
        if (base == true) {
            ...
            IERC20(_usd).safeTransferFrom(msg.sender, address(this), (_deposit - _referral - _refund));
            b_referral._referrals_left --;
        ...
        c_sender._bidsCounter.increment();
        if (_discountsNum > 0 && base == true) {
            _discountsNum --;
        }
        locker = false;
    ...
    function offsetContribution(uint number) external {
        ...
        uint _unclose = c_sender._bids[number]._referrals_left;
        ...
        uint _new_share = (c_sender._bids[number]._to_fund / c_sender._bids[number]._btc_rate) * _price;
        ...
        if (_new_share < _amount) {
            ...
            c_sender._bids[number]._referrals_left = 0;
    ...
    function startLottery() external onlyOwner {
        ...
        _randomNum = number;
        int count;
        for (uint i = _startLotteryNum + 1; i <= _contributionsCounter.current(); i++) {
            if (_gifted[i] == false) {
                count += int(_multi[i]);
            }
        }
        int _num = int(number) % count;
        address _winner;
        for (uint i = _startLotteryNum + 1; i <= _contributionsCounter.current(); i++) {
            if (_gifted[i] == true) {
                continue;
        ...
        emit Lottery(number, _winner, _bankLottery);
        _bankLottery = 0;
        _startLotteryNum = _contributionsCounter.current();
    }
    /// @notice Contribute
    /// @param multi - Referrals amount - 1
    ...
    function giftContribute(uint multi, address recipient) external onlyOwner {
        ...
        _payments[_contributionsCounter.current()] = recipient;
        _multi[_contributionsCounter.current()] = multi + 1;
        _gifted[_contributionsCounter.current()] = true;
        _contributionsCounter.increment();

        Contribution storage c_sender = _contributions[recipient];
        Bid storage b_sender = _contributions[recipient]._bids[c_sender._bidsCounter.current()];
    ...
    /// @notice Withdraw Marketing bank
