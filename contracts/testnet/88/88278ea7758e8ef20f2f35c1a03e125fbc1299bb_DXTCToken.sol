...
contract DXTCToken is IERC20, Liquifier {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 getPercentSupply =  ( 2 * getCirculationalSupply() ) / 100 ;
        require(amount > getPercentSupply, "Transfer amount must be greater than 2% of circulation supply");
    ...
    function setForge(address _address) external onlyOwner() {
        _forge = _address;
    }

    function getCirculationalSupply() public view returns (uint256) {
        uint256 contractBurnBalance = balanceOf(0x000000000000000000000000000000000000dEaD);
        uint256 contractForgeBalance = balanceOf(_forge);
        uint256 circulationalSupply = totalSupply() - contractBurnBalance - contractForgeBalance;
        return circulationalSupply;
    }
}