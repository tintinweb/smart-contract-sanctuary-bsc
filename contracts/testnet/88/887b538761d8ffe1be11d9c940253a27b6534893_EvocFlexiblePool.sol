...
interface IEvocPool {
    struct UserInfo {
        ...
        uint256 evocAtLastUserAction; // keep track of evoc deposited at the last user action.
...
// File: contracts/EvocFlexiblePool.sol

contract EvocFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 evocAtLastUserAction; // keeps track of evoc deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Evoc token
    IEvocPool public immutable evocPool; //Evoc pool
    ...
    //When call evocpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositEvoc(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    /**
     * @notice Constructor
     * @param _token: Evoc token contract
     * @param _evocPool: EvocPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_evocPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the Evoc Flexible Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in EVOC)
     */
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            uint256 earnAmount = totalAmount - user.evocAtLastUserAction;
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    evocPool.withdrawByAmount(withdrawAmount);
        ...
        user.evocAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        emit DepositEvoc(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Evoc Flexible Pool
    ...
    /**
     * @notice Withdraws from Evoc Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking evoc");
        staking = false;
        evocPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Evoc Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in EvocPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = evocPool.userInfo(address(this));
        uint256 pricePerFullShare = evocPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into EvocPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            evocPool.deposit(bal, 0);
        }
    }
}