...
contract Marketplace721 is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createListing(
        ...
        IERC721Upgradeable(collection).transferFrom(msg.sender, address(this), tokenId);
    ...
    function cancel(uint256 listingId) external {
        ...
        IERC721Upgradeable(listings[listingId].collection).safeTransferFrom(address(this), msg.sender, listings[listingId].tokenId);
    ...
    function boost(uint256 listingId_, uint256 amount_) external {
        ...
        listings[listingId_].currency.safeTransferFrom(msg.sender, boostAccount, amount_);

        emit Boosted(listingId_, listingsAdditional[listingId_].boost);
    }
    ...
    function _buy(uint256 listingId) private {
        ...
        emit ListingBought(listingId, msg.sender, listings[listingId].seller, listings[listingId].minimalBid);
    }
    ...
    function _bid(uint256 listingId, uint256 amount) private {
        ...
        emit ListingBid(listingId, msg.sender, listings[listingId].seller, amount, listings[listingId].timeEnd);
    }
