...
contract MoonToken is Context, IERC20, Ownable {
    ...
    address payable _marWalletAddress = payable(0xaC1256f355Dbed31cF18cAF50e6e32EDaFDcE269);
    address payable _charWalletAddress = payable(0xa8D44f53D57dfD77CF88Cd42735F52cEe924352F);
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _DexFee,address routerAddress,address feeaddress,address tokenOwner,address service) payable {
        ...
        minimumTokensBeforeSwap = (_tTotal * 5 / 10000) * 10 ** _decimals;
    ...
    function setDevWalletAddress(address _devaddr) public onlyOwner {
        _devWalletAddress = _devaddr;
    }

    function setMarWalletAddress(address _maraddr) external onlyOwner() {
        _marWalletAddress = payable(_maraddr);
    }

    function setCharWalletAddress(address _charaddr) external onlyOwner() {
        _charWalletAddress = payable(_charaddr);
    }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            ...
            if (buyBackEnabled && balance > uint256(1 * 10**18)) {
                ...
                buyBackTokens(balance.div(100).mul(3));
            }
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(_marWalletAddress, transferredBalance.div(_liquidityFee).mul(feeDivisor));
        transferToAddressETH(_charWalletAddress, transferredBalance.div(_liquidityFee).mul(feeDivisor));
    }
    ...
    function setNumTokensSellToAddToLiquidity(uint256 amountToUpdate) external onlyOwner {
        minimumTokensBeforeSwap = amountToUpdate;
    }
