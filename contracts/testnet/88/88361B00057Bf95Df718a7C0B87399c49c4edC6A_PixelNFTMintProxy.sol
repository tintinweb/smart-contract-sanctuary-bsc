...
import "../interface/IPixelNFTFactory.sol";
...
contract PixelNFTMintProxy is Governance, IGegoRuleProxy{
    ...
    IPixelNFTFactory public _factory = IPixelNFTFactory(address(0));
        ...
        onlyGovernance
    {
     ...
     function setFactory( address factory )         
        ...
        onlyGovernance 
     {
        _factory = IPixelNFTFactory(factory);
     }

    function cost( MintParams calldata params) external override returns (  uint256 mintAmount,address mintErc20 ){
        require( _factory == IPixelNFTFactory(msg.sender)," invalid factory caller" );
       (mintAmount,mintErc20) =  _cost(params);
    } 

    function destroy(  address owner, IPixelNFT.Gego calldata gego) external override {
        require( _factory == IPixelNFTFactory(msg.sender)," invalid factory caller" );
    ...
    function generate( address user , uint256 ruleId, uint256 randomNonce ) external override view returns (  IPixelNFT.Gego memory gego ){
        require( _factory == IPixelNFTFactory(msg.sender) ," invalid factory caller" );
...
import "../interface/IPixelNFT.sol";

interface IPixelNFTFactory {
    ...
    function getGegoStruct(uint256 tokenId)
        external view
        returns (IPixelNFT.Gego memory gego);
...
interface IGegoRuleProxy  {
    ...
    function destroy( address owner, IPixelNFT.Gego calldata gego ) external ;

    function generate( address user,uint256 ruleId, uint256 randomNonce ) external view returns ( IPixelNFT.Gego memory gego );
