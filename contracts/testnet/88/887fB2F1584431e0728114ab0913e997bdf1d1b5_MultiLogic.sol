...
contract MultiLogic is LogicUpgradeable {
     ...
     */
    function setStrategy(singleStrategy[] calldata _multiStrategy)
        external
        onlyOwnerAndAdmin
    {
    ...
    function takeToken(uint256 _amount, address _token) external {
        ...
        IStorage(storageContract).takeToken(_amount, _token, msg.sender);
    ...
    function returnToken(uint256 _amount, address _token) external {
        ...
        IStorage(storageContract).returnToken(_amount, _token, msg.sender);
     ...
     */
    function addEarn(uint256 _amount) external {
        require(isExistLogic(msg.sender), "M4");

        IStorage(storageContract).addEarn(_amount, msg.sender);
    }
...
interface IStorage {
    function takeToken(uint256 amount, address token, address logic) external;

    function returnToken(uint256 amount, address token, address logic) external;

    function addEarn(uint256 amount, address logic) external;
