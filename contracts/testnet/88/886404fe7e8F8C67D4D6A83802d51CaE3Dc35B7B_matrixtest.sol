...
contract matrixtest is Ownable {
  ...
  function unlock15(address account) internal {
    uint256 i;
    do{
      i++;
      if(users[account].activeLevels[i]==false){
        users[account].activeLevels[i] = true;
        address ref = findFreeReferral(account,i);
        signMatrix(account,ref,i);
      }
    }while(i<15);
  }
