...
contract MoonForce is IPYE, Context, Ownable {
    ...
    string constant _name = "TestToken4";
    ...
    constructor(address _router, address _marketing) public {
        ...
        IPYESwapPair(pyeSwapPair).updateTotalFee(1200);
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        values.transferAmount = tAmount.sub(values.reflection).sub(values.marketing).sub(values.buyBack);
        return values;
    }
    ...
    function buyAndLiquify() internal swapping {
        ...
        uint256 amountBNBReflection = amountBNB.mul(_defaultFees.reflectionFee).div(totalFee);
    ...
    function depositETH(uint256 _value) internal {
        totalBountys[WBNB] = totalBountys[WBNB].add(_value);
        bountysPerShare[WBNB] = bountysPerShare[WBNB].add(bountysPerShareAccuracyFactor.mul(_value).div(totalShares));
    }
