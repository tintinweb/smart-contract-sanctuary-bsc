...
contract luckyTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("luckyToken_Dividend_Tracker", "luckyToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "luckyToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "luckyToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main luckyToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "luckyToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");    
        require(newClaimWait != claimWait, "luckyToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract luckyToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    luckyTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("lucky Token", "ST") 
    {
        BNBRewardsFee = 5;
        liquidityFee = 1;
        marketingFee = 3;
        developerFee = 3;
        extraFeeOnSell = 0;
        marketingWallet = payable(0xdD2304a165923940e6eC14FE02c8657ec214EA88);
        developerWallet = payable(0x86e8883409E96d3c9d7D272c42d8DD1393c0b21d);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee); // total fee transfer and buy

        dividendTracker = new luckyTokenDividendTracker();
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "luckyToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "luckyToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "luckyToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "luckyToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "luckyToken: Cannot update gasForProcessing to same value");
