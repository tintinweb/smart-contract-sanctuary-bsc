...
contract SquidWorld is Context, IBEP20, Ownable {
    ...
    address public MarketingWallet =
        0x787AAe22aeA1dCC22fb229d80Fda923508d0E891;
    ...
    string private constant _name = "Squid World";
    string private constant _symbol = "SQW";
    ...
    uint256 public _sellPrizeFee = 1;
    uint256 public _sellBurnFee = 2;
    uint256 public _sellTeamFee = 1;

    uint256 public _internalTaxFee = 1;
    uint256 public _internalLiquidityFee = 2;
    uint256 public _internalMarketingFee = 3;
    uint256 public _internalPrizeFee = 1;
    uint256 public _internalBurnFee = 2;
    uint256 public _internalTeamFee = 1;
    ...
    function UpdateBuyTaxFeePercentage(
        ...
        _buyPrizeFee = PrizeFee;
        _buyTeamFee = _teamFee;

        uint256 TotalFee =  taxFee.add(liquidityFee).add(burnFee).add(MarketingFee).add(TeamFee);
        require(TotalFee.add(PrizeFee)
            <= (feeDenominator.div(5)),
            "You can't set more than 25%"
        );
        emit BuyTaxFeeUpdated(
           TotalFee.add(PrizeFee)
        );
    }
    ...
    function UpdateInternalTaxFeePercentage(
        uint256 taxFee,
        uint256 liquidityFee,
        uint256 burnFee,
        uint256 MarketingFee,
        ...
        _internalTaxFee = taxFee;
        _internalLiquidityFee = liquidityFee;
        _internalBurnFee = burnFee;
        _internalMarketingFee = MarketingFee;
        _internalPrizeFee = PrizeFee;
        _internalTeamFee = TeamFee;
    ...
    function UpdateSwapThresholdValue(uint256 thresholdValue)
        ...
        onlyOwner
    {
        numTokensSellToAddToLiquidity = thresholdValue * 10**9;
    }
    ...
    function _transfer(
        ...
        if(!_isAtomaticMarketMaker[from] && !_isAtomaticMarketMaker[to]){
            _taxFee = _internalTaxFee;
            _liquidityFee = _internalLiquidityFee;
            _MarketingFee = _internalMarketingFee;
            _burnFee = _internalBurnFee;
            _PrizeFee = _internalPrizeFee;
            _teamFee = _internalTeamFee;
        }
    ...
    function _tokenTransfer(
        ...
        if (
            ...
            (!_isAtomaticMarketMaker[sender] && !_isAtomaticMarketMaker[recipient])
        ) {
            restoreAllFee();
        }
    }
    ...
    function _transferStandard(
        ...
        (tTransferAmount, rTransferAmount) = takePrize(
            sender,
            tTransferAmount,
            rTransferAmount,
            tAmount
        ...
        (tTransferAmount, rTransferAmount) = takeTeam(
            sender,
            tTransferAmount,
            rTransferAmount,
            tAmount
    ...
    function takeTeam(
        ...
        uint256 tteam = tAmount.mul(_teamFee).div(feeDenominator);
        ...
        emit Transfer(sender, DeadWallet, tteam);
        return (tTransferAmount, rTransferAmount);
    }
    ...
    function takePrize(
        ...
        uint256 tPrize = tAmount.mul(_PrizeFee).div(feeDenominator);
        uint256 rPrize = tPrize.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rPrize);
        tTransferAmount = tTransferAmount.sub(tPrize);
        _rOwned[PrizeWallet] = _rOwned[PrizeWallet].add(
            rPrize
        );
        emit Transfer(sender, PrizeWallet, tPrize);
        return (tTransferAmount, rTransferAmount);
    }
