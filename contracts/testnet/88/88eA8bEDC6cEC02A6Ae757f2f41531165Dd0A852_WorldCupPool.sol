    ...
    PausableUpgradeable
{
    ...
    // Bet end time for each game (groupMatchID 1-48 => BetEndTime)
    //    mapping(uint256 => uint256) groupStageBetEndTime;

    // game results: (groupMatchID 1-48 => results 0,1,2)
    //    mapping(uint256 => uint256) groupMatchResult;

    // heros (S and S+) represent countries (32)
    // country => hero
    //    mapping(string => uint256[]) heroRepresentCountryGroupStage;

    // match => country
    //    mapping(uint256 => string[]) matchToCountries;

    // Total number of Group Games
    uint256 public constant totalMatches = 48;
