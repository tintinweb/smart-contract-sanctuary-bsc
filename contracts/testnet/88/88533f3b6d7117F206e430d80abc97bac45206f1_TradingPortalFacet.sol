...
contract TradingPortalFacet is ITradingPortal {
...
//        ITradingChecker(address(this)).openMarketTradeCheck(data);
        ...
        ITradingReader.PendingTrade memory pt = ITradingReader.PendingTrade(
   ...
   */
    function settleLpFundingFee(uint256 lpReceiveFundingFeeUsd) external override {
        ...
        if (tokenIns.length == 1) {
            ...
            LibTrading.transferFundingFeeToVault(ts, mb, lpReceiveFundingFeeUsd, 1e4);
        } else {
            ...
            for (UC i = ONE; i < uc(balances.length); i = i + uc(1)) {
                ...
                points -= share;
                LibTrading.transferFundingFeeToVault(ts, mb, lpReceiveFundingFeeUsd, share);
            }
            LibTrading.transferFundingFeeToVault(ts, balances[0], lpReceiveFundingFeeUsd, points);
        }
    }
