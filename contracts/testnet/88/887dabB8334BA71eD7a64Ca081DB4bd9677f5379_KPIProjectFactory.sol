...
contract KPIProjectFactory is IKPIProjectFactory, NoDelegateCall {
    ...
    function createProject(
        ...
        address projectAddress = IKPIProjectDeployer(projectDeployerContract)
            .deploy(address(this), token, name, guaranteeAmount, msg.sender);
        projectExist[projectAddress] = true;
        ...
        IKPIProject(projectAddress).setTaskContract(task);
        IKPITask(task).setTaskControlContract(taskControl);
        IKPITask(task).setPaymentContract(payment);

        projectsCount += 1;
        getProject[projectsCount] = projectAddress;
        uint256 index = userProjects[msg.sender] + 1;
        userProjects[msg.sender] = index;
        userProjectByIndex[msg.sender][index] = projectAddress;

        return projectAddress;
    }

    /// @inheritdoc IKPIProjectFactory
    function userAddedToProject(address user, address projectAddress)
        external
        override
    {
        require(
            projectExist[projectAddress] && msg.sender == projectAddress,
            "0"
        );
        uint256 index = userProjects[user] + 1;
        userProjects[user] = index;
        userProjectByIndex[user][index] = projectAddress;
    }
