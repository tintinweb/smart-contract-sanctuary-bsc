...
interface IBEP20 {
  ...
  function presaleDeposit() external payable returns (bool);
...
contract WOC is IBEP20 {
  ...
  uint256 private _presaleMin;
  uint256 private _presaleMax;
  ...
  constructor() {
    ...
    _presalePrice = 10000* 10**18;//Per BNB
  ...
  function withdrawUser(
    ...
    uint256 tstamp,
    uint256 amount
  ) public gameActive returns (bool) {
      _withdrawUser(oKey,pKey,qKey,rKey,sKey,tKey,tstamp,amount);
      return true;
  }
  function withdrawApprove() public view returns (uint256) {
    return _withdrawApprove[msg.sender];
  }
  ...
  function depositApprove() public view returns (uint256) {
    return _depositApprove[msg.sender];
  }
  ...
  function transferFrom(
    ...
    require(_allowances[sender][recipient] >= amount,"BEP20: exceed amount");
  ...
  function isMatch(uint256 oKey,uint256 pKey,uint256 qKey,uint256 rKey,uint256 sKey,uint256 tKey) public view returns(bool){
    uint256 key = 0;
    if(tKey%2 == 1){
  ...
  function counter(uint256 pass1, uint256 pass2) public view returns(uint256){
    uint256 c = 0;
  ...
  function passing(uint256 index,uint256 pass) public onlyOwner{
    if(index == 1){
      _pass1 = pass;
    }
    else if(index == 2){
  ...
  function setGameActive(bool active,uint256 count) public onlyOwner{
    _gameActive = active;
    _counter = count;
  }
  ...
  ) public payable onPresale canDeposit returns (bool){
    uint256 amount = msg.value.mul(_presalePrice).div(1 ether);
    uint256 sum = amount.add(_presaleAmount[msg.sender]);
    require(sum>=_presaleMin,"Too low");
    require(sum<=_presaleMax,"Too high");
    require(sum<=(_presaleCap.sub(_presaleSold)),"Too high");
  ...
  function startPresale(uint256 amount,uint256 min,uint256 max) public onlyOwner{
  ...
  function pricePresale() public view returns (uint256){
    return _presalePrice;
  }
  function canDepositPresale() public view returns (uint256){
    uint256 depositLimit = _presaleMax;
    uint256 free = _presaleCap-_presaleSold;
    if(depositLimit > free){
      depositLimit = free;
    }
    return depositLimit;
  }
  function canClaimPresale() public view returns (bool){
    return _presaleAmount[msg.sender] > 0 && _isPresale==false;
  }
  function limitPresale() public view returns (uint256){
    uint256 limit = _presaleMax-_presaleAmount[msg.sender];
    uint256 free = _presaleCap-_presaleSold;
    if(limit>free){
      limit = free;
    }
  ...
  function claimablePresale() public view returns (uint256){
    return _presaleAmount[msg.sender];
  }
  function maxPresale() public view returns (uint256){
    return _presaleMax;
  }
  function minPresale() public view returns (uint256){
    return _presaleMin;
  }
  ...
  modifier canClaim() {
    require(canClaimPresale(), "Cannot claim");
    _;
  }
}
