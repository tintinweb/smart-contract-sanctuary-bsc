...
interface IVestingModule{
    ...
    function transferRefundRecipientRole(address newRefundRecipient) external;    
...
contract Vesting is Initializable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    // errors
    error Error_Vesting_Unauthorized();
    error Error_Vesting_Unauthorized_Refundrecepient();
    error Error_Vesting_Zero_Address();
    error Error_Vesting_User_Grant_Exists();
    error Error_Vesting_Zero_Cliff();
    error Error_Vesting_Cliff_Longer_Than_Duration();
    error Error_Vesting_Batch_Length_Zero();
    error Error_Vesting_Batch_Length_Mismatch();
    error Error_Vesting_Zero_Amount_Per_Day();
    error Error_Vesting_Recipient_Transfer_Failed();
    error Error_Vesting_Refund_Recipient_Transfer_Failed();
    error Error_Vesting_Zero_Amount_Vested();
    error Error_Vesting_Sender_Transfer_Failed();
    ...
    modifier onlyRefundRecipient {
        if (msg.sender != refundRecipient){
            revert Error_Vesting_Unauthorized_Refundrecepient();
        }
        _;
    }
    ...
    modifier nonZeroAddress(address x) {
        if (x == address(0)){
            revert Error_Vesting_Zero_Address();
        }
        _;
    }

    modifier noGrantExistsForUser(address _user) {
        if (tokenGrants[_user].startTime != 0){
            revert Error_Vesting_User_Grant_Exists();
        }
        _;
    }

    function initialize(address _token, address __owner, address _refundRecipient) external initializer nonZeroAddress(_token) nonZeroAddress(__owner) nonZeroAddress(_refundRecipient) {
   ...
   function _authorizeUpgrade(address) internal override onlyOwner {}
    ...
    {
        if (_vestingCliff<=0){
            revert Error_Vesting_Zero_Cliff();
        }
        if (_vestingDuration <= _vestingCliff){
            revert Error_Vesting_Cliff_Longer_Than_Duration();
        }
        uint amountVestedPerDay = uint(_amount)/(_vestingDuration);
        if (amountVestedPerDay <= 0){
            revert Error_Vesting_Zero_Amount_Per_Day();
        }
    ...
    onlyOwner
    {
        if (_recipients.length <= 0){
            revert Error_Vesting_Batch_Length_Zero();
        }
        if (_recipients.length != _amounts.length){
            revert Error_Vesting_Batch_Length_Mismatch();
        }
        if (_vestingCliff <= 0){
            revert Error_Vesting_Zero_Cliff();
        }
        ...
        token.transferFrom(_owner, address(this), totalAmount);

        for (uint256 i = 0; i < _recipients.length; i++) {
            uint amountVestedPerDay = uint(_amounts[i])/(_vestingDuration);
            if (amountVestedPerDay <= 0) {
                revert Error_Vesting_Zero_Amount_Per_Day();
            }
            if (tokenGrants[_recipients[i]].startTime != 0) {
                revert Error_Vesting_User_Grant_Exists();
            }
    ...
    onlyRefundRecipient
    {
        ...
        uint128 amountNotVested = uint128(uint(tokenGrant.amount)-(tokenGrant.totalClaimed)-(amountVested));
        ...
        if (!token.transfer(_recipient, amountVested)) {
            revert Error_Vesting_Recipient_Transfer_Failed();
        }
        if (!token.transfer(refundRecipient, amountNotVested)) {
            revert Error_Vesting_Refund_Recipient_Transfer_Failed();
        }
    ...
    function claimVestedTokens() external {
        ...
        if (amountVested <= 0){
            revert Error_Vesting_Zero_Amount_Vested();
        }

        Grant storage tokenGrant = tokenGrants[msg.sender];
        tokenGrant.daysClaimed = uint16(uint(tokenGrant.daysClaimed)+(daysVested));
        tokenGrant.totalClaimed = uint128(uint(tokenGrant.totalClaimed)+(amountVested));

        if (!token.transfer(msg.sender, amountVested)){
            revert Error_Vesting_Sender_Transfer_Failed();
        }
    ...
    function calculateGrantClaim(address _recipient) public view returns (uint16, uint128) {
        ...
        uint elapsedTime = block.timestamp-(tokenGrant.startTime);
        ...
        if (elapsedDays >= tokenGrant.vestingDuration) { 
            uint16 daysVested = uint16(uint(tokenGrant.vestingDuration)-(tokenGrant.daysClaimed));
            uint128 remainingGrant = uint128(uint(tokenGrant.amount)-(tokenGrant.totalClaimed));
            return (daysVested, remainingGrant);
        } else {
            uint16 daysVested = uint16(elapsedDays-(tokenGrant.daysClaimed));
            uint amountVestedPerDay = uint(tokenGrant.amount)/(tokenGrant.vestingDuration);
            uint128 amountVested = uint128(uint(daysVested)*(amountVestedPerDay));
    ...
    function transferOwnership(address newOwner) public override onlyOwner {
        if(newOwner == address(0)){
            revert Error_Vesting_Ownable_Zero_Address_New();
        }
