...
contract UtopiaToken is Ownable, ERC20 {
    mapping(address=>bool) public derivation;
    address public uniswapV2Pair;
    uint256 public buyFeeDividend = 2;//分红买入费用
    uint256 public sellFeeDividend = 2;//管理买入费用
    uint256 public buyFeeTrusteeship = 3;//分红卖出费用
    uint256 public sellFeeTrusteeship = 7;//管理卖出费用
    ...
    address public _token_trusteeship;//币值管理地址

    uint256 autoDividendThreshold;//自动分红的分红价格
    uint256 public waitLPHolderDividendTokenNum;//等待分红
    ...
    uint256 public clearBuyUserNum;//清理流动性的数量
    uint256 public clearBuyUserTime;//清理流动性的时间
    bool public inSwap;
    ...
    constructor() ERC20("UtopiaPlanet", "UP") {
        ...
        _mint(msg.sender, circulation);
        autoDividendThreshold = 10000 ether; //自动分红的分红价格
        ...
        _token_trusteeship = address(owner());
    }
    ...
    //清除流动性为0的用户
    function clearBuyUser() public swapping{
        if (buyUser.length <= 0) {
            return;
        }
        if(clearBuyUserNum > 0){
            ...
            if(clearBuyUserTime + 86400 < block.timestamp){
                clearBuyUserTime = block.timestamp;
            }else{
                clear();
            }
    ...
    function clear() private {
        ...
        if(buyUserLen > iNum+1){
            for(uint256 i = buyUserLen - 1 - iNum; i > buyUserLen - 5 - 1; ) {
    ...
    function _transferWithBuyFee(address from, address to, uint256 amount) internal {
        ...
        uint256 dividendAmount = amount * buyFeeDividend / 100;
        if(dividendAmount > 0){
            waitLPHolderDividendTokenNum += dividendAmount;
            tAmount -= dividendAmount;
        }
        uint256 trusteeshipAmount = amount * buyFeeTrusteeship / 100;
    ...
    function _transferWithSellFee(address from, address to, uint256 amount) internal {
        uint256 tAmount = amount;
        uint256 dividendAmount = amount * sellFeeDividend / 100;
        if(dividendAmount > 0){
            waitLPHolderDividendTokenNum += dividendAmount;
            tAmount -= dividendAmount;
        }
        uint256 trusteeshipAmount = amount * sellFeeTrusteeship / 100;
        if(trusteeshipAmount > 0){
            super._transfer(from,_token_trusteeship,trusteeshipAmount);
            tAmount -= trusteeshipAmount;
        }
    ...
    function destruction(address account, uint256 amount) public onlyDerivation {
