...
contract MythCosmetic is ERC721 {
    ...
    function overrideOwnerOfDegen(uint256 _degenId, address newOwner)
        external
        isWhitelisted
        returns (bool)
    {
        for (uint256 i = 0; i < 7; i++) {
            ...
            if (tempCosmeticId > 0) {
                ...
                emit ownerChanged(newOwner, tempCosmeticId);
            }
        }
        return true;
    }
        ...
        returns (bool)
    {
        ...
        emit cosmeticEquipped(_degenId, _layerId, 0, "");
        return true;
    }
    ...
    function mint(
        ...
        cosmeticStats[tokenCount] = itemStat(
            _to,
            _imageId,
            _layerType,
            _nameId,
    ...
    function addCosmeticImageId(
        ...
        for (uint256 i = 0; i < _urls.length; i++) {
            ...
            cosmeticURL[_layerType[i]][_imageId[i]] = _urls[i];
            cosmeticExists[_layerType[i]][_imageId[i]] = true;
            cosmeticName[_layerType[i]][_imageId[i]] = _imageNames[i];
        }
    }
...
contract MythDegen is ERC721 {
    mapping(uint256 => stats) public degenStats; //Mapping from token Id to stats struct
    mapping(uint256 => equippedItems) public degenEquips; //Mapping from token Id to gear struct
    mapping(uint256 => cosmetics) public degenCosmetics; //Mapping from token Id to cosmetics struct
    ...
    mapping(address => bool) public cosmeticWhitelist; //Mapping of addresses allowed to unlock cosmetics
    mapping(address => bool) public degenWhitelist; //Mapping of addresses allowed to mint and regrade degens
    mapping(address => bool) public whitelistedAddresses; //Mapping of addresses allowed to
    mapping(uint256 => mapping(address => bool))
        public unlockedCosmeticAddressForDegen;
    mapping(address => mapping(uint256 => mapping(uint256 => bool)))
        public freeCosmetics; //Mapping of cosmetics unlocked by all degens
    string public defaultBackground;
    string public defaultEye;
    string public defaultMouth;
    string public defaultSkinColor;
    string public defaultNose;
    ...
    event cosmeticChanged(
        uint256 degenId,
        uint256 layerType,
        address cosmeticAddress,
        uint256 itemId
    );
    event ownerChanged(uint256 degenId, address ownerAddress);

    event degenReGrade(uint256 coreScore, uint256 damageCap);
    event degenOnMission(uint256 degenId, bool onMission);
    event degenUnlockedCosmeticAddress(
        address cosmeticAddress,
        uint256 degenId,
        bool isUnlocked
    );
    ...
    constructor() ERC721("Myth City Degen", "MYDG") {
        ...
        defaultNose = "https://mythcity.mypinata.cloud/ipfs/QmaaKkumxEX6YrpgqGqWcZEBFGJBhRPz5JxA6HSDMZs5mE/BODY%20COLOR/DEFAULT-min.png";
        whitelistedAddresses[msg.sender] = true;
        degenWhitelist[msg.sender] = true;
        cosmeticWhitelist[msg.sender] = true;
        mint(msg.sender, 5000, 10000);
    }
    ...
    function transfer(uint256 _degenId, address _to) external {
        require(
            degenStats[_degenId].owner == msg.sender,
            "Only the owner of the Degen can send it"
        );
        equippedItems memory tempEquips = degenEquips[_degenId];
        if (tempEquips.faceModData.idOfItem > 0) {
            MythCityMods tempMods = MythCityMods(
                tempEquips.faceModData.addressOfItem
            );
            require(
                tempMods.overrideOwner(tempEquips.faceModData.idOfItem, _to),
                "failed to transfer mods"
            );
        }
        if (tempEquips.weaponData.idOfItem > 0) {
            MythCityWeapons tempWeapons = MythCityWeapons(
                tempEquips.weaponData.addressOfItem
            );
            require(
                tempWeapons.overrideOwner(tempEquips.weaponData.idOfItem, _to),
                "Failed to transfer Weapons"
            );
        }
        if (tempEquips.equipmentData.idOfItem > 0) {
            MythCityEquipment tempEquipment = MythCityEquipment(
                tempEquips.equipmentData.addressOfItem
            );
            require(
                tempEquipment.overrideOwner(
                    tempEquips.equipmentData.idOfItem,
                    _to
                ),
    ...
    function mint(
        ...
        emit ownerChanged(tokenCount, _to);
        degenStats[tokenCount] = stats(_core, _damage, _to, false);
        tokenCount++;
    }

    function burnToken(uint256 _id) external isWhitelisted {
        emit ownerChanged(_id, owner);
    ...
    function reGradeDegen(
        ...
        emit degenReGrade(_newCore, _newDamage);
        ...
        returns (uint256[2] memory)
    {
        ...
        if (tempDegen.faceModData.idOfItem > 0) {
            MythCityMods tempMods = MythCityMods(
                tempDegen.faceModData.addressOfItem
            );
            MythCityMods.itemStat memory tempStats = tempMods.getStats(
                tempDegen.faceModData.idOfItem
            );
            tempNumbers[1] += tempStats.modStat;
        }
        if (tempDegen.equipmentData.idOfItem > 0) {
            MythCityEquipment tempEquipment = MythCityEquipment(
                tempDegen.equipmentData.addressOfItem
            );
            MythCityEquipment.itemStat memory tempStats = tempEquipment
                .getStats(tempDegen.equipmentData.idOfItem);
            tempNumbers[0] += tempStats.equipmentStat;
        }
        if (tempDegen.weaponData.idOfItem > 0) {
            MythCityWeapons tempWeapon = MythCityWeapons(
                tempDegen.weaponData.addressOfItem
            );
            MythCityWeapons.itemStat memory tempStats = tempWeapon.getStats(
                tempDegen.weaponData.idOfItem
            );
    ...
    function equipCosmetic(
        ...
        require(
            cosmeticWhitelist[_equipCosmeticAddress],
            "this cosmetic address is not allowed"
        );
        MythCosmetic tempCosmetics = MythCosmetic(_equipCosmeticAddress);
        ...
        emit cosmeticChanged(
            _degenId,
            tempStats.layerType,
            _equipCosmeticAddress,
            _cosmeticId
        );
        uint256 _layerId = tempStats.layerType;
        if (_layerId == 1) {
            degenCosmetics[_degenId].eyeData = itemData(
                _equipCosmeticAddress,
                _cosmeticId
            );
        } else if (_layerId == 2) {
            ...
            degenCosmetics[_degenId].noseData = itemData(
                _equipCosmeticAddress,
                _cosmeticId
            );
        } else if (_layerId == 4) {
            ...
            degenCosmetics[_degenId].bodyData = itemData(
                _equipCosmeticAddress,
                _cosmeticId
            );
        } else if (_layerId == 6) {
            degenCosmetics[_degenId].headData = itemData(
                _equipCosmeticAddress,
                _cosmeticId
            );
        } else if (_layerId == 0) {
            ...
            degenCosmetics[_degenId].bodyOuterData = itemData(
                _equipCosmeticAddress,
                _cosmeticId
            );
        }
    ...
    function unequipCosmetic(uint256 _degenId, uint256 _layerId) external {
        require(
            degenStats[_degenId].owner == msg.sender,
            "Only the owner of the degen can use it"
        );
        address tempCosmeticAddress;
        if (_layerId == 1) {
            tempCosmeticAddress = degenCosmetics[_degenId]
                .eyeData
                .addressOfItem;
        } else if (_layerId == 2) {
            tempCosmeticAddress = degenCosmetics[_degenId]
        ...
        require(tempCosmeticAddress != address(0), "No Cosmetic equipped");
        MythCosmetic tempCosmetics = MythCosmetic(tempCosmeticAddress);
    ...
    function equipMod(
        ...
        degenEquips[_degenId].faceModData = itemData(_modsAddress, _modId);
        //Add event to show mod equipped
    }
    ...
    function unequipMod(uint256 _degenId) external {
        ...
        require(
            degenEquips[_degenId].faceModData.addressOfItem != address(0),
            "No mod equipped"
        );
        MythCityMods tempMods = MythCityMods(
            degenEquips[_degenId].faceModData.addressOfItem
        );
    ...
    function equipWeapon(
        ...
        require(degenWhitelist[_weaponAddress], "this weapon is not allowed");
        MythCityWeapons tempWeapons = MythCityWeapons(_weaponAddress);
        ...
        degenEquips[_degenId].weaponData = itemData(_weaponAddress, _weaponId);
    }
    ...
    function unequipWeapon(uint256 _degenId) external {
        ...
        require(
            degenEquips[_degenId].weaponData.addressOfItem != address(0),
            "No weapon equipped"
        );
        MythCityWeapons tempWeapons = MythCityWeapons(
            degenEquips[_degenId].weaponData.addressOfItem
        );
    ...
    function equipEquipment(
        ...
        require(
            degenWhitelist[_equipmentAddress],
            "this equipment is not allowed"
        );
        MythCityEquipment tempEquipment = MythCityEquipment(_equipmentAddress);
        ...
        degenEquips[_degenId].equipmentData = itemData(
            _equipmentAddress,
            _equipmentId
        );
    }
    ...
    function unequipEquipment(uint256 _degenId) external {
        ...
        require(
            degenEquips[_degenId].equipmentData.addressOfItem != address(0),
            "No equipment equipped"
        );
        MythCityEquipment tempEquipment = MythCityEquipment(
            degenEquips[_degenId].equipmentData.addressOfItem
        );
    ...
    function getWingsURL(uint256 _degenId) public view returns (string memory) {
        ...
        if (tempEquipped.equipmentData.idOfItem == 0) {
            return "";
        }
        MythCityEquipment tempEquipment = MythCityEquipment(
            tempEquipped.equipmentData.addressOfItem
        );
        MythCityEquipment.itemStat memory tempStats = tempEquipment.getStats(
            tempEquipped.equipmentData.idOfItem
        );
        if (tempStats.isWings) {
            return
                tempEquipment.getImageURL(tempEquipped.equipmentData.idOfItem);
        } else {
            return "";
        }
    }
    ...
    {
        string[10] memory tempList; // 1: BackGround 2: Skin Color 3: Mods 4: Eyes 5: Mouth 6: Head
        tempList[0] = getBackgroundURL(_id);
        tempList[1] = getWingsURL(_id);
        tempList[2] = getSkinColorURL(_id);
        tempList[3] = getModURL(_id);
        tempList[4] = getNoseURL(_id);
        tempList[5] = getEyeURL(_id);
        tempList[6] = getMouthURL(_id);
        tempList[7] = getHeadURL(_id);
        tempList[8] = getBodyURL(_id);
        tempList[9] = getBodyOuterURL(_id);
        return tempList;
    }
    ...
    function tokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        uint256[2] memory tempStats = getDegenStats(tokenId);
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '",',
                        '"attributes": [{"trait_type": "Degen Core Score", "value": ',
                        Base64.uint2str(degenStats[tokenId].coreScore),
                        '},{"trait_type": "Degen Damage Cap", "value": ',
    ...
    function getModURI(uint256 _degenId) public view returns (string memory) {
        ...
        if (tempEquips.faceModData.idOfItem > 0) {
            ...
            string memory json = tempMods.getDegenData(
                tempEquips.faceModData.idOfItem
            );
        ...
        returns (string memory)
    {
        ...
        if (tempEquips.weaponData.idOfItem > 0) {
            ...
            string memory json = tempWeapons.getDegenData(
                tempEquips.weaponData.idOfItem
            );
            return json;
        } else {
            string
                memory weaponMetaString = '{"trait_type":"Weapon Id","value":0},{"trait_type":"Weapon Core","value":0},{"trait_type":"Weapon Damage","value":0},{"trait_type":"Weapon Image Url","value":""}';
    ...
    function getImageLayers(uint256 _id) public view returns (string memory) {
        ...
        string[10] memory tempList = getDegenImage(_id);
    ...
    function getBackgroundURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].backgroundData.addressOfItem)
                .getImageURL(degenCosmetics[_id].backgroundData.idOfItem);
    }

    function getModURL(uint256 _id) public view returns (string memory) {
        if (degenEquips[_id].faceModData.addressOfItem == address(0)) {
            return "";
        }
        return
            MythCityMods(degenEquips[_id].faceModData.addressOfItem)
                .getImageFromId(degenEquips[_id].faceModData.idOfItem);
    }
    ...
    function getNoseURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].noseData.addressOfItem)
                .getImageURL(degenCosmetics[_id].noseData.idOfItem);
    }
    ...
    function getBodyURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].bodyData.addressOfItem)
                .getImageURL(degenCosmetics[_id].bodyData.idOfItem);
    }

    function getBodyOuterURL(uint256 _id) public view returns (string memory) {
        if (degenCosmetics[_id].bodyOuterData.addressOfItem == address(0)) {
            return "";
        }
        return
            MythCosmetic(degenCosmetics[_id].bodyOuterData.addressOfItem)
                .getImageURL(degenCosmetics[_id].bodyOuterData.idOfItem);
    }
    ...
    function getHeadURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].headData.addressOfItem)
                .getImageURL(degenCosmetics[_id].headData.idOfItem);
    }
    ...
    function getEyeURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].eyeData.addressOfItem).getImageURL(
    ...
    function getMouthURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].mouthData.addressOfItem)
                .getImageURL(degenCosmetics[_id].mouthData.idOfItem);
    }
    ...
    function getSkinColorURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].skinColorData.addressOfItem)
                .getImageURL(degenCosmetics[_id].skinColorData.idOfItem);
    }
}
...
contract MythCityMods is ERC721 {
    ...
    function changeMod(string[10] calldata _url, uint256[10] calldata _id)
        external
        isWhitelisted
    {
        for (uint256 i = 0; i < 10; i++) {
            emit modAdded(_id[i], _url[i]);
...
contract MythCityWeapons is ERC721 {
    ...
    mapping(uint256 => skinEquip) public weaponSkins;
    mapping(address => bool) public weaponSkinAddress;

    event weaponAdded(uint256 id, string url);
    event weaponEquipped(uint256 weaponId, uint256 degenId);
    event weaponRegrade(
        uint256 weaponId,
        uint256 weaponCore,
    ...
    function getImageFromId(uint256 _id) public view returns (string memory) {
        if (weaponSkins[_id].weaponSkin.idOfItem > 0) {
            MythCityWeaponSkins tempSkins = MythCityWeaponSkins(
                weaponSkins[_id].weaponSkin.addressOfItem
            );
            return (
                tempSkins.getImageURL(weaponSkins[_id].weaponSkin.idOfItem)
            );
        } else {
    ...
    function transfer(uint256 _weaponId, address _to) external {
        ...
        skinEquip memory tempSkins = weaponSkins[_weaponId];
        if (tempSkins.weaponSkin.idOfItem > 0) {
            MythCityWeaponSkins tempSkinContract = MythCityWeaponSkins(
                tempSkins.weaponSkin.addressOfItem
            );
            require(
                tempSkinContract.overrideOwner(
                    tempSkins.weaponSkin.idOfItem,
                    _to
                ),
    ...
    function equipSkin(
        ...
        require(
            weaponSkinAddress[_skinAddress],
            "this skin address is not allowed"
        );
        MythCityWeaponSkins tempSkins = MythCityWeaponSkins(_skinAddress);
        ...
        weaponSkins[_weaponId].weaponSkin = itemData(_skinAddress, _skinId);
    }
    ...
    function unequipSkin(uint256 _weaponId) external {
        ...
        require(
            weaponSkins[_weaponId].weaponSkin.addressOfItem != address(0),
            "No Skin equipped"
        );
        MythCityWeaponSkins tempSkins = MythCityWeaponSkins(
            weaponSkins[_weaponId].weaponSkin.addressOfItem
        );
        bool isEquipped = tempSkins.unequipSkin(_weaponId);
        require(isEquipped, "Skin was not un equipped");
        delete weaponSkins[_weaponId].weaponSkin;
    }
        ...
        returns (bool)
    {
        ...
        emit weaponEquipped(_weaponId, _degenId);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        emit weaponEquipped(0, _degenId);
        return true;
    }

    function overrideOwner(uint256 _weaponId, address _newOwner)
        external
        isWhitelisted
        returns (bool)
    {
        ...
        emit ownerChanged(_newOwner, _weaponId);
        return true;
    }
    ...
    function regradeWeaponStat(
        ...
        weaponStats[_weaponId].weaponCore = _weaponCore;
        weaponStats[_weaponId].weaponDamage = _weaponDamage;
        emit weaponRegrade(_weaponId, _weaponCore, _weaponDamage);
    }
    ...
    function changeWeapon(string[10] calldata _url, uint256[10] calldata _id)
        external
        isWhitelisted
    {
        for (uint256 i = 0; i < 10; i++) {
            emit weaponAdded(_id[i], _url[i]);
...
contract MythCityWeaponSkins is ERC721 {
    ...
    event whitelistAdded(address whitelistedAddress, bool isWhitelisted);
    event skinMinted(address to, uint256 imageId, uint256 weaponType);
    event ownerChanged(address to, uint256 weaponId);
    event skinEquipped(uint256 weaponId, uint256 skinId);
    event skinURLAdded(uint256 skinId, string imageURL);
    modifier isWhitelisted() {
        require(
            whitelistedAddresses[msg.sender] || msg.sender == owner,
            "Not white listed"
    ...
    function changeSkinURLs(string[10] calldata _url, uint256[10] calldata _id)
        external
        isWhitelisted
    {
        for (uint256 i = 0; i < 10; i++) {
            emit skinURLAdded(_id[i], _url[i]);
            skinExists[i] = true;
            skinURL[i] = _url[i];
        }
    }
    ...
    function overrideOwner(uint256 _skinId, address _newOwner)
        external
        isWhitelisted
        returns (bool)
    {
        emit ownerChanged(_newOwner, _skinId);
    ...
    function mint(
        ...
        emit ownerChanged(_to, tokenCount);
        emit skinMinted(_to, _imageId, _weaponType);
        skinStats[tokenCount] = itemStat(_to, _imageId, 0, _weaponType);
        tokenCount++;
        return true;
    }
...
contract MythCityEquipment is ERC721 {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '},{"trait_type":"Equipment Route","value":',
    ...
    function getImageURL(uint256 _equipmentId)
        ...
        returns (string memory)
    {
        return equipmentURL[_equipmentId];
    }
    ...
    function mint(
        ...
        emit ownerChanged(_to, tokenCount);
        emit equipmentMinted(_to, _imageId, _equipmentStat, _route, _isWings);
    ...
    function changeEquipment(string[10] calldata _url, uint256[10] calldata _id)
        external
        isWhitelisted
    {
        for (uint256 i = 0; i < 10; i++) {
            emit equipmentAdded(_id[i], _url[i]);
