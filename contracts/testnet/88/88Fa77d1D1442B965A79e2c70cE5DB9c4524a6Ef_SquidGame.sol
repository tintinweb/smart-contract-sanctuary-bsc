...
contract SquidGame is SquidGameBasic {
    ...
    function sendETHDividends(uint8 level) private returns (bool) {
        ...
        if((users[prev_user].firstPayment) && (users[prev_user].secPayment))
        {
        ...
        {
            if(!users[prev_user].firstPayment)
            {
    ...
    function UserRewards(address userAddress)public view returns (uint64[] memory){
        ...
        for (uint8 i=0; i < LAST_LEVEL; i++) 
        {
            ...
            if(level_length>0)
            {
                if((!users[userAddress].activeMainLevels[i+2]) && (users[userAddress].firstPayment) && (users[userAddress].secPayment)){
                    rewardCount = 2;
                }
                else{
