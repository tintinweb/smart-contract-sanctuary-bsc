...
import "../../../core/staking/category/SharePoolStaking.sol";
import "../../../libraries/Structs.sol";

/// @title SharePoolStakingFactory
/// @notice To create new share pool staking pool instance from here,stakingType is 1
contract SharePoolStakingFactory is SherpaEarnFactory{
    struct CreateSharePoolRequest{
        /// @dev projectName
        bytes  projectName;
    ...
    /// @notice create a new share pool staking pool then register to instance manager
    /// @dev instance address can be find at event log when test manually
    /// @return Returns address of instance contract.
    function create (CreateSharePoolRequest memory request) public atLeastAdmin nonReentrant returns (address)
    {
        uint8 _projectId=getProjectId();
        SharePoolStaking newSharePoolStaking = new SharePoolStaking(Structs.StakingParams(_projectId, request.stakingToken, request.minStakeAmount, request.unboundPeriodBlock, request.instantUnboundFeePercentage, address(adminAccess), request.feeAddress, request.commissionPercentage,request.payoutIntervalBlock));
        register(address(newSharePoolStaking),Project({projectID:_projectId, projectName:request.projectName, createTime:block.timestamp, stakingContract:address(newSharePoolStaking), stakingType:1, discard:false}));
        return address(newSharePoolStaking);
    }
...
interface IBaseRewardPool{
    ...
    function allClaimableRewards(address,uint256) external view returns(address[] memory,uint256[] memory);

    function queueNewExtraRewards(address,uint256,uint256) external returns(bool);
    function queueNewRewards(uint256,uint256) external returns(bool);
    function addExtraReward(address) external returns(bool);
...
/// @title  SharePoolStaking 
/// @notice the APR is variable, its depends on user share in pool and planed rewards
contract SharePoolStaking is Staking{
    uint8 public stakingType=1;
    ...
    constructor(Structs.StakingParams memory params) Staking(params) {}
    modifier userStaked(address user) {
        require(stakeUsersInfo[user].exist,"!S");
        _;
    }
    /// @notice setRewardPool,a BaseRewardPool must be set to SharePoolStaking
    /// @param pool, instance address of BaseRewardPool
    function setBaseRewardPool(address pool) public atLeastProject returns(bool){
        baseRewardPool=IBaseRewardPool(pool);
        return true;
    }
    /// @notice addNewBaseRewardPlan,queue new reward to both BaseRewardPool and VirtualBalanceRewardPool
    ...
    function addNewBaseRewardPlan(uint256 _rewards,uint256 _rewardEndBlock) public atLeastProject returns(bool){
        return baseRewardPool.queueNewRewards(_rewards,_rewardEndBlock);
    }
    /// @notice addNewExtraRewardPlan,queue new reward to both BaseRewardPool and VirtualBalanceRewardPool
    /// @param _pool, contract address of reward pool instance
    /// @param _rewards, planed reward token amount will be add to reward pool
    /// @param _rewardEndBlock, planed block for mint all 'planReward'
    /// @dev _rewards and _rewardEndBlock ,example (10000 token,2 weeks) means yield 10000 token in 2 weeks
    /// @return returns success
    function addNewExtraRewardPlan(address _pool,uint256 _rewards,uint256 _rewardEndBlock) public atLeastProject returns(bool){
        return baseRewardPool.queueNewExtraRewards(_pool,_rewards,_rewardEndBlock);
    }
    /// @notice addExtraReward,add VirtualBalanceRewardPool to BaseRewardPool
    /// @param _rewardPool, contract address of BaseRewardPool instance
    function addExtraReward(address _rewardPool) public atLeastProject returns(bool){
        return baseRewardPool.addExtraReward(_rewardPool);
    }
    /// @notice clearExtraRewards,delete all VirtualBalanceRewardPool from BaseRewardPool
    function clearExtraRewards() public atLeastProject{
        baseRewardPool.clearExtraRewards();
    }
    /// @notice stake
    ...
    function stake(uint256 amount) public isWhitelisted(msg.sender) notStopped whenNotPaused nonReentrant returns(bool){
        ...
        require(token.balanceOf(user)>=amount,"!B");
        ...
        require(baseRewardPool.stake(msg.sender, amount),"BSF");
        emit StakeEvent(user,stakingToken,amount);
        return true;
    }
    /// @notice stake all balance of stakingToken
    function stakeAll() public returns(bool){
        return stake(IERC20(stakingToken).balanceOf(msg.sender));
    }
    /// @notice normalUnbound ,no fee be deducted,unbind amount can be withdraw after unbindPeriodBlock
    /// @param amount, amount of stakingToken
    function normalUnbound(uint256 amount) public whenNotPaused nonReentrant userStaked(msg.sender) returns(bool){
        ...
        require(baseRewardPool.withdraw(msg.sender, amount),"RWF");
    ...
    function instantUnbound(uint256 amount) public whenNotPaused nonReentrant userStaked(msg.sender) returns(bool){
        ...
        require(baseRewardPool.withdraw(msg.sender, amount),"WF");
    ...
    /// @notice withdraw , users can withdraw their unbindingAmount after unboundPeriodBlock
    function withdraw() public whenNotPaused nonReentrant userStaked(msg.sender) returns(bool){
        ...
        require(_claimRewards(user),"CF");
        require(IERC20(stakingToken).transfer(user,stakeUser.unbindingAmount), "TF");
    ...
    function _claimRewards(address user) internal returns(bool){
        if (block.number-stakeUsersInfo[user].lastSettleBlock<payoutIntervalBlock){
            return true;
        }
        stakeUsersInfo[user].lastSettleBlock=block.number;
        require(baseRewardPool.claimRewards(user, feeAddress,commissionPercentage),"BCF");
        return true;
    }
    /// @notice claimRewards , user can claim per payoutIntervalBlock
    function claimRewards() public whenNotPaused nonReentrant returns(bool){
        address user=msg.sender;
        require(block.number-stakeUsersInfo[user].lastSettleBlock>=payoutIntervalBlock,"!PIB");
        _claimRewards(user);
        return true;
    }
    /// @notice allClaimableRewards 
    /// @param user, stake user address
    /// @return Returns tokens and rewards
    function allClaimableRewards(address user) public view returns(address[] memory,uint256[] memory){
        return baseRewardPool.allClaimableRewards(user,commissionPercentage);
    }
