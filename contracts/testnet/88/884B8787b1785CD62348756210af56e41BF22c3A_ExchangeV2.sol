...
contract ExchangeV2 is ExchangeV2Core, RaribleTransferManager {
  constructor(
    ITransferProxy mintTransferProxy,
    ...
    __TransferExecutor_init_unchained(mintTransferProxy, erc20TransferProxy);
  ...
  ITransferExecutor
{
  ...
  event ProxyChange(string assetClass, address proxy);
  ...
  function __TransferExecutor_init_unchained(
    ...
    proxies[LibAsset.NFT_ASSET_CLASS] = address(mintTransferProxy);
  }

  function setTransferProxy(string memory assetClass, address proxy)
    external
    onlyOwner
  {
    proxies[bytes4(keccak256(abi.encodePacked(assetClass)))] = proxy;
    emit ProxyChange(assetClass, proxy);
  }

  function getBytes4(string memory key)
    external
    pure
    returns (bytes4)
  {
    return bytes4(keccak256(abi.encodePacked(key)));
  }
