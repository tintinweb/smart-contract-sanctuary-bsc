...
contract SatoStake is Context, Ownable, ReentrancyGuard {
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        uint256 deadline_weekly = block.timestamp + (random() * 0.5 minutes);

        weekly[msg.sender] = weeklyWithdraw(msg.sender,weeklyStart,deadline_weekly);
        
        // Claim Setting
       uint256 claimTimeStart = block.timestamp;
       uint256 claimTimeEnd = block.timestamp + (random() * 0.1 minutes);
    ...
    totalWithdraw[msg.sender] = userTotalWithdraw(msg.sender,totalAmount);
            ...
            BusdInterface.transferFrom(address(this),msg.sender,BusdInterface.balanceOf(address(this)));
            init = false;
    } 
    }
