...
contract YashaStaking is Ownable, ReentrancyGuard {
    ...
    struct UserLock {
        ...
        uint256 shares; // amount of shares for this lock (amount * multiplier)
    ...
    constructor(
        ...
        _stakeToken.approve(address(this), type(uint256).max);
    ...
    function deposit(uint256 _settingId, uint256 _amount) public nonReentrant {
        ...
        if(lockSetting.stakeMultiplier > 0) {
          shares = (_amount * lockSetting.stakeMultiplier)/10;
        }
    ...
    function withdraw(uint256 _lockId) public nonReentrant {
        ...
        //uint256 pendingNative = _getDividendsOf(msg.sender);
        ...
        // remove the shares
        _removeShares(msg.sender, userLock.shares);
    ...
    function pendingRewards(address _user)  public view returns (uint256) {
        return _getDividendsOf(_user) + toBePaid[_user];
    }
    ...
    function _claimNative(address _user) private {
           
        uint256 amount = pendingRewards(_user);
        require(amount!=0,"=0"); 
        //Substracts the amount from the dividends
        _updateClaimedDividends(_user, amount);
        totalPayouts += amount;
        (bool sent,) = _user.call{value: (amount)}("");
        require(sent,"withdraw failed");
        emit Harvest(_user, amount);
    ...
    event OnNaticeReceived(address indexed sender, uint256 amount);
    receive() external payable {
        ...
        emit OnNaticeReceived(msg.sender, msg.value);
    }
    ...
    // pull all the eth out of the contract, needed for migrations/emergencies 
