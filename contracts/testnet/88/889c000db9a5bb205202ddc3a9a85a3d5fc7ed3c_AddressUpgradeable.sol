...
contract victorshiba is IVictorshiba, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "victorshiba: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "victorshiba: Invalid tier index");
        _;
    }
    ...
    preventBlacklisted(_account, "victorshiba: Selected account is in blacklist")
    {
        require(_account != address(0), "victorshiba: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "victorshiba: Invalid address");
        require(_accountsTier[_account] > 0, "victorshiba: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "victorshiba: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "victorshiba: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "victorshiba: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "victorshiba: Owner address is blacklisted")
    preventBlacklisted(spender, "victorshiba: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "victorshiba: Address is blacklisted")
    preventBlacklisted(from, "victorshiba: From address is blacklisted")
    preventBlacklisted(to, "victorshiba: To address is blacklisted")
    isRouter(_msgSender())
    {
    ...
    external
    preventBlacklisted(account, "victorshiba: Migrated account is blacklisted")
    override
    {
        require(migration != address(0), "victorshiba: Migration is not started");
        require(_msgSender() == migration, "victorshiba: Not Allowed");
