...
contract TokenStaking is Ownable{
    ...
    function setTax(uint256 withdrawfees,uint256 dipositfees) public onlyOwner {
       depositTax = dipositfees;
       unStakeTax = withdrawfees;
    }
    function hashStake(bool sale) public onlyOwner{
        hasStart = sale;
    }
    ...
    function invest(uint256 _amount, address refferAddress) public {
        ...
        require(hasStart,"staking is deactive please wait for staking activated");
        require(_amount > 0, "Please enter greater then value");
        require(_amount<=token.allowance(msg.sender, address(this)),"Insufficient Allowence to the contract");
        ...
        users.deposites.push(deposit(_amount, block.timestamp));
        users.timeForCalculateReward = block.timestamp;

        if(block.timestamp+5 minutes - users.boostlaval.boostTime > 5 minutes){
            users.boostlaval.boostTime = 0;
            users.boostlaval.boostCount = 0;
            users.boostlaval.ratePerDay =100;
        }else if((users.boostlaval.boostTime != 0) && (block.timestamp+5 minutes - users.boostlaval.boostTime) < 5 minutes){
            users.boostlaval.boostTime = 0;
            users.boostlaval.boostCount = 0;
            users.boostlaval.ratePerDay =100;
        }
    ...
    function booster(uint256 boostAmount) public {
       user storage users =investment[msg.sender];
       require(hasStart,"staking is deactive please wait for staking activated");
       require(boostAmount<=token.allowance(msg.sender, address(this)),"Insufficient Allowence to the contract");
       require(users.boostlaval.userAddress == msg.sender, "please invest first then you can booster earning");
       require(users.boostlaval.boostCount < 6, "Not enough the boost chance.");
        
        uint256 depositAmount = getUserdipositAddamount(msg.sender);
        if(depositAmount > 0 ether && depositAmount <= 40 ether){
        require(boostAmount >= 1 ether, "Please enter the minimum value 1 ether");
        ...
        }else if(depositAmount >= 41 ether && depositAmount <=80 ether){
        require(boostAmount >= 2 ether, "Please enter the minimum value 2 ether");
        token.transferFrom(msg.sender, address(this), boostAmount);
        users.userStoreReward +=calculateReward(msg.sender);
        users.timeForCalculateReward = block.timestamp;
        if(users.boostlaval.boostTime + 5 minutes > block.timestamp){
        ...
        }else if(depositAmount >= 81 ether && depositAmount <=120 ether){
        require(boostAmount >= 3 ether, "Please enter the minimum value 3 ether");
        token.transferFrom(msg.sender, address(this), boostAmount);
        users.userStoreReward +=calculateReward(msg.sender);
        users.timeForCalculateReward = block.timestamp;
        if(users.boostlaval.boostTime + 5 minutes > block.timestamp){
        ...
        }else if(depositAmount >= 121 ether && depositAmount <=160 ether){
        require(boostAmount >= 4 ether, "Please enter the minimum value 4 ether");
        token.transferFrom(msg.sender, address(this), boostAmount);
        users.userStoreReward +=calculateReward(msg.sender);
        users.timeForCalculateReward = block.timestamp;
        if(users.boostlaval.boostTime + 5 minutes > block.timestamp){
        ...
        }else if(depositAmount >= 161 ether && depositAmount <=200 ether){
        require(boostAmount >= 5 ether, "Please enter the minimum value 5 ether");
        token.transferFrom(msg.sender, address(this), boostAmount);
        users.userStoreReward +=calculateReward(msg.sender);
        users.timeForCalculateReward = block.timestamp;
        if(users.boostlaval.boostTime + 5 minutes > block.timestamp){
        ...
        }else if(depositAmount >200 ether){
        require(boostAmount >= 5 ether, "Please enter the minimum value 5 ether");
        token.transferFrom(msg.sender, address(this), boostAmount);
        users.userStoreReward +=calculateReward(msg.sender);
        users.timeForCalculateReward = block.timestamp;
    ...
    function remove(uint256 index) internal { 
        for(uint256 i = index; i<investment[msg.sender].deposites.length-1; i++){
            investment[msg.sender].deposites[i] = investment[msg.sender].deposites[i+1];
        }
    ...
    function withdrawToken(uint256 id) public {
        require(hasStart,"staking is deactive please wait for staking activated");
        uint256 duration = block.timestamp - investment[msg.sender].timeForCalculateReward;
    ...
    function compound() public {
        ...
        users.deposites.push(deposit(reward.sub(treasuryTax), block.timestamp));
        investment[msg.sender].timeForCalculateReward = block.timestamp;
    ...
    function reStakeRefferalReward()public {
        ...
        uint256 totalRefferReward = investment[msg.sender].refferReward;
        require(totalRefferReward > 0,"No refferal rewards found");
        investment[msg.sender].deposites.push(deposit(totalRefferReward, block.timestamp));
        if(users.boostlaval.userAddress != msg.sender){
        users.boostlaval.ratePerDay = 100;
        users.boostlaval.userAddress = msg.sender;
        users.timeForCalculateReward = block.timestamp;
    ...
    function getUserdipositAddamount(address _user) public view returns(uint256 ){
    ...
    function calculateReward(address _user) public view returns (uint256) {
        ...
        normalReward =  getUserdipositAddamount(_user).mul(100).mul(normalRewardDuration).div(1 days);
        normalReward = normalReward.div(10000);
        }
        else if((block.timestamp+5 minutes - boostTimestamp) < 5 minutes) {
        uint256 boostRewardtime = block.timestamp+5 minutes - boostTimestamp;
        boostedReward =  getUserdipositAddamount(_user).mul(users.boostlaval.ratePerDay).mul(boostRewardtime).div(1 days);
        boostedReward = boostedReward.div(10000);

        }else if(block.timestamp+5 minutes - boostTimestamp > 5 minutes)  {
        uint256 normalRewardtime = block.timestamp - boostTimestamp;
        boostedReward =  getUserdipositAddamount(_user).mul(users.boostlaval.ratePerDay).mul(5 minutes).div(1 days);
        boostedReward = boostedReward.div(10000);
        normalReward = getUserdipositAddamount(_user).mul(100).mul(normalRewardtime).div(1 days);
        normalReward = normalReward.div(10000);
        }
    ...
    function minimumWithdrawDuration(address _user) public view returns(uint256){
        return investment[_user].timeForCalculateReward;
    }
    ...
    function getUserTotalRefferalRewards(address _user) public view returns(uint256){
        return investment[_user].refferReward;
    }
    function getUserTotalWithdrawRewards(address _user) public view returns(uint256){
        return investment[_user].withdrawReward;
    }
    function getUserlastWithdrawRewardTime(address _user) public view returns(uint256){
        return investment[_user].lastWithdrawReward;
    }
    function getUserDepositeHistory( address _user) public view returns(uint256[] memory,uint256[] memory){
        uint256[] memory amount = new uint256[](investment[_user].deposites.length);
        uint256[] memory time = new uint256[](investment[_user].deposites.length);
