...
contract MEG is ERC721, Ownable {
  ...
  IMEG _meg = IMEG(0xDf6353Cd0a915fe89Ad9e64BF05D742Da0fA9a8c);

  event Purchase(address owner, uint256 price, uint256 id, string uri);

  constructor() ERC721("MEG", "MEG-NFT") {
  ...
  function airdrop(uint256 _id, address receiver)
    public
    onlyOwner
  {
    require(_exists(_id), "Error, wrong Token id");
    ...
    _transfer(address(this), receiver, _id);
    sold[_id] = true;
  }

  function buyByMEG(uint256 _id) public  
  {
    require(_exists(_id), "Error, wrong Token id");
    require(!sold[_id], "Error, Token is sold");
    // require(msg.value < price[_id], "Error, Token costs more");

    _meg.transfer(msg.sender, price[_id]);
