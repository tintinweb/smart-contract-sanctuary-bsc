...
contract PlanetFarm is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    // mapping(address => uint256) public gammaDeposited;
    // mapping(address => uint256) public gammaWithdrawn;
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) external nonReentrant {
        ...
        if (_wantAmt != 0) {
            ...
            if(_pid == iGammaPid) {
                ...
                //gammaDeposited[_msgSender()] += receivedAmount;
    ...
    function claimAllPoolsPendingGamma() external nonReentrant {
        ...
        for(uint256 i = 0; i < length; ++i) {
            ...
            if(pool.isInfinity){
                user.rewardDebt = ((IERC20(pool.iToken).balanceOf(_msgSender()) - user.iTokenToBeUnstaked)*pool.accGAMMAPerShare)/1e12;
            }
            else{
                user.rewardDebt = (user.shares*pool.accGAMMAPerShare + user.factor*pool.accGAMMMAPerFactorPerShare)/1e12;
            }
        ...
        if(totalPending != 0) {
            if(autoStakeGamma)
                ...
                IERC20(GAMMA).safeTransfer(_msgSender(), totalPending);
        }
    }
