...
error MarketPlace__PriceMustAboveZero();
...
error MarketPlace__PayNotEnough(
    address nftAddress,
    uint256 tokenId,
    uint price
);
error MarketPlace__NoProceeds();
error MarketPlace__TransferFall();
...
contract MarketPlace {
    ...
    {
        if (price <= 0) {
            revert MarketPlace__PriceMustAboveZero();
        }
        IERC721 nft = IERC721(nftAddress);
        ...
        isListed(nftAddress, tokenId)
    {
        ...
        if (msg.value < listedItem.price) {
            revert MarketPlace__PayNotEnough(
                nftAddress,
                tokenId,
                listedItem.price
            );
    ...
    function withdrawProceeds() external {
        ...
        if (!success) {
            revert MarketPlace__TransferFall();
        }
