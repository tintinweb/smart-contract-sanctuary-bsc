...
contract CrownNFT is ERC721Votes, ERC721Enumerable {
    ...
    uint256 private _transferCoolDown = (30 * 24 * 3600) / 3;
    ...
    function stakeOrUnstake(uint256 tokenId, bool status) external {
        require(_exists(tokenId), "CROWNNFT:Not exist");
        require(_isApprovedOrOwner(msg.sender, tokenId), "CROWNNFT:Not owner or approved");
        _approve(address(0),tokenId);
        CrownTraits storage traits = _tokenIdToTraits[tokenId];
        traits.staked = status;
    }
    ...
    function _mint(address to, uint256 tokenId) internal override(ERC721) {
        ...
        uint256 reduce = (randomNumber % 21) + 10;
        uint256 aprBonus = (uint256(
        ...
        _tokenIdToTraits[tokenId] = CrownTraits(reduce, aprBonus, 0, false);
    ...
    function getPastNumberOfTokenLocked(address account, uint256 blockNumber)
        public
        view
        returns (uint256)
    {
        uint256 numberLocked;
        ...
        returns (uint256)
    {
        return
            ...
            getPastNumberOfTokenLocked(account, blockNumber);
    }
    ...
    function _transfer(
        ...
        if (traists.lockDeadline == 0 || traists.lockDeadline >= block.number) {
            super._transfer(from, to, tokenId);
        } else {
            revert("CROWNNFT:Locking");
        }
    }
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "CROWNNFTERROR: Only owner");
        _;
    }
}