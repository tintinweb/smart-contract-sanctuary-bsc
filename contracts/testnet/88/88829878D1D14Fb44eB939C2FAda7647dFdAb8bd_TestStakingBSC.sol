...
contract TestStakingBSC is IStaking, RewardsAdministrator, ReentrancyGuard, Pausable {
    ...
    event UpdatePercentage(address token, uint256 percentage);
    ...
    function earned(address account, address token) public view override returns (uint256) {
        return users[account].vaults[token].mul(rewardPerToken(token).sub(users[account].rewardPaid[token])).add(users[account].rewards[token]);
    }
    ...
    function claimReward(address token) public override nonReentrant updateReward(msg.sender, token) {
        ...
        if (reward > 0) {
            ...
            IERC20(token).safeTransferFrom(rewardsVault, msg.sender, reward);
    ...
    function updatePercentage(address token, uint256 percentage) external override onlyRewardsAdministrator validToken(token) updateReward(address(0), token) {
        ...
        emit UpdatePercentage(token, percentage);
    }
}
...
abstract contract RewardsAdministrator is Ownable {
    ...
    function updatePercentage(address _token, uint256 _percentage) external virtual;
    ...
    function setRewardsVault(address _rewardsVault) external virtual onlyRewardsAdministrator {
