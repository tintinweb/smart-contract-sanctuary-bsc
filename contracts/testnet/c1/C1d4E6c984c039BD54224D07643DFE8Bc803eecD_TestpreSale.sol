...
contract TestpreSale is Ownable {
    ...
    function TokenPriceInETH() public view returns(int) {
        int ethPrice = getETHPrice();
        return tokenPriceUSD / ethPrice;
    }

    function settokenPriceUSD() public view returns(int) {
    ...
    function buyToken(uint256 _amount) public payable returns (address) {
        require(isPresaleOpen, "Presale is not open.");
                int facuTokenPriceETH = TokenPriceInETH();
        ...
        require(IToken(tokenAddress).balanceOf(address(this)) >= _amount);
        require(
        ...
        uint256 tokenAmount = _amount;//getTokensPerEth(msg.value);
