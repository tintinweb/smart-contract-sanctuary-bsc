...
contract CexVaultFacet is ICexVault {

    bytes32 private constant ADMIN_ROLE = keccak256("ADMIN_ROLE");

    function initCexVaultFacet(address cexOracle_, uint16 securityMarginRate_) external {
        LibAccessControlEnumerable.checkRole(LibAccessControlEnumerable.DEPLOYER_ROLE);
        require(cexOracle_ != address(0), "CexVaultFacet: Invalid cexOracle_ address");
        require(securityMarginRate_ > 0, "CexVaultFacet: Invalid securityMarginRate_");
        LibCexVault.initialize(cexOracle_, securityMarginRate_);
    }

    function cexOracle() external view override returns (address) {
        return LibCexVault.cexVaultStorage().cexOracle;
    }

    function updateCexOracle(address cexOracle_) external override {
        LibAccessControlEnumerable.checkRole(ADMIN_ROLE);
        require(cexOracle_ != address(0), "CexVaultFacet: Invalid cexOracle_ address");
        LibCexVault.updateCexOracle(cexOracle_);
    }

    function securityMarginRate() external view override returns (uint16) {
        return LibCexVault.cexVaultStorage().securityMarginRate;
    }
    ...
    function itemCexValue(address tokenAddress) external view override returns (IVault.LpItem memory lpItem) {
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        LibVault.AvailableToken storage at = vs.tokens[tokenAddress];
        require(at.tokenAddress != address(0), "CexVaultFacet: token does not exist");
        LibCexVault.CexVaultStorage storage cvs = LibCexVault.cexVaultStorage();
        ICexAsset.AssetDetailRecord[] memory assetRecords = ICexAsset(cvs.cexOracle).getRecordsAtIndex(0).records;
        int256 tokenValue;
        for (uint8 i; i < assetRecords.length; i++) {
            ICexAsset.AssetDetailRecord memory asset = assetRecords[i];
            if (asset.symbol == at.tokenAddress &&
                (asset.assetType == ICexAsset.AssetType.AssetBalance || asset.assetType == ICexAsset.AssetType.UnRealizedPnl)) {
                tokenValue += int256(asset.balance);
            }
        }
        return _itemCexValue(at, tokenValue);
    }

    function _itemCexValue(LibVault.AvailableToken storage at, int256 tokenValue) internal view returns (IVault.LpItem memory lpItem) {
        address tokenAddress = at.tokenAddress;
        uint256 price = LibPriceFacade.getPrice(tokenAddress);
        int256 valueUsd = int256(price) * tokenValue * int256((10 ** LibPriceFacade.USD_DECIMALS)) / int256((10 ** (LibPriceFacade.PRICE_DECIMALS + at.decimals)));
        lpItem = IVault.LpItem(tokenAddress, tokenValue, at.decimals, valueUsd, at.weight, at.feeBasisPoints, at.taxBasisPoints, at.dynamicFee);
    }

    function totalCexValue() external view override returns (IVault.LpItem[] memory lpItems) {
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        LibCexVault.CexVaultStorage storage cvs = LibCexVault.cexVaultStorage();
        return _totalCexValue(vs, cvs);
    }

    function _totalCexValue(LibVault.VaultStorage storage vs, LibCexVault.CexVaultStorage storage cvs) private view returns (IVault.LpItem[] memory lpItems) {
        ICexAsset.AssetDetailRecord[] memory assetRecords = ICexAsset(cvs.cexOracle).getRecordsAtIndex(0).records;
        uint256 index;
        IVault.LpItem[] memory _lpItems = new IVault.LpItem[](assetRecords.length);
        for (uint256 i; i < assetRecords.length; i++) {
            ICexAsset.AssetDetailRecord memory asset = assetRecords[i];
            LibVault.AvailableToken storage at = vs.tokens[asset.symbol];
            if (at.weight > 0 && asset.assetType == ICexAsset.AssetType.AssetBalance) {
                int256 tokenValue = int256(asset.balance);
                for (uint256 j = 0; j < assetRecords.length; j++) {
                    ICexAsset.AssetDetailRecord memory asset_j = assetRecords[j];
                    if (asset_j.symbol == asset.symbol && asset_j.assetType == ICexAsset.AssetType.UnRealizedPnl) {
                        tokenValue += int256(asset_j.balance);
                    }
                }
                _lpItems[index] = _itemCexValue(at, tokenValue);
                index++;
            }
        }
        lpItems = new IVault.LpItem[](index);
        for (uint256 i; i < index; i++) {
            lpItems[i] = _lpItems[i];
        }
    }

    function maxWithdrawAbleUsd() external view override returns (int256) {
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        LibCexVault.CexVaultStorage storage cvs = LibCexVault.cexVaultStorage();
        IVault.LpItem[] memory lpItems = _totalCexValue(vs, cvs);
        ICexAsset.AssetDetailRecord[] memory assetRecords = ICexAsset(cvs.cexOracle).getRecordsAtIndex(0).records;
        ...
        int256 cexTotalAsset;
        for (uint256 i; i < lpItems.length; i++) {
            cexTotalAsset += lpItems[i].valueUsd;
        }
