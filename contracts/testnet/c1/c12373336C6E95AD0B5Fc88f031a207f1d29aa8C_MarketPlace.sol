...
contract MarketPlace is ReentrancyGuard, ERC721Holder {
    ...
    event ORDER(address indexed ct, uint256 indexed tokenId, address indexed who, uint256 openPrice, uint256 startTime, uint256 duration, bool keep, uint256 timestamp);
    event MATCH(address indexed ct, uint256 indexed tokenId, address buyer, address seller, uint256 matchPrice, string data, uint256 timestamp);
    ...
    address payable ownerMartket; //owner of the smart contract

    constructor(){
        ownerMartket = payable(msg.sender);
    }
    ...
    function sell(
        ...
        require(openPrice > 0, "Price must be above zero");
        _itemIds.increment(); //add 1 to the total number of items ever created
        uint256 itemId = _itemIds.current();
        order[nftContract][tokenId] = Order(itemId, nftContract, tokenId, payable(msg.sender), payable(address(0)), openPrice, startTime, duration, sold);
        if (!pushedOrder[msg.sender][nftContract][tokenId]){
            pushedOrder[msg.sender][nftContract][tokenId] = true;
            myOrder[msg.sender].push(Path(_ver, nftContract, tokenId));
        }
        if (listOffers[nftContract][tokenId].length > 0){
            listOffers[nftContract][tokenId] = new address[](0);
        }
        IERC721(nftContract).transferFrom(msg.sender, address(this), tokenId);
        emit ORDER(nftContract, tokenId, msg.sender, openPrice, startTime, duration, sold, block.timestamp);
    }
    ...
    function buy(
        ...
        Order memory _order = order[nftContract][_tokenId];
        uint price = _order.openPrice;
        address payable seller = _order.seller;
        require(msg.value == price, "Please submit the asking price in order to complete purchase");
        //pay the seller the amount
        // uint tax = msg.value * _fees / 10**6;
        order[nftContract][_tokenId].seller.transfer(msg.value);
        // payable(ownerMartket).transfer(tax);
        ...
        order[nftContract][_tokenId].owner = payable(msg.sender); //mark buyer as new owner
        emit MATCH(nftContract, _tokenId, msg.sender, seller, price, 'buy', block.timestamp);
        // payable(owner).transfer(fee); //pay owner of contract the listing price
    }
    ...
    function setOffers(uint _ver, address _ct, uint256 _tokenId, address _unit, uint _offersPrice, uint256 _startTime, uint256 _duration) public isSale(_ver, _ct, _tokenId) {
        require(!isOwnerNFTOnMarket(_ct, msg.sender, _tokenId), "Owner can not offer!");
    ...
    function swap(uint _ver, address _ct, uint256 _tokenId, address _buyer, address _seller, uint256 _matchPrice, address _martket, uint _fees, string memory _data) public payable {
        order[_ct][_tokenId].seller.transfer(_matchPrice);
        IERC721(_ct).transferFrom(address(this), msg.sender, _tokenId);
        emit MATCH(_ct, _tokenId, _buyer, _seller, _matchPrice, _data, block.timestamp);
    }
    ...
    function accept(uint _ver, address _ct, uint256 _tokenId, address _buyer, address _martketAddress, uint _fees) public {
        require(isOwnerNFTOnMarket(_ct, msg.sender, _tokenId), "Error, you are not the owner");
        require(order[_ct][_tokenId].openPrice > 0, "Not Sale");
        require(offers[_buyer][_ct][_tokenId].offersPrice > 0, "");
        ...
        swap(_ver, _ct, _tokenId, _buyer, msg.sender, offers[_buyer][_ct][_tokenId].offersPrice, _martketAddress, _fees, "accept");
    }
