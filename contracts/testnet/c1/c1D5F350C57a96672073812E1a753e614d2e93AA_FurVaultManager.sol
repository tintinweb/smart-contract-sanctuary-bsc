...
import "./Interfaces/IFurVaultManager.sol";
import "./Interfaces/IFurStabilityPool.sol";
import "./Interfaces/IFurCollSurplusPool.sol";
import "./Interfaces/IFURUSDToken.sol";
import "./Interfaces/IFurSortedVaults.sol";
import "./Interfaces/ILOANToken.sol";
import "./Interfaces/ILOANStaking.sol";
import "./Dependencies/FurLiquityBase.sol";

contract FurVaultManager is BaseContract, FurLiquityBase, IFurVaultManager {
    ...
    IFurStabilityPool public stabilityPool;

    IFurCollSurplusPool collSurplusPool;
    ...
    IFurSortedVaults public sortedVaults;
    ...
    */
    ...
    /*
    * L_FUR and L_FURUSDDebt track the sums of accumulated liquidation rewards per unit staked. During its lifetime, each stake earns:
    *
    * An FUR gain of ( stake * [L_FUR - L_FUR(0)] )
    * A FURUSDDebt increase  of ( stake * [L_FURUSDDebt - L_FURUSDDebt(0)] )
    *
    * Where L_FUR(0) and L_FURUSDDebt(0) are snapshots of L_FUR and L_FURUSDDebt for the active Vault taken at the instant the stake was made
    */
    uint public L_FUR;
    ...
    // Object containing the FUR and FURUSD snapshots for a given active vault
    struct RewardSnapshot { uint FUR; uint FURUSDDebt;}
    ...
    **/
    ...
    struct ContractsCache {
        IFurActivePool activePool;
        IFurDefaultPool defaultPool;
        IFURUSDToken furUSDToken;
        ILOANStaking loanStaking;
        IFurSortedVaults sortedVaults;
        IFurCollSurplusPool collSurplusPool;
    }
    ...
    event Redemption(uint _attemptedFURUSDAmount, uint _actualFURUSDAmount, uint _FURSent, uint _FURFee);
    ...
    event LTermsUpdated(uint _L_FUR, uint _L_FURUSDDebt);
    event VaultSnapshotsUpdated(uint _L_FUR, uint _L_FURUSDDebt);
        ...
        onlyOwner
    {
        ...
        activePool = IFurActivePool(_activePoolAddress);
        defaultPool = IFurDefaultPool(_defaultPoolAddress);
        stabilityPool = IFurStabilityPool(_stabilityPoolAddress);
        collSurplusPool = IFurCollSurplusPool(_collSurplusPoolAddress);
        priceFeed = IFurPriceFeed(_priceFeedAddress);
        furUSDToken = IFURUSDToken(_furUSDTokenAddress);
        sortedVaults = IFurSortedVaults(_sortedVaultsAddress);
    ...
    function _liquidateNormalMode(
        IFurActivePool _activePool,
        IFurDefaultPool _defaultPool,
        address _borrower,
    ...
    */
    function liquidateVaults(uint _n) external override {
        ContractsCache memory contractsCache = ContractsCache(
            ...
            sortedVaults,
            IFurCollSurplusPool(address(0))
        );
        IFurStabilityPool stabilityPoolCached = stabilityPool;
        ...
        // Move liquidated FUR and FURUSD to the appropriate pools
        stabilityPoolCached.offset(totals.totalDebtToOffset, totals.totalCollToSendToSP);
        _redistributeDebtAndColl(contractsCache.activePool, contractsCache.defaultPool, totals.totalDebtToRedistribute, totals.totalCollToRedistribute);
        if (totals.totalCollSurplus > 0) {
            contractsCache.activePool.sendFUR(address(collSurplusPool), totals.totalCollSurplus);
        }
    ...
    */
        ...
        returns(LiquidationTotals memory totals)
    {
        LocalVariables_LiquidationSequence memory vars;
        LiquidationValues memory singleLiquidation;
        IFurSortedVaults sortedVaultsCached = sortedVaults;
    ...
    */
    function batchLiquidateVaults(address[] memory _vaultArray) public override {
        ...
        IFurActivePool activePoolCached = activePool;
        IFurDefaultPool defaultPoolCached = defaultPool;
        IFurStabilityPool stabilityPoolCached = stabilityPool;
        ...
        if (totals.totalCollSurplus > 0) {
            activePoolCached.sendFUR(address(collSurplusPool), totals.totalCollSurplus);
        }
    ...
    */
    ...
    function _movePendingVaultRewardsToActivePool(IFurActivePool _activePool, IFurDefaultPool _defaultPool, uint _FURUSD, uint _FUR) internal {
        _defaultPool.decreaseFURUSDDebt(_FURUSD);
        _activePool.increaseFURUSDDebt(_FURUSD);
        _defaultPool.sendFURToActivePool(_FUR);
    }
        ...
        override
    {
        ...
        while (currentBorrower != address(0) && totals.remainingFURUSD > 0 && _maxIterations > 0) {
            ...
            totals.totalFURDrawn = totals.totalFURDrawn.add(singleRedemption.FURLot);

            totals.remainingFURUSD = totals.remainingFURUSD.sub(singleRedemption.FURUSDLot);
            currentBorrower = nextUserToCheck;
        }
        require(totals.totalFURDrawn > 0, "VaultManager: Unable to redeem any amount");

        // Decay the baseRate due to time passed, and then increase it according to the size of this redemption.
        // Use the saved total FURUSD supply value, from before it was reduced by the redemption.
        _updateBaseRateFromRedemption(totals.totalFURDrawn, totals.price, totals.totalFURUSDSupplyAtStart);

        // Calculate the FUR fee
        totals.FURFee = _getRedemptionFee(totals.totalFURDrawn);
        _requireUserAcceptsFee(totals.FURFee, totals.totalFURDrawn, _maxFeePercentage);

        // Send the FUR fee to the LOAN staking contract
        contractsCache.activePool.sendFUR(address(contractsCache.loanStaking), totals.FURFee);
        contractsCache.loanStaking.increaseF_FUR(totals.FURFee);

        totals.FURToSendToRedeemer = totals.totalFURDrawn.sub(totals.FURFee);

        emit Redemption(_FURUSDamount, totals.totalFURUSDToRedeem, totals.totalFURDrawn, totals.FURFee);

        // Burn the total FURUSD that is cancelled with debt, and send the redeemed FUR to msg.sender
        contractsCache.furUSDToken.burn(msg.sender, totals.totalFURUSDToRedeem);
        // Update Active Pool FURUSD, and send FUR to account
        contractsCache.activePool.decreaseFURUSDDebt(totals.totalFURUSDToRedeem);
        contractsCache.activePool.sendFUR(msg.sender, totals.FURToSendToRedeemer);
    }
        ...
        internal returns (SingleRedemptionValues memory singleRedemption)
    {
        ...
        // Get the FURLot of equivalent value in USD
        singleRedemption.FURLot = singleRedemption.FURUSDLot.mul(DECIMAL_PRECISION).div(_price);

        // Decrease the debt and collateral of the current Vault according to the FURUSD lot and corresponding FUR to send
        uint newDebt = (Vaults[_borrower].debt).sub(singleRedemption.FURUSDLot);
        uint newColl = (Vaults[_borrower].coll).sub(singleRedemption.FURLot);
        ...
        if (newDebt == 0) {
            ...
            // send FUR from Active Pool to CollSurplus Pool
            _contractsCache.collSurplusPool.accountSurplus(_borrower, newColl);
            _contractsCache.activePool.sendFUR(address(_contractsCache.collSurplusPool), newColl);
    ...
    function _isValidFirstRedemptionHint(IFurSortedVaults _sortedVaults, address _firstRedemptionHint, uint _price) internal view returns (bool) {
    ...
    function getNominalICR(address _borrower) public view override returns (uint) {
        (uint currentFUR, uint currentFURUSDDebt) = _getCurrentVaultAmounts(_borrower);

        uint NICR = LiquityMath._computeNominalCR(currentFUR, currentFURUSDDebt);
        return NICR;
    }
    ...
    function _applyPendingRewards(IFurActivePool _activePool, IFurDefaultPool _defaultPool, address _borrower) internal {
        if (hasPendingRewards(_borrower)) {
            ...
            Vaults[_borrower].coll = Vaults[_borrower].coll.add(pendingFURReward);
            Vaults[_borrower].debt = Vaults[_borrower].debt.add(pendingFURUSDDebtReward);

            _updateVaultRewardSnapshots(_borrower);

            // Transfer from DefaultPool to ActivePool
            _movePendingVaultRewardsToActivePool(_activePool, _defaultPool, pendingFURUSDDebtReward, pendingFURReward);
    ...
    // Update borrower's snapshots of L_FUR and L_FURUSDDebt to reflect the current values
    ...
    function _updateVaultRewardSnapshots(address _borrower) internal {
        rewardSnapshots[_borrower].FUR = L_FUR;
        rewardSnapshots[_borrower].FURUSDDebt = L_FURUSDDebt;
        emit VaultSnapshotsUpdated(L_FUR, L_FURUSDDebt);
    }

    // Get the borrower's pending accumulated FUR reward, earned by their stake
    function getPendingFURReward(address _borrower) public view override returns (uint) {
        uint snapshotFUR = rewardSnapshots[_borrower].FUR;
        uint rewardPerUnitStaked = L_FUR.sub(snapshotFUR);

        if ( rewardPerUnitStaked == 0 || Vaults[_borrower].status != Status.active) { return 0; }

        uint stake = Vaults[_borrower].stake;

        uint pendingFURReward = stake.mul(rewardPerUnitStaked).div(DECIMAL_PRECISION);

        return pendingFURReward;
    }
    ...
    function hasPendingRewards(address _borrower) public view override returns (bool) {
        ...
        */
        ...
        return (rewardSnapshots[_borrower].FUR < L_FUR);
    }
    ...
    function getEntireDebtAndColl(address _borrower)
        ...
        view
        override
        returns (uint debt, uint coll, uint pendingFURUSDDebtReward, uint pendingFURReward)
    {
        debt = Vaults[_borrower].debt;
        coll = Vaults[_borrower].coll;

        pendingFURUSDDebtReward = getPendingFURUSDDebtReward(_borrower);
        pendingFURReward = getPendingFURReward(_borrower);

        debt = debt.add(pendingFURUSDDebtReward);
        coll = coll.add(pendingFURReward);
    }
    ...
    function _redistributeDebtAndColl(IFurActivePool _activePool, IFurDefaultPool _defaultPool, uint _debt, uint _coll) internal {
        if (_debt == 0) { return; }

        uint FURNumerator = _coll.mul(DECIMAL_PRECISION);
        uint FURUSDDebtNumerator = _debt.mul(DECIMAL_PRECISION);
        uint FURRewardPerUnitStaked = FURNumerator.div(totalStakes);
        uint FURUSDDebtRewardPerUnitStaked = FURUSDDebtNumerator.div(totalStakes);

        // Add per-unit-staked terms to the running totals
        L_FUR = L_FUR.add(FURRewardPerUnitStaked);
        L_FURUSDDebt = L_FURUSDDebt.add(FURUSDDebtRewardPerUnitStaked);

        emit LTermsUpdated(L_FUR, L_FURUSDDebt);

        // Transfer coll and debt from ActivePool to DefaultPool
        _activePool.decreaseFURUSDDebt(_debt);
        _defaultPool.increaseFURUSDDebt(_debt);
        _activePool.sendFUR(address(_defaultPool), _coll);
    }
    ...
    function _closeVault(address _borrower, Status closedStatus) internal {
        ...
        rewardSnapshots[_borrower].FUR = 0;
    ...
    function _updateSystemSnapshots(IFurActivePool _activePool) internal {
        totalStakesSnapshot = totalStakes;

        uint activeColl = _activePool.getFUR();
        uint liquidatedColl = defaultPool.getFUR();
    ...
    */
    ...
    // Check whether or not the system *would be* in Recovery Mode, given an FUR:USD price, and the entire system coll and debt.
    ...
    */
    function _updateBaseRateFromRedemption(uint _FURDrawn,  uint _price, uint _totalFURUSDSupply) internal returns (uint) {
        uint decayedBaseRate = _calcDecayedBaseRate();
        /* Convert the drawn FUR back to FURUSD at face value rate (1 FURUSD:1 USD), in order to get
        * the fraction of total supply that was redeemed at face value. */
        uint redeemedFURUSDFraction = _FURDrawn.mul(_price).div(_totalFURUSDSupply);
    ...
    function _getRedemptionFee(uint _FURDrawn) internal view returns (uint) {
        return _calcRedemptionFee(getRedemptionRate(), _FURDrawn);
    }

    function getRedemptionFeeWithDecay(uint _FURDrawn) external view override returns (uint) {
        return _calcRedemptionFee(getRedemptionRateWithDecay(), _FURDrawn);
    }

    function _calcRedemptionFee(uint _redemptionRate, uint _FURDrawn) internal pure returns (uint) {
        uint redemptionFee = _redemptionRate.mul(_FURDrawn).div(DECIMAL_PRECISION);
        require(redemptionFee < _FURDrawn, "VaultManager: Fee would eat up all returned collateral");
...
interface IFurVaultManager {
    ...
    function getPendingFURReward(address _borrower) external view returns (uint);
    ...
    function getRedemptionFeeWithDecay(uint _FURDrawn) external view returns (uint);
...
interface IFurStabilityPool {

    function provideToSP(uint _amount) external;

    function withdrawFromSP(uint _amount) external;

    function withdrawFURGainToVault(address _upperHint, address _lowerHint) external;
    ...
    function getDepositorFURGain(address _depositor) external view returns (uint);
...
import "../Interfaces/IFurActivePool.sol";
import "../Interfaces/IFurDefaultPool.sol";
import "../Interfaces/IFurPriceFeed.sol";

/* 
* Base contract for VaultManager, BorrowerOperations and StabilityPool. Contains global system constants and
* common functions. 
*/
contract FurLiquityBase is BaseMath {
    using SafeMath for uint;
    ...
    IFurActivePool public activePool;

    IFurDefaultPool public defaultPool;

    IFurPriceFeed public priceFeed;

    function getEntireSystemColl() public view returns (uint entireSystemColl) {
        uint activeColl = activePool.getFUR();
        uint liquidatedColl = defaultPool.getFUR();
...
library LiquityMath {
     ...
     *
     * This value of 1e20 is chosen for safety: the NICR will only overflow for numerator > ~1e39 FUR,
...
interface IFurActivePool {
    ...
    function sendFUR(address _account, uint _amount) external;
...
interface IFurDefaultPool {
    ...
    function sendFURToActivePool(uint _amount) external;
