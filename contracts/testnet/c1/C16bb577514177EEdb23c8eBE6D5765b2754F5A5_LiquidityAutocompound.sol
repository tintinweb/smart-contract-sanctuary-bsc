...
import "./interfaces/IMasterCSS.sol";

contract LiquidityAutocompound is Pausable, Ownable, ReentrancyGuard {
    ...
    address constant public MASTERCSS = 0xf34CA3A71Cd10D19F5E869275Cd2d3044142A1fC;
    ...
    constructor() {
        ...
        ercCSS = IERC20(CSS);
        ercBUSD = IERC20(BUSD);
        masterCSS = IMasterCSS(MASTERCSS);
    }
    ...
    function deposit(uint256 _amount) public whenNotPaused nonReentrant {
        ...
        if(treasureAddress == address(0)) {
            ...
            uint256 busdOut = _swapCSSForBUSD(halfAmount);
            require(busdOut > 0, "Invalid swap");
                    
            // adding CSS + BUSD to LP
            uint liquidity = _addLiquidity(halfAmount, busdOut);

            masterCSS.deposit(2, liquidity, 0x0000000000000000000000000000000000000000);
        }
        else {
            uint256 fee = _amount * referrerFee.div(10000);
            uint256 depositAmount = _amount.sub(fee);
            ercCSS.safeTransferFrom(msg.sender, treasureAddress, fee);
            ercCSS.safeTransferFrom(msg.sender, address(this), depositAmount);

            uint256 halfAmount = depositAmount.div(2);
            uint256 busdOut = _swapCSSForBUSD(halfAmount);
            require(busdOut > 0, "Invalid swap");
    ...
    function withdraw(uint256 _amount) external whenNotPaused nonReentrant {
        masterCSS.withdraw(2, _amount);
        emit Withdrawn(msg.sender, _amount);
    }

    // Claim reward from MasterCSS
    function claimReward() external {
        uint256 pendingReward = masterCSS.pendingReward(2, msg.sender);
        masterCSS.stakeReward(2);

        emit RewardPaid(msg.sender, pendingReward);

        deposit(pendingReward);
    }

    function _swapCSSForBUSD(uint256 _tokenAmount) private returns (uint256 amountOut) {
        uint256 busdBalanceBefore = ercBUSD.balanceOf(address(this));
        ...
        amountOut = ercBUSD.balanceOf(address(this)).sub(busdBalanceBefore);
        emit SwapCSSForBUSD(_tokenAmount, path);
    }

    function _addLiquidity(uint256 _cssAmount, uint256 _busdAmount) private returns (uint liquidity) {
...
interface IMasterCSS {
    ...
    function stakeReward(uint256 _pid) external;
