...
contract ProjectSale is IProjectSale, ReentrancyGuard, Pausable, Ownable {
    ...
    uint256 public immutable saleValueUsd;
        ...
        )
    {
        ...
        saleValueUsd = _saleAmount;
        tokenPriceUsd = _tokenPrice;
        saleStatus = NOT_FINALIZED;
    }
    ...
    modifier onlyOnce() {
        require(saleStatus == NOT_FINALIZED, "ProjectSale: sale is finalized");
        _;
        ...
        onlyWhiteListed(msg.sender)
    {
        ...
        totalSoldUsd = totalSoldUsd + _amount;
    }
        ...
        returns (uint256)
    {
        ...
        if (isRegistered(_account) && isStakingRound()) {
            ...
            return saleValueUsd - totalSoldUsd;
        } else {
            return 0;
        }
    }

    function finalizeSale(uint64 _cliff, uint64 _durationInSec)
        public
        onlyOwner
        onlyOnce
        returns (address)
    {
        ...
        uint256 projectTokenSold = totalSoldUsd.div(tokenPriceUsd);
...
contract SplittedVesting is PaymentSplitter, Ownable, Pausable, IVesting {
    ...
    function claimable() public view returns (uint256) {
        uint256 additional = vestingWallet.vestedAmount(
            uint64(block.timestamp)
        );
        uint256 amount = releasable(token, msg.sender);
        return amount + _shareOf(msg.sender, additional);
    }
    ...
    function totalClaimable() public view returns (uint256) {
        uint256 saleEnd = vestingWallet.start() + vestingWallet.duration();
        uint256 saleAmount = vestingWallet.vestedAmount(
            address(token),
            uint64(saleEnd)
        );
        return _shareOf(msg.sender, saleAmount);
    }

    function totalClaimed() public view returns (uint256) {
        return released(token, msg.sender);
    }
}
...
interface IVesting {
    ...
    /**
     * @dev Returns currently claimable amount of tokens by the sender
     */
    function claimable() external view returns (uint256);

    /**
     * @dev Return total claimable amount of tokens by the sender
     */
    function totalClaimable() external view returns (uint256);

    /**
     * @dev Return total claimed amount of tokens by the sender
     */
