...
import "./ISuperiorDinoManager.sol";

contract SuperiorDinoBox is ERC721Enumerable, Ownable, ReentrancyGuard {
    constructor(address _superiorDinoBoxManagerAddress)
        ERC721("Superior Dino Box", "DINOSDBOX")
    {
        superiorDinoBoxManagerAddress = _superiorDinoBoxManagerAddress;
        boxOpenableAt = block.timestamp;
        privateRoundStartAt = 1646820000;
        privateRoundEndAt = 1646820900;
        maxAllocationPerWallet = 2;
        totalSellingBoxes = 20;
        boxPrice = 1999 * 1e18;
    }
    ...
    mapping(address => bool) public privateRoundWhitelistedAddresses;
    uint256 public boxOpenableAt;
    address public marketplaceContractAddress;
    address public nftContractAddress;
    address public tokenContractAddress;
    address public superiorDinoBoxManagerAddress;
    string public boxTokenURI;
    uint256 public boxPrice;
    /// @dev Time  begin and end buy box
    uint256 public privateRoundStartAt;
    uint256 public privateRoundEndAt;

    /// @dev Total boxes sale
    uint256 public totalSellingBoxes;
    uint256 public totalSoldBoxes;
    ...
    function setPrivateRoundWhitelistedAddresses(
        address[] memory _addresses,
        bool _isWhitelisted
    ) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            privateRoundWhitelistedAddresses[_addresses[i]] = _isWhitelisted;
        }
    }
    ...
    function setBoxManagerAddress(address _superiorDinoBoxManagerAddress)
        external
        onlyOwner
    {
        superiorDinoBoxManagerAddress = _superiorDinoBoxManagerAddress;
    ...
    function buyBox(uint256 _amount) external noContract nonReentrant {
        ...
        require(
            currentTimestamp >= privateRoundStartAt &&
                currentTimestamp <= privateRoundEndAt,
            "Buy box is not available at this time"
        );
        require(_amount > 0, "Amount must be greater than 0");
        require(boxPrice > 0, "Box price does not set yet");
        require(privateRoundWhitelistedAddresses[msg.sender] == true, "You are not whitelisted");
        require(
            addressToTotalBoughtBoxes[msg.sender].add(_amount) <=
                maxAllocationPerWallet,
            "You have reached your allocation"
        );
        require(
            totalSoldBoxes.add(_amount) <= totalSellingBoxes,
        ...
        addressToTotalBoughtBoxes[msg.sender] = addressToTotalBoughtBoxes[msg.sender].add(_amount);
        totalSoldBoxes = totalSoldBoxes.add(_amount);
        for(uint256 i = 0; i < _amount; i++) {
            Box memory newBox = Box(block.number, block.timestamp, true);
            boxes.push(newBox);
            uint256 newBoxId = boxes.length - 1;
    ...
    function openBox(uint256 _boxId) external noContract nonReentrant {
        ...
        _burn(_boxId);
        uint256 dinoGenes = ISuperiorDinoManager(superiorDinoBoxManagerAddress)
...
interface ISuperiorDinoManager {
