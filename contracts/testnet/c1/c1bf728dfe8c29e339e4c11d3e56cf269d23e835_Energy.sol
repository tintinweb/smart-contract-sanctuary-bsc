...
contract Energy is IEnergy, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _balances[sender] = _balances[sender].sub(amount, "Energy: transfer amount exceeds balance");
     ...
     */
    function produce(uint256 amount, uint cleanness) external onlyProducer {
        require(amount > 0, "Energy: Cannot produce zero energy");
        require(cleanness >= 0, "Energy: cleanness cannot be less than 1 degree");
        require(cleanness <= 100, "Energy: cleanness cannot be over 100 degree");

        _totalProduced = _totalProduced.add(amount);
        _balances[_msgSender()] = _balances[_msgSender()].add(amount);
        emit Transfer(address(0), _msgSender(), amount);
        emit Production(_msgSender(), amount, cleanness);
    }
    ...
    function withdrawUnusedEnergy(IEnergy _energy) external onlyOwner {
        uint256 balance = _energy.balanceOf(address(this));
