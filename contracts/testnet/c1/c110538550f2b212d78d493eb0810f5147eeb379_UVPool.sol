...
import "./interfaces/IUVReverseFactory.sol";
...
contract UVPool is ERC20, ERC20Burnable, Ownable, AccessControl, IUVPool {
    ...
    function setPercentFee(uint8 _percentFee) public onlyRole(MANAGER_ROLE) {
        require(
            _percentFee > 10 && _percentFee < 300,
    ...
    function getReserve() public view returns (address) {
        ...
        //     "getPoolReserve(uint8)",
        //     orderNumber
        // );
        // (bool success, bytes memory returnData) = factoryReverse.call(payload);
        // require(success);
        (address _poolReserved,,,) = IUVReverseFactory(factoryReverse).getPoolInfo(
            orderNumber
        );
    ...
    function buyNativeToken(
        ...
        IERC20(stableCoin).approve(PANCAKE_ROUTER_ADDRESS, _amountIn);
    ...
    // Set the factory reverse
    function setFactoryReverse(address _factoryReverse) public onlyRole(MANAGER_ROLE) {
        factoryReverse = _factoryReverse;
    }
...
interface IUVPool {
    ...
    function transferToVesting(address _tokenAddress, uint256 _amount) external;

    function setPercentFee(uint8 _percentFee) external;
    ...
    function setMaxSizePool(uint256 _maxSizePool) external;
    ...
    // function getVoteInfo(uint8 _orderNumber) external;
