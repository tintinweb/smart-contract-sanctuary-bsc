...
contract RacingCarController is Ownable {
    ...
    uint256 private randNum;
     ...
     //出金地址 4个账户
    address public kickbackAddress = 0x53463812ca5000c357aDfC6B8112a0cd95829F14; //回扣
    // address public vipAddress = 0x03172203846453Ff8E0e759B70031731c6dC5a04;//会员 卡
    address public vipAddress = address(this);//会员 卡
    address public leagueAddress = 0x7451f1e8f54EF4c423f3CdE961Cc51af318CF580; //联赛
    address public marketingAddress = 0x5e8156A990355Fe9A4e03d416DD1159ED6D878af;//营销


    mapping(uint256 => Component) public components; //用户拥有的部件

    mapping(address => uint256[]) public myComponentCids; //用户拥有的部件

    mapping(uint256 => uint256) public cIdToIndexs; //用户拥有的部件
    ...
    mapping(uint256 => uint256) public tokenIdToWhtidrawTime;
    ...
    constructor() {
        RCN = RacingCarNFT(0x9AA7Ad0Adb5953498061deD3CC1Ed4f50594c7cB);
    }
    ...
    function setKickbackAddress(address _kickbackAddress) public onlyOwner {
        kickbackAddress = _kickbackAddress;
    }

    function setVipAddress(address _vipAddress) public onlyOwner {
        vipAddress = _vipAddress;
    }

    function setLeagueAddress(address _leagueAddress) public onlyOwner {
    ...
    function openBlindBox(uint256 types) public payable{
        ...
        require(msg.value == blindBoxPrice, "test");
        ...
        uint256 price1 = msg.value.mul(44).div(10);
        uint256 price2 = msg.value.mul(30).div(10);
        uint256 price3 = msg.value.mul(16).div(10);
    ...
    function settlement() public onlyController {
        // require(block.timestamp.div(24 hours) - settlementTimesToTime[settlementTimesToTime.length - 1].div(24 hours) > 0, "This user is not the owner of the component");

        settlementTimesToTime.push(block.timestamp);

        uint256 nft1Num = RCN.NFTTypeNumber(1);

        uint256 nft2Num = RCN.NFTTypeNumber(2);

        uint256 nft3Num = RCN.NFTTypeNumber(3);
    ...
    function whtidrawDividends() public {
        ...
        uint256[] memory ids = RCN.getUserNFTIDs(msg.sender);
        ...
        for(uint256 i = 0; i < ids.length; i++){
            if(ids[i] != 0){
                for(uint256 j = settlementTimesToTime.length; j > 0; j--){
                    if(settlementTimesToTime[j - 1] > tokenIdToWhtidrawTime[ids[i]]){
                        profit = profit.add(settlementTimesToDividends[j][RCN.NFTTypes(ids[i])]);
                    }
                }
                tokenIdToWhtidrawTime[ids[i]] = time;
            }
        }
    ...
    function deleteMyComponent(address user, uint256 cid) public onlyController {
        require(msg.sender == components[cid].holder, "This user is not the owner of the component");
    ...
    function getUserDividends(address user) public view returns(uint256) {
        ...
        uint256[] memory ids = RCN.getUserNFTIDs(user);
    ...
    function queryMyComponent(address user) public view returns(Component[] memory myComponents){
        ...
        for(uint256 i = 0; i < cids.length; i++){
            if(cids[i] != 0){
                myComponents[count] = components[cids[i]];
                count++;
            }
        }
