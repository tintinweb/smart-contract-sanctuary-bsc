...
interface ISaleVault {
    function createLock(uint8 _round, address _token, uint256 _token_amount, uint256 _weth_amount, uint256 _start, uint256 _end, uint256 _slippage, uint256 _deadline) external payable;
}
...
contract Sale is ReentrancyGuard {
    ...
    function finalizeSale (uint8 _round, uint256[5] memory _lock_details) public nonReentrant IsSaleEnded(_round) {
        ...
        if(sale_details.hasLiquidity == 1) {
            ...
            _saleVault.createLock{value:payAmounts[1]}(_round, sale_details.sale_token, liquidityToken, payAmounts[1], sale_info[_round].end, sale_info[_round].end + (sale_info[_round].liquidityLock * 1 minutes), _lock_details[3], _lock_details[4]);
        }
