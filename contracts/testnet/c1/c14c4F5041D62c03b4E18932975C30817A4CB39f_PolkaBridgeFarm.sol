...
contract PolkaBridgeFarm is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accPBRPerShare; // Accumulated PBRs per share, times 1e8. See below.        
    }
    // The PBR TOKEN!
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accPBRPerShare = accPBRPerShare + PBRReward * 1e8 / lpSupply;
        }
        return user.amount * accPBRPerShare / 1e8 - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 PBRReward = multiplier * PBRPerBlock * pool.allocPoint / totalAllocPoint;
        pool.accPBRPerShare = pool.accPBRPerShare + PBRReward * 1e8 / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    function harvest(uint256 _pid) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount * pool.accPBRPerShare / 1e8 - user.rewardDebt;
        ...
        user.rewardDebt = user.amount * pool.accPBRPerShare / 1e8;
