...
contract Sale is ERC721Holder, ERC1155Holder, Ownable, Initializable {
    ...
    event BidSingleProjectAdmin(uint256 indexed saleId, uint256 indexed tokenId, uint256 value);
    event BidSingleProjectUser(uint256 indexed saleId, uint256 indexed tokenId, uint256 percent, uint256 valueForAdmin, uint256 valueForUser);
    event BidMultiProjectAdmin(uint256 indexed saleId, uint256 indexed tokenId, uint256 amount, uint256 value);
    event BidMultiProjectUser(uint256 indexed saleId, uint256 indexed tokenId, uint256 amount, uint256 percent, uint256 valueForAdmin, uint256 valueForUser);
    ...
    modifier onlyAdmin() {
        require(_msgSender() == project.getSuperAdmin() || _msgSender() == project.getAdmin(), "caller is not the admin");
        _;
    }

    modifier onlyManagerWithSaleId(uint256 saleId) {
        if (project.getProject(sales[saleId].projectId).isCreatedByAdmin) {
            require(_msgSender() == project.getSuperAdmin() || _msgSender() == project.getAdmin() || _msgSender() == project.getManager(sales[saleId].projectId), "caller is not the manager");
        } else {
    ...
    modifier onlyManagerWithProjectId(uint256 projectId) {
        if (project.getProject(projectId).isCreatedByAdmin) {
            require(_msgSender() == project.getSuperAdmin() || _msgSender() == project.getAdmin() || _msgSender() == project.getManager(projectId), "caller is not the manager");
        } else {
    ...
    function addWinners(uint256 _saleId, address[] memory _accounts) external onlyAdmin accountsNotEmpty(_accounts) {
    ...
    function removeWinners(uint256 _saleId, address[] memory _accounts) external onlyAdmin accountsNotEmpty(_accounts) {
        for (uint256 i; i < _accounts.length; i++) {
    ...
    function createRaiseSingle(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _raisePrices) external onlyManagerWithProjectId(_projectId) {
        require(_projectId > 0, "create: invalid project id");
        require(_tokenIds.length > 0 && _tokenIds.length == _raisePrices.length, "create: invalid param");
        IProject.Project memory _project = project.getProject(_projectId);
        require(_project.status != IProject.Status.STARTED, "project started");
        ...
        Sale[] memory _sales = new Sale[](_tokenIds.length);
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) {
        require(_saleId > 0, "bid: invalid sale id");

        Sale storage sale = sales[_saleId];
        require(!sale.isSoldOut, "bid: sold out");
        IProject.Project memory _project = project.getProject(sale.projectId); 
        ...
        if (!_project.isCreatedByAdmin) {
            IProject.ApprovalInfo memory approval = project.getApproval(_project.id);
            uint256 profitShare = _getProfitShare(msg.value, approval.percent);
            uint256 remainingProfit = msg.value.sub(profitShare);
            payable(project.getSuperAdmin()).transfer(profitShare);
            payable(_project.manager).transfer(remainingProfit);
            emit BidSingleProjectUser(_saleId, sale.tokenId, approval.percent, profitShare, remainingProfit);
        } 
        else {
            payable(project.getSuperAdmin()).transfer(msg.value);
            emit BidSingleProjectAdmin(_saleId, sale.tokenId, msg.value);
        }
    ...
    function bidMulti(uint256 _saleId, uint256 _amount) external payable onlyWinner(_saleId) {
        ...
        if (!_project.isCreatedByAdmin) {
            ...
            payable(project.getSuperAdmin()).transfer(profitShare);
            payable(_project.manager).transfer(remainingProfit);
            emit BidMultiProjectUser(_saleId, sale.tokenId, _amount, approval.percent, profitShare, remainingProfit);
        } 
        else {
            payable(project.getSuperAdmin()).transfer(msg.value);
            emit BidMultiProjectAdmin(_saleId, sale.tokenId, _amount, msg.value);
        }
    ...
    function close(uint256 _projectId, uint256[] memory _saleIds) external onlyManagerWithProjectId(_projectId) {
        require(_projectId > 0, "close: invalid project id");
    ...
    function _getProfitShare(uint256 _amount, uint256 _percent) private pure returns (uint256) {
