...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(operator != _msgSender(), "018");
    ...
    function transferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "019");
    ...
    function safeTransferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "020");
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, _data), "021");
    }
     ...
     * Emits a {Transfer} event.
     */
    function _mint(address to, uint256 tokenId, string memory ceekUserId) internal virtual {
        require(to != address(0), "024");
        require(!_exists(tokenId), "025");
    ...
    function _transfer(
        ...
        require(ERC721.ownerOf(tokenId) == from, "026");
        require(to != address(0), "027");
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual override returns (uint256) {
        require(index < ERC721.balanceOf(owner), "029");
    ...
    function tokenByIndex(uint256 index) public view virtual override returns (uint256) {
        require(index < ERC721Enumerable.totalSupply(), "030");
...
abstract contract ERC721Burnable is Context, ERC721 {
    ...
    function burn(uint256 tokenId) public virtual {
        //solhint-disable-next-line max-line-length
        require(_isApprovedOrOwner(_msgSender(), tokenId), "033");
        _burn(tokenId);
    }
}
...
abstract contract AccessControl is Context, IAccessControl, ERC165 {
    ...
    function renounceRole(bytes32 role, address account) public virtual override {
        require(account == _msgSender(), "034");
...
contract ERC721Mochi is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
	...
	bytes32 public constant UNGROUP_ROLE = keccak256("UNGROUP_ROLE");
	string private _tokenBaseURI;
    Counters.Counter private _tokenIdCounter;
    bool public anyoneCanMint;
	address public contract_gate;
	
    // ungroup token: mapping tokenId to array of group members tokenUri
    mapping(uint256 => string[]) private _tokenURIsToUngroup;

	// ungroup token: mapping tokenId to array of group members coords
    mapping(uint256 => string[]) private _tokenCoordsToUngroup;
    ...
    constructor(address owner, string memory name, string memory symbol, bool _anyoneCanMint, address _contract_gate) ERC721(name, symbol) {
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
		_setupRole(UNGROUP_ROLE, owner);
        anyoneCanMint = _anyoneCanMint;
		contract_gate = _contract_gate;
    }
	
	function getTokenIdByCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public view returns (uint256) {
		if (coord_x > 0 && coord_y > 0) {
       		return _tokenCoords[string(abi.encodePacked(coord_x,'_',coord_y))];
		}
		
        return _tokenGroup[groupId];
	}
	
	function autoUngroupTokenURI(string memory _baseURI, string[] memory _tokenURIs, uint256[] memory _tokenCoordsX, uint256[] memory _tokenCoordsY, uint256 tokenId, string memory groupId) public onlyMinter {
		uint i;
		string[] memory tokenCoords;
		if (tokenId == 0) {
			tokenId = getTokenIdByCoordsOrGroup(groupId,0,0);
		}
		address owner = ERC721.ownerOf(tokenId);
		
		grantRole(UNGROUP_ROLE, owner);
		_tokenBaseURI = _baseURI;		
		_tokenURIsToUngroup[tokenId] = _tokenURIs;
		for (i=0; i<_tokenCoordsX.length; i++) {
			tokenCoords[i] = (string(abi.encodePacked(_tokenCoordsX[i],'_',_tokenCoordsY[i])));
		}
		_tokenCoordsToUngroup[tokenId] = tokenCoords;
	}

    function autoUngroup(uint256 tokenId, string memory groupId, string memory ceekUserId) public onlyUngroup {
		uint id;
		uint i;
		if (tokenId == 0) {
			tokenId = getTokenIdByCoordsOrGroup(groupId,0,0);
        ...
        address owner = ERC721.ownerOf(tokenId);
   		...
   		require((owner==msg.sender || contract_gate==msg.sender || (keccak256(abi.encodePacked((_ceekOwners[tokenId]))) != keccak256(abi.encodePacked((""))) && keccak256(abi.encodePacked((_ceekOwners[tokenId]))) == keccak256(abi.encodePacked((ceekUserId))))), "036");
		
		_burn(tokenId);
				
		for (i=0; i < _tokenURIsToUngroup[tokenId].length; i++) {
			do {
			  _tokenIdCounter.increment();
			  id = _tokenIdCounter.current();
			} while(_exists(id));
            _mint(owner, id, ceekUserId);

			_setTokenURI(id, string(abi.encodePacked(_tokenBaseURI, _tokenURIsToUngroup[tokenId][i])));
            if (i < _tokenCoordsToUngroup[tokenId].length) {
			    _tokenCoords[_tokenCoordsToUngroup[tokenId][i]] = id;
				_tokenCoordsKeys[id] = _tokenCoordsToUngroup[tokenId][i];
            }
		}
	}

    function autoMint(string memory _tokenURI, address to, uint256 coord_x, uint256 coord_y, string memory groupId, string memory ceekUserId) public onlyMinter {
        uint id;

		if (coord_x > 0 && coord_y > 0) {
		   	require(((_tokenCoords[string(abi.encodePacked(coord_x,'_',coord_y))]) == 0), "037");
		} else {
			require(((_tokenGroup[groupId]) == 0), "037");
		}
        ...
        _mint(to, id, ceekUserId);
        _setTokenURI(id, _tokenURI);
		_operatorApprovals[to][contract_gate] = true;
		
		if (coord_x > 0 && coord_y > 0) {
			_tokenCoords[string(abi.encodePacked(coord_x,'_',coord_y))] = id;
			_tokenCoordsKeys[id] = string(abi.encodePacked(coord_x,'_',coord_y));
		} else {
	...
	function autoMintCoordsOrGroup(string memory _baseURI, string[] memory _tokenURI, address to, uint256[] memory coord_x, uint256[] memory coord_y, string[] memory groupId, string memory ceekUserId) public onlyMinter {
        uint id;
		uint i;
		...
		for (i=0; i<_tokenURI.length; i++) {
			...
			if (i < groupId.length) {
           		require((((_tokenGroup[groupId[i]]) == 0)), "038");

				_tokenGroup[groupId[i]] = id;
				_tokenGroupKeys[id] = groupId[i];
			}
			if (i<coord_x.length && i<coord_y.length) {
           		require(((_tokenCoords[string(abi.encodePacked(coord_x[i],'_',coord_y[i]))]) == 0), "039");

				_tokenCoords[string(abi.encodePacked(coord_x[i],'_',coord_y[i]))] = id;
				_tokenCoordsKeys[id] = string(abi.encodePacked(coord_x[i],'_',coord_y[i]));
			}
		}
    }
	...
	function isUngroup(address account) public view returns (bool) {
		return hasRole(UNGROUP_ROLE, account);
	}
	...
	function tokenURICoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y)
        public
        view
        returns (string memory)
    {		
        return tokenURI(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
    ...
    /**
     * Open modifier to anyone can ungroup possibility
     */
    modifier onlyUngroup() {
        string memory mensaje;
        require(
            isUngroup(msg.sender),
            "041"
        );
        _;
    }	
...
contract CEEKLand is ERC721Mochi, ReentrancyGuard {
    ...
    address payable ceekDepositAdmin;
    ...
    constructor(
        EIP20 _payment_token, address _owner, address payable _admin, address _contract_gate, address payable _ceekDepositAdmin,
        uint256 _commissionRate, uint256 _royaltiesCommissionRate, string memory name, string memory symbol, bool _anyoneCanMint) 
        ERC721Mochi(_owner, name, symbol, _anyoneCanMint, _contract_gate) 
    {
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet, string memory ceekUserId) public {
        // onlyOwner
		require((ownerOf(tokenId)==msg.sender || contract_gate==msg.sender || (keccak256(abi.encodePacked((_ceekOwners[tokenId]))) != keccak256(abi.encodePacked((""))) && keccak256(abi.encodePacked((_ceekOwners[tokenId]))) == keccak256(abi.encodePacked((ceekUserId))))), "043");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "044");
    ...
    // Buy option
    function buy(uint256 tokenId, string memory ceekUserId) public nonReentrant {

        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "045");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "046");

        require((keccak256(abi.encodePacked((_ceekOwners[tokenId]))) == keccak256(abi.encodePacked((""))) && keccak256(abi.encodePacked((ceekUserId))) == keccak256(abi.encodePacked(("")))) || keccak256(abi.encodePacked((_ceekOwners[tokenId]))) != keccak256(abi.encodePacked((ceekUserId))), "047");

        require(sellBidPrice[tokenId] <= payment_token.balanceOf(msg.sender), "048");
        ...
        // to owner
        require(payment_token.transferFrom(msg.sender, _wallets[tokenId], amount4owner), "049");

        // to creator
        if (amount4creator>0) {
            require(payment_token.transferFrom(msg.sender, creators[tokenId], amount4creator), "049");
        }

        // to admin
        if (amount4admin>0) {
            require(payment_token.transferFrom(msg.sender, admin, amount4admin), "049");
        }
    ...
    function _mint(address to, uint256 tokenId, string memory ceekUserId) override internal {
        creators[tokenId] = msg.sender;
        super._mint(to, tokenId, ceekUserId);
    }
    ...
    function callOptionalReturn(IERC721 token, bytes memory data) private {
        ...
        // solhint-disable-next-line max-line-length
        require(address(token).isContract(), "050");
        ...
        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "052");
        }
    }

    // Update an auction data for a tokenId
    function updateAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice, bool _open, address payable _highestBidder, uint _highestBid, string memory _highestBidderCeekUserId) public {
        //require(contract_gate==msg.sender, "CEEKLand: only the gate can change the auctions data");
        require(getApproved(tokenId) == address(this) || getApproved(tokenId) == contract_gate, "CEEKLand: tokenId approve problem");

        auctions[tokenId].beneficiary = _beneficiary;
        auctions[tokenId].auctionEnd = _closingTime;
        auctions[tokenId].reserve = _reservePrice;
        auctions[tokenId].open = _open;
        auctions[tokenId].highestBidder = _highestBidder;
        auctions[tokenId].highestBid = _highestBid;
		auctions[tokenId].highestBidderCeekUserId = _highestBidderCeekUserId;
    }

    // Delete an auction for a tokenId
    function emitAuctionEvents(uint256 tokenId, bool emitRefund, bool emitHighestBidIncreased, bool emitAuctionFinalize, bool deleteAuction) public {
        require(contract_gate==msg.sender, "CEEKLand: only the gate can change the auctions data");
        require(getApproved(tokenId) == address(this) || getApproved(tokenId) == contract_gate, "CEEKLand: tokenId approve problem");

        if (emitRefund && auctions[tokenId].highestBid>0) {
            require(payment_token.transfer(auctions[tokenId].highestBidder, auctions[tokenId].highestBid), "CEEKLand: Transfer failed.");
            emit Refund(auctions[tokenId].highestBidder, auctions[tokenId].highestBid);
        }

        if (emitHighestBidIncreased) {
			require(payment_token.transferFrom(auctions[tokenId].highestBidder, address(this), auctions[tokenId].highestBid), "CEEKLand: Transfer failed.");
            emit HighestBidIncreased(auctions[tokenId].highestBidder, auctions[tokenId].highestBid, tokenId);
        }
		
		if (emitAuctionFinalize) {
		    require((auctions[tokenId].open && 
			...
			if (auctions[tokenId].highestBid>0) {
				...
				_ceekOwners[tokenId] = auctions[tokenId].highestBidderCeekUserId;
    ...
    function updateAdmin(address payable _admin, address _contract_gate, address payable _ceekDepositAdmin, uint256 _commissionRate, uint256 _royaltiesCommissionRate, bool _anyoneCanMint, EIP20 _payment_token) public {
        require(msg.sender==contract_owner, "053");
        admin = _admin;
        grantRole(MINTER_ROLE, admin);
		contract_gate = _contract_gate;
        ceekDepositAdmin = _ceekDepositAdmin;
