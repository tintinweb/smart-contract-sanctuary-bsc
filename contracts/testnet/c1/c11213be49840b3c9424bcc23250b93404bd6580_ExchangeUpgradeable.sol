...
contract ExchangeUpgradeable is BaseContract {
    ...
    function initialize(address multiSigAccount, address treasury) public virtual initializer {
        __BaseContract_init(multiSigAccount, treasury);
    }
    ...
    function createExchangeItem(
        ...
        returns (uint256 exchangeItemIndex)
    {
    ...
    function createExchangeSale(uint256 exchangeItemIndex, uint256 amount) external payable nonReentrant whenNotPaused {
        ExchangeItem memory exchangeItem = _exchangeItem[exchangeItemIndex];
        ...
        if (exchangeItem.paymentToken != address(0)) {
            ...
            if (feePercent != 0) treasury.transfer(treasuryReceivable);
        }
    ...
    ERC1155ReceiverUpgradeable
{
    ...
    modifier onlyWhitelistNFTContract(address nftContractAddress) {
        require(_whitelistNFTContractAddress[nftContractAddress], "Error: NFT contract not allowed");
        _;
    }
    ...
    function setWhitelistNFTContractAddress(
        ...
        require(
            _whitelistNFTContractAddress[nftContractAddress] != allowance || _isERC1155[nftContractAddress] != isERC1155
        );

        _whitelistNFTContractAddress[nftContractAddress] = allowance;
