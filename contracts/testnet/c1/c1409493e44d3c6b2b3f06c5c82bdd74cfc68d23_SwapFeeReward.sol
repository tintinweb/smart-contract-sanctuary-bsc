...
contract SwapFeeReward is Ownable{
    ...
    constructor(
        ...
        tallyToken = _tallyToken;
        oracle = _Oracle;
        targetToken = _targetToken;
    }

    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'TALLYSwapFactory: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'TALLYSwapFactory: ZERO_ADDRESS');
    }
    ...
    function getSwapFee(address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = uint(1000).sub(ITALLYPair(pairFor(tokenA, tokenB)).swapFee());
    }
    ...
    function withdraw(uint8 v, bytes32 r, bytes32 s) public returns(bool){
        ...
        if (balance > 0){
            tallyToken.transfer(msg.sender, balance);
    ...
    function getQuantity(address outputToken, uint256 outputAmount, address anchorToken) public view returns (uint256) {
        ...
        if (outputToken == anchorToken) {
        ...
        } else if (ITALLYFactory(factory).getPair(outputToken, anchorToken) != address(0) && checkPairExist(outputToken, anchorToken)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (ITALLYFactory(factory).getPair(outputToken, intermediate) != address(0) && ITALLYFactory(factory).getPair(intermediate, anchorToken) != address(0) && checkPairExist(intermediate, anchorToken)) {
    ...
    function setRewardToken(address _rewardToken) public onlyOwner {
        require(_rewardToken != address(0), "Reward: new Token is the zero address");
        tallyToken = ITallyToken(_rewardToken);
    }
...
interface ITallyToken is IERC20 {
