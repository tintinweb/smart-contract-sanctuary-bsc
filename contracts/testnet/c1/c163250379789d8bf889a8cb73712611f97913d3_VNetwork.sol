...
contract VNetwork is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function initialize() initializer public {
        ...
        buyLiquidityFee = 100;
        buyProjectFee = 500;
        buyTotalFees = buyTreasuryFee + buyLiquidityFee + buyProjectFee;

        sellTreasuryFee = 400;
        sellLiquidityFee = 100;
        sellProjectFee = 500;
        sellTotalFees = sellTreasuryFee + sellLiquidityFee + sellProjectFee;
    ...
    function updateBuyFees(uint256 _treasuryFee, uint256 _liquidityFee, uint256 _projectFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _treasuryFee, uint256 _liquidityFee, uint256 _projectFee) external onlyOwner {
        ...
        if(limitsInEffect){
            if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]){
                ...
                //when sell
                    ...
                    require(amount <= maxTransaction, "Sell transfer amount exceeds the max sell.");
                }
            }
        }
        ...
        if(takeFee){
            ...
            if(fees > 0){    
                ...
                if(tokensForLiquidity > 0){
                    super._transfer(address(this), liquidityAddress, tokensForLiquidity);
                }
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForTreasury + tokensForProject;
        ...
        uint256 ethForProject = ethBalance * tokensForProject / (totalTokensToSwap);
    ...
    function setProjectAddress(address _projectAddress) external onlyOwner {
        require(_projectAddress != address(0), "_treasuryAddress address cannot be 0");
