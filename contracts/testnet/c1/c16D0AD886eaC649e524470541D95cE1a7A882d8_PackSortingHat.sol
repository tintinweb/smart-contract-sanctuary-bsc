...
contract PackSortingHat is Context, IERC721Receiver, AccessControl {
    ...
    function createSlot(uint256 packId,uint8 times,uint256[] memory templates,uint256[] memory chances) external {
        ...
        require(_countSlot[packId] < pack.slot, "ERROR_OVER_SLOT_LIMIT");
    ...
    function fullFillRandomness(uint256 requestId,uint256[] memory randoms) public{
        ...
        uint256 tokenId = _requestId.requestId; //1
        uint256 packId = tokenPacks[tokenId];
        ...
        for (uint256 i = 0; i < pack.slot; i++){
            ...
            for (uint256 j = 0; j < slot.times; j++){
                ...
                for (uint256 k = 0; k < slot.chances.length; k ++) { 
                    ...
                    if (randoms[thisRandom] > chances[k]) {
                        continue;
                    } else {
                        rewardIndex = k;
                        ipfsPath.push(URIs[rewardIndex]);
                    }
                }
                
                //TODO mint to box owner

                IAsset(nftsAddress[rewardIndex]).mint(_requestId.ownerBox, URIs[rewardIndex]);
                thisRandom++;
            }
        }

        emit BuildAsset(ipfsPath);
    ...
    function getPackById(uint packId) external view returns (
        ...
        address owner,
        address tokenAddress,
        uint256[] memory boxTokenIds,
        address[] memory ownerBoxs
    ) {
        PackCollection memory pack = packCollection[packId];
        boxTokenIds = boxTokenId[packId];
        ownerBoxs = ownerBox[packId];
        uint256 thisPackid = packId;

        return (pack.packIpfsPath,pack.maxSupply,pack.slot,pack.price,thisPackid,pack.endDate,pack.owner,pack.tokenAddress,boxTokenIds,ownerBoxs);
    }
