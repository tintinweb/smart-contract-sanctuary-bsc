...
contract LaboManagement is ERC721Holder, Ownable{
    ...
    function cheatZaiMana(uint256 _zaiId, uint256 _manaAdded) external {
        require(_updateZai(_zaiId,_manaAdded,true));
    }
    ...
    function workInASpot(uint256 _zaiId, uint256 _laboId, uint256 _spotId) external canUseZai(_zaiId){
        address _zaiAddress = gameAddresses.getZaiAddress();
        require(
            ILaboratory(gameAddresses.getLaboratoryNFTAddress()).ownerOf(_laboId) !=
             gameAddresses.getClaimNFTsAddress(),
             "Lab not active");
        ...
        if(w.zaiId != 0){
            bool _manaMaxUpgrade = block.timestamp - w.beginingAt > 1 days;
            require(_updateZai(w.zaiId,_getManaWon(block.timestamp,w.beginingAt),_manaMaxUpgrade));
        }else{
    ...
    function stopWorking(uint256 _zaiId, uint256 _laboId, uint256 _spotId) external canUseZai(_zaiId){
        WorkInstance storage w = workingSpot[_laboId][_spotId];
    ...
    function _getManaWon(uint256 _finished,uint256 _start) internal pure returns(uint256 mana){
        uint256 _duration = _finished - _start;
        if(_duration <= 21600){ // less than 6 h
            mana = 0;
        }else if(_duration <= 43200){ // less than 12h
            mana = 500;
        }else if(_duration <= 86400){ // less than 24h
            mana = 1000;
        }else if(_duration <= 129600){ // less than 36h
            mana = 2000;
        }else{
            mana = 3000;
        }
    ...
    function _updateZai(uint256 _zaiId, uint256 _mana, bool _manaMaxUpgrade) internal returns(bool){
        IZaiNFT I = IZaiNFT(gameAddresses.getZaiAddress());
        I.updateStatus(_zaiId,0,0);
        if(_manaMaxUpgrade){
            return(I.updateMana(_zaiId,_mana,0,50)); 
        }else{
            return(I.updateMana(_zaiId,_mana,0,0)); 
        }
    }

    function buyStandardPotion(uint256 _quantity, uint256 _potionType) public {
        require(_potionType == 5 || _potionType == 6, "Doesn't exist");
        require(_quantity <= 5, "Only 5 potions max can be created by tx");
        address _paymentAddress = gameAddresses.getPaymentsAddress();
        uint256 _price = _quantity * potionsStandardPrice;
        
        require(payWithRewardOrWallet(msg.sender, _paymentAddress,_price));
        IPayments(_paymentAddress).distributeFees(_price);
        IPotions potion = IPotions(gameAddresses.getPotionAddress());

        for(uint256 i = 0 ; i < _quantity ; ){
            potion.buyPotion(msg.sender,_potionType);
            unchecked {++ i ;}
        }
    }
    ...
    function createAndSellPotion(
        ...
        _updateCredits(_laboId);
        require(_potionsCredits[_laboId] >= (_quantity * _power * pointCreditCost), "Not enough credits");

        _potionsCredits[_laboId] = _potionsCredits[_laboId] - (_quantity * _power * pointCreditCost);
        createdPotionsForLab[_laboId] = createdPotionsForLab[_laboId] + _quantity;
        IPotions potion = IPotions(gameAddresses.getPotionAddress());

        for(uint256 i = 0 ; i < _quantity ;){
            uint256 potionId = potion.mintPotionForSale(_laboId,_price,_type,_power);
    ...
    function buyPotion(uint256 _potionId) public {
        ...
        emit PotionSold(_seller, _price);
