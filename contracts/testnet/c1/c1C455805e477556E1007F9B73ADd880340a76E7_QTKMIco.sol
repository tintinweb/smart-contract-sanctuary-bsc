...
contract QTKMIco is Owner {
    ...
    struct icoOrder {
        ...
        uint256 tokenPerWei; // 1 wei 당 토큰 수. 3KM wei
    ...
    constructor(address _TKMToken, address _Operator) {
        ...
        Operator = _Operator;
        tge_ = 30; // ico진행 시 유저 지갑에 보낼 토큰 비율
        order = 1;
    }
    ...
    function setIcoOrder(
        ...
        if (0 == icoOrders[_order].totalAmount) {
            ...
            newOrder.tokenPerWei = _tokenPerWei;
        ...
        // 전체 수량 제외하고 수정 가능
        icoOrders[_order].tokenPerWei = _tokenPerWei;
...
*/
    ...
    function ico() public payable {
        ...
        // 구매 토큰
        uint256 validBuyToken = icoOrders[order].tokenPerWei * msg.value;
...
*/
    ...
    function icoClaim(uint8 claimOrder) public {
        ...
        // 이미 지급했는지 확인
        require(false == lockedTokenTransfer[msg.sender][claimOrder], "TKMIco: icoCalim already claimed");
        // 지급 설정
        lockedTokenTransfer[msg.sender][claimOrder] = true;
        // 날짜 체크
        // uint256 releaseTime = tokenClaimTime + ((releaseOrderDay * (claimOrder - 1)) * 1 days);
        uint256 releaseTime = tokenClaimTime + ((releaseOrderDay * (claimOrder - 1)) * 1 minutes);
