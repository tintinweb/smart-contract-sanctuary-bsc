...
contract Stake is Ownable {
    ...
    uint256[] private levelPer = [10,1,1,1,1,1,1,1,1,2];


    uint256 private stakingId = 0;
    uint256 private topUpId = 0;
    uint256 private USDTPrice = 1 ether; // 1 USDT
    uint256 private tokenPrice = 200000000000000000; // 0.2 PCH
    ...
    event Harvested(address indexed staker, uint256 stakeId, uint256 poolId, uint256 amount, uint256 time);
    ...
    event Referral(address indexed staker, address indexed referral, uint256 referralReward, uint256 time);
    ...
    function usdtToPCH(uint256 _amount) public view returns(uint256) {
        return (_amount * tokenPrice) / 10 ** 18;
    }

    function pchToUSDT(uint256 _amount) public view returns(uint256) {
        return (_amount * 10 ** 18) / tokenPrice;
    }

    function changeLevelPer(uint256[] memory _per) public onlyOwner returns (bool){
        levelPer = _per;
        return true;
    }

    function changeTokenPrice(uint256 _price) public onlyOwner returns (bool){
        tokenPrice = _price;
        return true;
    }

    function changeUSDTPrice(uint256 _price) public onlyOwner returns (bool){
        USDTPrice = _price;
        return true;
    }
    ...
    function stake(uint256 _poolId, uint256 _amount, address _referral) public payable returns(bool) {
        _stake(_poolId, usdtToPCH(_amount), _referral, msg.sender);
        return true;
    }
    ...
    function _stake(uint256 _poolId, uint256 _amount, address _referral, address _staker) internal {
        ...
        if(address(0) != userDetails[_staker].referral){
            userDetails[_referral].totalReferralReward += _amount * pool[_poolId].referralPer / 10000;
            ...
            emit Referral(msg.sender, userDetails[_staker].referral , _amount * pool[_poolId].referralPer / 10000, block.timestamp);
        }
    ...
    function _harvest(address _staker, uint256 _stakingId) internal  {
        ...
        emit Harvested(_staker, _stakingId, stakeDetails[_stakingId].poolId, harvestAmount, block.timestamp);
    }
