...
contract TestToken is IERC20, Auth {
    ...
    function excludeLockorPresale(address add) external onlyOwner {
        // Exclude from fees
    ...
    function takeFee(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if (to == pair && totalSellFee > 0) {
            fees = amount.mul(totalSellFee).div(taxDivisor);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Update balances
        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
        // We check the sender/receiver to not charge fee for inter-wallet transfers
