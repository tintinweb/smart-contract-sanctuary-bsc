...
contract RebasePair is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
        ...
        )
    {
        ...
        if (dif0 > dif1 && profitUSD > rebaseFeeUSD) {
            ...
            amountInMax = _getAmountInMax(amountOut, reserve0, reserve1);
            path[0] = _token1;
            path[1] = _token0;
        ...
        if (dif1 > dif0 && profitUSD > rebaseFeeUSD) {
            ...
            amountInMax = _getAmountInMax(amountOut, reserve1, reserve0);
            path[0] = _token0;
            path[1] = _token1;
            // netProfitUSD = profitUSD - rebaseFeeUSD;
        }
    ...
    function _getAmountInMax(
        uint256 amountOut,
        uint256 reserveIn,
        uint256 reserveOut
    ) internal pure returns (uint256) {
        return (reserveIn.mul(amountOut).mul(1000) / reserveOut.sub(amountOut).mul(997)).add(1);
    }
