...
contract StakingPool is IStakingPool, Ownable {
	...
	mapping(address => uint256) public validLPAmount; //only pool 1234 can claim dividend
	...
	function stakeTo(
		...
		if (user.amount == 0) user.lastClaimTime = block.timestamp;

		if (STAKE_TYPE == StakeType.LP && poolId > 0) {
			if (validLPAmount[to] == 0) lastDividendClaimTime[to] = block.timestamp;
			validLPAmount[to] += amount;
		}
		user.amount += amount;
	...
	function withdraw(uint256 poolId, uint256 amount) external needPoolOpen {
		require(poolId < pools.length, "invalid id");
		...
		if (STAKE_TYPE == StakeType.LP && poolId > 0) validLPAmount[msg.sender] -= amount;
	...
	function claim(uint256 poolId) public needPoolOpen {
		Pool storage pool = pools[poolId];
		ERC20Staker storage user = stakers[poolId][msg.sender];
		if (user.amount == 0) return;

		bool claimed = false;
		(uint256 pendingVNS, uint256 pedingCPD, bool includDividendVns) = pending(poolId, msg.sender);
		...
		if (pendingVNS > 0) {
			...
			if (includDividendVns) lastDividendClaimTime[msg.sender] = block.timestamp;
		}
		...
		if (claimed) user.lastClaimTime = block.timestamp;
	}
	...
	function pending(uint256 poolId, address _user)
		...
		returns (
			uint256 pendingVNS,
			uint256 pendingCPD,
			bool includDividendVns
		)
	{
		Pool memory pool = pools[poolId];
		ERC20Staker memory user = stakers[poolId][_user];
		if (STAKE_TYPE == StakeType.Single) {
			//vns
			pendingVNS = (user.amount * pool.rewardRate * (block.timestamp - user.lastClaimTime)) / 1000 / 1 days;
		} else {
			//vns
			if (user.amount > 0) {
				uint256 vnsBalanceOfLP = IERC20(VNS).balanceOf(STAKED_TOKEN);
				uint256 vnsamount = (user.amount * vnsBalanceOfLP) / IERC20(STAKED_TOKEN).totalSupply();
				pendingVNS = (vnsamount * pool.rewardRate * (block.timestamp - user.lastClaimTime)) / 1000 / 1 days;
				//cpd
				pendingCPD = pendingVNS;
			}

			//dividend vns
			if (block.timestamp > lastDividendClaimTime[_user] + dividendClaimColdTime && validLPAmount[_user] > 0) {
				pendingVNS += (validLPAmount[_user] * curDividendBalance) / totalStaked;
				includDividendVns = true;
			}
		}
	}
