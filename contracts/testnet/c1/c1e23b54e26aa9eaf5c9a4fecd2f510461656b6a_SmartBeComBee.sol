...
contract SmartBeComBee {
    ...
    struct B6 {
        ...
        uint[] typeFirstLevelReferrals;
        uint[] typeSecondLevelReferrals;
        uint reinvestCount;
        bool blocked;
    ...
    struct User {
        ...
        mapping(uint => mapping(uint => B3)) b3MatrixReinvest;
        mapping(uint => mapping(uint => B6)) b6MatrixReinvest;
    }
    ...
    function registration(address referrerAddress, address tknAddress, bool _atf) external payable {
        registrationbase(msg.sender, referrerAddress, tknAddress, _atf);
    }
    
    function registrationbase(address userAddress, address referrerAddress, address tknAddress, bool _atf) private {
    ...
    function buyLevel(uint8 matrix, uint8 level, address tknAddress, bool _atf) external payable {
        ...
        if (tokenAddress != nullAddress){
            ...
            if (atf){
                uint256 amount = 2*levelCost[level]*1e13;            
                uint256 allowance = _token.allowance(msg.sender, address(this));
                require(allowance >= amount, "Check the token allowance");
        ...
        if (matrix == 1) {
            ...
            users[msg.sender].b3Matrix[level].parentReferrer = freeB3Refer;
    ...
    function updB3Refer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[userAddress].b3Matrix[level].reinvestCount > 0)
            users[referrerAddress].b3Matrix[level].typeReferrals.push(3);
        else
            users[referrerAddress].b3Matrix[level].typeReferrals.push(1);


        if (false && users[referrerAddress].b3Matrix[level].referrals.length < 3) {
        ...
        //block matrix B3
         uint inv = users[referrerAddress].b3Matrix[level].reinvestCount;
         users[referrerAddress].b3MatrixReinvest[level][inv] = users[referrerAddress].b3Matrix[level];
        // users[referrerAddress].b3Matrix[level].reinvestTypeReferrals[inv] = users[referrerAddress].b3Matrix[level].typeReferrals;

        users[referrerAddress].b3Matrix[level].referrals = new address[](0);
        if (!users[referrerAddress].activeB3Levels[level+1] && level != LAST_LEVEL && users[referrerAddress].b3Matrix[level].reinvestCount == 1) {
        ...
        if (referrerAddress != owner) {
            ...
            if (users[referrerAddress].b3Matrix[level].currentReferrer != freeReferrerAddress) {
                ...
                users[referrerAddress].b3Matrix[level].parentReferrer = freeReferrerAddress;
            }
    ...
    function updB6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (false && users[referrerAddress].b6Matrix[level].firstLevelReferrals.length < 2) {
            
            //set user line1
            users[referrerAddress].b6Matrix[level].firstLevelReferrals.push(userAddress);
            users[referrerAddress].b6Matrix[level].typeFirstLevelReferrals.push(type_);
            ...
            users[userAddress].b6Matrix[level].parentReferrer = referrerAddress;
            ...
            users[ref].b6Matrix[level].typeSecondLevelReferrals.push(type_); 
        ...
        users[referrerAddress].b6Matrix[level].typeSecondLevelReferrals.push(type_);
    ...
    function updB6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        if (!x2) {
            ...
            users[users[referrerAddress].b6Matrix[level].firstLevelReferrals[0]].b6Matrix[level].typeFirstLevelReferrals.push(2);
            ...
            users[users[referrerAddress].b6Matrix[level].firstLevelReferrals[1]].b6Matrix[level].typeFirstLevelReferrals.push(2);
    ...
    function updB6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        //pay
        if (false && users[referrerAddress].b6Matrix[level].secondLevelReferrals.length < 4) {
            return sendETHDividends(referrerAddress, userAddress, 2, level);
        }
        ...
        if (b6.length == 2) {
         ...
         uint inv = users[referrerAddress].b6Matrix[level].reinvestCount;
         users[referrerAddress].b6MatrixReinvest[level][inv] = users[referrerAddress].b6Matrix[level];
         //users[referrerAddress].b6Matrix[level].reinvestFirstReferrals[inv] = users[referrerAddress].b6Matrix[level].firstLevelReferrals;
         //users[referrerAddress].b6Matrix[level].reinvestSecondReferrals[inv] = users[referrerAddress].b6Matrix[level].secondLevelReferrals;
         // users[referrerAddress].b6Matrix[level].reinvestTypeFirstReferrals[inv] = users[referrerAddress].b6Matrix[level].typeFirstLevelReferrals;
         // users[referrerAddress].b6Matrix[level].reinvestTypeSecondReferrals[inv] = users[referrerAddress].b6Matrix[level].typeSecondLevelReferrals;
    ...
    function usersB3Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, bool, uint[] memory) {
        return (users[userAddress].b3Matrix[level].currentReferrer,
                ...
                users[userAddress].b3Matrix[level].typeReferrals);
    }
    function usersB3Matrix2(address userAddress, uint8 level) public view returns(address, address[] memory, bool, uint, uint8, uint[] memory) {
        return (users[userAddress].b3Matrix[level].currentReferrer,
                users[userAddress].b3Matrix[level].referrals,
                users[userAddress].b3Matrix[level].blocked, 
    ...
    function usersB3MatrixReinvest2(address userAddress, uint8 level, uint reinvest) public view returns(address[] memory, uint, uint8) {
        return (users[userAddress].b3MatrixReinvest[level][reinvest].referrals,
                reinvest, 
                level);
    }

    function usersB6Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, address[] memory, bool, uint[] memory, uint[] memory) {
        return (users[userAddress].b6Matrix[level].currentReferrer,
                ...
                users[userAddress].b6Matrix[level].typeFirstLevelReferrals,
                users[userAddress].b6Matrix[level].typeSecondLevelReferrals);
    }
    function usersB6Matrix2(address userAddress, uint8 level) public view returns(address, address[] memory, address[] memory, uint, uint[] memory, uint[] memory, uint8) {
        return (users[userAddress].b6Matrix[level].currentReferrer,
                users[userAddress].b6Matrix[level].firstLevelReferrals,
                users[userAddress].b6Matrix[level].secondLevelReferrals,
    ...
    function usersB6Matrix21(address userAddress, uint8 level) public view returns(bool, uint8) {
        return (users[userAddress].b6Matrix[level].blocked,
                level);
    }
    function usersB6MatrixReinvest2(address userAddress, uint8 level, uint reinvest) public view returns(address[] memory, address[] memory, uint, uint8) {
        return (users[userAddress].b6MatrixReinvest[level][reinvest].firstLevelReferrals,
                users[userAddress].b6MatrixReinvest[level][reinvest].secondLevelReferrals,
                reinvest,
                level);
    }
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (tokenAddress != nullAddress){
            uint256 amount = levelCost[level]*1e13;   
    ...
    function allowtoken(address addr1, address addr2) public returns (uint256) {
        _token = IERC20(addr2);          
