...
contract Catcoin is Context, IERC20, Ownable {
    ...
    string private constant _name = "Catcoin World";
    string private constant _symbol = "CCW";
    ...
    address payable private _devwallet = payable(address(0x933F5A81f4F08537d09Ad6bCd911948e21022E6f));
    address payable private withdrawcontract;
    address public multisig;
    address payable private _charitywallet = payable(address(0x7D9657A9D796Bc53F8da3A1da2E81333FF27E680));        // require wallet address
    address payable private _operationwallet = payable(address(0x50f4ebdefdae5FD69499Ef245a5873F65BB1eCC3));      // require wallet address
    ...
    constructor(address _uniswaprouter, address _multisig, address devwallet, address charitywallet, address operationwallet) {
        ...
        _operationwallet = payable(operationwallet);
        ...
        _isExcludedFromFee[_charitywallet] = true;        ///  new item
        _isExcludedFromFee[_operationwallet] = true;      ///  new item
    ...
    function rBalanceOf(address account) public view returns (uint256) {        /// new item
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns (uint256)
    {
        ...
        (,uint256 tFee, uint256 tLiquidity, uint256 tWallet, uint256 tBurn) = _getTValues(tAmount);
    ...
    function updateDevWallet(address payable newAddress) external onlyMultisig{
    ...
    function updateCharityWallet(address payable newAddress) external onlyMultisig{
        require(msg.sender == _charitywallet);
        require(newAddress != address(0), "newAddress can not be zero address.");
    ...
    function updateOperatuibWallet(address payable newAddress) external onlyMultisig{
        require(msg.sender == _operationwallet);
        require(newAddress != address(0), "newAddress can not be zero address.");
            _operationwallet = newAddress;
            _isExcludedFromFee[newAddress] = true;
        emit DevWalletUpdate(newAddress);
    }
    ...
    function setFees(
        BuyFee memory buy_Fee,
        SellFee memory sell_Fee
        
    ) external onlyMultisig {
        require(buy_Fee.reflection + buy_Fee.marketing + buy_Fee.charity + buy_Fee.liquidity + buy_Fee.operation + buy_Fee.burn + buy_Fee.dev <= 10);
        require(sell_Fee.reflection + sell_Fee.marketing + sell_Fee.charity + sell_Fee.liquidity + sell_Fee.operation + sell_Fee.burn + sell_Fee.dev <= 10);
        buyFee = buy_Fee;
        sellFee = sell_Fee;
        emit SetAllFees(
            buy_Fee ,
        ...
        )
    {
        ...
        uint256 tWallet = calculateMarketingFee(tAmount) +
            calculateDevFee(tAmount) + calculateCharityFee(tAmount) + calculateOperationFee(tAmount);
        uint256 tBurn = calculateBurnFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
        tTransferAmount = tTransferAmount.sub(tWallet).sub(tBurn);
    ...
    function _takeBurnFee(uint256 tBurn) private {
        emit Burn(address(this), tBurn , address(0x0));
    }

    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_reflectionFee).div(10**2).div(10);      // corrent percent = percent / 10
    }

    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee).div(10**2).div(10);       // corrent percent = percent / 10
    }

    function calculateMarketingFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_marketingFee).div(10**2).div(10);       // corrent percent = percent / 10
    }

    function calculateDevFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_devFee).div(10**2).div(10);     // corrent percent = percent / 10
    }

    function calculateCharityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_charityFee).div(10**2).div(10);     // corrent percent = percent / 10
    }

    function calculateOperationFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_operationFee).div(10**2).div(10);     // corrent percent = percent / 10
    }

    function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_burnFee).div(10**2).div(10);        // corrent percent = percent / 10
    }
    ...
    function setBuy() private {
        ...
        _operationFee = buyFee.operation;
    ...
    function setSell() private {
        ...
        _operationFee = sellFee.operation;
    ...
    function openTrading(address payable _withdrawcontract) external onlyOwner {
        ...
        buyFee.reflection = 20;         // 10x original value
        buyFee.marketing = 10;
        buyFee.charity = 10;
        buyFee.liquidity = 10;
        buyFee.operation = 0;
        buyFee.burn = 5;
        buyFee.dev = 5;
        sellFee.reflection = 20;
        sellFee.marketing = 20;
        sellFee.charity = 10;
        sellFee.liquidity = 10;
        sellFee.operation = 10;
        sellFee.burn = 10;
        sellFee.dev = 0;
        tradingOpen = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (contractTokenBalance > 0 &&             // call swapAndLiquify in case of general transfer
            !inSwapAndLiquify &&
            from != uniswapV2Pair &&
            // to != uniswapV2Pair &&
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        uint256 charityAmt = unitBalance *
            2 *
            (buyFee.charity + sellFee.charity);
        uint256 operationAmt = unitBalance *
            2 *
            (buyFee.operation + sellFee.operation);
        ...
        if (marketingAmt > 0) {
            ...
            // withdrawcontract.call{value: marketingAmt}("");
        }

        if (charityAmt > 0) {
            payable(_charitywallet).transfer(charityAmt);
            // _charitywallet.call{value: charityAmt}("");
        }

        if (operationAmt > 0) {
            payable(_operationwallet).transfer(operationAmt);
            // _operationwallet.call{value: operationAmt}("");
        }

        if (devAmt > 0) {
            payable(_devwallet).transfer(devAmt);
            // _devwallet.call{value: devAmt}("");
        }
