...
contract OPVMarketplace is ReentrancyGuard {
    ...
    modifier checkOwner(uint256 _idOnMarket, address sender) {
        address seller = idToMarketItem[_idOnMarket].seller;
        address factory = idToMarketItem[_idOnMarket].nftContract;
        uint256 tokenId = idToMarketItem[_idOnMarket].tokenId;

        require(
            OPVFactory(factory).ownerOf(tokenId) == sender,
            "Is Not Owner Of NFT"
        );
        _;
    }

    modifier checkOwnerItemOnMarket(uint256 _idOnMarket, address sender) {
        address seller = idToMarketItem[_idOnMarket].seller;
        address factory = idToMarketItem[_idOnMarket].nftContract;
        uint256 tokenId = idToMarketItem[_idOnMarket].tokenId;

        require(
            OPVFactory(factory).ownerOf(tokenId) == address(this),
            "Is Not Owner Of NFT"
        );
        _;

        require(seller == sender, "Is Not Owner Of NFT");
        _;
    }
    ...
    ) public isFactory(_factoryAddress) nonReentrant {
        _itemIds.increment();
    ...
    function Buy(uint256 _idOnMarket) public nonReentrant {
        ...
        address factory = idToMarketItem[_idOnMarket].nftContract;
        address contractHold = idToMarketItem[_idOnMarket].contractHold;
        address seller = idToMarketItem[_idOnMarket].seller;
        ...
        if (AllFactoryVip[factory] == true) {
            if (blackListFee[seller] == true) {
                if (
                    RefContract.getRef(seller) != address(0) &&
                    OPVFactory(factory).creatorOf(tokenId) != address(0)
                ) {
                    //Have ref & creator
                    //Money to ref
                    MainToken.transferFrom(
                        msg.sender,
                        RefContract.getRef(seller),
                        feeRefItem
                    );
                    // Money to fund creator
                    MainToken.transferFrom(
                        msg.sender,
                        OPVFactory(factory).creatorOf(tokenId),
                        feeCreatorItem
                    );
                    // Money to seller
                    MainToken.transferFrom(
                        msg.sender,
                        seller,
                        price - feeCreatorItem - feeRefItem
                    );
                } else if (
                    RefContract.getRef(seller) != address(0) &&
                    OPVFactory(factory).creatorOf(tokenId) == address(0)
                ...
                if (
                    ...
                    MainToken.transferFrom(
                        msg.sender,
                        seller,
                        price - feeCreatorItem - feeRefItem - feeMarketItem
                    );
                } else if (
                    RefContract.getRef(seller) != address(0) &&
                    OPVFactory(factory).creatorOf(tokenId) == address(0)
            ...
            if (blackListFee[seller] == true) {
                if (RefContract.getRef(seller) != address(0)) {
                    //Money to ref
                    MainToken.transferFrom(
                        msg.sender,
                        RefContract.getRef(seller),
                ...
                if (RefContract.getRef(seller) != address(0)) {
                    ...
                    // Money to fund creator + market
                    MainToken.transferFrom(
                        msg.sender,
                        feeAddress,
                        feeCreatorItem + feeMarketItem
                    );
                    // Money to seller
                    MainToken.transferFrom(
                        msg.sender,
                    ...
                    MainToken.transferFrom(
                        msg.sender,
                        feeAddress,
                        feeCreatorItem + feeRefItem + feeMarketItem
                    );
                    // Money to seller
                    MainToken.transferFrom(
                        msg.sender,
