...
contract BoostActions is ReentrancyGuard {
  ...
  function liquifyAndStake(address token, uint256 tokenAmount) external payable nonReentrant {
    // Approve tokens
    ...
    // add the liquidity
    (, , uint256 liquidity) = _router.addLiquidityETH{value: 100000000000}(
      token,
      tokenAmount,
    ...
    // Staking the lp's token to masterChef
...
contract MasterChef is Ownable {
  ...
  constructor(SushiToken _sushi, address _devaddr, uint256 _sushiPerBlock, uint256 _startBlock, uint256 _bonusEndBlock) public {
    sushi = _sushi;
    devaddr = _devaddr;
