...
contract Bebit is IBebit, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "Bebit: Already upgraded");
        _;
        _upgraded = true;
    }
    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "Bebit: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __Bebit_init_unchained(_router);
    }
    function __Bebit_init_unchained(address _router) internal initializer {
        _name = "Bebit";
        _symbol = "$Bebit";
        _decimals = 18;
        _tTotal = 2000000000000 * 10**18;
        _rTotal = (MAX - (MAX % _tTotal));
        _maxFee = 1000;
        // swapAndLiquifyEnabled = true;
        _maxTxAmount = 5000 * 10**6 * 10**18;
        numTokensSellToAddToLiquidity = 500000000000 * 10**18;
    ...
    function __Bebit_tiers_init() internal initializer {
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(_account, "Bebit: Selected account is in blacklist")
    {
        require(_account != address(0), "Bebit: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner {
        require(_account != address(0), "Bebit: Invalid address");
        require(_accountsTier[_account] > 0, "Bebit: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "Bebit: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "Bebit: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner checkTierIndex(_tierIndex) {
        require(_owner != address(0), "Bebit: Address Zero is not allowed");
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "Bebit: Owner address is blacklisted")
        preventBlacklisted(spender, "Bebit: Spender address is blacklisted")
    {
    ...
    function _transfer(
        ...
        private
        preventBlacklisted(_msgSender(), "Bebit: Address is blacklisted")
        preventBlacklisted(from, "Bebit: From address is blacklisted")
        preventBlacklisted(to, "Bebit: To address is blacklisted")
        isRouter(_msgSender())
    {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        uint256 initialBalance = IBebit(address(this)).balanceOf(msg.sender);
        // swap BNB for Tokens
        swapBnbForTokens(half);

        // how much BNB did we just swap into?
        uint256 newBalance = IBebit(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        IBebit(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
    ...
    function migrate(address account, uint256 amount)
        external
        override
        preventBlacklisted(account, "Bebit: Migrated account is blacklisted")
    {
        require(migration != address(0), "Bebit: Migration is not started");
        require(_msgSender() == migration, "Bebit: Not Allowed");
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        IBebit(_token).transfer(msg.sender, _amount);
    }
