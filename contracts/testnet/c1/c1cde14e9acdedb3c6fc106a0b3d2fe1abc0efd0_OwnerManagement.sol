...
contract OwnerManagement is Ownable{
    ...
    function addInstitution() internal {
        ...
        instInfo[_institution].expiredTime = IDocumentStoreInterface(_institution).getExpiredTime();
        nameList.push(instInfo[_institution].name);
    }

    function whitelist(address _address) public onlyOwner{
        require(addressWhitelisted[_address] == false,"Error: address has been whitelisted");
    ...
    function setName(address _contract, string memory _name) external {
        require(msg.sender == _contract, "Error: caller is not the institution");
    ...
    function setExpiredTime(address _contract, uint256 _time) external expiredTimeTrue(_contract, _time) {
        require(msg.sender == _contract, "Error: caller is not the institution");
        instInfo[_contract].expiredTime = _time;
    }

    function approveExpiredTime(address _contract, uint256 _time) external onlyOwner {
        require(_time > instInfo[_contract].expiredTime, "Error: expired time has passed");
        expiredTimeCheck[_contract][_time] = true;
    }

    modifier expiredTimeTrue(address _contract, uint256 _time) {
        require(expiredTimeCheck[_contract][_time], "Error: new expired time is not registered");
        _;
    }

    modifier onlyWhitelisted() {
        require(addressWhitelisted[msg.sender], "Error: address is not whitelisted");
        _;
    }   
}
...
contract DocumentStore is OwnableUpgradeable {
    ...
    string private legalReference;
    string private intentDeclaration;
    string private host;
    uint256 private contractExpiredTime;
