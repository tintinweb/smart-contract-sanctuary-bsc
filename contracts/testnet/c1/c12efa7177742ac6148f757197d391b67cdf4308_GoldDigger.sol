...
/** Tokennomics
  * Gold Digger ($GOLD) boasts incredibly powerful and well thought out tokenomics. 
  * The token is utilising a reflections model, as the tokenomics create a “neater” 
  * looking chart, maintaining a higher level of buy pressure. By implementing an 
  * Automated Buyback feature, the $GOLD chart will be geared toward increasing 
  * the overall buy pressure, and maximising the prospects for the long term vision 
  * Of the Gold Digger ecosystem.
 */

/** Auto-Buyback
  * The accrued taxes for Buyback & Burn will be triggered to automatically occur 
  * following a sell, reducing negative price movement, and maintaining a more consistent 
  * buy pressure. This means buying a portion of the $GOLD tokens, and then sending them 
  * to a “dead” address, essentially reducing the overall supply, and increasing the value 
  * of the remaining tokens in rotation.
 */

/** Dynamic LP Tax
  * If the liquidity is higher than 25% of the overall Market Cap, this tax will be redirected 
  * into the Buyback & Burn Wallet. This will help support a healthy level of volatility, and 
  * accelerate the deflation of the total $GOLD in circulation.
 */
...
contract ERC20 is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x8242Dc797c189660f9ACCc8ac92c0BCcBa030A92); // Marketing Address
    address payable public devAddress = payable(0x0680277548c4680B59E520a59ab746105A18aF05); // Dev Address
    ...
    uint256 public _maxTxAmount = 780 * 10**9 * 10**9;
    uint256 private minimumTokensBeforeSwap = 78 * 10**9 * 10**9; 
    uint256 private buyBackUpperLimit = 1 * 10**9 * 10**9;
    uint256 private minimumBeforeBuyBack = 1 * 10**9 * 10**9;

    uint256 public percentToBuyBack = 3; // percent of fund to be used to buy back each time
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            ...
            if (buyBackEnabled && balance > minimumBeforeBuyBack) {
                ...
                buyBackTokens(balance.div(300).mul(percentToBuyBack));
            }
        }
...
contract GoldDigger is ERC1363, TokenRecover {
    constructor ()
        ERC1363("Gold Digger", "GOLD")
    {
    }
