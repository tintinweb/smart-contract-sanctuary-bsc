...
contract LuckyDraw is Ownable {
    ...
    function setTop10Winner(uint _winMEXIAmount) internal {
        users[msg.sender].totalWin += _winMEXIAmount;
    ...
    function setTop10Player(uint _betAmount) internal {
        users[msg.sender].totalSpin += _betAmount;
    ...
    function _handleSpinFee(uint _pid) internal {
        Pool memory _p = pools[_pid];
        uint burnAmount = _p.burnPercent * _p.price / percentDecimal;
        if(_pid < 3) require(IERC20(_p.spentToken).transferFrom(_msgSender(), burnAddress, burnAmount));
        else {
            require(IERC20(_p.spentToken).transferFrom(_msgSender(), address(this), burnAmount));
            jackpotPrize += burnAmount;
        }
        require(IERC20(_p.spentToken).transferFrom(_msgSender(), treasuryAddress, _p.price - burnAmount));
        setTop10Player(_p.price);
    }
    ...
    function _spinCommon(uint pickRandom) internal {
        ...
        uint winMEXIAmount;
        if(pickRandom >= 20 && pickRandom < 120) winMEXIAmount = 50_000 ether;
        if(pickRandom >= 120 && pickRandom < 820) winMEXIAmount = 25_000 ether;
        if(pickRandom >= 820 && pickRandom < 3017) winMEXIAmount = 10_000 ether;
        if(pickRandom >= 3017 && pickRandom < 4017) winMEXIAmount = 1500 ether;
        if(pickRandom >= 4017 && pickRandom < 6017) winMEXIAmount = 1000 ether;
        if(pickRandom >= 6017 && pickRandom < 10000) winMEXIAmount = 500 ether;
    ...
    function _spinEpic(uint pickRandom) internal {
        ...
        uint winMEXIAmount;
        if(pickRandom >= 3 && pickRandom < 103) winMEXIAmount = 100_000 ether;
        if(pickRandom >= 103 && pickRandom < 803) winMEXIAmount = 50_000 ether;
        if(pickRandom >= 803 && pickRandom < 3000) winMEXIAmount = 20_000 ether;
        if(pickRandom >= 3000 && pickRandom < 4000) winMEXIAmount = 3000 ether;
        if(pickRandom >= 4000 && pickRandom < 6000) winMEXIAmount = 2000 ether;
        if(pickRandom >= 6000 && pickRandom < 10000) winMEXIAmount = 1000 ether;
        _handleTop10Winner(winMEXIAmount);
    }
    function _spinLegendary(uint pickRandom) internal {
        ...
        uint winMEXIAmount;
        if(pickRandom >= 1 && pickRandom < 101) winMEXIAmount = 200_000 ether;
        if(pickRandom >= 101 && pickRandom < 801) winMEXIAmount = 100_000 ether;
        if(pickRandom >= 801 && pickRandom < 3000) winMEXIAmount = 40_000 ether;
        if(pickRandom >= 3000 && pickRandom < 4000) winMEXIAmount = 6000 ether;
        if(pickRandom >= 4000 && pickRandom < 6000) winMEXIAmount = 4000 ether;
        if(pickRandom >= 6000 && pickRandom < 10000) winMEXIAmount = 2000 ether;
        _handleTop10Winner(winMEXIAmount);
    }
    function _spinJackpot(uint pickRandom) internal {
        if(pickRandom <= 10) _takeJackpot();
        uint winMEXIAmount;
        if(pickRandom >= 1 && pickRandom < 101) winMEXIAmount = 20_000 ether;
        if(pickRandom >= 101 && pickRandom < 801) winMEXIAmount = 10_000 ether;
        if(pickRandom >= 801 && pickRandom < 3000) winMEXIAmount = 4000 ether;
        if(pickRandom >= 3000 && pickRandom < 4000) winMEXIAmount = 600 ether;
        if(pickRandom >= 4000 && pickRandom < 6000) winMEXIAmount = 400 ether;
        if(pickRandom >= 6000 && pickRandom < 10000) winMEXIAmount = 200 ether;
        _handleTop10Winner(winMEXIAmount);
    }
    function _takeGuaranteedSpin(uint _pid) internal {
        string memory hash;
        if(_pid == 0) hash = commonHash;
        if(_pid == 1) hash = epicHash;
        if(_pid == 2) hash = legendHash;
        uint _tokenId = box.currentTokenId() + 1;
    ...
    function _spin(uint _pid, uint nonce) internal {
        ...
        if(_pid < 3) {
            ...
            if(userTotalSpined[_msgSender()] == pools[_pid].guaranteedSpin) _takeGuaranteedSpin(_pid);
        }
    }
    ...
    function addPool(uint _dailySpin, uint _price, uint _burnPercent, address _spentToken, uint _guaranteedSpin) external onlyOwner {
        require(_price > 0, "LuckyDraw: _price invalid");
        require(_spentToken != address(0), "LuckyDraw: _spentToken invalid");
        pools.push(Pool(_dailySpin, _price, _burnPercent, true, _spentToken, _guaranteedSpin));
    }
