...
contract TokenFactoryTestV1 is TokenFactoryBase {
    using Address for address payable;
    using SafeMath for uint256;
    ...
    constructor(address factoryManager_) TokenFactoryBase(factoryManager_) {}
    ...
    function create(
        ...
        uint8 decimals, 
        uint256 totalSupply
    ) external payable enoughFee nonReentrant returns (address tokenAddress) {
        refundExcessiveFee();
        payable(feeTo).sendValue(flatFee);
        StandardToken tokenContract = new StandardToken();
        tokenAddress = address(tokenContract);
        StandardToken(tokenAddress).initialize(msg.sender, name, symbol, decimals, totalSupply);
        tokens.push(StandardToken(tokenAddress));
        assignTokenToOwner(msg.sender, tokenAddress, 0);
        emit TokenCreated(msg.sender, tokenAddress, TokenType.standard, 1);
    }
