...
contract CandaoCoordinator is Ownable, Pausable {
    ...
    // @dev: EVENTS
    event PriorityPassBought(address indexed buyer, string indexed reservationToken, uint256 packageIndex, uint256 price, uint256 domainLength);
    event DomainBought(address indexed buyer, string indexed reservationToken, uint256 price, uint256 domainLength);
    ...
    function buy(uint256 packagePrice, string memory domain, string memory reservationToken) external whenNotPaused {
        ...
        require(selectedPackage.length != 0, "CandaoCoordinator: packages for price not found");

        uint256 characterCount = bytes(domain).length;
        require(characterCount != 0, "CandaoCoordinator: Incorrect domainLength.");

        PriorityPassPackageValue memory domainConfiguration;
        for (uint256 i = 0; i <= selectedPackage.length; i++) {
            if (selectedPackage[i].domainLength == characterCount) {
                domainConfiguration = selectedPackage[i];
            }
        }

        require(domainConfiguration.valid, "CandaoCoordinator: package value not found.");

        token.transferFrom(msg.sender, wallet, domainConfiguration.totalPrice);
        _userInfo[msg.sender] = ProrityPass(packagePrice, characterCount, block.number, domainConfiguration.totalPrice, true);
        emit PriorityPassBought(msg.sender, reservationToken, packagePrice, domainConfiguration.totalPrice, characterCount);
    }
