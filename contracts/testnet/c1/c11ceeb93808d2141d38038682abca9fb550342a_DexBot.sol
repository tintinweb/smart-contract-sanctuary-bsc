...
contract DexBot is Initializable {
    ...
    function withdrawMineRewards() public {
        ...
        uint256 amount = usermine.amount + usermine.rewards;
    ...
    function withdrawStakeRewards(uint256 plan) public {
        ...
        uint256 amount = userstake.amount + userstake.rewards;

        IERC20(QbtAddresss).transfer(msg.sender, amount);
    }

    function withdrawReferalRewardsRewards() public {
        User storage user = users[msg.sender];

        uint256 amount = user.totalRewardsQbt;
        if (amount > 0) {
            IERC20(QbtAddresss).transfer(msg.sender, amount);
        }
