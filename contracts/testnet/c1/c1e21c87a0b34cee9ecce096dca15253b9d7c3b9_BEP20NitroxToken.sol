...
/** 
  ...
  - Fees
   ...
   - 1% Reflection
   - 1% Buyback
   - 0% Play To Earn (Play To Earn fee will be activated when Cryptonitro Game launches)

Auto Liquidity
Auto Burn
Holders Rewards (reflection)
...
contract BEP20NitroxToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint8 public maxPlayToEarnFee = 5;
    ...
    uint8 public _playToEarnFee = 0;
    uint8 private _previousPlayToEarnFee = _playToEarnFee;
    ...
    address payable public playToEarnWallet;
    ...
    constructor (address tokenOwner,string memory tokenName,
        ...
        address payable _feeWallet, address payable _playToEarnWallet
        )  {
        ...
        numTokensSellToAddToLiquidity = amountOfTokenWei.mul(1).div(10000);
   ...
   function turnOnAntiBot(bool _status, uint8 _interval) onlyOwner public {
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 burnFee, uint8 walletFee, uint8 playToEarnFee, uint8 buybackFee) public onlyOwner() {
        ...
        require(playToEarnFee >= 0 && playToEarnFee <=maxPlayToEarnFee,"WF err");
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _burnFee + _walletFee +  _playToEarnFee + _buybackFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _walletFee == 0 && _playToEarnFee == 0 && _buybackFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousBurnFee = _burnFee;
        _previousWalletFee = _walletFee;
        _previousPlayToEarnFee = _playToEarnFee;
    ...
    function restoreAllFee() private {
        ...
        _playToEarnFee = _previousPlayToEarnFee;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        //This needs to be distributed among burn, wallet, playToEarn and liquidity
        //burn
        uint8 totFee  = _burnFee + _walletFee + _playToEarnFee + _liquidityFee + _buybackFee; 
        ...
        if(_playToEarnFee != 0){
            spentAmount  = contractTokenBalance.div(totFee).mul(_playToEarnFee);
            _tokenTransferNoFee(address(this), playToEarnWallet, spentAmount);
            totSpentAmount = totSpentAmount + spentAmount;
        }
