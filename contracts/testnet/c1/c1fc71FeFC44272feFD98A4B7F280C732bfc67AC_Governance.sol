...
contract Governance {
    ...
    event RefundProduct(address indexed product, uint256 round);
    ...
    function removeProduct(address _product) public onlyVoter {
        ...
        if (approveRemoveProduct.length() == voters.length()) {
            // Must deactive before remove, or will fail
    ...
    /** @notice Any voter can issue refund for an active product */
    function refundProduct(address _product, uint256 _round) public onlyVoter {
        IProduct(_product).refund(_round);
        emit RefundProduct(_product, _round);
    }

    /** @notice Any voter can set sesame credit */
