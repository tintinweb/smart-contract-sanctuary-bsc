...
import "./interfaces/IKapexPresaleClaim.sol";
import "./libraries/BokkyPooBahsDateTimeLibrary.sol";

contract KapexPresaleClaimContract {
  ...
  mapping(address => uint) public blacklistedUsers;
  mapping(address => uint) public newTotalBought;
  ...
  constructor(
    ...
    address _kapexPresaleClaimAddress,
    uint256 _minClaim,
  ...
  function getAvailableTokenToClaim(address _address) public view returns (uint256) {
    uint256 totalKapex;
    newTotalBought[_address] == 0 ? 
      totalKapex = calculateBNBToPresaleToken(_address, getTotalBought(_address))
      : totalKapex = calculateBNBToPresaleToken(_address, newTotalBought[_address]);
  ...
  function getTotalClaimPercentage() private view returns (uint256) {
    ...
    uint256 interval = BokkyPooBahsDateTimeLibrary.diffMonths(kapexPresale.getStartDateClaim(), _currentClaimDate);
  ...
  function claim(uint256 requestedAmount) public {
    require(blacklistedUsers[msg.sender] == 0, "This user is in the blacklist.");
    ...
    newTotalBought[msg.sender] == 0 ? 
      remainingToken = calculateBNBToPresaleToken(msg.sender, getTotalBought(msg.sender)) - getTotalClaim(msg.sender) 
      : remainingToken = calculateBNBToPresaleToken(msg.sender, newTotalBought[msg.sender]) - getTotalClaim(msg.sender);
