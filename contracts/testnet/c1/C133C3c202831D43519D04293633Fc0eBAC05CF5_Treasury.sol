...
contract Treasury is Ownable {
    ...
    function withdraw(address _recipient, uint256 _amount) external onlyOperator {
        require(withdrawalEnabled, "Withdrawal disabled");
        require(USDT.balanceOf(address(this)) >= _amount, "Insufficient token balance in treasury");
    ...
    function enableWithdrawal(bool _boolValue) external onlyOwner {
        withdrawalEnabled = _boolValue;

        emit EnableWithdrawal(_boolValue);
    }
    ...
    event EnableWithdrawal(bool boolValue);
