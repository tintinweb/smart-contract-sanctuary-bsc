...
contract CCL is Context, Ownable, ReentrancyGuard {
   ...
   IERC20 private WBNBToken = IERC20(address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd)); // WBNB CONTRACT TEST NET
    ...
    uint256 public totalRequests = 0;  // Total Requestes
    mapping(address => Applications) public Data;  // UserData Mapping
    mapping(address => startupInformations) public StartupInformation;
    mapping(address => bool) public isScam; // Indicates if address was/is involved in a scam or there is an ongoing case
    mapping(address => bool) public isSuspected; // Review of Newly deployed contracts [LTS HOLDER FEATURE]  
    mapping(address => bool) public isVerified; // WE need to check if the public address is verified or not 
    mapping(address => bool) public Requested;     // Indicates if user has an ongoing request
    ...
    function newRequest(bool payLTS, bool isContract, address _contract, string memory _Chain) external returns (bool) {
        ...
        if(payLTS) { payInLTS(); } else { payInWBNB(); }
        // Application
    ...
    function _isVerfiedAddress(address _addr) external view returns (bool) { return isVerified[_addr]; }
    ...
    function StartupInfo(address _startupAddress) external view returns (address,string memory, string memory,string memory, uint256) { 
        ...
        return (startupAddress, startupName, founder, StartupField, StartupRate);
    }
