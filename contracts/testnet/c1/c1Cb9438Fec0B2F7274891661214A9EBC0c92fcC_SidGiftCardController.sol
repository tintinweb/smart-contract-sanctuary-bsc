...
contract SidGiftCardController is Ownable {
    ...
    constructor(SidGiftCardRegistrar _registrar, ISidPriceOracle _priceOracle) {
        registrar = _registrar;
        priceOracle = _priceOracle;
    }

    function price(uint256[] memory ids, uint256[] memory amounts) public view returns (uint256) {
        return priceOracle.giftCardPriceInBNB(ids, amounts).base;
    }

    function batchRegister(uint256[] memory ids, uint256[] memory amounts) public validVoucherIdList(ids) payable {
