...
contract BattleContract is ERC2771Context, BillionsBaseContract, ReentrancyGuard {
    enum BattleState {
        Betting,
        Started,
        Ended,
    ...
    mapping(uint256 => uint256[]) battlesEveryEpoch;    // epoch => array of battle ids

    modifier onlyAdmin() {
        require(_msgSender() == adminAddress, "Not administrator");
        _;
    }

    modifier onlyVerifiedPlayer() {
        require(verifiedPlayers[_msgSender()] == true, "No verified player");
        _;
    }
    ...
    event _CreateBattle(uint256 _battleId);
    ...
    event _SendReward2Admin(uint256 _battleId, address _creator, address _admin, uint256 _amount);
    ...
    function CreateBattle(
        ...
        _epoch = (_epoch / battlePeriod) * battlePeriod;
        require(_epoch >= currentEpoch, "You can't bet on old battle");
        ...
        battle.creatorAddress = _msgSender();
        battle.nftCount = _nftCount;
        battle.enterFee = _enterFee;
        battle.epoch = _epoch;
        battle.state = BattleState.Betting;

        battlesEveryEpoch[_epoch].push(battleId);
        emit _CreateBattle(battleId);
    }
    ...
    function BetBattle(
        ...
        PlayerInfo storage player = enteredPlayerInfos[_battleId][_msgSender()];

        for (i = 0; i < nftCount; i++) {
            uint256 _nftId = _nftIds[i];
            
            if(billionsNFT.getOwner(_nftId) != _msgSender()) {
                require(billionsNFT.isRenter(_nftId, _msgSender()));
                require(billionsNFT.getUseableCount(_nftId, _msgSender()) > 0);

                billionsNFT.decUseableCount(_nftId);
            }
        ...
        for (i = 0; i < _scalarIds.length; i++) {
            require(scalarNFT.isAvailableScalar(_msgSender(), _scalarIds[i]) == true, "This scalar is owned by another player");
            player.scalarIds.push(_scalarIds[i]);
        }

        // Pay enter fee
        IERC20 payToken = IERC20(playTokenAddress);
        payToken.transferFrom(_msgSender(), address(this), battle.enterFee);

        enteredPlayerAddress[_battleId].push(_msgSender());

        emit _BetBattle(_battleId, _msgSender(), _nftIds, _scalarIds);
    }
    ...
    function StartBattles() public onlyAdmin {
        ...
        for (uint256 i = 0; i < battleCount; i++) {
            ...
            if (playerCount < 4) {
                ...
                battles[_battleId].state = BattleState.Started;
            }
        }
        ...
        onlyAdmin
    {
        ...
        require(battle.state == BattleState.Started, "Battle have not started yet"); // check that battle status is started state

        battle.state = BattleState.Ended; // set battle status to ended state
        ...
        if (adminAddress != battle.creatorAddress) {
            ...
            validWithdrawAmount += (totalAmount * 5 * 75) / 10000;
            ...
            validWithdrawAmount += (totalAmount * 5) / 100;
        }
    }
        ...
        Claimable(_battleId)
    {
        ...
        payToken.transfer(_msgSender(), reward);

        emit _ClaimedBonus(_battleId, _msgSender(), reward);

        players[_msgSender()] = 0;
    }
        ...
        onlyVerifiedPlayer
    {
        ...
        payToken.transfer(_msgSender(), bonus);

        emit _ClaimedBonus(_battleId, _msgSender(), bonus);
...
contract BillionsBaseContract is Ownable {
    ...
    * calculate the amount of matic from USDC 
     */
    function getMaticAmountFromUSDT(uint256 amount) public view returns(uint) {
        uint256 bnbPrice =uint256( getMaticPrice() );
        uint256 bnbAmount = uint256( amount * (10 ** maticDecimals) * (10 ** maticDecimals) / bnbPrice / (10 ** decimals)) ;
        return bnbAmount;
    }
...
contract BillionsNFT is ERC721, BillionsBaseContract, ReentrancyGuard {
    ...
    string public baseURI = "https://ipfs.io/billions/nfts/";

    struct RentInfo {
        address renter;             // address of renter
        uint256 useableCount;       // useable count until expire
        uint256 expire;             // time of expire
    }
    
    struct NFTInfo {
        uint256 nftId;
        uint256 contestType;        // type of NFT, 1: stock, 2: crypto
        uint256 symbolId;           // 
        bool    rentable;           // flag of marketing
        address owner;              // address of owner
        uint256 rentPrice;
    }

    mapping(uint256 => uint256[]) public availableSymbolIds;    // nft type => Array of symbolId
    mapping(uint256 => mapping(uint256 => bool)) private usedSymbolId;   // contestType => (symbolId => used)

    mapping(uint256 => NFTInfo) public nftInfos;        // nftId => NFT
    mapping(address => uint256[]) public ownedNfts;     // player => Array of nft id

    mapping(uint256 => RentInfo[]) public rentInfos;    // nft id => Rent Infos
    mapping(address => uint256[]) public rentedNfts;    // address => nftIds 

    // Check if caller have enough fund to mint
    modifier Mintable() {
        require(msg.value >= getMaticAmountFromUSDT(mintPrice - mintPrice * slippage / 100) , "Not enough funds to mint");
        _;
    }

    event _BillionsNftMint(uint256 _type, uint256 _nftId, address _owner);
    ...
    function setRenter(uint256 tokenId, address user, uint256 useableCount, uint256 expire) public {
        require(_isApprovedOrOwner(msg.sender, tokenId),"ERC721: transfer caller is not owner nor approved");
        require(expire >= block.timestamp, "");
        uint256 i = 0;
        bool exist = false;
        RentInfo[] storage renters =  rentInfos[tokenId];
        uint256 count = renters.length;
        for(i = 0; i < count; i ++) {
            if(renters[i].renter == user) {
                renters[i].useableCount = useableCount;
                renters[i].expire = expire;
                exist = true;
            }
        }
        if(!exist) {
            renters.push(RentInfo(user, useableCount, expire));
        }

        uint256[] storage rentedNftIds = rentedNfts[user];
        count = rentedNftIds.length;
        exist = false;
        for(i = 0; i < count ; i ++) {
            if(rentedNftIds[i] == tokenId) {
                exist = true;
                break;
            }
        }
        if(!exist) {
            rentedNftIds.push(tokenId);
        }
    }

    function mint(uint256 _type, uint256 _symbolId) external payable Mintable returns (uint256) {
        ...
        emit _BillionsNftMint(_type, _nftId, msg.sender);
        return _nftId;
    }

    function mintByERC20(uint256 _type, uint256 _symbolId) external returns (uint256) {
        require(!usedSymbolId[_type][_symbolId], "not available");
        ...
        nftInfo.contestType = _type;
        nftInfo.symbolId = _symbolId;
    ...
    function mintByPack(uint256 _type) public  {
        ...
        for(uint256 idx = 0; idx < 3; idx ++) {
            ...
            nftInfo.rentable = false;
            nftInfo.rentPrice = 0;
            nftInfo.owner = tx.origin;

            usedSymbolId[_type][_symbolId] = true;
            ownedNfts[tx.origin].push(_nftId);

            availableSymbolIds[_type][_rand % _availableCount] = availableSymbolIds[_type][_availableCount - 1];
            availableSymbolIds[_type].pop();
        
            emit _BillionsNftMint(_type, _nftId, tx.origin);
        }
    }
    ...
    function tokenURI(uint256 _nftId) public view virtual override returns (string memory uri) {
        ...
        uri = bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, Strings.toString(nftInfos[_nftId].symbolId))) : "";
    }
    ...
    function getOwner(uint256 _nftId) external view returns (address) {
        return nftInfos[_nftId].owner;
    }

    function isRenter(uint256 _nftId, address _addr) external view returns (bool) {
        uint256[] memory nftIds = rentedNfts[_addr];
    ...
    function getUseableCount(uint256 _nftId, address _addr) external view returns (uint256) {
        RentInfo[] memory infos = rentInfos[_nftId];
        uint256 count = infos.length;
        for(uint256 i = 0; i < count; i ++) {
            if(infos[i].renter == _addr) {
                return infos[i].useableCount;
            }
        }
        return 0;
    }

    function decUseableCount(uint _nftId) external {
        RentInfo[] storage infos = rentInfos[_nftId];
        uint256 count = infos.length;
        for(uint256 i = 0; i < count; i ++) {
            if(infos[i].renter == msg.sender) {
                if(infos[i].useableCount > 0) {
                    infos[i].useableCount -= 1;
                }
                if(infos[i].useableCount == 0) {
                    infos[i] = infos[count - 1];
                    infos.pop();

                    uint256[] storage nftIds = rentedNfts[msg.sender];
                    uint256 nftCount = nftIds.length;
                    for(uint256 j = 0; j < nftCount; j ++) {
                        if(nftIds[j] == _nftId) {
                            nftIds[j] = nftIds[nftCount - 1];
                            nftIds.pop();
                        }
                    }
                    break;
...
contract ScalarNFT is ERC721, VRFConsumerBaseV2, BillionsBaseContract {
    ...
    modifier Refillable() {
        require(msg.value >= getMaticAmountFromUSDT(refillPrice - refillPrice * slippage / 100), "Not enough funds to refill");
        _;
    }
    ...
    * only operator can update
    ...
    function clearRandomData(uint256 _nftId) public OnlyOperator {
        ...
        emit _ScalarChanged(_nftId, scalarNfts[_nftId].player, scalarNfts[_nftId].scalarType, 0);
    }
