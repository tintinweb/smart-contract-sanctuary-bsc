...
contract xYSLBUSDVault is AccessControl, Initializable {
    ...
    function deposit(address user, uint amountLp) external {
        ...
        // if(isReceipt){
            // uint before = IERC20(xYSLBUSD).balanceOf(address(this));
            //todo sell
            // amountLp = IPancakeMaster(masterChef).userInfo(id, user).amount- before;
        // }else if(amountTokenA != 0 && amountTokenB != 0){
        //     path[0] = IUniswapV2Pair(xYSLBUSD).token0();
        //     path[1] = IUniswapV2Pair(xYSLBUSD).token1();
        ...
        //     IERC20(IUniswapV2Pair(xYSLBUSD).token0()).approve(router,amountTokenA);
        //     IERC20(IUniswapV2Pair(xYSLBUSD).token1()).approve(router,amountTokenB);
        ...
        //     ITokenSwap(Admin.tokenSwap()).swapTOKENtoLP(Admin.BUSD(), address(xYSLBUSD), amountTokenA, address(this));
        //     amountLp = IERC20(address(xYSLBUSD)).balanceOf(address(this));
        // }
        // else if(amountLp != 0){
            IERC20(address(xYSLBUSD)).transferFrom(user, address(this), amountLp);
        // }
    ...
    function withdraw(address user,bool isReciept, uint _amount) external{
        ...
        if(isReciept){
            ...
            // }
            // else{
            //     IERC20(address(xYSLBUSD)).safeApprove(router,balance);
            ...
            //     if(tokenA && tokenB){
            //         IERC20(xYSLBUSD.token0()).safeTransfer(user, amountA);
            //         IERC20(xYSLBUSD.token1()).safeTransfer(user, amountB);
            //     }else{
            //         address[] memory path = new address[](2);
            //         path[0] = Admin.xYSL();
            //         path[1] = Admin.BUSD();
            //         IERC20(Admin.BUSD()).safeTransfer(user, IERC20(Admin.BUSD()).balanceOf((address(this))));
            //         uint balancexYSL = IERC20(Admin.xYSL()).balanceOf((address(this)));
            //         IERC20(Admin.xYSL()).approve(router, balancexYSL); //BSC Testnetapeswap router address
            ...
            //     }
            // }
            INTT(xYSLBUSD_A).mint(address(this),balance);
        }
