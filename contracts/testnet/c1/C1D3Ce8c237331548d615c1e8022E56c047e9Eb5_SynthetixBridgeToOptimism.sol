...
contract SynthetixBridgeToOptimism is BaseSynthetixBridge, ISynthetixBridgeToOptimism {
    ...
    function migrateEscrow(bytes32 currencyKey, uint256[][] memory entryIDs) public requireInitiationActive requireZeroDebt {
        _migrateEscrow(currencyKey, entryIDs);
    }
    ...
    function finalizeWithdrawal(address to, uint256 amount) external onlyCounterpart {
        ...
        // emit IL1StandardBridge.WithdrawalFinalized(to, amount);
    }
    ...
    function _initiateDeposit(address _to, uint256 _depositAmount) private {
        ...
        // emit IL1StandardBridge.DepositInitiated(msg.sender, _to, _depositAmount);
    }

    function _migrateEscrow(bytes32 currencyKey, uint256[][] memory _entryIDs) private {
        ...
        for (uint256 i = 0; i < _entryIDs.length; i++) {
            ...
            (escrowedAccountBalance, vestingEntries) = rewardEscrowV2Manager().getRewardEscrow(currencyKey).burnForMigration(msg.sender, _entryIDs[i]);
...
interface ISynthetixBridgeToOptimism {
    ...
    function migrateEscrow(bytes32 currencyKey, uint256[][] calldata entryIDs) external;

    function depositReward(uint256 amount) external;

    function depositAndMigrateEscrow(bytes32 currencyKey, uint256 depositAmount, uint256[][] calldata entryIDs) external;
}
