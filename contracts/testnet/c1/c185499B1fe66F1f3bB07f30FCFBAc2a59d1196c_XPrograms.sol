...
contract XPrograms is XProgram {
	...
	function ChangeStatusALU(uint _AID) external OnlyAccountOwner(_AID, msg.sender) {
		_ChangeStatusALU(_AID);
	}

	function ChangeStatusALU(uint _AID, address _Owner) external OnlyAPI OnlyAccountOwner(_AID, _Owner) {
		_ChangeStatusALU(_AID);
	}
	...
	function _ShareReward(uint _AID, uint _XPro, uint _Level) private {
		...
		if (_XPro == X3) {
			...
			if (ccu1.XY[1][3] == _AID) Balances._Locking(cu1, 0, sr); // lock for recycle

			Balances._TransferReward(_AID, cu1, sr);
			//
		} else if (_XPro == X6 || _XPro == X7) {
			// Cycle storage ccu1 = GetCurrentCycle(_AID, _XPro, _Level);
			// uint cu1 = ccu1.CycleUplineID;
			// if (cu1 == 0) return; // cu1 = 0 means _A = root : do nothing
			...
			if (CFStatus) {
				// Community fund actived, CFRatio = 20%
				acu2 = (acu2 * 8) / 10;
				if (Accounts.num() > 20000) {
					CFLookup.AddNoDuplicate(_AID);
					_PendingToCF(_AID, _XPro == X6 ? BINARY : TERNARY, (sr - acu2)); // X6: BINARY, X7: TERNARY
				}
				Balances._TransferReward(_AID, Accounts.CFID(), sr - acu2);
			}
			Balances._TransferReward(_AID, cu2, acu2);
			//
		} else if (_XPro == X8 || _XPro == X9) {
			// Cycle storage ccu1 = GetCurrentCycle(_AID, _XPro, _Level);
			// uint cu1 = ccu1.CycleUplineID;
			// if (cu1 == 0) return; // cu1 = 0 means _A = root : do nothing
			...
			if (CFStatus) {
				...
				if (Accounts.num() > 20000) {
					CFLookup.AddNoDuplicate(_AID);
					_PendingToCF(_AID, _XPro == X8 ? BINARY : TERNARY, (sr - (acu2 + acu3))); // X8: BINARY, X9: TERNARY
				}
