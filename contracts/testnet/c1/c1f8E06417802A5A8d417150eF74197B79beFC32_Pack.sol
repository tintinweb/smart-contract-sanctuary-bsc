...
contract Pack is Context, IERC721Receiver {
    ...
    event OpenPack(address addressSender, uint256 _requestId ,uint256 tokenId);
    ...
    function afterRanOpenPack(
        uint256 requestId,
        uint256[] memory randoms
    ) public returns(bool){

        uint256 packId = requestIdPack[requestId]; //1
        ...
        for (uint256 i = 0; i < pack.dropCount; i++) {
            ...
            //each slot
            for (uint256 j = 0; j < pack.slot; j++){
                
                SlotCollection memory slot = slotCollection[slotInPack[j]];
                ...
                //each time
                for (uint256 k = 0; k < slot.times; k++){
                    //item result
                    for (uint256 l = 0; l < slot.chances.length; l ++) { 
                        uint256 chance = chances[l]; // [5,10,30,55]
