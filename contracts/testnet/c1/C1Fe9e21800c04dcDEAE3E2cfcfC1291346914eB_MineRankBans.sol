...
contract MineRankBans is Ownable, IMineBans {
    using EnumerableSet for EnumerableSet.UintSet;

    // data
    EnumerableSet.UintSet private bannedNfts; // iterable set containing all listings

    // events
    event RankBanned(uint256 indexed tokenId);
    event RankUnbanned(uint256 indexed tokenId);
    ...
    /**
     * @dev Get whether a token is banned or not
     */
    function isBanned(uint256 tokenId) external view override returns (bool) {
        return bannedNfts.contains(tokenId);
    }

    /**
     * @dev Get all banned tokens
     */
    function getBannedIds() external view returns (uint256[] memory) {
        return bannedNfts.values();
    }
     ...
     * @dev bans an nft
     */
    function ban(uint256 tokenId) external onlyOwner {
        bannedNfts.add(tokenId);
    }

    /**
     * @dev unbans an nft
     */
    function unban(uint256 tokenId) external onlyOwner {
        bannedNfts.remove(tokenId);
    }
}
