   ...
   // uint public startTime = 1671476400; // Monday, 19 December 2022 20:00:00 UTC https://www.unixtimestamp.com/ //mainnet
    ...
    function addTopDepositRewardToPlan(address userAddress, uint8 plan, uint256 win) private {
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            ...
            if(user.deposits[i].plan == plan && safety == 0){ //add reward to first matching plan
                user.deposits[i].amount.add(win);
                safety = 1;
            }
         }
    }
	...
	function _drawPool() private {
        ...
        for(uint8 i = 0; i < pool_bonuses.length; i++) {
            ...
            //user.deposits.push(Deposit(plan, win, block.timestamp, true));
		    //addTopDepositRewardToPlan(topDepositPool[i].pool_top_addr, plan, win);
            
            //uint8 safety = 0;
            for (uint8 j = 0; j < user.deposits.length; j++) {            
              if(user.deposits[j].plan == plan ){ //add reward to first matching plan //&& safety == 0
                user.deposits[j].amount.add(win);
               // safety = 1;
              }
            }
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 percent, uint256 amount, uint256 totalInvested, uint256 start, uint256 finish, bool reinvested) {
	    User storage user = users[userAddress];
