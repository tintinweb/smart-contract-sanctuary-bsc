    ...
    VRFConsumerBaseV2Upgradable
{
    ...
    mapping(uint256 => uint256) internal poolToDeletedStakesAmount;
        ...
        override(VRFConsumerBaseV2Upgradable)
    {
        ...
        for (uint256 i = 0; i < requestIdToSilverAmount[requestId]; i++) {
            if (
                poolToDeletedStakes[requestIdToPoolId[requestId]][
                    randomWords[i]
                ] == 0
            ) {
                silverArray[i] =
        ...
        ] -= poolToDeletedStakesAmount[requestIdToPoolId[requestId]];
        result = randomWords;
        goldRes = goldArray;
        silRes = silverArray;
        ...
        poolShouldExist(poolId_)
    {
        ...
        if (numWords > 0) {
            ...
            poolToStakesAmount[poolId_] -= poolToDeletedStakesAmount[poolId_];
    ...
    function withdrawStake(uint256 stakeId_)
        public
        shouldBeUsersToken(betIdToPoolId[stakeId_], stakeId_)
    {
        require(
            block.timestamp <=
                stages[pools[betIdToPoolId[stakeId_]].stageId].endAt,
            "This stage is finished"
        );
        uint256 poolId = betIdToPoolId[stakeId_];
        uint256 amountToWithdraw = (pools[poolId].stakePrice -
            (pools[poolId].stakePrice * CurrentEvent.commission) /
            10000) * poolIdToTokenIdToBet[poolId][stakeId_].amount;

        IERC20(erc20).transfer(msg.sender, amountToWithdraw);

        delete poolIdToAccountToTokenIds[poolId][msg.sender][
            getIndexOfItemInArray(
                poolIdToAccountToTokenIds[poolId][msg.sender],
                stakeId_
            )
        ];
        ...
        for (uint256 i = 0; i < stakes.length; i++) {
            string memory stakeOn = poolIdToTokenIdToBet[poolId][stakeId_]
                .stakeOn;
            poolToDeletedStakes[poolId][stakes[i]] = 1;
            poolToDeletedStakesAmount[poolId]++;
            poolToResultToAmount[poolId][stakeOn]--;
        }

        poolToAssets[poolId] -= amountToWithdraw;

        burn(stakeId_);
        emit StakeWithdrawn(msg.sender, stakeId_, poolId);
    }
