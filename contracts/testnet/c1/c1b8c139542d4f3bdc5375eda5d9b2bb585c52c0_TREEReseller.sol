...
contract TREEReseller is Ownable {
    ...
    address public boss3 = 0xf2FD6391E1c0dB6Eb4e3282eB20D3C8cb85c6f72;	
    address public boss4 = 0x16144fE7522B2D6729D617E2b29578603E7F0d03;

    bool public active;

    mapping (address => uint256) public interestBalance;

    event OnBuy(address indexed account, uint256 usdt, uint256 tree, uint256 rate);
    ...
    constructor(address USDTAddr, address TREEAddr, uint256 initialRate, uint256 initialPeriod, address initialOwner) public Ownable(initialOwner) {
        require(USDTAddr != address(0) && TREEAddr != address(0));
        require(initialRate > 0);

        USDT = IUSDT(USDTAddr);
        TREE = IERC20(TREEAddr);
    ...
    function buy(uint256 value, address _ref1, address _ref2, address _ref3, address _ref4, address _ref5) public onlyActive {
        ...
        TREE.freezeAndTransfer(msg.sender, amount, period);
    ...
    function freezeAndTransfer(address recipient, uint256 amount, uint256 _period) public onlyActive {
        ...
        TREE.freezeAndTransfer(recipient, amount, _period);
    ...
    function allowanceTREE(address account) public view returns(uint256) {
        return TREE.allowance(account, address(this));
    }
    ...
    function balanceTREE(address account) public view returns(uint256) {
        return TREE.balanceOf(account);
    }
