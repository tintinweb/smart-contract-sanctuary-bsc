...
contract MASTERSynth is Ownable, ReentrancyGuard {
    ...
    // REFFERAL SYSTEM
    uint256 internal REFERRAL_PERCENT = 50;
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({want: _want, wtoken: _wtoken, strat: _strat}));
    }
        ...
        returns (uint256)
    {
        ...
        uint256 userStaked = IERC20(pool.wtoken).balanceOf(_user);
        return calculateSwapSTtoTokens(_pid, userStaked);
    }

    function calculateSwapSTtoTokens(uint256 _pid, uint256 _amountWrapped)
        public
        view
        returns (uint256)
    {
        ...
        return _amountWrapped.mul(wantLockedTotal).div(totalSupply);
    }
    ...
    function deposit(
        ...
        if (_wantAmt > 0) {
            ...
            user.deposited = user.deposited.add(_wantAmt);
        }
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        uint256 pricePerShare = IStrategy(pool.strat).getPricePerFullShare();

        require(userSTokens > 0, "userSTokens is 0");
        require(totalSupply > 0, "totalSupply is 0");

        uint256 unWrappedTokens = calculateSwapSTtoTokens(_pid, _wantAmt);
        uint256 profitCheck;
        ...
        if (unWrappedTokens > user.deposited) {
            ...
            if (user.referrer != address(0)) {
                RefComission = profitCheck.mul(REFERRAL_PERCENT).div(
                    PERCENTS_DIVIDER
                );
                RefComission = RefComission.div(pricePerShare);
            }
        }
        ...
        pool.wtoken.safeIncreaseAllowance(pool.strat, _wantAmt);

        if (RefComission > 0) {
            _wantAmt = _wantAmt.sub(RefComission);
        }

        // Withdraw want tokens
        IStrategy(poolInfo[_pid].strat).withdraw(msg.sender, _wantAmt);

        // Send Referal Comission
        uint256 wTokenBal = IERC20(pool.wtoken).balanceOf(address(this));
        if (wTokenBal > 0) {
            pool.wtoken.safeTransfer(address(user.referrer), wTokenBal);
            userInfo[_pid][user.referrer].deposited = userInfo[_pid][
                user.referrer
            ].deposited.add(wTokenBal);
        }

        // Transfer Want tokens to User
        uint256 wantBal = IERC20(pool.want).balanceOf(address(this));
        if (wantBal > 0) {
            if (wantBal > user.deposited) {
                user.deposited = 0;
            } else {
                user.deposited = user.deposited.sub(wantBal);
            }

            pool.want.safeTransfer(address(msg.sender), wantBal);
        }
        emit Withdraw(msg.sender, _pid, wantBal);
    }

    function withdrawAll(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        withdraw(_pid, IERC20(pool.wtoken).balanceOf(msg.sender));
    }

    function depositAll(address _referral, uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        deposit(_referral, _pid, IERC20(pool.want).balanceOf(msg.sender));
    }

    function addReferral(address _referral, uint256 _pid) internal {
        UserInfo storage user = userInfo[_pid][msg.sender];
        if (user.referrer == address(0)) {
            ...
            if (upline != address(0)) {
                userInfo[_pid][upline].refferals = userInfo[_pid][upline]
                    .refferals
                    .add(1);
            }
        }
    ...
    function getReferral(address _user, uint256 _pid)
        public
        view
        returns (address)
    {
    ...
    function getReferralCount(address _user, uint256 _pid)
        public
        view
        returns (uint256)
    {
    ...
    function getReferralRewardTotal(address _user, uint256 _pid)
        public
        view
        returns (uint256)
    {
