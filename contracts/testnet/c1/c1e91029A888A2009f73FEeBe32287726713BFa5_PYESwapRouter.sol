...
// File: PyeSwap_swap-periphery/contracts/libraries/Context.sol
...
// File: PyeSwap_swap-periphery/contracts/libraries/Ownable.sol
...
// File: PyeSwap_swap-periphery/contracts/interfaces/IPYESwapRouter01.sol
...
// File: PyeSwap_swap-periphery/contracts/interfaces/IPYESwapRouter.sol
...
// File: PyeSwap_swap-periphery/contracts/interfaces/IToken.sol
...
// File: PyeSwap_swap-periphery/contracts/interfaces/IWETH.sol
...
// File: PyeSwap_swap-periphery/contracts/interfaces/IERC20.sol
...
// File: PyeSwap_swap-periphery/contracts/libraries/SafeMath.sol
...
// File: PyeSwap_swap-periphery/contracts/interfaces/IPYESwapPair.sol
...
// File: PyeSwap_swap-periphery/contracts/interfaces/IPYESwapFactory.sol
...
// File: PyeSwap_swap-periphery/contracts/FeeStore.sol
...
abstract contract FeeStore is Ownable {
    ...
    function feeAdddressSetWhileSwap(address pair,address tokenAddress) public onlyOwner {
        require(IPYESwapFactory(factoryAddress).pairExist(pair), "PYESwap FeeStore: Pair is not Exist");
        require(IPYESwapPair(pair).token0() == tokenAddress || IPYESwapPair(pair).token1() == tokenAddress, "PYESwap FeeStore: Invalid token address");
...
// File: PyeSwap_swap-periphery/contracts/libraries/PYESwapLibrary.sol
...
library PYESwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'665fe1cec9170157dda8e504d670cbe0b2bf81a98f892d03f233c84524f72047' // init code hash
            ))));
    }
...
// File: PyeSwap_swap-periphery/contracts/libraries/TransferHelper.sol
...
// File: PyeSwap_swap-periphery/contracts/SupportingSwap.sol
...
abstract contract SupportingSwap is FeeStore, IPYESwapRouter {
    ...
    function swapTokensForExactTokens(
        ...
        if(path[0] == pairFeeAddress[pair]) {
            ...
            amounts = PYESwapLibrary.getAmountsOut(factory, amounts[0], path);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, pair, amounts[0]
            );
    ...
    returns (uint[] memory amounts)
    {
        ...
        uint adminFeeDeduct;
       // if(path[0] == pairFeeAddress[pair]){
       //     (bnb, adminFeeDeduct) = PYESwapLibrary.adminFeeCalculation(bnb, adminFee);
       ...
       // }
        ...
        (bnb, adminFeeDeduct) = PYESwapLibrary.adminFeeCalculation(bnb, adminFee);
        payable(feeAddressGet()).transfer(adminFeeDeduct);
        uint swapAmt = msg.value.sub(adminFeeDeduct);

        amounts = PYESwapLibrary.getAmountsOut(factory, swapAmt, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PYESwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    returns (uint[] memory amounts)
    {
        ...
        uint outAmt = amountETHOut.sub(adminFeeDeduct);
        IWETH(WETH).withdraw(outAmt);
        TransferHelper.safeTransferETH(to, outAmt);
        TransferHelper.safeTransferETH(feeAddressGet(), adminFeeDeduct);
    }
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
    external
    virtual
    ...
    function _calculateAmounts(address input, address output, address token0) internal view returns (uint amountInput, uint amountOutput) {
        ...
        amountOutput = PYESwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, baseToken != address(0), totalFee);
    }
    ...
    {
        return PYESwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, false, 0);
    }
    ...
    {
        return PYESwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, false, 0);
    }
...
// File: PyeSwap_swap-periphery/contracts/PYESwapRouter.sol
...
contract PYESwapRouter is SupportingSwap {
    ...
    constructor(address _factory, address _WETH, uint256 _adminFee, address _adminFeeAddress) public {
        factory = _factory;
        WETH = _WETH;
    ...
    function _addLiquidity(
        ...
        if (getPair(tokenA, tokenB) == address(0)) {
            if(tokenA == WETH) {
                IPYESwapFactory(factory).createPair(tokenB, tokenA, supportsTokenFee);
