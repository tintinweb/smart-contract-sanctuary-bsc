...
contract Chain is Ownable, ReentrancyGuard {
    ...
    uint24 private constant DAY = 60;
    ...
    function setInputer(address _inputer) public onlyOwner {
        require(_inputer != address(0), "zero address");
        inputer = _inputer;
    }
    ...
    function claim() public {
        ...
        uint256 amountToClaim = user.reward - user.totalClaimed;
    ...
    function inputROI(address[] memory list, uint256[] memory reward) public {
        require(inputer == msg.sender, "Not authorized");
        require(list.length == reward.length, "invalide data");
        for(uint i = 0; i < list.length; i++){
             users[list[i]].reward = reward[i];
        }
    }
