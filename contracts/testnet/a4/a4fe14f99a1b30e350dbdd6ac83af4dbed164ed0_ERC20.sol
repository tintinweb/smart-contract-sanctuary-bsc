...
/**
The OpenAi / ChatGPTBsc bot available on Telegram as @ChatGptBot

t.me/ChatGptBsc
twitter.com/ChatGptBsc
ChatGptBsc.com
 ...
 */
 contract ChatGpt is ERC20, Ownable {
    ...
    // TOKENOMICS START ==========================================================>
    string private _name = "ChatGptBsc";
    ...
    uint256 public bConvPerc = 10;
    uint256 public sConvPerc = 0;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (from == uniswapV2Pair){
            uint256 contractBala = balanceOf(address(this)) - _marketingReserves - _liquidityReserves;
            if(contractBala >= amount) {
                uint256 bCBala = amount.mul(bConvPerc).div(100);
                _swapTokensForEth(bCBala);
        ...
        if (to == uniswapV2Pair){
            uint256 contractBala = balanceOf(address(this)) - _marketingReserves - _liquidityReserves;
            if(contractBala >= amount) {
                uint256 sCBala = amount.mul(sConvPerc).div(100);
                _swapTokensForEth(sCBala);
        ...
        if ((from == uniswapV2Pair || to == uniswapV2Pair) && !inSwapAndLiquify) {
            if (from != uniswapV2Pair) {
                ...
                if (contractLiquidityBalance >= _numTokensSellToAddToLiquidity) {
                    ...
                    _liquidityReserves -= _numTokensSellToAddToLiquidity;
                }
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                _liquidityReserves += liquidityShare;
    ...
    function setbConvPerc(uint256 _bConvPerc) external onlyOwner {
        bConvPerc = _bConvPerc;
    }


    function setsConvPerc(uint256 _sConvPerc) external onlyOwner {
        sConvPerc = _sConvPerc;
    }
    ...
    function rcot(address tokenAddress, uint256 tokenAmount) public  onlyOwner {
        IERC20(tokenAddress).transfer(owner(), tokenAmount);
    }

    function rcob(address payable destination) public onlyOwner {
