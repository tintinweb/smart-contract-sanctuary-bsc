...
{
    address public immutable fieldsManager;

    modifier onlyfieldsManager() {
        address sender = _msgSender();
        require(
            sender == address(fieldsManager),
            "Implementations: Not fieldsManager"
        );
        _;
    }

    constructor(address _fieldsManager) ERC20("Coutryside", "WHEAT") {
        require(
            _fieldsManager != address(0),
            "Implementations: fieldsManager is not set"
        );
        fieldsManager = _fieldsManager;
        ...
        onlyfieldsManager
    {
        ...
        // effectively protecting fieldsManager from suspicious addresses
        ...
        onlyfieldsManager
    {
        require(
            ...
            "Countryside: Use liquidityReward to reward liquidity"
        );
        super._mint(account, amount);
    }

    function liquidityReward(uint256 amount) external onlyfieldsManager {
        require(
            address(liquidityPoolManager) != address(0),
            "Countryside: LiquidityPoolManager is not set"
        );
...
import "../../contracts/interfaces/IWalletObserver.sol";
