...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event ParcelMinted(address indexed user, uint256 indexed parcelId);
    event MinerMinted(address indexed user, uint256 indexed minerId);
    ...
    event VouchersApplied(address indexed user, uint256 indexed minerId, uint256 vouchersCount);
    ...
    function mintMiners(uint256 _count, uint256 _referrerId)
        external
        onlySaleActive
        notZeroAddress
    {
    ...
    function applyVoucher(uint256 numRequestedVouchers, uint256 minerId) external notZeroAddress {
        address userAddress = IERC721Base(minerV1NFTAddress).ownerOf(minerId);
        require(msg.sender == userAddress, "Municipality: Invalid owner of the miner");

        uint256 numAvailableVouchers = userToElectricityVoucherAmountMapping[msg.sender];
        require(
            numAvailableVouchers >= numRequestedVouchers,
        ...
        emit VouchersApplied(msg.sender, minerId, numRequestedVouchers);
    }
    ...
    function attachDetachMinersToParcel(uint256[] calldata minersToAttach, uint256 parcelId) external notZeroAddress {
        ...
        _requireMinersBelongToUser(minersToAttach, msg.sender);

        _attachDetachMinersToParcel(minersToAttach, parcelId);
    }

    /// @notice Used by MinerPublicBuilding to attach miner to a parcel
    function attachMinerToParcel(address user, uint256 minerId, uint256 parcelId) external onlyMinerPublicBuilding {
        minerParcelMapping[minerId] = parcelId;
        parcelMinersMapping[parcelId].push(minerId);
        emit ParcelMinted(user, parcelId);
        emit MinerMinted(user, minerId);
        emit MinerAttached(user, parcelId, minerId);
    }
    ...
    /// @notice Transfers the given BUSD amount to this smart contract
    ...
    /// @notice Checks if the miners belong to the owner before making any operations with them
    function _requireMinersBelongToUser(
        uint256[] memory minersArray,
        address userWalletAddress
    ) private view {
        for (uint256 i; i < minersArray.length; ++i) {
            uint256 tokenId = minersArray[i];
            require(
                IERC721Base(minerV1NFTAddress).ownerOf(tokenId) == userWalletAddress,
                "Municipality: Invalid NFT owner"
            );
        }
    }
    ...
    function _attachDetachMinersToParcel(uint256[] memory newMiners, uint256 parcelId) private {
        ...
        for (uint256 index; index < oldMiners.length; index++) {
            ...
            if (!_isMinerInList(tokenId, newMiners)) {
                ...
                IMinerNFT(minerV1NFTAddress).updateTokenLocking(tokenId, false);
        ...
        for (uint256 index; index < newMiners.length; index++) {
            ...
            if (!_isMinerInList(tokenId, oldMiners)) {
                ...
                IMinerNFT(minerV1NFTAddress).updateTokenLocking(tokenId, true);
        ...
        if (newMiners.length == 0) {
            IParcelInterface(standardParcelNFTAddress).updateTokenLocking(parcelId, false);
        } else {
            if (oldMiners.length == 0) {
                IParcelInterface(standardParcelNFTAddress).updateTokenLocking(parcelId, true);
            }
        }
        parcelMinersMapping[parcelId] = newMiners;
    }

    /// @notice Mint the given parcels
    function _mintParcels(Parcel[] calldata parcels) private {
        for (uint256 i = 0; i < parcels.length; ++i) {
            ...
            require(
                !IParcelInterface(standardParcelNFTAddress).parcelExists(
                    parcel.x,
                    parcel.y,
            ...
            require(
                !IParcelInterface(businessParcelNFTAddress).parcelExists(
                    parcel.x,
                    parcel.y,
                    parcel.parcelLandType
                ),
            ...
            uint256 parcelId = IParcelInterface(addr).mint(
                msg.sender,
                parcel.x,
                parcel.y,
                parcel.parcelLandType
            );
            emit ParcelMinted(msg.sender, parcelId);
        }
    }

    /// @notice Mint a given amount of parcels
    function _mintMiners(uint256 _count) private {
        for (uint256 i = 0; i < _count; ++i) {
            uint256 minerId = IMinerNFT(minerV1NFTAddress).mint(msg.sender);
            emit MinerMinted(msg.sender, minerId);
        }
    }
    ...
    function _requireMinersCountMatchingWithParcelSlots(uint256 _parcelId, uint256 _count)
        ...
        view
    {
        bool isParcelUpgraded = IParcelInterface(standardParcelNFTAddress).isParcelUpgraded(
            _parcelId
        );
        require(
            isParcelUpgraded
    ...
    function _getPriceForParcels(Parcel[] memory parcels) private view returns (uint256) {
        ...
        uint256 standardPrice = currentStandardParcelPrice;
        uint256 businessPrice = currentBusinessParcelPrice;
        uint256 totalPrice = 0;
        for (uint256 i = 0; i < parcels.length; ++i) {
            Parcel memory parcel = parcels[i];
            if (parcel.parcelType == PARCEL_TYPE_STANDARD) {
                uint256 _price = soldCountToStandardParcelPriceMapping[currentStandards];
                if (_price != 0) {
                    standardPrice = _price;
                }
                totalPrice += standardPrice;
                currentStandards++;
            } else if (parcel.parcelType == PARCEL_TYPE_BUSINESS) {
                uint256 _price = soldCountToBusinessParcelPriceMapping[currentBusinesses];
                if (_price != 0) {
                    businessPrice = _price;
                }
