...
contract xCatheon is IxCatheon, ERC20Permit {
    ...
    /**
        @notice increases rCatheon supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs xCatheon of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized xCatheon
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "xCatheon: insufficient balance");
    }
    ...
    // Staking contract holds excess xCatheon
...
interface IxCatheon is IERC20 {
    function rebase(uint256 catheonProfit_, uint256 epoch_) external returns (uint256);
