...
import {erc721Error} from "./erc721Error.sol";
import {MintStorage} from "./MintStorage.sol";

contract Erc721Token is OwnableUpgradeable, 
    ...
    function _initialize(address controller_,address payable mintForwarder_, address payable royaltyForwarder_)  external initializer {
        __ERC721_init("Test", "TS");
   ...
   modifier tokenExists(uint256 tokenId_) {
    ...
    modifier validMint() {
        _;
        if (totalSupply() >= MAX_SUPPLY - reservedSupply)
            revert erc721Error.QuantityExceedsMaxSupply();
    ...
    function BatchMint(address[] memory _minter) public payable validMint{
    ...
    function mintReserved(address receiver_)
    external onlyOwner
    validMint
    {
        if (reservedSupply == 0)
            revert erc721Error.QuantityExceedsReservedSupply();
