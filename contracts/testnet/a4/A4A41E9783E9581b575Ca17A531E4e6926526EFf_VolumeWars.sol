...
interface IDifficultyAdjustment {
    function getAdjustedOdds() external returns (uint16 grade2, uint16 grade3, uint16 grade4);
}

interface IWinDestination {
    function receiveWinnerReward(address teamTokenAddress) external payable;
}
...
contract VolumeWars is OwnableUpgradeable, ProxySafeVRFConsumerBase  {
    ...
    address public difficultyAdjustmentContract;

    // Current season's reward for the winning team
    uint256 public winnerPool;
    address public defaultWinAddress;
    mapping (address => address) public winDestinations;
    // Can set burn and win destinations for all teams
    address public destinationController;

    // Exchanges to use for each team
    mapping (address => address) public swapRouters;

    // function initialize(
        // address _piggyToken, address _secondToken, address _router, 
        // address _coordinator, address _linkToken, bytes32 _hash, 
        // uint256 _fee, address _legendaryDividend, address nftAddress
    ...
    function setDifficultyAdjustmentContract(address _contract) external onlyOwner {
        difficultyAdjustmentContract = _contract;
    }

    function setReferralSettings(address _referralAddress, uint256 _referralPercentage) external onlyOwner {
        require(_referralPercentage < 10, "% too high");
        referralAddress = _referralAddress;
        referralPercentage = _referralPercentage;
    }

    function setBurnDestination(address teamAddress, address newDestination) external {
        require(msg.sender == destinationController || msg.sender == owner(), "Auth");
        burnDestinations[teamAddress] = newDestination;
    }

    function setWinDestination(address teamAddress, address newDestination) external {
        require(msg.sender == destinationController || msg.sender == owner(), "Auth");
        winDestinations[teamAddress] = newDestination;
    }

    function setSwapRouter(address teamAddress, address newSwap) external {
        require(msg.sender == destinationController || msg.sender == owner(), "Auth");
        swapRouters[teamAddress] = newSwap;
    }

    function setDefaultWinDestination(address newDestination) external onlyOwner {
        defaultWinAddress = newDestination;
    }

    function setDestinationController(address newDestinationController) external onlyOwner {
        destinationController = newDestinationController;
    }
    ...
    function distributeLegendaryDividends() external onlyOwner {
        require(legendaryPool > 0, "Empty");
        require(legendaryDividend.totalSupply() > 0, "No stakers");
    ...
    function openSeasonInternal() internal {
    ...
    function closeSeasonInternal() internal {
        open = false;
        ...
        if (winnerPool > 0) {
            IWinDestination winDestination = IWinDestination(payable(defaultWinAddress));
            if (winDestinations[winningTeam] != address(0)) {
                winDestination = IWinDestination(payable(winDestinations[winningTeam]));
            }
            uint256 wonAmount = winnerPool;
            winnerPool = 0;
            winDestination.receiveWinnerReward{value: wonAmount}(winningTeam);
        }
    ...
    function switchSeason() external {
        ...
        require(seasonExpiration < block.timestamp, "Season not expired");
        closeSeasonInternal();
        openSeasonInternal();
        (uint16 grade2, uint16 grade3, uint16 grade4) = IDifficultyAdjustment(difficultyAdjustmentContract).getAdjustedOdds();
        rareChance = RareChance({
            grade2: grade2,
            grade3: grade3,
            grade4: grade4
        });
        emit RareChanceSet(address(difficultyAdjustmentContract), grade2, grade3, grade4);
    }
    ...
    function withdrawAllDevETH(address payable _to) external {
        ...
        require(msg.sender == feeDestination, "Auth");
    ...
    function join(address teamTokenAddress) external {
        ...
        // Allow switching team only if the team is disabled
        require(players[msg.sender].team == address(0) || (teams[players[msg.sender].team].enabled == false), "Already joined");
    ...
    function attack(address team, bytes calldata referralCode, bool referral, uint256 amountOutMin) external payable {
        ...
        require(block.timestamp < seasonExpiration, "Season has expired");

        // Taking fees
        uint256 burnedAmount = msg.value / 2; // 50% gets burned in the attack
        legendaryPool += msg.value / 4; // 25% goes to the reward pool
        winnerPool += (msg.value * 15) / 100; // 15% goes to the winner

        // Take referral from devCut
        uint256 referralCut = 0;
        if (referral && referralAddress != address(0)) {
            referralCut = (msg.value * referralPercentage) / 100;
            IReferral(referralAddress).process{value: referralCut}(referralCode);
        }

        // Remaining (10% - referral) goes to the devs
        devPool += msg.value 
        - (burnedAmount + referralCut + (msg.value / 4) + ((msg.value * 15) / 100));

        // Attack

        // The team's corresponding token
        uint256 initialETHBalance = address(this).balance - burnedAmount;
        swapEthForTokensAndBurn(burnedAmount, amountOutMin); // Buy tokens for ETH
        require(initialETHBalance == address(this).balance, "VW ETH changed");
    ...
    function claimBoosterPacks(uint256 amountOutMin) external payable {
        require(players[msg.sender].unclaimedPacks.length > 0, "No b. packs");
        require(msg.value == redeemFee, "Fee required");
        if (autoSwapLink) {
            swapEthForLINK(msg.value, amountOutMin);
        } else {
    ...
    function assignNFTs(uint8 numCommon, bool getRare, uint256 seed, uint8 nonceIncrement) private {
        ...
        require(numCommon <= 3, "Too many com");
        if (getRare) {
            nonce +=1;
            // Mint Rare NFT
    ...
    function setRareChance(uint16 grade2, uint16 grade3, uint16 grade4) external onlyOwner {
        rareChance = RareChance({
            grade2: grade2,
            grade3: grade3,
            grade4: grade4
    ...
    function swapEthForTokensAndBurn(uint256 ethAmount, uint256 amountOutMin) private {
        IUniswapV2Router02 router = pancakeSwapRouter;
        if (swapRouters[players[msg.sender].team] != address(0)) {
            router = IUniswapV2Router02(swapRouters[players[msg.sender].team]);
        }
        ...
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}(
            amountOutMin,// get anything we can
            path,
            sendTo,
            block.timestamp
        );
    ...
    // @dev Swap tokens for eth
    function swapEthForLINK(uint256 ethAmount, uint256 amountOutMin) private {
        // generate the testSwap pair path of token -> weth
        address[] memory path = new address[](2);
