...
contract SCVault is Ownable, SCVaultI {
    ...
    event LoanPayed(address _borrower, uint256 _repayAmount, uint256 _remainingPrinciple, uint256 _remainingInterest);
    event StableCoinWithdraw(address _lender, uint256 _amountWithdrawn, uint256 _amountOfWarpBurnt);
    ...
    function accrueInterest() public {
        ...
        totalReservesNew =
            (reserveFactorMantissa * interestAccumulated) +
            reservesPrior;
        ...
        returns (uint256)
    {
        ...
        BorrowSnapshot storage borrowSnapshot = accountBorrowsSnapshot[_account];
    ...
    function lendToWarpVault(uint256 _amount) external {
        require(scToken.balanceOf(msg.sender) >= _amount, "Not enough Stable Coin to lend");
    ...
    function redeem(uint256 _amount) external {
        require(principalBalance[msg.sender] >= _amount, "You redeem more than you lent");
        bool success = scToken.transferFrom(address(this), msg.sender, _amount);
        if(success) {
            principalBalance[msg.sender] -= _amount;
        ...
        returns (bool)
    {
        ...
        accountBorrows = borrowBalancePrior(_account);
        accountBorrowsNew = accountBorrows + _amount;
        totalBorrowsNew = totalBorrows + _amount;
        accountBorrowsSnapshot[_account].principal = accountBorrowsNew;
        accountBorrowsSnapshot[_account].interestIndex = borrowIndex;
        totalBorrows = totalBorrowsNew;

        bool success = scToken.transferFrom(address(this), _account, _amount);
    ...
    function repayBorrow(uint256 _repayAmount) external returns (bool) {
        ...
        require(accountBorrows >= _repayAmount, "Trying to pay back more than you owe");
        if(_repayAmount == 0) {
        ...
        require(scToken.balanceOf(msg.sender) >= repayAmount, "Not enough stable coin to repay");
        bool success = scToken.transferFrom(msg.sender, address(this), repayAmount);
        if(success) {
            ...
            emit LoanPayed(msg.sender, repayAmount, accountBorrowsSnapshot[msg.sender].principal, accountBorrowsSnapshot[msg.sender].interestIndex);
        }
...
contract LocalOracle is Ownable {
    ...
    constructor(address[] memory lpAddress, address scAddress) {
        ...
        lpPrice[LPaddress[0]] = 1;
        lpPrice[LPaddress[1]] = 2;
        lpPrice[LPaddress[2]] = 3;
    ...
    function getTokenPricewithAmount(address _Token, uint256 _amount) external view returns (uint256) {
        if(_Token == SCaddress) {
            return scPrice * _amount;
        }
        else {
            return (getLPPrice(_Token) * _amount);
        }
    }
}
...
contract TriflePrototype2 is Ownable {
    ...
    function createSCVault(
        ...
        emit SCVaultCreated(_scName, newSCVaultAddress, _scToken);
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 count = 0; count < amountSCVault; count++) {
            ...
            uint256 accountBorrowed = scVaultInstance.borrowBalanceCurrent(_account);
    ...
    function viewAllCollateralValueAndPrice() public view returns (uint256[] memory, uint256) {
        uint256 amountLPVault = viewAmountLPVault();
    ...
    function viewAllCollateralValueAndPriceByAccount(address _account) public view returns (uint256[] memory, uint256) {
        uint256 amountLPVault = viewAmountLPVault();
        uint256[] memory collateralByVault = new uint256[](amountLPVault);
        uint256 totalCollateral;
        for (uint256 count = 0; count < amountLPVault; count++) {
            ...
            // uint256 assetPrice = oracle.getLPPrice(lpToken);
            uint256 assetPrice = localOracle.getLPPrice(lpToken);
            uint256 accountCollateral = lpVaultInstance.viewCollateralAmountByAccount(_account);
            uint256 collateralPrice = assetPrice * accountCollateral;
    ...
    function viewAllBorrowValueAndPrice() public view returns (uint256[] memory, uint256) {
        uint256 amountSCVault = viewAmountSCVault();
    ...
    function viewAllBorrowValueAndPriceByAccount(address _account) public view returns (uint256[] memory, uint256) {
        uint256 amountSCVault = viewAmountSCVault();
        uint256[] memory borrowByVault = new uint256[](amountSCVault);
        uint256 totalBorrow;
        for (uint256 count = 0; count < amountSCVault; count++) {
            ...
            // uint256 assetPrice = oracle.getTokenPrice(scToken);
            uint256 assetPrice = localOracle.getTokenPrice(scToken);
            uint256 accountBorrow = scVaultInstance.borrowBalancePrior(_account);
    ...
    function viewAllSCValueAndPrice() public view returns (uint256[] memory, uint256) {
        uint256 amountSCVault = viewAmountSCVault();
        uint256[] memory borrowByVault = new uint256[](amountSCVault);
        ...
        returns (uint256)
    {
        ...
        for (uint256 count = 0; count < amountSCVault; count++) {
            ...
            // uint256 assetPrice = oracle.getTokenPrice(scToken);
            uint256 assetPrice = localOracle.getTokenPrice(scToken);
            uint256 accountBorrowed = scVaultInstance.borrowBalancePrior(_account);
            uint256 borrowedPrice = assetPrice * accountBorrowed;
            totalBorrowed += borrowedPrice;
        }
...
contract LPVault is Ownable, LPVaultI {
    ...
    function withdrawCollateral(uint256 _amount) external returns (bool) {
        ...
        collateralizedLP[msg.sender] -= _amount;
        totalCollateral -= _amount;
        bool success = lpToken.transferFrom(address(this), msg.sender, _amount);
    ...
    {   
        ...
        bool success = lpToken.transferFrom(
            address(this),
            _liquidator,
            amount
        );
