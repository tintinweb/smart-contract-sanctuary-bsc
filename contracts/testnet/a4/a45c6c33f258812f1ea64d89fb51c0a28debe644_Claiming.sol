...
contract Claiming is Ownable {
    ...
    function reserveTokens(address businessAddress, uint256 amount) public onlyOwner
    {
    ...
    function sendBonuses(Business[] memory bonuses) public onlySigner {
        for (uint i = 0; i < bonuses.length; i++) {
            sendBonus(bonuses[i].addr, bonuses[i].amount);
        }
    }

    function sendBonus(address user, uint256 amount) public onlySigner {
        require(reserved[user].amount >= amount, "Insufficient amount of tokens in user");
        require(token.balanceOf(address(this)) >= amount,"Insufficient amount of tokens in contract");

        uint256 old = reserved[user].amount;
        reserved[user].amount = reserved[user].amount.sub(amount);
        token.approve(address(this), amount);
        if (!token.transferFrom(address(this), user, amount)) {
            reserved[user].amount = old;
            emit UnsuccessfulTransfer(msg.sender);
        } else {
            reserved[user].amount = reserved[user].amount.sub(amount);
    ...
    function totalreservedTokens() public view onlyOwner returns (uint256) {
        return totalReserved;
    }

    function getUserBalance() public view returns (Reserve memory) {
        return reserved[msg.sender];
    }

    function getReserved(address user) public view onlyOwner returns (Reserve memory) {
        return reserved[user];
    }

    function getAllBusinesses() public view onlyOwner returns(Business[] memory) {
        Business[] memory ret = new Business[](addresses.length);
        for (uint i = 0; i < addresses.length; i++) {
            Reserve memory ut = reserved[addresses[i]];
            ret[i] = Business(ut.amount, addresses[i]);
        }
        return ret;
    }
