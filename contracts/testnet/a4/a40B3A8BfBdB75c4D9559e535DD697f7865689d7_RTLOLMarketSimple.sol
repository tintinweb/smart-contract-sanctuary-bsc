...
contract RTLOLMarketSimple is AccessControl, ReentrancyGuard {
    ...
    constructor(address admin) {
        ...
        SWAP_HASH = keccak256(
            "Swap(address user,address listerOffererAddress,Item sellerItem,Item currency,uint256 pricePerToken,uint256 deadline,bool swapType,uint256 userNonce)Item(address contractAddr,uint256 itemId)"
        );
    ...
    // bytes32 listingHash = keccak256(address userAddress, Item sellerItem, Item currency, uint256 pricePerToken, uint256 deadline, bool swapType);
    ...
    function swap(SwapParams memory _params) external nonReentrant {
        ...
        //do tranfers here
        address sellerAddress = _params.swapType
            ? msg.sender
            : _params.listerOffererAddress;
        address buyerAddress = !_params.swapType
            ? msg.sender
            : _params.listerOffererAddress;
        ...
        require(
            sellerItemContract.contractAddr != address(0),
            "Contract not found"
        );
        if (sellerItemContract.ercVer == 20) {
            // erc20
            IERC20(sellerItemContract.contractAddr).transferFrom(
                sellerAddress,
                buyerAddress,
                _params.purchaseQuantity
            );
        } else if (sellerItemContract.ercVer == 721) {
            // erc721
            IERC721(sellerItemContract.contractAddr).safeTransferFrom(
                sellerAddress,
                buyerAddress,
                _params.sellerItem.itemId,
                ""
            );
        } else if (sellerItemContract.ercVer == 1155) {
            // erc1155
            // todo: implement safebatchtranfer instead of single id tranfers
            IERC1155(sellerItemContract.contractAddr).safeTransferFrom(
                sellerAddress,
                buyerAddress,
                _params.sellerItem.itemId,
                _params.purchaseQuantity,
        ...
        require(
            currencyContract.contractAddr != address(0),
            "Contract not found"
        );
        require(currencyContract.ercVer == 20, "ERC20 currency required");
        // erc20
        IERC20(currencyContract.contractAddr).transferFrom(
            buyerAddress,
            sellerAddress,
            _params.purchaseQuantity * _params.pricePerToken
        );

        emit Swap(sellerAddress, buyerAddress, _params.swapType, currNonce);
    }
