...
contract IGOFilledSubscribe is Ownable, ReentrancyGuard {
    ...
    function getSubscribePayAmount(uint256 _subscribeAmount) public view returns (uint256) {
        return payTokenAmount.div( subscribeTokenAmount.div(_subscribeAmount) );
    }
    ...
    function getUserSubscribeAmount(address _user) public view returns (uint256, uint256, uint256) {
        ...
        return (_userPreemptionSubscribeAmount, _userSubscribeAmount, userSubscribeAmountTotal[_user]);
    }

    // get user claim amount
    function getUserCanClaimAmount(address _user) public view returns (uint256, uint256, uint256) {
        ...
        return (_claimAmount, _periodCurrent, _periodTotal);
    }
    ...
    function claimSubscribe() public nonReentrant {
        require(checkOutputIsStart(), "CLAIM_NOT_GOING");
        require(checkUserSubscribeStatus(msg.sender), "USER_NOT_SUBSCRIBE");

        (uint256 _claimAmount, uint256 _periodCurrent, uint256 _periodTotal) = getUserCanClaimAmount(msg.sender);

        require(_claimAmount > 0, "USER_CLAIM_ERROR");
        ...
        if ( checkOutputIsEnd() && checkUserPledgeStatus(msg.sender) ){
            // output is end && user already pledge
