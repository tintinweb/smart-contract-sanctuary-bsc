...
contract MultiLogic is LogicUpgradeable {
    ...
    function __MultiLogicProxy_init(address _storage) public initializer {
    ...
    modifier onlyStorage() {
        require(msg.sender == storageContract, "M2");
        _;
    }
        ...
        onlyOwnerAndAdmin
    {
        ...
        for (index = 0; index < _count; ) {
            sumAvailable += tokenAvailableLogic[_token][multiStrategy[index].logicContract];
        ...
        if (sumAvailable > 0) {
            for (index = 0; index < _count;) {
                tokenAvailableLogic[_token][multiStrategy[index].logicContract] = sumAvailable * _percentages[index] / 10000;
            }
        }
    }
    ...
    function setStrategy(uint256 _index, singleStrategy memory _multiStrategy)
        ...
        onlyOwnerAndAdmin
    {
        require(_index < multiStrategyLength);
    ...
    function takeToken(uint256 _amount, address _token) external {
        ...
        require(IStorage(storageContract)._isUsedToken(_token), "M5");
        ...
        onlyStorage
    {
        ...
        for (uint256 i = 0; i < _count; i++) {
            uint256 logicBalance = _getTokenBalance(
                _token,
                multiStrategy[i].logicContract
            );
            if (logicBalance < _amount) {
                IStrategyContract(multiStrategy[i].strategyContract)
                    .releaseToken(logicBalance, _token);
                ...
                tokenAvailableLogic[_token][multiStrategy[i].logicContract] -= logicBalance;
                tokenBalanceLogic[_token][multiStrategy[i].logicContract] += logicBalance;                
            } else {
                IStrategyContract(multiStrategy[i].strategyContract)
                    .releaseToken(_amount, _token);
                tokenBalanceLogic[_token][
                    multiStrategy[i].logicContract
                ] -= _amount;
                IERC20Upgradeable(_token).safeTransferFrom(
                ...
                tokenAvailableLogic[_token][multiStrategy[i].logicContract] -= _amount;
                tokenBalanceLogic[_token][multiStrategy[i].logicContract] += _amount;
            }
            _amount -= logicBalance;
            if (_amount <= 0) break;
        }
    ...
    function setLogicTokenBalance(
        ...
        for (uint256 i = 0; i < _count; ) {
            ...
            if (_deposit_withdraw == 1) {
                ...
                //withdraw
                if (tokenAvailableLogic[_token][logicAddress] < cleftAmount)
                    cleftAmount = tokenAvailableLogic[_token][logicAddress];
                tokenAvailableLogic[_token][logicAddress] -= cleftAmount;
            }
            unchecked {
                ++i;
     ...
     */
    function addEarn(uint256 _amount, address blidToken) external {
        require(isExistLogic(msg.sender), "M4");

        IERC20Upgradeable(blidToken).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function getTotalDeposit() external view returns (uint256) {
        IStorage(storageContract).getTotalDeposit();
    }
