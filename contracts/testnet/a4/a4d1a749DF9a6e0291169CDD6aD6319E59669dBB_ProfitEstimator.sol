...
contract ProfitEstimator is Ownable {
	...
	function setExchance(address _sotaExchangeContract) external onlyOwner {
		sotaExchangeContract = _sotaExchangeContract;
	}
	...
	function profitToCreator(
		...
		if (buyPriceInUSD > _lastBuyPriceInUSD) {
			uint256 totalSell = buyPriceInUSD.mul(_amount).mul(ZOOM_FEE - xUser).div(ZOOM_FEE);
			uint256 totalBuy = _lastBuyPriceInUSD.mul(_amount).mul(ZOOM_FEE + xUser).div(ZOOM_FEE);
