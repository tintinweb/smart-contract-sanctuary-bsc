...
contract MBNFTStaking is Ownable, ReentrancyGuard, IERC721Receiver {
    using SafeERC20 for IERC20;

    uint public totalNFTSupply;
    uint public lastRewardBlock;
    address[] public listRewardTokens;
    IMBNFT public nftToken;
    ...
    function pendingReward(address _user) external view returns (address[] memory, uint[] memory) {
        ...
        for(uint i = 0; i < listRewardTokens.length; i++){
            ...
            rewards[i] = (_accTokenPerShare / 1e12) - rewardDebt[_user][curToken];
        }
        return (listRewardTokens, rewards);
    }
    ...
    function _updateRewardDebt(address _user) internal {
        for(uint i = 0; i < listRewardTokens.length; i++){
            rewardDebt[_user][listRewardTokens[i]] = rewardTokens[listRewardTokens[i]].accTokenPerShare / 1e12;
        }
    }
    ...
    function _withdrawReward() internal {
        ...
        for(uint i = 0; i < _listRewardTokens.length; i++){
            ...
            uint pending = curRewardToken.accTokenPerShare / 1e12 - rewardDebt[msg.sender][_listRewardTokens[i]];
            if(pending > 0){
                curRewardToken.rewardsForWithdrawal -= pending;
                rewardDebt[msg.sender][_listRewardTokens[i]] = curRewardToken.accTokenPerShare / 1e12;
    ...
    function stake(uint[] calldata tokensId) public nonReentrant {
        ...
        for(uint i = 0; i < tokensId.length; i++){
            ...
            nftToken.safeTransferFrom(msg.sender, address(this), tokensId[i]);
        ...
        //     totalNFTSupply += depositedRobiBoost;
        // }
    ...
    function unstake(uint[] calldata tokensId) public nonReentrant {
        ...
        for(uint i = 0; i < tokensId.length; i++){
            ...
            for(uint j = 0; j < user.stakedTokensId.length; j++){
                if(tokensId[i] == user.stakedTokensId[j]){
                    ...
                    nftToken.safeTransferFrom(address(this), msg.sender, tokensId[i]);
                    findToken = true;
                    totalNFTSupply -= 1;
                    break;
                }
        ...
        //     totalNFTSupply -= withdrawalRBAmount;
        //     _updateRewardDebt(msg.sender);
        // }
    ...
    function emergencyUnstake() public {
        ...
        totalNFTSupply -= userInfo[msg.sender].stakedTokensId.length;
