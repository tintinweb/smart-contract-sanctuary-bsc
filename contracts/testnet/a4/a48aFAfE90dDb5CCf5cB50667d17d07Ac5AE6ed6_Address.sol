...
contract TokenMultiTimelock is Ownable {
    ...
    constructor (IBEP20 token, string memory label) public {
        _token = token;
        _label = label;
    }
    ...
    function release(uint256 number) public view virtual returns (string memory) {
        ...
        if(number > 0 && number <= _releases.length) {
            ...
            if(r.fromTimestamp == r.toTimestamp) {
                rStr = string(abi.encodePacked(rStr, " at ", _dtString(r.fromTimestamp), " UTC"));
            } else {
                rStr = string(abi.encodePacked(rStr, " linear from ", _dtString(r.fromTimestamp), " UTC"));
    ...
    function claim() public virtual {
        ...
        (, , , uint256 claimable) = w(msg.sender);
