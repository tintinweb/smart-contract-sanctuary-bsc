...
contract admin is Ownable, IAdmin {
    ...
    uint256 public  initialMintFee;

    address payable public erc20RewardToken;

    uint256 public rwardPerMint;
    ...
    constructor() {
        ...
        initialMintFee = 0.001 ether; // in ETh
    }
    ...
    function setRewardPerMint(uint _fee) external onlyOwner{

        rwardPerMint=_fee;
    }

    function setERC20TokenReward(address payable _token) external onlyOwner{

        require(_token!=address(0),"token can't be zero address");
        erc20RewardToken=_token;
    }

    function setInitialRegFee(uint _fee) external onlyOwner{
