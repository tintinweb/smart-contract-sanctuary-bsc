...
contract TraderDAO is Ownable{
    ...
    mapping(uint => uint) public UserRealizedAmount;
    ...
    function DistributeReward(uint _poolid, uint[] memory _ids, uint[] memory _amounts, uint _poolRealizedAmount) external onlyOwner{
        ...
        for(uint i = 0; i< _ids.length; i++) {
            ...
            uint finalAmount = _amounts[i];
            if(_poolRealizedAmount > PoolTradeAmount[_poolid]){
                uint profit = 0;
                if(i == _ids.length - 1){
                    uint stakeamount = UserStakeAmount[_ids[i]] - PointerBalance;
                    profit = ProfitToShare(_amounts[i], stakeamount);
                }
                else{
                    profit = ProfitToShare(_amounts[i], UserStakeAmount[_ids[i]]);
                }
                
                USDT.safeTransfer(PlatformAccount, profit);
                finalAmount = finalAmount - profit;
            }

            UserRealizedAmount[_ids[i]] = UserRealizedAmount[_ids[i]] + finalAmount;

            if(UserReinvestStatus[UserList[_ids[i]]] == true){
                Insert(UserList[i], finalAmount);
            }
            else{
                UserAmountToClaim[UserList[_ids[i]]] = UserAmountToClaim[UserList[_ids[i]]] + finalAmount;
            }
        }
