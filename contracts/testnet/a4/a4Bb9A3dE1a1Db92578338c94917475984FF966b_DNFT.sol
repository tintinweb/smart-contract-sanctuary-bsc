    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant MAX_SALE_AMOUNT = 2500;
    ...
    function deployNewSalePhase(
        ...
        // require(_salePhase >= 1 && _salePhase <= 4, "DNFT: INVALID SALE PHASE");
        // require(_salePhase == currentSalePhase + 1, "DNFT: INVALID SALE PHASE");
        // require(
        //     _startTime >= block.timestamp && _startTime >= salePhaseStatistics[currentSalePhase].endTime,
        //     "DNFT: INVALID START TIME"
        // );
        require(_endTime > _startTime, "DNFT: INVALID END TIME");
        require(_priceInBUSD > 0, "DNFT: INVALID PRICE");
        require(_priceAfter24Hours > 0, "DNFT: INVALID PRICE AFTER 24 HOURS");
        require(
            _maxAmountUserCanBuy > 0 && _maxAmountUserCanBuy <= MAX_SALE_AMOUNT,
            "DNFT: INVALID MAX AMOUNT USER CAN BUY"
        );
    ...
    function temporaryMerge(
        ...
        require(verifyMergeSignature(msg.sender, _sessionId, _signature), "DNFT: INVALID SIGNATURE");
        require(!isMergeSessionIdExist[_sessionId], "DNFT: SESSION ID IS USED");
    ...
    function cancelTemporaryMerge(
        ...
        require(verifyMergeSignature(msg.sender, _sessionId, _signature), "DNFT: INVALID SIGNATURE");
        string memory mergeSet = generateMergeSet(_tokenIds);
        require(isMergeSessionIdExist[_sessionId], "DNFT: INVALID SESSION ID");
    ...
    function verifyMergeSignature(
        ...
        return (Whitelist.verifySignatureWhenMergeDNFT(signer, _to, _sessionId, _signature));
    }
    ...
    function getUserBuyAmount(uint8 _salePhase) external view returns (uint256) {
        return salePhaseStatistics[_salePhase].userBuyAmount[msg.sender];
    }
}