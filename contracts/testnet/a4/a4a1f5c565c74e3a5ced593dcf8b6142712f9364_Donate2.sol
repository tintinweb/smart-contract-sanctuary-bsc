...
contract Donate2 is Ownable {
    ...
    constructor() {
        usdt = IBEP20(0x7C7C38b9caE1A0ae847048F43A9E818fD4608EB4); // USDT bsc
        token = IBEP20(0x74276D2B0061cB0E07b382B1680665946d830BC9); // token bsc
    ...
    function withdrawReward() public {
        require(rewardsMap[msg.sender].rewardPaid < rewardsMap[msg.sender].balances.mul(2), "no balance");
    ...
    function mining(uint256 price, uint256 ts) public {
        delete removeRewardsIndex;
        if (ts == 0){
            ts = block.timestamp;
        }
        // require(DaysDiff(lastMiningTimestamp, ts) > 0, "Do not repeat mining");
        lastMiningTimestamp = ts;
        
        for (uint256 i = 0; i < rewardsMapIndex.length; i++) {
            UserRewards memory ur = rewardsMap[rewardsMapIndex[i]];
            uint256 count = rewardsMap[ur.account].count;
            ...
            if (count > 60) {
                ...
                uint256 dao = 0;
                if(count > 0 && count <= 20) {
                    dao = ur.balances.mul(50).div(1000).div(price);
                } else if(count > 20 && count <=60){
                    dao = ur.balances.mul(25).div(1000).div(price);
                }
                if (dao != 0) {
                    rewardsMap[ur.account].reward = rewardsMap[ur.account].reward.add(dao);
                    rewardsMap[ur.account].count = rewardsMap[ur.account].count.add(1);
                    emit Mining(
                        ur.account,
                        rewardsMap[ur.account].count,
                        dao,
                        rewardsMap[ur.account].reward,
                        rewardsMap[ur.account].rewardPaid,
                        rewardsMap[ur.account].timestamp
                    );
                }
            }
        }
