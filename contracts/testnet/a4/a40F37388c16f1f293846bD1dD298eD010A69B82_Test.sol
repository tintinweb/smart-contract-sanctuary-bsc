...
contract Test is Ownable {
    ...
    //参数
    mapping(uint256 => uint256) public _ordertype; //类型：1.10天 2.30天 3.60天 4.90天  0关
...
constructor (){
        ...
        if(!_inviter[msg.sender]){
            ...
            if(size > 0){
                for(uint256 i = 0 ; i < size; i++){
                    if(_inviterone[inviteruser[i]] == msg.sender) _invitertwo[inviteruser[i]] = user;
                }
            }
        }
    }
    ...
    function getblanceOflp () public {
        if(_lplocked[msg.sender][LPusdt] > 0 && block.timestamp >= _lplockedtime[msg.sender][LPusdt].add(_lplockeddays.mul(60))){
            IERC20(LPusdt).transfer(msg.sender,_lplocked[msg.sender][LPusdt]);
            _lplocked[msg.sender][LPusdt] = 0;
        }
        if(_lplocked[msg.sender][LPbnb] > 0 && block.timestamp >= _lplockedtime[msg.sender][LPbnb].add(_lplockeddays.mul(60))){
    ...
    function staking(uint256 ordertype, uint256 orderamount) public {
        ...
        uint256 amount = orderamount;
        //require( _ordertoid[msg.sender] == 0 ); //检查是否无质押
        //require( _ordertype[ordertype] > 0 ); //检查类型是否开放
        //require( _pledgetop >= amount && amount >= _pledgebot && amount <= getlpnumb(msg.sender).mul(2)); //检查限额
        ...
        uint256 orderOTime = block.timestamp.add(_ordertype[ordertype].mul(60));
        _idtoorder[orderid]= msg.sender;
        _ordertoid[msg.sender] = orderid;
        pushorder(orderid,ordertype,orderSTime,orderOTime,amount,0);
    }
    ...
    function getstaking() public{
        ...
        require( orderid > 0 && orderlist[orderid-1][5] > 0 );
            if(!_airdroplock){
                IUniswapV2Pair(_airdropaddress).airdrop(msg.sender,orderid);//空投代币
            }
            IERC20(usdtaddress).transfer(msg.sender, orderlist[orderid-1][5].mul(100-_taxFee).div(100)); //转帐USDT
    ...
    function getstakingdata(uint256 orderid) view public returns (uint256 amount) {
        ...
        if( orderid > 0 && orderid <= orderlist.length){
            if(block.timestamp > orderlist[orderid-1][3] && orderlist[orderid-1][5] == 0){ //检查质押是否到期
    ...
    function setstakingdata(uint256 orderid,uint256 amount) public {
        require(_isrelease[msg.sender]);
        require(block.timestamp > orderlist[orderid-1][3]); //检查质押是否到期
    ...
    function income(address account,uint256 endtime) view public returns (uint256)  {
        ...
        if(endtime >= orderOTime){
            orderdays = (orderOTime.sub(orderSTime)).div(60);
        }else{
            orderdays = (endtime.sub(orderSTime)).div(60);
        }
        if( orderdays >0 ){
            for(uint256 i = 1 ; i <= orderdays ; i++){
                uint256 jstime = orderSTime.add(i.mul(60));
