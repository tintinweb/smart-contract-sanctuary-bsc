...
contract SBT is ISBT, IERC5192, ERC721, IERC721Receiver, Ownable {
    ...
    function _safeMint(address to, uint256 tokenId) internal virtual override(ERC721) {
        _safeMint(to, tokenId, "");
    ...
    function onERC721Received(
        ...
        require(_originalNFTAddress == msg.sender, "msg.sender is not matched with registered original nft address");

        // burn received NFT
        IERC721(msg.sender).transferFrom(address(this), address(0x000000000000000000000000000000000000dEaD), tokenId);

        // mint SBT
        _safeMint(from, tokenId);

        // copy token uri
        _setTokenURI(tokenId, IERC721Metadata(msg.sender).tokenURI(tokenId));
