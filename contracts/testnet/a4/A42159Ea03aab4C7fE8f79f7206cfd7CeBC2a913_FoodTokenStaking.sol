...
contract FoodTokenStaking is Ownable {
    ...
    // IPair public lp = IPair(0x639bf1d9F3a683B28369708596aEfe38F2988e9C); // Main
    IPair public lp = IPair(0x162B1c499DF8Ca4f3caE97FB9960151CBAAE9677); // Test
    ...
    function stake(uint256 _amount, uint8 _plan) public {
        ...
        uint256 tokenAmount = getTokenForLP(_amount);
        uint256 _reward = tokenAmount.mul(rewardMultiplier[_plan]).div(
            rewardDivider
        );
    ...
    function withdraw(uint256 _index) public {
        ...
        user.totalWithdrawanLp = user.totalWithdrawanLp.add(_amount).add(
            _reward
        );
        totalWithdrawanLp = totalWithdrawanLp.add(_amount).add(_reward);
    ...
    function getTokenReserve() public view returns (uint256) {
        (uint256 token0Reserve, uint256 token1Reserve, ) = lp
            .getReserves();
    ...
    function SetLpRewards(uint256[] memory _rewrad, uint256 _divider)
        external
        onlyOwner
    {
    ...
    function SetMinLp(uint256 _amount) external onlyOwner {
        minLp = _amount;
    }
