...
contract Governance is Admin{
    ...
    function createProposal(string memory _proposalID, string memory _title, string memory _description) external{
        ...
        emit newProposal(_proposalID,msg.sender,_title,_description,initialProposal, block.timestamp);
    }
    function executeProposal(string memory _proposalID) external onlyValidator() returns (uint256){
        require(proposalid[_proposalID], "ProposalID none available");
    ...
    function activeDeposit(string memory _proposalID, uint256 _status) external onlyAdmin(){
        ...
        if(_status == 2){
            // refund
            safeGovernanceTransfer(proposalActive.proposer, proposalActive.initial);     
        }
        if(_status == 3){
    ...
    function ConvertStringToBytes32(string memory _text) external view returns(bytes memory){
        return abi.encode(_text);
    }

    function ConvertBytes32ToString(bytes calldata _text) external view returns(string memory){
        return abi.decode(_text, (string));
    }
}
