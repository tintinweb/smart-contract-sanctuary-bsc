...
contract ShadowFiNFTStaking is ERC721Holder, Ownable  {
    ...
    // The DKYC NFT TOKEN!
    IShadowFiNFT public SDFNFTToken;
    ...
    // Info on each user's total redeemed rewards.
    mapping (address => mapping (address => uint256)) public userTotalRedeemed;
    ...
    function add(
        ...
        require(_nftLevel >= 1, "invalid nftLevel");
    ...
    function pendingRewardsToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            uint256 reward = multiplier.mul(pool.allocPoint).div(BLOCKS_PER_DAY);
            accTokenPerShare = accTokenPerShare.add(reward);
        }
    ...
    function allPendingRewardsToken(address _user) external view returns (uint256[] memory) {
        ...
        for(uint256 _pid = 1; _pid <= length; _pid++) {
            ...
            rewards[_pid - 1] = user.amount.mul(accTokenPerShare).sub(user.rewardDebt);
        }
        return rewards;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(pool.allocPoint).div(BLOCKS_PER_DAY);
        pool.accTokenPerShare = pool.accTokenPerShare.add(reward);
        pool.lastRewardBlock = block.number;
    }
    ...
    function joinPool(uint256 _pid, uint256 _tokenId) external {
        ...
        require(SDFNFTToken.nftLevel(_tokenId) >= pool.nftLevel, "level mismatch");
        ...
        SDFNFTToken.safeTransferFrom(msg.sender, address(this), _tokenId);
    ...
    function joinPoolBatch(uint256 _pid,  uint256[] memory _tokenIds) external {
        ...
        for (uint i = 0 ; i < amount; i++) {
            ...
            require(SDFNFTToken.nftLevel(_id) >= pool.nftLevel, "nftLevel mismatch");
            SDFNFTToken.safeTransferFrom(msg.sender, address(this), _id);
    ...
    function leavePool(uint256 _pid, uint256 _id) external {
        ...
        SDFNFTToken.safeTransferFrom(address(this), msg.sender, _id);
    ...
    // Leave NFT items.
    function leavePoolForUser(uint256 _pid, uint256 _id, address _account) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        SDFNFTToken.safeTransferFrom(address(this), _account, _id);
    ...
    function leavePoolBatchForUser(uint256 _pid, uint256[] memory _ids, address _account) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_account];
    ...
    function getUserInfo(uint256 _pid, address _account) public view returns(uint256 amount, uint256 rewardDebt, uint256[] memory) {
        UserInfo storage user = userInfo[_pid][_account];
