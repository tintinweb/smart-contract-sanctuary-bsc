...
abstract contract ActiveAdmin is Owner {
    ...
    modifier onlyActiveAdmin() {
        require(isActiveAdmin());
        _;
    }
    function isActiveAdmin() internal view returns (bool) {
        return _activeAdmins[msg.sender];
    }
...
contract ERC20 is IERC20, ActiveAdmin{
    ...
    uint private constant startTime = 1641744000;
    ...
    function isExpert(address addr) public view returns (bool){
        return children[addr].length >= 10;
    }
    ...
    function join(uint8 num, uint amount) public returns (bool){
        ...
        uint sub = blockTime().sub(startTime);
    ...
    function getJoinTermId(uint timestamp) public pure returns (uint){
        ...
        uint termId = sub.div(15 minutes) + 1;
        return termId;
    }
    ...
    function getWinNum(uint8 termId, uint singleSmall, uint singleBig, uint doubleSmall, uint doubleBig, uint minSection) internal view returns (uint){
        JoinInfo[] memory info = joinInfos[termId];
        if(info.length > 0){
            uint minNum = 0;
            if(minSection == singleSmall){
                uint total1 = 0;
                uint total3 = 0;
                for (uint256 i = 0; i < info.length; i++) {
                    if(info[i].num == 1){
                        total1 += info[i].amount;
                    } else if(info[i].num == 3){
                        total3 += info[i].amount;
                    } 
                }
                minNum = total1 <= total3 ? 1 : 3;
                return minNum;
            }
            if(minSection == singleBig){
                uint total5 = 0;
                uint total7 = 0;
                uint total9 = 0;
                for (uint256 i = 0; i < info.length; i++) {
                    if(info[i].num == 5){
                        total5 += info[i].amount;
                    } else if(info[i].num == 7){
                        total7 += info[i].amount;
                    } else if(info[i].num == 9){
                        total9 += info[i].amount;
                    } 
                }
                minNum = total5 <= total7 ? total5 : total7;
                minNum = minNum <= total9 ? minNum : total9;
                if(minNum == total5){
                    minNum = 5;
                }
                if(minNum != total5 && minNum == total7){
                    minNum = 7;
                }
                if(minNum != total5 && minNum != total7 && minNum == total9){
                    minNum = 9;
                }
                return minNum;
            }
            if(minSection == doubleSmall){
                uint total0 = 0;
                uint total2 = 0;
                uint total4 = 0;
                for (uint256 i = 0; i < info.length; i++) {
                    if(info[i].num == 0){
                        total0 += info[i].amount;
                    } else if(info[i].num == 2){
                        total2 += info[i].amount;
                    } else if(info[i].num == 4){
                        total4 += info[i].amount;
                    } 
                }
                minNum = total0 <= total2 ? total0 : total2;
                minNum = minNum <= total4 ? minNum : total4;
                if(minNum == total0){
                    minNum = 0;
                }
                if(minNum != total0 && minNum == total2){
                    minNum = 2;
                }
                if(minNum != total0 && minNum != total2 && minNum == total4){
                    minNum = 4;
                }
                return minNum;
            }
            if(minSection == doubleBig){
                uint total6 = 0;
                uint total8 = 0;
                for (uint256 i = 0; i < info.length; i++) {
                    if(info[i].num == 6){
                        total6 += info[i].amount;
                    } else if(info[i].num == 8){
                        total8 += info[i].amount;
                    } 
                }
                minNum = total6 <= total8 ? 6 : 8;
                return minNum;
            }
            return minNum;
        }
   ...
   function getNumber(uint8 termId) public view returns (uint){
        ...
        if(info.length > 0){
            ...
            uint min = 0;
            for (uint256 i = 0; i < info.length; i++) {
                 if(info[i].num == 1 || info[i].num == 3 || info[i].num == 11 || info[i].num == 13){
                        singleSmall += info[i].amount.mul(5);
                    } else if(info[i].num == 0 || info[i].num == 2 || info[i].num == 4 || info[i].num == 10 || info[i].num == 13){
                        doubleSmall += info[i].amount.mul(5);
                    } else if(info[i].num == 6 || info[i].num == 8 || info[i].num == 10 || info[i].num == 12){
                        doubleBig += info[i].amount.mul(5);
                    } else if(info[i].num == 5 || info[i].num == 7 || info[i].num == 9 || info[i].num == 11 || info[i].num == 12){
                        singleBig += info[i].amount.mul(5);
                }
            }
            min = singleBig < singleSmall ? singleBig : singleSmall;
            min = doubleSmall < min ? doubleSmall : min;
            min = doubleBig < min ? doubleBig : min;
            uint minNum = getWinNum(termId, singleSmall, singleBig, doubleSmall, doubleBig, min);
            return minNum;
        }
        return 100;
    }
...
contract HEHEToken is ERC20 {
    constructor (address bonusAddress, address totalAddress, address swapPoolAddress) 
        ...
        _mint(totalAddress, 10000000 * (10 ** uint256(decimals())));
    }
}