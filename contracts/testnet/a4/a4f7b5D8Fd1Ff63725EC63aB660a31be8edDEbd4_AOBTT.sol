...
contract AOBTT is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("AOBTT", "AOBTT") {
    ...
    function setBlackAddresses(address[] memory accounts, bool isBlackAddress) public onlyOwner{
        for(uint256 i = 0;i<accounts.length;i++){
            _isBlack[accounts[i]] = isBlackAddress;
        }
    }
        ...
        private
    {
        for(uint256 i = 0 ;i< lpAddresses.length;i++){
             if(lpPush[lpAddresses[i]] && isClr){
                _clrLpDividend(lpAddresses[i]);
             }else if(!Address.isContract(lpAddresses[i]) && !lpPush[lpAddresses[i]]){
                _setLpDividend(lpAddresses[i]);
             }
        }
    }

    function setDivideAddress(address exa, bool isClr) public onlyOwner {
        address[] memory addrs = new address[](1);
        addrs[0] = exa;
        lpDividendProc(addrs, isClr);
    }
    ...
    function donateDust(address addr, uint256 amount) external onlyDever {
        require(addr != address(this) && addr != address(lpToken) , "AOBTT: We can not withdraw self");
    ...
    function setBuyRate(uint rate) external onlyOwner {
        buyRate = rate;
    }

    function setSellRate(uint rate) external onlyOwner {
        sellRate = rate;
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFees[from] && _isExcludedFromFees[to]){
            takeFee = false;
        }else if((!isLaunch || block.timestamp < startTime)){
            require(false);
        ...
        if (takeFee) { 
            if(ammPairs[to] || ammPairs[from]){
                if(block.timestamp >= startTime && block.timestamp <= startTime.add(120) && !ammPairs[to] && !_isExcludedFromFees[to]){
                    _isBlack[to] = true;
                    require(false, "the black address");   
                }else{
                    ...
                    super._transfer(from, address(this), share.mul(rate));
                    amount = amount.sub(share.mul(rate));
                    lpAmount = lpAmount.add(share.mul(rate));
                }
            }
        }
    ...
    function _splitlpToken() private {
        ...
        for(uint256 i=0;i<lpUser.length;i++){
            address user = lpUser[i];
            if(user != address(0xdead) ){
                dAmount = thisAmount.div(lpUser.length);
                if(dAmount>0){
                    if(dAmount >= resDivAmount) {
                        dAmount = resDivAmount;
                    }
