...
contract Zap is IZap, Ownable {
    ...
    event ZapInERC20(address fromToken, address toToken, uint zapAmount, uint receiveAmount);

    event ZapOutBNB(address lpToken, address token, uint lpTokenAmount, uint tokenAmount , uint bnbAmount);
    event ZapOutERC20(address lpToken, address token0, address token1, uint lpTokenAmount, uint token0Amount , uint token1Amount);
    ...
    */
    ...
    /*
     * initialize the addrees list of ERC20 token (not LP token)
     */
    function initialize(address psr, address pan, address usdt, address busd) onlyOwner public {
        ...
        setDirectRoutePairAddress(keccak256(abi.encodePacked(psr, wbnb)), wbnb);
        setDirectRoutePairAddress(keccak256(abi.encodePacked(psr, usdt)), usdt);
        setDirectRoutePairAddress(keccak256(abi.encodePacked(psr, busd)), busd);
        setDirectRoutePairAddress(keccak256(abi.encodePacked(pan, wbnb)), wbnb);
        setDirectRoutePairAddress(keccak256(abi.encodePacked(pan, usdt)), usdt);
        setDirectRoutePairAddress(keccak256(abi.encodePacked(pan, busd)), busd);
    ...
    function getRouter() external view override returns (ISwapRouter02) {
        return ROUTER;
    }
    ...
    function zapInToken(address fromToken, uint amount, address toToken) external override notPaused {
        uint fromTokenBalanceBefore = IERC20(fromToken).balanceOf(address(this));
        ...
        if (isLpToken(toToken)) {
            ...
            if (fromToken == token0 || fromToken == token1) {
                ...
                uint otherTokenBalanceBefore = IERC20(otherToken).balanceOf(address(this));

                liquidity = _swapAndAddLiquidity(fromToken, otherToken, amount, msg.sender);

                uint fromTokenBalanceAfter = IERC20(fromToken).balanceOf(address(this));
                ...
                uint otherTokenBalanceAfter = IERC20(otherToken).balanceOf(address(this));
        ...
        emit ZapInERC20(fromToken, toToken, amount , liquidity);
    }
    ...
    function zapOut(address fromToken, uint amount) external override notPaused {
        ...
        if (!isLpToken(fromToken)) {
            ...
            if (token0 == wbnb || token1 == wbnb) {
                ...
                emit ZapOutERC20(fromToken, token0, token1, amount, amountA , amountB);
            }
        }
    }
    ...
    */
    function _addLiquidity(address token0, address token1, uint amount, address receiver) private returns (uint liquidity) {
        uint token0BalanceBefore = IERC20(token0).balanceOf(address(this));
        uint token1BalanceBefore = IERC20(token1).balanceOf(address(this));
        ...
        uint token0BalanceAfter = IERC20(token0).balanceOf(address(this));
        uint token1BalanceAfter = IERC20(token1).balanceOf(address(this));
