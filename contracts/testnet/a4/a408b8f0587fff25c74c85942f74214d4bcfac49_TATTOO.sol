...
contract TATTOO is Context, IBEP20, Ownable {
    ...
    address public MarketingWallet = 0xF87DFBa2dC449C260bB702b21B713d606cd79929;
    address public CharityWallet = 0x13c248b1e08b8233189e7ce88d6dC0e04A0d76CF;
    address public OwnerWallet = 0x0Af787f02b5b59d97F0Ab947F9a6a293c315854A;
    ...
    string private constant _name = "TATTOO TOKEN";
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _MarketingFee==0 && _CharityFee==0 && _OwnerFee==0) return;
    ...
    function setMarketingWallet(address newMarkeitngWallet,address newCharityWallet,address newOwnerWallet) external onlyOwner() {
        require(newMarkeitngWallet != address(0) && newCharityWallet != address(0) && newOwnerWallet != address(0), "You can't set Zero Address");
        MarketingWallet = newMarkeitngWallet;
        CharityWallet = newCharityWallet;
        OwnerWallet = newOwnerWallet;
    }

    function setTaxFeePercentage(uint256 taxFee,uint256 liquidityFee,uint256 CharityFee,uint256 MarketingFee,uint256 OwnerFee) external onlyOwner() {
       require(taxFee.add(liquidityFee).add(CharityFee).add(MarketingFee).add(OwnerFee) <= 2500,"You can't set more ");
        _taxFee = taxFee;
        _previousTaxFee = _taxFee;
        ...
        emit TaxFeeUpdated(taxFee.add(liquidityFee).add(CharityFee).add(MarketingFee).add(OwnerFee));
    }
   function setMaxTxPercentage(uint256 maxTxPercentage) external onlyOwner() {
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tCharity = tAmount.mul(_CharityFee).div(feeDenominator);
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tOwner = tAmount.mul(_OwnerFee).div(feeDenominator);
