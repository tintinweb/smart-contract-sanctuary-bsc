...
contract babyKongMaster is IERC721Receiver{
    ...
    constructor(){
        ...
        //createPool(250 ether);
        //createPool(500 ether);
        //createPool(1000 ether);
    }
...
contract babyKongPool is IERC721Receiver {
    ...
    function updateStakeArray(uint256 _stakeId) internal {
        //babyStake[] storage userStakesArray = userStakeLists[_userAddress];
    ...
    function setActionedTime(uint256 _days)  public {
        //nextActionTime = block.timestamp + (_days * 86400);
        //nextCycleAction = _action;
        userStakeV2[] memory allBabyStakes = kongV2.getUserAllStakeDetailsByAddress(address(this));
        for(uint i = 0; i < allBabyStakes.length; i++){
            userStakeV2 memory babyStakeV2 = allBabyStakes[i];
            //babyStake memory userStakeFromArrayDetails = userStakesById[i];
            kongV2.setActionedTime(babyStakeV2.id,_days, address(this));
        }
    }

    function setNextActionTime(uint256 _days)  public {
        //nextActionTime = block.timestamp + (_days * 86400);
        //nextCycleAction = _action;
        userStakeV2[] memory allBabyStakes = kongV2.getUserAllStakeDetailsByAddress(address(this));
        for(uint i = 0; i < allBabyStakes.length; i++){
            userStakeV2 memory babyStakeV2 = allBabyStakes[i];
            //babyStake memory userStakeFromArrayDetails = userStakesById[i];
            kongV2.setNextActionTime(babyStakeV2.id,_days, address(this));
        }
    }
    ...
    function compoundSingleStakes(uint256 _stakeId) public {
        kongV2.compound(_stakeId);
    }

    function claimSingleStakes(uint256 _stakeId) public {
