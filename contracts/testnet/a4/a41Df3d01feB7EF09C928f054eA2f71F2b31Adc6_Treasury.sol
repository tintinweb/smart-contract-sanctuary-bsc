...
contract Treasury is ContractGuard {
    ...
    uint256 public mainTokenPercentToSell = 5000; // 50%
    ...
    event SetMainTokenPercentToSell(uint256 oldValue, uint256 newValue);
    ...
    function initialize(
        ...
        address pairAddress = FACTORY.getPair(mainToken, stableToken);
        ...
        IMainToken(mainToken).grantRebaseExclusion(address(this)); // excluded
    ...
    function getEstimateLpAmountAddLp(address _stableToken, uint256 _mainTokenAmount, uint256 _stableTokenAmount) public view returns (uint256) {
        uint256 totalSupply = PAIR.totalSupply();
        (uint256 reserve0, uint256 reserve1, ) = PAIR.getReserves();
        address token0 = PAIR.token0();
        
        uint256 amount0 = _mainTokenAmount;
        uint256 amount1 = _stableTokenAmount;

        if (_stableToken == token0) {
            amount0 = _stableTokenAmount;
            amount1 = _mainTokenAmount;
        }

        uint256 liquidity = Math.min(amount0.mul(totalSupply).div(reserve0), amount1.mul(totalSupply).div(reserve1));
        return liquidity;
    }
    ...
    function getEstimatedReward(uint256 _pid) external view returns (uint256) {
        ...
        uint256 mainTokenReward = mainTokenCirculatingSupply.mul(percentage).div(100).div(10**STABLE_DECIMALS);
        uint256 expansionRate = calculateExpansionRate(mainTokenCirculatingSupply);
        mainTokenReward = mainTokenReward.mul(expansionRate).div(10000);
        if (mainTokenReward > 0) {
            uint256 mainTokenAmountToSell = mainTokenReward.mul(mainTokenPercentToSell).div(10000);
            uint256 stableTokenEstimated = getEstimateStableToken(mainTokenAmountToSell);
            uint256 lpEstimated = getEstimateLpAmountAddLp(stableToken, mainTokenReward.sub(mainTokenAmountToSell), stableTokenEstimated);
            // After allocate LP to DAO, POL
            lpEstimated = lpEstimated.mul(10000 - daoFundPercent - polFundPercent).div(10000);
            uint256 medalEstimated = ILPNode(medalPool).getEstimateMedalToken(lpEstimated);
            BoardroomInfo storage boardroomPool = boardroomInfo[_pid];
            if (boardroomPool.allocPoint > 0 && medalEstimated > 0) {
                uint256 boardroomRewardPercent = 10000 - devFundPercent;
    ...
    function _sellMainTokenAndAddLp() internal {
        uint256 mainTokenBalanceOf = IERC20(mainToken).balanceOf(address(this));
        ...
        // Sell mainToken
        uint256 mainTokenAmountToSell = mainTokenBalanceOf.mul(mainTokenPercentToSell).div(10000);
        _approveTokenIfNeeded(mainToken, mainTokenBalanceOf);
        ...
        if (stableTokenBalanceOf > 0) {
            // Add LP
            _approveTokenIfNeeded(stableToken, stableTokenBalanceOf);
            ROUTER.addLiquidity(
                mainToken,
                stableToken,
                mainTokenBalanceOf.sub(mainTokenAmountToSell),
            ...
            if (pairAmount > 0) {
                uint256 daoFundAmount = pairAmount.mul(daoFundPercent).div(10000);
                uint256 polFundAmount = pairAmount.mul(polFundPercent).div(10000);
                if (daoFundAmount > 0) {
    ...
    function _sendToBoardroom() internal {
        ...
        if (medalTokenBalanceOf > 0) {
            uint256 devFundAmount = medalTokenBalanceOf.mul(devFundPercent).div(10000);
            if (devFundAmount > 0) {
    ...
    function setMainTokenPercentToSell(uint256 _value) external onlyOperator {
        require(_value <= 10000, "Treasury: Max percent is 100%");
        emit SetMainTokenPercentToSell(mainTokenPercentToSell, _value);
        mainTokenPercentToSell = _value;
    }
