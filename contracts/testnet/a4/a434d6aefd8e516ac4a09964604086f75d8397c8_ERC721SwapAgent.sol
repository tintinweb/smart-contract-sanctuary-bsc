    ...
    ERC721Holder
{
    ...
    mapping(address => mapping(uint256 => address)) public supposedOwners;
    ...
    function swap(
        ...
        if (registeredToken[dstChainId][tokenAddr]) {
            ...
            for(uint256 i = 0; i < tokenIds.length; i++){
                supposedOwners[tokenAddr][tokenIds[i]] = msg.sender;
    ...
    function fill(
        ...
        if (bridgedTokenAddr != address(0x0)) {
            ...
            for (uint256 i = 0; i < tokenIds.length; i++){
                ...
                if (ownerAddress == address(0x0)){
                    ...
                    bridgedToken.safeTransferFrom(address(this), recipient, tokenIds[i]);
                }
            }
        ...
        if (registeredToken[fromChainId][fromTokenAddr]) {
            ...
            for (uint256 i = 0; i < tokenIds.length; i++){
                require(supposedOwners[fromTokenAddr][tokenIds[i]] == recipient, "Attempting to send token to the wrong owner");
