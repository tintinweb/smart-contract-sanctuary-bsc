...
contract ERC721Base is IERC721Base {
    ...
    //TODO should be used when transfer is activated
...
contract MinerNFT is ERC721Base, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyMunicipality() {
        require(msg.sender == municipalityAddress, "MinerNFT: Only authorized contracts can call this function");
        _;
    }
    ...
    function setLastMinerId (uint256 _lastMinerId) external onlyOwner {
        lastMinerId = _lastMinerId;
    }

    /// @notice Mint a given amount of miners
    function mintMiners(address _user, uint256 _count) external onlyMunicipality returns(uint256, uint256) {
    ...
    function mint(address user) public onlyMunicipality returns (uint256) {
