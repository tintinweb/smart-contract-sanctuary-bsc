...
contract TownToken is BEP20Burnable, Ownable{
    ...
    constructor(
        ...
        _maxTotalSupply = 250000000 * _DECIMALFACTOR;
    ...
    function excludeToFee(address account) external onlyOwner() {
    ...
    function updateFee(uint256 _txFee,uint256 _burnFee, uint256 _liquidityFee) onlyOwner() public{
        _TAX_FEE = _txFee; 
        _BURN_FEE = _burnFee;
        _LIQUIDITY_FEE = _liquidityFee;
        ORIG_TAX_FEE = _TAX_FEE;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(!inTransfer){            
            uint256 currentRate =  _getRate();
            (uint256 tFee, uint256 tBurn, uint256 tLiquid) =  _getFee(amount);
            uint256 tTransferAmount = amount.sub(tFee).sub(tBurn).sub(tLiquid);
            ...
            if (_isExcluded[sender] || _isExcluded[recipient]) {
               ...
               super._transfer(sender, recipient, amount);
                ...
                if(tFee > 0){
                    super._transfer(sender, FeeAddress, tFee);
                }

                if(tLiquid > 0){
                    super._transfer(sender, address(this), tLiquid);
                }
                
                if(tBurn > 0){
                    _burn(burnAddress, tBurn);
                }
                ...
                _rTotal = _rTotal.sub(tFee.mul(currentRate)).sub(tBurn.mul(currentRate)).sub(tLiquid.mul(currentRate));
    ...
    function _getFee(uint256 tAmount) private view returns(uint256, uint256, uint256) {        
        uint256 tFee = tAmount.mul(_TAX_FEE).div(100);
        uint256 tBurn = tAmount.mul(_BURN_FEE).div(100);
        uint256 tLiquid = tAmount.mul(_LIQUIDITY_FEE).div(100);
    ...
    function mint(address account, uint256 amount) public virtual onlyOwner {
        require(_totalSupply < _maxTotalSupply, "The Contract Reached the limit of tokens to be minted.");
