...
interface IxMeb {
    function mintByMebStake(address account, uint256 amount) external;

    function burnByMebStake(address account, uint256 amount) external;
}
...
contract MebStake is Ownable {
    ...
    address public xMeb = 0x93f480b3325a5818818914f399964B48ebB5fcD3;
    address public meb = 0x1FFCF7F610fc615d0dc2d104B25a8bce452a66F0;
    uint256 public startTimestamp = block.timestamp;
    uint256 public endTimestamp = block.timestamp + 30 * 24 * 3600;
    uint256 public userDepositInterval = 12 * 3600; //质押时间间隔
    ...
    struct PoolInfo {
        uint256 allocPoint;
        uint256 lockSecond;
        uint256 xMebExchangeRate;
        uint256 lastRewardTimestamp;
        uint256 accAwardPerShare;
        uint256 totalAmount;
    }

    uint256 public punishMulti = 1.5 * 1e18;
    ...
    function add(
        ...
        poolInfos.push(PoolInfo({allocPoint: _allocPoint, lockSecond: _lockSecond, xMebExchangeRate: _xMebExchangeRate, lastRewardTimestamp: lastRewardTimestamp, accAwardPerShare: 0, totalAmount: 0}));
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (pool.lockSecond == 0) {
            ...
            userDepositInfos[msg.sender][_pid].push(
                UserDepositInfo({
                    amount: _amount, //
                    rewardDebt: _amount.mul(pool.accAwardPerShare).div(1e18),
                    totalAward: 0,
        ...
        IERC20(meb).safeTransferFrom(msg.sender, address(this), _amount);
        pool.totalAmount = pool.totalAmount.add(_amount);
        IxMeb(xMeb).mintByMebStake(msg.sender, _amount.mul(poolInfos[_pid].xMebExchangeRate).div(1e18));
        poolInfos[_pid] = pool;
    }
    ...
    function withdraw(
        ...
        PoolInfo memory pool = updatePool(_pid);
        if (pool.lockSecond == 0) {
            IxMeb(xMeb).burnByMebStake(msg.sender, _amount.mul(poolInfos[_pid].xMebExchangeRate).div(1e18));
            ...
            require(depositInfo.lockTimestampUtil <= block.timestamp || _force, "The deposit is not unlock");

            uint256 xMebAmount = _amount.mul(poolInfos[_pid].xMebExchangeRate).div(1e18);
            if (depositInfo.lockTimestampUtil > block.timestamp) {
                uint256 treasuryAmount = xMebAmount.mul(punishMulti).div(1e18).sub(xMebAmount);
                IERC20(xMeb).safeTransferFrom(msg.sender, treasury, treasuryAmount);
            }
            IxMeb(xMeb).burnByMebStake(msg.sender, xMebAmount);
    ...
    function harvest(uint256 _pid) public {
        ...
        if (pool.lockSecond == 0) {
            ...
            if (_pending > 0) {
                userCurrentInfos[msg.sender].totalAward = userCurrentInfos[msg.sender].totalAward.add(_pending);
                IERC20(meb).safeTransfer(msg.sender, _pending);
            }
        } else {
            ...
            if (totalPedding > 0) {
                IERC20(meb).safeTransfer(msg.sender, totalPedding);
            }
        }
    }
    ...
    function harvestBatch() public {
        ...
        for (uint256 i = 0; i < poolInfosMemory.length; i++) {
            if (poolInfosMemory[i].lockSecond == 0) {
                uint256 accumulatedAward = userCurrentInfos[msg.sender].amount.mul(poolInfosMemory[i].accAwardPerShare).div(1e18);
                uint256 _pending = accumulatedAward.sub(userCurrentInfos[msg.sender].rewardDebt);
                userCurrentInfos[msg.sender].rewardDebt = accumulatedAward;
                if (_pending > 0) {
                    userCurrentInfos[msg.sender].totalAward = userCurrentInfos[msg.sender].totalAward.add(_pending);
