...
contract Diamond is ERC20, Ownable {
    ...
    function distributeTokensEth() private {
        ...
        if (thisBalance() > minContractTokensToSwap) {
            uint256 partialTokensToSell = thisBalance()
                .mul(partialSellPercentage)
                .div(100);

            uint256 tokensForLiquidity = partialTokensToSell.mul(autoLP).div(
                100
            );
            uint256 halfLiquidity = tokensForLiquidity.div(2);

            uint256 tokensForSwap = partialTokensToSell.sub(halfLiquidity);

            swapTokensForEth(tokensForSwap);

            uint256 totalEth = address(this).balance.sub(initialETHBalance);
            ...
            //if there is remaining balance add to eth for liquidity
            uint256 remainingEthForFees = totalEth.sub(devFeesToSend).sub(
                marketingFeesToSend
            );
            ethForAddLP = ethForAddLP.add(remainingEthForFees);

            sendEthToWallets(devFeesToSend, marketingFeesToSend);

            if (ethForAddLP > 0 && halfLiquidity > 0) {
