...
contract ConfirmBracket is ReentrancyGuard, Ownable {
    ...
    modifier onlyPlearnLucky() {
        require(msg.sender == address(plearnLucky), 'Not Plearn Lucky');
        _;
    }
    ...
    function confirmTicket(
        ...
        require(plearnLucky.getLuckyStatus(_luckyId) == Status.Claimable, 'Lucky not claimable');
        require(_isConfirmable(_luckyId, _ticketNumber), 'Ticket cannot be confirmed yet');

        (uint256 ticketId, address owner, ) = plearnLucky.getTicketInfo(_luckyId, _ticketNumber);
    ...
    function setAvailablesBracket(uint256 _luckyId, uint32[] memory _avilablesBracket) external onlyOwner {
        require(plearnLucky.getLuckyStatus(_luckyId) == Status.Open, 'Lucky not Open');
    ...
    function _isConfirmable(uint256 _luckyId, uint256 _ticketNumber) private view returns (bool) {
        bool isConfirmable = false;
        uint32[] memory luckyNumbers = plearnLucky.getLuckyNumbers(_luckyId);
        for (uint256 i = 0; i < luckyNumbers.length; i++) {
            if (_ticketNumber == luckyNumbers[i]) {
                isConfirmable = true;
                break;
            }
        }

        if (!isConfirmable && plearnLucky.isEnableReserve(_luckyId)) {
            uint32[] memory reserveNumbers = plearnLucky.getReserveNumbers(_luckyId);
            for (uint256 i = 0; i < reserveNumbers.length; i++) {
                if (_ticketNumber == reserveNumbers[i]) {
                    isConfirmable = true;
                    break;
                }
            }
    ...
    //     //     bytes4(keccak256('getCurrentLuckyId()')) // `anotherfunction()` arguments
    //     // );
    //     (bool success, bytes memory data) = plearnLucky.call(
    //         //abi.encodePacked(bytes4(keccak256('startNumber()')))
    //         abi.encodeWithSignature('startNumber()')
    //     );
    //     uint32 startNumber = abi.decode(data, (uint32));
    //     console.log('Jozzee success: %s', success);
    //     console.log('Jozzee startNumber: %s', startNumber);
    // }

    // function testGetLuckyInfo(address _plearnLucky, uint256 _luckyId) external payable {
    //     console.log('Lucky Plearn Address: %s', _plearnLucky);
    //     // bytes memory data = abi.encodePacked(
    //     //     bytes4(keccak256('getLuckyInfo(uint256)')), // function signature
    //     //     abi.encode(_luckyId) // `anotherfunction()` arguments
    //     // );

    //     bytes memory data = abi.encodePacked(abi.encodeWithSignature('getLuckyInfo(uint256)', _luckyId));
    //     (bool success, bytes memory result) = _plearnLucky.call(data);
    ...
    //     ) = abi.decode(
    //             result,
    //             (uint8, uint256, uint256, uint256, uint256, uint32, uint32, uint32, uint32, uint256, uint256, uint256)
    //         );

    //     console.log('Lucky Info V0: %s', v0);
    //     console.log('Lucky Info V1: %s', v1);
    //     console.log('Lucky Info V2: %s', v2);
    //     console.log('Lucky Info V3: %s', v3);
    //     console.log('Lucky Info V4: %s', v4);
    //     console.log('Lucky Info V5: %s', v5);
    //     console.log('Lucky Info V6: %s', v6);
    //     console.log('Lucky Info V7: %s', v7);
    //     console.log('Lucky Info V8: %s', v8);
    //     console.log('Lucky Info V9: %s', v9);
    //     console.log('Lucky Info V10: %s', v10);
    //     console.log('Lucky Info V1: %s', v11);
    // }
}
