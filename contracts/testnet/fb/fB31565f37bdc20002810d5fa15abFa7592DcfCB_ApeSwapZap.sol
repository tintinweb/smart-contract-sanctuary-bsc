...
contract ApeSwapZap {
    ...
    /// @param _paths Path from input token to LP token0
    ...
    function _zap(
        ...
        if (_lpTokens[0] != address(_inputToken)) {
            require(
                _paths[0][0] == address(_inputToken),
                "ApeSwapZap: wrong path _path0[0]"
            );
            require(
                _paths[0][_paths[0].length - 1] == _lpTokens[0],
        ...
        if (_lpTokens[1] != address(_inputToken)) {
            require(
                _paths[1][0] == address(_inputToken),
                "ApeSwapZap: wrong path _path1[0]"
            );
            require(
                _paths[1][_paths[1].length - 1] == _lpTokens[1],
    ...
    function zapFarm(
        ...
        IERC20 lp = IERC20(factory.getPair(_lpTokens[0], _lpTokens[1]));
        lp.approve(address(masterApe), liquidity);
        masterApe.deposit(_farmPid, liquidity);
    }
