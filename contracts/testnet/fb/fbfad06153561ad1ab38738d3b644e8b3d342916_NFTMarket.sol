    ...
    ReentrancyGuardUpgradeable
{
    ...
    IERC20Upgradeable public fulfillmentToken;
    ...
    function initialize(IERC20Upgradeable _fulfillmentToken)
        public
        initializer
    {
        ...
        fulfillmentToken = _fulfillmentToken;
    }
    ...
    function addItemToMarket(
        ...
        require(
            fulfillmentToken.balanceOf(msg.sender) >= listingFee,
            "You require to have enough ERC20 tokens to pay the listing fee"
        );

        fulfillmentToken.transferFrom(msg.sender, address(this), listingFee);
        ...
        newMarketItem.timeBought = 0;
        newMarketItem.isSold = false;
        newMarketItem.buyer = address(0);
        newMarketItem.price = _price;
        newMarketItem.seller = msg.sender;
        newMarketItem.itemID = newItemID;
        newMarketItem.tokenID = _tokenID;
        newMarketItem.NFTContract = _NFTContract;
    ...
    function buyFromMarket(uint256 _itemID, address _NFTContract)
        external
        nonReentrant
    {
        require(
        ...
        require(
            fulfillmentToken.balanceOf(msg.sender) >= price,
            "You require to have enough ERC20 tokens to purchase the NFT"
        );

        uint256 tokenID = marketItems[_itemID].tokenID;
        address seller = marketItems[_itemID].seller;

        fulfillmentToken.transferFrom(msg.sender, seller, price);
        IERC721Upgradeable(_NFTContract).safeTransferFrom(
            address(this),
            msg.sender,
            tokenID
    ...
    function removeFromMarketplace(uint256 _itemID, address _NFTContract)
        external
        nonReentrant
    {
        require(
    ...
    function getUnsoldMarketItems(
        uint256 _paginationIndex,
        uint256 _paginationLimit
    ) external view returns (MarketItem[] memory unsoldItems) {
        uint256 index = 0;
        uint256 totalMarketItems = getTotalMarketItems();
        uint256 numUnsoldItems = getTotalUnsoldItems();

        require(
            _paginationIndex >= 0 && _paginationLimit <= totalMarketItems,
            "Pagination filters provided are invalid"
        );

        unsoldItems = new MarketItem[](numUnsoldItems);
        for (uint256 i = _paginationIndex; i < _paginationLimit; i++) {
            if (
