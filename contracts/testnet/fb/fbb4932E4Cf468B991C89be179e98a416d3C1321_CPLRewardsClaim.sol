...
contract CPLRewardsClaim {
    ...
    function claim(
        ...
        require(gameExpiryMap[gameId] != 0 && block.timestamp <= gameExpiryMap[gameId], "Claims for this game has ended");

        bytes32 _leaf = keccak256(abi.encode(wallet, amount, address(token)));

        require(isClaimable(root, _leaf, proof) && (claims[wallet] == false), "No claims available");
    ...
    function withdrawTokensFromContract(address payable walletAddress, IBEP20 token, uint256 amount) public payable onlyOwner {
        _distribute(walletAddress, token, amount);
    }

    // function contractHasEnoughTokensForTransfer(address token, uint256 amount) public view returns (bool){
    //     if (token == ETHER) {
    ...
    function setClaim(address walletAddress, bool claimStatus) external onlyOwner {
    ...
    function _distribute(address payable winner, IBEP20 token, uint256 amount) private {
        if (token == BNB_TOKEN_ADDRESS) {
