...
import './EnftyMarketStorageV3.sol';

contract EnftyMarketV3 is
  EnftyMarketStorageV3,
  ERC721Holder,
  ERC1155Holder,
  Ownable,
  ReentrancyGuard
{
  ...
  event OrderUpdate(uint256 indexed orderId, uint256 price);
  ...
  constructor(address _adminAddress, address _treasuryAddress)
    EnftyMarketStorageV3(_adminAddress, _treasuryAddress)
  {}

  function addPayToken(address _token) public onlyOwner {
    _payTokenSet.add(_token);
  }

  function addCollectionPayToken(address _collection, address _token)
    public
    onlyOwner
  {
    require(
    ...
    _collectionPayTokenSet[_collection].add(_token);
  }
   ...
   * @param _tokenCount: count of the NFT
   * @param _payToken: token to pay
  ...
  function createOrder(
    ...
    require(_tokenCount > 0, 'Collection: Token count must greater than zero');
    require(_price > 0, 'Collection: Price must greater than zero');
    require(
      _payTokenSet.contains(_payToken) ||
        _collectionPayTokenSet[_collection].contains(_payToken),
      'PayToken: Not supported'
    );

    uint256 _orderId = _orderIndex;
    ...
    if (_collections[_collection].collectionType == CollectionType.ERC1155) {
      ...
      require(_tokenCount == 1, 'Collection: Token count must equal to one');
   ...
   */
  function modifyOrder(uint256 _orderId, uint256 _newPrice)
    external
    nonReentrant
  {
    Order storage order = _orders[_orderId];

    // Verify the order is existed
    require(order.tokenCount != 0, 'Order: Not found');
    require(order.seller == msg.sender, 'Order: Caller is not the seller');

    // Verify collection is accepted
    require(
      _collections[_orders[_orderId].collection].status ==
    ...
    // Emit event
    emit OrderUpdate(_orderId, _newPrice);
  }

  /**
   * @notice Cancel existing order
   * @param _orderId: contract address of the NFT
   */
  function cancelOrder(uint256 _orderId) external nonReentrant {
    ...
    if (
      _collections[order.collection].collectionType == CollectionType.ERC1155
    ) {
   ...
   * @param _orderId: contract address of the NFT
   * @param _tokenCount: count of the NFT purchased
   */
  function buy(
    uint256 _orderId,
    uint256 _tokenCount,
    ...
    _buyToken(_orderId, _tokenCount, _payToken, _price, msg.value);
  }

  /**
   * @notice Buy token by matching the price of an existing ask order
   * @param _orderId: orderId
   * @param _tokenCount: count of the token to buy
   * @param _value: price (must match the askPrice from the seller)
   */
  function _buyToken(
    uint256 _orderId,
    uint256 _tokenCount,
    ...
    require(
      _collections[order.collection].status == CollectionStatus.Open,
      'Collection: Not for trading'
    );

    // Front-running protection
    require(_payToken == order.payToken, 'Buy: Incorrect payToken');
    ...
    require(
      _tokenCount > 0 && _tokenCount <= order.tokenCount,
      'Buy: Incorrect tokenCount'
    );
    ...
    uint256 toPayAmount = order.price.mul(_tokenCount);
    if (order.payToken == address(0)) {
      require(_value == toPayAmount, 'Buy: Incorrect value');

      // Transfer BNB to seller
      payable(order.seller).transfer(sellerIncome);
    } else {
      require(_value == 0, 'Buy: Incorrect price');

      // Transfer ERC20 to constract
      uint256 tradingAndCreatorFee = tradingFee.add(creatorFee);
      if (tradingAndCreatorFee > 0) {
        IERC20(order.payToken).safeTransferFrom(
          address(msg.sender),
          address(this),
          tradingAndCreatorFee
        );
      }
    ...
    // Record creatorFee
    if (creatorFee > 0) {
      address creatorAddress = _collections[order.collection].creatorAddress;
      pendingRevenue[creatorAddress][order.payToken] = pendingRevenue[
        creatorAddress
      ][order.payToken].add(creatorFee);
    }

    // Record tradingFee
    if (tradingFee > 0) {
      pendingRevenue[treasuryAddress][order.payToken] = pendingRevenue[
        treasuryAddress
      ][order.payToken].add(tradingFee);
    }
    ...
    order.tokenCount = order.tokenCount - _tokenCount;
    ...
    if (order.tokenCount == 0) {
      delete _orders[_orderId];
    }
  }

  /**
   * @notice Claim pending revenue (treasury)
   */
  function claimPendingRevenue(address payToken) external nonReentrant {
    uint256 revenueToClaim = pendingRevenue[msg.sender][payToken];
    require(revenueToClaim != 0, 'Claim: Nothing to claim');

    pendingRevenue[msg.sender][payToken] = 0;
    if (payToken == address(0)) {
      payable(msg.sender).transfer(revenueToClaim);
    } else {
      IERC20(payToken).safeTransfer(msg.sender, revenueToClaim);
    }
    emit RevenueClaim(msg.sender, payToken, revenueToClaim);
  }

  /**
   * @notice Claim revenue locked by mistake
   */
  function claimByMistake(address payToken, uint256 revenueToClaim)
    external
    onlyAdmin
    nonReentrant
  {
  ...
  function addCollection(
    ...
    bool isERC721 = IERC721(_collection).supportsInterface(0x80ac58cd);
    bool isERC1155 = IERC1155(_collection).supportsInterface(0xd9b67a26);
    require(isERC721 || isERC1155, 'Operations: Not ERC721 or ERC1155');
    ...
    CollectionType _collectionType = CollectionType.ERC721;
    if (isERC1155) {
      _collectionType = CollectionType.ERC1155;
    }
  ...
  function modifyCollection(
    ...
    _collections[_collection] = Collection({
      status: CollectionStatus.Open,
      collectionType: _collections[_collection].collectionType,
      creatorAddress: _creator,
      tradingFee: _tradingFee,
      creatorFee: _creatorFee
    });
    ...
    emit CollectionUpdate(
      _collection,
      _collections[_collection].status,
      _collections[_collection].collectionType,
      _creator,
      _tradingFee,
      _creatorFee
    );
    ...
    )
  {
    ...
    return (_calculatePriceAndFeesForCollection(collection, count, price));
  }
  ...
  {
    uint256 totalAmount = _price.mul(_tokenCount);
    tradingFee = totalAmount.mul(_collections[_collection].tradingFee).div(
      10**4
    );
    creatorFee = totalAmount.mul(_collections[_collection].creatorFee).div(
      10**4
    );
    sellerIncome = totalAmount.sub(tradingFee).sub(creatorFee);

    return (sellerIncome, tradingFee, creatorFee);
  }
}
...
contract EnftyMarketStorageV3 {
  ...
  // For treasury to claim: user -> token address -> amount
  mapping(address => mapping(address => uint256)) public pendingRevenue;

  EnumerableSet.AddressSet internal _collectionAddressSet;
  EnumerableSet.AddressSet internal _payTokenSet;

  mapping(address => EnumerableSet.AddressSet) internal _collectionPayTokenSet;
  ...
  uint256 internal _orderIndex = 0;
