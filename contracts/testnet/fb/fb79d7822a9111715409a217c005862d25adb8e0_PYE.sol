...
contract PYE is IPYE, Context, Ownable {
    ...
    function depositLPFee(uint256 amount, address token) public onlyExchange {
        ...
        if(tokenIndex < pairsLength) {
            ...
            uint256 reflectionAmount = amount.sub(marketingFeeAmount).sub(mechanismFeeAmount);

            try IERC20(token).transferFrom(msg.sender, _defaultFees.marketingAddress, marketingFeeAmount) {} catch {}
            try IERC20(token).transferFrom(msg.sender, _defaultFees.mechanismAddress, mechanismFeeAmount) {} catch {}

            try IERC20(token).transferFrom(msg.sender, address(this), reflectionAmount) {
        ...
        if(amount > 0){
            ...
            for (uint j = 0; j < pairsLength; j++) {
                if(gasUsed < distributorGas) {
                    ...
                    if(tokenAmount > 0) {
                        try IERC20(token).transfer(keeper, tokenAmount) {
