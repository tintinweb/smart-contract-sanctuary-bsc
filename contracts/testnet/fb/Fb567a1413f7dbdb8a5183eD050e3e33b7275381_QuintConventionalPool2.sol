...
$$ |  $$\ $$ |  $$ |$$ |  $$ | \$$$  /  $$   ____|$$ |  $$ | $$ |$$\ $$ |$$ |  $$ |$$ |  $$ |$$  __$$ |$$ |      
\$$$$$$  |\$$$$$$  |$$ |  $$ |  \$  /   \$$$$$$$\ $$ |  $$ | \$$$$  |$$ |\$$$$$$  |$$ |  $$ |\$$$$$$$ |$$ |      
 \______/  \______/ \__|  \__|   \_/     \_______|\__|  \__|  \____/ \__| \______/ \__|  \__| \_______|\__|      
...
contract QuintConventionalPool2 is Ownable {
    using SafeMath for uint256;
    address payable public distributor;
    // OldStaking public staking = OldStaking(0x8FD50c8886dc91111E52BBFcb2685368c29Bdc8d); // Main
    OldStaking public staking =
        OldStaking(0xE5426892c2EAA7a1B29CC639Fa91FA9590F3A115); // Test
    ...
    uint256 public totalTokenStakes;
    uint256 public totalLpStakes;

    uint256 public tokenReward = 4765;
    uint256 public lpReward = 9513;
    ...
    uint256 public withdrawTaxPercent = 20;
    ...
    // mapping(address => TokenStakeV1) tokenStakeRecordV1;
    // mapping(address => LpStakeV1) lpStakeRecordV1;
    ...
    event MIGRATE(address indexed staker, uint256 indexed amount);
    ...
    function stake(uint256 _amount, uint256 _index) public {
        ...
        if (_index == 0) {
            ...
            stakeLp(msg.sender, _amount, preReward);
    ...
    function reStake(uint256 _index) public {
        ...
        if (_index == 0) {
            ...
            userStake.reward = userStake.reward.add(preReward);
            stakeLp(msg.sender, 0, preReward);
        }

        emit RESTAKE(msg.sender, preReward, _index);
    }
    ...
    function stakeLp(
        ...
        userStake.lpAmount = userStake.lpAmount.add(_lpAmount);
        userStake.amount = userStake.amount.add(getTokenForLP(_lpAmount)).add(
            _tokenAmount
        );
        userStake.time = block.timestamp;
        user.stakeCount++;
        user.totalStakedLp = user.totalStakedLp.add(_lpAmount);
        totalStakedLp = totalStakedLp.add(_lpAmount);
    }
    ...
    function claim(uint256 _index) public {
        ...
        user.totalWithdrawanToken = user.totalWithdrawanToken.add(preReward);
        totalWithdrawanToken = totalWithdrawanToken.add(preReward);

        emit CLAIM(msg.sender, preReward, _index);
    }
        ...
        if (_index == 0) {
            ...
            if (preReward > 0) {
                if (
                    ...
                    uint256 taxAmount = preReward.mul(withdrawTaxPercent).div(
                        100
                    );
    ...
    function migrate() public {
        (bool isExsits, , , , , ) = staking.getUserInfo(msg.sender);
        require(isExsits, "No stake found");
        require(!isMigrated[msg.sender], "Already migrated");
        uint256 preReward;
        uint256 amount;
        uint256 time;
        (amount, time, , ) = staking.userTokenStakeInfo(msg.sender);
        if (amount > 0) {
            uint256 rewardDuration = block.timestamp.sub(time);
            preReward = amount.mul(rewardDuration).mul(tokenReward).div(
                rewardDivider
            );

            if (tokenStakeRecord[msg.sender].startTime != 0) {
                tokenStakeRecord[msg.sender].startTime = block.timestamp;
            }
            totalTokenStakes++;
        }
        (, amount, time, , ) = staking.userLpStakeInfo(msg.sender);
        if (amount > 0) {
            uint256 rewardDuration = block.timestamp.sub(time);
            preReward = amount.mul(rewardDuration).mul(lpReward).div(
                rewardDivider
            );
        }
        require(preReward > 0, "No pending reward");
        token.transferFrom(distributor, address(this), preReward);
        stakeToken(msg.sender, preReward);
    ...
    // function getDataFromV1() public {
    //     (bool isExsits, , , , , ) = staking.getUserInfo(msg.sender);
    //     require(isExsits, "no stake found");
    //     TokenStakeV1 storage userTokenStake = tokenStakeRecordV1[msg.sender];
    //     (userTokenStake.amount, userTokenStake.time, , ) = staking
    //         .userTokenStakeInfo(msg.sender);

    //     LpStakeV1 storage userLpStake = lpStakeRecordV1[msg.sender];
    //     (
    //         userLpStake.lpAmount,
    //         userLpStake.amount,
    //         userLpStake.time,
    //         ,

    //     ) = staking.userLpStakeInfo(msg.sender);
    // }

    // staked amount = 99000000000000000000000
    // rewrad duration = 990591
    // reward multiplier = 460
    // rewrad divider = 1000000000000
        ...
        returns (uint256 _reward)
    {
        LpStake storage userStake = lpStakeRecord[_user];
        uint256 rewardDuration = block.timestamp.sub(userStake.time);
        _reward = userStake.amount.mul(rewardDuration).mul(lpReward).div(
            rewardDivider
        );
    }
    ...
    function SetMinAmount(uint256 _token, uint256 _lp) external onlyOwner {
        minToken = _token;
        minLp = _lp;
    }

    function SetWithdrawTaxAndDuration(uint256 _tax, uint256 _duration)
        external
        onlyOwner
    {
