...
contract WakandaBurn is BEP20 {
    ...
    string public constant name = "Wakanda Burn";
    string public constant symbol = "WKDB";
...
contract Staking is Context, Ownable, WakandaBurn {
    using SafeMath for uint256;

    address[] internal stakeholders;

    WakandaBurn public WKDB;
    ...
    constructor(WakandaBurn _address) public payable {
        WKDB = _address;
    }
    ...
    function createStake(uint256 _numberOfTokens) public payable returns (bool)
    {
        ...
        require(
            WKDB.totalSupply() >=(_numberOfTokens.mul(WKDB.decimalFactor().add(totalTokenStaked))),
            "addition error"
        );
        require(
            _mint(_msgSender(), _numberOfTokens.mul(WKDB.decimalFactor())),
            "mint error"
        );
        stakeholders.push(_msgSender());
        totalTokenStaked = totalTokenStaked.add(
            _numberOfTokens.mul(WKDB.decimalFactor())
        );
        ...
        returns (bool)
    {
        ...
        _burn(_msgSender(), _numberOfTokens.mul(WKDB.decimalFactor()));
        return true;
    }
