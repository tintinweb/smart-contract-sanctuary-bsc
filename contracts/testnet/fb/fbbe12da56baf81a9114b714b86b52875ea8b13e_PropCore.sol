...
contract PropCore is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable {
    ...
    uint public propId;    //道具的tokenId

    function initialize(address usdt_, address soulToken_, address soul721_, address soul1155_, address soulTeam_, address soulStore_, address soulController_, address prop1155_, address referAddr_) public initializer {
        __Divestor_init();
        ...
        soulStore : ISoulStore(soulStore_),
        soulController : ISoulController(soulController_),
        prop1155 : IProp1155(prop1155_),
        referAddr : referAddr_
        });
        propId = 1;
    }

    function setContracts(address soulToken_, address soul721_, address soul1155_, address soulTeam_, address soulStore_, address soulController_, address prop1155_, address referAddr_) public onlyOwner {
        externalAddr.soulToken = ISoulToken(soulToken_);
        externalAddr.soul721 = ISoul721(soul721_);
        externalAddr.soul1155 = ISoul1155(soul1155_);
        externalAddr.soulTeam = ISoulTeam(soulTeam_);
        externalAddr.soulStore = ISoulStore(soulStore_);
        externalAddr.soulController = ISoulController(soulController_);
        externalAddr.prop1155 = IProp1155(prop1155_);
        externalAddr.referAddr = referAddr_;
    }

    /*
     *  设置道具的ID
     **/
    function setPropId(uint propId_) public onlyOwner{
        propId = propId_;
    }
     ...
     *  购买道具
     **/
    function buyProp() public returns(bool){
        //查询用户是否拥有
        uint count = externalAddr.prop1155.balanceOf(_msgSender(), propId);
        require(count == 0, "Already owned");

        //支付soul, 具体数额需要再定
        externalAddr.soulToken.transferFrom(_msgSender(), address(this), 1 ether);

        //发放道具
        externalAddr.prop1155.mint(_msgSender(), propId, 1);
     ...
     *  group_ : 群体： 1-女，2-男
     **/
    function batchBuyBox(uint8 theme_, uint8 group_, uint amount_, address recommender_) public{
        //推荐人不能是自己
        require(recommender_ != address(0) && recommender_ != _msgSender(), "not own addr");

        //支付u
        externalAddr.usdt.safeTransferFrom(_msgSender(), address(this), amount_ * 4 ether);

        //给用户soul
        externalAddr.soulToken.transfer(_msgSender(), amount_ * 1 ether);

        uint8 count = randomCeil(2) == 1 ? 1 : _getIndex();
        uint partIndex = (theme_ * 10000) + (group_ * 100) + count;
        
        (uint[] memory tokenIds, uint[] memory  amounts) = externalAddr.soulStore.extractBoxBatch(partIndex, amount_);

        require(tokenIds.length > 0, "does not exist");
        for(uint i=0; i < tokenIds.length; i++){
            //查询tokenId的拥有者
            address caster = externalAddr.soul721.ownerOf(tokenIds[i]);
            if(caster == address(0)){
                caster = externalAddr.referAddr;
            }

            //分成
            externalAddr.soulController._divide(recommender_, caster, amounts[i]);

            //发放1155
            externalAddr.soul1155.safeTransferFrom(address(externalAddr.soulStore), _msgSender(), tokenIds[i], amounts[i], '');
        }
...
interface ISoulController {
    function _divide(address recommender_, address caster_, uint amount) external;
}
