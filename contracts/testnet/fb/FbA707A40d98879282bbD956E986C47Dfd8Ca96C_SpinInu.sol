...
contract SpinInu is Context, IERC20Metadata, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!isExcludedFromFees(sender) && !isExcludedFromFees(recipient) //Only take fee if both sender & receiver aren't exclude from fee
        && (address(_swapRouter) == sender || address(_swapRouter) == recipient)) {//Only take fee if it's a trade
