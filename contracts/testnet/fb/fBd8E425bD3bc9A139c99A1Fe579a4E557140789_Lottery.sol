...
contract Lottery is Ownable {
  ...
  event BatchAddPoolType(uint256[] _poolTypeIds);
  event BatchAddPool(uint256[] _poolIds);
  ...
  function batchAddPoolType(PoolTypeInfo[] memory _data) external onlyOwner {
    uint256[] memory newIds = new uint256[](_data.length);

    for (uint256 i = 0; i < _data.length; i++) {
      poolTypeInfo.push(_data[i]);
      newIds[i] = poolTypeInfo.length - 1;
    }

    emit BatchAddPoolType(newIds);
  }

  function batchAddPool(PoolInfo[] memory _data) external onlyOwner {
    uint256[] memory newIds = new uint256[](_data.length);

    for (uint256 i = 0; i < _data.length; i++) {
      poolInfo.push(_data[i]);
      newIds[i] = poolInfo.length - 1;
    }

    emit BatchAddPool(newIds);
  }

  function startsRound(uint256[] memory _poolIds) external onlyOwner returns (uint256 roundId) {
    uint256 prevRoundId = roundInfo.length;
    if (prevRoundId > 0) {
      RoundInfo memory prevRound = roundInfo[prevRoundId];
    ...
    roundInfo.push(
      RoundInfo({
        status: Status.Open,
        totalPlayers: 0,
        winningNumbers: new uint16[](0),
        poolIds: _poolIds
      })
    );

    uint256 newRoundId = roundInfo.length;
    emit RoundStatusChange(newRoundId, Status.Open);
    return newRoundId;
  }

  function endsRound(uint256 _roundId, uint8[] calldata _winningNumbers) external onlyOwner {
    RoundInfo storage _round = roundInfo[_roundId];

    require(_round.status == Status.Closed, "Round not closed yet");
  ...
  function batchBuyTicket(
    ...
    uint256 _numberOfTickets = _numbers.length.div(_poolType.suffixLength);

    uint256 numberLengthPerTicket = _poolType.numbersLength;

    require(_stakes >= _pool.minStake, "Insufficient staking");
    require(_round.status == Status.Open, "Round not open yet");
    require(_matchPool(_roundId, _poolId), "Invalid pool");
    require(
      numberLengthPerTicket != 0 && _numbers.length.mod(numberLengthPerTicket) == 0,
      "Invalid ticket type"
    );
    ...
    uint16[] calldata numbers = _numbers;
    ...
    for (uint256 i = 0; i < _numberOfTickets; i++) {
      ...
      uint256 start = i.mul(numberLengthPerTicket);
      uint256 end = (i.add(1)).mul(numberLengthPerTicket);
      ...
      uint256 id = ticketInfo.length;
      newTicketIds[i] = id;
      ownerToTickets[owner][roundId].push(id);
    }

    emit BatchBuyTicket(owner, roundId, poolId, numbers, stakes);
  ...
  function _rewardPrizeForMatching(uint256 _ticketId, uint256 _roundId) private {
    ...
    uint16[] memory _winningNumbers = roundInfo[_roundId].winningNumbers;
    uint16[] memory _userNumbers = _ticketInfo.numbers;
    ...
    for (uint256 i = start; i < end; i++) {
      uint16 a = _winningNumbers[i];

      for (uint256 j = 0; j < _userNumbers.length; j++) {
        uint16 b = _userNumbers[j];
    ...
    uint8 hitCount = _matchedCounter.div(_poolType.numbersLength);
    if (hitCount > 0) {
      uint256 stake = _ticketInfo.stakes;
      uint256 odds = _poolType.odds;
      uint256 minStake = _poolInfo.minStake;

      uint256 prize = stake.mul(odds);

      bool multiple = _poolType.forTicketType == MULTI_1;

      if (multiple) {
        prize = prize.mul(hitCount);
      }

      if (minStake > 0) {
        uint8 decimals = _poolInfo.lpToken.decimals();

        prize = PRBMath
          .UD60x18({ value: prize })
          .div(PRBMath.UD60x18({ value: minStake }))
          .mul(PRBMath.UD60x18({ value: 10**decimals }))
          .value;
      }

      _poolInfo.lpToken.transfer(_ticketInfo.owner, prize);
    }
  }
