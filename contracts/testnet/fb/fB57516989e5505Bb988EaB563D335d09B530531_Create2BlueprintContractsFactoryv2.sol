    ...
    ICreate2BlueprintContractsFactoryv2
{
    ...
    function registerNewBlueprint(
        ...
        if (
            ...
            blueprintAddress == address(0) ||
            bytes(blueprintName).length == 0
        ) {
    ...
    function upgradeBlueprint(
        ...
        uint16 lastVersion = latestBlueprintVersion[blueprintName];

        if (
            blueprintAddress == address(0) ||
            !blueprintExists(blueprintName) ||
            blueprints[blueprintName][lastVersion] == blueprintAddress
        ) {
            revert WrongBlueprint();
        }

        uint16 newVersion = ++lastVersion;

        latestBlueprintVersion[blueprintName] = newVersion;
        blueprints[blueprintName][newVersion] = blueprintAddress;
