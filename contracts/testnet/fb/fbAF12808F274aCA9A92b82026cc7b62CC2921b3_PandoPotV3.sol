...
contract PandoPotV3 is Ownable, ReentrancyGuard, Pausable {
    ...
    function _updateLuckyNumber(uint256 _id, RoundInfo storage _roundInfo) internal  {
        ...
        seeds.push(_numbers[0]);
        seeds.push(_numbers[1]);
        seeds.push(_numbers[2]);
    }
    ...
    function _calculateNumbers(uint256 _number1, uint256 _number2, uint256 _number3, PRIZE_TYPE _type, uint256 _quantity, EnumerableSet.UintSet storage _set) internal {
        uint256 _seed = ( block.number + uint(_type) + _quantity ) % 256;
        uint256 _rand = uint256(keccak256(abi.encodePacked(_number1 >> _seed, _number2, _number3 >> (256 - _seed ), _type, _quantity)));

        uint256 _value = (12345+uint(_type)) % typePrize[_type].digit;
        _set.add(_value);

        for(uint256 i = 1; i < _quantity; i++) {
            _value = uint256(keccak256(abi.encodePacked(_rand, i))) % typePrize[_type].digit;
    ...
    function _calcPrize(uint256 _roundId, uint256[] memory _numbers, uint256 _expire, PRIZE_TYPE _type) internal returns(uint256, uint256, uint256) {
        ...
        uint256 _USDForWinner = USDForCurrentPot * typePrize[_type].percentage / ONE_HUNDRED_PERCENT;
        uint256 _PSRForWinner = PSRForCurrentPot * typePrize[_type].percentage / ONE_HUNDRED_PERCENT;
        uint256 _PANForWinner = PANForCurrentPot * typePrize[_type].percentage / ONE_HUNDRED_PERCENT;
    ...
    function _getTickets(uint256 _number) internal returns(uint, uint) {
        uint256 _minorTicket = _number % typePrize[PRIZE_TYPE.MINOR].digit;
        uint256 _miniTicket = _number % typePrize[PRIZE_TYPE.MINI].digit;
    ...
    function _getDiscountFee() internal view returns (uint256) {
        if(discountPerAmount <= ticketPrice && discountPerAmount > 0) {
            return (ticketPrice - discountPerAmount) * (ONE_HUNDRED_PERCENT - discountPercentage) / ONE_HUNDRED_PERCENT;
        } else {
            if (discountPercentage > 0) {
                return ticketPrice * (ONE_HUNDRED_PERCENT - discountPercentage) / ONE_HUNDRED_PERCENT;
            }
        }
        return ticketPrice;
    }
    ...
    function buyTickets(address _receiver, uint256[] memory _tickets, uint256 _roundID) external availableBuyTicket(_receiver, _roundID) onlyEOA whenNotPaused nonReentrant{
        ...
        uint256 _ticketNumber;
        uint256 _ticketMinor;
        uint256 _ticketMini;
        for (uint256 i = 0; i < _tickets.length; i++) {
            _ticketNumber = _tickets[i] % digitMega;
            _ticketMinor = _tickets[i] % digitMinor;
            _ticketMini = _tickets[i] % digitMini;

            tickets[_roundID][PRIZE_TYPE.MEGA][_ticketNumber][_receiver]++;
            tickets[_roundID][PRIZE_TYPE.MINOR][_ticketMinor][_receiver]++;
            tickets[_roundID][PRIZE_TYPE.MINI][_ticketMini][_receiver]++;

            nTickets[_roundID][PRIZE_TYPE.MEGA][_ticketNumber]++;
            nTickets[_roundID][PRIZE_TYPE.MINOR][_ticketMinor]++;
            nTickets[_roundID][PRIZE_TYPE.MINI][_ticketMini]++;
        }
    ...
    function claim(PRIZE_TYPE _type, uint256 _roundId, uint256 _ticketNumber, address _receiver) external whenNotPaused nonReentrant {
        ...
        if (_type != PRIZE_TYPE.LEADERBOARD) {
            (_USDAmount, _PSRAmount, _PANAmount) = _claim(_type, _roundId, _ticketNumber);
        }
        else {
    ...
    function updatePrizeInfo(uint256 _digits, uint256 _numberOfWinner, uint256 _percentage, string memory _name, PRIZE_TYPE _types) external onlyOwner whenPaused {
        require(_percentage + totalPrizePercentage <= ONE_HUNDRED_PERCENT, "PandoPot: percentage over");
        require(_numberOfWinner > 0 , "PandoPot: _numberOfWinner zero");
        require(_digits > 0 , "PandoPot: _digits zero");
        TypePrize storage _typePrize = typePrize[_types];
        ...
        _typePrize.name = _name;
        _typePrize.types = _types;
        emit PrizeUpdated(_types, _digits, _numberOfWinner, _percentage, _name);
    }
    ...
    event PrizeUpdated(PRIZE_TYPE _type, uint256 _digits, uint256 _numberOfWinner, uint256 _percentage, string  _name);
}