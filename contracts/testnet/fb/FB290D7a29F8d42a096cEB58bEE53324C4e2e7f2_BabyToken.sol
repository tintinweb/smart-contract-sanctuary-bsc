...
interface IERC20 {
     ...
     *
     * Returns a boolean _value indicating whether the operation succeeded.
     *
     ...
     * zero by default.
     *
     * This _value changes when {approve} or {transferFrom} are called.
     */
    ...
    /**
     * @dev Emitted when `_value` tokens are moved from one account (`from`) to
     * another (`to`).
     *
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     *
     * The default _value of {decimals} is 18. To select a different _value for
     ...
     *
     * Tokens usually opt for a _value of 18, imitating the relationship between
     * Ether and Wei. This is the _value {ERC20} uses, unless this function is
     * overridden;
     *
...
library SafeMathInt {
    ...
    /**
     * @dev Converts to absolute _value, and fails on overflow.
     */
...
// Dependency file: contracts/dividentTracker/IterableMapping.sol
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    // For more discussion about choosing the _value of `magnitude`,
    ...
    //   but the computed _value of `dividendPerShare * balanceOf(_user)` is changed.
    // To keep the `dividendOf(_user)` unchanged, we add a correction term:
    //   `dividendOf(_user) = dividendPerShare * balanceOf(_user) + dividendCorrectionOf(_user)`,
    //   where `dividendCorrectionOf(_user)` is updated whenever `balanceOf(_user)` is changed:
    //   `dividendCorrectionOf(_user) = dividendPerShare * (old balanceOf(_user)) - (new balanceOf(_user))`.
    // So now `dividendOf(_user)` returns the same _value before and after `balanceOf(_user)` is changed.
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool onSuccess = IERC20(rewardToken).transfer(
                user,
                _withdrawableDividend
            );
    ...
    function _mint(address account, uint256 _value) internal override {
        super._mint(account, _value);

        magnifiedDividendCorrections[account] =
            magnifiedDividendCorrections[account] -
            (magnifiedDividendPerShare.mul(_value)).toInt256Safe();
    }
    ...
    function _burn(address account, uint256 _value) internal override {
        super._burn(account, _value);

        magnifiedDividendCorrections[account] = magnifiedDividendCorrections[
            account
        ].add((magnifiedDividendPerShare.mul(_value)).toInt256Safe());
    }
...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    IterableMapping.Map private tokenHolders;
    ...
    event ClaimWaitUpdated(uint256 indexed value, uint256 indexed oldValue);
    ...
    function _transfer(
        ...
        require(false, "Dividend_Tracker: Transactions not permitted.");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: Withdrawal of dividends disabled. Please use the 'claim' function on the main BABYTOKEN contract"
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Dividend_Tracker: The 'claimWait' _value must be set to a duration between 1 to 24 hours."
        );
        require(
            newClaimWait != claimWait,
            "Dividend_Tracker: Updating 'claimWait' to the same _value is not allowed."
        );
    ...
    function getNumberOfTokenHolders() external view returns (uint256) {
        return tokenHolders.keys.length;
    }
        ...
        )
    {
        ...
        index = tokenHolders.getIndexOfKey(account);
        ...
        if (index >= 0) {
            if (uint256(index) > lastProcessedIndex) {
                ...
                uint256 processesUntilEndOfArray = tokenHolders.keys.length >
                    lastProcessedIndex
                    ? tokenHolders.keys.length.sub(lastProcessedIndex)
                    : 0;
    ...
    {
        if (index >= tokenHolders.size()) {
            return (address(0), -1, -1, 0, 0, 0, 0, 0);
        }

        address account = tokenHolders.getKeyAtIndex(index);
        ...
        onlyOwner
    {
        ...
        if (newBalance >= minimumTokenBalanceForDividends) {
            ...
            tokenHolders.set(account, newBalance);
        } else {
            _setBalance(account, 0);
            tokenHolders.remove(account);
        }
    ...
    {
        uint256 numberOfTokenHolders = tokenHolders.keys.length;
        ...
        while (gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            if (_lastProcessedIndex >= tokenHolders.keys.length) {
                _lastProcessedIndex = 0;
            }

            address account = tokenHolders.keys[_lastProcessedIndex];
...
contract BabyToken is ERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        rewardToken = _reward;
        _marketingAddress = _marketing;
        // require(
        //     msg.sender != _marketingAddress,
        ...
        // default gas for processing dividents 300,000
        gasForProcessing = 300000;

        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(_wallet))
        );
        ...
        if (block.chainid == 61) {
            //etc
            nativeCurrency = router.WETC();
        } else if (block.chainid == 128) {
            //heco chain
            nativeCurrency = router.WHT();
        } else if (block.chainid == 42262) {
            //oasis
            nativeCurrency = router.WROSE();
        } else if (block.chainid == 43114 || block.chainid == 43113) {
            //avalance
            nativeCurrency = router.WAVAX();
        } else {
            nativeCurrency = router.WETH();
        }
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner {
        require(
            wallet != address(0),
            "BABYTOKEN: The marketing wallet cannot be the _value of zero"
        );
        _marketingAddress = wallet;
    }

    function setTokenRewardsFee(uint256 _value) external onlyOwner {
        tokenRewardsFee = _value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }
    ...
    function _setAutomatedMarketMakerPair(address pairAddr, bool _value)
        private
    {
        require(
            automatedMarketMakerPairs[pairAddr] != _value,
            "BABYTOKEN: Automated market maker pair is already set to that _value"
        );
        automatedMarketMakerPairs[pairAddr] = _value;

        if (_value) {
            dividendTracker.excludeFromDividends(pairAddr);
        }

        emit SetAutomatedMarketMakerPair(pairAddr, _value);
    }

    function updateGasForProcessing(uint256 value) public onlyOwner {
        require(
            value >= 200000 && value <= 500000,
            "BABYTOKEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            value != gasForProcessing,
            "BABYTOKEN: Cannot update gasForProcessing to same _value"
        );
        emit GasForProcessingUpdated(value, gasForProcessing);
    ...
    function _transfer(
        ...
        require(
            ...
            "The contract cannot transfer to a zero address"
        );
        ...
        if (
            ...
            if (marketingFee > 0) {
                uint256 marketingTokens = contractTokenBalance
                    .mul(marketingFee)
                    .div(totalFees);
                swapAndDistributeFee(marketingTokens);
            }
    ...
    function swapAndDistributeFee(uint256 tokens) private {
        ...
        IERC20(rewardToken).transfer(_marketingAddress, newBalance);
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // approve token transfer amount to cover all possible cases
        _approve(address(this), address(router), tokenAmount);
        // swap tokesn for eth

        if (block.chainid == 61) {
            //etc
            router.swapExactTokensForETCSupportingFeeOnTransferTokens(
                tokenAmount,
            ...
            //oasis
            router.swapExactTokensForROSESupportingFeeOnTransferTokens(
                tokenAmount,
                0, // accept any amount of ETH
                path,
                address(this),
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        // approve token transfer amount to cover all possible cases
        _approve(address(this), address(router), tokenAmount);
        // add eth to liqduidity
        // add the liquidity
        if (block.chainid == 61) {
            //etc
            router.addLiquidityETC{value: ethAmount}(
                address(this),
                tokenAmount,
                0, // slippage is unavoidable
                0, // slippage is unavoidable
            ...
            //oasis
            router.addLiquidityROSE{value: ethAmount}(
                address(this),
                tokenAmount,
                0, // slippage is unavoidable
                0, // slippage is unavoidable
