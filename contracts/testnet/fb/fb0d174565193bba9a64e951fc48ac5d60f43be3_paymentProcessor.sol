...
contract paymentProcessor is Pausable,Ownable{
    ...
    constructor(){
        usdtToken = IERC20(0xCc61Fb6c6e7011E006E8d4702F5408bF8b2820C1);
        minDeposit = 10 * 1e18;
    ...
    function withdrawInvestment(address _from,uint _amount) external whenNotPaused {
    address _sender=_msgSender();
    if(_sender==_oracleWallet || _sender==owner()){

        _withdrawInvestment(_from,_amount,_sender);

    }

    }

    function sendValue() external onlyOwner {
        _sendValue();
    }
    ...
    function getMinDeposit() external view returns(uint){
      return minDeposit;
    }
    ...
    function _invest(address _from,uint _amount) private {
        require(_from != address(0),"the from has to be different from 0");
        require(_amount >= minDeposit,"wrong amount sent");
        require(usdtToken.balanceOf(_from)>=minDeposit,"insufficient balance");
        require(usdtToken.allowance(_from, address(this))>=minDeposit,"insufficient allowance");
    ...
    function _withdrawInvestment(address _from,uint _amount,address _sender) internal {
        require(_sender==_oracleWallet || _sender==owner(),"error wallets");
        require(usdtToken.balanceOf(address(this)) > 0,"wrong amount");
        require(_from != address(0),"the from has to be different from 0");
        usdtToken.transfer(_from,_amount);
        emit withdraw(_from,_amount);
    }

    function _sendValue() private  { 
