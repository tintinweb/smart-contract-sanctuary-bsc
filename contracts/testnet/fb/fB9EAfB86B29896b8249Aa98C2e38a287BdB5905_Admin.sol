...
contract IDO is IIDO {
    ...
    function purchaseTokens(uint256 amount_) external initialized saleIsOngoing {
        ...
        bool hcapXceeds = this.willHardcapExceedWith(capped);
        if (hcapXceeds) {
    ...
    function maxAllocation() external view returns (uint256) { return _limit.maxAllocationShare; }
    ...
    function yetToBeRaised() external view returns (uint256) { return _limit.hardcap - _idoProject.collected; }
    ...
    function willHardcapExceedWith(uint256 amount_) external view returns (bool) { return amount_ + _idoProject.collected > _limit.hardcap; }

    function tokenPrice() external view returns (uint256) { return this.isPublicSale() ? _idoProject.pubTokenPrice : _idoProject.pvtTokenPrice; }
