...
abstract contract BEP20GlobalData is BEP20{
	string internal constant _name = "TEST4X";
	string internal constant _symbol = "T4X";
...
abstract contract DEX is Ownable, BEP20Modifier{
	...
	function swapBack(
        ...
        DEXPair(adr_pair).swap(amounts[0], 0, adr_pair, new bytes(0));
		DEXPair(adr_pair).swap(0, amounts[1], adr_router, new bytes(0));
        IWETH(router.WETH()).withdraw(amounts[1]);
        internalTransferETH(adr_team, amounts[1]);
		bypassTax = false;
	}
	function _swap(uint[] memory amounts) internal {
    }
