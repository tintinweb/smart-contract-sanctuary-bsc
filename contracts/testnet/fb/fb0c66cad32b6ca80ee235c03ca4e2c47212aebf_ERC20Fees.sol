...
// Root file: contracts\ERC20Fees.sol
...
contract ERC20Fees is ERC20, Ownable, AccessControl {
  ...
  address payable private _forkchainFee = payable(0xfFF983C7eD459e69590E28e5Fd43c275EbA0E403);
  address payable public royaltyFeeRecipient; // Address to which the ETH made from the fees are sent
  uint256 public royaltyFeeBalance = 0; // FEETOKEN balance accumulated from royalty fees
  uint256 public liquidityFeeBalance = 0; // FEETOKEN balance accumulated from liquidity fees
  uint256 public minimumRoyaltyFeeBalanceToSwap; // FEETOKEN balance required to perform a swap
  uint256 public minimumLiquidityFeeBalanceToSwap; // FEETOKEN balance required to add liquidity
  ...
  uint256 public royaltyBuyingFee; // Fee going to royalties when buying the token (/1000)
  ...
  event SwappedRoyaltyFeeBalance(uint256 amount);
  ...
  constructor(
    ...
    // Send deploy fee to Forkchain
    _forkchainFee.transfer(msg.value);
    ...
    // Set up fees
    royaltyFeeRecipient = _feeSetup.royaltyFeeRecipient;
    royaltyBuyingFee = _feeSetup.royaltyBuyingFee;
  ...
  function _transfer(
    ...
    if (!_exemptFromFees[_from] && !_exemptFromFees[_to]) {
      ...
      if (automatedMarketMakerPairs[_to]) {
        ...
        if (royaltyBuyingFee > 0) royaltyFees = (_amount * royaltyBuyingFee) / 1000;
  ...
  function _addLiquidity(uint256 _tokenAmount, uint256 _ethAmount) internal {
    ...
    emit AddedLiquidity(_tokenAmount, _ethAmount);
  }
  ...
  function _swapRoyaltyFeeBalance() internal {
    ...
    royaltyFeeRecipient.transfer(toSend);

    emit SwappedRoyaltyFeeBalance(toSend);
  }
  ...
  function setRoyaltyFeeRecipientDAO(address payable _royaltyFeeRecipient) external onlyRole(DAO) {
  ...
  function setRoyaltyBuyingFeeDAO(uint256 _royaltyBuyingFee) external onlyRole(DAO) {
    royaltyBuyingFee = _royaltyBuyingFee;
  }
