...
contract BrokerManagerFacet is IBrokerManager {
    ...
    function initBrokerManagerFacet(
        ...
        require(commissionP > 0 && commissionP < Constants.BASIS_POINTS_DIVISOR, "BrokerManagerFacet: Invalid commissionP");
...
library LibBrokerManager {
    ...
    struct BrokerManagerStorage {
        ...
        // token => total amount
        mapping(address => uint256) allBrokerCommission;
    ...
    function withdrawCommission(uint24 id) internal {
        ...
        for (UC i = ZERO; i < uc(tokens.length); i = i + ONE) {
            ...
            if (c.pending > 0) {
                ...
                bms.allBrokerCommission[tokens[i.into()]] -= c.pending;
                c.pending = 0;
            }
        }
    }
