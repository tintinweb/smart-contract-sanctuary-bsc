...
/**
 * @title Pbex Farms Auctions.
 * @notice Auctions for new Farms, including multiplier.
 */
contract PbexFarmAuction is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;

    IERC20 public immutable pbexToken;
    ...
    /**
     * @notice Constructor
     * @param _pbexToken: address of the $Pbex token
     ...
     * @param _amount: amount of the bid in $Pbex token
    ...
    function claimAuction(uint256 _auctionId) external nonReentrant {
        ...
        pbexToken.safeTransfer(address(msg.sender), claimableAmount);
    ...
    /**
     * @notice Close an auction round, and store $Pbex leaderboard threshold
     * @param _bidLimit: minimal $Pbex committed to be a winner
     * @dev Callable by operator
     */
    ...
    /**
     * @notice Claim $Pbex for all bids by bidders in leaderboard
     * @param _auctionId: auction id
     * @param _bidders: addresses of bidders
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(pbexToken), "Recover: Cannot be Pbex token");
