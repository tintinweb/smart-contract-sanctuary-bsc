...
contract Donate2 is Ownable {
    ...
    function getRewardPerTokenPaid(address account) public view returns (uint256) {
        return _rewardPerTokenPaid[account];
    }
    ...
    function buy1() public {
        usdt.transferFrom(msg.sender, address(this), 100 * 10 ** 18);
    }
    ...
    function withdrawReward() public {
        ...
        uint256 reward = myReward(msg.sender).sub(_rewardPerTokenPaid[msg.sender]);
        uint256 realReward = reward.sub(_rewardPerTokenPaid[msg.sender]);
        token.transfer(msg.sender, realReward);
        emit WithdrawReward(msg.sender, realReward);
        _rewardPerTokenPaid[msg.sender] = reward;
    }

    function myReward(address account) public view returns(uint256) {
        if(_balancesTimestamp[account] == 0) return 0;
        uint256 daysDiff = DaysDiff(_balancesTimestamp[account], block.timestamp);
        uint256 a = 0;
        if(daysDiff <= 20){
