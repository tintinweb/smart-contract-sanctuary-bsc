...
contract MetaCoin is Context, IERC20, Ownable, OwnableBridge {
    ...
    event BridgeBurn(address from, uint256 amount, uint256 toNetwork);
    ...
    function bridgeMint(address to, uint256 amount) external onlyBridgeOwner{
        require(to != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        ...
        _rOwned[bridgeOwner()] = _rOwned[bridgeOwner()]+bridgeFeeAmount;
        
        _rOwned[to] = _rOwned[to]+userReceivesAmount;

        _tTotal = _tTotal+amount;
        
        emit BridgeMint(to, userReceivesAmount);
    }

    function bridgeBurn(address from, uint256 amount, uint256 toNetwork) external {
        require(from != address(0), "ERC20: transfer from the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(amount <= balanceOf(from),"You are trying to transfer more than your balance");
        require(!_isBCheck[from], "You are a bot");

        transfer(address(0), amount);

        _tTotal = _tTotal-amount;

        emit BridgeBurn(from, amount, toNetwork);
    }
