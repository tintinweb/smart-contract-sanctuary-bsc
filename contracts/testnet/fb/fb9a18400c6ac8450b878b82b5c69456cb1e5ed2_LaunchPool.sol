...
contract LaunchPool {
    ...
    function _finalize() internal {
        ...
        uint256 balanceTokenA = IERC20(tokenSell).balanceOf(address(this));
        uint256 balanceTokenB;
        
        if (activeCurrency == IERC20(wBNB)) {
            uint256 balance = address(this).balance;
            require(balance > 0, "does not have any balance");
            payable(owner).transfer(balance * 15 / 100);
            payable(ReceiverFee).transfer(balance * 5 / 100);
            balanceTokenB = balance * 80 / 100;
        } else {
            uint256 balance = IERC20(activeCurrency).balanceOf(address(this));
            require(balance > 0, "does not have any balance");
            IERC20(activeCurrency).transfer(owner, balance * 15 / 100);
            IERC20(activeCurrency).transfer(ReceiverFee, balance * 15 / 100);
            balanceTokenB = balance * 80 / 100;
        }
    ...
    function buyWithBNB() external payable {
        require(activeCurrency == IERC20(wBNB), "Cannot buy with BNB, Please buy with Token");
    ...
    function buyWithToken(uint amount) external {
        require(activeCurrency != IERC20(wBNB), "Cannot buy with Token, Please buy with BNB");
        require(block.timestamp != 0, "Raising period not set");
        require(block.timestamp >= startTime, "Raising period not started yet");
        require(block.timestamp < endTime, "Raising period already end");
...
contract LaunchPoolFactoryV3 is Ownable {
    ...
    function createNewLaunchPool(
        ...
        IERC20(_tokenSale).transferFrom(msg.sender, address(_newPool), _hardcap);
