...
// File: contracts/NFTFactory2.sol

pragma solidity 0.8.0;

contract NFTFactory3 {
    ...
    mapping(string => address) public nftMapping;
    ...
    modifier onlyOperator() {
        require(msg.sender == operator, "Restricted to operator.");
        _;
    }
    ...
    function setParameter(address _nftOwner, string memory _baseURI) external onlyAdmin {
        nftOwner = _nftOwner;
        baseURI = _baseURI;
    }
    ...
    function createNewNFT(string memory name, string memory symbol) external onlyOperator returns (address) {
        require(nftMapping[symbol] == address(0), "symbol aready exist!");
        BNFT nft = new BNFT(nftOwner, baseURI, name, symbol);
        nftMapping[symbol] = address(nft);
        totalCreated += 1;
        nftArray.push(symbol);
