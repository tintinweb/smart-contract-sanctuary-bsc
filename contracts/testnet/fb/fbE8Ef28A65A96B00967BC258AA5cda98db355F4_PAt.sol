...
contract PAt is Context, IERC20, Ownable {
    ...
    string private _name = "PAt token";
    ...
    address payable public marketingAddress =
        0xbAE5814D00E0Ed4469C96f66FDe9dE5ae7191314;

    address payable public DaoCommunityAddress =
        0x23cBcA420e441C60909C08e1BD840D2C06191314;
    address payable public DaoFoundationAddress =
        0x89d88aeab40C8c70093546e088BEe0B707141314;

    address public defaultReferrerAddress =
        0x8e6Bbe55754220E722638faa94ce981537191314;
    ...
    address[] DaoCommunity;
    mapping(address => uint256) DaoCommunityIndexes;

    address[] DaoFoundation;
    mapping(address => uint256) DaoFoundationIndexes;

    uint256 public _maxTxAmount = 10 * 10**8 * 10**9;

    event MinTokensBeforeSwapUpdated(uint256 minTokensBeforeSwap);
    event SwapAndSendUpdated(bool enabled);
    ...
    constructor() public {
        ...
        uniswapV2Pair2 = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(
                address(this),
                // 0x55d398326f99059fF775485246999027B3197955
                0xc362B3ed5039447dB7a06F0a3d0bd9238E74d57c //测试网
            );
    ...
    function _transferBothExcluded(
        ...
        uint256 tReferrerFee = calculateReferrerFee(tAmount);
        uint256 tBurnFee = calculateBurnFee(tAmount);
        _takeBurnFee(sender, tBurnFee);
        _takeMarketingFee(sender, tMarketingFee);
        _takeReferrerFee(sender, tReferrerFee);
    ...
    function setSwapAndSend(bool _enabled) public onlyOwner {
        ...
        emit SwapAndSendUpdated(_enabled);
    }
        ...
        )
    {
        ...
        uint256 tMarketingFee = calculateMarketingFee(tAmount);
        uint256 tReferrerFee = calculateReferrerFee(tAmount);
        uint256 a = tFee.add(tMarketingFee).add(tBurn).add(tReferrerFee);
        uint256 tTransferAmount = tAmount.sub(a);
        return (tTransferAmount, tFee, tMarketingFee, tBurn, tReferrerFee);
    }
        ...
        )
    {
        ...
        uint256 rReferrerFee = tReferrerFee.mul(currentRate);
        uint256 a = rFee.add(rMarketingFee).add(rBurn).add(rReferrerFee);
        uint256 rTransferAmount = rAmount.sub(a);
    ...
    function _takeMarketingFee(address from, uint256 tMarketingFee) private {
        if (tMarketingFee == 0) return;
        uint256 currentRate = _getRate();
        uint256 rMarketingFee = tMarketingFee.mul(currentRate);
    ...
    function _takeReferrerFee(address userAddress, uint256 tReferrerFee)
        private
    {
        if (tReferrerFee == 0) return;
        uint256 currentRate = _getRate();
        uint256 rReferrerFee = tReferrerFee.mul(currentRate);
        dividendsToReferrer(userAddress, tReferrerFee, rReferrerFee);
    }

    function _takeBurnFee(address userAddress, uint256 tBurnFee) private {
        ...
        emit Transfer(userAddress, address(0), tBurnFee);
    }

    function dividendsToReferrer(
        address from,
        uint256 tAmount,
        uint256 rAmount
    ) private {
        ...
        while (true) {
            ...
            uint256 tAmountDividend = getAmountDividend(tAmount, i);
            uint256 rAmountDividend = getAmountDividend(rAmount, i);
            if (referalAddress == address(0)) {
                _rOwned[defaultReferrerAddress] = _rOwned[
                    defaultReferrerAddress
                ].add(rAmountDividend);

                if (_isExcluded[defaultReferrerAddress]) {
                    _tOwned[defaultReferrerAddress] = _tOwned[
                        defaultReferrerAddress
                    ].add(tAmountDividend);
                }
                emit Transfer(from, defaultReferrerAddress, tAmountDividend);
            } else {
                _rOwned[referalAddress] = _rOwned[referalAddress].add(
                    rAmountDividend
                );

                if (_isExcluded[referalAddress]) {
                    _tOwned[referalAddress] = _tOwned[referalAddress].add(
                        tAmountDividend
                    );
                }
    ...
    function getAmountDividend(uint256 amount, uint8 i)
        private
        pure
        returns (uint256)
    {
    ...
    function calculateReferrerFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_referrerFee).div(10**2);
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair) {
            if (block.timestamp < tradingEnabledTimestamp + 3 hours) {
                buyAmount[to] = buyAmount[to].add(amount);
                require(
                    buyAmount[to] <= 100000 * 10**9,
        ...
        if (to == uniswapV2Pair || to == uniswapV2Pair2) {
            if (from != address(this)) {
                require(
                    balanceOf(from).sub(amount) >= 100000000,
                    "You must keep 0.1 tokens in your wallet"
                );
            }
        }
    ...
    function _updateReferralRelationship(address from, address to) internal {
        ...
        if (referralRelationships[to] != address(0)) {
            return;
        }
    ...
    function getReferralRelationship(address user)
        public
        view
        returns (address)
    {
