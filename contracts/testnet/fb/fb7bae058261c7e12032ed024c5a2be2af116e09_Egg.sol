...
contract Egg is ERC721, Ownable {
    ...
    address private _chicken = 0xc49D5D8c7b8F55DD9EfA90865200680e6cdcCa81;
    ...
    function openEgg(uint256[] memory IDs) external {
        ...
        for (uint ID = 0; ID < IDs.length; ID++) {
            ...
            uint256 factor = uint256(keccak256(abi.encodePacked(IDs[ID], block.number, msg.sender))).mod(5);

            if (factor == 0) {

                swapExactTokensForBNB(Prize(IDs[ID]), msg.sender);
                // ERC20(_chickenTown).transferFrom(msg.sender, _treasuryWallet, Prize(ID));

            } else if (factor == 1) {

                swapExactTokensForBUSD(Prize(IDs[ID]), msg.sender);
                // ERC20(_chickenTown).transferFrom(msg.sender, _treasuryWallet, Prize(ID));

            } else if (factor == 2) {

                swapExactTokensForUSDT(Prize(IDs[ID]), msg.sender);
                // ERC20(_chickenTown).transferFrom(msg.sender, _treasuryWallet, Prize(ID));

            } else if (factor == 3) {

                swapExactTokensForUSDC(Prize(IDs[ID]), msg.sender);
                // ERC20(_chickenTown).transferFrom(msg.sender, _treasuryWallet, Prize(ID));

            } else {

                ERC20(_chickenTown).transfer(msg.sender, Prize(IDs[ID]));
            ...
            emit PrizeInfo(Prize(IDs[ID]), msg.sender); 
