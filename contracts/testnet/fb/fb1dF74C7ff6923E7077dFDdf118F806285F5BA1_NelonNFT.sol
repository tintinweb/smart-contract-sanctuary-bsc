...
// File: contracts/common/AccessController.sol
...
// File: contracts/common/FusionNFT.sol


pragma solidity >=0.8.0;


contract FusionNFT is AccessController {
    constructor() {
    ...
    modifier onlyFusionContract() {
        require(fusionContract == msg.sender, "ENFT: caller is not the fusion contract");
        _;
    }

    function setFusionContract(address _fusionContract) public onlyAdmin {
        _setFusionContract(_fusionContract);
    }

    function _setFusionContract(address _fusionContract) internal {
...
// File: contracts/token/NelonNFT.sol
...
contract NelonNFT is ERC721, ERC721Enumerable, ERC721Burnable, AccessController, FusionNFT {
    using Strings for uint256;
    ...
    modifier onlyBox() {
        require(boxContract != address(0), "NelonNFT: Box contract is not valid.");
        require(boxContract == msg.sender, "NelonNFT: Caller is not the NELON box contract.");
        _;
    }
    ...
    event BatchBox(address indexed account, uint256[] tokenIds, Ranking ranking, uint8 level, uint256 hashRate, uint256 quantity);
    ...
    function setBoxContract(address _boxContract) public onlyAdmin {
    ...
    function batchMint(uint8 level, uint256 quantity) external onlyAdmin {
    ...
    function batchBox(uint8 level, uint256 quantity) external onlyBox {
        ...
        emit BatchBox(msg.sender, tokenIds, Ranking.Common, level, hashRate, quantity);
    }

    function batchFusion(Ranking ranking, uint8 level, uint256 quantity) external onlyFusionContract {
