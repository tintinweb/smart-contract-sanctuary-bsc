...
contract  dnaDapp is Ownable{
    ...
    //地址 =》 返利账单
    mapping(address => UpBill[]) public upBills;
    ...
    //我的账单数量
    function getUpBillsIndex(address from) public view returns (uint256) {
       return upBills[from].length;
    }
    ...
    function buyOrder(uint256 amount) public  {
        ...
        if(upAddress1 != address(0) && count1 > 0){
            upBills[upAddress1].push(UpBill({
                amount: amount.mul(algebra1).div(100),
        ...
        if(upAddress2 != address(0) && count2 > 0){
            upBills[upAddress2].push(UpBill({
                amount: amount.mul(algebra2).div(100),
                fromAddress: msg.sender,
                createTime: now
            }));
        ...
        if(upAddress3 != address(0) && count3 > 0){
            upBills[upAddress3].push(UpBill({
                amount: amount.mul(algebra3).div(100),
                fromAddress: msg.sender,
                createTime: now
            }));
