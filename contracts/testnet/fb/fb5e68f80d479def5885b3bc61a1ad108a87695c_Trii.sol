...
contract Trii is ERC20Detailed, Ownable {
    ...
    uint256 public totalFee =
        InsuranceFee.add(treasuryFee).add(DividendFee).add(
            autofirePitFee
        );
    ...
    constructor() ERC20Detailed("33Crypt", "Trii", uint8(DECIMALS)) Ownable() {
        ...
        InsuranceReceiver = 0x349c1AE680fB62B9B7018C97c6343b0fe3BcA520;
        treasuryReceiver = 0x4f7c5d5827a67Aec0A6D6ee99136B1168ddE54Bd;   
    ...
    function takeFee(
        ...
        if (recipient == pair) {
            ...
            _DividendFee = DividendFee.add(_sell);
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(_DividendFee))
        );
        _gonBalances[InsuranceReceiver] = _gonBalances[InsuranceReceiver].add(
            gonAmount.div(feeDenominator).mul(InsuranceFee)
        );
    ...
    function addLiquidity() internal swapping {
        uint256 autoLiquidityAmount = _gonBalances[InsuranceReceiver].div(
            _gonsPerFragment
        );
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            _gonBalances[InsuranceReceiver]
        );
        _gonBalances[InsuranceReceiver] = 0;
    ...
    function swapBack() internal swapping {
        ...
        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(DividendFee).div(treasuryFee.add(DividendFee))
        } () {} catch {}        
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no 3RCYPTO token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
