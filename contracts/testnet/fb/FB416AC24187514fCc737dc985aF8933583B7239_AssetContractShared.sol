...
import "./Library/TokenIdentifiers.sol";

/**
 * @title AssetContractShared
 * Dukia shared asset contract - A contract for easily creating custom assets on Dukia
 */
...
contract AssetContractShared is AssetContract, ReentrancyGuard {
    ...
    // mapping(address => bool) public sharedProxyAddresses; change---moved to ERC1155Tradable
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            _isCreator(_id, _msgSender()),  // (proxy related)
    ...
    constructor(
        ...
        // address _proxyRegistryAddress, (proxy related)
        address dukiaConduit,
        ...
        addSharedAllowanceAddress(dukiaConduit);
    }
    ...
    //     proxyRegistryAddress = _address;   (proxy related)L68 - L70
    // }

    /**
     * @dev Allows owner to add a shared allowance address
     */
    function addSharedAllowanceAddress(address _address) public onlyOwner {   // (proxy related)
        sharedAllowanceAddresses[_address] = true;
    }
    ...
    function removeSharedProxyAddress(address _address)
        public
        onlyOwner   /// (proxy related)
    {
        delete sharedAllowanceAddresses[_address];
    }

    function updateGlobalApprovalStatus(bool isApproved) external {
        require(notGloballyApproved[_msgSender()] != isApproved, "Approval Status already set");
        notGloballyApproved[_msgSender()] = isApproved;
    }

    /**
     * @dev Allows owner to disable the ability to migrate
     */
    // function disableMigrate() public onlyOwner {      // (proxy related)
     ...
     */
    // function migrate(Ownership[] memory _ownerships) public onlyOwner {   // (proxy related)
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                _isCreator(_ids[i], _msgSender()),   //(proxy related)
    ...
    function _requireMintable(address _address, uint256 _id) internal view {
        require(
            _isCreator(_id, _address),  // (proxy related)
        ...
        returns (bool)
    {
        ...
        return creator_ == _address;  //|| _isProxyForUser(creator_, _address); (proxy related)L247-L255
    }

    // Overrides ERC1155Tradable to allow a shared proxy address
    // function _isProxyForUser(address _user, address _address) (proxy related)L258-L268
    //     internal
    //     view
    //     override
    //     returns (bool)
...
/**
 * @title AssetContract
 * AssetContract - A contract for easily creating non-fungible assets on Dukia.
 */
...
// changed to dukia
    function dukiaVersion() public pure returns (string memory) {
        return "1.0.0";
    }
    ...
    /**
     * Compat for factory interfaces on Dukia
    ...
    function setTemplateURI(string memory _uri) public onlyOwner {   // (proxy related)
        templateURI = _uri;
    }
        ...
        returns (uint256)
    {
        ...
        return
            _isCreator(_id, _owner)      // (proxy related)
    ...
    function _isCreator(uint256, address _address)  /// (proxy related) 
        internal
        view
        virtual
        returns (bool)
    ...
    Pausable
{
    ...
    // (proxy related)address public proxyRegistryAddress;
    mapping(address => bool) public sharedAllowanceAddresses;
    // Global approval status
    mapping(address => bool) public notGloballyApproved;
    // Contract name
    string public name;
    // Contract symbol
    ...
    constructor(
        string memory _name,
        string memory _symbol
        // (proxy related)address _proxyRegistryAddress
    ) ERC1155("") {
        name = _name;
        symbol = _symbol;
        //(proxy related) proxyRegistryAddress = _proxyRegistryAddress;
     ...
     */
     // (proxy related) ---- L59 - L65. replaced with onlyOwner from Ownable.sol 
    ...
    function _isOwner(address _address) internal view returns (bool) {
        ...
        //(proxy related ) || _isProxyForUser(owner(), _address);
    }

    function pause() external onlyOwner { //(proxy related)
        _pause();
    }

    function unpause() external onlyOwner {  //(proxy related)
        _unpause();
    }
    ...
    /**
     * Override isApprovedForAll to whitelist dukia's accounts to enable gas-free listings.
     */
    ...
    {
        // Whitelist dukia's contracts for easy trading.
        if (sharedAllowanceAddresses[_operator] && !notGloballyApproved[_owner]) {
            return true;
        }
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) public virtual onlyOwner {       // (proxy related)
        _mint(_to, _id, _quantity, _data);
    }
    ...
    // function _isProxyForUser(address _user, address _address)  (proxy related) L500-L511
    //     internal
    //     view
    //     virtual
    //     returns (bool)
    ...
    /**
     * This is used instead of msg.sender as transactions won't be sent by the original token owner, but by Dukia.
     */
