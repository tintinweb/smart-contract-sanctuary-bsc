...
contract CamlyDEV is ERC721A, ERC2981, ReentrancyGuard, AccessControl {
    ...
    //@notice isTransferBlocked holds bool value that used for blocking transfer.
    ...
    constructor(address _camlyProxyAddress) ERC721A("NFT_TEST", "TEST") {
        ...
        _setupRole(SIGNER_ROLE, _msgSenderERC721A());
    }
    ...
    //@notice if value set as true the contract can't allow the user to transfer their NFT.
    //@notice if value set as false the contract  allow the user to transfer their NFT.
    /**
        restriction: the ADMIN_ROLE address only has the permission to change the target ipfs URL.
     */ 

    function blockOrUnblockTransfer(bool _value) external onlyRole(ADMIN_ROLE) {
        isTransferBlocked = _value;
    }
    ...
    */
    ...
    function verifySign(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(this, _msgSenderERC721A(), _tokenURI, sign.nonce)
        );
    ...
    function isApprovedForAll(address _owner, address operator) public view virtual override returns (bool) {
        if(_msgSenderERC721A() == camlyProxy) return true;
    ...
    function _beforeTokenTransfers(
        ...
        require(from == address(0)|| to == address(0) || _msgSenderERC721A() == camlyProxy || !isTransferBlocked, "Camly: Transfer not Allowed");
    }
