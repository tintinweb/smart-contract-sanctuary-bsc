...
contract Farming8FUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function calculateResult(uint256 amount, uint256 timestampStart, uint256 currentTimestamp) public view returns (uint256) {
        return amount * (currentTimestamp - timestampStart) * (currentRewardAmount * 10 ** 18) / (10 ** 18 * currentRewardTime);
    }
    ...
    function getCurrentRewardAmount(address account) public view returns (uint256 fullAmount, uint256 timestamp) {
        ...
        uint256 rewardPerSecond = currentRewardAmount / currentRewardTime;
    ...
    function changeRewardSystem(uint256 rewardTime, uint256 rewardAmount) public onlyOwner {
    ...
    function claimMyBody(uint256 amount) public {
        ...
        uint256 currentBody = userFarmBalances[userAddress];
