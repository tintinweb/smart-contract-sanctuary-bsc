...
contract TunaMaximumContract is IERC20 { 
    ...
    TokenInfo private _tokenInfo = TokenInfo ( //77,777  100,000
           "Tuna Maximum", "TLMN", 8,  100_000_000, /*10**_decimals;*/ 10_000_000_000_000, //77_777 * _denomination; 
           14, 16,      1, 86400, //24hrs
    ...
    function _payTax(address from, uint256 amount, uint256 taxAmount, Tax memory taxs) private {
        ...
        for(uint i = 0; i < 6; i++) {             
            if(_taxRateArray[i] > 0) {
                ...
                //swap if its more than threshold
                uint256 _addressBalance = _balances[_feeAddrArray[i]];
                if(dex.swapThreshold > 0 && _addressBalance > dex.swapThreshold) {
    ...
    function _provideLiquidity() private {
         ...
         if(dex.liquidityThreshold == 0 || liquidityTokenBalance < dex.liquidityThreshold || _msgSender() == address(dex.router)) return; 
   ...
   function updatePairThreshold(uint256 _liquidity_threshold, uint256 _swapThreshold) external onlyOwner {
    ...
    function _swapTokenAddress(address txAddress, bool isSwapToBNB) private {
         if(isSwapToBNB) {    
            ...
            _s wap(preTokenAddressBalance, true);
            ...
            _s wap(preTokenAddressBalance, false);
