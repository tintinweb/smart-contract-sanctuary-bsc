...
// File: contracts/NerdFaucetV2.sol
...
contract NerdFaucetV2 is Ownable, IFaucet {
    ...
    uint256 public minimumInitial = 1 ether;
    ...
    constructor(
        ...
        req[3] = KickbackReq(10, 10, 0);
        req[4] = KickbackReq(10, 10, 20);
        req[5] = KickbackReq(10, 20, 50);
        req[6] = KickbackReq(10, 50, 100);
    }

    //----------------------------------------------
    //                 USER FNS                   //
    //----------------------------------------------
    function deposit(uint256 _amount, address _upline) external {
    ...
    function getFaucetMax(uint256 _deposits) public view returns (uint256) {
    ...
    function getLeaderAmount(address _user)
        public
        view
        returns (uint256 _directLeaders, uint256 _structureLeaders)
    {
        _directLeaders = team[_user].leaders[0];
        _structureLeaders = team[_user].leaders[1];
    }
    ...
    function checkVault() internal {
        ...
        if (needToMint) {
            uint256 mintAmount = (totalSupply * 5) / (100 * 48);
    ...
    function payoutUser(address _user, uint256 amount) internal {
        uint256 realizedAmount = (amount * 9) / 10; // 10% Tax on claims
        uint256 stakeVaulted = IToken(token).balanceOf(vault);
        require(stakeVaulted >= realizedAmount, "Insufficient funds");
        IVault(vault).withdraw(_user, realizedAmount);
    }
    ...
    function claimRebase(
        ...
        Accounting storage user = accounting[_user];
        _payout = 0;
        (, uint256 userRebase, uint256 totalCount, ) = getUserAdjustedRebase(
            user.netFaucet,
        ...
        if (user.rebaseClaims + user.faucetClaims >= capPayout()) return 0;
        if (user.rebaseClaims + userRebase + user.faucetClaims > capPayout()) {
            userRebase = capPayout() - user.rebaseClaims - user.faucetClaims;
        }
        if (accumulate) {
    ...
    function addLiability(address _user) internal {
        ...
        if (user.faucetClaims >= faucetMax || grossClaim >= capPayout()) {
            return;
        }
    ...
    function setUpline(address _user, address _upline) internal {
        ...
        user.leaders = [0, 0]; // reset leaders count
        for (uint8 i = 0; i < maxRefDepth; i++) {
            if (_upline == address(0)) break;
            if (becameLeader) {
                if (i == 0) team[_upline].leaders[0]++;
                else team[_upline].leaders[1]++;
            }
    ...
    function userLevel(address _user) public view returns (uint8 _level) {
        ...
        _level = 0;
        (uint256 directLeader, uint256 structureLeader) = getLeaderAmount(
            _user
        );
        ...
        for (uint8 i = 1; i < 7; i++) {
            ...
            if (
                ...
                directLeader >= levelReq.directTeamLeaders &&
                structureLeader >= levelReq.structureLeaders
            ) _level++;
            else break;
        }
    }
    ...
    function setGovToken(address _govToken) external onlyOwner {
        require(
            govToken == address(0) && _govToken != address(0),
            "Invalid GovToken or set"
        );
        try IToken(govToken).balanceOf(owner()) {
            govToken = _govToken;
        } catch {
    ...
    function editLevelRequirements(
        ...
        KickbackReq storage currentReq = req[_level];
        require(
            prevReq.directReferrals <= _directRef &&
                prevReq.directTeamLeaders <= _directLeaders &&
                prevReq.structureLeaders <= _structureLeaders,
            "Invalid data"
        );
        req[_level] = KickbackReq(
            _directRef,
            _directLeaders,
            _structureLeaders
        );
