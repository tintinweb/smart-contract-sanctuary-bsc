...
//   ======================================
    ...
    function authSalesVault (address _salesVault) external onlyRole(Contract_Manager) {
        // Require "Sales Vault" to be linked to this contract
        require (ISalesVault(_salesVault).baseToken() == address(this), "Vault not Linked");
        require (Inventory.Basis[_salesVault].balance == 0,   "Vault not Empty");

        Inventory.setInternalStatus (_salesVault, false);

        emit VaultUpdated(_msgSender(), 4, _salesVault);
    }
