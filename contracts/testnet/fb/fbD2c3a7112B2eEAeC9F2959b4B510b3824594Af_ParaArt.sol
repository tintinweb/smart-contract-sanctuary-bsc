    ...
    ERC721Burnable
{
    ...
    event Locked(bytes32 hashed);
    event UnLock(bytes32 hashed);

    constructor() ERC721("ParaArt", "PRA") {
    ...
    modifier notLocked(bytes32 hashed) {
        require(!locked[hashed], "Locked");
        _;
    }

    function lock(bytes32 hashed)
        public
    ...
    function unlock(bytes32 hashed)
        public
        onlyRole(COPYRIGHT_ROLE)
        returns (bool)
    {
    ...
    function safeMint(address to, bytes32 hashed)
        public
        onlyRole(MINTER_ROLE)
        returns (uint256)
    {
        require(_metaData[hashed].tokenId == 0, "Hashed must be not found");
