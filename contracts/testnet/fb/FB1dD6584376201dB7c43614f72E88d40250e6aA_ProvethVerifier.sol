...
contract ProvethVerifier {
    ...
    function receiptProof(
        ...
        (result, index, t) = validateReceiptProof(blockHash, proofBlob);
        state = t.state;
        gasUsed = t.gasUsed;
        bloom = t.bloom;
        logs = t.logs;
    ...
    function validateReceiptProof(
        bytes32 blockHash,
        bytes memory proofBlob
    ) internal pure returns (uint8 result, uint256 index, ReceiptTransaction memory t) {
        result = 0;
        index = 0;
    ...
    function decodeProofBlob(bytes memory proofBlob) internal pure returns (Proof memory proof) {
        ...
        proof = Proof(
            proofFields[0].toRlpBytes(),
            bytes32(proofFields[0].toList()[TX_ROOT_HASH_INDEX].toUint()),
            bytes32(proofFields[0].toList()[RECEIPTS_ROOT_HASH_INDEX].toUint()),
            rlpTxIndex,
