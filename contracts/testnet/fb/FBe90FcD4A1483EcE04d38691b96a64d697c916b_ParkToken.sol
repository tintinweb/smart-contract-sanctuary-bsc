...
contract ParkToken is ERC20,IParkToken{
    ...
    //price in wei (one wei is the smallest unit)
    uint256 public constant IDO_PRICE =          4_000_000_000_000_000; //20% discount (0.004   BNB/Park Token)
    uint256 public constant PRIVATE_SALE_PRICE = 3_750_000_000_000_000; //25% discount (0.00375 BNB/Park Token)
    uint256 public constant LIQUIDITY_PRICE =    5_000_000_000_000_000; //0%  discount (0.005   BNB/Park Token)



    uint256 private constant _TOTAL_INITIAL_SUPPLY = 4_000_000_000e18; //scientific decimal notation
    uint256 private constant _INITIAL_SUPPLY =       2_000_000_000e18; //scientific decimal notation
    uint256 private constant _BURN_INITIAL_SUPPLY =  2_000_000_000e18; //scientific decimal notation
    ...
    constructor(string memory name_, string memory symbol_,address[] memory wallets_,address[] memory MOBs_,uint256 minHoldBalance_,bool isMainNetwork_)ERC20(name_,symbol_){
        
        //Mint Tokens 
        _mint(address(this), _TOTAL_INITIAL_SUPPLY);

        //Burn 50% Tokens
        _burn(address(this), _BURN_INITIAL_SUPPLY);
    ...
    function _initilizeTokenomics() private returns(bool){
        ...
        // IDO
        _transfer(address(this),idoWallet, _INITIAL_SUPPLY.mul(_IDO_PERCENTAGE).mul(_IDO_WALLET_PERCENTAGE).div(1000000));

        // Private Sale
        _transfer(address(this),privateSaleVault, _INITIAL_SUPPLY.mul(_PRIVATE_SALE_PERCENTAGE).mul(_PRIVATE_SALE_VAULT_PERCENTAGE).div(1000000));
        
        // Tokens to Liquidity Pool
        _transfer(address(this),liquidityWallet, _INITIAL_SUPPLY.mul(_LIQUIDITY_PERCENTAGE).mul(_LIQUIDITY_WALLET_PERCENTAGE).div(1000000));
        
        // Reward Buffer
        _transfer(address(this),rewardBufferVault, _INITIAL_SUPPLY.mul(_REWARD_BUFFER_PERCENTAGE).mul(_REWARD_BUFFER_VAULT_PERCENTAGE).div(1000000));
        
        // Team
        _transfer(address(this),teamVault, _INITIAL_SUPPLY.mul(_TEAM_PERCENTAGE).mul(_TEAM_VAULT_PERCENTAGE).div(1000000));
        
        // Development Fund
        _transfer(address(this),developmentWallet, _INITIAL_SUPPLY.mul(_DEVELOPMENT_PERCENTAGE).mul(_DEVELOPEMENT_WALLET_PERCENTAGE).div(1000000));
        _transfer(address(this),developmentVault, _INITIAL_SUPPLY.mul(_DEVELOPMENT_PERCENTAGE).mul(_DEVELOPMENT_VAULT_PERCENTAGE).div(1000000));       
  
        // Marketing
        _transfer(address(this),marketingWallet, _INITIAL_SUPPLY.mul(_MARKETING_PERCENTAGE).mul(_MARKETING_WALLET_PERCENTAGE).div(1000000));
        _transfer(address(this),marketingVault, _INITIAL_SUPPLY.mul(_MARKETING_PERCENTAGE).mul(_MARKETING_VAULT_PERCENTAGE).div(1000000)); 

        // Advisors
        _transfer(address(this),advisorsWallet, _INITIAL_SUPPLY.mul(_ADVISORS_PERCENTAGE).mul(_ADVISORS_WALLET_PERCENTAGE).div(1000000));
        _transfer(address(this),advisorsVault, _INITIAL_SUPPLY.mul(_ADVISORS_PERCENTAGE).mul(_ADVISORS_VAULT_PERCENTAGE).div(1000000));        
     
        // Airdrop/Bounty
        _transfer(address(this),airDropWallet, _INITIAL_SUPPLY.mul(_AIRDROP_PERCENTAGE).mul(_AIR_DROP_WALLET_PERCENTAGE).div(1000000));
        
        // Burn Reserve
        _transfer(address(this),burnReserveVault, _INITIAL_SUPPLY.mul(_BURN_PERCENTAGE).mul(_BURN_RESERVE_VAULT_PERCENTAGE).div(1000000));
...
interface IParkToken is  IERC20, IERC20Metadata {
    ...
    // function addLiquidity(uint256 tokenAmount_) external payable returns(bool);
