...
contract Center is Ownable {
    ...
    function buy(uint256 _depositId,uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        ...
        if (amount > 0) {
            ...
            safeTransferFrom(basicToken, msg.sender, claimAddr, (amount.mul(claimShareRate)).div(10000)); 
            safeTransferFrom(basicToken, msg.sender, insureAddr, amount.sub(claimAddrGet)); 
        }
