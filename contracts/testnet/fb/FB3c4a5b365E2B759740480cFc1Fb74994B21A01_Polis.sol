...
interface IPolis {
    ...
    function mint(address to, IUniswapV2Pair LPToken) external;
...
contract Polis is IPolis, Context, ERC721AQueryable {
    ...
    string constant MINT_TYPE = "mint(address to, IUniswapV2Pair LPToken)";
    ...
    function mintWithSignature(
        ...
        bytes32 messageHash = mintHash(to, LPToken);
        address signer = messageHash.toEthSignedMessageHash().recover(
            signature
        );
        _ensureIsAdminWallet(signer);
        _safeMint(to, 1);
        _setLPToken(_currentIndex, LPToken);
    }
        ...
        return
            keccak256(
                abi.encodePacked(
                    uint16(0x1901),
                    domainSeperator(),
                    keccak256(abi.encode(MINT_TYPE, to, LPToken))
                )
            );
    }
