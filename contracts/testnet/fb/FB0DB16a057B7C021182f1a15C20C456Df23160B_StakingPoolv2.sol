...
contract StakingPoolv2 is OwnableUpgradeable, PausableUpgradeable  {
    IBEP20 public bplus;
    IBEP20 public kob;
     ...
     */
    function initByOwner(IBEP20 _kob,IBEP20 _bplus) public onlyOwner {
        kob=_kob;
        bplus=_bplus;
    }
  ...
  function stake(uint256 _amount, uint32 _poolConfigId) external  whenNotPaused returns (uint256) {
        ...
        kob.approve(address(this), _amount*10**18);
        kob.transferFrom(msg.sender, address(this), _amount*10**18);
...
function claim(uint32 _stakingInfoId) external whenNotPaused  {
        ...
        kob.approve(address(this), staked);
        kob.transferFrom(address(this), msg.sender, staked);
    }
...
function setPoolConfig(
  ...
  function withdrawBplus(uint amount) public onlyOwner {
