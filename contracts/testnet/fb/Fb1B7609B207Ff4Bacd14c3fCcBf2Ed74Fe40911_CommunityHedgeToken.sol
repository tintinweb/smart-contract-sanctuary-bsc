...
contract CommunityHedgeToken is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x54944829363e1dEE06db70D338Dc25B23b6E5C1e;
    ...
    constructor() {
        ...
        IPancakeSwapRouter _pancakeSwapRouter = IPancakeSwapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    function _transfer(
        ...
        if (from == pancakeswapPair || to == pancakeswapPair) {
            (uint256 reserve0, uint256 reserve1,) = IPancakeSwapPair(pancakeswapPair).getReserves();
            (address token0,) = PancakeLibrary.sortTokens(address(this), busdAddress);
            
            if (token0 != address(this)) {
                uint256 _reserve = reserve0;
                reserve0 = reserve1;
                reserve1 = _reserve;
            }
            uint256 prePrice;
            ...
            if (from == pancakeswapPair) {
                prePrice = ((1 ** decimals()).mul(reserve1)).div(reserve0);
                nextPrice = ((1 ** decimals()).mul(reserve1)).div(reserve0 - amount);
                require ((nextPrice - prePrice) <= prePrice.mul(2).div(100), "This transaction impact the price more than 2%.");
            }
            if (to == pancakeswapPair) {
                prePrice = ((1 ** decimals()).mul(reserve1)).div(reserve0);
                nextPrice = ((1 ** decimals()).mul(reserve1)).div(reserve0 + amount);
                require ((prePrice - nextPrice) <= prePrice.mul(2).div(100), "This transaction impact the price more than 2%.");
            }
        }
    ...
    function swapAndLiquify(uint256 liquidifyAmount) private lockTheSwap {
        ...
        emit AutoLP (liquidifyAmount, newBalance);
    ...
    function swapTokensForBUSDForMarketing(uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = marketingWallet;
        path[1] = busdAddress;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tBuybackAndBurn, uint256 tMarketing, uint256 tLiquidity) {
        tMarketing = tAmount.mul(_marketingFee).div(10**2);
        tLiquidity = tAmount.mul(_liquidityFee).div(10**2);
        tBuybackAndBurn = tAmount.mul(_buyBackAndBurnFee).div(10**2);
    ...
    function _tokenTransfer(
        ...
        (uint256 tTransferAmount, uint256 tBuybackAndBurn, uint256 tMarketing, uint256 tLiquidity) = _getValues(amount);
    ...
    function _takeFee(
        ...
        if (tBuybackAndBurn > 0) {
            ...
            uint256 prePrice = ((1 ** decimals()).mul(reserve1)).div(reserve0);

            swapBUSDForTokens(tBuybackAndBurn);

            (reserve0, reserve1,) = IPancakeSwapPair(pancakeswapPair).getReserves();
            (token0,) = PancakeLibrary.sortTokens(address(this), busdAddress);
            if (token0 != address(this)) {
                uint256 _reserve = reserve0;
                reserve0 = reserve1;
                reserve1 = _reserve;
            }
            uint256 newPrice = ((1 ** decimals()).mul(reserve1)).div(reserve0);

            emit AutoBuybackAndBurn(tBuybackAndBurn, prePrice, newPrice);
        }
        if (tLiquidity > 0) {
        ...
        if (tMarketing > 0) {
            ...
            swapTokensForBUSDForMarketing(tMarketing);
        }
