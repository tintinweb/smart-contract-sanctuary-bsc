...
contract MetavieToken is Context, IERC20, Ownable {
    ...
    address payable public _feesCollectingAddress;

    // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router;
    address public _pancakeswapV2LiquidityPair; 

    // DEX Taxing Fee for Buying & Selling
    uint8 public dexTaxFee = 3;

    // DEBUGING ONLY
    bool public isSwapingEnabled = true;

     // General Info
    string private _name        =   "TokenName";
    string private _symbol      =   "TokenSymbol";
    uint8  private _decimals    =   18;
    uint256 private _maxSupply  =   30619192;

    // Supply
    uint256 private _totalSupply = _maxSupply * 10**_decimals;

    constructor () 
    {
         IUniswapV2Router02 pancakeswapV2Router = IUniswapV2Router02(0xD00C7e4c6bDd8FE20159E31FaD5389482CDbE46B);
        _pancakeswapV2Router = pancakeswapV2Router;

        // Mint the total supply balance to the deployer of this contract
   ...
   /**
    ...
    function setSwapingStatus(bool status) public onlyOwner() returns(bool) {
        isSwapingEnabled = status;
        return isSwapingEnabled;
    }
    ...
    function setFeesCollectingAddress(address feesCollectingAddress) external onlyOwner() {
        _feesCollectingAddress = payable(feesCollectingAddress);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == address(_pancakeswapV2Router)  || to == address(_pancakeswapV2Router)) 
        {
    ...
    function swapTokensForEthAndSend(uint256 tokenAmount, address payable recipient) private {
        ...
        path[1] = _pancakeswapV2Router.WETH();
         ...
         _approve(msg.sender, address(_pancakeswapV2Router), tokenAmount);
