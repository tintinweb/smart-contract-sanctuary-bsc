...
contract StakingSet is StakingSetStorage {
    ...
    function buyStakingSet(uint256 amount, uint256 tokenId) payable external onlyHub {
        ...
        (uint256 nimbAmount,uint256 gnimbAmount,uint256 cakeLPamount) = makeSwaps(amountBNB); 
    ...
    function updateNimbToken(address _nimbToken, address newStaking) external onlyOwner {
        ...
        require(IERC20Upgradeable(_nimbToken).approve(address(newStaking), type(uint256).max), "IERC20Upgradeable: APPROVE_FAILED");
        nimbToken = IERC20Upgradeable(_nimbToken);
        NimbStaking = IStaking(newStaking);
}
