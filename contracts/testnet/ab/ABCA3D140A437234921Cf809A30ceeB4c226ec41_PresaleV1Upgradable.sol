    ...
    UUPSUpgradeable
{
    ...
    function _transferPresaleTokens(
        address _address,
        uint256 _tokenValue
    ) private {
    ...
    function _payReward(address _address, uint256 _tokenValue) private {
        IERC20Upgradeable(rewardTokenContract).transfer(_address, _tokenValue);
        emit RewardTokenDistributed(_address, _tokenValue, rewardTokenContract);
    }
    ...
    function BuyWithETH(
        ...
        if (isBuyNStake) {
            ...
            _transferPresaleTokens(_msgSender, _tokenValue);
        }
        ...
        if (isPayRewardTokens) {
            _payReward(_msgSender, _rewardValue);
        }
    ...
    function BuyWithUSD(
        ...
        if (isPayReferral) {
            IERC20Upgradeable(USDContract).transfer(
                referralContract,
                _value.div(_getTotalReferralLevelRate())
            );
