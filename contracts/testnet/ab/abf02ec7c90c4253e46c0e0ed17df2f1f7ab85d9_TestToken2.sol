...
contract TestToken2 is Context, IBEP20, Ownable {
    ...
    function depositLiquidity(uint256 depositAmount)external returns(uint256) {
        IBEP20 pairContract = IBEP20(_pcsPair);
        pairContract.transferFrom(_msgSender(), address(this), depositAmount);

        _updateAccountDividends(_msgSender());
        _accounts[_msgSender()].liqBalance = _accounts[_msgSender()].liqBalance.add(depositAmount);
        _totalLiq += depositAmount;
