...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    mapping(uint256 => ParcelInfo) public parcelIdToParcelInfoMapping;
    ...
    function mintParcels(ParcelMintSignature calldata mintingSignature, uint256 _referrerId)
        external
        onlySaleActive
        returns (MintedNFT memory)
    {
        ...
        uint256 _amount = _getPriceForParcels(mintingSignature.parcels);

        _transferToContract(_amount);

        MintedNFT memory mintInfo = _mintParcels(mintingSignature, _amount);

        return mintInfo;
    }

    // @notice (Purchase) Mint the given amount of miners
    function mintMiners(uint16 count, uint256 _referrerId) 
    external 
    onlySaleActive
    returns (MintedNFT memory) 
    {
        ...
        uint256 _amount = count * minerPrice;

        _transferToContract(_amount);

        MintedNFT memory mintInfo = _mintMiners(count, _amount);

        return mintInfo;
    }

    // @notice (Purchase) Purchase a given amount of el. vouchers
    function purchaseVouchers(uint16 count) 
    external 
    onlySaleActive
    {
    ...
    function attachDetachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) external notZeroAddress {
        ...
        require(
            parcelIdToParcelInfoMapping[parcelId].parcelType != PARCEL_TYPE_BUSINESS,
            "Municipality: Can not attach Miner to Business Parcel"
        );
        require(
    ...
    function upgradeStandardParcel(uint256 _parcelId) 
    external 
    onlySaleActive
    {
    ...
    function _isMinerInList(uint256 _tokenId, uint256[] memory _minersList)
        private
        pure
        returns (bool isInList)
    {
    ...
    function _distributeNetGymStreet(
        ...
        if (_amount > 0) {
            _transferAmountTo(_amount, _user);
    ...
    function _mintParcels(ParcelMintSignature calldata mintingSignature, uint256 _amount)
        private
        returns (MintedNFT memory)
    {
        ...
        for (uint256 i = 0; i < mintingSignature.parcels.length; ++i) { 
            Parcel memory parcel = mintingSignature.parcels[i];
            require(
            ...
            require(
                !IParcelInterface(standardParcelNFTAddress).parcelExists(parcel.x, parcel.y),
                "Municipality: Parcel already exists as a standard parcel"
            );
            require(
                !IParcelInterface(businessParcelNFTAddress).parcelExists(parcel.x, parcel.y),
                "Municipality: Parcel already exists as a business parcel"
            );
            address addr = parcel.parcelType == PARCEL_TYPE_STANDARD
                ? standardParcelNFTAddress
                : businessParcelNFTAddress;

            uint256 parcelId = IParcelInterface(addr).mint(mintingSignature.wallet, parcel.x, parcel.y);
            if (i == 0) {
                mintInfo.firstParcelId = parcelId;
            }
            ParcelInfo memory parcelInfo;
            parcelInfo.isValid = true;
            parcelInfo.parcelLandType = parcel.parcelLandType;
            parcelInfo.parcelType = parcel.parcelType;
            parcelIdToParcelInfoMapping[parcelId] = parcelInfo;
        }
        _distributeAmounts(_amount, OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
        userToPurchasedAmountMapping[msg.sender] += _amount;
        return mintInfo;
    }

    function _mintMiners(uint16 _count, uint256 _amount) private returns (MintedNFT memory) {
        MintedNFT memory mintInfo;
        mintInfo.count = _count;
        ...
        _distributeAmounts(_amount, OPERATION_TYPE_MINT_MINERS, msg.sender);
        userToPurchasedAmountMapping[msg.sender] += _amount;
        return mintInfo;
    }
    ...
    function _requireMinersCountMatchingWithParcelSlots(uint256 _parcelId, uint256 _count)
        ...
        view
    {
        ParcelInfo memory parcelInfo = parcelIdToParcelInfoMapping[_parcelId];
        require(
            parcelInfo.isValid &&
                (
    ...
    function _requireParcelExists(uint256 parcelId) private view {
        ParcelInfo memory parcelInfo = parcelIdToParcelInfoMapping[parcelId];
...
interface IParcelInterface {
    function mint(address user, uint x, uint y) external returns (uint256);

    function parcelExists(uint _x, uint _y) external view returns(bool exists);
}
