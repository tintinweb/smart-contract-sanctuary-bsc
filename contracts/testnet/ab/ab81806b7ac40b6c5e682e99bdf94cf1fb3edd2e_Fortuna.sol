...
contract Fortuna {
    ...
    uint256 private constant farmReward = 800; // 8pc
    ...
    function _updateReward(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 1; i <= referDepth; i++){
            if(upline != address(0)){
                ...
                }else if(i > 4) {   // for L5-6
                    if( userInfo[upline].level == 4) {
                        reward = newAmount.mul(level4Percents[i - 5]).div(baseDivider); // L5 comm
                        upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    }
                }else if(i > 2) { //for L3-4
                    if( userInfo[upline].level == 3) {
                        reward = newAmount.mul(level3Percents[i - 3]).div(baseDivider); // L4 comm
                        upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    }
                }else if(i > 1){ // for L2
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 1; i <= referDepth; i++){
            if(upline != address(0)){
                ...
                if(i == 2 && userInfo[upline].level == 2){ //for L2
                    if(thirdLevelReward.level3Freezed > 0) {
                        uint256 level3Reward = newAmount.mul(level2Percents).div(baseDivider);
