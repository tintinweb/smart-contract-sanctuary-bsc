...
contract UberTaxiController is Ownable {
    ...
    constructor() {
        ...
        //初始化车辆
        vehicles[1] = Vehicle("Volkswagen",   1, 1000*10**6, 1000*10**6, 5*10, 5, true);
        vehicles[2] = Vehicle("Tesla",        2, 2000*10**6, 2000*10**6, 5.2*10, 5, true);
        vehicles[3] = Vehicle("Audi",         3, 3000*10**6, 3000*10**6, 5.4*10, 5, false);
        vehicles[4] = Vehicle("BMW",          4, 4000*10**6, 4000*10**6, 5.6*10, 5, false);
        vehicles[5] = Vehicle("MercedesBenz", 5, 5000*10**6, 5000*10**6, 5.8*10, 5, false);
        vehicles[6] = Vehicle("RollsRoyce",   6, 10000*10**6, 10000*10**6, 6*10, 5, false);
    }
    ...
    //修改车辆属性
    function updateVehicle(uint256 index, uint256 price, uint256 rate, uint256 purchaseLimit) public onlyOwner {
    ...
    function buyVehicle(uint256 index, uint8 tokenType) public {
        ...
        if (tokenType == 1) {//taix
            ...
            require(users[msg.sender].vehicleTypeNum[index - 1] < vehicles[index].purchaseLimit, "Exceed the purchase limit");
    ...
    function testUserUtaix() public {
        users[msg.sender].UTAXINum = 100000000000000000000000000000;
    }
