...
contract CKRacing is ERC20, Ownable {
    ...
    address public marketingWallet; //collects funds for marketing, buyback & game dev to save gas fees on contract
    ...
    constructor() ERC20("Crypto Kart Racing", "CKRACING") {
        ...
        // 25% sell tax to start for first 3 minutes, will be reduced over time.
    	...
    	marketingWallet = 0xA9B0D388850137fbED8CE347096f337E039f0985; // set as marketing wallet
    	teamWallet = 0xBE4a2Ecf5d583BCCF779D4B81a881329ce52b6dD; // set as team wallet
    ...
    // Blacklist function is only available for first 24 hours after launch
    function blackListAddress(address addr) external onlyOwner returns (bool) {
        require(block.number <= tradingActiveBlock + 28800,"Error: No longer available");
    ...
    function setMaxTransactionAmount(uint256 newNum) external onlyOwner {
        require(newNum >= 1 * 10**7, "Error: Max tx amount has to be equal to or greater than 10M");
    ...
    function setBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _teamFee) external onlyOwner {
    ...
    function setSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _teamFee) external onlyOwner {
