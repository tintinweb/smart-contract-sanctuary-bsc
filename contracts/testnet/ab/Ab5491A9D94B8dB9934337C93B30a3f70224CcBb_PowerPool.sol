...
contract PowerPool is Ownable {
    ...
    function processDynReward() public {
        ...
        for(uint i=0;i<n;i++)
        {
            ...
            {
                v1[i]=staker[i];
                v1TotalPower=v1TotalPower.add(userInfo[v1[i]].power);
            }
        }
        if(v1TotalPower==0) {
            return;
        ...
        //分配v1动态奖金给每一个用户
        for(uint s=0;s<v1.length;s++)
        {
           if(v1[s]!=address(0x0))
            {
            uint256 rewards=userInfo[v1[s]].power.mul(v1cafeReward).div(1e12);
            userInfo[v1[s]].dynRewardAmount=userInfo[v1[s]].dynRewardAmount.add(rewards);
            userInfo[v1[s]].lastdynRewardAmount=rewards;
            }
        }
        ...
        {
            uint256 k=0;
            for(uint256 j=0;j<invitee[staker[i]].length;j++)
            {
                ...
                {
                    v2[i]=staker[i];
                    v2TotalPower=v2TotalPower.add(userInfo[v2[i]].power);
                    break;
                }
            }
        }
        ...
        //分配v2动态奖金给每一个用户
        for(uint s=0;s<v2.length;s++)
        {
            if(v2[s]!=address(0x0))
            {

            uint256 rewards=userInfo[v2[s]].power.mul(v2cafeReward).div(1e12);
            userInfo[v2[s]].dynRewardAmount=userInfo[v2[s]].dynRewardAmount.add(rewards);
            userInfo[v2[s]].lastdynRewardAmount=rewards;
            }
        }
        ...
        for(uint256 i=0;i<staker.length;i++)
        {
            ...
            for(uint256 j=0;j<invitee[staker[i]].length;j++)
            {
                ...
                {
                    v3[i]=staker[i];
                    v3TotalPower=v3TotalPower.add(userInfo[v3[i]].power);
                    break;
                }
            }
        }
        ...
        //分配v3动态奖金给每一个用户
        for(uint s=0;s<v3.length;s++)
        {
            if(v3[s]!=address(0))
            {
            uint256 rewards=userInfo[v3[s]].power.mul(v3cafeReward).div(1e12);
            userInfo[v3[s]].dynRewardAmount=userInfo[v3[s]].dynRewardAmount.add(rewards);
            userInfo[v3[s]].lastdynRewardAmount=rewards;
            }
        } 
        ...
        for(uint256 i=0;i<staker.length;i++)
        {
            ...
            for(uint256 j=0;j<invitee[staker[i]].length;j++)
            {
                ...
                {
                    v4[i]=staker[i];
                    v4TotalPower=v4TotalPower.add(userInfo[v4[i]].power);
                    break;
                }
            }
        }
        ...
        //分配v4动态奖金给每一个用户
        for(uint s=0;s<v4.length;s++)
        {
            if(v4[s]!=address(0))
            {
            uint256 rewards=userInfo[v4[s]].power.mul(v4cafeReward).div(1e12);
            userInfo[v4[s]].dynRewardAmount=userInfo[v4[s]].dynRewardAmount.add(rewards);
            userInfo[v4[s]].lastdynRewardAmount=rewards;
            }
        }
    }
    }
