...
contract SmartMatrixForsageBasic {
    ...
    function init(address _ownerAddress, IERC20 _depositTokenAddress) public {
        BASIC_PRICE = 5e18;
        LAST_LEVEL = 12;
    ...
    function x3Partners(uint256 id, uint8 level)
        public
        view
        returns (uint256 NPartners)
    {
        return users[idToAddress[id]].x3Matrix[level].totalPartners;
    }

    function x6Partners(uint256 id, uint8 level)
        public
        view
        returns (uint256 NPartners)
    {
        return users[idToAddress[id]].x6Matrix[level].totalPartners;
    }

    function totalPartners(uint256 id) public view returns (uint256 NPartners) {
        uint256 partners;
        for (uint8 i = 1; i <= LAST_LEVEL; i++) {
            partners += users[idToAddress[id]].x3Matrix[i].totalPartners;
            partners += users[idToAddress[id]].x6Matrix[i].totalPartners;
        }
        return partners;
    }
    ...
    function updateX3Referrer(
        ...
        users[referrerAddress].x3Matrix[level].totalPartners++;
    ...
    function updateX6Referrer(
        ...
        users[referrerAddress].x6Matrix[level].totalPartners++;
