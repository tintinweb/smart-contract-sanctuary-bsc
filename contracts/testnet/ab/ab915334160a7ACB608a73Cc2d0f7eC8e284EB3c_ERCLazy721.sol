    ...
    Ownable
{
    modifier onlyCreator(uint256 tokenId) {
        require(
            ...
            "FTVIO: message sender does not own token id"
        );
        _;
    }
    ...
    bytes32 public constant MINT_PERMIT_TYPEHASH =
        0xbedd0c8895c1940fbd98b51809362b995b2d391a057dafaaa70c68e5b36136a3;
    ...
    ERCLazy721 private _thisAsOperator;
    ...
    constructor(
        ...
        _thisAsOperator = ERCLazy721(address(this));
        operationsAddress = _msgSender();
        payeeAddress = _msgSender();
    }
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IERCLazy).interfaceId ||
    ...
    function requireValidMintPermit(
        ...
        require(
            ...
            "FTVIO: permit revoked"
        );

        return signer;
    }

    function tokenIdMatchesCreator(
        uint256 tokenId,
        address creatorAddress
    ) public pure returns (bool isCreator) {
        uint160 metadataSHA1 = (uint160)(tokenId >> 96);

        uint256 tokenSpecificCreatorIdentifier = (uint256)(
            keccak256(abi.encode(metadataSHA1, creatorAddress))
        );
    ...
    function mintLazy(
        ...
        require(
            ...
            "FTVIO: permit period invalid"
        );

        if (permit.recipient != address(0)) {
            require(
                recipient == permit.recipient,
                "FTVIO: recipient does not match permit"
            );
        }
        ...
        if (permit.currency == address(0)) {
            require(
                ...
                "FTVIO: transaction value under minimum price"
            );

            fopt = (msg.value * fee) / FEE_DENOMINATOR;

            if (fopt > 0) {
                (bool hs, ) = payable(operationsAddress).call{value: fopt}("");
                require(hs, "Fopt transfer failed.");
            }

            (bool os, ) = permit.payee.call{value: msg.value - fopt}("");
            require(os, "Transfer failed.");
        } else {
            IERC20 token = IERC20(permit.currency);
            fopt = (permit.minimumPrice * fee) / FEE_DENOMINATOR;
            token.safeTransferFrom(msg.sender, operationsAddress, fopt);
            token.safeTransferFrom(
                msg.sender,
                signer,
                permit.minimumPrice - fopt
            );
        }

        _mint(signer, permit.tokenId);
        _setTokenURI(permit.tokenId, permit.uri);
    ...
    function mintWithURI(
        ...
        require(
            msg.value >= minFee,
            "FTVIO: transaction value under minimum fee"
        );

        if (currency == address(0)) {
            fopt = (amount * fee) / FEE_DENOMINATOR;
            if (fopt > 0) {
                (bool hs, ) = payable(operationsAddress).call{value: fopt}("");
                require(hs, "Fopt transfer failed.");
            }

            (bool os, ) = payable(payeeAddress).call{value: amount - fopt}("");
            require(os, "Transfer failed.");
        } else {
            IERC20 token = IERC20(currency);
            fopt = (amount * fee) / FEE_DENOMINATOR;
            token.safeTransferFrom(msg.sender, operationsAddress, fopt);
            token.safeTransferFrom(msg.sender, payeeAddress, amount - fopt);
        }
    ...
    function setOperationsAddress(
        address operationsNewAddress
    ) external onlyOwner {
        require(operationsNewAddress != address(0));
        operationsAddress = payable(operationsNewAddress);
    }

    function setPayeeAddress(address payeeNewAddress) external onlyOwner {
        require(payeeNewAddress != address(0));
        payeeAddress = payable(payeeNewAddress);
    }
