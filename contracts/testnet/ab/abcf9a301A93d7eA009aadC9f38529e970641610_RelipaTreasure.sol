...
contract RelipaTreasure is ERC1155, Ownable, IRelipaTreasure {
  mapping(address => mapping(uint256 => uint256)) internal balances;
  uint256 internal constant RELIPA_TREASURE = 1;
  address NFTaddress;
  ...
  function claimTreasure(uint256 amount) external override {
    require(amount > 0, 'Please input amount greater than 0');
    balances[msg.sender][RELIPA_TREASURE] += amount;
  ...
  function getBalanceOf(address account) public view override returns (uint256) {
    return balances[account][RELIPA_TREASURE];
  }

  function safeTransfer(address to, uint256 amount) public override CheckAddress(to) {
    require(amount > 0, 'Please input amount greater than 0');
    require(
      amount <= balances[msg.sender][RELIPA_TREASURE],
      "Amount must be less or equal than sender treasure's amount"
    );
    balances[msg.sender][RELIPA_TREASURE] = balances[msg.sender][RELIPA_TREASURE].sub(amount);
    balances[to][RELIPA_TREASURE] == balances[to][RELIPA_TREASURE].add(amount);
    _safeTransferFrom(msg.sender, to, RELIPA_TREASURE, amount, '');
    emit safeTransferEvent(msg.sender, to, RELIPA_TREASURE, amount);
  }

  function setNftAddress(address nftAddress) external override onlyOwner {
    NFTaddress = nftAddress;
    emit setNftAddressEvent(NFTaddress);
  }

  function unbox(uint256 amount) external override {
    require(amount > 0, 'Please input amount greater than 0');
    require(
      amount <= balances[msg.sender][RELIPA_TREASURE],
      "Amount must be less or equal than sender treasure's amount"
    ...
    for (uint256 i = 0; i < amount; i++) {
      balances[msg.sender][RELIPA_TREASURE] = balances[msg.sender][RELIPA_TREASURE].sub(1);
...
contract RelipaNFT is ERC721Enumerable, Ownable, IRelipaNFT {
...
interface IRelipaTreasure {
  ...
  event safeTransferEvent(address from, address to, uint256 idTreasure, uint256 amount);

  event setNftAddressEvent(address nftAddress);
