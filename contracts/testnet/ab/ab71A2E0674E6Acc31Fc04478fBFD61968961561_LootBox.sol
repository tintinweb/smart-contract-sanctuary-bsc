...
contract LootBox is ILootBox, ERC721 {
    ...
    event NewMic(uint8 body, uint8 head, uint8 kind, uint8 class);
    ...
    function generateMicro(uint256 _tokenId) external {
        address msgSender = _msgSender();
        (uint8 body, uint8 head, uint8 kind, uint8 class) = _generateMicro(_tokenId);
        emit NewMic(body, head, kind, class);
