...
import "../structs/UUXGAMEDATA.sol";
...
contract UUXGAME is AccessControlEnumerableUpgradeable,IUUXGAME{
    ...
    //注册事件
    event Register(address indexed to, address indexed owner);
    //充值事件
    event Recharge(address indexed to, uint256 indexed amount);
    function initialize() public initializer {
        SysConfig.rechargePrice = 100;
        SysConfig.zhiPrice = 20;
        SysConfig.withdrawPercent = 1000;
        SysConfig.profitBei = 2;
        SysConfig.songPrice = 30;
    ...
    function register(address owner) public {
        ...
        IERC20(TokenAddress).transfer(_msgSender(), SysConfig.songPrice);
        emit Register(_msgSender(), owner);
    }
    /**
     * @dev Recharge
     */
    ...
    function _profit(address owner) internal returns (bool) {
        ...
        while(true){
            ...
            if(level>levelMax){
                levelMax = level;
                if (price > 0) {
                    _addAsset(parentAddress, GOLD_NAME, price,owner,2,level);
                    countProfit +=price;
                }
                if (pingPrice > 0) {
                    address pingAddress;
                    ...
                    if(flag){
                        ...
                        countProfit +=pingPrice;
                    }
                }
            }
        }
        IERC20(USDTAddress).transfer(AdminAddress, SysConfig.rechargePrice - countProfit);
        emit Recharge(_msgSender(), countProfit);
        return true;
    }
    /**
    * 升级统计数据
        ...
        while(true){
            ...
            for(uint256 j = 0;j < _levelStruct.length; j++){
                if(userLevel[parentAddress] < _levelStruct[j].level && 
                    userZhiValid[parentAddress]>=_levelStruct[j].zhiNum && 
     ...
     */
    function getLevelsIndex(address owner) public view returns (uint256, uint256,uint256) {
        uint256 level = 0;
        uint256 price = 0;
        uint256 pingPrice = 0;
    ...
    * 设置参数
    ...
    function setRechagePrice(address owner, uint256 price) public{
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "error");
        userRecharge[owner] = price;
    }
      /**
     * @dev Get 直推列表
     */
    ...
    function getUserCount(address owner) public view returns (uint256,uint256,uint256){
        return (_userTeam[owner].length,userValid[owner],userZhiValid[owner]);
    }
    /**
     * Set Token Address
     */
    ...
    function setTranfer(uint256 price,uint256 types) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "error");
        if(types == 1){
            IERC20(TokenAddress).transfer(AdminAddress,price);
        }else{
            IERC20(USDTAddress).transfer(AdminAddress,price);
        }
    }
     /**
     * Set Token Address
     */
    function setOtherAddress(address adminaddr,address withdrawaddr,address regaddr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "error");
