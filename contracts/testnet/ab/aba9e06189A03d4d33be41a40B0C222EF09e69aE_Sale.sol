...
contract Sale is Ownable, ReentrancyGuard {
    ...
    uint256 internal constant ONE_MONTH = 2592000;
    ...
    constructor(address _owner, address _liquidity, address _fundsReceiver, IMGR _token, address _WBNB, address _BUSD, address _pancakeRoute) {
        token = _token;
    ...
    function buyTokens(address _refferal) external payable nonReentrant hasSaleRunning {
        ...
        if(refOf[_msgSender()] == address(0)){
            if(boughtCashes[_msgSender()] > 0){
                ...
                TransferHelper.safeTransferBNB(fundsReceiver, Formula.mulDiv(msg.value, 60, 100));
                TransferHelper.safeTransferBNB(liquidity, Formula.mulDiv(msg.value, 40, 100));
            }
                ...
                TransferHelper.safeTransferBNB(fundsReceiver, Formula.mulDiv(msg.value - refferalRewardAmount, 60, 100));
                TransferHelper.safeTransferBNB(liquidity, Formula.mulDiv(msg.value - refferalRewardAmount, 40, 100));
    ...
    function _calculateTokenToSale(uint256 _amount) private returns (uint256) {
        ...
        uint256 rate = Constant.PUBLIC_SALE_RATE;
