...
contract JuneLogic is OwnableUpgradeable, ERC20Upgradeable, Console, Manager {
    ...
    address []  public DividendAdds;
    mapping(address => uint256) DividendAddsIndex;
    ...
    // Set setSellMarketingRates
    function setSellMarketingRates(uint256 value) public onlyManager {
        sellMarketingRates = value;
    }
    // Set setBuyMarketingRates
    function setBuyMarketingRates(uint256 value) public onlyManager {
        buyMarketingRates = value;
    }
    // Set setDividendAdds
    function setDividendAdds(address value, bool _is) public onlyManager {
        if (_is) {
            DividendAdds.push(value);
            DividendAddsIndex[value] = DividendAdds.length;
        } else {
            delete DividendAdds[DividendAddsIndex[value]];
        }
    }
    // Set getDividendAdds
    function getDividendAdds(address value) public view virtual returns (bool) {
        return DividendAddsIndex[value] > 0;
    }
    // Set getDividendAddsAll
    function getDividendAddsAll() public view virtual returns (address [] memory) {
        return DividendAdds;
    }
    // Set getDividendTotal
    function getDividendTotal() public view virtual returns (uint256) {
        return balanceOf(MarketingWalletAddr);
    }
    // Set getLpTotal
    function getLpTotal() public view virtual returns (uint256) {
        IERC20Upgradeable LiquidityWallet = IERC20Upgradeable(LiquidityAddr);
        return LiquidityWallet.totalSupply();
    }
    ...
    function Dividend() public onlyManager {
        uint256 DividendTotal = balanceOf(MarketingWalletAddr);
        require(DividendTotal >= 0);
        IERC20Upgradeable LiquidityWallet = IERC20Upgradeable(LiquidityAddr);
        uint256 LpTotal = LiquidityWallet.totalSupply();
        require(LpTotal >= 0);
        uint256 oneDividend = DividendTotal.sub(LpTotal);
        for (uint i = 0; i < DividendAdds.length; i++) {
            uint256 userBalance = LiquidityWallet.balanceOf(DividendAdds[i]);
            uint256 userDividend = oneDividend.mul(userBalance);
            if (userDividend >= 0 && userBalance > DividendMin) {
                super._transfer(MarketingWalletAddr, DividendAdds[i], 100000000000000000);
            }
        }
    }
