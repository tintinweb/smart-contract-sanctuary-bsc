...
interface ISaleVault {
    function createLock(uint8 _round, uint256 _token_amount, uint256 _weth_amount, uint256 _start, uint256 _end, uint256 _slippage, uint256 _deadline) external payable;
}
...
contract Sale is ReentrancyGuard {
    ...
    event RemainingWithdrawn(uint256 _Round, uint256 _Amount);
    ...
    function userWithdrawTokens (uint256 _round) public nonReentrant IsSaleEnded(_round) {
        ...
        if(sale_details.hasVesting == 0) {
            ...
            if((tge_claim_amount + claim_amount) > 0) {
    ...
    function finalizeSale (uint8 _round, uint256[5] memory _lock_details) public nonReentrant IsSaleEnded(_round) {
        require(msg.sender == owner || msg.sender == adminAddress, "Final auth");
        require(saleStatusCheck(_round) == 2, "Not succeeded"); // Success
        require(sale_status[_round].sale_finalized == 0, "Finalized");
        require(_lock_details[0] != _lock_details[1] && _lock_details[0] != _lock_details[2] && _lock_details[1] != _lock_details[2], "Randoms cannot be equal");
        require((_lock_details[3] == 5 && msg.sender != adminAddress) || msg.sender == adminAddress, "Invalid slippage");

        uint256[3] memory payAmounts = [uint256(0),0,0];
        uint256 liquidityToken = 0;
        ...
        // Fee collection
        if(payAmounts[0] > 0) {
            payout(feeAddress,payAmounts[0]);
        }
        ...
        // Liquidity lock
        if(sale_details.hasLiquidity == 1) {
            payAmounts[1] = (sale_status[_round].sale_raised_amount * sale_info[_round].mult / 10000 * sale_info[_round].liquidity / 100);
            liquidityToken = (sale_status[_round].sale_raised_amount * sale_info[_round].mult / 10000 * sale_details.listRate / convert * sale_info[_round].liquidity / 100);
            ERC20(sale_details.sale_token).transfer(address(saleVaultAddress), liquidityToken);
            _saleVault.createLock{value:payAmounts[1]}(_round, liquidityToken, payAmounts[1], sale_info[_round].end, sale_info[_round].end + (sale_info[_round].liquidityLock * 1 minutes), _lock_details[3], _lock_details[4]);
        }
        ...
        if(sale_details.hasIncentive == 1) {
            if(_incentive.getType(_round) == 1) {
                ...
                _incentive.findWinnerToken(_round, [_lock_details[0],_lock_details[1],_lock_details[2]]);
            } else if(_incentive.getType(_round) == 2) {
                _incentive.calcPrize(_round, sale_status[_round].sale_raised_amount);
                _incentive.findWinnerBase{value:_incentive.getPrizeAmount(_round)}(_round, [_lock_details[0],_lock_details[1],_lock_details[2]]);
    ...
    function remainingTokens(uint256 _round, uint256 _withdraw) public onlyOwner IsSaleEnded(_round) {
        require(saleStatusCheck(_round) == 2, "Not succeeded"); // Success

        SaleStatus storage saleStatus = sale_status[_round];

        uint256 token_rate = sale_info[_round].token_rate / convert;
        uint256 remaining = ((sale_info[_round].hardcap * token_rate) - saleStatus.sale_sold_amount <= saleStatus.sale_token_removed) ? 0 : (sale_info[_round].hardcap * token_rate) - saleStatus.sale_sold_amount - saleStatus.sale_token_removed;

        saleStatus.sale_token_removed = remaining > 0 ? saleStatus.sale_token_removed + remaining : saleStatus.sale_token_removed;

        require(remaining > 0, "Nothing to remove");

        if (_withdraw == 1) {
            TransferHelper.safeTransfer(address(sale_details.sale_token), owner, remaining);

            emit RemainingWithdrawn(_round, remaining);
        } else {
            TransferHelper.safeTransfer(address(sale_details.sale_token), address(deadaddr), remaining);

            emit RemainingBurned(_round, remaining);
        }
