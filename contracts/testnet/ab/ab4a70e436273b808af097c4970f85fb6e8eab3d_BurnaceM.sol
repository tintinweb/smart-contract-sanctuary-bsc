...
Reddit: https://www.reddit.com/u/burnace
...
contract BurnaceM is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    uint8 public maxBurnFee = 10; //Burnaaaace
    uint8 public maxWalletFee = 3; //Not more than 3% of every transaction can be set as a marketing or vault wallet!
    uint8 public maxBuybackFee = 10; //will be used, set as 8% Automatic Buyback and Burn
    ...
    bool public mintedByBurnace = true;
    ...
    constructor (address tokenOwner,string memory tokenName,
        ...
        _maxTxAmount = _tTotal.mul(setMxTxPer).div(
            10**2
        ).div(10); //this will allow to be able to set the antiwhale to less than 1% of the 100m total supply. protect our mtest investors from whales! 
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent >= minMxTxPercentage && maxTxPercent <=1000,"err");
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(
            10**2
        ).div(10); //this will allow to set the antiwhale to less than 1% of the total 100m supply. protect our mtest investors from whales! 
    }

    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner() {
        require(maxWalletPercent >= minMxWalletPercentage && maxWalletPercent <=1000,"err"); //setting a 1000 value will disable anti-whale and set max tx amount to 100% of 100m total supply
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, "Transfer amount exceeds the max transferrable amount this is to protect against whales.");

        if(from != owner() && to != owner() && to != address(0) && to != dead && to != pcsV2Pair){
            uint256 contractBalanceRecepient = balanceOf(to);
            require(contractBalanceRecepient + amount <= _maxWalletAmount, "Exceeds maximum wallet amount please contact support."); 
        }
