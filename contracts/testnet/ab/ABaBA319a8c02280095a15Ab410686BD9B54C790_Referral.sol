...
contract Referral is IReferral, Ownable, ReentrancyGuard {
    ...
    event SetRollerLevel(address indexed _RollerLevelAddr);
    ...
    function getRollerLevel(address _referrer) public override view returns (uint256){
        ReferrerInfo storage info = referrerInfo[_referrer];
    ...
    function claimLpCommission() public override nonReentrant {
        ...
        if(info.pendingLpCommission > 0){
            ...
            rollerToken.safeTransfer(referrer, tmpAmount);
            if(address(RollerLevel) != address(0)){
                RollerLevel.updateLevel(referrer);
            }
