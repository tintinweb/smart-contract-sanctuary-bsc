...
contract RematicVoteV1 is ReentrancyGuard, Ownable {
    ...
    function uintToString(uint256 v) public pure returns (string memory) {
        ...
        while (v != 0) {
            reversed[numOfDigits++] = bytes1(0x30 + uint8(v % 10));
            v = v / 10;
        }
        ...
        bytes memory s = new bytes(numOfDigits);
        for (uint256 i = 0; i < numOfDigits; i++) {
            s[i] = reversed[numOfDigits - i - 1];
        }
    ...
    function verifier(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encodePacked(_voter, _pollId))
            )
        );
    ...
    function voteQuestions(
        ...
        require(
            verifier(voter, _pollId, _signature) == signatureVerifier,
