...
contract GeneralNFTRewardUpgradeable is IPool,GovernanceUpgradeable {
    ...
    function initialize(
        ...
        __GeneralNFTRewardUpgradeable_init_unchained(spynNftToken, gegoFactory, rewardAddress, startTime, spynReferral);
    }
    ...
    */
        ...
        checkStart
    {
        ...
        _gegoFactory.takeFee(gegoId, depositFee, _rewardPool);
        // Referral Fee: 5, 4, 3, 2, 1%
        uint256 referralFeeTaken = payDepositReferralCommission(msg.sender, degoAmount, gegoId);
        // Burn 20%
        uint256 burnFee = degoAmount.mul(_burnFeeRate).div(_baseRate);
        _gegoFactory.takeFee(gegoId, burnFee, _deadWallet);
        // SPYN 10%
        uint256 spynFee = degoAmount.mul(_spynFeeRate).div(_baseRate);
        _gegoFactory.takeFee(gegoId, spynFee, _rewardPool);
        degoAmount = degoAmount.sub(depositFee).sub(referralFeeTaken).sub(burnFee).sub(spynFee);
    ...
    function harvest() public updateReward(msg.sender) checkStart {
        ...
        if(canHarvest(msg.sender)){
            if (reward > 0 || _rewardLockedUp[msg.sender] > 0) {
                ...
                // reward for team
                uint256 harvestFeeTaken = reward.mul(_harvestFeeRate).div(_baseRate);
                uint256 harvestFeeTakenWithDecimal = harvestFeeTaken.div(REWARDS_PRECISION_FACTOR);
                if(harvestFeeTakenWithDecimal>0 && _rewardPool != address(0)){
                    _rewardERC20.safeTransfer(_rewardPool, harvestFeeTakenWithDecimal);
                }
                uint256 leftReward = reward.sub(harvestFeeTaken);

                uint256 referralFeeTaken = payHarvestReferralCommission(msg.sender, reward);
                leftReward = leftReward.sub(referralFeeTaken);
    ...
    function payDepositReferralCommission(address _user, uint256 _depositedAmount, uint256 _gegoId) internal returns (uint256 feeTaken) {
        if (address(_spynReferral) != address(0)) {
            ...
            for (uint256 i = 0; i < _depositReferralCommissionRates.length; i ++) {
                ...
                if (commissionAmount > 0 && referrersByLevel[i] != address(0)) {
                    feeTaken = feeTaken.add(commissionAmount);
                    address token = _gegoFactory.takeFee(_gegoId, commissionAmount, referrersByLevel[i]);
...
interface ISpynNFTFactoryUpgradeable {
    ...
    function takeFee(uint256 tokenId, uint256 feeAmount, address receipt) external returns (address);
