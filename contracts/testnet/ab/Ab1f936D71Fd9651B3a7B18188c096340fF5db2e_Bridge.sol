...
contract Bridge is Initializable, BscBank, BridgeBankPausable, Ownable, Validator, ReentrancyGuardUpgradeable {
    ...
    function factory() view public returns (address) {
        return address(0);
    }
    ...
    function lock(
        ...
        if (msg.value > 0) {
            ...
            lockFunds(payable(msg.sender), _recipient, _token, symbol, _amount, _chainName);
        }
        // BEP20 deposit
        else {
            ...
            lockFunds(payable(msg.sender), _recipient, _token, symbol, afterLock - beforeLock, _chainName);
        }
    }
    ...
    function unlock(
        ...
        if (tokenAddress == address(0)) {
            ...
            require(BridgeToken(tokenAddress).balanceOf(address(this)) >= _amount, "Insufficient ERC20 balance.");
        }
    ...
    function emergencyWithdraw(
        ...
        address tokenAddress,
        uint256 _amount
    ) public onlyOperator whenPaused nonReentrant validatorPrecheck(sigV, sigR, sigS) {
        require(_checkEmergencySig(sigV, sigR, sigS, tokenAddress, _amount), "Invalid signature");
        // Check if it is BNB
        if (tokenAddress == address(0)) {
            address thisadd = address(this);
            require(thisadd.balance >= _amount, "Insufficient balance.");
    ...
    function refund(
        ...
        if (_tokenAddress == address(0)) {
            address thisadd = address(this);
            require(thisadd.balance >= _amount, "Insufficient BNB for delivery.");
        } else {
            require(BridgeToken(_tokenAddress).balanceOf(address(this)) >= _amount, "Insufficient ERC20 for delivery.");
        }
...
contract BscBank {
    ...
    event LogUnlockFee(address _owner, address _token, string _symbol, uint256 _fee, bytes32 _interchainTX);
...
contract Validator {
    ...
    function _changeThreshold(uint256 _newThreshold) internal {
        require(_newThreshold > 0, "INVALID NUMBER");
