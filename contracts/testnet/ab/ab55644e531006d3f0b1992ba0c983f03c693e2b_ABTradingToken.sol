...
contract ABTradingToken is ERC20, Ownable {
    ...
    constructor(
        ...
        dividendTracker = new TokenDividendTracker(rewardToken, tokenBalanceForDividendReward);
    ...
    function getBuyTokenRewardsFee() external view onlyOwner returns(uint256) {
        return buyTokenRewardsFee;
    }
    function setBuyTokenRewardsFee(uint256 amount) public onlyOwner {
        buyTokenRewardsFee = amount;
    }
    function getSellTokenRewardsFee() external view onlyOwner returns(uint256) {
        return sellTokenRewardsFee;
    }
    ...
    function _transfer(
        ...
        emit logFees(AmountLiquidityFee, AmountTokenRewardsFee, AmountMarketingFee);
        ...
        if( canSwap &&
            ...
            emit logText("Start swapping");
            swapping = true;
            if(AmountLiquidityFee > 0) swapAndLiquify(AmountLiquidityFee);
            if(AmountTokenRewardsFee > 0) swapAndSendDividends(AmountTokenRewardsFee);
            swapping = false;
            emit logText("Finished swapping");
        }
        ...
        if(takeFee) {
            ...
            emit logText("Finished taking fees");
        }
        ...
        if(!swapping) {
            emit logText("Processing dividend");

            uint256 gas = gasForProcessing;

            try dividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                emit logDividendFees(iterations, claims, lastProcessedIndex);
                emit ProcessedDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                emit logText("Finished Processing dividend");
            }
            catch {
