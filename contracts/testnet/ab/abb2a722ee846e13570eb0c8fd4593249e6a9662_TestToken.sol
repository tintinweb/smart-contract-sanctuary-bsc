...
contract TestToken is Context, IERC20, Ownable {
    ...
    address public _donationWallet = payable(address(0x123));
   ...
   function setDonationWallet(address payable wallet) external onlyOwner {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 walletFees = buyFee.marketingFee +
                sellFee.marketingFee + buyFee.burnFee +
                sellFee.burnFee + buyFee.donationFee + sellFee.donationFee;

            uint256 forMarketing = contractTokenBalance.mul(buyFee.marketingFee + sellFee.marketingFee).div(walletFees + buyFee.liquidityFee + sellFee.liquidityFee );
            uint256 forDonation = contractTokenBalance.mul(buyFee.donationFee + sellFee.donationFee).div(walletFees + buyFee.liquidityFee + sellFee.liquidityFee);
            uint256 forBurn = contractTokenBalance.mul(buyFee.burnFee + sellFee.burnFee).div(walletFees + buyFee.liquidityFee + sellFee.liquidityFee);

            uint256 forLiquidity = contractTokenBalance - forMarketing - forDonation - forBurn;
           transferFrom( address(this),_marketingWallet, forMarketing);
           transferFrom(address(this), _donationWallet, forDonation);
            transferFrom( address(this),_burnWallet, forBurn);
