...
// File: contracts/giro/nft1155.sol
...
contract MyToken is ERC1155, Ownable{
    ...
    constructor(string memory name,string memory symbol, address receiveAddress) ERC1155("") {}

    modifier onlySupportedPaymentToken(address coinAddress) {
        require(
            isPaymentTokenSupported(coinAddress),
            "NFTMarketplace: unsupport payment token"
        );
        _;
    }

    function addPaymentToken(address coinAddress) 
        external 
        onlyOwner 
    {
        require(
        ...
        require(
            _supportedPaymentTokens.add(coinAddress),
            "NFTMarketplace: already supported"
        );
    }

    function isPaymentTokenSupported(address coinAddress)
        public
        view
        returns (bool)
    {
        return _supportedPaymentTokens.contains(coinAddress);
    }
...
//-------------------------SET VARRIBLE---------------------------------------//
    ...
    function setCostpurchase1 (uint256 cost) public onlyOwner{
        COST=cost;
    }


    function purchaseBox(address account,address coinAddress, uint256 id, uint256 amount, bytes memory data)
        public
        onlySupportedPaymentToken(coinAddress)
    {   
        uint256 price = COST * amount;
        require(IERC20(coinAddress).allowance( account,address(this))>=price,"del du tien ");
        require(IERC20(coinAddress).balanceOf(msg.sender)>price," del du tien ");
        require(amount <= MAX_MINT, "toi da 5 mot luc ");
        require(_purchaseAmounts[msg.sender] + amount <= MAX_PURCHASE, " du so luong r ");

        IERC20(coinAddress).transferFrom(msg.sender, RECEIVE_ADDRESS,price);
    ...
    function openBox1(address account_open,uint256 id_open,uint256 amount_open,bytes memory data,uint256[] memory id_claim,uint256[] memory amounts_claim)
        public 
        onlyOwner 
    {
