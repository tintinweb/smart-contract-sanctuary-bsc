...
contract AllStarsFarm is Ownable {
    ...
    uint256 public constant WITHDRAW_FEE = 10; // 10% of the withdrawing amount,  TODO: change by exact number
    ...
    function addRewardToken(
        ...
        if (rewardToken.rewardPerBlock == 0) {
            rewardTokens.push(token_);
            rewardToken.id = rewardTokens.length - 1;
        }
        rewardToken.rewardPerBlock = getRewardPerBlock(apr_);
    ...
    function removeRewardToken(address token_) external onlyOwner {
        RewardToken storage rewardToken = rewardTokenInfo[token_];
        require(rewardToken.rewardPerBlock > 0, "AllStarsFarm: RewardToken is not in list!");
    ...
    function stake(
        ...
        require(amount_ >= MIN_STAKE_AMOUNT, "AllStarsFarm: Insufficient amount!");
        if (isLowRisk_) {
