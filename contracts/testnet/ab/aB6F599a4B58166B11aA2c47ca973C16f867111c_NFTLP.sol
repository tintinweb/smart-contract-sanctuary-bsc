...
contract NFTLP is Ownable {
  ...
  ArmyRankNFT public  armyRankAddress;
  EmblemNFT public emblemAddress;
  ...
  constructor (address _honorAddress, Busd _busdAddress, address _armynftAddress)  {
    honorAddress = HonorProtocol(_honorAddress);
    busdAddress = _busdAddress;
    armyRankAddress = ArmyRankNFT(_armynftAddress);
  }
  //-----------------------------------------------------------------------------------------------------------------------------------
  // Owner
  ...
  function setnftAddress(address _armynftAddress, address _emblemnftAddress) external onlyOwner {
    armyRankAddress = ArmyRankNFT(_armynftAddress);
    emblemAddress = EmblemNFT(_emblemnftAddress);
  }
  //-----------------------------------------------------------------------------------------------------------------------------------
  // Sell NFT back to DEV
  function armyRetirement(address payable _address, uint256 _tokenId) external { // Payable to send BNB
    require(busdAddress.balanceOf(address(this)) >= armyBUSD, "NO BUSD Liquidity, Contact the Dev");
    require(msg.sender == armyRankAddress.ownerOf(_tokenId),"You are not the owner of this TokenId");
    armyRankAddress.transferFrom(armyRankAddress.ownerOf(_tokenId), owner(), _tokenId);
    busdAddress.transfer(_address, armyBUSD);
    _address.transfer(gasFee);
    nftBuyers[_tokenId] = owner();
  }

  function emblemRetirement(address payable _address, uint256 _tokenId) external { // Payable to send BNB
    require(honorAddress.balanceOf(address(this)) >= tokenPrice, "NO HONOR Liquidity, Contact the Dev");
    require(busdAddress.balanceOf(address(this)) >= emblemBUSD, "NO BUSD Liquidity, Contact the Dev");
    require(msg.sender == emblemAddress.ownerOf(_tokenId),"You are not the owner of this TokenId");
    emblemAddress.transferFrom(emblemAddress.ownerOf(_tokenId), owner(), _tokenId);
    honorAddress.transfer(_address, tokenPrice);
    busdAddress.transfer(_address, emblemBUSD);
    _address.transfer(gasFee);
    nftBuyers[_tokenId] = owner();
  }
  //-----------------------------------------------------------------------------------------------------------------------------------
  ...
  function nftArmyRankAddress() public view returns (address) {
    return address(armyRankAddress);
  }
  function nftEmblemAddress() public view returns (address) {
    return address(emblemAddress);
  }
  function tokenAddress() public view returns (address) {
    return address(honorAddress);
  }
  ...
  function withdrawToken() external onlyOwner {
    honorAddress.transfer(owner(), tokenBalance());
  }
  // Withdrawal Busd LP
