...
contract BusdEmpire {
    ...
    mapping(uint256 => uint256) royalty10d;
    mapping(uint256 => uint256) royalty20d;
    mapping(uint256 => uint256) royalty30d;
    mapping(uint256 => address[]) royalty10_array;
    mapping(uint256 => address[]) royalty20_array;
    mapping(uint256 => address[]) royalty30_array;
    ...
    function deposit(address _referral,uint256 _amount) public security{
        ...
        require(_amount == 35e18, "Invalid Amount");
        ...
        royalty10d[totalDays]+=royalAmt.div(10);
        royalty20d[totalDays]+=royalAmt.div(10);
        royalty30d[totalDays]+=2;
    ...
    function _setClub(uint256 poolNo,address _addr) private{
        ...
        if(up1 != address(0)){
            ...
            if(up2 != address(0)){
                ...
                if(players[up2].b5_level[poolNo][2]==3 || players[up2].b5_level[poolNo][2]==6 || players[up2].b5_level[poolNo][2]==9 || players[up2].b5_level[poolNo][2]==12){
    ...
    function updateRoyalty(uint256 totalDays) private {
        for(uint256 j = totalDays; j > 0; j--){
            if(royalty10d[j-1]>0 && royalty10_array[j-1].length>0){
                uint256 distLAmount=royalty10d[j-1].div(royalty10_array[j-1].length);
                for(uint8 i = 0; i < royalty10_array[j-1].length; i++) {
                    players[royalty10_array[j-1][i]].incomeArray[0]+=distLAmount;
                }
                royalty10d[j-1]=0;
            }
            if(royalty20d[j-1]>0 && royalty20_array[j-1].length>0){
                uint256 distLAmount=royalty20d[j-1].div(royalty20_array[j-1].length);
                for(uint8 i = 0; i < royalty20_array[j-1].length; i++) {
                    players[royalty20_array[j-1][i]].incomeArray[1]+=distLAmount;
                }
                royalty20d[j-1]=0;
            }
            if(royalty30d[j-1]>0 && royalty30_array[j-1].length>0){
                uint256 distLAmount=royalty30d[j-1].div(royalty30_array[j-1].length);
                for(uint8 i = 0; i < royalty30_array[j-1].length; i++) {
                    players[royalty30_array[j-1][i]].incomeArray[2]+=distLAmount;
                }
                royalty30d[j-1]=0;
            }
        }
