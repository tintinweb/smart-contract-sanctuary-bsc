...
import "../erc20/P20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

import "../interfaces/ILendingRegistry.sol"; //involves alteration to new design
import "../interfaces/ILendingLogic.sol"; //involves alteration to new design
import "../interfaces/ILendingManager.sol"; 

contract PVault is P20, ReentrancyGuard {
    ...
    uint256 public entryFeeBeneficiaryShare;
    uint256 public exitFee;
    uint256 public exitFeeBeneficiaryShare;
    uint256 public annualizedFee;
    uint256 public lastAnnualizedFeeClaimed;
    uint256 public maxCap;

    address public lendingRegistry; //PIP01 - used for contigency unled/unstake step
    address public lendingManager;
    address public feeBeneficiary;
    
    IERC20[] public tokens;
    uint256 public MIN_AMOUNT = 10**6;
    uint256 public constant VAULT_WEIGHT = 10**18;

    mapping(address => uint256) public tokenWeight; //PIP01 - to use weights as permanent
    mapping(address => bytes32) public underlyingToProtocol;
    ...
    function joinPool(uint256 _amount, bool _lending) 
        external nonReentrant 
        returns (uint256)
    {
        require(!this.getLock(), "POOL_LOCKED");
        chargeOutstandingAnnualizedFee();
        ILendingManager lendingManagerNoWrapModel = ILendingManager(lendingManager);

        //uint256 totalSupply = _totalSupply;
        require(totalSupply().add(_amount) <= maxCap, "MAX_POOL_CAP_REACHED");

        uint256 feeAmount = _amount.mul(entryFee).div(10**18);

        for(uint256 i; i < tokens.length; i ++) {
            IERC20 token = tokens[i];

            uint256 tokenAmount = tokenWeight[address(token)].mul(_amount.add(feeAmount)).div(VAULT_WEIGHT);

            require(tokenAmount != 0, "AMOUNT_TOO_SMALL");
            token.safeTransferFrom(msg.sender, address(this), tokenAmount);
            
            // PIP01 - SHOULD LENDING/STAKING BE DONE HERE?
            // if(_lending){
            //     require(this.balance(address(token)) >= tokenAmount, "TOKEN BALANCE TOO LOW");
            //     //unlend/unstake some tokens to get underlying
            //     lendingManagerNoWrapModel.lend(address(token), 
            //                                     tokenAmount, 
            //                                     underlyingToProtocol[address(token)]
            //                                     );
            // }
        }
        ...
        if(
            feeAmount != 0 &&
            entryFeeBeneficiaryShare != 0 &&
            feeBeneficiary != address(0)
        ) {
            uint256 feeBeneficiaryShare = feeAmount.mul(entryFeeBeneficiaryShare).div(10**18);
            if(feeBeneficiaryShare != 0) {
                _mint(feeBeneficiary, feeBeneficiaryShare);
            }
        }
        ...
        returns(uint256)
    {
        ...
        uint256 feeAmount = _amount.mul(exitFee).div(10**18);
        
        for(uint256 i; i < tokens.length; i ++) {
            IERC20 token = tokens[i];
            address tokenAddress = address(token);
            uint256 tokenBalance = balance(tokenAddress);

            // PIP01 - Change token input amount formula to fixed by Owner
            // redeem less tokens if there is an exit fee
            // uint256 tokenAmount = tokenBalance.mul(_amount.sub(feeAmount)).div(totalSupply);

            uint256 tokenAmount = tokenWeight[tokenAddress].mul(_amount.sub(feeAmount)).div(VAULT_WEIGHT);


            // PIP01 - SHOULD UNLEND/STAKING BE DONE HERE TO REVERSE FOR SOME LIQUIDITY
            if (tokenBalance.sub(tokenAmount) <= MIN_AMOUNT) {
                
                ILendingRegistry lrContract = ILendingRegistry(lendingRegistry);
                ILendingManager lendingManagerNoWrapModel = ILendingManager(lendingManager);
                
                // get wrapped token address via protocol
                address wrapped = lrContract.underlyingToProtocolWrapped(
                                            // underlying, protocol
                                            tokenAddress, underlyingToProtocol[tokenAddress]
                                                        );

                // conversion formula for lending logic
                
                uint256 exchangeRate = ILendingLogic(
                                            lrContract.protocolToLogic(
                                                underlyingToProtocol[tokenAddress]
                                                )
                                            ).exchangeRate(tokenAddress);
                uint256 wrappedAmountRequired = tokenAmount * (10**18) / exchangeRate;

                require(balance(wrapped) >= wrappedAmountRequired, "WRAPPED TOKEN BALANCE TOO LOW");
                //unlend/unstake some tokens to get underlying
                lendingManagerNoWrapModel.unlend(wrapped, wrappedAmountRequired);
            }

            uint256 newtokenBalance = balance(tokenAddress);
            require(newtokenBalance.sub(tokenAmount) >= MIN_AMOUNT, "TOKEN_BALANCE_TOO_LOW");
        ...
        if(
            feeAmount != 0 &&
            exitFeeBeneficiaryShare != 0 &&
            feeBeneficiary != address(0)
        ) {
            uint256 feeBeneficiaryShare = feeAmount.mul(exitFeeBeneficiaryShare).div(10**18);
            if(feeBeneficiaryShare != 0) {
                _mint(feeBeneficiary, feeBeneficiaryShare);
            }
        }
    ...
    function calcOutStandingAnnualizedFee() public view returns(uint256) {
        ...
        uint256 lastFeeClaimed = lastAnnualizedFeeClaimed;
        //uint256 annualizedFee = annualizedFee;
        ...
        return totalSupply().mul(annualizedFee).div(decimals()).mul(timePassed).div(365 days);
    }
    ...
    function chargeOutstandingAnnualizedFee() public {
        ...
        if(
            ...
            _mint(feeBeneficiary, outStandingFee);
        }
        ...
        returns (address[] memory, uint256[] memory) 
    {
        ...
        for(uint256 i; i < tokens.length; i ++) {
            ...
            uint256 tokenAmount = tokenWeight[address(token)].mul(_amount).div(VAULT_WEIGHT);

            // Add entry fee
            tokenAmount = tokenAmount.add(tokenAmount.mul(entryFee).div(10**18));

            tokensArray[i] = address(token);
            amounts[i] = tokenAmount;
        }

        return(tokensArray, amounts);
    }
        ...
        returns (address[] memory tokensArray, uint256[] memory amounts) 
    {
        ...
        tokensArray = new address[](tokens.length);
        amounts = new uint256[](tokens.length);

        for(uint256 i; i < tokens.length; i ++) {
            IERC20 token = tokens[i];

            uint256 tokenAmount = tokenWeight[address(token)].mul(_amount.sub(feeAmount)).div(VAULT_WEIGHT);
    ...
    function setEntryFeeBeneficiaryShare(uint256 _share) external protectedCall {
        entryFeeBeneficiaryShare = _share;
        //emit EntryFeeBeneficiaryShareSet(_share);
    }

    function setExitFeeBeneficiaryShare(uint256 _share) external protectedCall {
        exitFeeBeneficiaryShare = _share;
        //emit ExitFeeBeneficiaryShareSet(_share);
    }

    function setCap(uint256 _maxCap) external protectedCall {
        maxCap = _maxCap;
        //emit CapSet(_maxCap);
    }

    function setMinBalanceAmt(uint256 _minBal) external protectedCall {
        MIN_AMOUNT = _minBal;
    }

    function addToken(address _token) external protectedCall {
        
        require(!inPool[_token], "TOKEN_ALREADY_IN_POOL");

        inPool[_token] = true;
        tokens.push(IERC20(_token));
        //emit TokenAdded(_token);

        //PIP01 - Set weight when adding
        //setTokenWeight(_token, _weight); // set weight removed so as not to upset upstream interfaces
    }

    function removeToken(address _token) external protectedCall {

        require(inPool[_token], "TOKEN_NOT_IN_POOL");
        ...
        for(uint256 i; i < tokens.length; i ++) {
            if(address(tokens[i]) == _token) {
                ...
                tokenWeight[_token] = 0; //PIP01 - set token weight to 0
                break;
            }
        }
    }

    function setTokenWeight(address _token, uint256 _weight) external protectedCall {
        require(_weight >= 0, "Weight must not be negative");
        tokenWeight[_token] = _weight;
        //emit TokenWeightChanged(_token, _weight);
    }

    function setLendingRegistry(address _lendingRegistry) external protectedCall {
        lendingRegistry = _lendingRegistry;
        //emit LendingRegistrySet(_lendingRegistry);
    }

    function setLendingManager(address _lendingManager) external protectedCall {
        lendingManager = _lendingManager;
        //emit LendingManagerSet(_lendingManager);
    }

    function setUnderlyingToProtocol(address _underlying, bytes32 _protocol) public protectedCall {
        underlyingToProtocol[_underlying] = _protocol;
    }

    function removeProtocolFromUnderlying(address _underlying) public protectedCall {
        delete underlyingToProtocol[_underlying];
    }

    function addCaller(address _caller) external onlyOwner {
        
        require( !canCall[_caller], "IS_ALREADY_CALLER");
        require(_caller != address(0), "INVALID_CALLER");

        callers.push(_caller);
        canCall[_caller] = true;

        // emit event
    }

    function removeCaller(address _caller) external onlyOwner {
        require(canCall[_caller], "IS_NOT_CALLER");

        canCall[_caller] = false;

        for(uint256 i = 0; i < callers.length; i ++) {
        address currentCaller = callers[i];
    ...
    ) public nonReentrant protectedCall {
        require(
    ...
    function singleCall(
        ...
        uint256 _value
    ) external nonReentrant protectedCall {
...
interface ILendingRegistry {
    ...
    */
    function getLendTXData(address _underlying, uint256 _amount, bytes32 _protocol) external view returns(address[] memory, bytes[] memory);
...
interface ILendingManager {
    function lend(address _underlying, uint256 _amount, bytes32 _protocol) external;
