...
contract FOTAGamePVP is Auth, EIP712Upgradeable {
  mapping (address => uint) userTodayLose;
  ...
  function getUserTodayLose(address _user) external view returns (uint) {
    return userTodayLose[_user];
  }

  function setUserTodayLose(address _user, uint _todayLose) external {
    userTodayLose[_user] = _todayLose;
  }

  function setTotalWinInDay(address _user, uint _totalWinInDay) external {
    totalWinInDay[_user] = _totalWinInDay;
  }
}
