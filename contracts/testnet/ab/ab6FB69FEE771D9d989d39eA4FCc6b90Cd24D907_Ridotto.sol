...
contract Ridotto is ERC20PresetMinterPauserUpgradeable, UsingLiquidityProtectionService {
    ...
    function __Ridotto_init(address _nextOwner) public initializer {
...
contract RidottoDeployer {
    ...
    constructor(address _nextOwner) {
        Ridotto rdt = new Ridotto();
        ProxyAdmin admin = new ProxyAdmin();
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            address(rdt), address(admin), abi.encodeWithSelector(rdt.__Ridotto_init.selector, _nextOwner));
