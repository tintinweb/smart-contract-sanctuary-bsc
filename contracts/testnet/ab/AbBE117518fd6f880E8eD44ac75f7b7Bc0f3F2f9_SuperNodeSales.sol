...
interface ISuperNodeNFT {
    function mint(address) external;
}

contract SuperNodeSales is Ownable, ReentrancyGuard {
    IERC20 public BUSD;
    ISuperNodeNFT public superNodeNFT;

    uint256 private constant MAX_NODE = 250; 
    uint256 private contribution = 5000 * 10**18; 
    address private recipientAddress;  
    uint256 private startTimestamp;
    uint256 private endTimestamp;
    uint256 private totalSales;
    uint256 private totalNodeSold;

    mapping(address => bool) private whitelist;
    mapping(address => bool) private participants;

    constructor(address _busd, address _superNodeNFT, uint256 _startTimestamp, uint256 _endTimestamp, address _recipientAddress) {
        BUSD = IERC20(_busd);
        superNodeNFT = ISuperNodeNFT(_superNodeNFT);
    ...
    function contribute() external nonReentrant {
        ...
        require(totalNodeSold < MAX_NODE, "Sold out");
        require(!isParticipant(msg.sender), "Already purchased before");
        require(isWhitelisted(msg.sender), "Not whitelisted");
        require(BUSD.balanceOf(msg.sender) >= contribution, "Insufficient BUSD");

        // Forward funds to treasury
        bool status = IERC20(BUSD).transferFrom(msg.sender, recipientAddress, contribution);
        require(status, "Forward funds failed");

        // Add contribution amount to totalBnbPool
        totalNodeSold++;
        totalSales = totalSales + contribution;
        participants[msg.sender] = true;

        // Mint SuperNode NFT to user
        superNodeNFT.mint(msg.sender);

        emit Contributed(_msgSender(), contribution);
    }
    ...
    function getTotalNodeSold() external view returns(uint256) {
        return totalNodeSold;
    }
    ...
    function setSuperNodeNFT(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Zero address");
        superNodeNFT = ISuperNodeNFT(_newAddress);

        emit SetSuperNodeNFT(_newAddress);
    }
    ...
    function setContribution(uint256 _contribution) external onlyOwner {
        require(_contribution > 0, "Value must be larger than zero");
        contribution = _contribution;

        emit SetContribution(_contribution);
    }
    ...
    event Contributed(address _account, uint256 _contribution);
    event AddToWhiteList(address[] _accounts);
    event SetSuperNodeNFT(address _newAddress);
    event SetRecipientAddress(address _newAddress);
    event SetContribution(uint256 _contribution);
