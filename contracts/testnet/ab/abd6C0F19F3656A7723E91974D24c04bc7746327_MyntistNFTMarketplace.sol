...
contract MyntistNFTMarketplace is Initializable, UUPSUpgradeable, OwnableUpgradeable, Verification {
    ...
    function acceptOfferBid721(acceptOfferBid721Data memory _transferData) external payable {
        ...
        if(_transferData.stakeExists){
            transferStaking(_transferData.stakeIndex, _transferData.newOwner, myntistTokenAddress);
        }
    ...
    function buy721(buy721Data memory _buyData) external payable {
        ...
        if(_buyData.stakeExists){
            transferStaking(_buyData.stakeIndex, msg.sender, myntistTokenAddress);
        }
    ...
    function transferStaking(uint256 stakeIndex, address to, address tokenAddress) private {
        IERC20Token token = IERC20Token(tokenAddress);
