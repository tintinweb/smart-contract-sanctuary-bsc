...
contract DinoBreeding is OwnableUpgradeable {
    function initialize() public initializer {
        ...
        maxBreedCountByRarity[1] = 5;
        maxBreedCountByRarity[2] = 4;
        maxBreedCountByRarity[3] = 3;
        maxBreedCountByRarity[4] = 2;
        maxBreedCountByRarity[5] = 2;
    }
    ...
    mapping(uint256 => uint256) public scoreByIndex;
    mapping(uint256 => uint256) public maxBreedCountByRarity;
    ...
    function _get4Bits(uint256 _input, uint256 _slot)
        internal
        pure
        returns (uint8)
    {
    ...
    /// @dev Get expressing trait of Dino
    function expressingTraitsDino(uint256 _genes)
        public
        pure
        returns (uint8[7] memory)
    {
        uint8[7] memory express;
        for (uint256 i = 0; i < 7; i++) {
            express[i] = _get4Bits(_genes, i * 3);
        }
        return express;
    }

    /// @dev Get expressing class of Dino
    function expressingClassDino(uint256 _genes) public pure returns (uint8) {
        return expressingTraitsDino(_genes)[0];
    }

    /// @dev Get rarity of dino
    function getRarity(uint256 _genes) public view returns (uint256 rarity) {
        uint8[] memory traits = decodeDino(_genes);
        uint256 score = 0;
        for (uint256 i = 0; i < 21; i++) {
            if (i % 3 == 0) {
                if (i / 3 == 1 || i / 3 == 2 || i / 3 == 3 || i / 3 == 6) {
                    score += scoreByIndex[traits[i]];
                }
            }
        }
        if (score <= 8) {
    ...
    /// @dev Get max breed count by genes
    function getMaxBreedCountByGenes(uint256 _genes)
        public
        view
        returns (uint256)
    {
        uint256 rarity = getRarity(_genes);
        return maxBreedCountByRarity[rarity];
    }
    ...
    function breed(
        ...
        require(!isOldGenes[sireDino.id], "DinoBreeding: Sire is not evolved");
        require(
            !isOldGenes[matronDino.id],
        ...
        uint256 sireClass = expressingClassDino(sireDino.genes);
        uint256 matronClass = expressingClassDino(matronDino.genes);
