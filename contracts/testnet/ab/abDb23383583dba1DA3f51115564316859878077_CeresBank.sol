...
contract CeresBank is CeresBase, ICeresBank, Ownable {
    ...
    /* ---------- Public Func -- Staking Mint && Redeem ---------- */
    function mintFromStaking(address _collateral, uint256 _collateralAmount) external override onlyStakings
        returns (ICeresBank.MintResult memory result){

        require(factory.isStakingMineable(_collateral) == true, "Collateral is not available to mint in CeresBank");
        ...
        SafeERC20.safeTransfer(IERC20(_collateral), vault, _collateralAmount);

        // calc amounts
        (uint256 ascMintTotal, uint256 mintToCRS, uint256 mintToVol) = _calcMintAmount(_collateral, _collateralAmount);
        ...
        // update cr
        uint256 decreaseRatioStep = CeresLibrary.calcRatioStep(_collateral, _collateralAmount, ratioStep, minRatioStep, maxRatioStep);
        ...
        return ICeresBank.MintResult(_collateralAmount, crsBurnAmount, volAmountTransferred, mintToCol, mintToCRS, mintToVol, minterCRSBonus, stakingVolAddress);
    }
    ...
    function redeem(uint amount) external override onlyOwner {
        asc.burn(amount);
    }
