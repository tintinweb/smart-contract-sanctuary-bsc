...
contract Booster is Ownable, ERC1155 {
    ...
    function balanceBatch(address account, uint256[] memory ids) external view returns(uint256[] memory) {
        address[] memory addrs = new address[](ids.length);
        for (uint i = 0; i < ids.length; i++)
            addrs[i] = account;
        
        return (balanceOfBatch(addrs, ids));
    }
}
