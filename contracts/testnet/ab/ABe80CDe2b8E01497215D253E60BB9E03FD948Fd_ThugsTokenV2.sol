...
library HampuraSwapOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IHampuraSwapPair(pair).price0CumulativeLast();
        price1Cumulative = IHampuraSwapPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IHampuraSwapPair(pair).getReserves();
...
//import "@pancakeswap/contracts/libraries/HampuraSwapLibrary.sol";
library HampuraSwapLibrary {
    using SafeMath for uint;
    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'HampuraSwapLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'HampuraSwapLibrary: ZERO_ADDRESS');
    }
...
// $THUGS (thugs.fix)
// @dev [email protected]
contract ThugsTokenV2 is ERC20, Ownable, Pausable {
    using SafeMath for uint256;

    /// @notice HampuraSwap listing rate
    ...
    address public  WBNB = address(0xDf79692fd05e1AE67825f3151A1e5511a0d03792);

    /// @notice self-explanatory
    address public constant HampuraSwapFactory = address(0xbd1313b2ec591aE43B0f7F638AbF7EC9D9ca13C0);
    ...
    /// @notice liquidity sources (e.g. HampuraSwapRouter) 
    mapping(address => bool) public whitelistedSenders;

    /// @notice pancakeswap pair for THUGS/BNB
    address public HampuraSwapPair;
    ...
    function initializeTwap() external onlyInitialDistributionAddress {
        ...
        (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) = 
            HampuraSwapOracleLibrary.currentCumulativePrices(HampuraSwapPair);
    ...
    internal
    {
        // Initial Liquidity Pool (367500 THUGS) + HampuraSwap Pool (187500 THUGS)
    ...
    function _initializePair() public onlyOwner {
        (address token0, address token1) = HampuraSwapLibrary.sortTokens(address(this), address(WBNB));
        isThisToken0 = (token0 == address(this));
        HampuraSwapPair = HampuraSwapLibrary.pairFor(HampuraSwapFactory, token0, token1);

        setWhitelistedSender(HampuraSwapPair, true);
    }
