...
For each individual token, the intrinsic value must be deposited, then a token is mined. 
In this case, it is BNB with which each token is covered. 
Another special feature is that the value of a token cannot decrease, but it can increase.
When you mine tokens, you are credited with 93% of the value of the tokens, 
the other 7% of the value is credited to the owner of the contract to cover e.g. transaction costs and 
to create LP tokens to enable trading on exchanges. 
In addition, the value is mined a further 99 times and burned completely over a fixed period of time. 
The burn mechanism is the reason why the token is a Hodl token. 
The sale value is calculated based on the total amount of tokens and the value set in the contract. 
The purchase value is calculated based on the total amount of tokens minus the tokens that are still being burned. 
This ensures that the value of a token is maintained and can therefore only increase, 
which happens if someone trades in their tokens before the burning expires, or someone burns their tokens themselves.

In summary, minimal risk of loss (unless BNB crashes) if you wait, you lose a maximum of the 7%. On the other hand, 
if you buy the token below its intrinsic value on exchanges or sell it above its intrinsic value, you are sure to make a profit. 
...
buyRHT              - buy RHT for your value RHT, you do not need a BurnTokenTrigger beforehand, the token value is always calculated live.
...
BurnTokenTrigger    - Burns the token in the TokenBurns array and reduces the total supply and updates all values.
Burn                - You can burn your own token if you like.
sellRHT             - Burn your amount of tokens, the contract sends them the corresponding value (showRHTvalueForSell) * TokenAmount.
sellRHTforOwnerOfRHTcontract - is the same as described above, except that I sell directly, without waiting (simplifies administration).
...
contract RaddixHodlToken is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("RaddixHodlToken", "RHT") {
        RHTvalue = 100000 gwei; // This is the start value.
    }

    uint RHTvalue;
    uint private balanceContract;
    uint burningTimeInSeconds = 504; //5443200 = 9 weeks
    ...
    function buyRHT() external payable {
        require(msg.value >= RHTvalue);
        _BurnTokenTrigger();
        uint i = msg.value/_showRHTvalueForBuy();
        _mint(msg.sender, i*930000000000000000);
        _mint(owner(), i*70000000000000000);
        _mint(address(this), i*9900000000000000000);
        RHTvalue = _showRHTvalueForSell();
        tokenBurns.push(TokenBurnData(block.timestamp+burningTimeInSeconds, uint(i*9900000000000000000), burningTimeInSeconds));
    ...
    function showRHTvalueForBuy() public view returns (uint) {
        return _showRHTvalueForBuy();
    }

    function _showRHTvalueForBuy() private view returns (uint x) {
        uint sum = _showAllRHTinBurningMechanism();
        if(totalSupply() != 0){
            x = (balanceContract*10000000000000000)/(totalSupply() - sum);
        } else x = RHTvalue;
        
        return x;
    }

    function showAllRHTinBurningMechanism() public view returns (uint) {
        return _showAllRHTinBurningMechanism();
    }
    ...
    function BurnTokenInStruct(uint tokenBurnsId) private {
        ...
        if(secondsRemaining(tokenToBurn.timestamp) != 0) {
            ...
            _burn(address(this), tokenToBurn.amount);
            tokenToBurn.amount = 0;
            tokenToBurn.timeLeft = 0;
            remove(tokenBurnsId);
        }
    ...
    function BurnTokenTrigger() public {
        return _BurnTokenTrigger();
    }

    function sellRHT(uint RHTamountToSell) public {
        _BurnTokenTrigger();
        uint i = RHTvalue;
        _burn(msg.sender, RHTamountToSell);
    ...
    function showRHTvalueForSell() public view returns (uint) {
        return _showRHTvalueForSell();
    }

    function _showRHTvalueForSell() private view returns (uint i) {
        if(totalSupply() != 0){
            i = (address(this).balance*10000000000000000) / (totalSupply()-_TokenImagineBurnedTrigger());
        } else i = RHTvalue;
        return i;
    }
