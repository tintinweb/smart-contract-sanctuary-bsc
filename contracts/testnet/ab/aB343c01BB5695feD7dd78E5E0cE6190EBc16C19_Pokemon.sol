...
interface IHero {
    function mint(address to,uint256 pokemon_type) external returns (uint256);
}

contract Pokemon is ERC721Enumerable, Ownable,AccessControlEnumerable,IHero {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdTracker;
    string private _url;
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    event Mint(address to,uint256 pokemon_type,uint256 tokenid);

    constructor() ERC721("Pokemon", "PKT") {
    ...
    function mint(address to,uint256 pokemon_type) external override returns (uint256) {
        ...
        _mint(to, token_id);
        emit Mint(to,pokemon_type,token_id);
        return token_id;
    }
