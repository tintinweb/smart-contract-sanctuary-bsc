...
contract NFTRanking is Ownable {
    ...
    function quickSort(UserInfo[] memory arr, int left, int right) internal pure {
        int i = left;
        int j = right;
        if (i == j) return;
        uint pivot = arr[uint(left + (right - left) / 2)].power;
        while (i <= j) {
            while (arr[uint(i)].power > pivot) i++;
            while (pivot > arr[uint(j)].power) j--;
            if (i <= j) {
    ...
    function getRank() external view returns(UserInfo[] memory) {
        UserInfo[] memory arr = userInfos;
