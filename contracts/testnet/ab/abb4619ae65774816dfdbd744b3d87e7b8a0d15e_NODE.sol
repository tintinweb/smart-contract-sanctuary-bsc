...
interface IDividendDistributor {
    ...
    function mintShare(address shareholder, uint256 amount) external;
    function deposit() external;
    function process(uint256 gas) external;
    function getDistributor() external view returns (address);
    function getDistributorInfo() external view returns (uint256, uint256, uint256, uint256, uint256, uint256);
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 public RWRD = IBEP20(0x3d793dD4a700c852DA4F26DBF672Fac104043129);
    ...
    function getDistributorInfo() external override view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (totalShares, totalDividends, totalDistributed, dividendsPerShare, dividendsPerShareAccuracyFactor, shareholders.length);
    }
    ...
    function mintShare(address shareholder, uint256 amount) external override onlyToken {
        addShareholder(shareholder);
    ...
    function deposit() external override onlyToken {
        uint256 amount = RWRD.balanceOf(_distributor);
        totalDividends = amount;
        dividendsPerShare = dividendsPerShareAccuracyFactor.mul(amount).div(totalShares);
    ...
    function process(uint256 gas) external override onlyToken {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            ispaid(shareholders[currentIndex]);
    ...
    function ispaid(address shareholder) internal {
        shares[shareholder].paid = true;
    }

    function setcreditor(address shareholder) internal {
        shares[shareholder].paid = false;
    }

    function getUnpaidEarnings(address shareholder) public override view returns (uint256) {
...
contract NODE is IBEP20, Ownable {
    ...
    IBEP20 public Reward = IBEP20(0x3d793dD4a700c852DA4F26DBF672Fac104043129);
    ...
    uint256 public _maxTxAmount = 2000;
    uint256 public _maxWalletToken = 2000;
    ...
    constructor () Ownable(msg.sender) {
        ...
        treasury = 0xbC0Ecd9304c2BB1a35e5649f420cE8b294f12B5f; 
    ...
    function burn(address account, uint256 amount) external onlyOwner {
        ...
        if(!isDividendExempt[account] && _balances[account] == 0)
        isshareholder[account] = false;
    ...
    function CreateNode_Tokens(address account, uint256 _count) public saleIsOpen {
        ...
        (bool success) = IBEP20(Reward).transferFrom(msg.sender, address(this), price_Tokens(_count));
        ...
        if(!isDividendExempt[account] && !isshareholder[account]) {
            isshareholder[account] = true;
            try distributor.mintShare(account,_count) {} catch {}
        } else { try distributor.setShare(account,_balances[account]) {} catch {}
            }

        IDividendDistributor(distributor).deposit();
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isDividendExempt[recipient] && _balances[recipient] == 0)
        isshareholder[sender] = true;

        //Exchange tokens
        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
        _balances[recipient] = _balances[recipient].add(amount);

        if(!isDividendExempt[sender] && _balances[sender] == 0)
        isshareholder[sender] = false;
    ...
    function depositRewards(uint256 _amount) external onlyOwner {
        ...
        IBEP20(Reward).transferFrom(msg.sender, IDividendDistributor(distributor).getDistributor(), _amount);
        IDividendDistributor(distributor).deposit();
    }
    ...
    function _getDistributorInfo() public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return IDividendDistributor(distributor).getDistributorInfo();
    }

    function _getUnpaidEarnings(address shareholder) public view returns (uint256) {
        return IDividendDistributor(distributor).getUnpaidEarnings(shareholder);
    }
