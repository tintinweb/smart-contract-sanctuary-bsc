...
contract BRealEstate is Ownable {
    ...
    function withdrawIdo(string calldata _symbol) external isAdmin {
        ...
        uint256 totalBre = SafeMath.mul(
            stocks[_symbol].idoPrice,
            stocks[_symbol].circulatingSupply
        );
        BREToken.safeTransfer(_msgSender(), totalBre);

        stocks[_symbol].idoWithdrew = totalBre;

        emit WithdrawIdo(_symbol, _msgSender(), totalBre, block.timestamp);
    }
