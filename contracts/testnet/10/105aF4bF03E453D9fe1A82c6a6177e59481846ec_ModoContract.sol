...
contract ModoContract is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = "Modo";
    string public _symbol = "MODO";
    ...
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(modoRiskFreeFundFee).add(
            supplyControlFee
        );
    ...
    constructor() ERC20Detailed("Modo", "MODO", uint8(DECIMALS)) Ownable() {
        // BSC PCS Mainnet
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(
                _treasuryFee.add(modoRiskFreeFundFee)
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToTreasuryAndMRFF = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(treasuryFund).call{
            value: amountETHToTreasuryAndMRFF.mul(treasuryFee).div(
                treasuryFee.add(modoRiskFreeFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(modoRiskFreeFund).call{
            value: amountETHToTreasuryAndMRFF.mul(modoRiskFreeFundFee).div(
                treasuryFee.add(modoRiskFreeFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no MODO token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
