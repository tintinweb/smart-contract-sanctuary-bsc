...
contract Alora is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    AloraDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0xA11020AeF3973635E8594929C2b1584e055d014c);
    address payable public _buybackWalletAddress = payable(0xA11020AeF3973635E8594929C2b1584e055d014c);
    ...
    constructor() BEP20("ALORA", "ALR") 
    {
        dividendTracker = new AloraDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Alora: The dividend tracker already has that address");
        AloraDividendTracker newDividendTracker = AloraDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Alora: The new dividend tracker must be owned by the Alora token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Alora: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Alora: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Alora: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Alora: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Alora: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Alora: Cannot update gasForProcessing to same value");
...
contract AloraDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Alora_Dividend_Tracker", "Alora_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Alora_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Alora_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Alora contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Alora_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Alora_Dividend_Tracker: Cannot update claimWait to same value");
