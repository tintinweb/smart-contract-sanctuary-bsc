...
contract AirDropper is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 constant TIME_LIMIT = 48 hours;
    IERC20 public immutable GOEN = IERC20(System.GOEN);
    ...
    constructor(address _operator, address _treasuryVault) public {
        OPERATOR = _operator;
    ...
    function unlock(
        ...
        require(block.timestamp < startReleaseTimestamp.add(TIME_LIMIT), "Airdrop is over");
        ...
        totalAmount[_seasonID] = totalAmount[_seasonID].sub(_amount);
        emit Unlock(_seasonID, _account, _amount);
    }

    function takeBackUnclaimToken(uint256 _seasonID) external onlyOperator {
        uint256 startReleaseTimestamp = startReleaseTimestamps[_seasonID];
        require(block.timestamp > startReleaseTimestamp.add(TIME_LIMIT), "The season isn't finish");
        GOEN.transfer(treasuryVault, totalAmount[_seasonID]);
        totalAmount[_seasonID] = 0;
    }
    ...
    function addSeason(
        ...
        totalAmount[_seasonID] = _totalAmount;
    }
    ...
    function setTreasuryVault(address _vault) public onlyOwner {
        treasuryVault = _vault;
    }

    function withdraw(IERC20 token, uint256 _amount) public onlyOwner {
