...
contract Lottery is Initializable {
    ...
    modifier checkTicketSold(address buyer) {
        require(isTicketSold[buyer] == true, "Please Buy Ticket First");
        _;
    }
    modifier onlyOwner() {
    ...
    function buyTicket(uint256 _amount) external {
        ...
        require(isTicketSold[msg.sender] != true, "Ticket All Ready Bought");
            ...
            for (uint256 j = 0; j < roundPerson.length; j++) {
                if (
                    ...
                    roundPerson[j] = roundInfo(
                        ...
                        true,
                        amountPerWinner,
                        roundPerson[j].wininnigCategory,
    ...
    function stopLottery() external onlyOwner {
        require(
            tickets.length == 0,
            "Please Annouce Winner Before StoppingLottery"
        );
