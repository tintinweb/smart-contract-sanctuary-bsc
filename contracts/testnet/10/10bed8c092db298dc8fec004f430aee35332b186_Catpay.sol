...
contract Catpay is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        if(!takeFee && isActiveFees){
            ...
            uint256 takeLiquidity_fee = 0;
            uint256 takeMarketing_fee = 0;
            uint256 takeBurn_fee = 0;

            _balances[sender] = _balances[sender].sub(receivedAmount,"Insufficient Balance");
            takeLiquidity_fee = _takeLiquidity(receivedAmount);
            takeMarketing_fee = _takeMarketing(receivedAmount);
            takeBurn_fee = _takeBurn(receivedAmount);

            receivedAmount = receivedAmount.sub(takeLiquidity_fee).sub(takeMarketing_fee).sub(takeBurn_fee);
