...
contract ACMIDO is Ownable, ReentrancyGuard {
    ...
    function addRelationEx(address recommer) external nonReentrant {
        ...
        (bool stat, address father) = relation.addRelationEx(recommer, msg.sender);
        if (stat) {
            IERC20(USD).transferFrom(msg.sender, father, 10 * 10**18);
    ...
    function addPosition(uint startAmount) external onlyOwner {
        ...
        // 初始化第一期
        epochs[pl].push(Epoch(startTime, startTime + 12 * 60 * 60, startAmount * 10**18, 0, 0, 0, pl, 0));
        // 初始化第二期
        epochs[pl].push(Epoch(startTime + 12 * 60 * 60, startTime + 24 * 60 * 60, startAmount * 10**18, 0, 0, 0, pl, 1));
    }
    ...
    function addEpoch(uint idx) public nonReentrant {
        ...
        if (e.startTime < block.timestamp) {
            ...
            epochs[idx].push(Epoch(startTime, startTime + 12 * 60 * 60, p.startAmount, 0, 0, 0, idx, epochs[idx].length));
        }
    }
    ...
    function crowdfunding(uint idx, uint amount, address tokenContract) external {
        ...
        require (acmClaim.getFixedParticipatedFund(h) == 0 && participatedAmount <= e.amount / 100, "Insufficient supply");
    ...
    function listCrowdfundingStat(address addr, uint pidx, uint stat) external view returns (CrowdfundingStat[] memory stats) {
        ...
        for (uint i = 0; i< es.length; i++) {
            uint amount = acmClaim.getFixedParticipatedFund(hash(pidx, i, addr));
            if (amount > 0 && (stat == 0 || stat == getCrowdfundingStat(addr, pidx, i))) {
                l++;
            }
        }
    ...
    // 领取爆仓的LP
    function getLiquidationFunds(uint pidx, uint eidx) external view returns (uint) {
        ...
        return acmClaim.getFixedParticipatedFund(h) * subStageAmount / totalAmount;
    } 
