...
contract MGStake is Context,Ownable{
    ...
    mapping(address => mapping(uint256=>uint256)) nextBuy;  //限制用户每隔24小时每个类型的矿机只能购买一次,币安链每隔3秒出一个区块
    mapping(uint256 => mapping(uint256 => uint256)) public minerGroup;         //矿机类型对应信息  1.矿机所需u 2.算力 3.库存
    ...
    constructor () {
        ...
        fundAddress=0x49f61e12FA4eAcc29ea2a5227B7c2e832a956D11;    //0x243D86513b76844245CC177f72127dc852F9a7b7
        bufferAddress=0x49f61e12FA4eAcc29ea2a5227B7c2e832a956D11;//0xf04cdAF9BEeb756a239C89169943fb3171bc8628 
    ...
    function store(uint256 minerId) public view returns(uint256){
        return minerGroup[minerId][3];
    }
    ...
    function adminMiner(uint256 minerId,uint256 usdt,uint256 hashRate,uint256 storeTotal) external onlyOwner returns(bool){
        minerGroup[minerId][1]=usdt;
        minerGroup[minerId][2]=hashRate;
        minerGroup[minerId][3]=storeTotal;
        return true;
    }
    ...
    //购买矿机
    function deposit(address referrer,uint256 minerId) external returns (bool) {
        require(referrer!=address(0)&&addressToId[referrer]>0,"Invalid referrer");
        address sender=_msgSender();
        require(sender!=address(0),"Invalid sender");
        uint blockNum=block.number;
        require(nextBuy[sender][minerId] <=blockNum,"Invalid Block Num");
        require(store(minerId)>0,"Invalid store");
        nextBuy[sender][minerId]=blockNum.add(28800);
        minerGroup[minerId][3]=minerGroup[minerId][3].sub(1);
        ...
        (uint256 usdtNum,uint256 hashRate,uint256 _dayDeposit)=_minerType(minerId);
        require(minerDayDeposit[startBlock][minerId]<_dayDeposit,"Invalid unable deposit");
        minerDayDeposit[startBlock][minerId]=minerDayDeposit[startBlock][minerId].add(1);
    ...
    function _dividend(address sender,address referrer,uint256 payUsdt,uint256 payMc) internal returns (bool){
        ...
        uniswapV2Router.swapExactTokensForTokens(brunMGAmount,1,path,deadAddress,block.timestamp.add(1000000));
