...
contract SousChef {
    ...
    // The VETERANS TOKEN!
    IBEP20 public veterans;
    // rewards created per block.
    uint256 public rewardPerBlock;

    // Info.
    PoolInfo public poolInfo;
    // Info of each user that stakes Veterans tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = veterans.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 veteransSupply = veterans.balanceOf(address(this));
        if (veteransSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(veteransSupply));
        poolInfo.lastRewardBlock = block.number;
    }

    // Deposit Veterans tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];

        updatePool();
        veterans.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Veterans tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        veterans.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        veterans.safeTransfer(address(msg.sender), user.amount);
