    ...
    ProtocolFees
{
        ...
        returns (LibOrder.OrderInfo memory orderInfo)
    {
        ...
        // Allow taker amount = 0 because of presale

        // Validate order availability
        if (orderInfo.orderMakerAssetFilledAmount >= order.oData.mAmount) {
    ...
    function _assertFillableOrder(
        ...
        if (takerAssetFillAmount < order.oData.tAmount) {
    ...
    function _settleNativeOrder(
        ...
        if (receiveAmount > 0) _transferEth(to, receiveAmount);
