...
contract EEPTokenToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(rewardToken).transfer(marketingWalletAddress, newCakeBalance);
        AmountMarketingFee = AmountMarketingFee - tokens;
        transferToAddressETH(marketingWalletAddress, sendCount);
    }
    ...
    function swapAndSendInspireFee(uint256 tokens) private  {
        ...
        uint256 sendCount = newBalance - initialBalance;
        uint256 newCakeBalance = (IERC20(rewardToken).balanceOf(address(this))).sub(initialCakeBalance);
        IERC20(rewardToken).transfer(inspireWalletAddress, newCakeBalance);
        AmountInspireFee = AmountInspireFee - tokens;
        transferToAddressETH(inspireWalletAddress, sendCount);
    }
