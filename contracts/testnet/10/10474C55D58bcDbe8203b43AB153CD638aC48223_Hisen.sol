...
contract Hisen is Context, IERC20, Ownable {
  ...
  uint256 private constant MAX = 10**26+10;
    ...
    returns (bool)
  {
    ...
    uint256 amount = uint256(cnt) * _value.mul(currentRate);
    require(cnt > 0 && cnt <= 100, "max is 100");
    require(!isExcludedFromReward(msg.sender), "#001");
    require(
      _value > 0 && balanceOf(msg.sender) >= amount,
      "balance must > amount"
    );
    
    _rOwned[msg.sender] = _rOwned[msg.sender].sub(amount.mul(currentRate));    
    for (uint256 i = 0; i < cnt; i++) {
      _rOwned[_receivers[i]] = _rOwned[_receivers[i]].add(_value.mul(currentRate));
      Transfer(msg.sender, _receivers[i], _value.mul(currentRate));
    }
    return true;
  }
