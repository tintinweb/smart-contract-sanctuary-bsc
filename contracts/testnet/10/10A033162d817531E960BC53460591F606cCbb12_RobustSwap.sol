...
contract RobustMasterChef is Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	...
	struct PoolInfo {
		...
		uint256 accRBSPerShare;     // Accumulated RBS per share, times 1e18. See below.
	...
	//RobustSwap
	RobustSwap public immutable rbs;
	//Dev address
	...
	IRobustReferral public immutable rbsReferralContract;
	...
	constructor(
		RobustSwap _rbs,
		IRobustReferral _rbsReferralContract,
		uint256 _startBlock,
	...
	*/		
	function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate) external onlyOwner nonDuplicated(_lpToken){
		...
		require(_depositFeeBP <= MAXIMUM_DEPOSIT_FEE, "RobustMasterChef::add:Invalid deposit fee basis points");
	...
	*/			
	function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate) external onlyOwner validatePoolByPid(_pid) {
		require(_depositFeeBP <= MAXIMUM_DEPOSIT_FEE, "RobustMasterChef::set:Invalid deposit fee basis points");
	...
	*/		
	function pendingRBS(uint256 _pid, address _user) external view validatePoolByPid(_pid) returns (uint256) {
		...
		if (block.number > pool.lastRewardBlock && pool.lpSupply != 0 && totalAllocPoint > 0) {
			...
			accRBSPerShare = accRBSPerShare.add(rbsReward.mul(1e18).div(pool.lpSupply));
		}

		uint256 pending = user.amount.mul(accRBSPerShare).div(1e18).sub(user.rewardDebt);
	...
	*/
	function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
		...
		//Mint RBS rewards
		if (rbs.mintedSupply().add(rbsReward.mul(11).div(10)) <= rbs.MAXIMUM_SUPPLY()) {
			// The whole amount can be minted
			rbs.mint(devAddress, rbsReward.div(10));
			rbs.mint(address(this), rbsReward);
			pool.accRBSPerShare = pool.accRBSPerShare.add(rbsReward.mul(1e18).div(pool.lpSupply));
			pool.lastRewardBlock = block.number;		

		} else if (rbs.mintedSupply() < rbs.MAXIMUM_SUPPLY()) {
			// The amount can be partially minted
			rbsReward = rbs.MAXIMUM_SUPPLY().sub(rbs.mintedSupply());
			rbs.mint(address(this), rbsReward);
			pool.accRBSPerShare = pool.accRBSPerShare.add(rbsReward.mul(1e18).div(pool.lpSupply));
			pool.lastRewardBlock = block.number;

		} else {
			//No amount can be minted
			pool.allocPoint = 0;
			rbsPerBlock = 0;
			pool.lastRewardBlock = block.number;
		}
	...
	*/
	function deposit(uint256 _pid, uint256 _amount, address _referrer) external nonReentrant validatePoolByPid(_pid) {
		...
		user.rewardDebt = user.amount.mul(pool.accRBSPerShare).div(1e18);
	...
	*/		
	function payOrLockupPendingRBS(uint256 _pid) internal {
		...
		uint256 pending = user.amount.mul(pool.accRBSPerShare).div(1e18).sub(user.rewardDebt);
	...
	*/		
	function updateEmissionRate(uint256 _rbsPerBlock) external onlyOwner {
		massUpdatePools();
		require(rbsPerBlock <= MAX_EMISSION_RATE, "RobustMasterChef::updateEmissionRate:Emission rate too high");
	...
	function payReferralCommission(address _user, uint256 _pending) internal {
		if (address(rbsReferralContract) != address(0) && referralCommissionRate > 0) {
			...
			if (referrer != address(0) && commissionAmount > 0) {
				...
				//Mint RBS referral commission
				if (rbs.mintedSupply().add(commissionAmount) <= rbs.MAXIMUM_SUPPLY()) {
					// The whole amount can be minted
					...
					// The amount can be partially minted
					commissionAmount = rbs.MAXIMUM_SUPPLY().sub(rbs.mintedSupply());
					rbs.mint(referrer, commissionAmount);
