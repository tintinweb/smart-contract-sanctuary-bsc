...
contract OVGPreSale is ReentrancyGuard {
    ...
    mapping(address => uint) public amount_per_user_remain;
    ...
    function preSaleBnb(uint amount) payable external nonReentrant onSale {
        ...
        require(amount_per_max - amount_per_user_remain[user] >= amount * 10 ** 18, "presale: address over max limit 3000");
        uint bnb_value = amount * getBnbPrice();
        require(msg.value >= bnb_value, "presale: need more bnb");
        
        amount_per_user_remain[user] += amount * 10 ** 18;
