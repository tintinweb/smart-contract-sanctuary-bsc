...
// 24% Burnt to dead wallet
// 24% Liquidity
// 25% Pre-sale
// (Estimated Listing Price 0.000000002403)
...
// 15% LOCK WALLET 3 Months (NEKOPOT) This will be used to add "Liquidity or Burnt"
// 12% Neko Holder Airdrop 
// LP lock 6 Months
// ----------------------------------------------------------------------------
// REDISTRIBUTION VENTA: 16.5%
// >> 1% Burn
// >> 4% Back to the Liquidity Pool
// >> 2% To charity wallet
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1 * 1e6 * 1e12; //must hold 10,000,000+ tokens
    }
...
contract NEKOGAME is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public burnTokensAtAmount;          // 24% Burnt to dead wallet
    uint256 public liquidityTokensAtAmount;     // 24% Liquidity
    uint256 public presaleTokensAtAmount;       // 25% Pre-sale

    uint256 public lockTokensAtAmount;          // 15% Lock wallet 3 Months() This will be used to add "Liquidity or Burnt"
    uint256 public airdropTokensAtAmount;       // 12% Airdrop Wallet
    ...
    // lock wallet time
    uint public lockWalletInterval = block.timestamp + 3600 * 24 * 30 * 6;
    ...
    constructor() ERC20("$NKG", "NekoGame"){
        // address newOwner = address(0x94cDD0412438Aff7fA353AF01760ad84dfD88a21); // update to 0x94cDD0412438Aff7fA353AF01760ad84dfD88a21
    
        uint256 totalSupply = 1 * 1e12 * 1e12;
        
        holderWalletAtAmount = totalSupply * 5 / 100;           // 5% holder
        burnTokensAtAmount = totalSupply * 24 / 100;            // 24% Burnt to dead wallet
        liquidityTokensAtAmount = totalSupply * 24 / 100;       // 24% Liquidity
        presaleTokensAtAmount = totalSupply * 25 / 100;         // 25% Pre-sale
        maxTransactionAmount = totalSupply * 20 / 1000;         // 2% maxTransactionAmountTxn
        lockTokensAtAmount = totalSupply * 150 / 1000;           // 15% LOCK WALLET 6 Months () This will be used to add "Liquidity or Burnt"
        airdropTokensAtAmount = totalSupply * 120 / 1000;        // 12% Airdrop Wallet
    	...
    	marketingWallet = address(0x493beAf84b6677103F6BF0fF5152834F361bcbb0);  // set as marketing wallet
        charityWallet = address(0x40f1A64B139512A5B37F0E4d4a9a4c945E19E385);    // set as charity wallet
        lockWallet = address(0xa401e6b1bbDc0d95bf32b1bE4BDB67FeD8453080);       // set as lock wallet

        devWallet = address(0x5326363FC3514FfE60a217B90BbF082256363B22);        // set as dev wallet
        liquidityWallet = address(0xe87983Cf99b666Ee9856D8c62123d96c8e8D7362);  // set as liquidity wallet
        presaleWallet = address(0x74Cd22A2b16ddBA9d06f4ec8B0f5927D080F4F26);  // set as presale wallet
        airdropWallet = address(0x37F2222c34eDC38F101754faBa36b561dc9DdAb4);    // set as airdrop wallet
        BurnWallet = address(0xb12c589Fc05b7dC4865781e9Cf3C3746b1a1F7Ce);       // set as Burn wallet
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum > (totalSupply() * 5 / 1000)/1e12, "Cannot set maxTransactionAmount lower than 0.5%");
