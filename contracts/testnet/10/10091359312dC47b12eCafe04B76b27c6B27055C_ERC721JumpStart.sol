    ...
    Whitelist
{
    ...
    function mint(uint256 _numberOfNfts) public onlyWhitelisted {
        (
            uint256 startDate,
            uint256 endDate,
            bool refundActive
    ...
    function mintChainPay(address _minter, uint256 _numberOfNfts) private {
        uint256 fee = mintPrice * _numberOfNfts;

        if (priceDiscount > 0 && block.timestamp < priceDiscountEndDate) {
            fee = fee.mul(SafeMathUpgradeable.sub(100, priceDiscount)).div(100);
        }

        require(
            IERC20(paymentToken).balanceOf(_minter) >= fee,
            "Insufficient balance"
        );
        require(_pay(_minter, fee), "Payment failed");
