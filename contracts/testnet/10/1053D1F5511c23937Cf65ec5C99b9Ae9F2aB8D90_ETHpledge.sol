...
contract ETHpledge is IERC20,Ownable {
    ...
    uint256 public baijinzongshu=500;
    uint256 public huangjinzongshu=1000;
    ...
    uint256[6] public _team =[0,20,40,60,80,100];
    ...
    constructor(IERC20 _usdt,IERC20 _other,address _nft) {
    ...
    function getmymessage(address _my) public view returns (
            uint256 performance1,uint256 fatherperformance1,uint256 teamperformance1,address inviter1,uint256 sharenumber1,uint256 sharenumber2,uint256 bonus1,uint256 level22,uint256 teambonus1) {
                teambonus1=teambonus[_my];
                level22=level[_my];
                sharenumber2=youxiaosharenumber[_my];
                bonus1=bonus[_my];
        return (performance[_my],fatherperformance[_my],teamperformance[_my],inviter[_my],sharenumber[_my],sharenumber2,bonus1,level22,teambonus1);
    }
 ...
 //  event Newkuangji(uint zombieId, uint kuangji_level, uint kuangji_day, uint kuangji_chan); 
  function _createKuangji(address  _add) internal {
    uint kuangjinum = 0;
    uint kuangjiday = 0;
    uint kuangjichan = 0;
   ...
   Kuangjis.push(kuangji(kuangjinum,1,1));
   uint id = Kuangjis.length-1;
    ...
    function  ceshicreateKuangji(address  _add)  public   {
        require(Kuangjis.length<=2500,"pledgein low 1");
        _createKuangji(  _add);
  ...
  function wakuang(uint _kuangjiId) public {
    //decimals2=17;
  //  require(usdt.balanceOf(msg.sender)>=liliangfee*_ci*10**decimals2,"USDT balance too low");
   // usdt.transferFrom(msg.sender,address(this), liliangfee*_ci*10**decimals2);
    ...
    function teamfenhong(uint256 ltj) private{
        ...
        uint256 otherb=ltj*_price*10**18;
        for (int256 i = 0; i < 99; i++) {
         ...
         if(level[cur]<2 && teamperformance[cur]>=5001*10**18){level[cur]=2;l2[inviter[cur]]+=1;}
         if(level[cur]<3 && teamperformance[cur]>=20001*10**18){level[cur]=3;l3[inviter[cur]]+=1;}
         if(level[cur]<4 && teamperformance[cur]>=100001*10**18){level[cur]=4;l4[inviter[cur]]+=1;}
         if(level[cur]<5 && teamperformance[cur]>=200000*10**18){level[cur]=5;l5[inviter[cur]]+=1;}
 ...
 function importSeedFromThird(uint256 seed) public view returns (uint) {
    ...
    function  ETHreceive()  external returns (bool) {
        ...
        bool y1= otheryuejiazhi>= 200*10**18;
        ...
        if(shouyi>0){
            ...
            if (inviter[msg.sender] != address(0)&&performance[inviter[msg.sender]]>0) {
                other.transfer(inviter[msg.sender], chanbi*400/_baseFee);
