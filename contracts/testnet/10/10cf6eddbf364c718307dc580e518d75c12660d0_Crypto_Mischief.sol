...
contract Crypto_Mischief {
    ...
    bool private startContract; // Testnet - Fri Aug 19 2022 14:00:00 GMT+0000 https://www.unixtimestamp.com/
    ...
    uint256 public LOTTERY_STEP = 21600; // 6hrs
    uint256 public TOP_DEPOSIT_STEP = 86400;
    uint256 public TIME_STEP = 86400;
    uint256 public LOTTERY_TICKET_PRICE = 2 ether; /** 2 ether **/
    uint256 public MAX_LOTTERY_TICKET = 20;
    uint256 public MAX_LOTTERY_PARTICIPANTS = 200;
    uint256 public MAX_LOTTERY_POOL_PER_ROUND = 400 ether;
    ...
    function updateActionCooldown(uint256 value) external {
        require(msg.sender == owner);
        ACTION_COOLDOWN = value * 60 * 60; 
    }
    ...
    /*********************************************************/
     ...
     //first deposit will activate all event features so no need for manual setup.
     //will not be activated when the genesis wallet makes the initial funding of the contract.
    ...
    function buyEggs(address ref, uint256 amount) public nonReentrant {
        ...
        require(startContract || msg.sender == genesis); //genesis wallet will be funding and will eliminate early advantage.
    ...
    function hatchEggs(bool isCompound) public {
        User storage user = users[msg.sender];
        require(startContract || msg.sender == genesis);
    ...
    function sellEggs() public nonReentrant {
        require(startContract, "Contract is not Started.");
