    ...
    ERC721EnumerableUpgradeable
{
    ...
    /**
     *  @notice _paymentToken is interface of payment token.
     */
    ...
    /**
     *  @notice _isOpened mapping from token ID to opened status of box
     */
    mapping(uint256 => bool) public _isOpened;
    ...
    event Bought(address caller, uint256 times, uint256 timestamp);
    event Opened(address caller, uint256 tokenId, uint256 timestamp);
    event Deposit(address caller, uint256 amount, uint256 timestamp);
    event Withdraw(address caller, uint256 amount, uint256 timestamp);
        ...
        returns (string memory)
    {
        ...
        uint256 isOpened = _isOpened[tokenId] ? 1 : 0;

        return
            bytes(currentBaseURI).length > 0
                ? string(
                    abi.encodePacked(currentBaseURI, "/", isOpened, ".json")
                )
                : ".json";
    }
    ...
    function withdraw(uint256 amount) public onlyAdminOrOwner {
        uint256 balanceToken = _paymentToken.balanceOf(address(this));
        require(
    ...
    /**
     *  @notice Get is own token ID of owner address.
     *
     *  @dev    Only admin can call this function.
     */
    function tokenIdOfOwner(address owner, uint256 tokenId)
        public
        view
        returns (bool)
    {
    ...
    function open(uint256 tokenId) public nonReentrant {
        require(
            tokenIdOfOwner(_msgSender(), tokenId),
            "This token is not own !"
        );
        require(!_isOpened[tokenId], "Your NFT is opened !");
        _isOpened[tokenId] = true;
        _combatant.mint(_msgSender());
    ...
    function buy(uint256 _times) public nonReentrant {
        require(_tokenCounter + _times <= TOTAL_SUPPLY, "Sold out");
        ...
        // burn
        _paymentToken.transfer(address(1), PRICE_PER_NFT_BOX * _times * 2);
