...
contract BandZaiView is Ownable {
    ...
    string[4] eggOrMonstertype= ['bronze','silver','gold','platinum'];

    function setGameAddresses(address _address) external onlyOwner {
        gameAddresses = IAddresses(_address);
    }

    function getFullMonster(uint256 _monsterId) external view returns(
        string memory name,
        string memory uri,
        uint256[5] memory powers,// [water,fire,metal,air,stone]
        uint256[7] memory datas, // [state, xp, alchemyXp, level,creditForUpgrade,stamina, xpNeededToGoUp] 
        uint256[5] memory stats, // [wins, draw, loss, totalFight, totalRandomSelected]
        uint256[2] memory status // [status (free(0),training(1),coaching(2),working(3)), house of activity]
        ){

        IBandZaiNFT Monster = IBandZaiNFT(gameAddresses.getMonsterAddress());

        (,,,,string memory _uri,uint256[5] memory _powers ) = Monster.getMonster(_monsterId);

        MonsterStruct.Monster memory _monster = Monster.monster(_monsterId);

        uint256[7] memory _datas;
        _datas[0] = _monster.state;
        _datas[1] = _monster.xp;
        _datas[2] = _monster.alchemyXp;
        _datas[3] = _monster.level;
        _datas[4] = _monster.creditForUpgrade;

        _datas[5] = IFighting(gameAddresses.getGameAddress()).getFighterStamina(_monsterId);
        _datas[6] = Monster.getNextLevelUpPoints(_monster.level + 1) - _monster.xp;

        uint256[5] memory _stats = IStats(gameAddresses.getZaiStatsAddress()).getMonsterStats(_monsterId);

        uint256[2] memory _status = Monster.getStatus(_monsterId);

        return(_monster.name,_uri,_powers,_datas,_stats,_status);

    }

    function getNextEggDatas(uint256 _nursId) external view returns(
        uint256 idEgg,
        ...
        INursery Inurs = INursery(gameAddresses.getNurseryAddress());
        uint256 _type = Inurs.nextStateToMint(_nursId);
        MonsterStruct.EggsPrices memory _prices = Inurs.getEggsPrices(_nursId);

        MonsterStruct.MintedData memory _mintedDatas = Inurs.getNurseryMintedDatas(_nursId);

        uint256 _idEgg = _mintedDatas.bronzeMinted + _mintedDatas.silverMinted + _mintedDatas.goldMinted + _mintedDatas.platinumMinted + 1;

        return (
            _idEgg,
            eggOrMonstertype[_type],
            IOpenAndClose(gameAddresses.getOpenAndCloseAddress()).getNurseryName(_nursId),
            Inurs.getNextUnlock(_nursId),
            _type == 0 ? _prices.bronzePrice : _type == 1 ? _prices.silverPrice : _type == 2 ? _prices.goldPrice : _prices.platinumPrice
        );
