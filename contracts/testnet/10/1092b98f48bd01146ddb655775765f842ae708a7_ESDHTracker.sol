...
contract ESDHTracker is Initializable, OwnableUpgradeable {
    ...
    struct WeeklyTracker {
        ...
        uint256 closeIndex; // for further use if needed
        uint256 distributionIndex; // for further use if needed
    }
    ...
    function initialize() initializer public{
        ...
        tokenContract = 0xeb0dfF613265B8e6677B6E37c744eCa6D6D25948;


        //testnet eth
        _proposedTokens.add(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca); // ETH

        // _proposedTokens.add(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); // ETH
        // _proposedTokens.add(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D); // SHIB
        // _proposedTokens.add(0xbA2aE424d960c26247Dd6c32edC70B295c744C43); // DOGE
        // _proposedTokens.add(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c); // BTC

        minimumBalanceForRewards = 1e9 * (10**18); //1 000 000 000

        operatorWallets[msg.sender] = true;
    }
    ...
    modifier onlyToken() {
        require(msg.sender == tokenContract, "ESDHTracker: only token");
        _;
    }

    modifier onlyOperator() {
        require(operatorWallets[msg.sender], "ESDHTracker: only operators");
        _;
    }
    ...
    function setLastFriday(uint256 epoch) external onlyOwner {
        lastFriday = epoch;
    }
    ...
    function getUserHistory(address user) public view returns (UserHistory[] memory) {
        ...
        for (uint256 i = 1; i < weekIndex; i++) {
            ...
            history[i].tokens = weeklyTrackers[i].esdhToken.symbol();
            history[i].decimals = weeklyTrackers[i].esdhToken.decimals();
        }
        return (history);
    }
    ...
    function addOperator(address _operator) external onlyOwner {
        require(operatorWallets[_operator] != true, "ESDHTracker: User is already set to this status");
        require(_operator != address(0), "ESDHTracker: Zero address cannot be an operator");
    ...
    function vote(address token) external {
        require(
            ...
            "ESDHTracker: ES Token cannot be BNB"
        );
        require(weeklyTrackers[week].voted[msg.sender] == false, "ESDHTracker: already voted");
        require(isEligible(msg.sender), "ESDHTracker: Caller is not eligible for the week");

        if (!_proposedTokens.contains(token)) {
            require(allowNewTokens, "ESDHTracker: New tokens are not allowed");
    ...
    function removeFromES(address account) public onlyToken {
    ...
    function canClose() public view returns (bool) {
        if (block.timestamp >= lastFriday + 1 weeks) {
            return true;
        } else {
            return false;
        }
    ...
    function _closeWeek() private {
        require(canClose(), "ESDHTracker: Too early to close the week");
        ...
        for (uint256 i = 0; i < _eligibleBalanceHolders.length(); i++) {
        ...
        address esdhToken = weeklyTrackers[week].winningToken;
        if (esdhToken != address(0)){
            weeklyTrackers[week].esdhToken = IERC20Upgradeable(esdhToken);
        } else {
            weeklyTrackers[week].esdhToken = weeklyTrackers[week - 1].esdhToken;
        }

        if(weeklyTrackers[week].totalBNB > 0) {
            _swapBNBForESDHToken(weeklyTrackers[week].totalBNB);
        }
    ...
    function _swapBNBForESDHToken(uint256 amount) private {
        uint256 initialBalance = weeklyTrackers[week].esdhToken.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(weeklyTrackers[week].esdhToken);
        ...
        uint256 newBalance = weeklyTrackers[week].esdhToken.balanceOf(address(this)) - initialBalance;
    ...
    function distributeESDHReward() external onlyOperator {
        _distributeESDHReward();
    }

    function _distributeESDHReward() private {
        require(weeklyTrackers[weekIndex].isClosed == true, "ESDHTracker: Week is not closed");

        if(weeklyTrackers[weekIndex].rewardPerHolder > 0) {
            for (uint256 i = 0; i < weeklyTrackers[weekIndex].eligibleUsers.length; i++) {
                address user = weeklyTrackers[weekIndex].eligibleUsers[i];
                weeklyTrackers[weekIndex].esdhToken.transfer(user, weeklyTrackers[weekIndex].rewardPerHolder);
