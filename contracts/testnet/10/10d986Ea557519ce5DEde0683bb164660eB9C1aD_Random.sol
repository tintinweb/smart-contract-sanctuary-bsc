...
contract Random {
    ...
    function GenerateABCMatrixs(uint r1, uint c1, uint r2, uint c2) public returns(Matrix[3][2] memory)
    {
    ...
    function GenerateABCCovMatrixs(uint r1, uint c1, uint r2, uint c2) public returns(Matrix[3][2] memory)
    {
        int256[] memory matrixA = new int256[](r1 * c1);
        int256[] memory matrixB = new int256[](r2 * c2);
        for(uint i = 0;i< r1*c1;++i){
    ...
    function GenerateABCReLUMatrixs(uint r1, uint c1, uint r2, uint c2) public 
    ...
    function DivideSingleMatrix(int256[] memory matrixA, uint[2] memory shape)internal returns(Matrix[2] memory)
    {
        ...
        return [Matrix(matrixA, shape[0], shape[1]), Matrix(matrixA, shape[0], shape[1])];
    }
