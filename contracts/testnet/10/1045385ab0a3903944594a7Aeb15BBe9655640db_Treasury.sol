...
contract Treasury is ContractGuard, Operator {
    ...
    address public vaultFund;
    uint256 public vaultFundSharedPercent;
    ...
    event VaultFunded(uint256 timestamp, uint256 seigniorage);
    ...
    function setExtraFunds(
        ...
        address _vaultFund,
        uint256 _vaultFundSharedPercent,
        ...
        require(_vaultFund != address(0), "Magic Vault should be non-zero address");
        ...
        vaultFund = _vaultFund;
        vaultFundSharedPercent = _vaultFundSharedPercent;
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        ...
        uint256 _vaultFundSharedAmount = 0;
        if (vaultFundSharedPercent > 0) {
            _vaultFundSharedAmount = _amount.mul(vaultFundSharedPercent).div(10000);
            IERC20(crystal).transfer(vaultFund, _vaultFundSharedAmount);
            emit VaultFunded(block.timestamp, _vaultFundSharedAmount);
        }
        ...
        _amount = _amount.sub(_treasuryFundSharedAmount).sub(_vaultFundSharedAmount).sub(_lotteryFundSharedAmount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs || isBoostEnabled) {
            ...
            if (previousEpochCRSPrice > crystalPriceOne) {
                ...
                uint256 _mse = maxSupplyExpansionPercent.div(10000).mul(crystalPriceOne);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
