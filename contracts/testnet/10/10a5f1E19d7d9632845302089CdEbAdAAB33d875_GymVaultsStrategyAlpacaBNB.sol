    ...
    PausableUpgradeable
{
    ...
    function _farm() internal {
        ...
        IERC20Upgradeable(alpacaVaultContractAddress).safeIncreaseAllowance(fairLaunchAddress, ibWantAmt);
    ...
    function distributeFees(uint256 _earnedAmt) internal returns (uint256 _fee) {
        if (_earnedAmt > 0) {
            ...
            if (controllerFee > 0) {
                ...
                IERC20Upgradeable(alpacaTokenAddress).safeTransfer(operator, _fee);
                emit DistributeFee(alpacaTokenAddress, _fee, operator);
            }
        }
    }
}
