...
contract IrishFloki is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Irish Floki";
    string constant _symbol = "IRFL";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 100*10**9 * (10 ** _decimals);
    uint256 public _maxTxAmount = _totalSupply.div(500); // 0.2%
    uint256 public _maxWalletToken = _totalSupply.div(200); // 1% of total supply
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        ...
        if (msg.sender != owner && recipient != owner && recipient != address(0) && recipient != address(0xdead) && recipient != pair) {
