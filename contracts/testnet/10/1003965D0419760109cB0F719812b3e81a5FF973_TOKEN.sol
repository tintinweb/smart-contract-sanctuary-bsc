...
contract TOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
contract TOKEN is ERC20, Ownable, BaseToken {
    ...
    // onTotalSupply
    uint public onTotalSupply = 10 ** 18 * 500 ;
    
    uint public changeTaxOne = 1_000;
    uint  public changeTaxTwo = 1_000;
    uint public changeTaxThree= 1_000;
    uint public changeTaxFour = 1_000;
    uint public changeTaxFive = 1_000;

    // receiver Wallets will receive tax
    address public walletOne = 0x77547F859Ca0F1114c25Db094Bb0f682dD47512D;
    address public walletTwo = 0x77547F859Ca0F1114c25Db094Bb0f682dD47512D;
    address public walletThree =0x77547F859Ca0F1114c25Db094Bb0f682dD47512D;
    address public walletFour = 0x77547F859Ca0F1114c25Db094Bb0f682dD47512D;
    address public walletFive =0x77547F859Ca0F1114c25Db094Bb0f682dD47512D;

    // percetages for each tex receiver wallet
    uint public PercentageOne = 2_000;
    uint public PercentageTwo = 2_000;
    uint public PercentageThree = 2_000;
    uint public PercentageFour = 2_000;
    uint public PercentageFive = 2_000;

    uint256 public tokenRewardsFee = 4_000;
    // Divider 
    uint Divider = 100_000;
    ...
    constructor(address _cloneadd
        ...
        uint256 totalSupply_ = 1_000_000_000 ether;
        rewardToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;

        
        swapTokensAtAmount = totalSupply_.div(10_000); // 0.002%
        ...
        excludeFromFees(walletOne, true);
        // excludeFromFees(walletTwo, true);
        // excludeFromFees(walletThree, true);
        // excludeFromFees(walletFour, true);
        // excludeFromFees(walletFive, true);
        excludeFromFees(address(this), true);

        WhiteListTransferable[owner()] = true;
        WhiteListTransferable[walletOne] = true;
    ...
    function burn(address account, uint256 amount) public  {
        require(msg.sender == Ownable.owner() || BurnWhiteList[msg.sender],"You either be Onwer or BurnWhiteList Address");
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "TOKEN: The PanrewardSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _transfer(
        ...
        require(to != address(0), "ERC20: transfer to the zero address");
            ...
            if(WhiteListTransferable[from] || WhiteListTransferable[to]){
    ...
    function _takeFee(address from,uint256 amount)internal returns(uint256 fees){
        uint _one = totalSupply()<= onTotalSupply ? changeTaxOne:  PercentageOne; 
        uint _two = totalSupply()<= onTotalSupply ? changeTaxTwo:  PercentageTwo;
        uint _three = totalSupply()<= onTotalSupply ? changeTaxThree:  PercentageThree;
        uint _four = totalSupply()<= onTotalSupply ? changeTaxFour:  PercentageFour;
        uint _five = totalSupply()<= onTotalSupply ? changeTaxFive:  PercentageFive;
        // here function calculate Percentage and add into variable
        uint one    = ( amount * _one    ) / Divider ;
        uint two    = ( amount * _two    ) / Divider ;
        uint three  = ( amount * _three  ) / Divider ;
        uint four   = ( amount * _four   ) / Divider ;
        uint five   = ( amount * _five   ) / Divider ;
        uint reflectionFee = (amount * tokenRewardsFee   ) / Divider ;
    ...
    function updateDividenTracker(address _cloneadd,address _rewardToken) external onlyOwner{
        dividendTracker = TOKENDividendTracker(
            payable(Clones.clone(_cloneadd))
        );
        dividendTracker.initialize(
