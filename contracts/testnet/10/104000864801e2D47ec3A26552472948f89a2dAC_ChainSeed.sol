...
contract ChainSeed is AccessControl, ReentrancyGuard {
    ...
    // function _Seed() public returns(bytes32) {
    ...
    function test() public {
        for (uint256 i = 0; i < _providers.length; i++) {
            IChainLink dataFeed = IChainLink(_providers[i]);
            (uint80 _roundId, int256 _answer, , uint256 _updatedAt, ) = dataFeed
                .latestRoundData();

            data.push(Data(_roundId, _answer, _updatedAt));
        }
    }

    // function getdata() public view returns(Data[] memory){
    //     Data[] memory _data = new Data[](data.length);
    ...
    function getDta() public returns (uint256) {
        IChainLink dataFeed = IChainLink(
            0x298619601ebCd58d0b526963Deb2365B485Edc74
        );
        uint256 round = dataFeed.latestRound();
    ...
    function _secureRandomSeed() private returns (bytes32 seed) {
        ...
        bytes32 finalSeed = _getSeed(answer, updatedAt, roundId);
        data.push(Data(roundId, answer, updatedAt));

        // New Linked-Seed
        uint256 providerSafe = providerIndex + 1;
        uint256 seedLoopCount = (roundId % providerSafe) +
            ((roundId + block.number + uint256(finalSeed)) % providerSafe);
        _lastSeed = _getHashLoop(seedLoopCount, finalSeed);
