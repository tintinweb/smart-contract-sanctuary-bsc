...
contract WalletContract is Context, Ownable {
  ...
  function getExchangedAmount(uint amount, uint rate, uint decimalsStable, uint decimalsRct, bool buyOperation ) public pure returns (uint) {
    if(buyOperation) {
      return (amount).mul(_precisionRate).div(rate).mul(10**decimalsRct).div(10**decimalsStable);
    }
    return (amount).div(_precisionRate).mul(rate).div(10**decimalsRct).mul(10**decimalsStable);
  }
