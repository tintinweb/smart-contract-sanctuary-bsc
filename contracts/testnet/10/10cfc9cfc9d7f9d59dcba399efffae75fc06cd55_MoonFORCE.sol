...
// File: contracts/interfaces/IStakingContract.sol
...
interface IMoonshotMechanism {
    ...
    function shouldLaunchMoon() external view returns (bool);
}
...
contract MoonshotMechanism is IMoonshotMechanism {
    ...
    uint256 public minPeriod = 3 seconds;
    ...
    constructor() {
        ...
        admin = 0x41515a61d25F2F2E16084E79Bb06824bB6d7D0Bf;
    ...
    function pickMoonshot() internal {
        ...
        disbursalThreshold = disbursalValue * 10**3;
    ...
    function shouldLaunchMoon() external view override returns (bool) {
        return (IERC20(address(WBNB)).balanceOf(address(this)) >= disbursalThreshold && disbursalThreshold != 0); 
    }

    // calling pickMoonshot() required becasue upon contract deployment disbursalThreshold is initialized to 0
    function launchMoonshot() external override {
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) public onlyAdmin {
...
contract RewardDistributor is IRewardDistributor{
    ...
    uint256 public minPeriod = 5 seconds;
    ...
    function deposit(address token, uint256 amount) external override onlyToken {
        ...
        if(token == WBNB) {
            ...
            uint256 newAmount = IERC20(address(WBNB)).balanceOf(address(this)).sub(balanceBefore);
            
            totalBountys = totalBountys.add(newAmount);
            bountysPerShare = bountysPerShare.add(bountysPerShareAccuracyFactor.mul(newAmount).div(totalShares));
        }
    ...
    function process(uint256 gas) external override onlyToken {
        ...
        if(shouldDistribute(stakingContract)){
            distributeBountyStakingContract(stakingContract);
        }
    ...
    function setNewStakingContract(address _newStakingContract) external onlyAdmin {
        stakingContract = (_newStakingContract);
        StakingContract = IStakingContract(_newStakingContract);
    }

    // manually set distributor address
	// function updateRewardDistributorAddress(address payable newAddress) public onlyAdmin {
    //     require(newAddress != address(stakingContract), "The staking contract already has that address");
		
    //     stakingContract newStakingContract = stakingContract(newAddress);
    //     IStakingContract = newStakingContract;
    // }
    ...
    function distributeBounty(address keeper) internal {
        if(keeper == stakingContract) { return; }
    ...
    function distributeBountyStakingContract(address keeper) internal {
        if(shares[keeper].amount == 0){ return; }
        ...
        if(amount > 0){
            ...
            shares[keeper].totalRealised = shares[keeper].totalRealised.add(amount);
            shares[keeper].totalExcluded = getCumulativeBountys(shares[keeper].amount);
            StakingContract.depositWBNBToStakingContract(amount);
        }
    }
...
//--------------------------------------BEGIN MAPPINGS---------|
    ...
    // user states governing fee exclusion, blacklist status, bounty exempt (meaning no reflection entitlement)
...
//--------------------------------------BEGIN TOKEN PARAMS---------|
    ...
    // token details. minimumBuybackThreshold used in shouldAutoBuyback() to check if transfer amount is enough to trigger it automatically
...
//--------------------------------------BEGIN ROUTER, WBNB, BURN ADDRESS INFO---------|
    ...
    // launch time info for moonshot?
    uint256 public launchedAt;
    uint256 public launchedAtTimestamp;

//--------------------------------------BEGIN BUYBACK VARIABLES---------|

    // buyback function variables for determining values
...
//--------------------------------------BEGIN REWARD DISTRIB. AND MOONSHOT MECH. INSTANCES---------|

    //  Instantiate the RewardDistributor contract, which handles reflection calculations and entitlements.
    ...
    IMoonshotMechanism public MoonshotMechanismInterface;

    uint256 distributorGas = 500000;

//--------------------------------------BEGIN SWAP INFO---------|

    // swap state variables - ask tommy what swapThreshold is used for??
...
//--------------------------------------BEGIN CONSTRUCTOR AND RECEIVE FUNCITON---------|    
    ...
    constructor() ERC20("MoonForce", "FORCE") {
        ...
        MoonshotMechanismInterface = IMoonshotMechanism(moonshotAddress);
        ...
        _isExcludedFromFee[distributorAddress] = true;
        ...
        isBountyExempt[_msgSender()] = true;
...
//--------------------------------------BEGIN TOKEN GETTER FUNCTIONS---------|
    ...
    function totalSupply() public view override(ERC20, IPYE) returns (uint256) {
        return _tTotal;
    }

    // balanceOf function is identical to ERC-20 balanceOf fxn. As per tommy's request, function is still explicity declared.
    function balanceOf(address account) public view override returns (uint256) {
        return _balances[account];
    }

    // returns the balance of an address "keeper" which is an address entitled to Moonshot reflections.
    ...
    function depositLPFee(uint256 amount, address token) public onlyExchange {
        ...
        if(tokenIndex < pairsLength) {
            ...
            if(allowanceT >= amount) {
                ...
                try distributor.deposit(token, reflectionFeeAmount) {} catch {}
            }
        }
    }

    // allows user to set an address as bounty exempt
...
//--------------------------------------BEGIN DISTRIBUTOR FUNCTIONS---------|

    // set the specs for distribution amount and frequency
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external onlyOwner {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    // set gas settings for distributor
...
//--------------------------------------BEGIN TRANSFER FUNCTIONS---------|
   ...
   function _transfer(
        ...
        if(MoonshotMechanismInterface.shouldLaunchMoon()){ MoonshotMechanismInterface.launchMoonshot(); }

        //indicates if fee should be deducted from transfer
        bool takeFee = true;
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            takeFee = false;
        } else if(from != pyeSwapPair && to != pyeSwapPair){
            takeFee = false;
        }
