...
{
    string public constant VERSION = "TESTG";
...
Erc20C09Contract
{
    ...
    override
    {
        uint256 thisTokenForRewardToken = thisTokenForSwap * (shareMarketing + shareLper + shareHolder) / (shareMax - shareBurn);
        uint256 thisTokenForPoolToken = thisTokenForSwap * (shareLiquidity / 2) / (shareMax - shareBurn);
        uint256 thisTokenForLiquidity = thisTokenForSwap * (shareLiquidity / 2) / (shareMax - shareBurn);
        ...
        if (thisTokenForRewardToken > 0) {
            ...
            if (shareMarketing > 0) {
                doMarketing(rewardTokenForShare * shareMarketing / (shareMarketing + shareLper + shareHolder));
            }

            if (isUseFeatureLper && shareLper > 0) {
                doLper(rewardTokenForShare * shareLper / (shareMarketing + shareLper + shareHolder));
            }

            if (isUseFeatureHolder && shareHolder > 0) {
                doHolder(rewardTokenForShare * shareHolder / (shareMarketing + shareLper + shareHolder));
            }
        }
        ...
        if (thisTokenForPoolToken > 0) {
            ...
            uint256 poolTokenForLiquidity = IERC20(addressPoolToken).balanceOf(addressWrap);

            if (shareLiquidity > 0) {
                doLiquidity(poolTokenForLiquidity, thisTokenForLiquidity);
            }
        }
    }
...
Erc20C09FeatureTakeFeeOnTransfer
{
    ...
    override
    {
        ...
        if (_isSwapping) {
            ...
            if (
                ...
                super._transfer(from, to, amount);
            } else if (isUseFeatureTakeFeeOnTransfer && !isFromUniswapV2Pair && !isToUniswapV2Pair) {
    ...
    function doMarketing(uint256 rewardTokenForMarketing)
    internal
    {
        IERC20(addressRewardToken).transferFrom(addressWrap, addressMarketing, rewardTokenForMarketing);
    }
    ...
    {
        assembly {
            let __router := sload(addressWrap.slot)
