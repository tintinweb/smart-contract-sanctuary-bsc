...
interface IAdmins {
    ...
    function initialMintFee() external returns(uint256);
...
contract ERC721Factory {
    ...
    address[] public NFTS;
    address admin;
    mapping(address=>bool) public paidInitialMintFee;
    ...
    function createToken (string memory _tokenName, string memory _tokenSymbol,string memory _tokenURI) public payable returns(address,uint){
        ...
        if (paidInitialMintFee[_depCont]==false){

            require(msg.value>=IAdmins(admin).initialMintFee(),"invalid fee");
            address payable recevier = payable(IAdmins(admin).feeReceiver());
            recevier.transfer(msg.value); // fee return to owner
            paidInitialMintFee[_depCont]=true;
        }
