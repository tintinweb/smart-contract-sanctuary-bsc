...
contract FarmContract1212V2 is Initializable, OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function claimAllReferralReward() public {
        ...
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(msg.sender);
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            for (uint256 j = 0; j < poolInfo.length; j++) {
                uint256 refferalReward = pendingReferralReward(j, _msgSender());
                userCalculatedReferralRewards[j][InviteeAddress[i]][_msgSender()] = userCalculatedReferralRewards[j][InviteeAddress[i]][_msgSender()].add(refferalReward);
            }
        }
    ...
    function claimAllNodeReward() public {
        ...
        amount2 = pending2;
        safeTokenTransfer(miningToken, _msgSender(), totalPendingNodeReward);
    }
    ...
    function distributeNodeReward(uint256 pid) internal {
        ...
        uint256 availableNodeReward1 = calculatedNodeReward1.mul(getNodeRewardPercent(pid, _msgSender())).div(rateDenominator);
        uint256 availableNodeReward2 = calculatedNodeReward2.mul(getNodeRewardPercent(pid, _msgSender())).div(rateDenominator);
    ...
    function getRequiredLP(uint256 pid, uint256 amount) public view returns(uint256) {
        ...
        uint256 amountOut = IPancakeRouter02(Router).getAmountOut(amount, _reserve0, _reserve1);
        return Math.sqrt(amount.mul(amountOut));
    }
