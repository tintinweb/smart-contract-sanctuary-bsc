...
contract TheBillionPage is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    uint256 public origPrice = 3.3 ether;
    uint256 public whitelistPrice = 0.132 ether;

    uint256 public maxSupply = 12321;
    uint256 public preSale1Amount = 1232;
    uint256 public preSale2Amount = 2107;
    uint256 public origSaleAmount = 7814;

    mapping(address => bool) public whitelisted;
    mapping(address => uint256) public whitelistAddressMintedBalance;
    mapping(address => uint256) public generalAddressMintedBalance;
    uint256 public perWhitelistAddressLimit = 2;
    uint256 public perGeneralAddressLimit = 6;
        ...
        returns (uint256)
    {
        ...
        if (msg.sender != owner()) {
            if (whitelisted[msg.sender] == true && supply <= preSale1Amount) {
                uint256 ownerMintedCount = whitelistAddressMintedBalance[
                    msg.sender
                ];
                require(
                ...
                whitelistAddressMintedBalance[msg.sender] =
                    whitelistAddressMintedBalance[msg.sender] +
                    1;
            } else {
                uint256 ownerMintedCount = generalAddressMintedBalance[
                    msg.sender
                ];
                require(
                    ownerMintedCount + 1 <= perGeneralAddressLimit,
                    "The maximum per address reached"
                );
                ...
                generalAddressMintedBalance[msg.sender] =
                    generalAddressMintedBalance[msg.sender] +
                    1;
            }
        }
    ...
    function getCurrentPrice() public view returns (uint256) {
        ...
        if (supply < preSale1Amount) {
            price = (origPrice * 8) / 100;
        } else if (supply < preSale1Amount + preSale2Amount) {
            price = (origPrice * 50) / 100;
        } else if (supply < preSale1Amount + preSale2Amount + origSaleAmount) {
            price = origPrice;
        } else {
            price = (origPrice * 33) / 10;
        }
        return price;
    }
