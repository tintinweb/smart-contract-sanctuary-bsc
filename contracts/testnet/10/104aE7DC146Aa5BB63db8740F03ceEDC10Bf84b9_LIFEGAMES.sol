...
contract LIFEGAMES is ERC20Upgradeable {
    ...
    function initialize() public initializer {
        ...
        // give permissions to the router to spend tokens and busd of the contract
    ...
    function setBusdBuyBurnAddress(address _busdBuyBurnAddress) public onlyOwner {
        require(_busdBuyBurnAddress != address(0), "_busdBuyBurnAddress address cannot be 0");
    ...
    function _finalizeTransfer(address from, address to, uint256 amount) internal returns (bool) {

        // by default all wallets use included on fees
        bool takeFee = true;

        // check if sender or recipient is excluded
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            takeFee = false;
        }

        // transfer without any fee on swaps
        if (inSwap) {
            removeAllFee();
        ...
        // check if is transfer not take any kind of fee
        // BUY
        if (from == lpPair) {}
        // SELL
        else if (to == lpPair) {}
        ...
        if (takeFee) {
            ...
            removeAllFee();
            _tokenTransferNoFee(from, address(this), feeAmount);
            restoreAllFee();
        }
    ...
    function calcBuySellTransferFee(address from, address to, uint256 amount) internal view returns (uint256) {
        ...
        if (totalFeePercent > 0) {
            return ((amount * totalFeePercent) / staticVals.masterTaxDivisor);
        }
        return 0;
    }
    ...
    /*
        ...
        function calcBuySellTransferFee(address from, address to, uint256 amount) internal returns (uint256) {
