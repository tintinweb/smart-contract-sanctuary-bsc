...
contract EvocLaunchpad is Ownable, ReentrancyGuard {
    ...
    function buyWithToken(
        ...
        require(
            amount > 0 && amount.mul(_DECIMALFACTOR) <= dexBalance,
            "Insufficient amount for this transaction"
        );
        uint256 bonusPercent = amount.mul(item._bonusPercent).div(100);
        uint256 newBalance = amount+bonusPercent;
        ...
        if (isFeeExempt[contractAdd]) {
            require(
                ContractToken.transferFrom(
                    msg.sender,
                    item._contractAddReceive,
                    newBalance.mul(_DECIMALFACTOR)
                ),
                "A transaction error has occurred. Check for approval."
            );
        } else {
            uint256 feeTransaction = amount.mul(feeTransPercent).div(100);
            uint256 AmountTk = amount.sub(feeTransaction);
            uint256 bonusAmount = amount.mul(item._bonusPercent).div(100);
            uint256 newBalanceVal = AmountTk+bonusAmount;

            require(
                ContractToken.transferFrom(
                    msg.sender,
                    item._contractAddReceive,
                    newBalanceVal.mul(_DECIMALFACTOR)
                ),
                "A transaction error has occurred. Check for approval."
            );
            ContractToken.transferFrom(
        ...
        isPausable
    {
        ...
        require(
            amount.mul(_DECIMALFACTOR) > 0 && msg.value >= amount.mul(_DECIMALFACTOR),
            "Insufficient amount for this transaction"
        );

        if (isFeeExempt[contractAdd]) {
            payable(item._contractAddReceive).transfer(amount.add(amount.mul(item._bonusPercent).div(100)));
        } else {
            uint256 feeTransaction = amount.mul(feeTransPercent).div(100);
            uint256 value = (amount.sub(feeTransaction)).add(amount.mul(item._bonusPercent).div(100));
        ...
        isPausable
    {
        ...
        emit Received(msg.sender, tokenAmount, newBalance, contractAdd);
    }
    ...
    function createHardFork(
        ...
        HardForkRegister[_contractAdd]._bonusPercent = _bonusPercent;
        ...
        HardForkRegister[_contractAdd]._contractAddReceive = _contractAddReceive;
    ...
    function deleteHardFork(address _contractAdd) public onlyOwner {
        HardFork_register storage item = HardForkRegister[_contractAdd];
        require(
            item.exist,
            "A HardFork does not exist, check the contract or create it first"
