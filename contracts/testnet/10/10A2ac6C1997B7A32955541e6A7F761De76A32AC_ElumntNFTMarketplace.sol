...
contract ElumntNFTMarketplace is Initializable, UUPSUpgradeable, OwnableUpgradeable, Verification {
    ...
    event SweepBuyNFT(sweepEvent[] sweepEventData);
    ...
    function testsweepCollection721(sweepCollection721Data memory _buyData) external onlyProxy {
        ...
        for(uint8 i = 0; i < nftsData.length; i++) {
            ...
            if(nftsData[i].tokenId == 0) {
                string memory metadata = nftsData[i].metadata;
                address payable owner = payable(msg.sender);
                address nft = nftsData[i].nft;
                tokenId = callMint721(metadata, owner, nft);
            }
        }
    }
    function callMint721(string memory metadata, address owner, address nft) internal returns(uint256) {
        mint721Data memory mintData = mint721Data(
            metadata,
            payable(owner),
            nft
        );
        return mint721(mintData);
    }
    function buy721Process(single721Data memory _nftData, uint8 currency, address erc20token) internal returns(sweepEvent memory) {
        uint256 platformSharePercent;
        uint256 singleNFTAmount = _nftData.amount;
        if(_nftData.tokenId == 0) {
            _nftData.tokenId = callMint721(_nftData.metadata, _nftData.owner, _nftData.nft);
        }
        if(_nftData.percent > 0) {
            platformSharePercent = calculatePercentValue(_nftData.amount, _nftData.percent);
            singleNFTAmount = singleNFTAmount-platformSharePercent;
        }
        uint256 royaltyPercent;
        if(_nftData.royalty > 0 && _nftData.creator != msg.sender) {
            royaltyPercent = calculatePercentValue(_nftData.amount, _nftData.royalty);
            singleNFTAmount = singleNFTAmount-royaltyPercent;
        }
        transfer721(_nftData.nft, _nftData.owner, msg.sender, _nftData.tokenId);

        if(platformSharePercent > 0) {
            if(currency!=1) {
                transfer20(msg.sender, address(this), platformSharePercent, erc20token);
            }
        }
        if(royaltyPercent > 0) {
            if(currency==1) {
                payable(_nftData.creator).transfer(royaltyPercent);
            } else {
                transfer20(msg.sender, _nftData.creator, royaltyPercent, erc20token);
            }
        }
        if(currency==1) {
            payable(_nftData.owner).transfer(singleNFTAmount);
        } else {
            transfer20(msg.sender, _nftData.owner, singleNFTAmount, erc20token);
        }
        emit NftTransferred(_nftData.tokenId, _nftData.amount, _nftData.owner, msg.sender, royaltyPercent);
        // return sweepEvent(
        //     _nftData.tokenId,
        //     _nftData.owner,
        //     msg.sender,
    ...
    function sweepCollection721(sweepCollection721Data memory _buyData) external payable onlyProxy {
        ...
        single721Data[] memory nftsData = _buyData.nftsData;
        // sweepEvent[] memory sweepEventsData;
        for(uint8 i = 0; i < nftsData.length; i++) {
            // sweepEvent memory sweepEventData = buy721Process(nftsData[i], _buyData.currency, _buyData.erc20token);
            buy721Process(nftsData[i], _buyData.currency, _buyData.erc20token);
            // sweepEventsData[i] = sweepEventData;
        }
        // emit SweepBuyNFT(sweepEventsData);
    }
