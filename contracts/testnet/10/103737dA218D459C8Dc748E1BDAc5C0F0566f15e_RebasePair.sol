...
contract RebasePair is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize() public initializer {
        ...
        _grantRole(WITHDRAWLER_ROLE, owner);
        setTreasury(owner);
        setPair(
            0x08d781Fa9B5AAf359ed9D9003E7BF5f52F598a59, // pair eUSD/eGBP
            0x035dDaeE2b88c2842f94DAfd77eA83BF79c7d348, // eUSD
            0xCB10C030865c4778B7F3304b91e0F3Ce6c95B19f, // eGBP
            address(0), // USD 0x0
            address(0), // 0x8FAf16F710003E538189334541F5D4a391Da46a0 // GBP on BSC mainnet (dont work for testing)
            address(0) //  0x0567f2323251f0aab15c8dfb1967e4e8a7d42aee // bnb on BSC mainnet
        );
    }

    function setTreasury(address treasury) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(treasury != address(0), "Zero address!");
        _treasury = treasury;
    }

    function setAllowance(address token, uint256 amount) public {
        require(msg.sender == _treasury, "Sender is not treasury!");
    ...
    function rebase() external whenNotPaused {
        ...
        address router = 0x1D0EAa1038BA77270EF9698Ec41d26b0d83eb705;
        ...
        bool allowed = IERC20(path[0]).approve(router, amountInMax);
        require(allowed, "Can't get allowance");
        IARYZE_Router02(router).swapTokensForExactTokens(
            amountOut,
            amountInMax,
            path,
            address(this),
        ...
        )
    {
        ...
        uint256 rebaseFeeUSD = ((281058 * tx.gasprice * fxNT * 13) / 10) / 100000000; //  13/10 - 30% buffer for extra fee
    ...
    // function withdraw(address token, uint256 amount) external onlyRole(WITHDRAWLER_ROLE) returns (bool transferred) {
