...
contract ExoERC20 is IExoERC20 {
...
contract ExoPair is IExoPair, ExoERC20 {
    using SafeMath  for uint;
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'Exo: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'Exo: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IExoFactory(factory).feeTo();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'Exo: K');
        }
...
contract ExoFactory is IExoFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        IExoPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'Exo: FORBIDDEN');
        feeTo = _feeTo;
    }
