...
contract MerkleWhitelist is Ownable {
  bytes32 public wl1WhitelistMerkleRoot = 0x33acc7498611682fc45aaaeb6585428cd7a26eaa7b2ccab418bca934327119f6;
...
contract dungenon is Ownable, ERC721A, ReentrancyGuard,MerkleWhitelist {
    using SafeMath for uint256;
   
    uint256 public ALL_AMOUNT = 21;
    ...
    uint256 public WL1_AMOUNT = 1500;
    ...
    string public UNREVEALED_URI = "https://data.dungenonkiller.net/box/box.json";
    string public BASE_URI;
    string public CONTRACT_URI ="https://data.dungenonkiller.net/api/contracturl.json";
    ...
    constructor() ERC721A("DungenonKILLER", "dungenonkiller") {
        _safeMint(msg.sender, 1);
    }  
    
    function info(address user) public view returns (Info memory) {
        if(_step == 1){
             return  Info(ALL_AMOUNT,totalSupply(),0,0,WL1_WALLET_CAP[user],_step,WL1_LIMIT,WL1_MINTED,WL1_AMOUNT);
        }else if(_step == 2){
             return  Info(ALL_AMOUNT,totalSupply(),PRICE,0,WALLET_CAP[user],_step,LIMIT,MINTED,(ALL_AMOUNT-WL1_MINTED));
        }
    }
        ...
        onlyWl1Whitelist(proof)
    {
        ...
        require(WL1_MINTED.add(amount) <= WL1_AMOUNT,"Max supply for WL1 reached!");
        require(totalSupply().add(amount) <= ALL_AMOUNT,"max supply would be exceeded");

         if (WL1_MINTED + amount == WL1_AMOUNT){
            _step = 2;
        }

        WL1_MINTED = WL1_MINTED.add(amount);
