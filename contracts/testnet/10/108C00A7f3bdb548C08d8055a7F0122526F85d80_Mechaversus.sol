...
// TODO Optimize all uints size as minimum needed
contract Mechaversus is NFT_1155, Validator {

    using Counters for Counters.Counter;
    Counters.Counter private id_counter;

    uint immutable _season;
    ...
    struct Mecha {
        ...
        uint256 helmet_id;
        uint256 left_arm_id;
        uint256 right_arm_id;
        uint256 left_leg_id;
        uint256 right_leg_id;
    }

    mapping( uint256 => Mecha ) private _mechas;
    mapping( address => uint256[] ) private _owned;
    mapping( uint256 => uint256 ) private _mountedParts;

    event Genesys(
        address indexed owner,
        uint256 _core_id,
        uint256 _helmet_id,
        uint256 _left_arm_id,
        uint256 _right_arm_id,
        uint256 _left_leg_id,
        uint256 _right_leg_id
    );

    constructor( uint256 season , string memory name_, string memory symbol_, string memory uri_ )
    NFT_1155( name_, symbol_, uri_ ) {
        _season = season;
        id_counter.reset();
        id_counter.increment();
    }

    function getCurrentCounter() public view virtual returns ( uint256 ) {
        return id_counter.current();
    }

    function getMecha( uint256 id ) public view virtual returns ( Mecha memory ) {
        return _mechas[ id ];
    }

    function getCore( uint256 id ) public view virtual returns ( uint256 ) {
        return _mechas[ id ].core_id;
    }

    function getHelmet( uint256 id ) public view virtual returns ( uint256 ) {
        return _mechas[ id ].helmet_id;
    }

    function getLeftArm( uint256 id ) public view virtual returns ( uint256 ) {
        return _mechas[ id ].left_arm_id;
    }

    function getRightArm( uint256 id ) public view virtual returns ( uint256 ) {
        return _mechas[ id ].right_arm_id;
    }

    function getLeftLeg( uint256 id ) public view virtual returns ( uint256 ) {
        return _mechas[ id ].left_leg_id;
    }

    function getRightLeg( uint256 id ) public view virtual returns ( uint256 ) {
        return _mechas[ id ].right_leg_id;
    }

    function getMechaParts( uint256 id ) public view virtual returns ( uint256, uint256, uint256, uint256, uint256, uint256 ) {
        return ( _mechas[ id ].core_id,
                 _mechas[ id ].helmet_id,
                 _mechas[ id ].left_arm_id,
                 _mechas[ id ].right_arm_id,
                 _mechas[ id ].left_leg_id,
                 _mechas[ id ].right_leg_id );
    }

    function isMounted( uint256 partId ) public view virtual returns ( bool ) {
        return bool( _mountedParts[ partId ] != 0 );
    }

    function getOwned( address owner ) public view virtual returns ( uint256[] memory ) {
        return _owned[ owner ];
    }

    function getOwnedMechas( address owner ) public view virtual returns ( Mecha[] memory ) {
        uint256[] memory mechasIDs = _owned[ owner ];
    ...
    // TODO genesys should be return generated mecha id
    function genesys( address to, uint256 _core_id, uint256 _helmet_id, uint256 _left_arm_id, uint256 _right_arm_id, uint256 _left_leg_id, uint256 _right_leg_id ) public {
        uint256 _id = id_counter.current();
        id_counter.increment();

        // TODO find a way for fill array inline
        uint256[] memory partsIDs = new uint256[](6);
        partsIDs[0] = _core_id;
        partsIDs[1] = _helmet_id;
        partsIDs[2] = _left_arm_id;
        partsIDs[3] = _right_arm_id;
        partsIDs[4] = _left_leg_id;
        partsIDs[5] = _right_leg_id;

        _beforeGenesys( to, partsIDs );

        _mechas[ _id ] = ( Mecha({
            id: _id,
            core_id: _core_id,
            helmet_id: _helmet_id,
            left_arm_id: _left_arm_id,
            right_arm_id: _right_arm_id,
            left_leg_id: _left_leg_id,
            right_leg_id: _right_leg_id
        }));

        _afterGenesys( to, _id, partsIDs );
    }

    function _beforeGenesys ( address to, uint256[] memory partsIDs ) internal virtual {
        for ( uint256 i = 0; i < partsIDs.length; i++ ) {
            require( Validator.isValidPart( partsIDs[ i ], i ), "Provided part ID is not valid for mount on this Mecha");
            require( NFT_1155.ownerOfNF( partsIDs[ i ] ) == to,  "Address to is not the owner of all parts provided");
        }
    }

    function _afterGenesys ( address to, uint256 mechaID, uint256[] memory partsIDs ) internal virtual {
        _owned[ to ].push( mechaID );

        for ( uint256 i = 0; i < partsIDs.length; i++ ) {
            _mountedParts[ partsIDs[ i ] ] = mechaID;
        }
    }
}
...
contract Validator is Ownable{
    ...
    uint8 constant CORE_KEY = 0;
    uint8 constant HELMET_KEY = 1;
    uint8 constant LEFT_ARM_KEY = 2;
    uint8 constant RIGHT_ARM_KEY = 3;
    uint8 constant LEFT_LEG_KEY = 4;
    uint8 constant RIGHT_LEG_KEY = 5;

    mapping( uint256 => bool ) private core_ids;
    mapping( uint256 => bool ) private helmet_ids;
    mapping( uint256 => bool ) private left_arm_ids;
    mapping( uint256 => bool ) private right_arm_ids;
    mapping( uint256 => bool ) private left_leg_ids;
    mapping( uint256 => bool ) private right_leg_ids;

    function isValidPart( uint256 partID, uint256 index ) public view virtual returns ( bool ) {

        if( index == CORE_KEY ){
            return core_ids[ partID ];
        }

        else if( index == HELMET_KEY ){
            return helmet_ids[ partID ];
        }

        else if( index == LEFT_ARM_KEY ){
            return left_arm_ids[ partID ];
        }

        else if( index == RIGHT_ARM_KEY ){
            return right_arm_ids[ partID ];
        }

        else if( index == LEFT_LEG_KEY ){
            return left_leg_ids[ partID ];
        }

        else if( index == RIGHT_LEG_KEY ){
            return right_leg_ids[ partID ];
        }
        else{
            revert( "Invalid index for validation part check" );
        }
    }

    function addValidPart ( uint256 partID, uint256 index ) public virtual onlyOwner {

        if( index == CORE_KEY ){
            core_ids[ partID ] = true;
        }

        else if( index == HELMET_KEY ){
            helmet_ids[ partID ] = true;
        }

        else if( index == LEFT_ARM_KEY ){
            left_arm_ids[ partID ] = true;
        }

        else if( index == RIGHT_ARM_KEY ){
            right_arm_ids[ partID ] = true;
        }

        else if( index == LEFT_LEG_KEY ){
            left_leg_ids[ partID ] = true;
        }

        else if( index == RIGHT_LEG_KEY ){
            right_leg_ids[ partID ] = true;
        }
        else{
            revert( "Invalid index on adding part" );
        }
    }

    function removeValidPart ( uint256 partID, uint256 index ) public virtual onlyOwner {

        //TODO Test if is better false or delete
        if( index == CORE_KEY ){
            core_ids[ partID ] = false;
        }

        else if( index == HELMET_KEY ){
            helmet_ids[ partID ] = false;
        }

        else if( index == LEFT_ARM_KEY ){
            left_arm_ids[ partID ] = false;
        }

        else if( index == RIGHT_ARM_KEY ){
            right_arm_ids[ partID ] = false;
        }

        else if( index == LEFT_LEG_KEY ){
            left_leg_ids[ partID ] = false;
        }

        else if( index == RIGHT_LEG_KEY ){
            right_leg_ids[ partID ] = false;
        }
        else{
            revert( "Invalid index on removing part" );
        }
    }
