...
contract RewardManager is Ownable, ReentrancyGuard {
    ...
    uint256 public lockDuration = 86400 * 7 * 12;
    ...
    mapping(address => Lock[13]) public locks;
    ...
    function setLockDuration(uint256 _durationInSeconds) external onlyOwner {
        require(_durationInSeconds < 86400 * 7 * 12 + 1, "INVALID_DURATION");
    ...
    function mint(
        ...
        IProtocolToken(rewardToken).mintTo(address(this), amount);
        return true;
    }
    ...
    function withdrawUnlocked() external pausable nonReentrant {
        ...
        Lock[13] storage loc = locks[msg.sender];
        uint256 amount;
        ...
        bal.total = bal.total - amount;
        totalSupply = totalSupply - amount;
        IProtocolToken(rewardToken).transfer(msg.sender, amount);
    ...
    function withdrawAll() external pausable nonReentrant {
        ...
        delete bal.total;
        delete bal.unlocked;
        IProtocolToken(rewardToken).transfer(rewardReserve, penalizedAmount);
        IProtocolToken(rewardToken).transfer(msg.sender, claimableAmount);
