...
contract DFHVestingToken is Ownable, Pausable {
        ...
        returns (uint256)
    {
        ...
        if (time > _end) {
            return info.amount;
        } else if (time > _startSecond) {
            if (time < _startSecond.add(_secondsPerSlice)) {
                return info.amount.mul(_startSecond.sub(_start)).div(_duration);
            }

            uint256 durationFromStart = time.sub(_startSecond);
            uint256 vestedSlicePeriods = durationFromStart.div(_secondsPerSlice);
            uint256 vestedSeconds = vestedSlicePeriods.mul(_secondsPerSlice);
            uint256 cliffDuration = _startSecond.sub(_start);
            return info.amount.mul(vestedSeconds.add(cliffDuration)).div(_duration);
        } else if (time > _cliff) {
            return info.amount.mul(_firstReleasePercent).div(100);
        }
        return 0;
    }
