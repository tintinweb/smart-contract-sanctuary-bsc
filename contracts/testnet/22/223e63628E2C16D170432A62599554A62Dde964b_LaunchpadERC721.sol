...
import "./ILaunchpadERC721.sol";

contract LaunchpadERC721 is Ownable, ERC721, ILaunchpadERC721 {
    uint256 public override LAUNCH_MAX_SUPPLY;
    uint256 public override LAUNCH_SUPPLY;

    uint256 public netBalance;

    address public override LAUNCHPAD;

    address public override split;
    uint16 public override splitPortion;
    ...
    constructor(
        ...
        uint16 _splitPortion,
        address launchpad,
        uint256 maxSupply,
        address _payee,
        address _split
    ) ERC721(name_, symbol_) {
        require(_splitPortion < 10000, "portion is not correct");
        baseURI = baseURI_;
        splitPortion = _splitPortion;
    ...
    modifier onlyPayee() {
        require(msg.sender == payee, "must call by payee ");
        _;
    }
    ...
    receive() external payable {
        // uint256 _splitBalance = (msg.value * splitPortion) / 10000;
        // if (_splitBalance > 0) {
        //     payable(split).transfer(_splitBalance);
        // }

        emit Received(msg.sender, msg.value);
    }

    function updateSplitPortion(uint16 _splitPortion)
        external
        override
        onlyOwner
    {
        ...
        emit SplitPortionUpdated(_splitPortion);
    }

    function updateSplitAddress(address _split)
        external
        override
        onlyOwner
        validAddress(_split)
    ...
    function updatePayeeAddress(address _payee)
        external
        override
        onlyOwner
        validAddress(_payee)
    {
        payee = _payee;
        emit PayeeAddressUpdated(_payee);
    }
    ...
    function withdraw() external override onlyPayee {
        netBalance = 0;
        payable(msg.sender).transfer(address(this).balance);
    }

    function mintTo(address _to, uint256 _size)
        external
        override
        onlyLaunchpad
    {
        ...
        require(
            LAUNCH_SUPPLY + _size <= LAUNCH_MAX_SUPPLY,
            "max supply reached"
        );

        uint256 currentId = LAUNCH_SUPPLY;
        ...
        uint256 diff = address(this).balance - netBalance;
        uint256 _splitBalance = (diff * splitPortion) / 10000;
        if (_splitBalance > 0) {
            payable(split).transfer(_splitBalance);
        }

        netBalance += (diff * (10000 - splitPortion)) / 10000;
    }
    ...
    {
        royaltyAmount = (_salePrice * splitPortion) / 10000;
        return (split, royaltyAmount);
    }
...
interface ILaunchpadERC721 is IERC721, IERC2981 {
    ...
    function LAUNCH_MAX_SUPPLY() external view returns (uint256);
