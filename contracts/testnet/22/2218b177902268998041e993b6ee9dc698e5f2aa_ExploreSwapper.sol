...
contract ExploreSwapper {
    ...
    function swap(
        ...
        for (uint256 i = 0; i < pairs.length; i++) {
            ...
            UniswapRouter.swap(pairs[i],swapTypes[i], route[i],
...
library UniswapRouter {
    ...
    uint256 private constant DEFAULT_TYPE = 1;
    uint256 private constant SECOND_TYPE = 2;
    ...
    function swap(
        ...
        if(SECOND_TYPE == pt){
            ...
            ISecondPair(pair).swap(amount0Out,
                amount1Out, recipient);
        }
