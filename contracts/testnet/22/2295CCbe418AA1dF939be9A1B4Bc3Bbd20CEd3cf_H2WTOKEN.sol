...
// File contracts/H2W/H2WToken.sol
...
contract H2WTOKEN is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[from]) {
            ...
            if (referrers[to] != address(0)) {
                referralCommissionAmount[referrers[to]] = referralCommissionAmount[referrers[to]] + ((amount * referralCommissionPercentage) / 1000);
            }
        }
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 tokensForRewardDistribution = contractBalance - totalTokensToSwap;
        if (totalTokensToSwap == 0 || contractBalance == 0) {
            return;
        }

        uint256 tokensForLiquidity = _liquidityTokensToSwap / 2;
        uint256 amountToSwapForETH = _liquidityTokensToSwap - (tokensForLiquidity) + (_marketingTokensToSwap) + (_devTokensToSwap) + (_potTokensToSwap) + (_partnerTokensToSwap);
        ...
        uint256 ethForMarketing = (ethBalance * (_marketingTokensToSwap + _devTokensToSwap + _potTokensToSwap + _partnerTokensToSwap)) / (amountToSwapForETH);

        uint256 ethForLiquidity = ethBalance - ethForMarketing;

        // Transfer burn token to burn wallet
        uint256 currentRate = _getRate();
        uint256 rBurn = _burnTokens * currentRate;
        _rOwned[burnAddress] = _rOwned[burnAddress] + rBurn;
        _tOwned[burnAddress] = _tOwned[burnAddress] + _burnTokens;
        ...
        (success, ) = address(marketingAddress).call{
            value: (address(this).balance * (_marketingTokensToSwap)) / (_marketingTokensToSwap + _devTokensToSwap + _potTokensToSwap + _partnerTokensToSwap)
        }("");
        (success, ) = address(devAddress).call{ value: (address(this).balance * _devTokensToSwap) / (_devTokensToSwap + _potTokensToSwap + _partnerTokensToSwap) }("");
        (success, ) = address(potAddress).call{ value: (address(this).balance * _potTokensToSwap) / (_potTokensToSwap + _partnerTokensToSwap) }("");
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) private {
        if (buyOrSellSwitch == BUY) {
            ...
            if (_isExcluded[address(this)]) _tOwned[address(this)] = _tOwned[address(this)] + tFee;
        } else if (buyOrSellSwitch == SELL) {
