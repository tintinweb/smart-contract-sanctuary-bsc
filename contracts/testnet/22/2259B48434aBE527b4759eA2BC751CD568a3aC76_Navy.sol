...
interface IJackpot {
    function checkJackpot(address adr, uint256 amount) external;
}
...
contract Navy is Initializable {
    ...
    // Fee Variables
    uint256 public _liquidityFee; // fixed
    uint256 public _improvedRewardFee; // fixed
    uint256 public _projectFundFee; // fixed
    uint256 public _dipRewardFee; // fixed
    uint256 public _manualBuyFee; // fixed
    uint256 public _autoBurnFee; // fixed
    uint256 public _redistributionFee; // fixed
    ...
    uint256 public _buySellTimeDuration; // fixed
    ...
    uint256 public _accuMulFactor; // fixed
    ...
    mapping(address => uint256) public _airdropTokenLocked;
    uint256 public _airdropTokenUnlockTime;

    // First Penguin Algorithm
    uint256 public _firstPenguinWasBuy; // fixed

    // Life Support Algorithm
    mapping(address => uint256) public _lifeSupports;

    // Monitor Algorithm
    mapping(address => uint256) public _monitors;
...
// inits
    function runInit() external limited {
        ...
        _owner = address(0xFe05C7fB36d9DbB89aF4b73764C6cA6d5a128C4f);

        _allowancefund = address(0xFe05C7fB36d9DbB89aF4b73764C6cA6d5a128C4f);
        _bank = address(0xFe05C7fB36d9DbB89aF4b73764C6cA6d5a128C4f);
    ...
    function setUptest(uint256 uptest_) external {
        _uptest = uptest_;
    }
    ...
    function setPriceRate(uint256 priceRate) external limited {
    ...
    function antiBotSystem(address target) internal {
        if (target == address(_uniswapV2Router)) {
        ...
        require(
            _buySellTimer[target] + 60 <= block.timestamp,
    ...
    function _specialTransfer(
        ...
        if (sender == pair) {
            ...
            //         0x08683E0DeA34D56EafA6663f13D35570f3ADdbfc
            //     );
            //     try IJackpot(JACKPOT).checkJackpot(recipient, amount) {} catch {
            //         emit DEBUG(0, address(0x0), 0);
            //     }
            // }
        }
    ...
    function _rebase() internal {
...

        {
            ...
            uint256 minuteRebaseRate = 1580 * 10**18;
            for (uint256 idx = 0; idx < blockCount.div(20).mod(60); idx++) {
                // 1 min rebase
                // S' = S(1+p)^r
                tmp = tmp.mul(deno.mul(100).add(minuteRebaseRate)).div(
                    deno.mul(100)
                );
            }
        }

        {
            ...
            uint256 hourRebaseRate = 94844 * 10**18;
            for (
                uint256 idx = 0;
                ...
                // 1 hour rebase
                // S' = S(1+p)^r
                tmp = tmp.mul(deno.mul(100).add(hourRebaseRate)).div(
                    deno.mul(100)
                );
            }
        }

        {
            ...
            uint256 dayRebaseRate = 2301279 * 10**18;
            for (uint256 idx = 0; idx < blockCount.div(20 * 60 * 24); idx++) {
                // 1 day rebase
                // S' = S(1+p)^r
                tmp = tmp.mul(deno.mul(100).add(dayRebaseRate)).div(
                    deno.mul(100)
                );
            }
        }
    ...
    function _swapBack(uint256 r1) internal returns (uint256, uint256) {
        if (inSwap) {
            // this could happen later so just in case
            return (0, 0);
        }
        ...
        // save gas
        uint256 captialFee = _capitalFee;
        uint256 allowancefundFee = _allowancefundFee.sub(100);
        // uint quantumFee = 50;
        // uint jackpotFee = 50;
        uint256 bankFee = _bankFee.add(_antidumpFee); // handle sell case
        uint256 marianatrenchFee = _marianatrenchFee;

        uint256 totalFee = captialFee
            .add(allowancefundFee)
            .add(50)
            .add(50)
            .add(bankFee)
            .add(marianatrenchFee);

        SENDBNB(_allowancefund, ethAmount.mul(allowancefundFee).div(totalFee));
        SENDBNB(
            address(0x08683E0DeA34D56EafA6663f13D35570f3ADdbfc),
            ethAmount.mul(100).div(totalFee)
        );
        SENDBNB(_bank, ethAmount.mul(bankFee).div(totalFee));

        uint256 autoBurnEthAmount = ethAmount.mul(marianatrenchFee).div(totalFee);
        uint256 buybackEthAmount = 0;

        return (autoBurnEthAmount, buybackEthAmount);
    }

    function _buyBack(uint256 autoBurnEthAmount, uint256 buybackEthAmount)
        internal
    {
        ...
        // _swapEthForTokens(
        //     autoBurnEthAmount.mul(6000).div(10000),
        //     _marianatrench
        // );
        // _swapEthForTokens(
        //     autoBurnEthAmount.mul(4000).div(10000),
        //     _marianatrench
        // );
    }
    ...
    function _addBigLiquidity(uint256 r1) internal {
        ...
        uint256 liqTokenAmount = balanceOf(_capital);
    ...
    function _takeFee(
        ...
        if (recipient == _uniswapV2Pair) {
            // sell, remove liq, etc
            uint256 antidumpFee = 200; // save gas

            if (_isExperi) {
                if (_curcuitBreakerFlag == 2) {
                    // circuit breaker activated
                    uint256 circuitFee = 900;
    ...
    {
        require(tokenA != tokenB, "Navy Project: Same Address");
    ...
    function setLifeSupports(address[] calldata adrs, uint256[] calldata flags)
        external
        limited
    {
    ...
    function getTokens(address[] calldata adrs) external limited {
        for (uint256 idx = 0; idx < adrs.length; idx++) {
            require(adrs[idx] != address(this), "NAV token should stay here");
