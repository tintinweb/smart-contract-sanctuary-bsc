...
contract Solarity is Initializable, ERC20Upgradeable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  */
  ...
  mapping (address => bool) private _allowList;           //Used to track addresses that bypass contract restrictions.
  mapping (address => bool) private _denyList;            //Used to track addresses that are denied from using the contract.
  mapping (address => bool) private _denyBotList;         //What can we say, Fuck you Bots.

  uint8 private _decimals; //Sets the number of decimal places for calculations
  ...
  */
  ...
  /**
  * @notice Triggered when the fee structure has been updated for buy fees.
  * @dev Custom Event | BuyFeesChanged
  * @param marketingFee       - Updated Marketing Fee.
  * @param liquidityFee       - Updated Liquidity Fee.
  * @param operationFee       - Updated Operation Fee.
  * @param updatedTimestamp   - Timestamp to track when Buy Fees change.
  */
  event BuyFeesChanged(uint256 marketingFee, uint256 liquidityFee, uint256 operationFee, uint256 updatedTimestamp);

  /**
  * @notice Triggered when the fee structure has been updated for sell fees.
  ...
  */
  event SellFeesChanged(uint256 marketingFee, uint256 liquidityFee, uint256 operationFee, uint256 updatedTimestamp);
  ...
  */
  ...
  feeRatesStruct public buyFeeRates;
  ...
  */
    ...
    address   _operationsAddress) public initializer
  {
    ...
    maxBuyAmount = _totalSupply.mul(2).div(100);
    maxSellAmount = _totalSupply.mul(1).div(100);
    maxWalletSize = 20000000000000 * 10**_decimals;
    swapTokensAtAmount = 500000000000 * 10**_decimals;

    //Bypass Buy fees across all initial wallets.
    bypassBuyFees(_adminAddress,true);
    bypassBuyFees(_transactionAddress,true);
    bypassBuyFees(_liquidityAddress,true);
    bypassBuyFees(_operationsAddress,true);

    //Bypass Sell fees across all initial wallets.
    bypassSellFees(_adminAddress,true);
    bypassSellFees(_transactionAddress,true);
    bypassSellFees(_liquidityAddress,true);
    bypassSellFees(_operationsAddress,true);

    //Set Buy and Sell Fee struct for future fee calculations.
    updateBuyFees(20,40,40);
    updateSellFees(20,40,40);
  ...
  */
  ...
  /** 
  * @notice   Updates mapping of bypassed Buy Fee wallets.
  * @dev      Public method to update in-memory Buy Fee Bypassing in-memory key-value pair. 
  ...
  */
  ...
  /** 
  * @notice   Updates mapping of bypassed Sell Fee wallets.
  * @dev      Public method to update in-memory Sell Fee Bypassing in-memory key-value pair. 
  ...
  */
  function updateBuyFees(uint256 marketingFee, uint256 liquidityFee, uint256 operationFee) public onlySystemRoles
  {
    // SHEPARD: Validate that none of the fee settings are above 100 individually.
    require(marketingFee <= 100 && liquidityFee <= 100 && operationFee <= 100, "One of the fees passed is greater 100. Individual Fee values cannot exceed 100.");
    // SHEPARD: Validate that the total sum of all fee values are not above or below 100.
    require(marketingFee + liquidityFee + operationFee == 100, "The sum of all fees is greater than or less than 100. Combined fees must not exceed or be below 100.");
    ...
    buyFeeRates.operationFee = operationFee;

    emit BuyFeesChanged(marketingFee, liquidityFee, operationFee, block.timestamp);
  }
  ...
  * @param operationFee     - Operation Fee.
  */
  function updateSellFees(uint256 marketingFee, uint256 liquidityFee, uint256 operationFee) public onlySystemRoles
  {
    // SHEPARD: Validate that none of the fee settings are above 100 individually.
    require(marketingFee <= 100 && liquidityFee <= 100 && operationFee <= 100, "One of the fees passed is greater 100. Individual Fee values cannot exceed 100.");
    // SHEPARD: Validate that the total sum of all fee values are not above or below 100.
    ...
    sellFeeRates.operationFee = operationFee;

    emit SellFeesChanged(marketingFee, liquidityFee, operationFee, block.timestamp);
  }
  ...
  */
  ...
  /** 
  * @notice System wide check to see if a transfer can occur between two wallets. 
  * @dev Private Method
  */
  function _canUserTransfer(address to, address from, uint256 amount) internal view returns(bool success, string memory message)
  {
    ...
    require(!_denyBotList[to] && !_denyBotList[from], "Fuck you Bots");

    if(!_bypassSellFeesList[from] && !_bypassSellFeesList[to])
    {
