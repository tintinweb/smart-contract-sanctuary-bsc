...
contract NftPassport is ERC721, Ownable {
    ...
    event VerifierUpdate(uint indexed verifierId, address payable feeReceiver, address dataSigner, uint mintFee, string baseUri, bool active);
    ...
    function updateVerifier(uint _verifierId, address payable _feeReceiver, address _dataSigner, uint _mintFee, string calldata _baseUri) public onlyOwner {
        ...
        emit VerifierUpdate(_verifierId, _feeReceiver, _dataSigner, _mintFee, _baseUri, verifiers[_verifierId].active);
    }
    ...
    function getUserPassport(address _owner, uint _number) external view returns (PassportData memory) {
