...
contract SingleSignEntityStrategy is IEntityStrategy, Ownable, EIP712 {
  using Counters for Counters.Counter;

  bytes32 private constant ENTITY_TYPEHASH =
    keccak256(
      "Entity(address owner,address swapMile,address mileToken,address sMileToken,address entityFactory,uint256 nonce,uint256 deadline)"
    );
  ...
  event TrustedSignerChanged(address newTrustedSigner);

  constructor(address _trustedSigner) EIP712("SingleSignEntityStrategy", "1") {
    trustedSigner = _trustedSigner;
  }

  function setTrustedSigner(address _trustedSigner) external onlyOwner {
    require(_trustedSigner != trustedSigner, "SV");
    trustedSigner = _trustedSigner;
  ...
  function isValid(
    ...
    require(argumentsU256.length == 1, "UI");
    require(argumentsB32.length == 2, "BI");
    uint8 v = (uint8)(argumentsU256[0]);
    bytes32 r = argumentsB32[0];
    bytes32 s = argumentsB32[1];

    bytes32 digest = getDigest(entity, deadline);
    return ECDSA.recover(digest, v, r, s) == trustedSigner;
  }

  function getDigest(Entity calldata entity, uint256 deadline)
    public
    returns (bytes32)
  {
    return
