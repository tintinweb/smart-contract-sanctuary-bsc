...
contract FreeNodeStakePoolT1 is Ownable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns(uint256) {
        ...
        uint256 totalDeservedReward = user.totalDepositAmount.mul(accTokenPerShare).sub(user.claimedReward);
		uint256 referralReward = totalDeservedReward.mul(bonusPerReferral).div(100).mul(inviteContract.getMyReferrCount(msg.sender));
        return user.totalDepositAmount.mul(accTokenPerShare).add(referralReward).sub(user.claimedReward);
    }

    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 referralBonus){
		PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 totalDeservedReward = user.totalDepositAmount.mul(pool.accTokenPerShare).sub(user.claimedReward);
    ...
    function claim(uint256 _pid) public {
		...
		updatePool(_pid);
        uint256 pending = user.totalDepositAmount.mul(pool.accTokenPerShare).add(myReferralReward(_pid,msg.sender)).sub(user.claimedReward);
        require(pending > 0,"You have no reward tokens to claim");
        if(pending > 0 && IERC20(Token).balanceOf(address(this)) > 0){
            safeTokenTransfer(msg.sender, pending);
        }
        user.claimedReward = user.totalDepositAmount.mul(pool.accTokenPerShare).add(myReferralReward(_pid,msg.sender));
