...
contract StakeNEW is Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 _planExpired = 600000000;
    ...
    function transferToken(address to,uint256 amount) external onlyOwner{
        require(NEW.transfer(to, amount), "Token transfer failed!");  
    }

    function claimReward() external returns (bool){
        require(addressStaked[_msgSender()] == true, "You are not enrolled");
        require(stakeInfos[_msgSender()].endTS < block.timestamp, "Stake Time in progress!");
        ...
        NEW.transfer(_msgSender(), totalTokens);
    ...
    function stakeToken(uint256 stakeAmount) external payable whenNotPaused {
        ...
        require(addressStaked[_msgSender()] == false, "You already enrolled");
        require(NEW.balanceOf(_msgSender()) >= stakeAmount, "Insufficient Balance");
        
           NEW.transferFrom(_msgSender(), address(this), stakeAmount);
            totalStakers++;
