...
contract Product is Ownable, Initializable {
    ...
    function initialize(address tokenAdd, uint allocation, uint subsequentClaims, uint timestamp) external initializer {
        require(timestamp > 0, "Product : timestamp > 0");
        require(subsequentClaims > 0,"Product : subsequentClaims > 0");
    ...
    function addDistributionWallet( address[] memory account, uint[] memory amount, uint[] memory startTime) external onlyOwner {
        require(account.length < 30,"Product : length < 30");
        require((account.length == amount.length) && (startTime.length == amount.length),"Product : length mismatch");
        uint currentTime = block.timestamp;

        for(uint i=0; i< account.length; i++) {
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "Product : insufficient balance to allocate");
    ...
    function revokeDistributionWallet(address[] memory account) external onlyOwner {
        ...
        for(uint i=0; i< account.length; i++) {  
            ...
            require(user_.totalClaimed < user_.balance, "Product : user claimed all funds or user may not be added");
            uint amountToRevoke = user_.balance - user_.totalClaimed;
            delete user[account[i]];

            if(amountToRevoke > 0) {
                require(token.balanceOf(address(this)) >= amountToRevoke, "Product : insufficient balance to revoke");
                require(token.transfer(owner(),amountToRevoke),"Product : revoke transfer failed");
            }
        }
    }
    ...
    function claim() external {
        ...
        require(user_.balance > 0, "Product : user not exist");
        require(user_.totalClaimed < user_.balance, "Product : total claim < total balance");
        require((user_.lastClaim > 0) && ((user_.lastClaim + claimPeriod) <= block.timestamp), "Product : lastClaim < block.timestamp");
    ...
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "Product : insufficient balance");
            require(payable(owner()).send(amount), "Product : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "Product : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "Product : transfer failed");
        }
    }
}