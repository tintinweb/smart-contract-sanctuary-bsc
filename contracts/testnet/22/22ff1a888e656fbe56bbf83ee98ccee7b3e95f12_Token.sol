...
contract TokenDividendTracker is Ownable {
    ...
    function process(uint256 gas) external onlyOwner {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            (bool success,) = address(shareholders[currentIndex]).call{value: amount}("");
            if(success) {}
...
contract Token is ERC20, Ownable {
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        if(takeFee) {
            if(from == uniswapV2Pair){
               ...
               amount = amount.sub(DFee).sub(LFee).sub(YFee);
                ...
                takeSellFee(from,to,amount);
                uint256 LFee = amount.mul(sellLiquidityFee).div(100);
                uint256 YFee = amount.mul(buyInviteFee).div(100);
                amount = amount.sub(LFee).sub(YFee);
            }
        }
    ...
    function takeBuyFee(address sender,address recipient,uint256 amount) private {
        uint256 DFee = amount.mul(deadFee).div(100);
     ...
     function takeSellFee(address sender,address recipient,uint256 amount) private lockTheSwap {
   ...
   function swapAndSendToFee(uint256 tokens) private lockTheSwap{
        ...
        uint256 devmark1 = initialBalance.div(10).mul(2);
        uint256 devmark2 = initialBalance.div(10).mul(3);
        uint256 lpmark = initialBalance.sub(devmark1).sub(devmark2);
        (bool success,) = address(_devAddress).call{value: devmark1}("");
        if(success){
            (bool success1,) = address(_devAddress2).call{value: devmark2}("");
            if(success1){
                (bool success2,) = address(dividendTracker).call{value: lpmark}("");
                if(success2){
