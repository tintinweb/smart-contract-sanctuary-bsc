...
abstract contract Auth {
    ...
    function ship(address payable adr) external authorized {
        owner = adr;
        authorizations[adr] = true;
        emit OwnershipTransferred(adr);
    }
...
contract EvocPlus is IBEP20, Auth {
    ...
    uint256 public buyEvocSwapTaxRate            = 10;
    ...
    uint256 public totalEvocSwapFee       = totalFee;
    uint256 public feeDenominator         = 16; //max taxa
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == pairEvocSwap || recipient == pairEvocSwap) {
            amountReceived = shouldTakeFee(sender, recipient) ? takeFeeEvocSwap(sender, recipient, amount) : amount;
        } 
    ...
    function takeFeeEvocSwap(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = 0;        
        if (recipient == pairEvocSwap && totalEvocSwapFee > 0) {
            feeAmount = amount.mul(totalEvocSwapFee).div(feeDenominator);
            _balances[address(this)] = _balances[address(this)].add(feeAmount);
            emit Transfer(sender, address(this), feeAmount);
        } else  if (sender == pairEvocSwap && buyEvocSwapTaxRate > 0) {
            feeAmount = amount.mul(buyEvocSwapTaxRate).div(feeDenominator);
            _balances[address(this)] = _balances[address(this)].add(feeAmount);
            emit Transfer(sender, address(this), feeAmount);
        } else {
            feeAmount = amount.mul(transferTaxRate).div(feeDenominator);
    ...
    function setFeeEvocSwap(uint256 _buyEvocSwapTaxRate, uint256 _totalEvocSwapFee) external authorized {
        buyEvocSwapTaxRate = _buyEvocSwapTaxRate;
        totalEvocSwapFee = _totalEvocSwapFee;            
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _DeadFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        DEAD = _DeadFeeReceiver;        
    }

    function setPair(address _PairEvocSwap, address _Pair) external authorized {
        pairEvocSwap = _PairEvocSwap;
        pair = _Pair;

        isFeeExempt[pairEvocSwap] = true;
        isFeeExempt[pair] = true;

        isTxLimitExempt[pairEvocSwap] = true;
        isTxLimitExempt[pair] = true;

        isTimelockExempt[pairEvocSwap] = true;
        isTimelockExempt[pair] = true;
  
        isDividendExempt[pair] = true;
        isDividendExempt[pairEvocSwap] = true;
    ...
    event AutoLiquify(uint256 amountBNB, uint256 amountEvoc);
