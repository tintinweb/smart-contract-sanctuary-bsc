...
contract Cbee is IBEP20, Ownable {
    
    using SafeMath for uint256;

    string _name = "This is it";
    string _symbol = "<Cbee>";
    uint8 constant _decimals = 9;
    uint256 constant _totalSupply = 1 ** 9 * (10 ** _decimals); // 100M tokens
    ...
    uint256 public totalFee = liquidityFee + reflectionFee + marketingFee; // Reflections fee - 7%, Liquidity fee - 4% , Marketing/Development Fee - 4% 
    ...
    uint256 launchedAt;
    uint256 private _swapThreshold = _totalSupply / 100; // 1% max tx
    function getSwapThreshold() public view returns (uint256) { return _swapThreshold; }
     address _marketingFeeReceiver = 0x28e7B42A92f4D559cCbeE40437432F4CB65339e7;
...
//preserved if liquidity fee needs to be sent elsewhere to create more liquidity pairs
    function setFeeReceiver(address _autoLiquidityReceiver , address _marketingFeeReceiver) public onlyDev() { 
