...
contract Staking is ReentrancyGuard {
    ...
    mapping(uint => stakerbonus) private bonusinfo;
    ...
    { 
        ...
        if(account != address(0)){
             //address  referral = msg.sender;
             s_stakingToken.transferFrom(owner, account, amount*10/100);
             uint totalreferralId = _referralIds.current();
       
                _referralIds.increment();
              bonusinfo[totalreferralId] = stakerbonus(
                account,
            ...
            if (bonusinfo[i + 1].referral == account) {
            //itemCount += 1;
            uint currentId = i + 1;
            address account1 = bonusinfo[currentId].account;
            //address referral = msg.sender;
            uint totreferralId = _referralIds.current();
       
                _referralIds.increment();
                 bonusinfo[totreferralId] = stakerbonus(
                account1,
        ...
        bool success = s_stakingToken.transferFrom(owner, address(this), amount);
        if (!success) {
