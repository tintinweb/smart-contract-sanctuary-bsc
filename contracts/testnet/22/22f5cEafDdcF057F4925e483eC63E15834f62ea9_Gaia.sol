...
//import "./Router/interfaces/IMidasGoldRouter.sol";
import "./Token.sol";
import "hardhat/console.sol";

contract Gaia is Initializable, OwnableUpgradeable {
    IMidasGoldRouter public midasGoldRouter;
    address public tykheFortuneDistributorAddress;
    event TokenCreated(address _token);

    /// @custom:oz-upgrades-unsafe-allow constructor
    function initialize(address midasGoldRouterAddress, address _tykheFortuneDistributorAddress) public initializer {
        __Ownable_init();
    ...
    function createNewToken(
        ...
        if (_isPayingWithNativeCurrency) {
            uint256 requiredEthAmount = uint256(
                midasGoldRouter.getRequiredEthAmount()
            );

            require(msg.value >= requiredEthAmount, "low value");
            midasGoldRouter.swapBnbForUSD{value: msg.value}(tykheFortuneDistributorAddress);
        } else {
            ...
            if (paymentTokenAddress != midasGoldRouter.getStableCoinAddress()) {
        ...
        transferedAmount = IERC20(midasGoldRouter.getStableCoinAddress()).balanceOf(address(this));
        if (transferedAmount > 0) {
            IERC20(address(midasGoldRouter.getStableCoinAddress())).transfer(
                tykheFortuneDistributorAddress,
                transferedAmount
            );
        }
...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(
        ...
        swapTokenAddress = midasGoldRouter.getNativeTokenAddress();
    ...
    function setTaxes(
        ...
        require(
            buyFee + sellFee + transferFee <= 4500,
    ...
    function _transfer(
        ...
        if (contractMustSwap(from, to)) {
            ...
            // swap tokens
            midasGoldRouter.swapTokensForNativeToken(
                address(this),
                owner(),
    ...
    function autoLiquidity(uint256 tokenAmount) public {
        ...
        // swap tokens for ETH
        midasGoldRouter.swapTokensForNativeToken(address(this), owner(), half);

        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance - initialBalance;

        // add liquidity to uniswap
        midasGoldRouter.addLiquidity(address(this), owner(), half, newBalance);
    }
