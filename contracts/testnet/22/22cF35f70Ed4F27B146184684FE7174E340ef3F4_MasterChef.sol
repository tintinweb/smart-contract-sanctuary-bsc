...
contract MasterChef {
    ...
    address public DAO = 0x81A6e04D5924aA89dA1b53eD771498Fdfd289Ec2;
    ...
    uint256 public rewardtokenPerSecond =  0.2314 ether; // 20000 TOKEN / (24h * 60min * 60s)
    ...
    constructor(
        ...
        require(_rewardtoken != address(0), 'wrong reward token address');
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(rewardtokenPerSecond);
            return poolEndTime.sub(_fromTime).mul(rewardtokenPerSecond);
        } else {
    ...
    function pendingTOKEN(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accTokenPerShare = accTokenPerShare.add(_rewardtokenReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _rewardtokenReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accTokenPerShare = pool.accTokenPerShare.add(_rewardtokenReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if (_rewardtokenBalance > 0) {
            if (_amount > _rewardtokenBalance) {
                rewardtoken.safeTransfer(_to, _rewardtokenBalance);
            } else {
    ...
    function setDaoAddress(address _DAO) public{
        require(msg.sender == DAO, "setDaoAddress: FORBIDDEN");
