...
contract TokenVesting is Ownable {
     ...
     */
    constructor (IERC20 token, address receiver, uint256 amount, uint256[] memory schedule,
    ...
    function vestedAmount(uint256 ts) public view returns (uint256) {
        uint256 unreleasedIdx = _releasableIdx(ts);
        if (unreleasedIdx >= 0) {
            return _amount * (_percent[unreleasedIdx]) / (10000);
        } else {
            return 0;
        }
    ...
    function release() public {
        uint256 unreleasedIdx = _releasableIdx(block.timestamp);

        require(unreleasedIdx >= 0, "TokenVesting: no tokens are due");

        uint256 unreleasedAmount = _amount * (_percent[unreleasedIdx]) / (10000);

        _token.safeTransfer(_beneficiary, unreleasedAmount);

        _percent[unreleasedIdx] = 0;
        _released = _released + (unreleasedAmount);
     ...
     */
    function _releasableIdx(uint256 ts) private view returns (uint256) {
