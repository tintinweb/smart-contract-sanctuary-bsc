...
contract MotherBox is ERC721 {
  ...
  uint256 public max = 100250;
  uint256 public buyLimit = 15;
  ...
  function createTreasureBox(
    ...
    require(amount > 0 && amount < 51, "exceed max");
    uint256 tokenId = id;
    uint256 msgnonce = nonce[msg.sender]++;
    id = id + amount;
    BaseERC721BlindBox nft = blindbox;
    bytes32 payloadHash = keccak256(abi.encode(msg.sender, amount, msgnonce));
    bytes32 digest =
    ...
    for (uint256 i = 0; i < amount; i++) {
      nft.mint(msg.sender, tokenId + 1 + i);
      blindboxTokenIds.push(tokenId + 1 + i);
    }
    emit CreateTreasureBox(
      msg.sender,
      address(nft),
      tokenId + 1,
      amount,
