...
library SmartMatrix {
	...
	function InitNewNodeToMatrixes(Node storage _Node, mapping(uint256 => SmartMatrix.Node) storage _Nodes) internal {
		...
		uint256 binarylowernodeid = FindLowerNodeOnShortestLegOfSponsorNode(_Node, _Nodes, Binary);
		...
		uint256 ternarylowernodeid = FindLowerNodeOnShortestLegOfSponsorNode(_Node, _Nodes, Ternary);
	...
	function UpdateSponsorLevel(Node storage _Node, mapping(uint256 => SmartMatrix.Node) storage _Nodes) internal {
		// After init node -> Update
		if (_Nodes[_Node.SponsorID].SL == 1 && _Nodes[_Node.SponsorID].Matrix[0].Line1IDs.length == 2) {
			...
			if (_Nodes[sponsor2].SL < 5 && _Nodes[_Node.SponsorID].Matrix[0].Line1IDs.length >= 3) {
				_Nodes[sponsor2].SL += 1; // sponsor2 is SL2 (or 3,4) become to: SL3 (or 4,5)

				// sponsor3 is not max and have more than 10 F1
				if (_Nodes[sponsor2].SL == 5 && _Nodes[sponsor3].SL < 15 && _Nodes[sponsor3].Matrix[0].Line1IDs.length >= 10) {
					uint8 line1sl5count = 0;
					for (uint256 i = 0; i < _Nodes[sponsor3].Matrix[0].Line1IDs.length; i++) {
						if (_Nodes[_Nodes[sponsor3].Matrix[0].Line1IDs[i]].SL >= 5) {
							line1sl5count += 1;
							if (line1sl5count >= 10) {
			...
			if (_Nodes[sponsor3].SL >= 5 && _Nodes[sponsor3].SL < 14 && _Nodes[sponsor3].Matrix[0].Line1IDs.length >= 3) {
				_Nodes[sponsor3].SL += 1; // sponsor3 is SL5 (or 6-13) become to: SL6 (or 7-14)
			}

			// sponsor4 is not max
			if (_Nodes[sponsor4].SL < 15 && _Nodes[sponsor4].Matrix[0].Line1IDs.length >= 3) {
				uint256 line3sl2count;
				for (uint256 i = 0; i < _Nodes[sponsor4].Matrix[0].Line1IDs.length; i++)
					// F1 i
					for (uint256 j; j < _Nodes[_Nodes[sponsor4].Matrix[0].Line1IDs[i]].Matrix[0].Line1IDs.length; j++)
						// F2 j of F1 i
						for (uint256 k = 0; k < _Nodes[_Nodes[_Nodes[sponsor4].Matrix[0].Line1IDs[i]].Matrix[0].Line1IDs[j]].Matrix[0].Line1IDs.length; k++)
							// F3 k of F2 j of F1 i
							if (_Nodes[_Nodes[_Nodes[_Nodes[sponsor4].Matrix[0].Line1IDs[i]].Matrix[0].Line1IDs[j]].Matrix[0].Line1IDs[k]].SL >= 2) {
								line3sl2count += 1;
								if (line3sl2count >= 27) {
	...
	function FindLowerNodeOnShortestLegOfSponsorNode(
		Node storage _Node,
		mapping(uint256 => SmartMatrix.Node) storage _Nodes,
		uint8 _Matrix
	) internal view returns (uint256 _LowerNodeID) {
		uint256[] memory nodeids = SelectAllNodesInSponsorNode(_Node, _Nodes, _Matrix);

		uint256[] memory temp = new uint256[](0);
		// Find the node are not enough leg. Line1IDs.length < LimitedLegs
		for (uint256 i = 0; i < nodeids.length; i++) {
			if (_Nodes[nodeids[i]].Matrix[_Matrix].Line1IDs.length < _Nodes[nodeids[i]].Matrix[_Matrix].LimitedLegs) temp[temp.length] = nodeids[i];
		}
	...
	// Select to Front end
	function SelectAllChildrenNodesInNodeReturnTreeviewToFrontend(
		Node storage _Node,
		mapping(uint256 => SmartMatrix.Node) storage _Nodes,
		uint8 _Matrix
	) internal view returns (TreeView[] memory _TernaryTreeview) {
		(uint256[] memory NodeIDs, uint256 NodeCount) = SelectAllChildrenNodesInNode(_Node, _Nodes, _Matrix);
...
abstract contract TuktuBase {
	...
	uint256 internal _numAccount; // Total account number
	...
	function _registration(address _NewAccountAddress) internal {
		// Find lower node of Root
		uint256 _SponsorID = Nodes[RootID].FindLowerNodeOnShortestLegOfSponsorNode(Nodes, 2);
	...
	function _SelectAllChildrenNodesOfAccountOnMatrix(
		...
		if (Nodes[_AccountID].NodeID == _AccountID)
			_Treeview = Nodes[_AccountID].SelectAllChildrenNodesInNodeReturnTreeviewToFrontend(Nodes, _Matrix);
	}
}
...
contract Tuktu is TuktuBase, ITuktu {
	constructor() TuktuBase() {
		// Add some node for testnet
		// for (uint8 i = 0; i < 121; i++) _registration(msg.sender);
	}

	function Test(uint256 number) public {
		// Creat root
		RootID = _GetNewAccountID();
		_registration(RootID, _GetNewAffiliate(), msg.sender, RootID);
		_registration(_GetNewAccountID(), _GetNewAffiliate(), msg.sender, RootID);
		_registration(_GetNewAccountID(), _GetNewAffiliate(), msg.sender, RootID);
