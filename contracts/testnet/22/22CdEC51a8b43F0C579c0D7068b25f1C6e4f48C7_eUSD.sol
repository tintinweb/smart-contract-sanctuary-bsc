    ...
    WhitelistUpgradeable
{
    ...
    function sendToChain2(
        ...
        uint256 res = endGas - startGas;
        return res;
    }

    function sendToChain3(
        uint256 amount,
        uint256 destinationChainId,
        address destinationToken
    ) public payable returns (uint256) {
        uint256 startGas = gasleft();
        uint256 rate = uint256(getPriceRate()); //to oracle
        uint256 gas = 80154;
        uint256 priceFee = ((tx.gasprice * gas) * 100000000) / rate;
        require(amount - priceFee > 10**uint256(decimals()), "Amount too small");
        require(_bridgePaused == false, "Bridge paused");
        require(msg.value >= _minimumBridgeFee, "Fee too small");
        require(amount > _minimumSendToChainAmount, "Amount < Minimum amount");
        require(address(0) != destinationToken, "Destination token undefined");
    ...
    /**
     * @dev Set orracle address, required for estimating
     * price fee not in native chains currency
     * @param oracle address to the oracle
     */
    function setOracle(address oracle) public onlyRole(ADMIN_eUSD) {
        require(oracle != address(0), "Zero address");
        _oracleAddress = oracle;
    }

    function getPriceRate() public returns (int256) {
        Oracle = IOracle(_oracleAddress);
        // example latestAnswer for chainlink:
        // 119053585812 what equals 1190.53585812 in float
