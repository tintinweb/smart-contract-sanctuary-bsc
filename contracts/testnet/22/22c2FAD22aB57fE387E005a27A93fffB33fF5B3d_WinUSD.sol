...
contract WinUSD is IERC20, Context, Ownable {
    ...
    // token data
    string constant _name = "WinUSD";
    string constant _symbol = "WUSD";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        require(sender == msg.sender, 'cannot move funds for other accounts with WIN :D');
    ...
    function purchase(address buyer, uint256 bnbAmount) internal returns (bool) {
        ...
        uint256 tokensToSend = nShouldPurchase.mul(spreadDivisor).div(10**2).mul(buyFee).div(10**2);
        // revert if under 1
        if (tokensToSend < 1) {
