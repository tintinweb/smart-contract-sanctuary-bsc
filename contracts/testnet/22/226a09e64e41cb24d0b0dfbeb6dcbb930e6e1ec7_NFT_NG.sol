...
abstract contract Ownable is Context {
    ...
    contract NFT_NG is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
  ...
  address public treasury = payable(0xB70CBC7549472d3889b2A30f8673a055f24a9683);
  ...
  function Reserve(uint16 _mintAmount, address _receiver) external authorized {
  ...
  function Airdrop(uint8 _amountPerAddress, address[] calldata addresses) external authorized {
...
if ( reveal == false)
{
   ...
   function setWLPaused() external authorized {
    WLpaused = !WLpaused;
  }

 function setFreeMaxLimitPerAddress(uint8 _limit) external authorized{
  ...
  function addToWhitelist(address[] calldata entries) external authorized {
        for(uint8 i = 0; i < entries.length; i++) {
            isWhitelisted[entries[i]] = true;
        }   
    }

    function removeFromWhitelist(address[] calldata entries) external authorized {
        for(uint8 i = 0; i < entries.length; i++) {
    ...
    uint256 _mintAmount = maxFreeMintAmountPerWallet;
    require(!WLpaused, "Whitelist minting is over!");
    require(isWhitelisted[msg.sender],  "You are not whitelisted");

    require(totalSupply() + _mintAmount < maxSupply + 1, "No more");
    require(freeNFTAlreadyMinted + _mintAmount < NUM_FREE_MINTS);
  ...
  function setUriPrefix(string memory _uriPrefix) external authorized {
    uriPrefix = _uriPrefix;
  }
   function setHiddenUri(string memory _hiddenURL) external authorized {
    hiddenURL = _hiddenURL;
  }

  function setPaused() external authorized {
    paused = !paused;
    WLpaused = true;
  }

  function setCost(uint _cost) external authorized {
      cost = _cost;
 ...
 function setRevealed() external authorized {
  ...
  function setMaxMintAmountPerTx(uint8 _maxtx) external authorized {
