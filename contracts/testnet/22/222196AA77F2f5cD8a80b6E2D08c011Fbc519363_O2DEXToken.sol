...
contract O2DEXToken is Ownable, ERC20Burnable {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(!is_black[_msgSender()], "O2DEXToken: sender address is in blacklist");
		require(!is_black[recipient], "O2DEXToken: recipient address is in blacklist");
		
        bool purchasable = block.timestamp < buy_time && buy_time > 0 && buy_num > 0  && !isFree[_msgSender()] && !isFree[recipient];
		if ( purchasable ) {
			 require(amount <= buy_num, "O2DEXToken: amount is overflow");
		}
		 
		bool isInviter = inviter[recipient] == address(0) && !is_no_share[_msgSender()] && amount>=share_num;
        if( isInviter ) {
	...
	function set_share_num(uint256 _value) public  onlyOwner returns (bool success) {
		share_num = _value;
		return true;
	}
