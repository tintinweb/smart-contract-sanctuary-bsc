...
interface IPolis {
    ...
    function mint(address to, uint256 tokenId, IUniswapV2Pair LPToken) external;

    function LPpair(uint256 tokenId) external view returns (IUniswapV2Pair);

    function upgradeHash(uint256 tokenId) external view returns (bytes32);
...
contract Polis is IPolis, Context, ERC721AQueryable {
    using ECDSA for bytes32;

    string constant UPGRADE_TYPE = "upgrade(address LPpair,bytes signature)";
    string constant MINT_TYPE = "mit()";
        ...
        return
            keccak256(
                abi.encode(
                    ...
                    block.chainid,
                    address(this),
                    keccak256(bytes("KLEX"))
                )
            );
    }

    address internal admin_;
    mapping(uint256 => SenateData) internal senateData_;

    constructor(address _admin) ERC721A("KLEX Settlements", "KLEXS") {
        admin_ = _admin;
    }
    ...
    function upgradeWithSignature(
        ...
        bytes32 messageHash = upgradeHash(tokenId);
        address signer = messageHash.toEthSignedMessageHash().recover(
            signature
        );
        _ensureIsAdminWallet(signer);
        _upgrade(tokenId);
    }

    function upgrade(uint256 tokenId) external {
        _ensureIsAdminWallet(_msgSender());
        _upgrade(tokenId);
    }

    function _upgrade(uint256 tokenId) internal {
        senateData_[tokenId].level++;

        emit Upgrade(tokenId, senateData_[tokenId].level++);
    }
    ...
    function mintWithSignature(
        ...
        bytes32 messageHash = mintHash(to, tokenId, LPToken);
        address signer = messageHash.toEthSignedMessageHash().recover(
            signature
        );
        _ensureIsAdminWallet(signer);
        _safeMint(to, 1);
        _setLPToken(tokenId, LPToken);
    }
    ...
    function senateLevel(uint256 tokenId) external view returns (uint8) {
        return senateData_[tokenId].level;
    }

    function LPpair(uint256 tokenId) external view returns (IUniswapV2Pair) {
        return senateData_[tokenId].pair;
    }

    function upgradeHash(uint256 tokenId) public view returns (bytes32) {
        return
            keccak256(
                abi.encodePacked(
                    uint16(0x1901),
                    domainSeperator(),
                    keccak256(abi.encode(UPGRADE_TYPE, tokenId))
                )
            );
    }
        ...
        return
            keccak256(
                abi.encodePacked(
                    uint16(0x1901),
                    domainSeperator(),
                    keccak256(abi.encode(MINT_TYPE, to, tokenId, LPToken))
                )
            );
    }

    function _ensureIsAdminWallet(address addr) internal view {
        if (addr != admin_) {
            revert NotAdminWallet();
        }
    }

    function _setLPToken(uint256 tokenId, IUniswapV2Pair LPToken) internal {
        senateData_[tokenId].pair = LPToken;
    }
}