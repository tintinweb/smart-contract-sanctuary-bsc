...
/// @dev deployed on BSC chain(dest chain).
contract CrossFarmingReceiver is MessageSenderApp, MessageReceiverApp {
    ...
    function executeMessage(
        ...
        CrossFarmingProxy proxy = CrossFarmingProxy(
            cProxy[request.account] == address(0) ? _createProxy(request.account, _srcChainId) : cProxy[request.account]
        );
        ...
        if (request.msgType == DataTypes.MessageTypes.Deposit) {
            ...
            proxy.deposit(request.pid, request.amount, request.nonce);
            // send BNB CHANGE to new cross-farming user
            // can't according to nonce == 0 to sent BNB CHANGE, if contract balance < BNB_CHANGE in exetremely situation
     ...
     */
    function _createProxy(address _user, uint64 _srcChainId) internal returns (address proxy) {
        require(cProxy[_user] == address(0), "User already has proxy");

        bytes memory bytecode = type(CrossFarmingProxy).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(block.timestamp, block.number, _user, _srcChainId));
...
/// @dev auto deployed on BSC chain by cross farming receiver contract(factory) during user 1st cross farming transaction.
contract CrossFarmingProxy is ReentrancyGuard {
    ...
    /// @notice nonce from EVM chain(pid => nonce)
    mapping(uint256 => uint64) public latestNonce;
    /// @notice used fallback nonce (pid => (nonce => bool))
    mapping(uint256 => mapping(uint64 => bool)) public fallbackNonce;
    /// @notice (pid => amount)).
    ...
    event EmergencyWithdraw(address indexed caller, uint256 indexed pid, uint256 amount);
    event FallbackWithdraw(address indexed user, uint256 pid, uint256 amount, uint64 nonce);
    ...
    modifier onlyNotFallback(uint256 _pid, uint64 _nonce) {
        require(!fallbackNonce[_pid][_nonce], "used fallback nonce");
        _;
    }
    ...
    function deposit(
        ...
        emit Deposit(user, _pid, _amount);
    }
    ...
    function withdraw(
        ...
        emit Withdraw(user, _pid, _amount);
    }
        ...
        onlyNotFallback(_pid, _nonce)
    {
        ...
        emit EmergencyWithdraw(user, _pid, amount);
    }
    ...
    /// @notice after call this function, operator still need to call 'fallbackDeposit' on vault contract deployed on EVM
    function fallbackDeposit(uint256 _pid, uint64 _nonce) external onlyFactory onlyNotFallback(_pid, _nonce) {
        // only mark fallback nonce used
