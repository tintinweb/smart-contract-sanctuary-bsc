...
contract TBCrowdsale is Ownable {
    ...
    function setUSDTToken(IERC20 tokenAddress) public onlyOwner {
        usdtToken = tokenAddress;
        emit SetUSDTToken(tokenAddress);
    }
    ...
    function buyTokenByBNB() external payable {
        ...
        uint256 amount = bnbAmount * BNBRate;
        ...
        payable(wallet).transfer(bnbAmount); // chuyen BNB nhan duoc cho wallet.
        SafeERC20.safeTransfer(token, msg.sender, amount); // chuyen lai ico token cho buyer
        emit BuyTokenByBNB(msg.sender, amount);
    }

    function buyTokenByUSDT() external payable {
        uint256 usdtAmount = msg.value;
        uint256 amount = usdtAmount * USDTRate;

        require(amount > 0, "Amount is zero");
        // yeu cau Contract co lon hon amount icotoken de ban.
        require(token.balanceOf(address(this)) >= amount, "Insufficient account balance");

        // transfer de chuyen usdt cho wallet 
        payable(wallet).transfer(usdtAmount);

        // Contract chuyen icotoken cho sender
        SafeERC20.safeTransfer(token, msg.sender, amount);
        emit BuyTokenByUSDT(msg.sender, amount);
    }

    // Rut het icoToken ve cho owner
