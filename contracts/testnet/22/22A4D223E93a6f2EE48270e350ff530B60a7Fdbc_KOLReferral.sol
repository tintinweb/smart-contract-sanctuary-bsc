...
contract KOLReferral is Initializable, ERC2771Recipient {
    ...
    mapping(address => address[]) public kolToUsers;
    ...
    function storeUserInfo(string memory referralId) external {
        ...
        kolToUsers[kolWallet].push(_msgSender());
    }
    ...
    function getKOLTotalUsers(address kol) external view returns (uint256) {
        require(bytes(walletToReferralId[kol]).length != 0, "invalid kol");

        return kolToUsers[kol].length;
    }
