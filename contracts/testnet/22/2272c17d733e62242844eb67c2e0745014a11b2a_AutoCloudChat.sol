...
contract AutoCloudChat is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "AutoCloudChat";
    string constant _symbol = "ACC";
    ...
    uint256 public totalFee        = treasuryFee + reflectionFee + liquidityFee + devFee;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 5 * 10**9 * 10**_decimals;
    ...
    constructor (address newOwner) Auth(newOwner) {
        ...
        autoRebase = false;
        rebaseRate = 417550;
        rebaseTimerInterval = 30 minutes;
        _lastRebasedTime = block.timestamp;

        treasuryFeeReceiver = 0x3b38C0AFaC52803C76f1969db6Ec1218539dEA47;
        devFeeReceiver = 0x2B131Bc5F4aE10e3B95d1683563Ea561B9D71D79;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != treasuryFeeReceiver && recipient != devFeeReceiver){
    ...
    function cooldownEnabled(bool _status, uint8 _interval) external onlyOwner {
        require(_interval <= 3600, "Buy cooldown cannot be longer than 1 hour");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _treasuryFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        treasuryFee = _treasuryFee;
        devFee = _devFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_treasuryFee).add(_devFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/5, "Fees cannot be more than 20% for buy and 25% for sell");
    }
    ...
    function setMaxTxPercent_base1000(uint256 maxTXPercentage_base1000) external onlyOwner() {
        require(maxTXPercentage_base1000 > 0, "Max transfer amount cannot be lower than 0.1%");
