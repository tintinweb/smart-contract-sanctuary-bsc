...
contract PancakeFlashSwap {
    ...
    function startArbitrage(
        ...
        tokens = Tokens(
            _tokenBorrow,
            _tokenInPairForBorrow,
            _tokenSwap1,
            _tokenSwap2
        );
        IERC20(tokens.tokenBorrow).safeApprove(address(router), MAX_INT);
        IERC20(tokens.tokenSwap1).safeApprove(address(router), MAX_INT);
        IERC20(tokens.tokenSwap2).safeApprove(address(router), MAX_INT);

        // Get the Factory Pair address for combined tokens
        address pair = IUniswapV2Factory(factory).getPair(
            tokens.tokenBorrow,
            tokens.tokenInPairForBorrow
        );
    ...
    function pancakeCall(
        ...
        (
            ...
            address router,
            address myAddress
        ) = abi.decode(_data, (address, uint256, address, address, address));
        ...
        // // Pay Myself
        IERC20 otherToken = IERC20(tokens.tokenBorrow);
