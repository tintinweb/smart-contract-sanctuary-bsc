...
contract DefiBank {
    ...
    uint256 public InterestRateFee = 120;

    // create 1 array to add all your clients
    address[] public stakers;
    address[] public stakersBNB;

    // create a 3 maps 
    mapping(address => uint) public stakingBalance; //Clients balance
    mapping(address => uint) public stakingBalanceBNB; //Clients balance
    ...
    function depositBUSD(uint _amount) public {
     ...
     function setInterestRateFee(uint256) private{
          InterestRateFee = InterestRateFee; 
     }
    ...
    function withdrawBNB() public returns(uint256) {
        ...
        //Deduct from bank
        stakingBalanceBNB[msg.sender] -= balance;
        address payable receiver = payable(msg.sender);
        receiver.transfer(balance);
        return stakingBalanceBNB[msg.sender];
        
    }
   function sendInterestRateFee() public{
          // require the permission of the current owner
        require(owner == msg.sender, "Your are not the current owner");
