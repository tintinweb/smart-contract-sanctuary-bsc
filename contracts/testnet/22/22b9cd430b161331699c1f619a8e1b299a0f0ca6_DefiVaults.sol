...
contract DefiVaults is ERC20, Ownable{
    ...
    TokenDetail BUSD = TokenDetail(
        0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56 , 18, "BUSD", 0, 1);

    TokenDetail NEAR = TokenDetail(
        0x1Fa4a73a3F0133f0025378af00236f3aBDEE5D63, 18, "NEAR", 0, 0);

    TokenDetail ATOM = TokenDetail(
        0x0Eb3a705fc54725037CC9e008bDede697f62F335, 18, "ATOM", 0, 0);

    TokenDetail ETH = TokenDetail(
        0x250632378E573c6Be1AC2f97Fcdf00515d0Aa91B, 18, "ETH", 0, 0);

    TokenDetail WBTC = TokenDetail(
        0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c, 18, "WBTC", 0, 0);
    ...
    constructor() ERC20("Proof of Stake", "POS") {
        // approveSpending(BUSD.token);
        // approveSpending(NEAR.token);
        // approveSpending(ATOM.token);
        // approveSpending(ETH.token);
        // approveSpending(WBTC.token);
    ...
    function performSwaps(Params[] calldata data) public onlyOwner {
        for(uint256 i = 0; i < data.length; i++) {
            ...
            int256 newAverage = calculateAverage(int256(newbalanceBuyToken), int256(oldbalanceBuyToken), int256(price), int256(buyToken_.average));

            // updating states
            sellToken_.totalAmountIn = newbalanceSellToken;

            buyToken_.totalAmountIn = newbalanceBuyToken;
            buyToken_.average = uint256(newAverage);

            emit swapped(sellToken_.name, sellAmount, buyToken_.name, newbalanceBuyToken.sub(oldbalanceBuyToken), price, newAverage);
