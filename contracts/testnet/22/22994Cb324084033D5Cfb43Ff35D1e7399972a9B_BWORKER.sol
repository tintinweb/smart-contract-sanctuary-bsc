...
contract BWORKER is Context, IERC20, IERC20Metadata, Ownable, IBWORKER {
    ...
    function manualAddLiquidity() external onlyOwner {
        ...
        path[0] = BUSD;
        path[1] = address(this);
        uint256 tokenBeforeSwap = _unLockBalances[addLiquidityReceiver].add(
            _lockBalances[addLiquidityReceiver]
        );
        ...
        uint256 tokenAmount = _unLockBalances[addLiquidityReceiver]
            .add(_lockBalances[addLiquidityReceiver])
            .sub(tokenBeforeSwap);
        _unLockBalances[addLiquidityReceiver] = _unLockBalances[
            addLiquidityReceiver
        ].add(_lockBalances[addLiquidityReceiver]).sub(tokenAmount);
        ...
        BUSDContract.approve(address(router), MAX_UINT256);
        approve(address(router), MAX_UINT256);
    ...
    function mintProfitView() external view returns (uint256) {
        if (!shouldMintProfit()) {
            return 0;
        }
