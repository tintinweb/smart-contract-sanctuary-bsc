...
contract Staking is IStaking, Initializable, Context {
    ...
    modifier onlyIdoInstance() {
        require(
            _admin.isAnIDO(_msgSender()),
            "Staking: not an ido instance"
        );
        _;
    }
    ...
    modifier canUnstake() {
        ...
        require(
            block.timestamp > self.idoEndTime &&
    ...
    function setIdoEndTime(address address_, uint32 time_)
        external
        override
        onlyIdoInstance
    {
        if (_stakingState[address_].idoEndTime < time_) {
            _stakingState[address_].idoEndTime = time_;
        }
    }

    function setLockingPeriod(uint256 period_) external onlyOperator {
        require(period_ > 0, "Staking: Non-zero");

        _stakingLockPeriod = period_;
    }

    function _withdraw(address recipient_, uint256 amount_) private {
        _stakingState[_msgSender()]._updateStakeAmount(amount_, false);
        _CFLUToken.safeTransfer(recipient_, amount_);
    }
}
...
library StakingLibrary {
    ...
    function _setState(
        StakingDetails storage self_,
        uint256 amount_,
        uint256 canUnstakeAt_
    ) internal {
        self_.canUnstakeAt = uint64(canUnstakeAt_);
        self_.amount += uint128(amount_);
    }
    ...
    function _setIdoEndTime(StakingDetails storage self_, uint256 endTime_)
        internal
    {
        self_.idoEndTime = uint64(endTime_);
    }
}