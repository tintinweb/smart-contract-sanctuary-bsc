...
contract Lend is IERC721Receiver {
  ...
  uint256 internal lockPercentageThreshold;
  uint256 internal graceTime;
  address internal depositAddress;
  uint256 internal maxLendingPeriod;
  ...
  mapping(address => mapping(uint256 => User)) internal userInfo;
  ...
  function setCollateralPercentageThreshold(uint256 _maximumLockPercentage) public onlyAdmin {
    require(_maximumLockPercentage > 0, "Value cannot be less then 1");
    lockPercentageThreshold = _maximumLockPercentage;
  }

  function setGraceTime(uint256 _timeInBlocks) public onlyAdmin {
    require(_timeInBlocks > 0, "Value cannot be less then 1");
    graceTime = _timeInBlocks;
  }

  function setMaxLendingPeriod(uint256 _maxLendingPeriod) public onlyAdmin {
  ...
  function addSupportedAsset(
    ...
    supportedAddress2[veNFTAddress] = tokenAddress;
  }
  ...
  function getCollateralPercentageThreshold() public view returns (uint256) {
    return lockPercentageThreshold;
  }

  function getGraceTime() public view returns (uint256) {
    return graceTime;
  }
  ...
  function getMaxLendingPeriod() public view returns (uint256) {
    return maxLendingPeriod;
  }

  function getMaxTokenAmount(address _veNftAddress, uint256 _nftId) public view returns (uint256) {
    return IVeNft(_veNftAddress).getAmount(_nftId).mul(lockPercentageThreshold).div(1e18);
  }

  function getBorrowedAmount(address _veNftAddress, uint256 _nftId) public view returns (uint256) {
    return userInfo[_veNftAddress][_nftId].amount;
  }

  function getExpectedInterestAmount(address _veNftAddress, uint256 _nftId) public view returns (uint256) {
    uint256 val = userInfo[_veNftAddress][_nftId].amount;
    uint256 interest = (val.mul(block.number.sub(userInfo[_veNftAddress][_nftId].lockPeriod)).mul(interestRatePerBlock).div(1e18));
    uint256 _fee = (value.mul(feeRate).div(1e18));
    uint256 _total = interest.add(_fee);
    return _total;
  }
  ...
  function lend(
    ...
    require(_cheeTokenAmount <= IVeNft(_veNftAddress).getAmount(_nftId).mul(lockPercentageThreshold).div(1e18), "Value should be less then Max");
  ...
  function repay(
    ...
    uint256 lockPeriod = userInfo[_veNftAddress][_nftId].lockPeriod.add(maxLendingPeriod).add(graceTime);
    ...
    value = userInfo[_veNftAddress][_nftId].amount;
    uint256 interest = (value.mul(block.number.sub(userInfo[_veNftAddress][_nftId].lockPeriod)).mul(interestRatePerBlock).div(1e18));
    uint256 fee = (value.mul(feeRate).div(1e18));
    uint256 total = interest.add(fee);
    IChee(supportedAddress1[_veNftAddress]).burnFrom(msg.sender, _cheeAmount);
    IToken(supportedAddress2[_veNftAddress]).transferFrom(msg.sender, depositAddress, total);
  ...
  function liquidate(
    ...
    require(userInfo[_veNftAddress][_nftId].amount == _cheeAmount, "Enter Correct Amount");
    value = userInfo[_veNftAddress][_nftId].amount;
    uint256 interest = (value.mul(block.number.sub(userInfo[_veNftAddress][_nftId].lockPeriod.add(graceTime))).mul(interestRatePerBlock).div(1e18));
    uint256 fee = (value.mul(feeRate).div(1e18));
    uint256 total = interest.add(fee);
    IChee(supportedAddress1[_veNftAddress]).burnFrom(msg.sender, _cheeAmount);
    IToken(supportedAddress2[_veNftAddress]).transferFrom(msg.sender, depositAddress, total);
