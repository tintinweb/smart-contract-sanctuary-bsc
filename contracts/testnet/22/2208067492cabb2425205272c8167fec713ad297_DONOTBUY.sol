...
contract DONOTBUY is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal =1000000 * uint256 (10 ** _decimals);
    ...
    uint256 public defaultLiquidityFee = 15;
    uint256 public _liquidityFee = defaultLiquidityFee;
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _liquidityFee4Buyers = 10;
    uint256 private _previousLiquidityFee4Buyers = _liquidityFee4Buyers;

    uint256 public _liquidityFee4Sellers = 13;
    uint256 private _previousLiquidityFee4Sellers = _liquidityFee4Sellers;
    
    uint256 public _maxWallet = _tTotal * 2 / 100; // 1% Max Wallet
    uint256 private _previousMaxWallet = _maxWallet;
    uint256 public maxTransactionAmount = _tTotal * 1 / 100;// 1% maxTransactionAmount;
    ...
    uint256 public numTokensSellToAddToLiquidity = 100000 * 10**uint256(_decimals);      // contract balance to trigger swap & liquify + wallet transfer.                                                                                                   
    ...
    uint[] public _percentages = [18,40,18,6,18];                                         // % of liquidity fee split to wallet(s)
    address payable[] public _wallets = [                                                // wallet(s) to receive liquidity fee split
        ...
        payable(0xc50CAfB1A3cb6730257472ec207003A4A5E82c0d)                              // Team 3
 ];
 ...
 // once enabled, can never be turned off
    ...
    function sendETHToWallets(uint256 amount) private {
        ...
        for(uint i=0; i<_wallets.length-1; i++){
            uint amountToWallet = calculateAmount(ETHBalance, _percentages[i]);
    ...
    //function _goLive() external onlyOwner() {
     //   require(!isTokenLive, "DNB is already live");
      //  isTokenLive = true;
   // }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for BNB
       swapTokensForEth(liquifyHalves[0]); // <- this breaks the BNB -> DNB swap when swap+liquify is triggered
        ...
        sendETHToWallets(portionForFees);
