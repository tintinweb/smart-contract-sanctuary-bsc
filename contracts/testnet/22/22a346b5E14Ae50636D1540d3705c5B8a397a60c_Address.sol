...
// Farm distributes the ERC20 rewards based on staked tokens to each user.
//
// Cloned from https://github.com/SashimiProject/sashimiswap/blob/master/contracts/MasterChef.sol
// Modified by Ethos Project to work for non-mintable ERC20 and non-LP tokens.
contract BeanFarm is Ownable {
    ...
    // Info of each pool.
    struct PoolInfo {
        IERC20 stakedToken;         // Address of token contract to be staked.
    ...
    function fund(uint256 _amount) public {
        ...
        endBlock = block.number + erc20.balanceOf(address(this)).div(rewardPerBlock);
    }
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 tokenSupply = pool.stakedToken.balanceOf(address(this));
        uint256 lastBlock = block.number < endBlock ? block.number : endBlock;

        if (lastBlock > pool.lastRewardBlock && block.number > pool.lastRewardBlock && tokenSupply != 0) {
