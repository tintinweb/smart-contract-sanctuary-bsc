...
contract V3PegBoardroom is IV3PegBoardroom, OwnableUpgradeSafe, ContractGuard {
    ...
    event PegTokenRewardSacrificed(address indexed pegToken, address indexed user, uint256 reward);
    event PegTokenRewardAdded(address indexed pegToken, address indexed user, uint256 reward);

    /* ========== Modifiers =============== */

    modifier onlyMainboardroom() {
        require(mainBoardroom == msg.sender, "V3PegBoardroom: caller is not the main boardroom");
        _;
    }

    /* ========== GOVERNANCE ========== */

    function initialize(address _mainBoardroom) external initializer {
        OwnableUpgradeSafe.__Ownable_init();
        mainBoardroom = _mainBoardroom;
        initialized = true;
    }

    function setMainBoardroom(address _mainBoardroom) external onlyOwner {
        mainBoardroom = _mainBoardroom;
    }

    function addPegToken(address _token) external onlyOwner {
        require(boardroomHistory[_token].length == 0, "V3PegBoardroom: boardroomHistory exists");
    ...
    function earned(address _token, address _member) public override view returns (uint256) {
        ...
        return ShareWrapper(mainBoardroom).balanceOf(_member).mul(latestRPS.sub(storedRPS)).div(1e18).add(members[_token][_member].rewardEarned);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function updateReward(address _token, address _member) external override onlyMainboardroom {
        if (_token != address(0) && _member != address(0)) {
    ...
    function claimReward(address _token, address _member) external override onlyMainboardroom {
        uint256 reward = members[_token][_member].rewardEarned;
        if (reward > 0) {
            members[_token][_member].rewardEarned = 0;
            IERC20(_token).safeTransfer(_member, reward);
            emit PegTokenRewardPaid(_token, _member, reward);
        }
    }

    function sacrificeReward(address _token, address _member) external override onlyMainboardroom {
        uint256 reward = members[_token][_member].rewardEarned;
        if (reward > 0) {
            members[_token][msg.sender].rewardEarned = 0;
            IBasisAsset(_token).burn(reward);
            emit PegTokenRewardSacrificed(_token, _member, reward);
        }
    }

    function allocateSeignioragePegToken(address _token, uint256 _amount) external override onlyMainboardroom {
        require(_amount > 0, "V3PegBoardroom: Cannot allocate 0");
        uint256 _totalSupply = ShareWrapper(mainBoardroom).totalSupply();
        require(_totalSupply > 0, "V3PegBoardroom: Cannot allocate when totalSupply is 0");
        require(boardroomHistory[_token].length > 0, "V3PegBoardroom: Cannot allocate when boardroomHistory is empty");
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOwner {
        ...
        require(boardroomHistory[address(_token)].length == 0, "core");
