...
contract MoonshotMechanism is IMoonshotMechanism {
    ...
    function shouldLaunchMoon(address from, address to) external view override returns (bool) {
        return (IERC20(address(WBNB)).balanceOf(address(this)) >= disbursalThreshold && disbursalThreshold != 0 && from != address(this) && to != address(this)); 
    }
...
//--------------------------------------BEGIN TRANSFER FUNCTIONS---------|
   ...
   function _transfer(
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if(MoonshotMechanismInterface.shouldLaunchMoon(sender, recipient)){ try MoonshotMechanismInterface.launchMoonshot() {} catch {} }
    }
