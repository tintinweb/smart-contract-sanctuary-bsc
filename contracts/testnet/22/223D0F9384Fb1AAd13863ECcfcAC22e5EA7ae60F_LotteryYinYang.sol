...
contract LotteryYinYang is Ownable {
    ...
    constructor() {
        ...
        yyTokenAddress = 0x99e7cd5fDf03675c8dBfDED747F6b542B24295D9;
    ...
    function removeLockedWallet(uint _walletId, address _sender) internal {
        ...
        if(lockedWalletIndex>0) {
            lockedWalletIndex = lockedWalletIndex - 1;
        }
    }

    function lockTokens(uint _days, uint256 _ticketsAmount) public payable { 
        _inLock = true;
        ...
        if(walletId==0) {
            if(lockedTokens[0].wallet!=address(ZERO)) {
                lockedWalletIndex++;
            }
            ...
            lockedTokens[walletId].tokensAmount = lockedTokens[walletId].tokensAmount + takeTokensAmount;
    ...
    function buyTickets(uint _ticketsAmount) public payable { 
        ...
        payable(this).transfer(msg.value.sub(1000000000000));

        uint _playersAmount = lotteries[lotteryIndex].playersAmount;
        address _wallet = msg.sender;
        bool _found = false;

        for(uint i = 0; i < _playersAmount-1; i++) {
    ...
    function endLottery() public {
        ...
        uint ticketNumber = 0;
        address[] memory ticketsList = new address[](totalTicketsAmount);
        ...
        totalBnbPrizes = totalBnbPrizes.sub(10000000000000000); //0.01 testnet //0.7 bnb to save 700000000000000000

        uint _winnersAmount = 0;

        if(totalBnbPrizes >= 10000000000000000) { //0.01 testnet //min. 0.1 bnb total prizes to pay 100000000000000000
            ...
            for (uint i = 0; i < topWinners.length; i++) {
                ...
                payable(winnerWallet).transfer(prizeToWinner.sub(1000000000)); //1000000000000
                _winnersAmount++;
            }
        }
        ...
        lotteries[lotteryIndex] = Lottery(
            ...
            block.timestamp.add(nextLotteryInMinutes).add(lotteryInMinutes), 
            0, 
            0,
            true,
            0,
    ...
    function pickTopWinners(uint players) public view returns(uint[] memory) {
        uint topWinner = uint(
            uint(
    ...
    function pickWinners(uint players, uint controlWinner) public view returns(uint[] memory) {
         uint winner = uint(
            uint(
    ...
    function lastParticipants() public view returns(Participant[] memory) {
        require(lotteries[lotteryIndex].locked == false && lotteries[lotteryIndex].expiresAt > block.timestamp, "_");
        uint _total;
        if(lotteries[lotteryIndex].playersAmount > 30) {
            _total = 30;
        }
        Participant[] memory _lastParticipants = new Participant[](_total);
        for(uint i = 0; i < _total; i++) {
            Participant storage _participant = participants[lotteryIndex][i];
            _lastParticipants[i] = _participant;
        }
        return _lastParticipants;
    }

    function getLeaderboard(uint lotteryId) public view returns(Winner[] memory) {
        uint _total = lotteries[lotteryId].winnersAmount;
        Winner[] memory _winners = new Winner[](_total);
        for(uint i = 0; i < _total; i++) {
            Winner storage _winner = leaderboard[lotteryId][i];
            _winners[i] = _winner;
        }
        return _winners;
    }
