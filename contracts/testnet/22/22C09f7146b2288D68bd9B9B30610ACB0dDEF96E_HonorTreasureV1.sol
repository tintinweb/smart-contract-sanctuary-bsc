...
contract HonorTreasureV1 is Ownable {
  ...
  function setFinanceContract(address finance,bool active,uint256 limit) public onlyOwner {
    FINANCE_CONTRACTS storage fCont=financeContracts[finance];
    fCont.isActive=active;
    fCont.limit=limit;
  }
...
function swap(address router, address _tokenIn, address _tokenOut, uint256 _amount) private {
 ...
 function _addLiquidity(address token0,address token1,uint256 amount) public onlyOwner {
  ...
  function depositToken(address _token,uint256 amount) public {
    ...
    liqAmount=balance - balance.div(3);
