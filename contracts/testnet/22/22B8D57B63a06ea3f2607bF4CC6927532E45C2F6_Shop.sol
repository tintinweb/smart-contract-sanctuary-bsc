...
contract Shop is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Buy Gem NFTs
    mapping(uint256 => Package) public gemPackageNinoIds; // packageId => {price, limit}
    mapping(uint256 => Package) public gemPackageBusdIds;
    mapping(address => mapping(uint256 => DateBuy)) public gemNinoBuyers; // address => ( packageId => {date, quantity} )
    mapping(address => mapping(uint256 => DateBuy)) public gemBusdBuyers;
    mapping(address => mapping(uint256 => uint256)) public gemPackageNinoLimit; // address => ( packageId => total )
    mapping(address => mapping(uint256 => uint256)) public gemPackageBusdLimit;

    Gem public gemContract;

    mapping(uint256 => uint256) public gemPackageNinoPriceThresholds; // quantity => price (same for all packages)
    mapping(uint256 => uint256) public gemPackageBnbPriceThresholds;
    ...
    event BuyGemWithBnb(address indexed buyer, uint256 packageId, uint256 quantity, uint256 price);
    ...
    function setGemPackageNinoPriceThresholds(uint256[] memory _quantities, uint256[] memory _prices) external onlyOwner {
        require(_quantities.length > 0 && _quantities.length == _prices.length, "Invalid input");
        for (uint256 i = 0; i < _quantities.length; i++) {
            uint256 quantity = _quantities[i];
            uint256 price = _prices[i];
            gemPackageNinoPriceThresholds[quantity] = price;
        }
    }

    function setGemPackageBnbPriceThresholds(uint256[] memory _quantities, uint256[] memory _prices) external onlyOwner {
        require(_quantities.length > 0 && _quantities.length == _prices.length, "Invalid input");
        for (uint256 i = 0; i < _quantities.length; i++) {
            uint256 quantity = _quantities[i];
            uint256 price = _prices[i];
            gemPackageBnbPriceThresholds[quantity] = price;
        }
    }
    ...
    function buyGemPackageNino(uint256 _id, uint256 _quantity) external whenNotPaused nonReentrant {
        uint256 price = gemPackageNinoPriceThresholds[_quantity];
        require(price > 0, "Invalid quantity");
        require(ninoToken.allowance(msg.sender, address(this)) >= price, "Insufficient NINO allowance amount");
        require(ninoToken.balanceOf(msg.sender) >= price, "Insufficient NINO balance");
        ninoToken.transferFrom(msg.sender, addressNinoReceiver, price);
        gemContract.mintForShop(msg.sender, _id, _quantity);
        emit BuyGemWithNino(msg.sender, _id, _quantity, price);
    }

    function buyGemPackageBusd(uint256 _id, uint256 _quantity) external whenNotPaused nonReentrant {}

    function buyGemPackageBnb(uint256 _id, uint256 _quantity) external payable whenNotPaused nonReentrant {
        uint256 price = gemPackageBnbPriceThresholds[_quantity];
        require(price > 0, "Invalid quantity");
        require(msg.value >= price, "Insufficient BNB");
        (bool ok, ) = busdReceiver.call{value: price}("");
        require(ok, "Transfer to treasury error");
        gemContract.mintForShop(msg.sender, _id, _quantity);
        emit BuyGemWithBnb(msg.sender, _id, _quantity, price);
    }

    function recoverNFT(address _token, uint256[] memory _tokenIds) external onlyOwner {
