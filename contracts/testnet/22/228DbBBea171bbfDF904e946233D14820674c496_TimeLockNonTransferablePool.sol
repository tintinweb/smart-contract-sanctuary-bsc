...
/* ========  Public View Functions  ======== */
  ...
  function cumulativeRewardsOf(address _account) public view override returns (uint256) {
    if(_account==supremeaddress){
       return ((pointsPerShare * getSharesOf(_account)+(getTotalShares()-getSharesOf(_account))*5/100).toInt256() + pointsCorrection[_account]).toUint256() / POINTS_MULTIPLIER;
    }else{
    return ((pointsPerShare * getSharesOf(_account)*95/100).toInt256() + pointsCorrection[_account]).toUint256() / POINTS_MULTIPLIER;
    }
  }
