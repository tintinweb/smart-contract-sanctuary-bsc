...
contract THTTokenSale is ReentrancyGuard, Context, Ownable {
    ...
    function buyTokens(uint256 tokensAmount) public nonReentrant payable {
        ...
        _part[msg.sender] = _rewards[msg.sender]/16;
    ...
    function buyTokensToWallet(uint256 tokensAmount, address account) public nonReentrant payable {
        ...
        _token.approve(account, scaledAmount);
        _rewards[account] = scaledAmount;
        _wallet.transfer(address(this).balance);

        firstPayment(account, scaledAmount);
        _part[msg.sender] = _rewards[msg.sender]/16;
        _lastUpdatedTime[account] = block.timestamp;
        _unlockTime[account] = block.timestamp + 5256000;
        
        emit TokensPurchased(account, _wallet, weiAmount, scaledAmount);
    }
    ...
    function otherPayments(address account) public onlyOwner {
        require(block.timestamp >= _unlockTime[account], "Wallet is still locked");
        require(_rewards[account] >= 0, "Rewards should be more than 0");
        _token.transfer(account, _part[account]);
        _rewards[account] -= _part[account];
        _unlockTime[account] = block.timestamp + 2628000;
    }
