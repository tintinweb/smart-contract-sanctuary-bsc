...
contract CrossPair is ICrossPair, CrossERC20 {
    ...
    //Session global variables + CC, these 4 should use single storage slot
    address public controlCenter = 0x365f3b29ed8D6A942B82891eb220EE3E9B543810;
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        _mint(to, liquidity);
        currentSessionRatio = (balance0 * 10**18) / balance1;
    ...
    function swap(
        ...
        require(
            ICrossFactory(factory).isPairDelisted(address(this)) != true ||
                ICrossFactory(factory).isTradingHalted() == true,
            "Pair not listed"
        );
        /*************/
        require(
        ...
        );
        {
            ...
            uint256 balance0Adjusted = (
                balance0.mul(10000).sub(
                    amount0In.mul(
                        ICrossFactory(factory).dexFee(tx.origin, address(this))
                    )
                )
            );
            uint256 balance1Adjusted = (
