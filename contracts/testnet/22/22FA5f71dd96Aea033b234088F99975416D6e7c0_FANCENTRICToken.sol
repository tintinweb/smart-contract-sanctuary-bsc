...
// FANCENTRICToken with Alex2. 2022-03-11
contract FANCENTRICToken is BEP20 {
    ...
    uint16 public liquifyRate = 250; // INMUTABLE
    uint16 public feeRate = 250;
    uint16 public ufxFeeRate = 100;

    // Min amount to liquify. (default 500 FANCENTRIC)
    ...
    // FANCENTRIC ufxFeeWallet
    address public ufxFeeWallet; // INMUTABLE
        ...
        if(_maxHoldingEnable) {
            if (maxHolding() > 0 && sender != owner() && recipient != owner()) {
                if ( _excludedFromAntiWhale[recipient] == false ) {
                    require(amount <= maxHolding() - balanceOf(recipient) && balanceOf(recipient) <= maxHolding(), "FANCENTRIC::antiWhale: Transfer amount would result in a balance bigger than the maxHoldingRate");
                }
            }
        }
    ...
    /**z
     * @notice Constructs the FANCENTRIC token contract.
     */
    //_PCSRouter
    //0x10ED43C718714eb63d5aA57B78B54704E256024E mainnet
    //0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 testnet
    constructor(address _feeWallet,address _ufxFeeWallet ,address _PCSRouter) public BEP20("FANCENTRIC", "FANC") {
        ...
        ufxFeeWallet = address(_ufxFeeWallet);
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(fancentricSupply().add(_amount) <= cap, "UFX: cap exceeded");
        _mint(_to, _amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (sender == owner() || recipient == owner() || transferTaxRate == 0 || _isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            //ex 100fancentric
            //transferTaxRate == 6%
            //taxAmount == 6fancentric
            uint256 taxAmount = amount.mul(transferTaxRate).div(10000);// 6 fanc

            //ex) 100 fancentric transfer
            uint256 liquidityAmount = amount.mul(liquifyRate).div(10000); // 2.5% -> 2.5 swap and liquifiy
            uint256 feeWalletAmount = amount.mul(feeRate).div(10000); // 2.5% -> 2.5 fancentric -> feeWallet
            uint256 ufxFeeWalletAmount = taxAmount.sub(feeWalletAmount).sub(liquidityAmount); // 6 fancentric - 2.5 - 2.5 = 1fan

            require(taxAmount == liquidityAmount.add(feeWalletAmount).add(ufxFeeWalletAmount) , "FANCENTRIC::transfer: liquidityAmount, feeWalletAmount or ufxFeeWalletAmount value invalid");
            
            uint256 sendAmount = amount.sub(taxAmount);// 100 - 6 => 94fan
            require(amount == sendAmount.add(taxAmount), "FANCENTRIC::transfer: liquidityAmount, feeWalletAmount or ufxFeeWalletAmount value invalid");


            // Distributing FANCENTRICs (feeWallet, Liquify, Recipient)
            super._transfer(sender, address(this), liquidityAmount); // 2.5 fan
            super._transfer(sender, feeWallet, feeWalletAmount); // 2.5 fan
            super._transfer(sender, ufxFeeWallet, ufxFeeWalletAmount); // 1fancentric
            super._transfer(sender, recipient, sendAmount); // 96fancentric -> user
            amount = sendAmount;           
        }
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the FANCENTRIC pair path of token -> weth
    ...
    //     require(newPCSRouter != address(0), "FANCENTRIC::transferPCSRouter: new PCSRouter is the zero address");
    ...
    //     require(_maxHoldingRate >= 100, "FANCENTRIC::updateMaxHoldingRate: Max holding rate must not be below the minimum rate.");
    ...
    function fancentricSupply() public view returns (uint256) {
    ...
    function transferLpToken(address newLpToken) public onlyOperator {
        ...
        require(lpToken == address(0), "FANCENTRIC: LP Token Transfer can be only be set once");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        ...
        require(newOperator != address(0), "FANCENTRIC::transferOperator: new operator is the zero address");
    ...
    function openTrading() public onlyOperator {
        ...
        require(tradingOpen != true, "FANCENTRIC: Presale is already opened.");
        tradingOpen = true;
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "FANCENTRIC::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "FANCENTRIC::delegateBySig: invalid nonce");
        require(now <= expiry, "FANCENTRIC::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "FANCENTRIC::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying FANCENTRICs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "FANCENTRIC::_writeCheckpoint: block number exceeds 32 bits");
