...
contract WaltsFarmer {
	...
	address erctoken = 0xE3b3d3558f6F1b5bb59f2064CEa91482abE44d93; /** testnet WALT **/
    //address erctoken = 0x6E5997a44Ea15Dec9feD08d8131dB392D5f3A03A; /** mainnet WALT **/
    ...
    bool public started;
    uint256 public MIN_INVEST = 10 * 1e9; //10 walt
    uint256 public MAX_WALLET_DEPOSIT = 20000 * 1e9; //20,000 walt
    ...
    constructor(address payable ownerAddress, address payable projectAddress, address payable partnerAddress) {
        require(!isContract(ownerAddress) && !isContract(projectAddress) && !isContract(partnerAddress));
        owner = ownerAddress;
    ...
    function depositWithNickname(string calldata _nickname, uint256 _amount) payable external {
        require(started, "Contract not yet started.");
    ...
    function _deposit(address _addr, uint256 _amount) private {
        ...
        token_WALT.transferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw() external {
        ...
        token_WALT.transfer(msg.sender, payout);
    ...
    function airdrop(address payable _to, uint256 _amount) payable external {
        ...
        if(project_fee > 0){
		    token_WALT.transfer(project, project_fee);
        }
    ...
    function teamAirdrop(uint256 teamId, uint256 _amount) payable external {
    ...
    function fees(uint256 amount) internal returns(uint256){
        ...
        uint256 part = amount.mul(PARTNER).div(PERCENTS_DIVIDER);
		token_WALT.transfer(project, proj);
        token_WALT.transfer(partner, part);
        return proj.add(part);
    }

    function withdrawalTaxPercentage(uint256 to_payout) view external returns(uint256 finalPayout) {
      uint256 contractBalance = token_WALT.balanceOf(address(this));
   ...
   function changeTeamName(uint256 teamId, string memory name) external {
    ...
    function CHANGE_PARTNER_WALLET(address value) external {
        require(msg.sender == owner, "Admin use only");
    ...
    function CHANGE_PARTNER_FEE(uint256 value) external {
        require(msg.sender == owner, "Admin use only");
