...
contract DividendDistributor is IDividendDistributor {
    ...
    mapping (address => uint256) shareReleaseAmount;
    ...
    function distributeClaimToken(address shareholder) external {
         if(shouldDistribute(shareholder)){
            uint256 amountclaim = shareReleaseAmount[shareholder];
            // thiscontract.sendToken(shareholder, amountclaim);
...
contract DoToken is IBEP20, Auth {
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        if(sender == pair){tempTotalFee = buyFee;}
    ...
    function swapBack() internal swapping {
        ...
        amountBuyBack = amountBuyBack + swapThreshold.mul(buybackFee).div(totalFee);
        uint256 amountToSwap = swapThreshold.mul(240).div(totalFee);
        ...
        amountMarketing = amountMarketing + amountBNB.div(4);
        amountDev = amountDev + amountBNB.div(4);
        amountAdmin = amountAdmin + amountBNB.div(2);
        ...
        // payable(owner).transfer(amountBNBAdmin);
    ...
    //###################################################################
    function charityReceiver()external{
        require(msg.sender == charityFeeReceiver,"You are not CharityReceiver");
        _balances[msg.sender] =  _balances[msg.sender].add(amountCharity);
        _balances[address(this)] = _balances[address(this)].sub(amountCharity);
        amountCharity = 0;
    }

    function voterRewardsReceiverFun()external{
        require(msg.sender == voterRewardsReceiver,"You are not voter");
        _balances[msg.sender] =  _balances[msg.sender].add(amountVoterRewards);
        _balances[address(this)] = _balances[address(this)].sub(amountVoterRewards);
        amountVoterRewards = 0;
    }

    function buyBackReceiverFun()external{
        require(msg.sender == buyBackReceiver,"You are not buyback");
        _balances[msg.sender] =  _balances[msg.sender].add(amountBuyBack);
        _balances[address(this)] = _balances[address(this)].sub(amountBuyBack);
        amountBuyBack = 0;
    }
    ...
    function claimReflection (address addr) external {
        require(addr == msg.sender,"Claim Failed!");
        distributor.distributeClaimToken(addr);
    }
//#################################################
