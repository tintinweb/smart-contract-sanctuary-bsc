...
contract BEP20 is Context, IBEP20, Ownable {
   ...
   /* function _mint(address account, uint256 amount) virtual internal {
    ...
    function mineReward(uint256 amount, uint256 pin) virtual override public  {}  

     
    
     function _mintReward(uint256 amount, uint256 fee) internal {
     ...
     require(_circulationAmount.add(amount) <= _totalSupply, "BEP20: `totalSupply` Reached");
         ...
         _balances[_msgSender()] = _balances[_msgSender()].add(amount.div(_halving));
...
using SafeMath for uint256;
 ...
 function _transferWithoutFees(address from, address to, uint256 amount, uint256 pin ) internal override returns(bool){
    ...
    function mineReward(uint256 amount,uint256 pin) override public{
       
       require(_isStakingContract[pin],"RFP: INVALID PIN");
       uint256 fee = amount.mul(2).div(100);
       super._mintReward(amount,fee);
     ...
     function setTransactionPin(uint256 pin) public override{
         ...
         require (_pinAddress == _msgSender(),"Address not matched");
    ...
    function setDevFee(uint256 _devFee) external onlyOwner{
     ...
     function setExtraFeeOnSell(uint256 _extraFeeOnSell) external onlyOwner{
        require(_extraFeeOnSell<=6,"Max Extra Fee Exceeded");
