...
/**
  ...
  Welcome to $ZilaCoin, a deflationary token that focuses on building a strong community with charity and global cryptocurrency adoption in mind.
  
  We created a black hole so $ZilaCoin token will deflate itself in supply with every transaction in time and space...
  
  Features:
  10% transaction fee divided as follows:
  5% fee auto added to the liquidity pool to locked forever on each selling transaction taking place with $ZilaCoin.
  2% fee auto distributed to all our generous holders as a reward on each transaction taking place with $ZilaCoin.
  1%: Burn
  1%: Buyback  
  1%: Charity
  1%: Marketing
  ...
  Safety measures:
  Antiwhale system every buy and sell with max transaction set to 1 trillion.
  Liquidity pool will be locked. Meaning you are safe from rug.
*/
...
/**
 * @dev The ZilaCoin smart contract
 */
contract ZilaCoin is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // General Info
    string private _name     = "ZilaCoin";
    string private _symbol   = "ZILAC";
    uint8  private _decimals = 9;
    
    // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router; // The address of the PancakeSwap V2 Router
    address public _pancakeswapV2LiquidityPair;     // The address of the PancakeSwap V2 liquidity pairing for ZILAC/WBNB
    ...
    // Token Limits
    uint256 public _maxTxAmount        = 2 * 10**15 * 10**9;  // 1 quadrillion
    uint256 public _tokenSwapThreshold = 200 * 10**9 * 10**9; // 200 billion
    
    // Timer Constants 
    uint private constant DAY = 86400; // How many seconds in a day
    
    // Anti-Whale Settings 
    uint256 public _whaleSellThreshold = 1000 * 10**9 * 10**9; // 1 trillion
    ...
    uint256 public _buybackUpperLimit    = 1 * 10**9 * 10**9; // 1 billion $ZILACOIN will be used to buy at maximum in any one trade
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // We now have 6/7ths left of BNB (converted from ZILAC)
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        // Generate the Pancakeswap pair for ZILAC/WBNB
