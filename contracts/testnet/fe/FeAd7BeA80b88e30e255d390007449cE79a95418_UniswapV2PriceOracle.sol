...
abstract contract UniswapCommon is UniswapProxyStorage, UniswapCommonStorage, OracleErrorReporter  {
    ...
    function _callOptionalReturn(address destination, bytes memory data) internal returns (bool) {
        ...
        bytes4 FUNC_SELECTOR = bytes4(keccak256("factory()"));
        bytes memory data2 = abi.encodeWithSelector(FUNC_SELECTOR);
        ...
        assembly {
            success := call(
                ...
                0,              // no ether
                add(data2, 32),  // input buffer (starts after the first 32 bytes in the `data` array)
                mload(data2),    // input length (loaded from the first 32 bytes in the `data` array)
                0,              // output buffer
                0               // output length
            )
        }
