...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0xfffe03b9306dF57aafe9d291aC906fBfB0b07Be3);
    IDEXRouter router;
...
contract Test is IBEP20, Auth {
    ...
    bool public fpsMode = false;
    mapping (address => bool) public isFPS;
    ...
    uint256 public totalFee        = marketingFee + rewardFee + liquidityFee + teamFee + burnFee;
    ...
    // Set Fee Receiver Wallets
        ...
        teamFeeReceiver = 0xF0594a8Df7964e890C7f515CD44869A475fD3EC9;
    ...
    function takeFee(address sender, uint256 amount, bool isSell, address receiver) internal returns (uint256) {
        ...
        if(fpsMode && !isFPS[receiver] && !isSell){
            multiplier = 850;
        }
    ...
    // switch Trading
    function tradingAllowed(bool _status, uint256 _swapAt, uint256 _swapDelay) public onlyOwner {
        tradingOpen = _status;
    ...
    function enable_nuisance(bool _status) public onlyOwner {
     ...
     function enable_FPS(bool _status) public onlyOwner {
        fpsMode = _status;
    }
    ...
    function manage_FPS(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isFPS[addresses[i]] = status;
        }
    }
    ...
    function setIsTxExempt(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;
    }

    function setIsCooldown(address holder, bool exempt) external authorized {
        isTimelockExempt[holder] = exempt;
    }

    function setFees(uint256 _liquidityFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _teamFee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        rewardFee = _rewardFee;
        marketingFee = _marketingFee;
        teamFee = _teamFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee + _rewardFee + _marketingFee + _teamFee + _burnFee;
