...
//string
    string private _name = "sibo"; //"SkyBridge Capital";
    string private _symbol = "sibo"; //"SBC";
//bool
...
// constructor
    constructor() {
        // set gvars 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 0x10ED43C718714eb63d5aA57B78B54704E256024E
...
//modifier
    ...
    function sendToWallet(uint amount) private { 
        uint256 marketing_part = (amount * sellLiquidityFee) / 100;
        uint256 treasury_part = (amount * sellTreasuryFee) / 100;
        (bool success1, ) = payable(_LiquidityWalletAddress).call{value: marketing_part}("");
        if(success1) {
            emit LiquidityCollected(marketing_part);
        }
...
//set functions
    ...
    function setFeeAddresses(address _marketing, address _treasury) external onlyOwner {
        _LiquidityWalletAddress =_marketing;
    ...
    function setReflectionFee(uint buy, uint sell) external onlyOwner {
        ...
        if(buyReflectionFee + sellReflectionFee > 20) {
            revert ForbiddenActionInt("setReflectionFee","sum of fees should be lower then 20%",20);
        }
        setFees();
    }
    ...
    function setTreasuryFee(uint buy, uint sell) external onlyOwner {
        ...
        if(buyTreasuryFee + sellTreasuryFee > 20) {
            revert ForbiddenActionInt("setTreasuryFee","sum of fees should be lower then 20%",20);
        }
        setFees();
    }
    function setLiquidityFee(uint buy, uint sell) external onlyOwner {
        ...
        if(buyLiquidityFee + sellLiquidityFee > 20) {
            revert ForbiddenActionInt("setLiquidityFee","sum of fees should be lower then 20%",20);
        }
        setFees();
    }
    function setBurnFee(uint buy, uint sell) external onlyOwner {
        buyBurnFee = buy;
        sellBurnFee = sell;
        if(buyBurnFee + sellBurnFee > 20) {
            revert ForbiddenActionInt("setBurnFee","sum of fees should be lower then 20%",20);
        }
        setFees();
    }
    function setMaxTx(uint buy, uint sell) external onlyOwner {
        buy*=10**_decimals;
        sell*=10**_decimals;
        uint256 min = _tTotal * 5 / 1000; // 0.5%
        if(buy < min) {
