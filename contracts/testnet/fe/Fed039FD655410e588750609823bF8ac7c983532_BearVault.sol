...
contract BearVault is Ownable, Pausable, ReentrancyGuard {
    ...
    function withdraw(uint256 _amount) public whenNotPaused nonReentrant {
                ...
                if (pending > currentRewardBalance) {
                    if (_amount>0){
                        if (user.lastDeposit.add(feesDuration) < block.timestamp){
                            uint256 amountFees = currentRewardBalance.mul(withdrawFees).div(100);
                            currentRewardBalance.sub(amountFees);
                            safeTransferReward(feesWallet, amountFees);
                        }
                    }
                    safeTransferReward(
                        address(msg.sender),
                    ...
                    if (_amount>0){
                        if (user.lastDeposit.add(feesDuration) < block.timestamp){
                            uint256 amountFees = pending.mul(withdrawFees).div(100);
                            pending.sub(amountFees);
                            safeTransferReward(feesWallet, amountFees);
                        }
                    }
                    safeTransferReward(address(msg.sender), pending);
        ...
        if (_amount > 0) {
            ...
            myPool.lpToken.approve(address(msg.sender), _amount);
