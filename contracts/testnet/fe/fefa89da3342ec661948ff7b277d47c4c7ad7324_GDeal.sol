...
contract GDeal is ERC20, ERC20Detailed {
    ...
    constructor (address payable _owner)  ERC20Detailed("Great Deal Coin", "GDS", 8) {
        ...
        claim_periods.push(1 minutes);
        claim_periods.push(2 minutes);
        claim_periods.push(3 minutes);
        claim_periods.push(4 minutes);
    }
    ...
    function getInvestments(address _addr, uint _level) public view returns(uint256 deposit_amount, uint time, uint claim_period, uint256 usd_value, uint256 price) { 
        return (users[_addr].investment[_level].deposit_amount, users[_addr].investment[_level].time, users[_addr].investment[_level].claim_period, users[_addr].investment[_level].usd_value, users[_addr].investment[_level].price);
    }
