...
contract DreamHorseController is Ownable {
    ...
    mapping(uint256 => uint256) private tokenIdToActivatedTime;//NFT激活英雄时间

    uint256[5] public heroLifeCycle = [60 days,90 days,150 days,240 days,360 days];//英雄生命周期
    ...
    constructor() {
        ...
        monsters[3] = Monster(3,56,400,23,8,5,2);
    ...
    function buyHeroCard(uint256 tokenId) public {
        ...
        require(tokenIdToActivatedTime[tokenId] + heroLifeCycle[tokenIdToHeros[tokenId].id] > block.timestamp, "The hero life cycle is over");
    ...
    function userDrawMonsterProfit() public {
        ...
        for(uint256 i = 0; i < userTokenIds.length; i++){
            ...
            if(tokenId != 0){
                for(uint256 j = 0; j < tokenIdToMonsterProfits[tokenId].length; j++){
    ...
    //                 intervalDays = (drawTime - profit.fightTime).div(24 hours);
    ...
    function drawHeroLastMonsterProfit(uint256 tokenId) public {
        ...
        // AMT.transfer(msg.sender, monsterProfit * 10 ** 14);
    ...
    function getSellsOrSolds(uint256 types, uint256 level) public view returns(Sell[] memory sells) {
        ...
        if(types == 1 && totalSell > 0 && level == 0){

            sells0 = new Sell[](uint256(totalSell));
        ...
        if(types == 1 && totalSell > 0 && level != 0){
            ...
            for(uint256 i = 0; i < DHN.counter(); i++){
                ...
                if(tokenIdToHeros[tokenId].isOnsell && DHN.NFTIDToTypes(tokenId) == level){
        ...
        if(types == 2 && totalSold > 0 && level == 0){

            sells0 = new Sell[](uint256(totalSold));
        ...
        if(types == 2 && totalSold > 0 && level != 0){
            ...
            for(uint256 i = 0; i < DHN.counter(); i++){
                ...
                if(tokenIdToSells[tokenId].sold && DHN.NFTIDToTypes(tokenId) == level){
    ...
    function getUserPublishOrBuy(address user, uint256 types, uint256 level) public view returns(Sell[] memory sells) {
        ...
        if(types == 1 && level != 0){
            for(uint256 i = 0; i < userTokenIds.length; i++){
                tokenId = userTokenIds[i];
                if(tokenIdToHeros[tokenId].isOnsell && tokenIdToHeros[tokenId].id == level){
                    tokenIds[counter] = tokenId;
                    counter++;
                }
            }
        ...
        if(types == 2 && level != 0){
            for(uint256 i = 0; i < userTokenIds.length; i++){
                tokenId = userTokenIds[i];
                if(!tokenIdToHeros[tokenId].isOnsell && heroSource[tokenId] == 2 && tokenIdToHeros[tokenId].id == level){
                    tokenIds[counter] = tokenId;
                    counter++;
                }
            }
