...
contract INFLIV is Initializable, ERC20Upgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
    ...
    function initialize() initializer public {
		...
		bountyBonusesPool.startTime = block.timestamp + 90 days;
		...
		listingsLiquidityPool.startTime = block.timestamp + 90 days;
	...
	function setGovernanceFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
	    require(buy  <= 2500 , "Max fee limit reached for 'BUY'");
		require(sell <= 2500 , "Max fee limit reached for 'SELL'");
		require(p2p  <= 2500 , "Max fee limit reached for 'P2P'");
		
		GovernanceFee[0] = buy;
		GovernanceFee[1] = sell;
		GovernanceFee[2] = p2p;
	    emit GovernanceFeeUpdated(buy, sell, p2p);
	}
	...
	function setStakingAddress(address newAddress) public onlyOwner {
	...
	function setFarmAddress(address newAddress) public onlyOwner {
	    require(newAddress != address(0), "INFLIV: Zero address");
	...
	function setInsuranceAddress(address newAddress) public onlyOwner {
	    require(newAddress != address(0), "INFLIV: Zero address");
	...
	function setBountyAddress(address newAddress) public onlyOwner {
	    require(newAddress != address(0), "INFLIV: Zero address");
	...
	function withdrawalTokenFromPartnersInvestorsPool(address to, uint256 amount) public onlyOwner {
	...
	function withdrawalTokenFromBountyBonusesPool(address to, uint256 amount) public {
	...
	function withdrawalTokenFromTreasuryPool(address to, uint256 amount) public onlyOwner {
	...
	function withdrawalTokenFromInsurancePool(address to, uint256 amount) public onlyOwner {
	   uint256 available = availableInsurancePoolTokens();
	   
	   require(to != address(0), "INFLIV: Zero address");
	   require(available >= amount && amount > 0, "INFLIV: Amount is not available for mint");
	   
	   insurancePool.supplyMinted += amount;
	   _mint(to, amount);
    }
	...
	function withdrawalTokenFromCharityPool(address to, uint256 amount) public onlyOwner {
	   uint256 available = availableCharityPoolTokens();
	   
	   require(to != address(0), "INFLIV: Zero address");
	   require(available >= amount && amount > 0, "INFLIV: Amount is not available for mint or amount is zero");
	   
	   charityPool.supplyMinted += amount;
	   _mint(to, amount);
    }
	...
	function withdrawalTokenFromReservePool(address to, uint256 amount) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20Upgradeable){      
		...
		if(block.timestamp >= listingsLiquidityPool.lastClaimTime + listingsLiquidityPool.claimInterval && swapEnable && !swapping && automatedMarketMakerPairs[recipient]) {
		    swapping = true;
