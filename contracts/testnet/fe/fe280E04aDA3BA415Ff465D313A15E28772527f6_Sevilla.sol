...
contract Sevilla is Context, IERC20, IERC20Metadata {
    ...
    function transferFrom(
        ...
        require(currentAllowance >= amount, "SEV: transfer amount exceeds allowance");
    unchecked {
    ...
    function decreaseAllowance(address spender, uint subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "SEV: decreased allowance below zero");
    unchecked {
    ...
    function _transfer(
        ...
        require(sender != address(0), "SEV: transfer from the zero address");
        require(recipient != address(0), "SEV: transfer to the zero address");
        uint senderBalance = _balances[sender];
        require(senderBalance >= amount, "SEV: transfer amount exceeds balance");
        uint _amount = amount;

        if (!Excludes[sender] && !Excludes[recipient] && _totalSupply > 1000000000 * 1e18) {
            require(_switches == true, "SEV: buy from dex is closed");
            if (LpRatio > 0) {
    ...
    function _burn(address account, uint amount) internal virtual {
        require(account != address(0), "SEV: burn from the zero address");
        uint accountBalance = _balances[account];
        require(accountBalance >= amount, "SEV: burn amount exceeds balance");
    unchecked {
    ...
    function setTradeStatus(bool canBuy) public onlyAdmin {
        _switches = canBuy;
    }
