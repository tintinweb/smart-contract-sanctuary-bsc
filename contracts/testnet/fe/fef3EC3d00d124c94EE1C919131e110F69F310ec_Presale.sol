...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    function presaleStarted() public view returns(bool) {
        if(startTimestamp < block.timestamp && endTimestamp > block.timestamp) return true;
        else if (endTimestamp < block.timestamp) return false;
        else return false;
    }
 
 
    //Start Pre-Sale
    function setTimeframe(uint256 _startTimestamp, uint256 _endTimestamp) external onlyOwner {
    ...
    function buyTokens() public nonReentrant payable {
        require(presaleStarted(), "Presale not active");
        require(!whitelistEnabled || isWhitelisted[msg.sender], "Only whitelisted users");
    ...
    function setBulkWhitelistUsers(address[] calldata users, bool status) external onlyOwner{
