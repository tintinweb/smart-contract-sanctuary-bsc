...
//================================================ BLACKLIST
    ...
    // function setBuyTaxes(uint256 bracketOne, uint256 bracketTwo) external onlyOwner {
    //     require(bracketTwo < bracketOne "Must be descending.");
    ...
    function takeTaxes(address from, address to, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        if (buy) {
            ...
            if (memVals.taxLocked) {
                ...
                uint256 delta = block.timestamp - memVals.firstBuy;
                if (delta < 10 days) {
                    memVals.lockedBuyFee = 1000;
                    memVals.lockedSellFee = 2200;
                    memVals.taxLocked = true;
                } else if (delta < 20 days) {
                    memVals.lockedBuyFee = 1000;
                    memVals.lockedSellFee = 2000;
                    memVals.taxLocked = true;
                } else if (delta < 30 days) {
                    memVals.lockedBuyFee = 1000;
                    memVals.lockedSellFee = 1500;
                    memVals.taxLocked = true;
                } else if (delta < 40 days) {
                    memVals.lockedBuyFee = 500;
                    memVals.lockedSellFee = 1000;
                    memVals.taxLocked = true;
                } else if (delta < 60 days) {
                    memVals.lockedBuyFee = 500;
                    memVals.lockedSellFee = 500;
                    memVals.taxLocked = true;
                } else {
...
//                                                    REMOVE BEFORE MAINNET DEPLOYMENT
    ...
    function __setUserFirstBuy(address account, uint256 stamp) external onlyOwner {
        userValues[account].firstBuy = stamp;
    }

    function __unlockTaxes(address account) external onlyOwner {
        userValues[account].taxLocked = false;
    }
}