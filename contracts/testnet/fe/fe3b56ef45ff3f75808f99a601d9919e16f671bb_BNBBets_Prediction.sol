 ...
 */
contract BNBBets_Prediction is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public devFeeAmount; // treasury amount that was not claimed
    ...
    uint256 public houseBetAmount = 0.02 ether; // pot for the house bet
    bool public enableHouseBet = false;
    ...
    event TreasuryClaim(address indexed user, uint256 amount);
    ...
    function houseBetClaim(uint256 epoch) public nonReentrant notContract {
        uint256 reward; // Initializes reward
            ...
            if (rounds[epoch].oracleCalled) {
                require(claimable(epoch, address(this)), "Not eligible for claim");
    ...
    function executeRound() external whenNotPaused onlyOperator {
     ...
     function HouseBet(uint256 epoch) external onlyAdminOrOperator whenNotPaused notContract 
    {
        require(epoch == currentEpoch, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        
        if(houseBetPot >= houseBetAmount && enableHouseBet){
        _safeHouseBet();
        }
    ...
    function claimTreasury() external nonReentrant onlyAdmin {
        ...
        emit TreasuryClaim(adminAddress, currentTreasuryAmount);
    }

    /**
     * @notice Claim all rewards in dev fee pot
     * @dev Callable by admin
     */
    function claimDevFee() external nonReentrant onlyDev {
        uint256 currentDevFeeAmount = devFeeAmount;
        devFeeAmount = 0;
        _safeTransferBNB(devAddress, currentDevFeeAmount);

        emit TreasuryClaim(devAddress, currentDevFeeAmount);
    }
     ...
     * @dev Callable by admin
     */
    function claimHousePot() external nonReentrant onlyAdmin {
        uint256 currentHousePot = houseBetPot;
        houseBetPot = 0;
     ...
     * @dev Callable by admin
     */
    function seHouseBetAmount(uint256 _betAmount) external whenPaused onlyAdmin {
     ...
     * @dev Callable by admin
     */
    function seHouseBetEnable(bool status) external whenPaused onlyAdmin {
