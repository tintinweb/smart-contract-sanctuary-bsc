...
contract PreSaleTVT is Ownable {
    ...
    mapping (address => refWallet) public referWallets; //  Кошельки купивших
    ...
    uint256 public minToStartRefer = 200 * 1e18; // Всего токенов продано
    ...
    function buyToken(address _token, address _wallet, uint256 _amountIn, uint256 _amountOut) public {
        ...
        //Добавляем как реффереру купленые монеты
        referWallets[msg.sender].totalAmount += _amountOut;
        // Если у реферрера собарлось достаточно моент - открываем ему реф ссылку
    ...
    // Получить данные по своей реефеералке
