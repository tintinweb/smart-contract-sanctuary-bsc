        ...
        _beforeTokenTransfer(from_, amount_);
         ...
         if(_isTaxEnabled){
            if(!_isExcludedFromFees[from_]&&!_isExcludedFromFees[to_]){
                ...
                uint256 taxAmount= amount_.mul(80).div(1000);
                uint256 transferAmount= amount_.sub(taxAmount);
                ...
                //Transfer Tax
                _balances[communityLotteryVault] += taxAmount.mul(_COMMUNITY_LOTTERY).div(1000);
                emit Transfer(from_, communityLotteryVault, taxAmount.mul(_COMMUNITY_LOTTERY).div(1000));

                _balances[lpAcquisitionVault] += taxAmount.mul(_LP_ACQUISITION).div(1000);
                emit Transfer(from_, lpAcquisitionVault, taxAmount.mul(_LP_ACQUISITION).div(1000));

                _balances[companyVault] += taxAmount.mul(_COMPANY).div(1000);
                emit Transfer(from_, companyVault, taxAmount.mul(_COMPANY).div(1000));

                _balances[growthFundVault] += taxAmount.mul(_GROWTH_FUND).div(1000);
                emit Transfer(from_, growthFundVault, taxAmount.mul(_GROWTH_FUND).div(1000));
