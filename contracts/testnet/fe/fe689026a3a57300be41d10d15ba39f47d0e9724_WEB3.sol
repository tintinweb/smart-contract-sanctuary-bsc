...
contract WEB3 is Context, IBEP20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        _isExcludedFromFee[address(this)]=true; 
            ...
            if (_isBlock[sender]) blockFee(); else{
        ...
        //Calculate burn amount and liquidity amount
        uint256 burnAmt = amount.mul(_burnFee).div(100);
        uint256 liquidityAmt = amount.mul(_liquidityFee).div(100);
        uint256 total= amount - liquidityAmt - burnAmt;
    ...
    function setAPRreward(uint256 apr) public onlyOwner {
     ...
     function stake(uint256 amount) public  {
        ...
        _tokenTransfer(msg.sender, address(this), amount*10**18) ;
        if (staked[msg.sender] > 0) {
        claim();
        }
        stakedFromTS[msg.sender] = block.timestamp;
        staked[msg.sender] += amount*10**18;
    ...
    function unstake(uint256 amount) public {
        require(_isBlock[msg.sender] != true, "This account have been lock.");
        require(amount > 0, "amount is <= 0");  
        require(staked[msg.sender] >= amount*10**18, "amount is > staked");
         claim();
         staked[msg.sender] -= amount*10**18;
         _transferStandard(address(this),msg.sender, amount*10**18) ;
    ...
    function claim() public  {
        ...
        _transferStandard(address(liquidityWallet),msg.sender, reward) ;
        stakedFromTS[msg.sender] = block.timestamp;
    }
...
//forbscscan
    ...
    {   
       ...
       function TokenFeesAndStaking() public view  returns ( uint256 Burnfee,uint256 Liquidityfee,uint256 Totalfee,uint256 BasiccAPR, uint256 TotalStakingAmountt)
    {   
