...
contract CeresBank is CeresBase, ICeresBank, Ownable {
    ...
    constructor (address _owner, address _factory, address _asc, address _crs, uint256 _updateCooldown) Ownable(_owner) CeresBase(_factory){
    ...
    function mintFromStaking(address collateral, uint256 collateralAmount) external override onlyStakings
        ...
        require(decreaseRatioStep < maxRatioStep, "CeresBank: ratioStep must be less than maxRatioStep");
        require(decreaseRatioStep > minRatioStep, "CeresBank: ratioStep must be greater than minRatioStep");
    ...
    function redeem(uint ascAmount) external override onlyOwner {
        ...
        require(ascPriceMineable(), "CeresBank: ASC price now is not redeemable!");
