...
interface WithdrawClaim {
    function emergencyWithdraw(uint256 _amount) external;
}
contract ClaimV4 is AccessControl {
    using SafeMath for uint;
    using SafeERC20 for IERC20;
    bytes32 public constant CREATOR_ADMIN = keccak256("CREATOR_ADMIN");
    IERC20 public tokenHeroes;
    ClaimV2 public claimV2;
    WithdrawClaim public withdrawClaim;
    ...
    constructor(address minter, address _tokenHeroes, address _burnAddress, address _claimV2){
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(CREATOR_ADMIN, minter);
        tokenHeroes = IERC20(_tokenHeroes);
        claimV2 = ClaimV2(_claimV2);
        startTime = claimV2.startTime();
        vestingPeriod = claimV2.vestingPeriod();
        // burnToken = ERC20Burnable(_tokenHeroes);
        period = claimV2.period();
        burnAddress = _burnAddress;
        claim =  _claimV2;
        withdrawClaim = WithdrawClaim(_claimV2);
    }
    mapping(address => uint256) public totalAmount;
    mapping(address => uint256) public withdrawnAmount;
    event ClaimToken(
    ...
    function addMemberClaimV2(address[] memory member) external {
        require(hasRole(CREATOR_ADMIN, address(msg.sender)), "Caller is not a owner");
        uint256 deposit = 0;
        for(uint256 i = 0 ; i < member.length; i++){
            totalAmount[address(member[i])] = totalAmount[address(member[i])].add(claimV2.totalAmount(address(member[i])));
            withdrawnAmount[address(member[i])] = withdrawnAmount[address(member[i])].add(claimV2.withdrawnAmount(address(member[i])));
            totalWithdraw = totalWithdraw.add(claimV2.withdrawnAmount(address(member[i])));
            totalPool = totalPool.add(claimV2.totalAmount(address(member[i])));
            deposit = deposit.add(claimV2.totalAmount(address(member[i]))).sub(claimV2.withdrawnAmount(address(member[i])));
        }
        uint256 balancePool = tokenHeroes.balanceOf(address(claim));
        require(balancePool >= deposit, "Check Balance Pool ClaimV2");
        withdrawClaim.emergencyWithdraw(deposit);
        paused = false;  
    }
    function decomposeAddress(address decomposer, address[] memory member, uint256[] memory amount) external {
        require(hasRole(CREATOR_ADMIN, address(msg.sender)), "Caller is not a owner");
        uint256 deposit = 0;
        for(uint256 i = 0 ; i < member.length; i++){
            totalAmount[address(member[i])] = totalAmount[address(member[i])].add(amount[i]);
            deposit = deposit.add(amount[i]);
        }
        uint256 amountDecomposer = totalAmount[address(decomposer)].sub(withdrawnAmount[address(decomposer)]);
        require(amountDecomposer >= deposit, "Insufficient balance");
        withdrawnAmount[address(decomposer)] =  withdrawnAmount[address(decomposer)].add(deposit);
    }
