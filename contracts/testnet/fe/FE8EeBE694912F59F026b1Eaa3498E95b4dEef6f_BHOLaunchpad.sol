...
contract BHOLaunchpad is Ownable, Pausable, Whitelist, SharedConstants {
    ...
    function collectTokenPayment(address _recipient) external onlyRole("manager") {
        // Get 98% BUSD
     ...
     */
    function updatePercentCanBuyInFCFS(uint256 _percent) external onlyRole("manager") {
     ...
     */
    function updateEmergencyWithdrawFee(uint256 _percent) external onlyRole("manager") {
        require(_percent > 0, "Percent be greater than 0");
        require(_percent <= 100 * DECIMALS_PERCENT, "Percent be less than 100");
     ...
     */
    function updatePresaleType(PresaleType _presaleType) public onlyRole("manager") {
        launchpadData.presaleType = _presaleType;
    }

    /**
     * @dev update base allocation
     */
    function updateBaseAllocation(uint256 _baseAllocation) public onlyRole("manager") {
