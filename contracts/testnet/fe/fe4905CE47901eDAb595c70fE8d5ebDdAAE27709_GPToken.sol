...
//Mapping section for better tracking.
    ...
    mapping (address => bool) public _isrevisioned;
...
//Token Definition.
    string private constant _name = "GPToken";
...
//Readable Functions.
    ...
    //adding multiple addresses to the revision - Used to manually block known bots and scammers
    function addTorevision(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _isrevisioned[addresses[i]] = true;
      }
    }


    //Remove from revision 
        function removeFromrevision(address account) external onlyOwner {
            _isrevisioned[account] = false;
        }
...
//Transfer function, validate correct wallet structure, take fees, and other custom taxes are done during the transfer.
    function _transfer(
        ...
        uint256 amount
          ...
          //revisioned addreses can not buy! If you have ever used a bot, or scammed anybody, then you're wallet address will probably be revisioned
        require(!_isrevisioned[from] && !_isrevisioned[to], "This address is revisioned");
