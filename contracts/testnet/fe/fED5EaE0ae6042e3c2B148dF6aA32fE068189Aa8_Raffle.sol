...
contract Raffle is Context, AccessControlEnumerable, ERC721Enumerable {
    ...
    function updatePrizeContract(ERC721 prizeNftContractAddress) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Must have admin role");
    ...
    function revealResult(bytes32 reveal, uint256 blockNumber) public {
        ...
        uint256 winnerTokenId = _roll(keccak256(abi.encodePacked(reveal, rollHash, blockhash(blockNumber))), _tokenIdTracker.current());
        uint256 prizeTokenIndex = _roll(keccak256(abi.encodePacked(reveal, rollHash, blockhash(blockNumber))), prizeTokenIds.length());

        userWins[ownerOf(winnerTokenId)].add(winners.length);
        winners.push(Winner({
            winnerAddress: ownerOf(winnerTokenId),
            tokenId: winnerTokenId,
            prizeTokenId: prizeTokenIds.at(prizeTokenIndex),
            claimed: false
        }));
        emit WinnerPicked(winnerTokenId, prizeTokenIds.at(prizeTokenIndex), ownerOf(winnerTokenId));
