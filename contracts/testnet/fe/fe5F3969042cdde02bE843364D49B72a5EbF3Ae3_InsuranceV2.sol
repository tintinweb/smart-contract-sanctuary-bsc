    ...
    OwnableUpgradeable
{
    ...
    function betToContract(
        ...
        if (isWrappedNativeToken) {
            IWrappedNativeToken betToken = IWrappedNativeToken(
                payable(tokenAddress_)
            );
            betToken.transferFrom(userAddress_, address(this), amount_);
        } else {
    ...
    function transfer(
        ...
        if (isWrappedNativeToken) {
            ...
            betToken.transfer(receiver_, amount_);
        } else {
            IERC20Upgradeable betToken = IERC20Upgradeable(tokenAddress_);
    ...
    function withdraw(
        ...
        if (isWrappedNativeToken) {
            ...
            betToken.transfer(msg.sender, amount_);
        } else {
            IERC20Upgradeable betToken = IERC20Upgradeable(tokenAddress_);
    ...
    function withdrawNativeToken(
        ...
        require(
            ...
            "Error: your token is not wrapped native token!"
        );
    ...
    TokenWrapper
{
    ...
    mapping(address => TokenCharacteristic) whitelistTokenCharacteristic;
    ...
    function createGame(
        ...
        require(
            gameMaxBetAmount_ >= whitelistTokenCharacteristic[betToken_].min,
    ...
    function claimNative(uint256 gameId_) public nonReentrant {
        (
            uint256 claimAmount,
            uint256 totalDAOFee,
            uint256 totalRevenueFee
