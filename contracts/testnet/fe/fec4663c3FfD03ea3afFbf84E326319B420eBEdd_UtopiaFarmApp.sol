...
contract UtopiaFarmApp is ModuleBase, Lockable, IERC721Receiver, SafeMath {
    ...
    function sowSeed(uint256 amount, uint256 farmLandTokenId, address parent) external lock {
        uint256 needMUTAmount = _cumulateMUTAmount(0);
        require(amount >= needMUTAmount, "input amount error");
        ...
        require(resRange && usdtAmount >= minAmount && usdtAmount <= maxAmount, "sow amount overflow");
    ...
    function onERC721Received(
        ...
        emit NFTLandReceived(operator, from, tokenId, data);
        return 0;
    }
}