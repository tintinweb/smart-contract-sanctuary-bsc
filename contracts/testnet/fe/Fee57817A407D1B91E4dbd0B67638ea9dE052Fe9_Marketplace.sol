...
contract Marketplace is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    event BNBFeeChanged(address indexed account, uint256 newFee, uint256 oldFee);
    event GWGFeeChanged(address indexed account, uint256 newFee, uint256 oldFee);
    event TokenOnSale(
    ...
    /**
     * @notice update platform fee in BNB
     * @dev Caller must be contract owner
     * @param fee platform fee in percentage (using 2 decimals: 10000 = 100)
     */
    function updateBNBFee(uint256 fee) public onlyOwner {
        uint256 oldFee = platformFeeInBNB;
        platformFeeInBNB = fee;
        emit BNBFeeChanged(msg.sender, fee, oldFee);
    }

    /**
     * @notice update platform fee in GWG
     * @dev Caller must be contract owner
     * @param fee platform fee in percentage (using 2 decimals: 10000 = 100)
     */
    function updateGWGFee(uint256 fee) public onlyOwner {
        uint256 oldFee = platformFeeInGWG;
        platformFeeInGWG = fee;
        emit BNBFeeChanged(msg.sender, fee, oldFee);
    }
