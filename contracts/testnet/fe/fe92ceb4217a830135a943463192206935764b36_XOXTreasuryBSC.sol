...
contract XOXTreasuryBSC is Initializable {
    ...
    uint256 private constant TOTAL_BLOCK_IN_YEAR = 10512000;
    ...
    /* ========== CONSTRUCTOR ========== */
    function initialize(ERC20 busd_) public initializer {
        busd = busd_;
        admin = msg.sender;
        apy = 10;
        levelReward = [
    ...
    function swapBUSDtoXOX(
        ...
        if (block.number > buyer.lastBLockUpdated) {
            ...
            buyer.lastBLockUpdated = block.number;
            emit SwapBUSDToXOX(from, ref, amount);
        }
    }

    function claimReferralByLevel(uint256 level) external {
        require(level < 10, "XOX Treasry: not fount point");
        ...
        // claim
        safeBusdTransfer(msg.sender, rewardByLevel[level]);
        // update
    ...
    function claimReferralAll() external {
        ...
        // claim
        safeBusdTransfer(msg.sender, rewardPending);
        // log event
        emit ClaimReferral(msg.sender, rewardPending);
    }
    ...
    function emergencyWithdraw(address _to, uint256 _amount)
        ...
        onlyAdmin
    {
        safeBusdTransfer(_to, _amount);
    }

    // Safe busd transfer function, just in case if rounding error causes farm to not have enough BUSD.
    function safeBusdTransfer(address _to, uint256 _amount) internal {
        uint256 busdBal = busd.balanceOf(address(this));
        require(_amount <= busdBal, "XOX Treasury: not enough reward to claim");
