...
// File: contracts_ETH/main/StableToken.sol
...
// File: contracts_ETH/Parity/TokenParity.sol
...
event MintDeposit(address indexed _user, uint256 _id, uint256 _amount, 
            uint256 _choice, uint256 _weightAlpha ,uint256 _weightBeta , 
...
event BurnDeposit(address indexed _user, uint256 _id);
    ...
    // Setter functions
    function setUserAmountAlpha(address _user, uint256 _amount) public onlyProxy {
        userAmount[_user].alpha += _amount;

    }     

    function setUserAmountBeta(address _user, uint256 _amount) public onlyProxy {
        userAmount[_user].beta += _amount;

    }     

    function setUserAmountGamma(address _user, uint256 _amount) public onlyProxy {
    ...
    function mint(address _account, uint256 _tokenId, uint256 _amount, 
       ...
       index= index + 1;
       updateDepositData( _account, _amount, _choice, _weightAlpha , _weightBeta, 
       _weightGamma );
       emit MintDeposit( _account,  _tokenId, _amount, 
            _choice,  _weightAlpha, _weightBeta,  _weightGamma );
    }
    ...
    function updateDepositData(address _account, uint256 _amount, uint256 _choice,
        ...
        uint256 _alpha = (_amount * _weightAlpha) / 10000;
        uint256 _beta = (_amount * _weightBeta) / 10000;
        uint256 _gamma = (_amount * _weightGamma) / 10000;
     ...
     function ChangeChoice(address _account, uint256 _weightAlpha ,uint256 _weightBeta , uint256 _weightGamma, uint256 _choice, 
               uint256[3] memory _price ) public onlyProxy { 
        ...
        (userWeights[_account].gamma !=_weightGamma)){
        uint256 _valueTotal = userAmount[msg.sender].alpha * _price[0] + 
                userAmount[msg.sender].beta * _price[1] +
                userAmount[msg.sender].gamma * _price[2];
        uint256 deltaAmount;
        uint256 _newAmount;

        Amount memory _userAmount = userAmount[_account];
        //
        _newAmount = _valueTotal *_weightAlpha /10000;
      
        if (_newAmount < _userAmount.alpha * _price[0]){
            deltaAmount = (_userAmount.alpha * _price[0] - _newAmount) / 10e6;
            usersDeposit[_account].alpha -= Math.min(usersDeposit[_account].alpha,
            deltaAmount);
            amountDeposit.alpha -= Math.min(usersDeposit[_account].alpha,
            deltaAmount);
            deltaAmount -= Math.min(usersDeposit[_account].alpha,
            deltaAmount);
            deltaAmount = (deltaAmount/_price[0]) *10e6;
            usersWithdraw[_account].alpha += deltaAmount;
            amountWithdraw.alpha += deltaAmount;
        }

        else{
               deltaAmount = (_newAmount - _userAmount.alpha * _price[0]) / 10e6;
               usersDeposit[_account].alpha += deltaAmount;
               amountDeposit.alpha+= deltaAmount;
         }
        
        _newAmount = _valueTotal *_weightBeta /10000;
      
        if (_newAmount < _userAmount.beta * _price[1]){
            deltaAmount = (_userAmount.beta * _price[1] - _newAmount) / 10e6;
            usersDeposit[_account].beta -= Math.min(usersDeposit[_account].beta,
            deltaAmount);
            amountDeposit.beta -= Math.min(usersDeposit[_account].beta,
            deltaAmount);
            deltaAmount -= Math.min(usersDeposit[_account].beta,
            deltaAmount);
            deltaAmount = (deltaAmount/_price[1]) *10e6;
            usersWithdraw[_account].beta += deltaAmount;
            amountWithdraw.beta += deltaAmount;
        }

        else{
               deltaAmount = (_newAmount - _userAmount.beta * _price[1]) / 10e6;
               usersDeposit[_account].beta += deltaAmount;
               amountDeposit.beta+= deltaAmount;
         }       
        _newAmount = _valueTotal *_weightGamma /10000;
      
        if (_newAmount < _userAmount.gamma * _price[2]){
            deltaAmount = (_userAmount.gamma * _price[2] - _newAmount) / 10e6;
            usersDeposit[_account].gamma -= Math.min(usersDeposit[_account].gamma,
            deltaAmount);
            amountDeposit.gamma -= Math.min(usersDeposit[_account].gamma,
            deltaAmount);
            deltaAmount -= Math.min(usersDeposit[_account].gamma,
            deltaAmount);
            deltaAmount = (deltaAmount/_price[2]) *10e6;
            usersWithdraw[_account].gamma += deltaAmount;
            amountWithdraw.gamma += deltaAmount;
        }

        else{
               deltaAmount = (_newAmount - _userAmount.gamma * _price[2]) / 10e6;
    ...
    function updateWithdrawtData(address _account, uint256 _tokenId, uint256 _taux) public onlyProxy {

        uint256 _alpha = ( userAmount[msg.sender].alpha  * _taux)/10000;
        uint256 _beta = ( userAmount[msg.sender].beta  * _taux)/10000;
        uint256 _gamma = ( userAmount[msg.sender].gamma  * _taux)/10000;
...
// File: contracts_ETH/Parity/InvestementParity.sol
...
address _adminAlpha,address _adminBeta,address _adminGamma, address _addressToken, address  _addressStableToken ) {
     require(
    ...
    require(
      ...
      require(
        ...
        require(
            ...
            "Formation.Fi: admin alpha is the zero address"
        );
        require(
            _adminBeta != address(0),
            "Formation.Fi: admin beta is the zero address"
        );
        require(
            _adminGamma != address(0),
            "Formation.Fi: admin gamma is the zero address"
        );
        require(
    ...
    alpha = Investment(_addressAlpha);
    beta = Investment(_addressBeta);
    gamma = Investment(_addressGamma);
...
function depositRequest( address _user, uint256 _amount, uint256 _choice, uint256 _weightAlpha, uint256 _weightBeta, uint256 _weightGamma) external whenNotPaused {
        ...
        require((_weightAlpha + _weightBeta + _weightGamma) == 10000 , "Formation.Fi: sum of weights excedees 100");
...
function withdrawRequest(uint256 _taux) external whenNotPaused {
        require ( (_taux > 0) && ( _taux<=10000) , 
        "Formation Fi: taux is zero or more then 100");
        require (token.balanceOf(msg.sender)>0 , 
        "Formation Fi: token Parity doesn't exists");
        uint256 _tokenId = token.getTokenId(msg.sender);
        token.updateWithdrawtData(msg.sender, _tokenId, _taux);
        emit WithdrawRequest(msg.sender,  _taux);

     }    
        
    function changePreference(uint256 _choice, uint256 _weightAlpha, uint256 _weightBeta, 
            uint256 _weightGamma) external whenNotPaused {
       
        require((_weightAlpha + _weightBeta + _weightGamma)==10000 , "Formation.Fi:  sum of preferences excedees 100");

        require( _choice <=2, "Formation.Fi: user choice is not correct");

    
        uint256[3] memory _price = [adminAlpha.tokenPrice(), adminBeta.tokenPrice(), 
                                   adminGamma.tokenPrice()] ; 

        token.ChangeChoice(msg.sender, _weightAlpha , _weightBeta , _weightGamma,  _choice, _price);
    ...
    function distributeAlpha ( address [] memory _users) external whenNotPaused{
     ...
     uint256 _alphaTotal = alpha.validatedDepositParityTokenAmount();
     uint256 _alpha;

     uint256 alphaDeposit; 
     uint256 alphaWithdraw;

     uint256 _stableTotalAlpha = alpha.validatedWithdrawalParityStableAmount();  
     ...
     for (uint256 i = 0; i < _users.length ; i++) {
       ...
       _stableAlpha = ( alphaWithdraw * _stableTotalAlpha) / _alphaAmountWithdrawTotal;
        token.setUserAmountAlpha(_user, _alpha);
...
function distributeBeta ( address [] memory _users) external whenNotPaused{
     ...
     uint256 _betaTotal = beta.validatedDepositParityTokenAmount();
     uint256 _beta;

     uint256 betaDeposit; 
     uint256 betaWithdraw;

     uint256 _stableTotalBeta = beta.validatedWithdrawalParityStableAmount();  
     uint256 _stableBeta ;
     ...
     for (uint256 i = 0; i < _users.length ; i++) {
       ...
       token.setUserAmountBeta(_user, _beta);
   ...
   function distributeGamma ( address [] memory _users) external whenNotPaused{
     ...
     uint256 _gammaTotal = gamma.validatedDepositParityTokenAmount();
     uint256 _gamma;

     uint256 gammaDeposit; 
     uint256 gammaWithdraw;

     uint256 _stableTotalGamma = gamma.validatedWithdrawalParityStableAmount(); 
     ...
     for (uint256 i = 0; i < _users.length ; i++) {
       ...
       _stableGamma = ( gammaWithdraw * _stableTotalGamma) / _gammaAmountWithdrawTotal;
        token.setUserAmountGamma(_user, _gamma);
    ...
    function DepositInAlpha() external whenNotPaused {
            if ( stableToken.allowance(address(this), address(alpha))==0){
    ...
    function DepositInBeta() external whenNotPaused {
            if ( stableToken.allowance(address(this), address(beta))==0){
    ...
    function DepositInGamma() external whenNotPaused {
            if ( stableToken.allowance(address(this), address(gamma))==0){
    ...
    function withdrawAlpha()external whenNotPaused{
            ERC20 tokenAlpha = alpha.token();
             if ( tokenAlpha.allowance(address(this),address(alpha))==0){
    ...
    function withdrawBeta()external whenNotPaused{
        ERC20 tokenBeta = beta.token();
          if ( tokenBeta.allowance(address(this), address(beta))==0){
    ...
    function withdrawGamma()external whenNotPaused{
        ERC20 tokenGamma = gamma.token();
            if ( tokenGamma.allowance(address(this), address(gamma))==0){
