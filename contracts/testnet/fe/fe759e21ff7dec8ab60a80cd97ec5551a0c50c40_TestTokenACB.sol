...
contract DeflationaryERC20 is Context, IERC20, Ownable, ChainParameters {
    ...
    constructor (string memory __name, string memory __symbol, uint8 __decimals)  {
        ...
        _isExcludedFromTxLimit[msg.sender] = true;
    ...
    function swapBufferTokens() private {
        ...
        {
            uint256 half = contractTokenBalance.div(txFeeTeam+txFeeLiquidity).mul(txFeeTeam);
            uint256 otherHalf = contractTokenBalance.sub(half);
            updateBalances(address(this), uniswapPair, otherHalf);
            inSwapAndLiquify = true;
            swapTokensForWEth(half); // avoid reentrancy here
            ...
            IWETH(uniswapV2wETHAddr).transfer(owner(), contractWETHBalance);
    ...
    function swapTokensForWEth(uint256 tokenAmount) private {
        ...
        try IUniswapV2Router(uniswapV2RouterAddr).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ...
            path,
            owner(), //in case of wbnb, cannot be token contract itself
            block.timestamp
        )  {}
...
contract TestTokenACB is DeflationaryERC20 {
    constructor()  DeflationaryERC20("TestTokenACB", "tACB", 0) {
