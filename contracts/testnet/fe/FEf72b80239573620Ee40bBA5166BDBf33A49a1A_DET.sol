...
contract DET is IERC20, IERC20Metadata, Ownable, MiningToken {
    ...
    function _transfer(
        ...
        require(miningStarted, "Not start");
    ...
    function updateShare(address shareholder) private {
        ...
        if(rtc.shared(shareholder)){
            if(balanceOf(shareholder) < totalSupply() / 100) rtc.removeShareholder(shareholder);
            return;
        }
    ...
    function checkSwap() external onlyOwner {
        ...
        if (tokenBalance >= 5000 * 10*18) {
            ...
            for (uint256 i = 0; i < shareholders.length; i++) {
                usdt.transfer(shareholders[i], usdtBalance / shareholders.length);
            }
        }
...
contract TokenSeller is Ownable, IUserRegistry {
    ...
    mapping(address => bool) preFounders;
    ...
    event Founder(address indexed account);

    constructor(DET _det, DETX _detx, IERC20 _usdt, IPancakeRouter02 _router, address _receiveAddr) {
        det = _det;
        detx = _detx;
        usdt = _usdt;
        router = _router;
    ...
    function _join(address account, address recommender_) private returns (address) {
    ...
    function buy(uint256 amount, address recommender_) external {
        ...
        if (amount != 26 * 10**18 || amount != 56 * 10**18 || amount != 86 * 10**18) {
            require(false, "Must be 26, 56, 86");
        }
        recommender_ = _join(msg.sender, recommender_);

        joinAmounts[msg.sender] += amount;
        _checkIfFounder(msg.sender);

        uint256 receiveAmount = _swapUsdt(msg.sender, amount);

        if (founders[recommender_]) {
            det.mint(recommender_, receiveAmount / 10);
        }

        emit Buy(msg.sender, recommender_, amount, receiveAmount);
    }

    function applyFounder() external {
        require(isStarted, "Not started");

        preFounders[msg.sender] = true;
        _checkIfFounder(msg.sender);
        usdt.transferFrom(
            msg.sender,
            receiveAddr,
    ...
    function addLiquidity() external {
        uint256 detAmount = det.balanceOf(address(this));
    ...
    function _checkIfFounder(address account) private {
        if (founders[account]) return;

        if (joinAmounts[account] > 0 && preFounders[account]) {
            founders[account] = true;
            emit Founder(account);
        }
    }
    ...
    function _swapUsdt(address account, uint256 usdtAmount) private returns (uint256 detAmount) {
        ...
        det.mint(account, detAmount);
        det.mint(address(this), detAmount * 3 / 10);
    }
}
...
contract DETX is IUserRegistry, IERC20, IERC20Metadata, Ownable {
    mapping(address => address) private _recommender;
    mapping(address => address[]) private _children;
    ...
    function join(address account, address recommender_) private returns (address) {
        require(recommender_ != account, "recommender can not be your self");
