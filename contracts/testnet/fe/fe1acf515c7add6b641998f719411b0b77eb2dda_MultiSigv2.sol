...
contract MultiSigv2 {
    ...
    uint removeMemberSignersRequired;
    ...
    uint basicSignersRequired;
    
    constructor(address[] memory initialSigners,address _equinox, uint _amountNeeded){
        
        signersRequired = initialSigners.length;
        
        removeMemberSignersRequired = initialSigners.length * 70/100;

        addMemberSignersRequired = initialSigners.length * 70/100;

        transferSignersRequired = initialSigners.length;

        basicSignersRequired = initialSigners.length;
    ...
    function isSigned(uint256 index) public view returns(bool) {
        return proposalSigned[index][msg.sender];
    }
    ...
    function removeWalletMember(uint index) isRemovable(index) isSigner(msg.sender) public{
        ...
        removeMemberSignersRequired = signers.length * 70/100;
        addMemberSignersRequired = signers.length * 70/100;
        transferSignersRequired = signers.length;
        basicSignersRequired = signers.length;
    }
    ...
    // 8. Function : Checks if the user has signed the Proposal
    function isTransferProposalSigned(uint256 index) public view returns(bool) {
        return transferProposalSigned[index][msg.sender];
    }
    ...
    // 8. Function : Checks if the user has signed the Proposal
    function isBasicProposalSigned(uint256 index) public view returns(bool) {
        return basicProposalSigned[index][msg.sender];
    }
