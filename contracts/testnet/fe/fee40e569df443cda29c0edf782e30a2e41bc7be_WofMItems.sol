...
abstract contract WoM1155Payable is WoM1155Permit, AccessControlEnumerable {
    ...
    bytes32 public immutable _WITHWoMW_TOKEN_TYPEHASH =
    ...
    bytes32 public immutable _WITHWoMW_ITEM_TYPEHASH =
    ...
    function withdraw(uint256 channelId, uint256 invoiceNo, uint256 deadline, IERC20 tokenContract, address recipient, uint256 amount, bytes memory signature) external {
        ...
        require(orderSigner != address(0) && cashier != address(0), "WoM1155Payable: invalid channelId");
        require(!paymentChannel.usedInvoices[invoiceNo], "WoM1155Payable: order already paid");
        bytes32 structHash = keccak256(abi.encode(_WITHWoMW_TOKEN_TYPEHASH, channelId, invoiceNo, deadline, tokenContract, recipient, amount));
        bytes32 hash = _hashTypedDataV4(structHash);
        address signer = ECDSA.recover(hash, signature);
        require(signer == orderSigner, "WoM1155Payable: invalid signature");
    ...
    function withdrawOrMint(uint256 channelId, uint256 invoiceNo, uint256 deadline, address recipient, uint256[] memory nftItems, uint256[] memory ftItems, uint256[] memory ftAmounts, bytes memory signature) external {
        ...
        require(orderSigner != address(0) && cashier != address(0), "WoM1155Payable: invalid channelId");
        require(!paymentChannel.usedInvoices[invoiceNo], "WoM1155Payable: order already paid");
        bytes32 structHash = keccak256(abi.encode(_WITHWoMW_ITEM_TYPEHASH, channelId, invoiceNo, deadline, this, recipient, nftItems, ftItems, ftAmounts));
        bytes32 hash = _hashTypedDataV4(structHash);
        address signer = ECDSA.recover(hash, signature);
        require(signer == orderSigner, "WoM1155Payable: invalid signature");
