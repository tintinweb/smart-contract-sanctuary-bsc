...
abstract contract IDAO is IERC165{
     ...
     */
    function quorum(uint256 blockNumber, bool isFinal) public view virtual returns (uint256);
    /**
     * @notice module:voting
...
abstract contract DAO is Context, ERC165, EIP712, IDAO {
   ...
   event setFinalProposal(uint256 finalProposalId, uint64 voteStart, uint64 voteEnd);
    ...
    */
     ...
     */
     function setFirstFinalProposal() public virtual {
      ...
      require((_finalProposalId==0 || state(_finalProposalId) == ProposalState.Defeated) && higestRateProposalId  !=  0, "DAO: Invalid proposalId");
    ...
    function state(uint256 proposalId) public view virtual override returns(ProposalState){
        ...
        if(snapshot == 0){
            revert("DAO: unknown proposal id");
        }
     ...
     */
    function getVotes(address account,uint256 blockNumber) public virtual override view returns(uint256);
    /**
...
abstract contract DAOCountVotes is DAO {
    ...
    mapping(uint256 => ProposalVote)
    private _proposalVotes;
    ...
    function _voteSucceeded(uint256 proposalId) internal view virtual override returns (bool) {
        ...
        return proposalId == _finalProposalId ? proposalForRate_ >= 75 : proposalForRate_ >= 25;
    }

    /**
     * @dev See {DAO-highestRateFor}
     */
    function _highestRateFor() internal view virtual override returns (uint256) {
        uint256 highestRateProposal = 0;
        for(uint256 i = 0; i < _proposalKeys.length; i++){
            if(state(_proposalKeys[i]) == ProposalState.Queued && proposalForRate(_proposalKeys[i]) > proposalForRate(highestRateProposal)){
     ...
     * @dev proposalForRate
     */
    function proposalForRate(uint256 proposalId)public view virtual returns(uint256){
    ...
    function _resetCountVote(uint256 proposalId) public virtual override{
       ...
       for(uint i = 0; i < proposalVote.votedAddress.length; i++){
...
contract WinDAO is DAO, DAOCountVotes, DAOVotes, DAOQuorum{
        ...
        returns (uint256)
    {
     ...
     function getVotes(address account, uint256 blockNumber) public view  override (DAO, DAOVotes) returns (uint256){
         return super.getVotes(account, blockNumber);
     }

    function proposalForRate(uint256 proposalId)public view override returns(uint256){
        if(proposalId == 0) return 0;
        return super.proposalForRate(proposalId) * 100 / token.getPastTotalSupply(proposalSnapshot(proposalId)) ;
    }
    ...
    function propose(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, string memory description)
        ...
        returns (uint256)
    {
        require(token.getPastTotalSupply(block.number - 1) >= 50, "WinDAO:Insufficient total supply");
