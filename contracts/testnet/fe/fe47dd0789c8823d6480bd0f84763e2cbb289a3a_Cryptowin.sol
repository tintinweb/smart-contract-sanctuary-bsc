...
contract Cryptowin is Ownable {
    ...
    uint256 public totalAmount = 60000000 * 10**18;//总供应数量
    uint256 public levelAmount = 10000000 * 10**18;//每层总供应数量
    ...
    function getCurBalance() public view returns(uint256){
        uint256 a = soldAmount % levelAmount;
        return levelAmount - a;
    }
    //获取当前层编号
    ...
    function join(uint256 usdtAmount) public{
        require(usdtAmount >= 10 * 10**18 && usdtAmount <= 100000 * 10**18,"params error");
        ...
        uint256 curAreaBalance = 10000 * 10**18 - soldValue % 10000 * 10**18;
        if(realValue < curAreaBalance){
            ...
            uint256 oldMultiples = soldValue % 100000 * 10**18;
            uint256 newMultiples = (soldValue + realValue) % 100000 * 10**18;
            //是否越过10w刻度
