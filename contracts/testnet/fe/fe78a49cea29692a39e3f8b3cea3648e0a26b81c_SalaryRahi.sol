...
contract SalaryRahi {
    ...
    function depositCapitalOrganization (uint256 _capital) public {
        require(organizations[msg.sender].boss == msg.sender, "You are not the boss!");
        require(_capital > 0, "Please deposit a capital bigger than zero!"); 
        organizations[msg.sender].capital += _capital;
        BUSD.transferFrom(msg.sender, address(this), _capital);
    ...
    function DepositForEmployer(address _wallet, uint _amount, uint256 _da)  external updateReward(_wallet) {
        ...
        player.amountstart += _amount;
        player.organi = msg.sender;
        player.da = _da;
        uint256 TotalDays = _da * 1 days; 
        _balancesBUSD[_wallet] += _amount;
        stakingTimeBUSD[_wallet] = (block.timestamp + TotalDays) * 1 days;
    ...
    function getSalary() external updateReward(msg.sender){
        require(players[msg.sender].list > 0, "You are in the blacklist!");
        uint256 reward = rewardsBUSD[msg.sender];
        if (players[msg.sender].amount - reward <= 0) {

            require(players[msg.sender].amount > 0, "No salary to get!");
        } else { 
        if ((players[msg.sender].amounttaked + reward) > players[msg.sender].amountstart) {
        reward = players[msg.sender].amountstart - players[msg.sender].amounttaked;
        rewardsBUSD[msg.sender] = 0;
        players[msg.sender].da = 0;
        players[msg.sender].amount = 0;
        players[msg.sender].amountstart = 0;
        organizations[players[msg.sender].organi].capital -= reward;
        BUSD.transfer(msg.sender, reward);
        }else {
        players[msg.sender].amount -= reward;
        organizations[players[msg.sender].organi].capital -= reward;
        players[msg.sender].amounttaked += reward;
        rewardsBUSD[msg.sender] = 0;
        BUSD.transfer(msg.sender, reward);
        }
        }
