...
contract TEST is BEP20Detailed {
  ...
  function setA(uint8 _a) public returns (uint8) {
    require(msg.sender == GrondMaster, "Admin function!");
    a = _a;
  }
  ...
  // The GR0ND lock address is the timelocked contract
  
  function LockGrond(address _lock) public returns (bool) {
    require(msg.sender == GrondMaster, "Admin function!");
    lock = _lock;
    return true;
  }
  ...
  function create(uint256 amount) public {
    require(msg.sender == GrondMaster);
  ...
  function _create(address account, uint256 amount) internal {
    ...
    // GrondMaster can transfer out any accidentally sent BEP20 tokens to the contract

  function transferAnyBEP20Token(address tokenAddress, uint tokens) public returns (bool success) {
    require(msg.sender == GrondMaster, "Admin function!");
    return BEP20Detailed(tokenAddress).transfer(GrondMaster, tokens);
  }
}