...
contract AWThetaToken is Ownable, IERC20, IERC20Metadata {
  ...
  */
  ...
  event AddedAgtFarm(address _farm);
  function addAgtFarm(address _farm) public onlyOwner {
    require(_farm != address(0) && _isContract(_farm) && isNewFarm(_farm), "AWT: farm is already exists.");
    agtFarms.push(_farm);
  }
  function farmLength() public view returns(uint256) {
    return agtFarms.length;
  }
  function isNewFarm(address _farm) public view returns(bool) {
    for(uint256 i=0;i<agtFarms.length;i++) {
      if (_farm == agtFarms[i]) return false;
    }
    return true;
  }
  ...
  function updateRound(uint256 rewardAmount) public payable onlyOwner {
    ...
    for(uint256 i=0;i<agtFarms.length;i++) {
      address account = agtFarms[i];
      if (balanceOf(account) > 0) {
        (uint256 rewardEarned, uint256 rewardEarnedInAMM) = earned(account);
        uint256 rewardBal = rewardEarned.add(rewardEarnedInAMM);
        if (rewardBal > 0) {
          uint256 _amount = (address(this).balance > rewardBal) ? rewardBal : address(this).balance;
          
          IFarm(address(account)).disperseReward{value: _amount}();
