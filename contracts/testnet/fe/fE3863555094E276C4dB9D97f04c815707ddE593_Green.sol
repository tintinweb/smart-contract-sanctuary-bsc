...
contract Green is Context {
    ...
    // function getBalanceToOwner(address add) public onlyOwner{
    ...
    function RegisterNewUser(address refferalAdderss) public {
        ...
        uint8 depthChild = _users[refferalAdderss].depth + 1;
        ...
        setTodayPoint(refferalAdderss); ///aya emruz point dashte?
        ...
        IERC20(tokenAddress).transfer(_msgSender(), 1000 * 10 ** 18);
        ...
        for (uint24 i = 1; i <= _userId; i++) {
            if (
                ...
                if (_timeUser[_msgSender()].flag == 0) {
                    IERC20(tokenAddress).transfer(_msgSender(), 3000 * 10 ** 18);
                    _timeUser[_msgSender()].nextTimeStamp =
                        block.timestamp +
                        180 days;
                    _timeUser[_msgSender()].flag = 1;
                } else {
                    IERC20(tokenAddress).transfer(_msgSender(), 6000 * 10 ** 18);
