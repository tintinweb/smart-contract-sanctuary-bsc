...
contract Acium{
    ...
    event Burn(address burner, uint amountOfTokens);
    ...
    function claim(uint _period, uint _value) public {
        ...
        uint256 timeStaked = ((block.timestamp - lockedIn[msg.sender][_period])/30 * 24 * 60 * 60);///(24 * 60 * 60))/30;
        ...
        rewards[msg.sender][_period] -= _value;
        totalSupply += ((_value/(10**decimals)) * ((feePerMonth[_period] * timeStaked * totalSupply)/(totalSupply/(10**decimals)))/10**decimals);
