...
contract AMS is ERC20, Ownable {
    ...
    address public laoAMs;//老ams合约，通过leo挖出来的ams。用户可以把老ams转换为新ams。lp审查前能转换出50%添加lp，lp审查后可把余下50%转换出来但却是处理被锁定状态
    ...
    address[] public receiveAmsUser;//记录：领取过50%的ams的用户
    mapping(address => uint256[2]) public recordAddLPUser;//记录用户添加的lp数量，交易前添加，总添加
    address[] public recordAmsUser;//记录：添加过lp的用户
    ...
    function setInit(uint256 _startTradeTime, uint256 _rpVeryLpNum, address _pair, address _usdwp, address _RP,
        address _laoAMs, uint256 _FIVE_ORD, uint256 _FIVE_ORD_AMOUNT, uint256 _FIVE_OTH) public onlyOwner {
    ...
    //交易开始前，用户一次性映射出来50%的新ams，用于提前添加lp
    function callCoverAmsUser() public {
        //require(block.timestamp < startTradeTime, "user rw ams time stop");//50%的ams领取只能发生在交易开始前，用于去添加lp，交易后没这个业务了
        if (sbefReceiveAms[msg.sender] == false) { //用户的50%只能领取一次，拿去添加lp
            uint256 _num = ERC20(laoAMs).balanceOf(msg.sender) / 2;
            ERC20(address(this)).transfer(msg.sender, _num);
            if (_num > 0) {
                sbefReceiveAms[msg.sender] = true;
                receiveAmsUser.push(msg.sender);
            }
        }
    }
    ...
    function timeWriteVery(address _from, uint256 _amount) public {
        ...
        if (swapWriteList[_from]) {
            ...
            //在限购时间段内购买，只能买100u
            require(fiveOrdBuyAmount[_from] + _amount <= FIVE_ORD_AMOUNT, "file ord buy amount big");
            fiveOrdBuyAmount[_from] = fiveOrdBuyAmount[_from] + _amount;
        } else {
            //其他用户，在30分钟后可开始交易
            require(block.timestamp >= startTradeTime + FIVE_OTH, "swap time not stop");
        }
    ...
    function veryRpRwCall(address _usr, uint256 _lpnum) external {
        ...
        if (block.timestamp < startTradeTime) {//只有在交易开始前，才有做为生效的前提
            ...
            if (recordAddLPUser[_usr][0] == 0) {
                recordAmsUser.push(_usr);
            }
            recordAddLPUser[_usr][0] = recordAddLPUser[_usr][0] + _lpnum;//交易前添加
        }
        recordAddLPUser[_usr][1] = recordAddLPUser[_usr][1] + _lpnum;//总添加
    }
