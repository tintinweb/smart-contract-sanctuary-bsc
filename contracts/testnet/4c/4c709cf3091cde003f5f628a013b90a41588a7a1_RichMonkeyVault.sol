...
// 每天20% 回购销毁
    ...
    function startBonus() public {
        ...
        if(total_amount > origin_bonus){
            ...
            path[0] = usdtToken;
            path[1] = rmtToken;
            swapAmount(des_amount,0,path,address(this));
            // uint256 _rmt_amount = swapAmount(des_amount,0,path,address(this));
            // IERC20(rmtToken).burn(_rmt_amount);
            uint256 length = 0;
    ...
    function burn(uint256 _rmt_amount) public{
        require(devOwner[msg.sender]==true,' not dev');
        IERC20(rmtToken).burn(_rmt_amount);
    }
