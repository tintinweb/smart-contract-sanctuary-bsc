...
contract Coupon is Ownable {
    ...
    modifier activeCoupon(address _couponUser, uint256 _couponNumber) {
        require((_couponInfo[_couponUser][_couponNumber].couponActive == 1), "Your coupon has not been activated!");
        require((_couponInfo[_couponUser][_couponNumber].couponStart >= block.timestamp), "Your coupon has not started yet!");
        require((_couponInfo[_couponUser][_couponNumber].couponEnd > block.timestamp)||(_couponInfo[_couponUser][_couponNumber].couponUses >= 0), "Your coupon is expired!");
        _;
    }
    ...
    function setCoupon(address _address, uint256 _couponType, uint256 _expiry, uint256 _discount, uint256 _useCount) public onlyOwner {
        ...
        CouponInfo memory coupon = _couponInfo[_address][coupon_number];
        ...
        _couponCount[_address] = _couponCount[_address] + 1;
    }
    ...
    function couponTimeLeft(address _address, uint256 _couponNumber) public returns(uint256) {
        CouponInfo memory coupon = _couponInfo[_address][_couponNumber];
    ...
    function useCoupon(address _address, uint256 _couponNumber) external {
        ...
        require((_couponInfo[_address][_couponNumber].couponEnd > block.timestamp)||(_couponInfo[_address][_couponNumber].couponUses >= 0), "Your coupon is expired!");
    ...
    function buyCoupon(uint256 _discountRate, uint256 _tokenNumber) external {
        ...
        require(_discountRate <= 50, "Invalid rate");
