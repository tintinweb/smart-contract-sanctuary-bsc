    ...
    IChainLinkOracleSportData
{
    ...
    mapping(JobType => bytes32) public jobId;

    uint256 public payment = (1 * LINK_DIVISIBILITY) / 100;

    enum JobType {
        JobForMatchIdAndTeamId,
        JobForMatchId
    }
    ...
    constructor(
        address _oracle,
        string memory _jobForMatchIdAndTeamId,
        string memory _jobForMatchId,
        address _linkToken,
        address _pool
        ...
        jobId[JobType.JobForMatchIdAndTeamId] = _jobForMatchIdAndTeamId
            .toBytes32();
        jobId[JobType.JobForMatchId] = _jobForMatchId.toBytes32();
        pool = ICompetitionPool(_pool);
    }

    modifier onlyPermission(address _user) {
        require(
            pool.checkBettingContractExist(_user),
        ...
        returns (bytes32)
    {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId[JobType.JobForMatchIdAndTeamId],
            address(this),
            this.fulfill.selector
        );
        request.add("matchId", _matchId);
        request.add("teamId", _teamId);
    ...
    ) public override onlyPermission(msg.sender) returns (bytes32) {
        Chainlink.Request memory request = buildChainlinkRequest(
            jobId[JobType.JobForMatchId],
            address(this),
            this.fulfill.selector
        );
        request.add("matchId", _matchId);
        request.add("teamId1", _teamId1);
        request.add("teamId2", _teamId2);
        bytes32 myId = sendChainlinkRequest(request, payment);
        isRequest[myId] = true;
        emit GetData(myId);
        return myId;
