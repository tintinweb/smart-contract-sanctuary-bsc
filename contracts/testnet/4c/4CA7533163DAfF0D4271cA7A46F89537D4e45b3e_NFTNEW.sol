...
contract NFTNEW is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount >= price, "need to mint at least 1 NFT");
    uint256 supply = totalSupply();
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    require(IERC20(USDT).transferFrom(msg.sender, address(this), _mintAmount));
  ...
  function withdrawBalance() external onlyOwner {
    IERC20(USDT).transfer(owner(), getBalance());
  }
