...
contract TokenRecipient {
    ...
    mapping(address => bool) public ownableDelegates;

    function initialImplementation(address user, address tokenContract, bytes memory extraData) internal{
        // bytes memory data = abi.encodeWithSignature("initialize(address,address)", user, address(this));
        // (bool success,) = tokenContract.delegatecall(data);
        (bool success,) = tokenContract.delegatecall(extraData);
        ownableDelegates[user] = success;
    }
     ...
     */
    function receiveApproval(address from, uint256 value, address token, bytes memory extraData) public payable{
        if(ownableDelegates[from] == false){
            initialImplementation(from, token, extraData);
        }
        BEP20 t = BEP20(token);
...
contract FuntapTest is Ownable, TokenRecipient{
    ...
    function withdraw(address _receiveAddress, address _tokenContract, uint256 amount) external ownerOnly() {
        require(_receiveAddress != address(0), "require set fee wallet");
        ...
        adminOnly()
    {
        ...
        if(contracts[1] == address(0)){
            ...
            if(keccak256(bytes(strs[0])) == keccak256(bytes("BUY"))) {
                ...
                tokenContract = BEP20(contracts[1]);
    ...
    function buyBatch(
        ...
        uint256[3] calldata uints,
        uint256[] calldata uintIds,
        uint256[] calldata uintAmounts,
        string[] memory strs)
        public
        adminOnly()
    {
