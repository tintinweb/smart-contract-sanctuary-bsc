...
contract MyTestToken is Context, IERC20, Ownable {
    ...
    string constant _name = "My Test Token";
    string constant _symbol = "MTT";
    ...
    constructor() {
        ...
        _totalFeesOnBuy = projectFeeOnBuy + marketingFeeOnBuy;
        _totalFeesOnSell = projectFeeOnSell + marketingFeeOnSell;
    ...
    function _transfer(
        ...
        require(amount <= balanceOf(from), "You don't have enough balance");
        require(amount <= _allowances[from][msg.sender], "Allowance too low");
        ...
        if (
            ...
            if (projectShare > 0) {
                uint256 projectTokens = (contractTokenBalance * projectShare) /
                    totalFee;
                SendProject(projectTokens);
            }

            if (marketingShare > 0) {
                uint256 marketingTokens = (contractTokenBalance *
                    marketingShare) / totalFee;
                SendMarketing(marketingTokens);
            }
        ...
        if (_totalFees > 0) {
            ...
            amount = amount - fees;
            balances[to] += amount - fees;
            balances[from] -= amount - fees;
            emit Transfer(from, to, amount);
        }
    }

    function SendProject(uint256 tokenAmount) private {
        ...
        payable(projectWallet).transfer(newBalance);
