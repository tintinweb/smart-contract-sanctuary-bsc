...
contract FarmContract is ERC20, Ownable{
    ...
    //领取利息跟本金
    function Claim(address to,bool first,bool last) external{
        stakingInfo memory _staking = stakingList[msg.sender];
        require(block.timestamp>=_staking.lockTime || first, "LOCK_STAKING_TIME");
        uint256 Reward = _caclClaim(_staking);
        require(Reward>0 , "INSUFFICIENT_BALANCE");
        if (address(Farm) != address(0) && last) {
            //从外部mint 代币 如果有
