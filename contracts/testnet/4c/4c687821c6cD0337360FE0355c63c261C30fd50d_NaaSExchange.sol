...
abstract contract TransferManager is OwnableUpgradeable, TransferManagerBase
{
    ...
    function transferWithFees(
        ...
        bytes calldata // counterAssetExtraData, used for checking total amount on static transfer
    ) external override {
        ...
        rest = transferRoyalties(transferData, rest, counterAsset, assetExtraData);
    ...
    function transferRoyalties(
        ...
        if (
            ...
            return transferRegularRoyalties(transferData, rest, counterAsset, assetExtraData);
        } else if (counterAsset.assetClass == LibAsset.ERC1155_LAZY_ASSET_CLASS) {
            return transferLazyAssetRoyalties(transferData, rest, counterAsset, assetExtraData);
        } else if (counterAsset.assetClass == LibAsset.ERC1155_LAZY_MULTI_ASSET_CLASS) {
            return transferLazyMultiAssetRoyalties(transferData, rest, counterAsset, assetExtraData);
        } else if (counterAsset.assetClass == LibAsset.ERC1155_LAZY_NFTS_ASSET_CLASS) {
            return transferLazyNFTsRoyalties(transferData, rest, counterAsset, assetExtraData);
        }
        return rest;
    }

    function transferRegularRoyalties(
        TransferData memory transferData,
        uint256 rest,
        LibAsset.Asset memory counterAsset,
        bytes calldata assetExtraData
        ...
        return transferRoyalties(
            ...
            rest,
            royaltiesRegistry.getRoyalties(token, tokenId, transferData.from),
            assetExtraData
        );
    }

    function transferLazyAssetRoyalties(
        TransferData memory transferData,
        uint256 rest,
        LibAsset.Asset memory counterAsset,
        bytes calldata assetExtraData
    ) internal returns (uint256) {
        (address token, LibERC1155Lazy.MintSingleData memory data) = 
                abi.decode(counterAsset.data, (address, LibERC1155Lazy.MintSingleData));
        return transferRoyalties(
            transferData,
            transferData.asset.value,
            rest,
            royaltiesRegistry.getGeneralRoyalties(token, transferData.from, data.royalties),
            assetExtraData
        );
    }

    function transferLazyMultiAssetRoyalties(
        TransferData memory transferData,
        uint256 rest,
        LibAsset.Asset memory counterAsset,
        bytes calldata assetExtraData
    ) internal returns (uint256) {
        (address token, LibERC1155Lazy.MintData memory data) = 
    ...
    function transferLazyNFTsRoyalties(
        TransferData memory transferData,
        uint256 rest,
        LibAsset.Asset memory counterAsset,
        bytes calldata assetExtraData
    ...
    function transferRoyalties(
        ...
        if (royalties.length == 0) return rest;

        uint256 totalRoyalties = 0;
        uint256 restValue = rest;
        for (uint256 i = 0; i < royalties.length; i++) {
            totalRoyalties = totalRoyalties + royalties[i].value;
            uint256 feeValue;
            (restValue, feeValue) = subFeeInBp(restValue, amount, royalties[i].value);
            if (feeValue > 0) {
                _transfer(transferData, feeValue, royalties[i].account, ROYALTY, assetExtraData);
            }
        }
