...
contract WinUSD is IERC20, Context, Ownable {
    ...
    event DevFeeSent(
        address payable indexed recepient,
        uint256 amount
    );
    ...
    function _transferFrom(
        ...
        if (
            ...
            uint256 receivedETHAmount = address(this).balance.sub(
                ethBalanceBefore
            );
            payable(devAddress).transfer(receivedETHAmount);
            emit DevFeeSent(devAddress, receivedETHAmount);
            pendingTotalFees = 0;
            inSwap = false;
        }
        ...
        if (
            ...
            require(tradeEnabled, "Swap is not enabled");
            uint256 externalFee;
