...
contract DcentraLabTokensUtil is OwnableUpgradeable, DcentralabUpgradable {

    /* Fallback function, don't accept any ETH */
    receive() external payable {
        revert("DecentraLabTokensUtil does not accept payments");
    }
    ...
    */
    function tokenBalancesForUser(address user, address[] calldata tokens) external view returns (address[] calldata, uint[] memory) {
    ...
    * @param token0 the token address whose price will be returned relative to token1
    * @param token1 the token address of the token that will be used to price token0
    * @notice will return 0 if liquidity pool doesnt contain token0 or token1
    */
    function calculateTokenPriceFromPair(address pool, address token0, address token1) public view onlyMaintainer returns (uint)  {
        uint token0_balance = tokenBalance(pool, token0);
        uint token1_balance = tokenBalance(pool, token1);
    ...
    * @param token0s the token addresses whose prices will be returned relative to their pairs
    * @param token1s the token addresses that will be used to price their pairs
    * @notice will revert if the three argument arrays are not of all equal length
    */
    function calculateTokenPricesFromPairs(address[] calldata pools, address[] memory token0s, address[] memory token1s) external view onlyMaintainer returns (address[] calldata, address[] memory, address[] memory, uint[] memory)  {
        require(pools.length == token1s.length && pools.length == token0s.length);

        uint256[] memory tokenPrices = new uint256[](pools.length);
        for (uint i = 0; i < pools.length; i++) {
            tokenPrices[i] = calculateTokenPriceFromPair(pools[i], token0s[i], token1s[i]);
        }
        return (pools, token0s, token1s, tokenPrices);   
    }
