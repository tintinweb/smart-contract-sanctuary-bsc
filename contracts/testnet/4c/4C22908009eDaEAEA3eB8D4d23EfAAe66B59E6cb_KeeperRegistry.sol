...
import { EUpkeepFormat } from "./EUpkeepFormat.sol";
import { ConfirmedOwner } from "./ConfirmedOwner.sol";
import { IReceiver } from "./interfaces/IReceiver.sol";
import { IAggregatorV3 } from "./interfaces/IAggregatorV3.sol";
import { ITypeAndVersion } from "./interfaces/ITypeAndVersion.sol";
import { IKeeperCompatible } from "./interfaces/IKeeperCompatible.sol";
import { Config, State, IKeeperRegistryExecutable } from "./interfaces/IKeeperRegistry.sol";
  ...
  Pausable
{
  ...
  uint256 private s_ownerNativeBalance;
  uint256 private s_expectedNativeBalance;
  address private s_registrar;

  mapping(address => uint256) private s_userBalance;
  mapping(address => EnumerableSet.UintSet) private s_userUpkeeps;
  ...
  string public constant override typeAndVersion = "KeeperRegistry 1.0.0";
  ...
  event UpkeepGasLimitSet(uint256 indexed id, uint256 gasLimit);
    ...
    )
  {
    ...
    _prePerformUpkeep(upkeep, params.from, params.maxNativePayment);
  ...
  function cancelUpkeep(uint256 id) external override {
    ...
    s_userUpkeeps[s_upkeep[id].admin].remove(id);
   ...
   */
  function addFunds(uint256 id) external payable override onlyActiveUpkeep(id) {
    uint256 amount = msg.value;
    s_upkeep[id].balance += amount;
    s_expectedNativeBalance += amount;
  ...
  function addUserFunds(address userToFund) external payable {
    ...
    uint256 amount = msg.value;
    s_userBalance[userToFund] += amount;
    s_expectedNativeBalance += amount;
  ...
  /**
   * @notice uses Native's transferAndCall to LINK and add funding to an upkeep
   */
  ...
  function withdrawFunds(uint256 id, address to) external validRecipient(to) onlyUpkeepAdmin(id) {
    ...
    uint256 minUpkeepSpend = s_storage.minUpkeepSpend;
    uint256 amountLeft = s_upkeep[id].balance;
    uint256 amountSpent = s_upkeep[id].amountSpent;
    ...
    s_ownerNativeBalance += cancellationFee;

    s_expectedNativeBalance -= amountToWithdraw;
  ...
  function withdrawUserFunds(uint256 amount, address to) external validRecipient(to) {
    ...
    if (amount == type(uint256).max) {
      amount = s_userBalance[sender];
    } else {
      require(s_userBalance[sender] >= amount, "cannot withdraw more amount that user has");
    }
    unchecked {
      s_userBalance[sender] -= amount;
    }
  ...
  function withdrawOwnerFunds() external onlyOwner {
    uint256 amount = s_ownerNativeBalance;

    s_expectedNativeBalance -= amount;
    s_ownerNativeBalance = 0;
  ...
  function recoverFunds() external onlyOwner {
    ...
    payable(msg.sender).transfer(total - s_expectedNativeBalance);
  }
  ...
  function withdrawPayment(address from, address to) external validRecipient(to) {
    ...
    s_expectedNativeBalance -= keeper.balance;
    emit PaymentWithdrawn(from, keeper.balance, to, msg.sender);

    payable(to).transfer(keeper.balance);
  }
    ...
    )
  {
    ...
    state.ownerLinkBalance = s_ownerNativeBalance;
    state.expectedLinkBalance = s_expectedNativeBalance;
  ...
  function userUpkeeps(address user) external view returns (uint256[] memory) {
    return s_userUpkeeps[user].values();
  }

  function userupkeepsCount(address user) external view returns (uint256) {
    return s_userUpkeeps[user].length();
  }

  function userupkeepsAt(address user, uint256 index) external view returns (uint256) {
    return s_userUpkeeps[user].at(index);
  }

  function userupkeepsContains(address user, uint256 upkeepId) external view returns (bool) {
    return s_userUpkeeps[user].contains(upkeepId);
  }
  ...
  function _createUpkeep(
    ...
    s_expectedNativeBalance += balance;
    s_checkData[id] = checkData;
    s_upkeepIDs.add(id);
    s_userUpkeeps[admin].add(id);
  }
    ...
    returns (bool success)
  {
    ...
    uint256 userBal = s_userBalance[s_upkeep[params.id].admin];
    if (userBal >= payment) {
      s_upkeep[params.id].balance -= payment;
    } else {
      uint256 remainingAmount = payment - userBal;
      s_upkeep[params.id].balance = 0;
      s_userBalance[s_upkeep[params.id].admin] -= remainingAmount;
    }
    s_upkeep[params.id].amountSpent += payment;
    s_upkeep[params.id].lastKeeper = params.from;
    s_keeperInfo[params.from].balance += payment;
  ...
  function _prePerformUpkeep(
    ...
    if (upkeep.balance + s_userBalance[upkeep.admin] < maxNativePayment) revert InsufficientFunds();
