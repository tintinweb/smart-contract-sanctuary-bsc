...
contract FarmContract1216V4 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function getDeservedRefferalReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalRefferalReward) {
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            uint256 availableReward = availableReffReward[_pid][InviteeAddress[i]][_user].sub(claimedReffReward[_pid][InviteeAddress[i]][_user]);
            ...
            deservedPendingReward = deservedPendingReward.mul(getRefferalRewardPercent(_pid, _user)).div(rateDenominator);
    ...
    function getDeservedNodeReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalNodeReward) {
        ...
        uint256 totalAvailableReward;
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            uint256 availableReward = availableNodeReward[_pid][InviteeAddress[i]][_user].sub(claimedNodeReward[_pid][InviteeAddress[i]][_user]);
            totalAvailableReward = totalAvailableReward.add(availableReward);
            if(getLevel(_pid, InviteeAddress[i]) < getLevel(_pid, _user) && depositAmount[_pid][InviteeAddress[i]] > 0) {
                ...
                deservedPendingReward = deservedPendingReward.mul(getNodeRewardPercent(_pid, _user)).div(rateDenominator);
    ...
    function claimNodeReward(uint256 _pid) internal returns (uint256 totalNodeReward) {
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            ...
            if(getLevel(_pid, InviteeAddress[i]) < getLevel(_pid, _msgSender()) && depositAmount[_pid][InviteeAddress[i]] > 0) {
                uint256 deservedPendingReward = pendingMiningReward(_pid, InviteeAddress[i]).add(mintAmount[_pid][InviteeAddress[i]]);
    ...
    function claimAllRefferalReward() public {
        require(getAllPendingRefferalReward(_msgSender()) > 0);
        uint256 totalRefferalReward;
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            uint256 deservedReffReward = getDeservedRefferalReward(pid, _msgSender());
            if(deservedReffReward > 0) {
                uint256 deservedPendingReward = claimRefferalReward(pid);
                totalRefferalReward = totalRefferalReward.add(deservedPendingReward);
            }
        }
        require(IERC20Upgradeable(miningToken).balanceOf(address(this)) >= totalRefferalReward, "The mining reward token balance of this contract is insufficient");
    ...
    function claimAllNodeReward() public {
        ...
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            uint256 deservedNodeReward = getDeservedNodeReward(pid, _msgSender());
            if(deservedNodeReward > 0) {
                uint256 deservedPendingReward = claimNodeReward(pid);
                totalNodeReward = totalNodeReward.add(deservedPendingReward);
            }
        }
        require(IERC20Upgradeable(miningToken).balanceOf(address(this)) >= totalNodeReward, "The mining reward token balance of this contract is insufficient");
        safeTokenTransfer(miningToken, _msgSender(), totalNodeReward);
    }

    function snapShotReffReward(uint256 _pid) internal {
        ...
        if(firstInviter != address(0) && getLevel(_pid, _msgSender()) < getLevel(_pid, firstInviter) && getLevel(_pid, firstInviter) >= 1 && FDeservedReward > 0) {
        ...
        if(secondInviter != address(0) && getLevel(_pid, _msgSender()) < getLevel(_pid, secondInviter) && getLevel(_pid, secondInviter) >= 1 && SDeservedReward > 0) {
    ...
    function snapShotNodeReward(uint256 _pid) internal {
        address firstInviter = IInviteContract(inviteContract).getInviter(_msgSender());
        address secondInviter = IInviteContract(inviteContract).getInviter(firstInviter);
        uint256 currentMiningReward = pendingMiningReward(_pid, _msgSender()).add(mintAmount[_pid][_msgSender()]);
        ...
        if(firstInviter != address(0) && getLevel(_pid, _msgSender()) < getLevel(_pid, firstInviter) && getLevel(_pid, firstInviter) >= 2 && FDeservedReward > 0) {
        ...
        if(secondInviter != address(0) && getLevel(_pid, _msgSender()) < getLevel(_pid, secondInviter) && getLevel(_pid, secondInviter) >= 2 && SDeservedReward > 0) {
    ...
    function emergencyWithdraw(uint256 _pid) public validatePid(_pid) {
        require(enableContract == true,"The contract has not started");
        require(enableWithdraw == true,"Management does not turn on the emergency withdrawal option");
        require(depositAmount[_pid][_msgSender()] > 0,"The amount you stake in this pool is 0");
        updatePool(_pid);
