...
contract CRSSCompensator is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        if (_amount >= claimable) {
            uint128 amount0 = uint128(_amount);
            if (_vesting) {
                ...
                userWithdrawn[msg.sender] += (amount0 * 3) / 4;
                totalWithdrawn += _amount;
            }
            user.crssClaimed += amount0;
        }
    }
        ...
        returns (uint128 crssClaimable)
    {
        ...
        crssClaimable =
            (user.totalCrssOwed *
                ((uint128(block.number) - uint128(compensationStartBlock)) /
                    uint128(compensationLengthInBlocks))) -
            user.crssClaimed;
    }
