...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    // Used to get Infor for parcel Minting
    struct Parcel {
        uint16 x;
        uint16 y;
        uint8 parcelType;
    ...
    modifier onlyEOA() {
        ...
        (
            ...
            "Municipality: This functionality can be called only by Externaly Owned Address"
        );
        _;
    }
        ...
        returns (MintedNFT memory)
    {
        ...
        uint256 _parcelsPriceBUSD = getPriceForParcels(mintingSignature.parcels);
        uint256 _amount = _convertBUSDTo(_parcelsPriceBUSD, mintingSignature.currencyType);

        _transferToContract(_amount);

        MintedNFT memory mintInfo = _mintParcels(mintingSignature, _amount, _parcelsPriceBUSD);
    ...
    function mintMiners(uint16 count, uint8 currencyType)
        ...
        returns (MintedNFT memory)
    {
        uint256 _minersAmount = count * minerPrice;
        uint256 _amount = _convertBUSDTo(_minersAmount, currencyType);

        _transferToContract(_amount);

        MintedNFT memory mintInfo = _mintMiners(count, _amount, currencyType, _minersAmount);

        return mintInfo;
    }

    // @notice (Purchase) Purchase a given amount of el. vouchers
    function purchaseVouchers(uint16 count, uint8 currencyType) external onlyEOA onlySaleActive payable {
        uint256 _vouchersPrice = count * electricityVoucherPrice;
        uint256 _amount = _convertBUSDTo(_vouchersPrice, currencyType);

        _transferToContract(_amount);

        _distributeAmounts(_amount, currencyType, OPERATION_TYPE_PURCHASE_ELECTRICITY, msg.sender);
        userToElectricityVoucherAmountMapping[msg.sender] += count;
        userToPurchasedAmountMapping[msg.sender] += _vouchersPrice;
    }

    // @notice (Purchase) Repair ALL miners - reset the amortization points in the mining SC.
    function repairAllMiners(uint8 currencyType) external onlyEOA onlySaleActive payable {
        uint256 _price = minerRepairPrice;
        uint256 _amount = _convertBUSDTo(_price, currencyType);

        _transferToContract(_amount);

        _distributeAmounts(_amount, currencyType, OPERATION_TYPE_PURCHASE_REPAIR, msg.sender);
        userToRepairDatesMapping[msg.sender].push(block.timestamp);
        userToPurchasedAmountMapping[msg.sender] += _price;
        IMining(miningAddress).repairMiners(msg.sender);
    }

    // @notice Apply a voucher from the user's balance to a selected miner
    function applyVoucher(uint256 numRequestedVouchers, uint256 minerId) external onlyEOA {
    ...
    function attachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) external onlyEOA {
    ...
    function upgradeStandardParcel(uint256 _parcelId, uint8 _currencyType) external onlyEOA onlySaleActive payable {
        _requireParcelExists(_parcelId);
        require(
            IERC721Base(standardParcelNFTAddress).ownerOf(_parcelId) == msg.sender,
            "Invalid NFT owner"
        );
        uint256 _amount = _convertBUSDTo(upgradePrice, _currencyType);
        _transferToContract(_amount);
        _distributeAmounts(_amount, _currencyType, OPERATION_TYPE_UPGRADE_PARCEL, msg.sender);
    ...
    function _distributeAmounts(
        ...
        _distributeNetGymStreet(
            (_amount * distributionPercentages[6]) / 100,
            _amount,
            addressToCurrencyType[currency],
            _user
        );
    ...
    function _distributeNetGymStreet(
        ...
        if (_amount > 0) {
            ...
            INetGymStreet(netGymStreetAddress).distributeRewards(_distributeAmount, _token, _user);
        }
    }

    function _transferToContract(uint256 _amount) private {
        if (msg.value > 0) {
            require(msg.value >= _amount, "Municipality:: Prices do not match");
            IWETH(wbnbAddress).deposit{value: _amount}();
        } else {
    ...
    function _mintParcels(
        ParcelMintSignature calldata mintingSignature,
        uint256 _amount,
        uint256 _price
    ) private returns (MintedNFT memory) {
    ...
    function _mintMiners(
        uint16 _count,
        uint256 _amount,
        uint8 currencyType,
        uint256 _minersAmount
    ) private returns (MintedNFT memory) {
        MintedNFT memory mintInfo;
        ...
        _distributeAmounts(_amount, currencyType, OPERATION_TYPE_MINT_MINERS, msg.sender);
        userToPurchasedAmountMapping[msg.sender] += _minersAmount;
        return mintInfo;
    }
