...
contract FSPPool is Ownable, ReentrancyGuard {
    ...
    function getDepositFee() public view returns (uint256) {
        return FSPFactory(payable(address(SMART_CHEF_FACTORY))).depositFee().mul(rewardPercent).div(10**5);
    }

    function getWithdrawFee() public view returns (uint256) {
        return FSPFactory(payable(address(SMART_CHEF_FACTORY))).withdrawFee().mul(rewardPercent).div(10**5);
    }

    function getEmergencyWithdrawFee() public view returns (uint256) {
        return FSPFactory(payable(address(SMART_CHEF_FACTORY))).emergencyWithdrawFee().mul(rewardPercent).div(10**5);
    }
...
contract FSPFactory is Ownable {
    ...
    uint256 public poolCreateFee0 = .01 ether;
    uint256 public poolCreateFee1 = .02 ether;
    uint256 public poolCreateFee2 = .03 ether;
    uint256 public poolCreateFee3 = .04 ether;
    uint256 public depositFee = 0.007 ether;
    uint256 public withdrawFee = 0.014 ether;
    uint256 public emergencyWithdrawFee = 0.04 ether;
