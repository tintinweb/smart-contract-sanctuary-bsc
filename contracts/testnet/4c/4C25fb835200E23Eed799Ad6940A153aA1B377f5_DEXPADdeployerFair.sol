...
contract DEXPADdeployerFair is ReentrancyGuard {
    ...
    function createFairPreSaleTeam(
        ...
        uint256 tokenAmount = getTotalNumberOfTokensFair(pinkvalues[0].mul(10 ** (token.decimals())), values[3]);
        if(team){
    ...
    function getTotalNumberOfTokensFair(
        ...
        uint256 listSupply = _preSaleAmount.mul(liquidityPercent).mul(1000).div(100);
        listSupply = listSupply.div(1000);
        return _preSaleAmount.add(listSupply);
    }
...
contract PreSaleBnbFairTeam is ReentrancyGuard {
    ...
    function internalclaim(address _user) internal {
        ...
        if(amountRaised < softCap || presaleCancelled){
            ...
            require(Balance > 0,"DEXPAD: Zero balance");
