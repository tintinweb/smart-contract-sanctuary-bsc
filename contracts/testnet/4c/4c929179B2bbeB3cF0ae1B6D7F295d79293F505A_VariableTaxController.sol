...
contract VariableTaxController is OwnableUpgradeable {
    ...
    function upgrade141122() external onlyOwner {
        available_boosters.push(booster_template(1, 15)); //0.1%
        available_boosters.push(booster_template(2, 15)); //0.2%
        available_boosters.push(booster_template(3, 15)); //0.3%
        available_boosters.push(booster_template(4, 15)); //0.4%
        available_boosters.push(booster_template(5, 15)); //0.5%
    ...
    function getBoosterRate( address _addr ) public view returns (uint256){
        ...
        for(uint256 i = 0; i < userStats[_addr].boosters.length; i++){
            user_booster memory booster = userStats[_addr].boosters[i];
    ...
    function buyBooster( uint256 _slotIndex, uint256 _boost, uint256 _days ) external {
        ...
        require(getBoosterRate(_addr) < 10, "maximum boost reached");
        require(userStats[_addr].boosters[_slotIndex].end_time == 0 || userStats[_addr].boosters[_slotIndex].end_time < block.timestamp, "slot not available available" );
        ...
        userStats[_addr].boosters[_slotIndex] = user_booster(_boost, block.timestamp.add(_days.mul(1 days)));        
    }
    ...
    //function getAvailableBoosters() public view returns//TODO: only allow bosters that are known here
