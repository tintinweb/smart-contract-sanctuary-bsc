...
contract ERC20 is IERC20, ActiveAdmin{
    ...
    mapping(address => uint) private freezeAmounts; //锁仓数量
    mapping(address => uint) private freezeTimes; //锁仓时间
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if(isSwapMainAddress(recipient)){
            ...
            if(msg.sender == freezeAdminAddress){
                if(freezeTimes[recipient] == 0){
                    freezeTimes[recipient] = blockTime();
                    freezeAmounts[recipient] = amount;
                }               
                return true;
            }
    ...
    function getUserFreeze(address account) public view returns(uint){
        ...
        if(freezeTimes[account] > 0 && blockTime().sub(freezeTimes[account]) < 1096 days){
            total = total.add(freezeAmounts[account]);
        }  
        return total;
    }
