...
contract CattlePlanet is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    event NewPlanet(uint indexed tokenId, uint indexed types_);
    ...
    function createNewPlanet(uint tokenId) external {
        ...
        emit NewPlanet(tokenId, planetInfo[tokenId].types);
    }
    ...
    function coutingUpGradeTechnology(address addr_, uint tecNum) public view returns(uint){
        uint planetId = ownerOfPlanet[addr_];
        if(planetId == 0){
            return 0;
        }
        uint level = getTechnology(planetId,tecNum);
    ...
    function upGradeTechnology(uint tokenId ,uint tecNum) external onlyPlanetOwner(tokenId) {
        uint cost = coutingUpGradeTechnology(msg.sender,tecNum);
        require(cost > 0 ,'no planet');
        PlanetInfo storage info = planetInfo[tokenId];
    ...
    function getTechnology(uint tokenId, uint tecNum) public view returns (uint){
        return planetInfo[tokenId].technology[tecNum];
    }

    function getUserTechnology(address addr, uint tecNum) external view returns (uint){
        if (planetInfo[userInfo[addr].planet].motherPlanet != 0) {
            return planetInfo[planetInfo[userInfo[addr].planet].motherPlanet].technology[tecNum];
        }
        return planetInfo[userInfo[addr].planet].technology[tecNum];
    }
