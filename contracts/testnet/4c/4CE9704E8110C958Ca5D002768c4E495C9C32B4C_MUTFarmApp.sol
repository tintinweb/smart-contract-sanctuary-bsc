...
contract MUTFarmApp {
    ...
    function _sowSeed(address account, address parent, uint256 withPaymentAmount, uint256 withSharedProfit, uint256 withFomoReward) internal {
        ...
        require(consumeAmount >= ssSetting.getMinAmountBuy(0), "too small amount");
        require(consumeAmount <= ssSetting.getMaxAmountBuy(0), "too much amount");
        ...
        require(ERC20(ssAuth.getPayment()).transferFrom(account, ssAuth.getAppWallet(), withPaymentAmount), "sow seed error");
        ...
        require(_distributeMony(account, consumeAmount, withSharedProfit, withFomoReward), "Buy seed error");
    ...
    function claimSeed(uint256 roundNumber) external {
        ...
        require(
            appData.isUserSeedExists(roundNumber, msg.sender),
        ...
        require(appWallet.transferToken(ssAuth.getPayment(), msg.sender, amount), "Failed to claim seed");
    ...
    function collectForgottenSeed(address account, uint256 roundNumber, address to) external {
        ...
        require(avaible && amount > 0, "have no forgotten seed to collect");
  
        require(ERC20(ssAuth.getPayment()).balanceOf(ssAuth.getAppWallet()) >= amount, "insufficient balance in contract");
        // (bool transfered) = ERC20(ssAuth.getPayment()).transfer(to, amount);
        require(appWallet.transferToken(ssAuth.getPayment(), to, amount), "collect forgotten seed error");
    ...
    function withdrawSysFund(uint256 amount, address to) external {
        ...
        require(ERC20(ssAuth.getPayment()).balanceOf(ssAuth.getAppWallet()) >= amount, "$$$%%%");
    ...
    function withdrawCharity(uint256 amount, address to) external {
        ...
        require(appWallet.transferToken(ssAuth.getPayment(), to, amount), "Failed to withdraw charity");
    ...
    {
        (res, percent, hasOnePercent, onePercentTime) = appData.getBacklog(roundNumber, ssSetting.getBacklogTime(0));
    }
    ...
    function claimFomoReward(uint256 amount) external {
        ...
        require(
            ERC20(ssAuth.getPayment()).balanceOf(ssAuth.getAppWallet()) >= claimableAmount,
    ...
    function claimSharedProfit(uint256 amount) external {
        ...
        require(appWallet.transferToken(ssAuth.getPayment(), msg.sender, amount), "claim shared profit error");
    ...
    function sendFomoRewardByOwner(address account, address to) external {
        ...
        require(appWallet.transferToken(ssAuth.getPayment(), to, forgottenAmount), "sendFomoRewardByOwner");
