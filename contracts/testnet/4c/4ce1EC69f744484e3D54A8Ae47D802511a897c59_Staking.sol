...
contract Staking is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        pools[0].rewardPerBlock = 2500000 * 10**18;
        pools[1].rewardPerBlock = 3000000 * 10**18;

        pools[0].locktime = 2 minutes;
        pools[1].locktime = 3 minutes;
    ...
    function deposit(uint256 _lockid, uint256 _amount) public {
        require(_lockid < poollength, " Staking : Unavaliable Lock");
        require(
        ...
        IERC20(lptoken).transfer(feeWallet, feeAmount);
        ...
        returns (uint256, uint256)
    {
        ...
        if (pool.totalStaked == 0) return (0, 0);
        for (uint256 i = 0; i < locks[_lockid][account].length; i++) {
            Lock storage _lock = locks[_lockid][account][i];
            if (block.timestamp >= _lock.endlocktime)
    ...
    function withdraw(uint256 _lockid, uint256 _amount) public {
        ...
        require(
            _withdrawable >= _amount && _amount > 0,
        ...
        for (
            uint256 i = 0;
            i < locks[_lockid][msg.sender].length && pool.totalStaked > 0;
            i++
        ) {
            ...
            if (block.timestamp >= _lock.endlocktime) {
                ...
                if (_amount >= _lock.amount) {
                    _lock.amount = 0;
                    _amount = _amount.sub(_lock.amount);
                } else {
        ...
        IERC20(lptoken).transfer(feeWallet, feeAmount);
        IERC20(lptoken).transfer(msg.sender, tamount);
        if (_pending > 0) IERC20(bunny).transfer(msg.sender, _pending);

        pools[_lockid].totalStaked = pools[_lockid].totalStaked.sub(_amount);
    }

    function claim(uint256 _lockid) external payable {
        require(msg.value >= performanceFee, "should pay small gas to harvest");
        if (performanceFee > 0) {
            uint256 restAmount = msg.value - performanceFee;
        ...
        IERC20(bunny).transfer(msg.sender, _pending);
    }

    function setRewardPerBlock(uint256 _lockid, uint256 _newreward)
        external
        onlyOwner
    {
        pools[_lockid].rewardPerBlock = _newreward;
    }
    ...
    function setDuration(uint256 _lockid, uint256 _locktime)
        external
        onlyOwner
    {
        pools[_lockid].locktime = _locktime;
    }
    ...
    function removeStuckLptoken() external onlyOwner {
    ...
    function removeStuckToken(address _bunny) external onlyOwner {
        IERC20(_bunny).transfer(
            owner(),
            IERC20(_bunny).balanceOf(address(this))
        );
    }
