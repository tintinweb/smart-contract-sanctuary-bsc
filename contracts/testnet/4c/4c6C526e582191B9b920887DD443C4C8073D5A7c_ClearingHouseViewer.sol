...
contract ClearingHouseViewer {
     ...
     * @return position ClearingHouse.Position struct
     */
    function getPositionInfo(IAmm _amm, address _trader)
        external
        view
        returns (
            ClearingHouse.Position memory position,
            SignedDecimal.signedDecimal memory spotUnrealizedPnl,
            SignedDecimal.signedDecimal memory freeCollateral,
            SignedDecimal.signedDecimal memory marginRatio
        )
    {
        position = getPersonalPositionWithFundingPayment(_amm, _trader);
        spotUnrealizedPnl = getUnrealizedPnl(
            _amm,
            _trader,
            ClearingHouse.PnlCalcOption.SPOT_PRICE
        );
        freeCollateral = getFreeCollateral(_amm, _trader);
        if (position.size.toInt() != 0) {
            marginRatio = getMarginRatio(_amm, _trader);
        }
    }
...
abstract contract DecimalERC20 {
    ...
    function _approve(
        ...
        _updateDecimal(address(_token));
        // to be compatible with some erc20 tokens like USDC
