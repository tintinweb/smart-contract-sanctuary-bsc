...
contract BourbonNBlockchainFee is Permissions {
    bytes32 public constant UPDATE_ROLE = keccak256("UPDATE_ROLE");

    ERC20 usdcContract;
    DropERC721 dropERC721Contract;

    uint256 public monthlyFee;
    uint256 public yearlyFee;

    // address => tokenId => Payment[]
    mapping (address => mapping(uint => Payment[])) public payments;

    event MonthlyFeeUpdated(address by, uint256 oldFee, uint256 newFee);
    event YearlyFeeUpdated(address by, uint256 oldFee, uint256 newFee);
    event FeesWithdrawn(address by, uint256 amount);

    constructor(address dropERC721Address, address erc20Address) {
        _setupRole(UPDATE_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);


        require(dropERC721Address != address(0), "invalid dropERC721Address address");
        require(erc20Address != address(0), "invalid erc20Address address");

        dropERC721Contract = DropERC721(dropERC721Address);
        usdcContract = ERC20(erc20Address);
        
        uint8 decimals = usdcContract.decimals();

        monthlyFee = 1 * 10 ** decimals; // 1 USDC
        yearlyFee = 12 * 10 ** decimals; // 12 USDC
    }

    /**
     * @dev Function allowing NFT holder to pay the monthly fee
     * @param tokenIds the tokenId of the NFT
     */
    function payMonthlyFee(uint[] memory tokenIds) public {

        require(tokenIds.length > 0, 'Must pay for at least one NFT');
        
        // Update payment struct for all tokens
        for(uint256 i = 0; i < tokenIds.length; i++) {
            address owner = dropERC721Contract.ownerOf(tokenIds[i]);
            require(owner == msg.sender, 'Must be the owner of the NFT');

            payments[msg.sender][tokenIds[i]].push(
                Payment(monthlyFee, block.timestamp, 1)
            );
        }
        
        uint256 amount = monthlyFee * tokenIds.length;
        SafeERC20.safeTransferFrom(usdcContract, msg.sender, address(this), amount);
    }

    /**
     * @dev Function allowing NFT holder to pay the yearly fee
     * @param tokenIds the tokenId of the NFT
     */
    function payYearlyFee(uint[] memory tokenIds) public {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            payments[msg.sender][tokenIds[i]].push(
                Payment(yearlyFee, block.timestamp, 12)
            );
        }
        
        uint256 amount = yearlyFee * tokenIds.length;
        SafeERC20.safeTransferFrom(usdcContract, msg.sender, address(this), amount);
    }
    
    /**
     * @dev Function allowing admin to widthdraw all the paid fees.
     */
    function withdrawFees() external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint withdrawAmount = usdcContract.balanceOf(address(this));
        require(withdrawAmount > 0, "no fee to widthdraw");

        SafeERC20.safeTransfer(usdcContract, msg.sender, withdrawAmount);
        emit FeesWithdrawn(msg.sender, withdrawAmount);
    } 

    /**
     * @dev Function allowing anyone to view the payment history for an address
     * @param addr address of the NFT holder
     */
    function getPaymentsByAddr(address addr, uint[] memory tokenId) public view returns (Payment[] memory) {
        Payment[] memory paymentsList = new Payment[](tokenId.length);

        for(uint256 i = 0; i < tokenId.length; i++){
            paymentsList = payments[addr][tokenId[i]];
        }
        return paymentsList;
    }

    /**
     * @dev Function allowing anyone to view the payment history for an address and tokenId
     * @param addr address of the NFT holder
     * @param tokenId tokenId of the NFT
     */
    function getPaymentsByAddrToken(address addr, uint256 tokenId) public view returns (Payment[] memory) {
        return payments[addr][tokenId];
    }

    /**
     * @dev Function allowing anyone to check if a token has paid their fee as of today
     * @param addr address of the NFT holder
     * @param tokenId tokenId of the NFT
     */
    function isTokenIdAddrUptoDate(address addr, uint256 tokenId) public view returns (bool) {
        Payment[] memory paymentsList = payments[addr][tokenId];

        // if no payment found, user has not paid for this token's fee
        if (paymentsList.length == 0) {
            return false;
        }

        Payment memory lastPayment = paymentsList[paymentsList.length - 1];
        uint256 monthsPaid = lastPayment.monthsPaid;
        uint256 timestamp = lastPayment.timestamp;

        // if days paid is less than days since last payment, token is not up to date
        return block.timestamp - timestamp < 30 days * monthsPaid;
    }


    /**
     * @dev Function allowing anyone to check if a token has their fee in the last 3 months
     * @param addr address of the NFT holder
     * @param tokenId tokenId of the NFT
     * @param tokenMintTimestamp timestamp of the NFT mint
     */
    function isTokenIdAddrForfeitable(address addr, uint256 tokenId, uint256 tokenMintTimestamp) public view returns (bool) {
        Payment[] memory paymentsList = payments[addr][tokenId];

        // if no payment found, check against mint timestamp instead
        if (paymentsList.length == 0) {
            // if token was minted more than 3 months ago, it is forfeitable
            return block.timestamp - tokenMintTimestamp > 90 days;
        }
        ...
        uint256 timestamp = lastPayment.timestamp;
         ...
         // if they haven't paid for 3 months, token is forfeitable
        return block.timestamp - timestamp > (30 days * monthsPaid) + 90 days;
    }

    /**
     * @dev Function allowing admin to set the monthly fee.
     * @param amount the amount the fee should be set too.
     */
    function updateMontlyFee(uint256 amount) external onlyRole(UPDATE_ROLE) {
        uint256 oldFee = monthlyFee;
        monthlyFee = amount;
        emit MonthlyFeeUpdated(msg.sender, oldFee, amount);
    }

    /**
     * @dev Function allowing admin to set the yearly fee.
     * @param amount the amount the fee should be set too.
     */
    function updateYearlyFee(uint256 amount) external onlyRole(UPDATE_ROLE) {
        uint256 oldFee = yearlyFee;
        yearlyFee = amount;
        emit YearlyFeeUpdated(msg.sender, oldFee, amount);
    }

    /**
     * @dev Function allowing admin to grant update permission
     * @param addr account to grant role
     */
    function grantUpdaterole(address addr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(UPDATE_ROLE, addr);
    }

    /**
     * @dev Function allowing admin to revoke update permission
     * @param addr account to revoke role
     */
    function revokeUpdaterole(address addr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(UPDATE_ROLE, addr);
    }
...
import "./BourbonNBlockchainFee.sol";
...
struct TokenMintTimestamp {
    uint256 tokenId;
    uint256 timestamp;
}


contract BourbonNBlockchainRewards is Permissions {

    // token owners cannot claim rewards until 30 days after the token was minted
    uint256 public constant REWARD_CLAIM_DELAY = 30 days;
    bytes32 public constant INJECTION_ROLE = keccak256("INJECTION_ROLE");

    // contract instances
    DropERC721 dropERC721Contract;
    BourbonNBlockchainFee feeContract;
    ERC20 usdcContract;

    // array of all reward injections
    Injection[] public injections;

    // total amount of rewards that can be claimed by admins
    uint256 public forfeitedBalance;

    // whether rewards are paused
    bool rewardsPaused;

    // tokenId => timestamp mapping
    mapping(uint256 => uint256) public tokenMintTimestamps;

    // address => tokenId => total claimed mapping
    mapping(address => mapping(uint256 => uint256)) public tokenRewardsClaimed;

    // address => balance mapping
    mapping(address => uint256) public balances;

    event RewardInjected(address by, uint256 amount);
    event RewardClaimed(address by, uint256 amount);
    event RewardsPaused(address by);
    event RewardsUnpaused(address by);
    event ForfeitedBalanceWithdrawn(address by, uint256 amount);

    constructor(address dropERC721Address, address erc20Address, address bourbonNBlockchainFeeAddress) {
        _setupRole(INJECTION_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);

        dropERC721Contract = DropERC721(dropERC721Address);
        usdcContract = ERC20(erc20Address);
        feeContract = BourbonNBlockchainFee(bourbonNBlockchainFeeAddress);

        require(dropERC721Address != address(0), "invalid dropERC721Address address");
        require(erc20Address != address(0), "invalid erc20Address address");
        require(bourbonNBlockchainFeeAddress != address(0), "invalid bourbonNBlockchainFeeAddress address");
    ...
    /**
     * @dev Function allowing admin to inject rewards
     * @param amount 10 ** 6, the amount of the reward injection in USDC
     */
    function injectRewards(uint256 amount) public onlyRole(INJECTION_ROLE) {        
        require(amount > 0, 'Reward amount must be strictly positive');

        uint256[] memory validTokenIds = getTokendIdsEligibleForRewards();

        // distribute rewards to token owners
    ...
    /**
     * @dev Function allowing token owner to claim rewards
     * @param tokenIds An array of tokenIds for which the user wants to claim rewards.
     */
    function claimRewards(uint256[] memory tokenIds) public {
        require(!rewardsPaused, 'Rewards are paused');

        uint256 userBalance = balances[msg.sender];
        require(userBalance > 0, 'No rewards to claim');
        require(tokenIds.length > 0, 'Must claim rewards for at least one token');

        for (uint i = 0; i < tokenIds.length; i++) {
            bool tokenPaid = feeContract.isTokenIdAddrUptoDate(msg.sender, tokenIds[i]);
            require(tokenPaid, 'tokenId has not paid for their membership fee');
        }

        SafeERC20.safeTransfer(usdcContract, msg.sender, userBalance);
        balances[msg.sender] = 0;

        emit RewardClaimed(msg.sender, userBalance);
    }

    /**
     * @dev Function to get tokenIds eligible for rewards
     * @return An array of tokenIds eligible for rewards.
     */
    function getTokendIdsEligibleForRewards() public view returns (uint256[] memory) {
        uint256 count = 0;
        uint256 totalSupply = dropERC721Contract.totalSupply();
        
        for (uint256 i = 0; i < totalSupply; i++) {
            if (tokenMintTimestamps[i] > 0 && tokenMintTimestamps[i] < block.timestamp - REWARD_CLAIM_DELAY) {
                count++;
            }
        }
        uint256[] memory validSupply = new uint256[](count);
        uint256 validSupplyId = 0;
        
        for (uint256 i = 0; i < totalSupply; i++) {
            if (tokenMintTimestamps[i] > 0 && tokenMintTimestamps[i] < block.timestamp - REWARD_CLAIM_DELAY) {
                // the NFT is at least 30 days old
                validSupply[validSupplyId] = i;
                validSupplyId++;
            }
        }
        return validSupply;
    ...
    /**
     * @dev Function allowing token owner or admin to set the mint timestamp for a token.
     * This is necessary because the DropERC721 contract does not have a mint timestamp.
     * @param tokenTimestamps An array of TokenMintTimestamp structs.
     */
    function setTokenMintDate(TokenMintTimestamp[] memory tokenTimestamps) public {
        for (uint256 i = 0; i < tokenTimestamps.length; i++) {

            uint256 requestedTokenId = tokenTimestamps[i].tokenId;
            uint256 requestedTimestamp = tokenTimestamps[i].timestamp;

            // if `msg.sender` not admin, validate that `msg.sender` owns the token
            // and that the requested timestamp is within 24 hours of current time
            // this is necessary for admins to retroactively set mint timestamps while protecting against malicious users
            if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
                require(dropERC721Contract.ownerOf(requestedTokenId) == msg.sender, 'Only owner can set mint timestamp');            
                require(requestedTimestamp > block.timestamp - 1 days, 'Mint timestamp must be within 24 hours of current time');
            }


            // validate that the requested timestamp is not in the future
            // and that the token does not already have a mint timestamp set
            require(tokenMintTimestamps[requestedTokenId] == 0, 'Mint timestamp already set');
            require(requestedTimestamp <= block.timestamp, 'Mint timestamp must not be in the future');


            tokenMintTimestamps[requestedTokenId] = requestedTimestamp;
        }
    }

    /**
     * @dev Function allowing super-admin to withdraw forfeited balance
     * @param addr the address to withdraw forfeited balance to
     */
    function withdrawForfeitedBalance(address addr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        SafeERC20.safeTransfer(usdcContract, addr, forfeitedBalance);
        forfeitedBalance = 0;

        emit ForfeitedBalanceWithdrawn(addr, forfeitedBalance);
    }

    /**
     * @dev Function allowing super-admin to grant injection role
     * @param addr the address to grant injection role to
     */
    function grantInjection(address addr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(INJECTION_ROLE, addr);
    }

    /**
     * @dev Function allowing super-admin to revoke injection role
     * @param addr the address to revoke injection role from
     */
    function revokeInjection(address addr) public onlyRole(DEFAULT_ADMIN_ROLE) {
        revokeRole(INJECTION_ROLE, addr);
    }

    /**
     * @dev Function allowing super-admin to pause reward claiming for all users
     */
    function pauseRewards() public onlyRole(DEFAULT_ADMIN_ROLE) {
        rewardsPaused = true;
    ...
    /**
     * @dev Function allowing super-admin to unpause reward claiming for all users
     */
    function unpauseRewards() public onlyRole(DEFAULT_ADMIN_ROLE) {
        rewardsPaused = false;
    ...
    /**
     * @dev Function to get the number of injections
     * @return length of the injections array
     */
    function injectionCount() public view returns (uint256) {
