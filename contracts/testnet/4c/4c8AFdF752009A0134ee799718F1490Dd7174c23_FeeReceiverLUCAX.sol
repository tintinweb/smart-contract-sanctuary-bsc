...
contract FeeReceiverLUCAX {
    ...
    address public LUCAX = 0x8B883AFF4E72aeA6a95B3005b794d5F6DE91cCCE;
    IERC20 public DAI = IERC20(0xEC5dCb5Dbf4B114C9d0F65BcCAb49EC54F6A0867);
   IUniswapV2Router02 public router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
  uint public xAmt  = 50;
    constructor(address _LUCAX,address _DAI,address _router){
    ...
    function trigger() external {
        _trigger(address(DAI));
    }

    function bnbToDAI() external {
        _bnbToDAI();
    }

    function swapToDAI(address token) external {
        _swapToDAI(token);
    }

    function swapToDAI(address[] calldata tokens) external {
    ...
    function _swapToDAI(address token) internal {
        uint balance = IERC20(token).balanceOf(address(this));

        if (token == address(DAI) || balance <= 1000) {
            return;
        }
