...
contract CPTOKENPROGRAMME {
    ...
    /**
        ...
        @param _programId is the id of the program
        @param _effectiveDate is the effective date of the format mm/dd/yyyy
        @param _expiryDate is the expiry date of the format mm/dd/yyyy
     */

    function setProgram(uint256 _programId, string memory _effectiveDate, string memory _expiryDate) external {

        // must not be zero

        require(_programId != 0, "Program cannot be zero");
        require(_usedProgramId[_programId] == false, "Used Program Id");
        _program[_programId] = Program(_programId, _effectiveDate, _expiryDate, msg.sender);
        _usedProgramId[_programId] = true;
        emit SetProgram(_programId, msg.sender);
    ...
    function getProgram(uint256 _programID) public view returns (Program memory)  {
        ...
        require(_usedProgramId[_programID] == true, "Id does not exist");
    ...
    event SetProgram (uint256 indexed _programId, address indexed _owner);          //  event emitted after a program has been set
