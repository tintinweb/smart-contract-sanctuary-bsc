...
abstract contract Pausable is Context {
     ...
     */
    event unmuted(address account);

    bool private _paused;

    /**
     * @dev Initializes the contract in unmuted state.
     */
    constructor() {
        _paused = false;
    }
     ...
     * - The contract must be paused.
     */
    function _unmute() internal virtual whenPaused {
        _paused = false;
        emit unmuted(_msgSender());
    }
}
...
contract Metagame is ERC20, Ownable, Pausable {
    ...
    function _transfer(
        ...
        require(!paused() || _whiteList[sender] || _whiteList[recipient] || msg.sender == owner(), "Metagame: token transfer while paused");
        }
        require(!isBlisted(msg.sender), "Metagame: sender blacklisted");
        require(!isBlisted(recipient), "Metagame: recipient blacklisted");
        require(!isBlisted(tx.origin), "Metagame: sender blacklisted");
    ...
    function Trade() public onlyOwner {
        require(!paused(), "Metagame: Contract is already paused");
        _pause();
    }

    /**
     * @dev Unmutes transfers on the token.
     */
    function unmute() public onlyOwner {
        require(paused(), "Metagame: Contract is not paused");
        _unmute();
    }
     ...
     */
    function enableBlist(address account) public onlyOwner {
        require(!blacklist[account], "Metagame: Account is already blacklisted");
     ...
     */
    function disableBlist(address account) public onlyOwner {
        require(blacklist[account], "Metagame: Account is not blacklisted");
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "Metagame: Account is already excluded");
    ...
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "Metagame: Account is not excluded");
    ...
    function enableTax() public onlyOwner {
        require(!taxStatus, "Metagame: Tax is already enabled");
        taxStatus = true;
    }
    ...
    function disableTax() public onlyOwner {
        require(taxStatus, "Metagame: Tax is already disabled");
        taxStatus = false;
    }
     ...
     */
    function isBlisted(address account) public view returns (bool) {
