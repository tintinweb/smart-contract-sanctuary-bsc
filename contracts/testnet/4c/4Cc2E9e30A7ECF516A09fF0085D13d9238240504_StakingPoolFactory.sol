...
contract StakingPoolFactory is Ownable {
    ...
    function createPool(
        ...
        rewardsToken.approve(address(this), _totalRewards);
    ...
    // function getPools() public view returns (PoolForExport[] memory) {
    //     PoolForExport[] memory _pools;
    //     for(uint i=0; i<poolId.current(); i++) {
    //         PoolForExport storage _pool = pools[i];

    //     }
    // }


    // function getPoolsByOwner(address _owner) internal view returns (uint256[] storage) {
    //     uint256[] storage poolsByOwner;
