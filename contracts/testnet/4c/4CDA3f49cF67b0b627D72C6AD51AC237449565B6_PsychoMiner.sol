...
holders.push(wallet);
UniquePlayers += 1;
AddToEventLogInvest(wallet, value, false, source, block.timestamp, TGLINK);//CHANGE ADDRESS 0 TO INCOMING REF
}
...
AddToEventLogInvest(wallet, value, true, source, block.timestamp, TGLINK);//CHANGE ADDRESS 0 TO INCOMING REF
}
...
else{if(GetTimeTilNextPayout(holders[x]) == 0){PayoutUser(TotalPaidIn[holders[x]]/120, holders[x], TGLINK);}}
