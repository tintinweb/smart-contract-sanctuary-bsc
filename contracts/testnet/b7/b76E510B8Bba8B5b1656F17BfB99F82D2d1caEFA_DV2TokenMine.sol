...
contract DV2TokenMine is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    function updateMultiplier(address from) public returns (bool allow) {
      ...
      uint256 currentBasePrice = checkMultiplier() == 1 ? nextBasePrice() : nextBasePrice().div(100);
      uint256 currentSellTargetPrice = checkMultiplier() == 1 ? nextSellPrice() : nextSellPrice().div(100);
      if(isMultiEnabled){
        ...
        if(sellActive && buyprice <= currentBasePrice && sellprice <= currentBasePrice){
    ...
    function checkMultiplier() public view returns (uint256 mul) {
      uint256 newDenominator = readMultiplierLength() > 0 ? readPowerResult(denominatorMultiplier.mul(WAD),readMultiplierLength()).div(WAD).div(10**readMultiplierLength()): denominatorMultiplier;
      if(isMultiEnabled){
        mul = readPowerResult(globalMultiplier.div(newDenominator),readMultiplierLength()).div(WAD);
      }
    }
    function nextBasePrice() public view returns (uint256 _total){
      _total = checkMultiplier().mul(basePrice);
    }
    function nextSellPrice() public view returns (uint256 _total){
      _total = checkMultiplier().mul(sellTargetPrice);
    }
    // Check User Balance
    ...
    function readPower() public view returns(uint256 mul1,uint256 mul2,uint256 mul3,uint256 mul4,bool _newStatus1,bool _newStatus2,bool _status1,bool _status2,bool _status3,bool _status4){
        ...
        uint256 currentBasePrice = checkMultiplier() == 1 ? nextBasePrice() : nextBasePrice().div(100);
        uint256 currentSellTargetPrice = checkMultiplier() == 1 ? nextSellPrice() : nextSellPrice().div(100);
        uint256 timeStart = startTimeForSwap + intervalSecondsForSwap;
        mul1 = sellprice;
        mul2 = currentBasePrice;
        mul3 = currentSellTargetPrice;
        mul4 = buyprice;
        _newStatus1 = sellActive && buyprice <= currentBasePrice && sellprice <= currentBasePrice;
