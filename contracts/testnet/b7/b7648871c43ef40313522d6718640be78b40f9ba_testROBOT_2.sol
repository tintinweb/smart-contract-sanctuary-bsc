...
function robotprofit() public payable {
	...
	function migrate(address useradd, address referrer, uint256 totaldeposit, uint8 planno, uint256 olddividends, uint256 oldpassive, uint256 oldrefbonus, uint256 oldwithdrawn, uint256 withdrawndivi) public {
		...
		uint256 profitstillnow = user.withdrawndividend;
		if (user.olddividends != 0 ) profitstillnow = profitstillnow.add(user.olddividends);
		uint256 remainingprofit = (usertotaldeposit.mul(25000).div(PERCENTS_DIVIDER)).sub(profitstillnow);
		
		
		if(usertotaldeposit >= 1*10**17 && usertotaldeposit <= 2*10**17) {
			user.calpercent = 100;
        }
        if(usertotaldeposit > 2*10**17 && usertotaldeposit <= 3*10**17) {
            user.calpercent = 150;
        }
		if(usertotaldeposit > 3*10**17 && usertotaldeposit <= 4*10**17) {
            user.calpercent = 200;
        }
		if(usertotaldeposit > 6*10**18 && usertotaldeposit <= 30*10**18) {
             user.calpercent = 300;
        }
        if(usertotaldeposit > 30*10**18) {
           user.calpercent = 400;
        }
		
		uint256 dailyshare = usertotaldeposit.mul(user.calpercent).div(PLANPER_DIVIDER);
		user.caltime = remainingprofit.div(dailyshare);
		user.start = block.timestamp;
