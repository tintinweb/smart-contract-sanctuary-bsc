	...
	OwnableUpgradeable
{
	...
	function initialize() public initializer {
		__Ownable_init();
		ADMIN_ADDRESS = 0x72F07b3882C0E41575cD2e5e8fBE20f578F6B768;
	}
	 ...
	 */
	function withdrawToken(address token, uint256 amount) public OnlyAdmin OnlyWhiteList(token) {
	  if (amount > 0){
	...
	function depositByUSD(address token, uint256 amount) public OnlyWhiteList(token) {
		uint256 priceUsd = getPriceToken(token);
		uint256 quantity = amount * priceUsd / 1e18;
	  ERC20Upgradeable(token).transferFrom(msg.sender, address(this), amount);
		balance[msg.sender] += quantity;
		emit Deposit(msg.sender, token, quantity);
	}
    ...
    **/
	...
	function addPriceSource(address token, address priceSource) public onlyOwner {
