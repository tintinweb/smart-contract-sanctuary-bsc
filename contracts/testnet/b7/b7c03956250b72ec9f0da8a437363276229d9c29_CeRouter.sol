    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        IERC20(ceToken).approve(hDao, type(uint256).max);
    }
        ...
        returns (uint256 value)
    {
        ...
        value = _vault.deposit(address(this), realAmount - profit);
        ...
        returns (uint256 value)
    {
        ...
        value = _vault.deposit(address(this), amount);
        // provide collateral to helio Dao
        _provideCollateral(msg.sender, value);
    ...
    function claimYield(address recipient) external nonReentrant {
        ...
        require(profit > 0, "has not got yield");
        // let's check balance of CeRouter in aBNBc
        require(
        ...
        returns (uint256 realAmount)
    {
        ...
        realAmount = _vault.withdraw(address(this), amount);
        _pool.unstakeCerts(msg.sender, realAmount);
    ...
    {
        _withdrawCollateral(msg.sender, amount);
        _vault.withdraw(recipient, amount);
    ...
    function _provideCollateral(address account, uint256 amount) internal {
        ...
        // mint 'toDeposit' of hBNB to msg.sender
    ...
    function _withdrawCollateral(address account, uint256 amount) internal {
        ...
        // burn 'toWithdrawal' of hBNB from msg.sender
    ...
    function liquidation(
        ...
        uint256 value = _vault.withdraw(recipient, amount);
        _collateralToken.burn(owner, value);
    }
    ...
    function changeDao(address dao) external onlyOwner {
        _dao = IDao(dao);
        _ceToken.approve(address(_dao), type(uint256).max);
        emit ChangeDao(dao);
    }
