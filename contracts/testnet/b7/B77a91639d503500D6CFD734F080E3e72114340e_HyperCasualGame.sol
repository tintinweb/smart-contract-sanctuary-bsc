...
contract HyperCasualGame is Ownable {
  ...
  address public token; // token used for betting in the game
  ...
  event PaidOut(address[] players, uint256[] amounts);
  // ------ Events End ------


  /// @dev Constructor that is run once during the contract deployment
  constructor(address _token, uint256 _entryFee, uint256 _waitingTime, address _keeper) public {
    token = _token;
  ...
  function test(address sender) external returns (bool, uint256) {
    (bool success, bytes memory result) = token.call(abi.encodeWithSignature("allowance(address,address)", sender, address(this)));
    uint256 approvedAllowance = abi.decode(result, (uint));
    
    return (success, approvedAllowance);
  }
  ...
  /**
     ...
     */
  function requestMatch(uint256 amount) external returns (uint256) {
    ...
    (bool success, bytes memory result) = token.call(abi.encodeWithSignature("allowance(address,address)", msg.sender, address(this)));
    uint256 approvedAllowance = abi.decode(result, (uint));
    require(success, "Token allowance check failed");
    require(approvedAllowance >= amount, "Not enough token allowance");
    ...
    if (success) {
      emit MatchRequested(msg.sender, gameId, amount);
    }
     ...
     * the user must have approved this contract
     */
  function joinMatch(uint256 gameId, uint256 amount) external {
    ...
    (bool success, bytes memory result) = token.call(abi.encodeWithSignature("allowance(address,address)", msg.sender, address(this)));
    uint256 approvedAllowance = abi.decode(result, (uint));
    require(success, "Token allowance checked failed");
    require(approvedAllowance >= amount, "Not enough token allowance");

    (success, result) = token.call(abi.encodeWithSignature("transferFrom(address,address,uint256)", msg.sender, address(this), amount));
    // token.transferFrom(msg.sender, address(this), entryFee);
  ...
  function cancelGame(uint256 gameId) external mustBeCreator(gameId) mustBeOpened(gameId) mustHaveExpiredWaitingTime(gameId) {
    ...
    (bool success, ) = token.call(abi.encodeWithSignature("transfer(address,uint256)", msg.sender, game.entryFee));
     ...
     * NOTE: cancelGame enables a user (creator) to cancel her game
     */
  function payout(address[] calldata players, uint256[] calldata amounts) external onlyOwner() {
    ...
    (bool success, bytes memory result) = token.call(abi.encodeWithSignature("balanceOf(address)", address(this)));
    ...
    (success, ) = token.call(abi.encodeWithSignature("multiTransfer(address[],uint256[])", players, amounts));

    if (success) {
      emit PaidOut(players, amounts);
    }
  }
