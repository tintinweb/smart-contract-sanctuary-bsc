...
Created by Gen - TokensByGen.com
...
contract DividendDistributor is IDividendDistributor {
 ...
 ///   IERC20 BUSD = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    IERC20 BUSD = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee); //TEST
  ...
  ///  uint256 public minPeriod = 45 * 60; // 45 mins
///    uint256 public minDistribution = 1 * (10 ** 16); // $0.01

    uint256 public minPeriod = 5; // 45 mins///TEST SECONDS
    uint256 public minDistribution = 1 * (10 ** 2); // $0.01///TEST
...
contract REWARDS_TOKEN is Context, IERC20 { 
  ...
  ///  IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); 
    ...
    Rewards_Distributor = new DividendDistributor();
    ...
    // Set Distributor
    DividendDistributor public Rewards_Distributor;
    ...
    bool public blockInitialBuys = true;
    ...
    */
    ...
    function Contract_SetUp_04__Wallet_Limits(
        ...
        if (launchMode || !trade_Open){
            ...
            require(Max_Tokens_Each_Transaction >= _iTotal / 2000 / 10**_decimals, "E03"); // 0.05% minimum limit
            require(Max_Total_Tokens_Per_Wallet >= _iTotal / 2000 / 10**_decimals, "E04"); // 0.05% minimum limit


        } else {

            // Buyer protection - Minimum limits after Launch Mode 0.1% Transaction, 0.5% wallet
            require(Max_Tokens_Each_Transaction >= _iTotal / 1000 / 10**_decimals, "E05"); // 0.1% minimum limit
            require(Max_Total_Tokens_Per_Wallet >= _iTotal / 200 / 10**_decimals, "E06"); // 0.5% minimum limit
    ...
    during Launch Mode if blockInitialBuys is set to true.

    These wallets can then be checked for bot activity and blacklisted (and refunded) if required.
    blockInitialBuys can be set to false to allow early buyers to sell normally during Launch mode.
    ...
    */
    ...
    function Contract_SetUp_08__Deactivate_Launch_Mode() external onlyOwner {
        ...
        if (max_Tran < _iTotal / 1000) {

            max_Tran = _iTotal / 1000;
        }
    ...
    */
    ...
    function Contract_SetUp_09__Update_Wallets(
        ...
        require(BNB_Fee_Wallet != address(0), "E10"); // Enter a valid BSC wallet
        Wallet_Liquidity = Liquidity_Collection_Wallet;

        // Update BNB Fee Wallet
        require(BNB_Fee_Wallet != address(0), "E11"); // Enter a valid BSC wallet
    ...
    */
    ...
    function _Rewards__Exclude_From_Rewards(address Wallet_Address, bool true_or_false) external onlyOwner {
        ...
        if(true_or_false){
            ...
            Rewards_Distributor.setShare(Wallet_Address, 0);

            } else {

            Rewards_Distributor.setShare(Wallet_Address, _tOwned[Wallet_Address]);
        }
    }
    ...
    function _Rewards__Distribution_Triggers(uint256 Minutes_Between_Payments, uint256 Required_Reward_Balance) external onlyOwner {
        ...
        require(Minutes_Between_Payments <= 2880,"E17"); // Max Wait is 2 Days        
        require(Required_Reward_Balance <= 2_000_000_000_000_000_000,"E18"); // Max Required Balance for Payout is $2.00

        // Convert minutes to seconds
        uint256 _minPeriod = Minutes_Between_Payments * 60;

        Rewards_Distributor.setDistributionCriteria(_minPeriod, Required_Reward_Balance);
    ...
    */
    ...
    function _transfer(
        ...
        if(!isExcludedFromRewards[from]) {
            try Rewards_Distributor.setShare(from, _tOwned[from]) {} catch {}
        }

        if(!isExcludedFromRewards[to]) {
            try Rewards_Distributor.setShare(to, _tOwned[to]) {} catch {} 
        }

        try Rewards_Distributor.process(distributorGas) {} catch {}
    ...
    */
    ...
    function swapAndLiquify(uint256 Tokens) private {
        ...
        if(BNB_Rewards > 0){
            ...
            try Rewards_Distributor.deposit{value: BNB_Rewards}() {} catch {}
    ...
    */
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool Fee) private {
        ...
        if (burnFromSupply && recipient == DEAD) {
