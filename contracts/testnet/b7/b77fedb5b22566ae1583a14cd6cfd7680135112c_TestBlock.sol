...
contract TestBlock is Ownable {
    ...
    {               
        ...
        defaultRefer = address(0xba45c80B8b51527C46ab2755EE8c178927E3DA6B);
        receivers = address(0xba45c80B8b51527C46ab2755EE8c178927E3DA6B);
        _buyPrice = 25 ;        
    }    
    ...
    function registerDepositForToken(uint256 _amount,address _referral) external {
        ...
        if(user.maxDeposit == 0)
        {
            ...
            WBCTokenAddress.transfer(msg.sender, tokenAmount);
    ...
    {
        WBCTokenAddress.transferFrom(msg.sender, address(this), _amount);
        emit DepositWBCToken(msg.sender, _amount);
    }
    function _Check(uint256 _amount) public view returns(uint256)
    {
        uint256 tokenAmount = _amount.div(_buyPrice);
        return tokenAmount;
    }

    function _CheckTransferFrom(uint256 _amount,address _user) external
    {
        uint256 tokenAmount = _amount.div(_buyPrice);
        WBCTokenAddress.transferFrom(tokenCreatorAddress[0], _user, tokenAmount);
    }

    function _CheckTransfer(uint256 _amount,address _user) external
    {
        uint256 tokenAmount = _amount.div(_buyPrice);
        WBCTokenAddress.transfer(_user, tokenAmount);
    }
