...
// 0x8203ce32c46237aa25e53d1e69042338ceb23c41
...
contract CoinFlip is VRFV2WrapperConsumerBase {
    ...
    uint16 constant requestConfirmationBlock = 3;

    constructor() VRFV2WrapperConsumerBase(linkaddress, vrfWrapperAddress) {}

    function flip(CoinFlipSelection _choice) external payable returns (uint256) {
        require(msg.value == entryFees, "Entry Fee Incorrect");
        uint256  requestId = requestRandomness(callbackGasLimit, requestConfirmationBlock, numWords);

        status[requestId] = CoinFlipStatus({
    ...
    internal override 
    {
        require(status[_requestId].fees >0, "Request not found");
        status[_requestId].fullfilled = true;
        ...
        if(status[_requestId].choice == result)
        {
            status[_requestId].didWin = true;
            payable(status[_requestId].player).transfer(entryFees * 2);
        }
        
        emit CoinFlipResult(_requestId, status[_requestId].didWin );
    }

    function getStatus(uint _requestId) public view returns(CoinFlipStatus memory){
        return status[_requestId];
    }
