...
import "../galler/ILaunchpadNFT.sol";
import "./IMSNFT.sol";

contract DEXMSNFT is
    Context,
    ERC721Enumerable,
    ERC721Burnable,
    ERC721Pausable,
    ...
    IMSNFT
{
    ...
    modifier onlyDefaultAdmin() {
        require(
            ...
            "DEXMSNFT: must have DEFAULT_ADMIN_ROLE"
        );
        _;
    }
    ...
    modifier onlyMinter() {
        require(
            ...
            "DEXMSNFT: must have minter role to mint"
        );
        _;
    }
    ...
    modifier onlyPauser() {
        require(
            ...
            "DEXMSNFT: must have pauser role to do pause/unpause"
        );
        _;
    }

    modifier onlyLaunchpadRole() {
        require(
            hasRole(LAUNCHPAD_ROLE, _msgSender()),
            "DEXMSNFT: must have launchpad role to call mintTo"
        );
        _;
    }
    ...
    constructor(
        ...
        _setupRole(LAUNCHPAD_ROLE, _launchpadAddress);
    ...
    {
        require(_exists(tokenId), "DEXMSNFT: URI query for nonexistent token");
        ...
        returns (uint256[] memory)
    {
        ...
        require(balance != 0, "DEXMSNFT: Owner has no token");
    ...
    function increaseBreedCount(uint256 tokenId) public {
        require(
            hasRole(MINTER_ROLE, _msgSender()),
            "DEXMSNFT: must have minter role to increase breed count"
        );
    ...
    function mint(
        ...
        return
            _mintWithMetaData(_to, _matronId, _sireId, _breedCount, _fertile);
    }
    ...
    // set new max supply
    function setMaxLaunchpadSupply(uint256 newMaxLaunchpadSupply)
        public
        onlyDefaultAdmin
    {
        LAUNCH_MAX_SUPPLY = newMaxLaunchpadSupply;
    }
    ...
    function mintTo(address to, uint256 size) external onlyLaunchpadRole {
        require(to != address(0), "DEXMSNFT: can't mint to empty address");
        require(size > 0, "DEXMSNFT: size must greater than zero");
        require(
            LAUNCH_SUPPLY + size <= LAUNCH_MAX_SUPPLY,
            "DEXMSNFT: max supply reached"
        );

        for (uint256 i = 1; i <= size; i++) {
            uint256 __tokenId = _mintWithMetaData(to, 0, 0, 0, true);
            LAUNCH_SUPPLY++;
            emit GallerMint(to, __tokenId, block.timestamp, true);
        }
    }
}
