...
contract DcfContractBusd {
    ...
    uint256 constant private _timeLock1 = 3 hours;
    uint256 constant private _timeLock2 = 6 hours;
    uint256 constant private _timeLock3 = 12 hours;
    uint256 constant private _aprTime = 12 hours;
        ...
        if (hasUserJoined(user)) {
            ...
            for (uint256 i = 0; i < size; i++) {
                ...
                if (_userMaps[user].accounts[i].closed == false) {
                    info[12 + i * fieldCount] = timeNow.subNoNegative(_userMaps[user].accounts[i].depositTime); // seconds since deposit
                    info[13 + i * fieldCount] = (_userMaps[user].accounts[i].depositTime + _timeLock3).subNoNegative(timeNow); // seconds to finish time lock
                    info[14 + i * fieldCount] = 1; // active account user
                } else {
                    info[12 + i * fieldCount] = 0;
                    info[13 + i * fieldCount] = 0;
                    info[14 + i * fieldCount] = 0;
                }
                info[15 + i * fieldCount] = getWithdrawableDeposit(user, i);
                info[16 + i * fieldCount] = getUserRoiBusd(user, i);
                info[17 + i * fieldCount] = getRoiPercentPerYear(user, i);
            }
        } else {
            info = new uint256[](1);
            info[0] = 0;
