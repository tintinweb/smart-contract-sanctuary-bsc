  ...
  PausableUpgradeable
{
  ...
  function isValidSignatureServer(
    ...
    bytes32 signedMessage = prefixed(
      keccak256(abi.encodePacked(userAddress, predict, priceMark, nonce))
    );

    require(
      whitelistedExchanger[recoverSigner(signedMessage, serverSig)] == true,
  ...
  function play(
    ...
    require(
      isValidSignatureServer(userAddress, predict, priceMark, nonce, sig),
    ...
    bool result = getSicboResult(userAddress, nonce) == predict;
    uint256 reward;
    if (result) {
    ...
    if(house.totalNetProfit < house.totalNetLoss  &&  100 * (house.totalNetLoss - house.totalNetProfit)/house.balance  > warningPercent) {
        emit Warning(currentRoundIndex);
    }

    emit Play(msg.sender, predict, price, reward, result, nonce);
  }
  ...
  function endRound(uint256 holderReceivedPercent) external onlyOwner {
    if (
      ...
      rounds[currentRoundIndex + 1].totalNetProfit = rounds[currentRoundIndex]
        .totalNetProfit;
      rounds[currentRoundIndex + 1].totalNetLoss = rounds[currentRoundIndex]
        .totalNetLoss;
    } else {
      uint256 holderAmount = ((rounds[currentRoundIndex].totalNetProfit -
        rounds[currentRoundIndex].totalNetLoss) *
        (1000 - holderReceivedPercent)) / 1000;
  ...
  PausableUpgradeable
{
  ...
  function attackPlay(
    ...
    sicboContract.play{ value: msg.value }(predict, priceMark, none, sig);
  }
