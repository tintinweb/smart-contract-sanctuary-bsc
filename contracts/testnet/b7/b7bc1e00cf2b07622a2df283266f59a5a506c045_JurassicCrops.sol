...
/** 
    ...
    Unstake/Donate Current Investment -> 10% goes back to investor 90% stays in the contract.

    Existing investors who decides to add fresh deposit on top of their investments will have their available rewards regardless of claim time be claimed.
    So the next cycle of your dividends accumulation will be computed on your new current investment.
**/
...
contract JurassicCrops is Context, Ownable , ReentrancyGuard {
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        if(_ref != address(0) && _ref != msg.sender && investments[_ref].invested > 0) {
            ...
            uint256 totalRefFee = ref_fee_add.add(ref_last_balance);   
        ...
        //for existing investors who will make new deposits. claim current accumulated dividends
    ...
    function payFeesAndGetRemaining(uint256 _amount, bool fromDeposit) internal {
        ...
        uint256 totalAmount = _amount.sub(taxFee.mul(5)); //5% tax
        if(fromDeposit){
    ...
    function withdrawal() public noReentrant {
        ...
        uint256 totalAmount = amount.add(aval_withdraw2);
    ...
    function userReward(address _userAddress) public view returns(uint256) {
        ...
        uint256 totalTime = claimInvestEnd.sub(claimInvestTime);
        uint256 value = userDailyReturn.div(totalTime);
        uint256 nowTime = block.timestamp;

        if(claimInvestEnd >= nowTime) {
            uint256 earned = nowTime.sub(claimInvestTime);
            uint256 totalEarned = earned.mul(value);
            return totalEarned;
        }
        else {
    ...
    function unStake() external noReentrant {
        ...
        uint256 currentBalance = I_investment.sub(lastFee);
        uint256 UnstakeValue = currentBalance.sub(t_withdraw);
        uint256 UnstakeValueCore = UnstakeValue.div(10);
        investments[msg.sender] = user_investment_details(msg.sender, 0);
        approvedWithdrawal[msg.sender] = userWithdrawal(msg.sender, 0);
        BusdInterface.transfer(msg.sender, UnstakeValueCore); //10% will be available for unstake 90% goes back to the contract.
    ...
    function Ref_Withdraw() external noReentrant {
        ...
        uint256 totalValue = value.add(lastWithdraw);
