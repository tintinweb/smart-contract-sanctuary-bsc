...
contract Guess {
    ...
    mapping(bytes32 => mapping(uint256 => mapping(uint256 => UserGuessDataAndTimeForOrder)))
        private userGuessData; //matchid =>tokenid=>guessTime(orderid)
    ...
    mapping(bytes32 => bool) public isSetMatchResult;
    ...
    mapping(uint256 => uint256) public NFTFirstuseTime; //public
    ...
    function setMatchData(
        ...
        require(!matchData[matchId].islocked, "match is locked");
        require(
            matchData[matchId].matchTime == 0 ||
                matchData[matchId].matchTime - block.timestamp > 10 * 60,
            "begintime is less than 10 minutes"
        );
    ...
    function setUserGuessData(
        ...
        require(!isStopGame, "code0:game is stop");
        ...
        userGuessData[matchId][tokenId][guessTime] = _userGuessDataForOrder;
    ...
    function forceUnLockNFT() external {
        ...
        require(
            isNFTlocked[tokenId] &&
                block.timestamp - NFTorderTime[tokenId] <
                _tokenMeta.lockTime_h * 3600,
            "code009:NFT status is unlocked"
        );
        require(
            block.timestamp - matchBeginTime[tokenId] > 0,
            "code010:time of force unlock  is not reach"
        );
        ...
        uint256 allowance = guessToken.allowance(msg.sender, address(this));
        require(allowance >= forceMount, "code102:Check the token allowance");
    ...
    function reStore() external {
        ...
        require(availableRate < 100, "code301:glasses NFT not need to reStore");
        TokenMeta memory _tokenMeta = _tokenMetas[nftTypeId];
        ...
        uint256 allowance = guessToken.allowance(msg.sender, address(this));
        require(allowance >= restoreToken, "code102:Check the token allowance");
    ...
    function guessBalanceOf() public view returns (uint256) {
        return guessToken.balanceOf(address(this));
    }
        ...
        returns (UserGuessDataAndTimeForOrder[] memory orders, uint256 length)
    {
        ...
        require(isExist, "not have GuessGlass NFT");
    ...
    function userCheckResult(bytes32 matchId, uint256 orderid) external {
        require(
            isSetMatchResult[matchId],
            "code410:please wait,the result is up to block"
        );
        (
            bool isExist,
            uint256 tokenId,
        ...
        require(
            ...
            "code411:you no guess this match"
        );
        require(isContract(msg.sender), "code000:you no guess this match");
        ...
        if (checkOrder(matchResult[matchId], guessId)) {
        ...
        onlyAdmins
    {
        ...
        require(matchData[matchId].times > 0, "code404:matchData is null");
        ...
        returns (HtmlUserinfo memory userinfo)
    {
        ...
        _userinfo.balance = balanceAgent[msg.sender];
        return _userinfo;
    }
    ...
    modifier onlyAdmins() {
        require(msg.sender == owner || isAdmin[msg.sender], "You not Admins");
        _;
    }

    function changeOwner(address newOwner) external onlyOwner {
        require(newOwner != address(0), "address is invaild");
        owner = newOwner;
    }
}
