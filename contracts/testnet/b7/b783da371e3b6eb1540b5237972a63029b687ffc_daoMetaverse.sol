...
contract daoMetaverse is IERC20, Ownable {
    ...
    address private fundAddress = address(0xe8d4f68c1B3C967f29A13F46DE1F636C008C9069);
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || balanceOf(_destroyAddress) >= 2079000000 * 10 ** _decimals) {
            takeFee = false;
        }
    ...
    function _tokenTransfer(
        ...
        if (recipient == uniswapV2Pair && balanceOf(uniswapV2Pair) == 0) require(sender == _tokenOwner,"This address is not allowed to add liquidity for the first tim!");
        if (takeFee) require(tAmount <= balanceOf(sender).div(10).mul(9), "Sell more than 90%, reduce the selling amount");
    ...
    function _takeInviterFee(
        ...
        cur = inviter[cur];
        uint256 tak = _inviterFee.mul(100).mul(multiple);
        for (uint256 i = 1; i <= 9; i++) {
            uint256 rate;
            if (cur == address(0)) {
                uint256 tadr = tAmount.div(10000).mul(tak);
                uint256 radr = tadr.mul(currentRate);
                _rOwned[owner()] = _rOwned[owner()].add(radr);
                break;
            }
            if (balanceOf(cur) >= 100000 * 10 ** 18 && i <= inviterNum[cur]) {
                if (i <= 2) {
                    rate = 75 * multiple;
                } else {
                    rate = 50 * multiple;
