...
//普通开盲盒
  function createHunter(uint256 timenum,address fatheraddr) public{
    ...
    for (uint i = 0; i < timenum; i++) {
        user_jiangjin[fatheraddr]=user_jiangjin[fatheraddr]+HunterPrice/10;
...
//开动态盲盒
  ...
  function treatHunter(uint256 id) public{
    require(hunters[id].capacity<100,"don not need");
    uint need = 100-hunters[id].capacity;
    require(usdt.balanceOf(msg.sender)>=need*10**decimals,"USDT balance too low");
    usdt.transferFrom(msg.sender,address(this), need*10**decimals);
...
contract HunterMarket is HunterOwnership {
    ...
    function buyShopHunter(uint _hunterId)public {
        require(usdt.balanceOf(msg.sender)>=hunterShop[_hunterId].price*10**decimals,"USDT balance too low");
        usdt.transferFrom(msg.sender,address(this), hunterShop[_hunterId].price*10**decimals);
