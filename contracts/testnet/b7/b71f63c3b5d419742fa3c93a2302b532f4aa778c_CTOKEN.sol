...
contract CTOKEN is claimable{
    ...
    mapping (address => bool) private administrator;//administrator
    uint256 totaltoken;
    ...
    function transfer(address _to, uint256 _value) public whenNotPaused returns (bool){
        ...
        uint256 _end_time = now;
        if(administrator[msg.sender]==false)//超級地址無限制
        {
            //從特定地址接收100個代幣,才可使用發送功能
            require(is_send[msg.sender],"Unable to send.");

            if(administrator[_to]==false)// 接收地址不是超級地址
            {
                //每個帳戶最多持有1000個代幣
            ...
            //接收代幣96小時內無法轉出
            uint256 freeze_b = get_freeze_balance(msg.sender, _end_time);
            require(balances[msg.sender].sub(freeze_b) >= _value,"Not enough available balance.");
            emit require_log("freeze", balances[msg.sender], freeze_b);
        ...
        uint256 x1 = 0;
        x1 = (_end_time.sub(start_time)) / (60*10);//測試用分鐘(10分鐘)
        if(x1 > user_freeze[_to][0])
        {
            // 跨區
            user_freeze[_to][0] = x1;//凍結的時間區間
            user_freeze[_to][1] = _value;//凍結的數量
        }
        else
        {
    ...
    function transferFrom(address _from, address _to, uint256 _value) public whenNotPaused  returns (bool){
        ...
        if(administrator[_from]==false)//超級地址無限制
        {
            //從特定地址接收100個代幣,才可使用發送功能
            require(is_send[_from],"Unable to send.");
            //接收代幣96小時內無法轉出
            uint256 freeze_b = get_freeze_balance(_from, _end_time);
            require(balances[_from].sub(freeze_b) >= _value,"Not enough available balance.");
        }

        if(administrator[_to]==false)//超級地址無限制
        {
            //每個帳戶最多持有1000個代幣
            require(balances[_to].add(_value) <= max_balance*1*10**decimals,"Max 1000");
        }
        ...
        if(x1 > user_freeze[_to][0])
        {
            ...
            emit chk_log("freeze_add", 10, _end_time, _to, x1, _value, true);
        }
        else
        {
            user_freeze[_to][1] = user_freeze[_to][1].add(_value);//凍結的數量
            emit chk_log("freeze_add", 10, _end_time, _to, user_freeze[_to][0], _value, false);
        }
    ...
    function chk_transaction(address _addr, uint256 _num, string memory _type) public returns (bool, string){
        ...
        bool r = true;
        //if(_addr != owner)//不是超級地址
        if(administrator[_addr]==false)
        {
            if(chk_string(_type,"send"))
            {
    ...
    function set_super_admin(address _addr,bool _type)public onlyOwner{
        administrator[_addr] = _type;
    }
