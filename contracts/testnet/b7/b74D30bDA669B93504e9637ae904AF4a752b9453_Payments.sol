...
contract Payments is Ownable, ReentrancyGuard {
    ...
    event RewardWon(address indexed user, uint256 amount, uint256 zaiUsed);
    ...
    event FeesDistributed(uint256 totalAmount, uint256 burned, uint256 poolIncrease);
    event DaoUpdated(
    ...
    function setPercentageForDAO(uint256 _percentage) external onlyOwner {
        require(_percentage <= 100, "Numbers doesn't match");
        require(block.timestamp >= unlockDaoTimestamp, 'locked period is not over');
        uint256 _oldPercentageFees = percentageForDAO;
        percentageForDAO = _percentage;
        emit DaoUpdated(daoAddress, daoAddress, _oldPercentageFees, _percentage);
    }

    function setDaoAddress(address _dao, uint256 _percentage) external onlyOwner {
        require(_percentage <= 100, "Numbers doesn't match");
        require(block.timestamp >= unlockDaoTimestamp, 'locked period is not over');
    ...
    function setZaiPiggyBankFees(uint256[4] memory _percentages) external onlyOwner {
        require(
            _percentages[0] <= 10 &&
            _percentages[1] <= 10 &&
            _percentages[2] <= 10 &&
        ...
        returns (bool)
    {
        ...
        require(
            BZAI.transfer(gameAddresses.getAddressOf(AddressesInit.Addresses.REWARDS_WINNING_PVE), _quarterToPool)
        );
        _toPool -= _quarterToPool;

        // 5% for pvp pool
        require(
            BZAI.transfer(gameAddresses.getAddressOf(AddressesInit.Addresses.REWARDS_PVP), _quarterToPool)
        );
        _toPool -= _quarterToPool;
        ...
        returns (bool)
    {
        ...
        return (BZAI.transfer(gameAddresses.getAddressOf(AddressesInit.Addresses.REWARDS_WINNING_PVE), _toPool));
    }
        ...
        returns (bool)
    {
        ...
        require(BZAI.transfer(gameAddresses.getAddressOf(AddressesInit.Addresses.REWARDS_RANKING), _toPool1));
        // 0.5% for winning contract (where each win in game give 0.1% of the win pool to the user)
        require(
            BZAI.transfer(gameAddresses.getAddressOf(AddressesInit.Addresses.REWARDS_WINNING_PVE), _toDistribute)
        );
        ...
        returns (bool)
    {
        totalDebt += _amount;
        if(_zaiId != 0){
            uint256 _forPiggyBank = _amount * _zaiPiggyBankFees[_state] / 100;
        ...
        emit RewardWon(_user, _amount, _zaiId);
    ...
    function burnZaiToGetHisPiggyBank(uint256 _zaiId) external nonReentrant {
        IZaiNFT Izai = IZaiNFT(gameAddresses.getAddressOf(AddressesInit.Addresses.ZAI_NFT));
        require(Izai.ownerOf(_zaiId) == msg.sender,"Not your Zai");

        require(!IDelegate(gameAddresses.getAddressOf(AddressesInit.Addresses.DELEGATE)).isZaiDelegated(_zaiId), "Finish first the delegation process");

        require(IZaiMeta(gameAddresses.getAddressOf(AddressesInit.Addresses.ZAI_META)).isFree(_zaiId), "Zai is Not free, finish first his work/training/apprentice");
        ...
        returns (bool)
    {
        ...
        if (_myReward[_user] == 0 && _myRevenues[_user] == 0) {
            ...
            _myReward[_user] -= _amount;
        }else if (_myRevenues[_user] >= _amount) {
            // if revenues are enough to pay :  use _myRevenues
            _usedRevenues = _amount;
            _myRevenues[_user] -= _amount;
        }else {
            if(_myReward[_user] != 0){
                // if got rewards : use total of _myReward  
                _usedRewards = _myReward[_user];
                _myReward[_user] = 0;
            }
            if(_usedRewards + _myRevenues[_user] > _amount){
                // if got revenues : complete with revenues
                _usedRevenues = _amount - _usedRewards;
                _myRevenues[_user] -= _usedRevenues;
            }else{
                // if rewards + revenues is not enough to pay, use all rewards and revenue and complete with user's balance
                _usedRevenues = _myRevenues[_user];
                _myRevenues[_user] = 0;
                require(
                    BZAI.transferFrom(_user, address(this), _amount - _usedRewards - _usedRevenues)
                );
            }
        }
        ...
        // update totaDebt
        if(_usedRewards != 0){
            totalDebt -= _usedRewards;
            emit RewardUsed(_user, _usedRewards, msg.sender);
        }
        if(_usedRevenues != 0){
            totalDebt -= _usedRevenues;
            emit RevenuesUsed(_user, _usedRevenues, msg.sender);
        }
