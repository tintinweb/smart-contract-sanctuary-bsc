...
contract BlockstarLockupStaking is Ownable {
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > userdata[i].endtime && userdata[i].amount > 0 ){
                total += getCompoundedInterest(userdata[i].amount , userdata[i].returnPer * 10**18 , userdata[i].lockupDuration) -  userdata[i].amount;
            }
        }
        return total;
    }
    ...
    function withdraw() external {
        ...
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > userdata[i].endtime && userdata[i].amount > 0){
                uint256 totalAmount =  getCompoundedInterest(userdata[i].amount , userdata[i].returnPer * 10**18 , userdata[i].lockupDuration);
