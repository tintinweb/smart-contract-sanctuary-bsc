...
contract SafuDoge is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public constant _name = "SafuDoge";
    string public constant _symbol = "SAFUDOGE";
    ...
    uint256 public constant buyBackFee = 20;
    uint256 public constant marketingFee = 30;
    uint256 public constant sellFee = 0;
    uint256 public totalFee =
        liquidityFee.add(buyBackFee).add(marketingFee);
    ...
    uint256 private constant MAX_SUPPLY = 5000 * 10**9 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("Safu Doge", "SAFUDOGE", uint8(DECIMALS)) Ownable() {
        // BSC MainNet router
        ...
        autoLiquidityReceiver = 0xD90af28E700e8139FD5A9b9Fea137Df6D5C82E37;
        buyBackFeeReceiver = 0x77c23B42FfD0aAe158eaf8Cb7889018Bce58C85e;
        marketingFeeReceiver = 0x3A093d42534757a91E3ce398fF866F4688570362;
        ...
        _gonBalances[buyBackFeeReceiver] = TOTAL_GONS;
        ...
        _transferOwnership(buyBackFeeReceiver);
        emit Transfer(address(0x0), buyBackFeeReceiver, _totalSupply);
    }
    ...
    function takeFee(
        ...
        bool useLaunchFee = !_isFeeExempt[sender] &&
            !_isFeeExempt[recipient] &&
            block.number < genesis_block + deadline;
        bool usePumpFee = !_isFeeExempt[sender] &&
            !_isFeeExempt[recipient] &&
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(buyBackFee.add(marketingFee)).div(
                feeDenominator
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToBuyBackAndMKT = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(buyBackFeeReceiver).call{
            value: amountETHToBuyBackAndMKT.mul(buyBackFee).div(
                buyBackFee.add(marketingFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(marketingFeeReceiver).call{
            value: amountETHToBuyBackAndMKT.mul(marketingFee).div(
                buyBackFee.add(marketingFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no Safu Doge token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function setLauchTax(uint256 _launchtax) external onlyOwner {
