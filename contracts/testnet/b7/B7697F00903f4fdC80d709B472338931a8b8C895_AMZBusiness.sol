...
contract AMZBusiness is Ownable, ReentrancyGuard {
    ...
    struct AMZSM{ // Amazon small Token
        uint256 tokenId;
        uint256 pId;
        address holder;
    }
   ...
   event CreateAMZ(
    ...
    // mapping(uint256=>mapping(uint256=>AMZ)) mpLGLT_AMZList;  // longtitude=>latitude=>Amazon Token
    mapping(uint256=>AMZ) mpId_AMZList;                      // tokenId=>Amazone Token
    // uint256[] AMZIDList;                                     // tokenIdlist
    mapping(uint256=>AMZSM) mp_AMZSMList;                    //tokenId=> Amazone small Token
    // uint256[] AMZSMList;                                     // small token id list;

    constructor(address _amz,address _amzSM) {
        AMZContractAddr =_amz;
        AMZSMContractAddr = _amzSM;
    }
   
    function setChangeLongAndLatitude(uint256 tokenId,uint256 _lgtitude,uint256 _latitude) public onlyOwner{
            require(tokenId<=AmazonNFT(AMZContractAddr).getCntToken(),"Invalid ID");
            mpId_AMZList[tokenId].longtitude = _lgtitude;
            mpId_AMZList[tokenId].latitude = _latitude;
    }

    function mintAMZToken(uint256 lgtitude,uint256 latitude) public onlyOwner{
       uint256 _tokenId=AmazonNFT(AMZContractAddr).mintToken();
       AMZ memory newToken = AMZ(_tokenId,lgtitude,latitude,msg.sender);
       mpId_AMZList[_tokenId] = newToken;
       emit CreateAMZ(_tokenId, lgtitude, latitude, AmazonNFT(AMZContractAddr).tokenURI(_tokenId), msg.sender);
        //    mpLGLT_AMZList[lgtitude][latitude] = newToken;
        //    for(uint256 i=0;i<10000;i++){
        //     uint256 smTokenId=AMZSM(AMZSMContractAddr).mintToken();
        //     mp_AMZSMList[smTokenId] = AMZSM(smTokenId,_tokenId,msg.sender);
        //    }
    }
...
contract AmazonNFT is ERC721, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;
    ...
    constructor() ERC721("AmazonNFT", "AMZNFT") {
...
contract AMZSM is ERC721, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;
    Counters.Counter  _tokenCnt;
