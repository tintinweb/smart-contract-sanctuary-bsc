    ...
    ReentrancyGuardUpgradeable
{
    ...
    function mergeItems(MergeItemsStruct calldata data)
        public
        isBanned(_msgSender())
        whenNotPaused
    {
        // Make sure signature is valid and get the address of the signer
        address signer = _verifyMergeItems(data);
        // Make sure that the signer is authorized to withdraw an item
        require(signer == operator, "Signature invalid or unauthorized");
        ...
        // Pay tokens
        require(data.paymentAddresses.length == data.paymentAmounts.length, "Invalid payment arrays");
        for (uint i = 0; i < data.paymentAddresses.length; i++) {
            IERC20Upgradeable(data.paymentAddresses[i]).safeTransferFrom(
                _msgSender(),
                address(this),
                data.paymentAmounts[i]
            );
        }

        // Burn NFTs
        require(data.tokenIds.length > 1, "Invalid amount of items");
        for (uint i = 0; i < data.tokenIds.length; i++) {
            INFT(nftAddress).burn(
                data.tokenIds[i]
            );
        }

        // Mint new merged item
        uint256 newId = INFT(nftAddress).mintFromGameAndBringToGame(
            _msgSender(),
            data.id,
            data.itemType,
            data.extraType
    ...
    function _verifyMergeItems(MergeItemsStruct calldata data)
        internal
        view
        returns (address)
    {
        bytes32 digest = _hashMergeItems(data);
        return ECDSAUpgradeable.recover(digest, data.signature);
    }

    function _hashMergeItems(MergeItemsStruct calldata data)
        internal
        view
        returns (bytes32)
    {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "MergeItemsStruct(address walletAddress,string id,string itemType,string extraType,uint256[] tokenIds,address[] paymentAddresses,uint256[] paymentAmounts,string nonce)"
                        ),
                        _msgSender(),
                        keccak256(bytes(data.id)),
                        keccak256(bytes(data.itemType)),
                        ...
                        keccak256(abi.encodePacked(data.paymentAddresses)),
                        keccak256(abi.encodePacked(data.paymentAmounts)),
                        keccak256(bytes(data.nonce))
                    )
                )
            );
