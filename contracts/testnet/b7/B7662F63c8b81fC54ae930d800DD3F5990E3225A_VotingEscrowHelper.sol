...
contract VotingEscrowHelper is IVotingEscrowCallback, Ownable {
    ...
    function addRebatePool(address _rebatePool) public onlyOwner isContract(_rebatePool) {
        rebatePools.push(IVotingEscrowCallback(_rebatePool));
    }

    function replacePoolAtWith(uint256 index, address _rebatePool) public onlyOwner isContract(_rebatePool) {
        require(index<0 || index >= rebatePools.length,"Wrong argument" );
        require(_isPoolExisted(_rebatePool),"Pool is already existed");
        rebatePools[index] = IVotingEscrowCallback(_rebatePool);
    }

    function _isPoolExisted(address _rebatePool) private view returns (bool){
        for(uint256 i =0 ; i< rebatePools.length ; i++){
            if(rebatePools[i] == IVotingEscrowCallback(_rebatePool)){
                return true;
            }
        }
        return false;
    ...
    ProxyUtility
{
    ...
    function totalSupply() external view override returns (uint256) {
        ...
        if (weekCursor < currentWeek) {
            ...
            for (; weekCursor < currentWeek; weekCursor += 600) {
                ...
                newNextWeekSupply = newNextWeekSupply.sub(newTotalLocked.mul(600) / maxTime);
            }
            newTotalLocked = newTotalLocked.sub(scheduledUnlock[weekCursor]);
    ...
    function createLock(uint256 amount, uint256 unlockTime) external nonReentrant whenNotPaused {
        _assertNotContract();
        require(
            unlockTime + 600 == _endOfWeek(unlockTime),
    ...
    function _totalSupplyAtTimestamp(uint256 timestamp) private view returns (uint256) {
        ...
        uint256 total = 0;
        for (; weekCursor <= timestamp + maxTime; weekCursor += 600) {
    ...
    ///      - `newUnlockTime + 600 == _endOfWeek(newUnlockTime)`, i.e. aligned to a trading week
    ///
    ...
    function _checkpoint(
        ...
        uint256 newNextWeekSupply = nextWeekSupply;
        if (weekCursor < currentWeek) {
            for (uint256 w = weekCursor + 600; w <= currentWeek; w += 600) {
    ...
    function calibrateSupply() external {
        uint256 nextWeek = checkpointWeek + 600;
...
/*
    ...
    function _endOfWeek(uint256 timestamp) internal pure returns (uint256) {
        return ((timestamp.add(600) - SETTLEMENT_TIME) / 600) * 600 + SETTLEMENT_TIME;
    }
*/

    uint256 internal constant DURATION = 600;
