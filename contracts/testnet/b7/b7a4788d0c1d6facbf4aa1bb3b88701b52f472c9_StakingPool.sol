...
// File: contracts/Remix/StakingPool.sol
	...
	function earned(address account) public view returns (uint256) {
		...
		return
			pumpkins[account].add(
				blockTime.sub(lastUpdateTime[account]).mul(1e18).div(86400).mul(balanceOf(account).div(100 * 1e8))
			);
	}

	// stake visibility is public as overriding SotaTokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		require(amount.add(balanceOf(msg.sender)) <= 1000 * ZOOM_SOTA, "Cannot stake more than 5 meme");
