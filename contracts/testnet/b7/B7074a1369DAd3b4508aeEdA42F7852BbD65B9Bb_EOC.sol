...
contract EOC is ERC20Detailed, Ownable {
    ...
    uint256 public developmentAndMarketingFee = 10;
    uint256 public busdDividendFee = 70;
    uint256 public nftLotteryFee = 5;
    uint256 public autoFirePitFee = 5;
    uint256 public totalFee =
        liquidityFee.add(developmentAndMarketingFee).add(busdDividendFee).add(autoFirePitFee).add(nftLotteryFee);
    ...
    address public developmentAndMarketingReceiver;
    address public nftLotteryReceiver;
    ...
    constructor() ERC20Detailed("EOC", "EOC", uint8(DECIMALS)) Ownable() {
        ...
        developmentAndMarketingReceiver = 0x7dFF9A370d101B5dC0816b8e61276477449a9E05;
        nftLotteryReceiver = 0x41BDcADA2A5018b0F95dC219971cF171F2799A11;
        ...
        isDividendExempt[developmentAndMarketingReceiver] = true;
        isDividendExempt[nftLotteryReceiver] = true;
        isDividendExempt[autoLiquidityReceiver] = true;

        totalSupply = INITIAL_FRAGMENTS_SUPPLY;
        gonBalances[developmentAndMarketingReceiver] = TOTAL_GONS;
        gonsPerFragment = TOTAL_GONS.div(totalSupply);
        lastRebasedTime = block.timestamp;
        autoRebase = false;
        autoAddLiquidity = true;
        isFeeExempt[developmentAndMarketingReceiver] = true;
        isFeeExempt[nftLotteryReceiver] = true;
        isFeeExempt[autoLiquidityReceiver] = true;
        isFeeExempt[address(this)] = true;

        _transferOwnership(developmentAndMarketingReceiver);
        emit Transfer(address(0x0), developmentAndMarketingReceiver, totalSupply);
    }
     ...
     * Rebase 0.0025% Every 15 minutes and 0.24% daily
     */
    ...
    function takeFee(
        ...
        gonBalances[address(this)] = gonBalances[address(this)].add(
            gonAmount.mul(developmentAndMarketingFee).div(feeDenominator)
        );
        gonBalances[address(this)] = gonBalances[address(this)].add(
            gonAmount.mul(busdDividendFee).div(feeDenominator)
        );
        gonBalances[address(this)] = gonBalances[address(this)].add(
            gonAmount.mul(nftLotteryFee).div(feeDenominator)
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 feeTotal = developmentAndMarketingFee.add(nftLotteryFee).add(busdDividendFee);

        (bool success, ) = payable(developmentAndMarketingReceiver).call{
            value: amountETHToRespectiveReceiver.mul(developmentAndMarketingFee).div(feeTotal),
            gas: 30000
        }("");

        (success, ) = payable(nftLotteryReceiver).call{
            value: amountETHToRespectiveReceiver.mul(nftLotteryFee).div(feeTotal),
            gas: 30000
        }("");

        try distributor.deposit {
            value: amountETHToRespectiveReceiver.mul(busdDividendFee).div(feeTotal)
        } () {} catch {}        
    }
