...
contract BondFarmingPool is Pausable, ReentrancyGuard, Ownable, IBondFarmingPool {
    ...
    function setSiblingPool(BondFarmingPool siblingPool_) public onlyOwner {
        require(
    ...
    function totalPendingRewards() public view virtual returns (uint256) {
        int256 remoteTotalPendingRewards = bond.totalPendingRewards();
        ...
        // return
        //     (uint256(remoteTotalPendingRewards) * (bondToken.balanceOf(address(this)) / SHARE_PRECISION)) /
        //     (bondToken.totalSupply() / SHARE_PRECISION);

        return DuetMath.mulDiv(uint256(remoteTotalPendingRewards), poolBalance, bondToken.totalSupply());

        // ((uint256(remoteTotalPendingRewards) * (bondToken.balanceOf(address(this)) / 1e12)) /
        //    (bondToken.totalSupply() / 1e12));
    }
    ...
    function pendingRewardsByShares(uint256 shares_) public view returns (uint256) {
        ...
        // return (totalPendingRewards() * shares_) / totalShares;
    ...
    function sharesToBondAmount(uint256 shares_) public view returns (uint256) {
        ...
        // return (underlyingAmount() * shares_) / totalShares;
    ...
    function amountToShares(uint256 amount_) public view returns (uint256) {
        ...
        return
            totalShares > 0 // ? DuetMath.mulDiv(amount_, totalShares, underlyingAmount())
                ? DuetMath.mulDiv(amount_, totalShares, underlyingAmount())
                : amount_; // DuetMath.mulDiv(amount_, totalShares, underlyingAmount()) : underlyingAmount();
    }
...
contract ExtendableBond is ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
     ...
     */
    uint16 public immutable PERCENTAGE_FACTOR = 10000;
    BondFarmingPool bondFarmingPool;
    BondFarmingPool bondLPFarmingPool;
    /**
    ...
    function totalPendingRewards() public view returns (int256) {
        return int256(totalUnderlyingAmount()) - int256(totalBondTokenAmount());
    }
    ...
    function updateCheckPoints(CheckPoints calldata checkPoints_) public onlyAdmin {
    ...
    function setRedeemable(bool redeemable_) external onlyAdmin {
    ...
    function setConvertable(bool convertable_) external onlyAdmin {
     ...
     */
    function withdrawRemoteUnderlyingTokens(bool makeRedeemable_) public onlyAdmin {
        checkPoints.convertable = false;
    ...
    function extendBond(CheckPoints calldata checkPoints_) public onlyAdmin {
    ...
    // function allocateFees() external onlyAdmin {
...
contract MultiRewardsMasterChef is ReentrancyGuard {
    ...
    function setRewardSpec(
        ...
        rewardSpec.token.safeTransferFrom(
            ...
            (endedAtBlock - startedAtBlock) *
                rewardPerBlock -
                (rewardSpec.token.balanceOf(address(this)) - rewardSpec.claimedAmount)
        );
...
contract MockExtendableBond is IExtendableBond {
    ...
    function totalPendingRewards() public view returns (int256) {
        ...
        return int256(((block.number - startBlock) * 40 * 1e18) - mintedRewards);
    }
...
contract BondLPFarmingPool is Pausable, ReentrancyGuard, Ownable, IBondFarmingPool {
    ...
    function totalPendingRewards() public view virtual returns (uint256) {
        int256 totalBondPendingRewards = bond.totalPendingRewards();
        if (totalBondPendingRewards <= 0) {
            return 0;
        }
        return uint256(bond.totalPendingRewards());
    }
