...
// File: interfaces/ITokenLocker.sol
...
// File: interfaces/IPancakeSwapRouter.sol
...
contract FlameToken is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if (pairs[_from] && routerExists && !swapping) {
            // BUY
            taxAmount = ((buyLiqTax + buyBurnTax) * amount) / 100;
            burnAmount = (taxAmount * buyBurnTax) / (buyLiqTax + buyBurnTax);
        ...
        } else if (pairs[_to] && routerExists && !swapping) {
            // SELL
            taxAmount = ((sellLiqTax + sellBurnTax) * amount) / 100;
            burnAmount = (taxAmount * sellBurnTax) / (sellLiqTax + sellBurnTax);
            liqAmount = taxAmount - burnAmount;
            super._burn(_from, burnAmount);
            super._transfer(_from, address(this), liqAmount);
            swapAndLiquify(liqAmount);
