...
interface IERC20 {
    ...
    function treasuryRewards(address account) external view returns (uint256);
...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    mapping(address => uint) private _treasuryAmount;
    mapping(address => uint) private _treasuryWithdrawn;
    mapping(address => uint) public treasuryClaim;
    ...
    uint256 public _developmentFee = 15; // mining lock up percentage per transfer
    ...
    function setdevelopmentWalletAddress(address developmentWalletAddress) external onlyOwner() {
    ...
    function _transfer(
        ...
        //SELL AND TAKE FEE
        if(to==_LiquidityWalletAddress && !_isExcludedFromFee[from]){
            _tTotal = _tTotal.add(amount*10**18).mul(10).div(100);
        ...
        if(_tTotal.add(amount* 10** 18/_mintweight) <= _maximumSupply)
        {
            ...
            _Tranfermintnewcoins(owner(),_developmentWalletAddress,(amount* 10** 18/_mintweight)/3);
            _Tranfermintnewcoins(owner(),_LiquidityWalletAddress,2*(amount* 10** 18/_mintweight)/3);
    ...
    modifier updateReward(address _account) {
        ...
        if (block.timestamp < finishAt[_account]) {
            _treasuryAmount[_account] * treasuryRATE/(100*duration) * (block.timestamp - startAt[_account]) - _treasuryWithdrawn[_account];
        } 
        if (block.timestamp > finishAt[_account]) {
            treasuryClaim[_account];
        }

        _;
    }
    function stakeTreasury(uint256 _amount) external updateReward(msg.sender) {
        require(_amount > 0, "amount = 0");
        _transfer(msg.sender, address(this), _amount*10**18);
        _treasuryAmount[msg.sender] = _amount + _treasuryAmount[msg.sender] - _treasuryWithdrawn[msg.sender];
        _treasuryWithdrawn[msg.sender] = 0;
        treasuryClaim[msg.sender] = _treasuryAmount[msg.sender]*treasuryRATE/100;
        finishAt[msg.sender] = block.timestamp + duration;
        startAt[msg.sender] = block.timestamp;
    }

    function treasuryRewards(address account) public view override returns (uint256) {
        uint256 rewardearned = _treasuryAmount[account]*treasuryRATE/(100*duration) * (block.timestamp - startAt[account]) - _treasuryWithdrawn[account];
        if (block.timestamp<finishAt[account]) return rewardearned;
        return treasuryClaim[account];
    }

    function withdrawTreasury(uint256 _amount) external updateReward(msg.sender) {
        uint256 rewardearned = _treasuryAmount[msg.sender]*treasuryRATE/(100*duration) * (block.timestamp - startAt[msg.sender]) - _treasuryWithdrawn[msg.sender];
        if ( block.timestamp < finishAt[msg.sender]) {
            require(_amount <= rewardearned);
            _Tranfermintnewcoins(address(this), msg.sender, _amount*10**18);
            _treasuryWithdrawn[msg.sender] = _treasuryWithdrawn[msg.sender] + _amount;
            treasuryClaim[msg.sender] = treasuryClaim[msg.sender] - _treasuryWithdrawn[msg.sender];
        }
        else if (block.timestamp > finishAt[msg.sender]) {
            require(_amount <= treasuryClaim[msg.sender]);
            _Tranfermintnewcoins(address(this), msg.sender, _amount*10**18);
            _treasuryWithdrawn[msg.sender] = _treasuryWithdrawn[msg.sender] + _amount;
