...
contract Auction {
    ...
    constructor() {
        highestBid = 0;
        timeAuction = block.timestamp + 50;
        auctionAcceptedToken = address(
            0xf42857DA0Bf94d8C57Bc9aE62cfAAE3722ed9DAb
        );
    }

    function balanceOf() external view returns (uint256) {
        return
            IERC20(auctionAcceptedToken).balanceOf(
                address(0xAA7740DB30dcE972a5F1eFD8970e2D37ADD75034)
            );
    }
    ...
    function bid(string memory _name, uint256 _amount) public {
        ...
        highestBid = _amount;
        pendingReturn[msg.sender] = Bidder(_name, msg.sender, _amount);
        bidders.push(Bidder(_name, msg.sender, _amount));
        emit Bid(_name, msg.sender, _amount);
    }
