...
contract Staking {
    ...
    function stake(uint _cakeAmount) external {
        ...
        user.amount = user.amount.add(_cakeAmount);
    ...
    function getPending(address _account) public view returns(uint256) {
        ...
        uint256 _pending = user.amount.mul(accCakePerShare).div(1e12).sub(user.rewardDebt);
    ...
    function unStake(uint _cakeAmount) external {
        ...
        require(
            _cakeAmount <= userInfo[msg.sender].amount,
        ...
        uint256 __pending = user.amount.mul(pool.accCakePerShare).div(1e12).sub(user.rewardDebt);

        user.amount = user.amount.sub(_cakeAmount);
        user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e12);
