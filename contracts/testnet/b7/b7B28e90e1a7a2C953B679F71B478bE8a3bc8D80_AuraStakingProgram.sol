...
contract AuraStakingProgram  {
           ...
           for (index; index < numberOfDays; index++) {
               if(_dailyInterestRate + index <= _maxDailyInterest ){
                   secondsLeft -= INTEREST_CYCLE;
                     result += (_amount * (_dailyInterestRate + index) / 10000 * INTEREST_CYCLE) / (1);
               }
               else
               {
                 break;
            ...
            result += (((_amount.mul(_dailyInterestRate)).div(10000)) * secondsLeft) / (1);

            return result;

        }else{
            return (_amount * _dailyInterestRate / 10000 * (_now - _start)) / (1);
        }
    ...
    function sendAirdropToken(address _user,uint256 token) public payable
    {
        require(msg.sender==owner,"Only Owner.");
        require(isUserExists(_user),"User Not Exist.");
        AuraToken.transfer(_user,token*1e18);
