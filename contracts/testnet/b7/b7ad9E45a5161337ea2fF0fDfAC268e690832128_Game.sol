...
contract Game is Ownable, ReentrancyGuard {
    ...
    mapping (address => uint256) public userReinvestAmount;
    ...
    uint256 public reinvestInterestRate = 315;
    ...
    uint256 public gracePeriod = 600;
    ...
    function subscribe(uint256 _pid) external nonReentrant {
        ...
        createSubscription(msg.sender, _pid);
    ...
    function reinvest(uint256 _pid) external nonReentrant {
        ...
        require(isActivated(msg.sender), "Account not activated");
        require(totalSubscriptionAmount + packageInfo[_pid].subscribeAmount <= quotaAmount, "Quota limit reached");
        require(userReinvestAmount[msg.sender] >= packageInfo[_pid].subscribeAmount, "Insufficient USDT tokens in reinvest wallet");
        require(MBQ.balanceOf(msg.sender) >= packageInfo[_pid].registrationFee, "Insufficient MBQ tokens");
                
        // Take tax
        uint256 _tax = packageInfo[_pid].subscribeAmount * subscriptionTax / DENOM;
        USDT.safeTransfer(taxReceiver, _tax);    
    
        // Deduct from reinvest
        userReinvestAmount[msg.sender] -= packageInfo[_pid].subscribeAmount;
        ...
        emit Reinvest(msg.sender, _pid, packageInfo[_pid].subscribeAmount);
    }

    function autoSubscribe(address _userAddress, uint256 _pid) external onlyOperator {
        require(_pid < packageInfo.length, "Invalid package ID");
        require(isActivated(_userAddress), "Account not activated");
        require(totalSubscriptionAmount + packageInfo[_pid].subscribeAmount <= quotaAmount, "Quota limit reached");
        require(USDT.balanceOf(_userAddress) >= packageInfo[_pid].subscribeAmount, "Insufficient USDT tokens");
        require(MBQ.balanceOf(_userAddress) >= packageInfo[_pid].registrationFee, "Insufficient MBQ tokens");
        
        // Take tax
        uint256 _tax = packageInfo[_pid].subscribeAmount * subscriptionTax / DENOM;
        USDT.safeTransferFrom(_userAddress, taxReceiver, _tax);

        // Take funds
        USDT.safeTransferFrom(_userAddress, address(this), packageInfo[_pid].subscribeAmount - _tax);

        // Take registration fee
        MBQ.safeTransferFrom(_userAddress, feeReceiver, packageInfo[_pid].registrationFee);

        createSubscription(_userAddress, _pid);

        emit AutoSubscribe(_userAddress, _pid, packageInfo[_pid].subscribeAmount);
    }

    function createSubscription(address _userAddress, uint256 _pid) internal {
        // Create new package profile
        UserInfo storage _userPackageInfo = userPackageInfo[_userAddress][userPackageInfoLength[_userAddress]];
        ...
        _userPackageInfo.expiryTimestamp = block.timestamp + (initialDaysPerCycle * secondsPerDay) + gracePeriod;

        // Update user package count
        userPackageInfoLength[_userAddress]++;
    ...
    function renew(uint256 _sid) external nonReentrant {
        ...
        // Calculate principal + interest to user, based on their previous staked amount
        uint256 _rewards = getPendingRewards(msg.sender, _sid);

        // Calculate interest and add to user's reinvest wallet
        uint256 _reinvestInterestAmt = calculateReinvestInterestAmount(msg.sender, _sid);
        userReinvestAmount[msg.sender] += _reinvestInterestAmt;

        // Update user details
        _userPackageInfo.daysPerCycle = calculateDaysPerCycle(msg.sender, _sid);
        _userPackageInfo.cycle += 1;
        _userPackageInfo.renewTimestamp = block.timestamp;
        _userPackageInfo.expiryTimestamp = block.timestamp + (_userPackageInfo.daysPerCycle * secondsPerDay) + gracePeriod;
    ...
    function adjustUserExpiry(address _userAddress, uint256 _sid, uint256 _expiryTimestamp) external onlyOwner {
    ...
    function calculateReinvestInterestAmount(address _userAddress, uint256 _sid) public view returns(uint256) {
        UserInfo storage _userPackageInfo = userPackageInfo[_userAddress][_sid];
        if(!isRenewable(_userAddress, _sid))
            return 0;

        uint256 _interest = packageInfo[_userPackageInfo.package].subscribeAmount * reinvestInterestRate / DENOM;
    ...
    function setReinvestInterestRate(uint256 _reinvestInterestRate) external onlyOwner {
        require(_reinvestInterestRate > 0, "value must be larger than zero");
        reinvestInterestRate = _reinvestInterestRate;

        emit SetReinvestInterestRate(_reinvestInterestRate);
    }
    ...
    function setGracePeriod(uint256 _gracePeriod) external onlyOwner {
        require(_gracePeriod > 0, "value must be larger than zero");
        gracePeriod = _gracePeriod;

        emit SetGracePeriod(_gracePeriod);
    } 
    ...
    event AutoSubscribe(address userAddress, uint256 pid, uint256 amount);
    event Reinvest(address userAddress, uint256 pid, uint256 amount);
    ...
    event Activate(address _userAddress);
    ...
    event SetReinvestInterestRate(uint256 reinvestInterestRate);
    event SetCycleDivider(uint256 cycleDivider);
    event SetMaxDaysPerCycle(uint256 maxDayPerCycle);
    event SetGracePeriod(uint256 maxDayPerCycle);
