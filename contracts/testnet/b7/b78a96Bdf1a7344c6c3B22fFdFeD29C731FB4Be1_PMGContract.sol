    ...
    */
    ...
    function bidNFT(uint256 _auctionItemId,uint256 _price) public payable {
        ...
        if(auctionitem.cryptoToken == 0xDfb1211E2694193df5765d54350e1145FD2404A1){
            if(balance - freezeBalance< auctionitem.price){
                require((msg.sender).balance >= _price,"insufficient fund");
                uint256 remaining = auctionitem.price - (balance-freezeBalance);
                require(msg.value == remaining,"deposit bnb for bid");
                IStrategy(strategy[auctionitem.cryptoToken]).directDeposit{value:remaining}(_price  ,msg.sender,1);
            }else{
                require(msg.value == 0,"no bnb deposit require");
            ...
            if(balance - freezeBalance< _price){
                ...
                IStrategy(strategy[auctionitem.cryptoToken]).directDeposit(remaining  ,msg.sender,1);
            }else{
                IStrategy(strategy[auctionitem.cryptoToken]).lockFund(_price ,msg.sender);
            }
        }
