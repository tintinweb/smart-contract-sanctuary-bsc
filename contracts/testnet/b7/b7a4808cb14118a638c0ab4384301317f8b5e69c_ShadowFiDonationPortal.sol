...
contract ShadowFiDonationPortal is Ownable, ReentrancyGuard {
    ...
    function donateBNB() payable public nonReentrant {
        ...
        (uint256 amountToken, uint256 amountBNB, uint256 liquidity) = pancakeRouter.addLiquidityETH{value: bnbLoadAmount}(address(shadowFiToken), sumAmount, 0, 0, address(vaultContract), block.timestamp + 120);

        if (sumAmount > amountToken) {
            shadowFiToken.transfer(msg.sender, sumAmount - amountToken);
        }

        if (bnbLoadAmount > amountBNB) {
            payable(msg.sender).transfer(bnbLoadAmount - amountBNB);
        }
