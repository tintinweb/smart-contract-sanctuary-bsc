...
contract LuckyApeCoin is Context, IERC20, Ownable {
    ...
    bool public _preparePublicLaunch = false;
    bool public _startPublicLaunch = false;
    ...
    function lotterize() private view returns(address) {
        if(_addressLists.length >= 1){
		    ...
		    uint256 tokenBalance = _rOwned[_addressLists[randomNumber]];
    ...
    function preparePublicLaunch() public onlyOwner() {
        require(!_preparePublicLaunch, "LuckyApeCoin: function can only be called once");
        ...
        _transactionFees.buyLottoFee = 5;
        _transactionFees.totalSellBuybackMarketingFee = _transactionFees.sellBuybackFee.add(_transactionFees.sellMarketingFee);
        _preparePublicLaunch = true;
    }
    
    function startPublicLaunch() public onlyOwner() {
        require(_preparePublicLaunch, "LuckyApeCoin: beforePublicLaunch function must be called");
        require(!_startPublicLaunch, "LuckyApeCoin: function can only be called once");
        _startPublicLaunch = true;
    }
