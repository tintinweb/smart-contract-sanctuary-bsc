...
contract ZipmexStakingV1 is ReentrancyGuardUpgradeable {
    ...
    struct StakeData {
        ...
        uint256 collectedReward; // total rewards collected
    }
    ...
    function unStake(uint256 vault) external nonReentrant {
        ...
        if (reward > 0) {
            if(reward != stakeData.collectedReward){
                amountToTransfer = stakeData.stakeAmount + (stakeData.collectedReward-reward) - penalty;
                _bonusPoolAmount -= (stakeData.collectedReward-reward);
            }
            else{
    ...
    function claimReward(uint256 vault) external nonReentrant {
        ...
        stakeData.lastClaimTime = block.timestamp;
        stakeData.collectedReward += reward;
        _totalExpectedBonus -= reward;
        _bonusPoolAmount -= reward;

        if(stakeData.collectedReward == reward){
            emit RewardReleased(msg.sender, vault, reward);
            _tokenAddress.safeTransfer(msg.sender, reward);
        }
        else{
            emit RewardReleased(msg.sender, vault, stakeData.collectedReward-reward);
            _tokenAddress.safeTransfer(msg.sender, stakeData.collectedReward-reward);
        }
    }
