...
contract TombGenesisRewardPool {
    ...
    //pool fee
    mapping(address => uint) public poolFee;

    address public treasury;

    // MAINNET
//    uint256 public tombPerSecond = 0.09645 ether; // 25000 TOMB / (72h * 60min * 60s)
    uint256 public tombPerSecond = 0.14467592592 ether; // 25000 TOMB / (48h * 60min * 60s)
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            address _poolTokenAddress = address(pool.token);
            uint256 _tax = 0;
            if(_poolTokenAddress == shiba) {
                (uint256 _fee ,uint _amountAdding) = _calculateTaxFee(100, _amount);
                _tax = _fee;
//                 user.amount = user.amount.add(_amount.mul(9900).div(ONE_HUNDRED_PERCENT));
                user.amount = user.amount.add(_amountAdding);
            } else if(poolFee[_poolTokenAddress] > 0){
//                user.amount = user.amount.add(_amount.mul(poolFee[_poolTokenAddress]).div(ONE_HUNDRED_PERCENT));
                (uint256 _fee ,uint _amountAdding) = _calculateTaxFee(poolFee[_poolTokenAddress], _amount);
                _tax = _fee;
                user.amount = user.amount.add(_amountAdding);
            } else {
                user.amount = user.amount.add(_amount);
            }
            if(_tax > 0 && treasury != address(0)) {
                pool.token.safeTransfer(treasury, _tax);
            }
        }
    ...
    function _calculateTaxFee(uint256 _fee, uint256 _amount) internal pure returns (uint _tax, uint _amountAdding) {
            _tax = _amount.mul(_fee).div(ONE_HUNDRED_PERCENT);
            _amountAdding = _amount - _tax;
    }
    ...
    function addPoolFee(address _token, uint _fee) external onlyOperator {
        require(_fee < ONE_HUNDRED_PERCENT,"must lower than 100%");
