...
//    mapping (uint => uint) memory distributedToLandLord;
//    uint totalDistributedToLandLord;
...
//      distributedToLandLord[rewards[msg.sender][dayToClaim][i].mission] += rewards[msg.sender][dayToClaim][i].landLordShareAmount;
    }
...
//    uint totalMissions = gameProxyContract.totalMissions();
//    gameMiningPool.releaseGameAllocation(address(landLordManager), totalDistributedToLandLord);
//    for (uint i = 0; i < totalMissions; i++) {
//      if(distributedToLandLord[i] > 0) {
//        landLordManager.giveReward(i, distributedToLandLord[i]);
//      }
//    }
  }
  ...
  function updateFarmAddress(address _newAddress) external onlyMainAdmin {
    require(_newAddress != address(0), "Invalid address");
    farm = IFarm(_newAddress);
    fotaToken.approve(_newAddress, type(uint).max);
  }
  ...
  function setContracts(address _heroNft, address _gameMiningPool, address _fotaToken, address _landLordManager) external onlyMainAdmin {
