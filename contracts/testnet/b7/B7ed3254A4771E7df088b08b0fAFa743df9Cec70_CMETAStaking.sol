...
library Address {
    ...
    function verifyCallResultFromTarget(
        ...
        string memory errorMessage
    ) internal view returns (bytes memory val) {
        if (success) {
...
/// @notice CMETA Staking Contract
/// @author 0xhohenheim <[email protected]>
contract CMETAStaking is ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(
        ...
        require(
            _rewardsDuration > 0,
            "CMETAStaking: Duration must be greater 0"
        );
        rToken = _rToken;
        sToken = _sToken;
    ...
    {
        require(amount > 0, "CMETAStaking: Cannot stake 0");
        require((totalStaked + amount) <= cap, "CMETAStaking: Cap reached");
        ...
        updateReward(msg.sender)
    {
        require(
            ...
            "CMETAStaking: Staking period hasn't ended"
        );
    ...
    function setRewardsDuration(uint256 _rewardsDuration) external onlyOwner {
        require(
            ...
            "CMETAStaking: Previous staking period hasn't ended yet"
        );
        duration = _rewardsDuration.mul(1 days);
