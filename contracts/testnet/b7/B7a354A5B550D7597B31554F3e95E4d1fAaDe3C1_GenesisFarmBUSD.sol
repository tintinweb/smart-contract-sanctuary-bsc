...
contract GenesisFarmBUSD {
	...
	uint256 constant public UNSTAKE_FEE = 3000;
	uint256 constant public UNSTAKE_TEAM_DIVIDER = 3;
  ...
  function reinvest() public {
	...
	function unstake() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			if (deposit.plan == 0 && user.checkpoint < finish) {
		...
		} // else amountToWithdraw += 0

		stablecoin.transfer(msg.sender, amountToWithdraw);
		stablecoin.transfer(commissionWallet, min(dividendFee + (unstakeFee / UNSTAKE_TEAM_DIVIDER), getContractBalance()));
	}
