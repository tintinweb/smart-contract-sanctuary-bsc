...
contract EG is Context, IERC20, Ownable, Initializable {
    ...
    /**
     * @dev Function to receive ETH when msg.data is be empty
    ...
    /**
     * @param accounts list of clients to whitelist
     *
     * @dev exclude a wallet from tra fee
     **/
    function addClientsToWhiteList(address[] calldata accounts) external onlyOwner {
    ...
    /**
     * @param accounts list of clients to remove from whitelist
     *
     * @dev include a wallet to pay tax
     **/
    function removeClientFromWhiteList(address[] calldata accounts) external onlyOwner {
        for (uint256 i; i < accounts.length; i++) {
            if (whiteList[accounts[i]]) {
    ...
    /**
     * @param accounts client list to blacklist
     *
     * @dev add clients to black list
     **/
    ...
    /**
     * @param accounts accounts list to blacklist
     *
    ...
    function setMarketingWallet(address _wallet) external onlyOwner {
        require(
            _wallet != address(0),
            "EG: Then wallet should not be the zero address"
        );
    ...
    /**
     * @param _fee, maerketing wallet fee
     *
    ...
    function setLiquidityWallet(address _wallet) external onlyOwner {
        require(
            _wallet != address(0),
            "EG: The wallet should not be the zero address"
        );
     ...
     *
     * @dev set Staking Reward Wallet Address
     **/
    ...
    function _transfer(
        ...
        require(!blackList[from], "EG: transfer from the blacklist address");
        require(!blackList[to], "EG: transfer to the blacklist address");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "ERC20: Transfer amount must be greater than zero");
        require(balances[from] >= amount, "ERC20: insufficient tokens amount");
        require(from != to, "ERC20: Transfer to and from addresses the same");
        require(
            !inTokenCheck(),
        ...
        // is exclude fee
        bool _isNotExcludeFee = !(whiteList[from] ||
            whiteList[to]);
    ...
    /**
     * @dev not Liquidity Add or is owner, clear data from same block to allow balanceOf
     *
     **/
    ...
    function withdrawTokens() external onlyOwner {
        ...
        require(
            marketingWalletFee +
                ...
                100,
            "EG: Total Fees should not bigger than 100."
        );
        require(
     ...
     *
     * @dev You can withdraw tokens sent to the contract address
     **/
     ...
     * @param amount token amount
     *
     * @dev You can withdraw native tokens from the contract address
     **/
