...
*/
  ...
  /*
    ...
    */
    mapping (address => uint256) public indx;
  function buyLotteryTickets(uint256 numberOfTickets, address player) private {
    ...
    if (players[player] == false) {
      if (listOfPlayers.length > _numActivePlayers) {
        ...
        indx[player] = _numActivePlayers;
      } else {
        listOfPlayers.push(player); // otherwise append to array
         indx[player] = listOfPlayers.length;
      }
      players[player] = true;
 ...
 if (players[player] == true) {
   if (tickets[player] == 0){
       if(indx[player] == listOfPlayers.length)
       {
                 delete  listOfPlayers[indx[player]];
        delete indx[player];

       }else{
      delete  listOfPlayers[indx[player]];
      listOfPlayers[indx[player]] = listOfPlayers[listOfPlayers.length];
      delete listOfPlayers[listOfPlayers.length];
        delete indx[player];}
