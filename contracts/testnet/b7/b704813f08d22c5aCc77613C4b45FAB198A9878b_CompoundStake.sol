...
contract CompoundStake is Ownable, Pausable {
	...
	function stake(uint256 _pid, uint256 _amount) external verifyPoolId(_pid) whenNotPaused {
	...
	function harvest(uint256 _pid) external verifyPoolId(_pid) whenNotPaused returns (uint256) {
		PoolInfo storage pool = poolInfo[_pid];
		UserInfo storage user = userInfo[_pid][msg.sender];
