...
contract FeeManagerFacet is IFeeManager, OnlySelf {
    ...
    function feeSummary(address token) external view override returns (FeeSummary memory) {
        return LibFeeManager.feeManagerStorage().feeSummaries[token];
    }
    ...
    function chargeOpenFee(address token, uint256 amount, uint24 broker) external onlySelf override returns (uint24) {
        return LibFeeManager.chargeOpenFee(token, amount, broker);
    }

    function chargeCloseFee(address token, uint256 amount, uint24 broker) external onlySelf override {
        LibFeeManager.chargeCloseFee(token, amount, broker);
    }
}
