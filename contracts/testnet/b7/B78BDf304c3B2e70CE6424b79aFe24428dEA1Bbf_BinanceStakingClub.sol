...
contract BinanceStakingClub is Ownable, ReentrancyGuard {
    ...
    function isEarlyWithdrawal (Staking memory staking) internal view returns (bool) {
        return block.timestamp < staking.stakingTimestamp + (30 minutes) * staking.numberOfMonths; 
    }
    ...
    function isEarlyWithdrawalRequestLocked (Staking memory staking) internal view returns (bool) {
        return block.timestamp < staking.earlyWithdrawalLockedTimestamp + 3 minutes; 
    }

    // Checking for unlock time to pass for early withdrawal

    function isProfitWithdrawalRequestLocked (Staking memory staking) internal view returns (bool) {
        return block.timestamp < staking.profitWithdrawalLockedTimestamp + 1 minutes; 
    }
    ...
    function earlyLockupWithdrawal(uint numberOfStaking) external {
        Staking memory staking = getStakingByNumber(numberOfStaking);
        require(staking.isFinished == false, 'This staking is finished');
    ...
    function profitAmountCalculation(uint numberOfStaking) internal view returns (uint) {
        ...
        uint64 daysDiff = (stakingFinishTimestamp - stakingTimestamp) / 60;
    ...
    // Reffera incone methods

    function witdrawalReferralIncome(address payable recipient, uint256 amount) external nonReentrant onlyOwner {
        require(address(this).balance >= amount,  'Transfer amount exceeds balance');
        require(profile[recipient].availableReferralIncome >= amount,  'Transfer amount exceeds his income!');

        (bool success, ) = recipient.call{value:amount}("");
        require(success, "Transfer failed.");

        profile[recipient].availableReferralIncome = profile[recipient].availableReferralIncome - amount;
    }
