...
contract LootProgress is Ownable{
    ...
    // store array of weekly loot
    struct WeeklyLoot{
        bool claimable;
        bool claimed;
    }

    mapping (address =>mapping(uint256 => WeeklyLoot)) _weeklyLootClaimed;
    mapping (address =>uint256) _lastLootClaimed;


    modifier onlyGame{
        require(msg.sender == gameAddresses.getGameAddress(), "Only game authorized");
        _;
    }
    ...
    function getUserProgress(address _user) external view returns(uint256 lastActionTimestamp,uint256 dayOfWeek,uint256 weekNumber, uint256 lastLootClaimed){
        return _getUserProgress(_user);
    }

    function _getUserProgress(address _user) internal view returns(uint256 lastActionTimestamp,uint256 dayOfWeek,uint256 weekNumber, uint256 lastLootClaimed){
        Progress memory p = _progress[_user];
        uint256 _beginningDay = _getDayBegining();

        if(p.lastActionTimestamp == 0){
            return(0,1,1,0);
        }
        else if(p.lastActionTimestamp >= _beginningDay ){
            return(p.lastActionTimestamp,p.dayOfWeek,p.weekNumber,_lastLootClaimed[_user]);
        }
        else if(_beginningDay - p.lastActionTimestamp <= 1 days){
            return(p.lastActionTimestamp,p.dayOfWeek,p.weekNumber,_lastLootClaimed[_user]);
        }
        else if(_beginningDay - p.lastActionTimestamp <= 7 days ){
            return(p.lastActionTimestamp,0,p.weekNumber,_lastLootClaimed[_user]);
        }
        else if(
                _beginningDay - p.lastActionTimestamp > 7 days &&
                _beginningDay - p.lastActionTimestamp < 14 days
                ){
            return(p.lastActionTimestamp,0,p.weekNumber > 1 ? p.weekNumber - 1 : 1,_lastLootClaimed[_user] );
        }
        else if(_beginningDay - p.lastActionTimestamp >= 14 days){
            return(p.lastActionTimestamp,0,1,_lastLootClaimed[_user]);
        }
    ...
    function _getWeekLootClaimedDatas(address _user, uint256 _weekNumber) internal view returns(bool claimable, bool claimed){
        WeeklyLoot storage w = _weeklyLootClaimed[_user][_weekNumber];
        return (w.claimable, w.claimed);
    }
        ...
        if(p.lastActionTimestamp == 0){
            ...
            if(_beginningDay >= p.lastActionTimestamp){
                if(_beginningDay - p.lastActionTimestamp <= 1 days){
                    p.dayOfWeek += 1;
                    if(p.dayOfWeek == 7){
                        _weeklyLootClaimed[_user][p.weekNumber].claimable = true;
                        p.weekNumber += 1;
                        p.dayOfWeek = 1;
                    }
                }else {
    ...
    // lootType = 0 => potions , 1 => eggs
    function claimLoot(uint256 _weekNumber) external returns(uint256 lootType, uint256[] memory loots){
        WeeklyLoot storage w = _weeklyLootClaimed[msg.sender][_weekNumber];
        require(w.claimable && !w.claimed, "Reward not available");

        w.claimable = false;
        w.claimed = true;
        _lastLootClaimed[msg.sender] = _weekNumber;
    ...
    function _getPotionLoot(address _user, uint256 _numberOfPotions, uint256 _minLevel, uint256 _maxLevel) internal returns(uint256[] memory potions){
        IPotions iPotions = IPotions(gameAddresses.getPotionAddress());

        uint256[8] memory r = _generateRandomDatas(_user);
        uint256[] memory _potions;

        for(uint256 i = 0 ; i < _numberOfPotions ; i++){
            uint256 _power = _minLevel + (r[0] % (_maxLevel - _minLevel)); 
            uint256[7] memory _powers;
            uint256 _indexP;
            uint256 _indexR;
            while(_power > 0){
                _powers[_indexP] += r[_indexR] % 6;
                _indexP += 1;
                _indexR += 1;
                if(_indexP == 7){
                    _indexP = 0;
            ...
            _potions[i] = iPotions.mintMultiplePotion(_powers, _user);
        }
        return _potions;
    }
