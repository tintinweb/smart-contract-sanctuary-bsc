...
contract MySanta is Ownable, ReentrancyGuard {
    ...
    function transfer(
        ...
        if (token == Token.BNB) {
            payable(receiver).transfer(amountReceived);
            payable(developmentFeeReceiver).transfer(amountReceived);
            payable(daoFeeReceiver).transfer(amountReceived);

            donor[msg.sender].totalBNBDonation = amount;
