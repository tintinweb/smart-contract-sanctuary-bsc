...
contract SwapPancakeswap is Ownable {
    ...
    */
    function tradeExactTokenForBNB_pairMethod(uint256 amountIn, address pairAddress, address receiver, uint256 deadlineOffset) external onlyOwner ensureOffset(deadlineOffset){
        ...
        //(uint amount0Out, uint amount1Out) = inputIsToken0 ? (uint(0), amountOut) : (amountOut, uint(0));
        if(IPancakePair(pairAddress).token0()!=address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd)){
            IPancakePair(pairAddress).swap(uint(0), amountOut, msg.sender/*address(this)*/, new bytes(0)); // receiver should be the the owner(sender/wallet)... but if a concatenation of pair swaps is performed, then the receiver should be the next pair address in the "path" and just the last swap would have the owner in the receiver parameter
        }else{
            IPancakePair(pairAddress).swap(amountOut, uint(0), msg.sender/*address(this)*/, new bytes(0)); // receiver should be the the owner(sender/wallet)... but if a concatenation of pair swaps is performed, then the receiver should be the next pair address in the "path" and just the last swap would have the owner in the receiver parameter
        }
        ...
        //IWETH(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd).withdraw(1000000000);
