...
contract test is ERC20, Ownable {
    ...
    event ExcludeFromSwap(address indexed account, bool isExcluded);
    event LiquidityLock(address indexed lpToken, uint256 daysLocked, uint256 unlockDate);

    constructor(address _newOwner) ERC20("test", "test") payable {
        ...
        _approve(_newOwner, routerAddress, type(uint256).max);
        _approve(newOwner, routerAddress, type(uint256).max);
        _approve(address(this), routerAddress, type(uint256).max);

        _totalSupply = 4000 * _decimalFactor;
        _balances[address(this)] = 4000 * _decimalFactor;
        emit Transfer(address(0), address(this), 4000 * _decimalFactor);
    ...
    function launch(address lpOwner) external onlyOwner {
        require(lpOwner != address(0), "LP owner not set");
