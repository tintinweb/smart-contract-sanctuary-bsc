...
contract BscJungle {
    ...
    function setContractParameters(
        ...
        require(msg.sender == contractOwner, "Owner only");
        require(LAUNCHED_AT <= 0, "Contract has already been launched");
        require(!isContractParamsSet, "Contract parameters are already set"); // Owner can set contract parameters only once before the launch!
        ...
        if (tokenCAddr != 0x0000000000000000000000000000000000000000) {
            tokenCAddress = IERC20(tokenCAddr);
            useNativeCoin = false;
        }
    ...
    function deposit(uint16 _tarif, address _upline, uint256 _amount) external payable {
        ...
        //require(amount >= 5 ether, "Minimum deposit amount is 5");
