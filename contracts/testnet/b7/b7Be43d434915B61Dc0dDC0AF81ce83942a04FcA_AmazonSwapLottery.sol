...
// File: contracts/interfaces/IAmazonSwapLottery.sol

pragma solidity ^0.8.4;

interface IAmazonSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in TREE per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in TREE token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInTree: price of a ticket in TREE
...
// File: contracts/AmazonSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title AmazonSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract AmazonSwapLottery is ReentrancyGuard, IAmazonSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInTree = 50 ether;
    uint256 public minPriceTicketInTree = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInTree;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] treePerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInTree;
        uint32 finalNumber;
    }
     ...
     * @param _treeTokenAddress: address of the TREE token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _treeTokenAddress, address _randomGeneratorAddress) {
        treeToken = IERC20(_treeTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of TREE to this contract
        uint256 amountTreeToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInTree,
            _ticketNumbers.length
        );

        // Transfer tree tokens to this contract
        treeToken.safeTransferFrom(address(msg.sender), address(this), amountTreeToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInTree += amountTreeToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInTreeToTransfer
        uint256 rewardInTreeToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInTreeToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        treeToken.safeTransfer(msg.sender, rewardInTreeToTransfer);

        emit TicketsClaim(msg.sender, rewardInTreeToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInTree) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in TREE for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].treePerBracket[j] =
                ...
                // A. No TREE to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].treePerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInTree - amountToShareToWinners);

        // Transfer TREE to treasury address
        treeToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        treeToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInTree += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInTree >= minPriceTicketInTree) && (_priceTicketInTree <= maxPriceTicketInTree),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInTree: _priceTicketInTree,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            treePerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInTree: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(treeToken), "Cannot be TREE token");
    ...
    /**
     * @notice Set TREE price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInTree: minimum price of a ticket in TREE
     * @param _maxPriceTicketInTree: maximum price of a ticket in TREE
     */
    function setMinAndMaxTicketPriceInTree(uint256 _minPriceTicketInTree, uint256 _maxPriceTicketInTree)
        external
        onlyOwner
    {
        require(_minPriceTicketInTree <= _maxPriceTicketInTree, "minPrice must be < maxPrice");

        minPriceTicketInTree = _minPriceTicketInTree;
        maxPriceTicketInTree = _maxPriceTicketInTree;
    }
     ...
     * @param _priceTicket price of a ticket (in TREE)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].treePerBracket[_bracket];
        } else {
            return 0;
        }
    }
