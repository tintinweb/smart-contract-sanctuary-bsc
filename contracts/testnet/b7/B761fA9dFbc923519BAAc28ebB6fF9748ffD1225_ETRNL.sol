...
contract ETRNL is Context, Ownable, Token {
    ...
    function _compoundBUSD(address addr) internal {
        ...
        if(amount > 0) { //avoid reverts
            ...
            _refPayout(addr, adjustedAmount, _properties.refBonus);
            _stakeEternalToken(addr, adjustedAmount);
            _stats.totalCompounded += adjustedAmount;
        }
    ...
    function _compoundEternal(address addr) internal {
        ...
        if(claim > 0) {  //avoid reverts
            ...
            uint256 stakeFee = claim.mul(_properties.eternalTokenStakeTax).div(percentDivider); //5% ETRNL stake tax
            uint256 adjustedAmount = claim.sub(stakeFee);
    ...
    function claimFromBUSD(address addr, bool fromCompound) internal returns (uint256) {
        User storage user = _users[addr];
        ...
        //if claim from compound set claim tax to 0% as the amount will be compounded to the ETRNL Pool with 5% tax
        uint256 adjustedAmount = fromCompound ? _payoutFor(addr, amount, 0) : _payoutFor(addr, amount, _properties.BUSDTokenClaimTax);
    ...
    function claimFromEternal(address addr, bool fromCompound) internal returns (uint256) {
        User storage user = _users[addr];
        ...
        //if claim from compound set claim tax to 0% as the amount will be compounded to the ETRNL Pool with 5% tax
        uint256 adjustedAmount = fromCompound ? _payoutFor(addr, amount, 0) : _payoutFor(addr, amount, _properties.eternalTokenClaimTax);
