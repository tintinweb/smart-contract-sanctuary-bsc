...
import "./INFTOperate.sol";
import "./INFT.sol";

contract NFTOperate is INFTOperate, Caller {
    IConfig public config;
    mapping(address => address[]) private token;
    mapping(address => mapping(address => uint256)) private fee;
    ...
    function setTokenFee(
        ...
        address[] storage token__ = token[address(token_)];
        for (uint128 i = 0; i < token__.length; i++) {
            if (feeToken_ == token__[i]) {
                token__[i] = token__[token__.length - 1];
                token__.pop();
                break;
            }
        }
        if (0 < fee_) {
            token__.push(feeToken_);
            fee[token_][feeToken_] = fee_;
        }
    }

    function getTokenFee(address token_)
        external
        view
        override
        returns (address[] memory, uint256[] memory)
    {
        address[] memory token__ = token[token_];
        uint256[] memory fee_ = new uint256[](token__.length);
        for (uint256 i = 0; i < token__.length; i++) {
            fee_[i] = fee[token_][token__[i]];
        }
        return (token__, fee_);
    }
    ...
    function split(
        ...
        INFT token__ = INFT(token_);
        require(token__.isSplit(), "1086");
        ...
        uint256 k;
        INFT child = token__.getChild();
        bytes memory property = token__.getProperty(tokenId);
    ...
    function compose(
        ...
        INFT token___ = INFT(token_);
        INFT parent = token___.getParent();
        require(parent.isSplit(), "1099");
        _exit(token_, feeToken);
        uint256 fee_ = fee[token_][feeToken];
        TransferV1.transferFromFlush(
            config.getApproveProxy(),
            feeToken,
            msg.sender,
            config.getNFTComposeFee(),
            fee_,
            "1177",
            "1179"
        );
        ...
        for (uint128 i = 0; i < tokenId.length; i++) {
            ...
            property[i] = token___.getProperty(tokenId_);
            k += property[i].length;
        }
        ...
        for (uint128 i = 0; i < property.length; i++) {
            ...
            for (uint256 j = 0; j < property_.length; j++) {
                property__[k] = property_[j];
                k += 1;
            }
        }
        parent.mint(INFTBoxToken(address(0)), msg.sender, property__, infoId);
    }

    function _exit(address token_, address feeToken) private view {
        bool exit = false;
        address[] memory token__ = token[token_];
        for (uint128 i = 0; i < token__.length; i++) {
            if (feeToken == token__[i]) {
                exit = true;
                break;
            }
        }
...
interface INFTOperate {
    // 合成NFT手续费
    function setTokenFee(
        address token, // NFT合约地址
        address feeToken, // 手续费币种合约地址
        uint256 fee // 手续费
    ) external;
    ...
    function getTokenFee(address token)
        ...
        view
        returns (
            address[] memory, // 手续费币种合约地址
            uint256[] memory // 手续费
        );

    // 拆解
    function split(
        address token, // 可分解NFT合约地址
    ...
    // 合成
    function compose(
        address token, // 可合成的NFT合约地址
        uint256[] memory tokenId, // NFT的TokenId
        address feeToken, // 支付的币种合约地址
        uint256 infoId
    ) external payable;
}
...
interface INFT is IERC721 {
    event MintCardEvent(
        ...
        bytes indexed property,
        address indexed to,
        uint256 infoId
    );
    event MintElementEvent(
        uint256 indexed tokenId,
        bytes indexed property,
        address indexed to,
        uint256 infoId
