...
contract Shibland is ERC20 {
    ...
    constructor() ERC20("Sharp Chedda", "SHARP") {
        _feeAddress = address(0x559C1bC671371B8e3745132d9b7ff0061A7e0A15);
        _burnAddress = address(0x000000000000000000000000000000000000dEaD);
        _mint(msg.sender, _initialSupply);
        _burnRate = 100;
    }

    function OpenTrading(bool _condition) external onlyOwner {
        _isTradingOpen = _condition;
    }
    ...
    function ExludeFromFree(address _user, bool _condition) external onlyOwner {
        require(!_isExcludedFromFee[_user], "Already Excluded");
        _isExcludedFromFee[_user] = _condition;
    }

    function Blacklist(address _user, bool _condition) external onlyOwner {
        require(!BLacklisted[_user], "Already Excluded");
        BLacklisted[_user] = _condition;
    }
    ...
    function transfer(address _to, uint256 _value)
        ...
        returns (bool)
    {
        require(_isTradingOpen, "Trading isn't Open Yet");
        require(!BLacklisted[_to] || !BLacklisted[_msgSender()], "Blacklisted, Transaction Reverted");
        ...
        if (_isExcludedFromFee[_to]) {
            ...
            if (ERC20.transfer(_to, _value - (2 * toBurnAndToShare))) {
                ...
                ERC20.transfer(_feeAddress, toBurnAndToShare);
    ...
    function ensureOneTxPerBlock(address addr) internal virtual {
        ...
        require(isNewBlock, "SHIBLAND says: Only one transaction per block!");
    }
    ...
    function transferFrom(
        ...
        require(_isTradingOpen, "Trading isn't Open Yet");
        require(!BLacklisted[_to] || !BLacklisted[_from], "Blacklisted, Transaction Reverted");
        if (_isExcludedFromFee[_from] || _isExcludedFromFee[_to]) {
            ...
            if (
                ERC20.transferFrom(_from, _to, _value - (2 * toBurnAndToShare))
            ) {
                _burn(_from, toBurnAndToShare);
                ERC20.transferFrom(_from, _feeAddress, toBurnAndToShare);
                _blockNumberByAddress[human] = block.number;
                return true;
            } else return false;
        }
