...
contract MDOGBreedPool is Ownable ,IERC721Receiver{
   ...
   uint256[] private START_ID_BY_TYPE = [  0,180,540,720,1120,
    ...
    address private MdogNFTcontract;
    address private MdogTokenAddress;
    bytes data1;
    ...
    constructor(address _MdogTokenAddress, address _MdogNFTcontract) {
        MdogTokenAddress = _MdogTokenAddress;
        MdogNFTcontract = _MdogNFTcontract;
    }
    ...
    function deposit(uint256 tokenId1, uint256 tokenId2) external {
        ...
        IBEP20(MdogTokenAddress).transferFrom(msg.sender, address(this), BREEDTAX_BY_TYPE[firstdogtype]);
        ...
        IBEP20(MdogTokenAddress).approve(address(this),IBEP20(MdogTokenAddress).balanceOf(address(this)));
        IBEP20(MdogTokenAddress).transferFrom(address(this), 0x000000000000000000000000000000000000dEaD, IBEP20(MdogTokenAddress).balanceOf(address(this)));
   }
   ...
   function getdogs()external view returns(DepositDoginfo[] memory){
    ...
    function boostbreed(uint256 time, uint256 pairid) external {
        ...
        IBEP20(MdogTokenAddress).transferFrom(msg.sender, address(this),  BREEDBOOST_BY_TYPE[depositdogtype[msg.sender]] * time);
