...
contract TREE is Context, IERC20, Ownable {
    ...
    string private _name = "AmazonSwap Token";
    ...
    address payable public marketingWallet = payable(0x7C0870dC837472AbbFd536Fc52aD6fE05C6869AA);
    uint256 private _previousmarketingFee = _marketingFee;

    IAmazonRouter02 public  amazonRouter;
    address public  amazonPair;
    ...
    uint256 public maxWalletToken = 2500000 * (10**18);
    ...
    constructor() {
        ...
        IAmazonRouter02 _amazonRouter = IAmazonRouter02(0xf294bCD78361Ab5182F4B60d3A169cfD55bA2c17); // mainnet: 0x854736142CcdFa751b14eF33c2e63a67f8a9190f, testnet: 0xf294bCD78361Ab5182F4B60d3A169cfD55bA2c17
         // Create a amazon pair for this new token
        amazonPair = IAmazonFactory(_amazonRouter.factory())
            .createPair(address(this), _amazonRouter.WETH());

        // set the rest of the contract variables
        amazonRouter = _amazonRouter;
        
        //exclude owner and this contract from fee
        _isRouter[address(amazonRouter)] = true;

        //exclude owner and this contract from max tx amount
    ...
    //to recieve ETH from amazonRouter when swaping
    ...
    function _transfer(
        ...
        // also, don't swap & liquify if sender is amazon pair.
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the amazon pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = amazonRouter.WETH();

        _approve(address(this), address(amazonRouter), tokenAmount);

        // make the swap
        amazonRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        amazonRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    {
        if(!_isRouter[sender] && !_isRouter[recipient])
        {   
           removeAllFee(); 
        }
    ...
    function setRouterAddress(address newRouter) public onlyOwner() {
        IAmazonRouter02 _newAmazonRouter = IAmazonRouter02(newRouter);
        amazonPair = IAmazonFactory(_newAmazonRouter.factory()).createPair(address(this), _newAmazonRouter.WETH());
        amazonRouter = _newAmazonRouter;
    }
