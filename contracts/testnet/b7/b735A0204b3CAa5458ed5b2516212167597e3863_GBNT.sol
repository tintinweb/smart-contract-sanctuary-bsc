...
contract GBNT is Context, IERC20, Ownable, Pausable {
    ...
    uint256 private _tTotal = 800 * 10**9 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "GBNTOKEN";
    string private _symbol = "GBNT";
    ...
    function burnFrom(uint256 rAmount) external onlyOwner() {
        uint256 rAccountBalance = _rOwned[address(this)];
        uint256 tAccountBalance = _tOwned[address(this)];
        ...
        _rOwned[address(this)] = rAccountBalance.sub(rAmount);
        _tOwned[address(this)] = tAccountBalance.sub(tAmount);
        _tTotal = _tTotal.sub(tAmount); // Take tokens out of circulation
        _rTotal = _rTotal.sub(rAmount);
        
        emit Transfer(address(this), BURN_ADDRESS, rAmount);
    }

    function _mint(uint256 tAmount) private { //Increise tokens on circulation and add them to contract
        uint256 currentRate =  _getRate();
