...
contract TraderDAO is Ownable{
    ...
    function OwnerEndPool() external onlyOwner{
        ...
        bool first = true;
        for(uint i = MovingPointer; i <= PoolIdCounter; i++) {
            address user = UserList[i];
            if(first){
                if(PointerBalance > 0){
                    UserAmountToClaim[user] = UserAmountToClaim[user] +  PointerBalance;
                    first = false;
                }
            }
            else{
                UserAmountToClaim[user] = UserAmountToClaim[user] + UserStakeAmount[i];
            }
        }
        TotalFund = 0;
    }

    function RestartPool() public onlyOwner{
        End = false;
        for(uint i = 1; i <= PoolId; i++){
            if(PoolRealizedAmount[i] == 0){
                PoolRealizedAmount[i] = 1;
            }
        }
        MovingPointer = PoolIdCounter;
        PointerBalance = 0;
        Initial = true;
    }
