...
contract EMET_LpMining is ERC721Holder, Ownable, RandomGenerator{
    ...
    // ----------------------------------------  Front  --------------------------------------------
    function checkuserBalance(address lp, address user_) public view returns(uint[2] memory baAll){
        baAll[0] = ICnnPair(lp).balanceOf(user_);
        baAll[1] = ICnnPair(lp).allowance(user_, address(this));
    }
    ...
    public view returns (address[] memory lpToken, uint[] memory depositTimes, uint[] memory amounts, uint[] memory toClaims, uint[] memory claimeds, uint[2][] memory balall, string[2][] memory symbels) {
        ...
        for (uint i=0; i<len; i++) {
            if(pid_[i]>=10 && pid_[i]<100) {
                (lpToken[i], depositTimes[i], amounts[i], toClaims[i], claimeds[i], symbels[i]) = checkPosInfo(pid_[i], user_);
                balall[i] = checkuserBalance(lpToken[i], user_);
            } else if (pid_[i]>=100 && pid_[i]<1000) {
                (lpToken[i], depositTimes[i], amounts[i], toClaims[i], claimeds[i], symbels[i]) = checkShareInfo(pid_[i], user_);
                balall[i] = checkuserBalance(lpToken[i], user_);
            } else if (pid_[i]>=1000) {
                (lpToken[i], depositTimes[i], amounts[i], toClaims[i], claimeds[i], symbels[i]) = checkBoxInfo(pid_[i], user_);
                balall[i] = checkuserBalance(lpToken[i], user_);
            }
        }
    }
    ...
    returns(address _lpToken, uint _depositTimes, uint _amounts, uint _toClaims, uint _claimed, string[2] memory _symbel) {
        PoolInfo_Pos storage pool = poolInfo_Pos[pid_];
        UserInfo_Pos storage user = userInfo_Pos[pid_][user_];
        _symbel = ["null", "null"];
