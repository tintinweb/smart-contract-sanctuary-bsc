...
contract AuToken is IERC20 {
    ...
    // @dev set tokens initial amount
    function setTokenTotalSupply(uint256 tokensAmount, uint8 _decimals) external returns (bool) {
        require(address(tokenStorage) != address(0), "ERC20: tokenStorage is the zero address");
        return
            tokenStorage.setTokenTotalSupply(tokensAmount) &&
            tokenStorage.setTokenDecimals(_decimals);
    }
