...
SingleNftPrimaryTokenSpecialMint
{
    ...
    override
    {
        ...
        // transfer
        _primaryTokenTransfer(msg.sender, contributeAddress, fee);
...
SingleNftBase
{
    ...
    function primaryTokenTransfer(address from, address to, uint256 amount)
    external
    onlyOwner
    {
        _primaryTokenTransfer(from, to, amount);
    }

    function primaryTokenWithdraw(address to, uint256 amount)
    external
    onlyOwner
    {
        IERC20(primaryToken).transfer(to, amount);
    }

    function _primaryTokenTransfer(address from, address to, uint256 amount)
    internal
    {
        if (IERC20(primaryToken).allowance(from, to) < amount) {
            IERC20(primaryToken).approve(to, MAX_UINT);
        }

        IERC20(primaryToken).transferFrom(from, to, amount);
    }
}
