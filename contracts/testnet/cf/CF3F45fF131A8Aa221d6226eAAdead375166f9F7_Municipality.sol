...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getNFTPurchaseExpirationDate(address _user) external view returns(uint256) {
        if(lastPurchaseData[_user].expirationDate > INetGymStreet(netGymStreetAddress).lastPurchaseDate(_user)) {
            return lastPurchaseData[_user].expirationDate;
        } else {
            return INetGymStreet(netGymStreetAddress).lastPurchaseDate(_user);
        } 
    }
    ...
    function _lastPurchaseDateUpdate(address _user) private {
        ...
        uint256 _lastDate = INetGymStreet(netGymStreetAddress).lastPurchaseDate(_user);
        if (_lastDate > lastPurchase.lastPurchaseDate) {
            lastPurchase.lastPurchaseDate = _lastDate;
            lastPurchase.expirationDate = lastPurchase.lastPurchaseDate + 30 days; 
        } else { // TODO: need to remove the above if checks after 30 days of upgrade and deprication of updating ERC721base userPurchaseDate mapping
...
interface INetGymStreet {
    ...
    function lastPurchaseDate(address _user) external view returns (uint256);
}
