...
contract HEXNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function acceptOfferBid(acceptOfferBidData memory _transferData) external payable {
        ...
        uint256 owenerPercent = HUNDERED.sub(ROYALTY_SPLIT);
        uint256 ownerShare = calculatePercentValue(_transferData.amount, owenerPercent);
        uint256 royaltiesShare = _transferData.amount - ownerShare;

        if(ROYALTY_SPLIT > 0) {
            transferRoyaltiesToken(royaltiesShare, _transferData.nftRoyalty, _transferData.payThrough, _transferData.newOwner);
        }
        if(_transferData.payThrough==1) {
            transferMyntToOwner(_transferData.newOwner, tokenInfoById.currentOwner, ownerShare);
        }
        else {
            transferwbnbToOwner(_transferData.newOwner, tokenInfoById.currentOwner, ownerShare);
        }
    ...
    function buyNFT(buyNFTData memory _buyData) external payable {
        ...
        uint256 owenerPercent = HUNDERED.sub(ROYALTY_SPLIT);
        uint256 ownerShare = calculatePercentValue(_buyData.amount, owenerPercent);
        uint256 royaltiesShare = _buyData.amount - ownerShare;
        if(ROYALTY_SPLIT > 0) {
            transferRoyalties(royaltiesShare, _buyData.nftRoyalty);
        }
        payable(_buyData.owner).transfer(ownerShare);
        ...
        emit NftTransferred(tokenId, msg.value, _buyData.owner, msg.sender);
    }
    function acceptBid(acceptData memory _transferData) external payable onlyOwner {
        ...
        tokenInfoById.currentOwner = payable(_transferData.newOwner);
        allTokensInfo[_transferData.nftId] = tokenInfoById;
        emit AutoAccepted(_transferData.tokenId);
    }
    
    fallback () payable external {}
    receive () payable external {}
    function transferRoyalties(uint256 royaltiesFees, Royalty[] memory nftRoyalty) internal {
        RoyaltyResponse[] memory royaltyResponse;
        uint256 transferredAmount = 0;
        uint256 commissionPercentAmount = 0;
        uint256 comissionTransferred = 0;
        for(uint x = 0; x < nftRoyalty.length; x++) {
            Royalty memory royalty = nftRoyalty[x];
            commissionPercentAmount = calculatePercentValue(royaltiesFees, royalty.percent);
            RoyaltyResponse memory royaltyData = RoyaltyResponse(
                commissionPercentAmount,
                royalty.wallet
            );
            royaltyResponse[x] = royaltyData;
            comissionTransferred = comissionTransferred.add(commissionPercentAmount);
            payable(royalty.wallet).transfer(commissionPercentAmount);
        }
        // return royaltyResponse;
    }
    //file
    function transferRoyaltiesToken(uint256 royaltiesFees, Royalty[] memory nftRoyalty, uint256 paymentType, address buyer) internal {
        RoyaltyResponse[] memory royaltyResponse;
        uint256 transferredAmount = 0;
        uint256 commissionPercentAmount = 0;
        uint256 comissionTransferred = 0;
        for(uint x = 0; x < nftRoyalty.length; x++) {
            ...
            royaltyResponse[x] = royaltyData;
            if(paymentType==1) {
                transferMyntToOwner(buyer, royalty.wallet, commissionPercentAmount);
            }
            else {
                transferwbnbToOwner(buyer, royalty.wallet, commissionPercentAmount);
            }
            comissionTransferred = comissionTransferred.add(commissionPercentAmount);
        }
        uint256 platformShare = royaltiesFees - transferredAmount;
        if(paymentType==1) {
            transferMyntToOwner(buyer, address(this), platformShare);
        }
        else {
            transferwbnbToOwner(buyer, address(this), platformShare);
        }
        // return royaltyResponse;
    }
