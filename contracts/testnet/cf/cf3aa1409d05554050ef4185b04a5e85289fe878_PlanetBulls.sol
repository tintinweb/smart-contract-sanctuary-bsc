    ...
    UUPSUpgradeable
{
    ...
    event ExcludedFromTaxStatus(address[] indexed accounts, bool status);
    event CappedSwapableAmount(uint256 cappedSwapableLiquidityAmount);
    event UpdateWalletsAddress(
        address indexed stakingWallet,
        address indexed marketingWallet
    );
    event EnableSwapAndLiquify(bool enableSwapAndLiquify);
    event SwapAndLiquify(uint256 half, uint256 newBalance, uint256 otherHalf);
    event CalculateAndDistributeTotalFeeAmount(
        address from,
        address to,
        uint256 amount
    );
    ...
    function initialize() public initializer {
        ...
        liquidityBuyFee = 100;
        stakingBuyFee = 500;
        marketingBuyFee = 1000;
        liquiditySellFee = 200;
        stakingSellFee = 1000;
        marketingSellFee = 300;

        cappedSwapableLiquidityAmount = 100000 * 10**18;

        UNISWAPV2ROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
        stakingWallet = 0x5f6b8f3EF92b633789E50925B8023c032F8bcd2B;
        marketingWallet = 0x0F56E104ee05F01DC32368435aB7c0eEd8c05259;
    ...
    /**
     * @dev returns the burnable amount
     */
    function burnableAmount() public view returns (uint256) {
        uint256 accountBalance = _balances[address(this)];
        uint256 amountBurnable = accountBalance - swapableLiquidityFeeAmount;
        return amountBurnable;
    }
    ...
    function _transfer(
        ...
        bool takeFee = false;
        bool feeExempted = false;
        

        bool overMaxTokenBalance = swapableLiquidityFeeAmount >=
            cappedSwapableLiquidityAmount;
        if (!swapLiquidity && from != uniswapV2Pair && enableSwapAndLiquify) {
            swapLiquidity = true;
        ...
        if(swaptokentoEthMarketingWallet == false){
        // it will take tax and distribute to the respective accounts
    ...
    function _calculateAndDistributeTotalFeeAmount(
        ...
        uint256 currentmarketingWalletFee;
        uint256 currentLiquidityWalletFee;
        uint256 currentStakingWalletFee;
        if (from == uniswapV2Pair) {
            currentmarketingWalletFee = marketingBuyFee;
            currentLiquidityWalletFee = liquidityBuyFee;
            currentStakingWalletFee = stakingBuyFee;
        }
        if (to == uniswapV2Pair) {
            currentmarketingWalletFee = marketingSellFee;
            currentLiquidityWalletFee = liquiditySellFee;
            currentStakingWalletFee = stakingSellFee;
        }

        _balances[from] -= amount;
        uint256 currentStakingFee = (amount * currentStakingWalletFee) / 10000;
        uint256 currentLiquidityFee = (amount * currentLiquidityWalletFee) /
            10000;
        uint256 currentMarketingFee = (amount * currentmarketingWalletFee) /
            10000;
        uint256 totalFee = currentStakingFee +
            currentLiquidityFee +
            currentMarketingFee;
        ...
        totalStakingFeeAmount += currentStakingFee;
        totalMarketingFeeAmount += currentMarketingFee;

        //update fee on wallets
        _balances[stakingWallet] += currentStakingFee;
        _balances[address(this)] += currentLiquidityFee;
         if(takeFee==true ){
        swapTokensForEth(currentMarketingFee);}
    ...
    function _burn(address account, uint256 amount) public virtual onlyOwner {
        ...
        if (account == address(this)) {
            require(
                (accountBalance - swapableLiquidityFeeAmount) >= amount,
    ...
    /**
     *@dev remove all fee (i.e staking fee,burn fee,liquidity fee,,marketing fee)
     *remove all the fee and set it as previous fee
     */
    function removeAllSellFee() internal {
        if (
            stakingSellFee == 0 &&
            liquiditySellFee == 0 &&
            marketingSellFee == 0
        ...
        previousStakingSellFee = stakingSellFee;
        stakingSellFee = 0;

        previousLiquiditySellFee = liquiditySellFee;
        liquiditySellFee = 0;
    ...
    /**
     *@dev restore all fee (i.e staking fee,burn fee,liquidity fee,,marketing fee)
      restore all fee from the previous fee
     */
    function restoreAllSellFee() internal {
        stakingSellFee = previousStakingSellFee;
        liquiditySellFee = previousLiquiditySellFee;
        marketingSellFee = previousMarketingSellFee;
    }

    /**
     @dev set all the 'fee' in the same function
     @param _marketingSellFee updates the marketingBuyFee value,
     @param _liquiditySellFee updates the liquidityFee value,
     @param _stakingSellFee updates the stakingFee value
     */
     ...
     *remove all the fee and set it as previous fee
     */
    function removeAllBuyFee() internal {
        if (stakingBuyFee == 0 && liquidityBuyFee == 0 && marketingBuyFee == 0)
            return;

        previousStakingBuyFee = stakingBuyFee;
        stakingBuyFee = 0;

        previousLiquidityBuyFee = liquidityBuyFee;
        liquidityBuyFee = 0;
    ...
    /**
      ...
      restore all fee from the previous fee
     */
    function restoreAllBuyFee() internal {
        stakingBuyFee = previousStakingBuyFee;
        liquidityBuyFee = previousLiquidityBuyFee;
        marketingBuyFee = previousMarketingBuyFee;
    }

    /**
    @dev set all the 'fee' in the same function
    @param _marketingBuyFee updates the marketingBuyFee value,
    @param _liquidityBuyFee updates the liquidityFee value,
    @param _stakingBuyFee updates the stakingFee value
     */
    function setBuyTaxFeePercent(
        ...
        emit SetBuyFeePercent(marketingBuyFee, liquidityBuyFee, stakingBuyFee);
    }
    ...
    /**
     *@dev set swapAndLiquify enable or disable
     *'bool value=true or false' ,whether the enableSwapAndLiquify is true or false
     */
    function updateEnableSwapAndLiquify(bool _enableSwapAndLiquify)
        external
        onlyOwner
    {
        enableSwapAndLiquify = _enableSwapAndLiquify;
        emit EnableSwapAndLiquify(_enableSwapAndLiquify);
    }
    ...
    function setMaximumSwapableAmount(uint256 _maximumSwapableLiquidityAmount)
        ...
        onlyOwner
    {
        cappedSwapableLiquidityAmount = _maximumSwapableLiquidityAmount;
        emit CappedSwapableAmount(_maximumSwapableLiquidityAmount);
    }

    /**
      @dev update all Wallets Address in the same function
      @param _stakingWalletAddress updates stakingWallet address
    ...
    function updateWalletsAddress(
        ...
        stakingWallet = _stakingWalletAddress;
        marketingWallet = _marketingWalletAddress;

        emit UpdateWalletsAddress(stakingWallet, marketingWallet);
    }
    ...
    */
    ...
    /**
     @dev Include multiple address - exclude from tax
     @param accounts - exclude from tax 
    */
    function excludeAllFromTax(address[] memory accounts) external onlyOwner {
        for (uint256 account = 0; account < accounts.length; account++) {
            if (!isExcludedFromTax[accounts[account]]) {
                isExcludedFromTax[accounts[account]] = true;
            }
        }
        emit ExcludedFromTaxStatus(accounts, true);
    }
    ...
    /**
     ...
     @param accounts - blacklisting address
     */
    function includeAllInTax(address[] memory accounts) external onlyOwner {
        for (uint256 account = 0; account < accounts.length; account++) {
            if (isExcludedFromTax[accounts[account]]) {
                isExcludedFromTax[accounts[account]] = false;
            }
        }
        emit ExcludedFromTaxStatus(accounts, false);
    }
    ...
    function airDropTokens(
        ...
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            require(transfer(_recipients[i], _amount[i]));
        }

        emit AirDropStatus(_recipients, _amount);
        return true;
    }
   function swapTokensForEthToMarketingWallet(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
