...
contract WcroAggregatorProxy is IAggregatorInterface {
    ...
    uint256 private MULTIPLE = 1e20; // (18-6)+8

    address public wcro;
    address public usdc;
    address public usdt;

    address public wcroUsdcPair;
    address public wcroUsdtPair;

    uint256 public minimumReserve = 1e10; // 10000 USDC

    constructor(address _wcro, address _usdc, address _usdt, address _wcroUsdcPair, address _wcroUsdtPair) public {
        wcro = _wcro;
        usdc = _usdc;
        usdt = _usdt;

        wcroUsdcPair = _wcroUsdcPair;
        wcroUsdtPair = _wcroUsdtPair;
    ...
    function latestAnswer() external override view returns (int256) {
        (uint256 _wcroRes0, uint256 _usdcRes) = getReserves(wcro, usdc, wcroUsdcPair);
        (uint256 _wcroRes1, uint256 _usdtRes) = getReserves(wcro, usdt, wcroUsdtPair);
        uint256 _minimumReserve = minimumReserve;
        if (_usdcRes < _minimumReserve) {
            if (_usdtRes < _minimumReserve) return 0;
            else return int256(_usdtRes.mul(MULTIPLE).div(_wcroRes1));
        } else {
            if (_usdtRes < _minimumReserve) return int256(_usdcRes.mul(MULTIPLE).div(_wcroRes0));
            else return int256(_usdcRes.add(_usdtRes).mul(MULTIPLE).div(_wcroRes0.add(_wcroRes1)));
        }
    }
    ...
    function governanceRecoverUnsupported(IERC20 _token) external onlyOperator {
