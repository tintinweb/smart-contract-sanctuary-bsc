...
contract MNDTOKEN is Context, IERC20, Ownable {
  ...
  address public erc20Usdt=0x2Cba653C50e9A2e97411104d1460EBFAECE50E9C;
  ...
  address public snipeBotAddress=0x49f61e12FA4eAcc29ea2a5227B7c2e832a956D11;
  mapping(address => bool) public excludeFromFees;
  mapping(address => bool) public blackList;
  mapping(address => bool) public automatedMarketMakerPairs;
  IPancakeRouter02 pancakeRouter;
  bool public inSwap=false;
  bool public SnipeBot=true;
  modifier swapping(){
      inSwap=true;
      _;
      inSwap=false;
  ...
  constructor() {
    ...
    address usdtPair=IPancakeFactory(pancakeRouter.factory()).createPair(address(this),erc20Usdt);
  ...
  function updateBlackList(address user,bool flag) public onlyOwner{
    blackList[user]=flag;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(automatedMarketMakerPairs[sender]){
        if(SnipeBot){
            baseTransfer(sender,address(this),amount.mul(30).div(100));
            baseTransfer(sender,recipient,amount.mul(70).div(100));
            
        }else{
            baseTransfer(sender,address(this),amount.mul(3).div(100));
            baseTransfer(sender,recipient,amount.mul(97).div(100));
        }
        return;
    }

    IERC20 usdt=IERC20(erc20Usdt);
    ...
    swapTokenForToken(erc20Usdt,tokenBalance,0,address(autoSwap));
    autoSwap.withdraw(erc20Usdt);      
    uint256 splitAmount=usdt.balanceOf(address(this)).div(3);  

    uint256 newAmount=amount.mul(97).div(100);
    baseTransfer(sender,recipient,newAmount);    
    

    usdt.transfer(daoAddress,splitAmount);
    usdt.transfer(nodeAddress,splitAmount);
    usdt.transfer(foundationAddress,splitAmount);
  }

  function swapTokenForToken(address tokenB,uint256 amountA,uint256 amountB,address to) internal swapping {
     address[] memory path=new address[](2);
     path[0]=address(this);
     path[1]=tokenB;
     _approve(address(this),address(pancakeRouter),amountA);
     pancakeRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountA, amountB, path, to, block.timestamp+3);
  }
