...
contract Evaluation is AccessControl {
    ...
    function confirmEvaluation(
        ...
        _incentive > 0
            ? THB.transferFrom(treasuryAddress, _bidOwner, _incentive)
            : THB.transferFrom(_bidOwner, treasuryAddress, _penalty);
    }
    ...
    function _compensation(
        ...
        if (_Pav >= 75) {
            _incentive = _Pav * _offerredReduction * _bidPrice;
            _penalty = 0;
        } else if (_Pav < 75 && _Pav >= 60) {
            _incentive = (_Pav * _offerredReduction * _bidPrice) / 2;
            _penalty = 0;
        } else if (_Pav < 60) {
            _incentive = 0;
            _penalty = (60 - _Pav) * _offerredReduction * _bidPrice;
        }
