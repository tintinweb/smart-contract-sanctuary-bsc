...
contract Token is ERC20, ERC20Burnable, Crosschain, Withdrawable, Permissioned, AntiBot {
    ...
    ) ERC20(name, symbol) Permissioned(owner) {
    ...
    function _mint(address account, uint256 amount) internal override {
        require(
            ...
            string(abi.encode('Can not mint more than ', MAX_TOTAL_SUPPLY))
        );
    ...
    function _grantRole(bytes32 role, address account)
        internal
        override(AccessControlEnumerable, Crosschain)
    {
        super._grantRole(role, account);
    }
}
    ...
    modifier transactionThrottler(address from, address to) {
        if (isAntibotEnabled) {
            if (!hasRole(BOT_ROLE, from) && !hasRole(BOT_ROLE, to)) {
                require(
                    transactions[abi.encodePacked(from, to)] <
                        block.number - 1 &&
                        transactions[abi.encodePacked(to, from)] <
                        block.number - 1,
...
contract Permissioned is AccessControlEnumerable {
    bytes32 public constant ACCESS_MANAGER_ROLE =
    ...
    modifier onlyPermitted() {
        if (isPermissioned) {
            require(
                hasRole(PERMITTED_ROLE, msg.sender),
                'Permissioned mode: Only permitted can transact'
            );
        }
        _;
    }
    ...
    constructor(address owner) {
        _grantRole(PERMITTED_ROLE, owner);
    }

    function addAccessManager(address manager)
        external
    ...
    function removeAccessManager(address manager)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function addPermitted(address permitted)
        external
    ...
    function removePermitted(address permitted)
        external
        onlyRole(ACCESS_MANAGER_ROLE)
    {
    ...
    function setPermissionedMode(bool isPermissioned_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
...
contract Vault is VestingWallet {
    ...
    constructor(
        ...
        VestingWallet(
            beneficiaryAddress,
            startTimestamp,
            vestingPeriodCount_ * vestingPeriodDurationSeconds_
        )
    {
        require(
            tgeFraction_ <= tgeDenominator_,
            'tgeFraction_ can not be greater than tgeDenominator_'
        );
...
contract VestingController is Ownable, Withdrawable {
    ...
    constructor(
        ...
        require(
            vestingPeriodCount_ == 0 || vestingPeriodDurationSeconds_ > 0,
            'Vesting periods of duration 0 seconds'
        );
        require(capitalCollector_ != address(0), 'Capital collector can not be address zero');
        if (presaleDurationSeconds_ > 0)
            require(presaleStartTimestamp_ > 0, 'Can not specify duration without start');
        token = Token(token_);
    ...
    function claimTokens(uint256 totalAllocationWei, bytes32[] calldata proof) external payable {
        ...
        require(
            MerkleProof.verify(
                proof,
                merkleTreeRoot,
                keccak256(abi.encode(_msgSender(), totalAllocationWei))
            ),
    ...
    function releasable() external view returns (uint256) {
        if (address(_vault()) == address(0)) return 0;
        return
            _vault().vestedAmount(address(token), uint64(block.timestamp)) -
            _vault().released(address(token));
    }
    ...
    function _claimTokens(uint256 maxAllocation) private {
        ...
        uint256 walletAllocationWei = token.balanceOf(address(_vault())) +
            _vault().released(address(token));
        uint256 remainingETH = msg.value;
        if (_freeTokens()) {
            SafeERC20.safeTransfer(token, address(_vault()), maxAllocation);
        }
        if (
            !_freeTokens() &&
            !_hasPresaleEnded() &&
            remainingETH > 0 &&
            walletAllocationWei < maxAllocation
        ) {
            uint256 canBuyTokenWei = maxAllocation - walletAllocationWei;
            uint256 boughtTokenWei = (remainingETH / 1 ether) * tokensWeiWorthOneEther;
            if (boughtTokenWei <= canBuyTokenWei) {
                ...
                uint256 usedETH = (canBuyTokenWei / tokensWeiWorthOneEther) * 1 ether;
