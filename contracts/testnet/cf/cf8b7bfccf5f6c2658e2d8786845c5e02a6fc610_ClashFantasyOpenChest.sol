...
contract ClashFantasyOpenChest is Initializable, OwnableUpgradeable {
    ...
    function check(uint256 _tokenId) 
        public view returns(uint256[] memory, uint256[] memory, uint256, uint256)
    {
        openChestSkeleton storage user = arrayOpenChest[_tokenId];
        uint256 aditionalId = user.aditionalId;
        uint256 choosen = user.rarityId;
        ...
        returns(uint256)
    {
        ...
        uint256 purchasenumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, _tokenId, _aditional, randNonce))) % 100;
