...
contract SupeAvatarMintGold is Ownable {
    ...
    address fee_coin, uint256 fee_coin_amount,
    address reward_coin_a, uint256 reward_coin_a_amount,
    uint256 staked_balance)
    {
        ...
        fee_coin = pools[_pid].male_fee_coin;
        fee_coin_amount = pools[_pid].male_fee_coin_amount;
    ...
    uint256 staked_balance)
    {
        ...
        fee_coin = pools[_pid].female_fee_coin;
        fee_coin_amount = pools[_pid].female_fee_coin_amount;
    ...
    function set_male_pool_config(uint256 _pid, uint256 lock_duration, uint256 max_stake_count, address fee_coin, uint256 fee_coin_amount,
    address reward_coin_a, uint256 reward_coin_a_amount)  external onlyFactoryOrOwner
    {
        pools[_pid].male_lock_duration = lock_duration;
        pools[_pid].male_max_stake_count = max_stake_count;
        pools[_pid].male_fee_coin = fee_coin;
        pools[_pid].male_fee_coin_amount = fee_coin_amount;
        pools[_pid].male_reward_coin_a = reward_coin_a;
        pools[_pid].male_reward_coin_a_amount = reward_coin_a_amount;
    }
    function set_female_pool_config(uint256 _pid, uint256 lock_duration, uint256 max_stake_count, address fee_coin, uint256 fee_coin_amount,
    address reward_coin_a, uint256 reward_coin_a_amount)  external onlyFactoryOrOwner
    {
        pools[_pid].female_lock_duration = lock_duration;
        pools[_pid].female_max_stake_count = max_stake_count;
        pools[_pid].female_fee_coin = fee_coin;
        pools[_pid].female_fee_coin_amount = fee_coin_amount;
    ...
    function stake_male(uint256 _pid, uint256[] memory ids) public{
        ...
        //fee
        if(pools[_pid].male_fee_coin_amount > 0){
            IERC20(pools[_pid].male_fee_coin).safeTransferFrom(msg.sender, address(this), pools[_pid].male_fee_coin_amount.mul(ids.length));
        }
    ...
    function stake_female(uint256 _pid, uint256[] memory ids) public{
        ...
        //fee
        if(pools[_pid].female_fee_coin_amount > 0){
            IERC20(pools[_pid].female_fee_coin).safeTransferFrom(msg.sender, address(this), pools[_pid].female_fee_coin_amount.mul(ids.length));
        }
        for (uint256 i = 0; i < ids.length; ++i) {
    ...
    function exit_male_one(uint256 _round_id, uint256 _asset_id) internal{
        ...
        if(pools[_round_id].male_fee_coin_amount > 0){
            IERC20(pools[_round_id].male_fee_coin).safeTransfer(msg.sender, pools[_round_id].male_fee_coin_amount);
        }
        emit ExitOne(msg.sender, _asset_id, pools[_round_id].male_stakes[msg.sender][_asset_id].start_ts, pools[_round_id].male_stakes[msg.sender][_asset_id].end_ts, coin_a_amount, 0);
        emit ClaimReward(_round_id, msg.sender, _asset_id, pools[_round_id].male_reward_coin_a, address(0x0), coin_a_amount, 0);
    ...
    function exit_female_one(uint256 _round_id, uint256 _asset_id) internal{
        ...
        if(pools[_round_id].female_fee_coin_amount > 0){
            IERC20(pools[_round_id].female_fee_coin).safeTransfer(msg.sender, pools[_round_id].female_fee_coin_amount);
        }
        emit ExitOne(msg.sender, _asset_id, pools[_round_id].female_stakes[msg.sender][_asset_id].start_ts, pools[_round_id].female_stakes[msg.sender][_asset_id].end_ts, coin_a_amount, 0);
        emit ClaimReward(_round_id, msg.sender, _asset_id, pools[_round_id].female_reward_coin_a, address(0x0), coin_a_amount, 0);
