...
contract YDT is ERC20, AccessControl {
    ...
    onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!acceptedTokens[_token].accepted, "SubscriptionData: token already added");
        require(_token != address(0), "SubscriptionData: token address can not be zero address");
        require(_decimal > 0, "SubscriptionData: token decimal can not be zero");
    ...
    function removeTokens(address t) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(t != address(0), "SubscriptionData: token address can not be zero address");
    ...
    function deposit(address _token, uint256 _amount) public {
        ...
        uint amount = getPriceData(_token, _amount);
        uint fund = _toPrecision(_amount, amount, acceptedToken.priceFeedPrecision, acceptedToken.decimals);
        _transfer(address(this), msg.sender, fund);
        emit DepositToken(msg.sender, _token, _amount, fund);
    }

    function depositNative(address _token) payable public {
        Token memory acceptedToken = acceptedTokens[_token];
        (bool sent, bytes memory data) = address(this).call{value: msg.value}("");
        require(sent, "Failed to send BNB to contract");
        totalDeposit[_token] += msg.value;
        uint amount = getPriceData(_token, msg.value);
        uint fund = _toPrecision(msg.value, amount, acceptedToken.priceFeedPrecision, acceptedToken.decimals);
        _transfer(address(this), msg.sender, fund);
        emit DepositToken(msg.sender, _token, msg.value, fund);
    }
    ...
    function donateToProject(uint _proposalId, uint256 _amount) public {
        require(isDaoSet, "DAO has not been set");
        require(balanceOf(msg.sender) >= _amount, "Insufficient Amount");
        require(allowance(msg.sender, address(this)) >= _amount, "Insufficient Allowance");
        require(simiDAO.isValidForDonation(_proposalId), "Not a valid donation project");
        ...
        transferFrom(msg.sender, proposer, projectShare);
        simiDAO.receivedDonation(_proposalId, projectShare);
        transferFrom(msg.sender, address(this), poolShare);
        YDTpoolFund += poolShare;
    ...
    function injectFundToPool(uint256 _amount, uint256 _lotteryId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        require(_amount <= YDTpoolFund, "Insufficient Amount");
    ...
    /*
     * @notice Grant DEFAULT_ADMIN_ROLE role to a specified address
     * @param address Account to grant DEFAULT_ADMIN_ROLE role
     */
    ...
    /*
     * @notice Revoke DEFAULT_ADMIN_ROLE role from a specified address
     * @param address Account to revoke DEFAULT_ADMIN_ROLE role
     */
    ...
    function updateSimiDAO(address _simiDAO) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _simiDAO != address(0),
            "SimiDAO contract address can not be zero address"
        );
        isDaoSet = true;
    ...
    function updateLotteryAddress(address _lottery) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _lottery != address(0),
            "lottery contract address can not be zero address"
        );
        isLotterySet = true;
