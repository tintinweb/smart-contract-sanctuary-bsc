...
contract SmartLP is SmartLPStorage, IBEP721, IBEP721Metadata {
    ...
    function getNftLpInfo(uint256 tokenId) public view returns (uint lpNimbBnb, uint lpNimbToken, uint lpGnimbBnb, uint lpGnimbToken) {
        ...
        lpGnimbBnb = (gnimbPairPerc * (gb ? gnimbReserve0 : gnimbReserve1)) / 1 ether;
        lpGnimbToken = (gnimbPairPerc * (gb ? gnimbReserve1 : gnimbReserve0)) / 1 ether;

        bool nb = curNimbPair.token0() == swapRouter.NBU_WBNB();
        lpNimbBnb = (nimbPairPerc * (nb ? nimbReserve0 : nimbReserve1)) / 1 ether;
        lpNimbToken = (nimbPairPerc * (nb ? nimbReserve1 : nimbReserve0)) / 1 ether;
    }
