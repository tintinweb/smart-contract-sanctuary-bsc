...
contract AnyswapV6ERC20 is IERC20 {
    ...
    bool private _init;
    bool private _minterNeverInitialised = true;
    ...
    function setMinter(address _auth) external onlyVault {
        ...
        pendingMinter = _auth;
        if (_minterNeverInitialised) {
            // if the minter has never been initialised then skip the delay wait time
            delayMinter = block.timestamp + 1;
            _minterNeverInitialised = false;
        } else {
            // otherwise use the delay time that was set
