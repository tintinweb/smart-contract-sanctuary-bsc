...
contract Evaluation {
    ...
    function _compensation(
        ...
        if (_Pav >= 75) {
            _incentive = (_Pav * _offerredReduction * _bidPrice*1 ether/ 100);
            _penalty = 0;
        } else if (_Pav < 75 && _Pav >= 60) {
            _incentive = (_Pav * _offerredReduction * _bidPrice*1 ether) / 200;
            _penalty = 0;
        } else if (_Pav < 60) {
            _incentive = 0;
            _penalty = (60 - _Pav) * _offerredReduction * _bidPrice * 1 ether / 100;
        }
