...
contract MultiFeeDistribution is ReentrancyGuard, Ownable {
    ...
    uint256 public constant lockDuration =  1800;
    ...
    // Address and claimable amount of all reward tokens for the given account
    function climableRewards(address account) external view returns (RewardData[] memory rewardsAmount) {
        rewardsAmount = new RewardData[](2);
        uint supply=totalSupply;
        uint length=userStakes[account].length;
        for(uint j=0;j<length;j++){
            ...
            for (uint i = 0; i < length; i++) {
                ...
                if (userStakes[user][i].unlockTime > block.timestamp) {
                    ...
                    if(timeStaked > 60 days){
                        calcPenalty = rewardAmount.mul(50).div(100);
                    }
                    else if(timeStaked <= 60 days && timeStaked > 30){
                        calcPenalty = rewardAmount.mul(35).div(100);
                    }
                    else if(timeStaked <= 30 days && timeStaked > 0){
                        calcPenalty = rewardAmount.mul(25).div(100);
                    }
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        for (uint i = 0;i<length ; i++) {
                ...
                if (userStakes[msg.sender][i].unlockTime > block.timestamp) {
                    ...
                    uint256 timeStaked = userStakes[msg.sender][i].unlockTime.sub(block.timestamp);
                    if(timeStaked > 60 days){
                        calcPenalty = penalty.mul(50).div(100);
                    }
                    else if(timeStaked <= 60 days && timeStaked > 30){
                        calcPenalty = penalty.mul(35).div(100);
                    }
                    else if(timeStaked <= 30 days && timeStaked > 0){
                        calcPenalty = penalty.mul(25).div(100);
                    }
    ...
    function exit() external updateReward(msg.sender) {
        ...
        totalSupply=totalSupply.sub(bal.staked);
        stakedSupply=stakedSupply.sub(bal.staked);
        bal.staked = 0;
        bal.total=bal.total.sub(amount);
        stakingToken.safeTransfer(msg.sender, amount);
	    IERC20(rewardToken).safeTransfer(msg.sender,totalRewards);
