 ...
 *Submitted for verification at BscScan.com on 2022-06-10
 ...
 official website: https://BNBSeedX.COM

#                                                                                                                              
#                                                                        ,,                                                    
#  `7MM"""Yp, `7MN.   `7MF'`7MM"""Yp,  .M"""bgd                        `7MM `YMM'   `MP'                                       
#    MM    Yb   MMN.    M    MM    Yb ,MI    "Y                          MM   VMb.  ,P                                         
#    MM    dP   M YMb   M    MM    dP `MMb.      .gP"Ya   .gP"Ya    ,M""bMM    `MM.M'        ,p6"bo   ,pW"Wq.`7MMpMMMb.pMMMb.  
#    MM"""bg.   M  `MN. M    MM"""bg.   `YMMNq. ,M'   Yb ,M'   Yb ,AP    MM      MMb        6M'  OO  6W'   `Wb MM    MM    MM  
#    MM    `Y   M   `MM.M    MM    `Y .     `MM 8M"""""" 8M"""""" 8MI    MM    ,M'`Mb.      8M       8M     M8 MM    MM    MM  
#    MM    ,9   M     YMM    MM    ,9 Mb     dM YM.    , YM.    , `Mb    MM   ,P   `MM.  ,, YM.    , YA.   ,A9 MM    MM    MM  
#  .JMMmmmd9  .JML.    YM  .JMMmmmd9  P"Ybmmd"   `Mbmmd'  `Mbmmd'  `Wbmd"MML.MM:.  .:MMa.db  YMbmd'   `Ybmd9'.JMML  JMML  JMML.
#                                                                                                                              
#                                                                             

 /*   BNBSeedX - investment platform based on Binance Smart Chain blockchain smart-contract technology. Safe and legit!
 
 *
 *   [USAGE INSTRUCTION]
 *
 *   1) Connect browser extension Metamask 
 *   2) Choose one of the tariff plans, enter the BNB amount (0.01 BNB minimum) using our website "Stake BNB" button
 *   3) Wait for your earnings
 *   4) Withdraw earnings any time using our website "Withdraw" button
 *
 *   [INVESTMENT CONDITIONS]
 *
 *   BNBSeedX's mechanism of action.
 * 	 A total of 5 packages , investors earn different benefits depending on the package choice.

 *	 -Basic package (7 Days): 18% APY ,total 5% ref.
 *	 -High package (30 days): 38% APY ,total 5% ref.
 *	 -Advance package (90 days): 57% APY,total 8% ref.
 * 	 -Pro package (120 days): 109% APY ,total 8% ref.
 *	 -Premium package (360 days): 156% APY,total 15% ref.
 * 	 Your investment will be put into the staking pool, investors will receive daily profit, and can withdraw or reinvest.
 *
 *   [AFFILIATE PROGRAM]
 *
 *   - 1-level referral commission: 5% - 8% - 15%
 *
 *   [FUNDS DISTRIBUTION]
 *
 ...
 *   - 2% Support work, technical functioning, ad
*/
...
contract BNBSEEDX {
	...
	uint256[] public REFERRAL_PERCENTS = [50,80,150];
	...
	constructor(address payable ceoAddr, address payable devAddr, uint256 start) public {
        ...
        plans.push(Plan(7,  1));
        plans.push(Plan(30, 2));
        plans.push(Plan(90, 3));
        plans.push(Plan(120, 4));
        plans.push(Plan(360, 6));
	}

	function invest(address referrer, uint8 plan) public payable {
		require(block.timestamp > startDate, "contract does not launch yet");
		require(_canminamount(plan), "min amount is BNB");
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 1; i++) {
				if (upline != address(0)) {
					...
					if(plan>=2&&plan<4){
						amount = msg.value.mul(REFERRAL_PERCENTS[1]).div(PERCENTS_DIVIDER);
					}
					if(plan>=4){
						amount = msg.value.mul(REFERRAL_PERCENTS[2]).div(PERCENTS_DIVIDER);
					}
...
function _canminamount(uint8 plan) internal view returns(bool) {
			if(plan<2){
				return (msg.value >= INVEST_MIN_AMOUNT);
			}
			if(plan>=2&&plan<4){
				return (msg.value >= INVEST_MIN_AMOUNT*2);
			}
			if(plan>=4){
				return (msg.value >= INVEST_MIN_AMOUNT*100);
			}
	}
