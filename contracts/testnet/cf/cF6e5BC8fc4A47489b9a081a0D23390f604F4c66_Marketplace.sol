...
contract Marketplace is ReentrancyGuard {
    ...
    function purchaseItem(uint _itemId) external payable nonReentrant {
        ...
        uint _totalPrice = item.price + getFeePrice(item.price);
    ...
    function makeOffer(uint _itemId, uint _price) external payable nonReentrant {
        require(_itemId > 0 && _itemId <= itemCount, "Item doesnt exist");
        Item memory item = items[_itemId];
        require(_price > 0, "Price must be greater than zero");
        require(!item.sold, "Item already sold");
    ...
    function Offer(uint _itemId) external payable nonReentrant {
        require(_itemId <= itemCount, "Item doesnt exist");
        uint _id = getHighestAuctionPrice(_itemId);
        address _highestPriceAddr = auctionAddr[_id];
        uint highestPrice = auctionPrice[_highestPriceAddr].price;
        Item memory item = items[_itemId];
        require(!item.sold, "Item already sold");
        ...
        for (uint _index = 0; _index < getAuctionAddrCount(); _index++) {
            if (_index != _id) {
                address _address = auctionAddr[_index];
                uint _totalRefund = auctionPrice[_address].price + getFeePrice(auctionPrice[_address].price);
    ...
    function getAuctionAddrCount() public view returns(uint) {
        return auctionAddr.length;
    }

    function newAuctionAddr(address _addr, uint _price) public {
    ...
    function getHighestAuctionPrice(uint _itemId) public view returns (uint) {
        require(_itemId <= itemCount, "Item doesnt exist");
        require(getAuctionAddrCount() > 0, "Dont have any offer address");
        uint _returnId;
        for (uint _id = 0; _id < getAuctionAddrCount(); _id++) {
    ...
    function setFeeAmount(uint _fee) public {
        feeAmount += _fee;  
    }

    function getFeePrice(uint _price) public view returns (uint) {
        return ((_price * feePercent) / 100);
    }

    function getTotalPriceById(uint _itemId) view public returns(uint){
