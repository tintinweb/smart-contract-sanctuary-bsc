...
contract Main_v2 is ERC721Full {
    using SafeMath for uint256;
    ...
    function buyTicket(
        ...
        require(events[eventId] != msg.sender, "Can't buy own event tickets");
        ...
        tickets[msg.sender].push(tokenId);
        return true;
    }

    function refundTicket(uint256[] memory tokenId)
        public
        payable
        returns (bool)
    {
        ...
        address user1 = ownerOf(tokenId[0]);
        address payable owner = address(uint160(user1));
    ...
    function buyMultipleTicket(
        uint256 eventId,
        string[] memory _tokenURI,
        uint256[] memory tokenId
    ) public payable returns (bool) {
        require(_tokenURI.length > 0, "Invalid URI list length");
        require(events[eventId] != address(0), "Invalid Event ID.");
        require(msg.value > 0, "Invalid Ticket Fees.");
        require(events[eventId] != msg.sender, "Can't buy own event tickets");
        ...
        for (uint256 i = 0; i < _tokenURI.length; i++) {
            ...
            tickets[msg.sender].push(tokenId[i]);
        }
        owner.transfer(ownerFee);
        admin.transfer(msg.value.sub(ownerFee));
        return true;
    ...
    function createEvent(uint256 eventId, address _organiser)
        external
        returns (bool)
    {
