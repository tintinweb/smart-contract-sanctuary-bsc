...
contract Staking {
    ...
    bytes32 public constant PACKAGE4 = keccak256("PACKAGE4");
    ...
    mapping(bytes32 => Package) roleToPackage;
    ...
    function initialize() external {
        ...
        roleToPackage[PACKAGE1].apr = 100;
        roleToPackage[PACKAGE1].duration = 30;
        roleToPackage[PACKAGE2].apr = 138;
        roleToPackage[PACKAGE2].duration = 90;
        roleToPackage[PACKAGE3].apr = 220;
        roleToPackage[PACKAGE3].duration = 180;
        roleToPackage[PACKAGE4].apr = 5;
        roleToPackage[PACKAGE4].duration = 0;
    }
    ...
    function setProposal(uint256 percentChange, uint8 action) external {
        ...
        if (action == 0) {
            roleToPackage[PACKAGE1].apr -= percentChange;
            roleToPackage[PACKAGE2].apr -= percentChange;
            roleToPackage[PACKAGE3].apr = percentChange;
        } else {
            roleToPackage[PACKAGE1].apr += percentChange;
            roleToPackage[PACKAGE2].apr += percentChange;
            roleToPackage[PACKAGE3].apr += percentChange;
    ...
    {
        Package[4] memory finalPackage;
        finalPackage[0] = roleToPackage[PACKAGE1];
        finalPackage[1] = roleToPackage[PACKAGE2];
        finalPackage[2] = roleToPackage[PACKAGE3];
        finalPackage[3] = roleToPackage[PACKAGE4];
    ...
    function stake(
        ...
        Package memory finalPackage;
        if (_stakingId == 3) {
            finalPackage = roleToPackage[PACKAGE4];
        } else {
            require(
            ...
            finalPackage = getListPackage(_nftId)[_stakingId];
        }
    ...
    function unstake(uint256 _stakingId) external {
        ...
        // tránh số lẻ
        earned = (_amountUserSpent * staked.apr * stakedTimeClaim) / 12 / 30; // tiền lãi theo ngày * số ngày
        // trả gốc
    ...
    function getEarned(uint256 stakingId) external view returns (uint256) {
        ...
        require(msg.sender == ownerStaking.owner, "Ownable: Not owner");
