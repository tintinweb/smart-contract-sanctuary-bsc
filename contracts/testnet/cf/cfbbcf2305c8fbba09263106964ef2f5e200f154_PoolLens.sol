...
contract PoolLens {
    ...
    function poolAssets(address _pool, address _token) external view returns (PoolAsset memory poolAsset) {
        ...
        AssetInfo memory asset = getPoolAsset(self, _token);
    ...
    function getPoolAsset(IPoolForLens _pool, address _token) public view returns (AssetInfo memory asset) {
        for (uint256 i = 0; i < _pool.getAllTranchesLength();) {
            address tranche = _pool.allTranches(i);
            AssetInfo memory info = _pool.trancheAssets(tranche, _token);
            asset.poolAmount += info.poolAmount;
            asset.reservedAmount += info.reservedAmount;
            asset.totalShortSize += info.totalShortSize;
            asset.guaranteedValue += info.guaranteedValue;
            unchecked {
                ++i;
            }
        }
