...
contract Blockstaking is Context, Auth, IERC20 {
    ...
    uint256 private _totalSupplyManual;
    uint256 private _totalSupplyAuto;
    string private _name = "BlockSAFU Staking";
    string private _symbol = "BSAFU-SP";
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => uint256) public totalDistributeToToken;
    uint256 public totalDistributeToWeth;
    uint256 public totalReceiveWeekBNB;
    uint256 public totalReceiveWeekBSAFU;
    uint256 public totalClaimAutoWeek;
    uint256 public APR;
    uint256 public APY;
    ...
    bool public isCountAPREnable = false;
    uint public loopForBounty = 1;
    ...
    function stake(address account,uint256 amount,bool isAutoStake) external {
      ...
      registeringAddressForAutoStaking(account, isAutoStake);
        if(isAutoStakingAddress[account]) {
            _totalSupplyAuto = _totalSupplyAuto.add(amount);
        } else {
            _totalSupplyManual = _totalSupplyManual.add(amount);
        }
    ...
    function unstake(address account,uint256 amount) external {
      ...
      registeringAddressForAutoStaking(account,false);
        ...
        if(isAutoStakingAddress[account]) {
            if(_totalSupplyAuto >= amount){
                _totalSupplyAuto = _totalSupplyAuto.sub(amount);
            }
        } else {
            if(_totalSupplyManual >= amount) {
                _totalSupplyManual = _totalSupplyManual.sub(amount);
            }
        }
    ...
    function deposit() public payable {
      if(totalShares > 0){
            ...
            if(isCountAPREnable) countAPR(msg.value);
            emit Deposit(msg.sender,msg.value);
      }
    }

    function countAPR(uint256 amount) internal {
        if(block.timestamp.sub(lastResetAPR) >= 7 days) {
            totalReceiveWeekBSAFU = 0;
            totalReceiveWeekBNB = 0;
            totalClaimAutoWeek = 0;
        ...
        uint year = 365;
        uint day = 7;
        APR = totalReceiveWeekBSAFU.mul(percentTaxDenominator).div(_totalSupplyManual).mul(year.div(day)).mul(100).div(percentTaxDenominator);
        uint256 const = 5218;
    }
    ...
    function setEnableCountAPR(bool state) external onlyOwner {
        isCountAPREnable = state;
    }
    ...
    function setLoopBounty(uint _loopForBounty) external onlyOwner {
      loopForBounty = _loopForBounty;
    }

    function setIndexStaking(uint256 _no) external onlyOwner {
        indexCurrentAutoStaking = _no;
    }
    ...
    function registeringAddressForAutoStaking(address account,bool state) internal {
        ...
        for(uint256 j;j<maxLoop;j++){
          if(indexCurrentAutoStaking >= autoStakingAddress.length) {
            indexCurrentAutoStaking = 0;
              totalClaimAutoWeek = totalClaimAutoWeek.add(1);
          }
    ...
    function claimDividendAndAutoStaking(address account) internal {
        ...
        if(amount > 0){
            ...
            path[0] = router.WETH();
            path[1] = tokenAddress;
            IWETH(router.WETH()).approve(routerDexAddress,amount);
            uint256[] memory estimate = router.getAmountsOut(amount,path);
