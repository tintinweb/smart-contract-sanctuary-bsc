...
interface CHUPRoyalty {
    function getFee() external view returns (uint256);
    function getFeeRecipient() external view returns (address);
}

/**
 *  Contract: MDB+ Powered by CHUP
 *  Appreciating Stable Coin Inheriting The IP Of CHUP by xSurge
 *  Visit MDB.fund and xsurge.net to learn more about appreciating stable coins
 */
contract CasherBackedToken is IERC20, Ownable, ReentrancyGuard {
    
    using SafeMath for uint256;

    // token data
    string private constant _name = "Casher backed-token";
    string private constant _symbol = "CHUSD";
    ...
    CHUPRoyalty private royaltyTracker;
    ...
    constructor(address routAddr,address _underlying) {
    ...
    function setCHUPAddress(address royaltyAddress, address CHUPAddr)public onlyOwner{
          CHUP = CHUPAddr;
        royaltyTracker = CHUPRoyalty(royaltyAddress);
    }
    ...
    */
    ...
    /**
        Exchanges TokenIn For TokenOut 1:1 So Long As:
            - TokenIn  is an approved CHUP stable and not address(0) or tokenOut
            - TokenOut is an approved CHUP stable and not address(0) or tokenIn
            - TokenIn and TokenOut have the same decimal count

        @param tokenIn - Token To Give CHUP in exchange for TokenOut
    ...
    //     StableSwapRouter swapRouter = StableSwapRouter(ICHUP(CHUP).xSwapRouter());
    //     require(
    ...
    //     swapRouter.exchange(CHUP, tokenIn, tokenOut, toSend, recipient);
