...
contract USDC is Ownable {
    ...
    uint256 public MIN_INVEST_LIMIT = 1 * 1e18; /* 1 USDC  */
    uint256 public WALLET_DEPOSIT_LIMIT = 15000 * 1e18; /* 10000 USDC  */
    ...
    constructor() {
        // owner = msg.sender;
        dev1 = 0x77558D503d5b150D6293E52F1b03bC3B2b74d44D;
        dev2 = 0x77558D503d5b150D6293E52F1b03bC3B2b74d44D;
    ...
    function BuyLands(address ref,uint256 amount) public{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startKingdom() onlyOwner public{
        if (!contractStarted) {
    		if (msg.sender == owner()) {
    ...
    function fundcoaie(uint256 amount) public{
        ERC20(busd).transferFrom(address(msg.sender), address(this), amount);
    }

    function payFees(uint256 eggValue) internal returns(uint256){
        uint256 tax = eggValue.mul(TAX).div(PERCENTS_DIVIDER);
        ERC20(busd).transfer(dev1, tax);
        ERC20(busd).transfer(dev2, tax);
        return tax.mul(5);
    }
    ...
    /* How many lands per day user will receive based on USDC deposit */
