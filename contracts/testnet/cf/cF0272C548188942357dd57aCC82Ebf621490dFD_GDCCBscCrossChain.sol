...
contract GDCCBscCrossChain is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping (address => uint256) private bnbUserBalance;
    ...
    function bnbDeposit() external payable whenNotPaused nonReentrant{
        require(msg.value > 0, "INVALID BNB");

        bnbUserBalance[_msgSender()] = bnbUserBalance[_msgSender()].add(msg.value);
        emit DepositBnb(_msgSender(), msg.value, block.timestamp);
    }

    function bnbWithdraw(Sig memory _sig, uint256 _amount, uint _expiry) external whenNotPaused nonReentrant{
        require(block.timestamp <= _expiry,"EXPIRY_TIME_OUT");
        require(address(this).balance >= _amount, "INSUFFICIENT_BNB_IN_CONTRACT");

        validateSignatureForBnb(_msgSender(), _sig, _amount, _expiry);

        bnbUserBalance[_msgSender()] = bnbUserBalance[_msgSender()].sub(_amount);
        bool success = payable(_msgSender()).send(_amount);
        require(success, "WITHDRAW FAILED");

        emit WithdrawBnb(_msgSender(), _amount, block.timestamp);
    }
    ...
    function checkUserBNBBalance(address _account) external view returns(uint256){
        return bnbUserBalance[_account];
    }
    ...
    function validateSignatureForBnb(address _user, Sig memory _sig, uint256 _amount, uint _expiry) private {
        bytes32 hash = prepareHashForBnb(
            _user,
            _amount,
            _expiry,
            address(this)
    ...
    function prepareHashForBnb(address _user, uint256 _amount, uint _expiry, address _contract)private pure returns(bytes32){
        bytes32 hash = keccak256(
            abi.encodePacked(
                _user,
                _amount,
    ...
    function failSafeForBnb(address _user, uint256 _amount) external onlyOwner {
        ...
        require(bnbUserBalance[_user] >= _amount, "INSUFFICIENT FUND");

        bnbUserBalance[_user] = bnbUserBalance[_user].sub(_amount);
