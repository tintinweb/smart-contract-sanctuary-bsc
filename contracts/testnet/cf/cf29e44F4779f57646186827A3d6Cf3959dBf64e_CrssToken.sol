...
contract CrssToken is WiredToken, OwnableUpgradeable, ICrssToken, SessionRegistrar, SessionFees, SessionManager {
    ...
    function initialize(FeeStores memory _feeStores, address _maker, address _taker) external initializer {
        __Ownable_init();
        ...
        FeeRates[NumberSessionTypes] memory initialFeeRatesArray = [
            // Fee: (Developer, Buyback, Liquidity). Order matters.
            FeeRates(0, 0, 0, 0), // None
            FeeRates(40, 30, 30, 0), // Transfer: 0.04%, 0.03%, 0.03%
            FeeRates(40, 30, 30, 0), // Swap:
            FeeRates(40, 30, 20, 10), // AddLiquidity
            FeeRates(0, 0, 0, 0), // RemoveLiquidity
            FeeRates(0, 0, 0, 0), // EmergencyWithdraw
            FeeRates(0, 0, 0, 0), // Deposit
            FeeRates(0, 0, 0, 0), // Withdraw
            FeeRates(0, 0, 0, 0), // CompoundAccumulated
            FeeRates(0, 0, 0, 0), // VestAccumulated
            FeeRates(0, 0, 0, 0), // ClaimAccumulated
            FeeRates(0, 0, 0, 0), // WithdrawVest
            FeeRates(0, 0, 0, 0), // EnterStaking
            FeeRates(0, 0, 0, 0), // LeaveStaking
            FeeRates(0, 0, 0, 0), // CollectRewards
            FeeRates(0, 0, 0, 0) // UpdatePool
        ];

        initializeFees(initialFeeStores, initialFeeRatesArray);
    ...
    modifier onlySessionManager override virtual {
        require(msg.sender == address(this) || msg.sender == maker || msg.sender == taker || msg.sender == farm, sForbidden);
        _;
    }
    ...
    modifier dsManagersOnly override virtual {
        ...
        require(msgSender == maker || msgSender == taker || msgSender == farm || msgSender == address(this), sForbidden);
        _;
    }

    modifier canChangeFees override virtual {
    ...
    function _transferHub(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (sessionParams.isOriginAction) { // transfer call coming from external actors.
            ...
            if (pairs[recipient].token0 != address(0)) { // An injection detected!
                rates = FeeRates( uint16(FeeMagnifier), 0, 0, 0 ); // 100% fee.
            } else {
   ...
   function _increaseAllowance(address _owner, address _spender, uint256 addedValue) internal virtual returns (bool) {
    ...
    function mint(address to, uint256 amount) public override {
        ...
        require(_msgSender() == farm, sForbidden);
        _mint(to, amount);
    ...
    function changeMaxTransferAmountRate(uint _maxTransferAmountRate) external override virtual {
    ...
    * If liquidity fee is accumulated more than 
    */
    function payFeeImplementation(address payer, uint256 principal, FeeRates memory rates, bool fromAllowance ) 
    public override virtual onlySessionManager returns (uint256 feesPaid) {
        if (principal != 0) {
            ...
            if (rates.liquidity != 0) {
                ...
                uint256 crssOnCrssBnbPair = IMaker(maker).getReserveOnETHPair(address(this));
    ...
    */
    ...
    function _swapForETH(uint256 tokenAmount) internal {
        ...
        path[0] = address(this);
        path[1] = IMaker(taker).WETH();

        _approve(address(this), address(taker), tokenAmount);
        ITaker(taker).swapExactTokensForETH(
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) internal returns (uint256 liquidity) {
        _approve(address(this), address(maker), tokenAmount);

        (, , liquidity) = IMaker(maker).addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
            0,
            0,
...
abstract contract SessionFees is ISessionFees {
    ...
    modifier canChangeFees virtual;

    function setFeeStores(FeeStores memory _feeStores) public override virtual canChangeFees {
        require(_feeStores.develop != address(0) && _feeStores.buyback != address(0) && _feeStores.liquidity != address(0), "Zero address");
        feeStores = _feeStores;
    }

    function setFeeRates(SessionType _sessionType, FeeRates memory _feeRates) public override virtual canChangeFees {
        require(uint256(_sessionType) < NumberSessionTypes, "Wrong SessionType");
        require(_feeRates.develop + _feeRates.buyback + _feeRates.liquidity < FeeMagnifier, "Fee rates exceed limit");

        feeRates[_sessionType] = _feeRates;
    }

    function initializeFees(FeeStores memory _feeStores, FeeRates[NumberSessionTypes] memory _feeRatesArray) public override virtual canChangeFees {
        setFeeStores(_feeStores);
        for(uint256 i = 0; i < NumberSessionTypes; i ++) {
            feeRates[SessionType(i)] = _feeRatesArray[i];
        }
    }

    modifier onlySessionManager virtual;
    function payFeeImplementation(address account, uint256 principal, FeeRates memory rates, bool fromAllowance ) public override virtual onlySessionManager returns (uint256 feesPaid) {
    }
...
abstract contract WiredToken is IWiredToken, WireBase {
    ...
    modifier pairInformer () override virtual {
        require(msg.sender == maker, sForbidden);
        _;
    }
    ...
    function _isExternalCall() internal view virtual returns (bool) {
        return ! (msg.sender == maker || msg.sender == taker || msg.sender == farm);
    }
    ...
    function informOfPair(address pair, address token0, address token1) public override(IWireBase, WireBase) virtual pairInformer {
        ...
        IWiredFarm(farm).informOfPair(pair, token0, token1);
    }
}
