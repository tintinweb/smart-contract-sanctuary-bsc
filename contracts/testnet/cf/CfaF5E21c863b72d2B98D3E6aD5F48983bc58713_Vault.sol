 ...
 */
contract Vault is ERC20, ReentrancyGuard, AccessControl {
    ...
    event SafeFarm(address account, uint256 shares);
    ...
    */
    ...
    function calcShares(
        ...
        shares = balanceOf(_account) * _percent / 100;
        return shares;
    }
     ...
     */
    function safeFarm(
        address _account, uint256 _percent, address[] memory route
    ) external onlySFOracle {
        uint256 shares = calcShares(_account, _percent);
        _burn(_account, shares);
        strategy.safeFarm(_account, shares, totalSupply(), route);

        emit SafeFarm(_account, shares);
    }
    ...
    function safeFarm(
        ...
        uint256 shares = calcShares(_account, _percent);
        _burn(_account, shares);
        strategy.safeFarm(_account, shares, totalSupply(), route0, route1);
...
interface IStrategy {
    ...
    function STRATEGY_TYPE() external view returns (bytes32);
    function calcSharesAmount(
        uint256 share, uint256 totalShares
    ) external view returns (uint256 amount);
    function safeFarm(address account,
        uint256 share, uint256 totalShares,
