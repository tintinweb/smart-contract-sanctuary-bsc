...
contract ChainlinkRandomNumberGenerator is RandomNumberGeneratorLogic, VRFConsumerBaseV2Upgradeable {
    ...
    // mapping from chainlink request IDs to local request IDs
    mapping(uint256 => uint256) private _chainlinkToLocalRequestIds;
     ...
     */
    function _requestRandomNumbers(uint256 localRequestId, uint32 count) internal virtual override {
        uint256 chainlinkRequestId = COORDINATOR.requestRandomWords(
            s_keyHash,
            s_subscriptionId,
            s_requestConfirmations,
            s_callbackBaseGasLimit + s_callbackAdditionalGasLimit * count,
            count
        );

        // map it to our local ID so we can find it later
        _chainlinkToLocalRequestIds[chainlinkRequestId] = localRequestId;
    }

    /**
     * Chainlink calls this function when the random numbers are generated.
     */
    function fulfillRandomWords(uint256 chainlinkRequestId, uint256[] memory randomNums) internal override {
        uint256 localRequestId = _chainlinkToLocalRequestIds[chainlinkRequestId];

        // call our own function to set the random numbers for retrieval later
        _setRandomNumbers(localRequestId, randomNums);
    }
...
abstract contract RandomNumberGeneratorLogic is UUPSUpgradeable, AccessControlEnumerableUpgradeable, IRandomNumberGenerator {
    ...
    uint256 private _nextRequestId; // request ID
     ...
     */
    function requestRandomNumbers(uint32 count) external virtual override onlyRole(REQUESTER_ROLE) returns (uint256 requestId) {
        require(count > 0, "RandomNumberGeneratorLogic: Request count must be more than zero");

        requestId = _nextRequestId;
        ++_nextRequestId;
        _requestRandomNumbers(requestId, count);
     ...
     */
    function _requestRandomNumbers(uint256 requestId, uint32 count) internal virtual;
    ...
    function _setRandomNumbers(uint256 requestId, uint256[] memory randomNums) internal {
        require(!_requestsCompleted[requestId], "RandomNumberGeneratorLogic: Random number already set");
