...
contract EverRise is Context, IERC20, Ownable {
    ...
    uint256 public _liquidityFee = 7; // marketingDivisor + auto lp + burn 4 + 2 + 1
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && to == uniswapV2Pair) {
            if (overMinimumTokenBalance) {
                contractTokenBalance = minimumTokensBeforeSwap.div(_liquidityFee).mul(marketingDivisor.add(_burnFee)); // exchange 4 + 1 marketing + burn
                swapTokens(contractTokenBalance);    // marketing Divisor reward to market wallet in bnb. 4%
                swapAndLiquify(minimumTokensBeforeSwap.sub(contractTokenBalance)); // auto lp for 2%
            }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(marketingAddress, transferredBalance.mul(marketingDivisor).div(marketingDivisor.add(_burnFee)));
