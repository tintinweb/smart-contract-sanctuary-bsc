...
contract CrotaleToken is ERC20, Ownable {
    ...
    address public assuranceReceiver;
    address public autoSnakePit;
    ...
    uint256 public superfarmerFee;
    uint256 public assuranceFee;
    ...
    constructor(
        ...
        totalFee = autoliquidityFee.add(superfarmerFee).add(assuranceFee).add(dividendFee).add(snakePitFee);
        ...
        gonSwapThreshold = gonsTotal.div(10000).mul(1);
        ...
        _isFeeExempt[assuranceReceiver] = true;
    ...
    function rebase() internal {
        ...
        gonsPerFragment = gonsTotal.div(supplyTotal);
        lastRebasedTime = lastRebasedTime.add(times.mul(30 minutes));
    ...
    function takeFee(address _from, address _to, uint256 _gonAmount) internal  returns (uint256) {
        ...
        if (_to == pair) {
            ...
            _superfarmerFee = superfarmerFee.add(sellFee);
        }

        uint256 feeAmount = _gonAmount.mul(_totalFee).div(feeDenominator);
       
        _gonBalances[autoSnakePit] = _gonBalances[autoSnakePit].add(
            _gonAmount.mul(snakePitFee).div(feeDenominator)
        );

        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            _gonAmount.mul(_superfarmerFee).div(feeDenominator)
        );

        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            _gonAmount.mul(assuranceFee).div(feeDenominator)
        );
        ...
        _gonBalances[autoLiquidityReceiver] = _gonBalances[autoLiquidityReceiver].add(
            _gonAmount.mul(autoliquidityFee).div(feeDenominator)
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 feeTotal = superfarmerFee.add(assuranceFee).add(dividendFee);

        try distributor.deposit {
            value: amountETHToRespectiveReceiver.mul(dividendFee).div(feeTotal)
        } () {} catch {}

        (bool success, ) = payable(superfarmerReceiver).call{
            value: amountETHToRespectiveReceiver.mul(superfarmerFee).div(feeTotal),
            gas: 30000
        }("");

        (success, ) = payable(assuranceReceiver).call{
            value: amountETHToRespectiveReceiver.mul(assuranceFee).div(feeTotal),
            gas: 30000
        }("");
    ...
    function addLiquidity() internal swapping {
        ...
        uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : autoliquidityFee;
    ...
    function shouldRebase() internal view returns (bool) {
        return autoRebase && (supplyTotal < supplyMax) && _msgSender() != pair  && !inSwap && block.timestamp >= (lastRebasedTime + 30 minutes);
    }
    ...
    function shouldAddLiquidity() internal view returns (bool) {
        return autoAddLiquidity && !inSwap && _msgSender() != pair && block.timestamp >= (lastAddLiquidityTime + 6 hours);
    }
     ...
     * 1 - Super Farmer receiver
     * 2 - Assurance receiver
     * 3 - Auto Snake Pit
     */
     ...
     */
    function setFeeReceivers(address _autoLiquidityReceiver, address _superfarmerReceiver, address _assuranceReceiver, address _autoSnakePit) external authorized {
        _setFeeReceivers(_autoLiquidityReceiver, _superfarmerReceiver, _assuranceReceiver, _autoSnakePit);
    }

    /**
     * @dev Run internally to set all the fee receiver settings.
     */
    function _setFeeReceivers(address _autoLiquidityReceiver, address _superfarmerReceiver, address _assuranceReceiver, address _autoSnakePit) internal {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        superfarmerReceiver = _superfarmerReceiver;
        assuranceReceiver = _assuranceReceiver;
        autoSnakePit = _autoSnakePit;
    }
