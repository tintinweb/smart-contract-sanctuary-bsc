...
contract NFTSales is Ownable, ERC721Min, ReentrancyGuard {
    ...
    uint256 priceForThreeWithToken;

    address public pairedToken;
    uint256 public pairedTokenPerNFT;
    uint256 pairedTokenPerNFTForThree;
    ...
    function buyOneWithToken() external {
        ...
        IERC20(paymentToken).transferFrom(_msgSender(), address(this), priceWithToken);
        IERC20(pairedToken).transferFrom(_msgSender(), deadAddress, pairedTokenPerNFT);
        require(maxMintForOne > purchaseCount, "EXCEED_MAX_SALE_SUPPLY");
        require(maxPerAddress == 0 || balanceOf(_msgSender()) < maxPerAddress, "EXCEED_MAX_PER_USER");
        purchaseCount++;
        Purchases[_msgSender()].push(Purchase({ amount: 1, purchaseTime: block.timestamp, claimed: false }));
        emit BuyOneWithToken(_msgSender());
    }     

    function buyThreeWithToken() external {
        require(saleActive, "SALE_CLOSED");
        IERC20(paymentToken).transferFrom(_msgSender(), address(this), priceForThreeWithToken);
        IERC20(pairedToken).transferFrom(_msgSender(), deadAddress, pairedTokenPerNFTForThree);
        require(maxMintForThree > purchaseCount, "EXCEED_MAX_SALE_SUPPLY");
        require(maxPerAddress == 0 || balanceOf(_msgSender()) < maxPerAddressForThree, "EXCEED_MAX_PER_USER");
        purchaseCount += 3;
        Purchases[_msgSender()].push(Purchase({ amount: 3, purchaseTime: block.timestamp, claimed: false }));
        emit BuyThreeWithToken(_msgSender());
    }

    function buyWithToken(uint16 amount) external {
        require(saleActive, "SALE_CLOSED");
        IERC20(paymentToken).transferFrom(_msgSender(), address(this), priceWithToken * amount);
        IERC20(pairedToken).transferFrom(_msgSender(), deadAddress, pairedTokenPerNFT * amount);
        ...
        Purchases[_msgSender()].push(Purchase({ amount: amount, purchaseTime: block.timestamp, claimed: false }));
        emit BuyWithToken(_msgSender(), amount);
    }

    function claim() external {
        Purchase[] storage purchases = Purchases[_msgSender()];
        require(purchases.length > 0, "NO_PURCHASES");
        uint256 claimable;
        for(uint256 x; x < purchases.length; x++) {
            if (purchases[x].claimed || purchases[x].purchaseTime + lockTime > block.timestamp) continue;
            claimable += purchases[x].amount;
            purchases[x].claimed = true;
        }
        for(uint256 x; x < claimable; x++) {
            _mintToSender();
        }
    ...
    function getPurchases(address user) external view returns(Purchase[] memory) {
        Purchase[] memory result = new Purchase[](Purchases[user].length);
        for(uint256 x; x < Purchases[user].length; x++) {
            result[x] = Purchases[user][x];
        }
        return result;
    }
    ...
    function mintThree(address receiver) external onlyProxy {
        ...
        _mint(receiver);
        _mint(receiver);
        emit MintThree(_msgSender(), receiver);
    }   
    ...
    function distributeTokens() external nonReentrant {
        ...
        for(uint256 x = 1; x <= feeRecipientCount; x++) {
            ...
            amount = amount > IERC20(paymentToken).balanceOf(address(this)) 
    ...
    function gift(address[] calldata receivers, uint256[] memory amounts) external onlyOwner {
        for (uint256 x; x < receivers.length; x++) {
            for (uint256 i; i < amounts[x]; i++) {
                _mint(receivers[x]);
            }
        }
    }

    function updateConfig(uint16 _maxMint, uint16 _maxPerMint,  
        uint16 _maxPerAddress, bool _saleActive, string calldata _uri,
        address _paymentToken, uint256 _priceWithToken, address _pairedToken,
        uint256 _pairedTokenPerNFT, uint256 _lockTime) external onlyOwner
    {
        maxMint = _maxMint+1;
        ...
        pairedTokenPerNFT = _pairedTokenPerNFT;
        pairedTokenPerNFTForThree = _pairedTokenPerNFT * 3;
        lockTime = _lockTime * 1 days;
        emit UpdateConfig(_msgSender());
    }

    function setLockTime(uint256 value) external onlyOwner {
        lockTime = value * 1 days;
    }
    ...
    function updatePairedToken(address _pairedToken, uint256 _pairedTokenPerNFT) external onlyOwner {
        pairedToken = _pairedToken;
        pairedTokenPerNFT = _pairedTokenPerNFT;
        pairedTokenPerNFTForThree = _pairedTokenPerNFT * 3;
    }
    ...
    event BuyOneWithToken(address indexed user);
    event BuyThreeWithToken(address indexed user);
    event BuyWithToken(address indexed user, uint256 indexed amount);
    event Claim(address indexed user, uint256 indexed claimable);
    event MintOne(address indexed user, address indexed receiver);
    event MintThree(address indexed user, address indexed receiver);
    event Mint(address indexed user, address indexed receiver, uint256 indexed amount);
    event UpdateConfig(address indexed user);
}
