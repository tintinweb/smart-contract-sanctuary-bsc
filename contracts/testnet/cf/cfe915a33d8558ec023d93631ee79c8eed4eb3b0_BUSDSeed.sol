...
contract BUSDSeed {
    ...
    IERC20 public token = IERC20(0x0145634232364EbA74F6FBaCa309CeA587287c30); //** test token
	...
	constructor(address payable wallet) public {
		...
		admin=wallet;
		admins.push(wallet);
        plans.push(Plan(10 minutes, 500,5e18));
		plans.push(Plan(10 minutes, 1000,5e18));
		plans.push(Plan(180 days, 1500,10000e18));       
		plans.push(Plan(356 days, 2000,10000e18));
	}
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		require(amounts >= plans[plan].minAmount,"Amount must be greater then minimum amount");
		...
		user.deposits.push(Deposit(plan, amounts,  block.timestamp, 0,false));
	...
	function withdraw(uint256 index) public {
		...
		require(plans[user.deposits[index].plan].time+user.deposits[index].start<block.timestamp,"Locking period is not over!");
		require(user.deposits[index].isWithdraw==false,"You already withdraw your rewards!");
	...
	function getPlanInfo(uint8 plan) public view returns(uint256 time, uint256 percent,uint256 minAmount) {
	...
	function getUserDividends(address userAddress,uint256 index) public view returns (uint256) {
		...
		uint256 totalAmount;
		uint256 finish = user.deposits[index].start.add(plans[user.deposits[index].plan].time.mul(1 days));
		if (user.checkpoint < finish) {
			uint256 share = user.deposits[index].amount.mul(plans[user.deposits[index].plan].percent).div(PLANPER_DIVIDER);
	...
	function getUserSeedIncome(address userAddress,uint256 index) public view returns (uint256){
		...
		for	(uint256 y=1; y<= count; y++)
		{
			...
			User storage downline =users[addressdownline];
				...
				uint256 finish = downline.deposits[index].start.add(plans[downline.deposits[index].plan].time.mul(1 days));
				if (downline.deposits[index].start < finish) {
					uint256 share = downline.deposits[index].amount.mul(plans[downline.deposits[index].plan].percent).div(PLANPER_DIVIDER);
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 percent, uint256 amount, bool isWithdraw, uint256 start, uint256 finish) {
	    User storage user = users[userAddress];
