...
contract BaseRegistrarImplementation is ERC721, IBaseRegistrar, Ownable  {
    ...
    constructor(SID _sid, bytes32 _baseNode, IBaseRegistrar _prev) ERC721("","") {
        sid = _sid;
        baseNode = _baseNode;
        prev = _prev;
    }
	...
	if (!_exists(tokenId)) {
            prev.transferFrom(from, to, tokenId);
	}
        require(
	...
	if (!_exists(tokenId)) {
            prev.safeTransferFrom(from, to, tokenId, _data);
	}
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            "ERC721: caller is not approved or owner"
	...
	if (!_exists(tokenId)) {
	  // see if previous registrar has this NFT
          return prev.ownerOf(tokenId); 
        }
    ...
    function renew(uint256 id, uint duration) external override live onlyController returns(uint) {
        require(nameExpires(id) + GRACE_PERIOD >= block.timestamp); // Name must be registered here or in grace period
        require(nameExpires(id) + duration + GRACE_PERIOD > duration + GRACE_PERIOD); // Prevent future overflow
