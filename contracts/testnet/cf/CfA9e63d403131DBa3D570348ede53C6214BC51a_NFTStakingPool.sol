...
contract NFTStakingPool is INFTStakingPool, Ownable, IERC721Receiver {
	...
	mapping(address => NftStakedInfo) public stakers;
	...
	function stake(uint256[] calldata ids) external {
		...
		NftStakedInfo storage user = stakers[msg.sender];
	...
	function withdraw(uint256[] calldata ids) external {
		...
		require(block.timestamp >= user.lastDepositTime + LOCK_TIME, "withdraw: too early to withdraw");
	...
	function claimVNS(address _user) public {
		...
		NftStakedInfo storage user = stakers[_user];
	...
	function pendingVNSWithEdit(address _user) internal returns (uint256) {
		...
		uint256 sum;
		if (user.ids.length > 0) {
			uint256 reward = ((block.timestamp - user.lastVNSClaimTime) * DAY_QUOTA) / 1 days;
	...
	function pendingCPD(address _user) public view returns (uint256) {
		NftStakedInfo storage user = stakers[_user];
		return (user.ids.length * (block.timestamp - user.lastCPDClaimTime) * DAY_QUOTA) / 1 days;
	}
