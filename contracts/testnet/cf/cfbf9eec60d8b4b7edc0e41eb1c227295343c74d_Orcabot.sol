...
contract Orcabot is Ownable{
    ...
    event NewUser(address indexed from, string indexed email);

    function buyYourLicenceHere(string calldata email) external payable{
        require(msg.value >= price, "insufficient funds");
        payable(address(this)).transfer(price);
        emit NewUser(msg.sender, email);
    }
