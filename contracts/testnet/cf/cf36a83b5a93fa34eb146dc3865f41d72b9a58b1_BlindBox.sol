...
// File: contracts/nft/YooshiNFT.sol

pragma solidity ^0.6.12;

contract BlindBox is ERC721Tradable{

    mapping(uint256 => uint8[]) private levels;
    ...
    function mint(address to, uint256 tokenId,uint8[] calldata level) public onlyMint returns (bool) {
        _safeMint(to, tokenId);
        for (uint256 index = 0; index < level.length; index++) {
            levels[tokenId].push(level[index]);
        }
        return true;
    }

    function getLevel(uint256 tokenId) public view returns (uint8[] memory) {
        require(_exists(tokenId), "ERC721Metadata: level set of nonexistent token");
        uint8[] memory level = levels[tokenId];
        return level;
    }

    function setLevel(uint256 tokenId,uint8 index,uint8 _level) public onlyMint returns (uint256) {
        require(_exists(tokenId), "ERC721Metadata: level set of nonexistent token");
        require(index <= levels[tokenId].length,"exceed the array length");
        if(index < levels[tokenId].length){
            levels[tokenId][index] = _level;
        }else{
            levels[tokenId].push(_level);
        }

        return levels[tokenId].length;
    }
    /**
