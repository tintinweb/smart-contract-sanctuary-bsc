...
contract McashDev is ERC20, Ownable {
    using SafeMath for uint8;
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;

    address public usdtTokenAddress = 0x426554d98c6a35098f68A7C87f7FF605D2121a7e;
    address public mebTokenAddress = 0x1FFCF7F610fc615d0dc2d104B25a8bce452a66F0;
    address public mebUsdtLpAddress = 0x7c8D94217B4bCA5e8FB8E49b1B9b3699Fb9025F9;
    address public mcashMebLpAddress = 0x7c8D94217B4bCA5e8FB8E49b1B9b3699Fb9025F9;

    mapping(address => bool) public allAccounts;
    EnumerableSet.AddressSet private pairAddressSet;

    uint256 public maxUsdtValuePerTransactionLimit = 500_0000 * 10**18;
    uint256 public maxAmountPerAddressLimit = 500_0000 * 10**18;
    ...
    uint8 levelOneFriendRate = 8;
    uint8 levelTwoFriendRate = 3;
    uint8 levelThreeFriendRate = 1;
    uint256 levelOneFriendMebBalanceLimit = 1000 * 10**18;
    uint256 levelTwoFriendMebBalanceLimit = 2000 * 10**18;
    uint256 levelThreeFriendMebBalanceLimit = 3000 * 10**18;
    ...
    uint8 treasuryRate = 3;
    uint256 public treasuryBalance = 0;

    uint8 creationMemberRate = 2;
    ...
    function _transfer(
        ...
        require(this.balanceOf(to).add(amount) <= maxAmountPerAddressLimit, "The limit of a single account is exceeded");

        if (freeFeeAccounts[from] || freeFeeAccounts[to]) {
            super._transfer(from, to, amount);
        } else if (pairAddressSet.contains(from)) {
            require(amount.mul(_getMCashToMebPrice()).mul(_getMebToUsdtPrice()) < maxUsdtValuePerTransactionLimit, "The limit of a single transaction is exceeded");
            super._transfer(from, to, amount);
            uint256 exchangeBuyFee = amount.mul(exchangeBuyRate).div(100);
            amount = amount.sub(exchangeBuyRate);
            _processBuyExchangeFee(to, exchangeBuyFee);
            super._takeOutExchangeBuyFee(to, exchangeBuyFee);
        } else if (pairAddressSet.contains(to)) {
            require(_getMCashToMebPrice().mul(amount) < maxUsdtValuePerTransactionLimit, "The limit of a single transaction is exceeded");
            uint256 exchangeSellFee = amount.mul(exchangeSellRate).div(100);
            amount = amount.sub(exchangeSellFee);
            _processSellExchangeFee(exchangeSellFee);
            super._transfer(from, address(this), exchangeSellFee);
            super._transfer(from, to, amount);
        } else {
            uint256 standardTransferFee = amount.mul(standardTransferRate).div(100);
            amount = amount.sub(standardTransferFee);
            _processStandardTransferFee(standardTransferFee);
            super._transfer(from, address(this), standardTransferFee);
            super._transfer(from, to, amount);
        }
    ...
    function rewardWithdraw() public {
        require(availableRewardBalance[msg.sender] > 0, "Insufficient of available reward balance");
        ...
        IERC20(address(this)).transfer(msg.sender, availableRewardBalance[msg.sender]);
        availableRewardBalance[msg.sender] = 0;
    }

    function set_invite_address(address _parentAddress) public {
        require(allAccounts[_parentAddress], "parent account is not exist");
        require(accountParentAddress[msg.sender] == address(0), "AccountParentAddress is exist");
        accountParentAddress[msg.sender] = _parentAddress;
        levelOneFriendCount[_parentAddress] = levelOneFriendCount[_parentAddress].add(1);

        address levelTwoFriendAddress = accountParentAddress[_parentAddress];
        ...
        if (levelThreeFriendAddress != address(0)) {
            levelTwoFriendCount[levelThreeFriendAddress] = levelTwoFriendCount[levelThreeFriendAddress].add(1);
        }
    }

    function account_invite(address inviteAddress) public {
    ...
    function applyAgent() public {
        ...
        require(IERC20(mebTokenAddress).balanceOf(msg.sender) > levelOneFriendMebBalanceLimit, "Insufficient Balance of MEB");
        agentAddressSet.add(msg.sender);
    }

    function addPairAddress(address _pairAddress) public onlyOwner {
        require(!pairAddressSet.contains(_pairAddress), "Mcash: Address is already exist");
        pairAddressSet.add(_pairAddress);
    }

    function removePairAddress(address _pairAddress) public onlyOwner {
        require(pairAddressSet.contains(_pairAddress), "Mcash: Address is not exist");
    ...
    function addCreationMemberAddress(address _creationMemberAddress) public onlyOwner {
        require(!pairAddressSet.contains(_creationMemberAddress), "Mcash: Address is already exist");
        creationMemberSet.add(_creationMemberAddress);
    }

    function removeCreationMemberAddress(address _creationMemberAddress) public onlyOwner {
        require(creationMemberSet.contains(_creationMemberAddress), "Mcash: Address is not exist");
        creationMemberSet.remove(_creationMemberAddress);
    }
    ...
    function processCreationMemberFee() public onlyOwner {
    ...
    function processLpHoldersBalanceFee() public onlyOwner {
        uint256 totalMcashMebLPAmount = 0;
        for (uint256 i = 0; i < agentAddressSet.length(); i++) {
            totalMcashMebLPAmount = totalMcashMebLPAmount.add(_getMcashMebLpBalance(agentAddressSet.at(i)));
        }
        for (uint256 i = 0; i < agentAddressSet.length(); i++) {
            uint256 agentRewardAmount = lpHoldersBalance.div(totalMcashMebLPAmount).mul(_getMcashMebLpBalance(agentAddressSet.at(i)));
    ...
    function processTreasuryBalanceFee(address _toAddress, uint256 _amount) public onlyOwner {
        ...
        IERC20(address(this)).transfer(_toAddress, _amount);
    }

    function setMaxAmountMebPerTransactionLimit(uint256 _amount) public onlyOwner {
        require(maxUsdtValuePerTransactionLimit != _amount, "MaxUsdtValuePerTransactionLimit is already the value");
        maxUsdtValuePerTransactionLimit = _amount;
    }

    function setMaxAmountPerAddressLimit(uint256 _amount) public onlyOwner {
        require(maxAmountPerAddressLimit != _amount, "MaxAmountPerAddressLimit is already the value");
        maxAmountPerAddressLimit = _amount;
    }

    function setmMcashMebLPAddress(address _address) public onlyOwner {
        require(mcashMebLpAddress != _address, "McashMebLPAddress is already the value");
        mcashMebLpAddress = _address;
    }
    ...
    function initParentAddress(address[] calldata _address, address[] calldata _parentAddress) public onlyOwner {
        ...
        for (uint256 i = 0; i < _address.length; i++) {
            ...
            levelOneFriendCount[_address[i]] = levelOneFriendCount[_address[i]].add(1);
            address levelTwoFriendAddress = accountParentAddress[_address[i]];
            if (levelTwoFriendAddress != address(0)) {
                levelTwoFriendCount[levelTwoFriendAddress] = levelTwoFriendCount[levelTwoFriendAddress].add(1);
            }
            address levelThreeFriendAddress = accountParentAddress[levelTwoFriendAddress];
    ...
    function _processBuyExchangeFee(address buyAddress, uint256 exchangeBuyFee) internal {
        ...
        address levelOneFriend = accountParentAddress[buyAddress];
    ...
    function _processSellExchangeFee(uint256 exchangeSellFee) internal {
    ...
    function _processStandardTransferFee(uint256 standardTransferFee) internal {
    ...
    function _getMcashMebLpBalance(address _address) internal view returns (uint256) {
        return IERC20(mcashMebLpAddress).balanceOf(_address);
    }
    ...
    function _verifySatisfyAgentLevel(address _address, uint8 _rewardLevel) internal returns (bool) {
        ...
        uint256 mebTokenBalance = IERC20(mebTokenAddress).balanceOf(msg.sender);
        if (mebTokenBalance >= levelThreeFriendMebBalanceLimit) {
            return _rewardLevel <= 3;
        } else if (mebTokenBalance >= levelTwoFriendMebBalanceLimit) {
            return _rewardLevel <= 2;
        } else if (mebTokenBalance >= levelOneFriendMebBalanceLimit) {
    ...
    function _getMCashToMebPrice() internal view returns (uint256) {
        uint256 mebPairBalance = IERC20(mebTokenAddress).balanceOf(mcashMebLpAddress);
        uint256 mcashPairBalance = this.balanceOf(mcashMebLpAddress);
        return mebPairBalance.div(mcashPairBalance);
    }

    function _getMebToUsdtPrice() internal view returns (uint256) {
        uint256 mebPairBalance = IERC20(mebTokenAddress).balanceOf(mebUsdtLpAddress);
        uint256 usdtPairBalance = IERC20(usdtTokenAddress).balanceOf(mebUsdtLpAddress);
        return usdtPairBalance.div(mebPairBalance);
    }
}
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    /**
     *  @dev Take out `_fee` form the balance of `_address`.
     *
     */
    function _takeOutExchangeBuyFee(address _address,uint256 _fee) internal virtual {
        require(_balances[_address] > _fee,"ERC20: take out fee amount exceeds balance");
        _balances[_address] -=  _fee;
    }
}
