...
contract HexaFinityToken is Context, IERC20, Ownable {
    ...
    string private _name = "HexaFinity-V2";
    ...
    function _getTValues(uint256 tAmount) private view returns (tFeeValues memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(calculateTaxFee).sub(calculateLiquidityFee);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        //calculate burn amount and owner fee amount
        uint256 burnAmt = amount.mul(_burnFee).div(10**3);
        uint256 ownerAmt = amount.mul(_ownerFee).div(10**3);
        uint256 transAmount = amount.sub(burnAmt).sub(ownerAmt);

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, transAmount);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, transAmount);
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, transAmount);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, transAmount);
        } else {
            _transferStandard(sender, recipient, transAmount);
        }
        ...
        _transferStandard(sender, _taxReceiverAddress, ownerAmt);
