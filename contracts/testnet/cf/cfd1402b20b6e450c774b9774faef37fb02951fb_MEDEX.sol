...
contract MEDEX is MedexERC20 {
    ...
    uint256 public busdPerMedex = 118 * 1e18;
    uint256 public liqudityProviderFees = 5;
    ...
    function addLiquidity(uint256 amountA, uint256 amountB) public {
        ...
        require(
            amountA <= userBalance && amountA <= approveValue,
            "Insuffcient Funds"
        );
        ...
        require(
            amountB <= userBalanceNew && amountB <= approveValueNew,
            "Insuffcient Funds"
        );
        ...
        BEP20(tokenA).transferFrom(msg.sender, address(this), amountA);
        BEP20(tokenB).transferFrom(msg.sender, address(this), amountB);

        uint256 balance0 = BEP20(tokenA).balanceOf(address(this));
        uint256 balance1 = BEP20(tokenB).balanceOf(address(this));
        uint256 amount0 = balance0.sub(_reserve0);
        uint256 amount1 = balance1.sub(_reserve1);
        uint256 valueAmount = amountA * amountB;

        uint256 _totalSupply = totalSupply;
        uint256 liquidity;
        if (_totalSupply == 0) {
            liquidity = SafeMath.sqrt(valueAmount);
        } else {
            liquidity =
                amount0.mul(_totalSupply) /
                _reserve0.min(amount1.mul(_totalSupply) / _reserve1);
        }
    ...
    function removeLiquidity() external {
        ...
        _transferLiq(msg.sender, address(this), liquidity);
    ...
    function removeLiquidity1() external {
        uint256 balance0 = BEP20(tokenA).balanceOf(address(this));
        uint256 balance1 = BEP20(tokenB).balanceOf(address(this));
        uint256 liquidity = balanceOf[msg.sender];
    ...
    function buy(uint256 tokenIN, uint256 amountIn) external {
        uint256 balance0 = BEP20(tokenA).balanceOf(address(this));
        uint256 balance1 = BEP20(tokenB).balanceOf(address(this));
        if (tokenIN == 1) {
            uint256 amount = (busdPerMedex / 1e18) * (amountIn / 1e18);
            amount = amount * 1e18;

            if (amount >= balance1) {
                BEP20(tokenA).transferFrom(msg.sender, address(this), amountIn);
                BEP20(tokenB).transferFrom(address(this), msg.sender, amount);
            }
        } else {
            uint256 amount = (amountIn / 1e18) / (busdPerMedex / 1e18);
            amount = amount * 1e18;
            if (amount >= balance0) {
                BEP20(tokenA).transferFrom(address(this), msg.sender, amount);
                BEP20(tokenB).transferFrom(msg.sender, address(this), amountIn);
            }
        }
    }
}