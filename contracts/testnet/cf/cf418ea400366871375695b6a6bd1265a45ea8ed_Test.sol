...
contract Test is Ownable {
    ...
    function addToken(address addressToken, uint256 amount) external onlyAdmin{
        MAoE MATK = MAoE(addressToken);
        MATK.transferFrom(msg.sender ,address(this), amount);
        uint256 bf = tokenInContract[addressToken];
        tokenInContract[addressToken] = bf + amount;
    ...
    function createRandom() public returns(uint256) {
        return uint256(blockhash(block.number -1 ))%(numOfToken);
    }
}