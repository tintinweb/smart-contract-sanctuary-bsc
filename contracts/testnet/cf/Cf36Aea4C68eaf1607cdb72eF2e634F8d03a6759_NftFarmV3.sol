...
contract NftFarmV2Proof is SafeOwnable, ERC1155 {

    constructor() SafeOwnable(msg.sender) ERC1155("") {
    }
...
import "./NftFarmV2Proof.sol";
import 'hardhat/console.sol';
import "./P2EERC1155V2.sol";
import "./P2EERC1155.sol";

contract NftFarmV3 is SafeOwnable, ReentrancyGuard, ERC1155Holder {
    using SafeMath for uint256;
    ...
    NftFarmV2Proof public immutable prooftERC1155;
    P2EToken public rewardToken;
    uint256 public rewardPerBlock;
    uint256 public BONUS_MULTIPLIER;
    P2EERC1155 public v1Contract;
    P2EERC1155V2 public Nft;
    mapping(address => bool) public whitelist;
    mapping(address => bool) public avaliableNft;
    mapping(address => mapping (uint256 => bool)) public avaliableNftIds;
    ...
    function getNftId(address _nftContract, uint _nftId) internal returns (uint) {
        ...
        if (currentId == 0) {
            currentId = uint256(keccak256(abi.encodePacked(_nftContract, _nftId)));
            nftIds[_nftContract][_nftId] = currentId;
            emit NftId(_nftContract, _nftId, currentId);
        }
        return currentId;
    }
    ...
    event NftId(address nft, uint oldId, uint newId);
    event AddWhitelist(address addr);
    event DelWhitelist(address addr);
    event AddNftId(address nft, uint id);
    event DelNftId(address nft, uint id);
    event NewNft(address nft, address owner, uint[] ids, uint[] values);
    ...
    constructor(
        ...
        BONUS_MULTIPLIER = 1;
        prooftERC1155 = new NftFarmV2Proof();
        rewardVault = _rewardVault;
    }

    function addWhitelist(address _addr) external onlyOwner {
        require(!whitelist[_addr], "addr already whitelist");
        whitelist[_addr] = true;
        emit AddWhitelist(_addr);
    }

    function delWhitelist(address _addr) external onlyOwner {
        require(whitelist[_addr], "addr not in whitelist");
        delete whitelist[_addr];
        emit DelWhitelist(_addr);
    }

    modifier onlyOwnerOrWhitelist() {
        require(msg.sender == owner() || whitelist[msg.sender], "only owner can do this");
        _;
    }

    function addNftIds(address _nft, uint[] memory _ids) external onlyOwner {
        if (!avaliableNft[_nft]) {
            avaliableNft[_nft] = true;
        }
        for (uint i = 0; i < _ids.length; i ++) {
            require(!avaliableNftIds[_nft][_ids[i]], "id already exist");
            avaliableNftIds[_nft][_ids[i]] = true;
            emit AddNftId(_nft, _ids[i]);
        }
    }

    function delNftIds(address _nft, uint[] memory _ids) external onlyOwner {
        for (uint i = 0; i < _ids.length; i ++) {
            require(avaliableNftIds[_nft][_ids[i]], "id not exist");
            delete avaliableNftIds[_nft][_ids[i]];
            emit DelNftId(_nft, _ids[i]);
        }
    }

    function delNft(address _nft) external onlyOwner {
        delete avaliableNft[_nft];
        emit DelNftId(_nft, uint(-1));
    }

    modifier onlySupportedNftIds(address _nft, uint[] memory _ids) {
        require(avaliableNft[_nft], "nft not supported");
        for (uint i = 0; i < _ids.length; i ++) {
            require(avaliableNftIds[_nft][_ids[i]], "nft not supported");
        }
        _;
    }

    function createBatch(uint[] memory _ids, uint[] memory _values) external onlyOwnerOrWhitelist {
        Nft.createBatch(owner(), uint(-1), 0, _ids, _values, new bytes(0));
        emit NewNft(address(Nft), owner(), _ids, _values);
    }

    function swap(P2EERC1155 _nft, uint[] memory _ids, uint[] memory _amounts) public onlySupportedNftIds(address(_nft), _ids) returns (uint[] memory) {
        require(_ids.length > 0 && _ids.length == _amounts.length, "illegal length");
        for (uint i = 0; i < _ids.length; i ++) {
            uint newId = _nft.types(_ids[i]);
            require(newId > 0, "illeagl nftType");
            _nft.burn(msg.sender, _ids[i], _amounts[i]);
            Nft.mint(msg.sender, newId, _amounts[i], new bytes(0));
            _ids[i] = newId;
        }
        return _ids;
    }

    function setContractPendingOwner(P2EERC1155 _nft, address _newOwner) external onlyOwner {
        _nft.setPendingOwner(_newOwner);
    }

    function acceptContractOwner(P2EERC1155 _nft) external onlyOwner {
        _nft.acceptOwner();
    }
    ...
    function swapAndDeposit(uint _pid, uint[] memory _ids, uint[] memory _amounts, uint[] memory _v1Ids, uint[] memory _v1Amounts) external {
        require(_ids.length == _amounts.length, "illegal length");
        uint[] memory allIds = new uint[](_ids.length + _v1Ids.length);
        uint[] memory allAmounts = new uint[](_amounts.length + _v1Amounts.length);
        for (uint i = 0; i < _ids.length; i ++) {
            allIds[i] = _ids[i];
            allAmounts[i] = _amounts[i];
        }
        if (_v1Ids.length > 0) {
            require(_v1Ids.length == _v1Amounts.length, "illegal length");
            _v1Ids = swap(v1Contract, _v1Ids, _v1Amounts);
            for (uint i = 0; i < _v1Ids.length; i ++) {
                allIds[_ids.length + i] = _v1Ids[i];
                allAmounts[_amounts.length + i] =  _v1Amounts[i];
            }
        }
        deposit(_pid, allIds, allAmounts);
    }
    
    function deposit(uint256 _pid, uint[] memory _ids, uint[] memory _amounts) public nonReentrant validatePoolByPid(_pid) {
        ...
        if (_ids.length > 0) {
            ...
            for (uint i = 0; i < _ids.length; i ++) {
                ...
                totalValues = totalValues.add(value.mul(_amounts[i]));
            ...
            prooftERC1155.mintBatch(msg.sender, innerNftIds, _amounts, new bytes(0));
    ...
    function withdraw(uint256 _pid, uint[] memory _ids, uint[] memory _amounts) external nonReentrant validatePoolByPid(_pid) {
        ...
        if (_ids.length > 0) {
            ...
            prooftERC1155.burnBatch(msg.sender, innerNftIds, _amounts);
    ...
    function emergencyWithdraw(uint256 _pid, uint[] memory _ids) external nonReentrant validatePoolByPid(_pid) {
        ...
        if (_ids.length > 0) {
            ...
            prooftERC1155.burnBatch(msg.sender, innerNftIds, amounts);
        }
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        uint currentBalance = IERC20(rewardToken).balanceOf(rewardVault);
        if (currentBalance < _amount) {
            _amount = currentBalance;
        }
        IERC20(rewardToken).safeTransferFrom(rewardVault, _to, _amount);
    }
        ...
        returns (uint256, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            uint id = nftIds[nftContract][ids[i]];
            batchBalances[i] = prooftERC1155.balanceOf(account, id);
...
contract P2EERC1155V2 is ERC1155, SafeOwnable {
    using SafeMath for uint256;
    using Strings for uint256;
    ...
    function createBatch(
        ...
        require(_ids.length > 0 && _ids.length == _values.length, "illegal length");
        require(_initialSupply <= _maxSupply, "Initial supply cannot be more than max supply");
        for (uint i = 0; i < _ids.length; i ++) {
            uint id = _ids[i];
            require(creators[id] == address(0), "id already exists");
