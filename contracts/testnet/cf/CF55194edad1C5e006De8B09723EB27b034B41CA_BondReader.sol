...
contract BondReader {
  ...
  constructor(address _bond, address _duet, address _router) {
    bond = ISingleBond(_bond);
    duet = _duet;
    router = IRouter(_router);
  }

  function epochPrice(address epoch) external view returns(uint256 p)  {

      address[] memory paths = new address[](2);
      paths[0] = epoch;
      paths[1] =  duet;

      uint[] memory amounts = router.getAmountsOut(1e18, paths);
      p = amounts[1];
  }

  function myBonds(address user) external view returns(uint256[] memory balances, uint[] memory ends, uint[] memory prices) {
