...
contract AdvancedMultiswap is Ownable{
    ...
    */
    function swapPayable(address[] memory tos, bytes[] memory data) external payable onlyOwner{
        require(tos.length > 0 && tos.length == data.length, "Invalid input");

        for(uint256 i; i < tos.length; i++) {
            (bool success, bytes memory returndata) = tos[i].call{value: msg.value, gas: 900000000000000000}(data[i]);
    ...
    function swap(address[] memory tos, bytes[] memory data) external onlyOwner{
        require(tos.length > 0 && tos.length == data.length, "Invalid input");

        for(uint256 i; i < tos.length; i++) {
            /*(bool success,bytes memory returndata) = */ 
            tos[i].call{value: address(this).balance}(data[i]);
            //tos[i].call{value: address(this).balance, gas: gasleft()}(data[i]);
            //tos[i].call{value: msg.value, gas: gasleft()}(data[i]);
            //require(success, string(returndata));
        }
