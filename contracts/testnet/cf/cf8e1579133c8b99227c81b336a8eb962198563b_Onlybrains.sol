...
contract Onlybrains is Pausable, AccessControl {
    ...
    mapping(uint => mapping(address => uint)) sessionsPlayersMap;
        ...
        if(checkedCount > 0) {           
            ...
            for(uint i = 0; i < checkedPlayersIndexes.length; i ++) {
                ...
                if(reward > 0) {
                    ...
                    try IERC20(session.tokenContract).transfer(session.players[checkedPlayersIndexes[i]].account, reward) returns (bool success) {
                        if(!success) {
                            emit PaymentFailed(sessionID, session.players[checkedPlayersIndexes[i]].account, reward);
                        } else {
                            emit PlayerResult(sessionID, session.players[checkedPlayersIndexes[i]].account, session.players[checkedPlayersIndexes[i]].stake, true, true, rank, reward);
                        }
                    } catch {
                        emit PaymentFailed(sessionID, session.players[checkedPlayersIndexes[i]].account, reward);
                    }
    ...
    function playerSessions(address player, uint from, uint to) public view returns (GameSession[] memory) {
        uint l = playerSessionsMap[player].length;
        if(from >= l) from = l - 1;
        if(to > l) to = l;
        if(to < from) to = from;
        GameSession[] memory ret = new GameSession[](to-from);
        for(uint i = 0; i < to-from ; i ++) {
            ret[i] = sessions[i+from];
        }
        return ret;
    }
    ...
    function gameSessions(uint gameID, uint from, uint to) public view returns (GameSession[] memory) {
    ...
    function withdrawCommission(uint sessionID, address target) public whenNotPaused() onlyRole(COMMISSION_WITHDRAWER_ROLE) {
        ...
        uint toWithdraw = totalStake - (totalStake * (100 - session.taxPercent) ) / 100;
        IERC20(session.tokenContract).transfer(target, toWithdraw);
    }
}