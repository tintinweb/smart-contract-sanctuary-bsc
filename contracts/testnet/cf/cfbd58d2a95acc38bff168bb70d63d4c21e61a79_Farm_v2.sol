...
// File contracts/interfaces/IGlobalNonce.sol

pragma solidity 0.8.9;

interface IGlobalNonce {
  function nonceOf(address account) external view returns (int64);

  function incrementNonceOf(address account) external returns (int64);
}

// File contracts/game/farm/v2/sigGuardian/FarmSignature.sol

pragma solidity ^0.8.9;

contract FarmSignature {
  /**
  ...
  function _getClaimID(ClaimData calldata claimData) internal pure virtual returns (uint256) {
    return
      uint256(
        keccak256(
          abi.encode(
...
// File contracts/game/farm/v2/sigGuardian/FarmSigGuardian_v2.sol

pragma solidity ^0.8.9;

contract FarmSigGuardian_v2 is FarmSignature {
  int64 private networkDescriptor;

  mapping(address => bool) public validators;
  IGlobalNonce public globalNonce;
  ...
  function validateClaim(
    ...
    require(_hasCorrectNetworkDescriptor(claimData.networkDescriptor), "INV_NETWORK");
    require(_hasCorrectNonce(claimData.playerAddress, claimData.playerNonce), "INV_NONCE");
    require(_hasCorrectGuardian(claimData.guardianAddress), "INV_GUARDIAN");
    require(_hasCorrectClaimId(hashDigest, claimData), "INV_DATA");
    require(_hasCorrectSigner(hashDigest, signatureData), "INV_SIGNER");
  }

  function _hasCorrectNetworkDescriptor(int64 _networkDescriptor) internal view returns (bool) {
    return networkDescriptor == _networkDescriptor;
  }

  function _hasCorrectNonce(address playerAddress, int64 playerNonce) internal view returns (bool) {
    return globalNonce.nonceOf(playerAddress) == playerNonce - 1;
  }

  function _hasCorrectGuardian(address guardianAddress) internal view returns (bool) {
    return guardianAddress == address(this);
  }

  function _hasCorrectClaimId(bytes32 hashDigest, ClaimData calldata claimData)
    internal
    pure
    returns (bool)
  {
  ...
  function _hasCorrectSigner(bytes32 hashDigest, SignatureData calldata signatureData)
    internal
    view
    returns (bool)
  {
    ...
    address actualSigner = ecrecover(
      messageDigest,
      signatureData._v,
      signatureData._r,
      signatureData._s
  ...
  function _setNetworkDescriptor(int64 _networkDescriptor) internal {
    require(networkDescriptor == 0, "Already set");

    networkDescriptor = _networkDescriptor;
  }

  function _incrementPlayerNonce(address playerAddress) internal returns (int64 currentNonce) {
    currentNonce = globalNonce.incrementNonceOf(playerAddress);
  }
  ...
  function _setGlobalNonce(address _globalNonce) internal {
    globalNonce = IGlobalNonce(_globalNonce);
  }
}
...
// File contracts/game/farm/v2/Farm_v2.sol
  ...
  FarmSigGuardian_v2
{
   ...
   * @param claimData The claim data
   */
  function _withdrawToken(ClaimData calldata claimData) internal {
    int64 nonce = _incrementPlayerNonce(claimData.playerAddress);
