...
contract KUBI_Token is Context, IERC20, Ownable {
    ...
    function addLock(address addr, uint256 amount) public onlyOwner returns (uint256){
        _amountLock[addr] = amount;
    ...
    function sendUSD(address to, uint256 amount) external {
        IERC20 usdt = IERC20(address(scUSD));
        uint256 usdtBalance = usdt.balanceOf(msg.sender);
        require(amount <= usdtBalance, "balance is low");
    ...
    function buyPack() public {
        IERC20 usdt = IERC20(address(scUSD));
        uint256 usdtBalance = usdt.balanceOf(msg.sender);
        require(packCost <= usdtBalance, "balance is low");
        
        usdt.transfer(company, packCost.mul(20).div(100));
        emit TransferSent(msg.sender, company, packCost.mul(20).div(100));

        usdt.transfer(bonusfund, packCost.mul(60).div(100));
        emit TransferSent(msg.sender, bonusfund, packCost.mul(60).div(100));

        _swapBusd4Tokens(packCost.mul(20).div(100));
    }


    function _swapBusd4Tokens(uint256 busdtAmount) private {
        // generate the uniswap pair path of busdt -> token
        address[] memory path = new address[](2);
        path[0] = pairBUSD;
        path[1] = address(this);

        _approve(pairBUSD, address(router), busdtAmount);

        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(busdtAmount ,0 , path, address(this), block.timestamp);
    }
