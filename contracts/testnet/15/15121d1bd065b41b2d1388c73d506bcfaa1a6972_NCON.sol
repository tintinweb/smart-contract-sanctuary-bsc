...
// File: contracts/token/INCON.sol


pragma solidity ^0.8.0;

interface INCON {
    event MintToken(address indexed to, uint256 indexed amount);
    event BurnToken(address indexed form, uint256 indexed amount);

    function mint(address to, uint256 amount) external returns(bool);
    function burn(address form, uint256 amount) external returns(bool);
}
...
// File: contracts/security/PermissionRight.sol
...
contract NCON is ERC20, PermissionRight, INCON {
    constructor(string memory name_, string memory symbol_) ERC20(name_, symbol_) {

    }

    function mint(address to, uint256 amount) public override onlyOperator returns(bool){      
        _mint(to, amount);
        emit MintToken(to, amount);
        return true;
    }

    function burn(address form, uint256 amount) public override onlyOperator returns(bool){
        _spendAllowance(form, msg.sender, amount);
        _burn(form, amount);
        return true;
    }

    function transfer(address to, uint256 amount) public override onlyOperator returns (bool) {
        address owner = _msgSender();
        _transfer(owner, to, amount);
        return true;
    }
