...
abstract contract TaxableRouter is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier byFeeOwner(address token) {
        require(msg.sender == feeOwners[token], "CCM: INVALID_FEE_OWNER");
        _;
    }
    ...
    modifier taxActive(address token) {
        require(tokenETHBaseTax[token].isActive, "CCM: TOKEN_TAX_NOT_ACTIVATED");
        _;
    }
    ...
    function chooseTaxTierLevel(address token)
     external payable byFeeOwner(token) {
        ...
        if(msg.value == expertFee){
            ...
            require(!existing.isActive || existing.tax > 30, "CCM: NO_DOWNGRADE_TIER_LEVEL");
            ...
            TokenBaseTax memory existing = tokenETHBaseTax[token];
            require(!existing.isActive || existing.tax > 50, "CCM: NO_DOWNGRADE_TIER_LEVEL");
            tokenETHBaseTax[token] = TokenBaseTax(token, true, 50);
        } else if(msg.value == 0) {
            // Token must not be initialized.
            TokenBaseTax memory existing = tokenETHBaseTax[token];
            require(!existing.isActive, "CCM: NO_DOWNGRADE_TIER_LEVEL");
    ...
    function withdrawExcessiveETH() external onlyOwner {
        ...
        require(address(this).balance >= withdrawableETH, "CCM: WITHDRAW_TOO_MUCH_ETH");
        totalETHTaxEarned = 0;
