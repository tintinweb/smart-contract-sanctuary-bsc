...
contract StiIdo  {
    ...
    function viewInfo(address account_) public view returns(address idoToken, string memory name, uint totalSupply,  bool isW, uint[3] memory quotas,  uint[7] memory info, uint[4] memory userData) {
        ...
        quotas[2] = meta.target / idoDecimals;
        idoToken =  address(meta.idoToken);
        name = meta.idoToken.name();
        totalSupply = meta.idoToken.totalSupply() / idoDecimals;

        info[0] = meta.price;
        info[1] = meta.privateQuota / idoDecimals;
        info[2] = meta.publicQuota / idoDecimals;
        info[3] = times.pri;
        info[4] = times.pub;
        info[5] = times.end;
...
contract IDOFactory is OwnableUpgradeable {
    ...
    function getIdoInfoBathch(uint stage_) public view returns(address[] memory tokens, string[] memory names, uint[8][] memory infos) {
        ...
        for (uint i; i < size; i++) {
            (tokens[i], names[i], infos[i]) = idoInfo(list[i]);
        }
    } 
    ...
    function test(address ido_) public view  returns(address idoToken, string memory name, uint mode, uint buyAmount, uint target,   uint price,  uint privateTime, uint publicTime, uint endTime, uint releaseTime) {
