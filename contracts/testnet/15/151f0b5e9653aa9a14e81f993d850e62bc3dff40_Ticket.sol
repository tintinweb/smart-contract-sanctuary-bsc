...
/** 
 *  SourceUnit: d:\Projects\sportflix\smart-contracts\contracts\nfts\Ticket.sol
*/
...
contract Ticket is AccessControlEnumerable, ERC721Enumerable, ERC721Burnable  {
    ...
    function setBaseURI(string memory _uri)
        external
    {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Ticket: must have admin role to set");

        require(bytes(_uri).length > 0, "Ticket: uri is invalid");
    ...
    function mint(address _to)
        ...
        returns (uint256)
    {
        require(hasRole(MINTER_ROLE, _msgSender()), "Ticket: must have minter role to mint");
