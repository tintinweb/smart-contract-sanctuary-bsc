...
contract MythDegen is ERC721 {
    ...
    function unlockCosmetic(
        ...
        if (_layerType == 0) {
            ...
            require(
                MythCosmetic(_address).bodyCostToUnlock(_itemId) == msg.value,
                "Send exact currency to unlock"
            );
        } else if (_layerType == 6) {
            require(
                MythCosmetic(_address).headCostToUnlock(_itemId) == msg.value,
                "Send exact currency to unlock"
            );
        }
...
contract MythCosmetic {
    ...
    mapping(uint256 => uint256) public bodyCostToUnlock;
    mapping(uint256 => uint256) public headCostToUnlock;
        ...
        onlyOwner
    {
        ...
        if (_layerType == 0) {
            ...
            delete bodyCostToUnlock[_id];
        } else if (_layerType == 6) {
            delete headURL[_id];
            delete headExists[_id];
            delete headCostToUnlock[_id];
        }
    }
    ...
    function changeBodyUrl(
        string calldata _url,
        uint256 _id,
        uint256 _cost
    ) external onlyOwner {
        emit cosmeticAdded(5, _url, _cost);
        bodyExists[_id] = true;
        bodyURL[_id] = _url;
        bodyCostToUnlock[_id] = _cost;
    }
    ...
    function changeHeadUrl(
        ...
        emit cosmeticAdded(6, _url, _cost);
        headExists[_id] = true;
        headURL[_id] = _url;
        headCostToUnlock[_id] = _cost;
    }
}
