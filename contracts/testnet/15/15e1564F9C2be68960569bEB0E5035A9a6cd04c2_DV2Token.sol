...
contract DV2Token is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    function updateMultiplier(address from) public returns (bool allow) {
      ...
      uint256 sellprice = sellPriceNow().div(10**32);
      uint256 _tokenCount = tokenCount.current();
      uint256 currentBasePrice = checkMultiplier().mul(basePrice).div(10**18);
      uint256 currentSellTargetPrice = checkMultiplier().mul(sellTargetPrice).div(10**32);
      if(isMultiEnabled){
        if(buyprice <= currentBasePrice && sellprice <= currentBasePrice && rebaseswitch){
					rebaseswitch = false;
        ...
        if(sellprice >= currentSellTargetPrice){
          rebaseswitch = true;
          allow = true;
        }else{
          rebaseswitch = false;
    ...
    function checkMultiplier() public view returns (uint256 mul) {
      if(isMultiEnabled){
        mul = readPowerResult(globalMultiplier.div(denominatorMultiplier),readMultiplierLength());
      }
    }
    // Check User Balance
