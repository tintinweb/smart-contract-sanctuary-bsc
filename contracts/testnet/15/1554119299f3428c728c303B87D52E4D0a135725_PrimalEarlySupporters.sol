    ...
    ERC721Royalty
{
    ...
    modifier transferAllowed() {
        require(!paused() || allowlisted[msg.sender], "Pausable: paused");
    ...
    function allowlist(address _address, bool _status) public onlyOwner {
    ...
    function allowlistMultiple(
        ...
        for (uint256 i = 0; i < _addresses.length; ++i) {
            allowlist(_addresses[i], _statuses[i]);
        }
    }
    ...
    function blocklist(address _address, bool _status) public onlyOwner {
    ...
    function blocklistMultiple(
        ...
        for (uint256 i = 0; i < _addresses.length; ++i) {
            blocklist(_addresses[i], _statuses[i]);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        uint256 tokenId,
        uint256 batchSize
    ) internal override(ERC721, ERC721Enumerable) transferAllowed {
...
contract PrimalEarlySupporters is ERC721Base {
    constructor(
        string memory _name,
        string memory _symbol,
        string memory _contractURI,
        string memory _baseTokenURI
    ) ERC721Base(_name, _symbol, _contractURI, _baseTokenURI) {}
}