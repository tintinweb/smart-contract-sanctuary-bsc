...
contract Treasury is ITreasury, ContractGuard, ReentrancyGuard {
    ...
    // core components
    address public darko;
    address public ghost;

    address public override boardroom;
    address public darkoOracle;

    // price
    uint256 public darkoPriceOne;
    uint256 public darkoPriceCeiling;
    ...
    // 28 first epochs (1 week) with 4.5% expansion regardless of darko price
    uint256 public bootstrapEpochs;
    uint256 public bootstrapSupplyExpansionPercent;

    uint256 public override previousEpochDarkoPrice;
    uint256 public allocateSeigniorageSalary;
    uint256 public maxDiscountRate; // when purchasing bond
    uint256 public maxPremiumRate; // when redeeming bond
    uint256 public discountPercent;
    uint256 public premiumPercent;
    uint256 public mintingFactorForPayingDebt; // print extra darko during debt phase
    ...
    address[] public darkoLockedAccounts;
    ...
    modifier checkEpoch() {
        ...
        epoch_ = epoch_.add(1);
        epochSupplyContractionLeft = (getDarkoPrice() > darkoPriceCeiling)
            ? 0
            : IERC20(darko).totalSupply().mul(maxSupplyContractionPercent).div(
                10000
            );
    }
    ...
    modifier checkOperator() {
        require(
            IBasisAsset(darko).operator() == address(this) &&
    ...
    function getPegPrice() external view override returns (int256) {
        return IOracle(darkoOracle).getPegPrice();
    }

    function getPegPriceUpdated() external view override returns (int256) {
        return IOracle(darkoOracle).getPegPriceUpdated();
    }

    // oracle
    function getDarkoPrice() public view override returns (uint256 darkoPrice) {
        try IOracle(darkoOracle).consult(darko, 1e18) returns (uint144 price) {
    ...
    function getDarkoUpdatedPrice()
        public
        view
        override
        returns (uint256 _darkoPrice)
    {
        try IOracle(darkoOracle).twap(darko, 1e18) returns (uint144 price) {
            return uint256(price);
        } catch {
            revert("oracle failed");
        }
    ...
    function getBurnabledarkoLeft()
        external
        view
        returns (uint256 _burnabledarkoLeft)
    {
        uint256 _darkoPrice = getDarkoPrice();
        if (_darkoPrice <= darkoPriceOne) {
            uint256 _bondMaxSupply = IERC20(darko)
                .totalSupply()
                .mul(maxDebtRatioPercent)
                .div(10000);
            uint256 _bondSupply = IERC20(ghost).totalSupply();
            if (_bondMaxSupply > _bondSupply) {
                uint256 _maxMintableBond = _bondMaxSupply.sub(_bondSupply);
                uint256 _maxBurnabledarko = _maxMintableBond
                    .mul(getBondDiscountRate())
                    .div(1e18);
                _burnabledarkoLeft = Math.min(
    ...
    {
        uint256 _darkoPrice = getDarkoPrice();
        if (_darkoPrice > darkoPriceCeiling) {
            uint256 _totaldarko = IERC20(darko).balanceOf(address(this));
            uint256 _rate = getBondPremiumRate();
            if (_rate > 0) {
                _redeemableBonds = _totaldarko.mul(1e18).div(_rate);
            }
        }
    }
        ...
        returns (uint256 _rate)
    {
        ...
        if (_darkoPrice <= darkoPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = darkoPriceOne.mul(1e18).div(_darkoPrice); // to burn 1 darko
                uint256 _discountAmount = _bondAmount
                    .sub(darkoPriceOne)
                    .mul(discountPercent)
                    .div(10000);
                _rate = darkoPriceOne.add(_discountAmount);
    ...
    function getBondPremiumRate() public view override returns (uint256 _rate) {
        ...
        if (_darkoPrice > darkoPriceCeiling) {
            if (premiumPercent == 0) {
                ...
                uint256 _premiumAmount = _darkoPrice
                    .sub(darkoPriceOne)
                    .mul(premiumPercent)
                    .div(10000);
                _rate = darkoPriceOne.add(_premiumAmount);
                if (maxDiscountRate > 0 && _rate > maxDiscountRate) {
                    _rate = maxDiscountRate;
                }
            }
    ...
    function getDarkoCirculatingSupply()
        public
        view
        override
        returns (uint256)
    {
        return IERC20(darko).totalSupply().sub(getDarkoLockedBalance());
    }

    function getDarkoLockedBalance()
        public
        view
        override
        returns (uint256 _lockedBalance)
    {
        uint256 _length = darkoLockedAccounts.length;
        IERC20 _darko = IERC20(darko);
        for (uint256 i = 0; i < _length; i++) {
            _lockedBalance = _lockedBalance.add(
                _darko.balanceOf(darkoLockedAccounts[i])
            );
        }
    }
    ...
    function getNextExpansionRate() public view override returns (uint256) {
        if (epoch_ < bootstrapEpochs) {
            ...
            return getPegTokenExpansionRate(darko);
        }
    }

    function getNextExpansionAmount() external view override returns (uint256) {
        return getPegTokenExpansionAmount(darko);
    }
        ...
        returns (uint256 _rate)
    {
        ...
        if (_twap > darkoPriceCeiling) {
            uint256 _percentage = _twap.sub(darkoPriceOne); // 1% = 1e16
            uint256 _mse = (_pegToken == darko)
    ...
    function initialize(
        address _darko,
        address _ghost,
        address _darkoOracle,
        address _boardroom,
        uint256 _startTime
    ) public notInitialized {
        darko = _darko;
        ghost = _ghost;
        darkoOracle = _darkoOracle;
        ...
        darkoPriceOne = 10**18; // This is to allow a PEG of 1 darko per VVS
        darkoPriceCeiling = darkoPriceOne.mul(1001).div(1000);

        maxSupplyExpansionPercent = 300; // Upto 3.0% supply for expansion

        bondDepletionFloorPercent = 10000; // 100% of Bond supply for depletion floor
        seigniorageExpansionFloorPercent = 3500; // At least 35% of expansion reserved for boardroom
        maxSupplyContractionPercent = 450; // Upto 4.5% supply for contraction (to burn darko and mint ghost)
        maxDebtRatioPercent = 4500; // Upto 45% supply of ghost to purchase
        ...
        seigniorageSaved = IERC20(darko).balanceOf(address(this));
    ...
    function setDarkoOracle(address _darkoOracle) external onlyOperator {
        darkoOracle = _darkoOracle;
    }

    function setDarkoPriceCeiling(uint256 _darkoPriceCeiling)
        external
        onlyOperator
    {
        require(
            _darkoPriceCeiling >= darkoPriceOne &&
                _darkoPriceCeiling <= darkoPriceOne.mul(120).div(100),
            "out of range"
        ); // [$1.0, $1.2]
        darkoPriceCeiling = _darkoPriceCeiling;
    }
    ...
    function setNextSupplyTarget(uint256 _target) external onlyOperator {
        require(_target > IERC20(darko).totalSupply(), "too small");
        nextSupplyTarget = _target;
    }

    function setDarkoLockedAccounts(address[] memory _darkoLockedAccounts)
        external
        onlyOperator
    {
        delete darkoLockedAccounts;
        uint256 _length = _darkoLockedAccounts.length;
        for (uint256 i = 0; i < _length; i++) {
            darkoLockedAccounts.push(_darkoLockedAccounts[i]);
        }
    }
    ...
    function _updatedarkoPrice() internal {
        try IOracle(darkoOracle).update() {} catch {}
    }
    ...
    function buyBonds(uint256 _darkoAmount, uint256 targetPrice)
        external
        override
        onlyOneBlock
        checkOperator
        nonReentrant
    {
        require(_darkoAmount > 0, "zero amount");

        uint256 darkoPrice = getDarkoPrice();
        require(darkoPrice == targetPrice, "price moved");
        require(
            darkoPrice < darkoPriceOne, // price < $1
            "darkoPrice not eligible for bond purchase"
        );

        require(
            _darkoAmount <= epochSupplyContractionLeft,
        ...
        address _darko = darko;
        uint256 _bondAmount = _darkoAmount.mul(_rate).div(1e18);
        uint256 _darkoSupply = IERC20(darko).totalSupply();
        uint256 newBondSupply = IERC20(ghost).totalSupply().add(_bondAmount);
        require(
            newBondSupply <= _darkoSupply.mul(maxDebtRatioPercent).div(10000),
            "over max debt ratio"
        );

        IBasisAsset(_darko).burnFrom(msg.sender, _darkoAmount);
        IBasisAsset(ghost).mint(msg.sender, _bondAmount);
        ...
        emit BoughtBonds(msg.sender, _darkoAmount, _bondAmount);
    }
        ...
        nonReentrant
    {
        ...
        require(darkoPrice == targetPrice, "price moved");
        require(
            darkoPrice > darkoPriceCeiling, // price > $1.01
            "darkoPrice not eligible for bond purchase"
        );
        ...
        uint256 _darkoAmount = _bondAmount.mul(_rate).div(1e18);
        require(
            IERC20(darko).balanceOf(address(this)) >= _darkoAmount,
            "treasury has no more budget"
        );

        seigniorageSaved = seigniorageSaved.sub(
            Math.min(seigniorageSaved, _darkoAmount)
        );
        allocateSeigniorageSalary = 1000 ether; // 1000 darko salary for calling allocateSeigniorage()

        IBasisAsset(ghost).burnFrom(msg.sender, _bondAmount);
        IERC20(darko).safeTransfer(msg.sender, _darkoAmount);

        _updatedarkoPrice();
        if (regulationStats != address(0))
            IRegulationStats(regulationStats).addRedeemed(epoch_, _darkoAmount);

        emit RedeemedBonds(msg.sender, _darkoAmount, _bondAmount);
    }

    function _sendToBoardroom(uint256 _amount, uint256 _expanded) internal {
        address _darko = darko;
        IBasisAsset(_darko).mint(address(this), _amount);

        uint256 _daoFundSharedAmount = 0;
        if (daoFundSharedPercent > 0) {
            _daoFundSharedAmount = _amount.mul(daoFundSharedPercent).div(10000);
            IERC20(_darko).transfer(daoFund, _daoFundSharedAmount);
        }
        ...
        if (marketingFundSharedPercent > 0) {
            _marketingFundSharedAmount = _amount
                ...
                .div(10000);
            IERC20(_darko).transfer(marketingFund, _marketingFundSharedAmount);
        }
        ...
        if (insuranceFundSharedPercent > 0) {
            _insuranceFundSharedAmount = _amount
                ...
                .div(10000);
            IERC20(_darko).transfer(insuranceFund, _insuranceFundSharedAmount);
        }
        ...
        IERC20(_darko).safeIncreaseAllowance(boardroom, _amount);
        IBoardroom(boardroom).allocateSeigniorage(_darko, _amount);

        if (regulationStats != address(0))
            IRegulationStats(regulationStats).addEpochInfo(
                epoch_.add(1),
                previousEpochDarkoPrice,
                _expanded,
                _amount,
    ...
    {
        _updatedarkoPrice();
        previousEpochDarkoPrice = getDarkoPrice();
        address _darko = darko;
        uint256 _supply = getDarkoCirculatingSupply();
        ...
        if (epoch_ < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            if (epoch_ == 0) _supply = IERC20(_darko).totalSupply();
            _seigniorage = _supply.mul(bootstrapSupplyExpansionPercent).div(
                10000
            );
            _sendToBoardroom(_seigniorage, _seigniorage);
        } else {
            if (previousEpochDarkoPrice > darkoPriceCeiling) {
                // Expansion ($darko Price > 1 $ETH): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(ghost).totalSupply();
                uint256 _percentage = previousEpochDarkoPrice.sub(
                    darkoPriceOne
                );
                ...
                if (_savedForBond > 0) {
                    ...
                    IBasisAsset(_darko).mint(address(this), _savedForBond);
                    emit TreasuryFunded(block.timestamp, _savedForBond);
                }
            } else if (previousEpochDarkoPrice < darkoPriceOne) {
                if (regulationStats != address(0))
                    IRegulationStats(regulationStats).addEpochInfo(
                        epoch_.add(1),
                        previousEpochDarkoPrice,
        ...
        internal
    {
        ...
        if (_pegTokenTwap > darkoPriceCeiling) {
            uint256 _percentage = _pegTokenTwap.sub(darkoPriceOne); // 1% = 1e16
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(ghost), "bond");
