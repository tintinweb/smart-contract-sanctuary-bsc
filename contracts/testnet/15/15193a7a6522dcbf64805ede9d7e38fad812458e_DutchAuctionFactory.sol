...
contract DutchAuction is ReentrancyGuard {
    ...
    function tokensClaimable(address _user) public view returns (uint256 _claimerCommitment) {
        ...
        if (block.timestamp >= (finalizeTime + vestingDuration)) {
            _claimerCommitment = commitments[_user] * totalTokens / commitmentsTotal;
        } else {
            uint256 _time = block.timestamp - finalizeTime;
    ...
    function commitEth(address payable _beneficiary) public payable nonReentrant {
        require(payToken == ETH_ADDRESS, "DutchAuction: payment currency is not ETH address");
        ...
        require(commitmentsTotal <= address(this).balance, "DutchAuction: The committed ETH exceeds the balance");
    }
...
contract DutchAuctionFactory is Ownable {
    ...
    uint64 public constant MAX_VESTING_DURATION = 10 days;
    ...
    function createAuction(
        ...
        DutchAuction _newAuction = new DutchAuction(auctionToken,
            payToken,
            _totalTokens,
            _startTime,
            _endTime,
            ...
            treasury,
            vestingDuration);
        IERC20(auctionToken).safeTransferFrom(treasury, address(this), _totalTokens);
        IERC20(auctionToken).safeTransfer(address(_newAuction), _totalTokens);

        auctions.push(address(_newAuction));

        emit AuctionCreated(
            auctionToken, payToken, _totalTokens, _startTime, _endTime, _startPrice, _minPrice, admin, treasury
            );
    }

    function setAuctionToken(address _token) public onlyOwner {
        require(_token != address(0), "Invalid address");
        auctionToken = _token;
        emit AuctionTokenUpdated(_token);
    }

    function setPayToken(address _token) public onlyOwner {
        require(_token != address(0), "Invalid address");
        payToken = _token;
        emit PayTokenUpdated(_token);
    }
    ...
    event PayTokenUpdated(address indexed _address);
    event AuctionTokenUpdated(address indexed _address);
    event VestingDurationUpdated(uint64 _vestingDuration);
}