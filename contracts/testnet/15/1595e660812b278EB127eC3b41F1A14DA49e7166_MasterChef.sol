...
contract MasterChef is Ownable, ReentrancyGuard {
	...
	struct UserInfo {
		...
		//
		// We do some fancy math here. Basically, any point in time, the amount of Laro
		// entitled to a user but is pending to be distributed is:
		//
		//   pending reward = (user.amount * pool.laroPerShare) - user.rewardDebt
		//
		// Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
		//   1. The pool's `laroPerShare` (and `lastRewardTime`) gets updated.
	...
	struct PoolInfo {
		...
		uint256 allocPoint; // How many allocation points assigned to this pool. Laro to distribute per block.
		uint256 lastRewardTime; // Last block timestamp that laro distribution occurs.
		uint256 accLaroPerShare; // Accumulated laro per share, times 1e12. See below.
	...
	// Laro tokens created per second.
	uint256 public laroPerSecond;

	// Deposit Fee MAX CAP
	uint16 DEPOSIT_FEE_CAP = 400; // 400 -> 4%

	// Max Allocation Point Cap
	uint256 public ALLOC_POINT_CAP = 10000;//100%
	...
	// The block.timestamp when Laro mining starts.
	uint256 public startTime;
	// The block.timestamp when Laro mining ends.
    ...
    *****************/
	...
	function add(
		...
		poolInfo.push(PoolInfo({ lpToken: _lpToken, allocPoint: _allocPoint, lastRewardTime: lastRewardTime, accLaroPerShare: 0, depositFeeBP: _depositFeeBP }));
	}

	// Update the given pool's Laro allocation point and deposit fee. Can only be called by the owner.
	function set(
		uint256 _pid,
		uint256 _allocPoint,
		uint16 _depositFeeBP,
	...
	function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
		...
		uint256 accLaroPerShare = pool.accLaroPerShare;
		uint256 lpSupply = pool.lpToken.balanceOf(address(this));
		if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
			uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
			uint256 laroReward = multiplier.mul(laroPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
			accLaroPerShare = accLaroPerShare.add(laroReward.mul(1e12).div(lpSupply));
		}
		return user.amount.mul(accLaroPerShare).div(1e12).sub(user.rewardDebt);
	}
	...
	function updatePool(uint256 _pid) public {
		...
		uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
		uint256 laroReward = multiplier.mul(laroPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
		pool.accLaroPerShare = pool.accLaroPerShare.add(laroReward.mul(1e12).div(lpSupply));
		pool.lastRewardTime = block.timestamp;
	}

	// Deposit LP tokens to MasterChef for Laro allocation.
	function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
		...
		if (user.amount > 0) {
			uint256 pending = user.amount.mul(pool.accLaroPerShare).div(1e12).sub(user.rewardDebt);
			if (pending > 0) {
				safeLaroTransfer(_msgSender(), pending);
			}
		}
		...
		user.rewardDebt = user.amount.mul(pool.accLaroPerShare).div(1e12);
	...
	function harvestAll() public {
		...
		for (uint256 pid = 0; pid < length; ++pid) {
			...
			if (user.amount > 0) {
				...
				calc = user.amount.mul(pool.accLaroPerShare).div(1e12);
		...
		if (totalPending > 0) {
			safeLaroTransfer(msg.sender, totalPending);
		}
	}
	...
	// Safe Laro transfer function, reverts if staking reward doesn't have enough funds
	function safeLaroTransfer(address _to, uint256 _amount) internal {
		laro.safeTransferFrom(rewardManager, address(this), _amount);
		laro.safeTransfer(_to, _amount);
	}
