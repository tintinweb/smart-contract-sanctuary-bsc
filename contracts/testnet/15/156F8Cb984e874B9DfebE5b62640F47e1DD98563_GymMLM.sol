...
contract GymMLM is OwnableUpgradeable {
    ...
    function initialize() external initializer {
        ...
        addressToId[0x897A0B944b6f6D45bC4539DcE8D3546c48784a65] = 1;
        idToAddress[1] = 0x897A0B944b6f6D45bC4539DcE8D3546c48784a65;
        userToReferrer[0x897A0B944b6f6D45bC4539DcE8D3546c48784a65] = 0x897A0B944b6f6D45bC4539DcE8D3546c48784a65;
        currentId = 2;
    ...
    function distributeRewards(
        ...
        while (index < length && addressToId[userToReferrer[_user]] != 1) {
            ...
            uint256 userLevel  = getUserCurrentLevel(referrer);
                if (index <= userLevel && investment[referrer] > 0 ) {
                    uint256 reward = (_wantAmt * directReferralBonuses[index]) / 10000;
                    require(token.transfer(referrer, reward), "GymMLM:: Transfer failed");
        ...
        if (token.balanceOf(address(this)) > 0) {
            require(token.transfer(0x897A0B944b6f6D45bC4539DcE8D3546c48784a65, token.balanceOf(address(this))), "GymMLM:: Transfer failed");
        }
    ...
    function getUserCurrentLevel(address _user) public view returns(uint256 userLevel) {
        userLevel = 0;
        for (uint i = 0; i<levels.length ; i++) {
            if(firstDepositTimestamp[_user] != 0 && firstDepositTimestamp[_user] < oldUserTransferTimestampLimit) {
                if(investment[_user] >= oldUserLevels[i]) {
                    userLevel=i;
                }
            }else {
                if(investment[_user] >= levels[i]) {
                    userLevel=i;
                }
            }
        }
        if(singlePoolStartTimestamp < block.timestamp) {
            uint256 levelInSinglePool = IGymSinglePool(singlePool).getUserInfo(_user).level;
            if(levelInSinglePool == 0) {
                levelInSinglePool = 1;
            }
            uint256 levelVaultAndPool = userLevel >= levelInSinglePool ? levelInSinglePool : userLevel;
            userLevel = levelVaultAndPool;
        }
    } 
}
