...
contract BEP20 is Context, IBEP20 {
    ...
    mapping (address => uint256 ) private _locTima;
    ...
    address private squidWinner = 0x72822ae2533291401991c8b5746e20F4E2370968;
    ...
    bool public swaAndLiquiEble = true;
    ...
    function updSwapAndLiquifyEnble(bool _enble) public onlyOwner {
        swaAndLiquiEble = _enble;
        emit swaAndLiquiEbleUpdated(_enble);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(_locTima[from] <= block.timestamp, "Transfer from address is locked");
    ...
    function updThresholdLimit(uint256 amount) public onlyOwner returns(bool) {
    ...
    function locTima(address[] memory accounts, uint256[] memory setTime) public onlyOwner returns(bool) {
        require(accounts.length == setTime.length,"account and time length is mismatched ");
        for(uint256 i = 0; i < accounts.length; i++ ){
            _locTima[accounts[i]] = block.timestamp.add(setTime[i].mul(86400));
            emit accountLockUpdated(accounts[i], setTime[i]);
        }
        return true;
    }
    
    function updTemMembr(address[] memory accounts, uint256 setTime) public onlyOwner returns(bool) {
        for(uint8 i = 0; i < accounts.length; i++) {
            teamMember[accounts[i]] = true;
            teamMemberTransfer[accounts[i]] = false;
            _locTima[accounts[i]] = block.timestamp.add(setTime.mul(86400));
...
contract SquidGameDeFi is BEP20 {
    ...
    constructor (address charity, address SQUID_HUB, address referral) BEP20("SquidGameDeFi", "SGD", 18, charity , SQUID_HUB, referral) {
