...
contract IslandStake is OwnableUpgradeable {
    ...
    function initialize() external initializer {
        ...
        islandToken = IBEP20(0x5DDB21ebfaC25E6e0b1AD0064fca13960c4aA10D);
        seq = 1;
    ...
    function calculateAll(address addr) public view returns(uint, uint, uint){
        uint rew;
        ...
        return (rew, userInfo[addr].locked, userInfo[addr].lastUnlockTime + claimInterval);
    }
    ...
    function claimLocked() public {
        ...
        islandToken.mint(msg.sender,reward);
    }

    function claim(uint islandId, uint index) internal {
    ...
    function assign(uint id, uint selfPercent) external {
        ...
        IslandInfo storage landInfo = islandInfo[id];
        require(landInfo.contribute > 0 && (landInfo.lastAssignTime == 0 || block.timestamp - landInfo.lastAssignTime >= assignInterval),
            "assign cond");
