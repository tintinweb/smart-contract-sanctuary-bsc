...
interface ILimitOrderFeature {
    ...
    function setStableToken(address vndt) external;
}
    ...
    NativeOrdersInfo
{
    ...
    constructor(address stockExchange)
        FixinCommon()
        NativeOrdersInfo(stockExchange)
    {}

    function createLimitOrder(
        LibNativeOrder.LimitOrder calldata order,
        LibSignature.Signature calldata signature
    ) public override returns (LibNativeOrder.OrderInfo memory orderInfo) {
        ...
        if (order.isBuyer) {
            _transferERC20Tokens(
                LibNativeOrdersStorage.getStorage().stableToken,
                order.maker,
                address(this),
                order
                    .amount
                    .safeMul(order.price)
                    .safeDiv(
                        LibNativeOrdersStorage.getStorage().decimalPrice != 0
                            ? LibNativeOrdersStorage.getStorage().decimalPrice
                            : 10**10
                    )
                    .safeAdd(order.takerTokenFeeAmount)
            );
            ...
            _transferERC20Tokens(
                ...
                order.maker,
                address(this),
                order.amount.safeAdd(order.takerTokenFeeAmount)
            );
    ...
    function getOrderLocked(bytes32 orderHash)
        public
        view
        override
        returns (uint256)
    ...
    function getFilledOrder(bytes32 orderHash)
        public
        view
        override
        returns (uint256)
        ...
        returns (LibNativeOrder.OrderInfo memory orderInfo)
    {
        ...
        orderInfo = getLimitOrderInfoV2(order, order.amount);
    ...
    function setStableToken(address vndt) external override onlyAdmin {
        LibNativeOrdersStorage.getStorage().stableToken = vndt;
    }
    ...
    function migrate() external returns (bytes4 success) {
        ...
        _registerFeatureFunction(this.setStableToken.selector);

        _registerFeatureFunction(this.getLimitOrderInfo.selector);
        _registerFeatureFunction(this.getLimitOrderInfoV2.selector);
        ...
        _registerFeatureFunction(this.EIP712_DOMAIN_SEPARATOR.selector);
