...
contract DarkSport is OwnableUpgradeSafe {
    ...
    function editTokenBet(uint256 _index, address _tokenBet) external onlyOwner {
    ...
    function getTotalBetMatch(uint256 _matchId) public view returns (uint256[] memory _totalBetMatch) {
        _totalBetMatch = new uint256[](numToken);
        for (uint8 i = 0; i < numToken; i++){
            _totalBetMatch[i] = totalMatchesBet[_matchId][i];
        }
    }
    ...
    returns (uint8 _index, string memory _description, uint8 _numDoors, uint32[] memory _odds, uint8[] memory _doorResults,
        uint256 _numTickets, uint256[] memory _totalBetAmount, uint256[] memory _totalPayoutAmount, uint256 _maxBudget) {
