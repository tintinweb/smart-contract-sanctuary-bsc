...
contract AiMine is Ownable {
    ...
    function buyMine(uint256 id,uint256 num)  external {
        ...
        uint256 fee=totalValue.mul(20).div(100);
         uint256 realValue=totalValue.sub(fee);
        IERC20(detail.currency).safeTransferFrom(msg.sender,acceptAddress,realValue);
        if(fee>0){
            IERC20(detail.currency).safeTransferFrom(msg.sender,address(this),fee);
        }
        mineNum[id]=mineNum[id]+num;
        //buyType[id][msg.sender]=true;
        emit BuyMine(msg.sender,detail.currency,id,num,totalValue);
    }
