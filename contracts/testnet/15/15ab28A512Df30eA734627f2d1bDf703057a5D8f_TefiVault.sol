...
contract TefiVault is Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(uint _amount) external whenNotPaused nonReentrant {
        ...
        if (totalShare == 0 || poolBal < totalShare) {
            share = _amount;
        } else {
    ...
    function withdraw(uint _amount) external nonReentrant {
        UserInfo storage user = users[msg.sender];
        require (user.amount >= _amount, "exceeded amount");
    ...
    function _rebalance() internal {
        ...
        uint keepBal = rebalanceRate * balance() / 100;
        
        if (curBal <= keepBal) return;

        uint invest = curBal - keepBal;
        asset.safeTransfer(strategy, invest);
        underlying += invest;
    }

    function investable() external view returns (uint) {
        uint curBal = available();
        uint keepBal = rebalanceRate * balance() / 100;
