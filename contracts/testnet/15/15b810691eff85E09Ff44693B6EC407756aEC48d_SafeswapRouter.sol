...
contract SafeswapRouter is ISafeswapRouter02 {
    ...
    function switchTokenSTP(address _tokenAddress) onlyOwner public {
        nameToInfo[_tokenAddress].enabled = !nameToInfo[_tokenAddress].enabled;
    }
    
    /*function enableTokenSTP (address _tokenAddress) onlyOwner public {
        nameToInfo[_tokenAddress].enabled = true;
    }

    function disableTokenSTP (address _tokenAddress) onlyOwner public {
        nameToInfo[_tokenAddress].enabled = true;
    }

    function disableSTP () onlyOwner public {
        killSwitch = true;
    }

    function enableSTP () onlyOwner public {
        killSwitch = false;
    }*/
    ...
    function getTokenSTP(address _tokenAddress)
        public
        view
        returns(string memory tokenName, address tokenAddress, uint expectedSell, uint expectedBuy, uint actualBuy, uint actualSell)
    {
        tokenName = nameToInfo[_tokenAddress].tokenName;
        tokenAddress = nameToInfo[_tokenAddress].tokenAddress;
        expectedSell = nameToInfo[_tokenAddress].expectedSell;
        expectedBuy = nameToInfo[_tokenAddress].expectedBuy;
        actualSell = nameToInfo[_tokenAddress].actualSell;
        actualBuy = nameToInfo[_tokenAddress].actualBuy;
    }
    ...
    function swapExactTokensForTokens(
        ...
        if(nameToInfo[path[0]].enabled == true && (nameToInfo[path[0]].actualSell != nameToInfo[path[0]].expectedSell)) {
        ...
        if(nameToInfo[path[1]].enabled == true && (nameToInfo[path[1]].actualBuy != nameToInfo[path[1]].expectedBuy)) {
