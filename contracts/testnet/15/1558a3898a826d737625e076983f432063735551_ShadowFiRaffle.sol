...
contract ShadowFiRaffle is Ownable, ReentrancyGuard {
    ...
    uint256 public mainPoolPercent = 7143;
    uint256 public secondPoolPercent = 1429;
    uint256 public thirdPoolPercent = 1428;
    ...
    function setPoolPercents(uint256 _mainPoolPercent, uint256 _secondPoolPercent, uint256 _thirdPoolPercent) public onlyOwner {
        require(mainPoolPercent + secondPoolPercent + thirdPoolPercent <= 10000, "All three percents cannot equal more than 100%.");
    ...
    function setTicketCost(uint256 _raffleId, uint256 _newTicketCost) public onlyOwner {
        require(_raffleId > currentRaffleId || block.timestamp <= raffleInfos[currentRaffleId].startTime);
        raffleInfos[_raffleId].ticketCost = _newTicketCost;
    }
    ...
    function revealWinner() public onlyCourt {
        ...
        if (winningUser == DEAD_ADDRESS) {
            ...
            mainPool = (mainPool * 100) / (92 + mainPoolBonusPercent);
            payable(winningUser).transfer(mainPool);
            mainPool = secondPool + (mainPool * 100) / (8 - mainPoolBonusPercent);
        ...
        uint256 ticketCost = raffleInfos[currentRaffleId + 1].ticketCost;
        ...
        if (ticketCost == 0) {
            ticketCost = raffleInfos[currentRaffleId].ticketCost;
        }
        ...
        _startSession(currentRaffleId + 1, sessionTime, ticketCost, deadAccrual);
    }

    function startSession(uint256 _sessionId, uint256 _sessionTime, uint256 _ticketCost, uint256 _deadAccrual) public onlyOwner {
        require(raffleInfos[currentRaffleId].startTime == 0, "You already initiated the raffle contract.");
        _startSession(_sessionId, _sessionTime, _ticketCost, _deadAccrual);
    }

    function _startSession(uint256 _sessionId, uint256 _sessionTime, uint256 _ticketCost, uint256 _deadAccrual) internal {
        currentRaffleId = _sessionId;
        raffleInfos[currentRaffleId].startTime = _sessionTime;
        ...
        raffleInfos[currentRaffleId].ticketCost = _ticketCost;
    ...
    function deposit(address user) external payable {
        ...
        uint256 ticketCount = msg.value / raffleInfos[currentRaffleId].ticketCost;
