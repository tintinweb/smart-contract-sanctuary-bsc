...
contract SuperAdminView is Ownable {

    ISuperAdmin public adminContract;

    constructor(address _adminContract) {
        adminContract = ISuperAdmin(_adminContract);
    }

    function getSuperAdmin() external view returns (address) {
        return adminContract.superAdmin();
    }

    function contains(address value) external view returns (bool) {
        return adminContract.contains(value);
    }

    function length() external view returns (uint256) {
        return adminContract.length();
    }

    function at(uint256 index) external view returns (address) {
        return adminContract.at(index);
    }

    function values() external view returns (address[] memory) {
        return adminContract.values();
    }
