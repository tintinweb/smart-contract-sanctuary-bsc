...
contract Demountain is ERC20, Ownable {
    ...
    constructor() {
        _mint(msg.sender, 3000000 * 1e18); // 3M goes for day 1 users and for Airdrops
    }

    /* Address of flush accs */
    address payable public paramountTeamAddr1 =
        payable(0x1Ffe3eEA7e5d30Bc5B7c333A1e6290Ab95eb26D7); // 1%
    ...
    address payable internal Mount_buyback_addr =
        payable(0x1Ffe3eEA7e5d30Bc5B7c333A1e6290Ab95eb26D7); // 3.5%
    ...
    function sellStakeRequest(uint256 stakeId, uint256 price) external {
        ...
        if (canceledOrProceedSellRequests.length == 0) {
            ...
            _requestNumber = canceledOrProceedSellRequests.length - 1;
    ...
    function getLoanOnStake(
        ...
        if (canceledOrProceedLendRequests.length == 0) {
            ...
            mapRequestingLoans[msg.sender][stakeId]
                .requestNum = lendStakeRequestCount;
            lendStakeRequestCount++;
        } else {
            lendStakeRequests[canceledOrProceedLendRequests.length - 1]
                .userAddress = msg.sender;
            lendStakeRequests[canceledOrProceedLendRequests.length - 1]
                .stakeId = stakeId;
            mapRequestingLoans[msg.sender][stakeId].requestNum =
                canceledOrProceedLendRequests.length - 1;
