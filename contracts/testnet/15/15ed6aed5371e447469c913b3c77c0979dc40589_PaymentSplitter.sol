...
contract PaymentSplitter is Context, Ownable {
    ...
    //Show Total Shares of all payee
    function totalShares() public view returns (uint256) {
        return _totalShares;
    }

    //Show total released amount of all payee
    function totalReleased() public view returns (uint256) {
        return _totalReleased;
    }

    //Show a share of a payee
    function shares(address account) public view returns (uint256) {
        return _shares[account];
    }

    //Show Total Released Amount to a payee
    function released(address account) public view returns (uint256) {
        return _released[account];
    }

    //Show amount to be released to a payee
    function releasable(address account) public view returns (uint256) {
        uint256 totalReceived = address(this).balance + totalReleased();
        return _pendingPayment(account, totalReceived, released(account));
    }

    //Show Pending amount of Payee
    ...
    function count_payees() view public returns (uint256) {
        return _payees.length;
    }

    //Remove a payees
    function removepayees(uint index, address account, uint256 shares_) public {
    ...
    function AdminReleaseAllPayments() public onlyOwner {
        for (uint256 i = 0; i < _payees.length; i++) {
            try                 
                PaymentSplitter(payable(address(this))).release(
                    payable(_payees[i])
                )
            {} catch {
                continue;
    ...
    function payeeInformation() view public returns (address[] memory payees) {
        return _payees;
    }
}