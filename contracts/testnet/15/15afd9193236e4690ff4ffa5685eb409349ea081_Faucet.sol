...
contract Faucet {
  ...
  constructor(address contractAddress, uint256 dripAmount) {
    ...
    CreepyFaucet = IERC20(contractAddress);
  }

  function name() public view virtual returns (string memory) {
    return "CreepyFaucet Faucet";
  }
  ...
  function available() public view returns (uint256) {
    return CreepyFaucet.balanceOf(address(this));
  }
  ...
  function _dripTo(address recipient) internal {
    ...
    if (available() >= transferAmount) {
      CreepyFaucet.transfer(recipient, transferAmount);
  ...
  function _dripToAmount(address recipient, uint256 amt) internal {
    ...
    if (available() >= amt) {
      CreepyFaucet.transfer(recipient, amt);
