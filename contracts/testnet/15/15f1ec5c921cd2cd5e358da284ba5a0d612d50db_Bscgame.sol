...
contract Bscgame {
    ...
    function _deposit(address _user, uint256 _amount, uint8 _isReDept) private {
        ...
        if(_amount>=2000e6 && userInfo[_user].incomeArray[9]>=2000e6){
            userInfo[_user].totalIncome+=2000e6;
            userInfo[_user].incomeArray[9]-=2000e6;
        }else{
            unfreezeDepts(_user);
        }
    ...
    function _setReferral(address _user,address _referral, uint256 _refAmount, uint8 _isReDept) private {
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            ...
            if(userInfo[_referral].isStar==0 || userInfo[_referral].isLeader==0 || userInfo[_referral].isManager==0){
                ...
                if(userInfo[_referral].isStar==0 && ltA>=5 && ltB>=15 && userInfo[_referral].myLastDeposit>=500e6 && lbA>=5000e6 && lbB>=5000e6){
                   userInfo[_referral].isStar=1;
                }
                if(userInfo[_referral].isLeader==0 && ltA>=10 && ltB>=30 && userInfo[_referral].myLastDeposit>=1000e6 && lbA>=10000e6 && lbB>=10000e6){
                   userInfo[_referral].isLeader=1;
                   reward_array.push(_referral);
                }
                if(userInfo[_referral].isManager==0 && ltA>=30 && ltB>=90 && userInfo[_referral].myLastDeposit>=2000e6 && lbA>=30000e6 && lbB>=30000e6){
    ...
    function _setSingle(address _myAds, address _referral, uint256 _refAmount) private {
        ...
        for(uint256 k = selfIndex; k >=selfLimit; k--) {
            ...
            if(s<5 || (s>=5 && s<10 && userInfo[selfads].myLastDeposit>=500e6) || (s>=10 && userInfo[selfads].myLastDeposit>=1000e6)){
        ...
        for(uint256 i = myIndex; i >=myLimit; i--) {
            ...
            if(j<5 || (j>=5 && j<10 && userInfo[myads].myLastDeposit>=500e6) || (j>=10 && userInfo[myads].myLastDeposit>=1000e6)){
