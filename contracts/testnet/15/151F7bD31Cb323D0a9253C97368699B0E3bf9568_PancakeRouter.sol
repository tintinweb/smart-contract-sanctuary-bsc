...
abstract contract IPancakeRouter01 {
    function factory() external virtual view returns (address);
    function WETH() external virtual view returns (address);
...
abstract contract IPancakeRouter02 is IPancakeRouter01 {
...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(bytes20(keccak256(abi.encodePacked(
                ...
                hex'702caf6a0293529dabcd46bb89d7e70826935a0c1415a3b4fa6743ae17ff9e02' // init code hash of factory address
            )))));
    }
...
contract PancakeRouter is FeeStore, IPancakeRouter02 {
    ...
    // using SafeMath for uint; REMOVE
    ...
    constructor(address _factory, address _WETH, address _BUSD) {
        factory = _factory;  
        WETH = _WETH; 
        BUSD = _BUSD;
    }
    ...
    receive() external payable {
        // assert(msg.sender == WETH); // only accept ETH via fallback from the WETH contract REMOVED
    ...
    function _addLiquidity(
        ...
        if (IPancakeFactory(WETH).getPair(tokenA, tokenB) == address(0)) {
            if(tokenA == WETH) {
                IPancakeFactory(WETH).createPair(tokenB, tokenA, supportsTokenFee);
                pairFeeAddress[getPair(tokenA,tokenB)] = tokenA;
            } else {
                IPancakeFactory(WETH).createPair(tokenA, tokenB, supportsTokenFee);
                pairFeeAddress[getPair(tokenA,tokenB)] = tokenB;
            }
        }
        (uint reserveA, uint reserveB) = PancakeLibrary.getReserves(WETH, tokenA, tokenB);
    ...
    function getPair(address tokenA,address tokenB) public view returns (address){
        return IPancakeFactory(WETH).getPair(tokenA, tokenB);
    }
    ...
    function addLiquidity(
        ...
        address pair = PancakeLibrary.pairFor(WETH, tokenA, tokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = PancakeLibrary.pairFor(WETH, token, WETH);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? PancakeLibrary.pairFor(WETH, output, path[i + 2]) : _to;
            IPancakePair(PancakeLibrary.pairFor(WETH, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        amounts = PancakeLibrary.getAmountsOut(WETH, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(WETH, path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = PancakeLibrary.getAmountsIn(WETH, amountOut, path);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = PancakeLibrary.getAmountsOut(WETH, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(WETH, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
    ...
    function _calculateFees(address input, address output, uint amountIn, uint amount0Out, uint amount1Out) internal view virtual returns (uint amount0Fee, uint amount1Fee) {
        IPancakePair pair = IPancakePair(PancakeLibrary.pairFor(WETH, input, output));
        (address token0,) = PancakeLibrary.sortTokens(input, output);
        address baseToken = pair.baseToken();
        uint totalFee = pair.getTotalFee();
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(path.length == 2, "PancakeRouter: ONLY_TWO_TOKENS_ALLOWED");
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(WETH, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(WETH, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to) - (balanceBefore) >= amountOutMin,
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return PancakeLibrary.getAmountsOut(WETH, amountIn, path);
    }
    ...
    returns (uint[] memory amounts)
    {
        return PancakeLibrary.getAmountsIn(WETH, amountOut, path);
    }
}
