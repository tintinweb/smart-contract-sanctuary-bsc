...
/**
 * @title Token vesting contract for clone factory
 */
contract TokenVestingCloneable is Ownable {
    ...
    function addBeneficiaries(address[] memory addresses, uint256[] memory amounts) external onlyOwner {
    ...
    function _getAmountToClaim(address beneficiary) private view returns (uint256 amount) {
        ...
        for (uint256 i = 0; i < _vestings.length; i++) {
            if (block.timestamp < _vestings[i].claimTime) {
                break;
            }
