...
import "contracts/LockSimple.sol";
import "contracts/LockVesting.sol";

contract LockFactory{

    event LogCreateSimpleLock(LockSimple lock, address owner);
    event LogCreateVestingLock(LockVesting lock, address owner);

    function createSimpleLock(address _owner, uint _duration, uint256 amount, address _token, address payable _feeAddr, uint256 _fee) external{
        LockSimple lock = new LockSimple(_owner,_duration,amount, _token, _feeAddr, _fee);
        emit LogCreateSimpleLock(lock, _owner);
    }
    ...
    function createVestingLock(
        ...
        LockVesting lock = new LockVesting(
        _numberOfPortions,
        timeBetweenPortions,
        ...
        _feeAddr,
        _fee);
        emit LogCreateVestingLock(lock, _participant);
    }
}
...
contract LockSimple {
    ...
    event LogLock(uint end, uint256 lockedAmount);
    event LogWithdraw(address owner, uint256 lockedAmount);
    ...
    constructor(address _owner, uint _duration, uint256 amount, address _token, address payable _feeAddr, uint256 _fee){
       owner = _owner;
       duration = _duration;
       lockedAmount = amount;
       feeAddr = _feeAddr;
    ...
    function lock() external payable onlyOwner {
       require(msg.value >= fee, "Not enough BNB sent");
       require(token.balanceOf(msg.sender) >= lockedAmount, "Insufficient funds.");
       require(token.allowance(msg.sender, address(this)) >= lockedAmount, "Insufficient allowance.");
       end = block.timestamp + duration;
       token.transferFrom(msg.sender, address(this), lockedAmount);
       feeAddr.transfer(msg.value);
       emit LogLock(end, lockedAmount);
    }

    function withdaraw() external onlyOwner{
       require(block.timestamp >= end, "too early");
       uint256 contractBalance = lockedAmount;
       uint256 contractBalanceWReflectionsOrDividends = token.balanceOf(address(this));
       uint256 reflectionsOrDividends = contractBalanceWReflectionsOrDividends.sub(contractBalance);

       /** 
        * @notice Transfers accumulated reflections to the owner 
        */
       if(reflectionsOrDividends > 0){
          token.transfer(owner, contractBalanceWReflectionsOrDividends);
       } else {
...
contract LockVesting {
    ...
    mapping(address => Vesting) public addressToVesting;
    ...
    {
        require(msg.value >= fee, "Not enough BNB sent");
        token.transferFrom(participant, address(this), participationAmount);
        require(participationAmount <= token.balanceOf(address(this)),
        ...
        uint TGEPortionAmount = participationAmount.mul(tgePortionPercent).div(100);
        
        uint vestedAmount = participationAmount.sub(TGEPortionAmount);
        ...
        addressToVesting[participant] = p;
    ...
    external
    {
        ...
        Vesting storage p = addressToVesting[user];

        uint256 totalToWithdraw = 0;

        // Initial portion can be withdrawn
        if(!p.TGEPortionWithdrawn && block.timestamp >= TGEPortionUnlockingTime) {
            totalToWithdraw = totalToWithdraw.add(p.TGEPortion);
            // Mark initial portion as withdrawn
            p.TGEPortionWithdrawn = true;
        }
    ...
    {
        Vesting memory p = addressToVesting[account];
