...
contract LiquidityController is TokensRecoverable, ILiquidityController
{
    ...
    constructor(ISwapRouter02 _router, IERC20 _base, IERC20 _rooted, IERC31337 _elite, IFloorCalculator _calculator, RootedTransferGate _gate) 
    {
        swapRouter = _router;
        base = _base;
        elite = _elite;
        ...
        ISwapFactory _swapFactory = ISwapFactory(_router.factory());
        swapFactory = _swapFactory;        
        
        _base.approve(address(_elite), uint256(-1));
        _base.approve(address(_router), uint256(-1));
        _rooted.approve(address(_router), uint256(-1));
        IERC20 _rootedBaseLP = IERC20(_swapFactory.getPair(address(_base), address(_rooted)));
        _rootedBaseLP.approve(address(_router), uint256(-1));
        rootedBaseLP = _rootedBaseLP;
        _elite.approve(address(_router), uint256(-1));
        IERC20 _rootedEliteLP = IERC20(_swapFactory.getPair(address(_elite), address(_rooted)));
        _rootedEliteLP.approve(address(_router), uint256(-1));
