...
contract Fuckies_Testnet is ERC20, Pausable, Ownable {

    event FreeTokensMinted(
        address wallet
    );

    mapping(address => bool) public whitelisted;

    mapping (address => uint) private lastGetTokens;
    ...
    function getFreeTokens() public {
        require(block.timestamp >= lastGetTokens[_msgSender()] + 1 days, "Try it again tomorrow");
        _mint(_msgSender(), 100000000000000000000000);
        lastGetTokens[_msgSender()] = block.timestamp;
        emit FreeTokensMinted(_msgSender());
    }
