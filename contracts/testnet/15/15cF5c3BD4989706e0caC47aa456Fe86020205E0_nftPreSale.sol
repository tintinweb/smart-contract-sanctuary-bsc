...
contract nftPreSale is Ownable{
    ...
    uint[6][] public boxesBought;
    uint[6] ZERO_ARRAY = [0,0,0,0,0,0];
    uint counter;
    ...
    constructor() {
        boxSales.push(BoxSale(22*10e15, 1000)); //truck rare
        boxSales.push(BoxSale(62*10e15, 1000)); //truck epic
        boxSales.push(BoxSale(100*10e15, 500)); //truck legendary
        boxSales.push(BoxSale(22*10e15, 800)); //company rare
        boxSales.push(BoxSale(62*10e15, 500)); //company epic
        boxSales.push(BoxSale(100*10e15, 200)); //company legendary
    ...
    function getBuys(address who) external view returns(uint[6] memory){
        return boxesBought[buyerID[who]];
    }

    function buyBox(uint box, uint quantity_) external payable{
        require(isPresale, "No presale right now.");
        if (isWhitelist){
            require(whitelist[msg.sender], "You are not in the whitelist.");
        }
        require(boxSales[box].quantity >= quantity_, "Not enough boxes left.");
        uint amount = quantity_*boxSales[box].price;
        require(msg.value == amount, "Wrong value sent.");
        boxSales[box].quantity -= quantity_;
