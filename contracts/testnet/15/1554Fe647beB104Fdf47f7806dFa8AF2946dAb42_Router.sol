...
interface ITokenReward {
    function mint(address to) external view returns (uint256) ;
}
...
// fee protocol -> token reward ( burn nhan nft + token ) + receive reward
// fee make transaction -> distribution to holder of token reward
// burn token reward :  x tokennft + y token
interface Factory {
...
contract Router {
    ...
    function addLiquidity(
        ...
        address to, 
        uint deadline
        ) external virtual ensure(deadline) returns(uint amountErc, uint amountNft, uint amounTokenReward) {
        // mint token reward
        (amountErc, amountNft) = _addLiquidity(tokenerc, tokennft, id, amountErcDesired, amountNftDesired, amountErcMin, amountNftMin);
        ...
        amounTokenReward = ITokenReward(_token).mint(to);
    }
    function _addLiquidity(
        ...
        uint amountErcMin,
        uint amountNftMin
    ) internal virtual returns (uint amountErc, uint amounNft) {
