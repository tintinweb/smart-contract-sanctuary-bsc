...
contract OTC is Ownable {
    ...
    uint256 public fiatDecimal = 1e18;
    bool public pause;
    ...
    // order event
    event CreateOrder(address indexed token, uint256 indexed adId, uint256 indexed orderId, uint256 amount, uint256 price, uint256 fiat, uint256 timestamp, address nextAddress, uint256 adResidue);
    event ConfirmPayment(uint256 indexed orderId, uint256 timestamp, address nextAddress);
    event ConfirmReceive(uint256 indexed orderId, uint256 timestamp, address nextAddress);
    event Complaint(uint256 indexed orderId, uint256 timestamp);
    event CancelOrder(uint256 indexed orderId, uint256 timestamp, address nextAddress);
    ...
    function deleteAd(uint8 _adType, uint256 _adId) external returns (uint256) {
        ...
        require(_sender == ad.owner, "OTC : Have the right to operate !");
        require(ad.status == 1, "OTC : Ad Status Error !");
    ...
    function createOrder(uint8 _orderType, uint256 _adId, uint256 _amount) public returns (uint256) {
        ...
        emit CreateOrder(_order.tokenAddress, ad.id, _order.id, _order.amount, _order.price, _order.fiat, block.timestamp, _order.nextAddress, ad.amount);
    ...
    function cancelOrder(uint8 _orderType, uint256 _orderId) public returns (bool){
        ...
        require(msg.sender == _order.nextAddress, 'Have the right to operate !');
        ...
        emit CancelOrder(_orderId, block.timestamp, _order.nextAddress);
    ...
    function confirmPay(uint8 _orderType, uint256 _orderId) public returns (bool){
        ...
        emit ConfirmPayment(_orderId, block.timestamp, _order.nextAddress);
    ...
    function confirmReceive(uint8 _orderType, uint256 _orderId) public returns (bool){
        ...
        emit ConfirmReceive(_orderId, block.timestamp, _order.nextAddress);
    ...
    function complaintOrder(uint8 _orderType, uint256 _orderId) public returns (bool){
        ...
        require(_order.status == 1 || _order.status == 2, "OTC : Orders status err");
        ...
        if (_order.status == 1) {
            ...
            require(_order.payTimestamp + timeOut < block.timestamp, "OTC : Orders are not grievable");
        }
    ...
    function getUserOrderLength(address _user) public view returns (uint256){
        return userOrderList[_user].length;
    }
    ...
    function _cancelSellOrder(Order storage _order) private {
        ...
        IERC20(_order.tokenAddress).transfer(_order.owner, _order.amount);
    }
    ...
    function _cancelBuyOrder(Order storage _order) private {
        ...
        IERC20(_order.tokenAddress).transfer(_order.traderAddress, _order.amount);
    }
...
contract Trader is WithdrawAll {
    ...
    event SetTraderData(address trader, string newData);
    ...
    function inviteOwner(address from, address superior, uint256 bailAmount, uint256 status, string memory _data) public onlyOwner returns (bool) {

        require(inviter[from] == address(0), "Trader: The current user has been bound");
        require(bailAmount > 0 || status == 1, "Trader: New trader status error !");

        _invite(from, superior);

        if (bailAmount > 0) _addBail(bailAmount);

        TraderData storage _trader = traderList[msg.sender];
        if (bailAmount == 0) _trader.status = status == 0 ? 0 : 1;
        _setTraderData(_trader, _data);
    ...
    function _setTraderData(TraderData storage _trader, string memory _data) private {
        if (keccak256(abi.encodePacked(_data)) == keccak256(abi.encodePacked(''))) return;

        _trader.data = _data;
        emit SetTraderData(msg.sender, _data);
