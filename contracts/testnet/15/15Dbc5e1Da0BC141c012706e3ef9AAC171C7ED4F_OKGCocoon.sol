...
contract OKGCocoon is ERC721Enumerable {
	...
	event Cocoon(address[] receivers, uint256 fromID, uint256 toID);

	constructor(address _gov, string memory _uri) ERC721("OKG Cocoon", "OKGC") {
    ...
    */
	...
	/**
       	@notice Mint Cocoon NFT to `_receivers`
       	@dev  Caller must have MINTER_ROLE
		@param	_receivers		        A list of Receivers' addresses
		@param	_fromTokenId			A list of `tokenId` that will be minted to `_receiver`
    */
	function cocoon(address[] calldata _receivers, uint256 _fromTokenId)
		external
		isLocked
		onlyMinter
	{
		uint256 _size = _receivers.length;
		for (uint256 i; i < _size; i++) _safeMint(_receivers[i], _fromTokenId + i);

		emit Cocoon(_receivers, _fromTokenId, _fromTokenId + _size - 1);
	}
	...
	function _beforeTokenTransfer(
		...
		if (address(restriction) != address(0)) {
			if (!restriction.whitelisted(msg.sender)) {
				uint256 _restricted = restriction.allowances(address(this), tokenId);
				if (
					_restricted == type(uint64).max &&
					(from != address(0) || to != BLACK_HOLE)
				) revert("Not for Trade");
				else if (
					_restricted == type(uint32).max &&
					(!gov.hasRole(MARKETPLACE, msg.sender) || restriction.stores(to))
				) revert("Restricted listing");

				//	Item can be `Restricted Listing` and `On Leasing` concurrently
				require(
					restriction.onLeasing(address(this), tokenId) < block.timestamp,
					"On leasing"
				);
			}
		}
