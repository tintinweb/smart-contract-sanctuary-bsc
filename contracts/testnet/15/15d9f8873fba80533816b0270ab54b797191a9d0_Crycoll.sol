...
contract Crycoll is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldProvideLP(amount)) { provideLP(amount); }
        ...
        if(recipient == bankAddress && sender != pair){ bank.depositToBank(sender, amountReceived); }          
    ...
    function distributeFee() internal swapping {
    ...
    function provideLPManual(uint256 amount) external {
        provideLP(amount);
    }
