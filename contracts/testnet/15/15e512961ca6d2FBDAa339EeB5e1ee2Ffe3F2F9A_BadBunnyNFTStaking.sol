...
contract BadBunnyNFTStaking is Auth, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    mapping(uint256 => uint256) public nftDepositTime;
    mapping(uint256 => uint256) public nftDepositDays;
    mapping(uint256 => uint256) public nftDepositLevel;
    mapping(uint256 => bool) public allowedEmergency;
    bool public emergencyWithdrawlOn = false;

    uint256 lockPeriodOne = 30;
    uint256 lockPeriodTwo = 60;
    uint256 lockPeriodThree = 90;
    uint256 lockPeriodFour = 120;
    ...
    function calculateRewards(address account, uint256[] memory tokenIds) public view returns (uint256[] memory rewards) {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        rewards[i] = rate * (_deposits[account].contains(tokenId) ? 1 : 0) * (Math.min(block.number, expiration) - _depositBlocks[account][tokenId]) * nftDepositLevel[tokenId] / 2;}
        return rewards;
    }
    ...
    function calculateReward(address account, uint256 tokenId) public view returns (uint256) {
      ...
      return rate * (_deposits[account].contains(tokenId) ? 1 : 0) * (Math.min(block.number, expiration) - _depositBlocks[account][tokenId]) * nftDepositLevel[tokenId] / 2;
    }
    ...
    function setNFTDeposit(uint256[] calldata tokenIds) internal {
        for (uint256 i; i < tokenIds.length; i++) {
        if(nftDepositDays[tokenIds[i]] < lockPeriodOne){nftDepositLevel[tokenIds[i]] = multOne;}
        if(nftDepositDays[tokenIds[i]] >= lockPeriodOne && nftDepositDays[tokenIds[i]] < lockPeriodTwo){nftDepositLevel[tokenIds[i]] = multTwo;}
        if(nftDepositDays[tokenIds[i]] >= lockPeriodTwo && nftDepositDays[tokenIds[i]] < lockPeriodThree){nftDepositLevel[tokenIds[i]] = multThree;}
        if(nftDepositDays[tokenIds[i]] >= lockPeriodThree && nftDepositDays[tokenIds[i]] < lockPeriodFour){nftDepositLevel[tokenIds[i]] = multFour;}
        if(nftDepositDays[tokenIds[i]] >= lockPeriodFour){nftDepositLevel[tokenIds[i]] = multFive;}}
    }

    function deposit(uint256[] calldata tokenIds, uint256 lockDays) external whenNotPaused {
        require(msg.sender != stakingDestinationAddress, "Invalid address");
        uint256 lockTime = (lockDays * 86400);
        for (uint256 i; i < tokenIds.length; i++) {
            IERC721(stakingDestinationAddress).safeTransferFrom(msg.sender,address(this),tokenIds[i],"");
            _deposits[msg.sender].add(tokenIds[i]);
            nftDepositTime[tokenIds[i]] = (block.timestamp + lockTime);
            nftDepositDays[tokenIds[i]] = lockDays;}
            setBlock(tokenIds);
            setNFTDeposit(tokenIds);
    }

    function withdraw(uint256[] calldata tokenIds) external nonReentrant() {
        for (uint256 i; i < tokenIds.length; i++) {
            require(nftDepositTime[tokenIds[i]] <= block.timestamp);
    ...
    function emergencyWithdraw(uint256[] calldata tokenIds) external nonReentrant() {
        for (uint256 i; i < tokenIds.length; i++) {
            require(emergencyWithdrawlOn || allowedEmergency[tokenIds[i]]);
            require( _deposits[msg.sender].contains(tokenIds[i]),"Staking: token not deposited");
            _deposits[msg.sender].remove(tokenIds[i]);
            IERC721(stakingDestinationAddress).safeTransferFrom(address(this), msg.sender, tokenIds[i],"");}
            setBlock(tokenIds);
    ...
    function setLockTimeTokenId(uint256[] calldata tokenIds, uint256 _lockTime) external authorized {
        for (uint256 i; i < tokenIds.length; i++) {
        nftDepositTime[tokenIds[i]] = _lockTime;}
    }

    function setNFTLockLevel(uint256[] calldata tokenIds, uint256 _level) external authorized {
        for (uint256 i; i < tokenIds.length; i++) {
        nftDepositLevel[tokenIds[i]] = _level;}
    }

    function setIndividualEmergency(uint256 tokenId, bool _allowed) external authorized {
        allowedEmergency[tokenId] = _allowed;
    }

    function setMultLevel(uint256 _multOne, uint256 _multTwo, uint256 _multThree, uint256 _multFour, uint256 _multFive) external authorized {
        multOne = _multOne;
        multTwo = _multTwo;
        multThree = _multThree;
        multFour = _multFour;
        multFive = _multFive;
    }

    function setEmergencyWithdrawl(bool _on) external authorized {
        emergencyWithdrawlOn = _on;
    }

    function setLockPeriods(uint256 _lockOne, uint256 _lockTwo, uint256 _lockThree, uint256 _lockFour) external authorized {
        lockPeriodOne = _lockOne;
        lockPeriodTwo = _lockTwo;
        lockPeriodThree = _lockThree;
        lockPeriodFour = _lockFour;
    }
