...
contract Cheemspad is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'CHEEMSPAD';
    string private constant _symbol = 'CPAD';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 200000 * 10**_decimals;

    //Amount to Swap variable
    uint256 currentAmountToSwap = 150 * 10**_decimals;
    ...
    uint16 public constant MaxBuyLockTime= 19 seconds;
    ...
    //DevWallets
    address public TeamWallet=payable(0xaEEE930D7Dc148862051CC0F43114FedAbAF34BC);
    address public CINUBuyback=payable(0x376A67BFc7F42D5C07f9F1F6425e8c94567CC11F);
    //TestNet
	...
	uint256 private antiWhale = 500 * 10**_decimals;

    //Used for anti-bot autoblacklist
    uint256 private tradingEnabledAt; //DO NOT CHANGE, THIS IS FOR HOLDING A TIMESTAMP
	uint256 private autoBanTime = 65; // Set to the amount of time in seconds after enable trading you want addresses to be auto blacklisted if they buy/sell/transfer in this time.
    ...
    function _addLiquidity(uint256 tokenamount, uint256 bnbamount) private returns (uint256 tAmountSent, uint256 bnbAmountSent) {
        ...
        uint256 minBNB = (bnbamount*50) / 100;
    ...
    function TeamWithdrawALLMarketingBNB() external onlyOwner{
        ...
        payable(CINUBuyback).transfer((amount-(amount*50) / 100));
        emit Transfer(address(this), TeamWallet, (amount*50) / 100);
        emit Transfer(address(this), CINUBuyback, (amount-(amount*50) / 100));
    } 
    ...
    function TeamChangeCINUBuyback(address newCINUBuyback) external onlyOwner{
      require(newCINUBuyback != address(0),
      "Error: Cannot be 0 address.");
      CINUBuyback=payable(newCINUBuyback);
      emit ContractChangedAddress(CINUBuyback);
    }
