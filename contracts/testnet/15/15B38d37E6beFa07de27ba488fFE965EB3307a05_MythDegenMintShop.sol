...
contract MythDegen is ERC721 {
    ...
    mapping(uint256 => mapping(uint256 => bool)) public freeCosmetics; //Mapping of cosmetics unlocked by all degens
    ...
    function burnToken(uint256 _id) external isDegenWhitelisted {
    ...
    function changeDegenCosmetic(
        ...
        require(
            unlockedCosmetics[_degenId][_cosmeticAddress][_layerId][_itemId] ||
                freeCosmetics[_layerId][_itemId],
    ...
    function getImageLayers(uint256 _id) public view returns (string memory) {
        ...
        for (uint256 i = 0; i < 9; i++) {
            string memory tempIMG = concatenate('<image href="', tempList[i]);
