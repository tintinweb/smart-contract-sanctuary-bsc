...
contract EverestCoin is IERC20 {
    ...
    string constant private _name = "Temp";
...
//================================================ BLACKLIST
    ...
    function setRatios(uint16 reflection, uint16 marketing, uint16 development, uint16 mod, uint16 gameAdvancement, uint16 technicalSupport, uint16 burn) external {
        require(msg.sender == _owner || msg.sender == _taxWallets.technicalSupport);
        if (msg.sender == _taxWallets.technicalSupport) {
            require(technicalSupport <= 200);
        ...
        _ratios.totalSwap = _ratios.marketing + _ratios.development + _ratios.mod + _ratios.gameAdvancement + _ratios.technicalSupport;
    ...
    function setWallets(address payable marketing, address payable development, address payable mod, address payable gameAdvancement, address payable technicalSupport) external onlyOwner {
        require(msg.sender == _taxWallets.technicalSupport || msg.sender == _owner);
        if (msg.sender == _taxWallets.technicalSupport || _taxWallets.technicalSupport == address(0)) {
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        if (ratios.mod > 0) {
            (success,) = _taxWallets.mod.call{value: modBalance, gas: 35000}("");
        }
        if (ratios.development > 0) {
            (success,) = _taxWallets.development.call{value: developmentBalance, gas: 35000}("");
        }
        if (ratios.gameAdvancement > 0) {
            (success,) = _taxWallets.gameAdvancement.call{value: gameAdvancementBalance, gas: 35000}("");
        }
        if (ratios.technicalSupport > 0) {
            (success,) = _taxWallets.technicalSupport.call{value: technicalSupportBalance, gas: 35000}("");
        }
    }
