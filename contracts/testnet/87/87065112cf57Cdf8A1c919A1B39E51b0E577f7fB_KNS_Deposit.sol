...
contract KNS_Deposit is OwnableUpgradeable {
    ...
    event Deposit(address indexed player, uint indexed slot, bool indexed isRenew, uint amount);
    ...
    event ChangeRenew(address indexed player, uint indexed slot, bool indexed isRenew);
    event ClaimStage(address indexed player,uint indexed amount);
    event ClaimDynamic(address indexed player,uint indexed amount);
    ...
    function deposit(uint amount, bool _isRenew, address invitor) external onlyEOA {
        ...
        if (user.invitor == address(0)) {
            require(invitor == address(this) || userInfo[invitor].invitor != address(0), 'invitor error');
        ...
        emit Deposit(msg.sender, id, _isRenew, amount);
        id++;
        totalDeposit += amount;
    }
    ...
    function changeRenew(uint slotId, bool isRenew) external onlyEOA {
        SlotInfo storage slot = slotInfo[slotId];
        require(slot.owner == msg.sender, 'not owner');
        ...
        emit ChangeRenew(msg.sender, slotId, isRenew);
    }
    ...
    function buyPartner() external onlyEOA {
        require(round < 4, 'no amount');
        require(roundLeft[round] > 0, 'sell out');
        require(!isPartner[msg.sender], 'already partner');
        U.transferFrom(msg.sender, address(this), roundPrice[round]);
        userInfo[msg.sender].partnerCost = roundPrice[round];
        isPartner[msg.sender] = true;
        roundLeft[round] --;
        if (roundLeft[round] == 0) {
            round ++;
        }
    }

    function calculatePartner(address addr) public view returns (uint){
        if (startTime == 0) {
            return 0;
        }
        UserInfo storage user = userInfo[addr];

        uint cost = user.partnerCost;
        if (cost == 0) {
            return 0;
        }
        uint time = block.timestamp - startTime;
        uint times = time / (86400 * 30);
        uint rate;
        if (times > 5) {
            rate = 100;
        } else {
        ...
        uint rew = cost * rate / 100 - user.partnerClaimed;

        return rew;
    }

    function claimPartner() external onlyEOA {
        require(isPartner[msg.sender], 'not partner');
        uint rew = calculatePartner(msg.sender);
        require(rew != 0, 'no reward');
        U.transfer(msg.sender, rew);
        userInfo[msg.sender].partnerClaimed += rew;
    }
    ...
    function claimDynamic(uint reward,uint totalReward, bytes32 r, bytes32 s, uint8 v) external onlyEOA{
        bytes32 hash = keccak256(abi.encodePacked(msg.sender,reward,totalReward));
        address a = ecrecover(hash,v,r,s);
        require(a == banker,'not banker');
        require(userInfo[msg.sender].dynamicClaimed + reward <= totalReward,'out of limit');
        U.transfer(msg.sender,reward);
        userInfo[msg.sender].dynamicClaimed += reward;
        emit ClaimDynamic(msg.sender,reward);
    }
