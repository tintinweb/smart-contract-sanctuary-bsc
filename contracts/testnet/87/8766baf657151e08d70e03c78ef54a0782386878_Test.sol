...
contract Test is Context, ERC20, Ownable {
    ...
    mapping(address => uint256) private lastBuyList;
    ...
    uint256 public buyFeeRateInit = 15;
    uint256 public startTime;
    uint256 public LimitBuytime;
    ...
    uint256 public limitThreshold = 200;
 ...
 */
    ...
    modifier notGreaterThanLimitThreshold(uint256 rate) {
        require(rate < limitThreshold, "Max Limit threshold is 2%");
        _;
    }
 ...
 */
   ...
   function checkLastTime(address account) public view returns (bool) {
        return lastBuyList[account] > 0;
    }
    ...
    function checkLimitBalance(address account) public view returns (bool) {
        return this.balanceOf(account) < 250*10**6 * 10**18*200/10000;
    }
 ...
 */
    function _transfer(
        ...
        if( block.timestamp < startTime + LimitBuytime){
            require(!checkLastTime(recipient),"Wait!");
            lastBuyList[recipient] = 1;
            require(amount < LimitToken * 10**18, "Limit Token!");
        ...
        if (checkAntiBot()) {
            ...
            if (recipient == uniswapV2Pair) {
                ...
                if (checkAdmin(recipient)) {
                    ...
                    require(checkLimitBalance(recipient),"Limit Balance");
                    
                    if (block.timestamp < startTime + 600) {
                        feeRate = buyFeeRateInit;
                    }else{
                        feeRate = buyFeeRate;
                    }
