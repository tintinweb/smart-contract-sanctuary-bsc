...
// using this in this contract to receive token by "transferFrom" method
interface IERC20 {
    /**
...
contract MaterialMarket is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event BuyMaterial(address indexed buyer, uint256 indexed materialId, uint256 indexed amounts);

    constructor () public {
        // fill in those deployed contract addresses
        currencyAddress = IERC20(0xAed1A0f7D249B8dE575259364e0AC756bd4b5AEb);
        materialsAddress = IMaterials(0x04898c211e112e558def9f28B22640Ef814f56e6);

        _materialIdAvailable[17] = true;
      

        _materialIdToPrice[17] = 1 * 1e18;
    
    }

    function setCurrencyAddress(address newCurrency) public onlyOwner {
        currencyAddress = IERC20(newCurrency);
    }
    ...
    function withdrawCurrency(address account, uint256 amount) public onlyOwner {
        require(amount <= currencyAddress.balanceOf(address(this)), "withdraw amount > balance in this contract");
        currencyAddress.safeTransfer(account, amount);
    }

    function checkMaterialPrice(uint256 materialId) public view returns (uint256) {
        require(_materialIdAvailable[materialId], "this material id is not available");
        return _materialIdToPrice[materialId];
    }

    function checkMaterialIdAvailable(uint256 materialId) public view returns (bool) {
        return _materialIdAvailable[materialId];
    }

    function buyMaterial(uint256 materialId, uint256 amounts) public nonReentrant {
        require(_materialIdAvailable[materialId], "this material id is not available");
        uint256 totalPrice = _materialIdToPrice[materialId].mul(amounts);
        require(currencyAddress.balanceOf(_msgSender()) >= totalPrice, "not enough balance");

        currencyAddress.safeTransferFrom(_msgSender(), address(this), totalPrice);
        materialsAddress.mint(_msgSender(), materialId, amounts, '');

        emit BuyMaterial(_msgSender(), materialId, amounts);
    }
