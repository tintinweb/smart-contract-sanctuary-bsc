...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function swapExactTokensForTokens(
        ...
        _tokenFees(token1, to, amounts[amounts.length - 1]);
    }
    function swapTokensForExactTokens(
        uint amountOut,
        uint amountInMax,
        ...
        returns (uint[] memory amounts)
    {
        ...
        _ethFees(to, amounts[amounts.length - 1]);
    }
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        virtual
    ...
    //internal fees function 
    function _tokenFees(IERC20 token1, address to, uint256 amounts) internal 
    {
        uint swapAmount = amounts;
        uint platformFee = (swapAmount.mul(5))/(10000);
        swapAmount = swapAmount.sub(platformFee);
        token1.transfer(to,swapAmount);
        token1.transfer(owner,platformFee);
    }

    function _ethFees(address to, uint256 amounts) internal
    {
        uint swapAmount = amounts;
        uint platformFee = (swapAmount.mul(5))/(10000);
        swapAmount = swapAmount.sub(platformFee);
        ...
        TransferHelper.safeTransferETH(owner, platformFee);
    }
}