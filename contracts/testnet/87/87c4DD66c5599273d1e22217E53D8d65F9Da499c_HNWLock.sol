...
contract HNWLock is IPinkLock, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function lock(
        ...
        uint256 _amount = safeTransferFromEnsureExactAmount(
            token,
            msg.sender,
            address(this),
            amount
        ...
        if (isLpToken) {
            ...
            id = _lockNormalToken(owner, token, _amount, unlockDate);
        }
        emit LockAdded(id, token, owner, _amount, unlockDate);
        return id;
    }
    ...
    function editLock(
        ...
        if (newAmount > 0) {
            ...
            if (diff > 0) {
                uint256 _diff = safeTransferFromEnsureExactAmount(
                    userLock.token,
                    msg.sender,
                    address(this),
                    diff
                ...
                userLock.amount = userLock.amount + _diff;

                CumulativeLockInfo storage tokenInfo = cumulativeLockInfo[
                    userLock.token
                ];
                tokenInfo.amount = tokenInfo.amount + _diff;
            }
        }
        payfee(msg.sender, feetype);
        emit LockUpdated(
    ...
    function safeTransferFromEnsureExactAmount(
        ...
        require(
            newRecipientBalance - oldRecipientBalance > 0,
