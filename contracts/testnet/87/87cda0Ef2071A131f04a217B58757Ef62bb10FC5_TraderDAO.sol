...
contract TraderDAO is Ownable{
    ...
    mapping(address => mapping(uint => uint)) public UserStakeAmount; // UserAddress => PoolId => UserStakeAmount
    mapping(uint => uint) public PoolTotalDepositor; // PoolId => TotalCount
    mapping(uint => uint) public PoolPayoutTimestamp; // PoolId => PayoutTime
    mapping(uint => uint) public PoolRewardAmount;
    mapping(uint => uint) public PoolRewardCounter;
    mapping(uint => bool) public PoolRewardDistributed;
    mapping(uint => uint) public PoolTotalStakeAmount;
    ...
    event TradeStarted(uint indexed _poolid, uint indexed _starttime, uint indexed _endtime);
    ...
    function Deposit(uint amount) public{
        ...
        USDT.safeTransfer(PlatformAccount, amount);

        //Check if user is already in pool
        if(UserInPool[msg.sender][PoolId] != 0){
            UserStakeAmount[msg.sender][PoolId] = UserStakeAmount[msg.sender][PoolId] + amount; 
            UserCapitalAmountLock[msg.sender] = UserCapitalAmountLock[msg.sender] + amount;
        }
        else{
            PoolIdCount++;
            PoolData[PoolId][PoolIdCount] = msg.sender;
            UserInPool[msg.sender][PoolId] = PoolIdCount;
            UserStakeAmount[msg.sender][PoolId] = amount;
            UserCapitalAmountLock[msg.sender] = UserCapitalAmountLock[msg.sender] + amount;
        }
        
        PoolTotalStakeAmount[PoolId] = PoolTotalStakeAmount[PoolId] + amount;
        emit UserDeposit(msg.sender, PoolId, amount);
    }

    function StartTrade(uint _payoutTimeStamp) external onlyOwner{
        USDT.safeTransferFrom(PlatformAccount, address(this), PoolTotalStakeAmount[PoolId]);
        USDT.safeTransfer(TraderAccount, PoolTotalStakeAmount[PoolId]);
        PoolPayoutTimestamp[PoolId] = _payoutTimeStamp;
        
        emit TradeStarted(PoolId, block.timestamp, _payoutTimeStamp);
    ...
    function UpdateAPY(uint _poolid, uint _depositamount) external onlyOwner{
        DistributeReward(_poolid, PoolTotalDepositor[_poolid], _depositamount);
    }
    ...
    function DistributeReward(uint _poolid, uint _counter, uint _depositamount) public onlyOwner{
        ...
        for(i; i <= _counter; i++){
            ...
            uint rewardBeforeFee = CalculateReward(userstakeamount, PoolRewardAmount[_poolid]);
            uint fee = CalculateFee(userstakeamount);
            UserRewardAmount[useraddress][_poolid] = rewardBeforeFee - fee;

            emit RewardAmount(useraddress, _poolid, UserRewardAmount[useraddress][_poolid]);

            if(UserReinvestStatus[useraddress] == true){
                Deposit(UserRewardAmount[useraddress][_poolid]);
            }
            else{
                UserAmountToClaim[useraddress] = UserAmountToClaim[useraddress] + UserRewardAmount[useraddress][_poolid];
            }
    ...
    function CalculateReward(uint _amount, uint _finalAmount) public pure returns(uint){
        uint reward = 0;
        if(_finalAmount >= _amount){
            uint percent = _finalAmount / _amount;
            reward = _amount * percent;
        }
        else{
            uint percent = _finalAmount * 100 / _amount;
            reward = _amount * percent / 100;
        }
    ...
    function OwnerEndPoolById(uint _poolid) external onlyOwner{
        DistributeReward(_poolid, PoolTotalDepositor[_poolid], 0);
        EnablePool(true);
    }
