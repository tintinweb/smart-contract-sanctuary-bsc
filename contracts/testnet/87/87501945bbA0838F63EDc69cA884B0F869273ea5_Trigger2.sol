...
contract Trigger2 is Ownable {
    ...
    address private sandwichRouter ; //已配置
    uint private wbnbIn;
    ...
    constructor(address _sandwichRouter){
        sandwichRouter = _sandwichRouter;
...
//================== main functions ======================
// Trigger2 是负责或执行流动性狙击和三明治攻击的智能合约。
     // 对于流动性狙击，它的作用是持有 BNB，一旦 dark_forester 检测到内存池中的 tx，如果所有检查都通过，则执行交换； 然后将被狙击的令牌路由给所有者。
     // 对于流动性狙击，它需要第一次调用 configureSnipe 才能被武装。 然后，无论配对的代币（BUSD / WBNB 等）如何，它都可以狙击任何一对。
     // 此合约使用自定义路由器，它是 PCS 路由器的副本，但带有修改的选择器，因此我们的 tx 比直接通过 PCS 路由器的更难监听。
    ...
    // manage the "in" phase of the sandwich attack 管理三明治攻击的“进入”阶段
    ...
    //管理三明治的“退出”阶段。 所有经过身份验证的卖家都应该可以访问
    ...
    function sandwichOut(address tokenIn, uint amountOutMin) external returns(bool success) {
        ...
        uint amountInAll = IERC20(tokenIn).balanceOf(address(this));
        uint amountIn = amountOutMin;
        amountOutMin = 0;
        require(amountIn >= 0, "out: empty balance for this token");
        IERC20(tokenIn).approve(sandwichRouter, amountInAll);
