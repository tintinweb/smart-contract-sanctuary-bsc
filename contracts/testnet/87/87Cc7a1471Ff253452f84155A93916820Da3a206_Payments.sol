...
contract Payments is Ownable {
    ...
    constructor() {
        ...
        // usdt = IERC20 (address(0x0BBb231D2CA20020D5a7db92Eb343cf5cF5F1949)); //USDT in local

        //Test only
        // employeesArray.push(Employees(123123213, 2000000, payable(0x075D29D70FF3d5AD1a2569bba6F581CBf2be7Cee), Status.Enable));
        // employeesArray.push(Employees(567567564, 1000000, payable(0xE82b3de5C6cBbF102a168126C730C98952a7E37f), Status.Enable));
        // employeesArray.push(Employees(836554912, 1000000, payable(0x62C9511E06b0Aca785e69B5f81c29DF4AfB4F71B), Status.Enable));
    ...
    function getAllEmployees() onlyOwner public returns (uint[] memory, uint[] memory, uint[] memory) {
        delete indexLocal;
        delete id;
        delete amountLocal;
        uint arrayLength = employeesArray.length;
    ...
    function salaryPayments(uint[] memory _indexes) onlyOwner public payable {
        uint arrayLength = _indexes.length;
        for (uint i=0; i<arrayLength; i++) {
            salaryPay(_indexes[i]);
            i+1;
        }
    }
