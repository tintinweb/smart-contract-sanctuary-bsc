...
contract Miner is Context, IERC20, Ownable {
    ...
    function setWhitelistMultipleAddress(address[] memory account, bool newValue) public onlyOwner {
        for (uint256 i = 0; i < account.length; i++) {
            address wallet = account[i];
            isWhitelisted[wallet] = newValue;
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (inSwapAndLiquify) {
            ...
            if (recipient == address(0) || recipient == address(0xdead)) _totalSupply = _totalSupply.sub(finalAmount);
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient == address(0) || recipient == address(0xdead)) _totalSupply = _totalSupply.sub(amount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if (amountBNBTeam > 0) payable(treasuryAddress).transfer(amountBNBTeam);
    ...
    function withdrawForeignToken(address tokenAddress, uint256 amount) public onlyOwner {
        IERC20 foreignToken = IERC20(tokenAddress);
        foreignToken.transfer(owner(), amount);
    }
}