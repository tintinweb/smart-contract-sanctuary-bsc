...
contract SeedDistribution is Ownable {
    ...
    event ClaimToken(address seeder, uint256 amount);
    event ClaimTokenEmergency(address seeder, uint256 amount);
    ...
    function setEmergencyStatus(bool status) public {
    ...
    function setReleaseTime(uint256 time) public onlyOwner onlyEmergency {
    ...
    function claimToken(uint256 amount) public onlyInReleaseTime {
        require(amount > 0, "invalid paymentAmount");
        uint256 amountTokenCanClaim = getBalancers();
        uint256 finalClaim = amountTokenCanClaim;

        if (amount < amountTokenCanClaim) {
            finalClaim = amount;
        }

        moonRaiseToken.safeTransfer(msg.sender, finalClaim);
        seeders[msg.sender].hadWithdraw = seeders[msg.sender].hadWithdraw.add(
            finalClaim
        );
        emit ClaimToken(msg.sender, finalClaim);
    }
