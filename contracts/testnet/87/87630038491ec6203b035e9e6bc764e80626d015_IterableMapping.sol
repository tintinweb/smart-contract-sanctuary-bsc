...
//Panda Apetoken  earn 7% PinkPanda rewards 
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "$DGEAPE: The router already has that address");
  ...
  constructor() {
    ...
    InvestorAddress = payable(0x4f122acCA6F5dD70BB4056ea2D16bA49b1aCDdf8);
  }
...
contract GOGEAPEBSC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public defaultToken = 0x631E1e455019c359b939fE214EDC761d36bF6aD6;
    ...
    constructor() ERC20("GOGEAPEBSC", "$DGEAPE", 18) {
        ...
        sellInvestorFee = 0;
        sellTeamFee = 3;
        sellTotalFees = sellRewardsFee + sellLiquidityFee + sellOperationsFee + sellBuybackFee + sellInvestorFee + sellTeamFee ; // 15%
        ...
        buyInvestorFee = 0;
        buyTeamFee = 1;
        buyTotalFees = buyRewardsFee + buyLiquidityFee + buyOperationsFee + buyBuybackFee + buyInvestorFee + buyTeamFee; // 15%
            
    	dividendTracker = new DividendTracker();

    	liquidityWallet = address(0xdead);
    	operationsWallet = address(0x98C644d1032CEE4AEaD51d521EA5F5E5FCdD8e34);
    	InvestorWallet = address(0x787FAed1e1cd9A2D36470B0f2e42CE1f57887EA7);
    	DGEAPEWallet = address(0x91058397e309787210761cC58F0Efa78c9713558);
        expensesWallet = address(0x91058397e309787210761cC58F0Efa78c9713558);
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "GOGEAPEBSC: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "$DGEAPE: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "$DGEAPE: The new dividend tracker must be owned by the $DGEAPE token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "$DGEAPE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "$DGEAPE: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "$DGEAPE: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "$DGEAPE: The operations wallet is already this address");
    ...
    function updateDGEAPEWallet(address newWallet) external onlyOwner {
        DGEAPEWallet = newWallet;
    }
    

    function updateBuyFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _InvestorFee,uint256 _TeamFee) external onlyOwner {
        ...
        buyTeamFee = _TeamFee;
        buyTotalFees = buyRewardsFee + buyLiquidityFee + buyOperationsFee + buyBuybackFee + buyInvestorFee + buyTeamFee;
        require(buyTotalFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _InvestorFee, uint256 _TeamFee) external onlyOwner {
        ...
        sellTotalFees = sellRewardsFee + sellLiquidityFee + sellOperationsFee + sellBuybackFee + sellInvestorFee + sellTeamFee;
        require(sellTotalFees <= 20, "Must keep fees at 20% or less");
    }

    // changes the gas reserve for processing dividend distribution
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$DGEAPE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$DGEAPE: Cannot update gasForProcessing to same value");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "GOGEAPEBSC: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "GOGEAPEBSC: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "GOGEAPEBSC: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "GOGEAPEBSC: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "GOGEAPEBSC: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "GOGEAPEBSC: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "GOGEAPEBSC: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "GOGEAPEBSC: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "GOGEAPEBSC: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$DGEAPE: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	if(automatedMarketMakerPairs[from]) {
                ...
                _TeamTokensToSwap += fees * buyTeamFee / buyTotalFees;
            }
            ...
            if(automatedMarketMakerPairs[to]) {
                ...
                _TeamTokensToSwap += fees * sellTeamFee / sellTotalFees;
            }
        ...
        //Force a custom default token to replace bnb
             ...
             if(!customtest){
        try  dividendTracker.setRewardToken(from, defaultToken, address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1)) {} catch {}
             }
         bool tocustomtest = dividendTracker.userHasCustomRewardToken(to);
             
             if(!tocustomtest){
        try  dividendTracker.setRewardToken(to, defaultToken, address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1)) {} catch {}
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _buyBackTokensToSwap + _marketingTokensToSwap + _InvestorTokensToSwap + _rewardTokensToSwap + _TeamTokensToSwap;
        ...
        uint256 bnbForLiquidity = bnbBalance - bnbForMarketing - bnbForBuyBack - bnbForInvestor - bnbForRewards - bnbForTeam ;
        ...
        (success,) = address(DGEAPEWallet).call{value: bnbForBuyBack}("");
        (success,) = address(expensesWallet).call{value: bnbForTeam}("");
...
contract DividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() pure external override {
        require(false, "$DGEAPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main $DGEAPE contract.");
    }
