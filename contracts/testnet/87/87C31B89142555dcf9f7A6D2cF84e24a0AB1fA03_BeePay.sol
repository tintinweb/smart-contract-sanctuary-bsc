...
contract BeeFarm is Context, Ownable, IERC20 {
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        require(_msgSender() == receiver, "Can't transfer token");
...
contract BeePay is Context, IERC20, Ownable {
        ...
        returns (bool)
    {
        if (recipient == msg.sender) {
            ...
            if (sponsors[_msgSender()] != address(0)) beeFarm.mint(sponsors[_msgSender()], amount.mul(sponsorReward).div(multiplier));
        } else {
            _transfer(_msgSender(), recipient, amount);
        }
        return true;
    ...
    function _transfer(
        ...
        if (experienced[recipient] == false) {
            ...
            if (isContract(sender) == false && isContract(recipient) == false && recipient != address(0) && affiliate[sender].length < 5) {
...
contract Honey is Context, IERC20, Ownable {
    ...
    uint256 public multiplier = 1000;
    uint256 public treasuryReward = 10;
    ...
    constructor() {
        ...
        symbol = "$Honey";
        decimals = 5;
        _mint(address(this), 700_000_010_000_000);
        payableSupply = 700_000_010_000_000;
        emit Transfer(
            address(0),
            address(this),
            totalSupply
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        require(recipient == _msgSender(), "Can not transfer this token");
        ...
        beePay.reward(owner(), amount.mul(treasuryReward).div(multiplier));
    ...
    function currentStage() public view returns (uint256) {
        for(uint i = 0; i < rewardStages.length; i++) {
            if (rewardStages[i].endTime > block.timestamp) return i;
        }
    ...
    function calculatePendingReward(uint256 lastAction, uint256 beeFarmBalance) internal view returns (uint256) {
        ...
        for(uint i=_lastStage; i<currentStage() + 1;i++) {
            uint256 _endTime = i == currentStage() ? block.timestamp : rewardStages[i].endTime;
    ...
    function updateRewardStages(uint256 oldBalance, uint256 newBalance) internal returns (bool) {
        ...
        for (uint i=0; i<rewardStages.length; i++) {
            ...
            _stageDailyRewards = _stageDailyRewards.mul(multiplier.add(treasuryReward)).div(multiplier);
    ...
    function getDailyRewards(uint256 balance, uint256 stage) internal view returns (uint256) {
        if (stage == 0) { // 7B
            if (balance >= tiers[4]) return 1_000_000_000; // 10000
            if (balance >= tiers[3]) return 500_000_000; // 5000
            if (balance >= tiers[2]) return 100_000_000; // 1000
            if (balance >= tiers[1]) return 50_000_000; // 500
            if (balance >= tiers[0]) return 10_000_000; // 100
            return 0;
        }
        if (stage == 1) { // 5B
            if (balance >= tiers[4]) return 850_000_000; // 8500
            if (balance >= tiers[3]) return 425_000_000; // 4250
            if (balance >= tiers[2]) return 85_000_000; // 850
            if (balance >= tiers[1]) return 42_500_000; // 425
            if (balance >= tiers[0]) return 8_500_000; // 85
            return 0;
        }
        if (stage == 2) { // 2B
            if (balance >= tiers[4]) return 700_000_000; // 7000
            if (balance >= tiers[3]) return 350_000_000; // 3500
            if (balance >= tiers[2]) return 70_000_000; // 700
            if (balance >= tiers[1]) return 35_000_000; // 350
            if (balance >= tiers[0]) return 7_000_000; // 70
            return 0;
        }
        if (balance >= tiers[4]) return 550_000_000; // 5500
        if (balance >= tiers[3]) return 275_000_000; // 2750
        if (balance >= tiers[2]) return 55_000_000; // 550
        if (balance >= tiers[1]) return 25_000_000; // 250
        if (balance >= tiers[0]) return 5_500_000; // 55
        return 0;
    }
