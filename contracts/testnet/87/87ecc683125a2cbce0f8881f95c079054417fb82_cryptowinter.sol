...
Tokenomics:
7% Buy Tax
2% Reflection to Holders (TOWIN) | 3% Marketing | 2% Liquidity pool

8% Sell Tax
2% Reflection to Holders (TOWIN) | 4% Marketing & Development | 2% Liquidity pool
*/
...
contract cryptowinter is Context, IERC20, Ownable {
  ...
  string private _name = 'Crypto Winter';
  string private _symbol = 'TOWIN';
 ...
 uint256 public reflectFeeSell = 2;
  ...
  address private _marketingWalletAddress = 0x9Bf482A992BBD7C4288a0accFAe22418b2cDe653; 
  address public _buybackWallet = 0x9Bf482A992BBD7C4288a0accFAe22418b2cDe653;
  ...
  address public immutable TOWINUniswapV2Pair;
  ...
  constructor() {
      address _newOwner = 0xdd3d13993dc9fA1257F4Fec43F345c8e1FB92418; // TOWIN Owner
    ...
    TOWINUniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
 ...
 function setSellFees() private {
  ...
  function _transfer(
    ...
    if ((!launched() && to == TOWINUniswapV2Pair) || manualLaunch) {
      require(
        balanceOf(from) > 0,
    ...
    if (antiwhaleEnabled && ((from != owner() && to != owner()) || !(_isExcludedFromTxLimit[from]))) {
      require(
        amount <= _maxTxAmount,
        'TOWIN: Transfer amount exceeds the maxTxAmount.'
      );
    }
    if(!_isExcludedFromMaxWalletLimit[from] && !_isExcludedFromMaxWalletLimit[to] && to != TOWINUniswapV2Pair) {
  ...
  function _tokenTransfer(
    ...
    if(takeFee && sender == TOWINUniswapV2Pair) {
        // buy
        setBuyFees();
    } else if(takeFee && recipient == TOWINUniswapV2Pair) {
        // sell
        setSellFees();
    }
    ...
    if(takeFee && (sender == TOWINUniswapV2Pair || recipient == TOWINUniswapV2Pair)) {
        restoreAllFee();
    }
