...
// File: contracts/FFSFrontFeeSwapp/SafeswapRouter.sol
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex'1162d5a5b4c2a2284259640b704ba2f9960f37c8c02f086c7f601621a22078ba' // init code hash
                    )
                )
            )
        );
...
interface SFMFrontFeeSwap {
    function takeFFSFees(uint256[] calldata amounts, address[] calldata path) external payable returns (uint256);
}
...
contract SafeswapRouter is ISafeswapRouter02 {
    ...
    function swapExactTokensForTokens(
        ...
        // SFMFrontFeeSwap(ffs).takeFFSFees{value: msg.value}(amounts, path);
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        uint256 ffsFees = SFMFrontFeeSwap(ffs).takeFFSFees{value: msg.value}(amounts, path);
        amountOut = amountOut - ffsFees;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        // uint256[] memory amounts = SafeswapLibrary.getAmountsIn(factory, amountIn, path);
        // SFMFrontFeeSwap(ffs).takeFFSFees{value: msg.value}(amounts, path);
    }
