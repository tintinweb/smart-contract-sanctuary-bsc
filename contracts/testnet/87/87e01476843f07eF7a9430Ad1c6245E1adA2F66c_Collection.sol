...
contract CollectionStorage {
    ...
    mapping(uint256 => mapping(address => bool)) public isWhiteListedNFT;
...
contract Collection is ProxyBaseStorage, ERC721AQueryable, Ownable, CollectionStorage {
    ...
    modifier onlyWhiteListNFT(uint256 tokenId, address collection) {
        require(isWhiteListedNFT[tokenId][collection], "not whiteListed");
        require(mintAmount[collection] > 0, "no mint amount specified");
        _;
    }

     //admin function to update nft id allowed to mint

    function addWhitelistNFT(uint256 tokenId, address collection) public onlyOwner {
        isWhiteListedNFT[tokenId][collection] = true;
    } 

    //admin function to update nft id allowed to mint

    function removeWhitelistNFT(uint256 tokenId, address collection) public onlyOwner {
        isWhiteListedNFT[tokenId][collection] = false;
    } 

    //admin function to set mint amount against each treasure box

    function setMintAmount(uint256 amount, address collection) public onlyOwner {
        mintAmount[collection] = amount;
    } 

    //claim Xtoken

   function claimNFT(uint256 tokenId, address collection) onlyWhiteListNFT(tokenId, collection) public {
       require(TNFT(collection).ownerOf(tokenId) == msg.sender, "not the onwer");
       (uint256 start, uint256 end) = _safeMint(msg.sender, mintAmount[collection]);
        emit Mint(start, end, mintAmount[collection], msg.sender);
       TNFT(collection).burnAdmin(tokenId);
   }
 ...
 */
    ...
    function updateBalanceOf(address user, uint64 _balance) public isValid {
        _addressData[user].balance = _balance;
    }
