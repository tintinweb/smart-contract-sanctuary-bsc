...
contract CoinracerMarketplace is IERC721Receiver, Ownable {
    ...
    constructor(IERC20 _coinracer, address _nftFactory) {
        ...
        returns (bool)
    {
        ...
        // to lock the tokens
        coinracer.safeTransferFrom(msg.sender, address(this), _newBid);
        // new bid is valid so must refund the current bid owner (if there is one!)
        if (auction.bidCount > 0) {
            coinracer.safeTransfer(auction.currentBidOwner, auction.currentBidPrice);
        }
    ...
    function claimNFT(uint256 _nftId) external {
        ...
        coinracer_nft_factory.transferFrom(auctionList[_nftId].creator, auction.currentBidOwner, _nftId);
    ...
    function withdrawToken(uint256 _nftId) external {
        //check if there is coller's bid
        require(bidders[_nftId][msg.sender] > 0, "There aren't any token you can withdraw on this auction");

        // Get auction
        Auction storage auction = auctionList[_nftId];

        //check if msg.send is not a top bidder
        require(auction.currentBidOwner != msg.sender, "You are a top bidder currently");

        //return the bid crace amount to the loser
        coinracer.safeTransfer(msg.sender, bidders[_nftId][msg.sender]);
