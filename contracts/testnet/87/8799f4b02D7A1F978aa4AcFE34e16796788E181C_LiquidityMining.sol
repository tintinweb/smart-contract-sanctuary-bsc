...
contract LiquidityMining is Ownable{
    ...
    uint256 constant BZAIPerBlock = 317 * 1E16; // 3,17 BZAI per block => 50M initial reward / 365 / 43200(block per day in polygon mainnet);
    ...
    function deposit(uint256 amount) external {
        ...
        if (amount > 0) {
            uint256 balanceBefore = liquidityMining.lpToken.balanceOf(address(this));
            liquidityMining.lpToken.safeTransferFrom(
                address(msg.sender),
                address(this),
                amount
            );
            uint256 balanceAfter = liquidityMining.lpToken.balanceOf(address(this));
