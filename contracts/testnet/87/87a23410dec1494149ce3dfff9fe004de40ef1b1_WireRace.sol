...
{   
    ...
    function createBet(uint256 tokenId) external 
    {
    ...
    function getResult()  external
    {
       ...
       for(uint i=1; i<=3 ; i++ )
       {
          uint256 card_No = (uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp+a, card_probability.length))))%card_probability.length;          
          winner[i] = card_No;
          a = a+1002*50;
       }
    ...
    function withdrawbep20(IBEP20 token ,uint256 _amount ) public onlyOwner{
        token.transfer(msg.sender,_amount);
    }

    function enableForRace() public{
        uint id;
