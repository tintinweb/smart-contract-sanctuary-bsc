...
contract Project is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function closeProject(uint256 _projectId, uint256[] memory saleIds, bool _isGive) external nonReentrant onlyManager(_projectId) {
        uint256 closeCount;
        uint256 _closeLimit = totalBuyersWaitingToTokens[_projectId] > closeLimit ? closeLimit : totalBuyersWaitingToTokens[_projectId];
        ...
        for (uint256 i; i < saleIds.length; i++) {
            ...
            //TODO
            if (closeCount == _closeLimit) continue;
            closeCount = sale.close(_closeLimit, _project, saleInfo, closeCount, _isGive);
        }
        setTotalBuyersWaitingToToken(_projectId, totalBuyersWaitingToTokens[_projectId] - closeCount);
        if (totalBuyersWaitingToTokens[_projectId] == 0) projects[_projectId].status = ProjectStatus.ENDED;
    ...
    //TODO
    function setTotalBuyersWaitingToToken(uint256 _projectId, uint256 _total) public projectExists(_projectId) {
        totalBuyersWaitingToTokens[_projectId] = _total;
    }

    //TODO
    function getTotalBuyersWaitingToToken(uint256 _projectId) public view returns (uint256) {
