...
contract GoldToken is Initializable, ContextUpgradeable, AccessControlEnumerableUpgradeable, ERC20BurnableUpgradeable, ERC20PausableUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address _nftToken, string memory name, string memory symbol) public virtual initializer {
        __ERC20PresetMinterPauser_init(name, symbol);
        __Ownable_init();
        nftToken = GoldNFT(_nftToken);
    }
    ...
    function airdrop(uint quantity) external payable {
        require(quantity > 0,'Quantity > 0');
        for(uint i=0; i<quantity; i++){
            nftToken.createItem(msg.sender, tokenURI);
        }
