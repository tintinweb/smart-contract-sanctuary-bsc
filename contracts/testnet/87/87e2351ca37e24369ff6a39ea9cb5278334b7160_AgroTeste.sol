...
/**
    ...
    Contrato customizado, otimizado e com função adicionais
    Biblioteca safemath @OpenZeppelin adicionada e o contrato principal com herança devidamente herdada para uint256.
...
abstract contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        ...
        require(newOwner != address(0x000000000000000000000000000000000000dEaD), "Ownable: new owner is the dead address");
...
contract AgroTeste is Authorized, ERC20 {
  ...
  string constant _name = "AgroTest-BSC";
  string constant _symbol = "AgroFtestttt";

  // Token supply control
  uint8 constant decimal = 9;
  uint8 constant decimalBUSD = 9;  
  uint256 constant maxSupply = 1000000 * (10 ** decimal);
  ...
  // Fees
  uint256 public feeMarketingWallet = 500; // 8%

  uint256 public feeContractAddress = 500; // 4%
  ...
  function getFeeTotal() public view returns(uint256) { return feeContractAddress + feeMarketingWallet; }
  ...
  function setFees(uint256 pool) external isAuthorized(1) {
    feeContractAddress = pool;
  }

  function setFeesDirectWallet(uint256 Marketing) external isAuthorized(1) {
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    MarketingWallet = 0x09d7De0a6157dDb065D8eEa9504D8DEd2A7e118a;
  ...
  function exchangeFeeParts(uint256 incomingFeeTokenAmount) private returns (bool){
    ...
    // Deep Stack problem avoid
    {
      ...
      if (feeMarketingWallet > 0) tokenTransfer(busdAddress, MarketingWallet, (busdAmount * feeMarketingWallet) / getFeeTotal());
    }
    return true;
  }
