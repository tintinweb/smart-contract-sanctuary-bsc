...
contract CryptoBall {
    ...
    function payoutPlayers() public payable {
        ...
        for(i = 0; i < n - 1; i++){
            ...
            for(j = 0; j < n-i-1; j++){
                if(prizePools[prizePoolIndex].playerList[j].highestScore > prizePools[prizePoolIndex].playerList[j + 1].highestScore){
                    temp = prizePools[prizePoolIndex].playerList[j].highestScore;
                    prizePools[prizePoolIndex].playerList[j].highestScore = prizePools[prizePoolIndex].playerList[j + 1].highestScore;
                    prizePools[prizePoolIndex].playerList[j + 1].highestScore = temp;
                    swapped = true;
                }
            }
        ...
        for(uint k = 0; k < n; k++){
            players[prizePools[prizePoolIndex].playerList[k].walletAddress].highestScore = 0;
        }
