...
contract NAMETOKEN is Context, IERC20, Ownable {
    ...
    address private _devWalletAddress;     // Team wallet here
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee, uint256 _lpFee, uint256 _DexFee, address routerAddress, address feeaddress, address tokenOwner) 
    
     {
        ...
        _maxWalletToken = (_tTotal * 5 / 1000) * 10 ** _decimals; // Max Wallet: 1000000 * 10**3 * 10**18; 
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() || to != owner() || to != address(uniswapV2Pair) ) {
                uint256 contractBalanceTo = balanceOf(to);
                require(
                contractBalanceTo + amount <= _maxWalletToken, "Exceeds maximum wallet token amount."
            );
            }
