...
import "./ERC1155LazyMinimalV3.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract MiranaERC1155V3 is Initializable, ERC1155LazyMinimalV3 {
  ...
  function initialize(
    ...
    address royaltiesResgister
  ) public initializer {
    __Ownable_init();
    __ERC1155_init(baseTokenURI);
    __ERC1155Burnable_init();
    __Mint1155Validator_init_unchained();
    __ERC1155Lazy_init(royaltiesResgister);
...
abstract contract ERC1155LazyMinimalV3 is
  IERC1155LazyMint,
  ...
  RoyaltiesV2Impl
{
  ...
  address public addRoy;

  event DefaultApproval(address indexed operator, bool hasApproval);

  function __ERC1155Lazy_init(address _addRoy) internal onlyInitializing {
    __ERC1155Lazy_init_unchained(_addRoy);
  }

  function __ERC1155Lazy_init_unchained(address _addRoy)
    internal
    onlyInitializing
  {
    addRoy = _addRoy;
  ...
  function setDefaultApproval(address operator, bool hasApproval) public {
  ...
  function mintAndTransfer(
    ...
    if (supply[data.tokenId] == 0) {
      ...
      IRoyaltiesProvider(addRoy).setRoyaltiesCacheByTokenAndTokenId(
        address(this),
        data.tokenId,
        data.royalties
      );
  ...
  function mint(
    ...
    if (tokenId != 0) {
      id = tokenId;
      if (balanceOf(recipient, id) == 0) {
        _listHolder[id].push(recipient);
      }
    } else {
    ...
    emit Transfer(_msgSender(), address(0), recipient, id, amount);
  }
  ...
  function mintBatch(
    ...
    uint256[] memory ids;
    uint96 counter;
    for (uint96 index = 0; index < tokenIds.length; index++) {
      if (tokenIds[index] != 0) {
          ids[counter++] = tokenIds[index];
        if (balanceOf(recipient, tokenIds[index]) == 0) {
          _listHolder[tokenIds[index]].push(recipient);
        }
      } else {
        _tokenIdCounter.increment();
        ids[counter++] = _tokenIdCounter.current();
    ...
    emit TransferBatch(_msgSender(), address(0), recipient, tokenIds, amounts);
  }
  ...
  function setRoyaltiesRegister(address _addRoy) public onlyOwner {
    addRoy = _addRoy;
  }
...
abstract contract ERC1271Validator is EIP712Upgradeable {
    ...
    function getSigner(LibERC721LazyMint.Mint721Data memory data, bytes memory signature) public view returns (address signerFromSig) {
