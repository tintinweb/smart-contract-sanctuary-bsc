...
contract MonsterToken is ERC20, Ownable {
    ...
    function setAntibotModeWhitelist(address[] memory toAddAddresses, address[] memory toRemoveAddresses) public onlyOwner {
        for (uint256 i = 0; i < toAddAddresses.length; i++) antibotModeWhitelist[toAddAddresses[i]] = true;
        for (uint256 i = 0; i < toRemoveAddresses.length; i++) antibotModeWhitelist[toRemoveAddresses[i]] = false;
    }
