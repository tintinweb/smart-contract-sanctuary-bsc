...
contract SpoilsOfWar is Context, Ownable, IBEP20 {
    ...
    constructor() {	    
        ...
        tarifs[90] = Tarif(90, 2400); //26.66% daily for 90 days => 2400% ROI
        tarifs[60] = Tarif(60, 2000); //33.33% daily for 60 days => 2000% ROI
        tarifs[30] = Tarif(30, 1800); //60.00% daily for 30 days => 1800% ROI
        tarifs[20] = Tarif(20, 1500); //75.00% daily for 20 days => 1500% ROI
        tarifs[15] = Tarif(15, 1200); //80.00% daily for 15 days => 1200% ROI
        tarifs[11] = Tarif(11, 1100); //100.00% daily for 11 days => 1100% ROI
    ...
    function SignUp(address referral, uint8 clan, uint8 quickinvest) public payable {
        ...
        if(quickinvest > 0)
        {
            investToken(msg.sender, msg.value, tokens,90);
        }else{
            // hold the tokens
            transferTokens(address(this), msg.sender, tokens);
        }
    ...
    function Reinvest() external {
        ...
        investToken(msg.sender, bnb, player.dividends,90);
    ...
    function memberInfo(address _addr) view external returns(uint256 for_withdraw, uint256 numDeposits, uint256[6] memory investments, uint256[5] memory structure) {       
        Player storage player = players[_addr];
