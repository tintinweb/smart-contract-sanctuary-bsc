...
contract IToken is BEP20 {
    ...
    **/
    ...
    constructor(
        ...
        _decimals = (_asset == BNB_ADDRESS) ? 8 : IBEP20Metadata(_asset).decimals();
    }
    ...
    **/
    function _transfer(address _from, address _to, uint256 _amount) internal override {
        ...
        require(_amount <= vault.getUserAssetBalance(assetAddress, _from), "Amount more than the user deposit of asset");

        masterChef.duringITokenTransfer(assetAddress, _from, _amount);

        super._transfer(_from, _to, vault.storedAmount(assetAddress, _amount));
    }
    ...
    **/
    function mint(address _user, uint256 _amount) external onlyMasterChefContract {
        _mint(_user, vault.storedAmount(assetAddress, _amount));
    ...
    **/
    function burn(address _user, uint256 _amount) external onlyMasterChefContract {
        _burn(_user, vault.storedAmount(assetAddress, _amount));
    ...
    **/
    function balanceOf(address _user) override public view returns (uint256) {
        return vault.cumulatedAmount(assetAddress, super.balanceOf(_user));
    }
    ...
    **/
    function totalSupply() override public view returns (uint256) {
        return vault.cumulatedAmount(assetAddress, super.totalSupply());
    }
