...
contract MarketplaceV3 is Ownable, IERC721Receiver {
  /* ========== LIBs ========== */
  ...
  mapping(uint256 => NftSold) public sellingTokens;
  ...
  event CancelSelling(uint256 tokenId);
  ...
  function selling(
    ...
    require(!sellingTokens[_tokenId].isExist, 'the token is selling');
    require(
      paymentTokens[_paymentTokenAddress] == true,
    ...
    sellingTokens[_tokenId] = NftSold({
      isExist: true,
      owner: _msgSender(),
   ...
   */
  function cancelSelling(uint256 _tokenId) public {
    require(
      sellingTokens[_tokenId].isExist,
      'the token is sold or already bought'
    );
    // check current ownership
    NftSold memory nftSold = sellingTokens[_tokenId];
    ...
    delete sellingTokens[_tokenId];

    emit CancelSelling(_tokenId);
  }
  ...
  function buy(uint256 _tokenId, uint256 _price) external returns (bool) {
    require(sellingTokens[_tokenId].isExist, 'the token is not sold');
    NftSold memory nftSold = sellingTokens[_tokenId];
    require(nftSold.price == _price, 'the price has been changed');
