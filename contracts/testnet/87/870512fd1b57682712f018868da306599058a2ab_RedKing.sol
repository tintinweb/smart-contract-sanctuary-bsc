...
contract RedKing is ERC20, Ownable {
    ...
    //购买数量
    uint256 public _maxTxAmount =4 * 10**uint256(decimals()); 
    uint256 public _walletMax = 5 * 10**uint256(decimals());
    ...
    function swapManual() public onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));//查询当前账户
    ...
    function _transfer(
        ...
        //判断单次购买数量是否超过设置的最大持有量
        if(!_isExcludedFromFees[to] && !_isExcludedFromFees[from]){
            require(amount <= _maxTxAmount,"more than the maxTxAmount");
        }
        ...
        uint256 contractTokenBalance = balanceOf(address(this));//账户金额 

        bool canSwap = contractTokenBalance >= swapTokensAtAmount;//达到分红比例看是分红，默认是0.002%
        ...
        if( canSwap &&
            ...
            if(AmountMarketingFee > 0) swapAndSendToFee(AmountMarketingFee);//市场营销费用 
            if(AmountLiquidityFee > 0) swapAndLiquify(AmountLiquidityFee);//金额流动性费用
            if(AmountTokenRewardsFee > 0) swapAndSendDividends(AmountTokenRewardsFee);
            swapping = false;
        }
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                fees = LFee.add(RFee).add(MFee).add(DFee);//合计金额
            }
            ...
            //设置单个地址最大持有数量
            require(balanceOf(to).add(amount) <= _walletMax,"more than the walletMax");
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        uint256 initialCAKEBalance = IERC20(rewardToken).balanceOf(address(this));//查询当前钱包地址中Amount of reward tokens
