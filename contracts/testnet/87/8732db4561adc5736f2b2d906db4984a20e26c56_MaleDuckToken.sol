...
contract  MaleDuckToken is IERC20, Ownable {
    ...
    uint256 public _limitTimeMaxTx = 12 hours;
    
    uint256 public _limitAddLiuTime = 2 minutes;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        if( !_isExcludedFromFee[from] &&!_isExcludedFromFee[to]){
            ...
            if((_startTimeForSwap+_limitAddLiuTime)>block.timestamp  && from==uniswapV2Pair){
                revert("Swap not opened");
            }
            if (_isBot[from] ) {
   ...
   function _takeburnFee(
        ...
        if (_burnFee == 0) return;
         if((_tTotal.sub(_tOwned[address(0)]) ) >= burnEndNumber){
    ...
    function dividendMarketAddr(uint256 dividendAmount) private {
     ...
     function testProcess(uint256 gas) public onlyOwner{
       process(gas);
    }
