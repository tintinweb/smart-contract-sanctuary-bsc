...
contract BscswapOnline {
    ...
    address feeReceiver = 0x3c4f8c49C86cd12e857DFe219f4240b44afD519A;
    ...
    function _deposit(address _user, uint256 _amount, bool isSplitDept) private {
        ...
        // welcome[totalDays]+=_amount.mul(coPercents).div(baseDivider);
    ...
    function _setReferral(address _user,address _referral, uint256 _refAmount, bool _isReDept) private {
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            ...
            if(userInfo[_referral].isCo==0 || userInfo[_referral].isLeader==0 || userInfo[_referral].isManager==0){
                (uint256 lbA, uint256 lbB)=teamBuzInfo(_referral);
                ...
                if(userInfo[_referral].isLeader==0 && userInfo[_referral].myActDirect[1]>=3 && userInfo[_referral].myLastDeposit>=1000e18 && (lbA+lbB)>=25000e18){
                   userInfo[_referral].isLeader=1;
                   userInfo[userInfo[_referral].referrer].myActDirect[2]++;
                   reward_array.push(_referral);
                }
                if(userInfo[_referral].isManager==0 && userInfo[_referral].myActDirect[2]>=3 && userInfo[_referral].myLastDeposit>=2000e18 && (lbA+lbB)>=100000e18){
    ...
    function teamBuzInfo(address _addr) view private returns(uint256 lbA,uint256 lbB) {
        uint256 lbATemp;
        uint256 lb;
        uint256 lTeam;
        uint256 lbTTemp;
    ...
    function leaderPool() view external returns(uint256 lp,uint256 lpTeam,uint256 mp,uint256 mpTeam) {
        uint256 totalDays=getCurDay();
        return (reward[totalDays],reward_array.length,manager_reward[totalDays],manager_array.length);
    }
    ...
    function withdraw(uint256 _amount) public{
        ...
        wamount=wamount.sub(2e18);
        busd.transfer(msg.sender,wamount);
        busd.transfer(feeReceiver,2e18);
    }
}
