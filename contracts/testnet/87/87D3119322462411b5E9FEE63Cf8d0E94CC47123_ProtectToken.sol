...
contract ProtectToken is IProtectToken, Ownable {
    ...
    uint256 public maxNumberTransferToday = 10;
    uint256 public maxNumberTransferTodayForPartner = 2;
    ...
    function checkTransferToken(
        ...
        require(amount <= maxTokenTransfer, "The maximum amount of the transfer has been exceeded");
        if(partner && amount > maxTokenTransfer){
            // require(amount <= maxTokenTransfer, "Partner's maximum transfer amount has been exceeded");
            return false;
        }
        ...
        if(checkToday(timeTransfer) == false){
            numberTransfer = 0;
            numberTransferTodayForPartner = 0;
        }
        require(numberTransfer < maxNumberTransferToday, "The number of transactions exceeds the limit for the day");
        if(partner && numberTransferTodayForPartner >= maxNumberTransferTodayForPartner){
            return false;
        }
        return true;
    }
    ...
    function changeMaxTokenTransfer(uint256 amount) public onlyOwner {
        maxTokenTransfer = amount.mul(1e18);
    }

    function changeMaxTokenTransferPartner(uint256 amount) public onlyOwner {
        maxTokenTransferForPartner = amount.mul(1e18);
    }

    function changeSellAddress(address sell, bool _active) public onlyOwner {
        sellAdress[sell] = _active;
    }
    ...
    function checkToday(uint256 _time) public override view returns (bool inDate) {
