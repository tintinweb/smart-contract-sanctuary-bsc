...
contract Stake is Ownable {
    ...
    function viewTotalROI(address _staker) public view returns(uint256 roi){
        roi = 0;
        for(uint256 i = 0; i < userDetails[_staker].stakingIds.length; i++){
            uint256 stakeId = userDetails[_staker].stakingIds[i];
            uint256 poolId  = stakeDetails[stakeId].poolId;
            roi += ((block.timestamp - stakeDetails[stakeId].stakeTime) * stakeDetails[stakeId].amount * pool[poolId].rewardPer / 100) / 1 days;
        }
        return roi;
    }
