...
contract GrapeFootball {
    
    uint256 private constant ONE_HUNDRED = 100000000000000000000;
    ...
    uint256 public percentOfBetForCommunityFee;
    ...
    // Fixture => Player => ClaimedAmount
    mapping(uint => mapping(address => uint256)) private fixturePlayerClaimedAmount;

    // Fixture => Player => ClaimedToken
    mapping(uint => mapping(address => address)) private fixturePlayerClaimedToken;

    // Player => accumulated amount
    mapping(address => uint256) private playerAccumulated;
    ...
    struct ScoreFunctions {
        uint256 bet_betUsingNetworkCoin;
        uint256 winnerClaim;
        uint256 loserClaim;
    ...
    // Player counters
    mapping(address => ScoreFunctions) private playerCounters;

    // Points table
    ScoreFunctions private pointsTable; 


    //Events
    event OnBet(address player, uint fixture, uint betPosition, address tokenBet, uint256 amountTokenInWei, uint256 betAmount);
    event OnWinnerClaim(address player, uint fixture);
    event OnLoserClaim(address player, uint fixture);
    ...
    constructor() 
    {
        ...
        percentOfBetForCommunityFee = 500000000000000000; // 0.5% by default
        // networkcoinaddress = address(0x1110000000000000000100000000000000000111);

        // Default points table
        pointsTable.bet_betUsingNetworkCoin = 1000000000000000000;
        pointsTable.winnerClaim             = 1000000000000000000;
        pointsTable.loserClaim              = 1000000000000000000;
        pointsTable.withdrawAccumulated     = 1000000000000000000;
    ...
    *  *****************************/
    ...
    function transferFund(address token, address to, uint256 amountInWei) external onlyOwner
    {
        doTransferFund(0, token, to, amountInWei);
    }

    function transferFundProxied(address token, address to, uint256 amountInWei) external onlyOwner
    {
    ...
    function supplyParticipationAwardBag(uint256 amountInWei) external onlyOwner
    {
    ...
    function setParticipationAwardAmount(uint256 newValue) external onlyOwner
    {
        participationAwardAmount = newValue;
    }

    function supplyPool(uint fixture, uint position, uint256 amountInWei) external onlyOwner
    {
        require(IERC20(internalToken).allowance(msg.sender, address(this)) >= amountInWei, "ALINT"); //BET: Check the internal token allowance. Use approve function.
        IERC20(internalToken).transferFrom(msg.sender, address(this), amountInWei);
    ...
    function setPool(uint fixture, uint position, uint256 newValue) external onlyOwner
    {
        if(position == 1)
        {
    ...
    function getAccumulatedAmount(address player) external view returns (uint256)
    {
        return playerAccumulated[player];
    }
    ...
    *  *****************************/
    ...
    function setOwner(address newValue) external onlyOwner returns (bool success) 
    {
        owner = newValue;
        return true;
    }

    function setFeeTo(address newValue) external onlyOwner returns (bool success)
    {
        feeTo = newValue;
        return true;
    }

    function setMatchResultSetter(address newValue) external onlyOwner returns (bool success)
    {
        matchResultSetter = newValue;
        return true;
    }

    function setPercentOfBetForFairPlayFee(uint256 newValue) external onlyOwner
    {
        percentOfBetForFairPlayFee = newValue;
    }

    function setPercentOfBetForCommunityFee(uint256 newValue) external onlyOwner
    {
        percentOfBetForCommunityFee = newValue;
    }

    // function setNetworkCoinAddress(address newValue) external onlyOwner returns (bool success)
    // {
    //     networkcoinaddress = newValue;
    //     return true;
    // }

    function setChainWrapToken(address newValue) external onlyOwner
    {
        chainWrapToken = newValue;
    }

    function setSwapFactory(address newValue) external onlyOwner
    {
        swapFactory = newValue;
    }

    function setSwapRouter(address newValue) external onlyOwner
    {
        swapRouter = newValue;
    }

    function setInternalToken(address newValue) external onlyOwner
    {
        internalToken = newValue;
    }

    function setUSDToken(address newValue) external onlyOwner
    {
        usdToken = newValue;
    }

    function setCommunityContract(address newValue) external onlyOwner
    {
        communityContract = newValue;
    }

    /* **********************************************************
            FOOTBALL BLOCK DATA FUNCTIONS
    *  **********************************************************/
    ...
    // function getTop1000Fixture() external view returns (uint[1000] memory)
    // {
    //     uint[1000] memory result;

    //     for(uint ix = fixturelist.length - 1; ix >= 0; ix--)
    //     {
    //         result[ix] = fixturelist[ix];
    //     }

    //     return result;
    // }

    function getFixtureData(uint fixture, address player) external view returns (uint256[] memory, address[] memory)
    {
        uint256[] memory result = new uint256[](24);
        address[] memory resultAddress = new address[](2);
        ...
        if(player != address(0))
        {
            ...
            result[23] = fixturePlayerClaimedAmount[fixture][player]; // Player Claimed Amount

            resultAddress[0] = fixturePlayerBetToken[fixture][player]; // getFixturePlayerBetToken
            resultAddress[1] = fixturePlayerClaimedToken[fixture][player]; // Player Claimed Token
        }
    ...
    function setFixtureBetOpened(uint fixture, uint newValue) external onlyOwner
    {
    ...
    function setFixtureTreasuryFeePercent(uint fixture, uint256 newValue) external onlyOwner
    {
    ...
    function setFixturePercentOfTreasuryFeeForParticipationAward(uint fixture, uint256 newValue) external onlyOwner
    {
    ...
    function setFixturePercentOfParticipationAwardBagToPayForLosers(uint fixture, uint256 newValue) external onlyOwner
    {
    ...
    function setFixtureBetCloseTime(uint fixture, uint256 newValue) external onlyOwner
    {
    ...
    function getFixturePlayerClaimedAmount(uint fixture, address player) external view returns (uint256)
    {
        return fixturePlayerClaimedAmount[fixture][player];
    }

    function getFixturePlayerClaimedToken(uint fixture, address player) external view returns (address)
    {
        return fixturePlayerClaimedToken[fixture][player];
    }
    ...
    /* **********************************************************
            FOOTBALL CALCULATION FUNCTIONS
    *  **********************************************************/
    ...
    *  **********************************************************/
    ...
    function matchSetup(uint season, uint league, uint fixture, uint forceBetToBeOpened, uint256 treasuryFeePercent, uint256 percentOfTreasuryFeeForParticipationAward, uint256 percentOfParticipationAwardBagToPayForLosers, uint256 betCloseTime) external onlyOwner
    {
    ...
    function bulkSetMatchResult(uint[] calldata fixtures, uint[] calldata positions) external
    {
        require(msg.sender == owner || msg.sender == matchResultSetter, 'FN'); //Forbidden

        for(uint ix = 0; ix < fixtures.length; ix++)
        {
            fixtureMatchResultPosition[fixtures[ix]] = positions[ix];
            fixtureBetOpened[fixtures[ix]] = 0;
            fixtureBetCloseTime[fixtures[ix]] = block.timestamp;
        }
    }

    /* **********************************************************
            FOOTBALL BET DEPOSIT
    *  **********************************************************/
    ...
    function bet(uint fixture, uint betPosition, address tokenBet, uint256 amountInWei) external
    {
        ...
        if(tokenBet != internalToken)
        {
            ...
            swap(tokenBet, internalToken, amountInWei, amountOutMin, address(this));
        ...
        if(percentOfBetForCommunityFee > 0 && communityContract != address(0))
        {
            uint256 communityFee = SafeMath.safeDiv(SafeMath.safeMul(betAmount, percentOfBetForCommunityFee), ONE_HUNDRED);

            // NOTOPERATIONAL
            // IERC20(internalToken).transferFrom(msg.sender, communityContract, communityFee);

            IERC20(internalToken).transfer(communityContract, communityFee);

            enteredBetAmount = SafeMath.safeSub(enteredBetAmount, communityFee);
        }
        ...
        playerScore[msg.sender] = SafeMath.safeAdd(playerScore[msg.sender], pointsTable.bet_betUsingNetworkCoin);
        playerCounters[msg.sender].bet_betUsingNetworkCoin = SafeMath.safeAdd(playerCounters[msg.sender].bet_betUsingNetworkCoin, 1);
    ...
    function betUsingNetworkCoin(uint fixture, uint betPosition) external payable
    {
        ...
        swapNetworkCoinToToken(internalToken, amountOutMin, address(this));
        ...
        uint256 fairPlayAmountForEachPool;
        uint256 enteredBetAmount = amountOutMin;

        if(percentOfBetForFairPlayFee > 0)
        {
            uint256 fairPlayTotalAmount = SafeMath.safeDiv(SafeMath.safeMul(amountOutMin, percentOfBetForFairPlayFee), ONE_HUNDRED);
            fairPlayAmountForEachPool = SafeMath.safeDiv(fairPlayTotalAmount, 2);
            enteredBetAmount = SafeMath.safeSub(amountOutMin, fairPlayTotalAmount);
        }

        if(percentOfBetForCommunityFee > 0 && communityContract != address(0))
        {
            uint256 communityFee = SafeMath.safeDiv(SafeMath.safeMul(amountOutMin, percentOfBetForCommunityFee), ONE_HUNDRED);
        ...
        // Compute points (0 = bet/betUsingNetworkCoin)
        playerScore[msg.sender] = SafeMath.safeAdd(playerScore[msg.sender], pointsTable.bet_betUsingNetworkCoin);
        playerCounters[msg.sender].bet_betUsingNetworkCoin = SafeMath.safeAdd(playerCounters[msg.sender].bet_betUsingNetworkCoin, 1);
    ...
    /* **********************************************************
            FOOTBALL BET WITHDRAW
    *  **********************************************************/

    function winnerClaim(uint fixture, uint directInternalToken, uint accumulate) external
    {
        ...
        if(accumulate == 0)
        {
            ...
            if(directInternalToken == 0)
            {
                ...
                fixturePlayerClaimedAmount[fixture][msg.sender] = amountOutMin;
                fixturePlayerClaimedToken[fixture][msg.sender] = claimToken;
            }
            else
            {
                fixturePlayerClaimedAmount[fixture][msg.sender] = amountToReceiveInInternalToken;
                fixturePlayerClaimedToken[fixture][msg.sender] = internalToken;
            }
        }
        else
        {
            playerAccumulated[msg.sender] = SafeMath.safeAdd(playerAccumulated[msg.sender], amountToReceiveInInternalToken);
            fixturePlayerClaimedAmount[fixture][msg.sender] = amountToReceiveInInternalToken;
            fixturePlayerClaimedToken[fixture][msg.sender] = internalToken;
        }
        ...
        // Compute points (1 = winnerClaim)
        playerScore[msg.sender] = SafeMath.safeAdd(playerScore[msg.sender], pointsTable.winnerClaim);
        playerCounters[msg.sender].winnerClaim = SafeMath.safeAdd(playerCounters[msg.sender].winnerClaim, 1);

        emit OnWinnerClaim(msg.sender, fixture);
    }

    function loserClaim(uint fixture, uint directInternalToken, uint accumulate) external
    {
        require(fixtureMatchResultPosition[fixture] != 0, "LIVE"); //BET: Bet without result
        require(getFixtureCloseTimeLeft(fixture) == 0, "TIMELIVE"); //BET: Bet time not finished
        require(fixtureBetOpened[fixture] == 0, "OPENED"); //BET: Bet is opened
        ...
        // Compute points (2 = loserClaim)
        playerScore[msg.sender] = SafeMath.safeAdd(playerScore[msg.sender], pointsTable.loserClaim);
        playerCounters[msg.sender].loserClaim = SafeMath.safeAdd(playerCounters[msg.sender].loserClaim, 1);

        emit OnLoserClaim(msg.sender, fixture);
    }

    function withdrawAccumulated(address token) external
    {
        require(playerAccumulated[msg.sender] > 0, 'EMPTY');
        require(token == usdToken || token == internalToken, 'INV');

        // Send internal token to player address when direct internal
        if(token == internalToken)
        {
            systemWithdraw(msg.sender, internalToken, playerAccumulated[msg.sender], 0);
        }
        else
        {
            // If player wants to receive in USD token, do swap

            // Zero in amount out accepts any change in value
            swap(internalToken, usdToken, playerAccumulated[msg.sender], 0, msg.sender);
        }

        playerAccumulated[msg.sender] = 0;

        // Compute points (3 = withdrawAccumulated)
        playerScore[msg.sender] = SafeMath.safeAdd(playerScore[msg.sender], pointsTable.withdrawAccumulated);
        playerCounters[msg.sender].withdrawAccumulated = SafeMath.safeAdd(playerCounters[msg.sender].withdrawAccumulated, 1);
    }
    ...
    *  **********************************************************/
    ...
    function getCounters(address player) external view returns (ScoreFunctions memory)
    {
        return playerCounters[player];
    }

    function getPointsTable() external view returns (ScoreFunctions memory)
    {
        return pointsTable;
    }

    function setPointsTable(uint functionIndex, uint256 value) external onlyOwner
    {
        //  0 = bet/betUsingNetworkCoin
        //  1 = winnerClaim
        //  2 = loserClaim
        //  3 = withdrawAccumulated

        if(functionIndex == 0)
        {
            pointsTable.bet_betUsingNetworkCoin = value;
        }
        else if(functionIndex == 1)
        {
            pointsTable.winnerClaim = value;
        }
        else if(functionIndex == 2)
        {
            pointsTable.loserClaim = value;
        }
        else if(functionIndex == 3)
        {
            pointsTable.withdrawAccumulated = value;
        }
        else
        {
            return;
