...
contract TESTNETMAN is Context, IERC20, Ownable {
    ...
    uint256 private _boostedBuyBackFee = 0;
    ...
    uint256 private buyBackSellLimit = (_tTotal * 2) / 100;
    uint256 private buyBackAmount = 1 * 10**18;
...
,        address to,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalBNBFees = _marketingFee.add(_liquidityFee).add(_buyBackFee);
        if (totalBNBFees == 0)
            return;
        ...
        uint256 toLiquify = contractTokenBalance.sub(toMarketing).sub(toBuyBack);
        ...
        uint256 marketingBalance = fromSwap.sub(liquidityBalance.add(buyBackBalance));
