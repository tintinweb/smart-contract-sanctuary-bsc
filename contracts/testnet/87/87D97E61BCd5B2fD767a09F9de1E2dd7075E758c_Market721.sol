...
contract Market721 is OwnableUpgradeable, ERC721HolderUpgradeable{
    ...
    event UpdateGoodsStatus(uint indexed goodsId, bool indexed status, uint goodsType, uint tokenId, uint tradeType, uint price);
    ...
    function sell(uint goodsType_, uint tokenId_, uint tradeType_, uint price_) public {
        require(price_ < 1e26 && price_ % 1e16 == 0, "Price");
    ...
    function getUserSaleList(uint goodsType_, address addr_) public view returns (uint[4][] memory data) {
        uint len = _sellingList[goodsType_][addr_].length;
        data = new uint[4][](len);
        for (uint i = 0; i < len; i++) {
            uint[4] memory saleGoods;
            uint tokenId = _sellingList[goodsType_][addr_][i];
            saleGoods[0] = sellingGoods[goodsType_][tokenId].id;
            saleGoods[1] = tokenId;
            saleGoods[2] = sellingGoods[goodsType_][tokenId].tradeType;
            saleGoods[3] = sellingGoods[goodsType_][tokenId].price;
            data[i] = saleGoods;
        }
    }
