...
contract Project is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint256 => uint256) private totalBuyersWaitingToTokens;
    mapping(uint256 => uint256) private totalSalesNotClose;
    ...
    ///TODO
    function closeProject(uint256 _projectId, uint256[] memory saleIds, bool _isGive) external nonReentrant onlyManager(_projectId) {
        uint256 totalBuyersWaitingClose;
        uint256 totalCloseSale;
        uint256 loopCounted = totalBuyersWaitingToTokens[_projectId] + totalSalesNotClose[_projectId];
        ...
        for (uint256 i; i < saleIds.length; i++) {
            SaleInfo memory saleInfo = sale.getSaleById(saleIds[i]);
            require (saleInfo.projectId == _projectId && !saleInfo.isClose, "Invalid saleId");
            ...
            //TODO
            if (totalBuyersWaitingClose == _closeLimit) continue;
            (totalBuyersWaitingClose, totalCloseSale) = sale.close(_closeLimit, _project, saleInfo, totalBuyersWaitingClose, totalCloseSale, _isGive);
        }
        totalBuyersWaitingToTokens[_projectId] -= totalBuyersWaitingClose;
        totalSalesNotClose[_projectId] -= totalCloseSale;
        if (totalSalesNotClose[_projectId] == 0) projects[_projectId].status = ProjectStatus.ENDED;
    ...
    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable nonReentrant {
        ...
        totalSalesNotClose[lastId] = _saleInputs.length;
    ...
    //TODO
    function setTotalBuyersWaitingToToken(uint256 _projectId, uint256 _total) external projectExists(_projectId) onlySale {
        totalBuyersWaitingToTokens[_projectId] = _total;
    }

    //TODO
    function getTotalBuyersWaitingToToken(uint256 _projectId) external view returns (uint256) {
        return totalBuyersWaitingToTokens[_projectId];
    }

    function setTotalSalesNotClose(uint256 _projectId, uint256 _total) external projectExists(_projectId) onlySale {
        totalSalesNotClose[_projectId] = _total;
    }

    //TODO
    function getTotalSalesNotClose(uint256 _projectId) external view returns (uint256) {
        return totalSalesNotClose[_projectId];
    }
