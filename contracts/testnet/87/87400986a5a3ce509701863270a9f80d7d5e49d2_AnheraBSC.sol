...
contract AnheraBSC is Ownable {  
    ...
    function buyTokenWithBUSD(uint busdAmount) external {
      ...
      BEP20 receiveToken = BEP20(0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526);///Testnet
      uint tariff = 0;
    ...
    function tokenInBNB(uint amount) public view returns (uint) {
        uint tokenVal = (amount * priceOfBNB* 10**tokenPriceDecimal) /(tokenPrice* 10**26 ) ;
        return (tokenVal);
    }
    function tokenInBUSD(uint amount) public view returns (uint) {
        uint tokenVal = (amount * 10**tokenPriceDecimal ) /(tokenPrice* 10**18 ) ;
        return (tokenVal);
    }
    /*
