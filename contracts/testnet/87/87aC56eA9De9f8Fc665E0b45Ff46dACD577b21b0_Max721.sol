...
import "./Max721Implementation3.sol";
import "./lib/Roles.sol";
import "./lib/721.sol";
import "./lib/PsuedoRand.sol";

contract Max721 is Max721Implementation3 {
  ...
  constructor() {
    ...
    contractRoles.setAdmin(msg.sender);
...
abstract contract Max721Implementation3 is MaxErrors
                                         , MaxAccess
                                         , IERC721Enumerable
  ...
  , bytes calldata data
    ...
    virtual
    override {
    if (!token721.isApprovedOrOwner(msg.sender, _tokenId)) {
      revert MaxSplaining({
        reason: "Max721:!isApprovedOrOwner"
      });
    }
  ...
  , uint256 _tokenId
    ...
    address _owner = token721.getOwnerOf(_tokenId);
