...
contract Manager is Context, Ownable {
    ...
    function setToken(address payable token_) public onlyOwner {
        token = token_;
    }
...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    uint256 public buyTime = block.timestamp + 15 days * 1 seconds;//+ 15 days
    
    constructor (uint256 lockTime,uint8 price_type, address payable wallet_, IERC20 token_, address payable manager_,
    uint256 _min) {
   ...
   function buyTokens() public nonReentrant payable {
        ...
        require(weiAmount >=minBuy && weiAmount <=maxBuy,"please approve Busd according to limit");
        
        //new calulate amount
        uint256 one = 1 ether;
        uint256 tokens =  (one * weiAmount)/sale_price;
