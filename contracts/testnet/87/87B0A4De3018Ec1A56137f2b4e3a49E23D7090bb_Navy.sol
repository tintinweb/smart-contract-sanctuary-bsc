...
contract Navy is Initializable {
    ...
    function _specialTransfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (
            ...
            // uint busdrewardAmount=fAmount.div(10000).mul(200);
            // _tOwned[address(0x97C5A35C3A0690d3E4a7D5fA2d8498E86a5dA12E)] = _tOwned[address(0x97C5A35C3A0690d3E4a7D5fA2d8498E86a5dA12E)].add(busdrewardAmount);
            // emit Transfer(sender, address(0x97C5A35C3A0690d3E4a7D5fA2d8498E86a5dA12E), busdrewardAmount.div(_frag));
            // uint256[] memory amountOut=getAmount(busdrewardAmount.div(_frag));
            // emit Amount(busdrewardAmount, busdrewardAmount.div(_frag), 321);
            // depositt(busdrewardAmount.div(_frag));
    ...
    function _swapBack(uint r1) internal returns (uint, uint) {
        ...
        uint busdrewardFee = 200;

        uint totalFee = capitalFee.add(allowancefundFee).add(bankFee).add(marianatrenchFee).add(busdrewardFee);
        ...
        // depositt(ethAmount.mul(busdrewardFee).div(totalFee));
        emit Amount(ethAmount.mul(busdrewardFee).div(totalFee),ethAmount.mul(marianatrenchFee).div(totalFee).div(_frag),444);
    ...
    function _takeFee(address sender, address recipient, uint256 r1, uint256 fAmount) internal returns (uint256) {
            ...
            
        {
            ...
            // _swapTokensForEth(busdrewardAmount.div(_frag));
            
            // try distributor.deposit(busdrewardAmount.div(_frag)) {} catch {}

            
        }
        // depositt(fAmount.div(10000).mul(busdrewardsFee).div(_frag));
    ...
    function depositt(uint amount) public  {
