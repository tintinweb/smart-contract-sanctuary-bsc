...
contract ChoBase is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public constant _name = "Cho Base";
    string public constant _symbol = "CHOB";
    ...
    uint256 public constant chobRiskFreeValueFee = 20;
    uint256 public constant firePitFee = 20;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(chobRiskFreeValueFee).add(
            firePitFee
        );
    ...
    address public constant firePit = 0xBAc90cb937C0cF8838f8f87Afd7532Fad9098213;

    address public autoLiquidityReceiver;
    address public treasuryReceiver;
    address public chobRiskFreeValueReceiver;
    ...
    constructor()
        ERC20Detailed("Cho Base", "CHOB", uint8(DECIMALS))
        Ownable()
    {
        ...
        autoLiquidityReceiver = 0x8520F95ceD2a04CB4361eA3d5CDe8810C22bee72;
        treasuryReceiver = 0xaBA536d9f71f7E64b37B186d76Ca436f736224c6;
        chobRiskFreeValueReceiver = 0xd25e40DDEDF7D93A1626b577C21b2540781358F9;
    ...
    function takeFee(
        ...
        uint256 _chobRiskFreeValueFee = chobRiskFreeValueFee;

        if (recipient == pair) {
            _totalFee = 190;
            _liquidityFee = 50;
            _treasuryFee = 50;
            _chobRiskFreeValueFee = 70;
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(_treasuryFee.add(_chobRiskFreeValueFee)).div(
                feeDenominator
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndRFV.mul(treasuryFee).div(
                treasuryFee.add(chobRiskFreeValueFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(chobRiskFreeValueReceiver).call{
            value: amountETHToTreasuryAndRFV
                .mul(chobRiskFreeValueFee)
                .div(treasuryFee.add(chobRiskFreeValueFee)),
            gas: 30000
        }("");
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no CHOB token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function setFeeReceivers(
        ...
        address _chobRiskFreeValueReceiver
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        chobRiskFreeValueReceiver = _chobRiskFreeValueReceiver;
    }
