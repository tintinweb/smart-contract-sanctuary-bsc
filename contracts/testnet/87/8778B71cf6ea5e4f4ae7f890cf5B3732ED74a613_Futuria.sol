...
contract Futuria is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public minWeeklyTurnoverPerLeg = 200 ether;
    ...
    constructor() ERC721("Futuria", "FUTURIA") {
        ...
        subscriberToTimestamp[defaultReferralAccount] = 100000 days;
        ...
        if (isTestnet) {
            ...
            safeMint(
                1,
                admin,
                "ipfs://bafyreihoo3y2akgo7cyp6lxnvtiz24mch4gjv7trzc3txyyujdbxofbaka/metadata.json",
                400 ether,
                365 days
            );
            safeMint(
                1,
                admin,
                "ipfs://bafyreiamtpsheq2hv4ourcuvc2lfegwd6tka3wxorzlwuwwunrzm4epc5e/metadata.json",
                250 ether,
                182 days
            );
            safeMint(
                1,
                admin,
                "ipfs://bafyreigpixyh55bwaxad6kxaniblvgompbcx63jja26dlaxzntszn4hzzi/metadata.json",
                60 ether,
                30 days
            );
            safeMint(
                1,
                admin,
                "ipfs://bafyreifi46qr247dhvfxefct4uoazqqkmuh26ok5mapngizufxs7us4siq/metadata.json",
                150 ether,
                91 days
            );
        }
    ...
    {
        if (
            block.timestamp > subscriberToTimestamp[addr]
        ) {
    ...
    function getDirectCommissions(address addr) public view returns (uint256) {
        ...
        uint256 percentage = getDirectSalesPercentage(addr);
    ...
    function setTokenIdSubs(uint256 tokenId, uint256 subsLength)
        public onlyNftOwnerOrAdmin(tokenId)
    {
    ...
    function updateSubscription(
        ...
        uint256 price,
        uint256 subsLength
    ) external onlyNftOwnerOrAdmin(tokenId) {
    ...
    function subscribe(uint256 tokenId, address referralAddress) external {
        ...
        subscriberToTimestamp[msg.sender] = block.timestamp + tokenIdToSubsLength[tokenId];
    ...
    function setWeeklyEarnings() external onlyAdmin {
        ...
        onlyAdmin
    {
        ...
        weekTurnover += volume;
        subscriberToTimestamp[addr] = block.timestamp + tokenIdToSubsLength[tokenId];
    ...
    function updateDirectCustomersVolumeOnlyAdmin(address sponsor, uint256 amount)
        external
        onlyAdmin
    {
    ...
    function burn(uint256 tokenId) external virtual onlyAdmin {
        require(!isContractLocked, "Contract is locked");
        _burn(tokenId);
    }
    ...
    function transferFromBUSD(
        ...
        uint256 allowance = allowanceBUSD(sender, recipient);
    ...
    function updateDirectCustomersVolume(address sponsor, uint256 amount) internal {
