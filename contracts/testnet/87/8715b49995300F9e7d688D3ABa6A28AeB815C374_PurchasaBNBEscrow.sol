...
contract PurchasaBNBEscrow is Ownable {
    ...
    function lockBNBToken(string memory orderId,Orders[] memory orders, address reciever,string memory currency) payable public {
    ...
    function lockBEPToken(string memory orderId,Orders[] memory orders, address tokenAddress,uint256 amount, address reciever,string memory currency) public {
                    ...
                    for(uint k=0; k < userOrders[i].items.length; k++) { 
                        if(userOrders[i].items[k].isDispute == true) {
                            ...
                            if(userOrders[i].reciever == refundAddress) {
                                amount = amount.add(userOrders[i].items[k].totalLocked - userOrders[i].items[k].totalLockedCommission);
    ...
    function ownerChangeBEPCommission(address tokenAddress,uint commissions) public onlyOwner{
        require(tokenslist[tokenAddress] == true, "We are not providing services for this token");
            ...
            if(keccak256(abi.encodePacked(userOrders[i].currency)) == keccak256(abi.encodePacked(currency)) && userOrders[i].reciever == merchantAddress && userOrders[i].isDispute == false) {
               for(uint k=0; k < userOrders[i].items.length; k++) { 
                        if(userOrders[i].items[k].isDispute == false) {
                             totalAmount = totalAmount.add(userOrders[i].items[k].totalLocked - userOrders[i].items[k].totalLockedCommission);
                        }
                    }
