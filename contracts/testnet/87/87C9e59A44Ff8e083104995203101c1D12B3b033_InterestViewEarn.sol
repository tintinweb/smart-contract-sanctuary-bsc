...
contract InterestViewEarn {
    ...
    function getFarmsSummary(
        ...
        returns (
            ...
            UserFarmData[] memory farmDatas
        )
    {
        pools = new PoolData[](pairs.length);
        prices = new uint256[](tokens.length);
        farmDatas = new UserFarmData[](pairs.length);

        unchecked {
            for (uint256 i; i < pairs.length; i++) {
                pools[i] = _getPoolData(pairs[i], poolIds[i]);
                farmDatas[i] = getUserFarmData(
                    IERC20(pairs[i]),
                    account,
                    poolIds[i]
                );
    ...
    function getUserFarmData(
        ...
        address user,
        uint256 poolId
    ) public view returns (UserFarmData memory farmData) {
        ...
        (farmData.stakingAmount, ) = CASA_DE_PAPEL.userInfo(poolId, user);
    }
