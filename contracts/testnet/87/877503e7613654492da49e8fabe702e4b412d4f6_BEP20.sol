...
contract Space_Koala is BEP20, MinterRole {
    ...
    mapping (address => uint256) public lockrate;

    // tax Fee Wallet address
    address public feeWallet_NFT;
    address public feeWallet_marketing;
    address public feeWallet_liquidity;
    ...
    // In swap and liquify
    bool private _inSwapForUSDT;

    // Automatic swap and liquify enabled
    bool public swapForUSDTEnabled = true; // INMUTABLE

    // Burn address
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD; // INMUTABLE

    // USDT address
    address public constant USDT = address(0x377533D0E68A22CF180205e9c9ed980f74bc5050);
    ...
    address public constant V2ROUTER = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;

    // Events before Governance
    event OperatorTransferred(address indexed previousOperator, address indexed newOperator);
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);
    event UpdateFeeWallet_NFT(address indexed walletAddress);
    event UpdateFeeWallet_marketing(address indexed walletAddress);
    event UpdateFeeWallet_liquidity(address indexed walletAddress);
    ...
    /**z
     * @notice Constructs the SQL token contract.
     */
    constructor(address _NFTwallet, address _marketingwallet, address _liquiditywallet) public BEP20("Space Koala Test", "SKT") {
        operator = _msgSender();
        ...
        require(_NFTwallet != address(0), "wallet address is zero");
        feeWallet_NFT = _NFTwallet;

        require(_marketingwallet != address(0), "wallet address is zero");
        feeWallet_marketing = _marketingwallet;

        require(_liquiditywallet != address(0), "wallet address is zero");
        feeWallet_liquidity = _liquiditywallet;

   
        lpcontract = address(uniswapV2Pair);

        setExcludeFromFee(msg.sender, true);
        setExcludeFromFee(address(this), true);
        setExcludeFromFee(address(BURN_ADDRESS), true);
        mint(address(msg.sender), 10**26);
    }
    ...
    /// @dev overrides transfer function to meet tokenomics of SQL
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        // Pre-flight checks
        require(amount > 0, "Transfer amount must be greater than zero");
        if( 
            tradingOpen == true
            && swapForUSDTEnabled == true 
            && _inSwapForUSDT == false
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            //Sell Token
            else if(automatedMarketMakerPairs[recipient] && sellFee >0) {
    ...
    function swapTokensAndSend() private lockTheSwap {
      uint256 contractTokenBalance = balanceOf(address(this));
        uint256 contractTokenBalance_NFT = balanceOf(address(this)).mul(4).div(10);
        uint256 contractTokenBalance_marketing = balanceOf(address(this)).mul(3).div(10);
        uint256 contractTokenBalance_liquidity = balanceOf(address(this)).mul(2).div(10);
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
        contractTokenBalance_marketing,
        0,
        path,
        feeWallet_marketing,
        block.timestamp + 400
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
        contractTokenBalance_liquidity,
        0,
        path,
        feeWallet_liquidity,
        block.timestamp + 400
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "SQL::transferOperator: new operator is the zero address");
    ...
    function setlockup(bool sLock) public onlyOperator{
        lockup = sLock;
    }
    ...
    function updateBuyFee(uint16 _buyFee) public onlyOperator {
        require( _buyFee <= 100, "SQL:: Biu Fee can't exceed 100%" );
        emit UpdateBuyFee(_buyFee);
        buyFee = _buyFee;
    }

    function updateSellFee(uint16 _sellFee) public onlyOperator {
        require( _sellFee <= 100, "SQL:: Sell Fee can't exceed 100%");
        emit UpdateSellFee(_sellFee);
        sellFee = _sellFee;
    }


    function enableSwapTokenForUSDT(bool _enabled) public onlyOperator {
        swapForUSDTEnabled = _enabled;
    }
     ...
     */
    function updateFeeWallet_NFT(address _NFTwallet) public onlyOperator {
        require( _NFTwallet != address(0), "SQL:: tax fee address is zero" );
        emit UpdateFeeWallet_NFT(_NFTwallet);
        feeWallet_NFT = _NFTwallet;
    }

    function updateFeeWallet_marketing(address _marketingwallet) public onlyOperator {
        require( _marketingwallet != address(0), "SQL:: tax fee address is zero" );
        emit UpdateFeeWallet_marketing(_marketingwallet);
        feeWallet_NFT = _marketingwallet;
    }

    function updateFeeWallet_liquidity(address _liquiditywallet) public onlyOperator {
        require( _liquiditywallet != address(0), "SQL:: tax fee address is zero" );
        emit UpdateFeeWallet_liquidity(_liquiditywallet);
        feeWallet_NFT = _liquiditywallet;
    }


    function updateLpcontract(address _lpcontract) public onlyOperator {
        require( _lpcontract != address(0), "SQL:: lpcotontract address is not zero" );
