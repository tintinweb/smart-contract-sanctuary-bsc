...
contract PrivateWallet is Ownable, ReentrancyGuard {
    ...
    function buy(uint256 amount) external onlyAfterStart nonReentrant {
        ...
        require(
            isWhiteList1(msg.sender) ||
                isWhiteList2(msg.sender),
        ...
        if (isWhiteList1(msg.sender)) {
            ...
            require(user.balance + amount <= _maxW1, "PRIVATE:More than maxium");
        } else if (isWhiteList2(msg.sender)) {
            require(amount >= _minW2, "PRIVATE:Less than _minW2");
            require(user.balance + amount <= _maxW2, "PRIVATE:More than maxium");
        }
        user.balance += amount;
    ...
    function getUnlocked(address account) public view returns (uint256) {
        ...
        require(block.timestamp >= _firstLockingTime, "PRIVATE:Early for unlocking");
    ...
    function setUserBalance(address account, uint256 amount) external onlyOwner {
