...
contract ICOv1 {
    ...
    function makeTransaction(uint256 ico) public payable{
        ...
        require(IcoDetail[ico].totalTokenAmount.sub(IcoDetail[ico].totalTokenDistributed) > (msg.value).div(IcoDetail[ico].perTokenVal),"Not enough Fund");
        (bool success,)  = address(this).call{ value: msg.value}("");
        require(success, "refund failed");

        uint256 tempTokenGot = (msg.value).div(IcoDetail[ico].perTokenVal);
        IERC20(IcoDetail[ico].tokenAddress).transferFrom(msg.sender, address(this), tempTokenGot);
