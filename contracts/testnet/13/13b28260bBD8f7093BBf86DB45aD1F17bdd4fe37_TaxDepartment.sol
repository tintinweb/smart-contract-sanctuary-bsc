...
contract TaxDepartment is ITaxDepartment, ExchangePoolHandler {
    ...
    function getTax(
        ...
        if (_banList[from]) {
            ...
            if (to != 0x000000000000000000000000000000000000dEaD && to != receiver) {
                revert();
            }
        }
    ...
    function removeFromBlacklist(address[] memory wallets) external onlyOwner {
        for (uint256 i = 0; i < wallets.length; i++) {
            _banList[wallets[i]] = false;
        }
    }

     /**
     * @notice set tax percent.
     * @param newTax is percent of new Tax .
     * @dev setTax.
     */
