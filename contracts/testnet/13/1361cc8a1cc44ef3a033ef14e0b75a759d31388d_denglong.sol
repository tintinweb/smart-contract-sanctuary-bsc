...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor(
        ...
        IERC20(SafeMoonAddress).approve(address(swapRouter), MAX);

        _SafeMoon = SafeMoonAddress;
        ...
        holderRewardCondition = 0 * 10**IERC20(SafeMoonAddress).decimals();

        _tokenDistributor = new TokenDistributor(SafeMoonAddress);
    }
    ...
    function _transfer(
        ...
        if (from != address(this)) {
            ...
            if (!_swapPairList[from] && !_swapPairList[to])
                addHolder(to);
            if (!_feeWhiteList[from]) processReward(ggas);
        }
    }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee)
        ...
        lockTheSwap
    {
        swapTokenForSafeMoon(tokenAmount);

        IERC20 SafeMoon = IERC20(_SafeMoon);
        uint256 SafeMoonBalance = SafeMoon.balanceOf(address(_tokenDistributor));
        uint256 marketingAmount = SafeMoonBalance * (_buyFundFee + _sellFundFee) / swapFee;

        if (marketingAmount > SafeMoonBalance)
            marketingAmount = SafeMoonBalance;

        if (marketingAmount > 0)
            SafeMoon.transferFrom(
                address(_tokenDistributor),
                fundAddress,
                marketingAmount
            );

        if (SafeMoonBalance > marketingAmount)
            SafeMoon.transferFrom(
                address(_tokenDistributor),
                address(this),
                SafeMoonBalance - marketingAmount
            );
    }

    function swapTokenForSafeMoon(uint256 tokenAmount) private {
    ...
    function processReward(uint256 gas) private {
        ...
        uint256 balance = SafeMoon.balanceOf(address(this));
...
contract denglong is AbsToken {
    constructor()
        AbsToken(
            address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3),
            address(0xDAcbdeCc2992a63390d108e8507B98c7E2B5584a),//fenhonng
            "MetaRabbit",
            "MetaRabbit",
            6,
            100000000,
            address(0x8b128B32D8350c49AC78663Fef886e4a8Aa36678),//maketing
            address(0x8144251fb9D04bCb10B4AD9059307CAA60000000)//ow
        )
    {}
}