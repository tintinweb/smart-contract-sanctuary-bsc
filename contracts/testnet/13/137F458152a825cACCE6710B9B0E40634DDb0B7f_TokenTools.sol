...
abstract contract Service {
    ...
    address private serviceAdmin;
    
    constructor(address _serviceAdmin) payable {
        serviceAdmin = _serviceAdmin;
        payable(_serviceAdmin).transfer(msg.value);
    }

    function createdByTokenTools() public pure returns (bool) {
        return true;
    }
...
contract TokenTools is IERC20, Ownable, Service {
    ...
    // Tax, liquidity, burn and marketing fee
    ...
    constructor(
        ...
        address _router_,
        uint8 _taxFee_,
        uint8 _marketingFee_,
        uint8 _burnFee_,
        uint8 _liquidityFee_
    ) payable Service(_serviceAdmin) {
        _name = _name_;
        _symbol = _symbol_;
        _decimals = _decimals_;
        _tTotal = _initial_ * 10 ** _decimals_;
        ...
        numTokensSellToAddToLiquidity = (_initial_ * 10 ** _decimals_).mul(1).div(1000);

        IUniswapV2Router02 _uniV2Router = IUniswapV2Router02(_router_);
    ...
    function deliver(uint256 tAmount) public {
        ...
        require(!_isExcluded[sender], "Excluded addresses can not call this function");
    ...
    function updateAllFeePercents(
        ...
        require(liquidityFee >= 0 && liquidityFee <= maxLiqFee, "Liquidity fee error");
        require(burnFee >= 0 && burnFee <= maxBurnFee, "Burn fee error");
        require(marketingFee >= 0 && marketingFee <= maxMarketingFee, "Marketing fee error");
    ...
    function setMarketingWallet(address payable newMarketingWallet) external onlyOwner {
        require(newMarketingWallet != address(0), "Wallet can not be the ZERO ADDRESS");
        excludeFromReward(newMarketingWallet);
        marketingWallet = newMarketingWallet;
    }

    //to receive ETH from uniV2Router when swaping
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        //This needs to be distributed among burn, marketing and liquidity
        uint8 totFee = _burnFee + _marketingFee + _liquidityFee;
    ...
    function recoverToken(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        ...
        require(tokenAddress != address(this), "Only for different tokens");
