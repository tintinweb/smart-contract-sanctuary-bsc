...
contract NFTMarketplace is ReentrancyGuard{
  ...
  function createMarketSale(
    ...
    require(token.transferFrom(address(this), idToMarketItem[itemId].seller, idToMarketItem[itemId].price.mul(100 - _percentListing).div(_percentDivider)),"Error transferFrom to seller");
    require(token.transferFrom(address(this), _owner, idToMarketItem[itemId].price.mul(_percentListing).div(_percentDivider)),"Error transferFrom to owner");
  ...
  function fetchMarketItems() public view returns (MarketItem[] memory) {
    uint256 allItems = _itemIds.current().add(1);
  ...
  function fetchMyPurchasedItems(address _address) public view returns (MarketItem[] memory) {
    ...
    for (uint256 i = 0; i < allItems; i++) {
      if (idToMarketItem[i].buyer == _address) {
        myItemCount++;
      }
    }
  ...
  function fetchMySoldItems(address _address) public view returns (MarketItem[] memory) {
    ...
    uint256 j = 0;
    for (uint256 i = 0; i < allItems; i++) {
      if (idToMarketItem[i].buyer == _address && idToMarketItem[i].buyer != address(0)) {
        MarketItem memory currentItem = idToMarketItem[i];
        items[j] = currentItem;
        j++;
      }
  ...
  function fetchMyItemsOnSale(address _address) public view returns (MarketItem[] memory) {
    uint256 allItems = _itemIds.current() + 1;
    uint256 myItemCount = 0;
