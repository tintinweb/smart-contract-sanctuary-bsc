...
struct ClientRequestMatch {
  ...
  uint256 reward;
  uint256 tips;
  bool isClientConfirmedCompletion;
  bool isServiceProviderConfirmedCompletion;
  bool isAllowedDisputeHandling;
}
...
contract MetanovaPlatform is Ownable, AccessControl {
  ...
  function handleMatchDispute(uint256 _clientRequestId, uint256 refundPotion) public onlyRole(ADMIN_ROLE) {
    ...
    require(clientRequestMatchRecords[_clientRequestId].isAllowedDisputeHandling, 'Unauthorized action');
  ...
  function createClientRequest(string memory _clientRequestMetadataId) public payable {
  ...
  function createService(string memory _serviceMetadataId) public {
  ...
  function confirmMatchCompletion(uint256 _clientRequestId) public {
    ...
    require(!clientRequestMatch.isClientConfirmedCompletion || !clientRequestMatch.isServiceProviderConfirmedCompletion, 'Unauthorized action');

    // if client is calling this function
    if (clientRequestMatch.client == msg.sender) {
      require(!clientRequestMatch.isClientConfirmedCompletion, 'Duplicated function call');
      clientRequestMatch.isClientConfirmedCompletion = true;
    }
    if (clientRequestMatch.serviceProvider == msg.sender) {
      require(!clientRequestMatch.isServiceProviderConfirmedCompletion, 'Duplicated function call');
      clientRequestMatch.isServiceProviderConfirmedCompletion = true;
    }

    // if both confirmed, transfer fund to service provider, update match & clientRequest status
    if (clientRequestMatch.isServiceProviderConfirmedCompletion && clientRequestMatch.isClientConfirmedCompletion) {
      // retrieve platform fee
  ...
  function requestForMatchDisputeHandling(uint256 _clientRequestId) public {
    ...
    ClientRequestMatch storage clientRequestMatch = clientRequestMatchRecords[_clientRequestId];
    require(clientRequestMatch.client == msg.sender || clientRequestMatch.serviceProvider == msg.sender, 'Unauthorized action');
    require(!clientRequestMatch.isAllowedDisputeHandling, 'Unauthorized action');

    clientRequestMatch.isAllowedDisputeHandling = true;
  }
