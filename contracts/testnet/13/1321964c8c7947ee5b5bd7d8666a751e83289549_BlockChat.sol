...
contract Message is ERC20, ReentrancyGuard {
    ...
    modifier require_Block_owner(address BlockAddress) {
        if (Block_owner[BlockAddress] != address(0)){
            require(Block_owner[BlockAddress] == msg.sender , "You are not the owner of this Block.");
        }
        _;
    }
    ...
    function check_receiver_blacklist(address BlockAddress) public view require_non_zero(BlockAddress) returns(bool) {
        ...
        return blacklisted[BlockAddress][msg.sender];
    }
    function pause_Block(address BlockAddress) public nonReentrant() {
        require(msg.sender == Block_owner[BlockAddress], "Require Block's owner.");
    ...
    function create_Block(address BlockAddress, string memory set_name, string memory set_info) public nonReentrant() {
        ...
        Block_owner[BlockAddress]       = msg.sender;
        Block_name[BlockAddress]        = set_name;
        Block_subscribers[BlockAddress].push(msg.sender);
        Blocks_list.push(BlockAddress);
        Blocks_Owned[msg.sender].push(BlockAddress);
    }
    ...
    function Block_clear_all_chats(address BlockAddress) public require_Block_owner(BlockAddress) {
        ...
        Block[BlockAddress].push(Chat(msg.sender, block.timestamp, string("I just clear the Block.")));
    }
    function Block_subscribe_Block(address BlockAddress) public require_not_in_blacklist(BlockAddress) {
        Subscriptions[msg.sender].push(BlockAddress);
        Block_subscribers[BlockAddress].push(msg.sender);
    }
...
contract BlockChat is Message {
    ...
    function Block_Chat(address receiver, string memory _message) public nonReentrant() {
        ...
        if (Inviter[msg.sender] != address(0)) {
            pool(Inviter[msg.sender], check_saving_balance(Inviter[msg.sender]) + reward);
            reward = reward * 2;
        }
    ...
    function Block_mark_price(address BlockAddress, uint amount) public require_Block_owner(BlockAddress) {
        require(BlockAddress != msg.sender, "Can not sell your main Block.");
    ...
    function Block_trading(address BlockAddress) public nonReentrant() {
        require(balanceOf(msg.sender) >= Block_marked_price[BlockAddress], "Not enough balance.");
        require(Block_selling[BlockAddress] == true, "This Block is not selling.");
        _burn(msg.sender, Block_marked_price[BlockAddress]);
    ...
    function FiftyFifty() public nonReentrant() {
        require(balanceOf(msg.sender) >= (1 + check_user_level() * 10 ** decimals()) * MSGs_burnt / MSGs_minted, "Not enough balance.");
    ...
    function deposit_MSG(uint amount) public nonReentrant() {
        require(balanceOf(msg.sender) > (amount * 10 ** decimals()), "Not enough $MSG to withdraw.");
    ...
    function Block_withdraw_MSG(address BlockAddress, uint amount) public nonReentrant() {
        if (Block_owner[BlockAddress] != address(0)){
            require(Block_owner[BlockAddress] == msg.sender , "You are not the owner of this Block.");
        }
        require(balanceOf(BlockAddress) >= (amount * 10 ** decimals()), "Not enough $MSG to withdraw.");
        _burn(msg.sender, (amount * 10 ** decimals()));
        MSGs_burnt += (amount * 10 ** decimals());
        pool(msg.sender, check_saving_balance(msg.sender) + (amount * 10 ** decimals()));
    }
    ...
    function quit_VIP() public nonReentrant() {
        require(VIP[msg.sender] == true, "Have to be a VIP to quit.");
...
// Powered by BlockChat Limited (2022-12-21)
// Alpha Version 1.11 on Binance Smart Chain Testnet