...
contract sToken is BEP20Detailed, BEP20 {
  ...
  constructor() BEP20Detailed("s19", "S19", 18) {
  ...
  function getLiqudityPool() external view returns (address _lpAddress, bool _status) {
      _status = liquidityPool[_lpAddress];    
    return (_lpAddress, _status);
  } 
  ...
  function _getTime() external view returns(uint256 mdata, uint256 tdata) {
  ...
  function _transfer(address sender, address receiver, uint256 amount) internal virtual override {
    ...
    if(liquidityPool[sender] == true) {
      ...
      if ((tradingTime + 200 minutes) >= block.timestamp) { 

          //taxAmount = (amount * _getTime()) / 100; 
          } else {
          taxAmount = (amount * sellTax) / 100;  
          }
    ...
    if (whitelistTax[sender] || whitelistTax[receiver] || sender == marketingPool || receiver == marketingPool) {
       //taxAmount = 0;
    } else {
