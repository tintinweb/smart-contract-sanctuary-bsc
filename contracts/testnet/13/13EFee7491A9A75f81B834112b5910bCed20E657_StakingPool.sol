...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0) {
            ...
            accTokenPerShare = accTokenPerShare.add(reward.mul(1e12).div(pool.lpSupply));
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(reward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
