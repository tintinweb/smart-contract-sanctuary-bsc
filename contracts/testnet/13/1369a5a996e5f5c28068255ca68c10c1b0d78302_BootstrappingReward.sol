...
contract BootstrappingReward is Ownable {
    ...
    modifier verifyClaim(
        ...
        require(MerkleProof.verify(_merkleProof, merkleRoot, node), "BootstrappingReward:: Incorrect merkle proof");
        _;
    }
    ...
    function claimableRewards(
        ...
        uint256 _preRewards,
        bytes32[] memory _merkleProof
    ) external view verifyClaim(_index, _user, _rewards, _preRewards, _merkleProof) returns (uint256) {
        return _getClaimableRewards(_user, _rewards, _preRewards);
    }
    /*=================================== MULTATIVE ============================== */
    ...
    function claimRewards(
        ...
        uint256 _preRewards,
        bytes32[] memory _merkleProof
    ) external verifyClaim(_index, msg.sender, _rewards, _preRewards, _merkleProof) {
        uint256 _claimableRewards = _getClaimableRewards(msg.sender, _rewards, _preRewards);
    ...
    /*=================================== INTERNAL ============================== */
    function _getClaimableRewards(address _user, uint256 _rewards, uint256 _preRewards)
        internal
        view
        returns (uint256)
    {
        if (block.timestamp >= endRewardTime) {
            ...
            return (_time * _rewards / _rewardDuration) + _preRewards - rewardReceived[_user];
        }
    }
    /*=================================== EVENTS ============================== */
