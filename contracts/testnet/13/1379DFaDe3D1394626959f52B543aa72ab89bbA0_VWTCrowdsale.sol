...
contract ParticipantEnumerable {
    ...
    /**
     * Event for token purchase logging
     * @param referrer who invited purchaser to buy the tokens
     * @param beneficiary who got the tokens
     */
    event Register(address indexed referrer, address indexed beneficiary);
    ...
    function ParticipantOfAccountByIndex(address account, uint256 index) public view returns (address) {
        require(index < _participants[account].length, "ParticipantEnumerable: account index out of bounds");
    ...
    function participantByIndex(uint256 index) public view returns (address) {
        require(index < totalParticipant(), "ParticipantEnumerable: global index out of bounds");
...
contract Airdrop is Context, ParticipantEnumerable {
    ...
    function claimTokens(address referrer) public payable{
        ...
        _airdropReferralCommission[referrer] = _airdropReferralCommission[referrer].add(airdropAmount().mul(10).div(100));
    ...
    function _preValidateAirdrop(address beneficiary, address referrer, uint256 weiAmount) virtual internal view {
        ...
        require(_airdropClaimed[beneficiary] == false, 'Airdrop: beneficiary already claimed!');
        this;
    }
...
/**
 * @title VWCrowdSale
 * @dev The VWCrowdSale enables airdrop and purchasing of VW token.
 */
contract VWTCrowdsale is AllowanceCrowdsale, Airdrop {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    // Maximum sales amount of VikingsToken
    uint256 private _maxSale;
    
    // Amount of VikingsToken sold
     ...
     * @param vikingsWarToken The vikingsWarToken address, must be an ERC20 contract
     * @param vikingsWarTokensOwner Address holding the tokens, which has approved allowance to the crowdsale
     * @param maxSale Maximum sales amount of VikingsToken
     * @param airdropFee amount of BNB for claim airdrop
     */
    constructor(uint256 rate, address payable fundWallet, IERC20 vikingsWarToken, address vikingsWarTokensOwner, uint256 maxSale, uint256 airdropFee)
        Crowdsale(rate, fundWallet, vikingsWarToken)
        AllowanceCrowdsale(vikingsWarTokensOwner)
        Airdrop(airdropFee)
        public
    {
        _maxSale = maxSale;
        _addParticipant(fundWallet, _msgSender());
        _addParticipant(_msgSender(), fundWallet);
        _updatePurchasingState(fundWallet, 1);
    }

    /**
     * @return maximum sales amount of VikingsToken
     */
    ...
    /**
     * @return the amount of VikingsToken sold
     */
    ...
    function buyTokens(address beneficiary) public override payable {
        ...
        address referrer = parent(beneficiary);
        require(referrer != address(0), "VWTCrowdsale: referrer participant not exist!");
    ...
    /**
     * @dev check referrer account for purchase and airdrop
     * @param beneficiary Recipient of the token purchase
     * @param referrer Refer purchaser to purchase token
     */
    function _preValidateReferrer(address beneficiary, address referrer) internal view {
        require(referrer != beneficiary, "VWTCrowdsale: beneficiary participant can't referrer to self");
        require(parent(referrer) != address(0), "VWTCrowdsale: referrer participant not exist!");
        require(participantAmount(referrer) > 0, "VWTCrowdsale: referer must be participant befor!");
        require(parent(beneficiary) == address(0) || parent(beneficiary) == referrer, "VWTCrowdsale: invalid referrer participant");
    }
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal override view {
        require(beneficiary != address(0), "VWTCrowdsale: beneficiary is the zero address");
        require(weiAmount >= 1 * 1e17, "VWTCrowdsale: contributions must be at least 0.1 BNB during the crowdsale");
        require(_totalSold < _maxSale, "VWTCrowdsale: limit token sale");
    }
    ...
    function _getTokenAmount(uint256 weiAmount) override internal view returns (uint256) {
        ...
        if (amount >= 1500000 * 1e18) {
            return amount.mul(125).div(100);
        } else if (amount >= 500000 * 1e18) {
            return amount.mul(118).div(100);
        } else if (amount >= 100000 * 1e18) {
            return amount.mul(112).div(100);
        } else if (amount >= 50000 * 1e18) {
            return amount.mul(107).div(100);
        } else if (amount >= 20000 * 1e18) {
            return amount.mul(104).div(100);
        } else if (amount >= 5000 * 1e18) {
            return amount.mul(102).div(100);
        } else {
            return amount;
        }
    }
