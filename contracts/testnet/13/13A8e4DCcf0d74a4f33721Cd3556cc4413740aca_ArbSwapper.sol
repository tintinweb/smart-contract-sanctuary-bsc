...
contract ArbSwapper is Context, Ownable, IUniswapV2Callee {
    ...
    function printCrypto(address buyDEXRouterAddress, address sellDEXRouterAddress, address tokenToArb, uint amountToArb) public payable onlyOwner {
        ...
        ArbInfo memory arbInfo = ArbInfo(buyDEXRouterAddress, sellDEXRouterAddress, tokenToArb, amountToArb, address(msg.sender));

        bytes memory data = abi.encode(arbInfo);

        emit Log("before swap");

        //payable(address(uniswapPair)).transfer(address(this).balance);
  ...
  function pancakeCall(address _sender, uint _amount0, uint _amount1, bytes calldata _data) external override {
        ...
        IERC20(buyDEXRouter.WETH()).transfer(uniswapPair, address(this).balance);
