    ...
    WhiteList
{
    ...
    string constant normal = "normal";
    string constant white = "white";

    uint public normalPrice = 50e18;
    uint public whitePrice = 50e18;
    ...
    constructor(
        ...
        _setLimited(normal, 1000);
        _setLimited(white, 100);
    }
    ...
    uint constant normalGroup = 0;
    uint constant whiteGroup = 1;
    ...
    function getStatisticsLimit(string memory item) external view returns(uint){
        return statistics.getLimit(item);
    }
    ...
    function buy() external checkInTime(normalGroup) whenNotPaused(){
        statistics.addAmount(normal, 1);
        _BuyerPayUsdt(normalPrice);
        uint tokenId = _mintToken(_msgSender(), "limited.json");
        emit Buy(_msgSender(), tokenId, block.timestamp);
    }

    function batchBuy(uint amount) external checkInTime(normalGroup) whenNotPaused(){
        statistics.addAmount(normal, amount);
        _BuyerPayUsdt(normalPrice * amount);
    ...
    function whiteListBuy() external checkInTime(whiteGroup) checkInWhiteList checkisBought{
        statistics.addAmount(white, 1);
        _setBought(_msgSender());
        _BuyerPayUsdt(whitePrice);
        uint tokenId = _mintToken(_msgSender(), "white.json");
        emit Buy(_msgSender(), tokenId, block.timestamp);
    }
    ...
    function _setLimited(
        ...
        statistics.setLimited(item, amount);
    }

    function setLimited(string memory item, uint amount) external onlyRole(MANAGER_ROLE){
        _setLimited(item, amount);
    }

    function setAmount(string memory item, uint amount) external onlyRole(MANAGER_ROLE){
        statistics.setAmount(item, amount);
    }

    function setNormalPrice(uint price) external onlyRole(MANAGER_ROLE){
        normalPrice = price;
    }

    function setWhitePrice(uint price) external onlyRole(MANAGER_ROLE){
        whitePrice = price;      
    }
}
