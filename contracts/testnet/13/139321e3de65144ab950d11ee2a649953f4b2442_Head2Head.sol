...
contract StockDetails is Ownable, Pausable {
    error Game__StockSymbolAlreadyExists(string stockSymbol);

    //EVENT EMITTED WHEN STOCK ID IS UPDATED
    event StockIdUpdated(bytes32 stockId, string stockSymbol);

    mapping(bytes32 => string) public stockIdToSymbol;
    mapping(string => bytes32) public stockSymbolToId;
    mapping(bytes32 => bool) public stockIdToStatus;
    uint256 public currentStockId;

    /**
     * @notice Update stocks in a batch
     * @param _stockSymbols array of stock symbols which has to updated for given stock ID
     */
    function updateStocks(string[] memory _stockSymbols) external onlyOwner {
        for (uint256 i = 0; i < _stockSymbols.length; ++i) {
            updateStock(_stockSymbols[i]);
        }
    }

    /**
     * @notice Update Stock Symbol
     * @dev Revert if sender is not the owner , Emit event Of Updated Stock
     * @param _stockSymbol stock name which has to updated for guiven stock ID
     */
    function updateStock(string memory _stockSymbol) internal onlyOwner {
        if (isActiveStockSymbol(_stockSymbol)) {
            revert Game__StockSymbolAlreadyExists(_stockSymbol);
        }
        currentStockId += 1;
        bytes32 _stockId = bytes32(currentStockId);
        stockIdToSymbol[_stockId] = _stockSymbol;
        stockIdToStatus[_stockId] = true;
        stockSymbolToId[_stockSymbol] = _stockId;

        emit StockIdUpdated(_stockId, _stockSymbol);
    }

    /**
     * @notice To check whether a given stock symbol (string) is already present in the mapping
     * @param _stockSymbol stockSymbol you wish to find if, it is already present in the mapping
     */
    function isActiveStockSymbol(string memory _stockSymbol)
        public
        view
        returns (bool)
    {
        // bytes32 stockSymbol = keccak256(abi.encode(_stockSymbol));
        bytes32 id = stockSymbolToId[_stockSymbol];
        if (stockIdToStatus[id]) {
            return true;
        }
        return false;
    }

    function setStockStatus(bytes32 _stockId, bool status)
        public
        onlyOwner
        returns (bool)
    {
        stockIdToStatus[_stockId] = status;
        return true;
    }
}
