...
contract TestStaking3 is IStaking, Ownable, ReentrancyGuard, Pausable {
    ...
    event DebugReward(uint256 amount1, uint256 amount2, uint256 amount3, uint256 amount4);
    ...
    function getCampaignInfo(address stakingToken, address rewardToken) public view returns (uint256, uint256, uint256, uint256, uint256) {
        return (
    ...
    function _calculateRate(address stakingToken, address rewardToken, uint256 rewardAmount, uint256 blockPeriod) internal {
        ...
        if (block.number >= campaign.finishBlock) {
            ...
            uint256 remainingReward = getRemainingReward(stakingToken, rewardToken);
            campaign.rewardAmount = remainingReward.add(rewardAmount);
            emit DebugReward(campaign.stopBlock, campaign.lastRate, remainingReward, rewardAmount);
        }
