    ...
    IERC1155ReceiverUpgradeable
{
    ...
    function createMarketItem(
        ...
        tokenPrice[tokenId][nftContract] = price;
        tokenToId[tokenId][nftContract] = _itemIds;
        ++nftLength[nftContract];

        if (contractType == 0) {
            require(amount == 1, "Cannot transfer multibatch ERC721");
    ...
    function createMarketSale(
        ...
        uint256 price = idToMarketItem[itemId_][nftContract].price;
        uint256 tokenId = idToMarketItem[itemId_][nftContract].tokenId;
        bool sold = idToMarketItem[itemId_][nftContract].sold;
        uint256 amount = idToMarketItem[itemId_][nftContract].amount;
        address sender = _msgSender();
        uint256 totalPrice = price * amount_;
        require(amount_ <= amount, "Not enough amount of token");
        ...
        emit MarketItemSold(itemId_, sender);

        idToMarketItem[itemId_][nftContract].seller.transfer(msg.value);
        if (IERC721Upgradeable(nftContract).ownerOf(tokenId) == address(this)) {
            IERC721Upgradeable(nftContract).transferFrom(
                address(this),
                sender,
                tokenId
            ...
            ++_itemsSold;
            ++nftSold[nftContract];
            idToMarketItem[itemId_][nftContract].owner = payable(sender);
            idToMarketItem[itemId_][nftContract].sold = true;
            tokenPrice[tokenId][nftContract] = 0;
        } else {
            IERC1155Upgradeable(nftContract).safeTransferFrom(
                address(this),
                sender,
    ...
    function handleDelete(address nftContract, uint256 tokenId) public payable {
        ...
        idToMarketItem[itemId][nftContract].sold = false;
