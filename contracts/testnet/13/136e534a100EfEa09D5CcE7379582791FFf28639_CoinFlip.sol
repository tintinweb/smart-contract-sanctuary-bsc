    ...
    function claimRewardPerSession(uint256 _sessionId) external nonReentrant notContract() {
        ...
        require(Bets[msg.sender][_sessionId].choice == _sessions[_sessionId].flipResult , "You didn't win!"); // make sure they won
            ...
            if (_sessions[_sessionId].flipResult == 0) {
                playerWeight = (adjustedBet * accuracyFactor) / (_sessions[_sessionId].headsApple); // ratio of adjusted winner bet amt. / sum of all winning heads bets
            } else if (_sessions[_sessionId].flipResult == 1) {
                playerWeight = (adjustedBet * accuracyFactor) / (_sessions[_sessionId].tailsApple); // ratio of adjusted winner bet amt. / sum of all winning tails bets
            }
