 ...
 */
contract ArkRivalsStaking is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    function getEventInfoById(uint256 eventId) view public returns(uint256, uint256,  uint256, uint256, bool, uint256){
        ...
        return (eventInfoMap[eventId].startTime, eventInfoMap[eventId].endTime, eventInfoMap[eventId].period, eventInfoMap[eventId].maxAllEvent, eventInfoMap[eventId].allowEmergencyWithdraw, eventInfoMap[eventId].numTokenPerStake);
    }
    ...
    function stakeByEventId(uint256 eventId) nonReentrant external returns(uint256){
        require(eventInfoMap[eventId].isExist, "Event Info Not Exist");
        ...
        require(!stakeHolders[eventId][owner].isExist,"User already stake this event");
    ...
    //Unstake By StakeID
    function unstake(uint256 eventId) nonReentrant external {
        ...
        _safeSendNFT(msg.sender, stakeHolders[eventId][msg.sender].rewardNum, stakeHolders[eventId][msg.sender].rewardNum);
    ...
    //Unstake By StakeID
    function unstakeEmergency(uint256 eventId) nonReentrant external {
    ...
    function _safeSendNFT(address _to, uint256 nftid, uint256 amount) internal{
        //TODO: Send eip1155 token to _to
    }
    ...
    function getUserStakeStatus(uint256 eventId)  view public returns(uint256) {
        return getUserStakeStatusCustom(eventId, msg.sender);
    }

    
      //Get StakeInfo by StakeId
    function getUserStakeStatusCustom(uint256 eventId,address _owner)  view public returns(uint256) {
        //TODO:
        if (stakeHolders[eventId][_owner].isExist){
            if (stakeHolders[eventId][_owner].claimed){
                //claimed
                return STATUS_STAKE_CLAIMED;
            }
            if (stakeHolders[eventId][_owner].endTime <= block.timestamp){
                //ongoing
                return STATUS_STAKE_CAN_CLAIM;
            }
            return STATUS_STAKE_ON_GOING;
        }
        return STATUS_STAKE_NOT_STAKE;
    }

    //Common Event Info
    function getAllEventIds() view public returns(uint256){
        //TODO:
    }
