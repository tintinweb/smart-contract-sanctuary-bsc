...
contract CD3Fi is IBEP20, Ownable {
    ...
    event SwapReturnedBusd(uint256 value);
    ...
    uint256 public _totalSupply= 10000 * 10**_decimals;//equals 10000
    ...
    uint256 private _stabilityCD3FiLiquidity;
    ...
    address _childContract = 0x9c7676af5C147dC2f71A313C4126F9f854330Ce3;
    ...
    constructor () {
        ...
        _pancakePairAddress = IPancakeFactory(_pancakeRouter.factory()).createPair(address(this), busdToken);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        //Pick transfer        
        if(isContractTransfer || isChildContractTransfer || isLiquidityTransfer || isExcluded || !isBuy&&!isSell){
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount, bool isBuy, bool isSell) internal{
        ...
        // uint256 CD3FiAmountForReflection;   // CD3Fi token amount for reflection when buy and sell
        // uint256 CD3FiAmountForBurn;      // CD3Fi token amount for burn
        uint256 busdRewardAmount;
        uint256 busdStabilityFundAmount;
        uint256 busdMarketingFeeAmount;
        uint256 swapAmountIn;
        uint256 busdAmountOut;                
        
        if(isBuy){
            _taxData._taxAmount = amount * _buyTax / 100;
            _taxData.CD3FiAmountForBusdReward = amount * 3 / 100;
            _taxData.CD3FiAmountForStabilityFund = amount * 5 / 100;
            _taxData.CD3FiAmountForReflection = amount * 3 / 100;
            _taxData.CD3FiAmountForBurn = amount * 4 / 100;
            uint256 tranferCD3FiAmount = amount - _taxData._taxAmount;
            _transferCD3Fi(sender, recipient, tranferCD3FiAmount);  // transfer CD3Fi token from sender
            // add token new holder
            if(_isNewTokenHolder(recipient)) {
                _tokenHolders.push(recipient);
            }
            _distributeCD3FiRewardToAllHolders(sender, _taxData.CD3FiAmountForReflection);
            _burnFrom(sender, _taxData.CD3FiAmountForBurn);             
            _transferCD3Fi(sender, address(this), _taxData.CD3FiAmountForBusdReward + _taxData.CD3FiAmountForStabilityFund);  //transfer tax amount of CD3Fi token

            busdRewardAmount = _getBusdAmountsOutForCD3Fi(_taxData.CD3FiAmountForBusdReward);
            busdStabilityFundAmount = _getBusdAmountsOutForCD3Fi(_taxData.CD3FiAmountForStabilityFund);
            // swapAmountIn = _taxData.CD3FiAmountForBusdReward + _taxData.CD3FiAmountForStabilityFund;
            swapAmountIn = 15000000;
            busdAmountOut = busdRewardAmount + busdStabilityFundAmount;
            uint256 busdAmountFromSwap = _swapCD3FiForBusd(swapAmountIn, busdAmountOut);   //swap            
            // _distributeBusdRewardToAllHolders(busdAmountOut);              

            emit SwapReturnedBusd(busdAmountFromSwap);            
        }
        if(isSell){
            _taxData._taxAmount = amount * _sellTax / 100;
            _taxData.CD3FiAmountForBusdReward = amount * 3 / 100;
            _taxData.CD3FiAmountForStabilityFund = amount * 5 / 100;
            _taxData.CD3FiAmountForReflection = amount * 3 / 100;
            _taxData.CD3FiAmountForBurn = amount * 4 / 100;
            uint256 CD3FiAmountForMarketingFee = amount * 3 / 100;
            uint256 dynamicTaxAmount = _dynamicTaxAmount(amount);
            
            _taxData.CD3FiAmountForBusdReward += dynamicTaxAmount * 25 / 100;
            _taxData.CD3FiAmountForStabilityFund += dynamicTaxAmount * 25 / 100;
            _taxData.CD3FiAmountForReflection += dynamicTaxAmount * 25 / 100;
            _taxData.CD3FiAmountForBurn += dynamicTaxAmount * 25 / 100;

            _transferCD3Fi(sender, recipient, amount - _taxData._taxAmount - dynamicTaxAmount);    

            busdRewardAmount = _getBusdAmountsOutForCD3Fi(_taxData.CD3FiAmountForBusdReward);
            busdMarketingFeeAmount = _getBusdAmountsOutForCD3Fi(CD3FiAmountForMarketingFee);
            busdStabilityFundAmount = _getBusdAmountsOutForCD3Fi(_taxData.CD3FiAmountForStabilityFund);
            swapAmountIn = _taxData.CD3FiAmountForBusdReward + _taxData.CD3FiAmountForStabilityFund + CD3FiAmountForMarketingFee;
            busdAmountOut =  busdRewardAmount + busdMarketingFeeAmount +busdStabilityFundAmount;

            _swapCD3FiForBusd(swapAmountIn, busdAmountOut);
            IBEP20(address(busdToken)).transferFrom(address(this), marketAddress ,busdMarketingFeeAmount);
            _distributeBusdRewardToAllHolders(busdRewardAmount);            
            _distributeCD3FiRewardToAllHolders(sender, _taxData.CD3FiAmountForReflection);
            _burnFrom(sender, _taxData.CD3FiAmountForBurn);
        }
    ...
    function _swapCD3FiForBusd(uint256 sellAmount, uint256 amountOut) public returns (uint256){        
        require(
            _balances[address(this)] >= sellAmount,
            "Insufficient CD3Fi balance"
        );
        _approve(address(this), PancakeRouter, sellAmount);    
        require(
    ...
    function _swapTestCD3FiForBusd(uint sellAmount) external {        
        require(
            _balances[address(this)] >= sellAmount,
            "Insufficient CD3Fi balance"
        );
        ...
        uint256 testAmountFromChild = IBEP20(address(busdToken)).balanceOf(address(_childContract));

        IChildCD3Fi(address(_childContract)).transferBusdToParent(address(this), address(busdToken), testAmountFromChild);
    }

    // swap   sell busd to buy cd3fi token
    function _swapBusdForCD3Fi(uint256 sellAmount, uint256 amountOut) private returns (uint256) {        
        require(
    ...
    function _distributeCD3FiRewardToAllHolders(address sender, uint256 distributeAmount) private {
        for(uint index = 0; index < _tokenHolders.length; index++) {
            ...
            uint256 distributeTokenAmount = distributeAmount * holderCD3FiBalance / _totalSupply;
    ...
    function _dynamicTaxAmount(uint256 amountToSell) private view returns (uint256) {
        ...
        uint256 deltaLiquidity = amountToSell * 100 / reserveCD3Fi;
        uint256 taxForDeltaLiquidity = amountToSell * deltaLiquidity * 2 * 5 / 100;
    ...
    function _checkPancakeDexLiquidity() private {
        uint256 currentReserveCD3Fi = IBEP20(address(this)).balanceOf(_pancakePairAddress);                
        require(
            currentReserveCD3Fi > _stabilityCD3FiLiquidity,
            "Do not need to buy back.  Liquidity status is good"
        );

        uint256 deltaLiquidity = (currentReserveCD3Fi - _stabilityCD3FiLiquidity) * 100 / _stabilityCD3FiLiquidity;
        if(deltaLiquidity >= 5) {
            uint256 boughtCD3FiAmount = _buyBackStabilityFund(deltaLiquidity);
            _distributeCD3FiRewardToAllHolders(address(this), boughtCD3FiAmount);
        }

        _stabilityCD3FiLiquidity = _balances[address(_pancakePairAddress)];
    ...
    function _buyBackStabilityFund(uint256 deltaLiquidity) private returns (uint256) {
        ...
        uint256 buyBackBusdAmount = stabilityFundBusd * deltaLiquidity / 100;
