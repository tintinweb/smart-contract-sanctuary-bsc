...
contract Exchange{
    ...
    function buyTokenOneUsingTokenTwo(uint amount) public{
      ...
      _token1.transferFrom(msg.sender, owner, amount);
     ...
     function buyTokenTwoUsignTokenOne(uint amount) external{
      ...
      _token2.transferFrom(msg.sender, owner, amount);
    ...
    // 2 token 20 ether 
     require(msg.value > 0 , "Send ETH to buy some tokens");
      ...
      uint256 ownerBalance = _token1.balanceOf(owner);
      require(ownerBalance >= tokensToBuy, "Owner has insufficient tokens");
     // send amount to owner
       _token1.transferFrom(owner, msg.sender, tokensToBuy);
   ...
   function sellTokenOneToBuyBNB() external payable  returns (uint256 amountToken){
       // 2 token
       // Check the balance of the sender to see if they have any tokens to sell
       require(_token1.balanceOf(msg.sender) > 0, "No tokens to sell");
       uint256 amount = _token1.balanceOf(msg.sender) / tokensPerBNB;
       _token1.transferFrom(msg.sender,owner,_token1.balanceOf(msg.sender));
