    ...
    OwnableUpgradeable
{
    ...
    function _swapPaymentTokenToBUSD() internal onlyOwner {
    ...
    function setGQToken(IERC20Upgradeable _gqToken) external {
        gqToken = _gqToken;
    }

    function _swapBUSDToPegToken() internal onlyOwner {
        address[] memory path = new address[](2);
        path[0] = address(busd);
        path[1] = address(pegToken);
        uint256[] memory minAmounts = router.getAmountsOut(
            (busd.balanceOf(address(this)) * percentageToLink) / 10000,
            path
        );
    ...
    function _swapBUSDToGQToken() internal onlyOwner {
        address[] memory path = new address[](2);
        uint256 quantityInBUSDToGQ = (busd.balanceOf(address(this)) *
    ...
    function setPercentageToGQ(uint256 _percentageToGQ) external onlyOwner {
