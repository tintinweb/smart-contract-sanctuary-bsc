...
contract DragonWarShop is PermissionGroupUpgradeable {
    ...
    uint public pricePerHero = 1440;

    function _initialize (
        address _hero,
        address _draw,
        address _treasury) external initializer
    {
        __operatable_init();
        hero = IHero(_hero);
        draw = IERC20(_draw);
    ...
    function chargeToMint() public {
        bool transferToTreasury = draw.transferFrom(msg.sender, treasury, pricePerHero * 10 ** ERC20(address(draw)).decimals());
