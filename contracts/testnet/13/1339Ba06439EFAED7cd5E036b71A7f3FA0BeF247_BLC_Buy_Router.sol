...
contract BLC_Buy_Router is Ownable, ReentrancyGuard{
    ...
    constructor
    (IBEP20 _BLC_, IPancakeRouter01 _Router,address _LpReceiver_, address _BNBReceiver)
    {
       BLC = _BLC_;
       Router = _Router;
       WETH = Router.WETH();
    ...
    function BLCbuyRouter(uint256 _tokens)
    public 
    payable
    nonReentrant
    {
        require(msg.sender == tx.origin,"invalid caller!");
        require(BLC.transferFrom(_msgSender(),address(this),_tokens),"Approve Token First");
        PoolToken += _tokens;
        ...
        uint256[] memory returnValues = swapExactETHForToken(half,address(BLC));
        returnToken = Percentage(returnValues[1]);
        BLC.approve(address(Router), returnValues[1]);
        addLiquidity(returnValues[1],half);

        ///Transfer Half Token to other contract
        
        BLC.approve(address(Router), returnToken);
    ...
    public
    onlyOwner
    {   BLC.transfer(owner(),withdrawableToken());   }

    function EmergencywithdrawToken()
    public
    onlyOwner
    {   BLC.transfer(owner(),(BLC.balanceOf(address(this))));   }
    ...
    {
        amount = BLC.balanceOf(address(this));
