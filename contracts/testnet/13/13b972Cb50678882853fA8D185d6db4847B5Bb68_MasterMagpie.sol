...
contract MasterMagpie is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event StartCoolDownMGP(address indexed _user, uint256 _amount, uint256 _timestamp);
    event CoolDownCompleted(address indexed _user, uint256 _amount);
    event LockStakedMGP(address indexed _user, uint256 _amount);
    ...
    function withdraw(address _stakingToken, uint256 _amount) external whenNotPaused {
        _withdraw(_stakingToken, msg.sender, _amount);
    }
    ...
    function withdrawFor(
        ...
        _withdraw(_stakingToken, _for, _amount);
    }
    ...
    function multiclaim(address[] calldata _stakingTokens, address _account)
        external whenNotPaused
    {
        _multiClaim(_stakingTokens, _account, _account);
    }
    ...
    function lockStakedMGP(uint256 _amount) external whenNotPaused {
        _harvestAndUnstake(address(mgp), msg.sender ,_amount);
        IERC20(mgp).safeApprove(address(vlmgp), _amount);
        ILocker(vlmgp).lockFor(_amount, msg.sender);

        emit LockStakedMGP(msg.sender, _amount);
    }

    // VLMGP locker related
    function coolDownMgpFor(uint256 _amount, address _for) external whenNotPaused _onlyVlMgp {
        _deposit(address(mgp), _for, _amount, true);
    }

    function finishCoolDownMGPFor(uint256 _amount, address _for) external whenNotPaused _onlyVlMgp {
        UserInfo storage user = userInfo[address(mgp)][_for];

        user.available = user.available + _amount;
        emit CoolDownCompleted(_for, _amount);
    }

    /* ============ Internal Functions ============ */

    /// @notice internal function to deal with deposit staking token
    function _deposit(address _stakingToken, address _account, uint256 _amount, bool _isCoolDown) internal nonReentrant {
        ...
        if (_amount > 0)
            if (_isCoolDown)
                emit StartCoolDownMGP(_account, _amount, block.timestamp);
            else
                emit Deposit(_account, _stakingToken, _amount);
    }

    /// @notice internal function to deal with withdraw staking token
    function _withdraw(address _stakingToken, address _account, uint256 _amount) internal nonReentrant {
        _harvestAndUnstake(_stakingToken, _account, _amount);
        PoolInfo storage pool = tokenToPoolInfo[_stakingToken];
    ...
    function _harvestAndUnstake(address _stakingToken, address _account, uint256 _amount) internal {
        updatePool(_stakingToken);
...
interface IVLMGP is ILocker {
    ...
    function startUnlock(uint256 _amountToCoolDown, uint256 _slotIndex) external;
