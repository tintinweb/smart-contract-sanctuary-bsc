...
contract MFCCollateralTreasury is BackendAgent, ExchangeCheck, RegistrarClient, RegistrarMigrator, AdminGovernanceAgent, Governable {
  ...
  struct RefinanceVars {
    uint256 newLoanCollateralMFC;
    uint256 newLoanPrincipal;
    uint256 newLoanId;
    uint256 startAt;
    uint256 endAt;
    uint256 accruedInterest;
    uint256 processingFeeBUSD;
    uint256 refinanceCredit;
    uint256 loanId;
    uint256 mfcPrice;
  }
  ...
  uint256 public constant MINIMUM_REFINANCE_CREDIT = 10000000000000000000; // 10 Ethers
  ...
  struct LoanTerm {
    ...
    uint256 refinanceMinimumCredit;
  }
  ...
  mapping(address => uint256) private _openLoans;
  mapping(address => mapping(uint256 => Offer)) private _offers;
  mapping(address => uint256) private _loansNonce;
  ...
  event MigrateLoan(
    ...
    uint256 startAt,
    uint256 endAt,
    uint256 previousLoanId
  );
  event ExtendLoan(address borrower, uint256 loanId, uint256 endAt, uint256 interestPaid);
  ...
  function getLatestLoan(address borrower) external view returns (Loan memory) {
    return _loans[borrower][_loansNonce[borrower]];
  }

  function getLatestLoanId(address borrower) external view returns (uint256) {
    return _loansNonce[borrower];
  }
  ...
  function transferMigration(uint256 amount) external onlyAdminGovAgents {
    ...
    _transferMFC(_migration, amount);
  }
  ...
  function createLoan(uint256 loanTermId, uint256 busdAmount, uint256 mfcAmount, uint8 v, bytes32 r, bytes32 s) external {
    ...
    _createLoan(busdAmount, minCollateralMFC);
  }

  function _createLoan(uint256 busdAmount, uint256 collateralMFC) private {
    LoanTerm memory loanTerm = _loanTerms[_loanTermsNonce];
    require(busdAmount >= loanTerm.minimumLoanBUSD, "Minimum loan not met");

    uint256 loanId = ++_loansNonce[_msgSender()];
    ...
    _loans[_msgSender()][loanId] = Loan(_loanTermsNonce, collateralMFC, busdAmount, busdAmount, mfcPrice, startAt, endAt, 0);
    ...
    emit CreateLoan(_msgSender(), loanId, _loanTermsNonce, collateralMFC, busdAmount, busdAmount, mfcPrice, startAt, endAt);
  }

  function payLoan(uint256 loanId, uint256 busdAmount) external onlyActiveLoan(_msgSender(), loanId) {
    _checkSufficientBusdAmount(busdAmount);
    _checkActiveOffer(_offers[_msgSender()][loanId].isOpen);
  ...
  function extendLoan(uint256 loanId) external onlyActiveLoan(_msgSender(), loanId) {
    ...
    _checkSufficientBusdAmount(extensionFeeBUSD);
    ...
    emit ExtendLoan(_msgSender(), loanId, loan.endAt, accruedInterest);
  }

  function refinanceLoan(uint256 loanId) external onlyActiveLoan(_msgSender(), loanId) {
    _checkActiveOffer(_offers[_msgSender()][loanId].isOpen);

    Loan storage loan = _loans[_msgSender()][loanId];
    RefinanceVars memory loanData;

    loanData.loanId = loanId;
    loanData.mfcPrice = _mfcExchangeFloor.getPrice();
    loanData.accruedInterest = _accruedInterestMFC(_msgSender(), loanData.loanId); // Calculate accrued interest
    loanData.newLoanCollateralMFC = loan.collateralMFC - loanData.accruedInterest;
    loanData.newLoanPrincipal = loanData.newLoanCollateralMFC * MULTIPLIER / loanData.mfcPrice; // Calculate new loan principal

    // Create new loan
    loanData.newLoanId = ++_loansNonce[_msgSender()];
    loanData.startAt = block.timestamp;
    loanData.endAt = loanData.startAt + _loanTerms[loan.loanTermId].loanDurationInDays * DAY_IN_SECONDS;
    _loans[_msgSender()][loanData.newLoanId] = Loan(
      _loanTermsNonce,                // loanTermId
      loanData.newLoanCollateralMFC,  // collateralMFC
      loanData.newLoanPrincipal,      // originalPrincipalBUSD
      loanData.newLoanPrincipal,      // remainingPrincipalBUSD
      loanData.mfcPrice,              // ltv
      loanData.startAt,               // startAt
      loanData.endAt,                 // endAt
      0);                             // lastPaidAt

    // Close old loan
    uint256 currentPrincipal = loan.remainingPrincipalBUSD;
    loan.collateralMFC = 0;
    loan.remainingPrincipalBUSD = 0;

    // Update totalLoanValue
    if (loanData.newLoanPrincipal > currentPrincipal) {
      _totalLoanValue += loanData.newLoanPrincipal - currentPrincipal;
    }

    // Processing fee
    loanData.processingFeeBUSD = loanData.newLoanPrincipal * _loanTerms[loan.loanTermId].extensionFeePercentage / MULTIPLIER;

    // Refinance credit
    uint256 postFeePrincipal = loanData.newLoanPrincipal - loanData.processingFeeBUSD;
    if (postFeePrincipal >= currentPrincipal) { // Protect from negative refinanceCredit error when postFeePrincipal < currentPrincipal
      loanData.refinanceCredit = postFeePrincipal - currentPrincipal;
    }
    require(loanData.refinanceCredit >= MINIMUM_REFINANCE_CREDIT, "credit must be $10 or higher");

    _busdt.collateralTransfer(_msgSender(), loanData.refinanceCredit);
    _busdt.collateralTransfer(_busdComptrollerAddress, loanData.processingFeeBUSD);
    _transferToExchangeCap(loanData.accruedInterest);

    emit RefinanceLoan(
      _msgSender(),                   // borrower
      loanData.newLoanId,             // loanId
      _loanTermsNonce,                // loanTermId
      loanData.newLoanCollateralMFC,  // collateralMFC
      loanData.newLoanPrincipal,      // originalPrincipalBUSD
      loanData.newLoanPrincipal,      // remainingPrincipalBUSD
      ...
      loanData.startAt,               // startAt
      loanData.endAt,                 // endAt
      loanData.refinanceCredit,       // refinanceCredit
      loanData.loanId,                // previousLoanId
      loanData.accruedInterest        // interestPaid
    );
  }
  ...
  function tradeOffer(address borrower, uint256 loanId, uint256 amountBUSD) external onlyValidMember(_msgSender()) onlyActiveOffer(borrower, loanId) {
    _checkValidQuantity(amountBUSD);
    _checkSufficientBusdAmount(amountBUSD);
    ...
    _checkEnoughAmountToSell(amountBUSD <= info.maxInput);
    ...
    _transferMFC(_msgSender(), info.mfcOut - info.mfcFee);
    ...
    if (calc.excessCollateral > 0) {
      ...
      _transferMFC(borrower, calc.excessCollateral);
    }
  ...
  function _tradeCollateralTransfers(uint256 loanId, CalcOfferRepayment memory loanCalcs, uint256 amountOut, uint256 takerFee) private {
    ...
    if (loanCalcs.excessCollateral > 0) {
      // Return excess MFC to borrower (if any) once loan is repaid in full
      _transferMFC(borrower, loanCalcs.excessCollateral);
    }
  ...
  function migrateLoan(
    ...
    require(startAt > 0, "Previous loan invalid");

    uint256 loanId = ++_loansNonce[borrowerAddress];
    _loans[borrowerAddress][loanId] = Loan(loanTermId, collateralMFC, originalPrincipalBUSD, remainingPrincipalBUSD, ltv, startAt, endAt, lastPaidAt);
    _openLoans[borrowerAddress]++;
    _totalLoanValue += remainingPrincipalBUSD;

    emit MigrateLoan(borrowerAddress, loanId, loanTermId, collateralMFC, originalPrincipalBUSD, remainingPrincipalBUSD, ltv, startAt, endAt, previousLoanId);
  }
  ...
  function _payLoanBUSD(address borrower, uint256 loanId, uint256 busdAmount) private {
    ...
    _totalLoanValue -= busdAmount;
    _transferToExchangeCap(accruedInterest);
    _transferMFC(borrower, collateralReturned);
  ...
  function _tradeCollateralPrerequisite(uint256 loanId, uint256 amountMFC) private view {
    ...
    _checkEnoughAmountToSell(amountMFC <= remainingCollateralMFC);
  }

  function _transferToExchangeCap(uint256 amount) private {
    _transferMFC(_mfcExchangeCapAddress, amount);
  }
  ...
  function _registrarMigrate(uint256 amount) internal override {
    _mfc.registrarMigrateCollateralTreasury(getRegistrarMigrateDestination(), amount);
  }

  function _checkSufficientBusdAmount(uint busdAmount) private view {
  ...
  function _transferMFC(address recipient, uint256 amount) private {
    if (amount > 0) {
      _mfc.transfer(recipient, amount);
    }
  }
...
contract MFCExchangeCap is BackendAgent, ExchangeCheck, RegistrarClient, RegistrarMigrator, AdminGovernanceAgent, Governable {
  ...
  uint256 public constant FLOOR_PRICE_FACTOR = 1030000000000000000; // 1.03 multiplier
  ...
  function createOffer(uint256 quantity) external onlyBackendAgents {
    ...
    uint256 floorPrice = _mfcExchangeFloor.getPrice();
    require(floorPrice > 0, "Floor price must be greater than zero");
    uint256 price = CREATE_PRICE_FACTOR * MULTIPLIER / floorPrice;
  ...
  function _tradeOffer(uint256 quantity, uint256 price) private returns (uint256) {
    ...
    // Add floor price > 0 check to avoid error dividing by 0 floor price from the following cases:
    // 1. BUSDT contract swap making busdBalance 0
    // 2. mfcCirculation = 0 and initial price = 0
    uint256 floorPrice = _mfcExchangeFloor.getPrice();
    if (floorPrice > 0) {
      uint256 limitFloorPrice = FLOOR_PRICE_FACTOR * MULTIPLIER / floorPrice;
      // Ensure offer price is still above floor price to enforce rising floor price rule
      require(price >= limitFloorPrice, "Price must be >= limitFloorPrice");
    }
...
contract MFCToken is MFCBEP20, RegistrarClient, AdminAgent, BackendAgent {
  ...
  address private _mfcCollateralTreasury;
  ...
  /**
   * @dev Intended to be called by current MFCExchangeCap contract. This will transfer MFC
   * from the current MFCExchangeCap contract to the newly deployed replacement
   * MFCExchangeCap contract that will be swapped in afterwards.
   *
   * Important: this must be called before the Registrar is updated with new replacement
   * MFCExchangeCap address (this must be done pre-swap).
   */
  function registrarMigrateExchangeCap(address recipient, uint256 amount) public returns (bool) {
    require(_msgSender() == _mfcExchangeCap, "Caller must be exchange cap");
    return super.transfer(recipient, amount);
  }

  /**
   * @dev Intended to be called by current MFCCollateralTreasury contract. This will transfer MFC
   * from the current MFCCollateralTreasury contract to the newly deployed replacement
   * MFCCollateralTreasury contract that will be swapped in afterwards.
   *
   * Important: this must be called before the Registrar is updated with new replacement
   * MFCCollateralTreasury address (this must be done pre-swap).
   */
  function registrarMigrateCollateralTreasury(address recipient, uint256 amount) public returns (bool) {
    require(_msgSender() == _mfcCollateralTreasury, "Caller must be collateral treasury");
    return super.transfer(recipient, amount);
  }
  ...
  function updateAddresses() public override onlyRegistrar {
    ...
    _mfcCollateralTreasury = _registrar.getMFCCollateralLoan();
  }
...
import "../governance/Governable.sol";
import "./Treasury.sol";
import "../MFCCollateralTreasury.sol";
import "../RegistrarClient.sol";
import "../lib/token/BEP20/IBEP20.sol";
...
contract BUSDT is Treasury, AdminGovernanceAgent, Governable, RegistrarClient {
  ...
  MFCCollateralTreasury private _mfcCollateralLoan;
  ...
  function updateAddresses() public override onlyRegistrar {
    _mfcCollateralLoan = MFCCollateralTreasury(_registrar.getMFCCollateralLoan());
