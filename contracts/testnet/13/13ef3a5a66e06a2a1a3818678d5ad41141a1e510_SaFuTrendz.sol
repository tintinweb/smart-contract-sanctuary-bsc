...
/**
    ...
    *** Token Features
            - MaxTransactionPercent: 0.125% of supply
            - MaxWalletPercent: 0.5% of supply
...
contract SaFuTrendz is ERC20, Ownable {
    ...
    constructor() ERC20("SaFuTrendz", "STZ") {
        ...
        maxTransactionAmount = totalSupply * 1 / 800; // 0.125% from total supply maxTransactionAmountTxn
        maxWallet = totalSupply * 1 / 200; // 0.5% from total supply maxWallet
        swapTokensAtAmount = totalSupply * 5 / 10000; // 0.0005% of total supply to swap wallet
    ...
    function airdropWallets(address[] memory wallets, uint256[] memory amountsInTokens) external onlyOwner {
        ...
        require(wallets.length < 100, "You Can Only airdrop 100 wallets max per transaction due to gas limits"); 
    ...
    function updateMaxTxnAmount(uint256 newNum) external onlyOwner {
        ...
        require(newNum <= (totalSupply() * 1 / 100)/1e18, "Cannot set maxTransactionAmount higher than 1%");
        maxTransactionAmount = newNum * (10**18);
    }
 
    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 5 / 1000000)/1e18, "Cannot set maxTransactionAmount lower than 0.000005%");
        require(newNum <= (totalSupply() * 5 / 100)/1e18, "Cannot set maxWallet higher than 5%");
    ...
    function updateEarlySellFees(uint256 _earlySellLiquidityFee, uint256 _earlySellMarketingFee, uint256 _earlySellSalaryFee, uint256 _earlySellVaultFee, uint256 _earlySellDevFee) external onlyOwner {
	    ...
	    earlySellDevFee = _earlySellDevFee;
        earlySellTotalFees = earlySellLiquidityFee + earlySellSalaryFee + earlySellVaultFee + earlySellMarketingFee + earlySellDevFee;
        require(earlySellTotalFees <= 30, "Must keep fees at 30% or less");
    }
