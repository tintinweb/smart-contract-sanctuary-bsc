...
contract PandoAssemblyV2 is Ownable, IERC721Receiver, Pausable {
    ...
    uint256 public minLevelStaking = 4;
    ...
    modifier onlyMinLevel() {
        require( minLevelStaking <= userLevel.getUserLevel(msg.sender), "NFTStakingPool: caller is not the minimum level requirements");
        _;
    }
    ...
    function buySlot(address to) external whenNotPaused onlyMinLevel {
    ...
    function deposit(uint256[] memory tokenIds, address to) external whenNotPaused onlyMinLevel {
        updatePool();
    ...
    function setMinLevelStaking(uint256 _minLevelStaking) external onlyOwner {
        uint256 old = minLevelStaking;
        minLevelStaking = _minLevelStaking;
        emit MinLevelStakingChanged(old ,_minLevelStaking);
    }
    ...
    event MinLevelStakingChanged(uint256 _old ,uint256 _new);
