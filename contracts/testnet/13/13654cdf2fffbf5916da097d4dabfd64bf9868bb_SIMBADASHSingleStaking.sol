...
// will be transferred to a governance smart contract once SIMBA DASH is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract SIMBADASHSingleStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // Info of each user.
    struct UserInfo {
        uint256 amount;         // How many SIMBA DASH tokens the user has provided.
        uint256 rewardDebt;     // Reward debt. See explanation below.
        uint256 rewardLockedUp;  // Reward locked up.
        uint256 lastDepositTime; // when user deposited

        //
        // We do some fancy math here. Basically, any point in time, the amount of SIMBA DASH
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTokenPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws SIMBA DASH tokens to a pool. Here's what happens:
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint;         // How many allocation points assigned to this pool. SIMBA DASH to distribute per block.
        uint256 lastRewardBlock;    // Last block number that SIMBA DASH distribution occurs.
        uint256 accTokenPerShare;   // Accumulated SIMBA DASH per share, times 1e9. See below.
    ...
    // The SIMBA DASH TOKEN!
    IBEP20 public token;
    // SIMBA DASH tokens created per block.
    uint256 public tokenPerBlock;
    // Info of each pool.
    PoolInfo[] public poolInfo;
    // Info of each user that stakes SIMBA DASH tokens.
    mapping(uint256 => mapping(address => UserInfo)) public userInfo;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when SIMBA DASH mining starts.
    ...
    constructor() {
        token = IBEP20(0x6F2ce39363ba9DC3eb54Ce39B8cb4EbcDE483fdD);
    ...
    // Update the given pool's SIMBA DASH allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending SIMBA DASH on frontend.
    function pendingToken(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (myBlock > pool.lastRewardBlock && lpSupply != 0 ) {
            ...
            accTokenPerShare = accTokenPerShare.add(tokenReward.mul(1e9).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accTokenPerShare).div(1e9).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest SIMBA DASH.
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accTokenPerShare = pool.accTokenPerShare.add(tokenReward.mul(1e9).div(lpSupply));
        pool.lastRewardBlock = myBlock;
    }

    // Deposit SIMBA DASH tokens to Farm for SIMBA DASH allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e9);
    ...
    function harvest(uint256 _pid) external nonReentrant {
        ...
        uint256 pending = user.amount.mul(pool.accTokenPerShare).div(1e9).sub(user.rewardDebt);
    ...
    // Withdraw SIMBA DASH tokens from Farm.
    ...
    // Pay or lockup pending SIMBA DASH.
    ...
    function setEarlyWithdrawFee(uint256 _earlyFeePercent)  external onlyOwner {
        require(_earlyFeePercent <= 30, "Max is 30 percent");
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough SIMBA DASH.
