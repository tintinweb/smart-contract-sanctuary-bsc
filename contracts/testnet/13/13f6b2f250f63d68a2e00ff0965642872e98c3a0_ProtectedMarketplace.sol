...
contract ProtectedMarketplace {
    ...
    enum OfferStatus { Active, UnderDownsideProtectionPhase, Completed, Cancelled }
    ...
    modifier onlyBuyer(uint256 _offerId) {
        ...
        require(msg.sender == offer.buyerAddress, "Invalid Authentication");
        _;
    }
    ...
    function claimDownsideProtectionAmount(uint256[] memory _orderIds) external {        //Although receiver not a contract, adding reentrant guard for extra protection
        for ( uint256 i = 0; i < _orderIds.length; i ++ ) {
            uint256 _orderId = _orderIds[i];
            Order storage order = orders[_orderId];
    ...
    function createOffer(
        ...
        offer.isFixedProtection = _isFixedProtection;
    ...
    function acceptOffer(
        ...
        require(offer.tokenPrice == _tokenPrice && _tokenPrice > 0, "token price");
        require(offer.protectionRate == _protectionRate && offer.protectionTime == _protectionTime, "Invalid protection rate or time");
        require(IERC721(_nftAddress).ownerOf(_tokenId) == msg.sender, "Invalid token owner");
        
        offer.statusOffer = OfferStatus.UnderDownsideProtectionPhase;
        offer.sellerAddress = payable(msg.sender);

        uint256 companyShare = _tokenPrice * companyFeeRate / 10000;
        company.transfer(companyShare);                        // Transfer the company fees
        
        uint256 netAmount = _tokenPrice - companyShare;
        ...
        IERC721(_nftAddress).safeTransferFrom(msg.sender, offer.buyerAddress, _tokenId);     // Transfer the NFT
        offer.tokenAddress = _nftAddress;
        offer.nftTokenId = _tokenId;
        offer.protectionAmount = downsideAmount;
        offer.depositId = depositId;
        offer.soldTime = block.timestamp;

        uint256 protectionExpiryTime = offer.isFixedProtection ? offer.protectionTime : (offer.soldTime + offer.protectionTime);
        emit AcceptOffer(_offerId, offer.sellerAddress, offer.protectionAmount, protectionExpiryTime);
    }

    function claimDownsideProtectionAmountInOffer(uint256 _offerId) external {
        Offer storage offer = offers[_offerId];
        require(offer.statusOffer == OfferStatus.UnderDownsideProtectionPhase, "Invalid OfferStatus");   

        uint256 protectionExpiryTime = offer.isFixedProtection ? offer.protectionTime : (offer.soldTime + offer.protectionTime);
        // Fetch the token amount worth the face value of protection amount
        if(msg.sender == offer.sellerAddress 
            && block.timestamp > protectionExpiryTime
            && offer.soldTime != 0)
        {
    ...
    function cancelOffer(uint256 _offerId) external onlyBuyer(_offerId) {
        Offer storage offer = offers[_offerId];
        require(offer.statusOffer == OfferStatus.Active, "Invalid OfferStatus");
