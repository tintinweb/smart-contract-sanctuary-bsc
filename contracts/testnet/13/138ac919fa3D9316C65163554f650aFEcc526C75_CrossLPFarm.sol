...
contract CrossLPFarm is Context, ReentrancyGuard, ICrossFarm {
    ...
    mapping(uint256 => bool) public autocompoundingEnabled;
    ...
    function add(
        ...
        autocompoundingEnabled[index] = _autocompoundingEnabled;
    ...
    function set(
        ...
        autocompoundingEnabled[_pid] = _autocompoundingEnabled;
        ...
        returns (uint256)
    {
        ...
        return
            (((user.amount * accERC20PerShare) / 1e36 - user.rewardDebt) *
                3333) / 10000;
    }
        ...
        returns (uint256 reward)
    {
        ...
        if (user.isCompounding == true || user.amount > 0) {
            ...
            reward = ((userPercentage * compoundRewards) * 3333) / (10**22);
            return reward;
        } else return 0;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (nonCompoundReward > 0) {
            pool.accERC20PerShare =
                pool.accERC20PerShare +
                (nonCompoundReward * 1e36) /
                nonCompoundedLpTokens;
        }
    ...
    function compoundWithdraw(uint256 _pid, uint256 _amount) private {
        ...
        if (_amount > 0) {
            ...
            if (
                ...
                pool.lpToken.transfer(contractAddresses[5], taxAdjusted);
                pool.lpToken.transfer(
                    address(msg.sender),
                    _amount - taxAdjusted
                );
    ...
    function changeOwnerAddress(address _address) external {
        require(msg.sender == owner() || msg.sender == adminSetter);
        transferOwnership(_address);
    }

    function changeAdminSetter(address _address) external {
        require(msg.sender == adminSetter);
