...
contract Pool1 is Ownable {
    ...
    uint256 public interval = 2 hours; // 结算时间间隔
    IToken public token;
    ...
    function init(address _token, address _pair) public onlyOwner {
        token = IToken(_token);        
        lastBurnTime = token.getLastBurnTime();
        interval = token.getInterval();
    ...
    function _calculatedValue(address account) internal virtual returns (uint256 amount, uint256 token0Amount, uint256 token1Amount){
        ...
        //emit LOG(account, self_LP, string(abi.encodePacked("self_LP: ", Strings.toString(self_LP))));
        ...
        uint256 r0 = 0;
        r0 = r0.add(_reserve0);
        //emit LOG(account, r0, string(abi.encodePacked("DAO r0: ", Strings.toString(r0))));

        uint256 r1 = 0;
        r1 = r1.add(_reserve1);
        //emit LOG(account, r1, string(abi.encodePacked("USDT r1: ", Strings.toString(r1))));

        // 计算锁仓质押价值是否足够 1000u
        uint256 price = r1.div(r0);
        // token0 单价
        //emit LOG(account, price, string(abi.encodePacked("DAO price: ", Strings.toString(price))));

        uint256 self_p = self_LP.mul(1000).div(_Total_LP);
        // LP 占比
        //emit LOG(account, self_p, string(abi.encodePacked("self_p: ", Strings.toString(self_p))));

        uint256 self_a0 = r0.mul(self_p).div(1000).mul(price).div(10 ** 18);
        //  token0 总价值
        //emit LOG(account, self_a0, string(abi.encodePacked("DAO SUM self_a0: ", Strings.toString(self_a0))));

        uint256 self_a1 = r1.mul(self_p).div(1000).div(10 ** 18);
        // USDT 总价值
        //emit LOG(account, self_a1, string(abi.encodePacked("USDT SUM self_a1: ", Strings.toString(self_a0))));

        uint256 sum_lp = self_a0.add(self_a1);
        //emit LOG(account, sum_lp, string(abi.encodePacked("DAO + USDT SUM sum_lp: ", Strings.toString(sum_lp))));
    ...
    function queryRequiredQuantity() public returns (uint256, bool) {
        return _queryRequiredQuantity();
    }
