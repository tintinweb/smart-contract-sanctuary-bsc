...
contract Manager is Initializable, AccessControlUpgradeable, PausableUpgradeable, ERC721HolderUpgradeable {
...
contract MarketDex is Manager, ReentrancyGuardUpgradeable {
	...
	function createOrder(
		...
		require(IERC721Upgradeable(_tokenAddress).ownerOf(_tokenId) == msg.sender || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not-item-owner");
	...
	function cancelBid(uint256 _bidId) external whenNotPaused nonReentrant returns (uint256) {
		...
		if (bid.paymentToken == address(0)) {
			...
			IERC20Upgradeable(bid.paymentToken).safeTransfer(bid.bidder, payBackAmount);
		}
