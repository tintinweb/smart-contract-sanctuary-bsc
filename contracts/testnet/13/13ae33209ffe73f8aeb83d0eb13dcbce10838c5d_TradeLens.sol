...
contract TradeLens {
    function getLeverageOrderIds(IOrderManagerForTradeLens _orderbook, address _owner, bool _isMarket)
        public
        view
        returns (uint256[] memory, uint256)
    {
        ...
        for (uint256 i = totalOrder; i > 0; i--) {
            ...
            bool validType = _isMarket ? order.expiresAt != 0 : order.expiresAt == 0;
            if (!validType) {
                continue;
            }
    ...
    function getLeverageOrders(
        ...
        (uint256[] memory orderIds, uint256 totalOrder) = getLeverageOrderIds(_orderbook, _owner, _isMarket);
    ...
    function getAllLeverageOrders(IOrderManagerForTradeLens _orderbook, address _owner, bool _isMarket)
        external
        view
        ...
        returns (LeverageOrderForLens memory)
    {
        ...
        leverageOrder.expired = order.expiresAt < block.timestamp;
