...
contract RUSDFraction is ERC20Detailed, Ownable {
    ...
    function setRewardContract(IRUSDRewards _rusdRewardContract) public onlyOwner {
    ...
    function _presaleTransfer(
        ...
        _fracBalances[_presaleContract] = _fracBalances[_presaleContract].sub(
            fracValue
        );
        _fracBalances[_to] = _fracBalances[_to].add(fracValue);

        emit Transfer(_presaleContract, _to, _value);
    }
    ...
    function transfer(address to, uint256 value)
        ...
        returns (bool)
    {
        if (msg.sender == presaleContract || to==presaleContract) {
    ...
    function transferFrom(
        ...
        if (from == presaleContract || to==presaleContract) {
