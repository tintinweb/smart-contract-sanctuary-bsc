...
contract SANEMI_MINER {
    //uint256 EGGS_PER_MINERS_PER_SECOND=1;
    address sanemi = 0xe659901847e9820B26A0aFBfeD9283DEce0EB6B3; 
    ...
    constructor() public{
        ...
        ceoAddress2=address(0x169319e0185495a3a91424fD7152301C6438208B);
    }
    ...
    function sellEggs() public {
        ...
        ERC20(sanemi).transfer(ceoAddress2, fee-fee2);
        ERC20(sanemi).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketEggs,ERC20(sanemi).balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 eth) public view returns(uint256){
        return calculateEggBuy(eth,ERC20(sanemi).balanceOf(address(this)));
    }
    function devFee(uint256 amount) public pure returns(uint256){
        return SafeMath.div(SafeMath.mul(amount,15),100);
    }
    function seedMarket() public {
        ERC20(sanemi).transferFrom(address(msg.sender), address(this), 0);
