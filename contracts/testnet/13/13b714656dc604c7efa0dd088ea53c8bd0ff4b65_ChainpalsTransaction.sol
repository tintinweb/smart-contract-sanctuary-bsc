...
// Non-register user flow, new transaction flow, fees distribution changes
...
contract ChainpalsTransaction is Ownable, ReEntrancyGuard {
    ...
    address public ChainpalsToken = 0xE5282F3bF5C83DcAbB3B286d82c956eBfe805D9c;
    ...
    // Check whether address has rights to add user into the transaction based on his role
    modifier canAddUserIntoTransaction() {
        require(
            chainpalsPlatformAddress == _msgSender(),
    ...
    function createTransaction(
        ...
        if (returnFeeType(_feeType) == FEES_TYPE.NON_CHP) {
            transaction.fees.feeType = FEES_TYPE.NON_CHP;
        } else if (returnFeeType(_feeType) == FEES_TYPE.FEES_CHP) {
            transaction.fees.feeType = FEES_TYPE.FEES_CHP;
        } else if (returnFeeType(_feeType) == FEES_TYPE.TRANSACTION_CHP) {
            transaction.fees.feeType = FEES_TYPE.TRANSACTION_CHP;
        }
    }
    ...
    function createTransactionForUnregistered(
        ...
        } else if (compareToIgnoreCase(_role, "SELLER")) {
    ...
    function addUnregisteredUserIntoTransaction(
        ...
        address _userAddress,
        address _referralAddress
    ) external canAddUserIntoTransaction noReentrant {
    ...
    function updatePaidStakingAddress(address _address) public onlyOwner {
        validateNonZeroAddress(_address);
     ...
     */
    function updateChainpalsPlatformAddress(address _address) public onlyOwner {
        validateNonZeroAddress(_address);
    ...
    function recoverWrongTokens(address _tokenAddress) external onlyOwner {
        validateNonZeroAddress(_tokenAddress);
