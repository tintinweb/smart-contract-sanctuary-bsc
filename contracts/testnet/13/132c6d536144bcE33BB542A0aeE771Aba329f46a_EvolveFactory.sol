...
contract EvolveFactory is Context, Ownable, EvolveAccessControll, IEvolveFactory {
    ...
    function concatenateArraysWithReplaceAddress(address[] memory _Accounts, address[] memory _Accounts2 , bool[] memory _replaceAddressList, address _replaceAddress) private pure returns(address[] memory) {
        address[] memory returnArr = new address[](_Accounts.length + _Accounts2.length);

        uint i=0;
        for (;i < _Accounts.length; i++) {
            bool _needReplace =  _replaceAddressList[i];
            returnArr[i] = _needReplace == true ? _replaceAddress : _Accounts[i];
        }

        uint j=0;
        while (j < _Accounts2.length) {
            bool _needReplace =  _replaceAddressList[i++];
            returnArr[i++] =_needReplace == true ?  _replaceAddress :  _Accounts2[j++];
        }
