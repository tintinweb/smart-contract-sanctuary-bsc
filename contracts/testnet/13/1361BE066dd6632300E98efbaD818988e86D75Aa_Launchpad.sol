...
contract Launchpad is AccessControl {
    ...
    function getSwapAToBAmount(uint256 _amount) public view returns (uint256) {
        return _amount.div(rate).mul(1000);
    }

    function getFeeRate(uint256 _amount) public view returns (uint256) {
        return _amount.mul(feerate).div(100);
    }

    function swap(uint256 _amount) public {
        require(_amount > 100000000000000000, "amount to small");
        uint256 _payback = getSwapAToBAmount(_amount);
        uint256 _payfee = getFeeRate(_amount);
    ...
    function setFeeTo(address _feeTo) public onlyRole(ADMIN_ROLE) {
        feeTo = _feeTo;
    }
    ...
    function ownerReclaimA() public onlyRole(MERCHANT_ROLE) {
        tokenA.transfer(msg.sender, tokenA.balanceOf(address(this)));
    }

    function ownerReclaimB() public onlyRole(MERCHANT_ROLE) {
        tokenB.transfer(msg.sender, tokenB.balanceOf(address(this)));
    }

    function transferB(address _buyer, uint256 _amount)
        public
    ...
    function flushBNB() public onlyRole(ADMIN_ROLE) {
