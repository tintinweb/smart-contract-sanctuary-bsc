   ...
   /**
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (_msgSender() != uniswapPair) {
            ...
            // No fees for sending to contract wallets
            if (!excludedFromFees[_msgSender()] && excludedFromFees[recipient]) {
                _transfer(_msgSender(), recipient, amount);
                return true;
            }

            // No fees for sending airdrops or any amount to investor wallets
            if (excludedFromFees[_msgSender()] && !excludedFromFees[recipient]) {
                _transfer(_msgSender(), recipient, amount);
                return true;
            }

            // No Fees between dev wallets
            if (excludedFromFees[_msgSender()] && excludedFromFees[recipient]) {
                _transfer(_msgSender(), recipient, amount);
                return true;
            }
     ...
     * @dev Sends BNB to transaction
     */
    function disperseTokensManually() public onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));
        sendFeesToWallets(contractTokenBalance);
    }
