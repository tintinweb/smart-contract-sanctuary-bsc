...
contract Referral  is Ownable{
    ...
    function ref(address _parent,address _user) public onlyOwner {
        if(isBind(_user) || !isBind(_parent)){   //上级也必须绑定邀请人
            return;
        }
...
contract Fpt is ERC20, Ownable {
    ...
    uint256 public deadFee = 40;
    uint256 public referralFee1 = 0;
    uint256 public referralFee2 = 0;
    uint256 public referralFee3 = 0;
    uint256 public referralFee4 = 0;
    uint256 public referralFee5 = 0;
    uint256 public referralFee6 = 0;
    uint256 public referralFee7 = 0;
    uint256 public marketingFee = 20;
    uint256 public liquidityFee = 0;
    uint256 public lpRewardFee = 0;
    IERC20 public usdt = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);  //0x55d398326f99059fF775485246999027B3197955
    ...
    constructor(
        ...
        address rWalletAddress_,
        uint256 swapStartTime_
    ) payable ERC20(name_, symbol_)  {
        uint256 totalSupply = totalSupply_ * (10**18);
        maxHold = 5000 * (10**18);//最大持币量
        effiHold = 100 * (10**18);//有效地址
        ...
        rWalletAddress = rWalletAddress_;
        ...
        excludeFromFees(rWalletAddress, true);
    ...
    function _transfer(
        ...
        bool isSwapBuy = from == uniswapV2Pair;  //卖需要手续费，买不要   买
        ...
        if(takeFee) {
            if(to == uniswapV2Pair){
                amount =  takeAllFee(from, amount, uniswapV2Pair);
            }else{
                amount =  takeAllFee(from, amount, marketingWalletAddress);
            }
        }
    ...
    function takeAllFee(address from,uint256 amount,address _parent) private returns(uint256 amountAfter) {
        amountAfter = amount;

        //销毁
        if(_parent == marketingWalletAddress){
            uint256 DFee = amount.mul(deadFee).div(100);
            if(DFee > 0){
                uint256 isDeadAmount = balanceOf(deadWallet);
                if (isDeadAmount.add(DFee) > maxDeadAmount) {
                    if(isDeadAmount>maxDeadAmount){
                        DFee = 0;
                    }else{
                        DFee = maxDeadAmount.sub(isDeadAmount);
                    }
                }
