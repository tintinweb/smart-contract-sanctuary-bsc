...
contract ShadowFiNFTStaking is ERC721Holder, Ownable  {
    ...
    mapping (address => mapping (address => uint256)) private userTotalRedeemed;
    ...
    function redeem(uint256 _pid) public {
        ...
        uint256 pending = user.amount.mul(pool.accTokenPerShare).sub(user.rewardDebt);
        if(pending > 0) {
            address redeemedToken = address(IERC20(poolInfo[_pid].rewardsToken));
            userTotalRedeemed[msg.sender][redeemedToken] += pending;
            safeRewardTransfer(_pid, msg.sender, pending);
        }
    ...
    function checkUserTotalRedeemed(address user, address rewardsToken) public view returns (uint256 totalRedeemed) {
        return userTotalRedeemed[user][rewardsToken];
    }
