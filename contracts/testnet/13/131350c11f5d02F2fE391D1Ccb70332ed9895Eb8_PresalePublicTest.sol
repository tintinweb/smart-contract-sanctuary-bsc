    ...
    IPresalePublic
{
    ...
    function initialize(
        ...
        intermediate.closeTimeVoting = block.timestamp + 10 minutes;
    ...
    /** @dev Funtion to invest (send paymentToken and buy sale tokens)
        ...
        initialized
    {
        ...
        require(
            (currentTime > generalInfo.closeTime &&
                ...
                (intermediate.votes.yes <
                    intermediate.votes.no + votingParams.threshold &&
                    currentTime > intermediate.closeTimeVoting),
            "WRONG PARAMS"
        );
    ...
    function getStage() public view returns (uint8) {
        ...
        if (
            ...
            currentTime >= generalInfo.openTime + 20 minutes &&
            currentTime < generalInfo.openTime + 25 minutes
        ) return 2;
        else if (
            currentTime >= generalInfo.openTime + 25 minutes &&
            currentTime < generalInfo.openTime + 30 minutes
        ) return 3;
        else return 4;
    }
    ...
    function getRound() public view returns (uint8) {
        ...
        if (
            ...
            currentTime >= generalInfo.openTime + 35 minutes &&
            currentTime < generalInfo.closeTime
        ) {
            //PAPER ROUND
            return 2;
    ...
    {
        moonEnd = generalInfo.openTime + 5 minutes < generalInfo.closeTime
            ? generalInfo.openTime + 5 minutes
            : generalInfo.closeTime;
        diamondEnd = generalInfo.openTime + 30 minutes < generalInfo.closeTime
            ? generalInfo.openTime + 30 minutes
            : generalInfo.closeTime;
        paperEnd = generalInfo.closeTime;
        if (diamondEnd < generalInfo.closeTime) {
            stagesEnd = [
                generalInfo.openTime + 15 minutes,
                generalInfo.openTime + 20 minutes,
                generalInfo.openTime + 25 minutes,
                diamondEnd
            ];
        } else {
            stagesEnd = [diamondEnd, diamondEnd, diamondEnd, diamondEnd];
        }
    }

    /** @dev Get current num of break (if 0, it's not break).
     */
    function getBreakNumber() external view returns (uint8) {
        uint256 currentTime = block.timestamp;
        if (
            currentTime >= generalInfo.openTime + 5 minutes &&
            currentTime < generalInfo.openTime + 10 minutes
        ...
        if (_round == 0) {
            ...
            for (i = 1; i < 4; i++) {
                ...
                if (n == 0) {
                    ...
                    roundTokenAllocation[_round][i] =
                        (intermediate.eachPoolShare * POOL_WEIGHT[i - 1] * n) /
                        (DENOMINATOR);
                }
            }
