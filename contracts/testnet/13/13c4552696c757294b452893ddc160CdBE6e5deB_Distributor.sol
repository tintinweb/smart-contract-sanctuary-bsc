...
contract Distributor is Ownable, Initializable {
    ...
    function addDistributionWallet( address[] memory account, uint[] memory amount) external onlyOwner {
        require(account.length < 30,"Distributor : length < 30");
        require(account.length == amount.length,"Distributor : length mismatch");

        for(uint i=0; i< account.length; i++) {
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "Distributor : insufficient balance to allocate");
