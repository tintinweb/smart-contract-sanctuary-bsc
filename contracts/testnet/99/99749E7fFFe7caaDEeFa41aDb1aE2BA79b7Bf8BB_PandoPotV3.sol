...
//    uint256 public currentMinorNumber2;
    ...
    function _calculateNumbers(uint _number1, uint _number2, uint _number3, PRIZE_TYPE _type, uint _quantity, EnumerableSet.UintSet storage _set) internal{
        ...
        for(uint i = 0; i < _quantity; i++) {
            uint _value = uint256(keccak256(abi.encodePacked(_rand, i))) % typePrize[_type].digit;
    ...
    function _checkAndTransferPan(uint256 _quantity) internal{
        ...
        uint256 _amount_PAN = getAmountPan(msg.sender, _quantity);
        if(_amount_PAN > 0){
    ...
    function getAmountPan(address user, uint256 _quantityTicket) public view returns(uint256) {
    ...
    function buyTickets(address _receiver, uint256[] memory _tickets, uint256 _roundID) external availableBuyTicket(_receiver, _roundID) onlyEOA whenNotPaused nonReentrant{
        ...
        for (uint256 i = 0; i < _tickets.length; i++) {
            uint256 _ticketNumber = _tickets[i] % typePrize[PRIZE_TYPE.MEGA].digit;
...
//        require(_type < 3, 'PandoPot: invalid type');
    ...
    function getRoundInfo(uint _roundId) public view returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256 finishedAt, uint256 status) {
