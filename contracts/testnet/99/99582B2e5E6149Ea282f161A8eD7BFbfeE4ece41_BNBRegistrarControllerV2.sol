...
contract BNBRegistrarControllerV2 is Ownable, EIP712 {
    ...
    function register(uint name_len, 
      ...
      registerWithConfig(name_len, label, owner, duration, address(0), address(0), signature);
    }
	...
	require(
            _verify(
                _hash(labelStr, owner, duration, resolver, addr),
                signature
            ),
            "Invalid signature"
        );

	bytes32 label = stringToBytes32(labelStr); 
	...
	IPriceOracle.Price memory price = rentPrice(name_len, label, duration);
        ...
        emit NameRegistered(labelStr, label, owner, cost, expires);
    }
    ...
    function _hash(
        ...
        return
            _hashTypedDataV4(
        		...
        		abi.encode(
        		    keccak256(
        			"SID(string label,address owner,uint duration,address resolver,address addr)"
        			),
			    label,
			    owner,
			    duration,
