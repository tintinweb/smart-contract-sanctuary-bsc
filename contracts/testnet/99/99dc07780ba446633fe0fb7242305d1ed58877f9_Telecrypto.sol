...
contract Telecrypto {
    ...
    event UpdateIERC20(address indexed from, IERC20 tokenaddr);
    ...
    constructor() {
        ...
        Dollar = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);
    ...
    function buyTicket(
        ...
        Dollar.transferFrom(msg.sender, WalletDev, ShareDev);
        Dollar.transferFrom(msg.sender, WalletPool, SharePool);
    ...
    function updateIERC20(IERC20 _tokenaddr) external {
        require(msg.sender == Owner, "Only Owner can update Address!");
        Dollar = _tokenaddr;

        emit UpdateIERC20(msg.sender, _tokenaddr);
    }
    ...
    function getPoolBalance(uint256 _nrgame) public view returns (uint256) {
        return Dollar.balanceOf(address(games[_nrgame].wallet));
    }

    function getGamePrize(uint256 _nrgame) public view returns (uint256) {
        uint256 poolBalance = Dollar.balanceOf(address(games[_nrgame].wallet));
    ...
    function withdraw(address _tokenaddr, uint256 _amount) public {
        ...
        RandToken = IERC20(_tokenaddr);
        RandToken.transfer(msg.sender, _amount);
