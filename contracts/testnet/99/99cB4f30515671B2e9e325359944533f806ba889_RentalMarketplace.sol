...
// contract Marketplace is KeeperCompatibleInterface, Ownable, Pausable, IRentalMarket {
contract RentalMarketplace is Ownable, Pausable, IRentalMarket {
    ...
    uint256 public test_uint;

    // From ERC721 registry assetId to Order (to avoid asset collision)
    mapping(address => mapping(uint256 => rentalOrder)) public orderByAssetId;
    ...
    function expiredUser(address _nftAddress, uint256 _assetId) public returns (bool) {
        // rentalOrder memory order = _getValidOrder(_nftAddress, _assetId);
        rentalOrder memory order = orderByAssetId[_nftAddress][_assetId];
        bool timePassed = (order.expiresAt >= block.timestamp);

        if(order.user == address(0) || timePassed == false){
            orderByAssetId[_nftAddress][_assetId] = rentalOrder({
                id: order.id,
                owner: order.owner,
                rNFTAddress: _nftAddress,
                assetId: _assetId,
                user: address(0),
                expiresAt: 0,
    ...
    // // automatically expire user
    // function checkUpkeep(bytes memory)
    //     public
    //     returns (bool needsUpkeep, bytes memory)
    // {
    //     bool timePassed = (expireData >= block.timestamp);
    //     needsUpkeep = (timePassed);
    // }

    // //burns token once condition is met
    // function performUpkeep(bytes calldata, address _nftAddress, uint256 _assetId) external {
    //     rentalOrder memory order = _getValidOrder(_nftAddress, _assetId);
    //     (bool needsUpkeep, ) = checkUpkeep("", order.expiresAt);
    //     require(needsUpkeep == true, "Upkeep not needed.");
    //     order.user = address(0);
    //     order.expiresAt = 0;
    // }
     ...
     * @param _assetId - ID of the published NFT
     */
    function cancelRentalOrder(address _nftAddress, uint256 _assetId)
        public
        whenNotPaused
    {
        rentalOrder memory order = orderByAssetId[_nftAddress][_assetId];
        require(
            order.owner == msg.sender || msg.sender == owner(),
            "Marketplace: unauthorized sender"
        );

        require(order.user == address(0), "RentalMarketplace: This nft is renting.");
    ...
    //     require(order.owner == msg.sender, "Marketplace: sender not allowed");
    ...
    function ExecuteRentalOrder(
        ...
        rentalOrder memory order = _getValidOrder(_nftAddress, _assetId);
        require(order.user == address(0), "Marketplace: rented asset");
        address user;
        ...
        // require(order.daddr != address(0) || order.daddr == msg.sender, "Your address is disabled" );

        uint256 restDay = (_expiresAt - block.timestamp)/3600/24;
        require(restDay > 0, "Marketplace: expire time less than now time");
        uint256 rentalPrice = restDay*_price;

        uint256 saleShareAmount = rentalPrice.mul(buyFeePerMillion).div(1e6);
        uint256 total_price = rentalPrice + saleShareAmount;
        uint256 approved_amount = IERC20(order.acceptedToken).allowance(msg.sender, address(this));

        test_uint = approved_amount;

        require(approved_amount >= total_price, "The approved amount should be more than rental price.");
        bool success_transform = IERC20(order.acceptedToken).transferFrom(msg.sender, address(this), total_price);
        require(success_transform == true, "You should pay total price");

        bool success_transform_rent = IERC20(order.acceptedToken).transfer(order.owner, rentalPrice);
        require(success_transform_rent == true, "You should pay price");
    ...
    */
    ...
    function _getValidOrder(address _nftAddress, uint256 _assetId)
        internal
        view
        returns (rentalOrder memory order)
    {
    ...
    function _executeRentalOrder(
        ...
        // Transfer NFT asset
        // IERC721(_nftAddress).transferFrom(address(this), _user, _assetId);
    ...
    function _createOrder(
        ...
        // uint256 expireDate = _expiresAt + block.timestamp;
        ...
        // require(
        //     expireDate > block.timestamp.add(1 minutes),
    ...
    function _cancelRentalOrder(
        ...
        IERC721(_nftAddress).transferFrom(address(this), _owner, _assetId);
...
interface IRentalMarket {
    struct rentalOrder {
        bytes32 id;         //Order id
        address owner;      //original owner of nft
        address rNFTAddress;//nft for rent
        uint256 assetId;    //asset id of rental nft
        address user;       //renter
        uint256 expiresAt;  //expire time
        address acceptedToken; //token for trading
        uint256 price;      //diarly price
        address daddr;      //direct sale
    }
    ...
    event rentalOrderCreacted (
        bytes32 id,
        address indexed owner,
        address indexed rNFTAddress,
