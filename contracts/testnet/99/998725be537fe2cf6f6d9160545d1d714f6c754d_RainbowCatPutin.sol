...
contract RainbowCatPutin is Ownable, ERC20 {
	using SafeMath for uint256;

	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name = "Rainbow Cat Putin";
	string private constant _symbol = "RCP";
	uint8 private constant _decimals = 18;

	RainbowCatPutinDividendTracker public dividendTracker;
...
//	address public dividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //BUSD
    address public dividendToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; //Testnet BUSD
	...
	constructor() ERC20(_name, _symbol) {
		dividendTracker = new RainbowCatPutinDividendTracker();
//		dividendTracker.setUniswapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
		dividendTracker.setUniswapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //Testnet Router
...
//		IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // Mainnet
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "RainbowCatPutin: The dividend tracker already has that address");
		RainbowCatPutinDividendTracker newDividendTracker = RainbowCatPutinDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "RainbowCatPutin: The new dividend tracker must be owned by the RainbowCatPutin token contract");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "RainbowCatPutin: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "RainbowCatPutin: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "RainbowCatPutin: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "RainbowCatPutin: Account is already the value of 'excluded'");
		_isExcludedFromMaxWalletLimit[account] = excluded;
		emit ExcludeFromMaxWalletChange(account, excluded);
	}
	function setWallets(address newLiquidityWallet, address newMarketingWallet) public onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "RainbowCatPutin: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "RainbowCatPutin: The marketingWallet cannot be 0");
	...
	function setUniswapRouter(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "RainbowCatPutin: The router already has that address");
	...
	function setGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue != gasForProcessing, "RainbowCatPutin: Cannot update gasForProcessing to same value");
	...
	function setMaxTransactionAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxTxAmount, "RainbowCatPutin: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxWalletAmount, "RainbowCatPutin: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) public onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "RainbowCatPutin: Cannot update minimumTokensBeforeSwap to same value");
	...
	function setDividendToken(address newDividendToken) external onlyOwner {
		require(newDividendToken != dividendToken, "RainbowCatPutin: Cannot update dividend token to same value");
		require(newDividendToken != address(0), "RainbowCatPutin: The dividend token cannot be 0");
		require(newDividendToken != address(this), "RainbowCatPutin: The dividend token cannot be set to the current contract");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "RainbowCatPutin: Cannot send more than contract balance");
	...
	function _transfer(
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "RainbowCatPutin: Trading is currently disabled.");
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "RainbowCatPutin: Buy amount exceeds the maxTxBuyAmount.");
			}
			if (!_isExcludedFromMaxWalletLimit[to]) {
				require(balanceOf(to).add(amount) <= maxWalletAmount, "RainbowCatPutin: Expected wallet amount exceeds the maxWalletAmount.");
			}
		}
...
contract RainbowCatPutinDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("RainbowCatPutin_Dividend_Tracker", "RainbowCatPutin_Dividend_Tracker") {
		claimWait = 3600;
	...
	function _transfer(address, address, uint256) internal override {
		require(false, "RainbowCatPutin_Dividend_Tracker: No transfers allowed");
	}
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "RainbowCatPutin_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "RainbowCatPutin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "RainbowCatPutin_Dividend_Tracker: Cannot update claimWait to same value");
