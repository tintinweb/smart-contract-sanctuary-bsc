...
contract NONLaunchpadProxy is ILaunchpadProxy, Ownable, ReentrancyGuard {
    ...
    bytes4 internal constant PROXY_ID =
        bytes4(keccak256("NONLaunchpadProxyV12"));
...
library LaunchpadBuy {
    ...
    function callLaunchpadBuy(
        ...
        require(proxyCallData.length > 0, LaunchpadProxyEnums.LPD_ROUNDS_ABI_NOT_FOUND);
    ...
    function checkLaunchpadBuy(
        ...
        if (lpadRounds.buyToken != address(0)) {
            ...
            ) return LaunchpadProxyEnums.LPD_ROUNDS_PAYMENT_ALLOWANCE_NOT_ENOUGH;

            // pay value not need
            if (msg.value > 0) return LaunchpadProxyEnums.LPD_ROUNDS_PAY_VALUE_NOT_NEED;
        } else {
            // balance check
    ...
    function _updateAccountStats(
        ...
        require(!lpad.lockParam, LaunchpadProxyEnums.LPD_PARAM_LOCKED); // only when not locked
