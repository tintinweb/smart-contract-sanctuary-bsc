...
contract CentralBank is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable{
  uint256 private INFLAT_MULTIPLIER;
  ...
  modifier notInflatPaused(){
    require(inflatPaused == false, "Inflat paused");
    _;
  }

  /* ========== ADMIN ========== */
  function toggleInflat() external onlyOwner {
    inflatPaused = !inflatPaused;
  }
  ...
  function initialize(address _usdb,address _usds, address _router, uint256 _startAt) public initializer{
    require(!initialized, "initialized");
    usdb = _usdb;
    usds = _usds;
    router = _router;
    periodDuration = 60*10; //10 minutes
    halvingDuration = 10*60*60; // 60 minutes
    lastUpdateTime = block.timestamp;
    INFLAT_MULTIPLIER = 100000000;
    ORACLE_MULTIPLIER = 10000;
  ...
  function _inflat(uint256 _nonce) private notInflatPaused{
    require(initialized, "initialize not ready");
    ...
    address rewardToken;
    if(price<98){
      // price of usdb < 1, 200W usds reward
      rewardToken = usds;
      inflatAmount = 2000000*1e18;
      rewardAmount = inflatAmount;
    }else{
      uint256 toBankRate = abs(price, 105) * 5;
      toBankRate = Math.min(Math.max(toBankRate, 50), 80);

      rewardToken = usdb;
      inflatAmount  = IUSDB(usdb).totalSupply() * period.rate / INFLAT_MULTIPLIER;
      rewardAmount = inflatAmount - inflatAmount*toBankRate/100;
    }
  ...
  function _cumulatePrice() private view returns (uint256) {
    ...
    for (uint256 i = 0; i < oracleTokens.length; i++) {
      if(oracleTokens[i].token != address(0)){
        ...
        try IUniswapOracle(oracle).consult(usdb,1e18,token1) returns(uint amountOut){
          price = price + amountOut*100 * oracleTokens[i].multiplier;
        }catch{
          (uint112 reserve0, uint112 reserve1,) = pair.getReserves();
          if(usdbIsToken0){
            price = reserve1*100e18/reserve0;
          }else{
            price = reserve0*100e18/reserve1;
          }
        }
  ...
  function burn(address token, uint256 amount) public onlyOwner {
    if(token == usdb || token == usds){
      IUSDB(token).burn(amount);
    }else{
      IUSDB(token).transfer(address(0), amount);
    }

    emit Burn(token, amount);
  }

  function swap(uint amountIn,uint amountOutMin,address[] calldata path,uint deadline) public onlyOwner{
    IUSDB(path[0]).approve(router, amountIn);
