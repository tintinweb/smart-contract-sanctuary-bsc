	...
	ReentrancyGuardUpgradeable
{
	...
	// The CAKE TOKEN!
	IERC20Upgradeable public syrup;
	...
	// Minimum amount for points to apply.
	uint256 public minAmountRequirements;

	// Points per minAmountRequirements.
	uint256 public pointsPerAmount;
	...
	event Deposit(address indexed user, uint256 amount, uint256 points);
	event Withdraw(address indexed user, uint256 amount, uint256 points);
	event EmergencyWithdraw(address indexed user, uint256 amount);

	// Initializer for upgradeability
	function initialize(
		IERC20Upgradeable _syrup,
		IERC20Upgradeable _rewardToken,
		uint256 _allocPoint,
		uint256 _rewardPerBlock,
		uint256 _startBlock,
		uint256 _bonusEndBlock,
		uint256 _minAmountRequirements,
		uint256 _pointsPerAmount
	) public initializer {
		...
		minAmountRequirements = _minAmountRequirements;
		pointsPerAmount = _pointsPerAmount;
	...
	function currentPoints(address _user) external view returns (uint256) {
	...
	function updatePool(uint256 _pid) public {
		...
		//if staking has ended
		if (block.number <= pool.lastRewardBlock) {
			return;
		}
		uint256 lpSupply = totalStaked;
		// no rewards, marked this as ended
		if (lpSupply == 0) {
	...
	function deposit(uint256 _amount) public whenNotPaused nonReentrant {
		...
		user.points = computePoints(user.amount);
		user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e12);

		emit Deposit(msg.sender, _amount, user.points);
	}
	...
	function withdraw(uint256 _amount) public whenNotPaused nonReentrant {
		...
		user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e12);
		user.points = computePoints(user.amount);
		emit Withdraw(msg.sender, _amount, user.points);
	}
	...
	function computePoints(uint256 _amount) private view returns (uint256) {
		require(
			minAmountRequirements > 0 && pointsPerAmount > 0,
			"Err: Config points"
		);
		return _amount.mul(pointsPerAmount).div(minAmountRequirements);
	}

	function updatePointsAmount(uint256 _amount)
		external
		onlyOwner
		returns (uint256)
	{
		require(_amount > 0, "Amount error");
		pointsPerAmount = _amount;
		return pointsPerAmount;
	}

	function updateminAmountReqs(uint256 _amount)
		external
		onlyOwner
		returns (uint256)
	{
		require(_amount > 0, "Amount error");
		minAmountRequirements = _amount;
		return minAmountRequirements;
	}
