...
contract saleteste is IBEP20, ReentrancyGuard, ERC721Holder, Context, Ownable {
    ...
    // Pet rewards
    uint256 public PetRate = (20 * 10 ** _decimals) /  25 days;
    uint256 public PetRate2 = (35 * 10 ** _decimals) /  35 days;
    uint256 public PetRate3 = (55 * 10 ** _decimals) /  50 days;
    uint256 public PetRate4 = (100 * 10 ** _decimals) /  30 days;
    ...
    uint256 public LifeRate = (100 * 10 ** _decimals) /  30 days;


    mapping(address => uint) public lockTime; // lock pet level 1
    mapping(address => uint) public lockTime2; // lock pet level 2
    mapping(address => uint) public lockTime3; // lock pet level 3
    mapping(address => uint) public lockTime4; // lock pet level 4
    ...
    function buyGems(uint256 _value) external{
        require(_value > 0 && _value <= balanceOf(msg.sender).div(10 ** _decimals), "Token amount must be greater than zero and less than or equal to your balance.");
        _transfer(msg.sender, address(this), _value.mul(10 ** _decimals));
        Gem[msg.sender] += _value.mul(10); // 1 token = 10 gem
    }
    ...
    function feedPet(uint256 tokenId, uint256 _value) external{
         ...
         life[tokenId] += _value.div(10); // 10 gem = 1 life
    }
    ...
    function gameRewards(uint256 tokenId, uint256 cargo) external{
        ...
        if (cargo == 1) {
           ...
           _transfer(address(this), msg.sender, calculateTokens(tokenId));
            life[tokenId] -= petLifeRate(tokenId);
     
        } else if (cargo == 2) {
           require(tokenOwnerOf2[tokenId] == msg.sender, "You can't unstake");

          _transfer(address(this), msg.sender, calculateTokens2(tokenId) * level[tokenId]);
           life[tokenId] -= 10;

        } else if (cargo == 3) {
           require(tokenOwnerOf3[tokenId] == msg.sender, "You can't unstake");

          _transfer(address(this), msg.sender, calculateTokens3(tokenId) * level[tokenId]);
          life[tokenId] -= 15; 

        } else if (cargo == 4) {
           require(tokenOwnerOf4[tokenId] == msg.sender, "You can't unstake");

          _transfer(address(this), msg.sender, calculateTokens4(tokenId) * level[tokenId]);
          life[tokenId] -= 25;
        }
