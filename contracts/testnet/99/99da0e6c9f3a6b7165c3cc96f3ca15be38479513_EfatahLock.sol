...
contract EfatahLock is IERC20 {
    ...
    function withdrawCoin() external payable onlyAdmin {
        require(balanceOf(address(this)) > 0, "No Units in balance");

        _ADMIN_ADDR.transfer(balanceOf(address(this)));
    }

    function withdrawToken(address payable token_addr)
        external
        payable
        onlyAdmin
    {
        ...
        require(token.balanceOf(address(this)) >= 0, "No Units in balance");

        token.transfer(_ADMIN_ADDR, token.balanceOf(address(this)));
    }
...
/*
    ...
    {
        return ( MAX_SUPPLY, DECIMALS,_TAX_PCENT,_TRF_PCENT,_MAX_DAILY_TRANSFER_PCENT,_ADMIN_ADDR,balanceOf(address(this))); 
    }
*/
...
//     }
/* 
    function isValidAddress(address addr) public view returns (uint256){
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) external override validAddress(recipient) returns (bool) {
