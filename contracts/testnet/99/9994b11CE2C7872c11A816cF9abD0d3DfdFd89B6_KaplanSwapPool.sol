...
contract KaplanSwapPool is AccessControl {
    ...
    bool public withdrawingEnabled;
    ...
    constructor(address server) {
        ...
        _setRoleAdmin(SERVER, ADMIN);
        
        _grantRole(ADMIN, msg.sender);
        _grantRole(SERVER, server);

        depositingEnabled = true;
        withdrawingEnabled = true;
    }

    function deposit(IERC20 tokenToDeposit, uint256 amount) public {
        require(hasRole(ADMIN, msg.sender) || hasRole(SERVER, msg.sender), "Only administrators and authorized servers are allowed to deposit tokens");
    ...
    function withdraw(IERC20 tokenToWithdraw, address receiver, uint256 amount) external {
        require(hasRole(ADMIN, msg.sender) || hasRole(SERVER, msg.sender) || hasRole(SWAPCONTRACT, msg.sender), "Only administrators, authorized servers and authorized swap contracts are allowed to withdraw token from swap pool");
    ...
    function setWithdrawingEnabled(bool enabled) public {
        require(hasRole(ADMIN, msg.sender), "Only administrators are authorized to enable or disable withdrawing");
