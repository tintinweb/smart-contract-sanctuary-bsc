...
interface IDutch {
        function getPrice(uint price, uint discountRate, uint startingAt) external view returns(uint);
        function getPriceReverse(uint price, uint discountRate, uint startingAt) external view returns (uint);
    }

contract Marketplace is AccessControl, ReentrancyGuard, MarketPlaceEvents {
    ...
    MarketplaceItem[] public idToMarketplaceItem; //change it to private after done 
    

    constructor(address _froyoContract, address _dutch) payable {
        owner = payable(msg.sender);
        froyoContract = _froyoContract;
        dutchContract = _dutch;
    ...
    //checkinng of whitelist nft contract
    ...
    function CreateItemFixedPriceSales(
        ...
        uint256 isFroyo,
        uint256 _duration
    ) external payable isWhitelisted(nftContract) nonReentrant {
    ...
    function CreateItemDutchSales(
        ...
        require(price >= _discountRate * numberOfdays, "Starting price is too low");
    ...
    function CreateItemReverseDutchSales(
        address nftContract,
        uint256 tokenId,
        uint256 price,
        ...
        require(price >= _increaseRate * numberOfdays, "Starting price is too low");
    ...
    function getPriceDutchSales(uint _itemId) public view returns (uint) {
        bool dutchSale;
        bool isReversed;
        uint discount;
        uint startSale;
        ...
        if (
                idToMarketplaceItem[_itemId - 1].salesProperty.salesType == 3
           ) 
            {
            isReversed = true;
            discount = idToMarketplaceItem[_itemId -1].salesProperty.discountRate;
            startSale = idToMarketplaceItem[_itemId - 1].salesProperty.startAt;
            startingPrice = idToMarketplaceItem[_itemId -1].price;
            }
        else if (
                    idToMarketplaceItem[_itemId - 1].salesProperty.salesType == 2
                )
                {
                    isReversed = false;
                    dutchSale = true;
        ...
        if (isReversed == true && dutchSale == false){
            return IDutch(dutchContract).getPriceReverse(startingPrice, discount, startSale);
        }

        else if (isReversed == false && dutchSale == true) {
            return IDutch(dutchContract).getPrice(startingPrice, discount, startSale);
        }
        else {
            revert("The item is not a dutch sales OR reverse dutch sales!");
        }    
    ...
    function BuyNftWithFroyo(address nftContract, uint256 itemId)
        public
        payable
        nonReentrant
    {
        ...
        require(itemId != 0, "items cannot start with zero");
        require(idToMarketplaceItem[itemId - 1].sold == false, "The item is already sold!");
        require(
            currentBalance >= idToMarketplaceItem[itemId - 1].froyoPrice,
        ...
        IERC20(froyoContract).transferFrom(msg.sender,seller,price); 
    ...
    function BuyNftFixedPriceSales(address _nftContract , uint _itemId) 
        public 
        payable 
        nonReentrant 
    {
        require(_itemId != 0, "items cannot start with zero");
    ...
    function BuyNftDutchSales(address _nftContract , uint _itemId) 
        public 
        payable 
        nonReentrant 
    {
        ...
        uint256 price = getPriceDutchSales(_itemId);
        uint256 tokenId = idToMarketplaceItem[_itemId - 1].tokenId;
        uint256 expiredSale = idToMarketplaceItem[_itemId - 1].salesProperty.expiresAt;
        uint256 salestyp = idToMarketplaceItem[_itemId - 1].salesProperty.salesType;
        require(idToMarketplaceItem[_itemId - 1].sold == false, "The item is already sold!");
        require(salestyp == 2, "This item is not a dutch sales!");
        require(block.timestamp < expiredSale, "This sales has ended");
        require(
            msg.value >= price,
            "Please submit the asking price in order to complete the purchase"
    ...
    function BuyNftReverseDutchSales(address _nftContract , uint _itemId) 
        public 
        payable 
        nonReentrant 
    {
        ...
        uint256 salestyp = idToMarketplaceItem[_itemId - 1].salesProperty.salesType;
        require(idToMarketplaceItem[_itemId - 1].sold == false, "The item is already sold!");
        require(salestyp == 3, "This item is not a reverse dutch sales!");
        require(block.timestamp < expiredSale, "This sales has ended");
        require(
            msg.value >= price,
            "Please submit the asking price in order to complete the purchase"
    ...
    function ChangeNftPriceFixedSales(uint _newprice, uint _itemid) public {
    ...
    function ChangeNftSalesDuration(uint _salesDuration, uint _itemid) public payable {
    ...
    function removeItemFromMarket(uint _itemId) public nonReentrant {
        require(idToMarketplaceItem[_itemId - 1].seller == msg.sender, "You are not the owner!");
        address NFTcontract;
        uint token_id; 
    ...
    function sellStatus(uint _itemId) public view returns(bool) {
    ...
    function GrantRole(address _account) public onlyRole(DEFAULT_ADMIN_ROLE){
        // This is a function to make a wallet address to be able to mint shadew nft. 
