...
contract MetaAxPresale is Ownable {
  ...
  */
  function requestPreStake(uint256 amount, uint lockPeriod, address[] memory recommender) external returns(uint256) {
    uint256 depositBnb;
    // Check pre-stake validation
    ...
    require(recommender.length < _agent.length, "The number of referals has been exceeded!");
    for(uint i = 0; i < recommender.length; i ++)
    {
      uint256 bonus = additionalBnb.mul(_agent[i].bnbRewardPercent).div(100);
      tokenForPayment.transfer(recommender[i], bonus);
      agentBonus = agentBonus.add(bonus);
      metaAx.transfer(recommender[i], stakedToken.mul(_agent[i].mtaxRewardPercent).div(100));
    }
