...
contract BettingChinese is ReentrancyGuard, Ownable, BetTokenWrapper {
    ...
    function bet(
        ...
        if (predictResult_ == games[gameId_].creatorInitResult) {
            ...
            games[gameId_].counterpartPoolCap =
                (games[gameId_].odd / _ODD_RATE_DENOMINATOR) *
                games[gameId_]
    ...
    function claim(uint256 gameId_) public nonReentrant {
        ...
        if (
            ...
            claimAmount += creatorPoolLoserRemainCalculator(
                gameId_,
                totalWinnerBetting,
                totalLoserBetting
            );
        ...
        )
    {
        ...
        if (games[gameId_].result == games[gameId_].creatorInitResult) {
            ...
            award =
                (winnerbettingAmount_ * _ODD_RATE_DENOMINATOR) /
                games[gameId_].odd;
        }
    ...
    function creatorPoolLoserRemainCalculator(
        ...
        //incase both pools reach the cap
        if (
            (totalLoserBetting_ * games[gameId_].odd) / _ODD_RATE_DENOMINATOR ==
            totalWinnerBetting_
        ) {
            return creatorPoolLoserRemainClaimAmount;
        }

        //incase counterpart doesn't reach the cap
        ...
        uint256 totalCreatorPoolLoserRemain = totalLoserBetting_ -
            (totalWinnerBetting_ * _ODD_RATE_DENOMINATOR) /
            games[gameId_].odd;
