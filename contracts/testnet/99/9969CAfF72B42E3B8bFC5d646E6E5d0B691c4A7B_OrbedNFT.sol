...
contract OrbedNFT is Initializable, ERC721Upgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  * @param tokensMinted - Total number of tokens minted.
  * @param message - Message details of the event.
  */
  event MintCompleted(address indexed toAddress, uint256 tokensMinted, string message);
  ...
  */
    ...
    onlyRole(TRANSACTION_ROLE)  
  {
    ...
    for (uint256 i = 0; i < tokenIds.length - 1; i++)
    {
      ...
      if (result){
        // Mint the token
        _mintInternal(to, tokenIds[i]);
        tokensMinted++;
      }
      else {
        //Mint had to stop here, break and return.
        message = "Could not mint any other tokens as minting conditions were not met.";
        break;
      }
    }
    ...
    emit MintCompleted(to, tokensMinted, message);
  }
