...
interface ILockStakingRewards {
    ...
    function stakeLocks(address user, uint stakeNonce)
        external
        returns (uint256);

    function stakeAmounts(address user, uint stakeNonce)
        external
        returns (uint256);
    ...
    OwnableUpgradeable
{
    ...
    function burnNFT(uint256 tokenId) external {
        ...
        _burn(tokenId);
        uint256 amountToSend = (purchaseRecords[tokenId].amountBusd *
            (100 + bonusRate)) / 100;
     ...
     */
    function supplyBusd(uint256 tokenId, uint256 busdToSend)
        external
        onlyAllowedUpdaters
    {
     ...
     * Can be executed only by allowed updaters
     */
    function confirmOrder(uint256 tokenId, uint256 amountToSwap)
        external
        onlyAllowedUpdaters
    {
        require(ownerOf(tokenId) != address(0), "Token is burned");
     ...
     */
    function updateAllowedUpdater(address updater, bool isActive)
        external
        onlyOwner
    {
     ...
     */
    function getAmountsAndRates(uint256 tokenId)
        public
        view
        returns (
            uint256 BnbNburate,
        ...
        )
    {
        ...
        BusdNbuRate = (BnbBusdRate * 1 ether) / BnbNburate;
    }
     ...
     */
    function getTokenReserves(uint tokenId)
        public
        view
        returns (uint256 amountNbu, uint256 amountBusd)
    {
    ...
    /**
     * @notice View method to get tokens information
     * @param from - start of tokens indexes to get information
     * @param to - end of tokens indexes
     * @dev This method is to get informations about array of NFTs
     * Returns rates, reserves and information whether token was burned
     */
    function getListAmountsRates(uint256 from, uint256 to)
        public
        view
        returns (Rates[] memory)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (ownerOf(tokenId) == address(0)) rates.isBurned = true;
            RatesList[i] = rates;
        }
        return RatesList;
    }
