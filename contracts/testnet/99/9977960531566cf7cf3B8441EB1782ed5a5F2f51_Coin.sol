...
contract Coin is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        uint256 supply,
        address[] memory addrs, // 0-owner,1-service,2-autoLiquidity,3-marketing,4-router,5-dividendTracker,6-reardToken
    ...
    function swapAndLiquify() private swapping {
        ...
        uint256 liquidityTokens = contractTokenBalance.mul(liquidityShare).div(totalDistributionShares);
        if (liquidityTokens > 0) {
            swapAndAddLiquify(liquidityTokens);
        }
        uint256 marketingTokens = contractTokenBalance.mul(marketingShare).div(totalDistributionShares);
        if (marketingTokens > 0) {
            swapAndSendReward(marketingTokens);
        }
        uint256 dividendTokens = contractTokenBalance.sub(liquidityTokens).sub(marketingTokens);
        if (dividendTokens > 0) {
    ...
    function swapAndSendReward(uint256 tokens) private  {
        ...
        IERC20(rewardToken).transfer(marketingReceiver, newBalance);
    }
    ...
    function swapAndAddLiquify(uint256 tokens) private {
       // split the contract balance into halves
        ...
        uint256 initialBalance = IERC20(USD).balanceOf(address(usdStore));

        // swap tokens for USD
        swapTokensForUsd(half); // <- this breaks the USD -> TOKEN swap when swap+liquify is triggered

        // how much USD did we just swap into?
        uint256 newBalance = (IERC20(USD).balanceOf(address(usdStore))).sub(initialBalance);
        IERC20(USD).transferFrom(address(usdStore), address(this), newBalance);
        // add liquidity to uniswap
        addLiquidityUsd(otherHalf, newBalance);
    }
