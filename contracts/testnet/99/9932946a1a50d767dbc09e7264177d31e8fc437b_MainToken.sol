...
contract MainToken is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint32 public _limitedCount;
    mapping ( uint32 => LimitedAddress ) private _Limited;
    mapping (address => uint32[]) private _addressLimited;
    event SetLimited(uint32 , LimitedAddress);
   ...
   function CreateLimited(address wallet, uint256 amount,uint endDate) onlyOwner() public{
         
     _Limited[_limitedCount] = LimitedAddress(wallet,amount,endDate);
     _addressLimited[wallet].push(_limitedCount);

     emit SetLimited(_limitedCount,_Limited[_limitedCount]);

     _limitedCount++;
    }

    function GetLimited(address wallet) private view returns(uint256){
      uint256 Amount = 0;

      for  (uint i=0 ; i < _addressLimited[wallet].length;i++)
      {
            uint32 limitedId = _addressLimited[wallet][i];
      
           if(_Limited[limitedId].EndDate >= block.timestamp){
               Amount += _Limited[limitedId].Amount;
           }
      }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require((balanceOf(sender) - GetLimited(sender)) > amount,"Your account is limited.");
