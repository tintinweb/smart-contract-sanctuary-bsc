...
interface IMarketplace {
    ...
    function buy(uint256 _nftId, uint256 _amount, uint256 _refCode) external payable;
...
contract Marketplace is IMarketplace, Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    modifier validRefCode(uint256 _refCode) {
        require(_refCode >= 999, "Invalid ref code");
        _;
    }
    ...
    function setDiscountPercent(uint8 _discount) public override onlyOwner {
        require(_discount > 0 && _discount < 100, "Invalid discount value");
        discount = _discount;
    }

    function setCommissionPercent(uint8 _percent) public override onlyOwner {
        require(_percent > 0 && _percent < 100, "Invalid commission value");
    ...
    function buy(
        ...
        uint256 _amount,
        uint256 _refCode
    ) public payable override validNft(_nftId) validAmount(_amount) validRefCode(_refCode) {
        ...
        require(
            ...
            "Not enough balance to buy the NFT"
        );
        require(
            AnotherNFT(nft).balanceOf(address(this), _nftId) >= _amount,
            "No enough NFT amount to buy"
        );
        require(
        ...
        require(refAddress != address(0), "Caller must have a referral address");
    ...
    function sell(
        ...
        require(result == true, "Cannot send commission to receiver");
    ...
    function lockedReferralDataForAccount(address _user) public override {
        require(lockedReferralData[_user] == false, "User's referral data already locked");
    ...
    function setCurrencyAddress(address _currency) public override onlyOwner {
        require(_currency != address(0), "Must be not address zero");
        require(_currency != currency, "Must be different currency address");
        currency = _currency;
    }
    ...
    function deposit() public payable override {
        ...
        bool result = ERC20(token).transfer(address(this), msg.value);
        require(result == true, "Cannot send commission to receiver");
    }
    ...
    function withdraw(uint256 _amount) public override onlyOwner {
        ...
        bool result = ERC20(token).transfer(msg.sender, _amount);
        require(result == true, "Cannot send commission to receiver");
    }
