...
import "../library/BEP20BurnableFeeableUpgradeable.sol";

contract ABIGTURTLE is Initializable, BEP20BurnableFeeableUpgradeable {
    uint8 private constant _decimals = 8;
    string private constant _name = "ABIGTURTLE";
    string private constant _symbol = "TURT";
    ...
    function initialize() public initializer {
        __BEP20BurnableFeeable_init(_name, _symbol);
    }
...
// Updated to work with taxable BEP20 tokens
...
// Forked from OpenZeppelin to work with taxable BEP20 tokens
...
abstract contract BEP20Upgradeable is
    Initializable,
    OwnableUpgradeable,
    ERC20Upgradeable,
    IBEP20Additions,
    ...
    UUPSUpgradeable
{
    function __BEP20Upgradeable_init(string memory name_, string memory symbol_)
        internal
        onlyInitializing
    {
...
abstract contract BEP20FeeableUpgradeable is Initializable, BEP20Upgradeable {
    ...
    function __BEP20Feeable_init(string memory name_, string memory symbol_)
        internal
        onlyInitializing
    {
        __BEP20Upgradeable_init(name_, symbol_);
    ...
    function setLiquidity(address newLiquidity) external onlyOwner {
        require(
            ...
            "BEP20BurnableFeeable: liquidity address cannot be the same"
        );
...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./BEP20FeeableUpgradeable.sol";
import "./ERC20BurnableUpgradeable.sol";

abstract contract BEP20BurnableFeeableUpgradeable is
    Initializable,
    BEP20FeeableUpgradeable,
    ERC20BurnableUpgradeable
{
    function __BEP20BurnableFeeable_init(
        string memory name_,
        string memory symbol_
    ) internal onlyInitializing {
        __BEP20Feeable_init(name_, symbol_);
    ...
    function _beforeTokenTransfer(
        ...
        if (_shouldTakeFee(from, to)) {
            ...
            require(
                ...
                "BEP20BurnableFeeable: transfer amount with fees exceeds balance"
            );
            // mining fees
            _balances[from] = _balances[from] - miningFee;
            _balances[owner()] = _balances[owner()] + miningFee;
            emit Transfer(from, owner(), miningFee);
            // liquidity fees
            _balances[from] = _balances[from] - liquidityFee;
            _balances[liquidity] = _balances[liquidity] + liquidityFee;
            emit Transfer(from, liquidity, miningFee);
            // burn
            _burn(from, burnFee);
