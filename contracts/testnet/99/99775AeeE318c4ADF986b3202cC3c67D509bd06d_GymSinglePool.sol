...
contract GymSinglePool is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint256 public totalGGymnetInPoolUnlocked;
    ...
    function setMLMQualificationsAddressAddress(address _address) external onlyOwner {
    ...
    function depositFromOtherContract(
        ...
        bool isUnlocked,
        address _from
    ) external onlyWhitelistedContract {
        require(isPoolActive, "Contract is not running yet");
        if (isUnlocked) {
            _periodId = 0;
        }
    ...
    */
    function _deposit(
        ...
        uint256 UsdValueOfGym = ((amountToDeposit * IGYMNETWORK(tokenAddress).getGYMNETPrice()) /
            1e18) / 1e18;
        uint256 _ggymnetAmt = (amountToDeposit * ggymnetAlloc[_periodId]) / 1e18 / 1e18;
        ...
        if (_isUnlocked) {
            ...
            totalGGymnetInPoolUnlocked += _ggymnetAmt;
    ...
    */
    function _withdraw(uint256 _depositId) private {
        ...
        if (depositDetails.stakePeriod == 0) {
            ...
            totalGGymnetInPoolUnlocked -= depositDetails.ggymnetAmt;
        }
    ...
    */
    ...
    function pendingReward(uint256 _depositId, address _user) public view returns (uint256) {
        ...
        uint256 sharesTotal = totalGGymnetInPoolLocked - totalGGymnetInPoolUnlocked;
    ...
    */
    function transferFromOldVersion(
        ...
        address _from,
        uint256 avgPrice
    ) public onlyWhitelistedContract {
        UserInfo storage user = userInfo[_from];
        IERC20Upgradeable token = IERC20Upgradeable(tokenAddress);
        PoolInfo storage pool = poolInfo;
        token.approve(address(this), _depositAmount);
        ...
        uint256 burnTokensAmount = 0;
        uint256 amountToDeposit = _depositAmount - burnTokensAmount;
        uint256 UsdValueOfGym = (amountToDeposit  / 1e18) * avgPrice;
        uint256 _ggymnetAmt = (amountToDeposit * ggymnetAlloc[_periodId]) / 1e18 / 1e18;
