...
contract PreSaleReward is Ownable,ReentrancyGuard {
    ...
    uint256 public withdrawStartTime = 1660049108; // 开始提取时间
    uint256 public withdrawEndTime = 1691585107; // 结束提取时间
    ...
    function withdrawal() external {
        ...
        _withdrawAmountMapping[msg.sender][month] = happyCats._shareMapping(msg.sender) * _cashBack/tcdPrice; // 增加已领取数量
        IERC20(happyCats.paymentCurrencyMapping(0)).transfer(msg.sender, happyCats._shareMapping(msg.sender) * _cashBack/tcdPrice); // 发放
    }

    function addWithdrawStartTime (uint256 _timestamp) external {
        withdrawStartTime += _timestamp;
    }
    ...
    function awardInfo(address addr) external view returns (uint256 pastAmount, uint256 pendingAmount, uint256 totalAmount) {
        ...
        uint8 month = DateTime.getMonth(block.timestamp);
        pendingAmount = _withdrawAmountMapping[addr][month] == 0 ? happyCats._shareMapping(addr) * _cashBack / tcdPrice : 0;
