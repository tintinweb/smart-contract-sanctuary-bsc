...
contract Utopia is  Ownable {
    ...
    uint256[4] public mintPrices = [100 ether, 200 ether, 300 ether, 400 ether];
    uint256[4] public mintLadder = [10000, 30000, 40000 ,100000];

    mapping(address => uint256[7]) public referral_grade; //直推等级统计
    mapping(address=>address) public referrer;//推荐人
    mapping(address=>uint8) public vip;//个人vip级别
    uint8[7] public grade_requirements = [0,2,2,2,2,2,2];
    uint8[8] public dividend_ratio = [5,20,10,10,5,5,3,2];
    ...
    uint8[8] public rewardLadder = [5,25,35,45,50,55,58,60];//奖励阶梯
    uint256 public totalBonus;//总奖金
    uint256 public eachBonus;//每份奖金
    ...
    //获取加入金额
    function getMintPrices() public returns(uint256) {
        uint256 totalNftCount = utopiaNFT.totalSupply();
        uint256 price;
        if(totalNftCount <= mintLadder[0]){
            price = mintPrices[0];
        }else if(totalNftCount <= mintLadder[1]){
            price = mintPrices[1];
        }else if(totalNftCount <= mintLadder[2]){
            price = mintPrices[2];
        }else if(totalNftCount <= mintLadder[3]){
            price = mintPrices[3];
        }else{
            price = 0;
        }
    ...
    function setRecommender(address recommender) public {
        require(recommender != msg.sender, "Recommender should not be same with caller.");
        require(recommender == owner() || joined[recommender] == true, "Recommender address is invalid!");
        require(referrer[msg.sender] == address(0), "You have already recommender cannot change!");

        referrer[msg.sender] = recommender;
        referral_grade[recommender][0] += 1;
    ...
    function join() public {
        ...
        require(referrer[msg.sender] != address(0), "You cant mint without recommender!");
        address recommender_address = referrer[msg.sender];//上级地址
        ...
        uint256 amount = (join_price*10)/100;//直推奖励10%
        ...
        for(uint256 i = 1; i < 7; i++) {
            ...
            if(current_vip > remain_vip){
                ...
                recommender_address = referrer[current_address];//更新上级地址
            }else{
                break;
            }
        }
        uint256 totalNftCount = utopiaNFT.totalSupply();
        if(totalNftCount == mintLadder[0] || totalNftCount == mintLadder[1] || totalNftCount == mintLadder[2] || totalNftCount == mintLadder[3]){
            pause = true;
        }
    ...
    //结算奖励
    function StatisticsReward() public onlyOwner {
        uint256 totalNftCount = utopiaNFT.totalSupply();
        eachBonus = totalBonus/totalNftCount;
        numberOfTimes += 1;
    }
    ...
    function queryReward(address account) public view returns(uint256){
        uint256 amount = 0;
        if(numberOfTimes > numberOfIndividuals[account]){
            amount = rewardLadder[vip[account]]*eachBonus;
        }
        return amount;
    }
    ...
    function receiveRewards() public {
        require(numberOfTimes > numberOfIndividuals[msg.sender], "Unable to receive rewards!");
    ...
    function setUtopia(address utopiaNFT_) public onlyOwner {
        utopiaNFT = UtopiaNFT(utopiaNFT_);
    }
}