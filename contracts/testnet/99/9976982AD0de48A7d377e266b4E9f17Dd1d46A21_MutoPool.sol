...
contract MutoPool is OwnableUpgradeable {
  ...
  function initiatePool(InitialPoolData calldata _initData) external returns (uint256) {
    ...
    _initData.poolingToken.safeTransferFrom(msg.sender, address(this), (_ammount*1e18)/1e18);
  ...
  function _placeSellOrders(
    ...
    poolData[poolId].initData.biddingToken.safeTransferFrom(msg.sender, address(this), (sumOfSellAmounts*1e6)/1e18);
  }
  ...
  function sendOutTokens(
    ...
    if (poolingTokenAmount > 0) {
      poolData[poolId].initData.poolingToken.safeTransfer(userAddress, (poolingTokenAmount*1e18)/1e18);
    }
    if (biddingTokenAmount > 0) {
      poolData[poolId].initData.biddingToken.safeTransfer(userAddress, (biddingTokenAmount*1e6)/1e18);
    }
  }
