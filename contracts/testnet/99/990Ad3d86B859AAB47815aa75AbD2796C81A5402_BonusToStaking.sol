...
contract BonusToStaking{
    ...
    event Delivered(address indexed user, uint8 token, uint256 amount);

    constructor(address owner, address bonusContract, address stakingTheKey, uint8 token){
        _owner = owner;
        _bonusContract = bonusContract;
        _stakingTheKey = StakingTheKey(stakingTheKey);
        _token = token;
    }
    ...
    function setStakingTheKey(address stakingTheKey) public onlyOwner{
        _stakingTheKey = StakingTheKey(stakingTheKey);
    }
    ...
    function delivery(
        ...
        if(_token == 0){
            _stakingTheKey.delivery(user, _token, amount);
            emit Delivered(user, _token, amount);
        }else{
            uint256 am = 10 ** 18 * amount / _stakingTheKey.getPriceUSDT();
            _stakingTheKey.delivery(user, _token, am);
            emit Delivered(user, _token, am);
        }
	}
