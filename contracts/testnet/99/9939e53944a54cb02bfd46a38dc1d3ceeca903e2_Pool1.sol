...
// PI-DAO Pool 1
contract Pool1 is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using AddressUpgradeable for address;
    
    IToken public token;
    IUniswapV2Pair public uniswapV2Pair;
    uint256 public r; // div r0 or r1
    ...
    address[] private realNodes;

    // ---------------- Lock 365 days LP -----------------
   uint256 public lockDays = 0 days;
    mapping(address => uint256[]) private lockLPStartTime; // Start lock-up time recording
    mapping(address => uint256[]) private lockLPTotal;  // Lock-up records
    mapping(address => uint256) private lockLPSum;  // The total number of locked positions
    mapping(address => bool) private isLocked; // have participated in the lock-up
    address[] private lockList; // List of lock-up addresses
    mapping(address => uint256) public userRewardPerTokenPaid; //paid
    mapping(address => uint256) public rewards; // Unpaid Rewards
    ...
    address[] private NormalLockList;
    mapping(address => uint256) public normal_userRewardPerTokenPaid; 
    mapping(address => uint256) public normal_rewards;
    ...
    function initialize()public initializer{
        ...
        setToken(0x492e1abAE7F716197dAe20842533d56A1f119693);
        setPair(0x879A59D2a8AaABa671e07Ac04D775f0C7505bf4e);
        setR(0);
        n_1000 = 1000;
        n_200 = 200;
        requiredQuantity = 2;
    ...
    function setN1000(uint256 value) public onlyOwner {
        n_1000 = value;
    }
    function setN200(uint256 value) public onlyOwner {
        n_200 = value;
    }
    ...
    function getRootList(address account) public view returns(address[] memory) {
        return rootList[account];
    }

    function getRootNode(address account) public view returns (address) {
        return rootNode[account];
    }
    function isNode(address account) public view returns(bool) {
        return nodeStatus[account] > 0 ? true : false;
    }
    function getNodeStatus(address account) public view returns (uint256) {
        return nodeStatus[account];
    }

    function getRealNodesLength() public view returns(uint256) {
        return realNodes.length;
    }

    function getRealNodesList(uint256 start, uint256 limit) public view returns(address[] memory) {
    ...
    function upgradeToNode() public {
        ...
        (uint256 self_sum, ,) = calculatedValue(msg.sender, 0);
    ...
    function registerRootNode(address rootAccount) public {
        ...
        rootList[msg.sender] = rootList[rootAccount];
        rootList[msg.sender].push(rootAccount);

        emit RegisterRootNode(msg.sender, rootAccount);
    }

    function calculatedValue(address account, uint256 types) public view returns (uint256 amount, uint256 token0Amount, uint256 token1Amount) {
        uint256 self_LP = 0;
        if(types == 0)
    ...
    function _queryRequiredQuantity() internal virtual returns (uint256, bool) {
        ...
        for(uint256 i = 0; i < recommendAddrList[msg.sender].length; i++) {
            ...
            (uint256 s, ,) = calculatedValue(addr, 0);
    ...
    function Reward(address account, uint256 types) public view returns(uint256) {
        if(types == 0)
            return rewards[account];
        else
            return normal_rewards[account];
    }

    function RewardPaid(address account, uint256 types) public view returns(uint256) {
        if(types == 0)
            return userRewardPerTokenPaid[account];
        else
            return normal_userRewardPerTokenPaid[account];
    }
    ...
    function lottery() public returns (bool) {
        ...
        uint256 total = token.balanceOf(address(this)); // The total number of tokens owned by the mining pool
        require(total > 0, "token balance is zero");

        uint256 freed = total.mul(10).div(1000); // Release this round
        ...
        for (uint256 i = 0; i < lockList.length; i++) {
            ...
            (uint256 LockValue,,) = calculatedValue(account, 0);
        ...
        for (uint256 i = 0; i < NormalLockList.length; i++) {
            ...
            (uint256 nomarlLockValue,,) = calculatedValue(account, 1);
