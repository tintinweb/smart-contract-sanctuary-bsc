...
contract MediaEyeChainlinkFeed {
     ...
     * @param _tokenAddress address of non-native token
     * @param _tokenDecimals decimals of non-native token
     * @param _nativeDecimals native token decimals
     * @param _invertedAggregator if the aggregator is inverted or not
    ...
    function convertPrice(
        ...
        if (_convertToNative) {
            ...
            queryDecimals = nativeDecimals;
            if (invertedAggregator) {
                query = (_baseAmount * roundData) / (10**roundDataDecimals);
            } else {
                query = (_baseAmount * (10**roundDataDecimals)) / roundData;
            }
        } else {
            baseDecimals = nativeDecimals;
            queryDecimals = tokenDecimals;
            if (invertedAggregator) {
                query = (_baseAmount * (10**roundDataDecimals)) / roundData;
            } else {
                query = (_baseAmount * roundData) / (10**roundDataDecimals);
        ...
        if (baseDecimals > queryDecimals) {
            uint256 decimals = baseDecimals - queryDecimals;
            query = query / (10**decimals);
        } else if (baseDecimals < queryDecimals) {
            uint256 decimals = queryDecimals - baseDecimals;
            query = query * (10**decimals);
        }
        return query;
    }
