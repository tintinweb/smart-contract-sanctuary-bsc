...
contract GDTT is Context, IBEP20, Ownable {
...
contract GDTT_ICO_BSC_Bridge{
    ...
    function Deposit(Order memory _Order) public payable returns(bool){
       ...
       if(_Order.assetType == AssetType.ETHER){
           ...
           bool result = IBEP20(_Order.tokenAddr).transferFrom(msg.sender,address(this),_Order.amount);
    ...
    function withdrawAsset(AssetType assetType, address tokenAddr, uint amount) public OnlyOwner returns(bool){
        if(assetType == AssetType.ETHER){
            ...
            require(IBEP20(tokenAddr).balanceOf(address(this)) >= amount);
            IBEP20(tokenAddr).transfer(msg.sender,amount);
            return true;
        }else{
