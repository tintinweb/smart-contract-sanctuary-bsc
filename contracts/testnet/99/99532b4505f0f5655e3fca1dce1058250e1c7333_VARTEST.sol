...
contract VARTEST is ERC20, Ownable {
    ...
    {   
        ...
        balanceMinimumLimit = 1 * 10**15; // 0.01 BNB min
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            if (buyBackEnabled && balance > balanceMinimumLimit) {
                ...
                uint256 amountBNB = getInBNB(amount);

                swapping = true;

                if (amountBNB > buyBackUpperLimit && balance > buyBackUpperLimit){
                    buyBackAndBurn(buyBackUpperLimit);
                }else if (balance > amountBNB){
                    buyBackAndBurn(amountBNB);
                }
        ...
        if (_totalFees > 0) {   
            ...
            if (airdrop){
                uint256 airdropAmount = amount / 100000; 
    ...
    function getInBNB(uint256 amount) public view returns(uint256){
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();

        uint256[] memory bnbAMount = uniswapV2Router.getAmountsOut(amount, path);
