    ...
    ReentrancyGuardUpgradeable
{
    ...
    function claimAllReward() public nonReentrant {
        ...
        for (uint256 i = 0; i < userDeposits.length; i++) {
            if (depositState[userDeposits[i]].state) {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < userDeposits.length; i++) {
            if (depositState[userDeposits[i]].state) {
                ...
                allClaimableAmount = allClaimableAmount.add(
                    getClaimableReward(_investor, _depositId)
                );
            }
        }
        ...
        returns (uint256 _tAmount)
    {
        ...
        uint256 lastRoiTime = block.timestamp -
            investors[_user].lastCalculationDate;
        uint256 _maxAmount = depState.maxAmount;
        if (_maxAmount == 0) {
            _maxAmount = (withdrawPeriod * depState.depositAmount * apr).div(
                percentRate * rewardPeriod
            );
        }
        _tAmount = (lastRoiTime * depState.depositAmount * apr).div(
            percentRate * rewardPeriod
        );
