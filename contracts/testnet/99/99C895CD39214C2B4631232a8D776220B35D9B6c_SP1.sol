...
contract SP1 is Context, IERC20, Ownable {
    ...
    struct AddressTaxSplit {
        ...
        uint256 _buyDevTax;
        uint256 _buyJackpotTax;
        uint256 _sellMarketingTax;
        uint256 _sellDevTax;
        uint256 _sellJackpotTax;
    }
    ...
    event AddressTaxSplitChanged(address account, bool enabled, uint256 marketingTax, uint256 devTax, uint256 jackpotTax);
    event AddressBuyFeesChanged(address account, bool enabled, uint256 buyTaxFee, uint256 buyLiquidityFee);
    event AddressBuyTaxSplitChanged(address account, bool enabled, uint256 marketingTax, uint256 devTax, uint256 jackpotTax);
    event AddressSellFeesChanged(address account, bool enabled, uint256 buyTaxFee, uint256 buyLiquidityFee);
    event AddressSellTaxSplitChanged(address account, bool enabled, uint256 marketingTax, uint256 devTax, uint256 jackpotTax);

    event JackpotWalletChanged(address account, bool enabled, uint256 multiplicator, bool callFunction);
    ...
    constructor () {
        ...
        jackpotWallets[address(0x3dF7EE033D65573ee6418a17692B45061BD0d525)].name = 'Superspin';
        jackpotWallets[address(0x3dF7EE033D65573ee6418a17692B45061BD0d525)].enable = true;
        jackpotWallets[address(0x3dF7EE033D65573ee6418a17692B45061BD0d525)].multiplicator =  1;
        jackpotWallets[address(0x3dF7EE033D65573ee6418a17692B45061BD0d525)].callFunction = true;
        _jackpotWallets.push(address(0x3dF7EE033D65573ee6418a17692B45061BD0d525));

        jackpotWallets[address(0x3168a3E6b23b471c99B5CE19c06A4Df863e5e6Fd)].name = 'Megaspin';
        jackpotWallets[address(0x3168a3E6b23b471c99B5CE19c06A4Df863e5e6Fd)].enable = true;
        jackpotWallets[address(0x3168a3E6b23b471c99B5CE19c06A4Df863e5e6Fd)].multiplicator =  2;
        jackpotWallets[address(0x3168a3E6b23b471c99B5CE19c06A4Df863e5e6Fd)].callFunction = true;
        _jackpotWallets.push(address(0x3168a3E6b23b471c99B5CE19c06A4Df863e5e6Fd));

        jackpotWallets[address(0x6176d1913d4577a70adf93144dA355874355713D)].name = 'CyberSpin';
        jackpotWallets[address(0x6176d1913d4577a70adf93144dA355874355713D)].enable = true;
        jackpotWallets[address(0x6176d1913d4577a70adf93144dA355874355713D)].multiplicator =  1;
        jackpotWallets[address(0x6176d1913d4577a70adf93144dA355874355713D)].callFunction = true;
        _jackpotWallets.push(address(0x6176d1913d4577a70adf93144dA355874355713D));
    ...
    function setAddressTaxSplit(address _address, bool _enable, uint256 _addressMarketingTax, uint256 _addressDevTax, uint256 _addressJackpotTax) external onlyOwner {
        ...
        addressTaxSplit[_address]._jackpotTax = _addressJackpotTax;
        
        emit AddressTaxSplitChanged(_address, _enable, _addressMarketingTax, _addressDevTax, _addressJackpotTax);
    }
    
    function setBuyAddressTaxSplit(address _address, bool _enable, uint256 _addressMarketingTax, uint256 _addressDevTax, uint256 _addressJackpotTax) external onlyOwner {
        addressTaxSplit[_address].enable = _enable;
        addressTaxSplit[_address]._buyMarketingTax = _addressMarketingTax;
        addressTaxSplit[_address]._buyDevTax = _addressDevTax;
        addressTaxSplit[_address]._buyJackpotTax = _addressJackpotTax;
        
        emit AddressBuyTaxSplitChanged(_address, _enable, _addressMarketingTax, _addressDevTax, _addressJackpotTax);
    }
    
    function setSellAddressTaxSplit(address _address, bool _enable, uint256 _addressMarketingTax, uint256 _addressDevTax, uint256 _addressJackpotTax) external onlyOwner {
        addressTaxSplit[_address].enable = _enable;
        addressTaxSplit[_address]._sellMarketingTax = _addressMarketingTax;
        addressTaxSplit[_address]._sellDevTax = _addressDevTax;
        addressTaxSplit[_address]._sellJackpotTax = _addressJackpotTax;
        
        emit AddressSellTaxSplitChanged(_address, _enable, _addressMarketingTax, _addressDevTax, _addressJackpotTax);
    }

    function setJackpotWallets(address _address, bool _enable, uint256 _multiplicator, bool _callFunction) external onlyOwner() {
        jackpotWallets[_address].enable = _enable;
        jackpotWallets[_address].multiplicator =  _multiplicator;
        jackpotWallets[_address].callFunction = _callFunction;
        _jackpotWallets.push(_address);

        emit JackpotWalletChanged(_address, _enable, _multiplicator, _callFunction);
    }
    ...
    function _sendTax(uint256 rFee, uint256 tFee) private{
        ...
        if(tMarketingTax > 0){
            //tax reflection to the wallets
            //marketing T+R Values
            //_tOwned[marketingWallet] = _tOwned[marketingWallet].add(tMarketingTax);
            //_rOwned[marketingWallet] = _rOwned[marketingWallet].add(rMarketingTax);
        ...
        if(tDevTax > 0){
            ////dev T+R Values
            //_tOwned[devWallet] = _tOwned[devWallet].add(tDevTax);
            //_rOwned[devWallet] = _rOwned[devWallet].add(rDevTax);
            // instead of paying in tokens it will be directly sent as BNB to the wallet
            // capture the contract's current ETH balance.
            // this is so that we can capture exactly the amount of ETH that the
            // swap creates, and not make the liquidity event include any ETH that
        ...
        if(tFee > 0){
            //jackpot T+R Values (single => obsolet)
            //_tOwned[jackpotWallet] = _tOwned[jackpotWallet].add(tFee);
            //_rOwned[jackpotWallet] = _rOwned[jackpotWallet].add(rFee);
            //max multiplicator
