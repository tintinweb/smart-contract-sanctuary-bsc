...
contract Diamond is ERC20, Ownable {
    ...
    uint256 public minRewardTokensToSwap = 10000 gwei;
    ...
    function manualSendAll() public onlyOwner {
        uint256 totalEth = address(this).balance.sub(
            totalEthReward.sub(totalClaimedEthReward)
        );
    ...
    function takeFees(
        ...
        if (automatedMarketMakerPairs[_from]) {
            ...
            if (pendingTokensForReward > minRewardTokensToSwap) {
                addEthRewardPool(pendingTokensForReward);
            }
    ...
    function addEthRewardPool(uint256 _tokenAmount) private {
        uint256 totalEth = swapTokensForEth(_tokenAmount);
        totalEthReward = totalEthReward.add(totalEth);

        emit AddRewardPool(totalEth);
    }
    ...
    function availableEthRewards() public view returns (uint256) {
        return totalEthReward.sub(totalClaimedEthReward);
    }
