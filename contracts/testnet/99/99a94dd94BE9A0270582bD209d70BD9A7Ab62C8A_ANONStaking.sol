...
contract ANONStaking is Ownable {
    ...
    function setBNBwallet(address wallet) external {
    ...
    function stake(uint8 _pid, uint256 _amount) external payable returns (bool) {
        require(msg.value == depositFee, "Exact depositFee is not deposited");
    ...
    function unStake(uint8 _pid, uint256 _amount) external payable returns (bool) {
        require(msg.value == withdrawFee, "Exact withdrawFee is not deposited");
