...
contract Markets {
    ...
    mapping(address => uint256) private _METASHbalances;

    uint256 public METASHprice;  
    ...
    event AddWhiteList(address indexed account, bool isExcluded);
    ...
    function setfeeAddrs(address _feeAddr) external CheckPermit() {
        feeAddr = _feeAddr;
    }
    ...
    function addWhiteList(address account, bool excluded) public CheckPermit {
        WhiteList[account] = excluded;
        emit AddWhiteList(account, excluded);
    }
    function isBlacklist(address account) public view returns (bool) {
        return _isBlacklist[account];
    }

    function includeBlacklist(address account) public CheckPermit() {
        _isBlacklist[account] = true;
    }

    function excludeBlacklist(address account) public CheckPermit() {
    ...
    function setNFTPrice(uint256 _price) external CheckPermit {
        METASHprice = _price;
    }


    function setMETASHTokenAddress(address  _tokenAddress) external CheckPermit {
        stakeInToken = IERC20(_tokenAddress);
    }

    function balanceOfMetash(address account) public view returns (uint256) {
        return _METASHbalances[account];
    }


    function stake(uint256 quantity) public {
        require(stakeInToken.balanceOf(msg.sender) >= METASHprice * quantity, "You don't have enough balance.");

        stakeInToken.transferFrom(msg.sender, address(this), METASHprice * quantity);
        _METASHbalances[msg.sender] = _METASHbalances[msg.sender] + METASHprice * quantity;
    }
    ...
    function buy(address nft, uint256 nftId) external {
        // require(_METASHbalances[msg.sender] >= METASHprice, "you should be stake METASH token first");
        require(_isBlacklist[msg.sender] == false, "owner is in blacklist");
        uint256 idx = nftIndexes[nft][nftId];
    ...
    function buyForETH(address nft, uint256 nftId) external payable {
        require(_METASHbalances[msg.sender] >= METASHprice, "you should be stake METASH token first");
        _buy(msg.sender, nft, nftId); 
    }
    ...
    function _buy(
        ...
        _METASHbalances[user] = _METASHbalances[user] - METASHprice ;
        stakeInToken.transfer(_destroyAddress,METASHprice);
    }
