...
/*
Website Link: https://www.endlessburn.com/
Telegram Link: https://t.me/EndlessBurnOfficial
Discord Link: https://discord.com/channels/@me
Twitter Link: https://twitter.com/Endless_Burn
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0x9100b710464944b6a9dE7Ebae95e61434e9E5bD1); // PINKSALE Token
...
contract EndlessBurn is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Endless Burn", "BURNX") {
    	...
    	marketingWallet = address(0x97290D55E0Bd5Ce8cB8fbcf0627a6694E0269dE1); // set as marketing wallet
        buyBackWallet = address(0xad25B4d86E6d274dd7A9b48cB4F031e1b0D15f2A); // set as marketing wallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee) external onlyOwner {
        marketingBuyFee = _marketingFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
        buyBackBuyFee = _buybackFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee + buyBackBuyFee;
        require(totalBuyFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        buyBackSellFee = _buybackFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + buyBackSellFee;
    ...
    function setLimits(bool _flag) external onlyOwner returns (bool){
        limitsInEffect = _flag;
        transferDelayEnabled = _flag;
        return true;
    }
        ...
        if(limitsInEffect){
            if (
                ...
                //when sell
                    ...
                    require(amount <= maxTransactionAmount/2, "Sell transfer amount exceeds the maxTransactionAmount.");
                }else {
                    require(amount + balanceOf(to) <= maxWallet, "Max wallet exceeded");
                }
            }
