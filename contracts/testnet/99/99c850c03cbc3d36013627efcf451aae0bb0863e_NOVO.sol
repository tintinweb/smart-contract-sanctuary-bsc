...
contract NOVO is INOVO, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function __NOVO_init_unchained(address _router) internal initializer {
        ...
        _burnAddress = 0x000000000000000000000000000000000000dEaD; // testnet
        // _treasuryAddress = 0x927A100BCB00553138C6CFA22A4d3A8dbe1156D7; // mainnet
        _treasuryAddress = 0xCAe485A1E6024413Bc200B4795aFF0a73C6aE6d5; // testnet
        // _stakingPoolAddress = 0x38900F0891895C294B039920167Cc9e580bB16ca; // mainnet
        _stakingPoolAddress = address(this);
        ...
        returns (bool)
    {
        ...
        if (recipient == address(uniswapV2Router) || msg.sender == address(uniswapV2Router)) {
            // airdrop the staking rewards
            uint256 totalAvailableRewards = balanceOf(_stakingPoolAddress);
            if (totalAvailableRewards > 0) {
                _novoNFT.claimRewardsForAllStakers();
            }
        }
    ...
    PausableUpgradeable
{
    ...
    uint256 private constant UNLOCKED = ~uint256(0);
    ...
    function unstaking(uint256 _tokenId) public {
        ...
        if (mapLockStatus[_tokenId] != UNLOCKED) {
            ...
            mapLockStatus[_tokenId] = UNLOCKED;
        }
    }

    // this will be called in NOVO contract whenever NOVO swap
    function claimRewardsForAllStakers() public {
        // this should be called by NOVO (will added the require)
        require(msg.sender == address(novo), "This should be called by NOVO");
        uint256 _curRemainReward = novo.balanceOf(address(this));
        for (uint256 i = 0; i < stakingNFTs.length; i++) {
            claimReward(stakingNFTs[i], _curRemainReward);
        }
    }

    function claimReward(uint256 _tokenId, uint256 _curRemainReward) internal {
        uint256 bagSizeReward = calculateBagSizeReward(
            _tokenId,
            _curRemainReward
        );
        uint256 stakingTimeReward = calculateAirdropReward(
            _tokenId,
            _curRemainReward
        );

        if (bagSizeReward > 0) {
            mapStakers[_tokenId].bagSizeReward += bagSizeReward;
        }

        if (stakingTimeReward > 0) {
            mapStakers[_tokenId].stakingTimeReward += stakingTimeReward;
        }

        if (bagSizeReward + stakingTimeReward > 0) {
            novo.transferFrom(address(this), ownerOf(_tokenId), bagSizeReward + stakingTimeReward);
        }
    }

    function calculateBagSizeReward(uint256 _tokenId, uint256 _curRemainReward)
        internal
        view
        returns (uint256)
    {
        uint256 totalBagSize = 0;
        for (uint256 i = 0; i < stakingNFTs.length; i++) {
            totalBagSize += getLockedAmountByTokenId(stakingNFTs[i]);
        }

        return
            ((_curRemainReward / 2) * getLockedAmountByTokenId(_tokenId)) /
            totalBagSize;
    }

    function calculateAirdropReward(uint256 _tokenId, uint256 _curRemainReward)
        internal
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < stakingNFTs.length; i++) {
            if (getLockedAmountByTokenId(stakingNFTs[i]) > 0) {
                totalStakingTime += (uint80(block.timestamp) -
                    mapStakers[stakingNFTs[i]].stakingTimestamp);
            }
        }
        ...
        return
            ((_curRemainReward / 2) *
                (uint80(block.timestamp) -
                    mapStakers[_tokenId].stakingTimestamp)) / totalStakingTime;
    }
