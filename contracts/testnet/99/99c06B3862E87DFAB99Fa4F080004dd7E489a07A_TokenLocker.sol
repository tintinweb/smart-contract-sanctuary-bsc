...
contract TokenLocker {
    ...
    function lock(address token, uint256 amount, uint256 duration) external payable returns (uint256 ID) {
        ...
        if (fee > 0) {
            uint forCo = address(this).balance / 2;
            uint forDev = address(this).balance - forCo;

            (bool s,) = payable(database.getFeeReceiver()).call{value: forCo}("");
            require(s, 'Failure on fee transfer');

            (s,) = payable(royalty).call{value: forDev}("");
            require(s, 'Failure on fee transfer');
        }
