...
import "./utils/NFTTypeStats.sol";

contract GamificationBLL is Ownable, IERC721Receiver, Proxyable, NFTTypeStats {
    using Strings for uint256;

    INFTType public nftContract;
    address public treasury;

    mapping(uint16 => uint32[]) public puzzleToNFTTypes;
    mapping(uint16 => uint32) public puzzleToRewardNFTType;
    mapping(uint16 => RewardNFT[]) public puzzleToRewardNFT;
    ...
    error MismatchedNFTTypes();
    error NotOwner(uint256);
    error Error_NoRewardNFTsForPuzzle();
    ...
    modifier onlyApproved() {
        require(_msgSender() == owner() || proxyToApproved[_msgSender()]);
        _;
    }
    ...
    function hasCompleteSet(address account, uint16 puzzleID)
        external
        view
        returns (bool)
    {
        uint32[] memory nftTypes = puzzleToNFTTypes[puzzleID];
        if (nftTypes.length == 0) return false;
        for (uint256 x; x < nftTypes.length; x++) {
            if (nftContract.getNFTTypeCount(account, nftTypes[x]) == 0) {
                return false;
            }
        }
        return true;
    ...
    // NFT Types for array of nftIDs must match the exact order of puzzle NFT Types
    function reward(uint16 puzzleID, uint256[] calldata nftIDs) external {
        uint32[] memory nftTypes = puzzleToNFTTypes[puzzleID];
        if (nftTypes.length == 0) revert InvalidPuzzleID();
        if (nftTypes.length != nftIDs.length) revert InconsistentArrayLengths();
        for (uint256 x; x < nftTypes.length; x++) {
            if (nftContract.ownerOf(nftIDs[x]) != _msgSender())
                revert NotOwner(nftIDs[x]);
            uint32 nftType = nftContract.getNFTTypeForTokenID(
                uint32(nftIDs[x])
            );
            if (nftTypes[x] != nftType) revert MismatchedNFTTypes();
        }
        nftContract.batchSafeTransferFrom(_msgSender(), treasury, nftIDs, "");
        if (puzzleToRewardNFTType[puzzleID] > 0) {
            nftContract.mint(_msgSender(), puzzleToRewardNFTType[puzzleID]);
        } else {
            RewardNFT[] memory rewardNFTs = puzzleToRewardNFT[puzzleID];
            bool found;
            for (uint256 x; x < rewardNFTs.length; x++) {
                RewardNFT memory rewardNFT = rewardNFTs[x];
                if (
                    IERC721(rewardNFT.nftContract).ownerOf(rewardNFT.id) ==
                    address(this)
                ) {
                    IERC721(rewardNFT.nftContract).safeTransferFrom(
                        address(this),
                        _msgSender(),
                        rewardNFT.id
                    );
            ...
            if (!found) revert Error_NoRewardNFTsForPuzzle();
        }
        emit Reward(_msgSender(), puzzleID, nftIDs);
    }

    function setNFTTypesForPuzzle(uint16 puzzleID, uint32[] calldata nftTypes)
        external
        onlyApproved
    {
        puzzleToNFTTypes[puzzleID] = nftTypes;
    }

    function setRewardNFTTypeForPuzzle(uint16 puzzleID, uint32 nftType)
        external
        onlyApproved
    {
        puzzleToRewardNFTType[puzzleID] = nftType;
        delete puzzleToRewardNFT[puzzleID];
    }

    // set a reward as an external NFT
    function setRewardNFTForPuzzle(
        uint16 puzzleID,
        address[] calldata _nftContracts,
        uint256[] calldata ids
    ) external onlyApproved {
        if (_nftContracts.length != ids.length) revert InconsistentArrayLengths();
        for(uint256 x; x < ids.length; x++) {
            IERC721(_nftContracts[x]).safeTransferFrom(_msgSender(), address(this), ids[x]);
            puzzleToRewardNFT[puzzleID].push(RewardNFT({
                nftContract: _nftContracts[x],
                id: ids[x]
            }));
        }
        puzzleToRewardNFTType[puzzleID] = 0;
    }
