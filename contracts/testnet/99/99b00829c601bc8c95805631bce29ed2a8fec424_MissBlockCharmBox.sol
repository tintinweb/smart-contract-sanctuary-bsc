...
contract MissBlockCharmBox is ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, IMissBlockCharmBox {
    ...
    uint256[] private _boxes; // number of version box
    ...
    function _createMBCBox(address owner, uint256 version) private returns (uint256 boxId) {
        _boxes.push(version);
    ...
    function mint(uint256 amount, address buyer, uint256 version)
        external
        override
        onlyNotPaused
    {
        ...
        for (uint256 i = 0; i < amount; i++) {
            uint256 boxId = _createMBCBox(buyer, version);
...
interface IMissBlockCharmBox {
    ...
    function mint(uint256 amount, address buyer, uint256 version) external;
}
