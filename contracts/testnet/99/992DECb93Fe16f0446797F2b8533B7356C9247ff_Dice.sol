...
contract Dice is IDice, Ownable, ReentrancyGuard, Pausable {
    ...
    function _claimBonusAndLottery() internal {
        ...
        if(totalBonusAmount > 0){
            ...
            if(address(RollerLevel) != address(0)){
                token.safeTransfer(address(RollerLevel), tmpAmount);
                RollerLevel.updateBonus(address(token), tmpAmount);
            }
        }
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        { // avoid stack too deep
            ...
            if(address(token) == address(rollerToken)){
                ...
                uint256 rollerAmount = swapRouter.swapExactTokensForTokens(round.burnAmount, amountOut.mul(2).div(10), path, address(this), block.timestamp + (5 minutes))[1];
