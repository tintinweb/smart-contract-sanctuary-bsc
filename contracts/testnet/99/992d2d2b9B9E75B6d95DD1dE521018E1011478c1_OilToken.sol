...
contract OilToken is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 1000000 * 1e9; // 1 million total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Oil Token"; // token name
    string private _symbol = "OIL"; // token ticker
    ...
    uint256 public minTokenToSwap = 1000 * 1e9; // 1k amount will trigger the swap and add liquidity
    uint256 public maxHoldingAmount = 20000 * 1e9;
    uint256 public maxBuyLimit = 10000 * 1e9;
    ...
    // to check wether the address is excluded from max buy limit
    function isExcludedFromMaxBuyLimit(address _account) public view returns (bool) {
    ...
    //to include or exclude any address from max hold limit
    ...
    //to include or exclude any address from max buy limit
    function includeOrExcludeFromMaxBuyLimit(address _address, bool value)
        public
        onlyOwner
    {
   ...
   function _takeBurnFee(address sender,uint256 tAmount, uint256 currentRate) internal {
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            // sending bnb to marketing wallet
