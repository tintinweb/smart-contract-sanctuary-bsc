...
contract BordeauxCityBondIntegrationCode is
    ManagerLikeOwner,
    Initializable,
    IBordeauxCityBondIntegration
{
    using SafeERC20 for IERC20;

    uint256 internal constant STORAGE_PRICE_PERIOD = 365 * 24 * 60 * 60;
    // poolId => tokenId => lastPayForStorageTime
    mapping (uint256 => mapping (uint256 => uint256)) public lastPayForStorageTime;

    uint256 public override BCBOutFee;
    uint256 public override BCBFixedFee;
    uint256 public override BCBFlexedFee;
        ...
        initializer
    {
        ...
        _editBCBOutFee(BCBOutFee_);
        _editBCBFixedFee(BCBFixedFee_);
        _editBCBFlexedFee(BCBFlexedFee_);
    }

//////////////////////////////////////// Settings

    function _editBCBOutFee(
        uint256 BCBOutFee_
    )
        override
        public
        onlyManager
    {
        BCBOutFee = BCBOutFee_;
    }

    function _editBCBFixedFee(
        uint256 BCBFixedFee_
    )
        override
        public
        onlyManager
    {
        BCBFixedFee = BCBFixedFee_;
    }

    function _editBCBFlexedFee(
        uint256 BCBFlexedFee_
    )
        override
        public
        onlyManager
    {
        require(BCBFlexedFee_ < 1000, "MarketPlace::_editBCBFlexedFee - max");
        BCBFlexedFee = BCBFlexedFee_;
    }
...
//////////////////////////////////////// Orders
    ...
    function getCurrency()
        ...
        returns (IERC20)
    {
        return IERC20(IWineFirstSaleMarket(IWineManager(manager()).firstSaleMarket()).firstSaleCurrency());
    }
    ...
    function calculateStoragePrice(
        ...
        returns (uint256)
    {
        if (lastPayForStorageTime[poolId][tokenId] == 0) {
            return 0;
        }
        uint256 timeDiff = block.timestamp - lastPayForStorageTime[poolId][tokenId];
        IWinePoolFull pool = IWineManager(manager()).getPoolAsContract(poolId);
        uint256 storagePrice = (BCBFixedFee + BCBFlexedFee * pool.getWinePrice() / 1000) * timeDiff / STORAGE_PRICE_PERIOD;
        return withBCBOut
            ? storagePrice + BCBOutFee
            : storagePrice;
    }
    ...
    function onMint(
        ...
        public
    {
        require(IWineManager(manager()).getPoolAddress(poolId) == _msgSender(), "method allowed only for WinePool");
        lastPayForStorageTime[poolId][tokenId] = block.timestamp;
    }
    ...
    function onOrderExecute(
        ...
        public
    {
        require(IWineManager(manager()).marketPlace() == _msgSender(), "method allowed only for WineMarketPlace");
        lastPayForStorageTime[poolId][tokenId] = block.timestamp;
    }
    ...
    function onRequestDelivery(
        ...
        public
    {
        require(IWineManager(manager()).deliveryService() == _msgSender(), "method allowed only for WineMarketPlace");
        lastPayForStorageTime[poolId][tokenId] = 0;
    }


//////////////////////////////////////// Owner

    function withdrawBCBFee(address to, uint256 amount)
        override
        public
        onlyManager()
    {
