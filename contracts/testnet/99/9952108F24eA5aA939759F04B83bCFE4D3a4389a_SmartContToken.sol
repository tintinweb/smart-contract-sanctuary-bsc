...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    uint256 private _maxFeeAmount = 5000 * 10**9;
	...
	function updaMaxFeeAmount(uint256 _maxFeeValue) public onlyOwner() {
        _maxFeeAmount = _maxFeeValue * 10**9;
	}
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = balanceOf(address(this)) > tokenAmount;
        if (
            sender != address(0) &&
            recipient != address(0) &&
    ...
    function _getTokenFeeTotal(uint256 amount) private returns (uint256){
        uint256 currentRate = _getRate();
        (,,uint256 tLiquidity, uint256 tCompany) = _getTValues(amount);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rCompany = tCompany.mul(currentRate);
        ...
        uint256 tokenAmount = tLiquidity.add(tCompany);       
        return tokenAmount;
    }
    ...
    /* Parameters   : Set 'true' to turn ON and 'false' to turn OFF */
    function setFeePorcentEnabled(bool _enabled) external onlyOwner {
        _feeTypePorcent = _enabled;
    }
