...
contract Presale is Ownable {
    ...
    constructor(address NFTAddress,address houseNFT,address token,address game){
    ...
    function buyFirst(uint256 count) external payable {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            tokens.push(primaryId);
            userTokens[msg.sender] = tokens;
            _game.setNFTLevel(primaryId,1);
            primaryId += 1;  
        }
    ...
    function buySecond(uint256 count) external payable {
        ...
        for (uint256 i = 0; i < count; i++) {
            _nft.mint(msg.sender,2,seniorId);
              uint256[] storage tokens = userTokens[msg.sender];
            tokens.push(seniorId);
            userTokens[msg.sender] = tokens;
            _game.setNFTLevel(seniorId,2);
            seniorId += 1;  
        }
    ...
    function buyThird(uint256 count) external payable {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            uint256[] storage tokens = userTokens[msg.sender];
            tokens.push(ultimateId);
            userTokens[msg.sender] = tokens;
            _game.setNFTLevel(ultimateId,3);
            ultimateId += 1;  
        }
    ...
    function buyHouse(uint256 count) external payable {
        ...
        require(!_pauseBuy, "endSale");
        for (uint256 i = 0; i < count; i++) {
            _honseNFT.mint(msg.sender,houseId);
            _game.setHouseNFTLevel(houseId);
            houseId += 1;  
        }
