...
contract MinerNFT is ERC721Base, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function updateTokenLocking(uint256 _tokenId, bool _locking) external onlyAuthorizedContracts{
        _changeTokenLocking(_tokenId, _locking);
    }

    /// @notice IMinerNFT interface methods
    function mint(address user) external onlyAuthorizedContracts returns (uint256) {
