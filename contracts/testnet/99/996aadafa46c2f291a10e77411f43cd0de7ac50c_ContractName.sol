...
interface IHolderRewarderDistributor {
    ...
    function setRewardTermDuration(uint256 _minutes) external;
...
contract HolderRewarderDistributor is IHolderRewarderDistributor {
  ...
  uint256 private _shortTermDuration = (5 minutes);
  uint256 private _midTermDuration = (10 minutes);
  uint256 private _longTermDuration = (15 minutes);
  ...
  */
  ...
  function setRewardTokenAddress(address _newToken) external onlyToken {
    rewardToken = IBEP20(_newToken);
  }

  function setRewardTermDuration(uint256 _minutes) external onlyToken {
    _rewardTermDuration = _minutes * 60;
  }
...
contract ContractName is IBEP20, Authorization {
  ...
  uint256 public rewardTermDuration = (2 minutes);
  ...
  function setRewardTermDuration(uint256 _minutes) external onlyOwner {
    rewardTermDuration = _minutes * 60;
    distributor.setRewardTermDuration(_minutes);
  }
  ...
  function shouldAutoDistribute() internal view returns (bool) {
    return
      autoDistribution &&
      msg.sender != uniswapV2Pair &&
      !inSwapAndLiquify &&
      block.timestamp >= (_lastTimeDistrinuted + rewardTermDuration);
  }
  ...
  */
  ...
  /**
  * @dev Will distribute rewards for short or mid or long 
  */
