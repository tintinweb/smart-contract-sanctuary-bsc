...
contract PrivateDistribution is Ownable {
    ...
    enum AllocationType { PRESEED, SEED, PRIVATE1, TEAM, ADVISOR, VALUE_ADVISOR }
    ...
    function withdrawTokens(AllocationType _allocationType) external onlyInvestor(_allocationType) initialized() {
        ...
        _ps1token.safeTransfer(_msgSender(), tokensAvailable);
    ...
    function removeInvestor(address _investor, AllocationType _allocationType) external onlyOwner() {
        require(_investor != address(0), "invalid address");
        Investor storage investor = investorsInfo[_allocationType][_investor];
    ...
    {
        uint256 initialReleasePercentage = uint256(0).mul(1e18);
        ...
        if (allocationType == AllocationType.PRESEED || allocationType == AllocationType.SEED) {
            vestingDuration = _initialTimestamp + 30 days + 330 days;
            noOfRemaingDays = 330;
            initialCliff = _initialTimestamp + 30 days;
        } else if (allocationType == AllocationType.PRIVATE1) {
            vestingDuration = _initialTimestamp + 30 days + 300 days;
            noOfRemaingDays = 300;
            initialCliff = _initialTimestamp + 30 days;
        } else if (allocationType == AllocationType.TEAM) {
            vestingDuration = _initialTimestamp + 270 days + 390 days;
            noOfRemaingDays = 390;
            initialCliff = _initialTimestamp + 270 days;
        } else if (allocationType == AllocationType.ADVISOR) {
            vestingDuration = _initialTimestamp + 180 days + 480 days;
            noOfRemaingDays = 480;
            initialCliff = _initialTimestamp + 180 days;
        } else if (allocationType == AllocationType.VALUE_ADVISOR) {
