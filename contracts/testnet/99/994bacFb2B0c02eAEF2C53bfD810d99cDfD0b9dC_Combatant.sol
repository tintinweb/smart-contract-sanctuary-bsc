    ...
    ERC721EnumerableUpgradeable
{
    ...
    uint256 public constant SOLDIER_LIMIT_STAKING = 125000e18;
    uint256 public constant PILOT_LIMIT_STAKING = 250000e18;
    uint256 public constant GENERAL_LIMIT_STAKING = 500000e18;
     ...
     */
    mapping(TypeId => uint256) private currentIndexes;
    ...
    /**
     *  @notice _combatantBoxes mapping from token ID to CombatantBox
     */
    mapping(uint256 => CombatantBox) public _combatantBoxes;
    ...
    event SetUnlockTime(uint256 tokenId, uint256 startTime);
        ...
        returns (string memory)
    {
        ...
        string memory typeId = uint256(_combatantBoxes[tokenId].typeId)
            .toString();
    ...
    {
        return _combatantBoxes[tokenId];
    }
     ...
     *  @dev    All caller can call this function.
     */
    function getLimitStakingOf(TypeId typeId) public pure returns (uint256) {
        if (typeId == TypeId.GENERAL) {
            return GENERAL_LIMIT_STAKING;
        }

        if (typeId == TypeId.PILOT) {
            return PILOT_LIMIT_STAKING;
        }

        return SOLDIER_LIMIT_STAKING;
    }
    ...
    /**
     *  @notice Set status of start time staking.
     *
     *  @dev    Only admin can call this function.
     */
    function setStatusCombatantBox(uint256 tokenId, uint256 startTime)
        public
        onlyAdminOrOwner
    {
        _combatantBoxes[tokenId].startTimeOfStaking = startTime;
        emit SetUnlockTime(tokenId, startTime);
    }
    ...
    function mint(address owner) external onlyAdminOrOwner {
        ...
        _combatantBoxes[tokenId].typeId = typeId;
        _mint(owner, tokenId);
        _tokenCounter++;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0) && from != to) {
            require(
                _combatantBoxes[tokenId].startTimeOfStaking != 0 &&
                    _combatantBoxes[tokenId].startTimeOfStaking +
                        DURATION_UNLOCK >
                    block.timestamp,
