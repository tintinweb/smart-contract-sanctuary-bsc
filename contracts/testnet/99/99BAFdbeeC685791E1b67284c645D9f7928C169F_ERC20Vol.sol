...
contract ERC20Vol is Context, IERC20, Ownable {
  ...
  function faucet(address account,uint256 amount,uint256 price) external returns (bool) {
    require(MINTER_ROLE[msg.sender]==true);
    _mint(account,amount);
    _balances_mint[account] = _balances_mint[account].add(amount);
    uint256 vol = amount.mul(price).div(10**18);
  ...
  function burnt(address account,uint256 amount,uint256 price) external returns (bool) {
    require(MINTER_ROLE[msg.sender]==true);
