...
//        LockedBalance memory _lockedBalance = lockedBalance;
    ...
    // global：记录每个epoch的bias和slope，并记录未来过期的slopeChange，过期时减去。每一次create/increase lock操作，都会记录最新的point；及解锁时的slopeChange
    function _checkpoint(
        ...
        // Go over weeks to fill history and calculate what the current point is
        {
            ...
            for (uint i = 0; i < 255; ++i) {
                ...
                last_point.slope += d_slope;//减去过期的slope部分（d_slope已经是负数了）
    ...
    function withdraw(uint tokenId) external nonReentrant oncePerBlock(msg.sender){
        require(ownerOf(tokenId) == msg.sender, "ERC721: transfer caller is not owner");
//        require(attachments[tokenId] == 0 && !voted[tokenId], "attached");
    ...
    function powerOfNftAtBlock(uint tokenId, uint blockNo) external view returns (int256){
        ...
        uint max_epoch = _epoch;
        uint g_epoch = findBlockEpoch(blockNo, max_epoch);
        Point memory point_0 = _pointHistory[g_epoch];
        uint d_block = 0;
        uint d_t = 0;
        if (g_epoch < max_epoch) {
            Point memory point_1 = _pointHistory[g_epoch + 1];
    ...
    //Todo timestamp=0 => Error: Returned error: execution reverted
    ...
    int256 public testVal = 0;
    function testWriteTotalPowerAt(uint timestamp) external{
        uint thisEpoch = _epoch;
        Point memory lastPoint = _pointHistory[thisEpoch];
        // calculate power
        uint t_i = (lastPoint.ts / WEEK) * WEEK;
        ...
        testVal = int256(lastPoint.bias);
    }

    // Binary search to estimate timestamp for block number
    function findBlockEpoch(uint blockNum, uint maxEpoch) public view returns (uint) {
        // Binary search
        uint min = 0;
        uint max = maxEpoch;
        for (uint i = 0; i < 128; ++i) {
            ...
            if (_pointHistory[mid].blk <= blockNum) {
                min = mid;
            } else {
                max = mid - 1;
            }
