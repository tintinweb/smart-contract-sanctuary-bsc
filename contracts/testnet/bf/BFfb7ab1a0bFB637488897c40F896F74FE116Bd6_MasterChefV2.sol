...
// will be transferred to a governance smart contract once token community is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    event SetRewardsHolderAddress(address indexed user, address indexed newAddress);
    ...
    function pendingTokens(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0) {
            ...
            uint256 tokenReward = multiplier.mul(tokensPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTokenPerShare = accTokenPerShare.add(tokenReward.mul(1e12).div(pool.lpSupply));
        }
    ...
    function claim(uint256 _limit) external nonReentrant {
      ...
      for (uint256 i = 0; i < _limit; i++){
        ...
        if (block.number.sub(lockupPeriodBlocks) >= pending[msg.sender][i].endBlock){
          ...
          uint256 duration = pending[msg.sender][i].endBlock.sub(pending[msg.sender][i].startBlock) > 0 ? pending[msg.sender][i].endBlock.sub(pending[msg.sender][i].startBlock) : 1;
      ...
      uint256 j = 0;
      while(j < _limit){
        if (pending[msg.sender][k].amount == 0){ //deleted element
          pending[msg.sender][k] = pending[msg.sender][pending[msg.sender].length - 1];
    ...
    /**
     * @notice Delete certain pending struct
     * @param _index Index of struct to delete
     */
    function deletePendingAtIndex(uint256 _index) external {
      pending[msg.sender][_index] = pending[msg.sender][pending[msg.sender].length - 1];
      pending[msg.sender].pop();
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (token.balanceOf(address(rewardsHolder)) >= tokenReward){
          token.transferFrom(address(rewardsHolder), address(this), tokenReward);
        } else {
          //we are now in a deficit, stop rewards
          token.transferFrom(address(rewardsHolder), address(this), token.balanceOf(address(rewardsHolder)));
          updateEmissionRate(0);
        }

        pool.accTokenPerShare = pool.accTokenPerShare.add(tokenReward.mul(1e12).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        lastClaim[msg.sender][_pid] = block.number;
    ...
    function setRewardsHolder(address _rewardsHolder) public {
        require(msg.sender == rewardsHolder, "rewardsHolder: wut?");
        rewardsHolder = _rewardsHolder;
        emit SetRewardsHolderAddress(msg.sender, _rewardsHolder);
    }
    ...
    function toggleVault(address _vault) public onlyOwner {
      isVault[_vault] = !isVault[_vault];
    }

    function setLockupPeriod(uint256 _new) external onlyOwner {
