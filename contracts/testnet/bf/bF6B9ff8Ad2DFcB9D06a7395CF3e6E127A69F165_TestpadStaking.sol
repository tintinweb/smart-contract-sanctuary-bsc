...
contract TestpadStaking is Initializable, AdminableUpgradeable, IStakingLockable {
  ...
  function getRewardPerSecond(address account) public view returns (uint256) {
    return (userInfo[account].amount) / 100 / 100 / (365 * 24 * 3600);
  }

  // base 1000, 20% = 200 minutes
  function getWithdrawFee(address account) public view returns (uint256) {
    uint256 lockedTime = getLockedTime(account);
    if(lockedTime >= 90 days){
      return 0;
    }else if(lockedTime >= 75 days && lockedTime < 90 days){
      return 50;
    }else if(lockedTime >= 60 days && lockedTime < 75 days){
      return 100;
    }else if(lockedTime >= 45 days && lockedTime < 60 days){
      return 150;
    }else if(lockedTime >= 30 days && lockedTime < 45 days){
      return 200;
    }else if(lockedTime >= 15 days && lockedTime < 30 days){
      return 250;
    }else if(lockedTime >= 0 days && lockedTime < 15 days){
      return 350;
    }else{
      return 0;
    }
  ...
  function withdraw(uint256 amount) external {
    ...
    require(!tokensLocked, 'Account is locked');
    ...
    if (amount > 0) {
      ...
      if (tokensLocked) {
        (amount, fee) = takeFee(amount, getWithdrawFee(account));
