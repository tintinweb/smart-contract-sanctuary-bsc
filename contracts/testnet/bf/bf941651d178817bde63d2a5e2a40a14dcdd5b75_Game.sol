...
contract Game is BEP20Token {
    ...
    mapping (address => address) public my_inviter;
    mapping (address => address[]) public my_referrals;
    ...
    function Check_Status(uint number_circle, address adr) public view returns(string memory){
        uint256 number_status;
        if (number_circle == 1)
         number_status = M_Set_Number(tables,number_circle,adr);
        else if (number_circle == 2)
         number_status = M_Set_Number(tables2,number_circle,adr);
        else
    ...
    function Check_Number(uint number_circle, address adr) public view returns(uint256){
        if (number_circle == 1)
        return M_Set_Number(tables,number_circle,adr);
        else if (number_circle == 2)
        return M_Set_Number(tables2,number_circle,adr);
        else
        revert("error number circle");
    }
    ...
    function First_Game() public Set_InGame(){
       ...
       if (my_inviter[msg.sender] != address(0)){
           ...
           emit ReferalsSum(msg.sender,(entry_price[my_table[msg.sender]] - (entry_price[my_table[msg.sender]] - referal_price[my_table[msg.sender]])), my_table[msg.sender]+1, block.timestamp );
       }
    ...
    require(tokenUSDT.balanceOf(msg.sender)  >= entry_price2[my_table2[msg.sender]] * 10 ** 18, "Insufficient funds to pay" );
       ...
       if (my_inviter[msg.sender] != address(0)){
           ...
           emit ReferalsSum(msg.sender,(entry_price2[my_table2[msg.sender]] - (entry_price2[my_table2[msg.sender]] - referal_price2[my_table2[msg.sender]])), my_table2[msg.sender]+3, block.timestamp );
       }
...
////////////////////////////присваивает порядковый номер////////////////////////////////////////////
    function M_Set_Number(address[15][2] memory arr, uint256 number_circle,address adr) public view returns (uint256){

        if (number_circle == 1){
             for (uint i = 0; i < arr[my_table[adr]].length; i++){
            if (arr[my_table[adr]][i] == adr){
                return i;
            }
