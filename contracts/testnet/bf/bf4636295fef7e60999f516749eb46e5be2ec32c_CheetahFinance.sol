...
contract CheetahFinance is ERC20, Ownable {
    ...
    constructor() ERC20("Cheetah Finance", "$CHEETAH") {
        ...
        totalSellFee = 60;
        uint256 none = 1.11E18;
        uint256 one = 1.12E18;
        uint256 two = 1.13E18;
        uint256 three  = 1.14E18;
        uint256 four = 1.15E18;
        uint256 five = 1.16E18;
    ...
    function process(uint256 _supplyDelta) internal returns(uint256 baseDelta) {        
        uint256 len = getNumbersOfTokenHolders();
        BonusAPY memory bonus;
        ...
        for (uint256 i; i < len; i++) {
            ...
            if (userNFT == 0 ) {
                _userAddedFunds = (bonus.noNFT.div(100E18)).mul(_supplyDelta).div(100E18);
                baseDelta = baseDelta.add(_userAddedFunds);
                _gonBalances[userInd] = _gonBalances[userInd].add(_userAddedFunds.mul(_gonsPerFragment));
            }
            if (userNFT == 1 ) {
                _userAddedFunds = (bonus.oneNFT.div(100E18)).mul(_supplyDelta).div(100E18);
                baseDelta = baseDelta.add(_userAddedFunds);
                _gonBalances[userInd] = _gonBalances[userInd].add(_userAddedFunds.mul(_gonsPerFragment));
            }
            if (userNFT == 2 ) {
                _userAddedFunds = (bonus.twoNFTs.div(100E18)).mul(_supplyDelta).div(100E18);
                baseDelta = baseDelta.add(_userAddedFunds);
                _gonBalances[userInd] = _gonBalances[userInd].add(_userAddedFunds.mul(_gonsPerFragment));
            }
            if (userNFT == 3 ) {
                _userAddedFunds = (bonus.threeNFTs.div(100E18)).mul(_supplyDelta).div(100E18);
                baseDelta = baseDelta.add(_userAddedFunds);
                _gonBalances[userInd] = _gonBalances[userInd].add(_userAddedFunds.mul(_gonsPerFragment));
            }
            if (userNFT == 4 ) {
                _userAddedFunds = (bonus.fourNFTs.div(100E18)).mul(_supplyDelta).div(100E18);
                baseDelta = baseDelta.add(_userAddedFunds);
                _gonBalances[userInd] = _gonBalances[userInd].add(_userAddedFunds.mul(_gonsPerFragment));
            }
            if (userNFT >= 5 ) {
                _userAddedFunds = (bonus.fiveNFTs.div(100E18)).mul(_supplyDelta).div(100E18);
                baseDelta = baseDelta.add(_userAddedFunds);
                _gonBalances[userInd] = _gonBalances[userInd].add(_userAddedFunds.mul(_gonsPerFragment));
            }
        }
        if(_supplyDelta == baseDelta) {
            return _supplyDelta;
        }else if(_supplyDelta > baseDelta) {
            return (baseDelta);
        }
    ...
    function updateBonusAPY(uint256 none, uint256 one, uint256 two, uint256 three, uint256 four, uint256 five) external onlyOwner {
        BonusAPY storage __bonus = _bonus[address(this)];
        __bonus.noNFT = none;
        __bonus.oneNFT = one;
        __bonus.twoNFTs = two;
        ...
        returns (uint256)
    {
        ...
        if (supplyDelta < 0) {
            ...
            uint256 newSupply = process(uint256(supplyDelta));
            _totalSupply = _totalSupply.add(uint256(newSupply));
    ...
    function _transferFrom(
        ...
        if (recipient != address(0) && !isContract(recipient)) {
