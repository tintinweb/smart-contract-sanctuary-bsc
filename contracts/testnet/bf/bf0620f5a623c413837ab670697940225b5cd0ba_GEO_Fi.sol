...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 5 * ( 10 ** 13);
...
contract GEO_Fi  is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "GEO_Fi";
    string constant _symbol = "GEOFI";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10**6 * 10**_decimals;

    uint256 public _maxTxAmount = 3 * 10**4 * _decimals;
    uint256 public _maxWalletToken = 3 * 10**4 * _decimals;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen,"Not live Bruh!");
        }
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != devFeeReceiver  && recipient != autoLiquidityReceiver){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"You are trying to buy too many coins Bruh!");}
        
        if (sender == pair &&
            buyCooldownEnabled &&
            !isTimelockExempt[recipient]) {
            require(cooldownTimer[recipient] < block.timestamp,"You buying too fast. SLOW DOWN");
