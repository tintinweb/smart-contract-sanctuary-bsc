...
contract vefroyo is MinterRole {
    ...
    IERC20 Froyo = IERC20(0x3db54a7E3774c935747fca2a1AE64a392aA28791);
    address FroyoRewardsAdd = address(0);
    mapping(uint256 => uint256) public lockduration;
    mapping(address => uint256) public accountlockcounter;
    mapping(address => uint256) public accounttotallock;
    mapping(address => mapping(uint256 => uint256)) public accountlockduration;
    mapping(address => mapping(uint256 => uint256)) public accountlockamount;
    mapping(address => mapping(uint256 => uint256)) public accountveamount;
    mapping(address => mapping(uint256 => bool)) public accountlockclaimed;
    mapping(uint256 => address) public rewardpools;
    mapping(address => uint256) public rewardpoolsno;
    ...
    function lock(uint256 _selection, uint256 _amount) external {
        require(_selection <= 4 && _selection >= 1, "Invalid selection");

        uint256 veamount = veFroyoAmount(_selection, _amount);

        uint256 transferamount = froyoAmount(_selection, veamount);
    ...
    function veFroyoAmount(uint256 _selection, uint256 _amount)
        public
        pure
        returns (uint256 veamount)
    {
        if (_selection == 1) {
    ...
    function froyoAmount(uint256 _selection, uint256 veamount)
        public
        pure
    ...
    function SetEmergencyUnlockTime(
        address _user,
        uint256 _counter,
        uint256 _time
    ) external onlyOwner {
    ...
    function unlock(uint256 _counter) external {
        ...
        for (uint256 i = 1; i <= rewardpoolcounter; i++) {
            IFROYORewards(rewardpools[i]).withdraw(
                accountveamount[msg.sender][_counter],
                msg.sender
            );
        }
    ...
    function removeRewardPool(address _address) external onlyOwner {
        uint256 counter = rewardpoolsno[_address];
    ...
    function nextCounter() internal view returns (uint256) {
