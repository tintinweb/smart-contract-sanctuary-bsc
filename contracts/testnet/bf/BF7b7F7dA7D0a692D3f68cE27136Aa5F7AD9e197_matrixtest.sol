...
contract matrixtest is Ownable {
  ...
  constructor(address _tokenreceiver) public Ownable(msg.sender) {
  ...
  function receiverwallet() external view returns (address) {
  ...
  function setTokenReceiver(address account) external onlyOwner() returns (bool) {
  ...
  function payment(address register,address directref,uint256 level) internal {
      ...
      sendValue[5] = amount.mul(refdividend[5]).div(denominator);
      ...
      safeTransfer(tokenreceiver,sendValue[5]);
  }
