...
contract META is IERC20, Ownable {
    ...
    uint256 public _luckyStarFee = 0;
    ...
    uint256 distributorGas2 = 250000;
    uint256 public minPeriod = 5 minutes;
    // uint256 public minPeriod = 1 hours;
    uint256 public LPFeefenhong;
    uint256 public LPFeefenhong2;
    ...
    constructor() {
        ...
        founderAddress = address(0xbbC79680280031f28f048c8ee620159a0f7BF359);
        founderAddress2 = address(0x3B0a5640Fc43d3c21B232c879E91f844ed0FbFB1);
        founders.push(founderAddress);
        founders.push(founderAddress2);
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;  
        if(IERC20(usdtAddress).balanceOf(address(this)) >= 1 * 10**16 && from !=address(this) && LPFeefenhong2.add(minPeriod) <= block.timestamp) {
             process2(distributorGas2) ;
             LPFeefenhong2 = block.timestamp;
        }
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbanance = _tOwned[address(this)].mul(6).div(7);
        uint256 otherBanance = _tOwned[address(this)].sub(nowbanance);
        // process2(distributorGas2, otherBanance);
        swapTokensForCake(otherBanance);
    ...
    function process2(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < foundersCount) {
            if(currentIndex2 >= foundersCount){
                currentIndex2 = 0;
            }
          uint256 amount = initialCAKEBalance.div(founders.length);
        //   uint256 amount = initialCAKEBalance.mul(IERC20(uniswapV2Pair).balanceOf(founders[currentIndex2])).div(IERC20(uniswapV2Pair).totalSupply());
        //   uint256 amount = nowbanance.mul(IERC20(uniswapV2Pair).balanceOf(founders[currentIndex2])).div(IERC20(uniswapV2Pair).totalSupply());
         if( amount < 1 * 10**14) {
             currentIndex2++;
             iterations++;
             return;
         ...
         if(IERC20(usdtAddress).balanceOf(address(this))  < amount )return;
        //  if(_tOwned[address(this)]  < amount )return;
            distributeDividendFounder(founders[currentIndex2],amount);
    ...
    function distributeDividendFounder(address shareholder ,uint256 amount) internal {
    ...
    function _takeFounderFee(address sender,uint256 tAmount) private {
        ...
        // _tOwned[founderAddress2] = _tOwned[founderAddress2].add(tAmount);
        // if (sender != uniswapV2Pair) swapTokensForCake(tAmount);
        // emit Transfer(sender, founderAddress, tAmount);
        // emit Transfer(sender, founderAddress2, tAmount);
        _tOwned[address(this)] = _tOwned[address(this)].add(tAmount);
        emit Transfer(sender, address(this), tAmount);
    }
    ...
    function setFounders(address account) public onlyOwner {
