...
contract Staking is Ownable, ReentrancyGuard {
    ...
    constructor(address _lpToken, address _earnedToken) {
        apr[0] = 32;
        apr[1] = 115;
        apr[2] = 235;
        locktime[0] = 15 days;
    ...
    function deposit(uint256 _lockid, uint256 _amount) public {
        ...
        _lock.amount = _amount.mul(100 - depositFee).div(100);
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < locks[_lockid][account].length; i++) {
            Lock storage _lock = locks[_lockid][account][i];
            if (_lock.endlocktime >= block.timestamp && !_lock.withdrawnReward)
    ...
    function withdraw(uint256 _lockid, uint256 _amount) public {
        ...
        uint256 tamount = _amount.mul((100 - withdrawFee)).div(100);
        ...
        uint256 totalAmount = tamount + _pending.mul(apr[_lockid]).div(100);
        IERC20(earnedToken).transfer(msg.sender, totalAmount);
    }
    ...
    function removeStuckToken() external onlyOwner {
        IERC20(earnedToken).transfer(
            owner(),
            IERC20(earnedToken).balanceOf(owner())
        );
    }

    function removeStuckLPToken() external onlyOwner {
        IERC20(lpToken).transfer(owner(), IERC20(lpToken).balanceOf(owner()));
    }
}
