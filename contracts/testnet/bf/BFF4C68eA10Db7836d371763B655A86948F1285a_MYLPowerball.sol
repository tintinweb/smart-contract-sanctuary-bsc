...
contract MYLPowerball {
    ...
    uint256 private _ticketPrice;                           // Variable for price of each ticket (set as 0.01 bnb)
    ...
    */
    function initiateSmartContractValue(uint256 _elapseTime) public onlyOwner returns(bool){
        ...
        require(_elapseTime > 0,"Invalid time provided, Please try Again!!");
        require((_elapseTime > saleStartTimeStamp[saleIdNow]), "Lottery Event Elapsed time is less than start time");
        require((_elapseTime - saleStartTimeStamp[saleIdNow])<= 2629743 , "Lottery Event should expire in one month");
    ...
    */
    ...
    function purchaseTicket(uint256 _ticketNumbers, uint256 ticketCount) external payable returns(bool)
    {
            ...
            require(msg.value >= ticketCount.mul(_ticketPrice), "Insufficient bnb value");
    ...
    */
    function setElapsedTime(uint256 time) public onlyOwner returns(bool){
        ...
        require((time > saleStartTimeStamp[saleIdNow]), "Lottery Event Elapsed time is less than start time");
        require((time - saleStartTimeStamp[saleIdNow])<= 2629743 , "Lottery Event should expire in one month");
        elapsedTime = time;
        return true;
    }
    ...
    */
     ...
     function withdrawBNBFromContract(uint256 _savingsValue,address payable _savingsReceiver, uint256 _opexValue, address payable _opexReceiver) external onlyOwner returns(bool){
    ...
    */
    ...
    /**
     * @dev get total bnb by user address and saleId 
    */
