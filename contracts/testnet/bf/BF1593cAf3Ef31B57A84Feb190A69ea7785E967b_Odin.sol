...
contract Odin is Context, IERC20, Ownable {
    ...
    constructor(address _nftAddress, address payable routerAddress, address _usdtAddress, address stakingAddress, uint256 _presaleEndsIn) {
    ...
    function calculateTaxFee(uint256 _amount, address _account) private view returns (uint256) {
        uint256 NFTAmount = iBCNFT.balanceOf(_account);
        if (NFTAmount > 0) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (block.timestamp <= presaleEndsIn) {
            require(iBCNFT.balanceOf(to) > 0, "Receipient must be a NFT holder.");
        }
    ...
    function _takeCharity(uint256 tCharity) private {
        ...
        if (_rOwned[_charityWallet] >= charityMinimum) {
            // add liquidity
            swapTokensForUSDT(address(pancakeRouter), _rOwned[_charityWallet], usdtAddress);
    ...
    function _takeStaking(uint256 tStaking) private {
        ...
        if (_rOwned[_stakingAddress] >= stakingMinimum) {
            // add liquidity
            swapTokensForUSDT(address(pancakeRouter), _rOwned[_stakingAddress], usdtAddress);
            uint256 deltaBalance = IERC20(usdtAddress).balanceOf(address(this)).sub(initialBalance);
    ...
    function autoSwapAndLiquidity(uint256 _tokenAmount) internal {
        ...
        swapTokensForUSDT(address(pancakeRouter), tokenAmountToBeSwapped, usdtAddress);
        uint256 deltaBalance = IERC20(usdtAddress).balanceOf(address(this)).sub(initialBalance);
        uint256 usdtToBeAddedToLiquidity = deltaBalance.div(3);
        // add liquidity to pancake
        addLiquidity(address(pancakeRouter), owner(), otherPiece, usdtAddress, usdtToBeAddedToLiquidity);
    ...
    function selectWinner(uint256 seed) external view returns (address) {
        require(!pausedLottery, "Lottery is paused.");
        require(msg.sender == dailyAddress || msg.sender == hourlyAddress, "Can only be called by lottery contract");
        uint256 tSupply = iBCNFT.totalSupply();
        ...
        address winner = iBCNFT.ownerOf(rndNum);
        return winner;
    }
}