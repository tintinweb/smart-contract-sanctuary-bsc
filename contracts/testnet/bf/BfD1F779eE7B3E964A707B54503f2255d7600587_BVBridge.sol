...
import "contracts/interface/I721.sol";
...
contract BVBridge is ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable, DivestorUpgradeable {
    ...
    event SendERC20(address indexed account, address indexed token, uint256 amount);
    event SendERC721(address indexed account, address indexed token, uint256[] cardIds);
    ...
    constructor(
        ...
        // KTAChainInfo[66] = ChainInfo({ isOpen: true, price: 3 ether / 100000 });
    }
    ...
    //     uint256 amounts_,
    //     uint256 chainId_
    // ) public isOutOpen onlyBanker {
    //     uint256 balanceOf = MetaInfo.KTN.balanceOf(address(this), cid_);
    ...
    function acrossERC20(address token_, uint256 amount_) public isOpen {
        ...
        uint256 orderId = meta.orderId;
        meta.orderId++;
        orderInfo[orderId] = Order({ startTime: block.timestamp, amount: amount_, account: msg.sender, token: token_ });
    ...
    function acrossERC721(address token_, uint256[] calldata tids_) public isOpen {
        ...
        uint256 orderId = meta.orderId;
        meta.orderId++;
        orderInfo[orderId] = Order({ startTime: block.timestamp, amount: tids_.length, account: msg.sender, token: token_ });
    ...
    // function getSupportChain(uint256 protocol_) public view checkProtoc(protocol_) returns (uint256[] memory) {
    ...
    //     uint256[] memory infos = new uint256[](size);
