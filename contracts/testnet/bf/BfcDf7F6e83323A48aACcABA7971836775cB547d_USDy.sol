 ...
 */
contract USDy is ERC20, AccessControl, Pausable, IEvents, ReentrancyGuard {
    ...
    // uint public USDY_ThresholdLimit = 1000 * 10**18; //amount after YSL swap to BUSD
    uint public USDy_Tax = 1500; //Tax always multiply by 100
    uint [] public USDy_Tax_Allocation = [1000, 500]; //index-1 => , index-2 =>  All index value multiply by 100;
    uint protocolPriceUSDY = 1 * 10**18;
    ...
    **/
    function mint(address account, uint256 amount) external nonReentrant onlyMinter whenNotPaused{
     ...
     */
    function burn(address account, uint256 amount) external nonReentrant onlyMinter whenNotPaused{
    ...
    function mintByPrice() external  returns(uint){
        ...
        path[0] = Admin.BUSD();
        path[1] = address(this);
        uint poolPriceUSDy = IUniswapV2Router02(ApeswapRouter).getAmountsOut(1*10**18, path)[1];
    ...
    function setBurner(address _burner) external _isAdmin {
        require(_burner != address(0), "USDy: Null address provided");
    ...
    // @param _USDY_ThresholdLimit set amount of threshold

    // Note only admin can call
    //  */
    // function setUSDYThresholdLimit(uint _USDY_ThresholdLimit) external _isAdmin {
    //     require(_USDY_ThresholdLimit > 0, 'YSL: invalid Threshold limit');
    //     emit SetterForUint("YSLToken",address(this),USDY_ThresholdLimit,_USDY_ThresholdLimit,block.number,block.timestamp);
    //     USDY_ThresholdLimit = _USDY_ThresholdLimit;
    // }
    ...
    */
    ...
    function _transfer(
        ...
        if(sender == IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD) || recipient == IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)){
            address user = sender == IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD) ? recipient : sender;
            if(IWhitelist(Admin.whitelist()).getAddressesOfSwap(user)){
                super._transfer(sender, recipient, amount);

            } 
            else {
                if(sender == IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)){
                    ...
                    tax(taxAmount, sender,amount);
                    amount -= taxAmount;
                    uint256 prevAmount = balanceOf(IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)); 
                    super._transfer(sender, recipient, amount);
                    uint256 currentAmount = balanceOf(IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD));   
    ...
    **/
    ...
    function tax( uint256 taxAmount, address sender, uint amount) internal {
        ...
        path[0] = address(this);
        path[1] = BUSD;
        // if(IUniswapV2Router02(ApeswapRouter).getAmountsOut(balanceOf(address(this)),path)[1] >= USDY_ThresholdLimit){
            IERC20(address(this)).approve(ApeswapRouter, taxAmount); //BSC Testnet pancake router address
            swap((taxAmount * USDy_Tax_Allocation[0]) / USDy_Tax, path, address(this));
            (uint amount, uint leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution(sender, IERC20(BUSD).balanceOf((address(this))),amount);
            if(amount != 0){
                IERC20(address(this)).transfer(Admin.Refferal(),amount);
            }
            if(leftAmount != 0){
                IERC20(address(this)).transfer(Admin.TeamAddress(),leftAmount);
            }
            swap((taxAmount * USDy_Tax_Allocation[1]) / (USDy_Tax * 2), path, Admin.Treasury());
            swap((taxAmount * USDy_Tax_Allocation[1]) / (USDy_Tax * 2), path, Admin.TeamAddress());
        // }
    }
