...
contract LKMGashaOpener is Ownable {
    ...
    mapping(randomType => address) public randomRateAddress;
        ...
        returns (string memory)
    {
        ...
        if (_randomType == randomType.SUITE) {
            //BLUEPRINT_RARE
            uint16 ePartId = RANDOM_RATE(randomRateAddress[_randomType])
                .getBlueprintPool(5, equipmentRandom);
            partCode = createBlueprintPartCode(ePartId, _randomType);
        } else if (_randomType == randomType.WEAP) {
            //BLUEPRINT_RARE
            uint16 ePartId = RANDOM_RATE(randomRateAddress[_randomType])
                .getBlueprintPool(8, equipmentRandom);
            partCode = createBlueprintPartCode(ePartId, _randomType);
        }
