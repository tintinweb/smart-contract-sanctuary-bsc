...
contract FCFS is ERC1155Holder, Ownable {
    ...
    uint256 public feeNumerator = 145;
    ...
    mapping(uint256 => mapping(address => uint256)) public usersStakeForALot;
    mapping(uint256 => mapping(Tier => bool)) lotTiers;
    ...
    function placeALot(
        ...
        require(pricePerOne > 0, 'Price must be greater than zero');

        
        require(0 < allowedTiers.length && allowedTiers.length < 4, "tiers lenght must be greater than zero and less than 4");
    ...
    function stake(uint256 lotId, uint256 amount) external {
        ...
        require(
            lotTiers[lotId][tiers[msg.sender]],
        ...
        erc20.transferFrom(msg.sender, addressToSendFee, _calculateFee(lots[lotId].pricePerOne * amount));

        if (usersStakeForALot[lotId][msg.sender] == 0) {
            stakeCounter += 1;
            ...
            usersStakeForALot[lotId][msg.sender] = stakeCounter;

            emit StakeCreation(lotId, amount);
        } else {
            stakes[usersStakeForALot[lotId][msg.sender]].amount += amount;
            lots[lotId].amount -= amount;
        }
    }
    // TODO: менять status на false когда последний чел заклеймил
    function claim(uint256 stakeId) public {
        Stake storage stakeInfo = stakes[stakeId];
        require(stakeInfo.better == msg.sender, "Invalid stake id");
        uint256 lotId = stakeInfo.lotId;
        require(
        ...
        bool allStakesClaimed = true;
        for (uint256 i = 0; i < lots[lotId].stakeIds.length; i++) {
            if (!stakes[lots[lotId].stakeIds[i]].claimed) {
                allStakesClaimed = false;
                break;
            }
        }
        if (allStakesClaimed) {
    ...
    function withdrawNftFromContract(address _contractAddr, uint256 _tokenId, uint256 _amount) external onlyOwner {
        IERC1155 erc1155 = IERC1155(_contractAddr);
        erc1155.safeTransferFrom(address(this), msg.sender, _tokenId, _amount, DEF_DATA);
    }
    ...
    function setFee(uint256 _feeNumerator, uint256 _feeDenominator) external onlyOwner {
        require(_feeNumerator & _feeDenominator > 0, "Numerator and denominator must be greater than 0");
        require(_feeNumerator < _feeDenominator, "The numerator must be less than the denominator");
    ...
    function _createALot(
        ...
        for (uint i = 0; i < allowedTiers.length; ++i) {
            lotTiers[lotsCounter][allowedTiers[i]] = true;
        }
    ...
    function _calculateFee(uint256 bet) internal view returns (uint256 fee) {
        fee = (bet * feeNumerator) / feeDenominator;
        ...
        returns (Stake[] memory stakes_)
    {
        ...
        for (uint256 i = 0; i < stakeCounter; i++) {
            if (stakes[i + 1].better == user) {
                stakes_[j] = stakes[i + 1];
                j++;
            }
        }
    }

    function getAllUserLots(address user)
        external
        view
        returns (AccountPageFCFSCard[] memory userLots) {
        uint256 usetLotsCounter = 0;
        ...
        userLots = new AccountPageFCFSCard[](usetLotsCounter);
        uint256 j = 0;
        for (uint256 i = 0; i < stakeCounter; i++) {
            if (stakes[i + 1].better == user) {
                userLots[j].lotId = stakes[i + 1].lotId;
                userLots[j].amount = stakes[i + 1].amount;
                userLots[j].start = lots[stakes[i + 1].lotId].start;
                userLots[j].period = lots[stakes[i + 1].lotId].period;
                userLots[j].claimed = stakes[i + 1].claimed;
                ++j;
            }
        }
    } 
