...
//how to use this, for a reward token will be a max of 45 address x transation.
//The tested address for format is as follow. 
//["0x16c0a2b6716f8df253d9f7a8386ca00816b59d27",
//"0x97255715da30168d605f1c5f5dd1ae7f2b762401",
//"0x0f3d0334fb55a080231f58903173332b25d43b85",
//"0xc33425ce2153831a5077ab067c63835f039a0bf3",
//"0x8b271551a4f4d5215ec075cab77ed6ddb4ee3bc7"]
// Price is on wei, use the following calculator. https://eth-converter.com/ to identify the right amount.
// You need to increase allowance for the following address.
// Any questions feel free to use our telegram
// Support Telegram Group for This Multisender.
// https://t.me/BladeMultisender
// testnet = 0x181F68ca095EB54A5410Ed6401c5fD0488B2e5ea
...
//        uint256 newAllowance = token.allowance(address(this), spender).sub(value, "SafeIBEP20: decreased allowance below zero");
//        _callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
//    }
    ...
    function _callOptionalReturn(IBEP20 token, bytes memory data) private {
        ...
        bytes memory returndata = address(token).functionCall(data, "SafeIBEP20: low-level call failed");
        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "SafeIBEP20: IBEP20 operation did not succeed");
        }
    }
}
...
contract BladeMultisend {
    ...
    function sendSameValue(address _tokenAddress, address[] memory _to, uint256 _value) external {
      ...
      //  require(_to.length <= 255, 'exceed max allowed');
        ...
        for (uint256 i = 0; i < _to.length; i++) {
            token.approve(address(this), _value); //aprove token before sending it
    ...
    function sendDifferentValue(address _tokenAddress, address[] memory _to, uint[256] memory _value) external {
       ...
       // require(_to.length == _value.length, 'invalid input');
        ...
        for (uint256 i = 0; i < _to.length; i++) {
            token.approve(address(this),  _value[i]); //aprove token before sending it
       ...
       function ApproveBEP20Token (address _tokenAddress, uint256 _value) external  {
    ...
    function recoverETHfromTresLechesCC() external {
        payable(DEV).transfer(address(this).balance);
    }

    // Withdraw ERC20 tokens that are potentially stuck
    function recoverTokensFromTresLechesCC(address _tokenAddress, uint256 _amount) external {                               
