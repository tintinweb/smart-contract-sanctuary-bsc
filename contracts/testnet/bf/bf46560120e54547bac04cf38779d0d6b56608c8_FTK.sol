...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "BEP20: cannot burn tokens from the burn address");
    ...
    function _payref(address account, uint amount) internal virtual {
        require(account != address(0), "BEP20: referrer cannot be the zero address");
...
contract FTK is BEP20, Ownable {
    ...
    constructor() BEP20("FITREK Token", "FTK") {
  ...
  //function buyToken(address _refer, uint usd) public payable returns (bool success){
  function buyToken(address _refer, uint usd) public returns (bool success){
    require(sSBlock <= block.number && block.number <= sEBlock && sBBlock == 20);
    require(sTot < sCap || sCap == 0);
   // uint256 _eth = msg.value;
    uint256 _tkns;    
    _tkns = (sPrice*usd) ;/// 1 ether;
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are DECA Token!!!
    sTot ++;
    ...
    BEP20(busdAddress).transferFrom(msg.sender, mainAddress, usd);
  ...
  function buyToken2(uint usd) public returns (bool success){
    ...
    _transfer(address(this), msg.sender, _tkns); 
    BEP20(busdAddress).transferFrom(msg.sender, mainAddress, (usd**18));

     
    return true;
  }

  function buyToken3( uint usd) public returns (bool success){
    ...
    _transfer(address(this), msg.sender, _tkns); 
    BEP20(busdAddress).transferFrom(mainAddress, msg.sender, usd);
...
function setBusdAddress(address _busdAddress, address _mainAddress) public onlyOwner returns(bool)
  ...
  function StakingWallet(address _senderads, uint256 _amttoken, uint64 membcode, uint64 rcode, uint64 plan) public {  
    ...
    function LPFTK_Locked_Token(IBEP20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
