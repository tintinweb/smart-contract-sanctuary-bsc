...
contract BonfireMetaRouter is IBonfireMetaRouter, Ownable {
    ...
    event CheckerUpdate(address _checker);
    ...
    function _takeFee(uint256 fromAmount, address to)
        internal
        view
        returns (uint256 newAmount)
    {
        uint256 factorP;
        uint256 factorQ = IBonfireChecker(checker).totalShares();
        {
            factorP = IBonfireChecker(checker).validShares(to);
            if (to != msg.sender) {
                uint256 altFactorP = IBonfireChecker(checker).validShares(
                    msg.sender
                );
                factorP = altFactorP > factorP ? altFactorP : factorP;
            }
        }
        uint256 fee;
        if (factorP < factorQ) {
            if (factorP > 1e9 * 1e7) {
                fee =
                    (((fromAmount * feeP) / feeQ) * (factorQ - factorP)) /
                    factorQ;
            } else {
                fee = (fromAmount * maxFeePermille) / 1000;
            }
        } else {
            fee = (fromAmount * feeP) / feeQ;
        }
    ...
    function accumulate(address bonusToken, uint256 threshold) public override {
        if (bonusToken != address(0) && accumulator != address(0)) {
            ...
            gains = _takeFee(gains, msg.sender);
    ...
    function _swapToken(
        ...
        if (
            ...
            (amount, amountB) = (_takeFee(amount, to), amount);
            if (amountB > amount) {
                IERC20(tokenPath[0]).safeTransferFrom(
                    msg.sender,
                    address(this),
    ...
    function _buyToken(
        ...
        uint256 amount = _takeFee(msg.value, to);
        if (amount == 0) {
