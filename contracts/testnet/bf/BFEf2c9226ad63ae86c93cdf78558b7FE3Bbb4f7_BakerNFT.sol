...
contract BakerNFT is ERC721URIStorage, Ownable {
    ...
    function buyNFT(uint256 tokenId) public payable {
        ...
        uint256[] memory list = NFTs[msg.sender];
        uint256 index = findElementInArray(tokenId, list);
        remove(index);
        MarketItems[tokenId].owner = msg.sender;
    }

    function findElementInArray(uint256 element, uint256[] memory arr)
        public
        pure
        returns (uint256)
    {
    ...
    function remove(uint256 index) internal {
        require(NFTs[msg.sender].length > index, "Out of bounds");
        // move all elements to the left, starting from the `index + 1`
        for (uint256 i = index; i < NFTs[msg.sender].length - 1; i++) {
            NFTs[msg.sender][i] = NFTs[msg.sender][i + 1];
        }
        NFTs[msg.sender].pop(); // delete the last item
    }
    ...
    function getNFT(address adr) public view returns (uint256[] memory) {
        return NFTs[adr];
    }
