...
contract KingyoBTC is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "KingyoBTC";
    string constant _symbol = "KINGYO";
    ...
    uint256 public _maxNoOfTokensToGetReward = 1000_000 * (10 **_decimals);
    ...
    uint256 internal sellDevFee = 2;

    uint256 internal feeDenominator = 100;

    address public autoLiquidityReceiver;
    address public marketingFeeReceiver =
        0x291942a0CcDbE9098e3266967f20bd896C996b0E;
    address public devFeeReceiver = 0x17eb3D14401fa814c4455D4068AEF13FaBfcE1b1;
    ...
    function getTotalFee(bool selling) internal returns (uint256) {
        if (selling) {
            totalFee = sellliquidityFee.add(sellmarketingFee).add(sellDevFee).add(
                sellreflectionFee
            );
            return totalFee;
        }
        if (!selling) {
            totalFee = liquidityFee.add(marketingFee).add(devFee).add(reflectionFee);
            return totalFee;
        }
        return totalFee;
    }
    ...
    function setBuyFees(
        ...
        devFee = _buyDevFee;
        totalFee = _buyliquidityFee.add(_buyreflectionFee).add(devFee).add(
            _buymarketingFee
        );
    ...
    function setSellFees(
        ...
        totalFee = _SellliquidityFee.add(_SellreflectionFee).add(sellDevFee).add(
            _SellmarketingFee
        );
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator / 5, "You can't set more than 20%");
