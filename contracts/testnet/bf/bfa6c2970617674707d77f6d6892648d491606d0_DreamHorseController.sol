...
contract DreamHorseController is Ownable {
    ...
    constructor() {
        ...
        heros[1] = Hero(0,1,2,0,0,0,0,0,0,0,0,false,false);
        heros[2] = Hero(0,2,3,0,0,0,0,0,0,0,0,false,false);
        heros[3] = Hero(0,3,4,0,0,0,0,0,0,0,0,false,false);
        heros[4] = Hero(0,4,5,0,0,0,0,0,0,0,0,false,false);
        heros[5] = Hero(0,5,6,0,0,0,0,0,0,0,0,false,false);
              
        pools[1] = Pool(0,1,0,0,0,0,0,0,0);
        pools[2] = Pool(0,2,0,0,0,0,7  days,0,0);
        pools[3] = Pool(0,3,0,0,0,0,14 days,0,0);
        pools[4] = Pool(0,4,0,0,0,0,30 days,0,0);
        pools[5] = Pool(0,1,0,0,0,0,90 days,0,0);
    ...
    function inPool(uint256 tokenId, uint256 poolType, uint256 BNBAmount, uint256 BUSDAmount) public payable {
        ...
        if(BNBAmount > 0){
            BNBAmount = BNBAmount * 10 **6;

            payable(address(this)).transfer(BNBAmount);
        }

        if(BUSDAmount > 0){
            BUSDAmount = BUSDAmount * 10 **6;
        ...
        tokenIdToPools[tokenId] = Pool(
            ...
            BNBAmount,
            BUSDAmount,
            BNBAmount.mul(pools[poolType].rate),
            BUSDAmount.mul(pools[poolType].rate),
        ...
        emit InPool(msg.sender, tokenId, poolType, BNBAmount, BUSDAmount);
    }
    ...
    function getUserPublishTokenIds(address user) public view returns(uint256[] memory tokenIds) {
        ...
        uint256[] memory tokenIds0 = new uint256[](uint256(10000));

        for(uint256 i = 0; i < userTokenIds.length; i++){
            tokenId = userTokenIds[i];
            if(tokenId != 0 && heroSource[tokenId] == 2 && counter < 9999){
