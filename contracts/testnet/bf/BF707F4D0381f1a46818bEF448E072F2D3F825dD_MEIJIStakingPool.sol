...
contract MEIJIStakingPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier onlyLaunchpad() {        
        require(launchpad == msg.sender, "Caller is not launchpad");
        _;
    }
    ...
    function initialize(
        ...
        bonusEndBlock = 99999999; //_bonusEndBlock;
        ...
        PRECISION_FACTOR = 10**decimalsRewardToken;
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        // if (_amount > 0) {
        //     require(block.timestamp - user.lastDepositDate >= (1 days * lockDays), "This pool is locked");
    ...
    function invest(address _staker, uint256 _amount) external onlyLaunchpad {
        ...
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount);
            stakedToken.safeTransfer(launchpad, _amount);
        }

        if (pending > 0) {
            rewardToken.safeTransfer(_staker, pending);
        }

        user.rewardDebt = user.amount.mul(accTokenPerShare).div(PRECISION_FACTOR);

        emit Invest(_staker, _amount);
    }
}