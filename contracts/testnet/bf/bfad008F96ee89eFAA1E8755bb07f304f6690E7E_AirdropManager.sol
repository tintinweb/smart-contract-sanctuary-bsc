...
contract AirdropManager is OwnableUpgradeable, IAirdropManager {
    ...
    EnumerableSetUpgradeable.AddressSet private poolFactories;
    EnumerableSetUpgradeable.AddressSet private _pools;
    mapping(uint8 => EnumerableSetUpgradeable.AddressSet) private _poolsForVersion;
    mapping(address => EnumerableSetUpgradeable.AddressSet) private _poolsOf;
    mapping(address => EnumerableSetUpgradeable.AddressSet) private _contributedPoolsOf;
//    mapping(address => address) private _poolForToken;
    mapping(address => EnumerableSetUpgradeable.AddressSet) private _poolForToken;
    ...
    function isPoolFactory(address factory) public view returns (bool) {
        return poolFactories.contains(factory);
    }
    ...
    onlyAllowedFactory
    {
        _poolForToken[token].remove(pool);
        emit PoolForTokenRemoved(token, pool);
    }


    function getPoolsOfLength(address owner) public view returns (uint256) {
        return _poolsOf[owner].length();
    }

    function getPoolsForTokenLength(address token) public view returns (uint256) {
        return _poolForToken[token].length();
    }
    ...
    function getPoolsForToken(address token) public view returns (address[] memory) {
        uint256 length = _poolForToken[token].length();
        address[] memory allPools = new address[](length);
        for (uint256 i = 0; i < length; i++) {
            allPools[i] = _poolForToken[token].at(i);
        }
        return allPools;
    }
    ...
    returns (CumulativeLockInfo[] memory)
    {
        ...
        for (uint256 i = start; i <= end; i++) {
            ...
            ) = IAirdrop(_pools.at(i)).getPoolInfo();
            lockInfo[currentIndex] = CumulativeLockInfo(
                _pools.at(i), // poolAddress
                token, // token
