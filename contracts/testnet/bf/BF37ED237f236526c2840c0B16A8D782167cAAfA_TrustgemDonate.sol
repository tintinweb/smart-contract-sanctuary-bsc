...
contract TrustgemDonate is Ownable, ReentrancyGuard {
    ...
    function donateBNB(address toAddress) external payable nonReentrant {
        ...
        emit Donate(msg.sender, toAddress, address(0), amount, feeAmount);
    }

    function donateBusd(address toAddress, uint256 amount) external nonReentrant {
        require(amount > 0, "Invalid amount");
        require(toAddress != address(0), "Zero address");
        uint256 feeAmount = amount * fee / feeDivisor;
