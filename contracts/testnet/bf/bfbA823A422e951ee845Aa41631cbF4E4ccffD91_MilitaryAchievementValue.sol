...
contract MilitaryAchievementValue is AccessControlEnumerable, ERC20 {
    ...
    function mint(address to, uint amount) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "SFA: Must have minter role to mint");

        _mint(to, amount);
    }

    function swap(address token, uint amountIn) public {
        require(amountIn > 0, "SFA: Amount too small");

        uint amountOut = getAmountOut(token, amountIn);
        require(amountOut > 0, "SFA Amount Out Insuffcient");
    ...
    function setSwapPair(address token, uint amountIn, uint amountOut) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "SPA: Must have admin role to set price");
        require(token.code.length > 0, "SFA Token address invalid");
        require(amountIn > 0, "SFA Amount in too samll");
        require(amountOut > 0, "SFA Amount out too samll");

        swapPair[token] = [amountIn, amountOut];
    }

    function setWallet(address addr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "SFA Must have admin role to set wallet");

        wallet = addr;
    }

    function getAmountOut(address token, uint amountIn) public view returns (uint) {
        require(swapPair[token].length == 2, "SFA Swap pair length invalid");
