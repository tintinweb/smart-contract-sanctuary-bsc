...
contract OpenBox is Ownable {
    ...
    function handleOpen(uint nonce, uint tokenId, bool isImport, uint _importRequestId) internal {
        uint resultNumber = random(nonce, 100);
        nonces[nonce] = true;
        if(resultNumber < 10) randomHash(0, nonce, tokenId, isImport, _importRequestId);
        else if(resultNumber >= 10 && resultNumber < 40) randomHash(1, nonce, tokenId, isImport, _importRequestId);
        else if(resultNumber >= 40 && resultNumber < 83) randomHash(2, nonce, tokenId, isImport, _importRequestId);
        else randomHash(3, nonce, tokenId, isImport, _importRequestId);
    }
    function open(uint nonce, uint[] memory tokenIds, uint[] memory amounts) public {
        require(!nonces[nonce], "OpenBox::open: nonce used");
        require(tokenIds.length == 1 && amounts.length == 1 && tokenIds[0] == 1, "OpenBox::open: invalid token ids");
