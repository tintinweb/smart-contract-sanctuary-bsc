...
import "./IVPikachu.sol";

contract PikachuPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 pikachuAtLastUserAction; // keep track of pikachu deposited at the last user action.
    ...
    event NewVPikachuContract(address VPikachu);
    ...
    /**
     * @notice Constructor
     * @param _token: Pikachu token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: pikachu pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(pikachuPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the pikachu owner address or the operator address.
     */
    modifier onlyOperatorOrPikachuOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or pikachu owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onPikachuPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.pikachuAtLastUserAction;
    ...
    /**
     * @notice Unlock user pikachu funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrPikachuOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Pikachu Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in PIKACHU)
    ...
    function depositOperation(
        ...
        if (VPikachu != address(0)) {
            IVPikachu(VPikachu).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.pikachuAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Pikachu Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VPikachu != address(0)) {
            IVPikachu(VPikachu).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.pikachuAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.pikachuAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending PIKACHU tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingPikachu = masterchefV2.pendingPikachu(pikachuPoolPID, address(this));
        if (pendingPikachu > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(pikachuPoolPID, 0);
    ...
    /**
     * @notice Set VPikachu Contract address
     * @dev Callable by the contract admin.
     */
    function setVPikachuContract(address _VPikachu) external onlyAdmin {
        require(_VPikachu != address(0), "Cannot be zero address");
        VPikachu = _VPikachu;
        emit NewVPikachuContract(VPikachu);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Pikachu Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingPikachuRewards();
     ...
     * @return Returns total pending pikachu rewards
     */
    function calculateTotalPendingPikachuRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingPikachu(pikachuPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingPikachuRewards()) * (1e18)) / totalShares);
    }
