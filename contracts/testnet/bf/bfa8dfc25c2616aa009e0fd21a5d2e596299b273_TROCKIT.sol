...
contract TROCKIT is Context, IERC20, Ownable, Pausable 
{
    ...
    string private _symbol = "TROCKIT";
    ...
    {  
        ...
        return totalAmount.mul(_liquidityFee.add(_developerFee).add(_marketingFee)).div(100);
    }
    ...
    {
        uint256 allFee = _liquidityFee.add(_developerFee).add(_marketingFee);
        ...
        uint256 ethForMarketing = newBalance.mul(_marketingFee).div(allFee);
        uint256 ethForDev = newBalance.sub(ethForMarketing).sub(ethForLiquidity);
        developerWallet.transfer(ethForDev);
