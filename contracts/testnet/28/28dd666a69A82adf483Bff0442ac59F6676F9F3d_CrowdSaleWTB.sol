...
contract CrowdSaleWTB is Ownable {
    ...
    uint public BNB_rate;
    uint public USDT_rate;
    IERC20 public token;
    ...
    event SetBNBRate(uint newRate);
    event SetUSDTRate(uint newRate);

    constructor(uint bnb_rate,uint usdt_rate,address payable wallet,IERC20 wtbtoken){
        BNB_rate=bnb_rate;
        USDT_rate=usdt_rate;
        _wallet=wallet;
        token=wtbtoken;
    }
    function setUSDTToken(IERC20 token_address) public{
        usdtToken=token_address;
        emit SetUSDTToken(token_address);
    }
    function setBNBRate(uint new_rate) public{
        BNB_rate=new_rate;
        emit SetBNBRate(new_rate);
    }
    function setUSDTRate(uint new_rate) public{
        USDT_rate =new_rate;
        emit SetUSDTRate(new_rate);
    }

   function buyTokenByBNB(uint256 amount) payable external {
        require(
            amount>0,
            "Amount is zero"
        );
        ...
        uint256 bnbAmount=getBNBAmount(amount);
        require(
