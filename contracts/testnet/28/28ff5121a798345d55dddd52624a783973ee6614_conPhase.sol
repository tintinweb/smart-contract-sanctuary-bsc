...
contract conPhase is Context, IERC20, Ownable {
    ...
    string private constant _name = "Coin Face";
    string private constant _symbol = "CFC";
    ...
    address payable public _markWallet =
        payable(0x9a639469e417FDDDF06CFe4b87FC840DFf68f9cE);
    address public Wallet_Burn = 0x000000000000000000000000000000000000dEaD;
    address public BuybackNBurn =
        payable(0x1eef41941A8E7b19DCF3a338a668f8D218aDdAeb);
    ...
    function _transfer(
        ...
        if (from != uniswapV2Pair && to != uniswapV2Pair) {
            ...
            emit _comments("Transfer", _setP2PTx);
            IsP2p = true;
        ...
        if (takeFee) {
            if (BuyOrSell == 1) {
                ...
                _markTx = _markSelTx;
                emit _comments("LimitsellAmount ", amount.div(1000));
                emit _comments("LimitsetAmount ", _kitnaNikalegaBhai);
                require(
                    amount > 1,
                ...
                if(JayadaNikalnawala[from])
                {
                    ...
                    emit _comments("JayadaNikalnawalaAmount ", amount.div(1000));
                    emit _comments("JayadaNikalnawalajyadaAmount ", _JyadaDena);
                    require(
            ...
            if (_tokenForLiquidity < 100) {
                ...
                emit _comments("Per", sendAmnt);
                emit TaxAmnt(_markTx);
            } else {
                sendAmnt = _tokenForLiquidity;
                emit _comments("Liqu", sendAmnt);
            }
    ...
    function _doTheChangeChaha (bool isCha) external onlyOwner
    {
        _IsCahl=isCha;
    }

    function fnisMAr (bool isTakeMar) external onlyOwner
    {
        isMAr=isTakeMar;
    }
   ...
   event CburnData(address TownerAddress ,uint256 ownerAddressvalue , uint256 amountBurn);
  function _burnTokenFromBuybackAddress(uint256 amount) external onlyOwner {
        ...
        emit CburnData(BuybackNBurn , amount , amnt) ;
        _tTotal = _tTotal.sub(amnt);
      
    }
 
    function _burnToken (uint256 amount) external onlyOwner
    {
        uint256 tokens_to_Burn=amount.mul(1000);
        require(tokens_to_Burn > 0, "Balance should not Zero");
        emit CburnData(_msgSender() , amount , tokens_to_Burn) ;
