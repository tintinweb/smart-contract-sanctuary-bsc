    ...
    ProtocolFees
{
    ...
    {
        fillResults = _fillOrder(order, takerAssetFillAmount, signature);
        return fillResults;
    }
    ...
    function _fillOrder(
        ...
        uint256 remainingTakerAssetAmount = order.takerAssetAmount.safeSub(
            orderInfo.orderTakerAssetFilledAmount
        );
        uint256 takerAssetFilledAmount = LibSafeMath.min256(
...
contract LibEIP712ExchangeDomain {
    ...
    string internal constant _EIP712_EXCHANGE_DOMAIN_NAME = "Nft cities";
...
contract SignatureValidator {
    ...
    function _readValidSignatureType(
        ...
        signatureType = _readSignatureType(hash, signerAddress, signature);
    ...
    function _readSignatureType(
        ...
        address signerAddress,
        bytes memory signature
    ) private pure returns (SignatureType) {
    ...
    function _validateHashWithWallet(
        ...
        LibRichErrors.rrevert(
            LibExchangeRichErrors.SignatureWalletError(
                hash,
                walletAddress,
                signature,
                returnData
...
contract ProtocolFees is IProtocolFees, Ownable {
    ...
    function _payProtocolFeeToFeeCollector(
        ...
        (bool didSucceed, bytes memory returnData) = feeCollector.call{
            value: valuePaid
        }(payProtocolFeeData);
        if (!didSucceed) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.PayProtocolFeeError(
                    orderHash,
                    protocolFee,
                    makerAddress,
                    takerAddress,
