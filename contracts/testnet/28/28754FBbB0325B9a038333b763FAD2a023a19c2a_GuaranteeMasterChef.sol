...
contract GuaranteeMasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 poolLimit;  
        uint256 unlockDate;  
        uint256 minTokenIdRange;
        uint256 maxTokenIdRange;
        uint256 stakeAmount;
    }
    ...
    external onlyOwner nonDuplicated(_lpToken) {
        ...
        poolInfo.push(PoolInfo({
            ...
            minTokenIdRange: _minTokenId,
            maxTokenIdRange: _maxTokenId,
    ...
    function activateNfa(uint256 _tokenId, uint256 _pid) external {
    ...
    // update pool variables
        ...
        require(_tokenId > poolInfo[_pid].minTokenIdRange, "Nfa token id check");
         require(_tokenId < poolInfo[_pid].maxTokenIdRange, "Nfa token id check2");

        uint256 stakeAmount = poolInfo[_pid].stakeAmount;
        deposit(_pid, stakeAmount, false);
    ...
    function setRewardPeriod(uint256 _blockCount) external onlyOwner {
        rewardPeriod = _blockCount;
    }

    // Deposit LP tokens to MasterChef for StakedToken allocation.
    function deposit(uint256 _pid, uint256 _amount, bool _shouldHarvest) internal nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
