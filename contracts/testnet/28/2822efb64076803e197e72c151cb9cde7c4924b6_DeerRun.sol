...
contract DeerRun is IBEP20, Ownable {
    ...
    // Name and symbol
    string constant _name = "idkwhy";
    string constant _symbol = "HEyMAMA";
  ...
  if (amount == 0) {
    ...
    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _rewardFeeSell, uint256 _marketingFeeSell, uint256 _devFeeSell) external onlyOwner {
        ...
        totalFeeSell = _liquidityFeeSell.add(_rewardFeeSell).add(_marketingFeeSell).add(_devFeeSell).add(rewardFee);
    }
