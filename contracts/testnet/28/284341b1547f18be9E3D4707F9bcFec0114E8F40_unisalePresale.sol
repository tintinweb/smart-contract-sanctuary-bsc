...
contract unisalePresale {
   ...
   function returnPresale(uint256 _id) public view returns(Presale memory) {
   ...
   function participate(uint256 _id) payable external {
         ...
         require(block.timestamp < presales[_id].endTime , "Presale Not launched.");
    
         // check user participated or no
         // require(participateValue(_id, msg.sender) == 0, "You have already participated.");
         ...
         if (presales[_id].launchpadInfo[1] == 1) {
            require(whitelistValidate(_id,msg.sender) == true,"Your address is not in whitelist of this presale.");
   ...
   function whitelistValidate(uint _id, address _user) internal view returns (bool) {
      if (presales[_id].launchpadInfo[1] == 1) {
   ...
   function checkWhitelist(uint _id) private view returns (bool) {
      if (presales[_id].launchpadInfo[1] == 1) {
         return true;
      }
     return false;
   ...
   function addWlAddr(uint _id, address _addr) external returns (bool) {
      ...
      require(whitelistValidate(_id,_addr) == false, "Address already exists in whitelist!");
      require(checkWhitelist(_id), "This presale doesn't have whitelist method.");
   ...
   function removeWlAddr(uint _id, address _addr) external returns (bool) {
      require(presaleToOwner[msg.sender].length > 0, "you haven't made any presale yet!");
      require(msg.sender == prsIdtoOwner[_id], "You are not founder of this presale.");
      require(whitelistValidate(_id,_addr) == true, "Could not find address in this whitelist.");
      require(checkWhitelist(_id), "This presale doesn't have whitelist method.");
      for (uint i = 0; i < wlAddrs[_id].length; i++) {
   ...
   function payTokens(uint _id, address _token, address _to) external returns (bool) {
       ...
       require(whitelistValidate(_id, _to), "Could not find your address!");
   ...
   function distributePoolBnb(uint _id, address Poolowner) external payable returns (bool) {
         require(presales[_id].pool == msg.sender, 'The caller must be one pool.');
         // require(presales[_id].totalBnbRaised > presales[_id].launchpadInfo[1]*10**18, "The BNB's total raised must exceed Presale SoftCap.");
         // require(block.timestamp > presales[_id].endTime, "Please wait until presale ends, the presale is still running.");
         // uint256 _fee_amount = (presales[_id].totalBnbRaised / 100) * 1;
         // uint256 _amount = presales[_id].totalBnbRaised - _fee_amount;
         bool _pay = payTo(Poolowner,  presales[_id].totalBnbRaised);
         return _pay;
   }
