...
contract MountSapphire is Pausable, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    uint256 public totalDistributedReward = 0;
    ...
    function getRewardToken() public view returns (address, uint256) {
        return (rewardToken.tokenAddress, rewardToken.rewardPerDay);
    }
    ...
    function _validateWithdrawal(address _sender, uint256[] memory blucamonIds)
        ...
        view
    {
        require(blucamonIds.length > 0, "S_MTS_700");
        require(
    ...
    function stake(uint256[] memory _blucamonTokenIds) external whenNotPaused {
        ...
        for (uint256 i = 0; i < _blucamonTokenIds.length; i++) {
            ...
            userInfo[msg.sender].powerAll = userInfo[msg.sender].powerAll.add(
                blucamonPower[_blucamonTokenIds[i]]
            );
    ...
    function emergencyUnstake() public whenNotPaused {
