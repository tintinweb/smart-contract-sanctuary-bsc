...
contract MFETLock is Context, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(address => uint256[])) public locksByAddress;
        ...
        )
    {
        return (
            lockedToken[id].user,
            lockedToken[id].amount,
            lockedToken[id].unlockTime,
            lockedToken[id].isWithdrawn,
            lockedToken[id].token
        );
    }
    ...
    {
        return locksByAddress[token][wallet];
    }
    ...
    function createLock(
        ...
        uint256 totalLockAmount = _createLock(user, amount, unlockTimes, token);
    ...
    function _createLock(
        ...
        require(amount > 0, "MFET : amount cannot be zero");

        uint256 allowance = IERC20(token).allowance(
            _msgSender(),
            address(this)
        );
        require(allowance >= amount, "MFET : allowance error");

        uint256 i;
        for (i = 0; i < unlockTimes.length; i++) {
            require(
                unlockTimes[i] < 10000000000,
                "MFET : timestamp must be smaller then 10000000000"
            );

            // update balance of address
            walletTokenBalance[token][user] =
                walletTokenBalance[token][user] +
                amount;

            _lockId.increment();
            uint256 id = _lockId.current();
            lockedToken[id].user = user;
            lockedToken[id].amount = amount;
            lockedToken[id].unlockTime = unlockTimes[i];
            lockedToken[id].token = token;
            lockedToken[id].isWithdrawn = 0;

            locksByAddress[token][user].push(id);
        }
    ...
    function createMultipleLocks(
        ...
        uint256 totalLockAmount = _createMultipleLocks(
            user,
            amounts,
            unlockTimes,
            token
    ...
    function _createMultipleLocks(
        ...
        require(
            ...
            "MFET : unlock times array length cannot be zero"
        );
        require(
            amounts.length == unlockTimes.length,
            "MFET : amounts and unlock timestamp array length must same"
        );
        ...
        for (a = 0; a < amounts.length; a++) {
            require(amounts[a] > 0, "MFET : amount cannot be zero");
        ...
        require(allowance >= totalLockAmount, "MFET : allowance error");

        for (u = 0; u < unlockTimes.length; u++) {
            require(
                unlockTimes[u] < 10000000000 &&
                    unlockTimes[u] > block.timestamp,
                "MFET : timestamp must be smaller then 10000000000 and bigger then now"
            );

            _lockId.increment();
            uint256 id = _lockId.current();
            lockedToken[id].user = user;
            lockedToken[id].amount = amounts[u];
            lockedToken[id].unlockTime = unlockTimes[u];
            lockedToken[id].token = token;
            lockedToken[id].isWithdrawn = 0;
    ...
    function withdrawTokens(uint256 id) external nonReentrant {
        // set token address to clean view
        address _token = lockedToken[id].token;
        require(
            lockedToken[id].isWithdrawn == 0,
            "MFET : amount already withdrawn"
        );
        require(
            _msgSender() == lockedToken[id].user,
            "MFET : this is not your token"
        );
        require(
            walletTokenBalance[_token][_msgSender()] >= lockedToken[id].amount,
            "MFET : cannot withdraw then you have"
        );
        require(
            getContractTotalTokenBalance(_token) >= lockedToken[id].amount,
            "MFET : cannot withdraw more and contract have"
        );

        lockedToken[id].isWithdrawn = 1;

        // Update balance in address
        walletTokenBalance[_token][_msgSender()] =
            walletTokenBalance[_token][_msgSender()] -
            lockedToken[id].amount;

        // Everything is ok now, transfer tokens to wallet address
        require(
            IERC20(_token).transfer(_msgSender(), lockedToken[id].amount),
            "MFET : error while transfer tokens"
        );

        emit LogWithdrawal(_token, _msgSender(), lockedToken[id].amount);
    }
}
// Made with love.