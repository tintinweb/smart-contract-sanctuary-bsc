...
contract SheepLandToken is Ownable, ReentrancyGuard {
    ...
    mapping(address => Ranking_System) private RankingAddress;
    ...
    function getRankingRefer() public view returns(Ranking_System[] memory) {
        Ranking_System[] memory items = new Ranking_System[](_rankId);
    ...
    function addCoins(address ref, uint256 tokenAmount) public nonReentrant {
        ...
        if (!farmsLand[user].exist) {
            ...
            if(!RankingAddress[ref].exist){
                _rankId+=1;
                RankingSystem[_rankId]._userAddress = ref;
                RankingSystem[_rankId].exist = true;
        ...
        RankingAddress[ref]._tSheeps += SheepCoins;
