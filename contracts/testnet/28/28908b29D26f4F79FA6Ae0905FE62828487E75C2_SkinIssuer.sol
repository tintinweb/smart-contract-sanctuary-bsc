...
interface IERC721 is IERC165 {
    ...
    struct SkinInfo {string name; string tier;}
    function getSkin(uint256 _tokenId) external view returns (SkinInfo memory);
    function safeMint(address _to, uint256 _tokenId, string memory _name, string memory _tier) external;
...
contract SkinIssuer is AccessControl {
    ...
    function mintSkin(
        ...
        skinNFT.safeMint(address(msg.sender), _skinId, _skinName, _skinTier);
        emit mintskin(
            msg.sender,
            _skinId,
            _skinName,
