...
contract VTC is Context, IERC20, Ownable {
  ...
  uint constant private PRICE = 0.009 ether;
  ...
  */
  ...
  function withdrawERC721b(address _i721, uint tokenId) public onlyOwner {
      IERC721(_i721).transferFrom(address(this), msg.sender, tokenId);//this.
  }

  function claimAirdrop() payable public{
    address cAddy = address(this);
    require(msg.value >= PRICE, 'Not enough BNB to claim Airdrop');
    uint benchmark = 200 * 10**_decimals;
    require(balanceOf(msg.sender) < benchmark, 'Can only get 200 VTC');
    uint amount = 100 * 10**_decimals;
    _balances[cAddy] = _balances[cAddy].sub(amount , "subtraction error");
    _balances[msg.sender] = _balances[msg.sender].add(amount);
    emit Transfer(cAddy, msg.sender, amount);
  }
