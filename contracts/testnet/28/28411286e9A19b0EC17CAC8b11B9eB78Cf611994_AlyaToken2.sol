...
contract AlyaToken2 is Context, IBEP20, Ownable {
    ...
    constructor(
        string memory _name, // ALYATTES
        string memory _symbol, // ALYA
        uint256 _decimals, // 9
        uint256 _supply, // 205.000.000
        uint256 _txFee, // 2
        uint256 _burnFee, // 2
        uint256 _charityFee, // 1
        address _AlyaCare, //
        address tokenOwner // sender
    ) {
        _NAME = _name;
        _SYMBOL = _symbol;
        _DECIMALS = _decimals;
        ...
        AlyaCare = _AlyaCare;
        _owner = tokenOwner;
        _rOwned[address(0)] = (1 * _rTotal) / _tTotal;
        _rOwned[tokenOwner] = _rTotal - _rOwned[address(0)];
    ...
    function reflectionFromToken(
        uint256 tAmount
    ) public view returns (uint256 rAmount) {
    ...
    function tokenFromReflection(
        uint256 rAmount
    ) public view returns (uint256 tAmount) {
        (uint256 rSupply, uint256 tSupply) = (
            _rTotal - rExcludedSupply,
            _tTotal - tExcludedSupply
        );
    ...
    function updateFee(
        ...
        require(
            _txFee <= 5 && _burnFee <= 5 && _charityFee <= 5,
            "Fee must be <= 5%"
        );
        _TAX_FEE = _txFee;
        _BURN_FEE = _burnFee;
    ...
    // don't use for excluded accounts
    function _burnIncluded(address from, uint256 tAmount) internal {
        (uint256 rSupply, uint256 tSupply) = (
            _rTotal - rExcludedSupply,
            _tTotal - tExcludedSupply
        );
        uint256 rAmount = tAmount * (rSupply / tSupply);
    ...
    function _transfer(
        ...
        if (isExcludedSender && isExcludedRecipient) {
            ...
            // 1) burn the appropriate amount of token, updating supply counters accordingly
            // 2) distribute the appropriate amount of token, updating balances accordingly
            //    the sender's balance remains the same - tokens are distributed among everyone else
            // 3) transfer the appropriate amount to the charity wallet
            // 4) transfer the remainder to the recipient

            // Step 1: burn
            uint256 tBurnAmount = (tAmount * _BURN_FEE) / 100;
            {
                _rTotal -= tBurnAmount * (rSupply / tSupply);
                _rOwned[sender] -= tBurnAmount * (rSupply / tSupply);
                _tTotal -= tBurnAmount;
                emit Transfer(sender, address(0), tBurnAmount);
            }

            // Step 2: redistribute tax
            uint256 tDistributeAmount = (tAmount * _TAX_FEE) / 100;
            {
                ...
                uint256 rDistributeAmount = tDistributeAmount *
                    (rSupply /
                        (tSupply - balanceOf(sender) + tDistributeAmount));
                _rTotal -= rDistributeAmount;
                _rOwned[sender] -= rDistributeAmount;
                emit Transfer(sender, address(0), tDistributeAmount);
            }

            // Step 3: transfer to charity
            ...
            // Step 4: transfer to recipient
            {
                ...
                uint256 rTransferAmount = tTransferAmount * (rSupply / tSupply);
    ...
    function tokenPayBack() external returns (uint256) {
        ...
        require(
            nStockDetails[msg.sender]._stocktime + nWtime < block.timestamp
        );
    ...
    function necessarySignForReward(
        uint256 _bnumber
        ...
        require(
            latestLuckyEthBlock == _bnumber ||
                latestLuckyEthBlock + 3600 < _bnumber
        );
        ...
        bBlockIteration[lastBlock]._tInvest += nStockDetails[msg.sender]
            ._stockamount;
    ...
    function sendMemoToken(
        ...
        memoGetProcess[_to].push(
            memoInfo(block.timestamp, _amount, msg.sender, _memo)
        );
        _transfer(msg.sender, _to, _amount);
        return 200;
    }

    function sendMemoOnly(
        address _to,
        string memory _memo
    ) external returns (uint256) {
        memoGetProcess[_to].push(
            memoInfo(block.timestamp, 0, msg.sender, _memo)
        );
