...
contract Farming is Ownable {
    ...
    function update() internal {
        uint256 rewardLeft = rewardToken.balanceOf(address(this));
        ...
        // If not enough reward tokens left
        // just give it all
        // and calculate the next block to reward
        if (reward > rewardLeft) {
            reward = rewardLeft;
            _finalRewardBlock = _lastRewardBlock.add(reward.div(_stakeRewardPerBlock));
        }
    ...
    function stake(uint256 amount) external {
        ...
        if (staker.amount > 0) {
            ...
            rewardToken.safeTransfer(msg.sender, pending); // return pending reward
        }
    ...
    function unstake(uint256 amount) external {
        ...
        lpToken.safeTransfer(msg.sender, amount); // return stake amount
        rewardToken.safeTransfer(msg.sender, pending); // return pending reward
    ...
    function getStakeReward(address stakerAddr) external view returns (uint256) {
        ...
        if (block.number > _lastRewardBlock && _totalStake != 0) {
            uint256 multiplier = block.number.sub(_lastRewardBlock);
            uint256 reward = multiplier.mul(_stakeRewardPerBlock);
            accAmountPerShare = _accAmountPerShare.add(reward.mul(1e12).div(_totalStake));
        }
    ...
    function redeemStakeReward() external {
        ...
        require(pending < rewardToken.balanceOf(address(this)), "not enough rewardTokens to redeem");
