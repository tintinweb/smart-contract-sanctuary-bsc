...
import "./SeedERC20.sol";
import "hardhat/console.sol";

contract Seed is ISeed,SeedERC20,Ownable{
    ...
    function initialize(string memory name,
        ...
        uint256 matureTime,
        address shop,
        address farm) external override {
        require(msg.sender == _factory, 'CREATE SEED: FORBIDDEN'); // sufficient check
        _farm = farm;
        _yield = yield;
        _matureTime = matureTime;
        _shop = shop;
        super._initialize(name,symbol,0);
    ...
    modifier onlyOperator() {
        console.log("msgsend is",_msgSender());
        console.log("_farm is",_farm);
        console.log("_factory is",_factory);
        require(owner() == _msgSender()
            || (_shop != address(0) && _shop == _msgSender())
            || (_farm != address(0) && _farm == _msgSender())
            || (_factory != address(0) && _factory == _msgSender()), "Ownable: caller is not the operator");
        _;
    }
    ...
    function decimals() public view override(SeedERC20,ISeed) returns (uint8){
    ...
    function burnFrom(address account,uint256 amount) public override(SeedERC20,ISeed) onlyOperator{
        super.burnFrom(account,amount);
    }

    function permit(address owner, address spender, uint256 value,uint256 nonce, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external override{
        require(deadline == 0 || block.timestamp <= deadline, "Seed/permit-expired");
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'Seed: INVALID_SIGNATURE');
...
//        require(permitData.deadline == 0 || block.timestamp <= permitData.deadline, "Seed/permit-expired");
//        nonces[permitData.owner] = nonces[permitData.owner]+1;
//        require(permitData.nonce == nonces[permitData.owner], "Seed/invalid-nonce");
//
...
//interface ISeed is IERC20Metadata{
    ...
    function initialize(string memory name, string memory symbol, uint256 yield, uint256 matureTime, address shop, address farm) external virtual;
...
contract SeedERC20 is Context, IERC20, IERC20Metadata,Initializable {
    ...
    function _initialize(string memory name_, string memory symbol_, uint8 decimals_) public initializer {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
    }
...
contract SeedFactory is Ownable{
    ...
    mapping(string => mapping(string => mapping(uint256 => mapping(uint256 => address)))) public getSeed;

    address[] public allSeeds;

    event CreateSeed(address indexed seed);


    constructor (address _shop,address _farm) {
        shop = _shop;
        farm = _farm;
    }

    function createSeed(string memory name,
        string memory symbol,
        uint256 yield,
        uint256 matureTime) external onlyOwner returns(address seed){

        bytes memory bytecode = type(Seed).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(name, symbol,yield,matureTime));
        assembly {
            seed := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ISeed(seed).initialize(name, symbol,yield,matureTime,shop,farm);

        getSeed[name][symbol][yield][matureTime] = seed;
        allSeeds.push(seed);
