...
contract Crypto_Mischief {
    ...
    //for testing only remove in mainnet
    function updateLotteryStep(uint256 value) external {
        require(msg.sender == owner);
        LOTTERY_STEP = value; 
    }
    
    //for testing only remove in mainnet
    function updateTopDepositStep(uint256 value) external {
        require(msg.sender == owner);
    ...
    function buyEggs(address ref, uint256 amount) public nonReentrant {
        if(!contractStarted){
            if(buyCount[msg.sender] >= 2) //interacting to buy in the contract 2 times before launch will be blacklisted.
            {
                buyCount[msg.sender]++;
