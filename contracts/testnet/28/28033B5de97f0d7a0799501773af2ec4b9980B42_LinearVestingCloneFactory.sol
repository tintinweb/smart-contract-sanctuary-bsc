 ...
 * @dev
 */
contract LinearVestingClaimMaster is
    Initializable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    /// presale contract
    PresaleMaster public presale;

    /// Total amount of tokens vested
    uint256 public totalVested;
    /// Total amount of tokens claimed
    uint256 public totalClaimed;
    /// Start and entime for the vesting period
    uint256 public startTime;
    /// Entime for the vesting period
    uint256 public endTime;
    /// Basis point for calculation of the initial amount
    ...
    /// maps users address and vesting information
    ...
    /**
     * @notice Initialize a new Linear Vesting contract
     * @param _owner  Elixir platform owner
     * @param _presale presale contract address
     * @param _tokenAddress Project token
     * @param _startBps Basis points of the initial amount upfront
     * @param _startTime Vesting period start time
     * @param _endTime Vesting period end time
     */
    function initialize(
        address _owner,
        address _presale,
        ...
        // addresses check
        require(_owner != address(0), "LV: Invalid owner address");
        owner = _owner;

        require(_presale != address(0), "LV: Invalid presale address");
        presale = PresaleMaster(_presale);

        require(_tokenAddress != address(0), "LV: Invalid token address");
        projectToken = _tokenAddress;

        // check that the startTime is bigger than presale.endtime
        require(_startTime > presale.presaleEndTime(), "LV: Invalid timings 1");
        require(
            _startTime >= block.timestamp && _endTime > _startTime,
            "LV: Invalid timings 2"
        );
    ...
    /**
     * @notice Creates a new Linear Vesting claim schedule
     * @notice This function requires the correct allowance
     * @notice of project tokens before being called
     * @return bool
     */
    function createVesting() external returns (bool) {
        // Make sure the owner has provided the right allowance
        uint256 _totalTokens = presale.presaleSupply();
        require(
            _totalTokens <=
                IERC20Upgradeable(projectToken).allowance(
                    msg.sender,
                    address(this)
                ),
            "LV: Make sure to add enough allowance"
        );

        uint256 _length = presale.getInvestorsCount();
        uint256 _tokensAmount;

        // add vesting schedule for each user
        // iterate over the investors data on presale cotract
        for (uint256 j = 0; j < _length; j++) {
            address _user = presale.investorsList(j);
            (, , _tokensAmount) = presale.investorDetails(_user);
            uint256 _startAmount = (startBps == 0)
                ? 0
                : (_tokensAmount * startBps) / 10000;
            addUserVesting(_user, _tokensAmount, _startAmount);
        }

        // receives from owner the total tokens for the vesting
        totalVested += _totalTokens;
        emit VestingAdded(projectToken, _length, _totalTokens);
     ...
     * @dev private function, only called from createVesting
     */
    function addUserVesting(
        ...
        require(_user != address(0), "LV: Invalid address");
        require(
            _amount > 0 && _startAmount <= _amount,
            "LV: Invalid amount values"
        );
    ...
    function claim() external nonReentrant whenNotPaused returns (bool) {
        ...
        require(amount > 0, "LV: No claimable tokens available");
        userVesting[msg.sender].claimed += amount;
        totalClaimed += amount;
        emit Claimed(projectToken, msg.sender, amount);
        IERC20Upgradeable(projectToken).safeTransfer(msg.sender, amount);
        return true;
    }
        ...
        returns (uint256 claimableAmount)
    {
        ...
        if (block.timestamp < endTime) {
            uint256 timePassedRatio = ((block.timestamp - startTime) * 10**18) /
                (endTime - startTime);
...
import "./LinearVestingClaimMaster.sol";

contract LinearVestingCloneFactory is Ownable {
    /// linear vesting claim implementation contract
    address immutable linearVestingImplementation;

    /// Address of the last clone created so that it
    // can be returned offchain through a view function
    address private newVestingClone;

    /// event new presale event created
    event NewLinearVestingEvent(address indexed contractAddress);

    using Clones for address;

    /**
     * @notice Constructor
     * Only factory is deployed and it create an instance of de Linear Vesting
     * implementation contract
     */
    constructor() {
        linearVestingImplementation = address(new LinearVestingClaimMaster());
    }
    ...
    function createVesting(
        ...
        address clone = Clones.clone(linearVestingImplementation);
        newVestingClone = clone;
        LinearVestingClaimMaster(clone).initialize(
            owner(),
            _presale,
            _tokenAddress,
            _startBps,
        ...
        emit NewLinearVestingEvent(clone);
    }

    /**
     * @notice Address of the last presale clone created
     */
    function getNewVestingClone() external view returns (address) {
        return newVestingClone;
    }
}
