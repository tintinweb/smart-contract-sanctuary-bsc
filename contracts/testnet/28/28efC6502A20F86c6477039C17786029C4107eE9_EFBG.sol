...
contract EFBG is IERC20, Ownable {
    ...
    string constant _name = "EFB Giveaway V2";
    string constant _symbol = "$EFBGV2";
    ...
    uint256 public buyGiveawayFee = 3;
    ...
    uint256 public sellGiveawayFee = 4;
    ...
    constructor() {
        ...
        deployer = 0x7ab4Cc54C37BF34979F3aF0e9Ee949ebE9A11039;
        ...
        giveawayFeeReceiver = 0xC772558c3Ab669362273BEA212CE08FAc5aa4976;
        marketingReceiver = 0xC772558c3Ab669362273BEA212CE08FAc5aa4976;
    ...
    function updateBuyFees(
        ...
        buyRewardFee = reward;
        buyGiveawayFee = giveaway;
        buyLiquidityFee = liquidity;
        buyMarketingFee = marketing;

        buyTotalFees = reward.add(giveaway).add(liquidity).add(marketing);
        require(buyTotalFees <= 40, "Fees can not be greater than 30%");
    }
    ...
    function updateSellFees(
        ...
        sellGiveawayFee = giveaway;
        sellLiquidityFee = liquidity;
        sellMarketingFee = marketing;

        sellTotalFees = reward.add(giveaway).add(liquidity).add(marketing);
    ...
    function updateSwapPercentages(
        ...
        rewardSwap = reward;
        giveawaySwap = giveaway;
        liquiditySwap = liquidity;
        marketingSwap = marketing;

        totalSwap = reward.add(giveaway).add(liquidity).add(marketing);
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 tokensToMarketing = contractTokenBalance.mul(marketingSwap).div(
            totalSwap
        );
        uint256 tokensToRewardAndGiveaway = contractTokenBalance
            .sub(tokensToLiquidity)
            .sub(tokensToMarketing);

        if (tokensToRewardAndGiveaway > 0) {
            swapTokensForTokens(tokensToRewardAndGiveaway, REWARD);

            uint256 swappedTokensAmount = IERC20(REWARD).balanceOf(
                address(this)
            );

            uint256 rewardAndGiveawayFee = rewardSwap.add(giveawaySwap);

            uint256 tokensToGiveAway = swappedTokensAmount
                .mul(giveawaySwap)
                .div(rewardAndGiveawayFee);

            uint256 tokensToReward = swappedTokensAmount.sub(tokensToGiveAway);

            if (tokensToGiveAway > 0) {
                try
        ...
        if (tokensToMarketing > 0) {
            ...
            if (swappedTokensAmount > 0) {
                (bool charitySuccess, ) = payable(marketingReceiver).call{
