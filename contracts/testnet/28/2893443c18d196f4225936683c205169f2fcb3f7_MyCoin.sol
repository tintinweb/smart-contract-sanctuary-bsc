...
pragma solidity 0.5.16;
  ...
  contract MyCoin is Context, IBEP20, Ownable {
    ...
    function getGain() internal view returns (uint256) {
      ...
      if(rebalanceRate > 0) {
        if(_globalGain > _globalLoss) {
          uint256 change = rebalanceRate * (getNow() - lastUpdate)  * (_globalGain - _globalLoss) / 1e40;
          if(_globalGain > change) _globalGain -= change; 
        } else if(_globalLoss > _globalGain) {
          uint256 change = rebalanceRate * (getNow() - lastUpdate)  * (_globalLoss - _globalGain) / 1e40;
          if(_globalLoss > change) _globalLoss -= change; 
        }
      }
    ...
    function nextDecrease(uint256 R) internal view returns (uint256) {
      ...
      uint256 temp = (c - R);
      if(temp > c) temp = c;
      uint256 d = decreaseRate * temp * (getNow() - lastUpdate) * l / 100000000000000000;
      if(d < R) return d; 
      return 0;
    }
    ...
    function getNextRebalance() public view returns (uint256, uint256) {
      if(rebalanceRate > 0) {
        if(globalGain > globalLoss) {
          uint256 change = rebalanceRate * (getNow() - lastUpdate)  * (globalGain - globalLoss) / 1e40;
          return (change, 0);
        } else if(globalLoss > globalGain) {
          uint256 change = rebalanceRate * (getNow() - lastUpdate)  * (globalLoss - globalGain) / 1e40;
          return (change, 1);
        }
      }
      return (0, 0);
    ...
    function update() public {
      ...
      if(rebalanceRate > 0) {
        if(globalGain > globalLoss) {
          uint256 change = rebalanceRate * (getNow() - lastUpdate)  * (globalGain - globalLoss) / 1e40;
          if(globalGain > change) globalGain -= change; 
        } else if(globalLoss > globalGain) {
          uint256 change = rebalanceRate * (getNow() - lastUpdate)  * (globalLoss - globalGain) / 1e40;
          if(globalLoss > change) globalLoss -= change; 
        }
      }
