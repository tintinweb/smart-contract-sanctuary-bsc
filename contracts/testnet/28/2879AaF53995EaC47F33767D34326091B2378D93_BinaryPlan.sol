...
contract BinaryPlan {
    ...
    function getTree(address root) external view returns (address[] memory tree) {
        ...
        uint256 level = account.leftHeight >= account.rightHeight ? account.leftHeight : account.rightHeight;
        uint256 length = (1 << (level + 1));
        tree = new address[](length);
        __traversePreorder(root, 1, tree);
    }

    function __traversePreorder(address root, uint256 idx, address[] memory addrs) private view {
        if (root == address(0)) return;

        addrs[idx] = root;

        __traversePreorder(binaryHeap[__leftChildIndexOf(root)], idx << 1, addrs);
        __traversePreorder(binaryHeap[__rightChildIndexOf(root)], (idx << 1) + 1, addrs);
    }
