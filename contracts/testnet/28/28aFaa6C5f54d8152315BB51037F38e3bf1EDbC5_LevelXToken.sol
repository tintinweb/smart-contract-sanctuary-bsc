...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	uint256 constant WEEK_DURATION = 1 weeks;
	uint256 constant EPOCH_DURATION = 15 minutes;

	uint256 constant INITIAL_SUPPLY = 200_000_000e18; // 200M
	...
	uint256 constant DEFAULT_MINIMUM_REWARD_BALANCE_TO_SWAP = 1e18; // 1 WBNB

	uint256 constant DEFAULT_BURN_AMOUNT_TO_BUMP_LEVEL = 100_000e18; // 100k

	uint256 constant DEFAULT_MINIMUM_BALANCE_FOR_REWARDS = 100_000e18; // 100k

	uint256 constant DEFAULT_REBASE_RATE_PER_EPOCH = 0.0001e16; // 0.0001% every 15m

	address constant INTERNAL_ADDRESS = address(1);

	string private name_;
	string private symbol_;

	bool private bypass_;
	bool private inswap_;

	address public router;
	address public WBNB;
	address public pair;

	address[] public pathWBNB;

	uint256 public buyFee;
	uint256 public sellFee;

	uint256 public minimumFeeBalanceToSwap;
	uint256 public minimumRewardBalanceToSwap;
	...
	uint256 public burnAmountToBumpLevel;

	uint256 public minimumBalanceForRewards;

	uint256 public nextRebaseRatePerEpoch;
	mapping(uint256 => uint256) public rebaseRatePerEpoch;

	address[] public rewardIndex;
	mapping(address => RewardInfo) public rewardInfo;

	address[] public accountIndex;
	mapping(address => AccountInfo) public accountInfo;

	mapping(address => bool) public includeContractInRewards;

	mapping(address => bool) public excludeFromTransferPenalty;

	mapping(address => bool) public excludeFromTradeFee;
	...
	function week() public view returns (uint256 _lastWeek)
	{
		return (block.timestamp / WEEK_DURATION) * WEEK_DURATION;
	}

	function epoch() public view returns (uint256 _lastEpoch)
	{
	...
	function initialize(address _owner, string memory _name, string memory _symbol, address _router) public initializer
	{
		...
		excludeFromTransferPenalty[address(this)] = true;
	...
	function addRewardToken(address bankroll, address[] memory _path) external onlyOwner
	{
		require(bankroll != address(0), "invalid address");
		require(_path.length >= 2 && _path[0] == WBNB, "invalid path");
		address _rewardToken = _path[_path.length - 1];
		require(_rewardToken != address(this) && _rewardToken != WBNB, "invalid address");
		RewardInfo storage _rewardInfo = rewardInfo[_rewardToken];
		require(!_rewardInfo.exists, "already exists");
		_rewardInfo.exists = true;
		_rewardInfo.bankroll = bankroll;
	...
	{
		RewardInfo storage _rewardInfo = rewardInfo[_rewardToken];
		require(_rewardInfo.exists, "invalid address");
	...
	function updateRewardPath(address _rewardToken, address[] memory _path) external onlyOwner
	{
		RewardInfo storage _rewardInfo = rewardInfo[_rewardToken];
		require(_rewardInfo.exists, "invalid address");
		require(_path[0] == WBNB && _path[_path.length - 1] == _rewardToken, "invalid path");
		_rewardInfo.path = _path;
	}
	...
	function updateIncludeContractInRewards(address[] memory _accounts, bool _enabled) external onlyOwner
	{
		_updateEpoch();
		for (uint256 _i = 0; _i < _accounts.length; _i++) {
			address _account = _accounts[_i];
			require(_account.isContract(), "invalid address");
			_updateAccount(_account);
			includeContractInRewards[_account] = _enabled;
			_postUpdateAccount(_account, 0);
		}
	}

	function updateExcludeFromTransferPenalty(address[] memory _accounts, bool _enabled) external onlyOwner
	{
		for (uint256 _i = 0; _i < _accounts.length; _i++) {
			address _account = _accounts[_i];
			require(_account != address(this), "invalid address");
			excludeFromTransferPenalty[_account] = _enabled;
		}
	}

	function updateExcludeFromTradeFee(address[] memory _accounts, bool _enabled) external onlyOwner
	{
		for (uint256 _i = 0; _i < _accounts.length; _i++) {
			address _account = _accounts[_i];
			excludeFromTradeFee[_account] = _enabled;
		}
	}

	function withdrawLiquidity() external onlyOwner
	{
		uint256 _balance = IERC20(pair).balanceOf(address(this));
		IERC20(pair).safeTransfer(msg.sender, _balance);
	}
	...
	function claimAll() external returns (uint256[] memory _amounts)
	{
		...
		for (uint256 _i = 0; _i < rewardIndex.length; _i++) {
			...
			emit Claim(msg.sender, _rewardToken, _amount);
		}
		return _amounts;
	}
	...
	function _updateEpoch() internal
	{
		...
		while (_lastWeek > lastWeek) {
			...
			{
				// perform rebases
				uint256 _rate = _exp(100e16 + rebaseRatePerEpoch[weekIndex], _epochs);
	...
	function _updateAccount(address _account) internal
	{
		...
		while (lastWeek > _accountInfo.lastWeek) {
			...
			{
				// perform rebases
				uint256 _rate = _exp(100e16 + rebaseRatePerEpoch[_accountInfo.weekIndex], _epochs);
				...
				for (uint256 _i = 0; _i < rewardIndex.length; _i++) {
					...
					_accountRewardInfo.rewardDebt = 0;
				}
			}
			...
			{
				// accounts for level increments
				if (_accountInfo.activeBalance > 0) {
					_accountInfo.level++;
				}
			}
		}
	...
	function _postUpdateAccount(address _account, int256 _levelBump) internal
	{
		...
		uint256 _newActiveBalance = _balance < minimumBalanceForRewards || (!includeContractInRewards[_account] && _account.isContract()) ? 0 : _balance;
		uint256 _newLevel = _levelBump >= 0 ? _oldLevel + uint256(_levelBump) : uint256(-_levelBump) >= _oldLevel ? 0 : _oldLevel - uint256(-_levelBump);
	...
	function _transfer(address _from, address _to, uint256 _amount) internal override
	{
		...
		if (_from == pair) {
			// buying
			uint256 _feeAmount = excludeFromTradeFee[_to] ? 0 : _amount * buyFee / 100e16;
		...
		if (_to == pair) {
			// selling
			uint256 _feeAmount = excludeFromTradeFee[_from] ? 0 : _amount * sellFee / 100e16;
			super._transfer(_from, _to, _amount - _feeAmount);
			if (_feeAmount > 0) {
				super._transfer(_from, address(this), _feeAmount);
			}
		...
		{
			// converts fees to WBNB
			_updateAccount(address(this));
			uint256 _balance = balanceOf(address(this));
			if (_balance >= minimumFeeBalanceToSwap) {
				inswap_ = true;
				uint256 _swapAmount = _balance * (100e16 - (20e16 / 2)) / 100e16;
				uint256 _wbnbAmount = _swapToWBNB(_swapAmount);
				_injectWithWBNB(_balance - _swapAmount, _wbnbAmount * (20e16 / 2) / 100e16);
				inswap_ = false;
				return;
			}
		}
		...
		{
			// converts WBNB to reward tokens
			uint256 _wbnbAmount = IERC20(WBNB).balanceOf(address(this));
			if (_wbnbAmount > minimumRewardBalanceToSwap) {
				for (uint256 _i = 0; _i < rewardIndex.length; _i++) {
					address _rewardToken = rewardIndex[_i];
					_swapWBNBToReward(_rewardToken, _wbnbAmount / rewardIndex.length);
				}
				return;
			}
		}
	...
	{
		if (bypass_) return;
		require(_amount <= uint256(type(int256).max), "invalid amount");
		_updateEpoch();
		if (_from != address(0)) {
	...
	function _afterTokenTransfer(address _from, address _to, uint256 _amount) internal override
	{
		...
		if (_from == _to) return;
		if (_from != address(0)) {
			_postUpdateAccount(_from, excludeFromTransferPenalty[_from] || excludeFromTransferPenalty[_to] ? int256(0) : -1);
		}
		if (_to != address(0)) {
	...
	function _swapToWBNB(uint256 _amount) internal returns (uint256 _wbnbAmount)
	{
		return IUniswapV2Router(router).swapExactTokensForTokens(_amount, 0, pathWBNB, address(this), block.timestamp)[pathWBNB.length - 1];
	}

	function _injectWithWBNB(uint256 _amount, uint256 _wbnbAmount) internal
	{
		IUniswapV2Router(router).addLiquidity(address(this), WBNB, _amount, _wbnbAmount, 0, 0, address(this), block.timestamp);
	}

	function _swapWBNBToReward(address _rewardToken, uint256 _wbnbAmount) internal
	{
		RewardInfo storage _rewardInfo = rewardInfo[_rewardToken];
		uint256 _rewardAmount = IUniswapV2Router(router).swapExactTokensForTokens(_wbnbAmount, 0, _rewardInfo.path, address(this), block.timestamp)[_rewardInfo.path.length - 1];
