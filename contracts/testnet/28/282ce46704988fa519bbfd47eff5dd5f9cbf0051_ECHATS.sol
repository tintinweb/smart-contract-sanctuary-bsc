...
contract ECHATS is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Constructor
  constructor(string memory name, string memory symbol, string memory baseTokenURI, address dev2) ERC721(name, symbol) {
      // Save URI
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "ECHATS: minting has not started yet");
    require(_mintAmount > 0, "ECHATS: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "ECHATS: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "ECHATS: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "ECHATS: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "ECHATS: zero address");
    require(_amount > 0, "ECHATS: you have to mint at least one");
    require(lastSupply != 0,  "ECHATS: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"ECHATS: URI query for nonexistent token");
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      ...
      uint256 devShare = _amount.sub(mintShare).div(2);
