...
abstract contract Ownable is Context {
    ...
    /**
     * @dev Transfers ownership of the contract to a new account (`newOwner`).
     * public function without access restriction.
     */
...
library IdToAddressBiMap {
  ...
  function hasId(Data storage self, uint64 id) public view returns (bool) {
  ...
  function getAddressAt(Data storage self, uint64 id) public view returns (address) {
    require(hasId(self, id), "INVALID_ID");
    return self.idToAddress[id + 1];
  }

  function getId(Data storage self, address addr) public view returns (uint64) {
