...
contract Testnet is ERC20, Ownable {
	...
	constructor() ERC20("Testnet", "Testnet") {
	   ...
	   uniswapV2Router = IUniswapV2Router(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
	...
	function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20){      
		...
		if (!swapping && canSwap && swapAndLiquifyEnabled && isAutomatedMarketMakerPairs[recipient]) {
			...
			if(tokenToSwap >= swapTokensAtAmount) 
			{
				...
				if(liquidityPart > 0) 
				{
				    uint256 liqudityToken = swapTokensAtAmount.mul(tokenToLiquidity).div(tokenToSwap);
				    addLiquidity(liqudityToken, liquidityPart);
					liquidityFeeTotal = liquidityFeeTotal.sub(swapTokensAtAmount.mul(tokenToLiquidity).div(tokenToSwap)).sub(liqudityToken);
				}
