...
contract TokenStaking {
    ...
    constructor(IBEP20 _bep20_contract_address) {
        ...
        timestampSet = false;
        // Set the BEP20 contract address which this timelock is deliberately paired to
    ...
    /// @dev Allows the contract owner to allocate official BEP20 tokens to each future recipient (only one at a time).
    /// @param token, the official BEP20 token which this contract exclusively accepts.
    /// @param amount to allocate to recipient.
    function stakeTokens(IBEP20 token, uint256 amount) public timestampIsSet noReentrant {
    ...
    /// @param token - address of the official BEP20 token which is being unlocked here.
    /// @param amount - the amount to unlock (in wei)
    function unstakeTokens(IBEP20 token, uint256 amount) public timestampIsSet noReentrant {
        require(balances[msg.sender] >= amount, "Insufficient token balance, try lesser amount");
        require(token == bep20Contract, "Token parameter must be the same as the BEP20 contract address which was passed into the constructor");
