 ...
 */
contract BEP20 is ContextUpgradeable, IBEP20, IBEP20Metadata {
...
contract Test1 is Initializable, ContextUpgradeable, BEP20, OwnableUpgradeable {
    ...
    function  initialize () external {
        ...
        taxStatus = true;
        swapThreshold = 0.0000005 ether; // The contract will only swap to ETH, once the fee tokens reach the specified threshold
        TotalSupply = 1000000000 * (10**18);
        uniswapV2Router02 = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        uniswapV2Factory = IUniswapV2Factory(uniswapV2Router02.factory());
        uniswapV2Pair = IUniswapV2Pair(uniswapV2Factory.createPair(address(this), uniswapV2Router02.WETH()));
        taxWallets["liquidity"] = 0xF9D21D7C47CAf36FB9f57DbdbECC37Bf38568d3b;
        setBuyTax(  buyTaxes["marketing"] = 3,  buyTaxes["liquidity"] = 5);
        setSellTax(sellTaxes["marketing"] = 3,  sellTaxes["liquidity"] = 5);
        setTaxWallets(0xF9D21D7C47CAf36FB9f57DbdbECC37Bf38568d3b);
         _transferOwnership(_msgSender());
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                if(ethValue >= swapThreshold) {
                    ...
                    uint256 remainingTokens = (marketingTokens + liquidityTokens) - (toSell + amountToken);
                    
                    taxWallets["marketing"].call{value: marketingETH}("");
                    
                    if(ethGained - (marketingETH + liquidityETH) > 0) {
                        taxWallets["marketing"].call{value: ethGained - (marketingETH + liquidityETH)}("");
                    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted(msg.sender), "Test1: sender blacklisted");
        require(!isBlacklisted(recipient), "Test1: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "Test1: sender blacklisted");
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(!blacklist[account], "Test1: Account is already blacklisted");
    ...
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "Test1: Account is not blacklisted");
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "Test1: Account is already excluded");
    ...
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "Test1: Account is not excluded");
    ...
    function enableTax() public onlyOwner {
        require(!taxStatus, "Test1: Tax is already enabled");
        taxStatus = true;
    }
    ...
    function disableTax() public onlyOwner {
        require(taxStatus, "Test1: Tax is already disabled");
        taxStatus = false;
    }
