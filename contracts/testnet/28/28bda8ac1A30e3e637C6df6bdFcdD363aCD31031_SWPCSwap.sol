...
contract SWPCSwap {
    ...
    function swap(address a1, address a2, uint256 coinNum) public payable {
        if (a1 == coinbs) {
            ...
            require(swapCoinNum.buyCoinNum <= 5000 * 10 ** 18);
            ...
            uint256 burnNum = coinNum * 3 / 100;
            uint256 leftBurnFee =  ISWPC(a1)._maxBurnFee() - ISWPC(a1)._tBurnFeeTotal();

            // sell coin -> basecoin
            IERC20(a1).transferFrom(_msgSender(), address(this), coinNum);
            swapCoinNum.sellCoinNum += coinNum;
            require(swapCoinNum.sellCoinNum <= 5000 * 10 ** 18);
            
            if (leftBurnFee >= burnNum) {
                coinNum = coinNum * (10000 - _n[a1][5]) / 10000;
            } else if (leftBurnFee != 0) {
                coinNum = coinNum * (10000 - _n[a1][5] + 300) / 10000 - leftBurnFee;
            }
