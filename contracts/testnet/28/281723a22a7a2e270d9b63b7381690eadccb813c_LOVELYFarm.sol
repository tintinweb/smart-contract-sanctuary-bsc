...
contract LOVELYFarm is Ownable, ReentrancyGuard {
    ...
    // Finds pool index corresponding to a given token address.
    // function indexOf(address _liquidityToken) public view returns (uint256) {
    //
    //   for (uint256 i = 0; i < poolCount; i++) {
    //         if (address(pools[i].liquidityToken) == _liquidityToken) {
    //             return i;
    //         }
    //   }
    //
    ...
    function liquidityTokenBalanceOfPool(uint256 _poolIdentifier) public view returns (uint256) {
        ...
        return pool.liquidityToken.balanceOf(address(this));
    }
    ...
    function deposit(uint256 _poolIdentifier, uint256 _amount) public {
        ...
        if (_amount > 0) {
            safeLiquidityTokenTransferWithFee(_poolIdentifier, msg.sender, address(this), _amount);
    ...
    nonReentrant
    {
        ...
        if (_amount > 0) {
            ...
            safeLiquidityTokenTransferWithFee(_poolIdentifier, address(this), address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.rewardPerShare).div(1e12);
    ...
    function withdrawEmergency(uint256 _poolIdentifier) public nonReentrant {
        User storage user = pools[_poolIdentifier].users[msg.sender];
        safeLiquidityTokenTransferWithFee(_poolIdentifier, address(this), address(msg.sender), user.amount);
    ...
    function safeLiquidityTokenTransferWithFee(
        uint256 _poolIdentifier,
        address _from,
        address _to,
        uint256 _amount
        ...
        // For withdrawals, subtract fee amount from amount
        if (address(this) != _to) {
            _amount -= _amount.mul(pool.fee).div(1000);
        }

        pool.liquidityToken.safeTransferFrom(_from, _to, _amount);
    }
