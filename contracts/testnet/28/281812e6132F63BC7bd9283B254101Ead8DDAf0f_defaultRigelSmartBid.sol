...
contract defaultRigelSmartBid is  Ownable{
        ...
        ) 
    {
        ...
        tokenData(bidToken,_NFTContractAddress, URIRequired, _multiplier);
    }
    ...
    function tokenData(
        ...
        request_token_info.push(tokenInfo({
            ...
            URIRequireFromNFTs : URIRequired,
    ...
    function DistributeRewardsWithOther3(uint256 pid) public {
        ...
        if (status == true) {
            for (uint256 j = 0; j < temporaryOtherLast3Address.length; j++) {
                ...
                uint256 wUGet = b4TForRand(pid, wallet, rem );
                info.token.transfer(wallet, wUGet);
                emit luckyWinner(wallet, wUGet, block.timestamp);
            } 
        }
    ...
    function distributeRewardsWithRandomness(uint256 pid) public returns (uint256[] memory expandedValues) {
        ...
        if (status == true) {
            for (uint256 i = 0; i < bid.numberOfRandomAddress; i++) {
                ...
                uint256 wUGet = b4TForRand(pid, _wallet, rem ); 
                info.token.transfer(_wallet, wUGet);
                emit luckyWinner(_wallet, wUGet, block.timestamp);            
            } 
        }    
    ...
    function b4TForRand(uint256 pid, address _user, uint256 _remnt) public view returns(uint256 wUGet) {
        CreateBid storage bid = request_data_in_Bidding[pid];
        tokenInfo memory info = request_token_info[pid];
    ...
    function fundTopBidders(uint256 pid) public returns (bool, uint256 rem) {
        CreateBid storage bid = request_data_in_Bidding[pid];
        tokenInfo memory info = request_token_info[pid]; 
        (address FirstTBidder, address secondTBidder, address thirdTBidder) = Top3Bidders(pid);
        ...
        if ((nPOne + nPTwo + nPThree + devShare) < bid.totalBidding) {
            ...
            bid.totalBidding = bid.totalBidding - gtRem;
    ...
    function getUri(uint256 pid, address _user, uint256 tokenOwned) public view returns(bool WIHave) {
        ...
        string memory tokenSet = IERC1155(info.NFTContractAddress).setUri(info.URIRequireFromNFTs);
    ...
    function bInf(uint256 pid, address _user) public view returns(uint256 own) {
