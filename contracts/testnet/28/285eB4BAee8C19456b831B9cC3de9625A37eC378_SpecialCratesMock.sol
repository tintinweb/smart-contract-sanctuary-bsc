...
@author Leo
*/
contract SpecialCratesMock is ERC721Enumerable, AccessControl, Pausable {
  using Counters for Counters.Counter;

  Counters.Counter private _tokenIds;

  mapping(uint256 => uint256) public crateToType;
   ...
   */
  function batchMintCrate(address[] calldata users, uint256[] calldata crateTypes) external onlyRole(DEFAULT_ADMIN_ROLE) {
    for (uint256 i = 0; i < users.length; i++) {
      mintCrate(users[i], crateTypes[i]);
    }
   ...
   */
  function mintCrate(address user, uint256 _crateType) public onlyRole(MINTER_ROLE) whenNotPaused returns (uint256) {
    uint256 id = _tokenIds.current();
    _tokenIds.increment();

    crateToType[id] = _crateType;
    _mint(user, id);
