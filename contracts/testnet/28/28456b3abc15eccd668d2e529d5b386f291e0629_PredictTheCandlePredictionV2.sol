...
PREDICT 5MIN CANDLES EXACTLY AS YOU VIEW THEM.
...
//CONTRACT
contract PredictTheCandlePredictionV2 is Ownable, Pausable, ReentrancyGuard 
{
    ...
    function _safeStartRound(uint256 epoch, uint256 startTimestamp,uint256 lockTimestamp,uint256 closeTimestamp,uint256 houseBetBullAmount, uint256 houseBetBearAmount) internal 
    {

        Round storage rounde = Rounds[epoch];
        rounde.epoch = epoch;

        rounde.startTimestamp = uint32(startTimestamp);
        rounde.lockTimestamp =  uint32(lockTimestamp);
        rounde.closeTimestamp =  uint32(closeTimestamp);
    ...
    function RoundStart(uint256 startTimestamp, uint256 lockTimestamp, uint256 closeTimestamp) external onlyOwnerOrOperator whenNotPaused 
    {
        ...
        _safeStartRound(currentEpoch, uint32(startTimestamp), uint32(lockTimestamp), uint32(closeTimestamp), 0, 0);
        startOnce = true;
    }

    function RoundLock(int256 price, uint256 startTimestamp, uint256 lockTimestamp, uint256 closeTimestamp) external onlyOwnerOrOperator whenNotPaused 
    {
    ...
    function RoundExecute(int256 price, uint256 startTimestamp, uint256 lockTimestamp, uint256 closeTimestamp, uint256 houseBetBullAmount, uint256 houseBetBearAmount) external onlyOwnerOrOperator whenNotPaused 
    {                                                                                               
        ...
        // START
        _safeStartRound(currentEpoch, uint32(startTimestamp), uint32(lockTimestamp), uint32(closeTimestamp),houseBetBullAmount, houseBetBearAmount);                                                                 
