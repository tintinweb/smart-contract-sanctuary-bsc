...
contract TokenVestingWEFI is IERC20, Context, ReentrancyGuard {
    ...
    function _generateCSD(uint256 _cliff, uint256 _start, uint256 _duration) private view returns (uint256, uint256, uint256)
    {
        ...
        if (block.timestamp < _start) { // vesting not started yet hence all same
            newCliff = oldCliff;
            ...
            if (block.timestamp < _cliff) { // cliff has not yet been passed | between start and cliff
    ...
    function _computeReleasableAmount(VestingSchedule memory vestingSchedule) private view returns (uint256)
    {
        ...
        else
        {
            ...
            uint256 releaseableAmount = (vestingSchedule.amountTotal * releaseableSeconds) / vestingSchedule.duration;
    ...
    function claimFromAllVestings() external nonReentrant onlyIfBeneficiaryExists(_msgSender()) returns (bool)
    {
    ...
    function computeAllReleasableAmountForBeneficiary(address beneficiary) external view onlyIfBeneficiaryExists(beneficiary) returns(uint256) {
        uint256 vestingSchedulesCountByBeneficiary = getVestingSchedulesCountByBeneficiary(beneficiary);
