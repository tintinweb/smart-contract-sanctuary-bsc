...
contract MGNMarketplace is AccessControl, Pausable {
    ...
    function placeItem(
        ...
        IERC20(token).transferFrom(msg.sender, _wallet, wlToken.getFee(price));
         if (IERC721(item).supportsInterface(ERC721InterfaceId)) {
            ...
            require(
                IERC721(item).getApproved(tokenId) == address(this),
                "Item isn't approved"
            );
        ...
        ILAND(item)._lockLand(tokenId, msg.sender);
        ...
        onlyExistItem(marketId)
    {
        ...
        ILAND(itemData._item)._unlockLand(itemData._tokenId, msg.sender);
        tranferItem(itemData, msg.sender);
        ...
        onlyExistItem(marketId)
    {
        ...
        ILAND(itemData._item)._unlockLand(itemData._tokenId, msg.sender);
        tranferItem(itemData, offerData._buyer);
        IERC20(itemData._token).transferFrom(
            offerData._buyer,
            itemData._owner,
            offerData._price * offerData._amount
    ...
    {
        items[marketId]._available = false;
        ILAND(items[marketId]._item)._unlockLand(items[marketId]._tokenId, msg.sender);
        return true;
    }
