    ...
    RewardRateConfigurable
{
    ...
    function activateRockstarRank(
        ...
        _buyAndBurn(_pid, address(pool.want), _amount);
    ...
    function buyBurnTokens(uint256 _pid) external onlyOwner {
        ...
        _buyAndBurn(_pid, address(pool.want), burnableAmount);
    }
    ...
    function _withdraw(uint256 _pid) private {
        ...
        _claim(_pid, msg.sender);
        if (address(pool.want) == wbnbAddress) {
            buyAndBurnAmountBNB += _getAlpacaStrategyReward(_pid, user.shares);
        } else if (address(pool.want) == busdAddress) {
            buyAndBurnAmountBUSD += _getAlpacaStrategyReward(_pid, user.shares);
        }
     ...
     */
    function _buyAndBurn(uint256 _pid, address _address, uint256 _amount) private {
        IERC20Upgradeable(_address).safeTransferFrom(poolInfo[_pid].strategy, buyAndBurnAddress, _amount);
    ...
    /**
     * @notice View function to see alpaca reward tokens on frontend.
     * @param _pid: Pool id where user has assets
     * @param _userShares: User shares in pool
     */
    function _getAlpacaStrategyReward(uint256 _pid, uint256 _userShares) private view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        ...
        return (_userShares * (wantLockedTotal - sharesTotal)) / sharesTotal;
    }
