...
contract AgeioController is Ownable {
  ...
  function claimAgtReward(uint256 _amount) public onlyAgtMasterChef {
    uint256 agtBal = IERC20(agtToken).balanceOf(address(this));
    uint256 amount = _amount > agtBal ? agtBal : _amount;
    IERC20(agtToken).safeTransfer(_msgSender(), amount);
    IERC20(agtToken).safeTransfer(devaddr, amount.div(20));  //5% to developer
  }
  ...
  function swapAgtWithTfuel(uint256 amount) public payable returns (bool) {
    ...
    if (isSwappable) {
      _swapTfuelForTokens(amount, agtToken);
    }
    else {
      safeTransferTfuel(treasury, amount);
    }
    return true;
  }
  ...
  function getAgtAmountFromTfuel(uint256 tfuelAmount) public view returns(uint256) {
    ...
    //decimals
    uint256 res1 = reserve1.mul(10**token0.decimals());
    return tfuelAmount.mul(res1).div(reserve0);
  }
  ...
  function _swapTfuelForTokens(uint256 tfuelAmount, address forTokenAddress) internal {
    ...
    try _thetaswapRouter.swapETHForExactTokens{value: tfuelAmount}(
      0,
      path,
      address(this),
      block.timestamp
