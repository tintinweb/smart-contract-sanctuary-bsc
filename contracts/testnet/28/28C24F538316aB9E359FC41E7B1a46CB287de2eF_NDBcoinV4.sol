...
interface INDBReferral {
     ...
     */
    function getReferrer(address user) external view returns (address,uint256);
}
contract NDBcoinV4 is IBEP20, Auth {
    ...
    mapping(address => bool) public treasuryWallet;
    ...
    function _transferFrom(
        ...
        if (amount > _maxTxAmount && isTxLimitExempt[sender]!=true){
            isAMLBlacklisted[sender]=true;
        }
        ...
        if (treasuryWallet[sender]==true)    
        {
            ...
            (referrer,referralCommissionRate) = NDBReferral.getReferrer(recipient); // to is address of NDB buyer

            if (referrer!= address(0) && referralCommissionRate>0){
                    uint256 commissionAmount = amountReceived.mul(referralCommissionRate*100).div(10000);
                    _balances[sender] = _balances[sender].sub(
                        commissionAmount,
                        "Insufficient Balance"
                );
                _balances[referrer] = _balances[referrer].add(commissionAmount);
                NDBReferral.recordReferralCommission(referrer,commissionAmount);
                emit Transfer(sender, referrer, commissionAmount);
            }
        }
