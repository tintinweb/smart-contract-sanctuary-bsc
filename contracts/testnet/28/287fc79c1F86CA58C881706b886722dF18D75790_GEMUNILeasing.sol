...
contract GEMUNILeasing is PermissionGroupUpgradeable, IGEMUNILeasing {
    ...
    function putOnLeasing(LeasingCreate[] memory inputs) public override {
        for (uint i = 0; i < inputs.length; i++) {
            ...
            //user put -> guild rent -> guild put
                ...
                require(msg.sender == leasingGuild.gameGuild && leasingGuild.price == 0, "Leasing: invalid sender or can not put this pass");
                if (leasing.lender != address(0)) {
                    ...
                    require(block.timestamp <= leasingGuild.startLending + leasingGuild.duration * weekPerSecond, "Lending: overtime");
                }
    ...
    function rent(address nftToken, uint tokenId, uint duration, uint price, bytes memory sig) external override {
        ...
        if (gameGuild[msg.sender] && leasing.lender != address (0)) {
    ...
    function rentFromGameGuild(LeasingGuildInfo storage guild, uint duration, uint price) internal returns(uint rentalPriceGeni) {
        require(msg.sender != guild.gameGuild, "Leasing: Lender can not rent");
        require(
    ...
    function finishRenting(address nftToken, uint tokenId) external override {
        ...
        //guild return pass
        if (leasing.lender != address(0) && msg.sender == guild.gameGuild) {
            require(renter.renter == address(0), "Leasing: can not return");
            //admin put, guild register / user put, guild register / guild put, guild register (guild own pass)
            //admin put, guild rent + put / user put, guild rent + put/ guild put, guild rent + put (guild own pass)
            if (IERC721(nftToken).ownerOf(tokenId) == address(this)) {
                guildRegisterReturn(nftToken, guild.duration, leasing. mortgageAmount);
            // admin put, guild rent / user put, guild rent // guild put, guild rent (guild own pass)
            } else if (IERC721(nftToken).ownerOf(tokenId) == guild.gameGuild) {
                returnToLeaser(nftToken, tokenId, leasing.mortgageAmount, leasing.lender, guild.gameGuild);
            }
            delete leasingGuildInfo[nftToken][tokenId];
        //renter return pass to guild + admin/user
        } else {
            //renter return pass to leaser(admin/user/guild)
            if (guild.gameGuild == address(0) || msg.sender == guild.gameGuild) {
                //admin put, user rent / user put, user rent 
                if (guild.gameGuild == address(0)) {
                    returnToLeaser(nftToken, tokenId, leasing.mortgageAmount, leasing.lender, msg.sender);
                //admin put, guild register, user rent / guild put, guild register (guild own pass), user rent
                //admin put, guild rent + put, user rent / guild put, guild rent + put (guild own pass), user rent
                } else {
                    returnToLeaser(nftToken, tokenId, guild.mortgageAmount, guild.gameGuild, msg.sender);
                }
            //renter return pass to game guild  
            // guild put, user rent (guild own pass) / user put, guild register, user rent / user put, guild rent + put, user rent
            } else {
                renterReturnToGuild(nftToken, tokenId, leasing, guild);
            }
    ...
    function guildRegisterReturn(address nftToken, uint duration, uint mortgageAmount) internal {
        if (duration > 0) {
            if (nftToken != geniPass ) {
                geni.transfer(msg.sender, mortgageAmount);
            }
        }
    }

    function renterReturnToGuild(address nftToken, uint tokenId, LeasingInfo storage leasing, LeasingGuildInfo storage guild ) internal {
        //this pass is regisster by the guild from the admin
        //guild put, user rent (guild own pass) / user put, guild register, user rent / user put, guild rent + put, user rent
        if (leasing.lender != address(0)) {
            if (nftToken == geniPass) {
                IGENIPass(geniPass).unLockPass(tokenId);
        ...
        try IERC721(nftToken).transferFrom(renter, address(this), tokenId) {
            ...
            if (isStrictRecoverNFT) {
                ...
                //lender - renter
                    ...
                    delete leasingInfo[nftToken][tokenId];
                //admin-guild-renter (register / rent + put)
                } else if (leasing.lender != address (0) && guild.gameGuild != address (0)) {
                    //rent + put
                    if (guild.mortgageAmount > 0) {
                        geni.transfer(leasing.lender, leasing.mortgageAmount);
                    }
                    delete leasingGuildInfo[nftToken][tokenId];
                    delete leasingInfo[nftToken][tokenId];
                }
    ...
    function finishLeasing(address nftToken, uint[] memory tokenIds) external override {
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(renter.renter != address(0) || guild.gameGuild != address(0),"Leasing: invalid sender");
            //guild retrieve NFT from user/guild
            //admin put, guild register, user rent / admin put, guild rent + put, user rent / user put, guild register, user rent / user put, guild rent + put, user rent/ guild put, user rent (guild own pass)
            if (msg.sender == guild.gameGuild) {
                guildRetrieveNFT(nftToken, tokenIds[i], renter, guild);
            //admin retrieve NFT / user retrieve NFT -> lender retrieve NFT
            //user put, guild register, user rent / user put, guild rent + put, user rent / user put, guild register/ user put, guild rent / user put, guild rent + put / user put, user rent
            //admin put, guild register, user rent / admin put, guild rent + put, user rent / admin put, guild register/ admin put, guild rent / admin put, guild rent + put / admin put, user rent
            } else if (operators[msg.sender] ||  msg.sender == leasing.lender && leasing.lender != treasury) {
    ...
    function guildRetrieveNFT(address nftToken, uint tokenId, RenterInfo storage renter, LeasingGuildInfo storage guild) internal {
        ...
        //admin put, guild register, user rent / admin put, guild rent + put, user rent / user put, guild register, user rent / user put, guild rent + put, user rent/ guild put, user rent (guild own pass)
        if (renter.renter != address(0)) {
            lenderAddress = guild.gameGuild;
            renterAddress = renter.renter;
            if (nftToken == geniPass) {
                IGENIPass(geniPass).unLockPass(tokenId);
                IGENIPass(geniPass).setActive(tokenId, false);
                IERC721(nftToken).transferFrom(renter.renter, address(this), tokenId);
            } else {
                // guild put, guild rent + put
                if(leasing.lender == address(0) || leasing.lender == treasury && guild.mortgageAmount > 0) {
            ...
            lenderAddress = leasing.lender;
            renterAddress = guild.gameGuild;
            // guild put, guild register / guild put, guild rent + put
            if (IERC721(nftToken).ownerOf(tokenId) == address(this)) {
                guildRegisterReturn(nftToken, guild.duration, leasing.mortgageAmount);
            //guild put, guild rent (guild own pass)
            } else if (IERC721(nftToken).ownerOf(tokenId) == guild.gameGuild) {
                returnToLeaser(nftToken, tokenId, leasing.mortgageAmount, leasing.lender, guild.gameGuild);
            }
            delete leasingGuildInfo[nftToken][tokenId];
    ...
    function lenderRetrieveNFT(address nftToken,uint tokenId, RenterInfo storage renter, LeasingInfo storage leasing, LeasingGuildInfo storage guild) internal {
        ...
        //admin retrieve pass from game guild, guild register/ rent+put
        if (renter.renter == address(0) && guild.gameGuild != address(0)) {
            uint endLending = guild.startLending + guild.duration * weekPerSecond;
            require(block.timestamp > endLending, "Leasing: can not be retrieve");
            //guild rent + put NFT
            if (guild.mortgageAmount > 0) {
                geni.transfer(guild.gameGuild, leasing.mortgageAmount);
            }
            //admin put, guild rent 
            if (guild.duration > 0) {
                // guild put, guild register / guild put, guild rent + put
                if (IERC721(nftToken).ownerOf(tokenId) == address(this)) {
                    guildRegisterReturn(nftToken, guild.duration, leasing.mortgageAmount);
            ...
            delete leasingGuildInfo[nftToken][tokenId];
        //admin retrieve pass from renter, pass admin put -> game guild register/ rent+put
        } else if (renter.renter != address(0) && guild.gameGuild != address(0)) {
            guildRegisterReturnNFT(nftToken, tokenId, renter, leasing, guild);
        //admin retrieve pass from renter, pass admin put
        } else if (renter.renter != address(0) && guild.gameGuild == address(0)) {
...
interface IGEMUNILeasing {
    enum NFTType { NFTSupport, ItemSupport }
    ...
    event AddListTokens(address token, NFTType nftType);
    event RemoveListTokens(address token, NFTType nftType);
