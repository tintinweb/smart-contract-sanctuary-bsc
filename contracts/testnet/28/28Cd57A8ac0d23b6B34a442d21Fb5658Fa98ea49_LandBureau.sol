...
contract LandBureau is ModuleBase, Lockable {
        ...
        returns (uint256 mutAmount, uint256 landPrice)
    {
        ...
        mutAmount = PairPrice(moduleMgr.getPairPrice()).cumulateUTOAmountIn(landPrice);
    }
    ...
    function grouponLand(uint256 amount, address parent) external lock {
        _grouponLand(msg.sender, amount, 0, parent);
    }

    function grouponLand(uint256 amount, uint256 withRewardAmount, address parent) external lock {
        _grouponLand(msg.sender, amount, withRewardAmount, parent);
    }

    //group purchase with reward amount
    function _grouponLand(address account, uint256 amount, uint256 withRewardAmount, address parent) internal {

        if(singleLand) {
           require(ERC721(moduleMgr.getFarmLand()).balanceOf(account) == 0, "u'd have NFT land"); 
        }
        ...
        require(
            mapUserData[account][roundIndex] == 0,
            "u r in this round"
        );

        if(withRewardAmount > 0) {
            (bool resReward, uint256 totalAmountReward, uint256 claimedAmountReward) = RewardAccount(moduleMgr.getRewardAccount()).getRewardInfo(msg.sender);
        ...
        require(amount+withRewardAmount >= needMUTAmount - needMUTAmount * SystemSetting(moduleMgr.getSystemSetting()).getPriceTolerance(0) / 1000, 
        "amount too small");
        uint256 useAccountAmount = needMUTAmount >= withRewardAmount ? needMUTAmount - withRewardAmount: needMUTAmount;

        if(amount > 0 && useAccountAmount > 0) {
            require(
                ERC20(auth.getFarmToken()).balanceOf(account) >= useAccountAmount,
        ...
        //purchase
        _grouponLand(account, needMUTAmount, landPrice);
    }
            ...
            for (i = 1; i <= group_number; ++i) {
                ...
                if (hasParent) {
                    ...
                    if (
                        ...
                        rewardAmount = PairPrice(moduleMgr.getPairPrice()).cumulateUTOAmountIn(rewardInUsdt);
                        rewardType = 2;                        
                    } else {
                ...
                if(ud.account == account) {
                    ...
                    rewardAmount = PairPrice(moduleMgr.getPairPrice()).cumulateUTOAmountIn(rewardInUsdt);// 5% valued of reward
                    UserData storage udInUser = mapUserList[roundIndex][mapUserData[ud.account][roundIndex]];
                    udInUser.rewardAmount += rewardAmount;
                    udInUser.rewardInUsdt += rewardInUsdt;
