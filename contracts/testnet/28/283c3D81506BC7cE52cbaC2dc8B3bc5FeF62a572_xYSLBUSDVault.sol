...
contract xYSLBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    uint256 public tradeTax;// setting tradetax as uint
    ...
    */
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio() whenNotPaused() {
        require(amount > 0,"xyslBUSD: LPamount can't be 0");
        require(isBUSD,"xyslBUSD: Only Deposit BUSD");
        uint Price;
        ...
        uint reservexYSL;
        uint reservesBUSD;
        console.log(amount,"amount",(amount * 3)/10,"(amount * 3)/10");
        console.log("IERC20(path[1]).balanceOf(address(this))",IERC20(path[1]).balanceOf(address(this)));
        if(IERC20(path[1]).balanceOf(address(this)) >= reservexYSL){
            percentxYSL = 30;
            mintUSDy = 2;
            ...
            console.log("reservexYSL",reservexYSL);
            Price = convertBUSDToLP((amount * percentxYSL/100), (reservexYSL * (amount * percentxYSL/100)) / reservesBUSD, Admin.BUSD(), Admin.xYSL(), address(xyslBUSD));
            IReceipt(Admin.USDy()).mint(address(this), (amount * 1/2));
            console.log((amount * 1/2),"usdy minted");
            convertBUSDToLP((amount * 1/2), (amount * 1/2), Admin.BUSD(), Admin.USDy(), address(USDyBUSD));
        ...
        uint USDyAmount  = IReceipt(Admin.USDy()).mintByPrice();
        uint _amount = amount * mintUSDy * (USDyAmount/100);
        IReceipt(Admin.USDy()).mint(address(this),(_amount) / 10 ** 18);
        IReceipt(xyslBUSD_A).mint(address(this),(((_amount) / 10 ** 18) * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
        UserDeposit[user] += (((_amount) / 10 ** 18) * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100);
        USDyOfDollarValue += (_amount) / 10 ** 18;
    ...
    */
    ...
    function withdraw(address user, bool isReciept,uint _amount,address sendTo) nonReentrant external _lastExchangeRatio() whenNotPaused(){
        ...
        restrictTransfer[msg.sender]= block.number; 
        emit Withdraw("xyslBUSD Vault",address(this),user, _amount,block.number,block.timestamp);     
    ...
    */
    ...
    function convertBUSDToLP(uint amountBUSD,uint amountToken2,address path0,address path1,address lp) internal  returns(uint){
        console.log("11");
        ...
        console.log("balance",IERC20(path0).allowance(address(this), Admin.liquidityProvider()),IERC20(path1).allowance(address(this), Admin.liquidityProvider()));
