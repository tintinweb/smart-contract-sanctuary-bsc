...
contract LlamaLand is Context, ERC721Pausable, Ownable, URIStorage {
    using Strings for uint256;

    uint public serialNumber = 0;
    uint public max = 0;
    uint public price = 0.02 * 10 ** 18;

    string public furCid;

    string baseUri = "ipfs://";

    constructor() ERC721("Llama Land", "LL") {
        _pause();
    }

    function claim() whenNotPaused payable external {
        require(serialNumber < max, "Not enough");
        require(msg.value >= price, "Token is not enough");
        (bool success,) = payable(owner()).call{value : msg.value}("");
        require(success, "Payable is failed");
        addUri(serialNumber, furCid);
        uint index = serialNumber;
        serialNumber++;
        _safeMint(_msgSender(), index);
    }

    function invoke(uint newMax) onlyOwner external {
        require(newMax > serialNumber, "The new max value should be more than the serial number");
        _unpause();
        max = newMax;
    }

    function setPrice(uint value) onlyOwner external {
        price = value;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (keccak256(abi.encodePacked(getUri(tokenId))) == keccak256(abi.encodePacked(furCid))) {
    ...
    function setFurCid(string memory cid) onlyOwner external {
        furCid = cid;
    }
    ...
    function draw(uint startTokenId, string[] memory cidList, string memory seed) onlyOwner external {
