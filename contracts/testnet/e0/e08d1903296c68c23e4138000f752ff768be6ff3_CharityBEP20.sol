...
contract InitializableOwnable {
    ...
    modifier notInitialized() {
        require(!_INITIALIZED_, "CHARITY_INITIALIZED");
        _;
    }
...
contract CharityBEP20 is InitializableOwnable {
    ...
    function _transfer(
        ...
        charityAmount = amount.mul(10).div(10000);
        balances[charity] = balances[charity].add(charityAmount);

        burnAmount = amount.mul(5).div(10000);
        balances[address(0)] = balances[address(0)].add(burnAmount);

        feeAmount = amount.mul(5).div(10000);
