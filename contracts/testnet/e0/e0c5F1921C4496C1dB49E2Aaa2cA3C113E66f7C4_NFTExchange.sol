...
contract NFTExchange is Upgradable {
    ...
    * set the address of the super admin
    ...
    function getListOfMintedNFTsByGroupID(string memory _groupId) public view returns (MintedNFT[] memory) {
        uint256 tokenId;
        uint256 length = mintedAmount[_groupId].length;
        MintedNFT[] memory list = new MintedNFT[](length);
        for(uint256 i = 0; i < length; i++) {
            tokenId = mintedAmount[_groupId][i];
            list[i] = MintedNFT(
                tokenId, 
                PEM721(pem721Address).ownerOf(tokenId)
            );
        }
        return list;
    }
...
contract Upgradable is ReentrancyGuarded {
    ...
    mapping (string => uint256[]) mintedAmount;       // Group ID -> minted tokenIds
    ...
    function mintBatch721(
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            mintedAmount[_str[1]].push(_tokenIds[i]);
        }
