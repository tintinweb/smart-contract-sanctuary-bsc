...
contract ProjectLaunchFactory {
  ...
  function createSale(
    ...
    totalTokens = (_seed_details[4] * _seed_details[0]) + ((10000 - round1MultT[_sale_details[0]]) * (_seed_details[4] * _seed_details[0]) / 10000);
    totalTokens = totalTokens + (_presale_details[4] * _presale_details[0]) + ((10000 - round2MultT[_sale_details[0]]) * (_presale_details[4] * _presale_details[0]) / 10000);
    totalTokens = totalTokens + (_community_details[4] * _community_details[0]) + ((10000 - round3MultT[_sale_details[0]]) * (_community_details[4] * _community_details[0]) / 10000);
  ...
  function liquidityCalc(uint256 _hardcap, uint256 _mult, uint256 _rate, uint256 _percentage) internal pure returns(uint256) {
    return (_hardcap * _mult / 10000 * _rate  * _percentage / 100);
  }
...
contract Sale is ReentrancyGuard {
    ...
    function userDeposit (uint256 _round) public payable nonReentrant {
        ...
        require(saleStatus.sale_raised_amount * sale_info[_round].token_rate <= ERC20(sale_details.sale_token).balanceOf(address(this)), "Token remain error");
    ...
    function finalizeSale (uint8 _round) public nonReentrant {
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                ...
                //uint256 liquidityToken = (sale_status[_round].sale_sold_amount * (10000 - sale_info[_round].mult) / 10000 * sale_info[_round].token_rate  * sale_info[_round].liquidity / 100);
                uint256 liquidityToken = (sale_status[_round].sale_sold_amount * (10000 - sale_info[_round].mult) / 10000 * sale_info[_round].liquidity / 100);
                ...
                if(liquidityAmount > 0) {
                    //ERC20(sale_details.sale_token).transferFrom(address(this), address(saleVaultContract), liquidityToken);
