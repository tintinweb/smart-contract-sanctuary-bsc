...
contract Marketplace is Ownable, AccessControl {
    ...
    bytes32 public constant REGISTERED_CALLER_ROLE = keccak256("REGISTERED_CALLER_ROLE");
    ...
    event ItemListed(uint256 itemId, address listedBy, uint256 listingTime);
    event Whitelisted(address whitelistedUser, uint256 wlTime);

    modifier notListedForSale(uint256 itemId) {
        require(_idToItem[itemId].saleOngoing == false, "Marketplace: item listed for sale");
        _;
    }

    modifier notBlacklisted {
        require(!_isBlacklisted[msg.sender], "Marketplace: blacklisted");
        _;
    }
    
    modifier registeredCaller {
        require(hasRole(REGISTERED_CALLER_ROLE, msg.sender), "Marketplace: unauthorized");
        _;
    }
    
    modifier minterSet {
        require(ERC721Minter != address(0), "Marketplace: minter uninitialized");
        _;
    }
    
    /// @notice Grants default admin and admin role to contract creator/owner
    constructor() {
        _grantRole(DEFAULT_ADMIN_ROLE, 0xcc2bAc561da09811372656C93b12D538e803dA78);
        _grantRole(ADMIN_ROLE, 0xcc2bAc561da09811372656C93b12D538e803dA78);
        _transferOwnership(0xcc2bAc561da09811372656C93b12D538e803dA78);
        _grantRole(ADMIN_ROLE, msg.sender);
    }
    
    function getItemId(address nft, uint256 tokenId) external view returns (uint256) {
        return _tokenToItemId[nft][tokenId];
    }
    ...
    /// @notice Whitelists caller to make them eligible to participate in private presale phase
    function whitelist() external {
        require(adminMint, "Marketplace: whitelisting not active");
        require(_isWhitelisted[msg.sender] == false, "Marketplace: already whitelisted");
    ...
    function enablePresale() external onlyOwner {
        require(adminMint, "Marketplace: admin mint ended");
        adminMint = false;
    ...
    function getUnlockableContent(uint256 itemId) external view returns (string memory) {
        require(msg.sender == _idToItem[itemId].currentOwner, "Marketplace: must be item owner");
    ...
    /// @dev Caller must be a registered smart contract
    function addTrade(
        uint256 itemId, 
        address _from, 
        address _to, 
    ...
    function isPublic() external view returns (bool) {
        return adminMint == false && whitelistPresale == false;
    }
    
    /// @notice Lists an existing NFT
    /// @param _tokenId token ID of NFT
    /// @param _royaltyPercentage royalty percentage multiplied by 10
    /// @param _nftContract contract address of NFT collection
    /// @param _royaltyRecipient royalty recipient address
    /// @param _unlockableContent unlockable content url - optional
    ...
    function listItem(
        ...
        require(_tokenToItemId[_nftContract][_tokenId] == 0, "Marketplace: item already listed");
        if(_isERC1155(_nftContract)) {
            require(IERC1155(_nftContract).balanceOf(msg.sender, _tokenId) >= 1);
        }else {
            require(msg.sender == IERC721(_nftContract).ownerOf(_tokenId), "Marketplace: must be token owner");
        }

        if(adminMint == true || whitelistPresale == true) require(hasRole(ADMIN_ROLE, msg.sender), "Marketplace: unauthorized");
        ...
        emit ItemListed(currentId, msg.sender, block.timestamp);
    ...
    function bulkList(
        ...
        for(uint i = startingTokenId; i <= endingTokenId; i++) {
            listItem(i, royaltyPercentage, nftContract, royaltyRecipient, "");
        }
    }

    function bulkUnlist(
        uint256 startingItemId,
        uint256 endingItemId
    ) external onlyRole(ADMIN_ROLE) {
        for(uint i = startingItemId; i <= endingItemId; i++) {
            unlistItem(i);
        }
    }

    function setUnlockableContent(uint256 itemId, string calldata _unlockableContent) external {
        require(msg.sender == _idToItem[itemId].currentOwner, "Marketplace: must be item owner");
        require(bytes(_idToUnlockableContent[itemId]).length == 0, "Marketplace: uc already set");
        
        _idToUnlockableContent[itemId] = _unlockableContent;
    }
    ...
    function addWhitelisted(address[] calldata _users) external onlyRole(ADMIN_ROLE) {
        for(uint i=0; i < _users.length; i++) {
            _isWhitelisted[_users[i]] = true;
            emit Whitelisted(_users[i], block.timestamp);
        }
    }
    ...
    /// @param _royaltyPercentage royalty percentage multiplied by 10
    /// @param _royaltyRecipient royalty recipient address
    /// @param _tokenUri URI containing the metadata of the NFT
    /// @param _unlockableContent Unlockable content url - optional
    /// @return Item ID of the newly listed NFT
    function mintThenListItemRestricted(
        ...
        string memory _unlockableContent
    ) external minterSet returns (uint256) {
        require(hasRole(ADMIN_ROLE, msg.sender), "Marketplace: unauthorized");
    ...
    /// Regulate admin approval through frontend
    function mintThenListItemPublic(
        ...
        require(!adminMint && !whitelistPresale, "Marketplace: public mint not started yet");
        uint256 tokenId = IERC721Minter(ERC721Minter).mintToken(msg.sender, _tokenUri);
        uint256 itemId = listItem(tokenId, _royaltyPercentage, ERC721Minter, _royaltyRecipient, _unlockableContent);
    ...
    function unlistItem(uint256 itemId) public {
        require(_idToItem[itemId].saleOngoing == false, "Marketplace: cancel sale first");
        require(msg.sender == _idToItem[itemId].currentOwner || hasRole(ADMIN_ROLE, msg.sender), "Marketplace: unauthorized");
        delete _idToItem[itemId];
    }
    
    /// @dev Transfers given NFT to zero address
    /// @param itemId ID of item to burn
    /// @param amount Amount of tokens to burn, only applicable for ERC1155
    function burnItem(uint256 itemId, uint256 amount) external {
        require(msg.sender == _idToItem[itemId].currentOwner, "Marketplace: owner only");
    ...
    /// @param itemId Item ID to gift
    /// @param recipient Address that should receive the NFT
    function giftItemERC721(
        uint256 itemId, 
        address recipient
    ) external notBlacklisted notListedForSale(itemId) {
        require(msg.sender == _idToItem[itemId].currentOwner, "Marketplace: must be token owner");
    ...
    /// @param recipient Address that should receive the NFTs
