...
contract Syndicate is Initializable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function unstakeBoost() external whenNotPaused {
        ...
        if (vaultBalance < amountAvailableForUnstakeBoost) {
            uint256 differenceToMint = amountAvailableForUnstakeBoost -
                (vaultBalance);
            infinityToken.mint(infinityVaultAddress, differenceToMint);
        }
    ...
    function getVaultBalance() public view returns (uint256) {
        return infinityToken.balanceOf(infinityVaultAddress);
    }
    ...
    function _claim_out(address _wallet) internal {
        ...
        infinityVault.withdraw(address(msg.sender), realizedPayout);
        emit Leaderboard(
            _wallet,
            users[_wallet].referrals,
            users[_wallet].deposits,
    ...
    function eject() external whenNotPaused {
        ...
        infinityVault.withdraw(msg.sender, amountAvailableForEject);
...
interface IVault {
    function withdraw(address to, uint256 tokenAmount) external;
}
