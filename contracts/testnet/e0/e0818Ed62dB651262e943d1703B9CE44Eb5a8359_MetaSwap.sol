...
contract MetaSwap is IERC20 {

    using SafeMath for uint256;

    uint256 private _totalSupply = uint256(3000000 * 1 ether);
    uint256 public _swapTokenAmountCap = uint256(3000000 * 1 ether);
    address public _tokenContractAddress;

    string public constant name = "Meta Swap";
    ...
    constructor() {
       ...
       _transfer(address(this),_admin,uint256(3000000 * 1 ether)); 
    }
    ...
    function swapToken(uint256 amount)
        ...
        returns (bool)
    {
        require(_swapIsOpen,"Swapping not yet open");
        IErc20Contract tokenContract = IErc20Contract(_tokenContractAddress); 
        ...
        bool swap = tokenContract.swapToken(msg.sender, amount);
        if(swap){
            _swapTokenAmountCap = _swapTokenAmountCap.sub(amount);
            _transfer(msg.sender, _tokenContractAddress, amount);
             return true;
        }else{
            return false;
        }
    ...
    //set Swapping Status
    function setSwappingStatus() external onlyAdmin{
        _swapIsOpen = !_swapIsOpen;
    }
    modifier whenPaused() {
        // Is pause?
