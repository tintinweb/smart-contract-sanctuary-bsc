...
contract USDyVault is Initializable, ReentrancyGuard, Pausable, IEvents {
    ...
    */
        ...
        initializer
    {
        ...
        rebase_Reward = [20, 10, 10, 10, 20, 10,10,10,10];
    }
    ...
    */
    ...
    function rewards() external nonReentrant _isAdminOrOperator {
        ...
        getAPR[Admin.YSLBUSDVault()] = (mintReward * rebase_Reward[2]) / rebaseRate;
        IERC20(Admin.USDy()).safeTransfer(
            Admin.xYSLBUSDVault(),
            (mintReward * rebase_Reward[3]) / rebaseRate
        );
        getAPR[Admin.xYSLBUSDVault()] = (mintReward * rebase_Reward[3]) / rebaseRate;
        IERC20(Admin.USDy()).safeTransfer(
            Admin.USDyBUSDVault(),
            (mintReward * rebase_Reward[4]) / rebaseRate
        );
        getAPR[Admin.USDyBUSDVault()] = (mintReward * rebase_Reward[4]) / rebaseRate;
        IERC20(Admin.USDy()).safeTransfer(
            Admin.BShareBUSDVault(),
            (mintReward * rebase_Reward[5]) / rebaseRate
        );
        getAPR[Admin.BShareBUSDVault()] = (mintReward * rebase_Reward[5]) / rebaseRate;

        IERC20(Admin.USDy()).safeTransfer(
            Admin.BUSDVault(),
            (mintReward * rebase_Reward[1]) / rebaseRate
        );
        getAPR[Admin.BUSDVault()] = (mintReward * rebase_Reward[1]) / rebaseRate;
        ...
        getAPR[Admin.DAIVault()] = (mintReward * rebase_Reward[6]) / rebaseRate;
         IERC20(Admin.USDy()).safeTransfer(
            (Admin.USDCVault()),
            (mintReward * rebase_Reward[7]) / rebaseRate
        );

        getAPR[(Admin.USDCVault())] = (mintReward * rebase_Reward[7]) / rebaseRate;
         IERC20(Admin.USDy()).safeTransfer(
            Admin.USDTVault(),
            (mintReward * rebase_Reward[8]) / rebaseRate
        );

        getAPR[Admin.USDTVault()] = (mintReward * rebase_Reward[8]) / rebaseRate;
        // IERC20(Admin.USDy()).safeTransfer(
        //     Admin.YSLVault(),
        //     (mintReward * rebase_Reward[5]) / rebaseRate
        // );
        ...
        getAPR[address(this)] = (mintReward * rebase_Reward[0]) / rebaseRate;
