    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => bool) public executedAcceptBid;
    mapping(uint256 => bool) public executedOrder;

    mapping(uint256 => LibStruct.Order) public orders;
    mapping(bytes32 => uint256) private orderID;
    mapping(uint256 => LibStruct.Bid) public bids;
    ...
    function cancelOrder(uint256 _orderId) external whenNotPaused {
        LibStruct.Order storage order = orders[_orderId];
        require(
    ...
    function lazyBuy(LibStruct.LazyOrder calldata _order, uint256 _amount)
        external
        payable
        whenNotPaused
        returns (bool)
        ...
        returns (uint256)
    {
        ...
        require(!executedOrder[_order.nonce], "nonce-order");
        executedOrder[_order.nonce] = true;

        bytes32 ethTypedDataHash = _hashTypedDataV4(
            LibSign.getOrderHash(_order)
        );
        address owner = ECDSAUpgradeable.recover(
            ethTypedDataHash,
            _order.signature
        );
    ...
    {
        require(paymentMethod[_input.paymentToken], "payment");
        ...
        if (
            isHuukNFTs[_input.token.token] &&
            IHuukNFT(_input.token.token).getCreator(_input.token.id) ==
            _input.token.owner &&
            amountFirstSale[_input.token.token][_input.token.id] == 0 &&
            lastBuyPriceInUSDT[_input.token.owner][
                LibFunc.getTokenUnique(_input.token)
            ] ==
            0
        ) {
            amountFirstSale[_input.token.token][_input.token.id] = balance;
        }
    ...
    function _buy(
        ...
        require(paymentMethod[_paymentToken], "payment");
        ...
        returns (bool)
    {
        ...
        require(!executedAcceptBid[_lazyAcceptBid.nonce], "nonce-a");
        executedAcceptBid[_lazyAcceptBid.nonce] = true;
        bytes32 ethTypedDataHash = _hashTypedDataV4(
        ...
        returns (uint256)
    {
        ...
        require(paymentMethod[_lazyBid.paymentToken], "payment");
    ...
    function _acceptBid(LibStruct.LazyAcceptBid memory _lazyAcceptBid, address _owner)
        internal
        returns (bool)
    {
        LibStruct.Bid memory bid = bids[_lazyAcceptBid.lazyBid.id];
        LibStruct.Order memory order = orders[bid.orderId];
        ...
        return _updateBid(_lazyAcceptBid.lazyBid.id, _lazyAcceptBid.quantity);
    }

    function cancelBid(uint256 _bidId) external whenNotPaused nonReentrant {
        LibStruct.Bid storage bid = bids[_bidId];
            ...
            if (
                ...
                if (sellerRef != address(0)) {
                    ...
                    if (
                        ...
                        amountToSeller -= (orderAmount * xCreator) / ZOOM_FEE;
                        amountFirstSale[order.tokenAddress][
                            order.tokenId
                        ] -= _quantity;
                    } else {
                ...
                amountToSeller -= (orderAmount * xUser) / ZOOM_FEE;
                if (sellerRef != address(0)) {
                    _paid(
                        _paymentToken,
                        sellerRef,
...
library LibError {
    ...
    bytes4 public constant STANDARD_ERROR_SELECTOR =
        0x08c379a0;
    ...
    function revert(bytes memory errorData)
        public
        pure
    {
        assembly {
