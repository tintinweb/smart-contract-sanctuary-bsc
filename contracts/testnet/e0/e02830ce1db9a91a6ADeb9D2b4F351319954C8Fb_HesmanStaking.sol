...
contract HesmanStaking is Ownable, ReentrancyGuard, IERC721Receiver {
  ...
  function stakeNft(uint256[] calldata _idNftList) public nonReentrant isRun {
    ...
    benefit[msg.sender] += (block.number - stakeBlock[msg.sender]).mul(nftToHes).mul(nftIdList[msg.sender].length).mul(benefitPercentPerYear).div(100).div(10368000);
  ...
  function unstakeToken(uint256 _amount) public nonReentrant {
    ...
    amountClaim[msg.sender] += (unstakeBlock[msg.sender] - stakeBlock[msg.sender]).mul(nftToHes).mul(userNft).mul(benefitPercentPerYear).div(100).div(10368000)+benefit[msg.sender];
    stakeBlock[msg.sender] = block.number;
