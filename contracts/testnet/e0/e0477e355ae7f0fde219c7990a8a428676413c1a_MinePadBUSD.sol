...
contract MinePadBUSD is Context, Ownable , ReentrancyGuard, Events {
    ...
    function _updateAllocations(uint256 _amount) internal {        
        ...
        if (SafeMath.add(currentDistribution, yourAllocate) <= Max_reward_pool) {
            ...
            uint256 periodAlreadyClaimedPercent;
            uint256 nxtClaim;
            for (uint256 i; i < lent; ) {
                period memory _period = Period[i];
                if (i >= _cCounts) {
                    uint256 updateRewards = _calculate(yourAllocate, _period.p_period);
                    if (i == lent - 1) { 
                        vesting_percent[i] = SafeMath.add(
                            SafeMath.add(periodAlreadyClaimedPercent , updateRewards), _vesting[msg.sender].amountPerPeriod[i]
                        ); 
                    } else {
                        vesting_percent[i] = SafeMath.add(updateRewards, _vesting[msg.sender].amountPerPeriod[i]);
                    }
                    arg[i] = SafeMath.add(_period.t_period, block.timestamp);
                    nxtClaim = arg[lent - _cCounts];
                } else {
                    periodAlreadyClaimedPercent = _calculate(yourAllocate, _period.p_period);
                }
                unchecked {
                    i++;
                }
            ...
            // uint256[] amountPerPeriod;
            // uint256 nextClaimTime;
            _vesting[msg.sender] = vestingTime(msg.sender, 0, arg, vesting_percent, nxtClaim);
