...
// File: MutantDoodlez.sol

pragma solidity >=0.7.0 <0.9.0;

contract MutantDoodlez is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
...
//    function airdropMultiple(address[] calldata _list, uint256[] calldata _mintAmount)
//         external
//         onlyOwner
//     {
//         uint256 supply = totalSupply();
...
//             _safeMint(_list[i], supply + i + _mintAmount[i]);
//         }
//     }
  ...
  function updateMaxSupply(uint256 _newSupply) external onlyOwner {
    require(_newSupply < maxSupply, "decrease supply only");
  ...
  function withdrawMoney() external nonReentrant {
    ...
    payable(0xd124bA9AEC3a407A8f48001448563B94Ab0aBBD5).transfer(
        (balance * 50) / 100
    );
  }
}