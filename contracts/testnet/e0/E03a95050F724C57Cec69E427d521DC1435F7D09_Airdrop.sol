...
contract Airdrop is Ownable {
    ...
    event TrancheUnpause(uint256 indexed tranche);
     ...
     */
    constructor(IERC20 _token, address _rewardReserve) {
        token = _token;
    ...
    function newTranche(
        ...
        trancheReleases[tranche] = TrancheRelease({
    ...
    function unpauseTranche(uint256 _trancheId) external onlyOwner {
        ...
        emit TrancheUnpause(_trancheId);
    }
    ...
    function sweep(uint256 _trancheId) external onlyOwner {
        TrancheRelease memory _tranche = trancheReleases[_trancheId];
        require(_tranche.startTime + (86400 * 365) < block.timestamp, "TOO_EARLY");
        uint256 _amount = _tranche.totalAllocation - _tranche.claimed;
    ...
    function claim(
        ...
        require(!trancheReleases[_tranche].isPause, "PAUSED");
        _claim(msg.sender, _tranche, _amount, _merkleProof);
        _disburse(msg.sender, _tranche, _amount);
    }
    ...
    {
        if (claimed[_tranche][_walletAddress] == true) {
            return (0, 0);
        }
    ...
    function _claim(
        ...
        require(_tranche < tranche, "INVALID_TRANCHE");
        ...
        trancheReleases[_tranche].claimed = trancheReleases[_tranche].claimed + _amount;
    }
