...
contract FinalTest6 is Context, IERC20, Ownable {
    ...
    string private _name = 'FinalTest6';
    string private _symbol = 'FTEST6';
    ...
    uint256 public _HoldLimit = 5_000_000_000 * 10**18;
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _burnFee == 0 && _devFee == 0)
            return;
    ...
    function _tokenTransfer(
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(
                sender,
                recipient,
                (amount.sub(burnAmt).sub(marketingAmt).sub(devAmt))
            );
    ...
    function updateBuyFees(
        ...
        require(
            _taxFee
                ...
                .add(_liquidityFee)
                .add(_burnFee) <= 40,
            "Total fees can not grater than 40%"
        );
    }
    ...
    function setMaxHoldingPercents(uint256 maxHoldingPercents) external onlyOwner {
        ...
        require(maxHoldingPercents <= 300, 'Max holding percents cannot be more than 30%');
