...
contract MiningContract{
	...
	function withdrow(uint256 _value) external {
		...
		uint256 _withdrowTime = withdrowTime.add(_xzTime);
		require(_withdrowTime<=block.timestamp, "Err: error timestamp");
		withdrowTime = block.timestamp;
