...
contract LIFEGAMES is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) private _holderLastTransferTimestampAntiwhale; // to hold last Transfers temporarily during launch
    ...
    uint256 private _antiWhaleFeeAmount = 0;
    uint256 private _antiWhaleFeePercent = 10;
    ...
    function _transfer(
        ...
        if (limitsInEffect) {
            ...
            uint256 amountToSwapForBUSD = contractTokenBalance.sub(_liquidityFee);

            bool overMinTokenBalance = contractTokenBalance >= minimumTokensBeforeSwap;
      if (!inSwapAndLiquify && tradingActive && overMinTokenBalance && from != uniswapV2Pair) {
        ...
        //uint256 busdBalance = balanceOf(address(busdAddress)).sub(initialBUSDBalance);
         ...
         if (contractTokenBalance > 0) {
            swapAndSendToAddress(buybackAndBurnAddress, initialBUSDBalance.div(2));
            swapAndSendToAddress(liquidityAddress, initialBUSDBalance.div(2));
         }
      }
