...
contract Consumption is Ownable {
    ...
    event Purchase(address indexed user, uint indexed pay);
    ...
    function bindParent(address parent) external {
        ...
        require(inviter[parent] != address(0));
        inviter[msg.sender] = parent;
        inviterSuns[parent].push(msg.sender);
        emit Bind(msg.sender, parent);
    }

    function setParentByAdmin(address user, address parent) external onlyOwner {
    ...
    function burn(uint amount) external {
        require(userInfo[msg.sender].nftAmount >= amount, "Insufficient amount");

        nft.burn(msg.sender, uint32(amount));

        userInfo[msg.sender].nftAmount -= amount;
    ...
    function getInviterSuns(address user) public view returns (address[] memory) {
