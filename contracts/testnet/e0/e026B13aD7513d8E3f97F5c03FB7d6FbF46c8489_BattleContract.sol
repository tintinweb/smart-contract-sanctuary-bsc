    ...
    function BetBattle(
        ...
        for (i = 0; i < nftCount; i++) {
            ...
            if(billionsNFT.getOwner(_nftId) != _msgSender()) {
                require(billionsNFT.isRenter(_battleId, _nftId, _msgSender()));
            }
...
contract BillionsNFT is ERC721, BillionsBaseContract, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => mapping(address => bool))) public rentInfos;    // battle id => (nft id => (address => state))
    ...
    function setRentPrice(uint256 _nftId, uint256 _price) external {
    ...
    function isRenter(uint256 _bttleId, uint256 _nftId, address _addr) external view returns (bool) {
        return rentInfos[_bttleId][_nftId][_addr];
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        // require(tokenId, "not listed for rent")
    }
