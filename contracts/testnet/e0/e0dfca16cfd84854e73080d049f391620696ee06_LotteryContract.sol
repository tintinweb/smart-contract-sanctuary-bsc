...
contract LotteryContract is VRFConsumerBase, Ownable {
    ...
    function declareWinner(uint256 _lotteryId) public onlyOwner{
        ...
        for (uint256 k = 0; k < lottery.noOfWinner; k++) {
            uint256 winnerIndex = uint256(keccak256(abi.encode(52467517111356241020308667208771659213180219443876371025273020672164993104048, k))) % lottery.participants.length;
