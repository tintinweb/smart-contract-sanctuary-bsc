...
contract MetaCrowdV2 is AccessControlEnumerable {
    ...
    function setlLeverageFlag(bool flag) external {
        require(accounts[msg.sender].vips >= 4, "account level limited");
    ...
    function fund(uint256 regionIndex, uint256 amount) external {
        require(recordMetaPoint(msg.sender), "no meta account");
        require(amount > 0, "zero amount error");
        require(accounts[msg.sender].lastFundTime + fundInterval < block.timestamp || daoMap[msg.sender] == true, "funding time limited");
    ...
    function reserve(
        ...
        require(recordMetaPoint(msg.sender), "no meta account");
        require(amount > 0, "zero amount error");
        require(accounts[msg.sender].lastFundTime + fundInterval < block.timestamp || daoMap[msg.sender] == true, "reserve time limited");
        Project storage p = projects[regionIndex][round];
        ...
        for (uint256 i = 0; i < roundIds.length; i++) {
            if (harvestableIds[msg.sender][regionIndex].contains(roundIds[i])) {
                ...
                if (state > 3) {
                    ...
                    uint256 refund = amounts[1] - pf.reserveRefund * 300 / 1000;
                    pf.reserveRefund = 0;
                    pf.priorityRefund = 0;
                    if (restart == true) {
                        pf.version = version;
    ...
    // function setAddresses(address lp, address metaPoint, address stake) public {
    //     LP = lp;
    //     META_POINT = metaPoint;
    //     STAKE = stake;
    // }
