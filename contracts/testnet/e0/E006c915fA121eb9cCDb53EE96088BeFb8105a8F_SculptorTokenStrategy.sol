...
interface ISculptorChefIncentive {
...
interface ISculptorLendingPool {
...
import "../interfaces/ISculptorLendingPool.sol";
import "../interfaces/ISculptorChefIncentive.sol";
import "../interfaces/ISculptorMultifee.sol";

contract SculptorTokenStrategy is BaseStrategy {
    ...
    ISculptorLendingPool public immutable lendingPool;
    ISculptorMultifee public immutable multiFee;
    ISculptorChefIncentive public immutable chefIncentive;
    ...
    */
    constructor(
        ...
        address _factory,
        address _strategyExecutor,
        ISculptorLendingPool _lendingPool,
        ISculptorMultifee _multiFee,
        ISculptorChefIncentive _chefIncentive,
        address _sToken,
        ...
        multiFee = _multiFee;
        chefIncentive = _chefIncentive;
        sToken = _sToken;
        router = _router;
        feeCollector = _msgSender();

        ERC20(_rewardToken).safeApprove(address(_router), type(uint256).max);
        ERC20(_strategyToken).safeApprove(address(_lendingPool), type(uint256).max);
    }

    function _skim(uint256 amount) internal override {
        lendingPool.deposit(strategyToken, amount, address(this), 0);
    }

    function _harvest(uint256) internal override returns (int256) {
        address[] memory tokens = new address[](1);
        tokens[0] = sToken;
        chefIncentive.claim(address(this), tokens);
        multiFee.exit();
        return int256(0);
    }

    function _withdraw(uint256 amount) internal override {
        lendingPool.withdraw(strategyToken, amount, address(this));
    }

    function _exit() internal override {
        lendingPool.withdraw(strategyToken, type(uint256).max, address(this));
    }
