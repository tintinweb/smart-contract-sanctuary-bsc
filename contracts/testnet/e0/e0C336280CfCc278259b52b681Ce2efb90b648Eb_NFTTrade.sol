...
contract NFTTrade is Ownable, ReentrancyGuard {
    ...
    function buyToken(uint256 tokenId) payable public nonReentrant {
        require(msg.sender != address(0) && msg.sender != address(this), "NFTTrade: sender is the zero address");

        NftPrice memory nftPrice = nftSellPrices[tokenId];
        require(nftPrice.price > 0, "NFTTrade: NFT not for sell");

        address addressSeller = nftAddress.ownerOf(tokenId);

        if (nftPrice.token == CirculatingToken.BUSD) {
            require(busdAddress.balanceOf(msg.sender) >= nftPrice.price, "NFTTrade: BUSD amount is less than price");
            busdAddress.transferFrom(msg.sender, addressSeller, nftPrice.price);
        } else if (nftPrice.token == CirculatingToken.CONUT) {
            require(conutAddress.balanceOf(msg.sender) >= nftPrice.price, "NFTTrade: NFTC amount is less than price");
            conutAddress.transferFrom(msg.sender, addressSeller, nftPrice.price);

        } else if (nftPrice.token == CirculatingToken.BNB) {
            require(msg.value >= nftPrice.price, "NFTTrade: BNB amount is less than price");
