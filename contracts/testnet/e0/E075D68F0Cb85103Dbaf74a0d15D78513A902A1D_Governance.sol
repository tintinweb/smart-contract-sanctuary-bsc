...
contract Governance is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function createLoan(
        ...
        bytes32 salt = keccak256(abi.encodePacked(msg.sender, _tokenAddress, _tokenAmount, _duration, _paymentPeriod, _interestRate, teamWallet));
        address loan;
        assembly {
            loan := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        ILoan(loan).initialize(msg.sender, _tokenAddress, _tokenAmount, _duration, _paymentPeriod, _interestRate, teamWallet);
...
contract Loan {
    ...
    function initialize(
        ...
        require(msg.sender == governvor, 'Loan: FORBIDDEN'); // sufficient check
        owner = _owner;
        token = _token;
