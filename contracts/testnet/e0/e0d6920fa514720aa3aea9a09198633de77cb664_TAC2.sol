...
contract TAC2 is Context, IERC20, Ownable {
    ...
    string private constant _name = "TAC2.0";
    string private constant _symbol = "TAC2";
    uint8 private constant _decimals = 9;
    
    address payable private _developmentAddress = payable(0xDbCd0204a94B1319F6d04CC5be7A8E659970B914);
    address payable private _marketingAddress = payable(0x7d7B1f3C99114b269FaF196d8FE3Cf9A82D9D269);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rTax, uint256 tTransferAmount) = _getValues(tAmount);
    ...
    function _takeFee(uint256 rTax) private {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tTax) = _getTValues(tAmount, _taxFee);
        uint256 currentRate =  _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rTax) = _getRValues(tAmount, tTax, currentRate);
        return (rAmount, rTransferAmount, rTax, tTransferAmount);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tTax, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
