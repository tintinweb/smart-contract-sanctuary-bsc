...
contract Vault is BaseContract
{
   ...
   */
  struct Stats {
    uint256 totalParticipants; // 12909,
    uint256 totalDeposits; // 32919,
    uint256 totalDeposited; // 2040865956796064176142072,
    uint256 totalCompounds; // 1123593,
    uint256 totalCompounded; // 19097536095486065565834873,
    uint256 totalClaims; // 49225,
    uint256 totalClaimed; // 2561355694814802489986617,
    uint256 totalTaxed; // 1243219,
    uint256 totalTaxes; // 1661311510296616801513478,
    uint256 totalAirdropped; // 217391342508405934512745,
    uint256 totalAirdrops; // 29679,
    uint256 totalBonused; // 805157283889824458374846,
    uint256 totalBonuses; // 1081474
  }
...
//    _sendTokens(participant_, _refundAmount_);
  ...
  function _claim(address participant_, uint256 taxRate_) internal returns (uint256)
  {
    ...
    if(_penaltyClaims(participant_) + 1 >= _properties.penaltyClaims) { // _properties.penaltyClaims = 7
      // User is penalized
      _participants[participant_].penalized = true;
    }
    // Check effective claims
    if(_effectiveClaims(participant_, 1) >= _properties.negativeClaims) { // _properties.negativeClaims = 15
      // User is negative
  ...
  {
    uint256 _claims_ = 0;
    for(uint i = 0; i < _claims[participant_].length; i++) { // TODO do reverse lookup for gas saving
      if(_claims[participant_][i] >= timestamp_) {
        _claims_ ++;
      }
      // TODO break the loop for gas saving
    }
    return _claims_;
  }
  ...
  {
    if(_participants[participant_].startTime == 0) {
      return _rates[_properties.neutralClaims]; // _properties.neutralClaims = 13
    }
    //if(_participants[participant_].maxed) {
    //return _participants[participant_].maxedRate;
    //}
    if(_participants[participant_].penalized) {
      return _rates[_properties.lookbackPeriods]; // _properties.lookbackPeriods = 28
    }
