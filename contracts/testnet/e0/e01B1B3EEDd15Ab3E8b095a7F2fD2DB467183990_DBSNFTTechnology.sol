...
contract DBSNFTTechnology  is Ownable{
    ...
    constructor(address _dbsnft, IERC20 _erc20){
    ...
    function Cast() public  returns (uint256){
        // Transfer the token
        token.safeTransferFrom(msg.sender, pveAddress, 110);
    ...
    function Recast(uint256[] memory _ids) public  {
        ...
        require(quality > uint256(NFTQuality.NFTInvalid) && quality < uint256(NFTQuality.NFTGold));
        ...
        if(quality == uint256(NFTQuality.NFTCopper)){
            ...
            for (uint256 i = 1; i < _ids.length; i++) {
                ...
                require(camp == copperShip[IDToIndex[_ids[i]]].camp);
                values[i] = 1;
            }
            ...
            for (uint256 i = 1; i < _ids.length; i++) {
                require(quality == NftToQuality[_ids[i]]);
                require(camp == silverShip[IDToIndex[_ids[i]]].camp);
                values[i] = 1;
            }
            require(IDToIndex[_ids[0]] < silverShip.length);
            nftAddress.burnBatch(msg.sender, _ids, values);
            uint256[] memory sids;
            uint256 count;
    ...
    function GetSameShipCamp(uint256 _camp, uint256 _quality) public view returns (uint256[] memory ids, uint256 count){
        require(_quality > uint256(NFTQuality.NFTInvalid) && _quality <= uint256(NFTQuality.NFTGold), "DBSNFTTechnology: _quality error");
    ...
    function RecruitCopper(uint256 _id) public  {
        ...
        require(quality == uint256(NFTQuality.NFTCopper));
    ...
    function Decompose(uint256 _id) public  {
        uint256 quality = NftToQuality[_id];
        require(quality > uint256(NFTQuality.NFTCopper) && quality <= uint256(NFTQuality.NFTGold));
        if(quality == uint256(NFTQuality.NFTSilver)){
            ...
            for(uint256 i=0; i<3; i++){
                uint256 index = uint256(_random()).mod(cids.length - 1);
    ...
    function _pickShipRandomClass(uint256 _quality) internal returns (uint256) {
        require(_quality > uint256(NFTQuality.NFTInvalid) && _quality <= uint256(NFTQuality.NFTGold));
