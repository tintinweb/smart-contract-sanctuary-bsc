    ...
    ReentrancyGuardUpgradeable
{
    ...
    event InsuranceFundAdded(address newInsuranceFund, address oldInsuranceFund);
    event RewardPerTokenUpdated(int256 newRewardPerToken, int256 oldRewardPerTokenStored);
    ...
    function stake(
        ...
        rewardToken.transferFrom(msg.sender, address(insuranceFund), amount);
        user.amount += amount;
        totalStaked += amount;
    ...
    function unStake(
        ...
        UserInfo storage user = userInfo[msg.sender];
        uint256 withdrawAmount = Math.min(amount, user.amount);
        // decrease the amount then transfer amount
        user.amount -= withdrawAmount;
        totalStaked -= withdrawAmount;
        int256 actualAmount = earned(msg.sender).addUint(withdrawAmount);
        if (actualAmount > 0) {
            /// Withdraw from insuranceFund and send to user
            /// rewardToken.transfer(msg.sender, actualAmount.abs());
            insuranceFund.futuresStakingWithdraw(actualAmount.abs(), msg.sender, address(rewardToken));
            emit Unstaked(msg.sender, actualAmount.abs());
        }
        user.rewards = 0;
    }
    ...
    function harvest() public updateReward(msg.sender) nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        int256 reward = earned(msg.sender);
        if (reward > 0) {
            user.rewards = 0;
            /// Withdraw from insuranceFund and send to user
            /// rewardToken.transfer(msg.sender, reward.abs());
            insuranceFund.futuresStakingWithdraw(reward.abs(), msg.sender, address(rewardToken));

            emit RewardPaid(msg.sender, reward.abs());
        }
    }
    ...
    function setInsuranceFund(IInsuranceFund _insuranceFund) public onlyOwner {
        emit InsuranceFundAdded(address(_insuranceFund), address(insuranceFund));
        insuranceFund = _insuranceFund;
    }

    function setDefaultPairManager(address _defaultPairManager) public onlyOwner {
        defaultPairManager = _defaultPairManager;

    }

    function addCounterParty(address counterParty) public onlyOwner {
