...
//User parameters
  ...
  mapping (address => uint256) shield;
  ...
  mapping (address => bool) userHasOneTimeShield;
  mapping (address => uint256) userOneTimeShieldType;
...
//Backend User Parameters
  ...
  mapping (address => uint256) maxShield;
...
//Defence Systems
  ...
  uint256[] OneTimeShieldCost=[2500,4250,8100,16050,32000,63950];
  uint256[] ShieldAmount=[25,50,100,200,400,800,1600];  
...
//Static variables
  ...
  uint256 RechargeShieldPerUnitCost=10;
  uint256 UpgradeShieldPerUnitCost=10;
...
function getMaxShield (address user) public view returns(uint256){
  return maxShield[user];
}

function getShield (address user) public view returns(uint256){
  return shield[user];
}
...
function getOneTimeShield (address user) public view returns (bool){
  require (msg.sender==user,"Secret! :)");
  return userHasOneTimeShield[user];
}

function getOneTimeShieldType (address user) public view returns (uint256){
  require (msg.sender==user,"Secret! :)");
  return userOneTimeShieldType[user];
}
...
function rechargeShield() public {
  require(energy[msg.sender]>=(maxShield[msg.sender]*RechargeShieldPerUnitCost),"Not enough energy");
  energy[msg.sender]-=(maxShield[msg.sender]*RechargeShieldPerUnitCost);
  shield[msg.sender]=maxShield[msg.sender];
}

function upgradeShield(uint256 upgradeByUnits) public {
  require(energy[msg.sender]>=(upgradeByUnits*UpgradeShieldPerUnitCost),"Not enough energy");
  energy[msg.sender]-=(upgradeByUnits*UpgradeShieldPerUnitCost);
  maxShield[msg.sender]+=upgradeByUnits;
}
