...
interface iIncentive {
    function findWinner(uint256 _round) external;
...
contract Sale is ReentrancyGuard {
    ...
    function finalizeSale (uint8 _round) public nonReentrant IsSaleEnded(_round) {
        require(msg.sender == owner || msg.sender == adminAddress, "Not authorized: final");
        require(saleStatusCheck(_round) == 2, "Not succeeded"); // Success
        ...
        if(_incentive.getType(_round) == 1) {
            ...
            _incentive.findWinner(_round);
        } else if(_incentive.getType(_round) == 2) {
            _incentive.calcPrize(_round, sale_status[_round].sale_raised_amount);
            payable(address(incentiveAddress)).transfer(_incentive.getPrizeAmount(_round));
            _incentive.findWinner(_round);
        }
