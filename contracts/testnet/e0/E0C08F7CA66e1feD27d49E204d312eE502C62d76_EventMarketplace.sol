...
contract EventMarketplace is Ownable, Pausable, ReentrancyGuard {
    ...
    event AskCreated(address erc721, address erc20, address seller, uint256 price, uint256 tokenId, uint256 start, uint256 end);
    event AskCanceled(address erc721, address erc20, address seller, uint256 price, uint256 tokenId, uint256 start, uint256 end);
    ...
    /**
     * @dev Creates a event marketplace.
     * @param _start time to start an event
     * @param _end time to stop an event
     */
    constructor(uint256 _start, uint256 _end) {
        require(_start < _end, "Marketplace: cannot create contract because not sufficient time");
        start = _start;
        end = _end;
    }
    ...
    modifier inHappening(address erc721, uint256 tokenId) {
        require(start < block.timestamp, "Marketplace: Event not start yet");
        require(block.timestamp < end, "Marketplace: Event has ended");

        Ask memory info = asks[erc721][tokenId];
        require(info.start < block.timestamp, "Marketplace: Event not start yet");
        require(block.timestamp < info.end, "Marketplace: Event has ended");
    ...
    function setTaxAddress(address _taxAddress)
        ...
        onlyOwner
    {
        require(_taxAddress != address(0), "Marketplace: address is invalid");

        taxAddress = _taxAddress;

        emit TaxAddressUpdated(_taxAddress);
    }
    ...
    function setSalePrice(address erc721, address erc20, uint256 tokenId, uint256 price, uint256 _start, uint256 _end)
        public
        whenNotPaused
        nonReentrant
        inWhitelist(erc721, erc20)
    {
        ...
        require(_start < _end, "Marketplace: not sufficient time");
        ...
        asks[erc721][tokenId] = Ask(erc20, msgSender, price, _start, _end);

        emit AskCreated(erc721, erc20, msgSender, price, tokenId, _start, _end);
    }
        ...
        nonReentrant
    {
        ...
        emit AskCanceled(erc721, info.erc20, msgSender, info.price, tokenId, info.start, info.end);
        ...
        internal
    {
        ...
        if (erc20 == address(0)) {
            if (systemFeePayment > 0) {
                payable(taxAddress).transfer(systemFeePayment);
            }
            ...
            if (systemFeePayment > 0) {
                IERC20(erc20).safeTransfer(taxAddress, systemFeePayment);
            }
    ...
    function _computeSystemFee(uint256 price, uint256 feePercent)
        internal
        pure
        returns (uint256)
    {
