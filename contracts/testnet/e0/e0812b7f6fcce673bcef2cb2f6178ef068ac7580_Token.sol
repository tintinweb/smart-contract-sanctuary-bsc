...
contract Token is ERC20, Ownable {
  ...
  event changeTax(uint8 _sellTax, uint8 _buyTax, uint8 _burnTax, uint8 _transferTax);
  ...
  function setTaxes(uint8 _sellTax, uint8 _buyTax, uint8 _burnTax, uint8 _transferTax) external onlyOwner {
    sellTax = _sellTax;
    buyTax = _buyTax;
    burnTax = _burnTax;
    ...
    emit changeTax(_sellTax,_buyTax, _burnTax,_transferTax);
  }

  function getTaxes() external pure returns (uint8 _sellTax, uint8 _buyTax,uint8 _burnTax, uint8 _transferTax) {
    return (_sellTax, _buyTax, _burnTax, _transferTax);
  }
  ...
  function _transfer(address sender, address receiver, uint256 amount) internal virtual override {
    ...
    if(liquidityPool[sender] == true) {
      ...
      taxAmount = (amount * (buyTax + burnTax)) / 100;
    } else if(liquidityPool[receiver] == true) {      
      //It's an LP Pair and it's a sell
      taxAmount = (amount * (sellTax + burnTax)) / 100;
    ...
    if(taxAmount > 0) {
      super._transfer(sender, rewardsPool, amount * buyTax /100);
      super._transfer(sender, burnAddress, amount * burnTax /100);
    }    
