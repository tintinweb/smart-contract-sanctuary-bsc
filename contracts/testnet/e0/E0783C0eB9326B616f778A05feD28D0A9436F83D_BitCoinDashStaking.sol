    ...
    OwnableUpgradeable
{
        ...
        nonReentrant
    {
        ...
        BITD_TOKEN.transfer(AdminWallet, onePercentFee.mul(20));
        BITD_TOKEN.transfer(TreasuryWallet, onePercentFee.mul(20));

        BITD_TOKEN.transfer(MarketingWallet, onePercentFee.mul(5));
        BITD_TOKEN.transfer(CharityWallet, onePercentFee.mul(5));

        //leaders wallet length
        uint256 _leadersWalletLength = LeadersWallet.length;
        for (uint256 i = 0; i < _leadersWalletLength; i++) {
            BITD_TOKEN.transfer(LeadersWallet[i], onePercentFee.mul(2));
        }
    ...
    function claimReward(uint256 tokenAmountPerDollar) public nonReentrant {
        ...
        uint256 bonusToken = _bonus.mul(tokenAmountPerDollar);
        uint256 totalTokenByMining = totalMinerReward.mul(tokenAmountPerDollar);
        uint256 totalToken = bonusToken.add(totalTokenByMining);
        ...
        )
    {
        ...
        uint256 wbnbToToken =  ((wbnb / 10**18) / (token / 10**9)) * 10**10;

        int256 bnbPrice = _getLatestPriceBNBTOUSD();

        uint256 priceInDollars = wbnbToToken * uint256(bnbPrice);

        return (priceInDollars, wbnbToToken, uint256(bnbPrice)); //return decimal is in 10**18
    }

    function getTokenPrice2()
        public
        view
        returns (
            uint256
        ...
        )
    {
        ...
        return (priceInDollars); //return decimal is in 10**18
    }

    function calculateTokenToDollarsNeed(uint256 _dollarsAmount)
        public
        view
        returns (uint256 tokens)
    {
       tokens = DECIMAL * (_dollarsAmount * 10 ** 18) / getTokenPrice2();
...
//11,180.262490499 token
//0.559229606710777545 wbnb

//0.000011180262490499

//559,229,606.710777545
