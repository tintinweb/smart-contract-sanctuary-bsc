...
contract Trade is Ownable,IERC721Receiver {
    ...
    function _cancel(address from, uint256 fromIndex) private{
        ...
        orderSellMap[from][fromIndex] = orderSellMap[from][orderSellMap[from].length-1];
        orderSellMap[from][fromIndex].fromIndex = fromIndex;
        orderSellMap[from].pop();

        orderBuyMap[to][toIndex] = orderBuyMap[to][orderBuyMap[to].length-1];
    ...
    function sellNFT(address to, uint256 tokenIndex, uint256 tokenId, uint256 value) public{
        ...
        OrderNFT memory order = OrderNFT({
            ...
            value: value,
            tokenId: tokenId,
            typeNum: _nfts[tokenIndex].getTypeNum(tokenId)
        });
    ...
    function _cancelNFT(address from, uint256 fromIndex) private{
        ...
        orderNFTSellMap[from][fromIndex] = orderNFTSellMap[from][orderNFTSellMap[from].length-1];
        orderNFTSellMap[from][fromIndex].fromIndex = fromIndex;
        orderNFTSellMap[from].pop();

        orderNFTBuyMap[to][toIndex] = orderNFTBuyMap[to][orderNFTBuyMap[to].length-1];
    ...
    event Sell(address indexed user, uint256 tokenIndex, uint256 amount, uint256 value);
