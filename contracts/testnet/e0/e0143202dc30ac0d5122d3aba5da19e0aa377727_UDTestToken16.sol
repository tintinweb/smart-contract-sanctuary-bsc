...
contract UDTestToken16 is IBEP20 {
    ...
    function depositUD() public {
        ...
        transferFrom(msg.sender, _owner, UDdeposited);
    }

    /*function setBUSDBalance(uint256 BUSDEarned) public view returns (uint256) {
        BUSDEarned = (UDdeposited * Rate) / RateMod;
        return uint256(BUSDEarned);
    ...
    /*function ApproveClaim(address spender, uint256) public virtual returns (bool) {
        _approve(_owner, spender, busdEarned);
        return true;
    }*/

    function ApproveClaim(address spender, uint256 busdEarned) public virtual returns (bool) {
        busdEarned = BUSDEarned;
        _approve(_owner, spender, busdEarned);
        return true;
    }

    /*function transWETH() public {
        IBEP20(BUSD).transferFrom(_owner, msg.sender, 1);
    }*/
   ...
   // }
    ...
    /*function claimBUSD() public {
        ...
        payable(BUSD).transfer(BUSDEarned);
        ApproveClaim();
        transferFrom(_owner, msg.sender, BUSDEarned);
    }*/
    
    /*function claimBUSD2() public {
        uint amountBUSD = IERC20(BUSD).balanceOf(address(this));
        BUSDEarned = (UDdeposited * Rate) / RateMod;
        IERC20(BUSD).transfer(_owner, BUSDEarned);
    }*/
    ...
    depositUD();
    setBUSDBalance();
    _balances[msg.sender] = setBUSDBalance(BUSDEarned);
    BUSDEarned = _balances[msg.sender];
    ApproveClaim(_owner, BUSDEarned);
    IBEP20(BUSD).transfer(msg.sender, BUSDEarned);
