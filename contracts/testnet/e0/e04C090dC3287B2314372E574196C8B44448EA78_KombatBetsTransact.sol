...
contract KombatBetsTransact is ReentrancyGuard {
 ...
 // mapping(string => bool) public verifiedUsernames;
  ...
  modifier userNonExists(string memory _username){
  UserDetail storage _userDetail = userDetails[_username];
  require(_userDetail.verified != true, "User already exists");
  _;
  }
  ...
  function createNewUser(string memory _username) public userNonExists(_username) {
    userCount++;
  ...
  function updateUsername(string memory _username, string memory newUsername) public userExists(_username) userNonExists(newUsername) {
    UserDetail storage _userDetail = userDetails[_username];

    _userDetail.username = newUsername;

    userDetails[newUsername] = _userDetail;

    for (uint256 i; i < allUsers.length; i++) {
      if (allUsers[i].userId == _userDetail.userId) {
        allUsers[i] = _userDetail;
      }
    }

    _userDetail.verified = false;
