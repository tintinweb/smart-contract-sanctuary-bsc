...
contract MMMPool is Context, Initializable {
    ...
    function initialize(string memory pname, string memory psymbol, uint8 pdecimals, uint256 poolType, uint256 ticketNumber, uint256 bonusRate) public initializer {
        _name = pname;
        _symbol = psymbol;
        _decimals = pdecimals;
        _owner = msg.sender;
        ...
        for(uint256 i = _startPos; i <= _endPos; i++) {
            ...
            if(_posActive[i] == false ) {
                ...
                for (uint256 k = i + 1; k <= _endPos; k++) {
                    if(_posActive[k] == true && _posAddress[k] != address(0)) {
            ...
            if(!resetFlg) {
                ...
                _posActive[i] = true;
               // _posActive[kIndex] = false;
            }
            
        }
        _fcfsTime = block.timestamp;
        _state = 2;
        emit StartFCFS(_activePosCount, roundActiveNum);
    }
    ...
    event JoinPool(address who);
    event JoinRound(address who);
    ...
    event StartFCFS(uint256 activePosCount, uint256 roundActiveNum);
}