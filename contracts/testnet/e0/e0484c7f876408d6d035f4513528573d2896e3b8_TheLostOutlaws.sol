...
contract TheLostOutlaws is ERC165, IERC721, IERC721Metadata ,IERC721Enumerable,Ownable {
    // Token name
    string private _name="THE LOST Outlaws";
    // Token symbol
    ...
    uint256 public currentValue=10**17/10; //0.01BNB per NFT
    uint[] public Minted;
    constructor() {
    ...
    event CrowdMint(uint Count, uint StartIndex, uint[] PrototypeIDs, address to);
    function GetLost() public payable isInFunction{
        require(msg.sender==tx.origin,"No Bots Allowed");
        require(LostSale,"Sales Ain't open");
        uint256 getLosts=msg.value/currentValue;
        require(getLosts>0,"Send More BNB");
        require(NFTHolders.length+getLosts<=MaxNFTCount);
        uint[] memory MintedIDs=new uint[](getLosts);
        uint seed= block.timestamp+block.difficulty+block.gaslimit; 
        for(uint i=0;i<getLosts;i++){
            MintedIDs[i]=_mint(msg.sender,seed+NFTHolders.length);
        }
        uint256 remainder=msg.value%getLosts;
        if(remainder>0){
            (bool sentBack,)=msg.sender.call{value:remainder}("");
            require(sentBack,"FAIL");
        }

        if(getLosts>1)
            emit CrowdMint(getLosts,NFTs.length-getLosts,MintedIDs,msg.sender);
    ...
    function transferFrom(address from, address to, uint256 tokenId) external override{
        require(_isApprovedOrOwner(msg.sender, tokenId), "You Are Not Owner Nor Approved");
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory _data) private {
        _transfer(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, _data), "ERC721: Transfer To A Non-ERC721Receiver Contract");
    ...
    function _mint(address to, uint seed) private returns (uint){
        uint ID;
        uint value=_prng(10000, seed);
        if(value<6000) ID=0;
        else if(value<8000) ID=1;
        else if(value<9200) ID=2;
        else if(value<9900) ID=3;
        else ID=4;
