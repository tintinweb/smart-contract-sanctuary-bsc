...
contract DreamHorseNFT is ERC721URIStorage, Ownable {
    ...
    function _rand() internal virtual returns(uint256) {
        ...
        uint256 number1 =  uint256(keccak256(abi.encodePacked(block.timestamp, randNum++, msg.sender))) % (4 * 10 ** 15) + 1968769868;

        uint256 number2 =  uint256(keccak256(abi.encodePacked(block.timestamp, randNum++, msg.sender))) % (2 * 10 ** 15) + 1936586796;
    ...
    function getUserIamgeId(address user) public view returns(uint256[] memory iamgeIds) {
