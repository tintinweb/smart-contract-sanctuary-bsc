...
contract PresalePublicTest is Context, ReentrancyGuard, VRFConsumerBase {
    ...
    constructor(
        ...
        require(
            _info.openTime >= block.timestamp + 14 minutes &&
                _info.openTime + 15 minutes <= _info.closeTime &&
    ...
    function initialize() external nonReentrant onlyPresaleCreator {
        ...
        generalInfo.closeTime = generalInfo.openTime + 25 minutes;
    ...
    function register() external initialized notCreator {
        ...
        require(
            generalInfo.openTime <= currentTime + 8 minutes &&
                generalInfo.openTime >= currentTime + 3 minutes,
            "TIME"
        );
        address sender = _msgSender();
        (
        ...
        levelsCounter[level]++;
        if ((level > 0 && level < 4) && (lock30 || lock90))
            levelsCounter[10 + level]++;
    ...
    function claimTokens() external nonReentrant liquidityAdded initialized notCreator {
        address sender = _msgSender();
    ...
    function claimLps()
        ...
        liquidityAdded
    {
        uint256 amount = intermediate.lpAmount;
        require(
    ...
    function getRound() external view returns (Rounds) {
        uint256 currentTime = block.timestamp;
        if (
            currentTime <= generalInfo.openTime + 3 minutes &&
    ...
    function getIntermediate() external view returns(
        Votes memory,
        bool,
        bool,
        uint256,
        ...
        if (_round == Rounds.MOON) {
            ...
            for (i = 1; i < 10; i++) {
                if (i > 0 && i < 4) {
                    roundTokenAllocation[Rounds.DIAMOND][i] =
                        ((eachPoolShare * POOL_WEIGHT[i - 1]) / DENOMINATOR) *
                        ((levelsCounter[i + 10] * WINNING_CHANCE[i - 1]) /
                            DENOMINATOR);
                } else
                    roundTokenAllocation[Rounds.DIAMOND][i] =
                        ((eachPoolShare * POOL_WEIGHT[i - 1]) / DENOMINATOR) *
                ...
                //console.log("LEVEL ", i);
                //console.log("ALLOC ", roundTokenAllocation[Rounds.DIAMOND][i]);
            }
            ...
            for (i = 1; i < 10; i++) {
                roundTokenAllocation[Rounds.PAPER][i] =
                    ((eachPoolShare * POOL_WEIGHT[i - 1]) / DENOMINATOR) *
                    ((levelsCounter[i] * WINNING_CHANCE[i - 1]) / DENOMINATOR);
            }
        }
    }
