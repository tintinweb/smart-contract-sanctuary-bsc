...
contract Farm {
     ...
     */
    function init(FarmInfo memory _farmInfo) public {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 reward = user.amount.mul(accRewardPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function withdraw(uint256 _amount) public {
        ...
        require(
            user.unlockPeriod + user.depositTime <= block.timestamp,
...
contract FarmGenerator is Ownable {
    ...
    event Test(address token0, address token1);
    ...
    function createFarmV2(
        ...
        emit Test(lpair.token0(), lpair.token1());
    }
