...
contract  BSG50_50 is Ownable, SignVerify, Pausable, ReentrancyGuard{
    ...
    // uint256 private constant dayRewardPercents = 1500000000000000000000;
    // uint256 private constant dayReward2Percents = 1800000000000000000000;
    ...
    uint256[4] private level2to5Percents = [100, 200, 200, 200];
    uint256[5] private level6to10Percents = [100, 100, 100, 100, 100];
    uint256[10] private level11to20Percents = [50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
    ...
    uint256 private constant CTOPoolPercents = 200;
    ...
    uint256 public BUSDperULE;
    uint256 public BUSDperFTM;
    ...
    // (address _BUSDAddr, address _defaultRefer, address[2] memory _feeReceivers, 
    {
        ...
        BUSDperULE = 70449600000000000000;
        BUSDperFTM =  4306920000000000000;
    }
    ...
    function deposit(uint256 _tokenAmount,uint256 _packageAmount
    , uint256 _nonce, bytes memory _signature
    )  
    external
    payable
    nonReentrant
    whenNotPaused
    {
        ...
        _deposit(msg.sender,_packageAmount,_tokenAmount,_FTMamount);
    }
    ...
    function _deposit(address _user, uint256 packageAmount, uint256 _tokenAmount,uint256 _FTMamount) public {
        ...
        if(userInfo[user.referrer].directsNum >= 3)
        {
    ...
    function _distributeCTOPOol() public {
    ...
    function getRewards(address _user) public view returns(uint256 splitAmt,uint256 uleAmt,uint256 FTMamt){
        ...
        uleAmt = (withdrawAmt.mul(BUSDperULE)).div(1e18);
        FTMamt = (withdrawAmt.mul(BUSDperFTM)).div(1e18);
    ...
    function withdraw() external {
        ...
        require(
        BUSD.balanceOf(address(this)) >= withdrawableULE
        &&
    ...
    function _calLevelNow(address _user) public view returns(uint256) {
        ...
        //     if(total >= 300e18 && user.teamNum >= 6 && maxTeam >= 250e18 && otherTeam >= 250e18)
        //     {   levelNow = 5;   } // PLATINUM

        //     else if(total >= 200e18 && user.teamNum >= 4 && maxTeam >= 200e18 && otherTeam >= 200e18)
        //     {   levelNow = 4;   } // GOLD

        //     else if(total >= 100e18 && user.teamNum >= 2 && maxTeam >= 100e18 && otherTeam >= 100e18)
        //     {   levelNow = 3;   } // SILVER
        ...
        if(total >= 500e18)
        {
            ...
            else if(total >= 1000e18 && user.teamNum >= 100 && maxTeam >= 16000e18 && otherTeam >= 16000e18)
            {   levelNow = 4;   } // GOLD

            else if(total >= 500e18 && user.teamNum >= 50 && maxTeam >= 10000e18 && otherTeam >= 10000e18)
            {   levelNow = 3;   } // SILVER
    ...
    function _distributeDeposit(uint256 _amount,uint256 _tokenAmount,uint256 _FTMamount) public {
    ...
    function setValue_BUSDperULE(uint256 _BUSDperULE) public onlyOwner
    {
            BUSDperULE = _BUSDperULE;
    }   

    function setValue_BUSDperFTM(uint256 _BUSDperFTM) public onlyOwner
    {
        BUSDperFTM = _BUSDperFTM;
    }

    
    function totalROI(address _user) public view returns(uint256){
