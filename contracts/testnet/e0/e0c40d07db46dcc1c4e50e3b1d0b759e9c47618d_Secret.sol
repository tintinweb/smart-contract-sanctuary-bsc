...
contract Secret is Context, IERC20, IERC20Metadata, Ownable {
    ...
    //address public pcsv2Factory = 0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73;
    ...
    address public teamWallet = payable(0x31db60F65fEf00C409d67557a2C2d540b96AC60a);
    address public marketingWallet = payable(0x0b44990a808E7874D257fFf4e4CF454f6cfb41e4);
    address public buyBackWallet = payable(0x4489b27fEDE9777091677a0349A9aad44660c7FB);
    ...
    bool public autoTaxesEnabled = false;
    bool private isSniper = false;
    ...
    event setTaxesUpdate(uint256 buyTax_, uint256 buyTaxDenominator_, uint256 sellTax_, uint256 sellTaxDenominator_);
    event setFeeShareUpdate(uint256 marketingFeeShare_, uint256 teamFeeShare_, uint256 buyBackFeeShare_);
    ...
    function setFeeShares(
        ...
        require(marketingFeeShare_.add(teamFeeShare_).add(buyBackFeeShare) == 100, "GanjaInu: Share1 + Share2 + Share3 != 100");
    ...
    function compare(string memory a, string memory b) internal view returns (bool) {
        return (keccak256(abi.encodePacked((a))) == keccak256(abi.encodePacked((b))));
    }

    function toggleModes(uint256 mode_) public onlyOwner {
        require(mode_ == 1 || mode_ == 2 || mode_ == 3, "GanjaInu: Invalid mode selection");

        if (mode_ == 1 && !compare(taxMode, "STONED MODE")) {
            uint256 tax = 42;
            setTaxes(tax.div(100), 1, tax.div(10), 1);
            setFeeShares(24, 24, 52);
            taxMode = "STONED MODE";
        } else if (mode_ == 2 && !compare(taxMode, "420 MODE")) {
            uint256 tax = 42;
            setTaxes(tax.div(10), 1, tax.div(10), 1);
            setFeeShares(24, 24, 52);
            taxMode = "420 MODE";
        } else if (mode_ == 3  && !compare(taxMode, "MUNCHIE MODE")) {
            setTaxes(4, 1, 20, 1);
            setFeeShares(10, 10, 80);
    ...
    function setFeeWallets(address marketingWallet_, address teamWallet_, address buyBackWallet_) public onlyOwner{
        teamWallet = teamWallet_;
        marketingWallet = marketingWallet_;
        buyBackWallet = buyBackWallet_;
    }

    function setTaxes(uint256 buyTax_, uint256 buyTaxDenominator_, uint256 sellTax_, uint256 sellTaxDenominator_) public onlyOwner {
        ...
        emit setTaxesUpdate(buyTax_, buyTaxDenominator_, sellTax_, sellTaxDenominator_);
    }
    ...
    function _transfer(
        ...
        //if not from or to the owner
        if (to != owner() && from != owner()) {
            require(tradingEnabled == true, "GanjaInu: Trading is not open yet");
        }

        // SwapAndLiquify is triggered after every X transactions - this number can be adjusted using swapTrigger
        if(txCount >= swapTrigger && !inSwapAndLiquify && from != pcsV2Pair && swapAndLiquifyEnabled){  
            txCount = 0;
        ...
        if(from != pcsV2Pair && to != pcsV2Pair){
            ...
            if(!noFeeToTransfer && (!isExcludedFromFees_[from] || !isExcludedFromFees_[to])){
                _takeFeeWithTransferTax(from, to, amount);
            }
            else if(noFeeToTransfer || (!noFeeToTransfer && (isExcludedFromFees_[from] || isExcludedFromFees_[to])) ){
        ...
        if (tradingEnabled) {
            ...
            if(autoTaxesEnabled){ 
                if(currentPrice <= ATH && currentPrice >= ATH.sub(ATH.mul(20).div(100)) ){
                    toggleModes(1); //stoned mode
                } else if(currentPrice <= ATH.sub(ATH.mul(21).div(100)) && currentPrice >= ATH.sub(ATH.mul(70).div(100))){
    ...
    function _transferTokens(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tMarketing, uint256 tTeam, uint256 tBuyBack) = _getValues(tAmount);
        ...
        _balances[address(this)] = _balances[address(this)].add(tBuyBack);   
        emit Transfer(sender, recipient, tTransferAmount);
    }

    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 totalFee = tAmount.mul(swapFee).div(100);
        uint256 tMarketing = totalFee.mul(marketingFeeShare).div(100);
        uint256 tTeam = totalFee.mul(teamFeeShare).div(100);
        uint256 tBuyBack = totalFee.mul(buyBackFeeShare).div(100);
        uint256 tTransferAmount = tAmount.sub(totalFee);
        return (tTransferAmount, tMarketing, tTeam, tBuyBack);
    }
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        IPancakeRouter02(pcsV2Router).swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
            address(this),
    ...
    function commitBurn(address account_, uint256 amount_) public onlyOwner {
        ...
        require(
            ...
            "GanjaInu: Insufficient token balance to burn"
        );
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        require(
            ...
            "GanjaInu: burn amount exceeds balance"
        );
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(
                ...
                "GanjaInu: insufficient allowance"
            );
            unchecked {
    ...
    function withdrawBEP20(address payout, address token) public onlyOwner {
        ...
        IERC20(token).transfer(payout, balance);
    }
}