...
contract Validator is Ownable {
    ...
    function rmDev(address oldDev) public onlyOwner {
        ...
        require(flag, "GValidator: is not dev");
        if (i < _devs.length - 1) {
...
contract Storage is Validator {
    ...
    function withdraw(
        ...
        uint256 tid,
        address to
    ) public onlyDevOrOwner {
        require(validTid(tid), "tid invalid");
        if (tid == 0) {
            payable(to).transfer(amount);
        } else {
            IERC20(_tokenList[tid]).safeTransfer(to, amount);
        }
        emit WithdrawEvent(to, tid, amount);
    }
    ...
    function setColdWallet(address coldWallet_) public onlyOwner {
        coldWallet = coldWallet_;
    }
}