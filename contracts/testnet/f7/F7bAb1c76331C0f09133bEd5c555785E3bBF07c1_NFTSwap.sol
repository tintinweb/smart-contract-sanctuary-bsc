...
contract NFTSwap is Ownable, Pausable, ERC721Holder, ERC1155Holder {
    ...
    function getWithdrawFeeBalance(address payable recipient, uint256 amounts) public onlyOwner {
        uint256 am = getFeeBalance();
        if (am >= amounts) {
            recipient.transfer(amounts);
            _feeBalance -= amounts;
        } else {
            recipient.transfer(am);
            _feeBalance -= am;
        }
    }
