...
import "./QmallTransferManager.sol";
import "./IRoyaltiesProvider.sol";

contract ExchangeV2 is ExchangeV2Core, QmallTransferManager {    
    function __ExchangeV2_init(
        ...
        __QmallTransferManager_init_unchained(newProtocolFee, newCommunityWallet, newRoyaltiesProvider);
...
abstract contract TransferExecutor is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ITransferExecutor {
    ...
    function setAllowedTokens(address token, bool isAllowed) onlyOwner external nonReentrant {
    ...
    function transfer(
        ...
        if (asset.assetType.assetClass == LibAsset.ETH_ASSET_CLASS) {
            ...
            require(allowedTokens[token], "erc20 is not allowed");
...
abstract contract QmallTransferManager is OwnableUpgradeable, ReentrancyGuardUpgradeable, ITransferManager {
    ...
    function __QmallTransferManager_init_unchained(
        uint newProtocolFee,
        address newCommunityWallet,
        IRoyaltiesProvider newRoyaltiesProvider
    ...
    function setProtocolFee(uint newProtocolFee) external nonReentrant onlyOwner {
        protocolFee = newProtocolFee;
    }
