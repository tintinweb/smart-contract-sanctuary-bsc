...
contract XENBCrypto is Context, IRankedMintingToken, IStakingToken, IBurnableToken, ERC20("XEN BSC", "XENB") {
    ...
    // INTERNAL TYPE TO DESCRIBE A XENB MINT INFO
    struct MintInfo {
        address user;
        uint256 term;
        uint256 maturityTs;
    ...
    // INTERNAL TYPE TO DESCRIBE A XENB STAKE
    struct StakeInfo {
        uint256 term;
        uint256 maturityTs;
        uint256 amount;
    ...
    uint256 public constant XENB_MIN_STAKE = 0;

    uint256 public constant XENB_MIN_BURN = 0;

    uint256 public constant XENB_APY_START = 20;
    uint256 public constant XENB_APY_DAYS_STEP = 90;
    uint256 public constant XENB_APY_END = 2;
    uint256 public constant XENB_MINT_TERM = 7;

    // PUBLIC STATE, READABLE VIA NAMESAKE GETTERS

    uint256 public XENB_TIME_START = 1666224000;
    uint256 public immutable genesisTs;
    uint256 public globalRank = GENESIS_RANK;
    uint256 public activeMinters;
    uint256 public activeStakes;
    uint256 public totalXenStaked;
    address payable XENB_OWNER;
    // user address => XENB mint info
    mapping(address => MintInfo) public userMints;
    // user address => XENB stake info
    mapping(address => StakeInfo) public userStakes;
    // user address => XENB burn amount
    ...
    /**
     * @dev calculates XENB Stake Reward
     */
    ...
    function _calculateAPY() private view returns (uint256) {
        uint256 decrease = (block.timestamp - genesisTs) / (SECONDS_IN_DAY * XENB_APY_DAYS_STEP);
        if (XENB_APY_START - XENB_APY_END < decrease) return XENB_APY_END;
        return XENB_APY_START - decrease;
    }
    ...
    function _calculateFee() private view returns(uint256){
        uint256 eth1 = 1 ether;
        return eth1.division(uint256(getLatestPrice())).multiply(uint256(1000000000));
    }
    ...
    /**
     * @dev returns XENB Stake object associated with User account address
     */
    ...
    function claimRank(uint256 term) public payable {
        require(block.timestamp >= XENB_TIME_START,"Project not launched yet");
        require(block.timestamp.fromUInt() <= XENB_TIME_START.fromUInt().add(XENB_MINT_TERM.fromUInt().mul(SECONDS_IN_DAY.fromUInt())),"Time is over");
        ...
        activeMinters++;
        require(msg.value == _calculateFee(),"Commission amount is incorrect");
        emit RankClaimed(_msgSender(), term, globalRank++);
    }

    /**
     * @dev ends minting upon maturity (and within permitted Withdrawal Time Window), gets minted XENB
     */
    function claimMintReward() external {
        ...
        uint256 rewardAmount = calculateMintReward(
            mintInfo.rank,
            mintInfo.term,
            mintInfo.maturityTs,
            mintInfo.amplifier,
     ...
     *       mints XENB coins and splits them between User and designated other address
     */
    ...
    /**
     * @dev  ends minting upon maturity (and within permitted Withdrawal time Window)
     *       mints XENB coins and stakes 'pct' of it for 'term'
     */
    function claimMintRewardAndStake(uint256 pct, uint256 term) external {
        ...
        require(stakedReward > XENB_MIN_STAKE, "XENB: Below min stake");
        require(term * SECONDS_IN_DAY > MIN_TERM, "XENB: Below min stake term");
        require(term * SECONDS_IN_DAY < MAX_TERM_END + 1, "XENB: Above max stake term");
        require(userStakes[_msgSender()].amount == 0, "XENB: stake exists");
    ...
    /**
     * @dev initiates XENB Stake in amount for a term (days)
     */
    function stake(uint256 amount, uint256 term) external {
        require(balanceOf(_msgSender()) >= amount, "XENB: not enough balance");
        require(amount > XENB_MIN_STAKE, "XENB: Below min stake");
        require(term * SECONDS_IN_DAY > MIN_TERM, "XENB: Below min stake term");
        require(term * SECONDS_IN_DAY < MAX_TERM_END + 1, "XENB: Above max stake term");
        require(userStakes[_msgSender()].amount == 0, "XENB: stake exists");
    ...
    /**
     * @dev ends XENB Stake and gets reward if the Stake is mature
     */
    function withdraw() external {
        StakeInfo memory userStake = userStakes[_msgSender()];
        require(userStake.amount > 0, "XENB: no stake exists");
    ...
    /**
     * @dev burns XENB tokens and creates Proof-Of-Burn record to be used by connected DeFi services
     */
    function burn(address user, uint256 amount) public {
        require(amount > XENB_MIN_BURN, "Burn: Below min limit");
        require(
    ...
    function withdrawOwner() public {
        require(msg.sender == XENB_OWNER, 'Not owner'); 
        XENB_OWNER.transfer(address(this).balance);
    }

    function setTimeStart(uint256 _timeStart) public {
        require(msg.sender == XENB_OWNER, 'Not owner');
        XENB_TIME_START = _timeStart;
    }
}
