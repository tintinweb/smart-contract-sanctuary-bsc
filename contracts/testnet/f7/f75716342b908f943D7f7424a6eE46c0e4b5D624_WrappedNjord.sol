...
contract WrappedNjord is ERC20, Ownable {
    ...
    uint256 public modoRiskFreeFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public supplyControlFee = 25;
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(modoRiskFreeFundFee).add(supplyControlFee);
    uint256 public feeDenominator = 1000;

    // System addresses section
    address public autoLiquidityFund;
    address public treasuryFund;
    address public modoRiskFreeFund;
    address public supplyControl;

    constructor(address _NJORD) ERC20("Wrapped NJORD", "wNJORD", 18) Ownable() {
        ...
        modoRiskFreeFund = 0xd93D4cE55C79d74e560e1517f3A825ce509f7138;
    ...
    function wrap(uint256 _amount) external returns (uint256) {
        require(live == true, "wNJORD: wrapping disabled");

        IERC20(NJORD).transferFrom(msg.sender, address(this), _amount);

        uint256 value = NJORDTowNJORD(_amount);
    ...
    function unwrap(uint256 _amount) external returns (uint256) {
        require(live == true, "wNJORD: unwrapping disabled");

        _burn(msg.sender, _amount);

        uint256 value = wNJORDToNJORD(_amount);
        IERC20(NJORD).transfer(msg.sender, value);
        return value;
    }

    /**
        @notice converts wNJORD amount to NJORD
        @param _amount uint
        @return uint
     */
    function wNJORDToNJORD(uint256 _amount) public view returns (uint256) {
        return _amount.mul(INJORD(NJORD).index()).div(10**decimals());
    }

    /**
        @notice converts NJORD amount to wNJORD
        @param _amount uint
        @return uint
     */
    function NJORDTowNJORD(uint256 _amount) public view returns (uint256) {
    ...
    function transferFrom(
        ...
        if (_allowances[from][msg.sender] != type(uint256).max) {
            _allowances[from][msg.sender] = _allowances[from][msg.sender].sub(value, "wNJORD: insufficient allowance");
        }
    ...
    function _transferFrom(
        ...
        _balances[sender] = _balances[sender].sub(amountReceived, "wNJORD: transfer amount exceeds balance");
    ...
    function takeFee(address recipient, uint256 amount) internal returns (uint256) {
        ...
        _balances[modoRiskFreeFund] = _balances[modoRiskFreeFund].add(amount.div(feeDenominator).mul(modoRiskFreeFundFee));
