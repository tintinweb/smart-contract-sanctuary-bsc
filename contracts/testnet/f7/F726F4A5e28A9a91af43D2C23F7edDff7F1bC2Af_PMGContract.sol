...
interface IStrategy{
    function directDeposit(uint256 _wantAmt,address _user,uint256 action) external payable;
    function lockFund(uint256 _wantAmt,address _user) external ;
    function unlockFund(uint256 _wantAmt,address _user) external ;
    function deposit(uint256 _wantAmt,address seller,uint256 _tokenId,address buyer,uint256 action,uint256 isFreeze) external payable;
    // function tradeComplete(uint256 _wantAmt,address seller,uint256 _tokenId,address buyer) external ;
    function withdraw(uint256 _wantAmt,address _user) external payable returns(uint256);
    function cancelTrade(uint256 _wantAmt,address _user) external ;
    function userInfos(address _user) external returns(uint256,uint256,uint256,uint256);
...
contract PMGContract {
    ...
    mapping (uint256=>nftProfitSharing) private nftProfit;
    ...
    constructor(){
        ...
        isTokenSupport[0x734778123652973B37053c1b3CeD4256b2FEdbCb] = true; //usdc
        isTokenSupport[0x734778123652973B37053c1b3CeD4256b2FEdbCb] = true; //busd
        isTokenSupport[0xDfb1211E2694193df5765d54350e1145FD2404A1] = true; //wbnb
        isTokenSupport[0x734778123652973B37053c1b3CeD4256b2FEdbCb] = true; //weth
    ...
    function buyNFT(uint256 _index ) public payable {
        ...
        if(profit.pioneer == address(0)){
            ...
            nftProfit[item.tokenId].exOwner[profit.ownerCount] = (item.seller);
            nftProfit[item.tokenId].ownerCount+=1;
        }
        ...
        if(item.cryptoToken == 0xDfb1211E2694193df5765d54350e1145FD2404A1){
            if(balance - freezeBalance< item.price){
                require((msg.sender).balance > item.price,"insufficient fund");
                require(msg.value == item.price,"deposit bnb for bid");
                IStrategy(strategy[item.cryptoToken]).deposit{value:msg.value}(item.price  ,item.seller,item.tokenId,address(0),0,0);
            }else{
                IStrategy(strategy[item.cryptoToken]).deposit(item.price  ,item.seller,item.tokenId,msg.sender,1,0);
            }
        }else{
            if(balance - freezeBalance< item.price){
                ...
                IStrategy(strategy[item.cryptoToken]).deposit(item.price  ,item.seller,item.tokenId,address(0),0,0);
            }else{
                IStrategy(strategy[item.cryptoToken]).deposit(item.price  ,item.seller,item.tokenId,msg.sender,1,0);
            }
        }
    ...
    //     // require(0 < _day && _day < 7,"not support");
    //     address[14] memory array;
    //     uint256[14] memory percentage ;
    //     array[0] = (msg.sender);
    //     percentage[0] = (1);
    ...
    //             require(msg.value == remaining,"deposit bnb for bid");
    //             require(IERC20(auctionitem.cryptoToken).balanceOf(msg.sender)>remaining,"insufficient fund");
    //             // IStrategy(strategy[auctionitem.cryptoToken]).deposit(remaining ,array,percentage,1);
    //             IStrategy(strategy[auctionitem.cryptoToken]).deposit{value:remaining}(remaining ,array,percentage,1);
    //             // IStrategy(strategy[auctionitem.cryptoToken]).deposit(remaining);
    ...
    //             IERC20(auctionitem.cryptoToken).approve(strategy[auctionitem.cryptoToken],remaining);
    //             IStrategy(strategy[auctionitem.cryptoToken]).deposit(remaining ,array,percentage,1);
    //             // IStrategy(strategy[auctionitem.cryptoToken]).deposit(remaining);
    //             /* might have problem here if two transaction happen together */
    //             userBalance[msg.sender][auctionitem.cryptoToken].availableBalance = 0 ;
    ...
    //     IStrategy(strategy[auctionitem.cryptoToken]).withdraw(auctionitem.highestBid * 840 / 1000);
    //     IERC20(auctionitem.cryptoToken).transfer(auctionitem.seller,auctionitem.highestBid * 840 / 1000);
        ...
        */
        ...
        if(_tokenContract == 0xDfb1211E2694193df5765d54350e1145FD2404A1){
            if(balance - freezeBalance< _price){
                require((msg.sender).balance > _price,"insufficient fund");
        ...
        //         require((msg.sender).balance > remaining,"insufficient fund");
        //         // IStrategy(strategy[_tokenContract]).deposit(remaining);
        //         IStrategy(strategy[_tokenContract]).deposit{value:remaining}(remaining ,array,percentage,1);
        //         userBalance[msg.sender][_tokenContract].freezeBalance += remaining ;
        //     }else{
        ...
        //         IERC20(_tokenContract).approve(strategy[_tokenContract],remaining);
        //         // IStrategy(strategy[_tokenContract]).deposit(remaining);
        //         IStrategy(strategy[_tokenContract]).deposit(remaining ,array,percentage,1);
        //         userBalance[msg.sender][_tokenContract].freezeBalance += remaining ;
        //     }else{
        //         userBalance[msg.sender][_tokenContract].availableBalance -= _price ;
        //         userBalance[msg.sender][_tokenContract].freezeBalance += _price;
    ...
    function acceptOffer(uint256 _offerId) public  {
        ...
        if(profit.pioneer == address(0)){
            ...
            nftProfit[offerdetail.tokenId].exOwner[profit.ownerCount] = (msg.sender);
            nftProfit[offerdetail.tokenId].ownerCount+=1;
        }
        
        IERC721(offerdetail.nftContract).transferFrom(msg.sender,offerdetail.offerAddress,offerdetail.tokenId);
        IStrategy(strategy[offerdetail.cryptoToken]).deposit(offerdetail.price  ,msg.sender,offerdetail.tokenId,offerdetail.offerAddress,1,1);
        // IStrategy(strategy[offerdetail.cryptoToken]).withdraw(offerdetail.price * 840 / 1000 );
    ...
    function deposit(address _tokenContract,uint256 _amount) public payable {
        ...
        if(_tokenContract == 0xDfb1211E2694193df5765d54350e1145FD2404A1){
            ...
            // IStrategy(strategy[_tokenContract]).deposit(_amount);
            IStrategy(strategy[_tokenContract]).directDeposit(_amount ,msg.sender,0);
        }
        
        userBalance[msg.sender][_tokenContract].availableBalance += _amount;
    }
    ...
    function editStrategy(address _tokenContract,address _strategy) public onlyOwner  {
        require(strategy[_tokenContract] != _strategy,"same strategy found");
        strategy[_tokenContract] = _strategy;
    }

    function nftProfitDetail(uint256 _tokenId) public view returns (nftProfitSharing memory)  {
        return nftProfit[_tokenId];
    }
}
