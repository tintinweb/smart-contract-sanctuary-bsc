...
//stake
    ...
    function test(uint256 amount) public view returns(uint256) {
      if (_isExcludedFromFee[msg.sender]==true) amount=amount ;
        else amount=amount*99/100;
        return amount;
    }
...
//lottery
    ...
    function lotterybuy(uint256 amount) public   {
        ...
        require(amount >25000 ,"amount is too low");
        if (players.length>5) 
        {
...
//web3tokeninfo
    function TotalHolder() external view  returns (uint64){
        return holder;
    }
