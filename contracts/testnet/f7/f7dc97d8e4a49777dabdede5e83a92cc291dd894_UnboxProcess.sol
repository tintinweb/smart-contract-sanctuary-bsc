...
contract UnboxProcess is ReentrancyGuard, Runnable
{
    ...
    constructor() {
        ...
        starInfos[12] = StarInfo(1, 10);
        starInfos[11] = StarInfo(11, 20);
        starInfos[10] = StarInfo(21, 30);
        starInfos[9] = StarInfo(31, 40);

        starInfos[8] = StarInfo(41, 50);
        starInfos[7] = StarInfo(51, 60);
        starInfos[6] = StarInfo(61, 70);
        starInfos[5] = StarInfo(71, 80);

        starInfos[4] = StarInfo(81, 90);
        starInfos[3] = StarInfo(91, 100);
        starInfos[2] = StarInfo(101, 110);
        starInfos[1] = StarInfo(111, 120);
    }

    function openBox(uint256 boxTokenId) external nonReentrant whenRunning {
        require(boxTokenId > 0, "BOX TOKEN ID SHOULD BE GREATER THAN 0");
        ...
        for (uint256 i = 0; i < listLevelByBox.length; i++) {
            if (starInfos[listLevelByBox[i]].endId >= starInfos[listLevelByBox[i]].nextId) {
                count += 1;
            }
        }
        uint256[] memory listLevel = new uint256[](count);
        uint256 j = 0;
        for (uint256 i = 0; i < listLevelByBox.length; i++) {
            if (starInfos[listLevelByBox[i]].endId >= starInfos[listLevelByBox[i]].nextId) {
    ...
    function getBoxType(uint256 boxTokenId) public view returns (uint256) {
    ...
    function getListLevelByBoxType(uint256 boxType) public pure returns (uint256[] memory){
        uint256[] memory listLevel = new uint256[](4);
        if (boxType == 1) {
            listLevel[0] = 1;
            listLevel[1] = 2;
            listLevel[2] = 3;
    ...
    function getStarInfo(uint256 level) public view returns (uint256, uint256){
        return (starInfos[level].nextId, starInfos[level].endId);
    }

    function getBoxInfo(uint256 boxType) public view returns (uint256, uint256){
        return (boxInfos[boxType].startId, boxInfos[boxType].endId);
    }
