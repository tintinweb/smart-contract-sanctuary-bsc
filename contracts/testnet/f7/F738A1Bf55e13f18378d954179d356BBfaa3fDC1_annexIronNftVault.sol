...
contract annexIronNftVault is Owned {
    ...
    address private _tUSDToken = 0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47;
    address private _annToken = 0xB8d4DEBc77fE2D412f9bA5B22B33A8f6c4d9aE1e;
    IABep20Interface public comptroller;

    constructor(IABep20Interface _comptroller,IERC721 _nft, address _token) public {
        ...
        comptroller = IABep20Interface(_comptroller);
    }
    ...
    function swapTokensForExactTokens(uint256 tokenAmount) private returns(uint256[] memory) {
        address[] memory path = new address[](2);
        path[0] = _tUSDToken; // pleaase change address TUSD address
        path[1] = _annToken; //ann token

        BEP20Interface(_tUSDToken).approve(
    ...
    function transferTokens(address src, address dst, uint tokens) public returns (uint) {
         /* Fail if transfer not allowed */
         //_tUSDToken this is aToken
        // comptroller.transferAllowed(_tUSDToken, src, dst, tokens);
        // comptroller.transferVerify(_tUSDToken, src, dst, tokens);
    }
    modifier enableSwap() {
        Staker storage staker = stakerInfo[msg.sender];
