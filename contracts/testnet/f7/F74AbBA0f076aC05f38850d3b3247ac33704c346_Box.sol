...
contract Box is ERC721, ERC721Enumerable, ERC721Burnable {
    IGovernance public gov;
    INFT721 public nftContr;
    ...
    event BoxOpened(address indexed to, uint256 boxType, uint256[] tokenIds);
    ...
    constructor(address _gov, string memory _newbaseURI) ERC721("Box", "Box") {
    ...
    */
    function updateGov(address _newGov) external onlyManager {
    ...
    */
    ...
    /**
     * @dev add to locklistTokens.
     */
    function lockTokens(uint256[] memory tokenIds) public onlyManager {
        uint256 counter = tokenIds.length;
        require(counter > 0, "Box: number of tokens must be greater then 0");
    ...
    /**
     * @dev Remove from locklistTokens.
     */
    function unlockTokens(uint256[] memory tokenIds) public onlyManager {
        uint256 counter = tokenIds.length;
        require(counter > 0, "Box: number of tokens must be greater then 0");
        for (uint256 i = 0; i < counter; i++) {
    ...
    */
    ...
    function mint(address _to, uint256 _tokenId) external onlyMinter {
        require(!_exists(_tokenId), "Box: must have unique tokenId");
        _mint(_to, _tokenId);
    }
        ...
        onlyMinter
    {
        ...
        require(counter > 0, "Box: number of tokens must be greater then 0");
        for (uint256 i = 0; i < counter; i++) {
            require(!_exists(tokenIds[i]), "Box: must have unique tokenId");
    ...
    */
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        if (gov.hasRole(OPERATOR_ROLE, operator)) {
            return true;
        }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721, ERC721Enumerable) isMaintenance {
        require(
            !blacklist[from] && !blacklist[to],
            "Box: User in the blacklist"
        );

        require(lockedTokens[tokenId] == false, "Box: TokenId is locked");
    ...
    */
    function addBlacklist(address _account) external onlyManager {
    ...
    */
    function removeBlacklist(address _account) external onlyManager {
