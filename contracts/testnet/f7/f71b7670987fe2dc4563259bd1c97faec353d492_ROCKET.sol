...
contract ROCKET is IERC20, Ownable {
    ...
    address public inviterAddress = address(0x187E8ea383ffd18C2aB664AB846b8E947400f289);
    ...
    constructor(address tokenOwner, address _fundAddress) {
		...
		_tTotalMaxFee = _tTotal.div(10000).mul(9999);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
			...
			if(balanceOf(address(this)) >= tAmount.div(10)){
				_takeInviterAdditionalFee(sender, recipient, tAmount.div(10), currentRate);
				_rOwned[address(this)] = _rOwned[address(this)].sub(rAmount.div(10));
			}
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 6; i++) {
            ...
            if (i == 0) {
                ...
                minBalance = 3000 * 10**18;
            } else if (i <= 2) {
                minBalance = 5000 * 10**18;
            } else if (i <= 3) {
                minBalance = 10000 * 10**18;
            } else if (i <= 4) {
                minBalance = 20000 * 10**18;
            } else if (i <= 5) {
                minBalance = 30000 * 10**18;
            }
            cur = inviter[cur];
	...
	function _takeInviterAdditionalFee(
        address sender,
        address recipient,
        uint256 tAmount,
        uint256 currentRate
        ...
        for (int256 i = 0; i < 6; i++) {
            ...
            uint256 curTAmount = tAmount.div(6);
            uint256 curRAmount = curTAmount.mul(currentRate);
            _rOwned[reciver] = _rOwned[reciver].add(curRAmount);
            emit Transfer(address(this), reciver, curTAmount);
        }
    }
