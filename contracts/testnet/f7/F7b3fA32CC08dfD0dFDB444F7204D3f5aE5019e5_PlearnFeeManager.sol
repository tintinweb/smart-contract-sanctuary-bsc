...
contract PlearnFeeManager is Ownable {
    ...
    function processAllFee(bool ignoreError) external onlyOwner {
        ...
        for (uint256 i = 0; i < pairCount; ++i) {
            ...
            if (lpBalance > 0) {
                sendLP(pair);
                uint lpAmountOfFeeHandler = pair.balanceOf(address(plearnFeeHandler));
                (uint reserve0, uint reserve1, ) = pair.getReserves();
                (uint amountAMin, uint amountBMin) = _getLiquidityTokenMinAmount(reserve0, reserve1, lpAmountOfFeeHandler, totalSupply);
