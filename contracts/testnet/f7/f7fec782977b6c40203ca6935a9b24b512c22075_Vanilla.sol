...
/** 
 *  SourceUnit: c:\Users\anike\Work\vnla-contracts\src\test\Vanilla.sol
*/
...
contract Vanilla is Context, IERC20, Ownable {
    ...
    uint256 private _numTokensSellToAddToLiquidity = 10**uint256(_decimals - 5); //todo test only

    uint256[10] public lastPrices;
    ...
    constructor() {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0x14e9203E14EF89AB284b8e9EecC787B1743AD285
        ); // testnet PCS V2
        ...
        _isExcludedFromFee[0x14e9203E14EF89AB284b8e9EecC787B1743AD285] = true; //testnet
        ...
        returns (TValuesStruct memory)
    {
        ...
        uint256 tTransferAmount = tAmount -
            ...
            tLiquidity;
        return
            TValuesStruct(tTransferAmount, tFee, tBurn, tMarketing, tLiquidity);
    }
    ...
    {
        return
            (_amount * _taxFee * (isSell ? _sellFeeMultiplier : SELLFEEDENOM)) /
            DENOMINATOR /
            SELLFEEDENOM;
    }

    function calculateBurnFee(uint256 _amount, bool isSell)
        private
        view
        returns (uint256)
    {
        return
            (_amount *
                _burnFee *
                (isSell ? _sellFeeMultiplier : SELLFEEDENOM)) /
            DENOMINATOR /
            SELLFEEDENOM;
    }

    function calculateMarketingFee(uint256 _amount, bool isSell)
        private
        view
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        if (
            from == uniswapV2Pair ||
            (to == uniswapV2Pair && block.timestamp > nextUpdateTime)
        ) {
            updatePrice();
        }
    }
    ...
    function updatePrice() internal {
        ...
        try
            uniswapV2Router.getAmountsOut(
                _numTokensSellToAddToLiquidity / 2,
                path
            )
        returns (uint256[] memory price) {
            swapPrice = price[1];
        } catch {
            emit InsufficientLiquidity(lastPrices[0]);
        }
        if (swapPrice != 0) {
            lastPriceIndex = lastPriceIndex == 9 ? 0 : lastPriceIndex + 1;
            lastPrices[lastPriceIndex] = swapPrice;
            nextUpdateTime = block.timestamp + 1 minutes;
            emit PriceAdded(lastPriceIndex, swapPrice);
        }
    }

    function getSwapAveragePrice() public view returns (uint256) {
        uint256 sum;
        uint256 count;
        for (uint256 i = 0; i < 10; i++) {
            sum += lastPrices[i];
            if (lastPrices[i] != 0) count++;
        }
        return sum / count;
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uint256 minAmount = (getSwapAveragePrice() * 9) / 10;
    ...
    function disableAllFees() external onlyOwner {
        require(
            _taxFee != uint256(0) &&
                _liquidityFee != uint256(0) &&
                _marketFee != uint256(0) &&
                _burnFee != uint256(0),
    ...
    function setSellFeeMultiplier(uint256 newSellFeeMultiplier)
        external
        onlyOwner
    {
        require(
            newSellFeeMultiplier >= 100 && newSellFeeMultiplier <= 300,
            "Multiplier has to be from 100 to 300"
        );
    ...
    function clearStuckBNBBalance(address to, uint256 amount)
        external
        onlyOwner
    {
    ...
    event SetNumTokensSellToAddToLiquidity(
        uint256 oldAmount,
        uint256 newAmount
    );
    ...
    event PriceAdded(uint256 lastIndex, uint256 lastPrice);
    event InsufficientLiquidity(uint256 lastPrice);
}