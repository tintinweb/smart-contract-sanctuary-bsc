...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  ///     but keeping track of such data on-chain costs VLTh more than
...
contract VLT is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    uint256 public maxBuyTransactionAmount = 1000000000000000 * (10**9);
    ...
    address payable public projectDevWallet;
    ...
    function setFee(uint256 _ethRewardFee, uint256 _liquidityFee, uint256 _developmentFee, uint256 _burnFee) public onlyOwner {
        ETHRewardFee = _ethRewardFee;
        liquidityFee = _liquidityFee;
        developmentFee = _developmentFee;
        burnFee = _burnFee;

        totalFees = ETHRewardFee.add(liquidityFee).add(developmentFee).add(burnFee).add(burnFee); // total fee transfer and buy
    }
    ...
    function setprojectDevWallet(address payable _newprojectDevWallet) public onlyOwner {
        projectDevWallet = _newprojectDevWallet;
    }
    ...
    constructor() ERC20("Voltoken", "VLT") {
        ...
        developmentFee = 5;
        burnFee = 1;
        projectDevWallet = payable(0x7ec2DD2B374470b3CC26AE9841B455dc732C5b78);

        totalFees = ETHRewardFee.add(liquidityFee).add(developmentFee).add(burnFee); // total fee transfer and buy
        ...
        excludeFromFees(projectDevWallet, true);
        excludeFromFees(address(this), true);
        
        // exclude from max tx
        _isExcludedFromMaxTx[owner()] = true;
        _isExcludedFromMaxTx[address(this)] = true;
        _isExcludedFromMaxTx[projectDevWallet] = true;
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(developmentFee);
        projectDevWallet.transfer(marketingAmount);
    ...
    function swapAndSendBNBToMarketing(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, projectDevWallet);
    }
