...
contract Staking is TokenStakeOwnable, ReentrancyGuard {
        ...
        whenStakingNotPaused(stakeTokenAddr)
    {
        ...
        totalReservedAmount[stakeTokenAddr] -= s.amount;
        totalReservedAmount[s.rewardToken] -= s.rewards;
        totalStakedTokens -= s.amount;

        IERC20(stakeTokenAddr).safeTransfer(_msgSender(), s.amount);
        // since token is unlocked, return all claimable tokens
        uint256 leftRewards = s.rewards - s.claimedTokens;
        totalClaimedTokens += leftRewards;
        IERC20(s.rewardToken).safeTransfer(_msgSender(), leftRewards);
    ...
    function _claimRewards(address account, uint16 stakeIndex) internal {
        ...
        require(
            s.rewards >= claimableAmount + s.claimedTokens,
        ...
        totalReservedAmount[s.rewardToken] -= claimableAmount;
    ...
    ) external onlyOwner 
    {
        ...
        uint256 leftRewards = s.rewards - s.claimedTokens;
        totalClaimedTokens += leftRewards;
        IERC20(s.rewardToken).safeTransfer(account, leftRewards);
    ...
    function updateStakeOptions(
        ...
        require(token == stakeTokenAddr, "!Not DECODE");
        stakeOptions[token][index] = StakeOption(period, apy, rewardToken);
    }
