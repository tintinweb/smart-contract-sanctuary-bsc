...
contract AuctionDex is Initializable, AccessControlUpgradeable  {
	...
	function initialize(address _xanaliaAddress) public initializer {
		__AccessControl_init_unchained();
		_setupRole(DEFAULT_ADMIN_ROLE, _xanaliaAddress);
	}
	...
	function createAuction(
		...
		uint256 _startTime,
		uint256 _endTime
	) external onlyAdmin returns (uint256 _auctionId) {
		totalAuctions++;
	...
	function bidAuction(
		...
		uint256 _price,
		uint256 _expireTime
	) external onlyAdmin returns (uint256 _bidAuctionId) {
	...
	function cancelAuction(uint256 _auctionId, address _auctionOwner) external onlyAdmin returns (uint256) {
		require(auctions[_auctionId].owner == _auctionOwner, "Not-auction-owner");

		Auction storage currentAuction = auctions[_auctionId];
		require(tokenOnAuction[currentAuction.collectionAddress][currentAuction.tokenId] || currentAuction.endTime > block.timestamp, "Auction-cancelled-or-ended");
	...
	function cancelBidAuction(uint256 _bidAuctionId, address _auctionOwner) external onlyAdmin returns (uint256, uint256, address) {
	...
	function reclaimAuction(uint256 _auctionId, address _auctionOwner) external onlyAdmin returns (address, uint256) {
		Auction memory currentAuction = auctions[_auctionId];

		require(currentAuction.endTime < block.timestamp || !tokenOnAuction[currentAuction.collectionAddress][currentAuction.tokenId], "Auction-not-end-or-cancelled");
	...
	function acceptBidAuction(uint256 _bidAuctionId, address _auctionOwner) external onlyAdmin returns (uint256, address, address, uint256, address) {
		BidAuction storage currentBid = bidAuctions[_bidAuctionId];
		...
		return (currentBid.bidPrice, currentBid.collectionAddress, currentBid.paymentToken, currentBid.tokenId, currentAuction.owner);
	}
}
