...
contract AccidentHandler20220715V2 is Initializable, Adminable {
  ...
  function initialize(address admin_, uint endingBlockNumber_) public initializer {
    require(admin_ != address(0), "Can't set admin to zero address");
    _setAdmin(admin_);
    setEndingBlockNumber(endingBlockNumber_);
  }
  ...
  function setEndingBlockNumber(uint endingBlockNumber_) public onlyAdmin {
    require(endingBlockNumber_ != 0 && endingBlockNumber_ > block.number, "Can't set a zero block number");
    endingBlockNumber = endingBlockNumber_;
  }

  function retrieveTokens(address[] calldata tokens) external {
    require(endingBlockNumber > block.number, "Out of window");
    for (uint i; i < tokens.length; i++) {
