...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    */
    ...
    uint256 private bnbStackingLimit;
...
function getStackingCounter(uint256 cycle) private view returns (uint256) {
    ...
    function getStacked(address _address) public view returns (uint256) {
        ...
        if (rewardStacking[_address].enabled) {
            ...
            if (address(this).balance >= rewardStacking[_address].hardcap)
            {
                reward = rewardStacking[_address].hardcap.mul(rewardStacking[_address].amount).div(totalsupply).mul(stacked);
                if (address(this).balance.sub(reward) < rewardStacking[_address].hardcap)
                {
                    reward = address(this).balance - address(this).balance.mul(1-(rewardStacking[_address].amount.div(totalsupply))**stacked);
                }
            } else {
                reward = address(this).balance - address(this).balance.mul(1-(rewardStacking[_address].amount.div(totalsupply))**stacked);
            }
    ...
    function TestAddLiquidity(uint256 _token) public {
    ...
    function TestAddLiquidityWithoutTax(uint256 _token) public {
