...
contract OPT_IDO is OwnableUpgradeable {
    ...
    function bond(address invitor) public onlyEOA {
        require(userInfo[invitor].isBought || invitor == address(refer), 'wrong invitor');
    ...
    function buyIDO(bool withStm, address invitor) public onlyEOA {
        require(!userInfo[msg.sender].isBought, 'you have been IDO');

        if (refer.checkUserInvitor(msg.sender) != address(0)) {
            require(refer.checkUserInvitor(invitor) != address(0) || invitor == address(refer), 'wrong invitor');
            require(userInfo[invitor].isBought, 'wrong invitor');
            refer.bond(msg.sender, invitor, 0, 0);
        }
        ...
        address temp = refer.checkUserInvitor(invitor);
        userInfo[temp].referIDO++;
        if (!userInfo[invitor].isGetNode && userInfo[temp].referIDO >= 10) {
            node.minInitNode(invitor);
            userInfo[invitor].isGetNode = true;
        }
    }
