...
*/
    ...
    mapping (address => bool) public isLockedForCoolDown;
    mapping(address=>bool) public isPair;
    mapping(address => uint) public isUnlockTime;
    ...
    uint256 public DumpLimitAmount;
    uint public CoolDownTimeInSecs = 60;
    ...
    bool public enableDumpLimit= true;
...
//get return info about contract 
    ...
    //
    ///
    function getCoolDownTimeInSecs() public view returns (uint){return CoolDownTimeInSecs;}
    function getDumpLimitAmount() public view returns(uint256){return DumpLimitAmount;}
    function getDumpLimitStatus() public view returns(bool){return enableDumpLimit;}
//
///
//
//
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isTxLimitExempt[recipient] && restrictWhales)
        {
            require(_balances[recipient].add(amount) <= _walletMax, "wallet");
             ...
             if (enableDumpLimit)
             {
                _checkforCoolDown(sender, recipient, amount);

                 require(isLockedForCoolDown[sender] == false, "Seller Trying to Dump Dont LetHim");
             }
        }
...
//scattertransfer
    ...
    // 
    function _checkforCoolDown(address sender, address recipient, uint amount) private{
     //trade most be enabled
    //require(tradingStatus && !isTxLimitExempt[sender] && !isFeeExempt[sender], "check for expemted wallets");
       
      //checks if whale cool down time is spent..
      if(isLockedForCoolDown[sender] == true)
      if(block.timestamp > isUnlockTime[sender])
      {
          //Lock Time of trader is  Spent , will take cool down of trade neck
        isLockedForCoolDown[sender]= false;
      }
      //
       bool isSell = isPair[recipient];
       uint256 LimitAmount = DumpLimitAmount;
        if(isSell){ 
            //someone trying to sell tokens and amount too great...
            require(amount > LimitAmount, "revert transaction");
            //put a cool down if wallet balance too large.....
            if(balanceOf(sender) > LimitAmount)
            {
                isLockedForCoolDown[sender] = true;
                //sets Locktime for whatevr(secs) from whatever time person try swap limit value 
                isUnlockTime[sender] = CoolDownTimeInSecs + block.timestamp;
            }
        }
    }
//
...
//////codes.........:{}
//
     function ChangeCoolDownTime(uint newTime) external onlyOwner{
        CoolDownTimeInSecs = newTime;
    }
    //
     function ChangeDumpLimit(uint divisor) external onlyOwner{
           uint256 LimitAmount = _walletMax / divisor;
            DumpLimitAmount = LimitAmount;
     }

     function ToggleDumpLimit() external onlyOwner{
         enableDumpLimit = !enableDumpLimit;
     }
//
