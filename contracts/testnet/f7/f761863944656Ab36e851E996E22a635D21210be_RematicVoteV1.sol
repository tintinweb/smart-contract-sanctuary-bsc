...
contract RematicVoteV1 is ReentrancyGuard, Ownable {
    ...
    function registerManager() external payable nonReentrant {
        require(
    ...
    function signature(address _addr, string memory _privateKey) external pure returns (bytes32) {
        return keccak256(abi.encodePacked(_privateKey, _addr));
    }
}
