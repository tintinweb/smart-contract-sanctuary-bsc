...
contract StackingUSD is Ownable, ReentrancyGuard {
    ...
    uint refAmount = 5000 ether; // Reward Amount For Reffral
    uint regAmount = 100000 ether; // Reward Amount For Registration
    uint minAmountStake = 5 ether; // Minimum Amount For stacking
    uint256 RegPrice = 200000000; // USD Reqiure To Registration
    uint dailyReward = 100 ether; // Daily Reward
    uint rewardgap = 2 minutes; // Reward Gap
    uint maxClaimTime = 10 minutes; // MAx Time To Withdraw Claim Reward
    uint minClaimTime = 5 minutes; // Minimum Time To claim
        ...
        )
    {
        ...
        if (_lastclaimed == 0 && block.timestamp >= _startime + maxClaimTime) {
            ...
            _minmumtoken = pricepertoken.mul(_staking);
            _check = true;
        } else if (
