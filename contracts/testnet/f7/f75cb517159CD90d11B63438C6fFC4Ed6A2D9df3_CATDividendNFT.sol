...
contract CATDividendNFT is Ownable, ERC721Min {
    ...
    string private _tokenBaseURI = ""; // metadata URI
    address public treasuryAddress;
    bool public useBaseURIOnly = true;
    mapping(uint256 => string) public tokenURIs; // allows for assigning individual/unique metada per token
    mapping(uint256 => uint256) public claimTime; 
    mapping(uint256 => uint256) public rewardDebt;
    uint256 public accRewardPerShare;
    uint16 public maxSupply = 100;
    uint16 public maxPerAccount;
    uint16 public maxPerMint;    
    bool public saleActive;
    uint256 public price;
    uint256 public revenue;
    uint256 public constant unclaimedRewardRecoveryDelay = 183 days; // 6 months

    event WithdrawRevenue(address indexed sender, uint256 indexed amount);
    event WithdrawUnclaimedReward(address indexed sender, uint256 indexed amount);
    event ClaimReward(address indexed account, uint256 indexed tokenId, uint256 indexed amount);
    event BatchClaimReward(address indexed account, uint256 indexed amount);
    
    constructor(string memory name, string memory symbol, address treasury) ERC721Min(name, symbol) {
    ...
    function rewardBalance() public view returns (uint256) {
        return address(this).balance - revenue;
    }

    function pendingReward(uint256 tokenId) public view returns (uint256) {
        return accRewardPerShare - rewardDebt[tokenId];
    }
    ...
    function buy() external payable saleIsActive {
        ...
        require(maxPerAccount == 0 || maxPerAccount > balanceOf(_msgSender()), "Max mint per account reached");
        revenue += msg.value;
        _mintAndUpdateRewardDebt(_msgSender());
    }

    function batchBuy(uint8 amount) external payable saleIsActive {
        require(msg.value == price * amount, "Incorrect amount of ETH sent");
        require(maxSupply >= totalSupply() + amount, "Max supply reached");
        require(maxPerAccount == 0 || maxPerAccount >= balanceOf(_msgSender()) + amount, "Max mint per account reached");
        require(maxPerMint >= amount, "Amount exceeds max per mint");
        revenue += msg.value;
        _batchMintAndUpdateReward(_msgSender(), amount);
    }    

    function mint(address receiver) external onlyProxy {
        _mintAndUpdateRewardDebt(receiver);
    }

    function batchMint(address receiver, uint16 amount) external onlyProxy {
        _batchMintAndUpdateReward(receiver, amount);
    }

    function _mintAndUpdateRewardDebt(address receiver) private {
        _mint(receiver);
        uint256 tokenId = _owners.length - 1;
        rewardDebt[tokenId] = accRewardPerShare;
        claimTime[tokenId] = block.timestamp;        
    }

    function _batchMintAndUpdateReward(address receiver, uint16 amount) private {
        for (uint256 i; i < amount; i++) {
            _mintAndUpdateRewardDebt(receiver);
        }
    }  

    // deposit rewards fund
    receive() external payable onlyOwner {
        accRewardPerShare += msg.value / totalSupply();
    }    

    function claimReward(uint256 tokenId) external {
        require(_msgSender() == _owners[tokenId], "Only owner");
        uint256 pending = pendingReward(tokenId);
        require(pending > 0, "Nothing to claim");
        rewardDebt[tokenId] = accRewardPerShare;
        claimTime[tokenId] = block.timestamp;
        (bool success, ) = _msgSender().call{value: pending}("");
        require (success, "Transfer failed");
        emit ClaimReward(_msgSender(), tokenId, pending);        
    }
    ...
    function batchClaimReward(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            uint256 pending = pendingReward(tokenId);
            if (pending > 0 && _msgSender() == _owners[tokenId]) {
                total += pending;
                rewardDebt[tokenId] = accRewardPerShare;
                claimTime[tokenId] = block.timestamp;
            }
        ...
        require (success, "Transfer failed");
        emit BatchClaimReward(_msgSender(), total);        
    }

    function withdrawRevenue() external {
        require(_msgSender() == owner() || _msgSender() == treasuryAddress || proxyToApproved[_msgSender()], "Not allowed");
        require(revenue > 0, "Nothing to withdraw");
    ...
    function withdrawUnclaimedReward(uint256[] calldata tokenIds) external {
        require(_msgSender() == owner() || _msgSender() == treasuryAddress || proxyToApproved[_msgSender()], "Not allowed");
        uint256 total;
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            uint256 pending = pendingReward(tokenId);
            if (pending > 0 && block.timestamp >= claimTime[tokenId] + unclaimedRewardRecoveryDelay) {
                total += pending;
                rewardDebt[tokenId] = accRewardPerShare;
                claimTime[tokenId] = block.timestamp;
            }
        ...
        (bool success, ) = treasuryAddress.call{value: total}("");
        require (success, "Transfer failed");
        emit WithdrawUnclaimedReward(_msgSender(), total);
    }
   
    function airdrop(address[] calldata receivers, uint256[] calldata amounts) external onlyOwner {
        require(receivers.length == amounts.length, "Arrays aren't the same size");
    ...
    function setMaxPerAccount(uint16 _maxPerAccount) external onlyOwner {
    ...
    function isApprovedForAll(address _owner, address operator) public view override returns (bool) {
        ...
        return 
            address(proxyRegistry.proxies(_owner)) == operator || proxyToApproved[operator] 
                ? true 
