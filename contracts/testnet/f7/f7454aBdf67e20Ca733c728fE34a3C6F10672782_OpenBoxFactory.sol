...
contract OpenBoxFactory is Ownable, Pausable {
    ...
    function openBox(
        ...
        // Check type of box
        require(
            _getBoxType(_boxSmartContract, _boxId) == 1 ||
                _getBoxType(_boxSmartContract, _boxId) == 2 ||
                _getBoxType(_boxSmartContract, _boxId) == 3,
            "Revert: Box are invalid types"
        );

        // Burn box
        _boxSmartContract.burnBox(_sender, _boxId);
        if (_getBoxType(_boxSmartContract, _boxId) == 1) {
            try
                ISneaker(sneakerSmartContractAddress).generateSneaker(
                    _sender,
                    _metadataId
                )
            returns (uint256 sneakerId) {
                return sneakerId;
            } catch {
                revert("Revert: failed to open box");
            }
        } else if (_getBoxType(_boxSmartContract, _boxId) == 2) {
            try
                IBike(sneakerSmartContractAddress).generateBike(
                    _sender,
                    _metadataId
                )
            returns (uint256 bikeId) {
                return bikeId;
            } catch {
                revert("Revert: failed to open box");
            }
        } else if (_getBoxType(_boxSmartContract, _boxId) == 3) {
            try
                IPlaylist(sneakerSmartContractAddress).generatePlaylist(
                    _sender,
                    _metadataId
                )
            returns (uint256 playlistId) {
                return playlistId;
            } catch {
                revert("Revert: failed to open box");
            }
    ...
    function _getBoxType(IBox _boxSmartContract, uint256 _boxId)
        ...
        returns (uint8)
    {
        try _boxSmartContract.getTypeOfBox(_boxId) returns (uint8 boxType) {
            return boxType;
        } catch {
            revert("OpenBox: failed to get type from the box");
        }
    }
