...
contract MultiTransfer2 is Ownable {
  using SafeERC20 for IERC20;

  fallback() external payable {
    revert("Contract isn't received BNB!"); 
  }

  function distributeSingle(address[] memory recipients, uint256 value, address[] memory tokens, uint256 tokenValue) external {
    for (uint256 i = 0; i < recipients.length; i++) {
      (bool sent, ) = recipients[i].call{value: value}("");
      require(sent, "Failed to send Ether");

      for (uint256 j = 0; j < tokens.length; j++) {
        IERC20(tokens[j]).safeTransferFrom(msg.sender, recipients[i], tokenValue);
      }
    }
  }
  ...
  function distributeMultiple(address[] memory recipients, uint256[] memory values, address[] memory tokens, uint256[] memory tokenValues) external {
    for (uint256 i = 0; i < recipients.length; i++) {
      ...
      for (uint256 j = 0; j < tokens.length; j++) {
        IERC20(tokens[j]).safeTransferFrom(msg.sender, recipients[i], tokenValues[j]);
      }
    }
  }
  ...
  function distributeTokenSingleValue(address token, address[] memory recipients, uint256 value) external {
    for (uint256 i = 0; i < recipients.length; i++) {
      IERC20(token).safeTransferFrom(msg.sender, recipients[i], value);
    }
  }
