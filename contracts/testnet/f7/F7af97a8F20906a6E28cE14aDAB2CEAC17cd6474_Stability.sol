...
contract Stability{
    ...
    uint256 public BNBreceive;
    uint256 public USDSCout;
    uint256 public BUSDCout;


    constructor(address _usdsctoken,address _busdtoken){
    ...
    function getUSDSC() public payable {
        ...
        USDSCout = BUSDCout.add(value);// counted total usdsc out
    }

    function getBUDSC() public payable {

        require(msg.value > 0,"please transfer correct amount");
        USDSCToken.transfer(owner,msg.value);//transfered bnb to owner account
        uint256 ourPercent = msg.value.mul(1e18).div(1e20);// calculated 1%
        uint256 remainingUsdsc = msg.value.sub(ourPercent);//subtracted 1% from 100% to get 98%
        uint256 amount = remainingUsdsc.div(Bnbusdprice);//multiplied 99% bnb to bnb/usd price
        uint256 value = amount.div(10**18);//make wei to eth formant
        BUSDToken.transfer(msg.sender,value);//transfer usdsc to user account
        BNBreceive = BNBreceive.add(msg.value);//add bnb to total bnb receive
        BUSDCout = BUSDCout.add(value);// counted total usdsc out
    ...
    function getUSDSCout()public view returns(uint256){
        return USDSCout;
    }
    ...
    function getBNBreceive()public view returns(uint256){
        return BNBreceive;
    }

    //setting the price
    function setPrice() external payable{
        Bnbusdprice = getPrice();
    }
