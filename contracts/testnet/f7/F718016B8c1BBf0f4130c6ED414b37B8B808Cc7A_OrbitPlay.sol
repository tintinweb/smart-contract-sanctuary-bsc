...
contract OrbitPlay is Ownable, ReentrancyGuard {
    ...
    event DiceRollResult(address bidder, uint256 betAmount, uint256 betNumber, uint256 result, uint256 returned);
    event CoinFlipResult(address bidder, uint256 betAmount, string betSide, string result, uint256 returned);

    constructor(address _tokenAddress) {
        tokenAddress = _tokenAddress;        
    }

    function placeDiceRollBet(uint256 amount, uint256 betNumber) public returns (uint256, uint256){
		require(
            dicePlayable,
        ...
        emit DiceRollResult(msg.sender, amount, betNumber, destiny, userInfo.returningAmount);

        return (destiny, userInfo.returningAmount);
	}

    function placeCoinFlipBet(uint256 amount, uint256 betSide) public returns (uint256, uint256){ //betSide 1: heads 2: tails
		require(
            coinFlipPlayable,
		...
		if(destiny == betSide){ //win (1: heads 2: tails)
			...
			userInfo.returningAmount = 0;
            uint256 burnAmount = amount.mul(coinFlipBurnOnLossPercentage).div(10000);
            IERC20(tokenAddress).safeTransfer(DeadAddress, burnAmount);
            updateCoinFlipInfo(0, burnAmount);
		}        
        
        emit CoinFlipResult(msg.sender, amount, getSideFromNo(betSide), getSideFromNo(destiny), userInfo.returningAmount);
