...
struct DonationPool {
    ...
    mapping (address => mapping (address => uint)) donors;
}
...
contract Ethnos {
    ...
    event PoolCreated(string _charity, address _treasury, uint _id, uint _databaseId);
    ...
    function addPool(string calldata _charity, address _treasury, uint _databaseId) public {
        _requireAdmin();
        require(charities[_charity].enabled, 'Charity is not enabled');

        DonationPool storage pool = pools.push();
        pool.charity = _charity;
        pool.treasury = _treasury;
        pool.databaseId = _databaseId;

        uint id = pools.length - 1;
        charities[_charity].pools.push(id);
        emit PoolCreated(_charity, _treasury, id, _databaseId);
    }
