...
contract BuyTest is IERC1155ReceiverUpgradeable {

    IBellyBoxCore private bellybox;
    IERC20Upgradeable private csw;
    IRandoms private seedBuilder;
    IEquipmentERC1155 private equiment;

    uint[3] typeArray;

    constructor(address _csw) {
        bellybox = IBellyBoxCore(0xF542C10Cf2b7A209D05124faa241CEe2af2514bC);
        csw = IERC20Upgradeable(_csw);
        seedBuilder = IRandoms(0x88A7Ce9F7471BE796Fb8c0eE97D61fE1fcAD988B);
        equiment = IEquipmentERC1155(0xE32aab39A72cD10F0cD9A785589840c57f5617B2);
        unchecked{
            SafeERC20Upgradeable.safeApprove(csw, address(bellybox), reduce(0, 1));
        }
        typeArray = [1,2,3];
    }
    
    function setSeedBuilder(address _builder) external{
        seedBuilder = IRandoms(_builder);
    }

    function buy(uint8 boxType) external{
        (uint256 price, , , uint256 opened, ) = bellybox.getBellyBox(boxType);
        uint256 seed = seedBuilder.getRandomSeedUsingHash(address(this), keccak256(abi.encodePacked(block.timestamp, blockhash(block.number - 1), opened)));
        if(boxType == 3) {
            boxType = uint8(typeArray[Random.createRandom(1, 3, seed, block.timestamp) - 1]);
        }
        if(boxType != 3){
            revert("bad thing");
        }
        bellybox.createBellyBox(price, boxType);
    }

    function withraw20(address _token) external{
        IERC20Upgradeable token = IERC20Upgradeable(_token);
        SafeERC20Upgradeable.safeTransfer(token, msg.sender, token.balanceOf(address(this)));
    }
