...
interface IMining {
    function deposit(address _user, uint256 _hashRate) external;
...
interface IParcelInterface {
    ...
    function mintingParcel(address user) external returns (uint256);
...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => ParcelInf) public parcelInf;
    mapping(address => MinerInf) public minerInf;
    address public web2BackendAddress;
    address public netGymStreetAddress;

    // ------------------------------------ EVENTS ------------------------------------ //

    event ParcelsSlotsCountSet(uint8 indexed standardParcelSlotsCount);
    ...
    event NFTMinted(address indexed user, uint8 indexed operationType, uint256[] nftId);
    event SlotHash(address indexed user, uint256 parcelCount, uint256 freeSlots, uint256 totalHash, uint256 freeHash, bool isNegative);
    ...
    // @notice (Purchase) Generic minting functionality for parcels, regardless the currency
     function mintParcels(uint256 _count) external onlySaleActive notZeroAddress {   
        uint256 parcelId = _mintParcels(_count, msg.sender);
        uint256[] memory nftId = new uint256[](1);
        nftId[0] = parcelId;
        emit NFTMinted(msg.sender, OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, nftId);
    }

    function _mintParcels(uint _count, address _user) private returns(uint256) {
        require(_count > 0, "Municipality: Can not mint 0 parcels");
        require( usersMintableNFTAmounts[_user].parcels >= _count, "Municipality: User do not have sufficient balance");
        //INetGymStreet(netGymStreetAddress).addGymMlm(_user, _referrerId);
        usersMintableNFTAmounts[_user].parcels -= _count;
        emit BalanceUpdated(_user, _count, 0, true);
        uint256 parcelId;
        if (IERC721Base(standardParcelNFTAddress).balanceOf(_user)==0) {
            parcelId = IParcelInterface(standardParcelNFTAddress).mintingParcel(_user);
        }
        parcelInf[_user].parcelCount += _count;
        parcelInf[_user].freeSlots += _count * standardParcelSlotsCount;
        currentlySoldStandardParcelsCount += _count;
        //_attachMiners(_user, 500000 * 1000); // to attach free miners to newly purchased parcels if available supposed that nobody has 500 000 miners
       return parcelId;
    }

     function mintMiners(uint256 _count) external onlySaleActive notZeroAddress {
        uint256 minerId = _mintMiners(_count, msg.sender);
        uint256[] memory minerIds = new uint256[](1);
        minerIds[0] = minerId;
        emit NFTMinted(msg.sender, OPERATION_TYPE_MINT_MINERS, minerIds);
    }

    function _mintMiners(uint256 _count, address _user) private returns (uint256) {
        require(_count > 0, "Municipality: Can not mint 0 miners");
        require( usersMintableNFTAmounts[_user].miners >= _count, "Municipality: User do not have sufficient balance");
        //INetGymStreet(netGymStreetAddress).addGymMlm(_user, _referrerId);
        usersMintableNFTAmounts[_user].miners -= _count;
        emit BalanceUpdated(_user, 0, _count, true);
        uint256 minerId;
        if (IERC721Base(minerV1NFTAddress).balanceOf(_user) == 0) {
            minerId = IMinerNFT(minerV1NFTAddress).mint(_user);
        }
        minerInf[_user].totalHash += _count * 1000;
        minerInf[_user].freeHash += _count * 1000;
        //_attachMiners(_user, _count * 1000);
        return minerId;
    }
    ...
    //TODO change the following functions
    ...
    //     IERC721Base(standardParcelNFTAddress).setApprovalForAll(address(this), _approved); //TODO msg.sender is the contract so in setApp will not be th owner change setApprovaal?
    // }
    ...
    // }
     ...
     function web2AttachMiners(address _user, uint256 _hashRate) external onlyWeb2Backend{
        _attachMiners(_user, _hashRate);
    }

    function attachMiners(uint256 _hashRate) external {
        _attachMiners(msg.sender, _hashRate);
    }

    function _attachMiners(address _user, uint256 _hashRate) private {
        _hashRate = _hashRate > minerInf[_user].freeHash ? minerInf[_user].freeHash : _hashRate;
        uint256 freeSlotsH = parcelInf[_user].freeSlots * 1000;
        if (freeSlotsH < _hashRate) {
            IMining(miningAddress).deposit(_user, freeSlotsH);
            minerInf[_user].freeHash -= freeSlotsH;
            parcelInf[_user].freeSlots -= freeSlotsH / 1000;
            emit SlotHash(_user, 0, freeSlotsH/ 1000, 0, freeSlotsH, true);
        } else {
            IMining(miningAddress).deposit(_user, _hashRate);
            minerInf[_user].freeHash -= _hashRate;
            parcelInf[_user].freeSlots -= _hashRate / 1000;
            emit SlotHash(_user, 0, _hashRate / 1000, 0, _hashRate, true);
        }
    }
    ...
    function detachMiners(uint256 _hashRate) external {
        _hashRate = _hashRate > (minerInf[msg.sender].totalHash - minerInf[msg.sender].freeHash) ? (minerInf[msg.sender].totalHash - minerInf[msg.sender].freeHash) : _hashRate;
        IMining(miningAddress).withdraw(msg.sender, _hashRate);
        minerInf[msg.sender].freeHash += _hashRate;
        parcelInf[msg.sender].freeSlots += _hashRate / 1000;
        emit SlotHash(msg.sender, 0, _hashRate / 1000, 0, _hashRate, false);
    }
    ...
    /// @notice Transfer miners between metaverses
    // function _minerToMeta(address _user, uint8 _metaId, uint256 _freeHash) private {
    //     address _address;
    //     if (_metaId == 1) {
    //         _address = municipalityAddressKB;
    //     // } else if(_metaId == 2)  {
    //     //     _address = municipalityCashFT;
    //     } else {
    //        require(_metaId == 1 || _metaId == 2, "Municipality: Wrong metaId");
    //     }
    //     _freeHash = _freeHash > minerInf[_user].freeHash ? minerInf[_user].freeHash : _freeHash;
    //     IMunicipality(_address).addFreeHash(_user, _freeHash);
    //     minerInf[_user].freeHash -= _freeHash;
    // }

    // // transfer miners between metaverses
    // function addFreeHash(address _user, uint256 _freeHash) external OnlyWhiteListed {
    //     minerInf[_user].totalHash += _freeHash;
    //     minerInf[_user].freeHash += _freeHash;
    // } 

    // function setMinerInf(address _user, uint256 _totalHash, uint256 _freeHash) external OnlyWhiteListed {
    //     minerInf[_user].totalHash = _totalHash;
    //     minerInf[_user].freeHash = _freeHash;
    // } 

    //added
    function switchNewLogic(address[] memory _user, uint256[] memory _parcelCount, 
    uint256[] memory _freeSlots, uint256[] memory _totalHash, uint256[] memory _freeHash) external onlyWhiteListed { 
        for(uint8 i; i < _user.length; i++) {
            parcelInf[_user[i]].parcelCount = _parcelCount[i];
            parcelInf[_user[i]].freeSlots = _freeSlots[i];
            minerInf[_user[i]].totalHash = _totalHash[i];
            minerInf[_user[i]].freeHash = _freeHash[i];
        }
    }
}