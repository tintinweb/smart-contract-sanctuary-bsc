...
contract PWC is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
//custom
    IUniswapV2Router02 public uniswapV2Router;
//string
    string private _name = "PrimeWhiteRockComapany";
...
//address
    ...
    address public _BurnWalletAddress = 0x000000000000000000000000000000000000dEaD; // 4% Buy 6% Sell
    address private _BuybackWalletAddress = 0x000000000000000000000000000000000000dEaD; // Mixture of Lotto + Buyback
    address[] private _excluded;
//uint
    uint public buyReflectionFee = 2;
    uint public sellReflectionFee = 2;
    uint private buyBuybackFee = 0;
    uint private sellBuybackFee = 0;
    uint public buyLiqFee = 0;
    uint public sellLiqFee = 5;
    uint public buyBurnFee = 3;
    uint public sellBurnFee = 3;
    uint public buyFee = buyReflectionFee + buyBuybackFee + buyLiqFee + buyBurnFee;
    uint public sellFee = sellReflectionFee + sellBuybackFee + sellLiqFee + sellBurnFee;
    ...
    uint private _tTotal = 1000 * (10**18);
...
// constructor
    constructor() {
        ...
        uint total_supply = 1000 * 10 ** decimals();
        // set gars
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        uniswapV2Router = _uniswapV2Router;
        maxSellTxAmount = total_supply ; // 0.5% supply
        maxBuyTxAmount = total_supply ; // 2% supply
        excludedFromFees[address(this)] = true;
        excludedFromFees[owner()] = true;
        premarketUser[owner()] = true;
        excludedFromFees[_BurnWalletAddress] = true;
    ...
    function setFees() private {
        buyFee = buyReflectionFee + buyBuybackFee + buyLiqFee + buyBurnFee;
        sellFee = sellReflectionFee + sellBuybackFee + sellLiqFee + sellBurnFee;
    }
    ...
    function setBuybackFee(uint buy, uint sell) private onlyOwner() {
        buyBuybackFee = buy;
        sellBuybackFee = sell;
        setFees();
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBuyback, uint256 tBurn) {
        (tTransferAmount, tFee, tLiquidity, tBuyback, tBurn) = _getTValues(tAmount);
        (rAmount, rTransferAmount, rFee) = _getRValues(tAmount, tFee, tLiquidity, tBuyback, tBurn, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tBuyback, tBurn);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBuyback, uint256 tBurn) {
        tFee = calculateReflectionFee(tAmount);
        tLiquidity = calculateLiquidityFee(tAmount);
        tBuyback = calculateBuybackFee(tAmount);
        tBurn = calculateBurnFee(tAmount);
        tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tBuyback).sub(tBurn);
        return (tTransferAmount, tFee, tLiquidity, tBuyback, tBurn);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBuyback, uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rBuyback = tBuyback.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rBuyback).sub(rBurn);
...
//set functions
    function setBurnAddress(address _value) external onlyOwner {
        _BurnWalletAddress = _value;
    }
    function setbuybackAddress(address _value) private onlyOwner {
...
// operational functions
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBuyback, uint256 tBurn) = _getValues(tAmount);
