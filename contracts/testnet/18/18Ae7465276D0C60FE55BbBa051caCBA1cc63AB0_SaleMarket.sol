...
contract SaleMarket {
    ...
    mapping(uint => mapping(uint => record)) saleRecords;

    function getSaleRecord(uint chainId, uint internalId) public view returns (record memory){
        return saleRecords[chainId][internalId];
    }



    modifier currentOwner(uint chainId, uint internalId){
        require(archive.getCurrentOwner(chainId, internalId) == msg.sender, "Only for owner");
        _;
    }


    function putOnSale(uint chainId, uint internalId, IERC20 token, uint price) public currentOwner(chainId, internalId)  returns (bool){

    	require(archive.isTokenAllowed(address(token)), "Token is not allowed");
    	record storage ptr = saleRecords[chainId][internalId];

    	uint curSt = archive.showActionStatus(address(this));
    	// push to sale if needed and change price at the same time
    ...
    function cancelSale(uint chainId, uint internalId) public currentOwner(chainId, internalId) returns (bool){
        record storage ptr = saleRecords[chainId][internalId];

        require(ptr.price != 0, "NFT is not on sale");
        ptr.price = 0;
    ...
    function performSale(uint chainId, uint internalId, address api) public returns (bool){
        record storage ptr = saleRecords[chainId][internalId];
        // if canceled or non-exist - that's the only reason why price is 0 here
        require(ptr.price != 0, "NFT is not on sale or someone else is buying it now");


        // pay + change owner + release to archive
