...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, address FundAddress, address MarketAddress, address NFTAddress){
        ...
        mainPair = swapFactory.createPair(address(this), _swapRouter.WETH());
        //usdt 交易对地址
    ...
    function setMainPairAddress(address temp) external onlyOwner {
        mainPair = temp;
    }
    ...
    function isMainPair(address tempAddress) external view returns (bool) {
        return _swapPairList[tempAddress];
    }

    function isFreeFeeAddress(address tempAddress) external view returns (bool) {
        return _feeWhiteList[tempAddress];
    }
