...
contract BXFinancePrivateSale is Crowdsale, FinalizableCrowdsale, AccessControlUpgradeable {
    ...
    // fee(%) in token sold fee to BXFinance owner
    uint256 public tokenSoldFee;

    // BXFinance owner account
    ...
    function initialize(
      ...
      penalityFee = 10;
      deadWallet = 0xA52867b75eBf41acA065a3502D4203222b3503B9;
      capPercentage = 50;
    }
  ...
  function updateCapPercentage(uint256 _capPercentage) external onlyOwner {
    ...
    function finalization() virtual internal override{
      ...
      uint256 refundBalance = tokenBalanceInContract - tokensSold - tokensForBXFinanceOwner;
      if(refundType == 1){
  ...
  */
  ...
  function getLaunchPadDetails() external view returns (address, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
    return(
      address(token),
      rate,
      params.hardcap,
  ...
  /**
    @dev update dead wallet address
    @param _deadWallet dead wallet address
   */
