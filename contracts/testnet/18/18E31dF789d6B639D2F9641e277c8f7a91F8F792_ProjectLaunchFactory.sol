...
contract ProjectLaunchFactory {
  ...
  // Token multiplier
  mapping(uint => uint256) internal round1MultT;
  mapping(uint => uint256) internal round2MultT;
  mapping(uint => uint256) internal round3MultT;
  ...
  function createSale(
    ...
    Sale sale = new Sale(
      msg.sender,
      iSaleVaultFactory(vaultContract).createVault(msg.sender, _sale_token),
      _sale_token,
      _sale_details,
      _seed_details,
      _seed_vesting,
  ...
  function getOwner() public view returns(address admin) {
      return (adminAddress);
  }
  ...
  function getAdmin() public view returns(address owner_) {
      return (_owner);
  }
