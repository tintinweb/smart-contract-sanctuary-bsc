...
contract testdapp  {
    ...
    uint public X4_LEVEL_POOL_SUM = 100;
    uint public X5_LEVEL_POOL_SUM = 200;
    uint public X6_LEVEL_POOL_SUM = 1000;
    uint public X7_LEVEL_POOL_SUM = 2000;
    address public admin = 0x7A62CC9a488E0BcF03323612c3279CA5c3029326;
        ...
        for(uint i=0; i < level_income.length; i++){
            ...
            if(my_sponser_level.sponser_id != address(0)){
                if(i == 0){ 
                    ...
                    balances[my_sponser_level.user_id] = balances[my_sponser_level.user_id].add(level_income[1]);               
 ...
 function upgradeAccount(uint8 plan ) external {
        ...
        for(uint i=0; i < level_income.length; i++){
            ...
            if(my_sponser_level.sponser_id != address(0)){
                if(sponser_count == plan+1   && my_sponser_level.package_id >= plan ){
