 ...
 */
abstract contract VinoxHubStorage {
    mapping(address => bool) internal _playerMinted;
    mapping(address => Types.OnboardingStatus) _userOnboarded;
    mapping(address => bool) internal _governAdmins;
}

pragma solidity ^0.8.9;

import "../interfaces/IUtility.sol";
import "../storage/VinoxStorage.sol";
import "../lib/types.sol";
import "../../../tokens/ERC1155Base.sol";

contract Utility is ERC1155Base, VinoxHubStorage, IUtility {
    string public name;
    ...
    function mint(address to, uint256 id, uint256 amount, bytes memory data) public override onlyRole(GOVERNOR_ROLE) {
        require(id != 1 || !_playerMinted[to], "Player already claimed onboarding pass");
        _mint(to, id, amount, data);
        _playerMinted[to] = true;
    }
    ...
    function burn(
        ...
        uint256 id,
        uint256 value
    ) public override(IUtility, ERC1155Burnable) {
    ...
    function mintBatch(
        ...
        require(!onboardingCharacter || !_playerMinted[to], "Player already claimed onboarding pass");
        _mintBatch(to, ids, amounts, data);
        if (onboardingCharacter) {
            _playerMinted[to] = true;
        }
    }
...
abstract contract ERC1155Base is ERC1155Burnable, Pausable, FixinAccessControl {
    /**
