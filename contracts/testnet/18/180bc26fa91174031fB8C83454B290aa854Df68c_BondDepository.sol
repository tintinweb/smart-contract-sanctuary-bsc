...
contract BondDepository is Ownable {
  ...
  address public immutable EVHY; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints EVHY when receives principle
  ...
  struct Bond {
    uint256 payout; // EVHY remaining to be paid
  ...
  function setBondTerms(PARAMETER _parameter, uint256 _input) external onlyOwner {
    if (_parameter == PARAMETER.VESTING) {
      // 0
      // 43200 is estimate block for 36 hours
      require(_input >= 43200, "BondDepository: Vesting must be longer than 36 hours");
      terms.vestingTerm = _input;
    } else if (_parameter == PARAMETER.PAYOUT) {
      // 1
      require(_input <= 1000, "BondDepository: Payout cannot be above 1 percent");
    ...
    } else if (_parameter == PARAMETER.MINIMUMPRICE) {
      // 4
  ...
  function deposit(
    ...
    require(_maxPrice >= nativePrice, "BondDepository: slippage limit more than max price"); // slippage protection

    uint256 value = ITreasury(treasury).valueOfToken(principle, _amount);
    uint256 payout = payoutFor(value); // payout to bonder is computed

    require(payout >= 1e16, "BondDepository: bond too small"); // must be > 0.01 EVHY ( underflow protection )
    ...
    /**
      ...
      approved and
      deposited into the treasury, returning (_amount - profit) EVHY
    */
    ERC20(principle).safeTransferFrom(msg.sender, address(this), _amount);
    ERC20(principle).approve(address(treasury), _amount);
    ITreasury(treasury).deposit(_amount, principle, profit);

    if (fee != 0) {
      // fee is transferred to dao
      ERC20(EVHY).safeTransfer(DAO, fee);
    }
  ...
  function stakeOrSend(
    ...
    if (!_stake) {
      ...
      ERC20(EVHY).safeTransfer(_recipient, _amount); // send payout
    } else {
      // if user wants to stake
      if (useHelper) {
        // use if staking warmup is 0
        ERC20(EVHY).approve(stakingHelper, _amount);
  ...
  function maxPayout() public view returns (uint256) {
    return ERC20(EVHY).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  function payoutFor(uint256 _value) public view returns (uint256) {
    return _value.mul(1e18).div(bondPrice()).div(1e16);
  }
  ...
  /**
   *  @notice calculate current ratio of debt to EVHY supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = ERC20(EVHY).totalSupply();
    debtRatio_ = currentDebt().mul(1e18).div(supply);
  }
  ...
  function standardizedDebtRatio() external view returns (uint256) {
    // TODO: update after research LP bond
    // if ( isLiquidityBond ) {
