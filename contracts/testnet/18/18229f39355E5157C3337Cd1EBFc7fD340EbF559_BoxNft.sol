...
ERC721Burnable {
    ...
    function setReceivedAddress(address _receivedAddress) external {
        require(_receivedAddress != address(0), "_receivedAddress must be not equal address 0x");
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "must have admin role");
    ...
    function buyBox(address toAddress, uint256 quantity, uint256 price) external {
        ...
        // check approve
        uint256 pay_amount = price * quantity;

        uint256 approveAmount = paymentToken.allowance(toAddress, address(this));
        require(approveAmount >= pay_amount, "allowance is not enough");

        uint256 tokenBalance = paymentToken.balanceOf(toAddress);
        require(tokenBalance >= pay_amount, "token balance is not enough");

        paymentToken.transferFrom(toAddress, receiveAddress, pay_amount);
    ...
    function summonItem(uint256 tokenId) external {
        ...
        require(ERC721.ownerOf(tokenId) == msg.sender, "sender is not own of tokenId");

        // check tokenId exist
        require(_exists(tokenId), "operator query for nonexistent token");

        uint256 itemTokenId = nftToken.itemMint(msg.sender);

        ERC721Burnable.burn(tokenId);

        emit itemSummoned(tokenId, itemTokenId);
    }
