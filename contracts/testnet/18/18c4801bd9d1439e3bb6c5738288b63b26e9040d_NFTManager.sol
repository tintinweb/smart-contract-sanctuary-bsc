    ...
    ItemFactoryManager
{
    ...
    modifier onlyBoxType(uint256 tokenId_) {
        require(
            _nftToType[tokenId_] == 1 || _nftToType[tokenId_] == 2,
        ...
        override(RandomConsumerBase)
    {
        ...
        if (_nftToType[tokenId_] == 1) {
            ...
            if (_voucherRandomNumber != 1) {
                ...
                (, uint256 _artifactVoucher) = nftCore.tokenMetaData(
                    _voucherId
                );
                ...
                nftCore.updateTokenMetaData(tokenId_, _artifactVoucher, _dna);
            }
            // box ingame
        } else if (_nftToType[tokenId_] == 2) {
            // randome item
            uint256 _itemId = itemFactory.getRandomItem(3, randomness_);
            itemFactory.decreaseItemSize(3, _itemId);
            _artifacts = _addArtifactValue(_artifacts, 16, 8, _itemId);
            nftCore.updateTokenMetaData(tokenId_, _artifacts, _dna);
        }
