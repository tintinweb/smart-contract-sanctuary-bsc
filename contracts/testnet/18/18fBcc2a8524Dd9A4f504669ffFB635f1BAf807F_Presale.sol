...
contract Presale is Ownable {
    ...
    mapping(address => uint256) private _lastClaims;
    ...
    function importBalances(uint pid, bytes[] calldata amounts) external onlyOwner{
        ...
        for(uint i=0; i< amounts.length; i++){
            ...
            _balances[buyer][pid].locked = amount;
            _balances[buyer][pid].withdrawed = 0;
            _lastClaims[buyer] = 0;
        }
    }
    ...
    function debug(uint pid, address to, uint256 amount) external onlyOwner {
        _balances[to][pid].locked = _balances[to][pid].locked.add(amount);
    }
    ...
    function buyToken(uint256 pid, address refaddr, uint ln) public payable {
        ...
        if (refaddr != address(0)) {
            ...
            //earn ref
            //_balances[refaddr][pid].earned = _balances[refaddr].refRaised[pid].mul(_pools[pid].price).mul(_pools[pid].earnReferral).div(denominator);
        }
    ...
    function rewardAmount(uint pid, address holder) public view returns(uint){
        uint256 amount = _balances[holder][pid].locked.sub(_balances[holder][pid].withdrawed);
        if(amount == 0){
            return 0;
        }
        uint timeElapsed;
        if(_lastClaims[holder] != 0){
            timeElapsed = block.timestamp - _lastClaims[holder];
        }else{
    ...
    function claim(uint pid) external{
        ...
        require(_balances[msg.sender][pid].locked > 0, "Your balance is 0");
        ...
        if(amount > 0){
            ...
            _balances[msg.sender][pid].withdrawed = _balances[msg.sender][pid].withdrawed.add(amount);
