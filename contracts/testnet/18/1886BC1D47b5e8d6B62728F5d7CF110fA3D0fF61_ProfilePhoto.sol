...
contract ProfilePhoto is OwnableUpgradeable, ERC1155Upgradeable {
    ...
    function queryUserProfilePhotos(address addr_) public view returns (string[]memory) {
        uint[] memory userArr = userPhotos[addr_];

        string[] memory arr = new string[](userArr.length);
        for (uint i = 0; i < userArr.length; i++) {
            arr[i] = string(abi.encodePacked("Profile Photo-",userArr[i]));
        }
