...
// File contracts/interfaces/IsTAZOR.sol


interface IsTAZOR is IERC20 {
    function rebase( uint256 tazorProfit_, uint epoch_) external returns (uint256);
...
// File contracts/interfaces/IgTAZOR.sol


interface IgTAZOR is IERC20 {
  ...
  function migrate( address _staking, address _sTAZOR ) external;
}
...
contract TazorDAOStaking is TazorAccessControlled, Ownable, ReentrancyGuard {
    ...
    uint256 public timeIntervalForTest  = 3600; // 1hour               // time interval which is updated when user stake | unstake | getReward.
    ...
    function calcReward(address account) public view returns (uint256) {
        ...
        uint256 newEarned = user.tazorNum.mul(rate).div(10 ** 9);
        newEarned = newEarned.mul(user.apr).mul(timeinterval).div(365 days).div(10 ** 9);         // apr value should be divide by 10 ** 9 
        return (newEarned).mul(144);
    }


    function calcBurn(address account) public view returns (uint256) {
        
        uint256 timeinterval = block.timestamp.sub(userInfos[account].lastUpdateTime);      
               
        if ( _totalTazorSupply > 0 && _totalTazSupply > 0) {

            UserInfo memory user = userInfos[account];
            uint256 numberOfEpoch = timeinterval.mul(10 ** 9).div(1 days);                    
            uint256 diffNode1 = user.tazNum.mul(user.tazorNum).div(_totalTazorSupply).mul(3).div(100);
            uint256 diffNode2 = user.tazNum.mul(user.tazNum).div(_totalTazSupply).mul(3).div(100);
            uint256 burnAmount = (diffNode1.add(diffNode2)).mul(numberOfEpoch).div(10 ** 9);

            return burnAmount.mul(144);
        }
    ...
    function stakeTazor(uint256 amount, address receipient) external payable nonReentrant {
    ...
    function stakeTaz(uint256 amount, address receipient) external payable nonReentrant {
    ...
    function unstakeTazor(uint256 amount) public payable nonReentrant {
    ...
    function unstakeTaz(uint256 amount) public payable nonReentrant {
        require(amount >= 0, "Cannot withdraw <0");
        // require(block.timestamp > userInfos[msg.sender].lastUpdateTime + 12 hours, "can't draw tokens");
    ...
    function canClaim(address _user) public view returns (bool) {
        ...
        return block.timestamp >= user.epochStartTime + timeIntervalForTest;
    }
    ...
    function claim() internal {
        ...
        if (msg.sender != address(0)) {
            ...
            uint256 burnAmount = calcBurn(msg.sender);

            if (burnAmount > userInfos[msg.sender].tazNum) {
                _totalTazSupply = _totalTazSupply - userInfos[msg.sender].tazNum;
                burnAmount = burnAmount - userInfos[msg.sender].tazNum;
                require(_totalTazorSupply >= 0, "Fatal Error, totalTazorSupply < 0");
                tazToken.burn(userInfos[msg.sender].tazNum);
                userInfos[msg.sender].tazNum = 0;
                setAPRvalue();

                if (reward > 0) {
                    if (reward > burnAmount) {
                        reward = reward - burnAmount;
                    } else {
                        reward = 0;
                    }
                ...
                if (burnAmount > 0) {
                    userInfos[msg.sender].tazNum = userInfos[msg.sender].tazNum.sub(burnAmount);
    ...
    function getReward(address account) external view returns(uint256) {
        ...
        uint256 newEarned = user.tazorNum.mul(rate).div(10 ** 9);                                 // rate / 10**9
        newEarned = newEarned.mul(user.apr).mul(timeinterval).div(365 days).div(10 ** 9);         // apr value should be divide by 10 ** 9 
        return newEarned.mul(144);
    }
    ...
    function setTimeIntervalForTest(uint256 _timeIntervalForTest) external onlyGovernor {
        require(_timeIntervalForTest > 300, "_timeIntervalForTest can't less than 5 min");
        timeIntervalForTest = _timeIntervalForTest;
    }
    ...
    function secondsToNextEpoch(address account) external view returns (uint256) {
        ...
        if (user.lastUpdateTime != 0) {
            ...
            return (remain.div(timeIntervalForTest) + 1).mul(timeIntervalForTest).sub(remain);
        } else {
            return 0;
        }
    }
