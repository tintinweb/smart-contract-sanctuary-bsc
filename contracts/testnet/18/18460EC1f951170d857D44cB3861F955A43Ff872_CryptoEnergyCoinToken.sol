 ...
 */
contract CryptoEnergyCoinToken is Context, IERC20, IERC20Metadata, Ownable, Pausable {
    ...
    address private constant UNISWAPV2ROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        {
            uint256 amt = tAmount;
            uint256 tTransferAmount = amt.sub(tFee).sub(tLiquidity).sub(tBurn).sub(tCharityFee);
    ...
    function getRValues(uint256 amount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tCharityFee) internal view returns (uint256, uint256, uint256) {
        ...
        {
            uint256 amt = rAmount;
            uint256 rTransferAmount = amt.sub(rFee).sub(rliquidity).sub(rBurn).sub(rCharityFee);
    ...
    function takeCharityFee(address sender, uint256 tCharityFee) internal {
        uint256 currentRate =  getRate();
        uint256 rCharityFee = tCharityFee.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)] + rCharityFee; 
        if(_isExcluded[address(this)]){
             _tOwned[address(this)] = _tOwned[address(this)] + tCharityFee;
        }
        if(tCharityFee > 0) emit Transfer(sender, address(this), tCharityFee);
    }
    ...
    function removeAllFee() internal {
        if(taxFee == 0 && liquidityFee == 0 && transactionBurn == 0 && charityFee == 0 ) return;
