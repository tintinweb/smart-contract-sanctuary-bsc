...
contract BattleContract is ERC2771Context, Ownable, ReentrancyGuard {
    ...
    uint8[4] rankPercent = [5, 10, 20, 45]; // x100
    uint8[4] rewardPercent = [30, 20, 20, 30]; // x100
    uint16[5] bonusPercent = [500, 250, 150, 75, 25]; // x1000

    uint256 public baseEnterFee = 0.1 ether;
    ...
    modifier Bettable(uint256 _battleId) {
        ...
        require(
            battle.state == uint8(BattleState.Betting),
    ...
    modifier Claimable(uint256 _battleId) {
        ...
        BattleInfo memory battle = battles[_battleId];
        require(
            battle.state == uint8(BattleState.Ended),
            "Battle is not ended yet"
        );
        _;
    }

    event _CreateBattle(uint8 _contestType, uint256 _battleId, address _creator, uint8 _battleType);
    ...
    function BetBattle(
        ...
        for (uint8 i = 0; i < _scalarIds.length; i++) {
    ...
    function StartBattles() public onlyAdmin {
        ...
        for (uint256 i = 0; i < battleCount; i++) {
            ...
            if (playerCount < 4) {
                ...
                battles[_battleId].state = uint8(BattleState.Ended);
            } else {
                battles[_battleId].state = uint8(BattleState.Started);
            }
        }
        ...
        onlyAdmin
    {
        ...
        require(battle.state == uint8(BattleState.Started), "Battle have not started yet"); // check that battle status is started state

        battle.state = uint8(BattleState.Ended); // set battle status to ended state
        ...
        uint8 bonusCount = uint8(playerCount < 5 ? playerCount : 5);
...
contract ScalarNFT is ERC721, VRFConsumerBaseV2, BillionsBaseContract {
    ...
    * store the price to refill(USDC)
    ...
    /**
    * return the price to refill(USDC)
     */
    function getRefillPrice() external view returns (uint256) {
        return refillPrice;
    }
