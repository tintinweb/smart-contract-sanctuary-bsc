...
contract TokenFactory is AccessControl, DeployFee {
    ...
    function deployToken(
        ...
        if(!hasRole(WHITE_LIST, _msgSender()) || deployFeePaymentOption != NO_PAYMENT_OPTION){
...
abstract contract DeployFee is Context {
    ...
    function tokenChargeDeployFee(IERC20Metadata paymentToken) internal {
        ...
        if (deployFeePaymentOption == FIXED_PAYMENT_OPTION) {
            ...
            paymentToken.safeTransferFrom(
                ...
                deployFeeBeneficiary,
                tokenDecimals >= EXP_VALUE
                    ? ((deployFeeFixedAmount * deployFeePercentageAmount) /
                        EXP_VALUE) * (tokenDecimals / EXP_VALUE)
                    : ((deployFeeFixedAmount * deployFeePercentageAmount) /
                        EXP_VALUE) / (EXP_VALUE / tokenDecimals)
            );
        }
    }
    ...
    function calculateRequiredETH()
        internal
        view
        returns (uint256 fixedRequiredETH, uint256 percentageRequiredETH)
    {
        ...
        if (priceFeedDecimals >= EXP_VALUE) {
            ...
            percentageRequiredETH =
                ((deployFeeFixedAmount / (uint256(ethUsdPrice) / EXP_VALUE)) *
            ...
            percentageRequiredETH =
                (((deployFeeFixedAmount * priceFeedDecimals) /
                    uint256(ethUsdPrice)) * deployFeePercentageAmount) /
                EXP_VALUE;
        }
    }
}
