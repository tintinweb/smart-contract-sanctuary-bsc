...
contract TestPTP is ERC20 {
    ...
    function createFreeStakeRecord(address freeStakeHolder,string memory btcAddress,uint256 day,uint256 balance,address refererAddress) internal onlyOwner{

        freeStakePerDayInfo memory myRecord = freeStakePerDayInfo({walletAddress:freeStakeHolder,balanceAtMoment:balance,btcAddress:btcAddress,dayFreeStake:day,refererAddress:refererAddress});
    ...
    function createFreeStake(address user,string memory btcAddress,uint balance,address refererAddress) public onlyOwner{
        ...
        createFreeStakeRecord(user,btcAddress,day,balance, refererAddress); 
        claimedBtcAddrCount ++ ;
