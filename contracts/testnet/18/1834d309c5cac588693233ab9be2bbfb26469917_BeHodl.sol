...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("BeHodl_Busd_Dividend_Tracker", "BeHodl_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 500000 * (10**18); //must hold 5,00,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "BeHodl_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "BeHodl_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BeHodl contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BeHodl_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BeHodl_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BeHodl is ERC20, Ownable {
    ...
    bool public developerEnabled = true;
    ...
    address public DevSponserShipfund;
    address public _admin;
 
    uint256 public maxBuyTranscationAmount = 500000000 * (10**18);  //500 million
    uint256 public maxSellTransactionAmount = 10000000 * (10**18);   //10 million
    uint256 public maxWalletBalance = 1000000000 * (10**18);  //1 billion
    uint256 public swapTokensAtAmount = 30000000 * 10**18;   //30 million

    uint256 public initsupp = 1000000000 * 10**18; //1 billion
    ...
    uint256 public previousdevfunds;

    uint256 public totalFees = busdDividendRewardsFee.add(marketingFee).add(liquidityFee).add(devfunds).add(buybackfee);
    
    uint256 public _busdDividendRewardsFeeBuy = 6;
    uint256 public _marketingFeeBuy = 2;
    uint256 public _devfundbuy = 2;  //
    uint256 public _LpFeeBuy = 2;
    uint256 public _buybackbuy = 1;  //

    uint256 public _busdDividendRewardsFeeSell=6;
    uint256 public _LpFeeSell=2;
    uint256 public _marketingFeeSell=2;  
    uint256 public _devfundsell = 2;  //
    uint256 public _buybacksell = 1;  //
 
    uint256 public busdDividedRewardsInContract;
    uint256 public tokensForLpInContract;
    uint256 public tokensForbuybackInContract;  //
    ...
    event developerEnabledUpdated(bool enabled);
    ...
    constructor() ERC20("BeHodl", "BEHDL") {
        	
    	marketingWallet = 0x80b5a6C5813a94d81d4F6030EC88D3279dF258F9; 
        DevSponserShipfund = 0xb8Cc9C99A759463b14400cEb86e5adF4BEd72aBE;
        _admin = 0x47e74F803671757138d5692034951D4478aE9721;
        ...
        excludeFromFees(DevSponserShipfund, true);
        excludeFromFees(address(this), true);
        excludeFromFees(deadAddress, true);
        excludeFromFees(_admin, true);
 
        setAuthOnDividends(_admin);
    ...
    function setBuyBackLimit(uint256 _limit) external onlyOwner {
  	    buyBackUpperLimit = _limit;
  	}
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "BeHodl: The marketing wallet is already this address");
    ...
    function updateDeveloperWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != DevSponserShipfund, "BeHodl: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit DeveloperWalletUpdated(DevSponserShipfund, _newWallet);
  	    DevSponserShipfund = _newWallet;
  	}
...
//
    function setbuybackEnabled(bool _enabled) external onlyOwner {
     ...
     function setDeveloperEnabled(bool _enabled) external onlyOwner {
        require(developerEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            previousdevfunds = devfunds;
            devfunds = 0;
            developerEnabled = _enabled;
        }
 
        emit developerEnabledUpdated(_enabled);
    }
...
///
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "BeHodl: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "BeHodl: The new dividend tracker must be owned by the BeHodl token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "BeHodl: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "BeHodl: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BeHodl: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "BeHodl: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "BeHodl: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "BeHodl: Trading has not started yet");
        ...
        if (
            ...
            devfunds = _devfundbuy;
            buybackfee = _buybackbuy;
            ...
            devfunds = _devfundsell;
            buybackfee = _buybacksell;
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (buybackEnabled) {
                uint256 buybackTokens = tokensForbuybackInContract;
                buyBackAndBurn(buybackTokens);
                tokensForbuybackInContract = 0;
            }
        ...
        if(takeFee) {
            ...
            uint256 tmpdevSponserRewardPercent;
            uint256 tmpBusdDividedRewardsInContract;
            uint256 tmpLpRewardInContract;
            uint256 tmptokensForbuybackInContract;
            ...
            tmpdevSponserRewardPercent = amount.mul(devfunds).div(100);
            tmptokensForbuybackInContract = amount.mul(buybackfee).div(100);
            


            fees = tmpMarketingRewardPercent.add(tmpBusdDividedRewardsInContract)
                            .add(tmpLpRewardInContract).add(tmpdevSponserRewardPercent).add(tmptokensForbuybackInContract);
            ...
            tokensForbuybackInContract = tokensForbuybackInContract.add(tmptokensForbuybackInContract);
            ...
            super._transfer(address(this),DevSponserShipfund,tmpdevSponserRewardPercent);  ///
        }
