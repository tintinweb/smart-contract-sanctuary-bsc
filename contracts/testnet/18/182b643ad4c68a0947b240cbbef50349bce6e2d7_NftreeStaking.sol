...
// Get a link to Carbon Token smart contract
interface ICOT {
    // Mint new tokens
    function mint(
        address user,
...
contract NftreeStaking {
  ...
  // Fee for carbon token registration
  uint256 public carbonTokenFee;
  ...
  /** 
    * @dev Sets locking period for carbon token generation
    *
  ...
  /** 
    * @dev Sets fee for carbonToken generation
    *
    * @notice restricted access function 
    * @param fee_ unsigned integer defines fee
    */
  function setCarbonTokenFee(
     uint256 fee_
  ) 
    external 
  {
  ...
  /** 
    * @dev Sets harvest fee for carbon token generation
    *
    * @notice restricted access function 
    * @param fee_ unsigned integer defines fee
    */
  ...
  function harvest(uint256 index_) public payable {
    ...
    uint256 _tco2Amount = _unclaimed * 1e12;

    // Calculate fee
    uint256 _fee = (carbonTokenFee * _tco2Amount) / 1e18;
    ...
    ICOT(tco2).mint(msg.sender, _tco2Amount);
    ...
    returns (uint256)
  {
    ...
    return (_unclaimed * 1e12 * carbonTokenFee) / 1e18;
  }
