...
/// @title WEFIN Bond Depository V2

contract EFINBondDepositoryV2 is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) NoteKeeper(_authority, _wEFIN, _gEFIN, _staking, _treasury) {
        // save gas for users by bulk approving stake() transactions
        _wEFIN.approve(address(_staking), 1e45);
    }
     ...
     * @return payout_     the amount of gEFIN due
        ...
        )
    {
        ...
        console.log("Price: %s -- maxPrice: %s", price, _maxPrice);
         ...
         * price = quote tokens : WEFIN (i.e. 42069 BUSD : WEFIN)
         *
         * 1e18 = WEFIN decimals (9) + price decimals (9)
         */
        ...
        // console.log("amount %s --- price %s --- payout_ %s", _amount, price, payout_);
        // console.log("price %s --- decimals %s", price, 10**metadata[_id].quoteDecimals);
        require(payout_ <= market.maxPayout, "Depository: max size exceeded");

        /*
         * each market is initialized with a capacity
         *
         * this is either the number of WEFIN that the market can sell
        ...
        // purchased, and how much WEFIN has been sold
        ...
        // console.log("quoteToken",address(market.quoteToken));
        // console.log(" ");

        // console.log("165");
        market.quoteToken.transferFrom(msg.sender, address(treasury), _amount);
        // console.log("167");

        // if max debt is breached, the market is closed
        // this a circuit breaker
        // console.log("maxDebt %s", term.maxDebt);
        // console.log("totalDebt %s", market.totalDebt);
        if (term.maxDebt < market.totalDebt) {
            market.capacity = 0;
            console.log("MARKET CLOSED");
            emit CloseMarket(_id);
        } else {
            console.log("MARKET TUNED");
    ...
    function _decay(uint256 _id, uint48 _time) internal {
        ...
        if (adjustments[_id].active) {
            ...
            if (stillActive) {
                // console.log("--- change %s", adjustBy);
                // console.log("--- adjustment.change before %s", adjustment.change);
                adjustment.change -= adjustBy;
                adjustment.timeToAdjusted -= secondsSince;
                adjustment.lastAdjustment = _time;
                // console.log("--- adjustment.change after %s", adjustment.change);
            } else {
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        if (_time >= meta.lastTune + meta.tuneInterval) {
            ...
            // WEFIN decimals (9) + price decimals (9)
             ...
             * is 10,000 WEFIN. max payout would be 1,000 WEFIN (10,000 * 1 / 10).
             */
            ...
            if (newControlVariable >= terms[_id].controlVariable) {
                ...
                // console.log("***info.change %s", info.change);
                // console.log("***price %s", price);
                // console.log("***treasury.baseSupply() %s", treasury.baseSupply());
                // console.log("***targetDebt %s", targetDebt);
                // console.log("***terms[_id].controlVariable %s", terms[_id].controlVariable);
                // console.log("***newControlVariable %s", newControlVariable);
     ...
     * @param _market      [capacity (in WEFIN or quote), initial price / WEFIN (9 decimals), debt buffer (3 decimals)]
    ...
    function create(
        ...
        // console.log("_terms[1]", _terms[1]);
        uint256 secondsToConclusion = _terms[1] - block.timestamp;

        // the decimal count of the quote token
        uint256 decimals = IERC20Metadata(address(_quoteToken)).decimals();
        // console.log("_quoteToken", address(_quoteToken));
         ...
         *
         * 1e18 = WEFIN decimals (9) + initial price decimals (9)
         */
        // console.log("targetDebt calc _booleans[0]: %s -- if true %s -- if false %s",_booleans[0], uint256(((_market[0] * 1e18) / _market[1]) / 10**decimals), uint256(_market[0]) );
        uint64 targetDebt = uint64(_booleans[0] ? ((_market[0] * 1e18) / _market[1]) / 10**decimals : _market[0]);

        /*
         * max payout is the amount of capacity that should be utilized in a deposit
         * interval. for example, if capacity is 1,000 WEFIN, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 WEFIN.
         */

        console.log("Capacity in quote ? ", _booleans[0]);
        console.log("targetDebt", targetDebt);
        console.log("secondsToConclusion", secondsToConclusion);
        console.log("maxPayout", (targetDebt * _intervals[0]) / secondsToConclusion);
         ...
         */
        console.log("------------------- > _market[2]", _market[2]);
        ...
        emit CreateMarket(id_, address(wEFIN), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in WEFIN decimals
     *
    ...
    function marketPrice(uint256 _id) public view override returns (uint256) {
        console.log(" ");
        // console.log("currentControlVariable(_id) ", currentControlVariable(_id));
        // console.log("debtRatio(_id)", debtRatio(_id));
        // console.log("10**metadata[_id].quoteDecimals-- ", 10**metadata[_id].quoteDecimals);
        console.log(" ");
     ...
     * @param _id          ID of market
     * @return             amount of WEFIN to be paid in WEFIN decimals
     *
     * @dev 1e18 = WEFIN decimals (9) + market price decimals (9)
     */
     ...
     * @param _id          ID of market
     * @return             current debt for market in WEFIN decimals
     */
     ...
     * @param _id          ID of market
     * @return             control variable for market in WEFIN decimals
     */
    ...
    function _debtRatio(uint256 _id) internal view returns (uint256) {
        // console.log("");
        // console.log("_debtRatio");
        // console.log("markets[_id].totalDebt", markets[_id].totalDebt);
        // console.log("10**metadata[_id].quoteDecimals", 10**metadata[_id].quoteDecimals);
        // console.log("treasury.baseSupply()", treasury.baseSupply());
        // console.log("");
        ...
        )
    {
        ...
        // console.log("  ");
        // console.log(" - ");
        // console.log(" active_ ", active_);
        // console.log(" info.change", info.change);
        // console.log(" secondsSince_ ", secondsSince_);
        // console.log(" info.timeToAdjusted ", info.timeToAdjusted);
        // console.log(" info.change ", info.change);
        // console.log(" - ");
        // console.log("  ");
        // console.log(" info.change * secondsSince_ ", info.change * secondsSince_);
...
abstract contract FrontEndRewarder is EFINAccessControlled {
    /* ========= STATE VARIABLES ========== */
    ...
    IERC20 internal immutable wEFIN; // reward token

    constructor(IEFINAuthority _authority, IERC20 _wEFIN) EFINAccessControlled(_authority) {
        wEFIN = _wEFIN;
    }
    ...
    function getReward() external {
        ...
        wEFIN.transfer(msg.sender, reward);
    }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    IgEFIN internal immutable gOHM;
    ...
    function redeem(
        ...
        console.log("Payout amount ", payout_);
        if (_sendgOHM) {
