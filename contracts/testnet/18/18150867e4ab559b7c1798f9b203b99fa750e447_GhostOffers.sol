...
contract GhostOffers is Auth, ERC721Holder {
    ...
    function removeTokenOffer(
        ...
        if (offerIndex >= _tokenOffersByCollection[nftAddress][nftId].length)
            return;

        for (
            uint256 i = offerIndex;
            i < _tokenOffersByCollection[nftAddress][nftId].length - 1;
            i++
        ) {
            _tokenOffersByCollection[nftAddress][nftId][
                i
            ] = _tokenOffersByCollection[nftAddress][nftId][i + 1];
        }
    ...
    function removeNFTOffer(
        ...
        if (offerIndex >= _nftOffersByCollection[nftAddress][nftId].length)
            return;

        for (
            uint256 i = offerIndex;
            i < _nftOffersByCollection[nftAddress][nftId].length - 1;
            i++
        ) {
            _nftOffersByCollection[nftAddress][nftId][
                i
            ] = _nftOffersByCollection[nftAddress][nftId][i + 1];
        }
    ...
    function rejectOffer(
        ...
        if (isToken) {
            //token offer
            require(
                _tokenOffersByCollection[_nftAddress][_nftId][offerIndex]
                    .tokenOfferId == offerId,
            ...
            //nft offer
            require(
                _nftOffersByCollection[_nftAddress][_nftId][offerIndex]
                    .nftOfferId == offerId,
                "The offers have updated."
            );
            _nftOffersByCollection[_nftAddress][_nftId][offerIndex]
    ...
    function acceptNFTOffer(
        ...
        nft.transferFrom(
            msg.sender,
            _nftOffersByCollection[_nftAddress][_nftId][offerIndex].buyer,
            _nftId
        );

        uint256 expectedTokenAmount = _nftOffersByCollection[_nftAddress][
            _nftId
        ][offerIndex].soldNftIds.length;
    ...
    function cancelTokenOffer(
        ...
        require(
            msg.sender ==
                _tokenOffersByCollection[_nftAddress][_nftId][offerIndex].buyer,
        ...
        token.transfer(
            msg.sender,
            _tokenOffersByCollection[_nftAddress][_nftId][offerIndex].price
        );
        removeTokenOffer(_nftAddress, _nftId, offerIndex);
    }
    ...
    function createNFTOffer(
        ...
        uint256 _nftId,
        address offeredNFT,
        uint256[] calldata offeredIds
    ) public {
        IERC721 offeredNftAddress = IERC721(offeredNFT);

        require(
            isAcceptableNFT(offeredNFT),
            "You can not offer this token currently."
        );
        require(
            isAcceptableOfferedNFT(_nftAddress),
        ...
        for (uint256 i = 0; i < offeredIds.length; i++) {
            require(
                offeredNftAddress.ownerOf(offeredIds[i]) == msg.sender,
