...
contract shareBUSD is Ownable, Pausable, SafeBEP20{
    ...
    event DepositBUSD(address indexed User, uint256 TokenAmount, uint256 Time);
    ...
    constructor(address[3] memory _tokens_wallets) {
        BUSDtoken = IBEP20(_tokens_wallets[0]);
        wallet1 =_tokens_wallets[1];
        wallet2 =_tokens_wallets[2];
    }
    ...
    function deposit(uint256 _tokenAmount, address user) external whenNotPaused  {

        safeTransferFrom(BUSDtoken, _msgSender(), address(this), _tokenAmount);

        safeTransfer(BUSDtoken, user, _tokenAmount * sharePercentage[0] / 1e3);
        ...
        emit DepositBUSD(_msgSender(), _tokenAmount, block.timestamp);
    ...
    function updateWallet(address[2] memory _wallets) external onlyOwner{
        require(_wallets[0] != address(0) && _wallets[1] != address(0), "Invalid wallet.");
        wallet1 = _wallets[0];
        wallet2 = _wallets[1];
    }
