...
contract GForcexXx2 is Ownable {
    ...
    mapping(address => JoinParam) internal JoinParams;
    mapping(address => bool) public GlobalUserExists;
    mapping(address => User) public GlobalUsers;
    uint256 public userIndex;
    IERC20 public currency;
    uint public pricePerDraw;

    constructor() {

        createNewUserInternal(msg.sender, "1", 0x0000000000000000000000000000000000000000);
    }
    ...
    function printUser(address id) public view returns(string memory) {
    ...
    function createNewUserInternal(address id, string memory position, address uplineId) internal returns(User storage) {
        User storage user = GlobalUsers[id];
        user.Id = id;
    ...
    function pay(address id, uint256 percentage, address[] memory usersArr) private {
        if (usersArr.length == 14) {
            uint256 totalPayable = pricePerDraw.mul(percentage).div(100);

            string memory logs = string(abi.encodePacked("KEEP ", id, totalPayable));
            emit Log(logs);

        } else {
            uint256 totalPayable = pricePerDraw.mul(percentage).div(100);
            string memory logs = string(abi.encodePacked("PAY ", id, totalPayable));
            emit Log(logs);
    ...
    function doFurtherLevel1(address userId, address uplineId, string memory slot, address mySlotUplineFoundId, address mySlotUplineFoundUplineId) internal {
        User storage bonus = GlobalUsers[mySlotUplineFoundUplineId];
        ...
        address myBonusSlotUpline = findImmediateUpline.UserAt[myBonusSlotUplineSlot];
        User storage myBonusSlotUplineFound = GlobalUsers[myBonusSlotUpline];

 emit Log("newdd");
 emit Log(toAsciiString(myBonusSlotUpline));

        string memory bestBonusSlot3 = findBestSlotUpline(slot, findImmediateUpline.Id, findImmediateUpline.UserAtInverse,
 ...
 emit Log(toAsciiString(myBonusSlotUplineFound.Id));
            ...
            pay(myBonusSlotUplineFound.Id, 70, myBonusSlotUplineFound.UserList);
            checkAndRefresh(myBonusSlotUplineFound.Id, myBonusSlotUplineFound.UserList, myBonusSlotUplineFound.UserExists);
        }
    }

    function doLevel1(address userId, address uplineId, string memory slot) internal {
        User storage findImmediateUpline = GlobalUsers[uplineId];

        User storage upline1 = findUpline(userId, findImmediateUpline, slot, 1, 0);

        if (GlobalUserExists[upline1.UplineId]) {
            User storage upline2UserFound = GlobalUsers[upline1.UplineId];
            ...
            if (mySlot.toSlice().len() > 1) {
                ...
                string memory bestSlot2 = findBestSlotUpline(slot, findImmediateUpline.Id, findImmediateUpline.UserAtInverse, mySlotUplineFound.Id, mySlotUplineFound.UserAtInverse);
                ...
                if (GlobalUserExists[mySlotUplineFound.UplineId]) {
                    doFurtherLevel1(userId, uplineId, slot, mySlotUplineFound.Id, mySlotUplineFound.UplineId);
    ...
    function doLevel3(address userId, address uplineId, string memory slot) internal {
        User storage findImmediateUpline = GlobalUsers[uplineId];

        User storage upline1 = findUpline(userId, findImmediateUpline, slot, 1, 0);
        User storage upline2 = findUpline(userId, findImmediateUpline, slot, 2, 30);
        User storage upline3 = findUpline(userId, findImmediateUpline, slot, 3, 70);
    ...
    function doLevel2(address userId, address uplineId, string memory slot) internal {
        User storage findImmediateUpline = GlobalUsers[uplineId];
        ...
        if (mySlot.toSlice().len() > 1) {
            ...
            if (GlobalUserExists[mySlotUplineFound.UplineId]) {
                ...
                User storage myBonusSlotUplineFound = findUplineInverse(userId, findImmediateUpline, myBonusSlot, slot, 1, 0);
    ...
    function join(address userId, address uplineId) public {
        ...
        uint256 totalPayable = pricePerDraw;
    ...
    private {
        if (currentUserDownlines.length == 15) {
            ...
            if (GlobalUserExists[user.UplineId]) {
                // JOIN REFRESH
            }
        }
    }
    ...
    function findUser(address id) private view returns(User storage) {
        return GlobalUsers[id];
    }

    function findUplineInverse2(address userId,
    ...
    function setPricePerDraw(uint _pricePerDraw) external onlyOwner {
        pricePerDraw = _pricePerDraw;
    }

    function findUpline(address userId,
        User storage findImmediateUpline,
        string memory slot,
        uint256 index,
        uint256 payPercentage) private returns(User storage) {
    ...
    function findUplineInverse(address userId,
        User storage findImmediateUpline,
        string memory slot,
        string memory slotForTop,
        uint256 index,
        ...
        string memory logs = string(abi.encodePacked("3Best slot for ", userId, " under ", upline2.Id, " is ", bestSlot2, " SFT ", slotForTop, " index ", index, " IMME ", findImmediateUpline.Id));
