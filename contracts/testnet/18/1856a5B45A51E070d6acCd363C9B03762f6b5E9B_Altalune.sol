...
contract Altalune is Ownable {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 5_000_000 * 10 ** DECIMALS;
    uint private constant MAX_SUPPLY = INITIAL_FRAGMENTS_SUPPLY * 1000;
    ...
    uint16 public totalFee = uint16(liquidityFee.add(treasuryFee).add(altaluneInsuranceFundFee).add(burnFee));

    uint16 public dailySellLimit = 5; // 0.5%
    ...
    uint constant _rebaseInterval = 20 minutes;
    ...
    constructor() {
        ...
        _approve(treasuryReceiver, pair, ~uint(0));
        _approve(autoLiquidityReceiver, pair, ~uint(0));
        _approve(treasuryReceiver, address(router), ~uint(0));
    ...
    function getRebaseRate() internal view returns (uint) {
        uint deltaTimeFromInit = block.timestamp - _initRebaseStartTime;

        if (deltaTimeFromInit >= (7 * 365 days)) { // 7 years
            return 26;
        }
        if (deltaTimeFromInit >= ((15 * 365 days) / 10)) { // 1.5 years
            return 182;
        }
        if (deltaTimeFromInit < (365 days)) { // less than 1 year
            return 30716;
        }
        return 2752; // between 1 and 1.5 years
    }
    ...
    function transferFrom(address from, address to, uint256 value) external validRecipient(to) returns (bool) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (marketplaces[recipient] && msg.sender != address(router)) {
            uint maxTransferAllowed = dailyMaxSell();
            require(amount <= maxTransferAllowed, "Transfer exceeds max transfer");
            SaleInterval storage sales = _sales[msg.sender];
            uint soldInLast24Hours = soldIn24Hours(sender);
            require(soldInLast24Hours + amount <= maxTransferAllowed, "Sell limit reached");
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount) internal returns (uint256) {
        ...
        _gonBalances[altaluneInsuranceFundReceiver] = _gonBalances[altaluneInsuranceFundReceiver].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(altaluneInsuranceFundFee))
        );
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require(amountToSwap > 0, "There is no Altalune token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _treasuryReceiver, address _altaluneInsuranceFundReceiver) external onlyOwner {
    ...
    function getTreasuryFundValue() external view returns (uint) {
        return balanceOf(treasuryReceiver);
    }

    function getInsuranceFundValue() external view returns (uint) {
    ...
    function dailyMaxSell() public view returns (uint){
    ...
    function soldIn24Hours(address addr) public view returns (uint) {
        SaleInterval memory sales = _sales[addr];
        uint soldInLast24Hours;
        if (block.timestamp - sales.timestamp < 24 hours) {
            for (int i = int(sales.sales.length); i -- > 0;) {// that operator is cursed.
                if (block.timestamp - sales.sales[uint(i)].timestamp > 24 hours)
                    break;
                soldInLast24Hours += sales.sales[uint(i)].amount;
            }
        }
    ...
    function checkUpkeep(bytes calldata checkData) external view returns (bool upkeepNeeded, bytes memory performData) {
        checkData; // to stop variable-not-used compiler warning
        return (shouldRebase(), bytes(""));
    }

    function performUpkeep(bytes calldata performData) external {
        performData; // to stop variable-not-used compiler warning
        rebase();
    }

    function getRebaseInterval() external pure returns (uint) {
