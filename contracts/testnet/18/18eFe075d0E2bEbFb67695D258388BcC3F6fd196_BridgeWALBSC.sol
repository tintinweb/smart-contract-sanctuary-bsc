...
contract BridgeWALBSC is AccessControl, ReentrancyGuard, Ownable {
    ...
    constructor(IERC20 _acceptedToken, uint256 _swapFee) {
    ...
    function withdraw() external onlyRole(CONTROLLER_ROLE) {
        uint256 balance = acceptedToken.balanceOf(address(this));
        require(balance > 0, "Bridge: not eligible");
        acceptedToken.transfer(owner(), balance);

        emit Withdraw(owner(), balance);
    }

    function withdrawAmount(uint256 amount) external onlyRole(CONTROLLER_ROLE) {
        uint256 balance = acceptedToken.balanceOf(address(this));
        require(amount <= balance, "Bridge: sufficient");
