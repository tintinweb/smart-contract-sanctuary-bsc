...
library Constants {
    /* Chain */
    uint256 private constant CHAIN_ID = 97; // Cronos Mainnet
    ...
    uint256 private constant EPOCH_START = 1651658783; // Tuesday, 8 February 2022 00:00:00 (GMT+0)
    uint256 private constant EPOCH_PERIOD = 120; // 10 minutes
    ...
    /* Deployed */
    address private constant DAO_ADDRESS = address(0x54b4Fdcca8F197BE4741536eA0dca0Ac6E5Df96f);
    address private constant DOLLAR_ADDRESS = address(0xEf9dD8a9Cf1EaD4fDC90311E59e3d2b185bdb1df);
    address private constant PAIR_ADDRESS = address(0xd270A31D0BC52A62ef2c0023F812f026AeF7C3DF);
    ...
    address private constant DIAMOND_ROOM_ADDRESS = address(0xE0607497c374Fb506bA24b4Fc75927604225B912);
    address private constant FAIR_ROOM_ADDRESS = address(0x7FA153f9580663c5DAC36AdE36A356b8592ed26b);
    ...
    function getUniswapFactoryAddress() internal pure returns (address) {
...
contract IOracle {
    ...
    function twap() external view returns (Decimal.D256 memory);
...
interface IFairRoom {
    ...
    function updateRewardRate(uint256 _newRate, uint256 _runningTime) external;
}
...
contract Comptroller is Setters {
    ...
    function increaseSupply(uint256 newSupply) internal returns (uint256, uint256) {
        ...
        // 0-f. Pay out to Fair Room
        uint256 fairRoomReward = newSupply.mul(Constants.getFairRoomRatio()).div(10000);
        allocateToFairRoom(fairRoomReward);

        uint256 rewards = poolReward.add(treasuryReward).add(daoFundReward).add(safeReward);
        rewards = rewards.add(diamondRoomReward).add(fairRoomReward);
    ...
    function allocateToDiamondRoom(uint256 amount) private {
        if (amount > 0) {
            address _droom = Constants.getDiamondRoomAddress();
    ...
    function allocateToFairRoom(uint256 amount) private {
        if (amount > 0) {
            address _froom = Constants.getFairRoomAddress();
            Constants.EpochStrategy memory strategy = Constants.getEpochStrategy();
            uint256 _period = strategy.period;
            dollar().mint(_froom, amount);
            IFairRoom(_froom).updateRewardRate(amount.div(_period), _period);
        }
    }
...
contract Regulator is Comptroller {
    ...
    function oraclePrice() public view returns (Decimal.D256 memory) {
        uint256 _epoch = epoch();
        if (_epoch == 0 || bootstrappingAt(_epoch.sub(1))) {
            return Constants.getBootstrappingPrice();
        }
        return oracle().twap();
    }

    function nextGrowSupplyAmount() external view returns (uint256) {
        Decimal.D256 memory price = oraclePrice();
        if (price.greaterThan(Decimal.one())) {
            Decimal.D256 memory delta = limit(price.sub(Decimal.one()), price);
            return delta.mul(totalNet()).asUint256();
        }
        return 0;
    }
}
