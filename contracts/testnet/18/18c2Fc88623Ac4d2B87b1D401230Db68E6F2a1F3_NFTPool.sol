...
abstract contract User {
    ...
    function getIsNode(address account) public view returns (bool) {
        return userInfos[account].isNode;
    }
...
abstract contract NFTReward is Ownable, User, Initializable {
    using SafeMath for uint256;
    ...
    function isRewardToken(address token) public view returns(bool){
        return address(poolInfos[token].rewardToken) == address(0);
    }
...
contract NFTPool is NFTReward {
    using SafeMath for uint256;

    mapping(address => uint256[]) public nftCard;
    ...
    function createTokenLockHandle() public onlyOwner {
        ...
        tokenLock = TokenLock(poolAddress);
        tokenLock.initialize(address(token));
    }
    ...
    function upgradeNode(uint256 tokenId) external {
        ...
        _upgradeNode();
        uint256 tokenAmount = (uint64(tokenId >> 112)) * 10**18;
        token.transferFrom(hotWallet, address(tokenLock), tokenAmount);
        tokenLock.locking(msg.sender, tokenAmount);

        nftCard[msg.sender].push(tokenId);
        super.stake(msg.sender, 10**18);
    }
