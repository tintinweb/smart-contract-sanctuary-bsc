...
contract LuckyBlock is Context, IERC20, Ownable {
    ...
    address public distributionWallet =
        0xE605c32F883EAae4b5777989a673430Ecc87c41E;
    address public nftRoyalPoolWallet =
        0xdCb9780E81AbDb5598350D70acDA89CE7BaAFC9E;
    address public lotteryPoolWallet =
        0x1C68DFB90D6F2acb05b7b34dF191e9C6B38E9Cb1;
    ...
    function collectFee(
        ...
        if (lotteryPoolFee != 0 && to == pancakeSwapV2Pair) {
            uint256 _lotteryPoolFee = amount.mul(lotteryPoolFee).div(
                10**(feeDecimal + 2)
            );
            transferAmount = transferAmount.sub(_lotteryPoolFee);
            _reflectionBalance[lotteryPoolWallet] = _reflectionBalance[
                lotteryPoolWallet
            ].add(_lotteryPoolFee.mul(rate));
            if (_isExcluded[lotteryPoolWallet]) {
                _tokenBalance[lotteryPoolWallet] = _tokenBalance[
                    lotteryPoolWallet
                ].add(_lotteryPoolFee);
            }
        ...
        if (nftRoyalPoolFee != 0 && to == pancakeSwapV2Pair) {
            uint256 _nftRoyalPoolFee = amount.mul(nftRoyalPoolFee).div(
                10**(feeDecimal + 2)
            );
            transferAmount = transferAmount.sub(_nftRoyalPoolFee);
            _reflectionBalance[nftRoyalPoolWallet] = _reflectionBalance[
                nftRoyalPoolWallet
            ].add(_nftRoyalPoolFee.mul(rate));
            if (_isExcluded[nftRoyalPoolWallet]) {
                _tokenBalance[nftRoyalPoolWallet] = _tokenBalance[
                    nftRoyalPoolWallet
                ].add(_nftRoyalPoolFee);
            }
