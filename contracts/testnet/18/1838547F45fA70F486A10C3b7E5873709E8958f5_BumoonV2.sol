...
contract BumoonV2 is IERC20, Whitelist {
    ...
    //EVENTS
    event CrossIn(address indexed to, uint256 amount, uint256 fromChainId);
    event CrossOut(address indexed from, uint256 amount, uint256 toChainId);
    ...
    uint256 public crossChainFees = 0.005 ether;
    ...
    constructor() Whitelist("BUMooN", "1") {
        _approve(_msgSender(), address(this), 1e36);
    ...
    function isStaked(address account) external view returns (bool) {
        return addrdata[account].isStaked;
    }
    ...
    function setCrossChainFees(uint256 price) external onlyOwner {
        crossChainFees = price;
    }
    ...
    function seeOutput(address account)
        public
        view
        returns (uint256[2] memory)
    {
        uint256 balanceAmount = v1contract.balanceOf(account);
        uint256 balanceOut = ((balanceAmount * 1e18) / (1e6 * 1e9));
        return [balanceOut, balanceAmount];
    }
    ...
    ) external isSigned(amount, _nonce, _signature) onlyEOA {
        require(isCrossAvailable, "Cross chain is not available");
        require(amount > 0, "Amount must be greater than 0");
        _mint(_msgSender(), amount);
        emit CrossIn(_msgSender(), amount, fromChainId);
    }
    ...
    function crossOut(
        ...
        uint256 _nonce,
        bytes memory _signature
    ) external payable isSigned(amount, _nonce, _signature) onlyEOA {
        require(
            msg.value >= crossChainFees,
            "Value must be equal or greater than cross chain minimum fee"
        );
        require(isCrossAvailable, "Cross chain is not available");
        require(
        ...
        emit CrossOut(_msgSender(), amount, toChainId);
    }
}
