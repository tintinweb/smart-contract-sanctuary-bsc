...
contract Marketplace is ReentrancyGuard, Ownable, Pausable {
    ...
    mapping(address => mapping(address => uint256)) public pendingRevenue; // For creator/treasury to claim

    // Address(0) is mean allow for pay by ETH or not
    mapping(address => bool) public allowUnits;
    ...
    event Buy(uint256 indexed marketItemId, address buyer);
    // Collection is closed for trading and new listings
    ...
    constructor() {
        // default allow pay by ETH
        allowUnits[address(0)] = true;
    }
     ...
     */
    function claimPendingRevenue(address unitAddress) external nonReentrant {
        uint256 revenueToClaim = pendingRevenue[msg.sender][unitAddress];
        require(revenueToClaim != 0, "Claim: Nothing to claim");
        pendingRevenue[msg.sender][unitAddress] = 0;
        
        _transferByUnit(msg.sender, revenueToClaim, unitAddress);

        emit RevenueClaim(msg.sender, revenueToClaim);
    }

    function setAllowUnit(address unitAddress, bool isAllow) public onlyOwner {
        require(
            IERC20(unitAddress).totalSupply() > 0 && unitAddress != address(0),
            "Invalid unit address"
        );

        if (allowUnits[unitAddress] != isAllow) {
            allowUnits[unitAddress] = isAllow;
        }
    }

    /**
     * @dev Listing an item, requiring a listing fee and transfering the NFT token from
    ...
    function createOrder(
        ...
        require(
            _collections[nftContractAddress].status == CollectionStatus.Open,
        ...
        require(allowUnits[unitAddress], "Invalid pay unit");
     ...
     */
    function cancelOrder(uint256 marketItemId)
        external
        nonReentrant
        whenNotPaused
    {
        uint256 tokenId = marketItemIdToMarketItem[marketItemId].tokenId;
        address nftContractAddress = marketItemIdToMarketItem[marketItemId]
            .nftContractAddress;
     ...
     */
    function buyWithNative(uint256 marketItemId)
        external
        payable
        nonReentrant
        whenNotPaused
    {
        _buy(marketItemId, msg.value);
    }
     ...
     * marketplace to the msg.sender
     */
    function buyWithToken(uint256 marketItemId, uint256 amount)
        external
        nonReentrant
        whenNotPaused
    {
        _buy(marketItemId, amount);
    }

    /**
     * @notice Add a new collection
     * @param newCollection: collection address
     * @param newCreator: creator address (must be 0x00 if none)
     * @param newTradingFee: trading fee (100 = 1%, 500 = 5%, 5 = 0.05%)
     * @param newCreatorFee: creator fee (100 = 1%, 500 = 5%, 5 = 0.05%, 0 if creator is 0x00)
    ...
    function addCollection(
        ...
        require(
            !_collectionAddressSet.contains(newCollection),
            "Operations: Collection already listed"
        );
        require(
            IERC721(newCollection).supportsInterface(0x80ac58cd),
            "Operations: Not ERC721"
        );

        require(
            (newCreatorFee == 0 && newCreator == address(0)) ||
                (newCreatorFee != 0 && newCreator != address(0)),
            "Operations: Creator parameters incorrect"
        );

        require(
            newTradingFee + newCreatorFee <= TOTAL_MAX_FEE,
            "Operations: Sum of fee must inferior to TOTAL_MAX_FEE"
        );

        _collectionAddressSet.add(newCollection);

        _collections[newCollection] = Collection({
     ...
     * @dev Callable by admin
     */
    function closeCollectionForTradingAndListing(address newCollection)
        external
        onlyOwner
    {
        require(
        ...
        _collections[newCollection].status = CollectionStatus.Close;
        _collectionAddressSet.remove(newCollection);

        emit CollectionClose(newCollection);
    }
    ...
    /**
     * @dev Fetch an market item by id
     */
    function fetchAvailableMarketItemById(uint256 marketItemId)
        external
        view
    ...
    {
        tradingFee = (price * _collections[collection].tradingFee) / 10000;
        creatorFee = (price * _collections[collection].creatorFee) / 10000;
    ...
    /**
     * @dev Function buy internal
     */
    function _buy(uint256 marketItemId, uint256 amount) internal {
        address nftContractAddress = marketItemIdToMarketItem[marketItemId]
            .nftContractAddress;
        require(
            _collections[nftContractAddress].status == CollectionStatus.Open,
        ...
        require(msg.sender != seller, "Buy: Buyer cannot be seller");
        ...
        (
            ...
            uint256 tradingFee,
            uint256 creatorFee
        ) = _calculatePriceAndFeesForCollection(nftContractAddress, price);

        marketItemIdToMarketItem[marketItemId].owner = payable(msg.sender);
        marketItemIdToMarketItem[marketItemId].sold = true;

        address unitAddress = marketItemIdToMarketItem[marketItemId].unitAddress;
        IERC20(unitAddress).safeTransferFrom(address(msg.sender), address(this), price);

        // Update pending revenues for treasury/creator (if any!)
        if (creatorFee != 0) {
            pendingRevenue[
                _collections[nftContractAddress].creatorAddress
            ][unitAddress] += creatorFee;
        }
        
        // Update trading fee if not equal to 0
        if (tradingFee != 0) {
            pendingRevenue[treasuryAddress][unitAddress] += tradingFee;
        }
        // Transfer to seller
        _transferByUnit(seller, netPrice, unitAddress);

        IERC721(nftContractAddress).transferFrom(seller, msg.sender, tokenId);

        _tokensSold.increment();
        emit Buy(marketItemId, msg.sender);
    }

    /**
     * @dev Function transfer
     */
    function _transferByUnit(address toAddress, uint256 amount, address unitAddress) internal returns(bool){
        if(unitAddress == address(0)) {
            payable(address(toAddress)).transfer(amount);
        }
        if(unitAddress != address(0)) {
            IERC20(unitAddress).safeTransfer(address(toAddress), amount);
        }
        return true;
    }
}
