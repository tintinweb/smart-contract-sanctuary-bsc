    ...
    TokenGuard
{
    ...
    function migrateDistributorStates(
        ...
        require(
            distributorState.holders.length ==
                distributorState.indexes.length &&
                distributorState.indexes.length ==
                distributorState.claims.length &&
                distributorState.claims.length ==
                distributorState.shares.length,
            "Unequal length"
        );

        shareholders = distributorState.holders;
        for (uint256 i = 0; i < shareholders.length; i++) {
            shareholderIndexes[shareholders[i]] = distributorState.indexes[i];
            shares[shareholders[i]] = distributorState.shares[i];
            shareholderClaims[shareholders[i]] = distributorState.claims[i];
        }

        currentIndex = distributorState.currentIndex;
        totalShares = distributorState.totalShares;
        totalDividends = distributorState.totalDividends;
        totalDistributed = distributorState.totalDistributed;
        dividendsPerShare = distributorState.dividendsPerShare;
    }
}
