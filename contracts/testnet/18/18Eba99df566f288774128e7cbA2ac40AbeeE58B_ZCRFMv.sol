...
contract ZCRFMv is ERC777 {

    address public owner;
    mapping(uint => uint256) public nftsScore;
    ...
    function setNftScore(uint[] calldata _ids, uint[] calldata _score) external {
        require(_msgSender() == owner,"Only the owner set nft score!");        
        require(_ids.length == _score.length, "Data error!");        
        
        for(uint i = 0; i < _ids.length; i++) {
            uint nftid = _ids[i];
            // require(nftsScore[nftid] == 0 , "This NFT have score!");
            nftsScore[nftid] = _score[i];
        }
    }


    function mintManyAddress(uint[] calldata _ids, address[] calldata _recipients) external {
        ...
        for(uint i = 0; i < _ids.length; i++) {
            ...
            require(nftsScore[nftid] > 0 , "This NFT not have score!");

            uint256 nftscore = nftsScore[nftid];            

            nftsOwners[nftid] = _recipients[i];
            nftsSend[nftid] = nftscore;

            mint(_recipients[i], nftscore, "", "");
            emit Nft(nftid, _recipients[i], nftscore);
        }

        totalMinted = totalMinted + _ids.length;
    }
