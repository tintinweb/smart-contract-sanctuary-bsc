...
contract GNSPriceAggregatorV6_2 is ChainlinkClient {
    ...
    event Llegada(string s);
    event Llegada2(string s, uint u);
    event Llegada3(string s, bool u);
    ...
    function fulfill(
        ...
        emit Llegada3("Entro al fulfill Y ANTES DE NADA COMPRUEBO SI YA HE INICIADO: %s", r.initiated);
        if(!r.initiated){
            emit Llegada("HAGO RETURN");
            return;
        }

        uint[] storage answers = ordersAnswers[orderId];
        uint feedPrice;
        emit Llegada2("Entro al fulfill numero: %s", answers.length);
