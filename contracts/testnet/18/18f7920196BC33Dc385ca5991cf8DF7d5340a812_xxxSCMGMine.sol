...
contract xxxSCMGMine {
    ...
    function sellEggs() public {
        ...
        IERC20(mined).transfer(ceoAddress, fee);
        IERC20(mined).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    function buyEggs(address ref, uint256 amount) public {
        require(initialized);
    
        IERC20(mined).transferFrom(address(msg.sender), address(this), amount);
        
        uint256 balance = IERC20(mined).balanceOf(address(this));
    ...
    function calculateEggSell(uint256 eggs) public returns(uint256) {
        return calculateTrade(eggs,marketEggs,IERC20(mined).balanceOf(address(this)));
    }
    function calculateEggBuy(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketEggs);
    }
    function calculateEggBuySimple(uint256 eth) public returns(uint256){
        return calculateEggBuy(eth,IERC20(mined).balanceOf(address(this)));
    }
    ...
    function getBalance() public returns(uint256) {
        return IERC20(mined).balanceOf(address(this));
    }
