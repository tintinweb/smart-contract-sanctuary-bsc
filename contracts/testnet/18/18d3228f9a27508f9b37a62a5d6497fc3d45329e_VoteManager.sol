...
contract VoteManager {
    ...
    function createNewProposal(string calldata exchange, address withdrawer, uint32 epochStart, uint32 epochEnd, uint256 amountTokens) external onlyOwner {
        ...
        // Proposal _contract = new Proposal(exchange, currentToken, epochStart, epochEnd, amountTokens, withdrawer);
        ...
        try currentToken.transferFrom(msg.sender, voteAddy, amount) {} catch {
    ...
    function withdraw(uint256 proposalAtIndex) external {
        ...
        Proposal _contract = Proposal(proposalArray[proposalAtIndex].contractAddress);
        address account = msg.sender;
        uint256 votingPower = getVotingPower(proposalAtIndex, account);
        uint256 nowStamp = block.timestamp;
