...
ReentrancyGuardUpgradeable
{
    ...
    function depositwBNB(uint256 amount)
    external
    override
    nonReentrant
    returns (uint256 value)
    {
        uint256 balance = IWETH(_wBnbAddress).balanceOf(msg.sender);
        console.log("here");
        console.log(balance);
        console.log("here");
        ...
        console.log("here");
        console.log(amount);
        IWETH(_wBnbAddress).transferFrom(msg.sender, address(this), amount);
        IWETH(_wBnbAddress).withdraw(amount);
        return _deposit(amount);
    }

    function _deposit(uint256 amount) private returns(uint256 value) {
    ...
    function getAmountOut(address tokenIn, address tokenOut, uint256 amountIn) public view returns (uint256 amountOut) {
        ...
        uint256[] memory outAmounts = _dex.getAmountsOut(amountIn, path);
