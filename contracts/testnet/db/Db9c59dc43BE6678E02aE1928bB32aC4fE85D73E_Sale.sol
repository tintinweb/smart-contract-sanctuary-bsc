...
contract Sale is ISale, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    function creates(address _caller, bool _isCreateNewToken, bool _isSetRoyalty, ProjectInfo memory _projectInfo, SaleInput[] memory _saleInputs) external nonReentrant onlyProject returns (uint256) {
        ...
        for (uint256 i = 0; i < _saleInputs.length; i++) {
            lastId.increment();
            currentSalesInPack[_projectInfo.id].push(lastId.current());
    ...
    function getCurrentDutchPrice(uint256 _saleId) public view returns (uint256) {
        if (_saleId == 0 || _saleId > lastId.current()) return 0;
    ...
    function buy(uint256 _saleId, bytes32[] memory _merkleProof, uint256 _amount) external payable nonReentrant projectIsValid(_saleId, false) {
        require(_saleId > 0 && _saleId <= lastId.current(), "Invalid sale");
        ...
        if (_project.isInstantPayment) {
            if (sale.isSoldOut) _closeSale(_saleId);
            if (soldAmount == _project.amount) project.end(_project.id);
    ...
    function buyPack(uint256 _projectId, bytes32[] memory _merkleProof, uint256 _amount) external payable nonReentrant projectIsValid(_projectId, true) {
        ...
        require(_amount <= currentSalesInPack[_projectId].length, "Invalid amount");
        ...
        uint256 sentByIndex = currentSalesInPack[_projectId].length - 1;
        uint256 totalPrice;

        for (uint256 i = 0; i < _amount; i++) {
            SaleInfo storage saleInfo = sales[currentSalesInPack[_projectId][sentByIndex - i]];
            currentSalesInPack[_projectId].pop();
            
            uint256 _price = projectInfo.isFixed ? saleInfo.fixedPrice : getCurrentDutchPrice(saleInfo.id);
            totalPrice += _price;
            saleInfo.amount = 0;
            ...
            _sharing(projectInfo, saleInfo, _amount, _price);
        }

        //slither-disable-next-line incorrect-equality
        require(projectInfo.isFixed ? msg.value == totalPrice : msg.value >= totalPrice, "Invalid value");

        uint256 soldAmount = projectInfo.sold + _amount;
        project.setSoldQuantityToProject(_projectId, soldAmount);
        if (projectInfo.isInstantPayment && soldAmount == projectInfo.amount) project.end(_projectId);
    }
