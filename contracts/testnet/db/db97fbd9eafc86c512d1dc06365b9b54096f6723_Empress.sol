     ...
     function _transfer(address from,address to,uint256 amount) internal override {
        ...
        if(takeFee){
            ...
            if(automatedMarketMakerPairs[from]){
                LFee = amount.mul(buyFee).div(100);
                AmountLiquidityFee += LFee;
            }

            if(automatedMarketMakerPairs[to]){
                LFee = amount.mul(sellFee).div(100);
                AmountLiquidityFee += LFee;
            }

            uint256 BurnWallet = amount.mul(buyFee).div(100);
            uint256 trueAmount = amount.sub(BurnWallet); 

            super._transfer(from,deadwallet,BurnWallet);
