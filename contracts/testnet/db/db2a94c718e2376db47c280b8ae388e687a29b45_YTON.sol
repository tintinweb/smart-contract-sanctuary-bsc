...
contract YTON is Ownable{
    ...
    IERC20 public token; //0x047aaf694b4251a9d9d4f81919466f6c4617b342

	address public adminAddress;

    event WithdrawnToken(address indexed to, uint256 amount);
	event InvestUsdtAndHds(address indexed user, uint256 totalAAmount, uint256 totalBAmount, uint256 number, uint256 nftID);
    event InvestTicket(address indexed user,address indexed to, uint256 totalAmount);

	constructor(IERC20 _usdt,IERC20 _token,address _adminAddress) public {
        usdt = _usdt;
        token = _token;
    ...
    //IDO
	function investUsdtAndHds(uint256 tokenAAmount,uint256 tokenBAmount,uint256 number,uint256 nftID) public returns (bool) {

        usdt.transferFrom(msg.sender, address(this),tokenAAmount);
        //90%
        token.transferFrom(msg.sender,address(this),tokenBAmount);
		emit InvestUsdtAndHds(msg.sender, tokenAAmount, tokenBAmount,number,nftID);
    ...
    //ticket
	function ticket(uint256 number) public returns (bool) {
        //10U
        usdt.transferFrom(msg.sender, address(this),10000000000000000000);
		
        emit InvestTicket(msg.sender,address(this), number);
		return true;
	}
