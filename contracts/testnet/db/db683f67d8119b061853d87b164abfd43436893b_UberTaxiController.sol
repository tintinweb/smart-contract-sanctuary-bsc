...
contract UberTaxiDate is Ownable {
    ...
    bool public openUtaxiExchangeTaxi;
    ...
    function setLicensePrice(uint256 price) public onlyController {
        licensePrice = price;
    }

    function setInsurancePrice(uint256 price) public onlyController {
    ...
    function queryUserUtaxiDetailsByPage(address addr, uint256 pageNumber) public view returns(uint256 total, UtaxiDetail[6] memory details) {
...
contract UberTaxiBaseController is Ownable {
    ...
    constructor() {
        ...
        USDT = ERC20(0x5710EE664a8Ca6c9e8C056cDa6b8d6A60bbfD4E4);

        // UTD = UberTaxiDate(0x43108645877826cec91e314fc5f4A78dD6f8A7cb);
    }
    ...
    //     require(hold == msg.sender && UTN.ownerOf(tokenId) == msg.sender, "you are not the owner of the current vehicle");
    //      _;
    // }
    ...
    function TaxiTransfer(address from, address to, uint256 amount) public onlyController returns(bool) {
        ...
        TAXI20.transferFrom(msg.sender, to, amount);
...
contract UberTaxiController is Ownable {
    ...
    constructor() {
        ...
        UTD = UberTaxiDate(0x54fAE4A75cC239A48e5eE378A63D5AfAe4f54D5A);

        UTBC = UberTaxiBaseController(0x57e8b5d444C95F2B0E334de96bB197Fc3c8546D7);
    }
    ...
    function openBlindBox() public {
        ...
        if(types > 7){
            ...
            // TAXI20.approveToController(UTD.depositAddress(), amountOrTokenId.mul(10 ** 18));

            // TAXI20.transferFrom(UTD.depositAddress(), msg.sender, amountOrTokenId.mul(10 ** 18));
            UTBC.TaxiTransfer(UTD.depositAddress(), msg.sender, amountOrTokenId.mul(10 ** 18));
        }
    ...
    function buyVehicle(uint256 index, uint8 tokenType) public {
        ...
        (,,uint256 price,uint256 uPrice,,uint256 purchaseLimit,) = UTD.vehicles(index);

        (,address welMember,,uint256 UTAXINum,,,) = UTD.users(msg.sender);
        ...
        if (tokenType == 1) {
            ...
            require(UTAXINum >= uPrice, "You don't have enough Utaxi");

            UTD.subUserUTAXINum(msg.sender, uPrice);

            UTD.setUtaxiDetails(msg.sender, 6, UTBC.toString(ids), uPrice.div(10 ** 18), block.timestamp);
        }
        ...
        if (welMember != address(0)) {
            ...
            UTD.addUserUTAXIReward(welMember, price.mul(UTD.commissionFee()).div(100));

            UTD.addUserUTAXINum(welMember, price.mul(UTD.commissionFee()).div(100));

            UTD.setUtaxiDetails(welMember, 4, UTBC.toString(msg.sender), price.mul(UTD.commissionFee()).div(10 ** 20), block.timestamp);
        }
    ...
    function vehicleRent(uint256 index) public isUsers(index) {
        ...
        (,,uint256 vehicleState,,uint256 durability,,,,,,) = UTD.allVehicles(index);
    ...
    function _vehicleRent(uint256 index) private {
        ...
        if(UTD.openLicense() && !UTD.isBuyLicense(index)){
            ...
            UTBC.USDTTransfer(msg.sender, UTD.withDrawAddress(), UTD.licensePrice());
    ...
    function vehicleGetReward(uint256 index) public isUsers(index) {
        ...
        (uint256 types,,uint256 vehicleState,uint256 expireTime,uint256 durability,,,,uint256 profit,,) = UTD.allVehicles(index);
        ...
        require(UTD.queryUserDrawTimeIsToLimit(msg.sender), "Draw times out of limit");
        ...
        if(UTD.openInsurance() && !UTD.isBuyInsurance(index)){
            ...
            UTBC.USDTTransfer(msg.sender, UTD.withDrawAddress(), UTD.insurancePrice());
        ...
        UTD.setUtaxiDetails(msg.sender, 5, UTBC.toString(index), profit, block.timestamp);

        UTD.setVehicleDurability(index, durability - 1);

        if (welMember != address(0) && UTD.queryUserRentVehicleAmount(welMember) > 0) {

            UTD.addUserUTAXINum(msg.sender, profit.mul(UTD.recommenderFee()).div(100));

            UTD.addUserUTAXIReward(msg.sender, profit.mul(UTD.recommenderFee()).div(100));

            UTD.setUtaxiDetails(welMember, 4, UTBC.toString(msg.sender), profit.mul(UTD.recommenderFee()).div(10 ** 20), block.timestamp);
        }
    ...
    function exchange(uint256 exchangeNum) public {
        ...
        UTD.setUtaxiDetails(msg.sender, 3, UTBC.toString(msg.sender), exchangeNum, block.timestamp);

        uint256 exchangeNums = exchangeNum.mul(10 ** 18);

        require(exchangeNums <= UTD.onceExchangeLimit(), "The exchange quantity exceeds the limit");

        require(UTD.checkUserDayExchangeTimeIsToLimit(msg.sender), "The exchange times has reached the limit");

        UTD.subUserUTAXINum(msg.sender, exchangeNums);

        UTD.addUExchangeTotal(exchangeNums);

        // TAXI20.approveToController(UTD.depositAddress(), exchangeNums * (100 - UTD.serviceCharge()) / 100);
        
        // TAXI20.transferFrom(UTD.depositAddress(), msg.sender, exchangeNums * (100 - UTD.serviceCharge()) / 100);
        UTBC.TaxiTransfer(UTD.depositAddress(), msg.sender, exchangeNums * (100 - UTD.serviceCharge()) / 100);
    }

    function playerTransfer(address adds, uint256 transferNum) public {

        UTD.setUtaxiDetails(msg.sender, 2, UTBC.toString(adds), transferNum, block.timestamp);

        UTD.setUtaxiDetails(msg.sender, 1, UTBC.toString(msg.sender), transferNum, block.timestamp);

        uint256 transferNums = transferNum.mul(10 ** 18);

        UTD.subUserUTAXINum(msg.sender, transferNums);

        UTD.addUserUTAXINum(adds, transferNums * (100 - UTD.transferCharge()) / 100);
    }

    function userInvitation(address adds) public {

        require(UTD.checkBindingInput(msg.sender, adds), "The reference address entered is incorrect");

        UTD.setUserWelMember(msg.sender, adds);
    }
    ...
    function buyMemberRender(uint256 index) public {
        ...
        // TAXI20.approveToController(msg.sender, sellPrice);

        uint256 fee = sellPrice * UTD.sellFee() / 100;

        // TAXI20.transferFrom(msg.sender, UTD.deadWallet(), fee);

        // TAXI20.transferFrom(msg.sender, hold, sellPrice - fee);
        UTBC.TaxiTransfer(msg.sender, UTD.deadWallet(), fee);
        ...
        // UTN.approveToController(hold, tokenId);

        // UTN.transferFrom(hold, msg.sender, tokenId);
        ...
        if (welMember != address(0)) {
            ...
            UTD.addUserUTAXIReward(welMember, sellPrice.mul(UTD.commissionFee()).div(100));

            UTD.addUserUTAXINum(welMember, sellPrice.mul(UTD.commissionFee()).div(100));

            UTD.setUtaxiDetails(welMember, 4, UTBC.toString(msg.sender), sellPrice.mul(UTD.commissionFee()).div(10 ** 20), block.timestamp);
        }
    ...
    function vehicleTransfer(address addr, uint256 index) public isUsers(index) {
        ...
        // UTN.approveToController(msg.sender, tokenId);

        // UTN.transferFrom(msg.sender, addr, tokenId);
