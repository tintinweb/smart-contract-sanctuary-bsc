...
contract Trading is Ownable {
    uint256 private coinPrice = 0.000000001 ether;

    function setRate(uint256 _coinPrice) public onlyOwner {
    ...
    Asiannet public AsiannetAddress;

    function inititalize(Asiannet _AsiannetAddress) public onlyOwner {
        AsiannetAddress = Asiannet(_AsiannetAddress);
    }

    uint256 investerID;
    mapping(address => uint256[]) getIDByAddress;
    mapping(address => uint256) balances;
    mapping(uint256 => uint256) public priceChange;
    ...
    function Buy(uint256 amount) public payable {
        // for example 1 token 1 ether
        require(msg.value == amount * coinPrice, "Insufficent balance");
        investerID++;
        getIDByAddress[msg.sender].push(investerID);
        balances[msg.sender] += amount;
        AsiannetAddress.transferPrice(owner(), msg.sender, amount);
        priceChange[investerID] =  (amount * 1);
        coinPrice = coinPrice + (amount * 1);
    }

    function sell(uint256 amount, uint256 id) public {
        id = investerID;
        payable(msg.sender).transfer(amount * (coinPrice - priceChange[id]));
        AsiannetAddress.transferPrice(msg.sender, owner(), amount);
        coinPrice = coinPrice - (amount * 1);
        balances[msg.sender] -= amount;
