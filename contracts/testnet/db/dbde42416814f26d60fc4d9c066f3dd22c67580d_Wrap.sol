...
contract Wrap {
    ...
    function setAcp(address _acp) public {
    ...
    function withdraw() public {
        ...
        if (usdtBalance > 0) {
            IERC20(usdt).transfer(address(acp), usdtBalance);
        }
        uint256 acpBalance = IERC20(acp).balanceOf(address(this));
        if (acpBalance > 0) {
            IERC20(acp).transfer(address(acp), acpBalance);
        }
    }
