...
contract ArmedaAirDrop is OwnableUpgradeable, DivestorUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
    ...
    function _claimERC721(uint airdropId_, uint fromAddrId_, uint amount_) private {
        IATA(addrList[fromAddrId_]).mintMulti(_msgSender(), amount_);
    ...
    function _claimERC1155(uint airdropId_, uint fromAddrId_, uint cardId_, uint amouns_) private {
        IATN(addrList[fromAddrId_]).mint(_msgSender(), cardId_, amouns_);
    ...
    function airdrop(uint airdropId_, uint category_, uint fromAddrId_, uint cardId_, uint amounts_, bytes32 r_, bytes32 s_, uint8 v_) isOpen public {
        ...
        if (category_ == 2) {
            _claimERC721(airdropId_, fromAddrId_, amounts_);
            return;
        }
        if (category_ == 3) {
...
interface IATA {
    ...
    function mintWithId(address player_, uint tokenId_) external returns (bool);
    function mintMulti(address player_, uint amount_) external returns(uint[] memory);
