...
contract FundManagement  is Initializable,Ownable,IFundManagement {
   ...
   modifier onlyMerchantSalesContract() {
    ...
    function queryWithdrawFee(address addr) external view returns (uint256, uint256, uint256){
        WithdrawFee storage _withdrawFee = withdrawFee[addr];
        (uint256 _amount,,uint256 _day, uint256 _withdrawNum) = calcWithdrawFee(addr, _withdrawFee, 0);
    ...
    {   
        ...
        if (_num > 0){
            for (uint256 i = start; i < end; i++) {
                ...
                amounts[j] = _computingPowerData.computingPower[addrs[i]];
                j++;
            }
        }
