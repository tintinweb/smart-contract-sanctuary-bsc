...
contract KIZOINU is ERC20, Ownable, CustomIERC20 {
    ...
    //this is used to exclude users from swap taxes and paying for swap&liquify function gas (recommend whitelisting all connected project contracts)
    //excludes address from max wallet limit, but whitelisted address can't transfer to another un-whitelisted address if => _amountSent + _walletBalance > _maxWalletLimit
        ...
        returns (bool)
    {
        ...
        // ***must update whitelist for router change uint8
    ...
    function _transfer(
        ...
        bool whitelistedReceiver = isWhitelisted[to];

        if (!whitelistedReceiver) {
            require(
        ...
        if (selling || whitelistedLpPair[from] == true) {
            if (selling) {
                if (!isWhitelisted[from]) {
                    ...
                    uint256 taxAmount = (amount * zeroCheckedSellTax) / 100;
...
contract OptimizedStaking is ERC20, ReentrancyGuard, Ownable {
    ...
    function stake(uint128 _amount, uint32 lockTime) external nonReentrant {
        ...
        // USER SHOULD APPROVE STAKING CONTRACT BEFORE CALLING STAKE FUNCTION
        ...
        //requires approval amount of user to staking contract to be larger than "_amount"
        ...
        if (lockTime == 2419200) {
            userStakes[user].push(
                StakingObject(currentTimestamp, 2419200, true, _amount, _amount)
            );
            if (tax > 1 || tax == 0) {
    ...
    function unstake() external nonReentrant {
        uint256 userStakingInstances = userStakes[msg.sender].length;
        //goes through all existing user staking instances, claims all rewards from instances whose timelock period has expired
        for (uint256 i = 0; i < userStakingInstances; i++) {
            StakingObject memory userObject = userStakes[msg.sender][i];

            //checks if enough time has passed, extra check for user activity for gas savings
            //block timestamp (measures in seconds) was used for all time related functionalities
            if (
                userObject.active == true &&
                userObject.startTimestamp + userObject.stakingLength <=
                block.timestamp
            ) {
                uint256 userReward = uint256(userObject.stakingReward);
                uint256 userAmountStaked = uint256(userObject.amountStaked);
                ...
                userStakes[msg.sender][i] = userStakes[msg.sender][
                    userStakingInstances - 1
                ];
                delete userStakes[msg.sender][userStakingInstances - 1];

                _mint(msg.sender, userReward);
                require(IERC20(token0).transfer(msg.sender, userAmountStaked));
            }
        }
    }
