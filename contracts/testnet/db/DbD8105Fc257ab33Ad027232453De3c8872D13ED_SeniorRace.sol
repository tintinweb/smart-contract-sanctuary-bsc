...
contract SeniorRace is Caller, Fallback {
    ...
    uint256[] public test; //TODO 生产要删除

    constructor(IConfig config_) {
        raceInfo = RaceInfo(0, 0, 0, 2000, 100, 0, 1, 0, 0, 0, address(0));
        config = config_;
    }
    ...
    function _returnReward(address to, uint256 surplusCount) private {
        ...
        for (uint256 i = 0; i < pizePoolTokenSet.lengthAddressSet(); i++) {
            ...
            if (0 < amount) {
                exitAmount = amount.div(surplusCount, "1069"); //TODO 去标记数据库
                //计算税率
                //currentRound:3
                //decrease:1%=100
                //exitTax:10%=1000
