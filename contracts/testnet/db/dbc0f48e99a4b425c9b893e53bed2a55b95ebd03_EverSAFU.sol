...
/**
 * New Version of EverSAFU
 * 
 * EverSAFU Token
 * Dual Reward with BUSD Reflection & Diamond Hand Protocol
 *
...
interface ESDHTracker {
    function deposit(uint256 amount) external;
    function updateTracker(address user, uint256 balance, bool isSell) external;
    function removeFromESDH(address account) external;
}

contract EverSAFU is ERC20, Ownable {
    uint256 public buyFee  = 10;
    uint256 public sellFee = 10;

    uint256 public burnShare = 15;
    uint256 public developmentShare       = 31;
    uint256 public rewardShare    = 36;
    uint256 public buybackShare   = 6;
    uint256 public esdhShare       = 12;

    address public developmentWallet = 0xc964f4a2607B0270f845b5Da7a53aC445fe4d440;
    ...
    uint256 public bnbValueForBuyBurn = 1 * (10**18);
    uint256 public accumulatedBuybackBNB;

    bool public tradingEnabled;

    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) private _isExcludedFromESDH;
    mapping (address => bool) public automatedMarketMakerPairs;
    //0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
    //0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7 TESDHTNET
    address public BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;

    DividendTracker public dividendTracker;
    uint256 public gasForProcessing = 300000;

    ESDHTracker public esdhTracker;

    struct Dashboard {
        uint256 totalRewardDistributed;
        uint256 totalDividendHolder;
    ...
    event BuyBackAndBurn(uint256 bnbAmount);
    ...
    event AutoBurn(uint256 tokenburn);
    event SendDev(uint256 busdSend);
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
    event UpdateDividendTracker(address indexed newAddress, address indexed oldAddress);
    event GasForProcessingUpdated(uint256 indexed newValue, uint256 indexed oldValue);
    event SendDividends(uint256 amount);
    event DepositESDH(uint256 amount);
    ...
    constructor() ERC20("EverSAFU", "EverSAFU") 
    {   
        transferOwnership(0xa9f1247373B37c0736234d00852f12b5694Ba87d);

        _mint(owner(), 1e12 * (10 ** 18)); //1Trillion Supply
        swapTokensAtAmount = 4e8 * (10 ** 18); //400M to swap

    	dividendTracker = new DividendTracker(1e7, BUSD); //10 000 000 minimum dividend
        ...
        _isExcludedFromESDH[owner()] = true;
        _isExcludedFromESDH[address(this)] = true;
        _isExcludedFromESDH[address(uniswapV2Router)] = true;
        _isExcludedFromESDH[address(_uniswapV2Pair)] = true;
        _isExcludedFromESDH[DEAD] = true;
    ...
    function updateFees(uint256 _buyFee, uint256 _sellFee) external onlyOwner {
        require(_buyFee <= 15 && _sellFee <= 15, "Buy fee and sell fee cannot be more than 15%");
        buyFee = _buyFee;
        sellFee = _sellFee;
        ...
        external onlyOwner 
    {
        ...
        esdhShare       = _esdhShare;

        emit FeeSharesUpdated(developmentShare, burnShare, rewardShare, buybackShare, esdhShare);
    }
    ...
    function changeDevWallet(address _developmentWallet) external onlyOwner {
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 burnTokens;
            if(burnShare > 0) {
                burnTokens = contractTokenBalance * burnShare / 100;
                autoBurn(burnTokens);
            }

            contractTokenBalance -= burnTokens;
            uint256 bnbShare = developmentShare + rewardShare + buybackShare + esdhShare;
            ...
            if(bnbShare > 0) {
                ...
                if (developmentShare > 0) {
                    uint256 devBNB = newBalance * developmentShare / bnbShare;
                    swapAndSendDev(devBNB);
                }

                if (esdhShare > 0) {
                    uint256 esBNB = newBalance * esdhShare / bnbShare;
                    sendESDH(esBNB);
                }
        ...
        if (!_isExcludedFromESDH[from]) {
            try esdhTracker.updateTracker(from, balanceOf(from), to == uniswapV2Pair) {} catch {}
        }
        if (!_isExcludedFromESDH[to]) {
            try esdhTracker.updateTracker(to, balanceOf(to), false) {} catch {}
        }
    }
    ...
    function autoBurn(uint256 tokens) private {
        super._burn(address(this), tokens);
        emit AutoBurn(tokens);
    }
    ...
    function swapAndSendDividends(uint256 amount) private{
        ...
        uint256 balanceRewardToken = IERC20(BUSD).balanceOf(address(this));
        bool success = IERC20(BUSD).transfer(address(dividendTracker), balanceRewardToken);
    ...
    function sendESDH(uint256 amount) private{
        (bool success, ) = payable(address(esdhTracker)).call{value: amount}("");

        if (success) {
            esdhTracker.deposit(amount);
        }
    }

    function swapAndSendDev(uint256 amount) private{
        ...
        uint256 balanceAfter = IERC20(BUSD).balanceOf(address(this));
        uint256 _amount = balanceAfter - balanceBefore;

        IERC20(BUSD).transfer(address(developmentWallet), _amount);

        emit SendDev(_amount);
    }
    ...
    //=======ESDH Tracker // EverSAFU Diamond Hand=======//
    function isExcludedFromESDH(address account) public view returns(bool) {
        return _isExcludedFromESDH[account];
    }

    function excludeFromESDH(address account, bool value) public onlyOwner {
        _isExcludedFromESDH[account] = value;
        esdhTracker.removeFromESDH(account);
    }

    function setEsdhTracker(address newEsdhTracker) external onlyOwner {
        esdhTracker = ESDHTracker(newEsdhTracker);
    }
    ...
    function getDashboard(address user) external view returns (Dashboard memory) {
        ...
        dashboard.totalDividendHolder = dividendTracker.getNumberOfTokenHolders();
