...
contract SellBox is Ownable {
    ...
    constructor(IERC1155 _box, IERC20 _tokenBuy, address _ceo, uint[] memory _tokenIds, uint[] memory _prices) {
        box = _box;
        tokenBuy = _tokenBuy;
        ceo = _ceo;
    ...
    function buy(uint _tokenId, uint _amount) external {
        ...
        uint comm = totalBill * 25 / 100;
        tokenBuy.transferFrom(_msgSender(), ceo, totalBill - comm);
        tokenBuy.transferFrom(_msgSender(), address(commTreasury), totalBill - comm);
        box.safeTransferFrom(ceo, _msgSender(), _tokenId, _amount, '0x');
        remains[_tokenId] -= _amount;

        commTreasury.handleComm(_msgSender(), _amount);
    }
