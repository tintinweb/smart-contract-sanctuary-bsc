...
contract LeslarMetaverseCollection is ERC721A, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721A, AccessControlUpgradeable) returns (bool) {
  ...
  function _setMintedTokenURI(uint256 _mintAmount) internal{
    ...
    do{
      if(!firstTokenHasMinted && updatedIndex == 1){
        if(!revealed){
          _setTokenURI(updatedIndex, hiddenMetadataUri);
        }else{
          string memory currentBaseURI = _baseURI();
          uint256 tokenId = updatedIndex;
          _setTokenURI(updatedIndex++, string(abi.encodePacked(currentBaseURI,tokenId.toString(), uriSuffix)));
        }
 ...
 */
contract ERC721A is ContextUpgradeable, ERC165Upgradeable, IERC721Upgradeable, IERC721MetadataUpgradeable {
    using AddressUpgradeable for address;
    using StringsUpgradeable for uint256;
    ...
    function _ERC721A_init(string memory name_, string memory symbol_) internal {
        _name = name_;
        _symbol = symbol_;
