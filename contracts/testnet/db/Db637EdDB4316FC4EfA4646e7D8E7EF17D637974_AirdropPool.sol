...
contract AirdropPool is Ownable {
    ...
    struct UserInfo {
        // How many `DefinnPay Tokens` the user has Airdroped.
        uint256 amount;       
        // Airdrop get date.
        uint256 getDate;      
    }
    ...
    /// @dev The token to be Airdroped.
    IERC20 public airdropToken;   
    RewardTreasury public rewardTreasury;
   
    /// @dev The total amount of tokens airdroped.
    ...
    /// @dev Maximum of total token can be stakes.
    uint256 public maximumAirdrop;

   
    /// @dev Info of each user that get airdrop tokens.
    mapping (address => UserInfo) public usersInfo;
    mapping (address => AirdropList) public airdropsList;
   

    
    /// @dev Emitter when `user`get Airdrop tokens.
     ...
     */
    constructor(
        IERC20 airdropToken_,        
        uint256 startBlock_,
        uint256 endBlock_,
        uint256 maximumAirdrop_
        
    ) {
        airdropToken = airdropToken_;        
        startBlock = startBlock_;
        endBlock = endBlock_;
        maximumAirdrop = maximumAirdrop_;       
        rewardTreasury = new RewardTreasury(airdropToken_);
    }
    ...
    function addRewardTokens(uint256 amount) external {
        airdropToken.safeTransferFrom(msg.sender, address(rewardTreasury), amount);
    }
     ...
     */
     ...
     function setMaximumAirdrop(uint256 maximumAirdrop_) external onlyOwner {
        require(maximumAirdrop_ > 0, "Pool: Maximum Airdrop must more than 0");
    ...
    function addAirdropList(address addressX) external onlyOwner {
             AirdropList storage airdropList = airdropsList[addressX]; 
             airdropList.address0 = addressX;  
    ...
    function airdrop() external {
        ...
        AirdropList storage airdropList = airdropsList[msg.sender];
       
        
        uint256 amount = userInfo.amount;        
        require(amount == 0, "Pool: Already get Airdroped");
        require(airdropList.address0 == msg.sender, "Pool: this dddress not in airdrop list");
        require(block.number >= startBlock, "Pool: pool is not open yet");
        require(block.number < endBlock, "Pool: pool is already closed");
           
        userInfo.getDate = block.timestamp;     
        userInfo.amount += maximumAirdrop;   
       
        totalAirdropTokens += maximumAirdrop;
       
        _sendReward(msg.sender);
        emit Airdrop(msg.sender, maximumAirdrop);
    }
    ...
    function _sendReward(address user) internal {
        uint256 pendingReward = maximumAirdrop;
