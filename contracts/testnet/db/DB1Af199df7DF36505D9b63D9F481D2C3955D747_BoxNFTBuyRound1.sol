...
function buyBox(uint256 rank, uint256 _usdt) external  whenNotPaused returns (uint256) {
        ...
        if(startFlashSaleTime <=current && current <= startTime){
          ...
          require(checkBuyFlashSaleBox(msg.sender, rank)==true,'You can not buy FlashSale');
...
function checkBuyFlashSaleBox(address sender,uint256 rank) public view returns(bool){ 
  ...
  if(indexOfWhitelist[sender] != 0 && totalBoughtFlashSaleBox < 1 && rank==1){
    return true;
  }
  // StakingPool.StakingInfo[] memory data= new StakingPool.StakingInfo[](index);
  // uint tmp=0; 
