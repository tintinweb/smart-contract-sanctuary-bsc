...
contract CrossPair is ICrossPair, CrossERC20 {
    ...
    function swap(
        ...
        emit Swap(
            msg.sender,
            balance0 > uint256(_reserve0).sub(amount0Out)
                ? balance0.sub(uint256(_reserve0).sub(amount0Out))
                : 0,
            balance1 > uint256(_reserve1).sub(amount1Out)
                ? balance1.sub(uint256(_reserve1).sub(amount1Out))
                : 0,
            amount0Out,
            amount1Out,
            to
        ...
        /*************/
        uint256 amount0In = balance0 > uint256(_reserve0).sub(amount0Out)
            ? balance0.sub(uint256(_reserve0).sub(amount0Out))
            : 0;
        uint256 amount1In = balance1 > uint256(_reserve1).sub(amount1Out)
            ? balance1.sub(uint256(_reserve1).sub(amount1Out))
            : 0;
        ...
        if (userFee == 10000) {
            ...
            }
            {
                ...
                if (crossPriceCheckActive == true) {
                    uint256 currentTokenRatio = (
                        balance0.add(amount0In).mul(10**18)
                    ) / (balance1.add(amount1In));
...
contract CrossFactory is ICrossFactory {
    ...
    constructor(address _controlCenter) public {
        controlCenter = _controlCenter;
