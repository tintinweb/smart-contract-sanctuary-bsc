...
contract CPLRewardsClaim {
    ...
    mapping(address => bytes32) private claims;
    ...
    function claim(
        ...
        require(gameExpiryMap[gameId] != 0 && block.timestamp <= gameExpiryMap[gameId], "Claims expired");

        bytes32 _leaf = keccak256(abi.encode(wallet, amount, address(token)));
        require(isClaimable(root, _leaf, proof) && (claims[wallet] == keccak256(abi.encode(gameId))), "No claims available");
    ...
    function _setClaimed(address wallet, string memory gameId) private {
        claims[wallet] = keccak256(abi.encode(gameId));
    }
