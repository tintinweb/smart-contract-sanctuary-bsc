...
contract BCTGAME {
    string public constant name = "BCTGAME";
    string public constant symbol = "BCTGAME";
    ...
    uint256 public immutable bctDecimals;
    address public immutable bctAddress;
    address public immutable tokenAddress;
    address public immutable nftAddress;

    address public fundAddress = 0x0000000000000000000000000000000000000010;
    ...
    constructor(
        ...
        bctDecimals = 10 ** IBCT(bctAddress).decimals();
    ...
    function initGame() private {
        setGameConfig(1, 50, 20, 0, 0);
        setGameConfig(2, 20, 50, 20, 5);
        setGameConfig(3, 16, 55, 30, 10);
        setGameConfig(4, 14, 60, 40, 15);
        setGameConfig(5, 12, 65, 50, 20);
        setGameConfig(6, 10, 65, 60, 20);
    }
    ...
    function repair(uint256 _value) external returns (bool success) {
        ...
        require(
            durable[msg.sender] + _value * 3 <= maxDurable * bctDecimals,
            "maxDurable overflow"
        );

        uint256 balance = IBCT(bctAddress).balanceOf(msg.sender);
        ...
        if (amountOf[msg.sender] > _value) {
            ...
            IBCT(bctAddress).burn(msg.sender, sub_value);
        }
    ...
    function exchangeDurable(
        ...
        require(
            durable[_account] + _value * 3 <= maxDurable * bctDecimals,
            "maxDurable overflow"
        );

        extraBurnToken(_value);

        IBCT(bctAddress).burn(msg.sender, _value);
    ...
    function withdrawReward(uint256 _value) external returns (bool) {
        ...
        IBCT(bctAddress).mint(msg.sender, _value);
        if (taxFee > 0) {
    ...
    function getDurableLv(address account_) public view returns (uint8 Lv) {
        ...
        if (durable_ < 502 * bctDecimals) {
            Lv = 1;
        } else if (durable_ < 5000 * bctDecimals) {
            Lv = 2;
        } else if (durable_ < 10000 * bctDecimals) {
            Lv = 3;
        } else if (durable_ < 20000 * bctDecimals) {
            Lv = 4;
        } else if (durable_ < 40000 * bctDecimals) {
            Lv = 5;
        } else if (durable_ >= 40000 * bctDecimals) {
            Lv = 6;
        }
