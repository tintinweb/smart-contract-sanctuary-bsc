...
contract AustralianFarm is Ownable{
    ...
    // address NFTAddress = 0x585F785b458f4972d8b4c96C1e071df57421a5CB;
    ...
    constructor(){
        ...
        pledgeRules[1] = PledgeRule(2,50,2000,260);
    ...
    function pledgeReceive(uint256 NO) public{
        ...
        // 总收益  
        KG20.transfer(msg.sender,userPledge[msg.sender].totalRevenue);

        // 账单
        _updateBills(msg.sender,true,userPledge[msg.sender].totalRevenue,BillType.PledgeReceive);
    ...
    // 放入建筑,背包中物品类型
    function putBuliding(BackpackType backpackType,uint256 landTokenId)public{
    ...
    function putCropInBackpack(uint256 bulidingTokenId,uint256 cropNO)public{
        ...
        if(crop.isBreeding){
            ...
            for(uint256 i = 0 ; i < cropNos.length; i++){
                ...
                Crop memory breedingCrop = userCrops[cropNos[i]];

                if(breedingCrop.NO != cropNO && breedingCrop.NO != 0 && breedingCrop.isBreeding){

                    _dropCrop(bulidingTokenId,cropNos,breedingCrop.NO);
        ...
        _dropCrop(bulidingTokenId,cropNos,cropNO);
    ...
    function buyNFTs(uint256 saleIndex,uint256 amount) public {
        ...
        _updateBills(NFTGoods[saleIndex].owner,true,realAmount - saleFees,BillType.NFTSold);
    ...
    function getMyAssets(address owner) public view returns(uint256[10] memory){
        ...
        for(uint256 i = 0 ; i < 10; i++){
            ...
            if(i != uint256(BackpackType.Chick)){
                ...
                for(uint256 j = 0 ; j < userLands[landTokenId].bulidings.length; j++){
                    ...
                    if(bulidingTokenId != 0){
                        ...
                        for(uint256 k = 0 ; k < buliding.crops.length; k++){
                            ...
                            if(crop.NO != 0 && crop.cropType != CropType.Corn){
        ...
        for(uint256 i = 1 ; i <= totalSales ; i++){
            ...
            if(NFTGoods[i].surplusAmount != 0 && NFTGoods[i].owner == msg.sender){
    ...
    function _putInTheBackpack(address owner,BackpackType backpackType,bool isPut,uint256 tokenId) private returns(uint256){
        ...
        uint256 count = userBackpacks[owner][backpackType].count;
    ...
    function _checkOwnLand(uint256 landTokenId) private view returns(uint256){
        ...
        for(uint256 i = 0 ; i < userLandTokenIds[msg.sender].length ; i++){
            ...
            if(landTokenId == userLandTokenIds[msg.sender][i] && landTokenId != 0){
        ...
        // 别人的土地
        require(landIndex != 1299676513,"The land not found");
    ...
    // 删除作物
    function _dropCrop(uint256 bulidingTokenId,uint256[] memory cropNos, uint256 cropNO) private{
        ...
        BackpackType backpackType = _mainType2BackpackType(2,uint256(userCrops[cropNO].cropType));
