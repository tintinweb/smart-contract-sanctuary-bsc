...
contract QTKMIco is Owner {
    ...
    event Ico(address indexed from, uint8 order, uint256 amount);
    ...
    function setIcoOrder(
        ...
        require(_order > 0, "[TKMIco][setIcoOrder]: order must be greater than zero");
        // require(_order < 4, "[TKMIco][setIcoOrder]: order must be less than four");

        require(_startTime > block.timestamp, "[TKMIco][setIcoOrder]: _startTime is under now");
        require(_endTime > block.timestamp, "[TKMIco][setIcoOrder]: _endTime is under now");
        require(_endTime > _startTime, "[TKMIco][setIcoOrder]: _endTime is under _startTime");
...
*/
    ...
    function ico() public payable {
        // 진행중인지 체크
        require(0 < icoOrders[order].totalAmount, "[TKMIco][ico]: data is not found");
        // 코인 수량 체크
        require(1 ether <= msg.value, "[TKMIco][ico]: value should not be less than 1 ether");
        // 기간 체크
        require(icoOrders[order].startTime <= block.timestamp, "[TKMIco][ico]: ico is not started");
        require(icoOrders[order].endTime >= block.timestamp, "[TKMIco][ico]: ico is ended");
        ...
        // 남은 토큰 비교
        require(validSellToken >= validBuyToken, "[TKMIco][ico]: tokens are sold out");

        // 유저의 현재 구매 토큰으로 구매 가능한 수량 체크
        uint256 boughtToken = tokenPerUser[msg.sender][order];
        // 유저의 구매 내역 증가
        tokenPerUser[msg.sender][order] += validBuyToken;
        require(icoOrders[order].limitPerUser >= boughtToken + validBuyToken, "[TKMIco][ico]: can not buy more than allocated ammount of tokens");
        ...
        require(
            ...
            "[TKMIco][ico]: unable to send token, recipient may have reverted"
        );

        emit Ico(msg.sender, order, validBuyToken);
    }

    // 토큰 상장 후 최초로 찾아갈 수 있는 시간 설정. 한번만 가능함
    function setTokenClaimTime(uint256 _tokenClaimTime) public onlyOwner {
        require(0 == tokenClaimTime, "[TKMIco][setTokenClaimTime]: already set token claim time");
...
*/
    ...
    function icoClaim(uint8 claimOrder) public {
        ...
        require(0 < tokenClaimTime, "[TKMIco][icoClaim]: time is not setup yet");
        // order 값 체크
        ...
        // 날짜 체크
        // uint256 releaseTime = tokenClaimTime + ((releaseOrderDay * (uint256(claimOrder) - 1)) * 1 days);
        uint256 releaseTime = tokenClaimTime + ((15 * (uint256(claimOrder) - 1)) * 1 minutes);     // 15분
        require(block.timestamp > releaseTime, "[TKMIco][icoClaim]: remaining time is still left");
        // 지급 토큰 수량
        uint256 tokenAmount = lockedTokens[msg.sender];
        require(tokenAmount > 0, "[TKMIco][icoClaim]: receivable tokens are not found");
        // 지급 처리
        require(
    ...
    function withdrawEth(address to) public onlyOwner {
        require(to != address(0), "[TKMIco][withdrawEth]: transfer to the zero address");
