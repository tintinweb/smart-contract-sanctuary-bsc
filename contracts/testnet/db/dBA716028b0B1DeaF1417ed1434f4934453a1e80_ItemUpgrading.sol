...
contract ItemUpgrading is BaseUpgrading {
  ...
  function _validateEather(uint _eatherId) private view returns (uint) {
    require(itemNft.ownerOf(_eatherId) == msg.sender, "Item Upgrading: not owner of eather token");
    (uint16 eatherGene,,,uint eatherOwnPrice,) = itemNft.getItem(_eatherId);
    require(eatherGene == 0, "Item Upgrading: invalid eather item");
    bool approvedEather = itemNft.isApprovedForAll(msg.sender, address(this)) || itemNft.getApproved(_eatherId) == address(this);
    require(approvedEather, "Item Upgrading: please approve eather token first");
  ...
  function _finishUpgrade(
    ...
    if (_success) {
      for(uint i = 0; i < _materials.length; i++) {
        itemNft.transferFrom(msg.sender, address(this), _materials[i]);
        itemNft.burn(_materials[i]);
      }
    } else {
      uint _totalFee = _fee + _eatherOwnPrice;
      for(uint i = 0; i < _materials.length; i++) {
        (,,,uint suppliedItemOwnPrice,) = itemNft.getItem(_materials[i]);
        require(_totalMaterialOwnPrice > 0, "Item Upgrading: total own price is invalid");
        itemNft.updateFailedUpgradingAmount(_materials[i], _totalFee * suppliedItemOwnPrice / _totalMaterialOwnPrice);
      }
    }
  }
