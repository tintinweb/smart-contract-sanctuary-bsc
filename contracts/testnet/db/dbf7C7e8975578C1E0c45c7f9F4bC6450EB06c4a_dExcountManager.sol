...
import "./interfaces/IdExcount.sol";
import "./dExcount.sol";

contract dExcountManager is Ownable, Pausable{    
    ...
    uint256 public maximumVestingDuration;
    uint256 public minimumAllowedBuyBackShare;
    uint8 public platformShare;
    
    address public treasuryWallet;

    mapping (address => EnumerableSet.AddressSet) private pairInfo;
    ...
    constructor(
        ...
        maximumVestingDuration = 15770000;
    }   
    ...
    */
    ...
    function platformFlatFeeUpdate(uint256 amount) public onlyOwner {
        require(amount != 0, "dExcountManager: invalid amount");
        platformFlatFee = amount;
    }

    function platformShareUpdate(uint8 newPlatformShare) public onlyOwner {
        require(newPlatformShare != 0, "dExcountManager: invalid amount");
        platformShare = newPlatformShare;
    }

    function setMinimumAllowedBuyBackShare(uint256 newMinimumAllowedBuyBackShare) external onlyOwner {
        minimumAllowedBuyBackShare = newMinimumAllowedBuyBackShare;
    }
     ...
     */   
    function setMaximumVestingDuration(uint256 newMaximumVestingDuration) public onlyOwner {
        maximumVestingDuration = newMaximumVestingDuration;
    }
    ...
    struct Parameters {
        ...
        uint8 discount;
        uint8 projectOwnerShare;
        uint8 buyBackShare;
        uint8 referralShare;
        uint8 saleType;
        bool isReferral;
        bool isReferralTokenEnable;
        string profile;
    }
    ...
    function createPool(Parameters calldata store) external payable whenNotPaused {
        require(validateSellDate(store.startTime,store.endTime), "dExcountManager: Invalid time duration");
        require(msg.value >= platformFlatFee, "dExcountManager: Insufficient bnb amount");
        require(store.discount >= 1 && store.discount <= 90, "dExcountManager: Discount is invalid");
        require(store.vestingDuration <= maximumVestingDuration, "dExcountManager:  Claim Date is invalid");
        require((store.projectOwnerShare +
                platformShare +
                store.buyBackShare +
                store.referralShare) <= 100, "dExcountManager: Buyback is invalid");

        if(store.saleType == 1) {
           require(store.buyBackShare >= minimumAllowedBuyBackShare, "dExcountManager: BuyBackShare is failed");
        }

        for(uint8 i;i<pairInfo[store.token].length();i++) {
             require(IdExcount(pairInfo[store.token].at(i)).isSaleOver(), "dExcountManager: Still not overed");
        }

        dExcount newPool = new dExcount{salt: keccak256(abi.encode(store.token,block.timestamp))}(
                store.token,
                store.projectOwner,
                store.discount,
                store.projectOwnerShare,
                platformShare,
                store.buyBackShare,
                store.referralShare,
                store.saleType
...

        {
            newPool.initialize(                
                ...
                store.startTime,
                store.endTime,
                store.currentBasePrice,
                store.vestingDuration,
                store.isReferral,
                store.isReferralTokenEnable,
                store.profile
            );
        }

        pairInfo[store.token].add(address(newPool));
        pools.add(address(newPool));
        IERC20(store.token).safeTransferFrom(_msgSender(),address(newPool),store.offeringAmount);
    ...
    function validateSellDate(uint256 startDate,uint256 endDate) internal view returns (bool) {
        return (
            ...
            startDate < endDate && 
            block.timestamp.add(maxSellStartDuration) >= startDate &&
            startDate.add(maxSellEndDuration) >= endDate
        );
    }
     ...
     * @dev Returns true if the contract is in the discountpool. 
     */
    function saleTokenContains(address pool,address saleToken) public view returns (bool) {
        return pairInfo[pool].contains(saleToken);
    }

    /**
     * @dev Returns the number of discountPools.
     */
    function saleTokenLength(address pool) public view returns (uint256) {
        return pairInfo[pool].length();
    }
     ...
     * - `index` must be strictly less than {length}.
     */
    function saleTokenAt(address pool,uint256 index) public view returns (address) {
        return pairInfo[pool].at(index);
    }

    /**
     * @dev Return the entire discountPool in an array
     */
    function allSaleToken(address pool) public view returns (address[] memory) {
        return pairInfo[pool].values();
    }
}
...
interface IdExcount{
    ...
    function initialize(        
        ...
        uint256 _startTime,
        uint256 _endTime,
        uint256 _currentBasePrice,
        uint256 _vestingDuration,
        bool _isReferral,
        bool _isReferralTokenEnable,
...
import "./interfaces/IdExcountManager.sol";

contract dExcount is Initializable, Pausable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address payable;
    ...
    IdExcountManager public dExcountManager;
    ...
    uint256 public totalDiscountedAmount;
    uint256 public totalSoldOutTokens;
    uint256 public minimumDeposit;
    uint256 public maximumDeposit;
    uint256 public vestingDuration;
    uint256 public currentBasePrice;
    uint8 public projectOwnerShare;
    uint256 public startTime;
    uint256 public endTime;
    uint256 public currentLockTicket;
    uint8 public currentDiscount;
    uint8 public platformShare;
    uint8 public buyBackShare;
    uint8 public referralShare;
    ...
    constructor(        
        ...
        uint8 _discount,    
        uint8 _projectOwnerShare,    
        uint8 _platformShare,
        uint8 _buyBackShare,
        uint8 _referralShare,  
        uint8 _saleType    
        ) {        
        require(_saleType <= uint8(type(saleType).max), "dExcount: Invalid reward type");

        dExcountManager = IdExcountManager(_msgSender());
        saleToken = IERC20(_token); 
        projectOwner = payable(_projectOwner);
        currentDiscount = _discount;

        projectOwnerShare = _projectOwnerShare;
    ...
    function initialize(        
        ...
        endTime = _endTime;
        currentBasePrice = _currentBasePrice;
        vestingDuration = _vestingDuration;
        isReferral = _isReferral;
        isReferralTokenEnable = _isReferralTokenEnable;
    ...
    modifier onlyOperator() {
        require((msg.sender == projectOwner) || 
                (msg.sender == treasuryWallet()), "dExcount: project owner can only accessible ");
        _;
    }

    modifier onlyMainAdmin() {
        require(msg.sender == treasuryWallet(), "dExcount: platform owner can only accessible");
        _;
    } 
    ...
    */
    ...
    function recoverLeftOvercoin(uint256 amount) external onlyMainAdmin {
    ...
    function recoverLeftOverToken(address token,uint256 amount) external onlyMainAdmin {
        if(!isClosed) require(token != address(saleToken), "dExcount: unable to recover this token");
     ...
     */     
    function blockPool(bool status) public onlyMainAdmin {
        isPoolBlock = status;
    }    
    ...
    function endPool() external nonReentrant onlyOperator{
        require(endTime < block.timestamp, "dExcount: Not closed");
        require(!isClosed, "dExcount: Already closed");

        isClosed = true;
        uint256 claimableAmount = offerringTokenBalance().sub(totalDiscountedAmount);
        saleToken.safeTransfer(projectOwner,claimableAmount);
    }

    function buy(address referrer) external payable nonReentrant whenNotPaused returns (bool) {
        require(startTime < block.timestamp && endTime > block.timestamp, "dExcount: expired");
        require(!isPoolBlock, "dExcount: Pool is blocked");
     ...
     * 
     * 
     * - E.g. After the discount sale admin can be able to buyback the coin to token.
     * 
     */  
    ...
    function multiClaim() external whenNotPaused nonReentrant {
        ...
        uint8 totalCount = getUserLockTicketLength(user);
    ...
    function claim(uint256 lockTicket) external nonReentrant whenNotPaused {
        ...
        require(getUserLockTicketContains(_msgSender(),lockTicket), "dExcount: Not able to access this ticketid");
        require(store.unLockedTime <= block.timestamp, "dExcount: Unable to access this time");
        
        saleToken.safeTransfer(msg.sender,store.lockedAmount);
        store.lockedAmount = 0;
        store.claimTime = block.timestamp;
        totalDiscountedAmount = totalDiscountedAmount.sub(store.lockedAmount);
    ...
    function _buy(address user,address referrer,uint256 amount) internal returns (bool){
        require(minimumDeposit <= amount && maximumDeposit >= amount, "dExcount: Deposit amount is invalid");
        require(user != referrer, "dExcount: Referrer Address is invalid");

        uint256 amountOut;
        uint256 discount;
        uint256 totalAmountOut;
        if(currentSale == saleType.BASEPRICE) {
           (amountOut,discount,totalAmountOut) = _getAmountOutForBasePrice(amount);
        } else if(currentSale == saleType.MARKETPRICE) {
            (amountOut,discount,totalAmountOut) = _getAmountOutForMarketPrice(amount);
        }

        if(referralCommission[user].referrer == address(0) && referrer != address(0)) {
            referralCommission[user].referrer = referrer;
        }

        referrer = referralCommission[user].referrer;
        totalSoldOutTokens = totalSoldOutTokens.add(totalAmountOut);
        distribute(referrer,amount);

        if(discount > 0) {
            currentLockTicket++;
            totalDiscountedAmount = totalDiscountedAmount.add(discount);
    ...
    function distribute(
        ...
        if(isReferral && referrer != address(0)) {
            ...
            referralCommission[referrer].coinEarned = referralCommission[referrer].coinEarned.add(referrerPayout);
        }

        if(projectOwnerShare > 0) projectOwner.sendValue(amount.mul(projectOwnerShare).div(1e2));
        
        if(platformShare > 0) payable(treasuryWallet()).sendValue(amount.mul(platformShare).div(1e2));
    ...
    function _getAmountOutForBasePrice(
        ...
        discount = amountOut.mul(currentDiscount).div(100);
        totalAmountOut = amountOut.add(discount);
        return (amountOut,discount,totalAmountOut);        
    }
    ...
    function _getAmountOutForMarketPrice(
        ...
        discount = getAmountOut[1].mul(currentDiscount).div(100);
        totalAmountOut = getAmountOut[1].add(discount);
        return (getAmountOut[1],discount,totalAmountOut);   
    }
    ...
    function getUserLockTicketLength(address account) public view returns (uint8) {
         return uint8(userLockTicketInfo[account].length());
    }
    ...
    function treasuryWallet() public view returns (address) {
        return dExcountManager.treasuryWallet();
    }
