    ...
    */
    ...
    function traitRarity() internal {
        ...
        if(totalPizzas > 0) {
            ...
            if(sauceUsed > 0) {
                ingredientRarityPercent[5000] = 1 ether * (hundered.sub(sauceUsed.mul(100).div(totalPizzas))); // percentage for sauce not used
            }
            if(cheeseUsed > 0) {
                ingredientRarityPercent[5001] = 1 ether * (hundered.sub(cheeseUsed.mul(100).div(totalPizzas))); // percentage for cheese not used
            }
        }
        else {
            for(uint256 i = 1; i <= totalIngredients; i++) {
