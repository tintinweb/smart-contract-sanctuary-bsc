...
contract DrunkCarts is ERC721, ERC721Enumerable, Governable, ERC721Burnable  {
    using Counters for Counters.Counter;

    string public baseURI;
    uint8[][] private cartTypes;
    ...
    function mint(address to, uint8[] memory types) public onlyGovernance {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();

        _safeMint(to, tokenId);

        cartTypes.push(types);
    }

    function distributeType(uint cartId, uint typeId, uint8 val) public onlyGovernance{
        require(cartTypes[cartId].length >= typeId);

        if(cartTypes[cartId].length == typeId){
            cartTypes[cartId].push(val);
        }
        else{
            cartTypes[cartId][typeId] = val;
        }
    }

    function getCartTypes(uint id) public view returns(uint8[] memory){
        return cartTypes[id];
    }

    function getCartType(uint id, uint typeId) public view returns(uint8){
        if(cartTypes[id].length <= typeId){
            return 0;
        }
        return uint8(cartTypes[id][typeId]);
    }
