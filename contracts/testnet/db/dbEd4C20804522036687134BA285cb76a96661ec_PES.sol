...
contract PES is ERC20, Ownable {
    ...
    constructor() ERC20("PES", "PES") payable {
        ...
        if(block.chainid == 1){
            operationsAddress = address(0xbB6642bEC30053e4898aa4b15D67d6462b7f9B56);
            devAddress = address(0x69f6E524DC1AF1bb42065f5C35eF729Ef01a6226);
            pesOwner = address(0xb3118cE69ffF8be2f59055a3D24B05aC893C3be1);
        } else {
            operationsAddress = address(msg.sender);
            devAddress = address(msg.sender);
            pesOwner = address(msg.sender);
        }
        ...
        _excludeFromMaxTransaction(address(pesOwner), true);
        ...
        excludeFromFees(address(pesOwner), true);

        
        _createInitialSupply(address(this), totalSupply * 66 / 100);  // update with % for LP
        _createInitialSupply(pesOwner, totalSupply - balanceOf(address(this)));
