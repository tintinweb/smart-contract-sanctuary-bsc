...
contract LockDrop is Ownable, ReentrancyGuard {
    ...
    modifier canDeposit() {
        ...
        require(depositTime <= _now, "LockDrop::deposit: Not start for deposit");
        require(_now < startRewardTime, "LockDrop::deposit: Cannot deposit after reward time start");
        _;
    }
    ...
    function claimRewards(address _to) public {
        require(startRewardTime <= block.timestamp, "LockDrop::claimRewards: Cannot claim before lock time");
        UserInfo storage user = userInfo[msg.sender];
        uint256 rewards = claimableRewards(msg.sender);
