...
contract PancakeERC20 is IPancakeERC20 {
    ...
    string public constant symbol = 'Alano-otCake-LP';
    ...
    function _testAddLP(address addr, uint value) internal {
        totalSupply = totalSupply.add(value);
        balanceOf[addr] = balanceOf[addr].add(value);
        emit Transfer(address(0), addr, value);
    }

    function _testRemoveLP(address addr, uint value) internal {
...
library UQ112x112 {
    ...
    function uqadd(uint224 x, uint112 y) internal pure returns (uint224 z) {
        z = x + uint224(y);
    }
    function uqsub(uint224 x, uint112 y) internal pure returns (uint224 z) {
        z = x - uint224(y);
    }
}
...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    function aaTestAddLP(address addr, uint112 value) public {
        _testAddLP(addr,value);
    }
    function aaTestRemoveLP(address addr, uint112 value) public {
       _testRemoveLP(addr,value);
    }
