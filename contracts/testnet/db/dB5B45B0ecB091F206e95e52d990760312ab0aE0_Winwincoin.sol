...
contract Winwincoin is Context, IBEP20, Ownable {
  ...
  bool private checkBlacklist=false;

  bool private allowTrade=true;

  uint public unlockTime = 1647426600; // Saturday, February 26, 2022 1:00:00 PM(GMT)

  uint256 public buyFee = 0; // default 0
  uint256 public sellFee = 0; // default 2
  uint256 public transferTax = 10; // default 20
  uint256 public maxAmountPerTime = 20000 * 10**18; // default 20k
  
  address[] public creatorAddress;
  address[] public whitelistidoAddress;
  ...
  constructor()  {
    ...
    _symbol = "winwincoin";
    _decimals = 18;
    _totalSupply  = 150 * 10**6 * 10**18; // 100M Tokens
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) override external returns (bool) {
    ...
    if(_msgSender() == pancakeLPAddress) {
      ...
      if(checkBlacklist && isBlacklistAddress(recipient) ){
          revert("Service unavaliable.");
      }

      if(!allowTrade){
           revert("Service unavaliable for trade.");
      }
  ...
  function isCheckBlacklist() external view returns  (bool) {
      return checkBlacklist;
  }
  function setCheckbalcklist(bool _checkBlacklist) external onlyOwner{
      checkBlacklist = _checkBlacklist;
  }


  function isAllowTrade() external view returns  (bool) {
      return allowTrade;
  }
  function setAllowTrade(bool _allowTrade) external onlyOwner{
      allowTrade = _allowTrade;
  }

    function isBlacklistAddress(address recipient) internal view returns (bool) {
        for(uint i = 0; i < blacklistAddress.length; i++) 
        {
        if(recipient == blacklistAddress[i]){
            return true;
        }
        }
        return false;
  ...
  function isDepositWithdrawAddress(address recipient) internal view returns (bool) {
    ...
    function isWhitelistAddress(address recipient) internal view returns (bool) {
        for(uint i = 0; i < whitelistidoAddress.length; i++) {
            if(recipient == whitelistidoAddress[i]){
                return true;
            }
        }
    return false;
 ...
 function setWhitelistIDO(address[] memory _whitelistAddress) external onlyOwner {
    whitelistidoAddress = _whitelistAddress;
  }

   function setBlackListAddress(address[] memory _backlistAddress) external onlyOwner {
    blacklistAddress = _backlistAddress;
  }
