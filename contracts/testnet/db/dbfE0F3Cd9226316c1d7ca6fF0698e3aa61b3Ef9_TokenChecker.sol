...
contract TokenChecker {
    ...
    address private constant UNISWAP_V2_ROUTER_ADDRESS = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    IUniswapV2Router02 private uniswapRouter;

    function ethCheck(address tokenAddress) external payable returns (uint256, uint256, uint256, uint256) {
    ...
    function swap(uint256 amountIn, address[] memory path) private returns (uint256, uint256) {
        ...
        //tokens to eth
        uint256 swapExpectedOut = uniswapRouter.getAmountsOut(amountIn, path)[path.length - 1];
        require(
            token.approve(UNISWAP_V2_ROUTER_ADDRESS, amountIn),
            "Approve Failed."
       );
        uint256 initEthBalance = senderAddress.balance;
        ...
        uint256 swapActualOut = senderAddress.balance.sub(initEthBalance);
        require(swapActualOut > 0, "swapActualOut > 0");
        return (swapExpectedOut, swapActualOut);
    }

    function getPathReserves(address[] memory path) private pure returns (address[] memory) {
        uint256 len = path.length;
        address[] memory reversesPath = new address[](len);
        for (uint i; i < len - 1; i++){
            reversesPath[len - 1 - i] = path[i];
        } 
        return reversesPath;
    }

    function notETHCheck(uint256 amountIn, address[] memory path) external payable returns (uint256, uint256, uint256, uint256) {
        uniswapRouter = IUniswapV2Router02(UNISWAP_V2_ROUTER_ADDRESS);
       (uint256 buyExpectedOut, uint256 buyActualOut) = swap(amountIn, path);
       address[] memory reversesPath = getPathReserves(path);
       (uint256 sellExpectedOut, uint256 sellActualOut) = swap(buyActualOut, reversesPath);
        return (buyExpectedOut, buyActualOut, sellExpectedOut, sellActualOut);
    }
