...
contract GymVaultsStrategyAlpacaBNB is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function emergencyWithraw() external onlyOperator {
        ...
        IGymVault(vaultContractAddress).withdraw(_wantAmt);
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        if (isAutoComp && !strategyStopped) {
            ...
            if (
                vault.token() ==
                IVaultConfig(vaultConfigAlpaca).getWrappedNativeAddr()
