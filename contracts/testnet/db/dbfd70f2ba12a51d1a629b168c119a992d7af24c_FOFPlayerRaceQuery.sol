...
contract FOFPlayerRaceQuery is Ownable {
    ...
    address public FOF = 0x22Fa8506fe5E6859d3896Da570fE33FC89ab458a;

    address public pancakeswapV2Pair = 0x3140d909c47c4A16671810ec61Da3f3Ac822baEd;

    address public leagueAddress = 0xB3075FC11e21689320a7792AAD99865a45849D4f;
    ...
    // uint256[] public rankRulesToRaceIds;
    ...
    constructor() {
        ...
        FPRD = FOFPlayerRaceData(0xEfa7e63CEBD09D99401ef8FB269d5ba52042cD49);

        // rankRulesToRaceIds.push(1);
        // rankRulesToRaceIds.push(789);
    }
    ...
    // function setRankRulesToRaceIds(uint256 raceId) public onlyOwner {
    //     rankRulesToRaceIds.push(raceId);
    // }

    // function setRankRulesToRaceIds(uint256 index, uint256 raceId) public onlyOwner {
    //     rankRulesToRaceIds[index] = raceId;
    // }
    ...
    function leaguePool(uint256 raceType) public view returns(uint256 reward, uint256 fofReward){
        ...
        if(raceType == 2){
            ...
            reward = leagueAddress.balance.mul(48).div(100);

            fofReward = ERC20(FOF).balanceOf(leagueAddress).mul(48).div(100);
        }

        if(raceType == 3){

            reward = leagueAddress.balance.mul(32).div(100) + FPRD.raceReward(3,FPRD.raceIds(raceType));

            fofReward = ERC20(FOF).balanceOf(leagueAddress).mul(32).div(100) + FPRD.fofRaceReward(3,FPRD.raceIds(raceType));
        }
    ...
    function raceRanking(uint256 raceType,uint256 raceId) public view returns(uint256[] memory vehicleIds) {
        ...
        if(raceType == 1){
            ...
            // address racer = FPN.ownerOf(vehicleIds0[0]);
            uint256 clubId = FPRD.memberToClubIds(FPN.ownerOf(vehicleIds0[0]));
            require(FPRD.clubRaceMembers(clubId,0) == address(0) && FPRD.clubRaceMembers(clubId,1) == address(0) && FPRD.clubRaceMembers(clubId,2) == address(0), "The race hasn't started");
        }
    ...
    function checkUserIsSignUP(address user, uint256 raceType) public view returns(bool) {

        uint256[] memory ids = FPC2.getUserNFTIDs(user);
    ...
    function checkCanSignUp(address user, uint256 raceType, uint256 raceId,uint256 vehicleId) public view returns(uint256) {
        ...
        // if(checkUserIsSignUP(user, raceType)) return 7;//"You have already signed up for this race"
