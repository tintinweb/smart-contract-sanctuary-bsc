...
interface IRouter is IRouter01 {
...
contract Router is IRouter {
    ...
    constructor(address _factory, address _WBNB) {
        require(_factory != address(0) && _WBNB != address(0), "Router: Zero address(es)");
        factory = _factory;
        WBNB = _WBNB;
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = RouterLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = RouterLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = RouterLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function addLiquidityBNB(
        ...
        address pair = RouterLibrary.pairFor(factory, token, WBNB);
    ...
    function removeLiquidity(
        ...
        {
        address pair = RouterLibrary.pairFor(factory, tokenA, tokenB);
        require(IPair(pair).transferFrom(msg.sender, pair, liquidity), "Router: Error on transfering"); // send liquidity to pair
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? RouterLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IPair(RouterLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, RouterLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWBNB(WBNB).transfer(RouterLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactBNB(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WBNB, 'Router: INVALID_PATH');
        amounts = RouterLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, RouterLibrary.pairFor(factory, path[0], path[1]), amounts[0]
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = RouterLibrary.sortTokens(input, output);
            IPair pair = IPair(RouterLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = RouterLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, RouterLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWBNB(WBNB).transfer(RouterLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IBEP20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IBEP20(path[path.length - 1]).balanceOf(to) - balanceBefore >= amountOutMin,
    ...
    function pairFor(address tokenA, address tokenB) external view returns (address) {
        return RouterLibrary.pairFor(factory, tokenA, tokenB);
    }
