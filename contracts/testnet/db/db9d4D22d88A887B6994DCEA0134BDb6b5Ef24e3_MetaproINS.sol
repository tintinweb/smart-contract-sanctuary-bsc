...
contract MetaproINS is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    function withdraw(uint256 insId, bytes memory _data) external nonReentrant {
        ...
        uint256[] memory createdArray = createdInsTokenIds;
        ...
        uint256[] memory newCreatedinsTokenIds = new uint256[](
            createdArray.length - 1
        );

        if (allInsForTokenIdFinalized) {
            for (uint256 i = 0; i < createdArray.length; i++) {
                if (createdArray[i] == configuration.tokenId) {
                    for (uint256 a = 0; a < i; a++) {
                        newCreatedinsTokenIds[i] = createdArray[i];
                    }
                    for (uint256 a = i + 1; i < createdArray.length; i++) {
                        newCreatedinsTokenIds[a - 1] = createdArray[a];
                    }
                }
            }
        }

        createdInsTokenIds = newCreatedinsTokenIds;
        availableIns[insId].valid = false;
