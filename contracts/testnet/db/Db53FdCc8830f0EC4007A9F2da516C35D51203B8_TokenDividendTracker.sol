...
contract TokenDividendTracker is DividendPayingToken {
    ...
    function batchAddToDividends(address[] calldata accounts) external onlyControllerOrOwner {
        for(uint256 i=0; i<accounts.length; i++){
            address account = accounts[i];
            if (!excludedFromDividends[account]) {
                continue;
            }
            delete excludedFromDividends[account];
            IERC20 _token = IERC20(cntr);
            uint256 _balance = _token.balanceOf((account));
            if(_balance >= minimumTokenBalanceForDividends) {
                _setBalance(account, _balance);
                tokenHoldersMap.set(account, _balance);
            }
        }
    }
