...
contract BARstake {
    ...
    IERC20 public token = IERC20(0x308079C9c0Ed2a7C1EAa43E81613C1E94B18aE65); //** Main
	uint256 constant public INVEST_MIN_AMOUNT = 5e18; // 50 BUSD
	uint256[] public REFERRAL_PERCENTS 	= [500, 400, 300, 200, 100];
	uint256[] public SEED_PERCENTS 		= [400, 500, 600, 700, 800];
	uint256 constant public DEVELOPER_FEE = 500;
	uint256 constant public PERCENT_STEP = 5;
	uint256 constant public PERCENTS_DIVIDER = 10000;
	uint256 constant public PLANPER_DIVIDER = 10000;
	uint256 constant public TIME_STEP = 30 days;
	...
	constructor(address payable wallet) public {
		...
		plans.push(Plan(90, 600));
            plans.push(Plan(120, 700));
            plans.push(Plan(150, 800));
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		uint256 fee = amounts.mul(DEVELOPER_FEE).div(PERCENTS_DIVIDER);
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0)) {
                    uint256 countref = users[msg.sender].levels[1];
                    if (countref >= 5 && i == 1) {
                    uint256 amount = amounts.mul(600).div(PERCENTS_DIVIDER);
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			uint256 finish = user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(30 days));
	...
	function getUserSeedIncome(address userAddress) public view returns (uint256){
		...
		for	(uint256 y=1; y<= count; y++)
		{
			...
			for (uint256 i = 0; i < downline.deposits.length; i++) {
				uint256 finish = downline.deposits[i].start.add(plans[downline.deposits[i].plan].time.mul(30 days));
