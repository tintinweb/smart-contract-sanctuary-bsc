...
contract HomeFeeManagerMultiAMBErc20ToErc677Ext is EternalStorage{
    ...
    bytes32 public constant REWARD_ADDRESS_COUNT = 0xabc77c82721ced73eef2645facebe8c30249e6ac372cce6eb9d1fed31bd6648f; // keccak256(abi.encodePacked("rewardAddressCount"))

    address public sparkyVIPDirectory;
    address public SFUEL;

    constructor(address _sparkyVIPDirectory, address _SFUEL) public {
      sparkyVIPDirectory = _sparkyVIPDirectory;
      SFUEL = _SFUEL;
    }

    function calculateFee(bytes32 _feeType, address _token, uint256 _value, address _receiver) public view returns (uint256, bool) {
        uint256 _fee = getFee(_feeType, _token, ISparkyVIPDirectory(sparkyVIPDirectory).isSparkyVIP(_receiver));
        uint256 _calFee = _value.mul(_fee).div(MAX_FEE);
        uint256 _sfuelFee = uintStorage[keccak256(abi.encodePacked("sfuelFee",_feeType))];

        if (_sfuelFee == 0) return (_calFee, false);

        if(ERC20(SFUEL).allowance(_receiver, msg.sender) >= _sfuelFee && ERC20(SFUEL).balanceOf(_receiver) >= _sfuelFee){
    ...
    function distributeFee(bytes32 _feeType, address _token, uint256 _value, address _receiver) public returns (uint256) {
        uint256 numOfAccounts = uintStorage[REWARD_ADDRESS_COUNT];
        ...
        address nextAddr = addressStorage[keccak256(abi.encodePacked("rewardAddressList", F_ADDR))];
        ...
        while (nextAddr != F_ADDR) {
            ...
            if(_useSFUEL && _feeType == HOME_TO_FOREIGN_FEE){
                ERC677(SFUEL).transferFrom(_receiver, nextAddr, feeToDistribute);
        ...
        if (_useSFUEL && _feeType == HOME_TO_FOREIGN_FEE) _fee = 0;
        return _fee;
    }
}