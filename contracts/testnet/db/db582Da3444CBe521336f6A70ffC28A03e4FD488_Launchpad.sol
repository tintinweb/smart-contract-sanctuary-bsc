...
contract Launchpad is Initializable {
  ...
  event LaunchpadOperatorChanged(address indexed _address);
  event AuctionOperatorChanged(uint256 indexed _pid, address indexed _address);
   ...
   */
  modifier onlyAuctionOpperator(uint256 _pid) {
    require(msg.sender == PROJECTS[_pid].auction.getOperator(), 'ONLY_AUCTION_OPERATOR');
    _;
  }
  ...
  function changeLaunchpadOperator(address _launchpadOperator) external onlyLaunchpadOperator {
    ...
    LAUNCHPAD_OPERATOR = _launchpadOperator;
    emit LaunchpadOperatorChanged(_launchpadOperator);
  }

 /**
   * @notice Change Auction Operator
   * @param _auctionOperator new auction opertor address
   */
  function changeAuctionOperator(uint256 _pid, address _auctionOperator) external onlyAuctionOpperator(_pid) {
    require(_auctionOperator != address(0), 'INVALID_ADDRESS');
    PROJECTS[_pid].auction.changeOperator(_auctionOperator);
    emit AuctionOperatorChanged(_pid, _auctionOperator);
  }
  ...
  function setAllowlistSigner(uint256 _pid, address _signer)
    external isInitialized(_pid) onlyAuctionOpperator(_pid) {
...
contract BatchAuction is ReentrancyGuard {
  ...
  /// @notice Operator of this contract
  ...
  /* ========== EVENTS ========== */
  event OperatorChanged(address indexed _addr);
  ...
  /* ========== MODIFIERS ========== */
  /**
   * @notice Only Operator can manage the project
   */
   ...
   * @param _operator new opertor address
   */
  ...
  function setAllowlistSigner(address _signer) external onlyOperator {
   ...
   */
  function transferAuctionTokenFromVault() external onlyOperator nonReentrant {
   ...
   */
  function cancelAuctionBeforeStart() external onlyOperator {
    require(!isAuctionFinalized(), "AUCTION_SHOULD_NOT_BE_FINALIZED"); 
   ...
   */
  function withdrawUnclaimedAuctionToken() external isClaimablePeriodEnded onlyOperator {
   ...
   * only owner (admin) can execute this
   */
  function withdrawUnclaimedLPToken() external isClaimablePeriodEnded onlyOperator {
  ...
  function getCommitmentBack() external canCommitmentBack nonReentrant {
    ...
    require(userCommited > 0, "NO_COMMITMENTS");
   ...
   */
  function finalizeAuction() external canFinalizeAuction onlyOperator {
  ...
  */
  ...
  function _setupPancakeSwapPool(
    ...
    uint256 _token1Amount,
    uint256 _token2Amount
  ) internal onlyOperator returns (address, uint256) {
