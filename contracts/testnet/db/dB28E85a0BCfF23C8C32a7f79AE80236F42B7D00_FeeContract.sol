...
interface IIYAL { 
    ...
    function currentMonth() external returns (uint256);
    function c_amountHistory(address) external returns(uint256);
    function totalAmount(uint256) external returns(uint256);
    function feeHistory(uint256) external returns(uint256);
    function calcFeeAmount() external;
}
...
contract FeeContract is Ownable {
    ...
    function withdrawTokens(address _token) external onlyOwner {
        ...
        if(_token == address(0)) {
            ...
            uint256 ca = iyal.feeHistory(currentMonth) + iyal.feeHistory(nextMonth);
            uint256 amount = IERC20(_token).balanceOf(address(this));
            require(ca < amount, "Not enough tokens");
            amount -=  ca;
    ...
    function claimToken() external {
        calcMonth();
        uint256 amount = iyal.feeHistory(currentMonth) * iyal.c_amountHistory(msg.sender) / iyal.totalAmount(currentMonth);
        iya.transfer(msg.sender, amount);
    }
