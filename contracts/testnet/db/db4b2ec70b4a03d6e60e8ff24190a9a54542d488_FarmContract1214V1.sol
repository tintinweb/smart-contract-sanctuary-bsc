...
contract FarmContract1214V1 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    struct DepositInfo {
        ...
        uint256 refferalRewardDebt;
        uint256 nodeRewardDebt;
        uint256 depositBlock;
        uint256 depositTime;
        uint256 lastUpdateRefferalRewardBlock;
        uint256 lastUpdateNodeRewardBlock;
        bool onGoing;
    }
    ...
    mapping(uint256 => mapping(address => uint256)) public availableRefferalReward;
    mapping(uint256 => mapping(address => uint256)) public claimedRefferalReward;
    mapping(uint256 => mapping(address => uint256)) public availableNodeReward;
    mapping(uint256 => mapping(address => uint256)) public claimedNodeReward;
    ...
    function pendingRefferalReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalMiningPending) {
        uint256 accMiningTokenPerShare = poolInfo[_pid].accMiningTokenPerShare;
        uint256 lpSupply = poolInfo[_pid].lpToken.balanceOf(address(this));
        if (block.number > poolInfo[_pid].lastUpdateBlock && lpSupply != 0) {
            uint256 blockInterval = block.number.sub(poolInfo[_pid].lastUpdateBlock);
        ...
        for(uint256 i = 0; i < userDepositInfo[_pid][_user].length; i++) {
            if(userDepositInfo[_pid][_user][i].onGoing) {
                uint256 amount = userDepositInfo[_pid][_user][i].amount;
                uint256 recordAccMiningTokenPerShare = currentAccMiningTokenPerShare[_pid][userDepositInfo[_pid][_user][i].lastUpdateRefferalRewardBlock];
                uint256 actualAccMiningTokenPerShare = accMiningTokenPerShare.sub(recordAccMiningTokenPerShare);
                uint256 pendingReward = amount.mul(actualAccMiningTokenPerShare).div(1e12).sub(userDepositInfo[_pid][_user][i].refferalRewardDebt);
                totalMiningPending = totalMiningPending.add(pendingReward);
            }
        }
    }

    function pendingNodeReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalMiningPending) {
        uint256 accMiningTokenPerShare = poolInfo[_pid].accMiningTokenPerShare;
        uint256 lpSupply = poolInfo[_pid].lpToken.balanceOf(address(this));
        if (block.number > poolInfo[_pid].lastUpdateBlock && lpSupply != 0) {
            uint256 blockInterval = block.number.sub(poolInfo[_pid].lastUpdateBlock);
        ...
        for(uint256 i = 0; i < userDepositInfo[_pid][_user].length; i++) {
            if(userDepositInfo[_pid][_user][i].onGoing) {
                uint256 amount = userDepositInfo[_pid][_user][i].amount;
                uint256 recordAccMiningTokenPerShare = currentAccMiningTokenPerShare[_pid][userDepositInfo[_pid][_user][i].lastUpdateNodeRewardBlock];
                uint256 actualAccMiningTokenPerShare = accMiningTokenPerShare.sub(recordAccMiningTokenPerShare);
                uint256 pendingReward = amount.mul(actualAccMiningTokenPerShare).div(1e12).sub(userDepositInfo[_pid][_user][i].refferalRewardDebt);
                totalMiningPending = totalMiningPending.add(pendingReward);
            }
    ...
    function settlePendingRefferalReward(uint256 _pid, address _user) internal{
        uint256 totalRefferalPending;
        address myFristInviter = IInviteContract(inviteContract).getInviter(_user);
        address mySecondInviter = IInviteContract(inviteContract).getInviter(myFristInviter);
        if(depositAmount[_pid][_user] > 0) {
            for(uint256 i = 0; i < userDepositInfo[_pid][_user].length; i++) {
                if(userDepositInfo[_pid][_user][i].onGoing) {
                    uint256 amount = userDepositInfo[_pid][_user][i].amount;
                    uint256 recordAccMiningTokenPerShare = currentAccMiningTokenPerShare[_pid][userDepositInfo[_pid][_user][i].lastUpdateRefferalRewardBlock];
                    uint256 actualAccMiningTokenPerShare = poolInfo[_pid].accMiningTokenPerShare.sub(recordAccMiningTokenPerShare);
                    uint256 pendingReward = amount.mul(actualAccMiningTokenPerShare).div(1e12).sub(userDepositInfo[_pid][_user][i].refferalRewardDebt);
                    userDepositInfo[_pid][_user][i].refferalRewardDebt = userDepositInfo[_pid][_user][i].refferalRewardDebt.add(pendingReward);
                    userDepositInfo[_pid][_user][i].lastUpdateRefferalRewardBlock = block.number;
                    totalRefferalPending = totalRefferalPending.add(pendingReward);
                }
            }
            uint256 FInviterDeservedReward = totalRefferalPending.mul(getRefferalRewardPercent(_pid, myFristInviter)).div(rateDenominator);
            uint256 SInviterDeservedReward = totalRefferalPending.mul(getRefferalRewardPercent(_pid, mySecondInviter)).div(rateDenominator);
            availableRefferalReward[_pid][myFristInviter] = availableRefferalReward[_pid][myFristInviter].add(FInviterDeservedReward);
            availableRefferalReward[_pid][mySecondInviter] = availableRefferalReward[_pid][mySecondInviter].add(SInviterDeservedReward);
        }
    }

    function settlePendingNodeReward(uint256 _pid, address _user) internal {
        uint256 totalNodePending;
        address myFristInviter = IInviteContract(inviteContract).getInviter(_user);
        address mySecondInviter = IInviteContract(inviteContract).getInviter(myFristInviter);
        bool canSettle = getLevel(_pid, _user) < getLevel(_pid, myFristInviter) || getLevel(_pid, _user) < getLevel(_pid, mySecondInviter);
        if(depositAmount[_pid][_user] > 0 && canSettle) {
            for(uint256 i = 0; i < userDepositInfo[_pid][_user].length; i++) {
                if(userDepositInfo[_pid][_user][i].onGoing) {
                    uint256 amount = userDepositInfo[_pid][_user][i].amount;
                    uint256 recordAccMiningTokenPerShare = currentAccMiningTokenPerShare[_pid][userDepositInfo[_pid][_user][i].lastUpdateNodeRewardBlock];
                    uint256 actualAccMiningTokenPerShare = poolInfo[_pid].accMiningTokenPerShare.sub(recordAccMiningTokenPerShare);
                    uint256 pendingReward = amount.mul(actualAccMiningTokenPerShare).div(1e12).sub(userDepositInfo[_pid][_user][i].nodeRewardDebt);
                    userDepositInfo[_pid][_user][i].nodeRewardDebt = userDepositInfo[_pid][_user][i].nodeRewardDebt.add(pendingReward);
                    userDepositInfo[_pid][_user][i].lastUpdateNodeRewardBlock = block.number;
                    totalNodePending = totalNodePending.add(pendingReward);
                }
            } 
            if(getLevel(_pid, _user) < getLevel(_pid, myFristInviter)) {
                uint256 FInviterDeservedReward = totalNodePending.mul(getNodeRewardPercent(_pid, myFristInviter)).div(rateDenominator);
                availableNodeReward[_pid][myFristInviter] = availableNodeReward[_pid][myFristInviter].add(FInviterDeservedReward);
            }
            if(getLevel(_pid, _user) < getLevel(_pid, mySecondInviter)) {
                uint256 SInviterDeservedReward = totalNodePending.mul(getNodeRewardPercent(_pid, mySecondInviter)).div(rateDenominator);
                availableNodeReward[_pid][mySecondInviter] = availableNodeReward[_pid][mySecondInviter].add(SInviterDeservedReward);
            }
        }
    }

    function getAllPendingRefferalReward(address _user) public view returns(uint256 totalPendingRefferalReward) {
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        uint256 totalRefferalReward;
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            for (uint256 i = 0; i < InviteeAddress.length; i++) {
                uint256 pending = pendingRefferalReward(pid, InviteeAddress[i]).mul(getRefferalRewardPercent(pid, _user)).div(rateDenominator);
                totalRefferalReward = totalRefferalReward.add(pending);
            }
            totalRefferalReward = totalRefferalReward.add(availableRefferalReward[pid][_user].sub(claimedRefferalReward[pid][_user]));
            totalPendingRefferalReward = totalPendingRefferalReward.add(totalRefferalReward);
        }
    }

    function getAllPendingNodeReward(address _user) public view returns(uint256 totalPendingNodeReward) {
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        uint256 totalNodeReward;
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            for (uint256 i = 0; i < InviteeAddress.length; i++) {
                if(getLevel(pid, InviteeAddress[i]) < getLevel(pid, _user)) {
                    uint256 pending = pendingNodeReward(pid, InviteeAddress[i]).mul(getNodeRewardPercent(pid, _user)).div(rateDenominator);
                    totalNodeReward = totalNodeReward.add(pending);
                }
            }
            totalNodeReward = totalNodeReward.add(availableNodeReward[pid][_user].sub(claimedNodeReward[pid][_user]));
            totalPendingNodeReward = totalPendingNodeReward.add(totalNodeReward);
        }
    }

    function claimAllPendingRefferalReward() public {
        require(getAllPendingRefferalReward(_msgSender()) > 0, "You have no refferal reward to claim");
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_msgSender());
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            for (uint256 i = 0; i < InviteeAddress.length; i++) {
                if(getLevel(pid, _msgSender()) >= 1) {
                    settlePendingRefferalReward(pid, InviteeAddress[i]);
                }
            }
            uint256 pendingreward = availableRefferalReward[pid][_msgSender()].sub(claimedRefferalReward[pid][_msgSender()]);
            claimedRefferalReward[pid][_msgSender()] = claimedRefferalReward[pid][_msgSender()].add(pendingreward);
        }
    }

    function claimAllPendingNodeReward() public {
        require(getAllPendingNodeReward(_msgSender()) > 0, "You have no node reward to claim");
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_msgSender());
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            for (uint256 i = 0; i < InviteeAddress.length; i++) {
                if(getLevel(pid, InviteeAddress[i]) < getLevel(pid, _msgSender()) && getLevel(pid, _msgSender()) >= 2) {
                    settlePendingNodeReward(pid, InviteeAddress[i]);
                }
            }
            uint256 pendingreward = availableNodeReward[pid][_msgSender()].sub(claimedNodeReward[pid][_msgSender()]);
            claimedNodeReward[pid][_msgSender()] = claimedNodeReward[pid][_msgSender()].add(pendingreward);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePid(_pid) {
        ...
        updatePool(_pid);
        settlePendingRefferalReward(_pid, _msgSender());
        settlePendingNodeReward(_pid, _msgSender());
        ...
        userDepositInfo[_pid][_msgSender()].push(DepositInfo({
            ...
            refferalRewardDebt: 0,
            nodeRewardDebt: 0,
            depositBlock: block.number,
            depositTime: block.timestamp,
            lastUpdateRefferalRewardBlock: block.number,
            lastUpdateNodeRewardBlock: block.number,
            onGoing: true
        }));
