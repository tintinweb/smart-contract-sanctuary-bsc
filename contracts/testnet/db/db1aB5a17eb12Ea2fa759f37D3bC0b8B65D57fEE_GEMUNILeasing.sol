...
contract GEMUNILeasing is PermissionGroupUpgradeable, IGEMUNILeasing{
    ...
    function registerLeasing(address nftToken, uint[] memory tokenIds, uint duration, uint minDuration, uint maxDuration, uint[] memory pricePerWeek, uint mortgageAmount, uint rentalPrice) external override {
        ...
        for(uint i = 0; i < tokenIds.length; i++) {
            ...
            require(minDuration > 0 && maxDuration > minDuration && pricePerWeek[i] > 0, "Leasing: invalid input");
    ...
    function updateLeasing(address nftToken, uint tokenId, uint newMinDuration, uint newMaxDuration, uint newPricePerWeek, bool newTransferWallet) external override {
        ...
        emit UpdateLeasing(nftToken, tokenId, newMinDuration, newMaxDuration, newPricePerWeek, newTransferWallet);
    }
    ...
    function rent(address nftToken, uint tokenId, uint duration, uint price, bytes memory sig) external override {
        ...
        if (guild.gameGuild != address(0)) {
            if (leasing.lender != address(0)) {
                require( block.timestamp + duration <= guild.startLending + guild.duration, "Leasing: invalid duration");
            }
    ...
    function checkTransferNFT(address nftToken, uint tokenId, uint amount, address renter, address lender, bool isTransferWallet) internal {
        if (IERC721(nftToken).ownerOf(tokenId) == renter) {
            if (IERC721(nftToken).getApproved(tokenId) == address(this) || IERC721(nftToken).isApprovedForAll(renter, address(this))) {
                LeasingInfo memory leasing = leasingInfo[nftToken][tokenId];
                LeasingGuildInfo memory guild = leasingGuildInfo[nftToken][tokenId];
                //transfer pass to guild (this pass is rented by the guild from the admin)
    ...
    function finishLeasing(address nftToken, uint[] memory tokenIds) external override {
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (msg.sender == guild.gameGuild) {
                guildRetrieveNFT(nftToken, tokenIds[i], renter, guild, leasing);
            } else {
    ...
    function guildRetrieveNFT(address nftToken, uint tokenId, RenterInfo storage renter, LeasingGuildInfo storage guild, LeasingInfo storage leasing) internal {
        ...
        if (nftToken == geniPass) {
            ...
            IGENIPass(geniPass).setActive(tokenId, false);
            if (leasing.lender != address(0)) {
                IERC721(geniPass).transferFrom(renter.renter, address(this), tokenId);
            } else {
                transferNFT(geniPass, tokenId, renter.renter, msg.sender, guild.isTransferWallet);
            }
        } else if (nftToken == gemuniItem) {
            IGEMUNIItem(gemuniItem).unLockItem(tokenId);
            if (leasing.lender != address(0)) {
