...
/****************************************
*                                       *                 
*    effortlessorder.org [email protected]     *
*                                       *
****************************************/
...
contract ELOPresale is ReentrancyGuard {
    ...
    struct PresaleInfo {
        ...
        uint256 soft_token_rate; // token rate during soft presale, 1 base token = ? s_tokens, fixed price
        uint256 public_token_rate; // token rate during public presale
        ...
        uint256 soft_start; // Soft launch presale start date
        uint256 soft_end; // Soft launch presale end date
        uint256 public_start; // Public presale start date
        uint256 public_end; // Public presale end date
    }
    ...
    event UserDepositedSuccess(address, uint256);
    ...
    function init_private (
        address _sale_token,
        uint256 _soft_token_rate,
        uint256 _public_token_rate,
        uint256 _raise_min, 
        uint256 _raise_max, 
        uint256 _softcap, 
        uint256 _hardcap,
        ...
        presale_info.soft_token_rate = _soft_token_rate;
        presale_info.public_token_rate = _public_token_rate;

        token_rate = presale_info.soft_token_rate;
        ...
        presale_info.soft_start = _soft_start;
        presale_info.soft_end = _soft_end;
        presale_info.public_start = _public_start;
        presale_info.public_end = _public_end;
    ...
    function presaleStatus() public view returns (uint256) {
        if ((block.timestamp > presale_info.public_end) && (status.raised_amount < presale_info.softcap)) {
            return 5; // Failure
        }
        if (status.raised_amount >= presale_info.hardcap) {
            return 4; // Wonderful - reached to Hardcap
        }
        if ((block.timestamp > presale_info.public_end) && (status.raised_amount >= presale_info.softcap)) {
            return 4; // SUCCESS - Presale ended with reaching Softcap
        }
        if ((block.timestamp >= presale_info.soft_start) && (block.timestamp <= presale_info.soft_end)) {
            return 3; // ACTIVE - Soft presale is active
        }
        if ((block.timestamp > presale_info.soft_end) && (block.timestamp < presale_info.public_start)) {
            return 2; // DEACTIVE - Soft presale launched but Public presale did not start
        }
        if ((block.timestamp >= presale_info.public_start) && (block.timestamp <= presale_info.public_end)) {
            return 1; // ACTIVE - Public presale is active
        }
    ...
    function userDeposit () public payable nonReentrant {
        require(presaleStatus() == 1 || presaleStatus() == 3, "Not Active");
        ...
        if(block.timestamp >= presale_info.public_start) {
            token_rate = presale_info.public_token_rate;
        }

        uint256 tokensSold = amount_in.mul(token_rate);

        require(tokensSold > 0, "ZERO TOKENS");
        require(status.raised_amount * token_rate <= IERC20(presale_info.sale_token).balanceOf(address(this)), "Token remain error");
        ...
        emit UserDepositedSuccess(msg.sender, msg.value);
    }
    
    // withdraw presale tokens
    // percentile withdrawls allows fee on transfer or rebasing tokens to still work
    function userWithdrawTokens () public nonReentrant {
        require(presaleStatus() == 4, "Not succeeded"); // Success
    ...
    function userWithdrawBaseTokens () public nonReentrant {
        require(presaleStatus() == 5, "Not failed."); // FAILED
    ...
    function ownerWithdrawTokens () private onlyOwner {
        require(presaleStatus() == 5, "Only failed status."); // Failed
    ...
    function refundTokens() public onlyOwner {
        require(presaleStatus() == 4, "Not succeeded"); // Success
        require(block.timestamp > presale_info.public_end, "Must be after presale end time");
