...
contract METAC_Vesting is BEP20("MetaComs token", "METAC") {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "METAC_Vesting::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "METAC_Vesting::delegateBySig: invalid nonce");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "METAC_Vesting::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying METAC_Vestings (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "METAC_Vesting::_writeCheckpoint: block number exceeds 32 bits");
...
contract StakingMETAC_Vesting is Ownable {
    using SafeMath for uint256;
    ...
    // The METAC_Vesting TOKEN!
    METAC_Vesting public local;
    // METAC_Vesting tokens created per block.
    ...
    constructor(
        METAC_Vesting _local,
        address _feeAddress
    ) public {
        local = _local;
        feeAddress = _feeAddress;
        bonusRateForTime[60] = 5;
        bonusRateForTime[90] = 10;
        bonusRateForTime[180] = 40;
        bonusRateForTime[360] = 100;
        // bonusRateForTime[5] = 5;
        // bonusRateForTime[10] = 10;
        // bonusRateForTime[20] = 40;
    ...
    // Update the given pool's METAC_Vesting allocation point and deposit fee. Can only be called by the owner.
    ...
    function getBonusRateFromLockedDate(uint256 _days) public view returns (uint256 bonusRate) {
        ...
        else if (_days >= 90 && _days < 180) {
            bonusRate = bonusRateForTime[90];
        }
        else if (_days >= 180 && _days < 360) {
            bonusRate = bonusRateForTime[180];
        }
        else if (_days >= 360) {
    ...
    // View function to see pending METAC_Vestings on frontend.
    ...
    function addOldUsers(address[] memory _walletAddresses, uint256[] memory amounts, uint256[] memory lockedDaysList, uint256[] memory stakedDaysList) public onlyOwner() {
        require(_walletAddresses.length == amounts.length, 'different length of list');
        require(_walletAddresses.length == lockedDaysList.length, 'different length of list');
        ...
        for (uint256 i = 0; i < _walletAddresses.length; i ++) {
            ...
            uint256 _lockedDays = lockedDaysList[i];
            uint256 _stakedDays= stakedDaysList[i];
            user.amount = user.amount.add(_amount);
            // user.lockedDays= user.lockedDays.add(_lockedDays);
            // user.stakedDays= user.stakedDays.add(_stakedDays);
            uint256 harvestTime= _lockedDays.sub(_stakedDays);
            user.userRewardInfo.push(UserRewardInfo({
                amount: _amount,
                depositAt: block.timestamp.sub(_lockedDays * 1 days),
    ...
    /*
     *Updated addOldUser function //_amount is the METAC_Vesting amount with decimals
     ...
     **/
    function addOldUser(address _walletAddress, uint256 _amount, uint256 _lockedDays,uint256 _stakedDays) public onlyOwner() {
    ...
    // Deposit LP tokens to MasterChef for METAC_Vesting allocation.
    ...
    // Safe local transfer function, just in case if rounding error causes pool to not have enough METAC_Vestings.
    ...
    function currentStakedDays(uint256 _pid, address _user) public view returns (uint256 stakedDays){
    ...
    function lockedTokenForDays(uint256 _pid, address _user) public view returns (uint256 lockedDays){
