...
contract MarketGeneration is TokensRecoverable, IMarketGeneration {
    ...
    modifier checkUserContribution(uint256 _value) {
        bool isAllowedToContributeInPresale = contributorType.getContributionTypeInfo(contributor.getContributor(msg.sender).contributorType).isAllowedToContributeInPresale;
        uint256 _newContributionTotal = (contributor.getContributor(msg.sender).contributionAmount + _value);
        uint256 maxAmountAllowed = contributorType.getContributionTypeInfo(contributor.getContributor(msg.sender).contributorType).maxAmountAllowed;
        if(!isPublic) {
    ...
    function contribution(address _account) public view returns (uint256) {
        return contributor.getContributor(_account).contributionAmount;
    }

    function contributionAllowed(address _user) public view returns (uint256) {
        bool isAllowedToContributeInPresale = contributorType.getContributionTypeInfo(contributor.getContributor(_user).contributorType).isAllowedToContributeInPresale;
        uint256 contributedAmount = contributor.getContributor(_user).contributionAmount;
        uint256 maxAmountAllowed = contributorType.getContributionTypeInfo(contributor.getContributor(_user).contributorType).maxAmountAllowed;
    ...
    function refund() public refundsActive() {
        uint256 amount = contributor.getContributor(msg.sender).contributionAmount;
    ...
    function contribute() public payable checkUserContribution(msg.value) checkHardCap() contributionActive() {
        ...
        if(keccak256(abi.encodePacked(contributor.getContributor(msg.sender).contributorType)) != keccak256(abi.encodePacked(""))) {
            contributor.setContributor(msg.sender, contributor.getContributor(msg.sender).contributionAmount + msg.value, contributor.getContributor(msg.sender).contributorType);
        } else {
