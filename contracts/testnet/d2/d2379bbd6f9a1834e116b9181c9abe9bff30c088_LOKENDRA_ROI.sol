...
function topUp(uint256 _amount) public{
    ...
    function payReferral(uint _level, address _user, uint _value) internal {
      ...
      function withdrawROI() public {
       ...
       function withdrawableROI() public view returns(uint256 reward) {

        uint256 numDays = (block.timestamp - freezeTime[msg.sender]) / 86400;
              // Increase the reward by 0.01% per day
        reward = users[userList[users[msg.sender].referrerID]].stakedToken + ((users[userList[users[msg.sender].referrerID]].stakedToken * numDays * 1 ) / 10000 );
