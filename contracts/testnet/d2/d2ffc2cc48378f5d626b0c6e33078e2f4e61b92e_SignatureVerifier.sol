  ...
  PausableUpgradeable
{
  ...
  function verifyBox(
    ...
    address signer = keccak256(abi.encode(ownerBox, _boxId))
      .ethSignedMessage()
      .recover(signature);
    require(signer == ownerBox, "Invalid request");
    return true;
  }
    ...
    returns (bool)
  {
    address signer = keccak256(
      abi.encode(nonce, receiver, tokenIds, tokenURIs, amountNFTs)
    ).ethSignedMessage().recover(signature);
    require(signer == TRUSTED_PARTY, "Invalid request");
    nonceUsed[nonce] = true;
    emit UsedNonce(signer, nonce, "claim");
    return true;
  }
