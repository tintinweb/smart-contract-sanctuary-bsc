...
contract SafuContract is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'SafuContract';
    ...
    //DevWallets
    address public Development=payable(0xE2f9b08730630Ff213522245B8ef4F45d4172Df3);
    address public Marketing=payable(0xe4fACAF29bB7713A40EE537c56977b846Da6fd57);
    //TestNet
    ...
    // max balance 0.5% supply 
    // max sell  0.2% supply 
    uint256 private _circulatingSupply = InitialSupply ;
    uint256 public  balanceLimit = _circulatingSupply / (5 * (10 ** 3)) ;
    uint256 public swapThreshold = _circulatingSupply / (5 * (10 ** 4));
    //Tracks the current Taxes, different Taxes can be applied for buy/sell/transfer
    uint8 private _maxBuyTax;
    uint8 private _maxSellTax;

    uint8 private _buyFundTax;
    uint8 private _buyLiquidityTax;
    uint8 private _buyMarketingTax;
    uint8 private _sellFundTax;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        bool normalTransfer = !isBuy && !isSell;
        //Pick transfer

        if( isContractTransfer || isLiquidityTransfer || isExcluded || normalTransfer) {
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            tax=_maxSellTax;
            contractToken = _calculateFee( amount, tax, _sellMarketingTax +  _sellLiquidityTax + _sellFundTax );
            ...
            if((sender != _pancakePairAddress) && ( !manualConversion) && (!_isSwappingContractModifier ) )
                _swapContractToken();
        } else if(isBuy){//staking and liquidity Tax get treated the same, only during conversion they get split
            contractToken = _calculateFee ( amount, tax, _buyMarketingTax + _buyLiquidityTax + _buyFundTax );
        }
        
        
        //Subtract the Taxed Tokens from the amount
        uint256 taxedAmount = amount - contractToken;
        require(recipientBalance+taxedAmount <= balanceLimit,"recipient amount exceeding max wallet");

        //Removes token 
        _balances[sender] -= contractToken;
    ...
    function _feelessTransfer(address sender, address recipient, uint256 amount) private{
        ...
        require(senderBalance >= amount, "Transfer exceeds balance");
        uint256 recipientBalance = _balances[recipient];
        require(recipientBalance+amount <= balanceLimit,"recipient amount exceeding max wallet");
        //Removes token 
    ...
    function _swapContractToken() private lockTheSwap {
        ...
        uint16 totalTax= _sellLiquidityTax +_sellMarketingTax + _sellFundTax ;
        //only swap if contractBalance is larger than tokenToSwap, and totalTax is unequal to 0
        if(contractBalance<swapThreshold||totalTax==0){
            return;
        }
        //splits the token in TokenForLiquidity and tokenForMarketing
        uint256 tokenForLiquidity=(contractBalance  * _sellLiquidityTax)/totalTax;
    ...
    function getLimits() public view returns(uint256 balance){
        return(balanceLimit/10**_decimals);
    }

    function getTaxes() public view returns(uint256 buyFundTax,uint256 buyLiquidityTax,uint256 buyMarketingTax, uint256 sellFundTax,uint256 sellLiquidityTax,uint256 sellMarketingTax){
        return (_buyFundTax,_buyLiquidityTax,_buyMarketingTax, _sellFundTax,_sellLiquidityTax,_sellMarketingTax);
    }
    ...
    function TeamSetBuyTaxes(uint8 buyFundTax, uint8 buyLiquidityTax, uint8 buyMarketingTax) public onlyOwner{
        uint8 totalTax=buyLiquidityTax+buyFundTax+buyMarketingTax ;
        require(totalTax==10, "burn+liq+marketing needs to less then equal to 10");
        
        
        _buyFundTax = buyFundTax;
    ...
    function TeamSetSellTaxes(uint8 sellFundTax, uint8 sellLiquidityTax, uint8 sellMarketingTax) public onlyOwner{
        uint8 totalTax=sellLiquidityTax+sellFundTax+sellMarketingTax;
        require(totalTax<=15, "burn+liq+marketing3 needs to be less then equal to 15");
        
        _sellFundTax = sellFundTax;
    ...
    function TeamRemoveLiquidity() public onlyOwner{
        //Only callable if liquidity Unlock time is over
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
