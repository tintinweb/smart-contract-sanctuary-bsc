...
contract RefineryV1 is OwnableUpgradeable {
    ...
    IToken private hfuelToken; // TODO this will become Hfuel
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        (uint256 realizedDeposit, uint256 taxAmount) = hfuelToken.calculateTransferTaxes(_addr, _amount);
        ...
        */
        ...
        //todo check below for math errors 'refPayout'
        //_refPayout(_addr, realizedDeposit + taxedDivs, ref_bonus);
        _refPayout(_addr, realizedDeposit + taxedDivs);
    ...
    function _refPayout(address _addr, uint256 _amount) internal {
        ...
        for(uint8 i = 0; i < 15; i++) { //15 max depth
    ...
    function payoutOf(address _addr) public view returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
        ...
        if(users[_addr].payouts < max_payout) {
            ...
            //todo payoutRate below will be replaced by payOutRateOf()
    ...
    function airdrop(address _to, uint256 _amount) external {
        ...
        (uint256 _realizedAmount, uint256 taxAmount) = hfuelToken.calculateTransferTaxes(_addr, _amount);
