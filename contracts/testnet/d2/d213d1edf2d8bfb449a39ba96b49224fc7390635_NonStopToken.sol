...
contract NonStopTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("NonStopToken_Dividend_Tracker", "NonStopToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "NonStopToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "NonStopToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NonStopToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "NonStopToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "NonStopToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract NonStopToken is BEP20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    NonStopTokenDividendTracker public dividendTracker;
    uint256 public _totalSupply = 1000_000_000 * (10**18);
    uint256 public maxSellTransactionAmount = _totalSupply.div(100); //1% of total supply. 
    uint256 public swapTokensAtAmount = _totalSupply.div(100_000);
    ...
    public onlyOwner 
    {
        ...
        require(extraFeeOnSell<=7, "Too High Fee");
    }
    ...
    constructor() BEP20("NonStopToken", "NST") 
    {
        BNBRewardsFee = 3;
        liquidityFee = 5;
        marketingFee = 2;
        ...
        marketingWallet = payable(0x15569e95194530929d32Ef118198E661513B9400);
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new NonStopTokenDividendTracker();
    ...
    {
        require(newAddress != address(uniswapV2Router), "NonStopToken: The router already has that address");
    ...
    {
        require(_isExcludedFromFees[account] != excluded, "NonStopToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "NonStopToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NonStopToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "NonStopToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "NonStopToken: Cannot update gasForProcessing to same value");
