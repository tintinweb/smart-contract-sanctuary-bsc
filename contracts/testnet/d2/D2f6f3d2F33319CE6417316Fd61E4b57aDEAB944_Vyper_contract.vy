...
fee_unit: public(uint256)
total_fee_amount: uint256
...
@external
def setFeeUnit(_fee_unit: uint256):
    """
    @notice Set Fee Unit
    @param _fee_unit Set fee Unit
    """
    assert msg.sender == self.admin  # dev: admin only
    self.fee_unit = _fee_unit

@internal
def _checkpoint_token():
    token_balance: uint256 = ERC20(self.token).balanceOf(self)
    to_distribute: uint256 = token_balance - self.token_last_balance - self.total_fee_amount
    self.token_last_balance = token_balance - self.total_fee_amount
...
def claim(_addr: address = msg.sender) -> uint256:
    ...
    # take 1 TNODE transaction fee from every claim
    if amount > self.fee_unit and self.token_last_balance > amount:
        token: address = self.token
        self.token_last_balance -= amount
        amount -= self.fee_unit
        self.total_fee_amount += self.fee_unit
...
def claim_many(_receivers: address[20]) -> bool:
    ...
    for addr in _receivers:
        ...
        # take 1 TNODE transaction fee from every claim
        if amount > self.fee_unit:
            total += amount
            amount -= self.fee_unit
            total_fee += self.fee_unit
            assert ERC20(token).transfer(addr, amount)

    if total != 0:
        self.token_last_balance -= total
    if total_fee != 0:
        self.total_fee_amount += total_fee
    return True
...
@external
def withdraw_fee(_addr: address):
    """
    @notice Withdraw fee
    """
    assert msg.sender == self.admin
    ...
    token: address = self.token
    assert ERC20(token).transfer(_addr, self.total_fee_amount)
