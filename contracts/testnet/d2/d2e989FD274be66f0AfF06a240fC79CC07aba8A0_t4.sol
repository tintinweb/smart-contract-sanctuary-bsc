...
contract t4 is VRFConsumerBase,ReentrancyGuard {
	using SafeMath for uint256;
    ...
    address linkTrading = 0x84b9B910527Ad5C03A9Ca831909E21e236EA7b06;
    address krstmTrading = 0x8a9424745056Eb399FD19a0EC26A14316684e274;

    address internal krstmContract = 0xadF21D017d3aC316f84C7D50312072eDe9dFc21c;
    address internal btcContract = 0x7eeD9fab53670de3C367ab6Fa1758036C69c3ee8;
    address internal ethContract = 0x4FA8021584EE1c199be457F9d6D01d4E87b77d71;
    ...
    uint internal TICKET_VAL = 50000000000000000000;  // Amount (in wei) for getting registered on list
	...
	event LotteryLog(uint256 timestamp,address adrs, string message,uint number,string message2,uint choosennumber1,uint choosenNumber2,uint choosenNumber3);
    ...
    function swapTokensToUSD(uint256 tokenAmount) private {
        ...
        wmatic.approve(address(pancakeRouter), tokenAmount);
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override onlyVRFCoordinator {
        ...
        if(jackpotNumber == choosenNumber1 || jackpotNumber == choosenNumber2 || jackpotNumber == choosenNumber3) {
            ...
            emit LotteryLog(block.timestamp,lastJackpotWinner, "Jackpot is hit! Jackpot Number was: ",jackpotNumber,"Player choosen numbers was: ",choosenNumber1,choosenNumber2,choosenNumber3);
            ...
            jackpot = wmatic.balanceOf(address(this)) * 80/100;
            emit LotteryLog(block.timestamp,lastWinner, "We have a new raffle Winner but no jackpot! Jackpot Number was: ",jackpotNumber,"Player choosen numbers was: ",choosenNumber1,choosenNumber2,choosenNumber3);
        }
    }
    ...
    function autoSpin() public {
        ...
        if(getLatestPrice() > 10000000000){
         if(krstm.balanceOf(msg.sender) > 5000000000000000000){
          ...
          minimumPot = 60000000000000000000; // if MATIC price over 100 change to 60 MATIC minimum to auto spin
        } else if(getLatestPrice() > 1000000000){
         if(krstm.balanceOf(msg.sender) > 5000000000000000000){
          botReward = 4000000000000000000; // 4.00 usd stablecoin
         }else{
          botReward = 2200000000000000000; // 2.20 usd stablecoin
         }
          minimumPot = 600000000000000000000; // if MATIC price over 10 change to 600 MATIC minimum to auto spin
        }else{
         if(krstm.balanceOf(msg.sender) > 5000000000000000000){
          ...
          minimumPot = 6000000000000000000000; // 6k MATIC default
        }

        require(wmatic.balanceOf(address(this)) >= minimumPot && registeredsAccts.length > 5 && block.timestamp > lastAutoSpin + 1800,"autoSpin parameters not met");
    ...
    // Play Function
    function BT2(uint256 ticketValue,uint number1,uint number2, uint number3) public nonReentrant {
        ...
        Registered memory registered = Registered(0,number1,number2,number3,0,0);
        ...
        if(krstm.balanceOf(msg.sender) > 5000000000000000000 && msg.sender != derivativesContract ){
         usd.transferFrom(payable(msg.sender),address(this),TICKET_VAL.mul(70).div(100) - halfFee);
         swapTokens(wmaticTrading,bonusHolder,address(this));
        }else{
         usd.transferFrom(payable(msg.sender),address(this),TICKET_VAL - halfFee);
         swapTokens(wmaticTrading,jackpotFeed,address(this));
        }

        // fee for LP providers
        usd.transferFrom(payable(msg.sender),stakingContract,halfFee);
        ...
        emit LotteryLog(block.timestamp,msg.sender, "Ticket Bought, Good luck!",0,"Ticket Numbers :",number1,number2,number3);
    ...
    function ForfeitTicket(uint256 index) public nonReentrant {
       ...
       require(registereds[msg.sender].amountOfDraws < 3,"You already drawed 3 and can't forfeit.");     // Paranoic checking
        if(getLatestPrice() > 10000000000){
         minimumPot = 30000000000000000000; // if MATIC price over 100 change to 30 MATIC minimum
        } else if(getLatestPrice() > 1000000000){
         minimumPot = 300000000000000000000; // if MATIC price over 10 change to 300 MATIC minimum
        }else{
         minimumPot = 4000000000000000000000; // 4k MATIC default
        }
       require(wmatic.balanceOf(address(this)) >= minimumPot,"Contract has not enough MATIC to do a safe refund");             
        // Getting Registered user Info and Value
        uint totalDraws = registereds[msg.sender].amountOfDraws;
        uint256 refundValue;

        if(totalDraws == 0){
            refundValue = TICKET_VAL.mul(85).div(100);
