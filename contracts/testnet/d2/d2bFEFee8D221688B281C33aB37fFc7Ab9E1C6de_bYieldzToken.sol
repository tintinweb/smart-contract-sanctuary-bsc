...
contract bYieldzToken is ERC20, Ownable {
    ...
    uint256 public preWithdrawPercent;
    ...
    event Stake(address indexed account, uint256 amount, uint256 period);
    event Claim(address indexed account);
    event Withdraw(address indexed account);
 
    constructor(address _yieldToken) ERC20("bYieldz", "bYZ") {
        yzToken = _yieldToken;
        period = 6 hours;
    }
    ...
    * @param lockPeriod: the lock period as 2 weeks and 4 weeks
    */
    function stake(uint256 amount, uint256 lockPeriod) external  {
        StakeData storage data = userData[msg.sender];
        ...
        data.amount += amount;
        data.stakeTime = block.timestamp;
        data.lockTime = lockPeriod * 1 days  + block.timestamp;

        if (lockPeriod == 20) data.rate = rateForTwo;
        else data.rate = rateForFour;
    ...
    */
    function withdraw() external {
        ...
        uint256 byzAmount = getReward(msg.sender);
        uint256 yzAmount = data.amount;
        uint256 yzRemainAmount;

        if (block.timestamp < data.lockTime) {
            yzRemainAmount = yzAmount * (100 - preWithdrawPercent) / 100;
            yzAmount = yzAmount * preWithdrawPercent / 100;
        }
    ...
    */
    function getReward(address receiver) public view returns(uint256) {
        StakeData storage data = userData[receiver];

        uint256 reward;
        if (block.timestamp > data.lockTime) {
            reward = (data.lockTime - data.stakeTime) / period * data.amount * data.rate / 4000;
        } else {
            reward = (block.timestamp - data.stakeTime) / period * data.amount * data.rate / 4000;
        }
    ...
    // Get the preWithdrawPercent 
    function getPrewithdrawPercent() external view returns(uint256) {
        return preWithdrawPercent;
    }
   
    // Set the preWithdrawPercent by only owner
    function setPrewithdrawPercent(uint256 _newPercentage) external onlyOwner {
        preWithdrawPercent = _newPercentage;
    }
}
