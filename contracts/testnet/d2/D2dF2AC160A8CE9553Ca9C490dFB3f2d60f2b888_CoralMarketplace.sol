...
contract CoralMarketplace is Auth {
    ...
    uint256 public constant CRL_METHOD = 0;
    uint256 public constant MATIC_METHOD = 1;
    uint256 public constant USDC_METHOD = 2;
    ...
    uint256 public discountForPlatform;
    ...
    constructor(
        ...
        uint256 _discountForPlatform,
        uint256 _feeDenominator
    ) Auth(
        _msgSender()
    ) {
        usdc = USDCToken(_usdcAddress);
        crl = Coraltoken(_crlAddress);
        priceFeed = AggregatorV3Interface(_feedAddress);
        fitNft = FitNft(_nftAddress);
        setCounterStartValue(10);
        setFees(_marketingFee, _platformFee, _discountForPlatform, _feeDenominator);      
        setFeeReceivers(_marketingFeeReceiver, _platformFeeReceiver);  
    }
    ...
    function createNft(
        ...
        fitNft.mintNft(_msgSender(), _tokenID, _tokenCID);
    }
    ...
    function buyNft(
    ...
    ) public payable isUser(_msgSender()){
        require(
        ...
        if(_paymentMethod == CRL_METHOD){
            uint256 crlValue = nfts[_tokenID].fixedPrice.mul(feeDenominator - discountForPlatform).div(feeDenominator) / getCrlPrice();
            crl.transferFrom(_msgSender(), nfts[_tokenID].owner, crlValue);
            crl.transferFrom(_msgSender(), platformFeeReceiver, crlValue.mul(platformFee).div(feeDenominator));
        } else if(_paymentMethod == MATIC_METHOD){
            uint256 maticValue = nfts[_tokenID].fixedPrice / getMaticPriceOfUSD();
            require(
                msg.value >= maticValue.mul(feeDenominator + platformFee).div(feeDenominator),
                ""
            );

            (bool successToOwner, ) = payable(nfts[_tokenID].owner).call{value : maticValue}("");
            require(successToOwner, "Sending Matic to owner failed");

            (bool successToPlatform, ) = payable(platformFeeReceiver).call{value : maticValue.mul(platformFee).div(feeDenominator)}("");
            require(successToPlatform, "Sending Matic to platform to failed");

        } else if(_paymentMethod == USDC_METHOD){
            usdc.transferFrom(_msgSender(), nfts[_tokenID].owner, nfts[_tokenID].fixedPrice);
            usdc.transferFrom(_msgSender(), platformFeeReceiver, nfts[_tokenID].fixedPrice.mul(platformFee).div(feeDenominator));
    ...
    function rentNft(
        ...
        uint256 basicPrice = nfts[_tokenID].rentalPricePerDay * _rentingDays;

        if(_paymentMethod == CRL_METHOD){
            uint256 crlValue = basicPrice.mul(feeDenominator - discountForPlatform).div(feeDenominator) / getCrlPrice();
            crl.transferFrom(_msgSender(), nfts[_tokenID].owner, crlValue);
            crl.transferFrom(_msgSender(), platformFeeReceiver, crlValue.mul(platformFee).div(feeDenominator));
        } else if(_paymentMethod == MATIC_METHOD){
            uint256 maticValue = basicPrice / getMaticPriceOfUSD();
            require(
                msg.value >= maticValue.mul(feeDenominator + platformFee).div(feeDenominator),
                ""
            );
            ...
            usdc.transferFrom(_msgSender(), nfts[_tokenID].owner, basicPrice);
            usdc.transferFrom(_msgSender(), platformFeeReceiver, basicPrice.mul(platformFee).div(feeDenominator));
    ...
    function getMaticPriceOfUSD()internal view returns (uint256){
        (, int price, , , ) = priceFeed.latestRoundData();

        return uint256(price);
    }

    function getCrlPrice()internal pure returns (uint256){
        return 5*10**7;
    }
    ...
    function setFees(
        ...
        discountForPlatform = _discountForPlatform;
