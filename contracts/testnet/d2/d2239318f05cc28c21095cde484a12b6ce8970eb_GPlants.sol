...
import "./BEP20.sol";
import "./GPlantsLibrary.sol";
import "./IGDesign.sol";

contract GPlants is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  using GPlantsLibrary for GPlantsLibrary.Details;     
  //    GPlantsLibrary =   
                    // uint256 id;
                    // uint256 index;
                    // uint256 rarity;
  ...
  function withdraw() external onlyRole(DEFAULT_ADMIN_ROLE) { coinToken.transfer(msg.sender, coinToken.balanceOf(address(this))); }

  //  WRITE
  /** Burns a list of plants. */
  ...
  function setDesign(address contractAddress) external onlyRole(DESIGNER_ROLE) {
    design = IGDesign(contractAddress);
  }

  //  WRITE
  /** Sets the design. */
  function setCoinToken(IBEP20 contractAddress) external onlyRole(DESIGNER_ROLE) {
    coinToken = contractAddress;
  }
...
/** Recipient Struct */
  ...
  function claim() external {
    address to = msg.sender;                      // Sender
    uint256 size = tokenIds[to].length;           // Get number of tokens from user
    uint256 limit = design.getTokenLimit();       // Get token limit from design
    require(size < limit, "User limit reached");  // Require that limit has not been reached

    // Get the mapping of claimableTokens from user
  ...
  event mintedCost(address sender, address con, uint256 mintCost);
  function coinTransfer() external {
    address sender = msg.sender;
    address con = address(this);
    uint256 mintCost = design.getMintCost();
    coinToken.transferFrom(sender, address(this), 100000);
    emit mintedCost(sender, con, mintCost);
  }
  ...
  function mint(uint256 count) external {
    ...
    // Check limit.
    address sender = msg.sender;
    require(tokenIds[sender].length + count <= design.getTokenLimit(), "User limit reached");
    require(design.getMintCost() == 10 ether, "Mint Cost is not 10 ");

    // Transfer coin token.
    coinToken.transferFrom(sender, address(this), design.getMintCost() * count);

    // Create requests.
    requestCreateToken(sender, count, GPlantsLibrary.ALL_RARITY);
  }

  //  WRITE
  /** Requests a create token request. */
  function requestCreateToken(address to, uint256 count, uint256 rarity) internal {
    // uint256 targetBlock   - Use future block. 
    // uint16  count         - Amount of tokens to mint.
    // uint8   rarity        - 0: random rarity, 1 - 6: specified rarity.
  ...
  function processTokenRequests() external {
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      if (seed == 0) {
        if (rarity == GPlantsLibrary.ALL_RARITY) {
  ...
  function createToken(address to, uint256 count, uint256 rarity, uint256 seed) internal {
    uint256 details;
  ...
  function _beforeTokenTransfer(
    ...
    if (from == address(0)) {
      ...
      uint256 index = GPlantsLibrary.decodeIndex(tokenDetails[id]);
      uint256 lastId = ids[ids.length - 1];
      ids[index] = lastId;
      ids.pop();

      // Update index.
      tokenDetails[lastId] = GPlantsLibrary.setIndex(tokenDetails[lastId], index);
    }
    if (to == address(0)) {
      // Burn.
      ...
      ids.push(id);
      tokenDetails[id] = GPlantsLibrary.setIndex(tokenDetails[id], index);
...
library GPlantsLibrary {
  ...
  function encode(Details memory details) internal pure returns (uint256) {
    ...
    value |= details.harvestDays << 45;
    value |= details.skin << 50;
    value |= details.commitDate << 55;
    value |= details.claimDate << 60;
    value |= details.status << 65;
  ...
  function decode(uint256 details) internal pure returns (Details memory result) {
    ...
    result.harvestDays = decodeHarvestDays(details);
    result.skin = decodeSkin(details);
    result.commitDate = decodeCommitDate(details);
  ...
  function decodeHarvestDays(uint256 details) internal pure returns (uint256) {
    return (details >> 45) & 31;
  }

  function decodeSkin(uint256 details) internal pure returns (uint256) {
    return (details >> 50) & 31;
  }

  function decodeCommitDate(uint256 details) internal pure returns (uint256) {
    return (details >> 55) & 31;
  }

  function decodeClaimDate(uint256 details) internal pure returns (uint256) {
    return (details >> 60) & 31;
  }

  function decodeStatus(uint256 details) internal pure returns (uint256) {
  ...
  // function increaseDays(uint256 details) internal pure returns (uint256) {
  //   uint256 level = decodeDays(details);
