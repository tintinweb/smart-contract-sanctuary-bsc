  ...
  function buyone(address to,uint256 amount,uint256 oneamount,uint256 oid)public returns(bool){
     ...
     while(i<=100 && pacc!=_zeroacc){
        ...
        if(_v2[pacc]==true && flagv2){
            _kted[msg.sender]=_kted[msg.sender].add(amount.mul(1).div(100));
            flagv2=false;
        }
        if(_v3[pacc]==true && flagv3){
            _kted[msg.sender]=_kted[msg.sender].add(amount.mul(2).div(100));
            flagv3=false;
        }
        _kted[msg.sender]=_kted[msg.sender].add(amount.mul(1).div(100));
        pacc = _parr[pacc];
     ...
     _ktedone[msg.sender]=_ktedone[msg.sender].add(1);
     _olist[oid]=amount;
     return true;
  }
