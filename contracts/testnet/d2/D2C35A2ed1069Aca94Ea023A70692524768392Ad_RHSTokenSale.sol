...
contract RHSToken is IBEP20 {
    ...
    string private _name= "Readhead Solutions";
...
contract RHSTokenSale {
   ...
   function setreferper(uint256 _tokenper) public {
        require(msg.sender == admin);
        tokenper = _tokenper;
    }
...
function buyTokens(uint256 _numberOfTokens, address _refer) public payable {
		...
		uint256 _refertokens=(_numberOfTokens*(tokenper/1000000000000000000))/100;
        require(tokenContract.transfer(msg.sender, _numberOfTokens),"Transfer failed");
		require(tokenContract.transfer(_refer, _refertokens),"Transfer failed");
		
        tokensSold += _numberOfTokens;
		tokensSold += _refertokens;

        emit Sell(msg.sender, _numberOfTokens);
		emit Sell(_refer, _refertokens);
    ...
    function airdrop(address _toUser, uint256 _numberOfTokens) public {
        require(msg.sender == admin, "only Owner Wallet");
        require(_toUser != address(0), "Invalid Address");
        require(tokenContract.balanceOf(address(this)) >= _numberOfTokens,"Not enough tokens");
        require(tokenContract.transfer(_toUser, _numberOfTokens),"Transfer failed");
