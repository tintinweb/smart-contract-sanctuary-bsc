...
contract Box is Ownable{
    ...
    function openBox(uint32 _index)  public {
        ...
        (monsterId,_monster.ce,_monster.armor,_monster.luk,_monster.name) = _badHero.getMonsterType();
        
        tokenId = _game.createCard(_monster.ce,_monster.armor,_monster.luk,block.timestamp+_lockTime, nftKindId,_monster.name,_temNum);
...
contract Game is AccessControl,Ownable {
    ...
    address public IPancakeRouter01Address = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    address public usdtAddress;
    address public minerTokenAddress;
    IERC20 internal erc20;
    NFT internal _nft;
    IMonster private _monster;
    ...
    // mapping(uint256=>uint256) _earnings;  // 等级/收益
    ...
    // 生成卡片信息
    function createCard(uint256 ce,uint256 armor,uint256 luk,uint256 unLockTime,uint256 nftKindId,string memory name,uint256 maxNum) public onlyRole(MINTER_ROLE) returns(uint256){
        uint256 tokenId = _nft.safeMint(msg.sender);
        // uint256 unLockTime = _gameInfo.enlistTime*36000;
        //初始化卡片信息
    ...
    function adRecruit(uint256 tokenId)  public{
        // require(ownerOf(tokenId)!=msg.sender,"Have no legal power");
    ...
    function setIPancakeRouter(address _IPancakeRouter01Address) public onlyOwner{
        IPancakeRouter01Address = _IPancakeRouter01Address;
    }
    ...
    function setminerAddress(address minerToken) public onlyOwner{
        minerTokenAddress = minerToken;
    }
    ...
    function setMINTER(address upAddress)public onlyOwner{
    ...
    function speedFee(uint256 remainTime) view public returns(uint256){
        ...
        uint[] memory amounts = getRateByAddress(_gameInfo.speedMoney,IPancakeRouter01Address,usdtAddress,minerTokenAddress);
    ...
    function upLevel(uint256 _tokenId) public{
        ...
        //
        uint[] memory amounts = getRateByAddress(_tokenLevel[_tokenId][_tokenDetail[_tokenId].level],IPancakeRouter01Address,minerTokenAddress,minerTokenAddress);
    ...
    // 获取怪物等级下对应的收益
    // function incomByLevel(uint256 _tokenId,uint256 _level) internal view returns(uint256){
    //     // tokenEarnings[] memory earnings = _tokenLevel[_tokenId];
    //     // uint256 income;
    //     // for(uint256 i=0;i<earnings.length;i++){
    //     //     if(_level == earnings[i].level){
    //     //         income = earnings[i].income;
    //     //         break;
    //     //     }
    //     // }
    //     return _tokenLevel[_tokenId][_level];
    // }
...
modifier isteam(uint256 tokenId,address sender){
    ...
    // function delTask(uint256 id) public onlyOwner{
    //     for(uint256 i=0;i<specialTask.length;i++){
    //         if(specialTask[i].id == id){
    //            delete specialTask[i];
    //            break;
    //         }
    //     }
        
    // }
    // function editTask(uint256 id,uint256 odds,uint256 reward,uint256 xp,uint256 hp,string memory name) public onlyOwner{
    //     for(uint256 i=0;i<specialTask.length;i++){
    //         if(specialTask[i].id == id){
    //             specialTask[i].odds = odds;
    //             specialTask[i].basicReward = reward;
    //             specialTask[i].basicXp = xp;
    //             specialTask[i].basicHp = hp;
    //             specialTask[i].name = name;
    //             break;
    //         }
    //     }
    ...
    function getTokenDetail(uint256 tokenId) view public returns(uint256 ce,uint256 xp,uint256 armor,uint256 luk){
        return (_tokenDetail[tokenId].ce,_tokenDetail[tokenId].xp,_tokenDetail[tokenId].armor,_tokenDetail[tokenId].luk);
    }
    ...
    // 根据不同的地址获取不同的汇率
    function getRateByAddress(uint amountIn,address IPanAddress,address path0,address path1) view public returns(uint[] memory amounts){
        IPancakeRouter01 IPancakeRouter = IPancakeRouter01(IPanAddress);
        address[] memory path ;
        path[0]=path0;
        path[1]=path1;
        amounts = IPancakeRouter.getAmountsOut(amountIn, path);
        return amounts;
    }

    function getUsdtPrice(uint amountIn)  public view returns(uint,uint){
        uint[] memory amounts =getRateByAddress(amountIn,IPancakeRouter01Address,usdtAddress,minerTokenAddress);
...
contract BADHero is Ownable{
    ...
    function initStart()public onlyOwner{
        initEq();
        _setNftKind();
        _setMonsterInfo();
    }
 ...
 // 设置怪物NFT属性
    ...
    // 获取英雄装备信息
    function getHeroEq(address _addr) public  returns(heroAttribute[] memory){
        if (_userHero[_addr].length == 0){
            initHeroEq(_addr);
        }
