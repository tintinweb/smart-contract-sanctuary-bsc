...
contract MdaoCrowdManager is AccessControl {
    ...
    function chargeCrowdTxFee(address crowdAddr, uint256 amount) external {
        ...
        IERC20(USDT).safeTransferFrom(msg.sender, FEE, amount);
    ...
    function takeCrowdProfit(uint256 amount) external onlyRole(CROWD_ROLE) {
        if (profitRate > 0) {
            ...
            IERC20(USDT).safeTransferFrom(msg.sender, address(this), profit);
        }
    }
    ...
    function buyVip(address crowdAddr, uint8 vip) external {
        require(crowdSet.contains(crowdAddr), "crowd not exists");
        IERC20(USDT).safeTransferFrom(msg.sender, address(this), vipFee[vip] * 1e18);
