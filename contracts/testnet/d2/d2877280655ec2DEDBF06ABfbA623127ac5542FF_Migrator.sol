...
contract Migrator {
    ...
    GiftPack public newNftContract;
    ...
    constructor(
        ...
        newNftContract = GiftPack(_newNft);
    }

    function getData() public {
        require(
            msg.sender == oldNftContract.owner(),
            "Only owner can migrate Smart Contract data"
        );
        // Owner
    ...
    function setData() public {
        ...
        // Set Token
        newNftContract.setTokenCount(tokenCount);

        // Set Dna
        newNftContract.setDnaCount(dnaCount);
        newNftContract.setDnasKeys(dnasKeys);

        DnaBase.Dna[] memory dnasArr = new DnaBase.Dna[](dnaCount);
        for (uint i = 0; i < dnaCount; i++) {
            (uint id, uint dna, uint rarity, bool isOpened) = oldNftContract
                .dnas(dnasKeys[i]);
            dnasArr[i] = DnaBase.Dna(id, dna, rarity, isOpened);
        }
        newNftContract.setDnas(dnasKeys, dnasArr);

        // Set Zombie
        newNftContract.setZombieCount(zombieCount);
        newNftContract.setZombiesKeys(zombiesKeys);
    }

    // function setZombies() public {
    //     require(
    //         msg.sender == oldNftContract.owner(),
    //         "Only owner can migrate Smart Contract data"
    //     );

    //     ZombieBase.Zombie[] memory zombiesArr = new ZombieBase.Zombie[](
    //         zombieCount
    //     );
    //     for (uint i = 0; i < zombieCount; i++) {
    //         (
    ...
    //             uint exp
    //         ) = oldNftContract.zombies(zombiesKeys[i]);
    //         zombiesArr[i] = ZombieBase.Zombie(
    //             id,
    //             name,
    //             dna,
    //             level,
    ...
    //         );
    //     }
    //     newNftContract.setZombies(zombiesKeys, zombiesArr);
    // }



    function getOwner() public view returns (address) {
        return oldNftContract.owner();
    }

    function getTokenCount() public view returns (uint) {
        return oldNftContract.tokenCount();
    }

    function getDnaCount() public view returns (uint) {
        return oldNftContract.dnaCount();
    }

    function getDnaKeys() public view returns (uint[] memory) {
        uint oldDnaCount = oldNftContract.dnaCount();
        uint[] memory arr = new uint[](dnaCount);
        for (uint i = 0; i < oldDnaCount; i++) {
            arr[i] = oldNftContract.dnasKeys(i);
        }
        return arr;
    }

    function getDnas() public view {}

    function getZombieCount() public view returns (uint) {
        return oldNftContract.zombieCount();
    }

    function getZombieKeys() public view returns (uint[] memory) {
        uint oldZombieCount = oldNftContract.zombieCount();
        uint[] memory arr = new uint[](zombieCount);
        for (uint i = 0; i < oldZombieCount; i++) {
            arr[i] = oldNftContract.zombiesKeys(i);
        }
        return arr;
    }

    function getZombies() public view {}

    // ======================== Get ownerOf() of ERC-721 ========================
}
...
contract ZombieHelper is ZombieFeeding {
    ...
    // Setter functions for data migration
    function setTokenCount(uint _tokenCount) public {
        tokenCount = _tokenCount;
    }

    function setDnaCount(uint _dnaCount) public {
        dnaCount = _dnaCount;
    }

    function setZombieCount(uint _zombieCount) public {
        zombieCount = _zombieCount;
    }

    function setDnasKeys(uint[] memory _dnasKeys) public {
        dnasKeys = _dnasKeys;
    }

    function setZombiesKeys(uint[] memory _zombiesKeys) public {
        zombiesKeys = _zombiesKeys;
    }

    function setDnas(uint[] memory dnasKeys, Dna[] memory _dnas)
        public
    {
        for (uint i = 0; i < dnaCount; i++) {
            dnas[dnasKeys[i]] = _dnas[i];
        }
    }

    function setZombies(uint[] memory zombiesKeys, Zombie[] memory _zombies)
        public
    {
        for (uint i = 0; i < dnaCount; i++) {
            zombies[zombiesKeys[i]] = _zombies[i];
        }
    }
}
