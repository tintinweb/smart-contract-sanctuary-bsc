...
contract JewelToken is ERC20, Ownable, Authorizable {
    ...
    function _unlock(address holder, uint256 amount) internal {
        ...
        _burn(holder, amount);
        _moveDelegates(_delegates[holder], address(0), amount);
        maze.safeTransfer(holder, amount);
...
contract MasterGardener is Ownable, Authorizable, ReentrancyGuard {
    ...
    function getLockPercentage(uint256 _from, uint256 _to) public view returns (uint256) {
        uint256 result = 0;
        if (_from < START_BLOCK) return 1000;
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                // 12% fee: 4% burn, 5% to dev, 1.25% to lp, 1.25% to com, 0.5% to founder
                uint256 harvestFee = pending.mul(12).div(100);
                uint256 devFee = harvestFee.mul(500).div(1200);
                uint256 lpFee = harvestFee.mul(125).div(1200);
                uint256 comFee = harvestFee.mul(125).div(1200);
                uint256 founderFee = harvestFee.mul(50).div(1200);
                uint256 burnAmount = harvestFee.sub(devFee).sub(lpFee).sub(comFee).sub(founderFee);
                govToken.transfer(address(devaddr), devFee);
                govToken.transfer(address(liquidityaddr), lpFee);
                govToken.transfer(address(comfundaddr), comFee);
                govToken.transfer(address(founderaddr), founderFee);
                govToken.burn(burnAmount);

                uint256 harvestAmount = pending.sub(harvestFee);
                ...
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    ...
                    lockAmount = harvestAmount.mul(lockPercentage).div(1000);
                    govToken.transfer(msg.sender, lockAmount);
                    govToken.lock(msg.sender, lockAmount);

                    uint256 receivedAmount = harvestAmount.sub(lockAmount);
                    govToken.burn(receivedAmount);
                    maze.safeTransferFrom(mazeKeeper, msg.sender, receivedAmount);
                }

                // Reset the rewardDebtAtBlock to the current block for the user.
                user.rewardDebtAtBlock = block.number;

                emit SendGovernanceTokenReward(msg.sender, _pid, harvestAmount, lockAmount);
            }
    ...
    function withdrawFee(uint256 _pid, address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_pid][_user];
        uint256 blockdelta;
        if (user.lastWithdrawBlock > 0) {
        ...
        if (
            blockdelta == blockDeltaStartStage[0] ||
            block.number == user.lastDepositBlock
        ) {
            return uint256(10000).sub(userFeeStage[0].mul(100));
        } else if (
            blockdelta >= blockDeltaStartStage[1] &&
            blockdelta <= blockDeltaEndStage[0]
        ) {
            return uint256(10000).sub(userFeeStage[1].mul(100));
        } else if (
            blockdelta >= blockDeltaStartStage[2] &&
            blockdelta <= blockDeltaEndStage[1]
        ) {
            return uint256(10000).sub(userFeeStage[2].mul(100));
        } else if (
            blockdelta >= blockDeltaStartStage[3] &&
            blockdelta <= blockDeltaEndStage[2]
        ) {
            return uint256(10000).sub(userFeeStage[3].mul(100));
        } else if (
            blockdelta >= blockDeltaStartStage[4] &&
            blockdelta <= blockDeltaEndStage[3]
        ) {
            return uint256(10000).sub(userFeeStage[4].mul(100));
        } else if (
            blockdelta >= blockDeltaStartStage[5] &&
            blockdelta <= blockDeltaEndStage[4]
        ) {
            return uint256(10000).sub(userFeeStage[5].mul(10));
        } else if (
            blockdelta >= blockDeltaStartStage[6] &&
            blockdelta <= blockDeltaEndStage[5]
        ) {
            return uint256(10000).sub(userFeeStage[6].mul(1));
        } else if (blockdelta > blockDeltaStartStage[7]) {
            return uint256(10000).sub(userFeeStage[7].mul(1));
        }
        return 0;
    }
    ...
    function BPR(uint256 _pid) public view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        ...
        uint256 multiplier = getMultiplier(block.number - 1, block.number);
        return multiplier.mul(REWARD_PER_BLOCK).mul(pool.allocPoint).mul(1e12).div(totalAllocPoint).div(lpSupply);
    }
    ...
    function setMazeKeeper(address _mazeKeeper) public onlyAuthorized() {
        mazeKeeper = _mazeKeeper;
    }
}