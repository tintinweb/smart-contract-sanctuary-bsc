 ...
 */
contract ArmySpecialV2 is Ownable {
    ...
    uint8 public constant previousNumberArmyIds = 12;

    uint256 public endBlock;
    uint256 public thresholdUser;

    // Map if armyId to its tokenURI
    mapping(uint8 => string) public armyTokenURI;

    // Map if address has already claimed a NFT
    mapping(address => bool) public hasClaimed;

    // Map teamId to its armyId
    mapping(uint256 => uint8) public teamIdToArmyId;

    event ArmyAdd(uint8 armyId, uint256 teamId);
    ...
    function mintNFT() external {
        ...
        (userId, , userTeamId, , , isUserActive) = dutyProfile.getUserProfile(senderAddress);
        ...
        uint8 armyId = teamIdToArmyId[userTeamId];

        require(armyId >= previousNumberArmyIds, "NOT_VALID");

        string memory tokenURI = armyTokenURI[armyId];

        uint256 tokenId = armyMintingStation.mintCollectible(senderAddress, tokenURI, armyId);

        emit ArmyMint(senderAddress, tokenId, armyId);
    }

    /**
     * @notice Add/modify armyId for a teamId and metadata
    ...
    function addArmy(
        ...
        teamIdToArmyId[_teamId] = _armyId;
        armyTokenURI[_armyId] = _tokenURI;

        emit ArmyAdd(_armyId, _teamId);
    }
    ...
    function canClaim(address _userAddress) external view returns (bool) {
        if (hasClaimed[_userAddress]) {
            ...
            if (!dutyProfile.getUserStatus(_userAddress)) {
                ...
                uint256 userId;
                (userId, , , , , ) = dutyProfile.getUserProfile(_userAddress);
