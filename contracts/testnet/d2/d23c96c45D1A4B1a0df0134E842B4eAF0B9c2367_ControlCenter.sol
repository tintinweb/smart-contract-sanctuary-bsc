...
interface ICrossPair {
    ...
    function UpdateCRSSPriceCheckSettings(bool _isActive0, bool _isActive1)
        external;
...
contract ControlCenter is Ownable, IControlCenter {
    ...
    function _updateSession() external {
        if (currentSessionTimestamp + sessionInterval <= block.timestamp) {
            currentSessionTimestamp = block.timestamp;
        }
    }
    ...
    function updatePairPriceCheckStatus(
        ...
        ICrossPair(_pair).UpdateCRSSPriceCheckSettings(_isActive0, _isActive1);
    }
