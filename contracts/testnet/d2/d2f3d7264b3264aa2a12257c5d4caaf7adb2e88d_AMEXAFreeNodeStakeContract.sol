 ...
 */
contract AMEXAFreeNodeStakeContract is Ownable {
   ...
   struct DepositInfo {
	...
	uint256 public bonusPerReferral;
    ...
    function getBlockInterval(uint256 _to, uint256 _from) public pure returns (uint256) {
    ...
    function pendingReward(uint256 _pid, address _user) public view returns (uint256 totalPending) {
        ...
        for (uint256 i = 0; i < depositInfo.length; i++) {
            if (depositInfo[i].lpToken == pool.lpToken && depositInfo[i].user == _user) {
                ...
                uint256 userClaimAmountPerBlock=  poolClaimAmountPerDay.mul(depositInfo[i].amount).div(pool.lpToken.balanceOf(address(this))).div(28800);
    ...
    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 referralBonus){
		if(inviteContract.getMyReferrCount(msg.sender) > 0) {
			referralBonus = pendingReward(_pid,_user).mul(bonusPerReferral).div(100).mul(inviteContract.getMyReferrCount(msg.sender));
		}
        return referralBonus;
    }

    function deposite(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function claim(uint256 _pid) public {
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.totalDepositAmount > 0,"claim: user amount must be greater than 0");
    ...
    function withdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.totalDepositAmount > 0,"withdraw: user amount must be greater than 0");
        uint256 pending = pendingReward(_pid,msg.sender).add(myReferralReward(_pid,msg.sender));
