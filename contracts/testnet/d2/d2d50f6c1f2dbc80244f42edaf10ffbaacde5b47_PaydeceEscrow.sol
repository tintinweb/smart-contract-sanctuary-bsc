...
contract PaydeceEscrow is  ReentrancyGuard , Ownable  {
   ...
   function getState(uint _orderId) public view returns (EscrowStatus){
    ...
    function  createEscrowNativeCoin(uint _orderId, address payable _seller, uint256 _value) payable external virtual   {
        require(escrows[_orderId].status == EscrowStatus.Unknown, "Escrow already exists");
        ...
        escrows[_orderId] = Escrow(payable(msg.sender), _seller, _value, feeSeller, feeBuyer, IERC20(address(0)),EscrowStatus.Funded);
    ...
    function releaseEscrowNativeCoin(uint _orderId) external onlyBuyer(_orderId) {
        _releaseEscrowNativeCoin(_orderId);         
    }
    ...
    function _releaseEscrowNativeCoin (uint _orderId) private nonReentrant{
        ...
        escrows[_orderId].seller.transfer(escrows[_orderId].value - _amountFeeSeller);
    ...
    function refundBuyerNativeCoin(uint _orderId) external nonReentrant onlyOwner {       
        //require(escrows[_orderId].status == EscrowStatus.Refund,"Refund not approved");        
                
        // dont charge seller any fees - because its a refund
        delete escrows[_orderId];

        escrows[_orderId].buyer.transfer(escrows[_orderId].value);
    ...
    */
    ...
    function cancelEscrowNativeCoin(uint _orderId) external nonReentrant onlyOwner{
        require(escrows[_orderId].status == EscrowStatus.Funded,"Refund not approved"); 
