...
contract DividendBNBKillBlock is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(string memory _name, string memory _symbol, uint256 _supply, uint256[3] memory _buyFees, uint256[3] memory _sellFees, address[3] memory _addresses, uint256 _minimumTokenBalanceForDividends, uint256 _coolBlock, address serviceFeeReceiver_, uint256 serviceFee_) payable ERC20(_name, _symbol) {
        buyback = _buyFees[0];
        ...
        dividendTracker = new CoinDividendTracker(_minimumTokenBalanceForDividends);
    ...
    function setCoolBlock(uint256 _coolBlock) public onlyOwner {
        coolBlock = _coolBlock;
    }
    ...
    function _transfer(
        ...
        if (to == pancakePair && balanceOf(pancakePair) == 0) {
            require(canTransferBeforeTradingIsEnabled[from], "This address is not allowed to add liquidity for the first tim!");
            genesisBlock = block.number;
        }

        if (from == pancakePair && block.number < genesisBlock + coolBlock) {
            killBlock[to] = true;
        }
...
contract CoinDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(uint256 _minimumTokenBalanceForDividends) DividendPayingToken("CoinDividendTracker", "Coin_DividendTracker") {
        claimWait = 3600;
