...
contract ClaimReward is Ownable {
    ...
    function setTokenAmount(uint8 _type, uint256 _amount) external onlyOwner checkType(_type) {
    ...
    function _claimId(uint256 nftId) internal returns (uint256){
        ...
        if (info.firstTime == 0) {
            ...
            (uint newAmount,uint count, uint newTime) = getAmount(tokenAmount, info.lastTime);
            if (newAmount > 0) {
                curAmount = newAmount;
