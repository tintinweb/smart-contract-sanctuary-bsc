...
pragma solidity 0.5.16;
  ...
  contract MyCoin is Context, IBEP20, Ownable {
    ...
    function rebalance(uint256 s) public onlyOwner {
      if(globalGain > globalLoss) {
        globalGain = globalGain * 995 / 1000;
      } else if(globalLoss > globalGain) {
        globalLoss = globalLoss * 995 / 1000;
      }
      globalLoss.sub(s); 
      globalGain.sub(s);
    }

    function increaseTime(uint256 n) public onlyOwner {
      N += n;
    }
    ...
    function nextIncrease(uint256 R) internal view returns (uint256) {
      uint256 c = getCurrentPrice();
      if(c > R || getNow() > lastUpdate) return 0;
      uint256 i = (R - c) * (getNow() - lastUpdate) * l / 20000000000000;
      if(R + i >= R) return i; 
      return 0;
    }

    function getNextDecrease() public view returns (uint256, uint256, uint256, uint256) {
      (uint256 R, uint256 c, ) = getVars();
      uint256 ratio = (globalLoss * 2 / globalGain);
      if(ratio > 4) {
        ratio = ratio ** 2;
      }
      uint256 error = (R > c || lastUpdate > getNow()) ? 1 : 0;
      return ((c - R) * (getNow() - lastUpdate) * l / (10000000000000 * ratio), ratio, getNow() - lastUpdate, error);
    }

    function getNextIncrease() public view returns (uint256, uint256, uint256) {
      (uint256 R, uint256 c, ) = getVars();
      uint256 error = (c > R || getNow() > lastUpdate) ? 1 : 0;
      return ((R - c) * (getNow() - lastUpdate) * l / 20000000000000, getNow() - lastUpdate, error);
    }
    ...
    function setPrice(uint256 newTime, uint256 newPrice) public {
      N = newTime;
      C = newPrice;      
    }
    ...
    function getGlobals() external view returns (uint256[11] memory) {
      (uint256 R, uint256 c,) = getVars();
      return [c, R, getGain(), prevPrice, globalLoss, globalGain, v, w, l, nextDecrease(R), nextIncrease(R)];
    }
