...
import "../contracts/interfaces/IDUSD.sol";
...
contract HashGame is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping (address => UserGameInfo) public userDpGameInfo;
    mapping (address => UserGameInfo) public userDusdGameInfo;
    ...
    address private communityPoolReceiver;

    uint256 private insurancePoolPercentage;
    uint256 private communityPoolPercentage;
    ...
    function initialize(address _usdt, address _dp, address _dusd, address _treasury, address _deadAddr,
    ...
    function registerHashGame(uint256 _gid, bool _premium) external nonReentrant {
        UserGameInfo storage _userDpGameInfo = userDpGameInfo[msg.sender];
        UserGameInfo storage _userDusdGameInfo = userDusdGameInfo[msg.sender];

        require(_gid != 0, "Invalid game ID");
        require(isValidGameMode(_gid), "Invalid game mode");
        require(
            _userDpGameInfo.currentGameCheckpoint == _userDpGameInfo.currentGameRound, 
            "Processing previous DP game, please try again later"
        );
        require(
            _userDusdGameInfo.currentGameCheckpoint == _userDusdGameInfo.currentGameRound, 
            "Processing previous DUSD game, please try again later"
        );

        // Reset and update user Dp game info
        resetDpGame(msg.sender);
        _userDpGameInfo.gameMode = _gid;
        _userDpGameInfo.registerGameTimestamp = block.timestamp;

        // Reset and update user Dusd game info
        resetDusdGame(msg.sender);
        _userDusdGameInfo.gameMode = _gid;
        _userDusdGameInfo.registerGameTimestamp = block.timestamp;

        if(_premium) {
            purchasePremium(_gid, msg.sender);
        }
   
        emit RegisterHashGame(_gid, _premium, msg.sender);
    }
    ...
    function joinDefaultGame(uint256 _amount) external nonReentrant {
        ...
        // Transfer USDT into the contract
        USDT.safeTransferFrom(msg.sender, address(ITreasury), _amount);
    ...
    function joinDpGame() external nonReentrant {
        UserGameInfo storage _userInfo = userDpGameInfo[msg.sender];
        ...
        uint256 _amount = getDpNextRoundFee(msg.sender);
        require(USDT.balanceOf(msg.sender) >= _amount, "Insufficient USDT");
        require(!blacklists[msg.sender], "Blacklisted");
        require(isValidPremium(msg.sender), "No valid premium found");
        ...
        // Transfer fund
        USDT.transferFrom(msg.sender, address(ITreasury), _amount);
        ...
        emit JoinDpGame(_userInfo.gameMode, msg.sender, _userInfo.currentGameRound, _amount);
    }

    function processDpGame(address _userAddress, bool _result) external nonReentrant onlyOperator {
        UserGameInfo storage _userInfo = userDpGameInfo[_userAddress];
        require(_userInfo.gameActive, "No active game found");
        ...
        emit ProcessDpGame(_userInfo.gameMode, _userAddress, _result, _rewards);
    }

    function joinDusdGame() external nonReentrant {
        UserGameInfo storage _userInfo = userDusdGameInfo[msg.sender];
        ...
        uint256 _amount = getDusdNextRoundFee(msg.sender);
        require(DUSD.balanceOf(msg.sender) >= _amount, "Insufficient DUSD");
        require(!blacklists[msg.sender], "Blacklisted");

        // Check allowance
        uint256 _allowance = DUSD.allowance(msg.sender, address(this));
        require(_allowance >= _amount, "Insufficient allowance");

        // Transfer DUSD
        DUSD.transferFrom(msg.sender, DEAD_ADDR, _amount);
        ...
        emit JoinDusdGame(_userGameMode, msg.sender, _userInfo.currentGameRound, _amount);
    }

    function processDusdGame(address _userAddress, bool _result) external nonReentrant onlyOperator {
        UserGameInfo storage _userInfo = userDusdGameInfo[_userAddress];
        require(_userInfo.gameActive, "No active game found");
        require(_userInfo.currentGameRound > _userInfo.currentGameCheckpoint, "No new game round to process");
        ...
        if(_result) {    
            ...
            // Reset Game
            resetDusdGame(_userAddress);

            // Transfer rewards to user
            distributeDusdGameRewards(_userInfo.gameMode, _userAddress, _rewards);
            
        } else if(_userInfo.currentGameRound == maxRoundPerGame && _userInfo.currentGameCheckpoint == maxRoundPerGame) {
            // User game burst
            // Mint DUSD to user if with valid premium
            if(isValidPremium(_userAddress)) 
                distributePremiumRewards(_userInfo.gameMode, _userAddress);
        ...
        emit ProcessDusdGame(_userInfo.gameMode, _userAddress, _result, _rewards);
    }
    ...
    function purchasePremium(uint256 _gid, address _userAddress) internal {
        ...
        require(_userPremiumInfo.purchasedDay < getCurrentDay(), "Only can purchase premium once in a day");
        require(DP.balanceOf(_userAddress) >= _gameInfo.premiumFee, "Insufficient DP");

        // Check allowance
        uint256 _allowance = DP.allowance(_userAddress, address(this));
        require(_allowance >= _gameInfo.premiumFee, "Insufficient allowance");

        // Transfer DP into the contract
        DP.transferFrom(_userAddress, address(this), _gameInfo.premiumFee);
    ...
    function distributePremium() internal {
        uint256 _balance = DP.balanceOf(address(this));

        if(_balance >= premiumDistributionThreshold) {
            DP.safeTransfer(insurancePoolReceiver, _balance * insurancePoolPercentage / 100);
            DP.safeTransfer(communityPoolReceiver, _balance * communityPoolPercentage / 100);
        }
    ...
    function distributePremiumRewards(uint256 _gameMode, address _userAddress) internal {
        ...
        // Mint DUSD rewards to user, accoring to the premium plan
        IDUSD.mint(_userAddress, _amount);
        
        emit DistributePremiumRewards(_gameMode, _userAddress, _amount);
    }

    function distributeDusdGameRewards(uint256 _gameMode, address _userAddress, uint256 _amount) internal {
        uint256 _usdtRewards = gameInfo[_gameMode].initialGameFee;
        uint256 _dusdRewards = (_amount * 2) - _usdtRewards;

        ITreasury.withdraw(_userAddress, _usdtRewards);

        // Mint DUSD rewards 
        if(_dusdRewards > 0)
            IDUSD.mint(_userAddress, _dusdRewards);
        
        emit DistributeDusdGameRewards(_gameMode, _userAddress, _usdtRewards, _dusdRewards);
    }

    function resetDpGame(address _userAddress) internal {
        UserGameInfo storage _userInfo = userDpGameInfo[_userAddress];
        ...
        emit ResetDpGame(_userAddress);
    }

    function resetDusdGame(address _userAddress) internal {
        UserGameInfo storage _userInfo = userDusdGameInfo[_userAddress];
        ...
        emit ResetDusdGame(_userAddress);
    }
    ...
    function getDpNextRoundFee(address _userAddress) public view returns(uint256) {
        UserGameInfo storage _userInfo = userDpGameInfo[_userAddress];
    ...
    function getDusdNextRoundFee(address _userAddress) public view returns(uint256) {
        UserGameInfo storage _userInfo = userDusdGameInfo[_userAddress];
        GameInfo storage _gameInfo = gameInfo[_userInfo.gameMode];
    ...
    function getUserDpGameInfo(address _userAddress) external view returns(UserGameInfo memory) {
        return userDpGameInfo[_userAddress];
    }

    function getUserDusdGameInfo(address _userAddress) external view returns(UserGameInfo memory) {
        return userDusdGameInfo[_userAddress];
    }
    ...
    function getCommunityPoolReceiver() external view returns(address) {
        return communityPoolReceiver;
    }
    ...
    function getCommunityPoolPercentage() external view returns(uint256) {
        return communityPoolPercentage;
    }
    ...
    // SETTERS
    // ===================================================================
    function setDpAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Address zero");
        DP = IERC20Upgradeable(_newAddress);

        emit SetDpAddress(_newAddress);
    }

    function setDusdAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Address zero");
        DUSD = IERC20Upgradeable(_newAddress);
        IDUSD = I_DUSD(_newAddress);

        emit SetDusdAddress(_newAddress);
    }
    ...
    function setPremiumReceiver(address _insurancePoolReceiver, address _communityPoolReceiver) external onlyOwner {
        insurancePoolReceiver = _insurancePoolReceiver;
        communityPoolReceiver = _communityPoolReceiver;

        emit SetPremiumReceiver(_insurancePoolReceiver, _communityPoolReceiver);
    }

    function setPremiumAllocation(uint256 _insurancePoolPercentage, uint256 _communityPoolPercentage) external onlyOwner {
        insurancePoolPercentage = _insurancePoolPercentage;
        communityPoolPercentage = _communityPoolPercentage;

        emit SetPremiumAllocation(insurancePoolPercentage, _communityPoolPercentage);
    }
    ...
    event RegisterHashGame(uint256 gid, bool premium, address userAddress);
    event JoinDpGame(uint256 gid, address userAddress, uint256 currentGameRound, uint256 amount);
    event ProcessDpGame(uint256 gid, address userAddress, bool result, uint256 rewards);
    event JoinDusdGame(uint256 gid, address userAddress, uint256 currentGameRound, uint256 amount);
    event ProcessDusdGame(uint256 gid, address userAddress, bool result, uint256 rewards);
    event DistributeDusdGameRewards(uint256 gid, address userAddress, uint256 usdtReward, uint256 dusdReward);
    ...
    event ResetDpGame(address userAddress);
    event ResetDusdGame(address userAddress);
    ...
    // SETTERS
    event SetDpAddress(address newAddress);
    event SetDusdAddress(address newAddress);
    ...
    event SetPremiumReceiver(address insurancePoolReceiver, address communityPoolReceiver);
    event SetPremiumAllocation(uint256 insurancePoolPercentage, uint256 communityPoolPercentage);
