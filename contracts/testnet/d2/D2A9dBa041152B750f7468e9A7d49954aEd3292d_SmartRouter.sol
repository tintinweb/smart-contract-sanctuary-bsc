...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getPoolAddress(
        ...
        pool = IPancakeV3Pool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
...
abstract contract PeripheryPaymentsExtended is IPeripheryPaymentsExtended, PeripheryPayments {
    ...
    function unwrapWETH9(uint256 amount) external payable override {
    ...
    function refund() public payable override {
        if (address(this).balance > 0) TransferHelper.safeTransferETH(msg.sender, address(this).balance);
    }
}
...
interface IPeripheryPaymentsExtended is IPeripheryPayments {
    function unwrapWETH9(uint256 amount) external payable;
