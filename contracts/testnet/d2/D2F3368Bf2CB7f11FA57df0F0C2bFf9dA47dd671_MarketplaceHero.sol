...
contract MarketplaceHero is Pausable, Ownable, ERC721Holder, IBidNFT {
    ...
    function getTokenSellOf(address _address)
        public
        view
    ...
    function buyToken(uint256 _tokenId) public override whenNotPaused {
        ...
        _removeTokenSellOf(_seller, _tokenId);
    ...
    function cancelToSellToken(uint256 _tokenId) public override whenNotPaused {
        ...
        nft.safeTransferFrom(address(this), _msgSender(), _tokenId);
        _removeTokenSellOf(_msgSender(), _tokenId);
        _deleteSell(_tokenId);
        emit CancelSellToken(_tokenId);
    }

    function _removeTokenSellOf(address _address, uint256 _tokenId) private {
        uint256 index;
        bool exits = false;
        for (uint256 i = 0; i < _tokenSell[address(_address)].length; i++) {
            if (_tokenSell[address(_address)][i] == _tokenId) {
                index = i;
                exits = true;
            }
        }
        ...
        if (exits) {
            for (
                uint256 i = index;
                i < _tokenSell[address(_address)].length - 1;
                i++
            ) {
                _tokenSell[address(_address)][i] = _tokenSell[address(_address)][
                    i + 1
                ];
            }
            _tokenSell[address(_address)].pop();
        }
    }
