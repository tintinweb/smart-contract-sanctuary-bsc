...
contract RewardManager is RewardAuth, PausableUpgradeable {
  ...
  event Claimed(address indexed _user, uint _totalDistributed, uint _totalFotaDistributed, uint _fotaAmount, uint _usdAmount, uint _fotaPrice, uint _dayCallClaimed);
  event ReferralShare(address indexed _inviterOrTreasury, address _user, uint _fotaAmount, uint _usdAmount, uint _fotaPrice);
...
//    secondInADay = 86400; // 24 * 60 * 60
  ...
  function claim() public whenNotPaused {
    ...
    for (uint i = 0; i < rewards[msg.sender][dayToClaim].length; i++) {
      if (distributedToUser < userMaxReward) {
        if (distributedToUser.add(rewards[msg.sender][dayToClaim][i].userAmount) >= userMaxReward) {
          ...
          _increaseHeroTotalFOTAProfited(rewards[msg.sender][dayToClaim][i], userMaxReward.sub(distributedToUser));
        } else {
          distributedToUser = distributedToUser.add(rewards[msg.sender][dayToClaim][i].userAmount);
          _increaseHeroTotalFOTAProfited(rewards[msg.sender][dayToClaim][i], rewards[msg.sender][dayToClaim][i].userAmount);
        }
      }
      ...
      gameMiningPool.releaseGameAllocation(address(this), landLordTokenAmount);
      landLordManager.giveReward(rewards[msg.sender][dayToClaim][i].mission, landLordTokenAmount);
      totalDistributed += distributedToUser + distributedToFarm + distributedToReferralOrTreasury + landLordTokenAmount;
    }
    ...
    if (distributedToReferralOrTreasury > 0) {
      ...
      emit ReferralShare(inviterOrTreasury, msg.sender, _convertUsdToFota(distributedToReferralOrTreasury), distributedToReferralOrTreasury, fotaPricer.fotaPrice());
    }

    userDailyClaimed[msg.sender][dayPassed] = distributedToUser;
    dailyClaimed[dayPassed] += distributedToUser;
    uint toUserTokenAmount = _convertUsdToFota(distributedToUser);
    gameMiningPool.releaseGameAllocation(msg.sender, toUserTokenAmount);
    ...
    emit Claimed(msg.sender, totalDistributed, _convertUsdToFota(totalDistributed), toUserTokenAmount, distributedToUser, fotaPricer.fotaPrice(), getDaysPassed());
  }
  ...
  function _increaseHeroTotalFOTAProfited(Reward storage _reward, uint _amount) private {
    uint profitShared = _convertUsdToFota(_amount) / _reward.heroIds.length;
    for(uint i = 0; i < _reward.heroIds.length; i++) {
      heroNft.increaseTotalFOTAProfited(_reward.heroIds[i], profitShared);
    }
  }
