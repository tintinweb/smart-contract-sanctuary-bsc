...
contract Referral is ReferralFetchers, Ownable {
    ...
    function getInsuranceCount() external view returns (uint256) {
        ...
        return noOfActiveInsuranceInvestment;
    }

    function getRegularCount() external view returns (uint256) {
        uint256[] memory noOfRegularInvestmentsArray = regularInvestmentContract
            .getNoOfInvestmentsArray(msg.sender);
        uint256 noOfActiveRegularInvestment = noOfRegularInvestmentsArray[1];
        return noOfActiveRegularInvestment;
    }
    ...
    function claimReward(address friendAddress) external {
        ...
        // Last time
            ...
            // uint256 newClaimCount = claimCount + 1;
            // uint256 newRewardCount = rewardCount + 1;
            ...
            //     newClaimCount = claimCount + 2;
            //     newRewardCount = rewardCount + 2;
            // }

            // uint newClaimCount = rewardCount == 1 ? claimCount + 2 : claimCount + 1;
            // uint newRewardCount = rewardCount == 1 ?  rewardCount + 2 : rewardCount + 1;
            uint newRewardPercentage = rewardCount == 1 ? 100 - percentageSumUp : rewardPercentage;
            ...
            newReward.totalRewardAmount = totalRewardAmount;
            newReward.rewardClaimed = newRewardClaimed;
            newReward.rewardCount = rewardCount == 1 ?  rewardCount + 2 : rewardCount + 1;
            newReward.lastClaimingDay = lastClaimingDay;
            newReward.percentageSumUp = newPercentageSumUp;
            newReward.rewardDays = rewardDaysArray;
            newReward.rewardPercentages = rewardPercentagesArray;
            newReward.claimCount = rewardCount == 1 ? claimCount + 2 : claimCount + 1;
        } else {
