...
contract ISHOES is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable {
    ...
    event shoesMinted(uint256 indexed tokenId, address caller, address toAddress, uint256 boxType, address marketplaceAddress);

    constructor(address _marketplaceAddress) ERC721("ISTEP Shoes Nft", "ISHOES") {
    ...
    function setBaseURI(string memory baseTokenURI) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ISHOES admin role required");
    ...
    function mint(address toAddress, uint256 boxType) external returns (uint256) {
        require(hasRole(MINTER_ROLE, _msgSender()), "ISHOES minter role required");
        require(toAddress != address(0), "ISHOES cann't mint zero address");
        require(boxType > 0, "ISHOES boxType must greater than 0");

        uint256 newItemId = _tokenIdTracker.current();

        _mint(toAddress, _tokenIdTracker.current());

        _creators[newItemId] = tx.origin;
        ...
        emit shoesMinted(newItemId, _msgSender(), toAddress, boxType, marketplaceAddress);
        _tokenIdTracker.increment();
