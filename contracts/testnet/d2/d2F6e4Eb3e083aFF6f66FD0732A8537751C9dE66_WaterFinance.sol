...
contract WaterFinance is ERC20Detailed, Ownable, MinterRole {
    ...
    constructor() ERC20Detailed() {
        ...
        BUSD = IERC20(0x7cf8737084Bc90086c680F45041B3dB89dd6512B);

        router = IDEXRouter(0x1Ed675D5e63314B760162A3D1Cae1803DCFC87C7); //Sushi 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506 // Cake 0x10ED43C718714eb63d5aA57B78B54704E256024E

        pair = IDEXFactory(router.factory()).createPair(
            0x7cf8737084Bc90086c680F45041B3dB89dd6512B,
            address(this)
        );

        TreasuryReceiver = 0x84629e3A9E21376B76141e28bF34E4f5774Af86D;
        ...
        returns (uint256)
    {
        ...
        uint256 amountETHBuyTreasury = feeAmount.mul(BuyTreasuryFee).div(_totalFee);
        uint256 amountETHBuyMarketing = feeAmount.mul(BuyMarketingFee).div(_totalFee);
        uint256 amountETHBuyDev = feeAmount.mul(BuyDevFee).div(_totalFee);

        _gonBalances[TreasuryReceiver] = _gonBalances[TreasuryReceiver].add(amountETHBuyTreasury);
        _gonBalances[MarketingReceiver] = _gonBalances[TreasuryReceiver].add(amountETHBuyMarketing);
        _gonBalances[DevReceiver] = _gonBalances[TreasuryReceiver].add(amountETHBuyDev);
        
        emit Transfer(sender, TreasuryReceiver, amountETHBuyTreasury.div(_gonsPerFragment));
        emit Transfer(sender, MarketingReceiver, amountETHBuyMarketing.div(_gonsPerFragment));
        emit Transfer(sender, DevReceiver, amountETHBuyDev.div(_gonsPerFragment));
        ...
        returns (uint256)
    {
        ...
        uint256 amountETHSellTreasury = feeAmount.mul(SellTreasuryFee).div(_totalFee);
        uint256 amountETHSellMarketing = feeAmount.mul(SellMarketingFee).div(_totalFee);
        uint256 amountETHSellDev = feeAmount.mul(SellDevFee).div(_totalFee);

        _gonBalances[TreasuryReceiver] = _gonBalances[TreasuryReceiver].add(amountETHSellTreasury);
        _gonBalances[MarketingReceiver] = _gonBalances[TreasuryReceiver].add(amountETHSellMarketing);
        _gonBalances[DevReceiver] = _gonBalances[TreasuryReceiver].add(amountETHSellDev);
        
        emit Transfer(sender, TreasuryReceiver, amountETHSellTreasury.div(_gonsPerFragment));
        emit Transfer(sender, MarketingReceiver, amountETHSellMarketing.div(_gonsPerFragment));
        emit Transfer(sender, DevReceiver, amountETHSellDev.div(_gonsPerFragment));
