...
import "./interface/ITokieNFTFactoryUpgradeable.sol";
import "./interface/ITokieNFTUpgradeable.sol";
...
contract GeneralNFTRewardUpgradeable is IPool,GovernanceUpgradeable {
    ...
    ITokieNFTFactoryUpgradeable public _gegoFactory;
    ITokieNFTUpgradeable public _gegoToken;
    ...
    function initialize(
        address tokieNftToken, address gegoFactory, address rewardAddress, uint256 startTime
    ) public initializer {
        __GovernanceUpgradeable_init_unchained();
        __GeneralNFTRewardUpgradeable_init_unchained(tokieNftToken, gegoFactory, rewardAddress, startTime);
    }

    function __GeneralNFTRewardUpgradeable_init_unchained(address tokieNftToken, address gegoFactory, address rewardAddress, uint256 startTime) internal initializer {
        _rewardERC20 = IERC20WithDecimals(rewardAddress);
        _gegoToken = ITokieNFTUpgradeable(tokieNftToken);
        _gegoFactory = ITokieNFTFactoryUpgradeable(gegoFactory);
...
import "./ITokieNFTUpgradeable.sol";
...
interface ITokieNFTFactoryUpgradeable {
    ...
    function getGegoStruct(uint256 tokenId)
        external view
        returns (ITokieNFTUpgradeable.Gego memory gego);
...
interface ITokieNFTUpgradeable is IERC721Upgradeable {
