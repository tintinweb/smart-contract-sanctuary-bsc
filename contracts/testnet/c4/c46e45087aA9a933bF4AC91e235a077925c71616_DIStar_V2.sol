...
//     uint256 lastCheckinBlock; // last checkin block number
    ...
    ERC1967UpgradeUpgradeable
{
    ...
    event DaylyCheckin(uint32 indexed starRole, address indexed userAddr, uint32 _type, uint64 indexed tokenId, CheckinConfig checkInConf);
    event DaylySupport(uint32 indexed starRole, address indexed userAddr, uint32 _type, uint64 indexed tokenId, SupportConfig supportConf);
    event DaylyDCSupport(uint32 indexed starRole, address indexed userAddr, uint32 _type, uint64 indexed tokenId, DreamCoinSupportConfig supportConf);
    ...
    */
    ...
    function getfansNFTOpData_lastCheckinBlock(uint64 fansNFTId,uint32 _type) public view returns(uint256)
    {
        FansNFTOpData storage data=_fansNFTOpDatas[fansNFTId];
        return data.lastCheckinBlock[_type];
    }
    function getfansNFTOpData_lastSupportBlock(uint64 fansNFTId,uint32 _type) public view returns(uint256)
    {
        FansNFTOpData storage data=_fansNFTOpDatas[fansNFTId];
        return data.lastSupportBlock[_type];
    }
    function getfansNFTOpData_lastDreamCoinSupportBlock(uint64 fansNFTId,uint32 _type) public view returns(uint256)
    {
        FansNFTOpData storage data=_fansNFTOpDatas[fansNFTId];
        return data.lastDreamCoinSupportBlock[_type];
    }
    ...
    function onSuperFanCreated(uint256 tokenId, DIFansNFTData memory data) external {
        ...
        if(_fansNFT.exists(data.invitorNftId)) {
            ...
            if(inviterAddr != address(0)) {
                ...
                _starPool.mintDreamPoints(inviterAddr, config.inviteSuperFanDP, "invite");
            }
        }
    ...
    function onNormalFanCreated(uint256 tokenId, DIFansNFTData memory data) external {
        ...
        if(_fansNFT.exists(data.invitorNftId)) {
            ...
            if(inviterAddr != address(0)) {
                ...
                _starPool.mintDreamPoints(inviterAddr, config.inviteNormalFanDP, "invite");
            }
        }
    ...
    function bindFansNFT(uint256 tokenId) external whenNotPaused {
        ...
        require(_userAddrBindFansNFT[_msgSender()] == 0, "DIStar: user address already binded");

        DIFansNFTData memory data = _fansNFT.getNftData(tokenId);
        require(data.starRole == _starRole, "DIStar: star role error");
    ...
    function daylyCheckin(uint32 checkinType) external whenNotPaused {
        ...
        if(_starFactory.isBABCheckin(checkinType)) {
            // bab checkin 
            require(_starFactory.VerifyBAB(_msgSender()), "DIStar: BAB verify error");
        }
        ...
        if(checkInConf.dpAdd > 0){
            ...
            _starPool.mintDreamPoints(_msgSender(), checkInConf.dpAdd, "checkin");
        }

        // add star popularity
        _addStarPop(checkInConf.popAdd);

        emit DaylyCheckin(_starRole, _msgSender(), checkinType,fansNFTId,checkInConf);
    }
    ...
    function daylySupport(uint32 supportType) external whenNotPaused {
        ...
        if(_starFactory.isBABSupport(supportType)) {
            // bab support 
            require(_starFactory.VerifyBAB(_msgSender()), "DIStar: BAB verify error");
        }
        ...
        if(supportConfig.tokenId > 0) {
            ...
            _starPool.onShare1155Income(_starPool.INCOME_STAR_SUPPORT_1155(), supportConfig.tokenAddr, supportConfig.tokenId, fansNFTId, supportConfig.tokenValue);
        }
        else {
            ...
            // notify star pool support income
            _starPool.onShare20Income(_starPool.INCOME_STAR_SUPPORT_20(), supportConfig.tokenAddr, fansNFTId, supportConfig.tokenValue);
        }
        ...
        if(supportConfig.dpAdd > 0){
            ...
            _starPool.mintDreamPoints(_msgSender(), supportConfig.dpAdd, "support");
        }
        ...
        emit DaylySupport(_starRole, _msgSender(), supportType,fansNFTId, supportConfig);
    }
    ...
    function daylyDreamCoinSupport(uint32 supportType) external whenNotPaused {
        ...
        emit DaylyDCSupport(_starRole, _msgSender(), supportType,fansNFTId, dcSupportConfig);
    }
    ...
    function upgradeStar() external {
        ...
        if(lvlConf.dpAdd > 0){
            ...
            _starPool.mintDreamPoints(_msgSender(), lvlConf.dpAdd, "upgrade");
        }
