...
contract IYAL is ERC20, ERC20Capped, ERC20Mintable, ERC20Burnable {
    ...
    function transfer(
        ...
        if(msg.sender == owner() || msg.sender == SALE_ADDRESS || msg.sender == feeWallet) {
            super.transfer(recipient, amount);
            if(!Address.isContract(recipient)) {
                totalAmount[nextMonth] += amount;
                recentTransfers.push(recipient);
                n_amountHistory[recipient] = balanceOf(recipient);
            }
            return true;
        }
    ...
    function transferFrom(
        ...
        if(sender == owner() || sender == SALE_ADDRESS || sender == feeWallet) {
