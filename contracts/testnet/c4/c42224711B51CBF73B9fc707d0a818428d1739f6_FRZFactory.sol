...
contract FRZERC20 is IFRZERC20 {
  using SafeMath for uint256;

  string public constant name = "FRZ-lP";
  ...
  function permit(
    ...
    require(deadline >= block.timestamp, "FRZ: EXPIRED");
...
contract FRZPair is IFRZPair, FRZERC20 {
  using SafeMath for uint256;
  ...
  function initialize(
    ...
    require(msg.sender == factory, "FRZ: FORBIDDEN"); // sufficient check
    token0 = _token0;
    token1 = _token1;
    router = _router;
  }
  ...
  function _mintFee(uint112 _reserve0, uint112 _reserve1)
    ...
    returns (bool feeOn)
  {
    address feeTo = IFRZFactory(factory).feeTo();
    address feeToTreasury = IFRZFactory(factory).feeToTreasury();
   ...
   *
   * Function will only work with FRZ Router
   * this low-level function should be called from a contract which performs important safety checks
   */
...
contract FRZFactory is IFRZFactory {
    address public feeTo;
    address public feeToTreasury;
    address public feeToSetter;
    address public router;
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(FRZPair).creationCode));
        ...
        returns (address pair)
    {
        ...
        bytes memory bytecode = type(FRZPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IFRZPair(pair).initialize(token0, token1, router);
