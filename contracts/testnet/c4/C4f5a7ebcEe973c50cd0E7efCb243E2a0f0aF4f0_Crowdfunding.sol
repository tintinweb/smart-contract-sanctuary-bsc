...
contract Crowdfunding is AccessControlEnumerable {
    ...
    uint256[] public equitys = [0, 0, 10, 20, 25];
    bytes32 public constant OPERATER_ROLE = keccak256("OPERATER_ROLE");
    uint256 public constant fundInterval = 30;

    address public lp; //lp token
    address public fee; //手续费地址
    address public donate; //捐赠池
    address public stake; //质押池
    address public metaPoint; //metaPoint address
    uint16 public staticRewardRate = 100; //静态奖比例
    uint16 public brokerRewardRate = 85; //经纪人奖比例
    uint16 public baseRefundRate = 800; //失败前三期返还本金比例
    uint16 public donateRate = 10; //捐赠比例1%
    uint16 public stakeRate = 50; //质押池比例
    uint16 public bonusRate = 500; //失败当期彩蛋池奖励比例
    uint16 public freeWithdrawRate = 200; //免费提现比例
    uint16 public rewardFeeRate = 10; //盈利提现手续费 本金提现也收手续费
    uint16 public rewardLockRate = 200; //奖励电灯解锁比例
    uint16 public maxFundPercent = 50; //单用户最大投资比例
    ...
    event PoolAmountChange(uint256 region, uint256 project, uint256 amount);
    ...
    constructor() {
        ...
        metaPoint = 0x8347A810ff4d856A1571A5Ecc4672C0d0A52fA6c;
        fee = 0x221A126E0F2B4A6c5848689145589f9A229dc3aa;
    }

    function setEquitys(uint256 _index, uint256 _equity) external onlyRole(OPERATER_ROLE) {
    ...
    function brokerEquitys(address _addr) internal view returns (address[] memory, uint256[] memory) {
        uint256 level = 0;
        uint256 total = 0;
        address[] memory addresss = new address[](4);
        uint256[] memory equities = new uint256[](4);
        uint256 index = 0;
    ...
    //设置奖励比例
    function setRates(uint16 _staticRewardRate, uint16 _brokerRewardRate) public onlyRole(OPERATER_ROLE) {
        staticRewardRate = _staticRewardRate;
        brokerRewardRate = _brokerRewardRate;
    }
    ...
    function _addProject(uint256 _regionIndex) internal {
        ...
        p.maxAmountPerAccount = (p.targetAmount * maxFundPercent) / 1000 > maxFundAmount ? (p.targetAmount * maxFundPercent) / 1000 : maxFundAmount;
    ...
    function fund(
        ...
        require(lastFundTime[msg.sender] + fundInterval < block.timestamp, "funding time limited");
        ...
        // require(regions[_regionIndex].currentRound == _round, "wrong project round");
        // require(block.timestamp > p.startTime, "project not start");
        // require(block.timestamp < p.startTime + regions[_regionIndex].period, "project finished");
        // require(p.state <= 2, "wrong project state");
        uint256 amountLeft = p.targetAmount - p.currentAmount - p.reserveFundAmount;
        if (amountLeft == 0) revert ProjectAmountError();
        // require(amountLeft > 0, "funding max amount limited");
        ...
        // require(accountAmountLeft > 0, "account max amount limited");
    ...
    function reserve(
        ...
        if (_round <= currentRound || _round > currentRound + 5) revert ProjectTimeError();
        // require(_round > currentRound && _round < currentRound + 6, "not reserve time");
        Project storage p = projects[_regionIndex][_round];
        if (block.timestamp + one_hour > p.startTime) revert ProjectTimeError();
        // require(block.timestamp + one_hour < p.startTime, "reserve time passed");
        uint16 version = p.version;
        ProjectBalance storage pb = projectBalance[_regionIndex][_round];
    ...
    function finishProject(uint256 _regionIndex, uint256 _round) public {
        ...
        // require(_round == regions[_regionIndex].currentRound, "wrong project id");
        Project storage p = projects[_regionIndex][_round];
        ...
        if (_currentAmount == p.targetAmount) {
            ...
            if (_round >= 3 && projects[_regionIndex][_round - 3].state == 3) {
                ...
                p3.refundAmount = (_currentAmount * (1000 + staticRewardRate + brokerRewardRate)) / 1000;
                _poolAmount -= p3.refundAmount;
                emit PoolAmountChange(_regionIndex, _round - 3, _poolAmount);
            }
        } else {
            //当期失败
            p.state = 4;
            ...
            if (_round > 0) {
                ...
                for (uint256 i = start; i < _round; i++) {
                    ...
                    if (pi.state == 3) {
                        ...
                        pi.refundAmount = (pi.targetAmount * (baseRefundRate + brokerRewardRate)) / 1000;
                        _poolAmount -= pi.refundAmount;
                        emit PoolAmountChange(_regionIndex, i, _poolAmount);
                    }
                }
            }
            //当期奖励底池50%
    ...
    function resetProjects(uint256 _regionIndex, uint256 _round) internal {
        ...
        for (uint256 i = _round + 1; i < _length; i++) {
            ...
            p.maxAmountPerAccount = (_targetAmount * maxFundPercent) / 1000 > maxFundAmount ? (_targetAmount * maxFundPercent) / 1000 : maxFundAmount;
            p.currentAmount = 0;
            p.reserveAmount = 0;
        ...
        )
    {
        ...
        if (p.state == 4) {
            ...
            uint256 _currentAmount = p.currentAmount;
            brokerReward = (_currentAmount * brokerRewardRate) / 1000;
            rewardAmount = _currentAmount == 0 ? 0 : ((p.refundAmount - _currentAmount) * _fundAmount) / (_currentAmount * 1000);
            freeWithdrawAmount = _fundAmount;
        } else if (p.state == 5) {
            fundAmount = _fundAmount;
            rewardAmount = (_fundAmount * staticRewardRate) / 1000;
            brokerReward = (_fundAmount * brokerRewardRate) / 1000;
        } else if (p.state == 6) {
            fundAmount = (_fundAmount * baseRefundRate) / 1000;
            freeWithdrawAmount = (_fundAmount * freeWithdrawRate) / 1000;
    ...
    function harvest(uint256 _regionIndex, uint256 _round) public {
        ...
        // require(p.state > 3, "wrong project state");
        if (!harvestableIds[msg.sender][_regionIndex].contains(_round)) revert ProjectRoundError();
        // require(harvestableIds[msg.sender][_regionIndex].contains(_round), "wrong project id");
        ...
        // accounts[msg.sender].withdrawableAmount[_regionIndex] += fundAmount + rewardAmount;
        uint256 lockedAmount = (_rewardAmount * rewardLockRate) / 1000;
        if (lockedAmount > 0) {
        ...
        if (freeWithdrawAmount >= _amount) {
            ...
            uint256 _feeAmount = ((_amount - freeWithdrawAmount) * rewardFeeRate) / 1000;
        ...
        processBrockerReward(msg.sender, _brokerReward);
    ...
    //经纪人奖
    function processBrockerReward(address account, uint256 amount) public {
        (address[] memory addresses, uint256[] memory equities) = brokerEquitys(account);
        uint256 left = 0;
        for (uint256 i = 0; i < 4; i++) {
            if (equities[i] > 0) {
                uint256 _amount = (amount * equities[i]) / 100;
                if (addresses[i] != address(0) && _amount > 0) {
                    IERC20(lp).transfer(addresses[i], (_amount * (1000 - freeWithdrawRate)) / 1000);
                    accounts[addresses[i]].lockedWithdrawAmount += (_amount * freeWithdrawRate) / 1000;
                } else {
                    left += _amount;
                }
            }
