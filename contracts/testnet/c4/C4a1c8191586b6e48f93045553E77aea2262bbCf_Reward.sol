...
contract Reward is Ownable2StepUpgradeable {
    ...
    event AdjustSingleReward(address distributor, address user, uint pid, uint index, uint reward);
    event AdjustReward(address distributor, address[] userList, uint[] pidList, uint[] indexList, uint[] adjustAmountList);
    ...
    event BatchClaim(address indexed user, uint[] pidList, uint[] indexList, uint[] amountList);
    ...
    function distributeReward(address[] calldata userList, uint[] calldata pidList, uint[] calldata indexList, uint[] calldata rewardList, uint totalRewards) external {
        ...
        require(userList.length == pidList.length && userList.length == indexList.length && userList.length == rewardList.length, "incorrect params");
    ...
    function claim(uint pid, uint index, uint amount) public {
        LockInfo storage lockInfo = userInfo[address(msg.sender)][pid][index];
    ...
    function adjustReward(address[] calldata userList, uint[] calldata pidList, uint[] calldata indexList, uint[] calldata adjustAmountList) external {
        require(address(msg.sender) == distributor, "only distributor can call");
        require(userList.length == pidList.length && userList.length == indexList.length && userList.length == adjustAmountList.length, "incorrect params");
        for (uint i = 0; i < userList.length; i++) {
            LockInfo storage lockInfo = userInfo[userList[i]][pidList[i]][indexList[i]];
            require(lockInfo.totalRewards >= adjustAmountList[i] && lockInfo.unclaimedRewards >= adjustAmountList[i], "rewards less than adjust amount");
            lockInfo.totalRewards -= adjustAmountList[i];
            lockInfo.unclaimedRewards -= adjustAmountList[i];
            emit AdjustSingleReward(address(msg.sender), userList[i], pidList[i], indexList[i], adjustAmountList[i]);
        }
        emit AdjustReward(address(msg.sender), userList, pidList, indexList, adjustAmountList);
    }

    // 批量领取收益
    function batchClaim(uint[] calldata pidList, uint[] calldata indexList, uint[] calldata amountList) external {
        require(pidList.length == indexList.length && pidList.length == amountList.length, "incorrect params");
        if (pidList.length == 0) {
            return;
        }
        for (uint i = 0; i < pidList.length; i++) {
            claim(pidList[i], indexList[i], amountList[i]);
        }
        emit BatchClaim(address(msg.sender), pidList, indexList, amountList);
    }
