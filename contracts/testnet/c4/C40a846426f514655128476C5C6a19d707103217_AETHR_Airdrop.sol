...
contract AETHR_Airdrop is Ownable, Pausable {
    ...
    function canClaim(address _user, uint256 _to) public view returns (bool) {
        if (_to < startTime) return false;
        uint256 userClaimTo = lastClaimToken[_user] > startTime
            ? (lastClaimToken[_user] - startTime) / SECONDS_IN_DAY + 1
            : 0;
        uint256 claimTo = (_to - startTime) / SECONDS_IN_DAY;
