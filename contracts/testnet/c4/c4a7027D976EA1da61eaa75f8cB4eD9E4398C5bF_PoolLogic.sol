...
contract PoolLogic is ReentrancyGuard {
    ...
    address public owner = 0x2E2854cb744b8dd6FF48291F59671076612f28Dd;
    address public manager;
    address payable[] private investors;

    address public depositedToken;  // deposit token address
    uint256 public minimumPrice;    // price per unit

    mapping(address => uint256) public indexOfInvestor; // map stores 1-based
    mapping(address => uint256) private units;          // unit of users
    uint private totalUnit;                             // total amount of unit
    ...
    event AssetRemoved(address indexed pool, address manager, address asset);

    constructor(address _router, address _WBNB, address _tokenIn, uint256 _minPrice) {
        manager = msg.sender;
        swapRouter = _router;
        WBNB = _WBNB;
        depositedToken = _tokenIn;
        totalUnit = 0;
    ...
    function deposit(address _tokenIn, uint _amountIn) external {
        require(depositedToken == _tokenIn, "token incorrect");
        require(_amountIn > 0, "require amount of token");
        require(_amountIn >= minimumPrice, "require minimum amount");

        // transfer token from sender to contract
        IERC20(_tokenIn).safeTransferFrom(msg.sender, address(this), _amountIn);

        // calculate the proportion
        uint numberOfUnit = _calculateNumberOfUnit(_amountIn, minimumPrice);
    ...
    function withdraw(uint256 _unitOut) external nonReentrant {
        ...
        uint numberOfUnit = units[msg.sender];
        ...
        // emit Withdraw(_tokenOut, msg.sender, _unitOut);
    }

    function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin) external onlyManager {
        if (_tokenIn == _tokenOut) {
            return;
        }

        // Approve if not enough swap allowance
        if (IERC20(_tokenIn).allowance(address(this), address(swapRouter)) < _amountIn) {
        ...
        IPancakeRouter02(swapRouter).swapExactTokensForTokens(_amountIn, _amountOutMin, path, address(this), block.timestamp);
    ...
    function _addAsset(Asset calldata _asset) internal {
        ...
        if (assetPosition[asset] != 0) {
            ...
            assets[index].isDeposit = isDeposit;
        } else {
            assets.push(Asset(asset, isDeposit));
            assetPosition[asset] = assets.length;
        }
    ...
    function _removeAsset(address asset) internal {
        ...
        require(getBalance(address(this), asset) == 0, "cannot remove non-empty asset");

        uint256 length = assets.length;
        Asset memory lastAsset = assets[length.sub(1)];
        uint256 index = assetPosition[asset].sub(1);

        assets[index] = lastAsset;
    ...
    function _removeInvestor(address _investor) internal {
        ...
        uint index = indexOfInvestor[_investor].sub(1);
    ...
    function _calculateNumberOfUnit(uint _amountIn, uint _price) internal pure returns (uint numberOfUnit) {
        numberOfUnit =  _amountIn.mul(10**18).div(_price);
    }

    function _calculateNumberOfWithdrawnToken(address _pool, address _asset, uint256 _proportion) internal view returns (uint withdrawnAmount) {
