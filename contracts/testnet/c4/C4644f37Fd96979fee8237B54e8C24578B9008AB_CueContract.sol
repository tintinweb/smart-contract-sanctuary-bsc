    ...
    ERC721URIStorageUpgradeable
{
    ...
    mapping(uint256 => uint256) private _maxDurability;
    ...
    function mint(
        ...
        require(maxDurability > 0, "Max durability must be greater than zero");
        ...
        cue.rarity = rarity;
        _maxDurability[tokenId] = maxDurability;
    ...
    function mintBatch(
        ...
        uint256[] memory maxDurabilities,
        ...
        require(
            durabilities.length == recipients.length,
            "Max durabilities and recipients list must be same length"
        );
        require(
    ...
    function approveBatch(address to, uint256[] memory tokenIds)
        public
        whenNotPaused
    {
        require(tokenIds.length > 0, "Token IDs list must not be empty");
    ...
    function updateMaxDurability(
        uint256[] memory tokenIds,
        uint256[] memory maxDurabilities
    ) public onlyOwner {
        require(tokenIds.length > 0, "Token IDs list must be not empty");
        require(
            tokenIds.length == maxDurabilities.length,
            "Token IDs and max durabilities list must be same length"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _maxDurability[tokenIds[i]] = maxDurabilities[i];
        }
    }
        ...
        )
    {
        ...
        maxDurability = _maxDurability[tokenId] == 0
            ? cue.durability
            : _maxDurability[tokenId];
