...
contract GoldDuckCustomDiscount is Ownable, ReentrancyGuard, Pausable {
    ...
    */
    ...
    function setSalePrice(uint8 newType) external onlyOwner {
    ...
    function claim(uint256 lockTicket) external nonReentrant whenNotPaused {
        ...
        require(getUserLockTicketContains(_msgSender(),lockTicket), "PoolHarbor: Not able to access this ticketid");
        require(store.lockedTime.add(vestingDuration) <= block.timestamp, "PoolHarbor: Unable to access this time");
        
        saleToken.safeTransfer(msg.sender,store.lockedAmount);
        store.lockedAmount = 0;
        store.claimTime = block.timestamp;
        emit unlockEvent(msg.sender,store.lockedAmount,block.timestamp);
    }
    ...
    function _buy(address user,address referrer,uint256 amount) internal returns (bool){
        ...
        userLockTicketInfo[_msgSender()].add(currentLockTicket);
