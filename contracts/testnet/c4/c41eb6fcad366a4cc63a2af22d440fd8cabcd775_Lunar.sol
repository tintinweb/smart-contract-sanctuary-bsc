...
/**
Copyright © 2022-2023 Lunar Foundation. All rights reserved.
...
contract Lunar is Initializable, ERC20Upgradeable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  */
  ...
  mapping (address => bool) private _exchangesList;           //Used to track exchange (CEX/DEX) addresses across the contract.
  ...
  */
  struct FeesPaidHistory 
  {
    ...
    /** The total fees that have ever been taken to support development efforts. */
  ...
  */
  function updateBuyFees(uint256 liquidityFee, uint256 marketingFee, uint256 operationsFee, uint256 developmentFee) public whenNotPaused onlySystemRoles
  {
    ...
    buyFeeRates.developmentFee = developmentFee;
  ...
  */
  function updateSellFees(uint256 liquidityFee, uint256 marketingFee, uint256 operationsFee, uint256 developmentFee) public whenNotPaused onlySystemRoles
  {
    ...
    sellFeeRates.developmentFee = developmentFee;
  ...
  */
  function updateTransferFees(uint256 liquidityFee, uint256 marketingFee, uint256 operationsFee, uint256 developmentFee) public whenNotPaused onlySystemRoles
  {
    ...
    transferFeeRates.developmentFee = developmentFee;
  ...
  */
  function _getTransactionDetails(uint256 amount, TransactionType transactionType) internal view returns (TransactionDetails memory)
  {
    ...
    {
      uint256 totalTransferFees = transferFeeRates.liquidityFee.add(transferFeeRates.marketingFee).add(transferFeeRates.operationsFee).add(transferFeeRates.developmentFee);
      ...
      return TransactionDetails({
        ...
        developmentFee : amount.mul(transferFeeRates.developmentFee).add(500).div(1000),
    ...
    {
      uint256 totalBuyFees = buyFeeRates.liquidityFee.add(buyFeeRates.marketingFee).add(buyFeeRates.operationsFee).add(buyFeeRates.developmentFee);
      ...
      return TransactionDetails({
        ...
        developmentFee : amount.mul(buyFeeRates.developmentFee).add(500).div(1000),
        shouldBypassFees : false,
        transactionType : transactionType
      });
    }
    ...
    {
      uint256 totalSellFees = sellFeeRates.liquidityFee.add(sellFeeRates.marketingFee).add(sellFeeRates.operationsFee).add(sellFeeRates.developmentFee);
      ...
      return TransactionDetails({
        ...
        developmentFee : amount.mul(sellFeeRates.developmentFee).add(500).div(1000),
        shouldBypassFees : false,
        transactionType : transactionType
      });
    }
  ...
  */
  function _recordFeesPaidHistory(TransactionDetails memory details) private
  {
    ...
    contractFeesPaidHistory.totalDevelopmentFeesPaid += details.developmentFee;
  }
