...
contract DV2TokenMine is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    function redeem(uint256 amount) public payable {
        ...
        require(!getBlackListStatus(msg.sender),'Address is blacklisted!');
    ...
    // normal transaction
        ...
        require(!getBlackListStatus(from),'Address is blacklisted!');
        require(!getBlackListStatus(to),'Address is blacklisted!');
        ...
        // tx limits
            ...
            if(address(this).balance > minimumWeiForTokenomics) {
                ...
                uint256 marketingTokens = caBalance;
                (bool success,) = address(devAddress).call{value: marketingTokens}("");
                if(success) {
                    emit feeCollected(marketingTokens);
                }
            }
    ...
    // Reset Multiplier
    function resetMultiplier() internal {
      uint256 buyprice = buyPriceNow();
      uint256 _tokenCount = readMultiplierLength();
      uint256 currentBasePrice = nextBasePrice();
      if(isMultiEnabled){
        if(sellActive && buyprice <= currentBasePrice){
          lastMultiplierData[_tokenCount].lastMulti = checkMultiplier();
          tokenCount.increment();
          lastMultiplierData[readMultiplierLength()].currentMulti = readPowerResult(globalMultiplier.mul(10 ** decimals()).div(denominatorMultiplier),readMultiplierLength());
          toggleCanSell(false);
        }
      }
    }
    ...
    // Set Fees
    function setFee(uint256 Buydev,uint256 Selldev) external onlyOperator {
        //buy
        totalBuyFee = Buydev;
        //sell
        totalSellFee = Selldev;
        //checks
        require(totalBuyFee + totalSellFee <= 200,"Total fees cannot be over 20%");
        emit FeesChanged(Buydev,Selldev);
    }
    // Set Fees Status
    function setFeeStatus(bool buy, bool sell, bool _state) external onlyOperator {
        feeStatus = _state;
        buyFeeStatus = buy;
        sellFeeStatus = sell;
    ...
    // Set Fees Address
    function setFeesAddress(address dev) external onlyOperator {
        devAddress = dev;
    }
    // Add Liquidity
    ...
    // }
    // Swap Tokens
    function swapTokens(uint256 contractTokenBalance) public FastTx {
        uint256 liq_part = contractTokenBalance / 100;
    ...
    function swapTokensForEth(uint256 tokenAmount) public {
        ...
        path[0] = address(this);
        path[1] = (IUniswapV2Pair(pancakeV2Pair).token1() == address(this) ? IUniswapV2Pair(pancakeV2Pair).token0() : IUniswapV2Pair(pancakeV2Pair).token1());
        path[2] = pancakeV2Router.WETH();
        _approve(address(this), address(pancakeV2Router), tokenAmount);
    ...
    function checkUserCanSold(address _user,uint256 _soldAmount) public view returns (bool) {
      ...
      if(isMultiEnabled){
        if(_soldAmount < maxSellTxAmount && sellActive){
          return true;
        }else{
          return false;
        }
    ...
    // Set Excluded From Fees users
    function editPremarketUser(address _target, bool _status) external onlyOperator {
    ...
    function increaseTokenCount()public {
        tokenCount.increment();
    }
    // Airdrop Function
...
abstract contract StructData is EventsLog {
    ...
    // Address
    address public devAddress = 0x07AfB4dA9B4fC723779c6554d52db647F0a8d8c7;
    // Struct
    struct userData {
      uint lastBuyTime;
      uint lastSellTime;
...
abstract contract EventsLog is BlackList {
    ...
    event FeesChanged(uint256 buyDev, uint256 sellDev);
