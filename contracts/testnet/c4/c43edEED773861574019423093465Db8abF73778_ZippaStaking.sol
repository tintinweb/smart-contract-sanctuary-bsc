...
contract ZippaStaking is Initializable, OwnableUpgradeable {
    ...
    function stake(uint256 _amount, address _staker) external {
        ...
        require(
            IIERC20(StakeToken).allowance(address(this), _staker) >= _amount,
        ...
        if (stakings[_staker].amountStaked > 0) {
            ...
            uint256 earningPerSec = stakings[_staker].amountStaked.mul(
                EarningPercentagePerSeconds.div(100)
            );
        ...
        stakings[_staker].amountStaked = stakings[_staker].amountStaked.add(
            _amount
        );
    ...
    function claim() external {
        ...
        require(
            block.timestamp.sub(userData.lastTimeStaked) >= minimumClaimSeconds,
        ...
        uint256 earningPerSec = userData.amountStaked.mul(
            EarningPercentagePerSeconds.div(100)
        );
        uint256 totalClaim = (earningPerSec * totalSeconds * 10e18).add(
            userData.accuredBeforeZipRestake
        );
    ...
    function setEarningPercentagePerSeconds(uint256 amount) external onlyOwner {
        require(amount > 0, "Invalid Amount");
        EarningPercentagePerSeconds = amount;
    }
