...
contract JuneLogic is OwnableUpgradeable, ERC20Upgradeable, Manager, Console {
    ...
    function Dividend() public onlyManager {
        uint256 DividendTotal = balanceOf(SlippageAddr).mul(100000000);
        log("DividendTotal:", DividendTotal);
        IERC20Upgradeable LiquidityWallet = IERC20Upgradeable(LiquidityAddr);
        uint256 LpTotal = LiquidityWallet.totalSupply();
        log("LpTotal:", LpTotal);
        uint256 oneDividend = DividendTotal.div(LpTotal);
        log("oneDividend:", oneDividend);
        for (uint i = 0; i < DividendAdds.length; i++) {
            ...
            log("userDividend", userDividend);
            if (userBalance >= DividendMin && DividendAdds[i] != address(0)) {
