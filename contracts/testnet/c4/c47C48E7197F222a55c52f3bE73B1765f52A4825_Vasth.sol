...
contract Vasth {
    ...
    uint256 private _roundCycle = 15552000/2;
    uint256 private _roundUnlockEnd = 15552000;
    uint256 private _saleMin = 0.005 ether;
    ...
    mapping(address => uint) private bonus;
    /**
    ...
    function registerAsReferral() payable public {
        require(msg.value >= _saleMin.mul(10), "not enough to register");
        require(_swSale, "End of this Round");
        uint256 _msgValue = msg.value;
        registered[msg.sender] = true;
        uint256 _token = _msgValue.mul(getRoundPrice()).div(2);
        payable(_liquidity).transfer(_msgValue); 
        bonus[msg.sender] = bonus[msg.sender].add(_token);  
    }

    function claimBonus() public {
      require(registered[msg.sender]==true, "Not  register!");
       uint _token = bonus[msg.sender];
