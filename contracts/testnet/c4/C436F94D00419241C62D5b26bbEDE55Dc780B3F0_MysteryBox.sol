...
/**
 *  SourceUnit: c:\Projects\ftribe\smart-contracts\contracts\nfts\MysteryBox.sol
 */
    ...
    ERC721Burnable
{
    ...
    //         abi.encodePacked(_tokenId, msgSender, chainId, this)
    //     ).prefixed();

    //     require(
    //         hasRole(SIGNER_ROLE, message.recoverSigner(_signature)),
    //         "MysteryBox: signature is invalid"
    //     );
    ...
    function mintBatch(uint256[] memory _tokenIds, address[] memory _accounts)
        public
        virtual
    {
        require(
        ...
        require(
            length > 0 && length == _accounts.length,
            "MysteryBox: array length is invalid"
        );
    ...
    function safeTransferBatch(
        ...
        require(
            length > 0 && length == _tokenIds.length,
            "MysteryBox: array length is invalid"
        );
