 ...
 */
interface ERC721metadata is ERC721Interface {
    /**
...
contract ERC721U is ERC165Upgradeable, IERC721Upgradeable {
    ...
    //if I have modifier onlyOwner, do I need to have line 71

    function approve(address to, uint256 tokenId) public virtual override {
        address owner = ERC721U.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");
        _approve(to, tokenId);
    }

    function setApprovalForAll(address operator, bool approved) public virtual override{
        return _setApprovalForAll(msg.sender, operator, approved);
    }
    ...
    function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) public virtual override{
        require(_isApprovedOrOwner(msg.sender, tokenId));
        return _safeTransferFrom(from, to, tokenId, data);
    }
    ...
    function _safeMint(address to, uint256 tokenId, bytes memory data) internal virtual {
        _mint(to, tokenId);
        require(this.OnERC721Received(address(0), to, tokenId, data));
    }
    ...
    function _safeTransferFrom(address from, address to, uint256 tokenId, bytes memory data) internal virtual {
        _transfer(from, to, tokenId);
        require(this.OnERC721Received(from, to, tokenId, data));
    }

    function _transfer(address from, address to, uint256 tokenId) internal virtual  {
        require(ERC721U.ownerOf(tokenId) == from);
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(ERC721U.ownerOf(tokenId), to, tokenId);
    }

    function _setApprovalForAll(address owner, address operator, bool approved) internal virtual{
        require(owner != operator, "operator is the owner");
        operatorApprovals[owner][operator] = approved;
        emit ApprovalForAll(owner, operator, approved);
    }

    function _isContract(address user) private view returns (bool){
    uint32 size;
