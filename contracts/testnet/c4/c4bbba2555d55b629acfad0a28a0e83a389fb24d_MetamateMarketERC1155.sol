...
abstract contract MetamateMarketBase is Initializable, IMetamateMarketBase, AcceptedTokenList, Operators, ReentrancyGuardUpgradeable {
    ...
    external payable override nonReentrant
  {
    ...
    if (order.isSelling) {
      ...
      if (order.nftToken == MTM_DARLING) {
        ...
        uint256 taxPercent = getTaxPercentage(nft, order.tokenId);
        if (taxPercent > 0) {
  ...
  function getTaxPercentage(MetaMateDarlingERC721 nft_, uint256 tokenId_) public view returns (uint256) {
