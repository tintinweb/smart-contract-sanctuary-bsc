...
contract MyToken is IBEP20 {
    ...
    function addLPoolPCS(address tokenA, uint256 amountTokens, uint256 amounTkMin, uint256 amountBNBMin, address _to, uint256 deadline) ownerOnly public payable returns(uint amountToken, uint amountETH, uint liquidity) {
        IPancakeRouter01 pancakeContract = IPancakeRouter01(_lpContract);
        pancakeContract.addLiquidityETH(tokenA, amountTokens, amounTkMin, amountBNBMin, _to, deadline);
        //liquidity = _liquidityBNB.div(amountTokens);
        return (amountTokens, msg.value, liquidity);
    }
}