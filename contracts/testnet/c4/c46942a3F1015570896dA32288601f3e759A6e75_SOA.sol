...
contract SOA is IERC20, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function __SOA_v1_init_unchained(address routerAddress) internal initializer {
        ...
        _buyLiquidityFee = 3; //0.03%
        _sellLiquidityFee = 5; //0.05% 
        maxLiquidityFee = 300; //Owner can't change auto lp tax more than 3%. 

        _MarketingFee = 0; // Set dynamically
        _buyMarketingFee = 3; //0.03%  
        _sellMarketingFee = 3;//0.03%
        ...
        maxTxPercent = 15; // You can't send more than 1.5% of the total supply in one transaction.
        maxTxDivisor = 1000;

        _maxTxAmount = (_tTotal * maxTxPercent) / maxTxDivisor;
        _previousMaxTxAmount = _maxTxAmount;
        maxTxAmountUI = (startingSupply * maxTxPercent) / maxTxDivisor; // Actual amount for UI's
