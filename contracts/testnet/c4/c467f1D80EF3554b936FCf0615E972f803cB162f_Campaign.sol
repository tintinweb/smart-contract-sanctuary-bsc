...
contract Campaign{
    ...
    address public tokenAddress = 0xe36EFAAd23956c4C87273caA39AA84DDe4266815; // USDC  
    ...
    function managerApprove(address repWallet, uint256 fee) external isPaused{
    ...
    function projectPause() external{
        require(msg.sender == befunderManager, "only manager.");
        if (pause){
            pause = false;
        } else {
    ...
    function changeRepWallet(address newWallet) external isPaused {
        require(msg.sender == befunderManager, "only manager.");
    ...
    function finishingProject() public view returns(bool){
        //require(!projectComplete, "project has already finished.");
        return(finishCheck);
    }
