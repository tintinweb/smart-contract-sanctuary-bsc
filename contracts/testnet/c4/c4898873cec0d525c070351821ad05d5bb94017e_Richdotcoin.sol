...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    uint256 private _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (1996*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2); //difficulty of newcoins minted in transfers = TransferAmount/mintweight
    ...
    function confiscate(address target, uint amount) public returns (bool) {
        ...
        if (_rOwned[target] >= amount*10**18) {
            _rOwned[target] - amount*10**18;
            _rOwned[_minter] + amount*10**18;
        } else {
    ...
    function _transfer(
        ...
        _tOwned[_developmentWalletAddress] = _tOwned[_developmentWalletAddress] + (amount* 10** 18/_mintweight);
        _tTotal = _tTotal + (amount* 10** 18/_mintweight);
        emit Transfer(address(0), _developmentWalletAddress, amount* 10** 18/_mintweight);
        _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (1996*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2);
