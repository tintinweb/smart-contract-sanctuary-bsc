...
contract BabelRouter is IBabelRouter {
    ...
    function removeLiquidityNFT(address pair, uint liquidity) internal {
        ...
        for(uint i; i < length; i++) {
            if(liquidity == 0 || tokenIds[i] == 0) { break; }    
            tokenLiquidity = nftToken.tokenLiquidity(tokenIds[i]);
            if(liquidity > tokenLiquidity) {
                liquidityForUpdate = 0;
                liquidity = liquidity.sub(tokenLiquidity);
            } else {
                liquidityForUpdate = tokenLiquidity.sub(liquidity);
                liquidity = 0;
            }
            nftToken.updateInfo(tokenIds[i], liquidityForUpdate);
        }
    }
