...
contract LIFEGAMES is IERC20 {
    ...
    function calculateWhaleFee(uint256 amount) public view returns (uint256) {
        ...
        uint256 liquidityAmount = 5000;
        //uint256 liquidityAmount = getReserves()[0];
        // if amount in busd exceeded the % setted as whale, calc the estimated fee
        if (busdAmount >= ((liquidityAmount * whaleFeePercent) / 10000)) {
            // mod of busd amount sold and whale amount
            uint256 modAmount = busdAmount % (liquidityAmount * whaleFeePercent) / 10000;
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if (from == lpPair) {
            ...
            // SELL
            // todo disable max tx fees when antiwhales is actived - no es necesario ya que en el set nunca va a dejar que sea
            // mayor al 3% y si no es ballena calculateWhale devolverá 0
