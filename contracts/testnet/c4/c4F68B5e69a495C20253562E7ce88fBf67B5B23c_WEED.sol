...
contract WEED is IBEP20, Ownable {
    ...
    function setCouncil(address newCouncil) external onlyOwner {
        require(
            council != newCouncil,
            "WEED: council address cannot be the same"
        );
    ...
    function setDividend(address newDividend) external onlyOwner {
        require(
            ...
            "WEED: dividend address cannot be the same"
        );
    ...
    function _transfer(
        ...
        if (_shouldTakeFee(sender, recipient)) {
            ...
            require(
                ...
                "WEED: transfer amount + fees exceeds balance"
            );
    ...
    /**
     * @dev Transfers the fee `amount` of WEED tokens from the `sender` to the `feeWallet`
     */
