...
contract DGEAPEToken is ERC20, Ownable  {
    ...
    DGEAPEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DOOGE APE BSC", "DGEAPE") {
        
        _balances[owner()] = _balances[owner()].add(TOTAL_SUPPLY);
        
        emit Transfer(address(0), address(this), TOTAL_SUPPLY);
        
        dividendTracker = new DGEAPEDividendTracker();
    ...
    function triggerBuyBack(uint256 amount) public onlyOwner {
        require(!swapping, "DGEAPE: A swapping process is currently running, wait till that is complete");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        DGEAPEDividendTracker newDividendTracker = DGEAPEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DGEAPE: The new dividend tracker must be owned by the FLOKIBUSD token contract");
    ...
    function updateDividendRewardFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DGEAPE: Dividend reward tax must be between 0 and 10");
        dividendRewardsFee = newFee;
    }
    
    function updateBuyBackFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DGEAPE: Buy Back Fee must be between 0 and 10");
        buyBackFee = newFee;
    }
    
    
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DGEAPE: Marketing Fee must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateCharityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DGEAPE: Charity Fee must be between 0 and 10");
        charityFee = newFee;
    }
    
    function updatePartyFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "DGEAPE: Party Fee must be between 0 and 10");
        partyFee = newFee;
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFee[account] != excluded, "DGEAPE: Account is already the value of 'excluded'");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) public onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "DGEAPE: The buy back wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "DGEAPE: The marketing wallet is already this address");
    ...
    function updateCharitykWallet(address newCharityWallet) public onlyOwner {
        require(newCharityWallet != buyBackWallet, "DGEAPE: The charity wallet is already this address");
    ...
    function updatePartyWallet(address newPartyWallet) public onlyOwner {
        require(newPartyWallet != buyBackWallet, "DGEAPE: The party wallet is already this address");
...
contract DGEAPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() DividendPayingToken("DGEAPE_Dividend_Tracker", "DGEAPE_Dividend_Tracker") {
    	claimWait = 86400; // every 24 hours
        minimumTokenBalanceForDividends = 20071  * (10**18); //must hold 20071+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "DGEAPE_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DGEAPE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DGEAPE_Dividend_Tracker: Cannot update claimWait to same value");
