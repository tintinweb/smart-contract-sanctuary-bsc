...
contract SnailToken is Initializable {
  /*//////////////////////////////////////////////////////////////
  ...
  function buy() external payable {
    uint256 teamCut = (msg.value * fee) / 100;
    uint256 userCut = msg.value - teamCut;

    _owner.transfer(teamCut);
    emit Bought(msg.sender, userCut, userCut * rate, block.number);
  }
  ...
  function sell(
    ...
    require(nonce == addressNonce[msg.sender], "COUPON_USED");

    bytes32 digest = keccak256(
      abi.encode(CouponType.Sell, snailAmount, nonce, msg.sender)
    );
    require(_isVerifiedCoupon(digest, coupon), "INVALID_COUPON");

    uint256 amount = snailAmount / rate;
    require(address(this).balance >= amount, "INSUFFICIENT_BALANCE");

    addressNonce[msg.sender] += 1;
    payable(msg.sender).transfer(amount);
    emit Sold(msg.sender, amount, snailAmount, block.number);
  }
