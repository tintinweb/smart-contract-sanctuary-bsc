...
contract BNBInvestFeb17{
    ...
    struct UserAutoPoolList {
        uint256 iduserautopool;
        uint256[] downlineautopooluserid;
	}
    ...
    mapping (uint256 => UserAutoPoolList) internal downlineautopooluser;
		...
		if (user.referrer == address(0)) {
			if (users[referrer].deposits.length > 0 && referrer != msg.sender) {
				user.referrer = referrer;
                    ...
                    if(getUserTotalReferrals(user.referrer)==0){
			...
			for (uint256 i = 0; i < 1; i++) {
				if (upline != address(0)) {
                    ...
                    if(getUserTotalReferrals(referrer)==1){
                            for(uint256 ap = 0; ap < totalUserAutoPool;ap++) {
                                uint256 randomno= random(totalUserAutoPool);
                                    if(totalUserAutoPool>0){
                                        if(randomno>=1 && randomno!=0 && randomno!=lastautopoolid){
                                            autopooltransfer(getUserAddress(randomno),ceo);
                                            
                                            UserAutoPoolList storage autopooldownline=downlineautopooluser[randomno];
                                            autopooldownline.iduserautopool = randomno;
                                            autopooldownline.downlineautopooluserid.push(getUserID(referrer));

                                            lastautopoolid = getUserID(referrer);

                                            users[getUserReferrer(referrer)].transType="depositwithautopooladmin";    
                                            ...
                                            if(lastautopoolid==0){
                                                ...
                                                UserAutoPoolList storage autopooldownline=downlineautopooluser[1];
                                                autopooldownline.iduserautopool = 1;
                                                autopooldownline.downlineautopooluserid.push(getUserID(referrer));
    ...
    function getUserAutoPoolList(uint256 iduserautopool) public view returns(uint256 downlineautopooluserid) {
        
        for(uint256 pa = 1; pa <= totalUserAutoPool;pa++) {
            return downlineautopooluser[iduserautopool].downlineautopooluserid[pa];
        }
		//return downlineautopooluser[iduser].downlineautopooluserid;
	}


    function getUserAutoPoolList2(uint256 _iduserautopool, uint256 _dataIndex) public view returns (uint256) {
        return downlineautopooluser[_iduserautopool].downlineautopooluserid[_dataIndex];
    }
