...
// File: DynamicSwap-v2-core/contracts/libraries/TransferHelper.sol
...
// File: DynamicSwap-v2-core/contracts/DynamicRouter02.sol
...
contract DynamicRouter02 is IDynamicRouter02 {
    ...
    address public immutable DMC_token;
    ...
    constructor(address _factory, address _WETH) public {
        ...
        pairImplementation = IDynamicFactory(_factory).pairImplementation();
    ...
    function createPair(
        ...
        address pair = IDynamicFactory(factory).createPair(tokenA, tokenB, vars, isPrivate, protectedToken, voteVars);
    ...
    ) external virtual override payable returns (uint liquidity) {

        address pair = IDynamicFactory(factory).createPair(WETH, token, vars, isPrivate, protectedToken, voteVars);
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        if (token != DMC_token) TransferHelper.safeTransfer(DMC_token, to, IERC20(DMC_token).balanceOf(address(this)));
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            IDynamicPair(pairFor(input, output)).swap(
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            IDynamicPair pair = IDynamicPair(pairFor(input, output));
    ...
    function pairFor(address tokenA, address tokenB) internal view returns (address pair) {
        ...
        pair = IDynamicFactory(factory).getPair(tokenA, tokenB);
    ...
    function getReserves(address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IDynamicPair(pairFor(tokenA, tokenB)).getReserves();
...
*/
    ...
    function getAmountsOut(uint amountIn, address[] memory path) public view override returns (uint[] memory amounts) {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            amounts[i + 1] = IDynamicPair(pairFor(path[i], path[i + 1])).getAmountOut(amounts[i], path[i], path[i + 1]);
    ...
    function getAmountsIn(uint amountOut, address[] memory path) public view override returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = IDynamicPair(pairFor(path[i - 1], path[i])).getAmountOut(amounts[i], path[i - 1], path[i]);
