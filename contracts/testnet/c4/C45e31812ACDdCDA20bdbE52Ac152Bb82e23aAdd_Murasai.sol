...
contract Murasai is ERC721, IERC2981, Ownable {
  ...
  mapping (address => uint8) private _alreadyMinted;

  constructor(address _beneficiary, address _royalties, string memory _initialBaseURI
  ...
  function addWhitelist(address addr, uint8 nb) public onlyOwner {
      whitelist[addr] = nb;
  }

  function addMultipleWhitelist(address[] memory addrs, uint8[] memory nb) public onlyOwner {
      uint gasCost = 0;
      uint gas = gasleft();
      for(uint16 i = 0; i < addrs.length && gasCost > gasleft();i++){
          addWhitelist(addrs[i], nb[i]);
          if(gasCost>0){
            gasCost = gas - gasleft();
          }
      }
  ...
  function mintWhitelist(uint8 amount) public payable {
    require(isWhitelistActive, "mint is closed");
    require(amount > 0 && whitelist[_msgSender()] >= amount, "you can't mint this number");
    require(msg.value >= price * amount, "Incorrect payable amount");
    _internalMint(_msgSender(), amount);
    whitelist[_msgSender()] -= amount;
  }

  function mintFree(uint8 amount) public {
    require(isFreeMintActive, "mint is closed");
    require(amount > 0 && freemint[_msgSender()] >= amount, "you can't mint this number");
    _internalMint(_msgSender(), amount);
    freemint[_msgSender()] -= amount;
  }

  function mint(uint8 amount) public payable {
    require(isActive, "mint is closed");
    require(amount > 0 && _alreadyMinted[_msgSender()] + amount <= MAX_MINT , "you can't mint this number");
    require(msg.value >= price * amount, "Incorrect payable amount");
    _internalMint(_msgSender(), amount);
    _alreadyMinted[_msgSender()] += amount;
  }

  function bendoPassMint(uint amount) public {
    require(isBendoPassActive, "mint is closed");
    require(amount > 0 && amount <= bendoPass.balanceOf(_msgSender(), idBendo), "you can't mint this number");
    require(true, "you have no bendo pass"); // vérifié si le owner possède un bendopass
    _internalMint(_msgSender(), amount);
