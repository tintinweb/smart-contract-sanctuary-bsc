...
contract Coin11 is Initializable, ContextUpgradeable, OwnableUpgradeable, ERC20 {
    ...
    function initialize () external initializer
    {
        ...
        uniswapV2Router02 = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
         WBNB = uniswapV2Router02.WETH();
         // Create a uniswap pair for this new token
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router02.factory())
            .createPair(address(this), WBNB);
        marketingTaxWallet = 0x513ba6EF9c7d235Ff4aB6236d20ba1d591F35701;
        liquidityTaxWallet = 0x513ba6EF9c7d235Ff4aB6236d20ba1d591F35701;
        denominator = 100;
        taxStatus = true;
        swapThreshold = 20000 wei; // The contract will only swap to ETH, once the fee tokens reach the specified threshold
        marketingTaxBuy = 5;
        liquidityTaxBuy = 10;
        marketingTaxSell = 5;
        liquidityTaxSell = 10 ;
    ...
    function handleTax(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                liquidityTokens += baseUnit * liquidityTaxBuy;

            } else if(to == address(uniswapV2Pair)) {
                tax += baseUnit * marketingTaxSell;

                tax += baseUnit *  liquidityTaxSell;
                ...
                marketingTokens += baseUnit * marketingTaxSell;

                liquidityTokens += baseUnit *  liquidityTaxSell;
                ...
                if(ethValue >= swapThreshold) {
                    ...
                    marketingTaxWallet.call{value: marketingETH}("");

                    
                    if(ethGained - (marketingETH + liquidityETH) > 0) {
                        marketingTaxWallet.call{value: ethGained - (marketingETH + liquidityETH)}("");
                    }
     ...
     */
    function setBuyTax( uint256 marketingBuy, uint256 liquidityBuy) public onlyOwner {
        marketingTaxBuy = marketingBuy;
        liquidityTaxBuy = liquidityBuy;
    }
    
    /**
     * @dev Sets tax for sells.
     */
    function setSellTax(uint256 marketingSell, uint256 liquiditySell) public onlyOwner {


        marketingTaxSell = marketingSell;
        liquidityTaxSell = liquiditySell;
     ...
     */
    function setTaxWallets(address marketingWallet, address liquidityWallet) public payable onlyOwner {
        marketingTaxWallet = marketingWallet;
        liquidityTaxWallet = liquidityWallet;
    }
    ...
    /**
    *@dev will update router swap if necessary.
     */
