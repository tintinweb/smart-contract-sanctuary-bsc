...
contract QatarInu is ERC20, Ownable {
   ...
   // uint256 public sellFee = 0;
    ...
    function setAntibotSystem(bool _antibotSystemEnable) external onlyAuthorize{
        antibotSystemEnable = _antibotSystemEnable;
    }

    function exclude(address account) external onlyOwner {
        require(_isExcluded[account] != true, "Account is already the value of 'excluded'");
        _isExcluded[account] = true;

        emit Exclude(account, true);
    }
    ...
    function _transfer(
        ...
        if (launchTime + 1 minutes > block.timestamp && to == uniswapV2Pair && antibotSystemEnable) { 
                return;  
        }
