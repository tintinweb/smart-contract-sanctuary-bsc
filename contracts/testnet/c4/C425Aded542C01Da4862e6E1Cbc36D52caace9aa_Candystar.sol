...
contract Candystar is BaseNFT, ReentrancyGuard, EIP712, IERC721Receiver {
    bytes32 public constant SIGN_ROLE = keccak256("SIGN_ROLE");

    //////////////////////////////////
    //          events
    /////////////////////////////////
    event MintNftEvent(uint256 requestId, address to, uint256 [] tokenIds, address owner) ;
    ...
    constructor(string memory baseUri, address managerAddr, uint256 _kind)
    BaseNFT("Candystar NFT","CSNFT", baseUri, managerAddr) EIP712("Candystar",  "v1.0.0") {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(BaseNFT) returns (string memory) {
        return string(abi.encodePacked(_baseURI(),"/",kind,"/", Strings.toString(tokenId)));
    }
    ...
    function mintNft(uint256 requestId, uint256 [] memory tokenIds, bytes memory signature) public {
        ...
        _mint(tos, tokenIds) ;
        emit MintNftEvent(requestId, _msgSender(), tokenIds, _msgSender()) ;
    }

    function _mint(address [] memory tos, uint256 [] memory tokenIds) private nonReentrant whenNotPaused {
    ...
    function getAllNftInfoByPage(uint256 page, uint256 limit) external view returns(uint256 [] memory, address [] memory, uint256, uint256 ) {
        ...
        return (nftInfoArr, owners, totalSupply(), kind);
    }

    //  get ed NFT Info By Page & owner, page start zero
    function getOwnerNftInfoByPage(uint256 page, uint256 limit, address owner) external view returns(uint256 [] memory, uint256, uint256) {
        uint256 startIndex = page * limit ;
        ...
        return (nftInfoArr, balanceOf(owner), kind);
    }
}