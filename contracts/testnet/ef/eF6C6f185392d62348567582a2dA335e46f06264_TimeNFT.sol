...
contract TimeNFT is ERC721, ERC721Enumerable, Ownable, ERC2981PerTokenRoyalties {
    ...
    event CreateTimeNFT(uint256 indexed id);
     ...
     */
    constructor(address VIP, string memory collection_name, string memory collection_symbol) {
        _setOwner(VIP);
...
contract Chubie_Factory is Ownable {
    address[] public newContracts;
    TimeNFT private TimeNFT_new;

    event TokenCreated(address tokenAddress);

    function createContract(address VIP, string memory collection_name, string memory collection_symbol) public {
        TimeNFT_new = new TimeNFT(VIP, collection_name, collection_symbol);
        emit TokenCreated(address(TimeNFT_new));
        address newContract = address(TimeNFT_new);
