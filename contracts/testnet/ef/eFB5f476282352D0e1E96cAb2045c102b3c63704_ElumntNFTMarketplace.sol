...
contract ElumntNFTMarketplace is Initializable, UUPSUpgradeable, OwnableUpgradeable, Verification {
    ...
    struct sweepCollection1155Data {
        bytes signature;
        string encodeKey;
        uint256 nonce;
        address owner;
    ...
    event CollectionOfferAccepted(uint256[] tokenIds, address from, address to, uint256[] creatorEarnings);
    event SweepBuyNFT(uint256[] tokenIds, address from, address to, uint256[] creatorEarnings);
    ...
    // function testsweepCollection721(sweepCollection721Data memory _buyData) public view onlyProxy returns (single721Data[] memory) {
    function testsweepCollection721(sweepCollection721Data memory _buyData) public view onlyProxy returns (mint721Data memory) {
        single721Data[] memory nftsData = _buyData.nftsData;
        mint721Data memory mintData = mint721Data(
            nftsData[0].metadata,
            payable(msg.sender),
            nftsData[0].nft
    ...
    function sweepCollection721(sweepCollection721Data memory _buyData) external payable onlyProxy {
        ...
        uint256 totalPlatformShare;
        uint256[] memory tokenIds;
        uint256[] memory creatorEarnings;
        for(uint8 i = 0; i < nftsData.length; i++) {
            uint256 singleNFTAmount = nftsData[i].amount;
            uint256 tokenId = nftsData[i].tokenId;
            if(nftsData[i].tokenId == 0) {
                mint721Data memory mintData = mint721Data(
                    nftsData[i].metadata,
                    payable(msg.sender),
                    nftsData[i].nft
            ...
            if(nftsData[i].percent > 0) {
                uint256 platformSharePercent = calculatePercentValue(nftsData[i].amount, nftsData[i].percent);
                singleNFTAmount = singleNFTAmount-platformSharePercent;
                totalPlatformShare += platformSharePercent;
            }
            uint256 royaltyPercent;
            if(nftsData[i].royalty > 0 && _buyData.creator != msg.sender) {
                royaltyPercent = calculatePercentValue(nftsData[i].amount, nftsData[i].royalty);
                singleNFTAmount = singleNFTAmount-royaltyPercent;
                totalRoyalty += royaltyPercent;
                creatorEarnings[i] = royaltyPercent;
            }
            amountToTransfer += singleNFTAmount;
            transfer721(nftsData[i].nft, _buyData.owner, msg.sender, nftsData[i].tokenId);
        }
        if(totalPlatformShare > 0) {
            if(_buyData.currency!=1) {
               transfer20(msg.sender, address(this), totalPlatformShare, _buyData.erc20token);
            }
        }
        if(totalRoyalty > 0) {
            if(_buyData.currency==1) {
                payable(_buyData.creator).transfer(totalRoyalty);
            } else {
                transfer20(msg.sender, _buyData.creator, totalRoyalty, _buyData.erc20token);
            }
        }
        if(_buyData.currency==1) {
        ...
        emit SweepBuyNFT(tokenIds, _buyData.owner, msg.sender, creatorEarnings);
    }
    function sweepCollection1155(sweepCollection1155Data memory _buyData) external payable onlyProxy {
        require(!seenNonces[msg.sender][_buyData.nonce], "Invalid request");
        seenNonces[msg.sender][_buyData.nonce] = true;
        require(verify(msg.sender, msg.sender, _buyData.amount, _buyData.encodeKey, _buyData.nonce, _buyData.signature), "invalid signature");

        single1155Data[] memory nftsData = _buyData.nftsData;
        uint256 amountToTransfer;
        uint256 totalRoyalty;
        uint256 totalPlatformShare;
        uint256[] memory tokenIds;
        ...
        for(uint8 i = 0; i < nftsData.length; i++) {
            ...
            amountToTransfer += singleNFTAmount;
            transfer1155(nftsData[i].nft, _buyData.owner, msg.sender, nftsData[i].quantity, nftsData[i].tokenId);
        }
        if(totalPlatformShare > 0) {
            if(_buyData.currency!=1) {
                transfer20(msg.sender, address(this), totalPlatformShare, _buyData.erc20token);
    ...
    function buy721Owner(buy721DataOwner memory _buyData) external payable onlyProxy onlyOwner {
        uint256 tokenId = _buyData.tokenId;
        if(_buyData.tokenId == 0) {
            mint721Data memory mintData = mint721Data(
                _buyData.metadata,
        ...
        transfer721(_buyData.nft, _buyData.owner, _buyData.buyer, tokenId);
        payable(_buyData.owner).transfer(amountToTransfer);
        emit NftTransferred(tokenId, msg.value, _buyData.owner, msg.sender, royaltyPercent);
    }
    ...
    function buy1155Owner(buy1155DataOwner memory _buyData) external payable onlyProxy onlyOwner {
        uint256 tokenId = _buyData.tokenId;
        if(_buyData.tokenId == 0) {
            mint1155Data memory mintData = mint1155Data(
                payable(_buyData.owner),
        ...
        transfer1155(_buyData.nft, _buyData.owner, _buyData.buyer, _buyData.quantity, tokenId);
