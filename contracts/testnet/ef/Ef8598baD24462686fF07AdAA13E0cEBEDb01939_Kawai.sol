...
contract Kawai is ERC20, Ownable {
    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;
    IERC20 public constant WBNB =
        IERC20(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd); // WBNB
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (tokensForTeam > 0) {
                super._transfer(address(this), teamWallet, tokensForTeam);
            }
    ...
    function swapBack() private {
        ...
        uint256 liquidityTokens = (contractBalance * tokensForLiquidity) /
            totalTokensToSwap /
            2;
        uint256 amountToSwapForBNB = contractBalance - liquidityTokens;

        uint256 initialBNBBalance = WBNB.balanceOf(address(this));

        swapTokensForBNB(amountToSwapForBNB);

        uint256 bnbBalance = WBNB.balanceOf(address(this)) - initialBNBBalance;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        WBNB.approve(address(uniswapV2Router), bnbAmount);
