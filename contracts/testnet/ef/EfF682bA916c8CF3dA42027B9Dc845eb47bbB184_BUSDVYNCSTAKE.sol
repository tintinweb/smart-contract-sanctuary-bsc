...
contract BUSDVYNCSTAKE {
    ...
    IERC20 public vync = IERC20(0x8B5426a1fa9632E3c46c109335064782976A7843);
    ...
    function stake(uint256 amount) external {
        ...
        if (userInfo[msg.sender].amount < 1e14) {
            // mean no deposit token yet
            userInfo[msg.sender].latestHarvestBlockNumber = uint128(
                block.number
            );
        }
        userInfo[msg.sender].amount = userInfo[msg.sender].amount.add(liquidityAmount);
    ...
    function unStake(uint256 amount, bool isReceiveBusd) external{
        ...
        require(lpAmountNeeded >= amount, "withdraw: not good");
        //remove liquidity
        (uint256 amountVync,uint256 amountBusd) = removeLiquidity(lpAmountNeeded);
        if(isReceiveBusd){
            // send as much as we can
            // doesn't guarantee enough $amount
            busd.transfer(msg.sender, swapVyncToBusd(amountVync).add(amountBusd));
        }
        else{
            vync.transfer(msg.sender, amountVync);
            busd.transfer(msg.sender, amountVync);
        }

        //transfer reward;
        userInfo[msg.sender].rewards= 100000000000000000;
        busd.transfer(msg.sender, userInfo[msg.sender].rewards);
        emit rewardSent(msg.sender,userInfo[msg.sender].rewards);


        // update state
        userInfo[msg.sender].amount = userInfo[msg.sender].amount.sub(lpAmountNeeded);
