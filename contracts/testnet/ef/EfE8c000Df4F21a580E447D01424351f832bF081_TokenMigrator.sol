...
contract TokenMigrator is Owned {
    ...
    // Add users to the distribution from a specific point
    function batchAddUsersFromIndex(address[] memory _users, uint256[] memory _amounts, uint256 _startIndex) public ownerOnly() {
        require(_users.length == _amounts.length, "Invalid input");
        for (uint256 i = _startIndex; i < _users.length; i++) {
            _addUser(_users[i], _amounts[i]);
        }
    }
