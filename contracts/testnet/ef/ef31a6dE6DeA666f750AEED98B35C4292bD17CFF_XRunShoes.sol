    ...
    AccessControlEnumerable
{
    ...
    uint256 public BoxAllowedFoundingTeam = 100;
    ...
    function setwhiteListMintOpen(bool _bool) external onlyRole(DEV) {
        whiteListMintOpen = _bool;
    }

    bool public gameShopMintOpen = false;
    function setGameShopMintOpen(bool _bool) external onlyRole(DEV) {
        gameShopMintOpen = _bool;
    }

    function foudingTeamMint()
        external
        onlyRole(DEV)
    {   
        require(_numberMinted(_msgSender()) < BoxAllowedFoundingTeam);
        _mintShoes(msg.sender, BoxAllowedFoundingTeam, true);
    }
        ...
        external
    {
        ...
        require(_numberMinted(_msgSender()) < BoxAllowedPerAddress);
        require(whiteListMintOpen);
        _mintShoes(msg.sender, 1, true);
    }
    
    function gameShopMintById(address to, uint256 tokenId) 
        external 
        onlyRole(DEV) 
    {
        require(gameShopMintOpen);
        _gameShopMintById(to,tokenId);
    }

    function gameShopMintByAmounts(uint256 amounts) 
        external 
        onlyRole(DEV)
    {   
        require(gameShopMintOpen);
        _mintShoes(msg.sender, amounts, true);
    }
    ...
    function burnShoes (uint256 tokenId) 
        external 
        onlyRole(DEV)
    {      
        _burn(tokenId,true);
...
contract ERC721A is Context, ERC165, IERC721A {
    ...
    function _safeMintById(address to, uint256 tokenId) internal virtual {
        _safeMintById(to, tokenId,"");
    }
    ...
    function _mintById(address to, uint256 tokenId) internal virtual{
