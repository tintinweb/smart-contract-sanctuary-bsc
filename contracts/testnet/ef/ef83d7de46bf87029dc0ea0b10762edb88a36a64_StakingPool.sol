...
contract Upgradable {
    ...
    struct PoolInfo {
        ...
        uint256[] configs; // startDate(0), endDate(1), duration(2), endStakeDate(3), stakingLimit(4), stopPool(5), blockPool(6)
...
contract StakingPool is Upgradable {
    ...
    */
    function stakeToken(
        ...
        if (pool.configs.length >= 7) {
            require(pool.configs[6] == 0,"This pool has been Blocked");
        }
   ...
   */
    ...
    */
        ...
        poolExist(strs[0])
    {
        ...
        ERC1155(_tokenAddress).safeTransferFrom(address(this),_to,  _tokenId, 1, "");
        emit ClaimRewardNFT(_to, address(this), _signer, _tokenId, 1, strs[0], strs[1]);
    }
    ...
    */
    function canGetReward(string memory poolId) public view returns (bool) {
        ...
        if (pool.configs.length >= 7) {
            if (pool.configs[6] == 1) return false;
        }
   ...
   */
    function createPool(string[] memory strs, address[] memory addr, uint256[] memory data, uint256[] memory configs) external onlyAdmins {
        ...
        poolInfo[strs[0]].configs.push(0);  //Block pool
        totalPoolCreated += 1;
   ...
   */
    ...
    */
     ...
     * @param poolId: poolId
    */
    function setBlockPool(string memory poolId, bool value ) external onlyAdmins poolExist(poolId) {
        PoolInfo storage pool = poolInfo[poolId];
        require(block.timestamp < pool.configs[1], "this pool has been ended");
        if (pool.configs.length >= 7) {
            require(pool.configs[5] == 0,"This Pool is already stop");
