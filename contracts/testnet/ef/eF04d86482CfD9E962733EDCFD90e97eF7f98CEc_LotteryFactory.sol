...
contract LotteryFactory is AccessControl {
    ...
    event MintLotteryTicket(address owner, Lottery.LotteryTickets[] lotteryTickets);
     ...
     */
    function mintBatchLotteryTicket(uint8 _amountTicket) external payable hasMintRunning {
        ...
        _mintLotteryTicket(_msgSender(), _amountTicket);
     ...
     */
    function _mintLotteryTicket(address _owner, uint8 _amountTicket) internal {
        Lottery.LotteryTickets[] memory tickets = new Lottery.LotteryTickets[](_amountTicket);
        for (uint i = 0; i < _amountTicket; i++) {
            ...
            Lottery.LotteryTickets memory newTicket = Lottery.LotteryTickets(block.timestamp, block.timestamp + 7 days, numberLucky, tokenId, rank, _owner);
            tickets[i] = newTicket;
            lotteryOfUser[_owner][_lotteryIdCounter.current()].push(newTicket);
            lotteryTicket[tokenId] = newTicket; 
            lotteryToday[_lotteryIdCounter.current()].tokenIds.push(tokenId); 
            _tokenIdCounter.increment();
        }
        emit MintLotteryTicket(_msgSender(), tickets);
    } 
    ...
    */
    function generateWinningNumber(uint256 _pancakeNumber) external onlyRole(MINTER_ROLE)  { 
        ...
        lotteryCurrent.pancakeNumberWin = uint32(_pancakeNumber) % 1000000;
