...
contract Biticoin is ERC20, Ownable {
    ...
    uint256 public tSupply = 8000000 * (10**18);

    mapping(address => bool) public excludedFromFees;
    mapping(address => bool) public automatedMarketMakerPairs;
    mapping(address => bool) blacklisted;

    constructor() ERC20("Biticoin", "BITI") {
    ...
    onlyOwner
    {
        require(pair != dexPair, "The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(excludedFromFees[account] != excluded, "Already excluded");
    ...
    function _updateTotalBuyFee() internal {
        totalBuyFee = burnFee.add(teamBuyFee);
    }

    function _updateTotalSellFee() internal {
        totalSellFee = burnFee.add(teamSellFee);
    }
    ...
    function _transfer(
        ...
        require(!blacklisted[sender], "Address is blacklisted");
        require(
            swapEnable ||
        ...
        bool excludedAccount = excludedFromFees[sender] ||
        excludedFromFees[recipient];

        if (!swapping || !automatedMarketMakerPairs[recipient] || !automatedMarketMakerPairs[sender]) {
            swapping = true;
            ...
            if (excludedAccount) {
                ...
                if (burnFee > 0) {
                    ...
                    super._transfer(sender, deadAddress, tokensToBurn);
                }

                if (automatedMarketMakerPairs[recipient]) {
                    if (teamSellFee > 0) {
                        uint256 tokensToTeam = amount.mul(teamSellFee).div(100);
                        super._transfer(sender, teamWallet, tokensToTeam);
                    }
                } else {
                    if (teamBuyFee > 0) {
