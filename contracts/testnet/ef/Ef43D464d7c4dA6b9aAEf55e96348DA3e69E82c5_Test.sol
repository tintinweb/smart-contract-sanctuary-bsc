...
contract Test is Ownable {
    ...
    mapping(address => WillLib.Account[]) private _benDetails; // reverse of testament address is bens beneficiary is owner
    ...
    function getWill()
        ...
        view
        returns (
            WillLib.Account[] memory beneficiaries,
    ...
    {
        return (
            _will[msg.sender].beneficiaries,
            _will[msg.sender].tokens,
            _will[msg.sender].nfts,
            _will[msg.sender].signRecheck,
            _will[msg.sender].signDate,
            _will[msg.sender].withdrawCount
        );
    }

    //set will
    function setWill(
        WillLib.Account[] calldata beneficiaries,
    ...
    function _setBenDetails(WillLib.Account[] calldata beneficiaries) private {
        for (uint256 index = 0; index < beneficiaries.length; index++) {
            WillLib.Account memory t;
            t.wallet = msg.sender; //
    ...
    function GetBeneficiaryDetails()
        public
        view
        returns (WillLib.Account[] memory)
    {
        return _benDetails[msg.sender];
    }
...
library WillLib{
  ...
  function getBeneficiary(Will storage self,address beneficiary) public view returns  (uint256 share,string memory title){
    for (uint256 index = 0; index < self.beneficiaries.length; index++) {
      if (self.beneficiaries[index].wallet == beneficiary) {
        return (self.beneficiaries[index].share,self.beneficiaries[index].title);
      } 
    }
  }
  
  function setBeneficiaries(Will storage self , Account[] memory beneficiaries) public {
    
    for (uint256 index = 0; index < beneficiaries.length; index++) {
      self.beneficiaries.push(beneficiaries[index]);
    }
  }

  function setTokens(Will storage self, address[] memory tokens) public {
    for (uint256 index = 0; index < tokens.length; index++) {
      Token memory t = Token({tokenAddress : tokens[index],initialBalance : 0});
      self.tokens.push(t);
    }
  }


  function setNfts(Will storage self ,NFT[] memory nfts ) public {
    for (uint256 index = 0; index < nfts.length; index++) {
      self.nfts.push(nfts[index]);
    }
  }
}
