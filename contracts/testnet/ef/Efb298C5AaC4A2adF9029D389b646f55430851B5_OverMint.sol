...
interface FeeManager {
    function addAsset(address asset, address _feeReceiver, uint256 percent) external;
    function distributionFee(address asset, address _paymentToken, uint256 amount) external;
}
contract OverMint is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => mapping(address => bool)) public paymentToken;
    mapping(bytes => bool) public usedSignatures;
    address public feeManager;
    uint256 public transactionFee;
    function initialize(address _feeManager) initializer public {
    ...
    function setFeeToAddress(address _feeManager) public onlyRole(ADMIN_ROLE) {
        feeManager = _feeManager;
    }
    ...
    function addToken(address asset, address _feeReceiver, uint256 percent, address[] calldata _paymentToken) external onlyRole(ADMIN_ROLE){
        assetToken[asset] = true;
        FeeManager(feeManager).addAsset(asset, _feeReceiver, percent);
        for(uint256 i=0; i < _paymentToken.length; i++){
            paymentToken[asset][_paymentToken[i]] = true;
        }
        emit AddToken(asset,_feeReceiver,percent,_paymentToken,block.timestamp);
    }
    ...
    function removePaymentTokenByAsset(address asset, address[] calldata _paymentToken) external onlyRole(ADMIN_ROLE){
        for(uint256 i =0; i < _paymentToken.length; i++){
            paymentToken[asset][_paymentToken[i]] = false;
        }
        emit RemovePaymentToken(asset, _paymentToken);
    }
    ...
    function matchTransaction(
        ...
        require(paymentToken[addresses[1]][addresses[2]],"OverMint: invalid paymentToken.");
        ...
        require(IERC20Upgradeable(addresses[2]).balanceOf(_msgSender()) >= values[1],"OverMint: buyer doesn't have enough token to buy this item");
        require(IERC20Upgradeable(addresses[2]).allowance(_msgSender(), address(this)) >= values[1],"OverMint: buyer doesn't approve marketplace to spend payment amount");
        uint256 fee = transactionFee.mul(values[1]).div(1000);
        uint256 payToSellerAmount = values[1].sub(fee);
        IERC20Upgradeable(addresses[2]).safeTransferFrom(_msgSender(), addresses[0], payToSellerAmount);
        IERC20Upgradeable(addresses[2]).safeTransferFrom(_msgSender(), feeManager, fee);
        FeeManager(feeManager).distributionFee(addresses[1],addresses[2],fee);
    ...
    function matchOffer(
        ...
        bytes32 ethSignedMessageHash = ECDSAUpgradeable.toEthSignedMessageHash(criteriaMessageHash);
        require(ECDSAUpgradeable.recover(ethSignedMessageHash, signature) == addresses[0],"OverMint: invalid buyer signature");
        require(IERC721Upgradeable(addresses[1]).ownerOf(values[0]) == _msgSender(),"OverMint: seller is not owner of this item now");
        require(IERC20Upgradeable(addresses[2]).balanceOf(addresses[0]) >= values[1],"OverMint: buyer doesn't have enough token to buy this item");
        require(IERC20Upgradeable(addresses[2]).allowance(addresses[0], address(this)) >= values[1],"OverMint: buyer doesn't approve marketplace to spend payment amount");
        uint256 fee = transactionFee.mul(values[1]).div(1000);
        uint256 payToSellerAmount = values[1].sub(fee);
        IERC20Upgradeable(addresses[2]).safeTransferFrom(addresses[0], _msgSender(), payToSellerAmount);
        IERC20Upgradeable(addresses[2]).safeTransferFrom(addresses[0], feeManager, fee);
        IERC721Upgradeable(addresses[1]).safeTransferFrom( _msgSender(), addresses[0], values[0]);
        FeeManager(feeManager).distributionFee(addresses[1],addresses[2],fee);
