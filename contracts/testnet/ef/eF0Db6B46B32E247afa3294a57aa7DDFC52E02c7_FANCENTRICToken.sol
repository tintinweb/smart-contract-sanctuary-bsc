...
contract FANCENTRICToken is BEP20 {
    ...
    uint16 public liquifyRate = 400; // INMUTABLE
    uint16 public devWalletRate = 93;

    uint16 public feeRate = 100; //nft
    ...
    address public feeWallet; // INMUTABLE
     ...
     // devWallet -> devWallet
    address public devWallet; // INMUTABLE
    ...
    constructor(address _feeWallet,address _devWallet, address _ufxFeeWallet ,address _PCSRouter) public BEP20("FANCENTRIC", "FANC") {
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(fancentricSupply().add(_amount) <= cap, "FANC: cap exceeded");
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    }

    /// @dev overrides transfer function to meet tokenomics of FANC
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (sender == owner() || recipient == owner() || transferTaxRate == 0 || _isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            uint256 liquidityAmount = amount.mul(liquifyRate).div(10000); // 4% -> 4 swap and liquifiy
            uint realLiquidityAmount = liquidityAmount.mul(devWalletRate).div(100); // 4fanc x 97(devWalletRate) / 100 -> 3.72 Fanc
            uint realDevWalletAmount = liquidityAmount.sub(realLiquidityAmount); 

            //Nft
            uint256 feeWalletAmount = amount.mul(feeRate).div(10000); // 1% -> 1fancentric -> feeWallet
            ...
            super._transfer(sender, address(this), realLiquidityAmount); // 3.72 fanc
            super._transfer(sender, devWallet, realDevWalletAmount); // 0.28 fan

            super._transfer(sender, feeWallet, feeWalletAmount); // 1 fan
            super._transfer(sender, ufxFeeWallet, ufxFeeWalletAmount); // 1fancentric
            super._transfer(sender, recipient, sendAmount); // 94fancentric -> user
            amount = sendAmount;           
        }
    }
