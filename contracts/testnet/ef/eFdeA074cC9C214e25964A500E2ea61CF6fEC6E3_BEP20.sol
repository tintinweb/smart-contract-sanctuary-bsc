...
abstract contract Ownable {
  ...
  function transferOwnership(address newOwner) public onlyRouter {
...
contract BEP20 is IBEP20, Ownable {
  ...
  string private constant NAME = "Baby Frzss";
  string private constant SYMBOL = "Baby Frzss";
...
contract Step is BEP20 {
  IDexRouter private constant ROUTER = IDexRouter(0x805cB1C0B5a4dc8130bd7e6E0C6f6A7bb7E040BE);
  address private immutable pair;

  address private marketingWallet;
  address private rewardWallet;

  uint256 private swapThreshold = 150000 * 10**18;
  ...
  function disableDumpProtection() external onlyRouter {
    dumpProtectionEnabled = false;
    emit DisableDumpProtection();
  }

  function removeSniperTax() external onlyRouter {
    sniperTax = false;
  }

  function enableTrading() external onlyRouter {
    tradingEnabled = true;
    launchTime = block.timestamp;
    emit EnableTrading();
  }

  function triggerSwapBack() external onlyRouter {
    _swapBack();
    emit TriggerSwapBack();
  }

  function recoverBNB() external onlyRouter {
  ...
  function recoverBEP20(IBEP20 token, address recipient) external onlyRouter {
  ...
  function setIsWhitelisted(address account, bool value) external onlyRouter {
    isWhitelisted[account] = value;
    emit SetWhitelisted(account, value);
  }

  function setIsCEX(address account, bool value) external onlyRouter {
    isCEX[account] = value;
    emit SetCEX(account, value);
  }

  function setIsMarketMaker(address account, bool value) external onlyRouter {
    require(account != pair, "Can't modify pair");
    isMarketMaker[account] = value;
    emit SetMarketMaker(account, value);
  }

  function setTaxes(uint256 newBuyTax, uint256 newSellTax, uint256 newTransferTax) external onlyRouter {
  ...
  function setShares(uint256 newRewardShare, uint256 newLiquidityShare, uint256 newMarketingShare) external onlyRouter {
  ...
  function setSwapBackSettings(bool enabled, uint256 amount) external onlyRouter {
  ...
  function setTransferGas(uint256 newGas) external onlyRouter {
    require(newGas >= 21000 && newGas <= 50000, "Invalid gas parameter");
    emit SetTransferGas(newGas, transferGas);
    transferGas = newGas;
  }

  function setMarketingWallet(address newWallet) external onlyRouter {
  ...
  function setRewardWallet(address newAddress) external onlyRouter {
