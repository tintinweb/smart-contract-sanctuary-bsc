...
contract IrenaUsers is Owned,VerifySignature {
    ...
    function emergencyWithdraw(uint256 _shoePid,uint256 _amount) public returns (UserShoeInfo memory){
     require(isEmergencyWithdrawable,"Admin should enable emergency withdrawable!");
      emergencyWithdrawTransfer(_shoePid,_amount);
      setDataAfterEmergencyWithdraw(_shoePid);
      return (userShoeDetails[msg.sender][_shoePid]);
   }

   function emergencyWithdrawTransfer(uint256 _shoePid,uint256 _amount) public {
     require(_amount <= userShoeDetails[msg.sender][_shoePid].stakedAmount,"Amount exceeds staked amount");
     require(irena.transfer(msg.sender,_amount),"insufficient irena to withdraw !");
   }

  function setDataAfterEmergencyWithdraw(uint256 _shoePid) public {
