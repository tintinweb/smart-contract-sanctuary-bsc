...
contract EverStaking is Ownable {
    ...
    uint256 public minTokenPerShare = 0;

    uint256 public maxStakePercentage = 80;

    //APR is in percent * 100 (e.g. 2500 = 25% = 0.25)
    uint256 public _7daysApr = 1824;
    uint256 public _30DaysApr = 3718;
    uint256 public _60DaysApr = 6424;
    uint256 public _90daysApr = 10311;

    uint256 public maxStakeIn7Days = 100000 * 10**18;
    uint256 public maxStakeIn30Days = 200000 * 10**18;
    uint256 public maxStakeIn60Days = 300000 * 10**18;
    uint256 public maxStakeIn90Days = 400000 * 10**18;

    uint256 public currentStakeTokenIn7Days;
    uint256 public currentStakeTokenIn30Days;
    uint256 public currentStakeTokenIn60Days;
    uint256 public currentStakeTokenIn90Days;
    ...
    function changeMaxToken(
        uint256 _7days,
        uint256 _30days,
        uint256 _60days,
        uint256 _90days
    ) external onlyOwner {
        maxStakeIn7Days = _7days;
        maxStakeIn30Days = _30days;
        maxStakeIn60Days = _60days;
        maxStakeIn90Days = _90days;
    }
    ...
    function newStake(uint256 amount, uint256 time) public {
        ...
        require(
            time == 7 || time == 30 || time == 60 || time == 90,
            "Invalid time. Select between 7,30,60 and 90 days"
        );
    ...
    function calcApyMul100(uint256 time) public view returns (uint256) {
        require(
            time == 7 || time == 14 || time == 30 || time == 60 || time == 0,
            "Invalid time. Select between 0,7,14,30 and 60 days"
        );
        if (time == 0) {
            time = 1;
        }
        ...
        uint256 rewardAmount = calculatRewardByAPY(amount, apr, 365);
    ...
    function calculateReward(
        ...
        if (!useApy) {
            ...
            rewardAmount = calculatRewardByAPY(amount, apr, totalTimeDays);
        }
        emit CalcReward(amount, time, totalTime, apr, rewardAmount);
        return rewardAmount;
    }
    ...
    function checkLimit(uint256 time, uint256 amount) internal returns (bool) {
        if (time == 7) {
            if (currentStakeTokenIn7Days.add(amount) > maxStakeIn7Days) {
                return false;
            } else {
                return true;
            }
        } else if (time == 30) {
            if (currentStakeTokenIn30Days.add(amount) > maxStakeIn30Days) {
                return false;
            } else {
                return true;
            }
        } else if (time == 60) {
            if (currentStakeTokenIn60Days.add(amount) > maxStakeIn60Days) {
                return false;
            } else {
                return true;
            }
        } else if (time == 90) {
            if (currentStakeTokenIn90Days.add(amount) > maxStakeIn90Days) {
                return false;
            } else {
                return true;
            }
    ...
    function setCurrentStakeAmount(uint256 time, uint256 amount) internal {
        if (time == 7) {
            currentStakeTokenIn7Days = currentStakeTokenIn7Days.add(amount);
        } else if (time == 30) {
            currentStakeTokenIn30Days = currentStakeTokenIn30Days.add(amount);
        } else if (time == 60) {
            currentStakeTokenIn60Days = currentStakeTokenIn60Days.add(amount);
        } else if (time == 90) {
            currentStakeTokenIn90Days = currentStakeTokenIn90Days.add(amount);
        }
    }
}
