...
interface EdafaceCollective721 {
    function mint(
...
interface EdafaceCollective1155 {
    function mint(
        string memory ipfsmetadata,
        address from,
        address to,
...
contract EdafaceTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    event Create(address indexed _from, address indexed _to, uint256 indexed tokenId);
...
// ids[0] - tokenId, ids[1] - amount, ids[2] -  nooftoken, ids[3] - nftType
    function _saleToken(
        ...
        uint256 val = pERCent(ids[1], serviceValue).add(ids[1]);
        uint256 _royal = 0;
        address create;
        if (ids[3] == 721) {
            (create, _royal) = EdafaceCollective721(impact721)
                .getCreatorsAndRoyalty(ids[0]);
        } else {
            (create, _royal) = EdafaceCollective1155(impact1155)
                .getCreatorsAndRoyalty(ids[0]);
        }
        if (
            ...
            if (roy != 0) {
                t.transferFrom(msg.sender, create, roy.div(10**Tokendecimals));
            }
            if (netamount != 0) {
                t.transferFrom(
                    msg.sender,
...
// ids[0] - tokenId, ids[1] - amount, ids[2] -  nooftoken, ids[3] - nftType
    function _acceptBId(
        ...
        if (roy != 0) {
            t.transferFrom(from, create, roy.div(10**Tokendecimals));
        }
        if (netamount != 0) {
    ...
    function batchMinting(
        ...
        _tid = _tid.add(size);
        if (nftType == 721) {
            EdafaceCollective721(impact721).mintBatch(
                ipfsmetadata,
                from,
                to,
                _tid,
            ...
            EdafaceCollective1155(impact1155).mintBatch(
                ipfsmetadata,
                from,
                to,
                supply,
    ...
    function getTokenId(uint256 id) external returns(uint256){
        _tid = _tid.add(id);
        return _tid;
    }
    function minting(
        ...
        if (nftType == 721) {
            EdafaceCollective721(impact721).mint(
                ipfsmetadata,
                from,
                to,
                royal,
            ...
            if(price > 0){
                orderPlace(id_,price,impact721,nftType);
            }
            else{
                emit OrderPlace(msg.sender, id_, price);
            }
        } else {
            EdafaceCollective1155(impact1155).mint(
                ipfsmetadata,
                from,
                to,
                supply,
            ...
            if(price > 0){
                orderPlace(id_,price,impact1155,nftType);
            }
            else{
                emit OrderPlace(msg.sender, id_, price);
            }
        }
        emit Create(from, to, id_);   
    }
    function _minting(
        string[] memory ipfsmetadata,
        uint256[] memory supply,
        ...
        uint256 _id = _tid;
        _tid = _tid.add(ipfsmetadata.length);
        EdafaceCollective1155(impact1155)._minting(ipfsmetadata, supply, royal, _id, msg.sender);
    }
        ...
        onlyOwner
    {
        ...
        EdafaceCollective721(impact721)._openUri(open);
        EdafaceCollective1155(impact1155)._openUri(open);
    }
}