...
function deposit(address _referredBy, uint stakePackagesIndex, uint monthlyPackageIndex)public{
   //  require(_packages[_referredBy].staked == true);
    ...
    //distribution according to the levels
      ...
      if (_stakeInfo[msg.sender].staked != true){
       _stakeInfo[msg.sender].holdingTime = block.timestamp;
      }
       if (_stakeInfo[msg.sender].staked != true){
      stakeInfo.lastStakingWithdraw = block.timestamp;
      }
//staking levels income distribution
   
   
      for (uint8 i = 0; i < stakingLevelIncomes.length; i++){
         if( _stakeInfo[_referredBy].holdingTime >= 2 minutes){
     address ref = _stakeInfo[msg.sender].referredBy;
 ...
 if(_stakeInfo[msg.sender].holdingTime >= 2 minutes && _stakeInfo[msg.sender].package >= 7000){
 ...
 if(_stakeInfo[msg.sender].holdingTime >= 2 minutes && _stakeInfo[msg.sender].package >= 10000){
...
//withdraw function
  ...
  function withdraw() public{
   ...
   require(block.timestamp >= _stakeInfo[msg.sender].lastStakingWithdraw + 2 minutes,  "No withdraw before one month");
    ...
    //  _stakeInfo[msg.sender]._stakingIncome += stakingIncomePerMonth;
     ...
     uint stakingIncomePerMonth = (sum * 15 ) /100;
      token.transfer(msg.sender, stakingIncomePerMonth);
       ...
       _stakeInfo[msg.sender].lastStakingWithdraw = block.timestamp;
  }
//withdraw monthly reward income
function _monthlyRewardWithdraw()public {
require(block.timestamp >= lastPoolwithdraw4 + 2 minutes, " No withdraw before one month");
...
//  require(superIncomes > 0, "Nothing for withdrawl");
 require(block.timestamp >= lastPoolWithdraw1 + 2 minutes, " No withdraw before one month");
...
require(block.timestamp >= lastPoolWithdraw2 +2 minutes, " No withdraw before one month");
...
require(block.timestamp >= lastPoolWithdraw3 + 2 minutes, " No withdraw before one month");
