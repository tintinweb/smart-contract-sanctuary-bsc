...
contract KojiGamepass is ERC721URIStorage, ERC165, Ownable, ReentrancyGuard {
    ...
    function mintGamepass(string memory _holder) external payable nonReentrant {
        ...
        require(keccak256(abi.encode(msg.sender)) == keccak256(abi.encode(bytes(_holder))), "Sender is not the same as requested recipient");
    ...
    function viewHashes(address _address, string memory _holder) external pure returns (bytes32, bytes32) {
        return(keccak256(abi.encode(_address)),keccak256(bytes(_holder)));
    }

    function viewBytes(address _address, string memory _holder) external pure returns (bytes memory, bytes memory) {
        return (abi.encode(_address), bytes(_holder));
    }
}
