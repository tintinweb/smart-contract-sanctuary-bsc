...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  function finishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature, uint _timestamp) external {
    ...
    if (energy >= energyConsumption || from == 0 || to < block.timestamp) {
      firstConsumeInDay = energyManager.updateEnergy(msg.sender, energyConsumption);
  ...
  function haveTodayFreeMission(address _user) public view returns (bool) {
    ...
    return todayFreeMission > 0 && todayFreeMission >= todayFreeMissionFinished[_user];
  }
  ...
  function _calculateReward(uint _mission, bool _firstConsumeInDay, bool _firstCompleteMission) private returns (uint) {
    ...
    if (_firstConsumeInDay) {
      ...
      if (rewarded[msg.sender].add(reward) >= maxReward) {
        reward = maxReward.sub(rewarded[msg.sender]);
      }
  ...
  function _validateFinishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature, uint _timestamp) private returns (bool) {
    require(block.timestamp + finishGameTimeOut >= _timestamp, "PVE: signature time out");
  ...
  // TODO remove
  function validateSignature(uint _mission, uint[] calldata _heroIds, bytes memory _signature, uint _timestamp) external view returns (bool) {
    bytes32 digest = _buildDigest(_mission, _heroIds, _timestamp);
    address signer = ECDSAUpgradeable.recover(digest, _signature);
    require(signer == contractAdmin, "MessageVerifier: invalid signature");
    require(signer != address(0), "ECDSAUpgradeable: invalid signature");
  ...
  function _validateHeroRight(uint[] calldata _heroIds) private view {
    for(uint i = 0; i < _heroIds.length; i++) {
      address currentRentingHero = marketPlace.currentRentingHero(_heroIds[i]);
      if (currentRentingHero != msg.sender) {
  ...
  function _validateItemRight(uint[] calldata _itemIds) private view {
    for(uint i = 0; i < _itemIds.length; i++) {
      address currentRentingItem = marketPlace.currentRentingItem(_itemIds[i]);
      if (currentRentingItem != msg.sender) {
        require(itemNft.ownerOf(_itemIds[i]) == msg.sender, "PVE: item invalid");
      }
    }
  }
