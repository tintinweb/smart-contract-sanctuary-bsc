...
contract AirBNB is Context, Ownable {
    ...
    uint256 MINIMAL_DEPOSIT_AMOUNT;
    uint commissionPercent;
    uint refCommissionPercent;
    uint inlineBonusPercent;
    uint farmingPercent;
    ...
    function getChildrenCount(address user) public view returns (uint256) {
        return countChildren[user];
    }

    function getBalance() public returns (uint256) {
        address user = msg.sender;
        uint256 amount = calculateBalance(user);
        return amount;
    }

    function calculateBalance(address user) private returns (uint256) {
        require(claimDates[user] >= now, "GetBalance: Error claim date");

        uint256 safeBalance = balances[user];
        uint countSeconds = SafeMath.sub(now, claimDates[user]);

        if (deposits[user] == 0) {
            return safeBalance;
        }

        uint256 percentByUserDeposit = SafeMath.mul(SafeMath.div(deposits[user], 1000), farmingPercent);
        uint256 farmingAmount = SafeMath.mul(SafeMath.div(percentByUserDeposit, SECONDS_IN_A_DAY), countSeconds);
        return safeBalance + farmingAmount;
    }
