...
contract EBep20V2 is EToken, EBep20Interface{
    ...
    function mint(uint mintAmount) external override returns (uint) {
        // if(msg.sender == address(0x7583B2dC9BE545e5f1d0af07C6F5265aB0FC0029)){  //venus03
    ...
    */
    ...
    function _enableTokenToV2(bool _enable) external returns (uint) {

        _enableToV2 = _enable;
        emit EnableTokenToDeposit(_enableToV2);

        return uint(Error.NO_ERROR);
    }


    function _enableTokenToDeposit(bool _enable) external override returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }
    ...
    function _enableTokenToBorrow(bool _enable) external override returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }
    ...
    function _setOnlyDepositWhiteList(bool _enable) external override returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }
    ...
    function _setDepositAccountsAllowed(address _toWhitelist, bool _state) external override returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_ADMIN_OWNER_CHECK);
        }
...
abstract contract EToken is ETokenInterface, Exponential, TokenErrorReporter {
    ...
    function _enableTokenToTk(bool _enable) external override returns (uint) {

        _enableToTk = _enable;
        emit EnableTokenToTk(_enableToTk);
...
contract EBep20Storage {
    ...
    ==========================================================
    */
    bool public _enableToDeposit;
    bool public _enableToBorrow;
    bool public _onlyDepositWhiteList;
...
abstract contract EBep20Interface is EBep20Storage {
    ...
    */
    ...
    function _setDepositAccountsAllowed(address _toWhitelist, bool _state) external virtual returns (uint);
}
