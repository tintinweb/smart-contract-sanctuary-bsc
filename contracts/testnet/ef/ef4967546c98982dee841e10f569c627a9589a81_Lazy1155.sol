...
contract Lazy1155 is Initializable,BEP1155Upgradeable, OwnableUpgradeable, BEP1155BurnableUpgradeable {
   event Approve(
    ...
    function lazyMint(uint256 royal, uint256 nooftoken, string memory ipfsmetadata, address payable from,uint256 amount, string memory bidtoken) public payable{
         _tid = _tid.add(1);
        uint256 id_ = _tid.add(block.timestamp);
        _mint(msg.sender, id_, nooftoken, "");
        _setTokenURI(id_, ipfsmetadata);
