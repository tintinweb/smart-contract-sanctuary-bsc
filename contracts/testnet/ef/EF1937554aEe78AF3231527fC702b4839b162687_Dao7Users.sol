...
contract Dao7Users is Ownable {
    ...
    function register(uint256 inviteCode) public {
        require(!usersMap.inserted[msg.sender],"already registered");
        
        address inviteAddr = usersMap.keys[inviteCode];

        usersMap.inserted[msg.sender] = true;
        usersMap.validReferral[msg.sender] = false;
        usersMap.inviter[msg.sender] = inviteAddr;
        usersMap.registerTime[msg.sender] = block.number;
        usersMap.indexOf[msg.sender] = usersMap.keys.length;
        usersMap.keys.push(msg.sender);
        if(inviteCode>0){
            usersMap.lowerUsers[inviteAddr].push(msg.sender);
        }
    }
