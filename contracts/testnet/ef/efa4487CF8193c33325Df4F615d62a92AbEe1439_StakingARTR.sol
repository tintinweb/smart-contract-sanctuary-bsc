...
contract StakingARTR is Ownable {
    ...
    mapping (address => uint) holder_CanClaim;
    ...
    mapping (address => uint) holder_Unstaked;
    ...
    uint totalStaked;
    uint currentVestingTime;
    uint currentVestingAmount;
    ...
    function Stake(uint months, uint amount) payable external {
        ...
        totalStaked += amount;
        holder_CanClaim[holder] = block.timestamp + currentVestingTime;
    ...
    function Unstake(uint amount) payable external {
        ...
        require(amount < currentVestingAmount, "Amount higher than you can unstake");
        require(block.timestamp >= holder_CanClaim[holder], "It's not time yet");
        holder_Amount[holder] -= amount;
    ...
    function testClaim() payable external {
        ARTR.transfer(msg.sender, 1000 * 10e18);
    }

    function setVesting(uint time, uint amount) public onlyOwner {
        currentVestingAmount = amount;
        currentVestingTime = time;
    }
