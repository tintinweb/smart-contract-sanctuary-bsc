...
contract SignatureVerifier is ISignatureVerifier, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    returns (bool) {
        address signer = keccak256(
            abi.encode(nonce, receiver, tokenRarity)
        ).ethSignedMessage().recover(signature);
        require(signer == TRUSTED_PARTY, "Invalid request");
        
        nonceUsed[nonce] = true;
        emit UsedNonce(receiver, nonce, "Claim Darling");
        return true;
    }
    ...
    returns (bool) {
        address signer = keccak256(
            abi.encode(nonce, receiver, tokenRaritys)
        ).ethSignedMessage().recover(signature);
        require(signer == TRUSTED_PARTY, "Invalid request");
        
        nonceUsed[nonce] = true;
        emit UsedNonce(receiver, nonce, "Claim Fashion Item");
        return true;
    }
}
