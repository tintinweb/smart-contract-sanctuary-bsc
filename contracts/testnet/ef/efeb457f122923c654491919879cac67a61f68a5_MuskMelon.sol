...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function updateArray(uint index,address _address) private returns(bool){
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        require(findindex(tokenId,owner),"not update tokeid array");
...
contract MuskMelon is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(string memory _cid) public payable {
    ...
    tokenurl[supply] = _cid ;
    // for (uint256 i = 1; i <= _mintAmount; i++) {
    //   _safeMint(msg.sender, supply + i);
    // }
  }
    function uniqmint(string memory _cid) public payable {
        uint256 supply = totalSupply()+1;
        require(!paused);
        require(supply <= uniqnft,"100 random mint is close");
  ...
  function setuniqnft(uint256 _newlimit) public onlyOwner {
    uniqnft = _newlimit;
  }
