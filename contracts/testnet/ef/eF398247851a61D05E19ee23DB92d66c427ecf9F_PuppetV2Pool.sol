...
contract PuppetV2Pool {
    ...
    function borrow(uint256 borrowAmount) external {
        require(_token1.balanceOf(address(this)) >= borrowAmount, "Not enough token balance");

        // Calculate how much WETH the user must deposit
        uint256 depositOfWETHRequired = calculateDepositOfWETHRequired(borrowAmount);
        
        // Take the WETH
        _token0.transferFrom(msg.sender, address(this), depositOfWETHRequired);

        // internal accounting
        deposits[msg.sender] += depositOfWETHRequired;

        require(_token1.transfer(msg.sender, borrowAmount));
    ...
    function _getOracleQuote(uint256 amount) private view returns (uint256) {
        (uint256 reservesWETH, uint256 reservesToken) = PancakeLibrary.getReserves(
            _factory, address(_token0), address(_token1)
        );
        return PancakeLibrary.quote(amount.mul(10 ** 18), reservesToken, reservesWETH);
    }
}