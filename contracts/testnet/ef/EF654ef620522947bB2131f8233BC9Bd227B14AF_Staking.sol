...
contract Staking is Ownable {
    ...
    event OnHarvest(uint256 amount, uint256 amount2);
    ...
    function enterStaking (uint256 _amount, uint256 _period) external {
        ...
        userTotalWeightedStakedAmount[_msgSender()] += _amount * getMultiple(_msgSender(), userStakedTime[_msgSender()]);
        totalWeightedStakedAmount += _amount * getMultiple(_msgSender(), userStakedTime[_msgSender()]);
    ...
    function harvest () public {
        ...
        emit OnHarvest(rewardPending(_msgSender()), userReward[_msgSender()]);
    }
    ...
    function getSingleReward(address _adr, uint256 _id) public view returns (uint256) {
        uint256 _rewards;
        _rewards = userStakedAmount[_adr][_id] * getMultiple(_adr, _id) * (block.timestamp - userLastCollect[_adr]) * rewardPerYear / (totalWeightedStakedAmount * (365*24*60*60));
