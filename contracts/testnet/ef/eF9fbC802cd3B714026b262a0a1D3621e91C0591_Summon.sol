...
contract Summon is Ownable, Pausable {
    ...
    event SummonHero(address summoner, string heroId, uint256 tokenId);
    ...
    function summon(string memory _heroId, bytes memory signature)
        external
        whenNotPaused
        returns (uint256)
    {
        require(!executed[_heroId], "Summon: nonce already used");

        address _sender = _msgSender();

        address signer = keccak256(abi.encode(_heroId, _sender))
        ...
        acceptedToken.transferFrom(_sender, owner(), fee);
        uint256 tokenId = IHero(heroSmartContractAddress).mintWithSummon(_sender);
        executed[_heroId] = true;
        emit SummonHero(_sender, _heroId, tokenId);
        return tokenId;
    }

    function summonNoSigner(string memory _heroId)
        external
        whenNotPaused
        returns (uint256)
    {
