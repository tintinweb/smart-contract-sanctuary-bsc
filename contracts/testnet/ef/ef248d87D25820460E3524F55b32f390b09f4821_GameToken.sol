...
contract GameToken is ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    address private usdtContract = 0xb3fC29D8656c1c5950D0dEaBf5f1287Cf741159C;
    // prd config
    ...
    function reGive(address _to, uint tokenId) public onlyOwner returns (uint256) {
        address owner = ERC721.ownerOf(tokenId);
        require(owner == _to, "token is not belong to the owner");
        // burn the old nft
        _burn(tokenId);
        _tokenId++;
        // mint a new one
    ...
    function getAllTokenIdsByOwner(address owner) public view onlyOwner returns (uint256[] memory ids){
    ...
    function transferToken(address tokenAddress, address recipient, uint256 amount) public onlyOwner {
        IFeeProtocol(tokenAddress).transfer(recipient, amount);
    }
