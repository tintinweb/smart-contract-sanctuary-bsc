...
contract FastMoney is Ownable, Pausable, VerifySignature {
    ...
    mapping(uint256 => uint256[2]) roundInfo;
    ...
    mapping(uint256 => uint256) public roundPartnersCount;
    ...
    function topUpBalance(uint256 amount) public whenNotPaused {
    ...
    function upgradeLevel(uint256 levelID) public {
        ...
        require(users[userID].balance >= priceLevel, "Small balance");
        // require(
        //     verify(
        //         msg.sender,
        //         msg.sender,
        ...
        users[userID].balance = (users[userID].balance).sub(priceLevel);
    ...
    function claimProfit(uint256 amount) external {
        ...
        require(amount <= users[userID].profit, "Insufficient funds profit");
        amount = amount > 0 ? amount : users[userID].profit;

        users[userID].profit = users[userID].profit.sub(amount);
        users[userID].received = users[userID].received.add(amount);
        if (!paymentToken.transfer(msg.sender, amount)) {
            paymentToken.transfer(users[2].wallet, amount);
        }
        //     emit WithdrawEvent(msg.sender, _value, block.timestamp);
    }

    function transferProfitToBalance(
        uint _amount,
        ...
        uint256 userID = userWallets[msg.sender];
        require(_amount <= users[userID].profit, "Insufficient funds profit");
        ...
        users[userID].balance = users[userID].balance.add(users[userID].profit);
        users[userID].profit = 0;

        levelUp(userID, users[userID].level);
    }

    function getUserInfo(uint256 userID, bool payments)
        public
        view
        returns (
            uint256,
    ...
    function getUserInfoByWallet(address _wallet, bool payments)
        external
        view
        returns (
            uint256,
    ...
    {
        return getUserInfo(userWallets[_wallet], payments);
    }
    ...
    function levelUp(uint256 userID, uint256 levelID) private {
        ...
        if (level == 0)
            roundInfo[round] = [roundInfo[round][0] + 1, roundInfo[round][1]];
        else roundInfo[round] = [roundInfo[round][0], roundInfo[round][1] + 1];
    ...
    function getRoundInfo(uint256 round)
        public
        view
        returns (uint256[2] memory levelsRound)
    {
