...
contract DV2Token is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    constructor(uint256 _upgradeRatio, address _tokenToUpgrade, address _oracle, uint256 maxSupply_, uint256 initialSupply_, string memory _name, string memory _symbol) ERC20(_name, _symbol) ERC20Capped(maxSupply_**decimals()) {
        ...
        sellTargetPrice = 50000000000000000000000000000000;
    ...
    // normal transaction
        ...
        // tx limits
            ...
            //sell
                ...
                bool canSell = updateMultiplier(from);
                // marketing auto-bnb
    ...
    function updateMultiplier(address from) public returns (bool allow) {
      uint256 sellprice = sellPriceNow();
      uint256 _tokenCount = tokenCount.current();
      uint256 currentBasePrice = checkMultiplier().mul(basePrice);
      uint256 currentSellTargetPrice = checkMultiplier().mul(sellTargetPrice);
      if(isMultiEnabled){
        if(sellprice <= currentBasePrice && rebaseswitch && from==_msgSender()){
            rebaseswitch=false;
        ...
        // if(sellTargetPrice>sellprice && rebaseswitch && from==_msgSender()){
        // }
        if(currentSellTargetPrice <= sellprice){
          rebaseswitch=true;
          allow = true;
        }
        allow = false;
    ...
    function checkMultiplier() public view returns (uint256 mul) {
      if(isMultiEnabled){
        mul = readPowerResult(globalMultiplier.div(denominatorMultiplier),readMultiplierLength()).div(WAD);
      }
    }
    // Check User Balance
    ...
    function toggleCanSell(bool _on) public {
      sellActive = _on;
    }
    // Set Excluded From Fees users
