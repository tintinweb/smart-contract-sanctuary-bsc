...
contract ShibaFarm {
    ...
    IERC20 public token = IERC20(0x743DC5Addf8ae78334b79B2A9B3B01AeFbD83C0e); 
	uint256 constant public INVEST_MIN_AMOUNT = 500000e18; // 1000000 Shiba
	uint256[] public REFERRAL_PERCENTS 	= [500, 0, 0, 0, 0, 0, 0, 0, 0, 0];
	uint256[] public SEED_PERCENTS 		= [1000, 500, 400, 300, 300, 300, 300, 400, 500, 1000];
    ...
    uint256 constant public mark = 166;

	uint256 public totalInvested;
	uint256 public totalRefBonus;
	address marketing1=0xE9A2310C3FA6938cdca8b008Ae7b9CDF8f3B941E;
    ...
    function reInvestFunction( uint8 plan, uint256 amounts) internal {
	...
	function withdraw() public {
        ...
        uint256 marketingFee=(totalAmount)*5/100;
        totalAmount=totalAmount-(totalAmount*5)/100;
        token.transfer(msg.sender,totalAmount);
        token.transfer(marketing1,marketingFee);
    ...
    function reinvest() public {
        User storage user = users[msg.sender];
       uint256 depositAmount = getUserDividends(msg.sender);
       uint256 seedAmount1 = getcurrentseedincome(msg.sender);
       depositAmount = depositAmount.add(seedAmount1);
       uint256 referralBonus = getUserReferralBonus(msg.sender);
		if (referralBonus > 0) {
			user.bonus = 0;
			depositAmount = depositAmount.add(referralBonus);
		}
        user.withdrawnseed = user.withdrawnseed.add(seedAmount1);
       require(user.deposits.length > 0);
        reInvestFunction(0,depositAmount);       
