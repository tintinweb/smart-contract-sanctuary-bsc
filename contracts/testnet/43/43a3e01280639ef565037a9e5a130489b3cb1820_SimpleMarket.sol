...
contract SimpleMarket is EventfulMarket, DSMath {
    ...
    // function activatedTrade(uint id) external synchronized  onlyOwner  returns (bool active) {
    //     return offers[id].locked = true;
    // }
    // function deActivatedTrade(uint id ) external synchronized  onlyOwner  returns (bool active) {
    //     return offers[id].locked  = false;
    // }
        ...
        returns (bool)
    {
        ...
        safeTransferFrom(offer.buy_gem, msg.sender, address(this), spend);
        ...
        returns (bool success)
    {
        ...
        OfferInfo memory offer = offers[id];
        delete offers[id];
        uint256 feeAmount = (((offer.pay_amt)/(100)) * (feePercent) );
