...
contract UberTaxiDate is Ownable {
    ...
    function setCommissions(uint256 cIds, address user, uint256 buyTime, uint256 vehicleTypes, uint256 reward) public onlyController {
        commissions[cIds] = Commission(user, buyTime, vehicleTypes, reward);
        myCommissionsIds[users[user].welMember].push(cIds);
    }

    function setUserVehicleIds(uint256 ids, address user) public onlyController {
