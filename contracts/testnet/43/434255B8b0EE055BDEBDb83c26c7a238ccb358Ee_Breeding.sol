...
contract Breeding is ReentrancyGuard, Ownable {
    address constant MILK_TOKEN_ADDRESS = 0x3eFA66aB2b1690e9BE8e82784EDfF2cF2dc150e0;
    ...
    uint public maxRecoveryTime = 200 days;
    uint[5] public baseRecoveryTime;
    uint public breedingPrice = 90*10**18;
    uint256 rarityProbability = 50;
    uint256 breedProbability = 80;
    uint256 cowProbability = 90;
    ...
    function breed(uint _cowTokenId, uint _bullTokenId) public {
        ...
        require(maxRecoveryTime / 1 days > _old / 1 days, "Bull is too old");
        ...
        uint256 weight =10 - (maxRecoveryTime - _old) * 10 / maxRecoveryTime;
        newBreeding.unlockTime = block.timestamp + baseRecoveryTime[bullAttr.rarity] + baseRecoveryTime[bullAttr.rarity] * weight /10;
        breedingVaults.push(newBreeding);

        cowNfts.transferFrom(msg.sender, address(this), _cowTokenId);
        bullNfts.transferFrom(msg.sender, address(this), _bullTokenId);
    ...
    function claimCattle(uint _bullTokenId) public {
        ...
        require(breedingIdx < breedingVaults.length, "Invalid breeding");
        require(breedingVaults[breedingIdx].owner == msg.sender, "Not owner");
        require(block.timestamp > breedingVaults[breedingIdx].unlockTime, "Wait until Recovery");

        INftMinter minter = INftMinter(address(minterAddr));
        ICowNFT cowNfts = ICowNFT(address(minter.cowNftColl()));
        ICowNFT.CattleAttr memory cowAttr = cowNfts.attrOf(breedingVaults[breedingIdx].cowTokenId);

        IBullNFT bullNfts = IBullNFT(address(minter.bullNftColl()));
        IBullNFT.CattleAttr memory bullAttr = bullNfts.attrOf(breedingVaults[breedingIdx].cowTokenId);
        ...
        if(randProbability < cowProbability) {
            minter.mintCow(breedingVaults[breedingIdx].owner, rarityOfNew, breedOfNew);
        } else {
            minter.mintBull(breedingVaults[breedingIdx].owner, rarityOfNew, breedOfNew);
        }
        ...
        emit ClaimedNewCattle(breedingVaults[breedingIdx].owner, breedingVaults[breedingIdx].cowTokenId, breedingVaults[breedingIdx].bullTokenId);
    ...
    function withdrawMilk() public onlyOwner{
        IERC20 payContract = IERC20(address(MILK_TOKEN_ADDRESS));
    ...
    returns(
        ...
        uint256 _blength = breedingVaults.length;
        uint k = 0;
        uint256[] memory cowTokenIds;
        uint256[] memory raritis;
        uint256[] memory cowBreeds;
        uint256[] memory bullTokenIds;
        uint256[] memory bullBreeds;
        uint256[] memory unLockTimes;
        for(uint i = 0 ; i < _blength; i++) {
        ...
        if(k == 0 ) {
            return (new uint256[](0),new uint256[](0),new uint256[](0),new uint256[](0),new uint256[](0),new uint256[](0) );
        }
