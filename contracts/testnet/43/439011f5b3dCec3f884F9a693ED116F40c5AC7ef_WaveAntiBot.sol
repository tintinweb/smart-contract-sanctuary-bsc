...
contract WaveAntiBot is Initializable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(address => mapping(address => bool)) public blacklistedUsers;
    ...
    function configAnti(address token, address pairToken, address routerExchange, address factoryExchange, uint256 amountLimitPerTrade, uint256 amountAddedPerLock, uint256 timeLimitPerTrade, uint256 totalLockBlocks) external whenNotPaused {

        require(token != address(0), 'Invalid Token');
        require(IAntiBotToken(token).owner() == _msgSender(), 'Only Owner!');
        ...
        uint256 balance = IERC20(antiBotInfo.token).balanceOf(antiBotInfo.pair);
        require(balance == 0, 'Can Not Config Now!');
        antiBotInfo.owner = _msgSender();
        antiBotInfo.token = token;
    ...
    function enableAntiBot(address token, bool enabled, uint256 amount) external payable whenNotPaused {
        AntiBotInfo storage antiBotInfo = antiBotInfos[token];
    ...
    function calculateMaxTrade(AntiBotInfo memory antiBotInfo, TradeLogInfo memory tradeLogInfo) internal view returns (uint256) {
        ...
        result = antiBotInfo.amountLimitPerTrade.add(blocks.mul(antiBotInfo.amountAddedPerLock));
        return result;
    }
    ...
    function onPreTransferCheck(address sender, address recipient, uint256 amount) external whenNotPaused {
        ...
        if (!antiBotInfo.enabled) {
            ...
            if (whitelistedUsers[antiBotInfo.token][sender] || whitelistedUsers[antiBotInfo.token][recipient]) {
                return;
            }
            ...
            if (sender == antiBotInfo.pair || recipient == antiBotInfo.pair) {
                if (blacklistedUsers[antiBotInfo.token][sender] || blacklistedUsers[antiBotInfo.token][recipient]) {
    ...
    function WhitelistUsers(address token, bool status, address[] memory users) external whenNotPaused {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            whitelistedUsers[token][users[i]] = status;
        }
    }
    ...
    function BlacklistUsers(address token, bool status, address[] memory users) external whenNotPaused {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            require(users[i] != address(0), 'Invalid User');
            blacklistedUsers[token][users[i]] = status;
        }
    }

    function getOwner(address token) public view returns (address) {
        return IAntiBotToken(token).owner();
