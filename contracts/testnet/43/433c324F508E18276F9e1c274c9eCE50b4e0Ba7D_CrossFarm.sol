...
contract CrossFarm is Node, ICrossFarm, BaseRelayRecipient, SessionManager {
     ...
     */
    function deposit(uint256 _pid, uint256 _amount, UserRewardBehavior memory behavior)
     ...
     */
    function withdraw(uint256 _pid, uint256 _amount, UserRewardBehavior memory behavior) 
    ...
    public virtual override validPid(_pid) returns (uint256 vested) {
    ...
    public virtual override validPid(_pid) returns (uint256 harvested) {
    ...
    public virtual override validPid(_pid) returns (uint256 staked) {
     ...
     */
    function switchCollectOption(uint256 _pid, CollectOption newOption, UserRewardBehavior memory behavior) public override validPid(_pid) {
        ...
        behavior.crssBusd = crssBusd;
        if (
            FarmLibrary.switchCollectOption(pool, user, behavior, newOption, msgSender, feeParams, nodes, farmParams, feeStores)
        ) {
    ...
    function _payTransactonFee(
        ...
        if (actionParams.isUserAction && principal > 0) {
            if (address(payerToken) == nodes.token) {
                ...
                feesPaid = CrossLibrary.transferFeesFrom(payerToken, address(this), principal,
                    feeRates[actionParams.actionType], feeStores, nodes.token
...
enum RewardOption { FullProcess, IndividualOnly, NoProcess }
...
interface ICrossFarm {
    ...
    function deposit(uint256 _pid, uint256 _amount, UserRewardBehavior calldata behavior) external returns (uint256 deposited);
    function withdraw(uint256 _pid, uint256 _amount, UserRewardBehavior memory behavior) external returns (uint256 withdrawn);
    ...
    function switchCollectOption(uint256 _pid, CollectOption newOption, UserRewardBehavior memory behavior) external;
...
library FarmLibrary {
        ...
        if (address(sourceLpToken) != address(0) && lpAmount > 0) {
            if (sourceLpToken == nodes.token) {
                ...
                (uint256 amount0, uint256 amount1) 
                    = IMaker(nodes.maker).sim_removeLiquidity(token0, token1, lpAmount);

                newCrss += _sim_swapExactNonCrssForCrss(ITaker(nodes.taker), nodes.token, token0, amount0);
                newCrss += _sim_swapExactNonCrssForCrss(ITaker(nodes.taker), nodes.token, token1, amount1);
            }
        }
    }
    ...
    function changeCrssInXTokenToLpInFarm(
        ...
        if (targetLpToken != address(0) && amountCrssInXToken > 0) {
            ...
            if (targetLpToken == nodes.token) {
                ...
                require( (nodes.token == token0 || INode(nodes.maker).pairFor(nodes.token, token0) != address(0))
                && (nodes.token == token1 || INode(nodes.maker).pairFor(nodes.token, token1) != address(0) ), 
                "Direct swap path not found");

                amount0 = _swapExactCrssForNonCrss(ITaker(nodes.taker), nodes.token, token0, amount0); // From farm to farm
                amount1 = _swapExactCrssForNonCrss(ITaker(nodes.taker), nodes.token, token1, amount1); // From farm to farm
                require(amount0 > 0 && amount1 > 0, "Swap failed");

                uint256 balance0 = IERC20(targetLpToken).balanceOf(address(this));
                ...
                (uint256 _amount0, uint256 _amount1, ) = IMaker(nodes.maker).wired_addLiquidity(
                    token0, token1, amount0, amount1, 0, 0,
    ...
    function takePendingCollectively(
        ...
        if (lpSupply > 0) {
            ...
            if (periodic) {
                ...
                if (subPoolPending > 0) {
                    newLpAmountInFarm = changeCrssInXTokenToLpInFarm(
                        address(pool.lpToken), nodes, subPoolPending, feeParams.stakeholders
                    );
            ...
            if (periodic) {
                ...
                if (halfToCompound > 0) {
                    newLpAmountInFarm = changeCrssInXTokenToLpInFarm(
                        address(pool.lpToken), nodes, halfToCompound, feeParams.stakeholders
                    );
        ...
        for (uint256 pid = 0; pid < len; pid++) {
            ...
            uint256 newLpAmount;
            {
                ...
                newLpAmount = changeCrssInXTokenToLpInFarm(
                    address(pool.lpToken), nodes, accumCrss, feeParams.stakeholders
                );
            }
    ...
    function updatePool(
        ...
        if (pool.lastRewardBlock < block.number) {
            ...
            if (0 < pool.allocPoint && 0 < lpSupply) {
                ...
                ICrssToken(nodes.token).mint(feeStores.accountant, teamEmission);
    ...
    function finishRewardCycle2(
        ...
        uint256 pendingEstimate = ( behavior.pendingCrss +  behavior.pendingPerBlock * (block.number - behavior.blockNo) ) * reserveBusd / reserveCrss;
        ...
        if (pendingEstimate > threshold) { // if larger than 50 usd
            payReferralComission(pool, user, msgSender, feeParams, nodes); // Pay commission before pool.accRewardPerShare will change.
        ...
        } else if (collectiveAssets > threshold) { // if larger than 50 usd
            takeIndividualReward(pool, user);
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"e7e541901ed0d1b0d1eaa40c156d0707fdebbf61b4f99e31d65f5b48b0c49f5f" // init code hash
                        )
                    )
                )
            )
