...
contract Xegora is Ownable {
    ...
    uint private _RefYouRecive;
    uint private _RefFriendReceive;
    
    uint public referralCount;

    mapping (address => Referral) private _Referral;
    event savingsReferralEvent(address _myAddress, Referral);
   ...
   //-----------------------------------------
    ...
    function addReferral(string calldata code,uint youReceive,uint friendReceive,bool isActive,uint256 createDate) public {

     require(_Referral[msg.sender].Wallet != address(0), "Dadeh Tekrari");
