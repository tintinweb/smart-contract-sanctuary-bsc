...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address _rewardTokenAddress, uint256 _minimumTokenBalanceForDividends) DividendPayingToken("Dividen_Tracker", "Dividend_Tracker", _rewardTokenAddress) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = _minimumTokenBalanceForDividends * (10**18);
    }
...
contract AAA is ERC20, Ownable {
    ...
    uint256 public minInviterAmount = 1;
    ...
    constructor(
        ...
        address fundWalletAddress_
    ) payable ERC20("AAA", "AAA")  {
        rewardToken = rewardTokenAddress_;
        marketingWalletAddress = marketingWalletAddr_;
        fundWalletAddress = fundWalletAddress_;
        ...
        dividendTracker = new TokenDividendTracker(rewardTokenAddress_, 2000);
        ...
        excludeFromFees(fundWalletAddress, true);
    ...
    function setMinInviterAmount(uint256 amount) public onlyOwner {
        minInviterAmount = amount;
    }
    ...
    function _transfer(
        ...
        bool shouldSetInviter = balanceOf(to) == 0 && inviter[to] == address(0) && from != uniswapV2Pair && amount >= minInviterAmount;
        if (shouldSetInviter) {
            inviter[to] = from;
        }
        ...
        if(takeFee && (automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to])) {
            ...
            uint256 FFee = amount.mul(fundFee).div(100);
            uint256 DFee = amount.mul(deadFee).div(100);

            uint256 fees = LFee.add(RFee).add(MFee).add(FFee).add(DFee);
            ...
            if(FFee > 0) super._transfer(from, fundWalletAddress, FFee);
            if(DFee > 0) super._transfer(from, deadWallet, DFee);
            super._transfer(from, address(this), fees.sub(MFee).sub(FFee).sub(DFee).add(IFeeCurrent));
        }
    ...
    function takeInviter(address from, address to, uint256 amount) private returns(uint256,uint256)  {
            ...
            if (IFeeAmount > 0){
                    ...
                    for (int256 i = 0; i < 8; i++) {
                        ...
                        if(curTAmount > 0) super._transfer(from, cur, curTAmount);
