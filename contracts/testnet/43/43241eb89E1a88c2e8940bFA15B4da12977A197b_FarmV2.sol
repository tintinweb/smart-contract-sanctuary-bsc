...
contract FarmV2 {
    ...
    mapping(address => uint40) timeSaveDay;
    mapping(address => uint8) contSaveDay;
    ...
    function getCount(address owner) public view returns(uint8) {
        return contSaveDay[owner];
    }

    function getTime(address owner) public view returns(uint40) {
        return timeSaveDay[owner];
    }
    ...
    function sync(Event[] memory _events) public hasFarm  returns (Farm memory) {
        ...
        uint40 varTime = timeSaveDay[msg.sender];
        uint8 varCont = contSaveDay[msg.sender];
        if(varCont == 0) {
            timeSaveDay[msg.sender] = uint40(block.timestamp);
            contSaveDay[msg.sender] = varCont + 1;
        }
        uint8 contVar = contSaveDay[msg.sender];
        contSaveDay[msg.sender] = contVar + 1;
        
        if(varCont > 3){
            require(block.timestamp >=  varTime + 15 minutes , "TIME_IN_FISHING");
            contSaveDay[msg.sender] = 0;
        }
