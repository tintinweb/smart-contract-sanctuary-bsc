...
interface events {
   /**
    ...
    event CastVote(uint96 productID, uint256 indexedOf, address indexed who);

    event CancelTransaction(uint96 tokenID, address indexed user, bool releaseStatus, uint256 time);
...
contract Storage is events, Ownable {
    ...
    mapping(uint96 => BuyersTick) private hasBuyerTick;
    ...
    modifier checkBlacklist(uint96 productID) {
    ...
    function _storeProducts(address purchaseToken, address from, address to, uint256 amountInDecimals) internal returns(uint96) {
    ...
    function _cancelTrx(uint96 productID, bool release) internal {
        address seller = allProducts[productID].seller;
        address _buyer = allProducts[productID].buyer;
        require(msg.sender == seller || msg.sender == _buyer, "Not_Found" );
        if (msg.sender == _buyer && release == false) {
            hasBuyerTick[productID].hasBuyerTick = release;
            _contractTransfer(allProducts[productID].token, seller, allProducts[productID].amount);
            allProducts[productID].isCompleted = true;
            emit CancelTransaction(productID, msg.sender, false, block.timestamp);
        } else {
            if (msg.sender == _buyer && release == true) {
                hasBuyerTick[productID].hasBuyerTick = release;
                emit CancelTransaction(productID, msg.sender, true, block.timestamp);
            } else {
    ...
    function _updateBuy(uint96 productID) internal returns(uint256 subGas,uint256 remnant, address to) {
    ...
    function _updateAndEmit(uint96 productID) internal {
        ...
        if (lent > maxNumbersofCouncils) revert CompletedMember();
        if (lent == maxNumbersofCouncils) {
            raisedDispute[productID].votesEnded = uint128(block.timestamp + votingEllapseTime);
        }
    ...
    function _tip(uint96 productID, address who) internal {
        ...
        if(s_qualified != address(0)) {  
            _contractTransfer(s_token, s_qualified, (s_amount - _payment));  
    ...
    function hasBuyerConfirmed(uint96 productID) external view returns(bool) {
...
abstract contract Rigelp2pPriceSet is Storage {
    ...
    function initializedDispute(uint256 unlistedDisputeTokensForSeller, uint256 unlistedDisputeTokensForBuyer, uint256 defaultWrongVote, uint256 _pendingTime
    )  external onlyOwner {        
        _initializedDispute(unlistedDisputeTokensForSeller, unlistedDisputeTokensForBuyer, defaultWrongVote,_pendingTime);
    }
...
contract RigelDecentralizedP2PSystem is Rigelp2pPriceSet {
    ...
    */
    ...
    function makeSellOrder(IERC20 purchaseToken, uint256 amount, address from, address to, address referral) external returns(uint256){   
        ...
        uint96 id = _storeProducts(address(purchaseToken), from, to, amountInDecimals);
    ...
    function cancelTransactions(uint96 productID, bool release) external completed(productID) invalid(productID) {
        _cancelTrx(productID, release);
    }
