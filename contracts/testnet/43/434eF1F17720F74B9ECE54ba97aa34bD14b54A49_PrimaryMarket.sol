...
contract PrimaryMarket is ReentrancyGuard, Authorizable, ERC1155Holder {  
    ...
    constructor(address _nftAddress, address _busdAddress, address _oCoinAddress,uint _price, address _wbnbAddress, address _walletAddress, address _pancakeRouterAddress){
        ...
        _busdContract = IERC20(_busdAddress);
 ...
 function ListNFT(
    ...
    uint256 price,
    uint256 amount
  ) public nonReentrant onlyAuthorized returns(uint256){
...
function oCoin_in_BNB(uint _ocoinQuant) public view returns(uint){
    ...
    revPath[0] = path[1];
    revPath[1] = path[0];
    uint temp = _ocoinQuant*oCoinPriceInDollar/1e18;


     uint256 bnbPrice = _pancakeRouter.getAmountsOut(temp, revPath)[revPath.length - 1];
    ...
    bool isBNB
        ...
        if(isBNB){
             price = idToMarketItem[itemId].price * _amount;
             finalPrice = price;
            ...
            uint price_after_max_discount_per = (price*maxOCoinUsedPrcnt)/10000;
            ...
            finalPrice = price - buyer_desired_discount;
           ...
           // idToMarketItem[itemId].seller.transfer(msg.value);
            ...
            price = idToMarketItem[itemId].price * _amount;
            uint priceInBUSD = BNB_in_BUSD(price) ; 
             finalPrice = priceInBUSD;

             if(_ocoinAmount>0){

             uint max_ocoin_quant_in_busd = ((maxOCoinUsedQTY * oCoinPriceInDollar)/1e18);
             uint price_after_max_discount_per = (priceInBUSD*maxOCoinUsedPrcnt)/10000;
            ...
            finalPrice = priceInBUSD - buyer_desired_discount;
            ...
            require(_maxBUSDAmount >= finalPrice,'required exceed the max amount');
    ...
    function removeItem(uint256 itemId) public nonReentrant onlyAuthorized{
  ...
  // Getter and Setter Functions 
    ...
    function calcPricesBNB(uint _itemId, uint _amount) external view returns(uint256[] memory){
        uint[] memory values = new uint[](3);

          uint price = idToMarketItem[_itemId].price * _amount;
          ...
          uint max_ocoin_quant_in_bnb = oCoin_in_BNB((maxOCoinUsedQTY * oCoinPriceInDollar)/1e18);
            ...
            uint finalPrice = price - low_max_eligible_discount;
            uint temp = BNB_in_BUSD(low_max_eligible_discount)/oCoinPriceInDollar;
     ...
     function calcPricesBUSD(uint _itemId, uint _amount) external view returns(uint256[] memory){
        uint[] memory values = new uint[](3);

        uint price = idToMarketItem[_itemId].price * _amount;
            uint priceInBUSD = BNB_in_BUSD(price) ; 
            uint finalPrice = priceInBUSD;
            ...
            finalPrice = priceInBUSD - low_max_eligible_discount;
            uint ocoinQuant = low_max_eligible_discount/oCoinPriceInDollar;
            values[0] = priceInBUSD;
            values[1] = finalPrice;
            values[2] = ocoinQuant;
    ...
    function setBUSDAddress(address _token) external onlyOwner{
      path[1] = _token;
        _busdContract = IERC20(_token);
    }

    function busdAddress() external view returns(address){
        return address(_busdContract);
    }

    function setoCoinAddress(address _token) external onlyOwner{
        _oCoinContract = IERC20(_token);
    }

     function setoCoinMaxPercent(uint _amt) external onlyOwner{
        maxOCoinUsedPrcnt = _amt;
    }

    function ocoinAddress() external view returns(address){
        return address(_oCoinContract);
    }
    ...
    function setMAXoCoinPercent(uint _maxPer) external onlyOwner{
        maxOCoinUsedQTY = _maxPer;
    }
    ...
    function withdrawBUSD() external onlyOwner {
      SafeERC20.safeTransfer(
        _busdContract,
        wallet,
        _busdContract.balanceOf(address(this))
      );
    ...
    // check balances
    function feeReceivedBNB() external view returns(uint) {
     return address(this).balance;
    }

    function feeReceivedBUSD() external view returns(uint) {
     return _busdContract.balanceOf(address(this));
    }
...
// TODO :: buy me extra param for BUST amount
...
// TODO :: buy me extra param for BUST amount, maxOCoinUsedQTY setter, ocoin price in constructor
// update path when update address
// 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 - router
