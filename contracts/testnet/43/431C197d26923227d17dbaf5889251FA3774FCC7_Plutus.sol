...
contract NODERewardManagement {
    ...
    function _getNodeAvailableRewardAmountOf(address account, uint256 creationTime)
        public
        view
        returns (uint256)
    {
    ...
    function _getNodesClaimCounts(address account)
        external
        view
        returns (string memory)
    {
        ...
        uint256 nodesCount = nodes.length;
        NodeEntity memory _node;
        string memory _claimCounts = uint2str(nodes[0].claimCount);
        string memory separator = "#";
        ...
        for (uint256 i = 1; i < nodesCount; i++) {
            ...
            _rewardsAvailable = string(
                abi.encodePacked(
                    _rewardsAvailable,
                    separator,
                    uint2str(_getNodeAvailableRewardAmountOf(account, _node.creationTime))
                )
            );
        }
...
contract Plutus is ERC20, Ownable, PaymentSplitter {
    ...
    IJoeRouter02 public pancakeV2Router;
    ...
    mapping(address => uint256) public usersWithdrawals;
        ...
        PaymentSplitter(payees, shares)
    {
        ...
        IJoeRouter02 _pancakeV2Router = IJoeRouter02(routerAddress);

        address _pancakeV2Pair = IJoeFactory(_pancakeV2Router.factory())
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        ...
        emit UpdateUniswapV2Router(newAddress, address(pancakeV2Router));
        pancakeV2Router = IJoeRouter02(newAddress);
        address _pancakeV2Pair = IJoeFactory(pancakeV2Router.factory())
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        pancakeV2Router.addLiquidityAVAX{value: ethAmount}(
            address(this),
            tokenAmount,
