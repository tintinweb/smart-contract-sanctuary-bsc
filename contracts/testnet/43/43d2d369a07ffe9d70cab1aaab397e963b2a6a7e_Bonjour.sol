...
contract Bonjour is ContextUpgradeable, OwnableUpgradeable {
    ...
    function updatereinvestExternalAmount(uint256 _amount) public onlyOwner {
        reinvestExternalAmount = _amount;
    }
    ...
    function checkLevelUptoNew(address _user) public view returns(uint256){
        uint256 lastinvstdate = users[_user].levelgainlastDate;
        if(lastinvstdate == 0){
            lastinvstdate = users[_user].investmentDate;
        }
    ...
    function checkAvailableUpto(address _user) public view returns(uint256){
     ...
     function claimOrTopop(uint256 _loopcnt, address[] memory _addresses, uint256[] memory _ramount)
     public
    {
        for (uint256 i = 0; i < _loopcnt; i++) {
           levelTotalgained[_addresses[i]] += _ramount[i];
           users[_addresses[i]].levelIncome += _ramount[i];
        }
