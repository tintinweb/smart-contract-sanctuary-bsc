...
//    gameMiningPool = IFOTAToken(0x0A4E1BdFA75292A98C15870AeF24bd94BFFe0Bd4); // TODO use this on mainnet deploy
  ...
  function deposit(uint _amount, PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    uint discount;
    uint amountOfCurrency = fundAmountInDollar  * (oneHundredPercentageDecimal3 - discount) / oneHundredPercentageDecimal3;
  ...
  function summonHero(address _user, uint _heroPrice) external onlyContractAdmin  returns (uint) {
    require(userRewards[_user] >= _heroPrice, "RewardManager: insufficient balance");
    userRewards[_user] -= _heroPrice;
    emit UserRewardChanged(_user, int(_heroPrice) * -1, userRewards[_user]);

    return _heroPrice;
  }

  function summonPrestigeHero(address _user, uint _heroPrice) external onlyContractAdmin returns (uint) {
    require(userRewards[_user] >= _heroPrice, "RewardManager: insufficient balance");
