...
contract VTokenYieldSource is ERC20, IProtocolYieldSource, Manageable, ReentrancyGuard {
  ...
  address public usdtAddress = address(0xA11c8D9DC9b66E209Ef60F0C8D969D3CD988782c);
  ...
  function _tokenToShares(uint256 _tokens) internal view returns (uint256) {
    ...
    if (_totalSupply == 0) {
      ...
      uint256 _exchangeMantissa = _totalSupply.div(vToken.balanceOf(address(this)));
      _shares = _tokens.mul(_exchangeMantissa);
    }
  ...
  function _sharesToToken(uint256 _shares) internal view returns (uint256) {
    ...
    if (_totalSupply == 0) {
      ...
      _tokens = _shares.mul(vToken.balanceOf(address(this))).div(_totalSupply);
    }
  ...
  function _depositToVenus(uint256 mintAmount) internal {
    ...
    _depositToken.safeIncreaseAllowance(address(vToken), mintAmount);
  ...
  function redeemToken(uint256 redeemAmount) external override nonReentrant returns (uint256) {
    ...
    require(IVToken(vToken).redeemUnderlying(redeemAmount) > 0, "VTokenYieldSource/redeemAmount-gt-zero");
