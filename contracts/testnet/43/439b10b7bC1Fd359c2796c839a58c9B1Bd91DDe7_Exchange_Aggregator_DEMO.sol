...
contract Exchange_Aggregator_DEMO is Context {
    ...
    */
    ...
    function swap(swapRoute[] memory xs, string memory selector, bool test) external {

        for (uint i = 0; i < xs.length; i++) {
            swapRoute memory x = xs[i];
            _internal_swap(x.router, x.tokens, x.amountIn, x.amountOutMin, selector);
        }
    ...
    */
    ...
    // Test to swap Token A with Token B on their pair with a sample forked Pancake Swap's Router.
    function _internal_swap(address router, address[] memory tokens, uint amountIn, uint amountOutMin, string memory selector) internal {
