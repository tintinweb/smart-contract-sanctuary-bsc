...
contract StiIdoFactory is OwnableUpgradeable {
    struct Contract {
        address account;
        uint index;
    }
    ...
    modifier onlyBanker() {
        require(bankers[_msgSender()], "Ownable: caller is not the backers");
        _;
    }

    function initialize() public initializer {
        __Ownable_init_unchained();
        bankers[address(this)] = true;
        bankers[0x6ceA57bD9FC0557fF80B3BeC3a758E8c2f3bAA62] = true;
    }

    function deleteIdo(address idoToken_) public onlyBanker {
    ...
    function createIdo(uint price_,  uint mode_, bool wHolder_, address[3] memory tokens_, uint[3] memory quotas_, uint[5] memory times_) public onlyBanker returns (address) {
        ...
        address ido = create(idoToken);

        IERC20(idoToken).transferFrom(_msgSender(), ido, idoAmount);
        IStiIdo(ido).init(idoAmount, price_, mode_, wHolder_, tokens_, quotas_, times_);
    ...
    function create(address idoToken_) internal returns(address ido) {
        bytes memory bytecode = type(StiIdo).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(idoToken_, block.timestamp));
        assembly {
    ...
    function getIdoList(uint stage_) public view returns(address[] memory opt) {
     ...
     function getIdoInfoBathch(uint stage_) public view returns(address[] memory idos, address[] memory tokens, string[] memory names, uint[8][] memory infos) {
        ...
        for (uint i; i < size; i++) {
            (tokens[i], names[i], infos[i]) = idoInfo(idos[i]);
        }
    } 

    function idoInfo(address ido_) public view returns(address  tokens, string memory names, uint[8] memory infos) {
        (address token, string memory name, uint mode, uint buyAmount, uint target, uint price, uint[5] memory times_) = IStiIdo(ido_).datas();
        tokens = token;
        names = name;
        infos = [mode, buyAmount, target,  price,  times_[0],  times_[1],  times_[2],  times_[3]];
    }

    
    function getIdoInfo(address ido_, address account_) public view  returns(address idoToken, string memory name, uint totalSupply,  bool isW, uint[3] memory quotas,  uint[8] memory info, uint[4] memory userData) {
...
interface IStiIdo {
    function viewInfo(address account_) external view returns(address idoToken, string memory name, uint totalSupply,  bool isW, uint[3] memory quotas,  uint[8] memory info, uint[4] memory userData);
    function datas()  external view returns(address idoToken, string memory name, uint mode, uint buyAmount, uint target, uint price,  uint[5] memory times_);
    function times() external view returns(uint first, uint begin, uint end, uint release);
    function init(uint idoAmount, uint price,  uint mode, bool wHolder, address[3] calldata tokens, uint[3] calldata quotas, uint[5] calldata times_) external;
    function editWhiteList(address[] calldata accounts, bool isAdd_) external;
...
contract StiIdo  {
    ...
    modifier onlyBanker() {
        require(IStiFactory(addr.factory).bankers(msg.sender), "Ownable: caller is not the backers");
        _;
    }

    modifier onlyFactory() {
        require(msg.sender == addr.factory, "Ownable: caller is not the factory");
        _;
    }

    constructor() {
        addr.factory = msg.sender;
    }

    function init(uint idoAmount_, uint price_,  uint mode_, bool wHolder_, address[3] calldata tokens_, uint[3] calldata quotas_, uint[5] calldata times_) public onlyBanker {
        addr.idoToken = IBEP20(tokens_[0]);

        address buyToken = tokens_[1];
        addr.usdt = IBEP20(buyToken);

        if (wHolder_) {
            meta.wHolder = wHolder_;
            addr.wHolderToken = tokens_[2];
        }

        meta.idoAmount = idoAmount_;
        
        uint buyDecimal =  10 ** addr.usdt.decimals();
        meta.price = price_;
        meta.target = quotas_[0] * buyDecimal;
        meta.privateQuota = quotas_[1] * buyDecimal;
        meta.publicQuota = quotas_[2] * buyDecimal;
        meta.mode = mode_;
        ...
        times.releaseEnd = times_[4];
    }

    function edit(uint price_, uint[3] memory quotas_, uint[5] memory times_) public onlyBanker {
        meta.price = price_;
    ...
    function editTime(uint stage_) public onlyBanker {
        ...
        if (stage_ == 0) {
            ...
            times.pri = tm - 30 ;
            times.pub = times.pri - 20;
            times.end = times.pub - 10;
            times.release = times.end - 2;
        }
    }
    ...
    function datas()  external view returns(address idoToken, string memory name, uint mode, uint buyAmount, uint target,   uint price,  uint[5] memory times_) {
        idoToken =  address(addr.idoToken);
        name = addr.idoToken.symbol();
        mode = meta.mode;
        ...
        times_[1] = times.pub;
        times_[2] = times.end;
        times_[3] = times.release;
        times_[4] = times.releaseEnd;
    }

    function viewInfo(address account_) public view returns(address idoToken, string memory name, uint totalSupply,  bool isW, uint[3] memory quotas,  uint[8] memory info, uint[4] memory userData) {
        quotas[0] = meta.mode;
        ...
        idoToken =  address(addr.idoToken);
        name = addr.idoToken.symbol();
        totalSupply = meta.idoAmount / idoDecimals;
    ...
    function decimals() public view returns(uint idoDecimals, uint buyDecimals) {
        idoDecimals = 10 ** addr.idoToken.decimals();
        buyDecimals = 1e18;
    }
    ...
    function viewUserInfo(address account_) public view returns(bool isW, uint quota, uint swapQuota, uint claimQuota, uint claimedQuota) {
        ...
        claimQuota = viewReleaseQuota(account_) / buyDecimals;
        claimedQuota = info.claimed / buyDecimals;
    }

    function viewReleaseQuota(address account_) public view returns(uint release) {
        User memory info = userInfo[account_];
        if (info.quota == 0 || block.timestamp <= times.release || info.quota == info.claimed) {
            return 0;
        }

        if (times.releaseEnd == 0 || block.timestamp >= times.releaseEnd) {
            return info.quota - info.claimed;
        }
        
        uint rate = info.quota * (times.releaseEnd - times.release); 
        uint tm = info.lastClaimTm == 0 ? times.release : info.lastClaimTm;
    ...
    function withdraw() external {
        ...
        require(user.quota == user.claimed, "already withdraw");

        uint releaseQuota = viewReleaseQuota(msg.sender);

        addr.idoToken.transfer(msg.sender, releaseQuota);
        user.claimed += releaseQuota;
    ...
    function purchase(uint amount_) external {
        ...
        addr.usdt.transferFrom(msg.sender, address(this), amount_);
        ...
        if (meta.mode != 1 && block.timestamp < times.pub) {
            if (meta.wHolder) {
                require(IERC721(addr.wHolderToken).balanceOf(msg.sender) > 0, "only whiteList");
            } else {
                require(user.isW, "only whiteList");
            }

            require(user.quota <= meta.privateQuota, "out of limit");
            emit Purchase(msg.sender, amount_, amount_);
            return;
        }

        require(user.quota <= meta.publicQuota, "out of limit");
        emit Purchase(msg.sender, amount_, amount_);
    }
