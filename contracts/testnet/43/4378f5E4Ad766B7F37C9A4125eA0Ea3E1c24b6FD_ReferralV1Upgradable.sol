    ...
    UUPSUpgradeable
{
    ...
    uint256 private maxLevels;

    uint256 private pendingReferralETH;
    uint256 private pendingReferralUSD;
    uint256 private pendingReferralStaking;
    ...
    event StakingReferralIncomeGenerated(
        address indexed referee,
        address indexed referrer,
        uint256 indexed amount,
        uint256 level,
    ...
    event StakingReferralRewardClaimed(
        address indexed referee,
        address indexed referrer,
        uint256 indexed amount,
        uint256 level,
    ...
    function initialize() public initializer {
        ...
        presaleContract = 0x31AAA234A1829490890722D7A2c9416525e13b8F;
        stakingContract;
    ...
    //         address[] memory userRefereeAddress,
    //         uint256 userRefereeCount,
    //         address[] memory userTeamAddress,
    //         uint256 userTeamCount,
    //         uint256 userTotalBusinessETH,
    //         uint256 userTotalBusinessUSD,
    //         uint256 userPendingRewardETH,
    //         uint256 userPendingRewardUSD,
    //         uint256 userPendingRewardStaking,
    //         uint256[] memory userRewardClaimedETH,
    //         uint256 userTotalRewardClaimedETH,
    //         uint256[] memory userRewardClaimedTimeETH,
    //         uint256[] memory userRewardClaimedUSD,
    //         uint256 userTotalRewardClaimedUSD,
    //         uint256[] memory userRewardClaimedTimeUSD,
    //         uint256[] memory userRewardClaimedStaking,
    //         uint256 userTotalRewardClaimedStaking,
    //         uint256[] memory userRewardClaimedTimeStaking
    //     )
    // {
    //     Account storage userAccount = accounts[_address];
    //     userReferrer = userAccount.referrer;
    //     userRefereeAddress = userAccount.referredAddresses;
    //     userRefereeCount = userRefereeAddress.length;
    //     userTeamAddress = userAccount.teamAddress;
    //     userTeamCount = userTeamAddress.length;
    //     userTotalBusinessETH = userAccount.totalBusinessETH;
    //     userTotalBusinessUSD = userAccount.totalBusinessUSD;
    //     userPendingRewardETH = userAccount.pendingRewardETH;
    //     userPendingRewardUSD = userAccount.pendingRewardUSD;
    //     userPendingRewardStaking = userAccount.pendingRewardStaking;
    //     userRewardClaimedETH = userAccount.rewardPaidETH;
    //     for (uint256 i; i < userRewardClaimedETH.length; i++) {
    //         userTotalRewardClaimedETH += userRewardClaimedETH[i];
    //     }
    //     userRewardClaimedTimeETH = userAccount.rewardPaidTimeETH;
    //     userRewardClaimedUSD = userAccount.rewardPaidUSD;
    //     for (uint256 i; i < userRewardClaimedUSD.length; i++) {
    //         userTotalRewardClaimedUSD += userRewardClaimedUSD[i];
    //     }
    //     userRewardClaimedTimeUSD = userAccount.rewardPaidTimeUSD;
    //     userRewardClaimedStaking = userAccount.rewardPaidStaking;
    //     for (uint256 i; i < userRewardClaimedStaking.length; i++) {
    //         userTotalRewardClaimedStaking += userRewardClaimedStaking[i];
    //     }
    //     userRewardClaimedTimeStaking = userAccount.rewardPaidTimeStaking;
    // }
    ...
    function getTotalReferralBonus() external view returns (uint256) {
        uint8 levelLength = uint8(levelRateReferral.length);
        uint8 referralBonus;

        for (uint8 i; i < levelLength; i++) {
            referralBonus += uint8(levelRateReferral[i]);
        }
        return referralBonus;
    }

    function getLevelRatesReferral() external view returns (uint256[] memory) {
        return levelRateReferral;
    }
    ...
    function getLevelRatesStaking() external view returns (uint256[] memory) {
        return levelRateStaking;
    }
    ...
    function getMaxLevels() external view returns (uint256) {
        return maxLevels;
    }

    function setMaxLevels(uint256 _value) external onlyOwner returns (uint256) {
        return maxLevels = _value;
    }
    ...
    function totalPendingRewards()
        ...
        view
        returns (
            uint256 referralPendingETH,
            uint256 referralPendingUSD,
            uint256 referralPendingStaking
        )
    {
        referralPendingETH = pendingReferralETH;
        referralPendingUSD = pendingReferralUSD;
        referralPendingStaking = pendingReferralStaking;
    }
    ...
    function _payReferralInETH(
        ...
        if (!_hasReferrer(_userAddress) && _referrer != address(0)) {
            _addReferrer(_userAddress, _referrer);
        }

        if (!_hasReferrer(_referrer) && _referrer != address(0)) {
            _addReferrer(_referrer, defaultReferrer);
        }
        ...
        pendingReferralETH += totalReferral;
    }
    ...
    function payReferralETHAdmin(
        ...
        _payReferralInETH(_value, _userAddress, _referrer);
        return true;
    }
    ...
    function _payReferralInUSD(
        ...
        pendingReferralUSD += totalReferral;
    }
    ...
    function payReferralUSDAdmin(
        ...
        _payReferralInUSD(_value, _userAddress, _referrer);
        return true;
    }

    function _payReferralInTokenStaking(
        uint256 value,
        address _referee
    ) private {
        ...
        for (uint256 i; i < levelRateStaking.length; i++) {
            ...
            emit StakingReferralIncomeGenerated(
                _referee,
                referrer,
                c,
                i + 1,
        ...
        pendingReferralStaking += totalReferral;
    }
    ...
    function payStakingReferralAdmin(
        ...
        _payReferralInTokenStaking(_value, _referee);
        return true;
    }
