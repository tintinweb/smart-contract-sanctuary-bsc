...
contract OGMARKTEPLACE {
    ...
    paymentmethod public payment  = paymentmethod.BOTH;
    ...
    function cancelAsk(address token,uint256 tokenID) external {
        require(tokenexits[token], "Marketplace::token not exists");
        require(
            asks[token][tokenID].seller == msg.sender
        );
        IOGNft(token).transferFrom(address(this), msg.sender, tokenID);
        delete asks[token][tokenID];

    }

    function cancelBid(address token,uint256 tokenID) external {
        require(bids[token][tokenID].buyer == msg.sender);
        IOGNft(token).transferFrom(address(this), msg.sender, tokenID);

        delete bids[token][tokenID];
    ...
    function acceptAsk(address token,uint256 tokenID) external payable {
        ...
        require(asks[token][tokenID].exists);
        require(asks[token][tokenID].seller != msg.sender);
        if (asks[token][tokenID].to != address(0)) {
            require(asks[token][tokenID].to == msg.sender);
        }

        totalPrice += asks[token][tokenID].price;

        if (asks[token][tokenID].eth) {
            escrow[asks[token][tokenID].seller].ethamount += _takeFee(
                asks[token][tokenID].price
            );
            escrow[wallet1].ethamount += (((asks[token][tokenID].price - _takeFee(asks[token][tokenID].price)) * wallet1fee)/percentDivider);
            escrow[wallet2].ethamount += (((asks[token][tokenID].price - _takeFee(asks[token][tokenID].price)) * wallet2fee)/percentDivider);
        } else {
            escrow[asks[token][tokenID].seller].tokenamount += _takeFee(
                asks[token][tokenID].price
            );
            escrow[wallet1].tokenamount += (((asks[token][tokenID].price - _takeFee    (asks[token][tokenID].price))*wallet1fee)/percentDivider);
            escrow[wallet2].tokenamount += (((asks[token][tokenID].price - _takeFee (asks[token][tokenID].price))*wallet2fee)/percentDivider);
        }
        IOGNft(token).transferFrom(address(this), msg.sender, tokenID);
        delete asks[token][tokenID];
        delete bids[token][tokenID];
    ...
    function acceptBid(address token,
        uint256 tokenID, uint256 amount) external payable {
        require(bids[token][tokenID].exists, "bid does not exist");
        require(bids[token][tokenID].buyer != msg.sender, "cant accept own bid");
        require(block.timestamp < bids[token][tokenID].bidendtime, "bid expired");

        if (bids[token][tokenID].eth) {
            require(
                msg.value >= bids[token][tokenID].lastbid,
                "bid price need to be higher than sent value"
            );
            if (bids[token][tokenID].topbider != address(0)) {
                escrow[bids[token][tokenID].topbider].ethamount += bids[token][tokenID]
                    .lastbid;
            }

            bids[token][tokenID].lastbid = msg.value;
        } else {
            require(
                amount >= bids[token][tokenID].lastbid,
                "bid price need to be higher than sent value"
            );
            OGToken.transferFrom(msg.sender, address(this), amount);
            if (bids[token][tokenID].topbider != address(0)) {
                escrow[bids[token][tokenID].topbider].tokenamount += bids[token][tokenID]
                    .lastbid;
            }
            bids[token][tokenID].lastbid = amount;
        }
        bids[token][tokenID].topbider = msg.sender;
    }
    ...
    function claimbid(address token,uint256 tokenID) external {
        ...
        if (bids[token][tokenID].topbider == address(0)) {
            require(
                bids[token][tokenID].buyer == msg.sender,
            ...
            if (bids[token][tokenID].eth) {
                escrow[bids[token][tokenID].buyer].tokenamount += _takeFee(
                    bids[token][tokenID].lastbid
                );
                escrow[wallet1].tokenamount += (((bids[token][tokenID].lastbid-_takeFee(bids[token][tokenID].lastbid))*wallet1fee)/percentDivider);
                escrow[wallet2].tokenamount += (((bids[token][tokenID].lastbid-_takeFee(bids[token][tokenID].lastbid))*wallet2fee)/percentDivider);
            } else {
                escrow[bids[token][tokenID].buyer].ethamount += _takeFee(
                    bids[token][tokenID].lastbid
                );
                escrow[wallet1].ethamount += (((bids[token][tokenID].lastbid-_takeFee(bids[token][tokenID].lastbid))*wallet1fee)/percentDivider);
                escrow[wallet2].ethamount += (((bids[token][tokenID].lastbid-_takeFee(bids[token][tokenID].lastbid))*wallet2fee)/percentDivider);
            }
            IOGNft(token).transferFrom(address(this), msg.sender, tokenID);
        }
