...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function upgradeStandardParcel(uint256 _parcelId, uint8 _paymentType) external onlySaleActive {
        ...
        uint256[2] memory upgradePriceBusdGym = _priceInBusdGym(upgradePrice, _paymentType);
        if(usersMintableNFTAmounts[msg.sender].upgrades > 0) {
            usersMintableNFTAmounts[msg.sender].upgrades--;
        } else {
    ...
    function _sumBusdGymInBusd(uint256[2] memory _array) private view returns(uint256) {
        return _array[0] + _array[1] * _getGymnetPriceInBUSD() / 1e18;
    }
    ...
    function _getGymnetPriceInBUSD() private view returns (uint256) {
        ...
        path[0] = wbnbAddress;
        path[1] = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;//TODO change to variable busdAddress; (here the address is hardcoded for accurate price, current busdaddress gives wrong ratio)
