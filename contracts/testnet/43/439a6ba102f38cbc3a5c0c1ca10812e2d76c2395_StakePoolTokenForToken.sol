...
contract StakePoolTokenForToken is States, Ownable {
    ...
    function calculateApy() public view returns (uint256){
      if(stakingToken==rewardingToken){
        ...
        return stakingToken.balanceOf(address(this)).mul(10**18).div(totalStaked);
      }
    }
...
contract FactoryStakingPool{
    ...
    function createTokenForToken(address ownerAddress, uint256 _earlyUstakeFee, IBEP20 _stakingToken, IBEP20 _rewardingToken, uint256 _lockPeriod, uint256 _earlyRewardUstakeFee, bool _earlyUstake) public returns (address)
    {
