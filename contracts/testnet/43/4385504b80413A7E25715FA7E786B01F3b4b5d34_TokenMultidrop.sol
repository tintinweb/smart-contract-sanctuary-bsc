...
contract TokenMultidrop is Ownable {
    ...
    function minimumHoldings(uint256 _minimumCoins) public onlyOwner {
        minimumCoins = _minimumCoins;
    }
    ...
    function becomeMember(uint256 _day) public payable returns(bool success) {
        ...
        if(token.balanceOf(msg.sender) >= minimumCoins){
