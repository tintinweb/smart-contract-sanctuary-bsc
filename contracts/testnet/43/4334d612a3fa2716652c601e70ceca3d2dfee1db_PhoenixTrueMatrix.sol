...
contract PhoenixTrueMatrix {
    ...
    function _distributelevelIncome(address _user,uint8 level) private {
        users[users[_user].referrer].directincome += directPrice[level];                       
        users[users[_user].referrer].totalincome +=directPrice[level];
        emit Transaction(users[_user].referrer,_user,directPrice[level],level,1);
