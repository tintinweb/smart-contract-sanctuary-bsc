...
contract AiMine is Ownable {
    ...
    function buyMine(uint256 id,uint256 num)  external {
        ...
        uint256 fee=detail.amount.mul(20).div(100);
        ...
        mineNum[id]=mineNum[id]+1;
        emit BuyMine(msg.sender,detail.currency,id,num,detail.amount);
    }
    function setMine(uint256 id,uint256 value,address _currency, uint256 num,uint256 status) external onlyOwner {
        MineDetail[id]=MineConfig(id,value,_currency,num,status);
    }
