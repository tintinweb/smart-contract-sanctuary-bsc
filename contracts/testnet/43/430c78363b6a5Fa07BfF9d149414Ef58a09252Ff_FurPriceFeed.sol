...
import "./Interfaces/IFurAveragePriceOracle.sol";
...
/*
* PriceFeed for mainnet deployment, to be connected to Chainlink's live USDC:USD aggregator reference 
* contract.
*
...
* Chainlink oracle.
*/
contract FurPriceFeed is BaseContract, BaseMath, IFurPriceFeed {
    ...
    IFurAveragePriceOracle public averagePriceOracle;
    ...
    // The last usdc good price seen from an chainlink oracle - 18-digit decimal
    uint public lastUSDCGoodPrice;

    //The  Fur price - 18 digit precision
    uint public lastFURPrice;
    ...
    event LastGoodPriceUpdated(uint _lastUSDCGoodPrice, uint _lastFURPrice);
        ...
        onlyOwner
    {
        ...
        averagePriceOracle = IFurAveragePriceOracle(_averagePriceOracle);

        // Get an initial price from Chainlink to serve as first reference for lastUSDCGoodPrice
        ...
        //Fur token amount per USDC (18-digit decimal)
        averagePriceOracle.updateFurUSDCPrice();
        uint furPerUSDC = averagePriceOracle.getAverageFurForOneUSDC();

        _storeBothPrice(chainlinkResponse, furPerUSDC);
    ...
    */
    function fetchPrice() external override whenNotPaused returns (uint price) {
        ...
        if (_chainlinkIsBroken(chainlinkResponse, prevChainlinkResponse)) {
            uint currentFURPrice = lastUSDCGoodPrice.mul(DECIMAL_PRECISION).div(furPerUSDC);
            _storePrice(lastUSDCGoodPrice, currentFURPrice);
            return currentFURPrice;
        }
        ...
        return _storeBothPrice(chainlinkResponse, furPerUSDC);
    }
    ...
    function _storePrice(uint _currentUSDCPrice, uint _currentFURPrice) internal {
        lastUSDCGoodPrice = _currentUSDCPrice;
        lastFURPrice = _currentFURPrice;
        emit LastGoodPriceUpdated(_currentUSDCPrice, _currentFURPrice);
    }

    function _storeBothPrice(ChainlinkResponse memory _chainlinkResponse, uint256 _furPerUSDC) internal returns (uint) {
        uint scaledChainlinkPrice = _scaleChainlinkPriceByDigits(uint256(_chainlinkResponse.answer), _chainlinkResponse.decimals);
        uint currentFurPrice = scaledChainlinkPrice.mul(DECIMAL_PRECISION).div(_furPerUSDC);

        _storePrice(scaledChainlinkPrice, currentFurPrice);
        return currentFurPrice;
    }
...
interface IFurAveragePriceOracle {
    function getAverageFurForOneUSDC()
    external
    view
    returns(uint256 amountOut);

    function updateFurUSDCPrice() external;
}
