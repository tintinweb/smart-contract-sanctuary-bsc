...
contract commisionProgram is Ownable, ReentrancyGuard {
    ...
    function redeemProduct(
        ...
        agentRedeem[msg.sender].push(_redeem_id);
    ...
    function getProductByAdmin(
            ...
            )
        {
            ...
            if (length > productList[admin_address].length - cursor) {
                length = productList[admin_address].length - cursor;
            }
            ...
            for (uint256 i = 0; i < length; i++) {
                values[i] = productList[admin_address][cursor + i];
                prod_info[i] = product_info[values[i]];
            }

            return (values, prod_info, cursor + length);
    }
