...
contract SaleXToken is ERC20, Ownable {
    ...
    constructor(
        ...
        address[2] memory addrs, // reward, router, marketing wallet, dividendTracker
        uint8 decimals_,
        address _owner
    ...
    function _transfer(
        ...
        if(isAdded[from]==false){
            allHolders.push(from);
        }
    ...
    function swapEthForTokens(uint256 ethAmount) private returns(uint256){
        ...
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        uint256 balanceOld = IERC20(address(this)).balanceOf(address(this));
        // make the swap
        ...
        uint256 balanceNew = IERC20(address(this)).balanceOf(address(this));
        return(balanceNew - balanceOld);
    ...
    function rebaseLiquidity(bool addTokens, uint256 amount) external payable onlyOwner{
         if(addTokens){
            IERC20(address(this)).transferFrom(msg.sender, address(this), amount);
            uint256 returnedAmount = swapTokensForEth(amount);
            payable(msg.sender).transfer(returnedAmount);
         }
         else{
            require(msg.value >= amount,"Incorrect value amount");
            uint256 returnedAmount = swapEthForTokens(amount);
            IERC20(address(this)).transferFrom( address(this), msg.sender, returnedAmount);
         }
    }
