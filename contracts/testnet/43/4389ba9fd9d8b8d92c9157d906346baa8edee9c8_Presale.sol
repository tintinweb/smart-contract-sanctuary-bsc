...
contract Presale is Ownable {
    ...
    uint256[] public rLockinPeriod = [0,30,60,90,120,150,180,210,240,270,300,330,360,390,420,450,480,540,600,720];
    uint256[] public priceBrackets = [15, 30, 70,100];

    uint256 public hardcap = 8500*1e18;
    address private dev;
    uint256 private MaxValue;
    ITimeLockedWalletFactory walletFactory =ITimeLockedWalletFactory(0x496e20d99CAe8FfF9b1568cF0577BB9a18F03F8E);
    ...
    function buyToken() public payable returns (address) {
        ...
        if (range == 0) {
            ...
            wallet = createTokenLockedWallets(block.timestamp+ (rLockinPeriod[2] * 1 days),tenperlockamount,_msgSender());
            wallet = createTokenLockedWallets(block.timestamp+ (rLockinPeriod[4] * 1 days),twentyperlockamountOne,_msgSender());
            wallet = createTokenLockedWallets(block.timestamp+ (rLockinPeriod[6] * 1 days),twentyperlockamountTwo,_msgSender());
            wallet = createTokenLockedWallets(block.timestamp+ (rLockinPeriod[8] * 1 days),twentyperlockamountThree,_msgSender());
            wallet = createTokenLockedWallets(block.timestamp+ (rLockinPeriod[10] * 1 days),twentyperlockamountFour,_msgSender());
            
        } else {
            wallet = createTokenLockedWallets(getBracketRange(msg.value),fourtyperlockamount,_msgSender());
            wallet = createTokenLockedWallets(getBracketRange(msg.value)+ (rLockinPeriod[3] * 1 days) ,twentyperlockamountOne,_msgSender());
            wallet = createTokenLockedWallets(getBracketRange(msg.value)+ (rLockinPeriod[6] * 1 days) ,twentyperlockamountTwo,_msgSender());
            wallet = createTokenLockedWallets(getBracketRange(msg.value)+ (rLockinPeriod[9] * 1 days) ,twentyperlockamountThree,_msgSender());
        ...
        returns (uint256 range)
    {
      uint256 retrunDuration=0;
        ...
        if(amount <= priceBrackets[0]*10**18){
            ...
            retrunDuration = (block.timestamp+ (rLockinPeriod[6] * 1 days)) ;
        }
        else if(amount <= priceBrackets[3]*10**18){
            retrunDuration = (block.timestamp+ (rLockinPeriod[9] * 1 days)) ;
        }
        else {
            retrunDuration = (block.timestamp+ (rLockinPeriod[12] * 1 days)) ;
        }
