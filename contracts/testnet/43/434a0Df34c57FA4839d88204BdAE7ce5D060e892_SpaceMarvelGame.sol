...
contract SpaceMarvelGame is Ownable, Pausable, ReentrancyGuard {
    ...
    struct Fee {
        ...
        bool isMultiSupport;
        // uint256 maxRequest;
        mapping(address => uint256) userToRequestNumber;
        bool valid;
    }
    ...
    event SetNFT(address nftAddress, uint256 durationActive, bool valid);
    ...
    function setNFT(
        ...
        emit SetNFT(_nftAddress, _durationActive, _valid);
    }
    ...
    function setFees(
        ...
        bool _isMultiSupport,
        bool _valid
    ) external onlyOwner {
        require(_vault != address(0), "Error: Vault address(0)");
        require(nfts[_nftAddress].valid, "Error: NFT invalid");
        require(_amounts.length > 0, "Error: Amounts invalid");
        require(_feeType < 4, "Error: Fee type invalid");

        nfts[_nftAddress].fees[_feeType].currency = _currency;
        nfts[_nftAddress].fees[_feeType].vault = _vault;
        nfts[_nftAddress].fees[_feeType].amounts = _amounts;
        nfts[_nftAddress].fees[_feeType].isMultiSupport = _isMultiSupport;
        nfts[_nftAddress].fees[_feeType].valid = _valid;
    }

    function getFee(address _nftAddress, uint8 _feeType)
        external
        view
        returns (
            address vault,
    ...
    {
        vault = nfts[_nftAddress].fees[_feeType].vault;
        currency = nfts[_nftAddress].fees[_feeType].currency;
        amounts = nfts[_nftAddress].fees[_feeType].amounts;
        isMultiSupport = nfts[_nftAddress].fees[_feeType].isMultiSupport;
        valid = nfts[_nftAddress].fees[_feeType].valid;
    }
    ...
    function setVault(
        ...
        require(_vault != address(0), "Error: Vault address(0)");
        require(nfts[_nftAddress].fees[_feeType].valid, "Error: Fee invalid");
        nfts[_nftAddress].fees[_feeType].vault = _vault;
    }
        ...
        nonReentrant
    {
        ...
        //charge fee
        Fee storage fee = nfts[_nftAddress].fees[0];
        uint256 amount = 0;
        if (fee.valid) {
            amount = fee.amounts[0];
            if (fee.isMultiSupport) {
                uint256 requestNumber = fee.userToRequestNumber[_msgSender()] +
                    1;
                amount = fee.amounts[requestNumber - 1];
                fee.userToRequestNumber[_msgSender()] += 1;
            }
            if (amount > 0) {
                chargeFee(fee.currency, amount, fee.vault);
            }
        }
        ...
        nonReentrant
    {
        ...
        //charge fee
        Fee storage fee = nfts[_nftAddress].fees[1];
        uint256 amount = 0;
        if (fee.valid) {
            amount = fee.amounts[0];
            if (fee.isMultiSupport) {
        ...
        nonReentrant
    {
        ...
        Fee storage fee = nfts[_nftAddress].fees[2];
        require(
            fee.userToRequestNumber[_msgSender()] < fee.amounts.length,
            "Error: reach breed limit"
        );
        ...
        nonReentrant
    {
        ...
        //charge fee
        Fee storage fee = nfts[_nftAddress].fees[3];
        uint256 amount = 0;
        if (fee.valid) {
            amount = fee.amounts[0];
            if (fee.isMultiSupport) {
    ...
    function chargeFee(
        ...
        if (_currency == address(0)) {
            ...
            IERC20(_currency).safeTransferFrom(_msgSender(), _vault, _amount);
