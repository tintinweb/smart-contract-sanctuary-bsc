...
contract PricingController is IPricingController, RecoverableFunds, AccessControl {
    ...
    function calculatePriceLadder(INFTMarket.MarketItem calldata item) override external view returns (PriceStep[] memory) {
        PricingStrategies.PricingStrategy storage strategy = pricingStrategies.get(item.pricingStrategy);
        PriceStep[] memory prices = new PriceStep[](strategy.intervals.length + 1);
...
library PricingStrategies {
     ...
     * already present.
     */
    function set(Map storage map, uint256 key, PricingStrategy memory value) internal returns (bool) {
     ...
     */
    function at(Map storage map, uint256 index) internal view returns (uint256, PricingStrategy storage) {
     ...
     */
    function get(Map storage map, uint256 key) internal view returns (PricingStrategy storage) {
        PricingStrategy storage value = map._values[key];
