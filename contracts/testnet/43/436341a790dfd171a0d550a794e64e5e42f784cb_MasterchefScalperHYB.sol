...
contract MasterchefScalperHYB is ERC20("ScalpLP ", "Slp-"), Ownable, Pausable, ReentrancyGuard {
    ...
    event Compounded(address, uint256,uint256);
    ...
    function deposit(uint256 _amount,address _referrar) public whenNotPaused nonReentrant {
        ...
        //Mandatory as it needs to compund all pending rewards
        _harvest();
        if(share[msg.sender]==0 && _referrar!=address(0)){
        ...
        stableShare[msg.sender]=_amount;
        totalshares = share[msg.sender];
        totalStableShares = stableShare[msg.sender];
                }
        else {
            uint256 baseShareval = _amount.mul(totalshares).div(baseAssetBalanceOf().sub(_amount));
            share[msg.sender] = share[msg.sender].add(baseShareval); 
            totalshares = totalshares.add(baseShareval);
            uint256 stableShareval = _amount.mul(totalStableShares).div(stableAssetBalanceOf().add(totalSupply()));
            stableShare[msg.sender] = stableShare[msg.sender].add(stableShareval); 
            totalStableShares = totalStableShares.add(stableShareval);
        }
    ...
    function withdraw(uint256 _amount) public nonReentrant {
        ...
        _harvest();
        _compound();
        uint256 calcStable = share[msg.sender].mul(getPricePerFullShareStable()).div(1e18);
        uint256 stablepart = calcStable.sub(balanceOf(msg.sender));
        uint256 usershare = stablepart.mul(99).div(100);
        uint256 referrarStableshare = stablepart.div(100);
        uint256 tempStableShare = totalStableShares-stableShare[msg.sender];
        uint256 tempStableSupply = baseAssetBalanceOf().add(stableAssetBalanceOf()).sub(balanceOf(msg.sender)).sub(stablepart);
        
        uint256 calc = share[msg.sender].mul(getPricePerFullShare()).div(1e18);
        uint256 calcWithdraw = _amount.mul(calc).div(balanceOf(msg.sender));
        ...
        if(tempshare==0){
        share[msg.sender] = calc.sub(calcWithdraw);
        }
        else{
        share[msg.sender] = (calc.sub(calcWithdraw)).mul(tempshare).div(tempsupply); 
        }
        if(tempStableShare==0){
        stableShare[msg.sender] = balanceOf(msg.sender).sub(_amount);
        }
        else{
       // stableShare[msg.sender] = (calc.sub(calcWithdraw)).mul(tempshare).div(tempsupply); 
        uint256 valu = balanceOf(msg.sender).sub(_amount); 
        stableShare[msg.sender] = valu.mul(tempStableShare).div(tempStableSupply);
        }
        totalStableShares = tempStableShare.add(stableShare[msg.sender]);
        totalshares = tempshare.add(share[msg.sender]); 
        uint256 bal = available();
        if (bal < calcWithdraw) {
            uint256 balWithdraw = calcWithdraw - bal;
            ...
            if (diff < balWithdraw) {
            calcWithdraw = balAfter;
            }
        }      
        if(referrar[msg.sender]!=address(0))
        {
        IERC20(baseAsset).safeTransfer(msg.sender, calcWithdraw.sub(referrarshare));
        IERC20(baseAsset).safeTransfer(referrar[msg.sender], referrarshare);
        IERC20(stableAsset).safeTransfer(msg.sender, usershare);
        IERC20(stableAsset).safeTransfer(referrar[msg.sender], referrarStableshare);
        }
        else{
        IERC20(baseAsset).safeTransfer(msg.sender, calcWithdraw);
        IERC20(stableAsset).safeTransfer(msg.sender, stablepart);
        }
        _burn(msg.sender, _amount);    
        emit Withdraw(msg.sender, calcWithdraw , interest);
    }

   //View function

    function baseAssetBalanceOf() public view returns (uint256) {
       // uint256 _amount = IMasterChef(masterChef).earned(address(this));
...
//       uint256 _amount = IMasterChef(masterChef).earned(address(this));
   //     return IERC20(baseAsset).balanceOf(address(this)).add(_amount);
    return IERC20(baseAsset).balanceOf(address(this)).add(IMasterChef(masterChef).balanceOf(address(this)));
    }
    function stableAssetBalanceOf() public view returns (uint256) {
...
//UI
    ...
    function getPricePerFullShareStable() public view returns (uint256) {
        return totalSupply() == 0 ? 1e18 : (totalSupply().add(stableAssetBalanceOf())).mul(1e18).div(totalStableShares);
    }
...
//Internal executions   
    ...
    function _compound() internal {
        uint256 stablehalf = rewardAssetBalance().mul(515).div(1000); 
        if(stablehalf > 1000){
        if(rewardAsset!=stableAsset){
        uint256 before = stableAssetBalanceOf();
        IUniswapRouterETH(addresses[0]).swapExactTokensForTokens(stablehalf, 0, rewardToStable, address(this), block.timestamp);
        uint256 diff = stableAssetBalanceOf().sub(before);

        IERC20(stableAsset).safeTransfer(addresses[1], diff.mul(30).div(1545));
        IERC20(stableAsset).safeTransfer(addresses[2], diff.mul(60).div(1545));

        } 
        uint256 half = rewardAssetBalance().div(2);       
        if(rewardAsset!=lp0){
        ...
        emit Compounded(msg.sender, stablehalf, half);
        }
        if(baseAssetBalanceOf()>0){
        _earn();
        }
    }
