...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant payable validatePoolByPid(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (pool.lpToken != IERC20(address(0))) {
            _amount = _amount * 10 **uint256((IERC20(pool.lpToken).decimals()));
