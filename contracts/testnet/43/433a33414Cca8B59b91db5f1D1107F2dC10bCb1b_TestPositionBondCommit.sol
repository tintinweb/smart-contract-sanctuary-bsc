...
abstract contract PositionBondCommit is
    ...
    ReentrancyGuard
{
    mapping(address => uint256) private amountCommittedMapping;
        ...
        nonReentrant
    {
        ...
        require(
            amountCommittedMapping[_trader] < maxAmountCanCommit,
            "over amount commit"
        );

        uint256 amountCommitValidate = BondMath.validateAmountCommit(
            amountCommit,
            amountCommittedMapping[_trader],
            maxAmountCanCommit
        );
        FaceAsset._transferIn(amountCommitValidate);
        amountCommittedMapping[_trader] += amountCommitValidate;
        emit Committed(_trader, amountCommitValidate);
    }

    function distribution() external onlyDistribution nonReentrant {
        address _trader = msg.sender;
        uint256 bondAmount;
        uint256 remainder;
        (bondAmount, remainder) = amountDistribution(_trader);
        if (remainder > 0) {
            FaceAsset._transferOut(_trader, remainder);
        }
        BondUnit._mint(_trader, bondAmount);
    }

    function amountCommitted(address _trader) public view returns (uint256) {
...
abstract contract MockStake {
    ...
    function setMockConvertStakeBUSD(uint256 _mockConvert, address _trader) public {
...
library BondMath {
    ...
    function calculateBondAmountDistribution(
        ...
        uint256 maxAmountCanCommit,
        uint256 bondSupply
    ) internal pure returns (uint256 bondAmount, uint256 remainder) {
        if (amountCommitted <= maxAmountCanCommit) {
            bondAmount =
                bondSupply *
                ((amountCommitted * 10**18) / totalAmountCommitted);
        } else {
            bondAmount =
                bondSupply *
                ((maxAmountCanCommit * 10**18) / totalAmountCommitted);
            remainder = amountCommitted - maxAmountCanCommit;
        }

        return (bondAmount / 10**18, remainder);
    }
}
