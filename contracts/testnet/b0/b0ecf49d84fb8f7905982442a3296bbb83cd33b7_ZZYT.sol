...
contract ZZYT is Context,IERC20Metadata,Ownable {
    ...
    //mainnet: 
    string private constant _symbol = "ZZYT";
    ...
    function createWhite(address _address,bool flag) external onlyOwner{
             require(whiteList[_address] != flag);
    ...
    function _transfer(
        ...
        if(
          ...
          if(isBuy){ //买
           ...
           ytHolder[sender]=  ytHolder[sender].add(trueAmount);
          ...
          if(ytHolder[sender]!=0){
             ytHolder[sender]=  ytHolder[sender].add(amount);
          }
        }
