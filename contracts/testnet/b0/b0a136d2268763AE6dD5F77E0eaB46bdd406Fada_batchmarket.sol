...
contract batchmarket is AccessControl {
    ...
    function batchSales(
        ...
        for (uint256 i = _tokenId; i < total; i = unsafei(i)) {
            ...
            ++_tokenId;
            /*console.log("El orden es %s", index);
            console.log("Token ID es %s", _tokenId);
            console.log("length de Array %s", _baseUri.length);*/
            ...
            //console.log("El orden despues es %s", index);
            nftContractBatchs[msg.sender][_nftContractAddress][_tokenId] = true;
        }
        emit SaleCreated(msg.sender);
    }

    function cancelSale(address _nftContractAddress, uint256 _tokenId) public {
        if (!nftContractBatchs[msg.sender][_nftContractAddress][_tokenId]) {
            revert nottheowner();
        }
        market.withdrawAuction(_nftContractAddress, _tokenId);

        emit SaleCancelled(_nftContractAddress, _tokenId, msg.sender);
    }
