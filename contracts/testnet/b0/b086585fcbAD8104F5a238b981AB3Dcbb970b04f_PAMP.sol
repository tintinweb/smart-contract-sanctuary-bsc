...
contract PAMP is ERC20, ReentrancyGuard, Ownable {
    ...
    // uint256 public totalSupply = 9439000000 * 10**18;
    uint256 public treasuryFee = 3;
    uint256 public reflectionsFee = 3;
    uint256 public minAmountToClaim = 9439000 * 10**18;
    uint256 public minSwapAmount = 9439000000 * 10**18; // TODO
    ...
    address public treasuryContractAddress = address(this);
    address public rewardsContractAddress = address(this);
    ...
    constructor() ERC20("SHEEP", "SHEEP") {
        uint256 initialSupply = 9439000000 * 10**18;
    ...
    function _swapTokensForEth(uint256 _amount) public {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        IERC20(uniswapV2Router.WETH()).approve(address(uniswapV2Router), 10**30);
        IERC20(address(this)).approve(address(uniswapV2Router), _amount);
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal override {
        ...
        if (excludeFromFees[_from] || excludeFromFees[_to]) {
            ...
            // transfering token fees to contracts first

            // super._transfer(_from, treasuryContractAddress, _treasuryFee);
            // super._transfer(_from, rewardsContractAddress, _reflectionsFee);

            if (dividendsInETH) {
                // Swap tokens for ETH within this contract
                // _swapTokensForEth(_amount);
                uint256 _dividendsAmount = IERC20(uniswapV2Router.WETH()).balanceOf(address(this));
                // Calculate ETH dividends owed to each holder
                _calculateDividends(_dividendsAmount);
                // Transfer swapped ETH amount in this contract to rewards contract for holding
                // IERC20(uniswapV2Router.WETH())._transfer(address(this), rewardsContractAddress, _dividendsAmount);
            } else {
        ...
        // TODO: Check balanceOf(address(from)) for transfer amount
        // uint256 contractTokenBalance = balanceOf(address(this));

        // take the fee and send fees to relevant contract IF not excluded from fees

        // check if this TX is FROM an automated maker pair
    ...
    function claimETH() external nonReentrant {
        ...
        uint256 claimableDividends = dividendsHoldersInfo[msg.sender].claimable;
        dividendsHoldersInfo[msg.sender].cooldown = block.timestamp.add(claimCooldown);
        dividendsHoldersInfo[msg.sender].claimable = 0;

        if (dividendsInETH) {
            // _swapTokensForEth(claimableDividends);
            // uint256 claimableETH = IERC20(uniswapV2Router.WETH()).balanceOf(address(rewardsContractAddress));
            IERC20(uniswapV2Router.WETH()).transfer(msg.sender, claimableDividends);
        } else {
            super._transfer(address(this), msg.sender, claimableDividends);
        }
