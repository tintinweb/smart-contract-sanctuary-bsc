...
contract pool is owned {
    ...
    mapping(address => uint[4]) public entryTime;

    mapping(address => uint[4]) public holdingAmount;    

    uint[4] public lastRewardTime;
    uint[4] public totalSystemToken;

    //last reward time => reflectionPercentSum[tokenType]
    mapping( uint => uint[4]) public reflectionPercentSum;
