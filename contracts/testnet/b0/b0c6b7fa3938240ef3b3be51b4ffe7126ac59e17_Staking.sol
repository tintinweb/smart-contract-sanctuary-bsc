...
contract Staking is Ownable{
    ...
    event stakingevent(uint256 stakingid,bool activate,uint256 peroied,address user,uint256 cliamtime);
    event Claim(uint256 skingid,uint256 rewoed,uint256 nextcliamtime,address useraddress);
    ...
    function stacked(uint256 _amount,uint256 period)public returns(bool){
        ...
        emit stakingevent(totalstakingcount,true,timeperiod,msg.sender,timecount[totalstakingcount]);
        return true;
    }
    function claim(uint256 skingid)public claimtime(skingid) returns(bool){
        ...
        if(stakingdata[skingid].months == 6){
            reward = ((stakingdata[skingid].amount).mul(200)).div(10000);
        }else if(stakingdata[skingid].months == 12){
            reward = ((stakingdata[skingid].amount).mul(250)).div(10000);
        }
        else{
            reward = ((stakingdata[skingid].amount).mul(300)).div(10000);
        }
        IERC20 token = IERC20(tokenaddress);
        token.transfer(msg.sender,reward);
        timecount[skingid] = block.timestamp + 2 minutes;
        emit Claim(skingid,reward,timecount[skingid],msg.sender);
