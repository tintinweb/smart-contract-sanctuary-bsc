...
contract LaunchPad is Auth {
  ...
  event PoolCreated(uint indexed poolId, string name, uint minDeposit, uint interestRate, uint lockingTime, uint vestingTime, uint timestamp);
  event PoolNameUpdated(uint indexed poolId, string name, uint timestamp);
  event PoolMinDepositUpdated(uint indexed poolId, uint amount, uint timestamp);
  event PoolInterestRateUpdated(uint indexed poolId, uint interestRate, uint timestamp);
  event PoolLockingTimeUpdated(uint indexed poolId, uint time, uint timestamp);
  event PoolVestingTimeUpdated(uint indexed poolId, uint time, uint timestamp);
  event PoolLockingStarted(uint indexed poolId, uint timestamp);
  event PoolStakingStatusUpdated(uint indexed poolId, bool status, uint timestamp);
  event UserBlocked(address indexed userAddress, bool status, uint timestamp);
  event Deposited(address indexed userAddress, uint poolId, uint amount, uint timestamp);
  event Claimed(address indexed userAddress, uint amount, uint timestamp);
  ...
  function createPool(
    ...
    emit PoolCreated(poolIndex, _name, _minDeposit, _interestRate, _lockingTime, _vestingTime, block.timestamp);
    poolIndex++;
  }
  ...
  function updatePoolName(uint _poolId, string calldata _name) external onlyMainAdmin notStartVesting(_poolId) {
    ...
    emit PoolNameUpdated(_poolId, _name, block.timestamp);
  }
  ...
  function updatePoolMinDeposit(uint _poolId, uint _amount) external onlyMainAdmin notStartVesting(_poolId) {
    ...
    emit PoolMinDepositUpdated(_poolId, _amount, block.timestamp);
  }
  ...
  function updatePoolInterestRate(uint _poolId, uint _percent) external onlyMainAdmin notStartVesting(_poolId) {
    ...
    emit PoolInterestRateUpdated(_poolId, _percent, block.timestamp);
  }
  ...
  function updatePoolLockingTime(uint _poolId, uint _seconds) external onlyMainAdmin notStartVesting(_poolId) {
    ...
    emit PoolLockingTimeUpdated(_poolId, _seconds, block.timestamp);
  }
  ...
  function updatePoolVestingTime(uint _poolId, uint _seconds) external onlyMainAdmin notStartVesting(_poolId) {
    ...
    emit PoolVestingTimeUpdated(_poolId, _seconds, block.timestamp);
  }
  ...
  function blockUser(address _userAddress, bool _status) external onlyMainAdmin {
    ...
    emit UserBlocked(_userAddress, _status, block.timestamp);
  }
  ...
  function deposit(uint _poolId, uint _amount) external poolExist(_poolId) {
    ...
    emit Deposited(msg.sender, _poolId, _amount, block.timestamp);
  }
  ...
  function _updatePoolStakingStatus(uint _poolId, bool _status) private {
    ...
    emit PoolStakingStatusUpdated(_poolId, _status, block.timestamp);
  }
}
