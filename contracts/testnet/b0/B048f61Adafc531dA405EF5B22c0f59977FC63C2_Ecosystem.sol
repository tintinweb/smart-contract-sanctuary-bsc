...
contract Ecosystem is Ownable, Initializable {
    ...
    function initialize(address tokenAdd, uint allocation, uint subsequentClaims, uint _claimPeriod) external initializer {
        ...
        require(_claimPeriod > 0, "Ecosystem : claimPeriod > 0");
        require(subsequentClaims > 0,"Ecosystem : subsequentClaims > 0");

        token = IBEP20(tokenAdd);
        claimPeriod = _claimPeriod;
        subsequentClaim = subsequentClaims;
        require(IBEP20(token).transferFrom(msg.sender, address(this), allocation), "Ecosystem : transferFrom failed");
    }
    ...
    function claim() external {
        ...
        total[1] += claimAmount;
        require(token.transfer(msg.sender, claimAmount), "Ecosystem : transfer failed");
        emit Claim(
            msg.sender,
            claimAmount
        );
