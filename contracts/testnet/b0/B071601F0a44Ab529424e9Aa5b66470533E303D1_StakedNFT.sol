 ...
 */
contract StakedNFT is ERC721Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    IStakedSoccerStarNftV2 public cardNFTStake;
    IBIBNode public NODE;
    ISoccerStarNftMarket public soccerStartNftMarket;
    mapping(address=>bool) public allowProtocolToCallTb;
    //st => nft
    mapping(uint256 => uint256) public stNFTMap;
    // nft => st
    mapping(uint256 => uint256) public nftSTMap;
    uint256 public ticket;
    address public nftAuction;

    function initialize(
        IStakedSoccerStarNftV2 _stake,
        IBIBNode _node,
        ISoccerStarNftMarket _market,
        address _nftAuction
    ) public reinitializer(1)  {
        cardNFTStake = _stake;
        NODE = _node;
        soccerStartNftMarket = _market;
        nftAuction = _nftAuction;

        __Pausable_init();
        __Ownable_init();
        __ERC721_init("stakedNFT", "stNFT");
        ticket = 1;
    }
     ...
     */
    function mint(address to, uint256 cardNftId) public onlyAllowProtocolToCall whenNotPaused {
        stNFTMap[ticket] = cardNftId;
        nftSTMap[cardNftId] = ticket;
        _mint(to, ticket);
        ticket++;
    }

    function batchMint(uint256[] calldata cardNftIds) public onlyOwner {
        for(uint256 i=0; i<cardNftIds.length;i++) {
            require(cardNFTStake.isStaked(cardNftIds[i]), "TOKEN_NOT_SATKED");
            address _to = cardNFTStake.getTokenOwner(cardNftIds[i]);
            stNFTMap[ticket] = cardNftIds[i];
            nftSTMap[cardNftIds[i]] = ticket;
            _mint(_to, ticket);
            ticket++;
        }
    }
     ...
     */
    function burn(uint256 cardNftId) public onlyAllowProtocolToCall whenNotPaused {
        uint256 ticketId = nftSTMap[cardNftId];
        _burn(ticketId);
        delete stNFTMap[ticketId];
        delete nftSTMap[cardNftId];
    }
    ...
    function transferDirect(
        ...
        address to,
        uint256 nftId
    ) public onlyAllowProtocolToCall whenNotPaused {
        uint256 _stId = nftSTMap[nftId];
        super._transfer(from, to, _stId);
    }
    ...
    function _transfer(
        ...
        if (to == address(soccerStartNftMarket) || to == nftAuction || from == to) {
            return;
        }
        uint256 _cardNFTId = stNFTMap[tokenId];
        require(!NODE.isStakedAsNode(_cardNFTId), "TOKEN_STAKED_AS_NODE");
    ...
    function setNftAuction(address _nftAuction) external onlyOwner {
        nftAuction = _nftAuction;
    }
