...
contract BUSDVYNCSTAKE is ReentrancyGuard,Ownable{
    ...
    uint256 public compoundRate= 30;
    ...
    IERC20 public vync = IERC20(0xfeFd8e66b6FcEf530557d219a6F4f088cCfEA80A);
    IERC20 public busd = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    address lpToken= 0x8d100170226776D1B7588E03CEE042CBd7138AF4;
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        if(userInfo[msg.sender].isStaker == true && compoundedReward(msg.sender)> 0){
            uint256 _pendingReward= compoundedReward(msg.sender);
            userInfo[msg.sender].stakeBalance= userInfo[msg.sender].stakeBalance+ _pendingReward;
            userInfo[msg.sender].totalClaimedReward= userInfo[msg.sender].totalClaimedReward+ _pendingReward;
            userInfo[msg.sender].autoClaimWithStakeUnstake= userInfo[msg.sender].autoClaimWithStakeUnstake + _pendingReward;
        }
        ...
        userInfo[msg.sender].stakeBalance = userInfo[msg.sender].stakeBalance + (busdAdded + amountToSwap);
    ...
    function unStake(uint256 amount) external nonReentrant{
        ...
        require(amount <= balance,"invalid unStake amount- input balanceOf() value");
        ...
        uint256 percentOfunstake= (_amount*100)/balance;
        ...
        userInfo[msg.sender].stakeBalance = userInfo[msg.sender].stakeBalance.sub(_amount);
        ...
        returns (uint256 _compoundedReward)
    {
        ...
        uint256 compoundTime= block.timestamp - userInfo[user].lastStakeTimestamp;
        ...
        uint256 a= _a();
        _compoundedReward=0;
        uint256 balance = userInfo[user].stakeBalance;
        ...
        _compoundedReward= _compoundedReward-userInfo[user].totalClaimedReward+userInfo[user].autoClaimWithStakeUnstake;
        }
        if(userInfo[user].isClaimAferUnstake == true){
            _compoundedReward= _compoundedReward + userInfo[user].pendingRewardAfterFullyUnstake;
        }
        
    }

    function compoundedRewardInVync( address user) public view returns(uint256 _compoundedVyncReward){
        uint256 reward;
    ...
    function pendingReward(address user) public view returns(uint256 _pendingReward){
        ...
        if(_pendingReward!=0){
        _pendingReward= _pendingReward-userInfo[user].totalClaimedReward+userInfo[user].autoClaimWithStakeUnstake;
        }
        if(userInfo[user].isClaimAferUnstake == true){
            _pendingReward= _pendingReward + userInfo[user].pendingRewardAfterFullyUnstake;
        }

        _pendingReward = _pendingReward - compoundedReward(user);
    }

    function pendingRewardInVync(address user) public view returns(uint256 _pendingVyncReward) {
        uint256 reward;
    ...
    function rewardCalculation(address user) internal {
        ...
        if(userInfo[user].isClaimAferUnstake == true){
            totalReward= totalReward + userInfo[user].pendingRewardAfterFullyUnstake;
        }
    ...
    function claim() public nonReentrant{
        require(userInfo[msg.sender].isStaker == true || userInfo[msg.sender].isClaimAferUnstake == true , "user not staked");

        uint256 compoundTime= block.timestamp - userInfo[msg.sender].lastStakeTimestamp;
        require(compoundTime >= compoundRate,"wait for next compound"); //86400

        rewardCalculation(msg.sender);
        uint256 reward = userInfo[msg.sender].lastClaimedReward + userInfo[msg.sender].autoClaimWithStakeUnstake;
        ...
        if(userInfo[msg.sender].isClaimAferUnstake == true && userInfo[msg.sender].isStaker == false){
            userInfo[msg.sender].lastStakeTimestamp= 0;
            userInfo[msg.sender].lastClaimedReward = 0;
            userInfo[msg.sender].totalClaimedReward = 0;
        }

        if(userInfo[msg.sender].isClaimAferUnstake == true && userInfo[msg.sender].isStaker == true){
            userInfo[msg.sender].totalClaimedReward= userInfo[msg.sender].totalClaimedReward - userInfo[msg.sender].pendingRewardAfterFullyUnstake;
        }
