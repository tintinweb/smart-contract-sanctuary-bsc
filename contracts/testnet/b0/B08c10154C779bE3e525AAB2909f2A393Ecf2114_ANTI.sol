...
contract ERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(isPair[sender]==false && isPair[recipient]==false){
            emit Transfer(sender, recipient, trueAmount);//lp tihuan or yincang
        }
    }
...
contract ANTI is ERC20, CoinFactoryAdminRole{
    ...
    function init(address _RP, uint256 _startTradeTime, uint256 _sUserNum, uint256 _snaNum,
        address _buyToken, uint256 _snaTokenStartTime) external onlyCoinFactoryAdmin {
