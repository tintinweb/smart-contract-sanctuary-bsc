...
contract DinoBreeding is Ownable {
    constructor() {
        blockTime = 3;
        maxBreedCount = 5;
        breedCountToBreedPrice[0] = 375 * 1e18;
        breedCountToBreedPrice[1] = 750 * 1e18;
        breedCountToBreedPrice[2] = 1125 * 1e18;
        breedCountToBreedPrice[3] = 1750 * 1e18;
        breedCountToBreedPrice[4] = 3000 * 1e18;

        minLifeDurationToBreed = 15 days;
    }
    ...
    function getChildrenIdsByDinoId(uint256 _dinoId) external view returns(uint256[] memory) {
    ...
    function breed(
        ...
        require(
            dinoIdToBreedCount[_sireId] < maxBreedCount,
            "DinoBreeding: Sire is already at max breed count"
        );
        require(
            dinoIdToBreedCount[_matronId] < maxBreedCount,
            "DinoBreeding: Matron is already at max breed count"
        );
        /// @dev Get the Breeding Fee
        uint256 breedingFee = breedCountToBreedPrice[
        ...
        require(matronDino.gender == 2, "DinoBreeding: Matron gender is not valid");
        ...
        if (matronClass == sireClass) {
            if (matronClass == DARK_CLASS) {
                require(stoneType == StoneType.Moon, "DinoBreeding: Need Moon Stone");
            } else if (matronClass == NOVIS_CLASS) {
                require(stoneType == StoneType.Fire, "DinoBreeding: Need Fire Stone");
            } else if (matronClass == AQUIS_CLASS) {
                require(stoneType == StoneType.Water, "DinoBreeding: Need Water Stone");
            } else if (matronClass == TERROS_CLASS) {
                require(stoneType == StoneType.Leaf, "DinoBreeding: Need Leaf Stone");
            }
        } else {
            if (matronClass == DARK_CLASS || sireClass == DARK_CLASS) {
                require(stoneType == StoneType.Moon, "DinoBreeding: Need Moon Stone");
            } else {
                require(stoneType == StoneType.Rainbow, "DinoBreeding: Need Rainbow Stone");
            }
        }
        /// @dev Burn the stone
