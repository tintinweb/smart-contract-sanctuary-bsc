...
contract MyNFTStorage is ERC1155Holder, ReentrancyGuard {
    ...
    function buyOrder(uint orderId) payable public nonReentrant {
        ...
        if (!isNativePayment) {
            ...
            require(msg.value >= totalPrice, "Total price exceeded balance");
            if (fee > 0)
        ...
        if (royaltyShared > 0)
            sellerReceived = realOrderPrice.sub(royaltyShared);
        ...
        if (!isNativePayment)
        {
            ...
            payable(address(order.owner)).transfer(sellerReceived);
        }
