...
contract Staking is IStaking, RewardsAdministrator, ReentrancyGuard, Pausable {
    ...
    constructor() {
        rewardsAdministrator = msg.sender;
        rewardsVault = msg.sender;
        rewardsToken = IERC20(0x6f2f89Bd53F622619479E7805B2F54716f545D19);
        stakingToken = IERC20(0xc111cb11E22B622fb66083777A2BBf52665a611d);
    }
    ...
    function getRequestUnstake (address user) public view returns (uint256 amount, uint256 withdrawalTime) {
        uint256 amount = _usersReqUnstake[user].amount;
        uint256 withdrawalTime = _usersReqUnstake[user].withdrawalTime;
        return (amount, withdrawalTime);
    }
    ...
    function addRewards(uint256 reward) external override onlyRewardsAdministrator updateReward(address(0)) {
    ...
    function setStakingToken(address _stakingToken, address _rewardsToken) external onlyRewardsAdministrator {
        require(block.timestamp >= periodFinish, "Previous rewards period must be complete before changing the token for the new period");
