...
contract PowermadeTokenThresholdV2 is ExternalEligibilityContract {
    ...
    // DEBUG:
    uint private constant update_deadtime = 30 minutes;           // Minimum deadtime between price updates
    ...
    // Testnet variables
    uint public last_price_pancake;             // Divided by 10 due to testnet environment
    uint public last_price_pancake_real;        // Real price from pancake
    ...
    // For BUSD use 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56 (mainnet) 
    // Or 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7 (Testnet - https://bsc.pancake.kiemtienonline360.com/#/swap)
    constructor(address _powermadeAddress, address _powermadeToken, address _BUSD_Token, uint bypass_price) public {
        ...
        while (updated_thresholds) {
            ( , updated_thresholds) = updatePriceThresholds(true, bypass_price);
        }
    }
    ...
    function updatePriceThresholds(bool is_init, uint bypass_price) private returns (bool updated_price, bool updated_thresholds) {
        if (!is_init) {
        ...
        last_price_pancake_real = pancakeRouter.getAmountsOut(1e18, path)[2];
        last_price_pancake = last_price_pancake_real * 10;
        last_price = bypass_price > 0 ? bypass_price : last_price_pancake;
        updated_price = true;
    ...
    // Testnet function to force the update with a custom price
    function forceupdatePriceThresholds(bool is_init, uint bypass_price) external onlyPowermadeOwner returns (bool updated_price, bool updated_thresholds) {
        return updatePriceThresholds(is_init, bypass_price);
    }
