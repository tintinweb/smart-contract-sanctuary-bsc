...
contract DeuxStake is Context, Ownable {
    ...
    function getUserStakeIds(address _user)
        external
        view
    ...
    function _addUserStakeId() internal returns (uint256) {
        userStakeIds[_msgSender()].push();
        uint256 userStakeId = userStakeIds[_msgSender()].length - 1;
        userStakeIds[_msgSender()].push(userStakeId);
        return userStakeId;
    }
    ...
    function _stake(uint256 _amount, uint256 _rate) internal returns (uint256) {
        ...
        uint256 userStakeId = _addUserStakeId();

        allStakeIds.push(userStakeId);
    ...
    function _withdraw(uint256 _id, bool _emergeny) internal returns (uint256) {
        ...
        // Remove id from userStakeIds address
        uint256 j;
        uint256 arrLength = userStakeIds[current_stake.user].length;
        for (j = 0; j < arrLength; j++) {
            if (userStakeIds[current_stake.user][j] == _id) {
                userStakeIds[current_stake.user][j] = userStakeIds[
                    current_stake.user
                ][arrLength - 1];
                break;
            }
