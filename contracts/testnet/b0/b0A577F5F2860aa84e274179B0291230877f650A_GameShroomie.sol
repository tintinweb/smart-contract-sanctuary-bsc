...
contract GameShroomie is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    function deposit(uint256 _tokenId) external nonReentrant whenNotPaused {
        require(_tokenId != 0, "Deposit: No tokenIds provided");

        nftToken.safeTransferFrom(msg.sender, address(this), _tokenId);
        _tokenApprovals[_tokenId] = msg.sender;

        uint256 length = _ownedTokens[msg.sender].length;
        _ownedTokens[msg.sender].push(_tokenId);
        _ownedTokensIndex[_tokenId] = length;
        
        _deposit(1);
        emit Deposited(msg.sender, 1, _tokenId);
    }

    function withdraw(uint256 _tokenId) public nonReentrant {
        require(_tokenId != 0, "Withdraw: No tokenIds provided" );
        require( _tokenApprovals[_tokenId] == msg.sender, "Deposit: Not the depositor of the token" );

        nftToken.safeTransferFrom(address(this), msg.sender, _tokenId);
        _tokenApprovals[_tokenId] = address(0);


        uint256 lastTokenIndex = _ownedTokens[msg.sender].length - 1;
        ...
        _withdraw(1);
        emit Withdrawn(msg.sender, 1, _tokenId);
    }

    function tokenOfOwner(uint256 tokenId) public view returns (address){
    ...
    function tokenOfOwnerByIndex(address _owner, uint256 _index) public view returns (uint256) {
        require(_index < _balances[_owner], "ERC721: owner query for nonexistent token");
    ...
    event Deposited(address indexed user, uint256 amount, uint256 tokenIds);
