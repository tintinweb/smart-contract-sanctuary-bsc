...
contract ContactToken is ERC20, ERC20Burnable, Pausable, Ownable {
  ...
  address[] public taxReceivers;
  mapping(address => uint256) public taxReceiversPerThousand;
  ...
  // Tax receivers
  function isTaxReceiver(address _wallet) public view returns (bool) {
    return taxReceiversPerThousand[_wallet] > 0;
  }

  function setTaxReceiver(address _wallet, uint32 _taxPerThousand) public onlyOwner {
    ...
    bool isExistent = isTaxReceiver(_wallet);
    uint256 currentAddressTaxPerThousand = taxReceiversPerThousand[_wallet];
    ...
    if (!isExistent) {
      taxReceivers.push(_wallet);
    }

    // Update wallet perThousand
    taxReceiversPerThousand[_wallet] = _taxPerThousand;
  }

  function removeTaxReceiver(address _wallet) public onlyOwner {
    require(isTaxReceiver(_wallet), 'ContactToken: Wallet not present');

    // Find index to remove
    uint256 arrayLength = taxReceivers.length;
    uint256 indexToBeDeleted;
    for (uint256 i = 0; i < arrayLength; i++) {
      if (taxReceivers[i] == _wallet) {
        indexToBeDeleted = i;
        break;
      }
    }
    ...
    // Decrease tax total
    taxFee -= taxReceiversPerThousand[_wallet];

    // Remove from mapping
    delete taxReceiversPerThousand[_wallet];
  }

  // Tax whitelist
  function isTaxWhitelisted(address _address) public view onlyOwner returns (bool) {
    return taxWhitelistedAddresses[_address];
  }

  function addToTaxWhitelist(address _address) public onlyOwner {
    require(!taxWhitelistedAddresses[_address], 'ContactToken: Address already exists');
    taxWhitelistedAddresses[_address] = true;
  }

  function removeFromTaxWhitelist(address _address) public onlyOwner {
    require(taxWhitelistedAddresses[_address], 'ContactToken: Address not present');
    delete taxWhitelistedAddresses[_address];
  }
  ...
  function handleTax(
    ...
    for (uint256 i = 0; i < taxReceivers.length; i++) {
      address _wallet = taxReceivers[i];
      uint256 partialTax = baseUnit * taxReceiversPerThousand[_wallet];
      cumulativeTax += partialTax;
      super._transfer(from, _wallet, partialTax);
    }
