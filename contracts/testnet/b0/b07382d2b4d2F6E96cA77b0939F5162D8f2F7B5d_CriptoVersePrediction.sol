...
abstract contract JackPot is Ownable {

  bool public jackPotEnabled = true;
  uint256 public currentJackpotEpoch;
  uint256 public jackPotTicketCost = 20000000000000000;

  uint256 public currentDailyJackpotEpoch;  
  bool public jackpotDailyStart = false;
  uint[] public DailyDistribution = [40,30,20,10,1,1,1,1];

  enum JackPotType {Daily, Weekly, Monthly}
  ...
  mapping(uint256 => Jackpot) public DailyJackPot;
  mapping(uint256 => mapping(address => JackPotWinner)) public JackPotWinners;
  mapping(address => uint256[]) public UserWinnersJackpot;
  
  mapping(uint256 => mapping(address => uint256)) public JackpotDailyUserAmount;
  mapping(uint256 => address[]) public JackpotDailyUsers;


  function _dailyJackpotStart() internal {
    currentJackpotEpoch += 1;
    jackpotDailyStart = true;
    currentDailyJackpotEpoch = currentJackpotEpoch;

    Jackpot storage jackpot = DailyJackPot[currentDailyJackpotEpoch];
    jackpot.jackpotType = JackPotType.Daily;
    jackpot.startTimestamp = uint32(block.timestamp);
    jackpot.closed = false;
  }

  function _safeAddJackPotUser(uint256 amount) internal {
    require(jackpotDailyStart, "Jackpot is not started.");
    bool userExist = false;
    JackpotDailyUserAmount[currentDailyJackpotEpoch][msg.sender] += amount;
    
    for (uint256 i = 0; i < JackpotDailyUsers[currentDailyJackpotEpoch].length; i++) {
      if(JackpotDailyUsers[currentDailyJackpotEpoch][i] == msg.sender) {
        userExist = true;
        break;
      }
    }
    if(userExist == false) { JackpotDailyUsers[currentDailyJackpotEpoch].push(msg.sender); }
  }

  function ownerSetDailyDistribution(uint[] calldata _distribution) external onlyOwner {
    require(_distribution.length > 2, "Minimal two divider");
    uint sum_ = 0;
    for (uint i = 0; i < _distribution.length; i++) {
      sum_ += _distribution[i];
    }
    require(sum_ == 100, "The result of the array sum must be 100!");
    DailyDistribution = _distribution;
  }

  function dailyJackpotDraw(uint256 epoch) external onlyOwner {
    require(!DailyJackPot[epoch].closed, "The Jackpot already drawn!");

    Jackpot storage dailyJackpot = DailyJackPot[epoch];
    // address[] memory addressTickets;
    uint length = 0;
    
    for (uint256 i = 0; i < JackpotDailyUsers[epoch].length; i++) {
      address account = JackpotDailyUsers[epoch][i];
      uint tickets = _calcTicketsCustomer(epoch, account);
      for (uint256 j = 0; j < tickets; j++) {
        length++;
      }
    }

    address[] memory addressTickets = new address[](length);
    address[] memory addressTicketSorted = new address[](length);
    ...
    for (uint256 i = 0; i < JackpotDailyUsers[epoch].length; i++) {
      ...
      uint tickets = _calcTicketsCustomer(epoch, account);
      for (uint256 j = 0; j < tickets; j++) {
        addressTickets[indexTicket] = account;
        indexTicket++;
      }
    }

    dailyJackpot.totalTickets = addressTicketSorted.length;

    addressTicketSorted = _shuffle(addressTickets);
    for (uint256 k = 0; k < DailyDistribution.length; k++) {
      uint n = k + uint(keccak256(abi.encodePacked(block.timestamp))) % (addressTicketSorted.length - k);

      if(JackPotWinners[epoch][addressTicketSorted[n]].amount > 0) {
        k--;
        continue;
      }

      uint256 amountWining = DailyJackPot[epoch].rewardAmount * (DailyDistribution[k] / 100);
      JackPotWinner memory winner = JackPotWinners[epoch][addressTicketSorted[n]];
      winner.position = k+1;
      winner.claimed = false;
      winner.amount = amountWining;
      JackPotWinners[epoch][addressTicketSorted[n]] = winner;
      
      if(JackpotDailyUsers[epoch].length == k+1) {
        break;
      }
    }

    dailyJackpot.pickWinTimestamp = uint32(block.timestamp);
    dailyJackpot.closed = true;
  }

  function _shuffle(address[] memory tickets) internal view returns (address[] memory) {
    for (uint i = 0; i < tickets.length; i++) {
        uint n = i + uint(keccak256(abi.encodePacked(block.timestamp))) % (tickets.length - i);
  ...
  function _calcTicketsCustomer(uint256 epoch, address account) internal view returns (uint) {
    uint256 amount = JackpotDailyUserAmount[epoch][account];
    uint256 credit;
    uint256 ticketsAmount;
    credit = amount % jackPotTicketCost;
    ticketsAmount = (amount - credit) / jackPotTicketCost;
    return ticketsAmount;
  }

  function _safeJackpotsRewardAdd(uint256 amount) internal {
      Jackpot storage dailyJackpot = DailyJackPot[currentDailyJackpotEpoch];
      dailyJackpot.rewardAmount += amount;
      dailyJackpot.totalUsers = JackpotDailyUsers[currentDailyJackpotEpoch].length;
  }
}

contract CriptoVersePrediction is Ownable, Pausable, ReentrancyGuard, JackPot {
  ...
  uint256 public intervalSeconds = 60; // 300
  ...
  function _calculateRewards(uint256 epoch) internal {
    ...
    uint256 feeAmount = (totalAmount - rewardAmount);
    uint256 potAdd = (feeAmount / 2);
    potAmount += potAdd;
    _safeJackpotsRewardAdd((feeAmount - potAdd));
  ...
  function controlJackpotStart() external onlyOwnerOrOperator whenNotPaused {
    _dailyJackpotStart();
  }
