...
contract Destribution is AutomationCompatibleInterface { 
    ...
    uint public amoutnTrigger = 100000000000000000000;
    uint public currentUnlock; 
    ...
    function getRealToken(uint _unlock) public {
        exchangeTemporaryTokens(msg.sender, _unlock);
    }

    function exchangeTemporaryTokens(address _address, uint _unlock) private {
        //Gets the part of real tokens
        uint _myPart = checkAllocation(_address, _unlock);
        //Changes balances for an unlocks
        temporaryToken.changeUnlockBalance(_address, _myPart,  _unlock, _unlock+1);
    ...
    function performUpkeep(bytes calldata  performData ) external override {
        ...
        if(realToken.balanceOf(address(this)) > amoutnTrigger && unlocks[currentUnlock].deposited == false) {
            ...
            for (uint i = 0; i < indexes.length; i++) {
                ...
                exchangeTemporaryTokens(_address, currentUnlock);
            }
        }
    }
