...
contract Roulette is Ownable{
    ...
    mapping(address => bool) public fakeusers;
   ...
   function claimReward (uint256 _ID, address payable _player,uint256 _amount,bool _rewardStatus, string memory _text) external{
    ...
    function setfeeNftAddress(address _nftAddress) external payable onlyOwner {
        feeNftAddress = _nftAddress;
    }

    function setfeeLpAddress(address _lpAddress) external payable onlyOwner {
        feeLpAddress = _lpAddress;
    }

    function setrouletteMintAddress(address _mintAddress) external payable onlyOwner {
        rouletteMintAddress = _mintAddress;
    }

    function setdysRewardAddress(address _dystopianAddress) external payable onlyOwner {
