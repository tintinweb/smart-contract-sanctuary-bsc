...
contract EtherStore_NOAPI {
    ...
    {
        require(isGW(msg.sender), "Only GW can call");//the problem is im checkin msg.sender i need to  check the from of internal transcation of the deleagte
        price =_price;
    ...
    roman = _new_name;
      ...
      function _calc(int256 _num1, int256 _num2)  internal  returns (int256)
    {
        if (isGW(msg.sender)){
        sum = _num1 + _num2;
