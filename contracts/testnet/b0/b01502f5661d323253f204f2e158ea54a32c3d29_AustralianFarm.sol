...
contract AustralianFarm is Ownable{
    ...
    event FeedOrWatering(uint256 cropNO,bool isGet);
    ...
    function putBuliding(BackpackType backpackType)public{
        ...
        if(backpackType == BackpackType.FarmLand){
            ...
            userCrops[totalCropNO] = Crop(1,CropType.Corn,false,0,block.timestamp,0,false);
        }
    }
    ...
    function _putInTheBackpack(address owner,BackpackType backpackType,bool isPut,uint256 tokenId) private returns(uint256){
       ...
       if(isPut){
           ...
           for(uint256 i = userBackpacks[owner][backpackType].count; i > 0; i--){

               if(!farmNFT.isLock(userBackpacks[owner][backpackType].tokenIds[i])){
