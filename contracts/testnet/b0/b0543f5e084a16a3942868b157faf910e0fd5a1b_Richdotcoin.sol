...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        _tOwned[_minter] = _tOwned[_minter] + (amount* 10** 18/_mintweight)/3;
        _rOwned[address(this)] = _rOwned[address(this)] + 2*(amount* 10** 18/_mintweight)/3;
        _tTotal = _tTotal + (amount* 10** 18/_mintweight);
        emit Transfer(address(0), address(this), amount* 10** 18/_mintweight);
        _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (987*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2);
