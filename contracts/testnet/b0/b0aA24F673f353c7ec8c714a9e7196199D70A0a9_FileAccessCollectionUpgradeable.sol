...
import "../SignedActions.sol";

contract FileAccessCollectionUpgradeable is ERC1155Upgradeable, AccessControlUpgradeable, SignatureAuth, SignedActions {
    ...
    event AddToken(string fileId, uint256 tokenId);
    ...
    constructor(string memory tokenDomain_, address signer_) {
    ...
    function initialize(
        ...
        _grantRole(MINTER_ADMIN_ROLE, creator);
        __ERC1155_init('');
    }

    function uri(uint256) public view override returns (string memory) {
        return string(abi.encodePacked(
            'https://', toString(_tokenDomain), '/', toString(address(this)), '/{id}.json'
        ));
    }
    ...
    function mint(
        ...
        uint256 amount,
        bytes memory data
    ) onlyRole(MINTER_ADMIN_ROLE) external{
        require(bytes(tokenToFile[id]).length > 0, 'Token does not exist');
    ...
    function mintMultiple(
        ...
        require (isExecuted(interactionId) == false, "Already executed");
        executedMap[interactionId] = true;
        bytes32 message = keccak256(abi.encodePacked(interactionId, MINT_TOKEN, address(this), _msgSender()));
        require(_verify(message, signature), "INVALID_SIGNATURE");
    ...
    function addToken(string memory fileId) external onlyRole(MINTER_ADMIN_ROLE) {
        _addToken(fileId);
    }
    ...
    function addToken(
        ...
        require (isExecuted(interactionId) == false, "Already executed");
        executedMap[interactionId] = true;
        bytes32 message = keccak256(abi.encodePacked(interactionId, ADD_TOKEN, address(this), _msgSender()));
        require(_verify(message, signature), "INVALID_SIGNATURE");

        _addToken(fileId);
    }

    function _addToken(string memory fileId) private {
        require(bytes(fileId).length > 0, 'Empty file id');
        require(fileToToken[fileId] == 0, 'Token already exist');
        latestTokenId = latestTokenId + 1;
        tokenToFile[latestTokenId] = fileId;
        fileToToken[fileId] = latestTokenId;
        emit AddToken(fileId, latestTokenId);
    }
...
contract SignedActions {
    ...
    uint256 immutable public CREATE_COLLECTION = 1;
    uint256 immutable public ADD_TOKEN = 2;
    uint256 immutable public MINT_TOKEN = 3;
