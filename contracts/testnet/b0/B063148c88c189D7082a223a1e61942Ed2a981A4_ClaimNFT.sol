...
contract ClaimNFT is OwnableUpgradeable, PausableUpgradeable {
    ...
    function initByOwner(bool _isClaim, PassportNFT _passportNFT,IBEP20 _token)
        public
        // ,IBEP20 _bep20
        onlyOwner
    {
    ...
    function registerClaimNFT(string memory ref) external whenNotPaused {
        ...
        indexOfClaimNFTs[msg.sender] = newInfoClaimNFT;
        infoClaimNFTs[newInfoClaimNFT].receiver = msg.sender;
        infoClaimNFTs[newInfoClaimNFT].timeClaim = block.timestamp + timeWait;
        infoClaimNFTs[newInfoClaimNFT].ref = ref;
        string memory _refs = getSlice(3, 8);
        refs[msg.sender] = _refs; 
        refAddresses[_refs] = msg.sender;
        emit claimNFTEvent(newInfoClaimNFT);
    }
    ...
    function claimNFT() external whenNotPaused {
        ...
        require(infoClaimNFTs[index].timeClaim  > block.timestamp , "You can not claim");
        ...
        if(index > 500 && index <= 600){
            ...
            uint256 feeUp = 1 + (1000 - index)/500;
            fee = 5 + feeUp;
        }
        if(fee!=0){
        token.approve(address(this), fee*10**18);
        token.transferFrom(msg.sender, address(this), fee*10**18);
        }
        uint256 id = passportNFT.createPassport(msg.sender, 1, 1);
...
function getSlice(uint256 begin, uint256 end) public view returns (string memory) {
...
// }
    ...
    function setPassportNFT(PassportNFT _passportNFT) public onlyOwner {
        passportNFT = _passportNFT;
    }
    ...
    PausableUpgradeable
{
    ...
    // using PassportsUpgradeable for Passport;
    ...
    function initialize() public initializer {
        __ERC721_init("Passport NFT METAECO", "PANM");
        __Ownable_init();
    }

    mapping(address => mapping(uint256 => uint256)) public passports; // address - id - details // cach lay details = passports[address][passportId]
    mapping(uint256 => address) public passportIndexToOwner;
    ...
    function createPassport(
        ...
        // _safeMint(msg.sender, newPassportId);
        _safeMint(owner, newPassportId);
        passports[owner][newPassportId] = encode(
            Passport(newPassportId, passportType, rank, block.timestamp, 0)
        );
        passportIndexToOwner[newPassportId] = owner;
    ...
    function updatePassport(
        ...
        passports[owner][nftId] = encode(
            Passport(id, passportType, rank, createdTime, isDeleted)
        );
        return nftId;
    }

    function getPassport(address owner, uint256 id)
        public
        view
        returns (Passport memory _passport)
    {
        uint256 details = passports[owner][id];
        _passport.id = uint256(uint48(details >> 100));
        _passport.passportType = uint256(uint16(details >> 148));
        _passport.rank = uint256(uint16(details >> 164));
        _passport.createdTime = uint256(uint32(details >> 196));
        _passport.isDeleted = uint256(uint8(details >> 228));
    }

    function getPassportPublic(address _owner, uint256 _id)
        public
        view
        returns (
            uint256 id,
    ...
    {
        Passport memory _passport = getPassport(_owner, _id);
        id = _passport.id;
        passportType = _passport.passportType;
        rank = _passport.rank;
        createdTime = _passport.createdTime;
        isDeleted = _passport.isDeleted;
    }

    function encode(Passport memory passport) public pure returns (uint256) {
        // function encode(Passport memory passport)  external view returns  (uint256) {
        uint256 value;
        value = uint256(passport.id);
        value |= passport.id << 100;
        value |= passport.passportType << 148;
        value |= passport.rank << 164;
        value |= passport.createdTime << 196;
        value |= passport.isDeleted << 228;
        return value;
    }
    ...
    function getPassportOfSender(address sender)
        external
        view
        returns (Passport[] memory)
    {
        ...
        uint256 x = 0;
        for (i; i <= range; i++) {
            Passport memory passport = getPassport(sender, i);
            if (passport.id != 0) {
                index++;
            }
        }
        Passport[] memory result = new Passport[](index);
        i = 1;
        for (i; i <= range; i++) {
            Passport memory passport = getPassport(sender, i);
            if (passport.id != 0) {
    ...
    function transfer(uint256 _nftId, address _target) external whenNotPaused {
        ...
        // if (msg.sender != passportMarketPlace) {
        //     require(
        //         _target == passportMarketPlace,
        //         "function only support for Marketplace"
        //     );
        //     require(msg.sender != _target, "Can not transfer myself");
        // }
        Passport memory passport = getPassport(ownerOf(_nftId), _nftId);
        passports[_target][_nftId] = encode(passport);
        passports[ownerOf(_nftId)][_nftId] = encode(Passport(0, 0, 0, 0, 0));
        passportIndexToOwner[_nftId] = _target;
    ...
    function transferFrom(
        ...
        Passport memory passport = getPassport(from, tokenId);
        passports[to][tokenId] = encode(passport);
        passports[from][tokenId] = encode(Passport(0, 0, 0, 0, 0));
        passportIndexToOwner[tokenId] = to;
    ...
    function updatePassportIndexToOwner(uint256 nftId, address owner)
        public
        onlyOwner
    {
        passportIndexToOwner[nftId] = owner;
    }
        ...
        returns (string memory)
    {
        ...
        Passport memory passport = getPassport(sender, tokenId);
        ...
        return
            bytes(baseURI).length > 0
                ? string(
                    abi.encodePacked(
                        baseURI,
                        passport.passportType.toString(),
                        "_",
                        passport.rank.toString(),
                        json
                    )
                )
                : "";
