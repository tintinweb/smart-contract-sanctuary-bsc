...
contract AuctionHouse is IERC721Receiver, Ownable {
    ...
    function requestFinishAuction(uint256 _auctionId) public payable {
        ...
        require(auction[_auctionId].endTime < block.timestamp, "Auction has not ended");

        // If no bidder, cancel auction
        ...
        payable(address(auction[_auctionId].auctioneer)).transfer(auctioneerReceive);

        auction[_auctionId].completed = true;
        auction[_auctionId].active = false;
    }

    function finishAuction(uint256 _auctionId) public payable onlyAuctioneer(_auctionId) {
    ...
    function cancelAuction(uint256 _auctionId) public payable onlyAuctioneer(_auctionId) {
        ...
        // Refund token to previous bidder
        if (auction[_auctionId].lastBidder != address(0)) {
            payable(address(auction[_auctionId].lastBidder)).transfer(auction[_auctionId].lastBid);
        }
