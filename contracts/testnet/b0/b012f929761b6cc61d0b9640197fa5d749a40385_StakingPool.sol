...
contract StakingPool is Upgradable {
   ...
   */
    ...
    */
    function rewardPerToken(string memory poolId) public view returns (uint256) {
        ...
        if (currentTimestamp <= pool.lastUpdateTime || pool.configs[5] == 1) return pool.rewardPerTokenStored;
   ...
   */
    ...
    */
    ...
    function showConfigs(string memory poolId) external view onlyAdmins poolExist(poolId) returns(uint256[] memory) {
        PoolInfo storage pool = poolInfo[poolId];
        return pool.configs;
    }
