...
contract FILY is ERC20 {
    ...
    uint256 public totalStakeAmount; // 所有人的总铸造数量,为了前端查询方便
    ...
    constructor() {
        ...
        // Router = address(0xCc7aDc94F3D80127849D2b41b6439b7CF1eB4Ae0); // 测试网
    ...
    // 测试版本的构造函数
    // constructor(address _USDT) ERC20("test", "test") {
    //     TopPrice24H = 1e18; // 价格
    //     LastPrice = 1e18; // 开场价格
    //     BOSS = msg.sender; // 上线改
    //     Admin = msg.sender;
    ...
    //  for test
    // function init(address _LP) external OnlyOperator {
    //     LPInstance = IERC20(_LP);
    //     uniswapV2Pair = _LP; // For test 上线删除
    // }
    ...
    function _transfer(
        ...
        if ( from == address(uniswapV2Pair) && TaxEnabled ) {
            if ( totalSupply() > 60 * 1e4 * 1e18) { // 最后剩下60万就不销毁了
    ...
    function updateTEAMAndReward(
        ...
        if (_upper != address(0) && !BlackList[_upper]) {
            ...
            // 级差
            uint LevelGap;
            if (getTEAMLevel(_upper) > getTEAMLevel(account)) {
                LevelGap = getTEAMLevel(_upper) - getTEAMLevel(account);
            }
            // 这里结算上级极差奖励，极差奖励只有一层
            
            UserTEAMRewardNoPaid[_upper] +=
                (block.timestamp - UserLastTEAMWithDrawTimestamp[account]) * // 这是给上级结算的时间记录在下级的头上
                TEAMStakeAmount[account] * // 下级的质押业绩
                LevelGap * // 级差 0 - 5
                11574074074074;
            UserLastTEAMWithDrawTimestamp[account] = block.timestamp; // 记录最后一次给上级的结算时间
    ...
    function getUserStakeReward(address account) public view returns (uint256) {
        // 查询自己的质押奖励
        uint _stakeReward = UserStakeAmount[account] *
            (block.timestamp - UserLastStakeWithDrawTimestamp[account]) *
            getTEAMLevel(account) *
            11574074074074;
    ...
    function withdrawStakeReward() external EOA {
        ...
        uint _teamReward = UserTEAMRewardNoPaid[msg.sender];
        UserTEAMRewardNoPaid[msg.sender] = 0; // 先清空未领取奖励
        UserLastStakeWithDrawTimestamp[msg.sender] = block.timestamp; // 记录最后一次提款时间
        // 团队时间戳不用记录，因为是下级结算的
        require(totalSupply() <= 6000 * 1e4 * 1e18 - FarmAmountLimit, "totalSupply < 1200w"); // 铸造上限
        super._mint(
            msg.sender,
            reward + _teamReward
        );
