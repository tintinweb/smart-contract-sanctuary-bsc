...
contract SharesReward is Ownable {
    ...
    event GetReward(address token, address to, uint256 amount);
    ...
    function snapshotPool(address[] memory tokens) public onlyController {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            snapshotPoolTokenAmount[lastSnapshotId][tokens[i]] = balance;
        }
    }
    ...
    function getReward(address token) public {
        ...
        hasReward[lastSnapshotId][token][msg.sender] = true;
        emit GetReward(token, msg.sender, amount);
    }
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > lastSnapshotTime + period) {
            return 0;
        }

        if (hasReward[lastSnapshotId][token][account]) {
            return 0;
        }

        uint256 balance = IERC20(sharesToken).balanceOfAt(account,lastSnapshotId);
        uint256 totalSupply = IERC20(sharesToken).totalSupply();
        uint256 poolAmount = snapshotPoolTokenAmount[lastSnapshotId][token];
