...
/**
 *Submitted for verification at BscScan.com on 07/01/2023
*/

/**
 * ADAT Contract
 * Telegram: ADAT Official https://t.me/+B7mER7XlQP85NDM0
...
/*
 ***Main features - subject to change in the course of the project either by community demand or what is best for the project, moving forward***
* 0 % Tax Fee (Holders)
* 2 % Liquidity Fee
* 1 % Auto Burn
* 2 % Marketing wallet
* 5 % Projects and Charity wallet
*/
 ...
 */
contract ADAT is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    uint256 private constant _tTOTAL = 1414000 * 1e6 * 10**_DECIMALS;
     ...
     * construction.
     */
    string private constant _NAME = "A Decentralised Africa Token";
    string private constant _SYMBOL = "ADAT";
     ...
     */   
    uint256 public _projectsandcharityFee = 5;
...
uint256 public constant MIN_TOTAL_SUPPLY = 1000 * 1e6 * 10**18;

// The total supply of the token
uint256 private _totalSupply = 1414000 * 1e6 * 10**_DECIMALS;

// Burn until Minimum total supply of 1 billion ADAT is reached
    function burn(uint256 _amount) public {
        require(_totalSupply - _amount >= MIN_TOTAL_SUPPLY, "Cannot burn tokens because it would bring the total supply below the minimum total supply");
     ...
     * Whale actions.
     */       
    uint256 public _maxTxAmount = 7 * 1e6 * 10**_DECIMALS;
    ...
    constructor () {
        ...
        IPancakeRouter02 _pancakeswapV2Router =
            IPancakeRouter02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //main- 0x10ED43C718714eb63d5aA57B78B54704E256024E 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 testnet
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Calculate burn amount, marketing amount and projectsandcharity amount
        uint256 burnAmt = amount.mul(_burnFee).div(100);
        uint256 marketingAmt = amount.mul(_marketingFee).div(100);
        uint256 projectsandcharityAmt = amount.mul(_projectsandcharityFee).div(100);

         if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(projectsandcharityAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(projectsandcharityAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(projectsandcharityAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(projectsandcharityAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(marketingAmt).sub(projectsandcharityAmt)));
        }
        ...
        //Send transfers to burn address, marketing wallet and projectsandcharity wallet
        ...
        _transferStandard(sender, marketingWallet, marketingAmt);
         ...
         _transferStandard(sender, projectsandcharityWallet, projectsandcharityAmt);
    ...
    function setProjectsandcharityFeePercent(uint256 projectsandcharityFee) external onlyOwner() {
    ...
    function setMaxTxAmount(uint256 newAmount) external onlyOwner() {
        require(newAmount >= 3 * 1e6 * 10**_DECIMALS, 
     ...
     * wallet and projectsandcharity wallet.
     *
     * Requirements:
     *
