...
function betm(uint8 yz,uint8 ma) public view returns(uint32 won,string memory ff){
  uint256 k = 344;
  uint32[15] memory temp = getrandom(k);
  uint8 key = uint8(uint256(keccak256(abi.encodePacked(block.timestamp,k ))) % 1000);
