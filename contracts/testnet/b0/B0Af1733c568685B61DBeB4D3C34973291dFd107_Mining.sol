...
contract Mining is Ownable, ReentrancyGuard {
    ...
    struct IssueMsg {
        ...
        uint256 gloryCardCount;               // glory card deposit lp total.(only depoist eteCardToken).
    ...
    address public lockPositions;          // lock positions contract address.
    uint256 public canRatio = 200;         // account can deposit one issue is ratio. default 200%%.
    uint256 public burnCrystalsRatio = 2;  // burn crystal ratio. default is 2%.
    ...
    mapping(address => uint256) public burnCardTakedCount; // burn card taked count.
    ...
    constructor(
        ...
        crystal = crystal_;
        lockPositions = lockPositions_;

        // default first turn first issue.
        turn = 1;
        issue[turn] = 1;
        // start first turn first issue.
        _startIssue(turn, issue[turn], 2000*(10**18));
    }
    ...
    event TeamEarnTake(uint256 turn, uint256 issue, address teamEarn, uint256 earnAmount);                   // team earn.
    event SetNowIssueEndTime(uint256 turn, uint256 issue, uint256 oldEndTime, uint256 nowEndTime);           // change now issue end time.
    event TakeEarnBurnCard(address user, uint256 cardNFT, uint256 takedEarn, uint256 burnLimitEarn);         // taked earn then burn card.
    ...
    function setLockPositions(address _lockPositions) public onlyOwner {
        require(_lockPositions != address(0), "0 address error");
        lockPositions = _lockPositions;
    }
    ...
    function _startIssue(uint256 _turn, uint256 _issue, uint256 _totalUsdt) private {
        uint256 _third = _totalUsdt.div(3);
        ...
        _IssueMsg[_turn][_issue] = IssueMsg({
            ...
            userCount: _third,
            gloryCardCount: _third,
            starCardCount: _totalUsdt.sub(_third).sub(_third),
    ...
    function userDeposit(uint256 tokenAmount) public nonReentrant {
        ...
        address depositTokenAddress = eteCardToken;
        if(_cardNFT == 0) {
            ...
            depositTokenAddress = eteUserToken;
        }else if(_cardNFT == 1) {
            require(_nowIssueMsg.gloryCardCount > 0, "not count1");
            tokenAmount = _nowIssueMsg.gloryCardCount >= tokenAmount ? tokenAmount : _nowIssueMsg.gloryCardCount;
            _nowIssueMsg.gloryCardCount = _nowIssueMsg.gloryCardCount.sub(tokenAmount);
            // card deposit count
        ...
        if(_nowIssueMsg.userCount == 0 && _nowIssueMsg.gloryCardCount == 0 &&  _nowIssueMsg.starCardCount == 0) {
            // now issue is 2.
            ...
            if(_nowIssue > 1) {
                ...
                // last issue start earn. teamEarn contract = 11%, leader address = 1%. leader2 address = 1%.
                uint256 _price = _IssueMsg[turn][_lastIssue].usdtPriceETE;
                uint256 _v0 = _IssueMsg[turn][_lastIssue].totalUsdt.mul(_price).div((1e18)).mul(1100).div(10000);
                ...
                emit TeamEarnTake(turn, _lastIssue, teamEarn, _v0);
    ...
    function userTake(bytes32 order) public nonReentrant {
        ...
        uint256 _earnMy = _valueEte.mul(_ratioMy).div(100);               // my earin.
        uint256 _earnMySuper = _valueEte.div(100);                        // super1 earn = 1%.
        uint256 _earnMyOtherSuper = _valueEte.mul(5).div(1000);           // super2 - super5 earn = 0.5%.
        ...
        // burn card
        _burnCard(_orderMsg, _user, _earnMy, _usdtPriceETE);
    }
    ...
    function _burnCrystal(OrderMsg storage _orderMsg, address _user) private {
        ...
        uint256 _burnNumber = crystalAmount.mul(burnCrystalsRatio).div(100);
        ICrystal(crystal).burnCrystal(_user, _burnNumber);
    }

    function _burnCard(OrderMsg storage _orderMsg, address _user, uint256 _earnMy, uint256 _usdtPriceETE) private {
        // 3 mul earn then burn card.
        if(_orderMsg.cardNFT > 0) {
            burnCardTakedCount[_user] = burnCardTakedCount[_user].add(_earnMy); // is card then count.
            uint256 _eteAmount = ILockPositions(lockPositions).lockPriceUSDT(_orderMsg.cardNFT).mul(_usdtPriceETE).div(1e18).mul(burnCardEarnMul);
            if(burnCardTakedCount[_user] > _eteAmount) {
                burnCardTakedCount[_user] = burnCardTakedCount[_user].sub(_eteAmount);
                ICardNFT(cardNFT).burnCard(_user);
                // emit
                emit TakeEarnBurnCard(_user, _orderMsg.cardNFT, burnCardTakedCount[_user], _eteAmount);
            }
        }
    }
