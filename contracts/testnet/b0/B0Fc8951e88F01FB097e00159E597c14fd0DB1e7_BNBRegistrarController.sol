 ...
 */
contract BNBRegistrarController is Ownable, IBNBRegistrarController {
    ...
    function makeCommitment(
        ...
        if (data.length > 0) {
            require(
                resolver != address(0),
                "BNBRegistrarController: resolver is required when data is supplied"
            );
        }
        return
            keccak256(
    ...
    function register(
        ...
        require(
            ...
            "BNBRegistrarController: Not enough BNB provided"
        );
        ...
        uint256 expires = nameWrapper.registerAndWrapBNB2LD(
            name,
            owner,
            duration,
            resolver,
        ...
        override
    {
        ...
        require(
            ...
            "BNBController: Not enough BNB provided for renewal"
        );
        ...
        returns (bool)
    {
        return
            ...
            interfaceID == type(IBNBRegistrarController).interfaceId;
    }
    ...
    function _consumeCommitment(
        ...
        require(
            ...
            "BNBRegistrarController: Commitment is not valid"
        );

        // If the commitment is too old, or the name is registered, stop
        require(
            commitments[commitment] + maxCommitmentAge > block.timestamp,
            "BNBRegistrarController: Commitment has expired"
        );
        require(available(name), "BNBRegistrarController: Name is unavailable");
    ...
    function _setRecords(
        ...
        // use hardcoded .BNB namehash
        bytes32 nodehash = keccak256(abi.encodePacked(BNB_NODE, label));
        for (uint256 i = 0; i < data.length; i++) {
            // check first few bytes are namehash
            bytes32 txNamehash = bytes32(data[i][4:36]);
            require(
                txNamehash == nodehash,
                "BNBRegistrarController: Namehash on record do not match the name being registered"
            );
            resolver.functionCall(
                data[i],
                "BNBRegistrarController: Failed to set Record"
            );
        }
    }
