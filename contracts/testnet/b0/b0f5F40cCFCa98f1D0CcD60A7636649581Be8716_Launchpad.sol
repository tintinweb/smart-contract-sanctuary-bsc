...
contract BatchAuction is Ownable, ReentrancyGuard {
  ...
  function initAuction(
    ...
    if (COMMIT_CURRENCY == ETH_ADDRESS) {
      ...
      WITHDRAW_CAP_MIN = _isDebug ? 5e15 : 5e18; // 5 BNB
      WITHDRAW_CAP_MAX = _isDebug ? 250e15 : 250e18; // 250 BNB
      WITHDRAW_CAP_LIMIT = _isDebug ? 75e15 : 75e18; // 75 BNB
      WITHDRAW_CAP_INTERCEPT = 14; 
      WITHDRAW_CAP_INTERCEPT_PLUS = _isDebug ? 175e15 : 175e18; // 175 BNB
  ...
  /**
   * @notice Get estimated instant token amount after deposit
   */
  function getInstantTokenAmountAfterDeposit(address _user, uint256 _deposit) external view returns (uint256) {
    require(COMMITMENTS[_user].add(_deposit) > 0, "NO_COMMITMENTS");
    uint256 userShare = (COMMITMENTS[_user].add(_deposit)).div(10).mul(7); // 70%
    return auctionData.totalOfferingTokens
            .mul(userShare)
