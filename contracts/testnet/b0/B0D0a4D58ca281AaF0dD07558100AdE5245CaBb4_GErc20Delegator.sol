...
contract GErc20Delegator is GTokenInterface, GErc20Interface, GDelegatorInterface {
   ...
   /**
    ...
    /**
     * @notice estimates the max amount that be reduced by the admin from the market
     * @dev calculates max reduce amount based on discounts and totalReserves
     * @return uint reduceAmountMax, the max amount that be reduced by the admin from the market
     */
    function estimateMaxReduceReserves() external view returns (uint) {
        bytes memory data = delegateToViewImplementation(abi.encodeWithSignature("estimateMaxReduceReserves()"));
        return abi.decode(data, (uint));
    }
