...
contract PlanetFarm is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
   ...
   function stakeGammaOnInfinityvault(uint _gammaAmount) internal {
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            ...
            if(_pid == iGammaPid) {
                updateFactorInternal(_msgSender());
                GammaTrollerAddress.updateFactor(_msgSender(),IERC20(iGammaAddress).balanceOf(_msgSender()));
            }
        }
    ...
    function depositAuthorized(address _user,uint256 _wantAmt, bool takeWantFromMsgsender) internal  {
        ...
        if (_wantAmt > 0) {
            ...
            updateFactorInternal(_user);
            GammaTrollerAddress.updateFactor(_user,IERC20(iGammaAddress).balanceOf(_user));
    ...
    function updateFactorByItoken(address _user, address _recepient, uint256  _amount) external {
        ...
        _updateUserAndPool(recepient, pool, _amount, true);
        updateFactorInternal(_user);
        updateFactorInternal(_recepient);
        GammaTrollerAddress.updateFactor(_user,IERC20(iGammaAddress).balanceOf(_user));
        GammaTrollerAddress.updateFactor(_recepient,IERC20(iGammaAddress).balanceOf(_recepient));
    ...
    function updateFactorInternal(address _user) internal {

        uint256 len = usersPoolList[_user].length;
        uint256 iGammaBalance = IERC20(iGammaAddress).balanceOf(_user);
        ...
        for(uint256 i = 0; i < len; ++i) {
            ...
            uint256 newFactor = _getUserFactor(shares, iGammaBalance);
   ...
   // Withdraw LP tokens from Strategy.
    ...
    function unstakeAfterMinWithdrawTime(uint256 _pid) external notContract{
        unstakeInternal (_pid, 0, false);
    }

    function unstakeInstantly(uint256 _pid, uint256 unstakeAmount ) external notContract{
        unstakeInternal (_pid, unstakeAmount, true);
    }

    function unstakeInternal (uint256 _pid, uint256 unstakeAmount, bool isInstantUnstake) internal {
        ...
        uint256 temp_exchange_rate;
        uint totalItokensUnstaking;
        uint useriTokenBalance = IERC20(pool.iToken).balanceOf(_msgSender());

        if (isInstantUnstake)
        {
            currentAmount =  unstakeAmount * 1e18 / GToken(pool.gToken).exchangeRateStored();
            temp_exchange_rate  = IStrategy(pool.strat).iTokenExchangeRate(pool.iToken);

        }
        else{
            itokens = user.iTokenToBeUnstaked;
            totalItokensUnstaking = user.iTokenToBeUnstaked;
            ...
            currentAmount = user.gTokenToBeUnstaked;
            temp_exchange_rate  = (currentAmount * 1e18) / itokens;
        }
        
        uint sharesRemoved;
        uint wantAmtTobeSentToUser;
        uint stratWantTokensRemoved;
        if (currentAmount > 0) {
            (sharesRemoved, wantAmtTobeSentToUser, stratWantTokensRemoved ) = IStrategy(pool.strat).unstake(currentAmount, false);
        }

        itokens = (stratWantTokensRemoved * 1e18) / temp_exchange_rate;
        ...
        if(pool.iToken != iGammaAddress && isInstantUnstake){
