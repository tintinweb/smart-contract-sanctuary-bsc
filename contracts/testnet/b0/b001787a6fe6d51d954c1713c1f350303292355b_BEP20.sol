...
contract BEP20 is IBEP20, Ownable {
  ...
  string private constant NAME = "Movil Reup";
  string private constant SYMBOL = "MREUP";
...
// File: contracts/movilreuptoken.sol
...
contract movilreuptoken is BEP20 {
  ...
  function recoverBEP20(IBEP20 token, address recipient) external onlyOwner {
    require(address(token) != address(this), "Can't withdraw MREUP");
  ...
  function setTaxes(uint256 newBuyTax, uint256 newSellTax, uint256 newTransferTax) external onlyOwner {
    require(newBuyTax <= 300 && newSellTax <= 300 && newTransferTax <= 300, "Too high taxes");
    buyTax = newBuyTax;
    sellTax = newSellTax;
