...
contract MutilTransfer is Ownable {
    ...
    uint public  fee     = 0.0002 * 10 ** 18;
    
    function transferEth(address payable[] memory to, uint256[] memory amount) public payable {
    ...
    function transferToken(IERC20 token, address[] memory to, uint256[] memory amount) public payable {
        ...
        require(token.allowance(msg.sender, address(this)) >= allAmount);
        for (uint256 i = 0; i < length; i++) {
