...
contract YesToken is ERC20Detailed, Ownable {
    ...
    address public rewardReceiver = 0x96324F71D7dBfe14fcB52a7b16cABF28b54984CC;
    address public treasuryReceiver = 0xA5A09572D40740755d560b0153D42480794955c3;
    address public buyBackBurnReceiver = 0x72C75b0e688230638Ee9d673197aae18B27bE45B;
    ...
    uint256 public buyFeeBBB = 3;
    uint256 public sellFeeRewardAdded = 1;
    uint256 public sellFeeBBBAdded = 1;
    uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(rewardFee).add(buyFeeBBB).add(burnFee);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeRewardAdded).add(sellFeeBBBAdded);
    ...
    constructor() ERC20Detailed("YES ZOB", "YES", uint8(DECIMALS)) {
        ...
        _isFeeExempt[buyBackBurnReceiver] = true;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToTreasury = contractTokenBalance.sub(amountToLiquify).sub(amountToBBB);
        uint256 amountToReward = contractTokenBalance.sub(amountToLiquify).sub(amountToBBB).sub(amountToTreasury);
        ...
        if(amountToBBB > 0){
            _swapTokensForBNB(amountToBBB, buyBackBurnReceiver);
        }
        ...
        emit SwapBack(contractTokenBalance, amountToLiquify, amountToBBB, amountToTreasury, amountToReward);
    }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _buyBackBurnReceiver, address _rewardReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        buyBackBurnReceiver = _buyBackBurnReceiver;
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToBBB,uint256 amountToTreasury, uint256 amountToReward);
