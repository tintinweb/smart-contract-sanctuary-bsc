...
// File: FootBallFansOrganization.sol


contract FootBallFansOrganization is ERC721 ,Ownable {
    ...
    address public settleToken; // token contract

    address public opsAddress; // ops contract
    ...
    mapping(uint8 => Collection) private collectionList;

    event MintFor( address indexed nftAddress,address indexed seller,uint,uint,uint, uint);

    event Mint( address indexed nftAddress,address indexed seller,uint,uint,uint);


    constructor(address _settleToken,
                address payable _paymentReceiver, 
                uint256 _maxSupply) 
                public ERC721("FootBallFansStar", "FFS") {
        paymentReceiver = _paymentReceiver;

        require(_paymentReceiver != address(0), "Payment receiver not set");
        require(_settleToken != address(0), "token contract not set");

        maxSupply = _maxSupply;
        paymentReceiver = _paymentReceiver;
        settleToken = _settleToken;
        opsAddress = msg.sender;
    }
     ...
     * @dev Get CollectionId for a specific tokenId.
     */
    function getCollectionTokens(uint8 _collectionId) external view returns (uint256) {
        return collectionCount[_collectionId] ;
    }

    /**
     * @dev Get the associated CollectionName for a specific CollectionId.
     */
    function getCollection(uint8 _collectionId)
        external
        view
        returns (Collection memory)
    {
        return collectionList[_collectionId];
    }

    /**
     * @dev Get the associated CollectionName for a unique tokenId.
     */
    function getCollectionOfTokenId(uint256 _tokenId)
        external
        view
        returns (Collection memory) {
    ...
    function mint(
        ...
        Collection memory c1 = collectionList[_collectionId];
        bytes memory tmpName = bytes(c1.name); // Uses memory
        require(tmpName.length > 0,"ERROR:Invalid CollectionId");
        require(collectionCount[_collectionId] < maxSupply,"ERROR:exceed max supply");

        uint256 newId = _tokenIds.current();
        string memory tokenURI = c1.tokenURI;
        _tokenIds.increment();
        collectionIds[newId] = _collectionId;
        collectionCount[_collectionId] = collectionCount[_collectionId] + 1;
        _mint(_to, newId);
        _setTokenURI(newId, tokenURI);

        emit Mint( address(this),_to,_collectionId,newId, collectionCount[_collectionId]);
        return newId;
    }

       /**
     * @dev Mint NFTs. pay for it.
     */
    function mintFor (
        address _to,
        uint8 _collectionId,
        ...
        require(payPrice == price, "token sent does not match price");
        ...
        token.transferFrom(msg.sender, paymentReceiver, payPrice);
        ...
        emit MintFor( address(this),_to,_collectionId,newId, collectionCount[_collectionId], payPrice);
        return newId;
    }
     ...
     */
    function setCollection(uint8 _collectionId, 
                        string calldata _name,      
                        string calldata _tokenURI
                        )
        external {
        require(msg.sender == opsAddress, "Error: not ops address");
        
        Collection storage c1 = collectionList[_collectionId];
        c1.name = _name;
        c1.tokenURI = _tokenURI;
    }
    ...
    function setOpsAdress(address _opsAddress) external onlyOwner {
   ...
   /**
    ...
    function burn(uint256 _tokenId) external onlyOwner {
        ...
        collectionCount[collectionIdBurnt] = collectionCount[collectionIdBurnt] - 1;
        collectionBurnCount[collectionIdBurnt] = collectionBurnCount[collectionIdBurnt] + 1;
        _burn(_tokenId);
    }
