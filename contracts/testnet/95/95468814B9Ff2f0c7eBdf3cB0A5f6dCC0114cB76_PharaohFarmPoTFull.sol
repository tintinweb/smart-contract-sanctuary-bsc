...
contract PharaohFarmPoTFull is Ownable {
    ...
    event Transfer3(address indexed from, address indexed to, uint256 value);
    ...
    function claimRewards() public {
        ...
        uint256 maxTax = maximumInterest.mul(withdrawTax).div(100);

            uint256 toClaim = maximumInterest -
            maxTax -
                claimed;
            ...
            user_data[msg.sender].claimed += maximumInterest;
    
            user_data[msg.sender].previousDoubleUp = interest - maximumInterest;
            emit Transfer1(address(this), msg.sender, toClaim);
        } else if (interest + claimed > maximumInterest) {
        uint256 maxIfClaimed = (maximumInterest-claimed).mul(withdrawTax).div(100);

            uint256 toClaim = (maximumInterest - claimed) -maxIfClaimed;
            ...
            emit Transfer2(address(this), msg.sender, toClaim);

        } else {
            uint256 toClaim = (interest.sub(tax));
