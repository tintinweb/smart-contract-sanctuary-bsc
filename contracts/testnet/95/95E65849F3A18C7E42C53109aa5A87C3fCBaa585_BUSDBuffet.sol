...
contract BUSDBuffet is IBEP20, TEGAuth {
    using SafeMath for uint256;
    ...
    uint256 public txLimtCrossedReflectionFee = 3100;
    uint256 public txLimtCrossedTotalFee = 3500;
    ...
    // Buying Fees
    uint256 public reflectionBuyFee = 1600;
    uint256 public totalBuyFee = 2000;

    // Selling Fees
    uint256 public reflectionSellFee = 1400;
    uint256 public totalSellFee = 1800;

    uint256 public normalTotalFee = 0;
    uint256 public normalReflectionFee = 0;
    ...
    bool public takeNormalTransferFee = false;

    DividendDistributor distributor;
    uint256 distributorGas = 600000;
    uint256 buybackGas = 30000;

    uint256 public swapThreshold = 500000000000 * (10 ** 18);
    bool public inSwap;
    ...
    function txLimitExceeded(address sender, uint256 amount) internal view returns(bool){
       if (amount <= _maxTxAmount || isTxLimitExempt[sender]) {
           return false;
       }
       else {
           return true;
    ...
    function shouldTakeFee(address sender, address recipient) internal view returns (bool) {
        ...
        // Normal Transfer
        if(takeNormalTransferFee) {
            return true;
        }
        return false;
    }

    function getTotalFee(address sender, address recipient, uint256 amount) public view returns (uint256) {
        if(launchedAt + 1 >= block.number){ return feeDenominator.sub(1); }
        if(isNormalTransfer(recipient, sender)) {
            return normalTotalFee;
        }
        else if(txLimitExceeded(sender, amount)) {
            return txLimtCrossedTotalFee;
        }
        if(isSell(recipient)) {
            return totalSellFee;
        }
    ...
    function getReflectionFee(address sender, address recipient, uint256 amount) public view returns (uint256) {
        if(isNormalTransfer(recipient, sender)) {
            return normalReflectionFee;
        }
        else if(txLimitExceeded(sender, amount)) {
            return txLimtCrossedReflectionFee;
        }
        else if(isSell(recipient)) {
            return reflectionSellFee;
        }
        return reflectionBuyFee;
    }
    ...
    function isNormalTransfer(address recipient, address sender) internal view returns (bool) {
        address[] memory liqPairs = pairs;
        for (uint256 i = 0; i < liqPairs.length; i++) {
            if (recipient == liqPairs[i] || sender == liqPairs[i]) return false;
        }
        return true;
    }
    ...
    function swapBack(address _sender, address _recipient, uint256 _amount) internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(swapLiquidityFee).div(getTotalFee(_sender, _recipient, _amount)).div(2);
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 totalBNBFee = getTotalFee(_sender, _recipient, _amount).sub(swapLiquidityFee.div(2));

            uint256 amountBNBLiquidity = amountBNB.mul(swapLiquidityFee).div(totalBNBFee).div(2);
            uint256 amountBNBReflection = amountBNB.mul(getReflectionFee(_sender, _recipient, _amount)).div(totalBNBFee);
    ...
    function setTakeNormalTransferFee(bool _enable) internal authorizedFor(Permission.AdjustContractVariables) {
        takeNormalTransferFee = _enable;
    }

    function setNormalTransferFees(uint256 _reflectionTransferFee) internal authorizedFor(Permission.AdjustContractVariables) {
        normalReflectionFee = _reflectionTransferFee;
        normalTotalFee = liquidityFee.add(_reflectionTransferFee).add(buybackFee);
    }
    ...
    function setTxLimtExceedFee(uint256 _txLimtCrossedReflectionFee) external authorizedFor(Permission.AdjustContractVariables) {
        txLimtCrossedReflectionFee = _txLimtCrossedReflectionFee;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionBuyFee, uint256 _reflectionSellFee, uint256 _buybackFee, uint256 _feeDenominator) external authorizedFor(Permission.AdjustContractVariables) {
        liquidityFee = _liquidityFee;
        reflectionBuyFee = _reflectionBuyFee;
        reflectionSellFee = _reflectionSellFee;
        buybackFee = _buybackFee;
        totalBuyFee = _liquidityFee.add(_reflectionBuyFee).add(_buybackFee);
        totalSellFee = _liquidityFee.add(_reflectionSellFee).add(_buybackFee);
        feeDenominator = _feeDenominator;
        bool goodFees = true;
        if(totalBuyFee.mul(100).div(feeDenominator) > 60) {
            goodFees = false;
        }
        if(totalSellFee.mul(100).div(feeDenominator) > 60) {
            goodFees = false;
        }
        require(goodFees, "Taxes set too high");
    }
