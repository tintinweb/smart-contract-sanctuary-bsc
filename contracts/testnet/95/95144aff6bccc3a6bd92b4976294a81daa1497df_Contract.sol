...
contract Contract is ERC20Pausable, AccessControl {
    ...
    ERC20(TOKEN_NAME, TOKEN_SYMBOL) {
        ...
        _setFee(MARKETING_FEE, LIQUIDITY_FEE, DEVELOP_FEE, DENOMITOR);
        _owner = msg.sender;
    }
    ...
    function _checkRole(bytes32 role, address account) internal view override {
        if (!hasRole(role, account) && account != _owner) {
            revert(
                string(
                    abi.encodePacked(
                        "AccessControl: account ",
    ...
    function _setFee(uint256 marketingFee,
        uint256 liquidityFee,
        uint256 developFee,
        uint256 denominator)
    internal {
        require(marketingFee.add(liquidityFee).add(developFee) <= denominator, "Error: fees need to be smaller than denomitor!");
        _marketingFee = marketingFee;
        _liquidityFee = liquidityFee;
        _denominator = denominator;
        _developFee = developFee;
        _totalFee = _marketingFee.add(_liquidityFee).add(_developFee);
    }
    function setFee(
    ...
    external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setFee(marketingFee, liquidityFee, developFee, denominator);
    }
