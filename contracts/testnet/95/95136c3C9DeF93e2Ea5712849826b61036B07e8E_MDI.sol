...
/*
  ...
  Website: https://metadogeinu.io 

  Token: MDI

  Token Supply: 21,000,000

  Transaction Tax is 9% broken into:
    - 3% going to token holders,
    - 3% going to the liquidity pool,
    - 3% going to the burn address.
...
contract MDI is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 21000000 * 10**0 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _tBurnTotal;

    string private _name = "METADOGEINU";
    string private _symbol = "MDI";
    ...
    uint256 public _maxTxAmount = 500000 * 10**0 * 10**9;
    uint256 private minimumTokensBeforeSwap = 10000 * 10**0 * 10**9;
    ...
    function setBurnAddress(address payable burnWallet) external onlyOwner() {
        burnAddress = burnWallet;
    }
    ...
    function TransferBurn(address payable recipient, uint256 amount) private {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Of total here (6% -> 3%:LP, 3%:Burn)
        // 50% goes to LP
        // 50% goes to Burn 
        // Lets calc amount for burn wallet
        uint256 burnContractTokenBalancePortion = contractTokenBalance.div(2);

        //I do not need to swap for ETH/BNB - just move our tokens 
        //swapTokensForETH(burnContractTokenBalance); 
        uint256 beforeTransferBalance = address(this).balance;      

        //make the burn transfer
        TransferBurn(burnAddress, burnContractTokenBalancePortion); 
        
        // The burn total is tracked.
        uint256 afterTransferBalance = address(this).balance;
        _tBurnTotal = _tBurnTotal.add(beforeTransferBalance.sub(afterTransferBalance));

        // split the remaining balance into halves
        uint256 half = afterTransferBalance.div(2);
        uint256 otherHalf = afterTransferBalance.sub(half);
        ...
        uint256 newBalance = address(this).balance.sub(afterTransferBalance);
