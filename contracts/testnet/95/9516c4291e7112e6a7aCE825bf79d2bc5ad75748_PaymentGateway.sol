...
contract PaymentGateway is Ownable {
    ...
    bytes32 constant PAY_TYPEHASH = keccak256(
        "PaymentInfo(uint appId,uint paymentId,uint256 amount,address dev,address token)"
    );

    function hash(EIP712Domain memory eip712Domain) private view returns (bytes32) {
    ...
    function hash(PaymentInfo memory payment) private view returns (bytes32) {
    ...
    function pay(PaymentInfo memory paymentInfo, bytes32 sigR, bytes32 sigS, uint8 sigV) public {
        address recovered_signer = ecrecover(hash(paymentInfo), sigV, sigR, sigS);
        require(recovered_signer == msg.sender, "Signer must be sender");
        payToken(paymentInfo.appId, paymentInfo.paymentId, paymentInfo.amount, paymentInfo.dev, paymentInfo.token);
    }
    ...
    function payToken(uint appId, uint paymentId, uint256 amount, address dev, address token) public {
        ...
        if (devFee > 0) {
            _allowances[token][dev] += devFee;    
        }
        if (fee - devFee > 0) {
            _allowances[token][owner()] += (fee - devFee);    
        }
        erc20.transferFrom(_msgSender(), address(this), fee);
    ...
    function withdraw(address token, uint256 amount) public {
        ...
        uint256 allowance = _allowances[token][to];
        require(allowance >= amount, "Not enough token");
        _allowances[token][to] = allowance - amount;
        if (token != address(0)) {
    ...
    function myAllowance(address token) public view returns (uint256) {
        return _allowances[token][_msgSender()];
    }
}
