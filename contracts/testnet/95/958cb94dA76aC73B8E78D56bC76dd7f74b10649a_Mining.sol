...
contract  Mining is Ownable,IMining{
    ...
    uint constant public Oneday = 10 minutes;
    //address=>ID_NFTO
    mapping (address=>uint256) public Mining_take_NFTO;
    //MPN=>total_NFTO
    mapping (uint256=>uint256) public TotalNFTO_inMP;
    //address=>MPN
    mapping (address=>uint256) public getMPN;
    //MPN=>IDNFTA=>address;
    mapping (uint256 => mapping (uint256 => address)) public MPN_NFTO_True;

    uint constant public TimeMP = 30 minutes;
    
    //MiningPoolNum=>True
     mapping (uint256=>bool) public TrueMiningPoolNum;
    ...
    //apr NFTO
    function ADD_NFTO (uint256 ID_NFTO) public {
        address sender = msg.sender;
        uint256 MPN = MP.getMiningPoolNum();
        require(MP.getblockTimestamps(MPN)-block.timestamp<=Oneday);
        require(NFTO_A.ownerOf(ID_NFTO) ==sender);
        require(NFTO.get_LIFE_ID(ID_NFTO)>=1);
        require(Mining_take_NFTO[sender]==0);
        
        
        TotalNFTO_inMP[MPN]++;
        getMPN[sender]=MPN;
        Mining_take_NFTO[sender]=ID_NFTO;
        MPN_NFTO_True[MPN][ID_NFTO]=sender;

        NFTO_A.transferFrom(sender, Contr.getStock(), ID_NFTO);
        NFTO.set_LIFE_ID(ID_NFTO);
    }

    function delite_NFO() public {
        address sender = msg.sender;
        require(getMPN[sender]>0);
        uint256 MPN = getMPN[sender];
        require(MP.getblockTimestamps(MPN)-block.timestamp>=TimeMP);
        uint256 ID_NFTO = Mining_take_NFTO[sender];
        require(ID_NFTO>0);
        uint256 Total = MP.getMiners(MPN);
        uint256 Procent = getProcent(MPN,ID_NFTO);
        uint BalanceNFTO = Total/100*Procent;
         NFTO_A.transferFrom(Contr.getStock(),sender, ID_NFTO);
         MITI.transfer(sender, BalanceNFTO);

         Mining_take_NFTO[sender]=0;
         TotalNFTO_inMP[MPN]--;
         getMPN[sender]=0;
    }

    function getTrueMiningPoolNum(uint256 _MiningPoolNum) public virtual override view returns (bool){
        return TrueMiningPoolNum[_MiningPoolNum];
    }
    function getTimeMP () public virtual override view returns (uint) {
        return TimeMP;
    }
    function getOneday () public virtual override view returns (uint) {
        return Oneday;
    }
    function getProcent (uint256 _MPN,uint256 ID_NFTO) public virtual override view returns (uint256) {
        uint256 Procent = St.getTotalStakeNFTO(_MPN,ID_NFTO)/(St.getMPNtotalStaked(_MPN)/100);
        return Procent;   
}
    function getMPN_NFTO_True (uint256 _MPN,uint256 ID_NFTO) public virtual override view returns (address) {
        return MPN_NFTO_True[_MPN][ID_NFTO];
    }
    function getMining_take_NFTO(address _address) public virtual override view returns (uint256) {
        return Mining_take_NFTO[_address];
    }
    function  update()  public virtual override{
        MP=IMiningPool(address(Contr.getMiningPool()));
        NFTO=INFTO(address(Contr.getNFTO()));
        NFTO_A=ERC721A(address(Contr.getNFTO()));
        MITI=IERC20(address(Contr.getMiticoin()));
        St=IStaking(address(Contr.getStaking())); 
  } 
}
