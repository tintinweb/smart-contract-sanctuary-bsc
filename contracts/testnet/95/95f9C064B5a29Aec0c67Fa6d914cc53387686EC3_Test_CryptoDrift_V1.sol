...
contract Test_CryptoDrift_V1 is ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable,PausableUpgradeable, ERC721URIStorageUpgradeable,UUPSUpgradeable {
    ...
    function initialize() public initializer {
        ...
        maxNFTPurchase = 10000;
        maxNFTOwnership = 10000;
    ...
    function addAccountGenesis2Key(uint256 qty, address recepient) external onlyRole(MINTER_ROLE){
        require(genesis2SaleOpen, "Genesis1 sale is closed");
    ...
    function getAccountCars(address walletAddress) external view returns(uint256[] memory attributes){
        uint256 count = balanceOf(walletAddress);
        uint256[] memory result = new uint256[](count); 
        for(uint256 i = 0;i<count;i++){
            uint256 tokenId = tokenOfOwnerByIndex(walletAddress,i);
            uint256  attr = CAR_TYPES[tokenId];
            result[i] = attr;
        }
        return result;
    }
    ...
    function getCarTypes(uint256[] memory carIds) external view returns (uint256[] memory _types){
        ...
        for(uint256 i=0;i<carIds.length;i++){
            uint256 car = carIds[i];
            types[i] = CAR_TYPES[car];
        }
        return types;
    }
