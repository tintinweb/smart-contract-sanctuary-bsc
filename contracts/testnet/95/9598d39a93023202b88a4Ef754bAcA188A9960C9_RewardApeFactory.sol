...
contract ERC20RewardApeV1 is ReentrancyGuard, Ownable, Initializable {
    ...
    function setBonusEndTime(uint256 _bonusEndTime) external onlyOwner {
        require(_bonusEndTime > block.timestamp, 'new bonus end timestamp must be greater than current');
        bonusEndTime = _bonusEndTime;
    ...
    function pendingReward(address _user) external view returns (uint256 pending) {
        ...
        if (block.timestamp > poolInfo.lastRewardTime && totalStaked != 0) {
            ...
            uint256 tokenReward = multiplier * rewardPerSecond * poolInfo.allocPoint / totalAllocPoint;
            accRewardTokenPerShare = accRewardTokenPerShare + (tokenReward * 1e30 / totalStaked);
        }
        pending = user.amount * accRewardTokenPerShare / 1e30 - user.rewardDebt;
    }
