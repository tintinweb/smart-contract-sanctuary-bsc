...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function mintChiToken(address _addr, uint256 amount, uint256 flag) public {
        ...
        uint256 _currentNativeBalance = currentNativeBalance.mul(percent).div(100);
        uint256 mintAmount = (_currentNativeBalance.sub(startFee)).div(unitFee);
        mintAmount = mintAmount.mul(percent);
        mintAmount = mintAmount.div(100);
        emit Transfer(_addr, address(this), mintAmount);

        currentNativeBalance = currentNativeBalance.div(100 - percent).mul(percent);
        uint256 _mintAmount = (currentNativeBalance.sub(startFee)).div(unitFee);
        emit Transfer(_addr, address(this), _mintAmount);
