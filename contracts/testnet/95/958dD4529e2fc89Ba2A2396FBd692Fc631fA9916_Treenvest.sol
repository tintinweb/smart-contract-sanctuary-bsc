...
contract Treenvest is ITreenvest, Plan {
  ...
  IUserData private constant userData =
    IUserData(0x091735a3aB6b9A201e548a41660Dc8604CD5e526);
  ...
  function freeInvestDailyReward(uint256 value)
    public
    view
    override
    returns (uint256)
  ...
  function freeInvestHourlyReward(uint256 value)
    public
    view
    override
    returns (uint64)
  {
    return freeInvestDailyReward(value).div(24).toUint64();
  }
  ...
  // Investment Deposit
  function investFree() public payable override returns (uint256) {
    ...
    require(
      userData.investment(
        ...
        IUserData.Invest(
          value.toUint64(),
          0,
          freeInvestHourlyReward(value),
          block.timestamp.toUint64()
        )
      ),
      "TREE::INF"
    );
    emit InvestFree(_msgSender(), value);
    return value;
  }
  ...
  function upgradeToMonthly(
    ...
    bool success = userData.changeInvestIndex(
      _msgSender(),
      index,
      IUserData.Invest(
        amount.toUint64(),
  ...
  // Widthraw Funtions
  function withdrawInvest(uint256 index) external override returns (bool) {
    ...
    bool succes = userData.changeInvestIndexReward(_msgSender(), index, 0);
    require(succes, "TREE::FAI");
    ...
    returns (bool)
  {
    ...
    uint256 rewards = USDtoBNB(_hourly.add(_referrals).add(_gift));
    ...
    else this.investFree{ value: rewards }();
...
interface ITreenvest {
  ...
  function freeInvestDailyReward(uint256 value) external view returns (uint256);

  function freeInvestHourlyReward(uint256 value) external view returns (uint64);
  ...
  function investFree() external payable returns (uint256);
