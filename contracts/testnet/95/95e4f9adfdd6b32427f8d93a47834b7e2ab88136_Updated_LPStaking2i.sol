...
// will be transferred to a governance smart contract once DogeDash is sufficiently
// distributed and the community can show to govern itself.


contract Updated_LPStaking2i is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DogeDash
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardBlock;    // Last block number that CRANK distribution occurs.
        uint256 accTokenPerShare;   // Accumulated CRANK per share, times 1e12. See below.
    ...
    // The block number when CRANK mining starts.
    ...
    // View function to see pending CRANK on frontend.
    ...
    // View function to see if user can harvest DogeDash.
    ...
    // Deposit LP tokens to MasterChef for CRANK allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        require(token.balanceOf(address(msg.sender)) >= 100 * 10 ** 18 , 'CRANK token balance is too low');
    ...
    // Withdraw LP tokens only from MasterChef.
    ...
    // Withdraw LP tokens and rewards from MasterChef.
    ...
    // Pay pending CRANK.
    function payPendingHarvest(uint256 _pid) internal nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
     ...
     //Lockup pending CRANK. (For LP withdrawal without rewards)
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough DogeDash.
