...
contract ERC721Staking is ReentrancyGuard {
    ...
    // value of now.
    function stakeMore(uint256[] memory _tokenArray) external nonReentrant {
        require(_tokenArray.length > 0);
        for(uint32 i=0;i<_tokenArray.length;i++){
            uint256 _tokenId = _tokenArray[i];
    ...
    function withdrawMore(uint256[] memory _tokenArray) external nonReentrant {
        require(_tokenArray.length > 0);
        for(uint32 i=0;i<_tokenArray.length;i++) {
            uint256 _tokenId = _tokenArray[i];
            // Make sure the user has at least one token staked before withdrawing
