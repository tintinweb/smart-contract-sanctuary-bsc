...
contract GladiatorDexCrate is ERC721, Pausable, Ownable, ERC721Enumerable, ERC721Burnable, ReentrancyGuard {
    ...
    mapping(uint256 => PackageInfo) packageInfo;

    event PackageMinted(address to, uint256 token_id, uint256 upline_id, uint256 package_id, uint256 package_attribute, bool paid);
    ...
    constructor() ERC721("GladiatorDex Crate", "GDEXCRATE") {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://api.gladiatordex.com/nft/crates/";
    }
    ...
    function mintPackage(address to, uint256 upline_id, uint256 package_id, uint256 package_attribute, bool paid) external onlyOperators returns(uint256) {
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(to, tokenId);
        packageInfo[tokenId] = PackageInfo(tokenId, upline_id, package_id, package_attribute);
        emit PackageMinted(to, tokenId, upline_id, package_id, package_attribute, paid);
        return tokenId;
    }
    ...
    function getCratesOfAddress(address player)
        public
        view
