...
contract CropYard is ReentrancyGuard, ICropYard {
  ...
  function stake(uint256 _pId, uint256 amount, address account) public override nonReentrant returns (bool) {
    ...
    emit Enable(account, amount, capital_in_usd, _pId);

    return true;
  }

  function claim(uint256 _pId, address account) public override nonReentrant returns (bool) {
    User storage _user = _userInfo[_pId][account];

    require(_user.lockedTill < block.timestamp, Errors.NOT_CLIAMABLE_YET);
    ...
    emit Claim(account, claim_amount, _pId);

    return true;
  }

  function lock(uint256 _pId, uint256 duration, address account) public override nonReentrant returns (bool) {
    bool _is = IWhitelist(ADDRESSES_PROVIDER.getWhitelistContract()).isWhitelisted(account);
  ...
  function unStake(uint256 _pId, uint256 amount, address account) public override nonReentrant returns (bool) {
    bool _is = IWhitelist(ADDRESSES_PROVIDER.getWhitelistContract()).isWhitelisted(account);
  ...
  function _memberClaim(uint256 _pId, address account, uint256 claimble_capital, uint256 claimble_reward) private returns (uint256 total_claimable) {
    ...
    // send game fees there
    IWhitelist(ADDRESSES_PROVIDER.getWhitelistContract()).addGameAmount(account, game_fee);
    ...
    uint256 fee_token_amount = _getAmountTokenValue(acc_fees, _pId);
    // send acc fee tokens to vault
    IFisk(ADDRESSES_PROVIDER.getFiskContract()).addFarmFees(fee_token_amount);
  ...
  function _getAmountUsdValue(uint256 amount, uint256 _pId) public view returns (uint256 value) {
    (uint256 r0, uint256 r1, uint256 ts) = _getReserves(_pId);
    uint256 spent_price = IBarterRouter(ADDRESSES_PROVIDER.getBarterRouter()).getAmountOut(1 ether, r0, r1);

    uint256 spent_pool_value = r0.mul(spent_price);
  ...
  function create(address inputToken, address outPutToken, bool trigger, uint256 duration, uint256 apr) external override nonReentrant returns (bool) {
  ...
  function _getAmountTokenValue(uint256 amount, uint256 _pId) private view returns (uint256 token_value) {
    ...
    uint256 eusd_price = IBarterRouter(ADDRESSES_PROVIDER.getBarterRouter()).getAmountOut(1 ether, r1, r0);

    token_value = eusd_price.mul(amount);
...
interface IFisk {
  function addFarmFees(uint256 amount) external returns (bool);
}
