...
contract SEKUYA is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
...
//   uint256 public MAX_PER_Address = 40; // maximam amount that user can mint
  ...
  function mint(uint256 quantity) external payable callerIsUser {
    ...
    // require(quantity <= MAX_PER_Transtion,"can not mint this many");
    require(msg.value >= PRICE * quantity, "Need to send more ETH.");
    require(IERC20(0x2f47898684492e3018C6f4E557d5FBb84ED20c96).balanceOf(msg.sender) > 1000000*10**18, "Need more SKUYA" );
  ...
  function setReserve(uint256 r)external onlyOwner{
     ...
     function configuration(uint256 Status ,  uint256 Price, uint256 Stop )external onlyOwner{
      status = Status;
      stopat = Stop;
      PRICE = Price;
