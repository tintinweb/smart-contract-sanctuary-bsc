...
contract RunTogetherMarketplace is Ownable, ReentrancyGuard, Pausable {
    ...
    // Membership
    uint256[] public Membership;
    mapping(uint256 => uint256) public MembershipMap;
    // percent sale off Membership
    mapping(uint256 => uint256) public percentSaleOffMembershipMap;
    // RunTogetherBoxNft Type
    RunTogetherBoxNft runTogetherBoxType;
    // RunTogetherBoxNft Map
    mapping(uint256 => RunTogetherBox) public runTogetherBoxTypeMap;

    struct RunTogetherBox {
        bool adminSale;
        uint256 adminOrigin;
        address[] listBuyerAdminOrigin;
    }
    ...
    function setFeeAdmin(uint256 _newRate) external onlyOwner {
        feeAdmin = _newRate;
    }
    ...
    // Set Membership
    function setMembership(uint256[] calldata _Membership) public onlyOwner {
        Membership = _Membership;
        for (uint256 index = 0; index < _Membership.length; index++) {
            MembershipMap[_Membership[index]] = _Membership[index];
        }
    }

    // Set percent sale off for membership Nft
    function setPercentSaleOffMembership(uint256[] calldata _percentSaleOffMembershipMap) public onlyOwner {
        for (uint256 index = 0; index < _percentSaleOffMembershipMap.length; index++) {
            percentSaleOffMembershipMap[Membership[index]] = _percentSaleOffMembershipMap[index];
        }
    }

    function sellItemAdmin(
        address _nftAddress,
        ...
        for (uint256 index = 0; index < _nftIdList.length; ++index) {
            uint256 _nftId = _nftIdList[index];
            ...
            runTogetherBoxTypeMap[_nftId].adminSale = true;
            runTogetherBoxTypeMap[_nftId].adminOrigin = 1;
    ...
    function buyItem(uint256[] memory saleIndexList) external nonReentrant {
        for (uint256 _saleIndex = 0; _saleIndex < saleIndexList.length; ++_saleIndex) {
            uint256 saleIndex = saleIndexList[_saleIndex];
            ...
            // Sale off for box sold by admin
            if (runTogetherBoxTypeMap[nftId].adminSale == true) {
                for (uint256 index = 0; index < Membership.length; index++) {
                    if (saleIndexList.length >= Membership[index]) {
                        pricing = min(
                            pricing,
                            saleItem.priceListing - calPercentSaleOff(saleItem.priceListing, percentSaleOffMembershipMap[Membership[index]])
                        );
                    }
                }
            }

            if (runTogetherBoxTypeMap[nftId].adminOrigin > 0) {
                runTogetherBoxTypeMap[nftId].listBuyerAdminOrigin.push(msg.sender);
                runTogetherBoxTypeMap[nftId].adminOrigin += 1;
            }
            ...
            // Cal trading fee
            uint256 feeTrading = calTradingFee(pricing, runTogetherBoxTypeMap[nftId].adminSale);
            ...
            // Trading action
            if (runTogetherBoxTypeMap[nftId].adminSale == true) {
                currency.safeTransferFrom(msg.sender, poolReward, feeTrading);
                currency.safeTransferFrom(msg.sender, poolAdmin, sellerValueReceive);
            } else {
    ...
    function getListBuyerAdminOrigin(uint256 _idNft) public view returns (address[] memory) {
        return runTogetherBoxTypeMap[_idNft].listBuyerAdminOrigin;
    }
    ...
    function calTradingFee(uint256 _pricing, bool adminSale)
        public
        view
        returns (uint256 feeTrading)
    {   
        feeTrading = 0;
        if (adminSale == true) {
            feeTrading = _pricing * feeAdmin / baseRate;
        } else {
            feeTrading = _pricing * feeRate / baseRate;
        }
    }

    function calPercentSaleOff(uint256 _pricing, uint256 _percentSaleOff)
        public
        view
        returns (uint256 saleOff)
    {
