...
contract DtevilLock is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        lockups.push(Lockup(0, 30, 0, 50, 382927296831633642715083, 0, 0, 0)); // 1.25% liquidity
        lockups.push(Lockup(1, 60, 0, 30, 413561480578164334132290, 0, 0, 0)); // 1.35% liquidity
        lockups.push(Lockup(2, 90, 0, 10, 505464031817756408383910, 0, 0, 0)); // 1.65% liquidity
        lockups.push(Lockup(3, 120, 0, 0, 612683674930613828344133, 0, 0, 0)); // 2% liquidity
        _resetAllowances();
    }
