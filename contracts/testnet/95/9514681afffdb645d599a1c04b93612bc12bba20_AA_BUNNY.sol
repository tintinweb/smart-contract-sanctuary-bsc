...
contract AA_BUNNY is Context, IERC20 { 
    ...
    */
    ...
    uint256 public _P2P_Fee = _Fee_P2P_Marketing + _Fee_P2P_Liquidity;



    // Fee Tracking

    uint256 public Track_Buy;
    uint256 public Track_Sell;
    uint256 public Track_P2P;
    ...
    */
    ...
    function _set_Fees(uint256 Liquidity_BUY,
        ...
        _P2P_Fee = _Fee_P2P_Marketing + _Fee_P2P_Liquidity;
    ...
    */
    ...
    function _transfer(
        ...
        if (_isPair[from]){
            ...
            // BUY FEE TRACK
            Track_Buy = Track_Buy + amount;

        } else if (_isPair[to]){

            // SELL FEE TRACK
            Track_Sell = Track_Sell + amount;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Does not incldue the P2P fees calculations!!!


        // Clean track numbers to percent splits
        uint256 Split_Buy   = Track_Buy * 100 / (Track_Buy + Track_Sell); 
        uint256 Split_Sell  = 100 - Split_Buy;

        // Split CTB tokens
        uint256 CTB_BUY     = contractTokenBalance * Split_Buy / 100;
        uint256 CTB_SELL    = contractTokenBalance * Split_Sell / 100;

        // Calculate LP Tokens
        uint256 Tokens_L = ((CTB_BUY * _FeeBuyLiquidity / _Fee_on_Buys) + (CTB_SELL * _FeeSellLiquidity / _Fee_on_Sells))/2;

        // Calculate Marketing Tokens
        uint256 Tokens_M = (CTB_BUY * _FeeBuyMarketing / _Fee_on_Buys) + (CTB_SELL * _FeeSellMarketing / _Fee_on_Sells);

        // Calculate Development and Charity Tokens
        uint256 Tokens_D = (CTB_BUY * _FeeBuyDevelopment / _Fee_on_Buys) + (CTB_SELL * _FeeSellDevelopment / _Fee_on_Sells);

        // Calculate Solidity Dev Tokens
        uint256 Tokens_S = (CTB_BUY * _FeeBuyDeveloper / _Fee_on_Buys) + (CTB_SELL * _FeeSellDeveloper / _Fee_on_Sells);


        uint256 TrackBNB = Tokens_S + Tokens_D + Tokens_M + Tokens_L;


        // Swap tokens for BNB
        uint256 contract_BNB = address(this).balance;
        swapTokensForEth(contractTokenBalance - Tokens_L);
        uint256 returned_BNB = address(this).balance - contract_BNB;


        uint256 BNB_L = returned_BNB * Tokens_L / TrackBNB;
        uint256 BNB_D = returned_BNB * Tokens_D / TrackBNB;
        uint256 BNB_S = returned_BNB * Tokens_S / TrackBNB;

        // Add liquidity 
        if (Tokens_L != 0){
            addLiquidity(Tokens_L, BNB_L);
            emit SwapAndLiquify(Tokens_L, BNB_L, Tokens_L);
        }

        // Take development and charity fee
        if(BNB_D > 0){sendToWallet(Wallet_Development, BNB_D);}

        // Take solidity developer fee
        if(BNB_S > 0){sendToWallet(Wallet_SolidityDev, BNB_S);}
