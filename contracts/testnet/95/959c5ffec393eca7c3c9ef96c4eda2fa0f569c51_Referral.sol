...
contract Referral is Ownable, Pausable, ReentrancyGuard, Signable {
    ...
    function withdraw(address _to, uint256 _amount)
        ...
        nonReentrant
    {
        require(_amount > 0, "TokenReward: amount is invalid");
    ...
    function claimToken(uint256 _amount, uint256 _nonce, bytes memory _signature)
        public
        whenNotPaused
        nonReentrant
    {
        ...
        require(!nonces[_nonce], "TokenReward: nonce was used");

            address msgSender = _msgSender();

            require(_verifySignature(abi.encodePacked(msgSender, _amount, _nonce, block.chainid, this), _signature), "TokenReward: signature is invalid");

            nonces[_nonce] = true;

        token.safeTransfer(msgSender, _amount);

        emit TokenClaimed(msgSender, _amount, _nonce);
    }
