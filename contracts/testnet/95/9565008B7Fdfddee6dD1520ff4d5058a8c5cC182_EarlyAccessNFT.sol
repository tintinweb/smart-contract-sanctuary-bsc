...
contract EarlyAccessNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {

    using Address for address;

    // Token name
    string private constant _name = "EarlyAccessNFT";

    // Token symbol
    string private constant _symbol = "EANFT";
    ...
    address public feeReceiverSetter;

    // cost for minting NFT
    uint256 public cost = 5 * 10**17;

    // cost increase per mint
    uint256 public costIncrease = 4 * 10**16;
    uint256 private constant INCREASE_DENOM = 10**18;
    ...
    function setFeeReceiver(address newReceiver) external {
        require(msg.sender == feeReceiverSetter, 'Only Fee Receiver Setter');
        feeReceiver = newReceiver;
    }

    function setFeeReceiverSetter(address newReceiverSetter) external {
        require(msg.sender == feeReceiverSetter, 'Only Fee Receiver Setter');
        feeReceiverSetter = newReceiverSetter;
    }

    function setCostIncrease(uint newIncrease) external {
        costIncrease = newIncrease;
    }
    ...
    function mint() external payable {
        ...
        // increase cost
        cost += ( cost * costIncrease ) / INCREASE_DENOM;

        // mint tokens to sender
        _safeMint(msg.sender, currentSupplyIndex);
        
        // send payment to fee receiver
    ...
    function getIDsByOwner(address owner) external view returns (uint256[] memory) {
        ...
        uint256 count = 0;
        for (uint i = 0; i < currentSupplyIndex; i++) {
            if (_owners[i] == owner) {
                ids[count] = i;
                count++;
            }
    ...
    */
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        // increment balances and supplies
        unchecked {
            _balances[to] += 1;
            _totalSupply++;
            currentSupplyIndex++;
