...
// File: contracts/MetaBombBox.sol
...
contract MetaBombBox is Initializable, ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _addPoint) public initializer {
        ...
        __Ownable_init_unchained();
        __Pausable_init();
        __ERC721_init("Metabomb Box", "METABOMB_BOX");
...
// File: contracts/MetabomBoxWhiteList.sol
...
contract MetaBombBoxWhiteList is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    MetaBombBox public metaBombBox;

    uint256 public priceBNBPerBox;
    uint256 public maxBoxBuyPerUser;
    address public addressBNBReceiver;

    bool public enableWhiteList;
    uint256[] public typeBoxs;

    mapping(address => uint256) public whiteList; //(user => amount can buy)
    mapping(address => uint256) public addressBoughtBox; //(user => number box bought)

    event NonFungibleTokenRecovery(address indexed token, uint256 tokenId);
    event TokenRecovery(address indexed token, uint256 amount);
    event BuyBox(address buyer, uint256 amount);

    constructor() {
        enableWhiteList = true;
        priceBNBPerBox = 11 * 10**16;
        maxBoxBuyPerUser = 999;
    ...
    function setPriceBNBPerBox(uint256 _price) external onlyOwner {
        priceBNBPerBox = _price;
    }

    function setMaxBoxBuyPerUser(uint256 _maxBuy) external onlyOwner {
        maxBoxBuyPerUser = _maxBuy;
    }

    function setWhiteList(address[] memory _addressWhiteList, uint256[] memory _amounts) external onlyOwner {
        require(_addressWhiteList.length == _amounts.length, "input invalid");
        uint256 total = _addressWhiteList.length;
        for (uint256 i = 0; i < total; i++) {
            address add = _addressWhiteList[i];
            uint256 amount = _amounts[i];
            whiteList[add] = amount;
        }
    }
    ...
    function setBNBReceiver(address _add) external onlyOwner {
        addressBNBReceiver = _add;
    }
    ...
    function buyNFTBox(uint256 _amount) external payable whenNotPaused nonReentrant {
        require(priceBNBPerBox > 0, "price invalid");

        uint256 maxBuy = maxBoxBuyPerUser;

        if (enableWhiteList) {
            require(whiteList[msg.sender] > 0, "address invalid");
            maxBuy = whiteList[msg.sender];
        }
        require(addressBoughtBox[msg.sender] + _amount <= maxBuy, "you run out of purchases");

        require(msg.value >= priceBNBPerBox * _amount, "Token payment allowance is insufficient");

        for (uint256 i = 0; i < _amount; i++) {
            _mintBoxToAddress(msg.sender);
        }

        addressBoughtBox[msg.sender] = addressBoughtBox[msg.sender] + _amount;
        (bool bnbReceived, ) = addressBNBReceiver.call{value: msg.value}("");
        require(bnbReceived);
        emit BuyBox(msg.sender, _amount);
    }

    function setBoxAddress(address add) external onlyOwner {
        metaBombBox = MetaBombBox(add);
    }

    function changeBoxOwner(address newOwner) external onlyOwner {
        metaBombBox.transferOwnership(newOwner);
    }

    function    _mintBoxToAddress(address to) private {
        metaBombBox._mintBatchToAddress(to, 2, typeBoxs);
    }
}