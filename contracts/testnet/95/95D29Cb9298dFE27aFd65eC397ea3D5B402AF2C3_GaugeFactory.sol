...
contract GaugeFactory is ProtocolGovernance, ReentrancyGuard {
    ...
    function addGauge(address _tokenLP) external returns (address) {
        ...
        require(
            (baseTokens[_token0] && verifiedTokens[_token1]) ||
                (baseTokens[_token1] && verifiedTokens[_token0]),
            "!verified"
        );
        require(
            msg.sender == governance || msg.sender == admin,
    ...
    // address _referralsContract, uint256 _referralFee, uint256[] memory _refLevelPercent
