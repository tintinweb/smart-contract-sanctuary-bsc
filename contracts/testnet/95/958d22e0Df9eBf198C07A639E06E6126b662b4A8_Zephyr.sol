...
/**
 * @title Zephyr contract
 */
contract Zephyr is ERC721Enumerable, ERC721URIStorage, Ownable {
    string public PROVENANCE_HASH = "";
    uint256 public MAX_ZEPHYRS;
    ...
    modifier whenMetadataNotFrozen {
        require(!METADATA_FROZEN, "Zephyr: Metadata already frozen.");
        _;
    }

    modifier whenProvenanceNotFrozen {
        require(!PROVENANCE_FROZEN, "Zephyr: Provenance already frozen.");
        _;
    }

    constructor() ERC721("Zephyr", "ZEPHYR") {
        baseUri = 'ipfs://QmZ92ca7LGRgan52Gn94nLiNq7XMbSHu5Lcx3nVjRGj87j/';
        saleIsActive = false;
        MAX_ZEPHYRS = 500;
        maxPerWallet = 0;
        // 0.1 ETH
    ...
    function remainingSupply() external view returns (uint256) {
        return MAX_ZEPHYRS - totalSupply();
    }
    ...
    function mint(address account, uint256 amount) external payable {
        require(amount <= maxPerMint, "Zephyr: Amount exceeds max per mint");
        require(totalSupply() + amount <= MAX_ZEPHYRS, "Zephyr: Purchase would exceed cap");
        require(maxPerWallet <= 0 || balanceOf(account) + amount <= maxPerWallet, "Zephyr: Maximum mints per wallet is exceeded");
        require((saleIsActive ? mintPrice : presaleMintPrice) * amount <= msg.value, "Zephyr: Ether value sent is not correct");

        for (uint256 i = 0; i < amount; i++) {
            uint256 mintIndex = totalSupply() + 1;
            if (totalSupply() < MAX_ZEPHYRS) {
    ...
    function mintForCommunity(address _to, uint256 _numberOfTokens) external onlyOwner {
        require(_to != address(0), "Zephyr: Cannot mint to zero address.");
        require(totalSupply() + _numberOfTokens <= MAX_ZEPHYRS, "Zephyr: Minting would exceed cap");
