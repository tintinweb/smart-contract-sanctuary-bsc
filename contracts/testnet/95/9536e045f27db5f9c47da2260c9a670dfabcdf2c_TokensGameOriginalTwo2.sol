...
contract TokensGameOriginalTwo2 {
    ...
    uint256[] public REFERRAL_PERCENTS = [5, 4, 3, 2, 3, 4, 5]; //deposit
    uint256[] public REFERRAL_PERCENTS_SELL = [1, 0, 2, 0, 3, 0, 4]; //withdraw
...
struct User {
    ...
    mapping (address => bool) private OneGetFree; // Получил ли игрок бонус
    mapping (address => bool) private BUY_TOKEN_SUM;
    mapping (address => bool) private buy;
    ...
    function hatchTokens() public {
        ...
        uint256 newTokens = SafeMath.div((tokensUsed + bonus), TOKENS_TO_HATCH_1TOKENS);
        hatcheryMiners[msg.sender] = SafeMath.add(hatcheryMiners[msg.sender], newTokens);
    ...
    function buyTokens(address ref, uint256 amount) public payable {
        ...
        ERC20(busdt).transfer(TimerAddress, fee);
        ERC20(busdt).transfer(Delevoper, fee2);
    ...
    function getDop() public view returns(uint256){
        return Dop;
    }
