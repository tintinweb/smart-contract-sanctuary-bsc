...
// File: contracts/StrtVesting.sol
...
contract StrtVesting is Ownable, IStrtVesting {
    ...
    uint256 public creoDecimals;

    uint256 public rate;
    address public locking; 
    uint256 public strtDecimals;

    mapping(uint256 => uint256) public linear_dateTime;
    mapping(address => UserReward) public reward;

    constructor(uint256 _startLinear, uint256 _tge, address _locking, uint256 _rate, uint256 _strtDecimals, address _creoToken, uint256 _creoDecimals) {
        tge = _tge;
        locking = _locking;
        rate = _rate;
        creoToken = _creoToken;
        creoDecimals = _creoDecimals;
        strtDecimals = _strtDecimals;
     ...
     * @dev Check rewards
     */
    function check(address _address) public view returns(uint256) {
        if(block.timestamp < tge){
            return 0;
        }

        uint256 rewardTotal = _calculateTotalReward(msg.sender);
        if(rewardTotal == 0) {
            return 0;
        }
        ...
        if(!reward[msg.sender].tgeStatus){
            amount = (rewardTotal * 3) / 100;
        }

        uint256 round = linearRound();
        if(round > 0){
            if (round == 12) {
                amount = rewardTotal - userTotalClaimed[_address];
            } else {
                amount += (((rewardTotal * 97) / 100) / 12) * (round - reward[_address].lastRound);
            }
        }
    ...
    function calculateTotalReward() public view returns(uint256) {
        return _calculateTotalReward(msg.sender);
    }

    function _calculateTotalReward(address _address) internal view returns(uint256) {
        uint256 rewardTotal = reward[_address].rewardTotal;
        if(rewardTotal == 0){
            rewardTotal = ISTRTXCREO(locking).userTotalLocks(_address) / rate;
            if (strtDecimals < creoDecimals) {
                rewardTotal = rewardTotal * 10**(creoDecimals - strtDecimals);
            } else if (strtDecimals > creoDecimals) {
                rewardTotal = rewardTotal / 10**(strtDecimals - creoDecimals);
            }
        }
        return rewardTotal;
    }
    ...
    function claim() external{
        ...
        if(reward[msg.sender].rewardTotal == 0){
            reward[msg.sender].rewardTotal = rewardTotal;
        }

        uint256 amount = check(msg.sender);

        require(reward[msg.sender].rewardTotal > 0, "No rewards");
        require(userTotalClaimed[msg.sender] < reward[msg.sender].rewardTotal, "All claimed");
