    ...
    OwnableUpgradeable
{
    ...
    //Mapping to maximum amount of tokens that can be minted
    mapping(address => uint256) public maxTokensMinting;
    //Mapping to amount of tokens that have been minted
    ...
    {
        bytes32 currency = _addressToBytes32(token);
        require(!currenciesApproved[currency], "Currency approved");
    ...
    function updateMaxTokenMinting(address token, uint256 newMaxTokenMinting)
        external
        override
        onlyOwner
    {
        ...
        require(currenciesApproved[currency], "Currency not approved");
        require(
            newMaxTokenMinting > totalTokensMinted[token],
            "The new max token minting should be greater than total token minted"
        );

        maxTokensMinting[token] = newMaxTokenMinting;
        emit MaxTokenMintingChanged(_msgSender(), token, newMaxTokenMinting);
    }
        ...
        for (uint256 i; i < length1; ++i) {
            ...
            for (uint256 j; j < length2; ++j) {
                ...
                unchecked {
                    for (uint256 k; k < length3; ++k) {
                        ...
                        address recoverAddress = ECDSAUpgradeable.recover(digest, signature);
                        if(validatorsApproved[recoverAddress]) {
            ...
            if (total > 0) {
                ...
                if (contractAddress != address(0)) {
                    require(
                        totalTokensMinted[contractAddress] + total <=
                            maxTokensMinting[contractAddress],
                        "Minting amount exceeds maximum"
                    );
                    totalTokensMinted[contractAddress] += total;
            ...
            emit Distributed(_msgSender(), currency, reward.amounts, reward.uuids);
        }
    }
}
...
interface IGameRewardsDistributor {
     ...
     */
    event CurrencyRefused(address account, bytes32 currency);

    /**
     * @dev Emitted when `threshold` is changed from `account`
     */
    event ThresholdChanged(address account, uint256 threshold);
    ...
    /**
     * @dev Emitted when max token minting is changed from `account`
     */

    event MaxTokenMintingChanged(address account, address token, uint256 newMaxTokenMinting);
   ...
   /**
    ...
    /**
     * @dev Updates max token minting.
     *
     * Requirements:
     * - the caller is the owner.
     * - the `newMaxTokenMinting` should be greater than total token minted
     * Emits an {MaxTokenMintingChanged} event.
     */
    function updateMaxTokenMinting(
        address token,
        uint256 newMaxTokenMinting
    ) external;
     ...
     */
    function claimGameRewards(GameReward[] calldata gameRewards) external;
}
