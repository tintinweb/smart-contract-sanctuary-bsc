    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            // Sell
                ...
                if (_holderLastTransferTimestampAntiwhale[from] <= _holderLastTransferTimestampAntiwhale[from] + 1 days) {
                    if (amount >= _tTotal.mul(_antiWhaleFeePercent).div(BASE_FEE_PERCENT)) {
                        _antiWhaleFeeAmount = amount.mul(_antiWhaleFeePercent).div(BASE_FEE_PERCENT);
                    }
                }
                _holderLastTransferTimestampAntiwhale[from] = block.timestamp;
                _distributionHoldersFee = _sellDistributionHoldersFee;
                _liquidityFee = _sellLiquidityFee + _sellBuybackAndBurnFee + _antiWhaleFeeAmount;
                if (_liquidityFee > 0) {
