...
contract HoneybeePledge is Initializable, OwnableUpgradeable, DivestorUpgradeable {
    ...
    function _tranferTeam() public {
        ...
        if (tm > 1 days) {
            for (uint i = 0; i < tm / 1 days; i++) {
                addrInfo.HBEE.safeTransfer(addrInfo.market, dailyInfo.output * 2 / 100);
                addrInfo.HBEE.safeTransfer(addrInfo.technology, dailyInfo.output * 3 / 100);
            }
    ...
    function _out(address account_, uint stakeBurn_, uint commBurn_) private {
        ...
        if (nodeSlotId != 0) {
            ...
            nodeInfo[nodeSlotId].owner = address(0);
            // nodeInfo[nodeSlotId].lastTime = block.timestamp;
        ...
        delete userInfo[account_];
        userInfo[account_].inviter = userInfo[account_].inviter;
        userInfo[account_].directInviter = userInfo[account_].directInviter;
        userInfo[account_].commPower = userInfo[account_].commPower;
        userInfo[account_].lastStakeTime = userInfo[account_].lastStakeTime;
        // userInfo[account_].inviter = userInfo[account_].inviter;
        userInfo[account_].totalClaim = totalClaim;
        userInfo[account_].totalClaimValue = totalClaimValue;
    ...
    function _updateCommPool(address account_, uint poolId_, bool inPool_) private {
        ...
        if (commPool[poolId_].endTime != 0 && commPool[poolId_].totalValue == 1) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, (block.timestamp - commPool[poolId_].endTime) * dailyInfo.commRate);
        }
    } 
    ...
    function __changeBurnRate(uint rate_) public onlyOwner{
        dailyInfo.burnRate = rate_;
    }
    
    function _start() private onlyOwner {
    ...
    function isValidInviter(address account_) public view returns (bool) {
        if (account_ == address(0) || account_ == address(this)) {
            return true;
        }
    ...
    function _viewNodeSlotReward(uint slowId) public view returns(uint) {
        if (nodeInfo[slowId].lastTime == 0) {
            return 0;
        }
        uint reward = (block.timestamp - nodeInfo[slowId].lastTime) * dailyInfo.nodeRate;
    ...
    function stake(uint slot_, uint value_, bool isHBEE_, address inviter_) public {
        ...
        if (userInfo[_msgSender()].inviter == address(0)) {
            ...
            if (inviter_ == address(0)) {
                userInfo[_msgSender()].inviter = address(this);
            } else {
    ...
    function claimNodeReward() public returns (uint) {
        ...
        nodeInfo[slotId].lastTime = block.timestamp;
    ...
    function claimStakeReward() public returns (uint) {
        ...
        uint gas = reward * 5 / 100;
        uint inviterReward = reward / 2;  // 一半收益数值往上返
        _allocateInviterReward(_msgSender(), inviterReward);
        _claimReward(_msgSender(), reward, reward - gas, gas);

        // burn over reward
        if (realReward > reward) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, realReward - reward);
        }
