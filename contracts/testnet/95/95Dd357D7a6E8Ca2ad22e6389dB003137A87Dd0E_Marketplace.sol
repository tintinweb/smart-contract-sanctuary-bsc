...
contract Marketplace is ReentrancyGuard {
    ...
    function listNFT(
       // address nftContract,
        ...
        require(isRentableNFT(_nftAddress), "Contract is not an ERC4907");
        require(IERC721(_nftAddress).ownerOf(tokenId) == msg.sender, "Not owner of nft");
        require(IERC20(_tokenAddress).balanceOf(msg.sender) >= _listingFee, "Not enough ether for listing fee");
        ...
        require(_listingMap[_nftAddress][tokenId].nftContract == address(0), "This NFT has already been listed");
        IERC20(_tokenAddress).transferFrom(msg.sender,_marketOwner,_listingFee);
       // payable(_marketOwner).transfer(_listingFee);
        _listingMap[_nftAddress][tokenId] = Listing(
            msg.sender,
            address(0),
            _nftAddress,
            tokenId,
        ...
        EnumerableSet.add(_nftContractTokensMap[_nftAddress], tokenId);
        EnumerableSet.add(_nftContracts, _nftAddress);
        emit NFTListed(
    ...
    function rentNFT(
        ...
        Listing storage listing = _listingMap[_nftAddress][tokenId];
        ...
        // Update listing
        IERC20(_tokenAddress).transferFrom(msg.sender,_marketOwner,_listingFee);
        IERC4907(_nftAddress).setUser(tokenId, msg.sender, expires);
    ...
    function unlistNFT( uint256 tokenId) public payable nonReentrant {
        Listing storage listing = _listingMap[_nftAddress][tokenId];
        ...
        // clean up data
        IERC4907(_nftAddress).setUser(tokenId, address(0), 0);
        EnumerableSet.remove(_nftContractTokensMap[_nftAddress], tokenId);
        delete _listingMap[_nftAddress][tokenId];
        if (EnumerableSet.length(_nftContractTokensMap[_nftAddress]) == 0) {
            EnumerableSet.remove(_nftContracts, _nftAddress);
        }
