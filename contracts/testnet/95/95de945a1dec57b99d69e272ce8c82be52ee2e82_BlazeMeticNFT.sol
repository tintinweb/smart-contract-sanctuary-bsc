...
contract BlazeMeticNFT is ERC1155, Ownable {
  ...
  function purchase(IERC20 token, address refer, address to, uint256 amount, uint256 tokenId, uint256 nftqty) public {

    uint256 erc20balance = token.balanceOf(msg.sender);

    require(amount <= erc20balance, "balance is low");

    token.transfer(to, amount*95/100);

    token.transfer(refer, amount*5/100);

    _mint(msg.sender, tokenId, nftqty, "");

    emit TransferSent(msg.sender, to, amount*95/100);

    emit TransferSent(msg.sender, to, amount*5/100);
  } 
