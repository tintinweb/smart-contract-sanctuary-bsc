...
contract B8DexTokenSale is IB8DexTokenSale, Ownable {
    ...
    IERC20 public b8dToken;            // The Token token itself
    ...
    mapping(Status => uint256) public _prices;
    ...
    event LogBuy           (Status status, uint256 time, address user, uint256 amount, uint256 amount1, uint256 amount2, uint256 amount3, uint256 amountBUSD);
    ...
    onlyOwner
    {
        b8dToken.transferFrom(address(msg.sender), address(this), totalSupply);
    }
...
//        b8dToken.burn(amount);
    ...
    function _buy(
        ...
        uint256 b8dAmount = uint(busdCount).div(uint(_prices[status])) * 10**uint(decimals);
        uint256 b8dAmount1 = uint(busdCount).div(1000000000000000000);
        uint256 b8dAmount2 = uint(busdCount) / 1000000000000000000;
        uint256 b8dAmount3 = uint(_prices[status]).div(1000000000000000000);
        ...
        totalBoughtTokens = totalBoughtTokensWithAmount;

        emit LogBuy(status, time, sender, b8dAmount, b8dAmount1, b8dAmount2, b8dAmount3, busdCount);
    }
