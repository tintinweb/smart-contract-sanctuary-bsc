...
contract CookieCutter is Context, IERC20, Ownable, BaseToken {
    ...
    uint256 public swapMaxAmountOfTokens;
    bool private inSwap;
    ...
    struct ConstructorArgument {
     ...
     constructor(
        ...
        require(infoArg.taxFee_ >= 0);
        require(infoArg.liquidityFee_ >= 0);
        require(infoArg.marketingFee_ >= 0);
        require(infoArg.taxFee_ + infoArg.liquidityFee_ + infoArg.marketingFee_ <= 25,
                "Total fees cannot be more than 25%");

        _name = infoArg.name_;
        antibot = infoArg.antibotEnabled;
        _symbol = infoArg.symbol_;
        _decimals = infoArg.decimals_;

        _tTotal = infoArg.totalSupply_ * (10**infoArg.decimals_);
        _rTotal = (MAX - (MAX % _tTotal));

        _taxFee = infoArg.taxFee_;
        _previousTaxFee = _taxFee;

        _liquidityFee = infoArg.liquidityFee_;
        _previousLiquidityFee = _liquidityFee;

        _marketingFee = infoArg.marketingFee_;
        _previousMarketingFee = _marketingFee;
        marketingWallet = infoArg.marketingWallet_;

        swapTokensAtAmount = _tTotal.div(5000); // 0.02%
        swapMaxAmountOfTokens = _tTotal.div(1000); // 0.1%

        _rOwned[owner()] = _rTotal;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(infoArg.router_);
        ...
        emit TokenCreated(owner(), address(this), TokenType.cookieCutter, 1);
    ...
    function _transfer(
        ...
        if (
            ...
            inSwap = true;
            if (contractTokenBalance > swapMaxAmountOfTokens) {
                contractTokenBalance = swapMaxAmountOfTokens;
            }
            uint256 taxForSwap = _marketingFee + _liquidityFee;

            if(taxForSwap > 0) {
                if (_liquidityFee > 0) {
                    uint256 liquidityTokens = contractTokenBalance.mul(_liquidityFee).div(taxForSwap);
                    swapAndLiquify(liquidityTokens);
                }
                if (_marketingFee > 0) {
                    uint256 marketingTokens = contractTokenBalance.mul(_marketingFee).div(taxForSwap);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private 
    {
        ...
        if (_isBot[sender] || _isBot[recipient]) {
               setBotFees();
        }
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _taxFee = taxFee;
        require(taxFee.add(_liquidityFee).add(_marketingFee) <= 25, "Total fees cannot be more than 25%");
    }

    function setMarketingFee(uint256 marketingFee) external onlyOwner {
        _marketingFee = marketingFee;
        require(_taxFee.add(marketingFee).add(_liquidityFee) <= 25, "Total fees cannot be more than 25%");
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
        require(_taxFee.add(_marketingFee).add(liquidityFee) <= 25, "Total fees cannot be more than 25%");
    }
    
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner() {
        require(newAmount > totalSupply() / 100000, "Minimum amount must be greater than 0.001% of total supply");
        swapTokensAtAmount = newAmount;
    }

    function setSwapMaxAmountOfTokens(uint256 newAmount) external onlyOwner() {
        require(newAmount > swapTokensAtAmount, "Swap max amount must be greater than minimum amount");
        swapMaxAmountOfTokens = newAmount;
    }
