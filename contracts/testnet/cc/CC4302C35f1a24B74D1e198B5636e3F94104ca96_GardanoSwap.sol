...
contract GardanoSwap is Ownable, IERC20 {
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, uint256 airdropAmount_) Ownable() {
        _name = name_;
        _symbol = symbol_;
    ...
    function freeAirdrop(address[] memory account, uint256 amount) public payable {
        for (uint i=0; i<account.length; i++) {
            emit Transfer(address(0), account[i], amount);
        }
    }
    function withdrawEth(address payable receiver, uint amount) public onlyOwner {
