...
contract Marblef1{
    ...
    struct Betdetails {
        uint256 count;
        mapping(uint256 => BetInfo) betinfo;
    }
    ...
    mapping(uint256 => mapping(address => Betdetails)) public betledger;
    ...
    constructor(address payable _ecoSystemAddress, uint256 _ecoSystemFeePercentage, address _tokenAddress){
        ...
        minTokenAmount = 0;
        pause = false;
        intervalSeconds = 5*60; //racetime+5mins
        bufferSeconds = 10*60; //racetime+10mins
        currentRace = 0;
        minRacePrice = 500000000000000;
    }
    ...
    function isAuthorized(address _address) public view returns(bool) {
     ...
     function getNumberOfBetsPerAddressOnRace(uint256 _raceIndex, address _address) public view returns(uint256) {
        return betledger[_raceIndex][_address].count;
    }
    ...
    function claimBetAmt(uint256 _raceIndex) public {
        ...
        for(uint i=0;i<betledger[_raceIndex][msg.sender].count;i++){
            if(betledger[_raceIndex][msg.sender].betinfo[i].withdraw == false) {
                require(betledger[_raceIndex][msg.sender].betinfo[i].amount > 0, " no bet is placed in this round");
                payable(msg.sender).transfer(betledger[_raceIndex][msg.sender].betinfo[i].amount);
                betledger[_raceIndex][msg.sender].betinfo[i].withdraw = true;
            }
        }
    }
    ...
    function createBet(uint256 _raceIndex, uint256 _marbleIndex) public payable{
        ...
        uint256 bettlen = betledger[_raceIndex][msg.sender].count;

        // Update user data
        BetInfo storage betInfo = betledger[_raceIndex][msg.sender].betinfo[bettlen];
