...
contract CreateMiranaCollection is ICollection, Ownable {
  ...
  event CollectionUpdated(uint256 _id, string tokenType, address owner);
  ...
  function updateValueCollection(Collection memory collection)
    internal
    virtual
  {
    for (uint256 index = 0; index < collections.length; index++) {
      if (collections[index]._id == collection._id) {
  ...
  function _checkTokenType(string memory tokenType)
    internal
    virtual
    returns (bool)
  {
  ...
  function createCollection(
    ...
    MockERC721 erc721 = new MockERC721(owner, tokenName, tokenSymbol, "");

    // if (
    //   keccak256(abi.encodePacked((tokenSymbol))) ==
    //   keccak256(abi.encodePacked(("Single")))
    // ) {
    //   erc721 = new MockERC721(owner, tokenName, tokenSymbol, "");
    // }
    // if (
    //   keccak256(abi.encodePacked((tokenSymbol))) ==
    //   keccak256(abi.encodePacked(("Multiple")))
    // ) {
    //   MockERC1155 erc1155 = new MockERC1155(owner, tokenName, tokenSymbol, "");
    // }
    ...
    emit CollectionCreated(_id, tokenType, tokenName, tokenSymbol, owner, address(erc721));
  }

  function updateCollection(uint256 _id, string memory tokenType)
    public
    override
  {
    ...
    (Collection memory collection, uint256 index) = getCollection(
      _msgSender(),
      _id
    );
    require(collection._id == _id, "collection is not exist");
    require(collection.owner == owner, "caller is not owner collection");
    ...
    userCollections[owner][index] = collection;
    updateValueCollection(collection);

    emit CollectionUpdated(_id, tokenType, owner);
  }
}
...
contract MockERC721 is ERC721, ERC721Burnable, Ownable {
  ...
  constructor(
    ...
    require(owner != address(0), "ERC721: address is not zero");
...
contract MockERC1155 is ERC1155, ERC1155Burnable, Ownable {
  ...
  using Counters for Counters.Counter;
  Counters.Counter public _tokenIdTracker1155;
  ...
  function mintNewId(address recipient, uint256 amount) external {
    _tokenIdTracker1155.increment();
    _mint(recipient, _tokenIdTracker1155.current(), amount, "");
  }
