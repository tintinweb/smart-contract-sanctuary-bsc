...
contract TimeLocker is Ownable {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _beneficiaries.length; i += 1) {
            ...
            tl.balance = uint256(_amount[i]);
            tl.startDate = _start;
            tl.cliffDate = _cliff;
            tl.periods = _periods;
            tl.valid = true;

            timeLocks[_beneficiaries[i]] = tl;
        }
    }
    ...
    function vest(
        ...
        require(!timeLocks[beneficiary].valid, "Already vesting");
        require(
        ...
        emit Vest(beneficiary, amount, periods[periods.length - 1]);
    ...
    function release() public virtual returns (bool) {
        ...
        require(tl.balance > 0, "Error");

        uint256 vested = _vestingSchedule(tl, uint64(block.timestamp));
        
        tl.released += vested;
        timeLocks[msg.sender] = tl;
        emit Release(msg.sender, vested);

        token.approve(address(this), vested);

        require(vested > 0, "No tokens to transfer");
        require(token.transferFrom(address(this), msg.sender, vested), "Error Transfer");
    ...
    function nextVestSchedule(address beneficiary) public view returns (uint256) {
        TimeLock memory tl = timeLocks[beneficiary];
        ...
        if (block.timestamp > lastPeriod) {
            return 0;
        } else if (block.timestamp > tl.periods[0]) {
            return tl.periods[0];
        }
    ...
    function _vestingSchedule(TimeLock memory tl, uint64 timestamp)
        ...
        returns (uint256)
    {
        if (tl.balance <= 0 || timestamp < tl.startDate || tl.revoked) {
            return 0;
        }
