...
contract Trade {
    ...
    mapping(address => bool) public freeBuyers;
    mapping(address => bool) public freeSellers;
    ...
    function updateFreeBuyerList(address _buyerAddr, bool _newStatus) public onlyOwner returns(bool) {
        freeBuyers[_buyerAddr] = _newStatus;
        return true;
    }

    function updateFreeSellerList(address _sellerAddr, bool _newStatus) public onlyOwner returns(bool) {
        freeSellers[_sellerAddr] = _newStatus;
        return true;
    }
    ...
    function getFees( Order memory order) internal view returns(Fee memory){
        ...
        uint buyerFee = (freeBuyers[order.buyer]==false) ? order.amount.sub(price) : 0;
        uint sellerFee = (freeSellers[order.seller]==false) ? price.mul(sellerFeePermille).div(1000) : 0;
