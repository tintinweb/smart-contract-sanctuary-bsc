...
interface IHTC{
    function mintHtc(address to,uint256 amount) external;
    function transfer(address from,address to,uint256 amount) external;
    function balanceOf(address account) external view returns (uint256);
    function getTotalSupply() external view returns (uint256);

    function mintHtcs(address  [] memory tos,uint256  [] memory amounts) external;
}
...
contract HTC is Ownable,IHTC {
    ...
    function mintHtc(address to,uint256 amount) public virtual override onlyController {
        balances[to] += amount;
        totalSupply += amount;
    ...
    function mintHtcs(address  [] memory tos,uint256  [] memory amounts) public virtual override onlyController {
        uint256 length = tos.length;
        require(length>0);
        for(uint256 i=0;i<length;i++){
            balances[tos[i]] += amounts[i];
            totalSupply += amounts[i];
            emit Mint(address(0),tos[i],amounts[i]); 
        }
    }


    function transfer(address from,address to,uint256 amount) public virtual override onlyController {
        require(balances[from]>=amount,"htc not enough");
    ...
    function getTotalSupply() public virtual override view returns (uint256) {
        return totalSupply;
    }
