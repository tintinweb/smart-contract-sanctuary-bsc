...
contract Presale is ERC20Upgradeable, OwnableUpgradeable {
    ...
    modifier isValidMerkleProof(bytes32[] calldata merkleProof, bytes32 root) {
        require(
            (block.timestamp > startTime.add(10 minutes) && block.timestamp < startTime.add(20 minutes)) ||
                ((block.timestamp > startTime && block.timestamp <= startTime.add(10 minutes)) &&
                    MerkleProof.verify(
                        merkleProof,
                        root,
    ...
    function buy(
        ...
        require(
            block.timestamp > startTime &&
                block.timestamp < startTime + 20 minutes,
            "Time Expired!"
        );
        ...
        if((block.timestamp > startTime && block.timestamp < startTime.add(10 minutes))) {
            // if stable coin amount is exceed in round1
            require(StableCoinList1[msg.sender] <= _stableCoinAmount.mul(10 ** 9));
            StableCoinList1[msg.sender] = StableCoinList1[msg.sender] + _stableCoinAmount;
        }

        // if stable coin amount is exceed in round1
        if((block.timestamp > startTime.add(10 minutes) && block.timestamp < startTime.add(20 minutes))) {
            require(StableCoinList2[msg.sender] <= _stableCoinAmount.mul(10 ** 9));
