...
contract Efatah33 is IERC20 {
    ...
    function _baseTransfer(address from, address to, uint256 amount, uint256 feePercent) internal returns (bool) { 
        require(_gonBalances[from] >= amount, "Fees cannot exceed Txn. Amount"); 
        _gonBalances[from] = _gonBalances[from].sub(amount);

        uint256 feeAmt = feePercent > 0? amount.mul(feePercent).div(_pcent100) : 0;       
        _gonBalances[to] = _gonBalances[to].add(amount.sub(feeAmt));
    ...
    function _baseTransferFee(address to, uint256 amount, uint256 feePercent, uint is_buy) internal  {  
        if(feePercent > 0 && amount > 0) {
            ...
            _gonBalances[to] = _gonBalances[to].add(feeAmt);
    ...
    function _transferFrom(address sender, address recipient,  uint256 amount) internal returns (bool) {
        ...
        if (block.timestamp > (userDailyTxns[sender].txnTimeSecs.add(maxTransferTimeSecs))) {
            ...
            uint256 _max_txn_allowed = maxDailyWalletTransferPercent >= 100? _gonBalances[sender] : _gonBalances[sender].mul(maxDailyWalletTransferPercent).div(_pcent100);
        ...
        uint256 account_balance = userDailyTxns[sender].txnAmount.add(_gonBalances[sender]);
        ...
        )
    {
        return ( 
            _gonBalances[DEAD], 
            _gonBalances[marketingReceiver],  
            _gonBalances[liquidityReceiver],  
            _gonBalances[projectReceiver],  
            _gonBalances[epifReceiver],  
            _gonBalances[devReceiver],     
            _gonBalances[teamReceiver]
        ); 
    }
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return (MAX_SUPPLY.sub(_gonBalances[DEAD]));
    }
