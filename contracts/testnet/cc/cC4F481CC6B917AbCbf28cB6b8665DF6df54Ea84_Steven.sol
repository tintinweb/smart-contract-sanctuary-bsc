...
contract Steven is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 1_000_000_000 ether; // 30 trillion total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = " Steven"; // token name
    string private _symbol = "SJ"; // token ticker
    uint8 private _decimals = 18; // token decimals

    IPancakeRouter02 public pancakeRouter;
    address public pancakePair;
    address payable public wheelWallet;
    address payable public instantRewardWallet;
    ...
    uint256 public lastTxTime = block.timestamp;
    ...
    // buy tax fee
    uint256 public redistributionFeeOnBuying = 10; // 2% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnBuying = 50; // 0% will be added to the liquidity pool
    uint256 public wheelWalletFeeOnBuying = 40; // 0% will go to the wheelWallet address
    uint256 public instantRewardwalletFeeOnBuying = 50; // 0% will go to the instantRewardWallet address
    ...
    uint256 public wheelWalletFeeOnSelling = 40; // 1% will go to the market address\
    uint256 public instantRewardwalletFeeOnSelling = 50; // 1% will go to the instantRewardWallet address
    ...
    uint256 public instantRewardwalletFee = 0; // 0% will go to the instantRewardWallet address
    ...
    uint256 private _currentinstantRewardwalletFee;
    ...
    constructor() {
        ...
        instantRewardWallet = payable(msg.sender);
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        uint256 percentage = (tAmount *
            (_currentRedistributionFee +
                ...
                (_currentinstantRewardwalletFee))) / (1e3);
        return percentage;
    }
    ...
    function _takeInstantRewardFee(uint256 tAmount, uint256 currentRate)
        internal
    {
        uint256 tInstantRewardFee = (tAmount * (_currentwheelWalletFee)) /
            (1e3);
        uint256 rInstantRewardFee = tInstantRewardFee * (currentRate);
        _rOwned[instantRewardWallet] =
            _rOwned[instantRewardWallet] +
            (rInstantRewardFee);
        if (_isExcluded[instantRewardWallet])
            _tOwned[instantRewardWallet] =
                _tOwned[instantRewardWallet] +
                (tInstantRewardFee);
        emit Transfer(_msgSender(), instantRewardWallet, tInstantRewardFee);
    }
    ...
    function removeAllFee() private {
        ...
        _currentinstantRewardwalletFee = 0;
    }
    ...
    function setBuyFee() private {
        ...
        _currentinstantRewardwalletFee = instantRewardwalletFeeOnBuying;
    }
    ...
    function setSellFee() private {
        ...
        _currentinstantRewardwalletFee = instantRewardwalletFeeOnSelling;
    }
    ...
    function setNormalFee() private {
        ...
        _currentinstantRewardwalletFee = instantRewardwalletFee;
    }
    ...
    function setSellFeePercent(
        ...
        instantRewardwalletFeeOnSelling = _instantRewardwalletFee;
        require(
            redistributionFeeOnSelling +
                (liquidityFeeOnSelling) +
                (wheelWalletFeeOnSelling) +
                (instantRewardwalletFeeOnSelling) <=
                maxFee,
            "BEP20: Can not be greater than max fee"
        );
    }
    ...
    function setNormalFeePercent(
        ...
        instantRewardwalletFee = _instantRewardwalletFee;
        require(
            redistributionFee +
                (liquidityFee) +
                (wheelWalletFee) +
                (instantRewardwalletFee) <=
                maxFee,
            "BEP20: Can not be greater than max fee"
        );
    }
    ...
    function _transferStandard(
        ...
        if (_currentinstantRewardwalletFee > 0) {
            _takeInstantRewardFee(tAmount, currentRate);
        }
        if (_currentinstantRewardwalletFee > 0) {
            _takeInstantRewardFee(tAmount, currentRate);
        }
