...
contract rent is Ownable  {
    ...
    IERC1155 public market = IERC1155(0x562577584dc0c1544220942F9B88C574E47ff662);//0x5ee32021cE10ee60860a28c6e198c7F666Ac9Bbd); 
    ...
    function listing(uint256 heroId, uint256 price, uint256 day) external {
        ...
        if(data.from != address(0) && data.to != address(0)){
    ...
    function Claim(address from, uint256 id, uint256 curDate, uint256 heroId, bytes memory signature) external {
        ...
        for(j=0;j<listData[msg.sender].length;j++){
            if(listData[msg.sender][j].from != address(0) && listData[msg.sender][j].to != address(0)){
                listData[msg.sender][index] = listData[msg.sender][j];
                index++;
            }
        }

        for(j=0; j<index; j++){
            listData[msg.sender].pop();
        }
    ...
    function CancelList(uint256 id) external {
        ...
        for(i=0;i<listData[msg.sender].length;i++){
            if(listData[msg.sender][i].from != address(0) && listData[msg.sender][i].to != address(0)){
                listData[msg.sender][index] = listData[msg.sender][i];
                index++;
            }
        }
    ...
    function CancelBid(address owner, uint256 id) external {
        ...
        for(j=0;j<listBid[owner][id].length;j++){
            if(listBid[owner][id][j].from != address(0)){
                listBid[owner][id][index2] = listBid[owner][id][j];
                index2++;
            }
        }

        for(j=0; j<index2; j++){
            listBid[owner][id].pop();
        }
    ...
    function GetListAddrApprove(address addr) public view virtual returns (uint256[] memory) {
        uint256 i =0;
        
        RentData[] memory data = GetListOwnerRent(addr);
        uint256[] memory id = new uint256[](data.length);
        
        for(i=0; i<data.length; i++){
            if(data[i].to != address(0x0)){
                id[i] = data[i].heroId;
            }
        }
        return id;
    ...
    function GetCountListRent() public view virtual returns (uint256){
        uint256 i =0;
        uint256 j =0;
        uint256 count =0;

        for(i=0; i<playerBid.length;i++){
            RentData[] memory data = GetListOwnerRent(playerBid[i]);
            for (j=0;j<data.length;j++){
                if(data[j].from!=0x0000000000000000000000000000000000000000){
                    count+=1;
                }
            }
        }
