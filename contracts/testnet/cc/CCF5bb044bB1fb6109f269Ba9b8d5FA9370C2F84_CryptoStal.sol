...
contract CryptoStal is Ownable, ReentrancyGuard {
    ...
    // dig
    function redCatDig(uint _tokenId, uint digCount) external payable nonReentrant canDigCryptoStal {
        uint totalPrice = digCount * digPrice;
        require(msg.value == totalPrice, "money sent is not correct");
        require(isRedCatOwner(_tokenId), "cat isn't your");

        checkBasePool(totalPrice);

        (, uint rarity) = getRarity(_tokenId);
        if (rarity == 0 || rarity == 1) {
            for(uint count = 1; count <= digCount; count++) {
                uint end = gameProbability.rareProbability[gameProbability.rareProbability.length - 1] + 1;
                uint reward = gameReward.rareReward[gameProbability.rareProbability.findUpperBound(random(rarity, count) % end)] * 1 gwei;
                ...
                emit RedCatDigLog(msg.sender, digCount, rarity, armsNumber, roleNumber, thingsNumber, reward);
            }
        } else {
            for(uint count = 1; count <= digCount; count++) {
                uint end = gameProbability.legendProbability[gameProbability.legendProbability.length - 1] + 1;
                uint reward = gameReward.legendReward[gameProbability.legendProbability.findUpperBound(random(rarity, count) % end)] * 1 gwei;
                (bool success1,) = msg.sender.call{value: reward}("");
                require(success1, "msg.sender pay failed");
    ...
    // no redCat dig
    function dig(uint digCount) external payable nonReentrant canDigCryptoStal {
        uint totalPrice = digCount * digPrice;
        require(msg.value == totalPrice, "money sent is not correct");
        ...
        for(uint count = 1; count <= digCount; count++) {
            ...
            uint reward = gameReward.commonReward[gameProbability.commonProbability.findUpperBound(random(NO_RARITY, count) % end)] * 1 gwei;
            (bool success1,) = msg.sender.call{value: reward}("");
            require(success1, "msg.sender pay failed");

            (uint armsNumber, uint roleNumber, uint thingsNumber) = calculateNumber(reward, NO_RARITY, count);

            emit RedCatDigLog(msg.sender, digCount, NO_RARITY, armsNumber, roleNumber, thingsNumber, reward);
        }
    }
    ...
    function checkBasePool(uint totalPrice) private {
        if (getPool() - totalPrice > baseDigPool) {
    ...
    function random(uint rarity, uint count) public view returns (uint) {
