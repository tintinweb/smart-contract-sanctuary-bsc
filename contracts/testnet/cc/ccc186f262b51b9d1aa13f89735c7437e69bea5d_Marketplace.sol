...
interface IERC1155 is IERC165 {
    ...
    function royalty(uint256 id) external view returns (Royalty memory);
...
function updateNftContract(address _nft) public onlyOwner{
 ...
 function list( uint id,uint price,uint amount)public payable{
    //  IERC1155 nft= IERC1155(_nft);
     ...
     require(price>=100,"Minimum Price should be greater than 99 wei");
...
//  //updatelist
 ...
 //sell
  function sell(address _owner,uint _id,uint amount)public payable{
    require(_owner!= msg.sender,"Caller is Owner");
    Listed[] storage temp;
    temp=listedMapping[_owner];
    for(uint i=0;i<temp.length;i++){
        if(temp[i].tokenID==_id){
            require(temp[i].amount>= amount,"Not enough Listed");
            require(temp[i].price* amount<=msg.value,"Please enter the correct Amount");
            ...
            uint tempAmt=msg.value* amount;
            uint tempAmt1=tempAmt - (tempAmt *nft.royalty(_id).per / 100);
              payable(nft.royalty(_id).owner).transfer(tempAmt * nft.royalty(_id).per / 100 );
              payable(_owner).transfer(tempAmt1);
