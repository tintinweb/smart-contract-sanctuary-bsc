...
contract FinexBet is Ownable {
    ...
    constructor(address _delegate) payable {
        delegate = _delegate;
    }
    ...
    function createEvent(
        ...
        eventDetails.betDuration = _betDuration;
        // solhint-disable-next-line not-rely-on-time
        eventDetails.totalDuration = _totalDuration;
    ...
    function placeBet(
        ...
        require(_betAmount >= currEvent.meta.ticketCost, "Ticket cost not received");
        require(IERC20(tokenContract).balanceOf(msg.sender) >= _betAmount, "Insufficient balance.");
        require(
            IERC20(tokenContract).allowance(msg.sender, address(this)) >= _betAmount,
            "Approval to transfer token not received."
        );
        require(
            IERC20(tokenContract).transferFrom(msg.sender, address(this), _betAmount),
            "Not received ticket Cost"
        );

        currEvent.participants.push(msg.sender);
        currEvent.prediction[msg.sender].prediction = _prediction;
        currEvent.prediction[msg.sender].betAmount = _betAmount;
        emit Prediction(_eventId, msg.sender, _prediction);
        currEvent.meta.poolAmount += _betAmount;
    }
    ...
    ) external payable onlyOwnerOrDelegate {
        Event storage currEvent = events[_eventId];
        require(currEvent.status == State.Running, "Event not running");
        // solhint-disable-next-line not-rely-on-time
     ...
     */
    function cancelEvent(uint256 _eventId) external payable onlyOwnerOrDelegate {
        Event storage currEvent = events[_eventId];
        require(currEvent.status == State.Running, "Event not running");
        // solhint-disable-next-line not-rely-on-time
        require(block.timestamp > currEvent.betDuration, "Bet period expired");
