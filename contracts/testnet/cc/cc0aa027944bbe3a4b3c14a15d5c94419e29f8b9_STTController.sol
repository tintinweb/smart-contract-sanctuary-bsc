...
contract STTData is Ownable {
    ...
    struct TokenRecord {
        ...
        //STT:1-freeCabinTokenReward, 2-orbitalCabinTokenReward, 3-inviteReward, 4-teamReward, 5-swapUsdt
        uint256 types;
    ...
    uint256 public investUsdtLimit = 1 * 10 **18;//复投门槛
    uint256 public lpTokensLimit = 1 * 10 **18;//复投

    uint256 public freeCabinValueRewardLimit = 10 * 10 **16;
    uint256 public orbitalCabinValueRewardLimit = 10 * 10 **16;
    ...
    uint256 public totalDrawAmount;
    uint256 public todayDrawAmount;
    uint256 public totalUserProfit;
    uint256 public todayUserProfit;
    uint256 public totalBlackHoleValue;
    uint256 public todayBlackHoleValue;
    uint256 public totalTransferAmount;
    uint256 public todayTransferAmount;
    uint256 public totalMineOutput;
    uint256 public todayMineOutput;
    ...
    function queryInviteesNum(address user) public view returns(uint256 inviteesNum) {
    ...
    function addUserTokenRecords(address user, uint256 tokenType, int256 tokenAmount, uint256 types) public lock onlyController {
    ...
    function addUserInvestRecords(address user, uint256 usdtAmount) public lock onlyController {
...
contract STTQuery is Ownable {
    ...
    constructor() {
        STTD = STTData(payable(0xcF75d5e1Cb10AD5a9C800B66f81E9d28043524Df));
    }
    ...
    function queryInvestProfit(address user, uint256 types) public view returns(uint256) {
        ...
        if(types == 1){//freeCabin
            ...
            for(i = 0; i < indexs.length; i++){
                (,,,orbitalCabinValue,,,,,) = STTD.investRecords(indexs[i]);
                if(orbitalCabinValue >= STTD.orbitalCabinValueRewardLimit() && ((orbitalCabinDrawTime > 0 && orbitalCabinDrawTime + 24 < nowTime) || (orbitalCabinDrawTime == 0 && investTime + 24 < nowTime))){
                    gammaValue = (freeCabinValue + orbitalCabinValue).mul(120).div(100);
                    profit += gammaValue.mul(25).div(tokenEqualsToUsdt(10 **21));
                }
            }
        }
    ...
    function queryInvestPrincipalCanDraw(address user, uint256 types) public view returns(uint256) {
        ...
        if(types == 1){//freeCabin
            for(i = 0; i < indexs.length; i++){
                // records = investRecords[indexs[i]];
    ...
    function queryInviteRewardLevels(address user) public view returns(uint256) {
        ...
        uint256 inviteesNum = STTD.queryInviteesNum(user);
        uint256 i;
        ...
        for(i = 0; i < invitees0.length; i++){
            if(count0 < 10000){
                ...
                do {
                    ...
                    i = 0;
                } while (count1 <= count0 && (i == invitees0.length - 1 || invitees0.length == 0));
            }
        }
    ...
    function queryInviteLink(address user) public view returns (string memory inviteLink){
        if(queryUserGammaValue(user) > STTD.investUsdtLimit()){
    ...
    function checkBindRecommender(address user, address recommender) public view returns (uint256){
        // require(!isContract(msg.sender), "Caller must be a user");
        ...
        // if(isContract(user)) return 1;
        if(queryUserRecommender(user) != address(0)) return 1;
        if(queryUserPropulsiveCabinValue(user) == 0) return 2;
    ...
    function checkPropulsiveCabin(address user, uint256 usdtAmount) public view returns (uint256){
        ...
        if(usdtAmount < STTD.investUsdtLimit()) return 2;
        if(queryUserBlackHoleValue(user) < usdtAmount.mul(20).div(100)) return 3;
...
contract STTController is Ownable {
    ...
    modifier permit(address operator) {
        require(!isContract(operator) || !STTQ.queryUserDisable(operator) || !STTD.paused(), 'Not allowed');
        _;
    }
    ...
    constructor() {
        ...
        STTD = STTData(payable(0xcF75d5e1Cb10AD5a9C800B66f81E9d28043524Df));
        STTQ = STTQuery(0xC2df9E192eFAa8cE35a3604D494A150BA177e337);
    }
    ...
    function propulsiveCabin(uint256 usdtAmount) public permit(msg.sender) {
        ...
        STTQ.checkPropulsiveCabin(msg.sender, usdtAmount);
    ...
    function _recommenderRewards(address user, uint256 reward) private {
        ...
        for(i = 0; i < 6; i++){
            ...
            if(i < generation){
                ERC20(STTD.stt()).transfer(recommender, reward.mul(STTD.inviteLevelRewardRates(i)).div(100));
                
                STTD.addUserTokenRecords(user, 2, -int(reward.mul(STTD.inviteLevelRewardRates(i)).div(100)), 3 + 10);
            }
            user = recommender;
        }
    }
    ...
    function _teamRewards(address user, uint256 reward) private {
        ...
        for(i = 0; i < 10000; i++){
            ...
            if(recommender == address(0)){
                ...
                if(teamLevel > 0){
                    ERC20(STTD.stt()).transfer(recommender, reward.mul(STTD.teamLevelRewardRates(teamLevel - 1)).div(100));
                    
                    STTD.addUserTokenRecords(user, 2, -int(reward.mul(STTD.teamLevelRewardRates(teamLevel - 1)).div(100)), 4 + 10);
                }
                user = recommender;
            }
        }
