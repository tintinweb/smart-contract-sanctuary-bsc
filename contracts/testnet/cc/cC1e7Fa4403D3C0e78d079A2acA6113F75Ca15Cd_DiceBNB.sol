...
import "../interfaces/ILuckyRollerRouter02.sol";
import "../interfaces/IBetMining.sol";
import "../interfaces/IRollerLevel.sol";
import "../libraries/SafeBEP20.sol";
import "../token/DiceToken.sol";
import "../token/ROLLERToken.sol";
...
contract DiceBNB is IDice, Ownable, ReentrancyGuard, Pausable {
    ...
    IRollerLevel public RollerLevel;
    address public immutable WBNB;
    ROLLERToken public rollerToken;
    DiceToken public diceToken;    
    ILuckyRollerRouter02 public swapRouter;
    ...
    event SetRollerLevel(uint256 indexed block, address RollerLevelAddr);
    ...
    constructor(
        address _WBNBAddr,
        address _rollerTokenAddr,
        address _diceTokenAddr,
        address _RollerLevelAddr,
        address _devAddr,
        ...
        WBNB = _WBNBAddr;
        rollerToken = ROLLERToken(_rollerTokenAddr);
        diceToken = DiceToken(_diceTokenAddr);
        RollerLevel = IRollerLevel(_RollerLevelAddr);
        devAddr = _devAddr;
    ...
    // Claim all bonus to RollerLevel
    function _claimBonusAndLottery() internal {
        ...
        if(totalBurnAmount > 0){
            ...
            totalBurnAmount = 0;
            rollerToken.burn(address(this), tmpAmount);
        }
        if(totalBonusAmount > 0){
            tmpAmount = totalBonusAmount;
            totalBonusAmount = 0;
            if(address(RollerLevel) != address(0)){
                IWBNB(WBNB).deposit{value: tmpAmount}();
                assert(IWBNB(WBNB).transfer(address(RollerLevel), tmpAmount));
                RollerLevel.updateBonus(WBNB, tmpAmount);
            }
        } 
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        { // avoid stack too deep
            ...
            if(round.burnAmount > 0 && address(swapRouter) != address(0)){
                ...
                path[0] = WBNB;
                path[1] = address(rollerToken);
                uint256 amountOut = swapRouter.getAmountsOut(round.burnAmount, path)[1];
                uint256 rollerAmount = swapRouter.swapExactETHForTokens{value: round.burnAmount}(amountOut.mul(2).div(10), path, address(this), block.timestamp + (5 minutes))[1];
                totalBurnAmount = totalBurnAmount.add(rollerAmount);
            }
        }
    ...
    function getWithdrawFeeRatio(address _user) public view returns (uint256 ratio){
        ratio = 0;
        if(address(RollerLevel) != address(0) && address(oracle) != address(0)){
            BankerInfo storage banker = bankerInfo[_user];
            (uint256 totalLevel,,,,,) = RollerLevel.pendingLevel(_user);
            uint256 tokenAmount = banker.diceTokenAmount.mul(netValue).div(1e12);
            uint256 bankerTvl = oracle.getQuantity(WBNB, tokenAmount);
            if(bankerTvl > 0 && fullyWithdrawTh > 0 && totalLevel < bankerTvl.mul(fullyWithdrawTh).div(TOTAL_RATE)){
                // y = - x * maxWithdrawFeeRatio / fullyWithdrawTh + maxWithdrawFeeRatio
                uint256 x = totalLevel.mul(TOTAL_RATE).div(bankerTvl);
    ...
    function setSwapRouter(address _router) external onlyAdmin {
        ...
        swapRouter = ILuckyRollerRouter02(_router);
    ...
    // Update the roller level.
    function setRollerLevel(address _RollerLevelAddr) external onlyAdmin {
        RollerLevel = IRollerLevel(_RollerLevelAddr);
        emit SetRollerLevel(block.number, _RollerLevelAddr);
    }
...
import "./ROLLERBEP20.sol";

// LuckyRoller token with Governance.
contract ROLLERToken is ROLLERBEP20('LuckyRoller', 'ROLLER') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "ROLLER::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "ROLLER::delegateBySig: invalid nonce");
        require(now <= expiry, "ROLLER::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "ROLLER::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ROLLERs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "ROLLER::_writeCheckpoint: block number exceeds 32 bits");
 ...
 */
contract ROLLERBEP20 is Context, IBEP20, Ownable {
...
interface IRollerLevel {
    function updateBonus(address bonusToken, uint256 amount) external;
    function updateLevel(address account) external;
    function getLevel(address account) external view returns (uint256);
    function pendingLevel(address account) external view returns (uint256, uint256, uint256, uint256, uint256, uint256);
}
...
interface ILuckyRollerRouter02 {
...
interface IBetMining {
    ...
    function getRollerLevel(address user) external view returns (uint256);
}
