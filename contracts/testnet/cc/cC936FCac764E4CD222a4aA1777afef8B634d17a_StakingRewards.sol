...
contract StakingRewards is Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(uint256 _pid, uint256 _amount) external payable nonReentrant whenNotPaused {
        ...
        // require(_days == 7, "Lock-in period does not meet requirements");
        uint256 _lockEndTime = block.timestamp + 180;
        if (user.amount > 0) {
            require(_lockEndTime >= user.lockEndTime, "Timelock period does not match previous");
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant whenNotPaused {
        ...
        _getRewards(_pid, msg.sender);
        user.amount = user.amount - _amount;
        user.pendingAmount = user.pendingAmount + _amount;
    }
     ...
     */
    function withdrawVerify(uint256 _pid, address _user, bool _agree) external nonReentrant onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        require(user.pendingAmount > 0, "In this pool, the user has not withdrawn");
        uint256 _pendingPay = user.pendingAmount;
        user.pendingAmount = 0;
        if (_agree) {
            if(pool.token == IERC20(address(0))) {
                payable(_user).transfer(_pendingPay); // 2300 gas
            } else {
                pool.token.safeTransfer(_user, _pendingPay);
            }
        } else {
            _getRewards(_pid, _user);
            user.amount = user.amount + _pendingPay;
        }
    }
     ...
     */
    function getRewardPerSeconds(IERC20 _token, uint256 _amount, uint256 _price) public view returns(uint256) {
        uint256 decimals = 0;
    ...
    function _getRewards(uint256 _pid, address _user) internal {
        ...
        uint256 rewardPerSec = getRewardPerSeconds(pool.token, user.amount, pool.tokenPrice);
     ...
     */
    function transferOut(address token, address to) external onlyOwner {
        if (token == address(0)) {
