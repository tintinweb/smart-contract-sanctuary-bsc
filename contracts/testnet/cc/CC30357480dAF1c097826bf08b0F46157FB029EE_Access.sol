...
contract Access is AccessControl {
    ...
    function grantUserRole(address account) external onlyRole(MODERATOR_ROLE) {
        _grantRole(USER_ROLE, account);
    }

    function revokeUserRole(address account) external onlyRole(MODERATOR_ROLE) {
        _revokeRole(USER_ROLE, account);
    }

    function grantModeratorRole(address account)
        external
