...
abstract contract TokenWrapper is ReentrancyGuard, Ownable {
...
contract Insurance is ReentrancyGuard, Ownable, TokenWrapper, VerifySignature {
    string public _name;
    address public _DAO;
    ...
    function createGame(
        ...
        require(
            gameMaxBetAmount_ >= _MINIMUM_BET_AMOUNT &&
                gameMaxBetAmount_ <= _MAXIMUM_BET_AMOUNT,
            "Error: game max bet amount is not greater than minimum bet and lower than Maximum bet"
        );
    ...
    function bet(
        ...
        require(
            amount_ <= games[gameId_].gameMaxBetAmount,
            "Error: Please betting with an amount token that is lower than game max bet amount!"
        );
        ...
        if (predictResult_ != games[gameId_].creatorInitResult) {
            console.log(
                "getPoolRemainOfCounterpart(gameId_): ",
                getPoolRemainOfCounterpart(gameId_)
            );
