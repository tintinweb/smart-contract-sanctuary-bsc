...
contract deployer is OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        insuranceFund = 25;
        tokenLocker = 0x55a3654d762750E5586909315095b7990d4E7AC0;
        minimumLPLockTime = 30 days;

        __Ownable_init();

        whitelistedRouters[0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3] = true;
    }
    ...
    function createNewPool(
        ...
        require(
            whitelistedRouters[_addresses[1]],
        ...
        require(
            _poolDetails[4] >= _poolDetails[2].mul(_poolDetails[3]).div(10**18),
            "Invalid total tokens amount"
        );
        ...
        uint256[2] memory _fees = [bnbFeePercentage, tokenFeePercentage];

        bool[2] memory _status = [isWhitelisted, _isLpBurn];
        ...
        newPool.setPoolOwner(msg.sender);
...
contract Launchpad is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    **/
    ...
    function initialize(
        ...
        router = IUniswapV2Router02(_addresses[1]);
    ...
    function participateToSale() public payable nonReentrant {
        ...
        if (remainingBNB < minimumParticipate) {
            require(msg.value <= remainingBNB, "You have exceed max amount");
        } else {
            require(
    ...
    function finalizePool() external nonReentrant {
        ...
        if (isLpToBurn) addLpAndBurn(bnbForAddLp, tokensForLp);
        else addLiquidity(bnbForAddLp, tokensForLp, msg.sender);
        ...
        payable(poolOwner).transfer(bnbForOwner);
    ...
    function addLiquidity(
        ...
        IUniswapV2Pair(pair).approve(address(tokenLocker), pirBalance);
    ...
    function addLpAndBurn(uint256 bnbAmount, uint256 tokenAmount) internal {
        pair = IUniswapV2Factory(router.factory()).getPair(
            router.WETH(),
            address(tokenAddress)
        );
    ...
    function changeLockerAddress(address _locker) public onlyOwner {
        tokenLocker = ITokenLocker(_locker);
    }

    function setPoolOwner(address _owner) external onlyOwner {
        poolOwner = _owner;
    }
}
