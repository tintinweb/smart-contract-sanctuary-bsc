...
contract UniStaking is TwoStageOwnable, UniStakingTokensStorage {
    ...
    event FeeReceiverAddressEvent(address FeeReceiverAddress);
    ...
    constructor(
        ...
        address owner_
    ) TwoStageOwnable(owner_) UniStakingTokensStorage(rewardsToken_, stakingToken_, feeReceiver_) {
    }
    ...
    function setFeeReceiverAddress(address FeeReceiverAddress) public  onlyOwner {
        _setfeeReceiverAddress(FeeReceiverAddress);
        emit FeeReceiverAddressEvent(FeeReceiverAddress);
    }
...
abstract contract UniStakingTokensStorage {
    ...
    function _setfeeReceiverAddress(address FeeReceiverAddress) internal {
        _feeReceiverAddress = FeeReceiverAddress;
    }

    constructor(IERC20 rewardsToken_, IERC20 stakingToken_, address feeReceiver_) {
        _rewardsToken = rewardsToken_;
        _stakingToken = stakingToken_;
        _feeReceiverAddress = feeReceiver_;
    }
    ...
    function _unstake(address account, uint256 amount) internal {
        ...
        /**
        */
        _stakingToken.safeTransfer(_feeReceiverAddress, fee);
