...
// 
/**
 * @title Stable Forage Staking
 * @author Trader Forage
 * @notice StableForageStaking is a contract that allows Forage deposits and receives stablecoins sent by MoneyMaker's daily
 * harvests. Users deposit Forage and receive a share of what has been sent by MoneyMaker based on their participation of
 * the total deposited Forage. It is similar to a MasterChef, but we allow for claiming of different reward tokens
 ...
 */
contract StableForageStaking is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    struct UserInfo {
        ...
        /**
         * @notice We do some fancy math here. Basically, any point in time, the amount of Forages
         * entitled to a user but is pending to be distributed is:
         *
         *   pending reward = (user.amount * accRewardPerShare) - user.rewardDebt[token]
         *
         * Whenever a user deposits or withdraws Forage. Here's what happens:
    ...
    /// @dev Internal balance of Forage, this gets updated on user deposits / withdrawals
    /// this allows to reward users with Forage
    uint256 public internalForageBalance;
    ...
    /// @dev Info of each user that stakes Forage
    mapping(address => UserInfo) private userInfo;

    /// @notice Emitted when a user deposits Forage
    ...
    /// @notice Emitted when a user withdraws Forage
    event Withdraw(address indexed user, uint256 amount);

    /// @notice Emitted when a user claims reward
    event ClaimReward(address indexed user, address indexed rewardToken, uint256 amount);

    /// @notice Emitted when a user emergency withdraws its Forage
    ...
    /**
     * @notice Initialize a new StableForageStaking contract
     * @dev This contract needs to receive an ERC20 `_rewardToken` in order to distribute them
     * (with MoneyMaker in our case)
     * @param _rewardToken The address of the ERC20 reward token
     * @param _forage The address of the Forage token
    ...
    function initialize(
        ...
        __Ownable_init();
        require(address(_rewardToken) != address(0), "StableForageStaking: reward token can't be address(0)");
        require(address(_forage) != address(0), "StableForageStaking: Forage can't be address(0)");
        require(_feeCollector != address(0), "StableForageStaking: fee collector can't be address(0)");
        require(_depositFeePercent <= 5e17, "StableForageStaking: max deposit fee can't be greater than 50%");
    ...
    /**
     * @notice Deposit Forage for reward token allocation
    ...
    function deposit(uint256 _amount) external {
        ...
        internalForageBalance = internalForageBalance.add(_amountMinusFee);
        forage.safeTransferFrom(_msgSender(), feeCollector, _fee);
        forage.safeTransferFrom(_msgSender(), address(this), _amountMinusFee);
     ...
     * @return The amount of Forage user has deposited
    ...
    function addRewardToken(IERC20Upgradeable _rewardToken) external onlyOwner {
        require(
            ...
            "StableForageStaking: token can't be added"
        );
        require(rewardTokens.length < 25, "StableForageStaking: list of token too big");
        rewardTokens.push(_rewardToken);
        isRewardToken[_rewardToken] = true;
    ...
    function removeRewardToken(IERC20Upgradeable _rewardToken) external onlyOwner {
        require(isRewardToken[_rewardToken], "StableForageStaking: token can't be removed");
        updateReward(_rewardToken);
    ...
    function setDepositFeePercent(uint256 _depositFeePercent) external onlyOwner {
        require(_depositFeePercent <= 5e17, "StableForageStaking: deposit fee can't be greater than 50%");
    ...
    function pendingReward(address _user, IERC20Upgradeable _token) external view returns (uint256) {
        require(isRewardToken[_token], "StableForageStaking: wrong reward token");
        UserInfo storage user = userInfo[_user];
        uint256 _totalForage = internalForageBalance;
        uint256 _accRewardTokenPerShare = accRewardPerShare[_token];

        uint256 _currRewardBalance = _token.balanceOf(address(this));
        uint256 _rewardBalance = _token == forage ? _currRewardBalance.sub(_totalForage) : _currRewardBalance;

        if (_rewardBalance != lastRewardBalance[_token] && _totalForage != 0) {
            uint256 _accruedReward = _rewardBalance.sub(lastRewardBalance[_token]);
            _accRewardTokenPerShare = _accRewardTokenPerShare.add(
                _accruedReward.mul(ACC_REWARD_PER_SHARE_PRECISION).div(_totalForage)
            );
        }
        return
    ...
    /**
     * @notice Withdraw Forage and harvest the rewards
     * @param _amount The amount of Forage to withdraw
     */
    function withdraw(uint256 _amount) external {
        UserInfo storage user = userInfo[_msgSender()];
        uint256 _previousAmount = user.amount;
        require(_amount <= _previousAmount, "StableForageStaking: withdraw amount exceeds balance");
        ...
        internalForageBalance = internalForageBalance.sub(_amount);
    ...
    function updateReward(IERC20Upgradeable _token) public {
        ...
        // Did StableForageStaking receive any token
        if (_rewardBalance == lastRewardBalance[_token] || _totalForage == 0) {
            return;
        }
    ...
    function safeTokenTransfer(
        ...
        uint256 _currRewardBalance = _token.balanceOf(address(this));
        uint256 _rewardBalance = _token == forage ? _currRewardBalance.sub(internalForageBalance) : _currRewardBalance;
