...
contract NetherMarketplace is OwnableUpgradeable {
    struct ItemForSale {
        address seller;
        uint256 price;
    }
    ...
    mapping (address => mapping (uint256 => ItemForSale)) public _nftInfoForSale;
    ...
    address public netherNFTAddress;
    ...
    event BuyItem(address indexed _collection, uint256 indexed _tokenID, uint256 _price, address seller, address _buyer);
    event CreateItem(address indexed collection, uint256 indexed _tokenID, string _tokenURI, uint256 _price, address _creator);
    event CreateCollection(address indexed _collection, string _collectionName, string _collectionSymbol, uint256 _maxSupply, address _creator);
    ...
    event EndAuction(address indexed _collection, uint256 indexed _tokenID, uint256 _lastPrice, address _lastBidder);
    event CancelAuction(address indexed _collection, uint256 indexed _tokenID);
    event RefundAuctionFund(address indexed _collection, uint256 indexed _tokenID, uint256 _bidPrice, address _bidder);
    ...
    function initialize(address _register) public initializer {
        ...
        distributionFee.marketplaceFee = 15;
        distributionFee.creatorFee = 15;
        distributionFee.level1 = 7;
        distributionFee.level2 = 14;
        distributionFee.level3 = 21;
        distributionFee.level4 = 14;
        distributionFee.level5 = 7;
        distributionFee.level6 = 7;
        distributionFee.sellerPercentage = 
            denominator - distributionFee.marketplaceFee - distributionFee.creatorFee - 
            distributionFee.level1 - distributionFee.level2 - distributionFee.level3 - 
            distributionFee.level4 - distributionFee.level5 - distributionFee.level6;

        createItemFeeForTeam = .001 ether;
        createItemFeeForCreator = .001 ether;
        createCollectionFee = .001 ether;

        //create nether nft
        // netherNFTAddress = createCollection("NetherNFT", "NNT", 2**256 - 1);
    }

    function changeCreateFee(uint8 _createItemFeeForTeam, uint256 _createItemFeeForCreator, uint8 _createCollectionFee) public onlyOwner() {
        ...
        ) public onlyOwner() 
    {
        ...
        distributionFee.level1 = _level1Fee;
        distributionFee.level2 = _level2Fee;
        distributionFee.level3 = _level3Fee;
        distributionFee.level4 = _level4Fee;
        distributionFee.level5 = _level5Fee;
        distributionFee.level6 = _level6Fee;
        distributionFee.sellerPercentage = 
            denominator - distributionFee.marketplaceFee - distributionFee.creatorFee - 
            distributionFee.level1 - distributionFee.level2 - distributionFee.level3 - 
            distributionFee.level4 - distributionFee.level5 - distributionFee.level6;
    ...
    function createCollection(string memory name, string memory symbol, uint256 maxSupply) public payable onlyRegisterUser returns(address) {
        require(msg.value == createCollectionFee, "Insufficient Funds");
        ...
        emit CreateCollection(address(collection), name, symbol, maxSupply, msg.sender);
        return address(collection);
    }

    function createItems(address collection, string memory tokenURI, uint32 supply, uint256 price) public payable onlyRegisterUser {
        ...
        for(uint32 i = 0; i < supply; i++) {
            ...
            _nftInfoForSale[collection][tokenID] = ItemForSale({seller: msg.sender, price: price});
    ...
    function editItemForSale(address collection, uint256 tokenID, uint256 newPrice) public onlyRegisterUser {
        ...
        _nftInfoForSale[collection][tokenID] = ItemForSale({seller: msg.sender, price: newPrice});
    ...
    function cancelItemForSale(address collection, uint256 tokenID) public onlyRegisterUser {
        require(_nftInfoForSale[collection][tokenID].seller == msg.sender, "Not Your NFT");
        _nftInfoForSale[collection][tokenID] = ItemForSale({seller: address(0), price: 0});
        IERC721(collection).transferFrom(address(this), msg.sender, tokenID);
        emit CancelItemPrice(collection, tokenID, msg.sender); 
    }

    function buyForListedItem(address collection, uint256 tokenID) public payable onlyRegisterUser {
        ItemForSale memory itemForSale = _nftInfoForSale[collection][tokenID];
        // Verify that the item is for sale
        require(itemForSale.seller != address(0x0) || itemForSale.price != 0, "This item is not for sale");
        // Ensure buyer can't be seller
        require(itemForSale.seller != msg.sender, "Sellers cannot buy from themselves");
        // Ensure price
        require(itemForSale.price == msg.value, "Insufficient price");

        //Transfer NFT to buyer
        IERC721(collection).transferFrom(address(this), msg.sender, tokenID);

        //Transfer money by distribution
        _distributeFunds(itemForSale.seller, msg.sender, _addedCollections[collection].creatorAddress, msg.value);

        emit BuyItem(collection, tokenID, itemForSale.price, itemForSale.seller, msg.sender);
    }
    ...
    function bidOnAuction(address collection, uint256 tokenID) public payable onlyRegisterUser {
        ...
        if (auctionInfo.highestBidAddress != address(0x0)) {
            ...
            emit RefundAuctionFund(collection, tokenID, auctionInfo.highestBidPrice, auctionInfo.highestBidAddress);
        }
    ...
    function endAuction(address collection, uint256 tokenID) public onlyRegisterUser {
        AuctionInfo storage auctionInfo = _nftInfoForAuction[collection][tokenID];
        require(auctionInfo.state == AuctionState.OPEN, "This is non exist Auction");
        require(auctionInfo.endAt < block.timestamp, "It's not time yet");
        require(auctionInfo.highestBidAddress == msg.sender, "Not allowed for you");
        ...
        emit EndAuction(collection, tokenID, auctionInfo.highestBidPrice, auctionInfo.highestBidAddress);
    }
    ...
    function cancelAuction(address collection, uint256 tokenID) public onlyRegisterUser {
        ...
        emit CancelAuction(collection, tokenID);
    }
    ...
    function _distributeFunds(address _seller, address _buyer, address _creator, uint256 _price) private {
        ...
        payable(referrerAddresses[0]).transfer(_price * distributionFee.level1 / denominator);
        payable(referrerAddresses[1]).transfer(_price * distributionFee.level2 / denominator);
        payable(referrerAddresses[2]).transfer(_price * distributionFee.level3 / denominator);
        payable(referrerAddresses[3]).transfer(_price * distributionFee.level4 / denominator);
        payable(referrerAddresses[4]).transfer(_price * distributionFee.level5 / denominator);
        payable(referrerAddresses[5]).transfer(_price * distributionFee.level6 / denominator);
        payable(_seller).transfer(_price * distributionFee.sellerPercentage / denominator);
    }

    function getSaleInfo(address _collection, uint256 _id) public view returns(ItemForSale memory){
