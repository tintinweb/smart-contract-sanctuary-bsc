...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  address public immutable token = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43); // DOGE Token
...
contract Yutu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Yutu", "YUTU") {
        address newOwner = address(0xa4AA78C303aF54Da128E757b61E5741C028deF0F);
        safuDevAddress = _msgSender();
        _decimals = 9;
    
        uint256 totalSupply = 1e14 * (10**_decimals);
        ...
        stakingSellFee = 0;
        burnSellFee = 1;
        totalSellFees = rewardsSellFee + marketingSellFee + liquiditySellFee + stakingSellFee + burnSellFee;
        ...
        stakingBuyFee = 6;
        burnBuyFee = 0;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + liquidityBuyFee + stakingBuyFee + burnBuyFee;
        
    	dividendTracker = new DividendTracker();
    	
    	marketingWallet = address(0x5259151ecc87382df57868a44f4802836726168B); // set as marketing wallet
        stakingWallet = address(0x347B99ADB47701500b8D8f034df28950Bb3629ab); // set as staking wallet
        ...
        excludeFromMaxTransaction(safuDevAddress, true);
    ...
    function addPresaleAddressForExclusions(address _presaleAddress, address _presaleRouterAddress) external isAuth {
    ...
    function emergencyPresaleAddressUpdate(address _presaleAddress, address _presaleRouterAddress) external isAuth {
        presaleAddress = _presaleAddress;
        presaleRouterAddress = _presaleRouterAddress;
    }
    
     // disable Transfer delay - cannot be reenabled
    function disableTransferDelay() external isAuth returns (bool){
        transferDelayEnabled = false;
        return true;
    }

    // excludes wallets and contracts from dividends (such as CEX hotwallets, etc.)
    function excludeFromDividends(address account) external isAuth {
        dividendTracker.excludeFromDividends(account);
        emit ExcludeFromDividends(account);
    }

    // removes exclusion on wallets and contracts from dividends (such as CEX hotwallets, etc.)
    function includeInDividends(address account) external isAuth {
    ...
    function updateMaxAmount(uint256 newNum) external isAuth {
        require(newNum > (totalSupply() * 1 / 100)/(10** _decimals), "Cannot set maxTransactionAmount lower than 0.5%");
        maxTransactionAmount = newNum * (10**_decimals);
    }
    
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _burnFee) external {
        ...
        burnBuyFee = _burnFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + liquidityBuyFee + burnBuyFee;
        require(totalBuyFees + totalSellFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _burnFee) external isAuth {
        require(msg.sender == safuDevAddress || (safuDevAddress == address(0xdead) && msg.sender == owner()), "Only safuDev can change fees for first 14 days. Owner can changes after Safu dev is renounced");
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        burnSellFee = _burnFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + burnSellFee;
        require(totalBuyFees + totalSellFees <= 20, "Must keep fees at 30% or less");
    }

    function excludeFromMaxTransaction(address updAds, bool isEx) public isAuth {
    ...
    function excludeMultipleAccountsFromFees(address[] calldata accounts, bool excluded) external isAuth {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public isAuth {
    ...
    function updateGasForProcessing(uint256 newValue) external isAuth {
    ...
    function updateClaimWait(uint256 claimWait) external isAuth {
    ...
    function removeLimits() external isAuth returns (bool){
    ...
    function swapBack() private {
        ...
        if (tokensForBurn > 0){
            super._transfer(address(this), address(this), tokensForBurn);
        }
    ...
    function buyBackTokens(uint256 bnbAmountInWei) external isAuth {
