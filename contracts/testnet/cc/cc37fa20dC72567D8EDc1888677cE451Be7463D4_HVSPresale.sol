...
contract HVSPresale is Context, ReentrancyGuard, Ownable {
    ...
    uint256 public constant PRESALE_DURATION = 12 days;
    ...
    function getTokenPrice() public view returns (uint256) {
        if(block.timestamp < presale_info.presale_start)
            return 0;
        else if((block.timestamp >= presale_info.presale_start) && (block.timestamp < (presale_info.presale_start + 1 days)))
            return 1;
        else if((block.timestamp >= (presale_info.presale_start + 1 days)) && (block.timestamp < (presale_info.presale_start + 2 days)))
            return 3;
        else if((block.timestamp >= (presale_info.presale_start + 2 days)) && (block.timestamp < (presale_info.presale_start + 3 days)))
            return 6;
        else if((block.timestamp >= (presale_info.presale_start + 3 days)) && (block.timestamp < (presale_info.presale_start + 10 days)))
            return 30;
        return 0;
    }
