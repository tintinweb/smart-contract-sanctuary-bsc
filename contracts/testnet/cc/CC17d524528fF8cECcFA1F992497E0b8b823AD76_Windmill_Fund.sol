    ...
    function updateWithdrawFees(Fraction memory tradeProfit) external onlyRole(TRADE_ROLE){
        if (tradeProfit.numerator > tradeProfit.denominator){
            ...
            if (tradeProfit.numerator >= tradeProfit.denominator){
                ...
                uint256 delta = ((tradeProfit.denominator -  tradeProfit.numerator) * 1e10) / tradeProfit.denominator;
                if (delta >= dynamicFees.startDynamicFeesNumerator){
                    dynamicFees.startDynamicFeesNumerator = 0;
                }else{
                    dynamicFees.startDynamicFeesNumerator -= delta;
                }
            }
