	...
	AccessControlEnumerable
{
	...
	mapping(IMockToken => uint256) public tokenPrices;
	BlockTokenInfo[] public blockTokenInfos;

	struct BlockTokenInfo {
		IMockToken token0; // How many staked tokens the user has provided
		IMockToken token1; // Reward debt
		IMockToken tokenLp;
	}

	struct UserInfo {
		uint256 depositAmount; // How many staked tokens the user has provided
		uint256 rewardsStartBlock; // Reward debt
	}
	...
	constructor(
		...
		BlockTokenInfo[] memory _blockTokenInfos,
		...
		for (uint8 i = 0; i < _blockTokenInfos.length; i++) {
			blockTokenInfos.push(
				BlockTokenInfo({
					token0: _blockTokenInfos[i].token0,
					token1: _blockTokenInfos[i].token1,
					tokenLp: _blockTokenInfos[i].tokenLp
				})
			);
		}

		for (uint8 i = 0; i < _priceInfos.length; i++) {
			tokenPrices[_priceInfos[i].token] = _priceInfos[i].price;
		}
	}
	...
	function setSettings(StrategySettings.Action[][] memory _settings)
		external
		override
	{
		revert('method not supported');
	...
	function getFailedSetting(uint256 _blockIndex)
		external
		view
		override
	...
	function setFailedSetting(
		uint256 _blockIndex,
		StrategySettings.FailAction memory _setting
	) external override {
		revert('method not supported');
	}
	...
	function deposit(
		...
		require(_blockIndex == 0, 'Invalid block index');
		require(
			_token == address(blockTokenInfos[0].token0) ||
				_token == address(blockTokenInfos[0].token1) ||
				_token == address(blockTokenInfos[0].tokenLp),
			'invalid token'
		);
		IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
		IMockToken(_token).burn(_amount);

		uint256 depositAmount = _amount
			.mul(tokenPrices[IMockToken(_token)])
			.div(tokenPrices[blockTokenInfos[0].tokenLp]);
		IMockToken(blockTokenInfos[0].tokenLp).mint(depositAmount);

		if (userInfo[msg.sender].depositAmount == 0) {
			userInfo[msg.sender].rewardsStartBlock = block.number;
		}

		userInfo[msg.sender].depositAmount = userInfo[msg.sender]
			.depositAmount
			.add(depositAmount);
	}
	...
	function withdrawAll(
		...
		_withdraw(0, userInfo[msg.sender].depositAmount, _outputTokens[0]);
		_withdraw(1, 0, _outputTokens[1]);
	}

	function withdraw(uint256 _blockIndex, uint256 _amount)
		external
		override
		nonReentrant
	{
	...
	function withdraw(
		...
		_withdraw(_blockIndex, _amount, _outputTokens);
	}
	...
	function _withdraw(
		...
		require(_blockIndex < 2, 'Invalid block index');
		require(_outputTokens.length > 0, 'No output tokens');

		if (_blockIndex == 0) {
			require(_outputTokens.length == 1, 'Too many output tokens');
		} else {
			require(_outputTokens.length <= 2, 'Too many output tokens');
		}
		require(
			address(_outputTokens[0]) ==
				address(blockTokenInfos[_blockIndex].token0) ||
				address(_outputTokens[0]) ==
				address(blockTokenInfos[_blockIndex].token1),
			'Invalid output token'
		);

		if (_outputTokens.length == 2)
			require(
				address(_outputTokens[1]) ==
					address(blockTokenInfos[_blockIndex].token0) ||
					address(_outputTokens[1]) ==
					address(blockTokenInfos[_blockIndex].token1),
				'Invalid output token'
		...
		if (_blockIndex == 0) {
			require(
				_amount <= userInfo[msg.sender].depositAmount,
				'too much withdrawal amount'
			);
			userInfo[msg.sender].depositAmount = userInfo[msg.sender]
				.depositAmount
				.sub(_amount);
			IMockToken(blockTokenInfos[0].tokenLp).burn(_amount);
			_sendOutputTokens(
				_outputTokens,
				_amount.mul(tokenPrices[blockTokenInfos[0].tokenLp])
			);
		} else {
			require(
				_amount == 0,
				'accum blocks do not have partial withdrawals'
			);
			_sendOutputTokens(
				_outputTokens,
				getUserBalances(msg.sender)[1].mul(
					tokenPrices[blockTokenInfos[_blockIndex].tokenLp]
				)
			);
			userInfo[msg.sender].rewardsStartBlock = userInfo[msg.sender]
				.depositAmount > 0
				? block.number
				: 0;
		}
	...
	function _sendOutputTokens(
		IERC20[] calldata _outputTokens,
		uint256 _amountValue
	) internal {
		if (_outputTokens.length == 2) {
			uint256 amountToSend = _amountValue.div(2);
			IMockToken(address(_outputTokens[0])).mintTo(
				msg.sender,
				amountToSend.div(
					tokenPrices[IMockToken(address(_outputTokens[0]))]
				)
			);
			IMockToken(address(_outputTokens[1])).mintTo(
				msg.sender,
				(_amountValue.sub(amountToSend)).div(
					tokenPrices[IMockToken(address(_outputTokens[1]))]
				)
			);
		} else {
			IMockToken(address(_outputTokens[0])).mintTo(
	...
	function withdrawProfit() external override nonReentrant {
		revert('method not supported');
	}

	function run() external override nonReentrant {
		lastRunTime = block.timestamp;
		lastRunBlock = block.number;
		numRuns = numRuns.add(1);
	}

	function run(uint256[][] memory _minAmountOut)
		external
		override
		nonReentrant
	{
	...
	function getUserBalances(address user)
		public
		view
		returns (uint256[] memory _userBalances)
	{
		_userBalances = new uint256[](2);
		_userBalances[0] = userInfo[user].depositAmount;
		_userBalances[1] = userInfo[user].rewardsStartBlock > 0
			? rewardPerBlock.mul(
				block.number.sub(userInfo[user].rewardsStartBlock)
			)
			: 0;
	...
	function setMockPrice(IMockToken _token, uint256 _price)
		external
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
...
interface IStrategyRunner {
	...
	function getSettings()
		external
		view
		returns (StrategySettings.Action[][] memory _settings);
	...
	function run(uint256[][] memory _minAmountOut) external;
