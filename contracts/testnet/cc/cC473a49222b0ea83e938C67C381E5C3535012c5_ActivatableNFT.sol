...
contract ActivatableNFT is ERC721Base, Ownable, IActivatableNFT {
    ...
    event BrokerPBContractAddressChanged(address brokerPBContractAddress);
    
    address internal brokerPBContractAddress;

    modifier onlyBroker() {
        require(msg.sender == brokerPBContractAddress, "ActivatableNFT: This funcrion can be called only by Broker Public Building Contract");
        _;
    }
    ...
    function _setBrokerPBContractAddress(address _brokerPBContractAddress) internal {
        ...
        emit BrokerPBContractAddressChanged(_brokerPBContractAddress);
    }
}
