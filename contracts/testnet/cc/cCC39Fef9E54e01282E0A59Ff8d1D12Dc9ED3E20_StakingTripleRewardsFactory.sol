...
contract StakingTripleRewardsFactory is Ownable {
        ...
        onlyOwner
    {
        ...
        for (uint8 i = 0; i < _rewardsTokens.length; ++i) {
            info.rewardsTokens.push(_rewardsTokens[i]);
            info.rewardAmounts.push(_rewardAmounts[i]);
        }
    ...
    function notifyRewardAmount(uint8 stakingId) public onlyOwner {
        ...
        uint256[] memory rewardAmounts = new uint256[](rewardTokensAmount);
        if (info.duration > 0) {
...
contract StakingTripleRewards is IStakingTripleRewards, TripleRewardsDistributionRecipient, ReentrancyGuard {
    ...
    uint256[] public rewardRates = new uint256[](rewardTokensAmount);
    uint256[] public lastUpdateTime = new uint256[](rewardTokensAmount);

    uint256[] public rewardPerTokensStored = new uint256[](rewardTokensAmount);

    mapping(address => uint256[rewardTokensAmount]) public userRewardPerTokensPaid;
    mapping(address => uint256[rewardTokensAmount]) public rewardPerTokens;
    ...
    function rewardPerToken(uint index) public view override returns (uint256) {
        ...
        return rewardPerTokensStored[index] + (lastTimeRewardApplicable() - lastUpdateTime[index]) * rewardRates[index] * 1e18 / _totalSupply;
    }
    ...
    function notifyRewardAmount(uint256[] calldata rewardAmounts, uint256 rewardsDuration) external onlyTripleRewardsDistribution updateReward(address(0)) {
        ...
        for (uint8 i = 0; i < rewardAmounts.length; ++i) {
            ...
            lastUpdateTime[i] = block.timestamp;
