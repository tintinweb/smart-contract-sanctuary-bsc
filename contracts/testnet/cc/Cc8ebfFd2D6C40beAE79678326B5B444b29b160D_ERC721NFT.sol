...
contract ERC721NFT is ERC721, Ownable{
    ...
    function mint(string memory uri, uint256 _royalty) public returns(bool){
    ...
    function batchMint(string[] memory uri, uint256 _royalty) public returns(uint256[] memory, string[] memory){
        
        require(isMinter[_msgSender()], "NFT: Not Minter");
        require(uri.length <= 100, "NFT: maximum 100 batch mint is allowed");

        uint256[] memory idlist = new uint256[](uri.length);

            for(uint256 j = 0; j < uri.length; j++){
