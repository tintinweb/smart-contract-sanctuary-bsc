    ...
    PausableUpgradeable
{
    ...
    struct AdditionalPoolInfo {
        ...
        mapping(uint16 => uint16) penaltyRate; // 100% = 10000
    ...
    function linearAddListWhitelist(uint256 _poolId, address[] calldata _accounts)
        external
        onlyOwner
        linearValidatePoolById(_poolId)
    {
        ...
        linearValidatePoolById(_poolId)
    {
        ...
        require(
            poolInfo.isPenaltyEnabled[_percentage] && poolInfo.penaltyRate[_percentage] <= HUNDRED_PERCENT,
        ...
        require(
            block.timestamp <
                stakingData.joinTime +
                    pool.lockDuration -
                    poolInfo.delayDuration,
            "LinearStakingPool: invalid early withdraw time"
        );
