...
contract BSG is Ownable{
    ...
    uint256 public constant dayPerCycle = 60 minutes;

    uint256 private constant dayReward2Percents = 180000000000000000000;
    uint256 private constant dayRewardPercents = 146666666666666666667;
    ...
    uint256 private constant doubleDiamondPoolPercents = 200;

    uint256 private constant discountpercentage = 500;
    mapping(address => bool) public firstDeposite;

    uint256 private constant maximunwithdraw = 3;
    ...
    function withdraw() external {
        ...
        lastwithdraw[msg.sender] = lastwithdraw[msg.sender].add(Total[msg.sender]);
        ...
        withdrawable = withdrawable.add(remainingReward[msg.sender]); 

        // uint256 withDraw_ = Total[msg.sender] * maximunwithdraw;
        if(withdrawable > withDraw_)
        {
            remainingReward[msg.sender] = withdrawable.sub(withDraw_);
            withdrawable = withDraw_;
        }
        else
        {
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 500e18){
            ...
            if(total >= 500e18 && user.teamNum >= 6 && maxTeam >= 500e18 && otherTeam >= 500e18){
                levelNow = 5;
            }else if(user.teamNum >= 4 && maxTeam >= 300e18 && otherTeam >= 300e18){
                levelNow = 4;
            }else{
                levelNow = 3;
            }
        }else if(total >= 250e18){
            levelNow = 2;
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        if(!firstDeposite[msg.sender])
        {
            uint256 discountamount = _amount.mul(discountpercentage).div(baseDivider);
            BUSD.transfer(msg.sender, discountamount);
            firstDeposite[msg.sender] =true;
        }
        ...
        Total[msg.sender] = user.totalDeposit.sub(_amount);
        Total[msg.sender] = Total[msg.sender].sub(lastwithdraw[msg.sender]);
        withDraw_ = Total[msg.sender] * maximunwithdraw;
