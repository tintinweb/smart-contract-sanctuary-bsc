...
contract MimosContract is Authorized, ERC20 {

  uint8     constant             decimal                   = 18;
  uint8     constant             decimalBUSD               = 18;  
  string    constant            _name                     = "Mimos Coin";
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    MIMOSCOIN_POOL = address(PancakeFactory(router.factory()).createPair(WBNB, address(this)));
    liquidityPool.push(MIMOSCOIN_POOL);
    address ownerWallet = _msgSender();
    administrationWallet = 0x73a21A4F77DfD4C232AA9a851eAA5ED1FbB4F9bF;
    
    exemptStaker[MIMOSCOIN_POOL]             = true;
    ...
    exemptTxLimit[MIMOSCOIN_POOL]            = true;
  ...
  */ 
  function exchangeFeeParts(uint256 incomingFeeTokenAmount) private returns (bool){
    ...
    address pairBnbMimosCoin = MIMOSCOIN_POOL;
    if (_msgSender() == pairBnbMimosCoin) return false;
    ...
    (uint112 reserve0, uint112 reserve1) = getTokenReserves(pairBnbMimosCoin);
    bool reversed = isReversed(pairBnbMimosCoin, wbnbAddress);
    if (reversed) { uint112 temp = reserve0; reserve0 = reserve1; reserve1 = temp; }
    _balances[pairBnbMimosCoin] += feeTokenAmount;
    ...
    swapToken(pairBnbMimosCoin, reversed ? 0 : wbnbAmount, reversed ? wbnbAmount : 0, swapHelperAddress);
...
*/
  ...
  function buyBackWithDecimals(uint256 decimalAmount, address destAddress) private {
    ...
    bool previousExemptFeeState = exemptFee[MIMOSCOIN_POOL];
    exemptFee[MIMOSCOIN_POOL] = true;
    
    address pairBnbMimosCoin = MIMOSCOIN_POOL;
    address swapHelperAddress = address(swapHelper);
    (reserve0, reserve1) = getTokenReserves(pairBnbMimosCoin);
    reversed = isReversed(pairBnbMimosCoin, WBNB);
    if (reversed) { uint112 temp = reserve0; reserve0 = reserve1; reserve1 = temp; }

    tokenTransfer(WBNB, pairBnbMimosCoin, wbnbAmount);
    
    uint256 igtAmount = getAmountOut(wbnbAmount, reserve0, reserve1);
    if (destAddress == address(0)) {
      swapToken(pairBnbMimosCoin, reversed ? igtAmount : 0, reversed ? 0 : igtAmount, swapHelperAddress);
      _burn(swapHelperAddress, igtAmount);
      totalBurned += igtAmount;
    } else {
      swapToken(pairBnbMimosCoin, reversed ? igtAmount : 0, reversed ? 0 : igtAmount, destAddress);
    }
    exemptFee[MIMOSCOIN_POOL] = previousExemptFeeState;
  }
...
*/
  ...
  function getMIMOSCOIN_POOL() external view returns(address) { return MIMOSCOIN_POOL; }
