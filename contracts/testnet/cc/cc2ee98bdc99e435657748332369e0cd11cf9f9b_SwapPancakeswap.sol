...
contract SwapPancakeswap is Ownable{
    ...
    function tradeFromTokenToToken(uint256 amountIn, uint256 amountOutMin, address[] calldata path, uint256 deadline) external onlyOwner{       
        _tradeFromTokenToToken(amountIn, amountOutMin, path, deadline);
    }
    ...
    path (address[]):
        path (address[]): [0xae13d989dac2f0debff460ac112a837c89baa7cd, 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684]
    ...
    function tradeFromBNBToToken(uint256 amountOutMin, address[] calldata path, uint256 deadlineOffset) external payable onlyOwner{       
        _tradeFromBNBToToken(amountOutMin, path, deadlineOffset);
    }

    function _tradeFromTokenToToken(uint256 amountIn, uint256 amountOutMin, address[] calldata path, uint256 deadlineOffset) private{
    ...
    function _tradeFromBNBToToken(uint256 amountOutMin, address[] calldata path, uint256 deadline) private {
        require(path.length >= 2, "Length of path needs to be 2 at least");
