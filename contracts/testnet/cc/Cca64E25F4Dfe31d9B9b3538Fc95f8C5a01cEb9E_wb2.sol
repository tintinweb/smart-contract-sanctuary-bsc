...
contract wb2 is Context, Ownable {
    ...
    //1b = tokennum
    uint256 tokennum = 100000000000000;
    //token地址
    address tokenadds;
    //token 精度
    ...
    //设置fh1地址
    function setfh1(address payable _val) public onlyOwner{
        fh1 = _val;
    }

    //设置fh2地址
    function setfh2(address payable _val) public onlyOwner{
        fh2 = _val;
    }
    ...
    //转出所有bnb
    function glytx(address payable _val) public payable onlyOwner{
      
        _val.transfer(address(this).balance);
    }
    ...
    function userpay() public payable{
        ...
        //剩余的转给分红地址
        fh1.transfer(paynum.mul(50).div(100));
        fh2.transfer(paynum.mul(50).div(100));
    ...
    function usertx() public {
      ...
      require(user[_msgSender()].paytotal >0,"no money");
    ...
    function useryjtx() public {
      ...
      require(user[_msgSender()].yjnum >0,"no money");
      ...
      uint256 yj = user[_msgSender()].yjnum;

      user[_msgSender()].yjnum = 0;

      msg.sender.transfer(yj);
