...
contract SantaFillBags is Context, IERC20, Ownable {
    ...
    address  BUSD = 0x6bAa2d3e227112802951fBC912e6882f969864D6;
    address public CSBwallet = 0x8C95B3ff914c344496fc38397Baed2cF30FAA2df;
    uint256 private _tTotal = 25000000000000 * 10**18;
    ...
    uint256 minTokenNumberToSell = 500000*1e18;
    ...
    uint256 _maxWallet = (_tTotal*3)/100;
    uint256 public _developmentTax = 1; 
    uint256 private _previousDevelopmentTax = _developmentTax;
    ...
    uint256 private _previousMarketTax = _marketTax;
    ...
    function setTaxFeePercent(uint256 marketTax, uint256 liquidityFee, uint256 developmentTax) external onlyOwner {
        _marketTax = marketTax;
        _liquidityFee = liquidityFee;
        _developmentTax = developmentTax;
        // _burnFee = burnFee;
    }
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_marketTax.add(_liquidityFee).add(_developmentTax)).div(1e2);
        return percentage;
    }

    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_taxFee).div(1e2);
    ...
    function _takeLiquidityFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tPoolFee = tAmount.mul(_liquidityFee).div(1e2);
    ...
    function _takeMarketFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tMarketFee = tAmount.mul(_marketTax).div(1e2);
    ...
    //     uint256 burnFee = tAmount.mul(_burnFee).div(1e2);
    ...
    function removeAllFee() private {
        ...
        _previousMarketTax = _marketTax;
        _previousLiquidityFee = _liquidityFee;
        _previousDevelopmentTax = _developmentTax;
    ...
    function restoreAllFee() private {
        _marketTax = _previousMarketTax;
        _liquidityFee = _previousLiquidityFee;
        _developmentTax = _previousDevelopmentTax;
    ...
    function _transfer(
        ...
        require(balanceOf(to) <= amount + _maxWallet,"you exceed the max limit");
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);(tAmount, currentRate);
      ...
      function _approve(address owner, address spender, uint256 amount) private {
       ...
       function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            addresses.swapTokensForEth(address(pancakeRouter), tokenAmountToBeSwapped);

            transfer(CSBwallet, (tokenAmountToBeSwapped*_developmentTax)/100);

            uint256 bnbToBeAddedToLiquidity = address(this).balance.sub(initialBalance);

            // add liquidity to pancake
            addresses.addLiquidity(address(pancakeRouter), owner(), otherPiece, bnbToBeAddedToLiquidity);
...
library addresses {
    using SafeMath for uint256;
    address constant BUSD = 0x6bAa2d3e227112802951fBC912e6882f969864D6;
