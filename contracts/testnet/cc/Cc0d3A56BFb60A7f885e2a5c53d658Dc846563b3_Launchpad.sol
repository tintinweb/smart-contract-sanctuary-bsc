...
contract Launchpad is Ownable {
    ...
    function setWhitelists(address token, address[] memory addresses) public {
        ...
        for (uint256 i = 0 ; i < addresses.length ; i++) {
            tokenConfigs[token].whitelisted.push(addresses[i]);
        }
    }
    ...
    function includedInWhite(address token, address addr) private view returns (bool) {
        address [] memory whitelists = tokenConfigs[token].whitelisted;
        for (uint256 i = 0 ; i < whitelists.length ; i++)
            if (whitelists[i] == addr) 
                return true;
        return false;
    }
    ...
    function buy(address token, uint256 rate) external payable {
        ...
        if (tokenConfigs[token].enableWhitelist == true) {
            require(includedInWhite(token, msg.sender));
        }
