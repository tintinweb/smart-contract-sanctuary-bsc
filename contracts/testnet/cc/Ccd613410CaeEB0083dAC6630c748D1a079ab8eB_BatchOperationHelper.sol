...
contract BatchOperationHelper {
    ...
    function batchClaimRewards(address[] calldata contracts, address account_) public {
        uint256 count = contracts.length;
        for (uint256 i = 0; i < count; i++) {
            IClaimRewards(contracts[i]).claimRewards(account_);
        }
    }
    ...
    function batchClaimRewardsAndUnwrap(
        ...
        batchClaimRewards(contracts, account_);
        uint256 count = wrappedContracts.length;
        for (uint256 i = 0; i < count; i++) {
            IClaimRewards(wrappedContracts[i]).claimRewardsAndUnwrap(account_);
        }
    }
}
