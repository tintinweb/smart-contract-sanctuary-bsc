...
contract STM_IDO {
  ...
  function buyShare(address _user,uint256 _amount) external{
        usdt.transfer(feeReceivers,_amount);
        UserInfo storage user = userInfo[msg.sender];
        ...
        require( 1 == user.shareCount, "Limited to one copy");

        user.shareCount = 1;
        user.share = _amount;
        user.shareTime = block.timestamp; 
         _distributeReward(_user,_amount,1);
  ...
  function buyNode(address _user,uint256 _amount) external{
       ...
       usdt.transfer(feeReceivers,_amount);
        ...
        require( 1 == user.nodeCount, "Limited to one copy");
        addHolder(_user);
        user.nodeCount = 1;
        user.node = _amount;
        user.nodeTime = block.timestamp; 
         _distributeReward(_user,_amount,2);
  ...
  function _distributeReward(address _user,uint256 _amount, uint256 _referDepth) private {
