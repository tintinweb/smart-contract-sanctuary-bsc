...
contract ShareBounds is Ownable {
    ...
    event ShareOut(address to,address token,uint256 amount,uint256 timestamp,string data,bytes32 msghash,bytes32 r,bytes32 s,uint8 v);

    constructor(address _funAddress){
        funAddress = _funAddress;
    }

    function shareOut(address to,address token,uint256 amount,uint256 timestamp,string memory data,bytes32 msghash,bytes32 r,bytes32 s,uint8 v) public {
        ...
        string memory signDataStr = string(abi.encodePacked(string(abi.encodePacked("0x",addressToStr(to))),string(abi.encodePacked("0x",addressToStr(token))),uint2str(amount),uint2str(timestamp),data));
        ...
        emit ShareOut( to, token,amount,timestamp, data, msghash, r, s, v);
