...
contract FiveElements is Permission {
    ...
    function getMarquee() public view returns (Order[] memory){
        if(marquee.length >= 10){
            ...
            for(uint i = marquee.length - 1; i > marquee.length - 11; i--){
                newMarquee[i] = marquee[i];
            }
            return newMarquee;
        }else{
            Order[] memory newMarquee = new Order[](marquee.length);
            for(uint i = marquee.length - 1; i > 0; i--){
                newMarquee[i] = marquee[i];
            }
            return newMarquee;
        }
    ...
    function getAllOrderList(address _sender) public view returns (Order[] memory){
        ...
        for(uint i = orderList[_sender].length - 1; i > 0; i--){
    ...
    function allocation() private{
        if(fiveCamps[0].length == 0){
            ...
            transferUsdt([clubAddress, oneAddress, twoAddress], [(ticketPrice * 25 / 1000 ), (ticketPrice * 56 / 1000 ), (ticketPrice * 14 / 1000 )]);
        }else{
    ...
    function luckFive() private {
        ...
        if(count < 8){
            ...
            winBouns = pool * 1 / 1000;
            pushOrderList(newHash, winBouns);
        }else if(count == 9){
            winBouns = pool * 3 / 1000;
            pushOrderList(newHash, winBouns);
        }else if(count == 10){
            winBouns = pool * 5 / 1000;
            pushOrderList(newHash, winBouns);
        }else if(count == 11){
            winBouns = pool * 10 / 1000;
            pushOrderList(newHash, winBouns);
        }else if(count == 12){
            winBouns = pool * 50 / 1000;
            endTime = endTime - 30;
            pushOrderList(newHash, winBouns);
        }else if(count == 13){
        ...
        if(count >= 9){
            marquee.push(Order({orderTime : block.timestamp, orderHash: newHash, winBouns: winBouns}));
        }
    }
    ...
    function pushOrderList(bytes memory newHash, uint winBouns) private {
        ...
        string memory strHahs = string(newHash);
        emit Win(msg.sender, strHahs, winBouns, block.timestamp);
    }
