...
contract StableDAOBondingCalculator is IBondingCalculator {
    using FixedPoint for *;
    using SafeMath for uint256;

    IERC20 internal immutable STBL;

    constructor(address _STBL) {
        require(_STBL != address(0), "Zero address: STBL");
        STBL = IERC20(_STBL);
    }
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(STBL)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(STBL), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(STBL)).decimals())).div(getTotalValue(_pair));
    }
}