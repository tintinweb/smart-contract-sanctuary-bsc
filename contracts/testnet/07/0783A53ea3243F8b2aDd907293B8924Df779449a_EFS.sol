...
contract EFS is IERC20, Ownable {
    ...
    address public MARKETING = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    address public BURN = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    address public SHIBA = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;

    string constant _name = "ETH Fan Shiba";
    string constant _symbol = "$EFS";
    ...
    uint256 public sellShibaBurn = 2;
    ...
    uint256 public shibaBurnSwap = 3;
    ...
    function updateSwapPercentages(
        ...
        burnSwap = burn;
        shibaBurnSwap = shibaBurn;
        totalSwap = reward.add(marketing).add(liquidity).add(burn).add(
            shibaBurn
        );
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 tokensToShiba = contractTokenBalance.mul(shibaBurnSwap).div(
            totalSwap
        );
        ...
        if (tokensToMarketing > 0) {
            swapTokensForTokens(tokensToMarketing, MARKETING);
            uint256 swappedTokensAmount = IERC20(MARKETING).balanceOf(
                address(this)
            );

            try
                IERC20(MARKETING).transfer(
        ...
        if (tokensToShiba > 0) {
            swapTokensForTokens(tokensToShiba, SHIBA);
            uint256 swappedTokensAmount = IERC20(SHIBA).balanceOf(
                address(this)
            );

            try IERC20(SHIBA).transfer(DEAD, swappedTokensAmount) {} catch {}
        }
    ...
    function changeEFTBurnToken(address _tokenAddress) external onlyOwner {
        BURN = _tokenAddress;
    }

    function changeMarketingFeeToken(address _tokenAddress)
        external
        onlyOwner
    {
        MARKETING = _tokenAddress;
    }
