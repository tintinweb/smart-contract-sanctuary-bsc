...
contract sToken is BEP20Detailed, BEP20 {
    using SafeMath for uint256;
  ...
  constructor() BEP20Detailed("s23", "S23", 18) {
    uint256 totalTokens = 50000000 * 10**uint256(decimals());
    _mint(msg.sender, totalTokens);
    sellTax = 5;
    buyTax = 4;
    transferTax = 3;
    marketingPool = 0x0d7C33aeb9bF049Df4080A826fd756d056D3a7A6;
  ...
  function getTrade() external view returns (bool _tradingEnabled, uint256 _tradingTime) {
    return (tradingEnabled, tradingTime);
  }  

  function getLiqudityPool(address _lpAddress) external view returns (bool _status) {
      _status = liquidityPool[_lpAddress];    
    return (_status);
  } 
  ...
  function enableTrading(bool _tradingEnabled) external onlyOwner {
    ...
    if (tradingEnabled) { tradingTime = block.timestamp; }
  }  

  function _getTime() external view returns(uint256 mdata, uint256 tdata, uint256 _ttime) {
    mdata = ((block.timestamp - tradingTime) / 60) % 60;
    tdata = 100 - (mdata / 2);  
    _ttime = tradingTime + 200 minutes;
    return (mdata, tdata, _ttime);
  }

  function __getTime() internal returns(uint256) {
    return 100 - ((((block.timestamp - tradingTime) / 60) % 60) / 2);
  }
  ...
  function _transfer(address sender, address receiver, uint256 amount) internal virtual override {
    ...
    if(liquidityPool[sender] == true) {
      ...
      uint256 ttime = tradingTime + 200 minutes;
      if (ttime >= block.timestamp) { 
          taxAmount = (amount * __getTime()) / 100; 
          } else {
          taxAmount = (amount * sellTax) / 100;  
          }
    } 
