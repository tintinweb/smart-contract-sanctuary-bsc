...
contract DifiFortuneStaking is Initializable{
    ...
    function stake(
        ...
        require(!blackListed[msg.sender], "Cannot Stake Funds! You are blacklisted!");
    ...
    function withdraw(uint256 _index) public {
        ...
        require(!maintenance, "Maintenance in progress, please be patient");
        require(!blackListed[msg.sender], "Cannot withdraw Funds! You are blacklisted!");
    ...
    function reinvest(uint256 _index, uint256 _plan) public {
        ...
        require(_plan < totalPlans, "Invalid Plan!");
        require(user.stakes[_index].isActive, "Stake is not Active");
        require(!blackListed[msg.sender], "Cannot reinvest Funds! You are blacklisted!");
    ...
    function changeExtraBonusPercentage(uint256 _bonusPercentage) external onlyOwner
    {
    ...
    function changePlan(
        ...
        uint256 _stakingIntervalLimits,
        uint256 _stakingPeriods,
        uint256 _minDepositAmount,
        uint256 _maxDepositAmount,
        uint256 _referralPercentages
    ) external onlyOwner returns (bool) {
        require(_plan < totalPlans, "Invalid Plan!");
        require(_maxDepositAmount <= 0, "_maxDepositAmount needs to be bigger than 0");

        plans[_plan].percentPerInterval     = _percentPerInterval;
        plans[_plan].stakingIntervalLimits  = _stakingIntervalLimits;
        plans[_plan].stakingPeriods         = _stakingPeriods;
        plans[_plan].minDepositAmount      = _minDepositAmount;
        plans[_plan].maxDepositAmount      = _maxDepositAmount;
        plans[_plan].referralPercentages    = _referralPercentages;

        return true;
    }

    function addPlan(Plan memory _plan) public onlyOwner returns (bool) {
    ...
    function banOrUnbanWallets(address[] memory _users, bool _ban) external onlyOwner {
        for (uint256 i; i < _users.length; i++) {
            blackListed[_users[ i ]] = _ban;
        }
    }
    ...
    function enableOrDisableMaintenance(bool _disable) external onlyOwner
    {
        maintenance = _disable;
    }
