...
contract SynthrBridge is BaseBridge {
    ...
    modifier onlySynthr() {
        require(
            msg.sender == getAddressFromResolver("Synthetix") ||
    ...
    function _nonblockingLzReceive(
        ...
        if (packetType == PT_SYNTH_MINT) {
            ...
            if (!burnToTarget) {
                ISynthrIssuer(getAddressFromResolver("Issuer")).destBurn(accountForSynth, synthKey, synthAmount, debtShare);
            }
    ...
    function _broadcast(bytes memory lzPayload, uint16 packetType) internal {
        ...
        for (uint ii = 0; ii < length; ii++) {
            ...
            outBoundNonce = ILayerZeroEndpoint(lzEndpoint).getOutboundNonce(dstChainId, address(this));
