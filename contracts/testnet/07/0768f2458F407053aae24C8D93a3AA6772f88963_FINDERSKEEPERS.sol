    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            ...
            if(shouldTakeFee(recipient)) {
                ...
                if(_holders[recipient].exists == false && (_balances[recipient] + amount) >= _minHolderTargetAmount && recipient != address(this) && recipient != pair) {
        ...
        if(recipient == pair) {
            if(_balances[sender] <= _minHolderTargetAmount && sender != address(this) && sender != pair) {
