    ...
    ReentrancyGuardUpgradeable
{
    ...
    /* NFT items */
    uint256 public constant MERCURY = 1;
    uint256 public constant VENUS = 2;
    uint256 public constant EARTH = 3;
    uint256 public constant MARS = 4;
    uint256 public constant JUPITER = 5;
    uint256 public constant SATURN = 6;
    uint256 public constant URANUS = 7;
    uint256 public constant NEPTUNE = 8;

    /* Events */
    event NomineeAdded(address owner, address nominee);
    event OwnerChanged(address newOwner);
    event MintedPlanet(address account, uint256 planetId, uint256 amount);
    ...
    /**
     * @notice function for retrieving the uris of the planetIds
     * @dev override the existing function
     * @param planetId tokenId of the planet
     * @return uri of the corresponding planetId
     */
    function uri(uint256 planetId)
        public
        view
        override(ERC1155Upgradeable)
        returns (string memory)
    {
        _checkPlanetId(planetId);

        return _tokenUris[planetId];
    }
     ...
     * @dev only 8 tokenIds are available corresponding to the planets
     * on the solar system
     * @param account address to which nfts has to be minted
     * @param planetId tokenId which represent the planet
    ...
    function mintNft(
        ...
        require(
            bytes(uri(planetId)).length > 0,
            "SolarSystemNft: Uri does not exist for the planetId"
        );

        // increase total supply for the planet
        _totalSupply[planetId] += amount;

        // mint the tokens
        emit MintedPlanet(account, planetId, amount);
        _mint(account, planetId, amount, "");
    }

    /**
     * @notice function for setting the uri
     * @dev only owner can set the uri
     * @dev uris are set corresponding to the planetId
     * @param planetId tokenId of the planet
     * @param uri_ uri string
     */
    function setURI(uint256 planetId, string memory uri_) external onlyOwner {
        _checkPlanetId(planetId);
        require(bytes(uri_).length > 0, "SolarSystemNft: Invalid uri");
        require(
            bytes(uri(planetId)).length == 0,
            "SolarSystemNft: Uri exist for this planetId"
        );
        require(!_uriExist[uri_], "SolarSystemNft: Duplicate uri");

        _tokenUris[planetId] = uri_;
        _uriExist[uri_] = true;
    }
    ...
    /**
     * @notice function for getting the total supply of the planetIds
     * @param planetId tokenId of the planet
     * @return _totalSupply total number of nfts minted for the particular planet
     */
    function totalSupply(uint256 planetId) external view returns (uint256) {
        return _totalSupply[planetId];
    }
     ...
     * @param planetId tokenId of the nft
     */
    function _checkPlanetId(uint256 planetId) private pure {
        require(
            planetId > 0 && planetId < 9,
            "SolarSystemNft: Invalid planetId"
        );
    }
