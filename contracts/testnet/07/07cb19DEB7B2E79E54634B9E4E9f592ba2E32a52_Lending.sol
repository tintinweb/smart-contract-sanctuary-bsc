...
// File: contracts/borrowcontract.sol
...
contract Lending is ReentrancyGuard, AccessProtected {
    using SafeMath for uint256;
    struct borrowDuration{
        uint256 amount;
        uint256 duration;
        uint256 end;
    }

    mapping(address => borrowDuration) public userBorrows;

    uint256 public durations; //borrow duration
    ...
    function borrow(address token, uint256 amount)
        ...
        moreThanZero(amount)
    {
        borrowDuration storage borrows = userBorrows[msg.sender];
        uint256 end = block.timestamp.add(durations);
        ...
        //insert new borrow
        borrowDuration storage newBorrow = borrows;
        newBorrow.end = end;
        newBorrow.amount = amount+ userBorrows[msg.sender].amount;
        newBorrow.duration = durations;
    ...
    function _repay(
        ...
        uint256 amount
            ...
            require(userBorrows[msg.sender].end>block.timestamp,"Borrow time is over, your cannot repay now ");
    ...
    function maxBorrow() public view returns (uint256 ){
        (uint256 borrowedValueInEth, uint256 collateralValueInEth) = getAccountInformation(msg.sender);
        uint256 amountMax = (collateralValueInEth * LIQUIDATION_THRESHOLD) / 100;
        uint256 amount = amountMax - borrowedValueInEth;
        return amount;
    }

    function withdrawLiquidatedFunds(uint256 _amount) external onlyOwner returns (bool){
        IERC20(s_allowedTokens[0]).transfer(msg.sender, _amount);
        return true;
    }
