...
interface IBCNFT {
    function getAccountLevels(address _account) external view returns (uint256[] memory);
...
contract BCNFTStaking is Context, Ownable {
    using SafeMath for uint256;

    IBCNFT public iBCNFT;
    IBCToken public iBCToken;
    ...
    uint256[] public rewardPercentage = [5, 10, 15, 20];

    mapping (address => Staker) public stakers;
    mapping (address => mapping(uint256 => bool)) public levelOfOwner;
    mapping (address => uint256) public levelCountOfOwner;

    constructor(address nftAddress_, address rewardAddress_) {
        rewardTokenAddress = address(rewardAddress_);
        NFTTokenAddress = address(nftAddress_);

        iBCNFT = IBCNFT(nftAddress_);
        iBCToken = IBCToken(rewardAddress_);
    }

    function stake(uint256 period) external {
        bool isStake = isStakable(msg.sender);
        require(isStake, "Can not Stake");

        stakers[msg.sender].tokenIds = getTokensOfOwner(msg.sender);
        stakers[msg.sender].stakeLevel = setStakeLevel(msg.sender);
        stakers[msg.sender].lastStartTime = _getNow();
        stakers[msg.sender].readyTime = _getNow() + SECONDS_PER_DAY.mul(period);
    ...
    function isStakable (address owner) public returns(bool) {
        ...
        uint256 tokensOfOwner = getTokenCount(owner);
        require(tokensOfOwner > 1, "Must have at least 2 NFTs");
        uint256[] memory levels = iBCNFT.getAccountLevels(owner);
    ...
    function getAccountLevels(address _owner) public view returns(uint256[] memory) {
        return iBCNFT.getAccountLevels(_owner);
    }

    function getAllHolders() public view returns(address[] memory) {
        return iBCNFT.getHolders();
    }

    function getIsHolder(address _owner) public view returns(bool) {
        return iBCNFT.checkNFTHolder(_owner);
    }

    function getTokenCount(address _owner) public view returns(uint256) {
        return iBCNFT.balanceOf(_owner);
    }

    function getTokensOfOwner(address _owner) public view returns(uint256[] memory) {
        return iBCNFT.tokensOf(_owner);
    }
    ...
    function setStakeLevel(address _owner) public returns (uint256) {
        uint256 level = levelCountOfOwner[_owner];
        require(level >=2, "Must have at least 2 NFTs");

        uint _random = random(6);
        uint256 _stakeReward = _random.add(rewardPercentage[level-2]);

        return _stakeReward;
    }

    function getStakeLevel(address _owner) public view returns(uint256) {
        require(stakers[_owner].isStaking, "Must be staked.");
        return stakers[_owner].stakeLevel;
    }

    function getRestTime(address _owner) external view returns(uint256) {
        require(stakers[_owner].isStaking, "Must be staked.");
        return stakers[_owner].readyTime.sub(stakers[_owner].lastStartTime);
    }
    ...
    function setNFTContract() public onlyOwner {
        iBCNFT = IBCNFT(NFTTokenAddress);
    }

    function setRewardContract() public onlyOwner {
        iBCToken = IBCToken(rewardTokenAddress);
    }
    ...
    function claimReward() external returns(bool) {
        ...
        bool result = iBCToken.transfer(rewardTokenAddress, msg.sender, totalToken.mul((stakers[msg.sender].stakeLevel.mul(10)).add(1000).add(stakers[msg.sender].period)).div(10000));
