...
contract VaultInfoBundler is Ownable {
    ...
    IPriceGetter public priceGetterContract;
    ...
    struct VaultPageInfo {
        StakingVaultV600.Controls controls;
        StakingVaultV600.UserInfo1 userInfo1;
        StakingVaultV600.UserInfo2 userInfo2;
        FeeDistributorV6.EntryExitFees entryExitFees;
        IRewardDistributorV6.APYData apyData;
        address adminPayOutWallet;
    ...
    struct VaultListInfo {
        address[4] addresses;
        StakingVaultV600.VaultVariables vaultVariables;
        address token;
        address secondToken;
        StakingVaultV600.Durations durations;
    ...
    constructor(
        address creator_,
        address _priceGetterContract
    ) {
        creator = Creator(creator_);
        priceGetterContract = IPriceGetter(_priceGetterContract);
    }
        ...
        returns (VaultListInfo memory)
    {
        ...
        ) = StakingVaultV600(payable(staking)).getAddresses();
        StakingVaultV600.VaultVariables memory _vaultVariables = StakingVaultV600(
            payable(staking)
        ).getVaultVariables();
        (address _token, address _secondToken) = StakingVaultV600(payable(staking))
            .getTokens();
        StakingVaultV600.Durations memory _durations = StakingVaultV600(
            payable(staking)
        ).getDurations();
        uint256 _pricePerFullShare = StakingVaultV600(payable(staking))
            .getPricePerFullShare();
        uint256 _price;
        try priceGetterContract.getPrice(_token) returns (uint256 _result) {
            _price = _result;
        } catch {
            _price = 0;
        }
        Creator.Level _level = creator.getVaultLevel(staking);
        ...
        returns (VaultPageInfo memory)
    {
        ...
        StakingVaultV600.Controls memory _controls = StakingVaultV600(
            payable(staking)
        ).getAllControls();
        StakingVaultV600.UserInfo1 memory _userInfo1 = StakingVaultV600(
            payable(staking)
        ).getUserInfo1(user, 0);
        StakingVaultV600.UserInfo2 memory _userInfo2 = StakingVaultV600(
            payable(staking)
        ).getUserInfo2(user, 0);
        (, , IFeeDistributorV6 feeDistributorAddress, IRewardDistributorV6 rewardDistributorAddress) = StakingVaultV600(
            payable(staking)
        ).getAddresses();
        FeeDistributorV6.EntryExitFees memory _entryExitFees = FeeDistributorV6(
            payable(feeDistributorAddress)
        ).getEntryExitFees(0);
        IRewardDistributorV6.APYData memory _apyData = IRewardDistributorV6(
            rewardDistributorAddress
        ).getAPYData();
        StakingVaultV600.VaultVariables memory _vaultVariables = StakingVaultV600(
            payable(staking)
        ).getVaultVariables();
        (address token, address secondToken) = StakingVaultV600(payable(staking))
            .getTokens();
        ...
        //(uint256 _userBalance, ) = StakingVaultV600(payable(staking))
