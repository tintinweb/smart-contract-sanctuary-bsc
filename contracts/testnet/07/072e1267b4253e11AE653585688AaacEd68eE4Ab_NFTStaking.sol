...
contract NFTStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        stakingToken = _bep20;
        plans.push(Plan(100 * 10**18, 1, 1652092611, 1653388611));
        plans.push(Plan(100 * 10**18, 2, 1652092611, 1654684611));
        plans.push(Plan(150 * 10**18, 3, 1652092611, 1659868611));
        plans.push(Plan(200 * 10**18, 4, 1652092611, 1662460611));
        nft = _nft;
    }
        ...
        whenNotPaused
    {
        ...
        require(plans[plan].start <= block.timestamp, "Pool not started");
        require(block.timestamp <= plans[plan].finish, "Pool was ended");
    ...
    function _invest(
        ...
        User storage user = users[plan][userAddress];
        ...
        emit NewDeposit(userAddress, plan, _amount, PROJECT_FEE);
    }
    ...
    function unStake(uint256 plan) external payable nonReentrant whenNotPaused {
        require(plans[plan].fixedInvest != 0, "Invalid plan");
        require(msg.value == UNLOCK_FEE, "Required: Pay fee for unlock stake");
        User storage user = users[plan][msg.sender];
        require(!user.isUnStake, "already unstake");
        require(block.timestamp > plans[plan].finish, "Pool was not ended");
        user.isUnStake = true;
        ...
        emit UnStake(msg.sender, plan, user.amount, tokenId);
        if (UNLOCK_FEE > 0) {
    ...
    function updatePlan(uint256 planId, Plan memory plan) external onlyOwner {
        plans[planId] = plan;
    }
    ...
    function getUserInfo(uint256 plan, address userAddress)
        public
        view
        returns (User memory userInfo)
    {
        userInfo = users[plan][userAddress];
    }
