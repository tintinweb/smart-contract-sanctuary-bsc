...
contract Orders is Ownable {
    ...
    function deposit(address _token,uint256 _Famount, uint256 _Tamount) public payable {
        IERC20(_token).transferFrom(msg.sender, address(this), _Tamount);
        counter = counter + 1;
        TDetails[msg.sender] = Tdetails(_token,_Famount,_Tamount,false);
        emit Deposit(msg.sender, _Tamount);
    }

    function withdraw(address _token) public payable onlyOwner {
        uint256 totalAmount = IERC20(_token).balanceOf(address(this));
        IERC20(_token).approve(msg.sender,totalAmount);
    ...
    function getContractBalance(address _token) public view onlyOwner returns (uint256) {
