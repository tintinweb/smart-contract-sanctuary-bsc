...
contract ABLETOK is ERC20, Ownable {
    ...
    ABLETOKDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("ABLETOK", "ABLETOK") {
    	...
    	dividendTracker = new ABLETOKDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ABLETOK: The dividend tracker already has that address");

        ABLETOKDividendTracker newDividendTracker = ABLETOKDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ABLETOK: The new dividend tracker must be owned by the ABLETOK token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ABLETOK: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ABLETOK: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ABLETOK: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ABLETOK: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "ABLETOK: The liquidity wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ABLETOK: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ABLETOK: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "ABLETOK: This account cannot send tokens until trading is enabled");
        }
        ...
        if(isFixedSaleBuy) {
            require(block.timestamp >= fixedSaleStartTimestamp, "ABLETOK: The fixed-sale has not started yet.");
            ...
            if(!openToEveryone) {
                require(fixedSaleEarlyParticipants[to], "ABLETOK: The fixed-sale is only available to certain participants at the start");
            }
...
contract ABLETOKDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("ABLETOK_Dividend_Tracker", "ABLETOK_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "ABLETOK_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "ABLETOK_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ABLETOK contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ABLETOK_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ABLETOK_Dividend_Tracker: Cannot update claimWait to same value");
