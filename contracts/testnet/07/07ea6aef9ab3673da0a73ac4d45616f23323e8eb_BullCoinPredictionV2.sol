...
// File: bullcoin/pred/AggregatorV3Interface.sol
...
// File: bullcoin/pred/Address.sol
...
// File: bullcoin/pred/SafeERC20.sol
...
// File: bullcoin/pred/ReentrancyGuard.sol
...
// File: bullcoin/pred/Context.sol
...
// File: bullcoin/pred/Pausable.sol
...
// File: bullcoin/pred/Ownable.sol
...
// File: bullcoin/pred/contract.sol
 ...
 */
contract BullCoinPredictionV2 is Ownable, Pausable {
    ...
    constructor(
        ...
        BULC = IERC20(address(_bullcointoken));
    ...
    function betBear(uint256 epoch , uint256 amount) external whenNotPaused {
        require(amount > 0 , "The value must be greater than 0");
        uint256 allowance = BULC.allowance(msg.sender , address(this));
    ...
    function _betBear(uint256 epoch, uint256 _amount) internal {
        uint256 balancebefore = BULC.balanceOf(address(this));
        BULC.transferFrom(_msgSender(), address(this), _amount);
     ...
     * @param epoch: epoch
     */
     function betBull(uint256 epoch , uint256 amount) external whenNotPaused {
        require(amount > 0 , "The value must be greater than 0");
        uint256 allowance = BULC.allowance(msg.sender , address(this));
        require(allowance >= amount, "Check the token allowance");
    ...
    function _betBull(uint256 epoch, uint256 _amount) internal {
        uint256 balancebefore = BULC.balanceOf(address(this));
        BULC.transferFrom(msg.sender, address(this), _amount);
        uint256 balanceafter = BULC.balanceOf(address(this));
        _amount = balanceafter - balancebefore ;
    ...
    function claim(uint256[] calldata epochs) external {
        ...
        if (reward > 0) {
            BULC.safeTransfer(address(msg.sender), reward);
        }
    }
    ...
    function claimTreasury() external onlyAdmin {
        ...
        treasuryAmount = 0;
        BULC.safeTransfer(adminAddress, currentTreasuryAmount);
