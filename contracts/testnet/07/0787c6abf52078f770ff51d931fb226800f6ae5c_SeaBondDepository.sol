...
contract SeaBondDepository is OwnableUpgradeable {
    ...
    IERC20Upgradeable public Sea; // token given as payment for bond
    IERC20 public principle; // token used to create bond
    ITreasury public treasury; // mints Time when receives principle
    ...
    IBondCalculator public bondCalculator; // calculates value of LP tokens
    ...
    /* ======== INITIALIZATION ======== */
    function  initialize (address _Sea,  address _principle,  address _treasury, address _DAO, address _bondCalculator)  public initializer
    {
    ...
    function deposit( 
        ...
        uint balanceBefore = Sea.balanceOf(address(this));
        /**
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Sea.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Sea.balanceOf(address(this)), "Not enough Time to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Sea.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Sea.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Sea.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Sea.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        if( isLiquidityBond ) {
            ...
            price_ = bondPrice().mul( 10 ** IERC20_ext(address(principle)).decimals() ) / 100;
        }
    }
    ...
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( address(_token) != address(Sea), "NAT" );
        require( address(_token) != address(principle), "NAP" );
