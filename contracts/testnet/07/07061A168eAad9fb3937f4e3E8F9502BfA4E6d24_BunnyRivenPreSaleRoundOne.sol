...
contract BunnyRivenPreSaleRoundOne is Ownable {
    ...
    uint256 public constant LIMIT_TICKET = 100 ether;
    ...
    function withdrawBunnyRiven(uint256 amount) external {
        ...
        bunnyRivenToken.safeTransferFrom(address(this), msg.sender, amount);
        balance -= amount;
    ...
    function usdtNeedToBuyRemainOf(address account, string calldata ticket)
        ...
        returns (uint256)
    {
        if (_ticketBalances[account][ticket] < LIMIT_TICKET) {
            return
                LIMIT_TICKET
                    .sub(_ticketBalances[account][ticket])
                    .mul(usdtPerToken)
                    .div(1 ether);
        }
    ...
    function buyTokenByUsdt(
        ...
        if (
            _ticketBalances[msg.sender][ticket].add(amountToken) >= LIMIT_TICKET
        ) {
        ...
        emit BuyToken(msg.sender, amountToken, perToken);
    }
        ...
        payable
    {
        ...
        uint256 amountToken = amount.mul(perToken).div(1 ether);
