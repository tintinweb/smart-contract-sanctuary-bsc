...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    */
    ...
    function deposit(uint256 _wantAmt) external virtual onlyOwner whenNotPaused returns (uint256) {
        return deposit(_wantAmt, true);
    }

    // Receives new deposits from depositAuthorised
    function deposit(uint256 _wantAmt, bool _chargeFee) public virtual onlyOwner nonReentrant whenNotPaused returns (uint256) {
    ...
    function unstake(uint256 _wantAmt, bool instantly) public virtual onlyOwner nonReentrant returns (uint256, uint256) {
    ...
    function emergencyWithdraw(uint256 _wantAmt) external virtual onlyOwner returns (uint256, uint256) {
