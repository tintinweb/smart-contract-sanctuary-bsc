...
contract BeeSkyv2 is Ownable {
  ...
  address public USDT = 0x25674b140eCbB27DD125c5C6AF15C708626d04d1;
  address public GLOVERNANCE = 0x9d5b48f7b5CBC7A56740b212F7720a7314F46f3a;
  ...
  function contribution(uint256 _id,address _refer) external noReentrant() returns (bool) {
    ...
    contribute.push(Contribute( block.timestamp, _id, msg.sender, rewardperblock, 0));
    users[msg.sender].cid.push(contribute.length.sub(1));
    return true;
  }

  function getRewardAtTotal(uint _cid) external view returns (uint256) {
    uint256 result = 0;
    do{
      result = result.add(contribute[_cid].blockreward);
    }while(result<getMaxClaimed(_cid));
    return result;
  }

  function getMaxClaimed(uint256 _cid) internal view returns (uint256) {
    IERC20 usd = IERC20(USDT);
  ...
  function claimTopShared(uint256 _month) external returns (bool) {
    ...
    safeusdtransfer(msg.sender,spenderamount);
