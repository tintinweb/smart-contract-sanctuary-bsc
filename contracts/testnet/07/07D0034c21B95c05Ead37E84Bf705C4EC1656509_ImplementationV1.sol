...
contract ImplementationV1 is Implementation {
    ...
    function handleMintRequest(
        ...
        (bool success, ) = mintHandler.call{value: msg.value}(
    ...
    function handleBuyRequest(
        ...
        (bool success,) = buyHandler.call{value: msg.value}(
            abi.encodeWithSignature(
...
contract Upgradeable is ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(msg.sender == this.owner(), "Implementation: Only owner");
        _;
    }

    modifier onlyAdmins() {
        require(
            adminList[msg.sender] || msg.sender == this.owner(),
