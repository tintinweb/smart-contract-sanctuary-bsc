...
contract HexaCron is ERC20Detailed, Auth {
    ...
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(insuranceFee).add(
            firepitFee).add(reflectionFee);
    ...
    constructor() ERC20Detailed("HexaCron", "$HexaC", uint8(DECIMALS)) Auth(msg.sender) {
        ...
        liquidityFund = 0x28C7fe0e700429c63852055e228313FD37f777be;
        treasuryFund = 0x8e4D978099C3c1bb0DfFEC2Fa1Aa8Ee80864216d;
        insuranceFund = 0xe54a8e45Ec3d6d5F4429D360b61AF0EAc4FaEF2d;
        firePit = 0xb0Cf2bE01e3Df0f2E3be55A2cF123d9115Fd3830;
        sellFeeFund = 0xa58b8b07637366A64753f4cBcD116578b235c89E;
        ...
        isDividendExempt[insuranceFund] = true;
        isDividendExempt[firePit] = true;
    ...
    function clearStuckBalance(uint256 amountPercentage) external authorized {
        ...
        payable(treasuryFund).transfer(amountBNB * amountPercentage / 100);
    }

    function addLiquidity() internal swapping {
        uint256 autoLiquidityAmount = _gonBalances[liquidityFund].div(
            _gonsPerFragment
        );
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            _gonBalances[liquidityFund]
        );
        _gonBalances[liquidityFund] = 0;
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalBNBFee = reflectionFee.add(treasuryFee).add(insuranceFee);

        uint256 amountBNBReflection = amountBNB.mul(reflectionFee).div(totalBNBFee);
        uint256 amountBNBTreasury = amountBNB.mul(treasuryFee).div(totalBNBFee);
        uint256 amountBNBFund = amountBNB.mul(insuranceFee).div(totalBNBFee);
        ...
        (success, ) = payable(insuranceFund).call{
            value: amountBNBFund
            ,
            gas: 75000
        }("");
    ...
    function setExchangeAddress(address _addr) external authorized {
