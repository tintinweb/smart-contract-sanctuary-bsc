...
contract BIG {
    ...
    uint256[6] private level4Percents = [300, 200, 200, 100,100, 100];
    uint256[13] private level5Percents = [200, 200, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(block.timestamp > order.unfreeze  && order.isUnfreezed == false && _amount >= order.amount){
                ...
                uint256 staticReward = order.amount.mul(dayRewardPercents).mul(dayPerCycle).div(timeStep).div(baseDivider); // daypercycle = 1 or 15 mul(dayperCycle)
                if(isFreezeReward){
