...
contract ERC721 is Context,ERC165,IERC721,IERC721Metadata,IERC721Enumerable{
    ...
    function _checkOnERC721Received(address from,address to,uint256 tokenId,bytes memory _data) private returns (bool) {
        ...
        bytes memory returndata = to.functionCall(
            abi.encodeWithSelector(IERC721Receiver(to).onERC721Received.selector,_msgSender(),from,
                tokenId,_data),"ERC721: transfer to non ERC721Receiver implementer");
...
contract SSSNFTMarket is ERC721 {
    ...
    uint256 referralfees = 0.1 ether;
    address referral= 0xC5C65074064e5283C9D40403208Af004c8960b95;
    // define nft struct
    struct NFT {
        uint256 tokenId;
        string tokenURI;
    ...
    function mintNFT(string memory _tokenURI,uint256 _price) external payable {
        ...
        require( msg.value == mintingfees,"Price must be equal to listing+minting");
        // mint the token
        ...
        payable(owner).transfer(mintingfees);
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        require( msg.value == referralfees,"Price must be equal to referral");
        ...
        address payable sendTo = nft.currentOwner;
         payable(referral).transfer(referralfees);
