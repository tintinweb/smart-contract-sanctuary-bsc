...
contract BoxNFTBuyRound1 is OwnableUpgradeable, PausableUpgradeable  {
    ...
    mapping(uint256 => string) public refs;
    function initialize() public initializer {
        isActive=true;
        priceFlashSales[11]=203832;   // Random Box
        priceSales[11]=203832;        // Random Box
        saleOffs[11]=50;              // Percent
        limits[11]=1000000;
        __Ownable_init();
    }
     ...
     */
    function initByOwner(BoxNFT _boxNFT, IBEP20 _bep20, uint256 _startFlashSaleTime, uint256 _startTime, uint256 _endTime) public onlyOwner {
...
function buyBox(uint256 rank, uint256 _bep20, string memory _ref) external  whenNotPaused returns (uint256) {
        uint256 current=block.timestamp;
        uint256 newBoxId=0;
        require(isActive==true,'Round not active');
        require(_bep20!=0,'value must be greater than 0');
        require(rank==11,'Rank must be 11');
        // Flash Sale
        ...
        uint256 id= boxNFT.buyBox(msg.sender,rank);
        refs[newBoxId] = _ref;
        return id;
    }
...
function removeIndexOfWhitelist(address _whitelist) public onlyOwner { 
  ...
  function getTotalRefByKey(string memory _ref) public view returns(uint256 ){
        uint index = _totalBuy.current();
        uint total=0;
        for(uint i=1; i <= index; i++){
          if(keccak256(abi.encodePacked((refs[i]))) == keccak256(abi.encodePacked((_ref)))){
            total=total+1;
          }
        }
        return total;
