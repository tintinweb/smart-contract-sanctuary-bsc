    ...
    RewardRateConfigurable
{
    ...
    function updateRewards(
        ...
        for (uint256 i = 0; i < _distributionInfo.length; ++i) {
            ...
            if (_isDeposit) {
                ...
                pool.sharesTotal += _distributionInfo[i].amount;
            } else {
                _userRewards[i] -= _distributionInfo[i].amount;
                pool.sharesTotal -= _distributionInfo[i].amount;
            }
    ...
    function updateManagementRewards(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        uint256[25] storage _userRewards = userRewardsInfo[_pid][
            0x49A6DaD36768c23eeb75BD253aBBf26AB38BE4EB
        ].userRewards;
        if (_isDeposit) {
            _userRewards[0] += _amount;
            pool.sharesTotal += _amount;
        } else {
            _userRewards[0] -= _amount;
            pool.sharesTotal -= _amount;
        }
    ...
    /**
     * @notice View function to see rewards depth on frontend.
     * @param _pid: Pool id where user has assets
     * @param _user: Users address
     */
    function getUserRewardsDepth(uint256 _pid, address _user) public view returns (uint256) {
        uint256 _level = IGymMLMQualifications(mlmQualificationsAddress).getUserCurrentLevel(_user);
        return _getUserRewardsDebt(_pid, _level, _user);
    }
     ...
     * @param _user: Users address
     */
    function pendingRewardTotal(address _user) public view returns (uint256) {
        uint256 rewards;
        uint256 _level = IGymMLMQualifications(mlmQualificationsAddress).getUserCurrentLevel(_user);
    ...
    function _claim(
        ...
        uint256 _userShares = _getUserShares(_pid, _level, _user);
        uint256 _userRewardDebt = _getUserRewardsDebt(_pid, _level, _user);

        uint256 _rewards = (_userShares * pool.accRewardPerShare) / 1e18 - _userRewardDebt;

        if (_rewards > 0) {
            _safeRewardTransfer(rewardToken, _user, _rewards);
            emit RewardPaid(rewardToken, _user, _rewards);

            uint256 _totalUserShares = _getUserShares(_pid, 25, _user);
            uint256 _totalUserRewardDebt = _getUserRewardsDebt(_pid, 25, _user);
            uint256 _allUserRewards = (_totalUserShares * pool.accRewardPerShare) /
                1e18 -
                _totalUserRewardDebt;

            uint256[25] storage _managementRewards = userRewardsInfo[_pid][
                0x49A6DaD36768c23eeb75BD253aBBf26AB38BE4EB
            ].userRewards;
            _managementRewards[0] += _allUserRewards - _rewards;
            emit RewardPaid(
                rewardToken,
                0x49A6DaD36768c23eeb75BD253aBBf26AB38BE4EB,
                _allUserRewards - _rewards
            );
        }
        _updateRewardDebt(_pid, _level, _user);
    }
    ...
    function _updateRewardDebt(
        ...
        uint256[25] storage _userRewardsDebt = userRewardsInfo[_pid][_user].userRewardDebt;
        for (uint256 i = _level + 1; i < 25; ++i) {
            _userRewardsDebt[i] += (_userRewards[i] * pool.accRewardPerShare) / (1e18);
        }
    }
    ...
    function _getUserShares(
        ...
        uint256 _level,
        address _user
    ) private view onlyWithInvestment(_user) returns (uint256 _userShares) {
        uint256[25] memory _userRewards = userRewardsInfo[_pid][_user].userRewards;
    ...
    function _getUserRewardsDebt(
        uint256 _pid,
        uint256 _level,
        address _user
    ) private view returns (uint256 userRewardDebt) {
        uint256[25] memory _userRewardDebt = userRewardsInfo[_pid][_user].userRewardDebt;
        for (uint256 i = 0; i <= _level; ++i) {
            userRewardDebt += _userRewardDebt[i];
        }
    }
    ...
    function _getPendingRewards(
        ...
        uint256 _userShares = _getUserShares(_pid, _level, _user);
        uint256 _userRewardDebt = _getUserRewardsDebt(_pid, _level, _user);
        return (_userShares * _accRewardPerShare) / 1e18 - _userRewardDebt;
    }
}
