...
contract Helper is HelperBase {
    ...
    function getActiveLoans(
        ...
        external
        view
        returns (CoreBase.Loan[] memory activeLoans, ActiveLoanInfo[] memory activeLoanInfos)
    {
        IAPHCore iAPHCore = IAPHCore(aphCoreAddress);
        require(start > 0, "APHCore/start-loan-id-minimum-is-zero");
        uint256 loanIndex = iAPHCore.currentLoanIndex(nftId);
        require(stop <= loanIndex, "APHCore/stop-loan-id-must-less-than-equal-loanIndex");
        activeLoans = new CoreBase.Loan[](loanIndex);
        activeLoanInfos = new ActiveLoanInfo[](loanIndex);
        uint256 count = 0;
        for (uint256 i = start; i <= stop; i++) {
            if (iAPHCore.getLoanExt(nftId, i).active) {
                activeLoans[count] = iAPHCore.loans(nftId, i);

                activeLoans[count].interestOwed +=
                    ((block.timestamp - activeLoans[count].lastSettleTimestamp) *
                        activeLoans[count].owedPerDay) /
                    1 days;

                activeLoanInfos[count].id = i;

                activeLoanInfos[count].currentLTV = iAPHCore.getLoanCurrentLTV(i, nftId);
                ...
                activeLoanInfos[count].apr =
                    (activeLoans[count].owedPerDay * 365 * WEI_PERCENT_UNIT) /
        ...
        loanIndex = loanIndex - count;
        assembly {
            mstore(activeLoans, sub(mload(activeLoans), loanIndex))
        }
        assembly {
            mstore(activeLoanInfos, sub(mload(activeLoanInfos), loanIndex))
        }

        return (activeLoans, activeLoanInfos);
    }
    ...
    function calculateBorrowingInterest(
        ...
        require(borrowTokenAddress != address(0), "Helper/invalid-pool-address");

        uint256 interestRate = IAPHPool(poolAddress).getNextBorrowingInterest(0);

        interest = ((borrowingAmount * interestRate * daySecond) /
            (WEI_PERCENT_UNIT * 365 * 86400));

        (uint256 rate, uint256 precision) = IPriceFeed(iAPHCore.priceFeedAddress()).queryRate(
        ...
        returns (uint256 minimumCollateral, uint256 removableCollateral)
    {
        ...
        (uint256 settledBorrowAmount, , uint256 rate, uint256 precision) = _getSettleBorrowInfo(
            iAPHCore,
            loan
        );
        ...
        returns (uint256 maximumBorrowAmount)
    {
        ...
        (, uint256 settledLTV, uint256 rate, uint256 precision) = _getSettleBorrowInfo(
            iAPHCore,
            loan
        );
        if (loanConfig.maxLTV > settledLTV) {
            maximumBorrowAmount =
                ((loanConfig.maxLTV - settledLTV) * loan.collateralAmount * rate) /
                (WEI_PERCENT_UNIT * precision);
        }
    }

    function getSettleBorrowInfo(uint256 nftId, uint256 loanId)
        public
        view
        returns (
            uint256,
    ...
    function _getSettleBorrowInfo(IAPHCore iAPHCore, CoreBase.Loan memory loan)
        private
        view
        returns (
        ...
        )
    {
        ...
        settledBorrowAmount =
            loan.borrowAmount +
            Math.max(loan.minInterest, loan.interestOwed) +
            (((block.timestamp - loan.lastSettleTimestamp) * loan.owedPerDay) / 1 days);
    ...
    function getStakePoolNextSettleTimeStamp(address poolAddress) public view returns (uint256) {
        uint256 poolNextSettle = block.timestamp -
            ((block.timestamp - IStakePool(poolAddress).poolStartTimestamp()) %
                IStakePool(poolAddress).settleInterval()) +
            IStakePool(poolAddress).settleInterval();
...
contract CoreBase is AssetHandler, Manager, ReentrancyGuard, SelectorPausable {
...
import "../../externalContract/openzeppelin/SafeERC20.sol";
...
contract PoolBase is AssetHandler, Manager, ReentrancyGuard, SelectorPausable {
    struct Lend {
        uint8 rank;
        uint64 updatedTimestamp;
    }
