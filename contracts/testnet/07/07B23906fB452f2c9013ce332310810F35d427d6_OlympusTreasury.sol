...
contract OlympusTreasury is Ownable {
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        require( isReserveToken[ _token ] || isLiquidityToken[ _token ], "isReserveToken or isLiquidityToken Not accepted" );
        IERC20( _token ).safeTransferFrom( msg.sender, address(this), _amount );

        if ( isReserveToken[ _token ] ) {
            require( isReserveDepositor[ msg.sender ], "isReserveDepositor Not approved" );
        } else {
            require( isLiquidityDepositor[ msg.sender ], "isLiquidityDepositor Not approved" );
        }
    ...
    function withdraw( uint _amount, address _token ) external {
        require( isReserveToken[ _token ], "isReserveToken Not accepted" ); // Only reserves can be used for redemptions
        require( isReserveSpender[ msg.sender ] == true, "isReserveSpender Not approved" );
    ...
    function incurDebt( uint _amount, address _token ) external {
        require( isDebtor[ msg.sender ], "isDebtor Not approved" );
        require( isReserveToken[ _token ], "isReserveToken Not accepted" );
    ...
    function manage( address _token, uint _amount ) external {
        if( isLiquidityToken[ _token ] ) {
            require( isLiquidityManager[ msg.sender ], "isLiquidityManager Not approved" );
        } else {
            require( isReserveManager[ msg.sender ], "isReserveManager Not approved" );
        }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        require( isRewardManager[ msg.sender ], "isRewardManager Not approved" );
