...
contract INFLIVFarm is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable{
	...
	function activePackage(uint256 package, address sponsor) external nonReentrant{
	...
	function deposit(uint256 amount, uint256 stakingID) external nonReentrant {
	...
	function withdraw(uint256 stakingID, uint256[] calldata ids) external nonReentrant{
	    if(mapUserInfo[msg.sender][stakingID].amount > 0) 
		{
			uint256 pending = pendingReward(msg.sender, stakingID);
			uint256 amount;
	...
	function updatePool(uint256 amount) external nonReentrant{
