...
contract EasyStaking is AccessControl, ReentrancyGuard {
     ...
     */
    function makeForcedWithdrawal(uint256 _depositId, uint256 _amount)
        external
    {
        ...
        external
    {
        ...
        require(
            timestamp < lockEnd.add(withdrawalUnlockDuration()),
            "too late"
        );
     ...
     */
    function setWithdrawalLockDuration(uint256 _value)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     * @param _value The new duration value (in seconds).
     */
    function setWithdrawalUnlockDuration(uint256 _value)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function setTotalSupplyFactor(uint256 _value)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_value <= 1e6, "should be less than or equal to 1 ether");
     ...
     */
    function setSigmoidParameters(
        uint256 _a,
        int256 _b,
        uint256 _c
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _a <= MAX_EMISSION_RATE.div(2),
            "should be less than or equal to a half of the maximum emission rate"
        );
     ...
     */
    function setLiquidityProvidersRewardAddress(address _address)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function liquidityProvidersRewardAddress() public view returns (address) {
        AddressParam memory param = liquidityProvidersRewardAddressParam;
        return
            _paramUpdateDelayElapsed(param.timestamp)
                ? param.newValue
                : param.oldValue;
    }
     ...
     * @return total Total accrued emission (for the user and Liquidity Providers), userShare user share, timePassed and seconds passed since the previous deposit started.
     */
    function getAccruedEmission(uint256 _depositDate, uint256 _amount)
        public
        view
        returns (
            uint256 total,
    ...
    function _deposit(
        ...
        if (sigmoidParamA == 0 && totalSupplyFactor() == 0)
            revert("emission stopped");
    ...
    function _withdraw(
        ...
        (uint256 accruedEmission, uint256 timePassed) = _mint(
            _sender,
            _id,
            _amount
        );
    ...
    function _mint(
        ...
        (
            ...
            uint256 userShare,
            uint256 timePassed
        ) = getAccruedEmission(depositDates[_user][_id], amount);
        if (total > 0) {
            // Note No mint, use reserves
     ...
     */
    function _updateUintParam(UintParam storage _param, uint256 _newValue)
        internal
    {
     ...
     */
    function _getUintParamValue(UintParam memory _param)
        internal
        view
        returns (uint256)
    {
        return
            _paramUpdateDelayElapsed(_param.timestamp)
                ? _param.newValue
                : _param.oldValue;
    }
