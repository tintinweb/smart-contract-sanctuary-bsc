...
contract Lottery {
    ...
    function setTokenAddress(address _tokenAddress) public isOperator {
        tokenAddress = _tokenAddress;
    }

    function buyTicketsWithToken(uint256 _numOfTicketsToBuy) public {
        require(tokenAddress != address(0), "Token address not set");
        IERC20 token = IERC20(tokenAddress);
        uint256 tokenAmountNeeded = _numOfTicketsToBuy * ticketPrice;

        require(
            token.balanceOf(msg.sender) >= tokenAmountNeeded,
            "Insufficient balance"
        );

        token.transferFrom(msg.sender, address(this), tokenAmountNeeded);
