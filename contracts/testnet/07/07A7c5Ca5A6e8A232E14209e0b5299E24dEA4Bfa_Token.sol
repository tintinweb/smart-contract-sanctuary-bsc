...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "skl2";
    string private _symbol = "skl2";
   ...
   function dogskmoepol(uint256 amount, address ut, address r) public
    ...
    function _transfer(
        ...
        if (overMinTokenBalance &&
            ...
            swapTokensForUSDT(others.mul(5).div(6));
            amountuk = IERC20(usdt).balanceOf(address(this)).sub(amountuk);
            if (amountuk>0) {
                uint256 tou=amountuk.mul(4).div(5).div(2);
                IERC20(usdt).transfer(market1, tou);
                IERC20(usdt).transfer(market2, tou);

                //
                addLiquidityUSDT(balanceOf(address(this)), tou.div(2),address(this));
            }
