...
contract WethEthConverter is Ownable{
    ...
    function withdrawAndSendToSender(uint256 _value) external onlyOwner { 
        IWETH(WETH).withdraw(_value);
        (bool success, ) = owner().call{value: _value}(new bytes(0));
        require(success, 'TransferHelper::safeTransferETH: ETH transfer failed');
    } 

    function withdrawAndSendToSenderHelper1(uint256 _value) external onlyOwner { 
        IWETH(WETH).withdraw(_value);
        TransferHelper.safeTransferETH(owner(), _value);
    }

    function withdrawAndSendToSenderHelper2(uint256 _value) external onlyOwner { 
        IWETH(WETH).withdraw(_value);
        TransferHelper.safeTransferETH(payable(owner()), _value);
    }
