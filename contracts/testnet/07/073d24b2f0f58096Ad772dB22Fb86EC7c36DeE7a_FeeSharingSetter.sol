...
import "./VotingFeeDistributor.sol";
...
contract FeeSharingSetter is Ownable, ReentrancyGuard {
    ...
    struct Reward {
        ...
        VotingFeeDistributor votingDistributor;
    }

    Reward[] public rewards;

    event UpdateFeeRewards(uint256 indexed block, string rewardTokenName, uint256 staking, uint256 voting);
    event RewardWithdraw(uint256 indexed block, string rewardTokenName, uint256 amount);
    event RewardUpdate(string rewardTokenName, uint256 stakeRatio, uint256 votingRatio);
    ...
    function addReward(
        ...
        uint128 _stakeRatio,
        uint128 _votingRatio,
        address _stakingDistributor,
        address _votingDistributor
    ) external onlyOwner {
        require(_stakeRatio + _votingRatio == 100, "");
        require(_rewardToken != address(0), "");
        require(!exists(_rewardToken), "");

        StakingRewardDistributor(_stakingDistributor).addRewardToken(_rewardToken);
        rewards.push(Reward(_rewardName, IERC20(_rewardToken), _stakeRatio, _votingRatio, StakingRewardDistributor(_stakingDistributor), VotingFeeDistributor(_votingDistributor)));
    }

    function exists(address _rewardToken) public view returns (bool) {
        for (uint256 index = 0; index < rewards.length; index++) {
            if (address(rewards[index].rewardToken) == _rewardToken) {
                return true;
            }
        }
        return false;
    ...
    function updateRewards() external payable onlyOwner nonReentrant {
        for (uint256 index = 0; index < rewards.length; index++) {
            ...
            emit UpdateFeeRewards(block.number, reward.rewardName, stakeAmount, votingAmount);
        }
    }
    ...
    function _splitAndTransferReward(Reward memory reward) internal returns (uint256 stakeAmount, uint256 votingAmount) {
        ...
        require(balance > 0, "Setter: not sufficient funds");
    ...
    function setStakeRewardRadio(
        address token,
        uint128 _stakeRatio,
        uint128 _votingRatio
    ) external onlyOwner {
        require(_stakeRatio + _votingRatio == 100, "The sum must be equal 100");

        for (uint256 index = 0; index < rewards.length; index++) {
            if (address(rewards[index].rewardToken) == token) {
                rewards[index].stakeRatio = _stakeRatio;
                rewards[index].votingRatio = _votingRatio;

                emit RewardUpdate(rewards[index].rewardName, _stakeRatio, _votingRatio);
            }
        }
    }
}
...
contract StakingRewardDistributor is ReentrancyGuard, Ownable {
     ...
     */
    function updateRewards(address token, uint256 reward) external onlyOwner {
    ...
    function _calculatePendingRewards(address user, address token) internal view returns (uint256) {
        return ((userInfo[user].shares * (_rewardPerToken(token) - (userInfo[user].userRewardPerTokenPaid[token]))) / PRECISION_FACTOR) + userInfo[user].rewards[token];
    }
    ...
    function _rewardPerToken(address token) internal view returns (uint256) {
        ...
        return rewardPerTokenStored[token] + ((_lastRewardBlock() - lastUpdateBlock) * (currentRewardPerBlock[token] * PRECISION_FACTOR)) / totalShares;
    }
 ...
 * @notice It distributes GLR tokens with rolling Merkle airdrops.
 */
contract VotingFeeDistributor is Pausable, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;
    ...
    function withdrawTokenRewards(uint256 amount) external onlyOwner whenPaused {
        ...
        require(success, "Rewards: Transfer Fee failed");
