...
contract StakePoolBNBForToken is Ownable {
    using SafeMath for uint256;
    ...
    function deposit(uint256 _amount) public payable{
        ...
        // amount in wei
        require(_amount==msg.value, "amount is not correct");
    ...
    function withdraw() public {
        ...
        if (user.amount > 0) {
            _msgSender().transfer(user.amount);
...
contract FactoryStakingPool{
    ...
    function createBNBForToken(address ownerAddress,  IBEP20 _rewardingToken, address _rewardingTokenWallet,uint256 _rewardPerBlock, uint256 _lockPeriod, uint256 _maxReward, bool _withUpdate) public returns (address)
    {
        StakePoolBNBForToken toDeploy = new StakePoolBNBForToken(_rewardingToken, block.number, _rewardPerBlock, _rewardingTokenWallet, _lockPeriod);
