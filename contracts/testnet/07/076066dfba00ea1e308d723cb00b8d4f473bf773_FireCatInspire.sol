...
contract FireCatInspire is IFireCatInspire, ModifyControl, FireCatTransfer {
    ...
    mapping(address => uint256) public yieldReward;
    mapping(address => uint256) public yieldRewardClaimed;
    mapping(address => uint256) public nftLevelUpReward;
    mapping(address => uint256) public nftLevelUpRewardClaimed;
    ...
    function yieldRewardOf(address user_) public view returns (uint256, uint256, uint256) {
        uint256 totalReward = yieldReward[user_];
        uint256 userClaimed = yieldRewardClaimed[user_];
        return (
    ...
    function nftLevelUpRewardOf(address user_) public view returns (uint256, uint256, uint256) {
        uint256 totalReward = nftLevelUpReward[user_];
        uint256 userClaimed = nftLevelUpRewardClaimed[user_];
        return (
            totalReward.sub(userClaimed),
            userClaimed,
            totalReward
    ...
    function yieldBootsShare(address user_, uint256 amount_) external onlyRole(FIRECAT_GATE) returns (uint256) {
        ...
        if (fireCatYieldBoots.stakedOf(inviter) > userStaked) {
            ...
            yieldReward[inviter] = yieldReward[inviter].add(actualInviterReward);
        }
        ...
        if (superior != address(0) && fireCatYieldBoots.stakedOf(superior) > userStaked) {
            ...
            yieldReward[superior] = yieldReward[superior].add(actualSuperiorReward);
        } else {
            actualSuperiorReward = doTransferIn(rewardToken, msg.sender, superiorReward);
    ...
    function nftLevelUpShare(address user_, uint256 tokenId_, uint256 amount_) external onlyRole(FIRECAT_GATE) returns (uint256) {
        ...
        if (fireCatNFTStake.isStaked(inviter)) {
            ...
            if (inviterTokenLevel >= userTokenLevel) {
                actualInviterReward = doTransferIn(rewardToken, msg.sender, getReward(inviterRate, amount_));
                nftLevelUpReward[inviter] = nftLevelUpReward[inviter].add(actualInviterReward);
            }
        }
        ...
        if (superior != address(0)) {
            ...
            if (fireCatNFTStake.isStaked(superior)) {
                ...
                if (superiorTokenLevel >= userTokenLevel) {
                    actualSuperiorReward = doTransferIn(rewardToken, msg.sender, superiorReward);
                    nftLevelUpReward[superior] = nftLevelUpReward[superior].add(actualSuperiorReward);
                }
            }
    ...
    function claim() external beforeClaim nonReentrant returns (uint256) {
        (uint256 _yieldReward,,) = yieldRewardOf(msg.sender);
        (uint256 _nftLevelUpReward,,) = nftLevelUpRewardOf(msg.sender);
        uint256 availableClaim = _yieldReward.add(_nftLevelUpReward);
        require(availableClaim > 0, ":");

        IERC20(rewardToken).approve(msg.sender, availableClaim);
        uint256 actualYieldClaimed = doTransferOut(rewardToken, msg.sender, _yieldReward);
        uint256 actualNftLevelUpRewardClaimed = doTransferOut(rewardToken, msg.sender, _nftLevelUpReward);
        uint256 actualClaimedAmount = actualYieldClaimed.add(actualNftLevelUpRewardClaimed);

        yieldRewardClaimed[msg.sender] = yieldRewardClaimed[msg.sender].add(actualYieldClaimed);
        nftLevelUpRewardClaimed[msg.sender] = nftLevelUpRewardClaimed[msg.sender].add(actualNftLevelUpRewardClaimed);
