...
contract FactoryStakingInto20 is AccessControl {
    ...
    function createStaking20Into20(IERC20 _stakeToken, IERC20Metadata _rewardToken, uint256 _startTime, uint256 _endTime, uint256 _rewardPerSecond, uint256 _penaltyPeriod, uint16 _feePercentage) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        instance.setFeeReceiver(_msgSender());
        IERC20(_rewardToken).safeTransferFrom(_msgSender(), address(instance), (_endTime - _startTime) * _rewardPerSecond);
        emit NewContract(address(instance));
    }

    function createStaking20Plus20Into20(IERC20[2] memory _stakeToken, IERC20Metadata _rewardToken, uint256[2] memory _proportion, uint256 _startTime, uint256 _endTime, uint256 _rewardPerSecond, uint256 _penaltyPeriod, uint16 _feePercentage) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
contract Staking20Plus20Into20 is ReentrancyGuard {
    ...
    constructor(IERC20[2] memory _stakeToken, IERC20Metadata _rewardToken, uint256[2] memory _proportion, uint256 _startTime, uint256 _endTime, uint256 _rewardPerSecond, uint256 _penaltyPeriod, uint16 _feePercentage) {
