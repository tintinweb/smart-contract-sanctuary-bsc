...
contract UFMarketplace is ERC721, Ownable, Pausable {
    ...
    function modifyMarketItem(
        ...
        MarketItem storage currentItem = marketItems[tokenId][conAdd];
        MarketItem memory _currentItem = marketItems[tokenId][conAdd];
        {
            // scope to avoid stack too deep errors
            require(
                _currentItem.tokenId == tokenId,
                "Modify Item: Item doesn't match."
            );
            require(
                _currentItem.contractAdd == conAdd,
                "Modify Item: Contract Address doesn't match."
            );
            require(
                _currentItem.listed,
                "Modify Item: Need to list item First."
            );
            require(
                _currentItem.owner == address(this),
                "Modify Item: Item is not listed to this marketplace."
            );
            require(
                _currentItem.seller == _msgSender(),
                "Modify Item: You are not the rightful owner."
            );
        }
        require(price > 0, "Create Item: Price must be more than 1Wei.");
        checkIfContract(tokenAddress);
     ...
     * @param contractAdd contract address of the NFT that will be listed/transferred
     * @param tokenId tokenId of the NFT that will be listed/transferred
     */
     ...
     * @param contractAdd contract address of the NFT for which bid to be added
     * @param tokenId tokenId of the NFT for which bid to be added
     * @param bid bid amount of the NFT
     ...
     * @param contractAdd contract address of the NFT for which bid to be accepted
     ...
     * @param tokenAddress ERC20 token address which they want to cancel bid for
     */
    function cancelBid(
        address contractAdd,
        uint256 tokenId,
     ...
     * @return marketItems which is a struct which keeps the data of a particular NFT
     */
    function showBid(
        address contractAdd,
        uint256 tokenId
    ...
    /**
     * @notice ipdates the listing price of the particular NFT
