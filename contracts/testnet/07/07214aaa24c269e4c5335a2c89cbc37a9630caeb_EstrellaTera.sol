...
contract EstrellaTera is Ownable, Pausable, ReentrancyGuard{
    ...
    uint256 private endcycles= 100;  /// Change
    // uint256 private endcycles= 5;  /// Change
    uint256 public referDepth = 10;
    uint256 public fixedPrice = 1e18;
    uint256 public tokenPrice = 1e18;

    uint256 private percentage70 = 70;
    uint256 private percentage30 = 30;

    uint256 public minDeposit = 10e18;
    uint256 public maxDeposit = 100e18; // Change
    uint256 public cycleSupply = 100e18; // change
    uint256 public roundSupply = 10000e18; //change
    // uint256 public maxDeposit = 1000e18; // Change
    // uint256 public cycleSupply = 1000e18; // change
    // uint256 public roundSupply = 5000e18; //change
    ...
    constructor()
    {
        ...
        ETAToken = IERC20(0xc0c01E11f85E4B022A7ADF46DD91D3eD512e2Dc3);
        defaultRefer = 0xC353bC8E1C4d3C6F4870D83262946E8C32e126b3;
        offchain = 0xB4E9A91c810d4e3feF0b4f336f41E6F470e098da;
        round = 0;
        cycle[0] = 0;
    }
        ...
        if(roundbool == false)
        {
            ...
            { 
                ...
                if(IERC20(tokenAddress) == USDTToken)
                {
                    ...
                    USDTToken.transferFrom(msg.sender, owner(), Percentage1);
                    USDTToken.transferFrom(msg.sender, offchain, Percentage2);
            ...
            if(rounndDetails[2] > 1)
            {
                ...
                while(remainingbuyerToken > 0)
                {
                    ...
                    trasnferAmount(tokenAddress,SellerAddress,totalamount,remainingbuyerToken);
                    ...
                    else
                    {
                        ...
                        trasnferAmount(tokenAddress,SellerAddress,totalamount,TokenBuy_User[0]);
    ...
    function trasnferAmount(address _tokenAddress, address _SellerAddress,uint256 _totalamount,uint256 _tokentransfer) 
    private{
        _totalamount = _totalamount.div(1e18);
        (uint256 Percentage1,uint256 Percentage2) = percentage(_totalamount);
        if(IERC20(_tokenAddress) == USDTToken)
        {
            USDTToken.transferFrom(msg.sender, _SellerAddress, Percentage1);
            USDTToken.transferFrom(msg.sender, offchain, Percentage2);
            // ETAToken.transferFrom(owner(),address(this),_tokentransfer);
        }else if(IERC20(_tokenAddress) == USDACEToken)
        {
            // USDACEToken.transferFrom(msg.sender, _SellerAddress, Percentage1);
            // USDACEToken.transferFrom(msg.sender, offchain, Percentage2);
            // ETAToken.transferFrom(owner(),address(this),_tokentransfer);
        }    
    }
    ...
    public{
        uint256 _total = (TotalUSDSpent[msg.sender].sub(TotalUSDTreceived[msg.sender])).mul(3);
    ...
    function updatebalance(address _user) private{
        ...
        {
            for(uint256 j = 0 ; j <= seller_Count[round] ; j++)
            {
        ...
        SellTotalToken[_user] = 0;
        
    }
///////////////////////USDT TOken/////////////////////////////////////////
    function withdrawUSDTToken(uint256 _count)
    public
    onlyOwner
    {   USDTToken.transfer(owner(),_count);   }

    function emergancyWithdrawUSDTToken()
    public
    onlyOwner
...
function withdrawTeraToken(uint256 _count)
    public
    onlyOwner
    {   ETAToken.transfer(owner(),_count);   }

    function emergancyWithdrawTeraToken()
    public
    onlyOwner
    {   ETAToken.transfer(owner(),ETAToken.balanceOf(address(this)));  }
///////////////////////////////////////////////////////////////////////////////
