...
contract P2PExchangeContract is Ownable, Pausable {
    ...
    event SetChainBSCFee(uint256 chainBSCFee);
    ...
    //set claim fee
    function setChainBSCFee(uint256 value) external virtual onlyOwner {
        chainBSCFee = value;
        emit SetChainBSCFee(chainBSCFee);
    }
    ...
    //create new order
    function createNewOrder(OrderData memory data) external payable virtual {
        ...
        require(msg.value >= chainBSCFee, "don't enought fee!");
        ...
        if(chainBSCFee != 0) {
            payable(msg.sender).transfer(chainBSCFee);
        }
        order[data.id] = data;
    ...
    //cancel order 
    function cancelOrder(string memory id) external payable virtual {
