...
contract DragonNFT is ERC721Enumerable, Ownable {
    ...
    address public RewardToken = 0x2496Bb0214F05333fa4574B1357b2873853b5121;
    address public FeeReceiver = 0xBa5e48dC8760135602EBFE21072c0DCCe54Dac7e;
    ...
    function mint(uint256 _mintAmount) public payable {
        // require
        require(!paused, "DragonNFT: minting has not started yet");
        require(_mintAmount > 0, "DragonNFT: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "DragonNFT: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "DragonNFT: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "DragonNFT: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "DragonNFT: zero address");
        require(_amount > 0, "DragonNFT: you have to mint at least one");
        require(lastSupply != 0, "DragonNFT: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "DragonNFT: URI query for nonexistent token");
