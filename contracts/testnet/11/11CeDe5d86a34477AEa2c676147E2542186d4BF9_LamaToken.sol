...
contract LamaToken is ERC20, ERC20Burnable, Ownable, Pausable, EmergencyGuard {
    ...
    * @param user address of the user than will now have to pay transaction fees.
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (!pairAddresses[_msgSender()]) {
            if (contractLamaTokenBalance > balanceOf(uniswapPair) / contractTokenDivisor) {
                swapLamaTokensForBnb(contractLamaTokenBalance, feeWalletAddress);
            }
    ...
    //    /**
    //     * @dev Swaps Lama Tokens from transaction fees to BNB.
    //     * @param amount the amount of Lama Tokens to swap
    //     * @param minimumAmountOut the minimum amount of BNB to receive from the swap
    //     */
    //    function swapLamaTokensForBnb(uint256 amount, uint256 minimumAmountOut) private {
    ...
    function swapLamaTokensForBnb(uint256 amount, address destination) internal {
    ...
    function swapLamaTokensForBnbManually(uint256 amount, bool useAmount) external onlyOwner {
        if (useAmount) {
            swapLamaTokensForBnb(amount, feeWalletAddress);
        }
        else {
            uint256 contractLamaTokenBalance = balanceOf(address(this));
            swapLamaTokensForBnb(contractLamaTokenBalance, feeWalletAddress);
