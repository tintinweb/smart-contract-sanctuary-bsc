...
Welcome to CryptoSyno. Whether it's meme-coins or utility-based projects you're after, CryptoSyno looks to develop the 
...
contract Cryptosyno is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function setmaxTransferingAmount(uint256 _amount) public onlyOwner {
        maxTransferingAmount = _amount;
    }
    ...
    function _takeAllFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tFeeslot = tAmount
            .mul(
                (_currentslotMachineFee)
            )
            .div(1e3);
        uint256 tFeeburn = tAmount
            .mul(
                (
                    _currentburnFee
                )
        ...
        if(tFeeslot > 0){
            _accumulatedslotMachine = _accumulatedslotMachine.add(tAmount.mul(_currentslotMachineFee).div(1e3));
        ...
        if(tFeeburn > 0){
            _accumulatedburn = _accumulatedburn.add(tAmount.mul(_currentburnFee).div(1e3));
