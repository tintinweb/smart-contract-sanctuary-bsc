...
contract Farm is Initializable, AccessControlUpgradeable {
    ...
    uint256 baseBoost; // 28.5%
    uint256 maxBoost; // 71.5%
    ...
    function recycle(uint256 _amount) internal {
        ...
        console.log("total rec", totalRecycled);
        calculateRecycleReserveAmount(
        ...
        for (uint pid = 0; pid < poolInfo.length; pid++) {
            ...
            if (user.amount > 0) {
                ...
                uint256 pendingAmount = user
                    ...
                    .div(1e18)
                    .sub(user.rewardDebt);
                // console.log("Timestamp for Mass Claim", block.timestamp);
                // console.log("User", _user);
                // console.log("CODE: Pending", pendingAmount);
                // console.log("Pool", pid);
                // console.log("User Amount", user.amount);
                // console.log("accStarsPerShare", pool.accStarsPerShare);
                // console.log("Reward Debt", user.rewardDebt);
                ...
                // console.log("Mass CLaim User Boost", userBoost);
                uint256 updatedBase = baseBoost + userBoost; // 57000 | 57

                uint256 amountAfterBoostCalc = pendingAmount
                    .mul(updatedBase)
                    .div(100000);
                // console.log("Amount after Calc", amountAfterBoostCalc);
                safeStarsTransfer(_user, amountAfterBoostCalc);
                // console.log("Pending before sub", pendingAmount);
                // console.log("Sub amount", amountAfterBoostCalc);
                pendingAmount -= amountAfterBoostCalc;
                // console.log("Pending after sub", pendingAmount);
                if (pendingAmount > 0 && block.timestamp < poolEndTime) {
                    recycle(pendingAmount);
                }
                if (pid == 0) {
