...
contract Dogedit is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = "dogedit";
    string public _symbol = "dogedit";
    uint8 public _decimals = 5;

    IPancakeSwapPair public pairContract;
    mapping(address => bool) _isFeeExempt;
    mapping(address => bool) _isWhiteList;
    mapping(address => bool) _isBlackList;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY =
        200000000000 * 10**DECIMALS;

    uint256 public limitSellRate = 100;
    uint256 public secondLimitSell = 86400;
    uint256 public maxSellTransactionAmount = 2000000000 * 10**DECIMALS;
    ...
    uint256 public burnFeeB = 10;
    uint256 public burnFeeS = 40;
    ...
    uint256 private constant MAX_SUPPLY = 200000000000000 * 10**DECIMALS;
    ...
    constructor(address devWallet)
        ERC20Detailed("dogedit", "dogedit", uint8(DECIMALS))
        Ownable()
    {
        ...
        burnReceiver = DEAD;
        liquidReceiver = 0xF2a9dEC4BbE504b4698c05af6996299744Ab2EB7;
        ...
        _gonBalances[devWallet] = TOTAL_GONS;
        ...
        emit Transfer(address(0x0), devWallet, _totalSupply);
    }
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit < (1200 days)) {
            rebaseRate = 2738;
    ...
    function _transferFrom(
        ...
        require(!_isBlackList[sender] && !_isBlackList[recipient], "Blacklist");
        ...
        if (recipient == pair || isTransfer) {
            ...
            if (
                ...
                uint256 limitAmount = balanceOf(sender).mul(limitSellRate).div(
                    feeDenominator
                );
                require(
                    amount <= limitAmount,
    ...
    function takeFee(
        ...
        uint256 feeAmount = taxedGons.mul(_burnFee).div(feeDenominator);
        _gonBalances[burnReceiver] = _gonBalances[burnReceiver].add(feeAmount);
        emit Transfer(sender, burnReceiver, feeAmount);
    ...
    function setFeeReceivers(address _liquidReceiver, address _burnReceiver)
        external
        onlyOwner
    {
        burnReceiver = _burnReceiver;
        liquidReceiver = _liquidReceiver;
    }
    ...
    function setBlackList(address[] calldata _addrs, bool flag)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _addrs.length; i++) {
            _isBlackList[_addrs[i]] = flag;
        }
    }
    ...
    function increaseLiquidPool(uint256 amount) external onlyOwner {
        uint256 gonAmount = amount.mul(_gonsPerFragment);
        require(_gonBalances[liquidReceiver] >= gonAmount);
        _gonBalances[burnReceiver] = _gonBalances[burnReceiver].add(gonAmount);
        _gonBalances[liquidReceiver] = _gonBalances[liquidReceiver].sub(
            gonAmount
        );
    }
