...
contract NftStaking is ERC721Holder, Ownable {
    ...
    event ItemIdUpdated(uint256 newItemId);
    event Deposit(address user, uint256[] tokenIDs);
    event Withdrawal(address user, uint256[] tokenIDs);
    ...
    function stake (
        ...
        address[] memory _beneficiaries = new address[](tokenIds.length);
        uint256[] memory _itemIds = new uint256[](tokenIds.length);
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            _beneficiaries[i] = msg.sender;
            _itemIds[i] = itemId;
        }

        rewardToken.issueTokens(_beneficiaries, _itemIds);
    ...
    /**
     * @notice Sets another Item Id
     * @param _itemId ItemId
     */
    function setItemId(uint256 _itemId) external onlyOwner {
        itemId = _itemId;
        emit ItemIdUpdated(_itemId);
    }
}
