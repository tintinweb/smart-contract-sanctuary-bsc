...
contract HeroNFT is BaseNFT {
  ...
  function mintHeroesForTest(address[] calldata _owners, uint16 _classId, uint _price, uint8 _level, uint _quantity, uint[3] calldata skills) onlyMintAdmin external {
    for(uint i = 0; i < _owners.length; i++) {
      for(uint j = 0; j < _quantity; j++) {
        _mintForTest(_owners[i], _classId, _price, j, _level, skills);
      }
    }
  }
