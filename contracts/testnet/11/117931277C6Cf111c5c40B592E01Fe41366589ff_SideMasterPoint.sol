...
contract SideMasterPoint is ERC20Votes, Pausable, Roles {
    using SafeERC20 for IERC20;

    ISyncProtocol public syncProtocol;

    event Deposit(address user, uint convertedPoints);
    event Withdraw(address user, address token, uint points);
    ...
    constructor(
        ...
        IGnosisSafeProxyFactory _gnosisSafeProxyFactory,
    ...
    function migrateSyncProtocol(ISyncProtocol _syncProtocol) external onlyRole(GUARDIAN_ROLE) {
        require(_syncProtocol != syncProtocol, "Point::Already existed sync protocol");
        syncProtocol = _syncProtocol;
    }
    ...
    function handleSyncData(bytes memory _callData) external onlyRole(SYNC_ROLE) {
        ...
        address userGnosisSafe = gnosisSafeProxyFactory.userToProxy(user);
        require(userGnosisSafe != address(0), "Point::Caller does not have gnosis safe!");
        _mint(userGnosisSafe, amount);
    ...
    function withdraw(
        ...
        address userGnosisSafe = gnosisSafeProxyFactory.userToProxy(msg.sender);
        require(userGnosisSafe != address(0), "Point::Caller does not have gnosis safe!");
        _burn(userGnosisSafe, _amount);
        _syncWithdraw(_amount, _collateralToken);
        emit Withdraw(msg.sender, _collateralToken, _amount);
    }
    ...
    function setGnosisSafeProxyFactory(
        IGnosisSafeProxyFactory _gnosisSafeProxyFactory
    ) public onlyRole(GUARDIAN_ROLE) {
        gnosisSafeProxyFactory = _gnosisSafeProxyFactory;
    }

    function _syncWithdraw(uint _amount, address _collateralToken) internal {
