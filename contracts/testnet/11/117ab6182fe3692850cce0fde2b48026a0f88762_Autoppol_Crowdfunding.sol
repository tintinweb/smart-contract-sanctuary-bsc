   ...
   uint public pool18_price;
    ...
    UserStruct[] public requests;
      ...
      constructor(address _token, address stableCoin_, address exam_) public {
        ...
        LEVEL_PRICE = REGESTRATION_FESS/100*30;
    ...
    pool18activeUserID=pool18currUserID;
      ...
      require(amount == REGESTRATION_FESS, 'Incorrect Value');
       require(stableCoin.allowance(msg.sender, address(this)) >= amount,"NEED_TO_APPROVE_TOKEN"); 
       stableCoin.transferFrom(msg.sender,address(this),amount);
       tokenReward =  amount / tokenPrice() * 1e18;
       ...
       userList[currUserID]=msg.sender;
        ...
        users[userList[users[msg.sender].referrerID]].buyPoolBalance = users[userList[users[msg.sender].referrerID]].buyPoolBalance + (REGESTRATION_FESS / 10*3);
        users[userList[users[msg.sender].coreferrerID]].buyPoolBalance = users[userList[users[msg.sender].coreferrerID]].buyPoolBalance + (REGESTRATION_FESS / 100*15);
        users[userList[users[msg.sender].referrerID]].income = users[userList[users[msg.sender].referrerID]].income + (REGESTRATION_FESS / 10*3);
        users[userList[users[msg.sender].coreferrerID]].income = users[userList[users[msg.sender].coreferrerID]].income + (REGESTRATION_FESS / 100*15);
     ...
     function payReferral(uint _level, address _user) internal {
        ...
        referer = userList[users[_user].referrerID];
         ...
         bool sent = false;
                        ...
                        sent = stableCoin.transfer(address(uint160(referer)),LEVEL_PRICE);
            if (sent) {
       ...
       function paycoReferral(uint _level, address _user) internal {
        ...
        referer = userList[users[_user].coreferrerID];
         ...
         bool sent = false;
            ...
            sent = stableCoin.transfer(address(uint160(referer)),LEVEL_PRICE/2);
            if (sent) {
       ...
       function upgradePool1() public {
       require(users[msg.sender].buyPoolBalance >= pool1_price, "Buy Pool balance is low");
       ...
       sent1 = stableCoin.transfer(address(uint160(referer)),pool1_price/10);
       ...
       bool sent2 = false;
       sent2 = stableCoin.transfer(address(uint160(introducer)),pool1_price/20);
       emit IntroducerPool1Payment(msg.sender,introducer);
       

       users[userList[users[msg.sender].coreferrerID]].income = users[userList[users[msg.sender].coreferrerID]].income + (pool1_price / 20);
       ...
       pool1userList[pool1currUserID]=msg.sender;
       users[pool1userList[pool1activeUserID]].buyPoolBalance = users[pool1userList[pool1activeUserID]].buyPoolBalance + pool1_price/10*4;
                users[pool1userList[pool1activeUserID]].income = users[pool1userList[pool1activeUserID]].income + pool1_price/10*4;
                users[msg.sender].buyPoolBalance = users[msg.sender].buyPoolBalance-pool1_price;
       bool sent = false;
 
       sent = stableCoin.transfer(address(uint160(pool1Currentuser)),pool1_price/10*4);
            if (sent) {
