...
contract Staking {
    ...
    function padding(address _user) public view returns(uint) {

        (uint _Amount, uint _RewardDebt) = UserInfoFun(0, _user);
        (address _lpToken, uint256 _allocPoint, uint256 _lastRewardBlock, uint256 _accCakePerShare) = PoolInfoFun(0);
    ...
    function UserInfoFun(uint _num, address _user) public view returns(uint amount, uint rewardDebt) {
        return cakePool.userInfo(_num, _user);
    }

    function PoolInfoFun(uint _num) public view returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accCakePerShare) {
