...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function setLargeBuyLimitTime(uint256 time) public onlyOwner {
        largeBuyLimitTime = time;
    }
    ...
    function excludedFromTransferFee(address addr)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function includedFromTransferFee(address addr)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function _transfer(
        ...
        uint256 burnFee = _sellBurnFee;
        uint256 rewardFee = _sellRewardFee;
        uint256 tokenTransferFee = _transferBurnFee;
        ...
        if (from == uniswapV2Pair) {
            if (_isRemoveLiquidity()) {
                ...
                if (block.timestamp <= openSwapTime + largeBuyLimitTime) {
                    if (!inWhitelist[to]) {
                        require(amount <= largeBuyAmount, "large buy");
                    }
                }
                //buy
                takeFee = false;
            ...
            isExcludedFromTransferFee[from] || isExcludedFromTransferFee[to]
        ) {
            takeFee = false;
        } else {
            transferToken = true;
        ...
        if (takeFee) {
            if (sell) {
                ...
                _tokenSell(from, to, amount, burnFee, rewardFee);
            }
            if (transferToken) {
                _tokenTransfer(from, to, amount, tokenTransferFee);
            }
        } else {
            unchecked {
