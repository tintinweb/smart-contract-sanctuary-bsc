...
contract Loto is Ownable{
    ...
    // values to the array
    function arrayviwer() public view returns(uint){  
        return data.length;
    }  

    function random() public view returns(uint) {
        return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, data)));
    }

    function pickWinner() public view returns(uint){

        uint randomNumber = random() % data.length;
        uint winner;
        // select the winner
        winner = data[randomNumber];
        return winner;
