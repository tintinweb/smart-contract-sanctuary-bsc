...
contract GovSaleRemote is
    ...
    function buyToken(uint128 _amountIn) external payable whenNotPaused nonReentrant {
        ...
        raised += _amountIn;
        revenue += feeCharged;
        summaries[sender] = Summary(tokenReceivedEstimate, _amountIn, feeCharged);
    ...
    function changeToOriginalDecimal(uint128 _amount) internal view returns(uint128 result){
        result = (_amount * paymentDecimals) / 1e6;
    }
    ...
    function receiveCall(bytes calldata _payload) external {
        ...
        ) = abi.decode(_payload, (address, uint128, uint128, uint128, address[], uint128[], uint128[], uint128[]));
        ...
        for(uint16 i = 0; i < remoteUsers.length; ++i){
            ...
            if(summary.bought > remoteUsersBought[i]){
                uint128 surplusFee;
