...
// File: IEmpireLock.sol


pragma solidity =0.8.17;

interface IEmpireLock {
    function lock(
        address owner,
        address token,
        bool isLpToken,
...
/**
 * @title SafeEMP20
 * @dev Wrappers around EMP20 operations that throw on failure (when the token
 * contract returns false). Tokens that return no value (and instead revert or
 * throw on failure) are also supported, non-reverting calls are assumed to be
 * successful.
 * To use this library you can add a `using SafeEMP20 for IEMP20;` statement to your contract,
...
library SafeEMP20{
     ...
     * {IEMP20-approve}, and its usage is discouraged.
     *
    ...
    function safeApprove(
        ...
        require(
            ...
            "SafeEMP20: approve from non-zero to non-zero allowance"
        );
    ...
    function safeDecreaseAllowance(
        ...
        unchecked {
            ...
            require(oldAllowance >= value, "SafeEMP20: decreased allowance below zero");
     ...
     */
    function _callOptionalReturn(IEMP20 token, bytes memory data) private {
        // We need to perform a low level call here, to bypass Solidity's return data size checking mechanism, since
        // we're implementing it ourselves. We use {Address.functionCall} to perform this call, which verifies that
        // the target address contains contract code and also asserts for success in the low-level call.

        bytes memory returndata = address(token).functionCall(data, "SafeEMP20: low-level call failed");
        if (returndata.length > 0) {
            // Return data is optional
            require(abi.decode(returndata, (bool)), "SafeEMP20: EMP20 operation did not succeed");
        }
    }
}
...
contract EmpireLock is IEmpireLock,ReentrancyGuard {
    using Address for address payable;
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableSet for EnumerableSet.UintSet;
    using SafeEMP20 for IEMP20;
    ...
    // FEES
    uint256 public vaultFee = 1*10**16;
    uint256 public maxVaultFee = 3*10**18;
    address public vaultWallet= 0x3e42Aa6a1883D7632564f810850e1EB18168f4E4;
   ...
   function changeFees(uint256 newVaultFee) external onlyOwner {
    ...
    function _normalUnlock(Lock storage userLock) internal {
        ...
        IEMP20(userLock.token).safeTransfer(msg.sender, unlockAmount);
    ...
    function _vestingUnlock(Lock storage userLock) internal {
        ...
        IEMP20(userLock.token).safeTransfer(userLock.owner, withdrawable);
    ...
    function _safeTransferFromEnsureExactAmount(
        ...
        uint256 oldRecipientBalance = IEMP20(token).balanceOf(recipient);
        IEMP20(token).safeTransferFrom(sender, recipient, amount);
        uint256 newRecipientBalance = IEMP20(token).balanceOf(recipient);
        require(
