...
contract TokenWEFI is IERC20, Ownable {
    ...
    uint256 private minimumBNBsBeforeSwap = 4 * 10**18;
    ...
    function tokensBurnt() public view returns (uint256) {
    ...
    function _transfer(address from, address to, uint256 amount) private
    {
        ...
        if((from == uniswapV2Pair || to == uniswapV2Pair) && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            require(amount <= maxTxAmount, "TokenWEFI: transfer amount exceeds the maxTxAmount");
        }
    ...
    function _swapBNBForTokens(uint256 amount) private {
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{ value: amount }(0, path, _burnAddress, (block.timestamp + 300));
    ...
    function reflectToHolders(uint256 tAmount) external {
        ...
        _reflectToHolders(sender, tAmount);
