...
contract Airdrop is Ownable {
    ...
    function calculateClaimAmount() internal {
        AirdropFactory factory = AirdropFactory(airdropFactory);
        uint currentWhitelistCount = factory.getWhitelistCount();
        uint remainingTokens = airdropToken.balanceOf(address(this));
        if(currentWhitelistCount != whitelistCount) {
            claimAmount = remainingTokens / (currentWhitelistCount - usersClaimed);
            whitelistCount = currentWhitelistCount;
        }
    }
...
contract AirdropFactory is AccessControl, Ownable {
    /* ========== STATE VARIABLES ========== */

    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    uint public immutable fee = 0.5 ether;
    ...
    constructor(address payable _feeRecipient) {
        ...
        _grantRole(ADMIN_ROLE, 0x800B87ca498729A44F81bD4C69d3751Af4F867b7);
        _grantRole(ADMIN_ROLE, 0xc794104B6757BF5202978410A32179AE6363BB1c);
    ...
    function whitelistAddresses(address[] calldata _users) external {
    ...
    function removeWhitelistAddresses(address[] calldata _users) external {
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        uint length = _users.length;
        for (uint i; i < length; i++) {
            if(isWhitelisted[_users[i]]) {
