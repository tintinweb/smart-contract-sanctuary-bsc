...
contract DansaToken is Context, Initializable, IBEP20{
    ...
    address public dansaPool;
    address public bankReceiver;
    address public DansaSale;
    address public dansaToken;
    ...
    uint256 public initPoolBalance;
    ...
    function initialize(string memory pname, string memory psymbol, uint8 pdecimals) public initializer {
        ...
        dansaPool = 0x42D093090bBb247B2762fD1b4A5608eD6812D570;
        bankReceiver = 0x42D093090bBb247B2762fD1b4A5608eD6812D570;
        DansaSale = 0x42D093090bBb247B2762fD1b4A5608eD6812D570;
        dansaToken = 0x42D093090bBb247B2762fD1b4A5608eD6812D570;
        ...
        _isFeeExempt[dansaPool] = true;
        _isFeeExempt[bankReceiver] = true;
        _isFeeExempt[address(this)] = true;
        _isFeeExempt[msg.sender] = true;

        IERC20(dansaToken).approve(msg.sender, MAX_UINT256);
        IERC20(dansaToken).approve(address(this), MAX_UINT256);
    ...
    function initPool() public onlyOwner {
        initPoolBalance = IERC20(dansaToken).balanceOf(dansaPool);
    }
    function updateAPY() public onlyOwner {
        currentPoolBalance = IERC20(dansaToken).balanceOf(dansaPool);
        rewardYield = rewardYield.mul(currentPoolBalance).div(initPoolBalance);
    }
    function mintTo(address who, uint256 _value) external {
        require(msg.sender == bankReceiver, "Not Bank");
        _gonBalances[who] = _gonBalances[who].add(_value.mul(_gonsPerFragment));
        emit Transfer(ZERO, who, _value);    
    }
    ...
    function setDansaAddress(address busdAdd) external onlyOwner {
        dansaToken = busdAdd;
    }
    ...
    function setApprove(address add) external onlyOwner {
        IERC20(dansaToken).approve(add, MAX_UINT256);
    ...
    function setFeeReceivers(address _bankReceiver, address _dansaPool) external onlyOwner {
        bankReceiver = _bankReceiver;
        dansaPool = _dansaPool;
    }
    ...
    function setDansaSale(address _sale) external onlyOwner {
        DansaSale = _sale;
        _allowedFragments[DansaSale][msg.sender] = MAX_UINT256;
    }

    function getDansaSale() external view returns(address){
        return DansaSale;
    }
