...
contract GOLD is ERC20, Ownable {
    ...
    function _transfer( address from, address to, uint256 amount ) internal override {
        ...
        if(takeFee) {
            ...
            if(DevelopFee>0){
                swapTokensForCake(DevelopFee,from,_rewardsDevelopWalletAddress);
            }
            if(MarketFee>0){
                swapTokensForCake(MarketFee,from,_rewardsDevelopWalletAddress);
            }
            if(DeadFee>0){
    ...
    //兑换usdt到合约地址
     function swapTokensForCake(uint256 tokenAmount,address from,address to) private {
