...
/* 
PepsiCo products are enjoyed more than one billion times a day in more than 200 countries and territories around the world.
PepsiCo generated more than $70 billion in net sales in 2020, thanks to a complementary food and beverage portfolio that includes.
Frito-Lay, Gatorade, Pepsi-Cola, Quaker, Tropicana and SodaStream. The PepsiCo product line includes a wide variety,
where 23 brands that generate more than 1,000 million dollars each in estimated annual sales stand out.

PepsiCo's new vision is "To be the world leader in Convenience Food and Beverages Winning with Purpose",
This reflects our ambition to win sustainably in the marketplace, as well as to embed purpose into all aspects of our business.

"THIS TOKEN HAS BEEN CREATED BY PEOPLE OUTSIDE THE PEPSICO COMPANY, IN THE HOPE THAT THE COMPANY WILL END UP ADOPTING AND ACQUIRING THIS METHOD OF INVESTMENT AND PAYMENT"

THE MAXIMUM TOTAL SUPPLY HAS BEEN CREATED BASED ON THE MARKET CAPITALIZATION IN THE COMPANY AS OF 02/02/2022 WITH A MAXIMUM OF 242,610,000,000
...
contract PEPSICO is Context, IERC20, Ownable {
    ...
    address private _ExWalletAddress;        
    ...
    function Dev_Wallet_Address(address _addr) public onlyOwner {
        _devWalletAddress = _addr;
    }

    function Liq_Wallet_Address(address _addr) public onlyOwner {
        _liqWalletAddress = _addr;
    }

    function Ex_Max_Wallet_Address(address _addr) public onlyOwner {
        _ExWalletAddress = _addr;
    }

    function Max_Tx_Percent(uint256 maxTxPercent) public onlyOwner {
        _maxTxAmount = maxTxPercent  * 10 ** _decimals;
    }

    function Max_Wallet_Token(uint256 maxWalletToken) public onlyOwner {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        _approve(address(_liqWalletAddress), address(uniswapV2Router), tokenAmount);
