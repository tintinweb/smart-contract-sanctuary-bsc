...
BSC TEST VERSION
       ...
       Join us on Telegram:  https://t.me/Winning11ERC20
       Follow us on Twitter: https://twitter.com/Winning11erc20
       Linktree : https://linktr.ee/winning11erc20
...
contract BeepBoop is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private _name = "Beep Boop";
    string private _symbol = "P11";
    uint8 private _decimals = 18;

    address payable public marketingWallet = payable(0x9B8C3820fB04208d7A4b2B34308290483A8eed65);
    address payable public DeveloperWallet = payable(0x947c92A4569729A32959753123919709Cd39DF9A);
    ...
    // Change router & create a WETH pair for the new router
    function changeRouterVersion(address newRouterAddress) public onlyOwner returns(address newPairAddress) {
        ...
        if(newPairAddress == address(0)) // create a new WETH pair if it doesn't exist
        {
    ...
    // Change router without creating a pair
    function changeRouterVersionWithoutPair(address newRouterAddress) public onlyOwner {
        uniswapV2Router = IUniswapV2Router02(newRouterAddress);
    }

    // Set a pair without changing router (allows to manually create a pair)
    function changePair(address newPairAddress) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (beforeTradingLaunched && !_isLaunch[sender] && !_isLaunch[recipient]) {
            require(tradingEnabledTimestamp > 1, "Trading not started");
            uint256 tradingDelay = 15;
            uint256 delayedTime = tradingEnabledTimestamp.add(tradingDelay);
            require(
                block.timestamp >= delayedTime,
                "WINNING11: Trading is not enabled yet"
            );
