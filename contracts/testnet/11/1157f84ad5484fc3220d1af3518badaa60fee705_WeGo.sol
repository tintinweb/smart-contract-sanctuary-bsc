...
contract WeGo is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WeGoDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WeGo", "WG") {
        MarketingWallet = payable(0x29d64BEeB2d0e78A4fE80638762F55e55F752D26);
        TeamWallet = payable(0x29e18e019bee6D2fd3BCebA36377f72932BFeb5b);
        ...
        dividendTracker = new WeGoDividendTracker(
            payable(this),
            router,
...
contract WeGoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        WeGoContract = WeGo(mainContract);
    ...
    function _transfer(
        ...
        require(false, "WeGo_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WeGo_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WeGo contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WeGoContract);

            uint256 prevBalance = WeGoContract.balanceOf(address(this));
            ...
            {
                uint256 received = WeGoContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WeGoContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
