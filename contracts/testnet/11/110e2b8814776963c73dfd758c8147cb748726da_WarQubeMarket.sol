...
interface IWarQubeMintableStore {
	function mint(
		address from,
		address to,
		uint256 id,
...
contract WarQubeMarket is OrderState, TransferSafe {
	...
	IWarQubeMintableStore private _WarQubeMintableStore;
	address public weth;
	mapping (address => uint256) public averageNFTPrice;
	...
	function buy(
		...
		emitMatchOrder(order, msg.sender);
		//wei to eth
		averageNFTPrice[order.key.sellAsset.token] = (averageNFTPrice[order.key.sellAsset.token] + order.buying/2)/1000000000000000000;
	}
		...
		if (
			...
			if (order.orderType == 1 || order.orderType == 3) {
				if (!isStore) {
					...
					require(
						order.key.sellAsset.token == address(_WarQubeMintableStore),
						"invalid sell asset"
					);
					erc1155safeMintTransferFrom(
						IWarQubeMintableStore(order.key.sellAsset.token),
						order.key.owner,
						buyer,
						order.key.sellAsset.tokenId,
						1,
				...
				if (!isStore) {
					...
					require(
						order.key.buyAsset.token == address(_WarQubeMintableStore),
						"invalid buy asset"
					);
					erc1155safeMintTransferFrom(
						IWarQubeMintableStore(order.key.buyAsset.token),
						buyer,
						order.key.owner,
						order.key.buyAsset.tokenId,
						1,
	...
	function setMintableStore(address newMintableStore) external onlyOwner {
		...
		_WarQubeMintableStore = IWarQubeMintableStore(newMintableStore);
	...
	function WarQubeMintableStore() external view returns (address) {
		return address(_WarQubeMintableStore);
	}
}