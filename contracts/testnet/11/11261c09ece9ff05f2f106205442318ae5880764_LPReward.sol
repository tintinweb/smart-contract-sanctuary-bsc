...
contract LPReward is Ownable {
    ...
    IFactory public immutable swapFactory;
    ...
    function recordRemoveLiquidity(address recipient, address tokenA, address tokenB, uint amountA, uint amountB, uint liquidity) external lock onlyRouter { 
        ...
        if (tokenA != SystemToken && tokenB != SystemToken) {
            address tokenToPair = swapFactory.getPair(tokenA, SystemToken);
            if (tokenToPair != address(0)) {
                amountSystemToken = IRouter(swapRouter).getAmountsOut(amount0, getPathForToken(tokenA))[1];
            }

            tokenToPair = swapFactory.getPair(tokenB, SystemToken);
            if (tokenToPair != address(0)) {
                if (amountSystemToken != 0) {
                    amountSystemToken = amountSystemToken + IRouter(swapRouter).getAmountsOut(amount1, getPathForToken(tokenB))[1];
                } else  {
                    amountSystemToken = IRouter(swapRouter).getAmountsOut(amount1, getPathForToken(tokenB))[1] * 2;
                }
            } else {
    ...
    function systemtokenAmountForPair(address pair, uint amountA, uint amountB) private view returns (uint amountSystemToken) {
        ...
        if (tokenA != SystemToken && tokenB != SystemToken) {
            address tokenToPair = swapFactory.getPair(tokenA, SystemToken);
            if (tokenToPair != address(0)) {
                amountSystemToken = IRouter(swapRouter).getAmountsOut(amountA, getPathForToken(tokenA))[1];
            }

            tokenToPair = swapFactory.getPair(tokenB, SystemToken);
            if (tokenToPair != address(0)) {
                if (amountSystemToken != 0) {
                    amountSystemToken = amountSystemToken + IRouter(swapRouter).getAmountsOut(amountB, getPathForToken(tokenB))[1];
                } else  {
                    amountSystemToken = IRouter(swapRouter).getAmountsOut(amountB, getPathForToken(tokenB))[1] * 2;
                }
            } else {
                amountSystemToken = amountSystemToken * 2;
            }
