...
contract PreSale is OwnerWithdrawable {
    ...
    function updateTokenRate(
        ...
        for(uint256 i = 0; i < _tokens.length; i+=1){
            require(tokenWL[_tokens[i]] == false, "Presale: Token not whitelisted");
        ...
        returns (uint256)
    {
        uint256 amtOut;
        if(token != address(0)){
            require(tokenWL[token] == false, "Presale: Token not whitelisted");
    ...
    function buyToken(address _token, uint256 _amount) external payable saleDuration{
        ...
        if(_token != address(0)){
            ...
            require(tokenWL[_token] == false, "Presale: Token not whitelisted");
