...
contract chocolatefactory is Ownable {
    ...
    function buyBeans(uint256 amt, address ref) public payable {
        ...
        uint256 totalBnbFee = percentFromAmount(amt, DEPOSIT_FEE);
        uint256 bnbValue = Math.sub(amt, totalBnbFee);
    ...
    function sendFees(uint256 totalFee, uint256 giveAway) private {
        ...
        USDC.safeTransfer(_dev, dev);
        USDC.safeTransfer(_marketing, marketing);
        USDC.safeTransfer(_ceo, ceo);
