...
contract PoolFactory is OwnableUpgradeable, IPoolFactory {
    ...
    function changeAssetHandler(address _assetHandler) public onlyOwner {
        ...
        assetHandler = _assetHandler;
        emit AssetHandlerChanged(_assetHandler);
    }
}
...
contract PoolLogic is ERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    event UpdatePerformanceFee(address manager, uint256 fee);
    ...
    function calculatePerformanceFee() external nonReentrant {
        uint256 performanceFee = IFeeManage(poolManagerLogic).performanceFee();
        if (performanceFee == 0) {
        ...
        uint256 managerFeeValue = profit.sub(profit.mul(performanceFee).div(10000));
        ...
        if (systemFee > 0) {
            _mint(address(this), systemFee);
        }

        emit UpdatePerformanceFee(manager, managerFee);
    }
...
contract PoolManagerLogic is Initializable, IPoolManagerLogic, ISupportedAsset,IFeeManage {
    using SafeMathUpgradeable for uint256;
    ...
    uint256 internal immutable TYPE_FEE_PERFORMANCE = 0;
    uint256 internal immutable TYPE_FEE_MANAGEMENT = 1;
    uint256 internal immutable TYPE_FEE_ENTRY = 2;
    uint256 internal immutable TYPE_FEE_EXIT_SPECIFIC = 3;
    uint256 internal immutable TYPE_FEE_EXIT_SHARE_IN_KIND = 4;

    uint256 private MAXIMUM_TYPE_FEE = 5;
    ...
    function setupFee(Fee[] calldata _fee) public onlyManager {
        _setupFee(_fee);
    }
    ...
    function _setupFee(Fee[] calldata _fee) internal {
        ...
        require(length < MAXIMUM_TYPE_FEE, "over maximum type fee");
        for (uint256 i = 0; i < length; i++) {
            _changeFee(_fee[i]);
        }
    }

    /// @dev chage fee in pool - based on 100%(10000)
    function _changeFee(Fee memory _fee) internal {
        ...
        uint256 oldFee;
        if (_fee.feeType == TYPE_FEE_PERFORMANCE) {
            oldFee = performanceFee;
            performanceFee = _fee.feeRate;
        } else if (_fee.feeType == TYPE_FEE_MANAGEMENT) {
            oldFee = managementFee;
            managementFee = _fee.feeRate;
        } else if (_fee.feeType == TYPE_FEE_ENTRY) {
            oldFee = entryFee;
            entryFee = _fee.feeRate;
        } else if (_fee.feeType == TYPE_FEE_EXIT_SPECIFIC) {
            oldFee = exitSpecificFee;
            exitSpecificFee = _fee.feeRate;
        } else if (_fee.feeType == TYPE_FEE_EXIT_SHARE_IN_KIND) {
        ...
        recipients[_fee.feeType] = _fee.recipient;

        emit FeeChanged(manager, _fee.feeType, oldFee, _fee.feeRate, _fee.recipient);
    }
