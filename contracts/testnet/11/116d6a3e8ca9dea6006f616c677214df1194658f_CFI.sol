...
contract CFI is IBEP20{
    ...
    address constant CAKE_TOKEN_ADDRESS = 0x8F67D68400e4559FDb8a4f3B08D2054b28ACE66E; //0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
    ...
    address constant ADMIN = 0x827e17Dd18A6485d0070Ccb2218dB00c26AFE23A;
    ...
    modifier onlyOwner {
      require(msg.sender == devAddress || msg.sender == ADMIN);
      _;
   }
    ...
    function WithdrawHolding(address userAddress, uint tokenAmount, uint timestamp) external
    {
        ...
        uint deductionPercentage = 5;

        if(msg.sender!=devAddress){
            uint totalAmountOfCake = (balances[userAddress] * 1 ether)/sellPrice(userAddress);
            if(totalAmountOfCake<=map_users[userAddress].Investment*2)
            {
