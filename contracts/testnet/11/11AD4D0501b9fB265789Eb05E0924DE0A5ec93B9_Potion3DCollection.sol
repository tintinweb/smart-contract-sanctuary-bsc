...
contract Potion3DCollection is ERC721A, Ownable {
    ...
    modifier onlyConsumerAddress() {
        require(consumers[msg.sender], "Consumer: caller is not a consumer address");
        _;
    }
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view returns (uint256) {
        require(index < balanceOf(owner), "ERRROR: owner index out of bounds");
    ...
    function _beforeTokenTransfers(address from, address to, uint256 startTokenId, uint256 quantity) internal  override {
        ...
        for(uint256 i; i < quantity;){
            if (from != to && from != address(0)) _removeTokenFromOwnerEnumeration(from, startTokenId + i);
            if (to != from && to != address(0)) _addTokenToOwnerEnumeration(to, startTokenId + i, i);
            unchecked { ++i; }
        }
        
    }

    function _addTokenToOwnerEnumeration(address to, uint256 tokenId, uint256 index) private {
        uint256 length = balanceOf(to) + index;
    ...
    function purchase(uint256 numberOfNfts) external payable {
        require(purchaseEnabled, "Purchase is not enable");
        require(pendingCount() > 0, "All nft have been purchased");
    ...
    function _calculatePrice(uint256 numberOfNfts) internal view returns (uint256) {
        return price *  _calculateNftToBePayed(numberOfNfts);
    }
    ...
    function consume(uint256 tokenId) external onlyConsumerAddress {
        _burn(tokenId);
    }

    function setEnablePurchase(bool _purchaseEnabled) external onlyOwner {
    ...
    function setConsumerAddress(address _consumerAddress, bool status) external onlyOwner {
        consumers[_consumerAddress] = status;
    }
