...
contract Exchange is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    address public constant MCOIN = 0x993EE20C86E07D84BdF9277d4636149d4876631f;

    address public constant MAI_EXCHANGE = 0xf90E4c79060117babc4077D59aB5436F3a4afc93;
    address public constant LENDING_POOL = 0x0DFCb5fdDc83e45FD396b19c876897E801fd806c;
    address public constant FULL_BACK = 0x4e0454bea7B143805DacB106A372aacEedB0d2C0;
    ...
    // MUSD-MCOIN 消费合约
    // USDT-MUSD 借贷合约
    // USDT-MCOIN  待定USDT-MUSD-MCOIN
    // MCOIN-MUSD 消费合约
    // MCOIN-USDT 待定MCOIN-MUSD-USDT
    // MAI-MUSD MAI兑换合约
    // MAI-USDT 待定MAI-MUSD-USDT
    // MAI-MCOIN 待定MAI-MUSD-MCOIN
    ...
    function _musdToMcoin(uint256 amount) internal returns (uint256 outAmount) {
        IERC20Upgradeable(MUSD).safeApprove(FULL_BACK, amount);
        IFullBack(FULL_BACK).pointMint(amount);
        outAmount = (amount * IFullBack(FULL_BACK).pointRate()) / 1000;
    }

    function _mcoinToMusd(uint256 amount) internal returns (uint256 outAmount) {
        IERC20Upgradeable(MCOIN).safeApprove(FULL_BACK, amount);
        IFullBack(FULL_BACK).pointBack(amount);
        outAmount = (amount * 1000) / IFullBack(FULL_BACK).pointRate();
    }
