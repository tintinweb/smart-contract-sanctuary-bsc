...
library AffiliateCreator {
	...
	function Creator(bytes32 _Bytes32, uint8 _len) internal pure returns (bytes32 _r) {
		string memory s = ToHex(_Bytes32);
		bytes memory b = bytes(s);
		bytes memory r = new bytes(_len);
		for (uint i = 0; i < _len; ++i) r[i] = b[i + 5];
		return bytes32(bytes(r));
	}
}
...
library Uint256Array {
	...
	function RemoveIndex(uint256[] storage _Array, uint64 _Index) internal {
		require(_Array.length > 0, "Can't remove from empty array");
		require(_Array.length > _Index, "Index out of range");
		// Move the last element into the place to delete
		_Array[_Index] = _Array[_Array.length - 1];
	...
	function AddNoDuplicate(uint256[] storage _Array, uint256 _Value) internal {
		for (uint256 i = 0; i < _Array.length; ++i) if (_Array[i] == _Value) return;
		_Array.push(_Value);
	}

	function TrimRight(uint256[] memory _Array) internal pure returns (uint256[] memory _Return) {
...
abstract contract TAccount is TMatrix {
	...
	struct Account {
		uint256 AccountID;
		bytes32 Affiliate; // For share link. User can modify
		address Address; // Address only, One address can have multiple accounts
		uint256 RegTime; // Registration datetime
		bool AutoNextLevel;
		bool Stoped; // User can stop account and withdraw all
	}

	// uint256 private RootAccount;
	uint256 private numAccount; // Total account number
	mapping(uint256 => Account) Accounts; // AccountID to user info
	mapping(bytes32 => uint256) Affiliates; // To AccountID. Affiliates is used as AccountID
	mapping(address => uint256[]) AccountOf; // Accounts of address

	constructor(uint256 _Starting) TMatrix(_Starting) {
		InitializeAccount(_Starting);
	}

	function InitializeAccount(uint256 Starting) private {
		// RootAccount = Starting;
		Accounts[Starting] = Account({
			AccountID: Starting,
			Affiliate: bytes32(bytes("Affiliate")),
			Address: msg.sender,
			RegTime: block.timestamp,
			AutoNextLevel: true,
			Stoped: false
		});
		Affiliates[bytes32(bytes("Affiliate"))] = Starting;
	...
	function _AffiliateCreator() internal view returns (bytes32 _Affiliate) {
		uint i;
		while (true) {
			_Affiliate = bytes32(keccak256(abi.encodePacked(msg.sender, block.difficulty, block.timestamp, ++i))).Creator(10);
			if (Affiliates[_Affiliate] == 0) return _Affiliate;
		}
	}

	function _AccountIDCreator() internal returns (uint256 NewAccountID) {
		while (true) {
			unchecked {
				++numAccount;
	...
	function _ChangeAffiliate(uint256 _AccountID, bytes32 _NewAffiliate) internal {
		Affiliates[Accounts[_AccountID].Affiliate] = 0;
		Affiliates[_NewAffiliate] = _AccountID;
		Accounts[_AccountID].Affiliate = _NewAffiliate;
	}

	function _AccountsOf(address _address) internal view returns (uint256[] memory _AccountIDs) {
		return AccountOf[_address];
	}

	// Return a account id LATEST of Address
	function _GetLatestAccountsOf(address _address) internal view returns (uint256 _AccountID) {
		uint256[] memory accounts = _AccountsOf(_address);
...
abstract contract TMatrix {
	using Uint256Array for uint256[];
	using Algorithms for uint256;

	struct ColumnsY {
		// Each position per RowsX (Line/Row of root)
		uint256 NodeID; // NodeID != 0 when this node is missing leg
		bool isMissingLeg; // If TRUE: This nodeID is not enough leg
	}

	struct RowsX {
		// Each line/Row of root
		mapping(uint256 => ColumnsY) Y; // Columns
		bool isFull; // Full or not full NodeIDs in each positions of Y
		uint256[] NodeIDsMissingLeg; // List of NodeIDs missing leg on this X.
		uint256 YExistedCount; // If YExistedCount == MaxYOfXCount then Y is FULL
	}

	mapping(uint8 => mapping(uint256 => RowsX)) MatrixStorage; // Unilevel, Binary, Ternary

	struct Info {
		uint256 UplineID;
		uint256[] X1IDs; // List of F1 IDs. X1IDs.length <= LIMITED_LEG
		uint256[] PathFromRoot; // X Of Root: is line number in matrix root, and = PathToRoot.length
		uint256[] PathToSponsor; // X Of Sponsor: is line number in matrix of sponsor, and = PathToSponsor.length
		uint256 Y;
		uint256 FullYToXOfRoot; // >= X Of Root
	}
	...
	mapping(uint256 => Node) Nodes; // AccountID to node info
	uint256 private RootID;

	constructor(uint256 _Starting) {
		InitializeMatrix(_Starting);
	}

	function InitializeMatrix(uint256 _Starting) private {
		RootID = _Starting;
	...
	function InitNode(uint256 _NodeID, uint256 _SponsorID) internal virtual {
		...
		newnode.InfoOf[UNILEVEL] = Info({
			...
			PathToSponsor: new uint256[](0), //.push(_SponsorID),
			Y: Nodes[_SponsorID].InfoOf[UNILEVEL].X1IDs.length + 1,
			FullYToXOfRoot: 0
		});
	...
	// Is lower node on shortest leg of root node in Ternary matrix
	function FindSponsor() internal virtual returns (uint256 _SponsorID) {
		uint256 sfyx = Nodes[RootID].InfoOf[TERNARY].FullYToXOfRoot;
		do {
			++sfyx;
	...
	function _FindUplineID(Node storage _newnode, uint8 _MATRIX) private view returns (uint256 _UplineID) {
		uint256 sid = _newnode.SponsorID;
		uint256 sx = Nodes[sid].InfoOf[_MATRIX].PathFromRoot.length; // X Of Root;
		uint256 sfyx = Nodes[sid].InfoOf[_MATRIX].FullYToXOfRoot;
		uint256 sy = Nodes[sid].InfoOf[_MATRIX].Y;

		uint256 XCountFromSponsor; // Count row from Sponsor of _newnode
		uint256 MaxYOfXCount;
		uint256[2] memory RangeYOfXCountOnYRoot; // 0: min/begin pos, 1: max/end pos - On row of root
		uint256[] memory MissingLeg;
		uint256 MissingLegLength;
		...
		do {
			...
			MaxYOfXCount = Algorithms.Exponential(_MATRIX, XCountFromSponsor - 1); // x^y
			MissingLegLength = MatrixStorage[_MATRIX][sx - 1].NodeIDsMissingLeg.length;

			// Here: Row x is not full but Row x in Sponsor maybe is full
			if (--MaxYOfXCount >= MissingLegLength) {
				// Use missing leg array
				MissingLeg = new uint256[](MissingLegLength);
				MissingLeg = MatrixStorage[_MATRIX][sx - 1].NodeIDsMissingLeg;
				for (uint256 i = 0; i < MissingLegLength; ++i) {
					if (
				...
				for (uint256 y = RangeYOfXCountOnYRoot[0]; y <= RangeYOfXCountOnYRoot[1]; ++y) {
	...
	function _UpdateNodeAndMatrixStorage(Node storage _newnode, uint8 _MATRIX) private {
		uint256 sid = _newnode.SponsorID;
		uint256 nid = _newnode.NodeID;
		uint256 ny;

		uint256 uid = _newnode.InfoOf[_MATRIX].UplineID;
		uint256 ux1ids = Nodes[uid].InfoOf[_MATRIX].X1IDs.length;
		uint256 uy = Nodes[uid].InfoOf[_MATRIX].Y;
		uint256 ux = Nodes[uid].InfoOf[_MATRIX].PathFromRoot.length; // X Of Root;
		...
		_newnode.InfoOf[_MATRIX].X1IDs = new uint256[](0);
	...
	function _UpdatePath(Node storage _newnode, uint8 _MATRIX) private {
		...
		_newnode.InfoOf[_MATRIX].FullYToXOfRoot = _newnode.InfoOf[_MATRIX].PathFromRoot.length;
	...
	function _SelectX1IDsOfNode(
		uint256 _NodeID,
		uint8 _MATRIX
	) internal view virtual returns (uint256[] memory _NodeIDs) {
	...
	TAccount //, ITuktu {
{
	...
	constructor(uint256 _StartingAt) TAccount(_StartingAt) {}
	...
	function Registration(string memory _Affiliate) public {
		bytes32 affiliateref = bytes32(bytes(_Affiliate));
	...
	function Registration(address _AddressOfSponsor) public {
		...
		uint256 latestaccountid = _GetLatestAccountsOf(_AddressOfSponsor);
		latestaccountid != 0 ? _registration(msg.sender, latestaccountid) : _registration(msg.sender);
	}

	function Registration(address _NewAccountAddress, uint256 _SponsorID) public {
	...
	function ChangeAffiliate(uint256 _AccountID, string memory _NewAffiliate) public onlyAccountOwner(_AccountID) {
		...
		bytes32 newaffiliate = bytes32(bytes(_NewAffiliate));
		bytes32 oldaffilitae = Accounts[_AccountID].Affiliate;
		require(
	...
	// Dashboard
	function AccountsOfAddress(address _address) public view returns (uint256[] memory _AccountIDs) {
	...
	function _ReturnTreeviewArray(uint256 _AccountID, uint8 _MATRIX) private view returns (Treeview[] memory _Treeviews) {
		uint256[] memory _AccountIDs = _SelectX1IDsOfNode(_AccountID, _MATRIX);
	...
	function F1OfAccountOnUnilevel(uint256 _AccountID) public view returns (Treeview[] memory _Treeviews) {
		require(_AccountID != 0 && Accounts[_AccountID].AccountID == _AccountID, "Account: does not existed");
		return _ReturnTreeviewArray(_AccountID, UNILEVEL);
	}

	function F1OfAccountOnBinary(uint256 _AccountID) public view returns (Treeview[] memory _Treeviews) {
		require(_AccountID != 0 && Accounts[_AccountID].AccountID == _AccountID, "Account: does not existed");
		return _ReturnTreeviewArray(_AccountID, BINARY);
	}

	function F1OfAccountOnTernary(uint256 _AccountID) public view returns (Treeview[] memory _Treeviews) {
		require(_AccountID != 0 && Accounts[_AccountID].AccountID == _AccountID, "Account: does not existed");
