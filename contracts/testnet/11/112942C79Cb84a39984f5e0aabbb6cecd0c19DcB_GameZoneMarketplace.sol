...
interface IGameZoneAddressRegistry {
...
interface IGameZoneBundleMarketplace {
...
interface IGameZoneNFTFactory {
    function exists(address) external view returns (bool);
}

interface IGameZoneTokenRegistry {
    function enabled(address) external view returns (bool);
}

interface IGameZonePriceFeed {
    function wFTM() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract GameZoneMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IGameZoneAddressRegistry public addressRegistry;
    ...
    function listItem(
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            ...
            require(
             ...
             nft.setApprovalForAll(addressRegistry.plan(), true);
        } else if (
        ...
        validListing(_nftAddress, _tokenId, _owner)
    {
        ...
        require(_quantity <= listedItem.quantity, "quanity must be lower than listed");
    ...
    function _buyItem(
        ...
        {
            require(msg.value >= price,"No enough balance");
            feeReceipient.sendValue(feeAmount);
        }
        else{
        ...
        if (minter != address(0) && royalty != 0) {
            ...
            if(_payToken == address(0))
            {
                payable(minter).sendValue(royaltyFee);
            }
            else{
                 IERC20Upgradeable(_payToken).safeTransferFrom(
                    _msgSender(),
        ...
        if(_payToken == address(0))
        {
            payable(_owner).sendValue(price.sub(feeAmount));
        }
        else{
            IERC20Upgradeable(_payToken).safeTransferFrom(
                _msgSender(),
       ...
       // Transfer NFT to buyer
        ...
        // IGameZoneBundleMarketplace(addressRegistry.bundleMarketplace())
    ...
    function createOffer(
        ...
        IGameZoneAuction auction = IGameZoneAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isGameZoneNFT(_nftAddress), "invalid nft address");
    ...
    function _isGameZoneNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.artion() == _nftAddress ||
            IGameZoneNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IGameZoneNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IGameZoneNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IGameZoneNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IGameZonePriceFeed priceFeed = IGameZonePriceFeed(
    ...
    /**
     @notice Update GameZoneAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IGameZoneAddressRegistry(_registry);
    }
        ...
        require(
            _payToken == address(0) ||
                (addressRegistry.tokenRegistry() != address(0) &&
                    IGameZoneTokenRegistry(addressRegistry.tokenRegistry())
                        .enabled(_payToken)),
            "invalid pay token"
        );
    }
