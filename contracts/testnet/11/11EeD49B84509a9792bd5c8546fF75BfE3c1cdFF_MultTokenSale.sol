...
contract MultTokenSale is Context, IBEP20, Ownable {
  ...
  */ 
  function buyBoxWithToken(string memory token, uint256 boxQuant, uint boxId) public isPausable() {
    ...
    require(boxId > 0 && box.exist, "It was not possible to locate this Box");
  ...
  function buyBoxWithBNB(uint256 boxQuant, uint boxId) public payable {
    ...
    emit Received(msg.sender, BoxVal, boxQuant, boxId, "BNB");
  }
  ...
  function editTokenContract(string memory _tkname, address _tkad) public onlyOwner() isPausable(){
    for (uint i = 1; i < _tokensId; i++) {
      TokenList storage item = tokenlists[i];
      if(_compareString(item.tkname, _tkname)){
        item.tkContrat = _tkad;
      }
    }
  }
  ...
  */
  ...
  event Received(address indexed from, uint256 amount, uint256 boxQuant, uint boxId, string token);
}