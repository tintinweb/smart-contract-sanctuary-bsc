...
contract Router is ERC1155Holder, Ownable {
    ...
    constructor(address _factory, address _charity, address _referral){
    ...
    function addLiquidity(
        ...
        address to, 
        uint deadline
        ) external virtual ensure(deadline) returns(uint amountErc, uint amountNft, uint liquidity) {
        // mint token reward
    ...
    function removeLiquidity(
        ...
        (uint amounterc, uint amountnft) = ITokenReward(_token).burn(to);
    ...
    function buy(
        ...
        address pool = ITokenReward(_token).getPool();
        ...
        IERC20(tokenerc).transferFrom(msg.sender, pool, feeBuy);
        ITokenReward(_token).swap(0, amountNft, to, 0, msg.sender);
        ITokenReward(_token).updateReward(feeBuy);
    ...
    function sell(
        ...
        IERC1155(tokennft).safeTransferFrom(msg.sender, _token, id, amountNft, '0x0');
        ITokenReward(_token).updateReward(feeSell);
        ITokenReward(_token).swap(amount, 0, to, feeSell, msg.sender);
