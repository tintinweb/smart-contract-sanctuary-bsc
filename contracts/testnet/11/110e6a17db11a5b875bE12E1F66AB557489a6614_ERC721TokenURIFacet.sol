...
import {LibERC721TokenURIv2} from "../libraries/LibERC721TokenURIv2.sol";
import {IERC721TokenURIFacet} from "../interfaces/IERC721TokenURIFacet.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/IERC721Metadata.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

contract ERC721TokenURIFacet is IERC721TokenURIFacet {
    AppStorage internal s;

    function initialize(bytes calldata data) external override {
        string memory baseTokenUri = abi.decode(data, (string));
        LibERC721TokenURIv2.setBaseTokenTokenUri(baseTokenUri);
        LibDiamondv2.diamondStorage().supportedInterfaces[
            type(IERC721Metadata).interfaceId
        ] = true;
    }

    function encode(string calldata baseTokenURI)
        external
        pure
        returns (bytes memory)
    {
        return abi.encode(baseTokenURI);
    }
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    LibERC721TokenURIv2.baseTokenURI(),
...
library LibERC721TokenURIv2 {
    ...
    function setBaseTokenTokenUri(string memory _baseTokenURI) internal {
        erc721TokenUriStorage().baseTokenUri = _baseTokenURI;
    }

    function baseTokenURI() internal view returns (string memory) {
        return erc721TokenUriStorage().baseTokenUri;
    }
}
...
interface IERC721TokenURIFacet is IInitializableFacet {
    function tokenURI(uint256 tokenId) external view returns (string memory);
}
