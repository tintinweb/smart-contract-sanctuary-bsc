...
import '../libraries/LuckyRollerLibrary.sol';
import "../interfaces/IBEP20.sol";
import "../interfaces/IOracle.sol";
import "../interfaces/IDice.sol";
import '../interfaces/ILuckyRollerFactory.sol';
import '../interfaces/ILuckyRollerPair.sol';
...
contract Oracle is Ownable, IOracle {
    ...
    function update(address tokenA, address tokenB) external override returns (bool) {
        address pair = LuckyRollerLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function consult(
        ...
        address pair = LuckyRollerLibrary.pairFor(factory, tokenIn, tokenOut);
        ...
        if(timeElapsed == 0){
            (uint112 reserve0, uint112 reserve1,) = ILuckyRollerPair(pair).getReserves();
            (address token0, ) = LuckyRollerLibrary.sortTokens(tokenIn, tokenOut);
            if (token0 == tokenIn) {
    ...
    function getAveragePrice(address token) public view returns (uint256 price) {
        ...
        if (token == anchorToken) {
            price = amount;
        } else if (ILuckyRollerFactory(factory).getPair(token, anchorToken) != address(0)) {
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (
                    LuckyRollerLibrary.pairFor(factory, token, intermediate) != address(0) &&
                    LuckyRollerLibrary.pairFor(factory, intermediate, anchorToken) != address(0)
                ) {
    ...
    function getCurrentPrice(address token) public view returns (uint256 price) {
        ...
        if (token == anchorToken) {
        ...
        } else if (LuckyRollerLibrary.pairFor(factory, token, anchorToken) != address(0)) {
            (uint256 reserve0, uint256 reserve1) = LuckyRollerLibrary.getReserves(factory, token, anchorToken);
            ...
            for (uint256 index = 0; index < length; index++) {
                ...
                if (
                    ...
                    (uint256 reserve0, uint256 reserve1) = LuckyRollerLibrary.getReserves(factory, token, intermediate);
                    uint256 amountOut = 10**tokenDecimal.mul(reserve1).div(reserve0);
                    (uint256 reserve2, uint256 reserve3) = LuckyRollerLibrary.getReserves(factory, intermediate, anchorToken);
    ...
    function getLpTokenValue(address _lpToken, uint256 _amount) public override view returns (uint256 value) {
        ...
        address token0 = ILuckyRollerPair(_lpToken).token0();
        address token1 = ILuckyRollerPair(_lpToken).token1();
        uint256 token0Decimal = IBEP20(token0).decimals();
        uint256 token1Decimal = IBEP20(token1).decimals();
        (uint256 reserve0, uint256 reserve1) = LuckyRollerLibrary.getReserves(factory, token0, token1);
...
library OracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = ILuckyRollerPair(pair).price0CumulativeLast();
        price1Cumulative = ILuckyRollerPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = ILuckyRollerPair(pair).getReserves();
...
library LuckyRollerLibrary {
    using SafeMath for uint;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'LuckyRollerLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'LuckyRollerLibrary: ZERO_ADDRESS');
    }
    ...
    function getSwapFee(address factory, address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = ILuckyRollerPair(pairFor(factory, tokenA, tokenB)).swapFee();
    }

    // fetches and sorts the reserves for a pair
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = ILuckyRollerPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'LuckyRollerLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'LuckyRollerLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint swapFee) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'LuckyRollerLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'LuckyRollerLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint swapFee) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'LuckyRollerLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'LuckyRollerLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'LuckyRollerLibrary: INVALID_PATH');
