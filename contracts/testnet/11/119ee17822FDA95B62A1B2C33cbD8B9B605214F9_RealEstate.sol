...
contract RealEstate is ERC721 {
    ...
    uint256 public onSellPropertyID = 1;
    uint256 public addPropertyFee;
    uint256 public sellPropertyFee;
    uint256 public transferPropertyFee;
    IERC20 public USDT;
    // struct
    struct Property {
        uint256 id;
    ...
    // mapping(uint256 => address) requestedProperty;
    // mapping(uint256 => bool) public requestSell;
    // mapping(uint256 => address) public SellRequester;
    // mapping(uint256 => uint256) public sellRequestPrice;
    ...
    // function checkPropertyRequests(uint256 _pendingID)
    //     public
    //     view
    //     returns (address)
    // {
    //     return requestedProperty[_pendingID];
    // }

    function checkFeeValues() public view returns (uint256,uint256,uint256) {
        return (transferPropertyFee, addPropertyFee, sellPropertyFee);
    }
    ...
    function requestToAddProperty(
        ...
        // requestedProperty[AddedPropertyID] = _CurrentOwner;
        AddedPropertyID++;
    }

    function requestToSellProperty(uint256 _propertyId , uint256 _payment) public {
        // require(sellPropertyFee == _fee, "please Add Valid fee");
    ...
    function buyProperty(uint256 _propertyId, uint256 _payment) public {
        require(
        ...
        uint256 fee = ((_payment) * transferPropertyFee) / 100;
        ...
        onSellProperties[_propertyId] = Property(0, "" , "" , 0 , address(0), "", "","");

        delete onSellProperties[_propertyId];

        // SellRequester[_propertyId] = address(0);
        // onSellProperties[_propertyId].CurrentOwner = msg.sender;
        ownerOfProperty[_propertyId] = msg.sender;
        // properties[_propertyId].price = (payment+fee);
    }

    // only Owner
    function ApproveRequestToAddProperty(uint256 _ID, bool _approval)
        public
        onlyOwner
    {
        ...
        if (_approval == true) {
            ...
            requests[_ID] = Property(0, "" ,"" ,0 , address(0),"","", "");
            // requestedProperty[_ID] = address(0);
            // (payable(owner)).transfer(addPropertyFee);
            // delete requestedProperty[_ID];
            // delete requests[_ID];
        } else if (_approval == false) {
            requests[_ID] = Property(0,"","" ,0, address(0), "","", "");
            // (payable(requestedProperty[_ID])).transfer(addPropertyFee);
            // delete requestedProperty[_ID];
            // delete requests[_ID];
        }
    }
    ...
    function ApproveRequestToSellProperty(
        ...
        if (_approval == true) {
            ...
            requestsSellProperty[_pendingSellId] = Property(0,"","",0,address(0),"","","");
            properties[_pendingSellId] = Property(0,"","",0, address(0), "","", "");
            delete requestsSellProperty[_pendingSellId];
            delete properties[_pendingSellId];
        } else if (_approval == true) {
            requestsSellProperty[_pendingSellId] = Property(0,"","",0,address(0),"","","");
            delete requestsSellProperty[_pendingSellId];
        }
    }
    ...
    function changetransferPropertyFee(uint256 _newFee) public onlyOwner {
