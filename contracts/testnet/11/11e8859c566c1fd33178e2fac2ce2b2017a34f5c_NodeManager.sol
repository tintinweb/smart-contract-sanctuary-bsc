...
contract NodeManager is Initializable {
  ...
  mapping(uint32 => string) public nodeMap;
  ...
  function initialize(address _feeManager, address _nftAddress) public initializer {
    ...
    addTier('bronze', 10 ether, 0.13 ether, 1 days, 5 ether, 100);
    addTier('silver', 50 ether, 0.80 ether, 1 days, 15 ether, 10);
    addTier('gold', 100 ether, 2 ether, 1 days, 25 ether, 5);
  ...
  function _create(
    ...
    for (uint32 i = 0; i < count; i++) {
      ...
      nodeMap[uint32(nodesTotal.length)] = title;
    }
  ...
  function _claim(uint256 exceptAmount) private {
    ...
    feeManager.transfer(address(msg.sender), claimableAmount - exceptAmount);
  }
      ...
      if (nodeIndex > 0) {
        ...
        if (node.owner == msg.sender && tierIndexFrom - 1 == node.tierIndex) {
          ...
          uint256 claimed = uint256(block.timestamp - node.claimedTime)
            * tierFrom.rewardsPerTime
            / tierFrom.claimInterval;
  ...
  function transfer(
    ...
    uint256 fee = feeManager.getAmountFee(tier.price * count);
    ...
    if (fee > claimableAmount)
      ...
      feeManager.transfer(address(msg.sender), claimableAmount - fee);
  ...
  function claimAirdrop(string memory _name, string memory _tier, uint32 _amount, bytes32[] calldata _merkleProof) public {
    ...
    _create(msg.sender, _tier, '', _amount);   
