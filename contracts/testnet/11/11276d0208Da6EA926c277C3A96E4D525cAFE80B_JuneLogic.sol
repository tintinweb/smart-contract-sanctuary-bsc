...
contract JuneLogic is OwnableUpgradeable, ERC20Upgradeable, Console, Manager {
    ...
    function getDividendTotal(address value) public view virtual returns (uint256) {
        return balanceOf(MarketingWalletAddr);
    }
    // Set getLpTotal
    function getLpTotal(address value) public view virtual returns (uint256) {
    ...
    function Dividend() public onlyManager {
        ...
        for (uint i = 0; i < DividendAdds.length; i++) {
            uint256 userDividend = oneDividend.mul(LiquidityWallet.balanceOf(DividendAdds[i]));
            super._transfer(MarketingWalletAddr, DividendAdds[i], userDividend);
        }
    }
