...
contract WQCtoken is Context, IBEP20, Ownable {
  ...
  address payable feeOwner;

  constructor(
      string memory contractName,
      string memory contractSymbol,
      uint8 contractDecs,
      uint256 intnialTotalSupply
  ) public {
    _name = contractName;
    _symbol = contractSymbol;
    _decimals = contractDecs;
    _totalSupply = intnialTotalSupply;
  ...
  using Roles for Roles.Role;
    ...
    function addBurner(address account) public virtual onlyBurner {
        _addBurner(account);
    }
  ...
  */
   ...
   * - `recipient` cannot be the zero address.
   * - the caller must have a balance of at least `amount`.
   * - transferFee setted as a public option
   */
  function transferWithFee(address recipient, uint256 amount) external returns (bool) {
    _transfer(_msgSender(), feeOwner, (amount*_fee)/100);
    _transfer(_msgSender(), recipient, amount-((amount*_fee)/100));
    return true;
  }
