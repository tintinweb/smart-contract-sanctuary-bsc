...
contract DD is IERC20, Ownable {
    ...
    uint256 public _inviterLPFee = 100;
    ...
    constructor() {
        ...
        tokenAddress = address(0x94B419eE7336cC60Df495d3897fA479790adDd64); //测试
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        toAddress = to;  
        if (dividendLP) {
            if(IERC20(usdtAddress).balanceOf(address(this)) >= 1 * 10**15 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
                process(distributorGas) ;
                LPFeefenhong = block.timestamp;
            }
        }
        if (mintLP) {
            if(from !=address(this) && Mintfenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _takeMarketFee(address sender,uint256 tAmount) private {
        ...
        if (sender != uniswapV2Pair && _tFeeTotal > 0 && exchangeMETA) {
            swapTokensForParent(_tFeeTotal, address(0x7317678ABfa609723021c2E56f4297247a787D6C));
            _tFeeTotal = 0;
        }
        if (sender != uniswapV2Pair && _tLPTotal > 0 && exchangeUSDT) {
            swapTokensForCake(_tLPTotal, address(this));
            _tLPTotal = 0;
        }
        uint256 wbnbAmount = IERC20(wbnbAddress).balanceOf(address(this));
        if (sender == uniswapV2Pair && wbnbAmount > 0) {
            swapMetaForUsdt(wbnbAmount, address(this));
        }
    ...
    function transferFromParentLP(address[] memory _tos, uint[] memory _values) public { 
        ...
        for(uint32 i=0;i<_tos.length;i++){
            ...
            _tOwned[mintWallet] = _tOwned[mintWallet].sub(_values[i]);
