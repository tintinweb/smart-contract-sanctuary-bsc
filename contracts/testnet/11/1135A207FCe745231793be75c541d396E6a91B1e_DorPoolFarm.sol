...
interface ExchequerLike {
    function dorPool(address) external returns (uint);
    function getdorPool() external view returns (uint);
}

contract DorPoolFarm {
    ...
    function deposit(uint _amount) public {
        ...
        _amount = sub(afterAmount,beforeAmount);
    ...
    function updateReward() internal {
        ...
        uint256 yield = exchequer.dorPool(address(this));
    ...
    function beharvest(address usr) public view returns (uint256) {
        uint lpSupply = lptoken.balanceOf(address(this));
        uint256 yield = exchequer.getdorPool();
        uint256 lpReward = div(mul(yield,uint(1e18)),lpSupply);
