...
contract BeanLabs is Context, Ownable {
    ...
    function getpotionsSinceLastHireTESTREMOVE(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(potions_TO_Hire_1chemist,SafeMath.sub(block.timestamp,lastHire[adr]));
        uint256 timeBetweenReorganizationAndHire = lastHire[adr] > lastreorganization ? SafeMath.sub(lastHire[adr], lastreorganization) : 0;
        uint256 reduction;

        if (lastHire[adr] > lastreorganization) {
            // Only reduce the potions made between the hire and the reorganization
            reduction = SafeMath.mul(timeBetweenReorganizationAndHire, Factory[adr]).div(100).mul(lastrebasepercent);
        } else {
            reduction = 0;
        }
