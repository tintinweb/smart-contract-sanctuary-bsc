...
contract ShadowFiRaffle is Ownable, ReentrancyGuard {
    uint256 private mainPool;
    uint256 private secondPool;
    uint256 private thirdPool;

    uint256 private mainPoolPercent = 7142;
    uint256 private secondPoolPercent = 1429;
    uint256 private thirdPoolPercent = 1429;

    uint256 private mainCarryPercent = 7500;
    uint256 private secondCarryPercent = 2500;
    ...
    mapping (address => uint256) public ticketsOwnedByUser;
    ...
    function setPoolPercents(uint256 _mainPoolPercent, uint256 _secondPoolPercent, uint256 _thirdPoolPercent) public onlyOwner {
        ...
        require(thirdPoolPercent <= 2858, "20% is the largest amount allowed to Treasury");
    ...
    function setSessionParameters(uint256 _sessionTime, uint256 _deadAccrual, uint256 _percentToMainPool, uint256 _percentToSecondPool) public onlyOwner {
        require(mainCarryPercent + secondCarryPercent == 10000, "Main + Secondary percents must equal 100%.");
    ...
    function revealWinner() public onlyCourt {
        ...
        if (winningUser == DEAD_ADDRESS || winnerTicketNo == 0 || raffleInfos[currentRaffleId].lastSoldTicketNo == 0) {
            ...
            uint256 payout = (mainPool * (92 + mainPoolBonusPercent)) / 100;
    ...
    function deposit(address user, uint256 ticketAmount) external payable {
        ...
        for (uint256 ticketNo = lastTicketNo + 1; ticketNo <= lastTicketNo + (ticketCount * accrual1); ticketNo++) {
            ...
            ticketsOwnedByUser[user] = raffleInfos[currentRaffleId].lastSoldTicketNo + 1;
    ...
    function _startSession(uint256 _sessionId, uint256 _sessionTime, uint256 _deadAccrual, uint256 _mainCarryPercent, uint256 _secondCarryPercent) internal {
        ...
        mainCarryPercent = _mainCarryPercent;
        secondCarryPercent = _secondCarryPercent;
    ...
    function getPoolInfo() public view returns (uint256 mainPoolBNB, uint256 mainPoolPercentage, uint256 mainCarryover, uint256 secondPoolBNB, uint256 secondPoolPercentage, uint256 secondCarryover, uint256 treasuryPercentage) {
        return (mainPool, mainPoolPercent, mainCarryPercent, secondPool, secondPoolPercent, secondCarryPercent, thirdPoolPercent);
    }
