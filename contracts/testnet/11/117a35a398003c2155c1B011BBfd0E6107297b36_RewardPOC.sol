...
contract RewardPOC is Pausable {
    ...
    function approverSignaturePUB(Reward memory reward) external view virtual returns (address signer) {
        bytes32 structHash = keccak256(abi.encode(reward._amount, reward._holder, address(this), reward._ref, _nonces[reward._holder].current(), reward._deadline));
        signer = recover(structHash, reward._signature);
    }

    function recoverPUB(bytes32 structHash, bytes memory _signature) external view virtual returns (address signer) {
