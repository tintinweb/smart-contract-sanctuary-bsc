...
contract TheBillionPage is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    uint256 public finalPrice = 0.05 ether;
    uint256 public whitelistedPrice = 0.02 ether;

    uint256 public maxSupply = 123210;
    uint256 public preSale1Amount = 12321;
    uint256 public preSale2Amount = 26613;

    mapping(address => bool) public whitelisted;
    mapping(address => uint256) public addressMintedBalance;
    uint256 public perWhitelistAddressLimit = 2;
        ...
        returns (uint256)
    {
        ...
        if (msg.sender != owner()) {
            if (whitelisted[msg.sender] == true && supply <= preSale1Amount) {
                uint256 ownerMintedCount = addressMintedBalance[msg.sender];
                require(
                    ownerMintedCount + 1 <= perWhitelistAddressLimit,
                    "The maximum per address reached"
                );

                require(
                    msg.value >= whitelistedPrice,
                    "The amount sent must be equal to whitelisted price"
                );
    ...
    function getCurrentPrice() public view returns (uint256) {
        ...
        if (supply < preSale1Amount) {
            price = (finalPrice * 9) / 100;
        } else if (supply < preSale2Amount) {
            price = (finalPrice * 50) / 100;
        } else {
            price = finalPrice;
        }
        return price;
    ...
    function isCoordinatesOwned(uint256 metadataID) public view returns (bool) {
        return existingIDs[metadataID] == true;
    }

    //!  ONLY OWNER
    // Price change functions is used in case BNB price goes crazy
    function setFinalPrice(uint256 _newFinalPrice) public onlyOwner {
        finalPrice = _newFinalPrice;
    }

    function setWhitelistedPrice(uint256 _whitelistedPrice) public onlyOwner {
        whitelistedPrice = _whitelistedPrice;
    }
