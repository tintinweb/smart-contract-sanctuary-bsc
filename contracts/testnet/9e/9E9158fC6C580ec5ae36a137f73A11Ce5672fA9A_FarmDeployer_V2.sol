...
contract FarmDeployer_V2 is
    ...
    IFarmDeployer
{
    // account -> true: Account can deploy special farms
    mapping(address => bool) public hasFarmDeployerRole;
    uint256 public deploymentCost;
    uint256 public maxLockTime;
    uint256 public incomeFee;
    address payable public feeReceiver;

    address public erc20MintingFarmImplementation;

    event DeployedERC20MintingFarm(
        address farmAddress,
        address stakeToken,
        address rewardToken,
        uint256 startBlock,
    ...
    event SetFarmsImplementations(
        address erc20MintingFarmImplementation
    );
    event FarmDeployerRoleSet(address _account, bool _enable);

    modifier onlyFarmDeployer() {
        require(hasFarmDeployerRole[msg.sender], "Only farm deployer!");
        _;
    }
    ...
    /*
     * @notice Deploys ERC20MintingFarm contract. Requires amount of BNB to be paid
     ...
     * @dev Since we can guaranty proper rewards only for approved reward tokens, this farm should not be deployed by random wallets
     * @dev This farm should be deployed only with preapproved tokens and settings
     * @dev This restricted farm will be free to deploy.
     */
    function deployERC20MintingFarm(
        address _stakeToken,
        address _rewardToken,
        uint32 _startBlock,
        uint256 _rewardPerBlock,
        uint256 _userStakeLimit,
        uint32 _minimumLockTime,
        uint16 _earlyWithdrawalFee,
        ...
        farmAddress = createMiniProxy(erc20MintingFarmImplementation);
        IERC20MintingFarm(farmAddress).initialize(
            _stakeToken,
            _rewardToken,
            _startBlock,
            _rewardPerBlock,
     ...
     * @param _erc20MintingFarmImplementation: ERC20 minting farm with fixed reward per block implementation address
     */
    function setImplementations(
        address _erc20MintingFarmImplementation
    ) external onlyOwner {
        require(_erc20MintingFarmImplementation != address(0), "Invalid address");
        erc20MintingFarmImplementation = _erc20MintingFarmImplementation;

        emit SetFarmsImplementations(
            _erc20MintingFarmImplementation
        );
    }
    ...
    /*
     * @notice Allows/disallows specific account to deploy restricted farms
     * @param account Account address
     * @param enable true - allow, false - disallow
     */
    function setFarmDeployerRole(address account, bool enable) external onlyOwner {
        require(hasFarmDeployerRole[account] != enable, "Already set");
        hasFarmDeployerRole[account] = enable;

        emit FarmDeployerRoleSet(account, enable);
    }
...
contract MiniProxyCreator {

    /**
     * @notice Creates mini proxy
     * @param logic Implementation address
     * @return newProxy Deployed proxy address
     */
    function createMiniProxy(address logic) internal returns(address newProxy){
