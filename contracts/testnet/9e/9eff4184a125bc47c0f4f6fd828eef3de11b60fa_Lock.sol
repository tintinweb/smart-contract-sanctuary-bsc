...
contract Lock is BasicMetaTransaction {
    ...
    event deposited(address depositer,address token, uint256 amount);
    event spotted(address depositer,address token, bool check, uint256 amount, address vault);
    ...
    function deposit(uint256 _amount, address token) external payable {
        ...
        emit deposited(_msgSender(), token, _amount);
    }
