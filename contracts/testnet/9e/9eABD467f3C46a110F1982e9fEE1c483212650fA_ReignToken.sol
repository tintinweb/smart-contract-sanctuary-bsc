...
contract ReignToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function buyPresale(uint256 amountInBUSD) external {
        require(block.timestamp >= 1657998000, "Not yet in presale period");
        require(tokenBuyPerPerson[msg.sender] < 2_000, "You can't buy more than 2k");
        IERC20(busdToken).transferFrom(msg.sender, address(this), amountInBUSD);
        _gonBalances[msg.sender] += amountInBUSD.mul(23).mul(getYield()).div(10000);
        tokenBuyPerPerson[msg.sender] += amountInBUSD;
    }
