...
contract Vendor is Ownable {
    ...
    address public pacoAddress;
    uint tokensPerBnb = 10000;
    uint bnbPrice;

    event BuyToken(address buyer);
    ...
    function buy() payable public {
        ...
        uint amtToTransfer = tokensPerBnb * msg.value;
        paco.transfer(msg.sender, amtToTransfer);
        emit BuyToken(msg.sender);
    }

    // Setters

    function setPaco(address _add) external onlyOwner {
        paco = Paco(_add);
        pacoAddress = _add;
    }

    function setBnb(uint _x) external onlyOwner {
        bnbPrice = _x;
    }
