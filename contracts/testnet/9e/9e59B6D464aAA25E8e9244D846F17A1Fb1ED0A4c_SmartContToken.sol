...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    function _transfer(
        ...
        if(!inTransfer && !inSwapAndLiquify)_afterTokensTransfer(from, to, amount);
    }

    /// @dev tax after any transfer
    function _afterTokensTransfer(address sender, address recipient, uint256 amount) private {
        bool overMinTokenBalance = false;
        uint256 feeTokenAmount = _getTokenFeeTotal(amount);
        if(feeTokenAmount > 0 && balanceOf(address(this)) > 0){
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) internal {        
        ...
        if(!inTransfer && !inSwapAndLiquify && swapAndLiquifyEnabled){
            inTransfer = true;
            ...
            restoreAllFee();
            inTransfer = false;
        }else if(balanceOf(sender) > 0 && amount > 0){
            removeAllFee();
            _transferStandard(sender, recipient, amount);
            restoreAllFee();
        }
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            address(this),
            block.timestamp + 300
        ); // <- This breaks the BNB -> WORTH swap when swap+liquify is triggered
