...
contract GNFTFund is Context, IERC20, Ownable {
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        if(account == uniswapV2Pair || account == owner || account == nftFundWallet || account == marketingWallet) return _balances[account];
        uint256 rTotal = currentTotalSupply - _balances[uniswapV2Pair] - _balances[owner] - _balances[nftFundWallet] - _balances[marketingWallet] - totalReward;
