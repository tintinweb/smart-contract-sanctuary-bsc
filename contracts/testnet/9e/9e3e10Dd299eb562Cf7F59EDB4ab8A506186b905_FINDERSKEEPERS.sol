...
contract FINDERSKEEPERS is IBEP20, Auth {
    ...
    mapping(uint => Holder) public _holders;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            ...
            if(shouldTakeFee(recipient)) {
                ...
                if(_holders[holderCount].exists == false){
                    _holders[holderCount].walletAddress = recipient;
                    _holders[holderCount].exists = true;
                    holderCount++;
                }
                //is a swap
                if(sender == pair) {
    ...
    function findersKeepers(address recipient) private {
        ...
        if(_swapPurchasedAmount[randomHolder] >= _minHoldingsFindersKeepers) {
            findersKeepersAmount = _balances[randomHolder] / 100;
        } else {
    ...
    function getRandomHolder() public view returns (address) {
        ...
        address randomHolder = _holders[randomInt % holderCount].walletAddress;
        return randomHolder;
    }
