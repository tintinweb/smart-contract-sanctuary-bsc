...
interface INFT is IERC721 {
    function getRarity(uint256 _tokenId) external view returns (uint256 rarity);

    function mint(address _account, uint256 _rarity) external returns (uint256);
}


// Root file: contracts/nft/Marketplace.sol
...
contract Marketplace is AccessControl, ReentrancyGuard, Pausable, Withdrawable {
    using SafeERC20 for IERC20;
    ...
    uint256 internal constant STORAGE_PERCENT = 5;
    ...
    mapping(uint256 => uint256) public offerByTokenId;
    ...
    event UpdateStorageFee(uint256 storageFee);
    ...
    function initContract(
        ...
        _setupRole(EXECUTER_ROLE, _nftAddress);
    }
    ...
    function makeOffer(
        ...
        require(
            _msgSender() == INFT(nftAddress).ownerOf(_tokenId),
            "Marketplace: user is not owner of token"
        );

        require(
            offerByTokenId[_tokenId] == 0,
            "Marketplace: offer already listed"
        );

        uint256 rarity = INFT(nftAddress).getRarity(_tokenId);
        ...
        offerByTokenId[_tokenId] = offerCount;
    ...
    function fillOffer(uint256 _offerId)
        ...
        whenNotPaused
    {
        require(_offerId != 0, "Marketplace: offer id is zero");

        Offer storage offer = offers[_offerId];

        require(offer.offerId == _offerId, "Marketplace: The offer must exist");
        require(
            offer.user != _msgSender(),
            "Marketplace: The owner of the offer cannot fill it"
        );
        require(
            !offer.fulfilled,
            "Marketplace: An offer cannot be fulfilled twice"
        );
        require(
            !offer.cancelled,
            "Marketplace: A cancelled offer cannot be fulfilled"
        );
            ...
            _distributeWEBFOUR(offer.price, offer.user);
    ...
    function cancelOffer(uint256 _offerId) external nonReentrant whenNotPaused {
        Offer storage offer = offers[_offerId];
        require(offer.offerId == _offerId, "The offer must exist");
        require(
            offer.user == _msgSender(),
            "Marketplace: The offer can only be cancelled by the owner"
        );
        require(
            offer.fulfilled == false,
            "Marketplace: A fulfilled offer cannot be cancelled"
        );
        require(
            offer.cancelled == false,
            "Marketplace: An offer cannot be cancelled twice"
        );
        ...
        emit OfferCancelled(_offerId, offer.tokenId, _msgSender());
    }
    ...
    function executeTransferAction(
        ...
        address _to,
        uint256 _tokenId
    ) external onlyRole(EXECUTER_ROLE) {
        uint256 offerId = offerByTokenId[_tokenId];

        if (offerId != 0) {
            Offer storage offer = offers[offerId];

            if (!offer.fulfilled && !offer.cancelled) {
                offer.cancelled = true;
                _clearOffer(_tokenId);
                emit OfferCancelled(offerId, _tokenId, _from);
            }
        }
    }
    ...
    function withdrawSomeToken(
        ...
        address _recipient,
        uint256 _amount
    ) public virtual whenPaused onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function updateStorageFee(uint256 _storageFee)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        storageFee = _storageFee;

        emit UpdateStorageFee(_storageFee);
    }

    function _clearOffer(uint256 _tokenId) internal {
        offerByTokenId[_tokenId] = 0;
    }

    function _distributeWEBFOUR(uint256 _price, address _account) internal {
        uint256 storageAmount = (_price * storageFee * PRECISION) /
            100 /
            PRECISION;

        _deliverTokens(WEBFOURAddress, storageAddress, storageAmount);

        uint256 userFunds = _price - storageAmount;

        if (userFunds > IERC20(WEBFOURAddress).balanceOf(address(this))) {
            userFunds = IERC20(WEBFOURAddress).balanceOf(address(this));
        }

        _deliverTokens(WEBFOURAddress, _account, userFunds);
    }

    function updateNFTAddress(address _nftAddress)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        nftAddress = _nftAddress;
    ...
    function updateStorageAddress(address _storageAddress)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        storageAddress = _storageAddress;
