...
contract Claiming is Initializable, ContextUpgradeable, OwnableUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    bool public isAllowedWithdraw = true;
    ...
    function getAvailableTokens(uint256 _totalAllocation) public returns (uint256) {
    ...
    function _getAvailableTokens(uint256 _totalAllocation) internal returns (uint256) {
        ...
        for (uint256 _periodIndex = 0; _periodIndex < unlocksNumber; _periodIndex++) {
            ...
            if (block.timestamp >= _unlockPeriod.startDate) {
                ...
                if (_unlockPeriod.isUnlockedBeforeStart) {
                    unlocks[_periodIndex].isUnlockedBeforeStart = false;
                    if (_unlockPeriod.periodUnit == 0) {
                        claims[_msgSender()].totalClaimed += _calculatePercentage(_totalAllocation, _unlockPeriod.tokensPercentage);
                    } else {
                        claims[_msgSender()].totalClaimed += _calculatePercentage(_totalAllocation, _unlockPeriod.tokensPercentage * ((_unlockPeriod.endDate - _unlockPeriod.startDate) / _unlockPeriod.periodUnit + 1));
                    }
                    continue;
                }
                if (_unlockPeriod.periodUnit == 0) {
