...
contract Quiz is ERC721URIStorage, Ownable {
    ...
    mapping (uint256 => QuizItem) public quitList;
    ...
    function createQuizProject (uint256 _quizID, uint256 _beginTimem, uint256 _endTime) public {
        ...
        require(quitList[_quizID].status != 0, "The ID has been occupied");
        require(_beginTimem < _endTime, "Time anomaly");

        quitList[_quizID].status = 1;
        quitList[_quizID].beginTime = _beginTimem;
        quitList[_quizID].endTime = _endTime;
    }


    function editQuizProject (uint256 _quizID, uint256 _beginTimem, uint256 _endTime, uint256 _result) public {
        require(operators[msg.sender], "Operators only");
        require(quitList[_quizID].status == 1, "The ID has been occupied");
        require(_beginTimem < _endTime, "Time anomaly");
        
        quitList[_quizID].beginTime = _beginTimem;
        quitList[_quizID].endTime = _endTime;
        quitList[_quizID].result = _result;
    }


    function createQuitRoom (uint256 _quizID, uint256 _nftID, uint256 _oddsType) public {
        require(ownerOf(_nftID) == msg.sender, "You are not the owner");
        require(quitList[_quizID].status == 1, "Abnormal state");
        require(block.timestamp >= quitList[_quizID].beginTime && block.timestamp < quitList[_quizID].endTime, "Abnormal state");
