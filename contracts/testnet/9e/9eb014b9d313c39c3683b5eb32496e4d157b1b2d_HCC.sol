...
contract HCC is ERC721, Ownable, ERC721Enumerable, ReentrancyGuard, VRFConsumerBaseV2 {
	...
	function mintReservedNFT(address[] calldata _to, uint32[] calldata _count) external onlyOwner nonReentrant{
		...
		for(uint i=0; i < _to.length; i++){
			...
			callbackGasLimit = _count[i] * 150000;
			RESERVED_MINTED += _count[i];
		    uint256 s_requestId = COORDINATOR.requestRandomWords(keyHash,s_subscriptionId, requestConfirmations, callbackGasLimit, _count[i]);
	...
	function mintSaleNFT(uint32 _count) external payable nonReentrant{
		...
		callbackGasLimit = _count * 150000;
		SALE_MINTED += _count;
		users[msg.sender].salemint = users[msg.sender].salemint + _count;
		uint256 s_requestId = COORDINATOR.requestRandomWords(keyHash,s_subscriptionId, requestConfirmations, callbackGasLimit, _count);
	...
	function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
	   ...
	   for (uint i=0; i< randomWords.length; i++) {
	       _safeMint(minter, (randomWords[i] % (SALE_NFT + RESERVED_NFT)) + 1);
	   }
    }
