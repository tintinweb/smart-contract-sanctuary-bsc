...
contract BlucamonSummoningV2 {
    ...
    uint256 public summonFee = 0.004 ether;
    modifier onlyFounder() {
        require(msg.sender == founder, "S_SMN_100");
        _;
    }
    ...
    function setBuyBackPercent(uint256 _newPercent) external onlyFounder {
    ...
    function summon(uint256 _id) external payable {
        ...
        require(owner == msg.sender, "S_SMN_200");
        require(msg.value == summonFee, "S_SMN_300");
        (bool result, ) = blucamonOwnershipContract.call(
            abi.encodeWithSignature("summon(uint256)", _id)
        );
        require(result, "S_SMN_400");
        if (msg.value > 0) {
            uint256 buybackAmount = msg.value.mul(buybackPercent).div(100_00);
            uint256 treasuryAmount = msg.value.sub(buybackAmount);
            if (buybackAmount > 0) {
