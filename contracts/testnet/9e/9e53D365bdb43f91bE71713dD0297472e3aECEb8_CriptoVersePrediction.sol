...
abstract contract JackPot is Ownable {
  ...
  function _dailyJackpotStart() internal {
    ...
    jackpot.rewardDistribution = DailyDistribution;
  ...
  function dailyJackpotDraw(uint256 epoch) external onlyOwner {
    ...
    address[] memory addressTicketSorted;
    uint indexTicket = 0;
    ...
    for (uint256 k = 0; k < dailyJackpot.rewardDistribution.length; k++) {
      ...
      uint256 amountWining = DailyJackPot[epoch].rewardAmount * (dailyJackpot.rewardDistribution[k] / 100);
