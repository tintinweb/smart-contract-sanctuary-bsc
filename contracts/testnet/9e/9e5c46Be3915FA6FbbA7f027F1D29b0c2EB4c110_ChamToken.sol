...
contract ChamToken is Operator, IERC20 {
    ...
    constructor(address _polWallet, address _wbnbAddress, address _router) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        // Selling token
		if(marketLpPairs[recipient] && !excludedSellingTaxAddresses[sender]) {
            require(excludedAccountSellingLimitTime[sender] || block.timestamp > _lastTimeReceiveToken[sender].add(timeLimitSelling), "Selling limit time");
            if (taxSellingPercent > 0) {
                uint256 taxAmount = amount.mul(taxSellingPercent).div(10000);
                if(taxAmount > 0)
                {
                    amount = amount.sub(taxAmount);
                    _transferBase(sender, polWallet, taxAmount);
                }
            }
		} 
        // Buying token
        else if(marketLpPairs[sender] && !excludedBuyingTaxAddresses[recipient] && taxBuyingPercent > 0) {
