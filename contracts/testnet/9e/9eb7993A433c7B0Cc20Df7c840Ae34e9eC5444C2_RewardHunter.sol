...
pragma solidity ^0.8.11;
 ...
 contract Unbox is Ownable {
  ...
  // Estrutura das NFTs
  struct UnboxUser {
      uint256 simpleRarity;
      uint256 mediumRarity;
      uint256 rareRarity;
      // Id da NFT
      uint256 boxUser;
      ...
      uint256 boxLimit;
      // Atutal Dono da NFT
      address payable ownerBox;
      // Preço da NFT
      uint256 numberPercent;
      uint counterBox;
    ...
    function _generateNft(uint _index) internal returns(uint256 numberPercent) {
   ...
   function burnBox(uint _index) public {
       UnboxUser storage nft = idToMarketItem[_index];
       require(nft.ownerBox == payable(msg.sender), "RHT:Precisa ser o dono da Box");
       nft.ownerBox = payable(address(0));
   }
   ...
   function _infoBox(uint _index) public view returns( uint counterBox, uint boxLimit, uint numberPercent, bool isBox) {
    UnboxUser storage nft = idToMarketItem[_index];
    return(nft.counterBox, nft.boxLimit, nft.numberPercent, nft.isBox);
  }


  function _luckyBox(uint _index) internal returns(uint256) {
    UnboxUser storage nft = idToMarketItem[_index];
    require(nft.ownerBox == payable(msg.sender), "RHT:Precisa ser o dono da Box");
  ...
  function _unboxClass(address _usr) internal{
      require(msg.value == box1 || msg.value == box2 || msg.value == box3, "RHT:Precisa de um valor identico ao preco da box");
      if (msg.value == box1) {
...
interface IBEP20 {
 ...
 contract RewardHunter is ERC1155, Ownable, Unbox {
  ...
  struct NftUsers {
    ...
    function Boxing(uint _index) public {
      _luckyBox(_index);
       _UnboxUser(msg.sender);
    }

    function mintBox() public payable {
    ...
    function mintNFT(uint256 _amount) external payable returns(uint256) {
    ...
    function buyNfts(uint _tokenId, uint _price) public  {
    ...
    function removeMarket(uint _tokenId) external {
