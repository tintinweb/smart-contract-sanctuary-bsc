...
contract RykerVip_BUSD {
    ...
    uint256 public compoundMinimum = 5;
    uint256 private maxCompoundBonusTime = 5;

    /** time steps **/
    uint256 public cutOffTimeStep = 48 hours;
    uint256 public lotteryTimeStep = 6 hours;
    uint256 public tossCoinTimeStep = 6 hours;
    uint256 public horseBettingTimeStep = 6 hours;
    uint256 public lastDepositTimeStep = 12 hours;
    uint256 public compoundTimeStep = 24 hours; //set to 24 hours before deploying to main
    uint256 public topReferrerTimeStep = 24 hours;
    // uint256 private allowExitTimeStep = 7 days;
    ...
    uint256 public topReferrerRound;
    uint256 public topReferrerBalance;
    ...
    address[] private currentUserBettors;
    address[] private currentUserHorseBettors;
    ...
    mapping(address => UserBet) private userBetHistory;
    ...
    constructor(address devt, address lif, address mkt, address exec) {
        ...
        tokenERC20        = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); //0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
    ...
    function executeAutoCompound(address _addr) external contractActivated {
        ...
        emit AutoCompoundEvent(_addr, getCurTimeTest());
    }
    ...
    function initializeContract(address addr, uint256 amount) public onlyOwner {
        ...
        LOTTERY_START_TIME = getCurTimeTest();
        TOSS_COIN_START_TIME = getCurTimeTest();
        TOP_REFERRER_START_TIME = getCurTimeTest();
        LAST_DEPOSIT_START_TIME = getCurTimeTest();
        HORSE_RACE_START_TIME = getCurTimeTest();
    ...
    function compoundAddress(bool isCompound, address _address) internal {
        ...
        if(getCurTimeTest().sub(user.lastHatch) >= compoundTimeStep) {
    ...
    function sellProfit() public nonReentrant {
        ...
        user.lastWithdrawTime = getCurTimeTest();  
        user.lastHatch = getCurTimeTest();
        user.claimedEggs = 0;
    ...
    function invest(address ref,uint256 amount) public nonReentrant {
        ...
        if(!contractStarted){
            ...
            if(user.initialDeposit < 1) {
                ...
                userDeposit.timeStamp = getCurTimeTest();
            ...
            if(LOTTERY_ACTIVATED) {
                if(getCurTimeTest().sub(LOTTERY_START_TIME) >= lotteryTimeStep || participants >= 200 || currentPot >= maxIncentiveBalance) {
                    drawLotteryWinner();
                }
            ...
            if(LAST_DEPOSIT_ACTIVATED) {
                if(getCurTimeTest().sub(LAST_DEPOSIT_START_TIME) >= lastDepositTimeStep && totalLastDepositJackpot > 0 && potentialLastDepositWinner != address(0)) {
            ...
            if(TOP_REFERRER_ACTIVATED) {
                if(getCurTimeTest().sub(TOP_REFERRER_START_TIME) >= topReferrerTimeStep) {
                    drawTopReferrerPool();
                }
            ...
            user.yieldLastWithdrawTime = getCurTimeTest();
    ...
    function chooseWinners() external {
        ...
        if(LOTTERY_ACTIVATED && (getCurTimeTest().sub(LOTTERY_START_TIME) >= lotteryTimeStep || participants >= 200 || currentPot >= maxIncentiveBalance)) {
            drawLotteryWinner();
        }

        if(TOSS_COIN_ACTIVATED && getCurTimeTest().sub(TOSS_COIN_START_TIME) >= tossCoinTimeStep && currentHeadsBetCount > 0 && currenttailsBetCount > 0) {
            drawTossCoinRoundWinners();
        }

        if(HORSE_BETTING_ACTIVATED && getCurTimeTest().sub(HORSE_RACE_START_TIME) >= horseBettingTimeStep && currentUserHorseBettors.length > 0) {
            drawHorseRaceRoundWinners();
        }

        if(LAST_DEPOSIT_ACTIVATED && getCurTimeTest().sub(LAST_DEPOSIT_START_TIME) >= lastDepositTimeStep && totalLastDepositJackpot > 0 && potentialLastDepositWinner != address(0)) {
            drawLastDepositWinner();
        }
        
        if(TOP_REFERRER_ACTIVATED && getCurTimeTest().sub(TOP_REFERRER_START_TIME) >= topReferrerTimeStep) {
            drawTopReferrerPool();
        }
   }   
    ...
    function drawTopReferrerPool() private {
        ...
        for(uint256 i = 0; i < topReferrerBonusArr.length; i++) {
            ...
            emit PoolPayoutEvent(poolTop[i], minerRewards, getCurTimeTest());
        }
    ...
    function drawLastDepositWinner() private {
        ...
        emit LastBuyEvent(currentLastBuyRound, potentialLastDepositWinner, busdReward, minerRewards , getCurTimeTest());
        ...
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    emit LotteryEvent(currentLotteryRound, winnerAddress, minerRewards, participants, totalTickets, getCurTimeTest());
    ...
    function getRandomValue() private view returns(uint256) {
        ...
        return uint256(keccak256(abi.encode(_blockhash, getCurTimeTest(), currentPot, block.difficulty, marketEggs, getContractBalance())));
    }
    ...
    function drawTossCoinRoundWinners() private contractActivated {
        ...
        for(uint256 i = 0; i < memCurrentUserBettors.length; i++){
            ...
            userbet.tossTimeStamp = getCurTimeTest();
            userbet.roundNum = currentTossCoinRound;
        ...
        emit TossCoinEvent(currentTossCoinRound,  winner,  currentTossCoinPot,  winnerOdds, winnersCount,  getCurTimeTest());
    ...
    function drawHorseRaceRoundWinners() private contractActivated {
        ...
        emit HorceRaceEvent(currentHorseRaceRound, winner, winnerOdds, winnersCount, getCurTimeTest());
    ...
    function getEggsSinceLastHatch(address adr) public view returns(uint256) {
        uint256 secondsSinceLastHatch = getCurTimeTest().sub(users[adr].lastHatch);
    ...
    function getYieldEarnings(address adr) public view returns(uint256) {
        ...
        uint256 curTime = getCurTimeTest();
    ...
    function withdrawYieldEarnings() public nonReentrant {
        ...
        //if contract balance is less than amount to withdraw, give everything.
    ...
    // uncomment this enablers after testing
...
//////////////////////////////////////////////////////////////////REMOVE AFTER TESTING////////////////////////////////////////////////////////////////////////
    ...
    function generateTestInvestors(uint max) public {
        uint maxCt = max;
        uint iteration = 0;
        address[] memory addr = new address[](max);
        while(iteration <= maxCt){
            ...
            addr[iteration] = randomAddress;
            iteration++;
        }
        emit GeneratedUser(addr);
    }

    function investTest(address testUser) private {
        require(!isContract(testUser), "Not a valid user address.");
        ...
        user.claimedEggs = 0;
        user.lastHatch = getCurTimeTest().sub(compoundTimeStep);     
    }
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
}
