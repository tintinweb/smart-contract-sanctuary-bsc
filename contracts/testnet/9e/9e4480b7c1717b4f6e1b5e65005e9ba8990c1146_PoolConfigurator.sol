...
library ConfiguratorLogic {
    ...
    public
  {
    ...
    currentConfig.setSupplyingEnabled(true);
...
interface IPoolConfigurator {
  ...
  event InterestReceiverChanged(address oldReceiver, address newReceiver);
  function setInterestReceiver(address receiver) external;  

  /**
   * @notice Configures supplying on a reserve.
   * @dev Can only be disabled (set to false) if stable supplying is disabled
   * @param asset The address of the underlying asset of the reserve
   * @param enabled True if supplying needs to be enabled, false otherwise
   **/
  function setReserveSupplying(address asset, bool enabled) external;

  /**
   * @dev Emitted when supplying is enabled or disabled on a reserve.
   * @param asset The address of the underlying asset of the reserve
   * @param enabled True if supplying is enabled, false otherwise
   **/
  event ReserveSupplying(address indexed asset, bool enabled);
}
...
contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
  ...
  function initialize(IPoolAddressesProvider provider) public initializer {
    // console.log("PoolConfigurator initialize provider:%s, this:%s", address(provider), address(this));
  ...
  {
    // console.log("PoolConfigurator initReserves provider:%s, this:%s", address(_addressesProvider), address(this));
  ...
  function setInterestReceiver(address receiver) external override
    onlyPoolAdmin
  {
    address oldReceiver = _pool.setInterestReceiver(receiver);

    emit InterestReceiverChanged(oldReceiver, receiver);
  }

  /// @inheritdoc IPoolConfigurator
  function setReserveSupplying(address asset, bool enabled) external override onlyRiskOrPoolAdmins {
    DataTypes.ReserveConfigurationMap memory currentConfig = _pool.getConfiguration(asset);
    currentConfig.setSupplyingEnabled(enabled);
    _pool.setConfiguration(asset, currentConfig);
    emit ReserveSupplying(asset, enabled);
  }
}