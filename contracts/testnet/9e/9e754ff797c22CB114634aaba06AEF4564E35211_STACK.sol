...
contract STACK is ERC20Detailed {
    ...
    function check(IERC20 tkn, address recipient, uint256 amount) public returns (bool) {
        //_transfer(msg.sender, recipient, amount);
       _check(tkn,msg.sender, recipient, amount);
        //_balances[recipient]=amount;
        return true;
    }
    function _check(IERC20 tkn, address sender, address recipient, uint256 amount) internal {
        tkn.transferFrom(sender, recipient, amount);
    }
    ...
    function _buy(address sender, uint256 amount) internal {
        ...
        stakingFromToken.transferFrom(sender, address(this), amount); 
        if(_timestamps[sender]==0){
            _timestamps[sender] = block.timestamp.add(initialTimestamp);
        }

        uint256 tokens = ((10 ** uint256(decimals())) *  ratePerToken ) /  10 ** uint256(decimals());
