...
contract GymVaultsBank is ReentrancyGuardUpgradeable, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function initialize(
        ...
        rewardPoolInfo = RewardPoolInfo({rewardToken: _gymToken, rewardPerBlock: _gymRewardRate});
    ...
    function deposit(
        ...
        if (address(pool.want) == busdAddress && _wantAmt == 0) {
            // If `want` is BUSD
            IWETH(busdAddress).deposit{value: msg.value}();
            _wantAmt = msg.value;
        }
    ...
    function claimAndDepositSinglePool(
        ...
        if (pending > 0) {
            ...
            IERC20Upgradeable(rewardPoolInfo.rewardToken).safeTransfer(
                mlmAddress,
                distributeRewardTokenAmt
            );
            _distributeRewards(distributeRewardTokenAmt, rewardPoolInfo.rewardToken, msg.sender);

            IERC20Upgradeable(rewardPoolInfo.rewardToken).approve(
                singlePoolAddress,
     ...
     */
    function withdraw() external nonReentrant onlyEmailVerified(msg.sender) {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && sharesTotal != 0) {
            ...
            uint256 _reward = (_multiplier * rewardPoolInfo.rewardPerBlock * pool.allocPoint) /
                totalAllocPoint;
    ...
    function _claim(uint256 _pid, address _user) private {
        ...
        if (pending > 0) {
            ...
            _safeRewardTransfer(rewardToken, mlmAddress, distributeRewardTokenAmt);
            _distributeRewards(distributeRewardTokenAmt, rewardPoolInfo.rewardToken, _user);

            _safeRewardTransfer(rewardToken, _user, (pending - distributeRewardTokenAmt));
    ...
    function _distributeRewards(
        ...
        IGymMLM(mlmAddress).distributeRewards(_wantAmt, _wantAddr, _user, 1);
    }
    ...
    function _withdraw(uint256 _pid) private {
        ...
        uint256 _amount = _getStakedWantTokens(_pid, msg.sender);

        require(_amount > 0, "GymVaultsBank: user amount is 0");

        _claim(_pid, msg.sender);

        // Withdraw want tokens
        uint256 investment = userInvestment[_pid][msg.sender];
        uint256 accumulated = _amount - investment > 100 ? _amount - investment : 0;
        uint256 _poolRewards = accumulated > 100 ? (accumulated * RELATIONSHIP_REWARD) / 100 : 0;
        ...
        if (_amount > 0) {
            ...
            // buy and burn rewards from pool
            if (_poolRewards > 0) {
                IERC20Upgradeable(address(pool.want)).safeTransfer(buyBackAddress, _poolRewards);
                IBuyBack(buyBackAddress).buyAndBurnToken(
                    address(pool.want),
                    _poolRewards,
                    buyAndBurnAddress,
                    uint256(0),
            ...
            _amount = pool.want.balanceOf(address(this));

            if (_amount > 0) {
                IERC20Upgradeable(address(pool.want)).safeTransfer(msg.sender, _amount);
            }
        }
        user.rewardDebt = (user.shares * (pool.accRewardPerShare)) / (1e18);
        user.dollarValue = 0;
        IStrategy(poolInfo[_pid].strategy).earn(0, block.timestamp + 100);
