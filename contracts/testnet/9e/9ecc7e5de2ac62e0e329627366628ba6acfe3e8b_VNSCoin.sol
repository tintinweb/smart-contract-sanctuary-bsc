...
contract VNSCoin is ERC20, Ownable {
    ...
    uint256 public constant SPY = (2 * BASE_RATIO) / 100 / 1 days;
    string constant _name = "VNS TOKEN";
    string constant _symbol = "VNS";
    uint8 constant _decimals = 18;
    uint256 public constant TRANSFER_RATIO = (10 * BASE_RATIO) / 100;
    uint256 public constant NFT_RATIO = (4 * BASE_RATIO) / 100;
    uint256 public constant LP_RATIO = (3 * BASE_RATIO) / 100;
    uint256 public constant IDO_RATIO = (90 * BASE_RATIO) / 100;
    uint256 public constant IDO_PID_RATIO = (10 * BASE_RATIO) / 100;
    //换成usdt,usdc,busd的合约地址 
    address public constant _usdt = 0x02820b19E3c5b04Bfc1CE9865c96A6651eaEBE0D;
    address public constant _usdc = 0xcb3A4d68dcA7eA80A4998a1B66faBaB1206e8BA3;
    address public constant _busd = 0xFB505f32F9B7Da508fa97eD46559191314fd5985;
    address public  _nft = 0x84d2615810252a8e49D02a3e6406065667a4164A;
    address public  _lp = 0xe161A7681444D487Fe587148843e5fac5bca65eE;
    //最后更新时间
    mapping(address => uint256) public lastUpdateTime;
    mapping(address => address) public parentAdderss;
    mapping(address => bool) public isParentAdderss;
    mapping(address => bool) public parentWL;
    function addParentWL(address _addr) external onlyOwner {
        parentWL[_addr] = true;
    }

    function delParentWL(address _addr) external onlyOwner {
        delete parentWL[_addr];
    }
    ...
    function setCanInterest(bool enable) external onlyOwner {
        canInterest = enable;
    }
    address _welfare = 0x0E0289dAe83EeEBD72c47B3425b42BD232Cc136d;
    ...
    function idoUSDT(address pid,uint256 amount) external  {
        if(pid == address(0)){
            ...
            if(isParentAdderss[msg.sender]){
                pid = parentAdderss[msg.sender];
            }
            parentAdderss[msg.sender] = pid;
            isParentAdderss[msg.sender] = true;
        }
        require(amount >= _coinConfig._commonValueMin, "amount need > min");
        uint256 idoAmount = amount.mul(_coinConfig._rate_denominator.div(_coinConfig._rate_numerator));
        uint256 idoAmountUsdt  = amount.mul(IDO_RATIO).div(BASE_RATIO);
        uint256 PidAmountUsdt  = amount.mul(IDO_PID_RATIO).div(BASE_RATIO);
        ...
        _mint( address(this) , idoAmount * 2);
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        ...
        address owner = _msgSender();
        require(
            canTransfer || whitelist[owner] || whitelist[to],
            "can not transfer"
        );
        require(
        ...
        if(!isTransferFromWL(owner)){
            fee = amount.mul(TRANSFER_RATIO).div(BASE_RATIO);
            //税费逻辑 需要分三份来处理  销毁地址/nft均分/lp均分
            _transfer(owner, address(0), amount.mul(LP_RATIO).div(BASE_RATIO));
            _transfer(owner, _nft, amount.mul(NFT_RATIO).div(BASE_RATIO));
            _transfer(owner, _lp, amount.mul(LP_RATIO).div(BASE_RATIO));
        }
    ...
    function transferFrom(
        ...
        require(
            canTransfer || whitelist[from] || whitelist[to],
            "can not transfer"
        );
        require(
        ...
        if(!isTransferFromWL(from)){
            ...
            _transfer(from, address(0), amount.mul(LP_RATIO).div(BASE_RATIO));
            _transfer(from, _nft, amount.mul(NFT_RATIO).div(BASE_RATIO));
            _transfer(from, _lp, amount.mul(LP_RATIO).div(BASE_RATIO));
        }
    ...
    //计算收益
    function calculateReward(address _addr) internal virtual {
        if (_addr != address(0)) {
            uint256 reward = getReward(_addr);
            if (reward > 0) {
                _balances[_addr] = _balances[_addr].add(reward);
                _balances[address(this)] = _balances[address(this)].sub(reward);
                if(isParentAdderss[_addr] && parentWL[parentAdderss[_addr]]){
                    _balances[parentAdderss[_addr]] = _balances[parentAdderss[_addr]].add(reward.mul(IDO_PID_RATIO).div(BASE_RATIO));
                    _balances[address(this)] = _balances[address(this)].sub(reward.mul(IDO_PID_RATIO).div(BASE_RATIO));
                }
            }
            lastUpdateTime[_addr] = lastTime();
        }
    }
    function getReward(address account) public view returns (uint256) {
        if (lastUpdateTime[account] == 0 || isInterestBlackList(account)|| !canInterest) {
            return 0;
        }
        return _balances[account].mul(SPY).div(BASE_RATIO).mul(
