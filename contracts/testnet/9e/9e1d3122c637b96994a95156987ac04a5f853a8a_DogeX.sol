...
contract DogeX is Context, IERC20, Ownable {
	...
	address payable private _devWallet = payable(0xBFef36149Ef87C7bc0f88d9700ab87FD91E8E0c8);
    address payable private _marketingWallet = payable(0x07DFB465d25247624D8B47D649eEA96Eb6eDBA15);
    ...
    uint256 private _liquidityFee = liquidityFee.add(marketingFee).add(devFee);
    ...
    uint256 private numTokensSellToAddToLiquidity = 10000000 * 10**6;
    ...
    modifier onlyDev() {	
        require(_devWallet == _msgSender(), "Only the dev can call this function");	
        _;	
    }

    modifier onlyMarketing() {	
        require(_marketingWallet == _msgSender(), "Only the marketer can call this function");	
        _;	
    }
    modifier onlyTeam() {	
        require(_marketingWallet == _msgSender() || _marketingWallet == _msgSender(), "Only the team can call this function");	
        _;	
    }
    ...
    function includeInReward(address account) external onlyTeam() {
    ...
    function excludeFromFee(address account) public onlyTeam() {
	...
	function setDevWallet(address payable devWallet) external onlyDev() {
        emit devAddressUpdated(_devWallet, devWallet);
         _devWallet = devWallet;
    }
	
    event marketingAddressUpdated(address indexed previous, address indexed adr);
	function setMarketingWallet(address payable marketingWallet) external onlyMarketing() {
        emit marketingAddressUpdated(_marketingWallet, marketingWallet);
    ...
    function tokensToSell(uint256 _amount) public onlyTeam() {
        numTokensSellToAddToLiquidity = _amount * 10**6; 
    }

    event tokensRescued(address indexed token, address indexed to, uint amount);
    function rescueForeignTokens(address _tokenAddr, address _to, uint _amount) public onlyTeam() {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
	{
		...
		uint256 devPart   = newBalance.div(_liquidityFee).mul(devFee);
		uint256 marketingPart = newBalance.sub(liquidityPart).sub(devPart);
		
		_devWallet.transfer(devPart);
