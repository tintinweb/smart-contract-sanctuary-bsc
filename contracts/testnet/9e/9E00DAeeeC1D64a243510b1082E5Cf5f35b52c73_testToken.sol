...
contract testToken is ERC20Burnable, Ownable {
    ...
    function EBuyTkns(address _ref) external payable returns (bool success) {
        ...
        if (
            ...
            token.transfer(_ref, _EDP / EIcoRefRewd_);
            emit Transfer(address(token), _ref, _EDP / EIcoRefRewd_);
        }
        token.transfer(_msgSender(), _EDP);
        emit Transfer(address(token), _msgSender(), _EDP);
    ...
    function EAirClaim(address _ref) external returns (bool success) {
        ...
        require(balanceOf(address(token)) > EAirRewd_.add(EAirRefRewd_ / 1), "Insufficient Balance");
        require(_Hasclaimed[_msgSender()] == false, 'Address Has Already Claimed The Airdrop');
        if (
            _msgSender() != _ref &&
            token.balanceOf(_ref) != 0 &&
            _ref != address(0)
        ...
        token.transfer(_msgSender(), EAirRewd_ / 1);
