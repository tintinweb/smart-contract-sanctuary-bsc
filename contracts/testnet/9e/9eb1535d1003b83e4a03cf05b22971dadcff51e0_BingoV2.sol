...
contract BingoV2 is AccessControl, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => bool)) public roundWinCards; // epoch => card no. => is win
    ...
    function startRound(uint256 epoch, uint256 prevEpoch, uint16[] memory prevWinCards, uint prevPrize) external onlyRole(DEFAULT_ADMIN_ROLE) {
        currentEpoch = epoch;
        _endRound(prevEpoch, prevWinCards, prevPrize);
    ...
    function _endRound(uint epoch, uint16[] memory winCards, uint prize) internal {
        Round storage round = rounds[epoch];
        round.isEnded = true;
    ...
    function forceEndRound(uint epoch, uint16[] memory winCards, uint prize) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _endRound(epoch, winCards, prize);
    }
