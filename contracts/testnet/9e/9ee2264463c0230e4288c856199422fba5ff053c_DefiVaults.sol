...
contract DefiVaults is ERC20, Ownable{
    ...
    function deposit(uint256 deposit_amount) public {
        uint256 _fee = deposit_amount.mul(depositFee).div(100);
    ...
    function withdraw() public {
        ...
        uint256 _fee = busdToWithdraw.mul(withdrawFee).div(100);
    ...
    function performSwaps(Params[] calldata data) public  {
        for(uint256 i = 0; i < data.length; i++) {
            ...
            {
                uint256 _tradeFee = newbalanceBuyToken.sub(oldbalanceBuyToken).mul(tradeFee).mul(100);
