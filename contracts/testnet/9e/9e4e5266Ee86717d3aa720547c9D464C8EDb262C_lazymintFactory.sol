...
contract lazymintFactory is AccessControl {
    ...
    constructor(address _market) {
        ...
        _setupRole(CREATE_ROLE, admin);
        _setupRole(CREATE_ROLE, 0x1637cc6d7c2849BEB5DC0F07546D57c1b5FfcEBf); //Francisco.
        _setupRole(CREATE_ROLE, 0x7Af53A6599628AE87F77A4F7a4bA82fE999CE0BA); //Eduardo. 
        _setupRole(CREATE_ROLE, 0x8f674D0666f9ec9681D4040FDF1Ab8D128ea5a33); //Eulalio. 
        market = _market;
    }
    ...
    ///@dev --> The address of the metadata: _uri
    ...
    function create(
        ...
        if (!hasRole(CREATE_ROLE, msg.sender)) {
            revert ("is not a creator");
        }
        address _name = collections[name_];
        if (_name != address(0)) {
            revert ("collection exists");
        }
        LazyNFT lazynft = new LazyNFT(market, _maxsupply, _presaleSupply, name_, symbol_, _uri, _collection, price, presalePrice);
        collections[name_] = address(lazynft);
        emit contractcreated(address(lazynft), msg.sender);
    }

    function getAddress(string memory _name) public view returns (address _collection){
...
contract LazyNFT is AccessControl, ERC721P {
    ...
    uint256 internal valuation;
    uint256 internal price;
    uint256 internal presale;
    string private _base;
    string public _uriCollection;
    bool preSaleFlag;
    ...
    address public admin = 0x30268390218B20226FC101cD5651A51b12C07470;
    address public mortgagecontroller;
    ...
    constructor(
        ...
        mortgagecontroller = 0x30268390218B20226FC101cD5651A51b12C07470;
        ...
        _setupRole(MINTER_ROLE, mortgagecontroller);
         _setupRole(MINTER_ROLE, msg.sender);
        maxsupply = _maxsupply;
        presaleSupply = _presaleSupply;
        _base = _baseUri;
        _uriCollection = _collection;
        price = _price;
    ...
    function nftcost() public view returns (uint256 _price){
        return valuation / maxsupply;
    }

    function setApprovalForAll(address operator, bool approved) public virtual override{
        if(operator == mortgagecontroller && approved == false){
            revert("cannot remove the approve");
        }
        super.setApprovalForAll(operator,approved);
    }

    function setValuation(uint256 _newValuation) public {
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
    ...
    function setmortgagecontroller(address _newcontroller) public {
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            revert ("have no admin role");
        }
        mortgagecontroller = _newcontroller;
    }
...
import "./IERC721PMetadata.sol";
import "@openzeppelin/contracts/utils/Address.sol";
import "@openzeppelin/contracts/utils/Context.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/utils/introspection/ERC165.sol";

abstract contract ERC721P is Context, ERC165, IERC721P, IERC721PMetadata {
    ...
    mapping(uint256 => OwnerData) private _owners;

    // Mapping owner address to token count
    mapping(address => BalanceData) private _balances;
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165, IERC165) returns (bool) {
        return
            interfaceId == type(IERC721P).interfaceId ||
            interfaceId == type(IERC721PMetadata).interfaceId ||
    ...
    function balanceOf(address owner) public view virtual override returns (uint256) {
        if(owner == address(0)){
            revert("address zero is not a valid");
        } 
    ...
    function holdInfo(uint256 tokenId) public view virtual returns (uint32) {
        OwnerData memory owner = _owners[tokenId];
        if(owner.owner == address(0)){
            revert("invalid token ID");
        }   
        uint32 _lastTransfer = owner.lastTransfer;
        return _lastTransfer;
    }

    function mintInfo(address _owner) public view virtual returns (uint32) {
        BalanceData memory balance = _balances[_owner];
        if(_owner == address(0)){
            revert("invalid address");
        }   
        uint32 _mintAmount = balance.mintedAmount;
        return _mintAmount;
    }
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        if(to == address(0)){
            revert("mint to the zero address");
        } 
        if(_exists(tokenId)){
        ...
        _balances[to].balance += uint32(1);
        _balances[to].mintedAmount += uint32(1);
        _owners[tokenId].owner = to;
        _owners[tokenId].lastTransfer = uint32(block.timestamp);
    ...
    function _transfer(
        ...
        if(ERC721P.ownerOf(tokenId) != from){
            revert("transfer from incorrect owner");
        }
        if(to == address(0)){
            revert("transfer to the zero address");
        }
 ...
 */
interface IERC721PMetadata is IERC721P {
    /**
