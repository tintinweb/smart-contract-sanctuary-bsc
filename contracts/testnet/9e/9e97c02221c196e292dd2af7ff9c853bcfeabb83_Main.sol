...
Ownable
{
  ...
  //nft price in usdt
  mapping(address => uint256) public nftPriceMap;
  ...
  ) public
  {
    ...
    threeLpAddress = _threeLpAddress;
    fee = _fee;
  }

  function setPrice(address _nft, uint256 _price) onlyOwner external {
    nftPriceMap[_nft] = _price;
  }
  ...
  function buyNFT(address _nft, address _token) public {
      require(ERC20(threeAddress).balanceOf(msg.sender) >= fee, "buy: fee not good");
      uint256 price = nftPriceMap[_nft];
      require(price > 0, "buy: _nft price not good");
      require(_token == usdtAddress || _token == threeAddress, "buy: _token not good");
      uint _price = price;
      if (_token == threeAddress) {
        _price = price.div(getThreePrice(times)).div(times);
        require(ERC20(threeAddress).balanceOf(msg.sender) >= fee.add(_price), "buy: balance not good");
      } else {
      ...
      ERC20(threeAddress).safeTransferFrom(
        address(msg.sender),
        address(this),
        fee
      );
  ...
  function getThreePrice(uint256 _times) public view returns (uint256) {
    uint256 balanceThree = ERC20(threeAddress).balanceOf(threeLpAddress);
    uint256 balanceUsdt = ERC20(usdtAddress).balanceOf(threeLpAddress);
    return balanceUsdt.mul(_times).div(balanceThree);
  }
