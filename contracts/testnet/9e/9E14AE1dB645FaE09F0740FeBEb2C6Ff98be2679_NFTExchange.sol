...
import {MintedNFT} from "../common/Structs.sol";
import { PEM721 } from "../721/PEM721.sol";
...
contract NFTExchange is Upgradable {
    ...
    * set the address of the super admin
    ...
    function getListOfMintedNFTsByGroupID(string memory _groupId) public view returns (MintedNFT[] memory) {
        return mintedAmount[_groupId];
    }

    function getAmountOfMintedNFTsByGroupID(string memory _groupId) public view returns (uint256) {
        return mintedAmount[_groupId].length;
    }
    
    function getOwnerOfTokenID(uint256 _tokenId) public view returns (address) {
        return PEM721(pem721Address).ownerOf(_tokenId);
    }
...
import { PEM1155 } from "../1155/PEM1155.sol";
import { PEM721 } from "../721/PEM721.sol";
import { MintedNFT } from "./Structs.sol";
...
contract Upgradable is ReentrancyGuarded {
    ...
    mapping (uint256 => uint256) soldQuantity;          // Token ID -> soldQuantity
    mapping (bytes => uint256) soldQuantityBySaleOrder; // signature -> soldQuantity
    mapping (string => MintedNFT[]) mintedAmount;       // Group ID -> minted NFTs
    
    
    event AdminMintEvent(
    ...
    function mintBatch721(
        ...
        uint256[] memory _data,     // (0) total copies, (1) onSaleQuantity
        string[] memory _str        // (0) uri, (1) internalTxID
    ) public reentrancyGuard {
        require(isAdmin(msg.sender), "AOG: Caller is not admin");
        require(_data[0] > 0, "AOG: Out of copy");
        require(_data[0] - _data[1] - mintedAmount[_str[1]].length >= _tokenIds.length, "AOG: Mint amount exceeds current amount of copies");
        
        PEM721(pem721Address).mintBatch(msg.sender, _tokenIds, _str[0], bytes(_str[1]));

        for(uint256 i = 0; i < _tokenIds.length; i++) {
            mintedAmount[_str[1]].push(MintedNFT(_tokenIds[i], msg.sender));
        }
        
        emit AdminMintEvent(msg.sender, _tokenIds, _str[1]);
    }
...
struct SaleOrder {
    ...
    uint256 saleOrderSalt;
    address creator;    // the creator's address, who will receive the royalty fee
...
contract PEM721 is Ownable, ERC721URIStorage {
    ...
    function mintBatch (
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            _safeMint(_receiver, _tokenIds[i], _data);
...
contract PEM1155 is Ownable, ERC1155{
    ...
    function mintBatch (
        ...
        _mintBatch(_receiver, _tokenIds, _amount, _data);
        for(uint256 i = 0; i < _tokenIds.length; i++) {
            setURI(_tokenIds[i], _uri[i]);
        }
    }
