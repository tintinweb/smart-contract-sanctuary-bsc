...
interface ReferralsContractInterface {
    ...
    function calculate_referral_father_fee(uint256 _amount, address _childReferral)
        external
        view
        returns (uint256);
...
interface GamesPoolContractInterface {
    ...
    function set_init_reserved_amount(address _address) external payable;

    function delete_reserved_amount(address _address) external payable;
}
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        set_new_treasury_ids_internal(_treasuries);
    ...
    function play_game(string memory _gameName, uint8 _bet)
        public
        payable
        nonReentrant
    {
        ...
        uint256 fee = calculate_current_transaction_fee_internal(_gameName);
        ...
        send_fees_and_referrals((msg.value * fee) / 10_000, msg.sender);
        otherContract.play{value: newAmount}(_bet, msg.sender);
    }

    function update_game(Structs.GameWithName memory newGame)
        public
        payable
        onlyOwner
    {
        ...
        if (!gamesPoolContract.reservedAmount(newGame.gameAddress).isPresent) {
            gamesPoolContract.set_init_reserved_amount{value: 1}(
                newGame.gameAddress
            );
        }
    ...
    function delete_game(string memory _gameName) public payable onlyOwner {
        assert_one_yocto();
        require(
            games[_gameName].gameAddress != address(0),
            "Game does not exists"
        );
        gamesPoolContract.delete_reserved_amount{value: 1}(games[_gameName].gameAddress);
    ...
    function update_referral_contract_address(address _referralsContractAddress)
        public
        payable
        onlyOwner
    {
    ...
    function update_games_pool_contract_address(
        address _gamesPoolContractAddress
    ...
    function calculate_current_transaction_fee(string memory _gameName)
        public
        payable
        returns (uint256)
    {
        return calculate_current_transaction_fee_internal(_gameName);
    }

    function set_new_treasury_ids(Structs.Treasuries[] memory _treasuries)
        public
        payable
        onlyOwner
    {
    ...
    function get_treasuries_length() public view returns (uint256) {
        return treasuries.length;
    }

    function get_keyListgamesAddresses_length() public view returns (uint256) {
    ...
    function calculate_current_transaction_fee_internal(string memory _gameName)
        internal
        returns (uint256)
    {
        require(
    ...
    function set_new_treasury_ids_internal(
        Structs.Treasuries[] memory _treasuries
    ) internal {
    ...
    function send_fees_and_referrals(uint256 _amount, address _caller)
        internal
    {   
        uint256 _referralFeeAmount = referralsContract.calculate_referral_father_fee(_amount, _caller);
        if (_referralFeeAmount != 0) {
            referralsContract.add_calculated_father_fee{
