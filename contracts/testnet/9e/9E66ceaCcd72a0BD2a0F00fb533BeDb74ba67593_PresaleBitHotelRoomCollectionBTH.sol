...
contract PresaleBitHotelRoomCollectionBTH is AccessControl, ERC721Holder, Pausable, ReentrancyGuard {
    ...
    function initializer(GameNFT[] calldata gameNfs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i = 0; i < gameNfs.length; i++) {
            ...
            // solhint-disable-next-line reason-string
            require(
                address(gameNft.collection) != address(0),
                "BitHotelRoomCollection: collection address is the zero address"
            );
            // solhint-disable-next-line reason-string
            require(gameNft.tokenId != 0, "BitHotelRoomCollection: tokenId is zero");
            addGameNft(collectionAddress, gameNft.tokenId, gameNft.uri, gameNft.royaltyValue, gameNft.price);
        }
    }
     ...
     *
     */
    function bulkRemoveDiscountedAddresses(address[] calldata discountedAddresses)
        external
    ...
    function setRoomInfos(
        ...
        BitHotelRoomCollection(collectionAddress).setRoomInfos(
            tokenId,
            number,
            floorId,
            roomTypeId,
    ...
    function getAmountsOut(uint256 amountIn) public view virtual returns (uint256[] memory) {
     ...
     */
    function gameNFTbyTokenId(address collection, uint256 tokenId)
        public
        view
        returns (
            uint256,
    ...
    function presetTokenIdByCollection(address collectionAddress, uint256 starWith)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function buy(
        ...
        if (isWhitelistEnabled()) {
            ...
            require(
                ...
                "PresaleBitHotelRoomCollection: sender is not whitelisted"
            );
            // solhint-disable-next-line reason-string
        ...
        // solhint-disable-next-line reason-string
        require(
            balanceBefore >= bthAmount,
            "PresaleBitHotelRoomCollection, not enough BTH token in beneficiary wallet"
        );
    ...
    function _safeMint(
        ...
        BitHotelRoomCollection(collection).safeMint(beneficiary, tokenId, uri, wallet(), royaltyValue, data_);
    }
}
