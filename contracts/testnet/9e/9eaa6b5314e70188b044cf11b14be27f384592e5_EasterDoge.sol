...
Tokenomics
Buy Tax:
Reflection in EDOGE | Liquidity pool | Marketing wallet in BNB
...
contract EasterDoge is Context, IERC20, Ownable {
  ...
  uint256 private _tTotal = 10 * 10**12 * 10**18; // 10 Trillion Tokens
  uint256 private _rTotal = (MAX - (MAX % _tTotal));
  uint256 private _tFeeTotal;
  uint256 public maxWalletAmount = 10 * 10**12 * 10**18; 
  string private _name = 'EasterDoge';
  ...
  address private _marketingWalletAddress = 0xB899dd2822b2fc60403ad8fFb34914d710823f3B; // Marketing Tax in BNB
  address public _buybackWallet = 0xB899dd2822b2fc60403ad8fFb34914d710823f3B; // in BNB

  uint256 private launchedAt; 
  bool private manualLaunch = false;

  IUniswapV2Router02 public immutable uniswapV2Router;
  address public immutable EasterDogeUniswapV2Pair;
  ...
  constructor() {
      address _newOwner = 0xEeE24561F7CF7675D99c505C11e7f12Ce9B88d87; // must change this before deployment
    ...
    EasterDogeUniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
 ...
 function setSellFees() private {
  ...
  function _transfer(
    ...
    if ((!launched() && to == EasterDogeUniswapV2Pair) || manualLaunch) {
      require(
        balanceOf(from) > 0,
    ...
    if (antiwhaleEnabled && ((from != owner() && to != owner()) || !(_isExcludedFromTxLimit[from]))) {
      require(
        amount <= _maxTxAmount,
        'EasterDoge: Transfer amount exceeds the maxTxAmount.'
      );
    }
    if(!_isExcludedFromMaxWalletLimit[from] && !_isExcludedFromMaxWalletLimit[to] && to != EasterDogeUniswapV2Pair) {
  ...
  function _tokenTransfer(
    ...
    if(takeFee && sender == EasterDogeUniswapV2Pair) {
        // buy
        setBuyFees();
    } else if(takeFee && recipient == EasterDogeUniswapV2Pair) {
        // sell
        setSellFees();
    }
    ...
    if(takeFee && (sender == EasterDogeUniswapV2Pair || recipient == EasterDogeUniswapV2Pair)) {
        restoreAllFee();
    }
