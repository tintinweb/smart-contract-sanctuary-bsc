...
contract Teomb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 Teomb-BNB LP -> Teomb pool
    // uint256 public constant INITIAL_Teomb_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the Teomb ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("Teomb Token", "Teomb") {
        // Mints 1 Teomb to contract creator for initial pool setup
    ...
    function _getTeombPrice() internal view returns (uint256 _TeombPrice) {
        try IOracle(TeombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Teomb: failed to fetch Teomb price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _TeombPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_TeombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setTeombOracle(address _TeombOracle) public onlyOperatorOrTaxOffice {
        require(_TeombOracle != address(0), "oracle address cannot be 0 address");
        TeombOracle = _TeombOracle;
    }
    ...
    /**
     * @notice Operator mints Teomb to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Teomb to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentTeombPrice = _getTeombPrice();
            currentTaxRate = _updateTaxRate(currentTeombPrice);
            if (currentTeombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_TeombPool != address(0), "!_TeombPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_TeombPool, INITIAL_Teomb_POOL_DISTRIBUTION);
    ...
    function getRemainingTimeForTeombSellingEnable() public view returns(uint256) {
        ...
        /// @dev overrides transfer function to meet tokenomics of Teomb
