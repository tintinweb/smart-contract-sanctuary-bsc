...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 1 * 1e10 * 1e18; //must hold 10,000,000,000+ tokens
    }
...
contract ToonKingV3 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event poolWalletUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("ToonKingV3", "TKTv3"){
        address newOwner = msg.sender; // update to 0xC7D8C9050C39918BB3025f687555c30e69095780
    
        uint256 totalSupply = 1000000000000000 * 1e18;
        
        maxTransactionAmount = totalSupply * 1 / 1000; // 0.5% maxTransactionAmountTxn
        swapTokensAtAmount = totalSupply * 1 / 10000; // 0.05% swap tokens amount
        ...
        poolSellFee = 0;
        totalSellFees = rewardsSellFee + marketingSellFee + liquiditySellFee + poolSellFee;
    	...
    	marketingWallet = address(0xed7790247b555d8ded6a3e58ED63100991B3Bfda); 
        poolWallet = address(0xf8E7566e8c48A8F067341E8934C9F99cC587Da47); 
        liquidityWallet = address(0x9F5B59428d4E844B3387C2cDc003ed16c4A66008); 
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _poolFee) external onlyOwner {
        marketingSellFee = _marketingFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
        poolSellFee = _poolFee;
        totalSellFees = marketingSellFee + rewardsSellFee + liquiditySellFee + poolSellFee;
    ...
    function updatepoolWallet(address newWallet) external onlyOwner {
        require(newWallet != address(0), "cannot set to 0 address");
        excludeFromFees(newWallet, true);
        emit poolWalletUpdated(newWallet, poolWallet);
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForRewards + tokensForpool;
        ...
        uint256 ethForpool= ethBalance.mul(tokensForpool).div(totalTokensToSwap);
        ...
        (success,) = address(poolWallet).call{value: ethForpool}("");
