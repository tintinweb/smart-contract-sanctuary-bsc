...
contract SoulBonus is DivestorUpgradeable {
    ...
    function _getSameDay() private returns (uint) {
        ...
        if (tempQuotient != userInfos[_msgSender()].lastSettleDay) { // 不是当天
            _settleDay(userInfos[_msgSender()].lastSettleDay, tempQuotient); // 结算
            userInfos[_msgSender()].lastSettleDay = tempQuotient;
        }
    ...
    // 代币进入记录
    function inputTokenRecord(uint add) public {
        require(_msgSender() == addr.soulControllerAddr, "owner err");
        require(add > 0, "inv param");

        bonusPool[_getSameDay()] += add;
    }
    ...
    function receiveTokenApi() public {
        ...
        UserInfo storage userInfo = userInfos[_msgSender()];
        uint lp = userInfo.lp - dayToDayToken[sameDay][_msgSender()].lp;
    ...
    function searchUserBonusApi(address player_) view public returns (UserInfo memory, DayTokenTotal memory, uint, uint) {
        uint sameDay = userInfos[player_].lastSettleDay;
        uint tempQuotient = block.timestamp / DAY;

        uint notGet = _searchUserBonus(player_, sameDay, tempQuotient);

        UserInfo storage nowUserInfo = userInfos[player_];

        DayTokenTotal storage dayToken = dayToDayToken[tempQuotient - 1][player_];
        ...
        uint lp = nowUserInfo.lp - dayToDayToken[sameDay][player_].lp;

        return (userinfo, totalDayToken[tempQuotient - 1], bonusPool[tempQuotient - 1], lp);
    }
}
...
interface ISoulController {
    function divide(address recommender_, address caster_, uint amount) external;
}
