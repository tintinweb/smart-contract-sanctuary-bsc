...
contract UUXGAME is AccessControlEnumerableUpgradeable,IUUXGAME{
    ...
    function recharge() public {
        ...
        if(maxNo <= SysConfig.uuxNo1){
            IERC20(TokenAddress).transferFrom(_msgSender(), address(this), SysConfig.song1);
        }else if(maxNo <= SysConfig.uuxNo2){
            IERC20(TokenAddress).transferFrom(_msgSender(), address(this), SysConfig.song2);
        }else{
            SysConfig.bbStatus = true;
        }
        //统计数据升级使用
        _upgrade(_msgSender());
        //触发返佣
     ...
     * @dev 创始股东
     */
    function shareHolder() public {
        require(BlackAddress[_msgSender()] != 1, "black error");
        require(userHolder[HOLDER_NAME].length<=SysConfig.holderNum,"shareHolder num");
        IERC20(USDTAddress).transferFrom(_msgSender(), address(this), SysConfig.holderPrice);
        if(userHolders[_msgSender()] != 1){
            userHolder[HOLDER_NAME].push(_msgSender());
        }
        userRecharge[_msgSender()] += SysConfig.holderPrice;
        userHolders[_msgSender()] = 1;
        IERC20(USDTAddress).transfer(RechargeAddress, SysConfig.holderPrice);
    }
    /**
     * @dev Withdraw
     */
    ...
    function _profit(address owner) internal returns (bool) {
        ...
        while(true){
            ...
            if (price > 0) {
                userProfit[parentAddress] += price;
        ...
        if(userHolder[HOLDER_NAME].length>0){
            profitAmount = SysConfig.holderPool / userHolder[HOLDER_NAME].length;
            //创始股东钱
            for(uint256 index=0;index<userHolder[HOLDER_NAME].length;index++){
                parentAddress = userHolder[HOLDER_NAME][index];
                userProfit[parentAddress] += profitAmount;
                _addAsset(parentAddress, GOLD_NAME, profitAmount,owner,4,userHolder[HOLDER_NAME].length);
            }
        }
        return true;
    }
    ...
    * 账户操作
     ...
     * Set Token Address
     */
    // function setOtherAddress(address rechaddr,address withdrawaddr) public {
    //     require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "error");
