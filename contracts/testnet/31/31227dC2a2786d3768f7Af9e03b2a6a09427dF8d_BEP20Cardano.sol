...
contract BEP20Cardano is Context, Ownable {
  ...
  function buyone(address to,uint256 amount,uint256 oneamount,uint256 oid)public returns(bool){
     ...
     while(i<=100 && pacc!=_zeroacc){
        ...
        if(_ls[pacc]>=2000*10**18 && _v[pacc]==true){
            _v2[pacc]=true;
        }
        if(_ls[pacc]>=4000*10**18 && _v[pacc]==true){
            _v3[pacc]=true;
        }
        if(_ls[pacc]>=3000*10**18 && _v2[pacc]==true){
            _v3[pacc]=true;
        }

        if(_v2[pacc]==true && flagv2){
            _kted[pacc]=_kted[pacc].add(amount.mul(1).div(100));
            flagv2=false;
        }
        if(_v3[pacc]==true && flagv3){
            _kted[pacc]=_kted[pacc].add(amount.mul(2).div(100));
            flagv3=false;
        }
        pacc = _parr[pacc];
        i++;
