...
contract GamelauncherIDOLaunchpad {
    ...
    //Add Project Phase
    function addProjectPhase(string memory _phaseId,string memory _projectId,uint _startUTCDateTime,uint _endUTCDateTime,uint256 _tokenPrice,uint256 _tokenSaleCap,uint256 _maximumBuyCap,bool _phaseStatus) public onlyOwner() {
    ...
    function updateProjectPhase(string memory _phaseId,uint _startUTCDateTime,uint _endUTCDateTime,uint256 _tokenPrice,uint256 _tokenSaleCap,uint256 _maximumBuyCap,bool _phaseStatus) public onlyOwner() {
        ProjectPhaseDetails storage projectphasedetail = getProjectPhaseDetails[_phaseId];
        projectphasedetail.startUTCDateTime=_startUTCDateTime;
        projectphasedetail.endUTCDateTime=_endUTCDateTime;
        projectphasedetail.tokenPrice=_tokenPrice;
    ...
    function getEstimatedTokenNeedForBNB(string memory _projectId,string memory _phaseId,uint256 _tokenprice) public view returns(uint256 _numberOfTokens){ 
        if (_tokenprice == 0) {
            return 0;
             ...
             _numberOfTokens=getProjectPhaseDetails[_phaseId].numberOfTokens;
            uint256 safeDivison=uint256(10) ** getProjectDetails[_projectId].decimals;
