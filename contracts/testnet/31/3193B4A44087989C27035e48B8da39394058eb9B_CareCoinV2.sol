...
contract CareCoinV2 is ERC20, Ownable {
    ...
    function updateBuyFees(
        ...
        {
        require(
            (launchedAt > 0 && launchedAt + 5 minutes < block.timestamp) ||
    ...
    function setAntibotStatus(bool _antibotEnabled) external onlyOwner {
        require(
            block.timestamp < launchedAt + 7 minutes ||
            !_antibotEnabled,
    ...
    function _transfer(
        ...
        if(antibotEnabled && launchedAt > 0){
            if (launchedAt + 7 minutes > block.timestamp) 
            {
        ...
        if(takeFee) {
            uint256 _totalFees;
            if (launchedAt + 5 minutes > block.timestamp && from == uniswapV2Pair) 
            {
                _totalFees = 0;
            }
