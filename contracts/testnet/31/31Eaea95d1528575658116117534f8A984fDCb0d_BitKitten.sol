...
contract BitKitten is IERC20 {
    ...
    string constant private _name = "BitKitten";
    string constant private _symbol = "BKT";
    ...
    address payable public marketingWallet = payable(0xe46d3712f28CB0F340BCe2f8f48C17A7D3c6a418);
    ...
    uint256 public lpBurnPercent = 200;
    uint256 public lpBurnTimeLimit = 15 minutes;
    uint256 private lpLastBurnStamp;
...
//================================================ BLACKLIST
    ...
    function setLpBurnEnabled(bool enabled) external onlyOwner {
        lpBurnEnabled = enabled;
    }

    function setLpBurnTimeLimit(uint256 timeInSeconds) external onlyOwner {
        require(timeInSeconds >= 15 minutes, "Cannot set below 15 minutes.");
        lpBurnTimeLimit = timeInSeconds;
    }

    function setLpBurnPercent(uint256 percent) external onlyOwner {
        require(percent <= 200, "Cannot set above 2%.");
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (sell) {
            if (!inSwap) {
                ...
                if (lpBurnEnabled) {
                    if (block.timestamp - lpLastBurnStamp >= lpBurnTimeLimit) {
                        uint256 burnAmount = (_tOwned[lpPair] * lpBurnPercent) / masterTaxDivisor;
                        lpLastBurnStamp = block.timestamp;
                        _tOwned[lpPair] -= burnAmount;
                        _tOwned[DEAD] += burnAmount;
                        emit Transfer(lpPair, DEAD, burnAmount);
                        IV2Pair(lpPair).sync();
                    }
                }
            }
