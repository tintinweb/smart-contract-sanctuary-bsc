    ...
    ReentrancyGuard
{
    constructor(
        ...
        address launchpad,
        uint256 maxSupply
    ) ERC721("Leftover Loserz", "LoL") {
        baseURI = baseURI_;
        LAUNCHPAD = launchpad;
        LAUNCH_MAX_SUPPLY = maxSupply;
        mintMode = MintMode.launchpad;
    }
    ...
    function mintTo(address to, uint256 size) external onlyLaunchpad {
        require(mintMode == MintMode.launchpad, "launchpad end");
        ...
        require(total + size <= LAUNCH_MAX_SUPPLY, "max supply reached");
    ...
    // mint after launch pad
    function mint(uint256 size) external nonReentrant {
        require(mintMode == MintMode.open, "mint not start");
        require(minted[msg.sender] == false, "already minted");
        require(size > 0, "size must greater than zero");
        require(size <= 10, "size must less than 10");
        uint256 total = ERC721Enumerable.totalSupply();
        require(total + size <= LAUNCH_MAX_SUPPLY, "max supply reached");
    ...
    function startLaunchpad() external onlyOwner {
        mintMode = MintMode.launchpad;
    }
