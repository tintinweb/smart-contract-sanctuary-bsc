    ...
    VRFConsumerBaseV2Upgradable
{
    ...
    {
        uint256 updatedStakesAmount = poolToStakesAmount[
            requestIdToPoolId[requestId]
        ] - poolToDeletedStakesAmount[requestIdToPoolId[requestId]];
        uint256 updatedGoldAmount = (updatedStakesAmount *
            pools[requestIdToPoolId[requestId]].goldTicketChance) / 10000;
        uint256 updatedSilverAmount = (updatedStakesAmount *
            pools[requestIdToPoolId[requestId]].silverTicketChance) / 10000;

        uint256[] memory goldArray = new uint256[](updatedGoldAmount);
        uint256[] memory silverArray = new uint256[](updatedSilverAmount);

        for (uint256 i = 0; i < updatedSilverAmount; i++) {
            uint256 finalNumber = randomWords[i] %
        ...
        for (
            ...
            if (
                ...
                goldArray[i - requestIdToSilverAmount[requestId]] = finalNumber;
            }
    ...
    function withdrawStake(uint256 stakeId_)
        ...
        stageShouldNotBeFinished(stakeId_)
    {
        require(
            block.timestamp <=
