...
contract BullRunGame is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) public IsStaked; // whether or not a wallet is staked
    ...
    uint8 public constant AlphaID = 4;
    ...
    // TOPIA: 0x01f2063Ec3cbeB709aF133cD5BD149962207f551
    // COINFLIP: 0xbDc24108825128F89F2fE04a75dbcEbD13f7eAEd
    // Randomizer: 0x00725cbf4fc5c8ed006747f4abdad11d0243e018
    // BULLPEN: 0x70Ef2e1b4DFDb47e9F0fd82Bf703023Ee09Ed8bd
    // ARENA: 0xAd83eF9A2123aF708e99A28d93A836021cC77a7F
    ...
    modifier isStaked() {
        require(IsStaked[msg.sender] , "Caller is not staked");
        _;
    }
    ...
    function setBatchNFTType(uint16[] calldata _idNumbers, uint8[] calldata _types) external onlyOwner {
        require(_idNumbers.length == _types.length , " _idNumbers.length != _types.length: Each token ID must have exactly 1 corresponding type!");
        for (uint16 i = 0; i < _idNumbers.length; i++) {
            require(_types[i] != 0 && _types[i] <= 3 , "Invalid nft type - cannot be 0 or greater than 3");
    ...
    // fxn to claim daily topia rewards for staking. Matadors do not get daily topia.
    function claimDailyTopia(uint16[] calldata _idNumbers) public nonReentrant requireContractsSet allowedContractsOnly() isStaked() {
    ...
    allowedContractsOnly() {
        ...
        for (uint16 i = 0; i < BetNFTsPerEncierro[msg.sender][_encierroId].length; i++) { // fetch their bet NFT ids for this encierro
            ...
            "Caller address != tokenID Owner address");
    ...
    function stakeRunner(uint16 tokenID) internal {
        ...
        if (!IsStaked[msg.sender]) {IsStaked[msg.sender] = true;}
    ...
    function unstakeMany(uint16[] calldata _tokenIds) external payable nonReentrant requireContractsSet allowedContractsOnly() {
        claimDailyTopia(_tokenIds);

        require(tx.origin == msg.sender, "Only EOA");
        // require(msg.value == SEED_COST, "Invalid value for randomness");
        uint256 numWords = _tokenIds.length;
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            if (NFTType[_tokenIds[i]] == 1) {
                unstakeRunner(_tokenIds[i], seed[i]);
            } else if (NFTType[_tokenIds[i]] == 2) {
                unstakeBull(_tokenIds[i], seed[i]);
            } else if (NFTType[_tokenIds[i]] == 3) {
                unstakeMatador(_tokenIds[i]);
            } else if (NFTType[_tokenIds[i]] == 0) {
                continue;
            }
        }
        if (NumberOfStakedNFTs[msg.sender] == 0) {IsStaked[msg.sender] = false;}
    ...
    function unstakeRunner(uint16 tokenID, uint256 seed) internal {
        ...
        "Staking token must belong to owner!");
        require(IsNFTStaked[tokenID] , 
        "NFT is not staked");
        ...
        // make sure there are matadors available to steal bulls
        if (BullpenInterface.bullCount() > 0) {

            if ((seed % 100) < Risk[1]) { 
                // nft gets stolen
                address thief = BullpenInterface.selectRandomBullOwnerToReceiveStolenRunner(tokenID);
    ...
    function unstakeBull(uint16 tokenID, uint256 seed) internal {
        ...
        // make sure there are matadors available to steal bulls
        if (ArenaInterface.matadorCount() > 0) {

            if ((seed % 100) < Risk[2]) { 
                // nft gets stolen
                address thief = ArenaInterface.selectRandomMatadorOwnerToReceiveStolenBull(tokenID);
    ...
    function unstakeAlpha(uint16[] calldata _tokenIDs) external nonReentrant requireContractsSet allowedContractsOnly() {
    ...
    function stakeManyAlphas(uint16[] calldata _tokenIds) external nonReentrant {
        require(tx.origin == msg.sender, "Only EOA");
        for (uint i = 0; i < _tokenIds.length; i++) {
            require(IERC721(Alpha).ownerOf(_tokenIds[i]) == msg.sender, "You don't own this token");
    ...
    function claimManyAlphas(uint16[] calldata _tokenIds, bool unstake) external nonReentrant requireContractsSet allowedContractsOnly() isStaked() {
        require(tx.origin == msg.sender, "Only EOA");
        uint256 owed = 0;
        for (uint i = 0; i < _tokenIds.length; i++) { 
            require(StakedAlphaInfo[_tokenIds[i]].owner == msg.sender, "Don't own the given token");
    ...
    function getUnclaimedGenesis(uint16 tokenId) requireContractsSet external view returns (uint256 owed) {
        if (NFTType[tokenId] == 1) {
            owed = (runnerRewardMult * ((block.timestamp - StakedNFTInfo[tokenId].stakeTimestamp) / 1800));
        } else if (NFTType[tokenId] == 2) {
            owed = (bullRewardMult * ((block.timestamp - StakedNFTInfo[tokenId].stakeTimestamp) / 1800));
    ...
    allowedContractsOnly() {
        ...
        require(IERC20(TopiaTokenContract).balanceOf(address(msg.sender)) >= (_betAmount * _tokenIds.length), "You don't have enough TOPIA to place this bet!");
        require(_choice == 1 || _choice == 0, "Choice can only be 0 (for bulls), or 1 (for runners)");
        require(Encierros[_encierroId].status == Status.Open, "This encierro must be open to bet!");
        require(_betAmount >= Encierros[_encierroId].minBet && _betAmount <= Encierros[_encierroId].maxBet, "Bet not within max/min limits");

        uint16 numberOfNFTs = uint16(_tokenIds.length);
        uint256 totalBet = _betAmount * numberOfNFTs;
        for (uint i = 0; i < numberOfNFTs; i++) {
            require(StakedNFTInfo[_tokenIds[i]].owner == msg.sender, "Staking token must belong to owner!");
    ...
    function betRunner(uint16 _runnerID, uint256 _encierroId, uint256 _betAmount, uint8 _choice) internal {
        ...
        require(IsNFTStaked[_runnerID] , "Token IDs must be staked to place bets on them!");
        require(StakedNFTInfo[_runnerID].owner == msg.sender , "Caller address != tokenID Owner address");
    ...
    function betBull(uint16 _bullID, uint256 _encierroId, uint256 _betAmount, uint8 _choice) internal {
        ...
        require(IsNFTStaked[_bullID] , "Token IDs must be staked to place bets on them!");
        require(StakedNFTInfo[_bullID].owner == msg.sender , "Caller address != tokenID Owner address");
   ...
   function betMatador(uint16 _matadorID, uint256 _encierroId, uint256 _betAmount, uint8 _choice) internal {
        ...
        require(IsNFTStaked[_matadorID] , "Token IDs must be staked to place bets on them!");
        require(StakedNFTInfo[_matadorID].owner == msg.sender , "Caller address != tokenID Owner address");
    ...
    function startEncierro(
        ...
        require(
            ...
            "Encierro must be closed, claimable, or voided"
        );

        require(
            ((_endTime - block.timestamp) >= minDuration) && ((_endTime - block.timestamp) <= maxDuration),
            "Encierro length outside of range"
        );
    ...
    function closeEncierro(uint256 _encierroId) external nonReentrant {
        require(Encierros[_encierroId].status == Status.Open , "Encierro must be open first");
        require(block.timestamp > Encierros[_encierroId].endTime, "Encierro not over yet");
        _fetchRandomNumber();
   ...
   */
    ...
    function flipCoinAndMakeClaimable(uint256 _encierroId) external nonReentrant requireContractsSet allowedContractsOnly() returns (uint256) {
        ...
        require(Encierros[_encierroId].status == Status.Closed , "Enceirro must be closed first");
...
interface IArena {
    ...
    function selectRandomMatadorOwnerToReceiveStolenBull(uint16 _stolenBullID) external view returns (address);
...
interface IBullpen {
    ...
    function selectRandomBullOwnerToReceiveStolenRunner(uint16 _stolenRunnerID) external returns (address);
