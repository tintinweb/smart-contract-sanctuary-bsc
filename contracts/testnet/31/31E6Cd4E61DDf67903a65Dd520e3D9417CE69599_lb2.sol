...
interface IluckblocksNodes {
    function updateNodeInfoE(address caller, uint256 reward) external returns (bool);
...
contract lb2 is VRFConsumerBase,ReentrancyGuard,Ownable {
	using SafeMath for uint256;
	...
	IERC20 eth = IERC20(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
    ...
    address ethTrading = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    ...
    uint256 minimumPot = 5000 * 10**18;
    uint256 botReward = 15 * 10**18;
    ...
    uint internal TICKET_VAL = 200000000000000000000; //minimum amount (in wei) for getting registered on list
    uint internal KrstmPremium = 5000000000000000000; // Amount in KRSTM for discount
	uint internal prizeValue = TICKET_VAL.div(2); //half ticket value to pay raffle prize
    uint internal sustainingFee = prizeValue.mul(1).div(100); // 0.5% of sustaining fee
    
    uint internal jackpotFeed = prizeValue.mul(99).div(100); //amount (in wei) which buy the crypto to be added to the jackpot
    // > total allocation is 50% to raffle () 0.50% to fee () 49.50% to jackpot
    uint internal bonusHolder = prizeValue.mul(39).div(100); //amount (in wei) which buy the crypto to be added to the jackpot with discount for those who hold KRSTM - 30% discount on top of ticket 
	...
	uint256 internal jackpot = eth.balanceOf(address(this)) * 80/100; // current jackpot - 20% accumulates for next jackpot
	...
	event LotteryLog(uint256 timestamp,address adrs, string message,uint number,uint number2,string message2,uint choosennumber1,uint choosennumber2, uint choosennumber3);
        ...
        )
    {
        ...
        priceFeed = AggregatorV3Interface(0x143db3CEEfbdfe5631aDD3E50f7614B6ba708BA7); // eth/USD Price feed
    ...
    function autoSpin(address _caller, uint256[] calldata _nodes) public nonReentrant {
        ...
        require(getJackpotinUSD() >= minimumPot && registeredsAccts.length > 10 && block.timestamp > lastAutoSpin + 900,"autoSpin parameters not met");

        if(lbNodes.getUserActivation(_caller) == false){
            lbNodes.activateNodes(_caller,_nodes,3);
        }

        if(block.timestamp > lastAutoSpin + 960){
            lbNodes.resetQueue(3);
        } else if(lbNodes.updateNodeInfoE(_caller,botReward) == true){
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override onlyVRFCoordinator {
        ...
        if(jackpotNumber1 == choosenNumber1 && jackpotNumber2 == choosenNumber2 || jackpotNumber1 == choosenNumber2 && jackpotNumber2 == choosenNumber1 || jackpotNumber1 == choosenNumber3 && 
        jackpotNumber2 == choosenNumber2 || jackpotNumber1 == choosenNumber3 && jackpotNumber2 == choosenNumber1 || jackpotNumber1 == choosenNumber1 && jackpotNumber2 == choosenNumber3 
        || jackpotNumber1 == choosenNumber2 && jackpotNumber2 == choosenNumber3) {
            ...
            if(registeredsAccts.length > 1){
                ...
                eth.safeTransfer(lastJackpotWinner,jackpot);
                ...
                jackpot = eth.balanceOf(address(this)) * 80/100;

                emit LotteryLog(block.timestamp,lastJackpotWinner, "Jackpot is hit!  Jackpot Numbers was: ",jackpotNumber1,jackpotNumber2,"Player choosen numbers was: ",choosenNumber1,choosenNumber2,choosenNumber3);
            ...
            emit LotteryLog(block.timestamp,lastWinner, "We have a new raffle Winner but no jackpot! Jackpot Numbers was: ",jackpotNumber1,jackpotNumber2,"Player choosen numbers was: ",choosenNumber1,choosenNumber2,choosenNumber3);
        }
    }

    // Play Function
    function BuyTicket(address player,uint number1,uint number2,uint number3) public nonReentrant {
        ...
        require(number1 <= 25 && number1 > 0 && number2 <= 25 && number2 > 0 && number3 <= 25 && number3 > 0 
        && number1 != number2 && number1 != number3 && number2 != number3 ,"Failed: numbers has to be between 1 and 25 and not be equal.");
       
        addressMapping[player] = true;
        	
        if(player != derivativesContract){
         Registered memory registered = Registered(0,number1,number2,number3,0,0,0,0);
         registereds[player] = registered;
        }else{
         Registered memory registered = Registered(derivativesDraws+3,number1,number2,number3,0,0,0,0);
         registereds[player] = registered;
         derivativesDraws += 3;
        }
      ...
      if(krstm.balanceOf(player) >= KrstmPremium && player != derivativesContract ){
         usd.transferFrom(payable(player),address(this),TICKET_VAL.mul(70).div(100) - sustainingFee);
         swapTokens(bonusHolder);
      }else{
         usd.transferFrom(payable(player),address(this),TICKET_VAL - sustainingFee);
         swapTokens(jackpotFeed);
      }
         usd.transferFrom(payable(player),stakingContract,sustainingFee);

         emit LotteryLog(block.timestamp,player, "Ticket Bought, Good luck!",0,0,"Ticket Numbers :",number1,number2,number3);
    ...
    function ForfeitTicket(uint256 index) public nonReentrant {
       ...
       require(getJackpotinUSD() >= 400 * 10**18, "Contract need to have atleast 400 USD in value.");
