...
// File: Contracts/PlutoVoting.sol
...
contract PlutoVoting is IVoting, owned {
    ...
    function addProposals (string memory newProposal, uint40 startTime, uint40 endTime) public {
        ...
        IERC20(PLUTO).transferFrom(msg.sender, address(this), (proposalFee * 10 ** uint256(decimals)));
    ...
    function voteProposal(string memory proposal, bool choice) public override {
        ...
        IERC20(PLUTO).transferFrom(msg.sender, address(this), (amount * 10 ** uint256(decimals)));
        emit votedProposal(proposal, choice);
    }

    function withdraw() public onlyOwner {
      require(IERC20(PLUTO).balanceOf(address(this)) > 0, "Balance is 0");
      require(withdrawAddress != address(0), "the withdraw address is invalid");
        (bool os, ) = payable(withdrawAddress).call{
            value: address(this).balance
        }("");
        IERC20(PLUTO).transfer(withdrawAddress, IERC20(PLUTO).balanceOf(address(this)));
        require(os);
   }
