...
contract Referals is Ownable {
    
    address public top = 0x7Af53A6599628AE87F77A4F7a4bA82fE999CE0BA;
    address[] empty;
    struct User {
        address inviter;
        address self;
    ...
    function enter(address payable inviter) external payable {
        require(tree[msg.sender].inviter == address(0), "No puedes registrarte mas de una vez con el mismo patrocinador");
        require(tree[inviter].self == inviter, "Ese patrocinador no existe");
        require(tree[inviter].myReferred.length < 90, "Tu patrocinador ya alcanzo su maximo de invitados");
