...
contract Test_Casa_V1 is AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable, IERC721ReceiverUpgradeable,ReentrancyGuardUpgradeable{
    ...
    address payable public cutWalletAddress; 
    address public usdtWalletAddress;
    ...
    uint256 public transactionCut;
    address public transactionCutWalletAddress;

    Sale[] public SALES_ALL;
    ...
    function initialize() public initializer {
        ...
        cutWalletAddress = payable(msg.sender);
        _isPaused = false;
        transactionCut = 500;
    }
    ...
    function _createSale(uint256 _nftId,uint256 _sellingPrice, uint256 _carType) external {
      ...
      Sale memory sale = Sale(saleId,_nftId,msg.sender,_sellingPrice,1,_carType);
    ...
    function _buy(uint256 _nftId) external {
      Sale memory sale = SALES[_nftId];
      require(sale.status == 1 && sale.seller != address(0),"Sale not created or already cancelled!");
    ...
    function retSales() external view returns(Sale[] memory _sales){
      Sale[] memory new_data = new Sale[](SALES_ALL.length);
      for(uint256 i=0;i< SALES_ALL.length;i++){
        Sale memory sale = SALES_ALL[i];
        Sale memory map_sale = SALES[sale.nftId];
        Sale memory new_sale = Sale(sale.id,sale.nftId,sale.seller,sale.sellingPrice,map_sale.status,sale.carType);
        new_data[i] = new_sale;
      }
      return new_data;
    }
    ...
    function _setUSDTWalletAddress(address _usdtWalletAddress) external onlyRole(DEFAULT_ADMIN_ROLE){
      tokenContract = IERC20Upgradeable(_usdtWalletAddress);
    }
    ...
    function _setTransactionCut(uint256 cut) external onlyRole(DEFAULT_ADMIN_ROLE){
      transactionCut = cut;
    }

    function _setTransactionCutWallet(address wallet) external onlyRole(DEFAULT_ADMIN_ROLE){
