...
contract LunaStation is Context, IBEP20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address public BuyBackToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;

    address payable public ecosystemAddress = payable(0x7219C7602Bf0633E26A723F2c7E43b58605Bad7A); 
    ...
    uint256 private _tTotal = 350_000_000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "LunaStation";
    ...
    uint256 public _otherTokenBuybackFee = 0;
    uint256 private _previousOtherTokenBuybackFee = _otherTokenBuybackFee; 

    uint256 public _sellLiquidityFee = 3;
    uint256 public _sellEcosystemFee = 1;
    uint256 public _sellBuybackFee = 5;
    uint256 public _sellOtherTokenBuybackFee = 3;

    uint256 public otherTokenBuybackTokkens = 0;

    uint256 public _maxBuyTxAmount = _tTotal.div(100).mul(2); //2%  - max tx limit other than sell
    uint256 public _maxSaleTxAmount = _tTotal.div(100).mul(2); //2%  - max tx limit on sale only 
    ...
    {
        uint256 _totalFees = _liquidityFee.add(_ecosystemFee).add(_buybackFee).add(_otherTokenBuybackFee);

        uint256 halfLiquidityTokens = contractTokenBalance.mul(_liquidityFee).div(_totalFees).div(2);
        uint256 otherTokenBuyBackTokens = contractTokenBalance.mul(_otherTokenBuybackFee).div(100);
        uint256 swapableTokens = contractTokenBalance.sub(halfLiquidityTokens).sub(otherTokenBuyBackTokens);
        ...
        uint256  bnbForEcoSystem = newBalance.mul(_ecosystemFee).div(_totalFees);
        ecosystemAddress.transfer(bnbForEcoSystem);

        doBuyBackOtherToken(otherTokenBuyBackTokens);
    }
    



    function doBuyBackOtherToken(uint256 tokenAmount) private  
    {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private 
    {
        ...
        {   
            otherTokenBuybackTokkens += amount.mul(_sellOtherTokenBuybackFee).div(100);
        }
    ...
    {
        uint256 _fees = _liquidityFee.add(_ecosystemFee).add(_buybackFee).add(_otherTokenBuybackFee);
    ...
    function restoreAllFee() private 
    {
        ...
        _otherTokenBuybackFee = _previousOtherTokenBuybackFee;
    }
    ...
    function setSaleFee() private 
    {
        ...
        _otherTokenBuybackFee = _sellOtherTokenBuybackFee;
    }
    
    function setAllBuyFeePercent(uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee, uint256 otherTokenBuybackFee) external onlyOwner() 
    {
        _liquidityFee = liquidityFee;
        _previousLiquidityFee = liquidityFee;

        _ecosystemFee = marketingFee;
        _previousEcosystemFee = marketingFee;

        _buybackFee = buybackFee;
        _previousBuybackFee = buybackFee;

        _otherTokenBuybackFee = otherTokenBuybackFee;
        _previousOtherTokenBuybackFee = _otherTokenBuybackFee;
    ...
    function setAllSaleFeePercent(uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee, uint256 sellOtherTokenBuybackFee) external onlyOwner() 
    {
        _sellLiquidityFee = liquidityFee;
        _sellEcosystemFee = marketingFee;
        _sellBuybackFee = buybackFee;
        _sellOtherTokenBuybackFee = sellOtherTokenBuybackFee;
        uint256 _fees = _sellLiquidityFee.add(_sellEcosystemFee).add(_sellBuybackFee).add(_sellOtherTokenBuybackFee);
    ...
    function excludeWalletsFromWhales() private 
    {
        ...
        _isExcludedFromWhale[ecosystemAddress]=true;
    }
