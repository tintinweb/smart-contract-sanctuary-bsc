...
contract LaunchPadManager is Auth {
  ...
  function initialize(address _heroNft, uint _maxTotalSupply) public initializer {
  ...
  function mintHeroes(address _owner, uint16 _classId, uint _price, uint _quantity) external onlyLaunchPad {
    require(totalSupply + _quantity <= maxTotalSupply, "max supply reached");
  ...
  function setLaunchPad(address _launchPad, bool _status) external onlyMainAdmin {
