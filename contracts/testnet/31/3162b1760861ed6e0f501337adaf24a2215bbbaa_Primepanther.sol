...
contract Primepanther {
    ...
    function deposit(address _referer, uint256 amount, bool isSplitDep) public security{
        ...
        else if(isSplitDep==true){
            require(user.incomeArray[2]>=amount,"Insufficient Split fund to deposit.");
            user.incomeArray[2]-=amount;
        }
    ...
    function workingWithdraw(uint256 _amount) public security{
        ...
        _amount = _amount.mul(50).div(100);
        user.incomeArray[2] = _amount;
    ...
    function userInfo(address _addr) view external returns(uint256[16] memory team, uint256[16] memory referrals, uint256[16] memory income, uint256 dr, uint256 bdr, uint256 split) {
