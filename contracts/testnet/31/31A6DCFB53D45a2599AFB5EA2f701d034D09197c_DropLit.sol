...
contract DropLit is ERC20Burnable, Operator {
    ...
    // constants
    uint256 public constant POOL_DISTRIBUTION = 12000 ether;
    uint256 public constant BOT_DISTRIBUTION = 1200 ether;
    ...
    modifier noBots () {
        ...
        // if tax collecter is allowed
        taxCollectorAddress == _msgSender() || 
        // genesis defined as immutable is always allowed to use transfer
        genesisAddress == _msgSender() || 
        // boardroom defined as immutable is always allowed to use transfer
        boardroom == _msgSender() || 
        // treasury defined as immutable is always allowed to use transfer
        treasury == _msgSender() || 
        // pair defined as immutable is always allowed to use transfer
        PairWBNB == _msgSender() || 
        // pair defined as immutable is always allowed to use transfer
        PairBUSD == _msgSender() || 
        // BOND defined as immutable is always allowed to use transfer
        BOND == _msgSender() || 
        // router defined as immutable is always allowed to use transfer
        address(uniswapV2Router) == _msgSender() || 
        // sharerewardpool defined as immutable is always allowed to use transfer
        shareRewardPool  == _msgSender() || 
        // for future contracts can be whitelisted to use transfer
        whitelist[msg.sender], 
        "you are trying to interact with the token from a contract, contact deployer for possible whitelist");
        _;
    }
    ...
    constructor(address _BOND, address _router,address _genesisAddress, address _boardroom, address _treasury, address _shareRewardPool, address _BUSD) ERC20("DropLit", "DROPS") {
        ...
        whitelist[PairWBNB] = true;
        whitelist[PairBUSD] = true;
        whitelist[address(uniswapV2Router)] = true;
        whitelist[_genesisAddress] = true;
        whitelist[_boardroom] = true;
        whitelist[_treasury] = true;
        whitelist[_shareRewardPool] = true;
        whitelist[_BOND] = true;
    ...
    function setWhiteList(address _WhiteList) public onlyAdmin {
        // only contracts can be whitelisted
    ...
    function setTaxRate(uint256 _taxRate) onlyTaxCollector public {
        require(_taxRate <= 1 ,"taxrate has to be between 0% and 1%" );
        taxRate = _taxRate;
    }
    ...
    function transferFrom(address sender,address recipient,uint256 amount) public override noBots returns (bool) {
    ...
    function transfer(address recipient, uint256 amount) public virtual override noBots returns (bool) {
