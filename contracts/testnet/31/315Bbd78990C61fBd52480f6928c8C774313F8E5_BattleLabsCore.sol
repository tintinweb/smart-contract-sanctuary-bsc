...
interface LABS_CONTRACTS {
    function forgingWeapon(uint256 amount)
        external
        view
        returns (bool);
}
...
contract BattleLabsCore is Ownable, ECIOHelper {
    ...
    address public WeaponLabs;
    address public BattleSuitLabs;
    address public BattleGearLabs;
    address public BattleBotLabs;
    address public BattleDroneLabs;
    ...
    //Setup
    function updateNFTsContract(address contractAddress) public onlyOwner {
        NFTCoreContract = contractAddress;
    }
    ...
    function updateWeaponLabsContract(address contractAddress)
        public
        onlyOwner
    {
        WeaponLabs = contractAddress;
    }

    function updateBattleSuitLabsContract(address contractAddress)
        public
        onlyOwner
    {
        BattleSuitLabs = contractAddress;
    }

    function updateBattleGearLabsContract(address contractAddress)
        public
        onlyOwner
    {
        BattleGearLabs = contractAddress;
    }

    function updateBattleBotLabsContract(address contractAddress)
        public
        onlyOwner
    {
        BattleBotLabs = contractAddress;
    }

    function updateBattleDroneLabsContract(address contractAddress)
        public
        onlyOwner
    {
        BattleDroneLabs = contractAddress;
    }

    //Forge Fragment
    function forgeWeapon(uint256 warriorId, uint256[] memory tokenIds) public {
        require(
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            if(index != 0){
                require(
                    keccak256(bytes(firstPartCode)) == keccak256(bytes(partCode)),
            ...
            if (index == tokenIds.length - 1) {
                IERC20(ecioTokenContract).transferFrom(msg.sender, address(this), ecioPrice * tokenIds.length);
                IERC20(lakrimaTokenContract).transferFrom(msg.sender, address(this), lakrimaPrice * tokenIds.length);
                bool success = LABS_CONTRACTS(WeaponLabs).forgingWeapon(
                    tokenIds.length
                );
                if (success) {
                    // partCodeCheck 
                    string memory oldPartCode = getPartInfo(warriorId);
                    // string[] memory warriorPartCodes = splitPartCode(oldPartCode); 
                    // string memory partType = warriorPartCodes[PC_NFT_TYPE];
                    // require(
                    //     keccak256(bytes(partType)) ==  keccak256(bytes("06")),
                    //     "Wrong warrior."
                    // );
                        ERC721(NFTCoreContract).transferFrom(
                            msg.sender,
                    ...
                    string memory newWarriorPartCode = _partReplace(oldPartCode, PC_WEAPON, firstPartCode);
                    ECIOERC721(NFTCoreContract).safeMint(msg.sender, newWarriorPartCode);
                }
            }
        }
    }
    ...
    function getPartInfo(uint256 tokenId) private view returns (string memory) {
        ...
        (tokenInfo.partCode, tokenInfo.createAt) = ECIOERC721(NFTCoreContract).tokenInfo(tokenId);
    ...
    function checkWarrior(string memory warrior) internal pure returns (bool) {
     ...
     function _partReplace(string memory oldPartCode, uint256 _pos, string memory fragmentPartCode) internal pure returns (string memory) {
