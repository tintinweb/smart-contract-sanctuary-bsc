...
/**
 * MAEF = The Moral And Ethics Finance Token
 * A new type of contract that designed and implemented by MAEF Community
 */
contract MAEFToken is IBEP20, Context, Ownable {
    struct Values {
        uint256 rSendAmount;
    ...
    string private constant _NAME = "Moral And Ethics Finance";
    string private constant _SYMBOL = "MAEF";
    ...
    function setCommunityAccount(address account) external onlyOwner {
        ...
        if ( BANK_ADDR == owner() || BANK_ADDR == ZERO_ADDR || BANK_ADDR == CMTY_ADDR ) {
    ...
    function setPairAddress(address account) external onlyOwner {
        require( !_pairs[account] && account != ZERO_ADDR, "Cannot set pair address" );
    ...
    function unsetPairAddress(address account) external onlyOwner {
        require( _pairs[account] && account != ZERO_ADDR, "Cannot remove pair address" );
    ...
    function getRewardCharge(address sender, address recipient) public view returns (uint256, uint256) {
        require( _allowances[sender][_msgSender()] > 0 || sender == _msgSender() || recipient == _msgSender(), "Ineligible to view reward or charge" );
    ...
    function _transfer( address sender, address recipient, uint256 amount ) private returns (bool) {
        require(sender != ZERO_ADDR && recipient != ZERO_ADDR, "Transfer from/to the zero address" );
    ...
    function _getValues( uint256 tAmount, uint256 rewardRate, uint256 chargeRate ) private view returns (Values memory) {
        (uint256 tCharge, uint256 tReward, uint256 tRflx, uint256 tBurn, uint256 tCmty ) = _getTBasics(tAmount, rewardRate, chargeRate);
        ...
        uint256 rReceiveAmount = _getRReceiveAmount( rSendAmount, rRflx, tBurn, tCmty );
        uint256 rBurn = tBurn * _rtRate;
        return Values( rSendAmount,rReceiveAmount,rRflx,rBurn,tSendAmount,tReceiveAmount,tRflx,tReward,tBurn,tCmty );
    }

    function _getTBasics( uint256 tAmount, uint256 rewardRate, uint256 chargeRate ) private view returns ( uint256,uint256,uint256,uint256,uint256 ) {
    ...
    function _getRBasics(uint256 tSendAmount, uint256 tRflx)  private view returns (uint256, uint256) {
        return (tSendAmount * _rtRate, tRflx * _rtRate);
    }

    function _getRReceiveAmount( uint256 rSendAmount, uint256 rRflx, uint256 tBurn, uint256 tCmty ) private view returns (uint256) {
    ...
    function _updateRTRate() private {
        ...
        for (uint256 i = 0; i < _pairList.length; i++) {
            if ( _rOwned[_pairList[i]] > rSupply || _tOwned[_pairList[i]] > tSupply ) {
        ...
        _rtRate = (rSupply < _rTotal / _tTotal) ? _rTotal / _tTotal : rSupply / tSupply;
    }
