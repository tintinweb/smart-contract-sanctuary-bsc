...
contract CheatNurseryManagement is Nursery {
    constructor(
        IERC20 _BZAI,
        uint256 _preMint,
        address _NFTreserve,
        string memory _URI
    ) Nursery(_preMint, _NFTreserve, _URI) {
        BZAI = _BZAI;
    }
    ...
    mapping(uint256 => ZaiStruct.MintedData) public nurseryMintedDatas;
    mapping(uint256 => ZaiStruct.MintedData) _tempCounter;
    mapping(uint256 => uint256) _nextStateToMint; // 0 bronze ; 1 Silver ; 2 Gold ; 3 Platinum
    mapping(uint256 => uint256) public nextUnlock; // use for prevent minting
    mapping(uint256 => uint256) public lastTimeReserveEgg; // preventing nursery owner reserve one egg per day max

    mapping(uint256 => uint256) public numberOfEggsOffered;
    mapping(uint256 => mapping(uint256 => ZaiStruct.MintedData))
        public offeredEggReference;

    // ------------------------------------------- //
    // TESTING FN TO DELETE !!!!!!!!!!!!!!
    function cheatNurseryForTest(
        uint256 _nurseryId,
        uint256 _bronzeMinted,
        uint256 _silverMinted,
        uint256 _goldMinted,
        uint256 _platinumMinted
    ) external {
        ZaiStruct.MintedData storage total = nurseryMintedDatas[_nurseryId];
        total.bronzeMinted = _bronzeMinted;
        total.silverMinted = _silverMinted;
        total.goldMinted = _goldMinted;
        total.platinumMinted = _platinumMinted;
    }
    ...
    function setMaturityDurations(uint256 _index, uint256 _numberOfDays)
        external
        onlyOwner
    {
        require(_index < 4, "Doesn't exist");
        require(_numberOfDays >= 1 && _numberOfDays <= 60, "Not good range");

        maturities[_index] = _numberOfDays * 1 days;
    }

    function _canMint(uint256 _tokenId) internal view returns (bool) {
        return ownerOf(_tokenId) != gameAddresses.getClaimNFTsAddress();
    }
    ...
    {
        return nurseryMintedDatas[_tokenId];
    }
    ...
    function getNextUnlock(uint256 _tokenId) external view returns (uint256) {
        return nextUnlock[_tokenId];
    }

    function nextStateToMint(uint256 _tokenId) external view returns (uint256) {
        return _nextStateToMint[_tokenId];
    }

    function _updateNextMint(uint256 _tokenId, uint256 state) internal {
        require(
            state == _nextStateToMint[_tokenId],
            "State has changed since tx began"
        );
        ZaiStruct.MintedData storage temp = _tempCounter[_tokenId];
        ZaiStruct.MintedData storage total = nurseryMintedDatas[_tokenId];
        uint256 _next;

        if (temp.bronzeMinted < 5) {
            temp.bronzeMinted += 1;
            total.bronzeMinted += 1;
            if (temp.bronzeMinted == 5) {
                _next = 1;
                nextUnlock[_tokenId] = block.timestamp + 1 days;
            }
            ...
            temp.platinumMinted += 1;
            total.platinumMinted += 1;
            temp.bronzeMinted = 0;
            temp.silverMinted = 0;
            temp.goldMinted = 0;
        }
        _nextStateToMint[_tokenId] = _next;
    }
    ...
    function setPrice(
        ...
        require(ownerOf(_tokenId) == msg.sender, "Not your nurs");
        ...
        ZaiStruct.EggsPrices storage e = eggsPrices[_tokenId];
    ...
    function buyEgg(
        ...
        require(state == _nextStateToMint[_tokenId], "Not the good state");
        require(block.timestamp >= nextUnlock[_tokenId], "To soon to mint");
        ...
        emit EggSold(_owner, msg.sender, _price);

        nurseryRevenues[_tokenId] += _price;

        return
            IEggs(gameAddresses.getEggsAddress()).mintEgg(
                msg.sender,
                state,
                maturities[state]
            );
        ...
        returns (uint256)
    {
        ...
        require(
            lastTimeReserveEgg[_tokenId] < block.timestamp + 1 days,
            "You can reserve only one egg by day"
        );
        uint256[4] memory averagePrices = _getZaiPriceAverage();
        lastTimeReserveEgg[_tokenId] = block.timestamp;
        uint256 state = _nextStateToMint[_tokenId];

        require(
            IPayments(gameAddresses.getPaymentsAddress()).burnRevenuesForEggs(
                msg.sender,
                averagePrices[state]
            ),
        ...
        _updateNextMint(_tokenId, state);
        numberOfEggsOffered[_tokenId] += 1;
        offeredEggReference[_tokenId][
            numberOfEggsOffered[_tokenId]
        ] = nurseryMintedDatas[_tokenId];
        return
            IEggs(gameAddresses.getEggsAddress()).mintEgg(
                _to,
                state,
                block.timestamp + maturities[state] * 2
            );
    }
    ...
    {
        uint256 price;
        ZaiStruct.EggsPrices memory e = eggsPrices[_tokenId];
        if (_state == 0) {
            price = e.bronzePrice;
        }
        if (_state == 1) {
    ...
    function _getZaiPriceAverage() internal view returns (uint256[4] memory) {
        ...
        for (uint256 i = 1; i <= _preMintNumber; ) {
            _prices[0] += eggsPrices[i].bronzePrice;
            _prices[1] += eggsPrices[i].silverPrice;
            _prices[2] += eggsPrices[i].goldPrice;
            _prices[3] += eggsPrices[i].platinumPrice;
...
contract Nursery is ERC721Enumerable, Ownable {
    IERC20 public BZAI;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _preMint; ) {
            ...
            _safeMint(_NFTreserve, _newItemId);
            eggsPrices[_newItemId] = ZaiStruct.EggsPrices(
                1000 * 1E18,
                5000 * 1E18,
                10000 * 1E18,
                100000 * 1E18
    ...
    mapping(uint256 => ZaiStruct.EggsPrices) public eggsPrices;

    modifier onlyAuth() {
        require(msg.sender == gameAddresses.getOpenAndCloseAddress());
        _;
    }
    ...
    function mintNursery(address _to, uint256[4] memory _prices)
        external
        onlyAuth
        returns (uint256)
    {
        require(_pricesOk(_prices), "Prices too low");

        _tokenIds.increment();
        uint256 _newItemId = _tokenIds.current();
        _safeMint(_to, _newItemId);
        ZaiStruct.EggsPrices storage e = eggsPrices[_newItemId];
        e.bronzePrice = _prices[0];
        e.silverPrice = _prices[1];
        e.goldPrice = _prices[2];
        e.platinumPrice = _prices[3];
    ...
    {
        return eggsPrices[_nursId];
    }

    function _pricesOk(uint256[4] memory _prices) internal view returns (bool) {
        return (_prices[0] >= _minPrices[0] &&
            _prices[1] >= _minPrices[1] &&
            _prices[2] >= _minPrices[2] &&
            _prices[0] >= _minPrices[0]);
    }
}
