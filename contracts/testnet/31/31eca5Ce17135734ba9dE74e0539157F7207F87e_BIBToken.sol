...
contract BIBToken is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => bool) public _isFromWhiteList;
    mapping(address => bool) public _isToWhiteList;
    ...
    function setFeeWhiteList(address account, bool excluded, bool isFrom) public onlyOwner {
        if (isFrom) {
            require(_isFromWhiteList[account] != excluded, "Token: Account is already the value of 'excluded'");
            _isFromWhiteList[account] = excluded;
        } else {
            require(_isToWhiteList[account] != excluded, "Token: Account is already the value of 'excluded'");
            _isToWhiteList[account] = excluded;
        }
    ...
    function setSellFee(uint16 _rewardfee, uint16 _blackhole, uint16 _liquidity) external onlyOwner {
        ...
        totalSellFee = rewardFee + blackholeFee + liquidityFee;
        require(totalSellFee > 1, "INVALID_FEE_RATIO");
    }
    ...
    function isExcludedFromFees(address account) public view returns(bool, bool) {
        return ( _isFromWhiteList[account], _isToWhiteList[account]);
    }
    ...
    uint256 amount
        ...
        if(swapEnabled && !swapping && from != owner() && canSwap) {
            swapping = true;
            contractTokenBalance = swapTokensAtAmount;

            // part goes to liquadation pool
            uint256 swapTokens = contractTokenBalance.mul(
                liquidityFee).div(totalSellFee);
            swapAndLiquify(swapTokens);

            // part gose to dividens pool
            uint256 sellTokens = contractTokenBalance.mul(
                rewardFee).div(totalSellFee);
            swapAndSendDividends(sellTokens);

            // burn the left
            uint burned = contractTokenBalance.sub(swapTokens).sub(sellTokens);
            super._transfer(address(this), deadAddress, burned);
            swapping = false;
        }
        ...
        if(_isFromWhiteList[from] || _isToWhiteList[to]) {
            takeFee = false;
        }
