...
contract CarNFTv1 is ERC721, Ownable {
  ...
  function MintFor(address account) external onlyPermission returns (bool) {
    totalSupply++;
    uint256 randra = _createRandomNum(10000);
    Nft memory newNft = Nft( totalSupply, randra);
    Nfts.push(newNft);
    _safeMint(account, totalSupply);
    return true;
  }

  function UpdateTokenData(uint256 tokenid,uint256 value) external onlyPermission returns (bool) {
    Nfts[tokenid].rarity = value;
    return true;
  }
