...
contract Vesting is Initializable, AccessControl {
    ...
    modifier onlyOperator() {
        require(hasRole(OPERATOR, msg.sender), "OnlyOperator");
        _;
    }
     modifier onlyAdmin() {
        require(
    ...
    function initialize(
        ITokenSale _tokenSale,
        IERC20 _token,
        uint256[2][] memory _vestingPoints
    ) public initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRoleAdmin(OPERATOR, DEFAULT_ADMIN_ROLE);
        tokensale = _tokenSale;
        vestingPoints = _vestingPoints;
        token = _token;
