...
contract ExchangePool is AccessControlEnumerableUpgradeable {
    ...
    mapping(address => mapping(uint8 => uint256)) public scales; // 兑换比例->需带18位精度
    ...
    function init(uint256 newInitPrice, address newMai) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function deposit(
        ...
        require(scales[msg.sender][method] != 0, "Mai: Forbidden.");
        require(tokenSet.contains(token), "Mai: Nonsupport token.");

        uint256 mintAmount;
        if (initPrice != 0) {
            mintAmount = (amount * initPrice) / 1e18;
            initPrice = 0;
        } else {
            mintAmount = (amount * scales[token][method] * lastPrice) / 1e36;
        }

        totalUAmount += amount;
        lastPrice = (IERC20Upgradeable(mai).totalSupply() * 1e18) / totalUAmount;
        IERC20Upgradeable(token).safeTransferFrom(msg.sender, address(this), amount);
        IMai(mai).mint(msg.sender, mintAmount);
    }
