...
contract HonorDao is Ownable {
  ...
  uint public cooldownTimeInSeconds = 180;  // 5 minutes testnet diamondhand / wd
  ...
  uint public taxPercentage = 2; // Tax %
  uint public rewardPercentage = 3; // NFT Reward %
  address public TaxWallet;
  uint public accountNodeLimit = 25;
  bool public nodeAlive = false;
  
  constructor(HonorProtocol _honorAddress, Busd _busdAddress, ArmyRankNFT _armynftAddress, Metadata _metadataAddress) {
  ...
  // Migration
  function migrateMultiple(address [] memory _addresses, uint [][] memory _nodeArgs) external onlyOwner {
  ...
  function setNodeMultipliers(uint _newRewards, uint _nodeType) external onlyOwner {
  ...
  // Update node prices
  function setRequiredAmounts(uint _requiredType, uint _newAmountRequired, uint _nodeType) external onlyOwner {
    require(_requiredType >= 1 && _requiredType <= 4, "Required type not recognized");
    require(_newAmountRequired > 0, "Required amount cant be is zero!");
    require(_nodeType >= 0 && _nodeType <= 7, "Node type not recognized");
    if (_requiredType == 1){
        requiredAmounts[_nodeType] = _newAmountRequired;
    }
    else if (_requiredType == 2){
        requiredAmountsDiscount5[_nodeType] = _newAmountRequired;
    }
    else if (_requiredType == 3){
        requiredAmountsDiscount10[_nodeType] = _newAmountRequired;
    }
    else if (_requiredType == 4){
        requiredAmountsDiscount30[_nodeType] = _newAmountRequired;
    } 
  }
  // Update node ROT target
  function setRotTargetForNode(uint _newRotTarget, uint _nodeType) external onlyOwner {
  ...
  function setCooldownTimeInSeconds(uint _cooldownTimeInSeconds) external onlyOwner {
    cooldownTimeInSeconds = _cooldownTimeInSeconds;
  }
  function setPercentageOfRewardBeforeCooldown(uint _percentageOfRewardBeforeCooldown) external onlyOwner {
    percentageOfRewardBeforeCooldown = _percentageOfRewardBeforeCooldown;
  }
  function setPercentageOfRewardAfterCooldown(uint _percentageOfRewardAfterCooldown) external onlyOwner {
    percentageOfRewardAfterCooldown = _percentageOfRewardAfterCooldown;
  }
  function setDiamondHands(uint _diamondHands) external onlyOwner {
    diamondHands = _diamondHands;
  }
  function setTaxRewardPercentage(uint _taxPercentage, uint _rewardPercentage) external onlyOwner {
    require(_taxPercentage >= 0 && _taxPercentage <= 5, "Min 0% - Max 5%, invalid tax percentage");
    require(_rewardPercentage >= 0 && _rewardPercentage <= 5, "Min 0% - Max 5%, invalid reward percentage");
  ...
  function setHonorNFT(address _armynftAddress, address _emblemnftAddress) external onlyOwner {
    armyRankNFT = ArmyRankNFT(_armynftAddress);
    emblemRankNFT = EmblemNFT(_emblemnftAddress);
  }

  function archiveAccount(address _address) external onlyOwner {
  ...
  function activateAccount(address _address) external onlyOwner {
    require(_address != address(0), "_address is address 0");
  ...
  // Create a node given the address, type and amounts are correct (Normal)
  function mintNode(address _address, uint _honorAmount, uint _busdAmount, uint _nodeType) external {
    require(nodeAlive == true, "Honor Protocol Mint Node Offline");
  ...
  function isWithdrawalAvailable(address _to, uint _timestamp) external view returns(bool){
    ...
    return ((_timestamp - account.lastWithdrawalAt) / 15) >= 1; //180 / 86400 10 for test
  }
  ...
  function estimateInterestSingleNode(address _to, uint _nodeId, uint _timestamp) external view returns(uint) {
    ...
    return estimateWithdrawAmountForNodeTax(2,account, account.nodes[_nodeId], _timestamp);
  }

  // Estimate receives the current timestamp so we can show in the UI the total pending = (UI view)
  ...
  function nftReward (address _to, uint creatorType, uint amount, uint _tokenId, uint _tokenId2) private {
    address Creator = metadataAddress.checkCreator(1,_tokenId);
    address Creator2 = metadataAddress.checkCreator(1,_tokenId2);
    address Creator3 = metadataAddress.checkCreator(2,_tokenId2);
    uint taxamount = amount / 100 * taxPercentage;
    uint creatorAmount = amount / 100 * rewardPercentage;
    uint holderAmount;

    // ArmyRank
    if (creatorType == 1) {
      if (Creator == address(0) && Creator2 == address(0)) {
        honorAddress.transfer(_to, amount);
      }
      else if (Creator != address(0) || Creator2 != address(0)) {
        holderAmount = amount / 100 * 97;
        honorAddress.transfer(Creator, creatorAmount); // Creator
        honorAddress.transfer(_to, holderAmount);
      }
      else {
        holderAmount = amount / 100 * 94;
        honorAddress.transfer(Creator, creatorAmount); // Creator
        honorAddress.transfer(Creator2, creatorAmount); // Creator
        honorAddress.transfer(_to, holderAmount);
      }
    }
    // Emblem Creator
    else if (creatorType == 2) {
      if (Creator == address(0) && Creator3 == address(0)) {
        honorAddress.transfer(_to, amount);
      }
      else if (Creator != address(0) || Creator3 != address(0)) {
        holderAmount = amount / 100 * 97;
        honorAddress.transfer(Creator, creatorAmount); // Creator
        honorAddress.transfer(_to, holderAmount);
      }
        ...
        holderAmount = amount / 100 * 94;
        honorAddress.transfer(Creator, creatorAmount); // Creator
        honorAddress.transfer(Creator3, creatorAmount); // Creator
        honorAddress.transfer(_to, holderAmount);
      }
    }
    honorAddress.transfer(TaxWallet, taxamount);
  }
  // Does the same as the method above, but actually transfers the tokens owed. (Withdrawal)
  // Gets the timestamp from the block instead of it being a parameter
  // Require for NFT inside Metadata Security
  function withdrawInterest(address _to, uint _wdType, uint256 _tokenId, uint256 _tokenId2) external {
    ...
    uint daysSinceLastWithdrawal = (block.timestamp - account.lastWithdrawalAt) / 15; //86400 - 10 for testnet
    ...
    uint amount = estimateWithdrawAmountForAccountTax(0,account, block.timestamp); // Normal daily
    uint amounttax5 = estimateWithdrawAmountForAccountTax(1,account, block.timestamp); // Tax 5% after (ROT)
    uint amounttax10 = estimateWithdrawAmountForAccountTax(2,account, block.timestamp); // Tax 10% after (ROT)
    uint reward5 = estimateWithdrawAmountForAccountTax(0,account, block.timestamp) / 100 * 105; // Claim 5% extra reward with NFT (30-days)
    uint reward10 = estimateWithdrawAmountForAccountTax(0,account, block.timestamp) / 100 * 110; // Claim 10% extra reward with 2 NFTs (30-days)

    if (_wdType == 0){ // Special Day + No Rot Tax + No Withdrawal Tax
      ...
      require(metadataAddress.checkCounter(1,_tokenId2) < 30, "Token ID Need Maintenance - Input Token B "); // Army Rank

      nftReward(_to, 1, amount, _tokenId, _tokenId2);   // Send calc for NFT reward
      metadataAddress.addCounter(_to, _wdType, _tokenId, _tokenId2);
    }
    else if (_wdType == 2){ // 5% tax after ROT + 1 NFT  - Daily Reward
      require(metadataAddress.checkCounter(1,_tokenId) < 30, "Token ID Need Maintenance - Input Token A "); // Army Rank

      nftReward(_to, 1, amounttax5, _tokenId, 0);   // Send calc for NFT reward
      metadataAddress.addCounter(_to, _wdType, _tokenId, 0);
    }
    else if (_wdType == 3){ // 10% tax after ROT. No NFT  - Daily Reward
      uint tax10 = amounttax10 / 100 * taxPercentage;
      honorAddress.transfer(TaxWallet, tax10);
      ...
      nftReward(_to, 1, reward5, _tokenId, 0);   // Send calc for NFT reward
      metadataAddress.addCounter(_to, _wdType, _tokenId, 0);
      ...
      require(metadataAddress.checkCounter(1,_tokenId) < 30, "Token ID Need Maintenance - Input Token A ");   // Army Rank
      require(metadataAddress.checkCounter(2,_tokenId2) < 30, "Token ID Need Maintenance - Input Token B ");  // Emblem

      nftReward(_to, 2, reward10, _tokenId,_tokenId2);   // Send calc for NFT reward
      metadataAddress.addCounter(_to, _wdType, _tokenId, _tokenId2);
      metadataAddress.levelUp(_to, 1, _tokenId, 1);
  ...
  function transferHonor(address _address, uint _amount) external onlyOwner {
    require(_address != address(0), "_address is address 0");
    honorAddress.transfer(_address, _amount);
  }

  function transferBusd(address _address, uint _amount) external onlyOwner {
    require(_address != address(0), "_address is address 0");
  ...
  function awardNode(address _address, uint _nodeType) external onlyOwner {
    require(_address != address(0), "_address is address 0");
    require(_nodeType >= 0 && _nodeType <= 7, "Invalid node type");
    ...
    require(account.nodesLength < accountNodeLimit, "25 nodes maximum already reached!");
  ...
  function mintNodeDiscount(address _address, uint _honorAmount, uint _busdAmount, uint _tokenId, uint _discountType, uint _nodeType) external {
    ...
    bool discount = metadataAddress.checkDiscount(1,_tokenId);
  ...
  //-----------------------------------------------------------------------------------------------------------------------------------
  // Start Calcualtion
  // Return amount with 5%-10 tax for withdrawal
  // Diamond Hand +10% included
  function estimateWithdrawAmountForAccountTax(uint taxType, Account storage _account, uint _timestamp) private view returns(uint AmountTax) {
    uint amount = 0;
    uint amounttax5 = 0;
    uint amounttax10 = 0;
    uint amountnormal =0;
    ...
    for(uint i=0; i<_account.nodesLength; i++){
      Node memory node = _account.nodes[i];
        amount += estimateWithdrawAmountForNodeTax(0,_account, node, _timestamp);
        amounttax5 += estimateWithdrawAmountForNodeTax(1,_account, node, _timestamp);
        amounttax10 += estimateWithdrawAmountForNodeTax(2,_account, node, _timestamp);
        amountnormal += estimateWithdrawAmountForNodeTax(3,_account, node, _timestamp); 
    }
    if (taxType == 0) {
        amountTax = amount;
        return amountTax;
        ...
        amountTax = amounttax5;
        return amountTax;
    }
    else if (taxType == 2) {
        amountTax = amounttax10;
        return amountTax;
    }
    else if (taxType == 3) {
        amountTax = amountnormal;
        return amountTax;
    }
  }
  function estimateWithdrawAmountForNodeTax(uint taxType, Account storage _account, Node memory _node, uint _timestamp) private view returns(uint RewardTax) {
    ...
    if(_timestamp > rotReachedTimestamp && _account.lastWithdrawalAt < rotReachedTimestamp){
      ...
      uint amount;
      uint amountnormal;
      reward = ((nodeMultiplers[_node.nodeType] / 86400) / 100) * diamondHands;
      rewardnormal = ((nodeMultiplers[_node.nodeType] / 86400));
      amount = reward * (rotReachedTimestamp - _account.lastWithdrawalAt);
      amountnormal = rewardnormal * (rotReachedTimestamp - _account.lastWithdrawalAt);
      ...
      else if (taxType == 1){ // Tax 5%
      rewardtax5 = reward / 20 * 19;
      amount += rewardtax5 * (_timestamp - rotReachedTimestamp);
      return amount; // return amounttax5 Tax 5%
      }
      else if (taxType == 2){ // Tax 10%
      rewardtax10 = reward / 20 * 18;
      amount += rewardtax10 * (_timestamp - rotReachedTimestamp);
      return amount; // return amounttax10 Tax 10%
      }
    else if (taxType == 3){
      amountnormal += reward * (_timestamp - rotReachedTimestamp);
      return amountnormal;
      }
    }
    // ROT was reached
  ...
  function estimateWithdrawAmountForAccountAll(Account storage _account, uint _timestamp) private view returns(uint, uint, uint, uint) {
    ...
    for(uint i=0; i<_account.nodesLength; i++){
      Node memory node = _account.nodes[i];
      amount += estimateWithdrawAmountForNodeTax(0,_account, node, _timestamp);
      amounttax5 += estimateWithdrawAmountForNodeTax(1,_account, node, _timestamp); // 5%
      amounttax10 += estimateWithdrawAmountForNodeTax(2,_account, node, _timestamp); // 10%
      amountnormal += estimateWithdrawAmountForNodeTax(3,_account, node, _timestamp);
    }
    return (amount, amounttax5, amounttax10, amountnormal);
  }

  function nodeOnline() external onlyOwner {
    nodeAlive = true;
  }
  function nodeOffline() external onlyOwner {
    nodeAlive = false;
  }
