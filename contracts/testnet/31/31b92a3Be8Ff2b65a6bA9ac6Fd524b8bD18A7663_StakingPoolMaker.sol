...
contract StakingPoolMaker is Ownable {
    ...
    function createPool(
        ...
        rewardsToken.transferFrom(payable(msg.sender), address(this), _totalRewards);
        _pool.id = _id;
    ...
    function stake(
        ...
        uint256 _id;
        for(uint256 i=0; _pool.stakes[i].amount > 0; i++) {
            _id++;
        }
    ...
    function unstake(uint256 _poolId, uint256 _stakeId) external {
        ...
        require(msg.sender == _stake.owner, "unstake: not owner");
        require(
            _stake.createdTime + _stake.period >= block.timestamp,
        ...
        rewardsToken.approve(msg.sender, _rewardsAmount);
        rewardsToken.transferFrom(address(this), msg.sender, _rewardsAmount);
        _stake.isUnstaked = true;
        _pool.totalPoint = _pool.totalPoint - _stake.amount * _stake.period;
    ...
    function estimateRewards(uint256 _poolId, uint256 _amount, uint256 _period)
        public
        view
        returns (uint256, uint256)
    {
    ...
    //     Pool[] memory PoolsByOwner;
    //     Pool memory _pool;
    //     for (uint256 i = 0; i < poolId.current(); i++) {
    //         _pool = pools[i];
    //         if (_pool.owner == _owner) {
    //             PoolsByOwner.push(_pool);
    //         }
    //     }
    //     return PoolsByOwner;
    // }
