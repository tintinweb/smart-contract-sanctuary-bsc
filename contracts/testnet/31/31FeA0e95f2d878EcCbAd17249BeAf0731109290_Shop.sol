...
contract Shop is Ownable, Pausable, ReentrancyGuard {
    IERC20 public ninoToken;
    IERC20 public mataToken;
    ...
    mapping(address => DateBuy) public ninoBuyers;
    mapping(address => DateBuy) public mataBuyers;
    ...
    constructor(address _ninoToken, address _mataToken) {
        addressNinoReceiver = owner();
        addressMataReceiver = owner();
        ninoToken = IERC20(_ninoToken);
        mataToken = IERC20(_mataToken);
    }
    ...
    function setPackageNinoId(
        ...
        require(_ids.length > 0 && _ids.length == _prices.length && _prices.length == _limits.length, "Input invalid");
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 id = _ids[i];
            uint256 price = _prices[i];
            uint256 limit = _limits[i];
            packageNinoIds[id] = Package(price, limit);
        }
    }
    ...
    function buyPackageNino(uint256 _id, uint256 _quantity) external whenNotPaused nonReentrant {
        ...
        DateBuy storage dateBuy = ninoBuyers[msg.sender]; // dateBuy la ngay gan nhat no mua
        ...
        require(dateBuy.number + _quantity <= limit, "Over limit"); // check xem ngay hom nay no mua vuot gioi han chua
    ...
    function buyPackageMata(uint256 _id, uint256 _quantity) external whenNotPaused nonReentrant {
        ...
        uint256 currentDate = getDateNumber(block.timestamp);
        DateBuy storage dateBuy = mataBuyers[msg.sender];
        if (dateBuy.dateLastBuy != currentDate) {
            dateBuy.number = 0;
            dateBuy.dateLastBuy = currentDate;
        }

        require(dateBuy.number + _quantity <= limit, "Over limit");
        dateBuy.number = dateBuy.number + _quantity;
