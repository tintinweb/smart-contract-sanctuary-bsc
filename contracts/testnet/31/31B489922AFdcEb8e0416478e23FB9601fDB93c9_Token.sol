...
import './utils/MaintenanceMode.sol';
import './utils/AntiBot.sol';

/// @custom:security-contact [email protected]
contract Token is ERC20, ERC20Burnable, Withdrawable, MaintenanceMode, AntiBot {
    bytes32 public constant MINTER_ROLE = keccak256('MINTER_ROLE');

    constructor(uint256 initialSupply, address owner)
        ERC20('Token', 'TKN')
        MaintenanceMode(owner)
    {
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) internal override maintenanceMode transactionThrottler(from, to) {
...
contract MaintenanceMode is AccessControlEnumerable {
    ...
    modifier maintenanceMode() {
        if (isMaintenanceMode) {
            require(
                ...
                'Maintenance mode: Only maintainers can transact'
            );
        }
        _;
    }
    ...
    function setMaintenanceMode(bool isMaintenanceMode_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (isMaintenanceMode != isMaintenanceMode_) {
            isMaintenanceMode = isMaintenanceMode_;
            emit MaintenanceModeUpdated(isMaintenanceMode_);
        }
    }
}
...
contract Withdrawable is AccessControlEnumerable {
    using SafeERC20 for IERC20;

    receive() external payable {}

    function withdrawToken(address to, address token_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
