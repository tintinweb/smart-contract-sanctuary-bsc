...
interface whitelistContract {
    function viewWhitelist(address _add, uint256 tokenId) external view returns (bool);
}
...
contract FUCKERS is Auth, ERC721, ERC721Enumerable {
    ...
    constructor(string memory name, string memory symbol, string memory assetsBaseURI) ERC721(name, symbol) Auth(msg.sender){
        ...
        whitelist = whitelistContract(0x86C6e8e769aA7349701dD202616CD21D064355d0);
    ...
    function whitelistMint(uint tokenId) public payable whenMintingAllowed {
        bool wl = whitelist.viewWhitelist(msg.sender, tokenId) ? true : false;
        if(wl){whitelistMintToken(_msgSender(), tokenId);}
    }

    function whitelistMintToken(address recipient, uint tokenId) internal {
            _mint(recipient, tokenId);
            _originalMinters[tokenId] = recipient;
            _claimedDividends[tokenId] = _totalDividend;
            if (tokenId > 0) {
            ...
            if(!_hasMinted[recipient]){
                _minters = _minters + 1; _hasMinted[recipient] = true;}
        _mintedWallet[recipient] += 1;
        setMintingRoll();
        setnewMinter(recipient, 1);
        emit TokensMinted(totalSupply(), _maxSupply, _balanceReflections);
    }
