...
contract RootOfPools_v013 is Initializable, OwnableUpgradeable {
    ...
    event createdPool(string name, address pool);
    event fundraisingStarted(string name);
    event fundraisingStoped(string name);
    event emergencyStoped(string name);
    ...
    function createPool(string calldata name, address pool) public onlyOwner {
        require(isContract(pool), "ERROR: Pool must be a contract!");
        require(
    ...
    function depositUSDT(string calldata name, uint256 amount) public {
        require(
            _poolsTable[name] != address(0),
            "ROOT: Selected pool does not exist!"
        );

        BranchOfPools(_poolsTable[name]).depositUSDT(amount);
    }
...
contract BranchOfPools is Ownable, Initializable {
    ...
    uint256 public _priceToken; //Цена за токен
    ...
    uint256 public _vesting;
    //===============================================================

    address public _usdt;
    address public _token;
    address private _devUSDTAddress;
    address private _devInteractionAddress;

    mapping(address => uint256) private _valueUSDTList;
    //mapping(address => uint256) _distList;
    mapping(address => uint256) private _usdtEmergency;
    ...
    constructor(
        ...
        uint256 Step,
        uint256 price,
        address devUSDTAddress,
        address devInteractionAddress
    ) {
        _root = Root;
        _usdt = RootOfPools_v013(_root).getUSDTAddress();
        _VALUE = VALUE;
        _stepValue = Step;
        _priceToken = price;
        _devUSDTAddress = devUSDTAddress;
        _devInteractionAddress = devInteractionAddress;
    }

    function init() public onlyOwner initializer returns (uint256) {
        uint256 temp = 10**ERC20(_usdt).decimals();
        _VALUE = _VALUE * temp;
    ...
    function changePrice(uint256 step)
        public
        onlyOwner
        onlyNotState(State.TokenDistribution)
        onlyNotState(State.WaitingToken)
    ...
    function paybackEmergency() public onlyState(State.Emergency) {
        uint256 usdtT = _usdtEmergency[tx.origin];

        _usdtEmergency[tx.origin] = 0;
        ...
        if (usdtT != 0) {
            ERC20(_usdt).transfer(tx.origin, usdtT);
        }
    }

    function depositUSDT(uint256 amount) public onlyState(State.Fundrasing) {
        ...
        uint256 Min = (10**ERC20(_usdt).decimals()) *
            Ranking(RootOfPools_v013(_root).getRanks()).getParRankOfUser(
                tx.origin
            )[0];
        uint256 Max = (10**ERC20(_usdt).decimals()) *
            Ranking(RootOfPools_v013(_root).getRanks()).getParRankOfUser(
                tx.origin
            )[1];
        ...
        require(
            amount + _valueUSDTList[tx.origin] <= Max,
        ...
        require(
            ERC20(_usdt).allowance(tx.origin, address(this)) >= amount,
            "DEPOSIT: ALLOW ERROR"
        );

        require(
            ERC20(_usdt).transferFrom(tx.origin, address(this), amount),
            "DEPOSIT: Transfer error"
        );

        _usdtEmergency[tx.origin] += amount;

        _valueUSDTList[tx.origin] += amount - commission;
        _CURRENT_COMMISSION += commission;
        _CURRENT_VALUE += amount - commission;

        require(
            ERC20(_usdt).balanceOf(address(this)) ==
    ...
    function collectFunds() public onlyOwner onlyState(State.WaitingToken) {
        ...
        //Send to devs
        require(
            ERC20(_usdt).transfer(_devUSDTAddress, _FUNDS_RAISED),
            "COLLECT: Transfer error"
        );

        //Send to admin
        require(
            ERC20(_usdt).transfer(
    ...
    function entrustToken(address tokenAdr, uint256 amount)
        public
        onlyDeveloper
    ...
    {
        require(
            ERC20(tokenAdr).balanceOf(address(tx.origin)) >= amount,
        ...
        _CURRENT_VALUE_TOKEN = ERC20(tokenAdr).balanceOf(address(this));
    ...
    function claim() public onlyState(State.TokenDistribution) {
        ...
        for (uint256 i = _openUnlocks[tx.origin]; i < currentUnlocks; i++) {
            amount += (_unlocks[i] * _valueUSDTList[tx.origin]) / _FUNDS_RAISED;
        }
...
contract Ranking is Ownable {
    ...
    /// pNames[] - An array of parameter names
    /// pValues[] - An array of parameter values
    /// isChangeable - Flag of rank variability
    /// @return bool (On successful execution returns true)
    function createRank(
        string memory Name,
    ...
    /// If the rank is variable
    /// @param Name - Unique rank identifier
    /// NewParName - New parameter name
    /// NumberPar - The number of the parameter you want to change
    /// @return bool (On successful execution returns true)
