...
contract DualMinerV5 is Initializable, OwnableUpgradeable {
        ...
        if (enable) {
            ...
            claimedSeas[referrals[msg.sender]] = SafeMathUpgradeable.add(
                ...
                SafeMathUpgradeable.div(
                    SafeMathUpgradeable.mul(seasUsed, 13),
                    100
                )
            );
    ...
    function sellSeas() public {
        ...
        IERC20Upgradeable(Moceans).transfer(devAddress, fee);
        IERC20Upgradeable(Moceans).transfer(
            address(msg.sender),
    ...
    function checker() internal {
        ...
        if (
            (block.timestamp > startTime + 6 minutes) &&
            (block.timestamp < startTime + 25 minutes)
        ) {
            casteSpell(2);
        }
        if (block.timestamp > startTime + 25 minutes) {
            startTime = block.timestamp;
        }
    }
    ...
    function manualAutoSurf() public {
        casteSpell(2);
    }
    ...
    function magicSurf(address _target) internal {
        ...
        lastHatch[_target] = block.timestamp;
        marketSeas = SafeMathUpgradeable.add(marketSeas, hasSeas);
        IERC20Upgradeable(Moceans).transfer(address(_target), seasValue);
        trident[_target]._SurfTime = block.timestamp;
    }

    function casteSpell(uint256 _pid) internal {
    ...
    function shouldCompound(address _adr) internal view returns (bool) {
        return
            trident[_adr]._CompoundTime + 1 minutes <= block.timestamp &&
            !LockMagic[_adr];
    }

    function shouldSurf(address _adr) internal view returns (bool) {
        return
            trident[_adr]._SurfTime + 5 minutes <= block.timestamp &&
            !LockMagic[_adr];
    }
    ...
    function setEnable(bool _value) external {
        enable = _value;
    }
}
