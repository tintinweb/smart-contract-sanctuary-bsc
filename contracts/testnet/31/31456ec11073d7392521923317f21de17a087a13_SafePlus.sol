...
contract SafePlus is ERC20, Ownable {
     ...
     */
     ...
     uint timeForSellTotalMaxTxAmount = 60 seconds;
    ...
    constructor() ERC20("SafePlusV2", "SPLUSV2") {
    ...
    function setMaxTxAmountTime(uint val) public onlyOwner {
        require(val > 100000, "Minimum 100000");
    ...
    function _transfer(
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if(buybackFee > 0) {
                ...
                payable(buybackAddress).transfer(buybackTokens);
            }
            if(greenmoonFee > 0) {
                uint256 greenmonTokens = finalBalance.div(totalFees).mul(greenmoonFee);
                payable(greenMoonAddress).transfer(greenmonTokens);
            }
        ...
        if(takeFee) {
            ...
            if(referrals[from] != address(0) || referrals[to] != address(0)) {
                fees -= amount.div(100).mul(referralFeeDecrease);
            }
