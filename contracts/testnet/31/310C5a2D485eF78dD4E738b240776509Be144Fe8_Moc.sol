...
contract Moc is ERC20Detailed, Ownable {
    ...
    function rebase() internal {
        ...
        uint256 rebaseAmount = _totalSupply.sub(unTotalSupply);
        ...
        _gonsPerFragment = TOTAL_GONS.div(rebaseAmount);
        _totalSupply = rebaseAmount + unTotalSupply;
    ...
    function _transferFrom(
        ...
        if (to == mocPair) {
            ...
            unTotalSupply += gonAmountReceived.div(_gonsPerFragment);
        } else {
    ...
    function takeFee(
        ...
        //buy,but exclude removeLiquidity
        if (from == mocPair && to != mocPair) {
            ...
            if (IERC20(usdtAddress).balanceOf(mocPair) >= reserve0 + amountA) {
                _totalFee = buyInviteFee.add(buyBackExiFee).add(
                    buyLiqMocFee
                );
                //invite reward when buy
                ...
                for (uint8 i = 0; i < _parentInviters.length; i++) {
                    ...
                    if (_isTokenAccount(_parentInviters[i])) {
                        uint256 addInviteAmount = _balances[_parentInviters[i]]
                            ...
                            unTotalSupply += _inviteFee.div(_gonsPerFragment);
                        if (
        ...
        if (from != mocPair && to == mocPair) {
            ...
            if (balanceA < reserve0 + amountA) {
                ...
                unTotalSupply += tempFee;
                emit TakeFee(from, marketingReceiver, tempFee);
            }
        }

        //when transfer
        if (from != mocPair && to != mocPair) {
            _totalFee = transferFee;
            uint256 _deadFee = gonAmount
                .mul(transferFee)
                .div(feeDenominator)
                .div(_gonsPerFragment);
            _balances[DEAD] += _deadFee;
            unTotalSupply += _deadFee;
            emit TakeFee(from, DEAD, _deadFee);
        }
