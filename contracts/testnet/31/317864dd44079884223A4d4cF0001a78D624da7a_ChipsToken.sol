...
abstract contract Referral {
    ...
    function signin(address ref, uint256 amount) public {
        ...
        require(ref_parent[msg.sender][amount] == address(0), "Can't sign in twice!");

        bool unsigned_parent = false;

        if(ref_parent[ref][amount] == address(0)){
            unsigned_parent = true;
        }
        ...
        if(ref != address(0) && ref_parent[ref][amount] != address(0))
        {
    ...
    function withdraw(uint256 amount_category) public {
        ...
        _ref_token.safeTransferFrom(address(this), msg.sender, with_amount);
    ...
    function platform_withdraw() public {
        ...
        _ref_token.safeTransferFrom(address(this), platform_address, platform_balance);
        platform_balance = 0;
...
contract ChipsToken is BEP20, Referral {
    ...
    function faucet() public {
        ...
        require(game_api.token_balances(address(this), msg.sender) == 0, "in-game wallet balance should be zero!");

        require(balanceOf(_msgSender()) == 0, "wallet balance should be zero!");
    ...
    function _transfer(
        ...
        super._transfer(sender, recipient, amount - withdraw_fee);
