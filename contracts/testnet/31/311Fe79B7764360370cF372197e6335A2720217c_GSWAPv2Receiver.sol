...
contract GSWAPv2Receiver is Ownable {
    using SafeMath for uint256;

    address public V1 = 0xC166CF284472a9254e1A024e899239f64f290F51; // TESTNET
    ...
    function getMigratedAmounts(address _address) external view returns (uint256) {
        return _receivedV1Token[_address];
    }
    ...
    function startMigration(bool _value) external onlyOwner {
        started = _value;
    }

    function migrate(uint256 amount) public {
        require(started, "Migration is not started yet.");
        address account = _msgSender();
        require(amount > 0, "Transfer amount must be greater than zero");
        require(amount <= IBEP20(V1).balanceOf(account), "You don't have enough token balance.");
