...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) private treasuryeOf;
    ...
    uint public duration = 240;
    // Minimum of last updated time and reward finish time
    uint public updatedAt;
    // Reward to be paid out per second
    uint public treasuryRATE = 196;
    ...
    constructor () {
        ...
        treasuryeOf[address(this)] = 3000*10**18;
    ...
    modifier updateReward(address _account) {
        uint256 treasurybalance = treasuryeOf[_account];

        if (_account != address(0)) {
            treasuryeOf[_account] = treasurybalance * treasuryRATE/(10**8) * duration;
        }
    ...
    function stake(uint _amount) external updateReward(msg.sender) {
        ...
        _treasuryTransfer(address(this), msg.sender, _amount);
        finishAt[msg.sender] = block.timestamp + duration;
        startAt[msg.sender] = block.timestamp;
    }
    function CheckTreasuryPerformance(address _account) external updateReward(msg.sender) {
        uint256 treasurybalance = treasuryeOf[_account];
        treasuryeOf[_account] = treasurybalance * treasuryRATE/(10**8) * (block.timestamp - startAt[_account]);
    }

    function withdraw(uint _amount) external updateReward(msg.sender) {
        require(_amount > 0, "amount = 0");
        require(block.timestamp > finishAt[msg.sender], "amount = 0");
        _treasuryTransfer(msg.sender, address(this), _amount);
        _transfer(address(this),msg.sender, (_amount*10**18)*90/100);
    }
    ...
    function _treasuryTransfer(address sender, address recipient, uint256 tAmount) private {
        treasuryeOf[sender] = treasuryeOf[sender].sub(tAmount);
        treasuryeOf[recipient] = treasuryeOf[recipient].add(tAmount);
    }
