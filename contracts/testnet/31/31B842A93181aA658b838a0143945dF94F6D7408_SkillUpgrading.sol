...
contract SkillUpgrading is Auth {
  ...
  function skillUp(
    ...
    uint eatherPrice = _validateEatherRight(_eatherId);
    ...
    _upgradeSkill(_tokenId, upgradingInfo, eatherPrice);
  ...
  function _upgradeSkill(
    ...
    heroNft.updateOwnPrice(_tokenId, ownPrice + _eatherPrice);
  ...
  function _validateHero(uint _tokenId) private view returns (uint, uint8) {
    ...
    require(heroNft.ownerOf(_tokenId) == msg.sender, "Upgrading: hero invalid");
  ...
  function _validateEatherRight(uint _eatherId) private view returns (uint) {
