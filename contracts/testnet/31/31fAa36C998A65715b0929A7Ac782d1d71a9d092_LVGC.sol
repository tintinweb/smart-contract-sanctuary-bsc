...
contract LVGC is Context, IERC20, Ownable {
    ...
    constructor (address _referralFeeReceiver, address _genesisNodeFeeReceiver) {
    ...
    function deliver(uint256 tAmount) external {
        ...
        (, uint256 tFee, uint256 tLiquidity, uint256 tReferral, uint256 tGenesisNode) = getTValues(tAmount);
        uint256 currentRate = getRate();
        (uint256 rAmount,,, uint256 rReferralFee, uint256 rGenesisNodeFee) = getRValues(tAmount, tFee, tLiquidity, tReferral, tGenesisNode, currentRate);

        _rOwned[sender] = _rOwned[sender] - rAmount;
        _rOwned[referralFeeReceiver] = _rOwned[referralFeeReceiver] + rReferralFee;
        _rOwned[genesisNodeFeeReceiver] = _rOwned[genesisNodeFeeReceiver] + rGenesisNodeFee;
    ...
    function transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tReferral, uint256 tGenesisNode) = getTValues(tAmount);
        uint256 currentRate = getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee,,) = getRValues(tAmount, tFee, tReferral, tGenesisNode, tLiquidity, currentRate);
        ...
        takeTransactionFee(genesisNodeFeeReceiver, tGenesisNode, currentRate);
    ...
    function getTValues(uint256 tAmount) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tGenesisNode = calculateFee(tAmount, genesisNodeFee);
        uint256 tTransferAmount = tAmount - (tFee + tLiquidity + tReferral + tGenesisNode);
        return (tTransferAmount, tFee, tLiquidity, tReferral, tGenesisNode);
    }

    function getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tReferral, uint256 tGenesisNode, uint256 currentRate) 
    internal pure returns (uint256, uint256, uint256, uint256, uint256) {
        uint256 rAmount = tAmount * currentRate;
        uint256 rFee = tFee * currentRate;
        uint256 rLiquidity = tLiquidity * currentRate;
        uint256 rReferral = tReferral * currentRate;
        uint256 rGenesisNode = tGenesisNode * currentRate;
        uint256 rTransferAmount = rAmount - (rFee + rLiquidity + rReferral + rGenesisNode);
        return (rAmount, rTransferAmount, rFee, rReferral, rGenesisNode);
    }
    ...
    function setGenesisNodeFeeReceiver(address newGenesisNodeFeeReceiver) external onlyOwner {
        require(newGenesisNodeFeeReceiver != address(0), "Address zero");
        genesisNodeFeeReceiver = newGenesisNodeFeeReceiver;

        emit SetGenesisNodeFeeReceiver(newGenesisNodeFeeReceiver);
    }
    ...
    event SetGenesisNodeFeeReceiver(address genesisNodeWallet);
