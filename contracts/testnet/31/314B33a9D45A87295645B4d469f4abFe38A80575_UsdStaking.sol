...
contract UsdStaking is StakeUser {
    ...
    uint256 public bounsDuration = 1800; // 奖励周期 86400
    ...
    uint256 public DEPOSIT_DURATION_1 = 600; //30d 2592000
    uint256 public DEPOSIT_DURATION_2 = 1200; //60d 5184000
    uint256 public DEPOSIT_DURATION_3 = 1800; //90d 7776000
    ...
    // 设置升级合约
    function setUpgradeAddress(address addr) public onlyOwner {
    ...
    function initUpgrade() public onlyOwner {
    ...
    function setBounsRate(uint256 rate) public onlyOwner {
        bounsRate = rate;
    }
        ...
        returns (UserInfo memory _user)
    {
        ...
        if (userUpgrade[_account] == false && upgradeAddress != address(0)) {
    ...
    function _unstake(uint256 _amount) public {
        if (_amount > 0) {
            UserInfo storage user = _userInfo[msg.sender];
