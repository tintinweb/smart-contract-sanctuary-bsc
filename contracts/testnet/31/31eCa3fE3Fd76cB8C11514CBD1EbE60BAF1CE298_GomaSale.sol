...
contract GomaSale is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    uint256 public fee;
    ...
    function buyGoma(uint256 amount) private whenNotPaused nonReentrant {
        ...
        uint256 gwonTokenBalance = IERC20Upgradeable(tokenOut).balanceOf(address(this));
        uint256 gwonTokenOut = getExchangeAmount(amount);
        require((gwonTokenOut * 2) <= gwonTokenBalance, "insufficent liquidity");
        addLiquidityToPool((amount - commission), gwonTokenOut);
        IERC20Upgradeable(tokenOut).transfer(_msgSender(), gwonTokenOut);
        emit Buy(_msgSender(), amount, commission, gwonTokenOut);
    }
    ...
    function buy(uint256 amount) external whenNotPaused nonReentrant {
        ...
        uint256 commission = getCommission(amount);
        uint256 gwonTokenBalance = IERC20Upgradeable(tokenOut).balanceOf(address(this));
        uint256 gwonTokenOut = (getCurrentGomaPrice() * amount) / 10**18;
        require((gwonTokenOut * 2) <= gwonTokenBalance, "insufficent liquidity");
        addLiquidityToPool((amount - commission), gwonTokenOut);
        IERC20Upgradeable(tokenOut).transfer(_msgSender(), gwonTokenOut);
        emit Buy(_msgSender(), amount, commission, gwonTokenOut);
    ...
    function addLiquidityToPool(uint256 _amountUSD, uint256 _amountGwon) internal {
        address router = address(pancakeswapRouterV2);
        IERC20Upgradeable(tokenIn).approve(router, _amountUSD);
        IERC20Upgradeable(tokenOut).approve(router, _amountGwon);

        (uint256 amountUSD, uint256 amountGwon, uint256 liquidity) = pancakeswapRouterV2.addLiquidity(
            tokenOut,
            tokenIn,
            _amountGwon,
            _amountUSD,
        ...
        emit LiquidityAdded(tokenIn, tokenOut, amountGwon, amountUSD, liquidity);
    }

    function getExchangeAmount(uint256 amount) public view returns (uint256) {
        uint256 _amount = amount - getCommission(amount);
        address[] memory path;
        path = new address[](2);
        path[0] = tokenIn;
        path[1] = tokenOut;
        uint256[] memory amountOut = pancakeswapRouterV2.getAmountsOut(_amount, path);
