...
contract ReferralCrowdsale is AccessControl {
    ...
    EnumerableSet.UintSet private availableFatherPercents;
    ...
    // 0 - BTCMT, 1 - USDT, 2 - regular staking, 3 - autofarm, 4 - BTCMT-USDT pair, 5 - signer
    constructor(
        ...
        availableFatherPercents.add(0);
        availableFatherPercents.add(2500);
        availableFatherPercents.add(5000);
        availableFatherPercents.add(7500);
        availableFatherPercents.add(10000);
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        require(
            (dexInfo.token0 == address(BTCMT) ||
                ...
                (dexInfo.token0 == address(USDT) ||
                    dexInfo.token1 == address(USDT)) &&
    ...
    function buyTokens(
        ...
        address linkFather,
        uint256 fatherPercent,
        bytes memory linkSignature,
        uint256 usdtAmount,
        uint256 btcmtAmount,
        uint256 expirationTime,
        bytes memory buySignature
    ) external {
        require(usdtAmount > 0, "Wrong amount");

        if (linkHash == bytes32(0) || links[linkHash].owner != address(0)) {
            require(links[linkHash].owner != _msgSender(), "Wrong sender");
            if (kids[_msgSender()].registered) {
                require(
                    kids[_msgSender()].fatherLink == linkHash,
                    "Wrong link"
                );
                if (!kids[_msgSender()].comeSecondTime) {
                    kids[_msgSender()].comeSecondTime = true;
                    linkStatistic[linkHash].totalSonsTwice++;
                }
            } else {
                require(links[linkHash].enabled, "Unactive link");
                _register(links[linkHash].owner, linkHash);
            }
        } else {
            // create new link
            require(
            ...
            require(
                linkFather != address(0) && linkFather != _msgSender(),
                "Wrong father's address"
            );
            require(
                availableFatherPercents.contains(fatherPercent),
                "Wrong father's percent value"
            );
            require(!kids[_msgSender()].registered, "Not new user");

            require(
                fathers[linkFather].linksEndbled.length() < MAX_ENABLED_LINKS,
                "Limit of acive links"
            );
            fathers[linkFather].linksEndbled.add(linkHash);
            fathers[linkFather].allLinks.push(linkHash);

            links[linkHash].enabled = true;
            links[linkHash].owner = linkFather;
            links[linkHash].ownerPercent = fatherPercent;

            _register(linkFather, linkHash);

            emit NewLinkCreated(
                linkHash,
                linkFather,
                fatherPercent,
                DENOMINATOR - fatherPercent
            );
        }

        kidStatistic[_msgSender()].numOfPurchases++;

        uint256 bonusPercent = (linkHash == bytes32(0))
            ? 0
            : getUserBonusPercent(links[linkHash].owner);

        if (dexInfo.enabled) {
            _buyFromDex(usdtAmount, bonusPercent, linkHash);
        } else {
            _buyFromCex(
                usdtAmount,
                btcmtAmount,
    ...
    function getPrice(uint256 payAmount) public view returns (uint256) {
        ...
        if (dexInfo.enabled) {
            ...
            uint256 reserveUsdt = (address(USDT) == dexInfo.token0)
                ? reserve0
                : reserve1;
            uint256 reserveBtcmt = (address(BTCMT) == dexInfo.token0)
                ? reserve0
                : reserve1;

            return (payAmount * reserveBtcmt) / (payAmount + reserveUsdt);
        } else return 0;
    }
    ...
    function _register(address _father, bytes32 _link) internal {
        kids[_msgSender()].registered = true;
        kids[_msgSender()].fatherLink = _link;

        linkStatistic[_link].totalSons++;
        fathers[_father].allKids.push(_msgSender());
        fatherStatistic[_father].totalKids++;
        kidStatistic[_msgSender()].firstPurchaseDate = block.timestamp;

        emit NewUserRegistered(_msgSender(), _father, _link);
    }
    ...
    function _buyFromCex(
        ...
        _buy(payAmount, buyAmount, bonusPercent, link);
    }
    ...
    function _buyFromDex(
        ...
        _buy(payAmount, getPrice(payAmount), bonusPercent, link);
    }
    ...
    function _buy(
        ...
        kidStatistic[_msgSender()].sumOfPurchases += buyAmount;
        kidStatistic[_msgSender()].sumOfBonuses += (bonusAmount - fatherAmount);
