...
contract LotteryContract is Ownable {
    ...
    function adminGiveFreeTickets(uint256 _lotteryId, uint256[] memory _ticketNumber, address _receipent) public onlyOwner{
        require(_ticketNumber.length != 0 && _ticketNumber.length <= maxNumberTicketsPerBuyOrClaim, "Either no ticket specified or To many tickets requested");
        // require(_ticketNumber.length <= maxNumberTicketsPerBuyOrClaim, "Too many tickets");
        require(lotteries[_lotteryId].status == Status.Open && lotteries[_lotteryId].endTime > block.timestamp, "Either lottery is not open or Lottery is ended");
    ...
    function getFinalNo(uint256 randomNo) internal pure returns(uint256) {
        while(true){
            if(randomNo >= 100000 && randomNo <= 999999){
                return randomNo;
            }
            else{
    ...
    function buyTickets(uint256 _lotteryId, uint256[] memory _ticketNumber, uint256 _amount) public {
        require(_ticketNumber.length != 0 && _ticketNumber.length <= maxNumberTicketsPerBuyOrClaim, "Either no ticket specified or To Many Tickets are requested");
        // require(_ticketNumber.length <= maxNumberTicketsPerBuyOrClaim, "Too many tickets");
