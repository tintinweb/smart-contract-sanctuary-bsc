 ...
 */
contract ERC721Escrow is Ownable, ReentrancyGuard, Pausable {
    ...
    function placeOrder(
        ...
        _nftCollection.transferFrom(msg.sender, address(this), _tokenIds[i]);
        orderNonce++;
    ...
    function buyNowPayment (
      ...
      require(order[_orderId].seller != address(0),"order does not exist");  
      require(order[_orderId].endTime>=block.timestamp,"sale ended");
      require(order[_orderId].startTime<=block.timestamp,"sale not started");
      rewardsToken.safeTransferFrom(msg.sender, order[_orderId].seller, order[_orderId].pricePerNFT);
      order[_orderId].nftCollection.transferFrom(address(this),msg.sender,order[_orderId].tokenId);
      delete order[_orderId];
      return true;
    }
    function claimBack(uint256 _orderId)external returns (bool){
      require(order[_orderId].seller != address(0),"order does not exist");  
      require(order[_orderId].seller == msg.sender,"not owner of token");
      require(order[_orderId].endTime<block.timestamp,"buy periord not ended yet");
      order[_orderId].nftCollection.transferFrom(address(this),msg.sender,order[_orderId].tokenId);
      delete order[_orderId];
      return true;
    }
     ...
     */
    function stake(uint256[] calldata _tokenIds , IERC721 _nftCollection) external whenNotPaused {
        ...
        for (uint256 i; i < len; ++i) {
            require(
                _nftCollection.ownerOf(_tokenIds[i]) == msg.sender,
     ...
     */
    function withdraw(uint256[] calldata _tokenIds,IERC721 _nftCollection) external nonReentrant {
        Staker storage staker = stakers[msg.sender];
        require(
        ...
        for (uint256 i; i < lenToWithdraw; ++i) {
            ...
            _nftCollection.transferFrom(address(this), msg.sender, _tokenIds[i]);
        }
