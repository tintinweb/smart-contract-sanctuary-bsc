...
contract NFTOpenBox is INFTOpenBox, Caller {
    ...
    function addToken(
        ...
        _setBoxTokenNFTToken(boxToken_, nftToken_);
    ...
    function setBoxTokenNFTToken(address boxToken_, address nftToken_)
        external
        onlyCaller
    {
        _setBoxTokenNFTToken(boxToken_, nftToken_);
    ...
    function _setBoxTokenNFTToken(address boxToken_, address nftToken_)
        private
    {
        boxToken[boxToken_] = boxToken_;
        nftToken[boxToken_] = nftToken_;
    }
...
interface INFTOpenBox {
    // 开盲盒事件
    event OpenBoxEvent(
        address indexed boxToken, //盲盒代币合约地址
        address indexed nftToken, //NFT合约地址
