...
contract ERC721 is Context, ERC165, IERC721, IERC2981, IERC721Metadata, IERC721Enumerable {
    ...
    mapping(uint256 => RoyaltyInfo) internal _merchantRoyalties;
    ...
    function _setTokenMerchantRoyalty(
        uint256 tokenId,
        address recipient,
        uint256 value
    ) internal {
        
        _merchantRoyalties[tokenId] = RoyaltyInfo(recipient, uint24(value));
    }
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        royaltyAmount = (price * royalties.amount) / 100;
    }
    ...
    function merchantRoyaltyInfo(uint256 tokenId, uint256 price)
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        RoyaltyInfo memory royalties = _merchantRoyalties[tokenId];
        receiver = royalties.recipient;
        royaltyAmount = (price * royalties.amount) / 100;
    }
    ...
    */
    ...
    function _safeMint(address to, uint256 tokenId, uint256 fee,uint256 merchantfee,address merchant) internal virtual {
        _safeMint(to, tokenId, fee,merchantfee,merchant, "");
    }

    function _safeMint(address to, uint256 tokenId, uint256 fee,uint256 merchantfee,address merchant, bytes memory _data) internal virtual {
        _mint(to, tokenId, fee,merchantfee,merchant);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
    }

    function _mint(address to, uint256 tokenId, uint256 fee,uint256 merchantfee,address merchant) internal virtual {
        ...
        _setTokenMerchantRoyalty(tokenId,merchant,merchantfee);
...
contract MusicNFT is ERC721 {
    ...
    function createCollectible(string memory tokenURI, uint256 fee,uint256 merchantFee,address merchant ) public returns (uint256) {
        uint256 newItemId = tokenCounter;
        // verifySign(tokenURI, sign);
        _safeMint(msg.sender, newItemId, fee,merchantFee,merchant);
