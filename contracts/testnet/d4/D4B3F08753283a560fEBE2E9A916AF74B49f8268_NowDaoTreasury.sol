...
contract NowDaoTreasury is Ownable {
    ...
    uint public totalMinted; // Total amount of minted NMETA
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        require( IERC20( Nmeta ).balanceOf(address(this)) >= send_, "Not enough balance" );
        IERC20( Nmeta ).transfer( msg.sender, send_ );
        totalMinted = totalMinted.add( send_ );
    ...
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IERC20( Nmeta ).safeTransferFrom( msg.sender, address(this), value );
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        require( IERC20( Nmeta ).balanceOf(address(this)) >= _amount, "Not enough balance" );

        IERC20( Nmeta ).transfer( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( totalDebt );
    }
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding reserve or lp tokens) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external onlyManager() returns ( bool ) {
        require( !isReserveToken[ _token ] );
        require( !isLiquidityToken[ _token ] );
