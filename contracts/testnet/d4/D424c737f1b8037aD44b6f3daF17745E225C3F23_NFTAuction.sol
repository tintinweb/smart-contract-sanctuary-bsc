...
contract NFTAuction is Ownable, ReentrancyGuard {
    ...
    function initialize(address _platformNormalNFT, address _platformLazyNFT, address _platformERC1155NFT, uint256 _platformPercentage) public onlyOwner {
    ...
    */
    ...
    { 
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd  = nftContractAuctions[_nftContractAddress][_tokenId].auctionBidPeriod + (block.timestamp);  
        ...
        internal
    {
        ...
        nftContractSale[_nftContractAddress][_tokenId].nftSeller = msg
            .sender;
        nftContractSale[_nftContractAddress][_tokenId].ownerPercentage = _ownerPercentage;
        ...
        if(_isERC20Auction(erc20Token)){    // if sale is ERC20
            ...
            if (!success) {
                failedTransferCredits[seller] =
                    failedTransferCredits[seller]+ (totalAmount);
            }
        }
        IERC721(_nftContractAddress).transferFrom(
                address(this),
...

    {
        uint256 bidIncreaseAmount= (nftContractAuctions[_nftContractAddress][
            _tokenId
        ].nftHighestBid)*(100 +_getBidIncreasePercentage(_nftContractAddress, _tokenId))/100;
        require(msg.value>= bidIncreaseAmount || _tokenAmount>= bidIncreaseAmount, "Bid amount not enough");
    ...
    */
        ...
        if(_isERC20Auction(erc20Token)){    // if sale is ERC20
            ...
            if (!success) {
                failedTransferCredits[_nftSeller] =
                    failedTransferCredits[_nftSeller] +(sellerAmount);
            }
        }
    }
    /*
        ...
        isAuctionOver(_nftContractAddress, _tokenId)
    {
        ...
        address erc20Token= nftContractAuctions[_nftContractAddress][_tokenId].ERC20Token;
        nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid=
            totalAmt*(100-settlePenalty)/(100);
        ...
        if(_isERC20Auction(erc20Token)){
            ...
            if (!success) {
                failedTransferCredits[owner] =
                    failedTransferCredits[owner] + (penaltyAmt);
            }
        }
