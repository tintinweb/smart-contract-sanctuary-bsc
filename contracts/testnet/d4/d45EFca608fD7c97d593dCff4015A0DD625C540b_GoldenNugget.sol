...
contract GoldenNugget is Context, Ownable {
    ...
    uint256 private GOLD_TO_DIG_1MINERS = 1080000;//for final version should be seconds in a day
    uint256 private PSN = 10000;
    uint256 private PSNH = 5000;
    uint256 private devFeeVal = 3;
    bool private initialized = false;
    address payable private recAdd;
    mapping (address => uint256) private pitMiners;
    mapping (address => uint256) private claimedGOLD;
    mapping (address => uint256) private lastDig;
    ...
    function DigGOLD(address ref) public {
        require(initialized);
        ...
        uint256 newMiners = SafeMath.div(GOLDUsed,GOLD_TO_DIG_1MINERS);
        pitMiners[msg.sender] = SafeMath.add(pitMiners[msg.sender],newMiners);
        claimedGOLD[msg.sender] = 0;
        lastDig[msg.sender] = block.timestamp;
    ...
    function sellGOLD() public {
        ...
        uint256 eggValue = calculateGOLDell(hasGOLD);
    ...
    function buyGOLD(address ref) public payable {
        require(initialized);
        uint256 GOLDBought = calculateEggBuy(msg.value,SafeMath.sub(address(this).balance,msg.value));
    ...
    function getMyMiners(address adr) public view returns(uint256) {
        return pitMiners[adr];
    }
    
    function getMyGOLD(address adr) public view returns(uint256) {
        return SafeMath.add(claimedGOLD[adr],getGOLDSinceLastDig(adr));
    }
    
    function getGOLDSinceLastDig(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(GOLD_TO_DIG_1MINERS,SafeMath.sub(block.timestamp,lastDig[adr]));
        return SafeMath.mul(secondsPassed,pitMiners[adr]);
    }
