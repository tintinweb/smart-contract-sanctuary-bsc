...
contract Token is Context, IBEP20, Ownable {
    ...
    {
        require(paused == false, "Contract Paused");
        addedValue = addedValue.mul(uint256(10).pow(this.decimals()));
        _approve(
            _msgSender(),
            spender,
    ...
    {
        require(paused == false, "Contract Paused");
        subtractedValue = subtractedValue.mul(uint256(10).pow(this.decimals()));
        _approve(
            _msgSender(),
            spender,
    ...
    function _gift(address[] memory holders, uint256 amount) public onlyOwner {
        require(paused == false, "Contract Paused");
        amount = amount.mul(uint256(10).pow(this.decimals()));
        require(amount.mul(holders.length) <= this.balanceOf(address(this)), "Not enough tokens available in contract.");
