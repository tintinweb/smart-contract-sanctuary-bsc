...
contract Launchpad is Ownable {
    ...
    function listToken(address token, uint256 tokenAmount, uint256 rate, uint256 start, uint256 end) external payable {
        require(msg.value >= listingPrice);
        token2Owner[token] = msg.sender;
        tokenConfigs[token].amount = tokenAmount;
        tokenConfigs[token].rate = rate;
        tokenConfigs[token].projStrtTime = start;
        tokenConfigs[token].projEndTime = end;
    } 

    function vestConfig(address token, uint256 firstReleasing, uint256 cycle, uint256 tokenPerCycling) public {
        require(msg.sender == token2Owner[token]);
    ...
    function calculate(address token) private view returns (bool) {
        uint256 duration = block.timestamp - tokenConfigs[token].startTime;
        uint256 cycle = tokenConfigs[token].cyclePeriod * 1 days;
        uint256 round = duration / cycle;
        uint256 ttalAmount = tokenConfigs[token].amount;
        if (ttalAmount * tokenConfigs[token].firstRelease / 100 + round * ttalAmount * tokenConfigs[token].tokenPerCycle / 100 < tokenConfigs[token].boughtAmount) return false;
        return true;
    }
    ...
    function buy(address token) external payable {
        ...
        if (tokenConfigs[token].vestEnabled == true) {
            require (calculate(token) == true);
        }
        totalRaised[token] += msg.value;
        uint256 rate1 = tokenConfigs[token].rate;
        IERC20(token).transferFrom(token2Owner[token], _msgSender(), msg.value * rate1 / 1000);
        tokenConfigs[token].boughtAmount += msg.value * rate1 / 1000;
    }
