...
contract MpoToken is ERC20Upgradeable, OwnableUpgradeable {
        ...
        if (!w[msg.sender] && !w[recipient] && !w[sender]) {
            ...
            if (
                ...
                if (!transferW[msg.sender] && !transferW[recipient]) {
                    ...
                    burnToken(sender, 2, (transferFee.burning * amount) / 100);
        ...
        returns (bool)
    {
        ...
        if (isPair[msg.sender] || isPair[recipient]) {
            ...
            if (!transferW[msg.sender] && !transferW[recipient]) {
                if (
                    balanceOf(msg.sender) > minHold &&
                    amount + minHold > balanceOf(msg.sender)
                ) {
                    amount = balanceOf(msg.sender) - minHold;
                }
                fee = (transferFee.total * amount) / 100;
                _transfer(
                    msg.sender,
                ...
                burnToken(msg.sender, 2, (transferFee.burning * amount) / 100);
    ...
    function checkLowestHold(uint amount) internal view returns (uint) {
        uint ba = balanceOf(msg.sender);

        if (ba > minHold && amount + minHold > ba) {
            return (ba - minHold);
        }
        return amount;
    }
    ...
    function withdrawToMarketingWallet() public returns (uint) {
        ...
        IMar(marketingWallet).externalCall(ba);
    ...
    function bouns(address user_, uint amount_) internal {
        if (
            ...
            buyTBonus[phases][inv_f] +=
                (buyTBonusInfo[phases].first * amount_) /
                100;
            buyTBonus[phases][inv_s] +=
                (buyTBonusInfo[phases].second * amount_) /
                100;
