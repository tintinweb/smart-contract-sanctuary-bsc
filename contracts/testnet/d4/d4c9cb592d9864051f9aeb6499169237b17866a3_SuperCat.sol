...
contract SuperCat is Context, Ownable{
    ...
    struct LPPool{
        ...
        uint256 liquidity;
        uint256 team_usdt;
        uint256 team_liquidity;
        uint256 team_receive_time;
    }
    ...
    event BindNFT(uint256 _usdt,uint256 _cat,uint256 _liquidity,address nft_contract,uint256 nft_id);
    event AddLiquidity(uint256 _usdt,uint256 _cat,uint256 _usdta,uint256 _cata,uint256 _liquidity);
    event RemoveLiquidity(uint256 _usdt,uint256 _cat,uint256 _liquidity);
    ...
    event NftInfoShot(address contractAddr,uint256 _index,uint256 nft_num,uint256 pay_amount,uint256 pay_type,uint256 cat_release,uint256 relese_time,uint256 lp_amount,uint256 receive_rate);
    event Swap(uint256 usdt_amount,uint256 cat_amount,uint256 w_u, uint256 w_c,uint256 _fee,uint256 _valut);
    ...
    constructor() public {
        ...
        SuperCatVault_addr = 0x3130AD3620ba376d9EC68436DE1FaFa911974E77;
    ...
    function initNftIndex() private{
        nft_index[0x5961a23c0CA9D4d3D5A80b84ed76FD6b836e2964] = 1;
        ...
        nft_index[0x2F552697A5b2bF26b725282f8cDbAaFCD3cF5410] = 7;
    }
    ...
    function initNftInfo() private{
        ...
        nft[7].lp_amount = 50 * usdtToken_decimals;
    ...
    function setNftInfo(address contractAddr,uint256 _index,uint256 nft_num,uint256 pay_amount,uint256 pay_type,uint256 cat_release,uint256 relese_time,uint256 lp_amount,uint256 receive_rate) public onlyOwner{
        nft_index[contractAddr] = _index;
        nft[_index].nft_num = nft_num;
        nft[_index].pay_amount = pay_amount;
        nft[_index].pay_type = pay_type;
        nft[_index].cat_release = cat_release;
        nft[_index].relese_time = relese_time;
        nft[_index].lp_amount = lp_amount;
        nft[_index].receive_rate = receive_rate;
        emit NftInfoShot(contractAddr,_index,nft_num,pay_amount,pay_type,cat_release,relese_time,lp_amount,receive_rate);
    }
    ...
    function swapAmount(uint amountIn,uint amountOutMin,address[] memory path,address to) private returns (uint){
        IPancakePair _pair = IPancakePair(pair);
        require(_pair.isSwap(address(this)), ' SWAP PAUSE');
    ...
    function weightOf(address _addr) public returns (uint256 cat,uint256 usdt){
        (cat,usdt) = IERC20(catToken).weightOf(_addr);
    }
    ...
    function bindNFT(uint256 usdt_amount,uint256 cat_amount,uint256 minusdt_amount,uint256 mincat_amount,uint256 _nftId,address _nft_contract) public{
        ...
        // uint256 mincat_amount = cat_amount.mul(90).div(100);
        (uint256 amountA, uint256 amountB, uint256 liquidity) = _AddLPLiquidity(usdtToken,catToken,usdt_amount,cat_amount,minusdt_amount,mincat_amount,msg.sender,block.timestamp);
        if(usdt_amount>amountA){
            // uint256 usdt_return = usdt_amount - amountA;
            // IERC20(usdtToken).transfer(msg.sender, usdt_return);
            usdt_amount = amountA;
        }
        if(cat_amount>amountB){
            // uint256 cat_return = cat_amount - amountB;
            // IERC20(catToken).transfer(msg.sender, cat_return);
            cat_amount = amountB;
        }
        ...
        if(reserve0>reserve1){
            ...
            lp_pool[msg.sender].cat = reserve1*lp_pool[msg.sender].liquidity/lp_weight_total;
        }else{
            lp_pool[msg.sender].cat = reserve0*lp_pool[msg.sender].liquidity/lp_weight_total;
        ...
        if(lp_pool[msg.sender].usdt >= min_lp && lp_pool[msg.sender].team_receive_time==0 && lp_pool[msg.sender].team_liquidity>0){
            lp_pool[msg.sender].team_receive_time = block.timestamp;
        }
        ...
        team_usdt(usdt_amount,liquidity,true);
        SuperCatVault(SuperCatVault_addr).addLPLiquidity(msg.sender,liquidity);
        emit BindNFT(usdt_amount,cat_amount,liquidity,_nft_contract,_nftId);
    }

    function addLPLiquidity(uint256 usdt_amount,uint256 cat_amount,uint256 minusdt_amount,uint256 mincat_amount) public{
        require(usdt_amount>=usdtToken_decimals, "EE: usdt_amount must be a multiple of 1");        
        // IERC20(catToken).transferFrom(msg.sender,address(this), cat_amount);
        // IERC20(usdtToken).transferFrom(msg.sender,address(this), usdt_amount);
        // (uint256 amountA, uint256 amountB, uint256 liquidity) = Router.addLiquidity(usdtToken,catToken,usdt_amount,cat_amount,minusdt_amount,mincat_amount,msg.sender,block.timestamp);
        (uint256 amountA, uint256 amountB, uint256 liquidity) = _AddLPLiquidity(usdtToken,catToken,usdt_amount,cat_amount,minusdt_amount,mincat_amount,msg.sender,block.timestamp);
        if(usdt_amount>amountA){
            // uint256 usdt_return = usdt_amount - amountA;
            // IERC20(usdtToken).transfer(msg.sender, usdt_return);
        ...
        SuperCatVault(SuperCatVault_addr).addLPLiquidity(msg.sender,liquidity);
        team_usdt(amountA,liquidity,true);
        emit AddLiquidity(usdt_amount,cat_amount,amountA,amountB,liquidity);
    }
    ...
    function removeLPLiquidity(uint256 usdt_amount,uint256 cat_amount,uint256 liquidity) public{
        ...
        (uint256 amountA, uint256 amountB) = Router.removeLiquidity(usdtToken,catToken,liquidity,usdt_amount,cat_amount,msg.sender,block.timestamp);
        if(amountA>usdt_amount){
            usdt_amount = amountA;
        }
        if(amountB>cat_amount){
            cat_amount = amountB;
        }
        ...
        if(lp_pool[msg.sender].usdt < min_lp ){
            lp_pool[msg.sender].team_receive_time = 0;
        }
        ...
        team_usdt(usdt_amount,liquidity,false);
        SuperCatVault(SuperCatVault_addr).RemoveLPLiquidity(msg.sender,liquidity);
        emit RemoveLiquidity(usdt_amount,cat_amount,liquidity);
    }

    // function weightOf() public view (uint256 cat,uint256 usdt){
    //     (cat,usdt) = IERC20(catToken).weightOf(msg.sender);
    // }

    function swap(uint256 _amount,uint256 min_amount,bool _Is) public{
        ...
        if(_Is){
            ...
            uint256 usdt_fee = 0;
            if(usdt>0&&cat>0){
                usdt_fee = usdt * _amount / cat;
            }
            uint256 _fee = _usdt * 15/1000;
            uint256 _valut = 0;
            if(_usdt>usdt_fee&&usdt_fee>0){
            ...
            emit Swap(_usdt,_amount,usdt, cat,_fee,_valut);
        }else{
            uint256 _fee = _amount * 15/1000;
            uint256 trans_amount = _amount - _fee;
            IERC20(usdtToken).transferFrom(msg.sender,SuperCatVault_addr,_fee); 
            fee_total = fee_total+_fee;
            path[0] = usdtToken;
            path[1] = catToken;
            uint256 _cat = swapAmount(trans_amount,min_amount,path,msg.sender);
            _isc_b = true;
            _isu_b = true;
            _cat_w = _cat;
            _usdt_w = _amount;
    ...
    function receiveTeamProfit() public{
        require(lp_pool[msg.sender].team_usdt>0,'EE:: not team amount');
        uint256 _time_interval = block.timestamp - lp_pool[msg.sender].team_receive_time;
        uint256 _amount = (re_release /86400) * (lp_pool[msg.sender].team_liquidity *_time_interval)/(lp_weight_total*15/10) ;
        if(_amount>0){
            IERC20(catToken).transfer(msg.sender, _amount);
            lp_pool[msg.sender].team_receive_time = block.timestamp;
            nft_pledge_info[msg.sender].recommend_cat = nft_pledge_info[msg.sender].recommend_cat + _amount;
            emit ReceiveProfit(msg.sender,_amount,_time_interval,block.timestamp,lp_pool[msg.sender].team_liquidity,lp_weight_total,re_release);
        }
    }
    ...
    function mintNFT(address to,address contractAddress,bool _is)  public onlyOwner {
        ...
        nft_pledge[contractAddress][nft_id].total_profit = nft[_index].cat_release;
        nft_pledge[contractAddress][nft_id].less_profit = nft[_index].cat_release;
        if(_is){
    ...
    //添加团队业绩
    function team_usdt(uint256 _usdt,uint256 _amount,bool _is) private{
        if(_amount>0){
            uint256 team_amount;
            uint256 _team_usdt;
            ...
            for (uint i = 1; i <= 15; i++) {
                ...
                team_amount = _amount * miner_ratio[i].recommend /1000;
                _team_usdt = _usdt * miner_ratio[i].recommend /1000;
                if(_is){
                    lp_pool[pre].team_liquidity = lp_pool[pre].team_liquidity.add(team_amount);
                    lp_pool[pre].team_usdt = lp_pool[pre].team_usdt.add(_team_usdt);
                    // if(lp_pool[pre].team_usdt>=min_lp&&lp_pool[pre].usdt>=min_lp){
                    if(lp_pool[pre].usdt>=min_lp&&lp_pool[pre].team_receive_time==0){
                        lp_pool[pre].team_receive_time = block.timestamp;
                    }
                }else{
                    if(lp_pool[pre].team_liquidity>team_amount){
                        lp_pool[pre].team_liquidity = lp_pool[pre].team_liquidity.sub(team_amount);
                    }else{
                        lp_pool[pre].team_liquidity = 0;
                    }
                    if(lp_pool[pre].team_usdt>_team_usdt){
                        lp_pool[pre].team_usdt = lp_pool[pre].team_usdt.sub(_team_usdt);
                    }else{
                        lp_pool[pre].team_usdt = 0;
                    }
                    // if(lp_pool[pre].team_usdt< min_lp||lp_pool[pre].usdt< min_lp){
                    if(lp_pool[pre].usdt< min_lp||lp_pool[pre].team_liquidity<=0){
                        lp_pool[pre].team_receive_time = 0;
                    }
                }
                pre = relation[pre];
            }
