...
contract Marketplace is ERC721Holder, Ownable {
    ...
    // development fee 15%
    uint256 public developmentFeePercent = 1500;
    // burn fee 5%
    ...
    modifier onlyWhitelistNFT(address _adderess) {
        require(whitelistNft[_adderess] == true, "nft address not whitelist");
        _;
    }

    constructor(uint16 _feePercent,
        uint16 _treasureFeePercent,
        address _treasureFeeAddress,
        uint16 _developmentFeePercent,
        address _developmentFeeAddress,
        uint16 _burnFeePercent,
        address _fuel
     ) {
        require(_feePercent <= 10000, "input value is more than 100%");
        
        feePercent = _feePercent;
        treasureFeePercent = _treasureFeePercent;
        treasureAddress = _treasureFeeAddress;
        developmentFeePercent = _developmentFeePercent;
        developmentAddress = _developmentFeeAddress;
    ...
    function _makeOrder(
        ...
        uint256 _endPrice,
        uint256 _endBlock
    ) internal  onlyWhitelistNFT(address(_token)){
    ...
    function bid(bytes32 _order, uint256 _amount) external {
        Order storage o = orderInfo[_order];
        ...
        fuel.safeTransferFrom(msg.sender, address(this), _amount);

        if (lastBidPrice != 0) {
            require(
                _amount >= lastBidPrice + (lastBidPrice / 20),
                "low price bid"
            ); 
        } else {
            require(_amount >= o.startPrice && _amount > 0, "low price bid");
        }
        ...
        o.lastBidPrice = _amount;

        if (lastBidPrice != 0) {
            fuel.safeTransfer(lastBidder, lastBidPrice);
        }

        emit Bid(o.token, o.tokenId, _order, msg.sender, _amount);
    }
    ...
    function buyItNow(bytes32 _order) external {
        ...
        fuel.safeTransferFrom(msg.sender, address(this), currentPrice);
        o.isSold = true;
        
        uint256 fee = (currentPrice * feePercent) / 10000;
        fuel.safeTransfer(o.seller, currentPrice - fee);
        //fee
        uint256 treasureFee = fee * treasureFeePercent / 10000;
        fuel.safeTransfer(treasureAddress, treasureFee);
        uint256 developmentFee = fee * developmentFeePercent / 10000;
        fuel.safeTransfer(developmentAddress, developmentFee);
        uint256 burnFee = fee * burnFeePercent / 10000;
    ...
    function claim(bytes32 _order) external {
        ...
        fuel.safeTransfer(seller, lastBidPrice - fee);
        //fee
        uint256 treasureFee = fee * treasureFeePercent / 10000;
        fuel.safeTransfer(treasureAddress, treasureFee);
        uint256 developmentFee = fee * developmentFeePercent / 10000;
    ...
    function setWhiteListAddressState(address _addresses, bool _state)
        external
        onlyOwner
    {
        whitelistNft[_addresses] = _state;
    }
}
