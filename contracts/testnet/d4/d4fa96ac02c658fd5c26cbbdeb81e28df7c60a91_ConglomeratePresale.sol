...
contract ConglomeratePresale {
    ...
    function ikeBalance(address _user) public view returns (uint256) {
    ...
    function buyfromToken(
        ...
        if (
            ref == address(0) || ref == msg.sender || referral[msg.sender][ref]
        ) {} else {
        ...
        if (_pid == 1) {
            ...
            if (check == 2) {
                uint256 per5 = (amount * 5) / 100;
                uint256 per95 = (amount * 95) / 100;
            ...
            uint256 temp = amount;
            multiplier = (perDollarPrice * temp) / 10**18;
            ...
            require(
                usdc.allowance(msg.sender, address(this)) > 0,
