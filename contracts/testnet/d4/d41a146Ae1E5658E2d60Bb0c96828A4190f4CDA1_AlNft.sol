...
contract AlNft is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    function init(address alToken_) public {
      tiers[1].mintPrice = 0.001 ether;
      tiers[1].rewardPerSec = 1 ether;
      tiers[2].mintPrice = 0.001 ether;
      tiers[2].rewardPerSec = 1 ether;
      tiers[3].mintPrice = 0.001 ether;
      tiers[3].rewardPerSec = 1 ether;
      tiers[4].mintPrice = 0.001 ether;
      tiers[4].rewardPerSec = 1 ether;
      tiers[5].mintPrice = 0.001 ether;
      tiers[5].rewardPerSec = 1 ether;
      tiers[6].mintPrice = 0.001 ether;
      tiers[6].rewardPerSec = 1 ether;
    ...
    function claimTotalTiersRewards() public returns(uint256) {
      uint256 total = 0;
      for(uint8 i = 1; i <= LAST_ID; i ++) {
        tiers[i].accRewards = rewardsOfTier(i);
        tiers[i].lastRwTimeStamp = block.timestamp;
        total += tiers[i].accRewards;
      }
      return total;
    }
    ...
    function claimRewards(address account) public {
      ...
      lastRwTime[account] = block.timestamp;
    }

    function rewardsOfTier(uint8 id) public view returns(uint256) {
      if (tiers[id].lastRwTimeStamp == 0)
        return 0;
      return tiers[id].accRewards 
        + ((block.timestamp - tiers[id].lastRwTimeStamp) 
    ...
    function rewardBalance(address account) public view returns (uint256) {
        ...
        for (uint8 i = 1; i <= LAST_ID; i++) {
            uint256 reward = balanceOf(account, i) *
                tiers[i].rewardPerSec *
                (block.timestamp - lastRwTime[account]);
