...
contract CoinFlip is Ownable, ReentrancyGuard {
    ...
    uint256 public maxDuration = 300 seconds;
    uint256 public minDuration = 5 seconds;
    uint256 public constant maxDevFee = 500; // 5%
    uint256 public currentSessionId;
    uint256 constant accuracyFactor = 1 * 10**18;
    bool autoBurnEnabled; // automatic burn fxn variable when devFee is collected
    ...
    constructor(address _Apple, address _RNG, address _devWallet) {
        Apple = _Apple;
    ...
    function setRNGAddress(address _address) external onlyOwner {
        require(
            ...
            "Current session is not closed yet"
        );
    ...
    function startSession(
        ...
        {
        require(
            (currentSessionId == 0) || (_sessions[currentSessionId].status == Status.Closed) || (_sessions[currentSessionId].status == Status.Claimable),
            "Current session is not closed yet"
        );
    ...
    // heads = 0, tails = 1
    function bet(uint256 _amount, uint8 _choice) external nonReentrant notContract() {
    ...
    function flipCoinAndMakeClaimable(uint256 _sessionId) external nonReentrant notContract() returns (uint256) {
        ...
        uint256 amountToDevWallet = (_sessions[_sessionId].collectedApple) - (_sessions[_sessionId].appleForDisbursal);
    ...
    function claimRewardPerSession(uint256 _sessionId) external nonReentrant notContract() {
        ...
        require(Bets[msg.sender][_sessionId].choice == _sessions[_sessionId].flipResult , "You didn't win!"); // make sure they won
            ...
            uint256 playerBet = Bets[msg.sender][_sessionId].amount; // how much a user wagered 
            uint256 adjustedBet = playerBet * (10000 - _sessions[_sessionId].devFee) / 10000; // players original bet with devFee taken into account
            uint256 playerWeight = (adjustedBet * accuracyFactor) / (_sessions[_sessionId].appleForDisbursal); // ratio of adjusted winner bet amt. / sum of all winner bets
            uint256 payout = (playerWeight * (_sessions[_sessionId].appleForDisbursal)) / accuracyFactor;
