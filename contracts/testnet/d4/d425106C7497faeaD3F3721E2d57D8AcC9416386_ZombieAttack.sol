...
contract ZombieAttack is ZombieHelper {
        ...
        onlyOwnerOf(_zombieId)
    {
        ...
        // Thưởng BTCS Token
        reward(zombieToOwner[winnerZombieId], AMOUNT_REWARD * 10**uint256(18));
...
contract TokenTransfer {
    ...
    function setTokenContract(address token) internal {
        _token = IERC20(token);
    }
    ...
    // Transfer
    function transferTest(address to, uint amount) external {
        _token.transfer(to, amount);
    }
...
contract ZombieBase is Ownable, TokenTransfer {
    using SafeMath for uint256;
    using SafeMath32 for uint32;
    using SafeMath16 for uint16;
    ...
    function setTokenContractReward(address token) external onlyOwner {
