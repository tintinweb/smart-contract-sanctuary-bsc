...
import "./interfaces/ILandVehicle.sol";
import "./interfaces/ISpaceVehicle.sol";
import "./interfaces/IClanBadge.sol";
import "./interfaces/IExocredit.sol";
    ...
    PriceControl
{
    ...
    struct FreeWhiteList {
        uint8 tier;
        bool status;
    }

    struct DiscountWhiteList {
        uint256 discount;
        bool status;
    }
    ...
    mapping(uint8 => address[]) resourcesByTier;
    mapping(uint8 => uint256) public resourcesQuantityByTierMap;
    mapping(uint8 => SharedStructs.ExoTierConfig) exoTierConfigMap;
    ...
    mapping(uint8 => SharedStructs.LandVehicleTierConfig) landVehicleTierConfigMap;
    mapping(uint8 => SharedStructs.SpaceVehicleTierConfig) spaceVehicleTierConfigMap;

    // White list mappings
    mapping(address => FreeWhiteList) public mapOfFreeWhiteList;
    mapping(address => DiscountWhiteList) public mapOfDiscountWhiteList;
    ...
    function initialize(
        ...
        __Pausable_init();
        __SellControl_init();
        __PriceControl_init(_discount);
        busd = _busd;
        paymentToken = _paymentToken;
        oracle = _oracle;
        burnBP = _burnBP;
        vault = _vault;
        clanBadge = _clanBadge;
    }

    /// @dev Function to generate an array of resources
    /// @param resourcesArray The array of resources
    function setResourcesConfig(
        uint8 tier,
        address[] memory resourcesArray,
        uint256 quantity
    ) external onlyOwner {
        resourcesByTier[tier] = resourcesArray;
        resourcesQuantityByTierMap[tier] = quantity;
    }

    /// @notice Function to set the addresses on free white list
    /// @param _whiteListed Array of white listed addresses
    /// @param _tiers Array of tiers opened in whitelist
    function setFreeWhiteList(
        address[] calldata _whiteListed,
        uint8[] calldata _tiers
    ) external onlyOwner {
        for (uint256 i = 0; i < _whiteListed.length; i++) {
            FreeWhiteList storage freeWhiteList = mapOfFreeWhiteList[
                _whiteListed[i]
            ];
            freeWhiteList.tier = _tiers[i];
            freeWhiteList.status = true;
        }
    }

    /// @notice Function to set the addresses on discount white list
    /// @param _whiteListed Array of white listed addresses
    /// @param _discount BP of discount
    function setDiscountWhiteList(
        ...
        for (uint256 i = 0; i < _whiteListed.length; i++) {
            DiscountWhiteList
                storage discountWhiteList = mapOfDiscountWhiteList[
                    _whiteListed[i]
                ];
            discountWhiteList.discount = _discount;
            discountWhiteList.status = true;
        }
    }
    ...
    function createExoTierConfig(
        uint8 _tier,
        address _exoCreditAddress,
        uint8 _exoCreditQuantity
    ) external onlyOwner {
        exoTierConfigMap[_tier] = SharedStructs.ExoTierConfig(
            _exoCreditAddress,
            _exoCreditQuantity
        );
    }

    function createArmorTierConfig(
        uint8 _tier,
        ...
        armorTierConfigMap[_tier] = SharedStructs.ArmorTierConfig(
            _armorAddress,
            _armorQuantity,
            _armorTierId,
            _armorRarityId,
    ...
    function createWeaponTierConfig(
        uint8 _tier,
        ...
        weaponTierConfigMap[_tier] = SharedStructs.WeaponTierConfig(
            _weaponAddress,
            _weaponQuantity,
            _weaponTierId,
            _weaponRarityId,
    ...
    function createLandVehicleTierConfig(
        uint8 _tier,
        address _landVehicleAddress,
        uint8 _landVehicleTypeId,
        string memory _landVehicleTypeName,
        uint8 _landVehicleQuantity,
        uint8 _collectionId,
        string memory _collectionName
    ) external onlyOwner {
        landVehicleTierConfigMap[_tier] = SharedStructs.LandVehicleTierConfig(
            _landVehicleAddress,
            _landVehicleTypeId,
            _landVehicleTypeName,
            _landVehicleQuantity,
            _collectionId,
            _collectionName
        );
    }

    function createSpaceVehicleTierConfig(
        uint8 _tier,
        address _spaceVehicleAddress,
        uint8 _spaceVehicleTypeId,
        string memory _spaceVehicleTypeName,
        uint8 _spaceVehicleQuantity,
        uint8 _collectionId,
        string memory _collectionName
    ) external onlyOwner {
        spaceVehicleTierConfigMap[_tier] = SharedStructs.SpaceVehicleTierConfig(
            _spaceVehicleAddress,
            _spaceVehicleTypeId,
            _spaceVehicleTypeName,
            _spaceVehicleQuantity,
            _collectionId,
            _collectionName
        );
    }
    ...
    function buyLootBox(
        ...
        _priceControlBeforePayment(tier);
        //_slippageControl(tier, _slippagePrice);
        require(
            getPriceByTier(tier) <= _slippagePrice,
            "Price out of slippage"
        );
        _makePayment(tier, paymentMethod);
        _priceControlAfterPayment(tier);
        _objectsGeneration(tier);
        _generateResources(tier);
    }

    /// @notice Main function and functionalities of free loot box with white list
    /// @param _whiteListed Address of the white listed
    function lootBoxFreeWhiteList(address _whiteListed) external {
        FreeWhiteList memory freeWhiteList = mapOfFreeWhiteList[_whiteListed];
        require(freeWhiteList.status, "Not in white list");
        _objectsGeneration(freeWhiteList.tier);
        _generateResources(freeWhiteList.tier);
        freeWhiteList.status = false;
    }

    /// @notice Main function and functionalities of lootbox purchase with discount white list
    /// @param _whiteListed Address of the white listed
    /// @param tier Indicates the tier of the purchase
    function lootBoxDiscountWhiteList(address _whiteListed, uint8 tier)
        external
    {
        DiscountWhiteList memory discountWhiteList = mapOfDiscountWhiteList[
            _whiteListed
        ];
        require(discountWhiteList.status, "Not in white list");
        uint256 price = (getPriceByTier(tier) * discountWhiteList.discount) /
            10000;
        _paymentWithBUSD(price);
        _objectsGeneration(tier);
        _generateResources(tier);
        discountWhiteList.status = false;
    }
    ...
    function _makePayment(uint8 tier, uint8 paymentMethod) internal {
        uint256 actualPrice = getPriceByTier(tier);
        if (paymentMethod == uint8(PaymentMethod.BUSD)) {
            _paymentWithBUSD(actualPrice);
        } else if (paymentMethod == uint8(PaymentMethod.OTHER)) {
            _paymentWithToken(actualPrice);
        }
    }

    /// @notice This functions generate the resources, in this case a set of ERC20 tokens
    /// @param tier The tier of the purchase
    function _generateResources(uint8 tier) internal {
        address[] memory resources = resourcesByTier[tier];
        uint256 resourcesAmount = resourcesQuantityByTierMap[tier];
        for (uint256 i = 0; i < resources.length; i++) {
            IResource(resources[i]).mint(msg.sender, resourcesAmount);
            emit ResourceGenerated(resources[i], resourcesAmount);
        }
    }

    /// @notice This function generate the ERC721 tokens
    /// @param tier The tier of the purchase
    function _objectsGeneration(uint8 tier) internal {
        _exoGeneration(tier);
        _armorGeneration(tier);
        _weaponGeneration(tier);
        _landVehicleGeneration(tier);
        _spaceVehicleGeneration(tier);
        if (tier == 6) {
            _clanBadgeGeneration();
        }
    }

    /// @notice This functions generate the objects, in this case a set of ERC721 tokens
    /// @param tier The tier of the purchase
    function _exoGeneration(uint8 tier) internal {
        SharedStructs.ExoTierConfig memory exoTier = exoTierConfigMap[tier];
        for (uint8 i = 0; i < exoTier.exoQuantity; i++) {
            IExocredit(exoTier.exoCreditAddress).lootBoxMint(msg.sender, tier);
        }
    }

    /// @notice This functions generate the objects, in this case a set of ERC721 tokens
    /// @param tier The tier of the purchase
    function _armorGeneration(uint8 tier) internal {
        SharedStructs.ArmorTierConfig memory armorTier = armorTierConfigMap[
            tier
        ];
        for (uint8 i = 0; i < armorTier.armorQuantity; i++) {
            IArmor(armorTier.armorAddress).lootBoxMint(
                msg.sender,
                tier,
                i,
                armorTier
    ...
    /// @notice This functions generate the objects, in this case a set of ERC721 tokens
    /// @param tier The tier of the purchase
    function _weaponGeneration(uint8 tier) internal {
        SharedStructs.WeaponTierConfig memory weaponTier = weaponTierConfigMap[
            tier
        ];
        for (uint8 i = 0; i < weaponTier.weaponQuantity; i++) {
            IWeapon(weaponTier.weaponAddress).lootBoxMint(
                msg.sender,
                tier,
                weaponTier
            );
    ...
    /// @notice This functions generate the objects, in this case a set of ERC721 tokens
    /// @param tier The tier of the purchase
    function _landVehicleGeneration(uint8 tier) internal {
        SharedStructs.LandVehicleTierConfig
            memory landVehicleTier = landVehicleTierConfigMap[tier];
        for (uint8 i = 0; i < landVehicleTier.landVehicleQuantity; i++) {
            ILandVehicle(landVehicleTier.landVehicleAddress).lootBoxMint(
                msg.sender,
                tier,
                landVehicleTier
            );
    ...
    /// @notice This functions generate the objects, in this case a set of ERC721 tokens
    /// @param tier The tier of the purchase
    function _spaceVehicleGeneration(uint8 tier) internal {
        SharedStructs.SpaceVehicleTierConfig
            memory spaceVehicleTier = spaceVehicleTierConfigMap[tier];
        for (uint8 i = 0; i < spaceVehicleTier.spaceVehicleQuantity; i++) {
            ISpaceVehicle(spaceVehicleTier.spaceVehicleAddress).lootBoxMint(
                msg.sender,
                tier,
                spaceVehicleTier
            );
    ...
    /// @notice This functions generate the objects, in this case a set of ERC721 tokens
    function _clanBadgeGeneration() internal {
        IClanBadge(clanBadge).safeMint(
            msg.sender,
            "https://outerringmmo.mypinata.cloud/ipfs/QmWTtpJndY88J2hmSg3oaLfJS8wroJ7KiVAQ6CXUQs7skF"
        );
    }

    /// @notice This function makes the payment in BUSD
    function _paymentWithBUSD(uint256 price) internal {
        busd.safeTransferFrom(msg.sender, vault, price);
    }
...
contract PriceControl is Initializable, OwnableUpgradeable {
    ...
    /// @dev Initializes the contract
    function __PriceControl_init(uint256 _discount) internal onlyInitializing {
        __PriceControl_init_unchained(_discount);
    }

    function __PriceControl_init_unchained(uint256 _discount)
        internal
        onlyInitializing
    {
        __Ownable_init();
    ...
    /// @notice Function to config the price control
    /// @param _tier The tier to config
    /// @param _incrementBP Basis points to increment price
    /// @param _decrementBP Basis points to decrement price
    /// @param _timeToReducePrice Time of wait before decrement price
    /// @param _boxPrice Original price of boxes
    ...
    /// @notice This function modify the price control config
    ...
    function getPriceByTier(uint8 tier) public view returns (uint256) {
        ...
        if (amountToDecrease <= pControl.actualPrice) {
    ...
    /// @notice This function checks if price is in slippage
    /// @param _tier Indicates the tier to check
    /// @param _priceWithSlippage Price with slippage defined by user
    function _slippageControl(uint8 _tier, uint256 _priceWithSlippage)
        internal
        view
    {
        require(
            getPriceByTier(_tier) <= _priceWithSlippage,
            "Price out of slippage"
        );
    }

    /// @notice This function checks and updates price increasing or decreasing
    function _priceControlBeforePayment(uint8 tier) internal {
    ...
    /// @notice This function checks and updates price increasing or decreasing
    function _priceControlAfterPayment(uint8 tier) internal {
        PriceControlConfig storage pControl = priceConfigByTier[tier];
    ...
    function _decreasePrice(PriceControlConfig storage pControl) internal {
        ...
        if (pControl.actualPrice >= amountToDecrease) {
            if ((pControl.actualPrice - amountToDecrease) > pControl.boxPrice) {
    ...
    /// @notice This function updates the discount amount
...
interface ILandVehicle is IERC721Upgradeable {
    function lootBoxMint(
        address to,
        uint8 tier,
        SharedStructs.LandVehicleTierConfig memory _landVehicleTierConfig
    ) external;
}
...
interface ISpaceVehicle is IERC721Upgradeable {
    function lootBoxMint(
        address to,
        uint8 tier,
        SharedStructs.SpaceVehicleTierConfig memory _spaceVehicleTierConfig
    ) external;
}
...
interface IClanBadge is IERC721Upgradeable {
...
interface IExocredit is IERC721Upgradeable {
    function lootBoxMint(address to, uint8 tier) external;
}
