...
contract LimitTimeAuction is Member {
    ...
    function getTxFee(uint256 createTime) public view returns(uint256 fee) {
        require(msg.sender == manager.members("owner"));
        // 初始手續費20%，以后每15天递减2.5%共遞減四次直至5%後固定，其中2% 作為版权费給 mint holder，其餘项目方。 
        uint256 round =  (block.timestamp - createTime) / 15 hours;
...
contract NFT_MasterChef is Ownable, Member {
    ...
    uint256[] public nftweight = [100,210,440,92,200,420];
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = IPromote(promote).total3GenBonusWeight();
        ...
        return user.pendingReward.add(userTotalWeight.mul(accSushiPerShare).div(1e12).sub(user.rewardDebt));
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if(block.number >= lastCutBlock.add(CUT_PERIOD)){
            ...
            emit PerShareChanged(sushiPerBlock, sushiPerBlockOrigin.mul(level).mul(lpSupply).div(200000));
            sushiPerBlock = sushiPerBlockOrigin.mul(level).mul(lpSupply).div(200000);
        }
        ...
        if(level2 != level) {
            level = level2;
            emit PerShareChanged(sushiPerBlock, sushiPerBlockOrigin.mul(level2).mul(lpSupply).div(200000));
            sushiPerBlock = sushiPerBlockOrigin.mul(level2).mul(lpSupply).div(200000);
        }
    ...
    function claimReward(uint256 _pid, address _user) internal {
...
contract Promote is Member {
    ...
    uint256 public total3GenBonusWeight;
        ...
        for(uint8 i=0; i < 8; i++) {
            ...
            if (changeToValid) {
                if (i < 6) {
                    ...
                    console.log("user.upline8Gen", user.upline8Gen.length);
                    // 檢查是否為佈道者
                    console.log("start upgradePreacher~~~~");
                    IPreacher(manager.members("PreacherAddress")).upgradePreacher(_user);
                    console.log("end upgradePreacher~~~~");
                }
            }
...
contract Preacher is Member {
    ...
    function upgradePreacher(address _user) external {
        //  IPromote(manager.members("PromoteAddress")).userInfo(_user);
        // console.log('IPromote(manager.members("PromoteAddress"))', );
        console.log("getUser", _user);
        IPromote._UserInfo memory promoteUserInfo = IPromote(manager.members("PromoteAddress")).getUser(_user);
        console.log("getUser.f", promoteUserInfo.f);
        // 檢查傳教士等級升降
        checkPreacherLevel(_user, promoteUserInfo);
        console.log("checkPreacherLevel");
        // 檢查傳教士海盜大將升降
    ...
    function checkPreacherLevel(address _user, IPromote._UserInfo memory promoteUserInfo) internal validSender {
        ...
        if(isPreacher[_user]){
            // 檢查降級條件
            if(promoteUserInfo.level < 2 || promoteUserInfo.numDown6Gen < 150){
                claimReward(_user);
