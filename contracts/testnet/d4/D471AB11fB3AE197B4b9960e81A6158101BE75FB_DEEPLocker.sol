...
contract DEEPLocker {
    ...
    Counters.Counter private _lockTokenId;
    ...
    mapping(address => uint256[]) public lengthTokenLock;
    ...
    function unsetSaleType(string memory _sale, bool _exist) public {
    ...
    function lock(
        ...
        uint256 currentLockId = _lockTokenId.current();
        ...
        _lockTokenId.increment();

        userTokenLock[_owner][currentLockId].push(
            TokenLock(_expiration, _claimAmount, _claimStatus, _amount)
        );

        lengthTokenLock[_owner].push(currentLockId);
    }
    ...
    function unlock(
        ...
        TokenLock[] memory _tokenLock = getTokenLockByIndex(_owner, _index);

        for (uint256 index = 0; index < _tokenLock.length; index++) {
            uint256[] memory expiration = _tokenLock[index].expiration;
            for (uint256 j = 0; j < expiration.length; j++) {
                if (expiration[j] == _expiration) {
                    uint256 amount = _tokenLock[index].claimAmount[j];
                    deepToken.safeTransfer(msg.sender, amount);
                    userTokenLock[_owner][_index][j].claimStatus[j] = true;
                }
            }
        }
    }

    function getTokenLockLength(address _owner) public view returns (uint256) {
        return lengthTokenLock[_owner].length;
    }

    function getTokenLockByIndex(address _owner, uint256 _index)
        public
        view
        returns (TokenLock[] memory)
    {
        TokenLock[] memory _tokenLock = userTokenLock[_owner][_index];
        return _tokenLock;
    }
}
