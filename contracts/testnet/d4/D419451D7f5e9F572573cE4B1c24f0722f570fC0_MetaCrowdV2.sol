...
contract MetaCrowdV2 is AccessControlEnumerable {
    ...
    uint16[] public bonus = [0, 20, 30, 40, 45, 50];
    /*
     ...
     */
    uint16[] public defaultParams = [110, 20, 700, 10, 50, 600, 500, 10, 200, 10, 100, 2, 5, 10];
    ...
    struct ProjectFund {
        uint16 version;
        uint256 fund; //投入本金
        uint256 refund; //已返回本金
        uint256 reserve; //预约金额
        uint256 reserveFund; //预约支付金额
        uint256 reserveRefund; //重启返回预约支付金额
        uint256 priorityRefund; //重启返回优先额度
    }
    ...
    function setRegionParams(uint256 regionIndex, uint16 paramIndex, uint16 value) external onlyRole(OPERATOR_ROLE) {
        regions[regionIndex].params[paramIndex] = value;
    }

    function getRegionParams(uint256 regionIndex) public view returns(uint16[] memory) {
    ...
    function addRegion(
        ...
        regions.push(Region({period: period, initAmount: initAmount, multiplier: multiplier, startTime: startTime, currentRound: 0, poolAmount: 0, params: defaultParams}));
    ...
    function fund(uint256 regionIndex, uint256 amount) external {
        ...
        require(block.timestamp > p.startTime && block.timestamp < p.startTime + min(regions[regionIndex].period, uint256(regions[regionIndex].params[13]) * 60), "funding time limited");
    ...
    function reserve(
        ...
        require(recordMetaPoint(msg.sender), "no meta point account");
        require(amount > 0, "zero amount error");
        require(accounts[msg.sender].lastFundTime + fundInterval < block.timestamp, "reserve time limited");
        Project storage p = projects[regionIndex][round];
    ...
    function finishProject(uint256 regionIndex) public {
        ...
        if (currentAmount == p.targetAmount) {
            ...
            if (round >= 3 && projects[regionIndex][round - 3].state == 3) {
                ...
                uint256 refundAmount = (targetAmount * (1000 - regions[regionIndex].params[2] + regions[regionIndex].params[0] + bonus[5])) / 1000 - p3.brokerRewardLeft;
    ...
    function pending(
        ...
        view
        returns (
            uint256[] memory amounts, //查询收益 0:投入本金,1:返回本金,2:静态收益,3:免费提现额度,4:优先额度,5:已返回本金
            uint16 state,
            uint16 version,
            bool restart
        )
    {
        ...
        if (pf.version == projects[regionIndex][round].version) {
            if (state == 3 && p.refundAmount > 0) {
                amounts[1] = (fundAmount * regions[regionIndex].params[2]) / 1000 - amounts[5]; //减去已返回本金
            } else if (state == 4) {
            ...
            amounts[1] += pf.reserveRefund;
            amounts[3] += pf.reserveRefund;
            amounts[4] += pf.priorityRefund;
        } else {
            restart = true;
            amounts[1] += (pf.reserveFund + pf.reserveRefund);
            amounts[3] += (pf.reserveFund + pf.reserveRefund);
    ...
    function recordMetaPoint(address account) public returns (bool) {
    ...
    function processBrokerReward(
        ...
        if (bonus[5] > used) {
            projects[regionIndex][round].brokerRewardLeft += (fundAmount * (bonus[5] - used)) / 1000;
        }
    }
    ...
    function account_mapping(address[] calldata addresses) external onlyRole(OPERATOR_ROLE) {
        MetaCrowdV2 metaCrowd = MetaCrowdV2(0x23115626Efc42abF9eF2399648533C9bc236D2aa);
        for (uint256 i = 0; i < addresses.length; i++) {
            (uint256 priorityFundAmount, uint256 freeWithdrawAmount, , , , uint8 vips, , address head, ) = metaCrowd.accounts(addresses[i]);
            Account storage account = accounts[addresses[i]];
