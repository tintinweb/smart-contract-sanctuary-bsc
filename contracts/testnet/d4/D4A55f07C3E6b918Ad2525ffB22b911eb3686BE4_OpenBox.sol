...
contract OpenBox is Ownable {
    ...
    mapping(uint => string) public importRequestHash;
    ...
    function randomHash(uint group, uint nonce, uint tokenId, bool isImport, uint _importRequestId) internal {
        ...
        else importRequestHash[_importRequestId] = itemsFlag[group][resultNumber];
    }
    function handleOpen(uint nonce, uint tokenId, bool isImport, uint _importRequestId) internal {
        ...
        nonces[nonce] = true;
        if(resultNumber < 15) randomHash(0, nonce, tokenId, isImport, _importRequestId);
        else if(resultNumber >= 15 && resultNumber < 53) randomHash(1, nonce, tokenId, isImport, _importRequestId);
        else if(resultNumber >= 53 && resultNumber < 89) randomHash(2, nonce, tokenId, isImport, _importRequestId);
