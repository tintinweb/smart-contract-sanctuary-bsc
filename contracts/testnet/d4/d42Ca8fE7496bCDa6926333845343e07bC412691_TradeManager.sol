...
contract TradeManager is TradeOwner, ReentrancyGuard {
  ...
  function createNewTrade(
    ...
    if (token0 == address(0)) {
      ...
      TransferHelper.safeTransferETH(address(this), token0Amount);
    } else {
...
library TransferHelper {
    function safeApprove(address token, address to, uint value) internal {
        ...
        require(success, 'TransferHelper: APPROVE_FAILED'); //  && (data.length == 0 || abi.decode(data, (bool)))
    }

    function safeTransfer(address token, address to, uint value) internal {
        // bytes4(keccak256(bytes('transfer(address,uint256)')));
        (bool success,) = token.call(abi.encodeWithSelector(0xa9059cbb, to, value));
        require(success, 'TransferHelper: TRANSFER_FAILED'); //  && (data.length == 0 || abi.decode(data, (bool)))
    }

    function safeTransferFrom(address token, address from, address to, uint value) internal {
        // bytes4(keccak256(bytes('transferFrom(address,address,uint256)')));
        (bool success,) = token.call(abi.encodeWithSelector(0x23b872dd, from, to, value));
        require(success, 'TransferHelper: TRANSFER_FROM_FAILED'); //  && (data.length == 0 || abi.decode(data, (bool)))
    }
    ...
    function depositStakeToken(address stakingRewards, address token, address beneficiary, uint256 amount) internal {
        // bytes4(keccak256(bytes('deposit(address,address,uint256)')));
        (bool success,) = stakingRewards.call(abi.encodeWithSelector(0x8340f549, token, beneficiary, amount));
        require(success, 'stakingRewards Deposit Token: DEPOSIT_FAILED'); //  && (data.length == 0 || abi.decode(data, (bool)))
    }

    function withdrawStakeToken(address stakingRewards, address token, address beneficiary, uint256 amount) internal {
        // bytes4(keccak256(bytes('withdraw(address,address,uint256)')));
        (bool success, bytes memory data) = stakingRewards.call(abi.encodeWithSelector(0xd9caed12, token, beneficiary, amount));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'stakingRewards Withdraw Token: WITHDRAW_FAILED');
    }
}
