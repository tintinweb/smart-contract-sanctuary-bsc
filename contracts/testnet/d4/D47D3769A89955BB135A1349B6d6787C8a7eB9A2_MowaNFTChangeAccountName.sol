...
contract MowaNFTChangeAccountName is Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    MowaItemsCore public mowaItemsCore;
    IERC1155 public mowaItemsNFT;
    uint256 public chracterIdFee = 25;
    uint256 public qtyFee = 1;

    struct UserChangeName {
        string name;
        uint256 timeChange;
    }
    mapping(address => UserChangeName) public userChangeName;
    ...
    function changeChracterIdFee(uint256 chracterId) public onlyOwner {
        chracterIdFee = chracterId;
    }

    function changeQtyFee(uint256 fee) public onlyOwner {
        qtyFee = fee;
    }

    function petUpgradeLevel(string memory name) public {
//        require(keccak256(abi.encodePacked(name)) != keccak256(abi.encodePacked("")), "Username cannot be left blank");
       uint256 tokenId = mowaItemsCore.getTokenWithCharacter(chracterIdFee);
        require(mowaItemsNFT.balanceOf(_msgSender(), tokenId) >= qtyFee, "not enough fee");
        mowaItemsCore.burnNFT(_msgSender(), tokenId, qtyFee);
        userChangeName[_msgSender()].name = name;
        userChangeName[_msgSender()].timeChange = block.timestamp;
    }
}
