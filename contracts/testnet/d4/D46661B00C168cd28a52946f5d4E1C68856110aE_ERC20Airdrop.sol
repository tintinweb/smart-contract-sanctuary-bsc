...
contract ERC20Airdrop is ERC20, ERC20Burnable, Pausable, Ownable {
    ...
    constructor(
        ...
        bytes32 _merkleRoot,
        bytes32[][] memory _proofsBytes
    )   
        ERC20(_name, _symbol) {
        merkleRoot = _merkleRoot; // Update root
        proofsBytes = _proofsBytes;
    }
    ...
    function convertProofToBytes32(string[] memory proof) internal pure returns (bytes32[] memory) {
        bytes32[] memory proofBytes = new bytes32[](proof.length);
        for (uint i = 0; i < proof.length; i++) {
            proofBytes[i] = bytes32(bytes(proof[i]));
        }
        return proofBytes;
    }
    ...
    /// @param proof index of the merkle proof to use
    function claimAirdrop(address to, uint256 amount, string[] memory proof) external {
        // Throw if address has already claimed tokens
        if (hasClaimed[to]) revert AlreadyClaimed();

        // Convert proof to bytes32
        bytes32[] memory proofBytes = convertProofToBytes32(proof);

        // Verify merkle proof, or revert if not in tree
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, amount));
        bool isValidLeaf = MerkleProof.verify(proofBytes, merkleRoot, leaf);
