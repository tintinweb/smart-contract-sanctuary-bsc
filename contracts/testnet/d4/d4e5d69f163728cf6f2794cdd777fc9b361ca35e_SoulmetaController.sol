...
import "../interface/I_soul_minter_1155.sol";
...
contract SoulmetaController is DivestorUpgradeable, RandomGeneratorUpgradeable {
    ...
    ISoulMinter1155 public aSoulMinter;
    mapping(address => uint) public payAmount;       //购买数量
...
//        }
        ...
        function setContracts(address[] calldata addrs_, bool status_, bool isOpenCast_, bool isOpenExtract_) public onlyOwner {
            ...
            aSoulMinter = ISoulMinter1155(addrs_[12]);
    ...
    function buildBox(uint8 theme_, uint8 group_, uint soulCount_, uint amount_, address recommender_) public {
        ...
        uint count = aSoulMinter.balanceOf(_msgSender(), 1);
    ...
    function drawBox(address caller_, uint8 theme_, uint8 group_, uint amount_, address recommender_) public returns (bool){
        ...
        //目标池子数量
        uint targetCount = _getTarget(caller_, amount_);

        payAmount[caller_] += amount_;
        ...
        for (uint8 i = 0; i < 3; i++) {
            if (i == 0 && targetCount > 0 ){
                poolId = getPoolId(theme_, group_, 10);
                (ids, counts) = cSoulStore.extractBoxBatch(poolId, targetCount);
...
//    }
     ...
     *  获取指定池子的数量
     **/
    function _getTarget(address caller_, uint amount_) public view returns(uint){
        uint openCount = payAmount[caller_];
        uint balance = (amount_ + openCount)/3 - openCount/3 + (amount_ + openCount)/10 - openCount/10 - ((amount_ + openCount)/30 + openCount/30);
        return balance;
    }
}
