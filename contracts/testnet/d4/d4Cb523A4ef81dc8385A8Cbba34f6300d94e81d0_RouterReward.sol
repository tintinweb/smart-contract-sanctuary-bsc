...
contract AddLiquUitl is SafeOwnable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        address pair = IEMEFactory(emeSwapV2Router.factory()).getPair(
            address(TOKEN),
            address(USDT)
        );
        if (pair == address(0)) {
            pair = IEMEFactory(emeSwapV2Router.factory()).createPair(
                address(TOKEN),
                address(USDT)
            );
        }
        emeSwapV2Pair = IEMEPair(pair);
    }

    function swapAndLiquify(uint256 contractTokenBalance) public {
        TOKEN.safeTransferFrom(msg.sender, address(this), contractTokenBalance);
        ...
        swapTokensForToken(half, address(TOKEN), address(USDT));
...
contract RouterReward is AddLiquUitl {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address;
    ...
    mapping(address => uint256) public stakeLiu;
    ...
    constructor(
        ...
        address _swapRouter,
        address _treasury
    ) AddLiquUitl(_token, _usdt, _swapRouter) {
        TOKEN = IERC20(_token);
        router = _router;
    ...
    function updateTransferFee(
        ...
        require(_hole.add(_fund).add(_bonus) <= 100, "not rate");
        holeFee = _hole;
        fundFee = _fund;
        bounsFee = _bonus;
    }
    ...
    function downgrade(address _inviter) private {
        ...
        if (invLevel == 1 && teamUsdt[_inviter] < 26000 * 10**18) {
        ...
        if(invLevel != 0) {
            SmartDisPatchInitializable(smartDisPatch[invLevel]).withdraw(_inviter, 1);
        }
        invLevel = invLevel - 1;
        SmartDisPatchInitializable(smartDisPatch[invLevel]).stake(_inviter, 1);
    ...
    function stakeFor(address account, uint256 liquidity) public {
        ...
        emeSwapV2Pair.transferFrom(account, address(this), liquidity);

        calculateInvReward(account, liquidity);

        updateTeamAmount(account, liquidity, true);
        updateStakeTeam(account, liquidity);
        emit Staked(account, liquidity);
    }

    function withdrawFor(address account, uint256 liquidity) public {
        require(router == msg.sender, "not router");
        ...
        updateWithdrawTeam(account, liquidity);
    ...
    function updateStakeTeam(address _account, uint256 _team) private {
        address _inviter = userMap[_account];
        address que;
    ...
    function updateWithdrawTeam(address _account, uint256 _team) private {
        address _inviter = userMap[_account];
        if (userLevel[_inviter] != 1) return;
        address que;
        for (uint256 i = 0; i != 20; i++) {
            if (_inviter == address(0) || que == _inviter) {
                break;
    ...
    function calculateInvReward(address _sender, uint256 reward) private {
        ...
        if (balanceOf(inviterAddr) < power) {
            ...
            emeSwapV2Pair.transfer(
                treasury,
                reward.mul(40).div(100).sub(value.div(10**18))
            );
        } else {
            userActive[inviterAddr] = userActive[inviterAddr].add(
                reward.mul(40).div(100)
            );
        }
        //team reward
        inviterAddr = _sender;
    ...
    function donate(uint256 reward) external {
        TOKEN.safeTransferFrom(msg.sender, hole, reward.mul(holeFee).div(100));
        ...
        swapAndLiquify(bouns.mul(addLiquidityFee).div(100));
        ...
        returns (BounsInfo[] memory bonusInfo)
    {
        ...
        bonusInfo[1].pool = smartDisPatch[1];
        bonusInfo[1].bonus = v1;
        uint256 v2 = SmartDisPatchInitializable(smartDisPatch[2]).available(
            account
        );
        bonusInfo[2].pool = smartDisPatch[2];
        bonusInfo[2].bonus = v2;
        uint256 v3 = SmartDisPatchInitializable(smartDisPatch[3]).available(
            account
        );
        bonusInfo[3].pool = smartDisPatch[3];
        bonusInfo[3].bonus = v3;
        uint256 v4 = SmartDisPatchInitializable(smartDisPatch[4]).available(
            account
        );
        bonusInfo[4].pool = smartDisPatch[4];
        bonusInfo[4].bonus = v4;
    }
