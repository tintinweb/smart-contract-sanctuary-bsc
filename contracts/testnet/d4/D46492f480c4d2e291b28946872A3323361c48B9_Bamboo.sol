...
contract Bamboo is Context, Ownable {
    ...
    mapping(address=>uint) private _balances;//余额
    ...
    mapping(address=>uint) private _userStaticRewardTotal;//用户静态奖总收益
    ...
    mapping(address=>uint) public _userStakeLastTime;
    ...
    constructor() {
        ...
        _userStakeTotal[_first]=500*10**18;
    }
    //设置参数-------------------start
    ...
    function getLevel(uint userId) public view returns(uint){
        ...
        for(uint i;i<_investmentLevelBetweens.length;i++){
            if(i%2==1) continue;
            if(i==_investmentLevelBetweens.length-2){
                //最大级别
                if(inAmount>=_investmentLevelBetweens[i]){
                    return _investmentLevelBetweens.length/2;
                }
            }else if(inAmount>=_investmentLevelBetweens[i]&&inAmount<_investmentLevelBetweens[i+1]){
                if(i==0) return 1;
                return i/2+1;
            }
        }
    ...
    // function stakeHisory(address account,uint amount,bool status) internal {
    //     if(_userStakeList[account].length==0){
    //     }else{
    //         if(_userStakeList[account][_userStakeList[account].length-1].dayTime!=bambooLibrary.dayTime()){
    //             _userStakeList[account].push(bambooLibrary.stakeInfo(bambooLibrary.dayTime(),usdt));
    //         }else{
    //             _userStakeList[account][_userStakeList[account].length-1].amount=_userStakeList[account][_userStakeList[account].length-1].amount.
    //         }
    //         info
    //     }
    // }
    ...
    //+
    function _plus(address account,uint amount) internal {
        _balances[account]=_balances[account].add(amount);
    }
    //-
    function _sub(address account,uint amount) internal {
        require(_balances[account]>=amount,"error9");
