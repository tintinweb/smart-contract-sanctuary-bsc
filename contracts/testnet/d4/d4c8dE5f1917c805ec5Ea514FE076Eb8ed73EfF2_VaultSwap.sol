...
contract VaultSwap is Initializable{
    ...
    /**
        @dev swapping vault with the token
        @param wantToken address of wantToken
        @param vaultToken address of vaultToken
        @param amount amount user want to send
     */
    function vaultToToken(address wantToken,address vaultToken,uint amount) external {
        (address vault, bool isLp,) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(vaultToken));
        IOptVaultFactory(IAdmin(Admin).optVaultFactory()).withdraw(msg.sender,vaultToken,false,amount);
        uint balance = IERC20(vaultToken).balanceOf(address(this));
        if(isLp){
            ILpSwap(IAdmin(Admin).LPSwap()).LPToToken(vaultToken,wantToken,balance,msg.sender);
        }
        else{
            if(wantToken != vaultToken){
                ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoTOKEN(wantToken,vaultToken,balance,msg.sender);
                }
            else{
                IERC20(vaultToken).safeTransfer(msg.sender,balance);
                }
            }
        }
    function vaultToLp(address wantToken,address vaultToken,uint amount)external {
        (address vault, bool isLp,) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(vaultToken));
        IOptVaultFactory(IAdmin(Admin).optVaultFactory()).withdraw(msg.sender,vaultToken,false,amount);
        uint balance = IERC20(vaultToken).balanceOf(address(this));
        if(isLp){
            if(wantToken != vaultToken){
                ILpSwap(IAdmin(Admin).LPSwap()).LPToLP(vaultToken,wantToken, msg.sender, amount);
            }
            else{
                IERC20(vaultToken).safeTransfer(msg.sender,balance);
            }
        }else{
            ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoLP(wantToken,vaultToken,balance,msg.sender);
        }
    }
    /**
        @dev swapping desired vault with the vaultToken
        @param wantToken address of wantToken
        @param vaultToken address of vaultToken
        @param amount amount user want to send
     */
    function vaultToVault(address wantToken,address vaultToken,uint _level,uint amount)external {
        (address vault, bool isLp,) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(vaultToken));
        (, bool isLpWant,) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(wantToken));
        IOptVaultFactory(IAdmin(Admin).optVaultFactory()).withdraw(msg.sender,vaultToken,false,amount);
        uint balance = IERC20(vaultToken).balanceOf(address(this));
        if(isLp){
            if(isLpWant){
        ...
        IOptVaultFactory(IAdmin(Admin).optVaultFactory()).Deposit(msg.sender,wantToken,_level,IERC20(wantToken).balanceOf(address(this)));
    }
    /**
        @dev swapping desired vault with the vaultToken 
        @param wantToken address of wantToken
        @param vaultToken address of vaultToken
        @param amount amount user want to send
        @param minAmount amount is greater than or equal to the amount 
     */
    function vaultToReciept(address wantToken,address vaultToken,uint amount, uint minAmount) external {
        (address vault, bool isLp,address recieptToken) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(vaultToken));
        (, bool isLpWant,) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(wantToken));
        IOptVaultFactory(IAdmin(Admin).optVaultFactory()).withdraw(msg.sender,vaultToken,false,amount);
        uint balance = IERC20(vaultToken).balanceOf(address(this));
        // address recieptToken = recieptInstance;
        IERC20(recieptToken).safeApprove(vaultToken,amount);
        if(isLp){
            if(isLpWant){
                ILpSwap(IAdmin(Admin).LPSwap()).LPToLP(vaultToken,wantToken, msg.sender, amount);
            ...
            IERC20(vaultToken).safeApprove(vault,IERC20(vaultToken).balanceOf(address(this)));
            IOptVault(vault).purchase(msg.sender,IERC20(vaultToken).balanceOf(address(this)), minAmount);
        }else{
            if(isLpWant){
                ITokenSwap(IAdmin(Admin).tokenSwap()).swapTOKENtoTOKEN(wantToken,vaultToken,balance,msg.sender);
            }else{
