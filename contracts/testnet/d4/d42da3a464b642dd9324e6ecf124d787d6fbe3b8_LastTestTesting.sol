...
contract LastTestTesting is Context, IERC20, Ownable {
    ...
    uint256 private constant CRO_DECIMALS = 18;
    uint256 private constant USDT_DECIMALS = 18;

    // Cronos Testnet
    address private constant USDT = 0xE2064B0B8576C2a433E45A61aBc8aFD5F05B4e0b;
    // Cronos
    ...
    uint256 private constant JACKPOT_DEATHROLL_MIN = 5000 * 10**USDT_DECIMALS;
    uint256 private constant JACKPOT_DEATHROLL_MAX = 250000 * 10**USDT_DECIMALS;

    uint256 private constant JACKPOT_BUYER_SHARE_MIN = 5000;
    uint256 private constant JACKPOT_BUYER_SHARE_MAX = 10000;

    uint256 private constant JACKPOT_MINBUY_MIN = 5 * 10**(CRO_DECIMALS);
    uint256 private constant JACKPOT_MINBUY_MAX = 500 * 10**(CRO_DECIMALS);

    uint256 private constant JACKPOT_CASHOUT_MIN = 4000;
    uint256 private constant JACKPOT_CASHOUT_MAX = 7000;

    uint256 private constant JACKPOT_DEATHROLL_BUYBACK_MIN = 3000;
    uint256 private constant JACKPOT_DEATHROLL_BUYBACK_MAX = 7000;

    string private constant _name = "LastTestTesting";
    string private constant _symbol = "LTT";
    ...
    // Buy fees 10%
    // 3% liquidity
    uint256 public bLiquidityFee = 300;
    // 3% marketing
    ...
    // Sell fees 10%
    // 3% liquidity
    uint256 public sLiquidityFee = 300;
    // 3% marketing
    uint256 public sMarketingFee = 300;
    // 1% dev
    uint256 public sDevFee = 100;
    // 3% jackpot
    uint256 public sJackpotFee = 300;
    ...
    // 60% jackpot cashout to last buyer
    uint256 public jackpotCashout = 6000;
    // 90% of jackpot cashout to last buyer
    uint256 public jackpotBuyerShare = 9000;
    // Buys > 0.1 CRO will be eligible for the jackpot
    uint256 public jackpotMinBuy = 10 * 10**(CRO_DECIMALS);
    // Jackpot time span is initially set to 10 mins
    uint256 public jackpotTimespan = 10 * 60;
    // Jackpot hard limit, CRO value
    uint256 public jackpotHardLimit = 20000 * 10**(CRO_DECIMALS);
    ...
    uint256 private _lastDEATHROLLCash = 0;
    uint256 private _lastDEATHROLLTokens = 0;
    uint256 private _lastDEATHROLLTimestamp = 0;
    ...
    uint256 public numTokensSellToAddToLiquidity = 200_000 * 10**_decimals;

    // Pending balances (CRO) ready to be collected
    uint256 private _pendingMarketingBalance = 0;
    uint256 private _pendingDevBalance = 0;
    uint256 private _pendingJackpotBalance = 0;

    // Total CRO/LAS collected by various mechanisms (dev, marketing, jackpot)
    ...
    event DevFeesCollected(uint256 CROCollected);
    event MarketingFeesCollected(uint256 CROCollected);
    ...
    event DEATHROLL(uint256 cashedOut, uint256 tokensOut);
    ...
    event DEATHROLLFeaturesChanged(
        uint256 jackpotHardBuyback,
        uint256 jackpotHardLimit
    );

    event JackpotFund(uint256 CROSent, uint256 tokenAmount);
    ...
    constructor() Ownable(msg.sender) {
        ...
        uniswapV2Router = IUniswapV2Router02(
            // Cronos TestNet
            //0x989dBb40f0B8a431e9D79dfd28fdC3df3717D9c0
        ...
        _isExcludedFromFee.add(msg.sender);
    ...
    function setJackpotFeatures(
        ...
        require(
            ...
            "Jackpot min buy needs to be between 0.05 and 0.5 CRO"
        );
    ...
    function setJackpotHardFeatures(
        ...
        require(
            _jackpotHardBuyback >= JACKPOT_DEATHROLL_BUYBACK_MIN &&
                _jackpotHardBuyback <= JACKPOT_DEATHROLL_BUYBACK_MAX,
        ...
        require(
            hardLimitUsd >= JACKPOT_DEATHROLL_MIN &&
                hardLimitUsd <= JACKPOT_DEATHROLL_MAX,
            "Jackpot hard value limit for the big bang needs to be between 30K and 250K USD"
        );
        jackpotHardLimit = _jackpotHardLimit;

        emit DEATHROLLFeaturesChanged(jackpotHardBuyback, jackpotHardLimit);
    }
    ...
    function fundJackpot(uint256 tokenAmount) external payable onlyAuthorized {
        ...
        uint256 CROSent = msg.value;
        _pendingJackpotBalance = _pendingJackpotBalance.add(CROSent);
        if (tokenAmount > 0) {
            transferBasic(msg.sender, address(this), tokenAmount);
            _jackpotTokens = _jackpotTokens.add(tokenAmount);
        }

        emit JackpotFund(CROSent, tokenAmount);
    }
    ...
    function usdEquivalent(uint256 CROAmount) public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = USDT;

        return uniswapV2Router.getAmountsOut(CROAmount, path)[1];
    }
    ...
    function jackpotBuyerShareAmount() public view returns (uint256, uint256) {
        uint256 CRO = _pendingJackpotBalance
            .mul(jackpotCashout)
            .div(MAX_PCT)
    ...
    {
        return (_lastDEATHROLLCash, _lastDEATHROLLTokens, _lastDEATHROLLTimestamp);
    }
    ...
    function processDEATHROLL() private lockTheSwap {
        ...
        emit DEATHROLL(cashedOut, tokensOut);

        _lastDEATHROLLCash = cashedOut;
        _lastDEATHROLLTokens = tokensOut;
        _lastDEATHROLLTimestamp = block.timestamp;
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 tokensForCROExchange = toBeSwapped.sub(pureLiqTokens);

        uint256 initialBalance = address(this).balance;
        swapTokensForCRO(tokensForCROExchange);

        // How many CROs did we gain after this conversion?
        uint256 gainedCRO = address(this).balance.sub(initialBalance);

        // Calculate the amount of CRO that's assigned to the marketing wallet
        uint256 balanceToMarketing = gainedCRO.mul(marketingTokens).div(
            tokensForCROExchange
        );
        _pendingMarketingBalance += balanceToMarketing;

        // Same for dev
        uint256 balanceToDev = gainedCRO.mul(devTokens).div(
            tokensForCROExchange
        );
        _pendingDevBalance += balanceToDev;

        // Same for Jackpot
        uint256 balanceToJackpot = gainedCRO.mul(jackpotTokens).div(
            tokensForCROExchange
        );
        _pendingJackpotBalance += balanceToJackpot;

        uint256 remainingCRO = gainedCRO
            .sub(balanceToMarketing)
            .sub(balanceToDev)
            .sub(balanceToJackpot);

        if (liqTokens >= LIQ_SWAP_THRESH) {
            // The leftover CROs are purely for liquidity here
            ...
            uint256 prevCROBalance = address(this).balance;
            addLiquidity(pureLiqTokens, remainingCRO);
            uint256 usedCROs = prevCROBalance.sub(address(this).balance);
            ...
            // Reallocate the unused CROs to the pending marketing wallet balance
            if (usedCROs < remainingCRO) {
                _pendingMarketingBalance += remainingCRO.sub(usedCROs);
            }

            emit SwapAndLiquify(tokensForCROExchange, usedCROs, usedTokens);
        } else {
            // We could have some dust, so we'll just add it to the pending marketing wallet balance
            _pendingMarketingBalance += remainingCRO;

            emit SwapAndLiquify(tokensForCROExchange, 0, 0);
        }
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 CROAmount) private {
        ...
        // Add the liquidity
        uniswapV2Router.addLiquidityETH{value: CROAmount}(
            address(this),
            tokenAmount,
    ...
    function prepForLPRemoval(address payable receiver) public onlyOwner {
        // Recover any trapped CRO
