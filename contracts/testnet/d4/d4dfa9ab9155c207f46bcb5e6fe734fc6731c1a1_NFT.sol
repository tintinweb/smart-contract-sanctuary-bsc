    ...
    IERC721Enumerable
{
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        ownerByTokenId[to].push(tokenId);
    ...
    function _transfer(
        ...
        delete ownerByTokenId[from][tokenId];
        ownerByTokenId[to].push(tokenId);
...
contract NFT is Ownable, ERC721, IERC721Receiver {
    ...
    mapping(uint256 => address) public authers; //创作者
    mapping(uint256 => uint256) public nftPrice; //nft价值
    mapping(uint256 => bool) public nftTemp; //体验nft
    mapping(address => uint256) public userTemp; //体验用户
    mapping(address => uint256) public userWhite; //白名单
    ...
    address public daoAddress = 0x787E515DeFCD20fB2629d339a2a6B92771350Bb0; //dao地址
    address public usdAddress = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; //usdt合约 0x55d398326f99059fF775485246999027B3197955
    uint256 public basePrice = 100 * 1e18;
    uint256 public tempPrice = 50 * 1e18;
    uint256 public maxNft = 10000;

    constructor() ERC721("Plato-NFT", "Plato-NFT") {}

    //是否仅Miner可以铸造
    function setCheckMiner(bool _value) public onlyOwner {
        isCheckMiner = _value;
    }

    //是否仅白名单可铸造
    function setCheckWhite(bool _value) public onlyOwner {
        isCheckWhite = _value;
    }

    function setDaoAddress(address _value) public onlyOwner {
        daoAddress = _value;
    }
    ...
    //设置miner
    function setMiner(address new_addr, bool _value) public onlyOwner {
        miners[new_addr] = _value;
    }
    ...
    function setTempPrice(uint256 _value) public onlyOwner {
        tempPrice = _value;
    }
        ...
        returns (uint256)
    {
        ...
        uint256 newItemId = _mintToCaller(caller, tokenURI);
        if (basePrice > 0) {
            if (whiteList[caller] && userWhite[caller] == 0) {
                userWhite[caller] = newItemId;
            } else {
                TransferHelper.safeTransferFrom(
                    usdAddress,
                    msg.sender,
    ...
    // 体验nft
    function tempMintToCaller(address caller, string memory tokenURI)
        public
        onlyOwner
        returns (uint256)
    {
        uint256 newItemId = _mintToCaller(caller, tokenURI);
        nftPrice[newItemId] = tempPrice;
        return newItemId;
    }

    function _mintToCaller(address caller, string memory tokenURI)
        private
        returns (uint256)
    {
        //白名单才允许铸造
