...
contract Flikker is Ownable {
  ...
  function depositFund() external payable onlyOwner {
    ...
    uint256 tokenBalance = tokenAddress.balanceOf(address(tokenAddress));
    IERC20(tokenAddress).approve(fundAddress, tokenBalance);
    tokenAddress.transferFrom(address(tokenAddress), fundAddress, tokenBalance);
  }
