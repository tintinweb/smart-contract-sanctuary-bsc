...
contract TradeLine is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    TradeLineDividendTracker public dividendTracker;
    ...
    address payable public _buybackWalletAddress = payable(0x038AD631521D4130A7d016986EE6B73093AB850c);
    ...
    constructor() BEP20("Trade Line", "Trade") 
    {
        dividendTracker = new TradeLineDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "TradeLine: The dividend tracker already has that address");
        TradeLineDividendTracker newDividendTracker = TradeLineDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "TradeLine: The new dividend tracker must be owned by the TradeLine token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TradeLine: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TradeLine: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TradeLine: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TradeLine: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TradeLine: Cannot update gasForProcessing to same value");
...
contract TradeLineDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("TradeLine_Dividend_Tracker", "TradeLine_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 1+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TradeLine_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TradeLine_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TradeLine contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TradeLine_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TradeLine_Dividend_Tracker: Cannot update claimWait to same value");
