/*
   __  __ ______ _____           __     _______ ______ _      _____  
  |  \/  |  ____/ ____|   /\     \ \   / /_   _|  ____| |    |  __ \ 
  | \  / | |__ | |  __   /  \     \ \_/ /  | | | |__  | |    | |  | |
  | |\/| |  __|| | |_ | / /\ \     \   /   | | |  __| | |    | |  | |
  | |  | | |___| |__| |/ ____ \     | |   _| |_| |____| |____| |__| |
  |_|  |_|______\_____/_/    \_\    |_|  |_____|______|______|_____/ 
...
contract DividendDistributor is IDividendDistributor {
    ...
    //Testnet
    IERC20 public ocash = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
...
contract MegaYield2 is Initializable, OwnableUpgradeable {
    ...
    address public MegaDividendReceiver;
    ...
    bool public _autoLiquiditybyLimit;
    ...
    mapping (address => bool) public RewardDisabler; 

    /* ============= Anti Whale Mechanism ===================*/

    mapping(address => bool) public isSellLimitExempt;
    bool public enableDisableAntiWhale;  //true -> enable
    uint256 public txfee;
    uint256 public TwentyFourhours;  //initally 24 hrs
    ...
    function initialize() public initializer {
        ...
        INITIAL_FRAGMENTS_SUPPLY = 5 * 10**9 * 10**_decimals;

        MAX_SUPPLY = 1 * 10**12 * 10**_decimals; //1T

        gonSwapThreshold = 1000 * 10**_decimals;
        ...
        _sellTax = _sellLiquidity.add(_sellTreasury).add(_sellRewards).add(_sellBurnFee).add(
            _sellDevFee
        );
        _buyTax = _buyLiquidity.add(_buyTreasury).add(_buyRewards).add(_buyBurnFee).add(
            _buyDevFee
        );
        ...
        TreasuryWallet = 0x8ff44A05A31b588245BfC4dC7E3fc265E657baCC;
        DeveloperWallet = 0x97E0Ceb36E37646e58B05d7B5A461e05c0906417;  
        ...
        nextRebase = 1660991835;
        nexthalving = 1660991835 + 2629743;
    ...
    function runInit() external onlyOwner {
        ...
        address router_ = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; //testnet
        ...
        MegaDividendReceiver = address(distributor);
        ...
        isDividendExempt[address(MegaDividendReceiver)] = true;
    ...
    function _transferFrom(
        ...
        if(AutoReward && !RewardDisabler[msg.sender]) {
    ...
    function takeFee(
        ...
        if (automatedMarketMakerPairs[sender]) {
            ...
            if (UplineAddressBuyer != address(0)) {
                ...
                uint adjust = _realFee - referee;

                uint256 _uplineBuyerReward = gonAmount.mul(referrer).div(feeDenominator);

                feeAmount = gonAmount.mul(adjust).div(feeDenominator);
    ...
    function ActivateAntiWhale(address from,uint amount) private {
        ...
        uint256 onePercent = balanceOf(from).mul(txfee).div(feeDenominator); //Should use variable
    ...
    function swapBack() private swapping {
        ...
        if (
            !_autoLiquiditybyLimit &&
            contractTokenBalance > gonSwapThreshold
        ) {
            contractTokenBalance = gonSwapThreshold;
        }
        ...
        uint256 amountToTreasury = contractTokenBalance
            .mul(_buyTreasury.add(_sellTreasury))
            .div(realTotalFee);

        uint256 amountToBurn = contractTokenBalance
            .mul(_buyBurnFee.add(_sellBurnFee))
            .div(realTotalFee);
        ...
        uint256 amountToRewardDiv = contractTokenBalance
            .sub(amountToLiquify)
        ...
        if (amountToTreasury > 0) {
            _swapTokensForBNB(amountToTreasury, TreasuryWallet);
        }
        ...
        if(amountToBurn > 0) {
            uint256 gonAmount = amountToBurn;
            _gonBalances[address(this)] = _gonBalances[address(this)].sub(gonAmount);
            _gonBalances[DEAD] = _gonBalances[DEAD].add(gonAmount);
            emit Transfer(address(this),DEAD,gonAmount);
        }

        if (amountToRewardDiv > 0) {
            uint256 initialBalance = address(this).balance;
            _swapTokensForBNB(amountToRewardDiv, address(this));
            uint256 newBalance = address(this).balance.sub(initialBalance);
    ...
    function setSwapByLimit(bool status) external onlyOwner {
        _autoLiquiditybyLimit = status;
    }

    function setSwapAndLiquify(bool status) external onlyOwner {
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return
            (_totalSupply.sub(_gonBalances[DEAD]).sub(_gonBalances[ZERO]));
    }
    ...
    function setAutoReward(bool _status) external onlyOwner {
        AutoReward = _status;
    }

    function setRewardDisabler(address _adr, bool _status) external onlyOwner {
        RewardDisabler[_adr] = _status;
    }

    function enableAntiWhale(bool _status) external onlyOwner {
        enableDisableAntiWhale = _status;
    }
    
    function setSellLimitPer(uint _percentage) external onlyOwner {
        txfee = _percentage;
    }

    function setSellLimitTimer(uint _timeInSec) external onlyOwner {
        TwentyFourhours = _timeInSec;
    }
