...
contract CsDogeToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private feeThreshold = 40000000000 * PRECISION;
    ...
    function swapAndLiquify() private lockTheSwap {
        if(liquidityFee >= feeThreshold) {
            ...
            liquidityFee = 0;
        }  else if(marketFee >= feeThreshold) {
            swapTokensForEth(marketFee, marketFeeAddress);
            marketFee = 0;
        } else if(nftFee >= feeThreshold) {
    ...
    function _transfer(
        ...
        if (
            (liquidityFee >= feeThreshold || nftFee >= feeThreshold || marketFee >= feeThreshold || burnFee >= feeThreshold) &&
            !inSwapAndLiquify &&
            from != pancakePair &&
