...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    /// `accCakePerShare` Accumulated RP1s per share, times 1e12.
    ...
    ///     the RP1 rewards to all the PancakeSwap products.
    ...
    /// @param _MASTER_CHEFF The PancakeSwap MCV1 contract address.
    ...
    constructor(
        IMasterChef _MASTER_CHEFF,
        IERC20 _RP1,
        uint256 _MASTER_PID,
        address _burnAdmin
    ) {
        MASTER_CHEF = _MASTER_CHEFF;
        RP1 = _RP1;
    ...
    /// only for RP1 distributions within PancakeSwap products.
    function add(
        uint256 _allocPoint,
        IERC20 _lpToken,
        bool _isRegular,
    ...
    function pendingCake(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cakeReward = multiplier.mul(cakePerBlock(pool.isRegular)).mul(pool.allocPoint)/(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accCakePerShare = accCakePerShare.add(cakeReward.mul(ACC_RP1_PRECISION)/(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid))/(BOOST_PRECISION);
        return boostedAmount.mul(accCakePerShare)/(ACC_RP1_PRECISION).sub(user.rewardDebt);
    }
    ...
    function cakePerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_RP1_PER_BLOCK.mul(cakeRateToRegularFarm)/(RP1_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_RP1_PER_BLOCK.mul(cakeRateToSpecialFarm)/(RP1_RATE_TOTAL_PRECISION);
        }
    }
    ...
    function cakePerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_RP1_PER_BLOCK.mul(cakeRateToBurn)/(RP1_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accCakePerShare = pool.accCakePerShare.add((cakeReward.mul(ACC_RP1_PRECISION)/(lpSupply)));
            }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            pool.totalBoostedShare = pool.totalBoostedShare.add(_amount.mul(multiplier)/(BOOST_PRECISION));
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(multiplier)/(BOOST_PRECISION).mul(pool.accCakePerShare)/(
            ACC_RP1_PRECISION
        );
        poolInfo[_pid].totalBoostedShare = poolInfo[_pid].totalBoostedShare.sub(
            _amount.mul(multiplier)/(BOOST_PRECISION)
        );
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        uint256 boostedAmount = amount.mul(getBoostMultiplier(msg.sender, _pid))/(BOOST_PRECISION);
    ...
    function updateBoostMultiplier(
        ...
        user.rewardDebt = user.amount.mul(_newMultiplier)/(BOOST_PRECISION).mul(pool.accCakePerShare)/(
            ACC_RP1_PRECISION
        );
        pool.totalBoostedShare = pool.totalBoostedShare.sub(user.amount.mul(prevMultiplier)/(BOOST_PRECISION)).add(
            user.amount.mul(_newMultiplier)/(BOOST_PRECISION)
        );
        poolInfo[_pid] = pool;
    ...
    function settlePendingCake(
        ...
        uint256 boostedAmount = user.amount.mul(_boostMultiplier)/(BOOST_PRECISION);
        uint256 accCake = boostedAmount.mul(poolInfo[_pid].accCakePerShare)/(ACC_RP1_PRECISION);
