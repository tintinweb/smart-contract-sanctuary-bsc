...
contract TreasureNFT is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => bool) public userBlacklist; // Add users on BlackList

    //BNF-01, SFR-01
    event Initialize(string baseURI);
    event TokenMint(address indexed to, uint indexed tokenId, string uid);

    function initialize(
        string memory baseURI
    ) public initializer {
        __ERC721_init("Treasure NFT", "TreasureNFT");
    ...
    function getUID(uint tokenId) external view returns (string memory) {
        return _tokens[tokenId].uid;
    }
    ...
    function addBlackListUser(address[] calldata addresses) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < addresses.length; i++) {
            userBlacklist[addresses[i]] = true;
        }
    }

    function delBlackListUser(address[] calldata addresses) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < addresses.length; i++) {
            delete userBlacklist[addresses[i]];
        }
    }
    ...
    function mint(address to, uint256 total, string calldata uid, address token, bool isFreeze) public onlyRole(TOKEN_MINTER_ROLE) nonReentrant {
        require(to != address(0), "Address can not be zero");
        _lastTokenId += 1;
        uint tokenId = _lastTokenId;
        _tokens[tokenId].createTimestamp = block.timestamp;
        _tokens[tokenId].total = total;
        _tokens[tokenId].uid = uid;
        _tokens[tokenId].stakeFreeze = isFreeze;
        _tokens[tokenId].isExchanged = false;
        _tokens[tokenId].token = token;
    ...
    function launchpadMint(
        ...
        address token,
        bool freeze
    ) public onlyRole(LAUNCHPAD_TOKEN_MINTER) nonReentrant returns (uint256 id) {
        require(to != address(0), "Address can not be zero");
        _lastTokenId += 1;
        uint tokenId = _lastTokenId;
        ...
        _tokens[tokenId].total = total;
        _tokens[tokenId].uid = uid;
        _tokens[tokenId].stakeFreeze = freeze;
        _tokens[tokenId].isExchanged = false;
        _tokens[tokenId].token = token;

        _safeMint(to, tokenId);
        
        // Must set freeze after mint
        _tokens[tokenId].stakeFreeze = freeze;
        return _lastTokenId;
    }
    ...
    function launchpadBurn(
        ...
        // _burn(tokenId);
        _tokens[tokenId].stakeFreeze = true;
        _tokens[tokenId].isExchanged = true;
    ...
    function _safeMint(address to, uint256 tokenId) internal override {
        ...
        emit TokenMint(to, tokenId, _tokens[tokenId].uid);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && paused == true) {
            revert("ERC721: Paused");
        }
        
        if (userBlacklist[from] == true) {
            revert("ERC721: Blacklist");
        }
