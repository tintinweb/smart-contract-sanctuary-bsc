...
// File: contracts/nft-collection-2.sol
...
contract AICarePro is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable {

    uint256 public constant RUBY = 1;
    uint256 public constant SAPPHIRE = 2;
    uint256 public constant DIAMOND = 3;

    uint256 public rubyPrice;
    uint256 public sapphirePrice;
    ...
    function initialize(
        ...
        __ERC1155_init("ipfs://QmeDYLWc6EoAohqbJWMR6LRmujEFUumySgT5hhZjb6KjYq/{id}.json");
        __Ownable_init();
        __Pausable_init();
        ...
        rubyPrice = 100 ether;
        sapphirePrice = 500 ether;
        diamondPrice = 1000 ether;
  ...
  function mint(address _to, uint _id, uint _amount) external {
    ...
    if (msg.sender != owner()) {
          ...
          else if(_id == 2)
          {
            busdContract.safeTransferFrom(
                    msg.sender,
                    owner(),
                    sapphirePrice * _amount
            );
            _total = sapphirePrice * _amount;
          }
          else
          {
            busdContract.safeTransferFrom(
                    msg.sender,
                    owner(),
                    diamondPrice * _amount
            );
            _total = diamondPrice * _amount;
          }
    }
    ...
    function updatePrice(uint256 _rubyPrice, uint256 _sapphirePrice, uint256 _diamondPrice) public payable {
        require(
            owner() == msg.sender,
            "Only owner can update price."
        );
        rubyPrice = _rubyPrice;
        sapphirePrice = _sapphirePrice;
