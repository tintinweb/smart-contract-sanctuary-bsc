...
// Dependency file: contracts/libraries/SafeDecimal.sol
...
// Dependency file: contracts/interfaces/IOnwardIncentivesController.sol
...
// import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
// import "contracts/interfaces/IOnwardIncentivesController.sol";

interface IMasterChef {
    function owner() external view returns (address);

    function emos() external view returns (address);

    function xEmos() external view returns (address);

    function startTime() external view returns (uint256);

    function emosPerSecond() external view returns (uint256);
    ...
    // Update the given pool's EMOS allocation point. Can only be called by the owner.
    ...
    // Stake EMOS tokens to MasterChef
    function enterStaking(uint256 _amount) external;

    // Withdraw EMOS tokens from STAKING.
    function leaveStaking(uint256 _amount) external;

    // Deposit LP tokens to MasterChef for EMOS allocation.
    ...
    function setEmosPerSecond(uint256 _emosPerSecond) external;
}
...
// Root file: contracts/tools/Dashboard.sol
...
// import "contracts/libraries/SafeDecimal.sol";
...
contract Dashboard {
    ...
    IEvmoSwapFactory private factory;
    ...
    function rewardPerYearOfPool(uint pid) view public returns (uint) {
        ...
        return master.emosPerSecond().mul(multiplier).mul(SEC_PER_YEAR).mul(allocPoint).div(master.totalAllocPoint());
    }
    ...
    function valueOfAsset(address asset, uint amount) public view returns (uint valueInETH, uint valueInUSD) {
        ...
        } else if (keccak256(abi.encodePacked(IEvmoSwapPair(asset).symbol())) == keccak256("EMOS-LP")) {
