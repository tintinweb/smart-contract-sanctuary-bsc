...
interface IinstanceRegistry {
    ...
    function instances(ITornadoInstance _tornado) external view returns (
        bool isERC20,
        IERC20 token,
        InstanceState state,
...
contract WhiteProxyLight {
    ...
    function deposit(
        ...
        (bool isERC20, IERC20 token, IinstanceRegistry.InstanceState state, , ) = instanceRegistry.instances(_tornado);
        require(state != IinstanceRegistry.InstanceState.DISABLED, "The instance is not supported");

        if (isERC20) {
            token.transferFrom(msg.sender, address(this), _tornado.denomination());
        }
        _tornado.deposit {
            value: msg.value
        }(_commitment);
    ...
    function withdraw(
        ...
        (, , IinstanceRegistry.InstanceState state, , ) = instanceRegistry.instances(_tornado);
        require(state != IinstanceRegistry.InstanceState.DISABLED, "The instance is not supported");
