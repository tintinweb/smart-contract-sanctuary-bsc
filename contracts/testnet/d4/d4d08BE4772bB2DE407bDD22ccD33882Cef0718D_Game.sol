...
contract Game is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 private _totalSilver = 50;
    uint256 private _totalGold = 15;
    ...
    uint256 private gen0Price = 1200 * 10 ** 18;
    ...
    function mint(uint256 _amount) external {
        require(_circulatingSupply + _amount <= 500, 'All tokens were minted');
    ...
    function setMintPriceGoldenHour(uint256 newPrice, uint256 startTime) external onlyOwner {
        if (startTime == 0) {
    ...
    function removeMultipleToWhitelist(address[] memory _addresses) external onlyOwner {
        for (uint256 i; i < _addresses.length; i++) {
            _removeToWhitelist(_addresses[i]);
        }
    }
    ...
    function _removeToWhitelist(address _address) internal {
