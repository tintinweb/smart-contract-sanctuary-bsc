...
contract ApeHeroVault is ApeHeroPOWEmission, ReentrancyGuard {
    ...
    struct ContractData {
        ...
        uint currentWeek;
        uint emissionStart;
        uint unlockStartWeek;
        uint unlockFullWeek;
    }

    mapping(address => uint) private BALANCES;
    mapping(address => uint) private LAST_WITHDRAWAL_WEEK;

    event Activated(uint timestamp);
    event Withdraw(address sender, uint amount, uint timestamp);
    event Deposit(address depositor, address forAccount, uint amount, uint timestamp);

    constructor(address pow) Ownable() {
        POW = pow;
    }

    // --- OWNER --- //
    function activateVault() public onlyOwner {
        ...
        // After contract is configured, we immediately renounce ownership of Vault!
        renounceOwnership();
    ...
    // -- PUBLIC -- //
    function unlockStarted() public view returns (bool) {
        return (currentWeek() >= UNLOCK_FULL_WEEK) || (currentWeek() >= UNLOCK_START_WEEK);
    }

    function unlockedPercentForCurrentWeek() public view returns (uint) {
        if (!unlockStarted()) {
            return 0;
        }
        if (currentWeek() >= UNLOCK_FULL_WEEK) {
            return 1e4;
        }
        return UNLOCK_SCHEDULE[currentWeek() - UNLOCK_START_WEEK];
    }

    function unlockedBalance(address account) public view returns (uint) {
        if (LAST_WITHDRAWAL_WEEK[account] == currentWeek()) {
            return 0;
        }
        return (BALANCES[account] * unlockedPercentForCurrentWeek()) / 1e4;
    }

    function deposit(address forAccount, uint amount) public {
        bool _deposited = IPOW(POW).transferFrom(_msgSender(), address(this), amount);
        require(_deposited, "Deposit failed");

        BALANCES[forAccount] += amount;
        emit Deposit(_msgSender(), forAccount, amount, block.timestamp);
    }

    function withdraw() public nonReentrant {
        uint _cWeek = currentWeek();
        address  _account = _msgSender();
        require(_cWeek > LAST_WITHDRAWAL_WEEK[_account], string(abi.encodePacked("Unlocked balance already withdrew for week ", _cWeek.toString())));

        uint _unlockedAmount = unlockedBalance(_account);
        require(_unlockedAmount > 0, "Insufficient unlocked balance");

        BALANCES[_account] -= _unlockedAmount;
        bool _success = IPOW(POW).transfer(_account, _unlockedAmount);
        require(_success, "Withdrawal failed");
        LAST_WITHDRAWAL_WEEK[_account] = currentWeek();

        emit Withdraw(_account, _unlockedAmount, block.timestamp);
    }

    function cData() public view returns (ContractData memory) {
        uint _balance = address(POW) == address(0) ? 0 : IPOW(POW).balanceOf(address(this));
        return ContractData(
            POW,
            _balance,
            (_balance * unlockedPercentForCurrentWeek()) / 1e4,
            currentWeek(),
            EMISSION_START,
            UNLOCK_START_WEEK,
            UNLOCK_FULL_WEEK
    ...
    function wData(address account) public view returns (VaultAccountData memory) {
        return VaultAccountData(
            BALANCES[account],
            unlockedBalance(account),
            LAST_WITHDRAWAL_WEEK[account]
        );
    }

    function aData() public view returns (VaultAccountData memory) {
