...
contract YSL is ERC20, AccessControl, Pausable, IEvents, ReentrancyGuard {
    ...
    address public BUSD;
    uint256 public compensationLimit = 10000 * 10**18; //Total amount for compensation
    uint256 public priceImpactProtection = 10; //Price impact protection of 1%
    uint256 public lockTransactionTime = 10 seconds;
    //For testing 1 day = 10s; for deployment 1 day = 86400s;
    uint256 public DAYS_IN_SECONDS = 300;
    uint256 public YSL_Tax = 1500; //Tax always multiply by 100
    uint256 public YSL_ThresholdLimit = 1000 * 10**18; //amount after YSL swap to BUSD
    uint256[] public YSL_Tax_Allocation = [1000, 500]; //index-1 => temporaryHolding, index-2 => Treasury; All index value multiply by 100;
    uint256 public DaysCompensationsLimit = 180;
    IAdmin public Admin;

    mapping(address => uint256) public compensationUsers; // amount to compensate
    mapping(address => uint256) public restrictTransfer; // last block number when interacted
    mapping(address => uint256) public transactionTimeLimit; // timelimit for transaction

    constructor(address _admin) ERC20("YSL", "YSL") {
        Admin = IAdmin(_admin);
    ...
    **/
    modifier _isBurner() {
        require(hasRole(BURNER_ROLE, msg.sender), "YSL: Caller is not  Burner");
        _;
    }
    ...
    **/
     ...
     */
    function setYSLThresholdLimit(uint256 _YSL_ThresholdLimit)
        external
        _isAdmin
    {
    ...
    **/
    function setDaysCompensationLimit(uint256 _DaysCompensationLimit)
        external
        _isAdmin
    {
        ...
        emit SetterForUint(
            "YSLToken",
            address(this),
            DaysCompensationsLimit,
            _DaysCompensationLimit,
            block.number,
            block.timestamp
        );
    ...
    function claimYSL() external nonReentrant {
        ...
        uint256 day = (block.timestamp - deploymentTime) / DAYS_IN_SECONDS;
        require(day <= DaysCompensationsLimit, "YSL: Period over");
        require(totalSupply() < compensationLimit, "YSL: reached limit");
        uint256 amount = totalSupply() + compensationUsers[msg.sender];
    ...
    */
    function getUnClaimedYSL() external _isAdmin {
        ...
        require(
            totalSupply() < compensationLimit,
            "YSL: No unclaimed tokens left"
        );
        uint256 amount = compensationLimit - totalSupply();
        ...
        if (
            ...
            if (IWhitelist(Admin.whitelist()).getAddressesOfSwap(user)) {
                ...
                if (
                    ...
                    require(
                        prevAmount +
                            ((prevAmount * priceImpactProtection) / 1000) >=
                            currentAmount,
                        "YSL: priceImpactProtection"
                    );
                    tax(taxAmount);
                    exitRate(sender, amount);
                    transactionTimeLimit[user] = block.timestamp;
    ...
    function tax(uint256 taxAmount) internal {
        ...
        if (Admin.buyBackActivation()) {
            if (
                ...
                (10**18 + 10**16) <=
                    IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(
                        10**18,
                        path
                    )[1]
            ...
            swap(taxAmount, address(this));
            path[0] = Admin.BUSD();
            path[1] = Admin.USDy();
            IUniswapV2Router02(Admin.ApeswapRouter()).swapExactTokensForTokens(
                IReceipt(Admin.BUSD()).balanceOf(address(this)),
                0,
                path,
                address(this),
            ...
            IReceipt(Admin.USDy()).burn(
                address(this),
                IReceipt(Admin.USDy()).balanceOf(address(this))
            );
        } else {
            if (
                Admin.buyBackActivationEpoch() + (3 * Admin.epochDuration()) <
            ...
            swap(
                (taxAmount * YSL_Tax_Allocation[0]) / YSL_Tax,
                Admin.temporaryHolding()
            );
            swap(
                ((taxAmount * YSL_Tax_Allocation[1]) / (YSL_Tax)),
                Admin.Treasury()
            );
        }
    }
    ...
    **/
    ...
    function exitRate(address sender, uint256 amount) internal view {
        uint256 newTime = block.timestamp - transactionTimeLimit[sender];
        uint256 timeInDays = newTime / DAYS_IN_SECONDS;
        uint256 exitRateAmount;
        if (timeInDays > 2) {
            uint256 exitRateOfDay = timeInDays - 2;
