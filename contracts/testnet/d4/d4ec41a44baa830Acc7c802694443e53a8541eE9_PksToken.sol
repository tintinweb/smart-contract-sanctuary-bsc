...
contract PksToken is Context, IERC20, Ownable {
    ...
    function processLP(uint256 gas) private {
        if (progressLPBlock + 200 > block.timestamp) {
            return;
        }
        ...
        uint256 pksBalance = balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (pairBalance > 0 && !_isExcludedFromFee[shareHolder]) {
                amount = usdtBalance * pairBalance / totalPair;
                PksAmount = pksBalance * pairBalance / totalPair;
                if (amount > 0 && PksAmount > 0 ) {
                    USDT.transfer(shareHolder, amount);
                    transfer(shareHolder, PksAmount);
                }
            }
