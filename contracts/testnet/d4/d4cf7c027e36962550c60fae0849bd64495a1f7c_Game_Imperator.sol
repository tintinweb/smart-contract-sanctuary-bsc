...
contract Game_Imperator is BEP20Token {
    ...
    event MyLog(address indexed Emperor, uint256 sum, uint256 number_imperia, uint256 date);
    event EmperorLog(address indexed Legion, uint256 money, uint256 number_imperia, uint256 date);
    event Referalsum(address indexed my_referrals, uint256 revards, uint256 number_imperia, uint256 date);
    ...
    function TEST() public view returns(address){
        return  all_game[0].adr[all_game[0].count];
    }
   
   function TEST2(uint256 number) public view returns(uint256){
        return  all_game[number-1].number[msg.sender];
    }
}