...
import "./interfaces/IMinerInterface.sol";
...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    mapping(address => uint) userToPurchasedAmountMapping;
    ...
    mapping(address => uint16) userToElectricityVoucherAmountMapping;
    ...
    mapping(uint8 => uint[10]) public operationTypeToPercentagesMapping;
    ...
    function initialize(
        ...
        address _nftReflectionPoolAddress,
        address _netGymStreetAddress,
        uint8 _standardParcelSlotsCount,
        uint8 _upgradedParcelSlotsCount
        ...
        operationTypeToPercentagesMapping[OPERATION_TYPE_MINT_MINERS] = [20,5,7,8,10,3,39,7,5,23];
        operationTypeToPercentagesMapping[OPERATION_TYPE_PURCHASE_REPAIR] = [20,5,7,8,10,3,39,7,5,23];
        operationTypeToPercentagesMapping[OPERATION_TYPE_MINT_PARCELS] = [20,5,6,4,15,3,39,7,5,23];
        operationTypeToPercentagesMapping[OPERATION_TYPE_PURCHASE_ELECTRICITY] = [20,0,40,0,40,0,0,0,0,0];
    }
    ...
    function setStandardParcelsSoldCountPricing(uint256[] calldata prices) public onlyOwner {
    ...
    function setBusinessParcelsSoldCountPricing(uint256[] calldata prices) public onlyOwner {
        require(prices[0] == 0, "The given array must start from 0");
        for (uint256 i = 0; i < prices.length; i += 2) {
            uint256 soldCount = prices[i];
            uint256 price = prices[i + 1];
    ...
    function setStandardParcelAddress(address _standardParcelNFTAddress) public onlyOwner {
        standardParcelNFTAddress = _standardParcelNFTAddress;
    }

    function setBusinessParcelAddress(address _businessParcelNFTAddress) public onlyOwner {
        businessParcelNFTAddress = _businessParcelNFTAddress;
    }

    function setMinerV1Address(address _minerV1NFTAddress) public onlyOwner {
    ...
    function mintParcels(
        ParcelMintSignature calldata mintingSignature,
        bool checkSigner
    ) public payable returns (MintedNFT memory) {
        if(checkSigner) {
        ...
        if (mintingSignature.currencyType == CURRENCY_TYPE_BNB) {
            ...
            uint allowance = IERC20Upgradeable(busdAddress).allowance(msg.sender, address(this));
        ...
        returns (MintedNFT memory)
    {
        ...
        for (uint256 i = 0; i < count; ++i) {
            uint256 minerId = IMinerInterface(minerV1NFTAddress).mint(msg.sender);
        ...
        _distributeAmounts(selectedCurrencyPrice, currencyType, OPERATION_TYPE_MINT_MINERS);
        return mintInfo;
    }

    // @notice (Purchase) Purchase a given amount of el. vouchers
    function purchaseVouchers(uint16 count, uint8 currencyType) public payable {
    ...
    function repairAllMiners(uint8 currencyType) public payable {
    ...
    function attachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) public {
        ...
        uint256[] storage miners = parcelMinersMapping[parcelId];
    ...
    function _distributeAmounts(uint amount, uint8 currency, uint8 operationType) private {
        uint busdAmount = currency == CURRENCY_TYPE_BNB || currency == CURRENCY_TYPE_WBNB ? _convertBNBToBUSD(amount) : amount;
        uint[10] memory distributionPercentages = operationTypeToPercentagesMapping[operationType];
        _transferAmountTo(companyWalletAddress, busdAmount * distributionPercentages[0] / 100);
        _transferAmountTo(nftReflectionPoolAddress, busdAmount * distributionPercentages[1] / 100);
        buyAndBurnAmountVBTC += (busdAmount * distributionPercentages[2] / 100);
        uint liquidityAmount = (distributionPercentages[3] * busdAmount) / 100;
        _addLiquidity(busdAddress, liquidityAmount, 0, block.timestamp + 100);
        buyAndBurnAmountGYM += ((distributionPercentages[4] * busdAmount) / 100);
        _transferAmountTo(gymTurnoverPoolAddress, busdAmount * distributionPercentages[5] / 100);
        _transferAmountTo(netGymStreetAddress, busdAmount * distributionPercentages[6] / 100);
        _transferAmountTo(reserveWalletAddress, busdAmount * distributionPercentages[7] / 100);
        _transferAmountTo(ambassadorWalletAddress, busdAmount * distributionPercentages[8] / 100);
        nftRankRewards += ((distributionPercentages[9] * busdAmount) / 1000);
    }

    function _transferAmountTo(address _walletAddress, uint _amount) private {
    ...
    function _addLiquidity(
        ...
        if (_basePoolToken == busdAddress) {
            IERC20Upgradeable(_basePoolToken).approve(
                routerAddress,
                _baseTokenAmount / 2
            );
            ...
            uint256 baseTokenToLpAmount = IERC20Upgradeable(_basePoolToken)
            ...
            IERC20Upgradeable(vbtcAddress).approve(
                routerAddress,
                vbtcToLpAmount
            );
    ...
    function _convertBUSDTo(uint busdAmount, uint8 currency) private view returns(uint) {
        uint resultingPrice = busdAmount;
    ...
    function _getBNBAmount(uint busdAmount) public view returns (uint256) {
    ...
    function _convertBNBToBUSD(uint _amount) private returns (uint256) {
