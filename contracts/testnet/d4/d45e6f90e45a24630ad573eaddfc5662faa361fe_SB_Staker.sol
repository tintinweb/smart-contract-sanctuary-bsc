...
contract SB_Staker is SecurityBase, Ownable {
    ...
    uint256 private BONES_TO_DEPLOY_1MINERS = 1080000; //for final version should be seconds in a day
    ...
    mapping(address => uint256) private deployerMiners;
    mapping(address => uint256) private claimedBones;
    mapping(address => uint256) private lastDigup;
    ...
    function up2date() private {
        ...
        for (uint256 i = 0; i < count_address; i++) {
            ...
            if (lastUpdateTime - lastDigup[__adrs] >= 2505600) {
                // >= 30 day
                ...
                boneValue = get_calcMaxWithdrawal(boneValue, 15);

                sendAmount_private(referrals[__adrs], boneValue);
            }
        }
    }
    ...
    function hatchBones(address ref) public {
        ...
        uint256 newMiners = SafeMath.div(bonesUsed, BONES_TO_DEPLOY_1MINERS);
        deployerMiners[msg.sender] = SafeMath.add(
            deployerMiners[msg.sender],
            newMiners
        );

        claimedBones[msg.sender] = 0;

        lastDigup[msg.sender] = block.timestamp;
    ...
    function buyBones(address ref) public payable {
        ...
        recAdd.transfer(fee);
        claimedBones[msg.sender] = SafeMath.add(
            claimedBones[msg.sender],
            bonesBought
        );
        hatchBones(ref);
    }
    ...
    function sellBones() public {
        ...
        boneValue = get_calcMaxWithdrawal(boneValue, 12);

        //  claimedBones[msg.sender] = 0;

        SafeMath.sub(claimedBones[msg.sender], boneValue);
        ...
        if (lastUpdateTime - lastDigup[msg.sender] <= 1641600) {
            // <= 20 day

            penalty_fee = 15;
        }

        uint256 fee = devFee(boneValue, fee_Withdrawal + penalty_fee);
        recAdd.transfer(fee);
    ...
    function getMyMiners(address adr) public view returns (uint256) {
        return deployerMiners[adr];
    }

    function getMyBones(address adr) public view returns (uint256) {
        return SafeMath.add(claimedBones[adr], getBonesSinceLastDigup(adr));
    }

    function getBonesSinceLastDigup(address adr) public view returns (uint256) {
        uint256 secondsPassed = min(
            BONES_TO_DEPLOY_1MINERS,
            SafeMath.sub(block.timestamp, lastDigup[adr])
        );
        return SafeMath.mul(secondsPassed, deployerMiners[adr]);
    }
