...
contract UFXToken is BEP20 {
    ...
    address busd = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; // live busd
    IBEP20 token;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && sender != lpToken
            && sender != owner()
            // && address(this).balance > minAmountToLiquify 
        ) {
            swapAndLiquify();
        }
    ...
    function swapAndLiquify() private lockTheSwap transferTaxFree {
        uint256 contractTokenBalance = balanceOf(address(this));// 4000ufx
        ...
        if (contractTokenBalance >= minAmountToLiquify) {
            ...
            uint256 initialBalance = address(this).balance;//ex) how much bnb we have now

            // swap tokens for ETH
            swapTokensForEth(half); //ex) 25ufx -> $25 amount of BNB// 

            // how much ETH did we just swap into?
            uint256 newBalance = address(this).balance.sub(initialBalance);
            
            swapBNBForBUSD(newBalance); //ex) 50eth -> xxBNB 150ufx

            uint256 newBusdBalance = token.balanceOf(address(this));
    ...
    function swapBNBForBUSD(uint256 tokenAmount) private{
        ...
        path[1] = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;  //pancakeRouter.BUSD();

        PCSRouter.swapExactETHForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 
            path, 
            address(this), 
            block.timestamp
