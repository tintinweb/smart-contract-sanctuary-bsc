...
contract GovernorBravoDelegate is Initializable, UUPSUpgradeable, GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    string public constant name = "Garage Governor Bravo";

    /// @notice The minimum setable proposal threshold
    uint public constant MIN_PROPOSAL_THRESHOLD = 50000e18; // 50,000 Garage

    /// @notice The maximum setable proposal threshold
    uint public constant MAX_PROPOSAL_THRESHOLD = 10000000000000e18; //10000000000000 Garage
    ...
    uint public constant quorumVotes = 2000000000e18; // 2 Billion
    ...
    /**
      ...
      * @param sGarage_ The address of the sGarage token
      * @param votingPeriod_ The initial voting period
      * @param votingDelay_ The initial voting delay
      * @param proposalThreshold_ The initial proposal threshold
      */
    function initialize(address timelock_, address sGarage_, uint votingPeriod_, uint votingDelay_, uint proposalThreshold_, address treasury_) public initializer {
        require(address(timelock) == address(0), "GovernorBravo::initialize: can only initialize once");
        require(timelock_ != address(0), "GovernorBravo::initialize: invalid timelock address");
        require(sGarage_ != address(0), "GovernorBravo::initialize: invalid sGarage address");
        ...
        sGarage = sGarageInterface(sGarage_);
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(sGarage.checkHighestStaker(0, msg.sender), "GovernorBravo::propose: only top staker");
    ...
    function castVoteInternal(address voter, uint proposalId, uint8 support) internal returns (uint256) {
        require(!sGarage.checkHighestStaker(0, msg.sender), "GovernorBravo::castVoteInternal: Top staker cannot vote");
        require(state(proposalId) == ProposalState.Active, "GovernorBravo::castVoteInternal: voting is closed");
        require(support <= 2, "GovernorBravo::castVoteInternal: invalid vote type");
        Proposal storage proposal = proposals[proposalId];
        Receipt storage receipt = proposal.receipts[voter];
        require(receipt.hasVoted == false, "GovernorBravo::castVoteInternal: voter already voted");
        uint256 votes = sGarage.getPastVotes(voter, proposal.startBlock);
...
contract GovernorBravoDelegateStorageV1 is GovernorBravoDelegatorStorage {
    ...
    /// @notice The address of the Garage Protocol Timelock
    TimelockInterface public timelock;

    /// @notice The address of the Garage governance token
    sGarageInterface public sGarage;
