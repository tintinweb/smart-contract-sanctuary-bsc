 ...
 */
abstract contract MaybeSendValue is Initializable {
...
// File: mowsse/MowseMarketPlace.sol
    ...
    IERC721ReceiverUpgradeable
{
    ...
    //mapping of address to bool to check tokenListRole
    mapping(address => bool) private tokenListRole;
     ...
     * @param _iERC721CreatorRoyalty address to set as _IRoyalty.
     */
    function initialize(
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev check caller is item owner only
     * @param id to query
     */
    ...
    /////////////////////////////////////////////////////////////////////////
    // modifiers onlyProductSeller
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev check caller is item seller only
     * @param id to query
     */
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev gets the bool of _account to check hasTokenListRole
     * @param _account address to query Token List Rols
     */
    function hasTokenListRole(address _account) public view returns (bool) {
        return tokenListRole[_account];
    }
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Grants a Artist access to market
     * @param _account address
     */
    function grantTokenListRole(address _account) external {
        require(
            iMarketplaceSettings.checkModeratorRole(msg.sender) ||
                msg.sender == owner(),
            'Caller is Neither Admin Nor Moderator'
        );

        require(
            !tokenListRole[_account],
            'Address already has Token List Role'
        );
        tokenListRole[_account] = true;
    }
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev revokes an Artist's access to market
     * @param _account address
     */
    function revokeTokenListRole(address _account) external {
        require(
            iMarketplaceSettings.checkModeratorRole(msg.sender) ||
                msg.sender == owner(),
            'Caller is Neither Admin Nor Moderator'
        ...
        require(
            tokenListRole[_account] == true,
            'Error: no TokenListRole or already removed'
        );
        tokenListRole[_account] = false;
    }
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev creates a market item on market place for sale
     * @param nftContract address of origin Token smart contract
     * @param tokenId  of NFT from origin Token smart contract
     * @param price  NFT to sale
     *
     */
    function createMarketItem(
        ...
        require(
            tokenListRole[msg.sender] == true,
            'Should have tokenListRole to list NFT '
        );
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Creates the sale of a marketplace item and Transfers ownership of the item, as well as funds between parties
     * @param itemId  of NFT from origin Token smart contract
     */
    function createMarketSale(uint256 itemId) public payable nonReentrant {
        ...
        require(
            tokenPriceFeeIncluded(itemId, nftAddress) == msg.value,
        ...
        Payments.payout(
            ...
            payable(payee),
            payable(owner()),
            iERC721CreatorRoyalty.tokenCreator(nftAddress, tokenId),
            payable(owner())
        );
        ...
        idToMarketItem[itemId].firstSold = true;
     ...
     * @dev Creates the sale of a marketplace item and Transfers ownership of the item, as well as funds between parties
     * @param itemId  of NFT from origin Token smart contract
     * @param newPrice  to sell token at
     */
    function resellItem(uint256 itemId, uint256 newPrice)
        public
        nonReentrant
        onlyItemOwner(itemId)
    {
        ...
        if(idToMarketItem[itemId].firstSold){
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev transfer token to addres
     * @param itemId  of NFT from origin Token smart contract
     * @param to address to transfer token at
     */
    function transferMarketSale(uint256 itemId, address to)
        public
        nonReentrant
    {
        MarketItem storage tokenMarketData = idToMarketItem[itemId];
        uint256 tokenId = tokenMarketData.tokenId;
        address nftContract = tokenMarketData.nftContract;
        uint256 price = tokenMarketData.price;

        if (
            tokenMarketData.owner != msg.sender &&
            tokenMarketData.seller != msg.sender
        ) {
            revert('transferMarketSale: Wrong item or NFT not listed');
        } else if (
            tokenMarketData.seller == msg.sender &&
            IERC721Upgradeable(nftContract).ownerOf(tokenId) == address(this)
        ) {
            IERC721Upgradeable(nftContract).transferFrom(
                address(this),
                to,
            ...
            tokenMarketData.owner = payable(to);
            //setting seller to to, becasue seller can un-list item
            tokenMarketData.seller = payable(to);
            tokenMarketData.firstSold = true;

            emit MarketItemTransferred(
                itemId,
                nftContract,
                tokenId,
                tokenMarketData.seller,
                tokenMarketData.owner,
                price
            );
        } else if (
            tokenMarketData.owner == msg.sender &&
            IERC721Upgradeable(nftContract).isApprovedForAll(
                msg.sender,
                address(this)
            ) !=
            true
        ) {
            revert('Not approved to MP');
        } else if (
            tokenMarketData.owner == msg.sender &&
            IERC721Upgradeable(nftContract).isApprovedForAll(
                msg.sender,
            ...
            require(
                ERC721Upgradeable(nftContract).ownerOf(tokenId) !=
                    address(this),
                'Error'
            );
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev unListItem the MarketItem
     * @param itemId of NFT to unlist
     * @param to addres to transfer NFT
     * conditions, owner must be seler or mp owner
     */
    function unListItem(uint256 itemId, address to) public {
        MarketItem storage tokenMarketData = idToMarketItem[itemId];
        uint256 tokenId = tokenMarketData.tokenId;
        address nftContract = tokenMarketData.nftContract;
        require(
            tokenMarketData.seller == msg.sender || msg.sender == owner(),
            'Error: Not Owner not Admin'
        );
        if(idToMarketItem[itemId].firstSold){
            _itemsSold.increment();
        }
        ...
        emit MarketItemUnlisted(
            tokenMarketData.itemId,
            tokenMarketData.nftContract,
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev updates the price of MarketITem
     * @param id itemId of NFT to change price
     * @param newPrice  for sale NFT at
     */
        ...
        onlyProductSeller(id)
    {
        ...
        emit MarketItemUpdated(id, oldPrice, newPrice);
    }

    /////////////////////////////////////////////////////////////////////////
    // _marketPlaceStillOwnsTheToken
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Checks that the token is owned by the market place
     * @param nftContract to check token balance
     * @param itemId to check Is token sold
     */
    function _marketPlaceStillOwnsTheToken(address nftContract, uint256 itemId)
        public
        view
        returns (bool status)
    {
        ...
        IERC721Upgradeable erc721 = IERC721Upgradeable(nftContract);

        if (
            erc721.balanceOf(address(this)) != 0 &&
     ...
     * @param itemId uint256 ID of the token
     * @return uint256 sale price of the item of the token
     */

    function tokenPrice(uint256 itemId, address _nftContract)
        external
        view
        returns (uint256)
    {
        if (_marketPlaceStillOwnsTheToken(_nftContract, itemId)) {
     ...
     */
    function tokenPriceFeeIncluded(uint256 itemId, address _nftContract)
        public
        view
        returns (uint256)
    {
     ...
     */
    function fetchCollections(address _user)
        public
        view
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     *@param itemId to fetch details
     *@return _item nftContract seller price sold owner
     */
    function fetchSingleItem(uint256 itemId)
        public
        view
        returns (MarketItem memory _item)
    {
        MarketItem storage tokenMarketData = idToMarketItem[itemId];
    ...
    // /////////////////////////////////////////////////////////////////////////
    // /**
    //  *@param _tokenID to fetch details
    //  *@return item nftContract seller price sold owner
    //  */
    //    function fetchMarketItemByTokenID(uint256 _tokenID)
    //     public
    //     view
