...
contract NftAuction is IERC721Receiver, INftAuction {
  ...
  */
  function createTokenAuction(address _nft, uint256 _tokenId, uint256 _price, uint256 _duration) external override {
    require(_nft != address(0), 'Address can not be zero address');
    require(_price > 0, 'Price should be more than 0');
    require(_duration > 0, 'Invalid duration value');
    address seller = msg.sender;
    require(seller != address(0), 'Address can not be zero address');
    require(seller == ERC721(_nft).ownerOf(_tokenId), 'Seller must be an owner of nft');
    ...
    ERC721(_nft).safeTransferFrom(seller, address(this), _tokenId);
    tokenToAuction[_nft][_tokenId] = _auction;
    emit ActionCreated(_nft, _tokenId, _price, seller, block.timestamp);
  }

  /**
    Users bid for a particular nft, the max bid is compared and set if the current bid id highest
  */
  function bid(address _nft, uint256 _tokenId) external payable override {
    ...
    require(auction.duration + auction.startTime > block.timestamp, 'Deadline already passed');
    ...
    emit BidCreated(_nft, _tokenId, msg.value, msg.sender, block.timestamp);
  }
  ...
  */
  function executeSale(address _nft, uint256 _tokenId) external override {
    tokenDetails storage auction = tokenToAuction[_nft][_tokenId];
    require(auction.duration + auction.startTime <= block.timestamp, 'Deadline did not pass yet');
    ...
    emit SaleExecuted(_nft, _tokenId, auction.seller, msg.sender, auction.price, block.timestamp);
  }
  ...
  */
  function cancelAuction(address _nft, uint256 _tokenId) external override {
    tokenDetails storage auction = tokenToAuction[_nft][_tokenId];
    require(auction.seller == msg.sender, 'Not seller');
    require(auction.isActive, 'auction not active');
    auction.isActive = false;
    ...
    ERC721(_nft).safeTransferFrom(address(this), auction.seller, _tokenId);
    emit ActionCanceled(_nft, _tokenId, auction.seller, block.timestamp);
  }

  function getTokenAuctionDetails(address _nft, uint256 _tokenId) external override view returns (tokenDetails memory) {
...
interface INftAuction {
  event ActionCreated(address nftAddress, uint256 tokenId, uint256 price, address indexed seller, uint256 eventTime);
  event ActionCanceled(address nftAddress, uint256 tokenId, address indexed seller, uint256 eventTime);
  event BidCreated(address nftAddress, uint256 tokenId, uint256 price, address indexed buyer, uint256 eventTime);
  event SaleExecuted(address nftAddress, uint256 tokenId, address indexed seller, address indexed buyer, uint256 price, uint256 eventTime);
  ...
  function createTokenAuction(address _nft, uint256 _tokenId, uint256 _price, uint256 _duration) external;
  ...
  function getTokenAuctionDetails(address _nft, uint256 _tokenId) external view returns (tokenDetails memory);
}
