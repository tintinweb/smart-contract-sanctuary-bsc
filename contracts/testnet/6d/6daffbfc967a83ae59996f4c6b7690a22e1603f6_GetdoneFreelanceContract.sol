...
contract GetdoneFreelanceContract is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    event MilestoneCreated(uint256 contractId, uint256 milestoneId, uint256 orderId, address talent, address customer, uint256 price, uint256 contractPay, address erc20, uint256 status);
    ...
    event ContractDeposited(uint256 contractId, uint256 orderId, address customer, uint256 price, address erc20);
    event ContractReceived(uint256 contractId, address sender, uint256 price, address erc20);
    event ContractPay(uint256 contractId, address customer, uint256 price, address erc20);
    ...
    modifier allowDeposit(uint256 contractId, address customer) {
        ...
        allowCreate(contractId, _msgSender())
    {
        ...
        uint256 missing = 0;
        uint256 contractPay = 0;
        if (contractInfo.init == true && contractInfo.deposit > 0) {
            if(contractInfo.deposit >= price) {
                contractPay = price;
            } else {
                contractPay = contractInfo.deposit;
                missing = price - contractPay;
            }
        } else {
            missing = price;
        }
        ...
        if (missing > 0) {
            if (erc20 == address(0)) {
                require(msg.value == missing, "GetdoneFreelanceContract: deposit amount is not enough");
            } else {
                IERC20Upgradeable(erc20).safeTransferFrom(customer, address(this), missing);
            }
        }

        milestones[customer][contractInfo.contractId][milestoneId] = Milestone(contractId, milestoneId, orderId, talent, customer, price, 0, 0, 0, MILESTONE_STATUS_NEW);
        milestoneIds[customer][contractInfo.contractId].push(milestoneId);

        if (contractInfo.init == false) {
            contractInfos[customer][contractInfo.contractId] = ContractInfo(contractId, customer, talent, contractInfo.erc20, price, 0, 0, 0, false, false, true);
        } else {
            contractInfos[customer][contractInfo.contractId].price = contractInfo.price + price;
            if (contractInfo.talent == address(0)) {
                contractInfos[customer][contractInfo.contractId].talent = talent;
            }
            if (contractPay > 0) {
                contractInfos[customer][contractInfo.contractId].deposit = contractInfo.deposit - contractPay;
                emit ContractPay(contractInfo.contractId, customer, contractPay, contractInfo.erc20);
            }
        }
        
        emit MilestoneCreated(contractInfo.contractId, milestoneId, orderId, talent, customer, missing, contractPay, contractInfo.erc20, MILESTONE_STATUS_NEW);
    }
        ...
        allowDecline(contractId, milestoneId, customer)
    {
        ...
        if (balance > 0) {
            ...
            contractInfos[customer][contractId].deposit = contractInfo.deposit + balance;
            contractInfos[customer][contractId].price = contractInfo.price - balance;
            emit ContractReceived(contractId, talent, balance, contractInfo.erc20);
        }
        ...
        allowPayLess(contractId, milestoneId, customer)
    {
        ...
        require(milestone.talent == talent, "GetdoneFreelanceContract: can not change milestone if sender is not a talent");
        require(price > 0 && milestone.payless == price, "GetdoneFreelanceContract: price is invalid");
        ...
        if (refund > 0) {
            ...
            // }
            milestones[customer][contractId][milestoneId].refund = milestone.refund + refund;
            contractInfos[customer][contractId].deposit = contractInfo.deposit + refund;
            contractInfos[customer][contractId].price = contractInfo.price - refund;
            emit ContractReceived(contractId, talent, refund, contractInfo.erc20);
        }
        ...
        allowEnd(contractId, customer)
    {
        ...
        if(!contractInfo.isEndCustomer && !contractInfo.isEndTalent) {
            ...
            require(((isTalent && !contractInfo.isEndTalent ) || (isCustomer && !contractInfo.isEndCustomer )), "GetdoneFreelanceContract: Contract is already end");
            ...
            if (length > 0) {
                for (uint256 i = 0; i < length; i++) {
                    uint256 milestoneId = milestoneIds[contractInfo.customer][contractInfo.contractId][i];
                    Milestone memory milestone = milestones[contractInfo.customer][contractInfo.contractId][milestoneId];
                    uint256 balance = milestone.price - milestone.paid - milestone.refund;
                    if (balance > 0) {
                        amountRefund += balance;
                        milestones[contractInfo.customer][milestone.contractId][milestoneId].refund = milestone.refund + balance;
                    }
                }
            }
            ...
            contractInfos[customer][contractId].isEndCustomer = true;
            contractInfos[customer][contractId].isEndTalent = true;
            contractInfos[customer][contractId].deposit = 0;
    ...
    function depositContract(uint256 contractId, uint256 orderId, uint256 price, address erc20, address talent) 
        public
        payable
        whenNotPaused
        nonReentrant
        inWhitelist(erc20)
        allowDeposit(contractId, _msgSender())
    {
        address customer = _msgSender();
        ...
        if (contractInfo.init == true) {
            require(contractInfo.erc20 == erc20, "GetdoneFreelanceContract: Erc20 can not change");
            require(contractInfo.customer == customer, "GetdoneFreelanceContract: Cannot deposit contract");
        }
        ...
        if (contractInfo.init == false) {
            contractInfos[customer][contractId] = ContractInfo(contractId, customer, talent, erc20, 0, price, 0, 0, false, false, true);
        } else {
            contractInfos[customer][contractId].deposit = contractInfo.deposit + price;
        }
        emit ContractDeposited(contractId, orderId, customer, price, erc20);
    }
