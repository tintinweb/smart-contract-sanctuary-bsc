...
contract BitdriveLaunchpad is Owned {
    ...
    function getTokenPerBusd(address _userAddress) public view returns(uint256){
        ...
        return perBusdToken; // original value id should divide by 10000
    }
    ...
    function getTokenPerUser() public view returns(uint256){
        //get Bronze Users 
            ...
            uint256 remainingTokens =  hardCap - soldTokens;
            uint256 eachTier = (remainingTokens * 10000)/round2Users;
    ...
    function getUserAllocationRound2(address _userAddress) public view returns(uint256){
        ...
        return remainingallocationAmount;  // original value id should divide by 10000
    }
    ...
    function buyTokenRound1(uint256 amount) public {
        ...
        require(remainingAllocation >= userInfo[msg.sender].investAmount1 * 10000,"Amount not exceed");
        require(IToken(_crypto).transferFrom(msg.sender,address(this), amount),"Insufficient balance from User");
        userInfo[msg.sender].remainingallocationAmount1 = remainingAllocation - (userInfo[msg.sender].investAmount1  * 10000);
        ...
        userInfo[msg.sender].round1Tokens += swappedToken;
            ...
            if(halfAmount >= (userInfo[msg.sender].investAmount1  * 10000)){
    ...
    function buyTokenRound2(uint256 amount) public {
        ...
        require(remainingAllocation >= userInfo[msg.sender].investAmount2 * 10000,"Amount not exceed");
        require(IToken(_crypto).transferFrom(msg.sender,address(this), amount),"Insufficient balance from User");
        userInfo[msg.sender].remainingallocationAmount2 = remainingAllocation - (userInfo[msg.sender].investAmount2 * 10000);
        uint256 swappedToken = (tokensPerBusd * amount) / 1000;
        userInfo[msg.sender].SwapTokens += swappedToken;
...
contract Proxy {
    ...
    function deposit(uint256 _amount) public {
        ...
        require( (IToken(tokenAddress).balanceOf(msg.sender)) > STAKING_PRICE[1] ,"You dont have minimum balance");
    ...
    function tierAllocationPerUser(uint256 maxSupply,uint _tier) public view returns (uint256) {
        ...
        uint256 eachTier = (maxSupply/getallocationToken) * 10000;
