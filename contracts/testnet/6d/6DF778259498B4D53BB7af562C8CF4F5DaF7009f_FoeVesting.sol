...
contract FoeVesting is Ownable, ReentrancyGuard {
    ...
    function blockval() external view returns (uint256){
        return block.timestamp;
    }

    function nextclaim_() external view returns (uint256){
        return vests[msg.sender].nextClaim;
    }

     function balance() external view  returns (uint256){
        return vests[msg.sender].availableAmount;
    }


    function vest(address account,uint256 _amount, uint256 _initailReleasePer, uint256 lockDurationInMonth, uint256 vestDurationInMonth) external onlyOwner nonReentrant returns(bool){
        require(!vests[account].status, "Already vested");
        require(_amount > 0, "Amount should be greater than zero");
        uint256 initaitReleaseAmount;
        if(_initailReleasePer != 0)
             initaitReleaseAmount = (_amount * _initailReleasePer) / 100;
        vests[account] = VestDetail(
            account,
            _amount,
            _amount - initaitReleaseAmount,
            block.timestamp,
        ...
        if(_initailReleasePer != 0)
            require(token.transfer(account,initaitReleaseAmount), " token transfer failed");
