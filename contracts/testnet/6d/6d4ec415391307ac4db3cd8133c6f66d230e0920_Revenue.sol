...
contract Revenue is Ownable,IRevenue{
    ...
    event RechageTokenEvent(address from,address token,uint256 amount,uint256 _type);
    event DistributeTokenEvent(address token,address from,uint256 amount);

    function rechargeToken(address from,address token,uint256 amount,uint256 _type) public virtual override onlyController {
    ...
    function distributeToken(address token) public onlyController {
        ...
        for(uint256 i=0;i<length;i++){
            ...
            emit DistributeTokenEvent(token, account, amount);
        }
        balanceOf[token] = 0;

    }

    function withdraw(address token,address account,uint256 amount) public onlyController {
        if(token==address(0)){
            payable(account).transfer(amount);
