...
contract Governance is Admin{
    ...
    Stake public stake;
    constructor(address _governanceToken, address _stake) Admin(100000000000000000000,1000000000000000000000,1209600,604800,100,334,500) public{
    ...
    function getSubmit() public view returns (Submit[]  memory) {
        return submit;
    }
    struct RejectSubmit {
        string proposalID;
    }
    RejectSubmit[] public rejectSubmit;
    function getRejectSubmit() public view returns (RejectSubmit[]  memory) {
        return rejectSubmit;
    }
    struct Proposal {
        // string proposalID;
    ...
    mapping(address => mapping(string => uint256)) public mapDeposit;
    ...
    function activeDeposit(string memory _proposalID, uint256 _status) external onlyAdmin(){
        ...
        if(_status == 2){
            // refund
            governanceToken.safeTransfer(msg.sender, proposalActive.initial);
    ...
    function deposit(string memory _proposalID, uint256 _amount) external {
        ...
        require(proposalDeposit.status == 3, "Can't deposit");
        ...
        mapDeposit[msg.sender][_proposalID] += _amount;
        emit newDeposit(
            msg.sender,
            _proposalID,
            _amount,
