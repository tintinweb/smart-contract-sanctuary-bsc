...
contract A4StakeAPY_V4 is Initializable, ReentrancyGuardUpgradeable {
    ...
    uint256 private constant DAY = 10 minutes;

    event StakeCreate(address indexed stakeholder, uint256 stake, uint256 balance);
    event StakeHold(address indexed stakeholder, uint256 hold, uint256 balance);
    event StakeWithdraw(address indexed stakeholder, uint256 reward, uint256 commission);
    ...
    uint256 totalStaked;
    mapping(address => bool) _holders;

    address private constant ADMIN = 0x36174b2978bc66c3e9cE1Dc4a3b215eeAC1f658C;
   ...
   */
    ...
    */
    ...
    function _createStake(address stakeholder, uint256 stake) internal limitedStake(stake){
        uint256 newBalance = rewardOf(stakeholder).add(stake);

         totalStaked = totalStaked.sub(_stakes[stakeholder].amount).add(newBalance);
        ...
        emit StakeCreate(stakeholder, stake, newBalance);
    ...
    function _holdStake(address stakeholder, uint256 amount) internal onlyStakeExists(stakeholder, amount) {
        ...
        uint256 newBalance = rewardOf(stakeholder).sub(amount);
         totalStaked = totalStaked.sub(_stakes[stakeholder].amount).add(newBalance);
        ...
        emit StakeHold(stakeholder, amount, newBalance);
    }
    ...
    function _removeStake(address stakeholder) internal onlyStakeUnheld(stakeholder) {
        ...
        emit StakeWithdraw(stakeholder, balance, balance.div(100));
    }
    ...
    function isStakeholder(address stakeholder) public view returns (bool, uint256) {
        if (_holders[stakeholder] == true) return (true, 0); 
        return (false, 0);
    }
    ...
    function _addStakeholder(address stakeholder) internal {
        ...
        if (!_isStakeholder) _holders[stakeholder] = true;
    }
    ...
    function migrate(uint256 start, uint256 stop) external {
        require(msg.sender == ADMIN, "Not ADMIN");
        for (uint256 s = 0; s < _stakeholders.length; s++) {
             _holders[_stakeholders[s]] = true;
             totalStaked = totalStaked.add(_stakes[_stakeholders[s]].amount);
        }
    }

    function migrate2(uint256 start, uint256 stop) external returns (uint256) {
        require(msg.sender == ADMIN, "Not ADMIN");
        for (uint256 s = start; s < stop; s++) {
             _holders[_stakeholders[s]] = true;
             totalStaked = totalStaked.add(_stakes[_stakeholders[s]].amount);
        }
