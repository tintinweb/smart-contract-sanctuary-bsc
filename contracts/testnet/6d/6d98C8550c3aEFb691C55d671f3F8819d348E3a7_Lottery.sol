...
contract Lottery is Ownable{
    ...
    uint public target_amount_default = 50; //300.000 tickets default to super draw
    ...
    uint public draw_periods = 5 hours;
    ...
    uint public express_target_amount_default = 20;
    ...
    uint public express_draw_periods = 1 hours;
    address[] public  players_express;
    address[] public  express_winners;
    mapping(address => Winners[]) public express_mapwinners;
    ...
    uint public levels_affiliate = 1;
    uint public levels_affiliate_percent = 100; //50% per level
    //********END AFFILIATE SETTINGS*********
    ...
    function MegaLottoTicket(address payable _addrinviter, address payable _addrplayer) public payable onGame{
        ...
        total_team += msg.value.mul(percent_company).div(100);
        ...
        if (_addrinviter != manager){
            IAffiliate(address_affiliate).pay{value: value_affiliate_pay}(payable(_addrplayer), levels_affiliate, levels_affiliate_percent, _addrinviter);
        }else{
            total_team += value_affiliate_pay;
        }
        //**** END AFFILIATE PROGRAM

//        team_address.transfer(total_team); //SENT COMPANY FUNDS
    ...
    function pickWinner() private {
        ...
        isGameEnded = true;
        mapwinners[winner].push(Winners(payable(winner), amount_draw, block.timestamp, draw_edition, index));
        winners.push(winner);
    ...
    function pickWinnerExpress() private {
        ...
        express_mapwinners[winner].push(Winners(payable(winner), express_amount_draw, block.timestamp, express_draw_edition, index));
    ...
    function updateCompanyPercent(uint _amount) public onlyOwner {
        percent_company = _amount;
    }

    function updateAffiliatePercent(uint _amount) public onlyOwner {
        percent_affiliate = _amount;
    }

    function updateAffiliateLevelPercent(uint _amount) public onlyOwner {
        levels_affiliate_percent = _amount;
    }

    function updateAffiliateLevels(uint _amount) public onlyOwner {
        levels_affiliate = _amount;
    }

    function updateAffiliateAddress(address payable _address) public onlyOwner {
        address_affiliate = payable(_address);
    }

    function updateDoublePrizeAddress(address payable _address) public onlyOwner {
        address_doubleprize = payable(_address);
    }
    ...
    function getPlayers(uint _from, uint max)public view returns(PlayerStructure[] memory){
        PlayerStructure[] memory w = new PlayerStructure[](max);

        if (
            players.length == 0 ||
            _from > players.length - 1
        ) {
            return new PlayerStructure[](0);
        }

        for (uint i = 0; i < max; i++) {
            w[i] = PlayerStructure(payable(players[_from]), block.timestamp, ticket_price, _from, draw_edition);
            _from++;
        }
    ...
    function getPlayersExpress(uint _from, uint max)public view returns(PlayerStructure[] memory){
        PlayerStructure[] memory w = new PlayerStructure[](max);
        ...
        for (uint i = 0; i < max; i++) {
            w[i] = PlayerStructure(payable(players_express[_from]), block.timestamp, ticket_price, _from, draw_edition);
            _from++;
        }
    ...
    function getWinnersExpressList() public view returns (Winners[] memory){
        Winners[] memory w = new Winners[](getWinnersNumberExpress());
        for (uint i = 0; i < getWinnersNumberExpress(); i++) {
            uint totalprizes =  express_mapwinners[express_winners[i]].length;
            for (uint j = 0; j < totalprizes; j++) {
                Winners storage wr = express_mapwinners[express_winners[i]][j];
                w[i] = wr;
            }
        }
        return w;
    ...
    function getListPurchases(uint _from, uint _draw) public view returns (address[] memory){
        address[] memory p;
        uint max = 30;
        if (players_express.length <= _from){
            max = players_express.length;
        }
        for (uint i = _from; i < max; i++) {
            p[i] = _draw == 1 ? players[i] : players_express[i];
        }
        return p;
    }
    ...
    function withdraw() public onlyOwner {
        require(total_team > 0, 'There are no team funds for withdrawal');
        team_address.transfer(total_team);
        total_team = 0;
    }
}
