...
// File: contracts/token/nemNFT.sol
...
contract nemNFT is ERC721, Ownable, InemNFT, PermissionRight {
    ...
    function mint(address to) public override onlyOperator returns(bool){
        _tokenIds.increment();
        uint256 itemId = _tokenIds.current();

        _safeMint(to, itemId);
        tokenURI(itemId);
        emit Minting(to, itemId);
        return true;
    }

    function burn(uint256 tokenId) public override onlyOperator returns(bool){
        _burn(tokenId);
        return true;
    }
...
// File: contracts/token/NEMCharacter.sol


pragma solidity ^0.8.0;


contract NEMCharacter is nemNFT {

    constructor(string memory _URI) nemNFT(_URI , "NEM Character", "NEMC") {}
