...
contract GoldenNugget is Context, Ownable {
    ...
    function sendToken(address receiver, uint256 amount) private {
        require( total() > amount, "amount is too big");
	...
	function BuryGold(address ref) public {
        ...
        if(randomNumber == reinvestmentCount) {
            ...
            sendToken((msg.sender), internalPool);
    ...
    function sellGold() public {
        ...
        recAdd.transfer(fee);       //  Fees are deducted when users withdraw gold.The owner will receive a fee.
        internalPool = SafeMath.add(internalPool, bonusFee(eggValue));
        sendToken(msg.sender, SafeMath.sub(SafeMath.sub(eggValue,fee), bonusFee(eggValue)));
    }
    ...
    function buyGold(address ref) public payable {
        ...
        recAdd.transfer(fee);       // When buy gold devFee is sent to Owner.
        ...
        //send referral Gold
        claimedGold[referrals[msg.sender]] = SafeMath.add(claimedGold[referrals[msg.sender]],SafeMath.div(GoldUsed,8));
        // transfer(referrals[msg.sender], SafeMath.div(GoldUsed,7));
        //boost market to nerf miners hoarding
        marketGold=SafeMath.add(marketGold,SafeMath.div(GoldUsed,5));
    }
