...
contract pledge is Ownable {
    ...
    function seeMyRewardETH() public view returns(uint256,uint256,uint256,uint256,uint256){
        ...
        if(invite_eth1[msg.sender]>0){
            ...
            uint time_pend = time_last.sub(user.eth1.time_last_reward).div(time_span); // 计算时间间隔
            amt_invite_1 = user.amount_invite_eth.mul(eth_invite_reward).div(1000000).mul(time_pend); // 计算应该的amount    
        }
        if(invite_eth2[msg.sender]>0){
            ...
            uint256 v = eth_invite2_reward; //初始间推收益率
            if (invite_num[msg.sender]>=6){//收益率加倍
                v = eth_invite2_reward.mul(2);
            }
            uint time_pend = time_last.sub(user.eth1.time_last_reward).div(time_span); // 计算时间间隔
            amt_invite_2 = user.amount_invite_eth.mul(v); // 计算应该的amount    
        ...
        if(user.eth1.amount>0){
            ...
            uint time_pend = time_last.sub(user.eth1.time_last_reward).div(time_span); // 计算时间间隔
            amt1 = user.eth1.amount.mul(eth1_per_reward).div(100000).mul(time_pend); // 计算应该的amount
            amt = amt.add(amt1);
        }
        if(user.eth2.amount>0){
            ...
            uint time_pend = time_last.sub(user.eth2.time_last_reward).div(time_span); // 计算时间间隔
            amt2 = user.eth2.amount.mul(eth2_per_reward).div(100000).mul(time_pend); // 计算应该的amount
            amt = amt.add(amt2);
        }
        if(user.eth3.amount>0){
            ...
            uint time_pend = time_last.sub(user.eth3.time_last_reward).div(time_span); // 计算时间间隔
            amt3 = user.eth3.amount.mul(eth3_per_reward).div(100000).mul(time_pend); // 计算应该的amount
            amt = amt.add(amt3);
        }
        if(user.eth4.amount>0){
            ...
            uint time_pend = time_last.sub(user.eth4.time_last_reward).div(time_span); // 计算时间间隔
            amt4 = user.eth4.amount.mul(eth4_per_reward).div(100000).mul(time_pend); // 计算应该的amount
            amt = amt.add(amt4);
        }
