...
contract AnitoAirdrop is Pausable, Ownable, ReentrancyGuard {
    ...
    function airDrop(address[] memory _players, uint256[] memory _anito_category) external onlyOperators {
        require(_players.length == _anito_category.length,"!length");
        for(uint256 i=0;i<_players.length;i++){
            anito.safeMintSingle(_players[i],_anito_category[i]);
        }
    }
}
