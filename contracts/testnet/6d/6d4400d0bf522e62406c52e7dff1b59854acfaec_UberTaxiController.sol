...
contract UberTaxiController is Ownable {
    ...
    constructor() {
        ...
        UTD = UberTaxiDate(0x5Ee3B29b83BCc1EB4C4A59a13C019C096967CF1a);
    }
    ...
    function openBlindBox() public {
        ...
        if(types > 7){
            ...
            amountOrTokenId = UTN.createNFT(msg.sender, index);
            ...
            amountOrTokenId = UTD.blindBoxAward(types - 1).mul(10 ** 18);

            TAXI20.approveToController(UTD.depositAddress(), amountOrTokenId);

            TAXI20.transferFrom(UTD.depositAddress(), msg.sender, amountOrTokenId);
        }

        emit OpenBlindBox(msg.sender, UTD.blindBoxPrice(), types, amountOrTokenId);
    }
    ...
    function buyVehicle(uint256 index, uint8 tokenType) public {
        ...
        if (tokenType == 1) {
            ...
            UTD.setUtaxiDetails(msg.sender, 6, toString(ids), uPrice.div(10 ** 18), block.timestamp);
        }
        ...
        if (welMember != address(0)) {
            ...
            UTD.setUtaxiDetails(welMember, 4, toString(msg.sender), price.mul(UTD.commissionFee()).div(10 ** 20), block.timestamp);
        }
    ...
    function vehicleRent(uint256 index) public isUsers(index) {
        ...
        if (isEffectiveUser == false){
            ...
            UTD.addEffectiveUserNum();
        }

        _vehicleRent(index);

        // if(durability == 0){
        //     allVehicles[index].expireTime = block.timestamp;
        // }
    ...
    function vehicleGetReward(uint256 index) public isUsers(index) {
        ...
        UTD.setUtaxiDetails(msg.sender, 5, toString(index), profit, block.timestamp);
        ...
        if (welMember != address(0) && UTD.queryUserRentVehicleAmount(welMember) > 0) {
            ...
            UTD.setUtaxiDetails(welMember, 4, toString(msg.sender), profit.mul(UTD.recommenderFee()).div(10 ** 20), block.timestamp);
    ...
    function exchange(uint256 exchangeNum) public {
        ...
        UTD.setUtaxiDetails(msg.sender, 3, toString(msg.sender), exchangeNum, block.timestamp);
    ...
    function playerTransfer(address adds, uint256 transferNum) public {
        ...
        UTD.setUtaxiDetails(msg.sender, 2, toString(adds), transferNum, block.timestamp);

        UTD.setUtaxiDetails(msg.sender, 1, toString(msg.sender), transferNum, block.timestamp);
    ...
    function buyMemberRender(uint256 index) public {
        ...
        if (welMember != address(0)) {
            ...
            UTD.setUtaxiDetails(welMember, 4, toString(msg.sender), sellPrice.mul(UTD.commissionFee()).div(10 ** 20), block.timestamp);

            // users[welMember].totalDeposit += allVehicles[index].sellPrice;

            // uint256 reward = allVehicles[index].sellPrice.mul(UTN.commissionFee()).div(100);

            // users[welMember].UTAXIReward += reward;

            // users[welMember].UTAXINum += reward;

            // userUtaxiDetails[welMember].push(UtaxiDetail(4,toString(msg.sender),reward,block.timestamp));
        }
