...
contract TokenTrade is Modifier, Util {
    ...
    function getExchangeAmount(uint256 _orderId, uint256 exchangeAmount) public view returns (uint256 amountToWei, uint256 poundage) {
        uint256 price = toWei(orderMapping[_orderId].receiveAmount, 18).div(orderMapping[_orderId].outputAmount);
        amountToWei = backWei(exchangeAmount.mul(price), 18);
        poundage = amountToWei.mul(orderMapping[_orderId].exchangePoundage).div(1000);
    }
