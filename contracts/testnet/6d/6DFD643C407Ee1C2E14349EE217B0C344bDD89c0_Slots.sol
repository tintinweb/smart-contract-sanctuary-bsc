    ...
    ReentrancyGuardUpgradeable
{
    ...
    // function changePassportsAddress(address _passportsAddress) external onlyOwner {
    //     require(
    ...
    function register(string memory name, address _referrer) external {
        ...
        // IPassport(passportsAddress).mint(msg.sender);
        ...
        returns (address[10] memory sortedPlayers)
    {
        ...
        for (uint i = sortedPlayers.length - 1; i > 0; i--) {
            uint j = randomizer.random(game.id, i + 1, sortedPlayers[i]);
    ...
    function test_withdraw(uint256 _amount) public onlyOwner {
        payable(msg.sender).transfer(_amount);
    }

    function testTableDrawEvents(uint256 _bet, address[10] memory _players)
        public
        onlyOwner
    {
        gamesCounter++;
    ...
    function testFakePayRef(
        address addr1,
        address addr2,
        uint256 amo
    ) public {
        emit RefPaid(addr1, addr2, amo, lastRefBlockForUser[addr2]);
        lastRefBlockForUser[addr2] = block.number;
    }
}
