...
contract WalletRegistry is InitOwner{
    ...
    mapping(address => mapping(uint => mapping(address => uint))) public Nonce;
    ...
    function getTrade(address _nft,uint _tokenId,address _seller,uint _index) external view returns(address _c,uint _n,uint _a,bool _s)
    {
        ...
        _n = Nonce[_nft][_tokenId][_seller];
        _s = TradeRecord[_nft][_tokenId].status;
    ...
    // 设置nft交易次数
    function setTradeNonce(address _nft,uint _tokenId,address _seller,uint _nonce) external onlyMarket
    {
        Nonce[_nft][_tokenId][_seller] = _nonce;
    }
    // 设置nft交易次数
    function setTradeStatus(address _nft,uint _tokenId,bool _status) external onlyMarket
    {
        TradeRecord[_nft][_tokenId].status = _status;
    }
    // 设置nft交易量
