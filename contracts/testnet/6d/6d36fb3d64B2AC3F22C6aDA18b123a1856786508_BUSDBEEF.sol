...
contract BUSDBEEF is Context, Ownable {
    ...
    constructor(IBEP20 _gusd, address _ceoAddress) {	
        gusd = _gusd;	
    ...
    function buyEggs(uint256 amount, address ref) public {
        require(initialized);
        gusd.transferFrom(address(msg.sender), address(this), amount);
        uint256 eggsBought=calculateEggBuy(amount,SafeMath.sub(gusd.balanceOf(address(this)),amount));
        eggsBought=SafeMath.sub(eggsBought,devFee(eggsBought));
        
        uint256 fee=devFee(amount);
        gusd.transfer(ceoAddress,fee);
        claimedEggs[msg.sender]=SafeMath.add(claimedEggs[msg.sender],eggsBought);
        hatchEggs(ref);
    }

    function safeTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = gusd.balanceOf(address(this));

        if(tokenBalance > 0) {
            if(_amount > tokenBalance) {
                gusd.transfer(_to, tokenBalance);
            } else {
                gusd.transfer(_to, _amount);
            }
        }
    }
    ...
    function getBalance() public view returns(uint256) {
        return gusd.balanceOf(address(this));
    }
