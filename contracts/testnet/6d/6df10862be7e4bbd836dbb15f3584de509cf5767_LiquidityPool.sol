...
contract PancakeSwap {
  function getPrice(string memory _token0, string memory _token1) public view returns (uint256) {
    // Code to call the PancakeSwap API and return the current price of the token pair
  }
}
...
contract LiquidityPool {
  IERC20 public BNB;
  IERC20 public USDC;
  PancakeSwap public pancakeSwap;

  address public owner;
  mapping (address => uint256) public allowed;

  constructor(address _BNB, address _USDC, address _pancakeSwap) {
    BNB = IERC20(_BNB);
    USDC = IERC20(_USDC);
    pancakeSwap = PancakeSwap(_pancakeSwap);
    owner = msg.sender;
  }
  ...
  function checkPrice() public view returns (uint256) {
    return pancakeSwap.getPrice("BNB", "USDC");
  }
