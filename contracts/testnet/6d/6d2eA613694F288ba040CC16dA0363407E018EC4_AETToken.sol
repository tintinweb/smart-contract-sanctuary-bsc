...
contract AETToken is IERC20, Ownable,Util {
    ...
    uint256 private destroyAmount; //已销毁数量
    uint256 private maxDestroyAmount; //最大销毁量
    ...
    constructor() {
        ...
        _owner = msg.sender;
        shortestTradingTime = 50;
        //USDT合约
        usdtToken = ERC20(0x55d398326f99059fF775485246999027B3197955);
        //销毁数量
        destroyAmount = 0;
        //最大销毁量 10W
        maxDestroyAmount = 100000000000000000000000;
    }
  ...
  function _transfer(
        ...
        if(sender == swapRouter && !_isExcludedFromFee[recipient]) {
            ...
            for(uint8 i=0; i<slippageAmounts.length; i++) {
                if(slippageAddresses[i] != address(0)) {
                    //A币销毁达到10W不在销毁
                    if(i == 1 && destroyAmount < maxDestroyAmount){
                        destroyAmount = destroyAmount.add(slippageAmounts[i]);
    ...
    /*设置销毁A币数量*/
    function setDestroyAmount(uint256 _amount) public onlyOwner{
        destroyAmount = _amount;
    }

    /*获取销毁A币数量*/
    function getDestroyAmount() public view returns(uint256){
        return destroyAmount;
    }

    /*设置最大销毁量*/
    function setMaxDestroyAmount(uint256 _amount) public onlyOwner{
        maxDestroyAmount = _amount;
    }

    /*获取最大销毁量*/
    function getMaxDestroyAmount() public view returns(uint256){
