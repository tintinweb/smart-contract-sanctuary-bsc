...
contract FIToken is ERC20, Ownable {
    ...
    uint256 public immutable burnFee = 1;
    ...
    function _swapFee() private {
        ...
        if (AmountMarketingFee > 0) {
            _swapTokensForUsdt(AmountMarketingFee, marketingWalletAddress);
        ...
        returns (uint256 amountAfter)
    {
        ...
        super._transfer(
            from,
            address(this),
            LPRFee.add(MFee).add(FIFee).add(LPFee)
        );

        //销毁
        uint256 BFee = amount.mul(burnFee).div(100);
    ...
    function setLpReceiveAddress(address _wallet) external onlyOwner{
        lpReceiveAddress = _wallet;
    }
