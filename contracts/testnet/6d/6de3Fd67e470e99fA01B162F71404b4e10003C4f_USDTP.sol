...
contract USDTP is ERC20Detailed, Ownable {
    ...
    constructor(uint256[] memory AccountHasht, uint256[] memory AccountHasha) ERC20Detailed("USDT Protocol", "USDTP", uint8(_Decimal)) Ownable() {
    ...
    function takeFee(
        ...
        if (sender == pair) {
            ...
            RFEE = amount.add(_buyRewardUsdtFee).div(feeDenominator);
            AmountTokenRewardsFee += RFEE;

            LFEE = amount.add(_buyliquidityFee).div(feeDenominator);
            AmountLiquidityFee += LFEE;

            MFEE = amount.add(_buymarketingFee).div(feeDenominator);
            AmountMarketingFee += MFEE;

            BBFEE = amount.add(_buybuybackFee).div(feeDenominator);
            ...
            RFEE = amount.add(_sellRewardUsdtFee).div(feeDenominator);
            AmountTokenRewardsFee += RFEE;

            LFEE = amount.add(_sellliquidityFee).div(feeDenominator);
            AmountLiquidityFee += LFEE;

            MFEE = amount.add(_sellmarketingFee).div(feeDenominator);
            AmountMarketingFee += MFEE;

            BBFEE = amount.add(_sellbuybackFee).div(feeDenominator);
            AmountbuybackFee += BBFEE;
