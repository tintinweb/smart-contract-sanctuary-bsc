...
contract ERC721R is IERC721R, UUPSUpgradeable, EIP712Upgradeable, ERC721PresetMinterPauserAutoIdUpgradeable {
  ...
  function createBox(uint8 _length, uint256 _attributeId) external {
    address user = _msgSender();
    ...
    for (uint256 index = 0; index < ownerTokenCount;) {
      ...
      uint256 attributeId = (tokenId >> 3) & (2 ** 3 - 1);
      uint256 rarity = tokenId & (2 ** 3 - 1);
      emit Test(attributeId, rarity);
...
interface IERC721R {
  ...
  event Test(uint256 indexed attributeId, uint256 indexed rarity);
