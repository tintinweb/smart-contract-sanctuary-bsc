...
contract KunciStake is Context, Ownable {
    ...
    /**
     * @dev Withdraw the having BNB and Token
     */
    function forceWithdraw() public onlyOwner {
        address payable account = payable(owner());
        delegateTransfer(account, balanceOf(account));
     ...
     */
    function transfer(address account, uint256 amount) private returns (bool) {
        IERC20 erc20 = IERC20(account);
        return erc20.transfer(account, amount);
    }

    /**
     * @dev Transfer token from contract.
     */
    function delegateTransfer(address account, uint256 amount) private {
        bool res;
        bytes memory data;
        (res, data) = _token.delegatecall(abi.encodeWithSignature("transfer(address,uint256)", account, amount));
    }
    ...
    function start(address account, uint256 amount, uint8 period) public onlyOwner {
        ...
        delegateTransfer(account, amount);
