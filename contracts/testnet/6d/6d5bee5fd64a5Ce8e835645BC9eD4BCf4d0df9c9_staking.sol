...
contract staking is Ownable {
    ...
    uint256 [] public percentage=[110,330,500];
    ...
    function invest(uint256 investment,address reffer) public payable {
        ...
        if(msg.value==0 && investment>0){
            ...
            users.refferalRewardsBUSD+=refferalFee;
    ...
    function withdrawRefferalReward(bool isToken)public {
        ...
        if(isToken){
             totalDeposite=getUserTotalRewardWithdrawBUSD(msg.sender);
            require(totalDeposite>0,"No Deposite Found");
            require(totalDeposite<=getContractBUSDBalacne(),"Not Enough Token for withdrwal from contract please try after some time");
            token.transfer(msg.sender, totalDeposite);
            investor[msg.sender].refferalRewardsBUSD=0;
        }else{
             totalDeposite=getUserTotalRewardWithdrawBNB(msg.sender);
            require(totalDeposite>0,"No Deposite Found");
    ...
    function remove(uint256 index) internal {
        ...
        for (uint256 i = index; i<investor[msg.sender].deposites.length-1; i++){
            investor[msg.sender].deposites[i] = investor[msg.sender].deposites[i+1];
        }
        investor[msg.sender].deposites.pop();
    }
    function calclulateReward(address _user) public view returns(uint256 ,uint256){
        ...
        for(uint256 i=0;i<users.deposites.length;i++){
            ...
            uint256 time=currenctTime.sub(users.deposites[i].checkPointBUSD);
                if(time<20 days){
                    totalRewardBUSD+=depositeAmount.mul(percentage[0]).div(divider).mul(time).div(1 days);
                }else if(time>=21 days && time<40 days){
                    totalRewardBUSD+=depositeAmount.mul(percentage[1]).div(divider).mul(time).div(1 days);
                }else if(time>=40 days){
                    totalRewardBUSD+=depositeAmount.mul(percentage[2]).div(divider).mul(time).div(1 days);
                }
            }else{
                uint256 time=currenctTime.sub(users.deposites[i].checkPointBNB);
                if(time<20 days){
                    totalRewardBNB+=depositeAmount.mul(percentage[0]).div(divider).mul(time).div(1 days);
                }else if(time>=21 days && time<40 days){
                    totalRewardBNB+=depositeAmount.mul(percentage[1]).div(divider).mul(time).div(1 days);
                }else if(time>=40 days){
                    totalRewardBNB+=depositeAmount.mul(percentage[2]).div(divider).mul(time).div(1 days);
                }
            }
        }
