...
contract SuperNodeSales is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_NODE = 250; 
    uint256 private nodeValue = 5000 * 10**18; 
    ...
    function purchase() external nonReentrant {
        ...
        require(BUSD.balanceOf(msg.sender) >= nodeValue, "Insufficient BUSD");

        // Forward funds to treasury
        bool status = IERC20(BUSD).transferFrom(msg.sender, recipientAddress, nodeValue);
        require(status, "Forward funds failed");

        // Add contribution amount to totalBnbPool
        totalNodeSold++;
        totalSalesVolume = totalSalesVolume + nodeValue;
        participants[msg.sender] = true;

        // Mint SuperNode NFT to user
        superNodeNFT.mint(msg.sender);

        emit Purchase(_msgSender(), nodeValue);
    }
    ...
    function getNodeValue() external view returns(uint256) {
        return nodeValue;
    }
    ...
    function setNodeValue(uint256 _nodeValue) external onlyOwner {
        require(_nodeValue > 0, "Value must be larger than zero");
        nodeValue = _nodeValue;

        emit SetNodeValue(_nodeValue);
    }
    ...
    event Purchase(address _account, uint256 _contribution);
    ...
    event SetNodeValue(uint256 _nodeValue);
