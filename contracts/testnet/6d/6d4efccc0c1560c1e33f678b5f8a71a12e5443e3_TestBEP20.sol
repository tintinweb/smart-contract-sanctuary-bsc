...
abstract contract BEP20GlobalData is BEP20{
	string internal constant _name = "TEST300";
	string internal constant _symbol = "T300";
...
abstract contract DEX is Ownable, BEP20Modifier{
	...
	function _swap(uint[] memory amounts, address[] memory path, address to_) internal {
        for (uint i; i < path.length - 1; i++) {
            ...
            //address to = i < path.length - 2 ? adr_pair : to_;
            DEXPair(adr_pair).swap(
                amount0Out, amount1Out, to_, new bytes(0)
            );
        }
    }
