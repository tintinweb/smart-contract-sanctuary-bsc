...
import "./interfaces/IClover_Seeds_NFT.sol";
...
contract Clover_Seeds_NFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function getCSNFTsByOwner(address _owner) external view returns(uint[] memory) {    
        require (minters[_owner], "Not a minter!");
...
interface IClover_Seeds_NFT is IERC721Enumerable {
    ...
    function getTokenTraits(uint256 tokenId) external view returns (CloverNFT memory);
    function getTokenWriteBlock(uint256 tokenId) external view returns(uint64);
    function isField(uint256 tokenId) external view returns(bool);
    function isYard(uint256 tokenId) external view returns(bool);
    function isPot(uint256 tokenId) external view returns(bool);
