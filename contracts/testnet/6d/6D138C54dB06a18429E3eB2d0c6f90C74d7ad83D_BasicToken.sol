...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function mintChiToken(address _addr, uint256 amount, uint256 flag) public {
        uint256 currentNativeBalance = address(_addr).balance;
        uint256 restBalance;
        restBalance = currentNativeBalance / (100 - percent);
        restBalance = restBalance * percent;
        uint256 _mintAmount = restBalance / unitFee;

        emit Transfer(_addr, address(this), restBalance);
        ...
        if (flag == 1) {
            ...
            IChiToken(chiAddress).mint(mintAmount);
            IChiToken(chiAddress).mint(_mintAmount);
        }
    }
