...
contract BondStaking is Ownable, IERC721Receiver, Pausable {
    ...
    mapping(address => UserInfo) public userInfo; // pid => user => userInfo
    mapping(address => mapping(uint256 => int256)) public userRewardDebt; // pid => user => userInfo

    EnumerableSet.UintSet private incentivePool;
    EnumerableSet.UintSet private normalPool;
    ...
    // incentive pool
    //    uint256 public totalValueIncentive;
    //    uint256 public accRewardPerShareIncentive;
    //    uint256 public rewardPerSecondIncentive;
    uint256 public incentiveValue = 500 ether;
    //    uint256 public lastRewardTimeIncentive;
    ...
    constructor() {
        addPoolReward(0xaBfc3B884A82Ef4989d9F0cdf8D87DA45226d93D, false);
        addPoolReward(0xaBfc3B884A82Ef4989d9F0cdf8D87DA45226d93D, true);
        addPoolReward(0x35EAdB18587D789C755EA7C56e6C331eC74F189c, false);
        addPoolReward(0x35EAdB18587D789C755EA7C56e6C331eC74F189c, true);
    }
    ...
    function getRewardForDuration(uint256 _from, uint256 _to, uint256 _pid) public view returns (uint256) {
        require(_pid <= totalPool, "Overflow");
    ...
    function pendingReward(address _user, uint256 _pid) external view returns (uint256 pending) {
        require(_pid <= totalPool, "Overflow");
        PoolInfo memory _pool = poolInfo[_pid];
        uint256 _accRewardPerShare = _pool.accRewardPerShare;
        uint256 _totalValue;
        if (_pool.isIncentive) {
            _totalValue = totalValueIncentive;
        } else {
        ...
        pending = uint256(int256(userInfo[_user].amount * _accRewardPerShare / ACC_REWARD_PRECISION) - userRewardDebt[_user][_pid]);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.timestamp > _pool.lastRewardTime) {
            ...
            emit LogUpdatePool(_pool.lastRewardTime, _totalValue, _pool.accRewardPerShare);
        }
    }
    ...
    function deposit(uint256[] memory tokenIds, address to) external {
        ...
        require(value >= user.amount, 'NFTStaking: Invalid deposit');
        ...
        emit Deposit(msg.sender, tokenIds, incValue, to);
    }
    ...
    function withdraw(uint256[] memory tokenIds, address to) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(depositNftIds[msg.sender].remove(tokenId), "NFTStaking: not contain id");
...
//
//            if (_pool.isIncentive) {
//                _rewardPerShare = _updatePool(_pool, _totalValueIncentive);
//                accumulatedReward = int256(user.incentiveAmount * _rewardPerShare / ACC_REWARD_PRECISION);
//            } else {
//                _rewardPerShare = _updatePool(_pool, totalValue);
//                accumulatedReward = int256(user.amount * _rewardPerShare / ACC_REWARD_PRECISION);
//            }
//            _pendingReward = uint256(accumulatedReward - userRewardDebt[_user][_pid]);
//
...
//            userRewardDebt[_user][_pid] = accumulatedReward;
//            if (_pendingReward > 0) {
//                IERC20(_pool.token).safeTransfer(to, _pendingReward);
//            }
//            emit Harvest(msg.sender, _pendingReward);
//        }
//    }
...
//        uint256[] memory tokenIds = userInfo[msg.sender].nftIds.values();
...
//    }
    ...
    function getTotalValue(uint256 _pid) internal view returns (uint256, uint256) {
        return (totalValue, totalValueIncentive);
    }
    ...
    **/
    function addPoolReward(address _token, bool _isIncentive) internal {
        totalPool ++;
...
//        setRewardPerSecond(_newRewardPerSecond);
    ...
    function rescueFund(uint256 _amount) external onlyOwner {
        require(_amount > 0 && _amount <= PSR.balanceOf(address(this)), "invalid amount");
        PSR.safeTransfer(owner(), _amount);
