...
contract FINAL is Context, IERC20, Ownable {
    ...
    string private _name = "FINAL4";
    string private _symbol = "FCHT4";
    ...
    function checkClaimEligibility(address account) public view returns (
        ...
        if (lastTransfer > 0 && held < _minHoldToClaim){
            return (0, 0, 0, 0, 0, "Hold at least 10,000");
        }                      
        else if (trans >= 157784630 && lastTransfer > 0){ //5 years hold
            return (held, lastTransfer, trans, 60, 8, "TBA");
        }
        else if (trans >= 110449241 && lastTransfer > 0){ //42 months hold
            return (held, lastTransfer, trans, 42, 7, "TBA");
        }
        else if (trans >= 63113852 && lastTransfer > 0){ //24 months hold
            return (held, lastTransfer, trans, 24, 6, "8X multiplier");
        }
        else if (trans >= 47335389 && lastTransfer > 0){ //18 months hold
            return (held, lastTransfer, trans, 18, 5, "5X multiplier");
        }
        else if (trans >= 31556926 && lastTransfer > 0){ //12 months hold
            return (held, lastTransfer, trans, 12, 4, "3X multiplier");
        }
        ...
        else if (trans >= 300 && lastTransfer > 0){ //5 minute test
            return (held, lastTransfer, trans, 50, 50, "5 minute test");
        }
        else if (trans > 120 && lastTransfer > 0){ //2 minute test
            return (held, lastTransfer, trans, 20, 20, "2 minute test");
        }
        else if (trans == block.timestamp){
            return (held, 0, transferTimeTo[account], 0, 0, "Diamond Hands!");
            }
        else {
