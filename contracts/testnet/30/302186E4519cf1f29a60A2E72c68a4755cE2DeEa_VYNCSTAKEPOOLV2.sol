    ...
    OwnableUpgradeable
{
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        usdAmount -= (stake_fee * _price) / decimal4; /// changed
    ...
    function unStake(uint256 amount) external nonReentrant {
        ...
        uint256 _price = busdPrice.price();
        uint256 amountInDollars = (amount * _price) / decimal4; /// changed

        require(amountInDollars <= maxUnstakePerTx, "exceed unstake limit per tx"); /// changed
        ...
        // reward update
        // if ((amountInDollars + unstakeFeeInDollars) < stakeBalance)
        if ((amount + unstake_fee) < _userInfo.stakedVync) {
            ...
            _userInfo.nextCompoundDuringStakeUnstake = nextCompound();
            _userInfo.totalClaimedReward = 0;
            _userInfo.stakeBalanceWithReward = _userInfo.stakeBalance - amountInDollars + _pendingReward; /// changed
            ...
            // u += amountInDollars + unstakeFeeInDollars; /// changed
