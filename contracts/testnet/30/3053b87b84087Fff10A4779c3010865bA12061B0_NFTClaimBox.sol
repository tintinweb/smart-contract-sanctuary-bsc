...
contract NFTClaimBox is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        saleBoxs[0] = SaleBox(0, 0, 1636386254, 1670744879, 0, 0, 0, 0);
        saleBoxs[1] = SaleBox(
            1,
            160 * 10**18,
            1636386254,
        ...
        saleBoxs[2] = SaleBox(2, 0, 1636386254, 1670744879, 10, 50, 0, 400);
    }
    ...
    function setUserInfo(UserInfo memory userInfo, address userAddress)
        external
        onlyOwner
    {
    ...
    function _claimNormal(uint256 _typeBox, uint256 _amount) internal {
        ...
        emit Sale(tokenId, _msgSender(), _amount, _typeBox, true);
    }
    ...
    function _claimWithoutFee(uint256 _typeBox, uint256 _amount) internal {
        ...
        if (rareRand <= saleBoxs[_typeBox].successPercent) {
            randManager.randMod(_msgSender(), PERCENTS_DIVIDER);
            ...
            emit Sale(0, _msgSender(), _amount, _typeBox, false);
        }
    }
    ...
    function openBox(uint256 boxId, uint256 nftCommonId)
        public
        whenNotPaused
        nonReentrant
    {
        ...
        require(
            nft.ownerOf(nftCommonId) == _msgSender(),
            "not own nft commmon"
        );
        require(
            nftCore.getNFT(nftCommonId).rare >= 1,
            "require nft doge rare better than normal"
        );
        ...
        if (typeBox == 0) {
            if (rare == 3) {
                ...
                if (rareRand > 5 && rareRand <= 20) {
                    rareNftDoge = 2;
                }
                if (rareRand > 20 && rareRand <= 50) {
                    rareNftDoge = 1;
                }
                if (rareRand > 50) {
                    rareNftDoge = 0;
        ...
        if (typeBox == 1) {
            if (rare == 4) {
                ...
                if (rareRand > 5 && rareRand <= 10) {
                    rareNftDoge = 3;
                }
                if (rareRand > 10 && rareRand <= 20) {
                    rareNftDoge = 2;
                }
                if (rareRand > 20 && rareRand <= 50) {
                    rareNftDoge = 1;
        ...
        nft.transferFrom(_msgSender(), deadAddress, boxId);
