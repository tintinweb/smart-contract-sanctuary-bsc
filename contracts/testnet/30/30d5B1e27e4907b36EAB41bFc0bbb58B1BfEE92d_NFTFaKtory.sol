...
contract NFTFaKtory is ERC721URIStorage, Ownable {
        ...
        returns (uint256)
    {
        require(
            msg.value >=
                Oracle(oracleAddress).getBambooByDollar(nftPriceInDollar), "Insufficient funds.");
        _tokenIds.increment();
        ...
        console.log(amount);
        BambooToken(bambooTokenAddress).burn(amount);
        targetAddress.transfer(msg.value);
        _setTokenURI(newItemId, tokenURI);
        emit MintedNFT(newItemId, amount);
        return newItemId;
    }
...
contract Oracle is Ownable{
    ...
    function getBambooByDollar(uint256 _dollars)
        public
        view
        returns (uint256)
    {
