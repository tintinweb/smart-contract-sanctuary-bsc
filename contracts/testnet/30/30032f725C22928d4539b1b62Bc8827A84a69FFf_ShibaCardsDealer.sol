...
contract ShibaCardsDealer is IDealer, ShibaCardsAccessible {
  ...
  uint256 public set;
  mapping(uint256 => uint256[]) public mintableBySet;
 
  uint256 constant EDITIONS = 3;


  // TODO maybe predefine shares for each edition
  ...
  function getSharesOf(uint256[] memory tokenIds) public override view returns(uint256) {
    uint256 totalShares;
  ...
  function getRandomId(uint256 randomNumber)
    ...
    returns (uint256)
  {
    return _getRandomId(randomNumber);
  }

  function _getRandomId(uint256 randomNumber)
    internal
    view
    returns (uint256)
  {
    return
      randomNumber.mod(mintableBySet[set].length).div(EDITIONS).mul(EDITIONS).add(pickEdition(randomNumber.mod(100)));
  }

  function getRandomIds(uint256 randomNumber, uint256 n) public view override onlyWhitelisted returns(uint256[] memory) {
    uint256[] memory ids = new uint256[](n);
    for (uint256 i = 0; i < n; i++) {
      ids[i] = _getRandomId(uint256(keccak256(abi.encode(randomNumber, i))));
    }
    return ids;
  }
  ...
  /**
   * @dev Set the current set to mint cards from
   */
  function activateSet(uint256 _set) public onlyAdmin {
    set = _set;
  }

  /**
   * @dev Creates a new mintable token with editions.
   * @dev adds 20% shares for platinum cards, platinums are not mintable
   */
  function create(bool platinum) public override onlyWhitelistedOrAdmin returns(uint256[] memory) {
    uint256[] memory ids = new uint256[](EDITIONS);
    
    for (uint256 index = 0; index < EDITIONS; index++) {
      uint256 newId = _getNewId();
      uint256 shares = 10**index;

      if (platinum) {
        sharesOf[newId] = shares.add(shares.mul(100).div(20));
      } else {
        sharesOf[newId] = shares;
        mintableBySet[set].push(newId);
      }

      ids[index] = newId;
    }

    emit CardCreated(ids, !platinum);
