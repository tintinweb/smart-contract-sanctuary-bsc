...
interface IFactory {
    function collections(uint256 _collectionId) external view returns(address);
}
...
contract KunciNFTOffering is Ownable, ReentrancyGuard, Pausable {
    ...
    {
        address _collectionAddr = IFactory(_stash).collections(_collectionId);
    ...
    whenNotPaused
    {
        ...
        if (_lastBuyer != address(0)) --buyPerAccount[_lastBuyer];
        unchecked{++buyPerAccount[msg.sender];}
