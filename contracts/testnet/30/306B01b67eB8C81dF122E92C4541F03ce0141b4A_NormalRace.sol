...
contract NormalRace is Caller {
    ...
    uint256 public exitTax; //退出税率
    uint256 public k; //退出税率递减率
    mapping(address => uint8) private signUpLimit; //报名数量限制
    IConfig public config;
    uint256 public signUpStartBlock; //报名开始区块高度
    uint256 public signUpEndBlock; //报名截止区块高度
    uint256 public signUpCount; //报名总人数
    uint8 public state; //竞赛状态 0：未开始  1:报名开始  2：竞赛开始  3：结束
    address[] private pizePoolTokens; //奖池资产类别
    mapping(address => uint256) public pizePoolMap; //奖池资产数量

    EnumerableMap.UintToAddressMap private surplusTokenMap; //存活NFT记录
    mapping(address => uint8) public signUpCountMap; //单个用户已报名数量
    uint8 public currentRound; //当前轮次
    uint256 public pureEliminateBlock; //上次淘汰区块高度
    uint256 public eliminateIntercalBlock; //淘汰间隔区块数量
    uint256 public championTokenId; //冠军tokenid
    ...
    constructor(IConfig config_) {
        ...
        state = 0;
        currentRound = 0;
        pureEliminateBlock = 0;
        eliminateIntercalBlock = 0;
        championTokenId = 0;
        config = config_;
    }

    // pass
    function setExitTaxAndK(uint256 exitTax_, uint256 k_) external onlyCaller {
        require(2 > state && 10000 > exitTax_ && k_ <= exitTax_, "state error");
        exitTax = exitTax_;
        k = k_;
    }
    ...
    function setRaceParam(
        uint256 signUpStartBlock_,
        uint256 signUpEndBlock_,
        uint256 eliminateIntercalBlock_
    ) external onlyCaller {
        uint256 currentNum = block.number;
        if (0 == state) {
            require(
                currentNum < signUpEndBlock_ &&
                    signUpStartBlock_ < signUpEndBlock_,
                "state error"
            );
        } else {
            require(
                1 == state &&
                    signUpStartBlock_ < currentNum &&
                    signUpStartBlock_ < signUpEndBlock_,
                "state error"
            );
        }
        require(0 < eliminateIntercalBlock_, "");
        require(signUpStartBlock_ + 10 < signUpEndBlock_, "");
        signUpStartBlock = signUpStartBlock_;
        signUpEndBlock = signUpEndBlock_;
        eliminateIntercalBlock = eliminateIntercalBlock_;
    }

    // pass
    function setRaceParamV2() external onlyCaller {
        uint256 currentNum = block.number;
        uint256 signUpEndBlock_ = currentNum + 2;
        require(
            1 == state &&
                signUpStartBlock < currentNum &&
                signUpStartBlock < signUpEndBlock_,
            "state error"
        );
        require(0 < eliminateIntercalBlock, "");
        require(signUpStartBlock + 10 < signUpEndBlock_, "");
        signUpEndBlock = signUpEndBlock_;
    }

    function signUp(uint256 tokenId) external {
        uint256 currentNum = block.number;
        require(
            signUpStartBlock < currentNum && currentNum < signUpEndBlock,
            "sign up time error"
        );
        require(1 == state, "stat is not correct for signup");
        //判断报名数量是否超过限制
        uint8 registed = signUpCountMap[msg.sender];
        require(50 > registed, "user registed more than limit 50");
        ...
        require(ticket.getUse(tokenId), "token is used");
        ticket.setUse(tokenId);
        INormalTicket.MintInfo memory mintInfo;
        (, mintInfo) = ticket.tryGetNFTInfo(tokenId);
        address token = mintInfo.token;
        //处理奖池token和奖池费用
        bool hasToken = false;
        for (uint256 i = 0; i < pizePoolTokens.length; i++) {
            if (token == pizePoolTokens[i]) {
                hasToken = true;
                break;
            }
        }
        if (!hasToken) {
            pizePoolTokens.push(token);
        }
        pizePoolMap[token] = pizePoolMap[token].add(mintInfo.prizePoolFee, "");
        surplusTokenMap.setUintToAddressMap(tokenId, msg.sender);
        signUpCount = signUpCount.add(1, ""); //报名人数+1
        emit SignUpEvent(address(ticket), msg.sender, tokenId);
    }

    // pass
    //修改竞赛状态
    function setState() external onlyOwner {
        uint256 currentNum = block.number;
        if (0 == state) {
            if (signUpStartBlock < currentNum) {
                state = 1;
            }
        } else if (1 == state) {
            if (signUpEndBlock < currentNum) {
                state = 2;
                pureEliminateBlock = currentNum;
            }
        }
    }
    ...
    function exit(uint256 tokenId) external {
        require(
            2 == state && 1 < currentRound,
            "state or round error for exit"
        );
        require(msg.sender == surplusTokenMap.getUintToAddressMap(tokenId), "");
        uint256 surplusCount = surplusTokenMap.lengthUintToAddressMap();
        require(1 < surplusCount, "surplus count less than 1");
        //删除存活记录
    ...
    // pass
    function _returnReward(address owner_, uint256 surplusCount) private {
        address token;
        uint256 amount;
        uint256 exitAmount;
        uint256 decrease;
        //退奖金逻辑
        for (uint256 i = 0; i < pizePoolTokens.length; i++) {
            token = pizePoolTokens[i];
            amount = pizePoolMap[token];
            if (0 < amount) {
                exitAmount = amount.div(
                    surplusCount,
                    "compute exitAmount error"
                );
                //计算税率
                //currentRound:3
                //decrease:1%=100
                //exitTax:10%=1000
                decrease = k.mul(currentRound, "");
                if (exitTax > decrease) {
                    decrease = decrease.add(10000, "").sub(exitTax, "");
                    exitAmount = exitAmount.mul(decrease, "").div(10000, "");
                }
                pizePoolMap[token] = amount.sub(
                    exitAmount,
                    "sub pize poll amount error."
                );
                TransferV1.safeTransfer(token, owner_, exitAmount);
            }
        }
    }

    // pass
    function getHash(uint256 index, bytes32 blockBytes)
        private
        pure
        returns (uint8)
    {
        uint256 len = blockBytes.length;
        uint256 mod = index.mod(len, "execute block hash mod error.");
        return uint8(blockBytes[mod]);
    }

    function clear(uint256 number, uint8 currentRound_) external {
        uint256 currentNum = block.number;
        require(
            2 == state &&
                pureEliminateBlock + eliminateIntercalBlock <= currentNum,
            ""
        );
        require(currentRound <= currentRound_, "");
        currentRound = currentRound_; //TODO
        pureEliminateBlock = currentNum; //TODO
        ...
        while (0 < number) {
            timestamp = timestamp.add(uint256(getHash(number, blockBytes)), "");
            number = number.sub(1, "");
            clearNum = timestamp.mod(count, "clear mod count error.");
            (tokenId, owner_) = surplusTokenMap.atUintToAddressMap(clearNum);
            surplusTokenMap.removeUintToAddressMap(tokenId);
            emit ClearEvent(ticket, owner_, tokenId);
    ...
    function _setChampion() private {
        require(
            1 == surplusTokenMap.lengthUintToAddressMap(),
            "surplus num is error"
        ); //TODO
        address championAddress;
        (championTokenId, championAddress) = surplusTokenMap.atUintToAddressMap(
            0
        );
        surplusTokenMap.removeUintToAddressMap(championTokenId);
        state = 3;
        //TODO 李强 授权
        config.getNormalTicket().safeTransferFrom(
            config.getNormalManagerFeeAddress(),
            championAddress,
            championTokenId
        );
        ...
        for (uint256 i = 0; i < pizePoolTokens.length; i++) {
            ...
            amount = pizePoolMap[token];
            pizePoolMap[token] = 0;
            TransferV1.safeTransfer(token, championAddress, amount);
        }
        //TODO 送高级场次门票
    }
}
