...
// File: contracts/PoolMaster1.sol
...
contract PoolIDO1 is Ownable, ReentrancyGuard {
    ...
    uint256 public totalOfSlot;
    uint256 public totalOfUserStaked;
    uint256 public requireStakingToken;
    uint256 public feeAmount;
    uint256 public deminomator = 100;
    uint256 public vestTimes = 4;
    ...
    modifier validUnstakeTime() {
        require(unStakeTime < block.timestamp, "Time return invalid");
        _;
    }
    ...
    function setBuyTime(uint256 _startBuyTime, uint256 _endBuyTime)
        public
        onlyOwner
    {
    ...
    function setClaimTime(
        ...
        uint256[] calldata _percentList
    ) public onlyOwner {
        require(_startTimeList.length > 0, "Input stage invalid");
        require(
            _startTimeList.length == _percentList.length,
            "Input stage invalid"
        );
    ...
    // Set fee and number
    function setFeePayIdo(uint256 _feeAmount) public onlyOwner {
        feeAmount = _feeAmount;
    }
    ...
    function setRequireStakingAmount(uint256 _requireStakingToken)
        public
        onlyOwner
    {
        requireStakingToken = _requireStakingToken;
    ...
    // Staking to get Whitelist
    function stakeGetSlot() public validStakeTime nonReentrant isRun {
        require(blacklistMap[msg.sender] == false, "You are in Blacklist");
        require(whitelistMap[msg.sender] == false, "You are in Whitelist");
        require(totalOfUserStaked < totalOfSlot, "Full staking slot");
        ...
        stakeToken.safeTransferFrom(msg.sender, devWallet, feeAmount);
    ...
    //     require(
    //         currentIndexVesting < stageList.length,
    //         "Claim index is invalid"
    //     );
    //     require(currentPercentVesting > 0, "Percent vesting is invalid");
    //     require(
    //         currentTimeVesting < block.timestamp,
    //         "Claim time is not started"
    //     );

    //     vestingMap[msg.sender] = currentIndexVesting + 1;

    //     uint256 amountClaim = currentPercentVesting.mul(tokenPerSlot).div(deminomator);
    //     claimToken.safeTransfer(msg.sender, amountClaim);

    //     emit EventUserClaim(msg.sender, amountClaim, block.timestamp);
    // }

    function claimIdo() external nonReentrant isRun {  
        require(blacklistMap[msg.sender] == false, "You are in Blacklist");
            require(
                whitelistMap[msg.sender] == true,
                "You are not whitelist before"
            ...
            uint256 currentVestStage;

            // if(currentTime >= stageList[0].startTime && currentTime < stageList[1].startTime) {
            //      currentVestStage = 1;
            // }

            // if(currentTime >= stageList[1].startTime && currentTime < stageList[2].startTime) {
            //      currentVestStage = 2;
            // }

            // if(currentTime >= stageList[2].startTime && currentTime < stageList[3].startTime) {
            //      currentVestStage = 3;
            // }

            // if(currentTime >= stageList[3].startTime) {
            //      currentVestStage = 4;
            // }

            // for(uint256 i = vestTimes - 1; i >= 0; i--) {
            //     if(currentTime >= stageList[i].startTime) {
            //         currentVestStage = i + 1;
            //         break;
            //     } 
            // }

            for(uint256 i = 0; i < vestTimes; i++) {
                if(currentTime < stageList[i + 1].startTime && currentTime < stageList[vestTimes - 1].startTime) {
                    currentVestStage = i + 1;
                    break;
                } else if( currentTime >= stageList[vestTimes - 1].startTime) {
                    currentVestStage = vestTimes;
                }
            }

            vestingMap[msg.sender] = currentVestStage;

            uint256 amountClaim = currentPercentVesting.mul(currentVestStage).mul(tokenPerSlot).div(deminomator);
            claimToken.safeTransfer(msg.sender, amountClaim);
    ...
    function unstakeToken() public nonReentrant validUnstakeTime {
        require(blacklistMap[msg.sender] == false, "You are in Blacklist");
        require(whitelistMap[msg.sender] == true, "You are not paid before");
        require(
            unstakeMap[msg.sender] == false,
            "You are not staked token before"
        );

        unstakeMap[msg.sender] = true;

        stakeToken.safeTransfer(msg.sender, requireStakingToken);
    ...
    //Withdraw all token from contract to idoOwner
    function urgentWithdrawAllToken() public onlyOwner {
        uint256 stakeTokenInContract = stakeToken.balanceOf(address(this));
        if (stakeTokenInContract > 0) {
            stakeToken.safeTransfer(idoOwner, stakeTokenInContract);
        }

        uint256 claimTokenInContract = claimToken.balanceOf(address(this));
        if (claimTokenInContract > 0) {
            claimToken.safeTransfer(idoOwner, claimTokenInContract);
        }
    }
}