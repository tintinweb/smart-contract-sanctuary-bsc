...
contract SpaceDaoMiner {
    using SafeMath for uint256;

    IERC20 public token_space;
	address erctoken = 0x490Aad6E3ceA8a9109f5E5fa8F174CBC24881604; /** testnet SPACE DAO**/
    ...
    uint256 public PARTNER1 = 20;
    uint256 public PARTNER2 = 10;
    ...
    uint256 public LOTTERY_PERCENT = 5; /** 0.5% **/
    uint256 public LOTTERY_STEP = 3 * 60 * 60; /** every 3 hours. **/
    uint256 public LOTTERY_TICKET_PRICE = 3 * 1e9; /** 3 SPACE **/
	...
	uint256 public CUTOFF_STEP = 26 * 60 * 60;
	uint256 public WITHDRAW_COOLDOWN = 12 * 60 * 60; /** 12 hours  **/
    uint256 public WALLET_DEPOSIT_LIMIT = 5000 * 1e9; /** 5,000 SPACE  **/
    ...
    constructor(address payable _owner, address payable _project, address payable _partner1, address payable _partner2) {
		require(!isContract(_owner) && !isContract(_project) && !isContract(_partner1) && !isContract(_partner2));
        token_space = IERC20(erctoken);
        owner = _owner;
        project = _project;
        partner1 = _partner1;
        partner2 = _partner2;
    ...
    function sellEggs() public{
        ...
        token_space.transfer(msg.sender, eggsPayout);
    ...
    function buyEggs(address ref, uint256 amount) public payable{
        ...
        token_space.transferFrom(address(msg.sender), address(this), amount);
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                /** referral rewards will be in SPACE **/
                uint256 refRewards = amount.mul(REFERRAL).div(PERCENTS_DIVIDER);
                token_space.transfer(upline, refRewards);
                /** referral rewards will be in SPACE value **/
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        (uint256 projectFee, uint256 partnerFee1, uint256 partnerFee2) = getFees(eggValue);
        token_space.transfer(project, projectFee);
		token_space.transfer(partner1, partnerFee1);
		token_space.transfer(partner2, partnerFee2);
        return projectFee.add(partnerFee1).add(partnerFee2);
    }

    function getFees(uint256 eggValue) public view returns(uint256 _projectFee, uint256 _partner1Fee,  uint256 _partner2Fee) {
        _projectFee = eggValue.mul(PROJECT).div(PERCENTS_DIVIDER);
        _partner1Fee = eggValue.mul(PARTNER1).div(PERCENTS_DIVIDER);
        _partner2Fee = eggValue.mul(PARTNER2).div(PERCENTS_DIVIDER);
    }
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
        ...
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    token_space.transfer(project, back);
    ...
    function getBalance() public view returns (uint256) {
        return token_space.balanceOf(address(this));
	}
    ...
    /** How many miners and eggs per day user will recieve based on SPACE deposit **/
    ...
    **/
    ...
    function PRC_PARTNER1(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 10 && value <= 50); /** 5% max **/
        PARTNER1 = value;
    }

    function PRC_PARTNER2(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
    ...
    function SET_WALLET_DEPOSIT_LIMIT(uint256 value) external {
        require(msg.sender == owner, "Admin use only");
        require(value >= 20);
        WALLET_DEPOSIT_LIMIT = value * 1 * 1e9;
    }
}
