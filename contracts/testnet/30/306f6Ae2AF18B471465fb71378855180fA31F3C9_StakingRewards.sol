...
contract StakingRewards {
    ...
    uint256 public DEFAULT_POOL_SIZE;
    ...
    )
    {
        ...
        DEFAULT_POOL_SIZE = _defaultEpochRewardSize;
    ...
    function getCurrentEpoch() public view returns(uint256) {
        ...
        return epochNum < NUMBER_OF_EPOCHS ? epochNum : NUMBER_OF_EPOCHS;
    }
    ...
    function getStakesByEpoch(uint256 epochNum, address userAddr) private view returns(uint256, uint256, uint256) {
        // total
        uint256 prevTotalStake = 0; // S_0
        uint256 prevPoolSize = 0; // POOL_0
        uint256 prevPoolResidue = 0;

        // user
        uint256 prevUserStake = 0;
        uint256 prevUserReward = 0;
        ...
        for (uint256 epoch = 1; epoch <= epochNum; epoch++) {
            ...
            uint256 curPoolMaxSize = DEFAULT_POOL_SIZE + prevPoolResidue;
            ...
            // total
            prevPoolSize = curPoolSize;
            prevPoolResidue = curPoolResidue;
            prevTotalStake = curTotalStake;

            // user
            prevUserStake = curUserStake;
            prevUserReward = curUserReward;

            console.log("--------Epoch", epoch);
            console.log("curTotalStake", curTotalStake);
            console.log("curPoolSize", curPoolSize);
            console.log("curPoolResidue", curPoolResidue);
            console.log("curUserStake", curUserStake);
            console.log("curUserReward", curUserReward);
            console.log("overrated", overrated);
        }

        return (prevTotalStake, prevUserStake, prevUserReward);
    }
    ...
    function getCurrentEpochStakes(address userAddr) public view returns(uint256, uint256, uint256) {
        ...
        return getStakesByEpoch(curEpoch, userAddr);
    }
    ...
    function getNextEpochStakes(address userAddr) public view returns(uint256, uint256, uint256, uint256) {
        ...
        (uint256 totalStake, uint256 userStake, uint256 userReward) = getStakesByEpoch(nextEpoch, userAddr);

        uint256[] memory userStakes = stakeholderToStakes[userAddr];
        uint256 userNextEpochStake = userStakes.length > 0 ? userStakes[nextEpoch] : 0;

        return (totalStake, userStake, userReward, userNextEpochStake);
    }
    ...
    function stake(uint256 amount) public {
        ...
        if (stakesLength == 0) {
            stakeholderToStakes[msg.sender] = new uint256[](NUMBER_OF_EPOCHS);
        }
    ...
    function unstake(uint256 amount) public {
        ...
        if (unstakesLength == 0) {
            stakeholderToUnstakes[msg.sender] = new uint256[](NUMBER_OF_EPOCHS);
        }
