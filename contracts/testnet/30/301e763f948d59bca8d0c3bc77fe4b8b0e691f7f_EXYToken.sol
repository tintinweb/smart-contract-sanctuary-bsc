...
contract EXYToken is ERC20, ERC20Burnable, ERC20Snapshot, Ownable {
    using SafeMath for uint256;
    address private _marketAddress = 0x05a46cD17BDaBb0b230832B93b3F9D72Bff942EA;
    ...
    constructor() ERC20("EXYToken", "EXY") {
        _mint(msg.sender, 1000 * 10000000000 * 10 ** decimals());
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if(_feeEnabled && !_isExcludedFromFees[msg.sender]){
            uint256 burnAmount = amount.mul(_destroyFee).div(100);
            uint256 marketAmount = amount.mul(_marketFee).div(100);
            uint256 realAmount = amount.sub(burnAmount).sub(marketAmount);
            super.burn(burnAmount);
            super.transfer(_marketAddress,marketAmount);
            return super.transfer(recipient, realAmount);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if(_feeEnabled && !_isExcludedFromFees[msg.sender]){
            ...
            super.transferFrom(sender, _marketAddress, marketAmount);
            return super.transferFrom(sender, recipient, realAmount);
        }else{
