...
contract MEMEINDEX is Context, IBEP20, Ownable 
{
    ...
    address public developmentAddress = 0x7219C7602Bf0633E26A723F2c7E43b58605Bad7A; 
    address public marketingAddress = 0x7219C7602Bf0633E26A723F2c7E43b58605Bad7A; 
    address public investmentAddress = 0x47D3550C1C17b77f3523A6D435fdd48Cf0c6b75E; 
    ...
    uint256 public _investmentFee = 3;
    ...
    constructor () 
    {
        ...
        _isExemptFromTxLimit[investmentAddress] = true;
    }
    ...
    {
        uint256 _totalFees = _developmentFee.add(_marketingFee).add(_investmentFee);
        swapTokensForBUSD(contractTokenBalance);
        uint256 newBalance = (IBEP20(BUSD).balanceOf(address(this)));
        
        uint256 developmentShare = newBalance.mul(_developmentFee).div(_totalFees);
        uint256 marketingShare = newBalance.mul(_marketingFee).div(_totalFees);
        uint256 investmentShare =  newBalance.sub(marketingShare).sub(developmentShare);

        IBEP20(BUSD).transfer(developmentAddress, developmentShare);
        IBEP20(BUSD).transfer(marketingAddress, marketingShare);
        IBEP20(BUSD).transfer(investmentAddress, investmentShare);
    ...
    {
        return _amount.mul(_developmentFee.add(_marketingFee).add(_investmentFee)).div(100);
    }
    ...
    function setinvestmentAddress(address _investmentAddress) external onlyOwner() 
    {
