...
// File: contracts/BalanceStake.sol
...
contract BalanceStake {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = balance.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 balanceSupply = balance.balanceOf(address(this));
        if (balanceSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(balanceSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit Balance tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        balance.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Balance tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        balance.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        balance.safeTransfer(address(msg.sender), user.amount);
