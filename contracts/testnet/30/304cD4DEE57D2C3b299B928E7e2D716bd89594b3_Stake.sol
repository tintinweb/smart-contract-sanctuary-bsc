...
contract Stake is Initializable,OwnableUpgradeable{
    ...
    struct ProjectInfor{
        ...
        mapping(uint256 => uint256)  theStakeProjectTokenAmountByTheday;
         uint256  StakePoolStartTime;
        uint256  StakePoolEndTime;
    }
   ...
   // mapping (uint256=>uint256) TheDaysAllAmount;
    ...
    // }
    function setProjectInfor(uint256 project, uint256 totalfundraised,uint256 maxreward_project)public onlyOwner{
            ProjectInfor storage _projectinfor = getProjectInforByProjectId[project];
            _projectinfor.Totalfundraised=totalfundraised;
            _projectinfor.MaxReward= maxreward_project;
    }

    function setTokenAddress(address usdtaddress, address labsaddress,address arenaBaddress,address kngaddress)public onlyOwner{
           USDTaddress=usdtaddress;
        Labsaddress=labsaddress;
        ArenaBaddress=arenaBaddress;
        Kngaddress=kngaddress;
    ...
    function setStakePoolStartTime(uint256 project,uint256 stakepoolstarttime) public onlyOwner {

       
        getProjectInforByProjectId[project].StakePoolStartTime=stakepoolstarttime;
        getProjectInforByProjectId[project].StakePoolEndTime=stakepoolstarttime+86400*365;
       
        

    }

    function updateTheStakeProjectTokenAmountByTheday() public {
    ...
    function getKngamount() public view returns(uint256){
      ...
      function getStakePoolStartTime(uint256 project ) public view returns(uint256 _stakepoolstarttime,uint256 _stakepoolendtime)  {

       
        _stakepoolstarttime=getProjectInforByProjectId[project].StakePoolStartTime;
        _stakepoolendtime=getProjectInforByProjectId[project].StakePoolEndTime;
       
        

    }

    function getStakePoolEndTime(uint256 project) public view returns(uint256){
        return getProjectInforByProjectId[project].StakePoolEndTime;
    }

     function DailyRewardsPerTokenInReward(uint256 project,uint256 theday) public view  returns(uint256){
        uint256 dailyRewardsPerTokenInReward;
        if(getProjectInforByProjectId[project].theStakeProjectTokenAmountByTheday[theday]==0){
            dailyRewardsPerTokenInReward=10**20*getProjectInforByProjectId[project].MaxReward/365;
        }else{
             dailyRewardsPerTokenInReward=10**20*getProjectInforByProjectId[project].MaxReward/365/getProjectInforByProjectId[project].theStakeProjectTokenAmountByTheday[theday];
        }
       
        return dailyRewardsPerTokenInReward;
    }
    ...
    function gettheday( uint256 project,uint256 time)public view returns(uint256){
        return ((time-getProjectInforByProjectId[project].StakePoolStartTime)/86400)+1;


    }


    function getthestrartandendday( uint256 project, uint256 starttime,uint256 endtime) internal view returns(uint256 stratday,uint256 endday){
        if(gettheday(project,endtime)-gettheday(project,starttime)==(endtime-starttime)/86400){
                     stratday=gettheday(project,starttime);
                     endday=gettheday(project,endtime);
               }else{
                    stratday=gettheday(project,starttime);
                     endday=gettheday(project,endtime)-1;
               }
    ...
    function getRewardinfor( uint256 project, address account) public view returns(uint256){
        require(block.timestamp>getProjectInforByProjectId[project].StakePoolStartTime,"stake is not start");
        ProjectInfor storage _projectinfor = getProjectInforByProjectId[project];
        ...
        uint256 endday;
         uint256 getRewardtime=testtime;
                if(getRewardtime>_projectinfor.StakePoolEndTime){
                    getRewardtime=_projectinfor.StakePoolEndTime;
                }
        if(stakingaccount.stakeids.length==1){
                stakeInfor memory _stakeinfor= _projectinfor.stakeInforById[_stakeids[0]];
               (stratday,endday)=getthestrartandendday(project,_stakeinfor.stakeTime,getRewardtime);
                 for (uint256 k = stratday; k < endday; k++) {
                    getAllReward+=_stakeinfor.stakeAmount*DailyRewardsPerTokenInReward(project,k);
            ...
            for(uint256 y=0;y<_stakeids.length;y++){
                    ...
                    if(_stakeTime>_projectinfor.StakePoolEndTime){
                        k=y-1;
                        break;
                    }
            }
            ...
            for(uint256 i=0;i<k;i++){
                ...
                //uint256 stakedays;
                (stratday,endday)=getthestrartandendday(project,_stakeinfor.stakeTime,stakeinfor.stakeTime);
                uint256 getRewardonceStake;
                for (uint256 j = stratday; j < endday; j++) {
                    getRewardonceStake+=_stakeinfor.stakeAmount*_projectinfor.theStakeProjectTokenAmountByTheday[j];
            ...
            // uint256 getUSDTonceStake = stakedays*_stakeinfor.stakeAmount*DailyRewardsPerTokenInUSDT();
             ...
             stakeInfor memory laststakeinfor= _projectinfor.stakeInforById[_stakeids[k]];
                (stratday,endday)=getthestrartandendday(project,laststakeinfor.stakeTime,getRewardtime);
                uint256 getLastReward;
                for (uint256 m = stratday; m < endday; m++) {
                     getLastReward+=laststakeinfor.stakeAmount*DailyRewardsPerTokenInReward(project,m);
                }
    ...
    function stakeverify(uint256 project,uint256 amount) view internal {
        ...
        require(testtime>=getProjectInforByProjectId[project].StakePoolStartTime,"Stake Pool is not Start");
        require(testtime<=getProjectInforByProjectId[project].StakePoolEndTime,"Stake Pool is  end");
    }
    ...
    function beforestake(uint256 project) internal {
        uint256 staketime=(testtime-getProjectInforByProjectId[project].StakePoolStartTime)/86400+1;
        if(today==0){
            today=staketime;
        }else{
            if(staketime!=today){
                for (uint256 i = today; i < staketime; i++) {
                    getProjectInforByProjectId[project].theStakeProjectTokenAmountByTheday[i]=getProjectInforByProjectId[project].TotalStakeTokens;
                }
    ...
    function returnReward(uint256 project,address returnaddress) public  onlyOwner{
        require(testtime >= getProjectInforByProjectId[project].StakePoolEndTime, "TokenTimelock: current time is before release time");
        ProjectInfor storage _projectinfor = getProjectInforByProjectId[project];
    ...
    function getTheStakeProjectTokenAmountByTheday(uint256 project,uint256 theday) public view returns(uint256){
        ProjectInfor storage _projectinfor = getProjectInforByProjectId[project];
        return _projectinfor.theStakeProjectTokenAmountByTheday[theday];
    }
}