...
contract mintingmultipletier is BEP721, Ownable {
    ...
    function mint(uint256 tier) external payable {
        require(saleIsActive = true, " u can not mint");
        require(
            tiers[tier].maxSupply <= tiers[tier].totalSupply,
            " cannot mint more thna total supply "
        );
        require(tiers[tier].price <= msg.value, " not enough ether to mint ");
        require(
            addressCountsPerTier[tier][msg.sender] + 1 <=
                tiers[tier].mintPerAddress,
            "minting per address reached"
        );
        addressCountsPerTier[tier][msg.sender] =
            addressCountsPerTier[tier][msg.sender] +
            1;
        uint16 tierTotalSupply = tiers[tier].totalSupply;
        tiers[tier].totalSupply = tierTotalSupply + 1;
        _safeMint(msg.sender, tiers[tier].startingIndex + tierTotalSupply);
    }
