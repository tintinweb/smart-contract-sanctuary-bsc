...
import "./interfaces/ITechProfile.sol";

//need remove on build
import "@techswap/tech-token/contracts/TechToken.sol";

contract TechProfile is ITechProfile, AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public techToken;
    ...
    uint256 public numberTechToReactivate;
    uint256 public numberTechToRegister;
    uint256 public numberTechToUpdate;
    ...
    mapping(address => uint256) public teamOwner;
    ...
    modifier onlyTeam(uint256 teamId) {
        require(hasRole(TEAM_ROLE, _msgSender()) && teamOwner[_msgSender()] == teamId, "Not a team admin");
        _;
    }
    ...
    constructor(
        IBEP20 _techToken,
        uint256 _numberTechToReactivate,
        uint256 _numberTechToRegister,
        uint256 _numberTechToUpdate
    ) {
        techToken = _techToken;
        numberTechToReactivate = _numberTechToReactivate;
        numberTechToRegister = _numberTechToRegister;
        numberTechToUpdate = _numberTechToUpdate;
    ...
    function createProfile(
        ...
        techToken.safeTransferFrom(_msgSender(), address(this), numberTechToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        techToken.safeTransferFrom(_msgSender(), address(this), numberTechToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        techToken.safeTransferFrom(_msgSender(), address(this), numberTechToReactivate);
        nftToken.safeTransferFrom(_msgSender(), address(this), _tokenId);
        uint256 userTeamId = users[_msgSender()].teamId;
    ...
    function addTeam(
        ...
        require(teamOwner[_teamOwner] == 0, "Team already exists");
        ...
        teamOwner[_teamOwner] = newTeamId;
        _setupRole(TEAM_ROLE, _teamOwner);
    ...
    function claimFee(uint256 _amount) external onlyOwner {
        techToken.safeTransfer(_msgSender(), _amount);
    }

    function makeTeamJoinable(uint256 _teamId) external onlyTeam(_teamId) {
        require((_teamId <= numberTeams) && (_teamId > 0), "teamId invalid");
        teams[_teamId].isJoinable = true;
    }

    function makeTeamNotJoinable(uint256 _teamId) external onlyTeam(_teamId) {
        require((_teamId <= numberTeams) && (_teamId > 0), "teamId invalid");
    ...
    function updateNumberTech(
        uint256 _newNumberTechToReactivate,
        uint256 _newNumberTechToRegister,
        uint256 _newNumberTechToUpdate
    ) external onlyOwner {
        numberTechToReactivate = _newNumberTechToReactivate;
        numberTechToRegister = _newNumberTechToRegister;
        numberTechToUpdate = _newNumberTechToUpdate;
    }
    ...
    function getUserStatus(address _userAddress) external view override returns (bool) {
        return (users[_userAddress].isActive);
    }

    function getTemIdByOwner(address _teamOwner) external view returns (uint256) {
        return (teamOwner[_teamOwner]);
    }
    ...
    {
        uint256 teamId = teamOwner[_owner];
        return _getTeamProfile(teamId);
    }
    ...
    {
        return _getTeamProfile(_teamId);
    }

    function _getTeamProfile(uint256 _teamId)
        private
        view
        returns (
            string memory,
