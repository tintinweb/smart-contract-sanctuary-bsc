...
contract AnjiRouter is IDEXRouter02, Ownable {
    ...
    constructor(address _WETH) public {
        ...
        dexList[dexCount] = DEX({
            name: "PancakeSwap V1",
            //factory: 0xBCfCcbde45cE874adCB698cC183deBcF17952812,// mainnet
            factory: 0x6725F303b657a9451d8BA641348b6761A6CC7a17,  //testnet
            //router: 0x05fF2B0DB69458A0750badebc4f9e13aDd608C7F,   //mainnet
            router: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1,   //testnet
            //initHash: hex'd0d4c4cd0848c93cb4fd1f498d7013ee6bfb25783ea21593d5834f5d250ece66', //mainnet
            initHash: hex'd0d4c4cd0848c93cb4fd1f498d7013ee6bfb25783ea21593d5834f5d250ece66', //testnet
            fee: 9980,
            enabled: true,
            id: dexCount
        });
        ...
        dexList[dexCount] = DEX({
            ...
            factory: 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc, //testnet
            //router: 0x10ED43C718714eb63d5aA57B78B54704E256024E, //mainnet
            router: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3, //testnet
            //initHash: hex'00fb7f630766e6a796048ea87d01acd3068e8ff67d078148a3fa3f4a84f69bd5', // mainnet
            initHash: hex'ecba335299a6693cb2ebc4782e74669b84290b6378ea3a3873c7231a8d7d1074', //testnet
            fee: 9975,
            enabled: true,
            id: dexCount
        });
        ...
        dexList[dexCount] = DEX({
            name: "ApeSwap",
            //factory: 0x0841BD0B734E4F5853f0dD8d7Ea041c241fb0Da6,   // mainnet
            factory: 0x006031e2eFbBE9764A9276Dd0436d138FB4ac3B3,   //testnet
    ...
    function setSwapDex(uint256 dexId, address path0,address path1) public {
        if (dexId == 0) {
            dexId = getLargestDEX(path0, path1);
        }

        factory = dexList[dexId].factory;
        initHash = dexList[dexId].initHash;
        _fee = dexList[dexId].fee;
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? DEXLibrary.pairFor(initHash, factory, output, path[i + 2]) : _to;
            IDEXPair(DEXLibrary.pairFor(initHash, factory, input, output)).swap(
        ...
        returns (uint[] memory amounts)
    {
        ...
        setSwapDex(dexId, path[0], path[1]);
        amounts = DEXLibrary.getAmountsOut(initHash, factory, amount, path, _fee);
        require(amounts[amounts.length - 1] >= amountOutMin, 'AnjiR: INSUF_OUTA');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(DEXLibrary.pairFor(initHash, factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        //send the fee to the fee receiver
        if (fee > 0) {
            TransferHelper.safeTransferETH(feeReceiver, fee);
        }
        if(IWETH(WETH).balanceOf(feeReceiver) > 3){
        ...
        returns (uint[] memory amounts)
    {
        ...
        //uint amount = msg.value - fee;
        setSwapDex(dexId, path[0], path[1]);
        amounts = DEXLibrary.getAmountsOut(initHash, factory, msg.value - fee, path, _fee);
        require(amounts[amounts.length - 1] >= amountOutMin, 'AnjiR: INSUF_OUTA');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(DEXLibrary.pairFor(initHash, factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(path[0] == WETH, 'AnjiR: INVALID_PATH');
        setSwapDex(dexId, path[0], path[1]);
        amounts = DEXLibrary.getAmountsIn(initHash, factory, amountOut, path, _fee);
        ...
        returns (uint[] memory amounts)
    {
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DEXLibrary.pairFor(initHash, factory, path[0], path[1]), amounts[0]
        );
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'AnjiR: INVALID_PATH');
        setSwapDex(dexId, path[0], path[1]);
        amounts = DEXLibrary.getAmountsOut(initHash, factory, amountIn, path, _fee);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = DEXLibrary.sortTokens(input, output);
            IDEXPair pair = IDEXPair(DEXLibrary.pairFor(initHash, factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = DEXLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, _fee);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        setSwapDex(dexId, path[0], path[1]);
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, DEXLibrary.pairFor(initHash, factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        setSwapDex(dexId, path[0], path[1]);
        assert(IWETH(WETH).transfer(DEXLibrary.pairFor(initHash, factory, path[0], path[1]), amount));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        if (fee > 0) {
            TransferHelper.safeTransferETH(feeReceiver, fee);
        ...
        ensure(deadline)
    {
        ...
        setSwapDex(dexId, path[0], path[1]);
        assert(IWETH(WETH).transfer(DEXLibrary.pairFor(initHash, factory, path[0], path[1]), msg.value - fee));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        if (fee > 0) {
            TransferHelper.safeTransferETH(feeReceiver, fee/2);
    ...
    function getAmountsOut(uint amountIn, address[] memory path, uint256 dexId)
        ...
        returns (uint[] memory amounts)
    {
        DEX memory dex = dexList[dexId];
        return DEXLibrary.getAmountsOut(dex.initHash, dex.factory, amountIn, path, dex.fee);
    }
    ...
    {
        DEX memory dex = dexList[dexId];
        return DEXLibrary.getAmountsIn(dex.initHash, dex.factory, amountOut, path, dex.fee);
    }
}