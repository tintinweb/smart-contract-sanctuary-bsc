...
contract NFTToken is ERC721URIStorage, Ownable {
  ...
  constructor(address marketplaceAddress) ERC721("NFT Token Master", "NFTToken") {
    contractAddress = marketplaceAddress;
      ...
      function createToken(string memory tokenURI, uint256 itemCount, uint256 rary) public onlyOwner returns (uint256[] memory) {
...
contract NFTMarketPlace is ReentrancyGuard, Ownable , WidgetInterface{
...
//GAME ITEM  
  ...
  //on ownwer saller
  uint256 public _adminFee = 3;  // 3%
  uint256 public _adminDev = 0;  // 0%
  // on user seller
  uint256 public _adminFee2 = 3;  // 3%
  uint256 public _adminDev2 = 0;  // 0%
  ...
  //TABLE
  uint256 private _lastDaySell; //timestamp
  mapping(uint256 => uint256) private _daySell;
  mapping(uint256 => uint256) private _daySellAmount;
  ...
  function setHiddenBox (uint id, bool status) public onlyOwner {
    require( _owner == msg.sender,"security guaranteed administrator only");
    hiddenBox[id]=status;
  }

  function setPriceBox (uint id, uint newPrice) public onlyOwner {
     require( _owner == msg.sender,"security guaranteed administrator only");
    _boxes[id].price = newPrice;
  }

  function setLoteBox (uint id, uint newLote) public onlyOwner {
     require( _owner == msg.sender,"security guaranteed administrator only");
    _boxes[id].price = newLote;
  }

  function getBoxStoreItem(uint256 boxStoreId) public view returns (boxStore memory) {
      return _boxes[boxStoreId];
  }

  function getBoxRateItem(uint256 boxRateId) public view returns (boxRate memory) {
      return _rates[boxRateId];
  }

  function setPriceItem(uint256 marketItemId, uint256 newPrice) public onlyOwner {
      require( _owner == msg.sender,"security guaranteed administrator only");
      idToMarketItem[marketItemId].price = newPrice;
  }

  function setLoteItem(uint256 marketItemId, uint256 newLote) public onlyOwner {
      require( _owner == msg.sender,"security guaranteed administrator only");
      idToMarketItem[marketItemId].price = newLote;
  }
  ...
  function createBoxStore (
    ...
    string memory xBox5
      ...
      require (_owner == msg.sender,"security guaranteed administrator only");
     _boxes.push(boxStore(ptBox,xBoxImg, xprice, xlote, xBox1, xBox2, xBox3, xBox4, xBox5));
     pendingBox = true;
     ptBox++;
  }
  ...
  require (_boxes[ID].price == msg.value, "Please submit the asking price in order to complete the purchase");
  ...
  function registerSell (uint256 amount) private {
    //verify last blocktimestamp if > 0 update
    uint startDate = _lastDaySell;
    uint endDate = block.timestamp; 
    uint diff = 0;

    diff = (endDate - startDate) / 60 / 60 / 24; // X days ?
    if(diff > 0) updateDaysSellTable();  //reorganiza a tabela se necessario
  ...
  function backToYesterday () public onlyOwner {
     require( _owner == msg.sender,"security guaranteed administrator only");
    _lastDaySell = block.timestamp - 1 days; 
  }
  ...
  require(nftContract == _nftContract, "Invalid NFT Contract");
    ...
    uint256 tax1 = ( amount * _adminFee )  / 100;
    uint256 tax3 = ( amount * _adminDev )  / 100;
    uint256 tax1_ = ( amount * _adminFee2 )  / 100;
    uint256 tax3_ = ( amount * _adminDev2 )  / 100;
    uint256 totalFee = 0;
    
    /*NFT Game Owner = taxa comportamento 0*/
    if (idToMarketItem[itemId].behavior == 0) {

       totalFee = totalFee.add(tax1).add(tax3);
       amount = amount.sub(totalFee); /*retirando as taxas total 6% */  

       /*BNBTransfers*/  
       idToMarketItem[itemId].seller.transfer(amount); 
       if (tax1 > 0) _wallet1.transfer(tax1);
       if (tax3 > 0) _wallet3.transfer(tax3);
    }

    /*NFT user seller taxa comportamento 1*/
    if (idToMarketItem[itemId].behavior == 1) {

       totalFee = totalFee.add(tax1_).add(tax3_);
       amount = amount.sub(totalFee); /*retirando as taxas total 6% */  
       
      /* BNB Transfers */  
       idToMarketItem[itemId].seller.transfer(amount);
       if (tax1_ > 0) _wallet1.transfer(tax1_);
       if (tax3_ > 0) _wallet3.transfer(tax3_);
    }

    //require(idToMarketItem[itemId].behavior == 1 || idToMarketItem[itemId].behavior == 0, "Only game's and user's NFT itens can to sell in this function");

    /*passando propriedade p comprador NFT ULTIMO DO LOTE*/  
    ...
    //clonando a partir do modelo MUITOS NO LOTE
    if (idToMarketItem[itemId].lote > 1) {
      idToMarketItem[itemId].lote = idToMarketItem[itemId].lote.sub(1);
  ...
  require(idToMarketItem[itemId].behavior == 2, "Only auction NFT itens can to claim in this function");
    ...
    uint256 totalFee = 0;
     ...
     _wallet1.transfer(amount); 
      ...
      IERC721(_nftContract).transferFrom(address(this), sellerr, tokenIdd);
