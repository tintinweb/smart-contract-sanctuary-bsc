...
/**
 *Submitted for verification at BscScan.com on 2022-24-19
*/
...
contract MilksFarmzy is Context, Ownable {
    using SafeMath for uint256;

    uint256 private Milks_TO_Hire_1BOOB = 720000;//for final version should be seconds in a day
    ...
    function HireBoobs(address ref) public {
        require(initialized);
        ...
        uint256 newBOOB = SafeMath.div(MilksUsed,Milks_TO_Hire_1BOOB);
        Factory[msg.sender] = SafeMath.add(Factory[msg.sender],newBOOB);
        claimedMilks[msg.sender] = 0;
        lastHire[msg.sender] = block.timestamp;
        
        //send referral Milks
        claimedMilks[referrals[msg.sender]] = SafeMath.add(claimedMilks[referrals[msg.sender]],SafeMath.div(MilksUsed,10));
    ...
    function SellMilks() public {
        ...
        uint256 fee = devFee(MilksValue);
        claimedMilks[msg.sender] = 0;
        lastHire[msg.sender] = block.timestamp;
        marketMilks = SafeMath.add(marketMilks,hasMilks);
        recAdd.transfer(fee);
        payable (msg.sender).transfer(SafeMath.sub(MilksValue,fee));
    }
    ...
    function BuyBoobs(address ref) public payable {
        require(initialized);
        uint256 MilksBought = calculateHireBoobs(msg.value,SafeMath.sub(address(this).balance,msg.value));
    ...
    function calculateHireBoobs(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(eth,contractBalance,marketMilks);
    }
    
    function calculateHireBoobsSimple(uint256 eth) public view returns(uint256) {
        return calculateHireBoobs(eth,address(this).balance);
    }
    ...
    function getMyBOOB(address adr) public view returns(uint256) {
        return Factory[adr];
    }
    ...
    function getMilksSinceLastHire(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(Milks_TO_Hire_1BOOB,SafeMath.sub(block.timestamp,lastHire[adr]));
