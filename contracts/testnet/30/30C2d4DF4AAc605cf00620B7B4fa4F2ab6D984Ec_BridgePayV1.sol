...
contract BridgePayV1 is Ownable, ReentrancyGuard, Pausable {
    ...
    {
        string[] memory _partnerName = new string[](partnerCount);
        uint256[] memory _iPartner = new uint256[](partnerCount);

        for (uint256 i = 0; i < partnerCount; i++) {
            _partnerName[i] = bridgePayPartners[i].partnerName;
            _iPartner[i] = bridgePayPartners[i].iPartner;
        }
        return (_partnerName, _iPartner);
    }

    function getBridgePartnerss()
        public
        view
        returns (string[] memory, uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < partnerCount; i++) {
            bridgePayPartner storage _bridgePayPartner = bridgePayPartners[i];
            _partnerName[i] = _bridgePayPartner.partnerName;
            _iPartner[i] = _bridgePayPartner.iPartner;
        }
        return (_partnerName, _iPartner);
    }
    ...
    function updateTaxInfo(uint256 _tax, uint256 _taxDenom) external onlyOwner {
        require(_tax <= 10, "Bridge partner tax cannot be more than 10%");
