...
contract YTON is Ownable{
	...
	uint256  public UP_MIN_AMOUNT = 0.001 ether;    //邀请人空投 最少付BNB的数量
	uint256  public BASE_PERCENT = 10;  // 1% 基础费率
	uint256  public REF_BASE_FEE = 20; //收益 20%给十代
	uint256[] public REFERRAL_PERCENTS = [30, 20, 15, 5, 5, 5, 5, 5, 5, 5];  //十代奖励
	uint256[] public REFERRAL_DAO = [3, 2, 1];  //奖励代币
	...
	uint256  public CONTRACT_BALANCE_STEP = 100000 * 10 ** 18; // 达到10WU 速率+0.1%
	uint256 constant public TIME_STEP = 86400; //24xiaoshi
	uint256 public _MAX_DAO_RATE = 30;  //持币最大算力 ..3%
    ...
    IERC20 public dao;   //0x26b8aF43665781E1a50e950e8b2442e9Cb49da84 test kudoge 
	...
	function updateBaseData(uint256[9] memory _values) public onlyMarket {
		INVEST_MIN_AMOUNT = _values[0] * 10 ** 18;  //U
		UP_MIN_AMOUNT = _values[1] * 10 ** 18;    //绑定所需要最少BNB
		BASE_PERCENT = _values[2];  // 1%
	...
	function withdraw() public {
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < REFERRAL_PERCENTS.length; i++) {
				if (upline != address(0)) {
					uint256 amount = totalAmount.mul(REF_BASE_FEE).div(BASE_FEE).mul(REFERRAL_PERCENTS[i]).div(BASE_FEE);
					users[upline].bonus = users[upline].bonus.add(amount);
	...
	function getContractBalanceRate() public view returns (uint256) {
		uint256 contractBalance = usdt.balanceOf(address(this));
		uint256 contractBalancePercent = contractBalance.div(CONTRACT_BALANCE_STEP); //1
		return BASE_PERCENT.add(contractBalancePercent); //10+1
	}
	//用户速率
	function getUserPercentRate(address userAddress) public view returns (uint256) {
		User storage user = users[userAddress];

		uint256 contractBalanceRate = getContractBalanceRate(); //11
