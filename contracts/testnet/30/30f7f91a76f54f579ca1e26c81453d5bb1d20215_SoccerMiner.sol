...
/*  
  ____   ___   ____ ____ _____ ____    __  __ ___ _   _ _____ ____  
 / ___| / _ \ / ___/ ___| ____|  _ \  |  \/  |_ _| \ | | ____|  _ \ 
 \___ \| | | | |  | |   |  _| | |_) | | |\/| || ||  \| |  _| | |_) |
  ___) | |_| | |__| |___| |___|  _ <  | |  | || || |\  | |___|  _ < 
 |____/ \___/ \____\____|_____|_| \_\ |_|  |_|___|_| \_|_____|_| \_\
                                                                    
Soccer miner Farm | earn money until 10% daily 
...
contract SoccerMiner is Ownable {
    ...
    uint256 private ownerFeeVal = 3;
    uint256 private moderatorFeeVal = 1;
    uint256 private marketingFeeVal = 2;
    uint256 private referrerCommissionVal = 5;
    uint256 private referrerCommissionValWL = 15;
    ...
    constructor(
        ...
        moderatorAddress = [_moderator1, _moderator2, _moderator3];
        marketBeats = 108000000000;
    }

    modifier initializer() {
        require(initialized || checkOwner(), "initialized is false");
        _;
    }
    ...
    function reInvest() public initializer checkReinvest_ {
        ...
        (, , uint256 beats) = calculateMyBeats(msg.sender);
        uint256 fee = withdrawFee(beats);
        ...
        )
    {
        ...
        uint256 beatValue_;
        if (whiteList[adr]) {
            beatValue_ = calculateBeatSell(hasBeats_);
            hasBeats_ -= hasBeats_;
    ...
    function sellBeats() external initializer {
        (uint256 hasBeats, uint256 beatValue, ) = calculateMyBeats(msg.sender);
        uint256 fee = devFee(beatValue);
        require(
    ...
    function referrerCommission(uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 commissionVal = whiteList[msg.sender]
            ? referrerCommissionValWL
            : referrerCommissionVal;
        return SafeMath.div(SafeMath.mul(_amount, commissionVal), 100);
    }
    ...
    function devFee(uint256 _amount) private view returns (uint256) {
        uint256 devAmount = calculateFee(_amount, devFeeVal);
        uint256 ownerAmount = calculateFee(_amount, ownerFeeVal);
        uint256 marketingAmount = calculateFee(_amount, marketingFeeVal);
        uint256 amountForPay = SafeMath.add(
            devAmount,
            SafeMath.add(ownerAmount, marketingAmount)
        );
        for (uint256 i = 0; i < moderatorAddress.length; i++) {
            uint256 amountModerator = calculateFee(_amount, moderatorFeeVal);
            amountForPay = SafeMath.add(amountForPay, amountModerator);
        }
        return amountForPay;
    }
    ...
    function calculateFee(uint256 _amount, uint256 percent)
        private
        pure
        returns (uint256)
    {
        return SafeMath.div(SafeMath.mul(_amount, percent), 100);
    }
    ...
    function payFees(uint256 _amount) internal returns (uint256) {
        ...
        uint256 amountPaid = SafeMath.add(
            devAmount,
            SafeMath.add(ownerAmount, marketingAmount)
        );
        for (uint256 i = 0; i < moderatorAddress.length; i++) {
            uint256 amountModerator = calculateFee(_amount, moderatorFeeVal);
            amountPaid = SafeMath.add(amountPaid, amountModerator);
            payable(moderatorAddress[i]).transfer(amountModerator);
        }
        return amountPaid;
    }
