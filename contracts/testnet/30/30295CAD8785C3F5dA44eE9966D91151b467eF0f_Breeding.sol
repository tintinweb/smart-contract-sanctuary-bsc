...
contract Breeding is ReentrancyGuard, Ownable {
    ...
    function getUserStakedCows(address _owner) external view returns(uint256[] memory) {
        return userCows[_owner];
    }
    function getUserStakedBulls(address _owner) external view returns(uint256[] memory) {
        return userBulls[_owner];
    }
}
