...
interface IMetaSportsClub is IERC721 {
...
import "./interfaces/IMetaSportsClub.sol";
...
ReentrancyGuard
{
    ...
    uint256 public salePrice = 800000000000000000000;
    uint256 public mintPrice = 400000000000000000000;
    ...
    IMetaSportsClub public immutable MS_NFT;
    ...
    bool public isUSDTApproveNFT;
    bool public isUSDTApproveRouter;

    mapping(address => bool) public whiteList;
    mapping(uint256 => ClubInfo) clubs;
    mapping(uint256 => EnumerableSet.AddressSet) clubUserList;
    mapping(address => ClubUserInfo) clubUserInfo;
    mapping(uint256 => mapping(address => uint256)) rewardMap;
    ...
    constructor(address _collectionAddressNFT, address _collectionAddressToken, address _USDTContract, address _pancakeRouter) {
        require(
            _collectionAddressNFT != address(0),
            "NFT collection address can not be zero"
        );
        require(
            _collectionAddressToken != address(0),
            "Token collection address can not be zero"
        );
        MS_NFT = IMetaSportsClub(_collectionAddressNFT);
        MST_TOKEN = IMetaSportsToken(_collectionAddressToken);
    ...
    function createClub(
        ...
        if (isWhiteList) {
            require(whiteList[msg.sender], "No permission to create");
        }

        require(clubs[_clubId].clubOwner == address(0), "Club have created");
        require(_clubRewardRate > 1, "Club reward rate Out of range");
        require(_clubRewardRate < 100, "Club reward rate Out of range");
        ...
        emit CreateClub(_clubId, msg.sender, 100 - _clubRewardRate, _clubRewardRate);
    }

    function saveClub(
        uint256 _clubId,
        uint256 _clubRewardRate
    ) external whenNotPaused onlyRole(CLUB_ROLE) {
        require(_clubId > 0, "Illegal clubId");
        ...
        emit SaveClub(_clubId, msg.sender, 100 - _clubRewardRate, _clubRewardRate);
    }
    ...
    function openClub(
        ...
        require(
            clubs[_clubId].status == ClubStatus.Close,
            "Club status must be close"
        );
        clubs[_clubId].status = ClubStatus.Open;

        emit OpenClub(_clubId, msg.sender);
    }
    ...
    callerIsUser {
        ...
        require(
            _address != address(0),
            "Invite address can not be zero"
        );
        require(
        ...
        if (mintInfo.mintedAmount + _amount > supply)
            revert ExceedsRoundSaleSupply();
        if (mintInfo.startTime <= block.timestamp && mintInfo.endTime >= block.timestamp) {
            if (mintInfo.mintedAmount + _amount > mintInfo.maxBuyByDay)
                revert ExceedsAddressSaleSupply();
        }

        // aprove usdt to NFT and pancake router
        if(!isUSDTApproveNFT){
            isUSDTApproveNFT = USDT_TOKEN.approve(address(MS_NFT), MAX);
        }
        if(!isUSDTApproveRouter){
            isUSDTApproveRouter = USDT_TOKEN.approve(address(ROUTER_MST_USDT), MAX);
        }

        USDT_TOKEN.safeTransferFrom(msg.sender, address(this), salePrice * _amount);
        ...
        if (mintStatus) {
            ...
            MST_TOKEN.safeTransfer(clubUserInfo[_address].clubUser, inviteReward);

            rewardMap[clubInfo.clubId][clubInfo.clubOwner] = rewardMap[clubInfo.clubId][clubInfo.clubOwner] + clubReward;
            rewardMap[clubInfo.clubId][clubUserInfo[_address].clubUser] = rewardMap[clubInfo.clubId][clubUserInfo[_address].clubUser] + inviteReward;
        }

        emit Minted(msg.sender, clubInfo.clubId, _address, _amount, clubReward, inviteReward);
    }
    ...
    function claimReward(uint256 _clubId)
    external
    nonReentrant
    whenNotPaused {
        require(_clubId > 0, "Illegal clubId");
        require(clubs[_clubId].clubId == _clubId, "Club does not exist");
        require(rewardMap[_clubId][msg.sender] > 0, "No reward");

        uint256 reward = rewardMap[_clubId][msg.sender];

        MST_TOKEN.safeTransferFrom(address(this), msg.sender, reward);

        rewardMap[_clubId][msg.sender] = 0;
    }

    function _mintCommon(uint256 _amount) internal {
        // confirm tokenId
        uint256 totalSupply = MS_NFT.totalSupply();
        // mint
        MS_NFT.mint(_amount);
        // transfer user
        for (uint256 i = 0; i < _amount; i++) {
            uint256 tokenId = totalSupply + i + 1;
            MS_NFT.safeTransferFrom(address(this), msg.sender, tokenId);
        }
    }
    ...
    function getClubReward(uint256 _clubId)
    public
    view
    returns (
        uint256 reward
    )
    {
        ...
        return rewardMap[_clubId][msg.sender];
    }
