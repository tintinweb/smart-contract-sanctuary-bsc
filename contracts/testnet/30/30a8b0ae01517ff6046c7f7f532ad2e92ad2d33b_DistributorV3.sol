...
contract DistributorV3 is OwnableUpgradeable {
    ...
    function initialize ( address _treasury, address _sea, uint32 _epochLength, uint32 _nextEpochTime ) public initializer 
    {        
    ...
    function nextRewardAt( uint _rate ) public view returns ( uint ) {
        return SEA.totalSupply().mul( _rate ).div( 1000000 );
    }
    ...
    function setInfo( address _recipient, uint _rewardRate, uint _index) external onlyOwner {
    ...
    /**
        ...
        @param _recipient address
     */
    function removeRecipientEX( uint _index, address _recipient ) external onlyOwner {
        info[_index] = info[info.length-1];
        adjustments[_index] = adjustments[ info.length-1 ];
        info.pop();
        delete adjustments[ info.length-1 ];
