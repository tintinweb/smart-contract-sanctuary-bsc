...
contract BoxSale is BoxShop {
    uint256 public uintPrice;

    uint256 public buyCountLimit;
    uint256 public maxOnce = 10;
    ...
    mapping(uint256 => uint8) public shopCardCount;
    ...
    constructor(
        ...
        for (uint256 index1 = 0; index1 < cardCount.length; index1++) {
            shopCardCount[index1 + 1] = cardCount[index1];
        }
    }
    ...
    function buy(uint256 quantity) external {
        ...
        require(
            buyCount[msg.sender] + 1 <= buyCountLimit || buyCountLimit == 0,
            "Out of max"
        );
    ...
    function onOpenPackage(
        ...
        uint8 _count = shopCardCount[cardType];
        cardIdPre = new uint8[](_count);
        for (uint256 index = 0; index != shopCardCount[cardType]; index++) {
            cardIdPre[index] = shopCardType[cardType];
        }
        return cardIdPre;
    }
...
import "../access/Member.sol";
import "../constelle/NFTPackage.sol";
import "../token/CopperToken.sol";
import "../interface/IEMERouter.sol";

abstract contract BoxShop is IShop, Member {
    using SafeERC20 for IERC20;

    uint256 public quantityMax;
    uint256 public quantityCount = 0;
    address public tokenContract;
    uint256 public mintRatio = (3 * 1e6) / 100;
    ...
    function _buy(
        ...
        for (uint256 i = 0; i != quantity; i++) {
            NFTPackage(manager.members("package")).mint(
                to,
                tokenAmount,
                1,
                padding + i
