...
contract secso is IBEP20, ReentrancyGuard, ERC721Holder, Context, Ownable {
	using Address for address;
    ...
    // Pet rewards
    uint256 public PetRate = (20 * 10 ** _decimals) /  5 days;
    ...
    function petLifeRate(uint256 tokenId) public view returns(uint256) {
        uint256 timeLife = block.timestamp - tokenStakedAt[tokenId];
    ...
    function gameRewards(uint256 tokenId, uint256 cargo) external{
        ...
        if (cargo == 1) {
           ...
           _transfer(address(this), msg.sender, calculateTokens(tokenId));
            uint256 subt = petLifeRate(tokenId).mul(10 ** _decimals);
            life[tokenId] -= subt;
