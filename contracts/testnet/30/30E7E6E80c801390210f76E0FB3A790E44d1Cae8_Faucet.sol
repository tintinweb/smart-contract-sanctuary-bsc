...
contract Faucet is Ownable {
    ...
    // how many tokens to distribute per day for a single address
    uint public dailyRewardAmountPerAddress;
    
    // how many addresses are allowed to harvest per day
    uint public dailyUniqueAddressesCount;

    // how often users are allowed to harvest
    uint public harvestPeriod;

    // whether faucet is paused
    ...
    // user harvest info
    struct UserHarvest {
        uint harvestCount; // how many times user has already harvested today
        uint lastHarvestedAt; // last harvest timestamp
    }

    // whether user has already harvested in a single day
    mapping (uint => mapping (address => UserHarvest)) public dayAddressUserHarvest;

    // how many unique addresses have already harvested
    mapping (uint => uint) public dayUniqueAddressesCount;
     ...
     */
    function harvest(address _parentRefAddress) public {
        // get today's day index
        uint dayIndex = block.timestamp / 1 days;

        // get user harvest info
        UserHarvest storage userHarvest = dayAddressUserHarvest[dayIndex][msg.sender];
        
        // check that contract is not paused
        require(!isPaused, 'PAUSED');

        // check if it is time to harvest
        require(block.timestamp >= userHarvest.lastHarvestedAt + harvestPeriod, 'TIME_THRESHOLD_REACHED');
        
        // check if unique addresses threshold is not reached
        if (userHarvest.harvestCount == 0) {
            require(dayUniqueAddressesCount[dayIndex] + 1 <= dailyUniqueAddressesCount, 'MAX_DAILY_USERS_REACHED');
            // update daily unique addresses count who can harvest
            dayUniqueAddressesCount[dayIndex]++;
        }
        
        // update user's harvest info
        userHarvest.harvestCount++;
        userHarvest.lastHarvestedAt = block.timestamp;

        // calculate reward amount
        uint rewardAmount = dailyRewardAmountPerAddress / 1 days * harvestPeriod;

        // accrue referral interest
        ReferralProgram(referralProgramAddress).accrue(ReferralProgram.SourceRefContract.Faucet, rewardAmount, _parentRefAddress, msg.sender);
    ...
    /**
     * @notice Updates contract params by owner
     * @param _dailyRewardAmountPerAddress how many tokens to distribute per day for a single address
     * @param _dailyUniqueAddressesCount how many addresses are allowed to harvest per day
     * @param _harvestPeriod how often users are allowed to harvest
     * @param _isPaused whether faucet is paused
     * @param _rewardTokenAddress faucet reward token
     * @param _referralProgramAddress address of the referral program contract
     */
    function updateByOwner(
        uint _dailyRewardAmountPerAddress,
        uint _dailyUniqueAddressesCount,
        uint _harvestPeriod,
        bool _isPaused,
        ...
        onlyOwner
    {
        dailyRewardAmountPerAddress = _dailyRewardAmountPerAddress;
        dailyUniqueAddressesCount = _dailyUniqueAddressesCount;
