...
contract SINVERSE is Ownable {
  using Strings for uint256;
  using SafeMath for uint256;
  bool public paused = false;
  string public baseURI;
  IBEP20 public ERC20_TOKEN = IBEP20(0x263BcCfD546092816e9B50FE1D5808C7CD2a8134);
 ...
 mapping(uint256 => _bidDetail) public bidDetail;
  ...
  function walletOfOwner(IERC721Enumerable nftContract, address _owner) public view returns (uint256[] memory){
    uint256 ownerTokenCount = IERC721Enumerable(nftContract).balanceOf(_owner);
    uint256[] memory tokenIds = new uint256[](ownerTokenCount);
    for (uint256 i; i < ownerTokenCount; i++) {
      tokenIds[i] = IERC721Enumerable(nftContract).tokenOfOwnerByIndex(_owner, i);
    }
    return tokenIds;
  }
  ...
  function pause(bool _state) public onlyOwner {
    ...
    function putonSale(uint256 tokenId,uint256 salePrice, IERC721Enumerable nftContract, SellType sellType, uint256 auctionStart, uint256 auctionExpire) public {
        require(msg.sender == IERC721Enumerable(nftContract).ownerOf(tokenId), "Caller is not token owner");
        require(
            sellType == SellType.FIXED_PRICE || sellType == SellType.AUCTION,
            "Wrong sell type"
        ); 

        tokenDetails[tokenId].nftContract = nftContract; 
    ...
    function buy(uint256 tokenId, uint256 token) public {
        ...
        IERC721Enumerable(tokenDetails[tokenId].nftContract).safeTransferFrom(tokenDetails[tokenId].owner,msg.sender,tokenId);
  ...
  function acceptBid(uint256 tokenId) external {
    ...
    IERC721Enumerable(tokenDetails[tokenId].nftContract).safeTransferFrom(tokenDetails[tokenId].owner,bidDetail[tokenId].bidder,tokenId);
  ...
  function cancelOrder(uint256 tokenId)  external {
      require(msg.sender == IERC721Enumerable(tokenDetails[tokenId].nftContract).ownerOf(tokenId), "Caller is not token owner");
