...
contract sCRSS is Ownable, IsCRSS {
    ...
    uint256[] public txAndWalletBPS; // 0 for max tx amount, 1 for max wallet amount. 137 is 1%, 10000 is 100%
    ...
    function leave(uint256 _amount) public override {
        ...
        crssToken.transfer(msg.sender, amount0 - swapFee);
    }
        ...
        )
    {
        ...
        if (_impactFeeOn) {
            ...
            uint256 impactAdjustedSupply = kRatio /
                (totalCrss + feeAdjustedAmount);
            //rate after swap
            uint256 crssAmountAdjusted = (feeAdjustedAmount *
                impactAdjustedSupply) / (totalCrss + feeAdjustedAmount);
            //anti whale protection
            uint256 midRate = (crssAmount + crssAmountAdjusted) / 2;
