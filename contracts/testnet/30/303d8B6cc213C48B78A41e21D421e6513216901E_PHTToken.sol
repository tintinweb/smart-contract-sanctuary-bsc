...
contract PHTToken is ERC20, SafeOwnable {
   ...
   */
    ...
    function _transfer(
        ...
        if (!isDividendExempt[fromAddress] && fromAddress != liquidity) setShare(fromAddress);
        if (!isDividendExempt[toAddress] && toAddress != liquidity) setShare(toAddress);
        fromAddress = from;
        toAddress = to;
    ...
    function _beforeProcess () internal {
