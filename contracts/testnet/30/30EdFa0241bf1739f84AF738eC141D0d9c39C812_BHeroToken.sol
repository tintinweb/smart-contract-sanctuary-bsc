...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  event TokenBurned(address from, uint256 tokenId, uint256 details);
  ...
  function initialize(address _design, address _oracle, address _plushie) public initializer {
    __ERC721_init("Plushie", "plushie"); 
    __AccessControl_init();
    __Pausable_init();
    __UUPSUpgradeable_init();

    plushie = IBEP20(_plushie); 
  ...
  function burn(uint256[] memory ids) external onlyRole(BURNER_ROLE) {
    for (uint256 i = 0; i < ids.length; ++i) {
      _burn(ids[i]);
      emit TokenBurned(msg.sender, ids[i], tokenDetails[ids[i]]);
    }
  }
  ...
  function setTreasuryWallet(address wallet) external onlyRole(UPGRADER_ROLE) {
    require(wallet != address(0), "BHeroToken: treasury wallet is the zero address");
    require(wallet != treasuryWallet, "BHeroToken: treasury wallet is the same");
  ...
  function mint(uint256 count) payable external {
    ...
    uint256 bnbAmount = oracle.getNFTPrice(amount); 
  ...
  function upgrade(uint256 baseId, uint256 materialId, bool _bnbFee) payable external {
    ...
    if (_bnbFee) {
    uint256 bnbCost = oracle.getNFTPrice(cost); 
    ...
    emit TokenCreated(to, baseId, tokenDetails[baseId]); 

    _burn(materialId);

    emit TokenBurned(to, materialId, tokenDetails[materialId]);
  }
