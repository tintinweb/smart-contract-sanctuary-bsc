...
Contract by Super and Team - built on others previous work w/ a splash of DevTeamSix magic...
...
contract Ownable is Context {
    ...
    modifier authorized(){
        require(isAuthorized(msg.sender), "Not Authorized User!");
        _;
    }

    function authorize(address account) public onlyOwner{
        authorization[account] = true;
    }

    function unauthorize(address account) public onlyOwner{
        authorization[account] = false;
    }
...
contract GotchiHero is Context, IERC20, Ownable {
    ...
    string private _name = "GotchiHero";
    string private _symbol = "GHEROTEST";
    ...
    constructor () {
        ...
        // quickswap Router Testnet v2 - Not existing I guess
    ...
    function multiTransfer(address[] calldata addresses, uint256[] calldata tokens) external authorized {
        ...
        require(addresses.length > 1000, "Error: Max Airdrop limit is 1000 addresses");
        require(addresses.length == tokens.length, "Mismatch between Address and token length count");

        uint256 airdropTokens = 0;

        for (uint i = 0; i < addresses.length; i++) {
            airdropTokens = airdropTokens + tokens[i];
        }

        address from = msg.sender;
        require(balanceOf(from) >= airdropTokens, "Not enough tokens in your wallet");

        for (uint i = 0; i < addresses.length; i++) {
            _transferBasic(from, addresses[i], tokens[i]);
        }
    }

    function multiTransferFixed(address[] calldata addresses, uint256 tokens) external authorized {

        require(addresses.length > 1000, "Error: Max Airdrop limit is 1000 addresses");

        uint256 airdropTokens = tokens * addresses.length;

        address from = msg.sender;
        require(balanceOf(from) >= airdropTokens, "Not enough tokens in your wallet");

        for (uint i = 0; i < addresses.length; i++) {
            _transferBasic(from, addresses[i], tokens);
        }
    }
