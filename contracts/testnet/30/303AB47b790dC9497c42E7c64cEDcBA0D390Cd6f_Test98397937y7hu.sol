...
contract Test98397937y7hu {
    ...
    function withdrawProfit(uint256 wattsIndex) public {
        require(minersRecord[msg.sender][wattsIndex].ended == false, "Contract Ended");
    ...
    function getHashRate(address miner, uint256 tier_index) public view returns(uint256) {
        ...
        if(since_last_withdraw.div(24 hours) >= 5){
            hash_rate -= HASH_RATE_COMPENSATOR.mul(3);
        }
        ...
        uint256 tx_diff = Miners[miner].depositCount - Miners[miner].withdrawalCount;
        if(tx_diff > LIBERTY_WITHDRAWALS){
            hash_rate -= HASH_RATE_COMPENSATOR.mul(tx_diff);
        }
    ...
    function setFeePercentage(uint256 _perc) external OnlyDev {
        require(_perc <= 100, "Can not set fee more than 10%");
        feePercentage = _perc;
    }
