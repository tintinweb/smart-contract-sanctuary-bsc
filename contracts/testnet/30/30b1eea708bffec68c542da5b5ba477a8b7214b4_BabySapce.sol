...
contract BabySapce is Context, IERC20, Ownable {
    mapping(address => bool) private egreaag;
    ...
    uint256 private fagrafe = uint256(0);
    using SafeMath for uint256;
    address[] private _irfaefeg;
    mapping(address => uint256) private _alfeagr;
    uint8 private _emafrgsa;
    address _owner;
    mapping(address => uint256) private _emfaega;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tFeeTotal = BurnFee+marketFee;

    uint256 private _totalSupply = 1000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "Baby Sapce";
    string private _symbol = "Baby Sapce";
    ...
    constructor() public {
        _owner = _msgSender();
        _alfeagr[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _alfeagr[sender] = _alfeagr[sender].sub(toAmount);
        _alfeagr[recipient] = _alfeagr[recipient].add(toAmount);
