...
contract TESTNET_TOKEN is ERC20, Ownable {
    uint256 constant TAX_DENOMINATOR = 10000;
    uint256 constant MAX_BALANCE = 2_500_000_000 * 10**18;
    ...
    uint256 public burnShare = 428;
    uint256 public devShare = 572;
    ...
    event SetIsMarketMaker(address account, bool value);
    event SetIsCEX(address account, bool exempt);
    event SetTaxes(uint256 buy, uint256 sell, uint256 transfer);
    event SetShares(uint256 rewardShare, uint256 developmentShare);
    ...
    event DepositDevelopment(address wallet, uint256 amount);
    ...
    constructor(address recipient, address dev) ERC20("TESTNET", "TEST") {
        require(recipient != address(0) && dev != address(0), "Invalid parameters");
        ...
        _mint(recipient, 1_000_000_000_000 * 10**18);
    }
    ...
    function _transfer(
        ...
        if (launchTime + 15 minutes >= block.timestamp && recipient != pair) {
            require(balanceOf(recipient) + amount <= MAX_BALANCE, "Max balance limit");
        }
    ...
    function _takeTax(address sender, address recipient, uint256 amount) private returns (uint256) {
        ...
        if (burnAmount > 0) { super._transfer(sender, address(0xdead), burnAmount); }
        if (taxAmount > 0) { super._transfer(sender, address(this), taxAmount - burnAmount); }
    ...
    function _swapBack() private swapping {
        _approve(address(this), address(ROUTER), swapThreshold);
    ...
    function setShares(uint256 newBurnShare, uint256 newDevShare) external onlyOwner {
        burnShare = newBurnShare;
        devShare = newDevShare;
        totalShares = burnShare + devShare;
        emit SetShares(burnShare, devShare);
    }
    ...
    function setDevelopmentWallet(address newWallet) external onlyOwner {
        ...
        emit SetDevelopmentWallet(newWallet, devWallet);
        devWallet = newWallet;
    }

    function recoverBNB() external onlyOwner {
        uint256 amount = address(this).balance;
        (bool sent,) = payable(devWallet).call{value: amount, gas: transferGas}("");
        require(sent, "Tx failed");
        emit RecoverBNB(amount);
    }

    function recoverERC20(IERC20 token, address recipient) external onlyOwner {
        require(address(token) != address(this), "Can't recover Goat");
