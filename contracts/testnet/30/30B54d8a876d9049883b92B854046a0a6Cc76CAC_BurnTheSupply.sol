...
contract BurnTheSupply is Context, IERC20 {
    ...
    address public routerAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // TESTNET BSC
    ...
    // Number of transactions before tax
    ...
    constructor() {
        ...
        _maxWalletToken = (_totalSupply * 3) / 100;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (!TradeOpen){
            require(_preLaunchAccess[sender] || _preLaunchAccess[recipient], "Trade is not open yet, please come back later");
        }
        ...
        // Limit wallet total
        if (!_isLimitExempt[recipient] && sender != owner()){
        ...
        if(txCount >= swapTrigger && !inSwapAndLiquify && !_isPair[sender] && swapAndLiquifyEnabled && block.number > swapBlock){  
        ...
        bool takeFee = true;
        bool isBuy;
        if(_isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || (noFeeToTransfer && !_isPair[sender] && !_isPair[recipient])){
            takeFee = false;
        } else {
            // Buy or Sell Tax
            if(_isPair[sender]) isBuy = true;
            txCount++;
        }
        _tokenTransfer(sender, recipient, amount, takeFee, isBuy);
    }
    ...
    /**
     * @dev update_name_symbol
     */
    event updateNameSymbol(string newName, string newSymbol);
    function update_name_symbol(string memory newName, string memory newSymbol) external onlyOwner() {  
        _name = newName;
        _symbol = newSymbol;
        emit updateNameSymbol(newName, newSymbol);
    }
    ...
    function Wallet_Update_Dev(address payable wallet) external onlyOwner() {
        ...
        _isExcludedFromFee[Wallet_Dev] = false; // Update mapping on old wallet
        Wallet_Dev = wallet;        
        _isExcludedFromFee[Wallet_Dev] = true;  // Update mapping on new wallet
    }
    ...
    function Wallet_Update_TerraLunaBurn(address payable wallet) external onlyOwner() {
        require(wallet != address(0), "new wallet is the zero address");
        emit updatedTerraLunaBurnWallet(Wallet_Terra_Luna_Burn, wallet);
        Wallet_Terra_Luna_Burn = wallet;
    }
    ...
    function Wallet_Update_LP(address payable wallet) external onlyOwner() {
        require(wallet != address(0), "new wallet is the zero address");
    ...
    function openTrade() external onlyOwner() {
        require(_Tax_Buy == 6 && _Tax_Sell == 6, "Trading open conditions are not met!");
        require(Percent_Terra_Luna_Burn == 50 && Percent_Dev == 33 && Percent_AutoLP == 17, "Trading open conditions are not met!");
        TradeOpen = true;
        emit open_trade();
    }
     ...
     */
    event setting_fees(uint256 dev_wallet_fee, uint256 auto_liquidity_fee, uint256 terrlunaburn_wallet_fee);
    function _set_Fee_Percent(uint256 dev_wallet_fee, uint256 auto_liquidity_fee, uint256 terrlunaburn_wallet_fee) external onlyOwner() {
        ...
        emit setting_fees(dev_wallet_fee, auto_liquidity_fee, terrlunaburn_wallet_fee);
    }
    ...
    /**
     * @dev Blacklist Switch.
     * Turn on/off blacklisted wallet restrictions 
     */
    event blacklisted_wallets_blocked(bool true_or_false);
    function blacklist_Switch(bool true_or_false) external onlyOwner {
        noBlackList = true_or_false;
        emit blacklisted_wallets_blocked(true_or_false);
    } 

    /**
     * @dev block wallet (can't sell or transfer tokens)
     */
    event blacklistAddWallets(address wallet_address);
    function blacklist_Add_Wallets(address wallet_address) external onlyOwner {
        ...
        emit blacklistAddWallets(wallet_address);
    }

    /**
     * @dev Blacklist - remove from blacklist
     */
    event blacklistRemoveWallets(address wallet_address);
    function blacklist_Remove_Wallets(address wallet_address) external onlyOwner {
        require (wallet_address != address(0), "Wallet_Address is the zero address");
        if(_isBlacklisted[wallet_address]) _isBlacklisted[wallet_address] = false;
        emit blacklistRemoveWallets(wallet_address);
    }
    ...
    function excluded_From_Fee(address Wallet_Address, bool true_or_false) external onlyOwner {
        require (Wallet_Address != address(0), "Wallet_Address is the zero address");
     ...
     */
    event setRouterAddressPair(address new_router_address, address new_pair_address);
    function set_New_Router_and_Make_Pair(address newRouter) external onlyOwner() {
        ...
        uniswapV2Router = _uniswapV2Router;
        _isPair[uniswapV2Pair] = true;
        emit setRouterAddressPair(newRouter, uniswapV2Pair);
    }

    /**
     * @dev Set New router address
     */
    event setRouterAddress(address new_router_address);
    function set_New_Router_Address(address newRouter) external onlyOwner() {
        require (newRouter != address(0), "newRouter is the zero address");
        IUniswapV2Router02 _newPCSRouter = IUniswapV2Router02(newRouter);
        uniswapV2Router = _newPCSRouter;
        emit setRouterAddress(newRouter);
    }

    /**
     * @dev Set new pair
     */
    event setNewPairAddress(address newPair);
    function set_New_Pair_Address(address newPair) external onlyOwner() {
        require (newPair != address(0), "newPair is the zero address");
        _isPair[uniswapV2Pair] = false;
        uniswapV2Pair = newPair;
        _isPair[uniswapV2Pair] = true;
        emit setNewPairAddress(newPair);
    }

    /**
     * @dev set the number of transactions required before the 'swapAndLiquify' function triggers
     */
    event TokenTriggerNumberUpdate(uint8 number_of_transactions);
    function set_Number_Of_Tx_Before_Liquify_Trigger(uint8 number_of_transactions) external onlyOwner {
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of WBNB (amountOutMin)
            path,
            address(this),
            block.timestamp
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 BNBAmount) private {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        uniswapV2Router.addLiquidityETH{value: BNBAmount}( // BNBAmount: amountETHDesired
            address(this),
            tokenAmount,  // amountTokenDesired
            0, // amountTokenMin  Must be <= amountTokenDesired.
            0, // amountETHMin    Must be <= msg.value (BNBAmount)
            Wallet_LP, 
            block.timestamp
        );
    } 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if(contractTokenBalance > 0){
            ...
            uint256 split_Luna = (Percent_Terra_Luna_Burn * 100) / (Percent_Terra_Luna_Burn + Percent_Dev);
            uint256 BNB_to_Luna = (BNB_Total * split_Luna) / 100;
            ...
            if(Percent_AutoLP != 0){
                // 1/2 Auto Liquidity
                uint256 tokens_to_LP_Half = tokens_to_LP / 2;
            ...
            _balances[address(this)] = _balances[address(this)] - contractTokenBalance;
        }
    }
     ...
     */
    event processSwapAndLiquifyNow(uint256 _percent_Of_Tokens_To_Liquify);
    function process_SwapAndLiquify_Now (uint256 percent_Of_Tokens_To_Liquify) external onlyOwner {
        ...
        emit processSwapAndLiquifyNow(percent_Of_Tokens_To_Liquify);
    }
}
