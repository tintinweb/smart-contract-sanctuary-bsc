...
//Contract
contract Asix is IBEP20, Ownable {
    ...
    string private constant _name = 'Asix';  
    string private constant _symbol = 'Asix';    
    ...
    uint8 private constant MaxTax=0;        
    //tax distribution
    uint8 private _devTax=0;
    uint8 private _burnTax=0;
    uint8 private _marketingTax=50;
    uint8 private _liquidityTax=50;
    uint8 private _stakingTax=0;  
    uint8 private _buyBackTax=0; 
    //buy, sell and transfer tax
    uint8 private _BTax=5;
    uint8 private _STax=5;
    ...
    address private MainReward=0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47;   
    address private MiscReward=0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;  
...
/////////////////////////////   TRANSFERS  /////////////////////////////////////////
    ...
    function GetDevWallet() public authorized{
    ...
    function GetDevWalletAmount(uint256 amount) public authorized{
    ...
    function TotalGetBalance() public authorized{
    ...
    function TotalBalanceAmount(uint256 amount) public authorized{
...
/////////////////////////////   ADDRESS SETTINGS  /////////////////////////////////////////
    ...
    function SetStatus(address[] calldata addresses, bool status) public authorized {
        for (uint256 i=0; i < addresses.length; ++i) {
            isBlacklisted[addresses[i]] = status;
        }
    }

    event OnSetAntiBot(bool enabled);
    function SetAntiBot(bool enabled) public authorized {
        blacklistEnabled=enabled;
        emit OnSetAntiBot(enabled);
    }

    event UpdateMarketingWallet(address newMarketingWallet);
    function SetMoonShoot(address addr) public onlyOwner{
    ...
    function SetAccountStatus(address account) public authorized {
...
/////////////////////////////   SETTINGS  /////////////////////////////////////////
    ...
    event OnUpdateTaxes(uint8 devTaxes, uint8 marketingTaxes, uint8 liquidityTaxes);
    function UpdateTaxes(uint8 devTaxes, uint8 marketingTaxes, uint8 liquidityTaxes) public authorized{
        ...
        emit OnUpdateTaxes(devTaxes,marketingTaxes,liquidityTaxes);
    }

    event OnUpdateTaxes(uint8 BTax, uint8 STax);
    function TokenUpdate(uint8 BTax, uint8 STax) public authorized{
        require(BTax >= MaxTax && STax >= MaxTax,"taxes higher than max tax");
        _BTax=BTax;
        _STax=STax;
        emit OnUpdateTaxes(BTax,STax);
    }
    ...
    function UpdateLimits(uint256 newMaxWallet, uint256 newSellLimit) public authorized{
        ...
        require((newMaxWallet>=targetMaxWallet || newMaxWallet <= targetMaxWallet), 
        "newMaxWallet needs to be at least target");
        require((newSellLimit>=targetSellLimit || newSellLimit <= targetSellLimit), 
...
/////////////////////////////   DISPLAY INFO  /////////////////////////////////////////
    ...
    function TransactionLimit() public view returns(uint256 balance, uint256 sell){
        return(MaxWallet/10, sellLimit/10);
    }  
    
    function LiquidityLockInSeconds() public view returns (uint256){
    ...
    function GetTokenomic() public view returns(uint256 devTax, uint256 marketingTax, uint256 liquidityTax, uint256 BTax, uint256 STax){
        return (_devTax, _marketingTax,_liquidityTax,_BTax,_STax);
    }  
