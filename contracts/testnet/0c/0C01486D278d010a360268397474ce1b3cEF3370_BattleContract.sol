...
contract BillionsNFT is ERC4907, BillionsBaseContract, ReentrancyGuard {
    ...
    function setSubURI(uint256 _type, string memory _uri) public onlyOwner {
        subURI[_type] = _uri;
    }
    ...
    function tokenURI(uint256 _nftId) public view virtual override returns (string memory uri) {
        ...
        uri = bytes(_baseURI).length > 0 ? string(abi.encodePacked(_baseURI, subURI[nft.nftType], "/", Strings.toString(nft.symbolId))) : "";
    }
