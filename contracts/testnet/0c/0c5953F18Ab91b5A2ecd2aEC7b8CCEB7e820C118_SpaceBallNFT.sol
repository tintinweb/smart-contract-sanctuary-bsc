...
interface VerifyContract {
    function verifyOpenBox(bytes memory data)
        external
        returns (
            string memory,
            address,
...
contract SpaceBallNFT is ERC721URIStorage, Member {
    ...
    BoxContract boxAddress;
    mapping(uint256 => bool) isBurnBox;
    ...
    constructor(address _verify, address boxNFT)
        ERC721("SpaceBallBox", "SpaceBallBox")
    {
        verify = VerifyContract(_verify);
        boxAddress = BoxContract(boxNFT);
    }

    function OpenBox(bytes memory data) public {
        (string memory tokenURI, address user, uint256[3] memory types) = verify
            .verifyOpenBox(data);
        require(!isBurnBox[types[2]], "This box was burn");
        boxAddress.burnNFT(types[2]);
        isBurnBox[types[2]] = true;

        uint256 tokenId = awardItem(user, tokenURI);
        cards[tokenId] = CardInfo(types[0], types[1]);
        emit OpenBoxEven(user, tokenId, types[1], types[0], types[2]);
    }
    ...
    function getCardInfo(uint tokenId)public view returns(CardInfo memory){
