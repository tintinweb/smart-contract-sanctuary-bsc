...
contract Treasury is UniswapCommon{
    constructor () public {}
    //添加流动性折损率(0为不折损 因有几率添加失败 故设置折损率)
    uint8 public _addLpWreck=5;
    //超级合约索取数量记录
    mapping(address=>uint256) public _claimInfos;
    function changeWreck(uint8 addLpWreck) external onlyOwner{
        require(addLpWreck<=100&&addLpWreck>=0);
        _addLpWreck=addLpWreck;
    }
    //超级合约索取代币给用户
    ...
    //添加流动性
    function addLiquidityByPay(uint256 payAmount) external onlySuperContract{
        require(payAmount>0);
        address _dol=Super(_super)._contract("dol");
        ...
        IBEP20(_pay).approve(address(uniswapV2Router),2**256-1);
        IBEP20(_dol).approve(address(uniswapV2Router),2**256-1);
        uniswapV2Router.addLiquidity(_pay,_dol,payAmount,dolAmount,payAmount,dolAmount.mul(100-_addLpWreck).div(100),address(this),block.timestamp);
    }
}
