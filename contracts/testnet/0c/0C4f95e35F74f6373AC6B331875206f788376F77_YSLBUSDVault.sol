...
contract YSLBUSDVault is Initializable, ReentrancyGuard, Pausable, IEvents {
    ...
    */
     ...
     */
    function deposit(address user, uint amount,bool isBUSD) nonReentrant external _lastExchangeRatio _completePause whenNotPaused {
        ...
        if(_isContract(msg.sender)){
            require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender),"YSLBUSDVault: No external contract interaction");
        }
        ...
        uint USDyLiquidity = (IERC20(Admin.USDy()).balanceOf(Admin.USDyBUSD()) * (amount * 1/2)/ IERC20(BUSD).balanceOf(Admin.USDyBUSD()));
        ...
        if (IERC20(Admin.YSL()).balanceOf(address(this)) >= YSLRequire) {
            ...
            if (
                IERC20(Admin.YSL()).balanceOf(address(this)) <=
                ...
                IReceipt(Admin.YSL()).burn(
                    address(this),
                    IERC20(Admin.YSL()).balanceOf(address(this))
                );
            }
        }
        _convertBUSDToLP((amount * 1/2), USDyLiquidity,BUSD, Admin.USDy());
        ...
        exchangeRatio = IERC20(Admin.USDy()).balanceOf(address(this)) * (10 ** 18)/(IERC20(yslBUSD_A).totalSupply());
    ...
    */
    ...
    function emergencyWithdraw() external{
        ...
        console.log(UserDeposit[msg.sender] > IERC20(Admin.USDy()).balanceOf(address(this)),UserDeposit[msg.sender],IERC20(Admin.USDy()).balanceOf(address(this)));
        console.log("msg.sender",msg.sender);
        _withdraw(msg.sender, UserDeposit[msg.sender], msg.sender);
    }

    function setyslBUSD(address _yslBUSD) external _isAdmin {
        require(_yslBUSD != address(0), "YSL-BUSD: invalid data");
        yslBUSD = _yslBUSD;
    }
    ...
    */
    ...
    function claimRebaseReward(address user) public nonReentrant whenNotPaused _completePause{
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "YSLBUSDVault: address is Blacklisted");
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "YSLBUSDVault: you can't interact in same block");
            require(!_isContract(msg.sender),"YSLBUSDVault: No external contract interaction");
        }
        _claimRebaseReward(user);
    }
    ...
    */
    function rewards(address user) public view returns (uint reward){
       reward = ((UserDeposit[user] * (currentReward - ss[user])) / 10 ** 18);
         if(Admin.PhoenixNFT() != address(0) && IReceipt(Admin.PhoenixNFT()).balanceOf(user) >= 1){
    ...
    */
    function _withdraw(address user,uint _amount,address sendTo) nonReentrant internal _lastExchangeRatio _completePause{
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "YSLBUSDVault: address is Blacklisted");
        ...
        console.log(_amount,UserDeposit[user],UserDeposit[user] == _amount);
    ...
    function _claimRebaseReward(address user) internal {
        uint reward = ((UserDeposit[user] * (currentReward - ss[user])) / 10 ** 18);
        if(msg.sender != address(this)){
        require(reward > 0,"YSLBUSDVault: Please claim after rebase");
        }
        if(Admin.PhoenixNFT() != address(0) && IReceipt(Admin.PhoenixNFT()).balanceOf(user) >= 1){
