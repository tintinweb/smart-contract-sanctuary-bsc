...
contract Marketplace is Ownable {
    ...
    constructor() {
        ...
        weth = IERC20(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
        platformFee = 0;
    }
    ...
    */
    function OfferAndMint(tuple memory values, string memory uri, uint8 v, bytes32 r, bytes32 s, IERC721 contractAddress) public payable returns (uint256) {
        require(weth.allowance(values._bidder, address(this)) >= values._amount, "not enought balaance");
        require(weth.balanceOf(values._bidder) >= values._amount, "not enought balaance");

        IERC721 Nft = IERC721(contractAddress);

        // require(deadline >= block.timestamp, "deadline passed");
        // require(values._seller == ecrecover(getSignedHash(keccak256(abi.encodePacked(this, values._seller , amount))), v, r, s), "Owner signature is Invalid");

        require(values._bidder == ecrecover(getSignedHash(keccak256(abi.encodePacked(this, values._bidder, values._amount))), v, r, s), "Bidder signature is Invalid");
    ...
    function AcceptBidAndMint(tuple memory values, string memory uri, uint256 deadline, uint8 sv, bytes32 sr, bytes32 ss,  uint8 bv, bytes32 br, bytes32 bs, IERC721 contractAddress) public payable returns (uint256) {
        require(weth.allowance(values._bidder, address(this)) >= values._amount, "not enought balaance");
        require(weth.balanceOf(values._bidder) >= values._amount, "not enought balaance");
    ...
    function BuyAndMint(tuple memory values, string memory uri, uint8 sv, bytes32 sr, bytes32 ss,  uint8 bv, bytes32 br, bytes32 bs, IERC721 contractAddress) public payable returns (uint256) {
        ...
        require(values._bidder == ecrecover(getSignedHash(keccak256(abi.encodePacked(this, values._bidder , values._amount))), bv, br, bs), "Buyer signature is Invalid");
         ...
         processEthPayments(values._bidder, values._amount);
    ...
    function OfferAndTransfer(tuple memory values, uint8 sv, bytes32 sr, bytes32 ss,  uint8 bv, bytes32 br, bytes32 bs, IERC721 contractAddress) public payable returns (uint256) {
    ...
    function BuyAndTransfer(address payable _seller, address payable _buyer, uint256 _amount, uint256 tokenId, uint8 sv, bytes32 sr, bytes32 ss,  uint8 bv, bytes32 br, bytes32 bs, IERC721 contractAddress) public payable returns (uint256) {
        ...
        require(_seller == ecrecover(getSignedHash(keccak256(abi.encodePacked(this, _seller , _amount))), sv, sr, ss), "Seller signature is Invalid");

        require(_buyer == ecrecover(getSignedHash(keccak256(abi.encodePacked(this, _buyer , _amount))), bv, br, bs), "Buyer signature is Invalid");

        processEthPayments(_buyer, _amount);

        Nft.safeTransferFrom(_seller, _buyer, tokenId);
        emit Buyandtransfer(_seller, _buyer, _amount ,tokenId, contractAddress);
    ...
    function processEthPayments(address _buyer, uint256 amount) internal{
        uint256 _fee = platformFee.mul(amount).div(100);
        payable(feeWallet).transfer(_fee);
        payable(_buyer).transfer(amount.sub(_fee));
    }

    function signVerification (address seller, address buyer, uint256 deadline, uint256 amount, uint8 sv, bytes32 sr, bytes32 ss,  uint8 bv, bytes32 br, bytes32 bs) internal view returns (bool){
