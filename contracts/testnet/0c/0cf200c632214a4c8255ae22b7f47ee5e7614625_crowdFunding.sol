...
contract crowdFunding is Ownable {
    ...
    // monthly widthdraw
    function monthlyWithdraw(uint256 _amount) public {
        require(_amount > 0, "amount should be greater than 0");
        require(
            userInvestment[msg.sender].timeperiod + 2 minutes <
                block.timestamp,
    ...
    // reInvest
    function reInvestMonthly(uint _amount) public {
    ...
    function updateROIWeekly(uint256 _per, bool pl) public onlyOwner {
        if (pl) {
            // roi.per=_per;
            uint256 length = investedAddresses.length;
            for (uint256 i = 0; i < length; i++) {
    ...
    // weekly widthdraw
    function weeklyWithdraw() public {
        require(
            userInvestment[msg.sender].timeperiod + 2 minutes >=
                block.timestamp,
    ...
    // // weekly Reinvest
    // function weeklyReInvest() public {
    //       require(
    //         userInvestment[msg.sender].invested,
    //         "Not Invested"
    //     );
    //       require(
    //         userInvestment[msg.sender].timeperiod + 10 minutes >=
    //             block.timestamp,
    //         "Your time period is over please widthdraw your Invested Amount or reinvest it."
    //     );
    //     require(withdrawl,"withdrawl is off currently contact admin");
    ...
    //     userInvestment[msg.sender] = Investment(
    //         userInvestment[msg.sender].amount +
    //             ((userInvestment[msg.sender].amount *userInvestment[msg.sender].profit) /
    //             100 )-
    //            ( (userInvestment[msg.sender].amount *
    //                 userInvestment[msg.sender].loss) /
    //             100),
    //         0,
    //         0,
    //         block.timestamp,
