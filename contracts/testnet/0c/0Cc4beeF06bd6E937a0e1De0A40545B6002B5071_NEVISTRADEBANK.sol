...
contract NEVISTRADEBANK  is ERC20 {
    ...
    uint256 constant public TOKEN_FEE = 5500;
	...
	function invest(uint8 plan) external payable {
       ...
       // token.transferFrom(address(msg.sender),address(this), msg.value);
        ...
        uint256 purchase_amount= msg.value.mul(TOKEN_FEE).div(PERCENTS_DIVIDER);

        buy_token(purchase_amount);

        uint256 purchase_TOKEN= msg.value.mul(OTHER_FEE).div(PERCENTS_DIVIDER);
    ...
    function buy_token(uint256 tokenAmount) private{
      ...
      function add_liquidity(uint256 tokenAmount,uint256 amount) private{
		...
		_approve(address(this), address(NevisSwapRouter), tokenAmount);
	     ...
	     NevisSwapRouter.addLiquidityETH{value: tokenAmount}(
                NEVIS_TOKEN,
                amount,
                0,
                0,
