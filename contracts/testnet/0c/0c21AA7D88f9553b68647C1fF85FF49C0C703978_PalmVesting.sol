...
contract PalmVesting is Ownable {
    ...
    struct VestingInfo {
        uint64 timeFromTge; // First release time from TGE
        uint64 tgePct; // First release percentage
        uint64 period; // Vesting period after first release
    }
    ...
    uint8 public lastCategory = 6;

    /// @dev palm token address
    IERC20 public immutable palmToken;
    ...
    /// @dev total vesting amount
    uint256 public totalVestingAmount;
    /// @dev claimed amount from vesting
        ...
        onlyValidCategory(category)
    {
        ...
        if (vestingInfo.tgePct != DENOMINATOR && vestingInfo.period == 0) {
    ...
    function setAmount(
        ...
        uint256 previousAmount = amounts[category][user];
        amounts[category][user] = amount;
        totalVestingAmount = totalVestingAmount + amount - previousAmount;
    ...
    /// @dev recover unnecessary PALM token
    function recoverPalm() external onlyOwner {
        uint256 palmBalance = palmToken.balanceOf(address(this));
        uint256 minimumAmountRequired = totalVestingAmount - totalClaimedAmount;
        if (palmBalance <= minimumAmountRequired) {
            revert Errors.NothingToRecover();
        }
        palmToken.safeTransfer(msg.sender, palmBalance - minimumAmountRequired);
    }
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp < firstReleaseTime || totalAmount == 0) {
            return 0;
        }
        ...
        uint64 elapsed = uint64(block.timestamp) - firstReleaseTime;

        uint256 vestedAmount = (totalVestedAmount * elapsed) /
            vestingInfo.period +
            tgeAmount;
    ...
    function claim(uint8 category, bool revertForZero) public {
        ...
        if (availableAmount != 0) {
            ...
            palmToken.safeTransfer(msg.sender, availableAmount);
