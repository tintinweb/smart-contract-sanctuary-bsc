...
contract PlearnNFTAirdrop is ERC721Enumerable, ERC721Pausable, ERC721Burnable, AccessControl, Ownable {
    ...
    constructor(uint256 maxSupply_) ERC721("Plearn Airdrop Collection", "PLNA") {
        require(maxSupply_ > 0, "Plearn NFT: Wrong max supply");
    ...
    function mint() external {
        ...
        require(tokenId > 0, "Plearn NFT: You has no right to receive airdrop");
    ...
    function canGetAirdrop(address user) public view returns (bool) {
        return tokenIds[user] > 0;
    }

    function addUser(address user, uint256 tokenId) external onlyRole(EXECUTOR_ROLE) {
        if (tokenIds[user] == 0) {
            tokenIds[user] = tokenId;
        }
    }

    function addUserBatch(address[] calldata users_, uint256[] calldata tokenIds_) external onlyRole(EXECUTOR_ROLE) {
        require(users_.length == tokenIds_.length, "Plearn NFT: Not same length");
    ...
    function updateUser(address user, uint256 tokenId) external onlyRole(EXECUTOR_ROLE) {
        tokenIds[user] = tokenId;
    }

    function removeUser(address user) external onlyRole(EXECUTOR_ROLE) {
        delete tokenIds[user];
    }
