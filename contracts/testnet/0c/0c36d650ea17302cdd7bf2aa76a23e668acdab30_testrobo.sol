...
contract testrobo {
	...
	function invest(address referrer) public payable {
        ...
        if(msg.value >= 11*10**17 && msg.value <= 3*10**18) {
             plan = 1;
        }
		if(msg.value >= 31*10**17 && msg.value <= 6*10**18) {
             plan = 2;
        }
		if(msg.value >= 61*10**17 && msg.value <= 30*10**18) {
             plan = 3;
        }
        if(msg.value >= 301*10**17) {
            plan = 4;
        }
	...
	function migrate(address useradd, address referrer, uint256 totaldeposit, uint8 planno, uint256 lastday, uint256 olddividends, uint256 oldpassive, uint256 oldrefbonus, uint256 oldwithdrawn) public {
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 5; i++) {
				if (upline != address(0)) {
					uint256 amount = totaldeposit.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
					users[upline].bonus = users[upline].bonus.add(amount);
					users[upline].totalBonus = users[upline].totalBonus.add(amount);
					emit RefBonus(upline, useradd, i, amount);
