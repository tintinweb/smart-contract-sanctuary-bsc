...
interface IStaking{
    ...
    function _stakeTimes() external view returns(uint);
    function _claimTimes() external view returns(uint);
    function _userNum() external view returns(uint);
    function _stakingTotal() external view returns(uint);
    function _interestTotal() external view returns(uint);
    function _breachRatio() external view returns(uint);
    function _stakeCharge() external view returns(uint);
    function _maxBuyAmountEveryDay() external view returns(uint);
    function _breachOpen() external view returns(bool);
    function _o2() external view returns(address);
    function _rbl() external view returns(address);
    function _recipientRblAddress() external view returns(address);
    function getStakes(address account,uint256 index,uint256 offset) external view returns(stakeInfo[] memory);
    function getClaims(address account,uint256 index,uint256 offset) external view returns(claimInfo[] memory);
    function getUnstakes(address account,uint index,uint offset) external view returns(unStakeInfo[] memory);
    function _stakeInfos(uint stakeId) external view returns(stakeInfo memory);
    function _userClaimInfos(address account,uint index) external view returns(claimInfo memory);
    function _users(uint userId) external view returns(address);
    function _userStakeList(address account,uint index) external view returns(uint);
    function userStakeNum(address account) external view returns (uint);
    function _userStakeTotal(address account) external view returns (uint);
    function _userInterestTotal(address account) external view returns (uint);
    function _userIds(address account) external view returns (uint);
    function _userUnStakeInfos(address account,uint index) external view returns(unStakeInfo memory);
    function _buyAmountEveryDay(uint day) external view returns (uint);
}
...
contract o2Staking is Context, Ownable {
    ...
    //质押手续费比例(每天)
    uint256 public _stakeCharge=10;//+
    //每日购买限量
    ...
    //允许质押的代币合约
    mapping(address=>bool) public _allowTokens;//
    ...
    bool public _syncFirstStatus;
    mapping(address=>bool) public _syncUserStatus;
    mapping(uint=>bool) public _syncStakeStatus;
    //利率列表
    ...
    function syncFirst(address token) public onlyOwner{
        _stakeTimes=IStaking(token)._stakeTimes();
        _claimTimes=IStaking(token)._claimTimes();
        _userNum=IStaking(token)._userNum();
        _stakingTotal=IStaking(token)._stakingTotal();
        _interestTotal=IStaking(token)._interestTotal();
        _breachRatio=IStaking(token)._breachRatio();
        _stakeCharge=IStaking(token)._stakeCharge();
        _maxBuyAmountEveryDay=IStaking(token)._maxBuyAmountEveryDay();
        _breachOpen=IStaking(token)._breachOpen();
        _o2=IStaking(token)._o2();
        _rbl=IStaking(token)._rbl();
        _recipientRblAddress=IStaking(token)._recipientRblAddress();
        _rbl=IStaking(token)._rbl();
        //同步今日限额
        _buyAmountEveryDay[block.timestamp/(24*60*60)]=IStaking(token)._buyAmountEveryDay(block.timestamp/(24*60*60));
        _syncFirstStatus=true;
    }
    function syncStake(address token,uint gasMinOut) external onlyOwner returns(uint success){
        if(_syncFirstStatus==false){
            syncFirst(token);
        }
        //同步质押数据
        for(uint i=1;i<=_stakeTimes;i++){
            if(gasleft()<gasMinOut){
                break;
            }
            if(_syncStakeStatus[i]==true){
                continue;
            }
            IStaking.stakeInfo memory info=IStaking(token)._stakeInfos(i);
            _stakeInfos[i]=stakeInfo(info.status,
                info.stakeId,
                info.account,
                info.stakeTime,
                info.claimTime,
                ...
                info.stakeTotal,
                info.stakeToken,
                info.stakeTokenTotal,
                info.stakeInterestTotal,
                info.stakeInterestTokenTotal,
                info.waitReceiveAmount
            );
            success++;
            _syncStakeStatus[i]=true;
        }
        if(success==0){
            revert("complete");
        }
    }
    function syncUser(address token,uint gasMinOut) external onlyOwner returns(uint success){
        if(_syncFirstStatus==false){
            syncFirst(token);
        }
        //同步用户信息
        for(uint i=1;i<=_userNum;i++){
            if(gasleft()<gasMinOut){
                break;
            }
            _users[i]=IStaking(token)._users(i);
            if(_syncUserStatus[_users[i]]==true){
                continue;
            }
            _userStakeTotal[_users[i]]=IStaking(token)._userStakeTotal(_users[i]);
            _userInterestTotal[_users[i]]=IStaking(token)._userInterestTotal(_users[i]);
            _userIds[_users[i]]=IStaking(token)._userIds(_users[i]);
            uint len=IStaking(token).userStakeNum(_users[i]);
            for(uint j;j<len;j++){
                _userStakeList[_users[i]].push(IStaking(token)._userStakeList(_users[i],j));
            }
            for(uint k;k<50;k++){
                try IStaking(token)._userUnStakeInfos(_users[i],k){
                    IStaking.unStakeInfo memory info=IStaking(token)._userUnStakeInfos(_users[i],k);
                    _userUnStakeInfos[_users[i]].push(unStakeInfo(info.unStakeTime,info.stakeTokenOutTotal));
                }catch{
                    break;
                }
            }
            //同步质押领取收益数据
            for(uint ii;ii<1000;ii++){
                try IStaking(token)._userClaimInfos(_users[i],ii){
                    IStaking.claimInfo memory info=IStaking(token)._userClaimInfos(_users[i],ii);
                    _userClaimInfos[_users[i]].push(claimInfo(
                            info.amount,
                            info.claimTime,
                            info.claimSecond,
                            info.claimType,
    ...
    function setParams(uint breachRatio,uint stakeCharge,uint maxBuyAmountEveryDay,bool breachOpen) external onlyOwner{
        ...
        _stakeCharge=stakeCharge;//质押手续费比例（千分位）
    ...
    //设置质押的代币合约
    function setAllowToken(address[] memory _tokens,bool status) external onlyOwner{
        for(uint i;i<_tokens.length;i++){
            _allowTokens[_tokens[i]]=status;
        }
        emit changeParams(_msgSender(),"setAllowToken");
    }

    //修改质押参数
    function changeStakeInfo(uint[] memory stakeIds,bool status,uint stakeTime,uint stakeEndTime) external onlyOwner{
        for(uint i;i<stakeIds.length;i++){
            stakeInfo storage info = _stakeInfos[stakeIds[i]];
            info.status=status;
            info.stakeTime=stakeTime;
            info.stakeEndTime=stakeEndTime;
        }
    }
    ...
    //赎回所有质押并待领取
    function unStakeAll(uint count) external onlyOwner returns (uint success){
        require(_stakeTimes>0,"error1");
        for(uint i=1;i<=_stakeTimes;i++){
            ...
            if(interestAmount>0){
                ...
                _userClaimInfos[info.account].push(claimInfo(interestAmount,block.timestamp,block.timestamp>info.stakeEndTime?(info.stakeEndTime>info.claimTime?info.stakeEndTime.sub(info.claimTime):0):block.timestamp.sub(info.claimTime),2,(info.stakeEndTime-info.stakeTime)/(24*3600),info.rate,info.stakeTotal,++_claimTimes));
            }
            info.status=false;
    ...
    function getInterestAmount(uint stakeId,bool _type) public view returns (uint){
        ...
        if(_info.claimTime>=_info.stakeEndTime){
            return 0;
        }
        if(_type==true){
            //显示u 按比例拿
            ...
            if(block.timestamp>=_info.stakeEndTime){
                nowTime=_info.stakeEndTime;
            }
            return _info.stakeTotal.mul(_info.rate).div(100)*(nowTime-_info.claimTime)/((_info.stakeEndTime-_info.stakeTime));
        }else{
            uint nowTime=block.timestamp;
            if(block.timestamp>=_info.stakeEndTime){
                nowTime=_info.stakeEndTime;
            }
            //公示=应发总利息*（最新时间-上次领取时间）/质押总秒数
            return _amountOut(_info.stakeTotal.mul(_info.rate).div(100),_usdt,_o2)*(nowTime-_info.claimTime)/((_info.stakeEndTime-_info.stakeTime));
        }
    }
    //获取用户可提现的线性利息
    ...
    function stake(uint256 amount,address _token,uint256 interestId) public{
        ...
        require(_allowTokens[_token]==true,"error3");
        uint today=block.timestamp/(24*60*60);
        require(_buyAmountEveryDay[today]+amount<=_maxBuyAmountEveryDay,"error5");
        _buyAmountEveryDay[today]=_buyAmountEveryDay[today].add(amount);
        interestInfo memory _info=_interestInfos[interestId];
        if(_stakeCharge>0){
            //扣rbl手续费
            IBEP20(_rbl).transferFrom(_msgSender(),_recipientRblAddress,_amountOut(amount,_usdt,_rbl).mul(_stakeCharge).mul(_info.stakeTime.div(24*3600)).div(1000));
        }
        uint inAmount;
        if(_usdt!=_token){
    ...
    function unStake(uint256 stakeId) external{
        ...
        if(info.status==false&&info.waitReceiveAmount>0){
            ...
            }else if(_breachOpen==false&&block.timestamp<info.stakeEndTime){
                revert("error3");
            }
            ...
            if(interestAmount>0){
                ...
                info.stakeInterestTokenTotal+=interestAmount;//添加到当前累计收益
                _userClaimInfos[_msgSender()].push(claimInfo(interestAmount,block.timestamp,block.timestamp>info.stakeEndTime?(info.stakeEndTime>info.claimTime?info.stakeEndTime.sub(info.claimTime):0):block.timestamp.sub(info.claimTime),1,(info.stakeEndTime-info.stakeTime)/(24*3600),info.rate,info.stakeTotal,++_claimTimes));
    ...
    function claim(uint256 stakeId) external{
        ...
        _userClaimInfos[_msgSender()].push(claimInfo(interestAmount,block.timestamp,block.timestamp>info.stakeEndTime?(info.stakeEndTime>info.claimTime?info.stakeEndTime.sub(info.claimTime):0):block.timestamp.sub(info.claimTime),0,(info.stakeEndTime-info.stakeTime)/(24*3600),info.rate,info.stakeTotal,++_claimTimes));
        info.claimTime=block.timestamp;
        info.stakeInterestTotal+=getInterestAmount(stakeId,true);
        info.stakeInterestTokenTotal+=interestAmount;//添加到当前累计收益
