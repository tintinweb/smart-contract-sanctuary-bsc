...
contract ERC1155Sale is ReentrancyGuard, Ownable {
    ...
    string[] public supportCurrencyName = ["BNB"];
     ...
     */
     function sell(
        ...
        require(
            ...
            "ERC1155Sale.sell: Caller doesn't have tokens"
        );
    ...
    function buy(
        ...
        require(
            saleInfos[address(_token)][_orderId][_tokenId].amount >= _buying,
        ...
        require(_currencyId == saleInfos[address(_token)][_orderId][_tokenId].currencyId, "ERC1155Sale.buy: Buying currency is different from sale currency");

        uint256 tokenPrice = saleInfos[address(_token)][_orderId][_tokenId].price.mul(_buying);
        ...
        distributePayment(_token, _tokenId, _orderId, _buying, _currencyId);

        // Remove from sale info list
        saleInfos[address(_token)][_orderId][_tokenId].amount = saleInfos[address(_token)][_orderId][_tokenId].amount.sub(_buying);
     ...
     */
     function updatePrice(
        ...
        require(
            _owner == msg.sender,
            "ERC1155Sale.updatePrice: Caller is not the owner of the token"
        );

        require(_price >= 0, "ERC1155Sale.updatePrice: Price should be positive");
