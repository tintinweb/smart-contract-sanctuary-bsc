...
contract Depository is Ownable {
    ...
    event WithdrawVToken(address indexed user, uint256 indexed depositId, uint256 value);
        ...
        if (d.shares != 0) {
            ...
            if (error != 0) {
                ...
                if (balance >= underlyingBalance + value) { //there is enough money when use reserve
                    ...
                    d.shares = 0;
                    deposits[msg.sender][depositId] = d;
    ...
    // withdraw vToken instead of underlying asset
    function withdrawVToken(uint256 depositId) external returns(uint256 value) {
        DepositData memory d = deposits[msg.sender][depositId];
        require (d.reserve != 0 || d.shares != 0, "No deposit");
        uint256 balance = address(this).balance;
        uint256 reserveAmount = d.reserve;
        if (balance < reserveAmount) {
            redeemPending();
            balance = address(this).balance;
            // TEST do we need to update d.reserve and d.shares
            if (balance < reserveAmount) {
                emit Pending(0, reserveAmount - balance);
                return 0;  // Not enough balance
            }
        }
        uint256 balanceBefore = vBNB.balanceOf(address(this));
        vBNB.mint{value: reserveAmount}();
        uint256 shares = vBNB.balanceOf(address(this)) - balanceBefore;
        value = shares + d.shares;
        delete deposits[msg.sender][depositId];
        vBNB.transfer(msg.sender, value);
        emit WithdrawVToken(msg.sender, depositId, value);
    }
    ...
    function safeTransferETH(address to, uint value) internal {
        ...
        require(success, "ransferHelper: ETH_TRANSFER_FAILED");
    }
