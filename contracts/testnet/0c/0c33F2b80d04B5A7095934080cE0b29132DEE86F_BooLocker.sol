...
contract BooLocker is Ownable {
    ...
    uint256 shortFee = 0.5 ether;
    uint256 penaltyFee = 1 ether;
    ...
    function releaseLock(uint256 lockId) external onlyEOA {
        ...
        require(LInfo.releaseComplete == false, "already release");
        LInfo.releaseComplete = true;
        LInfo.amount = 0;
        totalLocked--;
    ...
    function releaseVesting(uint256 lockId) external onlyEOA {
        ...
        require(LVInfo.releaseComplete == false, "already release");
        require(LVInfo.dynamicAmount > 0, "claim processes are finished");
    ...
    function viewLockByID(uint256 id, address addr)
        ...
        returns (
            ...
            uint256 releaseDate,
            uint256 amount,
            bool releasedComplete,
            bytes16 method,
            bool available
        )
    {
        AccountInfo storage AInfo = locker[addr];
        LockInfo storage LInfo = AInfo.lockinfo[id];
        ...
        return (
            ...
            LInfo.releaseDate,
            LInfo.amount,
            LInfo.releaseComplete,
            LInfo.method,
            LInfo.releaseDate >= block.timestamp
        );
    }
