...
/** 
 @dev StakingPool. Deposit LP tokens to earn rewards in the form of Token tokens.
 *  This contract mints new Token tokens everytime a new user deposits LP tokens to the pool
 */
contract FreeNodeStakeContract is Ownable {
   ...
   struct DepositInfo {
    ...
    struct PoolUpdateInfo {
        uint256 persistentBlocks; 
        uint256 lastUpdateBlock;
        uint256 currentTotalDepositAmount;
    }

    address public Token;
    uint256 public tokenPerBlock;
    uint256 public totalAllocPoint = 100;
	uint256 public bonusPerReferral = 0;
    ...
    mapping(uint256 => PoolUpdateInfo[]) public poolUpdateInfo;
    ...
    function CurrentPeriodReward(uint256 _pid, address _user) public view returns (uint256 currentPeriodPending) {
		PoolInfo storage pool = poolInfo[_pid];
        for (uint256 i = 0; i < depositInfo.length; i++) {
            if (depositInfo[i].lpToken == pool.lpToken && depositInfo[i].user == _user) {
                uint256 poolClaimAmountPerBlock = tokenPerBlock.mul(1e18).mul(pool.allocPoint).div(totalAllocPoint);
                uint256 userClaimAmountPerBlock=  poolClaimAmountPerBlock.mul(1e18).mul(depositInfo[i].amount).div(pool.totalSupply);
                uint256 blockNum =  getBlockInterval(pool.lastUpdateBlock,block.number);
                uint256 pending = userClaimAmountPerBlock.mul(blockNum).div(1e36);
                currentPeriodPending = currentPeriodPending.add(pending);
            }
        }
        return currentPeriodPending;
    }

    function PreviousPeriodReward(uint256 _pid, address _user) public view returns (uint256 previousPeriodPending) {
		PoolInfo storage pool = poolInfo[_pid];
        for (uint256 i = 0; i < depositInfo.length; i++) {
            if (depositInfo[i].lpToken == pool.lpToken && depositInfo[i].user == _user) {
                for (uint256 j = 0; j < poolUpdateInfo[_pid].length; j++) {
                    if(depositInfo[i].depositBlock < poolUpdateInfo[_pid][j].lastUpdateBlock) {
                        uint256 poolClaimAmountPerBlock = tokenPerBlock.mul(1e18).mul(pool.allocPoint).div(totalAllocPoint);
                        uint256 userClaimAmountPerBlock=  poolClaimAmountPerBlock.mul(1e18).mul(depositInfo[i].amount).div(poolUpdateInfo[_pid][j].currentTotalDepositAmount);
                        uint256 pending = userClaimAmountPerBlock.mul(poolUpdateInfo[_pid][j].persistentBlocks).div(1e36);
                        previousPeriodPending = previousPeriodPending.add(pending);
                    }
                }
            }
        }
        return previousPeriodPending;
    }

    function pendingReward(uint256 _pid, address _user) public view returns (uint256 totalPending) {
        UserInfo storage user = userInfo[_pid][_user];
		totalPending = CurrentPeriodReward(_pid,_user).add(PreviousPeriodReward(_pid,_user)).sub(user.rewardDebt);
        return totalPending;
    }

    function updatePoolInfo(uint256 _pid) internal {
        if(poolInfo[_pid].totalSupply == 0) {
            poolInfo[_pid].lastUpdateBlock = block.number;
            return;
        } else {
            poolUpdateInfo[_pid].push(PoolUpdateInfo({
                persistentBlocks: block.number.sub(poolInfo[_pid].lastUpdateBlock),
                lastUpdateBlock: block.number,
                currentTotalDepositAmount: poolInfo[_pid].totalSupply
                // persistentBlocks: block.number.sub(poolInfo[_pid].lastUpdateBlock),
                // currentTotalDepositAmount: poolInfo[_pid].lpToken.balanceOf(address(this))
            }));
            poolInfo[_pid].lastUpdateBlock = block.number;
        }
    }
    ...
    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 totalReferralBonus){
        ...
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 referralBonus = pendingReward(_pid,myInviteeAddress[i]).mul(bonusPerReferral).div(referralRateDenominator);
    ...
    function withdraw(uint256 _pid) public {
        ...
        if(user.totalDepositAmount > 0) {
            updatePoolInfo(_pid);
            // pool.lpToken.safeTransfer(address(msg.sender), user.totalDepositAmount);
            pool.totalSupply = pool.totalSupply.sub(user.totalDepositAmount);
        }
        emit Withdraw(msg.sender, _pid, user.totalDepositAmount);
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        require(user.totalDepositAmount > 0,"emergencyWithdraw: user amount must be greater than 0");
    ...
    function deleteAllWithdrawTX(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        for (uint256 i = 0; i < depositInfo.length; i++) {
    ...
    function setIInviteContract(address _InviteContract) public onlyOwner {
        InviteContract = IInviteContract(_InviteContract);
    }
    ...
    function setPoolAllocPointDenominator(uint256 _value) public onlyOwner {
