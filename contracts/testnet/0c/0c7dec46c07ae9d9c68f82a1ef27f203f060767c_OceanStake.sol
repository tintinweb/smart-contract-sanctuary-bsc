...
contract OceanStake is ReentrancyGuard {
    ...
    function _calcRewardTest(
        uint256 _invested,
        uint256 _checkpoint,
        uint256 _holdBonusStart,
        uint256 _tsNow,
        uint256 _balancePerc

    ) private pure returns (uint256) {
        if (_invested == 0) return 0;

        uint256 start = _checkpoint;
        uint256 diff = (start - _holdBonusStart) % DAY_SECONDS;
        uint256 end = start + DAY_SECONDS - diff;

        uint256 tsNow = _tsNow;
        //balance bonus retroactive
        uint256 rew = (_getBasePerc() + _balancePerc) * (tsNow - start);
        uint256 holdBonus = _getHoldBonusPercAtTest(_holdBonusStart, start);
        while (true) {
            if (end > tsNow || holdBonus == HOLD_BONUS_MAX) {
                end = tsNow; //calculate from start to now as the last step
            }
        ...
        rew = (rew * _invested) / (DAY_SECONDS * PERC_DIVIDER);
        return rew;
    }

    function _getHoldBonusPercAtTest(uint256 _holdBonusStart, uint256 timestamp)
        private
        pure
        returns (uint256)
    {
        ...
        uint256 i = (timestamp - _holdBonusStart) / DAY_SECONDS;
