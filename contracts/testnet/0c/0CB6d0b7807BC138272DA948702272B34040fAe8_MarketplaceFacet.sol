...
contract PodTransfer is ReentrancyGuard {
    ...
    function insertPlot(
        ...
        uint256 curIndex = s.firstPods[account];
        uint256 nextIndex = s.plotsIndexes[account][curIndex];
        uint256 countPod = s.countPods[account];

        if (countPod == 0) {
            s.firstPods[account] = id;
        } else {
            if (id < curIndex) {
                // insert in begin
                s.firstPods[account] = id;
                s.plotsIndexes[account][id] = curIndex;
            } else {
                if (countPod == 1) {
                    s.plotsIndexes[account][curIndex] = id;
                } else {
                    uint256 iter = 0;
                    while (iter < countPod - 2 && id > nextIndex) {
                        iter++;
                    ...
                    if (id < nextIndex) {
                        // insert in beetwn
                        s.plotsIndexes[account][curIndex] = id;
                        s.plotsIndexes[account][id] = nextIndex;
                    } else {
                        // insert in end
                        s.plotsIndexes[account][nextIndex] = id;
                    }
                }
            }
        }
    ...
    function removePlot(
        ...
        if (start != 0 || end != amount) {
            // if need div plot
            if (start == 0) {
                // if begin
                s.plotsIndexes[account][id + end] = s.plotsIndexes[account][id];
                s.plotsIndexes[account][id] = id + end;
                s.countPods[account]++;
            } else if (end == amount) {
                // if end
                s.plotsIndexes[account][id + start] = s.plotsIndexes[account][id];
                s.plotsIndexes[account][id] = id + start;
                s.countPods[account]++;
            } else {
                // if beetwen
                s.plotsIndexes[account][id + end] = s.plotsIndexes[account][id];
                s.plotsIndexes[account][id + start] = id + end;
                s.plotsIndexes[account][id] = id + start;
                s.countPods[account] = s.countPods[account] + 2;
            }
        }
        LibClaim._removeIndex(account, id + start, 0, false);
    }
