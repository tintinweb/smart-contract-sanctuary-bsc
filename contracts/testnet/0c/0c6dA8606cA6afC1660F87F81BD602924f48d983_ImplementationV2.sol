...
contract Implementation is Upgradeable {
     ...
     * @dev Set whilelist
     */
    function setWhitelist(bytes memory nftId, address addr) public onlyAdmins {
        require(!isAdmin(addr), "Implementation: Can not whitelist admins");
        isWhitelisted[nftId][addr] = true;
        emit SetWhitelistEvent(nftId, addr, true);
    }

    /**
     * @dev Revoke whilelist
     */
    function revokeWhitelist(bytes memory nftId, address addr) public onlyAdmins {
        isWhitelisted[nftId][addr] = false;
        emit SetWhitelistEvent(nftId, addr, false);
    }

    /**
     * @dev Set whilelist
     */
    function setWhitelists(bytes memory nftId, address[] memory addr) public onlyAdmins {
        uint256 len = addr.length;
        bool[] memory result = new bool[](len);
        for (uint256 i = 0; i < len; i++){
            if(!isAdmin(addr[i])) {
                isWhitelisted[nftId][addr[i]] = true;
                result[i] = true;
            }
        }
        emit SetWhitelistMultipleAddressesEvent(nftId, addr, result);
    }

    /**
     * @dev Revoke whilelist
     */
    function revokeWhitelists(bytes memory nftId, address[] memory addr) public onlyAdmins {
        uint256 len = addr.length;
        bool[] memory result = new bool[](len);
        for (uint256 i = 0; i < len; i++){
            isWhitelisted[nftId][addr[i]] = false;
            result[i] = false;
        }
        emit SetWhitelistMultipleAddressesEvent(nftId, addr, result);
    }
    ...
    /**
     * @dev Set discount rate
     */
    function setDiscountRate(bytes memory nftId, uint256 rate) public onlyAdmins {
        require(rate > 0 && rate < 100000, "Implementation: Invalid discount rate value");
        discountRate[nftId] = rate;
        emit SetDiscountRateEvent(nftId, rate);
    }

    /**
     * @dev Set total number a wallet can buy the NFT with discount price 
     */
    function setNFTCanBuy( uint256 num) public onlyAdmins {
        require(num > 0 && num != nftCanBuy, "Implementation: Invalid nftCanBuy value");
        nftCanBuy = num;
    }
...
contract Upgradeable is Ownable, ReentrancyGuard {
    ...
    mapping(bytes => uint256) public discountRate; //0 to 100000
    mapping(bytes => mapping(address => bool)) public isWhitelisted;
    uint256 public nftCanBuy;
    mapping(bytes => mapping(address => uint256)) public nftBought;
    ...
    event SetWhitelistEvent(
        bytes nftId,
        address addr,
        bool isWhitelisted
    );

    event SetWhitelistMultipleAddressesEvent(
        bytes nftId,
        address[] addr,
        bool[] isWhitelisted
    );

    event SetDiscountRateEvent(
        bytes nftId,
        uint256 rate
    );
