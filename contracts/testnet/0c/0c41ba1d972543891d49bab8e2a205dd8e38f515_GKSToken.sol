...
contract GKSToken is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public profitAddress = 0x79596fef4dAcab08b353233CA1Fd617b5Fa31b53;
    

    string private _name = "GKS";
    string private _symbol = "GKS";
    ...
    uint256 public dayProfit = 1369 * 10**_decimals;
    ...
    constructor(address liquidityReceiveAddress_) {
        ...
        _isExcludedFromFee[liquidityReceiveAddress] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[address(tokenHelper)] = true;
    ...
    function excludeFromFeeBatch(address[] calldata accounts, bool exclude) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFee[accounts[i]] = exclude;
        }
    }
    ...
    function isSwapWhite(address account) public view returns (bool) {
        return _swapWhiteList[account];
    }

    function addSwapWhite(address[] calldata accounts, bool isWhite) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _swapWhiteList[accounts[i]] = isWhite;
        }
    }

    function setLiquidityReceiveAddress(address liquidityReceiveAddress_) public onlyOwner {
        liquidityReceiveAddress = liquidityReceiveAddress_;
    }

    function setProfitAddress(address address_) public onlyOwner {
        profitAddress = address_;
    }
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair) {
            require(_swapWhiteList[to], "ERC20: to address is not in the white list");
        }

        if (liquifyEnabled && 
            _tOwned[address(this)] >= 10 * 10**_decimals && 
            !inSwapAndLiquify && 
            from != uniswapV2Pair) {
            _swapAndLiquify(_tOwned[address(this)]);
        }
    ...
    function process(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(_tOwned[profitAddress]  < amount) return;
    ...
    function distributeDividend(address shareholder ,uint256 amount) internal {
        _tOwned[profitAddress] = _tOwned[profitAddress].sub(amount);
        _tOwned[shareholder] = _tOwned[shareholder].add(amount);
        emit Transfer(profitAddress, shareholder, amount);
    }
    ...
    function _swapAndLiquify(uint256 tokenAmt) private lockTheSwap {
        ...
        emit SwapAndLiquify(half, usdtOut, otherHalf);
    }
