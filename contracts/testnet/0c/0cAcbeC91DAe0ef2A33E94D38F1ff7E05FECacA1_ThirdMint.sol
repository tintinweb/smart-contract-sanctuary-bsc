...
contract ThirdMint is Ownable, ReentrancyGuard, Pausable, IERC721Receiver {
    ...
    function transferNft(NftDesc memory nft, address to) private {
        if(nft.kind == KIND.SPACESHIP) {
            spaceship.transferFrom(address (this), to, nft.tokenId) ;
        } else if(nft.kind == KIND.DEFENSIVEFACILITY) {
            defensiveFacility.transferFrom(address (this), to, nft.tokenId) ;
        } else if(nft.kind == KIND.HERO) {
            hero.transferFrom(address (this), to, nft.tokenId) ;
        }
    }
