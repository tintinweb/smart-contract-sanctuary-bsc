...
// File @openzeppelin/contracts-upgradeable/proxy/utils/Init[email protected]
...
@notice function to update end of duel of league and calculate duel points
 ...
 Note only aadmin can call this function
 */
  function duelEnded(uint256 duelId, address winner, address loser, string memory league, uint256 lp, bool isDraw, address user) onlyAdmin public {
    ...
    users[user].league = league;
    users[user].points = lp;
    if(user == winner){
        users[user].wins++;
    }else if(isDraw){
         users[user].draws++;
    ...
    uint256 daysDiff = block.timestamp.sub(gameStartTime).div(1 days);
    _userDailyBattles[user][daysDiff]++;
    userInDuel[user] = false;
  }
  ...
  function getUserInduel(address _userAddress) public view returns(bool,address,uint256){
    ...
    if(userInDuel[_userAddress])
    {return (userInDuel[_userAddress], _deulDetail[lastDuelId].oponent == _userAddress ? _deulDetail[lastDuelId].user :  _deulDetail[lastDuelId].oponent, lastDuelId);}
    else{
 ...
 */
  ...
  function duelStarted(uint256 _duelId,  address _user, address oponent) onlyAdmin public {
    ...
    uint256 oponentLastDuelId = _userDuelIds[oponent].length > 0 ? _userDuelIds[oponent][_userDuelIds[oponent].length - 1] : 0;
    // require( _deulDetail[_duelId].duelTime == 0 || oponentLastDuelId == _duelId, "oponent in another duel");
    require(!userInDuel[_user] && ((_duelId == oponentLastDuelId || !userInDuel[oponent]) || oponent == address(0x0)), "on of players already in duel");
    require(users[_user].fuel >= fuelConsume[users[_user].league], "user does not have enough fuel" );
    require(_deulDetail[_duelId].duelEndTime > 0, "duel already has ended");
