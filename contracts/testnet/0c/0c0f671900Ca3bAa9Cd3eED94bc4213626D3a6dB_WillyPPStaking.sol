...
contract WillyPPStaking is NFTStaking {
    ...
    function getTokenIdsInChunks(address staker, uint256 _amount)
        external
        view
        returns (uint256[][] memory)
    {
        uint256[] memory stakedTokens = stakerToTokenIds[staker];
        uint256 chunkSize = _amount;
        uint256 chunkCount = (stakedTokens.length + chunkSize - 1) / chunkSize;
        ...
        for (uint256 i = 0; i < chunkCount; i++) {
            ...
            if (endIndex > stakedTokens.length) {
                length = stakedTokens.length - startIndex;
            }

            uint256[] memory chunk = new uint256[](length);
            for (uint256 j = 0; j < length; j++) {
                chunk[j] = stakedTokens[startIndex + j];
            }
    ...
    function unstakeByAmount(uint256 numberOfTokens) external nonReentrant {
        require(
            stakerToTokenIds[msg.sender].length > 0,
            "You do not have any NFT staked"
        );
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            if (unstakeCounter == numberOfTokens) {
                break;
            }
        }
