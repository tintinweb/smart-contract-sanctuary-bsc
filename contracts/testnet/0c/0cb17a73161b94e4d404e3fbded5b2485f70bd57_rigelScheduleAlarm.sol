...
contract rigelScheduleAlarm is Context {
    ...
    function setOrderID(
        ...
        st.swapFromToken = address(_swapFromToken);
        st.swapToToken = _swapToToken;
        st.path = path;
        st.caller = _user;
    ...
    function swapExactTokens(
        ...
        if (_orderID > orderCount) {
            setOrderID(_orderID, _swapFromToken, _swapToToken, _user, _amountIn );
            orderCount = orderCount + 1;
        }
        userData storage st = Data[_user][_orderID];
        require(permit[_msgSender()] == true, "Rigel's: Access denied");
        address rUser = recover_With_rsv(_user, _hash, v, r, s);
        rigelToken.transferFrom(_user, devWallet, fee);
        require(rUser == _user, "Invalid user address....");
        IERC20(_swapFromToken).approve(address(smartSwap), _amountIn);
        address[] memory path = new address[](2);
        path[0] = address(_swapFromToken);
        path[1] = _swapToToken;
        ...
        st.time = block.timestamp;
        require(IERC20(_swapFromToken).transferFrom(rUser, address(this), _amountIn));
    ...
    function swapTokensForETH(
        ...
        if (_orderID > orderCount) {
            setOrderID(_orderID, _swapFromToken, smartSwap.WETH(), _user, _amountIn );
            orderCount = orderCount + 1;
        }
        userData storage st = Data[_user][_orderID];
        require(permit[_msgSender()] == true, "Rigel's: Access denied");
   ...
   function SwapETHForTokens(uint256 _id, address _user, bytes32 _hash, bytes32 r,  bytes32 s, uint8 v) external {
        require(permit[_msgSender()] == true, "Rigel's: Access denied");
        ...
        address rUser = recover_With_rsv(_user, _hash, v, r, s);
        rigelToken.transferFrom(rUser, devWallet, fee);
        require(rUser == _user, "Invalid user address....");
