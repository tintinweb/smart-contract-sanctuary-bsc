...
If you make the biggest buy (in BNB) you become the TaxMan, and collect 5% fees (in BNB) the same way marketing does. 
...
The TaxMan is set via biggest buy in BNB.
The point is to reward big buys. There are no punishments other than disallowing sellers to be the TaxMan.

Each TaxMan has a timelimit. After 30 minutes (can be changed), they cease to be TaxMan and the record is reset to 0, giving more a chance and increasing volume.
*/
...
contract TaxMan is IBEP20, Auth {
    ...
    constructor () Auth(msg.sender) {      
        ...
        taxMan = msg.sender;  //tax collector wallet (defaults to marketing)
    ...
    function setTimeLimit(uint256 newLimit) external authorized {
        timeLimit = newLimit;
    }

    function setRecord(uint256 newRecord) external authorized {
        _lastRecord = _record;
        _record = newRecord;
    }

    function setRecordToLast() external authorized {
        _record = _lastRecord;
    }

    function setTaxMan(address holder) external authorized {
        taxMan = holder;
    }

    function setHasSold(address holder, bool sold) external authorized {
        hasSold[holder] = sold;
    }
	...
	function checkTxLimit(address sender, address recipient, uint256 amount) internal {
		if (sender != owner
            ...
            if (newRules){ //new rules are by bnb used
        ...
        if (newRules){ //in the new rules, taxman has a timeout
	...
	function setSendingAllowed(bool active) external authorized {
		sendingAllowed = active;
	}
...
}// A Vincent Contract | t.me/vincentscourtyard //💵💵💵💵💵💵💵