...
contract LotteryBetContract is Ownable{
    ...
    mapping (uint256 =>mapping(address=>uint256)) public _BUOM;
    ...
    //mapping (uint256=>NFTBet) public _BOM;
    //key: user  value:orderId  用户参与的投注订单ID集合
    //mapping (address=>uint256[]) public _UBOM;
    //订单Id
...
/*            _BOM[betInfo.orderId]=betInfo;
            _UBOM[_msgSender()].push(betInfo.orderId);*/
    ...
    function betPay(uint256 sumMoney) internal{
        transferToAddress(address(this),sumMoney);
        transfer(poolAddress,sumMoney.mul(poolFee).div(100));
        transfer(platformAddress,sumMoney.mul(platformFee).div(100));
        transfer(gameLpAddress,sumMoney.mul(gameLpFee).div(100));
        transfer(buyBackAddress,sumMoney.mul(buyBackFee).div(100));
    ...
    function transfer(address recipient, uint256 amount) private {
        if(amount>0)
            _LotteryToken.transfer(recipient,amount);
    }
    //返佣KOL分配 记录
