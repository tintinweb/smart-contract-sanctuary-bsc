...
contract BoxRarity is ERC1155SupplyUpgradeable, OwnableUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    EnumerableSetUpgradeable.AddressSet private _admins;

    mapping(uint256 => uint256) public minPriceAddListings;
    mapping(uint256 => uint256) public caps;
    mapping(uint256 => uint256) public amountMinted;

    modifier onlyAdmin() {
        require(_admins.contains(msg.sender),"Not Admin");
        _;
    }
    ...
    function setMinPriceAddListings(
        uint256[] memory rarities,
        uint256[] memory values
    ) external onlyOwner {
        require(rarities.length == values.length,"rarites and values not equal length");

        for(uint256 i = 0; i < rarities.length;i++){
            minPriceAddListings[rarities[i]] = values[i];
        }
    }
    ...
    function getMinPriceAddListing(uint256 rarity)  external view returns(uint256) {
        return minPriceAddListings[rarity];
    }
