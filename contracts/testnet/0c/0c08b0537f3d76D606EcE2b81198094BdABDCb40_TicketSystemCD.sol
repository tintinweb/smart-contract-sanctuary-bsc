...
contract TicketSystemCD is Ownable, ReentrancyGuard, Pausable {
    ...
    struct UserInfo {
        uint256 ticketBought;
        uint256 ticketWithdrawn;
    ...
    event TicketWithdrawn(address indexed user, uint256 amountRefund);
        ...
        nonReentrant
    {
        ...
        userInfo[_msgSender()].ticketBought += numOfTicket;
     ...
     * @param amount to select quantity of tickets to purchase
     */
    function withdrawTicket(address recipient, uint256 amount)
        external
        onlyAdmin
        whenNotPaused
        nonReentrant
    {
        ...
        if (userInfo[recipient].lastWithdrawalTime != 0) {
            require(
                userInfo[recipient].lastWithdrawalTime + 24 hours <=
                    block.timestamp,
                "Withdrawal is only allowed once every 24 hours"
            );
        }
        require(amount <= withdrawLimit, "Withdrawal amount exceeds Limit");

        uint256 numOfTicket = (amount * 10000) / ticketPrice;
        uint256 teamAmount = (numOfTicket * teamPercentage) / 10000;
        uint256 rewardPoolAmount = (numOfTicket * rewardPoolPercentage) / 10000;
        uint256 burnAmount = (numOfTicket * burnPercentage) / 10000;

        uint256 ticketAmount = amount -
            (teamAmount + rewardPoolAmount + burnAmount);

        userInfo[recipient].lastWithdrawalTime = block.timestamp;
        userInfo[recipient].ticketWithdrawn += numOfTicket;
        ...
        emit TicketWithdrawn(recipient, ticketAmount);
    }
    ...
    function setTicketPrice(uint256 newPrice) external onlyOwner {
        require(newPrice > 0, "New Price should be greater than Zero");
    ...
    function setWithdrawLimit(uint256 amount) external onlyOwner {
        require(amount > 0, "Withdraw limit be greater than Zero");
    ...
    function feesTransfer(
        ...
        bool burnTransfer = token.burn(burnAmnt);
        require(burnTransfer, "Burn failed");
