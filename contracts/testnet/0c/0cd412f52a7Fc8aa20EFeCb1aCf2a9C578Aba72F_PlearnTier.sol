...
contract PlearnTier is IPlearnTier, Ownable {
    ...
    modifier whenTierExists(uint256 id) {
        require(EnumerableSet.contains(tierIds, id), "PlearnTier: Tier not exists");
        _;
    }

    modifier whenPoolNotExists(address pool) {
        require(
            !EnumerableSet.contains(lockPools, pool) && !EnumerableSet.contains(foundingInvestorPools, pool),
    ...
    function addLockPool(address pool) external onlyOwner whenPoolNotExists(pool) {
        EnumerableSet.add(lockPools, pool);
    }

    function removeLockPool(address pool) external onlyOwner {
        EnumerableSet.remove(lockPools, pool);
    }

    function addFoundingInvestorPool(address pool) external onlyOwner whenPoolNotExists(pool) {
        EnumerableSet.add(foundingInvestorPools, pool);
    }

    function removeFoundingInvestorPool(address pool) external onlyOwner {
        EnumerableSet.remove(foundingInvestorPools, pool);
    }
    ...
    function addTier(
        ...
        require(!EnumerableSet.contains(tierIds, id), "PlearnTier: Tier already exists");
        tiers[id] = Tier({id: id, name: name, minimumAmount: minimumAmount});
        EnumerableSet.add(tierIds, id);
    ...
    function updateTier(
        ...
        string memory name,
        uint256 minimumAmount
    ) external onlyOwner whenTierExists(id) {
    ...
    function removeTier(uint256 id) external onlyOwner whenTierExists(id) {
        delete tiers[id];
        EnumerableSet.remove(tierIds, id);
        emit TierRemove(id);
    }

    //Public functions

    function getTier(uint256 id) external view whenTierExists(id) returns (Tier memory) {
        return tiers[id];
    }
    ...
    function getTierOf(address user) external view override returns (Tier memory) {
        ...
        uint256 length = EnumerableSet.length(tierIds);
        for (uint256 i = 0; i < length; i++) {
            Tier memory tier = tiers[EnumerableSet.at(tierIds, i)];
    ...
    function isFoundingInvestor(address user) public view returns (bool) {
        uint256 length = EnumerableSet.length(foundingInvestorPools);
        for (uint256 i = 0; i < length; i++) {
    ...
    function isInTierOrHigher(uint256 id, address user) external view override whenTierExists(id) returns (bool) {
        Tier memory tier = tiers[id];
        uint256 totalStake = _getTotalStake(user);
        return totalStake >= tier.minimumAmount;
    }

    function tierExists(uint256 id) external view override returns (bool) {
        return EnumerableSet.contains(tierIds, id);
    }
    ...
    function _getTotalStake(address user) internal view returns (uint256) {
        ...
        //Locked pools
        uint256 lockPoolsLength = EnumerableSet.length(lockPools);
        ...
        uint256 foundingPoolsLength = EnumerableSet.length(foundingInvestorPools);
