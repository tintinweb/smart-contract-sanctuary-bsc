...
// File: Safe/DividendPayingTokenInterface.sol
...
// File: Safe/DividendPayingTokenOptionalInterface.sol
...
// File: Safe/DividendPayingToken.sol
...
contract TEJI is ERC20, Ownable {
    ...
    TEJIDividendTracker public dividendTracker;
    ...
    address public marketingWallet = 0x262A5c82f84021416C45c88a9dB28f92A3D7f7F3;
    address public devWallet = 0xfb8d6042582B488F03Bd864d64f1f7df8f0a4235;
    address public teamWallet = 0xc6907B767452259fA993E918b0851776466AAeC9;
    ...
    constructor()  ERC20("TEJI TOKEN", "TEJI") {

    	dividendTracker = new TEJIDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TEJI: The dividend tracker already has that address");

        TEJIDividendTracker newDividendTracker = TEJIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TEJI: The new dividend tracker must be owned by the TEJI token contract");
    ...
    function updateRouter(address newAddress) public onlyOwner {
        require(newAddress != address(router), "TEJI: The router already has that address");
        emit Updaterouter(newAddress, address(router));
        router = IRouter(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TEJI: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address newPair, bool value) public onlyOwner {
        require(newPair != pair, "TEJI: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value, "TEJI: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TEJI: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TEJI: Cannot update gasForProcessing to same value");
...
contract TEJIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()  DividendPayingToken("TEJI_Dividend_Tracker", "TEJI_Dividend_Tracker") {
    	claimWait = 3600;
    }

    function _transfer(address, address, uint256) internal pure override{
        require(false, "TEJI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TEJI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TEJI contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TEJI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "TEJI_Dividend_Tracker: Cannot update claimWait to same value");
