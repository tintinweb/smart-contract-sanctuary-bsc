...
contract Cashflow_Money_Inu is IBEP20, Ownable {
    ...
    mapping(address => Time) public Usertime;
    ...
    constructor() {
        ...
        liquedityReceiver = 0xFA8FA2Ef81A7931fC97F7617FDFE81585B5F735E;
        marketFeeReceiver = 0x60D97F4dA0cfaae53a52a5a51615C7ed740959Fd;
        devFeeReceiver = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    function getsellfee() public view returns (uint256) {
        uint256 CalcTime = (block.timestamp - Usertime[msg.sender].time) /duration;
        if (CalcTime >= 12) {
            CalcTime = 12;
        }
        uint256 OverFee = Sinc - CalcTime;
        return OverFee;
    }
    ...
    function _transfer(
        ...
        if (!isLimitExempt[recipient]) {
            require(
                balanceOf(recipient) + amount <= maxHoldAmount,
        ...
        if (
            ...
            if (recipient == pcsPair) {
                feeAmount =
                    (amount * (totalSellFee + getsellfee())) /(feeDivider);
                amountReceived = amount - (feeAmount);
                _takeFee(sender, feeAmount);
                setSellFeeCount(amount);
                Usertime[msg.sender].time = block.timestamp;
            }
        }
    ...
    function setBuyFeeCount(uint256 _amount) internal {
     ...
     function setSellFeeCount(uint256 _amount) internal {
        _liquedityFeeCounter = (_amount * (_liqueditySellFee+(getsellfee()*_liqueditySellFee)/totalSellFee)) / (feeDivider);
        _reflectionFeeCounter = (_amount * (_reflectionSellFee+(getsellfee()*_reflectionSellFee)/totalSellFee)) / (feeDivider);
        _marketFeeCounter = (_amount * (_marketSellFee+(getsellfee()*_marketSellFee)/totalSellFee)) / (feeDivider);
