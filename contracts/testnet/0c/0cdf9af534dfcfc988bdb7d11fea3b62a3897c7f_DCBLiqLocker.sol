...
contract DCBLiqLocker is Initializable, OwnableUpgradeable, IStaking {
  ...
  PoolExtended[] public pools;

  mapping(uint256 => mapping(address => UserExtended)) public users;
  ...
  function set(
    ...
    PoolExtended storage pool = pools[_pid];

    pool.common.apy = _apy;
    pool.common.lockPeriodInDays = _lockPeriodInDays;
    pool.common.endDate = _endDate;
    pool.common.hardCap = _hardCap;
    pool.common.inputToken = _inputToken;
    pool.common.rewardToken = _rewardToken;
  }
  ...
  function addLiquidityAndLock(
    ...
    uint256 _lpAmount;
    IUniswapV2Pair pair = IUniswapV2Pair(pools[_pid].common.inputToken);
    (address native, uint8 pos) = _isWrappedNative(pair);
    ...
    if (_lpAmount > 0) {
      _lockLp(_pid, msg.sender, _lpAmount, _token0Amt, _token1Amt);
    }
    return true;
  }
   ...
   *
   * @return {PoolExtended[]} length of the pools
   *
   */

  function getPools() external view returns (PoolExtended[] memory) {
    return pools;
  }

  /**
   *
   * @dev get all pools common info
   *
   *
   */
  ...
  {
    Pool memory pool = pools[_pid].common;
   ...
   *
   */
  function claim(uint16 _pid) public override returns (bool) {
  ...
  function payout(uint16 _pid, address _addr) public view override returns (uint256 reward) {
    PoolExtended memory pool = pools[_pid];
    UserExtended memory user = users[_pid][_addr];

    if (!canClaim(_pid, _addr)) return 0;

    uint32 mpy = pool.common.apy / 12;

    //1 month = 30 days
    reward = mpy.mul(pool.common.lockPeriodInDays).mul(user.common.totalInvested).div(1000).div(30);
  ...
  function canClaim(uint16 _pid, address _addr) public view override returns (bool) {
    ...
    return (block.timestamp >= user.common.depositTime + (pool.common.lockPeriodInDays * 1 days));
  }
   ...
   *
   */
  function ownsCorrectNFT(uint16 _pid, address _addr) public view override returns (bool) {
  ...
  function calcMultiplier(uint16 _pid, address _addr) public view override returns (uint16 multi) {
    ...
    if (nft.active && ownsCorrectNFT(_pid, _addr)) {
  ...
  function _claim(uint16 _pid, address _user) internal returns (bool) {
    PoolExtended storage pool = pools[_pid];
    UserExtended storage user = users[_pid][_user];

    if (canClaim(_pid, _user)) {
      uint256 amount = payout(_pid, _user);
      _safeTOKENTransfer(pool.common.rewardToken, _user, amount); //Sending out rewards
      user.common.totalClaimed = user.common.totalClaimed.add(amount);
      emit Claim(_user, amount, block.timestamp);
    }
    ...
    if (native != address(0)) {
      if (pos == 0) {
        (token0, token1) = _removeLiquidityETH(pair.token1(), _user, user.common.totalInvested);
      } else {
        (token0, token1) = _removeLiquidityETH(pair.token0(), _user, user.common.totalInvested);
      }
    } else {
      (token0, token1) = _removeLiquidity(_pid, _user, user.common.totalInvested);
    }

    emit Unlock(_pid, _user, user.common.totalInvested, block.timestamp);
    emit LPRemoved(_user, user.common.totalInvested, token0, token1);

    pool.common.totalDeposit = pool.common.totalDeposit.sub(user.common.totalInvested);
    pool.token0Amount = pool.token0Amount.sub(user.token0Amount);
    pool.token1Amount = pool.token1Amount.sub(user.token1Amount);

    user.common.totalInvested = 0;
    user.token0Amount = 0;
    user.token1Amount = 0;
  ...
  function _lockLp(
    ...
    PoolExtended storage pool = pools[_pid];
    UserExtended storage user = users[_pid][_sender];

    uint256 stopDepo = pool.common.endDate.sub(pool.common.lockPeriodInDays * 1 days);
    require(block.timestamp <= stopDepo, "Locking is disabled for this pool");

    user.common.totalInvested = user.common.totalInvested.add(_lpAmount);
    pool.common.totalDeposit = pool.common.totalDeposit.add(_lpAmount);

    pool.token0Amount = pool.token0Amount.add(_token0Amt);
    pool.token1Amount = pool.token1Amount.add(_token1Amt);
    user.token0Amount = user.token0Amount.add(_token0Amt);
    user.token1Amount = user.token1Amount.add(_token1Amt);
   ...
   *
   */
  function _walletOfOwner(address _contract, address _owner)
    internal
    view
  ...
  function _isWrappedNative(IUniswapV2Pair _pair)
    internal
    view
    returns (address native, uint8 pos)
  {
