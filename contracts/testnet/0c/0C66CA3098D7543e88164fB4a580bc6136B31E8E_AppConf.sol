...
contract AppConf is IAppConf, Ownable {
    ...
    // token0 -> token1 -> status, 0=notexists, 1=enabled, 2=disabled
    ...
    uint256 farmDeltaRate = 20; // price percent 1
    ...
    constructor() {
        ...
        // token
        address betToken = 0x0f91b06a6143DCe1AEeDa804e461ABd1622f1Be2;
        address usdtToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
        ...
        farmAddr.farmUser = 0x7ef220A3b4C41BF3948D6b44233Be3c2aB67D256;
        farmAddr.farmPair = 0x730Fee8368094c72b8d3B2e6CA36cF795cCfa421;
        farmAddr.farmReward = address(0);

        // pair
        allPairMap[betToken][usdtToken] = Model.Pair({
            token0: betToken,
            token0Symbol: IERC20Ex(betToken).symbol(),
            token0Decimals: IERC20Ex(betToken).decimals(),
            token1: usdtToken,
            token1Symbol: IERC20Ex(usdtToken).symbol(),
            token1Decimals: IERC20Ex(usdtToken).decimals(),
            status: 1
        });
        allPairs.push(allPairMap[betToken][usdtToken]);
    ...
    function setHashrateConf(uint8 category, Model.HashrateConf memory hc) public onlyOwner {
        ...
        hashrateConfMap[category].minReward = hc.minReward;
        hashrateConfMap[category].maxReward = hc.maxReward;
        hashrateConfMap[category].totalReward = hc.totalReward;
    ...
    function getFarmAddr() public view override returns(Model.FarmAddr memory) {
        return farmAddr;
    }
    ...
    function getFarmDeltaRate() public view override returns(uint256) {
        return farmDeltaRate;
    }

    function setFarmDeltaRate(uint256 _farmDeltaRate) public onlyOwner {
        farmDeltaRate = _farmDeltaRate;
    }
