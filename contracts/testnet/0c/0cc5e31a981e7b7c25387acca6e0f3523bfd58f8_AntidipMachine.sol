...
contract AntidipMachine is Context, Ownable , ReentrancyGuard {
    ...
    address public dev = 0xa7636B900194eB968F62bF6AE9D7f1720184e9F6;
    ...
    mapping(address => dailyWithdraw) public daily;
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        // daily withdraw 
        uint256 dailyStart = block.timestamp;
        uint256 deadline_daily = block.timestamp + 10 minutes;

        daily[msg.sender] = dailyWithdraw(msg.sender,dailyStart,deadline_daily);

        // Claim Setting
       uint256 claimTimeStart = block.timestamp;
       uint256 claimTimeEnd = block.timestamp + 10 minutes;
       ...
       // fees 
        ...
        // weekly withdraw 
       // uint256 dailyStart = block.timestamp;
       // uint256 deadline_daily = block.timestamp + 7 days;
    ...
    require(init, "Not Started Yet");    
    require(daily[msg.sender].deadline <= block.timestamp, "You cant withdraw");
    ...
    uint256 aval_withdraw2 = SafeMath.div(aval_withdraw,1); // divide the fees
