...
contract Commerce is ERC20{
   ...
   function totalBalance() public view returns (uint256) {
    ...
    function _buyItem(uint256 index, uint256 value, address customer) private  {
      ...
      Order({
            orderStatus:paymentStatus.ORDERED,
            buyer:payable(customer),
            seller:payable(_person.accountAddress), 
            amount:value, 
            itemId:index,
            orderId:orderId
            });
    ...
    function approveSale(uint256 index) external {
        ...
        commissionWallet.transfer(_commissionFee);
     ...
     function terminateSale(uint256 index) external {
        Order storage order_ = orders[index];
        require(order_.buyer == msg.sender, "you are not allowed to approve this");
        Person storage seller_ = persons[order_.seller];
    ...
    function withdraw(uint256 amount) external {
        ...
        require(amount >= user_.availableBalance);
