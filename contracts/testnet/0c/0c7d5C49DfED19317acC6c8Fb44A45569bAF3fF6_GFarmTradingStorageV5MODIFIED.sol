...
contract GFarmTradingStorageV5MODIFIED {
    ...
    function storeTradeInfo(Trade calldata _trade) external onlyTrading{
        openTrades[_trade.trader][_trade.pairIndex][_trade.index] = _trade;
    }

    function storeTradeContinue(address trader, uint pairIndex) external onlyTrading{

        openTradesCount[trader][pairIndex]++;
        tradesPerBlock[block.number]++;

        if(openTradesCount[trader][pairIndex] == 1){
            pairTradersId[trader][pairIndex] = pairTraders[pairIndex].length;
            pairTraders[pairIndex].push(trader);
        }
    }
