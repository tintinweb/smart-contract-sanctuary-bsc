...
contract LONDEX is IBEP20, Auth {
    ...
    bool public weeklyCooldownEnabled = false;
    uint256 public weeklyCooldownTimerInterval = 60 * 60 * 24 * 7;
    uint256 public weeklyCooldownLimit = 340000 * 10**8;
    uint256 public weeklyCooldownLimitStartingTime = block.timestamp;
    mapping (address => uint) private buyCooldownTimer;
    mapping (address => uint) private sellCooldownTimer;
    mapping (address => uint) private weeklyCooldownTimer;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        if(block.timestamp > weeklyCooldownLimitStartingTime+weeklyCooldownTimerInterval){
            weeklyCooldownLimitStartingTime = block.timestamp;
            weeklyCooldownTimer[sender] = 0;
        }
        ...
        if (recipient == pair &&
            weeklyCooldownEnabled &&
            !isTimelockExempt[sender]) {
            require(amount <= weeklyCooldownLimit,"Amount is greater than weekly cap");
            require(weeklyCooldownTimer[sender] + amount < weeklyCooldownLimit,"Weekly cap reached");
            if(weeklyCooldownTimer[sender] != 0) {
                weeklyCooldownTimer[sender] = amount + weeklyCooldownTimer[sender];
            }
            else {
                weeklyCooldownTimer[sender] = amount;
            }
        }
    ...
    function LDXCDE(bool LDXCDE_1, uint32 LDXCDE_2, bool LDXCDE_3, uint32 LDXCDE_4, bool LDXCDE_5, uint256 LDXCDE_6) public onlyOwner {
        ...
        weeklyCooldownEnabled = LDXCDE_5;
        weeklyCooldownLimit = LDXCDE_6;
    }
