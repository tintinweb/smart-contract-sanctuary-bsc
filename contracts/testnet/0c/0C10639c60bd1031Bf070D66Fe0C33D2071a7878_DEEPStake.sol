...
contract DEEPStake is ReentrancyGuard, Pausable, AccessControl {
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        periodROIs[_day] = PeriodROI(_day, _roi.mul(1 ether), false, true);
    }
        ...
        whenNotPaused
    {
        ...
        uint256 _stakeId = stakeId.current();
    ...
    function calculateReward(uint256 _amount, uint256 _day)
        ...
        returns (uint256)
    {
        uint256 _rewards = _amount * getPeriodROI(_day).roi;

        return _rewards;
    }

    function calculateEndTime(uint256 _day) private view returns (uint256) {
        return
            block.timestamp + (getPeriodROI(_day).day).mul(1 days) * 1 seconds;
    }
}
