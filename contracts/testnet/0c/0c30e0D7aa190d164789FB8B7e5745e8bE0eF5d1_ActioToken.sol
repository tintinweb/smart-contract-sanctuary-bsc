...
contract ActioToken is Context, Ownable, IERC20, IERC20Metadata
{
    ...
    string private _name = "Actio91"; // testnet
    //string private _name = "Actio";  

    string private _symbol = "AIO91"; // testnet
    ...
    function _transferTax(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == _pair) { 
            ...
            if (!_excludedSellFee[sender] || !_excludedBuyFee[recipient]) {
                ...
                // ---- konsepnya gak usah paka takeFee karena routing itu harus disimpan dalam address(this)

                // kurangi balance pengirim
                _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");

                uint256 thefee = _tax.mul(amount).div(_taxDivider);   // fee yang didapatkan taxCollector
                uint256 amountToSend = amount.sub(thefee);            // yang dikirim kepada penerima setelah dikurangi fee

                // pindahkan fee ini kepada taxCollector
                _balances[_taxCollector] = _balances[_taxCollector].add(thefee);

                // pindahkan amount setelah dikurangi fee kepada penerima
