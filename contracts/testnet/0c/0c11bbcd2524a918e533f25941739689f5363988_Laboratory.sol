...
contract Laboratory is ERC721, Ownable {
    ...
    constructor(uint256 _preMint, address _NFTreserve, string memory _cid)
        ERC721("LABO_NFT", "LAB")
    {
    ...
    modifier onlyAuth() {
        require(
            msg.sender == openAndClose ||
                msg.sender == laboManagement,
    ...
    function updateInterfaces() external {
        openAndClose = gameAddresses.getAddressOf(AddressesInit.Addresses.OPEN_AND_CLOSE);
        laboManagement = gameAddresses.getAddressOf(AddressesInit.Addresses.LABO_MANAGEMENT);
    ...
    function updateCreditLastUpdate(uint256 _tokenId) external onlyAuth returns (bool) {
        ...
        returns (bool)
    {
        ...
        require(_numberOfWorkingSpots[_tokenId] + _quantity <= 10, "Max spots already hit");
