...
interface LendingPool:
    ...
    def withdraw(_share: uint256): nonpayable
...
USDT_LENDING_POOL: constant(address) = 0x3D6875FC972aE5bAD1178e4D0cF943651F66D0f1
BUSD_LENDING_POOL: constant(address) = 0xCE6809eE922071AbbA452D7293C33E4a237534A5
USDC_LENDING_POOL: constant(address) = 0x03D4a62709cC07D2adA2cde2D165f30Da6255c3A
...
def __init__(
    ...
    # approve transfer of underlying coin to alpaca lending pool
    _response: Bytes[32] = raw_call(
        _underlying_coins[0],
        concat(
            method_id("approve(address,uint256)"),
            convert(USDT_LENDING_POOL, bytes32),
    ...
    _response = raw_call(
        ...
        concat(
            method_id("approve(address,uint256)"),
            convert(BUSD_LENDING_POOL, bytes32),
            convert(MAX_UINT256, bytes32)
        ),
        max_outsize=32
    )
    ...
    _response = raw_call(
        ...
        concat(
            method_id("approve(address,uint256)"),
            convert(USDC_LENDING_POOL, bytes32),
            convert(MAX_UINT256, bytes32)
        ),
        max_outsize=32
    )
...
def add_liquidity(_amounts: uint256[N_COINS], _min_mint_amount: uint256, _use_underlying: bool = False) -> uint256:
    ...
    if _use_underlying:
        ...
        for i in range(N_COINS):
            ...
            if amount != 0:
                ...
                # deposit to alphaca lending pool
                if i == 0:
                    raw_call(
                        USDT_LENDING_POOL,
                        concat(
                            method_id("deposit(uint256)"),
                            convert(amount, bytes32),
                        )
                    )
                if i == 1:
    ...
    # Mint pool tokens
    HyperToken(lp_token).mint(msg.sender, mint_amount)
...
def exchange_underlying(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
    ...
    # deposit to alpaca lending pool
    if i==0:
        raw_call(
            USDT_LENDING_POOL,
            concat(
    ...
    if j==0:
        LendingPool(USDT_LENDING_POOL).withdraw(dy)
    if j==1:
        LendingPool(BUSD_LENDING_POOL).withdraw(dy)
    if j==2:
        LendingPool(USDC_LENDING_POOL).withdraw(dy)
...
def remove_liquidity(
    ...
    HyperToken(lp_token).burnFrom(msg.sender, _amount)  # dev: insufficient funds
    ...
    for i in range(N_COINS):
        ...
        if _use_underlying:
            if i == 0:
                LendingPool(USDT_LENDING_POOL).withdraw(value)
            if i == 1:
                LendingPool(BUSD_LENDING_POOL).withdraw(value)
            if i == 2:
                LendingPool(USDC_LENDING_POOL).withdraw(value)
        else:
...
def remove_liquidity_imbalance(
    ...
    HyperToken(lp_token).burnFrom(msg.sender, token_amount)  # dev: insufficient funds
    ...
    for i in range(N_COINS):
        ...
        if amount != 0:
            if _use_underlying:
                if i == 0:
                    LendingPool(USDT_LENDING_POOL).withdraw(amount)
                if i == 1:
                    LendingPool(BUSD_LENDING_POOL).withdraw(amount)
                if i == 2:
                    LendingPool(USDC_LENDING_POOL).withdraw(amount)
            else:
...
def remove_liquidity_one_coin(
    ...
    HyperToken(self.lp_token).burnFrom(msg.sender, _token_amount)  # dev: insufficient funds
