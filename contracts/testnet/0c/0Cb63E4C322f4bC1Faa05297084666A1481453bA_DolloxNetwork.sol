...
function updateBlacklist(address[] memory addresses, bool isBlacklisted) public onlyOwner {
    for (uint i = 0; i < addresses.length; i++) {
        blacklist[addresses[i]] = isBlacklisted;
        emit BlacklistUpdated(addresses[i], isBlacklisted);
    }
}
