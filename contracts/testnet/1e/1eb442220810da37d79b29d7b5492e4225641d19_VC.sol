...
contract VC is ERC20 {
    ...
    address private _fundAddress = address(0x9496BD2cFf5Ae32c49AFBe67a7319Fe970B79Cb4);
    // address public _poolAddress = address(0x000000000000000000000000000000000000dEaD);
    ...
    uint256 public  MAX_STOP_FEE_TOTAL = 189000000 * 10**18;
    uint256 public  _TotalBurn = 0;

    constructor(address tokenOwner) ERC20("VC", "VC") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from != uniswapV2Pair && to != uniswapV2Pair){
				...
				// 	if(balanceOf(from) == amount){amount = amount.div(100000).mul(99999);}
				// 	_splitOtherToken();
				// }else{
				// 	if(balanceOf(from) == amount){amount = amount.div(100000).mul(99999);}
				// 	takeFee = false;
        ...
        if (takeFee) {
            if(_TotalBurn<MAX_STOP_FEE_TOTAL){
            //销毁
            ...
            //销毁数量计数
            _TotalBurn = _TotalBurn.add(amount.div(100).mul(2));
            }else{
            //销毁
			//super._transfer(from, _destroyAddress, amount.div(100).mul(2));
            //营销钱包
	...
	uint256 public minAmount = 10**20;
    ...
    function destory(uint256 amount) public onlyOwner{
        _TotalBurn = _TotalBurn.add(amount);
        super._transfer(msg.sender, _destroyAddress, amount);
    }
    function get_TotalBurn() public view onlyOwner returns(uint256){
        return _TotalBurn;
    }
