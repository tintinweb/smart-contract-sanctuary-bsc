...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(isOwner(_msgSender()), "You are not an owner");
        _;
    }
    ...
    function isOwner(address account) public view returns(bool) {
        for (uint i=0; i<owners.length; i++) {
            if (owners[i].account == account && owners[i].activatedAt <= block.timestamp) {
                return true;
            }
        }
        return false;
    ...
    function addOwner(address account) public onlyOwner {
        for (uint256 i=0; i<owners.length; i++) {
            if (owners[i].account == account) {
                revert("Ownable: The owner already exists.");
            }
        }
        ...
        owners.push(Owner({
            ...
            addedBy: _msgSender(),
            addedAt: block.timestamp,
            activatedAt: block.timestamp + 1 days
        }));
    }
    ...
    function removeOwner(address account) public onlyOwner {
        require(_msgSender() != account, "Ownable: Impossible to delete yourself.");
 ...
 * @title HEN Token
 */
contract EGGOToken is ERC20("Rich Hes", "HES"), Ownable {
    /**
    ...
    function mint(address account, uint amount) public onlyOwner() {
        require(amount <= (totalAvailable() - totalSupply()), "Too many tokens to mint");
