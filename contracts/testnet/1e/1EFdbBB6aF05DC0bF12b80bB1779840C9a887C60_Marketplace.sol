...
contract Marketplace is IERC721Receiver, Whitelist, ReentrancyGuard {
    ...
    IItemCounter public itemCounter; // The external 
    ...
    constructor (
        ...
        itemCounter = IItemCounter(_itemCounter);
    ...
    function getItems(address _user, bool _realtime) external view returns (
        ...
        if (_realtime == true) {
            ...
            return (
                itemCounter.totalItemsOfTier(_user, 1), 
                itemCounter.totalItemsOfTier(_user, 2), 
                itemCounter.totalItemsOfTier(_user, 3), 
                itemCounter.totalItemsOfTier(_user, 4), 
                itemCounter.totalItemsOfTier(_user, 5), 
                itemCounter.totalItemsOfTier(_user, 6)
            );
        }
    }
    ...
    function getUserTotalItems(address _user) external view returns (uint256) {
        return (
            itemCounter.totalItemsOfTier(_user, 1) + itemCounter.totalItemsOfTier(_user, 2) + itemCounter.totalItemsOfTier(_user, 3) + 
            itemCounter.totalItemsOfTier(_user, 4) + itemCounter.totalItemsOfTier(_user, 5) + itemCounter.totalItemsOfTier(_user, 6)
        );
    }
    ...
    function woolPerSecondOf(address _user) public view returns (uint256) {
        (uint256 _tier1, uint256 _tier2, uint256 _tier3, uint256 _tier4, uint256 _tier5, uint256 _tier6) = itemCounter.viewItems(_user);
    ...
    function listItem(address _contract, uint256 _tokenId, uint256 _markup) ifTradingActive() public nonReentrant {
        ...
        uint256 _claimTotal = _claimWool(msg.sender);
        itemCounter.updateItems(msg.sender);
