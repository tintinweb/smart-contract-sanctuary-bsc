...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  event GameFinished(address indexed user, uint mission, uint userReward, uint currentEnergy, bool dailyQuestCompleted, uint dayPassed, bytes signature);
  ...
  function finishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature, uint _timestamp) external {
    bool firstCompleteMission = _validateFinishGame(_mission, _heroIds, _itemIds, _signature, _timestamp);
    (uint energy, bool firstConsumeInDay) = _loadEnergyAndFirstConsumeInDay(_mission);
    if (firstConsumeInDay) {
      totalWinInDay[msg.sender] = 1;
    } else {
      totalWinInDay[msg.sender]++;
    }
    (uint userReward, bool dailyQuestCompleted) = _giveReward(_mission, firstCompleteMission, _heroIds);
    (uint dayPassed) = rewardManager.getDaysPassed();
    emit GameFinished(msg.sender, _mission, userReward, energy, dailyQuestCompleted, dayPassed, _signature);
  }
  ...
  function _loadEnergyAndFirstConsumeInDay(uint _mission) private returns (uint, bool) {
    bool firstConsumeInDay = energyManager.updateEnergy(msg.sender, energyConsumptions[_mission]);
    uint energy = energyManager.getUserCurrentEnergy(msg.sender);
    return (energy, firstConsumeInDay);
  }

  function _giveReward(uint _mission, bool _firstCompleteMission, uint[] calldata _heroIds) private returns (uint, bool) {
    uint userReward;
    bool dailyQuestCompleted;
    uint reward = _firstCompleteMission ? missions[_mission].mainReward : missions[_mission].subReward;
    (userReward, dailyQuestCompleted) = rewardManager.addPVEReward(_mission, msg.sender, reward);
    uint profitShared = userReward / _heroIds.length;
//    for(uint i = 0; i < _heroIds.length; i++) {
//      heroNft.experienceUp(_heroIds[i], missions[_mission].experience);
//      heroNft.increaseTotalProfited(_heroIds[i], profitShared);
//    }
    emit RewardedRecorded(msg.sender, userReward);
    return (userReward, dailyQuestCompleted);
  }

  function _validateFinishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature, uint _timestamp) private returns (bool) {
    ...
    bytes32 digest = _buildDigest(_mission, _heroIds, _timestamp);
    nonces[msg.sender]++;
  ...
  function _validateHeroRight(uint[] calldata _heroIds) private view {
    for (uint i = 0; i < _heroIds.length - 1; i++) {
      ...
      _doValidateHeroRight(_heroIds[i]);
    }
    _doValidateHeroRight(_heroIds[_heroIds.length - 1]);
  }

  function _doValidateHeroRight(uint _heroId) private view {
    bool reachMaxProfit = heroNft.reachMaxProfit(_heroId);
    require(!reachMaxProfit, "PVE: hero reached max profit");
    address owner = heroNft.ownerOf(_heroId);
    if (owner != msg.sender) {
      require(marketPlace.currentRentingHero(_heroId) == msg.sender, "PVE: hero invalid");
    }
  }

  function _validateItemRight(uint[] calldata _itemIds) private view {
    if (_itemIds.length == 0) return;
    for (uint i = 0; i < _itemIds.length - 1; i++) {
      require(_itemIds[i] < _itemIds[i + 1], "PVE: item id is duplicated or wrong order");
      _doValidateItemRight(_itemIds[i]);
    }
    _doValidateItemRight(_itemIds[_itemIds.length - 1]);
  }

  function _doValidateItemRight(uint _itemId) private view {
    address owner = itemNft.ownerOf(_itemId);
    if (owner != msg.sender) {
      require(marketPlace.currentRentingItem(_itemId) == msg.sender, "PVE: item invalid");
    }
  }

  function _buildDigest(uint _mission, uint[] calldata _heroIds, uint _timestamp) private view returns (bytes32) {
    if (_heroIds.length == 3) {
      return _hashTypedDataV4(keccak256(abi.encode(
          keccak256("FinishGamePVE(address user,uint256 nonce,uint256 mission,uint256 timestamp,uint256 hero1,uint256 hero2,uint256 hero3)"),
          msg.sender,
          nonces[msg.sender],
          _mission,
          _timestamp,
      ...
      return _hashTypedDataV4(keccak256(abi.encode(
          keccak256("FinishGamePVE(address user,uint256 nonce,uint256 mission,uint256 timestamp,uint256 hero1,uint256 hero2,uint256 hero3,uint256 hero4)"),
          msg.sender,
          nonces[msg.sender],
          _mission,
          _timestamp,
      ...
      return _hashTypedDataV4(keccak256(abi.encode(
          keccak256("FinishGamePVE(address user,uint256 nonce,uint256 mission,uint256 timestamp,uint256 hero1,uint256 hero2,uint256 hero3,uint256 hero4,uint256 hero5)"),
          msg.sender,
          nonces[msg.sender],
          _mission,
          _timestamp,
  ...
  function setContracts(address _heroNft, address _itemNft, address _energyManager, address _rewardManager, address _marketPlace) external onlyMainAdmin {
...
interface IGameNFT is IERC721Upgradeable {
  ...
  function increaseTotalProfited(uint _tokenId, uint _amount) external;
  function increaseTotalFOTAProfited(uint _tokenId, uint _amount) external;
