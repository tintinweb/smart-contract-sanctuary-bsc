...
contract MoonStepToken  is ERC20Burnable, Ownable, AccessControlEnumerable {
    ...
    constructor (address _beneficiary, address _factoryAddress, address _pairToken) ERC20("Moon Step", "$MSA") {
        ...
        _setupRole(OPERATOR_ROLE, msg.sender);
        _mint(msg.sender, 10_000_000_000 ether); // 10.000.000.000 $MSA
        mintEnabled = true;
    }
    ...
    */
    function _getValues(uint256 transferAmount, uint taxFee) private view returns (uint256, uint256, uint256) {
        ...
        uint256 burnAmount = transferAmount.mul(burnFee).div(10**2);
    ...
    function setTaxFeePercent(uint256 _buyTaxFee, uint256 _sellTaxFee, uint256 _burnFee) external onlyOperator {
        ...
        require(burnFee <= MAX_FEE_PERCENTAGE, "Burn Fee reached the maximum limit");
