...
contract ProxyContract is OwnableUpgradeable {
    ...
    function addVaribaleAtSlot(uint256 _slotNumber, uint256 _lastSlotNumber) external {
        require(_slotNumber <= _lastSlotNumber, "ProxyContract: This slot is already available");
        uint256 realSlotNumber = _firstSlot() + _slotNumber;
        uint256 currentSlot = _firstSlot() + _lastSlotNumber;
        while(realSlotNumber - 1 != currentSlot) {
            _swapValues(currentSlot , currentSlot + 1);
            currentSlot--;
        }
    }
    ...
    function _swapValues(uint256 _slot1, uint256 _slot2) private {
        assembly {
            let value1 := sload(_slot1)
            sstore(_slot1, sload(_slot2))
            sstore(_slot2, value1)
        }
    }
