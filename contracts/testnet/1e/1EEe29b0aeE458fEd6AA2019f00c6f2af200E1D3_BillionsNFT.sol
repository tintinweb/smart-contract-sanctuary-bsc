...
contract BillionsNFT is ERC4907, BillionsBaseContract, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) private nftInfos;
    mapping(uint256 => mapping(uint256 => bool)) public usedSymbolId;
    mapping(address => uint256[]) public nftIdsOfPlayer;

    mapping(uint256 => uint256[]) public availableSymbolIds;
    ...
    event _BillionsNftMint(uint256 _type, uint256 _nftId, uint256 _companyId, address _owner);
    ...
    external payable Mintable returns (uint256) {
        ...
        require(!usedSymbolId[_type][_companyId], "not available");
        ...
        nftInfos[_nftId] = _companyId;
        usedSymbolId[_type][_companyId] = true;
        nftIdsOfPlayer[msg.sender].push(_nftId);

        for(uint256 i = 0; i < _availableCount; i ++) {
            if(symbolIds[i] == _companyId) {
        ...
        emit _BillionsNftMint(_type, _nftId, _companyId, msg.sender);
        return _nftId;
    }

    function mintByERC20(uint256 _type, uint256 _companyId) 
    ...
    function mintByPack(uint256 _type) public  {
        ...
        for(uint256 idx = 0; idx < 3; idx ++) {
            ...
            uint256 _companyId = availableSymbolIds[_type][_rand % _availableCount];
            ...
            emit _BillionsNftMint(_type, _nftId, _companyId, tx.origin);
        }
    }
    ...
    function tokenURI(uint256 _nftId) public view virtual override returns (string memory) {
        ...
        return bytes(_baseURI).length > 0 ? string(abi.encodePacked(_baseURI, nftInfos[_nftId])) : "";
    }
...
contract BillionsBaseContract is Ownable {
    ...
    * calculate the amount of matic from USDC 
    ...
    function getMaticAmountFromUSDC(uint256 amount) public view returns(uint) {
        ...
        uint256 bnbAmount = uint256( (amount * (10 ** decimals) * (10 ** maticDecimals)) / bnbPrice ) ;
        return bnbAmount;
    }
    ...
    * get price of 1 Matic for test
    ...
    function getMaticPrice() public view returns(int) {
        ...
        int usd = answer * int(10 ** maticDecimals); // per 1 Matic (18 decimals)
        return usd;
    }
