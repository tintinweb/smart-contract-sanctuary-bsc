...
contract C3SwapRouter1 is I3SwapRouter1 {
  ...
  function _swap(
    ...
    for (uint i; i < path.length - 2; i++) {
      ...
      uint amount2Out;
      if (token0 == output) {
        (amount0Out, amount1Out, amount2Out) = (amountOut, uint(0), uint(0));
      } else if (token1 == output) {
        (amount0Out, amount1Out, amount2Out) = (uint(0), amountOut, uint(0));
      } else {
        (amount0Out, amount1Out, amount2Out) = (uint(0), uint(0), amountOut);
      }
