...
contract FantomXenoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("FantomXeno_Dividend_Tracker", "FantomXeno_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "FantomXeno_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "FantomXeno_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FantomXeno contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FantomXeno_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "FantomXeno_Dividend_Tracker: Cannot update claimWait to same value");
...
contract FantomXeno is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    FantomXenoDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 500000000 * (10**18);
    uint256 public maxWalletToken = 1000000000 * (10**18);
    uint256 public swapTokensAtAmount = 30 * 10**5 * (10**18);


    uint256 public FantomRewardsFee;
    ...
    function setFee(uint256 _fantomRewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        FantomRewardsFee = _fantomRewardFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;

        totalFees = FantomRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy
    }

    function setExtraFeeOnSell(uint256 _extraFeeOnSell) public onlyOwner {
        extraFeeOnSell = _extraFeeOnSell; // extra sell fee
    }
    ...
    constructor() ERC20("FantomXeno", "FTMX") {
        
        FantomRewardsFee = 3;
        liquidityFee = 1;
        extraFeeOnSell = 3; // extra sell fee
        marketingFee = 5;
        marketingWallet = payable(0xF58b73ecbd2a56F9211B1171705c318ac6130E35);

    
        totalFees = FantomRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new FantomXenoDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "FantomXeno: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "FantomXeno: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FantomXeno: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "FantomXeno: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "FantomXeno: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "FantomXeno: Cannot update gasForProcessing to same value");
