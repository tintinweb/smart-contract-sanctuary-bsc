...
contract TokenStaking is Ownable {
    ...
    function getUserDepositDetail(uint256 id,address _address) public view returns(uint256 amount,uint256 depositeTime,bool isWithdraw){
        stack storage users = Stack[_address];
        amount = users.deposites[id].amount;
        depositeTime = users.deposites[id].depositeTime;
    ...
    function calclulateReward(address _address) public view returns(uint256){        
        ...
        for(uint256 i=0;i<users.deposites.length;i++){
            ...
            uint256 time = block.timestamp.sub(users.deposites[i].depositeTime);
