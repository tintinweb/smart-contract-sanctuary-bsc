...
contract Lottery is Ownable, Initializable, Testable {
    ...
    struct LottoInfo {
        ...
        uint256 canRewardTimestamp;     // Block timestamp for claim the reward
        uint16[] winningNumbers;     // The winning numbers
    }

    // All the needed info of a winning 
    struct WinningInfo{
        uint256 lotteryId;             // ID for lotto
        bool claimed;                  // status for this user has claim this reward or not
        address user;                  // address of winning user
    }

    // Lottery ID's to info
    mapping(uint256 => LottoInfo) internal  allLotteries_;

    // lottery ID's to reward level to number of winnings in that level
    mapping(uint256 =>mapping(uint8=>uint16)) internal allWinningsCount_;

    // lottery' ID to address to pricebrand to amount 
    mapping(uint256 => mapping( address => mapping(uint8 => uint16) ) ) internal userTicketBrandWinningCount_;

    // Lottery ID's to user address to winning amount
    mapping(uint => mapping(address=>WinningInfo)) internal userWinningAmounts_;
    //-------------------------------------------------------------------------
    // EVENTS
    //-------------------------------------------------------------------------
    ...
    event UpdatePricingPool(uint256 lotteryId, uint256 amount);

    event UserWinningPrice(uint256 lotteryId, address user, uint256 amount);
 
    //event for testing only
    event LogUserWin(uint256 lotteryId, address indexed user, uint8 indexed brand, uint16 perOfPool);
    ...
    function getBasisAllWinCountInfo(uint256 _lotteryId,uint8 level) external view returns(uint16 ){
        return allWinningsCount_[_lotteryId][level];
    }

    function getBasisUserTicketBrandWinCount(uint256 _lotteryId,uint8 level) external view returns(uint16){
        return userTicketBrandWinningCount_[_lotteryId][msg.sender][level];
    }

    function getUserWinAmount(uint256 _lotteryId) external view returns(WinningInfo memory){
        return userWinningAmounts_[_lotteryId][msg.sender];
    }
        ...
        onlyRandomGenerator()
    {
        ...
        if(requestId_ == _requestId) {
            ...
            allLotteries_[_lotteryId].canRewardTimestamp = allLotteries_[_lotteryId].closingTimestamp  + 10*60;
        }
        ...
        returns(uint256 lotteryId)
    {
        ...
        uint256 _canRewardTimestamps = 0;
    ...
    function updateLotteryStatus (uint256 _lotteryId, Status _status) external {
        allLotteries_[_lotteryId].lotteryStatus = _status;
    }
        ...
        notContract()
    {
        ...
        // accumulate pricing pool
        allLotteries_[_lotteryId].prizePoolInCake+= costWithDiscount;
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() returns(
        uint8
    ) {
        ...
        // ticket number
        uint16[] memory ticketNums =nft_.getTicketNumbers(_tokenId);
        //winning numbers =
        uint16[] memory winningNums = allLotteries_[_lotteryId].winningNumbers;
        ...
        allWinningsCount_[_lotteryId][matchingNumbers]+=1;

        userTicketBrandWinningCount_[_lotteryId][msg.sender][matchingNumbers]+=1;

        uint256 lotteryID = _lotteryId;
        bool status = false;
        address user = msg.sender;

        WinningInfo memory winningInfo =  WinningInfo(
            lotteryID,
            status,
            user
        );
        userWinningAmounts_[_lotteryId][msg.sender] = winningInfo;
        ...
        notContract()
  {
        ...
        require(
            allLotteries_[_lotteryId].canRewardTimestamp > getCurrentTime(),
            "Overtime to claim ticket"
        );
        ...
        for (uint256 i = 0; i < _tokeIds.length; i++) {
            ...
            // Update amount of winning number
            allWinningsCount_[_lotteryId][matchingNumbers] +=1;

            // upadte winning tickets number of a user
            userTicketBrandWinningCount_[_lotteryId][msg.sender][matchingNumbers]+=1;
        ...
        userWinningAmounts_[_lotteryId][msg.sender] = winningInfo;
        // store the winning price of a user
        // userWinningAmounts_[_lotteryId][msg.sender] = winningInfo;
    }


    function claimRewardPrice (uint256 _lotteryId) external notContract() {
        // check if in time to claim
        require(getCurrentTime()>= allLotteries_[_lotteryId].canRewardTimestamp, "Over time to claim winiing reward");


        // check if lottery finish to withdraw token
        require(allLotteries_[_lotteryId].lotteryStatus == Status.CanDrawToken,
            "wait until lottery finish to withdraw token"
        );

        WinningInfo storage winInfo = userWinningAmounts_[_lotteryId][msg.sender];
        
        // check if the user identify
        require(msg.sender == winInfo.user,"Invalid authenticate");

        // check if this user has claimed yet
        require(winInfo.claimed== false, "You has already claimed the reward");

        // check valid lottery size
        require(sizeOfLottery_ >0, "Invalid lottery size");

        uint256 totalWinPrice = 0;
        for (uint8 i=1 ;i<= sizeOfLottery_;i++){
            //numbers of winning tickets in this brand of this user
            uint16 numberOfWinTickets = userTicketBrandWinningCount_[_lotteryId][msg.sender][i];

            if(numberOfWinTickets>0){
                emit LogUserWin(
                    _lotteryId,
                    msg.sender,
                    i,
            ...
            // total of user has won on this brand
            uint16 totalPeopleWinInThisBrand = allWinningsCount_[_lotteryId][i];

            uint256 perOfPool = allLotteries_[_lotteryId].prizeDistribution[i-1];
            uint256 sum = perOfPool.mul(allLotteries_[_lotteryId].prizePoolInCake);
            sum = sum.div(100);
            sum = sum.div(totalPeopleWinInThisBrand);
            totalWinPrice += sum;
            }
            
        }
        cake_.safeTransfer(address(msg.sender), totalWinPrice ); 
        userWinningAmounts_[_lotteryId][msg.sender].claimed = true;
        emit UserWinningPrice(
            _lotteryId,
            msg.sender,
            totalWinPrice
        );
        allLotteries_[_lotteryId].prizePoolInCake=allLotteries_[_lotteryId].prizePoolInCake.sub(totalWinPrice);
    }
