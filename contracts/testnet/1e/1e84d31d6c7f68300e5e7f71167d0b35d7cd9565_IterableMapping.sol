...
contract Fart is ERC20, Ownable {
    ...
    FartDividendTracker public dividendTracker;
    ...
    uint256 public _BuyBackFartFee = 1;
  
    uint256 public _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackFartFee);
    
    uint256 public _SellETHRewardsFee = 4;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 5;
    uint256 public _SellBackFartFee = 1;
    uint256 public _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackFartFee);
    
    uint256 public maxWalletToken = 10000 * (10**9) * (10**18); //1%

    address public _marketingWalletAddress = 0x09b2336087a76490cc000d4096f666d2c1A9DBb8;
    address public _FartWalletAddress = 0xEeFeDe36a61d32113F5fB19fDFE9b5aE9C4633dE;
    ...
    constructor() ERC20("Fart", "FRT") {
    	...
    	dividendTracker = new FartDividendTracker();
        ...
        dividendTracker.excludeFromDividends(_FartWalletAddress);
        ...
        excludeOrIncludeFromFees(_FartWalletAddress, true);
        ...
        excludeOrIncludeFromMaxTx(_FartWalletAddress, true);
        ...
        excludeOrIncludeFromMaxWallet(_FartWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Fart: The dividend tracker already has that address");

        FartDividendTracker newDividendTracker = FartDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Fart: The new dividend tracker must be owned by the Fart token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Fart: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Fart: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "Fart: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "Fart: Account is already the value of 'excluded'");
    ...
    function setFartWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
        _FartWalletAddress = wallet;
    }

      function setBuyFees(uint256 buyEthRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackFartFee) external onlyOwner
    {
        _BuyETHRewardsFee = buyEthRewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _BuyBackFartFee = buyBackFartFee;
        _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackFartFee);

    }

    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackFartFee) external onlyOwner
    {
        _SellETHRewardsFee = sellEthRewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        _SellBackFartFee = sellBackFartFee;
        _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackFartFee);

    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Fart: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "Fart: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "Fart: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_SellMarketingFee.add(_SellBackFartFee)).div(_SellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(_SellMarketingFee).div(_SellBackFartFee.add(_SellMarketingFee));
            uint256 developmentShare = newBalance.sub(marketingShare);
            payable(_marketingWalletAddress).transfer(marketingShare);
            payable(_FartWalletAddress).transfer(developmentShare);
...
contract FartDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Fart_Dividend_Tracker", "Fart_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Fart_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Fart_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Fart contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Fart_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Fart_Dividend_Tracker: Cannot update claimWait to same value");
