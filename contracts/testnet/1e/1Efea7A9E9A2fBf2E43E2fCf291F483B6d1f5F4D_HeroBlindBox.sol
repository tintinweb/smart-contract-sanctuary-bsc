...
contract HeroBlindBox is Ownable, ReentrancyGuard {
    ...
    function setBoxInfo(address _nft, uint256[] memory _propertyIdList, uint256[] memory _numList, address _payToken, uint256 _payAmount, uint256 _startAt, uint256 _endAt) public nonReentrant onlyAdmin {
        ...
        for (uint256 _i = 0; _i < _propertyIdList.length; _i++) {
            ...
            require(_numList[_i] > 0, "NUM_ERROR");
        }
    ...
    function boxPurchase(uint256 _amount) public payable nonReentrant {
        ...
        boxNFTSupply = boxNFTSupply.sub(_amount);
        ...
        uint256[] memory _tempPropertyIdIndex = new uint256[](_amount);


        for (uint256 _i = 0; _i < _amount; _i++) {
            uint256 _propertyIdIndex = randomPropertyIdIndex();
            propertyIdNumList[_propertyIdIndex]--;
            _tempPropertyIdIndex[_i] = _propertyIdIndex;
        }
        ...
        for (uint256 _i = 0; _i < _amount; _i++) {
            uint256 propertyId = propertyIdList[_tempPropertyIdIndex[_i]];
            uint256 _tokenID = INFTcustom(boxNFT).mintItem(msg.sender, propertyId);
            _tokenIDList[_i] = _tokenID;
            _propertyIdList[_i] = propertyId;
        }

        emit BoxPurchase(msg.sender, boxNFT, _amount, boxPayToken, boxPayAmount, boxPayAmountTotal, _tokenIDList, _propertyIdList);
    }

    function randomPropertyIdIndex() internal view returns (uint256){
        uint256 _total = 0;
        uint256 _key = 0;
        for (uint256 _i = 0; _i < propertyIdNumList.length; _i++) {
            _total += propertyIdNumList[_i];
        }
        uint256[] memory propertyIdNumKeyList = new uint256[](_total);
        for (uint256 _i = 0; _i < propertyIdNumList.length; _i++) {
            for (uint256 _j = propertyIdNumList[_i]; _j > 0; _j--) {
                propertyIdNumKeyList[_key] = _i;
                _key++;
            }
        }

        uint256 _length = propertyIdNumKeyList.length - 1;
        uint256 _index = propertyIdNumKeyList[rand(_length)];
