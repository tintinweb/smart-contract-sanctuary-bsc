...
interface IShadowFiNFT {
    function mint(uint256 _mintAmount, address _receiver) external;
...
contract ShadowFiDonationPortal is Ownable, ReentrancyGuard {
    ...
    function donateBNB() payable public nonReentrant {
        ...
        uint8 tier = getTierByCost(msg.value * (convertedPrice / 1e8) / 1e18);
    ...
    function donateBUSD(uint256 _amountToken) nonReentrant public {
        require((_amountToken >= minBUSD), "You must donate more than the minimum");
        ...
        (IERC20(BUSD)).approve(address(pancakeRouter), _amountToken);
        (IERC20(BUSD)).transferFrom(address(msg.sender), address(this), _amountToken);
        ...
        uint8 tier = getTierByCost(_amountToken / 1e18);
        if (tier <= 3 && !nftContract.mintEnd() && !getTierMintedStatus(msg.sender, tier)) {
           mintedTier[msg.sender][tier] = true;
           nftContract.mint(1, msg.sender);
        }
    ...
    function getTierByCost(uint256 _cost) public view returns (uint8) {
        for (uint8 i = 3; i >= 0; i--) {
