...
contract BalanceFetcher is Ownable {
    ...
    function combinedValueOfLotto(address[] calldata tokens) external view returns (uint256 total) {
        return combinedValueOf(tokens, ILottoManager(lottoManager).currentLotto());
    }
    ...
    function valueOfLotto() external view returns (uint256 total) {
        ...
        uint len = tokens.length;
        for (uint i = 0; i < len;) {
            total += valueOfDestination(tokens[i], ILottoManager(lottoManager).currentLotto());
            unchecked { ++i; }
        }
    }

    function getBalances() external view returns (uint256[] memory) {
        return _getBalancesForListAtDestination(ILottoManager(lottoManager).currentLotto(), allAvailableTokens());
    }

    function getBalancesForList(address[] calldata tokens) public view returns (uint256[] memory) {
        return getBalancesForListAtDestination(ILottoManager(lottoManager).currentLotto(), tokens);
    }
    ...
    function getBalancesForListAtDestination(address destination, address[] calldata tokens) public view returns (uint256[] memory) {
        ...
        for (uint i = 0; i < len;) {
            balances[i] = IERC20(tokens[i]).balanceOf(address(destination));
            unchecked { ++i; }
        }
        return balances;
    }
