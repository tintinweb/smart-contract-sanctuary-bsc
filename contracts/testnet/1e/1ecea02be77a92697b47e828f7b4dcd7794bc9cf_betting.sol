...
contract betting is Ownable {
    ...
    function getBettingStatus(uint256 _betting_ID) public view returns (uint8) {
        ...
        if((block.timestamp >= betting_info[_betting_ID].start_date) && (block.timestamp <= betting_info[_betting_ID].end_date)) {
            return 2;   // betting is actived
        }

        if(block.timestamp >= betting_info[_betting_ID].end_date) {
            return 3;   //  betting is ended
        }
    }
    ...
    ///
    function openNewBetting(uint256 _startDate, uint256 _endDate) public onlyOwner {
        betting_ID += 1;
        betting_info[betting_ID].start_date = _startDate;
        betting_info[betting_ID].end_date = _endDate;
