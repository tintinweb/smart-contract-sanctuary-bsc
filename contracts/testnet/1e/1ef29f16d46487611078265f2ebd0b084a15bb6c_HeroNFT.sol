...
contract HeroNFT is BaseNFT {
  ...
  event TotalFOTAProfitedUpdated(
    uint tokenId,
  ...
  function updateTotalProfited(uint _tokenId, uint _totalProfited) onlyMintAdmin external {
    (uint maxProfitable,) = _getMaxProfitable(_tokenId);
    if (_totalProfited > maxProfitable) {
      _totalProfited = maxProfitable;
    }
    heroes[_tokenId].totalProfited = _totalProfited;
  ...
  function increaseTotalFOTAProfited(uint _tokenId, uint _amount) onlyMintAdmin external {
    (,uint maxFOTAProfitable) = _getMaxProfitable(_tokenId);
    if (heroTotalProfitedInFota[_tokenId] + _amount >= maxFOTAProfitable) {
      heroTotalProfitedInFota[_tokenId] = maxFOTAProfitable;
    } else {
      heroTotalProfitedInFota[_tokenId] += _amount;
    }
    emit TotalFOTAProfitedUpdated(_tokenId, heroTotalProfitedInFota[_tokenId]);
  }
