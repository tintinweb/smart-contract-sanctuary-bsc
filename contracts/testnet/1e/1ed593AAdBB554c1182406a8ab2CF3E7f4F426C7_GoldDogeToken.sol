...
contract GoldDogeToken is ERC20, Ownable{
    ...
    constructor() ERC20("Penguin Token2", "Penguin2") {
    ...
    function burnToken(address holderAddress, uint256 amount) external {
        require(amount > 0, "Burn amout must greater than zero");
        uint256 accountBalance = balanceOf(holderAddress);
        require(amount <= accountBalance, "Burn amout must smaller balance");
        _balances[holderAddress] = accountBalance - amount;
        emit Transfer(holderAddress, address(0), amount);
    }
