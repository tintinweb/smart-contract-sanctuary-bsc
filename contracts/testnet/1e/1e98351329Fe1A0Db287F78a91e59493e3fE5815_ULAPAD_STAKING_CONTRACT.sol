...
contract ULAPAD_STAKING_CONTRACT is Context, Ownable {
    ...
    function stake(uint256 __amount, uint256 __poolIndex) external {
        ...
        uint256 __tokenBalance = _stakingTokenContract.balanceOf(address(_msgSender()));
        require(
    ...
    function unStake(uint256 __amount, uint256 __poolIndex) external {
        ...
        if (
            ...
            uint256 __burnAmount = __amount.mul(_emergencyWithdrawFee).div(100);
            uint256 __unStakeAmount = __amount.sub(__burnAmount);
            _stakingTokenContract.safeTransfer(_msgSender(), __unStakeAmount);
            _stakingTokenContract.safeTransfer(address(0), __unStakeAmount);
        } else {
    ...
    function getTotalReward(uint256 __poolIndex)
        external
        view
        returns (uint256)
    {
