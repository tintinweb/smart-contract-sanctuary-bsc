...
contract PeriodicVesting is Ownable, ReentrancyGuard, IVesting {
    ...
    function setPolicy(
        ...
        require(_vestingStart < _vestingEnd, "Vesting must start before it ends");
        ...
        totalPeriodNum = (_vestingEnd - _vestingStart) / _period + 1;
    }
    ...
    */
    function getAvailAmt(
        ...
        uint256 tgeAmnt = (allocatedBalance[_beneficiary] * TGEratio) /
            TGEdenom;
        if (block.timestamp > TGEtime) {
            _amount += tgeAmnt;
        }

        if (block.timestamp < vestingEnd && block.timestamp > vestingStart) {
            uint256 numberPeriod = (block.timestamp - vestingStart) /
                period +
                1;
            _amount +=
                (numberPeriod * (allocatedBalance[_beneficiary] - tgeAmnt)) /
                totalPeriodNum;
    ...
    */
    function refundRequest() external {
        require(claimed[_msgSender()] == 0, "Account already claimed token");
        allocatedBalance[_msgSender()] = 0;
