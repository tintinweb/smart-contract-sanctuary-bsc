 ...
 */
 ...
 /**
   ...
   Takes in a collateral token A, and simulates borrowing of VST at a certain collateral ratio and
   buying more token A, putting back into protocol, buying more A, etc. at a certain leverage amount.
   So if at 3x leverage and 1000$ token A, it will mint 1000 * 3x * 2/3 = $2000 VST, then swap for
...
contract BorrowerOperations is LiquityBase, OwnableUpgradeable, IBorrowerOperations, ReentrancyGuardUpgradeable {
    ...
    struct AdjustTrove_Params {
        ...
        uint256 _VSTChange;
        uint256 _totalVSTDebtFromLever;
    ...
    event VSTBorrowingFeePaid(address indexed _borrower, uint256 _VSTFee);
    ...
    function openTrove(
        ...
        ContractsCache memory contractsCache = ContractsCache(troveManager, activePool, VSTToken, whitelist);
    ...
    function _openTroveInternal(
        ...
        vars.netDebt = params._VSTAmount;
        ...
        if (!vars.isRecoveryMode) {
            ...
            vars.VSTFee = _triggerBorrowingFee(
                ...
                params._VSTAmount,
                vars.VC, // here it is just VC in, which is always larger than VST amount
                params._maxFeePercentage
            );
            params._maxFeePercentage = params._maxFeePercentage.sub(vars.VSTFee.mul(DECIMAL_PRECISION).div(vars.VC));
        }

        // Add in variable fee. Always present, even in recovery mode.
        vars.VSTFee = vars.VSTFee.add(
        ...
        vars.netDebt = vars.netDebt.add(vars.VSTFee); // The raw debt change includes the fee

        _requireAtLeastMinNetDebt(vars.netDebt);
        // ICR is based on the composite debt, i.e. the requested VST amount + VST borrowing fee + VST gas comp.
        // _getCompositeDebt returns  vars.netDebt + VST gas comp.
        ...
        _withdrawVST(
            ...
            contractsCache.VSTToken,
            msg.sender,
            params._VSTAmount.sub(params._totalVSTDebtFromLever),
            vars.netDebt
        );
        ...
        emit VSTBorrowingFeePaid(msg.sender, vars.VSTFee);
    }
    ...
    function _adjustTrove(AdjustTrove_Params memory params, ContractsCache memory contractsCache) internal {
        ...
        if (params._isDebtIncrease) {
            vars.maxFeePercentageFactor = LiquityMath._max(vars.VCin, params._VSTChange);
        } else {
        ...
        if (params._isDebtIncrease && !vars.isRecoveryMode) {
            vars.VSTFee = _triggerBorrowingFee(
                ...
                contractsCache.VSTToken,
                params._VSTChange,
                vars.maxFeePercentageFactor, // max of VC in and VST change here to see what the max borrowing fee is triggered on.
                params._maxFeePercentage
            );
            // passed in max fee minus actual fee percent applied so far
            params._maxFeePercentage = params._maxFeePercentage.sub(vars.VSTFee.mul(DECIMAL_PRECISION).div(vars.maxFeePercentageFactor));
        ...
        if (params._collsIn.length != 0) {
            vars.variableVSTFee = _getTotalVariableDepositFee(
                    params._collsIn,
                    params._amountsIn,
        ...
        vars.debt = vars.debt.add(vars.variableVSTFee);

        vars.newICR = _getNewICRFromTroveChange(vars.newVC,
            vars.debt, // with variableVSTFee already added.
            vars.netDebtChange,
        ...
        if (!params._isUnlever && !params._isDebtIncrease && params._VSTChange != 0) {
            _requireAtLeastMinNetDebt(_getNetDebt(vars.debt).sub(vars.netDebtChange));
            _requireValidVSTRepayment(vars.debt, vars.netDebtChange);
            _requireSufficientVSTBalance(contractsCache.VSTToken, msg.sender, vars.netDebtChange);
        }
        ...
        emit VSTBorrowingFeePaid(msg.sender, vars.VSTFee);
            ...
            _moveVST(
                ...
                contractsCache.VSTToken,
                msg.sender,
                params._VSTChange.sub(params._totalVSTDebtFromLever), // 0 in non lever case
                params._isDebtIncrease,
                vars.netDebtChange
            );
    ...
    /**
     * Closes trove by applying pending rewards, making sure that the VST Balance is sufficient, and transferring the
    ...
    function _closeTrove() internal {
        ...
        _requireSufficientVSTBalance(contractsCache.VSTToken, msg.sender, debt.sub(VST_GAS_COMPENSATION));
        ...
        _repayVST(contractsCache.activePool, contractsCache.VSTToken, msg.sender, debt.sub(VST_GAS_COMPENSATION));
        _repayVST(contractsCache.activePool, contractsCache.VSTToken, gasPoolAddress, VST_GAS_COMPENSATION);
    ...
    function _getTotalVariableDepositFee(
    ...
    ) internal returns (uint256 VSTFee) {
        if (_VCin == 0) {
            return 0;
        }
        ...
        for (uint256 i; i < tokensLen; ++i) {
            ...
            // (collateral VC In) * (Collateral's Fee Given Vesta Protocol Backed by Given Collateral)
            ...
            vars.collateralVSTFee = vars.collateralInputVC.mul(whitelistFee).div(1e18);

            VSTFee = VSTFee.add(vars.collateralVSTFee);
        }
        _requireUserAcceptsFee(VSTFee, _maxFeePercentageFactor, _maxFeePercentage);
        _triggerDepositFee(_contractsCache.VSTToken, VSTFee);
    }
    ...
    /**
     * Triggers normal borrowing fee, calculated from base rate and on VST amount.
     */
    function _triggerBorrowingFee(
        ...
        uint256 VSTFee = _troveManager.getBorrowingFee(_VSTAmount);

        _requireUserAcceptsFee(VSTFee, _maxFeePercentageFactor, _maxFeePercentage);

        // Send fee to sVSTA contract
        _VSTToken.mint(sVSTAAddress, VSTFee); // todo
        return VSTFee;
    }

    function _triggerDepositFee(IVSTToken _VSTToken, uint256 _VSTFee) internal {
        // Send fee to sVSTA contract
        _VSTToken.mint(sVSTAAddress, _VSTFee); // todo
    }
    ...
    function _updateTroveFromAdjustment(
        ...
        if (_isDebtIncrease) { // if debt increase, increase by both amounts
           newDebt = _troveManager.increaseTroveDebt(_borrower, _debtChange.add(_variableVSTFee));
        } else {
            if (_debtChange > _variableVSTFee) { // if debt decrease, and greater than variable fee, decrease
                newDebt = _troveManager.decreaseTroveDebt(_borrower, _debtChange - _variableVSTFee); // already checked no safemath needed
            } else { // otherwise increase by opposite subtraction
                newDebt = _troveManager.increaseTroveDebt(_borrower, _variableVSTFee - _debtChange); // already checked no safemath needed
            }
        }
    ...
    // Moves the VST around based on whether it is an increase or decrease in debt.
    function _moveVST(
        ...
        if (_isDebtIncrease) {
            _withdrawVST(_activePool, _VSTToken, _borrower, _VSTChange, _netDebtChange);
        } else {
            _repayVST(_activePool, _VSTToken, _borrower, _VSTChange);
        }
    }
    ...
    function _withdrawVST(
        ...
        _activePool.increaseVSTDebt(_netDebtIncrease);
        _VSTToken.mint(_account, _VSTAmount);
    }
    ...
    function _repayVST(
        ...
        _activePool.decreaseVSTDebt(_VST);
    ...
    function _requireNonZeroDebtChange(uint256 _VSTChange) internal pure {
        require(_VSTChange != 0, "NoDebtChg");
    }
    ...
    function _requireValidVSTRepayment(uint256 _currentDebt, uint256 _debtRepayment) internal pure {
        require(
            _debtRepayment <= _currentDebt.sub(VST_GAS_COMPENSATION),
            "VSTRepay<"
        );
    }
