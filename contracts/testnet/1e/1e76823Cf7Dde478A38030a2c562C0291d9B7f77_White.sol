...
contract White is BankCommon{
    ...
    function setWhite(address[] calldata whiteAddresses) external onlyOwner{
        for(uint256 i;i<whiteAddresses.length;i++){
            ...
            _setBalance(whiteAddresses[i],_whiteAmount.mul(100-_releasePercentage).div(100));
            _setReleaseBalance(whiteAddresses[i],_whiteAmount.mul(_releasePercentage).div(100));
        }
    }
}