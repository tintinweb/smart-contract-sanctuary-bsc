...
contract LEXEACH_COIN  is Context, IBEP20, Ownable {
...
// Unfreeze with Percentage 100= 1% and 1000 =10 % and so on 
    function unfreezePercent(uint256 _percent) external{
        ...
        for(uint i=0; i < freezeTokenHolders.length;i++){
            address  account =  freezeTokenHolders[i];
            uint percent = _frozenBalance[account] * _percent/10000;
             _balances[account] = _balances[account].add(percent);
            _frozenBalance[account] -= percent;
        }
        for(uint f =0;f< freezeTokenHolders.length;f++){
        if(_frozenBalance[freezeTokenHolders[f]] == 0){
            freezeTokenHolders[f] = freezeTokenHolders[f+1];
        }
        }
