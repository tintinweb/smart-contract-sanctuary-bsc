...
contract LEXEACH_RBCF {
    ...
    constructor(address _token, address stableCoin_, address exam_) {
        ...
        users[ownerWallet].autopoolPayReciever = ownerWallet;
        // user
    ...
    function Registration(uint _referrerID, uint _coreferrerID,uint256 _amount) public payable {
        ...
        users[msg.sender].autopoolPayReciever = ownerWallet;
        users[msg.sender].referrerID= _referrerID;
        users[msg.sender].coreferrerID= _coreferrerID;
    ...
    function heightPayment(
        ...
        if (
            ...
            if (!isTokenAcceptance) {
                if(_value > 0){
                sent = payable(address(uint160(nextLevel))).send(Autopool_Level_Income);
                }else{
            ...
            if (
                ...
                if (!isTokenAcceptance) {
                    if(_value>0){
                    sent = payable(address(uint160(nextLevel))).send(
                        Autopool_Level_Income
                    );
                    }else{
                   sent = stableCoin.transfer(address(uint160(nextLevel)), Autopool_Level_Income);
    ...
    function autoPool(address _user, uint256 _value) internal {
        ...
        for (i = 1; i < currUserID; i++) {
            if (users[userList[i]].batchPaid < batchSize) {
                if (!isTokenAcceptance) {
                    if(_value>0){
                    sent = payable(address(uint160(userList[i]))).send(
                        Autopool_Level_Income
                    );
                    }else{
                        // Need to Add USDT
    ...
    function payReferral(uint _level, address _user, uint _value) internal {
        ...
        if (_level > 4) {
            if(users[_user].referredUsers > 3){ // If the Refferal is reffered more 5 users then run this block
                level_price_local = level_income/2;
                level_price_localtoken = level_incometoken/2;
            }else{ // If Not More than 5 users reffered by refferal then run this block

            }
          
        } else {
            if(users[_user].referredUsers > 2){ // If the Refferal is reffered more 3 users then run this block

            level_price_local = LEVEL_PRICE[_level];
            level_price_localtoken = LEVEL_PRICEtoken[_level];
        }else{ // If Not More than 3 users reffered by refferal then run this block

        }
        }
        if (!isTokenAcceptance) {
            if(_value>0){
            sent = payable(address(uint160(referer))).send(level_price_local);
            }else {
    ...
    function sendBalance(uint _value) private {
        ...
        if (!isTokenAcceptance) {
            if(_value>0){
            if (!payable(address(uint160(ownerWallet))).send(gettrxBalance(_value))) {}
            }else{
...
function exp( int256 a,int256  b) public pure returns(int256){
    int256 t =1;
