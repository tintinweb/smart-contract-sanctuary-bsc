...
contract BakedBeansV2Test is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        CEO_ADDRESS = 0x0Ca7038e2B9f972bCB55363E4b87B283f0479aA7; //NOT REAL CEO ADDRESS
        ...
        MIN_DEPOSIT = 10000000000000000; // 0.05 BNB
    ...
    event EmitAte(address indexed adr, uint256 bnbToEat, uint256 beansBeforeFee);
    ...
    function buyBeans(address ref) public payable {
        ...
        require(Math.add(baker.totalDeposit, msg.value) <= MAX_WALLET_TVL_IN_BNB, "Max total deposit reached");
        require(
        ...
        uint256 totalBnbFee = getPercentValue(msg.value, DEPOSIT_FEE);
        ...
        if (depositBonusEnabled) {
            beansBought = Math.add(
                beansBought,
                getPercentValue(beansBought, bonusRoundReward)
            );
        }
        ...
        if (
            ...
            if (hasInvested(baker.adr) == false) {
                uint256 refBonus = getPercentValue(
    ...
    function handleFees(uint256 totalFee) private {
        uint256 dev = getPercentValue(totalFee, DEV_FEE);
        uint256 marketing = getPercentValue(totalFee, MARKETING_FEE);
        uint256 ceo = getPercentValue(totalFee, CEO_FEE);
    ...
    function bake() public {
        ...
        uint256 beansFrom = baker.beans;
        uint256 rewardedBeans = getRewardedBeans(baker.adr);

        if (baker.upline != address(0) && baker.upline != msg.sender) {
            uint256 refBonus = getPercentValue(rewardedBeans, REF_BONUS);
            uint256 totalUplineBeans = addBeans(upline.adr, refBonus);
            upline.beans = totalUplineBeans;
        }

        uint256 totalBeans = addBeans(baker.adr, rewardedBeans);
    ...
    function eat() public {
        ...
        uint256 beansBeforeFee = getRewardedBeans(baker.adr);
        uint256 beansInBnbBeforeFee = beansToBnb(beansBeforeFee);

        uint256 totalBnbFee = getPercentValue(beansInBnbBeforeFee, WITHDRAWAL_FEE);
        uint256 bnbToEat = Math.sub(beansInBnbBeforeFee, totalBnbFee);
        bnbToEat = handleWithdrawalTaxes(baker.adr, bnbToEat);
    ...
    function handleWithdrawalTaxes(address adr, uint256 bnbWithdrawalAmount)
        private
        view
        returns (uint256)
    {
        ...
        if (daysPassed <= 0) return getPercentValue(bnbWithdrawalAmount, 10);
        if (daysPassed <= 1) return getPercentValue(bnbWithdrawalAmount, 20);
        if (daysPassed <= 2) return getPercentValue(bnbWithdrawalAmount, 30);
        if (daysPassed <= 3) return getPercentValue(bnbWithdrawalAmount, 40);
        if (daysPassed <= 4) return getPercentValue(bnbWithdrawalAmount, 50);
        if (daysPassed <= 5) return getPercentValue(bnbWithdrawalAmount, 60);
        if (daysPassed <= 6) return getPercentValue(bnbWithdrawalAmount, 70);
        if (daysPassed <= 7) return getPercentValue(bnbWithdrawalAmount, 80);
        if (daysPassed <= 8) return getPercentValue(bnbWithdrawalAmount, 90);
    ...
    function getBnbRewards(address adr) public view returns (uint256) {
        uint256 beansRewarded = getRewardedBeans(adr);
        uint256 bnbinWei = beansToBnb(beansRewarded);
        return bnbinWei;
    }

    function getBnbTvl(address adr) public view returns (uint256) {
    ...
    function getPercentValue(uint256 amount, uint256 fee)
        private
        pure
        returns (uint256)
    {
    ...
    function getRewardedBeans(address adr) private view returns (uint256) {
    ...
    function calcBeansReward(
        ...
        uint256 rewardsPerDay = getPercentValue(
            Math.mul(bakers[adr].beans, 100000),
            dailyReward
        );
        ...
        beansRewarded = Math.div(beansRewarded, 100000);
        return beansRewarded;
    }
}
