...
contract QubeLaunchPadV1 is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping (uint256 => bool) public storeStatus;

    uint256 public monthDuration = 2592000;

    IBEP20 public qube;  
    ILaunchPad public v1;

    constructor () {
        // mainnet 
        // qube = IBEP20(0x3e9d6430144485873248251fCB92bD856E95D1CD);
        // v1 = ILaunchPad(0xd0A9c0Fc38514B4EF0a0F4BDCdF4B4E59801b37a);

        // testnet 
        qube = IBEP20(0x02CABf9AC85DeEE72c6F1f2366dF06DBf25Ca45e);
        v1 = ILaunchPad(0xae7DA92EBbA611e0769DcDF707feed4c37472691);
    }
    ...
    function userLockContains(address account,uint256 value) public view returns (bool) {
        return v1.userLockContains(account,value);
    }

    function userLockLength(address account) public view returns (uint256) {
        return v1.userLockLength(account);
    }

    function userLockAt(address account,uint256 index) public view returns (uint256) {
        return v1.userLockAt(account,index);
    }

    function userTotalLockIds(address account) public view returns (uint256[] memory) {
        return v1.userTotalLockIds(account);
    } 

    function userLockInfoVersionOne(uint256 lockId) public view returns (ILaunchPad.userData memory) {
        return v1.userLockInfo(lockId);
    }

    function restore(uint256 lockId) private {
        if(!storeStatus[lockId]){
            ILaunchPad.userData memory store = v1.userLockInfo(lockId);
            userLockInfo[lockId] = userData({
                userAddress : store.userAddress,
                saleToken : store.saleToken,
                idoID : store.idoID,
                lockedAmount : store.lockedAmount,
                releasedAmount : store.releasedAmount,
                lockedDuration : store.lockedDuration * store.unlockCount,
                lastClaimed : 0,
                unlockCount : store.unlockCount,
                installmentMonths : store.installmentMonths,
                distributeROI : store.distributeROI,
                regTime : store.lockedDuration
            });

            storeStatus[lockId] = true;
        }
    }

    function claim(uint256 lockId) external whenNotPaused nonReentrant {
        restore(lockId);
        userData storage store = userLockInfo[lockId];
        require(store.userAddress == _msgSender(), "Caller is not the owner");        
