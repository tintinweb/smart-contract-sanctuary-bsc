...
contract MET is ERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x6788270D249d534357594d78DE54CaF8a0f3128d);
    ...
    uint256 private devFeesReferred = 0;
    uint256 public liquidityFee = 2;
    uint256 private liquidityFeeReferred = 1;
    uint256 public BNBRewardsBuyFee = 3;
    uint256 private BNBRewardsBuyFeeReferred = 1;
    uint256 public BNBRewardsSellFee = 3;
    uint256 private BNBRewardsSellFeeReferred = 0;
    ...
    constructor() ERC20("Moon Exchange Token", "M.E.T") {
        dividendTracker = new METDividendTracker(payable(this));

        liquidityWallet = 0x5B343381Ea745232d4f1C68f6BEaEc51F92B8BcE;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        METDividendTracker newDividendTracker = METDividendTracker(
            payable(newAddress)
        );
    ...
    function updatePayoutToken(address token) public {
        require(balanceOf(msg.sender) > 0, "You must own more than zero MET tokens to switch your payout token!");
...
contract METDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(address payable mainContract)
        DividendPayingToken("M.E.T_Dividend_Tracker",
            "M.E.T_Dividend_Tracker"
        )
    {
        MEDealer = MET(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "M.E.T: withdrawDividend disabled. Use the 'claim' function on the main M.E.T contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MEDealer);

            uint256 prevBalance = MEDealer.balanceOf(address(this));
            ...
            {
                uint256 received = MEDealer.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    MEDealer.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
