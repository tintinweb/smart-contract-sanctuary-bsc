...
contract BUSD is Owned {
    ...
    uint256 public INVEST_MAX_AMOUNT = 100000000000000000000; // 100 bnb
    ...
    constructor() {
        ...
        plans.push(Plan(20, 10));  // ❓ debug - 10, 10%/hour
    ...
    function withdrawdeposit(uint256 index) public {
        ...
        require(withdraw > 0, "8BUSD:No deposit amount");
    ...
    function redeposit(uint256 index) public {
        ...
        uint8 plan = 0;
        user.deposits.push(Deposit(plan, amount, block.timestamp, block.timestamp, 0));
    ...
    //     uint256[] memory penalty = new uint256[](user.deposits.length);
    ...
    //             penalty[i] = 100 - (block.timestamp - start[i]) / TIME_STEP  * (100 / plans[0].time)   ;
    //         }
    //     }
    ...
    function getUserDepositInfo(address userAddress, uint256 index) public view returns (
        ...
        if (block.timestamp < finish) {
            ...
            // penalty_amount  = profit * penalty_percent / 100;
        }
    }
