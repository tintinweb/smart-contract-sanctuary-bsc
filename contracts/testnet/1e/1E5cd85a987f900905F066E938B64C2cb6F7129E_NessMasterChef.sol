...
contract NessMasterChef is IRewardPool, OwnableUpgradeSafe, IERC721Receiver, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;        // How many allocation points assigned to this pool. NESS to distribute per block.
        uint256 lastRewardTime;    // Last block number that NESS distribution occurs.
        uint256 accNessPerShare;  // Accumulated NESS per share, times 1e18. See below.
    ...
    uint256 public totalNessBurn;

    // NESS tokens created per block.
    ...
    // The block number when NESS mining starts.
    ...
    // View function to see pending NESS on frontend.
    function pendingReward(uint256 _pid, address _user) public override view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accNessPerShare = pool.accNessPerShare;
        uint256 lpSupply = IERC20(pool.lpToken).balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            uint256 _seconds = block.timestamp.sub(pool.lastRewardTime);
            uint256 _nessReward = _seconds.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint_);
            accNessPerShare = accNessPerShare.add(_nessReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accNessPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    // Update the given pool's NESS allocation point and deposit fee. Can only be called by the owner.
    ...
    function depositNFT(address _nft, uint256 _tokenId, uint256 _slot, uint256 _pid) external nonReentrant checkContract {
        ...
        user.rewardDebt = user.amount.mul(poolInfo[_pid].accNessPerShare).div(1e18);
    ...
    function _updateRewardPerSecond() internal {
        ...
        rewardPerSecond = _totalRewardPerSecond.sub(_totalRewardPerSecond.mul(insuranceRate).div(10000));
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            uint256 _seconds = block.timestamp.sub(pool.lastRewardTime);
            uint256 _nessReward = _seconds.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint_);
            pool.accNessPerShare = pool.accNessPerShare.add(_nessReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function _harvestReward(uint256 _pid, address _account, bool _burnReward) internal {
        ...
        PoolInfo memory pool = poolInfo[_pid];
        uint256 _pending = user.amount.mul(pool.accNessPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            _topupInsuranceFund(_pending);
            if (_burnReward) {
                _safeNessBurn(_pending);
                emit RewardBurned(_account, _pid, _pending);
            } else {
                uint256 _boost = _pending.mul(getBoost(_account, _pid)).div(10000);
                _safeNessTransfer(msg.sender, _pending.add(_boost));
    ...
    // Deposit LP tokens to MasterChef for NESS allocation.
    function deposit(uint256 _pid, uint256 _amount) external override nonReentrant checkContract {
        ...
        user.rewardDebt = user.amount.mul(pool.accNessPerShare).div(1e18);
    ...
    // Safe NESS transfer function, just in case if rounding error causes pool to not have enough NESS.
    function _safeNessTransfer(address _to, uint256 _amount) internal {
        address _ness = ness;
        uint256 _bal = IERC20(_ness).balanceOf(address(this));
        if (_amount > _bal) _amount = _bal;
        if (_amount > 0) {
            IERC20(ness).transfer(_to, _amount);
        }
    }

    function _safeNessBurn(uint256 _amount) internal {
        IBasisAsset _ness = IBasisAsset(ness);
        if (_ness.operator() == address(this)) {
            _ness.mint(address(this), _amount);
            _ness.burn(_amount);
            totalNessBurn = totalNessBurn.add(_amount);
        }
    }
    ...
    function _topupInsuranceFund(uint256 _userReward) internal {
        ...
        _safeNessTransfer(insuranceFund, _insuranceAmount);
