...
contract Cashflow_Money_Inu is IBEP20, Ownable {
    ...
    uint256 public totalSellFee = 120;   //Total Sell Fee
    uint256 public feeDivider = 100; // Fee deniminator
    uint256 public feeDividerSell = 1000; // Fee deniminator
    ...
    uint256 _liqueditySellFee = 20;     // 2% on Selling
    uint256 _reflectionSellFee = 90;    // 9% on Selling
    uint256 _marketSellFee = 10;        // 1% on Selling
...
// intializing the addresses
    ...
    constructor(
        ...
        liquedityReceiver = 0x707f7d479f96e1020347786E0E508460dfb4Bc21;
        marketFeeReceiver = 0xBc1a903b98E34ea9E075E2f3D14377F15d263e87;
        devFeeReceiver = 0xFA8FA2Ef81A7931fC97F7617FDFE81585B5F735E;
    ...
    function _transfer(
        ...
        if (isFeeExempt[sender] || isFeeExempt[recipient] || (sender != pcsPair && recipient != pcsPair)) {
            ...
            if(recipient==pcsPair) {
                feeAmount = (amount * (totalSellFee+getsellfee()))/(feeDivider);
                amountReceived = amount - (feeAmount);
                _takeFee(sender, feeAmount);
 ...
 function setSellFeeCount(uint256 _amount) internal {
        _liquedityFeeCounter = (_amount * (_liqueditySellFee+(getsellfee()*_liqueditySellFee)/totalSellFee)) / (feeDividerSell);
        _reflectionFeeCounter = (_amount * (_reflectionSellFee+(getsellfee()*_reflectionSellFee)/totalSellFee)) / (feeDividerSell);
        _marketFeeCounter = (_amount * (_marketSellFee+(getsellfee()*_marketSellFee)/totalSellFee)) / (feeDividerSell);
        _devFeeCounter = (_amount * _devSellFee) / (feeDividerSell);
    }
