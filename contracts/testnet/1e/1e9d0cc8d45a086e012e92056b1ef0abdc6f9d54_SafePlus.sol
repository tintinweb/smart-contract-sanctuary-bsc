...
contract SafePlus is ERC20, Ownable {
    ...
    uint256 public token1DividendRewardsFee = 4;
    uint256 public previoustoken1DividendRewardsFee;
    uint256 public token2DividendRewardsFee = 5;
    ...
    address public greenMoonAddress;
    ...
    constructor() ERC20("SafePlus", "SPLUS") {
    	...
    	marketingWallet = 0xB7bFc608Be9356A049440a0E329681b90B76fF1c;
    	liquidityWallet = owner();
    	token1DividendToken = 0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47;
        token2DividendToken = 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd;
    ...
    function setExcludeStakingContract(address acc, bool value) public onlyOwner{
  	...
  	function updateLiquidityWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != liquidityWallet, "SafeplusV2: The liquidity wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function edit_dividends_settings(bool is_token1, uint256 _value) external onlyOwner {
        if (is_token1) {
            token1DividendTracker.updateClaimWait(_value);
        }
    }
    ...
    function updatetoken1DividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(token1DividendTracker), "SafeplusV2: The dividend tracker already has that address");
 
        Token1DividendTracker newtoken1DividendTracker = Token1DividendTracker(payable(newAddress));
 
        require(newtoken1DividendTracker.owner() == address(this), "SafeplusV2: The new dividend tracker must be owned by the SafePlus token contract");
        ...
        newtoken1DividendTracker.excludeFromDividends(address(deadAddress));
    ...
    function updatetoken2DividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(token2DividendTracker), "SafeplusV2: The dividend tracker already has that address");
 
       Token2DividendTracker newtoken2DividendTracker = Token2DividendTracker(payable(newAddress));
 
        require(newtoken2DividendTracker.owner() == address(this), "SafeplusV2: The new dividend tracker must be owned by the SafePlus token contract");
        ...
        newtoken2DividendTracker.excludeFromDividends(address(deadAddress));
    ...
    function excludeFromDividend(address account) public onlyOwner {
        token1DividendTracker.excludeFromDividends(address(account));
        token2DividendTracker.excludeFromDividends(address(account));
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "SafeplusV2: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if(swapAndLiquifyEnabled && liquidityFee > 0) {
                uint256 liqTokens = contractTokenBalance.div(totalFees).mul(liquidityFee).div(2);
                addLiquidity(finalBalance.mul(liquidityFee.div(2)).div(totalFees), liqTokens);
            }
 
            if (token1DividendEnabled && token1DividendRewardsFee > 0) {
                uint256 token1Tokens = contractTokenBalance.div(totalFees).mul(token1DividendRewardsFee);
                swapAndSendtoken1Dividends(token1Tokens);
            }
 
            if (token2DividendEnabled && token2DividendRewardsFee > 0) {
                uint256 token2Tokens = contractTokenBalance.div(totalFees).mul(token2DividendRewardsFee);
    ...
    function swapAndSendtoken1Dividends(uint256 tokens) private {
        ...
        uint256 token1Dividends = IERC20(token1DividendToken).balanceOf(address(this));
        transferDividends(token1DividendToken, address(token1DividendTracker), token1DividendTracker, token1Dividends);
        } else {
             swapTokensForDividendToken(tokens, address(token1DividendTracker), token1DividendToken);
        }
    }
    ...
    function swapAndSendtoken2Dividends(uint256 tokens) private {
        if (token2DividendToken != address(0)) {
            ...
            uint256 token2Dividends = IERC20(token2DividendToken).balanceOf(address(this));
            transferDividends(token2DividendToken, address(token2DividendTracker), token2DividendTracker, token2Dividends);
        } else {
 
            swapTokensForDividendToken(tokens, address(token2DividendTracker), token2DividendToken);
...
contract Token1DividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("SafePlus_Token1_Dividend_Tracker", "SafePlus_Token1_Dividend_Tracker", 0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47) {
    	claimWait = 3600;
...
contract Token2DividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("SafePlus_Token2_Dividend_Tracker", "SafePlus_Token2_Dividend_Tracker", 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    }
