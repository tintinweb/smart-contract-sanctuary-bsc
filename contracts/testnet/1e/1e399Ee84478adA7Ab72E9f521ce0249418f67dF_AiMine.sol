...
contract AiMine is Ownable {
    ...
    function buyMine(uint256 id,uint256 num)  external {
        ...
        uint256 fee=detail.amount.mul(rate).div(100);
        IERC20(tokenCurrency).safeTransferFrom(msg.sender,acceptAddress,detail.amount.sub(fee));
        if(fee>0){
            IERC20(tokenCurrency).safeTransferFrom(msg.sender,address(this),fee);
        }
        mineNum[id]=mineNum[id]+1;
        emit BuyMine(msg.sender,tokenCurrency,id,num,detail.amount);
    }
    function setMine(uint256 id,uint256 value,uint256 num,uint256 status) external onlyOwner {
        MineDetail[id]=MineConfig(id,value,num,status);
    }
