...
contract ClaimV2 is AccessControl {
    ...
    function claimOption2() external {
        require(!paused, "Please wait until the pool is open");
        require(startTime < block.timestamp, "has not started yet");
        uint256 amountPeriod = getAvaiableAmount2(msg.sender);
    ...
    function getAvaiableAmount2(address _account) public view returns(uint256){
        uint256 amountAvaiable = 0;
        uint256 _timeStamp = block.timestamp;
        if(_timeStamp > startTime){
            uint256 currentVesting = (_timeStamp.sub(startTime)).div(period);
            uint256 percentTime = currentVesting > vestingPeriod ? vestingPeriod : currentVesting;
            uint256 amountPeriod = percentTime.mul(totalAmount[address(_account)]).div(vestingPeriod);
            amountAvaiable = amountPeriod > withdrawnAmount[address(_account)] ? amountPeriod.sub(withdrawnAmount[address(_account)]) : 0;
        }
