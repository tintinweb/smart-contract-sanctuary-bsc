...
contract MetalShiba_Private is Ownable {
    ...
    uint256 _maxWalletToken  = 1 * 10**9;
    uint256 _privatesaleRate = 16800 * 10**3;
    uint256 MAX_Contribution = 5 * 10**18;
    uint256 MIN_Contribution = 4 * 10**17;
    uint256 soldtokens;
    ...
    function get_amount_nodecimals(uint256 _BNB) external view returns (uint256) {
        if(_BNB < 1)
        _BNB = _BNB * 10;
        uint256 amount = _privatesaleRate * _BNB;
        if(_BNB < 1)
