...
Transaction Tax: 12%
    5% auto yield direct to holders (no staking)
    2% Burn Rate per tx
...
contract MSDFLB is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 4 * 10**15 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public _feeSupplyCap = 5 * 10**8 * 10**18;

    string private _name = "Moon Shiba Doge Floki Lambo Baby";
    string private _symbol = "MSDFLB";
    uint8 private _decimals = 18;
    
    uint256 private constant DIVISION_FACTOR = 4;   // fee precision up to 2 decimals
    uint256 public _taxFee = 500;                   // 500 = 5.00% - tax fee is reflections to holders
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public _liquidityFee = 300;             // 300 = 3.0% will be paired and added to luqidity
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public marketingFee = 200;                // 200 = 2.00% to charity 
    address public MARKETING_WALLET;
    address public LIQUIDITY_HOLDER;

    uint public _burnRate = 200;                      // 200 = 2.00% - burn goes strait to dead wallet
    ...
    uint256 public _coolOffUnits = 12; //this variable is used to set the amount of blocks after a buy someone can sell. Default is set to 12 (approx 1 min) to avoid bots but can be adjusted from 0 (same block sell) to 4320 (approx 6 hrs) in cases of emergrency
    ...
    function setCoolOffUnits(uint256 coolOffUnits) public onlyOwner {
        require(coolOffUnits <= 4320, 'Cool off time to long');  // max cool off time in blocks. Approxamitly 6 hrs at 5 seconds per block
