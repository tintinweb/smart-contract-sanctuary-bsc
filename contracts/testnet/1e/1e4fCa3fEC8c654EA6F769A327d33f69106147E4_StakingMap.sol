...
contract StakingMap is Ownable, Pausable {
    ...
    event Staked(uint256 stakeId, address userAddress, uint32 stakesAmount);
    ...
    function stake(uint32 _stakesAmount) external whenNotPaused returns (uint256){
        ...
        STAKES.push(
            Stake({
                ...
                stakesAmount : _stakesAmount,
                unStaked : false
             })
        );
        // @formatter:on

        emit Staked(_index, _msgSender(), _stakesAmount);
    ...
    function unstake(uint256 _stakeId) public whenNotPaused{
        ...
        uint256 _totalTokenStakeAmount = STAKES[_stakeId].stakesAmount * TOKEN_SINGLE_STAKE_AMOUNT;
        TOKEN.safeTransfer(_msgSender(), _totalTokenStakeAmount);
        STAKES[_stakeId].unStaked = true;

        emit UnStaked(_stakeId, _msgSender());
    }

    // Param as array of IDs, e.g.: [1,3,7]
    function unstakeMultiple(uint256[] calldata _stakeIds) external whenNotPaused{
