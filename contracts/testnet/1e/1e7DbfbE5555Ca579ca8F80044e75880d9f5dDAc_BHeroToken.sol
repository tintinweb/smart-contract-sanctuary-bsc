...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  IBEP20 public plushie; // yes önemli nokta
  ...
  function initialize(address _design, address _oracle) public initializer {
    __ERC721_init("Plushie", "plushie"); // isim ne bilmmiyoruz
    __AccessControl_init();
    __Pausable_init();
    __UUPSUpgradeable_init();

    plushie = IBEP20(0x73A59d7b4D7d7316F0D933FA9739d9aAaAdEf98B); // mintable coin for test oracle da mintable yi güncelemedim haberin olsun token e yazayım mı
    oracle = IOracle(_oracle); // oracle adresi
    design = IBHeroDesign(_design);
  ...
  function withdraw() external onlyRole(DEFAULT_ADMIN_ROLE) {
    plushie.transfer(msg.sender, plushie.balanceOf(address(this)));
  ...
  /** Claims tokens. */
  function claim() external {  // oyun da cliam kullanmadığım hiç şimdi ben yeni nf talayım level upgrade yapaıyım ??
    // Check limit.
  ...
  function mint(uint256 count) payable external {
    require(count > 0, "No token to mint"); // mint ===> then reques thing..
    ...
    // Transfer coin token.
    uint256 amount = design.getMintCost() * count; // aynen ok
    uint256 bnbAmount = oracle.getNFTPrice(amount, true); // interfaceden değil mi bu ?? çektik işte
    require(msg.value >= bnbAmount, "Insufficient BNB");
    plushie.transferFrom(to, address(this), amount);
  ...
  function processTokenRequests() external {  // the heart of the contract
    address to = msg.sender;
    uint256 size = tokenIds[to].length;
    uint256 limit = design.getTokenLimit(); // yes 
    require(size < limit, "User limit reached");
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      if (seed == 0) {
        ...
        seed = uint256(blockhash(targetBlock)); // seed created for random number
      }
  ...
  function upgrade(uint256 baseId, uint256 materialId) payable external {
    ...
    require(baseLevel < design.getMaxLevel(), "Max level"); // ben yeni nft alıp level arttırcam şimdi yazarsan göremem ok ? okay buraya da emit koycaz
                                                                              
    // Transfer coin token.
    uint256 rarity = BHeroDetails.decodeRarity(baseDetails);
    uint256 cost = design.getUpgradeCost(rarity, baseLevel - 1); 
    uint256 bnbCost = oracle.getNFTPrice(cost, true); 
    require(msg.value >= bnbCost, "Insufficient BNB");

    payable(0x7b09Bf93Fc98056dCdD90e4b27454B04bAb8CF64).transfer(msg.value);
    
    plushie.transferFrom(to, address(this), cost);

    tokenDetails[baseId] = BHeroDetails.increaseLevel(baseDetails);
    emit TokenCreated(to, baseId, tokenDetails[baseId]);  // EMIT KOYULACAK  bir kere level atlatayım nft için geliyom 2 sk ye kadar
    _burn(materialId);
  }
  ...
  function _transfer(
    ...
    address to,
    uint256 tokenId
  ) internal override onlyRole(TRADER_ROLE) { // ı am in the house 
...
interface IOracle {
    function getNFTPrice(uint256 _price, bool _type) external view returns(uint256);
    function setRouter(address _router) external;
    function setTOKENPair(address _token) external;
