...
contract Insurance is Ownable {
    ...
    constructor() {
        ...
        totalInsurance = 0;
        address address_token_nain = 0x539CAFC25E2983bcFC47503F5FD582B20Cbb56c7; //address sc NAIN
        token_nain = IERC20(address_token_nain);
    }
    ...
    **/
    ...
    modifier checkAllowance(uint256 amount) {
        require(
            token_nain.allowance(msg.sender, address(this)) >= amount,
            "Error allowance"
        );
        _;
    }
    ...
    function withdrawTokens(address _receipient, uint256 _amount)
        external
        onlyContractCaller(msg.sender)
    {
        require(
            token_nain.balanceOf(address(this)) >= _amount,
            "Token does't enough, please check again!"
        );
        token_nain.transfer(_receipient, _amount);
    }

    function depositToken(address _receipient, uint256 _amount)
        external
        onlyContractCaller(msg.sender)
    {
        require(
            token_nain.balanceOf(address(msg.sender)) >= _amount,
            "Token does't enough, please check again!"
        );
        token_nain.transfer(_receipient, _amount);
    }
    ...
    function createInsurance(
        ...
        returns (InsuranceStruct memory)
    {
        require(msg.value >= _margin, "Value is not enough");
        require(
            _period >= 2 && _period <= 15,
            "The time must be within the specified range"
        );
        ...
        insurance[totalInsurance] = newInsurance;
        ...
        returns (bool)
    {
        require(
            compareString(_state, "Expired") ||
                compareString(_state, "PayoutWaiting") ||
                compareString(_state, "Paid"),
