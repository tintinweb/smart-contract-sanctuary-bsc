...
interface IRewarder {
    function onReward(uint256 pid, address user, address recipient, uint256 wolfilandAmount, uint256 newTokenAmount) external;
    function pendingTokens(uint256 pid, address user, uint256 wolfilandAmount) external view returns (IERC20[] memory, uint256[] memory);
}
...
// MasterChef is the master of wolfiland. He can make wolfiland and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once wolfiland is sufficiently
...
contract MasterChefV3 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of wolfilands
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accWolfilandPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws tokens to a pool. Here's what happens:
        //   1. The pool's `accWolfilandPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. wolfilands to distribute per block.
        uint256 totalStaked;      // Amount of tokens staked in given pool
        uint256 lastRewardTime;   // Last timestamp wolfilands distribution occurs.
        uint256 accWolfilandPerShare;  // Accumulated wolfilands per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
    }

    // The wolfiland TOKEN!
    MockToken public wolfiland;
    // Dev address.
    address public devaddr;
    // Dev fee percentage.
    uint256 public devFee = 100;
    // wolfiland tokens created per second.
    uint256 public wolfilandPerSecond;
    // Deposit Fee address
    ...
    // The block number when wolfiland mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 wolfilandPerSecond);
    ...
    event LogUpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 stakeSupply, uint256 accWolfilandPerShare);

    constructor(
        MockToken _wolfiland,
        address _devaddr,
        address _feeAddress,
        uint256 _wolfilandPerSecond,
        uint256 _startTime
    ) {
        wolfiland = _wolfiland;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        wolfilandPerSecond = _wolfilandPerSecond;
    ...
    // Update the given pool's wolfiland allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending wolfilands on frontend.
    function pendingWolfiland(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accWolfilandPerShare = pool.accWolfilandPerShare;
        uint256 stakeSupply = pool.totalStaked;
        if (block.timestamp > pool.lastRewardTime && stakeSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 wolfilandReward = multiplier.mul(wolfilandPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accWolfilandPerShare = accWolfilandPerShare.add(wolfilandReward.mul(1e12).div(stakeSupply));
        }
        return user.amount.mul(accWolfilandPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
        uint256 totalWolfiland = multiplier.mul(wolfilandPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
        if (totalWolfiland == 0) return;

        if(wolfiland.totalSupply() + totalWolfiland > wolfiland.cap()) {
            totalWolfiland = wolfiland.cap() - wolfiland.totalSupply();
            if(totalWolfiland == 0 && wolfilandPerSecond != 0)
                return _updateEmissionRate(0);
        }

        uint256 forDevs = totalWolfiland.mul(devFee).div(1000);
        uint256 wolfilandReward = totalWolfiland.sub(forDevs);
        pool.accWolfilandPerShare = pool.accWolfilandPerShare.add(wolfilandReward.mul(1e12).div(stakeSupply));
        pool.lastRewardTime = block.timestamp;
        emit LogUpdatePool(_pid, pool.lastRewardTime, stakeSupply, pool.accWolfilandPerShare);
    }

    // Deposit tokens to MasterChef for wolfiland allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            pending = user.amount.mul(pool.accWolfilandPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeWolfilandTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accWolfilandPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accWolfilandPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            safeWolfilandTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
    ...
    // Safe wolfiland transfer function, just in case if rounding error causes pool to not have enough wolfilands.
    function safeWolfilandTransfer(address _to, uint256 _amount) internal {
        uint256 wolfilandBal = wolfiland.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > wolfilandBal) {
            transferSuccess = wolfiland.transfer(_to, wolfilandBal);
        } else {
            transferSuccess = wolfiland.transfer(_to, _amount);
        }
        require(transferSuccess, "safeWolfilandTransfer: transfer failed");
    }
    ...
    function _updateEmissionRate(uint256 _wolfilandPerSecond) internal {
        require(_wolfilandPerSecond <= MAX_EMISSION_RATE, "Updated emissions are more than maximum rate");
        wolfilandPerSecond = _wolfilandPerSecond;
        emit UpdateEmissionRate(msg.sender, _wolfilandPerSecond);
    }

    function updateEmissionRate(uint256 _wolfilandPerSecond) public onlyOwner {
        _updateEmissionRate(_wolfilandPerSecond);
        massUpdatePools();
    }    
