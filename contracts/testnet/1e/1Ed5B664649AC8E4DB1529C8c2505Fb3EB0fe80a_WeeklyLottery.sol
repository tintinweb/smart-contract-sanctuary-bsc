...
contract WeeklyLottery is Initializable, IWeeklyLottery, OwnableUpgradeable {
        ...
        returns (address[10] memory winners)
    {
        ...
        for (uint256 i = winnersCount; i < 10; i++) {
            winners[i] = address(0);
        }
    }
    ...
    // TEST FUNCTIONS DELETE BEFORE DEPLOY
    function startLotteryTestEvent(uint256 _bet, address[] calldata _addrs) external {
        // uint256 _bet = 1000000000000000;
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            if (_addrs[i] != address(0)) {
                emit UserWon(
                    ...
                    _bet,
                    prize,
                    lastDrawBlockForUser[_bet][_addrs[i]]
                );
                lastDrawBlockForUser[_bet][_addrs[i]] = block.number;
            }
