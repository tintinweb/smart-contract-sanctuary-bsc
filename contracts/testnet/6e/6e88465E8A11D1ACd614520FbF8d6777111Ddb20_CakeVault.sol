...
contract CakeVault is Ownable, Pausable {
    ...
    IERC20 private immutable cakeToken; // cake token.
    ...
    uint256 public dummyCakePoolPID;            // dummyCakePool (dWP) PID 
     ...
     * @param _dummyCakePoolPID: Cake pool ID in MasterChef
     */
    constructor(
        ...
        uint256 _dummyCakePoolPID
    ) {
        cakeToken   = _masterchef.CakeAddress();
        MasterChef = _masterchef;
        ContractManager = _contractManager;
        FinancialController = _financialController;
        TreasuryAnalyst = _treasuryAnalyst;
        dummyCakePoolPID = _dummyCakePoolPID;
    }
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        MasterChef.deposit(dummyCakePoolPID, balance);
        emit Init();
    }

    function CakeAddress() external view returns (IERC20) {
        return cakeToken;
    }
    /**
    ...
    /**
     * @notice Checks if the msg.sender is either the cake owner address or the TreasuryAnalyst address.
     */
    modifier onlyTreasuryAnalystOrCakeOwner(address _user) {
        require(msg.sender == _user || msg.sender == TreasuryAnalyst, "Not TreasuryAnalyst or cake owner");
        _;
    }
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                if (!freeOverdueFeeUsers[_user] && ((user.lockEndTime + UNLOCK_FREE_DURATION) < block.timestamp)) {
                    ...
                    cakeToken.safeTransfer(FinancialController, currentOverdueFee);
                ...
                if (currentPerformanceFee > 0) {
                    cakeToken.safeTransfer(FinancialController, currentPerformanceFee);
     ...
     * @param _user: User address
     */
    function unlock(address _user) external onlyTreasuryAnalystOrCakeOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    function depositOperation(
        ...
        if (totalShares == 0) {
            ...
            cakeToken.safeTransfer(FinancialController, stockAmount);
        }
        // Update user share.
        ...
        uint256 pool = balanceOf();
        if (_amount > 0) {
            cakeToken.safeTransferFrom(_user, address(this), _amount);
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (!freeWithdrawFeeUsers[msg.sender] && (block.timestamp < user.lastDepositedTime + withdrawFeePeriod)) {
            ...
            cakeToken.safeTransfer(FinancialController, currentWithdrawFee);
            currentAmount -= currentWithdrawFee;
        }

        cakeToken.safeTransfer(msg.sender, currentAmount);
    ...
    function harvest() internal {
        uint256 pendingCake = MasterChef.pendingCake(dummyCakePoolPID, address(this));
        if (pendingCake > 0) {
            uint256 balBefore = available();
            MasterChef.withdraw(dummyCakePoolPID, 0);
     ...
     * @dev Callable by the contract ContractManager.
     */
    function setVCakeContract(address _VCake) external onlyContractManager {
    ...
    function inCaseTokensGetStuck(address _token) external onlyContractManager {
        require(_token != address(cakeToken), "Token cannot be same as deposit token");
    ...
    function calculateTotalPendingCakeRewards() public view returns (uint256) {
        uint256 amount = MasterChef.pendingCake(dummyCakePoolPID, address(this));
        return amount;
    }
    ...
    function balanceOf() public view returns (uint256) {
        return cakeToken.balanceOf(address(this)) + totalBoostDebt;
    }
