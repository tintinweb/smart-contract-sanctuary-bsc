...
//ssh -i ideapad.pem [email protected] 
    ...
    constructor(
       ...
       // vestingpercent=vesting*100000000000000000;
       // claimable1=claimable[msg.sender];
    ...
    function claim() external {
       ...
       // uint ratio = ethers.utils.formatEther(ratio) 
    ...
    function vestingClaim() external {
       // require(claimable[msg.sender]==0,'you  do not have enough tokens');
        ...
        require(
            ...
            "vesting claim is not allowed yet"
              );
            vestingPeriod= endTime+ 300 seconds;
        require(block.timestamp >vestingPeriod, "vesting time not yet started");
        ...
        require(vestingTerm[msg.sender] <= vestedTerm,'vesting term is not started yet');
            ...
            uint256 totalAmount=claimable1[msg.sender];
            //  uint256 claimable1 = claimable[msg.sender];
            // uint256 totalAmount=claimable[msg.sender]+(vestingTerm[msg.sender]*vestingpercent);
            uint256 vestingInstallment = (totalAmount * vesting) / 100;
            //if(claimable[msg.sender]>vestingInstallment)
            if(claimable[msg.sender]>vestingInstallment){

                uint256 vestingInstallment1=vestingInstallment*ratio;
                 // if(vestingInstallment>0){     
                token.safeTransfer(msg.sender, vestingInstallment1);
    ...
    function buyWithEth() public payable {
        ...
        totalOwed += amount;
        weiRaised += msg.value;
        claimable1[msg.sender]=claimable[msg.sender];
    ...
    function buy(uint256 weiAmount) public {
        testtoken=address(this);//pool address
        //symbols=token.name();
