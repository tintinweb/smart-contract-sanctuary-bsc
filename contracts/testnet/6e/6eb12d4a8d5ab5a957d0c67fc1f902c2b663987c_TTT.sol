...
contract TokenDividendTracker is Ownable {
    ...
    constructor(address uniswapV2Pair_, address lpRewardToken_, address rewardHolder_){
    ...
    function getRewardHolder() public view returns (address) {
        return rewardHolder;
    }

    function setHolderMin(uint256 _holderMin) public onlyOwner{
        holderMin = _holderMin * 10**18;
    } 
    function setHolderMax(uint256 _holderMax) public onlyOwner{
        holderMax = _holderMax * 10**18;
    } 
    ...
    function process(uint256 gas) external onlyOwner {
        ...
        uint256 nowbanance = IERC20(lpRewardToken).balanceOf( rewardHolder );
        if(currentIndex==0){
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 lpAmount = IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex]);
            if( lpAmount<holderMin ){
                gasUsed = gasUsed.add(gasLeft.sub(gasleft()));
                gasLeft = gasleft();
                currentIndex++;
                iterations++;
            }
            if( lpAmount>holderMax ){
                lpAmount = holderMax;
            }
            uint256 amount = nowbanance.mul(lpAmount).div(IERC20(uniswapV2Pair).totalSupply());
            if( amount == 0) {
                currentIndex++;
                iterations++;
                return;
            ...
            if(IERC20(lpRewardToken).balanceOf(rewardHolder)  < amount ) return;
            // IERC20(lpRewardToken).transfer(shareholders[currentIndex], amount);
            IERC20(lpRewardToken).transferFrom(rewardHolder, shareholders[currentIndex], amount);
            gasUsed = gasUsed.add(gasLeft.sub(gasleft()));
            gasLeft = gasleft();
            currentIndex++;
            iterations++;
...
contract TTT is ERC20, Ownable {
    ...
    address public buyFeeAddr=0x000000000000000000000000000000000000dEaD;
    address public sellFeeAddr=0x000000000000000000000000000000000000dEaD;
    address public deadAddr = 0x000000000000000000000000000000000000dEaD;
    address public lpManager = 0xb8a74CDaF0646092a56a612005285377b9134582;
        ...
        ERC20("TTT5", "TTT5")
    {
        ...
        // //testnet
        // lpRewardToken = 0x38C93854cB671bE1A23b2D9E86d892253EE4a725;
        // uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        // uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this),0x38C93854cB671bE1A23b2D9E86d892253EE4a725);
        // dividendTracker = new TokenDividendTracker(uniswapV2Pair, lpRewardToken, 0xFd71Ee8FBB76B10A307a5Dfd3b6A1b422499E4D8);
        // address initBalanceAddr = 0xb8a74CDaF0646092a56a612005285377b9134582;
        // lpManager = 0xb8a74CDaF0646092a56a612005285377b9134582;
        ...
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this),0x38C93854cB671bE1A23b2D9E86d892253EE4a725);
        dividendTracker = new TokenDividendTracker(uniswapV2Pair, lpRewardToken, 0xFd71Ee8FBB76B10A307a5Dfd3b6A1b422499E4D8);
        address initBalanceAddr = 0x0D56fe32a9DfeC571d42A837366C69e119FE1974;
        lpManager = 0x0D56fe32a9DfeC571d42A837366C69e119FE1974;
        ...
        isExcludedFromFees[lpManager]=true;

        isDividendExempt[address(this)] = true;
        isDividendExempt[address(0)] = true;
        isDividendExempt[address(dividendTracker)] = true;

        dividendTracker.setHolderMin(1000);
        dividendTracker.setHolderMax(3000);

        ammPairs[uniswapV2Pair] = true;
        _cast(initBalanceAddr, totalSupply);
    }
    ...
    function setFee(uint256 _deadFee,uint256 _buyFee,uint256 _sellFee,uint256 _lpRewardFee) public onlyOwner {
        deadFee=_deadFee;
        buyFee=_buyFee;
        sellFee=_sellFee;
    ...
    function setRewardHolder(address _rewardHolder) public onlyOwner {
        dividendTracker.setRewardHolder(_rewardHolder);
    }
    function setHolderMin(uint256 _holderMin) public onlyOwner{
        dividendTracker.setHolderMin(_holderMin);
    } 
    function setHolderMax(uint256 _holderMax) public onlyOwner{
        dividendTracker.setHolderMax(_holderMax);
    } 
        ...
        returns (bool)
    {
     ...
     function swapLPRewardToken(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ...
            0,
            path,
            dividendTracker.getRewardHolder(),
            block.timestamp
        );
