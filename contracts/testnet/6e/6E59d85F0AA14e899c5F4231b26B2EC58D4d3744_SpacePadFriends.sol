...
contract SpacePadFriends is Pausable, Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint256[] storage walletDeposits = walletRoundDeposits[msg.sender][
            currentRound.roundIndex
        ];

        require(
            walletDeposits.length >= currentRound.singleWalletDepositsLimit,
            "The deposits amount for the round has been reached"
        );

        require(currentRound.valid, "No active round at the moment");

        bool maxCapReached = currentRound.roundMaxCap >
    ...
    function depositOnReferrer(
        ...
        uint256 referralFeeAmount = _amount.mul(_referralFee).div(1000);
        uint8 level = 1;
