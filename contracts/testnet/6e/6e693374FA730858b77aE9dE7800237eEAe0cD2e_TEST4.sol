...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0xd66c6B4F0be8CE5b39D52E0Fd1344c389929B378);
    address WBNB = 0xd66c6B4F0be8CE5b39D52E0Fd1344c389929B378;
    IDEXRouter router;
...
contract TEST4 is IBEP20, Auth {
    ...
    uint256 public totalFee        = marketingFee + rewardFee + liquidityFee + devFee + burnFee + buybackFee;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBTeam = amountBNB.mul(devFee).div(totalBNBFee);
        uint256 amountBNBUtility = amountBNB.mul(buybackFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(devFeeReceiver).call{value: amountBNBTeam, gas: 30000}("");
        (tmpSuccess,) = payable(buybackFeeReceiver).call{value: amountBNBUtility, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _devFee, uint256 _buybackFee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        ...
        burnFee = _burnFee;
        totalFee = _liquidityFee + _rewardFee + _marketingFee + _devFee + _burnFee + _buybackFee;
