...
contract TestKBv4ARFinance is ERC20, Ownable {
    ...
    uint256 public rewardFeeSellFactor = 120;
    ...
    event FeesUpdated(uint256 indexed newRewardsFee, uint256 indexed newLiquidityFee, uint256 newOperationsFee, uint256 newMarketingFee, uint256 newBuyBackFee);
    ...
    constructor() ERC20("TestKBv4AR", "TKBv4", 18) {
    	...
    	operationsWallet = 0x58265fB4808A0500586D723dBcc7Ccb5ad9409A5;
    	marketingWallet = 0xBf838D36EB509B73df8Aa7d4b1c47619d8cB8389;
        buyBackWallet = owner();
    ...
    receive() external payable {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "TestKBv4AR: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "TestKBv4AR: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TestKBv4AR: The new dividend tracker must be owned by the TestKBv4AR token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "TestKBv4AR: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "TestKBv4AR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    
    // for airdrop campaign after contract launch
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "TestKBv4AR: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TestKBv4AR: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "TestKBv4AR: The operations wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TestKBv4AR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TestKBv4AR: Cannot update gasForProcessing to same value");
    ...
    function updateRewardFactor(uint256 rewardFactor) external onlyOwner {
        require(rewardFactor >= 100 && rewardFactor <= 350, "rewardFactor must be between 100 and 350");
        rewardFeeSellFactor = rewardFactor;
    }
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "TestKBv4AR: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "TestKBv4AR: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(!isBlacklistedToken(rewardTokenAddress), "TestKBv4AR: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "TestKBv4AR: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "TestKBv4AR: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "TestKBv4AR: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(!isBlacklistedToken(rewardTokenAddress), "TestKBv4AR: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "TestKBv4AR: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "TestKBv4AR: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TestKBv4AR: Automated market maker pair is already set to that value");
