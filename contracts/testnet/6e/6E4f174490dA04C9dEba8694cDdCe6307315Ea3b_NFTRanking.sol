...
contract NFTRanking is Ownable {
    ...
    function quickSort(UserInfo[] memory arr, uint left, uint right) private {
        uint i = left;
        uint j = right;
        if(i==j) return;
        // uint pivot = left + (right - left) / 2;
        uint pivotPower = userInfos[left + (right - left) / 2].power;
        while (i <= j) {
            UserInfo storage iUser = userInfos[i];
            UserInfo storage jUser = userInfos[j];

            while (iUser.power < pivotPower) i++;
            while (pivotPower < jUser.power) j--;
            if (i <= j) {
                (iUser, jUser) = (jUser, iUser);
                i++;
                j--;
            }
        }
    ...
    function setRank() external {
        ...
        quickSort(userInfos, 0, userInfos.length);
    }
