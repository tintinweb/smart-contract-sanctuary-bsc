...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private _burnMax = 200000*10**18;//燃烧的最大数
    ...
    uint16 private _inviterGeneration=0;//代数数 
    ...
    //address public husdtToken;// =0x79e9a5115df932e8936cFC9f2ecc2d3f30fbc871;//测试
    
    //address public husdtToken = 0x55d398326f99059fF775485246999027B3197955;//正式
...
string memory name_,string memory symbol_,uint256  totalSupply_,
    //address[5] memory addrs,// owownerAddres dedevAddress foundAddress reward router
    //uint256[5] memory feeSettings,//_taxFee _liquidityFee _burnFee _devFee _inviterFee
    uint256 minimumTokenBalanceForDividends_*/
    constructor (
        string memory name_,string memory symbol_,uint256  totalSupply_,
        address[4] memory addrs,// owownerAddres dedevAddress foundAddress reward router
        uint16 inviterGeneration_,
        uint256[4] memory feeSettings,//_taxFee _liquidityFee _burnFee _devFee _inviterFee
        uint256 minimumTokenBalanceForDividends_,uint16 [4] memory inviterFees_,uint256 burnMax_
    ) public {
        _name=name_;
        _symbol=symbol_;
        _decimals = 18;
        ...
        _inviterGeneration=inviterGeneration_;
        ownerAddres=addrs[0];
        devAddress=addrs[1];
        foundAddress=addrs[2];
        //husdtToken=addrs[3];
        _taxFee=feeSettings[0];
        _liquidityFee=feeSettings[1];
        _burnFee=feeSettings[2];
        _devFee=feeSettings[3];
        //_inviterFee=feeSettings[4];//总邀请费 
        _inviterFee0=inviterFees_[0];
        _inviterFee1=inviterFees_[1];
        _inviterFee2=inviterFees_[2];
        _inviterFee3=inviterFees_[3];
        /*_inviterFee=_inviterFee.add(_inviterFee0).add(_inviterFee1).add(_inviterFee2).add(_inviterFee3);
        if(_inviterGeneration>4){
            _inviterFee=_inviterFee.add(_inviterFee3*(_inviterGeneration-4));
        }*/
        _burnMax=burnMax_*10**18;
        numTokensSellToAddToLiquidity=minimumTokenBalanceForDividends_*10**18;
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[ownerAddres] = _rTotal;
        IPancakeRouter02 _uniswapV2Router = IPancakeRouter02(addrs[3]);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);//sub all fee
        ...
        if (balanceOf(burnAddress) < _burnMax && !_isExcludedFromFee[sender]) {
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < _inviterGeneration; i++) {
            uint256 rate;
            if (i == 0) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        _rOwned[address(this)] = _rOwned[address(this)].sub(devNumber);
        _rOwned[devAddress] = _rOwned[devAddress].add(devNumber);//sub all fee
        emit Transfer(address(this), devAddress, devNumber);
