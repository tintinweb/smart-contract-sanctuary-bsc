...
interface IERC20 {
    ...
    event TransferSend(uint256 Send);
...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        emit TransferSend(amount);
        //calculating Coin distribution
        uint256 inLiquiditycoins = balanceOf(_LiquiditypairAddress);
        uint256 LiquidWallet = balanceOf(_LiquidityWalletAddress);
        uint256 DevWallet = balanceOf(_developmentWalletAddress);
        uint256 NonCirculating = inLiquiditycoins + LiquidWallet + DevWallet;
        CirculatingSupply = _tTotal - NonCirculating;
