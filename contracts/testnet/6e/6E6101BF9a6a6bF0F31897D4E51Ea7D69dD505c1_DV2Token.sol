...
contract DV2Token is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    uint256 public globalMultiplier = 15;
    ...
    function burnFrom(address _account, uint256 _amount) public onlyOperator override(ERC20Burnable){
        super.burnFrom(_account, _amount);
    }

    function burn(uint256 _amount) public onlyOperator override(ERC20Burnable){
        super.burn(_amount);
    }
    ...
    function redeem(uint256 amount) public {
        ...
        super._mint(msg.sender, outAmount);
    } 
    ...
    // normal transaction
        ...
        // tx limits
            ...
            //sell
                ...
                // marketing auto-bnb
                if (swapAndLiquifyEnabled && canSell && balanceOf(pancakeV2Pair) > 0) {
    ...
    function updateMultiplier() public returns (bool) {
      if(isMultiEnabled){
        uint256 price = oracle.price(pancakeV2Pair,tokenToUpgrade,BUSD);
        if((price == basePrice && readMultiplierLength() == 1) || (price <= (wpow(globalMultiplier.div(denominatorMultiplier),(readMultiplierLength()))).mul(basePrice))){
          lastMultiplierData[tokenCount.current()+1].lastMulti = wpow(globalMultiplier.div(denominatorMultiplier),(tokenCount.current()));
          tokenCount.increment();
          lastMultiplierData[readMultiplierLength()].currentMulti = wpow(globalMultiplier.div(denominatorMultiplier),(readMultiplierLength()));
          return false;
        } else if((price >= sellPrice && readMultiplierLength() == 1) || (price >= (wpow(globalMultiplier.div(denominatorMultiplier),(readMultiplierLength()))).mul(sellPrice))){
          return true;
        }
        return false;
      }
    ...
    function checkMultiplier() public view returns (uint256) {
      if(isMultiEnabled){
        return wpow(globalMultiplier.div(denominatorMultiplier),(readMultiplierLength()));
      }
      return 1;
    }
    // Check User Balance
    function checkUserCanSold(address _user,uint256 _soldAmount) public view returns (bool) {
      ...
      if(_soldAmount < initialBalance.mul(minPercent).div(1000)){
        return true;
      }
      return false;
    }
    function readPrice()public view returns(uint256 currentPrice){
        return oracle.price(pancakeV2Pair,tokenToUpgrade,BUSD);
    }
