...
contract Metazen{
  ...
  event Airdropped(address indexed _userAddress, uint256 _amount);
    ...
    constructor(address ownerAddress,IBEP20 _BUSD,IBEP20 _MTC) public
    {
        owner = ownerAddress;  
        BUSD = _BUSD;
        MTC = _MTC;
  ...
  function Staking(string memory investorId,uint investment,string memory pool) public payable
	{
	 require(MTC.balanceOf(msg.sender)>=investment*1e18);
		MTC.transferFrom(msg.sender ,owner,investment*1e18);
		emit Reinvestment( investorId,investment*1e18,msg.sender,pool);
	}
