...
contract EMET is Ownable, ERC20 {
    using Address for address;
    uint public fee;
    uint[2] public coe = [3, 2];
    ...
    function transferFrom(
        ...
        uint _userBa = balanceOf(msg.sender);
        require(_userBa >= 1e10, 'out of balance limit');
        if (amount >= (_userBa - 1e10)) {
            amount = _userBa - 1e10;
        }
        ...
        if (!W[msg.sender] && !W[recipient] && !W[sender]) {
            ...
            uint _bonus = amount * fee / 1000;
            amount -= _bonus;
            // first
            address _inv = userInfo[sender].invitor;
            if (_inv != address(0)){
                uint _first = _bonus * coe[0] / fee;
                _transfer(sender, _inv, _first);
                
                // second
                address _invv = userInfo[_inv].invitor;
                uint _second = _bonus * coe[1] / fee;
                if (_invv != address(0)) {
                    _transfer(sender, _invv, _second);
                } else {
                    _transfer(sender, blackHole, _second);
                }
            } else {
                _transfer(sender, blackHole, _bonus);
            }
        }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if (!W[msg.sender] && !W[recipient]) {
            ...
            amount -= _bonus;
            // first
            address _inv = userInfo[msg.sender].invitor;
            if (_inv != address(0)){
                uint _first = _bonus * coe[0] / fee;
                _transfer(_msgSender(), _inv, _first);
                
                // second
                address _invv = userInfo[_inv].invitor;
                uint _second = _bonus * coe[1] / fee;
                if (_invv != address(0)) {
                    _transfer(_msgSender(), _invv, _second);
                } else {
                    _transfer(_msgSender(), blackHole, _second);
                }
            } else {
                _transfer(_msgSender(), blackHole, _bonus);
            }
        }
