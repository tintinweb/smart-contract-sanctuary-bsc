...
contract OpenBiSeaAuction is Ownable,IERC721Receiver,IERC1155Receiver {
    ...
    mapping(uint256 => Auction) public contractsPlusTokenIdsAuction;

    mapping(address => uint256[]) public contractsSenderPlusTokenIdList;

    mapping(address => uint256[]) public contractsTokenIdList;

    mapping(address => uint256) public consumersDealFirstDate;
    mapping(uint256 => address) public auctionIDtoSellerAddress;

    function getNFTsAuctionList(address contractNFT) public view returns (Auction[] memory) {
        Auction[] memory auctionList = new Auction[](contractsTokenIdList[contractNFT].length);
        for (uint i=0; i< contractsTokenIdList[contractNFT].length; i++) {
            ...
            auctionList[i] = contractsPlusTokenIdsAuction[index];
        }
        return auctionList;
    }

    function indexOfNFTsAuctionList(address contractNFT, uint256 lookingFor) public view returns (uint256) {
        for (uint i=0; i< contractsSenderPlusTokenIdList[contractNFT].length; i++) {
            if (contractsSenderPlusTokenIdList[contractNFT][i] == lookingFor) {
                return i;
            }
        }
        return type(uint256).max;
    ...
    function contractsSenderPlusTokenIdsListUpdated(address contractNFT, uint256 removedPosition) public view returns (uint256[] memory) {
        uint256[] memory array = new uint256[] (contractsSenderPlusTokenIdList[contractNFT].length - 1);
        for (uint i=0; i< contractsSenderPlusTokenIdList[contractNFT].length; i++) {
            if (contractsSenderPlusTokenIdList[contractNFT][i] != removedPosition) array[i] = contractsSenderPlusTokenIdList[contractNFT][i];
        }
        return array;
    }
    ...
    function contractsTokenIdsListUpdated(address contractNFT, uint256 removedPosition) public view returns (uint256[] memory) {
        ...
        for (uint i=0; i< contractsTokenIdList[contractNFT].length; i++) {
            if (contractsTokenIdList[contractNFT][i] != removedPosition) array[i] = contractsTokenIdList[contractNFT][i];
        }
        return array;
    }
    ...
    (
        ...
        uint256 index = addressPlusTokenId(_contractNFT, _tokenId);
        return (
    ...
    function createAuction(
        ...
        require(indexOfNFTsAuctionList(contractNFT, addressPlusTokenId(sender, tokenId)) == type(uint256).max, "OpenBiSeaAuction: auction is already created");  //contractsTokenIdsList[contractNFT].contains(uint256(sender).add(tokenId))
        ...
        contractsSenderPlusTokenIdList[contractNFT].push(addressPlusTokenId(sender, tokenId));
    ...
    function updateFirstDateAndValue(address buyer, address seller, uint256 value, address token) private {
        if (token == address (0x0) || IOpenBiSea(openBiSeaMainContract).getUsdContract() == token) { // only usd or main coin can add income value
            ...
            if (IOpenBiSea(openBiSeaMainContract).getUsdContract() == token) {
                ...
                uint8 decimals;
                (priceMainToUSD,decimals) = IOpenBiSea(openBiSeaMainContract).getOracleContract().getLatestPrice();
                valueFinal = value.div((priceMainToUSD).div(10 ** uint256(decimals)));
            }
            totalIncomeNFT = totalIncomeNFT.add(valueFinal);
            consumersRevenueAmount[buyer] = consumersRevenueAmount[buyer].add(value);
            consumersRevenueAmount[seller] = consumersRevenueAmount[seller].add(value);
    ...
    function _bidWin (
        ...
        updateFirstDateAndValue(sender, auction.seller, auction.price, auction.token);
        ...
        contractsSenderPlusTokenIdList[contractNFT] = contractsSenderPlusTokenIdsListUpdated(contractNFT, addressPlusTokenId(auction.seller, tokenId));
...
//        address auctionToken = address(auction.token);

        require(indexOfNFTsAuctionList(contractNFT, addressPlusTokenId(auction.seller, tokenId)) != type(uint256).max, "OpenBiSeaAuction: auction is not created");
        // require(contractsTokenIdsList[_contractNFT].contains(uint256(auction.seller).add(_tokenId)), "OpenBiSeaAuction: auction is not created"); // ERC1155 can have more than 1 auction with same ID and , need mix tokenId with seller address
    ...
    function _cancelAuction(
        ...
        contractsSenderPlusTokenIdList[contractNFT] = contractsTokenIdsListUpdated(contractNFT, addressPlusTokenId(auctionSeller, tokenId));
        contractsTokenIdList[contractNFT] = contractsTokenIdsListUpdated(contractNFT, tokenId);
    }
    ...
    function cancelAuction(
        ...
        require(msg.sender == openBiSeaMainContract, "OpenBiSeaAuction: only main contract can send it");
        require(isContractNFTWhitelisted(contractNFT), "OpenBiSeaAuction: contract must be whitelisted");
        require(indexOfNFTsAuctionList(contractNFT, addressPlusTokenId(sender, tokenId)) != type(uint256).max, "OpenBiSeaAuction: auction is not created");
    ...
    function checkTokensForClaim(
        address customer, uint256
        priceMainToUSD
    ) public view returns (uint256,uint256,uint256,bool) {
        uint256 amountTotalUSDwei = consumersRevenueAmount[customer].mul(priceMainToUSD);
