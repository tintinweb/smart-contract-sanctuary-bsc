...
contract LocoPartnerSale is Ownable, Pausable {
    ...
    function deposit(address _referred) public payable whenNotPaused {
        require(msg.value >= _pertoken,'DIVISION ERROR');
        UserInfo storage user = userInfo[msg.sender];

        uint256 _sold = msg.value / _pertoken;
    ...
    function deposit(address _referred) public payable whenNotPaused {
        ...
        uint256 _totalPrice = _sold * _pertoken;
        uint256 _payment = (_sold * _percent) / 100;

        require(
            msg.sender.balance >= _totalPrice,
    ...
    */
    ...
    // Helpers
    function setPertoken(uint256 pertoken)
        external
        onlyOwner
        returns (bool)
    {
