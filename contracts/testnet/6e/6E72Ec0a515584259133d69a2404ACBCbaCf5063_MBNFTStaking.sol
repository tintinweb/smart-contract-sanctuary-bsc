...
contract MBNFTStaking is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function getUserTotalReward(address _user, address _rewardToken) public view returns(uint256){
        return userInfo[_user].totalReward[_rewardToken];
    }
    ...
    function _withdrawReward() internal {
        ...
        for(uint i = 0; i < _listRewardTokens.length; i++){
            ...
            if(pending > 0){
                ...
                info.totalReward[_listRewardTokens[i]] = info.totalReward[_listRewardTokens[i]] + pending;
