...
contract PandoPotV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function _generateMegaTicket(uint256 _rand) internal view returns(uint256) {
        if (nSeeds > 0) {
            return _rand * seeds[_rand % nSeeds] % megaSampleSpace;
        }
        return _rand % megaSampleSpace;
    }

    function _generateMinorTicket(uint256 _rand) internal view returns(uint256, uint256) {
        if (nSeeds > 0) {
            return (_rand * seeds[_rand % nSeeds] % minorSampleSpace, _rand * seeds[(_rand % nSeeds + nSeeds) / 2] % minorSampleSpace);
        }
        return (_rand % minorSampleSpace, _rand / minorSampleSpace % minorSampleSpace);
    }
    ...
    function _updateRound(uint256 _id) internal {
        ...
        if (_roundInfo.status == 0) {
            ...
            updatePandoPot();
            _calcMegaPrize(_id, currentMegaNumber);
            _calcMinorPrize(_id, currentMinorNumber1);
            _calcMinorPrize(_id, currentMinorNumber2);
        }
    }

    function _calcMegaPrize(uint256 _roundId, uint256 _megaNumber) internal {
    ...
    function _calcMinorPrize(uint256 _roundId, uint256 _minorNumber) internal {
    ...
    function enter(address _receiver, uint256 _rand, uint256 _quantity) external whenNotPaused nonReentrant onlyWhitelist() {
        ...
        uint256 _space = megaSampleSpace;
        uint256 _salt = block.timestamp;
        for (uint256 i = 0; i < _quantity; i++) {
            _megaTicket = _generateMegaTicket(_rand);
            (_minorTicket1, _minorTicket2) = _generateMinorTicket(_rand);
            ...
            _rand = (_rand + i) * (_salt + i) % _space;
        }
        emit NewMegaTicket(_currentRoundId, _receiver, _megaTickets, block.timestamp + prizeExpireTime);
        emit NewMinorTicket(_currentRoundId, _receiver, _minorTickets, block.timestamp +prizeExpireTime);
    }
    ...
    function updatePandoPot() public {
        ...
        if (block.timestamp - lastDistribute >= timeBomb) {
            if (PSRForPreviousPot == 0 && USDForPreviousPot == 0) {
                USDForPreviousPot = USDForCurrentPot * megaPrizePercentage / 100;
    ...
    function updateLuckyNumber(uint256 _megaNumber, uint256 _minorNumber1, uint256 _minorNumber2) external onlyRNG {
        if (currentRoundId > 0) {
        ...
        _updateRound(currentRoundId);
        emit RoundResult(currentRoundId, _megaNumber, _minorNumber1, _minorNumber2);
        currentRoundId++;
    }
    ...
    event NewMegaTicket(uint256 roundId, address user, uint256[] number, uint256 expireTime);
    event NewMinorTicket(uint256 roundId, address user, uint256[] number, uint256 expireTime);
    ...
    event RoundResult(uint256 roundId, uint256 megaNumber, uint256 minorNumber1, uint256 minorNumber2);
}