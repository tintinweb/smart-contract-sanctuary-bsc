...
contract WorkspaceAccessFactoryUpgradeable
    is OwnableUpgradeable, SignatureAuth, SignedActions, PaidActions
{
    ...
    uint256 immutable public newWorkspaceAccessContractFee;

    event AddNewWorkspaceAccessContract (address contractAddress, uint256 workspaceId);

    constructor(address signer_, uint256 newWorkspaceAccessContractFee_, address feeCollector_) {
        _signer = signer_;
        newWorkspaceAccessContractFee = newWorkspaceAccessContractFee_;
    ...
    function newWorkspaceAccessContract(
        ...
        require (isExecuted(interactionId) == false, "WorkspaceAccessFactoryUpgradeable: Already executed");
        executedMap[interactionId] = true;
        bytes32 message = keccak256(abi.encodePacked(
            interactionId, ACCESS_CREATE_CONTRACT, workspaceId, _msgSender(), isFree
        ));
        require(_verify(message, signature), "WorkspaceAccessFactoryUpgradeable: INVALID_SIGNATURE");
        ...
        workspaceAccessContracts[workspaceId] = proxyAddress;
        ...
        emit AddNewWorkspaceAccessContract(proxyAddress, workspaceId);
        return proxyAddress;
    }
...
contract SignedActions {
    ...
    uint256 constant public FILES_CREATE_COLLECTION = 1;
    uint256 constant public FILES_ADD_TOKEN = 2;
    uint256 constant public FILES_MINT_TOKEN = 3;
    uint256 constant public FILES_CHANGE_IS_ON_DEX = 4;
    uint256 constant public FILES_SET_ROYALTY = 5;

    uint256 constant public ACCESS_CREATE_CONTRACT = 11;
    uint256 constant public ACCESS_SET_DONATION = 12;
    uint256 constant public ACCESS_PLAN_ACTIVATION = 13;
}
...
abstract contract PaidActions {
    ...
    modifier checkFeeValue(uint256 feeForOperation, bool isFree) {
        require(isFree || msg.value >= feeForOperation, 'Amount paid is less then expected fee');
        _;
    }
