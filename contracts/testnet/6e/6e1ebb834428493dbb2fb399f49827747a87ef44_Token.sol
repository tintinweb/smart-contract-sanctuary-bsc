...
contract Token is Context, IERC20, Ownable {
    ...
    address USDT = 0x2e8ECf3EE4A6C92410be70A9d9044d6B5c1Ba8BF;
    ...
    uint256 public lpFeeOnBuying = 10; // 1% will be added to the Lp address

    uint256 public liquidityFeeOnSelling = 20; // 1% will be added to the liquidity
    uint256 public lpFeeOnSelling = 10; // 1% will be added to the Lp address
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "$Token: already enabled");
        trading = true;
        feesStatus = true;
    ...
    function totalSellFeePerTx(uint256 amount) public view returns (uint256) {
        uint256 fee = amount.mul(lpFeeOnSelling.add(liquidityFeeOnSelling)).div(
            percentDivider
        );
        return fee;
    }
    ...
    function setFeeCountersOnBuying(uint256 amount) private {
        ...
        lpFeeCounter += amount.mul(lpFeeOnBuying).div(percentDivider);
    }
    ...
    function setFeeCountersOnSelling(uint256 amount) private {
        ...
        lpFeeCounter += amount.mul(lpFeeOnSelling).div(percentDivider);
    }
    ...
    function distributeAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 deltaBalance = IERC20(USDT).balanceOf(address(this)).sub(
                balanceBefore
            );

            uint256 USDTToBeAddedToLiquidity = deltaBalance
                .mul(halfLiquidity)
                .div(tokenAmountToBeSwapped);

            // add liquidity to Dex
            if (USDTToBeAddedToLiquidity > 0) {
                Utils.addLiquidity(
                    address(dexRouter),
                    owner(),
                    otherHalfLiquidity,
                    USDTToBeAddedToLiquidity
                );
            ...
            uint256 USDTForLp = IERC20(USDT).balanceOf(address(this)).sub(
                USDTToBeAddedToLiquidity
            );

            // sending USDT to Lp wallet
            if (USDTForLp > 0) IERC20(USDT).transfer(TaxWallet, USDTForLp);
...
library Utils {
    ...
    function swapTokensForTokens(address routerAddress, uint256 tokenAmount)
        internal
    {
        ...
        path[0] = address(this);
        path[1] = 0x2e8ECf3EE4A6C92410be70A9d9044d6B5c1Ba8BF;
    ...
    function addLiquidity(
        ...
        dexRouter.addLiquidity(
            address(this),
            0x2e8ECf3EE4A6C92410be70A9d9044d6B5c1Ba8BF,
            tokenAmount,
            USDTamount,
            0,
            0,
