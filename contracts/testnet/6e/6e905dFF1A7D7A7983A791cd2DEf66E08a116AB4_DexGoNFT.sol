...
contract DexGoNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable {
    ...
    function _distributeMoney(address sender, uint value, bool isIOS) private returns (uint) {
        ...
        Address.sendValue(payable(IDexGoStorage(storageContract).getDexGo()), partForRewards);

        uint valueForTeam = (value - valueForFriendTotal - partForRewards) ;
        Address.sendValue(payable(IDexGoStorage(storageContract).getAccountTeam1()), valueForTeam * 33 / 100);
        valueForTeam = valueForTeam - valueForTeam * 33 / 100;
        Address.sendValue(payable(IDexGoStorage(storageContract).getAccountTeam2()), valueForTeam * 33 / 100);
        valueForTeam = valueForTeam - valueForTeam * 33 / 100;
        Address.sendValue(payable(owner()), valueForTeam);
