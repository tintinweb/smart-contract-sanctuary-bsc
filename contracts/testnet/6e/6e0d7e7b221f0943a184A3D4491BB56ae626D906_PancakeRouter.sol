...
interface IPancakePair {
    ...
    function baseToken() external view returns (address);    function getTotalFee() external view returns (uint);
...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'd5470a2fb36b5538ea4f3d0082201cc0101bac22a39fbf912d2bacc66b2f8e23' // init code hash of factory address
            ))));
    }
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i; i < path.length - 1; i++) {
            IPancakePair pair = IPancakePair(pairFor(factory, path[i], path[i + 1]));
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            IPancakePair pair = IPancakePair(pairFor(factory, path[i - 1], path[i]));
            address baseToken = pair.baseToken();
            uint totalFee = pair.getTotalFee();
...
abstract contract FeeStore is Ownable {
    ...
    function feeAdddressSetWhileSwap(address pair,address tokenAddress) public onlyOwner {
        require(IPancakeFactory(factoryAddress).pairExist(pair), "Pancake FeeStore: Pair does not exist");
        require(IPancakePair(pair).token0() == tokenAddress || IPancakePair(pair).token1() == tokenAddress, "Pancake FeeStore: Invalid token address");
...
contract PancakeRouter is FeeStore, IPancakeRouter02 {
    using SafeMath for uint;
    ...
    function _addLiquidity(
        ...
        if (IPancakeFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            if(tokenA == WETH) {
                IPancakeFactory(factory).createPair(tokenB, tokenA, supportsTokenFee);
                pairFeeAddress[getPair(tokenA,tokenB)] = tokenA;
            } else {
                IPancakeFactory(factory).createPair(tokenA, tokenB, supportsTokenFee);
    ...
    function swapTokensForExactTokens(
        ...
        require(path.length == 2, "PancakeRouter: ONLY_TWO_TOKENS_ALLOWED");
    ...
    function _calculateAmounts(address input, address output, address token0) internal view returns (uint amountInput, uint amountOutput) {
        ...
        amountOutput = PancakeLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, baseToken != address(0), totalFee);
    }
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT'); // Could be problematic?
    ...
    {
        return PancakeLibrary.getAmountOut(amountIn, reserveIn, reserveOut, false, 0);
    }
    ...
    {
        return PancakeLibrary.getAmountIn(amountOut, reserveIn, reserveOut, false, 0);
    }
...
// File: Pancake_swap-periphery/contracts/interfaces/IToken.sol
