...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20  USDT = IBEP20(0xEdA5dA0050e21e9E34fadb1075986Af1370c7BDb);
    address WBNB = 0x4c6289890009d7358e522d8BA97287a29F1988bB;
    IDEXRouter router;
    ...
    constructor (address _router) {
        router = _router != address(0)
            ? IDEXRouter(_router)
            : IDEXRouter(0xB6BA90af76D139AB3170c7df0139636dB6120F7e);
        _token = msg.sender;
    }
...
/* Token contract */
contract RussianBlueCat is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address RBuyback = 0xbDbC91936044086BFF1CAe93e13aa0833A1cbbdE;
    ...
    // Name and symbol
    string constant _name = "RussianBlueCat";
    string constant _symbol = "RCat";
    ...
    uint256 public _maxSellTxAmount = _totalSupply * 400 / 10000; // 
    ...
    uint256 public RBuybackFeeBuy = 2;
    ...
    uint256 public RBuybackFeeSell = 0;
    ...
    constructor () Auth(msg.sender) {
        ...
        isTimelockExempt[RBuyback] = true;
        ...
        marketingFeeReceiver = 0xbDbC91936044086BFF1CAe93e13aa0833A1cbbdE;
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if (!authorizations[sender] && recipient != owner && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver && recipient != RBuyback){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBRBuyback = amountBNB.mul(RBuybackFee).div(totalBNBFee); 


        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool successMarketing, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool successRBuyback, /* bytes memory data */) = payable(RBuyback).call{value: amountBNBRBuyback, gas: 30000}(""); 
        require(successMarketing, "marketing receiver rejected ETH transfer");
        require(successRBuyback, "RBuyback receiver rejected ETH transfer");
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _RBuybackFeeBuy, uint256 _feeDenominator) external authorized {
        liquidityFeeBuy = _liquidityFeeBuy;
        buybackFeeBuy = _buybackFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        RBuybackFeeBuy = _RBuybackFeeBuy;
        totalFeeBuy = _liquidityFeeBuy.add(_buybackFeeBuy).add(_reflectionFeeBuy).add(_marketingFeeBuy).add(_RBuybackFeeBuy);
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _RBuybackFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        buybackFeeSell = _buybackFeeSell;
        reflectionFeeSell = _reflectionFeeSell;
        marketingFeeSell = _marketingFeeSell;
        RBuybackFeeSell = _RBuybackFeeSell;
        totalFeeSell = _liquidityFeeSell.add(_buybackFeeSell).add(_reflectionFeeSell).add(_marketingFeeSell).add(_RBuybackFeeSell);
        feeDenominator = _feeDenominator;
    }
