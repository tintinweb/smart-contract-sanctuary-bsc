...
contract NichoAuction is Ownable{
    ...
    event OfferSoldOut(address tokenAddress, uint256 tokenId, address seller, address buyer);
    event OfferCancel(address tokenAddress, uint256 tokenId);
    ...
    function acceptBid(uint256 auctionId, uint256 bidderId)
        ...
        external {
            ...
            if (IOERC721(item.tokenAddress).getApproved(item.tokenId) == address(this)) {
                ...
                emit SoldOut(auctionId, buyer.bidPrice, buyer.owner);
    ...
    function acceptOffer(address tokenAddress, uint256 tokenId, address offerCreator) 
        ...
        external {
            ...
            emit OfferSoldOut(tokenAddress, tokenId, msg.sender, item.creator);
        }
    ...
    function cancelOffer(address tokenAddress, uint256 tokenId)
        external {
            ...
            emit OfferCancel(tokenAddress, tokenId);
        }
