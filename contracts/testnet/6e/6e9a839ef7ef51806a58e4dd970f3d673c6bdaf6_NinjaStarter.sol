...
contract NinjaStarter is ReentrancyGuard, Ownable, Pausable {
    ...
    address public xNinjaMaster =
        address(0x4Dbb8A19FacB2877059078f24ddAad74a203D4C5);

    // total amount of offeringToken that will offer
    uint256 public offeringAmount = 5000000 * 1e18;

    // offering token decimals
    uint8 private _decimals = 13;

    // IDO starting date
    uint256 public startDate = 1649178000;

    // limit on each address on buy
    uint256 public buyCap = 0;

    // offering token price in BUSD 0.05 busd
    ...
    // initial unlocked tokens after ido ended
    uint256 public initialUnlockPercent = 20;
    ...
    uint256 public NinjaStakingBalanceRequired = 50;
    ...
    address payable public liqudityManager =
        0xB1f652a4130792cbD9ef1d42384Fc4124E690B1b;
    ...
    // is claimed tokens opened
    bool public isClaimTokensOpen = false;

    // is refund of funds open in case of un succesfull
    bool public isRefundClaimOpen = false;
        ...
        checkBuyCondition(msg.value, _beneficiary)
    {
        ...
        purchases[_beneficiary] = purchases[_beneficiary].add(
            tokensToBePurchased
        );
    ...
    modifier checkBuyCondition(uint256 amount, address _user) {
        ...
        require(!isRefundClaimOpen && !isClaimTokensOpen, "You can't buy now");
        require(amount > 0, "Please send more amount");
        require(isEnded == false, "IDO ended");
        if (whitelistChecker) {
            require(isWhitelisted(_user), "You're not whitelisted to join ido");
        }
        _;
    }

    function safeOfferingTokenTransfer(address _to, uint256 _amount) internal {
        if (_decimals == 18) {
            offeringToken.safeTransfer(_to, _amount);
        } else {
            uint256 diff = uint256(18).sub(uint256(_decimals));
            offeringToken.safeTransfer(_to, _amount.div(10**diff));
        }
    }
    ...
    function claimRefund() external {
        require(isRefundClaimOpen, "Refund Claims are not opened");
        require(
            busdDeposits[msg.sender] > 0 || bnbDeposits[msg.sender] > 0,
            "You have not bought any token or already claimed refund"
        );
        if (busdDeposits[msg.sender] > 0) {
            BUSD.safeTransfer(address(msg.sender), busdDeposits[msg.sender]);
            emit Refunded(msg.sender, busdDeposits[msg.sender]);
            busdDeposits[msg.sender] = 0;
        }
        if (bnbDeposits[msg.sender] > 0) {
            address payable refund = payable(msg.sender);
            refund.transfer(bnbDeposits[msg.sender]);
            emit Refunded(msg.sender, busdDeposits[msg.sender]);
            bnbDeposits[msg.sender] = 0;
        }
        purchases[msg.sender] = 0;
    }

    function initialTokenClaim() external {
        require(isClaimTokensOpen, "Token Claims are not opened");
        require(purchases[msg.sender] > 0, "You have not bought any token");
        uint256 _amount = purchases[msg.sender];
        uint256 initialUnlockAmount = _amount.mul(initialUnlockPercent).div(
            100
        );
        safeOfferingTokenTransfer(address(msg.sender), initialUnlockAmount);
        Claimed[msg.sender] = Claimed[msg.sender].add(initialUnlockAmount);
        uint256 remainingAmount = _amount.sub(initialUnlockAmount);
        uint256 unlockedTokensEveryRound = remainingAmount.div(vestingRounds);
    ...
    function releaseAll() external {
        ...
        safeOfferingTokenTransfer(address(msg.sender), available);
        Claimed[msg.sender] = Claimed[msg.sender].add(available);
    ...
    function pauseContract() external authorized whenNotPaused {
        _pause();
    }

    function unPauseContract() external authorized whenPaused {
        _unpause();
    }
    ...
    function setVestingSettings(
        ...
        initialUnlockPercent = _initialUnlockPercent;
    }

    function setClaimsSettings(bool _isClaimTokensOpen, bool _isRefundClaimOpen)
        public
        onlyOwner
    {
        isClaimTokensOpen = _isClaimTokensOpen;
        isRefundClaimOpen = _isRefundClaimOpen;
    }

    function setWhitelistSettings(
        bool _whitelistChecker,
        uint256 _NinjaStakingBalanceRequired
    ) public onlyOwner {
    ...
    function setIDoAddresses(
        address payable _feeAddress,
        IBEP20 _busd,
        IBEP20 _offeringToken,
        address _xNinjaMaster,
        address payable _liqudityManager
    ) external onlyOwner {
    ...
    function WithdrawFunds() public onlyOwner {
        ...
        if (busdBalance > 0) {
            ...
            uint256 busdLiqudity = busdBalance.mul(liquidityInPercentage).div(
                100
            );
        ...
        if (bnbBalance > 0) {
            ...
            uint256 bnbLiqudity = bnbBalance.mul(liquidityInPercentage).div(
                100
            );
    ...
    function isWhitelisted(address _address) public view returns (bool) {
        uint256 poolId = 0;
        (uint256 _deposited, , ) = IMasterChef(xNinjaMaster).userInfo(
            poolId,
            _address
        );
        return
            _deposited >= (NinjaStakingBalanceRequired * (10**18))
                ? true
                : false;
    }
}