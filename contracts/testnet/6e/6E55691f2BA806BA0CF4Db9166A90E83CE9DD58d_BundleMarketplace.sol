...
contract BundleMarketplace is OwnableUpgradeable, ReentrancyGuard{
    using SafeMath for uint256;
    using AddressUpgradeable for address payable;
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.Bytes32Set;
    // Events for the contract
    event ItemListed(address indexed owner,string bundleID,address payToken,uint256 price,uint256 startingTime);
    event ItemSold(address indexed seller,address indexed buyer,string bundleID, address payToken,int256 unitPrice,uint256 price);
    event ItemUpdated(address indexed owner,string bundleID,address[] nft,uint256[] tokenId,uint256[] quantity,address payToken,uint256 newPrice);
    event ItemCanceled(address indexed owner, string bundleID);
    event OfferCreated(address indexed creator,string bundleID,address payToken,uint256 price,uint256 deadline);
    event OfferCanceled(address indexed creator, string bundleID);
    event UpdatePlatformFee(uint256 platformFee);
    event UpdatePlatformFeeRecipient(address payable platformFeeRecipient);
    //Structure for Bundle Item Listing
    struct Listing {
        address[] nfts;
        uint256[] tokenIds;
        uint256[] quantities;
    ...
    // Owner -> Bundle ID -> Bundle Listing item
    ...
    modifier onlyContract() {
        require(addressRegistry.auction() == _msgSender() || addressRegistry.marketplace() == _msgSender(),"sender must be auction or marketplace");
        _;
    }
    // Contract initializer
    function initialize(address payable _feeRecipient, uint256 _platformFee)public initializer{
    ...
    // Owner address
    //_bundleID Bundle ID
    function getListing(address _owner, string memory _bundleID)external view
        returns (address[] memory nfts,uint256[] memory tokenIds,uint256[] memory quantities,uint256 price,uint256 startingTime){
    ...
    // _bundleID Bundle ID
    // _nftAddresses Addresses of NFT contract
    // _tokenIds Token IDs of NFT
    // _quantities token amounts to list (needed for ERC-1155 NFTs, set as 1 for ERC-721)
    // _price sale price for bundle
    // _startingTime scheduling for a future sale
    function listItem(string memory _bundleID,address[] calldata _nftAddresses,uint256[] calldata _tokenIds,uint256[] calldata _quantities,address _payToken,uint256 _price,uint256 _startingTime) external {
        bytes32 bundleID = _getBundleID(_bundleID);
        bundleIds[bundleID] = _bundleID;
        require(_nftAddresses.length == _tokenIds.length && _tokenIds.length == _quantities.length,"invalid data");
        require(owners[bundleID] == address(0) || (owners[bundleID] == _msgSender() &&
                    listings[_msgSender()][bundleID].price == 0),"already listed");
        address tokenRegistry = addressRegistry.tokenRegistry();
        require(_payToken == address(0) || (tokenRegistry != address(0) && ITokenRegistry(tokenRegistry).enabled(_payToken)),"invalid pay token");
        ...
        emit ItemListed(_msgSender(),_bundleID, _payToken,_price,_startingTime);
    }
    ...
    // _bundleID Bundle ID
    // _newPrice New sale price for bundle
    function updateListing(string memory _bundleID, address _payToken,uint256 _newPrice) external nonReentrant {
        bytes32 bundleID = _getBundleID(_bundleID);
        Listing storage listing = listings[_msgSender()][bundleID];
        require(listing.price > 0, "not listed");
        address tokenRegistry = addressRegistry.tokenRegistry();
        require(_payToken == address(0) || (tokenRegistry != address(0) &&
                    ITokenRegistry(tokenRegistry).enabled(_payToken)),"invalid pay token");
        listing.payToken = _payToken;
        listing.price = _newPrice;
        emit ItemUpdated(_msgSender(),_bundleID,listing.nfts,listing.tokenIds,listing.quantities,_payToken,_newPrice);
    }
    // Method for buying listed NFT bundle
    // _bundleID Bundle ID
    function buyItem(string memory _bundleID, address _payToken)external nonReentrant{
        bytes32 bundleID = _getBundleID(_bundleID);
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(feeTransferSuccess,"arketplace: Fee transfer failed");
            (bool ownerTransferSuccess, ) = owner.call{
                value: price.sub(feeAmount)
            }("");
            require(ownerTransferSuccess,"Marketplace: Owner transfer failed");
        } else {
            IERC20(_payToken).safeTransferFrom(_msgSender(),feeReceipient,feeAmount);
            IERC20(_payToken).safeTransferFrom(_msgSender(),owner,price.sub(feeAmount));
        }
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IMarketplace(addressRegistry.marketplace()).validateItemSold(listing.nfts[i],listing.tokenIds[i],owner,_msgSender());
        }
        ...
        emit ItemSold(owner,_msgSender(), _bundleID, _payToken,IMarketplace(addressRegistry.marketplace()).getPrice(_payToken),price);
    ...
    function createOffer(string memory _bundleID,IERC20 _payToken,uint256 _price,uint256 _deadline) external {
        bytes32 bundleID = _getBundleID(_bundleID);
    ...
    function acceptOffer(string memory _bundleID, address _creator)external nonReentrant{
        bytes32 bundleID = _getBundleID(_bundleID);
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IMarketplace(addressRegistry.marketplace()).validateItemSold(listing.nfts[i],listing.tokenIds[i],owners[bundleID],_creator);
        }
        ...
        emit ItemSold(_msgSender(),_creator,_bundleID,address(offer.payToken),
            ...
            if (_owner != address(0)) {
                ...
                if (listing.quantities[index] > _quantity) {
                    ...
                    if (listing.nfts.length == 1) {
                        ...
                        emit ItemUpdated(_owner,_bundleID,new address[](0),new uint256[](0),new uint256[](0),address(0),0);
                        continue;
                    } else {
                ...
                emit ItemUpdated(_owner,_bundleID,listing.nfts,listing.tokenIds,listing.quantities,listing.payToken,listing.price);
            }
        }
        delete (bundleIdsPerItem[_nftAddress][_tokenId]);
    }
    // Internal and Private 
    function _supportsInterface(address _addr, bytes4 iface)internal view returns (bool){
        return IERC165(_addr).supportsInterface(iface);
    }
    function _check721Owning(address _nft,uint256 _tokenId,address _owner) internal view {
