...
contract Test is IERC20, Ownable {
    ...
    function disableAllFees() internal returns (bool) {
        marketingFee = 0;
        totalFee = 0;
    ...
    function _transferFrom(
        ...
        // We want to disable all fees about six months after the token launch.
        bool shouldDisableAllFees = (block.timestamp -
            initialContractSwapTime) > (1 days);
        if (shouldDisableAllFees) {
            doContractSwap();
    ...
    function takeFee(address sender, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 feeAmount = (amount * totalFee);
    ...
    function doContractSwap() internal swapping {
        ...
        uint256 swappedBUSDAmount = address(this).balance;

        uint256 busdForMarketing = (swappedBUSDAmount * marketingFee) /
            totalFee;

        if (busdForMarketing > 0) {
            IERC20(BUSD).transfer(marketingWalletAddress, busdForMarketing);
        }
