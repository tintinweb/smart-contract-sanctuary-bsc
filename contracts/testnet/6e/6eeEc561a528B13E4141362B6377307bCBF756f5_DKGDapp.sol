...
contract DKGDapp is Ownable {
    ...
    uint256 constant MaxTime = 100000 days;
    ...
    address fistToken = address(0x3DeF6129681c977bCaFcd87FFFC084BAd068Eb07); //fist token
    address mineToken = address(0x659A5521578E510187d3B2c58197aA37b49C27f1); //
    ...
    function transferFee() private {
        uint256 marketAmount = 3 * 10 ** (FistDecimal - 1);
        {
            ...
            for (int256 i = 0; i < 2; i++) {
                ...
                uint256 amount;
                if (i == 0) {
                    amount = 1 * 10 ** (FistDecimal - 1);
                } else if (i == 1) {
                    amount = 5 * 10 ** (FistDecimal - 2);
                }
            ...
            if (15 * 10 ** (FistDecimal - 2) > totalAmount) {
                marketAmount = marketAmount + (15 * 10 ** (FistDecimal - 2) - totalAmount);
            }
        }
        ...
        {
            //! lp分红池
            uint256 amount = 2 * 10 ** (FistDecimal - 1);
        ...
        {
            //! 市场回购组LP
            uint256 amount = 35 * 10 ** (FistDecimal - 2);
    ...
    function transferFrom(address _inviter) public returns (bool) {
        require(IERC20(fistToken).balanceOf(msg.sender) >= 1 * 10 ** FistDecimal, "ERC20: not have enough fist amount");
        require(buyTimeMap[msg.sender].startTime == 0, "pls get your reward");
        require(IERC20(fistToken).approve(address(this), 10 ** FistDecimal), 'Failed to approve tokens');
    ...
    function getAward() public returns (bool) {
        ...
        require(IERC20(fistToken).balanceOf(msg.sender) >= 10 ** FistDecimal, "ERC20: not have enough fist amount");
        require(PhaseCnt > curPhase, "mine time is finish");

        require(PhaseCnt > curPhase, "mine time is finish"); // 数量限制
    ...
    //! 我的直接下级
    function getMyFistInvitees() public view returns (uint256) {
        return invitee[msg.sender].length;
    }
    ...
    function getMyStartTime() public view returns (uint256) {
        return buyTimeMap[msg.sender].startTime;
    }

    function setPlayerInfo(uint256 startTime, uint256 awardAmount) public {
        buyTimeMap[msg.sender] = PlayerInfo({
        startTime : startTime,
    ...
    function getLpAmount() public view returns (uint256) {
        return IBEP20(lpToken).balanceOf(msg.sender);
    }
