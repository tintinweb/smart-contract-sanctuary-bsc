...
contract SmartBeComBee {
    ...
    function registrationbase(address userAddress, address referrerAddress, address tknAddress) private {
        ...
        if (tokenAddress != nullAddress){
            ...
            if (tokenAddress == 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee){
    ...
    function usersB6Matrix2(address userAddress, uint8 level) public view returns(address, address[] memory, address[] memory, bool, address, uint, uint8) {
        return (users[userAddress].b6Matrix[level].currentReferrer,
                users[userAddress].b6Matrix[level].firstLevelReferrals,
                users[userAddress].b6Matrix[level].secondLevelReferrals,
                users[userAddress].b6Matrix[level].blocked,
