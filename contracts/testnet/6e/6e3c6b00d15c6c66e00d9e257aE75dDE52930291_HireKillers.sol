...
contract HireKillers is Context, Ownable {
    ...
    uint256 private devFeeVal = SafeMath.div(25,10); //2.5%
    ...
    function sellKillers() public {
        ...
        recAdd.transfer(fee);
        payable (msg.sender).transfer(SafeMath.sub(killValue,fee));
    }
    ...
    function buyKillers(address ref) public payable {
        require(initialized);
		uint256 killersBought = calculateKillerBuy(msg.value,SafeMath.sub(address(this).balance,msg.value));
    ...
    function calculateKillerSell(uint256 killers) public view returns(uint256) {
        return calculateTrade(killers,marketKillers,address(this).balance);
    }
    ...
    function calculateKillerBuySimple(uint256 eth) public view returns(uint256) {
        return calculateKillerBuy(eth,address(this).balance);
    }
