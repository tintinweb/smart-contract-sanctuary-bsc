...
abstract contract GEMUNIStaking is PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, IGEMUNIStaking{
    ...
    function addPoolNft(
        ...
        require(startTime < endTime, "Staking: Invalid time");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function withdrawNFT(uint pid, uint256 passId) external override onlyTokenOwnerStaking(pid, passId) validPool(pid) nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (user.currentRewardStored > 0) {
                claimRewardNft(pid, pool, user, pending);
            }
    ...
    function claimRewardNft(uint _pid, PoolInfoNft storage pool, UserInfoNft storage user, uint pending) internal {
    ...
    function emergencyWithdrawNFT(uint pid, uint[] memory passIds) external override validPool(pid) {
        ...
        for (uint256 i = 0; i < passIds.length; i++) {
            require(NFTStaked[pid][passIds[i]].user == msg.sender && IERC721(pool.tokenPoolInfo.stakedNFT).ownerOf(passIds[i]) == address(this), "Staking: not pass staked owner");
