...
contract AGG is ERC20, Ownable {
    ...
    AGGDividendTracker public dividendTracker;

    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0x173bBBA35eEF99Cf4809fAE4177bf31b8DCaC250;

    uint256 public swapTokensAtAmount = 5 * 10**9;
    uint256 public maxWalletBalance = 5 * 10**9;
    uint256 public maxBuyAmount = 5 * 10**9;
    uint256 public maxSellAmount = 5 * 10**9;
    ...
    Taxes public buyTaxes = Taxes(2, 1, 0);
    ...
    constructor() ERC20("AGG", "AGG") {
        dividendTracker = new AGGDividendTracker();
        ...
        */
        _tokengeneration(owner(), 21000 * (10**9));
    }

    receive() external payable {}

    function updateDividendTracker(address newAddress) public onlyOwner {
        AGGDividendTracker newDividendTracker = AGGDividendTracker(
            payable(newAddress)
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "AGG: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setMaxWallet(uint256 amount) external onlyOwner {
        ...
        require(amount >= 500000, "Cannot set max wallet amount lower than 0.05%");
    }
    ...
    function setMaxBuyLimits(uint256 maxBuy) external onlyOwner{
        ...
        require(maxBuy >= 500000, "Cannot set max buy amount lower than 0.05%");
    }

    function setMaxSellLimits(uint256 maxSell) external onlyOwner{
        maxSellAmount = maxSell * 10**decimals();
        require(maxSell >= 500000, "Cannot set max sell amount lower than 0.05%");
    }
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(
            ...
            "AGG: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[newPair] = value;
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "AGG: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "AGG: Cannot update gasForProcessing to same value"
        );
...
contract AGGDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("AGG_Dividen_Tracker", "AGG_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "AGG_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "AGG_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "AGG_Dividend_Tracker: Cannot update claimWait to same value"
        );
