...
contract GLDY is Context, IERC20 { 
    ...
    address payable public Wallet_bonus;
    ...
    function setWalletPoolAddress(address wallet)   public virtual onlyOwner  returns (bool) {
        ...
        _tOwned[Wallet_FirstEcology] = 81*10**8 * 10**_decimals;
        emit Transfer(address(0), Wallet_FirstEcology, 81*10**8 * 10**_decimals);

        return true;
    }
    //4.5亿空投、15亿卓越贡献，2亿技术，共21.5亿
    function setWalletbonusAddress(address wallet)   public virtual onlyOwner  returns (bool) {
         if(Wallet_bonus!=address(0)){
            _tOwned[Wallet_bonus]=0;
        }
        Wallet_bonus=payable(wallet);
        _tOwned[Wallet_bonus] = 215*10**7 * 10**_decimals;
        emit Transfer(address(0), Wallet_bonus, 215*10**7 * 10**_decimals);
        ...
        if(!takeFee){
            ...
            if(_isBurnAddress[recipient]==true){
               ...
               totalpool=totalpool+buyFEE;
                ...
                if (
                    !swapping && totalpool>=10000 * 10 **18
                ) {
                   swapping = true;
            ...
            emit Transfer(sender, recipient, tTransferAmount);
           //发送令牌消息
            // _tOwned[Wallet_bonus] = _tOwned[Wallet_bonus]+GtokenFEE;
            uint256 GtokenUserCount=tokenInterFace(Wallet_FirstEcology).GtokenUserCount();
            ...
            uint256 contractTokenBalance = balanceOf(address(this));
               if(totalfee>contractTokenBalance){
                ...
                if (
                    !swapping && totalfee>=10000 * 10 **18
                ) {
                    swapping = true;
                    swapTokensForEth(totalfee);
                    totalfee=0;
                    uint256 balance= address(this).balance;
                    Wallet_bonus.transfer(balance );
                    tokenInterFace(Wallet_bonus).addSharePools(GtokenUserCount,balance,address(0),1);
