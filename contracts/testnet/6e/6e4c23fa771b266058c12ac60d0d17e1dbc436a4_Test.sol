...
contract  MAoE is Context, ERC20, Ownable {
    ...
    uint256 public taxThreshold = 20;
...
// File: contracts/Token-test1.sol
...
contract Test is Ownable {
    ...
    uint256 public numOfToken = 0;
    uint256 public numOfRate = 0;
    // address public tokenFunds;
    uint256 public minFunds = 100;
    uint256 public maxFunds = 10000;
    ...
    mapping(address => uint256) public tokenInContract;


    constructor(
        address _BUSD,
        address[] memory _token,
        uint256[] memory _tokenRate
        // address _tokenFunds

    ) {
        BUSD = _BUSD;
        adminList[msg.sender] = 1;
        // tokenFunds = _tokenFunds;
        for(uint256 i =0; i < _token.length ;i++){
            token[i] = _token[i];
            tokenInContract[token[i]] = 0;
            numOfToken++;
        }
        for(uint256 i =0; i < _tokenRate.length ;i++){
            tokenRate[i] = _tokenRate[i];
            numOfRate++;
        }
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
        //     UNISWAP_V2_ROUTER
    ...
    modifier isNotInBlackList(){
        require(blackList[_msgSender()] == 0, "Revert blacklist");
        _;
    }
    ...
    // admin add  token and tokenrate
    function adminAddTokenToContract(address tokenAddress, uint256 amount) external onlyAdmin{
        IERC20 MATK = IERC20(tokenAddress);
        MATK.transferFrom(msg.sender ,  address(this), amount);
        tokenInContract[tokenAddress] += amount;

    }

    function changeMinFunds(uint256 funds) public onlyAdmin{
        minFunds = funds;
    }

    function changeMaxFunds(uint256 funds) public onlyAdmin{
        maxFunds = funds;
    }


    function addTokenList(address tokenAddress) external onlyAdmin{
        token[numOfToken] = tokenAddress;
        numOfToken++;
    }


    function addRateList(uint256 rate) external onlyAdmin{
        tokenRate[numOfRate] = rate;
        numOfRate++;
    }

     
    function changeTokenRate(uint256 index, uint256 rate) external onlyAdmin{
        tokenRate[index] = rate;
    }

    function removeTokenRate(uint256 index) external onlyAdmin{
        require(index < numOfRate,"over num of rate");
        tokenRate[index] = tokenRate[numOfRate-1];
        numOfRate--;
    }



    function checkAmount(address tokenSwap, uint256 tokenFundForAccount) public view returns (bool) {
        ERC20 eRC20 = ERC20(tokenSwap);
        uint256 tokenAllowance = eRC20.allowance(msg.sender, tokenSwap);
        return tokenAllowance > tokenFundForAccount;
    }


    function userFundToLuckyGame(address tokenAddress, uint256 funds) public isNotInBlackList{
        ERC20 eRC20 = ERC20(tokenAddress);
        require(funds < maxFunds,"Amount is too big");
        require(funds > minFunds,"Amount is too small");
        userAmount[msg.sender][tokenAddress] += funds;
        eRC20.transferFrom(msg.sender, address(this), funds);
    }

    function luckyGame(address tokenAddress, uint256 amount) public isNotInBlackList{
        ERC20 eRC20 = ERC20(tokenAddress);
        userAmount[msg.sender][tokenAddress] -= amount;
        tokenInContract[tokenAddress] += amount;
        uint256 fundsForAccount= tokenCalculationReturn(amount);
        if(checkAmount(tokenAddress,fundsForAccount) == false){
            // ERC20 nERC20 = ERC20(BUSD);
            userAmount[msg.sender][BUSD] += (fundsForAccount - amount);
            userAmount[msg.sender][tokenAddress] += amount;
            tokenInContract[tokenAddress] = 0;
            // nERC20.transfer(msg.sender, fundsForAccount - amount);
        }else{
            tokenInContract[tokenAddress] -= fundsForAccount;
            userAmount[msg.sender][tokenAddress] += fundsForAccount;
        }
        // eRC20.transfer(msg.sender, fundsForAccount);
    }


    function tokenCalculationReturn(uint256 funds) public view returns(uint256){
        uint256 random = createRandomNumber();
        return funds*tokenRate[random]/100;
    }

    function createRandomNumber() public view  returns(uint256) {
        return uint256(blockhash(block.number -1 ))%(numOfRate);
    }


    function addToAdminlist(address accounts) external onlyOwner{
        adminList[accounts] = 1;
        emit AddedAdmin(accounts);
    }
    ...
    function addToBlacklist(address accounts) external onlyOwner{
        blackList[accounts] = 1;
    }
