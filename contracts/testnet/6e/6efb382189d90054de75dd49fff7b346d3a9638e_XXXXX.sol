...
contract XXXXX {
	...
	constructor(uint256 start) public {
        ...
        plans.push(Plan(1000, 500));
        plans.push(Plan(15, 700));
        plans.push(Plan(15, 800));
		plans.push(Plan(1000, 680));
	}
	...
	function getExtraRate(address _userAddress, uint256 _depositID) public view returns (uint256) {
		...
		uint timeMultiplier = (block.timestamp.sub(user.deposits[_depositID].checkpoint)).div(TIME_STEP).mul(5);
		if (timeMultiplier > MAX_EXTRA_RATE) {
			timeMultiplier = MAX_EXTRA_RATE;
		}
