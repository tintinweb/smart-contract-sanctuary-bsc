...
abstract contract ERC20 {
    ...
    function endAfterN2Y(uint nodeId,uint nodeTotalLP,uint totalEffective,uint _scale,uint timeDistance) external virtual;
    function endAfterY2N(uint nodeId,uint nodeTotalLP,uint totalEffective,uint _scale,uint timeDistance) external virtual;
}
...
contract NodePool is Modifier {
     ...
     */
    function addPower(uint nodeId,uint amountToWei,uint timeDistance) external onlyNode {
        ...
        handleEffective(nodeId,timeDistance);//处理有效
    }
     ...
     * @param amountToWei  金额
     */
    function removePower(uint nodeId,uint amountToWei,uint timeDistance) external onlyNode {
        if(nodePoolMapping[nodeId].createTime == 0){ _status = _NOT_ENTERED; revert("NodePool : Node does not exist"); }
     ...
     * @dev 查询 | 所有人调用 | 处理有效节点
     */
    function handleEffective(uint nodeId,uint timeDistance) private {
        if(firstNodeId != 0){
            ...
            do{
                if(nodePoolMapping[startId].totalLp >= producePowerMin && effectiveCount < produceRankingMin){
                    ...
                    if(nodePoolMapping[startId].isEffective == 0){
                        ...
                        if(startId != nodeId){
                            ERC20(nodeAddress).endAfterN2Y(startId,nodePoolMapping[startId].totalLp,totalLpEffective,nodePoolMapping[startId].scale,timeDistance);
                        }
                    }
                    ...
                    if(nodePoolMapping[startId].isEffective == 1){
                        ...
                        if(startId != nodeId){
                            ERC20(nodeAddress).endAfterY2N(startId,nodePoolMapping[startId].totalLp,totalLpEffective,nodePoolMapping[startId].scale,timeDistance);
                        }
                    }
                }
