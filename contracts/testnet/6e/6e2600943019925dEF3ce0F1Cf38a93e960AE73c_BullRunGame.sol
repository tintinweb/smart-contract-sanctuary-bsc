...
contract BullRunGame is IERC721Receiver, Ownable, ReentrancyGuard {

    using EnumerableSet for EnumerableSet.UintSet;

    address public MetatopiaCoinFlipRNGContract; // VRF contract to generate coin flip result
    address public RandomizerContract; // VRF contract to decide nft stealing
    address public BullpenContract; // stores staked Bulls
    address public TopiaTokenContract; // the TOPIA token
    address public ArenaContract; // stores staked Matadors
    address public Genesis; // Genesis NFT contract
    address public Alpha; // Alpha NFT contract

    IMetatopiaCoinFlipRNG public MetatopiaCoinFlipRNGInterface;
    ITopia public TopiaInterface;
    IBullpen public BullpenInterface;
    IArena public ArenaInterface;
    IRandomizer public RandomizerInterface;
    ...
    mapping(address => bool) public ContractWhitelist; // external contracts that are approved to interact with this 
    ...
    // keeps track of stolen NFT's
    uint16 public stolenRunners;
    uint16 public stolenBulls;
    ...
    // @param: _minStakeTime should be # of SECONDS (ex: if minStakeTime is 1 day, pass 1800)
    // @param: _runner/bull/alphaMult = number of topia per day
    // TOPIA: 0xF44E1E634FAF243684410a5f9080024135dbc94a
    // COINFLIP: 0xbDc24108825128F89F2fE04a75dbcEbD13f7eAEd
    // Randomizer: 0x00725cbf4fc5c8ed006747f4abdad11d0243e018
    // BULLPEN: 
    // ARENA: 
    // GENESIS: 0x881935AC212d66C6C35317A93d39840bAbF67CF3
    // ALPHA: 0xf52a592D72EbC6CF1E8d8F15c8FbfC436834d1bE
    // 0
    ...
    constructor(
        ...
        ContractWhitelist[_topiaToken] = true;
        ContractWhitelist[_coinFlipContract] = true;
        ContractWhitelist[_bullpen] = true;
        ContractWhitelist[_arena] = true;
        ContractWhitelist[_randomizer] = true;
        ContractWhitelist[_genesis] = true;
        ContractWhitelist[_alpha] = true;
    ...
    modifier requireContractsSet {
        require(
            MetatopiaCoinFlipRNGContract != address(0) &&
            TopiaTokenContract != address(0) &&
            ArenaContract != address(0) &&
            BullpenContract != address(0) &&
            RandomizerContract != address(0) , 
            "All contracts must be set!");
        _;
    }

    modifier allowedContractsOnly() {
        require(msg.sender == tx.origin, "Proxy contract not allowed");
        if(_isContract(msg.sender)) {
            require(ContractWhitelist[msg.sender], 
            "Calling contract is not approved to interact with this game");
        }
        _;
    }
    ...
    // internal fxn to claim daily topia rewards for staking. Matadors do not get daily topia.
    function claimDailyTopia(uint16[] calldata _idNumbers, bool _isUnstaking) public requireContractsSet allowedContractsOnly() {
        
        uint256 length = uint256(_idNumbers.length);
        uint256 owed;

        for (uint16 i = 0; i < length; i++) {
            require(StakedNFTInfo[_idNumbers[i]].owner == msg.sender , "NFTs must belong to caller!");

            if (StakedNFTInfo[_idNumbers[i]].typeOfNFT == 1 && (block.timestamp - StakedNFTInfo[_idNumbers[i]].stakeTimestamp) > 1800) { 
                // if NFT is runner and has been staked for > 1 day
                owed += (runnerRewardMult * ((block.timestamp - StakedNFTInfo[_idNumbers[i]].value) / 1800));
                StakedNFTInfo[_idNumbers[i]].value = uint80(block.timestamp); // reset the stakeTime for this NFT

            } else if (StakedNFTInfo[_idNumbers[i]].typeOfNFT == 2 && (block.timestamp - StakedNFTInfo[_idNumbers[i]].stakeTimestamp) > 1800) { 
                // if NFT is bull...
                owed += (bullRewardMult * ((block.timestamp - StakedNFTInfo[_idNumbers[i]].value) / 1800));
                StakedNFTInfo[_idNumbers[i]].value = uint80(block.timestamp); // reset the stakeTime for this NFT 

            } else if (StakedNFTInfo[_idNumbers[i]].typeOfNFT == 3 && (block.timestamp - StakedNFTInfo[_idNumbers[i]].stakeTimestamp) > 1800) {
                // if NFT is matador
                owed += (TOPIAPerMatador - StakedNFTInfo[_idNumbers[i]].value);
                StakedNFTInfo[_idNumbers[i]].value = TOPIAPerMatador;
        ...
        if (_isUnstaking) {
            ...
            uint256 numWords = _idNumbers.length;
            ...
            for (uint i = 0; i < _idNumbers.length; i++) {
                if (NFTType[_idNumbers[i]] == 1) {
                    unstakeRunner(_idNumbers[i], seed[i]);
                } else if (NFTType[_idNumbers[i]] == 2) {
                    unstakeBull(_idNumbers[i], seed[i]);
                } else if (NFTType[_idNumbers[i]] == 3) {
                    unstakeMatador(_idNumbers[i]);
                } else if (NFTType[_idNumbers[i]] == 0) {
                    continue;
                }
            }
    ...
    function claimManyAlphas(uint16[] calldata _tokenIds, bool unstake) external nonReentrant requireContractsSet allowedContractsOnly() {
        uint256 owed = 0;
        for (uint i = 0; i < _tokenIds.length; i++) { 
            require(StakedAlphaInfo[_tokenIds[i]].owner == msg.sender, "not owner");
            owed += (block.timestamp - StakedAlphaInfo[_tokenIds[i]].value) * alphaRewardMult / 1800;
            if (unstake) {
                delete StakedAlphaInfo[_tokenIds[i]];
                stakedAlphas -= 1;
                StakedAlphasOfWallet[msg.sender].remove(_tokenIds[i]);
                IERC721(Alpha).safeTransferFrom(address(this), msg.sender, _tokenIds[i]);
    ...
    function stakeMany(uint16[] calldata _tokenIds) external nonReentrant requireContractsSet allowedContractsOnly() {
        require(msg.sender == tx.origin, "account to send mismatch");
        for (uint i = 0; i < _tokenIds.length; i++) {
            require(IERC721(Genesis).ownerOf(_tokenIds[i]) == msg.sender, "You don't own this token");
    ...
    function stakeRunner(uint16 tokenID) internal {
        ...
        IERC721(Genesis).safeTransferFrom(msg.sender, address(this), tokenID);
    ...
    function stakeBull(uint16 tokenID) internal {
        ...
        IERC721(Genesis).safeTransferFrom(msg.sender, BullpenContract, tokenID); // bulls go to the pen
    ...
    function stakeMatador(uint16 tokenID) internal {
        ...
        IERC721(Genesis).safeTransferFrom(msg.sender, ArenaContract, tokenID); // matadors go to the arena
    ...
    function unstakeRunner(uint16 tokenID, uint256 seed) internal {
        require(StakedNFTInfo[tokenID].owner == msg.sender , 
        "not owner");
        require(block.timestamp - StakedNFTInfo[tokenID].stakeTimestamp > minimumStakeTime , 
        "Must wait minimum stake time");
        ...
        if (BullpenInterface.bullCount() > 0 && (seed % 100) < Risk[1]) { 
            ...
            IERC721(Genesis).safeTransferFrom(address(this), thief, tokenID);
            emit RunnerStolen(msg.sender, thief);
        } else {
            IERC721(Genesis).safeTransferFrom(address(this), msg.sender, tokenID);
    ...
    function unstakeBull(uint16 tokenID, uint256 seed) internal {
    ...
    function unstakeMatador(uint16 tokenID) internal {
    ...
    function stakeManyAlphas(uint16[] calldata _tokenIds) external nonReentrant {
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            require(IERC721(Alpha).ownerOf(_tokenIds[i]) == msg.sender, "not owner");
            ...
            IERC721(Alpha).safeTransferFrom(msg.sender, address(this), _tokenIds[i]);
    ...
    function getUnclaimedAlpha(uint16 tokenId) requireContractsSet external view returns (uint256) {
        return (block.timestamp - StakedAlphaInfo[tokenId].value) * alphaRewardMult / 1800;
    }

    function getUnclaimedGenesis(uint16 tokenId) requireContractsSet external view returns (uint256 owed) {
        if (NFTType[tokenId] == 1) {
            owed = (runnerRewardMult * ((block.timestamp - StakedNFTInfo[tokenId].value) / 1800));
        } else if (NFTType[tokenId] == 2) {
            owed = (bullRewardMult * ((block.timestamp - StakedNFTInfo[tokenId].value) / 1800));
    ...
    function getUnclaimedTopiaForUser(address _account) requireContractsSet external view returns (uint256) {
        uint256 owed;
        ...
        for (uint i = 0; i < alphaLength; i++) {
            ...
            owed += (block.timestamp - StakedAlphaInfo[tokenId].value) * alphaRewardMult / 1800;
        }
    ...
    function startEncierro(
        ...
        require(
            (currentEncierroId == 0) || 
            (Encierros[currentEncierroId].status == Status.Claimable) ,
            "prev must be claimable"
        );

        require(
            ((_endTime - block.timestamp) >= minDuration) && ((_endTime - block.timestamp) <= maxDuration),
            "length outside of range"
        );
   ...
   */
    ...
    function flipCoinAndMakeClaimable(uint256 _encierroId) external nonReentrant requireContractsSet allowedContractsOnly() returns (uint256) {
