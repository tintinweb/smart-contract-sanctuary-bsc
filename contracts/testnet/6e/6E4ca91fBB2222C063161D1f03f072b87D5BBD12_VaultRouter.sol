...
contract VaultRouter is Ownable, ReentrancyGuard
{
    ...
    function Deposit(uint256 amount, uint256 poolId, uint256 time, uint256 blockNo) public onlyRegisteredUsers nonReentrant
    {
        UpdateShareVal(poolId);
        IShareToken shareToken = IShareToken(ShareTokens[poolId]);
        uint256 shares = shareToken.AmountToShare(amount);
        if(Fairlaunch != address(0)){
            IFairLaunch(Fairlaunch).WorkerDeposit_Test(shares, poolId, _msgSender(),time, blockNo);
        }
        address baseToken = IAlpacaWorker(Workers[poolId]).BaseToken();
        TransferHelper.safeTransferFrom(baseToken, _msgSender(), Workers[poolId], amount);
        shareToken.mint(_msgSender(),shares);
    ...
    function WithDraw_Test(uint256 share, uint256 poolId, uint256 time, uint256 blockNo) public onlyRegisteredUsers nonReentrant
    {
        UpdateShareVal(poolId);
        IShareToken shareToken = IShareToken(ShareTokens[poolId]);
        if(Fairlaunch != address(0))
        {
            IFairLaunch(Fairlaunch).WorkerWithdraw_Test(share, poolId, _msgSender(),time, blockNo);
        }
        address workerAddress = Workers[poolId];
        address baseToken = IAlpacaWorker(workerAddress).BaseToken();
        uint256 amount = shareToken.ShareToAmount(share);
