...
// File: RunTogetherPoolReward.sol
...
contract RunTogetherPoolReward is Ownable, ReentrancyGuard {
    ...
    constructor() {
        token = IERC20(0xc643E83587818202E0fFf5eD96D10Abbc8Bb48e7);

        beneficiary = 0x1e1D9C46A991388E6eD4819078Cf83AE02b86359;
        // 0 months
        cliff = 0;
        ...
        // 0 month
        slicePeriod = 0 * 30 * 24 * 3600;
        // 0
        amountTotal = 0 ether;
        // 40000000
        tgeAmount = 40000000 ether;
    ...
    function releaseWithAmount(uint256 amount) external nonReentrant {
        ...
        uint256 releasableAmount = amount;
        if (releasableAmount > token.balanceOf(address(this))) {
            releasableAmount = token.balanceOf(address(this));
        }

        released = released.add(releasableAmount);
        numberOfVest = numberOfVest.add(1);
