...
/////////////////////////////////////////////////////////////////////
  function setTokenA_address(address x) public {
      require(msg.sender == owner, "Only Novuxa Contract can perform this action.");
       TokenA = x;
   }
...
/////////////////////////////////////////////////////////////////////
  function setTokenB_address(address x) public {
      require(msg.sender == owner, "Only Novuxa Contract can perform this action.");
       TokenB = x;
   }
   ...
   function  novuxa_Balance_Token1() external view returns(uint) {
    ...
    function  nouvaxa_price() external view returns(uint) {
      ...
      //return (token.balanceOf(address(this))/tkdA) / (token_.balanceOf(address(this))/tkdB);
       return token.balanceOf(address(this)) / token_.balanceOf(reserve_address);
    }
...
function SwapAB(uint amountA) public swap_checkA(amountA) {
        ...
        uint256 price_quotient = (token.balanceOf(address(this))/tkdA)*100000000 / (token_.balanceOf(reserve_address)/tkdB);
        require(token.balanceOf(msg.sender) >= (amountq/tkdD * price_quotient), "Not Enough USDT Balance For this SWAP");
        token.transferFrom(msg.sender, address(this), ((amountq/tkdD)*price_quotient)/100000000);            
        require(token_.balanceOf(address(this)) >= amountA*tkdA, "Not Enough Liquidity For this SWAP");
        token_.transfer(msg.sender, amountA*tkdA);
      ...
      function SwapBA(uint amountB) public swap_checkB(amountB) {
        require(token_.balanceOf(msg.sender) >= amountB*tkdB, "Not Enough Deelian Balance For this SWAP");
          // token_.transferFrom(msg.sender, address(this), amountB*tkdB);
            token_.transferFrom(msg.sender, reserve_address, amountB*tkdB);
           ...
           token.transfer(msg.sender, ((amountq*tkdD)*price_quotient)/100000000);
           emit SWAPBAa(now, msg.sender, address(this), amountB*tkdB, token.balanceOf(address(this))/tkdA, token_.balanceOf(reserve_address)/tkdB);
           emit SWAPBAb(now, address(this), msg.sender,  amountq*tkdD, token.balanceOf(address(this))/tkdA, token_.balanceOf(reserve_address)/tkdB);
 ...
 function AProvide_Liquidity_Novuxa(uint256 value) swap_checkA(value) external  {
   ...
   emit ALiquidityProvider(now,  msg.sender, address(this), valueq,  A_ee_deelian[msg.sender]);
    ...
    function Remove_A_Liquidity_Novuxa(uint256 value) external  {
       ...
       emit RemoveALiquidityProvider(now,  address(this), msg.sender,  valueq, (token.balanceOf(address(this))/token_.balanceOf(reserve_address)));
    }
       ...
       function Remove_B_Liquidity_Novuxa(uint256 value) external only_novuxa {
            ...
            emit RemoveBLiquidityProvider(now,  address(this),  msg.sender, valueq, (token.balanceOf(address(this))/token_.balanceOf(reserve_address)));
