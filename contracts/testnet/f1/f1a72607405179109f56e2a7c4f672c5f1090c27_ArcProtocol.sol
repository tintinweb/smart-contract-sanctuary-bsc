...
contract ArcProtocol is Initializable {
  ...
  uint256 public lastPayAllFeesTimestamp;
  ...
  function initialize(address arcTokenAddress, address _admin) public {
    ...
    lastPayAllFeesTimestamp = block.timestamp;
    initDone = true;
  }
  ...
  function _safeSender() private view {
  ...
  function upgradeNode(uint256 nodeId, uint256 nodeTypeId) public {
    ...
    (uint256 availableRewards, uint256 claimFees, uint256 maintenanceFees) = calculateRewardsAndFees(node.id);
  ...
  /**
   * Pay all maintenance fees for current period
   */
  function payAllCurrentMaintenancefees() public payable {
    _safeSender();
    Node[] storage nodes = usersNodes[msg.sender];
    ...
    (uint256 availableRewards, uint256 claimFees, uint256 maintenanceFees) = calculateAllRewardsAndFees();
    ...
    for (uint256 i = 0; i < nodesCount; i++) {
      if (block.timestamp > nodes[i].maintenanceFeesDeadlineTimestamp) {
        // Need to pay the previous and current month
        nodes[i].lastMaintenanceFeesTimestamp = nodes[i].lastMaintenanceFeesTimestamp + 60 days;
        nodes[i].maintenanceFeesDeadlineTimestamp = nodes[i].maintenanceFeesDeadlineTimestamp + 60 days;
      } else {
        // Pay the current month
        nodes[i].lastMaintenanceFeesTimestamp = nodes[i].lastMaintenanceFeesTimestamp + 30 days;
        nodes[i].maintenanceFeesDeadlineTimestamp = nodes[i].maintenanceFeesDeadlineTimestamp + 30 days;
      }
    }

    lastPayAllFeesTimestamp =  lastPayAllFeesTimestamp + 30 days;
    // send payment to treasury wallet
    payable(treasuryPool).transfer(msg.value);
  }
   ...
   */
  function calculateAllRewardsAndFees() public view returns (uint256, uint256, uint256) {
    _safeSender();
    Node[] storage nodes = usersNodes[msg.sender];
    uint256 nodesCount = nodes.length;
    require(nodesCount > 0, "No nodes");
    ...
    for (i = 0; i < nodesCount; i++) {
      ...
      {
        (uint256 rewards, uint256 claimFees, uint256 maintenanceFees) = calculateRewardsAndFees(nodes[i].id);
        allClaimFees += claimFees;
        allRewards += rewards;
        allMaintenanceFees += maintenanceFees;
      }
    }
    // average of all the claim taxes
    allClaimFees = allClaimFees / i;
    return (allRewards, allClaimFees, allMaintenanceFees);
  }
  ...
  function claimRewards(uint256 nodeId) public {
    ...
    (uint256 availableRewards, uint256 claimFees, uint256 maintenanceFees) = calculateRewardsAndFees(nodeId);
