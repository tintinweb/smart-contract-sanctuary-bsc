...
contract LFWCore is OrderBook, FeeManager, ReentrancyGuard {
    using SafeERC20 for IERC20;

    address public immutable LFW;
    ...
    function canExecuteOrder(
        ...
        address[] memory _pathToLFW
    ) external view returns (bool success, string memory reason) {
        return _canExecuteOrder(_orderId, _pathToTokenOut, _pathToLFW);
    }
    ...
    function _canExecuteOrder(
        ...
        if (order.tokenIn != LFW && _pathToLFW.length > 1) {
            if (
                _pathToLFW[0] != order.tokenIn ||
                _pathToLFW[_pathToLFW.length - 1] != LFW
            ) return (false, "Invalid path to LFW token");
        }
    ...
    function executeOrder(
        ...
        _executeOrder(_orderId, _pathToTokenOut, _pathToLFW);
    }
    ...
    function _executeOrder(
        ...
        _transferFees(fee, executorFee, order.tokenIn, _pathToLFW);
    ...
    function _transferFees(
        ...
        if (_executorFee > 0) {
            if (_tokenIn == LFW || _pathToLFW.length <= 1) {
                IERC20(_tokenIn).safeTransfer(msg.sender, _executorFee);
            } else {
                _swap(
                    _pathToLFW,
                    _executorFee,
                    getAmountOutMin(_executorFee, _pathToLFW),
                    msg.sender
                );
            }
        }
