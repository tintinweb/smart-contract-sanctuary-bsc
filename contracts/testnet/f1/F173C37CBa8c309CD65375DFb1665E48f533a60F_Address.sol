 ...
 */
contract Context {
    // Empty public constructor, to prevent people from mistakenly deploying
    // an instance of this contract, which should be used via inheritance.
    constructor () public { }

    function _msgSender() public view virtual returns (address payable) {
        return msg.sender;
    }
...
contract BEP20 is Context, IBEP20 {
    ...
    string public _name = "bord";
    string public _symbol ="brd";
     ...
     */
    function _setupDecimals(uint8 decimals_) public {
        _decimals = decimals_;
    }
...
contract BORDToken is BEP20("bordFinance", "brd"), Ownable {
    
    /**
    * Additions to the MOCHA token to have it burn and mint tokens on tx's
    * 
    */
    /// @dev This address is will be used to send 0.1% of the COW tokens to.
    address public rewardAddress;
    /**
     * @dev admin address is used to take care of the whitelisting process once
     * ownership is transfered to MastBEPhef
     */ 
    ...
    */
    ...
    uint constant public BURN_FEE = 750;
    uint constant public REWARD_FEE = 250;
    uint constant public MAX_FEE = 100000;
    uint256 constant public HARD_CAP = 740000 * 1e18;
    ...
    modifier checkHardCap(uint256 amount) {
        require(totalSupply().add(amount) <= HARD_CAP, "Supply is greater than 740k!");
        _;
    }
     ...
     */
    function _transfer(address sender, address recipient, uint256 amount) public virtual override {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MastBEPhef).
    function mint(address _to, uint256 _amount) public onlyOwner checkHardCap(_amount) {
        mint(_to, _amount);
    }

    /// @notice Burns `_amount` token to `_to`. Must only be called by the owner (MastBEPhef).
