            ...
            for (UC i = ZERO; i < uc(count); i = i + ONE) {
                ...
                if (ai.actionType == ActionType.LIMIT || ai.actionType == ActionType.CANCEL_LIMIT || ai.actionType == ActionType.SYSTEM_CANCEL) {
                    ...
                    datas[i.into()] = OrderAndTradeHistory(
                        ...
                        oi.qty, oi.entryPrice, ti.margin, ti.openFee, ti.openExecutionFee, 0, 0, 0, 0
                    );
                } else {
                    TradeInfo memory ti = hs.tradeInfos[ai.hash];
                    ...
                    datas[i.into()] = OrderAndTradeHistory(
                        ai.hash, ai.timestamp, name, ai.actionType, oi.tokenIn, oi.isLong, oi.amountIn,
                        oi.qty, oi.entryPrice, ti.margin, ti.openFee, ti.openExecutionFee,
                        ci.closePrice, ci.fundingFee, ci.closeFee, ci.closeExecutionFee
                    );
                }
            }
        }
...
library LibOrderAndTradeHistory {
    ...
    // todo: 后面改回 apollox.order.trade.history.storage
    bytes32 constant ORDER_TRADE_HISTORY_POSITION = keccak256("apollox.order.trade.history.storage.20230306");
