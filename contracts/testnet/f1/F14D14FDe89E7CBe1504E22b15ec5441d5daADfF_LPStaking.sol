...
contract LPStaking is Ownable {
  ...
  function setLPToken(address _lpToken) external onlyOwner {
    require(!LPisSet, "LP token already set");
  ...
  function unstake() external {
    ...
    require(lpToken.transfer(msg.sender, user.amount));
  ...
  function increaseRewardPool(uint256 blocksAmount, uint256 amount)
    ...
    onlyOwner
  {
    require(LPisSet, "LP token is not set");
  ...
  function calcReward(uint256 _farmedLPAmount)
    public
    view
    returns (uint256 token0)
  {
    (uint256 reserve0, , ) = lpToken.getReserves();
    token0 = (2 * _farmedLPAmount * reserve0) / lpToken.totalSupply();
    return token0;
  }
