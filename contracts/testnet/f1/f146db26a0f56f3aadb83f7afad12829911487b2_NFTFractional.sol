...
contract NFTToken is Ownable, ERC721URIStorage {
    ...
    event MintNFT(address indexed _receiver, uint256 _tokenId, string _symbol, string _name, bytes _data);
    ...
    */
    ...
    function mintNFT (
        ...
        emit MintNFT(_receiver,_tokenId, name(), symbol(), _data);
        return _tokenId;
    }
...
contract NFTFractional is Upgradable {
    ...
    */
    ...
    function setNFTToken(string memory _name, string memory _symbol) external onlyController {
        nftToken = address(new NFTToken(_name, _symbol));
    }

    function setBaseUrl(string memory _baseUrl) external onlyController {
       NFTToken(nftToken).setBaseURI(_baseUrl);
    }
