...
contract HeroNFT is BaseNFT {
  ...
  function mintHeroesForTest(address[] calldata _owners, uint16 _classId, uint _price, uint8 _level, uint _quantity) onlyMintAdmin external {
    for(uint i = 0; i < _owners.length; i++) {
      for(uint j = 0; j < _quantity; j++) {
        _mintForTest(_owners[i], _classId, _price, j, _level);
      }
    }
  }
  ...
  function _mintForTest(address _owner, uint16 _classId, uint _price, uint _index, uint8 _level) private returns (uint) {
    require(_classId >= 1 && _classId <= countId, 'NFT: Invalid class');
    uint newId = _genNewId(_index);
    _mint(_owner, newId);
    heroes[newId].id = _classId;
