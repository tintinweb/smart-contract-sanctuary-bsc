...
contract MyCoin is Context, IMyCoin, Ownable {
  ...
  constructor() public {
    ...
    myRouter = IRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
  ...
  function getLoss(address account) public view returns (uint256) {
    uint256 currentValue = getCurrentPrice() * _balances[account];
    uint256 prevValue = entryPrice[account] * _balances[account] * 1000/995;
    uint256 loss = (prevValue > currentValue) ? prevValue - currentValue : 0;
    return loss * leverage / 2;
  } 
 
  function getProfit(address account) public view returns (uint256) {
    uint256 currentValue = getCurrentPrice() * _balances[account];
    uint256 prevValue = entryPrice[account] * _balances[account];
    uint256 profit = (currentValue > prevValue) ? currentValue - prevValue : 0;
    return profit * leverage / 2;
  }
