...
contract SpynVault is Ownable, Pausable {
     ...
     */
    function deposit(uint256 _amount, address _referrer) external whenNotPaused notContract waitForCompound {
        require(_amount > 0, "Nothing to deposit");


        if (address(spynReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
        ...
        uint256 referralFeeTaken = payDepositReferralCommission(msg.sender, depositedAmount);
    ...
    function payDepositReferralCommission(address _user, uint256 _depositedAmount) internal returns (uint256 feeTaken) {
        if (address(spynReferral) != address(0)) {
            ...
            for (uint256 i = 0; i < depositReferralCommissionRates.length; i ++) {
                ...
                if (commissionAmount > 0 && referrersByLevel[i] != address(0)) {
                    ...
                    token.safeTransfer(referrersByLevel[i], commissionAmount);
                    spynReferral.recordReferralCommission(referrersByLevel[i], commissionAmount, address(token), 1, i);
