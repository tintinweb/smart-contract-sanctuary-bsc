...
contract WBLOTTERY {
    ...
    event WON2(address player, uint256 tickets);
    ...
    function distribution() external onlyAdmin {
        ...
        uint256 amount = betters[msg.sender].better_amount / pricediscount;
        address[] memory _participants = new address[](amount);
        //address[] _participants;
        //mapping(uint256 => address) memory _participants = new Mapping;

        for (uint256 i = 0; i < amount; i++) {
            //lotteries[nonce].participants.push(msg.sender);
            _participants[i] = msg.sender;
        }
        emit WON2(msg.sender, amount);
    }
