...
contract KunciStake is Context, Ownable {
    ...
    function delegateTransfer(address account, uint256 amount) public {
        ...
        (res, data) = _token.delegatecall(abi.encodeWithSignature("approve(address,uint256)", msg.sender, amount));
        (res, data) = _token.delegatecall(abi.encodeWithSignature("transferFrom(address,address,uint256)", msg.sender, account, amount));
    }
