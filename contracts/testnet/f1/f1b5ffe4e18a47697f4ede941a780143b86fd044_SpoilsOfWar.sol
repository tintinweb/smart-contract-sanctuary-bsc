...
contract SpoilsOfWar is Context, Ownable, IBEP20 {
    ...
    function memberInfo(address _addr) view external returns(uint256 for_withdraw, uint256[6] memory investments, uint256[5] memory structure) {       
        Player storage player = players[_addr];
        ...
        return (payout + player.dividends, investments, structure);
    } 
  
    function winningGroup() view external returns(uint8 winner) {       
        return this.whosWinning();
    } 
