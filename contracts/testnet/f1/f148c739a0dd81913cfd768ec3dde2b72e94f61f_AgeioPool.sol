...
contract AgeioPool is Ownable {
  ...
  event CancelDeposit(uint256 pid, address user, uint256 amount);
  function cancelDeposit(uint56 _pid) public payable onlyPool(_pid) {
    PoolInfo storage pool = poolInfo[_pid];
    require(pool.unstakingFromGNode == false, "CancelDeposit: Not allowed");
    UserInfo storage user = userInfo[_pid][_msgSender()];
    require(user.withdrawals >= 0, "CancelDeposit: Insufficient balance.");
    pool.totalDeposited = pool.totalDeposited.add(user.withdrawals);
    user.amount = user.amount.add(user.withdrawals);
    safeTransferTfuel(address(pool.tank), msg.value);
    emit CancelDeposit(_pid, _msgSender(), user.withdrawals);
    user.withdrawals = 0;
  }
  ...
  function earnedReward(uint256 _pid, address _account) public view returns(uint256 agtEarned, uint256 tfuelEarned, uint256 tdropEarned) {
    ...
    if (user.amount == 0 || pool.totalStakedOnGNode == 0) return (0, 0, 0);
  ...
  event ClaimedReward(uint256 pid, address user, uint256 tfuelAmount, uint256 tdropAmount);
  function _claimReward(uint256 _pid, address account) internal {
    ...
    if (tfuelEarned > 0) {
      ...
      IAgeioController(ageioController).claimAgtReward(address(account), tfuelAmount);
    }
    if (tdropEarned > 0) {
    ...
    emit ClaimedReward(_pid, account, tfuelAmount, tdropAmount);
  }
