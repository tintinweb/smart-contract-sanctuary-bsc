...
/******************************************************************************************************************************
 ...
 +-+-+-+-+ +-+-+-+-+ +-+-+-+
 |B|A|B|Y| |E|L|M|O| |I|N|U|
 +-+-+-+-+ +-+-+-+-+ +-+-+-+
                                                                                                                                                                                                                          
Earn Elmo Inu by just holding Baby Elmo Inu!   

Website:  
- https://babyelmoinu.com/  

Social Media Links:  
- https://discord.gg/Elmo  
- https://twitter.com/BabyElmoInu  
- https://t.me/BabyElmoInuBSC  

For questions/concerns/inquiries, feel free to pop into the telegram or email us at: [email protected]  

Buy/Sell Tax
- 6/6% Elmo Inu reflections
- 2/2% Manual repurchase and burn  
- 2/2% Marketing
...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20Extended public ELMOINU =
        IERC20Extended(0x8516Fc284AEEaa0374E66037BD2309349FF728eA);
    ...
    uint256 public minDistribution = 1 * (10**ELMOINU.decimals());
    ...
    function deposit() external payable override onlyToken {
        uint256 balanceBefore = ELMOINU.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(ELMOINU);

        router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: msg.value
        }(0, path, address(this), block.timestamp);

        uint256 amount = ELMOINU.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if (amount > 0) {
            ...
            ELMOINU.transfer(shareholder, amount);
...
// main contract
contract BabyElmoInu is IERC20Extended, Auth {
    using SafeMath for uint256;

    string private constant _name = "Baby Elmo Inu";
    string private constant _symbol = "bbyELMO";
    uint8 private constant _decimals = 9;
    uint256 private constant _totalSupply = 1_000_000_000_000 * 10**_decimals;

    address public ELMOINU = 0x104F8A90EEB24a475BC3D1ADa264479593aAB8b9;
    ...
    constructor(address _market, address _dev) Auth(msg.sender) {
    ...
    function swapBack() internal swapping {
        uint256 totalFee = (_reflectionFeeCount).add(_marketingFeeCount).add(
            _devFeeCount
        );
    ...
    function setBuyFees(
        ...
        _devBuyFee = _devFee;
        totalBuyFee = (_reflectionFee).add(_marketingFee).add(_devFee);
    ...
    function setSellFees(
        ...
        _devSellFee = _devFee;
        totalSellFee = (_reflectionFee).add(_marketingFee).add(_devFee);
        feeDenominator = _feeDenominator;
        require(
