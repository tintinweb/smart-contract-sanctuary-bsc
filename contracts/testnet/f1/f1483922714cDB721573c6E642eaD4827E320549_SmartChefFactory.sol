...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when Afripay mining ends.
    uint256 public bonusEndBlock;

    // The block number when Afripay mining starts.
    ...
    // Afripay tokens created per block.
     ...
     */
    function deposit(uint256 _amount,address _ref) external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        userDetails storage _user = users[msg.sender];

        if (_user.referer == address(0)){
            require(msg.sender != _ref,"Referer and user not to be same");
            _user.referer = _ref;
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (pending > 0) {
            commission = pending.mul(refCommission).div(100);
            users[users[msg.sender].referer].earnings = users[users[msg.sender].referer].earnings.add(commission);
            rewardToken.safeTransfer(users[msg.sender].referer,commission);
            rewardToken.safeTransfer(address(msg.sender), pending.sub(commission));
        }
    ...
    function viewReferals(address _user)public view returns(address[] memory) {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 AfripayReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(AfripayReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 AfripayReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(AfripayReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
