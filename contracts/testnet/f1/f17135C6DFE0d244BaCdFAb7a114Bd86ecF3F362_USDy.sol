 ...
 */
contract USDy is ERC20,AccessControl, WhiteList {
    ...
    address public APE_SWAP_ROUTER_ADDRESS; //Apeswap router address
    IAdmin public Admin;
    ...
    **/
    ...
    Note allocation point should be equal to tax
        ...
        only admin can call 
     */
    function setUSDyAndAllocationTax(uint tax, uint[] memory allocationTax) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    */
    ...
    function _transfer(
        ...
        require(
            ...
            "USDy: you cann't interact in same block"
        );
        ...
        if (isContract(sender) || isContract(recipient)) {
            require(
                getProtocolAddresses(sender) ||
                    getProtocolAddresses(recipient),
                "USDy: No external contract intract"
            );
        }
        else {
            if(!getUserAddresses(sender) && !getUserAddresses(recipient)){
            require(transactionTimeLimit[sender] + lockTransactionTime <= block.timestamp,"USDy: transactionTimeLimit is greater than current time");
                ...
                // if(IUniswapV2Router02(APE_SWAP_ROUTER_ADDRESS).getAmountsOut(balanceOf(address(this)),path)[2] >= YSL_ThresholdLimit){
                    IERC20(address(this)).approve(APE_SWAP_ROUTER_ADDRESS, taxAmount); //BSC Testnet pancake router address
                    uint convertedBUSD = IUniswapV2Router02(APE_SWAP_ROUTER_ADDRESS).swapExactTokensForTokens( 
                        taxAmount,
                        0,
                        path,
                        address(this),
                    ...
                    IERC20(Admin.BUSD()).transfer(Admin.Treasury(), (convertedBUSD * USDy_Tax_Allocation[0])/USDy_Tax);
                    IERC20(Admin.BUSD()).transfer(TemporaryReferral, ((convertedBUSD * USDy_Tax_Allocation[1])/USDy_Tax));
                // }
                amount -= taxAmount;
                restrictTransfer[sender] = block.number;
                ...
                uint prev = IUniswapV2Router02(APE_SWAP_ROUTER_ADDRESS).getAmountsOut(amount,path)[2];
                super._transfer(sender, recipient, amount);
                uint current = IUniswapV2Router02(APE_SWAP_ROUTER_ADDRESS).getAmountsOut(amount,path)[2];
