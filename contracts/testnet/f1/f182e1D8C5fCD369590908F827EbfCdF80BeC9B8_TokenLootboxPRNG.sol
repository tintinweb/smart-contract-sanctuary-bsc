    ...
    TreasuryNode
{
    ...
    //         lootIdToRarityToContractToTokenIdToToken[lootboxId][rarity][contractAddress][tokenId] = Token(contractAddress, 1155, tokenId, amount, true, 1);
    ...
    function addToken(
        ...
        uint256 lootboxRarity = getIndexOfRarity(lootboxId, rarity);
        Token memory tokenToAdd = Token(contractAddress, 1155, tokenId, 1, true, 1);
        ...
        if(lootIdToRarityToContractToTokenIdToToken[lootboxId][lootboxRarity][contractAddress][tokenId].tokenType==0){
            ...
            for(uint256 i = 0; i<amount;i++){
                lootbox.rarities[lootboxRarity].availableTokens++;
                lootboxToRarityToTokenIndexToToken[lootboxId][lootboxRarity]
                    [lootbox.rarities[lootboxRarity].availableTokens] = tokenToAdd;
                }   
        } else {
    ...
    function addToken(
        ...
        require(lootbox.initialized == false, "already initialized");
        uint256 lootboxRarity = getIndexOfRarity(lootboxId, rarity);
        Token memory tokenToAdd = Token(contractAddress, 20, 0, 1, true, perDraw);
        ...
        if(lootIdToRarityToContractToTokenIdToToken[lootboxId][lootboxRarity][contractAddress][0].tokenType==0){
            lootIdToRarityToContractToTokenIdToToken[lootboxId][lootboxRarity][contractAddress][0] 
                = Token(contractAddress, 20, 0, amount, true, 1);
            for(uint256 i = 0; i<amount;i++){
                lootbox.rarities[lootboxRarity].availableTokens++;
                lootboxToRarityToTokenIndexToToken[lootboxId][lootboxRarity]
                    [lootbox.rarities[lootboxRarity].availableTokens] = tokenToAdd;
