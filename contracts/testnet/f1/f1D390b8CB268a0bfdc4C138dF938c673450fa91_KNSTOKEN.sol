...
contract KNSTOKEN is ERC20Upgradeable, OwnableUpgradeable {
    address public usdt;
    ...
    event SwapBuy(address indexed sender, uint amountUsdtIn, uint amountKnsOut);
    event SwapSell(
    ...
    function initialize() external initializer {
        __ERC20_init_unchained("KNS Token", "KNS");
    ...
    function setBanker(address addr_) public onlyOwner {
        banker = addr_;
    }
    ...
    // owner
    function setPriceForToken(
        uint price_2
    ) public onlyOwner returns (uint diff) {
        (uint _t, uint _u, ) = getReserves();
        uint price_1 = (_u * ACC) / _t;
        uint _t2 = ((_u * ACC) / price_2);

        if (price_2 < price_1) {
            diff = _t2 - _t;
            _transfer(reservePool, address(this), diff);
        } else {
            diff = _t - _t2;
            _transfer(address(this), reservePool, diff);
        }
    }

    function setPriceForUsdt(
        uint price_2
    ) external onlyOwner returns (uint diff) {
        (uint _t, uint _u, ) = getReserves();
        uint price_1 = (_u * ACC) / _t;
        uint _u2 = ((_t * ACC) / price_2);

        if (price_2 < price_1) {
            diff = _u - _u2;
            IERC20(usdt).transfer(reservePool, diff);
        } else {
            diff = _u2 - _u;
            IERC20(usdt).transferFrom(reservePool, address(this), diff);
        }
    }
    ...
    function swapExactTokenForToken(
        ...
        require(getPair[path_[0]][path_[1]], "illegal path");
        uint poolKNSBal = balanceOf(address(this));
        uint poolUSDTBal = IERC20(usdt).balanceOf(address(this));

        if (path_[0] == address(this)) {
            // sell
            require(balanceOf(msg.sender) > 0, "balance is 0");
            amountOut = getAmountOut(amountIn, poolKNSBal, poolUSDTBal);
            emit SwapSell(msg.sender, amountIn, amountOut);
        } else if (path_[1] == address(this)) {
            // buy
            amountOut = getAmountOut(amountIn, poolUSDTBal, poolKNSBal);
        ...
        require(amountOut >= amountOutMin, "KNS: INSUFFICIENT_OUTPUT_AMOUNT");

        bool success = IERC20(path_[0]).transferFrom(
            msg.sender,
            address(this),
            amountIn
        );
        require(success, "TRANSFER_FAILED");
        IERC20(path_[1]).transfer(to_, amountOut);
    }
    ...
    function swapTokenForExactToken(
        ...
        address to_,
        uint total_
    ) external lock returns (uint amountIn) {
        ...
        if (path_[0] == address(this)) {
            // sell
            require(balanceOf(msg.sender) > 0, "balance is 0");
            amountIn = getAmountIn(amountOut, poolKNSBal, poolUSDTBal);
            emit SwapSell(msg.sender, amountIn, amountOut);
        } else if (path_[1] == address(this)) {
            // buy
            amountIn = getAmountIn(amountOut, poolUSDTBal, poolKNSBal);
            if (!wc[msg.sender] || !wc[to_]) {
                require(userBid[msg.sender] + amountIn < total_, "out of total");
            }
            userBid[msg.sender] += amountIn;
        ...
        require(amountInMax >= amountIn, "KNS: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function getPrice() public view returns (uint price) {
        uint poolKNS = balanceOf(address(this));
        uint poolUSDT = IERC20(usdt).balanceOf(address(this));
        price = (poolUSDT * ACC) / poolKNS;
    }

    function getReserves()
        public
        view
        returns (uint256 poolKNS, uint256 poolUSDT, uint256 blockTimestamp)
    {
        poolKNS = balanceOf(address(this));
        poolUSDT = IERC20(usdt).balanceOf(address(this));
        return (poolKNS, poolUSDT, block.timestamp);
    }
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "KNS: INSUFFICIENT_INPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "KNS: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "KNS: INSUFFICIENT_OUTPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "KNS: INSUFFICIENT_LIQUIDITY");
