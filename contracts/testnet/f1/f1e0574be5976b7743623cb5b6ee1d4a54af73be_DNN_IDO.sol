...
contract DNN_IDO {
   ...
   struct Profit {
    ...
    function deposit(uint256 _amount) external {     
        ...
        require(_amount >= minDeposit && _amount <= maxDeposit, "less than min"); 
        require(_amount < minDeposit, "deposit amount error");
        usdt.transferFrom(msg.sender, address(this), _amount);
        if(user.referrer!=defaultRefer){
        uint256 referrerProfit =   _amount.mul(10).div(100);
        usdt.transferFrom(address(this),user.referrer,referrerProfit);
