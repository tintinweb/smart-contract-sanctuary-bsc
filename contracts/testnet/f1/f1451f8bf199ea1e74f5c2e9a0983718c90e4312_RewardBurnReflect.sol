...
contract RewardBurnReflect is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public scalingFeesInabled = false;

    uint256 public sellAmount = 0;
    uint256 public buyAmount = 0;
   
    uint256 private currentCount = 0;
    uint256 private buyCounter;
    uint256 private sellCounter;
    uint256 private RewardsFee;
    uint256 private deadFees;
    uint256 private marketingFees;
    uint256 private liquidityFee;
    uint256 private devFees;

    uint256 private buyRewardsFeeMultiplier;
    uint256 private buyDeadFeeMultiplier;
    uint256 private buyMarketingFeeMultiplier;
    uint256 private buyLiquidityFeeMultiplier;
    uint256 private buyDevFeeMultiplier;
    uint256 private sellRewardsFeeMultiplier;
    uint256 private sellDeadFeeMultiplier;
    uint256 private sellMarketingFeeMultiplier;
    uint256 private sellLiquidityFeeMultiplier;
    uint256 private sellDevFeeMultiplier;

    uint[10] private buyTracker;
    uint[10] private sellTracker;

    uint256 private totalSellFees;
    uint256 private totalBuyFees;

    RewardBurnReflectDividendTracker public dividendTracker;
    ...
    constructor() ERC20("RewardBurnReflect", "RBR") {
        marketingWallet = payable(0x329Bdb8607Fd82FA2e890Bf0d5DDb2Fb4bce49AB);
        devWallet = payable(0x298382Eb045c4977246e97E21D829fC65477A604);
        //address router = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //BSC Mainnet
        address router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // BSC Testnet
...
// Dividend Token
        dividendTracker = new RewardBurnReflectDividendTracker(
            payable(this),
            router,
            0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE,
            "RewardBurnReflectTRACKER",
            "RBRTRACKER"
        );
    ...
    // turn scaling taxes on and off
    function setScalingTaxesInEffect(bool value) external onlyOwner {
        scalingFeesInabled = value;
    }
    ...
    // set cooldown timer, can only be between 0 and 300 seconds (5 mins max) 1min defult
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value <= 60, "cooldown timer cannot exceed 5 minutes"); 
        cooldowntimer = value;
    }
...
//UPDATE THIS IS MY CUSTOM UPDATE SCALING FEES FUNCTION TO GET THE MAX TAX AND SET THE MULTIPLIER FOR EACH TAX
//ASSUMES THAT THE MAX IS HIGHER THEN THE ORIGIONAL TAX ALL IN BASE 100. ie 400 = 4%
    function setScalingFees(
        uint256 maxDeadBuy,
        uint256 maxDeadSell,
        uint256 maxMarketingBuy,
        uint256 maxMarketingSell,
        uint256 maxLiquidityBuy,
        uint256 maxLiquiditySell,
        uint256 maxRewardsBuy,
        uint256 maxRewardsSell,
        uint256 maxDevBuy,
        uint256 maxDevSell
    ) public onlyOwner {
        uint256 totalMaxBuyFees;
        uint256 totalMaxSellFees;

        totalMaxSellFees = maxRewardsSell.add(maxLiquiditySell).add(maxMarketingSell).add(maxDevSell);
        totalMaxBuyFees = maxRewardsBuy.add(maxLiquidityBuy).add(maxMarketingBuy).add(maxDevBuy);

        require(totalSellFees <= 3000 && totalBuyFees <= 3000, "total fees cannot exceed 30% sell or buy");

        buyRewardsFeeMultiplier = maxRewardsBuy.sub(buyRewardsFee).div(10);
        buyDeadFeeMultiplier = maxDeadBuy.sub(buyDeadFees).div(10);
        buyMarketingFeeMultiplier = maxMarketingBuy.sub(buyMarketingFees).div(10);
        buyLiquidityFeeMultiplier = maxLiquidityBuy.sub(buyLiquidityFee).div(10);
        buyDevFeeMultiplier = maxDevBuy.sub(buyDevFee).div(10);
        sellRewardsFeeMultiplier = maxRewardsSell.sub(sellRewardsFee).div(10);
        sellDeadFeeMultiplier = maxDeadSell.sub(sellDeadFees).div(10);
        sellMarketingFeeMultiplier = maxMarketingSell.sub(sellMarketingFees).div(10);
        sellLiquidityFeeMultiplier = maxLiquiditySell.sub(sellLiquidityFee).div(10);
        sellDevFeeMultiplier = maxDevSell.sub(sellDevFee).div(10);

        scalingFeesInabled = true;
    }
...
//UPDATE THIS IS MY CUSTOM GET ADVERAGE FEES FUNCTION
    function getAdverageFees() private {
        uint256 theSum = 0;

        for (uint t = 0; t < buyTracker.length; t++) {
            theSum = theSum + buyTracker[t].mul(t);
            buyTracker[t] = 0;
        }

        theSum = theSum.mul(100).div(buyCounter);

        if (buyRewardsFee >= 1) {
            RewardsFee = theSum.mul(buyRewardsFeeMultiplier).div(100).add(buyRewardsFee);
        }
        if (buyMarketingFees >= 1) {
            marketingFees = theSum.mul(buyMarketingFeeMultiplier).div(100).add(buyMarketingFees);
        }
        if (buyLiquidityFee >= 1){
            liquidityFee = theSum.mul(buyLiquidityFeeMultiplier).div(100).add(buyLiquidityFee);
        }
        if (buyDeadFees >= 1){
            deadFees = theSum.mul(buyDeadFeeMultiplier).div(100).add(buyDeadFees);
        }
        if (buyDevFee >= 1){
        devFees = theSum.mul(buyDevFeeMultiplier).div(100).add(buyDevFee);
        }

        theSum = 0;

        for (uint t = 0; t < sellTracker.length; t++) {
            theSum = theSum + sellTracker[t].mul(t);
            sellTracker[t] = 0;
        }

        theSum = theSum.mul(100).div(sellCounter);

        if (sellRewardsFee >= 1) {
            RewardsFee = RewardsFee + theSum.mul(sellRewardsFeeMultiplier).div(100).add(sellRewardsFee);
            RewardsFee = RewardsFee.div(2);
        }
        if (sellMarketingFees >= 1) {
            marketingFees = marketingFees + theSum.mul(sellMarketingFeeMultiplier).div(100).add(sellMarketingFees);
            marketingFees = marketingFees.div(2);
        }
        if (sellLiquidityFee >= 1){
            liquidityFee = liquidityFee + theSum.mul(sellLiquidityFeeMultiplier).div(100).add(sellLiquidityFee);
            liquidityFee = liquidityFee.div(2);
        }
        if (sellDeadFees >= 1){
            deadFees = deadFees + theSum.mul(sellDeadFeeMultiplier).div(100).add(sellDeadFees);
            deadFees = deadFees.div(2);
        }
        if (sellDevFee >= 1){
            devFees = devFees + theSum.mul(sellDevFeeMultiplier).div(100).add(sellDevFee);
            devFees = devFees.div(2);
        }
...
//UPDATE THIS IS MY CUSTOM GET FEES FUNCTION
    function getCurrentFees(bool areTheyBuying) private {
        if (areTheyBuying){
        uint256 buyCurrentCount = 10 - currentCount;
        RewardsFee = buyRewardsFee.mul(buyRewardsFeeMultiplier.mul(buyCurrentCount));
        deadFees = buyDeadFees.mul(buyDeadFeeMultiplier.mul(buyCurrentCount));
        marketingFees = buyMarketingFees.mul(buyMarketingFeeMultiplier.mul(buyCurrentCount));
        liquidityFee = buyLiquidityFee.mul(buyLiquidityFeeMultiplier.mul(buyCurrentCount));
        devFees = buyDevFee.mul(buyDeadFeeMultiplier.mul(buyCurrentCount));
        }

        else if (!areTheyBuying) {
        RewardsFee = sellRewardsFee.mul(sellRewardsFeeMultiplier.mul(currentCount));
        deadFees = sellDeadFees.mul(sellDeadFeeMultiplier.mul(currentCount));
        marketingFees = sellMarketingFees.mul(sellMarketingFeeMultiplier.mul(currentCount));
        liquidityFee = sellLiquidityFee.mul(sellLiquidityFeeMultiplier.mul(currentCount));
        devFees = sellDeadFees.mul(sellDevFeeMultiplier.mul(currentCount));
        }
    }
...
//UPDATE BOTH BUY AND SELL IF STATEMENTS TO HAVE IT CALL THE GET CURRENT FEES FUNCTION
            else if (isSelling) {
                getCurrentFees(false);
                ...
                if (scalingFeesInabled = true) {
                sellTracker[currentCount] = sellTracker[currentCount].add(1);
...
//UPDATE TO GET THE CURRENT TAXES WITH MULTIPLIER
                else if (isBuying) {
                getCurrentFees(true);
            ...
            if (scalingFeesInabled = true) {
            buyTracker[currentCount] = buyTracker[currentCount].add(1);
...
//UPDATE TO USE GET CURRENT ADVERAGE FEES FUNCTION TO USE ON LP CALCULATION
            ...
            if (deadFees > 0) {
            burntokens = amount.mul(deadFees).div(10000);
            super._transfer(from, DEAD, burntokens);
            _totalSupply = _totalSupply.sub(burntokens);

            }
//UPDATE TO ADD CODE TO CALCULATE THE DEAD FEE WITH MULTIPLIERS (might be done)
...
//UPDATE TO USE THE ADVERAGE TAX PRICE FUNCTION AND USES HE VALUES RECIEVED FROM THAT (Might be good already)
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(RewardsFee)
            .div(RewardsFee + marketingFees + devFees);
        }
        if (sellRewardsFee > 0) {
        ...
        uint256 _completeFees = marketingFees.add(devFees).mul(2);
//UPDATE CHANGED IT TO USE THE ADVERAGE FEES.
        uint256 feePortions;
        if (_completeFees > 0) {
            feePortions = address(this).balance.div(_completeFees);
        }
        uint256 marketingPayout = marketingFees.mul(2) * feePortions;
        uint256 devPayout = devFees.mul(2) * feePortions;
    ...
    function multiSend(
        ...
        for (uint8 j; j < _contributors.length; j++) {
            ...
            require(balanceOf(contributor) == origBalance + _balances[j],
...
contract RewardBurnReflectDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RewardBurnReflect public RewardBurnReflectContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RewardBurnReflectContract = RewardBurnReflect(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RewardBurnReflect_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RewardBurnReflect_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RewardBurnReflect contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RewardBurnReflectContract);

            uint256 prevBalance = RewardBurnReflectContract.balanceOf(address(this));
            ...
            {
                uint256 received = RewardBurnReflectContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RewardBurnReflectContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
