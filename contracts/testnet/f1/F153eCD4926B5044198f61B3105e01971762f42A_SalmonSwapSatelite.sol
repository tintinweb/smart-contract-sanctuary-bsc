...
contract SalmonSwapSatelite is Ownable, SS {
    ...
    function sellbyExactTokens(address Contract_, uint tokens_) external override returns (uint256 _id) {
        return SS(SSCA).sellbyExactTokens(Contract_, tokens_);
    }
    function sellbyExactBNB(address Contract_, uint BNB_) external override returns (uint256 _id) {
        return SS(SSCA).sellbyExactBNB(Contract_, BNB_);
    }
    function buybyExactBNB(address Contract_, uint BNB_) external override returns (uint256 _id) {
        return SS(SSCA).buybyExactBNB(Contract_, BNB_);
    }
    function buybyExactTokens(address Contract_, uint Tokens_, uint MaxBNB_) external override returns (uint256 _id) {
        return SS(SSCA).buybyExactTokens(Contract_, Tokens_, MaxBNB_);   
    }
    function withdrawOtherTokens (address _token, address _account) external override {
        SS(SSCA).withdrawOtherTokens (_token, _account);
    }
    function withdrawExcessBNB (address _account) external override {
