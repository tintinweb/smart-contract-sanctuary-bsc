...
contract SmartLP is SmartLPStorage, IBEP721, IBEP721Metadata {
    ...
    mapping(uint256 => uint256) public currentAPR;
    ...
    function buySmartLP() payable external {
        ...
        currentAPR[tokenCount] = rewardRate;
    ...
    function getTokenRewardsAmounts(uint tokenId) public view returns (uint lpBnbNimbUserRewards, uint lpBnbGnbuUserRewards, uint lendedUserRewards) {
        ...
        if(currentAPR[tokenId] > 0) curRewardRate = currentAPR[tokenId]; 
        else if (afterLPUpdate[tokenId]) curRewardRate = 45;
        else curRewardRate = 100;

        lpBnbNimbUserRewards = ((_balancesRewardEquivalentBnbNbu[tokenId] + _balancesRewardEquivalentBnbNimb[tokenId]) * ((block.timestamp - weightedStakeDate[tokenId]) * curRewardRate)) / (100 * rewardDuration);
        lpBnbGnbuUserRewards = ((_balancesRewardEquivalentBnbGnbu[tokenId] + _balancesRewardEquivalentBnbGnimb[tokenId]) * ((block.timestamp - weightedStakeDate[tokenId]) * curRewardRate)) / (100 * rewardDuration);
    ...
    function updateMinPurchaseAmountAndRate(uint newAmount, uint256 _rewardRate) external onlyOwner {
        require(newAmount > 0 && _rewardRate > 0, "Amount must be greater than zero");
