...
import "./IWETH.sol";
import "./ICollectionWhitelistChecker.sol";

contract MetaSportsNFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    event AskNew(address indexed collection, address indexed seller, uint256 indexed tokenId, address tokenAddress, uint256 askPrice);

    // Ask order is updated
    event AskUpdate(address indexed collection, address indexed seller, uint256 indexed tokenId, address tokenAddress, uint256 askPrice);
    ...
    /**
     * @notice Buy token with Token by matching the price of an existing ask order
     ...
     */
    function buyNFTUsingToken(
        address _collection,
        uint256 _tokenId,
        address _tokenAddress,
        uint256 _price
    ) external nonReentrant {
        IERC20(_tokenAddress).safeTransferFrom(address(msg.sender), address(this), _price);

        _buyNFT(_collection, _tokenId, _price, _tokenAddress);
    }
     ...
     */
    function claimPendingRevenue(address _tokenAddress) external nonReentrant {
        ...
        IERC20(_tokenAddress).safeTransfer(address(msg.sender), revenueToClaim);
    ...
    function createAskOrder(
        ...
        require(_collections[_collection].tokenAddress == _tokenAddress, "Collection: Not for listing");
        ...
        _askDetails[_collection][_tokenId] = Ask({seller: msg.sender, tokenAddress: _tokenAddress, price: _askPrice});

        // Add tokenId to the askTokenIds set
        _askTokenIds[_collection].add(_tokenId);

        // Emit event
        emit AskNew(_collection, msg.sender, _tokenId, _tokenAddress, _askPrice);
    }
    ...
    function modifyAskOrder(
        ...
        // Emit event
        emit AskUpdate(_collection, msg.sender, _tokenId, _tokenAddress, _newPrice);
    }
    ...
    function addCollection(
        ...
        emit CollectionNew(_collection, _creator, _tokenAddress, _whitelistChecker, _tradingFee, _creatorFee);
    }
    ...
    function modifyCollection(
        ...
        emit CollectionUpdate(_collection, _creator, _tokenAddress, _whitelistChecker, _tradingFee, _creatorFee);
    }
    ...
    function _buyNFT(
        ...
        // Transfer Token
        IERC20(_tokenAddress).safeTransfer(askOrder.seller, netPrice);
        ...
        // Emit event
        emit Trade(_collection, _tokenId, askOrder.seller, msg.sender, _price, netPrice, _tokenAddress);
    }
