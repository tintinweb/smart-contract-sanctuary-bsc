...
contract BlindBoxSale is ERC1155Holder, Ownable {
   ...
   function _transferNftToAuctionContract(
    ...
    function saleWithTimeLimits()
        ...
        view
    {
        uint256 diffTime  = block.timestamp.sub(saleStartTime).mod(60 minutes  * diffDay);
    ...
    {
        uint256 timeIndex  = 0;
        if(block.timestamp.sub(saleStartTime)<= 60 minutes ){
            timeIndex = saleStartTime;
        }else{
            timeIndex = block.timestamp.sub(saleStartTime).div(60 minutes  * diffDay).mul(60 minutes  * diffDay).add(saleStartTime);
        }
        return(timeIndex);
    }
