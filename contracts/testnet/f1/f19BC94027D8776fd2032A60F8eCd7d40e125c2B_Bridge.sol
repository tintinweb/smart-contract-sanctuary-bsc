...
contract Bridge is IBridge, Pausable, Ownable {
    ...
    function mint(
        ...
        emit Transfer(from, to, amount, chainId, nonce, Step.Mint);
    }

    /// Burn on Vechain => Mint on ETH
    function burn(address to, uint256 amount, uint256 chainId) public whenNotPaused {
        IBridgeToken(TOKEN_ADDRESS).bridgeBurn(msg.sender, amount);

        emit Transfer(msg.sender, to, amount, chainId, _nonce, Step.Burn);
        _nonce++;
    }
