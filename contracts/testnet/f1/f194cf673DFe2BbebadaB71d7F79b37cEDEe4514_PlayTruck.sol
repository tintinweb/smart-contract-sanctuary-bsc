...
contract PlayTruck is ERC20, Ownable {
    ...
    uint public constant MAX_TAX = 1000*10**14;
	uint public constant MAX_TIME = 1000;

    uint public antiSandwichSeconds = 12;
    uint public sellTaxValue = MAX_TAX;
    ...
    event setSellTaxStatus(bool STstatus);
    event setSellTaxValue(uint _tax);
	event Tax(address from, address to, uint taxAmount);

    constructor() ERC20("PlayTRK", "PTRK") {
    ...
    function setAntiFRLimit(uint maxGas_) external onlyOwner{
        maxGas = maxGas_;
        emit setAntiFRGas(maxGas);
    }

    function toogleSellTax() external onlyOwner{
        sellTax = !sellTax;
        emit setSellTaxStatus(sellTax);
    }

    function setSellTax(uint sellTaxValue_) external onlyOwner{
        require(sellTaxValue_ <= MAX_TAX, "Cant set tax more than 12%");
        sellTaxValue = sellTaxValue_;
        emit setSellTaxValue(sellTaxValue_);
    }
    ...
    function setTaxRecipient(address who) external onlyOwner{
        taxRecipient = who;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (!whitelist[from] && notTax){
            ...
            if (sellTax){
                taxAmount = amount*sellTaxValue/10**18;
                notTax = false;
                _transfer(from, taxRecipient, taxAmount);
                notTax = true;
                emit Tax(from, to, taxAmount);
            }
        }
    }
