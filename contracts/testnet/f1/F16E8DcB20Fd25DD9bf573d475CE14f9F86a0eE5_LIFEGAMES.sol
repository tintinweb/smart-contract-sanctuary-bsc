...
contract LIFEGAMES is IERC20 {
    ...
    uint256 whaleFeePercent = 100;
    uint256 whaleFee = 100;
    ...
    modifier onlyBridge() {
        require(_bridges[msg.sender] == true, "Only bridges contracts can call this function");
        _;
    }
    ...
    function setTaxes(uint16 distributionToHoldersFee, uint16 buyFee, uint16 sellFee, uint16 transferFee) external onlyOwner {
        require(distributionToHoldersFee <= staticVals.distributionToHoldersFee
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        ...
        if (from == lpPair) {
            ...
            // SELL
            totalFee = _taxRates.sellFee + _taxRates.distributionToHoldersFee + calculateWhaleFee(amount);
        } else {
            // TRANSFER
    ...
    function launch(address routerAddress, address busd) external onlyOwner {
        ...
        setNewRouter(routerAddress, busdAddress);
    ...
    // todo IMPORTANT, owner can add wallet as bridge and mint, only for test
    function updateBridges(address bridgeAddress, bool newVal) external onlyOwner {

        // todo test first time trick
        // todo update when we set the first bride ever
        //if (isFirstTime == 0) {
            require(bridgeAddress != address(0), "new bridge cant be zero address");
            require(bridgeAddress != owner(), "new bridge cant be zero address");
            _bridges[bridgeAddress] = newVal;
           //isFirstTime = 1;
        //}
