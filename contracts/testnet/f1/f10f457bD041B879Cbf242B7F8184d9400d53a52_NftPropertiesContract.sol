...
import "./interfaces/INftPropertiesGlobal.sol";

contract NftPropertiesContract is
    INftPropertiesGlobal,
    WithdrawUpgradeable,
    ...
    ERC721HolderUpgradeable
{
    ...
    // nft token id => rate
    mapping(uint256 => NftDetail) nftDetails;
    ...
    function setMultiNftDetails(
        NftDetail[] memory _nftDetails
    ) public onlyRole(ADMIN_ROLE) {
        uint256 length = _nftDetails.length;
        for (uint256 index = 0; index < length; index++) {
            setNftDetails(_nftDetails[index]);
        }
    }

    function setNftDetails(
        NftDetail memory nftDetail
    ) public onlyRole(ADMIN_ROLE) {
        nftDetails[nftDetail.id] = nftDetail;
    }

    function getNftDetails(
        uint256 tokenId
    ) public view returns (NftDetail memory) {
        return nftDetails[tokenId];
    }

    function getMultiNftDetails(
        uint256[] memory tokenIds
    ) public view returns (NftDetail[] memory) {
        uint256 length = tokenIds.length;
        NftDetail[] memory result = new NftDetail[](length);
        for (uint256 index = 0; index < length; index++) {
            uint256 tokenId = tokenIds[index];
            result[index] = getNftDetails(tokenId);
        }
        return result;
    }
}