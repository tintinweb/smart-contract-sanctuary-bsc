...
contract SKATEBOARD721 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public imagesPage;

    event AddMiner(address indexed miner);
    event MinerEnable(address indexed miner, uint256 totalSupply, bool enable);
    
    constructor(string memory newUri, uint256 _maxSupply, uint256 _imagesPage) ERC721("SKATEBOARD", "SKATEBOARD") {
        _uri = newUri;
    ...
    function setImagesPage(uint256 newImagesPage) public onlyOwner {
        imagesPage = newImagesPage;
    }

    function uri(uint256 id) public view returns (string memory) {
        uint256 s = id % imagesPage;
        string memory baseURI = _baseURI();
        return string(abi.encodePacked(baseURI, s.toString(), ".json"));
    }
