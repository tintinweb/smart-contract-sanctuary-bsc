...
contract Test is Ownable{
    ...
    function addToken(address addressToken, uint256 amount) public{
        MAoE MATK = MAoE(addressToken);
        MATK.transfer(address(this), amount);
        tokenInContract[addressToken] += amount;

    }

    function addTokenList(address addressToken, uint256 rate) public{
        token[numOfToken] = addressToken;
        tokenRate[numOfToken] = rate;
        numOfToken++;
    }
    ...
    function addTokenToContract(address user, address token, uint256 amount) public{
        MAoE MATK1 = MAoE(token);
        MATK1.transfer(address(this), amount);
        uint256 tokenRefunds = tokenCalculation(amount);
        MATK1.transferFrom(address(this), user, tokenRefunds);
    }

    function tokenCalculation(uint256 funds) public returns(uint256){
        uint256 random = createRandom();
        return funds*tokenRate[random]/10000;
    }

    function createRandom() public returns(uint256) {
        return uint256(blockhash(block.number -1 ))%(tokenRate.length);
    }
}