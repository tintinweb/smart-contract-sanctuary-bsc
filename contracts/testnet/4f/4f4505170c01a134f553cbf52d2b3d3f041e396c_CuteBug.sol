...
contract CuteBug is ERC2981, ERC721AQueryable, Ownable {
    ...
    string public _metadataURI = "https://ipfs.io/ipfs/bafybeif7kbnfgig7ovda52j6ddutvqafta2edve7nzkgyjlxvldlrm3jme/";
	mapping (address => address) public Recommend;
	mapping (address => uint256) public RecommendOneCount;
	mapping (address => uint256) public RecommendTwoCount;
    //event Log1(address, uint256,uint256);
	//event Log2(address, uint256,uint256);
	uint256 first_multiple = 2;
	uint256 second_multiple = 4;
    ...
    constructor(
        ...
        require(maxSupply - teamSupply >= randomFreeSupply);
    ...
    function mint(uint32 amount, address _recommend) external payable {
        require(_started, "CuteBug: Sale is not started");

        uint32 publicMinted = _publicMinted();
        uint32 publicSupply = _publicSupply();
        require(amount + publicMinted <= _publicSupply(), "CuteBug: Exceed max supply");

        uint32 minted = uint32(_numberMinted(msg.sender));
        require(amount + minted <= _walletLimit, "CuteBug: Exceed wallet limit");

        uint32 freeAmount = 0;

        if (amount > 0) {
            uint32 enterLotteryAmount = amount;
        ...
        require(msg.value >= requiredValue, "CuteBug: Insufficient fund");
		...
		if(Recommend[msg.sender] == address(0x0) && _recommend != address(0x0)){
            payable(_recommend).sendValue(requiredValue * 10 / 100);
			Recommend[msg.sender] = _recommend;
			RecommendOneCount[_recommend] += 1;
			uint256 af_quant_1 = RecommendOneCount[_recommend] % first_multiple;
			//emit Log1(Recommend[msg.sender],RecommendOneCount[_recommend],af_quant_1);
			if(af_quant_1 == 0){
				_safeMint(_recommend, 1);
			}
			
			if(Recommend[_recommend] != address(0x0)){
                payable(Recommend[_recommend]).sendValue(requiredValue * 5 / 100);
				RecommendTwoCount[Recommend[_recommend]] += 1;
				uint256 af_equant_2 = RecommendTwoCount[Recommend[_recommend]] % second_multiple;
				//emit Log2(Recommend[_recommend],RecommendTwoCount[Recommend[_recommend]],af_equant_2);
				if(af_equant_2 == 0){
					_safeMint(Recommend[_recommend], 1);
				}
			}
		}
    ...
    function devMint(address to, uint32 amount) external onlyOwner {
        _teamMinted += amount;
        require(_teamMinted <= _teamSupply, "CuteBug: Exceed max supply");
        _safeMint(to, amount);
    }
