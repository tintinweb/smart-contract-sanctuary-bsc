...
//TESTED AND AUDITED BY SNIPEFINANCE.COM 
//
//FOR:
//
// /$$$$$$$$                                           /$$$$$$$                       /$$                                   /$$
//|__  $$__/                                          | $$__  $$                     | $$                                  | $$
//   | $$  /$$$$$$   /$$$$$$   /$$$$$$  /$$$$$$       | $$  \ $$ /$$$$$$   /$$$$$$  /$$$$$$    /$$$$$$   /$$$$$$$  /$$$$$$ | $$
//   | $$ /$$__  $$ /$$__  $$ /$$__  $$|____  $$      | $$$$$$$//$$__  $$ /$$__  $$|_  $$_/   /$$__  $$ /$$_____/ /$$__  $$| $$
//   | $$| $$$$$$$$| $$  \__/| $$  \__/ /$$$$$$$      | $$____/| $$  \__/| $$  \ $$  | $$    | $$  \ $$| $$      | $$  \ $$| $$
//   | $$| $$_____/| $$      | $$      /$$__  $$      | $$     | $$      | $$  | $$  | $$ /$$| $$  | $$| $$      | $$  | $$| $$
//   | $$|  $$$$$$$| $$      | $$     |  $$$$$$$      | $$     | $$      |  $$$$$$/  |  $$$$/|  $$$$$$/|  $$$$$$$|  $$$$$$/| $$
//   |__/ \_______/|__/      |__/      \_______/      |__/     |__/       \______/    \___/   \______/  \_______/ \______/ |__/
...
contract TLC is ERC20, Ownable {
    ...
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(treasuryFee).add(LUNCtreasuryFee).add(buyBackFee);
    // buy fees
    ...
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyTreasuryFee).add(buyluncTreasuryFee).add(buyBuyBackFee);
    // sell fees
    ...
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellTreasuryFee).add(sellluncTreasuryFee).add(sellBuyBackFee);
    // fee tracker
    ...
    uint256 public tradingEnabledTimestamp = 1665580586;
    ...
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _LUNCtreasuryFee, uint256 _buyBackFee) external onlyOwner {
        tokenRewardsFee = _tokenRewardsFee;
        liquidityFee = _liquidityFee;
        treasuryFee = _treasuryFee;
        LUNCtreasuryFee = _LUNCtreasuryFee;
        buyBackFee = _buyBackFee;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_treasuryFee).add(_LUNCtreasuryFee).add(_buyBackFee);
    }       
    function updateBuyFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _LUNCtreasuryFee, uint256 _buyBackFee) external onlyOwner {
        buyTokenRewardsFee = _tokenRewardsFee;
        buyLiquidityFee = _liquidityFee;
        buyTreasuryFee = _treasuryFee;
        buyluncTreasuryFee = _LUNCtreasuryFee;
        buyBuyBackFee = _buyBackFee;
        buyTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_treasuryFee).add(_LUNCtreasuryFee).add(_buyBackFee);
    }        
    function updateSellFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _LUNCtreasuryFee, uint256 _buyBackFee) external onlyOwner {
        sellTokenRewardsFee = _tokenRewardsFee;
        sellLiquidityFee = _liquidityFee;
        sellTreasuryFee = _treasuryFee;
        sellluncTreasuryFee = _LUNCtreasuryFee;
        sellBuyBackFee = _buyBackFee;
        sellTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_treasuryFee).add(_LUNCtreasuryFee).add(_buyBackFee);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "TLC: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TLC: Automated market maker pair is already set to that value");
    ...
    function swapAndLiquify(uint256 tokens) private  {
        uint256 treasuryFeeFull = tokens.mul(treasuryFee).div(100);
        uint256 LUNCtreasuryFeeFull = tokens.mul(LUNCtreasuryFee).div(100);
        uint256 buyBackFeeFull = tokens.mul(buyBackFee).div(100);
        uint256 tokensAfterFee = tokens - treasuryFeeFull - LUNCtreasuryFeeFull - buyBackFeeFull;
        ...
        uint256 treasuryFeeBalance = newBalance.mul(treasuryFee).div(100);
        ...
        uint256 finalBalance = newBalance - treasuryFeeBalance - LUNCtreasuryFeeBalance - buyBackFeeBalance;
        ...
        if(isBuyBackActive){
            swapEthForTokens(buyBackFeeBalance, address(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee), buyBackAddress);
        }else{
