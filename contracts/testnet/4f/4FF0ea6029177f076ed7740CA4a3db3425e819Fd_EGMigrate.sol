...
contract EGMigrate is Ownable {
    ...
    struct MigrationToken {
        ...
        uint256 rate; // rate of migration
        address devAddress; // the address to burn the source token
        uint256 amountOfMigratedSourceToken; // total amount of migrated source token
        uint256 amountOfMigratedTargetToken; // total amount of migrated target token
        uint256 numberOfMigrators; // total number of migrators
        bool isPresent; // is migration present
        bool enabled; // is migration enabled
    }
    ...
    Counters.Counter public sourceTokenCounter; // counter for source tokens
     ...
     * @param devAddress the address to burn the source token
    ...
    function addMigrationToken(
        ...
        require(
            ...
            "EGMigration: source token address is zero"
        );
        require(
            !migrationTokens[sourceToken].isPresent,
            "EGMigration: source token already exists"
        );
        require(
            targetToken != address(0),
            "EGMigration: target token address is zero"
        );
        require(0 < rate, "EGMigration: rate is zero");

        MigrationToken memory migrationToken = MigrationToken({
            index: sourceTokenCounter.current(),
        ...
        sourceTokenIndices[sourceTokenCounter.current()] = sourceToken;
        sourceTokenCounter.increment();
        ...
        onlyOwner
    {
        require(
            migrationTokens[sourceToken].isPresent,
            "EGMigration: source token does not exist"
        );
     ...
     * @param toAddress address to send the tokens
    ...
    function migrate(
        ...
        require(
            ...
            "EGMigration: migration is disabled for this token"
        );
        require(
            toAddress != address(0),
            "EGMigration: transfer to the zero address is not allowed"
        );
        require(0 < amount, "EGMigration: amount is zero");

        MigrationToken storage migrationToken = migrationTokens[token];

        require(
            amount <= IERC20(token).balanceOf(msg.sender),
            "EGMigration: insufficient balance of source token in holder wallet"
        );
        require(
            amount <= IERC20(token).allowance(msg.sender, address(this)),
            "EGMigration: holder has insufficient approved allowance for source token"
        );

        uint256 migrationAmount = amount
            .mul(10**ERC20(migrationToken.targetToken).decimals())
            .div(10**ERC20(token).decimals())
            .div(migrationToken.rate);

        require(
            migrationAmount <
                IERC20(migrationToken.targetToken).balanceOf(address(this)),
            "EGMigration: insufficient balance of target token"
        );
        ...
        migrationToken.amountOfMigratedSourceToken = migrationToken
            .amountOfMigratedSourceToken
            .add(amount);

        IERC20(migrationToken.targetToken).transfer(toAddress, migrationAmount);
        migrationToken.amountOfMigratedTargetToken = migrationToken
            .amountOfMigratedTargetToken
            .add(migrationAmount);

        Migration[] storage userTxns = userMigrations[token][_msgSender()];
        if (userTxns.length == 0) {
            migrationToken.numberOfMigrators = migrationToken
                .numberOfMigrators
                .add(1);
        }
        ...
        migrationCounter = migrationCounter.add(1);
     ...
     *
     * @dev get length of user migrations
     */
        ...
        )
    {
        ...
        return (txn.migrationId, txn.timestamp, txn.amountOfSourceToken, txn.amountOfTargetToken);
    }
    ...
    function returnTokens(
        ...
        MigrationToken storage migrationToken = migrationTokens[sourceToken];
