    ...
    AccessControlEnumerable
{
    ...
    /**
    @notice Role of administrative users allowed to mint a XRunShoes from the
    box.
     */
    ...
    mapping(address => uint256) private BoxMintedByShop;
    ...
    modifier reducewhitelistPool(uint256 n) {
        require(n <= whitelistPool, "has minted");
        whitelistPool -= n;
        _;
    }
    ...
    function setGameShopMintOpen() external onlyRole(DEV) {
        gameShopMintOpen = true;
    }
    bool public gameUserMintOpen = false;

    function setGameUserMintOpen() external onlyRole(DEV) {
        gameUserMintOpen = true;
    }
    ...
    function foudingTeamMint()
        ...
        onlyRole(DEV)
    {   
        require(_numberMinted(_msgSender()) < BoxAllowedFoundingTeam, "minted 100");
        ...
        reducewhitelistPool(whitelistPool)
    {
        ...
        require(_numberMinted(_msgSender()) < BoxAllowedPerAddress,"One pack per address");
    ...
    function expelFromBox(uint256 tokenId) external onlyRole(DEV) {
        require(openBoxStarted[tokenId] != 0, "not OpenBox");
