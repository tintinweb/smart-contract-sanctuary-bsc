...
contract NFTAuction is ProofsVerifier{
    ...
    function startAuction(int x, int y, bytes32[] calldata proof) public {
        ...
        emit StartAuction(x, y, msg.sender, startPrice);
    ...
    function getTradedPlotPrice(int x, int y) public returns(uint256){
        return highestPrice[getHash(x,y,currentRound)];
    }

    function getTradedPlotOwner(int x, int y) public returns(address){
        return tokenOwner[getHash(x,y,currentRound)];
    }

    function timeLeft(int x, int y) public returns(uint256){
        return (block.timestamp - lastBidTime[getHash(x, y, currentRound)]);
    }
    ...
    event StartAuction(int indexed x, int indexed y, address firstOwner, uint256 amount);
