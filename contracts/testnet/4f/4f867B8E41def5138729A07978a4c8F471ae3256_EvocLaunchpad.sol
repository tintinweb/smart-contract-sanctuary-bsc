...
contract EvocLaunchpad is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => Partner_register) public PartnerRegister;
    ...
    function balanceOfPayment(
        uint256 launchId,
        address account,
        string memory tokenName
    ) public view returns (uint256 amount) {
        return
            PartnerRegister[launchId]
                .UserConfig[account]
                .payOut[tokenName]
                .amount;
    }

    function getUserConf(uint256 launchId, address account)
        public view returns (
            uint256 balance,
            uint256 maxAmount,
            address referAddress,
    ...
    {
        User_Config storage item = PartnerRegister[launchId].UserConfig[
            account
        ];
        return (
            item.balance,
    ...
    function getPartnerValues(uint256 PartnerId)
        public view returns (
            uint256 softCap,
            uint256 hardCap,
    ...
    function getLaunchStatus(uint256 PartnerId)
        public view returns (
            uint256 timeInit,
            uint256 timeEnd,
        ...
        )
    {
        ...
        return (
            ...
            item.PartnerStatus._LiquidityStatus,
            item.PartnerStatus._status,
            item.isClaiming
        );
    }

    function getPartnerReferConfig(uint256 PartnerId)
        public view returns (
    ...
    function buyWithPix(
        ...
        address refer
    ) public isPausable onlyOwner {
        require(launchStatus(PartnerId) == 1, "Launchpad Not Active");
        uint256 tokenAmount = _tokenAmount.mul(_DECIMALFACTOR);
        Partner_register storage item = PartnerRegister[PartnerId];
        require(
        ...
        require(
            tokenAmount > 0 ||
                tokenAmount >= item.PartnerValues._totalRemaining,
            "Insufficient amount for this transaction"
        );
        uint256 amount = item.PartnerValues._launchPriceRate.mul(_tokenAmount);
        require(
            amount > 0,
            "Insufficient amount for this transaction"
        );

        if (!item.UserConfig[_deliveryAddress].exist) {
            item.UserConfig[_deliveryAddress].maxAmount = tokenAmount;
            item.UserConfig[_deliveryAddress].exist = true;
        } else {
            item.UserConfig[_deliveryAddress].maxAmount += tokenAmount;
        }

        item.Balance["BNB"].amount += amount;
        item.UserConfig[_deliveryAddress].payOut["BNB"].amount += amount;
        ...
        if (item.PartnerRefer._referActive) {
            ...
            referReward = (tokenAmount * item.PartnerRefer._referBonusPercent)
                .div(100);
        ...
        item.UserConfig[_deliveryAddress].balance += newBalance;
        item.PartnerValues._totalRemaining -= newBalance.add(referReward);
        item.PartnerValues._totalTokenSold += newBalance.add(referReward);
        emit buyTokensSuccess(msg.sender, item._contractAdd, tokenAmount, "BNB");
    }
    ...
    function buyWithToken(
        ...
        uint256 tokenAmount = _tokenAmount.mul(_DECIMALFACTOR);
        uint256 tkAmount = item.PartnerValues._launchPriceRate.mul(
            _tokenAmount
        );
        require(
            tokenAmount > 0 ||
        ...
        require(
            ContractToken.transferFrom(msg.sender, address(this), amountPrice),
        ...
        item.PartnerValues._totalRemaining -= newBalance.add(referReward);
        item.PartnerValues._totalTokenSold += newBalance.add(referReward);
        emit buyTokensSuccess(msg.sender, item._contractAdd, tokenAmount, tkName);
    }
    ...
    function createPartner(
        ...
        PartnerRegister[partnerId_].PartnerValues._totalRemaining = (
            _hardCap.div(_launchPriceRate)
        ).mul(_DECIMALFACTOR);
        ...
        if(_LiquidityStatus){
            if (_compareString(_LiquidityTipe, "EVOC")) {
                PartnerRegister[partnerId_].PartnerStatus
                    ._tokenPair = _getEvcPairAddress(_contractAdd);
            } else {
                PartnerRegister[partnerId_].PartnerStatus
                    ._tokenPair = _getPckPairAddress(_contractAdd);
            }
            PartnerRegister[partnerId_].PartnerStatus._LiquidityStatus = true;
    ...
    function _receiveTokensPatner(
        ...
        uint256 dexBalance = ContractToken.balanceOf(msg.sender);
        uint256 liquidityAmount = (_hardCap.mul(_liquidityPercentage)).div(100);
        uint256 LiquifyBalance = liquidityAmount.div(_listingPriceRate);
        uint256 resultTokens = (_hardCap.div(_launchPriceRate)) + LiquifyBalance;
        uint256 tokensAmount = (resultTokens).mul(_DECIMALFACTOR);
        require(
    ...
    function updatePartner(
        ...
        emit UpdateLaunchpad(
            msg.sender,
            PartnerRegister[PartnerId]._contractAdd
        );
    }

    function cancelPartner(uint256 PartnerId) public payable isPausable {
        require(
            PartnerRegister[PartnerId].exist,
            "A Partner does not exist, check the contract or create it first"
        );
        ...
        require(
            !PartnerRegister[PartnerId].isClaiming,
            "Project cannot be cancelled, as it has already been released for claim"
        );
        ...
        if(dexBalance >= PartnerRegister[PartnerId].PartnerValues._totalRemaining){
            dexBalance = PartnerRegister[PartnerId].PartnerValues._totalRemaining;
        }

        if (dexBalance > 0) {
            ContractAdd.transfer(
                PartnerRegister[PartnerId]._OwnerAdd,
                dexBalance
            );
        }
    ...
    function configureReferSystem(
        ...
        PartnerRegister[PartnerId]
            .PartnerRefer
            ._referBonusPercent = _referBonusPercent;
    ...
    /**
     * ----------------------
     *  Claiming tokens of Users
     * ----------------------
     */
    function claimTokens(uint256 PartnerId) public nonReentrant isPausable {
        address wallet = msg.sender;
        Partner_register storage item = PartnerRegister[PartnerId];
        require(
            launchStatus(PartnerId) == 2,
            "Launchpad Release not yet completed"
        );
        require(
            wallet != address(0) && item.UserConfig[wallet].exist,
        ...
        if (!item.PartnerRefer._referActive) {
            ...
            uint256 amountClaim = balance
                .mul(item.PartnerRefer._referVesting)
                .div(100);
            IBEP20 tokenAddr = IBEP20(item._contractAdd);
            tokenAddr.transfer(wallet, amountClaim);
            item.UserConfig[wallet].balance -= amountClaim;
            item.UserConfig[wallet].timestamp =
                block.timestamp +
                item.PartnerRefer._referVesting;
        } else {
    ...
    /**
     * ----------------------
     *  Claiming Funds of Users on Launchpad canceled
     * ----------------------
     */
    function claimFunds(uint256 PartnerId) public nonReentrant isPausable {
        address wallet = msg.sender;
        Partner_register storage item = PartnerRegister[PartnerId];
        require(
            launchStatus(PartnerId) == 0  || !item.PartnerStatus._status,
            "It is not possible to reclaim your funds, as the project is still active"
        );        
        require(
        ...
        for (uint256 i = 0; i < item._paymentType.length; i++) {
            uint256 balance = balanceOfPayment(
                PartnerId,
                wallet,
                item._paymentType[i]
            );
            if (balance > 0) {
                if (_compareString(item._paymentType[i], "BNB")) {
                    ...
                    IBEP20 ContractAdd = IBEP20(
                        _getPayAddress(item._paymentType[i])
                    );
                    uint256 dexBalance = ContractAdd.balanceOf(address(this));
                    if (balance <= dexBalance) {
                        ContractAdd.transfer(wallet, balance);
                        item.UserConfig[wallet].balance = 0;
                        item
                            .UserConfig[wallet]
                            .payOut[item._paymentType[i]]
                            .amount = 0;
                        emit WithdrawnUser(wallet, balance, item._contractAdd);
                    }
                }
    ...
    /**
     * ----------------------
     * Finish Launchpad and Set Claiming function
     * ----------------------
     */
    function setClaiming(uint256 PartnerId) public nonReentrant isPausable {
        Partner_register storage item = PartnerRegister[PartnerId];
        require(launchStatus(PartnerId) == 2, "Release not yet completed");
        require(
            item.exist && item.PartnerStatus._status,
            "A Partner does not exist, check the contract or create it first"
        );
        ...
        if(item.PartnerStatus._LiquidityStatus){
            _swapAndLiquify(PartnerId);
        }

        for (uint256 i = 0; i < item._paymentType.length; i++) {
            uint256 balance = item.Balance[item._paymentType[i]].amount;
            if (balance > 0) {
                uint256 freetransactions = balance.mul(feeTransPercent).div(100);
                uint256 transfBalance = balance.sub(freetransactions);
                if (_compareString(item._paymentType[i], "BNB")) {
                    payable(_companyAddress).transfer(freetransactions);
                    payable(item._OwnerAdd).transfer(transfBalance);
                    ...
                    if (balance <= dexBalance) {
                        ContractAdd.transfer(_companyAddress, freetransactions);
                        ContractAdd.transfer(item._OwnerAdd, transfBalance);
                        item.Balance[item._paymentType[i]].amount = 0;
                        emit WithdrawnUser(
                            item._OwnerAdd,
                            balance,
    ...
    function _swapAndLiquify(uint256 PartnerId) private lockTheSwap {
        Partner_register storage item = PartnerRegister[PartnerId];

        // Number of tokens sold divided by the percentage that will be added to liquidity.
        uint256 SoldAmount = item.PartnerValues._totalTokenSold.mul(
            item.PartnerValues._launchPriceRate
        );
        uint256 liquidPercent = SoldAmount
            .mul(item.PartnerStatus._liquidityPercentage)
            .div(100);
        uint256 tokenAmount = liquidPercent.div(
            item.PartnerValues._listingPriceRate
        );
        ...
        for (uint256 i = 0; i < item._paymentType.length; i++) {
            ...
            if (amount > 0) {
                ...
                if (!_compareString(item._paymentType[i], "BNB")) {
                    address payAddress = _getPayAddress(item._paymentType[i]);
                    IBEP20 ContractAdd = IBEP20(payAddress);
                    uint256 dexBalance = ContractAdd.balanceOf(address(this));
                    if (balance <= dexBalance) {
                        _swapTokensForBNB(balance, payAddress); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered;
                        item.Balance[item._paymentType[i]].amount -= balance;
                    }
                }
            }
        }
        ...
        uint256 tokensReceived = (item.Balance["BNB"].amount)
            .mul(item.PartnerStatus._liquidityPercentage)
            .div(100);
        uint256 bnbAmount = tokensReceived.add(newBalance);
        item.Balance["BNB"].amount -= tokensReceived;

        // add liquidity to uniswap
        _addLiquidity(PartnerId, bnbAmount, tokenAmount);

        emit SwapAndLiquify(tokensSwapped, bnbAmount, tokenAmount);
    }

    function _swapTokensForBNB(uint256 tokenAmount, address contractAdd_)
        private
    {
        IUniswapV2Router02 routerAdress = IUniswapV2Router02(pckRouterAddress_);
        ...
        IBEP20(contractAdd_).approve(address(routerAdress), tokenAmount);
    ...
    function _addLiquidity(
        ...
        IUniswapV2Router02 routerAdress = IUniswapV2Router02(
            PartnerRegister[PartnerId].PartnerStatus._tokenPair
        );
    ...
    function launchStatus(uint256 PartnerId) public view returns (uint256) {
        Partner_register storage item = PartnerRegister[PartnerId];
        uint256 softCap = (
            item.PartnerValues._softCap.div(item.PartnerValues._launchPriceRate)
        ).mul(_DECIMALFACTOR);
        uint256 hardCap = (
            item.PartnerValues._hardCap.div(item.PartnerValues._launchPriceRate)
        ).mul(_DECIMALFACTOR);
        ...
        if (
            ...
            item.PartnerValues._totalRemaining >= softCap
        ) {
            return 0; // Failure
        }
        if (
            block.timestamp >= item.PartnerStatus._timeEnd &&
            item.PartnerValues._totalTokenSold >= hardCap
        ) {
            return 2; // Wonderful - reached to Hardcap
        }
        if (
            block.timestamp >= item.PartnerStatus._timeEnd &&
            item.PartnerValues._totalTokenSold >= softCap
        ) {
            return 2; // Wonderful - reached to softCap
        }
        if (
            block.timestamp >= item.PartnerStatus._timeEnd &&
            item.PartnerValues._totalTokenSold >= hardCap ||
            ...
            return 2; // SUCCESS - Presale ended with reaching Hardcap
        }
        if (
    ...
    /*
     * @dev gets the Address of Busd, Usdt and BNB.
     */
    ...
    /* Function     : Gat a Current router PancakeSwap  */
    /* Parameters   : Current router Address */
    function _getPckPairAddress(address tokenAddress)
        internal returns (address PairAdress)
    {
        require(tokenAddress != address(0), "Should not be address 0");
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            pckRouterAddress_
        );
        address tokenPair = IUniswapV2Factory(_uniswapV2Router.factory())
        ...
        return address(pckRouterAddress_);
    }

    /* Function     : Gat a Current router EvocSwap  */
    /* Parameters   : Current router Address */
    function _getEvcPairAddress(address tokenAddress)
        internal returns (address PairAdress)
    {
        require(tokenAddress != address(0), "Should not be address 0");
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
        ...
        return address(evcRouterAddress_);
    }
    ...
    /* Function     : Set Company Address  */
    /* Parameters   : New Company Address */
    function setCompanyAddress(address companyAddress) public onlyOwner {
        _companyAddress = companyAddress;
    }

    /* Function     : Set fee to created a new Project */
    /* Parameters   : Update fee Create Project */
    ...
    function setIsFeeExempt(address contractAddress)
        public onlyOwner
    {
        if(isFeeExempt[contractAddress]) {
            isFeeExempt[contractAddress] = false;
        }else{
            isFeeExempt[contractAddress] = true;
        }
    }
     ...
     * ----------------------
     */
    event CreateLaunchpad(address indexed from, address contractAdd);
     ...
     * ----------------------
     */
    event UpdateLaunchpad(address indexed from, address contractAdd);
