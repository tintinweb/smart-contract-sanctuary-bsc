...
contract RootV2 is AdminV1, VoucherV2 {
    ...
    function createProject(
        ...
        require(
            props.claimDates.length == props.claimPercent.length &&
                props.price > 0 &&
                props.token != IERC20UpgradeableWithDecimals(address(0)),
    ...
    function changeProjectProps(uint256 projectId, ProjectProps memory newProps)
        public
        onlyAdmin
    {
        require(
    ...
    function cancelParticipation(uint256 projectId, address investorAddr)
        public
    {
        require(
    ...
    function airDrop(uint256 projectId, address[] memory investors)
        public
        onlyAdmin
    {
        Project storage project = _projects[projectId];
    ...
    function returnProjectTokens(ReturnProjectTokensVoucher calldata voucher)
        public
    {
        require(
    ...
    function investorDataFromProject(uint256 projectId, address investor)
        public
        view
    ...
    function investorDataWithProject(uint256 projectId, address investor)
        public
        view
        returns (
            uint256 id,
    ...
    function getLastClaimAmount(uint256 projectId, address investorAddr)
        public
        view
        returns (uint256 lastClaimAmount, uint256 lastClaimIndex)
    {
        Project storage project = _projects[projectId];
        require(
    ...
    function getInoBalance(uint256 projectId, uint256[] memory boxIDs)
        public
        view
    ...
    function setFeeVault(address feeVault) public onlyOwner {
        _feeVault = feeVault;
    }

    ///@notice projectId => investorAddr => amount
