...
contract LiquidityStakingV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    modifier onlyAdmin {
        require (msg.sender == factory || msg.sender == admin || msg.sender == Ownable(factory).owner(), "!admin");
        _;
    }
    ...
    function addPool(uint256 _allocPoint, uint256 _lockupDuration, uint256 _startTime) public onlyAdmin {
    ...
    function setPool(uint _pid, uint _allocPoint, uint _lockupDuration) external onlyAdmin {
        PoolInfo storage pool = poolInfo[_pid];
        totalAllocPoint = totalAllocPoint - pool.allocPoint + _allocPoint;
        pool.lockupDuration = _lockupDuration;
    }

    function setStartTime(uint _pid, uint _startTimeInMins, bool _updateAcc) external onlyAdmin {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function expandEndTime(uint _mins) external onlyAdmin {
    ...
    // function setUnlockDuration(uint _durationMins) external onlyAdmin {
