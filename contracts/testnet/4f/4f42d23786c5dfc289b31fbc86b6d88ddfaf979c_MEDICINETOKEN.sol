...
contract MEDICINETOKEN is ERC721Enumerable, Ownable {
  using Strings for uint256;
  using SafeMath for uint256;
  bool public paused = false;
  string public baseURI;
  address public settlementFeeAddress = 0x78D79B709C8A0b9D472730028604A2CF4Db141aA;
  uint256 public settlementFeePercentage = 500; // 5% decimal number 100
  ...
  struct _tokenDetails{
      ...
      bool isActive;
      bool forSale;
      address correspondingAuthor;
      uint256 correspondingAuthorPercentage;
  }
 ...
 mapping(uint256 => _bidDetail) public bidDetail;
  ...
  function pause(bool _state) public onlyOwner {
    ...
    function _mint(uint256 tokenId,uint256 salePrice, uint256 royalty,SellType sellType, uint256 auctionStart, uint256 auctionExpire, address correspondingAuthor, uint256 correspondingAuthorPercentage ) public payable returns(uint256){
        ...
        tokenDetails[tokenId] = _tokenDetails(msg.sender,msg.sender,royalty,auctionStart,auctionExpire,salePrice,sellType,true,true, correspondingAuthor, correspondingAuthorPercentage);
        return tokenId;
    }
    ...
    function buy(uint256 tokenId) payable public {
        ...
        if(tokenDetails[tokenId].correspondingAuthor != address(0)){
            autherAmt = tokenDetails[tokenId].salePrice.mul(tokenDetails[tokenId].correspondingAuthorPercentage).div(100);
            payable(tokenDetails[tokenId].correspondingAuthor).transfer(autherAmt);
        }        
        ...
        payable(tokenDetails[tokenId].owner).transfer(tokenDetails[tokenId].salePrice.sub(royaltyAmt).sub(feeAmt).sub(autherAmt));
  ...
  function acceptBid(uint256 tokenId) external {
    ...
    payable(tokenDetails[tokenId].owner).transfer(bidDetail[tokenId].amount.sub(royaltyAmt).sub(feeAmt).sub(autherAmt));
