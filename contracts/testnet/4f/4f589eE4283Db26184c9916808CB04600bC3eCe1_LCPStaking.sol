...
contract LCPStaking {
    ...
    uint256[] public Duration = [30 days, 60 days, 90 days, 120 days];
    uint256[] public Bonus = [50, 120, 200, 500];
    uint256[] public totalStakedPerPlan;
    uint256[] public totalStakersPerPlan;
    ...
    mapping(address => NewUser) public Stakers;
    mapping(uint256 => address) public StakersID;
    mapping(address => mapping(uint256 => NewStake)) public stakersRecord;
    ...
    // function restake() public {
    //     // require(!pauseStaking, "restaking Paused");
    //     uint256 index = Stakers[msg.sender].stakeCount;

    //     oldStaking.User memory userOldStakeCount = oldStakingAddress.Stakers(
    //         msg.sender
    //     );
    //     uint256 stakedCount = userOldStakeCount.stakeCount;
    //     uint256 totalrestakeedAmount;
    //     for (uint256 i = 0; i < stakedCount; i++) {
    //         Stake storage userStake = stakersRecord[msg.sender][i];
    ...
    //         uint256 rewardAmount = rewardTime.mul(userOldStake.persecondreward);
    //         uint256 amountBefore = userOldStake.amount;
    //         uint256 total = rewardAmount.add(amountBefore);
    //         totalrestakeedAmount += total;
    //     }

    //     stakeToken.transferFrom(owner, address(this), totalrestakeedAmount);
    ...
    //     emit RESTAKE(msg.sender, totalrestakeedAmount);
    // }

    function StakingOnOff(bool _state) public onlyowner {
        pauseStaking = _state;
    }

    function _isExcludedFromReward(address _add) public view returns (bool) {
        return excludeFromReward[_add];
    }

    function changeoldstaking(address _staking) public onlyowner {
        oldStakingAddress = oldStaking(_staking);
    }
}
