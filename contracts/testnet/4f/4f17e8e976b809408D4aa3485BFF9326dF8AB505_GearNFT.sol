...
contract GearNFT is ERC1155Supply, Ownable {
    ...
    function createToken(uint256 _id, string calldata _uri) public onlyOwner {
        tokenURIs[_id] = _uri;
        emit URI(_uri, _id);
    }

    function createTokens(uint256[] calldata ids, string[] calldata uris) external onlyOwner {
        for (uint256 i; i < ids.length; i++) {
            createToken(ids[i], uris[i]);
        }
    }
