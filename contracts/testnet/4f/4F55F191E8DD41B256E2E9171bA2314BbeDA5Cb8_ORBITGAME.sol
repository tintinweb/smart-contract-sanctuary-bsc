...
contract ORBITGAME {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 0.01 ether; // 0.05 BNB
	uint256[] public REFERRAL_PERCENTS = [50, 30, 20,10,5];
    uint256[] public REFERRAL_MINIMUM = [0 ether, 0.1 ether, 0.3 ether,1.5 ether,2.5 ether];
	...
	constructor(address payable wallet, address payable _developer , address payable _ref) public {
		...
		plans.push(Plan(14, 86, 1)); // 8,6% per day for 14 days (Withdrawl 6 Hours)
	...
	function _invest(address payable referrer, uint8 plan, address payable sender, uint256 value) private {
		...
		user.deposits.push(Deposit(plan, percent, value, profit, block.timestamp, finish, ( block.timestamp+(plans[plan].withdrawl*60))));
	...
	function withdraw() public {
        ...
        for (uint256 i = 0; i <= user.deposits.length; i++) {
			 if(block.timestamp > user.deposits[i].cooldown) {
                 //uint256 number2 = user.deposits[i].plan;
                //user.deposits[i].cooldown = block.timestamp+(plans[number2].withdrawl*60*60);
             }
		}
