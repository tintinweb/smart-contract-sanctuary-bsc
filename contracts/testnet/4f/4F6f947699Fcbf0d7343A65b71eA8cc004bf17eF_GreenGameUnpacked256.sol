// https://bscscan.com/address/0xfbe4bab5e71fa0c54026e11954b137e64115e900#code
...
contract GreenGameUnpacked256 is Ownable {
    struct Table {
        uint256 thValue;
    ...
    function buy(address parent) public payable {
        ...
        if (address2id[msg.sender] != 0) { // ignore any parent value if it's not a first purchase
    ...
    function info(address customer) public view returns(CustomerData memory){
        CustomerData memory data;
        data.ID = address2id[customer];
        data.table = address2table[customer];
        data.parent = parents[customer];
        data.parentID = address2id[data.parent];
        data.refSum = refSum[customer];
        data.missedRefSum = missedRefSum[customer];
        data.donationSum = donationSum[customer];
        data.donationRefSum = donationRefSum[customer];
        data.missedDonationRefSum = missedDonationRefSum[customer];
        return data;
    }

    function infoTable(uint256 table, address customer) public view returns(CustomerTableData memory){
        CustomerTableData memory data;
        data.addressesCount = tableAddresses[table].length;
        data.donationsCountReceivedAlready = donationsCountReceivedAlready[table][customer];
        data.refSum = refTableSum[table][customer];
        data.missedRefSum = missedRefTableSum[table][customer];
        data.donationSum = donationTableSum[table][customer];
        data.donationRefSum = donationRefTableSum[table][customer];
        data.missedDonationRefSum = missedDonationRefTableSum[table][customer];
        return data;
    }
