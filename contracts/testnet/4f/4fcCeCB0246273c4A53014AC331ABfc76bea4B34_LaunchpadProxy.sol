...
contract IQZoneNFT is ERC1155, AccessControl {
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < length; ) {
            if (!properties[ids[i]].transferable) revert('token not transferable');
            unchecked {
                ++i;
            }
        }
    ...
    /// @param uri specific nft uri
    function create(Property memory property, string memory uri)
        external
    ...
    function mint(
        ...
        if (properties[id].singleton) {
            require(balanceOf(to, id) == 0, 'You cant have more than one');
            require(amount == 1, 'You cant have more than one');
        }
...
library DexRouterPairs {
    function getPair(
        ...
        return IDexFactory(router.factory()).getPair(tokenA, tokenB);
    }
    ...
    function price(
        ...
        require(pair != address(0), 'pair not listed');

        (uint256 reserve0, uint256 reserve1, ) = IDexPair(pair).getReserves();
        uint256 reserveA = (tokenA == IDexPair(pair).token0()) ? reserve0 : reserve1;
        uint256 reserveB = (tokenB == IDexPair(pair).token0()) ? reserve0 : reserve1;
...
contract LaunchpadProxy is ILaunchpad2, Initializable, AccessControlUpgradeable {
    ...
    uint256 public constant ROUNDS = 3;

    uint256 public hardCap;
    uint256 public softCap;
    uint256 public projectTokensAmount;
    ...
    uint256[ROUNDS] public raised;
    mapping(address => uint256)[ROUNDS] public invested;
    address[ROUNDS] public vestingContracts;
    ...
    function initialize(
        ...
        require(launchData_.projectTokensAmount != 0, 'zero project tokens');

        hardCap = launchData_.hardCap;
        softCap = launchData_.softCap;
        projectTokensAmount = launchData_.projectTokensAmount;
        maxVentureAllocation = launchData_.maxVentureAllocation;
        startTime = launchData_.startTime;
        duration = launchData_.duration;
        projectToken = IERC20Metadata(launchData_.projectToken);
        projectWallet = launchData_.projectWallet;
        ...
        // TODO: is this safe? what if this reverts?
        IERC20Metadata(launchData_.projectToken).safeTransferFrom(
            msg.sender,
            address(this),
            launchData_.projectTokensAmount
        );
    }

    function transferProjectToken(uint256 amount, address to)
        external
    ...
    function transferStableToken(uint256 amount, address to)
        external
        onlyRole(VESTING_ROLE)
    {
    ...
    function setVestingContracts(address[] calldata vestingContracts_)
        external
        onlyRole(MANAGER_ROLE)
    {
        require(vestingContracts_.length == ROUNDS, 'wrong number of vesting contracts');

        for (uint256 i = 0; i < ROUNDS; ) {
            require(vestingContracts_[i] != address(0), 'zero address');
            vestingContracts[i] = vestingContracts_[i];
            _grantRole(VESTING_ROLE, vestingContracts_[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function _hasInvested(address user) internal view returns (bool) {
        for (uint256 i = 0; i < ROUNDS; ) {
            if (invested[i][user] != 0) return true;
    ...
    function meetsAllocationLimit(
        ...
        for (uint256 i = 0; i < balances.length; ) {
            ...
            uint256 maxAllocationSize = (i == 4 && round <= 1)
                ? maxVentureAllocation
                : props.maxAllocationSize;

            if (
                balances[i] > 0 &&
                amount <= maxAllocationSize * balances[i] &&
                amount >= props.minAllocationSize
            ) return true;
    ...
    function invest(
        ...
        require(round < ROUNDS, 'round out of bounds');
        ...
        uint256 totalAmount = invested[round][investor] + stableAmount;
        require(
            meetsAllocationLimit(totalAmount, investor, round, data, signature),
            'amount does not meet the allocation limit'
        );

        if (!_hasInvested(investor)) investors.push(investor);

        invested[round][investor] = totalAmount;
        raised[round] += stableAmount;
        emit Invested(investor, round, stableAmount);

        stableToken.safeTransferFrom(msg.sender, address(this), stableAmount);
    }

    function _raisedTotal() internal view returns (uint256 total) {
        for (uint256 i = 0; i < ROUNDS; ) {
            total += raised[i];
            unchecked {
                ++i;
    ...
    function raisedTotalCapped() public view returns (uint256) {
        uint256 total = _raisedTotal();
        return total > hardCap ? hardCap : total;
    }

    function userTotal(uint8 round, address user) external view returns (uint256) {
        return (projectTokensAmount * invested[round][user]) / _raisedTotal();
    }

    function launchFailed() public view returns (bool) {
        return ((block.timestamp > startTime + duration) && (softCap > _raisedTotal()));
    }
    ...
    function _oneProjectToken() internal view returns (uint256) {
        return 10**projectToken.decimals();
    }
    ...
    function _vestingOver() internal view returns (bool) {
        for (uint256 i = 0; i < ROUNDS; ) {
            address vesting = vestingContracts[i];
            IVesting.VestingSchedule memory sched = IVesting(vesting).settings();
            if (
                startTime +
                    duration +
                    sched.pausePeriod +
                    sched.cliffPeriod +
                    sched.linearUnlockPeriod >
                block.timestamp
            ) return false;
            unchecked {
                ++i;
    ...
    function _idoTokenPrice() internal view returns (uint256) {
        require(block.timestamp > startTime + duration, 'ido not over yet');
        return (raisedTotalCapped() * _oneProjectToken()) / projectTokensAmount;
    }
    ...
    function _userShare(address user) internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < ROUNDS; ) {
            totalUser += invested[i][user];
            unchecked {
                ++i;
            }
        }

        return (totalUser * _oneUsd()) / _raisedTotal();
    }

    function _claimedTotal(address user) internal view returns (uint256) {
        uint256 total;

        for (uint256 i = 0; i < ROUNDS; ) {
            total += IVesting(vestingContracts[i]).claimed(user);
            unchecked {
                ++i;
            }
        }
    ...
    function payBackOnPrice(address investor) external onlyRole(MANAGER_ROLE) {
        ...
        require(!_vestingOver(), 'vesting already over');
        ...
        require((listedStable || listedNative), 'pair not listed on dex');
        ...
        uint256 currentPrice = listedNative
            ...
            : dexRouter.price(address(stableToken), address(projectToken));

        require((_idoTokenPrice() > currentPrice), 'ido price >= dex price');

        paidBack[investor] = true;

        uint256 claimedInUsd = (_claimedTotal(investor) * _idoTokenPrice()) / _oneUsd();
        uint256 amount = (raisedTotalCapped() * _userShare(investor)) /
            _oneUsd() -
            claimedInUsd;
        ...
        uint256 returnAmount = (projectTokensAmount * amount) / _raisedTotal();
        projectToken.safeTransfer(projectWallet, returnAmount);
    }
    ...
    function payBackProject() external onlyRole(MANAGER_ROLE) {
        require(launchFailed(), 'softcap met');

        uint256 returnAmount = projectToken.balanceOf(address(this));
        require(returnAmount != 0, 'nothing to pay back');

        emit PayBackProject(returnAmount);
        projectToken.safeTransfer(projectWallet, returnAmount);
    }
    ...
    function airdropOvercap(uint256 size) external onlyRole(MANAGER_ROLE) {
        ...
        for (i = overcapOffset; i < limit; ) {
            stableToken.safeTransfer(
                _investors[i],
                (amount * _userShare(_investors[i])) / _oneUsd()
            );
            unchecked {
                ++i;
            }
...
interface IIqStaking {
    ...
    function userShares() external view returns (UserSharesOutput[] memory);
}
...
interface ILaunchpad2 {
    event Invested(address indexed from, uint8 indexed round, uint256 stableAmount);
    ...
    /// @dev Allows to get the amount of stablecoins raised by the project in a particular round
    /// @param round the investment round
    /// @return stables the amount of stables invested
    function raised(uint256 round) external view returns (uint256 stables);

    /// @dev Allows to get the amount of stablecoins invested by a particular user in the project in a round
    /// @param round the investment round
    /// @param user the investor
    /// @return stables the amount of stablecoins invested
    function invested(uint256 round, address user)
        external
        view
        returns (uint256 stables);

    /// @dev Allows to get vesting contracts
    /// @param round investment round
    /// @return vestingContract the vesting contract
    function vestingContracts(uint256 round)
        external
        view
        returns (address vestingContract);

    /// @dev Allows to get the airdrop contract
    /// @return airdropContract the airdrop contract
    ...
    function payBackProject() external;
    ...
    /// @dev Allows the manager contract to add vesting contracts
    /// @param vestingContracts_ the contracts to add
    function setVestingContracts(address[] calldata vestingContracts_) external;

    /// @dev Allows the manager contract to add the airdrop contract
    /// @param airdropContract_ the airdrop contract
    ...
    function raisedTotalCapped() external view returns (uint256);
...
interface IManager2 {
    event CreateLaunch(address indexed token, address indexed lpad);
    ...
    function project(address token) external view returns (address launchpad);
    ...
    function airdropLogic() external view returns (address);

    function ROUNDS() external view returns (uint8);
    ...
    function projectsRead(uint256 offset, uint256 size)
        external
        view
    ...
    function payBackOnPrice(address token) external;

    function payBack(address token) external;

    function paybackProject(address token) external;

    // launchpad admin only
    function airdropOvercap(address token, uint256 size) external;

    function investorsLength(address token) external view returns (uint256);

    // launchpad admin only
    function setVestingContracts(address token, address[] calldata vestingContracts_)
        external;

    // launchpad admin only
    function changeVestingSettings(
        address token,
        uint8 round,
        IVesting.VestingSchedule calldata
    ) external;
    ...
    function create(
        ...
        IVesting.VestingSchedule[ROUNDS_] calldata sched,
    ...
    // project wallet only
    function airdrop(address token, IIqStaking.UserSharesOutput[] memory receivers)
        external;
}
...
interface IVesting2 {
    struct VestingSchedule {
        uint64 pausePeriod;
        uint64 firstUnlockPerc; // 18 digits
        uint64 cliffPeriod;
        uint64 linearUnlockSpeed; // 18 digits
        uint64 linearUnlockPeriod;
    }
    ...
    function initialize(
        uint8 round_,
        address launchpad_,
        VestingSchedule calldata settings_,
        address router_
    ) external;
    ...
    function changeSettings(VestingSchedule calldata) external;
}

interface IVesting is IVesting2 {
    function settings() external view returns (VestingSchedule calldata);
}
