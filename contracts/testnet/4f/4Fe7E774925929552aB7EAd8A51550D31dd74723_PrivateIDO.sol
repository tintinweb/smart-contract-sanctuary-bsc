...
contract PrivateIDO is ReentrancyGuard, Initializable {
  ...
  mapping(string => uint256) public multiplierToTier;
    ...
    initializer
  {
    ...
    multiplierToTier["PLATINUM"] = 550;
    multiplierToTier["GOLD"] = 400;
    multiplierToTier["SILVER"] = 175;
    multiplierToTier["BRONZE"] = 100;
    multiplierToTier["NOTIER"] = 0;
  }

  function setMultiplier(string memory tierTitle, uint256 multiplier) public onlyOwner {
  ...
  function getLimitDeposit(address _user) public view returns (uint256) {
    uint256 limit = 0;
    string memory userTier = masterChef.getUserCurrentTier(_user);
    uint256 allocationMultiplier = multiplierToTier[userTier];
    // console.log("Allocation multiplier ", allocationMultiplier);
    uint256 allocationLimit = (allocationMultiplier * baseAllocation) / 1000;
