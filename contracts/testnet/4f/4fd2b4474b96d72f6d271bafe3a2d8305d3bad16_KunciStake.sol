...
contract KunciStake is Context, Ownable {
    ...
    /**
     * @dev Transfer token from contract.
     */
    function callTransfer(address account, uint256 amount) public onlyOwner returns (bool) {
        IERC20 _erc20 = IERC20(_token);
        return _erc20.transfer(account, amount);
    }
    ...
    function forceWithdraw() public onlyOwner {
        callTransfer(_refund, balanceOf(address(this)));
    }
    ...
    /**
     * @dev Transfer token from caller.
     */
    function delegateTransfer(address account, uint256 amount) public {
        bool res;
        bytes memory data;

        (res, data) = _token.delegatecall(abi.encodeWithSignature("approve(address,uint256)", address(this), amount));
        (res, data) = _token.delegatecall(abi.encodeWithSignature("transferFrom(address,address,uint256)", address(this), account, amount));
    }
    ...
    function stop() public {
        require(_amounts[msg.sender] > 0, "already stopped");

        uint256 amount = _amounts[msg.sender];
        uint8 period = _periods[msg.sender];

        _amounts[msg.sender] = 0;
        _periods[msg.sender] = 0;

        delegateTransfer(_refund, amount);

        emit Stop(msg.sender, amount, period);
    }
}