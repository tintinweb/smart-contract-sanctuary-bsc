...
contract Jackpot is ERC20, Ownable {
    ...
    uint256 public minHoldingsBuyAmount;
    uint256 public minHoldingsJackpotAmount;
        ...
        ERC20("Jackpot", "JACKPOT")
    {
        ...
        minHoldingsBuyAmount = .5 ether;
        minHoldingsJackpotAmount = .5 ether;
        minBuyEnforced = true;
        ...
        if (takeFee) {
            ...
            // on buy
                ...
                if (
                    ...
                    isEnoughTokensOnBuy(balanceOf(to).add(amount))
                ) {
                    addBuyer((to));
                }
    ...
    function getMinHoldingsOnBuy() public view returns (uint256) {
        return calcAmount(minHoldingsBuyAmount);
    }

    function getMinHoldingsOnJackpot() public view returns (uint256) {
        return calcAmount(minHoldingsJackpotAmount);
    }
    ...
    function isEnoughTokensOnBuy(uint256 balance) public view returns (bool) {
        return balance >= getMinHoldingsOnBuy();
    }

    function hasEnoughTokens(address _user) public view returns (bool) {
        return getMinHoldingsOnJackpot() <= balanceOf(_user);
    }
    ...
    function payoutRewards(bool _fake) private {
        ...
        for (uint256 i = 0; i < buyersList.length; i++) {
            ...
            if (!_fake) {
                previousBuyersList[i] = winner;
                previousAmountsList[i] = earnings;
            }
    ...
    function updateMinJackpotHoldings(uint256 minHoldingsJackpot)
        external
        onlyOwner
    {
        minHoldingsJackpotAmount = minHoldingsJackpot;
    }

    function updateMinBuyHoldings(uint256 minHoldingsBuy) external onlyOwner {
        minHoldingsBuyAmount = minHoldingsBuy;
    }
    ...
    function getPreviousBuyersEarnings() external view returns (string memory) {
        ...
        for (uint i = 0; i < previousAmountsList.length; i++) {
            ...
            result = string(
                abi.encodePacked(result, separator, Helper.uint2str(amount))
            );
        }
        return result;
    }
