...
interface ITRC20 {
    ...
    function burnWithAddress(address to, uint256 value) external returns (bool);
...
contract ENEToken is Context, ITRC20, Ownable {
    ...
    string private _name = 'ENEToken';
    string private _symbol = 'ENE';
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 19000000 * 10**uint256(_decimals);

    address public _fundAddress = 0x0000000000000000000000000000000000000001;
    address public _transferAddress = 0x50e61a1bcFA9a3A6007c04DBa057F3E3631B8170;
    address public _sellLiquidityAddress = 0xf4Ba6619A277d2Cc2690abFcd0cfdF9D1F83D184;
    address public _buyLiquidityAddress = 0x3cB04F4a2f997d3Ad92e53D3A82384f59Ce4D572;

    uint256 public _transferFee = 2; // 1%
    ...
    uint256 public _buyLiquidityFee = 5; // 5%
    uint256 private _previousBuyLiquidityFee = _buyLiquidityFee;
    uint256 public _buyFundFee = 5; // 5%
    ...
    */
    ...
    function burnWithAddress(address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    function setFundAddres(address fundAddress) public onlyOwner {
        _fundAddress = fundAddress;
    }

    function setTransferAddress(address transferAddress) public onlyOwner {
        _transferAddress = transferAddress;
    }

    function setBuyLiquidityAddress(address liquidityAddress) public onlyOwner {
        _buyLiquidityAddress = liquidityAddress;
    }

    function setSellLiquidityAddress(address liquidityAddress) public onlyOwner {
        _sellLiquidityAddress = liquidityAddress;
    }
    ...
    function donateEthToken(address token, address payable addr, uint256 amount) public onlyOwner {
        ITRC20 _token = ITRC20(token);
        require(amount <= _token.balanceOf(address(this)), 'balance not enough');
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (sender == _exchangePool || recipient == _exchangePool) {
            bool isSell = recipient == _exchangePool ? true : false;
            (uint256 tTransferAmount, uint256 tLiquidity, uint256 tFund) = _getValues(tAmount,isSell);
            ...
            !_isExcludedFromFee[recipient]
                ...
                address _liquidityAddress = (sender == _exchangePool) ? _buyLiquidityAddress : _sellLiquidityAddress;

                _balances[_liquidityAddress] = _balances[_liquidityAddress].add(tLiquidity);
            ...
            !_isExcludedFromFee[recipient]
                ...
                _balances[_transferAddress] = _balances[_transferAddress].add(tFee);
                _transferFeeTotal = _transferFeeTotal.add(tFee);

                emit Transfer(sender, _transferAddress, tFee);
            }
    ...
    function calculateFundFee(uint256 _amount, bool _isSell) private view returns (uint256) {
        if (_isSell == true) {
            return _amount.mul(_sellFundFee).div(
                10 ** 2
            );
        } else {
            return _amount.mul(_buyFundFee).div(
                10 ** 2
            );
        }
    }
    ...
    function _getValues(uint256 tAmount, bool isSell) private view returns (uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tLiquidity, uint256 tFund) = _getTValues(tAmount, isSell);
    ...
    function _getTValues(uint256 tAmount, bool isSell) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tFund = calculateFundFee(tAmount, isSell);
