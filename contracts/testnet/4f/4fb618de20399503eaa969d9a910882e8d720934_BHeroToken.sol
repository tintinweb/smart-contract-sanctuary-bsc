...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  /// @custom:oz-upgrades-renamed-from senTokenV1
  IERC20Upgradeable public senToken;

  /// @custom:oz-upgrades-renamed-from isSuperBoxEnabled
  ...
  function processTokenRequests() external {
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      if (available < request.count) {
        ...
        createRandomToken(to, available, rarity, isSuperBox, seed);
        break;
      }
      available -= request.count;
      createRandomToken(to, request.count, rarity, isSuperBox, seed);
      requests.pop();
      if (available == 0) {
        break;
      }
  ...
  function createRandomToken(
    ...
    for (uint256 i = 0; i < count; ++i) {
      ...
      (, details) = design.createRandomToken(tokenSeed, id, rarity, isSuperBox);
      createTokenWithId(to, id, details);
    }
  }
