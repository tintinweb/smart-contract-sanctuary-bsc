...
contract PoolData is GameData{
    ...
    function setWinToken(address _token) public own {
        winToken[_token] = true;
    }
    ...
    function getUserWinNumber() public view returns(uint256[] memory) {
    ...
    function getUserRanking() public view returns(uint256[] memory){
        return userRanking;
    }
    ...
    function setRanking(address user,uint256 _outAmount) public own {
        require(user!=address(0),'Address input Eoff!');
        if(!joinRanking[user]){
            joinRanking[user] = true;
            userWinNumber.push(1);
            userRanking.push(_outAmount);
            ranking.push(user);
        }else{
            userWinNumber[getUserIndex(user)] += 1;
            userRanking[getUserIndex(user)] += _outAmount;
        }
    }



    function getUserIndex(address _user) internal view returns(uint){
        address[] memory _getRankingUserAddr = getRankingUserAddr();
        uint index;
