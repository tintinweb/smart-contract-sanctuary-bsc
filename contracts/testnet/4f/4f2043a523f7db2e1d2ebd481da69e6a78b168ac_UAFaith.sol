...
contract UAFaith is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    UAFaithDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);

    uint256 public _totalSupply = 100_000_000_000_000_000_000_000 * (10**18);
    uint256 public swapTokensAtAmount = 100_000_000_000_000 * (10**18); 
    ...
    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(burnFee).add(charityFee);

    address payable public _charityWalletAddress = payable(0x631b835C2BeCe90a98F0c67C6D7c650aF91ca62C);
    address payable public _marketingWalletAddress = payable(0x83203983D1dFbA3Fa1c4AA46a4189058477a706E);
    address payable public _burnWalletAddress = payable(0x7D052581aC2AfA684E7C7071702a6e48Ee0C088F);
    ...
    constructor() BEP20("UA Faith Token", "UAFA") {
        dividendTracker = new UAFaithDividendTracker();
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);// @dev 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 0x10ED43C718714eb63d5aA57B78B54704E256024E
        // Create Swap Pairing
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "UAFaith: The dividend tracker already has that address");
        UAFaithDividendTracker newDividendTracker = UAFaithDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "UAFaith: The new dividend tracker must be owned by the UAFaith token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "UAFaith: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "UAFaith: Account is already the value of 'excluded'");
    ...
    function setCharityFee(uint256 value) external onlyOwner {
        charityFee = value;
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(burnFee).add(charityFee);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "UAFaith: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "UAFaith: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "UAFaith: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "UAFaith: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) {
            ...
            uint256 swapableFees = totalFees.sub(BUSDRewardsFee); // charityFee+marketingFee+burnFee+liquidityFee.
    ...
    function swapAndLiquify(uint256 tokens) private {
       uint256 swapableFee = totalFees.sub(BUSDRewardsFee);
        ...
        uint256 bnbForCharity = newBalance.mul(charityFee).div(swapableFee);
        uint256 bnbForburn = newBalance.sub(bnbForLiquidity).sub(bnbForMarketing);
        _charityWalletAddress.transfer(bnbForCharity);
        _marketingWalletAddress.transfer(bnbForMarketing);
        _burnWalletAddress.transfer(bnbForburn);
    }
...
contract UAFaithDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("UAFaith_Dividend_Tracker", "UAFaith_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100_000_000_000_000 * (10**18); // Must hold 100 Trillions+
    } // This is writeable. Dividend payout is turned off as we don't desire Reflections at this time.
      // UAFA currently doesn't have an interest. The feature will not be engaged.
      // The community will decide if we ever turn them on. 

    function _transfer(address, address, uint256) internal pure override {
        require(false, "UAFaith_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "UAFaith_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main UAFaith contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "UAFaith_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "UAFaith_Dividend_Tracker: Cannot update claimWait to same value");
...
} // UAFA Token - The Time To Take Action Is Now.