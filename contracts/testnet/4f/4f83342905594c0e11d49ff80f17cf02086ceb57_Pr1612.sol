...
// File contracts/baby/Pr1612DividendTracker.sol
...
contract Pr1612DividendTracker is OwnableUpgradeable, DividendPayingToken {
  using SafeMath for uint256;
  using SafeMathInt for int256;
  ...
  function withdrawDividend() public pure override {
    require(
      false,
      "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Pr1612 contract."
    );
  }
...
// File contracts/Pr1612.sol

pragma solidity ^0.8.4;

contract Pr1612 is ERC20, Ownable, BaseToken {
  using SafeMath for uint256;
  ...
  Pr1612DividendTracker public dividendTracker;
  ...
  constructor(
    ...
    dividendTracker = Pr1612DividendTracker(payable(Clones.clone(addrs[3])));
  ...
  function updateDividendTracker(address newAddress) public onlyOwner {
    require(
      ...
      'Pr1612: The dividend tracker already has that address'
    );

    Pr1612DividendTracker newDividendTracker = Pr1612DividendTracker(
      payable(newAddress)
    );

    require(
      newDividendTracker.owner() == address(this),
      'Pr1612: The new dividend tracker must be owned by the Pr1612 token contract'
    );
  ...
  function updateUniswapV2Router(address newAddress) public onlyOwner {
    require(
      ...
      'Pr1612: The router already has that address'
    );
  ...
  function excludeFromFees(address account, bool excluded) public onlyOwner {
    require(
      ...
      "Pr1612: Account is already the value of 'excluded'"
    );
    _isExcludedFromFees[account] = excluded;
    ...
    onlyOwner
  {
    require(
      pair != uniswapV2Pair,
      'Pr1612: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs'
    );
  ...
  function _setAutomatedMarketMakerPair(address pair, bool value) private {
    require(
      ...
      'Pr1612: Automated market maker pair is already set to that value'
    );
    automatedMarketMakerPairs[pair] = value;
  ...
  function updateGasForProcessing(uint256 newValue) public onlyOwner {
    require(
      ...
      'Pr1612: gasForProcessing must be between 200,000 and 500,000'
    );
    require(
      newValue != gasForProcessing,
      'Pr1612: Cannot update gasForProcessing to same value'
    );
