...
/**
  * @title SourceOne's InterestRateModel Interface
...
// Dependency file: contracts/STokenInterfaces.sol
...
// Dependency file: contracts/SToken.sol

// pragma solidity ^0.5.16;

// import "contracts/ComptrollerInterface.sol";
// import "contracts/STokenInterfaces.sol";
...
/**
 * @title SourceOne's SToken Contract
...
contract SToken is STokenInterface, Exponential, TokenErrorReporter {
    ...
    function initialize(ComptrollerInterface comptroller_,
        ...
        require(accrualBlockNumber == 0 && borrowIndex == 0, "only one market initialization");

        // Set initial exchange rate
        initialExchangeRateMantissa = initialExchangeRateMantissa_;
        require(initialExchangeRateMantissa > 0, "initialExchangeRate must be greater than zero");

        // Set the comptroller
        uint err = _setComptroller(comptroller_);
        require(err == uint(Error.NO_ERROR), "setComptroller");
        ...
        require(err == uint(Error.NO_ERROR), "setInterestRateModel");
    ...
    function balanceOfUnderlying(address owner) external returns (uint) {
        ...
        require(mErr == MathError.NO_ERROR, "balance couldn´t be calculated");
        return balance;
    }
    ...
    function totalBorrowsCurrent() external nonReentrant returns (uint) {
        require(accrueInterest() == uint(Error.NO_ERROR), "accrueInterest");
        return totalBorrows;
    }
    ...
    function accrueInterest() public returns (uint) {
        ...
        require(borrowRateMantissa <= borrowRateMaxMantissa, "high rate");
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        require(vars.mathErr == MathError.NO_ERROR, "EXCHANGE");
        ...
        require(vars.mathErr == MathError.NO_ERROR, "NEW_TOTAL_SUPPLY");

        (vars.mathErr, vars.accountTokensNew) = addUInt(accountTokens[minter], vars.mintTokens);
        require(vars.mathErr == MathError.NO_ERROR, "NEW_ACCOUNT_BALANCE");
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        require(redeemTokensIn == 0 || redeemAmountIn == 0, "TokensIn or AmountIn must be zero");
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        require(vars.mathErr == MathError.NO_ERROR, "NEW_ACCOUNT_BORROW_BALANCE");

        (vars.mathErr, vars.totalBorrowsNew) = subUInt(totalBorrows, vars.actualRepayAmount);
        require(vars.mathErr == MathError.NO_ERROR, "NEW_TOTAL_BALANCE");
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, STokenInterface sTokenCollateral) internal returns (uint, uint) {
        ...
        require(amountSeizeError == uint(Error.NO_ERROR), "AMOUNT_SEIZE");

        /* Revert if borrower collateral token balance < seizeTokens */
        require(sTokenCollateral.balanceOf(borrower) >= seizeTokens, "SEIZE_TOO_MUCH");
        ...
        require(seizeError == uint(Error.NO_ERROR), "seizure failed");
    ...
    function _addReservesFresh(uint addAmount) internal returns (uint, uint) {
        ...
        /* Revert on overflow */
        require(totalReservesNew >= totalReserves, "addreserves overflow");
    ...
    function _setInterestRateModelFresh(InterestRateModel newInterestRateModel) internal returns (uint) {
        ...
        require(newInterestRateModel.isInterestRateModel(), "not comptroller");
...
// Dependency file: contracts/USXControllerInterface.sol
...
contract USXControllerInterface {
    ...
    function _initializeSourceOneUSXState(uint blockNumber) external returns (uint);
    function updateSourceOneUSXMintIndex() external returns (uint);
    function calcDistributeUSXMinterSourceOne(address usxMinter) external returns(uint, uint, uint, uint);
}
...
// import "contracts/USXControllerInterface.sol";
...
contract ComptrollerV1Storage is UnitrollerAdminStorage {
    ...
    struct Market {
        ...
        /// @notice Whether or not this market receives SRC1
        bool isSourceOne;
    }
    ...
    struct SourceOneMarketState {
        /// @notice The market's last updated sourceOneBorrowIndex or sourceOneSupplyIndex
        uint224 index;
    ...
    /// @notice The rate at which the flywheel distributes SRC1, per block
    uint public sourceOneRate;

    /// @notice The portion of sourceOneRate that each market currently receives
    mapping(address => uint) public sourceOneSpeeds;

    /// @notice The SourceOne market supply state for each market
    mapping(address => SourceOneMarketState) public sourceOneSupplyState;

    /// @notice The SourceOne market borrow state for each market
    mapping(address => SourceOneMarketState) public sourceOneBorrowState;

    /// @notice The SourceOne supply index for each market for each supplier as of the last time they accrued SRC1
    mapping(address => mapping(address => uint)) public sourceOneSupplierIndex;

    /// @notice The SourceOne borrow index for each market for each borrower as of the last time they accrued SRC1
    mapping(address => mapping(address => uint)) public sourceOneBorrowerIndex;

    /// @notice The SRC1 accrued but not yet transferred to each user
    mapping(address => uint) public sourceOneAccrued;
    ...
    /// @notice The rate at which the flywheel distributes SRC1 to USX Minters, per block
    uint public sourceOneUSXRate;
}

contract ComptrollerV2Storage is ComptrollerV1Storage {
    /// @notice The rate at which the flywheel distributes SRC1 to USX Vault, per block
    uint public sourceOneUSXVaultRate;
...
contract ComptrollerV5Storage is ComptrollerV4Storage {
    /// @notice The portion of SRC1 that each contributor receives per block
    mapping(address => uint) public sourceOneContributorSpeeds;

    /// @notice Last block at which a contributor's SRC1 rewards have been allocated
