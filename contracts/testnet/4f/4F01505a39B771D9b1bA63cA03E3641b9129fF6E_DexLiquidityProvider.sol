...
interface IDexLiquidityProvider {
    ...
    function settle(string calldata quoteId, address user, address settleAsset, uint256 settleAmount) external payable;
...
contract DexLiquidityProvider is DexBase, IDexLiquidityProvider, Initializable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address owner_, address babToken_, bool babSwitch_, address compensateToken_) public payable initializer {
    ...
    function deleteLiquidityProvider(address lpIn, address lpOut) external onlyOwner {
        require(lpIn != address(0) && lpOut != address(0), "DexLiquidityProvider: address is the zero address");
        require(liquidityProviderPending[lpOut] == 0, "DexLiquidityProvider: there is still pending trasanction.");
        uint256 amt = liquidityProviderSecure[lpOut];
        if (amt > 0) {
            _safeTransferAsset(compensateToken, lpOut, amt);
        }
    ...
    function addSecureFund(address token_, uint256 amount_) external payable nonReentrant onlyLiquidityProvider {
    ...
    function swapRequest(bytes calldata message, bytes calldata signature) external payable nonReentrant isCircuitBreaker {
        ...
        emit QuoteAccepted(msg.sender, quoteId, quoteMap[quoteId]);
    }

    function settle(string calldata quoteId, address user, address settleAsset, uint256 settleAmount) external payable nonReentrant onlyLiquidityProvider {
        ...
        require(quoteMap[quoteId].status == 0, "settlement error, status is not pending");
        ...
        _safeTransferAsset(quoteMap[quoteId].fromAsset, quoteMap[quoteId].lpIn, quoteMap[quoteId].fromAmount);
    ...
    function decline(string calldata quoteId, address user, address asset, uint256 fromAmount) external nonReentrant onlyLiquidityProvider {
        ...
        _safeTransferAsset(compensateToken, user, quoteMap[quoteId].compensateAmount);
    ...
    function calculateCompensate(address user_, string[] memory idArray) public view returns (uint256) {
        ...
        for (uint i = 0; i < idArray.length; i++) {
            ...
            if (quoteMap[id].user != address(0) && quoteMap[id].user == user_) {
                if (block.timestamp > quoteMap[id].tradeExpireAt && quoteMap[id].status == 0) {
    ...
    function compensate(string[] memory idArray) external nonReentrant {
        ...
        for (uint i = 0; i < idArray.length; i++) {
            ...
            if (quoteMap[id].user != address(0) && quoteMap[id].user == msg.sender) {
                if (block.timestamp > quoteMap[id].tradeExpireAt && quoteMap[id].status == 0) {
                    _safeTransferAsset(quoteMap[id].fromAsset, quoteMap[id].user, quoteMap[id].fromAmount);
    ...
    function _changeQuoteExpire(string calldata quoteId, uint expireAt) external {
        quoteMap[quoteId].tradeExpireAt = expireAt;
    }
...
contract DexBase {
    ...
    function addPairWhitelist(address lpOut, address tokenA, address tokenB) external onlyOwner {
        ...
        require(lpOut != address(0), 'DexLiquidityProvider: zero address');
        require(whitelistPairToIndex[lpOut][token0][token1] == 0, 'DexLiquidityProvider: pair exists'); // single check is sufficient
    ...
    function removePairWhitelist(address lpOut, address tokenA, address tokenB) external onlyOwner {
        require(tokenA != tokenB, 'DexLiquidityProvider: identical token addresses');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        uint256 index = whitelistPairToIndex[lpOut][token0][token1];
