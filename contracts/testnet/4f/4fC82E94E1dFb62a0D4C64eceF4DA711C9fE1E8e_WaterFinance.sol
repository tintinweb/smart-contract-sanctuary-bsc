...
contract WaterFinance is ERC20Detailed, Ownable, MinterRole {
    ...
    function _transferFrom(
        ...
        if(shouldTakeBuyFee(sender,recipient)){
            gonAmountReceived = takeBuyFee(sender, gonAmount);
        }else if(shouldTakeSellFee(sender,recipient)){
    ...
    function shouldTakeBuyFee(address sender, address recipient) internal view returns (bool) {
        return (pair == sender) && (!_isFeeExempt[recipient]) ;
    }

    function shouldTakeSellFee(address sender, address recipient) internal view returns (bool) {
        return (pair == recipient) && (!_isFeeExempt[sender]) ;
    }
