...
contract TimeMachine is Context, ERC20("Water Test", "WT3") {
    ...
    uint256 public constant BLOCK_IN_FOUR_YEAR = 35040;
    
    uint256 public constant BLOCK_PRODUCTION_SECONDS = 3600;
    uint256 public constant BLOCK_START_PRODUCTION_TIMA = 300;
    ...
    function _calculateCurrentBlock(uint256 currentTime) private view returns (uint256) {
        return currentTime.sub(genesisTs).div(BLOCK_PRODUCTION_SECONDS);
    }
    ...
    function _calculateUserMined(uint256 userTcp,uint256 userInBlock,uint256 userOutBlock) public view returns (uint256) {
        uint256 userMined;
        uint256 upBlockTCP=everyBlockTCP[0];
    ...
    function _calculateOneBlockMined(uint256 blockNumber) private pure returns (uint256) {
        ...
        if(blockNumber>=BLOCK_IN_FOUR_YEAR.mul(9) && blockNumber<BLOCK_IN_FOUR_YEAR.mul(9).add(29121)) return BLOCK_START_PRODUCTION_TIMA.mul(1e18).div(512);
        return 0;
    }
    ...
    function claimMintReward() external returns(bool success) {
        ...
        require(outBlock > machineInfo.lastInBlock, "TIMA: no mined");
