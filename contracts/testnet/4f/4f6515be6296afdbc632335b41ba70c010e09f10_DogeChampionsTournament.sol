...
contract DogeChampionsTournament is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, VRFConsumerBaseV2Upgradeable, ITournament, KeeperCompatibleInterface
{
    ...
    /*
     * @author returns tournament related values of given tokenIds
     */
    ...
    {
        if(s_randomWords.length != 12)
        {
            return uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce, (block.timestamp * tokenOrder + hitTurn)))) % 100;
        }
        else
        {
    ...
    function updateRandomWords() internal {
        ...
        for(uint256 i = 0; i < 12; i++) {
            if(shouldAddRandom) {
                s_randomWords[i] += block.timestamp; 
            } else {
                s_randomWords[i] -= block.timestamp;
            }
        }

        shouldAddRandom = !shouldAddRandom;
    }

    /*
     * @author requests random numbers from VRF
     */
    ...
    /*
     * @author creates random seed for randomization
     */
    function createRandomSeed() external onlyOwner {
        uint256 seedSource = s_randomWords[0];
        uint256[] memory randomNumbers = new uint256[](12);
        randomNumbers[0] = seedSource;
        seedSource = seedSource / (block.timestamp % 100);
        for(uint256 i = 1; i < 12; i++) {
            randomNumbers[i] += seedSource + block.timestamp + i;
            seedSource = seedSource / 2;
        }
        s_randomWords = randomNumbers;
    }
