...
contract moneyapp is Ownable,Pausable,ReentrancyGuard{
    ...
    mapping(bytes32 => transactionInfo) public transPoolInfo;
    mapping(bytes32 => userInfo) public userPoolInfo;

    constructor() {
        token = ERC20(0x9EbdCCe6239237fC7C6CD71Cfe06CDA2F3fFFC3A); //ROT Token
    }

    // Token Deposit and Save Transactions Token 
    function userDepositWithdrawToken(uint256 _token,uint256 _id, uint256 _amount, uint256 _types) public whenNotPaused nonReentrant
    {
        bytes32 _value = keccak256(abi.encodePacked(_token,msg.sender, _id,_types));
        bytes32 _userInfo = keccak256(abi.encodePacked(_token,msg.sender));
        if(_types == 1) // Deposit
        {
            require(token.balanceOf(msg.sender) >= _amount,"Not Enough Balance");
            token.transferFrom(msg.sender, address(this), _amount);
            userPoolInfo[_userInfo].totalAmount = userPoolInfo[_userInfo].totalAmount.add(_amount);
        }
        else if (_types == 2) // Withdraw
        {
            require(userPoolInfo[_userInfo].totalAmount >= _amount,"Not Enough Balance");
            token.transfer(msg.sender,_amount);
            userPoolInfo[_userInfo].totalAmount = userPoolInfo[_userInfo].totalAmount.sub(_amount);
        }
