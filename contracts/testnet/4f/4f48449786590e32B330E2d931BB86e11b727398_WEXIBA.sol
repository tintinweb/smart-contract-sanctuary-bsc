...
contract WEXIBA is  ERC20 , Ownable { 
    ...
    uint256 public dailyLimit = 0;
    uint256 public SellingLimit = 0;
    uint256 public buyingLimit = 0;
    ...
    modifier isPanCakeSwap(address sender, address recipient,uint256 amount){
         if (recipient == pancakePair){
            ...
            require(user.dailyBuying + amount <= dailyLimit,"Daily Selling maximum limit reached");
            user.dailySelling += amount;
            require(amount <= buyingLimit,"Amount is greater than Buying limit");
            //selling of token
         }
         ...
         if(sender == pancakePair){
           User storage user =  Users[recipient];
            ...
            require(user.dailySelling + amount <= dailyLimit,"Daily Buying maximum limit reached");
            user.dailySelling += amount;
            require(amount <= SellingLimit,"Amount is greater than Selling limit");
             // buying of token
         }
         _;
    }
   ...
   function burnToken(address _from , uint256 _amount) internal returns(uint256){
    ...
    function transfer(address to, uint256 amount) public virtual override  isPanCakeSwap(_msgSender(),to,amount) returns (bool) {
        address owner = _msgSender();
