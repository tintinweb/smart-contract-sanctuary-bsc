...
contract GoodGame {
    ...
    /*mapping(uint => Table) level1;
    mapping(uint => Table) level2;
    mapping(uint => Table) level3;
    mapping(uint => Table) level4;
    mapping(uint => Table) level5;
    mapping(uint => Table) level6;*/
    ...
    function participate() external {
        ...
        for (uint256 i = 1; i <= levlToAmount[1] + 1; i++) {
            if (levlToIdToTable[1][i].leader == address(0)) {
                ...
                break;
            } else if (levlToIdToTable[1][i].userCount < 6) {
                ...
                break;
            } else if (levlToIdToTable[1][i].isPaidAll == false) {
    ...
    function checkMyData() external view returns (Participant memory) {
        return adrToInfo[msg.sender];
    }
