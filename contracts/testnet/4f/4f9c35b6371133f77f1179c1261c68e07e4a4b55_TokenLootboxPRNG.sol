...
abstract contract LootboxPRNG is LootboxBase {
        ...
        )
    {
        ...
        uint256 chosenRarity = getRandomRarity(getRandomNumber() % 10000, lootboxId);

        _distributeFunds(lootbox.seller, msg.value);

        if (lootbox.rarities[chosenRarity].availableTokens > 0) {
            uint256 chosenToken = (getRandomNumber() %
    ...
    TreasuryNode
{
    ...
    function addToken(
        ...
        if(lootboxToContractToTokenIdToToken[lootboxId][contractAddress][tokenId]==0){
            ...
            Token storage token = lootboxToRarityToTokenIndexToToken[lootboxId][lootboxRarity]
                [lootboxToContractToTokenIdToToken[lootboxId][contractAddress][tokenId]];
            token.amount+=amount;
            for(uint256 i = 0; i< amount; i++) {
                lootbox.rarities[lootboxRarity].availableTokens++;
    ...
    function withdrawToken(
        ...
        Token storage token = lootboxToRarityToTokenIndexToToken[lootboxId][lootboxRarity][lootboxToContractToTokenIdToToken[lootboxId][contractAddress][tokenId]];
