...
contract SuperchargerNFT is ERC721, ERC721Enumerable, Ownable, Mintable {
    ...
    mapping(uint256 => uint256) private superchargerData;
    ...
    constructor(
        ...
        string memory _apiURI,
        address _dependency
    ) ERC721(string(_name), string(_symbol)) {
        require(_dependency != address(0), "zero address");
        baseTokenURI = string(abi.encodePacked(_apiURI, _symbol, "/"));
     ...
     * @dev Set Base URI
     */
    function setBaseURI(string memory _apiURI) external onlyOwner {
        string memory symbol_ = symbol();
        baseTokenURI = string(abi.encodePacked(_apiURI, symbol_, "/"));
    }
    ...
    function mint(
        ...
        require(_tokensToMint > 0, "tokensToMint is zero");
        require(_rarity > 0, "rarity is zero");

        for (uint256 i = 0; i < _tokensToMint; i++) {
            tokenIds.increment();
            uint256 newItemId = tokenIds.current();
            _safeMint(_recipient, newItemId);
            superchargerData[newItemId] = uint256(_rarity);
        }
        return tokenIds.current() - _tokensToMint + 1;
    }

    /**
     * @dev Get Supercharger Data
     */
    function getSuperchargerData(uint256 _tokenId) external view returns (uint256) {
        require(_exists(_tokenId), "nonexistent token");
        return superchargerData[_tokenId];
    }
    ...
    function _beforeTokenTransfer(
        ...
        IMarketManager marketManager = IMarketManager(dependency.getMarketManager());
        require(marketManager.isTransferAllowed(_from, _to, _tokenId), "no transfer allowed");
    }
...
// contracts/archive/utils/Mintable.sol
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.6;
