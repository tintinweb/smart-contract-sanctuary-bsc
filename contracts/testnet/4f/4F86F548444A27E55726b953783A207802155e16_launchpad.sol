...
contract launchpad is Ownable {
    ...
    function unStake(uint _amount) external {
        ...
        for(uint256 i = 0;i<j;i++){
            ...
            // idoBalance[msg.sender] = idoBalance[msg.sender] - (unstakeFrom[i] * idoMulti); 
            // IDO(address(idoToken)).burn(address(this), (unstakeFrom[i] * idoMulti));
        ...
        for(uint256 i = 0;i<jp;i++){
            ...
            tempPenalty += penalty;
            // idoBalance[msg.sender] = idoBalance[msg.sender] - ((unstakeFromPenalty[i] - penalty) * idoMulti); 
            // IDO(address(idoToken)).burn(address(this), ((unstakeFromPenalty[i] - penalty) * idoMulti));
            if (userStake[i].amount == 0) {
                userStake[i].unstakecomp = true;
            }
        }
