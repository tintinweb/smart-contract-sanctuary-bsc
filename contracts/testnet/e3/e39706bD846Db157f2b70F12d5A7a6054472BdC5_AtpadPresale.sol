...
/** 
    @title AtpadPresale this is the version for a single IDO funding, no launch
    @notice This is a Natspec commented contract by AtomPad Development Team
    
    @notice Version v1.1.11 date: mar 12, 2022 - Swappy edition
    this version specifically handles 
        - presale based allocations
        - allocPercentages by interface of iStakePool
        - no distribution of tokens
        - no vesting

/*
    ATPAD IDO 2204 : SWAPPY
    Date : 14 
    Contract : AtpadPresale.sol
    Version: v1.1.11 

        // stakePool Atpad
        // testnet stakePool v1.0.7 - date: dec 16, 2021  - address: 0xbfcf937588F02b9594184882D12C6e3278109Bcf
        // bscMain stakePool v1.0.8 - date: dec 17, 2021  - address: 0xC837D1c05119926D76ef302cd375C06880617216

        // investToken: MTV
        // bscMain MTV v1.0.8 - address: 0x8aa688AB789d1848d131C65D98CEAA8875D97eF1 - decimals:18

        1. constructor parameters
        var _want = 'SWAPPY'; // MTV-chain
        var _invest = 'MTV'; // BSC-chain BEP20

        2. Start sale:
        var _startTime = 1647287700;    // Date and time (GMT): Monday 14 March 2022 19:55:00
        var _duration = 345600;         // 4 day - end Wednesday 19 march 2022 20:00:00

        3. Claim :
        Token claim = 1647806400        // Date and time (GMT): Sunday 20 March 2022 20:00:00

        4. variable settings RATE / CAP
        hardCap = 20_000_000; // mtv tokens --> 200 000 swappy
        rate = 10; //  divide by  (10 ** 3) --> 1 wantToken for 100 investToken


        // Presale v.1.1.11
        - remove depositTokens()
        - remove resetContract()
        - remove getEndVest()
        - remove getStartVest()
        - remove swapClaim()
        - remove _swapClaim()
        - remove returnWantTokens()
        - remove setAllocPercentage()
        - remove getReleasedPercentage()
        - remove getReleasedPercentageFromTable()
        - remove setVest()
        - remove setInitialVestPercentage()
        - remove setWantToken()
        - remove setEnableClaim()
        - remove setSoftCap()
        - remove vesting variables
        - remove parameter softCap

        - add getAllocPoint()
        - add getTotalAllocPoint()
        - add constructor paramater stakePool    


    v.1.1.10
        - decimals of wantToken : decimals 18;
        - decimals of investToken : decimals 6;
        - setter setWantToken(_token, _decimals)
        - setter setInvestToken(_token, _decimals)
        - external swap() calls private _swap() 
        - external swapClaim() calls private _swapClaim() 
        - add fcfsPercentage to increase allocations 100 = 100% for extra round
        - include setter setFcfsPercentage()

    v1.1.9 
        - new routine getReleasedPercentageFromTable()
        - new better routine getReleasedPercentageCalculate() // not tested yet
        - routine swapClaim()()
            remove uint _free = getReleasedPercentage();
            add uint _free = getReleasedPercentageFromTable();   
...
contract AtpadPresale is ReentrancyGuard, Ownable, Pausable {
    ...
    */
    ...
    /// @dev constants
    string public constant name = 'AtpadPresale V1.1.11';
    ...
    uint public rate = 10; //  divide by  (10 ** 3) /  --> 1 wantToken for 100 investToken
...
//    }
    ...
    /// use fscfsPercentage to increase allocations 100 = 100%
    uint fcfsPercentage= 100;  
    ...
    function swap(uint _amount) external payable nonReentrant swapEnabled onProgress returns (bool) {
        ...
        /// check if the hardCap is filled
        require( swapTotal.div(10**(iDecimals)) < hardCap  , "hardCap has exceeded!" );

        /// private call    
        _swap(msg.sender, _amount, _perc);
        
        /// defaults true
        return true;
    }

    function _swap(address _from, uint _amount, uint _perc) private { 

        /// calculate allocation investToken
        uint _allocate = (hardCap * _perc);

        /// if investToken decimals > 6 (allocation percentage was already multiplied by 10**6)
        /// if investToken decimals is smaller than 6, this can lead to inproper rounding 
        if (iDecimals > 6) _allocate = _allocate.mul(10**(iDecimals-6)) ;
        if (iDecimals < 6) _allocate = _allocate.div(10**(iDecimals)) ;

        /// retrieve swapped investToken
        uint _swapped =  swaps[_from];

        /// check the amount for allocation
        require(_allocate - _swapped >= _amount, "!no allocation");

        /// transfer tokens to this stakePool
        investToken.transferFrom(_from, address(this), _amount);

        /// sum swaps
        swaps[_from] += _amount; 

        /// deduct allocation
        allocations[_from] = _allocate - swaps[_from]; 

        /// sum totalizer swapTotal
        swapTotal += _amount; 

        /// check if this is a new user
        if  (_checkOrAddUser(_from)){
            /// this is a new user
            counter++;
        }

        emit Swapped(_from, _amount);
    }
    ...
    function getUserSwapped() view public returns (uint) {
        /// retrieve absolute amount of swapped investToken for this wallet;
    ...
    function getAllocPercentage(address _wallet) view public returns (uint) {
        ...
        uint _allocPerc = (stakePool.allocPercentageOf(_wallet).mul(fcfsPercentage)).div(100);
        return _allocPerc;
    }
    ...
    function setFcfsPercentage(uint8 _fcfsPercentage) external onlyOwner {
        fcfsPercentage = _fcfsPercentage;
    }

    function setInvestToken(address _token, uint8 _decimals) external onlyOwner {
