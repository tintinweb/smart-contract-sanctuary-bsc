...
contract staking is Ownable {
    ...
    constructor() Ownable(0xB1073886DAe5A6369689948fA63b854C9a7294E6){
    ...
    function withdrawRewardBNB()public {
        ...
        investor[msg.sender].totalRewardWithdrawBNB=(investor[msg.sender].totalRewardWithdrawBNB).add(totalRewards);
        for(uint256 i=0;i<investor[msg.sender].deposites.length;i++){
            if(!investor[msg.sender].deposites[i].isToken){
                investor[msg.sender].deposites[i].checkPointBNB=block.timestamp;
            }
...
function transferToTreasury(uint256 amount,bool isToken) public payable{
       if(isToken) token.transfer(treasury, amount);
       else payable(treasury).transfer(amount);
    }
