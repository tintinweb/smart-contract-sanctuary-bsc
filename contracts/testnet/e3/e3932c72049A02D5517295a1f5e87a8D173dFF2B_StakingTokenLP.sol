...
//    address private tokenContract = 0x5fdAb5BDbad5277B383B3482D085f4bFef68828C;
    ...
    function getCurPrice(address _tokenContract)
        public
        view
        returns (uint256)
    {
        ...
        path[1] = bnbContract;
        path[2] = busdContract;
        uint256[] memory _oneTokenWithBusdArr = IPancakeRouter01(pcsRouter).getAmountsOut(1*(10**18), path);
        return _oneTokenWithBusdArr[2];
    }

    function calcReturnedValue(address lpToken)
        public
        view 
        returns(uint256, uint256, uint256)
    {
        uint256 _curPriceToken = getCurPrice(tokenContract);

        uint256 amountToken = IERC20(tokenContract).balanceOf(lpToken);

        uint256 amountBNB =  IERC20(bnbContract).balanceOf(lpToken);
        uint256 bnbPrice = getBnbPrice();
        uint256 _totalValueofLP = ((amountBNB * bnbPrice) + (amountToken * _curPriceToken)) / (10**18);
