...
contract PoolLogic is ERC20ExtendedUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Withdraw(uint256 amountUnit, WithdrawnAsset[] withdrawnAssets, address to);
    ...
    function deposit(address _tokenIn, uint256 _amountIn) public {
        ...
        uint256 investment = __payProtocolFee(IPoolManagerLogic(poolManagerLogic).denominationAsset(), _amountIn);
        uint256 numberOfUnit;
        if (totalUnitBefore > 0) {
            // get investment in USD unit (18 decimals)
            uint256 usdAmountIn = IPoolManagerLogic(poolManagerLogic).getAssetValue(_tokenIn, investment);
            numberOfUnit = usdAmountIn.mul(totalUnitBefore).div(totalValue);
        } else {
            numberOfUnit = investment;
        ...
        emit Deposit(_tokenIn, msg.sender, investment, numberOfUnit);
    }
    ...
    function withdraw(bool _isStablecoin, uint256 _unitOut) public nonReentrant {
        ...
        WithdrawnAsset[] memory withdrawnAssets = __withdraw(
            _isStablecoin,
        ...
        emit Withdraw(_unitOut, withdrawnAssets, msg.sender);
    }
    ...
    function swap(
        ...
        // pay protocol fee
        uint256 amountAfterFee = __payProtocolFee(_tokenIn, _amountIn);

        uint256 amountOut = SwapLib.__swap(
        ...
        __calculatePerformanceFee();
        emit Swap(_tokenIn, amountAfterFee, _tokenOut, amountOut);
    }

    /// @dev calculate performance fee
    function __calculatePerformanceFee() private {
        // if setting performance fee
        if (IFeeManage(poolManagerLogic).performanceFee() != 0) {
            (uint256 newSharePrice, uint256 managerFee) = IFeeManage(poolManagerLogic).availablePerformanceFee(
                sharePriceLatest
            );
    ...
    function redeemFee(bool _isStablecoin) public nonReentrant {
        ...
        address shareOwner = poolManagerLogic;
        address recipent = IPoolManagerLogic(poolManagerLogic).getRecipientWithdrawal(
    ...
    function __mintProtocolFee(
        uint256 feeType,
        uint256 _percent,
        uint256 _amount
    ) internal {
        uint256 fee = _amount.mul(_percent).div(PERCENT_BASE);
        _mint(factory, fee);
    ...
    function __withdraw(
        ...
        uint256 _proportion,
        address _recipient
    ) internal returns (WithdrawnAsset[] memory) {
        uint256 index = 0;
        ...
        uint256 amountStableCoin = 0;

        for (uint256 i = 0; i < assetCount; i++) {
            address asset = assets[i];
            uint256 withdrawnAmount = __calculateNumberOfWithdrawnToken(address(this), asset, _proportion);

            if (withdrawnAmount > 0) {
                if (_isStablecoin) {
                    if (asset != _denominationAsset) {
                        // swap to stable coin
                        withdrawnAmount = SwapLib.__swapForWithdrawal(
                    ...
                    amountStableCoin = amountStableCoin.add(withdrawnAmount);
                } else {
                    // asset in-kind : withdraw token to recipient
                    uint256 amountAfterFee = __payProtocolFee(asset, withdrawnAmount);
                    IERC20Upgradeable(asset).safeTransfer(_recipient, amountAfterFee);

                    withdrawnAssets[index] = WithdrawnAsset({asset: asset, amount: amountAfterFee});
                    index++;
                }
            }
        }
        ...
        if (!_isStablecoin) {
            ...
            uint256 amountAfterFee = __payProtocolFee(_denominationAsset, amountStableCoin);
            IERC20Upgradeable(_denominationAsset).safeTransfer(_recipient, amountAfterFee);
            withdrawnAssets[0] = WithdrawnAsset({asset: _denominationAsset, amount: amountAfterFee});
        }
    ...
    function __calculateNumberOfWithdrawnToken(
        address _pool,
        address _asset,
        uint256 _proportion
    ) internal view returns (uint256 withdrawnAmount_) {
        withdrawnAmount_ = (IERC20Upgradeable(_asset).balanceOf(_pool)).mul(_proportion).div(10**18);
    }

    function __payProtocolFee(address _token, uint256 _amount) internal returns (uint256 investment_) {
        uint256 fee = _amount.mul(IPoolFactory(factory).getProtocolFee()).div(PERCENT_BASE);
        IERC20Upgradeable(_token).safeTransfer(IPoolFactory(factory).getOwner(), fee);
        investment_ = _amount.sub(fee);
    }
}
...
library SwapLib {
    ...
    function __swapForWithdrawal(
        ...
        address[] memory path = __getPathSwap(_wbnb, _tokenIn, _tokenOut);
        uint256[] memory amountOutmin = IPancakeRouter02(_router).getAmountsOut(_amountIn, path);
        if (amountOutmin.length == 0) {
            return 0;
        }

        return __swap(_router, _tokenIn, _amountIn, amountOutmin[amountOutmin.length - 1], path, _to);
    }
