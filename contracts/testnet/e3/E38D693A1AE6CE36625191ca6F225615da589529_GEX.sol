...
* @notice Target supply: 100 million tokens (18 decimals) across all chains.
* Supply can only be minted by supplying collateral to Genesis Liquidity Pools.
* On TGE there is no existing supply.
*/
contract GEX is ERC20ElasticSupply {
    ...
    /// @notice Initializes the GEX token adding the addresses of the contracts
    /// of the pools that can mint it. This function can only be called once
    /// after deployment. Owner can't be a minter.
    /// @dev After initialization the addMinter method must be used
    /// @param poolsMinters array of pools addresses. 
...
* @author Geminon Protocol
* @notice Base implementation of tokens that can be minted and burned by
* whitelisted addresses. New minters can only be added after a 7 days
* period after requesting the addition. The maximum amount that can be
* minted each day is rate limited using a moving average. 
*/
contract ERC20ElasticSupply is IERC20ElasticSupply, ERC20, Ownable, TimeLocks {
    ...
    /// @dev Add minter address. It has a 30 days timelock.
