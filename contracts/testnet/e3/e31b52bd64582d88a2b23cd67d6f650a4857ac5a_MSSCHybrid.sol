...
contract MSSCHybrid is MSSCBase, MembraneAuth, HTLC {
  ...
  //////////////////////////////////////////////////////////////*/
     ...
     */
    function executeInstructions(bytes32 cycleId)
        external
        isNotLocked(cycleId)
    {
        _execute(cycleId);
        ...
        isNotLocked(cycleId)
    {
        ...
        Lock memory lock = _locks[cycleId];

        if (lock.commitment != bytes32(0)) {
            if (
                _cycles[cycleId].instructionsWithDeposits <
                _cycles[cycleId].instructions.length
            ) --_cycles[cycleId].instructionsWithDeposits;
...
abstract contract HTLC {
    uint256 immutable public claimablePeriod;
    uint256 immutable public refundablePeriod;
    ...
    modifier unlock(bytes32 lockId_, string calldata secret) {
        if (_isLocked(lockId_)) {
            if (_hash(secret) != _locks[lockId_].commitment) {
    ...
    function startLock(bytes32 lockId_) internal {
    ...
    function commit(bytes32 lockId_, bytes32 commitment_) internal {
        if (_locks[lockId_].commitment != bytes32(0)) {
    ...
    function _isLocked(bytes32 lockId_) private view returns (bool) {
        ...
        return
            (activeTime % (claimablePeriod + refundablePeriod)) <
            claimablePeriod;
    }
...
/**
 * @dev Revert with an error when a attempting to withdraw locked funds.
 *
 */
error Locked();
