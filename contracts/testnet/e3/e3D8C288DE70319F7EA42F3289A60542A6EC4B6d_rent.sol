...
contract rent is Ownable  {
    ...
    IERC20 public sb = IERC20(0xff8B8aE36b4B436Db4817Bec64Db9D5c14E413Da);

    IERC1155 public market = IERC1155(0x8Ce8C0e6D130e54Ec7c931e300E354D781ab1378); 
    ...
    function SetSB(address addr) external onlyOwner{
        sb = IERC20(addr);
    }
    ...
    function listing(uint256 itemId, uint256 price, uint256 day) external {
        require(itemId>0,"minimum 1");

        require(market.balanceOf(msg.sender, itemId)!=0, "balance 0");

        RentData memory data = RentData(msg.sender, address(0), itemId, price, 0, 0, day, RentCondition.listing);
    ...
    function Bid(address _owner, uint256 id, uint256 amount) external {
        ...
        if(amount > 0) sb.transferFrom(msg.sender, address(this), amount);
    }
    ...
    function Rent(uint256 id, address to, uint256 start, uint256 finish, bytes memory signature) external {
        ...
        for(i=0; i<bidData.length; i++){
            if(bidData[i].from == to){
                if(bidData[i].amount > 0) sb.transfer(msg.sender, bidData[i].amount);
            }else{
                if(bidData[i].amount > 0) sb.transfer(bidData[i].from, bidData[i].amount);
            }
        }

        listRentData[to][data.itemId]++;
    }

    function Claim(address from, uint256 id, uint256 curDate, uint256 itemId, bytes memory signature) external {
        bytes32 message = prefixed(keccak256(abi.encodePacked(id, msg.sender, from, itemId, curDate, this)));

        require(recoverSigner(message, signature) == owner(), "not owner");

        market.safeTransferFrom(address(this), msg.sender, itemId, 1, "");
        ...
        if(listRentData[from][itemId]!=0) listRentData[from][itemId]--; 
    }
    ...
    function CancelList(uint256 id) external {
        ...
        market.safeTransferFrom(address(this), msg.sender, listData[msg.sender][id].itemId, 1, "");
