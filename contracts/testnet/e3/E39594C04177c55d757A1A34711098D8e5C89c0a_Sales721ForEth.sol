...
contract Sales721ForEth is Ownable, ReentrancyGuard {
    ...
    function addTokenIdToSale(uint _saleID,address[] memory _tokenAddresses, uint256[] memory _tokenIDs) external onlyWhitelist{
        ...
        IERC721 tokenAddressCached = IERC721(_tokenAddresses[0]);
    ...
    address public LOOP_DAO;

    function setLoopDaoAddr(address _loop ) public onlyOwner {
        LOOP_DAO = _loop;
    }

    function setFlashSaleByContract(address[] memory _tokenAddresses,
        ...
        uint _startTime,
        uint _duration,
        bool prod) external nonReentrant{

            require(msg.sender == LOOP_DAO, "Transfer: Only OK LOOP_DAO contract");
            setFlashSale(_tokenAddresses, _tokenIDs, _payTokenAddress, _price,  _receiver,  _purchaseLimitation,  _startTime,  _duration,  prod);
        }

    function setFlashSaleByAdmin(address[] memory _tokenAddresses,
        uint[] memory _tokenIDs,
        address _payTokenAddress,
        uint _price,
        address _receiver,
        ...
        uint _startTime,
        uint _duration,
        bool prod) external nonReentrant onlyWhitelist{
    ...
    function setFlashSale(
        ...
        _checkFlashSaleParams(msg.sender, _tokenAddresses[0], _tokenIDs, _price, _startTime, _purchaseLimitation);
        ...
        base : BaseSale({
        seller : msg.sender,
        tokenAddresses : _tokenAddresses[0],
        tokenIDs : _tokenIDs,
