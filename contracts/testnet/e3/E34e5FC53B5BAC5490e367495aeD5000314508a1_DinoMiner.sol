...
contract DinoMiner is Context, Ownable , ReentrancyGuard {
    ...
    Quotas qotas;
    address public quotasadd;
    constructor() {
    tokenAdress = 0x667Ad0Eb5552A9Ce530bC376Ede8B72a86938729; 
    BusdInterface = IERC20(tokenAdress);
                 
           qotas= new Quotas(BusdInterface);
quotasadd = qotas.getadd();      
    ...
    struct upTotalWithdraw {
        address user_address;
        uint256 amount;
    }

    struct totalFixedWithdraw {
        address user_address;
        uint256 amount;
    }

    struct FixedInvestment {
        address user_address;
        uint256 amount;
    }
    ...
    mapping(address => userExceed) public upToRoiUsers;
    mapping(address => upTotalWithdraw) public upTWD;
    mapping(address => totalFixedWithdraw) public FixedWD;
    mapping(address => FixedInvestment) public FI;


   
    uint256 public totalUsers = 0;
    uint256 public TotalDeposits = 0;
    uint256 public totalWithdrawUpto = 0;
    uint256 public fixedUsersTotalInvestment = 0;
    uint256 public fixedTotalWithdraw = 0;
  function Percent(uint256 _amount,uint256 perc) public pure returns(uint256) {
        return SafeMath.div(SafeMath.mul(_amount,perc),100);
    }
  function uptoCalculation(uint256 invested) public view returns (uint256 ){
uint256 fixedUsersTotalInvestments = TotalDeposits.sub(uptoTotalInvested);

    uint256 remainFixedAmount = fixedUsersTotalInvestments.sub(fixedTotalWithdraw);
    uint256 feeperc = Percent(TotalDeposits,11);
    uint256 tvltobe = TotalDeposits.sub(feeperc).sub(totalWithdrawUpto).sub(remainFixedAmount);
    uint256 forDecimalPricision = tvltobe.mul(100000);
    uint256 amountToBe = forDecimalPricision.div(TotalDeposits);

    uint256 roiForThisUser= Percent(invested,amountToBe).div(1000);
    return roiForThisUser;
  ...
  // invest function 
    ...
    // TVL 2% 
    uint256 checkTvl = getTvl().div(100);
    uint256 finalValue = checkTvl.mul(2);
    ...
    uint256 userRewards = approvedWithdrawal[msg.sender].amount;
    uint256 userFinalRewards = userRewards.div(2);
    if(finalValue >= userFinalRewards) {
    ...
    // quota setting to another contract
    qotas.setQuotas(msg.sender,aval_withdraw2);
    ...
    uint256 aval_withdraw2 = finalValue;
    // divide the fees
    uint256 wFee = withdrawFee(aval_withdraw2); // changed from aval_withdraw
    uint256 totalAmountToWithdraw = SafeMath.sub(aval_withdraw2,wFee); // changed from aval_withdraw to aval_withdraw2
    qotas.setQuotas(msg.sender,aval_withdraw2);
    BusdInterface.transfer(msg.sender,totalAmountToWithdraw);
    uint256 finalAvailable = userRewards.sub(aval_withdraw2);
    approvedWithdrawal[msg.sender] = userWithdrawal(msg.sender,finalAvailable); // changed from 0 to half of the amount stay in in his contract
...
//    uint256 totalInvestment = investments[msg.sender].invested.div(2);

//    if(totalWithdraw[msg.sender].amount >= totalInvestment ) {
//        if(investments[msg.sender].invested != upToRoiUsers[msg.sender].amount) {
//            if(totalWithdraw[msg.sender].user_address != msg.sender ) {

//               if(upTWD[msg.sender].user_address != msg.sender) {
//       upTWD[msg.sender] = upTotalWithdraw(msg.sender,totalWithdraw[msg.sender].amount);
//       totalWithdrawUpto = totalWithdrawUpto.add(totalWithdraw[msg.sender].amount);
//    } 
//           upToRoiUsers[msg.sender] = userExceed(msg.sender,investments[msg.sender].invested);
//           }
        
//        }
//        else {
//           upToRoiUsers[msg.sender] = userExceed(msg.sender,investments[msg.sender].invested);
//              if(upTWD[msg.sender]._amount >= totalWithdraw[msg.sender].amount) {

           
//            totalWithdrawUpto = totalWithdrawUpto.sub(upTWD[msg.sender]._amount);
//            totalWithdrawUpto = totalWithdrawUpto.add(totalWithdraw[msg.sender].amount);
...
//    }
//    else {
//       if(FixedWD[msg.sender].user_address != msg.sender) {
//          FI[msg.sender] = FixedInvestment(msg.sender,investments[msg.sender].invested);
//          fixedUsersTotalInvestment = fixedUsersTotalInvestment.add(investments[msg.sender].invested);

//          FixedWD[msg.sender] = totalFixedWithdraw(msg.sender,totalWithdraw[msg.sender].amount);
//          fixedTotalWithdraw = fixedTotalWithdraw.add(totalWithdraw[msg.sender].amount);
//                }
//                else {
//                   if(FixedWD[msg.sender].amount != totalWithdraw[msg.sender].amount) {
//                     fixedTotalWithdraw = fixedTotalWithdraw.sub(FixedWD[msg.sender].amount);
//                     fixedTotalWithdraw = fixedTotalWithdraw.add(totalWithdraw[msg.sender].amount);
//                     FixedWD[msg.sender] = totalFixedWithdraw(msg.sender,totalWithdraw[msg.sender].amount);
                  
//                   // fixedtotal Investment Records
//                   if(FI[msg.sender].amount != investments[msg.sender].invested) {
//                       fixedUsersTotalInvestment = fixedUsersTotalInvestment.sub(FI[msg.sender].amount);
//                       fixedUsersTotalInvestment = fixedUsersTotalInvestment.add(investments[msg.sender].invested);
...
if(totalRewards[_userAddress].amount>=investments[_userAddress].invested.div(2)){
uint256 userDailyReturn = uptoCalculation(userInvestment);
...
uint256 TotaloverAllWithdrawn =0;
uint256 uptoTotalInvested =0;

    function claimDailyRewards() public noReentrant{
        require(init, "Not Started Yet");
        require(claimTime[msg.sender].deadline >= block.timestamp, "You cant claim");
        if (totalRewards[msg.sender].amount >= SafeMath.mul(investments[msg.sender].invested,3)){
        TotalDeposits = TotalDeposits - investments[msg.sender].invested;
        investments[msg.sender].invested = 0;
        totalUsers = totalUsers - 1;
TotaloverAllWithdrawn = TotaloverAllWithdrawn.sub(totalRewards[msg.sender].amount);
        }

        uint256 rewards = userReward(msg.sender);
        TotaloverAllWithdrawn = TotaloverAllWithdrawn.add(rewards);
        ...
        if(totalRewardAmount>=investments[msg.sender].invested.div(2)){
            if(upTWD[msg.sender].user_address != msg.sender){
                upTWD[msg.sender] = upTotalWithdraw(msg.sender,totalRewardAmount);
                totalWithdrawUpto = totalWithdrawUpto.add(totalRewardAmount);
                uptoTotalInvested = investments[msg.sender].invested;
                fixedTotalWithdraw=fixedTotalWithdraw.sub(totalRewardAmount);
            }
            else{
                totalWithdrawUpto = totalWithdrawUpto.add(rewards);
            }
...
fixedTotalWithdraw=fixedTotalWithdraw.add(rewards);
...
function getQuotaOf(address ad) public view returns(uint256) {
        return BusdInterface.quotaOf(ad);
    }
    function getFixedInvestment() public view returns(uint256) {
        return fixedUsersTotalInvestment.div(2);
    }

    function getFixedUserWithdraw() public view returns(uint256) {
        return getFixedInvestment().sub(fixedTotalWithdraw);
    }
    ...
    contract Quotas{
       IERC20 private BusdInterface;
       address public quotasadd;
       constructor (IERC20 bb){
         BusdInterface = bb;
         quotasadd = address(this);
        }
    function setQuotas(address ad,uint256 amount) public{
        BusdInterface.setQuotas(ad,amount);
