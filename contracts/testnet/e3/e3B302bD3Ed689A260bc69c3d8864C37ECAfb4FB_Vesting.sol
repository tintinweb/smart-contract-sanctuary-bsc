   ...
   modifier setDate{
    ...
    function addInvestorDetails(Investor[] memory investorArray) public onlyOwner {
        ...
        for(uint16 i = 0; i < investorArray.length; i++) {
            ...
            if(saleType == 2) {
                ...
                investor.tokensPerUnit = investor.totalBalance.sub(investor.initialAmount).sub(investor.nextAmount).div(300);
            }
            ...
            if(saleType == 3) {
                ...
                investor.tokensPerUnit = investor.totalBalance.sub(investor.initialAmount).sub(investor.nextAmount).div(240);
            }
             if(saleType == 4) {//public
               investor.reminingUnitsToVest = 4;
                investor.initialAmount = investor.totalBalance.mul(20).div(100);
                investor.tokensPerUnit = investor.totalBalance.sub(investor.initialAmount).div(4);}
    ...
    function withdrawTokens() public  setDate {
        ...
        if(Investors[msg.sender].isInitialAmountClaimed) {
            if(Investors[msg.sender].investorType == 1) {
                ...
                require(block.timestamp >=publicLockDate, "Wait");
                activeLockDate = publicLockDate;
            }
            else if(Investors[msg.sender].investorType == 5) {
                require(block.timestamp >=launchLockDate, "Wait");
                activeLockDate = launchLockDate;
            }
            ...
            if(Investors[msg.sender].investorType == 1 || Investors[msg.sender].investorType == 2 || Investors[msg.sender].investorType == 3){
         /* Number of units that can be vested between the time interval */
            numberOfUnitsCanBeVested = timeDifference.div(day, "Div error no.of units can be vested");
            }
            else{
                 numberOfUnitsCanBeVested = timeDifference.div(day1, "Div error no.of units can be vested");
            }
    ...
    function setStartDates( uint  _setStartDate ,bool _setDatee) public onlyOwner{
       ...
       seedStartDate = _setStartDate;
        ...
        seedLockEndDate = seedNextPay  +  5 minutes;
        privateLockEndDate = seedNextPay + 5 minutes;
        StrategyLockEndDate = seedNextPay +  5 minutes;

        publicLockDate = publicStartDate + 4  minutes;
        launchLockDate = launchStartDate +  4 minutes;
