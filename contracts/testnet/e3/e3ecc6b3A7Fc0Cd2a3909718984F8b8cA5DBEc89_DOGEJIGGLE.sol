...
contract DOGEJIGGLE {
    ...
    function enable_rebase(bool option) public{
    ...
    function presale_distribute() public{ //REMEMBER TO CHANGE TO INTERNAL
    uint sum = 0;
    ...
    for(uint j=1;j<=presalersCount;j++){
        ...
        if(share > 1){
        _transfer(address(this),presaleEntry_user[j],balanceOf(address(this))*share/10000);
        }
    }
