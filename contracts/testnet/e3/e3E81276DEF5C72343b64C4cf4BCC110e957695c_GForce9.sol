...
contract GForce9 is Ownable {
  ...
  function printUser(uint256 level, address id) public view returns(string memory) {
    ...
    function upgrade(uint256 level, address userId, bool useGForce) public {
        ...
        if (ProxyMapping[level][level1Upline] != 0x0000000000000000000000000000000000000000) {
            do {
                level1Upline = ProxyMapping[level][level1Upline];
            } while (ProxyMapping[level][level1Upline] != 0x0000000000000000000000000000000000000000);
        }
