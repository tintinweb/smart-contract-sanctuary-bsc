...
contract GameSweepstakes {
   ...
   // Events 
    ...
    event InvestorWithdrawal(address payable addr,uint256 amount, uint256 balance);
  ...
  struct Investment {
    ...
    mapping(uint => mapping(address => uint256)) public winnerAccounts;
     ...
     //stakeholders
     mapping(uint => address[]) internal stakeholders;
     mapping(uint => mapping(address => uint256)) internal _stakes;
     mapping(uint => mapping(address => uint256)) internal stakeholderBalanceOwed;
     mapping(uint =>uint256) internal _totalStakes;
    
    // investors
    mapping(address => Investment[]) public investors;
    mapping(address => uint256) public investorBalanceOwed;
    ...
    function getCurrentDeclaredWinnerTickets() public view returns (uint[] memory){
        ...
        function getWinnerBalanceOwed(uint gameId_,address account_) public view returns (uint256) {
        return winnerAccounts[gameId_][account_];
        }
        ...
        function stakeholderWithdrawFunds(uint amount_) public returns(bool) {  
            require(stakeholderBalanceOwed[gameID][msg.sender] > 0,"Zero funds!"); 
        ...
        return success;
           ...
           // sinvestor withdraw
        function investorWithdrawFunds(uint amount_) public returns(bool) {   
            require(amount_ > 0, "Zero amount!");
            uint256 fee = ((investorFee * amount_)/10000);
            require(investorBalanceOwed[msg.sender] > 0,"Zero funds!");
            require(investorBalanceOwed[msg.sender] >= amount_ + fee, "Insuficient funds!");// guards up front 
            investorBalanceOwed[msg.sender] -= (amount_ + fee);         // optimistic accounting

            // send notification
            emit InvestorWithdrawal(payable(msg.sender), amount_, investorBalanceOwed[msg.sender]);
        ...
        function winnerWithdrawFunds(uint gameId_, uint amount_) public returns(bool) {   
            require(winnerAccounts[gameId_][msg.sender] > 0,"Zero funds!"); 
        ...
        function investorPayment(uint gameid_, uint quantity) external payable {
            //require that the transaction value to the contract is 0.1 ether
            require(Games[gameid_].draw == 0, "Game closed!");   
            require(Games[gameid_].status != 1 , "Game started already!");  
            ...
            require(_stakes[gameid_][msg.sender] == 0 || investors[msg.sender].length > 0,"Already a stakeholder!");
            require((_totalStakes[gameid_] + (quantity * 10**2)) <= (100 * (10**2)),"Max stakes reached, can only be 100%!");

            // create struct
            Investment memory newInvestment;
            newInvestment.id = invId.add(1);
            newInvestment.gameid = gameid_;
            newInvestment.investor = msg.sender;
            newInvestment.amount = msg.value;
            newInvestment.quantity = quantity;
            newInvestment.stake = quantity * 10**2;
            newInvestment.stakeRate = investorPerStakeAmount;
            newInvestment.dateAdded = block.timestamp;
            newInvestment.lastClaimTimeStamp = block.timestamp;
            newInvestment.status = true;
            newInvestment.claimed = false;
            // add to array
            investors[msg.sender].push(newInvestment);
            // add as stakeholder
            if(!stakeholderExists(msg.sender))
             stakeholders[gameid_].push(msg.sender);
            _stakes[gameid_][msg.sender] += quantity;
            _totalStakes[gameid_] +=  quantity * 10**2;
            investorBalanceOwed[msg.sender] += msg.value;
        ...
        function addStakeholder(address account_, uint256 stakes_) public onlyOwner {
            ...
            require((_totalStakes[gameID] + _holderStakes) <= (100 * (10**2)),"Max stakes can only be 100%!");
            if(!stakeholderExists(account_)){
        ...
        function getStakeholderBalanceOwed(uint gameid_, address account_) public view returns (uint256) {
            return stakeholderBalanceOwed[gameid_][account_];
        }
        ...
        function getStakeholders() public view returns (address[] memory) {
         ...
         //get stakeholder exist
        function stakeholderExists(address account_) public view returns (bool result) {    
            for (uint i = 0; i < stakeholders[gameID].length; i++) {
                if(stakeholders[gameID][i] == account_) {
                    result = true;
                }
       ...
       function getInvestorInvestments(address account_) public view returns (Investment[] memory) {
             Investment[] storage investments = investors[account_];
            return investments;
        }
        // get the investor balance owed 
        function getInvestorBalanceOwed(address account_) public view returns (uint256) {
            return investorBalanceOwed[account_];
        }
