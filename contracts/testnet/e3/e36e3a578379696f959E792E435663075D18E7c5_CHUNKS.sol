...
contract CHUNKS is Context, IERC20, Ownable {
    ...
    string private _name = 'CH_V1';
    string private _symbol = 'CH_V1';
    uint8 private _decimals = 18;

    // Buy Fees
    uint256 public _marketingFee = 4;
    uint256 public _liquidityFee = 2;
    uint256 public _liqGOBPoolFee = 1;
    uint256 public _reflectionFee = 1;
    uint256 public _nftHolderFee = 1;
    ...
    uint256 public _sellNftHoldersFee = 1;
    ...
    uint256 public _mainMarketingFee = 0;
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_mainReflectionFee).div(10**2);
    }

    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_mainLiquidityFee).div(10**2);
    }

    function activateSellFee() private {
        _mainReflectionFee = _sellReflectionFee;
        _mainLiquidityFee = _sellLiquidityFee;
        _mainMarketingFee = _sellMarketingFee;
    ...
    function activateBuyFee() private {
        ...
        _mainMarketingFee = _marketingFee;
    ...
    function swapTokensForTokens(uint256 tokenAmount) public {
        // generate the pancake pair path of token -> usdt
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(usdt);
        require(balanceOf(address(this)) >= tokenAmount, 'Error amount');
    ...
    function _tokenTransfer(
        ...
        if (!sellIsActive && sender == uniswapV2Pair) revert SellIsNotActive();
        bool isSell = false;
        bool isBuy = false;
        if (recipient == uniswapV2Pair && !_isExcludedFromFee[sender]) {
            activateSellFee();
            isSell = true;
        } else if (sender == uniswapV2Pair && !_isExcludedFromFee[recipient]) {
            activateBuyFee();
            isBuy = true;
        }
        if (isSell || isBuy) {
            uint256 _marketingAmount = amount.mul(_mainMarketingFee).div(100);
            uint256 _burnAmount = amount.mul(_mainBurnFee).div(100);

            removeAllFee();

            _transferStandard(sender, address(0), _burnAmount);
            _transferStandard(sender, marketingWallet, _marketingAmount);

            if (isBuy) {
                uint256 _GOOBPoolAmount = amount.mul(_liqGOBPoolFee).div(100);
                uint256 _nftHolderAmount = amount.mul(_nftHolderFee).div(100);
                amount = amount.sub(_GOOBPoolAmount).sub(_nftHolderAmount);

                _transferStandard(sender, liqGOBWallet, _GOOBPoolAmount);
                _transferStandard(sender, nftHoldersPoolWallet, _nftHolderAmount);
                activateBuyFee();
            } else {
                uint256 _sellTreasuryAmount = amount.mul(_sellTreasuryFee).div(100);
                uint256 _sellNftHoldersAmount = amount.mul(_sellNftHoldersFee).div(100);
            ...
            amount = amount.sub(_marketingAmount).sub(_burnAmount);
        }
