...
contract TokensGameOriginalTwo2 {
    ...
    uint256 public TOKENS_TO_HATCH_1TOKENRS = 600; //for final version should be seconds in a day
    uint256 PSN = 10000;
    uint256 PSNH = 5000;
...
struct User {
    ...
    function hatchTokens() public {
        ...
        uint256 newTokenrs = SafeMath.div((tokensUsed + bonus), TOKENS_TO_HATCH_1TOKENRS);
    ...
    function getFreeEgs() public {    
        ...
        lastHatch[msg.sender] = now; 
        hatcheryMiners[msg.sender] = SafeMath.add(SafeMath.div(calculateTokensBuySimple(BONUS_10_BUSD),TOKENS_TO_HATCH_1TOKENRS),hatcheryMiners[msg.sender]);
    ...
    function getTokensSinceLastHatch(address adr) public view returns (uint256) {
        uint256 secondsPassed = min(TOKENS_TO_HATCH_1TOKENRS, SafeMath.sub(now, lastHatch[adr]));
