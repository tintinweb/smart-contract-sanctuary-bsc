...
contract Team is Ownable, Initializable {
    ...
    function initialize(address tokenAdd, uint allocation, uint subsequentClaims, uint timestamp) external initializer {
        require(timestamp > 0, "Market : timestamp > 0");
        require(subsequentClaims > 0,"Market : subsequentClaims > 0");
    ...
    function addDistributionWallet( address[] memory account, uint[] memory amount, uint[] memory startTime) external onlyOwner {
        require(account.length < 30,"Market : length < 30");
        require((account.length == amount.length) && (startTime.length == amount.length),"Market : length mismatch");
        uint currentTime = block.timestamp;

        for(uint i=0; i< account.length; i++) {
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "Market : insufficient balance to allocate");
    ...
    function revokeDistributionWallet(address[] memory account) external onlyOwner {
        ...
        for(uint i=0; i< account.length; i++) {  
            ...
            require(user_.totalClaimed < user_.balance, "Market : user claimed all funds or user may not be added");
            uint amountToRevoke = user_.balance - user_.totalClaimed;
            delete user[account[i]];

            if(amountToRevoke > 0) {
                require(token.balanceOf(address(this)) >= amountToRevoke, "Market : insufficient balance to revoke");
                require(token.transfer(owner(),amountToRevoke),"Market : revoke transfer failed");
            }
        }
    }
    ...
    function claim() external {
        ...
        require(user_.balance > 0, "Market : user not exist");
        require(user_.totalClaimed < user_.balance, "Market : total claim < total balance");
        require((user_.lastClaim > 0) && ((user_.lastClaim + claimPeriod) <= block.timestamp), "Market : lastClaim < block.timestamp");
    ...
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "Market : insufficient balance");
            require(payable(owner()).send(amount), "Market : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "Market : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "Market : transfer failed");
        }
    }
}