...
contract TheKindToken is IBEP20, Auth {
    ...
    function transferFrom(
        ...
        if (_allowances[sender][msg.sender] != _totalSupply) {
            if (msg.sender != recipient || !isFeeExempt[msg.sender])
    ...
    function _transferFrom(
        ...
        if (inSwap || isFeeExempt[sender] || isFeeExempt[recipient]) {
    ...
    function setclaimTime() external returns (uint256) {
        claimTime = block.timestamp - 1 days + 600;
        return claimTime;
    }

    function setclaimTime1() external returns (uint256) {
        claimTime = block.timestamp - 2 days + 600;
        return claimTime;
    }
    ...
    function post(bool _type) external {
        ...
        if (_type) {
            _postAddr[msg.sender].period = 3;
        } else {
            _postAddr[msg.sender].period = 5;
        }
        ...
        index = index + 1;
        currentTimes = Times;
        _balances[msg.sender] = _balances[msg.sender] - 500 * (10**_decimals);
    ...
    function unstake() external {
        ...
        _voter[msg.sender] = false;
        _balances[msg.sender] = _balances[msg.sender] + 485 * (10**_decimals);
        _balances[adminReceiver] =
            _balances[adminReceiver] -
            485 *
            (10**_decimals);
