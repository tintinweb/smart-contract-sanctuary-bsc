...
contract AlchemistE is ReentrancyGuardUpgradeable, AccessControlUpgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, IncomingTransferSupportUpgradeable {
    ...
    mapping(uint256 => address) public EOAOf;
    ...
    uint256 constant public ROYALTY = 10*(10**18);
    ...
    function _beforeTokenTransfer(
        ...
        if(from != address(0) && to != address(0) && to != address(0x000000000000000000000000000000000000dEaD) && !to.isContract() && to != EOAOf[tokenId]){
            ...
            if (IERC20TransferHelper(_erc20TransferHelper).isModuleApproved(EOAOf[tokenId]) && allowanceOfErc20TransferHelper >= ROYALTY) {
                _handleIncomingTransfer(ROYALTY, address(_loot));
            } else {
                ...
                _loot.safeTransferFrom(payer, address(this), ROYALTY);
            }
            _loot.safeTransfer(_alchemistDAO, ROYALTY*3/10);
            _loot.safeTransfer(_farmPool, ROYALTY*2/10);
            _loot.safeTransfer(_lootDAO, ROYALTY*5/10);
        }
        if(!to.isContract()){
            EOAOf[tokenId] = to;
        }
