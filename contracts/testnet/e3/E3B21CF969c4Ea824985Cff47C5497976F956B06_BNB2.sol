...
contract BNB2 {
    ...
    function investFromToken(address account, uint256 amount) external {
        ...
        checkIn(account, amount, address(0x0));
    }
    ...
    function invest(address referrer) public payable {
        ...
        checkIn(msg.sender, msg.value, referrer);
    }
    ...
    function reinvest() public {
        ...
        checkIn(msg.sender, totalAmount, address(0));
    }
    ...
    function checkIn(
        ...
        emit FeePayed(account, fee);

        User storage user = users[account];

        if (user.referrer == address(0) && referrer != account) {
        ...
        if (user.referrer != address(0)) {
            ...
            emit RefBonus(user.referrer, account, amount);
        }
