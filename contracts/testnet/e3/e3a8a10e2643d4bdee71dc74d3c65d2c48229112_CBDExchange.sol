...
contract CBDExchange is OrderState, Ownable, Pausable {
    ...
    function unpause() public onlyOwner {
     ...
     function sell(
        ...
        require(order.length == sig.length && buyerFeeSig.length == royaltyFee.length && vendorFee.length == royaltyReceipt.length && isStore.length ==  storeParams.length, "Invalid length");
        for (uint8 i; i < order.length; i++) {
            ...
            validateBuyerFeeSig(order[i], royaltyFee[i], royaltyReceipt[i], buyerFeeSig[i]);
            transferSellFee(
                order[i], 
                payable(royaltyReceipt[i]), 
                isStore[i] ? ICBDVendor(order[i].key.sellAsset.token).vendorWallet() : address(0), 
                royaltyFee[i], 
                vendorFee[i], 
                msg.sender
            );
    ...
    function buy(
        ...
        for (uint8 i; i < order.length; i++) {
            ...
            transferBuyFee(
                order[i], 
                royaltyReceipt[i],
                isStore[i] ? ICBDVendor(order[i].key.buyAsset.token).vendorWallet() : address(0), 
                royaltyFee[i], 
                vendorFee[i], 
                msg.sender
            );
