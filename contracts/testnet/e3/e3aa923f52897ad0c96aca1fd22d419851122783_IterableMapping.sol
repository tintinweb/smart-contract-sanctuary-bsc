...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("_Busd_Dividend_Tracker", "_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 20000000 * (10**18); //must hold 20,000,000 tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Boda contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract CR7 is ERC20, Ownable {
    ...
    uint256 public DevmeFee;
    uint256 public previousDevmeFee ;


    uint256 public totalFees = busdDividendRewardsFee.add(marketingFee).add(liquidityFee).add(DevmeFee);
    
    uint256 public _busdDividendRewardsFeeBuy = 30;
    uint256 public _marketingFeeBuy = 5;
    uint256 public _LpFeeBuy = 5;
    uint256 public _DevmeFeeBuy = 30;

    uint256 public _busdDividendRewardsFeeSell=5;
    uint256 public _LpFeeSell=5;
    uint256 public _marketingFeeSell=5;  
    uint256 public _DevmeFeeSell =5 ;
    ...
    event DevmeEnabledUpdated (bool enabled);
    ...
    event DevmeWalletUpdated (address indexed newDevmeWallet, address indexed oldDevmeWallet);
    ...
    constructor() ERC20("CR7 ULTRA", "CR7 U") {
        	
    	marketingWallet = 0xcFC3f2AF88f46A22C73e950e236C336002a371f2 ; 
        DevmeWallet = 0x7f621c237FD6FB9F7105c4936d882e18855FF1f0;
 ...
 function updateDevmeWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != DevmeWallet, ": The Devme wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit DevmeWalletUpdated(DevmeWallet, _newWallet);
  	 DevmeWallet = _newWallet;
  	}
    ...
    function setDevmeEnabled(bool _enabled) external onlyOwner {
        require(DevmeEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            previousDevmeFee = DevmeFee;
            DevmeFee = 0;
            DevmeEnabled = _enabled;
        } 
 
        emit DevmeEnabledUpdated(_enabled);
    }
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), ": The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), ": The new dividend tracker must be owned by the Boda token contract");
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            tmpDevmeRewardPercent = amount.mul(DevmeFee).div(100);
            ...
            fees = tmpMarketingRewardPercent.add(tmpDevmeRewardPercent).add(tmpBusdDividedRewardsInContract)
            ...
            super._transfer(address(this),DevmeWallet,tmpDevmeRewardPercent);
        }
