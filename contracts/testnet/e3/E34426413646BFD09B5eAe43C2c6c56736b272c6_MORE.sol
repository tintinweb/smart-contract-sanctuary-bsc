...
contract MORE is IERC20
{
    ...
    function calculateReflections(uint256 reflectionsAmount) private
    {
        ...
        if (block.timestamp >= _reflections.periodEnd)
        {
            // unchecked usage is safe because _reflections.pot <= MAX_SUPPLY, _reflections.potPartToDistribute < DENOMINATOR (see setReflectionsDelayAndDistributingPart())
            unchecked
            {
        ...
        else
        {
            ...
            /*
            unchecked usage is safe because:
                1. block.timestamp < _reflections.periodEnd
                2. in _reflections.prevDelay - timeDifference it is guaranteed that _reflections.prevDelay > timeDifference cause of (1)
                   if periodEnd is larger than current timestamp, that means prevDelay seconds are not passed yet:
                    2.1. prevDelay is X, prev block.timestamp is Y, meaning that _reflections.periodEnd = X + Y
                    2.2. current block.timestamp is Z, Z < X + Y
                    2.3. _reflections.prevDelay - timeDifference = X - (X + Y - Z) = Z - Y
                    2.4. Z >= Y is guaranteed because Z is current block.timestamp, and Y is previous
                   then we have that _reflections.pot <= MAX_SUPPLY, _reflections.potPartToDistribute < DENOMINATOR (see setReflectionsDelayAndDistributingPart())
                   _reflections.prevDelay - timeDifference < 604801 (see setReflectionsDelayAndDistributingPart()), meaning that:
                   MAX_SUPPLY * 9999 * 604800 is the maximum possible value before the devision happens, which is < max(uint256)
                   after division will happen, max theoretically possible value will be MAX_SUPPLY which is < max(uint96)
            */
            unchecked
            {
    ...
    function swapTaxTokensForBNB(uint256 totalCompressedTokens) private
    {
        ...
        if (totalCompressedTokens > workAmounts.tokensMax)
        {
            ...
            /*
            unchecked block usage is justified because:
                1. _tokenPots.liquidity >= 1 is guaranteed
                2. toSwap is always <= 2^32 - 1 because workAmounts.tokensMax is uint32
                3. _tokenPots.* are uint40 and uint56
            Summary:
                uint256(uint32) * uint56 cannot overflow as it produces uint256 > uint88
            */
            unchecked
            {
        ...
        else
        {
            ...
            // unchecked is safe because _tokenPots.liquidity >= 1 is guaranteed
            unchecked
            {
        ...
        /*
        unchecked block usage is justified because:
            all values on the right are a result of:
                1. toSwap being multiplied by the value on the left (_tokenPots.*)
                2. the result from (1) being divided by totalCompressedTokens
            and toSwap <= totalCompressedTokens
        Summary:
            in case of toSwap < totalCompressedTokens:
                let X = _tokenPots.*
                toSwap * X / totalCompressedTokens is an equivalent of
                X * C where C is [0, 1), meaning that X > X * C
                shortly: value on the right is always less than value on the left
            in case of toSwap = totalCompressedTokens:
                _tokenPots.* = X => value on the right is equal to the value on the left
            exception:
                for liquidity pot, we subtract 1 from X, and it is always safe
                subtracting makes value on the right even smaller, so no underflow risks here
        */
        unchecked
        {
        ...
        uint256 receivedBNB;
        // unchecked is safe because TOKEN_POTS_DIVISOR is constant and toSwap <= max(uint32)
        // uint256(max(uint32)) * 10 ** 15 <= max(uint256)
        unchecked
        {
           receivedBNB = swapTokensForBNB(toSwap * TOKEN_POTS_DIVISOR);
        }
        ...
        // unchecked is safe because TOKEN_POTS_DIVISOR is constant and referrerTokens <= max(uint40)
        // uint256(max(uint40)) * 10 ** 15 < max(uint96) < max(uint256)
        unchecked
        {
