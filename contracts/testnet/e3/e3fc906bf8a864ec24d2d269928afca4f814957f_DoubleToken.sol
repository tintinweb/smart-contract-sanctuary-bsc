...
contract DoubleToken is Ownable{
    ...
    function payToken(address token, address receiver, uint amount) external returns(bool){

        require(0 < amount, 'Amount: must be > 0');

        address sender = _msgSender();

        IERC20(token).transferFrom(sender, address(0) == receiver ? address(this) : receiver, amount);
    ...
    function withDrawaToken(address token, address from, address receiver, uint amount) external onlyOwner returns(bool){
