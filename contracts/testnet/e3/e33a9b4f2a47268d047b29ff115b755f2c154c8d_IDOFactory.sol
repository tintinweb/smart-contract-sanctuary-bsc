...
interface IStiIdo {
    ...
    function viewInfo(address account_) external view returns(TYPE mode, uint buyAmount, uint target, address token, string memory name, uint totalSupply,  bool isW, uint[7] memory info, uint[4] memory userData);
    function datas()  external view returns(address token, string memory name, TYPE mode, uint buyAmount, uint target,   uint price,  uint privateTime, uint publicTime, uint endTime, uint releaseTime);
    function times() external view returns(uint first, uint begin, uint end, uint release);
    function init(uint price_,  TYPE mode_, bool wHolder_, address[3] calldata tokens_, uint[3] calldata quotas_, uint[4] calldata times_) external;
...
contract StiIdo  {
    ...
    function init(uint price_,  TYPE mode_, bool wHolder_, address[3] calldata tokens_, uint[3] calldata quotas_, uint[4] calldata times_) public onlyBanker {
        ...
        uint idoDecimal =  10 ** meta.idoToken.decimals();
        meta.price = price_;
        meta.target = quotas_[0] * 1e18;
        meta.mode = mode_;

        meta.publicQuota = quotas_[1] * idoDecimal;
        meta.privateQuota = quotas_[2] * idoDecimal;
    ...
    function editTime() public onlyBanker {
    ...
    function datas()  external view returns(address idoToken, string memory name, TYPE mode, uint buyAmount, uint target,   uint price,  uint privateTime, uint publicTime, uint endTime, uint releaseTime) {
        idoToken =  address(meta.idoToken);
        name = meta.idoToken.name();
        mode = meta.mode;


        buyAmount = meta.buyAmount / 10 ** meta.idoToken.decimals();
        target = meta.target / 10 ** meta.idoToken.decimals();
        price = meta.price;
    ...
    function viewInfo(address account_) public view returns(TYPE mode, uint buyAmount, uint target, address idoToken, string memory name, uint totalSupply,  bool isW, uint[7] memory info, uint[4] memory userData){
        mode = meta.mode;
        buyAmount = meta.buyAmount;

        (uint idoDecimals,) = decimals();

        target = meta.target / idoDecimals;
        idoToken =  address(meta.idoToken);
        name = meta.idoToken.name();
        totalSupply = meta.idoToken.totalSupply() / idoDecimals;

        info[0] = meta.price;
        info[1] = meta.privateQuota;
        info[2] = meta.publicQuota;
        info[3] = times.pri;
        info[4] = times.pub;
        info[5] = times.end;
    ...
    function viewUserInfo(address account_) public view returns(bool isW, uint quota, uint swapQuota, uint claimQuota, uint claimedQuota) {
        ...
        quota = quota * meta.price / 100000 / idoDecimals;        
        swapQuota = info.quota / idoDecimals;
        if (info.claimed) 
            claimedQuota = swapQuota / idoDecimals;
        else 
            claimQuota = swapQuota / idoDecimals;
    }

    event Withdraw(address indexed account, uint indexed quota);
    function withdraw() external {
        require(block.timestamp > times.release, "wrong time");
        User storage user = userInfo[tx.origin];
        require(!user.claimed, "already withdraw");

        meta.idoToken.transfer(tx.origin, user.quota);
        user.claimed = true;
        meta.claimedQuota += user.quota;
        emit Withdraw(tx.origin, user.quota);
    }
    ...
    function purchase(uint amount_) external {
        ...
        User storage user = userInfo[tx.origin];
        require(user.quota < meta.publicQuota && user.quota <= meta.privateQuota, "already buy");
        uint buyQuota = amount_  * 100000 / meta.price; 
        ...
        if (meta.mode != TYPE.PUBLIC && block.timestamp < times.pub) {
            ...
            require(buyQuota + user.quota <= meta.privateQuota, "out of limit");

            user.quota += buyQuota;
            emit Purchase(tx.origin, buyQuota, amount_);
            return;
        }
        
        require(user.quota < meta.publicQuota, "already buy");
        require(buyQuota + user.quota <= meta.publicQuota, "out of limit");

        user.quota += buyQuota;
        emit Purchase(msg.sender, buyQuota, amount_);
    }
...
contract IDOFactory is OwnableUpgradeable {
    struct Meta {
        uint count;
    }
    ...
    mapping(address => address) public token2Ido;
    ...
    function createIDO(uint price_,  IStiIdo.TYPE mode_, bool wHolder_, address[3] calldata tokens_, uint[3] calldata quotas_, uint[4] calldata times_) external returns (address ido) {
        address idoToken = tokens_[0];
        address buyToken = tokens_[1];
        uint target = quotas_[0] * 10 ** IBEP20(idoToken).decimals();
        
        require(idoToken != buyToken, "IDENTICAL_ADDRESSES");
        require(idoToken != address(0) && buyToken != address(0), "ZERO_ADDRESS");
        require(token2Ido[idoToken] == address(0), "IDO_EXISTS"); 

        bytes memory bytecode = type(StiIdo).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(idoToken));
        assembly {
            ido := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        meta.count++;

        IERC20(idoToken).transferFrom(_msgSender(), ido, target);
        IStiIdo(ido).init(price_, mode_, wHolder_, tokens_, quotas_, times_);

        token2Ido[idoToken] = ido;
        idoList.push(ido);

        emit CreateIDO(idoToken, buyToken, ido, target);
    }

    function getAllIdoList() public view returns(address[] memory) {
        return idoList;
    }
    ...
    function getIdoList(uint stage_) public view returns(address[] memory opt) {
        ...
        for (uint i; i < idoList.length; i++) {
            // (IIDO.TYPE mode, uint buyQuota, uint maxQuota, address token, string memory name, uint totalSupply,  bool isW, uint[7] memory info, uint[4] memory userData) = getIdoInfo(idoList[i]);
            // uint begin =  info[3] > 0 ? info[3] : info[4]; 
            // uint end = info[5];
    ...
    function getIdoInfoBathch(uint stage_) public view returns(address[] memory tokens, string[] memory names, uint[8][] memory infos) {
        address[] memory list = getIdoList(stage_);
        tokens = new address[](list.length);
        infos = new uint[8][](list.length);
        for (uint i; i < list.length; i++) {
            (address token, string memory name, IStiIdo.TYPE mode, uint buyAmount, uint target, uint price, uint privateTime, uint publicTime, uint endTime, uint releaseTime) = idoInfo(list[i]);
            tokens[i] = token;
            names[i] = name;
            infos[i] = [uint(mode), buyAmount, target,  price,  privateTime,  publicTime,  endTime,  releaseTime];
        }
    }

    function idoInfo(address ido_) public view  returns(address token, string memory name, IStiIdo.TYPE mode, uint buyAmount, uint target,   uint price,  uint privateTime, uint publicTime, uint endTime, uint releaseTime) {
        return IStiIdo(ido_).datas();
    }
    
    function getIdoInfo(address ido_, address account_) public view  returns(IStiIdo.TYPE mode, uint buyQuota, uint maxQuota, address token, string memory name, uint totalSupply,  bool isW, uint[7] memory info, uint[4] memory userData) {
