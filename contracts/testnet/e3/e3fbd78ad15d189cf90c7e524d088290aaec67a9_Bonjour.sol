...
contract Bonjour is ContextUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => uint256) levelTotalgained;
        ...
        if (upliner != 0x0000000000000000000000000000000000000000) {
            ...
            if (_cnt < 8) {
                ...
                if (users[upliner].levelIncomeEligible >= _cnt && checkIsEarn(upliner)) {
                   // uint256 roiAmountEarnings = checkRoiUpto(upliner);
                    uint256 sendAmount = _amount.mul(LEVEL_INCOME[_cnt]).div(100000); // New
                    uint checkdiff =  checkdatediff(users[upliner].levelgainlastDate,block.timestamp);
                    if(checkdiff < 1){
                        users[upliner].levelIncome += sendAmount;
                    }else{
                        uint256 perdaySec = 360;
                        levelTotalgained[upliner] += users[upliner].levelIncome.mul(checkdiff);
                        users[upliner].levelgainlastDate = users[upliner].levelgainlastDate.add(perdaySec.mul(checkdiff));
                        users[upliner].levelIncome += sendAmount;
                    }

                    // uint256 newEarnings = roiAmountEarnings.add(users[upliner].referupto).add(users[upliner].earnedAmount).add(sendAmount);
                    // uint256 maxlimit = users[upliner].investmentAmount.mul(earnLimit).div(100);


                    // if (newEarnings >= maxlimit && upliner != ownerAddress) {
                    //     if (roiAmountEarnings >= maxlimit || levelTotalgained[upliner] > 0) {
                    //         sendAmount = 0;
                    //     } else {
                    //         sendAmount = maxlimit.sub(roiAmountEarnings).sub(users[upliner].earnedAmount).sub(users[upliner].referupto);
                    //     }
                    // }
                   // users[upliner].levelIncome += sendAmount;
    ...
    function checkIsEarn(address _user) public view returns(bool) {
        ...
        uint diff = (endDate - startDate) / 60 / 6;
        ...
        uint checkdiff =  checkdatediff(users[_user].levelgainlastDate,block.timestamp);
    ...
    function checkRoiUpto(address _user) public view returns(uint256){
     ...
     function checkLevelUpto(address _user) public view returns(uint256){
        uint256 startDate = users[_user].levelgainlastDate;
        uint256 endDate = block.timestamp;
        uint diff = (endDate - startDate) / 60 / 6;
    ...
    function getnumberofmins(address _user) public view returns(uint){
      uint256 startDate = users[_user].levelgainlastDate;
      uint256 endDate = block.timestamp;
      uint diff = (endDate - startDate) / 60 / 6;
      return diff;
    ...
    function checkAvailableUpto(address _user) public view returns(uint256){
         ...
         uint256 uptoEarn;
        if (totalearned.add(users[_user].referupto) >= maxlimit) {
