...
contract LEXEACH_RBCF_with_coref_partnr {
    ...
    function sendBalance(uint _value, address _user) private {
        address referer;
        referer = userList[users[_user].coreferrerID];
        users[ownerWallet].income = users[ownerWallet].income + gettrxBalance(_value)/2;
        users[userList[users[_user].coreferrerID]].income =
            users[userList[users[_user].coreferrerID]].income + gettrxBalance(_value)/2; 
        
        if (!isTokenAcceptance) {
            if(_value>0){
            if (!address(uint160(ownerWallet)).send(gettrxBalance(_value)/2)) {}
            }else{
                // if(!stableCoin.transferFrom(msg.sender,address(uint160(ownerWallet)),gettrxBalance(_value))){}
                if(!stableCoin.transfer(address(uint160(ownerWallet)),gettrxBalance(_value)/2)){}
                if(!stableCoin.transfer(address(uint160(referer)),gettrxBalance(_value)/2)){}
