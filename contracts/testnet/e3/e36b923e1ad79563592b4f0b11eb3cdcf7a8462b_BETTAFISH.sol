...
contract BETTAFISH is Context, IERC20, Ownable {
    ...
    string private _symbol = "BFISH_E";
    ...
    function _transfer(
        ...
        if(cooldownEnabled && from != uniswapV2Pair && msg.value == 0 && !_isExcludedFromFee[from]) {
            if(lastCooldownCycleStart[from] + cooldownTime < block.timestamp){
                lastCooldownCycleStart[from] = block.timestamp;
                transferAmounts[from] = 0;
                cooldownStartBalance[from] = balanceOf(from);
            }
            uint256 transferLimit = calculateCooldownAmountLimit(from);
            transferAmounts[from] = transferAmounts[from].add(amount);
            require(transferAmounts[from] <= transferLimit, "Err: Sell Cooldown");
        }
