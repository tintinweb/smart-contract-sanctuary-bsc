...
contract MoreBNB is ERC20, Ownable {
    ...
    uint256 internal constant LAUNCH_TIME = 1658504211;   
    uint256 currentDay;
	...
	function referralUpdate(address _address, uint256 amount) private {
		...
		for(i=0; i < 10; i++) {
			if(_nextReferrer != address(0x0)) 
			{
				...
				if(mapMemberLobby_overallData[_nextReferrer].referrerFund > mapMemberLobby_overallData[topSponsor[0]].referrerFund)
				{
				...
				else if(mapMemberLobby_overallData[_nextReferrer].referrerFund > mapMemberLobby_overallData[topSponsor[1]].referrerFund)
				{
				    topSponsor[3] = topSponsor[2];
					topSponsor[2] = topSponsor[1];
					topSponsor[1] = _nextReferrer;
				}
				else if(mapMemberLobby_overallData[_nextReferrer].referrerFund > mapMemberLobby_overallData[topSponsor[2]].referrerFund)
				{
				    topSponsor[3] = topSponsor[2];
					topSponsor[2] = _nextReferrer;
				}
				else if(mapMemberLobby_overallData[_nextReferrer].referrerFund > mapMemberLobby_overallData[topSponsor[3]].referrerFund)
				{
