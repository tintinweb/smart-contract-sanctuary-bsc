...
contract Market is Ownable {
    ...
    function sellToken(uint256 tokenId, uint256 price, uint32 expireTime) NFTContractMustSet public {
        ...
        totalTokens += 1;
        tokens[tokenId] = TokenSellingInfo(msg.sender, price, block.timestamp + expireTime);
    ...
    function buyToken(uint256 tokenId) NFTContractMustSet public payable {
        TokenSellingInfo memory sellingInfo = tokens[tokenId];
        require(sellingInfo.price > 0, "Token has not been sold");
        require(msg.value >= sellingInfo.price, "Your balance is not enough to buy this token");
        require(block.timestamp <= sellingInfo.expireTime, "This token is not available");
