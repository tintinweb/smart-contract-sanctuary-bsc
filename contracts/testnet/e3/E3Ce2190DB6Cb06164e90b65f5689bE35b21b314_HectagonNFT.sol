...
contract HectagonNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function getTokenIDsPage(
        ...
        uint256 pageEnd = _limit * _pageNumber;
        uint256 pageStart = pageEnd - _limit;

        uint256 tokensSize;

        if (balance < pageStart) {
            tokensSize = 0;
        } else if (pageStart <= balance && balance <= pageEnd) {
            tokensSize = balance - pageStart;
        } else {
            tokensSize = _limit;
        }

        uint256[] memory ids = new uint256[](tokensSize);
