...
* @dev 
* This contract is upgradeable and contains private functions that allow deposits and minting to occur for Lunar NFTs.
*/
contract OrbedNFT is Initializable, ERC721Upgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
  /*
  Includes
  */
  ...
  */
  ...
  /// tracking number of minting done by an account
  ...
  /// NFT URI poiting to metadata storage
  ...
  * @param tokenIds - An array of intergers representing the token Ids that were successfully minted.
  * @param message - Message details of the event.
  */
  event MintCompleted(address indexed toAddress, uint256[] tokenIds, string message);
  ...
  * @param message - Provide the reason for a refund was issued.
  * @param updatedTimestamp - Timestamp to track when a refund is issued.
  */
  event RefundCompleted(address indexed toAddress, string message, uint256 updatedTimestamp);

  /**
  * @notice Triggered when the mint state has been be modified.
  * @dev Custom Event | MintStateUpdated
  * @param mintState - The wallet address from which the event comes from.
  ...
  */
  function initialize(string memory _name, string memory _symbol, uint256 _maxTokenMintable, uint256 _mintPrice, string memory _nonRevealURI,
                      address _adminAddress, address _transAddress, address _liquidityAddress) public initializer
  {
    ...
    _grantRole(TRANSACTION_ROLE, _transAddress);
  ...
  *
  *   The following code is required by the contract specification.
  */
  ...
  */
  ...
  /** @notice Generic Deposit function
    * @dev deposits aganist the current contract and displayed publicly
  */
  function deposit() external payable
  {
    require(msg.value > 0, "You must deposit an amount greater than 0.");
    userBalances[msg.sender] += msg.value;
    //Possible: Add reserving of tokenIds after a deposit has been made. 
  ...
  */
  function withdrawFunds(address payable account) external onlyRole(LIQUIDITY_ROLE)
  {
    (bool sent,) = account.call{ value: address(this).balance }("");
    require(sent, "successfully withdrawn");
  }

  /**
  * @notice updates the NFT URI
  * @dev changes the metadata
  */
  function setBaseURI(string memory _uri) external onlyRole(TRANSACTION_ROLE)
  {
    baseURI = _uri;
  }
  ...
  */
  function plexusMint(address to, uint256[] memory tokenIds) external onlyRole(TRANSACTION_ROLE)  
  {
    //Check if system minting is possible
    _canMint();

    //Check if the user account can mint tokens.
    _canUserMint(to);

    //Private Members
    uint256 tokensMinted = 0; 
    uint256[] memory tokensIdsMinted = new uint256[](tokenIds.length);
    bool canUserResult = false;
    string memory canUserMessage = "";

    //Iterate through the tokens provided as a parameter.
    for (uint256 i = 0; i < tokenIds.length; i++)
    {
      // Make sure the user can mint this token.
      (canUserResult, canUserMessage) = _canUserMintToken(to);

      //If a user can mint an token, call internal mint function.
      if (canUserResult)
      {
        //Setup mint result members
        bool mintResult = false;
        string memory mintMessage = "";
        
        //Subtract cost of a single token. 
        userBalances[to] = userBalances[to] - mintPrice;

        // Mint the token
        (mintResult, mintMessage) = _mintInternal(to, tokenIds[i]);
        if (mintResult) 
        {
          tokensMinted++;
          tokensIdsMinted[i] = tokenIds[i];
        } 
        else 
        {
          console.log("Attempt to mint TokenId %s failed.", tokenIds[i]);
        }
      }
      else 
      {
        //Mint had to stop here, break and return.
        console.log("TokenId %s failed pre-mint checks.", tokenIds[i]);
        break;
      }
    }
    
    //All tokens have been processed.
    console.log("Total Tokens Successfully Minted: %s ", tokensMinted);

    //Check if the user defined tokens have been minted.
    if (tokensMinted > 0 && tokensMinted < tokenIds.length)
    {
      //Calculate the difference to refund.
      uint256 tokensToRefund = tokenIds.length - tokensMinted;
      console.log("Token Amounts to refund : %s ", tokensToRefund);
      uint256 refundTotal = tokensToRefund * mintPrice;
      //update user balance
      console.log("Total Refund amount: %s ", refundTotal);
      userBalances[to] = userBalances[to] + refundTotal;
      //refund difference
      payable(to).transfer(refundTotal);
      canUserMessage = "Not all requested tokens have been minted. We will refund minting cost.";
      emit RefundCompleted(to, canUserMessage, block.timestamp);
    }

    //If no 
    if(tokensMinted <= 0)
    {
      canUserMessage = "The requested tokens are either not available or incorrectly identified.";
    }
    
    // If we weren't able to mint any NFTs, tell them why.
    require(tokensMinted > 0, canUserMessage);

    emit MintCompleted(to, tokensIdsMinted, canUserMessage);
  }
  ...
  * @param newMintState - MintState enumeration values (0-3). 
  */
  function updateMintState(MintState newMintState) external onlyRole(DEFAULT_ADMIN_ROLE)
  {
    mintState = newMintState;
    emit MintStateUpdated(newMintState, block.timestamp);
  }
  ...
  */
  ...
  /** 
  * @notice System wide check to see if minting can occur aganist the contract. 
  * @dev Private check
  */
  ...
  /**
  * @notice Checks if a specific wallet address is allowed mint using the contract.
  * @dev Uses MintState and Whitelist
  * @param to - The wallet address attempting to mint.
  */
  function _canUserMint(address to) internal view
  {
    //Check if a user is on whitelist
  ...
  /**
  * @notice Checks if a given address can mint aganist the contract.
  ...
  * @return message - empty if successful, otherwise comtains a detailed response describing the failure.
  */
  function _canUserMintToken(address to) internal view returns(bool success, string memory message)
  {
      //Check if the user has a balance to mint one (1) token.
      uint256 _mintingCost = mintPrice * 1;
      if (userBalances[to] < _mintingCost)
      {
        return (false, "Wallet deposit balance is insufficent to complete the requested transaction.");
      }

    // Check if user has not exceeded allowed amounts based on the mint state.
    if (mintState == MintState.WHITELIST)
    {
    ...
    //Check if the token supply still exists
  ...
  */
  function _mintInternal(address to, uint256 tokenId) private returns(bool success, string memory message) 
  {
    try this.mintHelper(to, tokenId) {
      ...
      return (false, "The requested NFT was unable to be minted.");
    }
  }
  ...
  */
  function mintHelper(address to, uint256 tokenId) external {
    ...
    require(msg.sender == address(this), "Invalid Contract Caller");
