...
contract Mining {
    ...
    function setDAOTreasuryWallet(address _newDAOTreasuryWallet) external {
    ...
    function setProposal(uint256 percentChange, uint8 action) external {
        require(
            msg.sender == _owner || msg.sender == DAOTreasuryWallet,
            "Ownable: Not owner"
        );
    ...
    function getWDABuyPackageId(uint8 packageId, uint256 nftId)
        public
        view
    ...
    function mint(uint8 _miningType, uint256 _nftId) external {
        ...
        (uint256 finalAmount, uint256 duration) = getWDABuyPackageId(
            _miningType,
            _nftId
        );
