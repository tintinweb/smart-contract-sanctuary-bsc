...
contract GrowthBonding is Initializable, Ownable, ReentrancyGuard
{
	...
	function roundInfoLength() external view returns (uint256 _currentRound)
	{
	...
	function redeem() external nonReentrant
	{
		...
		uint256 _boostAmount = _accountInfo.burned * _roundInfo.boost / _roundInfo.burned;
		uint256 _payoutAmount = _accountInfo.amount * _roundInfo.payout / _roundInfo.amount;
		...
		uint256 _divsAmount = _accountInfo.amount * _roundInfo.reward / _roundInfo.amount;
		...
		emit Redeem(msg.sender, reserveToken, _reserveAmount, rewardToken, _rewardAmount);
	}
	...
	event Redeem(address indexed _account, address indexed _reserveToken, uint256 _reserveAmount, address indexed _rewardToken, uint256 _rewardAmount);
	event Reward(address indexed _account, address indexed _rewardToken, uint256 _amount, uint256 indexed _round);
}
