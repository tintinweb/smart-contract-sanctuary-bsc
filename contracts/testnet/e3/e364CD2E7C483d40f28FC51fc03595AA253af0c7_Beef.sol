...
contract Beef is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20 {
    ...
    string constant _message_link = "http://shrimp.com/msg";
    ...
    mapping(address => bool) public _bountyAddressExists;
    ...
    //Private messages
    bool private _isMessagingEnabled;
    ...
    event MintTo(address adr, uint256 amount);
    ...
    function initialize() public initializer {
        ...
        _sellTimeLimit = 120 seconds;//DAY;
    ...
    function decreaseAllowance(
        ...
        _approve(_msgSender(),spender, (_allowances[_msgSender()][spender] - subtractedValue)); //decreased allowance below zero
        return true;
    }
    ...
    function _getTValues(
        ...
        uint256 tTransferAmount = tAmount - (tFee) - (tLiquidity) - (tBounty);
    ...
    function _getRValues(
        ...
        uint256 rBounty = tBounty * (currentRate);
        uint256 rTransferAmount = rAmount - (rFee) - (rLiquidity) - ( rBounty );
    ...
    function _takeBounty(uint256 tBounty) private {
        uint256 currentRate = _getRate();
        uint256 rBounty = tBounty*(currentRate);
        _rOwned[_bountyAddress] = _rOwned[_bountyAddress]+(rBounty);
        if (_isExcluded[_bountyAddress]) {
            _tOwned[_bountyAddress] = _tOwned[_bountyAddress]+(tBounty);
        }
    ...
    function calculateTaxFee(uint256 _amount) private pure returns (uint256) {
        return _amount*15/10000;
    }

    function calculateBountyFee( uint256 _amount ) private pure returns (uint256) {
        return _amount*5/10000;
    }

    function calculateLiquidityFee( uint256 _amount) private pure returns (uint256) {
        return _amount*80/10000;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(_lockableAccountsProtection){
            checkForLocks(from,to);
        }

        //Anti whale protection
        if(_antiWhaleProtection && !_isExcludedFromAntiWhale[from]){
            if (to != uniswapV2Pair) {
                //buy
                require(balanceOf(to)+(amount) <= _maxWallet, "E200"); //Transfer exceeds max
            } else {
                //sell
                require(_sellIntervalStart[from] < block.timestamp,"Please wait for 1min between two sells");
                _sellIntervalStart[from] = block.timestamp + _sellTimeLimit;
                require(amount < _maxWhaleTxAmount,"Max Whale Limit");
            }
        }
       ...
       //SAL management
        ...
        //if sending account belongs to _isExcludedFromFee account then remove the fee
    ...
    function checkForLocks(address from, address to) virtual public {
      if (_isLocked[from]) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensToAddLiquidityWith = contractTokenBalance*20/80;
        ...
        uint256 BNBToAddLiquidityWith = deltaBalance*20/80;
        ...
        uint256 developerFee = remainingBalance*20/80;
        uint256 ownersFee = remainingBalance*20/80;
        uint256 marketingFee = remainingBalance*10/80;
        uint256 charityFee = remainingBalance*10/80;
    ...
    function mint(uint256 amount) public onlyOwner {
        _mintTo(amount, owner());
    }

    function mintTo(uint256 amount, address receiver) public onlyOwner {
        _mintTo(amount, receiver);
    }

    function _mintTo(uint256 amount, address receiver) private {
        ...
        _tOwned[receiver] = _tOwned[receiver]+(amount);
        rate = _getRate();
        uint256 rAmount = amount*(rate);
        _rOwned[receiver] += rAmount;
    ...
    function multiLock(
        ...
        uint256 defaultLockTime = block.timestamp + 730 days;

         uint256 walLength = wallets.length;

        for (uint256 i; i < walLength; ++i) {
    ...
    function mintDrop(
        ...
        require(walLength == amountsInTokens.length, "E309"); //arrays must be the same length
        require(walLength < 100, "E310"); //Can only airdrop 600 wallets per txn due to gas limits

        for (uint256 i; i < walLength; ++i) {
            _mintTo(amountsInTokens[i], wallets[i]);
        }
    }
    ...
    function setLockableAccountsProtection(bool enabled) external onlyOwner {
        _lockableAccountsProtection = enabled;
    }
    ...
    function increaseLockTime(
        ...
        _lockTime[_lockAccount] = _lockTime[_lockAccount]+(_secondsToIncrease);
    }

    function lockedAccountDetails(
        address account
    ...
    function posMerchantDistribute(
        ...
        uint256 walLen = wallets.length;
        for (uint256 i; i < walLen; ++i) {
            address wallet = wallets[i];
    ...
    function circulatingSupply() public view returns (uint256) {
        uint256 excluded = balanceOf(address(this))+(balanceOf(owner()));
        return _tTotal/(excluded);
    }
    ...
    // Public Messaging for more info goto http://anubit.com/msg
    function enableMessaging(bool enabled) external onlyPos {
    ...
    function getMessageID(address account) public view returns (uint256) {
        return (_msgID[account]);
    }
