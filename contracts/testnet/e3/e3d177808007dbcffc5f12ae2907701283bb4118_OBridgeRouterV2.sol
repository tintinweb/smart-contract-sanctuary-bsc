...
contract OBridgeRouterV2 is Initializable, OwnableUpgradeable {
    ...
    function swapOutEx(uint uID, address token, address to, uint amount) external onlyOwner returns (bool) {
        if(IOBridgeERC20(token).getOrgToken() == wNativeToken) {
            IOBridgeERC20(token).swapOut(uID, to, amount, 0);
        } else {
            IOBridgeERC20(token).swapOut(uID, to, amount, 1);
