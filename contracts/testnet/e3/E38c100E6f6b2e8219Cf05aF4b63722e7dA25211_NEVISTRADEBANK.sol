	...
	function invest(address referrer, uint8 plan,uint256 amount) public {
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 1; i++) {
				if (upline != address(0)) {
					uint256 amountq = amount.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
					users[upline].bonus = users[upline].bonus.add(amountq);
					users[upline].totalBonus = users[upline].totalBonus.add(amountq);
					emit RefBonus(upline, msg.sender, i, amountq);
