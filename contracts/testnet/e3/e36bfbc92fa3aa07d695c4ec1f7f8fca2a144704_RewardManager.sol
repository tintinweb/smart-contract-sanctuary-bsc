...
contract RewardManager is RewardAuth {
  ...
  mapping (address => mapping (uint => uint)) public userDailyRewarded; // TODO remove on next deployment;
...
//    secondInADay = 86400; // 24 * 60 * 60
  ...
  function _validateUser() private view returns (uint) {
    uint dayPassed = getDaysPassed();
    ...
    if (userDailyClaimed[msg.sender][dayPassed] + userMaxReward >= claimCondition.userMaxClaimPerDay) {
      userMaxReward = claimCondition.userMaxClaimPerDay - userDailyClaimed[msg.sender][dayPassed];
    }
