...
contract CriptoCarsEscrow is Initializable, ReentrancyGuardUpgradeable  {
   ...
   function getState(uint _orderId) public view returns (EscrowStatus){
    ...
    function createEscrow(uint _orderId, address payable _buyer, address payable _seller, uint256 _value, IERC20 _currency) external nonReentrant {
        ...
        uint256 _amontTrans = _value+(_value*(feeBuyer/100));

        //Valida el Allowance
        require( _currency.allowance( _buyer, address(this)) >= (_amontTrans),"Seller approve to Escrow first");                      

        //Transfer USDT to contract
        _currency.safeTransferFrom(_buyer, address(this) , (_amontTrans) );
