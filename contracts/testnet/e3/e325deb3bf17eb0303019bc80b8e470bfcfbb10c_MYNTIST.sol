...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    uint256 internal constant LAUNCH_TIME = 1670826600;
...
contract StakeableToken is GlobalsAndUtility {
    ...
    function nftStakeTransfer(uint256 stakeIndex, address transferToAddress,address transferFromAddress)
        external
    {
        require(msg.sender == nftContractAddress,"Not Nft Contract");
        StakeStore[] storage stakeListRef = stakeLists[transferFromAddress];
        StakeStore storage stRef = stakeLists[transferFromAddress][stakeIndex];
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "No Record");
        require(stakeIndex < stakeListRef.length, "invalid index");
        require(stRef.unlockedDay == 0, "Stake Completed");
    ...
    function _stakeStart(
        ...
        */
        ...
        emit StakeStart( // (auto-generated event)
           (stakeLists[msg.sender].length - 1),
            msg.sender,
            newStakeId
        );
