...
contract Payments is Ownable {
    ...
    Employees[] public employeesArray;  //Private in main
    ...
    Bonus[] private bonusArray; //Private in main
    ...
    function editEmployee(uint _index, uint _ID, uint _amount, address payable _addressUser) onlyOwner public {
        employeesArray[_index] = employeesArray[employeesArray.length - 1];
        employeesArray.pop();
        employeesArray.push(Employees(_ID, _amount, _addressUser, Status.Enable));
    }
    ...
    function editBonus(uint _index, uint _ID, uint _amount, address payable _addressUser) onlyOwner public {
        bonusArray[_index] = bonusArray[bonusArray.length - 1];
        bonusArray.pop();
        bonusArray.push(Bonus(_ID, _amount, _addressUser));
    }
    ...
    function getContractBalance() onlyOwner public view returns (uint) {  //También onlyOwner
    ...
    function pays(address payable[] memory _addr, uint[] memory _amount) onlyOwner public payable {
        for (uint256 i = 0; i < _addr.length; i++) {
            
            address payable addrLocal = _addr[i];
            uint amountLocal2 = _amount[i];
