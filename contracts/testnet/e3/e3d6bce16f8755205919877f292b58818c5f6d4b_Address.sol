...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0xB9Be9D8CCEe1795241432df98757744f33394c2f;
...
contract Pegainu is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0xd4FdF3096803C511fD782C0C4b64BB600ad73e44;
    address private _gamerWalletAddress = 0x08D71E2692dB1Ca37c2B08e3d6382EC56B4E4542;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Pega Inu";
    ...
    uint256 public _gamerFee = 40;
    uint256 private _previousGamerFee = _gamerFee;
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
      ...
      function setGamerFeePercent(uint256 gamerFee) external onlyOwner() {
        _gamerFee = gamerFee;
    }
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
     ...
     function _takeGamer(uint256 tGamer) private {
        uint256 currentRate =  _getRate();
        uint256 rGamer = tGamer.mul(currentRate);
        _rOwned[_gamerWalletAddress] = _rOwned[_gamerWalletAddress].add(rGamer);
        if(_isExcluded[_gamerWalletAddress])
            _tOwned[_gamerWalletAddress] = _tOwned[_gamerWalletAddress].add(tGamer);
    }
    ...
    function calculateDevelopmentFee(uint256 _amount) private view returns (uint256) {
       ...
       function calculateGamerFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_gamerFee).div(
            10**3
        );
    }
