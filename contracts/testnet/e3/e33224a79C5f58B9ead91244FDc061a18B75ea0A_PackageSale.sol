...
contract PackageSale {
    ...
    function getMonthlyRewards(uint256 id) external view returns (uint256) {
        if(block.timestamp > _monthlyPurchases[id].nextPayoutTimestamp && _monthlyPurchases[id].payouts < 12) {
            return _monthlyPurchases[id].payoutAmount;
        } else {
            return 0;
        }
    }
    ...
    function getQuarterlyRewards(uint256 id) external view returns (uint256) {
        if(block.timestamp > _quarterlyPurchases[id].nextPayoutTimestamp && _quarterlyPurchases[id].payouts < 4) {
            return _quarterlyPurchases[id].payoutAmount;
        } else {
            return 0;
        }
    }
