...
contract KronosBondDepository is Ownable {
    ...
    address public immutable Kronos; // token given as payment for bond
    ...
    function deposit(
        ...
        if (fee != 0) {
            ...
            IERC20(Kronos).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(Kronos).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(Kronos).approve(stakingHelper, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(Kronos).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(Kronos).totalSupply();
