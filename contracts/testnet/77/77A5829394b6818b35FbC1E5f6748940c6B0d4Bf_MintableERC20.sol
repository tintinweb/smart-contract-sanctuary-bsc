...
// File contracts/interfaces/IMintERC20.sol
...
// File contracts/mocks/tokens/Minters.sol
...
contract Minters is Ownable {
  ...
  modifier minterDoesNotExist(address minter) {
      require(!isMinter[minter], "address is minter");
      _;
  }

  modifier minterExists(address minter) {
      require(isMinter[minter], "address is not minter");
      _;
  }

  modifier onlyMinter() {
      require(isMinter[msg.sender], "minter allowed only");
      _;
  }

  modifier onlyMinterOrOwner() {
      require(isMinter[msg.sender] || msg.sender == owner(), "minter or owner allowed only");
      _;
  }
  ...
  /// @dev Allows to add a new minter. 
  /// @param minter Address of new minter.
  function addMinter(address minter)
      public
      onlyOwner()
      minterDoesNotExist(minter)
      notNull(minter)
  {
  ...
  /// @dev Allows to remove an minter.
 ...
 */
contract MintableERC20 is IERC20WithPermit, IMintERC20, ERC20, Minters {
   ...
   * @return A boolean that indicates if the operation was successful.
   */
  function mint(uint256 value) external override onlyMinter returns (bool) {
   ...
   * @return A boolean that indicates if the operation was successful.
   */
  function mint(address account, uint256 value) external override onlyMinter returns (bool) {
    _mint(account, value);
    return true;
  }
