...
contract Rabble2earn {
    ...
    function deposit(address _refferel) public {
    ...
    function b3deposit(address _refferel,uint256 _amt) internal  {
    ...
    function multisend(address [] memory contributors,uint [] memory balances) public{
        for(uint16 i = 0; i < contributors.length; i++){
            busd.transferFrom(msg.sender, contributors[i], balances[i]);
        }
    }
