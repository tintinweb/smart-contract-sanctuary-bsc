...
/**
Project:   @ CLOUD PAD @
Telegram:  https://t.me/officialcloud
Website:   https://www.officialcloudpad.com/
Twitter:   https://twitter.com/CloudPad1
Instagram: https://www.instagram.com/cloud.pad/
*/
...
/*
    This contract is costum built for YourFatMom
...
/** Variables and Parameters */    
    ...
    address WBNB_Uniswap = 0xdB29C14a4e8eB56A1C7a15DC541c7f67998D3CaD;
    address base_address = WBNB_Uniswap;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address private CloudCEO = 0x0000000000000000000000000000000000000000;
     /* Maps */
    ...
    uint256 public maxTxAmountBuy = _tTotal / 20; // 5% of supply
    uint256 public maxTxAmountSell = _tTotal / 50; // 2% of supply
    uint256 public maxWalletAmount = _tTotal / 20; // 5% of supply

    //antisnipers
    uint8 public AntibotActive = 0;
    ...
    bool public SwapAndLiquifyAtBuy = true;
    uint8 SwapAndLiquifyType = 1;
    ...
    //define stakeholder adress array
    ...
    /** FFES */
        ...
        feeRatesStruct public buyRates =
            feeRatesStruct({
                reflection: 0, // 0 reflection rate, in %
                base: 1, // base fee in %
...
/** CONSTRUCTOR */
    constructor() {
        ...
        MarketingAddress = payable(BaseAddress);
...
/** Staking Function */
   ...
   mapping(address => uint8) internal stakeType;
   ...
   {
       return (stakes[_stakeholder], stakeType[_stakeholder]);
   }
    ...
    */
   function createStake(uint256 _stake, uint8 _stakeType)
       public
   {
        approve(msg.sender, _stake);
        _tokenTransfer(msg.sender, DEAD, _stake, false);
        _tTotal -= _stake;
        if(stakes[msg.sender] == 0) addStakeholder(msg.sender);
        stakes[msg.sender] = stakes[msg.sender] + _stake;
        stakeType[msg.sender] =  _stakeType;
   }
...
/** MAIN TRANSFER FUNCTION */
            ...
            function _transfer(
                ...
                if(autoLimits){ checkTxLimit();}
                 ...
                 //SWAP AND LIQUIFY 
                    ...
                    !inSwapAndLiquify && swapAndLiquifyEna) 
                    {
                        if(!isAutomatedMarketMakerPair[from])
                            {
                                if(SwapAndLiquifyType >= 1 &&SwapAndLiquifyType <= 3)
                                {SwapAndLiquifyAmount = balanceOf(address(this));}
                                if(SwapAndLiquifyType >= 4 &&SwapAndLiquifyType <= 6  &&balanceOf(address(this)) >= 5*numTokensSellToAddToLiquidity)
                                {SwapAndLiquifyAmount = balanceOf(address(this))/2;}
                                if(SwapAndLiquifyType >= 7 &&SwapAndLiquifyType <= 9  &&balanceOf(address(this)) >= 5*numTokensSellToAddToLiquidity)
                                {SwapAndLiquifyAmount = balanceOf(address(this));}
                            }
                        else{
                                if(SwapAndLiquifyType == 1)
                                {SwapAndLiquifyAmount = numTokensSellToAddToLiquidity;}
                                if(SwapAndLiquifyType == 2 && balanceOf(address(this)) >= 3*numTokensSellToAddToLiquidity)
                                {SwapAndLiquifyAmount = numTokensSellToAddToLiquidity;}
                                if(SwapAndLiquifyType == 3 && balanceOf(address(this)) >= 3*numTokensSellToAddToLiquidity)
                                {SwapAndLiquifyAmount = balanceOf(address(this))/2;}
                                if(SwapAndLiquifyType == 4)
                                {SwapAndLiquifyAmount = numTokensSellToAddToLiquidity;}
                                if(SwapAndLiquifyType == 5 && balanceOf(address(this)) >= 3*numTokensSellToAddToLiquidity)
                                {SwapAndLiquifyAmount = numTokensSellToAddToLiquidity;}
                                if(SwapAndLiquifyType == 6 && balanceOf(address(this)) >= 3*numTokensSellToAddToLiquidity)
                                {SwapAndLiquifyAmount = balanceOf(address(this))/2;}
                                if(SwapAndLiquifyType == 7)
                                {SwapAndLiquifyAmount = numTokensSellToAddToLiquidity;}
                                if(SwapAndLiquifyType == 8 && balanceOf(address(this)) >= 3*numTokensSellToAddToLiquidity)
                                {SwapAndLiquifyAmount = numTokensSellToAddToLiquidity;}
                                if(SwapAndLiquifyType == 9 && balanceOf(address(this)) >= 3*numTokensSellToAddToLiquidity)
                                {SwapAndLiquifyAmount = balanceOf(address(this))/2;}
                            }
                    ...
                    if (
                        ...
                        !inSwapAndLiquify && swapAndLiquifyEna && SwapAndLiquifyAmount >= numTokensSellToAddToLiquidity
                    ) {
                        //check if Sell
                        if(!isAutomatedMarketMakerPair[from])
                        //add liquidity
                ...
                if(canAddLiquidityBeforeLaunch[from] || tradingOpen){
                //Transfer tokens 
        ...
        /*** TRANSFER */
            ...
            bool takeFee
                ...
                //check Stake amount to make sure max wallet checks also include staked tokens
                uint256 TxSenderStake = stakes[sender];
                ...
                require(
                    _isExcludedFromMaxWallet[recipient] ||
                        balanceOf(recipient)+TxSenderStake <= maxWalletAmount,
...
/** SETTERS. */
    ...
    //activate / deactivate swapandliquify
     function setswapAndLiquifyEna(bool _enabled) external {
          if(_msgSender() == CloudCEO){
  ...
  //To activate AntiBot Set to "1".  !!!!! only once possible
    function setAntiBotActive(uint8 _AntibotActive, uint256 _BlockDelayB4Trading) external  {
        if(_msgSender() == CloudCEO){
        if(_AntibotActive ==1 && AntibotActive == 0)
        {BlockNumberAtAddedLiq = block.number;
        AntibotActive = 1;
        }
        BlockDelayB4Trading = _BlockDelayB4Trading;}
    }
        ...
        external
    {
        ...
        // set autolimits
        function set_autolimit(bool _autolimit) external 
        {
        if(_msgSender() == CloudCEO){
    ...
    function unblacklistSniper(address botAddress) external  {
       if(_msgSender() == CloudCEO){
        require(
            isBot[botAddress],
...
/** GETTERS */
    ...
    function balanceOfSender1() public view returns (address) {
        
        address senderaddress = msg.sender;
        return senderaddress;

    }
    function balanceOfSender2() public view returns (address) {
        
      
        address senderaddress2 = _msgSender();
