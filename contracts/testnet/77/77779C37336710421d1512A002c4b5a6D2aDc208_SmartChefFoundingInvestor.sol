...
contract SmartChefFoundingInvestor is Ownable, ReentrancyGuard {
    ...
    // The block number when MOVE mining ends.
    uint256 public bonusEndBlock;

    // The block number when MOVE mining starts.
    uint256 public startBlock;

    // The block number when MOVE unlocking starts.
    uint256 public startUnlockBlock;

    // The block number when MOVE unlocking ends.
     ...
     */
    function updateStartUnlockAndEndUnlockBlocks(uint256 _startUnlockBlock, uint256 _endUnlockBlock)
        external
        onlyOwner
    {
...
contract SmartChefFoundingInvestorTreasury is Ownable {
    ...
    function safeRewardTransfer(address _to, uint256 _amount) public onlyAdmin {
        uint256 moveBalance = move.balanceOf(address(this));
        if (_amount > moveBalance) {
            move.transfer(_to, moveBalance);
        } else {
            move.transfer(_to, _amount);
        }
    }
