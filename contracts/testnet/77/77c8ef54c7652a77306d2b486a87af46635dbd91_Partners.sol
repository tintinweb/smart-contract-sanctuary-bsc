    ...
    OwnableUpgradeable
{
    ...
    address private _tokenHubAddress;
    ...
    function initialize(
        ...
        __Ownable_init();
        _operator = operator;
        _tokenHubAddress = tokenHub;
        percentOfDailyRewards = percent;
    }

    function addPartner(address partnerAddress, uint256 code)
        external
        onlyOwner
    {
    ...
    function getPartnerCodeByAddress(address partnerAddress)
        external
        view
        returns (uint256)
    {
    ...
    function getPartnerAddressByCode(uint256 code)
        external
        view
        returns (address)
    {
    ...
    function updatePartnerAddress(uint256 code, address newAddress)
        external
        onlyOwner
    {
    ...
    function addRewardsFor(uint256 amount, uint256 code)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function withdrawOperatorCommission() external nonReentrant onlyOperator {
        uint256 commission = _operatorCommission;
        require(commission > 0, "has not got commission");
        _operatorCommission = 0;
        address payable wallet = payable(_operator);
        bool success;
        assembly {
    ...
    function changeTokenHubAddress(address tokenHub) external onlyOwner {
        ...
        _tokenHubAddress = tokenHub;
        emit TokenHubAddressChanged(tokenHub);
    }

    receive() external payable {
        if (msg.sender == _tokenHubAddress) {
