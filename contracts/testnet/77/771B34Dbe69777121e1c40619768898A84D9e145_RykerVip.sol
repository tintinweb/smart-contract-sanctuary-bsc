...
contract RykerVip {
    ...
    function calculateDailyEarnings() public view returns(uint256){
        uint256 dayInseconds = 24 hours;
        
       return calculateEggSellForYield(users[msg.sender].miners.mul(dayInseconds),1);
    }
    function calculateDailyEarningsFromFixedYield() public view  returns(uint256){
         User storage user = users[msg.sender];
         if(user.initialDeposit>0){
             return user.initialDeposit.mul(FIXED_YIELD_INCOME_PRCT).div(PERCENTS_DIVIDER);
         }
    }
