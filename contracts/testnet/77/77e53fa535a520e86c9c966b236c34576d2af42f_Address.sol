...
/**
 * CRO is the home of the Jeets.
 * Together we make BSC Rebord again.
 *
 * Anti Jeet Tax system! First 24 Hours Tax will be 20% (Tax is hard coded to never be above 20%)
 */
...
abstract contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(owner() == _msgSender(), "[reborn][ownable] caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "[reborn][ownable] new owner is the zero address");
...
contract Reborn is Context, IERC20, Ownable{
    ...
    // addresses
        ...
        address public ADDRESS_BURN = 0x000000000000000000000000000000000000dEaD; // Burn Address
        address public ADDRESS_ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // Swap Router Address
        address public ADDRESS_ERC20_USDC = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d; // USDC Address
        address public ADDRESS_MARKETING = 0x063Cc056CC5Aaac3742fF9f9D51D7Ce47749F04E; // Marketing Wallet
        address public ADDRESS_DEVELOPER = 0x42069Af5D0b09977D1e9d52825A9ceB06fDEbBed; // Dev Wallet
        address public ADDRESS_BUYBACK = 0xDC76F15AC4Ce0c139Aa384c65892c0114Bf931f2; // Buyback and Burn Wallet
    ...
    // taxes
        ...
        uint16 public TAX_BUY_MARKETING = 50; // 5% Marketing Tax (BUY)
        uint16 public TAX_BUY_DEVELOPER = 40; // 4% Dev Tax (BUY)
        uint16 public TAX_BUY_BUYBACK = 0; // 0% Buyback Tax (BUY)

        uint16 public TAX_SELL_MARKETING = 50; // 5% Marketing Tax (SELL)
        uint16 public TAX_SELL_DEVELOPER = 40; // 4% Dev Tax (SELL)
        uint16 public TAX_SELL_BUYBACK = 10; // 1% Buyback and Burn Tax (SELL)

        uint16 public TAX24H_SELL_MARKETING = 160; // First 24 Hour 16% Marketing (SELL)
        uint16 public TAX24H_SELL_DEVELOPER = 30; // First 24 Hour 3% Dev Tax (SELL)
        uint16 public TAX24H_SELL_BUYBACK = 10; // First 24 Hour 1% Buyback and Burn Tax (SELL)

    // limits
        uint16 public LIMIT_WALLET = 20; // 2% Max Wallet Limit
        uint16 public LIMIT_TRANSACTION = 10; // 1% Max Transaction Limit
        uint32 public LIMIT_MCAP = 10000; // Limits Removed at 10,000 Market Cap
        bool public LIMIT_ACTIVE;
        bool public LIMIT24H_ACTIVE;
        uint256 public LIMIT24H_END;

    // tokenomics
        string private _name = "Space Reborn";
        string private _symbol = "REBORN";
        uint8 private _decimals = 9;
        uint256 private _totalSupply = 1000000000 * (10**9); // 1,000,000,000 (1 Billion Total Supply)
    ...
    // modifiers
        modifier onlyAuthorized(){
            require(_addressAuthorized[_msgSender()], "[reborn] only authorized wallets can call this function!");
            _;
        }
    ...
    function addLiquidity() public onlyAuthorized{
        require(!_liquidityProvided, "[reborn] liquidity can only be enabled once!");
        _addLiquidity();
    }

    function enableTrading() public onlyAuthorized{
        require(!_tradingEnabled, "[reborn] trading can only be enabled and not disabled!");
    ...
    function setTaxes(uint16 pMarketing, uint16 pDeveloper, uint16 pBuyback, bool pBuy) public onlyAuthorized{
        require((pMarketing + pDeveloper + pBuyback) <= 200, "[reborn] sum of all taxes cant exceed 20%!");
    ...
    function send(address pTo, uint256 pAmount) public onlyAuthorized{
        require(pAmount <= (balanceOf(address(this)).sub(TAXES_MARKETING.add(TAXES_DEVELOPER).add(TAXES_BUYBACK))), "[reborn] you cant send more tokens than the contract has!");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "[reborn] approve from the zero address");
        require(spender != address(0), "[reborn] approve to the zero address");
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool){
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "[reborn] decreased allowance below zero"));
        return(true);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns(bool){
        _transfer(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "[reborn] transfer amount exceeds allowance"));
        return(true);
    }
    ...
    function _transfer(address pFrom, address pTo, uint256 pAmount) private{
        ...
        if(_addressNoTaxes[pFrom] || _addressNoTaxes[pTo]){
            ...
            if(pFrom == ADDRESS_PAIR){
                require(_tradingEnabled, "[reborn] trading not enabled yet, stay tuned!");
                if(LIMIT_ACTIVE && !_addressNoLimits[pTo]){
                    require(pAmount <= limitWalletTokens && _balances[pTo].add(pAmount) <= limitWalletTokens, "[reborn] wallet limit reached, cant buy more tokens!");
                    require(pAmount <= limitTransactionTokens, "[reborn] transaction limit reached, cant buy more tokens!");
                }
            ...
            if(!Tax.buy && !Tax.sell){
                if(LIMIT_ACTIVE && !_addressNoLimits[pTo]){
                    require(pAmount <= limitWalletTokens && _balances[pTo].add(pAmount) <= limitWalletTokens, "[reborn] wallet limit reached, cant transfer more tokens!");
                }
    ...
    function _addLiquidity() private{
        ...
        require(balance > 0 && _balances[address(this)] >= liquidityForUniswap, "[reborn] you do not have any coins or tokens to provide liquidity!");
