...
contract World is ERC20Burnable, Operator {
    using SafeMath for uint256;

    // TOTAL MAX SUPPLY = 75,000 world
    uint256 public constant FARMING_POOL_REWARD_ALLOCATION = 52500 ether;
    ...
    uint256 public constant DAO_ALLOCATION = 0 ether;

    uint256 public constant VESTING_DURATION = 90 days;
    uint256 public constant VESTING_DURATION_FOR_DAO = 150 days;
    ...
    uint256 public daoLastClaimed;

    bool public rewardPoolDistributed = false;

    constructor(uint256 _startTime, address _communityFund, address _devFund, address _daoFund) public ERC20("ASTROWORLD Share Token","WORLD") {
    ...
    function unclaimedDaoFund() public view returns (uint256 _pending) {
        uint256 _now = block.timestamp;
        if (_now > startTime + VESTING_DURATION_FOR_DAO) _now = startTime + VESTING_DURATION_FOR_DAO;
        if (daoLastClaimed >= _now) return 0;
        _pending = _now.sub(daoLastClaimed).mul(daoFundRewardRate);
    }
