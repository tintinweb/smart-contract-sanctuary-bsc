    ...
    AccessControl
{
    ...
    address _swapWallet;
    uint256[2] collectingRatio = [50,50];


    IToken token;
    mapping(uint256 => uint256[]) ownedLandbyZone;
    mapping(uint256 => sZone) public zones;
    mapping(uint256 => sLand) allLands;
    uint256[] public openedZones;

    constructor(address _busd,  address _swap) ERC721("MGNLAND", "Land") {
        token = IToken(_busd);
    ...
    function buyLands(
        ...
        token.transferFrom(
            msg.sender,
            _globalWallet,
            (zones[_zone].price*_x.length*collectingRatio[0]/100)
        );
        token.transferFrom(
            msg.sender,
            _swapWallet,
            (zones[_zone].price*_x.length*collectingRatio[1]/100)
        );
    ...
    function getLandsByTokenId(uint256 _tokenId) public view returns (sLand memory){
