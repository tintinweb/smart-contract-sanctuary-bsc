...
import "./LaunchPadIDO/WithLimits.sol";
import "./LaunchPadIDO/Timed.sol";
import "./LaunchPadIDO/WithWhitelist.sol";
import "./LaunchPadIDO/WithLevelsSale.sol";
import "./LaunchPadIDO/Withdrawable.sol";
import "./LaunchPadIDO/GeneralIDO.sol";
    ...
    WithLevelsSale
{
    ...
    function setCurrencyDecimals(uint256 v) external onlyOwnerOrAdmin {
        currencyDecimals = v;
    }
    ...
    function voidAndRefund(address payable account, bool refund)
        external
        onlyOwnerOrAdmin
    {
        uint256 purchased = balances[account];
        uint256 value = contributed[account];

        tokensSold -= purchased;
        balances[account] = 0;
        raised -= value;
        contributed[account] = 0;
        participants -= 1;

        // Refund
        bool refunded = false;
        if (refund) {
            if (fundByTokens && fundToken.balanceOf(address(this)) > value) {
                fundToken.transfer(account, value);
                refunded = true;
            }
            if (!fundByTokens && address(this).balance > value) {
                payable(account).transfer(raised);
                refunded = true;
            }
        }

        emit UserRefunded(account, value, purchased, refunded);
    }
...
abstract contract WithWhitelist is Adminable, GeneralIDO, WithLimits {
    ...
    function batchAddWhitelisted(address[] calldata addresses)
        external
        onlyOwnerOrAdmin
    {
    ...
    WithWhitelist
{
    ...
    function reachedMinBaseAllocation() public view returns (bool) {
        ...
        uint256 allocation = baseAllocation > 0
            ? baseAllocation
...
abstract contract GeneralIDO is Adminable, Timed {
    ...
    function setTokensForSale(uint256 _tokensForSale)
        external
        onlyOwnerOrAdmin
    {
        require(
    ...
    function calculatePurchaseAmount(uint256 purchaseAmountWei)
        public
        view
        returns (uint256)
    {
