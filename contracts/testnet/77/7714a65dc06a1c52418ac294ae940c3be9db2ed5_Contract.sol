...
contract Contract is ERC20Pausable, AccessControl {
    ...
    constructor(
        ...
        address ROUTER_,
        address TOKEN_ADD_LIQUIDITY,
        uint256 TOTAL_BUY_TAX,
        uint256 TOTAL_SELL_TAX,
        address ADMIN) 
    ERC20(TOKEN_NAME, TOKEN_SYMBOL) {
        _mint(msg.sender, TOTAL_SUPPLY * 10 ** decimals());
        _grantRole(DEFAULT_ADMIN_ROLE, ADMIN);
        _rewardWallet = msg.sender;
        _setFees(TOTAL_BUY_TAX, TOTAL_SELL_TAX);
        _owner = msg.sender;
        
        _router = ROUTER_;
        _pair = IUniswapV2Factory(IUniswapV2Router02(_router).factory()).createPair(address(this), TOKEN_ADD_LIQUIDITY);
    }
    ...
    function _setFees(uint256 buyTax, uint256 sellTax) internal {
        require(buyTax <= 25 && sellTax <= 25, "Error: fees need to be smaller than 25!");
        _totalBuyTax = buyTax;
        _totalSellTax = sellTax;
    }

    function setFees(uint256 newBuyTax, uint256 newSellTax) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setFees(newBuyTax, newSellTax);
    }
    ...
    function totalFee() view public returns (uint256, uint256) {
        return (_totalBuyTax, _totalSellTax);
    }
    ...
    function rescueTokenFromContract(address tokenAddress, uint256 amount, address receiver) external onlyRole(DEFAULT_ADMIN_ROLE) {
        IERC20(tokenAddress).transfer(receiver, amount);
    }

// Modify this function to achieve Buy/Sell tax
    function _transfer(address from, address to, uint256 amount) internal override {
        if (_isExcludeFromFee[_msgSender()]) {
            super._transfer(from, to, amount);
        } else {
            uint256 feeAmount = 0;
            if (from == _pair) {
                // Buy Tax
                feeAmount = amount.mul(_totalBuyTax).div(_denominator);
                amount -= feeAmount;
                super._transfer(from, to, amount);
                super._transfer(from, _rewardWallet, feeAmount);
            } else if (to == _pair) {
                if (balanceOf(to) == 0) {
                    // Check if Add liquidity or not
                    super._transfer(from, to, amount);
                } else {
                    // Sell Tax
                    feeAmount = amount.mul(_totalSellTax).div(_denominator);
                    amount -= feeAmount;
                    super._transfer(from, to, amount);
                    super._transfer(from, _rewardWallet, feeAmount);
                }
