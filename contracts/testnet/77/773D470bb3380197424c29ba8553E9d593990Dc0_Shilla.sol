...
contract Shilla is IERC20, Ownable {
    ...
    IShillaBadge public shillaBadge;
    ...
    modifier onReceive(address holder) {
        _;
        if(address(autoStaker) != address(0) && holder != address(0)) {
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) private updateHolders(from, to) onReceive(to) {
        ...
        if(IDof[to] == 0) {
            lastID++;
            IDof[to] = lastID;
            holderOfID[lastID] = to;
        }

        emit Transfer(from, to, amount);
        if(from == presaleContract) {
            for(uint8 i = shillaBadge.highestBadgeLevel(); i >= 1; i--) {
                if(shillaBadge.canMintTo(address(this), i, to, _balanceOf[to], false)) {
                    shillaBadge.mint(i, to);
                }
            }
        }
    }
    ...
    function _setPresaleContract(address _presaleContract) external onlyOwner {
        _isExcludedFromFee[_presaleContract] = true;
        _maxSellFromExcluded[_presaleContract] = true;
        presaleContract = _presaleContract;
    }

    function _setShillaBadge(IShillaBadge _shillaBadge) external onlyOwner {
        shillaBadge = _shillaBadge;
    }
