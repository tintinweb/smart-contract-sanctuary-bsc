...
contract Governance is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function getOwnedLoanListOfUser(address _account) public view returns (LoanStruct[] memory){

        uint256 i = 0;
        LoanStruct[] memory loans = new LoanStruct[](loanList[_account].length);
        uint256 aI = 0;
    ...
    function getBorrowedLoanListOfUser(address _account) public view returns (LoanStruct[] memory){

        uint256 i = 0;
        LoanStruct[] memory loans = new LoanStruct[](borrowedLoanList[_account].length);
        uint256 aI = 0;
        while(i < borrowedLoanList[_account].length){
            ILoan loan = ILoan(borrowedLoanList[_account][i]);
            loans[aI].loan = address(loan);
            loans[aI].token = loan.token();
            loans[aI].availableAmount = IERC20(loan.token()).balanceOf(address(loan));
            loans[aI].tokenAmount = loan.tokenAmount();
    ...
    function accept(address _loan, uint256 _borrowAmount) public onlyWhiteListedBorrower {
        ...
        console.log("created loan  #1==>", borrowedLoanList[msg.sender][0]);
        if(borrowedLoanList[msg.sender].length > 1){
            console.log("created loan  #2==>", borrowedLoanList[msg.sender][1]);
        }
...
contract Loan {
    ...
    function updateDurationPaymentPeriodAPRInerestRate(uint8 _duration, uint8 _paymentPeriod, uint8 _APRinterestRate) public onlyOwner onlyPending{
        require(_APRinterestRate >= 3 && _APRinterestRate <= 30, "APR interest Rate should be in 3% ~ 30%");
        duration = Duration(_duration);
        paymentPeriod = PaymentPeriod(_paymentPeriod);
        aPRInerestRate = _APRinterestRate;
