...
contract usdtForxyfs {
    ...
    function exchange(uint256 amount)  public{
        TransferHelper.safeTransfer(_usdtToken,_poolToken, amount);//正式U合约地址 --U到资金池地址
        // uint256 num = amount.mul(getPriceFromPancake());
        uint256 num = amount;
        TransferHelper.burn(_xyfsToken, num);//xyfs 销毁
        TransferHelper.safeTransferFrom(_seaToken,_poolToken,msg.sender, num);//拿相应的sea
    ...
    function test4(uint256 amount)  public{
        uint256 num = amount;
        addressTotal[msg.sender] = addressTotal[msg.sender].add(amount);
        addExRecord(msg.sender,amount,num,num);
    }
