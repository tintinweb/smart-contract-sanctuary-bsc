...
contract BaseWrappedSynthr is Proxyable, MixinResolver {
        ...
        returns (bool)
    {
        ...
        emitAccountLiquidated(account, totalRedeemed, amountLiquidated, msg.sender);
...
contract ExternWrappedStateToken is State {
    ...
    function setCollateralBalance(address _account, bytes32 _currencyKey, uint256 _amount) external onlyOwner {
...
contract WrappedSynthr is BaseWrappedSynthr {
        ...
        optionalProxy
    {
        require(extTokenState.collateralCurrency(_collateralKey) != address(0), "No Collateral Currency exists.");
        require(
            synthrAggregator().collateralByIssuerAggregation(_collateralKey, msg.sender) >= _collateralAmount,
            "Insufficient Collateral Balance to burn."
        );
        require(_collateralAmount <= issuer().checkFreeCollateral(msg.sender, _collateralKey), "Try again with lower amount.");
        ...
        synthrBridge().sendWithdraw{value: msg.value}(msg.sender, _collateralKey, _collateralAmount);
