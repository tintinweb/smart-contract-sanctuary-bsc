...
contract ChickenFarm is Ownable, IBEP20 {
    ...
    uint256 public YieldPerPeriod = 8;
    ...
    string public name = "ChickenFarm";
    string public symbol = "BitCORN";
    ...
    function _compound(address account) external {
        ...
        uint256 daysSinceBuy = firstAdopt[account] / (1 days);
    ...
    function adoptChickens(address referral) public payable lock {
        ...
        if (firstAdopt[msg.sender] == 0) {
            ...
            //by default payout every 3 days
            payoutDays[msg.sender] = 3;
            firstAdopt[msg.sender] = block.timestamp;
        }
        (uint256 chickensBought, uint256 marketing) = getChickensPerETH(
            msg.value
        );
        chickensBought = bonusProvider.getChickensBuy(
            msg.sender,
            chickensBought
        );
        ...
        if (referral != address(0) && referral != msg.sender) {
            ...
            _addChickens(referral, (chickensBought * referralBonus) / 100);
        }
    ...
    function _hatchEggs(address account) private {
        ...
        newEggs = bonusProvider.getChickensCompound(account, newEggs);
        ...
        if (referrals[account] != address(0)) {
            _addChickens(referrals[account], (eggs * referralBonus) / 100);
        }
    ...
    {
        uint256 BaseWei = bonusProvider.getBNBOut(account, eggs * WeiPerEgg);
        ...
        returns (uint256 newEggs, uint256 cornConsumed)
    {
        ...
        uint256 idealNewEggs = bonusProvider.getYield(
            account,
            YieldPerPeriod,
            (chickens[account] * timeSinceLastHatch) / Period
        );
        uint256 cornRequired = bonusProvider.getCornConsumption(
            account,
            idealNewEggs
        );
