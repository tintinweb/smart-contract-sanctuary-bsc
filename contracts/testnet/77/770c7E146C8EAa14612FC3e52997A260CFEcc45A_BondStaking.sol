...
contract BondStaking is Ownable, IERC721Receiver, Pausable {
    ...
    mapping(address => EnumerableSet.UintSet) private depositNftIds;
    ...
    constructor(address _bondNFT) {
        _addPoolReward(0xaBfc3B884A82Ef4989d9F0cdf8D87DA45226d93D, false);
        _addPoolReward(0xaBfc3B884A82Ef4989d9F0cdf8D87DA45226d93D, true);
        _addPoolReward(0x35EAdB18587D789C755EA7C56e6C331eC74F189c, false);
        _addPoolReward(0x35EAdB18587D789C755EA7C56e6C331eC74F189c, true);
    ...
    function info(address _user) external view returns (uint256[] memory _nftIds){
        return depositNftIds[_user].values();
    }

    function currentValue(address _user) public view returns (uint256 _result) {
        uint256[] memory _ids = depositNftIds[_user].values();
        (_result,) = _getValueOfIds(_ids);
    }

    function getCurrentAmount(address _account) public view returns (uint256, uint256) {
    ...
    function pendingReward(address _user, uint256 _pid) public view returns (uint256 pending) {
        require(_pid <= totalPool, "BondStaking: Overflow");
        PoolInfo memory _pool = poolInfo[_pid];
        ...
        if (_pool.isIncentive) {
            ...
            (, _amount) = getCurrentAmount(_user);
        } else {
            (_totalValue,) = _getTotalValue();
            (_amount,) = getCurrentAmount(_user);
        }
        ...
        pending = uint256(int256(_amount * _accRewardPerShare / ACC_REWARD_PRECISION) - userRewardDebt[_user][_pid]);
    }

    function getUserInfo(address _user, uint256 _pid) external view returns (uint256, uint256, int256) {
        return (userInfo[_user].amount, userInfo[_user].incentiveAmount, userRewardDebt[_user][_pid]);
    }
    ...
    function pendingRewardAll(address _user) external view returns (PendingRewardResponse[] memory, UserInfo memory) {
    ...
    function massUpdatePool() public {
        ...
        for (uint256 i = 0; i < activePool.length(); i ++) {
            ...
            if (_pool.isIncentive) {
                ...
                _updatePool(_pool, totalValue, _pid);
            }
        }
    }
    ...
    function deposit(uint256[] memory tokenIds, address to) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            depositNftIds[to].add(tokenId);
            IERC721(address(bondNFT)).safeTransferFrom(msg.sender, address(this), tokenId);
        }

        (uint256 value, uint256 incentiveValue) = _getValueOfIds(depositNftIds[to].values());
        uint256 incValue = 0;
        uint256 incIncentive;

        require(value >= user.amount, 'BondStaking: Invalid deposit');

        incValue = value - user.amount;

        if (incentiveValue > user.incentiveAmount) {
            incIncentive = incentiveValue - user.incentiveAmount;
            user.incentiveAmount = incentiveValue;
        }
        user.amount = value;
        ...
        _updateRewardDebt(to, incValue, incIncentive, true);

        // after calculate accRewardPerShare, update total value
        totalValue += incValue;
        totalValueIncentive += incIncentive;
        _updateAccount(msg.sender);

        emit Deposit(msg.sender, tokenIds, incValue, incIncentive, to);
    }
    ...
    function withdraw(uint256[] memory tokenIds, address to) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(depositNftIds[msg.sender].remove(tokenId), "BondStaking: not contain id");
            IERC721(address(bondNFT)).safeTransferFrom(address(this), to, tokenId);
        }
        (uint256 value, uint256 incentiveValue) = _getValueOfIds(depositNftIds[msg.sender].values());
        uint256 withdrawValue = 0;
        uint256 withdrawIncentive = 0;
        require(user.amount >= value, 'BondStaking: Invalid withdraw');

        withdrawValue = user.amount - value;
        user.amount = value;

        if (incentiveValue < user.incentiveAmount) {
            withdrawIncentive = user.incentiveAmount - incentiveValue;
            user.incentiveAmount = incentiveValue;
        }

        _updateRewardDebt(to, withdrawValue, withdrawIncentive, false);

        totalValue -= withdrawValue;
        totalValueIncentive -= withdrawIncentive;
        _updateAccount(msg.sender);

        emit Withdraw(msg.sender, tokenIds, withdrawValue, withdrawIncentive, to);
    }
    ...
    //        for (uint256 i = 0; i < tokenIds.length; i++) {
    //            uint256 tokenId = tokenIds[i];
    //            require(user.nftIds.remove(tokenId), "NFTStaking: not contain id");
    //            IERC721(address(bondNFT)).safeTransferFrom(address(this), to, tokenId);
    //        }
    //
    //        uint256 value = _getValueOfIds(user.nftIds.values());
    //        require(user.amount >= value, 'NFTStaking: Invalid withdraw');
    //
    //        uint256 withdrawValue = user.amount - value;
    //
    //        user.rewardDebt = accumulatedReward - int256(withdrawValue * accRewardPerShare / ACC_REWARD_PRECISION);
    //        user.amount -= withdrawValue;
    //        totalValue -= withdrawValue;
    //
    //        // Interactions
    //        if (_pendingReward > 0) {
    //            PSR.safeTransfer(to, _pendingReward);
    //        }
    //        emit Withdraw(msg.sender, tokenIds, withdrawValue, to);
    ...
    function withdrawAndHarvest(uint256[] memory tokenIds, address to) public {
        _withdrawAndHarvest(tokenIds, to);
    }

    function withdrawAll(address to) public {
        uint256[] memory tokenIds = depositNftIds[msg.sender].values();
    ...
    function emergencyWithdraw(address to) public {
        ...
        uint256 valueIncentive = user.incentiveAmount;
        user.amount = 0;
        user.incentiveAmount = 0;
        totalValue -= value;
        totalValueIncentive -= valueIncentive;
        for (uint256 i = 0; i < activePool.length(); i++) {
        ...
        emit EmergencyWithdraw(msg.sender, tokenIds, value, to);
    }
    ...
    function _updateAccount(address account) internal {
        ...
        uint256 _oldBonusIncentive = user.bonusIncentive;
        uint256 _newBonus = _getBonus(user.amount, account);
        uint256 _newBonusIncentive = _getBonus(user.incentiveAmount, account);

        for (uint256 i = 0; i < activePool.length(); i++) {
            uint256 _pid = activePool.at(i);
            PoolInfo memory _pool = poolInfo[_pid];
            if (_pool.isIncentive) {
                if (_newBonusIncentive > _oldBonusIncentive) {
                    userRewardDebt[account][_pid] += int256((_newBonusIncentive - _oldBonusIncentive) * _pool.accRewardPerShare / ACC_REWARD_PRECISION);
                    totalBonusIncentive += _newBonusIncentive - _oldBonusIncentive;
                } else {
                    userRewardDebt[account][_pid] -= int256((_oldBonusIncentive - _newBonusIncentive) * _pool.accRewardPerShare / ACC_REWARD_PRECISION);
                    totalBonusIncentive -= _oldBonusIncentive - _newBonusIncentive;
                }
            } else {
                if (_newBonus > _oldBonus) {
                    userRewardDebt[account][_pid] += int256((_newBonus - _oldBonus) * _pool.accRewardPerShare / ACC_REWARD_PRECISION);
                    totalBonus += _newBonus - _oldBonus;
                } else {
                    userRewardDebt[account][_pid] -= int256((_oldBonus - _newBonus) * _pool.accRewardPerShare / ACC_REWARD_PRECISION);
                    totalBonusIncentive -= _oldBonus - _newBonus;
                }
            }
        }
    }
    ...
    function _updateRewardDebt(address _user, uint256 _value, uint256 _incentiveValue, bool _isDepositOrWithdraw) internal {
        ...
        for (uint256 i = 0; i < activePool.length(); i ++) {
            ...
            if (_pool.isIncentive) {
                ...
                uint256 _rewardPerShare = _updatePool(_pool, totalValue, _pid);
                if (_isDepositOrWithdraw) {
    ...
    function _updatePoolAndHarvest(address _to, address _user) internal {
        ...
        for (uint256 i = 0; i < activePool.length(); i ++) {
            ...
            if (_pool.isIncentive) {
                ...
                accumulatedReward = int256(user.incentiveAmount * _rewardPerShare / ACC_REWARD_PRECISION);
            } else {
                _rewardPerShare = _updatePool(_pool, totalValue, _pid);
                accumulatedReward = int256(user.amount * _rewardPerShare / ACC_REWARD_PRECISION);
            }
            _pendingReward = uint256(accumulatedReward - userRewardDebt[_user][_pid]);

            //update reward debt
            userRewardDebt[_user][_pid] = accumulatedReward;
            if (_pendingReward > 0) {
    ...
    function updateIncentiveThreshold(uint256 _value) external onlyOwner {
        uint256 old = incentiveThreshold;
        incentiveThreshold = _value;
        emit IncentiveThresholdChanged(old, _value);
    }
