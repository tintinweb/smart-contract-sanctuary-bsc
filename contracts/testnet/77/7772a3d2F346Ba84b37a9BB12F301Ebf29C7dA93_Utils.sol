...
contract MooniCorn is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function setDevFeeAdd(address payable  _DevFee) external onlyOwner() {
          DevFee = _DevFee;
    }
    ...
    bool public handleFeeFirstOnlyOne = true;

    
    function handleFeeFirstTime(address from, address to) public onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));
        require(contractTokenBalance >= minTokenNumberToSell, "Contract balance is too small");
        require(handleFeeFirstOnlyOne == true, "Allready called, this function can only be used one time");
        handleFeeFirstOnlyOne = false;
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            DevFee.transfer(bnbToBeAddedToLiquidity);
