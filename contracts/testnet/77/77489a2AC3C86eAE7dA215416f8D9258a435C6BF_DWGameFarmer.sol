...
contract DWNFTGenerator {
    ...
    function generateWP(uint256 _seed) external pure returns (uint8[6] memory) {
        ...
        uint8 mainWP;
        uint256 rarity;
        uint256 positionalSeed = _seed;
        _seed >>= 2;
        // Generate shuffled primary WP
        (mainWP, rarity) = getWPAndRarity(_seed, 100);
        uint256 mainPos = positionalSeed % 6;
    ...
    function _generateDurability(
        uint256 _seed,
        uint256 _rarity,
        uint8[2][5] memory _durabilityArray
    ) internal pure returns (uint256) {
        return
            (_seed %
                (_durabilityArray[_rarity][1] -
                    _durabilityArray[_rarity][0] +
                    1)) + _durabilityArray[_rarity][0];
    }

    function generateFarmerDurability(uint256 _seed, uint256 _rarity)
        external
        pure
        returns (uint256)
    {
        return
            _generateDurability(
                _seed,
                _rarity,
                [[63, 80], [55, 80], [30, 60], [30, 50], [30, 45]]
            );
    }

    function generateTractorDurability(uint256 _seed, uint256 _rarity)
        external
        pure
        returns (uint256)
    {
...
contract DWSubGame {
    ...
    DWRandomizator internal randomizator;
    DWNFTGenerator internal generator;
    ...
    function setNFTGenerator(address _generatorAddress) external onlySupport {
        generator = DWNFTGenerator(_generatorAddress);
    }
    ...
    function getStabilizer() internal view returns (address) {
        return
            stabilizers[
                uint256(
...
contract FarmerWaifu is ERC721 {
  ...
  */
    ...
    uint64[] public forSale;

    address internal owner = msg.sender;
    address internal support;
    mapping(address => bool) internal games;
    mapping(address => uint64) private ownerFarmerCount;
    mapping(address => uint64[]) private ownerToFarmers;
    mapping(uint64 => address) private farmerToOwner;
    mapping(uint64 => uint256) private farmerAtOwnerIndex;
    mapping(uint64 => uint256) private farmerToSaleIndex;
...
// File: contracts/DWGameFarmer.sol
...
contract DWGameFarmer is DWSubGame {
    using SafeMath for uint256;
    using SafeMath64 for uint64;
    FarmerWaifu farmer;
    uint256 mintPrice;

    // Wallet => Request ID => quantity
    mapping(address => mapping(uint256 => uint256)) internal mintRequests;

    mapping(uint256 => uint256) public healPrices;
    ...
    // TODO: permission modifiers
    function setHealPrices(uint256[] calldata _prices) external {
        for (uint256 i = 0; i < _prices.length; i++) {
            healPrices[i] = _prices[i];
        }
    }

    // Internal getters
    /// @return price
    function getLicensePrice(uint256 _days) internal pure returns (uint256) {
        if (_days == 7) return 7;
        if (_days == 15) return 13;
        if (_days == 30) return 26;
        return 0;
    }

    // Expendable functions
    function mint(uint256 _quantity) external payable {
        // Use the gas fee provided
        uint256 _gas = game.mintUnitGas().mul(_quantity);
        require(msg.value >= _gas, "Not enough BNB sent for minting.");
        require(_quantity <= 20, "Maximum parallel minting reached.");
        uint256 _rewardGas = _gas.mul(game.rewardComission()).div(100);
        uint256 _mintGas = _gas.sub(_rewardGas);
        sendBNB(game.rewardPool(), _rewardGas);
        sendBNB(game.bella(), _mintGas);
        // Make the PEACH transaction
        game.transferFrom(
            msg.sender,
            game.refillPool(),
            mintPrice
                .mul(game.decimalMultiplier().div(peach.getCurrentPrice()))
                .mul(_quantity)
        );

        uint256 requestID = randomizator.randomRequest();
        mintRequests[msg.sender][requestID] = _quantity;
        emit CallbackRequest(msg.sender, requestID);
    }

    function resolveMint(uint256 _id, address _target) external {
        uint64 farmerCount = farmer.farmerCount();
        for (uint256 i = 0; i < mintRequests[_target][_id]; i++) {
            uint256 seed = randomizator.getRandom(_id, _target, i);
            uint256 dna = generator.generateDNA(seed);
            uint256 rarity = 0;
            uint8[6] memory wps = generator.generateWP(seed);
            for (uint256 j = 0; j < wps.length; j++) {
                if (wps[j] / 50 > rarity) rarity = wps[j] / 50;
            }
            if (rarity == 5) rarity = 4;
            uint256 durability = generator.generateFarmerDurability(
                seed,
                rarity
            );
        ...
        farmer.increaseCounter(uint64(mintRequests[_target][_id]));
        // Already minted
        mintRequests[_target][_id] = 0;
    }

    function buy(uint256 _farmerId) external payable {
        uint256 _farmerPrice = farmer.sellPrice(_farmerId);
        // Use the gas fee provided
        require(
            msg.value >= _farmerPrice,
            "Not enough BNB sent for the marketplace transaction."
        );

        // Make the PEACH transaction
        address payable _farmerOwner = payable(farmer.ownerOf(_farmerId));
        uint256 _commission = _farmerPrice.mul(game.rewardComission()).div(10);

        sendBNB(_farmerOwner, _farmerPrice.sub(_commission));
        sendBNB(game.rewardPool(), _commission);
        // Make the purchase
    ...
    function heal(uint256 _farmerId) external payable {
        require(
            msg.value >= game.healGas(),
            "Not enough BNB sent for the heal transaction."
        );
        sendBNB(game.refillPool(), msg.value);

        game.transferFrom(
            msg.sender,
            game.refillPool(),
            healPrices[farmer.getRarity(_farmerId)].mul(
                game.decimalMultiplier().div(peach.getCurrentPrice())
            )
        );
    ...
    function multiHeal(uint256[] memory _farmerIds) external payable {
        require(
            msg.value >= game.healGas() * _farmerIds.length,
            "Not enough BNB sent for the heal transaction."
        );
        sendBNB(game.refillPool(), msg.value);

        uint256 totalHealingPrice = 0;
        for (uint256 i = 0; i < _farmerIds.length; i++) {
            farmer.heal(_farmerIds[i], 12);
            totalHealingPrice += healPrices[farmer.getRarity(_farmerIds[i])];
        }

        game.transferFrom(
            msg.sender,
            game.refillPool(),
            totalHealingPrice.mul(
                game.decimalMultiplier().div(peach.getCurrentPrice())
            )
        );
    }

    function addLicense(uint256 _farmerId, uint8 _days) external payable {
        require(
            msg.value >= game.licenseGas(),
            "Not enough BNB sent for the heal transaction."
        );
        sendBNB(game.refillPool(), msg.value);
        uint256 price = getLicensePrice(_days);
        require(price > 0, "Choose one option from the list, please");
        game.transferFrom(
            msg.sender,
            game.refillPool(),
            price.mul(game.decimalMultiplier().div(peach.getCurrentPrice()))
        );
        farmer.setLicensed(_farmerId, _days);
    }

    function addLicenses(uint256[] calldata _farmerIds, uint8 _days)
        external
        payable
    {
        require(
            msg.value >= game.licenseGas() * _farmerIds.length,
            "Not enough BNB sent for the heal transaction."
        );
        sendBNB(game.refillPool(), msg.value);
        uint256 price = getLicensePrice(_days);
        ...
        game.transferFrom(
            msg.sender,
            game.refillPool(),
            price.mul(_farmerIds.length).mul(
                game.decimalMultiplier().div(peach.getCurrentPrice())
            )
        );
        for (uint256 i = 0; i < _farmerIds.length; i++) {
    ...
    function burn(uint64 _tokenId) external {
        farmer.burn(_tokenId);
        // TODO: Give the reward here
    }
}