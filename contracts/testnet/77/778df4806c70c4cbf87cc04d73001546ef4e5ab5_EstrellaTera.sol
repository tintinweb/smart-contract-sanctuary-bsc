...
contract EstrellaTera is Ownable, Pausable, ReentrancyGuard{
    ...
    constructor()
    {
        ...
        USDACEToken = IERC20(0x74783801A97293D748a5824E0B3c55c08e86eD6a);
        ...
        if(roundbool == false)
        {
            ...
            { 
                ...
                if(IERC20(tokenAddress) == USDTToken)
                {
                    ...
                    Commission(devAddress,levelPercentage_[1]);
                ...
                {
                    USDACEToken.transferFrom(msg.sender, owner(), Rem_Amount1);
                    ETAToken.transferFrom(owner(),address(this),tokentransfer);
                }
            } 
            if(rounndDetails[2] > 1)
    ...
    private{
        ...
        if(_totalamount > 0)
        {
            ...
            if(IERC20(_tokenAddress) == USDTToken)
            {
                ...
                Commission(devAddress,levelPercentage_[1]);
                buyerReferralCommission(msg.sender,levelPercentage_[2],levelPercentage_[3]);
                sellerReferralCommission(msg.sender,levelPercentage_[4]);
            }else if(IERC20(_tokenAddress) == USDACEToken)
            {
                Commission(devAddress,levelPercentage_[1]);
                Commission(msg.sender,levelPercentage_[2]);
                Commission(msg.sender,levelPercentage_[3]);
                sellerReferralCommission(msg.sender,levelPercentage_[4]);
                USDACEToken.transferFrom(msg.sender, owner(), _totalamount);
            }    
        }
    }
    ...
    public{
        ...
        if(msg.sender == owner() || msg.sender == devAddress )
        {
        ...
        // USDACEToken.transfer(msg.sender,totalUSDACE);
