...
contract CriptoVersePrediction is Ownable, Pausable, ReentrancyGuard {
  ...
  function criptoVerseBetBull(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
  ...
  function criptoVerseBetBear(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
    require(epoch == currentEpoch, "Bet is too early/late");
    require(_bettable(epoch), "Round not bettable");
    require(msg.value >= minBetAmount, "Bet amount must be greater than minBetAmount");
    require(Bets[epoch][msg.sender].amount == 0, "Can only bet once per round");
  ...
  function criptoVerseClaim(uint256[] calldata epochs) external nonReentrant notContract {
