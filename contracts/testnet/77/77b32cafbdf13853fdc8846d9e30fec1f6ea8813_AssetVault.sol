...
contract AssetVault {
    ...
    function owner() public view returns(address) {
        return IERC721(wNFTAddr).ownerOf(wNFTId);
    }
    ...
    function withdrawAll() external onlyOwner onlyWithdrawEnabled {
        require(bundleNFTHoldings.length > 0, "AssetVault: No nft");
        for (uint256 i = 0; i < bundleNFTHoldings.length; i++) {
            NFTHolding memory nft = bundleNFTHoldings[i];
            if (nft.amount == 0 && IERC721(nft.tokenAddress).ownerOf(nft.tokenId) == address(this)) {
                IERC721(nft.tokenAddress).safeTransferFrom(
                    address(this),
                    msg.sender,
                    nft.tokenId
                );
            } else if (IERC1155(nft.tokenAddress).balanceOf(address(this), nft.tokenId) == nft.amount) {
