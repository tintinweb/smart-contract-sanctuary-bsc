...
contract BUSDVYNCSTAKE is ReentrancyGuard, Ownable {
    ...
    function stake(uint256 amount) external nonReentrant {
        bool success = busd.transferFrom(msg.sender, address(this), amount);
        require(success == true, "unable to transfer");
        userInfo[msg.sender]
        ...
        if (amount > busdAdded + amountToSwap) {
            require(
                true == busd.transfer(msg.sender, amount - (busdAdded + amountToSwap))
            );
        }
    ...
    function calculateSwapInAmount(uint256 reserveIn, uint256 userIn)
        ...
        returns (uint256)
    {
        uint256 sqrt = Math.sqrt(
            reserveIn * (userIn * 399000000) + (reserveIn * 399000625)
        );
        uint256 amount = sqrt - ((reserveIn * 19975) / 19950);
        return amount;
    }
