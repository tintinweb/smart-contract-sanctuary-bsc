...
import "./interfaces/ITestCoinInvitationDistributor.sol";
...
contract TestCoin is IERC20, Ownable {
  ...
  mapping (address => bool) private isInvitationExempt;
  mapping (address => bool) private isProtectionPeriodExempt;
  ...
  ITestCoinReflectionDistributor public reflectionDistributor;
  ITestCoinInvitationDistributor public invitationDistributor;

  // protection period
  uint public protectionPeriod = 180 days;
  mapping (address => uint) public userProtectionPeriods;
  ...
  constructor (
    ...
    isInvitationExempt[address(this)] = true;
    isInvitationExempt[address(pair)] = true;
    isInvitationExempt[address(router)] = true;
    
    isProtectionPeriodExempt[address(this)] = true;
    isProtectionPeriodExempt[msg.sender] = true;
    isProtectionPeriodExempt[_marketingAddress] = true;
    isProtectionPeriodExempt[_fundAddress] = true;
    isProtectionPeriodExempt[address(pair)] = true;
    isProtectionPeriodExempt[address(router)] = true;
  }
  ...
  function _transfer(address from, address to, uint256 amount) internal virtual {
    ...
    _addProtectionPeriod(to);
    _setInvitation(from, to);
    uint256 amountAfter = _checkProtectionPeriod(from, amount);

    if (inSwap || !shouldTakeFee(from)) { 
        _basicTransfer(from, to, amountAfter);
    } else {
        _transferWithFee(from, to, amountAfter);
    }

    _addReflectionShare(to);
    _addReflectionShare(from);

    _dropAirdropAmount(to);
    _dropAirdropAmount(from);
  }

  function _addProtectionPeriod(address to) internal {
    if (_balances[to] == 0) {
      userProtectionPeriods[to] = (block.timestamp).add(protectionPeriod);
    }
  }

  function _checkProtectionPeriod(address from, uint256 amount) internal returns(uint256) {
    if (!isProtectionPeriodExempt[from] && block.timestamp < userProtectionPeriods[from]) {
      uint256 fee = amount.mul(90).div(100);
      _autoLiquify(fee);
      uint256 protectedAmount = amount.sub(fee);
  ...
  function _depositReflection(uint256 reflectionFeeAmount) internal swapping {
    // sending portion of tax dedicated for reflection to reflection distributor contract
    _basicTransfer(msg.sender, address(reflectionDistributor), reflectionFeeAmount);
    // call depositReflectionAmount on reflectionDistributor contract
    reflectionDistributor.depositReflectionAmount(reflectionFeeAmount);
  }

  function _depositInvitationBonus(uint256 amount) internal swapping {
    // sending portion of tax dedicated for invitation bonus to invitation distributor contract
    _basicTransfer(msg.sender, address(invitationDistributor), amount);
    // call depositInvitationAmount on invitationDistributor contract
    invitationDistributor.depositInvitationAmount(amount, msg.sender);
  }

  function _setInvitation(address from, address to) internal {
    if (!isInvitationExempt[from] && balanceOf(to) == 0) {
      invitationDistributor.setInvitation(from, to);
    }
  }
  ...
  function _addReflectionShare(address account) internal {
    if (!isReflectionExempt[account]) { 
        try reflectionDistributor.setShare(payable(account), balanceOf(account)) {} catch {} 
    }
  }
  ...
  function setInvitationExempt(address _address, bool _exempt) public onlyOwner {
    isInvitationExempt[_address] = _exempt;
  }
  ...
  function setAutoLiquidator(address _autoLiquidator) public onlyOwner {
    ...
    isInvitationExempt[_autoLiquidator] = true;
  }
  ...
  function setReflectionDistributor(address _distributor) public onlyOwner {
    ...
    reflectionDistributor = ITestCoinReflectionDistributor(_distributor);
    isFeeExempt[_distributor] = true;
    isReflectionExempt[_distributor] = true;
    isAirdropExempt[_distributor] = true;
    isInvitationExempt[_distributor] = true;
  }

  function setInvitationDistributor(address _distributor) public onlyOwner {
    require(_distributor != address(0), '!nonzero');
    invitationDistributor = ITestCoinInvitationDistributor(_distributor);
    isFeeExempt[_distributor] = true;
    isReflectionExempt[_distributor] = true;
    isAirdropExempt[_distributor] = true;
    isInvitationExempt[_distributor] = true;
...
interface ITestCoinInvitationDistributor {
  function setInvitation(address inviter, address invitee) external;
  function depositInvitationAmount(uint256 amount, address account) external;
}
