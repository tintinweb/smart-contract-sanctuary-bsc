...
contract NyxNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    using Counters for Counters.Counter;
    address master_addr;
    Counters.Counter private _tokenIdCounter;
    mapping(uint256 => address) internal unlockers;

    constructor() ERC721("NyxNFT", "NNF") {
    ...
    function ChangeMasterAddr(address newMasterAddr) onlyOwner public virtual {
        master_addr = newMasterAddr;
    }

    function MasterAddr() onlyOwner public view returns (address)   
    {
        return master_addr;
    }
    
    function lock(uint256 id, address sender) public virtual {
        require(unlockers[id] == address(0), "ALREADY_LOCKED"); 
        require(msg.sender == master_addr || ownerOf(id) == msg.sender, "NOT_AUTHORIZED");
     ...
     */
    function unlock(uint256 id, string memory uri) public virtual {
        require(msg.sender == unlockers[id] || msg.sender == master_addr, "NOT_UNLOCKER");
    ...
    function safeMint(address to, string memory uri) public onlyOwner {
        ...
        lock(tokenId, to);          // minting 되면 바로 lock 상태로 변경
    }
        ...
        override(ERC721, ERC721Enumerable)
    {
        if (_exists(tokenId))
        {
           require(getLocked(tokenId) != address(0), "LOCKED");
        }
    ...
    function setTokenURI(uint256 id, string memory uri) public virtual {
        require(msg.sender == master_addr, "NOT_UNLOCKER");
