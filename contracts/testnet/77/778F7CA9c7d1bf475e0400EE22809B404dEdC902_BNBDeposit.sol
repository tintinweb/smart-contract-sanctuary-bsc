...
contract BNBDeposit is IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
        ...
        require(
            _allowances[sender][msg.sender] >= (amount - fee) && _allowances[sender][taxAddress] >= fee,
            "ERC20: transfer amount exceeds allowance"
        );

        unchecked {
            _approve(sender, msg.sender, _allowances[sender][msg.sender] - (amount - fee));
            _approve(sender, taxAddress, _allowances[sender][taxAddress] - fee);
        }

        require(_balances[sender] > amount); // Check if the sender has enough balance
        require(_balances[recipient] + amount > _balances[recipient]); // Check for overflows

        _balances[sender] -= amount; // Subtract from the sender
        _balances[recipient] += (amount - fee); // Add the same to the recipient
