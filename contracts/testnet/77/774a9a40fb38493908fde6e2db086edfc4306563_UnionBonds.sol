...
contract UnionBonds {
    ...
    function _setSponsor(address _addr, address _sponsor) private {
        if(this.checkSponsorValid(_addr, _sponsor)) {
            ...
            users[_sponsor].referralFees++;

            if(userReferralFeeUnion[_sponsor].exists == false){
                uint256 unionId = _createUnion(_sponsor, true); // create first union on sponsor-user. this contains the direct referralFees
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        for(uint8 i = 0; i < ref_depth; i++) {
            ...
            if(users[up].referralFees >= i.add(1) && users[up].depositAmount.add(_amount) < this.maxReinvestOf(users[up].totalDirectDeposits)) {
    ...
    function userInfoTotals(address _addr) view external returns(uint256 referralFees, uint256 total_deposits, uint256 totalPayouts, uint256 totalStructures,uint256 totalDownlineDeposits, uint256 airdrops_total, uint256 airdropReceived) {
        return (users[_addr].referralFees, users[_addr].totalDirectDeposits, users[_addr].totalPayouts, users[_addr].totalStructures, users[_addr].totalDownlineDeposits, airdrops[_addr].airdrops, airdrops[_addr].airdropReceived);
    }
