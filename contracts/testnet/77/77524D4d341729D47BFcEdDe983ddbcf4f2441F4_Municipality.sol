...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function attachMinerToParcel(uint256 _parcelId, uint256 _minerId) public {
    ...
    function autoAttach() external {
        ...
        for(uint i; i < userParcels.length && iteratorOnMinersArray < userMiners.length; ++i) {
            ...
            for(uint j = iteratorOnMinersArray; j < userMiners.length; ++j) {
                if(minerToParcelMapping[userMiners[j]] == 0) {
                    attachMinerToParcel(userParcels[i], userMiners[j]);
                }
