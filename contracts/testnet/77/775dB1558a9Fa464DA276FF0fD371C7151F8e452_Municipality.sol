...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setStandardParcelSoldCount (uint256 _soldParcelCount) external onlyOwner{
        currentlySoldStandardParcelsCount = _soldParcelCount;
    }
    ...
    function grantParcels(ParcelsMintSignature calldata _mintingSignature,
        uint256 _referrerId, address _user) external {
    ...
    function _updateAdditionLevel(address _user) private {
        ...
        if (termTimestamp + 2505600 > block.timestamp){ // 30 days
            if (userToPurchasedAmountMapping[_user] >= 22000 * 1e18) {
                _additionalLevel = 21;
            } else if (userToPurchasedAmountMapping[_user] >= 7000 * 1e18) {
                _additionalLevel = 14;
            } else if (userToPurchasedAmountMapping[_user] >= 3000 * 1e18) {
                _additionalLevel = 9;
            } else if (userToPurchasedAmountMapping[_user] >= 700 * 1e18) {
                _additionalLevel = 5;
            }
    ...
    function _updateUserLevel(address _user) private {
        if(userToPurchasedAmountMapping[_user] <= 500 * 1e18) {
