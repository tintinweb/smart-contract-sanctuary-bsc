...
/**
 * Play at https://dapp.fomo3darb.com
 * SoGrosslyIncandescent and zeroxcool proudly copypaste from those geniuses:
 * @title -FoMo-3D v2.1.6
 * ┌┬┐┌─┐┌─┐┌┬┐   ╦╦ ╦╔═╗╔╦╗  ┌─┐┬─┐┌─┐┌─┐┌─┐┌┐┌┌┬┐┌─┐
 *  │ ├┤ ├─┤│││   ║║ ║╚═╗ ║   ├─┘├┬┘├┤ └─┐├┤ │││ │ └─┐
 *  ┴ └─┘┴ ┴┴ ┴  ╚╝╚═╝╚═╝ ╩   ┴  ┴└─└─┘└─┘└─┘┘└┘ ┴ └─┘
 *                                  _____                      _____
...
contract FoMo3DTest is ReentrancyGuard {
    event onNewName(
    ...
    StakingPoolInterface private StakingPool;
    PlayerBookInterface private PlayerBook;
    TreasuryInterface private Treasury;
    
    address public treasury = 0xbD773cA70d8feBdE70A63B25B4E3845425fafd79;
    address public playerBook = 0x911523cd4f2ABD870A7Ac76946D4cbd6DF56a295;
    address public staking = 0x909D06D2681c2f9F1Cf869c5C24832A3E8E622d6;

    //==============================================================================
    //     _ _  _  |`. _     _ _ |_ | _  _  .
    //    (_(_)| |~|~|(_||_|| (_||_)|(/__\  .  (game settings)
    //=================_|===========================================================
    string public constant name = "FoMo3D Test Scaling";
    string public constant symbol = "Test3D";
    uint256 private rndExtra_ = 0; // length of the very first ICO
    uint256 private rndGap_ = 8 hours; // length of ICO phase, set to 1 year for EOS.
    uint256 private constant rndInit_ = 1 hours; // round timer starts at this
    uint256 private constant rndInc_ = 15 seconds; // every full key purchased adds this much to the timer
    uint256 private constant rndMax_ = 1 hours; // max length a round timer can be
    //==============================================================================
    //     _| _ _|_ _    _ _ _|_    _   .
    ...
    //==============================================================================
    constructor() {
        StakingPool = StakingPoolInterface(staking);
        PlayerBook = PlayerBookInterface(playerBook);
        Treasury = TreasuryInterface(treasury);
    ...
    modifier isActivated() {
        require(
            activated_ == true,
            "its not ready yet.  check ?eta in Telegram"
        );
        _;
    }
    ...
    function reLoadXid(
        ...
        uint256 _team,
        uint256 _eth
    ) public isActivated isHuman nonReentrant isWithinLimits(_eth) {
     ...
     */
    function withdraw() public isActivated isHuman nonReentrant {
        // setup local rID
        uint256 _rID = rID_;
     ...
     * @return round id // 1
     * @return total keys for round //2
     * @return time round ends //3
     * @return time round started //4
     * @return current pot //5
     * @return current team ID & player ID in lead //6
     * @return current player in leads address //7
     * @return current player in leads name //8
     * @return whales eth in for round //9
     * @return bears eth in for round //10
     * @return sneks eth in for round
     * @return bulls eth in for round //12
     * @return airdrop tracker # & airdrop pot //13
     */
        ...
        returns (F3Ddatasets.EventReturns memory)
    {
        ...
        // send share for p3d to staking pool
        if (_p3d > 0) {
            StakingPool.deposit{value: _p3d}();
        }
    ...
    /**
     * @dev generates a random number between 0-99 and checks to see if that
    ...
    function distributeExternal(
        ...
        if (_com > 0) {
            (bool tmpSuccess1,) = payable(treasury).call{value : _com, gas : 30000}("");
            tmpSuccess1 = false;
            _com = 0;
        }
        ...
        _p3d = _p3d.add(_long);
        if (_p3d > 0) {
            // deposit to staking pool contract
            (bool tmpSuccess1,) = payable(staking).call{value : _p3d, gas : 30000}("");
            tmpSuccess1 = false;
            // set up event data
    ...
    function distributeInternal(
        ...
        _eth = _eth.sub(
            ((_eth.mul(14)) / 100).add((_eth.mul(fees_[_team].p3d)) / 100)
        );
    ...
    function activate() public {
        ...
        require(
            msg.sender == 0xf6e6B55A5769Bd434e193DfFdbD8b494ab72C4f8,
            "only team just can activate"
        );
...
library F3DKeysCalcLong {
    ...
    function keysRec(uint256 _curEth, uint256 _newEth)
        ...
        returns (uint256)
    {
        return (keys((_curEth).mul(1334).add(_newEth)).sub(keys(_curEth.mul(1334))));
    }
                    ...
                    (
                        (
                            ((_eth).mul(1000000000000000000)).mul(
                                312500000000000000000000000.mul(1334)/10


                            )
                        ).add(
                                5624988281256103515625000000000000000000000000000000000000000000.mul(1334)/10
                            )
                    ).sqrt()
                ).sub(74999921875000000000000000000000.mul(1334)/10)
            ) / (156250000.mul(1334)/10);
    }
    ...
    function eth(uint256 _keys) internal pure returns (uint256) {
        return
            (
                (78125000.mul(1334)/10).mul(_keys.sq()).add(
                    ((149999843750000.mul(1334)/10).mul(_keys.mul(1000000000000000000))) /
                        (2)
                )
