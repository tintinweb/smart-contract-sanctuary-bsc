...
contract Numr is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint public minTimeBid = 21600;
    uint public stepPoint = 5;
    uint public basePrice = 50000000000000000; //0.05 eth
    uint public totalSupplyPaidMint = 0; 
    uint public priceBidFourNumer = 300000000000000000; // .3 eth
    uint public priceBidThreeNumer = 3000000000000000000; // 3 eth
    uint public totalAvailablesBidThreeNumber = 850;
    uint public totalAvailablesBidFourNumber = 8800;
    ...
    mapping(uint => uint) public timeBid;
    ...
    function mintReserve(uint[] memory _tokenId) external onlyOwner {
        for(uint i =0; i < _tokenId.length; i++){
            require((_tokenId[i] > 0 && _tokenId[i]< 100 ) ||  isReserveToken[_tokenId[i]])  ;
            _mint(msg.sender, _tokenId[i]);
        }
    }
    function updateBasePrice(uint _price) external onlyOwner {
        require(_price > 0);
        basePrice = _price;
    }
    ...
    function addNonBids(address[] calldata addresses, uint quantity, uint option) public onlyOwner {
        // option == 1 Bidding 3 number
        // option == 2 Bidding 4 number
        require(option == 1 || option == 2);
        for (uint i = 0; i < addresses.length; i++) {
            nonBids[addresses[i]][option] = quantity;
        }
    }
    ...
    /*//////////////////////////////////////////////////////////////
                                USER FUNCTIONS
    //////////////////////////////////////////////////////////////*/
    function swapReferralPoint(uint _point) external {
        require(referralPoint[msg.sender] >= _point);
        require(isSwap);
        uint paidCount = _point.div(stepPoint);
        nonPaidMint[msg.sender] += paidCount;
        referralPoint[msg.sender] -= paidCount * stepPoint;
    }
    function freeMint(address to, uint tokenId, address ref) external {
        require(!freeMintAddress[to]);
        require(thresholdFreeMint <= tokenId && tokenId <= maxFeeMint);
        addSponsor(msg.sender, ref);
        addPower(initValue(tokenId), msg.sender);
        _mint(to, tokenId);
        freeMintAddress[to] = true;
        emit Mint(to, tokenId, 0, block.timestamp);
    }

    function paidMint(address to, uint[] memory tokenId, address ref) external nonReentrant payable {
        require(isStartTimePaidMint);
        addSponsor(msg.sender, ref);
        if(nonPaidMint[msg.sender] >= tokenId.length){
            paidMintNonFee(tokenId);
        }else {
            paidMintWithFee(tokenId);
        }
        uint powerUser = 0;
        ...
        totalSupplyPaidMint = totalSupplyPaidMint + tokenId.length;
    }

    function paidMintWithFee(uint[] memory tokenId) internal {
    ...
    function paidMintNonFee(uint[] memory tokenId) internal {
        require(msg.value == 0);
        nonPaidMint[msg.sender] -= tokenId.length;
    }

    function bid(uint tokenId, uint _bid, address ref) external nonReentrant payable {
        uint priceBid = (tokenId < 1000) ? priceBidThreeNumer : priceBidFourNumer;
        ...
        require(totalBid > 0);
        require(msg.value == _bid && _bid >= priceBid && _bid >= minBid[tokenId] && _bid > maxBid[tokenId] && _bid.mod(basePrice) == 0);
        require(isTimeBid);
        ...
        timeBid[tokenId] = block.timestamp;        
        emit Bid(msg.sender, tokenId, _bid, block.timestamp);
    }

    function buyBid(address to, uint tokenId) external nonReentrant {
        require(bidUser[msg.sender][tokenId] == maxBid[tokenId] && (timeBid[tokenId] + minTimeBid) <= block.timestamp);
    ...
    function nonBid(address to , uint tokenId, address ref) external nonReentrant payable {
        uint _freeBid = (tokenId < 1000) ? totalAvailablesFreeBidThreeNumber - 1 : totalAvailablesFreeBidFourNumber - 1;
    ...
    function ascend(uint[] memory tokens) external payable {
        ...
        for (uint i = 1; i < tokens.length; i++) {
            _burn(tokens[i]);
            if(tokens[i] >= 10000 && tokens[i]< 100000){
                totalPaidBurn = totalPaidBurn + 1;
            }
        ...
        if(totalPaidBurn > 0){
            totalSupplyPaidMint = totalSupplyPaidMint - totalPaidBurn;
        }
        if(totalFreeBurn > 0){
    ...
    function getMetadata(uint tokenId) internal view returns (string memory) {
        ...
        if (!_exists(tokenId)) {
            ...
            '{"name": "NUMR ',
            utils.uint2str(tokenId),
            '", "description": "The Unique Number NFTs Collection.", "attributes":[{"trait_type": "', getType(tokenId),'", "max_value": 999999, "tokenId": ',
            utils.uint2str(tokenId),
            ', "Power": ', utils.uint2str(getPower(tokenId)),
            ', "Tier": ', getTier(tokenId),'}], "image": "https://img.numr.io/',
            utils.uint2str(tokenId),
            '"}'
        ));
        }
