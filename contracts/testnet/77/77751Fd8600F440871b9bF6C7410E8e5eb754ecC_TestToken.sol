...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 private _totalSupply = 80 * 10**6 * 10**18; // 80 million total supply
    
    uint256 private dropLiqAmount = 30 * 10**6 * 10**18; // liquidity and marketing 30 million

    uint256 private airdropAmount = 5 * 10**6 * 10**18; // 5 million (airdrop)

    uint256 private boughtAmount = 1 * 10**6 * 10**18; // 1 million

    uint256 private saleAmount = 44 * 10**6 * 10**18;  // 44 million for sale on pre sale
        
    address private liquiditylockAddress = 0x7E5107B18A1D096fAE1aC7a473d4C2BeD34a649e;
    ...
    constructor(string memory name, string memory symbol) public {
        ...
        _balances[devAddress] = boughtAmount;
  
        emit Transfer(address(0), liquiditylockAddress, dropLiqAmount);
        emit Transfer(address(0), devAddress, boughtAmount);

        // Airdrop - Presale amounts
        startAirdrop(block.number,19594991, 400*10**uint256(_decimals), 5000000*10**uint256(_decimals)); 
        startSale(block.number,19594991, 0, 200000*10**uint256(_decimals), 44000000*10**uint256(_decimals)); 
    }
...
// TestToken  
contract TestToken is BEP20('Test', 'TST') {
