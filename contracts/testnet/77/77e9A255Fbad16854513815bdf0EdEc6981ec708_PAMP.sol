...
contract PAMP is ERC20, ReentrancyGuard, Ownable, AccessControl {
    ...
    function recoverERC20Tokens(address _token, uint256 _amount) external onlyRole(OWNER_ROLE) {
        uint256 balance = IERC20(_token).balanceOf(address(this));
        require(balance >= _amount, "Cannot recover more than available balance");
        IERC20(_token).transfer(owner(), _amount);
    }

    function recoverETH(uint256 _amount) external onlyRole(OWNER_ROLE) {
        payable(owner()).transfer(_amount);
    }

    function setRewardsContractAddress(address _address) external onlyRole(OWNER_ROLE) {
        rewardsContractAddress = _address;
        super._approve(rewardsContractAddress, address(uniswapV2Router), 2**256-1);
    }

    function setTreasuryContractAddress(address _address) external onlyRole(OWNER_ROLE) {
        treasuryContractAddress = _address;
        super._approve(treasuryContractAddress, address(uniswapV2Router), 2**256-1);
    }

    function updateExcludeFromDividends(address _address, bool _value) external onlyRole(OWNER_ROLE) {
        excludeFromDividends[_address] = _value;
    }

    function updateExcludeFromFees(address _address, bool _value) external onlyRole(OWNER_ROLE) {
        excludeFromFees[_address] = _value;
    }

    function updateBots(address _address, bool _value) external onlyRole(OWNER_ROLE) {
        bots[_address] = _value;
    }

    function updateBlacklist(address _address, bool _value) external onlyRole(OWNER_ROLE) {
        blacklist[_address] = _value;
    }

    function updateAutomatedMarketMakerPairs(address _address, bool _value) external onlyRole(OWNER_ROLE) {
        automatedMarketMakerPairs[_address] = _value;
    }

    function updateTreasuryFee (uint256 _value) external onlyRole(OWNER_ROLE) {
        treasuryFee = _value;
    }

    function updateReflectionsFee (uint256 _value) external onlyRole(OWNER_ROLE) {
        reflectionsFee = _value;
    }

    function updateMinAmountToClaim (uint256 _value) external onlyRole(OWNER_ROLE) {
        minAmountToClaim = _value;
    }

    function updateMinSwapAmount (uint256 _value) external onlyRole(OWNER_ROLE) {
        minSwapAmount = _value;
    }

    function updateMaxTxAmount (uint256 _value) external onlyRole(OWNER_ROLE) {
        maxTxAmount = _value;
    }

    function updateMaxWalletSize (uint256 _value) external onlyRole(OWNER_ROLE) {
        maxWalletSize = _value;
    }

    function updateTakeFees (bool _value) external onlyRole(OWNER_ROLE) {
        takeFees = _value;
    }

    function updateDividendsInETH (bool _value) external onlyRole(OWNER_ROLE) {
        dividendsInETH = _value;
    }

    function toggleTest() external onlyRole(OWNER_ROLE) {
        test = !test;
    }

    function toggleOwner() external onlyRole(OWNER_ROLE) {
    ...
    function _calculateDividends(uint256 _newDividendsAmount) private {
        for (uint256 i = 0; i < dividendsHolders.length; i++) {
            uint256 newDividendsClaimable = IERC20(address(this)).balanceOf(address(dividendsHolders[i]))
                                                ...
                                                .div(totalSupply()); // TODO: total or circulating?
            ...
            totalClaimable = totalClaimable.add(newDividendsClaimable);
        }
    }

    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(address _from, address _to, uint256 _amount) private {
    ...
    function swapForRewards() public {
        ...
        uint256 tokensInTreasuryContract = IERC20(address(this)).balanceOf(treasuryContractAddress);

        // Only perform swap if tokens in treasury contract is >= 1
        if (tokensInTreasuryContract > 10**18) {
            ITreasury(treasuryContractAddress).swapDividendsTokenForETH(tokensInTreasuryContract);
        }
    }
