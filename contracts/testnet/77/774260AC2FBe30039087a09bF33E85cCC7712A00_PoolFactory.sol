...
contract PoolManagerLogic is Initializable, IPoolManagerLogic, ISupportedAsset, IFeeManage, PoolMember {
    ...
    uint256 internal immutable TYPE_FEE_PERFORMANCE = 1;
    uint256 internal immutable TYPE_FEE_MANAGEMENT = 2;
    uint256 internal immutable TYPE_FEE_ENTRY = 3;
    uint256 internal immutable TYPE_FEE_EXIT_SPECIFIC = 4;
    uint256 internal immutable TYPE_FEE_EXIT_SHARE_IN_KIND = 5;
    ...
    function setPerformanceFee(uint256 feeRate, address recipient) public onlyManager {
        _changeFee(Fee(TYPE_FEE_PERFORMANCE, feeRate, recipient));
    }
    ...
    function _changeFee(Fee memory _fee) internal {
        ...
        uint256 oldFee;
        if (feeType == TYPE_FEE_PERFORMANCE) {
            oldFee = performanceFee;
            performanceFee = _fee.feeRate;
        } else if (feeType == TYPE_FEE_MANAGEMENT) {
            oldFee = managementFee;
            managementFee = _fee.feeRate;
        } else if (feeType == TYPE_FEE_ENTRY) {
            oldFee = entryFee;
            entryFee = _fee.feeRate;
        } else if (feeType == TYPE_FEE_EXIT_SPECIFIC) {
            oldFee = exitSpecificFee;
            exitSpecificFee = _fee.feeRate;
        } else if (feeType == TYPE_FEE_EXIT_SHARE_IN_KIND) {
    ...
    {
        require(performanceFee != 0, "no performance fee");
