...
import "../contracts/interfaces/IUSDyVault.sol";
import "../contracts/interfaces/IAdmin.sol";
import "hardhat/console.sol";
/**
@dev TemporaryHolding contract 
 */
contract TemporaryHolding is AccessControl, Initializable {
    bytes32 public constant WITHDRAW_ROLE = keccak256("WITHDRAW_ROLE"); //role byte for admin
    uint256 public rebaseRate; //rebaserate
    uint256 public _rebaseRate; //_rebaserate
    address USDY_VAULT; //USDy vault address
    address USDyBUSD_VAULT; //USDyBUSD vault address
    address public YSL;
    address public xYSL;
    address public OptVault;
    IAdmin public Admin;
    /**
    ...
    function initialize(
        ...
        _setupRole(WITHDRAW_ROLE, IAdmin(Admin).bYSLVault());
    }
    /**
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        IUSDY(IAdmin(Admin).USDy()).transfer(recipient, IUSDY(IAdmin(Admin).USDy()).balanceOf(address(this)));
    }
    /**
    ...
    @param recipient address of recipient
     */
    function withdrawYSLAndxYSL(address recipient)
        external
        onlyRole(WITHDRAW_ROLE)
    {
        ...
        IERC20(IAdmin(Admin).YSL()).transfer(recipient, IERC20(IAdmin(Admin).YSL()).balanceOf(address(this)));
        IERC20(IAdmin(Admin).xYSL()).transfer(recipient, IERC20(IAdmin(Admin).xYSL()).balanceOf(address(this)));
    }
