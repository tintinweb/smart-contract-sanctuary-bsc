...
contract BCLLand is ERC721, Ownable{
    ...
    function activenft(bool _active) public onlyOwner returns(bool){
        isactiveNFT = _active;
        return true;
    }
    address public support;
    bool public isactiveNFT = false;
    function CreateCountry(uint256 _tokenID,string memory _name,uint256 totalblock,string memory _plan_cid,bool iscountrystatus,address _countryowner) public returns(bool){
        onlyvalidUser(_countryowner);
        string memory planid = _plan_cid;
        string memory newname = string(abi.encodePacked("BCL_",_name));
        if(isactiveNFT){
            require(nftcontract(nftaddress).BurnToken(_tokenID,_countryowner),"is not burn tokenid");
        }
        
        require(!isminted[newname],"country is minted");

        address _countryAddress = Deploymentcontract(DeploymentAddress).CountryContract(_countryowner,citizencontract,newname);
        ...
        bool s = Citizenship(citizencontract).createcountry(newname,_countryowner,_countryAddress);
    ...
    function extraMintBlock(uint256 _countryID,int[] memory x,int[] memory y,bool[] memory _isstatus,address[] memory _address)public returns(bool){
        ...
        require(countryInfo[_countryID].owner == msg.sender || owner() == msg.sender,"is not country owner");
        ...
        for(uint256 i=0;i<x.length;i++){
            ...
            _id[x[i]][y[i]] = id;
            creator[id] = _address[i];
            _mint(_address[i], id);
            emit onMint(id, "", _address[i],x[i],y[i],_isstatus[i]);
            Citizenship(citizencontract).ChangeTotalBlock(_address[i],x.length,"SUM");
        }
    ...
    function LockTheCountry(uint256 _countryId,bool _status) public onlyOwner returns(bool){
        ...
        bytes memory payload1 = abi.encodeWithSignature("LockTheCountry(uint256,bool)",_countryId,_status);
    ...
    function transferFrom(address from,address to,uint256 _tokenid) public override {
        ...
        require(ownerOf(_tokenid) == from,"is only call countryAddress");
        ...
        if (countryInfo[country].countryAddress == to){
            ...
            require(from == msg.sender,"is not tokenid owner call");
            onlyvalidUser(from);
            onlyvalidUser(to);
            Citizenship(citizencontract).ChangeTotalBlock(from,1,"SUB");
            Citizenship(citizencontract).ChangeTotalBlock(to,1,"SUM");
    ...
    event RevokeBlock(uint256 tid,int256 xaxis,int256 yaxis,address owner);
    function Revokeblock(uint256[] memory _tokenId) public returns(bool){
        ...
        for(uint256 i=0;i<_tokenId.length;i++){
            ...
            iscountry[BlockInfo[_tokenId[i]].contryId] = false;
            isminted[countryInfo[BlockInfo[_tokenId[i]].contryId].name] = false;
            delete BlockInfo[_tokenId[i]];
            emit RevokeBlock( _tokenId[i],BlockInfo[_tokenId[i]].x,BlockInfo[_tokenId[i]].y,ownerOf(_tokenId[i]));
            _burn(_tokenId[i]);
        }
        return true;
    }
    ...
    event ChangeDeploymentAddress(address newaddress,address oldaddress);
    function changeDeploymentAddress(address _DeploymentAddress) public onlyOwner returns(bool){
        address oldaddress = DeploymentAddress;
        DeploymentAddress = _DeploymentAddress;
        emit ChangeDeploymentAddress(DeploymentAddress, oldaddress);
        return true;
    }
