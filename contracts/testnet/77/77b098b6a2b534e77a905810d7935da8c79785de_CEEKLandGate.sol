...
contract CEEKLandGate is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
	...
	constructor(
        ...
        IERC721 _contract_ceekland, address _contract_decoder, address _swapRouter, address _swapFactory,
    ...
    function getExchangeRate(address currency) public view returns (uint256) {
        address pairAddress = IUniswapV2Factory(swapFactory).getPair(address(contract_payment_token), currency);

        bytes memory resultReserves = Address.functionStaticCall(pairAddress, abi.encodeWithSignature("getReserves()"), "CEEKLANDGATE_30");

        (uint256 reserve0, uint256 reserve1,) = abi.decode(resultReserves, (uint112,uint112,uint32));

        bytes memory resultToken0 = Address.functionStaticCall(pairAddress, abi.encodeWithSignature("token0"), "CEEKLANDGATE_31");
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet, bytes memory ceekUserIdParam, address[] memory paymentTokenNft) public {	
    ...
    function beforeBuyCheck(uint256 tokenId, uint256 tokenPrice, bytes memory ceekUserIdParam, address paymentTokenAddress) private returns(address, string memory, Currency memory) {
        // is on sale
        bool onSale = false;
    ...
    function getPriceInCurrency(uint256 tokenId, EIP20 payment_token) public view returns (uint256) {
        ...
        if (payment_token == contract_payment_token) {
            ...
            path[0] = address(contract_payment_token);
            path[1] = address(payment_token);

            uint256[] memory exRates = IUniswapV2Router(swapRouter).getAmountsOut(1000000000000000000, path);

            token_price = sellBidPrice[tokenId].mul(exRates[1]).div(exRates[0]);
    ...
    // Buy option
    function buy(uint256 tokenId, bytes memory ceekUserIdParam, EIP20 payment_token) public payable nonReentrant {
        uint256 token_price;
        uint256 i;
        bool ptOk1 = false;
        ...
        for (i=0;i<payment_tokens_nft[tokenId].length;i++) {
            if (address(payment_tokens_nft[tokenId][i])==address(payment_token)) {
                ptOk1 = true;
                break;
            }
        }
        ...
        (address tokenOwner, string memory ceekUserId, Currency memory saleBidPriceCurrency) = beforeBuyCheck(tokenId,token_price,ceekUserIdParam,address(payment_token));
    ...
    function updateAdmin(EIP20 _payment_token, EIP20[] memory _payment_tokens, IERC721 _contract_ceekland,
                         ...
                         address _wbnbToken, address _swapRouter) public {
