...
contract GOONGYE_NFT_STAKING is Ownable{
    ...
    function rewardOfUser(address Add) public view returns(uint256) {
        ...
        for(uint256 i = 0 ; i < Tokenid[Add].length ; i++){
            ...
            {
             RewardToken += (((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(time)))*5 ether;     
            }

            else if(Tokenid[Add][i] > common && Tokenid[Add][i] <= trare )
            {
             RewardToken += (((block.timestamp - (stakingTime[Add][Tokenid[Add][i]])).div(time)))*3 ether;     
            }
    ...
    ////////////    Functions to change the collection range   ////////////

    function set_common(uint256 _range) public{
        common = _range;
    }
    ...
    NFT ADDRESS : 
    0xd01c8E3D3aECac2A8E574C863e6EFA6167695b25
