...
interface RANDOM_RATE {
    ...
    function getBlueprintPool(uint16 _number) external view returns (uint16);
...
contract GashaponOpener is Ownable {
    ...
    mapping(uint8 => address) public randomRateAddress;
    ...
    function changeRandomRateWeap(address _address) public onlyOwner {
        randomRateAddress[BLUEPRINT_RARE] = _address;
    ...
    function generateNFT() internal {
        uint256 _randomNumber = RANDOM_CONTRACT(randomWorkerContract)
            .startRandom();

        string memory _partCode = createNFTCode(_randomNumber);
        mintNFT(msg.sender, _partCode);
        emit OpenBox(msg.sender, _partCode);
    }

    function openGasha() public {
        uint256 _balance = IERC20(lakrimaTokenContract).balanceOf(msg.sender);
        require(
    ...
    function createNFTCode(uint256 _randomNumber)
        internal
        view
        returns (string memory)
    {
        ...
        // uint16 eTypeId = RANDOM_RATE(randomRateAddress[BLUEPRINT_RARE]).getEquipmentPool(equipmentRandom); // 02 04

        // //BLUEPRINT_RARE
        uint16 ePartId = RANDOM_RATE(randomRateAddress[BLUEPRINT_RARE])
            .getBlueprintPool(equipmentRandom);

        partCode = createBlueprintPartCode(ePartId);
    ...
    function createBlueprintPartCode(uint16 equipmentPartId)
        private
        pure
        returns (string memory)
    {
