...
contract MFCBuyback is BackendAgent, RegistrarClient {
  ...
  /**
   * @dev Buyback offer from user exchange
   */
  function buybackOffer(uint256 offerId, address seller, uint256 amountBUSD) external onlyBackendAgents {
    _buybackOffer(offerId, seller, amountBUSD);
  }

  /**
   * @dev Buyback offer from exchange cap
   */
  function buybackOffer(uint256 offerId, uint256 amountBUSD) external onlyBackendAgents {
    _buybackOffer(offerId, address(0), amountBUSD);
  }

  function _buybackOffer(uint256 offerId, address seller, uint256 amountBUSD) private {
    require(amountBUSD > 0, "Invalid quantity");

    uint256 maxAmountBUSD = 0;
    if (seller == address(0)) {
      MFCExchangeCap.Offer memory offer = _mfcExchangeCap.getOffer(offerId);
      require(offer.isOpen, "Invalid offer");
      maxAmountBUSD = offer.quantity * offer.price / MULTIPLIER;
    } else {
    ...
    _mfcMembership.withdrawBuybackCredits(amountBUSD);

    // step 2: buy MFC from user exchange
    require(amountBUSD <= maxAmountBUSD, "Not enough to sell");
    if (seller == address(0)) {
      _busd.approve(address(_mfcExchangeCap), amountBUSD);
      _mfcExchangeCap.tradeOffer(offerId, amountBUSD);
    } else {
      _busd.approve(address(_mfcExchange), amountBUSD);
      _mfcExchange.tradeOffer(offerId, seller, amountBUSD);
    }

    // step 3: transfer MFC to MFCExchangeCap
    uint256 mfcBalance = _mfc.balanceOf(address(this));
    if (mfcBalance > 0) {
      _mfc.transfer(address(_mfcExchangeCap), mfcBalance);
    }
  }

  function updateAddresses() public override onlyRegistrar {
    _mfcMembership = MFCMembership(_registrar.getMFCMembership());
    _mfcExchange = MFCExchange(_registrar.getMFCExchange());
    _mfcExchangeCap = MFCExchangeCap(_registrar.getMFCExchangeCap());
    _mfc = IBEP20(_registrar.getMFCToken());
  }
}
...
contract MFCCollateralLoan is BackendAgent, ExchangeCheck, RegistrarClient {
