...
contract bardanoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("bardano_Dividend_Tracker", "bardano_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "bardano_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "bardano_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main bardano contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "bardano_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "bardano_Dividend_Tracker: Cannot update claimWait to same value");
...
contract bardano is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    bardanoDividendTracker public dividendTracker;
    ...
    constructor() ERC20("bardano", "bardano") 
    {
        BNBRewardsFee = 4;
        liquidityFee = 2;
        marketingFee = 3;
        ...
        marketingWallet = payable(0xb905b51423876B3CEf2bD3F3f44C9C1E4bf6E477);
        developerWallet = payable(0x4B54E3cd6910f47484C78de0C9cb2e405753eb8B);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee); // total fee transfer and buy

    	dividendTracker = new bardanoDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "bardano: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "bardano: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "bardano: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "bardano: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "bardano: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "bardano: Cannot update gasForProcessing to same value");
