...
contract DBSNFTCast  is Ownable{
    ...
    function Cast() public  returns (bool){
        ...
        if(CastShip[msg.sender] == 0){
            // Transfer the token
            token.safeTransferFrom(msg.sender, pveAddress, 110*(10**uint256(18)));
    ...
    function GetCast() public  returns (uint256, uint256){
        ...
        if(oldTime.add(7*24*3600) >= block.timestamp){
            uint256 id = techAddress.GetRandomShip(NFTSilver);
            uint256[] memory tokens = nftAddress.mint(msg.sender, id, 1);
            CastShip[msg.sender] = 0;
            emit newCast(msg.sender, tokens[0], id);
            return (tokens[0], id);
        }
