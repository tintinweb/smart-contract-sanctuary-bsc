...
import "./interfaces/IDiamondFacetsRegistryv2.sol";
import "./acl/OmniteAccessControl.sol";

contract DiamondFacetsRegistryv2 is IDiamondFacetsRegistryv2 {
    using EnumerableMap for EnumerableMap.Bytes32ToBytes32Map;
    using EnumerableMap for EnumerableMap.Bytes32ToUintMap;
    using EnumerableSet for EnumerableSet.Bytes32Set;

    EnumerableMap.Bytes32ToBytes32Map internal facetIdToFacetCategory;
    EnumerableMap.Bytes32ToUintMap internal facetCategoryToFacetsAmount;
    mapping(bytes32 => FacetData) public facetIdToFacetData;
    ...
    modifier facetIsNotRegistered(bytes32 facetId) {
        if (facetRegistered(facetId)) {
            revert FacetAlreadyRegistered();
        }
        _;
    }

    modifier isValidId(bytes32 _facetId) {
        if (_facetId == bytes32(0)) {
            revert InvalidId();
        }
        _;
    }

    modifier facetIsRegistered(bytes32 facetId) {
        if (!facetRegistered(facetId)) {
            revert FacetNotRegistered();
        }
        _;
    }

    function facetData(bytes32 facetId)
        external
        view
        facetIsRegistered(facetId)
        returns (FacetData memory)
    {
        return facetIdToFacetData[facetId];
    }

    function facetRegistered(bytes32 facetId) public view returns (bool) {
        return facetIdToFacetCategory.contains(facetId);
    }

    function registerFacet(
        bytes32 facetId,
        bytes32 facetCategory,
        FacetData calldata data
    )
        external
        hasFacetsRegistryEditorRole
        isValidId(facetId)
        isValidId(facetCategory)
        facetIsNotRegistered(facetId)
    {
        facetIdToFacetCategory.set(facetId, facetCategory);
        facetIdToFacetData[facetId] = data;

        uint256 facetCategoryAmount = facetCategoryToFacetsAmount.contains(
            facetCategory
        )
            ? facetCategoryToFacetsAmount.get(facetCategory)
            : 0;

        facetCategoryToFacetsAmount.set(facetCategory, facetCategoryAmount + 1);

        emit FacetRegistered(facetId, facetCategory);
    }
    ...
    function removeFacet(bytes32 facetId)
        ...
        facetIsRegistered(facetId)
    {
        bytes32 facetCategory = facetIdToFacetCategory.get(facetId);
        facetIdToFacetCategory.remove(facetId);
        delete facetIdToFacetData[facetId];

        facetCategoryToFacetsAmount.set(
            facetCategory,
            facetCategoryToFacetsAmount.get(facetCategory) - 1
        );

        emit FacetRemoved(facetId, facetCategory);
    }

    function replaceFacet(bytes32 facetId, FacetData calldata data)
        external
        hasFacetsRegistryEditorRole
        isValidId(facetId)
        facetIsRegistered(facetId)
    {
        ...
        emit FacetReplaced(facetId, facetIdToFacetCategory.get(facetId));
    }

    function getAllCategoires() public view returns (bytes32[] memory) {
        return facetCategoryToFacetsAmount._inner._keys.values();
    }

    function getAllFromCategory(bytes32 facetCategory)
        external
        view
        returns (FacetData[] memory)
    {
        (bool success, uint256 facetsLength) = facetCategoryToFacetsAmount
            .tryGet(facetCategory);

        FacetData[] memory facets = new FacetData[](facetsLength);

        if (!success) {
            return facets;
        }

        uint256 lastFacetIndex = 0;
        for (uint256 i = 0; i < facetIdToFacetCategory.length(); ++i) {
            (
                bytes32 currentFacetId,
                bytes32 currentFacetCategroy
            ) = facetIdToFacetCategory.at(i);

            if (currentFacetCategroy == facetCategory) {
                facets[lastFacetIndex] = facetIdToFacetData[currentFacetId];
                ++lastFacetIndex;
            }
        }
    ...
    function getAllFacets()
        external
        view
        returns (FacetDataWithCategory[] memory)
    {
        FacetDataWithCategory[] memory facets = new FacetDataWithCategory[](
            facetIdToFacetCategory.length()
        );
        ...
        for (uint256 i = 0; i < facets.length; ++i) {
            ...
            FacetData memory data = facetIdToFacetData[currentFacetId];

            facets[i] = FacetDataWithCategory(
                data.facetAddress,
                currentFacetCategroy,
...
interface IDiamondFacetsRegistryv2 {
    error FacetNotRegistered();
    error FacetAlreadyRegistered();
    error InvalidId();

    event FacetRegistered(bytes32 facetId, bytes32 facetCategory);
    event FacetReplaced(bytes32 facetId, bytes32 facetCategory);
    event FacetRemoved(bytes32 facetId, bytes32 facetCategory);
    ...
    function facetRegistered(bytes32 facetId) external view returns (bool);

    function replaceFacet(bytes32 facetId, FacetData calldata data) external;

    function getAllCategoires() external view returns (bytes32[] memory);
