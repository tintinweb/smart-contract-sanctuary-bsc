...
interface RichRabbit_Interface {
    ...
    function viewAccountTotal() external view returns(uint256);
    function viewDividendTotal() external view returns(uint256);
    function createAccount(uint tokenID) external returns(bool);
...
contract RichRabbitDividend is RichRabbit_Interface {
    ...
    address private tokenAddress_ = address(0x344C1C18DeB448e0B5619beef538d7EF765C2C6b);
    ...
    function viewAccountTotal() public view returns(uint256){
        uint256 total=0;
    ...
    function createAccount(uint tokenID) public returns(bool){
        require(!accounts[tokenID].exist);
        require(tokenID < firstCollection.totalSupply());
        accounts[tokenID] = Tokens(0,true);
        return true;
    }
    ...
    function reDistribution() public returns(bool) {
        ...
        totalSupply = firstCollection.totalSupply();
        uint256 single_share = tokenAmount_.div(totalSupply);
        callFromFallback(single_share);
        if (!accounts[totalSupply-1].exist){
            createAccount(totalSupply-1);
        }
        return true;
    }   
