...
contract StopLimitOrder is BoringOwnable, BoringBatchable {
    ...
    {
        {
            if (order.oracleAddress != IOracle(0)) {
                (bool success, uint256 rate) = order.oracleAddress.get(
                    order.oracleData
                );
                require(
        ...
        amountToBeReturned =
            order.amountOut.mul(amountToBeFilled) /
            order.amountIn;
        // Effects
        ...
        bentoBox.transfer(
            ...
            order.maker,
            address(receiver),
            bentoBox.toShare(tokenIn, amountToBeFilled, true)
        );
    ...
    function _fillOrderInternal(
        ...
        require(
            bentoBox.balanceOf(tokenOut, address(this)) >=
                bentoBox
                    .toShare(tokenOut, amountToBeReturned.add(fee), false)
                    .add(_feesCollected),
            "Limit: not enough"
        );
    }
    ...
    function fillOrderOpen(
        ...
        uint256 _feesCollected = _fillOrderInternal(
            tokenIn,
            tokenOut,
            receiver,
            data,
        ...
        feesCollected[tokenOut] = _feesCollected.add(
            bentoBox.toShare(tokenOut, fee, false)
        );
    ...
    function batchFillOrder(
        ...
        for (uint256 i = 0; i < order.length; i++) {
            uint256 amountToBeFilled;
            (, amountToBeReturned[i], amountToBeFilled) = _preFillOrder(
                order[i],
                tokenIn,
                tokenOut,
                receiver
    ...
    function batchFillOrderOpen(
        ...
        uint256 totalFee = totalAmountToBeReturned.mul(externalOrderFee) /
            FEE_DIVISOR;

        {
            ...
            feesCollected[tokenOut] = _feesCollected.add(
                bentoBox.toShare(tokenOut, totalFee, false)
            );
        }
