...
contract ERC20 is Context, IERC20, Ownable {
  ...
  // PancakeSwapTestnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 https://pancake.kiemtienonline360.com/#/swap
  ...
  function _takeFeeOnSwap(
    ...
    if (developmentFee == 0 && rewardsFee == 0 && marketingFee == 0) return _amount;
    uint256 _amountToContract = _amount.mul(developmentFee.add(rewardsFee).add(marketingFee)).div(100);
  ...
  function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
    ...
    uint256 amountDueDev = (ethBalanceUpdate.mul(developmentFee)).div(developmentFee.add(rewardsFee).add(marketingFee));
    uint256 amountDueMarketing = (ethBalanceUpdate.mul(marketingFee)).div(developmentFee.add(rewardsFee).add(marketingFee));
    uint256 amountDueRewards = ethBalanceUpdate.sub(amountDueDev).sub(amountDueMarketing);
    ...
    if (amountDueMarketing > 0) {
      _sendETHToMarketing(amountDueMarketing);
    }
  }
  ...
  function calculateRewardsShare(address wallet) public view returns(uint256) {
    uint256 rewardsShare = (address(this).balance).mul(balanceOf(wallet)).div(_totalSupply.sub(balanceOf(deadAddress)));
    return rewardsShare;
  }
