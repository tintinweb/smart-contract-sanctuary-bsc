...
contract WealthMountainLiquid is Ownable, ReentrancyGuard {
    ...
    constructor(){
        // BUSD = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // PRODUCTION
        BUSD = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // TEST
        // InvWallet = 0x64b7A3CD189a886438243F0337b64f7ddf1B18D3; // PRODUCTION
        InvWallet = 0x859DFde6953F90649Da1a4C6d5685CD3E5c24694; // TEST
        dailyValueTime = block.timestamp;
        leftOverDayLimit = 180; //change to 30 days after testing
        CharityAddress = 0x000000000000000000000000000000000000dEaD;
    }

    function DepoLiqProfits(uint256 profamount) public noReentrant {
        require (profamount > 5000); // add ether after testing
        BUSD.safeTransferFrom(msg.sender, address(this), profamount); 
        ...
        profit.lpendtime = block.timestamp + leftOverDayLimit;
        leftoverFunds = 0;
    ...
    function StakeLiq(uint256 stakeamt, address reffy) public noReentrant{
        ...
        require (stakeamt > 50, "Minimum stake amount not met."); // add ether after testing
        require (reffy != msg.sender, "Ref can not be yourself");
        ...
        if (reffy != 0x000000000000000000000000000000000000dEaD && ref.totalStaked > 50){ // add ether after testing
            liqf = liqf.sub(r);
            ref.refearned += r;
        }

        BUSD.safeTransfer(InvWallet, liqf);
    ...
    function WithdrawInitialLiq(uint256 key) public noReentrant {
        ...
        for (uint256 i = 0; i < user.LiqStakeList.length; i++){
            if (user.LiqStakeList[i].id == key){
                require (user.LiqStakeList[i].initialWithdrawn == false, "Already withdrawn!");
    ...
    function WithdrawLiqProfit(uint256 key) public noReentrant {
        ...
        require (LiqProfTaken[msg.sender][key] == false, "Already pulled!");
        uint256 halfvar;
        uint256 qualifiers;
        for (uint256 i = 0; i < user.LiqStakeList.length; i++){
        ...
        if (ctrigger == true){
            ...
            BUSD.safeTransfer(CharityAddress, damt);
            qualifiedamt = profitRecalc(qualifiedamt);
        }
        user.totalWithdrawn += qualifiedamt;
        user.rewardsClaimed.push(qualifiedamt);
        BUSD.safeTransfer(msg.sender, qualifiedamt);
        addDailyValues();
    }

    function returnUserClaims(address userAddress) public view returns (uint256[] memory claimList) {
        LiqUser storage user = LiqUserKey[userAddress];

        return user.rewardsClaimed;
    }
    ...
    function calcdivProf(address calcaddy, uint256 ckey) public view returns (uint256) {
        ...
        uint256 qualifiers;
        if (LiqProfTaken[msg.sender][ckey] == false){
            for (uint256 i = 0; i < user.LiqStakeList.length; i++){
                if (user.LiqStakeList[i].initialWithdrawn == false && user.LiqStakeList[i].staketime < profit.lptimestamp){
                    uint256 diff = (block.timestamp).sub(user.LiqStakeList[i].staketime);
                    if (diff < sixtydays){
     ...
     function WithdrawRefs() public noReentrant {	
        ...
        BUSD.safeTransfer(msg.sender, wdamt);	
    }
    ...
    function transferLeftoverLP(uint256 lkey) public onlyOwner {
        ...
        uint256 diff;
        if (lp.lpendtime < block.timestamp){
