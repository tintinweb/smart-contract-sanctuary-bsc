    ...
    IFlashClaim
{
    ...
    function flashClaim(uint256 bundleId, address to, bytes calldata params) external override nonReentrant {
        // step 1 checks
        require(ownerOf(bundleId) == msg.sender, "Unauthorized");


        // step 2 tracking and transferring user's asset
        ERC20Holding[] memory erc20Assets = bundleERC20Holdings[bundleId];
        uint256[] memory formerErc20Balances = new uint256[](erc20Assets.length);
        for (uint256 i = 0; i < erc20Assets.length;) {
            IERC20 token = IERC20(erc20Assets[i].tokenAddress);
            uint256 balance = token.balanceOf(address(this));
            formerErc20Balances[i] = balance;
            token.safeTransfer(to, erc20Assets[i].amount);
            unchecked { ++i; }
        }

        ERC721Holding[] memory erc721Assets = bundleERC721Holdings[bundleId];
        for (uint256 i = 0; i < erc721Assets.length;) {
            IERC721(erc721Assets[i].tokenAddress).safeTransferFrom(address(this), to, erc721Assets[i].tokenId);
            unchecked { ++i; }
        }
        
        ERC1155Holding[] memory erc1155Assets = bundleERC1155Holdings[bundleId];
        uint256[] memory formerErc1155Balances = new uint256[](erc1155Assets.length);
        for (uint256 i = 0; i < erc1155Assets.length;) {
            IERC1155 token = IERC1155(erc1155Assets[i].tokenAddress);
            formerErc1155Balances[i] = token.balanceOf(address(this), erc1155Assets[i].tokenId);
            token.safeTransferFrom(address(this), to, erc1155Assets[i].tokenId, erc1155Assets[i].amount, "");
            unchecked { ++i; }
        }

        uint256 ethAsset = bundleETHHoldings[bundleId];
        uint256 formerBalance = address(this).balance;
        if (ethAsset > 0) {
            // solhint-disable-next-line avoid-low-level-calls
            (bool sent,) = to.call{value: ethAsset}("");
            require(sent, "Failed to send Ether");
        }


        // step 3 execute flash claim hooks
        IFlashClaimReceiver(to).onFlashClaim(
            IFlashClaimReceiver.Assets({
                erc20Assets: erc20Assets,
                erc721Assets: erc721Assets,
                erc1155Assets: erc1155Assets,
                ethAmount: ethAsset
            }), 
            msg.sender, 
            params
        ...
        // step 4 check assets is sent back
        for (uint256 i = 0; i < erc20Assets.length;) {
            IERC20 token = IERC20(erc20Assets[i].tokenAddress);
            require(formerErc20Balances[i] == token.balanceOf(address(this)), "Did not receive enough ERC20 token");
            unchecked { ++i; }
        }

        for (uint256 i = 0; i < erc721Assets.length;) {
            require(IERC721(erc721Assets[i].tokenAddress).ownerOf(erc721Assets[i].tokenId) == address(this), "Did not receive ERC721 token");
            unchecked { ++i; }
        }

        for (uint256 i = 0; i < erc1155Assets.length;){
            IERC1155 token = IERC1155(erc1155Assets[i].tokenAddress);
            require(formerErc1155Balances[i] == token.balanceOf(address(this), erc1155Assets[i].tokenId), "Did not receive enough ERC1155 token");
            unchecked { ++i; }
        }

        if (ethAsset > 0) {
            require(formerBalance == address(this).balance, "Did not receive enough ETH");
        }
...
interface IFlashClaimReceiver is IAsset {

  struct Assets {
    ERC20Holding[] erc20Assets;
    ERC721Holding[] erc721Assets;
    ERC1155Holding[] erc1155Assets;
    uint256 ethAmount;
  }
