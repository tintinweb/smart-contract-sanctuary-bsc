...
contract SpinInu is Context, IERC20Metadata, Ownable {
    ...
    function totalFee() internal view returns (uint256) {
        return _marketingFeePercentage.add(_developmentFeePercentage).add(_treasuryPoolPercentage);
    }

    function setFees(uint256 marketingFeePercentage, uint256 developmentFeePercentage, uint256 treasuryPoolPercentage) public onlyOwner {
        require(_maxFeePercentage >= _marketingFeePercentage + _developmentFeePercentage + _treasuryPoolPercentage);
    ...
    function _takeFees(uint256 amount) private returns (uint256) {
        ...
        uint256 bnbToMarketing = receivedBNB.mul(marketingFee).div(totalFee);
    ...
    function _transferFeeType(address from, address to) internal view returns (uint8) {
