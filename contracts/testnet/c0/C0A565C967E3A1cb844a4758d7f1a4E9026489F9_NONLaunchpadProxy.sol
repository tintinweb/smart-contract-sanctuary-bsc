...
contract NONLaunchpadProxy is ILaunchpadProxy, Ownable, ReentrancyGuard {
    ...
    bytes4 internal constant PROXY_ID =
        bytes4(keccak256("NONLaunchpadProxyV10"));
    ...
    function processBuy(
        ...
        _updateAccountStats(lpad.id, roundsIdx, quantity, sender);
        //0 : 721, else 1155
        if (flag == 0) {
            ...
            // increase sale num
            _updateSaleQuantity(lpad.id, roundsIdx, saleQuantity);
        } else {
            DataType.LaunchpadRounds memory lpadRounds = lpad.roundss[
                roundsIdx
            ];
    ...
    function updateRoundsStartTimeAndFlags(
        ...
        require(
            flags.length == DataType.LPD_FLAG_MAX,
            Enums.LPD_ROUNDS_FLAGS_ARRAY_LEN
        );
    ...
    function updateStartTokenIdAndSaleQuantity(
        ...
        _updateSaleQuantity(launchpadId, roundsIdx, saleQuantity);
    ...
    function _updateAccountStats(
        ...
        require(!launchpads[launchpadId].lockParam, Enums.LPD_PARAM_LOCKED); // only when not locked
        uint32 totalBuyQty = launchpadVars[launchpadId]
            .accountRoundsStats[genRoundsAddressKey(sender, roundsIdx)]
            .totalBuyQty;
        launchpadVars[launchpadId]
            .accountRoundsStats[genRoundsAddressKey(sender, roundsIdx)]
            .totalBuyQty = totalBuyQty + uint32(quantity);
        launchpadVars[launchpadId]
            .accountRoundsStats[genRoundsAddressKey(sender, roundsIdx)]
        ...
        )
    {
        ...
        boolData = new bool[](DataType.LPD_FLAG_MAX + 1);
        // launchpad enable
        boolData[0] = lpad.enable;
