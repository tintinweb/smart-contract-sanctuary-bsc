...
contract ICO is Ownable {      
    ...
    uint256 private constant dayPerCycle = 365 days; 
    uint256 private constant rewardAfter = 1 minutes; // 30 days;
    ...
    constructor () payable {
        ...
        addresses[total_user]=msg.sender;
        stakingToken = IERC20(0x59CF87Cb18638fFF8A137C728f90a877D6f52E01);
        stakingFromToken = IERC20(0x59CF87Cb18638fFF8A137C728f90a877D6f52E01);
    ...
    function deposit(uint256 amount) external returns (bool) {
     ...
     function unfreeze(uint256 ordrId)external returns(bool){
        ...
        User storage user = users[msg.sender];
        uint256 blnc = block.timestamp.sub(order.start).div(rewardAfter);
        uint256 newIncome = blnc.mul(rewardPer).div(baseDivider);

        if(newIncome>0 && order.amount > order.freezed.add(newIncome)){
            order.freezed = order.freezed.add(newIncome);
            user.commision = user.commision.add(newIncome);
            user.balance = user.balance.add(newIncome);
        }
