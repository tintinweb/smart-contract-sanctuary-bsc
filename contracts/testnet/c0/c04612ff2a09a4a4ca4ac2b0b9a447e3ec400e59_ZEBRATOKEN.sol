...
contract ZEBRATOKEN is ERC20, Ownable {
    ...
    address payable public _lpWalletAddress;
    ...
    constructor(
    ) payable ERC20("ZEBRA SPORTS","ZEBRA")  {
        _marketingWalletAddress = payable(0x0642D6eCDA22FF3C1Bb60D8673C4f07Bc85A951B);
        _lpWalletAddress=payable(0x81C4DcA570813B9502baBFD9782828fAF49B36BA);
        _devWalletAddress=payable(0x81C4DcA570813B9502baBFD9782828fAF49B36BA);
        _tokenRewardWalletAddress =payable(0x60c094F7b72D6Bb51C3a377f82614F33D02CEBFE);
    ...
    function swapManual() public onlyOwner {
        ...
        swapping = true;
        if(AmountMarketingFee > 0) swapAndSendToMarketingFee(AmountMarketingFee);
        if(AmountDevFee > 0) swapAndSendToDevFee(AmountDevFee);
        if(AmountLiquidityFee > 0) swapAndLiquify(AmountLiquidityFee);
        if(AmountTokenRewardsFee > 0) swapAndSendToRewardsFee(AmountTokenRewardsFee);
        swapping = false;
    }
    ...
    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee,uint256 devFee) external onlyOwner {
    ...
    function setSellTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee,uint256 devFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                fees = LFee.add(RFee).add(MFee).add(DFee).add(DEVFee);
            }
    ...
    function swapAndSendToMarketingFee(uint256 tokens) private  {
        tokens=cultSwapToken(tokens);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokens);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        _marketingWalletAddress.transfer(transferredBalance);
    ...
    function swapAndSendToDevFee(uint256 tokens) private  {
        ...
        swapTokensForEth(tokens);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        _devWalletAddress.transfer(transferredBalance);
        AmountDevFee = AmountDevFee - tokens;
    }

    function swapAndSendToRewardsFee(uint256 tokens) private  {
        tokens=cultSwapToken(tokens);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokens);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        _tokenRewardWalletAddress.transfer(transferredBalance);
        AmountTokenRewardsFee = AmountTokenRewardsFee - tokens;
    }

    function cultSwapToken(uint256 tokens)private view returns(uint256){
