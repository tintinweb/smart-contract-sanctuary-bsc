...
contract iPandaInitial is iPandaWhiteList {
    ...
    struct Init {
        ...
        uint iPandaForInitial; // the initial iPanda amount
    ...
    function finishInvest(uint8 _initId) public adminOnly {
        ...
        for (uint8 i = 0; i < whiteList.length; i++) {
            ...
            init[_initId].iPandaForInitialMember[_m] = init[_initId].iPandaForInitial * init[_initId].investAmount[_m] / init[_initId].initialTotalAmount;
        }
    }

    function setInit(uint8 _initId, uint _initialTotalAmount, uint _iPandaForInitial) public adminOnly {
        if (init[_initId].initId == 0) {
            initCount = _initId;
        }
        init[_initId].initId = _initId;
        init[_initId].initialTotalAmount = _initialTotalAmount; // the actual invest total amount of BUSD
        init[_initId].iPandaForInitial = _iPandaForInitial; // the initial iPanda amount
        init[_initId].initialFinished = false; // will be true after finished the initial invest.
    }

    function getInit(uint8 _initId) public adminOnly view returns (uint8, uint, uint, bool) {

        return (
            init[_initId].initId,
            init[_initId].initialTotalAmount, 
            init[_initId].iPandaForInitial,
    ...
    function claim(uint8 _initId) public memberOnly {
        ...
        init[_initId].iPandaClaimed[msg.sender] += amountToClaim;
        iPandaERC20.mint(msg.sender, amountToClaim);
    }

    //计算一个账户，在各轮IDO中未claimed的iPanda总和
    function _calculateAmountUnclaimed(address _addr) internal view returns (uint) {
    ...
    function _calculateTotalUnclaimed() internal view returns (uint) {
        ...
        for (uint i=0; i<whiteList.length; i++ ) {
            _totalUnclaimed += _calculateAmountUnclaimed(whiteList[i]);
        }
...
contract iPandaVote is iPandaInitial {
    ...
    function _updateVote(uint _id) private {
        ...
        for(uint i=0; i < whiteList.length; i++) {
            ...
            uint _balance = iPandaERC20.balanceOf(v) + _calculateAmountUnclaimed(v);
...
contract iPandaBonus is iPandaInitial {
    ...
    function calculateBonus(address _addr) public view returns (uint) {
        ...
        return ((bonusPerIPanda - iPandaDebt[_addr]) * (iPandaStaked[_addr] + _calculateAmountUnclaimed(_addr)) / 10**18);         
