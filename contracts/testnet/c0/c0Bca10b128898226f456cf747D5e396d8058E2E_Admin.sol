...
contract IDO is IIDO {
    ...
    function init(
        ...
        _idoProject.pvtTokenPrice = _limit.hardcap / _limit.totalSupply;
    ...
    function changeLPshareTo(uint16 share_100_for_1_) external initialized onlyMultisig  {
        require( _limit.profitPercentage != share_100_for_1_, "already same value!");
        // 10 -> 0.1% and 10000 -> 100%
        require(share_100_for_1_ > 10 && share_100_for_1_ < 10000, "Invalid percentage value!");
        _limit.profitPercentage = share_100_for_1_;
    }
    ...
    function setPublicTokenPrice(uint256 busdPrice_) external {
    ...
    function setNew_N_value(uint64 N_value_, bool isPrivate_) external onlyMultisig {
    ...
    function withdrawFunds() external onlyMultisig saleHasEnded {
    ...
    function burnLeftoverTokens() external saleHasEnded onlyMultisig {
        require(_idoProject.collected == _limit.hardcap, "Hardcap not reached!");
    ...
    function purchaseTokens(uint256 amount_) external saleIsOngoing {
        require(amount_ > 0, "Amount is 0");
        uint256 normalized = this.normalize(amount_, msg.sender); // in busd w dec
        // this fails when either already exceed max alloc or neither staked nor aloowlisted
        require(normalized > 0, "no tokens possible for you!");
        
        bool hcapXceeds = this.willHardcapExceedWith(normalized);
        if (hcapXceeds) {
            normalized = this.getRemainingAmount();
        }
        require(IERC20(_address.stableCoin).allowance(msg.sender, address(this)) >= normalized, "BUSD allowance low");
        _idoProject.balances[msg.sender] += amount_;
        _idoProject.collected += amount_;
        // transfer busd from sender to this contract
        // so approval is must
        IERC20(_address.stableCoin).safeTransferFrom(msg.sender, normalized);
        if (hcapXceeds) {
            _saleCompleted = true;
        }
    ...
    function normalize(uint256 busdAmount_, address participant_) external view returns (uint256) {
        uint256 allocation = this.getTokenAllocationFor(participant_); // in busd w dec
        uint256 prvPurchase = this.tokensPurchasedBy(participant_); // in busd w dec
        // now cap given busd amount, if already purchased some!
        if (prvPurchase + busdAmount_ > allocation) {
            busdAmount_ = busdAmount_ - ((prvPurchase + busdAmount_) - allocation);
        }
    ...
    function getTokenAllocationFor(address participant_) external view returns (uint256) {
        ...
        alloc = n * alloc * this.getTokenPrice();
        uint256 gAlloc = _idoProject.giftAllocForParticipant[participant_]; // in busd w dec
        if(alloc == 0) {
        ...
        uint256 maxAlloc = this.getMaxAllocation(); // in busd w dec
    ...
    function tokensPurchasedBy(address participant_) external view returns (uint256) {
        return _idoProject.balances[participant_] * this.busdDecimals();
    }

    // returns: max_allocation per per participant in busd
    function getMaxAllocation() external view returns (uint256) {
        return _limit.maxAllocationShare;
    }
    ...
    function willHardcapExceedWith(uint256 amount_) external view returns (bool) {
        return amount_ + _idoProject.collected > _limit.hardcap;
    }

    // returns: remaining sale tokens
    function getRemainingAmount() external view returns (uint256) {
        return _limit.hardcap - _idoProject.collected;
    }

    function getGiftAllocationFor(address participant_) external view  returns(uint256) {
        return _idoProject.giftAllocForParticipant[participant_];
    }
    ...
    function isPrivateSale() external view returns(bool) {
        return this._now() > _timing.pvtStart && this._now() < (_timing.pvtStart + _timing.pvtDuration);
    }

    // returns: info if any of sales is still ongoing
    function isLive() external view  returns (bool) {
        return this.isPrivateSale() || this.isPublicSale();
    ...
    function busdDecimals() external view returns (uint256) {
    ...
    modifier saleHasEnded() {
        ...
        require(_saleCompleted, "ido not completed yet");
        _;
    }
    ...
    modifier paramsAreValid(Address memory a_, Timing memory t_, Limit memory l_, bool pEn_) {
        ...
        require(l_.maxAllocationShare > 0, "max allocation per user cant be 0!");
        ...
        // 10 -> 0.1% and 10000 -> 100%
        require(l_.profitPercentage > 10 && l_.profitPercentage < 10000, "Invalid percentage value!");
        _;
    }
    // -------------------------- Events ------------------------------------------
