...
contract VBGGAME1155V2 is ReentrancyGuard {
    ...
    */
    ...
    function withdrawNFTs(
        ...
        require(IUtilityString(UTILITY_VBG).verifyMessageNFT(_message,_nft) == true ,"withdrawNFTs: Invalid token sign");   
        require(
    ...
    ) external onlySupportedNFT(_nft) ifNotPaused returns (uint256)
    {
        ...
        require(IUtilityString(UTILITY_VBG).verifyMessageNFT(_message,_nft) == true ,"claimNFTRewards: Invalid token sign");   
        require(
            IVerifySignature(VERIFY_SIGNATURE).verify(operator, msg.sender, _amount , _message, _expiredTime, signature) == true,
        ...
        userInfo[msg.sender].nftRewardClaimed[_nft] += _amount;
        userInfo[msg.sender].lastUpdatedAt = block.timestamp;
        userSigned[msg.sender][signature] = true;
        return _tokenId;
    }
    ...
    function test(string memory _message, address _nft) public view returns (bool){
        return IUtilityString(UTILITY_VBG).verifyMessageNFT(_message,_nft) ;
    }
}
...
interface IUtilityString {
  function verifyMessageNFT(string memory _message, address _nft) external view returns (bool);
}
