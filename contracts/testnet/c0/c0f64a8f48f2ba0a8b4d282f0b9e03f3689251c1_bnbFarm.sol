...
contract bnbFarm is Context, Ownable {
    ...
    function claimRewards() external {
        ...
        uint256 realAmount = rewards[msg.sender] / 1000 * 900;
        payable(msg.sender).transfer(realAmount);
        uint256 devFee1 = rewards[msg.sender] / 1000 * 50; // 0.5% fee
        payable(dev1).transfer(devFee1);
        uint256 devFee2 = rewards[msg.sender] / 1000 * 50; // 0.5% fee
        payable(dev2).transfer(devFee2);
    }

    function withdraw(uint256 howMuch) external {
        require(howMuch >= Deposited[msg.sender],"Trying to withdraw more than actual balance");
