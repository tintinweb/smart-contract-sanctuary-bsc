...
contract plan is owned {
    ...
    uint40 public token_per_bnb = 1000;
    uint256 public min_tokens = 1;
    uint256 public distribute_persent = 30;
    uint256 public reward_persent = 6;
    ...
    function unstake() public returns(bool){
        ...
        if(users[msg.sender].upto <= block.timestamp){
           addition = (users[msg.sender].deposit_amount * reward_persent / 100) ;
        }
        if(users[msg.sender].upto <= block.timestamp){
           deduction = (users[msg.sender].deposit_amount * distribute_persent / 100) ;
        }
    ...
    function claimDirectCommision(uint256 _claim_amount) public returns(bool){
        require (users[msg.sender].direct_commision >= _claim_amount, "Insufficient Balance.");
        stakingToken.transfer(msg.sender, _claim_amount);
        users[msg.sender].direct_commision -= _claim_amount;
        users[msg.sender].claimed_income += _claim_amount;
        return true;
    }
    ...
    function setreward_persent(uint256 _percentage) public onlyOwner returns (bool){
