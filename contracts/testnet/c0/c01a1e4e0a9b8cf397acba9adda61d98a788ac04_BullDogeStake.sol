...
contract BullDogeStake is Ownable, ReentrancyGuard {
    ...
    constructor() {   
        ...
        addStakePool(
            7, // Duration in days
            1, // Multiplier -- applicable for dynamic APY pools only
            0, // APY % -- applicable for fixed APY pools only
            0, // Withdrawal Fee %
            0 // Unstaking Penalty %
        );
    ...
    function claimRewards(uint256 _stakeId) nonReentrant unpaused external returns (bool){
        ...
        userStake memory userStakeDetails = userStakesById[_stakeId];
        //require(((block.timestamp - userStakeDetails.createdTime)/3600) > 168,"You can start claiming after 7 days of strting the stake");
