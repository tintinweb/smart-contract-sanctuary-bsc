...
contract SmashNFTMarket is ERC721{
    
    using SafeMath for uint256;
    address devwallet;
    address smashlaunch = 0x6d79D2818B5B4ba9BAb865975F2bD4C797A89E93;
    ...
    mapping(uint256 => fixedsale) launchprice; 
    mapping(uint256=>uint256) public salelaunchlist; 
    uint256 []  salelaunch;
    ...
    function create(uint256 collectionid,address to,string memory _tokenURI,string memory _mintname,string memory _nftowner,uint256 _copies,string memory description) external 
    {
    ...
    function burnorinalnft(uint256 _collectionid,uint256 tokenid) external
    {
        ...
        originalowner[tokenidmint]=address(0);
        stopduplicate[(tokenURI(tokenid))]=false;
        _burn(tokenid);    
    }

    function createMulti(uint256 collectionid,address to,string memory _tokenURI,string memory _mintname,string memory _nftowner,uint256 _copies,string memory description, uint256 _times, uint256 _price) external 
    {
        for(uint256 i=1; i<=_times; i++){
           ...
           mint(to,tokenidmint,_tokenURI,_mintname,timeperiod,_nftowner,_copies,description);
           //stopduplicate[_tokenURI]=true;
           if (_price != 0) fixedsales(tokenidmint, _price, false);
       }        
    }    
    ...
    {
      require(originalowner[tokenid] == msg.sender,"you are not original owner");
    ...
    function buynft(uint256 _collectionid,uint256 tokenid,address token) external payable
    {
    ...
    {
        require(originalowner[tokenid] == msg.sender,"You are not originalowner");
        timeforauction[tokenid].minprice= 0;
        timeforauction[tokenid].biddingamount=0;
        timeforauction[tokenid].inlist=false;
        timeforauction[tokenid].time=0;
    ...
    function nftinformation(uint256 id) external view returns(uint256,string memory,uint256,string memory,uint256,string memory,string memory,uint256,address)
    {
        uint256 value = id;
    ...
    function stakevalue(uint256 _tokenid) external view returns(bool)
    {
        return nftstakestate[_tokenid];
    }
    ...
    function fixedsalelaunch(uint256 tokenid, uint256 price) external
    {
        require(!launchprice[tokenid].inlist,"already in sale");
        require(IERC721(smashlaunch).ownerOf(tokenid) == msg.sender,"You are not owner");
        launchprice[tokenid].price  = price;
        launchprice[tokenid].inlist = true;
        salelaunchlist[tokenid]     = salelaunch.length;
        salelaunch.push(tokenid);
        IERC721(smashlaunch).approve(smashlaunch, tokenid);
        IERC721(smashlaunch).safeTransferFrom(msg.sender, smashlaunch, tokenid);
    }
    
    function cancelfixedsalelaunch(uint256 tokenid) external
    {
      require(launchowner[tokenid] == msg.sender,"you are not original owner");
      launchprice[tokenid].price  = 0;
      launchprice[tokenid].inlist = false;
      IERC721(smashlaunch).safeTransferFrom(smashlaunch, msg.sender, tokenid);
      delete salelaunch[(salelaunchlist[tokenid])];
    }

    function buylaunch(uint256 tokenid) external payable
    {
        require(launchprice[tokenid].inlist,"nft not in sale");
        uint256 val = uint256(100) - csdogefees;
       
        uint256 values = msg.value;
        require(values >= launchprice[tokenid].price,"price should be greater");
        uint256 amount   = (values*uint256(val)/uint256(100));
        uint256 ownerinterest = (values*uint256(csdogefees)/uint256(100)); 
        address firstowner    = launchowner[tokenid];
        (bool success,)  = firstowner.call{ value: amount}("");
        require(success, "refund failed");
        (bool csdoges,)  = devwallet.call{ value: ownerinterest}("");
        require(csdoges, "refund failed");
        ...
        IERC721(smashlaunch).safeTransferFrom(smashlaunch, msg.sender, tokenid);
        //changecollectionlaunch(_collectionid, tokenid);
        launchprice[tokenid].price  = 0;
        launchprice[tokenid].inlist=false;
        launchowner[tokenid] = msg.sender;
        delete salelaunch[(salelaunchlist[tokenid])];       
    }

    // function changecollectionlaunch(uint256 _collectionid, uint256 tokenid) internal
    // {
    // //    delete collectionstored[_collectionid][(idnumber[_collectionid][tokenid])];
    // //    collectionstored[(totalcollection[msg.sender])].push(tokenid);
    // //    totalnft[(totalcollection[msg.sender])]+=1;
