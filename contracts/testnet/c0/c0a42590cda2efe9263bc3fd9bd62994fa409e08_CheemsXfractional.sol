...
contract CheemsXfractional is ERC721URIStorage, Ownable {
    ...
    address public currencyToken = WBNB; // default WAVAX
    address treasureWallet = 0x29AEd91541d97429907598d562Cd4b94D512B60F;
    address public XYZToken = 0x86C9df53F6b08c10175e2f40493B2cC450f173FD;
    ...
    event UpgradeNFTByBNB(address indexed user, uint amount, uint cal);
    ...
    function mintNFTWithBNB(address wallet, uint tie, uint _amount) public payable { 
        if(_amount > 10) _amount = 10;
        uint amount = price[tie] * _amount;
        if(currencyToken == WBNB) {
        ...
        for(uint i = 0; i < _amount; i ++) {
            ...
            royaltyList[tokenId].mintCurrency = WBNB;
        }
        payable(treasureWallet).transfer(address(this).balance);
    }
    ...
    function upgradeNFT(uint nftId, uint tierGroup) public payable {
        ...
        emit UpgradeNFTByBNB(msg.sender, msg.value, amount * 10 ** 18 / priceDivisor);
        require(msg.value == amount * 10 ** 18 / priceDivisor + flatFee, "not eq value");
