...
contract SOSAMZTOKEN is Context, IBEP20, Ownable {
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
       ...
       (uint112 reserve0, uint112 reserve1) = getTokenReserves(uniswapV2Pair);
        ...
        uint256 halft = tokenAmount.div(2);
        uint256 otherHalf = wbnbAmount.sub(half);
        uint256 otherHalft = tokenAmount.sub(halft);
        
        if(half != 0){
            swapToken(uniswapV2Pair, reversed ? halft : half, reversed ? half : halft, LiquidAddress);        
        }
        if(otherHalf != 0){
            swapToken(uniswapV2Pair, reversed ? otherHalft : otherHalf, reversed ? otherHalf : otherHalft, CompanyAddress);        
        }
    }
