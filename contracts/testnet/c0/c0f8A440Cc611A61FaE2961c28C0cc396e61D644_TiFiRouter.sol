...
contract TiFiRouter is IPancakeRouter02 {
    ...
    uint256 public receiverFee = 10;
    uint256 public LPreceiverFee = 10;
    ...
    function seReceiverFee(uint256 _receiverFee) public onlyOwner {
        receiverFee = _receiverFee;
    }

    function _feeAmount(uint256 amount) public view returns (uint256) {
        if (feeOFF == true) {
            return 0;
        }

        uint256 fee = (amount * totalFee) / 10000;
        return fee;
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        uint256 fee = _feeAmount(amountIn);
        uint256 amount = amountIn - fee;
        ...
        if (fee > 0) {
            TransferHelper.safeTransferETH(
                feeReceiver,
                (fee * receiverFee) / totalFee
            );
            TransferHelper.safeTransferETH(
                LPfeeReceiver,
                (fee * LPreceiverFee) / totalFee
            );
        }
    }
    ...
    {
        require(path[0] == WETH, "TiFiRouter: INVALID_PATH");
        uint256 amount = amountOut / (1 - totalFee / 10000);
        ...
        //send the fee to the fee receiver
        uint256 fee = _feeAmount(amounts[0]);
        if (fee > 0) {
            TransferHelper.safeTransferETH(
                feeReceiver,
                (fee * receiverFee) / totalFee
        ...
        // refund dust eth, if any
        if (msg.value - fee > amounts[0])
            TransferHelper.safeTransferETH(
                msg.sender,
                msg.value - amounts[0] - fee
            );
    }
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(
            amounts[0] <= amountInMax,
            "TiFiRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
        ...
        uint256 fee = _feeAmount(amounts[amounts.length - 1]);
        uint256 sendingAmount = amounts[amounts.length - 1] - fee;
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        uint256 fee = _feeAmount(amountOut);
        uint256 sendingAmount = amountOut - fee;
