...
contract NFTLP is Ownable {
  ...
  // Upon launching
  uint256 public tokenPrice = 5 * 10 ** 18; // Dev buyback at price 50% token (5 Honor)
  uint256 public armyBUSD = 30 * 10 ** 18; // Dev buyback at price 10% busd ($30)
  uint256 public emblemBUSD = 80 * 10 ** 18; // Dev buyback at price 20% busd ($80)
  uint256 public gasFee = 0.0005 ether; // GasFee Royalty
  ...
  constructor (address _honorAddress, Busd _busdAddress, address _armynftAddress)  {
    Token = Honor(_honorAddress);
    busdAddress = _busdAddress;
    Army = ArmyNFT(_armynftAddress);
  }
  ...
  function setArmyBUSD(uint256 _ArmyBUSD) external onlyOwner {
    armyBUSD = _ArmyBUSD;
  }
  function setEmblemBUSD(uint256 _emblemBUSD) external onlyOwner {
    emblemBUSD = _emblemBUSD;
  }
  function setnftAddress(address _armynftAddress, address _emblemnftAddress) external onlyOwner {
    Army = ArmyNFT(_armynftAddress);
    emblemRankNFT = EmblemNFT(_emblemnftAddress);
  }

  //-----------------------------------------------------------------------------------------------------------------------------------
  // Sell NFT back to dev
  function armyRetirement(address payable recipient, uint256 _tokenId) external {  
    require(busdAddress.balanceOf(address(this)) >= armyBUSD, "NO BUSD Liquidity, Contact the Dev");
    require(msg.sender == Army.ownerOf(_tokenId),"Invalid Token ID");
    Army.transferFrom(Army.ownerOf(_tokenId), owner(), _tokenId);
    busdAddress.transfer(recipient, armyBUSD);
    recipient.transfer(gasFee);
    nftBuyers[_tokenId] = owner();
  }

  function emblemRetirement(address payable recipient, uint256 _tokenId) external {  
    require(Token.balanceOf(address(this)) >= tokenPrice, "NO HONOR Liquidity, Contact the Dev");
    require(busdAddress.balanceOf(address(this)) >= emblemBUSD, "NO BUSD Liquidity, Contact the Dev");
    require(msg.sender == emblemRankNFT.ownerOf(_tokenId),"Invalid Token ID");
    emblemRankNFT.transferFrom(emblemRankNFT.ownerOf(_tokenId), owner(), _tokenId);
    Token.transfer(recipient, tokenPrice);
    busdAddress.transfer(recipient, emblemBUSD);
    recipient.transfer(gasFee);
    nftBuyers[_tokenId] = owner();
  }
  ...
  function nftArmyAddress() public view returns (address) {
    return address(Army);
  }
  function nftEmblemAddress() public view returns (address) {
    return address(emblemRankNFT);
  }
  ...
  //-----------------------------------------------------------------------------------------------------------------------------------
  // OWNER LP Section
  // Add BNB to smart contract for Gas Fee
  function addLiquidity(address payable _to) external payable onlyOwner {
