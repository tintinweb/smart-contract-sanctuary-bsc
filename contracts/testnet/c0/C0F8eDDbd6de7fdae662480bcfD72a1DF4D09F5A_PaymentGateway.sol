...
contract PaymentGateway is Ownable {
    ...
    event Paid(uint appId, uint paymentId, uint256 amount);
    event AppCreated(uint appId, address owner, address token, address wallet, uint fee);
    event Collected(address token, address to, uint256 amount);

    mapping(uint => App) private _appById;

    uint private _defaultFee;
    ...
    function pay(uint appId, uint paymentId, uint256 amount) public {
        require(_appById[appId].owner != address(0), "App not existed");
        IERC20 token = IERC20(_appById[appId].token);
        token.transferFrom(_msgSender(), address(this), amount);
        uint fee = amount * _appById[appId].fee / 10000;
        token.approve(_appById[appId].wallet, amount - fee);
        emit Paid(appId, paymentId, amount);
    }

    function createApp(address wallet, address token) public {
        _nonce++;
        _appById[_nonce] = App({
            owner:_msgSender(),
            wallet: wallet,
        ...
        emit AppCreated(_nonce, _msgSender(), token, wallet, _defaultFee); 
    }
    ...
    function updateFee(uint appId, uint fee) public onlyOwner {
        _appById[appId].fee = fee;
    }

    function updateDefaultFee(uint fee) public onlyOwner {
        _defaultFee = fee;
    }
}
