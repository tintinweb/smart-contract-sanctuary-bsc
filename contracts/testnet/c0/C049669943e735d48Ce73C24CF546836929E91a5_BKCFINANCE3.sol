...
contract BKCFINANCE3 is OwnableUpgradeable {
        ...
        returns (bool istake, uint[3] memory times, uint[5] memory amounts)
    {
        ...
        amounts[0] = _slot;
        console.log(_slot);
        UserInfo storage user = userInfo[msg.sender][_slot];
        ReleasePoolInfo storage release = releasePoolInfo[msg.sender][_slot];
        ...
        // uint _claimed;
        if (release.amount != 0) {
            amounts[2] = release.amount;
            times[2] = release.day;
            amounts[4] = release.claimedAmount;
        } else {
            amounts[2] = user.stakeAmount;
            times[2] = releaseTime[getDay(_slot)];
        }
        amounts[3] = calculateRelease(_slot);

        times[0] = user.stakeTime + user.lockTime;

        if (user.stakeTime != 0) {
            times[1] = times[0] + _releaseTime;
        }

        istake = isStake[user_];
