...
contract MiningUSDT is Ownable{
    ...
    function buyEggs(uint256 _Aamount, uint256 _usdtAmount, address ref) public {
        ...
        uint256 eggsBought = calculateEggBuy(_Aamount,SafeMath.sub(Bcoin.balanceOf(address(this)),_Aamount));
        eggsBought = SafeMath.sub(eggsBought,devFee(eggsBought));
        bought[msg.sender] = SafeMath.add(bought[msg.sender],_Aamount);

        uint256 fee = devFee(_Aamount);
        uint256 usdtFee = devFee(_usdtAmount);
        Acoin.transfer(ceoAddress,fee);
        usdt.transfer(ceoAddress,usdtFee);
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256){
        return calculateTrade(eggs,marketEggs,Acoin.balanceOf(address(this))) * 2;
    }
    // 计算购买数量
    ...
    function calculateEggBuySimple(uint256 eth) public view returns(uint256){
        return calculateEggBuy(eth,Acoin.balanceOf(address(this)));
    }
    // 开发费用
