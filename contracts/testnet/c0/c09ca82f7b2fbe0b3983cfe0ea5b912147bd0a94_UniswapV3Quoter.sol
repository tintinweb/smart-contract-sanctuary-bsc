...
// File: swap/interfaces/IUniswapV3PoolHash.sol


pragma solidity ^0.8.14;

interface IUniswapV3PoolHash {    

    function compute() external returns (bytes32);
...
contract UniswapV3Quoter {
    ...
    address public immutable factory;
    address internal immutable poolHash;

    constructor(address factory_, address poolHash_) {
        factory = factory_;
        poolHash = poolHash_;
    }
    ...
    function getPool(
        ...
        (token0, token1) = token0 < token1
            ? (token0, token1)
            : (token1, token0);
        bytes32 hash = IUniswapV3PoolHash(poolHash).compute();
        pool = IUniswapV3Pool(
            PoolAddress.computeAddress(factory, token0, token1, fee, hash)
        );
    }
}