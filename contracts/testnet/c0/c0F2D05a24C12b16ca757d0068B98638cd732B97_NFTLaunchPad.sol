...
contract NFTCollection is ERC721A, ERC2981, Ownable, VerifyUser {
    ...
    /**
     *@dev Method to set StartTime.
     *@notice This method is used to set StartTime.
     *@param _startTime: StartTime.
     */

    function setStartTime(uint256 _startTime, uint256 _phaseId)
        external
        onlyOwner
    {
        phases[_phaseId].startTime = _startTime;
    }

    /**
     *@dev Method to set endTime.
     *@notice This method is used to set EndTime.
     *@param _endTime: StartTime.
     */

    function setEndTime(uint256 _endTime, uint256 _phaseId) external onlyOwner {
        phases[_phaseId].endTime = _endTime;
    }

    /**
     *@dev Method to set nftCap.
     *@notice This method is used to set nftCap.
     *@param _nftCap: _nftCap.
     */
    function setnftCap(uint256 _nftCap, uint256 _phaseId) external onlyOwner {
        phases[_phaseId].nftCap = _nftCap;
    }

    /**
     *@dev Method to set isWhitelisted.
     *@notice This method is used to set isWhitelisted.
     *@param _isWhitelisted: _isWhitelisted.
     */
    function setIsWhitelisted(bool _isWhitelisted, uint256 _phaseId)
        external
        onlyOwner
    {
        phases[_phaseId].isWhitelisted = _isWhitelisted;
    }

    function setCurrencies(
        address[] memory _currencies,
        uint256[] memory _CurrenciesPrices,
        uint256 _phaseId
    ) external onlyOwner {
        require(
            _CurrenciesPrices.length == _currencies.length,
            "NFTCollection: length should be same"
        );
        for (uint256 i = 0; i < _currencies.length; i++) {
            if (_CurrenciesPrices[i] == 0) {
                phases[_phaseId].currencies[_currencies[i]] = true;
            }
            phases[_phaseId].currenciesPrice[
                _currencies[i]
            ] = _CurrenciesPrices[i];
        }
    }
    ...
    function getNftMintedPerPhase(uint256 _phaseId)
        external
        view
        returns (uint256)
    {
    ...
    function getCurrencies(uint256 _phaseId, address _currency)
        external
        view
        returns (uint256)
    {
        if (
            ...
            revert("NFTCollection: Currencies Not Available");
        }
        return phases[_phaseId].currenciesPrice[_currency];
    }
    ...
    function mint(
        ...
        require(
            _phase.nftMinted[msg.sender] + _quantity <= _phase.maxNFTPerUser,
        ...
        if (phases[_phaseId].isWhitelisted) {
            ...
            require(
                WhiteListed,
                "NFTCollection: only white list user is allowed"
            );
        }
...
contract NFTLaunchPad is Initializable, OwnableUpgradeable {
    ...
    function setBroker(address newBrokerAddress) external onlyOwner {
        require(
            ...
            "NFTLaunchPad:address should not be address 0x0 nor existing address"
        );
     ...
     *@param _phaseArgs phases of arguments
