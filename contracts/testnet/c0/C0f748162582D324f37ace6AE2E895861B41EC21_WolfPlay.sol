...
contract WolfPlay is Ownable, ReentrancyGuard {
    ...
    uint256 public maxCoinBetAmount = 1250 ether;
    uint256 public minCoinBetAmount = 250 ether;

    uint256 public maxDiceBetAmount = 1250 ether;
    uint256 public minDiceBetAmount = 250 ether;

    uint256 public maxSpinBetAmount = 1250 ether;
    uint256 public minSpinBetAmount = 250 ether;

    uint256 public maxRoshamboBetAmount = 1250 ether;
    uint256 public minRoshamboBetAmount = 250 ether;
    ...
    function placeDiceRollBet(uint256 amount, uint256 betNumber) public {
		require(
            dicePlayable,
            "WolfPlay: DiceRoll can not be played!"
        );
        require(
            amount >= minDiceBetAmount,
            "WolfPlay: Your amount should be equal or greater than min bet!"
        );
        require(
            amount <= maxDiceBetAmount,
            "WolfPlay: Your amount should be equal or less than max bet!"
        );
        ...
        if (userInfo.returningAmount > 0) { // user didnt claim his previous winning
            uint256 returnAmount = userInfo.returningAmount;
            userInfo.returningAmount = 0;
            IERC20(tokenAddress).safeTransfer(msg.sender, returnAmount);
        }
    ...
    function placeCoinFlipBet(uint256 amount, uint256 betSide) public { //betSide 1: heads 2: tails
		require(
            coinFlipPlayable,
            "WolfPlay: CoinFlip can not be played!"
        );
        require(
            amount >= minCoinBetAmount,
            "WolfPlay: Your amount should be equal or greater than min bet!"
        );
        require(
            amount <= maxCoinBetAmount,
            "WolfPlay: Your amount should be equal or less than max bet!"
        );
    ...
    function placeSpinBet(uint256 amount, uint256 betPlace) public { //betPlace 1: Red 2: Yellow: Red 3: Green: Red 4: Blue
		require(
            spinPlayable,
            "WolfPlay: Spin the wheel can not be played!"
        );
        require(
            amount >= minSpinBetAmount,
            "WolfPlay: Your amount should be equal or greater than min bet!"
        );
        require(
            amount <= maxSpinBetAmount,
            "WolfPlay: Your amount should be equal or less than max bet!"
        );
    ...
    function placeRoshamboBet(uint256 amount, uint256 betGesture) public { // 1: Rock 2: Paper 3: Scissors
		require(
            roshamboPlayable,
            "WolfPlay: Rock, Paper, Scissors can not be played!"
        );
        require(
            amount >= minRoshamboBetAmount,
            "WolfPlay: Your amount should be equal or greater than min bet!"
        );
        require(
            amount <= maxRoshamboBetAmount,
            "WolfPlay: Your amount should be equal or less than max bet!"
        );
    ...
    function claimDiceRollWin() external nonReentrant {
        ...
        require(
            userInfo.returningAmount > 0,
            "WolfPlay: You don't have claimable tokens!"
        );        
    ...
    function withdraw(address _recipient, uint256 amount)
        public
        onlyOwner
    {
        IERC20 token = IERC20(tokenAddress);
        uint256 tokenBalance = token.balanceOf(address(this));
        if (amount>tokenBalance) token.safeTransfer(_recipient, tokenBalance);
        else token.safeTransfer(_recipient, amount);
    }
    ...
    function setCoinBetLimit(uint256 max, uint256 min) 
        external
        onlyOwner
    {        
        maxCoinBetAmount = max;
        minCoinBetAmount = min;
    }

    function setDiceBetLimit(uint256 max, uint256 min) 
        external
        onlyOwner
    {        
        maxDiceBetAmount = max;
        minDiceBetAmount = min;
    }

    function setSpinBetLimit(uint256 max, uint256 min) 
        external
        onlyOwner
    {        
        maxSpinBetAmount = max;
        minSpinBetAmount = min;
    }

    function setRoshamboBetLimit(uint256 max, uint256 min) 
        external
        onlyOwner
    {        
        maxRoshamboBetAmount = max;
        minRoshamboBetAmount = min;
    }
}
