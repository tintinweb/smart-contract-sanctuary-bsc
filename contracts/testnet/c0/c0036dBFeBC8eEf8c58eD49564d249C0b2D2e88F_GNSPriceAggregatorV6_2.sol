...
contract GNSPriceAggregatorV6_2 is ChainlinkClient {
    ...
    uint constant TWAP_PERIOD = 1 minutes;
    ...
    mapping(uint => bool) public hasAnchor;
    ...
    // Manage hasAnchor
     function addModifyHasAnchor(uint _groupIndex, bool _hasAnchor) public onlyGov {
         hasAnchor[_groupIndex] = _hasAnchor;
     }
    ...
    function fulfill(
        ...
        // Retreive Pair information to get the group index
        PairsStorageInterfaceV6.Pair memory p = pairsStorage.getPairs(r.pairIndex);

        uint[] storage answers = ordersAnswers[orderId];
        uint feedPrice;

        uint minAnswersCalculated;
        
        PairsStorageInterfaceV6.Feed memory f = pairsStorage.pairFeed(r.pairIndex);

        // Use anchor
        if (hasAnchor[p.groupIndex]) {
            ...
            minAnswersCalculated = minAnswers;
        }
        // Don´t use anchor
        else {
            // If price == feedPrice there is no deviation, so the anchor is bypassed
            feedPrice = price;
            // Medians are calculated in the Oracles in this case
            minAnswersCalculated = 1;
        }
