...
contract SmartBeComBee {
    ...
    function buyLevel(uint8 matrix, uint8 level, address tokenAddress) external payable {
        ...
        if (tokenAddress != 0x0000000000000000000000000000000000000000){
            ...
            uint256 amount = 2*levelCost[level]*1e13 + 2e18;            
            uint256 allowance = _token.allowance(msg.sender, address(this));
            //emit AllowanceToken(allowance, amount);
            require(allowance >= amount, "Check the token allowance");
    ...
    function usersB3MatrixReinvest2(address userAddress, uint8 level) public view returns(uint8) {
        return users[userAddress].b3Matrix[level].reinvestCount;
    }
    ...
    function usersB6MatrixReinvest2(address userAddress, uint8 level) public view returns(uint8) {
        return users[userAddress].b6Matrix[level].reinvestCount;
    }
