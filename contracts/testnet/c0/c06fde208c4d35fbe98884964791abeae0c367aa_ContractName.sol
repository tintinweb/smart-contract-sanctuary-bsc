...
contract ContractName is IBEP20, Authorization {
  ...
  constructor(address _newOwner) Authorization(_newOwner) {
    ...
    //FEE SETUP
    _taxMatrix["longTerm"] = (TaxFees(150, 450, 150, 100, 200));
    _taxMatrix["midTerm"] = (TaxFees(125, 225, 125, 50, 200));
    _taxMatrix["standard"] = (TaxFees(100, 100, 25, 50, 0));
  ...
  */
  ...
  function setExcludeStakingContract(address acc, bool value) public onlyOwner{
        ...
        _isExcludedFromMaxWallet[deadWallet] = value;
  ...
  */
   ...
   */
  function setTaxFee(string memory taxFeeCode, uint256 _longTermFee, uint256 _midTermFee, uint256 _operationFee, uint256 _stakingFee, uint256 _liquidityFee)
    public authorized
    {
      require(_longTermFee + _midTermFee + _operationFee + _stakingFee + _liquidityFee <= 2000,  "Fee cannot be more than 20%");

      _taxMatrix[taxFeeCode] = (TaxFees(_longTermFee, _midTermFee, _operationFee, _stakingFee, _liquidityFee));
    }
}