...
abstract contract Matrix is IMatrix, Ownable2Step {
	...
	event LostCFReward(uint indexed Timestamp, uint indexed FromAccountID, uint LostAmount);
	event TransferCFReward(uint indexed Timestamp, uint indexed FromAccountID, uint RewardAmount);
	...
	function _ShareCFReward(uint _FromAID, uint _MATRIX) private {
		...
		// For marketing
		if (num <= 20000) {
			delete CFPending[_FromAID][_MATRIX][2];
			return;
		}
		...
		(uint xr, uint xs, uint amount) = (
			_CFPendingOf(_FromAID, _MATRIX)[0],
			_CFPendingOf(_FromAID, _MATRIX)[1],
			_CFPendingOf(_FromAID, _MATRIX)[2]
		);
		...
		for (uint j; j < xr; ++j) {
			if (SponsorLevel(distid) >= sl) {
				...
				emit TransferCFReward(block.timestamp, _FromAID, j < xs ? (aXR + aXS) : aXR);
			} else emit LostCFReward(block.timestamp, _FromAID, j < xs ? (aXR + aXS) : aXR);

			distid = _UplineOnMatrix(distid, _MATRIX);
		}
		delete CFPending[_FromAID][_MATRIX][2]; // a
	}
...
contract XPrograms is XProgram {
	...
	event LevelUpgraded(uint Timestamp, uint indexed AccountID, uint XProgram, uint LevelTo);
	event TransferReward(
		uint Timestamp,
		uint indexed FromAccountID,
		uint XProgram,
		uint Level,
		uint indexed ToAccountID,
		uint Amount
	);
	...
	function _UpgradeLevelManually(
		...
		success_ = true;
		emit LevelUpgraded(block.timestamp, _AID, _XPro, _LTo);
	...
	function _Recycling(uint _AID, uint _XPro, uint _Level) internal override returns (uint cycleCount_) {
		...
		// New cycle
		_FindCurrentCycleUpline(_AID, _XPro, _Level); // C 171
	...
	function _ShareReward(uint _AID, uint _XPro, uint _Level) private returns (bool success_) {
		...
		if (_XPro == X3) {
			...
			emit TransferReward(block.timestamp, _AID, _XPro, _Level, cu1, lc);
		...
		if (_XPro == X6 || _XPro == X7) {
			...
			emit TransferReward(block.timestamp, _AID, _XPro, _Level, cu2, acu2);
		...
		if (_XPro == X8 || _XPro == X9) {
			...
			emit TransferReward(block.timestamp, _AID, _XPro, _Level, cu2, acu2);
			emit TransferReward(block.timestamp, _AID, _XPro, _Level, cu3, acu3);
	 ...
	 * Find cycle upline
	 */
	function _FindCycleUpline(uint _AID, uint _XPro, uint _Level) private view returns (uint CU_) {
		...
		if (CU_ != 0)
			while (_UplineOnMatrix(CU_, matrix) != 0) {
				...
				// else emit LostPartner(block.timestamp, _AID, _XPro, _Level, CU_); // Over sponsor level or Level not active
