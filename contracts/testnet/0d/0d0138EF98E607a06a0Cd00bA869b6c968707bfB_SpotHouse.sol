    ...
    SpotHouseStorage
{
    ...
    function openMarketOrderWithQuote(
        ...
        if (_side == Side.BUY) {
            // deposit quote asset
            (uint256 amountTransferred, uint256 _fee) = _deposit2(
                _pairManager,
                _trader,
                Asset.Quote,
                _quoteAmount
            ...
            (sizeOutQuote, baseAmount) = _pairManager
                .openMarketPositionWithQuoteAsset(_quoteAmount, true, _trader);
            require(
            ...
            (sizeOutQuote, baseAmount) = _pairManager
                .openMarketPositionWithQuoteAsset(_quoteAmount, false, _trader);
            require(
                sizeOutQuote == _quoteAmount,
                Errors.VL_NOT_ENOUGH_LIQUIDITY
            );
            ...
            // SELL market
            (uint256 amountTransferred, ) = _deposit2(
                _pairManager,
                _trader,
                Asset.Base,
                baseAmount
        ...
        nonReentrant
    {
        ...
        (uint256 refundQuote, uint256 refundBase) = getAmountClaimable(
            _pairManager,
            _trader
        );
        ...
        for (uint64 i = 0; i < _listPendingLimitOrder.length; i++) {
            PendingLimitOrder memory PendingLimitOrder = _listPendingLimitOrder[
                i
            ];

            if (PendingLimitOrder.quantity == 0) {
                continue;
            }

            _listPips[i] = PendingLimitOrder.pip;
            _orderIds[i] = PendingLimitOrder.orderId;

            (uint256 refundQuantity, uint256 partialFilled) = _pairManager
                .cancelLimitOrder(
                    PendingLimitOrder.pip,
                    PendingLimitOrder.orderId
                );

            if (PendingLimitOrder.isBuy) {
    ...
    function cancelLimitOrder(
        ...
        delete _orders[_orderIdx]; // = blankLimitOrderData;
    ...
    function updateFee(uint128 _fee) external override onlyOwner {
        fee = _fee;
    }
    ...
    function claimFee(IPairManager _pairManager) external onlyOwner {
        SpotFactoryStorage.Pair memory _pairAddress = _getQuoteAndBase(
            _pairManager
        );
        ...
        IERC20(_pairAddress.BaseAsset).transferFrom(
            pairManagerAddress,
            owner(),
            baseFeeFunding
        );
        IERC20(_pairAddress.QuoteAsset).transferFrom(
            pairManagerAddress,
            owner(),
            quoteFeeFunding
        );
        _pairManager.resetFee();
    }
    ...
    function _openBuyLimitOrder(
        ...
        (uint256 quoteAmountTransferred, uint256 feeQuote) = _deposit2(
            _pairManager,
            _trader,
            Asset.Quote,
            quoteAmount
        ...
        if (
            quoteAmountTransferred != quoteAmountFilled &&
            (_quantity) != sizeOut
        ) {
            limitOrders[_pairManagerAddress][_trader].push(
                SpotLimitOrder.Data({
                    ...
                    orderId: orderId,
                    isBuy: true,
                    quoteAmount: quoteAmountTransferred - quoteAmountFilled,
    ...
    function _openSellLimitOrder(
...

        {
            ...
            // with token has RFI we need deposit first
            // and get real balance transferred
            (baseAmountTransferred, ) = _deposit2(
                _pairManager,
                _trader,
                Asset.Base,
                uint128(_quantity)
        ...
        if (
            quoteAmount != quoteAmountFilled && baseAmountTransferred != sizeOut
        ) {
            limitOrders[_pairManagerAddress][_trader].push(
                SpotLimitOrder.Data({
                    pip: _pip,
                    ...
                    baseAmount: baseAmountTransferred - sizeOut,
                    blockNumber: block.number
                })
            );
        }
    ...
    function _openMarketOrder(
        ...
        uint256 quoteAmount;
        if (_side == Side.BUY) {
            (sizeOut, quoteAmount) = _pairManager.openMarketPosition(
                _quantity,
                true,
                _trader
            );
            ...
            uint256 _feeQuote = _feeCalculator(quoteAmount);
            quoteAmount += _feeQuote;
            // deposit quote asset
            (uint256 amountTransferred, uint256 fee) = _deposit2(
                _pairManager,
                _trader,
                Asset.Quote,
                quoteAmount
            ...
            // SELL market
            (uint256 baseAmountTransferred, ) = _deposit2(
                _pairManager,
                _trader,
                Asset.Base,
                _quantity
    ...
    function _deposit2(
        ...
        Asset _asset,
        uint256 _amount
    ) internal returns (uint256 amountTransferred, uint256 fee) {
        ...
        if (_asset == Asset.Quote) {
            if (_pairAddress.QuoteAsset == WBNB) {
                ...
                quoteAsset.transferFrom(_trader, pairManagerAddress, _amount);
                uint256 _balanceAfter = quoteAsset.balanceOf(
                    pairManagerAddress
                );
                _amount = _balanceAfter - _balanceBefore;
            }
            _fee = _feeCalculator(_amount);
            ...
            if (_pairAddress.BaseAsset == WBNB) {
                ...
                baseAsset.transferFrom(_trader, pairManagerAddress, _amount);
