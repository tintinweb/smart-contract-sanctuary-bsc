...
/**
 * Mainnet Address        https://etherscan.io/address/0x7be8076f4ea4a4ad08075c2508e481d6c946d12b
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    struct Order {
        ...
        uint basePrice;
        /* When mixing payment, you need to use payment token 2 */
        address paymentTokenTwo;
        /* When mixing payments, use the price of payment token 2 */
        uint basePriceTwo;
    ...
    event OrderApprovedPartTwo    (bytes32 indexed hash, AuthenticatedProxy.HowToCall howToCall, bytes calldata, bytes replacementPattern, address staticTarget, address paymentToken, uint basePrice, address paymentTokenTwo, uint basePriceTwo, uint extra, uint listingTime, uint expirationTime, uint salt, bool orderbookInclusionDesired);
        ...
        returns (bytes32 hash)
    {
        ...
        index = ArrayUtils.unsafeWriteAddress(index, order.paymentTokenTwo);
        index = ArrayUtils.unsafeWriteUint(index, order.basePriceTwo);
        ...
        internal
    {
        ...
        {   
            emit OrderApprovedPartTwo(hash, order.howToCall, order.calldata, order.replacementPattern, order.staticTarget, order.paymentToken, order.basePrice, order.paymentTokenTwo, order.basePriceTwo, order.extra, order.listingTime, order.expirationTime, order.salt, orderbookInclusionDesired);
        }
    }
     ...
     * @return Match price
     */
    function calculateMatchPriceTwo(Order memory buy, Order memory sell)
        view
        internal
        returns (uint)
    {
        /* Calculate sell price. */
        uint sellPrice = SaleKindInterface.calculateFinalPrice(sell.side, sell.saleKind, sell.basePriceTwo, sell.extra, sell.listingTime, sell.expirationTime);

        /* Calculate buy price. */
        uint buyPrice = SaleKindInterface.calculateFinalPrice(buy.side, buy.saleKind, buy.basePriceTwo, buy.extra, buy.listingTime, buy.expirationTime);
        ...
        returns (uint)
    {
        ...
        uint priceTwo = calculateMatchPriceTwo(buy, sell);

        /* If paying using a token (not Ether), transfer tokens. This is done prior to fee payments to that a seller will have tokens before being charged fees. */
        if (price > 0 && sell.paymentToken != address(0)) {
            transferTokens(sell.paymentToken, buy.maker, sell.maker, price);
        }
        if (priceTwo > 0 && sell.paymentTokenTwo != address(0)) {
            transferTokens(sell.paymentTokenTwo, buy.maker, sell.maker, priceTwo);
        }
        ...
        returns (bool)
    {
        return (
            ...
            /* Must use same payment token 2. */
            (buy.paymentTokenTwo == sell.paymentTokenTwo) &&
...
contract Exchange is ExchangeCore {
    ...
    {
        return hashOrder(
          Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], uints[2], uints[3], addrs[3], feeMethod, side, saleKind, addrs[4], howToCall, calldata, replacementPattern, addrs[5], staticExtradata, ERC20(addrs[6]), uints[4], ERC20(addrs[7]), uints[5], uints[6], uints[7], uints[8], uints[9])
        );
    }
    ...
    function validateOrderParameters_ (
        ...
        returns (bool)
    {
        Order memory order = Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], uints[2], uints[3], addrs[3], feeMethod, side, saleKind, addrs[4], howToCall, calldata, replacementPattern, addrs[5], staticExtradata, ERC20(addrs[6]), uints[4], ERC20(addrs[7]), uints[5], uints[6], uints[7], uints[8], uints[9]);
    ...
    function ordersCanMatch_(
        ...
        returns (bool)
    {
        Order memory buy = Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], uints[2], uints[3], addrs[3], FeeMethod(feeMethodsSidesKindsHowToCalls[0]), SaleKindInterface.Side(feeMethodsSidesKindsHowToCalls[1]), SaleKindInterface.SaleKind(feeMethodsSidesKindsHowToCalls[2]), addrs[4], AuthenticatedProxy.HowToCall(feeMethodsSidesKindsHowToCalls[3]), calldataBuy, replacementPatternBuy, addrs[5], staticExtradataBuy, ERC20(addrs[6]), uints[4], ERC20(addrs[7]), uints[5], uints[6], uints[7], uints[8], uints[9]);
        Order memory sell = Order(addrs[8], addrs[9], addrs[10], uints[10], uints[11], uints[12], uints[13], addrs[11], FeeMethod(feeMethodsSidesKindsHowToCalls[4]), SaleKindInterface.Side(feeMethodsSidesKindsHowToCalls[5]), SaleKindInterface.SaleKind(feeMethodsSidesKindsHowToCalls[6]), addrs[12], AuthenticatedProxy.HowToCall(feeMethodsSidesKindsHowToCalls[7]), calldataSell, replacementPatternSell, addrs[13], staticExtradataSell, ERC20(addrs[14]), uints[14], ERC20(addrs[15]), uints[15], uints[16], uints[17], uints[18], uints[19]);
...
contract iNFTExchangeMix is Exchange {

    string public constant name = "Project iNFT Exchange Mix";
