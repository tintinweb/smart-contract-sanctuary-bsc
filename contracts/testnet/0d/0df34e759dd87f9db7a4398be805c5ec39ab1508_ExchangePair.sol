...
contract ExchangePair {
    ...
    function createOrder(address _user, address _buyToken, address _sellToken, uint _buyAmount, uint _sellAmount) public lock returns (uint _tBuyAmount , uint _tSellAmount){
        ...
        orderByHash[hash_] =  LibOrder.Order(_user, new bytes32[](0), _sellToken,  _buyToken , _sellAmount, _buyAmount, 0, 0, (block.timestamp+3 days),LibOrder.Status.PENDING);
        ...
        for(uint i = orders[price].lastIndex;i< orders[price].orderQueqe.length; i++) {
            ...
            if(!cancelled[__hash] &&! compleated[__hash] && orderByHash[hash_].sellToken == orderByHash[__hash].buyToken) {
                if ((orderByHash[__hash].sellAmount - filled[__hash]) > orderByHash[hash_].buyAmount) { 
                    filled[__hash] += _tBuyAmount;
                    filled[hash_] += _tSellAmount;
                    ...
                    _tBuyAmount = _tBuyAmount - (orderByHash[__hash].sellAmount - filled[__hash]);
                    ...
                    if(_tBuyAmount!=0){
                        ...
                        filled[hash_] += (orderByHash[__hash].sellAmount - filled[__hash]); 
                        filled[__hash] = orderByHash[__hash].sellAmount; 
                        ...
                        compleated[__hash] = true;
                        compleated[hash_] = true;
                        filled[hash_] = orderByHash[hash_].sellAmount; 
                        filled[__hash] = orderByHash[__hash].sellAmount; 
