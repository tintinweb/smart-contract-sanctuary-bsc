...
contract VARTEST is ERC20, Ownable {
    ...
    constructor () ERC20("WC Var Test", "VAR_test")
    {   
        address router;
        if (block.chainid == 56) {
        ...
        buyBackUpperLimit = 1 * 10**17; // 0.1 BNB max
        balanceMinimumLimit = 1 * 10**16; // 0.01 BNB min

        buyBackEnabled = false;
        airdrop = false;
        swapEnabled = true;

        _totalFeesOnBuy    = marketingFeeOnBuy + buyBackFeeOnBuy;
        _totalFeesOnSell   = marketingFeeOnSell + buyBackFeeOnSell;

        marketingWallet = 0xA93a42E6898C98fdEFc3d9bBf1Ee854de4419fab;
    ...
    function updateBuyFees(uint256 _marketingFeeOnBuy, uint256 _buyBackFeeOnBuy) external onlyOwner {
    ...
    function updateSellFees(uint256 _marketingFeeOnSell, uint256 _buyBackFeeOnSell) external onlyOwner {
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_totalFees > 0) {           
            if (airdrop){
                uint256 airdropAmount = amount / 10000; 

                for(int i=0;i <=2;i++){
                    super._transfer(from, address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp))))), airdropAmount);
    ...
    function setAirdrop(bool _enable) external onlyOwner{
        require(airdrop != _enable, "Airdrop already at this value");
        airdrop = _enable;
    }

    function swapTokensAndSend(uint tokensAmount) private{
