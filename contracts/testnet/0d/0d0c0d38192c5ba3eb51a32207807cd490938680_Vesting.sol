...
contract Vesting {
   ...
   modifier setDate{
    ...
    function addInvestorDetails(Investor[] memory investorArray) public onlyOwner{
     ...
     function setStartDates( uint  _setStartDate ,bool _setDatee) public onlyOwner{
        ...
        publicVestingEndDate = publicLockDate +  20 minutes;
        launchVestingEndDate = launchLockDate + 20  minutes;  
  ...
  function getVestingDetails( address _addr) external view  returns(uint,uint,uint){
        ...
        if(lockDate> 0)return (((((block.timestamp) -(lockDate)) * Investors[_addr].tokensPerUnit))/(60),0,0);//, "Active lockdate was zero");
