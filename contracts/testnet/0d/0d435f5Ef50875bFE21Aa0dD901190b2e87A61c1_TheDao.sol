...
library Constants {
    ...
    uint256 internal constant GOVERNANCE_INIT_VERSION = 0;
    ...
    uint256 internal constant SUPPLY_GROWTH_RATE = 10e18; // 10% deltaD
    ...
    uint256 internal constant CREDIT_BOND_EMISSION_WHEN_HIGH_DEBT_RATE = 0.5e18; // 0.5% new supply
    uint256 internal constant CREDIT_BOND_EMISSION_WHEN_LOW_DEBT_RATE = 1e18; // 1% new supply

    // The credit interest
    uint256 internal constant CREDIT_INTEREST_BOOTSTRAP_RATE = 60e18; // 60%
...
contract DaoStorage is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // The current version of DAO
    uint256 public daoVersion;
    ...
    // A account's history slots. Easy for history query
    mapping(address => uint256[]) public accountSlots;
    ...
    // A mapping from the hash of a Debt Order to the amount of Debts that the Debt Order is still willing to buy.
...
contract DaoEpoch is DaoStorage {
    ...
    function advance() external whenNotPaused nonReentrant {
        require(epochEndTime() < block.timestamp, "Epoch: still current epoch");
    ...
    // Return ending time of current epoch
    function epochEndTime() public view returns (uint256) {
        // Epoch has not start yet
        if (epochInfo.timestamp == 0) return epochInfo.start;

        return epochInfo.timestamp.add(Constants.EPOCH_CURRENT_LENGTH);
    }
    ...
    function rewardDollars(uint256 deltaD) internal {
        uint256 newSupply = deltaD.mul(Constants.SUPPLY_GROWTH_RATE).div(100e18);
    ...
    function stepCredit(int256 deltaD) internal {
        ...
        // total bond amount when epoch starts
        uint256 startBonds = creditInfo.startBonds;
        // remaining bonds
        uint256 bonds = creditInfo.bonds;

        uint256 bondDemand;
        if (startBonds > 0) {
            bondDemand = bonds.mul(100e18).div(startBonds);
        }
...
contract DaoCredit is DaoEpoch {
    ...
    function balanceOfDebtSlot(address account, uint256 slot) public view returns (uint256) {
        return accountInfo[account].debtSlots[slot];
    }

    function accountDebtSlots(address account) public view returns (uint256[] memory) {
        return accountSlots[account];
    }

    function purchaseBondFor(address purchaser, uint256 amount) internal whenNotPaused nonReentrant {
        require(amount <= creditInfo.bonds, "Credit: insufficient bonds");

        // Burn Dollar
        IDollar(dollar).burnFrom(purchaser, amount);
        ...
        // Save account slots
        accountSlots[purchaser].push(creditInfo.debts);
...
contract CreditMarket is DaoCredit {
    ...
    function createDebtOrder(
        ...
        return _createDebtOrder(dollarAmount, pricePerDebt, maxPlaceInLine);
    }
    ...
    // Cancel
    function cancelDebtOrder(uint256 pricePerDebt, uint256 maxPlaceInLine) external payable {
        _cancelDebtOrder(pricePerDebt, maxPlaceInLine);
    }
    ...
    function roundAmount(DebtListing calldata l, uint256 amount) private pure returns (uint256) {
        ...
        if (remainingAmount < (1e18 / l.pricePerDebt)) amount = l.amount;
        return amount;
    }
    ...
    function _cancelDebtOrder(uint256 pricePerDebt, uint256 maxPlaceInLine) internal {
...
contract DaoStake is CreditMarket {
    ...
    function balanceOfShares(address account) public view returns (uint256) {
        return accountInfo[account].shares;
    }

    // UserEarnedDollars = UserShares * DaoEarnedDollars / DaoTotalShares - UserClaimed
    ...
    // Return estimated withdrawal balances
    ...
    {
        _amount = Math.min(amount, accountInfo[account].deposits[token].amount);

        // DDV value of amount of withdraw
        _burnDdv = _amount.mul(accountInfo[account].deposits[token].ddv).div(
            accountInfo[account].deposits[token].amount
        );

        // UserBurnPapers = WithdrawDdvValue * UserPapers / UserDdv
        _burnPapers = _burnDdv.mul(accountInfo[account].papers).div(accountInfo[account].ddv);
        // UserBurnShares = WithdrawDdvValue * UserShares / UserDdv
        _burnShares = _burnDdv.mul(accountInfo[account].shares).div(accountInfo[account].ddv);
    }
    ...
    function collectFor(address depositor) internal whenNotPaused nonReentrant {
        ...
        // UserClaimed = UserShares * DaoEarnedDollar / DaoShares
        accountInfo[depositor].claimed = accountInfo[depositor].shares.mul(earnedDollar).div(stakeInfo.shares);
    ...
    function depositFor(
        ...
        // UserClaimed = UserClaimed + NewShares * DaoEarnedDollar / DaoShares
        accountInfo[depositor].claimed = accountInfo[depositor].claimed.add(
            sharesAmount.mul(earnedDollar).div(stakeInfo.shares)
        );
    ...
    function requestWithdraw(address token, uint256 amount) internal whenNotPaused nonReentrant {
        ...
        uint256 withdrawDdvValue = amount.mul(accountInfo[msg.sender].deposits[token].ddv).div(
            accountInfo[msg.sender].deposits[token].amount
        );

        // UserBurnPapers = WithdrawDdvValue * UserPapers / UserDdv
        uint256 burnPapers = withdrawDdvValue.mul(accountInfo[msg.sender].papers).div(accountInfo[msg.sender].ddv);
        // UserBurnShares = WithdrawDdvValue * UserShares / UserDdv
        uint256 burnShares = withdrawDdvValue.mul(accountInfo[msg.sender].shares).div(accountInfo[msg.sender].ddv);

        // UserClaimed = UserClaimed - BurnShares * DaoEarnedDollar / DaoShares
        accountInfo[msg.sender].claimed = accountInfo[msg.sender].claimed.sub(
            burnShares.mul(earnedDollar).div(stakeInfo.shares)
        );
        ...
        accountInfo[msg.sender].ddv = accountInfo[msg.sender].ddv.sub(withdrawDdvValue);
...
contract Governance is DaoStake {
    function initialize(
        ...
        creditInfo.interest = Constants.CREDIT_INTEREST_BOOTSTRAP_RATE;
    }

    ////////// Governance functions //////////

    function initGov() external onlyOwner {
        require(daoVersion == Constants.GOVERNANCE_INIT_VERSION, "Gov: invalid version");

        // Do stuffs

        daoVersion = daoVersion + 1;
    }
