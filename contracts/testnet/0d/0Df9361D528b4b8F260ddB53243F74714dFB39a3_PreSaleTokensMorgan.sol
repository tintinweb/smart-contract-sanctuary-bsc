 ...
 */
contract PreSaleTokensMorgan is Ownable, ReentrancyGuard {
    // SafeMath library
    ...
    bool _active = false;
    modifier isClaiming() {
        require(_active, "This contract has not yet released the claim");
        _;
    }
    ...
    // total raised And Sale
    uint256 private totalRaised;
    uint256 private totalTokenSale;

    // total tokens on pre sale
    uint256 private totalOnPreSale;

    // seed sale account balances
    mapping(address => uint256) private pendingBalance;

    // token preSale price
    uint256 private TokenPriceBusd;
    ...
    constructor(
        uint256 _TokenPriceBusd,
        address _tokenAddr,
        address _busdAddr,
        address _OwnerAddr,
        address _withdrOwner
    ) {
        // Setting preSale token sell price
        TokenPriceBusd = _TokenPriceBusd;
        busdAddress = _busdAddr;
        // Setting TOKEN contract address
        tokenAddr = _tokenAddr;
        // Setting total raised and sale
        totalRaised = 0;
        totalTokenSale = 0;
        // Token owner
        _owner = _OwnerAddr;
    ...
    function ContractStatusClaim() public view returns (bool) {
        return _active;
    }
    ...
    function getPriceTokenBUSD() public view returns (uint256) {
    ...
    function buyToken(uint256 tokenQuant) public isPausable() {
        require(tokenQuant > 0, "You need to say how many tokens you want");
        uint256 tokenVal = tokenQuant.mul(TokenPriceBusd);
        require(tokenVal > 0 , "Insufficient amount for this transaction");

        // calc how many town tokens that will be given to buyer
        IBEP20 ContractToken = IBEP20(busdAddress);
        uint256 dexBalanceToken = ContractToken.balanceOf(msg.sender);
        require(dexBalanceToken > 0 && dexBalanceToken >= tokenVal, "You do not have enough balance for this transaction");
        ContractToken.transferFrom(msg.sender, _companyAddress, tokenVal);

        pendingBalance[msg.sender] += tokenQuant;
        
        // Increasing total raised
        totalRaised = totalRaised.add(tokenVal);
        totalTokenSale = totalTokenSale.add(tokenQuant);

        emit buyTokensSuccess(msg.sender, tokenQuant, tokenVal);
    }

    function claimTokens(uint256 amount) public isPausable() isClaiming() {
        ...
        IBEP20 ContractAdd = IBEP20(tokenAddr);
    ...
    function setTokenPriceBusd(uint256 amount) external onlyOwner() {
        TokenPriceBusd = amount;
    }

    function updateContracToken(address _tokenAddr) external onlyOwner() {
        tokenAddr = _tokenAddr;
    }

    function updateContracbusd(address _busdAddr) external onlyOwner() {
        busdAddress = _busdAddr;
    }
  
    function withdToBNB() public onlyOwner(){
        require(_withdOwner != address(0), "To make the withdrawal, you need to register a valid address.");
    ...
    function setClaiming() public onlyOwner() {
        if(_active){
            _active = false;
        }else{
            _active = true;
