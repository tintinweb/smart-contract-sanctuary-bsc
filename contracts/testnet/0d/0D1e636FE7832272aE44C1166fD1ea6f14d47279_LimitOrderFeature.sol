    ...
    FCXAuth
{
    ...
    function getFeeTokenAddress() public view override returns (address) {
        return LibNativeOrdersStorage.getStorage().feeToken;
    }

    function setFeeTokenAddress(address _feeToken) onlyAdmin public override {
        LibNativeOrdersStorage.getStorage().feeToken = _feeToken;
    }
    ...
    {    
        ...
        .getStorage()
        .orderLocked[orderInfo.orderHash] = order.makerAmount;
    ...
    function cancelLimitOrderWithHash(bytes32 orderHash, IERC20TokenV06 token, address maker) onlyAdmin public override
    {
        ...
        sendBalanceTo(
            IERC20TokenV06(LibNativeOrdersStorage.getStorage().feeToken),
            maker,
            stor.orderHashToFeeAmountRemaining[orderHash]
        );

        stor.orderHashToFilledAmount[orderHash] |= 1 << 255;
        stor.orderLocked[orderHash] = 0;
        stor.orderHashToFeeAmountRemaining[orderHash] = 0;
    ...
    returns (bytes4 success)
    {
        ...
        _registerFeatureFunction(this.setFeeTokenAddress.selector);
        _registerFeatureFunction(this.getFeeTokenAddress.selector);
...
interface ILimitOrderFeature
{
  ...
  function setFeeTokenAddress(address _feeToken) external;
}
