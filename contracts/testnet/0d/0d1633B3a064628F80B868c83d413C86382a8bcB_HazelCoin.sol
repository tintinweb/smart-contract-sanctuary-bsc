...
contract HazelCoin is Context, IERC20, Ownable {
    ...
    uint8 private maxLiqFee = 50;
    uint8 private maxTaxFee = 50;
    uint8 private maxBurnFee = 30;
    uint8 private maxHazelWallet = 30;
    ...
    // 100,000,000,000,000,000 HazelCoin
    uint256 public _tTotal = 100000 * 10**3 * 10**9 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    
    string public _name = "hazel";
    string public _symbol = "hzl";
    ...
    uint8 public _HazelWallet = 3;
    uint8 private _previousHazelWallet = _HazelWallet;
    ...
    address payable public HazelWalletFee = payable(0x47049f3008223532eFFC4D45BeBD813FE8245943);
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 burnFee, uint8 HazelWallet, uint8 buybackFee) external onlyOwner() {
        ...
        require(HazelWallet >= 0 && HazelWallet <=maxHazelWallet,"WF err");
    ...
    function setHazelWalletFee(address payable newHazelWalletFee) external onlyOwner {
        require(newHazelWalletFee != address(0), "ZERO ADDRESS");
        HazelWalletFee = newHazelWalletFee;
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _burnFee + _HazelWallet + _buybackFee).div(10**2);
    }
    
    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _HazelWallet == 0 && _buybackFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousBurnFee = _burnFee;
        _previousHazelWallet = _HazelWallet;
    ...
    function restoreAllFee() private {
        ...
        _HazelWallet = _previousHazelWallet;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        //burn
        uint8 totFee  = _burnFee + _HazelWallet + _liquidityFee + _buybackFee; 
        ...
        if (_HazelWallet != 0) {
            spentAmount = contractTokenBalance.div(totFee).mul(_HazelWallet);
            _tokenTransferNoFee(address(this), HazelWalletFee, spentAmount);
