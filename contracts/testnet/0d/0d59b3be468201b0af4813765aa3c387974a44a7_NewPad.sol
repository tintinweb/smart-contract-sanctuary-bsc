...
contract NewPad is IBEP20 {
    ...
    address private InhouseTeam = 0x307E2e4158fF2dF57efEaF853ed8a510c6599070;
    address private DevTeam = 0x3D75139af18Ee14a46F1d73228FA48CaCC39A791;
    ...
    constructor(
        ...
        _addressLocked[InhouseTeam] = true;
    ...
    function initiateValues() internal {
        ...
        specialAddBal[InhouseTeam] = 18000000 * (10**uint256(_decimals));
        specialAddBal[DevTeam] = 2000000 * (10**uint256(_decimals));
        ...
        _transfer(_owner, InhouseTeam, specialAddBal[InhouseTeam]);
        _transfer(_owner, DevTeam, specialAddBal[DevTeam]);
    ...
    function transfer(address to, uint256 value)
        public
        override
        checkLockedAddresses(msg.sender)
        returns (bool)
    {
    ...
    function transferFrom(
        ...
        address to,
        uint256 value
    ) public override checkLockedAddresses(from) returns (bool) {
     ...
     */
    function whitelistUserAdress(address[] calldata _userAddresses)
        external
        onlyOwner
        returns (bool)
    {
    ...
    modifier checkSaleValidations(address _userAddress, uint256 _value) {
        if (whitelistFlag == true) {
            require(
                _whitelistedAddress[_userAddress] == true,
        ...
        require(
            _contributionBNB[_userAddress].add(_value) >= saleMinimumAmount,
            "Total amount should be more than minimum limit"
        );
        require(
            _contributionBNB[_userAddress].add(_value) <= saleMaximumAmount,
            "Total amount should be less than maximum limit"
        );
    ...
    function checkTokensExpected(uint256 _value)
        external
        view
        returns (uint256)
    {
    ...
    function _trasferInLockingState(
        ...
        uint256 earnedTokens = _recordSale[_userAddress].add(
            (_amountTransfer.mul(tokenPrice)).div(decimalBalancer)
        );
        _recordSale[_userAddress] = earnedTokens;
        _finalSoldAmount[_userAddress] = earnedTokens;
        _contributionBNB[_userAddress] = _contributionBNB[_userAddress].add(
            msg.value
        );
     ...
     */
    function setIDOavailable(uint256 _IDOHardCap)
        external
        onlyOwner
        returns (bool)
    {
        require(
            _IDOHardCap <= balanceOf(IDO) && _IDOHardCap > 0,
            "Value should not be more than IDO balance and greater than 0"
        );
        hardCap = _IDOHardCap;
     ...
     */
    function claimPurchasedTokens(uint256 _lockNumber)
        external
        validateClaim(msg.sender, _lockNumber)
        returns (bool)
    {
        _transfer(IDO, msg.sender, _finalSoldAmount[msg.sender].div(4));
        _recordSale[msg.sender] = _recordSale[msg.sender].sub(
            _finalSoldAmount[msg.sender].div(4)
        );
        reEntrance[msg.sender][_lockNumber] = true;
        _claimedByUser[msg.sender][_lockNumber] = _finalSoldAmount[msg.sender]
            .div(4);
    }
    ...
    modifier validateClaim(address _userAddress, uint256 _lockNumber) {
        require(
            _recordSale[_userAddress] > 0,
            "Not sufficient purchase Balance"
        );
        require(
            _lockNumber == 1 ||
                _lockNumber == 2 ||
        ...
        if (_lockNumber == 1) {
            //Users will be able to withdraw tokens only after 1.5 hours of end time
            require(
                block.timestamp > saleEndTime + 5400 &&
                    reEntrance[_userAddress][_lockNumber] != true,
        ...
        if (_lockNumber == 2) {
            // 2 weeks
            require(
                block.timestamp > saleEndTime + 1209600 &&
                    reEntrance[_userAddress][_lockNumber] != true,
                "Insufficient Unlocked Tokens"
            );
        }
        if (_lockNumber == 3) {
            // 2 weeks
            require(
                block.timestamp > saleEndTime + 2419200 &&
                    reEntrance[_userAddress][_lockNumber] != true,
                "Insufficient Unlocked Tokens"
            );
        }
        if (_lockNumber == 4) {
            // 2 weeks
            require(
                block.timestamp > saleEndTime + 3628800 &&
                    reEntrance[_userAddress][_lockNumber] != true,
                "Insufficient Unlocked Tokens"
            );
        }
     ...
     */
    function checkWhitelistedAddress(address _userAddress)
        external
        view
        returns (bool)
    {
     ...
     */
    function withdrawSpecialLocked(address _toAddress, uint256 _amount)
        external
        checkLockingRoles(msg.sender, _amount)
        returns (bool)
    {
     ...
     */
    function getUserTokensBy(address _userAddress)
        external
        view
        returns (
            uint256 _div1,
    ...
    function getSoldStatus()
        external
        view
        returns (uint256 _totalAvailable, uint256 _currentAvailable)
    {
        _totalAvailable = hardCap;
        _currentAvailable = IDOAvailable;
    }

    function getAmountPurchased(address _userAddress)
        external
        view
        returns (uint256 _contribution, uint256 _allocation)
    {
     ...
     */
    function getClaimedTokensHistory(address _userAddress)
        external
        view
        returns (
            uint256 r1,
     ...
     */
    function setBnbPricePerToken(uint256 _price)
        external
        onlyOwner
        returns (bool)
    {
     ...
     */
    function getBnbPricePerToken() external view returns (uint256) {
    ...
    modifier checkLockingRoles(address _add, uint256 _amountRequested) {
        require(
            _add == InhouseTeam ||
                ...
                _add == Marketing ||
                _add == Reserve,
            "Only for Special Addresses"
        );
        require(_amountRequested != 0, "amount should be greater than 0");
        if (_add == Reserve) {
            require(
                block.timestamp > deploymentTime + 31556926,
                "Tokens are locked for 1 years from TGE"
            );
        } else if (_add == Marketing) {
            // Sixth month from the deployment
            require(
                block.timestamp > deploymentTime + 13148715,
                "Amount Locked for 6 month"
            );
        } else if (_add == InhouseTeam) {
            // Sixth month from the deployment
            require(
                block.timestamp > deploymentTime + 31556926,
                "Amount Locked for 6 month"
            );
        } else if (_add == DevTeam) {
            // Sixth month from the deployment
            require(
                block.timestamp > deploymentTime + 31556926,
                "Amount Locked for 6 month"
            );
        } else if (_add == Partners) {
            // Sixth month from the deployment
            require(
                block.timestamp > deploymentTime + 63113852,
                "Amount Locked for 2 month"
            );
        } else if (_add == Liquidity) {
            // Sixth month from the deployment
            require(
                block.timestamp > deploymentTime + 13148715,
                "Amount Locked for 6 month"
