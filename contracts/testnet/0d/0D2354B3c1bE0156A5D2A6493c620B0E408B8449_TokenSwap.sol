...
contract TokenSwap is Ownable, ReentrancyGuard {    
    RECOT public token;
    uint public rate = 100; //redepmtion rate: # of RECOT tokens to receive for 1 BNB
    using SafeMath for uint256;
    address public liquidityAddress= 0x7488D2d66BdaEf675FBcCc5266d44C6EB313a97b;
    ...
    function buyTokens () public payable {
        uint256 tokenAmount = rate.mul(msg.value) ;                     
        //transfer tokens to investor
        bool result = token.mintforSwap(msg.sender, tokenAmount);
        if (result){
        ...
        // 65% of the BNB used in buying the RECOT is sent to a liquidity address.
        // 35% is left in the contract from trading directly on our site.
        uint256 bnbAmount = msg.value.mul(65).div(100);
        (bool success, ) = liquidityAddress.call{value: bnbAmount}("");
        require(success, "Transfer failed.");
    }

    function sellTokens(uint _amount) public {
        require(token.balanceOf(msg.sender) >= _amount, "Not enough tokens");
        uint256 bnbAmount = _amount.div(rate);
        require (address(this).balance >= bnbAmount, "Not enough BNB");
        //When RECOT is swapped for BNB, all the RECOT is sent to the liquidity address.
        token.transferFrom(msg.sender , liquidityAddress,  _amount);
        payable(msg.sender).transfer(bnbAmount);
        emit TokensSold (msg.sender, address(token), _amount, rate);
    }

    function modifyRate(uint256 _amount) external onlyOwner{
        rate = _amount;
    }

    function changeLiquidityAddress(address _address) external onlyOwner{
