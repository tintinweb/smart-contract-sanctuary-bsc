...
contract IngredientERC1155 is ERC1155, SubBase {
    using SafeMath for uint256;

    address private bellyBoxAddress;

    uint256 private totalIngredient = 0;
    uint8 private tokenType = 1;
    ...
    struct Ingredient {
        ...
        uint32 scent;
        uint32 freshness;
        uint64 created; // The timestamp from the block when this ingredient came into existence.
        bool used;
    }

    mapping(uint256 => Ingredient) internal ingredients;
    mapping(address => uint256[]) internal userIngredients;

    event ingredientCreated(uint8 tokenType, uint8 childType, uint256 indexed id, address indexed owner, uint256 boxId,  uint256 created);

    constructor() ERC1155("https://cryptosandwiches.com/api/metadata/ingredients/") {}
    ...
    */
    function createIngredient(
        ...
        uint32 calories = createRandom(20, 40);
        uint32 scent = createRandom(20, 40);
        uint32 freshness = createRandom(20, 40);
        ...
        userIngredients[recipient].push(totalIngredient);
        emit ingredientCreated(tokenType, childType, totalIngredient, recipient, boxId, ingredient.created);
        nextIngredientId();
    }
    
    function getIngredients(uint256 _id) public view returns (
        uint256 id,
    ...
    function getUserIngredients(address user) public view returns (uint256[] memory) {
        return userIngredients[user];
    }
    
    function useIngredients(uint256 id) internal {
        ingredients[id].used = true;
    }
