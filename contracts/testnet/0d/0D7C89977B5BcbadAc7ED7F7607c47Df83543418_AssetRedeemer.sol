...
// File contracts/AssetRedeemer.sol
...
contract AssetRedeemer is Ownable, Pausable, ReentrancyGuard {
	...
	function redeem(address asset, uint256 assetAmount, uint256 pricePerUnit) external payable whenNotPaused nonReentrant returns (uint256) {
		...
		emit Redeem(lastId, msg.sender, asset, assetAmount, outputToken, outputAmount, pricePerUnit, msg.value);

		return lastId;
	}

	function fulfill(uint256 id, uint256 pricePerUnit, uint256 fee) external onlyRelayer {
		Order storage order = orders[id];
		require(order.status == OrderStatus.PENDING, "!pending");
		require(pricePerUnit >= order.pricePerUnit, "!price");
		order.status = OrderStatus.FULFILLED;

		uint256 outputAmount = ((order.assetAmount * pricePerUnit) / 1 ether) - fee;
		IERC20(order.asset).approve(address(manager), order.assetAmount);
		manager.burn(order.asset, order.assetAmount);
		IERC20(outputToken).safeTransferFrom(vault, address(this), outputAmount);
		IERC20(outputToken).safeTransfer(order.sender, outputAmount);

		emit Fulfill(id, order.sender, msg.sender, order.asset, order.assetAmount, pricePerUnit, fee);
	}
