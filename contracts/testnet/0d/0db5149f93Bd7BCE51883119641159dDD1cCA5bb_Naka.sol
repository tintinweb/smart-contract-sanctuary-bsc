...
contract Naka {
    ...
    uint private secureAddProcent = 5;
    ...
    function setSecureAddProcent(uint percent) public IsOnwer{
        secureAddProcent = percent;
    }

    function getSecureAddProcent() private IsOnwer view returns(uint){
        return secureAddProcent;
    }

    function __percentValue(uint _amount) public view returns(uint){
        return(secureAddProcent * _amount)/ 100;
    }
   ...
   emit ProductAdded(msg.sender, name, __amount(price));
    ...
    function transferTokens(address _owner, uint _price, address _buyer) private {
        require(_price <= token.balanceOf(_buyer), "Insucient tokens to make transfer");
        require(token.allowance(_buyer, address(this)) >= _price, "Insucient allwance to make reserve");
    ...
    function withdrawBNB(address payable account) external IsOnwer{
        (bool success, ) = account.call{value: address(this).balance}("");
        require(success);
    }

    function withdraw(address to, uint256 amount) external IsOnwer{
