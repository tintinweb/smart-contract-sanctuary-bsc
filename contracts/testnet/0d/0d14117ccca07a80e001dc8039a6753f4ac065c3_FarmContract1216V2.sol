...
contract FarmContract1216V2 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    address public refferalRewardToken;
    address public nodeRewardToken;
    ...
    mapping(uint256 => mapping(address => uint256)) public deservedReward;
    mapping(uint256 => mapping(address => mapping(address =>uint256))) public availableReffReward;
    mapping(uint256 => mapping(address => mapping(address =>uint256))) public availableNodeReward;
    mapping(uint256 => mapping(address => mapping(address =>uint256))) public claimedReffReward;
    mapping(uint256 => mapping(address => mapping(address =>uint256))) public claimedNodeReward;
    mapping(uint256 => mapping(address => mapping(address =>uint256))) public calculatedReffReward;
    mapping(uint256 => mapping(address => mapping(address =>uint256))) public calculatedNodeReward;
    ...
    function initialize(address _miningTokenContract, address _dividendTokenContract, address _inviteContract) initializer public {
        ...
        miningToken = _miningTokenContract;
        dividendToken = _dividendTokenContract;
        enableContract = false;
        lockupTime = 86400;
        redemptionGracePeriod = 43200;
        oneStarUserPledgeThreshold = 1 * 10 ** 18; 
        twoStarUserPledgeThreshold = 2 * 10 ** 18;
        threeStarUserPledgeThreshold = 3 * 10 ** 18;
        usdt = _dividendTokenContract;
        refferalRewardToken = _miningTokenContract;
        nodeRewardToken = _miningTokenContract;
        inviteContract = _inviteContract;
        referralRewardRate = 10;
        nodeRewardRate = 5;
        rateDenominator = 100;
    ...
    function pendingMiningReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalMiningReward) {
        ...
        if (block.number > poolInfo[_pid].lastUpdateBlock && lpSupply != 0) {
            uint256 blockInterval = block.number.sub(poolInfo[_pid].lastUpdateBlock);
        ...
        for(uint256 i = 0; i < userDepositInfo[_pid][_user].length; i++) {
            if(userDepositInfo[_pid][_user][i].onGoing) {
                ...
                totalMiningReward = totalMiningReward.add(pendingReward);
            }
        }
    }

    function pendingDividendReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalDividendReward) {
        ...
        for(uint256 i = 0; i < userDepositInfo[_pid][_user].length; i++) {
            if(userDepositInfo[_pid][_user][i].onGoing) {
                ...
                totalDividendReward = totalDividendReward.add(pendingReward);
            }
        }
    }

    function claimPendingMiningReward(uint256 _pid) internal returns (uint256 totalMiningReward) {
    ...
    function claimPendingDividendReward(uint256 _pid) internal returns (uint256 totalDividendReward) {
        for(uint256 i = 0; i < userDepositInfo[_pid][_msgSender()].length; i++) {
            if(userDepositInfo[_pid][_msgSender()][i].onGoing) {
                uint256 amount = userDepositInfo[_pid][_msgSender()][i].amount;
    ...
    function getDeservedRefferalReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalRefferalReward) {
        if(getLevel(_pid, _user) == 0)  return 0;
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            uint256 availableReward = availableReffReward[_pid][InviteeAddress[i]][_msgSender()].sub(claimedReffReward[_pid][InviteeAddress[i]][_msgSender()]);
            totalAvailableReward = totalAvailableReward.add(availableReward);
            uint256 deservedPendingReward = pendingMiningReward(_pid, InviteeAddress[i]).add(mintAmount[_pid][InviteeAddress[i]]);
            deservedPendingReward = deservedPendingReward.sub(calculatedReffReward[_pid][InviteeAddress[i]][_user]);
            deservedPendingReward = deservedPendingReward.mul(getRefferalRewardPercent(_pid, _msgSender())).div(rateDenominator);
            totalRefferalReward = totalRefferalReward.add(deservedPendingReward);
        }
        totalRefferalReward = totalRefferalReward.add(totalAvailableReward);
    }

    function getDeservedNodeReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalNodeReward) {
        if(getLevel(_pid, _user) <= 1)  return 0;
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        uint256 totalAvailableReward;
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            uint256 availableReward = availableNodeReward[_pid][InviteeAddress[i]][_msgSender()].sub(claimedNodeReward[_pid][InviteeAddress[i]][_msgSender()]);
            totalAvailableReward = totalAvailableReward.add(availableReward);
            if(getLevel(_pid, InviteeAddress[i]) < getLevel(_pid, _user) && depositAmount[_pid][InviteeAddress[i]] > 0) {
                uint256 deservedPendingReward = pendingMiningReward(_pid, InviteeAddress[i]).add(mintAmount[_pid][InviteeAddress[i]]);
                deservedPendingReward = deservedPendingReward.sub(calculatedNodeReward[_pid][InviteeAddress[i]][_user]);
                deservedPendingReward = deservedPendingReward.mul(getNodeRewardPercent(_pid, _msgSender())).div(rateDenominator);
                totalNodeReward = totalNodeReward.add(deservedPendingReward);
            }
        }
        totalNodeReward = totalNodeReward.add(totalAvailableReward);
    }

    function getAllPendingRefferalReward(address _user) public view returns (uint256 totalRefferalReward) {
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            uint256 pending = getDeservedRefferalReward(pid, _user);
            totalRefferalReward = totalRefferalReward.add(pending);
        }
    }

    function getAllPendingNodeReward(address _user) public view returns (uint256 totalNodeReward) {
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            uint256 pending = getDeservedNodeReward(pid, _user);
            totalNodeReward = totalNodeReward.add(pending);
        }
    }

    function claimRefferalReward(uint256 _pid) internal returns (uint256 totalRefferalReward) {
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_msgSender());
        uint256 totalAvailableReward;
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            uint256 availableReward = availableReffReward[_pid][InviteeAddress[i]][_msgSender()].sub(claimedReffReward[_pid][InviteeAddress[i]][_msgSender()]);
            claimedReffReward[_pid][InviteeAddress[i]][_msgSender()] = claimedReffReward[_pid][InviteeAddress[i]][_msgSender()].add(availableReward);
            totalAvailableReward = totalAvailableReward.add(availableReward);
            uint256 deservedPendingReward = pendingMiningReward(_pid, InviteeAddress[i]).add(mintAmount[_pid][InviteeAddress[i]]);
            deservedPendingReward = deservedPendingReward.sub(calculatedReffReward[_pid][InviteeAddress[i]][_msgSender()]);
            calculatedReffReward[_pid][InviteeAddress[i]][_msgSender()] = calculatedReffReward[_pid][InviteeAddress[i]][_msgSender()].add(deservedPendingReward);
            deservedPendingReward = deservedPendingReward.mul(getRefferalRewardPercent(_pid, _msgSender())).div(rateDenominator);
            deservedPendingReward = deservedPendingReward.add(availableReward);
            totalRefferalReward = totalRefferalReward.add(deservedPendingReward);
        }
        totalRefferalReward = totalRefferalReward.add(totalAvailableReward);
    }

    function claimNodeReward(uint256 _pid) internal returns (uint256 totalNodeReward) {
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_msgSender());
        uint256 totalAvailableReward;
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            uint256 availableReward = availableNodeReward[_pid][InviteeAddress[i]][_msgSender()].sub(claimedNodeReward[_pid][InviteeAddress[i]][_msgSender()]);
            claimedNodeReward[_pid][InviteeAddress[i]][_msgSender()] = claimedNodeReward[_pid][InviteeAddress[i]][_msgSender()].add(availableReward);
            totalAvailableReward = totalAvailableReward.add(availableReward);
            if(getLevel(_pid, InviteeAddress[i]) < getLevel(_pid, _msgSender())) {
                uint256 deservedPendingReward = pendingMiningReward(_pid, InviteeAddress[i]).add(mintAmount[_pid][InviteeAddress[i]]);
                deservedPendingReward = deservedPendingReward.sub(calculatedNodeReward[_pid][InviteeAddress[i]][_msgSender()]);
                calculatedNodeReward[_pid][InviteeAddress[i]][_msgSender()] = calculatedNodeReward[_pid][InviteeAddress[i]][_msgSender()].add(deservedPendingReward);
                deservedPendingReward = deservedPendingReward.mul(getNodeRewardPercent(_pid, _msgSender())).div(rateDenominator);
                deservedPendingReward = deservedPendingReward.add(availableReward);
                totalNodeReward = totalNodeReward.add(deservedPendingReward);
            }
    ...
    function claimAllRefferalReward() public returns(uint256 totalRefferalReward) {
        require(getAllPendingRefferalReward(_msgSender()) > 0);
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            uint256 deservedPendingReward = claimRefferalReward(pid);
            totalRefferalReward = totalRefferalReward.add(deservedPendingReward);
        }
        safeTokenTransfer(miningToken, _msgSender(), totalRefferalReward);
    }

    function claimAllNodeReward() public returns(uint256 totalNodeReward) {
        require(getAllPendingNodeReward(_msgSender()) > 0);
        for (uint256 pid = 0; pid < poolInfo.length; pid++) {
            uint256 deservedPendingReward = claimNodeReward(pid);
            totalNodeReward = totalNodeReward.add(deservedPendingReward);
        }
        safeTokenTransfer(miningToken, _msgSender(), totalNodeReward);
    }

    function snapShotReffReward(uint256 _pid) public {
        address firstInviter = IInviteContract(inviteContract).getInviter(_msgSender());
        address secondInviter = IInviteContract(inviteContract).getInviter(firstInviter);
        uint256 currentMiningReward = pendingMiningReward(_pid, _msgSender()).add(mintAmount[_pid][_msgSender()]);
        uint256 FDeservedReward = currentMiningReward.sub(calculatedReffReward[_pid][_msgSender()][firstInviter]);
        uint256 SDeservedReward = currentMiningReward.sub(calculatedReffReward[_pid][_msgSender()][secondInviter]);
        if(getLevel(_pid, _msgSender()) < getLevel(_pid, firstInviter) && getLevel(_pid, firstInviter) >= 1) {
            calculatedReffReward[_pid][_msgSender()][firstInviter] = calculatedReffReward[_pid][_msgSender()][firstInviter].add(FDeservedReward);
            uint256 deservedPendingReward = FDeservedReward.mul(getRefferalRewardPercent(_pid, firstInviter)).div(rateDenominator);
            availableReffReward[_pid][_msgSender()][firstInviter] = availableReffReward[_pid][_msgSender()][firstInviter].add(deservedPendingReward);
        }
       if(getLevel(_pid, _msgSender()) < getLevel(_pid, secondInviter) && getLevel(_pid, secondInviter) > 0) {
            calculatedReffReward[_pid][_msgSender()][secondInviter] = calculatedReffReward[_pid][_msgSender()][secondInviter].add(SDeservedReward);
            uint256 deservedPendingReward = SDeservedReward.mul(getRefferalRewardPercent(_pid, secondInviter)).div(rateDenominator);
            availableReffReward[_pid][_msgSender()][secondInviter] = availableReffReward[_pid][_msgSender()][secondInviter].add(deservedPendingReward);
        }
    }

    function snapShotNodeReward(uint256 _pid) public {
        address firstInviter = IInviteContract(inviteContract).getInviter(_msgSender());
        address secondInviter = IInviteContract(inviteContract).getInviter(firstInviter);
        uint256 currentMiningReward = pendingMiningReward(_pid, _msgSender()).add(mintAmount[_pid][_msgSender()]);
        uint256 FDeservedReward = currentMiningReward.sub(calculatedNodeReward[_pid][_msgSender()][firstInviter]);
        uint256 SDeservedReward = currentMiningReward.sub(calculatedNodeReward[_pid][_msgSender()][secondInviter]);
        if(getLevel(_pid, _msgSender()) < getLevel(_pid, firstInviter) && getLevel(_pid, firstInviter) >= 2) {
            calculatedNodeReward[_pid][_msgSender()][firstInviter] = calculatedNodeReward[_pid][_msgSender()][firstInviter].add(FDeservedReward);
            uint256 deservedPendingReward = FDeservedReward.mul(getNodeRewardPercent(_pid, firstInviter)).div(rateDenominator);
            availableNodeReward[_pid][_msgSender()][firstInviter] = availableNodeReward[_pid][_msgSender()][firstInviter].add(deservedPendingReward);
        }
       if(getLevel(_pid, _msgSender()) < getLevel(_pid, secondInviter) && getLevel(_pid, secondInviter) >= 2) {
            calculatedNodeReward[_pid][_msgSender()][secondInviter] = calculatedNodeReward[_pid][_msgSender()][secondInviter].add(SDeservedReward);
            uint256 deservedPendingReward = SDeservedReward.mul(getNodeRewardPercent(_pid, secondInviter)).div(rateDenominator);
            availableNodeReward[_pid][_msgSender()][secondInviter] = availableNodeReward[_pid][_msgSender()][secondInviter].add(deservedPendingReward);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public validatePid(_pid) {
        ...
        updatePool(_pid);
        snapShotReffReward(_pid);
        snapShotNodeReward(_pid);
        poolInfo[_pid].lpToken.transferFrom(_msgSender(), address(this), _amount);
    ...
    function withdraw(uint256 _pid) public validatePid(_pid)  {
        ...
        snapShotReffReward(_pid);
        snapShotNodeReward(_pid);
        poolInfo[_pid].lpToken.transfer(_msgSender(), getTotalRedeemableLPAmount(_pid, _msgSender()));
    ...
    function emergencyWithdraw(uint256 _pid) public validatePid(_pid) {
        ...
        require(depositAmount[_pid][_msgSender()] > 0,"The amount you stake in this pool is 0");
        updatePool(_pid);
        poolInfo[_pid].lpToken.transfer(_msgSender(), depositAmount[_pid][_msgSender()]);
    ...
    function safeTokenTransfer(address token, address to, uint256 amount) internal {
