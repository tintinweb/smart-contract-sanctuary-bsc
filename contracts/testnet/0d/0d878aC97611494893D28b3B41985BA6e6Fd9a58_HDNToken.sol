...
contract HDNToken is ERC20, Ownable, Pausable, AccessController {
  ...
  VotingContract public vote;

  event BlackListAdded(address account);
  event BlackListRemove(address account);

  constructor(address voteContractAddress) ERC20('HDN Token', 'HDN') {
    _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    _setupRole(PAUSER_ROLE, msg.sender);
    _mint(msg.sender, _initial_supply);
    vote = VotingContract(voteContractAddress);
  }
  ...
  function claim(uint256 amount, uint256 _voteId) public {
    require(vote.endVote(_voteId) >= 50, 'Not enough vote to claim token');
    ...
    address mintTo = vote.getOwnerOfProposal(_voteId);
    _totalClaim += amount;
    _mint(mintTo, amount);
  }
...
contract VotingContract is Ownable {
  ...
  mapping(uint256 => Vote) public voteOfProposal;
  mapping(address => voter) public voterRegister;
  uint256 public totalVoter = 0;
  uint32 public timeVote = 60;
  ...
  function setTime(uint32 _newTime) external onlyOwner {
    require(_newTime > 0, 'Please input new time > 0');
    timeVote = _newTime;
  }

  function getOwnerOfProposal(uint256 _voteId) public view returns (address) {
    return voteOfProposal[_voteId].chairman;
  }
  ...
  function addVoter(address _voterAddress, string memory _voterName) public CheckAddress(_voterAddress) onlyOwner {
    voterRegister[_voterAddress].voterName = _voterName;
    voterRegister[_voterAddress].voted = false;
    voterRegister[_voterAddress].id = 1;
    totalVoter++;
  }

  function removeVoter(address _voterAddress) public CheckAddress(_voterAddress) onlyOwner {
    require(voterRegister[_voterAddress].id != 0, 'voter is not include in group vote');
    delete voterRegister[_voterAddress];
    totalVoter--;
  }

  function addProposal(string memory _title) public {
    require(voterRegister[msg.sender].id != 0, 'You are not include in group vote');
    voteIdCount.increment();
    uint256 voteId = voteIdCount.current();
    Vote storage _vote = voteOfProposal[voteId];
  ...
  function doVote(uint256 _voteId) public {
    require(voterRegister[msg.sender].id != 0, 'You are not include in group vote');
    require(!voterRegister[msg.sender].voted, 'You voted this proposal, can vote more');
    require(voteOfProposal[_voteId].endTime > block.timestamp, 'End time, can not vote');

    Vote storage _vote = voteOfProposal[_voteId];
    _vote.totalVote++;
    voterRegister[msg.sender].voted = true;
  }

  function endVote(uint256 _voteId) public view returns (uint256) {
    require(voteOfProposal[_voteId].endTime <= block.timestamp, 'Not end time to close vote');
    uint256 pesentageVote = (voteOfProposal[_voteId].totalVote * 100) / totalVoter;
    return pesentageVote;
  }
}
