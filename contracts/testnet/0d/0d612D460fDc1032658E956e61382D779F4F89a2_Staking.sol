...
contract Staking is IStaking, Ownable, Pausable, ReentrancyGuard {
    ...
    function deployAllocationProvider() external onlyOwner returns (address) {
        BasicAllocationProvider allocationProvider = new BasicAllocationProvider();
        uint256 accountCount = _balances.length();
        for (uint256 i = 0; i < accountCount; i++) {
            (address account, uint256 balance) = _balances.at(i);
            allocationProvider.grantAllocation(account, balance);
        }
        allocationProvider.transferOwnership(msg.sender);
        return address(allocationProvider);
    }
}
...
contract BasicAllocationProvider is IAllocationProvider, Ownable {
        ...
        onlyOwner
    {
        ...
        _totalAllocation = _totalAllocation + _amount;
    }

    function revokeAllocation(address _account) external onlyOwner {
        _totalAllocation = _totalAllocation - _allocations[_account];
        _allocations[_account] = 0;
    }

    function allocation() external view returns (uint256) {
        return this.allocationOf(msg.sender);
    }
