...
contract Referral is IReferral, AccessControl, ReentrancyGuard {
    ...
    mapping(address => uint256) private _agentRevenueRates;
    ...
    event NewRef(uint256 id, uint256 ratio);
    event NewAgent(address agent, uint256 revenueRate);
    event NewRefAgent(uint256 id, uint256 ratio, address agent);
    event RegisteredReferrer(address referee, uint256 id);
    ...
    function addReferrer(address user, uint256 id) external onlyRole(GAME_ROLE) {
        ...
        // Use same account as ref
        if (address(uint160(id)) == user) {
            return;
        }

        // referrer is not exist
        if (ratios[id] == 0) {
            return;
        }

        _accounts[user] = id;
        emit RegisteredReferrer(user, id);
    }
    ...
    function payReferral(
        ...
        address referrer = address(uint160(_accounts[user]));
        uint256 saleoffRate = _ledger[referrer].agent != address(0)
            ? _agentRevenueRates[_ledger[referrer].agent]
            : BASE_REVENUE_RATE;

        uint256 totalReferral = (amount * saleoffRate) / DECIMALS;
        uint256 credit = (totalReferral * ratios[_accounts[user]]) / DECIMALS;
    ...
    function _makeId(uint96 i) internal view returns (uint256 id) {
    ...
    function createAgentRef(uint256 ratio, address agent) external {
        ...
        require(agent != msg.sender, "Should not input your self");
        require(agent != address(0), "Agent is zero address");

        uint96 currentIndex = _ledger[msg.sender].index;
        require(
            currentIndex != 0 && _ledger[msg.sender].agent != address(0),
            "you are freedom user"
        );

        if (_ledger[msg.sender].agent == address(0)) {
            // Create 1st ref
            require(_agentRevenueRates[agent] > 0, "Agent not exist");
            require(agent != address(0), "Zero address");
            _ledger[msg.sender].agent = agent;
        }

        uint256 id = _makeId(currentIndex);
        ratios[id] = ratio;
        _ledger[msg.sender].index++;

        emit NewRefAgent(id, ratio, _ledger[msg.sender].agent);
    }

    function createRef(uint256 ratio) external {
        require(_ledger[msg.sender].index < 9, "Max is 10 ref per adress");
        require(_ledger[msg.sender].agent == address(0), "Could not create ref for agent-ref");
        require(ratio <= DECIMALS, "Rate invalid");

        uint256 id = _makeId(_ledger[msg.sender].index);

        ratios[id] = ratio;
        _ledger[msg.sender].index++;

        emit NewRef(id, ratio);
    }

    function viewRefInfo(address referrer) external view returns (uint256[] memory) {
        uint96 currentIndex = _ledger[referrer].index;

        uint256[] memory _ratios = new uint256[](currentIndex);
    ...
    function createAgent(
        ...
        _agentRevenueRates[agent] = revenueRate;

        emit NewAgent(agent, revenueRate);
    }
    ...
    /// @param agent Address of the referrer.
    /// @param token Address of the token.
    /// @return The number of tokens available to withdraw.
    function agentCreditOf(address agent, address token) external view returns (uint256) {
        return _credits[agent][token];
    }
    ...
    function getReferralAccount(address user)
        external
        view
        returns (address referrer, uint96 index)
    {
        referrer = address(uint160(_accounts[user]));
        index = uint96(_accounts[user] >> 160);
    }
}
...
interface IReferral {
    ...
    /// @return index Identify the index.
    function getReferralAccount(address user) external view returns (address referer, uint96 index);
}
