...
contract MysteryBoxPresale is Ownable {
    ...
    // @notice Price of one mystery box
    uint256 public price;

    event MysteryBoxReceipt(address buyer, uint256 amount, uint256 price);
     ...
     */
    function buyMysteryBox(uint256 _amount) external {
        uint256 purchasePrice = price * _amount;
        IERC20 token = IERC20(tokenAddress);
        uint256 allowance = token.allowance(msg.sender, address(this));
        require(allowance >= purchasePrice, "Insufficient allowance");
        token.transferFrom(msg.sender, address(this), purchasePrice);
        emit MysteryBoxReceipt(msg.sender, _amount, purchasePrice);
    }
