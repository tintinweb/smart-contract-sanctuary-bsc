...
contract GlobalVillage is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public transferDeadFee = 5;
    ...
    uint256 public minTokenOfInviterReward = 10 * 10 ** 18;
    ...
    constructor(
        ...
        excludeFromFees(superNodeWallet, true);
    ...
    function setTransferDeadFee(uint256 amount) public onlyOwner {
        transferDeadFee = amount;
    }
    ...
    function _transfer(
        ...
        if(!tradeOpen && from != owner() && to != owner()){
            require(_msgSender() != address(uniswapV2Router) && !automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] , "Trade not open");
        }
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to]){
                if(balanceOf(deadWallet) < totalSupply().mul(90).div(100)){
                    uint256 DFee = originAmount.mul(deadFee).div(100);    
                    if(DFee > 0){
                ...
                (uint256 IFeeAmount, uint256 IFeeCurrent) = takeInviter(from, to, originAmount);
                if(IFeeCurrent > 0){
                    super._transfer(from, fundWallet, IFeeCurrent);
                }
                amount = amount.sub(IFeeAmount);
            }else{
                if(balanceOf(deadWallet) < totalSupply().mul(90).div(100)){
                    uint256 DFee = originAmount.mul(transferDeadFee).div(100);    
                    if(DFee > 0){
                        super._transfer(from, deadWallet, DFee);
                        amount = amount.sub(DFee);
                    }
    ...
    function takeInviter(address from, address to, uint256 amount) private returns(uint256,uint256)  {
            ...
            if (IFeeAmount > 0){
                    ...
                    for (int256 i = 0; i < 10; i++) {
                        ...
                        if (balanceOf(cur) < minTokenOfInviterReward) {
                            continue;
                        }
