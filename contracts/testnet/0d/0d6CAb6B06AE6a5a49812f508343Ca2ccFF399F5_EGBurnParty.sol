...
contract EGBurnParty is Ownable, KeeperCompatibleInterface, ReentrancyGuard {
    ...
    event EndBurnPartyV2(uint256 partyId, address indexed caller, address indexed burnToken, uint256 indexed amount, uint256 realAmount, address burnAddress);
    event CancelBurnParty(uint256 partyId, address indexed caller, address indexed burnToken, uint256 indexed amount);
    event CancelBurnPartyV2(uint256 partyId, address indexed caller, address indexed burnToken, uint256 indexed amount);
    event StakeBurnParty(uint256 indexed partyId, address indexed staker, uint256 indexed amount, uint256 gasFeeAmount);
    event StakeBurnPartyV2(uint256 indexed partyId, address indexed staker, uint256 indexed amount, uint256 realAmount, uint256 gasFeeAmount);
    event UnstakeFromBurnParty(uint256 indexed partyId, address indexed staker, uint256 indexed amount);
    event UnstakeFromBurnPartyV2(uint256 indexed partyId, address indexed staker, uint256 indexed amount, uint256 realAmount);
    ...
    */
    function endBurnParty(uint256 partyId) public nonReentrant(){
        ...
        emit EndBurnPartyV2(partyId, msg.sender, _party.token, _party.currentQuantity, _burnAmount, burnTokens[_party.token].burnAddress);
    }
    ...
    */
    function stakeBurnParty(uint256 partyId, uint256 tokenAmount) external payable nonReentrant() {
        BurnParty storage _party = burnParties[partyId];
        ...
        emit StakeBurnPartyV2(partyId, msg.sender, tokenAmount, _tokenAmount, msg.value);
    }
    ...
    */
    function unstakeFromBurnParty(uint256 partyId) external {
        ...
        emit UnstakeFromBurnPartyV2(partyId, msg.sender, _stakeInfo.amount, _amount);
    }
}