...
contract SantaClubStaking is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    function isStakingAvailable()
        public
        returns (bool)
    {
        uint256 balanceOfStakingVault = rewardToken.balanceOf(address(this));
        stakingAvailable = balanceOfStakingVault > 0;
        return stakingAvailable;
    }
    ...
    function getTotalClaimableRewards(address account) public view returns (uint256) {
        uint256 tokenId;
    ...
    function stakeNFTs(uint256[] calldata tokenIds) external nonReentrant {
        require(stakingAvailable, "Stake NFTs: There are no more rewards in the staking vault.");
        uint256 tokenId;
        uint256 stakedCount;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];
    ...
    function _unstake(address account, uint256[] calldata tokenIds)
        internal
    {
