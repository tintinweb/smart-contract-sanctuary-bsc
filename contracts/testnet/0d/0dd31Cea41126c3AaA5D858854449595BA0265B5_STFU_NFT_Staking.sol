...
contract STFU_NFT_Staking is ReentrancyGuard, Ownable {
    ...
    event WithdrawnRewardsToken(address indexed to, uint256 indexed amount);
    ...
    function claimRewards() external {
        ...
        if (address(rewardsToken) == address(0)) { //bnb
    ...
    function getAllStakedInfo() public view returns (uint256[] memory, address[] memory) {
        uint256 bal = nftCollection.balanceOf(address(this));
        uint256[] memory tokens = new uint256[](bal);
        address[] memory owners = new address[](bal);
        for (uint256 i = 0; i < bal; i++) {
            tokens[i] = IERC721Enumerable(address(nftCollection)).tokenOfOwnerByIndex(address(this), i);
            owners[i] = stakerAddress[tokens[i]];
        }
    ...
    function ownerWithdrawTokens() public nonReentrant onlyOwner {
        ...
        if (address(rewardsToken) == address(0)) { //bnb
            ...
            bal = rewardsToken.balanceOf(address(this));
            require(bal > 0, "No tokens to withdraw.");
            rewardsToken.safeTransfer(msg.sender, bal);
        }
        emit WithdrawnRewardsToken(msg.sender, bal);
    }
}