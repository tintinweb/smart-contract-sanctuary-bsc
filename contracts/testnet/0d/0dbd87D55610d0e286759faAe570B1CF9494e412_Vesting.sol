...
contract Vesting is Ownable, ReentrancyGuard {
	...
	event DepositTokenToContract(address indexed accout, uint256 amount);

	event TokenRescueWithdrawn(address indexed accout, uint256 amount);

	event WithdrawTokenToContract(uint256 amount);

	event AddedVestingSchedules(uint256 amount);

	event CreatedVestingSchedule(address indexed accout, uint256 startTime, uint256 vestingPeriod, uint256 vestingCliff, uint256 releaseInterval, uint256 amount);
	...
	function addVestingSchedules(
		...
		emit AddedVestingSchedules(_totalAmount);
	}
	...
	function createVestingSchedule(
		...
		emit CreatedVestingSchedule(_recipient, _startTime, _vestingPeriod, _vestingCliff, _releaseInterval, _amount);
	}
