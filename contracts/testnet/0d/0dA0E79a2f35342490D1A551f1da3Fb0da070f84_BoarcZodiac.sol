...
import "contracts/token/ERC1155/ERC1155Supply.sol";
import "contracts/access/Ownable.sol";
import "contracts/utils/Pausable.sol";
import "contracts/utils/SafeTransfer.sol";
import "contracts/utils/libraries/EnumerableSet.sol";
import "contracts/utils/libraries/Strings.sol";

contract BoarcZodiac is ERC1155, Ownable, Pausable, ERC1155Supply, SafeTransfer {
	using EnumerableSet for EnumerableSet.AddressSet;
	using Strings for uint256;

	uint256 public constant MAX_SUPPLY = 12;
	uint256 public constant MAX_AMOUNT = 12;
	...
	function remainingAmount() public view returns (uint256 amount) {
		uint256 amountMinted;
		for (uint256 id = 1; id <= 12; id++) {
			amountMinted += totalSupply(id);
		}
		amount = (MAX_SUPPLY * MAX_AMOUNT) - amountMinted;
	}
	...
	function mint(
		...
		require(id >= 1 && id <= MAX_SUPPLY, "BZ: id invalid");
		require(MAX_AMOUNT - totalSupply(id) >= amount, "BZ: amount invalid");

		_mint(account, id, amount, data);
	}

	function claim(uint256 id, uint256 amount) public payable whenNotPaused {
		require(msg.value >= amount * feeMint, "BZ: Insufficient fee");
		require(id >= 1 && id <= MAX_SUPPLY, "BZ: id invalid");
		require(MAX_AMOUNT - totalSupply(id) >= amount, "BZ: amount invalid");

		payable(feeRecipient).transfer(feeMint);
		uint256 diff = msg.value - (amount * feeMint);
		if (diff > 0) payable(_msgSender()).transfer(diff);

		_mint(_msgSender(), id, amount, "");

		if (!whitelist.contains(msg.sender)) whitelist.add(msg.sender);

		emit Claimed(_msgSender(), id, amount);
	}
