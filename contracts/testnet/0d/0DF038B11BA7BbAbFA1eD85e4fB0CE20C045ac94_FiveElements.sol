...
contract FiveElements is Permission {
    ...
    string  public _newhash;
    function luckFive() private {
        string memory newHash = _getRandomNumber(msg.sender);
        _newhash = newHash;
    ...
    function get5(string memory _hash) public pure returns(uint8) {
        uint8 count = 0;
        bytes memory _32Hash = bytes(_hash);
        for(uint8 i = 31; i >= 0; i--){
            uint8 b = uint8(_32Hash[i]) % 16;
            if(b==5) count++;
            uint8 c = uint8(_32Hash[i]) / 16;
            if(c==5) count++;
        }
        return count;
    }
    ...
    string public _AnewHash;
    function _getRandomNumber(address player) public returns (string memory){
        _updateRamdomSeed();
        _Bhash =  keccak256(abi.encodePacked(
            randomSeed, block.timestamp, (block.number - 1), player
        ));
        for(uint8 i = 0; i < 31; i++){
            _AnewHash = string(abi.encodePacked(_AnewHash, _Bhash[i]));
        }
        // _newhash = _AnewHash;
        return _AnewHash;
    }

    function _getRandomNumber2(address player) public returns (string memory){
        _updateRamdomSeed();
