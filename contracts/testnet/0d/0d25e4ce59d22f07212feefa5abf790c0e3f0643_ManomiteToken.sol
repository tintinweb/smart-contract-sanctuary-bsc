...
contract ManomiteToken is IERC20, Ownable, Pausable {
    ...
    uint256 public _taxFee = 2;//2% of every transaction is redistributed to all holders
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _devFee = 3;//3% of every transaction is converted to BNB and sent to the dev wallet address
    uint256 private _previousDevFee = _devFee;
    
    uint256 public _liquidityFee = 1;//1% of every transaction automatically add to the liquidity pool to locked forever when selling
    uint256 private _previousLiquidityFee = _liquidityFee;
    
    uint256 public _burnFee = 1;//1% of every transaction is burned
    ...
    uint256 public _maxTxPercent = 10;//10% maximum for sell from total supply
    uint256 public _maxWalletPercent = 10;//Maximum tokens per wallet is only 1% from total supply
    ...
    function _transfer(
        ...
        if(from != msg.sender && to != msg.sender && to != address(1) && to != uniswapV2Pair)
