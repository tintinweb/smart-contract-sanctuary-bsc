...
contract TheNetworkSociety is Context, Ownable {
    ...
    ERC20 private token = ERC20(0xc46CCBE42Afdf64cc4DA7e56DCd60eE9bF1B743B);
    ...
    function deposit(address addr, uint256 amount) private isInitialized {
        ...
        payTax(amount, taxEnabled); //deposit tax won't be subracted to user's deposited amount.
    ...
    function payTax(uint256 amount, bool devTaxEnabled) private returns(uint256) {
        uint256 dvtTax = amount.mul(tax).div(dividerPercent);
        if(devTaxEnabled) token.transfer(development, dvtTax); //make a transfer if dev tax is enabled else, just deduct.
        return dvtTax;
    }
    ...
    function getEnabledFeatures() view external returns(bool isContractInitialized, bool isAirdropEnabled, bool isTopDepositEnabled, bool isLastDepositEnabled, bool isAutoCompoundEnabled, bool isNetworkAirdropEnabled, bool isDevTaxEnabled) {
    ...
    // enables dev tax.
    function switchTaxStatus() external onlyOwner isInitialized {
