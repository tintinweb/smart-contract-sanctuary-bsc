...
contract YieldMachineBUSD is Ownable {
    ...
    function getUserRawTotalDeposit(address user) public view returns (uint256) {
    ...
    function getUserAvailable(address user) public view returns (uint256) {
        return getUserRawTotalDeposit(user).add(calculateDividends(user)).sub(getUserTotalWithdrawn(user));
    }
