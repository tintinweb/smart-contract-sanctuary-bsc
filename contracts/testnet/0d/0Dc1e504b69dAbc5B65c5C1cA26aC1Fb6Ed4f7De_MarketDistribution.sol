...
contract MarketDistribution is TokensRecoverable, IMarketDistribution {
    ...
    function init(
        ...
        uint256 _projectTokens,
        uint256 _vestingDuration, 
        uint16 _projectSetupPercent, 
        uint16 _preBuyForContributorsPercent, 
        uint16 _preBuyForMarketStabilizationPercent) public onlyOwner
    {
        rootedToken = _rootedToken;
    ...
    function distribute() public override {
        ...
        rootedToken.transfer(multisplitterAddress, projectTokens);
    ...
    function createRootedEliteLiquidity() private {
        eliteToken.depositTokens(baseToken.balanceOf(address(this)));
        uniswapV2Router.addLiquidity(address(eliteToken), address(rootedToken), eliteToken.balanceOf(address(this)), rootedToken.balanceOf(address(this)), 0, 0, address(this), block.timestamp);
    }
    ...
    function getBaseRooted() public view returns (IUniswapV2Pair) {
        return rootedBaseLP;
    }
...
abstract contract GatedERC20 is StandardERC20, AccessControl, Pausable, TokensRecoverable, IGatedERC20
{
    using SafeMath for uint256;
    ...
    constructor(string memory _name, string memory _symbol) StandardERC20(_name, _symbol)
    {
    }
