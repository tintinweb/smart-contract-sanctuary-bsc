...
contract LumanagiPredictionV1 is Ownable, Pausable, ReentrancyGuard, AutomationCompatibleInterface  {
    ...
    bool public genesisLockRoundCalledStatus = false;
    bool public executeRoundCalledStatus = false;
    ...
    event CheckUpKeepChainLink(string functionName, bool upkeepNeeded);
    ...
    {
        //if (genesisStartOnce && needGenesisLock) {
        //    if ((block.timestamp >= _nextGenesisLockOnceExecuteTimeStamp) && block.timestamp < (_nextGenesisLockOnceExecuteTimeStamp + bufferSeconds))
        //    {
        //        upkeepNeeded = true;
        //    } 
        //}
        if (needExecuteRound) {
            if (block.timestamp >= _nextExecuteRoundTimeStamp && block.timestamp < (_nextExecuteRoundTimeStamp + bufferSeconds))
            {
                upkeepNeeded = true;
            }
        }
    ...
    function performUpkeep(bytes calldata /* performData */) external override 
    {
       //We highly recommend revalidating the upkeep in the performUpkeep function
        //if (needGenesisLock) {
        //    genesisLockRound();
        //    genesisLockRoundCalledStatus = true;
        //    emit CheckUpKeepChainLink("genesisLockRound()", genesisLockRoundCalledStatus);
        //}
        
        if (needExecuteRound) {
            executeRound();
            executeRoundCalledStatus = true;
            emit CheckUpKeepChainLink("executeRound()", executeRoundCalledStatus);
        }
    }
    ...
    function executeRound() internal {  //whenNotPaused onlyOperator
        ...
        // Keep the next timestamp to execute executeRound() via Chainlink performUpKeep
        _nextExecuteRoundTimeStamp = block.timestamp + intervalSeconds;
    }

    /**
     * @notice Start the next round n, lock price for round n-1, end round n-2
     * @dev Callable by operator
     * Demo Function to test temporary
     */
    function executeRoundTest() external whenNotPaused onlyOperator {
        require(
            genesisStartOnce && genesisLockOnce,
            "Can only run after genesisStartRound and genesisLockRound is triggered"
        );
