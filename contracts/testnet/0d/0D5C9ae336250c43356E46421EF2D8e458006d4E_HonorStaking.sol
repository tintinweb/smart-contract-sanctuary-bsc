...
contract HonorStaking is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  // 1M HONOR --> $1 per HONOR --> 1M HONOR --> 25% APR --> 150k HONOR per year
  // Diamond Hand --> +10% APR --> 30 days
  // NFT Reward --> +5% APR --> 30 days
  // HONOR to return per year (depositedHonorAmount / 4)
  // multiplierConstant comes from: pendingRewards = (((depositedHonorAmount / 4) / 365.25) / 86400) * secondsElapsed

  Metadata public metadataAddress;
  // uint multiplierConstant = 126230400; // 25%
  // uint multiplierConstant = 157788000; // 20%
  uint multiplierConstant = 3155760; // 1000% for testnet
  // uint public cooldownTimeInSeconds = 2.592 * 10 ** 6;  // 30 days in seconds
  uint public cooldownTimeInSeconds = 600;  // 30 days in seconds testnet
  uint public maxStake = 1000000 * 10 ** 18; // 1M HONOR LIMIT
  address Creator;
  address public TaxWallet;
  uint creatorshare; uint taxshare;

  struct Account {
    uint depositedHonorAmount;
    uint honorRewards;
    uint updatedAt;
  ...
  event Deposit(address indexed account, uint256 depositedHonorAmount);
  event WithdrawHonor(address indexed account, uint256 depositedHonorAmount);

  IERC20 public honor;
  bool public stakingAlive = false;

  constructor(address _honorAddress, Metadata _metadataAddress) {
    honor = ERC20(_honorAddress);
  ...
  // Deposit HONOR for staking
  function deposit(uint _amount) public {
    require(stakingAlive == true, "Honor Protocol Token Staking Offline");
    require(honor.balanceOf(address(this)) <= maxStake, "1M max stake reached");

    accounts[msg.sender].honorRewards += pendingHonorRewards(block.timestamp); // update pending rewards at deposit
    accounts[msg.sender].depositedHonorAmount += _amount;
    accounts[msg.sender].updatedAt = block.timestamp;
    honor.transferFrom(msg.sender, address(this), _amount);
    emit Deposit(address(msg.sender), _amount);
  }
  // Withdraw HONOR from staking
  function withdrawHonor(address _to, uint wdType, uint _tokenId, uint _amount) public {
    require(msg.sender == _to, "Only user can widthraw its own funds.");
    
    accounts[msg.sender].honorRewards += pendingHonorRewards(block.timestamp); // update pending rewards at withdrawal
    require(_amount <= accounts[msg.sender].depositedHonorAmount, "HONOR Amount error");

    if (wdType == 1) {
      uint updatedAt = (block.timestamp - accounts[msg.sender].updatedAt) / 600; // 7 days
      require(updatedAt >= 1, "Not past 7 days");

      accounts[msg.sender].depositedHonorAmount -= _amount; // withdraw amount
      accounts[msg.sender].updatedAt = block.timestamp;
      honor.transfer(msg.sender, _amount);
    }
    else if (wdType == 2) {
      require(metadataAddress.checkCounter(1,_tokenId) < 30, "Token ID Need Maintenance - Input Token A "); // Army Rank

      uint updatedAt = (block.timestamp - accounts[msg.sender].updatedAt) / 180; // 24 hours 86400
      require(updatedAt >= 1, "Not past 24 hours");

      metadataAddress.addCounter(_to, 4, _tokenId, 0);
      accounts[msg.sender].depositedHonorAmount -= _amount; // withdraw amount
      accounts[msg.sender].updatedAt = block.timestamp;
      honor.transfer(msg.sender, _amount);
    }
    emit WithdrawHonor(address(msg.sender), _amount);
  }
  // Withdraw HONOR Rewards
  function withdrawHonorRewards(address _to, uint wdType, uint _tokenId, uint _amount) public {
    require(msg.sender == _to, "Only user can widthraw its own funds.");
    require(TaxWallet != address(0), "Tax Wallet Zero Address");

    accounts[msg.sender].honorRewards += pendingHonorRewards(block.timestamp); // update pending rewards at withdrawal
    require(_amount <= accounts[msg.sender].honorRewards, "HONOR Amount error");

    if (wdType == 1) {
      uint daysSinceLastWithdrawal = (block.timestamp - accounts[msg.sender].lastWithdrawalAt) / 180; // 24 hours 86400
      require(daysSinceLastWithdrawal >= 1, "Not past 24 hours");

      accounts[msg.sender].honorRewards -= _amount; // withdraw amount
      accounts[msg.sender].updatedAt = block.timestamp;
      _amount = _amount * 100 * 95;
      taxshare = _amount / 100 * 2;
      honor.transfer(msg.sender, _amount);
      honor.transfer(TaxWallet, taxshare);
    }
    else if (wdType == 2)
    {
      uint rewardWithdrawal = (block.timestamp - accounts[msg.sender].lastWithdrawalAt) / cooldownTimeInSeconds; // 30-days
      require(rewardWithdrawal >= 1, "Need 30 days to claim 10% reward without NFT");
      
      accounts[msg.sender].honorRewards -= _amount; // withdraw amount
      accounts[msg.sender].updatedAt = block.timestamp;
      uint reward10 = _amount / 100 * 110;
      _amount = reward10 / 100 * 95;
      taxshare = reward10 / 100 * 2;
      honor.transfer(msg.sender, _amount);
      honor.transfer(TaxWallet, taxshare);
    }
    else if (wdType == 3) {
      ...
      accounts[msg.sender].honorRewards -= _amount; // withdraw amount
      accounts[msg.sender].updatedAt = block.timestamp;
      Creator = metadataAddress.checkCreator(1, _tokenId);
      uint reward15 = _amount / 100 * 115;
      _amount = reward15 / 100 * 95;
      creatorshare = reward15 / 100 * 3;
      taxshare = reward15 / 100 * 2;

      honor.transfer(msg.sender, reward15);
      honor.transfer(Creator, creatorshare);
      honor.transfer(TaxWallet, taxshare);
  ...
  function pendingHonorRewards(uint _timestamp) public view returns(uint) {
    ...
    return (accounts[msg.sender].depositedHonorAmount * secondsElapsed) / multiplierConstant;
  }
  function totalHonorRewards(uint _timestamp) public view returns(uint) {
    return accounts[msg.sender].honorRewards + pendingHonorRewards(_timestamp);
  }
  function totalHonorStaked() public view returns(uint) {
    return honor.balanceOf(address(this));
  }

  // In the future it might be necessary to remove the reward token from the pool
  function emergencyWithdrawHonor(uint _amount) public onlyOwner {
    honor.transfer(msg.sender, _amount);
  }
