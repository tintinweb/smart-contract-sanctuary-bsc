    ...
    ReentrancyGuard
{
    ...
    constructor(MediaEyeOrders.AuctionConstructor memory _auctionConstructor) {
        require(_auctionConstructor.treasuryWallet != address(0));
        require(_auctionConstructor.basisPointFee <= 500);
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, _auctionConstructor.owner);
        for (uint256 i = 0; i < _auctionConstructor.admins.length; i++) {
            _setupRole(ROLE_ADMIN, _auctionConstructor.admins[i]);
        }

        treasuryWallet = _auctionConstructor.treasuryWallet;
        feeContract = _auctionConstructor.feeContract;

        basisPointFee = _auctionConstructor.basisPointFee;
        mediaEyeMarketplaceInfo = IMarketplaceInfo(
            _auctionConstructor.mediaEyeMarketplaceInfo
        );

        for (uint256 i = 0; i < _auctionConstructor.priceFeeds.length; i++) {
            require(_auctionConstructor.priceFeeds[i] != address(0));
            priceFeeds[
                IChainlinkPriceFeeds(
                    _auctionConstructor.priceFeeds[i]
                ).getTokenAddress()
            ] = _auctionConstructor.priceFeeds[i];
        }
        ...
        returns (uint256)
    {
        ...
        if (_auctionInput.chainlinkPayment.isValid) {
            ...
            if (
                _auctionInput.chainlinkPayment.quoteAddress == address(0)
            ) {
                require(
                    auctionBuyItNowAmounts[auctionId][_auctionInput.chainlinkPayment.baseAddress] >
                        0,
                    "payment"
                );
                require(
                    priceFeeds[_auctionInput.chainlinkPayment.baseAddress] != address(0),
                    "price feed not supported"
                );
                salePriceFeed[auctionId] = priceFeeds[_auctionInput.chainlinkPayment.baseAddress];
            } else {
                require(
                    _auctionInput.chainlinkPayment.baseAddress == address(0),
                    "impossible chainlink payment"
                );
                require(
                    auctionBuyItNowAmounts[auctionId][address(0)] > 0,
                    "payment"
                );
                require(
                    priceFeeds[_auctionInput.chainlinkPayment.quoteAddress] != address(0),
                    "price feed not supported"
                );
                salePriceFeed[auctionId] = priceFeeds[_auctionInput.chainlinkPayment.quoteAddress];
            }
    ...
    function auctionBuyItNow(
        ...
        MediaEyeOrders.PaymentChainlink memory chainlink = saleChainlinkAddresses[_auctionId];
