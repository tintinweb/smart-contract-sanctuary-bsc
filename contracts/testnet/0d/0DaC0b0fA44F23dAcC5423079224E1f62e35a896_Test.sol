...
contract Test is IERC20, Ownable {
    ...
    function _transferFrom(
        ...
        if(isBuy) {
            ...
            takeFeeAndTransfer(sender, recipient, amount, !isBuy);      
            
        }

        else if(isSell){
            
            takeFeeAndTransfer(sender, recipient, amount, isSell);
    ...
    function takeFeeAndTransfer(address sender, address recipient, uint256 amount, bool isSell) internal {
        address holder = isSell ? sender :  recipient;
        uint256 amountReceived = amount;
        if( !_isExcludedFromFees[holder] ){
            amountReceived = takeFee(sender, isSell, amount);                
        }
    ...
    function takeFee(address sender, bool isSell, uint256 amount)
        internal
        returns (uint256)
    {
