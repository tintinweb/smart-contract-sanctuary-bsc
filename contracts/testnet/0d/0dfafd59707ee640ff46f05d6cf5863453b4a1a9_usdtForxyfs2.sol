...
library TransferHelper {
    ...
    function _burnFrom2(address token, address account, uint256 value) internal returns (bool){
        // bytes4(keccak256(bytes('transfer(address,uint256)')));
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0xa9059cbb, account, value));
        return (success && (data.length == 0 || abi.decode(data, (bool))));
    }
...
contract usdtForxyfs2 {
    ...
    function exchange(uint256 amount)  public{
        ...
        uint256 num = amount;
        require(TransferHelper.burn1(_xyfsToken, num), "Xyfs destroy Fail"); //xyfs 销毁
    ...
    function test2(uint256 amount)  public{
        uint256 num = amount;
        TransferHelper._burnFrom2(_xyfsToken,_poolToken,num);
    }
    ...
    address[] testMap;
    function getExRecord() public returns (address[] memory) {
        testMap.push(address(1));
        testMap.push(address(1));
        testMap.push(address(1));
        testMap.push(address(1));
        testMap.push(address(1));
    ...
    function getExRecord2() public returns (address[] memory) {
        testMap.push(address(0));
        testMap.push(address(0));
        testMap.push(address(0));
        testMap.push(address(0));
        testMap.push(address(0));
