...
contract ClaimNFTs is Ownable, ERC721Holder{
    ...
    function setNFTs(address _nursery, address _training, address _labo, address _ticket)external onlyOwner{
    ...
    function setNurseryOwner(uint256 _tokenId, address _user) external onlyOwner{
        // need this NFT to be in the smart contract
        require(nursery.ownerOf(_tokenId) == address(this),"NFT not on this smart contract");
        // if NFT already atributed, update it
        if(_nurseryOwner[_tokenId] != address(0x0)){
            EnumerableSet.remove(myNurseries[_nurseryOwner[_tokenId]], _tokenId);
        }
        _nurseryOwner[_tokenId] = _user;
        EnumerableSet.add(myNurseries[_user], _tokenId);
    }

    function setTrainingOwner(uint256 _tokenId, address _user) external onlyOwner{
        require(trainingCenter.ownerOf(_tokenId) == address(this),"NFT not on this smart contract");
        if(_trainingOwner[_tokenId] != address(0x0)){
            EnumerableSet.remove(myTrainings[_trainingOwner[_tokenId]], _tokenId);
        }
        _trainingOwner[_tokenId] = _user;
        EnumerableSet.add(myTrainings[_user], _tokenId);
    }

    function setLaboratoryOwner(uint256 _tokenId, address _user) external onlyOwner{
        require(laboratory.ownerOf(_tokenId) == address(this),"NFT not on this smart contract");
        if(_laboratoryOwner[_tokenId] != address(0x0)){
            EnumerableSet.remove(myLaboratories[_laboratoryOwner[_tokenId]], _tokenId);
        }
        _laboratoryOwner[_tokenId] = _user;
        EnumerableSet.add(myLaboratories[_user], _tokenId);
    }

    function setTicketOwner(uint256 _tokenId, address _user) external onlyOwner{
        require(ticket.ownerOf(_tokenId) == address(this),"NFT not on this smart contract");
        if(_ticketOwner[_tokenId] != address(0x0)){
            EnumerableSet.remove(myTickets[_ticketOwner[_tokenId]], _tokenId);
        }
        _ticketOwner[_tokenId] = _user;
        EnumerableSet.add(myTickets[_user], _tokenId);
    }

    // return list of NFTs user can claim
    function getWhatCanUserClaim(address _user) external view returns(uint256[] memory nurseries,uint256[] memory laboratories, uint256[] memory trainings, uint256[] memory tickets ){
        return _getWhatCanUserClaim(_user);
    }

    function _getWhatCanUserClaim(address _user) internal view returns(uint256[] memory nurseries,uint256[] memory laboratories, uint256[] memory trainings, uint256[] memory tickets ){
        uint256[] memory _nurseries = new uint256[](EnumerableSet.length(myNurseries[_user]));
        uint256[] memory _laboratories = new uint256[](EnumerableSet.length(myLaboratories[_user]));
        uint256[] memory _trainings = new uint256[](EnumerableSet.length(myTrainings[_user]));
        uint256[] memory _tickets = new uint256[](EnumerableSet.length(myTickets[_user]));

        for(uint256 i = 0 ; i <  _nurseries.length;){
            _nurseries[i] = EnumerableSet.at(myNurseries[_user],i);
            unchecked {++i ;}
        }

        for(uint256 i = 0 ; i <  _laboratories.length;){
            _laboratories[i] = EnumerableSet.at(myLaboratories[_user],i);
            unchecked {++i ;}
        }

        for(uint256 i = 0 ; i <  _trainings.length;){
            _trainings[i] = EnumerableSet.at(myTrainings[_user],i);
            unchecked {++i ;}
        }

        for(uint256 i = 0 ; i <  _tickets.length;){
            _tickets[i] = EnumerableSet.at(myTickets[_user],i);
            unchecked {++i ;}
        }
    ...
    function claimAllNFTs() external {
        (uint256[] memory nurseries,uint256[] memory laboratories, uint256[] memory trainings, uint256[] memory tickets ) = _getWhatCanUserClaim(msg.sender);
        require(nurseries.length > 0 || laboratories.length > 0 || trainings.length > 0 || tickets.length > 0, "Nothing to claim");
        ...
        if(laboratories.length > 0){
            for(uint256 i = 0 ; i < laboratories.length ;){
                laboratory.safeTransferFrom(address(this), msg.sender, laboratories[i]);
                EnumerableSet.remove(myLaboratories[msg.sender], laboratories[i]);
                unchecked{ ++ i ; }
            }
        }

        if(trainings.length > 0){
            for(uint256 i = 0 ; i < trainings.length ;){
                trainingCenter.safeTransferFrom(address(this), msg.sender, trainings[i]);
                EnumerableSet.remove(myTrainings[msg.sender], trainings[i]);
                unchecked{ ++ i ; }
            }
        }
        ...
        if(tickets.length > 0){
            for(uint256 i = 0 ; i < tickets.length ;){
                ...
                EnumerableSet.remove(myTickets[msg.sender], tickets[i]);
                unchecked{ ++ i ; }
            }
        }
    }
