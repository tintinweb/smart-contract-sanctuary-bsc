...
contract ShillaVault is Ownable {
    ...
    //Update weights before deposit
    function updateLocksWeights(uint256[] calldata weights) external onlyOwner {
        require(totalDeposits == 0, "Can't update weights on deposits");
        require(weights.length == locksCount, "Weights out of locks index");
        uint256 totalWeights;
        for (uint256 index = 0; index < weights.length; index++) {
            lock[index + 1].weight = weights[index];
            totalWeights += weights[index];
        }
        require(totalWeights / lock[1].weightDivisor == locksCount, "The total weights must equal to the total locks");
    }
