...
contract Crsfans is Context, IBEP20, Ownable {
    ...
    constructor (address cOwner, address marketingWallet, address bountyWallet) Ownable(cOwner) {
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            (, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tBounty)  = _getTValues(tAmount);
            uint256 currentRate = _getRate();
            (uint256 rAmount,,) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tBounty, currentRate);
            ...
            (, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tBounty) = _getTValues(tAmount);
            uint256 currentRate = _getRate();
            (, uint256 rTransferAmount,) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tBounty, currentRate);
    ...
    function setTransferFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 bountyFee) external onlyOwner {
        _taxFeeTransfer       = taxFee;
        _liquidityFeeTransfer = liquidityFee;
        _marketingFeeTransfer = marketingFee;
        _bountyFeeTransfer    = bountyFee;
    }
    function setBuyFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 bountyFee) external onlyOwner {
        _taxFeeBuy       = taxFee;
        _liquidityFeeBuy = liquidityFee;
        _marketingFeeBuy = marketingFee;
        _bountyFeeBuy    = bountyFee;
    }
    function setSellFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 bountyFee) external onlyOwner {
        _taxFeeSell       = taxFee;
        _liquidityFeeSell = liquidityFee;
        _marketingFeeSell = marketingFee;
        _bountyFeeSell    = bountyFee;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 previousBountyFee    = _bountyFeeTransfer;
        ...
        if (!takeFee || isBuy || isSell) {
            ...
            _bountyFeeTransfer    = previousBountyFee;
        }
    }
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tBounty) = _getTValues(tAmount);
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tBounty,currentRate);
        ...
        takeTransactionFee(sender, address(_bountyWallet), tBounty, currentRate);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tFee       = tAmount.mul(_taxFeeTransfer).div(40000);
        uint256 tLiquidity = tAmount.mul(_liquidityFeeTransfer).div(10000);
        uint256 tMarketing = tAmount.mul(_marketingFeeTransfer).div(30000);
        uint256 tBounty    = tAmount.mul(_bountyFeeTransfer).div(20000);
        uint256 tTransferAmount = tAmount.sub(tFee);
        tTransferAmount = tTransferAmount.sub(tLiquidity);
        tTransferAmount = tTransferAmount.sub(tMarketing);
        tTransferAmount = tTransferAmount.sub(tBounty);
        return (tTransferAmount, tFee, tLiquidity, tMarketing, tBounty);
    }
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint tBounty, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        rTransferAmount = rTransferAmount.sub(rBounty);
