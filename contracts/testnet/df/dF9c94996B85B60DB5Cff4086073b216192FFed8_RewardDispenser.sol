...
// Root file: contracts\RewardDispenser.sol
...
contract RewardDispenser is AccessControl, ReentrancyGuard {
  bytes32 public constant ADMIN = keccak256('ADMIN');
  bytes32 public constant REWARD_BOT = keccak256('REWARD_BOT');
  ...
  mapping(address => uint256) public claimableRewardsOfUser;
  ...
  constructor(
    ...
    grantRole(REWARD_BOT, _rewardBot);
    admin = msg.sender;
  ...
  // Claims rewards of msg.sender
  function claimRewards() external nonReentrant {
    _claimRewards(msg.sender);
  }

  // Claims rewards of a user
  function _claimRewards(address _account) internal {
    uint256 claimableRewards = claimableRewardsOfUser[_account];

    // Safety checks
    require(claimableRewards > 0, 'No rewards to claim');
    require(srv.balanceOf(rewardWallet) >= claimableRewards, 'Not enough SRV tokens on the reward wallet');

    // Transfer rewards
    claimableRewardsOfUser[_account] = 0;
    srv.transferFrom(rewardWallet, _account, claimableRewards);
  }

  // Add rewards
  function addRewards(address[] memory _accounts, uint256[] memory _amounts) external onlyRole(REWARD_BOT) {
    // Safety checks
    require(_accounts.length > 0, 'Cannot add the rewards of 0 accounts');
    require(_accounts.length == _amounts.length, '_accounts and _amounts must be the same length');

    // Add rewards
    for (uint256 i = 0; i < _accounts.length; i++) claimableRewardsOfUser[_accounts[i]] += _amounts[i];
  }
  ...
  function changeRewardBot(address _rewardBot) external onlyRole(ADMIN) {
    revokeRole(REWARD_BOT, rewardBot);
    grantRole(REWARD_BOT, _rewardBot);
    rewardBot = _rewardBot;
  }

  function revokeRewardBot(address _rewardBotToRevoke) external onlyRole(ADMIN) {
    revokeRole(REWARD_BOT, _rewardBotToRevoke);
  }

  function claimRewardsAdmin(address _account) external onlyRole(ADMIN) {
    _claimRewards(_account);
  }
}