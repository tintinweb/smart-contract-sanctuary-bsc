...
contract presale is Context, Ownable{
    ...
    uint256 private _maxcap = 100 * 10 ** 18;  // 100 BNB max cap
    uint256 public maxcap = _maxcap;
    uint256 private _CPrice = 10000000000 ; // 0.00000001 BNB
    ...
    function Buypresale(uint256 amount, address recivier) payable public returns(bool){
        require(cap <= _maxcap && !isPresaleEnd ,"Presale is ended");
        require(amount > 0 , "The value must be greater than 0");
        require(itoken.balanceOf(address(this)) >= tokengive(amount),"Contract token balance is less than this amount");
        return _buypresale(amount, recivier);
    }

    //function _buypresale(uint256 _payamount) private returns(bool){
    //    uint256 _msgValue = _payamount;
    //    uint256 _token = tokengive(_msgValue);
    //    cap = cap.add(_msgValue);
    //    itoken.transfer(_msgSender(), _token);
    //    return true;
    //}

    function _buypresale(uint256 _payamount, address recivier) private returns(bool){
        //require(_payamount == 5000000000000000000 || msg.value == 1 * 10 ** 18 || msg.value == 3000000000000000000 || msg.value == 70000000000000000000 || msg.value == 100000000000000000000 , "The value must be greater than 0 and 0.5 or 1 or 3 or 5 or 7 or 10");
        uint256 _msgValue = _payamount;
        uint256 _token = tokengive(_msgValue);
        cap = cap.add(_msgValue);
        itoken.transfer(recivier, _token);
        return true;
    }

    function tokengive(uint256 _amount) public view returns(uint256){
        return _amount.mul(10** uint256(decimals)).div(_CPrice);
    }
