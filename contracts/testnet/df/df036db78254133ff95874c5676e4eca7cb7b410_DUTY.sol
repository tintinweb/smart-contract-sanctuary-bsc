...
contract DUTY is BEP20, Ownable {
	...
	address payable public bigETHPayDayPotFeeAddress = payable(0xeEceE0dC9870CcADB7633Ce94705a66e868B866b);
	address payable public marketingFeeAddress = payable(0x2a527B2B37c27f7fF0840043223120B858864DF3);
	address payable public P2ERewardPotFeeAddress = payable(0x36834D96d66e221E65352a8ff8b45db65e7a8dd3);
	address payable public buyBackBurnFeeAddress = payable(0x2CE1E2086531c0e9D85FA1f673E5B12125db8b35);
	address payable public stakingRewardFeeAddress = payable(0x5A2a0225aae9834ffDc1E3fC61dA89c42dC17f34);
	...
	bool public bigETHPayDayPotPartEnable = false;
	bool public P2ERewardPotPartEnable = false;
	bool public stakingRewardPartEnable = false;
	bool public marketingPartEnable = false;
	bool public buyBackBurnPartEnable = false;
	bool public liquidityPartEnable = false;
    ...
    constructor() BEP20("DOGE DUTY", "DUTY Latest") {
		...
		if (!swapping && canSwap && swapEnable && automatedMarketMakerPairs[to]) {
			swapping = true;
			 ...
			 if(tokenToSwap >= swapTokensAtAmount)
			 {
				...
				if(liquidityPart > 0 && liquidityPartEnable) 
				{
				...
				if(buyBackBurnPart > 0 && buyBackBurnPartEnable) 
				{
				...
				if(marketingPart > 0 && marketingPartEnable) 
				{
				...
				if(stakingRewardPart > 0 && stakingRewardPartEnable) 
				{
				...
				if(P2ERewardPotPart > 0 && P2ERewardPotPartEnable) 
				{
				...
				if(bigETHPayDayPotPart > 0 && bigETHPayDayPotPartEnable) 
				{
	...
	function swapBNBForBUSD(uint256 BNBAmount, address payable receiver) private {
        address[] memory path = new address[](2);
	...
	function swapBNBForETH(uint256 BNBAmount, address payable receiver) private {
        address[] memory path = new address[](2);
        path[0] = pancakeSwapV2Router.WETH();
        path[1] = ETH;
	...
	function setStatus(bool _bigETHPayDayPotPartEnable, bool _P2ERewardPotPartEnable, bool _stakingRewardPartEnable, bool _marketingPartEnable, bool _buyBackBurnPartEnable, bool _liquidityPartEnable) external onlyOwner {
		
		bigETHPayDayPotPartEnable = _bigETHPayDayPotPartEnable;
		P2ERewardPotPartEnable = _P2ERewardPotPartEnable;
		stakingRewardPartEnable = _stakingRewardPartEnable;
		marketingPartEnable = _marketingPartEnable;
		buyBackBurnPartEnable = _buyBackBurnPartEnable;
		liquidityPartEnable = _liquidityPartEnable;
	}
}