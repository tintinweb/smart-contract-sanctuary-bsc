...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function drawing(uint8[4] memory _externalRandomNumber) external onlyAdmin {
        require(!drawed(), "reset?");
        require(drawingPhase, "enter drawing phase first");
        
        winningNumbers[0]= _externalRandomNumber[0];

        
        winningNumbers[1]=_externalRandomNumber[1];

        
        winningNumbers[2]=_externalRandomNumber[2];

        
        winningNumbers[3] = _externalRandomNumber[3];
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external returns(uint256){
        ...
        userHistory[msg.sender][issueIndex].push(tokenId);
