...
interface IInitContract {
    ...
    function connectToOtherContracts(address contracts) external;
}

contract Marketplace is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, ISimpleMarketplaceNativeERC721, IInitContract {
    ...
    //slot ====================================
    CountersUpgradeable.Counter private lastListingId; 
    address public nft;
    ...
    function isItemRealListing(uint256 tokenId) public onlyTransferApproval(tokenId) view returns(bool) {
        return tokensListing[tokenId] > 0 ;
    }

    function fetchListInfo(uint256 tokenId) public onlyTransferApproval(tokenId) view returns(Listing memory) {
        return listings[tokensListing[tokenId]];
    }
    ...
    function connectToOtherContracts(address contracts) external override onlyRole(DEFAULT_ADMIN_ROLE){
        setNFTContract(contracts);
    }
    ...
    function buy(uint256 tokenId) external payable override{
        ...
        require(seller != msg.sender, "Marketplace: seller has the same address as buyer");
        IERC721 token = getToken();
        _list.isSold = true;
        clearStorage(tokenId);  
        token.transferFrom(address(seller), msg.sender, tokenId);

        payable(seller).transfer(msg.value*95/100);     //hold 5% 
        emit Sold(tokenId, seller, msg.sender, msg.value, address(0), block.timestamp);
    }
