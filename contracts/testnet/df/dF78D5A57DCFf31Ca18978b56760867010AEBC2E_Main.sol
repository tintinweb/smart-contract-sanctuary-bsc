...
contract Main {
    // lockAddress text NOT NULL, profileId text NOT NULL, userAddress text NOT NULL, cid text NOT NULL
    struct User {
        address lockAddress;
        string handle;
        address userAddress;
    ...
    function registerLock(
        ...
        uint price,
        uint time,
        string memory streamId // encrypted stream id
    ) public {
        ...
        require(bytes(streamId).length > 0, "streamId cannot be empty");
        Lock lock = new Lock(price, time, streamId);
        lock.transferOwnership(msg.sender);
        users[msg.sender] = User(address(lock), handle, msg.sender);
        userArray.push(users[msg.sender]);
    }
...
contract Lock is ERC721, Ownable {
    ...
    uint public time;
    string public streamId; // encrypted stream id

    constructor(uint priceArg, uint timeArg, string memory streamIdArg) ERC721("Lock", "LOCK") {
        price = priceArg;
        time = timeArg;
