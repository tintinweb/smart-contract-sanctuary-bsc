...
contract MTBZRevivalKit is AccessControl {
    ...
    uint256 public kitPurchaseLimit = 5;
    ...
    Counters.Counter public kitCounter;
    Counters.Counter public kitCounterCommon;
    Counters.Counter public kitCounterRare;
    Counters.Counter public kitCounterEpic;
    Counters.Counter public kitCounterLegendary;
    Counters.Counter public kitCounterDivine;
    Counters.Counter public kitCounterImmortal;

    //Purchase Common via BNB
    function purchaseKitCommon(uint256 count) external payable {
        require(isSaleCommonOpen, "Kit Sale Closed!");
        require(supply_common - count >= 0, "Insufficient supply!");
        require(msg.sender != address(0), "Zero address");
        require(msg.value == (price_common*count), "Not enough BNB");
        require((KIT_COUNT[msg.sender]+count)<= kitPurchaseLimit, "Limit Exceeded!");

        COMMON_KITS[msg.sender] = COMMON_KITS[msg.sender] + count;
        KIT_COUNT[msg.sender] = KIT_COUNT[msg.sender] + count;
        kitCounterCommon.increment();
    }
    //Purchase Rare via BNB
    function purchaseKitRare(uint256 count) external payable {
        require(isSaleRareOpen, "Kit Sale Closed!");
    ...
    function purchaseKitImmortal(uint256 count) external payable {
        ...
        KIT_COUNT[msg.sender] = KIT_COUNT[msg.sender] + count;     
        kitCounterImmortal.increment();
    }

    //Purchase via Credits
    function _purchaseKitCredits(address walletAddress, uint256 count, uint256 kitType) external onlyRole(MINTER_ROLE) {
        if(kitType == 1){
            require(isSaleCommonOpen, "Kit Sale Closed!");
            require(supply_common - count >= 0, "Insufficient supply!");
            require((KIT_COUNT[walletAddress]+count)<= kitPurchaseLimit, "Limit Exceeded!");
    ...
    function _setKitLimit(uint256 _limit) external onlyRole(DEFAULT_ADMIN_ROLE){
       kitPurchaseLimit = _limit;
    }
