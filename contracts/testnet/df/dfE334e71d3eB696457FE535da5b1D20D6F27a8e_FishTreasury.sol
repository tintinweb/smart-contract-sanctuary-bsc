    ...
    ReentrancyGuard
{
    ...
    event FixGun(address indexed user, string priceID, uint256 amount, uint256 tokenId);
    ...
    function mintNFT(
        ...
        // method to use after December 2019.
        if (_tokenAddress == address(0) && _amount == 0) {
            (bool sent, ) = payable(rewardWallet).call{value: msg.value}("");
            require(sent, "Failed to send Ether");  
        } else {
            tokenAddress.transferFrom(
                _receipient, 
        ...
        emit ClaimNFT(_receipient, _amount, _priceID, tokenId);
    }
    ...
    function fixGun(
        ...
        if (_tokenAddress == address(0) && _amount == 0) {
            (bool sent, ) = payable(treasuryWallet).call{value: msg.value}("");
            require(sent, "Failed to send Ether");  
            emit FixGun(_msgSender(), _priceID, msg.value, _tokenId);
        } else {
            tokenAddress.transferFrom(
                _msgSender(), 
                address(treasuryWallet), 
            ...
            emit FixGun(_msgSender(), _priceID, _amount, _tokenId);
        }
    }
