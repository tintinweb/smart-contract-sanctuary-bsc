...
contract NEVISBANKv2  is ERC20,Ownable  {
	...
	function invest(address referrer, uint8 plan ,uint256 _amount) external payable {
        ...
        if (user.referrer == address(0)) {
                    if (users[referrer].deposits.length > 0 && referrer != msg.sender) {
                        ...
                        uint256 amount = _amount.mul(SEED_DAYSs).div(PERCENTS_DIVIDER);
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private  {
    ...
    function setrefpercentage(uint256 Ref_percentage) public {
        SEED_DAYSs= Ref_percentage;     
    } 
