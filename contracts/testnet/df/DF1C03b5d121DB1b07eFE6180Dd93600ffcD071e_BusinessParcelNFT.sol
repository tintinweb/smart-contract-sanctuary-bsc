    ...
    ReentrancyGuardUpgradeable
{
    ...
    function mint(address _user, uint256 _x, uint256 _y, uint256 _lt) external onlyMunicipality returns (uint256) {
        uint256 parcelId = _getParcelId(_x, _y, _lt);
        _mintFor(parcelId, _user);
        return parcelId;
    }

    function updateTokenLocking(uint256 _tokenId, bool _locking) external onlyMunicipality{
