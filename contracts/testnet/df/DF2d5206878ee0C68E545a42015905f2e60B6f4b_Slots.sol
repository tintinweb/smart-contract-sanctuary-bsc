    ...
    ReentrancyGuardUpgradeable
{
        ...
        returns (address[10] memory)
    {
        ...
        uint256 firstWinnerIndex = randomizer.random(game.id, 10);
        ...
        for (uint8 i = 1; i < 10; i++) {
            counter++;
            uint256 index = randomizer.addressRandom(lastPlayer, counter, 10);
    ...
    function _payRewardsAndUpdateStats(
        ...
        // IStorage(storageAddress).recordSlotPayment(_bet, _bet + reward, winners, losers);
        emit GameFinished(games[_bet].id, _bet, reward, winners, losers);
    }
        ...
        if (referrersCount > 0) {
            ...
            for (uint256 i = 0; i < _players.length; i++) {
                if (referrers[_players[i]] != address(0)) {
                    ...
                    // IStorage(storageAddress).recordRefPayment(
