...
contract Game_Imperator is BEP20Token {
    ...
    ///////////////////  
        ...
        if (all_game[a].adr.length > 0)
        {
            ...
            emit EmperorLog(all_game[a].adr[all_game[a].count], all_game[a].price - (all_game[a].price - all_game[a].referal_gift),a+1,block.timestamp);

            all_game[a].progress[all_game[a].adr[all_game[a].count]] += 1;
            emit MyLog(all_game[a].adr[all_game[a].count],all_game[a].price - (all_game[a].price - all_game[a].referal_gift),a+1,block.timestamp);
