...
contract SpotHouse is
    ...
    SpotHouseStorage
{
    function initialize(address _spotFunding) public initializer {
        __ReentrancyGuard_init();
        __Ownable_init();
        __Pausable_init();
        spotFunding = ISpotFunding(_spotFunding);
        emit SpotHouseInitialized(_spotFunding, _msgSender());
    }
    ...
    function openLimitOrder(
        ...
        require(_quantity != 0, Errors.VL_INVALID_SIZE);
        require(!_spotManager.isExpired(), Errors.VL_EXPIRED);
        address _trader = _msgSender();

        if (
            (_pip > _spotManager.getCurrentPip() && _side == Side.BUY) ||
            (_pip < _spotManager.getCurrentPip() && _side == Side.SELL)
        ) {
            _openMarketOrder(_spotManager, _side, _quantity, _trader);
        } else {
            if (_side == Side.BUY) {
                _openBuyLimitOrder(_spotManager, _quantity, _pip, _trader);
            } else {
                _openSellLimitOrder(_spotManager, _quantity, _pip, _trader);
            }
        }
    }
    ...
    function cancelAllLimitOrder(ISpotManager _spotManager)
        public
        override
        whenNotPaused
        nonReentrant
    {
        address _trader = _msgSender();
        address _spotManagerAddress = address(_spotManager);
        LimitOrderPending[] memory listLimitOrderPending = getListOrderPending(
            _spotManager,
            _trader
        );

        require(listLimitOrderPending.length > 0, Errors.VL_NO_LIMIT_TO_CANCEL);

        uint256 refundQuote;
        uint256 refundBase;

        uint128[] memory _listPips = new uint128[](
            listLimitOrderPending.length
        );
        uint64[] memory _orderIds = new uint64[](listLimitOrderPending.length);

        for (uint64 i = 0; i < listLimitOrderPending.length; i++) {
            LimitOrderPending memory limitOrderPending = listLimitOrderPending[
                i
            ];

            if (limitOrderPending.quantity == 0) {
                continue;
            }

            _listPips[i] = limitOrderPending.pip;
            _orderIds[i] = limitOrderPending.orderId;

            (uint256 refundQuantity, uint256 partialFilled) = _spotManager
                .cancelLimitOrder(
                    limitOrderPending.pip,
                    limitOrderPending.orderId
                );

            if (limitOrderPending.isBuy) {
                refundQuote += _spotManager.calculatingQuoteAmount(
                    refundQuantity,
                    limitOrderPending.pip
                );
                if (partialFilled > 0) {
                    refundBase += partialFilled;
                }
            } else {
        ...
        delete limitOrders[_spotManagerAddress][_trader];

        _withdraw(
            _spotManagerAddress,
            _trader,
            SpotFundingStorage.Asset.Quote,
            refundQuote
        );
        _withdraw(
            _spotManagerAddress,
            _trader,
            SpotFundingStorage.Asset.Base,
            refundBase
        );
    ...
    function cancelLimitOrder(
        ...
        require(_order.baseAmount != 0 && _order.quoteAmount !=0, Errors.VL_NO_LIMIT_TO_CANCEL);

        (bool isFilled, , , ) = _spotManager.getPendingOrderDetail(
            _order.pip,
            _order.orderId
        );
        ...
        if (_order.isBuy) {
            uint256 quoteAmount = _spotManager.calculatingQuoteAmount(
                refundQuantity,
                _order.pip
            );
            _withdraw(
    ...
    function claimAsset(ISpotManager _spotManager)
        public
        override
        whenNotPaused
        nonReentrant
    {
        ...
        (uint256 quoteAmount, uint256 baseAmount) = canClaimAsset(
            _spotManager,
            _trader
        );
        require(
        ...
        _clearLimitOrder(_spotManagerAddress, _trader);
    ...
    function canClaimAsset(ISpotManager _spotManager, address _trader)
        public
        view
        override
        returns (uint256 quoteAmount, uint256 baseAmount)
    {
        ...
        console.log("length ", listLimitOrder.length);
        ...
        for (i; i < listLimitOrder.length; i++) {
            ...
            (bool isFilled, bool isBuy, , uint256 partialFilled) = _spotManager
            ...
            console.log("listLimitOrder[i].orderId ", listLimitOrder[i].orderId);
            console.log("partialFilled ", partialFilled);
            ...
            if (isBuy) {
                ...
                if (isFilled) {
                    // full filled
                    baseAmount += listLimitOrder[i].baseAmount;
                } else {
                    // partial filled
                ...
                if (isFilled) {
                    // full filled
                    quoteAmount += listLimitOrder[i].quoteAmount;
                } else {
                    quoteAmount += _spotManager.calculatingQuoteAmount(
                        partialFilled,
                        listLimitOrder[i].pip
                    );
                }
    ...
    function getListOrderPending(ISpotManager _spotManager, address _trader)
        public
        view
        override
        returns (LimitOrderPending[] memory)
    {
        address _spotManagerAddress = address(_spotManager);
        ...
        LimitOrderPending[]
            memory listPendingOrderData = new LimitOrderPending[](
        ...
        for (uint256 i = 0; i < listLimitOrder.length; i++) {
            ...
            ) = _spotManager.getPendingOrderDetail(
                    listLimitOrder[i].pip,
                    listLimitOrder[i].orderId
                );
            if (!isFilled) {
    ...
    function _openBuyLimitOrder(
        ...
        // deposit quote asset
        // with token have RFI we need deposit first
        ...
        (uint64 orderId, uint256 sizeOut, uint256 openNotional) = _spotManager
            .openLimitPosition(_pip, uint128(_quantity), true);

        if (quoteAmount != openNotional && _quantity != sizeOut) {
            limitOrders[address(_spotManager)][_trader].push(
                SpotLimitOrder.Data({
                    pip: _pip,
                    orderId: orderId,
                    isBuy: true,
                    quoteAmount: quoteAmount - openNotional,
                    baseAmount: _quantity - sizeOut,
    ...
    function _openSellLimitOrder(
        ...
        uint256 quoteAmount = _spotManager.calculatingQuoteAmount(
            uint128(amountTransferred),
            _pip
        );

        (uint64 orderId, uint256 sizeOut, uint256 openNotional) = _spotManager
            .openLimitPosition(_pip, uint128(amountTransferred), false);
        ...
        if (quoteAmount != openNotional && _quantity != sizeOut) {
            limitOrders[address(_spotManager)][_trader].push(
                SpotLimitOrder.Data({
                    ...
                    isBuy: false,
                    quoteAmount: quoteAmount - openNotional,
                    baseAmount: amountTransferred - sizeOut,
                    blockNumber: block.number
                })
            );
        }
    ...
    function _openMarketOrder(
        ...
        address spManagerAddr = address(_spotManager);
        uint256 sizeOut;
        uint256 openNational;
        if (_side == Side.BUY) {
            (sizeOut, openNational) = _spotManager.openMarketPosition(
                _quantity,
                true
            );
    ...
    function _clearLimitOrder(address _spotManagerAddress, address _trader)
        internal
    {
        if (limitOrders[_spotManagerAddress][_trader].length > 0) {
            SpotLimitOrder.Data[]
                memory subListLimitOrder = _clearAllFilledOrder(
                    ISpotManager(_spotManagerAddress),
                    limitOrders[_spotManagerAddress][_trader]
                );
            delete limitOrders[_spotManagerAddress][_trader];
            for (uint256 i = 0; i < subListLimitOrder.length; i++) {
                ...
                limitOrders[_spotManagerAddress][_trader].push(
                    subListLimitOrder[i]
                );
            }
        }
        ...
        for (uint256 i = 0; i < listLimitOrder.length; i++) {
            ...
            if (!isFilled) {
                ...
                if (partialFilled > 0) {
                    ...
                    subListLimitOrder[index].quoteAmount = _spotManager
                        .calculatingQuoteAmount(
                            size - partialFilled,
                            listLimitOrder[i].pip
                        );
                }
                _spotManager.updatePartialFilledOrder(
                    listLimitOrder[i].pip,
                    listLimitOrder[i].orderId
                );
                index++;
    ...
    function _deposit(
        ...
        amountTransferred = spotFunding.deposit(
            _spotManager,
            _trader,
            asset,
            amount
    ...
    function _withdraw(
        ...
        if (amount > 0) {
            spotFunding.withdraw(_spotManager, _trader, asset, amount);
        }
    }
    ...
    function deposit(
        ...
        spotFunding.deposit(_spotManager, _trader, asset, amount);
    }
}
...
import "./ISpotManager.sol";

interface ISpotHouse {
    event SpotHouseInitialized(address spotFunding, address owner);
    ...
    function claimAsset(ISpotManager _spotManager) external;

    function canClaimAsset(ISpotManager _spotManager, address _trader)
        external
        view
        returns (uint256 quoteAsset, uint256 baseAsset);

    function cancelAllLimitOrder(ISpotManager _spotManager) external;

    function getListOrderPending(ISpotManager _spotManager, address _trader)
        external
        view
        returns (SpotHouseStorage.LimitOrderPending[] memory);
}
...
contract SpotHouseStorage {
    ...
    ISpotFunding public spotFunding;
...
import "../spot-exchange/libraries/types/SpotManagerStorage.sol";
...
interface ISpotManager {
    ...
    function getLiquidityInPipRange(
        ...
        external
        view
        returns (SpotManagerStorage.LiquidityOfEachPip[] memory, uint128);
...
library TickPosition {
        ...
        internal
    {
        ...
        console.log("self.filledIndex start ", self.filledIndex);
        console.log("self.currentIndex start", self.currentIndex);
        console.log("partialFilled ", self.orderQueue[self.filledIndex].partialFilled);
        console.log("size ", self.orderQueue[self.filledIndex].size);
        ...
        unchecked {
            ...
            while (totalSize < amount) {
                ...
                if (self.orderQueue[index].size > self.orderQueue[index].partialFilled  )
...
//            }
            ...
            console.log("update parital, ", uint120(totalSize - amount));
            self.orderQueue[index].updatePartialFill(
                uint120(totalSize - amount)
            );

            console.log("partialFilled ", self.orderQueue[index].partialFilled);
        }
    }
        ...
        returns (uint256, uint256)
    {
        ...
        if (self.liquidity >= uint128(size - partialFilled)) {
            console.log("into if cancel tick psoiton");
            self.liquidity = self.liquidity - uint128(size - partialFilled);
        }

        console.log("filledIndex ", self.filledIndex);
