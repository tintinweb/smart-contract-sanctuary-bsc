...
contract Dechachron is BEP20, LockToken {
    ...
    uint256 public _totalSupply = 2_0_000_000 * (10**18);
    uint256 public swapTokensAtAmount = 2_000_000 * (10**18); //100 millions. 
    uint256 public _maxTxAmount = _totalSupply.mul(2).div(100);//2%

    uint256 public BUSDRewardsFee = 10;
    uint256 public liquidityFee = 2;
    uint256 public marketingFee = 3;
    uint256 public developerfundFee = 3;

    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(developerfundFee);

    address payable public _marketingWalletAddress = payable(0x675c82790C1B9910334f306408dd3b8B04c72F3A);
    address payable public _developerfundWalletAddress = payable(0x038AD631521D4130A7d016986EE6B73093AB850c);
    ...
    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        ...
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(developerfundFee);
    ...
    function setdeveloperfundFee(uint256 value) external onlyOwner {
        developerfundFee = value;
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(developerfundFee);
        require(totalFees<=30, "To High Fee");
    } 
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            uint256 swapableFees = totalFees.sub(BUSDRewardsFee); // marketingFee+developerfundFee+liquidityFee.
    ...
    function swapAndLiquify(uint256 tokens) private 
    {
       uint256 swapableFee = totalFees.sub(BUSDRewardsFee);
        ...
        uint256 bnbFordeveloperfund = newBalance.sub(bnbForLiquidity).sub(bnbForMarketing);
        _marketingWalletAddress.transfer(bnbForMarketing);
        _developerfundWalletAddress.transfer(bnbFordeveloperfund);
    }
