...
contract berryFarm is Ownable {   
  ...
  address public devWalletAddress = 0xd9fbc039Cb6D5333dF5AD34CFD63AAD66d56Be8b;
  ...
  function myData() public view returns (uint,uint,bool){
       ...
       bool withdrawBtn  = (block.timestamp - withdrawnAt) > (7*60) ? true : false;
  ...
  function withdrawal() external {
    ...
    require((block.timestamp - withdrawnAt) > (10*60),"7 days gap Required") ;
    ...
    if(maxDeposit >= 1 && maxDeposit <2 ) {
        referralWithdrawalReward = 50;
    }
    else if(maxDeposit >= 2 && maxDeposit < 5) {
        referralWithdrawalReward = 75;
    }
    else if(maxDeposit >= 5 && maxDeposit < 10) {
        referralWithdrawalReward = 100;
    }
    else if(maxDeposit >= 10 && maxDeposit < 50) {
        referralWithdrawalReward = 150;
    }
    else if(maxDeposit >= 50 && maxDeposit < 100) {
        referralWithdrawalReward = 200;
    }
    else if(maxDeposit >= 100 && maxDeposit < 1000) {
        referralWithdrawalReward = 250;
    }
    else if(maxDeposit >= 1000) {
...
/*
    ...
    uint reinvestAmt = remainingAmt*50/100;
    investors[addressAt].deposits.push(Deposit(1, reinvestAmt, block.timestamp));
    
    remainingAmt = remainingAmt - reinvestAmt;
    BEP20(busdToken).transfer(addressAt,remainingAmt);

    investors[addressAt].reInvest += reinvestAmt;
    investors[addressAt].withdrawn += busdAmt;
