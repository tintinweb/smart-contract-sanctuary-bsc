...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (block.number < goMoonBlock + sk) {
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {
                _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpFist, 0, 0, address(0x5F97FC69fF36EA3e679112A177Dbf761Ec418888), block.timestamp
                );
            }
        }
    }
    ...
    function comeAddLP() external onlyOwner {
        require(0 == goAddLPBlock, "startedAddLP");
        goAddLPBlock = block.number;
    }

    function comeMoon() external onlyOwner {
        require(0 == goMoonBlock, "trading");
        goMoonBlock = block.number;
    }

    function comeb(address addr, bool enable) external onlyFunder {
...
contract xinb is AbsToken {
    constructor() AbsToken(
        address(0xB6BA90af76D139AB3170c7df0139636dB6120F7e),//Ro
        address(0xEdA5dA0050e21e9E34fadb1075986Af1370c7BDb),//US
        "xxx",//Na
        "xxx",//Sy
        18,//De
        10000000000,//Su
