...
contract BVStake is OwnableUpgradeable, DivestorUpgradeable {
    ...
    function initialize(address[9] memory tokens_)
        public
        // address stakeToken_,
        ...
        initializer
    {
        ...
        meta.usdt = tokens_[4];
        meta.wbnb = tokens_[5];
        meta.bnbPair = IPancakePair(tokens_[6]);
        meta.stakePair = IPancakePair(tokens_[7]);
        meta.rewardPair = IPancakePair(tokens_[8]);

        meta.stakeToken = I20(tokens_[0]);
        meta.rewardToken = I20(tokens_[1]);

        meta.banker = tokens_[2];
        meta.wallet = tokens_[3];

        // meta.usdt = usdt_;
        // meta.wbnb = wbnb_;
        // meta.bnbPair = IPancakePair(bnbPiar);
        // meta.stakePair = IPancakePair(stakePair_);
        // meta.rewardPair = IPancakePair(rewardPair_);

        // meta.stakeToken = I20(stakeToken_);
        // meta.rewardToken = I20(rewardToken_);
        ...
        pool.endTime = block.timestamp + 15 days;
    ...
    function coutingApy() public view returns (uint256[3] memory apys) {
        ...
        for (uint256 i; i < 3; i++) {
            ...
            // apys[i] = (apys[i] * soltInfo[i].rate) / 10;
        }
    }
        ...
        )
    {
        ...
        // uint256[2][3] memory poolInfo;
        // uint256[2][3] memory uPoolInfo;
        for (uint256 i; i < 3; i++) {
            uint256 lockTime = soltInfo[i].lockTime;
            // uint256 poolStake = soltInfo[i].stakeAmount;
    ...
    function coutingDebt1() public view returns (uint256, uint256) {
        Pool memory pInfo = pool;
        if (pInfo.endDebted != 0) {
            return (pInfo.endDebted, block.timestamp);
        }

        uint256 tm = block.timestamp > pool.endTime ? pool.endTime : block.timestamp;
        uint256 newDebt = pInfo.tvl > 0 ? (pInfo.rate * (tm - pool.lastTime) * ACC) / pInfo.tvl + pInfo.debted : 0 + pInfo.debted;
        return (newDebt, tm);
    }
    ...
    function binarySearch(uint256 tm) public view returns (uint256) {
        uint256 left;
        uint256 right = lastIndex;
        if (lastIndex == 0 || tm >= secDebt[right].tm) {
            return right;
        }
        while (right - left > 1) {
            ...
            // console.log("l", l, "r", r);
            // console.log("mid", mid);
            if (tm == secDebt[mid].tm) {
                return mid;
            } else if (tm > secDebt[mid].tm) {
                left = mid;
            } else if (tm < secDebt[mid].tm) {
                right = mid;
            } else {
    ...
    function viewRewardWithSlot(address account_, uint256 slotId_) public view returns (uint256 reward, uint256 newDepted) {
        ...
        // console.log("slot:", slotId_);
        // console.log("TM:", uInfo.lastStakeTime, "EndTm:", endTime);
        newDepted = slotId_ == 0 || uInfo.lastStakeTime == 0 ? coutingDebt() : coutingDebtWithTm(endTime);
        // console.log("stakeAmount", uInfo.stakeAmount);
        // console.log("newDepted", newDepted, uInfo.debted);
        // console.log("newDepted", coutingDebt(), coutingDebtWithTm(endTime));
        ...
        // if (uInfo.debted > newDepted) {
        // 	reward = ((uInfo.debted - newDepted) * stakeValue) / (ACC * 1e10);
        // 	console.log(reward);
        // }
        reward = ((newDepted - uInfo.debted) * stakeValue) / (ACC * 1e10);
    }
    ...
    function stake(
        ...
        if (toClaim > 0) {
            // console.log("stake", slotId_, toClaim);
            // console.log("newDebt", newDebt, coutingDebt());
            // uint256 toClaim = 0;
            // (toClaim, newDebt) = viewRewardWithSlot(msg.sender, slotId_);
    ...
    function _takeInviterReward(uint256 totalReward_, uint256 refReward_) private {
        ...
        for (uint8 i; i < refLevel; i++) {
            ...
            // meta.rewardToken.transferFrom(meta.wallet, inviter, reward);
            allowReward += reward;
        }
    ...
    function _updateUserAndPool(
        ...
        // console.log(lastIndex);
        // console.log("tm");
        // console.log(secDebt[lastIndex].tm);
        // console.log("newDebted_");
        // console.log(newDebted_);
        // console.log("newDebted_", newDebted_, pool.debted);
        // console.log("updatePool", newDebted_, pool.debted);
        // console.log("coutinDeb", coutingDebt());
        if (pool.debted == 0 || newDebted_ > pool.debted) {
            // isAdd_ ? pool.tvl += stakeValue_ : pool.tvl -= stakeValue_;
    ...
    function claimReward() public check(0) {
        (uint256 stakeReward, uint256 refReward, , uint256[3] memory nedDepteds) = viewRewards(msg.sender);
        ...
        uint256 newDebt = coutingDebt();
        // console.log("claimReard:", totalReward, newDebt, block.timestamp);
        ...
        for (uint256 i; i < 3; i++) {
            // console.log("claimReard:", i, uInfo.slots[i].debted, nedDepteds[i]);
            if (nedDepteds[i] > 0) {
                uInfo.slots[i].debted = nedDepteds[i];
            }
        }
        uInfo.toClaim = 0;
