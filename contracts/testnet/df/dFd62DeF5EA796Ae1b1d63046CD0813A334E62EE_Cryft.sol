...
contract Cryft is IERC20MetadataUpgradeable, LPSwapSupportUpgradeable, AntiLPSniperUpgradeable {
    ...
    mapping (address => uint256) public _tOwned; // TODO
    ...
    // Allow this address to be set only once to prevent security issues in buyback liquify function.
    function setAndLockEscrowAddress(address _escrowAddress) external onlyOwner {
        require(buybackEscrowAddress == address(0), "Escrow address already initialized and cannot be modified.");
        buybackEscrowAddress = _escrowAddress;
        _isExcludedFromFee[_escrowAddress] = true;
        excludeFromReward(_escrowAddress, true);
    }
...
//            _rOwned[account] = tOwned.mul(_getRate()); // TODO
        }
        ...
        if(from != owner() && to != owner() && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if(isLPPoolAddress[from]){ // Buy
                if(!_isExcludedFromTxLimit[to])
                    require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
                (rAmount, tTransferAmount, rTransferAmount) = takeFees(from, amount, buyFees);
            } else if(isLPPoolAddress[to]){ // Sell
                (rAmount, tTransferAmount, rTransferAmount) = takeFees(from, amount, sellFees);
            } else {
                (rAmount, tTransferAmount, rTransferAmount) = takeFees(from, amount, transferFees);
            }
    ...
    function selectSwapEvent() private lockTheSwap {
        ...
        if(buybackAndLiquifyEnabled && (address(this).balance >= minSpendAmount.mul(2) || buybackTriggered)){
    ...
    function takeFees(address from, uint256 amount, Fees memory _fees) private returns(uint256 rAmount, uint256 tTransferAmount, uint256 rTransferAmount){
        ...
        if(amount != tTransferAmount){
            emit Transfer(from, address(this), amount.sub(tTransferAmount)); // TODO
        }

        tTransferAmount = tTransferAmount.sub(tFees.reflection).sub(tFees.burn);
...
abstract contract LPSwapSupportUpgradeable is OwnableUpgradeable {
    ...
    bool internal buybackTriggered;
...
//        if(tokenAddress != address(this)){
//            bool approved = IBEP20(tokenAddress).approve(address(pancakeRouter), amount);
//            if(!approved){
//                return;
//            }
//
...
//        }
    ...
    function buybackAndLiquify(uint256 amount) internal {
        buybackTriggered = !buybackTriggered;
        if(buybackTriggered){
            // Buyback tokens
            swapCurrencyForTokensUnchecked(address(this), amount.div(2), buybackEscrowAddress);
            ...
            addLiquidity(_buybackTokensPending, amount);

            emit BuybackAndLiquify(_buybackTokensPending, amount);
        }
    }
