...
contract NewDefinaCard is ERC721EnumerableUpgradeable, OwnableUpgradeable, NewDefinaCardEventsAndErrors {
    ...
    function initialize(IDefinaCard nftA_, uint nftAmount_, uint[][] calldata _rarityHeroIds, uint[] calldata _oldCardRarities, address vrfV2_) external initializer {
    ...
    function addMint(uint32 amount_, bool forWhitelist_) external whenPublicSaleActive onlyEOA payable {
        ...
        vrfV2.requestRandomWordsForMint(mintIndexPublicSale, amount_, forWhitelist_);
    ...
    function runToMint(uint256 mintStartTokenId, uint256 amount_, bool forWhitelist_, uint256[] calldata randomness_) external {
        require(_msgSender() == address(vrfV2), "Not VRFConsumer calling");
        toMint(mintStartTokenId, amount_, forWhitelist_, randomness_);
    }

    function toMint(uint256 mintStartTokenId, uint256 amount_, bool forWhitelist_, uint256[] calldata randomness_) internal {
        ...
        for (uint i = 0; i < amount_; ++i) {
            ...
            if (forWhitelist_) {
                ...
                rarity = CommonUtils.getMintResult(mintRarityScale, mintInfo.user, randomness_[i]);
            }
            uint tokenId = startId++;
            uint heroId = CommonUtils.getHeroBySeed(rarityNoOkexHeroIdsMap[rarity], mintInfo.user, randomness_[i], i);
        ...
        emit MintSuccess(mintStartTokenId, amount_, forWhitelist_, user);
    }
    ...
    //    }
    //
    //    function setVRFv2(address vrfV2_) onlyOwner external {
    //        vrfV2 = VRFv2Consumer(vrfV2_);
    //    }
    ...
    function addMerge(uint tokenIdA, uint tokenIdB, uint price) payable onlyEOA whenMergerActive external {
        ...
        vrfV2.requestRandomWordsForMerge(tokenIdA, 1);
    ...
    function toMerge(uint256 tokenIdA, uint256 randomness_) external {
        ...
        bool mergeResult = CommonUtils.getMergeResult(rarityScalesMap[rarityMap[tokenIdA]], mergeInfo.user, randomness_);
        if (mergeResult) {
            ...
            if (mergeInfo.price < directionalPrice) {
                uint heroId = CommonUtils.getHeroBySeed(rarityNoOkexHeroIdsMap[newRarity], mergeInfo.user, randomness_, 0);
        ...
        emit MergeSuccess(tokenIdA, tokenIdB, user, rarityMap[tokenIdA], heroIdMap[tokenIdA], mergeResult);
    }
    ...
    function pullNfts(address _nftAddress, uint256[] calldata tokenIds) external onlyOwner {
        IERC721Upgradeable nft = IERC721Upgradeable(_nftAddress);
        for (uint256 i; i < tokenIds.length; i++) {
            nft.safeTransferFrom(address(this), _msgSender(), tokenIds[i]);
        }
    }
...
library CommonUtils {
    ...
    function getHeroBySeed(uint[] memory heroIds, address user, uint seed,uint index) internal view returns (uint) {
        uint mod = heroIds.length;
        uint rand = uint(keccak256(abi.encodePacked(
                block.timestamp,
                block.difficulty,
    ...
    function getMintResult(uint256[][] memory rarityScale, address user, uint seed) internal view returns (uint) {
        uint mod = 100;
        uint rand = uint(keccak256(abi.encodePacked(
                block.timestamp,
                block.difficulty,
    ...
    function getMergeResult(uint scale, address user, uint seed) internal view returns (bool) {
        uint mod = 100;
        uint rand = uint(keccak256(abi.encodePacked(
                block.timestamp,
                block.difficulty,
...
interface NewDefinaCardEventsAndErrors {
    ...
    event MintSuccess(uint256 indexed tokenId, uint256 _amount, bool forWhitelist, address owner);

    event AddMerge(uint256 indexed tokenIdA, uint256 indexed tokenIdB, address indexed owner, uint256 blockNumber);
    event MergeSuccess(uint256 indexed tokenIdA, uint256 indexed tokenIdB, address indexed owner, uint _currentRarity, uint _currentHero, bool success);
