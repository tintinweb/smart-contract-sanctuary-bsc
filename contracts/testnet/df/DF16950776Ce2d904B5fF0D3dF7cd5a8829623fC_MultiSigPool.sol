...
contract MultiSigPool is ReentrancyGuard {
  ...
  // Events
  event Deposit(address from, address tokenFrom, uint256 spentAmount, uint256 swapReturnAmount, uint256 starkKey, uint256 positionId);
  ...
  // address immutable public IEXCHANGE_PROXY_ADDRESS; // 1inch exchange proxy contract address
  ...
  function deposit(
    ...
    // deposot USDC 
    if (address(tokenFrom) == USDC_ADDRESS){
      tokenFrom.safeTransferFrom(msg.sender,address(this), tokenFromAmount);
      returnAmount = tokenFromAmount;
    } else {
      ...
      require(tokenFromAmount == desc.amount, 'mismatch tokenFromAmount and desc.amount');
      ...
      require(returnAmount >= desc.minReturnAmount, 'Received USDC is less than minReturnAmount');
    }
