...
Tokenomics
Buy Fee Total 8%
wETH Rewards 4% | Liquidity 1% | Buyback & Burn 1% | Marketing & Development 1% | Affordable Housing 1%

Sell Fee Total 13%
wETH Rewards 8% | Liquidity 2% | Buyback & Burn 2% | Marketing & Development 1% | Affordable Housing 2%
*/
...
contract DividendDistributor {
    ...
    IERC20 wETHReward = IERC20(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47); // Mainnet
    address wETH = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // Testnet (Link represent pool)
    //IERC20 wETHReward = IERC20(0xB4FBF271143F4FBf7B91A5ded31805e42b2208d6); // Testnet
    //address wETH = 0x326C977E6efc84E512bB9C30f76E30c160eD06FB; // Testnet (Pool token)
    ...
    function deposit() external payable onlyToken {
        uint256 balanceBefore = wETHReward.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = wETH;
        path[1] = address(wETHReward);
        ...
        uint256 amount = wETHReward.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            wETHReward.transfer(shareholder, amount);
...
abstract contract Taxable is Owned, Tradable {
    ...
    //
    uint256 private _DevelopmentTokensCollected;
    ...
    Tradable(symbol, name, tokenDistribution) {
        ...
        _totalBuyTaxes = buyTaxes.DevFee + buyTaxes.rewardsFee + buyTaxes.marketingFee + buyTaxes.BuybackBurnFee + buyTaxes.liqFee;
        _totalSellTaxes = sellTaxes.DevFee + sellTaxes.rewardsFee + sellTaxes.marketingFee + sellTaxes.BuybackBurnFee + sellTaxes.liqFee;
        _maxFees = maxFees;
    ...
    function setDevelopmentAddress(address payable newDevelopmentAddress) external onlyOwner() {
        require(newDevelopmentAddress != _DevelopmentAddress);
    ...
    function setBuyFees(uint8 newDevelopmentBuyFee, uint8 newRewardsBuyFee, uint8 newMarketingBuyFee, uint8 newBuybackBurnBuyFee, uint8 newLiqBuyFee) external onlyOwner {
        uint8 newTotalBuyFees = newDevelopmentBuyFee + newRewardsBuyFee + newMarketingBuyFee + newBuybackBurnBuyFee + newLiqBuyFee;
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(newDevelopmentBuyFee <= _maxDevFee, "Cannot set Development fee higher than max");
        require(newTotalBuyFees <= _maxFees, "Cannot set total buy fees higher than max");

        _buyTaxes = Taxes({ DevFee: newDevelopmentBuyFee, rewardsFee: newRewardsBuyFee, marketingFee: newMarketingBuyFee,
            BuybackBurnFee: newBuybackBurnBuyFee, liqFee: newLiqBuyFee });
        _totalBuyTaxes = newTotalBuyFees;
    }

    function setSellFees(uint8 newDevelopmentSellFee, uint8 newRewardsSellFee, uint8 newMarketingSellFee, uint8 newBuybackBurnSellFee, uint8 newLiqSellFee) external onlyOwner {
        uint8 newTotalSellFees = newDevelopmentSellFee + newRewardsSellFee + newMarketingSellFee + newBuybackBurnSellFee + newLiqSellFee;
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(newDevelopmentSellFee <= _maxDevFee, "Cannot set Development fee higher than max");
        require(newTotalSellFees <= _maxFees, "Cannot set total sell fees higher than max");

        _sellTaxes = Taxes({ DevFee: newDevelopmentSellFee, rewardsFee: newRewardsSellFee, marketingFee: newMarketingSellFee,
    ...
    function calculateTotalFees(uint256 amount, uint8 txType) private returns (uint256) {
        uint256 DevelopmentTokens = (txType == BUYTX) ? amount.mul(_buyTaxes.DevFee).div(100) : amount.mul(_sellTaxes.DevFee).div(100);
        ...
        _DevelopmentTokensCollected = _DevelopmentTokensCollected.add(DevelopmentTokens);
        ...
        return DevelopmentTokens.add(rewardsTokens).add(marketingTokens).add(BuybackBurnTokens).add(liqTokens);
    }

    function swapCollectedFeesForFunding() private lockTheSwap {
        uint256 totalCollected = _DevelopmentTokensCollected.add(_marketingTokensCollected).add(_liqTokensCollected)
        ...
        uint256 totalAmountToSwap = _DevelopmentTokensCollected.add(_rewardsTokensCollected).add(_marketingTokensCollected)
        ...
        uint256 DevelopmentFunds = newFunds.sub(liqFunds).sub(marketingFunds).sub(rewardsFunds).sub(BuybackBurnFunds);

        addLiquidity(otherHalfLiq, liqFunds);
        (bool sent, bytes memory data) = _DevelopmentAddress.call{value: DevelopmentFunds}("");
...
contract PinkBananaToken is Context, Owned, Taxable {
	using SafeMath for uint256;
	using Address for address;

    string private _Bname = "Pink Banana Token";
    string private _Bsymbol = "PBT";
    // 18 Decimals
    ...
    // 5% Max Development Fee
    uint8 private _BmaxDevFee = 5;
    ...
    address payable _BDevelopmentAddress = payable(address(0xc9aA5eaaf07Ac9701105a423f3B89edec3aeb3A7));
    ...
    constructor ()
    Owned(_msgSender())
    Taxable(_Bsymbol, _Bname, _BtokenDistribution, _BDevelopmentAddress, _BmarketingAddress, _BBuybackBurnAddress,
            Taxes({ DevFee: 1, rewardsFee: 2, marketingFee: 32, BuybackBurnFee: 3, liqFee: 61 }),
            Taxes({ DevFee: 1, rewardsFee: 2, marketingFee: 32, BuybackBurnFee: 3, liqFee: 61 }),
