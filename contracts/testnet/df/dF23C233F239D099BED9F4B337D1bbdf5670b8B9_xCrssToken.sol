...
contract xCrssToken is ERC20Upgradeable, IXCrssToken, OwnableUpgradeable {
    ...
    // The CRSS TOKEN!
    address public crss;
    address public crssFarm;

    function initialize(address _crss) external initializer {
        __Ownable_init();
        _name = "XCrosswise Token";
        _symbol = "XCRSS";
        _decimals = 18;
        crss = _crss;
    }

    function setFarm(address _crssFarm) external override onlyOwner {
        crssFarm = _crssFarm;
    }

    // Safe crss transfer function, just in case if rounding error causes pool to not have enough CRSSs.
    function safeCrssTransfer(address _to, uint256 _amount) external override onlyOwner {
        uint256 crssBal = ICrssToken(crss).balanceOf(address(this));
        if (_amount > crssBal) {
            ICrssToken(crss).transfer(_to, crssBal);
        } else {
            ICrssToken(crss).transfer(_to, _amount);
        }
    }
...
interface ICrssToken is IERC20Upgradeable {
    enum DexSession { None, Swap, AddLiquidity, RemoveLiquidity, Transfer } 
