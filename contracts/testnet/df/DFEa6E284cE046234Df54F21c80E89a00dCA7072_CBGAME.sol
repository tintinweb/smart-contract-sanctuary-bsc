...
import "./interface/ICBGAME.sol";

contract CBGAME is AccessControlEnumerableUpgradeable,ICBGAME{

    function initialize() public initializer {
        SysConfig.beginTime = 1678963562;
        SysConfig.price1 = 100;
        SysConfig.price2 = 1000;
        SysConfig.cbPercent = 1000;
        SysConfig.maxRechargePrice = 1000;
    ...
    function recharge(uint256 types) public {
        require(block.timestamp < SysConfig.beginTime, "timeout");
        require(userRechargeTypes[_msgSender()] != 1 && userRechargeTypes[_msgSender()] != types, "black error");
        uint256 price = 0;
        userRechargeTypes[_msgSender()] = types;
        //充值usdt
        if(types == 2){
            price = SysConfig.price2;
        }else{
            price = SysConfig.price1;
        }
        IERC20(USDTAddress).transferFrom(_msgSender(), address(this), price);
        userRecharge[_msgSender()] += price;
        //充值记录
        RechargeStruct[] storage recharges = _userRecharge[_msgSender()];
        recharges.push(RechargeStruct(price,block.timestamp));
        //直推用户百分比10% todo
        _upgrade(_msgSender());
        _profit(_msgSender(),price);
    }
    /**
     * @dev Withdraw
     */
     ...
     * Profit
     */
    function _profit(address owner,uint256 price) internal returns (bool) {
        ...
        while(true){
            ...
            if(userSuper[parentAddress] == 2){
                //超级节点
            }else if(userSuper[parentAddress] == 1){
                //节点
            }else if(userRechargeTypes[parentAddress]>0){
                //购买过
                //直推一代奖励
                uint256 userPrice = SysConfig.zhiPercent * price / 10000;
                _addAsset(parentAddress, GOLD_NAME, userPrice,owner,1,1);
            }
            level++;
            teamRecharge[parentAddress] += price;
        }
        return true;
    }
    /**
    * 升级统计数据
     */
     function _upgrade(address owner) internal returns(bool){
        ...
        if(userZhiValid[parentAddress] % 10 == 0){
            getNFT().mint(parentAddress, 1);
        }
        //节点
        if(userSuper[parentAddress] == 0 && (userRecharge[parentAddress] >= SysConfig.maxRechargePrice || userStatusValid[parentAddress] ==1) 
        && userZhiValid[parentAddress]>=10){
            userSuper[parentAddress] = 1;
        }
        //超级节点
        if(userSuper[parentAddress] == 1 && userRecharge[parentAddress] >= SysConfig.maxRechargePrice){
            uint256 count = 0;
            address addr;
            for (uint256 i = 0; i < _userTeam[parentAddress].length; i++) {
                addr = _userTeam[parentAddress][i].owner;
                if(userRechargeTypes[addr] == 2){
                    count++;
                }
                if(count>=5){
                    userSuper[parentAddress] = 2;
                    continue;
                }
            }
        }
    ...
    * 设置参数
    ...
    function getUserCount(address owner) public view returns (uint256,uint256,uint256){
        return (_userTeam[owner].length,teamRecharge[owner],userZhiValid[owner]);
    }
    /**
     * Set Token Address
     */
     ...
     * Set Token Address
     */
    function setNFTAddress(address token) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "error");
        NFTAddress = token;
    }
    /**
     ...
     * Set Token Address
     */
    function setOtherAddress(address adminaddr,address regaddr) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "error");
    ...
    * 获取我的余额
     ...
     * @dev Get nft address
     */
    function getNFT() public override view returns (ICBNFT) {
        return ICBNFT(NFTAddress);
    }
     uint256[50] private __gap;
}
...
import "../structs/CBDATA.sol";
import "../interface/ICBNFT.sol";

abstract contract ICBGAME is CBDATA {
    ...
    mapping(uint256 => mapping(address=>uint256)) public userAddressLevel;
    ...
    mapping(address => uint256) public teamRecharge;

    mapping(address => uint256) public userRechargeTypes;

    mapping(address => uint256) public userSuper;
     ...
     * @dev Get nft address
     */
    function getNFT() public view virtual returns (ICBNFT);
...
abstract contract ICBNFT is IERC721,CBDATA
{
    /**
     * Get templates
     */
