...
contract MythDegen is ERC721 {
    ...
    mapping(uint256 => string) public defaultBackground;
    mapping(uint256 => string) public defaultEye;
    mapping(uint256 => string) public defaultMouth;
    mapping(uint256 => string) public defaultSkinColor;
    mapping(uint256 => string) public defaultNose;
    mapping(uint256 => string) public defaultHead;
    ...
    function mint(
        ...
        for (uint256 i = 0; i < 6; i++) {
            ...
            if (i == 0) {
                ...
                degenDefaults[tempCount].head = _defaults[i];
            }
        }
        tokenCount++;
    }
    ...
    function addCosmeticDefault(
        ...
        for (uint256 i = 0; i < _urls.length; i++) {
            require(_layerType[i] >= 0 && _layerType[i] <= 6, "");

            if (_layerType[i] == 0) {
                defaultBackground[_imageId[i]] = _urls[i];
            } else if (_layerType[i] == 4) {
                defaultSkinColor[_imageId[i]] = _urls[i];
            } else if (_layerType[i] == 3) {
                defaultNose[_imageId[i]] = _urls[i];
            } else if (_layerType[i] == 1) {
                defaultEye[_imageId[i]] = _urls[i];
            } else if (_layerType[i] == 2) {
                defaultMouth[_imageId[i]] = _urls[i];
            } else if (_layerType[i] == 6) {
                defaultHead[_imageId[i]] = _urls[i];
            }
            if (_layerType[i] != 5) {
                defaultExists[_layerType[i]][_imageId[i]] = true;
                emit cosmeticAdded(_layerType[i], _imageId[i], _urls[i]);
            }
        }
    }
    ...
    function getBackgroundURL(uint256 _id) public view returns (string memory) {
        if (degenCosmetics[_id].backgroundData == 0) {
            return defaultBackground[degenDefaults[_id].background];
        }
        return
    ...
    function getNoseURL(uint256 _id) public view returns (string memory) {
        if (degenCosmetics[_id].noseData == 0) {
            return defaultNose[degenDefaults[_id].nose];
        }
        return
            MythCosmetic(cosmeticAddress).getImageURL(
    ...
    function getHeadURL(uint256 _id) public view returns (string memory) {
        if (degenCosmetics[_id].headData == 0) {
            return defaultHead[degenDefaults[_id].head];
        }
        return
            MythCosmetic(cosmeticAddress).getImageURL(
    ...
    function getEyeURL(uint256 _id) public view returns (string memory) {
        if (degenCosmetics[_id].eyeData == 0) {
            return defaultEye[degenDefaults[_id].eyes];
        }
        return
            MythCosmetic(cosmeticAddress).getImageURL(
    ...
    function getMouthURL(uint256 _id) public view returns (string memory) {
        if (degenCosmetics[_id].mouthData == 0) {
            return defaultMouth[degenDefaults[_id].mouth];
        }
        return
            MythCosmetic(cosmeticAddress).getImageURL(
    ...
    function getSkinColorURL(uint256 _id) public view returns (string memory) {
        if (degenCosmetics[_id].skinColorData == 0) {
            return defaultSkinColor[degenDefaults[_id].bodyColor];
        }
        return
            MythCosmetic(cosmeticAddress).getImageURL(
