...
contract FastMoney is Ownable, Pausable, VerifySignature {
    ...
    function allowancePaymentToken(uint256 amount) public view returns(bool){
        return (paymentToken.allowance(msg.sender, address(this)) >= amount);
    }

    function approvePaymentToken(uint256 amount) public returns(bool){
    ...
    function transferProfitToBalance(
        ...
        console.log("Balance user before %s", users[userID].balance / 10**18);
        ...
        console.log("Balance user after %s", users[userID].balance / 10**18);
