...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    function _sendToLiquidity(uint256 tLiquidity, address sender) private {
        ...
        address _liquidAddr = address(this);
        if(!swapAndLiquifyEnabled){
            _liquidAddr = LiquidAddress;
        }
        _rOwned[_liquidAddr] = _rOwned[_liquidAddr].add(rLiquidity);
        _tOwned[_liquidAddr] = _tOwned[_liquidAddr].add(tLiquidity);
        emit Transfer(sender, _liquidAddr, tLiquidity);
    }
    ...
    function _sendToCompany(uint256 tCompany, address sender) private {
        ...
        _rOwned[_liquidAddr] = _rOwned[_liquidAddr].add(rCompany);
        _tOwned[_liquidAddr] = _tOwned[_liquidAddr].add(tCompany);
        emit Transfer(sender, _liquidAddr, tCompany);
    }
