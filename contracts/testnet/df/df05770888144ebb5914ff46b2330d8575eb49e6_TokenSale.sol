...
contract TokenSale is IBEP20, ReentrancyGuard, Context, Ownable {
	using Address for address;
	using SafeMath for uint256;

  
    string private constant _name         = "Token Sale";
    ...
    function StartPreSale(uint256 referRewards, uint256 _tkPerBnb, uint256 _IcoAmount) external onlyOwner {
        require(!PresaleEnabled, "Pre-sale started!");
		require(referRewards >= 0 && referRewards <= 100, "Value out of range: values between 0 and 100");
		require(_tkPerBnb > 0, "Sale price must be greater than zero");
		require(_IcoAmount > 0 && _IcoAmount <= balanceOf(owner()).div(10**_decimals), "Token amount must be greater than zero and less than or equal to balance of owner.");

		_Rewards         = referRewards;
		_Price           = _tkPerBnb;
		_Value           = 0;
		_transfer(owner(), address(this), _IcoAmount.mul(10**_decimals));
		PresaleEnabled = true;
    }

	function finishPreSale() external onlyOwner {
		require(PresaleEnabled, "Pre-sale ended!");
	...
	function Contribute(address _refer) external payable returns (bool success) {
		require(PresaleEnabled, "Pre-sale ended.");
        require(msg.value >= 10000000 gwei && msg.value <= 1000 ether, "Value out of range: values between 0.01 and 1000 BNBs");

        uint256 _tokens = _Price.mul(msg.value).div(1 ether).mul(10**_decimals);

		if (_msgSender() != _refer && balanceOf(_refer) != 0 && _refer != address(0)) {
			uint256 referBonus = _tokens.mul(_Rewards).div(10**2);
            require((_tokens.add(referBonus)) <= balanceOf(address(this)), "Insufficient tokens for this sale");
			_transfer(address(this), _refer, referBonus);
    ...
    function Salecount() external view returns (uint256 referRewards, uint256 _tkPerBnb, uint256 SaleCap, uint256 remainingTokens, uint256 SaleCount) {
		require(PresaleEnabled, "Pre-sale ended.");

		return (_Rewards, _Price, address(this).balance, balanceOf(address(this)), _Value);
    }
    ...
    function withdrawTokens(uint256 amountPercent) external onlyOwner {
		...
		uint256 _IcoAmount = balanceOf(address(this));

		if (_IcoAmount > 0) {
		    _transfer(address(this), owner(), _IcoAmount.mul(amountPercent).div(10**2));
		}
    }

    function withdrawBNB(uint256 amountPercent) external onlyOwner {
        require(amountPercent >= 0 && amountPercent <= 100, "values between 0 and 100");
