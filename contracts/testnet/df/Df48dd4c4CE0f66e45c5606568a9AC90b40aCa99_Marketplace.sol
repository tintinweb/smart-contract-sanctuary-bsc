...
contract Marketplace is ERC1155Holder , ReentrancyGuard
{
    ...
    // const immutable keywords needs to be added 
    //Constructor
    constructor()
    {
    ...
    // Events
    event listitemforfixeprice(uint256 tokenid , uint256 amount , uint256 price , uint256 royaltyfee , address royaltyrecepient);   
    event buyFixedprice(uint256 tokenid , uint256 amount , address buyer , uint256 platformfee , uint256 royaltyfee , uint256 amountPaytoSeller);
    event listitemforauction(uint256 tokenid , uint256 amount ,  uint256 initialprice , uint256 auctionStartTime , uint256 auctionEndTime , address royaltyreceip);
    ...
    function listItemForFixedPrice(uint256 _tokenId, uint256 _amount , uint256 _price , uint256 _royaltyFee, address _nftAddress , address _royaltyRec)public  OnlyTokenHolders(_tokenId , _nftAddress) returns(uint256)
    {
        ...
        require(_price > 0  , "Price should be greater than 10 ether");
        ...
        emit listitemforfixeprice(_tokenId,_amount,_price,_royaltyFee,_royaltyRec);
        return newItemId; 
    ...
    public  OnlyTokenHolders(tokenId , _nftAddress) returns(uint256)
    {
        ...
        emit listitemforauction(tokenId,_numberofcopies,_initialPrice,_biddingStartTime,_biddingendtime,_royaltyRecipient);
    ...
    { 
        ...
        require(msg.value ==  Fixedprices[fixedid].price,"send wrong amount in fixed price");
        ...
        IERC1155(Fixedprices[fixedid].nftAddress).safeTransferFrom(address(this),Fixedprices[fixedid].newowner,Fixedprices[fixedid].tokenid,Fixedprices[fixedid].totalcopies,'0x00');            
        ...
        payable(Fixedprices[fixedid].royaltyReciepient).transfer(RoyaltyFes);
        ...
        emit buyFixedprice(Fixedprices[fixedid].tokenid,Fixedprices[fixedid].totalcopies,Fixedprices[fixedid].newowner ,ServiceFee,RoyaltyFes,amountToSendSeller );
    ...
    function bid( uint256 _auctionId ) payable public 
    {
        ...
        require(msg.value >= auctions[_auctionId].initialPrice, "Bid price must be equal or higher than initial price");
    ...
    function auctionEnd(uint256 _auctionId) public 
    {
       ...
       if(auctions[_auctionId].isSold == true){
        ...
        {
            uint256 RoyaltyFes = calculateRoyaltyFee(auctions[_auctionId].currentBidAmount, auctions[_auctionId].royaltyFee);
            ...
            payable(MarketPlaceOwner).transfer(ServiceFee);
            payable(auctions[_auctionId].royaltyReciepient).transfer(RoyaltyFes);
            payable(auctions[_auctionId].nftOwner).transfer(amountToSendSeller);

            IERC1155(auctions[_auctionId].nftAddress).safeTransferFrom(address(this),msg.sender,auctions[_auctionId].tokenId,auctions[_auctionId].numberofcopies,'0x00');
    ...
    {
        require(msg.sender == auctions[_auctionId].currentBidOwner , "You are not the highest bidder");
        if(auctions[_auctionId].isSold == true){
            revert("Auction Ended by the Owner , and NFT Already tranfer to your wallet address");
        }
