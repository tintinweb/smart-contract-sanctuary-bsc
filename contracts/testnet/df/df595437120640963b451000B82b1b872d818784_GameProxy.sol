...
contract GameProxy is Ownable {
    /// -----------------------------------------------------------------------
    /// Errors
    /// -----------------------------------------------------------------------

    enum GameProxyErrorCodes {
        NOT_TIME_TO_SET
    }

    error GameProxyError(GameProxyErrorCodes code);

    /// -----------------------------------------------------------------------
    /// Events
    /// -----------------------------------------------------------------------

    /// @dev Emits when min bet amount has set.
    /// @param newMinBetAmount New min bet amount
    ...
    /// @dev Set the min bet amount.
    /// @dev Manager cannot change the value within an hour.
    /// @param _newMinBetAmount New min bet amount
    function setMinBetAmount(uint256 _newMinBetAmount) external onlyOwner {
        if (block.timestamp < lastChangedTime + 1 hours) {
            revert GameProxyError(GameProxyErrorCodes.NOT_TIME_TO_SET);
        }
