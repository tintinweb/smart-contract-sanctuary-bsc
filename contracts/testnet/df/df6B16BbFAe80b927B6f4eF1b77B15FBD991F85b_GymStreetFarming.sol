    ...
    RewardRateConfigurable
{
    ...
    function setWBNBAddress(address _address) external onlyOwner validAddress(_address) {
        ...
        rewardTokenToWbnb = [rewardToken, _address];
    }
    ...
    function depositFromOtherContract(
        ...
        uint256 _vbtcTokenAmount,
        address _from
    ) external tradingEnabled poolExists(_pid) onlyWhitelisted {
        ...
        IERC20Upgradeable(rewardToken).approve(routerAddress, _vbtcTokenAmount);
        ...
        liquidityData.vbtcTokensAdded = _vbtcTokenAmount;
    ...
    function _deposit(
        ...
        if (_vbtcTokenAmount > 0) {
            ...
            convertedVbtcInBase = _swapTokens(
                address(rewardToken),
                poolBaseToken,
                _vbtcTokenAmount,
                address(this),
        ...
        liquidityData.totalAddedInBaseToken = poolBaseTokenAmount + convertedVbtcInBase;
    ...
    function _addLiquidity(
        ...
        if (_basePoolToken == wbnbAddress) {
            ...
            IERC20Upgradeable(rewardToken).approve(routerAddress, vbtcToLpAmount);
    ...
    function _updateUserInfo(
        ...
        if (poolBaseToken == wbnbAddress) {
            dollarValue += (_getBnbPrice(liquidityData.baseTokensAdded) / 1e18);
        } else {
            dollarValue += liquidityData.baseTokensAdded / 1e18;
        }

        if (liquidityData.vbtcTokensAdded > 0) {
            dollarValue += (_getVbtcPrice(liquidityData.vbtcTokensAdded) / 1e18);
        }
    ...
    function _isSupportedLP(address pair) private view returns (bool) {
        address baseToken = _getPoolBaseTokenFromPair(IPancakePair(pair));
