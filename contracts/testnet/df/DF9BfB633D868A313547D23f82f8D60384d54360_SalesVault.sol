...
contract SalesVault is BasicAccessControl, Pausable, ReentrancyGuard {
    ...
    constructor (address payable liquidityVault) {
        ...
        address contractManager = ILiquidityVault(liquidityVault).contractManager();
    ...
    function _finalizeSales() internal {
        ...
        if (_tokenBalance > 0) { 
            IBaseToken(_baseToken).InvestorBurn(_tokenBalance);
        }
    ...
    function getInvestedValue (address investor) public view returns (uint256 tokensInvested) {
