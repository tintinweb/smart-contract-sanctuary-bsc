...
contract PhoenixTrust is Ownable, ReentrancyGuard{
    ...
    constructor(IERC20 _tokenContract, IERC20 _busdToken){
    ...
    function startLottery(uint256 _loteryEndTime) external onlyOwner returns(bool){
        currentLotteryId++;
    ...
    // public 
    function buyTicket() external nonReentrant  returns(bool){
        require(_token.balanceOf(msg.sender)>=ticketPriceInToken,"less bal.");
        Lottery storage s = _lottery[currentLotteryId];
        require(s.status == Status.Open,"Lottery closed.");
        players.push(msg.sender);
        _token.transferFrom(msg.sender,deadAddress, ticketPriceInToken);
        return true;
    }
