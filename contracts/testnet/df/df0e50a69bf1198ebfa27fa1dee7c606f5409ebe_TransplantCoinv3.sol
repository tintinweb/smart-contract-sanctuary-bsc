...
contract TransplantCoinv3 is ITransplantV3Coin, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "TransplantV3 Coin: Already upgraded");
        _;
        _upgraded = true;
    }
    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "TransplantV3 Coin: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __Transplant_v3_init_unchained(_router);
    }
    function __Transplant_v3_init_unchained(address _router) internal initializer {
        _name = "The Transplant Coin v3";
        _symbol = "TPC";
        _decimals = 9;
        ...
        __TransplantV3_tiers_init();
        emit Transfer(address(0), _msgSender(), _tTotal);
    }
    function __TransplantV3_tiers_init() internal initializer {
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(_account, "TransplantV3 Coin: Selected account is in blacklist")
    {
        require(_account != address(0), "TransplantV3 Coin: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner {
        require(_account != address(0), "TransplantV3 Coin: Invalid address");
        require(_accountsTier[_account] > 0, "TransplantV3 Coin: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "TransplantV3 Coin: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "TransplantV3 Coin: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner checkTierIndex(_tierIndex) {
        require(_owner != address(0), "TransplantV3 Coin: Address Zero is not allowed");
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "TransplantV3 Coin: Owner address is blacklisted")
        preventBlacklisted(spender, "TransplantV3 Coin: Spender address is blacklisted")
    {
    ...
    function _transfer(
        ...
        private
        preventBlacklisted(_msgSender(), "TransplantV3 Coin: Address is blacklisted")
        preventBlacklisted(from, "TransplantV3 Coin: From address is blacklisted")
        preventBlacklisted(to, "TransplantV3 Coin: To address is blacklisted")
        isRouter(_msgSender())
    {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        uint256 initialBalance = ITransplantV3Coin(address(this)).balanceOf(msg.sender);
        // swap BNB for Tokens
        swapBnbForTokens(half);

        // how much BNB did we just swap into?
        uint256 newBalance = ITransplantV3Coin(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        ITransplantV3Coin(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
    ...
    function migrate(address account, uint256 amount)
        external
        override
        preventBlacklisted(account, "TransplantV3 Coin: Migrated account is blacklisted")
    {
        require(migration != address(0), "TransplantV3 Coin: Migration is not started");
        require(_msgSender() == migration, "TransplantV3 Coin: Not Allowed");
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        ITransplantV3Coin(_token).transfer(msg.sender, _amount);
    }
