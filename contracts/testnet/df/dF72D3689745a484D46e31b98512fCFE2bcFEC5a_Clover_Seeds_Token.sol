...
contract Clover_Seeds_Token is IBEP20, Auth, Pausable {
   using SafeMath for uint256;
    ...
    // @Dev Sell tax..
    uint8 public _sellTeamFee = 2;
    uint8 public _sellLiquidityFee = 3;
    
    // @Dev Buy tax..
    uint8 public _buyTeamFee = 2;
    uint8 public _buyLiquidityFee = 1;

    uint8 public _TeamFeeWhenNoNFTs = 15;
    uint8 public _LiquidityFeeWhenNoNFTs = 20;
    uint8 public _MarketingFeeWhenNoNFTs = 15;
    ...
    uint256 public first_5_Block_Buy_Sell_Fee = 50;
    ...
    constructor (address _teamAddress, address _marketingAddress) Auth(msg.sender) {
        ...
        isTxLimitExempt[0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD && recipient != ROUTER) {
            require(isTxLimitExempt[recipient] || (_balances[recipient] + amount <= _maxWalletSize 
                    && amount <= getLiquiditySupply() * _liquidityThreshold / 100)
    ...
    function takeFeeOnBuy(address account, uint256 amount) internal returns (uint256) {
        ...
        //@dev Take team fee
        if(_buyTeamFee != 0) {
            uint256 teamFee = amount.mul(_buyTeamFee).div(100);
        ...
        if(_buyLiquidityFee != 0) {
            uint256 liquidityFee = amount.mul(_buyLiquidityFee).div(100);
    ...
    function collectFeeOnSell(address account, uint256 amount) private returns (uint256) {
        ...
        //@dev Take team fee
        if(_sellTeamFee != 0) {
            uint256 teamFee = amount.mul(_sellTeamFee).div(100);
            transferAmount = transferAmount.sub(teamFee);
            _balances[address(this)] = _balances[address(this)].add(teamFee);
            _teamFeeTotal = _teamFeeTotal.add(teamFee);
            teamFeeTotal = teamFeeTotal.add(teamFee);
        ...
        if(_sellLiquidityFee != 0) {
            uint256 liquidityFee = amount.mul(_sellLiquidityFee).div(100);
            transferAmount = transferAmount.sub(liquidityFee);
            _balances[address(this)] = _balances[address(this)].add(liquidityFee);
            _liquidityFeeTotal = _liquidityFeeTotal.add(liquidityFee);
            liquidityFeeTotal = liquidityFeeTotal.add(liquidityFee);
    ...
    function collectFee(address account, uint256 amount) internal returns (uint256) {
        ...
        uint256 Fee = amount.mul(first_5_Block_Buy_Sell_Fee).div(100);
    ...
    function collectFeeWhenNoNFTs(address account, uint256 amount) internal returns (uint256) {
        ...
        if(_TeamFeeWhenNoNFTs != 0) {
            uint256 teamFee = amount.mul(_TeamFeeWhenNoNFTs).div(100);
            transferAmount = transferAmount.sub(teamFee);
            _balances[address(this)] = _balances[address(this)].add(teamFee);
            _teamFeeTotal = _teamFeeTotal.add(teamFee);
            teamFeeTotal = teamFeeTotal.add(teamFee);
        ...
        if(_LiquidityFeeWhenNoNFTs != 0) {
            uint256 liquidityFee = amount.mul(_LiquidityFeeWhenNoNFTs).div(100);
            transferAmount = transferAmount.sub(liquidityFee);
            _balances[address(this)] = _balances[address(this)].add(liquidityFee);
            _liquidityFeeTotal = _liquidityFeeTotal.add(liquidityFee);
            liquidityFeeTotal = liquidityFeeTotal.add(liquidityFee);
        ...
        if(_MarketingFeeWhenNoNFTs != 0) {
            uint256 marketingFee = amount.mul(_MarketingFeeWhenNoNFTs).div(100);
    ...
    function test(uint256 amount) public view returns(bool) {
        return getLiquiditySupply() * _liquidityThreshold / 100 < amount;
    }
    ...
    function setFees(uint8 sellTeamFee_, uint8 sellLiquidityFee_, uint8 buyTeamFee_, uint8 buyLiquidityFee_, uint8 marketingFeeWhenNoNFTs_, uint8 teamFeeWhenNoNFTs_, uint8 liquidityFeeWhenNoNFTs_) public onlyOwner {
