...
contract DynamicERC20 {
    ...
    function _updateReward(address user) internal {
            ...
            userEarnedRewards[user] = pendingReward(user);
    ...
    function _burn(address from, uint value) internal returns (uint rewardAmount) {
        ...
        rewardAmount = pendingReward(from); // mint reward token to user

        userEarnedRewards[from] = 0;

        userRewardPerSharePaid[from] = accumulatedRewardPerShare;
...
contract DynamicVoting is DynamicERC20 {
    ...
    event BallotCreated(uint256 indexed ruleId, uint256 indexed ballotId, bytes args);
    ...
    */
    ...
    function createBallot(uint256 ruleId, bytes calldata args, uint256 voteLP) external {
        ...
        require(voteLP >= totalSupply * minimalLevel / 10000, "require minimal Level to suggest change");
        ...
        emit BallotCreated(ruleId, ballotIds, args);
...
contract DynamicPair is DynamicVoting {
    ...
    uint creationBlock; // protection from fork using flash loan
    ...
    function getFloorPrice(address baseToken) external view returns(uint floorPrice, address currentBaseToken) {
    ...
    function initialize(address _token0, address _token1, uint32[8] calldata _vars, bool _isPrivate, uint baseProtectedToken) external {
        ...
        name = string(abi.encodePacked(IERC20(_token0).symbol(), "-", IERC20(_token1).symbol(), " LP"));
    ...
    function _getFeeAndDumpProtection(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private returns(uint fee0, uint fee1){
        ...
        uint _baseLinePrice0 = block.timestamp/_vars[uint(Vars.timeFrame)]; // reuse variable

        if (_baseLinePrice0 != lastPeriodCounter) {   //new time frame 
...

        {
            ...
            if (_floorPrice0.baseToken == 0) {
                ...
                require(priceAfter0 >= priceBefore0 ||  // allow to buy token0
                ...
                require(priceAfter0 <= priceBefore0 ||  // allow to buy token1
        ...
        uint32 timeElapsed = uint32(block.timestamp % 2**32) - blockTimestampLast; // overflow is desired
...
*/
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        uint last = block.timestamp/vars[uint(Vars.timeFrame)];

        if (last != lastPeriodCounter) {   //new time frame 
    ...
    function burnLP(uint value) external {
    ...
    function burn(address to) public lock returns (uint amount0, uint amount1) {
        ...
        uint last = block.timestamp/_vars[uint(Vars.timeFrame)];
        ...
        _vars[uint(Vars.maxTxDump0)] = _vars[uint(Vars.maxTxDump0)] < _vars[uint(Vars.maxTxDump1)] ? _vars[uint(Vars.maxTxDump0)] : _vars[uint(Vars.maxTxDump1)];

        require(baseLineLP * 10000 / (_totalSupply - liquidity) >= (10000 - _vars[uint(Vars.maxTxDump0)]), "TX limit exceeded");

        // select lowest slippage (maxDump) per per day

        _vars[uint(Vars.maxDump0)] = _vars[uint(Vars.maxDump0)] < _vars[uint(Vars.maxDump1)] ? _vars[uint(Vars.maxDump0)] : _vars[uint(Vars.maxDump1)];

        require(baseLineLP * 10000 / (_totalSupply - liquidity) >= (10000 - _vars[uint(Vars.maxDump0)]), "Day limit exceeded");
    ...
    function switchToPublic() external onlyVoting {
        ...
        require(isPrivate == true, "Pool already public");
