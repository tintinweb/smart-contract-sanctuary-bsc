...
contract FaceWish is ERC721Enumerable, Ownable{
    ...
    uint256 public minPublicMintPrice = 100000000000000000;    
    uint256 public maxPublicMint = 4;


////////////////////////// Auction Variables    ///////////////////////////////////
    uint256 IncreamentRate = 0.001 ether;
    uint256 priceIncrementTime = 3 minutes;
    uint256 _timeSlot = 2;  // 190
    ...
    constructor(IERC20 _token, address creatorWallet, address _collectionWallet) ERC721("FaceWish", "FW")
    {
      Token = _token;
      setCollection();
    ...
    modifier publicMinter(uint256 _count){
      require(_count <= maxPublicMint, "count too high");
      require(!isStarted, "preSale should be closed!!");
      require(totalSupply().add(_count) <= maxsupply, "Max Public Limit Reached!!");
      User storage user = userInfo[msg.sender];
      require((user.totalPublicMint).add(_count) <= maxPublicMint, "limit reached!!");
      _;
      user.totalPublicMint = (user.totalPublicMint).add(_count);
    }

    function Mint(uint256 _noOfMale,uint256 _noOfFemaleMint ) public publicMinter(_noOfMale.add(_noOfFemaleMint) ) payable {
        uint256 count=_noOfMale.add(_noOfFemaleMint);
        require(msg.value >= minPublicMintPrice.mul(count), "Less Amount!!");
        if(_noOfMale>0){
        publicMaleMint(_noOfMale);
        }
        if(_noOfFemaleMint>0){
        publicFemaleMint(_noOfFemaleMint);
        }
       
    }

    function publicMaleMint(uint256 _count) private  {
      uint256 newMint = previousMaleMint.add(_count);
      for(uint256 i = previousMaleMint+1; i <=newMint; i++){
        _safeMint(msg.sender, i);
        // collection[1].availableTokens--;
      }
      collection[1].availableTokens = collection[1].availableTokens.sub(_count);
      previousMaleMint = newMint;
      User storage user = userInfo[msg.sender];
    ...
    function publicFemaleMint(uint256 _count) private {
      ...
      collection[2].availableTokens = collection[2].availableTokens.sub(_count);
      previousFemaleMint = newMint;
      User storage user = userInfo[msg.sender];
   ...
   function tokenURI(uint256 _tokenId) public view virtual override returns (string memory){
    ...
    function setPublicMintingLimit(uint256 _limit) public onlyOwner{
        maxPublicMint = _limit;
    }

    function setSlots(uint256 _slot) public onlyOwner{
        _timeSlot = _slot;
    }

    function setMintingPrice(uint256 _price) public onlyOwner{
        minPublicMintPrice = _price;
    }

    function startStopSale() public onlyOwner{
        if(!isStarted){
            isStarted = true;
        }
        else{
