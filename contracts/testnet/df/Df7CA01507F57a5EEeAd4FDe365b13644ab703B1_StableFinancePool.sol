    ...
    ReentrancyGuardUpgradeable
{
    ...
    event StableFinancePoolCreated(uint256 indexed poolId, uint256 APR);
    ...
    function stableFinanceAddPool(
        ...
        emit StableFinancePoolCreated(stableFinancePoolInfo.length - 1, _APR);
    }
    ...
    function stableFinanceSetPool(
        ...
        uint64 _APR,
        uint256 _endJoinTime
    ) external onlyOwner stableFinanceValidatePoolById(_poolId) {
        ...
        stableFinancePoolInfo[_poolId].cap = _cap;
        stableFinancePoolInfo[_poolId].minInvestment = _minInvestment;
        stableFinancePoolInfo[_poolId].maxInvestment = _maxInvestment;
        stableFinancePoolInfo[_poolId].APR = _APR;
        stableFinancePoolInfo[_poolId].endJoinTime = _endJoinTime;
    }
        ...
        stableFinanceValidatePoolById(_poolId)
    {
        ...
        emit StableFinanceDeposit(_poolId, account, _amount);
    }
     ...
     */
    function stableFinanceDepositSpecifyReceiver(
        uint256 _poolId,
        uint256 _amount,
        address _receiver
    ) external nonReentrant stableFinanceValidatePoolById(_poolId) {
        StableFinancePoolInfo storage pool = stableFinancePoolInfo[_poolId];

        _stableFinanceDeposit(_poolId, _amount, _receiver);
        ...
        emit StableFinanceDeposit(_poolId, _receiver, _amount);
    }
     ...
     */
    function stableFinanceWithdraw(uint256 _poolId, uint256 _amount)
        external
        nonReentrant
        stableFinanceValidatePoolById(_poolId)
    {
        ...
        require(
            ...
            "StableFinanceStakingPool: invalid withdraw amount"
        );
        ...
        if (stakingData.reward > 0) {
            ...
            emit StableFinanceRewardsHarvested(_poolId, account, reward);
        }

        stakingData.balance -= _amount;
        if (pool.delayDuration == 0) {
            pool.stableFinanceAcceptedToken.safeTransfer(account, _amount);
            emit StableFinanceWithdraw(_poolId, account, _amount);
            return;
        }
     ...
     * @param _poolId id of the pool
     */
    function stableFinanceClaimReward(uint256 _poolId)
        external
        nonReentrant
        stableFinanceValidatePoolById(_poolId)
    {
     ...
     */
    function stableFinancePendingReward(uint256 _poolId, address _account)
        public
        view
        stableFinanceValidatePoolById(_poolId)
     ...
     * @param _poolId id of the pool
     */
    function stableFinanceEmergencyWithdraw(uint256 _poolId)
        external
        nonReentrant
        stableFinanceValidatePoolById(_poolId)
    {
        require(
            ...
            "StableFinanceStakingPool: emergency withdrawal is not allowed yet"
        );
        ...
        require(
            ...
            "StableFinanceStakingPool: nothing to withdraw"
        );
        ...
        pool.stableFinanceAcceptedToken.safeTransfer(account, amount);
        emit StableFinanceEmergencyWithdraw(_poolId, account, amount);
    }
    ...
    function _stableFinanceHarvest(uint256 _poolId, address _account) private {
        ...
        stakingData.reward = stableFinancePendingReward(_poolId, _account);
        stakingData.updatedTime = block.timestamp;
    }
}
