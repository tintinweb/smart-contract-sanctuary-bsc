...
contract Contract is ERC1155, IERC2981, Ownable, IERC1155Receiver {
    ...
    mapping (string => uint256) private cellsCount;
    mapping (uint256 => bool) private isAuction;
    mapping (uint256 => address) private auctionWinner;
    mapping (uint256 => uint256) private wonBidAmount;
    mapping (uint256 => bool) private isApproved;
    mapping (uint256 => bool) private isRejected;
    mapping (string => uint256) private blockedCellsFee;
    mapping (uint256 => uint256) private tokenSupply;

    string public name;
    string public symbol;

    uint256 cellCost = 1000000000000000;

    event Sold(address oldOwner, address newOwner, uint256 _tokenId);
    event Withdraw(address receiver, uint256 _tokenId);
    event Enter(address sender, uint256 _tokenId);
    event NftApproved(uint256 _tokenId);
    event NftRejected(uint256 _tokenId);
    ...
    */
    ...
    function setCellCost(uint256 _cellCost) public onlyOwner
    {
        require(_cellCost > 0, "Provided cell cost must be greater than zero");
        cellCost = _cellCost;
    }

    function acceptBid(uint256 _tokenId, address winner, uint256 bidAmount) public
    {
        require(_msgSender() == owners[_tokenId], "You are not an owner of this token");
        require(winner != address(0), "Invalid winner address");
        require(bidAmount > 0, "Provided cell cost must be greater than zero");
    ...
    ) external returns (uint256)
    {
        ...
        require(0 <= royalty && royalty <= 5000, "Royalty cannot exceed 50%");
        ...
        _mint(address(this), _id, 1, "0x0");

        prices[_id] = price;
        royalties[_id] = royalty;
        isAuction[_id] = _isAuction;
    ...
    function payCellsFee(string calldata uri, uint256 _cellsCount) public payable
    {
        require(_cellsCount > 0, "Cell count must be greater than zero");

        uint256 cellsPrice = cellCost * _cellsCount;
        require(msg.value >= cellsPrice, "Insufficient ETH amount to create this token");

        cellsCount[uri] = _cellsCount;
        blockedCellsFee[uri] = cellsPrice;
    ...
    function approveNft(uint256 _tokenId) public onlyOwner {
        ...
        (bool sent, ) = payable(owner()).call{value: blockedCellsFee[uris[_tokenId]]}("");
    ...
    function rejectNft(uint256 _tokenId) public onlyOwner {
        ...
        (bool sent, ) = payable(creators[_tokenId]).call{value: blockedCellsFee[uris[_tokenId]]}("");
    ...
    function buyToken(uint256 _id) public payable
    {
        ...
        require(msg.value >= prices[_id], "Insufficient ETH amount to buy this token");
        ...
        emit Sold(owners[_id], msg.sender, _id);

        uint256 royaltyAmount = price * royalties[_id] / 10000;

        (bool sent, ) = payable(owners[_id]).call{value: price - royaltyAmount}("");
        require(sent, "Failed to send ETH to current owner");

        (sent, ) = payable(creators[_id]).call{value: royaltyAmount}("");
        require(sent, "Failed to send royalty amount to creator of art");
    ...
    function enterToken(uint256 _tokenId) public
    {
        require(_exists(_tokenId), "Token doesn't exist");
        require(balanceOf(_msgSender(), _tokenId) > 0, "You do not have this token");
        require(isApprovedForAll(_msgSender(), address(this)), "You must allow tokens transfer for this contract");
