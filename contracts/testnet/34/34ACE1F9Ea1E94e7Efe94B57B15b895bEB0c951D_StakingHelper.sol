...
contract StakingHelper {
    ...
    address public immutable WeFIN;

    constructor(address _staking, address _WeFIN) {
        require(_staking != address(0));
        staking = _staking;
        require(_WeFIN != address(0));
        WeFIN = _WeFIN;
    }

    function stake(uint256 _amount, address recipient) external {
        IERC20(WeFIN).transferFrom(msg.sender, address(this), _amount);
        IERC20(WeFIN).approve(staking, _amount);
