...
contract BAM is Ownable, ReentrancyGuard {
    ...
    function deductAntiWhaleTax(uint256 _amount) private returns (uint256) {
    ...
    function addReferralCommissionToReferee(
        ...
        if (_referee == owner()) {
            ...
            if (maxEligibleLevel >= _referralLevel) {
                ...
                uint256 amountToAdd = min(amountAfterFee, maxAmountToAdd);
                amountToAddBeforeFee = min(_amount, maxAmountToAdd);
    ...
    function updateUserEarnings(address _user) private {
        ...
        if (userDetails[_user].activeStake.lastUpdated < block.timestamp) {
            if (userDetails[_user].activeStake.isActive) {
                ...
                uint256 earningsAfterFee = (earnings * performanceFee.fee) /
                    percentageDivider;
    ...
    function updateAdminEarnings() private {
        ...
        if (lastUpdated < block.timestamp) {
            ...
            // uint256 daysPassed = timePassed / (1 days);
            uint256 earningsPerSecond = AdminEarnings.residualCommissionPerDay /
                1 days;
            uint256 amountToAdd = (earningsPerSecond * timePassed);
