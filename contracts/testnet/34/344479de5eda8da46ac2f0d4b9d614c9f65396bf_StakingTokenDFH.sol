...
contract StakingTokenDFH is Pausable,Ownable {
    ...
    struct Test {
        ...
        uint256 _oneTokenWithBNB;
        uint256 _oneBNBWithBusd;
        uint256 amountToken;
        uint256 amountBNB;
    }
    ...
    mapping(uint256 => Test) public infoTest;

    address private lpTokenContract = 0x79C223E55A7579A91D2C031029b9AD4cC0918d79;
    address private pancakeRouterV2Contract = 0xCc7aDc94F3D80127849D2b41b6439b7CF1eB4Ae0;
    ...
    uint256 private idTest = 0;
    constructor(ERC20 _token) {
        token = _token;
    }

    function _calcReturnedValue()
        public
    {
        address[] memory path;
        path  = new address[](2);
        ...
        uint256 _curPriceToken = _oneTokenWithBNB*_oneBNBWithBusd;

        uint256 amountToken =  IERC20(tokenContract).balanceOf(lpTokenContract);

        uint256 amountBNB =  IERC20(bnbContract).balanceOf(lpTokenContract);

        uint256 _totalValueofLP = (amountBNB * _oneBNBWithBusd) + (amountToken * _curPriceToken);
        uint256 _totalSupplyofLP = IPancakePair(lpTokenContract).totalSupply();

        infoTest[0] = Test(_totalValueofLP,_totalSupplyofLP,_curPriceToken,_oneTokenWithBNB,_oneBNBWithBusd,amountToken,amountBNB);
    }
