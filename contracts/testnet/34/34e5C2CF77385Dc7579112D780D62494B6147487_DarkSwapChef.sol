...
contract DarkSwapChef is OwnableUpgradeSafe, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DASs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accDasPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accDasPerShare` (and `lastRewardTime`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DASs to distribute per block.
        uint256 lastRewardTime; // Last timestamp that DASs distribution occurs.
        uint256 accDasPerShare; // Accumulated DASs per share, times 1e18. See below.
        bool isStarted; // if lastRewardTime has passed
        uint256 startTime;
    }

    address public das = address(0x4B1df511d59F5c73a420217cE58a77b462151c9E); // DAS
    address public locker = address(0x0000000000000000000000000000000000000000); // Not using locker this time
    ...
    // The block number when DAS mining starts.
    uint256 public startTime;

    address public rewardReferral;
    uint256 public commissionPercent;

    address public rewardMining;
    ...
    modifier checkHalving() {
        if (halvingChecked) {
            ...
            if (now >= nextHalvingTime) {
                massUpdatePools();
                rewardPerSecond = rewardPerSecond.mul(rewardHalvingRate).div(10000); // x98.5% (1.5% decreased weekly)
    ...
    function initialize(
        ...
        rewardHalvingRate = 9850; // 98.5%
        halvingChecked = true;

        // staking pool
        poolInfo.push(PoolInfo({lpToken: IERC20(_das), allocPoint: 0, lastRewardTime: _startTime, accDasPerShare: 0, isStarted: false, startTime: _startTime}));
    }
    ...
    function startReleaseTime() public view returns (uint256) {
        return (lockPercent == 0 || locker != address(0)) ? 0 : ITokenLockerV2(locker).startReleaseTime();
    }

    function endReleaseTime() external view returns (uint256) {
        return (lockPercent == 0 || locker != address(0)) ? 0 : ITokenLockerV2(locker).endReleaseTime();
    }
    ...
    function setRewardMining(address _rewardMining) external onlyOwner {
        rewardMining = _rewardMining;
    }
    ...
    function addPool(
        ...
        poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accDasPerShare: 0, isStarted: _isStarted, startTime: _lastRewardTime}));
        if (_isStarted) {
            totalAllocPoint = totalAllocPoint.add(_allocPoint);
        }
    }

    // Update the given pool's DAS allocation point. Can only be called by the owner.
    ...
    function setRewardPerSecond(uint256 _rewardPerSecond) external onlyOwner {
        require(_rewardPerSecond <= 100 ether, "insane high rate");
        massUpdatePools();
        rewardPerSecond = _rewardPerSecond;
    }
    ...
    // View function to see pending DASs on frontend.
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDasPerShare = pool.accDasPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (now > pool.lastRewardTime && lpSupply != 0) {
            uint256 _seconds = now.sub(pool.lastRewardTime);
            if (totalAllocPoint > 0) {
                uint256 _dasReward = _seconds.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
                accDasPerShare = accDasPerShare.add(_dasReward.mul(1e18).div(lpSupply));
            }
        }
        return user.amount.mul(accDasPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _seconds = now.sub(pool.lastRewardTime);
            uint256 _dasReward = _seconds.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            pool.accDasPerShare = pool.accDasPerShare.add(_dasReward.mul(1e18).div(lpSupply));
        }
        pool.lastRewardTime = now;
    }
    ...
    function _harvestReward(uint256 _pid, address _account) internal {
        ...
        if (user.amount > 0) {
            PoolInfo storage pool = poolInfo[_pid];
            uint256 _claimableAmount = user.amount.mul(pool.accDasPerShare).div(1e18).sub(user.rewardDebt);
            if (_claimableAmount > 0) {
                ...
                if (lockPercent > 0 && locker != address(0)) {
                    uint256 _startReleaseTime = startReleaseTime();
                    uint256 _userLastHarvestTime = userLastHarvestTime[_pid][_account];
                    if (now <= _startReleaseTime) {
                        ...
                        ITokenLockerV2(locker).lock(_account, _lockAmount);
                        ...
                        uint256 _afterReleaseSeconds = now.sub(_startReleaseTime);
                        uint256 _lockAmount = _claimableAmount.mul(_beforeReleaseSeconds).div(_beforeReleaseSeconds.add(_afterReleaseSeconds)).mul(lockPercent).div(10000);
                        _claimableAmount = _claimableAmount.sub(_lockAmount);
                        ITokenLockerV2(locker).lock(_account, _lockAmount);
                    }
                }

                _safeDasTransfer(_account, _claimableAmount);
    ...
    function _sendCommission(address _account, uint256 _commission) internal {
        ...
        if (_referrer != address(0)) {
            ...
            if (rewardMining != address(0)) {
                // send via Reward Mining contract
                IERC20(das).safeIncreaseAllowance(address(rewardMining), _commission);
                ISwapFeeReward(rewardMining).farm(_account, _referrer, _commission);
            } else {
                // send direct to referrer
                _safeDasTransfer(_referrer, _commission);
            }
            emit Commission(_account, _referrer, _commission);
        } else {
            // or burn
            _safeDasBurn(_commission);
    ...
    function depositWithRef(
        ...
        user.rewardDebt = user.amount.mul(pool.accDasPerShare).div(1e18);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public notContract nonReentrant checkHalving {
        ...
        if (_amount > 0) {
            ...
            if (reserveFund != address(0) && block.timestamp < unfrozenDepositTime(_pid, msg.sender)) {
    ...
    function harvestAndRestake() external {
        harvestAllRewards();
        uint256 _dasBal = IERC20(das).balanceOf(msg.sender);
        if (_dasBal > 0) {
            enterStaking(_dasBal);
        }
    }
    ...
    function _safeDasTransfer(address _to, uint256 _amount) internal {
        uint256 _dasBal = IERC20(das).balanceOf(address(this));
        if (_dasBal > 0) {
            if (_amount > _dasBal) {
                IERC20(das).safeTransfer(_to, _dasBal);
            } else {
                IERC20(das).safeTransfer(_to, _amount);
            }
        }
    }

    function _safeDasBurn(uint256 _amount) internal {
        uint256 _dasBal = IERC20(das).balanceOf(address(this));
        if (_dasBal > 0) {
            if (_amount > _dasBal) {
                IBurnableERC20(das).burn(_dasBal);
            } else {
                IBurnableERC20(das).burn(_amount);
            }
        }
    }
