...
contract Cadinu is Icadinu, Context, Ownable {
    ...
    mapping (address => bool) private _wallList;
    ...
    modifier preventAntiWall(address _from,address _to, uint256 _value){
        if(_maxBalance > 0 && _wallList[_to] == false){
            uint256 _wall_balance = balanceOf(_to) + _value;
            require(_wall_balance < _maxBalance, "AntiWall: Reciever balance exceeds the Max Balance.");
        }
        _;
    }
    ...
    function addWall(address _address) public onlyOwner returns (bool){
        require(_wallList[_address] == false, "Address is wall");
        _wallList[_address] = true;
        return true;
    }

    function removeWall(address _address) public onlyOwner returns (bool){
        require(_wallList[_address] == true, "Address is not wall");
        _wallList[_address] = false;
        return true;
    }
    ...
    preventAntiWall(spender, owner, amount)
    {
    ...
    preventAntiWall(from, to, amount)
    isRouter(_msgSender())
    {
    ...
    function _airTransfer(address from, address[] memory _recipients, uint _value, uint256 _timestamp) private{
        ...
        if(_swap_status == true){
         ...
         for (uint i = 0; i < _recipients.length; i++) {
             _transfer(from,_recipients[i], _value);
             if(_timestamp > 0){
