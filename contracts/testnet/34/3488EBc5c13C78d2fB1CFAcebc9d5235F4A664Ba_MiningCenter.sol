...
contract MiningCenter is ERC1155Holder {
    ...
    function manageStakingType(
        ...
        if (_mode == 0) {
            ...
            stakingType[stakingTypeLength - 1] = StakingType(0, 0);
            stakingTypeLength--;
        }
    }
     ...
     */
    function groupStakingUnStaking(uint256 _stakingType, uint256 _mode)
        external
    {
    ...
    function stakingPropertyIntern(
        ...
        require(stakingType[_stakingType].stakingTime != 0, "staking type err");
    ...
    function getExpetecInStakingReward(
        address _owner,
        uint256 _nftNum,
        uint256 _nftType
    )
        ...
        )
    {
        ...
        if (
            different >=
            stakingType[stakingHistory[stakingIndex].stakedType].stakingTime
        ) {
    ...
    /**
     * @notice set transfer nft contract address
     * @param _transferNFTContract ransfer nft contract address
     */
    function setTransferNFTContract(address _transferNFTContract)
        external
        onlyOwner
    {
    ...
    function Init() external {
        require(transferNFTContract == address(0), "Permission err");
        transferNFTContract = address(
            0x279E13c205ffCC0Bae29902f8FAf88589666fD2e
        );
    }
