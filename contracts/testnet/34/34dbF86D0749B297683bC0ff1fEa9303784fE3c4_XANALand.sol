...
abstract contract Storage {
    ...
    modifier checkLockUp(uint256 tokenId){
...
contract XANALand is ProxyBaseStorage, ERC721AQueryable, Ownable, ReentrancyGuard, Storage {
 ...
 using Address for address;
    ...
    //Discounted Mint Function
     function mintDiscountCommon(uint256 _mintAmount,  bytes32[] calldata proof) checkSupply(_mintAmount,4, "Common") external payable nonReentrant{
        string memory rarity = "Common";
        uint256 _size=1;
         //Check user buy limit merkle tree
        require(checkWhiteList(_mintAmount, proof, 4, "Common", true), "XanaLand: Not whitelisted");
    ...
    function checkWhiteList( uint256 _mintAmount, bytes32[] calldata proof, uint256 whitelistType, string memory rarity, bool isLimit) nonReentrant internal returns (bool){
        ...
        require(whitelistRoot[whitelistType].supply > whitelistRoot[whitelistType].sold + _mintAmount, "Xanaland: sale ended");
        uint256 userBuyCount = isLimit ?  _userBought[whitelistType][msg.sender] + _mintAmount : 0;

        return isWhitelisted(msg.sender, proof,  userBuyCount, whitelistType,rarity);
    }
    ...
    function mintLand(string memory rarity,uint256 _mintAmount, uint256 _size, bytes32[] calldata proof, bool isLimit, uint256 whitelistType) checkSupply(_mintAmount,_size, rarity) external payable {
        ...
        require(checkWhiteList(_mintAmount, proof,  whitelistType,  rarity,  isLimit), "XanaLand: Not whitelisted");
    ...
    function freeMint(uint256 _mintAmount,  bytes32[] calldata proof) checkSupply(_mintAmount, 5, "Common") external{
        ...
        require(checkWhiteList(_mintAmount, proof, 5, "Common", true), "XanaLand: Not whitelisted");
    ...
    function isWhitelisted(address account, bytes32[] calldata proof, uint256 quantity, uint256 typeWhitelist, string memory rarity) public view returns (bool) {
        return _verify(_leaf(account, quantity, rarity), proof, whitelistRoot[typeWhitelist].root);
    }
    function _leaf(address account, uint256 quantity, string memory rarity) public pure returns (bytes32) {
        return keccak256(abi.encode(account, quantity, rarity));
    }
    ...
    //only owner methods
    //set whitelist
    function setWhitelistRoot(bytes32 newWhitelistroot, uint256 typeWhitelist, uint256 startTime, uint256 endTime, uint256 supply) public onlyOwner {
    ...
    function setLockUp(uint256 _nftId,uint256 _timestamp) public onlyOwner{
        _nftLockup[_nftId] = _timestamp;
    }
    ...
    function mintAdmin(address to_, uint256 countNFTs_, uint256 _size, string memory rarity) public onlyOwner {
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) checkLockUp(tokenId) public virtual  {
