...
contract VslVote is Transfers
{
    ...
    constructor() payable
    {
      ...
      _endingTime = _startingTime + 2 days;
    }
    ...
    // STATUS BLOCK
    //
    function GetVotingResults() view external returns(string memory myVote, uint256 OptionAVotes, uint256 OptionBVotes, uint256 percOptionA, uint256 percOptionB)
    {
        OptionAVotes = _totalOptionAVotes;
        OptionBVotes = _totalOptionBVotes;

        uint256 totalVotes = GetTotalVotesReceived();
        percOptionA = (totalVotes == 0) ? 0 : _totalOptionAVotes * 100 / totalVotes;
        percOptionB = (totalVotes == 0) ? 0 : _totalOptionBVotes * 100 / totalVotes;

        address _wallet = _msgSender();
        VoteState _vote = voters[idToVoter[_wallet]].vote;

        if (_vote == VoteState.OptionAVote) myVote = "Option A";
        if (_vote == VoteState.OptionBVote) myVote = "Option B";
        myVote = "Did not Vote";
    }
    ...
    // function GetMyVote() view external returns(string memory vote)
    // {
    //     address _wallet = _msgSender();
    //     VoteState _vote = voters[idToVoter[_wallet]].vote;

    //     if (_vote == VoteState.OptionAVote) return "Option A";
    //     if (_vote == VoteState.OptionBVote) return "Option B";
    //     return "Did not Vote";
    // }

    // May change vote as many times as desired during voting period
