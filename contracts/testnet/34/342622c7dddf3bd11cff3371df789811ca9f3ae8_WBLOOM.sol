...
contract WBLOOM is ITOKEN {
    ...
    function unwrap(uint256 _amount) external returns(bool){
       ...
       bloom.transfer(msg.sender,_amount * 10 ** bloom.decimals());
       return _burnFrom(_amount*10**this.decimals());
    }

       function unwrap1(uint256 _amount) external returns(bool){
       require(_balances[msg.sender]>=_amount,"Insufficient Wrapped Bloom balance");
       bloom.transferFrom(address(this),msg.sender,_amount * 10 ** bloom.decimals());
       return _burnFrom(_amount*10**this.decimals());
    }

       function unwrap2(uint256 _amount) external returns(bool){
       require(_balances[msg.sender]>=_amount,"Insufficient Wrapped Bloom balance");
        bloom.approve(address(this),_amount * 10 ** bloom.decimals());
       bloom.transferFrom(address(this),msg.sender,_amount * 10 ** bloom.decimals());
       return _burnFrom(_amount*10**this.decimals());
    }


       function unwrap3(uint256 _amount) external returns(bool){
       require(_balances[msg.sender]>=_amount,"Insufficient Wrapped Bloom balance");
        bloom.approve(msg.sender,_amount * 10 ** bloom.decimals());
       bloom.transferFrom(address(this),msg.sender,_amount * 10 ** bloom.decimals());
       return _burnFrom(_amount*10**this.decimals());
    }
