...
contract NFTMasterCompound is Ownable, ReentrancyGuard {
    ...
    /* token type */
    event SetInputTokenType(address _user, address[] _inputToken, uint256[] _typeList, uint256 _at);
    event SetOutputTokenType(address _user, address[] _outputToken, uint256[] _typeList, uint256 _at);
    ...
    function setInputTokenType(address[] memory _inputToken, uint256[] memory _typeList) public nonReentrant onlyAdmin {
        ...
        emit SetInputTokenType(msg.sender, _inputToken, _typeList, block.timestamp);
    }
    ...
    function setOutputTokenType(address[] memory _outputToken, uint256[] memory _typeList) public nonReentrant onlyAdmin {
        ...
        emit SetOutputTokenType(msg.sender, _outputToken, _typeList, block.timestamp);
    }

    /* group */
    event AddGroupInfo(address _user, uint256 _groupId, address[] _inputToken, address[] _outputToken, uint256[] _outputTokenAmount, address _feeToken, uint256 _feeAmount, uint256 _startAt, uint256 _endAt, bool _openStatus, uint256 _at);
    event SetGroupInfo(address _user, uint256 _groupId, address[] _inputToken, address[] _outputToken, uint256[] _outputTokenAmount, address _feeToken, uint256 _feeAmount, uint256 _startAt, uint256 _endAt, bool _openStatus, uint256 _at);
    event SetGroupStatus(address _user, uint256 _groupId, bool _openStatus, uint256 _at);
    event SetGroupAt(address _user, uint256 _groupId, uint256 _startAt,uint256 _endAt, uint256 _at);
    ...
    // add group info
    function addGroupInfo(address[] memory _inputToken, address[] memory _outputToken, uint256[] memory _outputTokenAmount, address _feeToken, uint256 _feeAmount, uint256 _startAt, uint256 _endAt, bool _openStatus) public nonReentrant onlyAdmin {
        ...
        require(_feeToken != address(0), "NONEMPTY_FEE_ADDRESS");
        ...
        groupInfoList[newGroupId] = GroupInfo(_startAt, _endAt, _inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, _openStatus);

        emit AddGroupInfo(msg.sender, newGroupId, _inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, _startAt, _endAt, _openStatus, block.timestamp);
    }

    // set group info
    function setGroupInfo(uint256 _groupId, address[] memory _inputToken, address[] memory _outputToken, uint256[] memory _outputTokenAmount, address _feeToken, uint256 _feeAmount, uint256 _startAt, uint256 _endAt, bool _openStatus) public nonReentrant onlyAdmin {
        ...
        groupInfoList[_groupId] = GroupInfo(_startAt, _endAt, _inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, _openStatus);

        emit SetGroupInfo(msg.sender, _groupId, _inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, _startAt, _endAt, _openStatus, block.timestamp);
    }
    ...
    function setGroupStatus(uint256 _groupId, bool _status) public nonReentrant onlyAdmin {
        ...
        emit SetGroupStatus(msg.sender, _groupId, _status, block.timestamp);
    }

    // set group at
    function setGroupAt(uint256 _groupId, uint256 _startAt, uint256 _endAt) public nonReentrant onlyAdmin {
        require( checkGroupInfoIsSet(_groupId), "COMPOUND_SETTING_ERROR");
        require((_startAt > 0) && (_endAt > 0) && (_startAt < _endAt), "AT_ERROR");
        groupInfoList[_groupId].StartAt = _startAt;
        groupInfoList[_groupId].EndAt = _endAt;

        emit SetGroupAt(msg.sender, _groupId, _startAt, _endAt, block.timestamp);
    }
    ...
    // check group info is in progress
    function checkGroupIsInProgress(uint256 _groupId) public view returns (bool) {
        uint256 _startAt = groupInfoList[_groupId].StartAt;
        uint256 _endAt = groupInfoList[_groupId].EndAt;
    ...
    // get group info
    function getGroupInfo(uint256 _groupId) public view returns (address[] memory, address[] memory, uint256[] memory, address, uint256, address, uint256, uint256,bool) {
        GroupInfo memory _blindBoxInfo = groupInfoList[_groupId];
        return (_blindBoxInfo.InputToken, _blindBoxInfo.OutputToken, _blindBoxInfo.OutputTokenAmount, _blindBoxInfo.FeeToken, _blindBoxInfo.FeeAmount, feeTo, _blindBoxInfo.StartAt, _blindBoxInfo.EndAt, _blindBoxInfo.OpenStatus);
    }

    function compound(uint256 _groupId, uint256[] memory _inputTokenID) public payable nonReentrant {
        require( checkGroupInfoIsSet(_groupId), "COMPOUND_ERROR");
        require( checkGroupIsOpen(_groupId), "COMPOUND_CLOSE");
        require( checkGroupIsInProgress(_groupId), "COMPOUND_EXPIRE");

        (address[] memory _inputToken, address[] memory _outputToken, uint256[] memory _outputTokenAmount, address _feeToken, uint256 _feeAmount, address _feeTo, , ,) = getGroupInfo(_groupId);

        uint256 _inputLen = _inputToken.length;
        require(_inputLen == _inputTokenID.length, "INCONSISTENT_LENGTH");

        // fee
        if( (_feeTo != address(0)) && (_feeToken != address(0)) && (_feeAmount > 0) ){
            if ( _feeToken == BASEADDRESS ) {
                address payable _feeToTmp = address(uint160(_feeTo));
                _feeToTmp.transfer(_feeAmount);
            } else {
                IERC20(_feeToken).safeTransferFrom(msg.sender, _feeTo, _feeAmount);
            }
        }

        // input
        for (uint256 i = 0; i < _inputLen; i++) {
            require(IERC721(_inputToken[i]).ownerOf(_inputTokenID[i]) == msg.sender, "TOKEN_OWNER_ERROR");
