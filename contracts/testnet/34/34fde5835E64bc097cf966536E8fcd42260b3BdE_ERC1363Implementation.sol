...
// File: contracts/IDEX/IPinkAntiBot.sol
...
// File: contracts/IDEX/IDEXFactory.sol
...
// File: contracts/IDEX/IDEXPair.sol
...
// File: contracts/IDEX/IDEXRouter.sol
...
contract BEP20 is Context, IBEP20{
    ...
    function _initialize(string memory name_, string memory symbol_, uint8 decimals_, uint256 amount, bool mintable_, address owner_) internal virtual {
        _owner = owner_;
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
 ...
 */
abstract contract ERC1363 is BEP20, IERC1363, ERC165 {
...
// File: contracts/ERC1363ImpLiquidity.sol
...
contract ERC1363Implementation is 
    Context, 
    ERC1363, 
    Initializable {
    ...
    uint256 private _prevTaxFee; // previous fee for each transaction

    uint256 public _liquidityFee; // fee for each liquidity
    uint256 private _prevLiquidityFee; // previous fee for each liquidity

    uint256 public _marketingFee; // fee for each transaction
    uint256 private _prevMarketingFee; // previous fee for each transaction
    ...
    function initialize(
        ...
        _initialize(name, symbol, decimals, amount, mintable_, owner);
    }
    ...
    function manageFee(
        ...
        tokenInit(router_, maxTxBps_, taxFeeBps_, liquidityFeeBps_, marketingFeeBps_, operationAddresses_, antiBot_);
    }
    ...
    function tokenInit(
        ...
        _tokenSupply = _totalSupply * (10**_decimals);
        _reflectionSupply = (MAX - (MAX & _tokenSupply));
        _taxFee = taxFeeBps_;
    ...
    function initVars(address owner_) internal initializer{
        ...
        _reflectionOwned[owner_] = _reflectionSupply;
    ...
    //     _tokenOwned[account] = _tokenOwne[account].add(amount);
  ...
  // your stuff
    ...
    //     uint256 accountBalance = _tokenOwne[account];
    //     require(accountBalance >= amount, "ERC20: burn amount exceeds balance");
    //     unchecked {
    //         _tokenOwned[account] = accountBalance - amount;
    //     }
    //     _totalSupply -= amount;
    ...
    function tokenFromReflection(uint256 rAmount) public view returns (uint256){
        require(rAmount <= _reflectionSupply, "Reflection amount overflow");
    ...
    function _getCurrentSupply() private view returns (uint256, uint256){
        ...
        if(reflectionSupply < _reflectionSupply.div(_totalSupply)) return (_reflectionSupply, _totalSupply);
        return (reflectionSupply, tokenSupply);
    }

    function _takeLiquidityFee(uint takeLiquidity) private{
        uint256 currentRate = _getRate();
        uint256 rLiquidity = takeLiquidity.mul(currentRate);
        _reflectionOwned[address(this)] = _reflectionOwned[address(this)].add(rLiquidity);
        if(_isExcluded[address(this)]){
            _tokenOwned[address(this)] = _tokenOwned[address(this)].add(takeLiquidity);
        }

        emit Transfer(_msgSender(), address(this), takeLiquidity);
    }

    function _takeMarketingFee(uint256 takeMarketing) private {
        if(takeMarketing>0){
            uint256 currentRate = _getRate();
            uint256 rMarketing = takeMarketing.mul(currentRate);
            _reflectionOwned[_marketingAddress] = _reflectionOwned[_marketingAddress].add(rMarketing);
            if(_isExcluded[_marketingAddress]){
                _tokenOwned[_marketingAddress] = _tokenOwned[_marketingAddress].add(takeMarketing);
            }
            emit Transfer(_msgSender(), _marketingAddress, takeMarketing);
        }
    }
    ...
    function reflectionOf(address account) external view returns (uint256){
        return _reflectionOwned[account];
    }

    function setSupportAccounts(address support, address marketing) external onlyOwner{
    ...
    function setAntiBot(address policeAddress) external onlyOwner {
        antiBot = IPinkAntiBot(policeAddress);
    }
