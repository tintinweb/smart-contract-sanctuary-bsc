...
contract Token is IBEP20, Ownable {
    ...
    function totalLiqfee() public view returns (uint256) {
        return _tLiqTotal;
    }
    ...
    function setFeesRecipients(address _fee, address _liq) external onlyOwner {
        ...
        if (_isExcluded[FeeAddress] == false) {
        ...
        if (_isExcluded[LiqAddress] == false) {
    ...
    function updateFee(
        ...
        require(_txFee < 100 && _burnFee < 100 && _MFee < 100 && _LiqFee < 100);
        require(
            _txFee + _burnFee + _MFee + _LiqFee <= 25,
        ...
        returns (uint256[4] memory)
    {
        ...
        uint256 tMFee = ((tAmount.mul(_M_FEE)).div(_GRANULARITY)).div(100);
        uint256 tLiqFee = ((tAmount.mul(_LIQ_FEE)).div(_GRANULARITY)).div(100);
        return [tFee, tBurn, tMFee, tLiqFee];
    }
    ...
    function getTTransferAmount(
        ...
        return tAmount.sub(tFee).sub(tBurn).sub(tMFee).sub(tLiqFee);
    }
