...
contract Sun is Weather {
    ...
    function stepSun(Decimal.D256 memory beanPrice, Decimal.D256 memory usdcPrice) internal returns (uint256) {
        ...
        uint256 currentBeans = sqrt((bean_reserve * (eth_reserve) * 1e18) / (beanPrice.mul(1e18).asUint256()));
        uint256 targetBeans = sqrt((bean_reserve * (eth_reserve) * 1e18) / (usdcPrice.mul(1e18).asUint256()));

        uint256 price = beanPrice.mul(1e18).div(usdcPrice).asUint256();
        uint256 newSilo;

        if (currentBeans < targetBeans) {
            newSilo = growSupply(targetBeans - currentBeans, price);
        } else if (currentBeans > targetBeans) {
            shrinkSupply(currentBeans - targetBeans, price);
        } else {
    ...
    function lockedReserves() public view returns (uint256, uint256) {
        ...
        ethReserve = (ethReserve * lockedLP) / lp;
        beanReserve = (beanReserve * lockedLP) / lp;
...
contract Weather is Silo {
    ...
    event WeatherChange(uint256 indexed season, uint256 caseId, int8 change, uint32 yield);
    ...
    function stepWeather(uint256 int_price, uint256 endSoil) internal {
        ...
        // Calculate Pod Rate
        Decimal.D256 memory podRate = Decimal.ratio(s.f.pods - s.f.harvestable, bean().totalSupply());
    ...
    function changeWeather(uint256 caseId) private {
        ...
        emit WeatherChange(season(), caseId, change, yield());
    }
    ...
    function sop() private {
        ...
        if (newEth <= s.s.roots / 1e32 || (s.sop.base > 0 && (newBeans * s.sop.base) / s.sop.weth / s.r.roots == 0)) return;
    ...
    function calculateSopBeansAndEth() private view returns (uint256, uint256) {
        ...
        beans = (beans * 10000) / 9985 + 1;

        uint256 beansWithFee = beans * 9975;
        uint256 numerator = beansWithFee * ethBeanPool;
        uint256 denominator = beansBeanPool * 10000 + beansWithFee;
...
contract Silo is Life {
    ...
    function rewardBeans(uint256 amount) private {
        ...
        s.s.stalk = s.s.stalk + (amount * C.getStalkPerBean());
        s.si.beans = s.si.beans + amount;
        s.bean.deposited = s.bean.deposited + amount;
        s.s.seeds = s.s.seeds + (amount * C.getSeedsPerBean());
    }
    ...
    function rewardEther(uint256 amount) internal {
        ...
        } else base = (amount * s.sop.base) / s.sop.weth;
