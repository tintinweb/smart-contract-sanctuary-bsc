...
// File: contracts\NFTMaster\v2\NFTMasterBlindBox.sol
...
contract NFTMasterBlindBoxNew is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // address => num
    mapping(address => uint256) transferAddressAndNum;
    ...
    event InitBox(uint256 _boxIndex, address[] _addressList, uint256[] _numberList, address _token, uint256 _price, bool _status, uint256 _startAt, uint256 _endAt, uint256 _purchaseAmountLimit, uint256 _preemptionDuration);
    event SetStatus(uint256 _boxIndex, bool _status);

    /* box */
    // init box
    function initBox(uint256 _boxIndex, address[] memory _addressList, uint256[] memory _numberList, address _token, uint256 _price, bool _status, uint256 _startAt, uint256 _endAt, uint256 _purchaseAmountLimit, uint256 _preemptionDuration) public onlyAdmin nonReentrant {
        ...
        emit InitBox(_boxIndex, _addressList, _numberList, _token, _price, _status, _startAt, _endAt, _purchaseAmountLimit, _preemptionDuration);
    }

    // get box info
    function getBoxInfo(uint256 _boxIndex) public view returns (uint256, address[] memory, uint256[] memory, address, uint256, bool, uint256, uint256, uint256, uint256, uint256){
        require(_boxIndex > 0, "BOX_INDEX_ERROR");

        BlindBoxInfo memory _blindBoxInfo = BlindBoxList[_boxIndex];
        uint256 _getRemainder = getRemainder(_boxIndex);
        return (_boxIndex, _blindBoxInfo.boxNftList, _blindBoxInfo.boxSupplyList, _blindBoxInfo.payToken, _blindBoxInfo.payPrice, _blindBoxInfo.boxStatus, _blindBoxInfo.boxStartAt, _blindBoxInfo.boxEndAt, _getRemainder, _blindBoxInfo.purchaseAmountLimit, _blindBoxInfo.preemptionDuration);
    }

    // set box supply
    function setBoxSupply(uint256 _boxIndex, address[] memory _addressList, uint256[] memory _numberList) public onlyAdmin nonReentrant {
        require(_boxIndex > 0, "BOX_INDEX_ERROR");
        require(_addressList.length > 0, "NONEMPTY_ADDRESS_LIST");
        require(_addressList.length == _numberList.length, "INCONSISTENT_LENGTH");
        ...
        _blindBoxInfo.boxNftList = _addressList;
    ...
    event BlindPurchase(address recipient, address payToken, uint256 payPrice, uint256 payAmount, uint256 payPriceTotal, address[] nftAddress, uint256[] nftTokenId, uint256 remaining, uint256 at);
    ...
    function blindPurchase(uint256 _boxIndex, uint256 _num) public payable nonReentrant {
        ...
        for (uint256 _dd = 0; _dd < _num; _dd++) {
            ...
            for (uint256 ind = 0; ind < _blindBoxInfo.boxSupplyList.length; ind++) {
                if (_randomness <= _blindBoxInfo.boxSupplyList[ind] && _blindBoxInfo.boxSupplyList[ind] > 0) {
                    resultAddress[_dd] = _blindBoxInfo.boxNftList[ind];
        ...
        for (uint256 _i = 0; _i < numEmit; _i++) {
            if (nftTokenType[resultAddress[_i]] > 0) {
                if (transferAddressAndNum[resultAddress[_i]] == 0) {
                    transferAddressAndNum[resultAddress[_i]] = 1;
                } else {
                    transferAddressAndNum[resultAddress[_i]] = transferAddressAndNum[resultAddress[_i]].add(1);
                }
            }
        }
        ...
        for (uint256 _i = 0; _i < numEmit; _i++) {
            if (nftTokenType[resultAddress[_i]] > 0) {
                // transfer
                require(IERC721(resultAddress[_i]).balanceOf(transferNftAddress) >= transferAddressAndNum[resultAddress[_i]], "NFT_BALANCE_ERROR");
                // get tokenID
                resultTokenId[_i] = IERC721Enumerable(resultAddress[_i]).tokenOfOwnerByIndex(transferNftAddress, transferAddressAndTokenIdIndex[resultAddress[_i]]);
                // transfer token
                IERC721(resultAddress[_i]).safeTransferFrom(transferNftAddress, msg.sender, resultTokenId[_i]);
                // drive transfer address token id index
                transferAddressAndTokenIdIndex[resultAddress[_i]] = transferAddressAndTokenIdIndex[resultAddress[_i]].add(1);
            } else {
                // mint
                resultTokenId[_i] = INFTcustom(resultAddress[_i]).mintItem(msg.sender);
            }

            boxIndexMapping[resultAddress[_i]][resultTokenId[_i]] = boxIndexArr[0];
        }

        for (uint256 _i = 0; _i < numEmit; _i++) {
            if (nftTokenType[resultAddress[_i]] > 0) {
                delete transferAddressAndNum[resultAddress[_i]];
                delete transferAddressAndTokenIdIndex[resultAddress[_i]];
            }
        }

        emit BlindPurchase(msg.sender, _payToken, _payPrice, numEmit, payPriceTotal, resultAddress, resultTokenId, remainingEmit, block.timestamp);
    }
