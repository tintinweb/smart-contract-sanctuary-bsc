...
contract Staking is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable{
  address public usdb;
  ...
  mapping(address => mapping(address => uint256)) private userRewardsPerTokenPaid;
  mapping(address => uint256) private lastRewardClaimTime;
  mapping(address => uint256) private lastRewardClaimPeriod;
  // staker address => reward token => claim amount
  mapping(address => mapping(address => uint256)) private userClaimedRewards;

  uint256 private _MULTIPLIER_;
  // staking token => Multiplier[]
  mapping(address => Multiplier[]) private multiplierStored;
  ...
  mapping(address => uint256) private rewardTokenAddrToIdx;

  // Administrative booleans
  bool public unstakePaused; // For emergencies
  bool public claimPaused; // For emergencies
  ...
  modifier notUnstakePaused() {
    require(unstakePaused == false, "Unstake paused");
    _;
  }
  modifier notClaimPaused() {
  ...
  function initialize(address _usdb, address _controller) public initializer{
    usdb = _usdb;
  ...
  function _calYieldStored(uint256 _rate, uint256 _timeElapse, uint256 _totalsupply) private pure returns(uint256){
    if(_totalsupply>0){
  ...
  function tokenMultiplierCurrent(address _token) public view returns(uint256){
    return tokenMultiplier(_token,currentPeriod);
  }
  ...
  function sync() public {
    ...
    for (uint256 i = lastUpdatePeriod; i < periodStore.length; i++) {
  ...
  function _claim(address _staker) private notClaimPaused{
    sync();
    
    uint256[] memory earnedAmount = _cumulateReward(_staker);
    ...
    for (uint256 i = 0; i < rewardTokens.length; i++) {
      ...
      if(earnedAmount[i]>0){
        ...
        userClaimedRewards[_staker][rewardToken] = userClaimedRewards[_staker][rewardToken] + rewardAmount;
        emit Claim(_staker, rewardToken, rewardAmount);
      }
    }
  }

  function _cumulateReward(address _staker) public view returns(uint256[] memory){
  ...
  function claimedRewards() public view returns(Reward[] memory){
    Reward[] memory rewards = new Reward[](rewardTokens.length);
    for (uint256 i = 0; i < rewardTokens.length; i++) {
      address token = rewardTokens[i];
      rewards[i] = Reward(token, userClaimedRewards[msg.sender][token]);
    }
    return rewards;
  }

  function cumulateReward(address _staker) public view returns(Reward[] memory){
    uint256[] memory earnedAmount = _cumulateReward(_staker);
    Reward[] memory cumulateRewards = new Reward[](earnedAmount.length);
    for (uint256 i = 0; i < rewardTokens.length; i++) {
      cumulateRewards[i] = Reward(rewardTokens[i], earnedAmount[i]);
    }

    return cumulateRewards;
  }

  function stake(address _token,uint256 _liquidity, uint256 _secs) public notStakingPaused{
  ...
  function unstake(address _token, uint256 _liquidity) public notUnstakePaused{
    require(_liquidity > 0, "Must stake more than zero");
