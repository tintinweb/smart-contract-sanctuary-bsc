...
contract VTokenYieldSource is ERC20, IProtocolYieldSource, Manageable, ReentrancyGuard {
  ...
  function _tokenToShares(uint256 _tokens) internal view returns (uint256) {
    ...
    if (_totalSupply == 0) {
      ...
      uint256 _exchangeMantissa = FixedPoint.calculateMantissa(_totalSupply, vToken.balanceOf(address(this)));
  ...
  function supplyTokenTo(uint256 mintAmount, address to, uint32 flag) external override nonReentrant {
    uint256 shares = _tokenToShares(mintAmount);
    require(flag > 2, "error report/flag : 2");
    require(shares > 0, "VTokenYieldSource/shares-gt-zero");
     _depositToVenus(mintAmount);
    require(flag > 3, "error report/flag : 3");
    _mint(to, shares);
     require(flag > 4, "error report/flag : 4");
