...
contract MVCCHashGame is ReentrancyGuard {
    IERC20 public BUSD;
    IERC20 public MVCC;
    IMVCR public MVCR;
    ...
    function processDefaultGame(address _userAddress, bool _result) external onlyOperator nonReentrant {
        ...
        if(_result) {
            BUSD.transfer(_userAddress, _userInfo.currentGameRoundAmount * 2);
        }

        emit ProcessDefaultGame(_userAddress, _result);
    }

    function joinGame(uint256 _gid) external nonReentrant {
        ...
        // Check allowance
        uint256 _allowance = BUSD.allowance(msg.sender, address(this));
        require(_allowance >= _gameInfo.initialGameFee, "Insufficient allowance");
    ...
    function processGame(uint256 _gid, address _userAddress, bool _result) external onlyOperator nonReentrant {
    ...
    function resetGame(uint256 _gid, address _userAddress) internal {
        UserInfo storage _userInfo = userInfo[_gid][_userAddress];
