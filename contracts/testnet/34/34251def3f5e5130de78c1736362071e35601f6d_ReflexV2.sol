...
contract ReflexV2 is Initializable, UUPSUpgradeable, IERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function initializeParams() internal {
      rewardToken = 0x177Aa3e37ef7c2F087d5C078735519A84609F437;
      liquidityFee = 200;
      buybackFee = 100;
      reflectionFee = 800;
      marketingFee = 300;
      ...
      autoBuybackThreshold = 1 * 10**12;
      autoBuybackBlockPeriod = 100;
    ...
    function triggerAutoBuyback() external onlyOwner whenNotPaused {
      swapEthForTokens(autoBuybackThreshold, DEAD);
      autoBuybackBlockLast = block.number;
    }

    function triggerSwapBack() external onlyOwner whenNotPaused {
      swapBack();
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _stakingFee, uint256 _feeDenominator) public onlyOwner whenNotPaused {
      ...
      require(
          ...
          "ReflexV2: Total fee must be greater than 1/4 of fee denominator"
      );
    }
    ...
    function includeInFee(address account) external onlyOwner whenNotPaused {
      ...
      require(isFeeExempt[account] != false, "ReflexV2: Already included in fee");
      isFeeExempt[account] = false;
    }

    function excludeFromFee(address account) external onlyOwner whenNotPaused {
      require(account != address(0), "ReflexV2: Address cant be zero address");
      require(isFeeExempt[account] != true, "ReflexV2: Already excluded from fee");
      isFeeExempt[account] = true;
    }

    function includeInBlacklist(address account) external onlyOwner whenNotPaused {
      require(account != address(0), "ReflexV2: Address cant be zero address");
      require(isBlacklisted[account] != true, "ReflexV2: Already blacklisted");
      isBlacklisted[account] = true;
    }

    function excludeFromBlacklist(address account) external onlyOwner whenNotPaused {
      require(account != address(0), "ReflexV2: Address cant be zero address");
      require(isBlacklisted[account] != false, "ReflexV2: Not blacklisted");
      isBlacklisted[account] = false;
    }

    function setIsDividendExempt(address holder, bool exempt) external onlyOwner whenNotPaused {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
      ...
      if (inSwap) {
        ...
        require(fromBalance >= amount, "ReflexV2: Amount exceeds balance");
        unchecked {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual {
      require(from != address(0), "ReflexV2: Transfer from zero address");
      require(to != address(0), "ReflexV2: Transfer to zero address");
      require(amount > 0, "ReflexV2: Amount must be greater than 0");
      require(!isBlacklisted[from] && !isBlacklisted[to], "ReflexV2: Account blacklisted");
    }
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20MetadataUpgradeable public rewardToken =
        IERC20MetadataUpgradeable(0x177Aa3e37ef7c2F087d5C078735519A84609F437);
    IDexRouter public router;
