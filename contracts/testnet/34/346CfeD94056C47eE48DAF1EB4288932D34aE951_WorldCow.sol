...
contract WorldCow is ERC721, ERC2981, Authorizable {
     ...
     */
    constructor(string memory _baseTokenURI, uint96 _royaltyFeesinBips) ERC721("WorldCow", "COW") {
        _setBaseURI(_baseTokenURI);
        setRoyaltyInfo(owner(), _royaltyFeesinBips);
    }

    /**
     * @dev Modifier to check if the card type is valid.
     */
    ...
    /**
     * @dev Sets the royalty receiver and the royalty fees.
     */
    ...
    /**
     * @dev Mints single card type 1 on 'to' address with starting index 1.
     * @dev Mints single card type 2 on 'to' address with starting index 10001.
     */
    function mint(address _to, uint256 _cardType) public checkValidCardType(_cardType) onlyAuthorized() {
        require(_to != address(0),"WorldCow: _to address not valid");
        counters[_cardType]++;
        uint256 tokenId = counters[_cardType];
        if(_cardType == 2){
            tokenId = tokenId.add(10000);
        }
        require(tokenId <= 20000,"WorldCow: this card type collection is full");
    ...
    /**
     * @dev Mints cards in batch of type 1 or 2 on 'to' address.
     */
    function batchMint(address _to, uint256 _cardType, uint256 _numberOfToken) external checkValidCardType(_cardType) onlyAuthorized() {
    ...
    /**
     * @dev Burns cards of entered token id.
     */
    ...
    /**
     * @dev Returns total card types i.e. 2.
     */
    function totalCardType() public pure returns(uint){
        return 2;
    }
    ...
    /**
     * @dev Returns the token URI taking token id as input.
     */
