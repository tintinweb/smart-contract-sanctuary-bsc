...
contract Crowdfunding is AccessControlEnumerable {
    ...
    function fund(uint256 _regionIndex, uint256 _amount) external {
        ...
        uint256 accountAmountLeft = getMaxAmountPerTime(p.targetAmount) - _fundAmount;
        if (accountAmountLeft == 0) revert ProjectAmountError();
        amountLeft = min(amountLeft, accountAmountLeft);
    ...
    function pendingForUnlock(address _account) public view returns (uint256 lockedAmount, uint256 unlockAmount) {
        lockedAmount = accounts[_account].lockedWithdrawAmount - accounts[_account].unlockedAmount;
        if (accounts[msg.sender].pointFlag) {
            ...
            if (point > _point) {
                unlockAmount = (point - _point) * 50 * 1e18;
            }
        }
    }
