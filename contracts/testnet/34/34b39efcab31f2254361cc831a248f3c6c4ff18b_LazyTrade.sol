...
contract LazyTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    // ipfsmetadata[0] - meta, ipfsmetadata[1] - datas, ipfsmetadata[2] - status
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], impact721, users[0], datas[1], ipfsmetadata[2]);
            } else {
    ...
    // ipfsmetadata[0] - meta, ipfsmetadata[1] - datas, ipfsmetadata[2] - status
    // users[0] - from, users[1] - to
    // datas[0] - supply, datas[1] - nftType, datas[2] -  royal, datas[3] - price, datas[4] - total, datas[5] - amount
    function lazyMint(
        string[] memory ipfsmetadata,
        address[] memory users,
        uint256[] memory datas,
    ...
    function _transferNFT(
        ...
        ImpactCollective1155(impact1155).TransferNFT(users[1], tokenid, count);
        transfersaleToken(users, amount, 1155, tokenid, bidtoken);
    }

    function changeCollectionOwner(address to) public {
        ImpactCollective721(impact721).changeCollectionOwner(to);
        ImpactCollective1155(impact1155).changeCollectionOwner(to);
    }
}