...
interface IBidNFT {
    ...
    function readyToSellToken(uint256 _tokenId, uint256 _price, address _NftAddress) external;
...
contract Market is IBidNFT, ERC721Holder, TransferOwnable, Pausable {
    using SafeMath for uint256;
    ...
    mapping(address => uint) public allowNftInfo;
    mapping(uint256 => address) public allowNftIndex;
    uint256 public allowNftLength = 0;
    
    mapping(uint256 => address) private tokenNftAddress;
    mapping(address => EnumerableMap.UintToUintMap) private _asksMap2;
    ...
    event EthFeeAddressTransferred(address indexed previousOwner, address indexed newOwner);
    event SetEthFeeAmount(address indexed seller, uint256 oldEthFeeAmount, uint256 newEthFeeAmount);
    ...
    constructor(
        ...
        //address _nftAddress = 0x50A5aF9047Ef6AC636ac2b14418a7F1e2Bfb32da;
        address _quoteErc20Address = 0x34abBccb30609C3bDB7E418B265444E0Eff02f42;
        
        //require(_nftAddress != address(0) && _nftAddress != address(this));
        require(_quoteErc20Address != address(0) && _quoteErc20Address != address(this));
        //nft = IERC721(_nftAddress);
        quoteErc20 = IERC20(_quoteErc20Address);
        feeAddr = 0x14c2dc5A2dA77f041051990eAc2192c3122525A7;
        feePercent = 15;
        ethFeeAddr = 0x14c2dc5A2dA77f041051990eAc2192c3122525A7;
        ethFeeAmount = 35e13;
        emit FeeAddressTransferred(address(0), feeAddr);
        emit SetFeePercent(_msgSender(), 0, feePercent);
        emit EthFeeAddressTransferred(address(0), ethFeeAddr);
        emit SetEthFeeAmount(_msgSender(), 0, ethFeeAmount);
    }

    function transferEthFeeAddress(address _ethFeeAddr) public onlyOwner {
        //require(_msgSender() == feeAddr, 'FORBIDDEN');
        ethFeeAddr = _ethFeeAddr;
        emit EthFeeAddressTransferred(_msgSender(), feeAddr);
    }

    function setEthFeeAmount(uint256 _ethFeeAmount) public onlyOwner {
        require(ethFeeAmount != _ethFeeAmount, 'Not need update');
        emit SetEthFeeAmount(_msgSender(), ethFeeAmount, _ethFeeAmount);
        ethFeeAmount = _ethFeeAmount;
    }
    
    function setAllowNftAddress(address NftAddress, bool isAllow) public {
        
        require(isPartner(msg.sender), 'setAllowNftAddress: require isPartner(msg.sender)');
        if(allowNftInfo[NftAddress] == 0){
            
            allowNftIndex[allowNftLength++] = NftAddress;
            
        }
        
        if(isAllow){
            
            allowNftInfo[NftAddress] = 1;
            
        }else{
            
            allowNftInfo[NftAddress] = 2;
    ...
    function buyTokenTo(uint256 _tokenId, address _to) public override whenNotPaused {
        ...
        IERC721 nft = IERC721(tokenNftAddress[_tokenId]);
        ...
        _asksMap2[tokenNftAddress[_tokenId]].remove(_tokenId);
        ...
        delete tokenNftAddress[_tokenId];
    }
    ...
    function setCurrentPrice(uint256 _tokenId, uint256 _price) public override whenNotPaused {
        ...
        _asksMap2[tokenNftAddress[_tokenId]].set(_tokenId, _price);
        emit Ask(_msgSender(), _tokenId, _price);*/
    }

    function readyToSellToken(uint256 _tokenId, uint256 _price, address _NftAddress) public override whenNotPaused {
        readyToSellTokenTo(_tokenId, _price, address(_msgSender()), _NftAddress);
    }
    ...
    function readyToSellTokenTo(
        ...
        require(_price != 0, 'Price must be granter than zero');
        require(allowNftInfo[_NftAddress] == 1, 'readyToSellTokenTo: require allowNftInfo[_NftAddress]');
        nft.safeTransferFrom(address(_msgSender()), address(this), _tokenId);
        tokenNftAddress[_tokenId] = _NftAddress;
        
        _asksMap.set(_tokenId, _price);
        _asksMap2[_NftAddress].set(_tokenId, _price);
    ...
    function getAskLength(address nftAddress) public view returns (uint256) {
        return _asksMap2[nftAddress].length();
    }

    function getAsksDesc(address nftAddress) public view returns (AskEntry[] memory) {

        uint256 length = getAskLength(nftAddress);
        AskEntry[] memory asks = new AskEntry[](length);
        if (length > 0) {
            for (uint256 i = length - 1; i > 0; --i) {
                (uint256 tokenId2, uint256 price2) = _asksMap2[nftAddress].at(i);
                asks[length - 1 - i] = AskEntry({tokenId: tokenId2, price: price2, owner: _tokenSellers[tokenId2]});
            }
            (uint256 tokenId, uint256 price) = _asksMap2[nftAddress].at(0);
            asks[length - 1] = AskEntry({tokenId: tokenId, price: price, owner: _tokenSellers[tokenId]});
        }
        return asks;
    }
    ...
    function getAsksDesc() public view returns (AskEntry[] memory) {
        ...
        if (_asksMap.length() > 0) {
            for (uint256 i = _asksMap.length() - 1; i > 0; --i) {
                (uint256 tokenId2, uint256 price2) = _asksMap.at(i);
                asks[_asksMap.length() - 1 - i] = AskEntry({tokenId: tokenId2, price: price2, owner: _tokenSellers[tokenId2]});
            }
            (uint256 tokenId, uint256 price) = _asksMap.at(0);
            asks[_asksMap.length() - 1] = AskEntry({tokenId: tokenId, price: price, owner: _tokenSellers[tokenId]});
        }
        return asks;
    }*/
    /*
    ...
    function getAsksByPageDesc(uint256 page, uint256 size) public view returns (AskEntry[] memory) {
        if (_asksMap.length() > 0) {
            ...
            if (to == 0) {
                for (uint256 i = 0; from > to; ++i) {
                    (uint256 tokenId2, uint256 price2) = _asksMap.at(from);
                    asks[i] = AskEntry({tokenId: tokenId2, price: price2});
                    --from;
                }
                (uint256 tokenId, uint256 price) = _asksMap.at(0);
    ...
    function getAsksByUserDesc(address user) public view returns (AskEntry[] memory) {
        ...
        if (_userSellingTokens[user].length() > 0) {
            for (uint256 i = _userSellingTokens[user].length() - 1; i > 0; --i) {
                uint256 tokenId2 = _userSellingTokens[user].at(i);
                uint256 price2 = _asksMap.get(tokenId2);
                asks[_userSellingTokens[user].length() - 1 - i] = AskEntry({tokenId: tokenId2, price: price2});
            }
    ...
    function setFeeAddr(address _feeAddr) public onlyOwner{
        ...
        require(_feeAddr != address(0) && _feeAddr != address(this));
        feeAddr = _feeAddr;
        emit FeeAddressTransferred(_msgSender(), feeAddr);
        
    }*/
    
    function transferFeeAddress(address _feeAddr) public onlyOwner {
        //require(_msgSender() == feeAddr, 'FORBIDDEN');
        feeAddr = _feeAddr;
        emit FeeAddressTransferred(_msgSender(), feeAddr);
    }
