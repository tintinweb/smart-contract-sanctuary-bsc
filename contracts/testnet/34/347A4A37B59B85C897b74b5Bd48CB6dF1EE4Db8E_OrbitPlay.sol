...
contract OrbitPlay is Ownable {
	...
	mapping(address => DiceBet) public userDiceInfo;

    PlayInfo public playInfo;
    PlayInfo public diceInfo;    

    uint256 private diceRandomFactor;
    bool public dicePlayable;
    uint256 public diceReturnsPercentage;
    uint256 public diceBurnOnLossPercentage;

    event DiceRollResult(address bidder, uint256 betAmount, uint8 betNumber, uint8 winNumber);
    ...
    function placeDiceRollBet(uint256 amount, uint8 diceNumber) public returns(uint8){
		require(
            dicePlayable,
            "OrbitPlay: DiceRoll can not be played!"
        );
        require(
            amount >= 250 * 10**18,
            "OrbitPlay: You should bet at least 250 tokens!"
        );
        require(
            amount <= 1250 * 10**18,
            "OrbitPlay: You can bet up to 1250 tokens!"
        );

		uint8 destiny = diceRandom();
		diceRandomFactor += uint256(destiny);
        ...
        DiceBet storage userInfo = userDiceInfo[msg.sender];
        userInfo.timesPlayed += 1;          
        userInfo.totalBetAmount += amount;
        
		if(destiny == diceNumber){ //win
            userInfo.returningAmount = amount.mul(diceReturnsPercentage).div(10000).add(amount);
            userInfo.wins += 1;           
            userInfo.totalReturned += userInfo.returningAmount;
		}else{ //lost
			userInfo.returningAmount = 0;
            uint256 burnAmount = amount.mul(diceBurnOnLossPercentage).div(10000);
            IERC20(tokenAddress).safeTransfer(DeadAddress, burnAmount);

		}        
        
        emit DiceRollResult(msg.sender, amount, diceNumber, destiny);

		return destiny;
	}

    function claimDiceRollWin() external {
        DiceBet memory userInfo = userDiceInfo[msg.sender];

        require(
            userInfo.returningAmount > 0,
            "OrbitPlay: You don't have claimable tokens!"
        );
    ...
    function diceRandom() private view returns (uint8) {
       	uint256 blockValue = uint256(blockhash(block.number-1 + block.timestamp));
        blockValue = blockValue + diceRandomFactor;
