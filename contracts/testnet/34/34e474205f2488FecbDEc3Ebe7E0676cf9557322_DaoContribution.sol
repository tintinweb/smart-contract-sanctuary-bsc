...
contract Referral{
   ...
   */
  function payReferral(uint256 value) internal returns(uint){
  ...
  function changeuserInfo(address payable user,address payable _referrer, uint _reward, uint _referredCount) public onlyOperator{
...
contract DaoContribution is ReentrancyGuard,IDaoContribution,Referral{
  ...
  function randomAgentName(uint256 index,address _user,address _upline) public view returns (string memory){
    ...
    return (uint256(keccak256(abi.encodePacked(block.timestamp,index,_user,_upline,id)))%(2**20)).toHexString(4);
  }
