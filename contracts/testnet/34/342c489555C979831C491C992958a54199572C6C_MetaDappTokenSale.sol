...
contract MetaDappTokenSale {
    ...
    function buy(uint tokens) public payable {
        require(msg.value / phase(currentPhaseIndex).price == tokens);
    ...
    function __unAmount(uint _amount, uint decimals) private pure returns(uint) {
    ...
    function __endSale() public isOwner {
    ...
    function __phase(uint phase_id) public view returns(Phase memory) {
        return phases[phase_id];
    }
