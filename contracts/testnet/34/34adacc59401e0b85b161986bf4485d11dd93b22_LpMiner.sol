...
contract LpMiner {
    ...
    constructor(address lpContractAddress, uint256 lpDecimal, 
                address edgContractAddress, uint256 edgDecimal) {
        LP_CONTRACT_ADDRESS = lpContractAddress;
        LP_DECIMAL = lpDecimal;
        EDG_CONTRACT_ADDRESS = edgContractAddress;
        EDG_DECIMAL = edgDecimal;
        owner = msg.sender;
    }
    
    // 启动挖矿
    function startMineJ(uint256 sMineTime) public onlyOwner {
        if (startMine) return;
        startMine = true;
        startMineTime = sMineTime;
        lastUnitPerBlock = getBlockHeight().sub(1);
    }
    ...
    function getDailyAmount() internal view returns (uint256) {
        return getTotalPerBlock(getBlockHeight());
    }
    ...
    function getPendingProfit(address user, uint256 id, uint256 currentBlockHeight) internal view returns (uint256) {
        uint256 itemu = 0;
        uint256 t = 0;
        if (addressStakeIdMap[user][id].lastTakeHeight >= currentBlockHeight) {
            return t;
        }
    ...
    function getTotalPerBlock(uint256 blockHeight) internal view returns (uint256) {
        ...
        if (blockHeight <= 30) {
            return uint256(10).power(EDG_DECIMAL).mul(2160);
        } else if (blockHeight >= 31 && blockHeight <= 60) {
            return uint256(10).power(EDG_DECIMAL).mul(1080);
        } else if (blockHeight >= 61 && blockHeight <= 90) {
            return uint256(10).power(EDG_DECIMAL).mul(540);
        } else {
            return uint256(10).power(EDG_DECIMAL).mul(270);
        }
    }
    ...
    function calculateBlockPer(uint256 blockHeight) internal view returns (uint256) {
        ...
        uint256 p = blockTotalCoin.div(netTotalStake);
        return p;
    }
    ...
    function extract(uint256 id) public {
        ...
        require(Bep20TransferHelper.safeTransfer(EDG_CONTRACT_ADDRESS, msg.sender, pendingProfit), "asset insufficient");
