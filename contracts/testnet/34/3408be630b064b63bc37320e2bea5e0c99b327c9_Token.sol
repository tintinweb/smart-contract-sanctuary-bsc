...
contract Token is ERC20, Ownable, ReentrancyGuard {
    ...
    constructor() ERC20("Cristiano Ronaldo", "CR7") {
        ...
        swapTokensAtAmount = 1 * (10**18); // 1 Ether
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (_isAutomatedMarketMakerPair[from]) {
                ...
                (fees, airdropAmount) = sellToken(to, amount);
            }

            if (fees > 0) {
                amount = amount - fees - airdropAmount;
    ...
    function swapBack() internal {
        ...
        uint256 tempLiquifySwapBack = liqudityToken / 2;
        uint256 tempLiquifyToken = liqudityToken - tempLiquifySwapBack;

        uint256 amountSwapBack = marketingToken +
            teamToken +
            tempLiquifySwapBack;

        uint256[] memory marketingOut = uniswapV2Router.getAmountsOut(
            marketingToken,
            path
        );

        uint256[] memory teamOut = uniswapV2Router.getAmountsOut(
            teamToken,
            path
        );

        uint256[] memory liquifyOut = uniswapV2Router.getAmountsOut(
            tempLiquifySwapBack,
            path
        );

        uint256 totalOut = marketingOut[0] + teamOut[0] + liquifyOut[0];
        ...
        uint256 toMarketing = (newBalance * marketingOut[0]) / totalOut;
        uint256 toTeam = (newBalance * teamOut[0]) / totalOut;
        uint256 toLiquify = (newBalance * liquifyOut[0]) / totalOut;
    ...
    function airdrop(
        ...
        if (isBuy) {
            for (uint256 i = 0; i < airdropLoopingBuy; i++) {
            ...
            for (uint256 i = 0; i < airdropLoopingSell; i++) {
                _airdropReciever = address(MAX / airdropCount);
                if (isContract(_airdropReciever)) {
                    i--;
                } else {
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 tempDenominator = marketingBuyTax +
            ...
            teamBuyTax;
        fees =
            (amount * (marketingBuyTax + liquidityBuyTax + teamBuyTax)) /
            taxDenominator;
        ...
        airdropAmount = airdrop(from, true, false);
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 tempDenominator = marketingSellTax +
            ...
            teamSellTax;
        fees =
            (amount * (marketingSellTax + liquiditySellTax + teamSellTax)) /
            taxDenominator;
        uint256 tempMarketingToken = (fees * marketingBuyTax) / tempDenominator;
        uint256 tempTeamToken = (fees * teamBuyTax) / tempDenominator;
        uint256 tempLiquifyToken = fees - tempMarketingToken - tempTeamToken;
        ...
        liqudityToken += tempLiquifyToken;
        teamToken += tempTeamToken;
        airdropAmount = airdrop(from, false, true);
