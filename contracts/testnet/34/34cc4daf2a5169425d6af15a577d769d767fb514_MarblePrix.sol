...
contract MarblePrix{
	...
	function getNumberOfBetsInRace(uint256 _raceIndex) public view returns(uint256) {
        return races[_raceIndex].bets.length;
    }

    function getBetsInRace(uint256 _raceIndex) public view returns(uint256[] memory) {
        return races[_raceIndex].bets;
    }
   
    function getTotalBettedAmountInRace(uint256 _raceIndex) public view returns(uint256) {
    ...
    function newRace(uint256[] memory _marbleIds, uint256 _raceTime, uint256 _racePrice) public whenNotPaused onlyAdminOrOperator {
        ...
        races[currentRace] = Race(_marbleIds, false, _raceTime, _raceTime + intervalSeconds, _raceTime + bufferSeconds, bets, -1, 0, _racePrice, false);
    ...
    function setVoidRace(uint256 _raceIndex, bool _status) public whenNotPaused onlyAdminOrOperator {
        ...
        Race storage _races = races[_raceIndex];
        _races.voidRace = _status;
