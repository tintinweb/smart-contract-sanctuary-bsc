...
contract TokenSwap is Ownable {
    ...
    event SwapExact(address indexed token1, address indexed token2, uint256 amount);
    event SwapForExact(address indexed token1, address indexed token2, uint256 amount);

    address public pool;
    address public router;

    modifier onlyPool() {
        require(msg.sender == pool, "Not Infinity Pool");
        _;
    }
    ...
    function swapExactTokens(address token1, address token2, uint256 amount) external onlyPool {
    ...
    function swapForExactTokens(address token1, address token2, uint256 amount, uint256 inputMax) external onlyPool {
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token2;
        IERC20(token1).safeTransferFrom(msg.sender, address(this), inputMax);
        IERC20(token1).approve(router, inputMax);
        ...
        emit SwapForExact(token1, token2, amount);
    }
}
