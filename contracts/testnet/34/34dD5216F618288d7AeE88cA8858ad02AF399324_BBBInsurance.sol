    ...
    VerifySignature
{
    ...
    mapping(uint256 => Insurance) internal insurances;
    ...
    modifier onlyInsuranceOwner(uint256 insuranceContentHash_) {
        require(
            insurances[insuranceContentHash_].creator == msg.sender,
            "Error: You are not the insurance's owner"
        );
        _;
    }
    ...
    event CreateInsurance(
        address indexed insuranceOwner_,
        uint256 insuranceId_,
        uint256 deadline_,
        uint256 odd_
    );
    event PlayerBet(
    ...
    event FinalizeInsurance(uint256 insuranceId_, Result result_);
    event Claim(address indexed player_, uint256 insuranceId_, uint256 amount_);
    ...
    event ReportInsurance(
        address indexed player_,
        uint256 insuranceId_,
        Result result_
    );
    event RequestAdminCheck(uint256 indexed insuranceId_);
    event TerminateInsurance(uint256 indexed insuranceId_);
    event GetRefund(
        uint256 indexed insuranceId_,
        address indexed player_,
    ...
    event AdminResolveReport(
        uint256 indexed insuranceId_,
        Result correctResult_
    );

    event AllowClaim(uint256 indexed insuranceId_, Result finalResult_);

    event Penalty(
        uint256 indexed insuranceId_,
        address insuranceOwner_,
    ...
    /**
     * @notice This function is used by player to bet on the insurance
     * @notice Player have to approve amount_ token before call this function
     * @param insuranceId_ insurance id
     * @param initFee_ the fee that host have to bet on this insurance first to init the insurance
     * @param predictResult_ predict result is base on Result enum
     * @param deadline_ unixtimestamp that is the deadline of this insurance
     */
    ...
    function createInsurance(
        ...
        uint256 deadline_,
        address betToken_,
        uint256 insuranceMinBetAmount_,
        uint256 odd_,
        ...
        require(
            insurances[insuranceId_].status == InsuranceStatus.NOT_CREATED,
            "Error: Your insurance was created!"
        );
        require(
        ...
        require(
            insuranceMinBetAmount_ >= _MINIMUM_BET_AMOUNT &&
                insuranceMinBetAmount_ <= _MAXIMUM_BET_AMOUNT,
            "Error: initFee is not greater than minimum bet and lower than Maximum bet"
        );

        require(
            initFee_ >= insuranceMinBetAmount_,
        ...
        insurances[insuranceId_].status = InsuranceStatus.OPEN;
        //Set deadline
        insurances[insuranceId_].deadline = deadline_;

        insurances[insuranceId_].creatorInitResult = predictResult_;

        insurances[insuranceId_].creator = msg.sender;

        insurances[insuranceId_].intermediary = intermediary_;

        insurances[insuranceId_].betToken = betToken_;

        insurances[insuranceId_].insuranceMinBetAmount = insuranceMinBetAmount_;

        insurances[insuranceId_].odd = odd_;
        insurances[insuranceId_].counterpartPoolCap =
            (initFee_ * _ODD_RATE_DENOMINATOR) /
            odd_;

        emit CreateInsurance(msg.sender, insuranceId_, deadline_, odd_);
        //init betting by host
        bet(insuranceId_, predictResult_, initFee_);
    }
     ...
     * @param insuranceId_ insurance id
     * @param predictResult_ predict result is base on Result enum
     * @param amount_ amount that player bet on insurance
     */
    function bet(
        uint256 insuranceId_,
        Result predictResult_,
        ...
        require(
            insurances[insuranceId_].status == InsuranceStatus.OPEN,
            "Error: Your insurance is not open!"
        );

        require(
            amount_ >= insurances[insuranceId_].insuranceMinBetAmount,
            "Error: Please betting with an amount token that is greater than insurance min bet amount!"
        );

        require(
            block.timestamp < insurances[insuranceId_].deadline,
            "Error: your insurance is timeout!"
        );

        require(predictResult_ != Result.HIDDEN, "Error: Result can not be 0");

        if (predictResult_ == insurances[insuranceId_].creatorInitResult) {
            require(
                msg.sender == insurances[insuranceId_].creator,
        ...
        if (predictResult_ != insurances[insuranceId_].creatorInitResult) {
            require(
                getPoolRemainOfCounterpart(insuranceId_) >= amount_,
        ...
        super.betToContract(
            amount_,
            msg.sender,
            insurances[insuranceId_].betToken
        );

        //update pool
        if (
            insurances[insuranceId_]
            ...
            insurances[insuranceId_]
                .pools[predictResult_]
                .playerBettingInfo[msg.sender]
                .index = insurances[insuranceId_]
                .pools[predictResult_]
                .players
                .length;

            insurances[insuranceId_].pools[predictResult_].players.push(
                msg.sender
            );
        }
        ...
        if (predictResult_ == insurances[insuranceId_].creatorInitResult) {
            ...
            insurances[insuranceId_].counterpartPoolCap =
                (insurances[insuranceId_]
                    .pools[insurances[insuranceId_].creatorInitResult]
                    .totalBettingAmount * _ODD_RATE_DENOMINATOR) /
                insurances[insuranceId_].odd;
        }

        emit PlayerBet(msg.sender, insuranceId_, amount_, predictResult_);
    }
    ...
    function bet(
        ...
        bytes32 messageHash = keccak256(
            abi.encodePacked(insuranceId_, predictResult_, amount_, msg.sender)
        );
        ...
        if (predictResult_ != insurances[insuranceId_].creatorInitResult) {
            console.log(
                "getPoolRemainOfCounterpart(insuranceId_): ",
                getPoolRemainOfCounterpart(insuranceId_)
            );
    ...
    /**
     * @notice This function get insurance counterpart
     * @param insuranceId_ insurance id
     */
    function getPoolRemainOfCounterpart(uint256 insuranceId_)
        public
        view
        returns (uint256)
    {
        if (insurances[insuranceId_].creatorInitResult == Result.RIGHT) {
            return
                insurances[insuranceId_].counterpartPoolCap -
                insurances[insuranceId_].pools[Result.WRONG].totalBettingAmount;
        } else {
            return
                insurances[insuranceId_].counterpartPoolCap -
                insurances[insuranceId_].pools[Result.RIGHT].totalBettingAmount;
        }
    }

    /**
     * @notice This function get insurance info
     * @param insuranceId_ insurance id
     */
    function getInsurance(uint256 insuranceId_)
        public
        view
        returns (InsuranceInfo memory)
    {
        ...
        insuranceInfo.creator = insurances[insuranceId_].creator;
        insuranceInfo.result = insurances[insuranceId_].result;
        insuranceInfo.counterpartPoolCap = insurances[insuranceId_]
            .counterpartPoolCap;
        insuranceInfo.wrongTotalBettingAmount = insurances[insuranceId_]
            .pools[Result.WRONG]
            .totalBettingAmount;
        insuranceInfo.rightTotalBettingAmount = insurances[insuranceId_]
            .pools[Result.RIGHT]
            .totalBettingAmount;
        insuranceInfo.status = insurances[insuranceId_].status;
        insuranceInfo.deadline = insurances[insuranceId_].deadline;
        insuranceInfo.betToken = insurances[insuranceId_].betToken;
        insuranceInfo.penalty = insurances[insuranceId_].penalty;
        insuranceInfo.insuranceMinBetAmount = insurances[insuranceId_]
            .insuranceMinBetAmount;

        return insuranceInfo;
    }

    /**
     * @notice This function list players address that bet in right by insurance id
     * @param insuranceId_ insurance id
     */

    function getRightPlayers(uint256 insuranceId_)
        public
        view
        returns (address[] memory)
    {
        return insurances[insuranceId_].pools[Result.RIGHT].players;
    }

    /**
     * @notice This function list players address that bet in wrong by insurance id
     * @param insuranceId_ insurance id
     */
    function getWrongPlayers(uint256 insuranceId_)
        public
        view
        returns (address[] memory)
    {
        return insurances[insuranceId_].pools[Result.WRONG].players;
    }

    /**
     * @notice This function gets player info by insurance id
     * @param insuranceId_ insurance id
     */
    function getPlayerInfo(uint256 insuranceId_)
        public
        view
        returns (PlayerBettingInfo memory right, PlayerBettingInfo memory wrong)
    {
        right = insurances[insuranceId_].pools[Result.RIGHT].playerBettingInfo[
            msg.sender
        ];
        wrong = insurances[insuranceId_].pools[Result.WRONG].playerBettingInfo[
            msg.sender
        ];
        return (right, wrong);
    }
    ...
    function betBalanceOf(address player_, uint256 insuranceId_)
        public
        view
    ...
    /**
     * @notice This function is used by admin who has joined the insurance to finalize the insurance with result
     * @param insuranceId_ insurance id
     * @param result_ insurance result
     */

    function finalizeInsurance(uint256 insuranceId_, Result result_)
        public
        nonReentrant
    {
        // require(
        ...
        //         insurances[insuranceId_].creator == msg.sender,
        //     "Error: you are not insurance's host or admin"
        // );

        require(
            insurances[insuranceId_].intermediary == msg.sender,
            "Error: you are not intermediary!"
        );
        ...
        require(
            insurances[insuranceId_].deadline < block.timestamp,
            "Error: Your insurance doesn't reach the deadline!"
        );

        require(result_ != Result.HIDDEN, "Error: Result can not be 0");

        insurances[insuranceId_].status = InsuranceStatus.CLOSE;

        insurances[insuranceId_].result = result_;

        emit FinalizeInsurance(insuranceId_, result_);
        emit AllowClaim(insuranceId_, insurances[insuranceId_].result);
    }
    ...
    /**
     * @notice player claim reward by insurance
     * @param insuranceId_ insurance id that player will claim
     */

    function claim(uint256 insuranceId_) public nonReentrant {
        require(
            insurances[insuranceId_].status == InsuranceStatus.CLOSE,
            "Error:Insurance result is processing"
        );
        ...
        require(
            insurances[insuranceId_]
                ...
                insurances[insuranceId_]
                    ...
                    .joined ==
                true,
            "Error: you didn't join this insurance!"
        );
        ...
        require(
            insurances[insuranceId_]
                ...
                insurances[insuranceId_]
                    ...
                    .claimed ==
                false,
            "Error: you have claimed on this insurance!"
        );
        ...
        totalWinnerBetting = insurances[insuranceId_]
            .pools[insurances[insuranceId_].result]
            .totalBettingAmount;
        ...
        winnerbettingAmount = insurances[insuranceId_]
            .pools[insurances[insuranceId_].result]
            .playerBettingInfo[msg.sender]
            .bettingAmount;
        ...
        if (
            ...
            if (hostCommission != 0) {
                ...
                super.transferHostCommission(
                    insurances[insuranceId_].creator,
                    hostCommission,
                    insurances[insuranceId_].betToken
                );
                emit HostCommission(msg.sender, insuranceId_, hostCommission);
            }
        ...
        if (
            insurances[insuranceId_]
                ...
                .joined ==
            true &&
            insurances[insuranceId_].creatorInitResult !=
            insurances[insuranceId_].result
        ) {
        ...
        // //claim
        super.withdraw(claimAmount, insurances[insuranceId_].betToken);
        ...
        emit Claim(msg.sender, insuranceId_, claimAmount);
    }
    ...
    function creatorPoolLoserRemainCalculator(
        ...
        if (
            (totalLoserBetting_ * _ODD_RATE_DENOMINATOR) /
                insurances[insuranceId_].odd ==
            totalWinnerBetting_
        ) {
        ...
        uint256 loserBettingAmount = insurances[insuranceId_]
            .pools[insurances[insuranceId_].creatorInitResult]
            .playerBettingInfo[msg.sender]
            .bettingAmount;
    ...
    /**
     * @notice player claims all reward by list of insurance ids
     * @param insuranceIds_ list of insurance ids that player will claim
     */
    function claimAll(uint256[] memory insuranceIds_) public {
        uint256 length = insuranceIds_.length;

        for (uint256 index = 0; index < length; index++) {
            claim(insuranceIds_[index]);
        }
    }
}
