...
contract Tournament is AccessControl {
    ...
    function initialize(
        ...
        require(
            ...
            "REGISTRATION START TIME MUST BE GREATER THAN NOW"
        );
        require(
            _registrationStartTime < _registrationEndTime,
            "REGISTRATION END TIME MUST BE GREATER THAN START TIME"
        );
    ...
    function setTokens(address[] calldata _tokens) external {
        ...
        require(_tokens.length > 0, "ATLEAST ONE TOKEN MUST BE ADDED");
        require(
    ...
    function setRegistrationFee(
        ...
        require(
            ...
            "REGISTRATION START TIME MUST BE SET"
        );
        require(
            _registrationStartTime < _registrationEndTime,
            "REGISTRATION START CANNOT BE GREATER THAN THE END TIME"
        );
        if (_registrationToken == address(0) && _registrationFeeAmount == 0) {
            require(_validToken(_registrationToken), "TOKEN NOT ALLOWED");
        }
        require(
            block.timestamp < registrationStartTime ||
                registrationStartTime == 0,
    ...
    function register() external payable {
        ...
        require(!registeredAddresses[msg.sender], "ALREADY REGISTERED");
        //require(block.timestamp > registrationStartTime && block.timestamp < registrationEndTime, "UNABLE TO REGISTER THIS TIME");
        ...
        if (registrationToken == address(0)) {
            require(
                ...
                "NOT ENOUGH REGISTRATION FEE AMOUNT"
            );
        } else {
    ...
    //    require(
    //        _verifyRefund(_userAddresses, _signature),
    //        "INVALID_SIGNATURE"
    //    );
    ...
    //        teamTokenRefund[_userAddresses[i]] += (registrationFeeAmount);
    ...
    function withDrawTournament() external {
        ...
        require(block.timestamp < registrationEndTime, "INVALID_TIME");
        ...
        uint256 calculatedPlatformFee = (registrationFeeAmount * pfeeRate) /
            100;

        uint256 calculatedTournamentFee = (registrationFeeAmount * tfeeRate) /
            100;
    ...
    function claimRefund() external {
        require(msg.sender == tx.origin, "CANNOT_CALL_FROM_CONTRACT");
        require(!blackListAddresses[msg.sender], "YOU ARE IN BLACK LIST");
        require(teamTokenRefund[msg.sender] > 0, "NO TOKEN TO CLAIM");

        uint256 claimAmount = teamTokenRefund[msg.sender];

        teamTokenRefund[msg.sender] = 0;

        currPrizePool[registrationToken] -= claimAmount;

        if (registrationToken == address(0)) {
            payable(msg.sender).transfer(claimAmount);
        } else {
            //Transfer reward to team captain
            IERC20(registrationToken).safeTransfer(msg.sender, claimAmount);
        }
    ...
    function removeSponsorFromPrizePool(address _token) external {
        ...
        require(
            block.timestamp < registrationStartTime,
            "UNABLE TO REMOVE SPONSOR THIS TIME"
        );
    ...
    function distributeTokensReward(
        ...
        address[] calldata _refundedAddresses,
        bytes memory _signature
    ) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
        ...
        //require(
        //    _refundedAddresses.length == 0 || registrationFeeAmount == 0,
        //    "MUST REFUND BEFORE DISTRIBUTE"
        //);
        require(
        ...
        // Refund to user
        if (registrationFeeAmount > 0 && _refundedAddresses.length > 0) {
            for (uint256 i = 0; i < _refundedAddresses.length; i++) {
                require(
                    registeredAddresses[_refundedAddresses[i]],
                    "USER NOT REGISTERED"
                );
                registeredAddresses[_refundedAddresses[i]] = false;
                teamTokenRefund[_refundedAddresses[i]] += (
                    registrationFeeAmount
                );
                totalPrizePool[registrationToken] -= registrationFeeAmount;
            }
        ...
        for (uint256 j = 0; j < _userAddresses.length; j++) {
            require(
                registeredAddresses[_userAddresses[j]],
                "USER NOT REGISTERED"
            );
            if (totalPrizePool[_tokenRewards[j]] <= 0) {
                continue;
            } else {
                uint256 rewardAmount = (totalPrizePool[_tokenRewards[j]] *
                    _percentages[j]) / (1e18);
                teamTokenRewards[_userAddresses[j]][
                    _tokenRewards[j]
                ] += rewardAmount;

                totalPrizePool[registrationToken] -= rewardAmount;
            }
        }
    ...
    function claimTokensReward() external {
        require(msg.sender == tx.origin, "CANNOT_CALL_FROM_CONTRACT");
        require(!blackListAddresses[msg.sender], "YOU ARE IN BLACK LIST");
        require(block.timestamp > claimTime, "NOT ALLOWED TO CLAIM THIS TIME");
        require(
    ...
    function _verifyDistributeToken(
        ...
        return
            verify(
                ITournamentFactory(factory).getSigner(),
                _userAddresses,
                _tokenAddresses,
                _amounts,
                _refundedAddresses,
    ...
    //    // Using Openzeppelin ECDSA cryptography library
    //    function getRefundMessageHash(address[] calldata _userAddresses)
    //        public
    //        pure
    //        returns (bytes32)
    //    {
    //        return keccak256(abi.encodePacked(_userAddresses));
    //    }
    ...
    //        bytes memory signature
    //    ) public pure returns (bool) {
    //        bytes32 messageHash = getRefundMessageHash(_userAddresses);
    //        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
