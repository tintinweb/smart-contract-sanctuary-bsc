 ...
 * @notice Handle the distribution of Sesame to stakers
 */
contract StakerRewardsDistributor is ReentrancyGuard, Ownable {
    ...
    /**
     * @notice Constructor
     * @param _sesameToken address of the token staked (Sesame)
     ...
     * @dev There is a limit of 1 Sesame per deposit to prevent potential manipulation of current shares
     */
        ...
        nonReentrant
    {
        require(
            ...
            "Deposit: Amount must be >= 1 Sesame"
        );
        ...
        // Transfer Sesame tokens to this address
        ...
        // Verify Sesame token allowance and adjust if necessary
        _checkAndAdjustSesameTokenAllowanceIfRequired(
            amount,
    ...
    /**
     * @notice Calculate value of Sesame for a user given a number of shares owned
     * @param user address of the user
     */
    function calculateSharesValueInSesame(address user)
        external
        view
        returns (uint256)
    {
    ...
    /**
     * @notice Calculate price of one share (in Sesame token)
     * Share price is expressed times 1e18
     */
    function calculateSharePriceInSesame() external view returns (uint256) {
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(
            address(this)
        );
     ...
     */
    function _checkAndAdjustSesameTokenAllowanceIfRequired(
        uint256 _amount,
        address _to
    ) internal {
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        // Retrieve total amount staked and calculated current amount (in Sesame)
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(
            address(this)
        );
        ...
        // Transfer Sesame tokens to sender
