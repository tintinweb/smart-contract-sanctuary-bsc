...
contract Grade24 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, IGrade24 {
	...
	struct Member {
		...
		/*P2 = breed*/
		int256 ownP2;      
		int256 directP2; //not include in p calculation
		int256 groupP2; 
		int256 level2P2;
    ...
    function initialize(IERC20Upgradeable _mbtc, address _root, address authorizer ) public initializer {
	...
	function registerExt(address from, address myParent) public {
	...
	function isRegistered(address from) public view returns (bool) {
	...
	function _register(address from, address myParent) private{
		...
		require(parent.children.length + 1 <= 50, "too many referees");
	...
	function updateTicketP(address from, int256 qty) public onlyValidCaller {
		Member storage fromMember = members[from];
		int256 totalP1;
		
		if (fromMember.round > 0) {
			fromMember.ownP1 += qty;
			totalP1 += qty;
			
			emit updateP(from, from, lastRound, fromMember.ownP1 + fromMember.directP1 + fromMember.groupP1 + fromMember.level2P1, uint8(1));
			
			Member storage fromMemberParent = members[fromMember.parent];
			
			if (fromMemberParent.round > 0) {
				...
				emit updateP(from, fromMember.parent, lastRound, fromMemberParent.ownP1 + fromMemberParent.directP1 + fromMemberParent.groupP1 + fromMemberParent.level2P1, uint8(1));
				
				Member storage fromMemberGrandParent = members[fromMemberParent.parent];
				
				if (fromMemberGrandParent.round> 0) {
				
					int256 highestP1 = 0;
					int256 addLevel2P1 = 0;
					int256 deductLevel2P1 = 0;
					
					totalP1 -= fromMemberGrandParent.level2P1;
					//recalculate for grandparent's level2P1
					uint256 i;
					if (fromMemberGrandParent.children.length == 1) {
						Member storage thisChild = members[ fromMemberGrandParent.children[i] ];
						addLevel2P1 += thisChild.directP1;
					} else {
						for(i=0; i< fromMemberGrandParent.children.length;i++) {
							...
							if (thisChild.round > 0) {
								if (thisChild.directP1 > highestP1) {
									deductLevel2P1 = thisChild.directP1;
					...
					fromMemberGrandParent.level2P1 = addLevel2P1 - deductLevel2P1;
					
					totalP1 += fromMemberGrandParent.level2P1;
					
					emit updateP(from, fromMemberParent.parent, lastRound, fromMemberGrandParent.ownP1 + fromMemberGrandParent.directP1 + fromMemberGrandParent.groupP1 + fromMemberGrandParent.level2P1, uint8(1));
				}
			}
	...
	function updateBreedP(address from, int256 qty) public onlyValidCaller {
		Member storage fromMember = members[from];
		int256 totalP2;
		...
		if (fromMember.round > 0) {
			...
			if (fromMemberParent.round > 0) {
				...
				if (fromMemberGrandParent.round > 0) {
					...
					int256 highestP2 = 0;
					int256 addLevel2P2 = 0;
					int256 deductLevel2P2 = 0;
					int256 addGroupP2 = 0;
					
					totalP2 -= fromMemberGrandParent.level2P2 + fromMemberGrandParent.groupP2;
					
					uint256 i;
					if (fromMemberGrandParent.children.length == 1) {
						Member storage thisChild = members[ fromMemberGrandParent.children[i] ];
						addLevel2P2 += thisChild.directP2;
						...
						for(i=0; i< fromMemberGrandParent.children.length;i++) {
							...
							if (thisChild.round > 0) {
								if (thisChild.directP2 > highestP2) {
									deductLevel2P2 = thisChild.directP2;
									highestP2 = thisChild.directP2;
								} else if (highestP2 > 0 && thisChild.directP2 == highestP2) { 
									deductLevel2P2 += thisChild.directP2;
								}
					...
					fromMemberGrandParent.level2P2 = addLevel2P2 - deductLevel2P2;
					fromMemberGrandParent.groupP2 = addGroupP2;
	...
	function endRound(int256 _totalP3, uint256[24] memory _countPerGrade, uint256[24] memory _normalDists, uint256[24] memory _tzedakahDists) public onlyOwner {
		uint256 lastTwoRound = lastRound - 1;
		
		require(rounds[lastTwoRound].status==roundStatus.PROCESSING, "round not processing");
		
		//check _countPerGrade
		if (lastTwoRound > 1) {
			Round storage lastThreeRound = rounds[lastTwoRound - 1];
			
			for(uint256 i=0; i < _countPerGrade.length; i++) {
				require(_countPerGrade[i] >= lastThreeRound.countPerGrade[i], "invalid grade count");
			}
		}
		
		rounds[lastTwoRound].countPerGrade = _countPerGrade;
	...
	function claim( uint256[] memory myRounds, uint256[] memory myGrades, uint256 timestamp, uint8 v, bytes32 r, bytes32 s) public nonReentrant onlyIfGranted(v, r, s, keccak256(abi.encodePacked(this, msg.sender,  timestamp))){
		...
		for(uint256 toClaimRound=lastClaimRound+1; toClaimRound<= maxClaimRound; toClaimRound++) {
			...
			for(uint256 j=myGrade-1; j>=0; j--) {
				uint256 totalCount = thisRound.countPerGrade[j];
				reward += thisRound.normalDists[j] / totalCount;
			}
		...
		if (reward > 0) {
			mbtc.safeTransfer(msg.sender, reward);
		}
	}
