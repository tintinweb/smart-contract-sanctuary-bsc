...
contract BandZaiView is Ownable {
    ...
    function getNextEggDatas(uint256 _nursId) external view returns(
        ...
        ZaiStruct.EggsPrices memory _prices = Inurs.getEggsPrices(_nursId);

        ZaiStruct.MintedData memory _mintedDatas = Inurs.getNurseryMintedDatas(_nursId);
    ...
    function getFullZai(uint256 _tokenId) external view returns(FullZai memory){
        FullZai memory z;
        IZaiNFT INFT = IZaiNFT(gameAddresses.getZaiAddress());
        z.zai = INFT.getZai(_tokenId);
        z.owner = INFT.ownerOf(_tokenId);
        z.currentLevelUpPoint = INFT.getNextLevelUpPoints(z.zai.level);
        z.nextLevelUpPoint = INFT.getNextLevelUpPoints(z.zai.level + 1);
        IFighting IFight = IFighting(gameAddresses.getFightAddress());
        z.stamina = IFight.getZaiStamina(_tokenId);
        z.lastTrain = ITrainingManagement(gameAddresses.getTrainingCenterAddress()).getZaiLastTrainBegining(_tokenId);
        z.numberOfWinThisDay = IFight.getDayWinByZai(_tokenId);
        z.stats = IStats(gameAddresses.getZaiStatsAddress()).getZaiStats(_tokenId);
        z.delegationData = IDelegate(gameAddresses.getDelegateZaiAddress()).getDelegateDatasByZai(_tokenId);
    ...
    function getMyLabs(address _address) external view returns(LaboDatas[] memory){
        ILaboratory LabNFT = ILaboratory(gameAddresses.getLaboratoryNFTAddress());
        ILabManagement LabManagement = ILabManagement(gameAddresses.getLaboratoryAddress());
        IOpenAndClose openAndClose = IOpenAndClose(gameAddresses.getOpenAndCloseAddress());
    ...
    function getLabSlots(uint256 _labId) external view returns(LabSlot[] memory){
        ILaboratory LabNFT = ILaboratory(gameAddresses.getLaboratoryNFTAddress());
        ILabManagement LabManagement = ILabManagement(gameAddresses.getLaboratoryAddress());
        ...
        for (uint256 i = 1 ; i <= slotsNumbers ; i++){
            ZaiStruct.WorkInstance memory W = LabManagement.workingSpot(_labId,i);
            LabSlot memory L;
    ...
    function getMyNurseries(address _address) external view returns(Nursery[]memory){
        INursery INurs = INursery(gameAddresses.getNurseryAddress());
    ...
    function _getNurseryById(uint256 _tokenId) internal view returns(Nursery memory){
        Nursery memory n;
        IOpenAndClose IOpen = IOpenAndClose(gameAddresses.getOpenAndCloseAddress());
        INursery INurs = INursery(gameAddresses.getNurseryAddress());
        n.nursId = _tokenId;
        n.nursName = IOpen.getNurseryName(_tokenId);
        n.nursStatus = "open";
        n.nursProfits = INurs.nurseryRevenues(_tokenId);
        n.mintedDatas = INurs.nurseryMintedDatas(_tokenId);
        n.nextStatusToMint = INurs.nextStateToMint(_tokenId);
        n.nextUnlock = INurs.getNextUnlock(_tokenId);
        n.eggsPrices = INurs.getEggsPrices(_tokenId);
    ...
    function getPotionsInSale(uint256 _page) external view returns(PotionStruct.Potion[] memory){
        IPotions potionContract = IPotions(gameAddresses.getPotionAddress());
        address laboManagementAddress = gameAddresses.getLaboratoryAddress();
        ...
        PotionStruct.Potion[] memory potions = new PotionStruct.Potion[](endOfRange - startingRange);

        for(uint256 i = startingRange; i < endOfRange ; ){
            uint256 potionId = potionContract.tokenOfOwnerByIndex(laboManagementAddress,i);
            PotionStruct.Potion memory p = potionContract.getFullPotion(potionId);
