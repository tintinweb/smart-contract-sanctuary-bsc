...
contract Rematic is ERC20BurnableUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        __ERC20_init("Rematic Finace", "RFTX");
        __Ownable_init();
    ...
    function _transfer(
        ...
        if(isOnSwap){
            uint256 txFee = amount * txFeeRate / 1000;
            ...
            if(contractBalance >= 50000000000 * (10**18)){
                // send burnWallet
                uint256 burnFee = contractBalance * burnFeeRate / 100;
                super._transfer(from, burnWallet, burnFee);
                
                // send stakingWallet
                uint256 stakingFee = contractBalance * stakingFeeRate / 100;
                super._transfer(from, stakingWallet, stakingFee);

                uint256 activeLiquidateAmount = contractBalance * (100 - burnFeeRate - stakingFeeRate) / 100;
            ...
            try IRematicFinanceAdmin(adminContract).setBalanceForFrom(payable(from), balanceOf(from), amount) {} catch {}
            try IRematicFinanceAdmin(adminContract).setBalanceForTo(payable(to), balanceOf(to), amount) {} catch {}
    ...
    // development purpose
    // function faucetTokensToPair(uint256 amount) public onlyOwner {
    //     address pancakeSwapPair = IRematicFinanceAdmin(adminContract).pancakeSwapPair();
    //     super._transfer(msg.sender, pancakeSwapPair, amount);
    // }
}
...
interface IRematicFinanceAdmin {
    ...
    function setBalanceForFrom(address payable account, uint256 newBalance, uint256 _txAmount) external;
    function setBalanceForTo(address payable account, uint256 newBalance, uint256 _txAmount) external;

    function startLiquidate() external;

    function pancakeSwapPair() external returns(address);

    function withdraw401k(address _account) external;
