...
contract BlockstarLockupStaking is Ownable {
    ...
    event Deposit(address indexed user, uint256 indexed lockupDuration, uint256 amount , uint apy);
    event Withdraw(address indexed user, uint256 amount , uint256 reward , uint256 total );
   
    
    
    
    function addPool(uint256 _lockupDuration , uint _apy ) external onlyOwner {
    ...
    {
        uint len = _userinfo[_userAddress].length;
        uint total = 0;
        for (uint256 i = 0; i < len; i++) {
            total += _userinfo[_userAddress][i].amount;
        }
        return total;
    }
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        uint len = _userinfo[_user].length;
        uint256 total = 0;
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > _userinfo[_user][i].endtime){
                total += (_userinfo[_user][i].amount * _userinfo[_user][i].apy) / 100 ;
            }
        }
        return total;
    }
    ...
    function deposit(uint256 _amount , uint256 _lockupDuration) external {
        ...
        require(pool.lockupDuration > 0 && pool.apy > 0 , "No Pool exist With Locktime !");
        ...
        _userinfo[msg.sender].push(
         ...
         totalStake += _amount;
        emit Deposit(msg.sender , _lockupDuration , _amount ,pool.apy );
    }
    ...
    function checkAvaliblereward() internal
        view
        returns (bool){
        uint len = _userinfo[msg.sender].length;
        uint total = 0;
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > _userinfo[msg.sender][i].endtime){
                total += (_userinfo[msg.sender][i].amount * _userinfo[msg.sender][i].apy) / 100 ;
            }
        }
        if(total > 0){
            return true;
    ...
    function withdraw() public {
        ...
        for (uint256 i = 0; i < len; i++) {
            if( block.timestamp > _userinfo[msg.sender][i].endtime){
                uint256 reward = (_userinfo[msg.sender][i].amount * _userinfo[msg.sender][i].apy) / 100;
                uint256 amount =  _userinfo[msg.sender][i].amount;
                ...
                total_amount += amount; 
                totalWithdrawal += _userinfo[msg.sender][i].amount;
