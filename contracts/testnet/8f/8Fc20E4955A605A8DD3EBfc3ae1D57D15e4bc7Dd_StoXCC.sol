...
contract StoXCC is AccessControlEnumerable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_router != address(0), "Empty address error.");
        router = _router;
    ...
    function switchTokenHalving(
        ...
        require(_token != ethToUsdPath[0], "Can't halve WETH.");
        require(_token == _tokenToUsdPath[0], "Wrong token to USD path.");
        require(_tokenToUsdPath[_tokenToUsdPath.length - 1] == ethToUsdPath[1], "Should be path to USD.");
        bytes32 hash = keccak256(
            abi.encodePacked(
                proposalIndex,
                _token,
