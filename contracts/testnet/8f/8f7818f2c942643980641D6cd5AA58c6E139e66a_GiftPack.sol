...
contract UserBase is Ownable {
    ...
    function _updateUserExp(address _userAddress, uint256 _exp) internal {
        ...
        bool _isUserLevelUp = false;
        uint8 _userCurrentLevel = _user.level;
        uint8 _userNextLevel = _user.level;
        ...
        while (
            ...
            _isUserLevelUp = true;
            _userNextLevel = _user.level;
        }

        if (_isUserLevelUp) {
            emit UserLevelUp(_userAddress, _userCurrentLevel, _userNextLevel);
        }
    }
    ...
    function createUser(address _owner, string memory name)
        external
        onlyOwner
        returns (User memory)
    {
        User memory user = User(name, START_LEVEL, START_EXP);

        users.push(user);

        emit UserCreated(_owner, name, START_LEVEL);
