...
contract DV2Token is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    function updateMultiplier(address from) public returns (bool allow) {
      ...
      uint256 currentSellTargetPrice = checkMultiplier().mul(sellTargetPrice).div(WAD);
      if(isMultiEnabled){
        if(sellprice <= currentSellTargetPrice && rebaseswitch && from==_msgSender()){
            rebaseswitch=false;
            ...
            editBasePrice(basePrice,checkMultiplier().mul(basePrice).div(WAD));
            editSellPrice(sellTargetPrice,checkMultiplier().mul(sellTargetPrice).div(WAD));
        }
