...
contract InvestorsDAO is BaseDAO, IDAOInvestors {
    ...
    constructor(address[] memory _treasuries, address _mf, address _usdt) {
    ...
    function updateVestingWithdrawnReadyAmount(
        ...
        for (uint256 i; i < investor.vestings.length; i = _unsafe_inc(i)) {
            ...
            if (progress > 0) {
                uint256 vestingAvailableAmount = (investor.vestings[i].amount *
                    progress) /
                    decimals -
    ...
    function getProfit(address _investor) external view returns (uint256) {
        Investor memory investor = investors[_investor];
    ...
    function getInvestorVesting(
        address _investor
    ) external view returns (VestingWithProgress[] memory) {
