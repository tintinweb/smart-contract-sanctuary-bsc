...
contract GYM is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public MAX_AMOUNT_PER_ADDRESS = MAX_SUPPLY;
    uint256 public PRICE_USD = 1 ether;
    ...
    uint256[] public refPercents = [5, 5]; // 10%
    ...
    constructor(
        ...
        uint256 priceUSD, // ether format (1e18)
    ...
    function rewardingBNB(address user, uint256 totalPrice) internal {
        address papa1 = fathers[user];
        address papa2 = fathers[papa1];
        if (papa1 != address(0)) {
            uint256 bonus1 = (totalPrice / 100) * refPercents[0];
            refund(papa1, bonus1);
        }
        if (papa2 != address(0)) {
            refund(papa2, (totalPrice / 100) * refPercents[1]);
        }
    }
    ...
    function addRef(address user, address ref) internal {
        if (ref != user) {
            if (fathers[user] == address(0)) {
                ...
                if ((papa1 != user) && (papa1 != address(0))) {
                    levels[papa1].lvl1 += 1;
                }
                if ((papa2 != user) && (papa2 != address(0))) {
                    levels[papa2].lvl2 += 1;
                }
            }
        }
    ...
    function getBNBperNFT(uint256 count) public view returns (uint256) {
        return getBNBper1USD()/(PRICE_USD/1e15) * count * 1e3;
    }
    function refund(address to, uint256 val) internal {
        if (val > 0) {
            (bool success, ) = payable(to).call{value:val}("");
            require(success);
        }
    }
    function buyForUSD(address to,uint128 mintAmount,address ref ) public nonReentrant cooldown {
        uint256 priceBUSD = getBUSDPerNFT(mintAmount);
        mint(to, mintAmount);
        addRef(to, ref);

        address papa1 = fathers[to];
        address papa2 = fathers[papa1];
        uint256 amount1 = 0;
        uint256 amount2 = 0;
        if (papa1 != address(0)) {
            amount1 = (priceBUSD / 100) * refPercents[0];
            _tokenBUSD.transferFrom(msg.sender, papa1, amount1);
        }
        if (papa2 != address(0)) {
            amount2 = (priceBUSD / 100) * refPercents[1];
            _tokenBUSD.transferFrom(msg.sender, papa2, amount2);
        }

        _tokenBUSD.transferFrom(
            msg.sender,
            address(this),
            priceBUSD - amount1 - amount2
        );
    ...
    function buyForBNB(
        ...
        uint256 mintAmount,
        address ref
    ) external payable cooldown nonReentrant {
        require(mintAmount>0);
        uint256 priceBNB = getBNBperNFT(mintAmount);
        require(msg.value >= priceBNB, "insuffucient value");
        mint(to, mintAmount);
        refund(msg.sender, msg.value - priceBNB);
        addRef(to, ref);
        rewardingBNB(to, priceBNB);
    }

    function mint(address to, uint256 mintAmount) internal whenNotPaused {
        uint256 supply = totalSupply();
        require(mintAmount > 0, "cant mint 0 tokens");
        require(mintAmount <= MAX_MINT_AMOUNT, "max mint reached");
        require(supply + mintAmount <= MAX_SUPPLY, "max supply reached");
        require(
            balanceOf(to) + mintAmount <= MAX_AMOUNT_PER_ADDRESS,
            "limit exhausted"
        );
    ...
    function setPriceUSD(uint256 newPrice) external onlyOwner {
        require(newPrice > 1e15, "Please send ether format");
        PRICE_USD = newPrice;
    }
    ...
    function setMaxMintAmount(uint32 newMaxMintAmount) external onlyOwner {
    ...
    function setMaxAmountPerAddress(
        ...
        MAX_AMOUNT_PER_ADDRESS = newMaxAmountPerAddress;
    }
