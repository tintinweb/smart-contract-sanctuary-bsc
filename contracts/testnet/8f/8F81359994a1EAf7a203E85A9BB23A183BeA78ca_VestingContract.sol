...
contract VestingContract is Ownable, ReentrancyGuard {
	...
	IERC20 public token;
	uint256 public constant END_TIME = 1662210000;		//	Sep 3rd, 2022 01:00PM UTC
    ...
    */
	function claim() external nonReentrant {
		...
		require(_amounts.length != 0, "Beneficiary not existed");

		uint256 _now = block.timestamp;
		uint256 _next = policies[_beneficiary].next;
		uint256[] memory _schedules = schedules;
		require(_schedules[_next] <= _now, "Too early");

		uint256 _totalAmt;
		for(_next; _schedules[_next] <= _now; _next++) {
			_totalAmt = _totalAmt + _amounts[_next];
			_amounts[_next] = 0;
			if (_next + 1 == _schedules.length) break;
		}
		require(_totalAmt != 0, "Vesting completed. Please check your policy again");
		...
		_releaseTokenTo(_beneficiary, _now, _totalAmt);
	}
