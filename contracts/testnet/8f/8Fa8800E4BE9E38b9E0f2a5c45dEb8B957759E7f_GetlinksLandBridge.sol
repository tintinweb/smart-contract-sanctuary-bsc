...
// File: contracts/getlinks_land_bridge.sol
...
//custom interface
  ...
  contract GetlinksLandBridge is
    EIP712,
    AccessControlEnumerable,
    Pausable
{
    ...
    function getBalance() public view returns (uint256)  {
        return IERC20(linksToken).balanceOf(address(this));
    }
    ...
    function mint(
        ...
        bytes32 digest = _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "mint(address to,uint256 tokenId,uint256 price,uint256 nonce,uint256 deadline)"
                        ),
                        to,
                        tokenId,
                        price,
        ...
        require(!isConfirmed[signer][nonce], "signer already confirmed"); 
        isConfirmed[signer][nonce] = true;

        //mint nft
        nft.mint(to, tokenId);
                
        //mint cost
        IERC20(linksToken).safeTransferFrom(_msgSender(), address(this), price);
