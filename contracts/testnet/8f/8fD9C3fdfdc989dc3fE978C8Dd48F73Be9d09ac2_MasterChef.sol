...
// File: contracts/Farm/ERC20Token.sol
...
contract ERC20Token is ERC20("AfriPayX", "AFX"), Ownable {
...
// File: contracts/Farm/SafeTransfer.sol
...
contract SafeTransfer{
    ...
    constructor(address _operator, IERC20 _token) public {
        operator = _operator;
        token = _token;
    }
    ...
    function safeRewardsTransfer(address _to, uint256 _amount) external {
        ...
        uint256 rewardTokenBal = token.balanceOf(address(this));
        if (_amount > rewardTokenBal) {
            token.transfer(_to, rewardTokenBal);
        } else {
...
contract MasterChef is Ownable {
    ...
    // Mapping LP token more than once.
    mapping(address => bool) public poolStatus;
    // Mapping pool id by LP token
    ...
    constructor(
        ...
        _safeTransfers = new SafeTransfer(address(this), _rewardToken);
    }

    // Modifiers
    modifier isPoolExist(uint256 pid){
    ...
    function add(
        ...
        require(poolStatus[address(_lpToken)] == false, "LP already added in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
        uint256 lastRewardBlock =
        ...
        poolStatus[address(_lpToken)] = true;
        poolId[address(_lpToken)] = poolInfo.length.sub(1);
    }
    ...
    // Return rewardToken multiplier over the given _from to _to block.
    ...
    // Update rewardToken vairables for all pools. Be careful of gas spending!
    ...
    // Update rewardToken variables of the given pool to be up-to-date.
    function updatePool(uint256 _pid) public isPoolExist(_pid) {
        ...
        rewardToken.mint(devaddr, reward.div(10));
        reward = reward.sub(reward.div(10));
        rewardToken.mint(address(_safeTransfers), reward);
    ...
    function deposit(uint256 _pid, uint256 _amount) public isPoolExist(_pid){
        ...
        if (user.amount > 0) {
            ...
            _safeTransfers.safeRewardsTransfer(msg.sender, pending);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public isPoolExist(_pid) {
        require(_amount > 0, "zero amount");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    function claimReward(uint256 _pid) public isPoolExist(_pid){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require((user.amount > 0), " Not User");
        updatePool(_pid);
        uint256 pending =
            user.amount.mul(pool.accRewardsPerShare).div(1e12).sub(
                user.rewardTokenDebt
    ...
    // Withdraw without caring about rewardTokens. EMERGENCY ONLY.
