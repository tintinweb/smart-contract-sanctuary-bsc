...
//    event CancelBidToken(address indexed bidder, uint256 indexed tokenId);
    ...
    function addCoins(address _address) public onlyOwner {
        // 判断是否存在
        require(!_coins.contains(_address), "coin exist");
        _coins.add(_address);
    ...
    function addNfts(address _address) public onlyOwner {
        // 判断是否存在
        require(!_nfts.contains(_address), "nft exist");
        _nfts.add(_address);
    ...
    function removeCoins(address _address) public onlyOwner {
        require(_coins.length() >= 1, "only one coin left");
        require(_coins.contains(_address), "coin not exist");
        _coins.remove(_address);
        emit UpdateCoins(msg.sender, getCoins());
    }
    // 移除可上架NFT
    function removeNfts(address _address) public onlyOwner {
        require(_nfts.length() >= 1, "only one nft left");
        require(_nfts.contains(_address), "coin not exist");
    ...
    function buyTokenTo(address _nftAddress, uint256 _tokenId, address _to) payable public whenNotPaused {
        ...
        if (_asksMapDataCoin[_nftAddress].get(_tokenId) == address(0)) {
            ...
            if(feeAmount != 0){
                ...
                // 返还多余的费用
                _msgSender().transfer(msg.value.sub(price));
            }
