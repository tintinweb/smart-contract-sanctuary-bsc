  ...
  UUPSUpgradeable
{
    ...
    onlyOwner
  {
    require(
      ...
      "INFLIV: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
    );
  ...
  function withdrawalTokenFromPartnersInvestorsPool(address to, uint256 amount)
    public
    onlyOwner
  {
    ...
    require(
      available >= amount,
      "INFLIV: Amount is not available for mint or amount is zero"
    );
  ...
  function availablePartnersInvestorsPoolTokens()
    public
    view
    returns (uint256)
  {
    if (block.timestamp >= partnersInvestorsPool.startTime) {
      uint256 available = ((block.timestamp - partnersInvestorsPool.startTime) /
        partnersInvestorsPool.timeInterval) *
        partnersInvestorsPool.unlockAmount;
  ...
  function availableListingsLiquidityPoolTokens()
    public
    view
    returns (uint256)
  {
    if (block.timestamp >= listingsLiquidityPool.startTime) {
      uint256 available = ((block.timestamp - listingsLiquidityPool.startTime) /
        listingsLiquidityPool.timeInterval) *
        listingsLiquidityPool.unlockAmount +
        listingsLiquidityPool.supplyFromGovernance;
      available = (available >
        listingsLiquidityPool.maxSupply +
          listingsLiquidityPool.supplyFromGovernance)
        ? listingsLiquidityPool.maxSupply +
          listingsLiquidityPool.supplyFromGovernance -
          listingsLiquidityPool.supplyMinted
  ...
  function withdrawalTokenFromBountyBonusesPool(address to, uint256 amount)
    public
  {
  ...
  function availableBountyBonusesPoolTokens() public view returns (uint256) {
    if (block.timestamp >= bountyBonusesPool.startTime) {
      uint256 available = ((block.timestamp - bountyBonusesPool.startTime) /
        bountyBonusesPool.timeInterval) *
        bountyBonusesPool.unlockAmount +
        bountyBonusesPool.supplyFromGovernance;

      available = (available >
        bountyBonusesPool.maxSupply + bountyBonusesPool.supplyFromGovernance)
        ? bountyBonusesPool.maxSupply +
          bountyBonusesPool.supplyFromGovernance -
          bountyBonusesPool.supplyMinted
  ...
  function withdrawalTokenFromTreasuryPool(address to, uint256 amount)
    public
    onlyOwner
  {
  ...
  function availableTreasuryPoolTokens() public view returns (uint256) {
    uint256 available = (block.number - treasuryPool.startingBlock) *
      treasuryPool.supplyPerBlock +
      treasuryPool.supplyFromGovernance;
    available = (available >
      treasuryPool.maxSupply + treasuryPool.supplyFromGovernance)
      ? treasuryPool.maxSupply +
        treasuryPool.supplyFromGovernance -
        treasuryPool.supplyMinted
      : available - treasuryPool.supplyMinted;
    return available;
  }

  function availableCommunityPoolTokens() public view returns (uint256) {
    uint256 available = (block.number - communityPool.startingBlock) *
      communityPool.supplyPerBlock +
      communityPool.supplyFromGovernance;
    available = (available >
      communityPool.maxSupply + communityPool.supplyFromGovernance)
      ? communityPool.maxSupply +
        communityPool.supplyFromGovernance -
        communityPool.supplyMinted
      : available - communityPool.supplyMinted;
    return available;
  }

  function availableTeamPoolTokens() public view returns (uint256) {
    uint256 available = (block.number - teamPool.startingBlock) *
      teamPool.supplyPerBlock +
      teamPool.supplyFromGovernance;
    available = (available > teamPool.maxSupply + teamPool.supplyFromGovernance)
      ? teamPool.maxSupply +
        teamPool.supplyFromGovernance -
        teamPool.supplyMinted
      : available - teamPool.supplyMinted;
    return available;
  }

  function withdrawalTokenFromInsurancePool(address to, uint256 amount)
    public
    onlyOwner
  {
    ...
    require(
      available >= amount && amount > 0,
      "INFLIV: Amount is not available for mint"
    );
  ...
  function withdrawalTokenFromCharityPool(address to, uint256 amount)
    public
    onlyOwner
  {
  ...
  function withdrawalTokenFromReservePool(address to, uint256 amount)
    public
    onlyOwner
  {
  ...
  function _transfer(
    ...
    if (
      block.timestamp >=
      listingsLiquidityPool.lastClaimTime +
        listingsLiquidityPool.claimInterval &&
      swapEnable &&
      !swapping &&
  ...
  function _updatePools() public {
    ...
    if (
      block.timestamp >= (teamPool.lastClaimTime + teamPool.claimInterval) &&
    ...
    if (
      block.timestamp >=
      (communityPool.lastClaimTime + communityPool.claimInterval) &&
      address(stakingAddress) != address(0) &&
      address(farmAddress) != address(0) &&
      CPAmount > 0
    ) {
  ...
  function _distributeFee(uint256 amount) private {
    uint256 communityPoolShare = (amount *
      governanceFeeShare.communityPoolShare) / 10000;
    uint256 liquidityPoolShare = (amount *
      governanceFeeShare.liquidityPoolShare) / 10000;
    uint256 treasuryPoolShare = (amount *
      governanceFeeShare.treasuryPoolShare) / 10000;
    uint256 bountyBonusesPoolShare = (amount *
      governanceFeeShare.bountyBonusesPool) / 10000;
    uint256 insurancePoolShare = (amount *
      governanceFeeShare.insurancePoolShare) / 10000;
    uint256 charityPoolShare = (amount * governanceFeeShare.charityPoolShare) /
      10000;
    uint256 teamPoolShare = (amount * governanceFeeShare.teamPoolShare) / 10000;
    uint256 reservePoolShare = (amount * governanceFeeShare.reservePoolShare) /
      10000;
