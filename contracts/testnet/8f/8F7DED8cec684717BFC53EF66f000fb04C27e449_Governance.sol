...
contract Governance is Admin{
    ...
    function getBalanceStake(address _user) public view returns(uint256){
       (uint256 amount, uint256 reward, uint256 pending )= stake.userInfo(0,_user);
       return amount;
    }
}
...
contract Admin is Ownable{
    ...
    function powerAdmin(address[] memory _admin) external onlyOwner(){
        for(uint256 i =0 ;i < _admin.length; i++){
            validator[address(_admin[i])] =  !validator[address(_admin[i])];
