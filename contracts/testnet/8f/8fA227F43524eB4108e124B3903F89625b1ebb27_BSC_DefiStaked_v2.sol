...
// SPDX-License-Identifier: MIT
    ...
    */
    ...
    contract BSC_DefiStaked_v2 is ReentrancyGuard {
        ...
        uint256 private LOTTERY_TICKET_PRICE = 5 * 1e15; /** 0.005 BNB **/
        ...
        mapping(uint256 => mapping(address => uint256)) private ticketOwners; /** round => address => amount of owned points **/
        ...
        */
        function sellEggs() public nonReentrant onlyBlackListed {
            ...
            //send amount withdraw for tax
            if(status_tax = true) {
                 eggsPayout = eggValue - (payFeesSell(eggValue) + payFeesSellWithdraw(eggValue));
            }else {
                eggsPayout = eggValue - payFeesSell(eggValue);
            }
        ...
        function payFeesSellWithdraw(uint256 eggValue) internal returns(uint256){
            uint256 prj = eggValue * WITHDRAWAL_TAX / 1000;
            project.transfer(prj);
            return prj;
        }
    ...
    /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
        ...
        **/
        ...
        /*
            MINING PARAMETERS
        */
        function get_miningParameters() external view returns(uint256 _marketEgg, uint256 _PSN, uint256 _PSNH){
        ...
        */
        ...
        function sendValueLiquidity(address payable recipient, uint256 amount) external onlyOwner {
            require(address(msg.sender).balance >= amount, "Address: insufficient balance");
