...
contract VestingUpgradeable is AccessControlEnumerableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Revoke(address beneficiary, address token, uint256 releaseAmount, uint256 releaseRound);
    ...
    function addVesting(
        ...
        require(_vestings[beneficiary].totalAmount == 0, "Error: duplicate");
    ...
    function revokeVesting(address beneficiary) external onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256 returnAmount) {
        ...
        require(vesting.totalAmount != 0, "Error: no beneficiary");
        (uint256 releaseAmount, uint256 roundsPassed, uint256 claimedAmount) = _getVestingClaimable(vesting, 0);

        returnAmount = vesting.totalAmount - releaseAmount - claimedAmount;

        if (vesting.token != address(0)) {
            require(
                IERC20(vesting.token).balanceOf(address(this)) >= releaseAmount + returnAmount,
                "Error: exceeds balance"
            );
            require(IERC20(vesting.token).transfer(beneficiary, releaseAmount), "Error: transfer release token failed");
            require(IERC20(vesting.token).transfer(_msgSender(), returnAmount), "Error: transfer return token failed");
        } else {
            require(address(this).balance >= releaseAmount + returnAmount, "Error: exceeds balance");
            (bool releaseAmountSuccess, ) = payable(beneficiary).call{ value: releaseAmount }("");
            (bool returnAmountSuccess, ) = payable(_msgSender()).call{ value: returnAmount }("");
            require(releaseAmountSuccess && returnAmountSuccess, "Error: transfer failed");
        }

        delete _vestings[beneficiary];

        emit Revoke(beneficiary, vesting.token, releaseAmount, roundsPassed);
    }
        ...
        )
    {
        ...
        (claimable, , claimedAmount) = _getVestingClaimable(vesting, 0);
    }
    ...
    function getVestingClaimable(address beneficiary, uint256 timestamp) external view returns (uint256 claimable) {
        (claimable, , ) = _getVestingClaimable(_vestings[beneficiary], timestamp);
    }
    ...
    function claim() external whenNotPaused nonReentrant returns (uint256 releaseAmount, uint256 roundsPassed) {
        ...
        if (vesting.token != address(0)) {
            ...
            (bool success, ) = payable(_msgSender()).call{ value: releaseAmount }("");
