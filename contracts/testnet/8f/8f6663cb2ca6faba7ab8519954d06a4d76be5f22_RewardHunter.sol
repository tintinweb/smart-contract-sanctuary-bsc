...
contract RewardHunter is ERC1155, Ownable {
    ...
    uint public _whitdrawbleToken;
    uint256  priceInRHT;
    bool public valueBNB = false;
    IBEP20 public token;
    address public wallet;


  event MintNfts(address indexed from, uint256 id, uint256 value);
  ...
  struct NftUsers {
    ...
    function mintNFT(uint256 _amount) external payable returns(uint256) {
      ...
      emit MintNfts(msg.sender, newTokenId, 0);
    ...
    function marketItem(uint _tokenId, uint _price) public   {
        ...
        require(_price > 2500, "RHT: Preco de venda em Token precisa ser maior do que zero");
    ...
    function buyNfts(uint _tokenId) public  {

      NftUsers storage ids = idToMarketItem[_tokenId];

          uint256 priceNftsToken = ids.priceToken;
          require(priceNftsToken == ids.priceToken, "RHT:Preco de compra precisa em Token ser indetico ao preco de venda");
          uint256 transferSeller = SafeMath.div(SafeMath.mul(priceNftsToken,10),100);
          uint256 divisorTax = SafeMath.sub(priceNftsToken,transferSeller);
           _whitdrawbleToken += transferSeller;
          IBEP20(token).transferFrom(msg.sender, ids.sellerNft, divisorTax);
    ...
    // Permite o Owner definir a forma de pegamento (Token ou BNB)
    function _setFormBuyAndSell(bool _status) external onlyOwner {
        valueBNB = _status;
    }
