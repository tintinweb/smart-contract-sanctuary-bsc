...
contract NanoMinions {
    ...
    INanoMinionsLottery NanoMinionsLottery;
    ...
    // Play lottery
    function participateLuckyLottery() external onlyNanoMinionsPlayer {
        ...
        uint useTime = nowWorksTime * LOTTERY_PRICE / nowPrice;
        lastWorking[msg.sender] += useTime;

        uint poolBalance = BUSD.balanceOf(address(this)) - LOTTERY_PRICE;
        marketRobot += calculateRobotClone(LOTTERY_PRICE, poolBalance);

        BUSD.transfer(address(NanoMinionsLottery), LOTTERY_PRICE);
        NanoMinionsLottery.participate(msg.sender);
    ...
    function setNanoMinionsLotteryContact(address _contractAddress) external onlyOwner {
        NanoMinionsLottery = INanoMinionsLottery(_contractAddress);
    }
