...
contract Token is ERC20, Ownable {
    ...
    uint256 private totalSupply_ = 10000 * 10**_decimals;

    IUniswapV2Router02 public uniswapV2Router;
    bool private swapping;

    uint256 public blackAmount;

    address public blackAddress = 0x000000000000000000000000000000000000dEaD;
    address public swapToken = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    // address public swapToken = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public lpRewardToken = 0xdA32631D0EFf3718cA948EbDAf8990fd63896450;
    ...
    event addSwap(address from, address to, bool res);
    event delSwap(address from, address to, bool res);

    event tadd(address token0, address token1, uint r0, uint r1, uint bal0, uint bal1, uint itype);
    event tdel(address token0, address token1, uint r0, uint r1, uint bal0, uint bal1, uint itype);

    constructor() ERC20(name_, symbol_)  {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(swapToken);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(lpRewardToken, address(this));
    ...
    function takeAllFee(address from, address to, uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(_marketPairs[from]) {
            bool ldxDel = _isDelLiquidityV1();
            if (ldxDel) {
                blackAmount = amount.mul(10).div(100);
                super._transfer(from, blackAddress, blackAmount);
                amountAfter = amount.sub(blackAmount);
            }
            emit delSwap(from, to, ldxDel);
        } else {
            bool ldxAdd = _isAddLiquidityV1();
            if (ldxAdd) {
                blackAmount = amount.mul(1).div(100);
                super._transfer(from, blackAddress, blackAmount);
                amountAfter = amount.sub(blackAmount);
            }
            emit addSwap(from, to, ldxAdd);
        }
    }

    function _isAddLiquidityV1()internal returns(bool ldxAdd){
        ...
        if( token0 == address(this) ){
            emit tadd(token0, token1, r0, r1, bal0, bal1, 0);
			if( bal1 > r1){
				uint change1 = bal1 - r1;
				ldxAdd = change1 > 1000;
			}
		}else{
            emit tadd(token0, token1, r0, r1, bal0, bal1, 1);
			if( bal0 > r0){
	...
	function _isDelLiquidityV1() internal returns(bool ldxDel){
        ...
        uint bal0 = IERC20(token0).balanceOf(address(uniswapV2Pair));
        if( token0 == address(this) ){
            emit tdel(token0, token1, r0, r1, bal0, bal1, 0);
			if( bal1 < r1){
				uint change1 = r1 - bal1;
				ldxDel = change1 > 1000;
			}
		}else{
            emit tdel(token0, token1, r0, r1, bal0, bal1, 1);
			if( bal0 < r0){
