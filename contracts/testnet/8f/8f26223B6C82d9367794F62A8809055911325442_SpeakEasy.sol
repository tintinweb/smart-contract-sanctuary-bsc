...
contract SpeakEasy is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        __Ownable_init();
        SILVER_ADDRESS = 0x93EDbf43c56241D21E020327069004475E00EcD8; // testnet
        EMP_SILVER_ADDRESS = 0x15E03A44C859C6E6819E5Be86390Da6366C6a0AA; // testnet
        GOLD_ADDRESS = address(0); // testnet
        PLATINUM_ADDRESS = address(0); // testnet
        GT_ADDRESS = 0xFd0a77395e4ed2b7236B451fB1bb08a26166c100; // testnet
        MARGARITA_ADDRESS = 0x6ea722eA09bBD508f7F39ac0A9fc2260645800e3; // testnet
        BLOODYMARY_ADDRESS = 0x737346B5a531C39bD33169677869339479b9806D; // testnet
        PINACOLADA_ADDRESS = 0x2949e6365805B5AF2cBC902357B71567AC7D3744; // testnet
        IRISHCOFFEE_ADDRESS = 0x1470d474F07dc4BEa8Aaf4a5082BC01AfD7f6797; // testnet
        OLDFASHIONED_ADDRESS = 0x37E74e6Fc4A2B10c56cA8AdA94fCc91DcB6eAacA; // testnet
        MINTING_CONTRACT_ADDRESS = 0xc9D83B652AC3379F4BeFED9442e793788a8351bE; // testnet
    ...
    function setInvestorsCap(bool enable, uint256 cap)
        public
        onlyTeam
        returns (bool enabled, uint256 capSize)
    {
    ...
    function deposit(address ref) public payable nonReentrant {
        ...
        require(
            investorsCapEnabled == false || (investorsCapEnabled && totalUsers < investorsCap),
    ...
    function reinvest() public payable nonReentrant {
        ...
        uint256 potFee = percentFromAmount(bnbRewardsBeforeFee, REINVEST_POT_FEE);
        uint256 devFee = percentFromAmount(potFee, DEV_FEE);
        potFee = potFee.sub(devFee);
        ...
        signer.reinvestBonusReward = Math.add(
            signer.reinvestBonusReward,
            10000
        ); // adding 0.01% to daily reward
    ...
    function referrals(address adr)
        public
        view
        returns (address[] memory downlines)
    {
    ...
    function dailyRewards(address adr) public view returns (uint256) {
        uint256 daily = rewardsFromNFTs(adr);
        return Math.min(1500000, daily);
    }

    function rewardsFromNFTs(address adr) public view returns (uint256) {
        if (hasMembership(adr) == false)
            return Math.add(250000, users[adr].reinvestBonusReward); // 0.25% + reinvest bonus
        if (_oldFashionedNft.balanceOf(adr) > 0)
            return Math.add(1500000, users[adr].reinvestBonusReward); // 1.5% + reinvest bonus
        if (_irishCoffeeNft.balanceOf(adr) > 0)
            return Math.add(1300000, users[adr].reinvestBonusReward); // 1.3% + reinvest bonus
        if (_pinaColadaNft.balanceOf(adr) > 0)
            return Math.add(1150000, users[adr].reinvestBonusReward); // 1.15% + reinvest bonus
        if (_bloodyMaryNft.balanceOf(adr) > 0)
            return Math.add(1000000, users[adr].reinvestBonusReward); // 1% + reinvest bonus
        if (_margaritaNft.balanceOf(adr) > 0)
            return Math.add(850000, users[adr].reinvestBonusReward); // 0.85% + reinvest bonus
        if (_ginAndTonicNft.balanceOf(adr) > 0)
            return Math.add(750000, users[adr].reinvestBonusReward); // 0.75% + reinvest bonus
        // if (_goldMKCNFT.balanceOf(adr) > 0)
        //     return Math.add(750000, users[adr].reinvestBonusReward); // 0.75% + reinvest bonus
        // if (_platinumMKCNFT.balanceOf(adr) > 0)
        //     return Math.add(1500000, users[adr].reinvestBonusReward); // 1.5% + reinvest bonus

        return Math.add(500000, users[adr].reinvestBonusReward); // 0.5% + reinvest bonus
    }
