...
contract NFTMasterCompound is Ownable, ReentrancyGuard {
    ...
    /* group */
    event AddGroupInfo(address _user, uint256 _groupId, address[] _inputToken, address[] _outputToken, uint256[] _outputTokenAmount, address _feeToken, uint256 _feeAmount, bool _openStatus, uint256 _at);
    event SetGroupInfo(address _user, uint256 _groupId, address[] _inputToken, address[] _outputToken, uint256[] _outputTokenAmount, address _feeToken, uint256 _feeAmount, bool _openStatus, uint256 _at);
    ...
    // add group info
    function addGroupInfo(address[] memory _inputToken, address[] memory _outputToken, uint256[] memory _outputTokenAmount, address _feeToken, uint256 _feeAmount, bool _openStatus) public nonReentrant onlyAdmin {
        ...
        groupInfoList[newGroupId] = GroupInfo(_inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, _openStatus);

        emit AddGroupInfo(msg.sender, newGroupId, _inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, _openStatus, block.timestamp);
    }

    // set group info
    function setGroupInfo(uint256 _groupId, address[] memory _inputToken, address[] memory _outputToken, uint256[] memory _outputTokenAmount, address _feeToken, uint256 _feeAmount, bool _openStatus) public nonReentrant onlyAdmin {
        ...
        groupInfoList[_groupId] = GroupInfo(_inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, _openStatus);

        emit SetGroupInfo(msg.sender, _groupId, _inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, _openStatus, block.timestamp);
    }
    ...
    // get group info
    function getGroupInfo(uint256 _groupId) public view returns (address[] memory, address[] memory, uint256[] memory, address, uint256, address, bool) {
        ...
        address _feeToken = groupInfoList[_groupId].FeeToken;
        uint256 _feeAmount = groupInfoList[_groupId].FeeAmount;
        bool _openStatus = groupInfoList[_groupId].OpenStatus;
        return (_inputToken, _outputToken, _outputTokenAmount, _feeToken, _feeAmount, feeTo, _openStatus);
    }
