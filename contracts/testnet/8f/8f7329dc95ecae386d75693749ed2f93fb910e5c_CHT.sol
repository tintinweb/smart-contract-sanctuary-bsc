...
contract CHT is ERC20, Ownable {
    ...
    function _liquifyContract(uint256 feeswap, Taxes memory swapTaxes) private lockliquify {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            IERC20 L = IERC20(liquidityPairToken);
            uint256 initialBalance = L.balanceOf(address(this));

            swapTokensForETH(toSwap);

            uint256 deltaBalance = L.balanceOf(address(this)) - initialBalance;
            ...
            if (marketingAmt > 0) {
                L.transferFrom(address(this), marketingWallet, marketingAmt);
            }

            uint256 buybackAmt = unitBalance * 2 * swapTaxes.buyback;
            if (buybackAmt > 0) {
                L.transferFrom(address(this), buybackWallet, buybackAmt);
            }

            uint256 devAmt = unitBalance * 2 * swapTaxes.dev;
            if (devAmt > 0) {
                L.transferFrom(address(this), devWallet, devAmt);
            }
