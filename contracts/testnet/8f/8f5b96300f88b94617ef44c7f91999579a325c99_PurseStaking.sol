...
contract PurseStaking is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function enter(uint256 purseAmount) external whenNotPaused returns (bool success) {
        ...
        userInfo[msg.sender].stakeAmount += purseAmount;
    ...
    function leave(uint256 xPurseAmount) external whenNotPaused returns (bool success){
        ...
        if(user.receiptToken <= 0) {
            ...
            noPurseReward = xPurseAmount.mul(user.stakeAmount).div(user.newReceiptToken);
            user.newReceiptToken -= xPurseAmount;
            handleReceipt(purseReward, noPurseReward);
        } else {
            if(xPurseAmount > user.receiptToken) {
                ...
                noPurseReward = newReceipt.mul(user.stakeAmount).div(user.newReceiptToken);
    ...
    function handleReceipt(uint256 _purseReward, uint256 _noPurseReward) internal {
        UserInfo storage user = userInfo[msg.sender];
        user.stakeAmount -= _noPurseReward;
        user.noWithdrawReward += _noPurseReward;                   
    ...
    function withdrawLockedAmount() public whenNotPaused returns (bool success){
        UserInfo storage user = userInfo[msg.sender];
        require(user.noWithdrawReward > 0 && user.withdrawReward > 0, "Insufficient Withdrawal Amount");
        uint256 lockDuration = block.timestamp.sub(user.lockTime); 
        uint256 withdrawAmount;
        if (lockDuration >= lockPeriod && user.lockTime != 0) {
            withdrawAmount = user.withdrawReward;
        } else {
            withdrawAmount = user.noWithdrawReward;
        }
         _totalLockedAmount -= user.withdrawReward;
        user.withdrawReward = 0;
        user.noWithdrawReward = 0;
        user.lockTime = 0;

        uint256 purseTransfer = safePurseTransfer(withdrawAmount);
        emit WithdrawNewStake(msg.sender, purseTransfer);
        purseToken.transfer(msg.sender, purseTransfer);
        return true;
    }

    function enterOldStake(uint256 purseAmount) external whenNotPaused returns (bool success) {
        require(purseToken.balanceOf(msg.sender) >= purseAmount, "Insufficient Purse Token");
