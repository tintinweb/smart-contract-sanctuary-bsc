...
contract Paragon is Context, IERC20, Ownable {
    ...
    function getCostBasis(address atAddress) public view returns (int256) {
        return _costBasis[atAddress];
    }

    function setCostBasis(address atAddress, int256 toAmount) internal {
        _costBasis[atAddress] = toAmount;
    }

    function pegRate() public view returns (uint256) {
        return _pegRate;
    }

    function _setPegRate(uint256 toRate) internal virtual {
        _pegRate = toRate;
    }
    ...
    function _transfer(
        ...
        //update cost basis
        int256 basisDelta =int256(amount.div(divisor()).div(pegRate()));
        setCostBasis(from, getCostBasis(from) - basisDelta);
        setCostBasis(to,   getCostBasis(to)   + basisDelta);
...
contract ParagonSwap is Paragon {
    ...
    uint256 public constant RES_FEE         = 330; //<---------------- 33  percent
    uint256 public feeBuy  = 75; //<---------------------------------- 7.5 percent
    uint256 public feeSell = 75; //<---------------------------------- 7.5 percent

    //rebases
    uint256 public rewardYield           = 100; //<------------------- 1 percent
    ...
    //buy cap
    uint256 public spendPerEpoch = 25000 ether; //<------------------- 25000 BUSD
    ...
    //historical state
    struct Epoch {
        uint256 historical_peg;
        uint256 historical_price;
        uint256 historical_supply;
        uint256 historical_liquidity;
        uint256 historical_holders;
    }

    //mappings
    mapping(uint256 => Epoch)   public epochs; //<-------------------- stores History at every epoch
    ...
    address public adminRecovery;

    //admins
    address[] public admins;

    //TESTNET ONLY ///////////////////////////////
    function deleteContract() external onlyAdmin {
    ...
    constructor(
        ...
        address _ops,
        address _liquidityXPG,
        address _liquitidyBUSD,
        address _marketing,
        address _admin,
        address _adminRecovery)
    {
        dev           =  msg.sender;
        ops           = _ops; //<-------------------------------------- initialize ops wallet
        liquidityXPG  = _liquidityXPG; //<----------------------------- initialize liquidity wallet
        liquidityBUSD = _liquitidyBUSD; //<---------------------------- initialize liquidity wallet
        marketing     = _marketing; //<-------------------------------- initialize marketing wallet
        admin         = _admin; //<------------------------------------ initialize admin wallet
        adminRecovery = _adminRecovery; //<---------------------------- initialize admin recovery wallet
        ...
        _setPegRate(100); //<----------------------------------------- initialize peg rate

        taxExemptions[dev] = true; //<-------------------------------- add dev to tax exemptions
        taxExemptions[ops] = true; //<-------------------------------- add ops to tax exemptions
        taxExemptions[liquidityXPG] = true; //<----------------------- add liquidityXPG to tax exemptions
        taxExemptions[liquidityBUSD]= true; //<----------------------- add liquidityBUSD to tax exemptions

        admins.push(dev); //<----------------------------------------- add dev to admins
        admins.push(admin); //<--------------------------------------- add admin to admins

        Epoch storage epoch    = epochs[currentEpoch]; //<------------ log epoch data
        epoch.historical_peg   = pegRate();
        epoch.historical_price = uint256(1 ether).div(pegRate());
    }
    ...
    modifier onlyAdminRecovery() {
        require(msg.sender == adminRecovery, "Not authorized.");
        _;
    }

    function buyTokens(uint256 amount, address referrer) external {
        require(enableBuy, "Buy is disabled");
        ...
        uint256 tokens     = amount.mul(divisor()).mul(pegRate());
        uint256 devFee     = amount.mul(DEV_FEE).div(FEE_DENOMINATOR);
        uint256 serviceFee = amount.mul(feeBuy).div(FEE_DENOMINATOR);
        uint256 sumFees    = devFee.add(serviceFee);
        ...
        if(isTaxExempt(buyer) == false) {
            (uint256 opsFee, uint256 liquidityFee, uint256 marketingFee, uint256 referralBonus) = distribute(serviceFee);
        ...
        int256 newCostBasis = getCostBasis(buyer) + int256(amount);
        setCostBasis(buyer, newCostBasis);
        _mint(buyer, liquidityXPG, tokens);
    }

    function sellTokens(uint256 amount) external {
        require(enableSell, "Sell is disabled");
        ...
        uint256 reserveFee = busdAmount.mul(RES_FEE).div(FEE_DENOMINATOR);
        uint256 serviceFee = busdAmount.mul(feeSell).div(FEE_DENOMINATOR);
        ...
        uint256 sumFees = devFee.add(reserveFee).add(serviceFee);
        ...
        uint256 willReceive = busdAmount.sub(sumFees);

        int256 newCostBasis = getCostBasis(seller) - int256(willReceive);
        setCostBasis(seller, newCostBasis);
        IERC20(busdAddress).transfer(payable(seller), willReceive);
    }
    ...
    function coreRebase(uint256 supplyDelta) private {
        ...
        uint256 liquidity = balanceOf(liquidityXPG).div(pegRate());
        liquidity         = liquidity.add(IERC20(busdAddress).balanceOf(liquidityBUSD));
        liquidity         = liquidity.add(IERC20(busdAddress).balanceOf(address(this)));

        Epoch storage epoch        = epochs[currentEpoch];
        epoch.historical_peg       = pegRate();
        epoch.historical_price     = uint256(1 ether).div(pegRate());
        epoch.historical_supply    = totalSupply();
        epoch.historical_holders   = holders;
        epoch.historical_liquidity = liquidity;
    ...
    function minSupply() public view returns (bool) {
    ...
    function isAdmin(address atAddress) public view returns (bool) {
        bool result;
        for (uint256 i = 0; i < admins.length; i++) {
            if(admins[i] == atAddress) {
                result = true;
            }
        }
        return result;
    ...
    function getEpoch(uint256 atIndex) public view returns (Epoch memory) {
        return epochs[atIndex];
    }

    //admin panel //////////////////////////////////////////////////////////
    function setEnableSwap(bool _enableBuy, bool _enableSell) external onlyAdmin {
    ...
    function setRewardYield(uint256 _rewardYield) external onlyAdmin {
        require(_rewardYield <= 500, "Invalid parameter: _rewardYield");
    ...
    function setPegRate(uint256 toRate) external onlyAdmin {
        _setPegRate(toRate);
    }
    ...
    function setMinReserve(uint256 toPercent) external onlyAdmin {
        require(toPercent >= 2000, "Invalid parameter: toPercent");
    ...
    function manualRebaseOverride() external onlyAdmin {
        rebase();
    }
    function addAdmin(address atAddress) external onlyAdminRecovery {
        admins.push(atAddress);
    }
    function removeAdmin(uint256 atIndex) external onlyAdminRecovery {
        admins[atIndex] = admins[admins.length - 1];
        admins.pop();
    }
    ////////////////////////////////////////////////////////////////////////
}