...
contract BuyGFT is Ownable {
    ...
    event ChangeInAdminAddress(address newAdminAddress); 
    event DoneWithdraw(uint256 withdrawAmount);

    constructor(uint256 _tokenPerBNB,uint256 _tokenPerBUSDT, address _gftAddress,address _busdtAddress, address _adminAddress) Ownable(){
        require(_adminAddress != address(0),"Invalid masterAccountAddess.");
    ...
    function transferBUSDT(uint256 _amount, address _sender) internal returns(uint256 _txAmount){
        ...
        bool txStatus= busdtToken.transferFrom(msg.sender,adminAddress,_amount);        
    ...
    // i/p param type: NA
    // o/p param type: uint256
    function withdrawGFT(uint256 _amount)external onlyOwner returns(uint256 amount){
        require(_amount >0,"Invalid amount.");
        require(gftToken.balanceOf(address(this)) >= _amount,"Insufficient balance.");
        bool txStatus= gftToken.transfer(payable(msg.sender), _amount);
        require(txStatus ,"Transaction failed.");
        emit DoneWithdraw(_amount);
        return _amount;
    }
