...
contract Auction is ERC1155Holder, Ownable {
    ...
    mapping(uint256 => uint256) public tokenIdsToProjects;

    mapping(address => uint256[]) public lotsOnProject;
    mapping(uint256 => Bet) public bets;
    mapping(address => uint256[]) private betsOfAUser;
    mapping(uint256 => mapping(Tier => bool)) lotTiers;
    mapping(address => mapping(uint256 => bool)) public participatedLots;
    ...
    function placeALot(
        ...
        require(minBet > 0, 'minBet must be greater than zero');
        require(maxBet > minBet, 'maxBet must be greater that minBet');
        require(
            0 < allowedTiers.length && allowedTiers.length < 4,
            'tiers lenght must be greater than zero and less than 4'
        );
    ...
    function createProject(Protocol protocol, address contractAddr) external onlyOwner {
        require(lotsOnProject[contractAddr].length == 0, 'Project already exists!');
        ...
        lotsOnProject[contractAddr].push(0);
    }

    function makeABet(uint256 lotId, uint256 betValue) external {
        require(
            erc20.balanceOf(msg.sender) >= betValue,
            "You don't have enough money to make a bet"
        );
        require(betValue >= lots[lotId].minBet, 'Your bet is lower than the min bet');
        require(
        ...
        require(lotTiers[lotId][tiers[msg.sender]], "You're not in the required tier");

        erc20.transferFrom(msg.sender, address(this), betValue);

        if (lots[lotId].betsIds.length > 1) {
            erc20.transfer(
        ...
        participatedLots[msg.sender][lotId] = true;
        emit BetCreation(lotId);
    }

    function claim(uint256 betId) external {
        uint256 lotId = bets[betId].lotId;
        require(
            block.timestamp - lots[lotId].start > lots[lotId].period,
            'Auction not finished yet'
        );
        require(lots[lotId].currentMaxBetId == betId, "Your bet isn't last");
    ...
    function withdrawNftFromContract(
    ...
    function setFee(uint256 _feeNumerator, uint256 _feeDenominator) external onlyOwner {
        require(
            _feeNumerator & _feeDenominator > 0,
    ...
    function getAllUserLots(address user)
        external
        view
        returns (AccountPageLotCard[] memory accountPageLots)
    {
        uint256 userLotsCounter = 0;
        for (uint256 i = 0; i < lotsCounter; ++i) {
            if (participatedLots[user][i + 1] == true) {
                ++userLotsCounter;
            }
        }
        accountPageLots = new AccountPageLotCard[](userLotsCounter);
        uint256 k = 0;
        for (uint256 i = 0; i < lotsCounter; ++i) {
            Lot storage lot = lots[i + 1];
            if (participatedLots[user][lot.lotId]) {
                accountPageLots[k].lotId = lot.lotId;
                accountPageLots[k].tokenId = lot.tokenId;
                accountPageLots[k].start = lot.start;
                accountPageLots[k].period = lot.period;
                accountPageLots[k].maxBet = bets[lot.currentMaxBetId].bet;
                uint256[] storage lotBetsIds = lot.betsIds;
                for (uint256 j = 0; j < lotBetsIds.length; ++j) {
                    if (bets[lotBetsIds[j]].better == user) {
                        accountPageLots[k].userLastBet = bets[lotBetsIds[j]].bet;
                    }
                }
                ++k;
            }
