...
contract Token is Context, IBEP20, Ownable {
	...
	address public FeeAddressNFT = 0xE503133ECbb3c2a60cA90258009ea18b1Ae024B7;
  ...
  // 排除账户之间转账
    ...
    function _sendToTxFee(uint256 tCharity, address sender) private {
        uint256 currentRate = _getRate();
        uint256 rCharity = tCharity.mul(currentRate);
        _rOwned[FeeAddressNFT] = _rOwned[FeeAddressNFT].add(rCharity);
        _tOwned[FeeAddressNFT] = _tOwned[FeeAddressNFT].add(tCharity);
        emit Transfer(sender, FeeAddressNFT, tCharity);
    }
