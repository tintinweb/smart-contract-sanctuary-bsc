...
contract JLCjoin  {
    ...
    function deposit(address recommender) public {
        ...
        if(user.recommend == address(0)){
           require(up.under.length < 3 &&  up.topBoss !=address(0) ,"JLCrank/2");
        ...
        up.orders +=1;
        if(up.orders%3 == 0) swapTokensForUsdt(quotedPrice[2],recommender);
        if(fundAmount >= swapTokensAtAmount) swapAndLiquify(fundAmount/2);
        
        if(up.under.length ==3 && up.under[2] == msg.sender) {
        ...
        uint256 bossAmount = quotedPrice[0]*(29-i)/100;
    ...
    function getUnderInfo(address usr) public view returns(address[6]  memory A,uint256[6]  memory amountA){
        UserInfo storage user = userInfo[usr];
        uint length = user.under.length;
        if(length >=1) {
            A[0] = user.under[0];
            amountA[0] = userInfo[A[0]].buyAmount;
        }
        if(length >=2){
            A[1] = user.under[1];
            amountA[1] = userInfo[A[1]].buyAmount;
        } 
        if(length ==3){
            A[2] = user.under[2];
            amountA[2] = userInfo[A[2]].buyAmount;
            UserInfo storage c = userInfo[A[2]];
            uint leng = c.under.length;
            if(leng >=1) {
               A[3] = c.under[0];
               amountA[3] = userInfo[A[3]].buyAmount;
            }
            if(leng >=2){
                A[4] = c.under[1];
                amountA[4] = userInfo[A[4]].buyAmount;
            } 
            if(leng ==3){
                A[5] = c.under[2];
                amountA[5] = userInfo[A[5]].buyAmount;
            }  
        }
    }
}