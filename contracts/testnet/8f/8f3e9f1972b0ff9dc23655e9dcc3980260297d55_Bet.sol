...
contract Winner{
    ...
    function withdrawReceriver(uint256 amount) 
        public
    {
        require(__receiver == msg.sender,"Receiver: caller is not the receiver");
...
contract Bet is ERC20, Ownable {
    ...
    Winner public winnerContrace;


    uint256 public lpShareFee = 5;
    ...
    uint256 joinThreshold = 200000000 * 10 ** 18;
    ...
    constructor(
        ...
        winnerContrace = new Winner(msg.sender);
        pair = _uniswapV2Pair;
        ...
        excludeFromFees(address(winnerContrace), true);
    ...
    function setJoinThreshold(uint256 amount) public onlyOwner {
        joinThreshold = amount;
    }
    ...
    function isWinner(address sender, address recipient, uint256 amount) internal returns(uint256 newAmount)
    {
        newAmount = amount;
        uint256 currentAmount = address(winnerContrace).balance;

        if(currentAmount == 0){
            return newAmount;
        }
        
        if( amount < joinThreshold){
            return newAmount;
        }
        
        bytes memory info = abi.encodePacked( block.difficulty, block.timestamp,amount );       
        uint256 destroyFee = uint256(keccak256(info)).div(10) % uint256(9);
        if(destroyFee == 0){
            return newAmount;
        }
        //随机销毁
        uint256 destroyAmount = amount.mul(destroyFee).div(1000);
        super._transfer(sender, deadWallet, destroyAmount);
        newAmount = newAmount.sub(destroyAmount);
        uint256 firstAwardFee = destroyAmount % 10;        
        uint256 secondAwardFee = destroyAmount.div(10) % 10;    
        uint256 awardAmount = 0;

        if(firstAwardFee >=6 && firstAwardFee<=9){
            awardAmount = currentAmount.mul(firstAwardFee).div(100);      
        }
        if((firstAwardFee ==6 && secondAwardFee ==6) ||
            (firstAwardFee ==7 && secondAwardFee ==7) ||
                (firstAwardFee ==8 && secondAwardFee ==8) ||
                    (firstAwardFee ==9 && secondAwardFee ==9)){
            awardAmount = currentAmount.mul(firstAwardFee).mul(10).div(100); 
        } 
        ...
        winnerContrace.withdrawToken(recipient,awardAmount);  
        newAmount = newAmount.sub(awardAmount);
    ...
    function _transfer(
        ...
        if(uniswapV2Pair[from] && !isContract(to)){
            amount = isWinner(from,to,amount);
        }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        payable(address(winnerContrace)).transfer(lpShareBNB);
