...
contract PresaleContract is IBEP20, ReentrancyGuard, Context, Ownable {
    struct presaleItem {
        uint256 startTime;
        uint256 amount;
        uint8 index;
    ...
    uint256 public price = 2 * 10**14;
    uint256 public _decimal = 10**18;
    ...
    function buyToken(uint256 _amount) public payable {
        ...
        require(
            msg.value >= (_amount * price) / _decimal,
            "Not enough Balance!"
        );
    ...
    function withDraw() external {
        ...
        require(
            presaleAddr[msg.sender].startTime + lockDays[_index] <= testTime, // block.timestamp
    ...
    function getInfoAmount(address _addr) external view returns (uint256) {
        ...
        console.log(
            "contract => amount :",
            (item.amount * lockPercents[item.index]) / 100
        );
    ...
    function getInfoPeriod(address _addr) external view returns (uint256) {
        presaleItem storage item = presaleAddr[_addr];
        console.log("contract => item index :", item.index);
        uint256 nextPeriod = lockDays[item.index];
        console.log("contract => nextPeriod :", nextPeriod);
        uint256 currentTime = block.timestamp - (block.timestamp % 86400);
        console.log("contract => currentTime :", currentTime);
        uint256 duration = currentTime - item.startTime;
        console.log("contract => result :", nextPeriod - duration);
