...
contract Creator is Ownable {
    ...
    function createStaking(
        ...
        uint256[] storage _userVaults = userVaults[msg.sender];
        _userVaults.push(_vaults.length - 1);
        userVaults[msg.sender] = _userVaults;
    }
}
...
contract FeeDistributorV6 is Ownable, Pausable, ReentrancyGuard {
    ...
    bool public excludedFromFee = false;
    ...
    function checkFee() external {
        ...
        token.safeTransferFrom(feeTrackerContract, address(this), 1000);
        uint256 diff = token.balanceOf(address(this)) - balanceBefore;
        if (diff == 1000) excludedFromFee = true;
        token.transfer(feeTrackerContract, diff);
    }
     ...
     */
    function runway(uint256 _rewardsPerBlock) public view returns (uint256) {
...
contract StakingVaultV6_0_0 is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    function checkFee() external {
        ...
        uint256 balanceBefore = token.balanceOf(address(this));
        token.safeTransferFrom(feeTracker, address(this), 1000);
        uint256 diff = token.balanceOf(address(this)) - balanceBefore;
        if (diff == 1000) excludedFromFee = true;
        token.transfer(feeTracker, diff);
    }
