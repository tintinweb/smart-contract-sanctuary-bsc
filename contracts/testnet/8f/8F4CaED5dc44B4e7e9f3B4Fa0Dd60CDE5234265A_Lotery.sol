...
contract Lotery is Ownable{
  ...
  uint public numberCost;
  uint public actualNumber = 1;
  uint public totalNumbers;

  uint[] public winersNumbers; 
  uint[] public percentForWiners;
  uint32 public cantOfNumbers; //cant of winers per gift

  address caller;
  address manager = msg.sender;
  IERC20 public token;
  RandomGenerator public vrf;
  mapping(uint => address) public numerOwner;
 

  constructor ( uint _winAmount, uint _numberCost){   
    winAmount = _winAmount;
    numberCost = _numberCost;
  }
  ...
  function buyNumber() public {
    token.transferFrom(msg.sender, manager, numberCost);

    numerOwner[actualNumber] = msg.sender;
  ...
  function finishPlay(uint[] memory randomNumber) public {
    ...
    for(uint i; i < randomNumber.length; i++){
      winersNumbers[i] = (randomNumber[i] % totalNumbers) + 1;
      amount = (winAmount * percentForWiners[i]) / 100;
      token.transfer( numerOwner[winersNumbers[i]], winAmount);
    }
  ...
  function setWinAmount(uint newWinAmount) public onlyOwner{
    winAmount = newWinAmount;
  }

  function withDrawhTokens() public  onlyOwner{
