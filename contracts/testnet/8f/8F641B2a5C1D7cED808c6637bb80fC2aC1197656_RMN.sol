...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    unchecked {
        ...
        uint256 fee;
        if (sender == swapPair || recipient == swapPair) {
            if (lpStaking != address(0) && _fNumerator > 0 && _fDenominator > 0) {
                ...
                _deposit(account, halfFee);
                _balances[address(0)] += fee - halfFee;
                emit Transfer(account, address(0), fee - halfFee);
            }
            if (sender == swapPair) {
...
contract RMN is ERC20 {

    constructor() ERC20("Real Man", "RMN") {
        _mint(msg.sender, 9999 * (10 ** uint256(decimals())));
        swapPair = 0x0000000000000000000000000000000000000001;
    }
    ...
    function setSwapConfig(uint256 numerator, uint256 denominator) public onlyOwner returns (bool) {
        _fNumerator = numerator;
        _fDenominator = denominator;
        return true;
    }

    function getSwapConfig() public view returns (uint256, uint256) {
        return (_fNumerator, _fDenominator);
    }
}