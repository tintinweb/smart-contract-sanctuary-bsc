...
contract Hero is BEP721, Ownable {
    ...
    uint256 boxPrice; // use for buy box
    uint256 totalSupplyBox; // tokenID => time minted

    bool openBuyBox = false;
    ...
    // constructor
    constructor(IBEP20 _HBG, uint256 initID) BEP721("HERO Token", "HERO") {
        HBG = IBEP20(_HBG); //0xA9Afb65805e143980C4FC231E008DA764FD29b53
        timeToBurn = 365 * 2 days;
        managers[msg.sender] = true;
        circulatingSupply = initID;
    }
    ...
    function getOpenBuyBox() public view returns (bool) {
        return openBuyBox;
    }

    function setOpenBuyBox(bool _bool) public onlyOwner {
        openBuyBox = _bool;
    }

    function buyBoxMulti(uint256 quantity) public {
        require(quantity > 0, "Quantity must greater than 0");
        require(
            HBG.balanceOf(msg.sender) >= boxPrice * quantity,
            "invalid Price "
        );
        require(openBuyBox, "The buy box event is closed");
        require(totalSupplyBox - quantity >= 0, "Out of box");

        HBG.transfer(owner(), boxPrice * quantity);
        for(uint i = 0 ; i < quantity; i++){
            circulatingSupply += 1;
            _mint(msg.sender, circulatingSupply);
            start[circulatingSupply] = block.timestamp;
        }
        totalSupplyBox -= quantity;
    }

    function burn(uint256[] memory ID_Tokens) public {
        for (uint256 i = 0; i < ID_Tokens.length; i++) {
            require(block.timestamp > (start[ID_Tokens[i]] + timeToBurn), "Token's age is not enough");
            _burn(ID_Tokens[i]);
        }
    }

    function forceBurn(uint256[] memory ID_Tokens) external onlyManager {
        for (uint256 i = 0; i < ID_Tokens.length; i++) {
            _burn(ID_Tokens[i]);
        }
    }
    ...
    function getTimeToBurn() public view returns (uint256) {
        return timeToBurn;
    }
    ...
    function getBoxPrice() view public returns (uint256) {
        return boxPrice;
    }
    ...
    function getTotalBox() view public returns (uint256) {
        return totalSupplyBox;
    }

    function setTotalBox(uint256 total) public onlyOwner {
        totalSupplyBox = total;
    } 
