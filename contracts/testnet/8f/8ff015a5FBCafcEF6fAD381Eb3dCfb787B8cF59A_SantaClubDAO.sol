...
contract SantaClubDAO is Ownable {
    ...
    uint256 public minimumTokensForProposal = SANTA_TOKEN_TOTAL_SUPPLY / 1000; // 1%
    uint256 public minimumNFTsForProposal = SANTA_NFT_TOTAL_SUPPLY / 100; // 1%

    uint256 public monoMinimumTokensForProposal =
        (SANTA_TOKEN_TOTAL_SUPPLY * 5) / 1000; // 0.5%
    uint256 public monoMinimumNFTsForProposal =
        (SANTA_NFT_TOTAL_SUPPLY * 5) / 1000; // 0.5%
    ...
    event SetMonoMinimumTokensForProposal(uint256 newMinAmount);
    event SetMonoMinimumNFTsForProposal(uint256 newMinAmount);
    event SetVotesNeeded(uint256 newVotesNeeded);

    constructor() {
        tokenCustody = ITokenCustody(
            0xC53e266528177dd274A2B5D69c9fa6d52BAB41c9
        );
        staking = ISantaClubStaking(0x4F1c83E2308C08E7ac55DCD40FAa332477C7535D);
        santaToken = IERC20(0x05e57F7EDe801817331c0C3745Fa450c73696cd5);
        santaNFT = IERC721Enumerable(
            0xF8f29c22f436de8bcAEc30c991D92AdE6D7CE258
        );
        chairman = 0xC8F577F5FB6C4a2ae6387d5269F211a36AcaCbF7;
    }
    ...
    function isEligibleProposer(address proposer) public view returns (bool) {
        ...
        return
            ...
            ((tokenAmount >= monoMinimumTokensForProposal) &&
                (nftAmount >= monoMinimumNFTsForProposal));
    }
    ...
    function setMonoMinimumTokensForProposal(uint256 amount)
        external
        onlyOwner
    {
        monoMinimumTokensForProposal = amount;

        emit SetMonoMinimumTokensForProposal(amount);
    }

    function setMonoMinimumNFTsForProposal(uint256 amount) external onlyOwner {
        monoMinimumNFTsForProposal = amount;

        emit SetMonoMinimumNFTsForProposal(amount);
    }
    ...
    function voteSuccessOrFail(uint256 proposalID) external returns (bool) {
        if (proposals[proposalID].votes >= votesNeeded) {
            endVote(proposalID);
        }
        return proposals[proposalID].passed;
...
contract TokenCustody is ReentrancyGuard, Ownable {
    ...
    constructor() {
        santaToken = IERC20(0x50C5C8e6b2B2e86135A1B909176436723866356D);
    }
    ...
    function lockTokens(uint256 endTime) external nonReentrant {
        require(
            ...
            "Lock Tokens: You cannot lock to the past date."
        );
        require(
            endTime > custody[msg.sender].unlockedAt,
            "Lock Tokens: You cannot lock to a past date than the current lock date."
        );
        require(
            endTime - block.timestamp < 8 days,
            "Lock Tokens: You cannot lock more than 8 days."
        );
        require(
            custody[msg.sender].owner == msg.sender,
            "Lock Tokens: You are not an owner."
        );
