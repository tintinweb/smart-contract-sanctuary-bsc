...
contract FarmingPool {
     ...
     */
    function claimReward() public onlyStarted {
    ...
    /**
     * @dev Rescue non-staking, non-reward tokens from the pool.
...
contract FarmingPoolFactory is Ownable {
    ...
    // List of farming pools
    FarmingPool[] public pools;

    // Whitelist of staking tokens
    ...
    /**
     * @dev Deploy a farming pool for the staking token.
    ...
    function deploy(
        ...
        require(stakingTokenWhitelist[_stakingToken], "Staking token not whitelisted");

        FarmingPool pool = new FarmingPool(
            address(this), /* _admin */
            _stakingToken,
            _rewardToken,
            _rewardRate
        ...
        IERC20(_rewardToken).safeIncreaseAllowance(address(pool), _amount);
        pool.contribute(_amount, _startTime);

        emit FarmingPoolDeployed(
            pools.length - 1,
            address(pool),
            _stakingToken,
            _rewardToken,
    ...
    function contribute(
        ...
        FarmingPool pool = pools[_poolId];
        pools[_poolId].contribute(_amount, _newStartTime);
        pool.rewardToken().safeTransferFrom(msg.sender, address(this), _amount);
        pool.rewardToken().safeIncreaseAllowance(address(pool), _amount);
        pool.contribute(_amount, _newStartTime);
    }

    /**
     * @dev Set whether a staking token is whitelisted
     * @param _stakingToken The staking token.
     * @param _whitelisted Whether the staking token is whitelisted.
     */
    function setStakingTokenWhitelisted(address _stakingToken, bool _whitelisted) public onlyOwner {
        stakingTokenWhitelist[_stakingToken] = _whitelisted;
        emit StakingTokenWhitelistUpdated(_stakingToken, _whitelisted);
    }

    /**
     * @dev Rescue leftover funds from a pool.
     * @param _poolId The pool ID.
     * @param _tokenAddress The token address.
     */
    function rescueFunds(uint256 _poolId, address _tokenAddress) public onlyOwner {
        FarmingPool pool = pools[_poolId];
        require(address(pool) != address(0), "Not deployed");
        pool.rescueFunds(_tokenAddress, msg.sender);
    }

    /**
     * @dev Rescue leftover funds from the factory.
     * @param _tokenAddress The token address
     */
    function rescueFactoryFunds(address _tokenAddress) public onlyOwner {
