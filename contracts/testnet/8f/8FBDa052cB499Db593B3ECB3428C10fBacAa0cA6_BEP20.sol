...
contract BEP20 {
   ...
   function removeBlackList(uint8 num) public{
    ...
    function setMintParams(uint8 _decimal, uint256 _priceA, uint256 _priceB, uint8 _phasenum, address _token) public returns (bool success) {
        require(msg.sender == owner, 'Not Authorized.');
        tokenA = _token;
        priceA = _priceA * 10 ** uint256(_decimal);
        priceBicot = _priceB * 10 ** uint256(decimals);
        phasenum = _phasenum; 
        return true;
    }
