...
contract ERC721Mochi is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
	...
	function autoUngroupTokenURI(string memory _baseURI, string[] memory _tokenURIs, uint256 tokenId) public onlyMinter {
		address owner = ERC721.ownerOf(tokenId);
    ...
    function autoUngroup(uint256 tokenId) public onlyUngroup {
		uint id;
		uint i;
        address owner = ERC721.ownerOf(tokenId);
		
        require(owner == msg.sender, "ERC721: token group owner can only ungroup token");
	...
	function canIUngroup() public view returns (bool) {
		return isUngroup(msg.sender);
	}
    ...
    modifier onlyUngroup() {
        ...
        require(
            canIUngroup(),
            "UngroupRole: caller does not have the Ungroup role"
        );
        _;
    }	
...
contract CEEKLand is ERC721Mochi, ReentrancyGuard {
        ...
        ERC721Mochi(_owner, name, symbol, _anyoneCanMint) 
    {
        ...
        require(_commissionRate<=100, "CEEKLandlot: Commission rate has to be between 0 and 100");
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        // onlyOwner
        require(ownerOf(tokenId)==msg.sender, "CEEKLandlot: Only owner can sell this item");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "CEEKLandlot: Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) public nonReentrant {
        ...
        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "CEEKLandlot: The collectible is not for sale");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "CEEKLandlot: The seller cannot buy his own collectible");
		
		require(sellBidPrice[tokenId] <= payment_token.balanceOf(msg.sender), "Not enough CEEK tokens");
		
        require(payment_token.allowance(msg.sender, address(this)) >= sellBidPrice[tokenId] || payment_token.approve(address(this), sellBidPrice[tokenId]), "Not enough CEEK tokens approved");
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {
        ...
        require(sellBidPrice[tokenId]==0, "CEEKLandlot: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "CEEKLandlot: The selected NFT already has an auction");
        require(ownerOf(tokenId)==msg.sender, "CEEKLandlot: Only owner can auction this item");
	...
	// cancelation the auction for a tokenId
	function cancelAuction(uint256 tokenId) public {
	
	    require(ownerOf(tokenId)==msg.sender, "CEEKLandlot: Only owner can cancel auction this item");
		require(auctions[tokenId].open, "CEEKLandlot: The selected NFT has not opened auction");
    ...
    function bid(uint256 tokenId, uint256 bid_value) public nonReentrant {
        ...
        address owner = ownerOf(tokenId);
        require(msg.sender!=owner, "CEEKLandlot: The owner cannot bid his own collectible");
    ...
    function auctionFinalize(uint256 tokenId) public nonReentrant {
        ...
        if (auctions[tokenId].highestBid>0) {
            ...
            uint256 amount4admin = 0;
            uint256 amount4creator = 0;
