...
contract OblivionBasicNftFactory {
    address public market;

    event NftDeployed(string _symbol, address _nft, address _owner);

    constructor (address _market) { market = _market; }

    function deployBasicNft(string memory _name, string memory _symbol, string memory _uri, uint256 _maxSupply, bool _whitelistMarket, bool _whitelistOwner) public returns (address) {
        ...
        if (_whitelistMarket) nft.whitelistAdmin(market, true);
