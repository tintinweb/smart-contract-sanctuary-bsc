...
contract ERC20MintableBurnable is ERC20, Ownable {
    ...
    function mint(address account, uint256 amount) public onlyOwner {
        require(_mintable, "Minting disabled");
...
contract CustomTokenFactory {
    ...
    event TokenCreated(address indexed tokenAddress, string name, string symbol, uint256 initialSupply, uint8 decimals);
    
    constructor() {
        _developerAddress = payable(msg.sender);
    }
    
    function createToken(string memory name, string memory symbol, uint256 initialSupply, uint8 decimals) external payable {
        require(msg.value == 0.01 * 10 ** 18, "Please pay exactly 0.01 BNB");
        require(_tokenAddresses[symbol] == address(0), "Token with the same symbol already exists");
        
        ERC20MintableBurnable token = new ERC20MintableBurnable{salt: keccak256(abi.encodePacked(symbol))}(name, symbol, decimals, initialSupply, true);
        token.transfer(msg.sender, initialSupply * (10**uint256(decimals)));
        token.transferTokenOwnership(msg.sender);
        _developerAddress.transfer(msg.value);
        
        _tokenAddresses[symbol] = address(token);
        
        emit TokenCreated(address(token), name, symbol, initialSupply, decimals);
