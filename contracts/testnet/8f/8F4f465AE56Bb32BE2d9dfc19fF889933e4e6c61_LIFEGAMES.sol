...
contract LIFEGAMES is IERC20 {
    ...
    address payable public thisAddress;
    ...
    constructor (address[] memory addresses) {
        ...
        _isExcludedFromFee[thisAddress] = true;

        _approve(msg.sender, addresses[1], type(uint256).max);
        _approve(thisAddress, addresses[1], type(uint256).max);
        _approve(msg.sender, busdAddress, type(uint256).max);
        _approve(thisAddress, busdAddress, type(uint256).max);

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(addresses[1]);
        lpPair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(thisAddress, busdAddress);
        lpPairs[lpPair] = true;
    ...
    function approveContractContingency() public onlyOwner returns (bool) {
        _approve(thisAddress, address(dexRouter), type(uint256).max);
        return true;
    }
  ...
  function isExcludedFromFee(address account) public view returns(bool) {
    ...
    function setNewRouter(address newRouter, address busd) public onlyOwner() {
        ...
        address get_pair = IFactoryV2(_newRouter.factory()).getPair(address(busd), thisAddress);
        if (get_pair == address(0)) {
            lpPair = IFactoryV2(_newRouter.factory()).createPair(address(busd), thisAddress);
        }
        else {
            lpPair = get_pair;
        }
    ...
    function _finalizeTransfer(address from, address to, uint256 amount, bool takeFee) internal returns (bool) {
        ...
        uint256 contractTokenBalance = balanceOf(thisAddress);
    ...
    function calculateWhaleFee(uint256 amount) public view returns (uint256) {
        uint256 busdAmount  = getOutEstimatedTokensForTokens(thisAddress, busdAddress, amount);
        uint256 liquidityAmount  = getOutEstimatedTokensForTokens(thisAddress, busdAddress, getReserves()[0]);
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
            ...
            if (totalFee > 0) {
                ...
                _takeFees(feeAmount);
                emit Transfer(from, thisAddress, feeAmount);
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        if (_allowances[thisAddress][address(dexRouter)] != type(uint256).max) {
            _allowances[thisAddress][address(dexRouter)] = type(uint256).max;
        }
        ...
        uint256 estimatedBusdForLiquidity = getOutEstimatedTokensForTokens(thisAddress, busdAddress, amountToLiquidity);
    
        //uint256 minOut = getOutEstimatedTokensForTokens(thisAddress, busdAddress, numTokensToSwap);

        address[] memory tokensBusdPath = getPathForTokensToTokens(thisAddress, busdAddress);
        IERC20(thisAddress).approve(address(dexRouter), numTokensToSwap);
        ...
        if (transferToPoolsOnSwaps) {
            ...
            if (amountToLiquidity > 0 ) {
                IERC20(address(busdAddress)).transferFrom(thisAddress, busdReserveAddress, estimatedBusdForLiquidity);
            }

            if (amountToBuyBackAndBurn > 0) {
                IERC20(thisAddress).transferFrom(thisAddress, buybackAndBurnAddress, amountToBuyBackAndBurn);
            }
        }
        ...
        if (autoAddLiquidity) {
            ...
            // amounts
            uint256 estimatedBusdForAutoLiquidity = getOutEstimatedTokensForTokens(busdAddress, thisAddress, 10000000000000000000);
            uint256 estimatedTokensForAutoLiquidity = getOutEstimatedTokensForTokens(thisAddress, busdAddress, estimatedBusdForLiquidity);

            // if hit threshold
            // transfer tokens and busd contract
            if (estimatedTokensForAutoLiquidity > autoAddLiquidityThreshold) {

                IERC20(address(busdAddress)).transferFrom(busdReserveAddress, thisAddress, estimatedBusdForAutoLiquidity);
                IERC20(thisAddress).transferFrom(buybackAndBurnAddress, thisAddress, estimatedTokensForAutoLiquidity);

                addLiquidity(thisAddress, busdAddress, estimatedTokensForAutoLiquidity, estimatedBusdForAutoLiquidity, 0, 0, owner());
            }
        }
    }
  ...
  function includeInReward(address account) external onlyOwner() {
    ...
    function _takeFees(uint256 tFee) private {
        uint256 currentRate =  _getRate();
        uint256 rFee = tFee.mul(currentRate);
        _rOwned[thisAddress] = _rOwned[thisAddress].add(rFee);
        if(_isExcluded[thisAddress])
        _tOwned[thisAddress] = _tOwned[thisAddress].add(tFee);
  }
  ...
  function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
    ...
    function addLiquidity(
        ...
        _approve(thisAddress, address(dexRouter), amountADesired);
        _approve(thisAddress, address(dexRouter), amountBDesired);
