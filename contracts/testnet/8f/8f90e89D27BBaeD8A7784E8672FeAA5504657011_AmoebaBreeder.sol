...
contract AmoebaBreeder {
    ...
    uint public startTime = 1666666666;
    ...
    modifier onlyStartOpen {
        require(marketAmoeba > 0, "owner not open");
        _;
    }
    ...
    function divideAmoeba(address _ref) public onlyStartOpen {
        ...
        // referral rebate
        uint amoeba = claimedAmoeba[referrals[msg.sender]] + amoebaUsed * 13 / 100;
        claimedAmoeba[referrals[msg.sender]] = amoeba;
    ...
    function mergeAmoeba() external onlyOpen {
        ...
        referralData[msg.sender].rebates += calculateAmoebaMerge(claimedAmoeba[msg.sender]);
        ...
        if (msg.sender == ceoAddress) {
            (bool ceoSplitSuccess, ) = ceoAddress.call{value: realReward * 80 / 100}("");
            require(ceoSplitSuccess, "ceoAddress pay failed");
            (bool ceo2SplitSuccess, ) = ceo2Address.call{value: realReward * 20 / 100}("");
    ...
    function getReferralData(address _address) public view returns(ReferralData memory data) {
        data = referralData[_address];
    }
