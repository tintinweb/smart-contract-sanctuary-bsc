...
contract BNBEE {
    ...
    event MissedBNBReceive(address indexed receiver, address indexed from, uint8 matrix, uint8 level,uint time);
    event SentExtraBNBDividends(address indexed from, address indexed receiver, uint8 matrix, uint8 level,uint time);
    event ReceivedBNB(address indexed from,address indexed receiver,uint256 matrix,uint256 level,uint256 amount,uint time);
    ...
    function injectUser(address[] memory _users) public{
        require(owner == msg.sender,"Caller is Not Owner");
        require(!injectStatus,"Inject is Closed");
        ...
        for (uint8 j = 1; j <= LAST_LEVEL; j++) {
            users[_users[i]].activeA3Levels[j] = true;
            users[_users[i]].activeA6Levels[j] = true;
        }
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external payable {
        ...
        if (matrix == 1) {
            ...
            emit Upgrade(msg.sender, freeA3Referrer, 1, level,block.timestamp);
            ...
            emit Upgrade(msg.sender, freeA6Referrer, 2, level,block.timestamp);
        }
    }   
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        require(msg.value == 0.001e18, "registration cost 0.1");
    ...
    function updateA3Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].A3Matrix[level].referrals.length < 3) {
            emit NewUserPlace(userAddress, referrerAddress, 1, level, uint8(users[referrerAddress].A3Matrix[level].referrals.length),block.timestamp);
            return sendBNBDividends(referrerAddress, userAddress, 1, level);
        }
        
        emit NewUserPlace(userAddress, referrerAddress, 1, level, 3,block.timestamp);
        //close matrix
        ...
        if (!checkInjectUser(referrerAddress)) {
            ...
            sendBNBDividends(referrerAddress, userAddress, 1, level);
            users[referrerAddress].A3Matrix[level].reinvestCount++;
            emit Reinvest(referrerAddress, address(0), userAddress, 1, level,block.timestamp);
        }
    }
    ...
    function updateA6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].A6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].A6Matrix[level].firstLevelReferrals.length),block.timestamp);
            ...
            if ((len == 2) && 
                ...
                if (users[referrerAddress].A6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 5,block.timestamp);
                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 6,block.timestamp);
                }
            }  else if ((len == 1 || len == 2) &&
                    users[ref].A6Matrix[level].firstLevelReferrals[0] == referrerAddress) {
                if (users[referrerAddress].A6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 3,block.timestamp);
                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 4,block.timestamp);
                }
    ...
    function updateA6(address userAddress, address referrerAddress, uint8 level, bool a2) private {
        if (!a2) {
            ...
            emit NewUserPlace(userAddress, users[referrerAddress].A6Matrix[level].firstLevelReferrals[0], 2, level, uint8(users[users[referrerAddress].A6Matrix[level].firstLevelReferrals[0]].A6Matrix[level].firstLevelReferrals.length),block.timestamp);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, 2 + uint8(users[users[referrerAddress].A6Matrix[level].firstLevelReferrals[0]].A6Matrix[level].firstLevelReferrals.length),block.timestamp);
            //set current level
            ...
            emit NewUserPlace(userAddress, users[referrerAddress].A6Matrix[level].firstLevelReferrals[1], 2, level, uint8(users[users[referrerAddress].A6Matrix[level].firstLevelReferrals[1]].A6Matrix[level].firstLevelReferrals.length),block.timestamp);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, 4 + uint8(users[users[referrerAddress].A6Matrix[level].firstLevelReferrals[1]].A6Matrix[level].firstLevelReferrals.length),block.timestamp);
            //set current level
    ...
    function updateA6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (!checkInjectUser(referrerAddress)) {
            ...
            emit Reinvest(referrerAddress, address(0), userAddress, 2, level,block.timestamp);
            sendBNBDividends(referrerAddress, userAddress, 2, level);
        }
    }

    function checkInjectUser(address _user) private view returns (bool){
        address _reffer;
        for(uint8 i=1;i<=10;i++){
            if(idToAddress[i] == _user){
                _reffer = idToAddress[i];
                break;
            } 
        }
        return (_reffer == _user);
    }
        ...
        if (matrix == 1) {
            while (true) {
                if (users[receiver].A3Matrix[level].blocked) {
                    emit MissedBNBReceive(receiver, _from, 1, level,block.timestamp);
            ...
            while (true) {
                if (users[receiver].A6Matrix[level].blocked) {
                    emit MissedBNBReceive(receiver, _from, 2, level,block.timestamp);
                    isExtraDividends = true;
    ...
    function sendBNBDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        emit ReceivedBNB(msg.sender,receiver,matrix,level,levelPrice[level],block.timestamp);

        if (isExtraDividends) {
            emit SentExtraBNBDividends(_from, receiver, matrix, level,block.timestamp);
        }
    }
    ...
    function failSafe(address _to,uint amount)public {
          require(owner == msg.sender,"Only Owner Accessible");
          require(address(this).balance >= amount,"Invalid Price");
          require(address(uint160(_to)).send(amount),"Transaction Failed");
    }
}