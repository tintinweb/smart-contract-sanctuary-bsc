...
contract AFD is Context, IERC20, IERC20Metadata, Ownable {
    ...
    mapping(address => bool) public isTxFeeExempt; //init ido.sol
    ...
    address public _dividendAddress = address(0xF6f7599e41B86963C6a9018079F995a73453B9c8);
    IRelation public relation = IRelation(0xb454607AaC6F781acD0989582B82CdFa33B85F7A);
    //mainnet
    ...
    modifier onlyHolder(){
        require(msg.sender==holder,'AFRD:not allowed');
        _;
    }
    ...
    constructor() {
        ...
        isTxFeeExempt[_receive] = true;
        isTxFeeExempt[_dividendAddress] = true;
    ...
    function _nftHandle(address sender,uint256 amount) internal {
        ...
        _balances[_dividendAddress] += _feeAmount;
        emit Transfer(sender, _dividendAddress, _feeAmount);
    }
    ...
    function _teamHandler(address sender,uint256 amount) internal{
        ...
        if(amount > _total){
            ...
            _balances[_receive] += remain;
            emit Transfer(sender, _receive, remain);
        }
    }

    function setDividendAddress(address _value) external onlyHolder{
        _refreshPermit(_dividendAddress,_value);
        _dividendAddress = _value;
    }

    function setReceiveAddress(address _value) external onlyHolder{
        _receive = _value;
    }

    function setTransferWhiteListsAddress(address _value,bool _isWhiteLists) external onlyHolder{
        isTxFeeExempt[_value] = _isWhiteLists;
    }
