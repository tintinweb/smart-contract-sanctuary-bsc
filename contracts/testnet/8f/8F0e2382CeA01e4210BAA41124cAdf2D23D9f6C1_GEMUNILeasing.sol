...
contract GEMUNILeasing is PermissionGroupUpgradeable, IGEMUNILeasing{
    ...
    function registerLeasing(address nftToken, uint[] memory tokenIds, uint duration, uint minDuration, uint maxDuration, uint[] memory pricePerWeek, uint mortgageAmount, uint rentalPrice) external override {
        ...
        for(uint i = 0; i < tokenIds.length; i++) {
            ...
            uint durationPerWeek = duration * weekPerSecond;
            ...
            require(durationPerWeek >= min && durationPerWeek <= max, "Leasing: invalid duration");
            ...
            leasingGuild.gameGuild = msg.sender;
            leasingGuild.duration = durationPerWeek;
    ...
    function rent(address nftToken, uint tokenId, uint duration, uint price, bytes memory sig) external override {
        ...
        if (guild.gameGuild != address(0)) {
            rentalPriceGeni =  guild.price * duration;
            rentPassFromGameGuild(guild, duration, price);
        } else if (leasing.lender != address(0) && guild.gameGuild == address(0)) {
            rentalPriceGeni =  leasing.price * duration;
            rentPassFromAdmin(leasing, duration, price);
        } 
        ...
        renter.duration = duration * weekPerSecond;
        emit Rent(nftToken, tokenId, rentalPriceGeni, renter.startLending, duration);
    }

    function rentPassFromGameGuild(LeasingGuildInfo storage guild, uint duration, uint price) internal {
        require(guild.price > 0, "Leasing: not on game guild");
        
        uint durationWeek = duration * weekPerSecond;
        require(durationWeek>= guild.minDuration && durationWeek <= guild.maxDuration, "Leasing: invalid duration");
        uint rentalPrice = guild.price * duration;
        require(price == rentalPrice, "Lending: invalid price");
        uint rentalPriceGeni = convertPrice(rentalPrice);
        
        geni.transferFrom(msg.sender, guild.gameGuild, rentalPriceGeni);
    }

    function rentPassFromAdmin(LeasingInfo storage leasing, uint duration, uint price) internal {
        require(leasing.price > 0, "Leasing: not on leasing");
        uint durationWeek = duration * weekPerSecond;
        require(durationWeek >= leasing.minDuration && durationWeek <= leasing.maxDuration, "Leasing: invalid duration");
...
interface IGEMUNILeasing {
    ...
    event Rent(address nftToken, uint indexed tokenId, uint price,uint startLending, uint duration);
