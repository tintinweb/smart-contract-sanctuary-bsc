...
contract IssuedSynthAggregator is Owned, MixinSystemSettings, IIssuedSynthAggregator {
    ...
    function totalIssuedSynths() external view returns (uint256 _issuedSynths) {
        ...
        for (uint256 ii = 0; ii < availableSynths.length; ii++) {
            ...
            if (currencyKey == sUSD) {
                uint256 totalSynths = synthrBridge().synthTotalSupply(currencyKey);
    ...
    function totalIssuedSynthsPerIssuer(address account) external view returns (uint256 _issuedSynths) {
        ...
        for (uint256 ii = 0; ii < availableSynths.length; ii++) {
            bytes32 currencyKey = availableSynths[ii];
            if (currencyKey == sUSD) {
                uint256 totalSynths = synthrBridge().synthBalanceOf(currencyKey, account);
                if (totalSynths > 0) {
                    _issuedSynths = _issuedSynths.add(totalSynths);
                }
            } else {
    ...
    function totalIssuedSynthPerAsset(bytes32 currencyKey) external view returns (uint256 _issuedSynth) {
        if (currencyKey == sUSD) {
            _issuedSynth = synthrBridge().synthTotalSupply(currencyKey);
        } else {
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
            uint256 totalSynths = synthrBridge().synthTotalSupply(currencyKey);
    ...
    {
        if (currencyKey == sUSD) {
            _issuedSynth = synthrBridge().synthBalanceOf(currencyKey, account);
        } else {
            (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
            uint256 totalSynths = synthrBridge().synthBalanceOf(currencyKey, account);
            _issuedSynth = totalSynths.multiplyDecimal(currencyRate);
