...
contract Dalchand {
    ...
    uint256 activeDepth = 1;
    uint256 activeId = 2;
    uint256 minActiveId = 2;
    ...
    mapping(uint256 => uint256) usersDynamicId;
    ...
    constructor(address payable ownerAddress) public {
        ...
        LEVEL_PRICE[4] = 0.003 ether;
        LEVEL_PRICE[5] = 0.009 ether;
        LEVEL_PRICE[6] = 0.009 ether;        
        LEVEL_PRICE[7] = 0.012 ether;
        LEVEL_PRICE[8] = 0.012 ether; 
    ...
    function _buyLevel4x4(address payable userAddress) internal{
        ...
        usersDynamicId[activeId]= lastIDCount4x4;
        activeId++;
        
    }

    function getReferral() public returns(uint256)
    {
        if(lastIDCount4x4<=
        5)
        {
        ...
        else
        {
            ...
            if((activeId)<(minActiveId+(4**activeDepth)))
            {
                return activeId;
            }
            else if(activeId>=(minActiveId+(4**activeDepth)))
            {
                if(userInfos4x4[usersDynamicId[activeId-1]].childCount==4){
                    minActiveId = activeId;
                    activeDepth++;
                    return activeId;
                }else{
                    activeId = minActiveId;
                }
