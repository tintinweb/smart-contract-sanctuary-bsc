...
contract StakePool is USDTWrapper, Ownable, Adminable {
    ...
    function getLpValue(uint256 lpAmount) public view returns (uint256){
        ...
        return tokenAmount.mul(price).mul(2);
    ...
    function _checkDayMoreThan7(uint256 _time) view internal {
        require(block.timestamp.sub(_time) >= ONE_DAY.mul(limitDayCount));
    }
    ...
    function stake(address _ref, uint256 amount)
    public
    ...
    function getRewardInfo(address _addr) public view returns (uint256, uint256, uint256){
        return (earned(_addr), refRewardMap[_addr], refTotalMap[_addr]);
    }
