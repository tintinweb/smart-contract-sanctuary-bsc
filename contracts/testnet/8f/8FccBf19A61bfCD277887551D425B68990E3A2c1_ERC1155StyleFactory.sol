...
import "./ERC1155Style.sol";

import "../../lib/Manager.sol";

contract ERC1155StyleFactory is Manager {

    event ERC1155StyleCreated(address indexed dao, address indexed styleItem);
     ...
     */
    function createStyle(string memory _url, address payable dexAddress, uint96 royaltyRate) external onlyDAO returns (address) {
        bytes32 salt = keccak256(abi.encodePacked(dao, block.timestamp));
        //create2
        ERC1155Style styleItem = new ERC1155Style{salt : salt}(_url, dexAddress, royaltyRate);
        styleItem.transferOwnership(dao);
        address result = address(styleItem);
        emit ERC1155StyleCreated(dao, result);
        return result;
    }
}
