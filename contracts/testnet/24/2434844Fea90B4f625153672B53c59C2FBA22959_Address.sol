...
contract MetaSlipNFT is ERC721, ERC721Enumerable, Pausable, Ownable {
    ...
    constructor() ERC721("MetaSlip", "MSP") {
        mintFee = 0.03e18;
        ...
        // Testnet 
        weth = IERC20(0x888a4548B5677267C068EE185a18fD22d6848337);

        excludedFromFee[_msgSender()] = true;
        feeReceiver = _msgSender();
        baseUri_ = "https://gateway.pinata.cloud/ipfs/QmQiNLDznRxBj9CtXk8a1FcV4EvaSyXUB2SivuoEgFThpU/";
    }
    ...
    function setExcludFromFee(address account,bool status) public onlyOwner {
    ...
    function safeMint(address to) public whenNotPaused {
        require(totalSupply().add(1) <= MAX_SUPPLY, "Supply exceed");
        uint256 tokenId = randomIndex();
        if(!excludedFromFee[_msgSender()]) {
            weth.safeTransferFrom(_msgSender(),feeReceiver,mintFee);
        }
        _safeMint(to, tokenId);
    }
