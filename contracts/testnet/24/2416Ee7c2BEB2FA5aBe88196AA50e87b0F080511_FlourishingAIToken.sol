...
import "./ERC20VestableInTimestampUpgradable.sol";

// https://blog.logrocket.com/using-uups-proxy-pattern-upgrade-smart-contracts/

contract FlourishingAIToken is Initializable, ERC20VestableInTimestampUpgradable, UUPSUpgradeable {
    uint8 private constant DECIMALS = 18;
    uint256 private constant TOKEN_WEI = 10**uint256(DECIMALS);

    uint256 private constant INITIAL_WHOLE_TOKENS = uint256(35 * (10**6)); // 55 million
    uint256 private constant INITIAL_SUPPLY = uint256(INITIAL_WHOLE_TOKENS) * uint256(TOKEN_WEI);
	
	function initialize(address admin) public initializer {
		__ERC20_init("Flourishing AI Token", "AI");
		__ERC20VestableInTimestamp_init(admin);
    ...
    function supplement(address to, uint256 amount) public onlyAdmin {
        _mint(to, amount);
    }
...
abstract contract ERC20VestableInTimestampUpgradable is Initializable, ERC20BurnableUpgradeable, AccessControlUpgradeable {

	function __ERC20VestableInTimestamp_init(address admin) internal onlyInitializing {
		// automatically called
		//__ERC20_init_unchained(name_, symbol_);
		//__ERC20Burnable_init_unchained();
		
		__ERC20VestableInTimestamp_init_unchained(admin);
    }

    function __ERC20VestableInTimestamp_init_unchained(address admin) internal onlyInitializing {
    ...
    /*************************
	   ...
	   These are addresses which can transfer while the contract is locked
	 *************************/
    ...
    function addWhiteList(address _whiteListAddress) public onlyAdmin {
		if (isWhiteList[_whiteListAddress]) return;
    ...
    function removeWhiteList(address _whiteListAddress) public onlyAdmin {
        ...
        delete isWhiteList[_whiteListAddress];
	...
	/*************************
	   ...
	   These addresses can't transfer at all
	 *************************/
	address[] internal blacklist;
	mapping(address => bool) internal blacklistmap;
	
	function getBlacklist() external view onlyAdmin returns (address[] memory) {
		return blacklist;
    }
		
	function addBlacklist(address _address) public onlyAdmin {
		// If we already have this address in our whiteLists, just bail out
		if (blacklistmap[_address]) return;
		
		blacklist.push(_address);
		blacklistmap[_address]= true;
    }
	
	function addBlacklists(address[] calldata _addresses) public onlyAdmin {
		for (uint256 i = 0; i < _addresses.length; i++) {
			addBlacklist(_addresses[i]);
		}
	}
	
	function removeBlacklist(address _address) public onlyAdmin {
		if (!blacklistmap[_address]) return;
		
        delete blacklistmap[_address];
	...
	function removeBlacklists(address[] calldata _addresses) public onlyAdmin {
		for (uint256 i = 0; i < _addresses.length; i++) {
	...
	/*************************
	   Locking support 
	   While the contract is locked, only whitelisted addresses can transfer
	 *************************/
	// Whether transfers are disabled or not
    bool internal contractLocked;
	
	function getProtection() public view onlyGrantor returns (bool) {
		return contractLocked;
	}

	function setProtection(bool _contractLocked) public onlyGrantor {
     ...
     */
    function getAvailableAmount(address _beneficiary, uint256 _onDay)
        public
        view
        returns (uint256)
    {
		if (blacklistmap[_beneficiary]) return uint256(0);
    ...
    function _fundsAreAvailableOn(
        ...
        return (_amount <= getAvailableAmount(_account, _onDay));
    }
