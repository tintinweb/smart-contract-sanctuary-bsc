...
contract StakingLP is Ownable {
  ...
  uint256 public dailyInterestRate = 274;
  ...
  modifier onlyStakeholder(uint256 _id) {
    require(_idToAddress[_id] == msg.sender, "StakingLP: Caller is not the stakeholder");
    _;
  }
  ...
  function setDailyInterestRate(uint256 _dailyInterestRate) external onlyOwner {
    require(!enabled, "StakingLP: Cannot set daily interest rate while enabled");
    require(_dailyInterestRate <= 10000, "StakingLP: Daily interest rate must be less than 100%");
    dailyInterestRate = _dailyInterestRate;
  }

  function stake(uint256 _amount) external {
    require(enabled, "StakingLP: Staking is disabled");
    require(_amount >= 1e18, "StakingLP: Amount must be >= 1 token");
    if (address(pair) != address(rewardToken)) {
      require(address(router) != address(0), "StakingLP: Router is not initialized");
    }
    ...
    returns (uint256)
  {
    ...
    if (interest >= MIN_AMOUNT) {
      uint256 pairPrice = getPairPrice(); // 0.02e18
      uint256 interestInBNB = interest.mul(pairPrice).div(1e18);
      address[] memory path = new address[](2);
      path[0] = address(wbnbToken);
      path[1] = address(rewardToken);
      uint256[] memory amounts = router.getAmountsOut(interestInBNB, path);
      interest = amounts[1];
    } else {
      interest = 0;
    }
  ...
  function transferPairToken(address _recipient, uint256 _amount)
    external
    onlyOwner
    returns (bool)
  {
