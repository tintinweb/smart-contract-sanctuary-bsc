...
contract FractionalTokenSale {
    ...
    mapping(uint256 => FSale) private _fSaleInfo;

    event FSaleCreated(uint256 saleId, uint256 indexed itemId, address seller, uint256 tokenAmt, uint256 ts);
    event FrBought(uint256 saleId, address buyer, uint256 amountBought, uint256 ts);
    event FSaleCancelled(uint256 saleId);

    /// @notice Creates a new fractional sale
    /// @param _itemId id of the item whose fractional tokens are being sold
    /// @param pricePerToken amount of BNB (including decimals) required to purchase one whole fractional token (including decimals)
    /// NOTE: Emits an event with the sale Id of the newly created sale
    function createSale(uint256 _itemId, uint256 tokenAmount, uint256 pricePerToken) external returns (uint256) {
        require(tokenAmount > 0 && pricePerToken > 0, "INVALID_PARAM");

        (address frToken, , ) = _fSale.getFractionInfo(_itemId);
        require(IERC20(frToken).balanceOf(msg.sender) >= tokenAmount, "INSUFFICIENT_BALANCE");
        require(IERC20(frToken).allowance(msg.sender, address(this)) >= tokenAmount, "INSUFFICIENT_ALLOWANCE");
        ...
        emit FSaleCreated(currentId, _itemId, msg.sender, tokenAmount, block.timestamp);

        return currentId;
    }

    /// @notice Cancels an active sale
    /// @param saleId id of the sale to cancel
    /// NOTE: Caller must be the seller or marketplace owner
    function cancelSale(uint256 saleId) external {
        require(msg.sender == _fSaleInfo[saleId].seller || msg.sender == _market.owner(), "INVALID_CALLER");
        delete _fSaleInfo[saleId];

        emit FSaleCancelled(saleId);
    }

    /// @notice Modify contract addresses --- Owner only
    function setContracts(address marketplaceAddr, address fractionalSaleAddr) external {
        require(msg.sender == _market.owner(), "ONLY_MP_OWNER");
        _market = IMarketplace(marketplaceAddr);
        _fSale = IFractionalSale(fractionalSaleAddr);
    }

    /// @notice Returns information about a given sale Id
    function getSaleInfo(uint256 saleId) external view returns (FSale memory) {
        return _fSaleInfo[saleId];
    }

    /// @notice Update the price per token for an active sale
    /// @param newPricePerToken new price in BNB (including decimals) for one whole fractional token
    function updateSalePrice(uint256 saleId, uint256 newPricePerToken) external {
        require(_fSaleInfo[saleId].seller == msg.sender, "INVALID_CALLER");
        _fSaleInfo[saleId].price = newPricePerToken;
    }

    /// @notice Returns the total number of sales that have been created
    function totalSales() external view returns (uint256) {
        return _fSaleIds.current();
    }

    /// @notice Buy fractions from a specific sale
    /// @param saleId id of the sale to buy tokens from
    /// NOTE: Caller will receive tokens in proportion to how much BNB was sent with the transaction
    function buyFractions(uint256 saleId) external payable {
        (address frToken, , ) = _fSale.getFractionInfo(_fSaleInfo[saleId].itemId);
        uint tokensBought = (msg.value * 10**18) / _fSaleInfo[saleId].price;
        if(tokensBought == 0) revert("INSUFFICIENT_BNB");
        
        require(tokensBought <= _fSaleInfo[saleId].amountOfTokens, "INSUFFICIENT_TOKENS");
        _fSaleInfo[saleId].amountOfTokens -= tokensBought;
