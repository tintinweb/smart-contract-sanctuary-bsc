    ...
    Pausable
{
    ...
    uint256 public constant xRunShoes = 0;
    uint256 public constant blindBox = 1;
    uint256 public constant maxShoesSupply = blindBox * 10**18;
    uint256 public numMintedBoxId;
    ...
    event ShoesMinted(address indexed _to, bytes32 indexed _vrfRequestId, uint256 _amount);

    string public name = "XRunShoes";
    ...
    function _setTokenURIPrefix(string memory _tokenURIPrefix) public onlyRole(DEV) {
        tokenURIPrefix = _tokenURIPrefix;
    }
    ...
    //     _mintShoes(_msgSender(), blindBox,1);
    // }
     function whitelistMintBox(uint256[] calldata _tokenIds, uint256[] calldata _supplies) external {
        ...
        for(uint i = 0; i < _tokenIds.length; i++) {
            _mintShoes(msg.sender, _tokenIds[i], _supplies[i]);
    ...
    function giftbox(address _to, uint256 _numbox) public onlyRole(DEV) {
        _mintShoes(_to, blindBox,_numbox);
    }

    // function _mintShoes(address _to, uint256 _id ,uint256 _amount) internal {
    //     require(numMintedBoxId + _amount <= maxShoesSupply, "Sold out");

    //     bytes32 requestId = _getRandomNumber();
    //     _vrfRequestIdToMintRequest[requestId] = MintRequest(_to, _amount);

    //     numMintedBoxId += _amount;
    //     boxMintedByAddress[_msgSender()] += 1;
    // }
    function _mintShoes(address _to, uint256 _id, uint256 _supply) internal {
        ...
        override
    {
        ...
        uint256[] memory shoesIds = new uint256[](request.amount);
        uint256[] memory amounts = new uint256[](request.amount);

        for (uint256 i = 0; i < request.amount; i++) {
            shoesIds[i] = (uint256(keccak256(abi.encode(randomness, i))) % numShoesTypes) + 1;
            amounts[i] = 1;
        }

        _mintBatch(request.to, shoesIds, amounts, "");

        emit ShoesMinted(request.to, requestId, request.amount);
    }
    ...
    function mintedMaxbox(address _addr) external view returns (bool) {
        return boxMintedByAddress[_addr] >= boxAllowedPerAddress;
    }

    function setboxAllowedPerAddress(uint256 _boxAllowedPerAddress)
        external
        onlyRole(DEV)
    {
        boxAllowedPerAddress = _boxAllowedPerAddress;
    }
