...
contract SkullFinanceNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(!paused, "SkullFinanceNFT: minting has not started yet");
    require(_mintAmount > 0, "SkullFinanceNFT: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "SkullFinanceNFT: you can only mint 3 at a time");
    require(lastSupply >= _mintAmount,  "SkullFinanceNFT: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "SkullFinanceNFT: total cost isn't match");
    }
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    ...
    require(_winner != address(0), "SkullFinanceNFT: zero address");
    require(_amount > 0, "SkullFinanceNFT: you have to mint at least one");
    require(lastSupply != 0,  "SkullFinanceNFT: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"SkullFinanceNFT: URI query for nonexistent token");
