...
contract Staking is ReentrancyGuard, Ownable, Runnable {
    ...
    uint256 public constant MAX_DAY_REFUND = 90;
    uint256 public constant MAX_DAY_INTEREST = 270;
    ...
    uint256 public constant SECOND_IN_DAY = 86400;
    ...
    function calculatePendingInterest(address userAddress) public view returns (uint256) {
        ...
        uint256 stakingDistanceTimestamp = allowClaimInterestTimestamp.sub(user.lastClaimInterestTimestamp);
        uint256 interestPercent = getInterestPercentSecond(user.currentDeposited) * stakingDistanceTimestamp;
        return user.currentDeposited.mul(interestPercent).div(MAX_DIVISOR);
    }
