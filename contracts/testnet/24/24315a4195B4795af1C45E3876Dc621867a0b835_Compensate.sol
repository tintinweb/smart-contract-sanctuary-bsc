...
contract Compensate is Ownable, ReentrancyGuard {
    ...
    bool public compensationDataWritten; // if users compensation data is already written into the contract, can only be written once
    address public compensationToken; // which token is used to pay users the compensation amount
    uint public currentRound; // currentRound is 1, when funds are sent first time to the contract, currentRound is 2, when funds are sent second time to the contract
    uint public totalCompensationAmountAllUsers; // total compensation amount this contract will distribute
    uint[] public fundsInRound; // how much fund is present for each round
    ...
    constructor(address _compensationToken, uint _totalCompensationAmountAllUsers) {
        compensationToken = _compensationToken; // if compensation token is busd, send busd token address
        totalCompensationAmountAllUsers = _totalCompensationAmountAllUsers; // if totalCompensationAmount the contract will distribute is 4Mil, send 4Mil
    } 

    /**
    user will call function to get their share of compensation
    */
    function compensateMe() public nonReentrant {

        UserCompensationDetail storage userCompensationDetail = usersCompensationDetails[msg.sender];
        uint amountTobeCompensated = 0;
        ...
        for(uint i = userCompensationDetail.compensatedTillRound; i < currentRound; i++){
            amountTobeCompensated = amountTobeCompensated + (userCompensationDetail.whatPercentage * fundsInRound[i])/10000000000;
        }

        require(amountTobeCompensated > 0, "already claimed");
        require(amountTobeCompensated <= userCompensationDetail.userTotalCompensationAmount, "amount exceeds");
        ...
        userCompensationDetail.remainingAmount = userCompensationDetail.userTotalCompensationAmount - amountTobeCompensated;
    
        IERC20(compensationToken).transfer(msg.sender, amountTobeCompensated); // transfer amount
     ...
     */
    function initiateNextRoundWithdrawals(uint _funds) public onlyOwner {
        currentRound++;
        fundsInRound.push(_funds);
    }

    /**
    Owner use this function to write the users compensation data into the contract
     */
    function writeUsersCompensationData(address[] memory _usersAddressList, uint[] memory _percentageList, uint[] memory _usersCompensationList) public onlyOwner {
        //require(compensationDataWritten == false, "data already written");
