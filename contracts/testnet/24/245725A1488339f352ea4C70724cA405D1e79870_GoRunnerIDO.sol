...
contract GoRunnerIDO is ERC20, ERC20Pausable, Ownable, ReentrancyGuard {
    ...
    */
    function buy() payable nonReentrant onlyEOA external returns (bool success) {
        require(block.timestamp>=1648869300 && block.timestamp<=1648869600, "GoRunnerIDO: not in IDO time");
        require(totalContributedAmount + msg.value <= 100 ether, 'GoRunnerIDO: max contribution amount 100 BNB reached');
