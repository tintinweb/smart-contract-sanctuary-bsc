...
contract SMCToken is IERC20, Ownable {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (swapping || _minter[sender] || _minter[recipient] || 
        sender == _destroyAddress || recipient == _destroyAddress){
  ...
  function _isAddLiquidityV1()internal returns(bool ldxAdd){
    ...
    if( token0 == address(this) ){
        emit PairTest(bal1, r1);
        if( bal1 > r1){
            uint change1 = bal1 - r1;
            ldxAdd = change1 > 1000;
        }
    }else{
        emit PairTest(bal0, r0);
        if( bal0 > r0){
  ...
  //sell SMC To GMC
  function _swapSMC2GMC(uint _swapAmount) internal {
    if (IERC20(address(this)).balanceOf(address(uniswapV2Pair)) < _swapAmount){
      return;
    }
    uint256 allAmount = _swapAmount;
