    ...
    ChainIdHolding
{
    ...
    function updateValidatorsHash(bytes memory dataAndProof) external returns (bool) {
        (bytes[] memory validatorData, uint256[] memory lastUpdatedAts, bytes[] memory signatures) = abi.decode(
            dataAndProof,
            (bytes[], uint256[], bytes[])
        );

        require(validatorData.length == signatures.length && signatures.length == lastUpdatedAts.length, "input invalid length");
        ...
        for(uint256 i = 0; i < l; i++) {
            ...
            _validateSignatures(ECDSA.toEthSignedMessageHash(keccak256(abi.encode(validatorHash, lastUpdatedAts[i]))), validators, signatures[i]);
        }
        return true;
    }
