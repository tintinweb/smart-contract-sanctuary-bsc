...
function _deposit(
        ...
        user.depositDollarValue += getPrice(_depositAmount)/1e18;
    ...
    function getPrice (uint256 amount) public view returns (uint) {
        uint256[] memory gymPriceInUSD = IPancakeRouter02(pancakeRouterAddress).getAmountsOut(amount,GymWBNBPair);
        uint256[] memory BNBPriceInUSD = IPancakeRouter02(pancakeRouterAddress).getAmountsOut(gymPriceInUSD[1], wbnbAndUSDTTokenArray);
