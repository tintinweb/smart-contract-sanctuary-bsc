...
contract MyToken is ERC20, Ownable {
    ...
    // Token to get fees in - USDT/TokenFeeContract
    address public immutable TokenFeeContract =
        address(0x337610d27c682E347C9cD60BD4b3b107C9d34dDd);

    uint256 public _totalSupply = 100000000 * 10**18; // 100 million
    uint256 public swapTokensAtAmount = 50000000 * 10**18; //50 millions.
    uint256 public _maxTxAmount = _totalSupply.mul(2).div(100); //2%

    uint256 public TokenFee = 2;
    uint256 public liquidityFee = 1;
    uint256 public marketingFee = 2;
    uint256 public buybackFee = 2;

    uint256 public totalFees =
        TokenFee.add(liquidityFee).add(marketingFee).add(buybackFee);

    address payable public _marketingWalletAddress =
        payable(0xF6D525ce39Bc67F1391ff71aFd7AAc9b451a7FA6);
    ...
    constructor() ERC20("Addictive Crypto Games", "ACG") {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "ACG: The router already is this address."
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "ACG: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setTokenFee(uint256 value) external onlyOwner {
        uint256 nextTotalFees = value.add(liquidityFee).add(marketingFee).add(
            buybackFee
        );
        require(nextTotalFees <= 15, "Total fees should not exceed 15%.");
        totalFees = value.add(liquidityFee).add(marketingFee).add(buybackFee);
        TokenFee = value;
    }

    function setLiquidityFee(uint256 value) external onlyOwner {
        uint256 nextTotalFees = value.add(TokenFee).add(marketingFee).add(
            buybackFee
        );
        require(nextTotalFees <= 15, "Total fees should not exceed 15%.");
        totalFees = value.add(TokenFee).add(marketingFee).add(buybackFee);
        liquidityFee = value;
    }

    function setMarketingFee(uint256 value) external onlyOwner {
        uint256 nextTotalFees = value.add(TokenFee).add(liquidityFee).add(
            buybackFee
        );
        require(nextTotalFees <= 15, "Total fees should not exceed 15%.");
        totalFees = value.add(TokenFee).add(liquidityFee).add(buybackFee);
        marketingFee = value;
    }
    ...
    function setBuybackFee(uint256 value) external onlyOwner {
        ...
        require(nextTotalFees <= 15, "Total fees should not exceed 15%.");
        totalFees = value.add(TokenFee).add(liquidityFee).add(marketingFee);
        buybackFee = value;
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "ACG: The pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "ACG: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "ACG: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "ACG: Cannot update gasForProcessing to same value"
        );
    ...
    function recoverBEP20Tokens(address _token, uint256 _amount)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 swapableFees = totalFees.sub(TokenFee); // marketingFee+buybackFee+liquidityFee.
            uint256 swapTokens = contractTokenBalance.mul(swapableFees).div(
                totalFees
            );
    ...
    function swapAndLiquify(uint256 tokens) private {
        uint256 swapableFee = totalFees.sub(TokenFee);
        uint256 halfLiquidityTokens = tokens
            .mul(liquidityFee)
            .div(swapableFee)
            .div(2);
        ...
        uint256 bnbForBuyback = newBalance.sub(bnbForLiquidity).sub(
            bnbForMarketing
        );
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividends = IERC20(TokenFeeContract).balanceOf(address(this));
        IERC20(TokenFeeContract).transfer(
            address(this), // add wallet to send busd
            dividends
        );
    }
    ...
    function checkForWhale(
        ...
        if (automatedMarketMakerPairs[to] && !_isExcludedFromWhale[from]) {
            ...
            if (_balance > _amountDueForSaleLimit) {
                uint256 allowedToSell = _balance
                    .mul(maxAllowedSalePercentage)
                    .div(100);
                require(
                    amount <= allowedToSell,
                    "Exceeding Allowed Sale Limit"
                );
            }
        }
    }

    function updateMaxAllowedSalePercentage(uint256 _percent)
        external
        onlyOwner
    {
