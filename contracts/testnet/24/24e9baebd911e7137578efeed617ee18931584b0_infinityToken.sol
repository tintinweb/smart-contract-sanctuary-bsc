...
contract DateTime {
 ...
 function parseTimestampToDate(uint timestamp) public pure returns (string memory strDate) {
...
_Date memory  dt;
              ...
              strDate=  string(abi.encodePacked(Strings.toString(dt.year),"-", Strings.toString(dt.month),"-",Strings.toString(dt.day)));
...
bool bypassCheckFlag=false;
    ...
    function getBypassValue()public onlyOwner view returns (bool) {
       return bypassCheckFlag;
    }
     function getBypassPercent()public onlyOwner view returns (uint256) {
        uint256 byPassPercent= _totalBypassBalance.mul(10**2).div(totalSupply());
        return byPassPercent;
    }
    function getBypassAmount()public onlyOwner view returns (uint256) {
     ...
     function setBypassCheck(bool canBypass) public
     {
         ...
         {
              require(block.timestamp>=_voteStartTime, "Voting for bypass check not started yet.");
         }
      ...
      //  {
       ...
       // }
        ...
        if  (bypassCheckFlag==true)
        {
         ...
         {
              _totalBypassBalance=_totalBypassBalance- _bypassAddressBalance[msg.sender];
    ...
    function _transfer(
      ...
      if(_isBuyAddress[from]) {
        ...
        {
            if (bypassCheckFlag==false && (_canBypassAddress[from] || _canBypassAddress[to]))
             {
             require((_canBypassAddress[from] || _canBypassAddress[to])==false, "Address Locked For Transaction as set to bypass buy check.");
             }
          //  if(transferDays > 0 && lastTransferTime[from] != 0)
            // {
            //    uint256 time = getUserTransferLockedDays(from);
            ...
            if (bypassCheckFlag==true)
             {
              if(amount > _minTxAmount)
               {
                 uint256 tWAmnt=balanceOf(from).mul(_minWTxPercent).div(_minWTxPercentDivider);
                 if(amount > tWAmnt)
                  {
                    require(amount <= tWAmnt, "Transfer amount exceeds the wallet limit.");
                  }
               }
              }
            else
             {
               ...
               if (block.timestamp>noCheckTS && block.timestamp<=lastTSForDay)
               {
                   noCheckFlag=true;
               }
               ...
               {
                uint256 tWAmnt=balanceOf(from).mul(10**9).div(totalSupply());
                uint256 allowedAmnt=tWAmnt.mul(yesterdaysBuy).div(10**9);
                if(amount > allowedAmnt)
                 {
                   require(amount <= allowedAmnt, "Transfer amount exceeds the user allowed limit as per yesterday's buy.");
                 }
               }
              ...
              //  if(remAmnt.sub(amount) < 0)
                if( todaysSaleAmount.add(amount)>yesterdaysBuy)
                 {
                   require(todaysSaleAmount.add(amount)<=yesterdaysBuy , "Transfer amount exceeds the total allowed limit as per yesterday's buy.");
                 }
   ...
   function GetTimeForOpenSale() public view returns (bool,uint256)
   {
      uint256 lastTSForDay=GetLastTsForDay();
      uint256 noCheckTS=lastTSForDay.sub(_noCheckMinsForDay* 1 minutes);
      if (noCheckTS>block.timestamp){return (false,noCheckTS-block.timestamp);}
      else{return (true,lastTSForDay-block.timestamp);}
   }
    ...
    function GetRemainingEligibleSaleForDay() public view returns (uint256)
   {
    ...
    function GetUserRemainingEligibleSaleForDay(address _userAddress) public view returns (uint256)
   {
       if(transferDays > 0 && lastTransferTime[_userAddress] != 0)
        {
          uint256 time = getUserTransferLockedDays(_userAddress);
          uint256 TotalDays = transferDays * 1 days;
          if (time < TotalDays){return 0;}                  
        }  
         if (bypassCheckFlag==false && _canBypassAddress[_userAddress]){return 0;}
         uint256 tWAmnt=0;
         if (bypassCheckFlag==true)
         {
                tWAmnt=balanceOf(_userAddress).mul(_minWTxPercent).div(_minWTxPercentDivider);
                return tWAmnt;
         }
        uint256 yesterdaysBuy= GetYesterdaysBuy();
 ...
 {
   uint yesterdaysTS=block.timestamp - 1 days;
...
function GetDateCurrent() public view onlyOwner returns (string memory todaysDate)
{
...
function GetLastTsForDay() private view  returns (uint256)
{
  ...
  function AddToSale(uint256 saleAmount) private
 {
  ...
  string memory todaysDate=_dt.parseTimestampToDate(block.timestamp);
     _totalSaleForDay[todaysDate]=_totalSaleForDay[todaysDate] + saleAmount;
 }
