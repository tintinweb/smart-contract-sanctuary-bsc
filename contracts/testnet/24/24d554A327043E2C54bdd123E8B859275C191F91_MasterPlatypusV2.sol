...
contract MasterPlatypusV2 {    
    ...
    */
    ...
    /**
        Reward Factor:
        _rfPTP X 1000 LP X 86400s X 10days = 1*10**18 PTP => _rfPTP = (1*10**18)/(8.64*10**8) = 1.1574*10**9
    */
    uint256 private _rfPTP = 1157400000;
    uint256 private _rfVePTP = 3000000000;
    uint256 private _rfVePTPMultiple = 100; // users can get reward maximum 100 times of staked PTP amount

    event PTPFactorUpdated(address indexed user, uint256 oldFactor, uint256 newFactor);
    ...
    function rewardFactorPTP() public view returns (uint256) {
        return _rfPTP;
    }
    ...
    function updateRewardFactorPTP(uint256 newFactor) public onlyOwner {
        require (newFactor > 0, "updateRewardFactorPTP: reward factor can not be negative");
        uint256 oldFactor = _rfPTP;
        _rfPTP = newFactor;
        emit PTPFactorUpdated(msg.sender, oldFactor, newFactor);
    }
    ...
    /**
        ...
        - rewardAmount += factor * lpAmount * (currentTimestamp - lastTimeStamp)
    ...
    */
    ...
    function unStakingLP(uint256 pid, uint256 amount) public {
        ...
        LPStakedUserInfo storage user = _lpStakedUserInfo[pid][msg.sender];
        require(user.lpAmount >= amount, "unStakingLP: insufficient amount");
        IERC20(lpTokens[pid]).transfer(msg.sender, amount);
        uint256 rewardAmount = user.rewardAmount + calcIncreasedPTPReward(user.lpAmount, user.lastTimestamp);        
    ...
    function unStakingPTP(uint256 amount) public {
        ...
        PTPStakedUserInfo storage user = _ptpStakedUserInfo[msg.sender];
        require(user.ptpAmount >= amount, "unStakingPTP: insufficient amount");
        PTP.transferWithoutFee(msg.sender, amount);
        uint256 rewardAmount = user.rewardAmount + calcIncreasedVePTPReward(user.ptpAmount, user.lastTimestamp);
        if (rewardAmount > user.ptpAmount * _rfVePTPMultiple) {
            rewardAmount = user.ptpAmount * _rfVePTPMultiple;
        }
        VePTP.transferWithoutFee(msg.sender, rewardAmount);
    ...
    function _multiClaimPTP(address u) private {
        uint256 rewardAmount;
        for (uint256 i; i < lpTokens.length; ++i) {
            LPStakedUserInfo storage user = _lpStakedUserInfo[i][u];
            rewardAmount += user.rewardAmount + calcIncreasedPTPReward(user.lpAmount, user.lastTimestamp);                       
        }
        PTP.transferWithoutFee(u, rewardAmount);
        _updateLPStakedUserInfoForMultiClaim(u);
        emit PTPClaimed(u, rewardAmount);
    }
    ...
    function _updateLPStakedUserInfoForStaking (uint256 pid, address user, uint256 amount) private {
        if (isExistLPStakedUserInfo(pid, user)) {
            LPStakedUserInfo storage u = _lpStakedUserInfo[pid][user];
            _lpStakedUserInfo[pid][user].rewardAmount += calcIncreasedPTPReward(u.lpAmount, u.lastTimestamp);
            _lpStakedUserInfo[pid][user].lastTimestamp = block.timestamp;
            _lpStakedUserInfo[pid][user].lpAmount += amount;
        } else {
            _lpStakedUserInfo[pid][user].rewardAmount = 0;
    ...
    function _updatePTPStakedUserInfoForStaking (address user, uint256 amount) private {
        if (isExistPTPStakedUserInfo(user)) {
            PTPStakedUserInfo storage u = _ptpStakedUserInfo[user];
            uint256 rAmount = u.rewardAmount + calcIncreasedVePTPReward(u.ptpAmount, u.lastTimestamp);
            if (rAmount > u.ptpAmount * _rfVePTPMultiple) {
                rAmount = u.ptpAmount * _rfVePTPMultiple;
            }
            _ptpStakedUserInfo[user].rewardAmount = rAmount;
            _ptpStakedUserInfo[user].lastTimestamp = block.timestamp;
            _ptpStakedUserInfo[user].ptpAmount += amount;
        } else {
            _ptpStakedUserInfo[user].rewardAmount = 0;
    ...
    function updateVePTP(VePTPV2 newVePtp) public onlyOwner {
    ...
    function lpStakedInfo(uint256 pid, address user) public view returns (uint256 lpAmount, uint256 rewardAmount) {
        LPStakedUserInfo storage u = _lpStakedUserInfo[pid][user];
        rewardAmount = u.rewardAmount + calcIncreasedPTPReward(u.lpAmount, u.lastTimestamp);
        lpAmount = u.lpAmount;
    }

    function multiLpStakedInfo(address u) 
        public 
        view 
        returns (
            uint256,
    ...
    {        
        ...
        for (uint i = 0; i<lpTokens.length; i++) {   
            LPStakedUserInfo storage user = _lpStakedUserInfo[i][u];
            uint256 rAmount = user.rewardAmount + calcIncreasedPTPReward(user.lpAmount, user.lastTimestamp);
            totalRewardAmount += rAmount;                       
            lpAmounts[i] = user.lpAmount;
    ...
    function ptpStakedInfo(address user) public view returns (uint256 ptpAmount, uint256 rewardAmount) {
        PTPStakedUserInfo storage u = _ptpStakedUserInfo[user];
        rewardAmount = u.rewardAmount + calcIncreasedVePTPReward(u.ptpAmount, u.lastTimestamp);
        if (rewardAmount > u.ptpAmount * _rfVePTPMultiple) {
            rewardAmount = u.ptpAmount * _rfVePTPMultiple;
        }
        ptpAmount = u.ptpAmount;
    }

    function calcIncreasedPTPReward (uint256 lpAmount, uint256 lastTimestamp) private view returns (uint256) {
        // increasedRewardAmount = _rfPTP * totalLPAmount * newStakedTime / e18
        // totalRewardAmount = originRewardAmount + increasedRewardAmount
        return _rfPTP * lpAmount * (block.timestamp - lastTimestamp) / (10**18);
    }

    function calcIncreasedVePTPReward (uint256 ptpAmount, uint256 lastTimestamp) private view returns (uint256) {
        return _rfVePTP * ptpAmount * (block.timestamp - lastTimestamp) / (10**18);
    }

    function calcVePTPReward (uint256 ptpAmount, uint256 stakingTimeSecond) public view returns (uint256) {        
        uint256 rAmount = _rfVePTP * ptpAmount * stakingTimeSecond / (10**18);
        if (rAmount > ptpAmount * _rfVePTPMultiple) {
            rAmount = ptpAmount * _rfVePTPMultiple;
        }
        return rAmount;
    }
}
...
contract PTPV2 is ERC20 {     
    ...
    uint256 private totalSupplyAmount = 300_000_000_000e18;
...
contract VePTPV2 is ERC20 {     
    using SafeMath for uint256;   
    address private _minter; // should be staking pool (MasterPlatypusV2)
    address private _owner;

    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public controlWallet = 0x4935b168eA3481E52922fA3052586afcc7EbadBb;
    uint256 public burnFee = 1;
    uint256 public controlFee = 2;
    uint256 private totalSupplyAmount = 1_000_000_000_000e18;
    
    event BurnFeeUpdated(address indexed user, uint256 oldBurnFee, uint256 newBurnFee);
    event ControlWalletUpdated(address indexed user, address indexed oldWallet, address indexed newWallet);
    event MinterChanged(address indexed minter, address indexed newMinter);
    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);    

    constructor() ERC20("Platypus VePTP", "VePTP") {
        _owner = msg.sender;
        uint256 half = totalSupplyAmount.div(2);
        uint256 anotherHalf = totalSupplyAmount.sub(half);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        require(to != address(0), "ERC20: transfer to the zero address");
        uint256 burnFeeAmount = amount.mul(burnFee).div(100);
        uint256 controlFeeAmount = amount.mul(controlFee).div(100);     
        uint256 realAmount = amount.sub(burnFeeAmount).sub(controlFeeAmount);
        super._transfer(from, to, realAmount);
        super._transfer(from, controlWallet, controlFeeAmount);
        super._transfer(from, deadWallet, burnFeeAmount);
    }
    ...
    function updateControlWallet(address newWallet) public {
        require(controlWallet != newWallet, "updateControlWallet: same address");
        address oldWallet = controlWallet;
        controlWallet = newWallet;
        emit ControlWalletUpdated(msg.sender, oldWallet, newWallet);
    }
}
