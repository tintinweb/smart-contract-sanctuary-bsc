...
// File: contracts/EnumerableArrays.sol
...
abstract contract EnumerableArrays {
    ...
    function yesterdayLotteryWinners() public view returns(address[] memory) {
    ...
    function yesterdayLotteryWinnersCount() public view returns(uint256) {
...
// free junctions -------------------------------------------------------------
    ...
    EnumerableSet.AddressSet freeJunctions;

    function getFreeJunctions() public view returns(address[] memory addressList) {
        return freeJunctions.values();
    }

}
// File: contracts/smartBinary/IOldSB.sol
...
contract SmartNFT is ERC721Enumerable, Ownable {
...
abstract contract DataStorage is EnumerableArrays {
    ...
    SmartNFT public SND;
    IERC20 public DPT;
    IOldSB public oldSmartBinary;

    uint256[] public lotteryFractions;
    ...
    function userSNDBalance(address userAddr) public view returns(uint256) {
        return SND.balanceOf(userAddr);
    }

    function userSNDTokenOfOwnerByIndex(address userAddr, uint256 index) public view returns(uint256) {
        return SND.tokenOfOwnerByIndex(userAddr, index);
    }

    function userSNDTokenByIndex(uint256 index) public view returns (uint256) {
        return SND.tokenByIndex(index);
    }

    function userSNDTokensOfOwner(address userAddr) public view returns (uint256[] memory){
        
        uint256 balanceOfUser = userSNDBalance(userAddr);
        uint256[] memory ids = new uint256[] (balanceOfUser);
        uint256 i;
        for(i = 0; i < balanceOfUser; i ++) {
           ids[i] = userSNDTokenOfOwnerByIndex(userAddr, i);
        }
        return ids;
    }
...
// File: contracts/NewSmartBinary.sol


pragma solidity ^0.8.9;


contract NewSmartBinary is DataStorage {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    constructor(
        ...
        address _SNDOwner,
        IOldSB _oldSmartBinary,
        IERC20 _DPT
    ) {
        oldSmartBinary = _oldSmartBinary;
        SND = new SmartNFT(_SNDOwner, address(this));
        DPT = _DPT; 
        owner = _backupOwner;
...
// Reward24 -------------------------------------------------------------------------------------
    ...
    function checkCanReward24() public view returns(bool) {
        require(
            _userData[msg.sender].todayPoints > 0,
...
// lottery --------------------------------------------------------------------
    ...
    function todayLotteryShare() public view returns(uint256[] memory shares, uint256 remainder) {
        ...
        uint256 len = lotteryFractions.length;
        shares = new uint256[](len);
        for (uint256 i; i < len; i++) {
            shares[i] = lotteryValue / lotteryFractions[i];
            lotteryValue = lotteryValue % lotteryFractions[i];
        }
    ...
    function _rewardLottery() internal {
        ...
        if(lotteryValue > 0 && todayLotteryCandidatesCount() > 0) {
            ...
            for (uint256 i; i < lotteryFractions.length; i++) {
                lotteryFraction = lotteryFractions[i];
...
// upload old users -------------------------------------------------------------
    ...
    function uploadOldUsers(
        ...
        freeJunctions.add(userAddr);
        if(_userData[upAddr].childs == 2) {
            freeJunctions.remove(upAddr);
        }
    }

    function registerAsOldUser(
        address upAddr
    ) public {
        address userAddr = msg.sender;
        ...
        require(
            oldSmartBinary.User_Upline(userAddr) != address(0),
            "You have not registered in old smart binary contract!"
        );
        if(freeJunctions.length() != 0) {
            require(
                freeJunctions.contains(upAddr),
                "you have to select the up address between freeJunctions"
            );
            if(_userData[upAddr].childs < 0) {
                freeJunctions.remove(upAddr);
            }
