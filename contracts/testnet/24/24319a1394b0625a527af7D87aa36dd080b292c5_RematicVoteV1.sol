...
contract RematicVoteV1 is ReentrancyGuard, Ownable {
    ...
    function setSecretKey(string calldata _secretKey) external onlyOwner {
    ...
    function _signatureHash(address _addr, uint256 _pollId)
        public
        view
        returns (bytes32)
    {
        return
            keccak256(
                abi.encodePacked(SECRET_KEY, _addr, uintToString(_pollId))
            );
    }
    ...
    function voteQuestions(
        ...
        require(
            _signatureHash(voter, _pollId) == _signature,
