...
// Main Contract Logic 
// --------------------------------------------------------------
contract Einar is Context, IERC20, Ownable {
    // Imports
    using SafeMath for uint256;
    ...
    // Context
    string private _name = "Einar";
    string private _symbol = "EINAR";
    uint8 private _decimals = 18;

    // Supply 
    uint256 private _totalSupply = 1 * 1e9 * 1e18;    
    uint256 private minimumTokensBeforeSwap = _totalSupply * 25 / 100000; // 0.025% swap tokens amount    

    // Restrictions
    uint256 public _maxTxAmount = (_totalSupply * 3) / 100; //max tx limit of 3%      
    uint256 public _walletMax = (_totalSupply * 3) / 100; //max wallet holding of 3% 
    bool public checkWalletLimit = true;
    
    // wallets
    address payable public liquidityWallet = payable(0xB8EF3054f4639f766458f8bEbb5ee9736e1D69A0);
    address payable public operationsWallet =  payable(0x5d410E4857e827c2bB8E6Fa59a7257a6905EeD67);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            tokensForLiquidity += feeAmount * liquidityFeeBuy / totalFeesBuy;
            tokensForOperations += feeAmount * operationsFeeBuy / totalFeesBuy;
        }
        else if(isMarketPair[recipient]) {
            feeAmount = amount.mul(totalFeesSell).div(100);
            tokensForLiquidity += feeAmount * liquidityFeeSell / totalFeesSell;
            tokensForOperations += feeAmount * operationsFeeSell / totalFeesSell;
        }
        if(feeAmount > 0) {
    ...
    function swapAndLiquify(uint256 swapAmount) private lockTheSwap {
        ...
        // determine liq and swap tokens
        uint256 tokensForLP = contractBalance * tokensForLiquidity / totalTokensToSwap / 2;
        uint256 tokensForSwap = swapAmount.sub(tokensForLiquidity);     
        ...
        // determine split        
        uint256 ethForOperations = amountReceived.mul(tokensForOperations).div(totalTokensToSwap - (tokensForLiquidity/2));
        uint256 ethForLiquidity = amountReceived - ethForOperations;

        // add liquidity    
        if(tokensForLP > 0 && ethForLiquidity > 0){
        ...
        if(ethForOperations > 0)
            transferToAddressETH(operationsWallet, address(this).balance);
    }
