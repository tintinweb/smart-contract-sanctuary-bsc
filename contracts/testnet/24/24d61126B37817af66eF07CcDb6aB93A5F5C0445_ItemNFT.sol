...
contract ItemNFT is BaseNFT {
  ...
  function getStrengthIndexReferences(uint16 _classId) external view returns (uint, uint, uint, uint, uint, uint) {
    return (
  ...
  function addItemClass(uint8 _gene, uint16 _classId, address _creator, uint[6] calldata _strengths) external onlyMainAdmin {
  ...
  function updateFailedUpgradingAmount(uint _tokenId, uint _amount) override onlyUpgradingContract external {
    Item storage item = items[_tokenId];
