...
interface IStrategy {
    ...
    function iTokenExchangeRate(address tokenAddress) external view returns (uint256);
...
contract PlanetFarm is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    event GammaTrollerChanged(GammaTroller, GammaTroller);
    event isDripChanged(bool oldStatus,bool newStatus);
    event AutoStakeGamma(bool isAutoStaked);
   ...
   function stakeGammaOnInfinityvault(uint _gammaAmount) internal {
    ...
    function _updateUserAndPool(UserInfo storage _user, PoolInfo storage _pool, uint256 _shares, bool _isDeposit) private {
        ...
        uint256 newFactor = _getUserFactor(newShares, IERC20(iGammaAddress).balanceOf(_msgSender()));
    ...
    function _distributePendingRewards(UserInfo storage _user, PoolInfo storage _pool) private {
        ...
        console.log("see usersClaimableGamma");
    ...
    function updateFactorByItoken(address _sender, address _recepient, uint256  _amount) external {
        require(_msgSender() == iGammaAddress, "unauthorised access");
    ...
    function updateFactorInternal(address _user, uint256 _iGammaBalance) internal {
        ...
        for(uint256 i = 0; i < len; ++i) {
            ...
            uint256 newFactor = _getUserFactor(shares, _iGammaBalance);
   ...
   // Withdraw LP tokens from Strategy.
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {    
        PoolInfo storage pool = poolInfo[_pid];
        require(!pool.isInfinity, "Unstake or instantUnskate to withdraw from infinity pools");
        updatePool(_pid);
        UserInfo storage user = userInfo[_pid][_msgSender()];
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        PoolInfo storage pool = poolInfo[_pid];
        require(!pool.isInfinity , "Should be a normal pool.");
        UserInfo storage user = userInfo[_pid][_msgSender()];
    ...
    function setGammaTroller(GammaTroller _gammaTroller) external onlyOwner {
        GammaTroller oldGammaTroller = GammaTrollerAddress;
        GammaTrollerAddress = _gammaTroller;
        emit GammaTrollerChanged(oldGammaTroller, GammaTrollerAddress);
    }
    ...
    function getUserITokenToBeUnstaked(address account, uint256 pid) external view returns(uint256) {
        ...
        return user.iTokenToBeUnstaked;
    }
    ...
    function startUnstakeProcess(uint256 _pid, uint256 unstakeAmount) external notContract{
        PoolInfo storage pool = poolInfo[_pid];
        require(pool.isInfinity, "Not an infinity Pool");
        UserInfo storage user = userInfo[_pid][_msgSender()];
        ...
        uint256 exchangeRate = IStrategy(pool.strat).iTokenExchangeRate(pool.iToken);
    ...
    function stopUnstakeProcessByIToken(address account, uint256 _pid) external notContract {
        PoolInfo storage pool = poolInfo[_pid];
        require((pool.iToken == _msgSender()),"unauthorised access");
    ...
    function unstakeInstantly(uint256 _pid, uint256 unstakeAmount ) external notContract{
        ...
        uint256 exchange_rate_before_withdraw  = IStrategy(pool.strat).iTokenExchangeRate(pool.iToken);
