...
contract TestToken2 is Context, IBEP20, Ownable {
    ...
    uint _totalAmountExcludedFromDividends = 0;
    uint _totalLiq = 1; //avoid division by zero
    ...
    function withdrawLiquidity()external returns(uint256 liquidityAmount) {
        _updateAccountDividends(_msgSender());
        liquidityAmount = _accounts[_msgSender()].liqBalance;
        if(liquidityAmount > 0){ 
            IBEP20(_pcsPair).transfer(_msgSender(), liquidityAmount); 
            _totalLiq -= liquidityAmount;
        }
    }
