...
// File: contracts/dualRoles/wrap/WrapNFT721Enumerable.sol
...
contract WrapNFT721Enumerable is ERC721,IERC_DualRoles,IWrapNFT{
    ...
    constructor(string memory name_, string memory symbol_,address originalAddress_) ERC721(name_,symbol_){
        initOriginalAddress(originalAddress_);
    }

    function setUser(uint256 tokenId, address user, uint64 expires) public {
        require(_isApprovedOrOwner(msg.sender, tokenId),"ERC721: transfer caller is not owner nor approved");
        require(expires > block.timestamp,"Error:expires <= block.timestamp");
    ...
    function userOf(uint256 tokenId)public view virtual returns(address user){
        if(_userEndTime[tokenId] > block.timestamp){
            user = _users[tokenId]; 
        }
        if(user == address(0)){
