...
contract FuntapMarketplace is Ownable, TokenRecipient{
        ...
        adminOnly()
    {
        ...
        address tokenContractAddress = contracts[0];
        ...
        if(tokenContractAddress == address(0)){
            ...
            transferETH(addrs[1], feeValue);
            if(addrs[3] != address(0) && addrs[3] == addrs[5]){
                transferETH(addrs[3], sellValue + royaltyValue);
            }else{
                transferETH(addrs[3], sellValue);
                transferETH(addrs[5], royaltyValue);
            } 
        }else{
            if(keccak256(bytes(strs[0])) == keccak256(bytes("BUY"))) {
                ...
                tokenContract = BEP20(tokenContractAddress);
                require(tokenContract.balanceOf(payable(address(this))) >= totalValue, "not enough token balance");
            }else{
                require(tokenContractAddress != address(0), "can not bid/offer for currency");
            }

            transferTokens(tokenContractAddress, fromFee, addrs[1], feeValue);
            if(addrs[3] != address(0) && addrs[3] == addrs[5]){
                transferTokens(tokenContractAddress, fromValue, addrs[3], sellValue + royaltyValue);
            }else{
                transferTokens(tokenContractAddress, fromValue, addrs[3], sellValue);
                transferTokens(tokenContractAddress, fromRoyalty, addrs[5], royaltyValue);
            } 
        }
        ...
        if(contracts[1] != address(0)){
            ...
            if(uintTokens[1] == 0){
                nftContract.safeTransferFrom(addrs[6], addrs[7], uintTokens[0], uintTokens[2]);
            }else{
    ...
    function breed(
        ...
        adminOnly()
    {
        address addressToken = addrs[0];
        address addressNft = addrs[1];
        require(addressToken != address(0) && addressNft != address(0), "invalid contract address");

        transferTokens(addressToken, addrs[2], addrs[3], feeValue);

        nftContract = ERC1155(addressNft);
        if(burnIds.length == 0){
        ...
        adminOnly()
    {
        ...
        address contractToken = addrs[0];
        address addFromFee = addrs[2];
        address addToFee = addrs[3];
        address addFromValue = addrs[4];
        address addFromRoyalty = addrs[6];
        address addToValue = addrs[5];
        address addToRoyalty = addrs[7];


        if(contractToken == address(0)){
            require(keccak256(bytes(strs[0])) == keccak256(bytes("BUY")),"invalid sale type");
            require(address(this).balance >=totalValue, "not enough eth balance");

            transferETH(addToFee, feeValue);
            if(addToValue != address(0) && addToValue == addToRoyalty){
                transferETH(addToValue, sellValue + royaltyValue);
            }else{
                transferETH(addToValue, sellValue);
                transferETH(addToRoyalty, royaltyValue);
            }
        }else{
            if(keccak256(bytes(strs[0])) == keccak256(bytes("BUY"))) {
                addFromFee = address(this);
                addFromValue = address(this);
                addFromRoyalty = address(this);

                tokenContract = BEP20(contractToken);
                require(tokenContract.balanceOf(payable(address(this))) >= totalValue, "not enough token balance");
            }else{
                require(contractToken != address(0), "can not bid/offer for currency");
            }

            transferTokens(contractToken, addFromFee, addToFee, feeValue);
            if(addToValue != address(0) && addToValue == addToRoyalty){
                transferTokens(contractToken, addFromValue, addToValue, sellValue + royaltyValue);
            }else{
                transferTokens(contractToken, addFromValue, addToValue, sellValue);
                transferTokens(contractToken, addFromRoyalty, addToRoyalty, royaltyValue);
            } 
        }
            
        if(addrs[1] != address(0)){
            nftContract = ERC1155(addrs[1]);
            nftContract.safeBatchTransferFrom(addrs[8], addrs[9], uintIds, uintAmounts,"0x0");
        }
    }
        ...
        internal
    {
        if (amount > 0 && to != address(0)) {
            if(from == address(this)){
                if(from != to && isSystemWallet[to] == false){
                    require(BEP20(token).transfer(to, amount));
                }
            }else{
                if(to == address(this) || isSystemWallet[to] == true){
                    require(BEP20(token).transferFrom(from, address(this), amount));
                }else{
                    require(BEP20(token).transferFrom(from, to, amount));
                }
            }
        }
    }
    ...
    function transferETH(address to, uint amount)
        internal
    {
        if (amount > 0 && to != address(0) && to != address(this) && isSystemWallet[to] == false) {
