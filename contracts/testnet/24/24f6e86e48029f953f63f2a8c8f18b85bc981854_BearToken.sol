...
abstract contract Governance is ERC20 {
    ...
    function delegateBySig(
        ...
        require(
            ...
            "BSCToken::delegateBySig: signature expired"
        );
...
contract BearToken is Ownable, Governance {
    using SafeMath for uint256;
    using Address for address;

    uint8 private constant _decimals = 6;

    uint256 private constant _initialSupply = 100 * 10**4 * 10**_decimals;

    bool public isAntiWhale;
    uint256 public maxBuy = 300 * 10**uint256(_decimals);
    uint256 public maxSell = 300 * 10**uint256(_decimals);

    uint256 public antiWhaleStart; // timestamp
    uint256 public antiWhaleEnd; // timestamp
    ...
    event RescueStuckErc20(address _address, uint256 _amount);
    ...
    function antiWhale(
        ...
        if (
            antiWhaleStart < curTime &&
            curTime < antiWhaleEnd &&
        ...
        if (
            ...
            // anti bot sell
            if (_recipient == pancakeLiquidPair) {
                if (_amount > maxSell) {
    ...
    function setAntiWhaleTime(uint256 _antiWhaleStart, uint256 _antiWhaleEnd)
        external
        onlyOwner
    {
    ...
    function rescueStuckErc20(address _token) external onlyOwner {
        ...
        emit RescueStuckErc20(_token, _amount);
    }
