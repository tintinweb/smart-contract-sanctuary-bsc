...
contract Pool is Initializable, PoolStorage, OwnableUpgradeable, ReentrancyGuardUpgradeable, IPool {
    ...
    function _calcFeeRate(
        ...
        uint256 _currentValue = _tokenPrice * _getTokenPoolAmount(_token);
    ...
    function _getTokenPoolAmount(address _token) internal view returns (uint256 amount) {
        for (uint256 i = 0; i < allTranches.length;) {
            amount += trancheAssets[allTranches[i]][_token].poolAmount;
            unchecked {
                ++i;
            }
        }
