...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        require(_msgSender() == _owner || ERC721.isApprovedForAll(_owner, _msgSender()),
    ...
    */
    ...
    function mintAndTransfer(address from, address to, address[] memory _royaltyAddress, uint256 _royaltyFee, string memory _tokenURI, bytes memory data) external virtual override returns(uint256){
        require(msg.sender == transferProxy,"ERC721: caller is not Approved");
        uint256 itemId = tokenCounter;
        uint[] memory royaltyFeeArr = new uint[](_royaltyAddress.length);
        uint256 royaltyPerUser = _royaltyFee / _royaltyAddress.length;
        for (uint i=0; i<_royaltyAddress.length; i++){
            royaltyFeeArr[i] = royaltyPerUser;
        }
        _safeMint(from, itemId, _royaltyAddress, royaltyFeeArr);
...
contract NFTTNNSPRO721v3 is ERC721 {
    ...
    function createCollectible(string memory tokenURI, address[] memory royalty, uint256 royaltyFee, Sign memory sign) public returns (uint256) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        
        require(royalty.length > 0 , "Royalty Cannot be Empty");
        uint256 newItemId = tokenCounter;
        verifySign(tokenURI, msg.sender, sign);

        uint[] memory royaltyFeeArr = new uint[](royalty.length);
        uint256 royaltyPerUser = royaltyFee / royalty.length ;
        for (uint i=0; i<royalty.length; i++){
            royaltyFeeArr[i] = royaltyPerUser;
        }
        
        _safeMint(msg.sender, newItemId, royalty, royaltyFeeArr);
