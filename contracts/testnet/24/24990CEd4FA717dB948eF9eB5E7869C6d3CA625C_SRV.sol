...
// DONE: Send SRV directly to feeRecipient, do not keep them on this wallet

// DONE: When the balance of feeRecipient goes above 25,000 SRV, do the following:
// DONE: #1: Liquify 5%, add liquidity (on behalf of who?)
// DONE: #2a: If SRV is under $0.01, send the remaining 95% to the reward wallet
// DONE: #2b: If SRV is above $0.01, swap the remaining 95% for BUSD (keep it on feeRecipient)

// DONE: Add a switch for the system above, so it can be enabled / disabled at any time

// DONE: Clean up - add manualSwap, manualLiquify
// DONE: Add remaining setters

// TODO: Test
// TODO: Test contract with 0 selling fees
// TODO: After testing: implement a similar logic on the marketplace contract. Key difference will be the inability to use "address(this)" when referring to SRV address, which is not a big deal.
// TODO: Readd ERC20Votes
...
// BNB / BUSD pancake pair
// * Mainnet: token0: WBNB, token1: BUSD
// ! Testnet: token0: BUSD, token1: WBNB
...
contract SRV is ERC20, AccessControl {
  ...
  address private feeRecipient; // Fees are sent to this address // TODO: This was payable, should it be?
  ...
  constructor(
    ...
    _exemptFromFees[rewardDispenser] = true;
  }
  ...
  function _transfer(
    ...
    if (allowProcessingFees) {
      if (!swapping && marketingFeeBalance > srvThresholdToProcessFees) {
        // If the fee balance is above a certain amount, swap it for BNB and transfer it to the fee recipient

        // Perform the swap
        _swapFees();
      } else if (!liquifying && liquidityFeeBalance > srvThresholdToProcessFees) {
        // Same logic as above for liquidity fees
  ...
  function _swapFees() internal {
    ...
    // If the price is under limitPrice, send SRV to rewardDispenser
    if (srvPrice < limitPrice)
      super._transfer(address(this), rewardDispenser, marketingFeeBalance);
  ...
  function getSrvPriceInUsd() public view returns (uint256) {
    ...
    // @dev Mainnet: token0: WBNB, token1: BUSD
    // @dev Testnet: token0: BUSD, token1: WBNB
  ...
  function setRewardDispenserAdmin(address _rewardDispenser) external onlyRole(ADMIN) {
    rewardDispenser = _rewardDispenser;
  }
