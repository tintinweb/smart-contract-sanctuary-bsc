...
contract StakePoolTokenForBNB is States, Ownable {
    ...
    function setData(uint256 _apy, uint256 _days, address _rewardingTokenWallet, uint256 _maxReward) public payable onlyOwner {
        apyTier = _apy;
        lockPeriod = _days;
        ...
        if(rewardbalance > 0){
          require(_maxReward >= rewardbalance + msg.value, "not enough Balance");
    ...
    function checkReward() public view returns (uint256){
        ...
        User memory cUser = deposit[_msgSender()];
        require(block.timestamp + 1 seconds > cUser.lastClaimTime, "Time");
