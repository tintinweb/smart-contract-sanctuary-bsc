...
contract Marketplace is Ownable, Pausable, IMarketplace {
    ...
    // mapping(address => mapping(uint256 => Bid)) public bidByOrderId;    kpr_bid
    ...
    function ExecuteOrder(
        ...
        bool success_transform_fee = IERC20(order.acceptedToken).transferFrom(msg.sender, address(this), saleShareAmount);
        require(success_transform_fee == true, "You should pay buy fee");

        bool success_transform_price = IERC20(order.acceptedToken).transferFrom(msg.sender, order.seller, _price);
        require(success_transform_price, "You should pay price");
    ...
    */
    ...
    function multiSale(
        ...
        Order memory order = _getValidOrder(nftAddressArray[0], assetIdArray[0]);
        uint256 count_price = 0;
        for(uint256 i = 0; i < priceArray.length; i++){
            count_price += priceArray[i];
        }

        uint256 multiSaleFeeShareAmount = count_price.mul(buyFeePerMillion).div(1e6);
        uint256 total_price = count_price + multiSaleFeeShareAmount;

        uint256 approved_price =IERC20(order.acceptedToken).allowance(msg.sender, address(this));
        require(approved_price >= total_price, "The approved amount should be more than total price");

        bool suc_transform_fee = IERC20(order.acceptedToken).transferFrom(msg.sender, address(this), total_price);
        require(suc_transform_fee == true, "You should pay muli-buy fee");
        bool suc_transform_price = IERC20(order.acceptedToken).transferFrom(msg.sender, address(this), count_price);
        require(suc_transform_price == true, "You shoud pay total price");
