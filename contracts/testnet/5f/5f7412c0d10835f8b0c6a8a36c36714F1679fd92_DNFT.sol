...
contract MergeWorker is Initializable, ReentrancyGuardUpgradeable {
    ...
    function temporaryMerge(
        ...
        if (!verifyMergeSignature(_signer, _tokenIds, _timestamp, _sessionId, _signature)) revert INVALID_SIGNATURE();
    ...
    function executeTemporaryMerge(uint256[] memory _tokenIds, address _sender)
        external
        nonReentrant
        onlyAdmin
        returns (bool)
    ...
    function verifyMergeSignature(
        ...
        string memory mergeSet = MergeHelper.generateMergeSet(_tokenIds);
        return (Whitelist.verifySignatureWhenMergeDNFT(_signer, mergeSet, _timestamp, _sessionId, _signature));
    }
}
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function temporaryMerge(
        ...
        mergeWorker.temporaryMerge(msg.sender, signer, _tokenIds, _timestamp, _sessionId, _signature);
    ...
    function cancelTemporaryMerge(
        ...
        mergeWorker.cancelTemporaryMerge(msg.sender, signer, _tokenIds, _timestamp, _sessionId, _signature);
    ...
    function executeTemporaryMerge(uint256[] memory _tokenIds) external whenNotPaused nonReentrant {
        bool success = mergeWorker.executeTemporaryMerge(_tokenIds, msg.sender);
    ...
    function permanentMerge(
        ...
        bool success = mergeWorker.permanentMerge(msg.sender, signer, _tokenIds, _timestamp, _sessionId, _signature);
        if (!success) revert NOT_OWNER_OF_TOKEN_OR_TOKEN_IS_LOCKED();
    ...
    function verifyMergeSignature(
        ...
        string memory mergeSet = MergeHelper.generateMergeSet(_tokenIds);
        return (Whitelist.verifySignatureWhenMergeDNFT(signer, mergeSet, _timestamp, _sessionId, _signature));
    }
