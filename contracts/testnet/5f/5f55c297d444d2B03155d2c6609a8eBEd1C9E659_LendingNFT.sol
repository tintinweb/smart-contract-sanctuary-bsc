...
contract LendingNFT is AccessControl, ILendingNFT, ReentrancyGuard, Pausable, ERC721Holder
{
  ...
  event CLAIMPRICE(address lender, uint256 orderId, uint8 nftType, uint256 nftId, uint256 receivedPrice, uint256 timeStamp);
  event CLAIMNFT(address lender, uint256 orderId, uint8 nftType, uint256 nftId, uint256 timeStamp);
  ...
  function setCommissionPercent(uint32 _value) external onlyAdmin {
    ...
    require(_value > 0 && _value < 100 * info.percentDecimals, "VALUE_INVALID");
  ...
  function paymentOrder(
    ...
    uint256 commissionPrice = (rentalPrice * info.commissionPercent) / (100 * info.percentDecimals);
    uint256 receivedPrice = rentalPrice - commissionPrice;
    _payment(info.transactionWallet, info.commissionWallet, info.currencyToken, borrower, receivedPrice, commissionPrice);
  ...
  function lenderRewardClaim(
    ...
    if (orderItem.status == statusPaid && orderItem.expiredTime < block.timestamp) {
      _storageAddress.updateOrderItem(_nftId, _nftType, statusExpired, 0);
    }

    _storageAddress.updateOrderItem(_nftId, _nftType, uint8(LendingStorage.OrderStatus.IsClaimPrice), 0);
    uint256 rentalPrice = orderItem.dailyRentalPrice * orderItem.rentalDuration;
    uint256 receivedPrice = (rentalPrice * ((100 * info.percentDecimals) - info.commissionPercent)) / (100 * info.percentDecimals);
    TransferHelper.safeTransferFrom(info.currencyToken, info.transactionWallet, msg.sender, receivedPrice);

    emit CLAIMPRICE(msg.sender, orderItem.id, _nftType, _nftId, receivedPrice, block.timestamp);
  }
  ...
  function lenderClaimNFT(
    ...
    emit CLAIMNFT(msg.sender, orderItem.id, _nftType, _nftId, block.timestamp);
  }
...
contract LendingStorage is AccessControl, ReentrancyGuard, Pausable {
  ...
  mapping(uint256 => Counters.Counter) _orderIds;
  mapping(uint256 => mapping(uint256 => OrderERC4907)) orders;
  mapping(string => bool) _orderUUids;
  ...
  function setMaxWaitingTime(uint32 _minutes) external onlyAdmin {
    require(_minutes > 0 && _minutes <= 60, "VALUE_INVALID");
    _maxWaitingTime = _minutes * 60;
  }

  function setPercentDecimals(uint32 _decimals) external onlyAdmin {
