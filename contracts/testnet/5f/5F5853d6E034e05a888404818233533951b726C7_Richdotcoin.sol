...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    function stakeTreasury(uint256 _amount) external updateReward(msg.sender) {
        ...
        _treasuryAmount[msg.sender] = _amount + remainingtreasuryAmount[msg.sender];
        _treasuryWithdrawn[msg.sender] = 0;
        treasuryClaim[msg.sender] = treasuryClaim[msg.sender] + _treasuryAmount[msg.sender]*treasuryRATE/100;
    ...
    function treasuryRewards(address _account) public view override returns (uint256) {
        uint256 rewardearned = _treasuryAmount[_account]*treasuryRATE/(100*duration) * (block.timestamp - startAt[_account]) - _treasuryWithdrawn[_account];
        if (block.timestamp <= finishAt[_account]) return rewardearned;
        return treasuryClaim[_account];
    }
