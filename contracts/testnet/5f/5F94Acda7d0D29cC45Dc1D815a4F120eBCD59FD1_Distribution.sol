...
contract Distribution is Vesting {
    ...
    uint16 private constant _publicSalePercent = 20;
    ...
    uint16 private constant _stakingPercent = 10;
    ...
    /**
     * @dev Returns distribution addresses
     */
    ...
    /**
     * @dev Returns distribution percents
     */
    ...
    {
        uint256 earlyAdoptersBalance = _token.balanceOf(_earlyAdoptersAddress);
        uint256 privateSaleBalance = _token.balanceOf(_privateSaleAddress);
        uint256 publicSaleBalance = _token.balanceOf(_publicSaleAddress);
        uint256 developmentBalance = _token.balanceOf(_developmentAddress);
        uint256 marketingBalance = _token.balanceOf(_marketingAddress);
        uint256 reservesBalance = _token.balanceOf(_reservesAddress);
        uint256 ecosystemBalance = _token.balanceOf(_ecosystemAddress);
        uint256 liquidityBalance = _token.balanceOf(_liquidityAddress);
        uint256 teamBalance = _token.balanceOf(_teamAddress);
        uint256 advisoryBalance = _token.balanceOf(_advisoryAddress);
        uint256 stakingBalance = _token.balanceOf(_stakingAddress);
    ...
    function distribute() public onlyOwner {
        ...
        _token.safeTransfer(_earlyAdoptersAddress, earlyAdoptersTotal);

        // Private Sale
        uint256 privateSaleTotal = totalSupplyPercent.mul(_privateSalePercent);
        _token.safeTransfer(_privateSaleAddress, privateSaleTotal);

        // Public Sale
        uint256 publicSaleTotal = totalSupplyPercent.mul(_publicSalePercent);
        _token.safeTransfer(_publicSaleAddress, publicSaleTotal);

        // Development
        uint256 developmentTotal = totalSupplyPercent.mul(_developmentPercent);
        vestingCreateSchedule({
            _name: "Development",
            _beneficiary: _developmentAddress,
            _start: _startDate,
        ...
        // Marketing
        uint256 marketingTotal = totalSupplyPercent.mul(_marketingPercent);
        vestingCreateSchedule({
            _name: "Marketing",
            _beneficiary: _marketingAddress,
            _start: _startDate,
        ...
        // Reserves
        uint256 reservesTotal = totalSupplyPercent.mul(_reservesPercent);
        vestingCreateSchedule({
            _name: "Reserves",
            _beneficiary: _reservesAddress,
            _start: _startDate,
        ...
        // Ecosystem
        uint256 ecosystemTotal = totalSupplyPercent.mul(_ecosystemPercent);
        _token.safeTransfer(_ecosystemAddress, ecosystemTotal);

        // Liquidity
        uint256 liquidityTotal = totalSupplyPercent.mul(_liquidityPercent);
        _token.safeTransfer(_liquidityAddress, liquidityTotal);

        // Team
        uint256 teamTotal = totalSupplyPercent.mul(_teamPercent);
        vestingCreateSchedule({
            _name: "Team",
            _beneficiary: _teamAddress,
            _start: _startDate,
        ...
        // Advisory
        uint256 advisoryTotal = totalSupplyPercent.mul(_advisoryPercent);
        vestingCreateSchedule({
            _name: "Advisory",
            _beneficiary: _advisoryAddress,
            _start: _startDate,
            _cliff: (_secondsOfMonth * 12), // the first distribution 12 months after
            _duration: (_secondsOfMonth * 24), // opens evenly for 24 months
            _slicePeriodSeconds: _secondsOfMonth, // opens every 1 month
        ...
        // Staking
        uint256 stakingTotal = totalSupplyPercent.mul(_stakingPercent);
        _token.safeTransfer(_stakingAddress, stakingTotal);
