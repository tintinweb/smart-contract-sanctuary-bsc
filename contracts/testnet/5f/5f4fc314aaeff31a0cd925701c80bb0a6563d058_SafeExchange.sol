...
contract SafeExchange is ReentrancyGuard, Ownable, TokenManagement {
  ...
  function crossChainUtility(
    ...
    uint256 _chainId,
    address _routerAddr
  ) external payable tokenAllowed(_anyToken) isUnderlying(_anyToken) nonReentrant {
    uint256 feeAmt = (msg.value * fee) / standardization;
    _anySwapOutNative(_anyToken, _to, msg.value - feeAmt, _chainId, _routerAddr);
  ...
  function crossChainToken(
    ...
    uint256 _chainId,
    address _routerAddr
  ) external tokenAllowed(_anyToken) nonReentrant {
    uint256 feeAmt = (_amt * fee) / standardization;
    // IERC20 token = IERC20(IAnyswapV1ERC20(_anyToken).underlying());
    IAnyswapV1ERC20(_anyToken).transferFrom(msg.sender, address(this), _amt - feeAmt);
    _anySwapOut(_anyToken, _to, _amt - feeAmt, _chainId, _routerAddr);
  ...
  function integrateCrossChainUtility(
    ...
    uint256 _chainId,
    address _routerAddr
  ) external payable tokenAllowed(_anyToken) isUnderlying(_anyToken) nonReentrant lockerExists(msg.sender) {
    uint256 feeAmt = (msg.value * fee) / standardization;
    ...
    require(isPassed || (tokenAmt >= tokenList[_anyToken].minAmt && tokenAmt <= tokenList[_anyToken].maxAmt), "amt must between the minimum and maximum");
  ...
  function crossChainUnderlying(
    ...
    uint256 _chainId,
    address _routerAddr
  ) external tokenAllowed(_anyToken) isUnderlying(_anyToken) nonReentrant {
    uint256 feeAmt = (_amt * fee) / standardization;
    IERC20 token = IERC20(IAnyswapV1ERC20(_anyToken).underlying());
    token.transferFrom(msg.sender, address(this), _amt - feeAmt);
    _anySwapOutUnderlying(_anyToken, _to, _amt - feeAmt, _chainId, _routerAddr);
    token.transferFrom(msg.sender, feeWallet, feeAmt);
    emit CrossChainLog(address(token), msg.sender, _to, _amt - feeAmt, _chainId);
  }
