...
contract SmartFINXInitializable is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20Metadata;

    // The address of the smart FINX factory
    address public immutable SMART_FINX_FACTORY;
    ...
    function initialize(
        ...
        require(msg.sender == SMART_FINX_FACTORY, "Not factory");
...
// File: contracts/SmartFINXFactory.sol

contract SmartFINXFactory is Ownable {
    event NewSmartFINXContract(address indexed smartFINX);
     ...
     * @param _numberBlocksForUserLimit: block numbers available for user limit (after start block)
     * @param _admin: admin address with ownership
     * @return address of new smart FINX contract
     */
    function deployPool(
        IERC20Metadata _stakedToken,
        IERC20Metadata _rewardToken,
        ...
        bytes memory bytecode = type(SmartFINXInitializable).creationCode;
        ...
        address smartFINXAddress;

        assembly {
            smartFINXAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        SmartFINXInitializable(smartFINXAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewSmartFINXContract(smartFINXAddress);
    }
}