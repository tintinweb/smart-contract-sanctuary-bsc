...
contract DuelGame is Ownable {
  address public BUSD = block.chainid==56 ? 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56 : 0x2BDF3aAb1c14B00fcBb7cf1A34241B180d4484BF;
  ...
  function createDuel(address _token, uint8 _categoryIndex, uint256 _price1) external notPaused {
    ...
    uint256 amount = amountArr[_categoryIndex] * (10 ** IERC20(_token).decimals());
    duels[duelCounter] = Duel(_token, msg.sender, address(0), amount, _price1, 0, 0, block.timestamp, 1); 
    duelCounter++;
  ...
  function createDuelForBNB(uint8 _categoryIndex, uint256 _price1) external payable notPaused {
    require(_categoryIndex<=9, "out of range");
    require(msg.value == amountArr[_categoryIndex] * 1 ether, "not matching amount");
    uint256 amount = amountArr[_categoryIndex] * 1 ether;
    duels[duelCounter] = Duel(WBNB, msg.sender, address(0), amount, _price1, 0, 0, block.timestamp, 1); 
    duelCounter++;
  }

  function cancelDuel(uint256 duelId) external {
    require(block.timestamp>=duels[duelId].createdTime+invitePeriod, "can not cancel");
    require(duels[duelId].wallet1==msg.sender || duels[duelId].wallet1==botAddress, "not matching wallet");
  ...
  function joinDuel(uint256 duelId, uint256 _price2) external notPaused {
    ...
    IERC20(duels[duelId].token).transferFrom(msg.sender, address(this), duels[duelId].depositAmount);
  }
  
  function joinDuelForBNB(uint256 duelId, uint256 _price2) external payable notPaused {
    require(block.timestamp<=duels[duelId].createdTime+invitePeriod, "expired duel");
    require(msg.sender!=duels[duelId].wallet1, "same wallet");
    require(msg.value==duels[duelId].depositAmount, "not matching amount");
  ...
  function finishDuel(uint256 duelId) external {
    require(duels[duelId].wallet2 != address(0) && duels[duelId].resultPrice==0, "condition not meet");
