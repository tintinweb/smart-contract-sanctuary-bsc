...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    function _transfer(
        ...
        // Remove fees for transfers recipient and sender account transfer or recipient excluded account
        if(!inTransfer && !inSwapAndLiquify && swapAndLiquifyEnabled && amount > 0){
            inTransfer = true;
            if(sender != uniswapV2Pair && recipient != uniswapV2Pair || _isExcluded[sender]){
                removeAllFee();
            }
            feeTokenAmount = _getTokenFeeTotal(amount);
