...
contract Ownable is Context {
    address public _owner =0xB5838B835899f37A5403B73bC170DF1Cf960B916;
...
contract SRDToken is Context, IERC20, Ownable {
    ...
    address  public operationAddress = 0xD8Bc69E336308C2eDE18CF7AfAbb6beFBf43e254;//上交易所费用 5% 3000000
    address  public pledgeAddress = 0x70bf5164109d4973D33588226453903Eb47C9f7a;//质押 30% 18000000
    // address  public genesisReceiveAddress;//创世100BNB接收地址
    address  public presaleAddress = 0xA5EA5bEB8Ff35ba0c86e5509f1092Df764d84666;//预售 25% 15000000
    address  public liquidAddress = 0xA20c49fB49169Ab72C2Ebd837d547C6a1F9C6E07;//流动 15% 9000000
    // address  public relayAddress;//手续费中转地址
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => address) public welcomeUser;
    ...
    event PledgeNFT(address account, uint256 tokenId, uint256 NFTType, uint256 types , uint256 ETHAmount, uint256 tokenAmount);
    ...
    constructor (
        ...
        isMarketPair[address(pancakeswapV2Pair)] = true;

        _balances[owner] = _totalSupply;
        payable(service).transfer(msg.value);
        emit Transfer(address(0), owner, _totalSupply);

        _balances[presaleAddress] = 15000000 * 10**18;                //预售 25% 15000000
        _balances[liquidAddress] = 9000000 * 10**18;                 //流动 15% 9000000
        // _balances[genesisPoolAddress] = 15000000 * 10**18;         //销毁 25% 15000000
        _balances[pledgeAddress] = 18000000 * 10**18;             //质押 30% 18000000
        _balances[operationAddress] = 3000000 * 10**18;            //上交易所费用 5% 3000000

        emit Transfer(owner, presaleAddress, 15000000 * 10**18);
        emit Transfer(owner, liquidAddress, 9000000 * 10**18);
        emit Transfer(owner, deadAddress, 15000000 * 10**18);
        emit Transfer(owner, pledgeAddress, 18000000 * 10**18);
        emit Transfer(owner, operationAddress, 3000000 * 10**18);

        PRN = PRNFT(0xF1cD9bcA0db10b02f119AB1ea6Ed635A2EC91d25);
    ...
    function setTeamWalletAddress(address newAddress) external onlyOwner() {
     ...
     function setPresaleAddress(address _presaleAddress) external onlyOwner {
        presaleAddress = payable(_presaleAddress);
    }

    // function setGenesisPoolAddress(address _genesisPoolAddress) external onlyOwner {
    //     genesisPoolAddress = payable(_genesisPoolAddress);
    // }

    function setPledgeAddress(address _pledgeAddress) external onlyOwner {
        pledgeAddress = payable(_pledgeAddress);
    }
    ...
    function setLiquidAddress(address _liquidAddress) external onlyOwner {
        liquidAddress = payable(_liquidAddress);
    }
    ...
    function setWelcomeUser(address _welcomeUser) public {
        welcomeUser[msg.sender] = payable(_welcomeUser);
    }

    function getWelcomeUser (address user) public view returns(address  _welcomeUser){
        _welcomeUser = welcomeUser[user];
        return _welcomeUser;
    }
    ...
    function marketingSell() private{
         if (_balances[marketingWalletAddress] >= 10000 * 10**18) {
            swapTokensForEth(5000 * 10**18);
         }
    }

    function directPush(address user,uint256 amount) public {
         address  first = getWelcomeUser(user);

         if(first != address(0x0)){
        emit Transfer(user, first, amount.mul(6).div(100));
        address  second = getWelcomeUser(first);   

         if(second != address(0x0)){
        emit Transfer(user, second, amount.mul(3).div(100));
        address  third = getWelcomeUser(second);
        
         if(third != address(0x0)){
        emit Transfer(user, third, amount.mul(1).div(100));
           }
          }
         }
     ...
     function pledgeNFT(uint256 types, uint256 tokenId) public {
        require(PRN.ownerOf(tokenId) == msg.sender, "You are not the owner");
        require(tokenIdToPledges[tokenId].joinTime == 0 && PRN.tokenIdToStatus(tokenId) == 0, "Your card is already pledged");

        require(types == 1 || types == 2, "Pledge type is wrong");

        PRN.setNFTStatus(tokenId, 2);

        uint256 NFTType = PRN.tokenIdToTypes(tokenId);

        uint256 ETHAmount = PRN.typeToNFTPrice(NFTType);
        ...
        if(types == 2){
            ...
            tokenAmount = ETHEqualToToken(ETHAmount);

            _transfer(msg.sender, pledgeAddress, tokenAmount);
        }

        tokenIdToPledges[tokenId] = Pledge(tokenId, types, ETHAmount, tokenAmount, block.timestamp, 0);

        emit PledgeNFT(msg.sender, tokenId, NFTType, types, ETHAmount, tokenAmount);
    }
    ...
    function quitPledgeNFT(uint256 tokenId) public {
        ...
        require(tokenIdToPledges[tokenId].joinTime > 0 && PRN.tokenIdToStatus(tokenId) == 2, "Your card is not pledged");

        PRN.setNFTStatus(tokenId, 0);

        uint256 NFTType = PRN.tokenIdToTypes(tokenId);

        uint256 tokenAmount = tokenIdToPledges[tokenId].SRDAmount;

        uint256 profit = getPledgeProfit(tokenId);

        if(tokenAmount + profit > 0){
            _transfer(pledgeAddress, msg.sender, tokenAmount + profit);
        }
    ...
    function withdrawPledgeProfit(uint256 tokenId) public {
        ...
        if(profit > 0){
            _transfer(pledgeAddress, msg.sender, profit);
        }
    ...
    function getMyPledgeNFT(address user) public view returns(Pledge[] memory myPledgeNFTs, uint256[] memory rates, uint256[] memory profits) {
        ...
        uint256[] memory ids = PRN.getUserPledgeNFTIDs(user);
    ...
    function ETHEqualToToken(uint256 ETHAmount) public view returns(uint256) {
        
        uint256 tokenOfPair = balanceOf(pancakeswapV2Pair);

        uint256 ETHOfPair = pancakeswapV2Pair.balance;

        uint256 amount = ETHAmount.mul(tokenOfPair).div(ETHOfPair);

        return amount;
    }

    function tokenEqualToETH(uint256 tokenAmount) public view returns(uint256) {
        
        uint256 tokenOfPair = balanceOf(pancakeswapV2Pair);

        uint256 ETHOfPair = pancakeswapV2Pair.balance;

        uint256 amount = tokenAmount.mul(ETHOfPair).div(tokenOfPair);
    ...
    function getPledgeProfit(uint256 tokenId) public view returns(uint256){
        ...
        uint256 ETHAmount = tokenIdToPledges[tokenId].ETHAmount;

        uint256 tokenAmount = tokenIdToPledges[tokenId].SRDAmount;

        tokenAmount = tokenAmount + ETHEqualToToken(ETHAmount);
    ...
    uint256 pledgeType,uint256 startTime,uint256 ETHAmount,uint256 SRDAmount,uint256 profit) {
        NFTType = PRN.tokenIdToTypes(tokenId);
        periodIndex = PRN.tokenIdToPeriod(tokenId);
        price = PRN.typeToNFTPrice(NFTType).div(10 ** 18);
        ...
        ETHAmount = tokenIdToPledges[tokenId].ETHAmount.div(10 ** 18);
        SRDAmount = tokenIdToPledges[tokenId].SRDAmount.div(10 ** 18);
...
contract PRNFT is ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => uint256) public tokenIdToPeriod;//NFT期数
	...
	constructor() ERC721("phantom Robot", "PR"){
		counter = 0;

        typeToNFTPrice[1] = 200 * 10 ** 18;

        typeToNFTPrice[2] = 500 * 10 ** 18;
