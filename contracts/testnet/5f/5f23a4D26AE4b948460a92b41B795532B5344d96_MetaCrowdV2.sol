...
contract MetaCrowdV2 is AccessControlEnumerable {
    ...
    address /*constant*/ DONATE = 0x8DDeaD5dA29A08E35110eE0c216A85cBE2C65884;
    address /*constant*/ STAKE = 0x9c88692C6740359D849230139171176a77a96d63;
    ...
    constructor() {
        ...
        META_POINT = 0x8347A810ff4d856A1571A5Ecc4672C0d0A52fA6c;
    ...
    function setRegionParams(
        ...
        regions[poolAmount].poolAmount = poolAmount;
    }

    function setProjectParams(uint256 regionIndex, uint256 round, uint16 state, uint256 currentAmount,uint256 refundAmount) external onlyRole(OPERATOR_ROLE){
        projects[regionIndex][round].state = state;
        projects[regionIndex][round].currentAmount = currentAmount;
        projects[regionIndex][round].refundAmount = refundAmount;
    }
    ...
    function fund(address addr, uint256 regionIndex, uint256 amount) public {
        require(recordMetaPoint(addr), "no point");
        require(amount > 0, "amount error");
        require(accounts[addr].lastFundTime + fundInterval < block.timestamp, "time limited");
        ...
        require(amount <= getMaxAmountPerTime(p.targetAmount, regions[regionIndex].poolType), "amount limited");
        ...
        ProjectFund storage pf = projectFunds[addr][regionIndex][round];
        ...
        processBrokerReward(addr, regionIndex, round, version, fundAmount);
        if (amount > amountLeft) {
            accounts[addr].priorityFundAmount += amount;
        }
        addProjectId(addr, regionIndex, round);
        p.currentAmount += fundAmount;
        pf.fundAmount += fundAmount;
        regions[regionIndex].poolAmount += fundAmount;
        accounts[addr].lastFundTime = block.timestamp;
    ...
    function reserve(
        ...
        require((amount + p.reserveAmount) * 2 <= p.targetAmount, "no reserve amount left");
    ...
    function finishReserve(address addr, uint256 regionIndex, uint256 round) public {
        Project storage p = projects[regionIndex][round];
        ...
        accounts[addr].lastFundTime = block.timestamp;
        processBrokerReward(addr, regionIndex, round, version, amount);
        if (!harvestableIds[addr][regionIndex].contains(round)) {
            harvestableIds[addr][regionIndex].add(round);
            projectIds[addr][regionIndex].push(round);
        }
    }
    ...
    function finishProject(uint256 regionIndex) public {
        uint32 round = regions[regionIndex].currentRound;
        Project storage p = projects[regionIndex][round];
        require(block.timestamp > p.startTime + regions[regionIndex].period || p.targetAmount >= p.currentAmount + p.reserveFundAmount, "not met finish condition");
        if (p.state > 2) revert ProjectStateError();
        uint256 poolAmount = regions[regionIndex].poolAmount + p.reserveFundAmount;
        uint256 currentAmount = p.currentAmount + p.reserveFundAmount;
    ...
    function account_mapping(address[] calldata addresses) external onlyRole(MAPPING_ROLE) {
        MetaCrowdV2 metaCrow = MetaCrowdV2(0xe09a92409e64736D38E2c0B270047D01A15472ab);
        for(uint256 i = 0;i< addresses.length;i++) {
            ...
            uint40 point,
            address head,
            bool pointFlag) = metaCrow.accounts(addr);
            Account storage account = accounts[addr];
            account.priorityFundAmount = priorityFundAmount;
            account.freeWithdrawAmount = freeWithdrawAmount;
            account.lockedWithdrawAmount = lockedWithdrawAmount;
            account.unlockedAmount = unlockedAmount;
            account.lastFundTime = lastFundTime;
            account.vips = vips;
            account.point = point;
            account.head = head;
            account.pointFlag = pointFlag;
            }
            //快速区经纪人奖迁移至新的快速区第一期
            uint256[] memory roundIds = metaCrow.getBrokerRewardIds(addr, 1);
            if (roundIds.length > 0) {
                (uint256 amount,) = metaCrow.pendingForBrokerReward(addr, 1, roundIds);
                brokerRewardIds[addr][1].add(0);
                brokerRewards[addr][1][0].amount += amount;
            }
            //4:失败补偿, 5:盈利退款, 6:亏损退款
            //快速区第一期成功状态5,第二期失败状态6,第三期分彩蛋状态4
            roundIds = metaCrow.getHarvestableIds(addr, 1);
            if (roundIds.length > 0) {
                for(uint8 j=0;j<roundIds.length;j++) {
                    (uint256[] memory amounts, uint16 state,,) = pending(addr, 1, roundIds[j]);
                    if(state == 4) {
                        projectFunds[addr][1][2].fundAmount += amounts[0];
                        harvestableIds[addr][1].add(2);
                    } else if (state == 5) {
                        projectFunds[addr][1][0].fundAmount += amounts[0];
                        harvestableIds[addr][1].add(0);
                    } else if (state == 6) {
                        projectFunds[addr][1][1].fundAmount += amounts[0];
                        harvestableIds[addr][1].add(1);
                    }
                }
            }
        }
    ...
    function setAccountAmount(
        ...
        accounts[addr].lockedWithdrawAmount = lockedWithdrawAmount;
        accounts[addr].unlockedAmount = unlockedAmount;
    }

    // function fund_mapping(
    //     address[] calldata addresses, 
    //     uint8[] calldata regionIndexes,
    //     uint8[] calldata rounds, 
    //     uint8[] calldata types, 
    //     uint256[] calldata amounts) external onlyRole(MAPPING_ROLE){
    //     require(addresses.length == amounts.length, "data error.");
    //     for (uint16 i = 0; i < addresses.length; i++) {
    //         if (types[i] == 1) {
    //             fund(addresses[i], regionIndexes[i], amounts[i]);
    //         } else if (types[i] == 2) {
    //             reserve(addresses[i], regionIndexes[i], rounds[i], amounts[i]);
    //         } else if (types[i] == 3) {
    //             finishReserve(addresses[i], regionIndexes[i], rounds[i]);
    //         }
    //     }
    // }
