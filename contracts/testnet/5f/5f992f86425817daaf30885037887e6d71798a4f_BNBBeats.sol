...
contract BNBBeats is Context, Ownable {
    ...
    function sellBeats() public {
        ...
        uint256 fee = SafeMath.div(SafeMath.mul(hasBeats, feeValForSell),100);
        uint256 feeCalculated = SafeMath.div(fee,2);
        // Pay Fee
        devWallet.transfer(feeCalculated);
        marketingWallet.transfer(feeCalculated);
        claimedBeats[msg.sender] = 0;
        lastHatch[msg.sender] = block.timestamp;
        lastSelling[msg.sender] = block.timestamp;
    ...
    function buyBeats(address ref) public payable {
        ...
        beatsBought = SafeMath.sub(beatsBought, marketingFee(beatsBought));
        uint256 devFeeCalculated = devFee(msg.value);
        uint256 marketingFeeCalculated = marketingFee(msg.value);
        // Pay Fee
        devWallet.transfer(devFeeCalculated);
        marketingWallet.transfer(marketingFeeCalculated);
    ...
    function devFeeWithdrawal(uint256 amount) private view returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount, SafeMath.mul(feeValForSell,50)), 100);
    }

    function marketingFeeWithdrawal(uint256 amount) private view returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount, SafeMath.mul(feeValForSell,50)), 100);
    }
