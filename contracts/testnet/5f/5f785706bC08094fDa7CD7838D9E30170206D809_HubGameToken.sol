...
contract HubGameToken is BEP20, Ownable {
    event event_lockSystemWallet(address _wallet, uint256 _lockedAmount, uint256 _releaseTime, uint8 _numOfPeriod, uint256 _amountInPeriod);
    ...
    uint256[] memory _gpeAmounts, uint256[] memory _releaseTimes, uint256[] memory _numOfPeriods, uint256[] memory _periodInSeconds) 
    public onlyAdmin{
        ...
        require(_gpeAmounts.length == _releaseTimes.length && 
                    _releaseTimes.length == _numOfPeriods.length &&
                    _numOfPeriods.length == _periodInSeconds.length, 
                    "mismatch locked param");
        
        for (uint256 i = 0; i < _wallets.length; i++) {
            uint256 _amount = _totalSupplyWei  * _percents[i]/100;
            _mint(_wallets[i], _amount); 
            _lockFund(_wallets[i], _amount, _gpeAmounts[i], _releaseTimes[i], _numOfPeriods[i], _periodInSeconds[i]);
    ...
    function adminRemove(address[] memory _addresses) public onlyOwner{
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    function blackListIndexOf(address _address) public view onlyOwner returns(uint256){
    ...
    function signerAdd(string memory _gameCode, address[] memory _signers) public onlyOwner {
    ...
    function signerIndexOf(string memory _gameCode, address _signer)  public view onlyOwner returns(uint256){
        require(_signer != address(0) && isSigner[_gameCode][_signer],"invalid signer");
    ...
    function signerRemove(string memory _gameCode, uint256 index) public onlyOwner {
    ...
    function getLockedAmount(address lockedAddress) internal view notInBlackList returns(uint256) {
        ...
        for(uint256 j = 0; j < items.length; j++) {
            ...
            if(block.timestamp < items[j].releaseTime) {
                uint256 temp = items[j].lockedAmount;
                lockedAmount += temp;
            }
        }
	    return lockedAmount;
    ...
    function getTransaction(uint256 _txIndex) public view returns (
        ...
        return (
            ...
            _transaction.lockParams
            );
    }

    function transactionSubmit(address _receiver, uint256 _value, uint256[] memory _lockParams) public onlySystem
    returns (uint256 _txIndex){
        ...
        emit SubmitTransaction(msg.sender, _txIndex, _receiver, _value);
        return _txIndex;
    }

    function transactionConfirm(uint256 _txIndex) public{
        Transaction storage transaction = transactions[_txIndex];
        ...
        && availableBalance(transaction.from) > transaction.value){
            ...
            if(transaction.lockParams.length > 0){
                uint256[] memory _lockParams = transaction.lockParams;
                _lockFund(transaction.to, transaction.value, _lockParams[0], _lockParams[1], _lockParams[2], _lockParams[3]);
            }
    ...
    function _lockFund(address _receiver, uint256 _amount, uint256 _gpeAmount, uint256 _releaseTime, uint256 _numOfPeriod, uint256 _periodInSecond) internal{
        uint256 _lockedAmount = _amount - _gpeAmount;
        uint256 _amountInPeriod = _lockedAmount/_numOfPeriod;
        LockItem memory item = LockItem({
            releaseTime: _releaseTime, 
            lockedAmount: _lockedAmount, 
            amountInPeriod: _amountInPeriod, 
            periodInSecond: _periodInSecond});

        lockList[_receiver].push(item);
        isLocked[_receiver] = true;
    }

    function transactionRevoke(uint256 _txIndex) public{
        Transaction storage transaction = transactions[_txIndex];
        require(validateBeforeConfirm(_txIndex, transaction)) ;
