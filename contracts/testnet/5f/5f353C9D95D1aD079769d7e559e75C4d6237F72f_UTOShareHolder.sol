...
contract UTOShareHolder is Lockable {
    ...
    function transfer(uint256 amount, address to) external onlyShareholder lock {
        ...
        if(mapSpend[index].status > 0) {
            mapSpend[++index] = SpendData(amount, to, msg.sender, address(0), address(0), block.timestamp, 0);
        } else {
    ...
    function getSpendData(uint256 _index) external view returns (bool res, uint256 amount, address receiver, address addr1, address addr2, address addr3, uint256 date, uint8 status) {
