...
contract Presale is Ownable {
    ...
    uint256 public vestingTime = 24 * 3600; // besting period of first public sale: Time number
    ...
    // set of sale end time
    function setSaleEndTime(uint256 _endTime) external onlyOwner {
        require(!started, "Sale has already started");
        require(block.timestamp > saleEndTime, "After saleEndtime, reset available");
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        require(block.timestamp < saleEndTime, "Can't deposit. Please check presale time!");
    ...
    function withdraw(uint256 _amount) external checkIfPaused {
        ...
        require(_vestingwithdrawTime > 30 * vestingTime, "please wait until climable time");
        ...
        if (user.withdrawStep < 4) {
            if (user.withdrawStep == 3) {
                ...
                require(_vestingwithdrawTime > vestingTime * (30 + 10 * (user.withdrawStep)), "please wait until next climable time");
                uint amount = _amount * 10 ** 9 * (20 + 10 * user.withdrawStep) / 100;
