...
contract Token is Context, Auth, IBEP20 {
    ...
    uint public percentStaking = 1;
    uint public percentTotalTax = percentTreasury + percentMarketing + percentLP +  percentStaking;
    ...
    function setTaxPercent(uint _percentTreasury, uint _percentMarketing, uint _percentLP, uint _percentStaking) external onlyOwner {
        percentTreasury = _percentTreasury;
        percentMarketing = _percentMarketing;
        percentLP = _percentLP;
        percentStaking = _percentStaking;
        require((percentTreasury + percentMarketing + percentLP + percentStaking) < 25,"Token: Maximum tax is 25%");
    }
    ...
    function _swapForWeth(uint256 amount) internal swapping {
        if (amount > 0) {
            uint256 baseUnit = amount / percentTaxDenominator;
            uint256 tokenMarketing = baseUnit * percentMarketing;
            uint256 tokenTreasury = baseUnit * percentTreasury;
            uint256 tokenReseve = baseUnit * percentStaking; 
            uint256 tokenLP = (baseUnit * percentLP) / 2; 
            uint256 amountSwap = tokenMarketing + tokenTreasury + tokenReseve + tokenLP;
            ...
            if(balanceETHAfter > 0){
                uint256 amountMarketing = getAmountPercent(balanceETHAfter, percentMarketing, percentTotalTax);
                uint256 amountTreasury = getAmountPercent(balanceETHAfter, percentTreasury, percentTotalTax);
                uint256 amountStaking = getAmountPercent(balanceETHAfter, percentStaking, percentTotalTax);
                
                payable(walletMarketing).transfer(amountMarketing);
                payable(walletTreasury).transfer(amountTreasury);
                if(stakingAddress != address(0)){
                    try IStake(stakingAddress).deposit{value: amountStaking}(amountStaking) {} catch {}
                }
    ...
    function _addLiquidity(uint256 amount) internal swapping {
        uint256 amountToken = balanceOf(address(this));
        _approve(address(this), pairWETHAddress, amountToken);
