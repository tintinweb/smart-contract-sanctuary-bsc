...
// File: contracts/operan/OperonOrigins.sol
...
contract OperonOriginsMarketplace {
    ...
    constructor(uint256 _serviceValue, address payable adminfeeAddress)
        payable
    {
    ...
    function placeOffering(
        ...
        tokenStatus[_tokenId].forsale = tokenStatus[_tokenId].forsale + _amount;
        tokenStatus[_tokenId].notfosale = tokenBalance  -  tokenStatus[_tokenId].forsale;
    ...
    function cancelOffering(bytes32 _offeringId) external {
        ...
        tokenStatus[offeringRegistry[_offeringId].tokenId].forsale =
            tokenStatus[offeringRegistry[_offeringId].tokenId].forsale -
            offeringRegistry[_offeringId].amount;
        tokenStatus[offeringRegistry[_offeringId].tokenId].notfosale =
            tokenStatus[offeringRegistry[_offeringId].tokenId].notfosale +
            offeringRegistry[_offeringId].amount;
        ...
        payable
    {
        ...
        // tokenStatus[offeringRegistry[_offeringId].tokenId].forsale = tokenStatus[_tokenId].forsale - offeringRegistry[_offeringId].amount;
        // tokenStatus[offeringRegistry[_offeringId].tokenId].notfosale = tokenStatus[_tokenId].notfosale +  offeringRegistry[_offeringId].amount;
        ...
        if (offeringRegistry[_offeringId].amount - amount == 0) {
            offeringRegistry[_offeringId].status = offeringStatus.CLOSED;
        }

        tokenStatus[offeringRegistry[_offeringId].tokenId].forsale = tokenStatus[offeringRegistry[_offeringId].tokenId].forsale - amount;
    ...
    function purchaseOffering1(bytes32 _offeringId, uint256 amount)
    external
    payable
{
    require(
...
function purchaseOffering2(bytes32 _offeringId, uint256 amount)
external
payable
{
require(
...
uint256 netamount = nftTotalPrice - roy;
    ...
    function burnToken(
        ...
        if(setOffering == 1) {
            offeringRegistry[offeringId].amount = offeringRegistry[offeringId].amount - amount;
            tokenStatus[tokenId].forsale = tokenStatus[tokenId].forsale - amount;
        }
    ...
    function transfer(
        ...
        require(tokenBalance >= amount, "Insufficient balance");
        if (_offeringId != "0x") {
            offeringRegistry[_offeringId].amount = tokenBalance - amount;
        }
