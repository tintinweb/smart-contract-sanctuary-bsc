...
contract TKMC is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    IERC20 public USDTR = IERC20(0xe94d1104769C0aEAC8c44845E8D76b2Bf35Ae46A);

    uint256 public presaleCost = 250 * 10**18;
    uint256 public publicCost = 275 * 10**18;
    uint256 private reserveLimit = 0;
    uint256 public tokenCount = 0;

    uint16 public maxSupply = 8;
    uint16 public currentSupply = 2;
    ...
    function presaleMint(uint8 _amount) external payable callerIsUser {
        ...
        if (tokenCount + _amount > currentSupply) revert MaxSupplyExceeded();
        ...
        uint256 cost = presaleCost * _amount;
        USDTR.transferFrom(msg.sender, address(this), cost);
    }
    ...
    function mint(uint8 _amount) external payable callerIsUser {
        ...
        uint256 cost = publicCost * _amount;
        USDTR.transferFrom(msg.sender, address(this), cost);
    }
    ...
    function updatePhase() external onlyOwner {
        if (currentSupply + 2 >= maxSupply) revert MaxSupplyExceeded();
        currentSupply += 2;
    }
