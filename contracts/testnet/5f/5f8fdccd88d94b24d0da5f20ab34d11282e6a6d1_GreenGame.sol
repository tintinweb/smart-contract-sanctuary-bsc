...
contract GreenGame is Ownable {
    ...
    function setnewRootAddress(address newRootAddress) public onlyOwner {
        ...
        for (uint256 i = 0; i < tables.length; i = inc256(i)) {
    ...
    function appendTable(uint256 thValue, uint256 charityShare, uint256 refShare, uint256 donationsCount, uint256 donationShare, uint256 refDonationShare, uint256 maxDonationsCount, bool forceRebuildJUmpValues) public onlyOwner {
        setTableParams(thValue, uint256(tables.length), charityShare, refShare, donationsCount, donationShare, refDonationShare, maxDonationsCount, forceRebuildJUmpValues);
        tableAddresses[uint256(dec256(tables.length))].push(rootAddress);
    }
    ...
    function rebuildJumpValues() public onlyOwner {
        for (uint256 i = 0; i < tables.length; i = inc256(i)) {
            for (uint256 j = 0; j < tables.length; j = inc256(j)) {
        ...
        uint256 accum = 0;
        for (uint256 j = 1; j < tables.length; j = inc256(j)) {
        ...
        uint256 val;
        for (uint256 i = 1; i < tables.length; i = inc256(i)) {
            for (uint256 j = 1; j < tables.length; j = inc256(j)) {
                if (j < i) {
                    jumpValues[i][j] = 0;
                } else {
                    val = jumpValues[dec256(i)][j] - jumpValues[dec256(i)][i];
                    jumpValues[i][j] = val;
                    value2table[i][val] = j;
                    unchecked {
    ...
    function process(address parent) private {
        ...
        for (uint256 i = 1; i <= t.donationsCount; i = inc256(i)){
            // Donation Ref Payout
        ...
        for (uint256 i = currentTable; i < newTable; i = inc256(i)){
            tableAddresses[inc256(i)].push(msg.sender);
        }
