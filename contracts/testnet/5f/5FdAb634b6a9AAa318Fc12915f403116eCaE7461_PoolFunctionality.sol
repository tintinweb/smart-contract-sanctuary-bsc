...
contract PoolFunctionality is Ownable, OrionPoolV2Router02, IPoolFunctionality {
    ...
    function doSwapThroughOrionPool(
        ...
        if (isFactory && factory != factories[0]) {
            factory = factories[0];
        }
    }
    ...
    function _doSwapTokens(
        ...
        IPoolSwapCallback(msg.sender).safeAutoTransferFrom(asset_spend, user,
            OrionPoolV2Library.pairFor(factory, path[0], path[1]), amounts[0]);

        _swap(amounts, path, toAuto);
