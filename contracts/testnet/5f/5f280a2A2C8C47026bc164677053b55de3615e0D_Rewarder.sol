...
contract Rewarder is Ownable, Pausable, Initializable {
    ...
    uint128 private constant ACC_REWARD_PRECISION = 1e12;
    IERC20 public rewardToken = IERC20(0x86772b1409b61c639EaAc9Ba0AcfBb6E238e5F83);
    IStaking public staking = IStaking(0x86772b1409b61c639EaAc9Ba0AcfBb6E238e5F83);
    ...
    modifier onlyStaking() {
        require(msg.sender == address(staking), "Rewarder: Only Staking");
        _;
    }
    ...
    function pendingReward(address _user) external view returns (uint256 pending) {
        ...
        uint256 _totalPower = staking.getTotalPower();
        ...
        pending = user.amount * _accRewardPerShare / ACC_REWARD_PRECISION - user.rewardDebt;
    }
    ...
    whenNotPaused
    {
        updatePool();
        UserInfo storage user = _userInfo[_user];
        uint256 accumulatedRewards = user.amount * accRewardPerShare / ACC_REWARD_PRECISION;
        uint256 pending = accumulatedRewards - user.rewardDebt;
        if (pending > 0) {
            rewardToken.safeTransfer(to, pending);
        }
        user.amount = lpToken;
        user.rewardDebt = lpToken * accRewardPerShare / ACC_REWARD_PRECISION;
        emit LogOnReward(_user, pending, to);
    }

    function onDeposit(address _user, address _to, uint256 _amount) external onlyStaking whenNotPaused {
        updatePool();
        UserInfo storage user = _userInfo[_user];

        uint256 _change = _amount - user.amount;
        user.rewardDebt += _change * accRewardPerShare / ACC_REWARD_PRECISION;
        user.amount = _amount;
        emit Deposit(_user, _change, _to);
    }

    function onWithdraw(address _user, uint256 _amount) external onlyStaking whenNotPaused {
        updatePool();
        UserInfo storage user = _userInfo[_user];
        uint256 _change = user.amount - _amount;
        user.rewardDebt -= _change * accRewardPerShare / ACC_REWARD_PRECISION;
        user.amount = _amount;
        emit Withdraw(_user, _change);
    }

    function onUpdate(address _user, uint256 _amount) external onlyStaking whenNotPaused {
        updatePool();
        UserInfo storage user = _userInfo[_user];
        if(user.amount < _amount) {
            user.rewardDebt += (_amount - user.amount) * accRewardPerShare / ACC_REWARD_PRECISION;
        }else {
            user.rewardDebt -= (user.amount - _amount) * accRewardPerShare / ACC_REWARD_PRECISION;
        }
        user.amount = _amount;
        emit Update(_user, _amount);
    }
