    ...
    ICryptonaireLottery
{
    ...
    uint256 public override currentTicketId;
    ...
    mapping(uint256 => bool) private _isWinningTicket;

    // Check ticket id in which lottery id
    mapping(uint256 => uint256) private _ticketInRound;
    ...
    function buyTickets(
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            _ticketInRound[currentTicketId] = _lotteryId;
    ...
    /**
     * @notice Re-close lottery
     * @param _lotteryId: lottery id
     * @dev For some reason, if chainlink can't execute callback function to fulfill random number
     * this function should be call as a resolve to the problem
     * Callable by operator (this function is a backup and should only be call if chainlink got error)
     */
    function reCloseLottery(uint256 _lotteryId)
        external
        onlyOperator
        nonReentrant
    {
        ...
        require(
            ...
            "Fulfillment function has already been executed"
        );

        // Make another request for a random number from the generator based on a seed
        randomGenerator.requestRandomWords();
    }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        _lotteries[_lotteryId].claimExpireDate =
            block.timestamp +
            maximumClaimTime;
        _isWinningTicket[finalTicketId] = true;
    ...
    /**
     * @notice Change the random generator address
     ...
     * @dev The require expression are used to verify the new ticket collection with matching token id.
     * It is necessary to check that ticket id is the same as token id on change.
     * Callable only by the contract owner
     * @param _ticketCollectionAddress: address of new ticket collection
     */

    function changeTicketCollection(address _ticketCollectionAddress)
        external
        onlyOwner
    {
        uint256 currentTokenId = ITicketCollection(_ticketCollectionAddress)
            .currentTokenId();
        require(
            currentTicketId == currentTokenId,
            "Ticket id must match with token id!"
        );
        ticketCollection = ITicketCollection(_ticketCollectionAddress);
    }
     ...
     * @param _oldOwner address of old ticket owner
    ...
    function updateTicketOwner(
        ...
        uint256 _lotteryId = _ticketInRound[_ticketId];
        address _ticketOwner = _tickets[_ticketId].owner;

        // If ticket not claimed (or not a winning ticket), update ticket owner
        if (_ticketOwner != address(0)) {
            require(_oldOwner == _ticketOwner, "Invalid ticket owner!");

            // Update winning address if winning tick & not claimed
            if (
                _tickets[_ticketId].number == _lotteries[_lotteryId].finalNumber
            ) {
                _lotteries[_lotteryId].winningAddress = _newOwner;
            }
            _tickets[_ticketId].owner = _newOwner;
        }

        // Update number of tickets user bought at lottery round
        if (_userTicketIdsPerLotteryId[_newOwner][_lotteryId].length == 0) {
            _playersPerLottery[_lotteryId]++;
        }

        _userTicketIdsPerLotteryId[_newOwner][_lotteryId].push(_ticketId);

        _removeElement(_oldOwner, _lotteryId, _ticketId);

        if (_userTicketIdsPerLotteryId[_oldOwner][_lotteryId].length == 0) {
            _playersPerLottery[_lotteryId]--;
        }
    ...
    /**
     * @notice View ticket stay in which round
     */
    function viewTicketInRound(uint256 _ticketId)
        external
        view
        returns (uint256)
    {
        return _ticketInRound[_ticketId];
    }
        ...
        returns (uint32[] memory)
    {
        ...
        uint256 firstTicketIdThisLottery = _lotteries[_lotteryId].firstTicketId;

        for (uint32 i = 0; i < length; i++) {
            ticketNumbers[i] = _tickets[firstTicketIdThisLottery].number;
            firstTicketIdThisLottery++;
        }
     ...
     * @param _lotteryId: lottery id
     */
    function viewTotalTicketUserPurchasedThisLotteryId(
        address _user,
        uint256 _lotteryId
    ) external view returns (uint256) {
    ...
    /**
     * @notice Remove a ticket from an existing wallet address
     * @param _userAddress: user address to be removed
     * @param _lotteryId: lottery id
     * @param _ticketId: ticket id
     */
    function _removeElement(
        ...
        uint256[] storage userTicketInfo = _userTicketIdsPerLotteryId[
            _userAddress
        ][_lotteryId];
        for (uint256 i; i < userTicketInfo.length; i++) {
            if (userTicketInfo[i] == _ticketId) {
                userTicketInfo[i] = userTicketInfo[userTicketInfo.length - 1];
                userTicketInfo.pop();
                break;
            }
        }
