...
contract MFTStaking {
    ...
    // test
    mapping(address => uint256) public Duration256;
    mapping(address => uint256) public _realRate256;
    mapping(address => uint256) public _interestOnly;
    mapping(address => uint256) public _balanceAF;
    //test
    ...
    function _stakingInDue(uint256 _amount) public {
        ...
        customHasStaked3[msg.sender] = true;
        customIsStakingAtm3[msg.sender] = true;
        Duration256[msg.sender] = (block.timestamp - startTime3[msg.sender]) / 86400; // test duration 30 days
        uint256 Duration256Real =  Duration256[msg.sender]; // transfer uint256 mapping to real uint256
        _realRate256[msg.sender] = customAPY3 * Duration256Real; // real rate not devided by 100000 | cannot store number < 0
        uint256 realRate256 = _realRate256[msg.sender];
        uint256 balance = customStakingBalance3[msg.sender];
        _interestOnly[msg.sender] = balance * realRate256 / 100000;
        uint256 interestOnly = _interestOnly[msg.sender];
        _balanceAF[msg.sender] = balance - balance * 20 / 100 + interestOnly;
