...
contract Token is Initializable, ERC20Upgradeable, Member {
    ...
    event SwapReward(address from, address to, address officalAddress, uint256 amount, uint256 reward);

    modifier onlyPool{
        require(msg.sender == address(manager.members("NFTStakePool")) || msg.sender == address(manager.members("nft")) || msg.sender == address(manager.members("owner")), "permission denied");
        _;
    }
    ...
    function mint(address to, uint256 amount) external onlyPool {
        require(MAX_MINT >= (totalSupply() + amount), "token totalSupply reach the upper limit");
        _mint(to, amount);
    }
    function transferFrom(address from, address to, uint256 amount) public virtual override returns (bool) {
        if (manager.members("OfficalAddress") != address(0) && manager.members("PancakeSwapPair") != address(0)) {
            address officalAddress = manager.members("OfficalAddress");
            address PancakeSwapPair = manager.members("PancakeSwapPair");
            uint256 reward = amount / 100 ;// 1%
            if (PancakeSwapPair != address(0) && to == PancakeSwapPair) {
                require(balanceOf(from) >= (amount + reward), "Token PancakeSwap: token not enough");
                _spendAllowance(from, _msgSender(), reward);
                _transfer(from, officalAddress, reward);
                emit SwapReward(from, to, officalAddress, amount, reward);
            }
        }
