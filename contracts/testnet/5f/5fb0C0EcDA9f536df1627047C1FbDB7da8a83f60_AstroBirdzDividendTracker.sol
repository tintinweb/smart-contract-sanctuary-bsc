...
import './abstracts/ERC20TokenRecover.sol';
import './interfaces/IDividendTracker.sol';

contract AstroBirdzDividendTracker is Ownable, DividendPayingToken, ERC20TokenRecover, IDividendTracker {
    ...
    constructor(address _dividendToken, address _parentToken)
        DividendPayingToken('AstroBirdz Dividend Tracker', 'ABZDT', _dividendToken, _parentToken)
    {
    ...
    {
        require(tokenAddress != dividendToken, 'AstroBirdzDividendTracker: Cannot retrieve USDT');
    ...
    function _transfer(
        ...
        require(false, 'AstroBirdzDividendTracker: No transfers allowed');
    }

    function withdrawDividend() public pure override(DividendPayingToken, IDividendPayingTokenInterface) {
        require(
            false,
            "AstroBirdzDividendTracker: Disabled. Use the 'claim' function on the AstroBirdz token contract."
        );
    }

    function excludeFromDividends(address account) external override onlyOwnerOrParentToken {
        require(!excludedFromDividends[account], 'AstroBirdzDividendTracker: Account already excluded');
    ...
    function includeInDividends(address account) external override onlyOwnerOrParentToken {
        require(excludedFromDividends[account], 'AstroBirdzDividendTracker: Account not excluded');
    ...
    function updateClaimWait(uint256 newClaimWait) external override onlyOwnerOrParentToken {
        require(
            ...
            'AstroBirdzDividendTracker: claimWait must be updated to between 1 and 24 hours'
        );
        require(newClaimWait != claimWait, 'AstroBirdzDividendTracker: Cannot update claimWait to same value');
...
import '@openzeppelin/contracts/access/Ownable.sol';
import '../interfaces/token/IERC20TokenRecover.sol';
