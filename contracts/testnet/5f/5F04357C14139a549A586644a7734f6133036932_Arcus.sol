...
contract Arcus is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _mintNft(
        ...
        if (_index == 1) {
            arcusWarrior.mintN(_minter, _edition, _rarity, _amount);
        } else if (_index == 2) {
            arcusEther.mintN(_minter, _edition, _rarity, _amount);
        } else if (_index == 3) {
            arcusWeapon.mintN(_minter, _edition, _rarity, _amount);
        }
    }
...
contract RoundManager is Initializable, AccessControlUpgradeable {
    ...
    function _updateRound(
        ...
        if (roundInfo.rarity != _roundInfo.rarity) {
            require(_roundInfo.rarity > 0, "E3005");
