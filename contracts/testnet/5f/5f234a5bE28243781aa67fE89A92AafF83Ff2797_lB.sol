...
contract lB is VRFConsumerBase,ReentrancyGuard {
	using SafeMath for uint256;
	...
	IERC20 usd = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // USDC
	...
	event LotteryLog(uint256 timestamp,address adrs, string message,uint number,uint number2,string message2,uint choosennumber1,uint choosenNumber2);
    ...
    function changeRouter(address _routerAddress) public isDeployer {
    ...
    function autoSpin(address _caller) public nonReentrant {
        ...
        require(block.timestamp >= cooldownAutoSpin[_caller] + 12000,"You are still in cooldown"); // 200 minutes cooldown per function call
     
        if(getLatestPrice() > 300000000000000){

          botReward = 16000000000000000000; // 16.00 usd stablecoin
          ...
          botReward = 1600000000000000000; // 1.60 usd stablecoin
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override onlyVRFCoordinator {
        ...
        uint256 jackpotNumber2 = (randomNumbers[2] % 25) + 1;
        ...
        if(jackpotNumber1 == choosenNumber1 || jackpotNumber1 == choosenNumber2 && jackpotNumber2 == choosenNumber2 || jackpotNumber2 == choosenNumber1) {
            lastJackpotWinner = lastWinner;
            lastJackpotWinningNumbers = [jackpotNumber1,jackpotNumber2];
            ...
            if(registeredsAccts.length > 1){
                ...
                emit LotteryLog(block.timestamp,lastJackpotWinner, "Jackpot is hit!  Jackpot Numbers was: ",jackpotNumber1,jackpotNumber2,"Player choosen numbers was: ",choosenNumber1,choosenNumber2);
            ...
            emit LotteryLog(block.timestamp,lastWinner, "We have a new raffle Winner but no jackpot! Jackpot Numbers was: ",jackpotNumber1,jackpotNumber2,"Player choosen numbers was: ",choosenNumber1,choosenNumber2);
        }
    }

    // Play Function
    function BT5124(address player,uint256 ticketValue,uint number1,uint number2) public nonReentrant {
        ...
        require(number1 <= 25 && number1 > 0 && number2 <= 25 && number2 > 0,"Failed: numbers has to be between 1 and 25");
        ...
        if(player != derivativesContract){
         ...
         Registered memory registered = Registered(derivativesDraws+3,number1,number2,0,0);
         registereds[player] = registered;
         derivativesDraws += 3;
        }
      ...
      if(krstm.balanceOf(player) >= 5000000000000000000 && player != derivativesContract ){
         ...
         emit LotteryLog(block.timestamp,player, "Ticket Bought, Good luck!",0,0,"Ticket Numbers :",number1,number2);
