...
contract contractA {
  ...
  function deposit(uint256 amount) public {
    ...
    require(token.balanceOf(msg.sender) >= amount, 'User not enough balance to deposit');
  ...
  function withdraw(uint256 amount) external {
    require(amount > 0, 'Invalid amount');
    require(balances[msg.sender] >= amount && token.balanceOf(owner) >= amount, 'User not enough balance to withdraw');
    require(
      lastTimeWithdraw[msg.sender] + cooldownWithdrawal <= block.timestamp,
      'You have to wait about 30 mins to withdraw again'
    );
    unchecked {
      balances[msg.sender] -= amount;
    }
