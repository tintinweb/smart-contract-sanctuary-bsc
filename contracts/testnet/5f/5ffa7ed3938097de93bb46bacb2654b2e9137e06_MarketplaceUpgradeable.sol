...
import "./ExchangeUpgradeable.sol";

contract MarketplaceUpgradeable is AuctionUpgradeable, ExchangeUpgradeable {
    function initialize(address multiSigAccount, address beneficiary)
        public
        override(AuctionUpgradeable, ExchangeUpgradeable)
        initializer
    {
        __Factory_init(multiSigAccount, beneficiary);
    }
}
...
contract AuctionUpgradeable is FactoryUpgradeable {
    struct AuctionItem {
    ...
    // auctionItemIndex => hightestBid
    mapping(uint256 => HightestBid) private _hightestBid;
    // auctionItemIndex => bider => isClaimed
    ...
    event AuctionItemCanceled(uint256 auctionItemIndex);
    event AuctionItemPlaceBided(uint256 auctionItemIndex, address bider, uint256 price);

    function initialize(address multiSigAccount, address beneficiary) public virtual initializer {
        __Factory_init(multiSigAccount, beneficiary);
    }
    ...
    {
        require(startTime > block.timestamp, "Error: Start time must be in future");
        require(duration > 0, "Error: Duration must be greater than zero");
        require(bidIncrement > 0, "Error: Bid increment must greater than zero");
    ...
    function cancelAuctionItem(uint256 auctionItemIndex) external nonReentrant {
        require(_auctionItems.length > auctionItemIndex, "Error: Auction not found");

        AuctionItem memory auctionItem = _auctionItems[auctionItemIndex];
        require(!auctionItem.isCanceled, "Error: Auction item is canceled");
        require(auctionItem.startTime > block.timestamp, "Error: Auction item is starting");
        ...
        emit AuctionItemCanceled(auctionItemIndex);
    }

    function placeBid(uint256 auctionItemIndex, uint256 price) external payable nonReentrant whenNotPaused {
        require(_auctionItems.length > auctionItemIndex, "Error: Auction not found");

        AuctionItem memory auctionItem = _auctionItems[auctionItemIndex];

        require(auctionItem.owner != _msgSender(), "Error: Cannot bid auction item from yourself");
        require(!auctionItem.isCanceled, "Error: Auction item is canceled");
        require(auctionItem.startTime <= block.timestamp, "Error: It's not time to bid yet");
        require(auctionItem.startTime + auctionItem.duration > block.timestamp, "Error: Auction item is completed");

        HightestBid memory latestBid = _hightestBid[auctionItemIndex];
        require(latestBid.bider != _msgSender(), "Error: You are the highest bidder");

        uint256 highestPriceBid = Math.max(auctionItem.startPrice, latestBid.price);

        require(price > highestPriceBid, "Error: Price must be greater than last price");

        price = Math.min(price, highestPriceBid + auctionItem.bidIncrement);

        if (auctionItem.paymentToken != address(0)) {
            require(
                IERC20(auctionItem.paymentToken).allowance(payable(_msgSender()), address(this)) >= price,
                "Error: Payment token is not allowed"
            );

            require(
                IERC20(auctionItem.paymentToken).transferFrom(_msgSender(), address(this), price),
                "Error: Payment token transfer failed"
            );

            if (latestBid.price != 0) {
                require(
                    IERC20(auctionItem.paymentToken).transferFrom(address(this), latestBid.bider, latestBid.price),
                    "Error: Transfer to last bider failed"
                );
            }
        } else {
            require(msg.value >= price, "Error: Not enough tokens");
            ...
            if (latestBid.price != 0) {
                latestBid.bider.transfer(latestBid.price);
            }
        }

        _hightestBid[auctionItemIndex] = HightestBid(payable(_msgSender()), price);

        emit AuctionItemPlaceBided(auctionItemIndex, _msgSender(), price);
    }
    ...
    function withdraw(uint256 auctionItemIndex) external nonReentrant {
        ...
        require(!auctionItem.isCanceled, "Error: Auction item is canceled");
        require(auctionItem.startTime + auctionItem.duration < block.timestamp, "Error: Auction item is not completed");
        require(!_isClaimed[auctionItemIndex][_msgSender()], "Error: Nothing to withdraw");

        if (_hightestBid[auctionItemIndex].price != 0) {
            HightestBid memory hightestBid = _hightestBid[auctionItemIndex];

            // winner the auction should be allowed ti withdraw the auction item
            if (_msgSender() == hightestBid.bider) {
                _transferAsset(
                    auctionItem.nftContractAddress,
                    address(this),
                    _msgSender(),
                ...
                _isClaimed[auctionItemIndex][_msgSender()] = true;
            }

            if (auctionItem.owner == _msgSender()) {
                uint256 beneficiaryReceivable = (hightestBid.price * feePercent) / (1 ether);
                uint256 ownerReceivable = hightestBid.price - beneficiaryReceivable;

                if (auctionItem.paymentToken != address(0)) {
                    require(
                        IERC20(auctionItem.paymentToken).transfer(auctionItem.owner, ownerReceivable),
                        "Error: Payment token transfer to owner failed"
                    );

                    if (feePercent != 0) {
                        require(
                            IERC20(auctionItem.paymentToken).transfer(beneficiary, beneficiaryReceivable),
                            "Error: Payment token transfer to beneficiary failed"
                        );
                    }
                } else {
                    auctionItem.owner.transfer(ownerReceivable);

                    if (feePercent != 0) {
                        beneficiary.transfer(beneficiaryReceivable);
                    }
                }
...
contract ExchangeUpgradeable is FactoryUpgradeable {
    struct ExchangeItem {
        address nftContractAddress;
        uint256 tokenId;
    ...
    event ExchangeItemSale(uint256 exchangeItemIndex, address buyer, uint256 price, uint256 amount, uint256 feePercent);
    ...
    function createExchangeItem(
        ...
        amount = _checkApproveAndReturnAmount(nftContractAddress, tokenId, _msgSender(), address(this), amount);
        require(amount > 0, "Error: Exceeds balance");
    ...
    function updateExchangeItemPrice(uint256 exchangeItemIndex, uint256 newPrice) external nonReentrant {
        require(_exchangeItems.length > exchangeItemIndex, "Error: NFT is not for sale");

        ExchangeItem memory exchangeItem = _exchangeItems[exchangeItemIndex];
        require(_msgSender() == exchangeItem.seller, "Error: You are not the seller");
        require(exchangeItem.amount > exchangeItem.amountSold, "Error: NFT is not for sale");
        require(!_exchangeItems[exchangeItemIndex].isCanceled, "Error: Exchange item is canceled");

        require(newPrice > 0, "Error: Price must be greater than 0");
    ...
    function createExchangeSale(uint256 exchangeItemIndex, uint256 amount) external payable nonReentrant {
        require(_exchangeItems.length > exchangeItemIndex, "Error: NFT is not for sale");
        require(!_exchangeItems[exchangeItemIndex].isCanceled, "Error: Exchange item is canceled");
        require(
        ...
        require(_msgSender() != exchangeItem.seller, "Error: You can not buy the NFT from yourself");

        uint256 totalPrice = (exchangeItem.price * amount) / exchangeItem.amount;

        uint256 beneficiaryReceivable = (totalPrice * feePercent) / (1 ether);
        uint256 sellerReceivable = totalPrice - beneficiaryReceivable;

        if (exchangeItem.paymentToken != address(0)) {
            require(msg.value == 0, "Error: Payment is ERC20 token");
            require(
                IERC20(exchangeItem.paymentToken).allowance(payable(_msgSender()), address(this)) >= totalPrice,
            ...
            if (feePercent != 0) {
                require(
                    IERC20(exchangeItem.paymentToken).transferFrom(_msgSender(), beneficiary, beneficiaryReceivable),
                    "Error: Payment token transfer to beneficiary error"
                );
            }
        } else {
            require(msg.value >= totalPrice, "Error: Not enough tokens");

            if (msg.value > totalPrice) {
                payable(_msgSender()).transfer(msg.value - totalPrice);
            }

            exchangeItem.seller.transfer(sellerReceivable);
        ...
        emit ExchangeItemSale(exchangeItemIndex, _msgSender(), totalPrice, amount, feePercent);
    }
    ...
    ERC1155ReceiverUpgradeable
{
    ...
    function __Factory_init(address multisigAddress_, address beneficiary_) internal onlyInitializing {
    ...
    function setBeneficiary(address newBeneficiary) public onlyAdmin nonReentrant whenPaused {
    ...
    function setFeePercent(uint256 newFeePercent) public onlyAdmin nonReentrant whenPaused {
        feePercent = newFeePercent;
        emit FeePercentChanged(newFeePercent);
    }

    function pause() public nonReentrant whenNotPaused onlyPauser {
        _pause();
    }

    function unpause() public nonReentrant whenPaused onlyPauser {
        _unpause();
    }
    ...
    function withdrawERC20(
        ...
        address token,
        uint256 amount
    ) public nonReentrant whenPaused onlyAdmin {
        if (token == address(0)) {
            ...
            require(IERC20(token).transfer(_msgSender(), amount), "Error: Transfer failed");
        }
    }
    ...
    function _checkApproveAndReturnAmount(
        address nftContractAddress,
        uint256 tokenId,
        address owner,
        address operator,
        ...
        if (_isERC1155[nftContractAddress]) {
            if (IERC1155(nftContractAddress).isApprovedForAll(owner, operator))
                return Math.min(IERC1155(nftContractAddress).balanceOf(owner, tokenId), amount);
        } else {
            if (
                IERC721(nftContractAddress).ownerOf(tokenId) == owner &&
                (IERC721(nftContractAddress).getApproved(tokenId) == operator ||
                    IERC721(nftContractAddress).isApprovedForAll(owner, operator))
            ) return 1;
        }
    ...
    function onERC721Received(
        ...
        uint256,
        bytes memory
    ) public virtual override whenNotPaused onlyWhitelistNFTContract(_msgSender()) returns (bytes4) {
