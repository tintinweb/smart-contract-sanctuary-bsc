...
contract Qualifying is Initializable, AccessControlUpgradeable, IQualifying, IERC1155ReceiverUpgradeable{
    ...
    function create(uint256 invaderId, uint128[] memory tableclothTypes) external override onlyRole(BATTLE_MANAGER_ROLE){
        ...
        battlesMapping[idIndex].id = idIndex;
    ...
    function join(uint256 battleId, uint256 sandwichId) external override{
        require(battlesMapping[battleId].endTime == 0, "Member is full, battle has already begun");
        ...
        battlesMapping[battleId].sandwiches.push(sandwichId);

        sandwichInfoMapping[sandwichId].battleId = battleId;
        sandwichInfoMapping[sandwichId].index = battlesMapping[battleId].sandwiches.length;
        //calculate max value and general
        {
            BattleMaxPower memory battleMaxPower = battlesMaxPowerMapping[battleId];
            (,,,uint256 aggressivity,uint256 defensive,uint256 healthPoint,,,uint16 attrnum) = sandwichERC1155.getSandwich(sandwichId);

            //judge attrs validity
            for(uint i = 0; i < battlesMapping[battleId].allowedAttrs.length; i++)
                if(attrnum == battlesMapping[battleId].allowedAttrs[i]){
                    attrnum = 0;
                    break;
                }
            ...
            if(aggressivity > battleMaxPower.maxA){
                ...
                battlesMapping[battleId].maxASandwich = sandwichId;
            }
            if(defensive > battleMaxPower.maxD){
                battleMaxPower.maxD = defensive;
                battlesMapping[battleId].maxDSandwich = sandwichId;
            }
            if(healthPoint > battleMaxPower.maxH){
                battleMaxPower.maxH = healthPoint;
                battlesMapping[battleId].maxHSandwich = sandwichId;
            }
            //judge general sandwich
            if(battlesMapping[battleId].sandwiches.length > battleMaxPower.maxS){
                battleMaxPower.maxS = battlesMapping[battleId].sandwiches.length;
                battlesMapping[battleId].generalSandwich = sandwichId;
            }
        ...
        if(battlesMapping[battleId].sandwiches.length >= heroNumber) _startBattle(battleId);
    }
    ...
    function _battleEnded(uint256 battleId) internal{
        ...
        if(invadersMapping[battle.invaderId].status == STATUS_INVADER_ONBATTLE){
            ...
            uint8 luckyIndex = Random.createRandom(1, battle.sandwiches.length, seedBiulder.getRandomSeed(_msgSender())) - 1;
            ...
            chiCoinERC20.approve(address(tableClothAwardsPool), 16000000 ether);
    ...
    function withDraw(uint256 sandwichId) external override{
        ...
        chiCoinERC20.transfer(_msgSender(), awards);
    ...
    function createInvader(uint256 id, uint256 bounty, string memory name, uint16 race) external override onlyRole(BATTLE_MANAGER_ROLE){
        require(id!=0 && invadersMapping[id].id == 0, "Id is zero or invader is exist");
        //transfer bounty from creater
        chiCoinERC20.transferFrom(_msgSender(), address(this), bounty);

        invadersMapping[id].id = id;
