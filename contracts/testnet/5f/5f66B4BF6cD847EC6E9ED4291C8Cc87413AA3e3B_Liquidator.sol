...
contract Liquidator is WithAdmin, ReentrancyGuard {
    ...
    /// @notice Address of eBNB contract.
    EBNB public eBnb;

    /// @notice Address of EvryLoans Unitroller contract.
    IComptroller comptroller;

    /// @notice Address of EvryLoans Treasury.
    ...
    event LiquidateBorrowedTokens(address liquidator, address borrower, uint256 repayAmount, address eTokenCollateral, uint256 seizeTokensForTreasury, uint256 seizeTokensForLiquidator);

    using SafeMath for uint256;

    constructor(
        address admin_,
        address payable eBnb_,
    ...
    ///         eToken.liquidateBorrow(...) directly.
    /// @dev For BNB borrows msg.value should be equal to repayAmount; otherwise msg.value
    ///      should be zero.
    /// @param eToken Borrowed eToken
    /// @param borrower The address of the borrower
    /// @param repayAmount The amount to repay on behalf of the borrower
    /// @param eTokenCollateral The collateral to seize
    function liquidateBorrow(
        address eToken,
        address borrower,
        uint256 repayAmount,
        EToken eTokenCollateral
    )
        external
        payable
        nonReentrant
    {
        uint256 ourBalanceBefore = eTokenCollateral.balanceOf(address(this));
        if (eToken == address(eBnb)) {
            require(repayAmount == msg.value, "wrong amount");
            eBnb.liquidateBorrow{value: msg.value}(borrower, eTokenCollateral);
        } else {
            require(msg.value == 0, "you shouldn't pay for this");
            _liquidateBep20(EBep20(eToken), borrower, repayAmount, eTokenCollateral);
        }
        uint256 ourBalanceAfter = eTokenCollateral.balanceOf(address(this));
        uint256 seizedAmount = ourBalanceAfter.sub(ourBalanceBefore);
        (uint256 ours, uint256 theirs) = _distributeLiquidationIncentive(eTokenCollateral, seizedAmount);
        emit LiquidateBorrowedTokens(msg.sender, borrower, repayAmount, address(eTokenCollateral), ours, theirs);
    }
    ...
    /// @dev Transfers BEP20 tokens to self, then approves eToken to take these tokens.
    function _liquidateBep20(
        ...
        internal
    {
        EIP20Interface borrowedToken = EIP20Interface(eToken.underlying());
        borrowedToken.transferFrom(msg.sender, address(this), repayAmount);
        borrowedToken.approve(address(eToken), repayAmount);
        requireNoError(
            eToken.liquidateBorrow(borrower, repayAmount, eTokenCollateral),
            "failed to liquidate"
        );
    }

    /// @dev Splits the received eTokens between the liquidator and treasury.
    function _distributeLiquidationIncentive(EToken eTokenCollateral, uint256 siezedAmount)
        internal returns (uint256 ours, uint256 theirs)
    {
        (ours, theirs) = _splitLiquidationIncentive(siezedAmount);
        require(
            eTokenCollateral.transfer(msg.sender, theirs),
            "failed to transfer to liquidator"
        );
        require(
            eTokenCollateral.transfer(treasury, ours),
...
/**
 * @title Evry.Finance's EBNB Contract
 * @notice EToken which wraps BNB
 * @author Evry.Finance
 */
contract EBNB is EToken {
    /**
     * @notice Construct a new EBNB money market
     ...
     */
    function mint(bool activateCollateral) external payable {
        (uint err,) = mintInternal(msg.value, activateCollateral);
     ...
     */
    function liquidateBorrow(address borrower, EToken eTokenCollateral) external payable {
        (uint err,) = liquidateBorrowInternal(borrower, msg.value, eTokenCollateral);
        requireNoError(err, "liquidateBorrow failed");
    }

    /**
     * @notice Send BNB to EBNB to mint
     */
    fallback() external payable {
        // Default activateCollateral (second argument) to false
        (uint err,) = mintInternal(msg.value, false);
        requireNoError(err, "mint failed");
    }
