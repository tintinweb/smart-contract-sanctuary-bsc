...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    /// @dev tax before any transfer
    function _beforeTokensTransfer(address from, address to, uint256 amount) private {
        if (
            from == address(uniswapV2Router) &&
            to == uniswapV2Router.WETH() &&
            !inSwapAndLiquify &&
            ...
            uint256 tAmount = (amount).sub(tokenAmount);
            /* Transfer amount, it will take Worth DVC Fund Fee and Liquidity Fee */
            _approve(address(this), address(uniswapV2Router), tokenAmount);
            super._transfer(from, address(this), tokenAmount);
            super._transfer(from,to,tAmount);

            restoreAllFee();
            inTransfer = false;
            emit SwapAndLiquify(msg.value, tokenAmount);
        }
    ...
    function _reflectFee(uint256 tAmount, address sender) private {
        ...
        if(tLiquidity > 0 && sender != address(uniswapV2Router)){
            _sendToLiquidity(tLiquidity, sender);
        }
        if(tCompany > 0 && sender != address(uniswapV2Router)){
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        emit SwapAndLiquify(tokenAmount, withAmount);
    }
