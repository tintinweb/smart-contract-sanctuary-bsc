...
contract PresalePublicTest is Context, ReentrancyGuard, VRFConsumerBase {
    ...
    function initialize() external nonReentrant onlyPresaleCreator {
        ...
        generalInfo.openTime = intermediate.closeTimeVoting + 7 minutes;
        generalInfo.closeTime = generalInfo.openTime + 34 minutes;
        ...
        notCreator
    {
        ...
        if (investor.roundInvestments.length == 0) {
            investor.roundInvestments = new uint256[](3);
        }
        uint256 currentTime = block.timestamp;
        ...
        if (currentTime <= generalInfo.openTime + 10 minutes) {
            //MOON ROUND
            round = Rounds.MOON;
            ...
            currentTime >= generalInfo.openTime + 12 minutes &&
            currentTime <= generalInfo.openTime + 22 minutes
        ) {
            //DIAMOND ROUND
            ...
            if (roundTokenAllocation[round][10] == 0)
                _getTokenAllocation(round);
        } else if (currentTime >= generalInfo.openTime + 24 minutes) {
            //PAPER ROUND
            round = Rounds.PAPER;
        ...
        require(
            roundTokenAllocation[round][sender.level] >= tokenAmount &&
                ...
                (intermediate.eachPoolShare * POOL_WEIGHT[sender.level - 1]) /
                    DENOMINATOR >=
                tokenAmount + investor.roundInvestments[uint256(round)],
            "WRONG TOKEN AMOUNT"
        );
        ...
        initialized
    {
        ...
        (amountEth, amountToken, intermediate.lpAmount) = router.addLiquidity(
            BUSD,
            generalInfo.tokenAddress,
            paymentAmount,
            tokenAmount,
