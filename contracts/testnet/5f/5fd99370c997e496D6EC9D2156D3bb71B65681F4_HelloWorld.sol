...
contract HelloWorld {
    ...
    event SayHi(address indexed from, address indexed to, string content);
    ...
    function trade(
        ...
        address to,
        uint256 amount
    ) public payable returns (uint256 afterAmount) {
        // token.universalApprove(msg.sender, amount);
        uint256 fromBefore = token.universalBalanceOf(msg.sender);
        uint256 toBefore = token.universalBalanceOf(to);
        token.safeTransferFrom(msg.sender, to, amount);
        uint256 fromNow = token.universalBalanceOf(msg.sender);
        uint256 toNow = token.universalBalanceOf(to);

        emit currentState(msg.sender, "from", fromBefore, fromNow);
        emit currentState(msg.sender, "to", toBefore, toNow);
