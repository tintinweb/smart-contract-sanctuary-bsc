...
contract MyCoin is Context, IMyCoin, Ownable {
  ...
  constructor() public {
    ...
    _symbol = "COIN";
    _decimals = 18;
    initSupply = 3000000000000000000;
    _totalSupply = initSupply;//1000000 * (10 ** 18);
    _balances[msg.sender] = _totalSupply;
    initContracts(0xb90b3521579a407F4863B1D07dEE5C0685729D78, 0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8, 0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8,0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8);
  ...
  function initContracts(address a1, address a2, address a3, address a4) public onlyOwner {
    myOracle = IOracle(a1);
    SBTC = ISBTC(a2);
    BITCOIN = IBTC(a3);
    gCoin = IgCoin(a4);
  ...
  function getRedempPrice() internal view returns (uint256) {
    if(_totalSupply > initSupply) {
      uint256 supply = _totalSupply - initSupply;
      uint256 collateral = BITCOIN.balanceOf(address(this));
      uint256 price = collateral / supply; 
      if(price > btcToLBtcRatio) {
        return 1000 * btcToLBtcRatio;
      }
      return (1000 * collateral) / supply;
    }
    return 1000 * btcToLBtcRatio;
  }

  function redeem(uint256 tokenAmount) public {
    require(gCoin.inDAO(_msgSender()));
    uint256 price = getRedempPrice();
    _balances[_msgSender()].sub(tokenAmount);
    uint256 amount = (1000 * tokenAmount * (1000 - transferFee / 2 ) / (price * 1000));
    require(tokenAmount / btcToLBtcRatio > amount);
    uint256 diff = tokenAmount / btcToLBtcRatio - amount;
    burnedTokens.add(diff);
    BITCOIN.transfer(_msgSender(), amount);
    _totalSupply = _totalSupply.sub(tokenAmount);
  }

function create(uint256 tokenAmount) public  {
    require(gCoin.inDAO(msg.sender));
    require(tokenAmount > 0);
    uint256 balanceBefore = BITCOIN.balanceOf(address(this));
    BITCOIN.transferFrom(_msgSender(), address(this), tokenAmount);
    uint256 balanceAfter = BITCOIN.balanceOf(address(this));
    uint256 diff = SafeMath.sub(balanceAfter, balanceBefore);
    require(diff > 0); 
    _mint(_msgSender(), diff * btcToLBtcRatio);
