...
contract LiquidityPoolFactory is Ownable {
    ...
    function deploy(
        ...
        pool.initialize(block.timestamp, block.timestamp.add(_period.mul(1 minutes)));
    ...
    function supplyRewardToken(address _pool, uint _amount) external payable {
        ...
        if (insuff < _amount) {
            ...
            poolInst.setRewardRate(supply.div(poolInst.endTime().sub(block.timestamp)));
        }
...
contract LiquidityStakingV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    function expandEndTime(uint _mins) external onlyFactory {
        ...
        setRewardRate(remain.div(endTime.sub(block.timestamp)));
    }
