...
contract ATHENA {
    ...
    address public usdtPairAddress; //usdtPairAddress
    bool public swapTokenToMarketEnabled = true;
    uint256 public swapTokenOverflowNum = 0; //超过多少个token可以转换成市场
    ...
    constructor(address dao_, address _lp_staking) {
        ...
        usdtPairAddress = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), usdtAddress);
        pairlist[usdtPairAddress] = true;
        approveUinswap();
    }
    ...
    function _afterTokenTransfer(
        ...
        if (pairlist[from] && buyFee > 0) {
            ...
            takeFee(to, usdtPairAddress, amount, lpFee);
    ...
    function swapTokensForStaking() external {
        require(
            msg.sender == lpStaking,
            "only lpStaking can swap tokens for token"
        );
