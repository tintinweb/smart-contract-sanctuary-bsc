...
contract VibeToken is IERC20, Ownable, Pausable{
    ...
    bool public tradingFeeEnabled = false;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            require(tradingFeeEnabled, "Trading is not started");
            if (to != UniswapV2Pair) {
    ...
    function enableTradingFee() external onlyOwner {
        require (!tradingFeeEnabled, "Already enabled");
        require(BUY_BACK_AND_BURN_ADDRESS != address(0), "Buy Back And Burn Wallet cannot be zero address");
        require(MARKETING_ADDRESS != address(0), "Marketing wallet cannot be zero address");
        tradingFeeEnabled = true;
    }

    function disableTradingFee() external onlyOwner {
        require (tradingFeeEnabled, "Already disabled");
        tradingFeeEnabled = false;
    }
