...
contract Auth {
    ...
    modifier onlyManager() {
        require(vaultParameters.isManager(msg.sender), "siUSD: AUTH_FAILED");
        _;
    }

    // ensures tx's sender is able to modify the Vault
    modifier hasVaultAccess() {
        require(vaultParameters.canModifyVault(msg.sender), "siUSD: AUTH_FAILED");
        _;
    }

    // ensures tx's sender is the Vault
    modifier onlyVault() {
        require(msg.sender == vaultParameters.vault(), "siUSD: AUTH_FAILED");
        _;
    }
}
...
contract VaultParameters is Auth {
    ...
    **/
    constructor(address payable _vault, address _foundation) public Auth(address(this)) {
        require(_vault != address(0), "siUSD: ZERO_ADDRESS");
        require(_foundation != address(0), "siUSD: ZERO_ADDRESS");
    ...
    function setFoundation(address newFoundation) external onlyManager {
        require(newFoundation != address(0), "siUSD: ZERO_ADDRESS");
     ...
     * @param siUSDLimit The siUSD token issue limit
    ...
    function setCollateral(
        ...
        setTokenDebtLimit(asset, siUSDLimit);
    ...
    function setLiquidationFee(address asset, uint newValue) public onlyManager {
        require(newValue <= 100, "siUSD: VALUE_OUT_OF_RANGE");
    ...
    /**
     * @notice Only manager is able to call this function
     * @dev Sets siUSD limit for a specific collateral
     * @param asset The address of the main collateral token
...
/**
 * @title sinuUSD token implementation
 * @dev ERC20 token
 **/
contract sinuUSD is Auth {
    using SafeMath for uint;

    // name of the token
    string public constant name = "SmilingInu USD";

    // symbol of the token
    string public constant symbol = "siUSD";
    ...
    **/
    ...
    /**
      * @notice Only Vault can mint siUSD
    ...
    function mint(address to, uint amount) external onlyVault {
        require(to != address(0), "siUSD: ZERO_ADDRESS");
    ...
    function transferFrom(address from, address to, uint amount) public returns (bool) {
        require(to != address(0), "siUSD: ZERO_ADDRESS");
        require(balanceOf[from] >= amount, "siUSD: INSUFFICIENT_BALANCE");

        if (from != msg.sender) {
            require(allowance[from][msg.sender] >= amount, "siUSD: INSUFFICIENT_ALLOWANCE");
    ...
    function _approve(address owner, address spender, uint amount) internal virtual {
        require(owner != address(0), "siUSD: approve from the zero address");
        require(spender != address(0), "siUSD: approve to the zero address");
