...
contract TokenineNFT is ERC721PsiBurnable, AccessControl, Pausable {
        ...
        mintable(_mintAmount)
    {
        if (!whitelisted[msg.sender]) {
            if (!presaleWallets[msg.sender]) {
                ...
                require(
                    !isOnlyPresaleUser,
                    "TokenineNFT: public sale is restrict"
                );
                require(
    ...
    function setIsOnlyPresaleUser(bool _isOnlyPresaleUser)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
