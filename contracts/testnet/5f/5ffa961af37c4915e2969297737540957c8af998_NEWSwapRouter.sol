...
abstract contract FeeStore is Ownable {
    ...
    function feeAdddressSetWhileSwap(address pair,address tokenAddress) public onlyOwner {
        require(INEWSwapFactory(factoryAddress).pairExist(pair), "FEESTORE: PAIR_DOES_NOT_EXIST");
        require(INEWSwapPair(pair).token0() == tokenAddress || INEWSwapPair(pair).token1() == tokenAddress, "FEESTORE: INVALID_ADDRESS");
...
library NEWSwapLibrary {
    using SafeMath for uint;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, "NEWSwapLibrary: CANNOT_USE_IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "NEWSwapLibrary: CANNOT_BE_ZERO_ADDRESS");
    }
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'7c36387bb4f01fe7ae3ea6d23ba68ccf1fa975af2ba8d8758bd2a591c3e3ed4f' // init code hash
            ))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,,) = INEWSwapPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, "NEWSwapLibrary: amountA_TOO_LOW");
        require(reserveA > 0 && reserveB > 0, "NEWSwapLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, bool tokenFee, uint totalFee) internal pure returns (uint amountOut) {
        require(amountIn > 0, "NEWSwapLibrary: amountIn_TOO_LOW");
        require(reserveIn > 0 && reserveOut > 0, "NEWSwapLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, bool tokenFee, uint totalFee) internal pure returns (uint amountIn) {
        require(amountOut > 0, "NEWSwapLibrary: amountOut_TOO_LOW");
        require(reserveIn > 0 && reserveOut > 0, "NEWSwapLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, "NEWSwapLibrary: INVALID_PATH");
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
            INEWSwapPair pair = INEWSwapPair(pairFor(factory, path[i], path[i + 1]));
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            INEWSwapPair pair = INEWSwapPair(pairFor(factory, path[i - 1], path[i]));
            address baseToken = pair.baseToken();
            uint totalFee = pair.getTotalFee();
...
abstract contract SupportingSwap is FeeStore, INEWSwapRouter {
    using SafeMath for uint;
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, "NEWSwapRouter: DEADLINE_EXPIRED");
        _;
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = NEWSwapLibrary.sortTokens(input, output);
            uint amountOut = amounts[i + 1];
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            (uint amount0Fee, uint amount1Fee) = _calculateFees(input, output, amounts[i], amount0Out, amount1Out);
            address to = i < path.length - 2 ? NEWSwapLibrary.pairFor(factory, output, path[i + 2]) : _to;
            INEWSwapPair(NEWSwapLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        require(path.length == 2, "NEWSwapRouter: ONLY_TWO_TOKENS_ALLOWED");
        address pair = NEWSwapLibrary.pairFor(factory, path[0], path[1]);

        uint adminFeeDeduct;
        if(path[0] == pairFeeAddress[pair]){
            (amountIn,adminFeeDeduct) = NEWSwapLibrary.adminFeeCalculation(amountIn, adminFee);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, feeAddressGet(), adminFeeDeduct
            );
        }

        amounts = NEWSwapLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, "NEWSwapRouter: CHECK_SLIPPAGE");
        TransferHelper.safeTransferFrom(
    ...
    function swapTokensForExactTokens(
        ...
        uint adminFeeDeduct;
        if(path[0] == pairFeeAddress[pair]) {
            amounts = NEWSwapLibrary.getAmountsIn(factory, amountOut, path);
            require(amounts[0] <= amountInMax, "NEWSwapRouter: amountInMax_TOO_HIGH");
            (amounts[0], adminFeeDeduct) = NEWSwapLibrary.adminFeeCalculation(amounts[0], adminFee);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, feeAddressGet(), adminFeeDeduct
            );

            amounts = NEWSwapLibrary.getAmountsOut(factory, amounts[0], path);
            TransferHelper.safeTransferFrom(
                path[0], msg.sender, pair, amounts[0]
            );
    ...
    {
        require(path.length == 2, "NEWSwapRouter: ONLY_TWO_TOKENS_ALLOWED");
        require(path[0] == WETH, "NEWSwapRouter: INVALID_PATH");

        uint eth = msg.value;
        address pair = NEWSwapLibrary.pairFor(factory, path[0], path[1]);
        uint adminFeeDeduct;
        if(path[0] == pairFeeAddress[pair]){
            (eth, adminFeeDeduct) = NEWSwapLibrary.adminFeeCalculation(eth, adminFee);
            if(address(this) != feeAddressGet()){
                payable(feeAddressGet()).transfer(adminFeeDeduct);
            }
        }

        amounts = NEWSwapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin,"NEWSwapRouter: CHECK_SLIPPAGE");
    ...
    {
        require(path.length == 2, "NEWSwapRouter: ONLY_TWO_TOKENS_ALLOWED");
        require(path[path.length - 1] == WETH, "NEWSwapRouter: INVALID_PATH");
        ...
        if(path[1] == pairFeeAddress[pair]){
            (amountETHOut,adminFeeDeduct) = NEWSwapLibrary.adminFeeCalculation(amountETHOut,adminFee);
        }
    ...
    returns (uint[] memory amounts)
    {
        ...
        if(path[0] == pairFeeAddress[pair]){
            amounts = NEWSwapLibrary.getAmountsIn(factory, amountOut, path);
            require(amounts[0] <= msg.value, "NEWSwapRouter: EXCESSIVE_INPUT_AMOUNT");
            ...
            require(amounts[0] <= msg.value, "NEWSwapRouter: EXCESSIVE_INPUT_AMOUNT");
            IWETH(WETH).deposit{value: amounts[0]}();
            assert(IWETH(WETH).transfer(NEWSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        }
    ...
    function _calculateFees(address input, address output, uint amountIn, uint amount0Out, uint amount1Out) internal view virtual returns (uint amount0Fee, uint amount1Fee) {
        INEWSwapPair pair = INEWSwapPair(NEWSwapLibrary.pairFor(factory, input, output));
        (address token0,) = NEWSwapLibrary.sortTokens(input, output);
        address baseToken = pair.baseToken();
        uint totalFee = pair.getTotalFee();
    ...
    function _calculateAmounts(address input, address output, address token0) internal view returns (uint amountInput, uint amountOutput) {
        ...
        amountOutput = NEWSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, baseToken != address(0), totalFee);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if(path[1] == pairFeeAddress[pair]){
            (amountOutMin,adminFeeDeduct) = NEWSwapLibrary.adminFeeCalculation(amountOutMin,adminFee);
        }
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            "NEWSwapRouter: CHECK_SLIPPAGE"
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return NEWSwapLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    {
        return NEWSwapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, false, 0);
    }
    ...
    {
        return NEWSwapLibrary.getAmountIn(amountOut, reserveIn, reserveOut, false, 0);
    }
    ...
    returns (uint[] memory amounts)
    {
        return NEWSwapLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    returns (uint[] memory amounts)
    {
        return NEWSwapLibrary.getAmountsIn(factory, amountOut, path);
    }
}

pragma solidity =0.6.6;

contract NEWSwapRouter is SupportingSwap {
    using SafeMath for uint;
    ...
    constructor(address _factory, address _WETH, address _BUSD, uint256 _adminFee, address _adminFeeAddress) public {
        ...
        INEWSwapFactory(_factory).routerInitialize(address(this));
    }
    ...
    function _addLiquidity(
        ...
        if (getPair(tokenA, tokenB) == address(0)) {
            if(tokenA == WETH) {
                INEWSwapFactory(factory).createPair(tokenB, tokenA);
                pairFeeAddress[getPair(tokenA,tokenB)] = tokenA;
            } else {
                INEWSwapFactory(factory).createPair(tokenA, tokenB);
                pairFeeAddress[getPair(tokenA,tokenB)] = tokenB;
            }
        }
        (uint reserveA, uint reserveB) = NEWSwapLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = NEWSwapLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "NEWSwapRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = NEWSwapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, "NEWSwapRouter: INSUFFICIENT_A_AMOUNT");
    ...
    function getPair(address tokenA,address tokenB) public view returns (address){
        return INEWSwapFactory(factory).getPair(tokenA, tokenB);
    }
    ...
    function addLiquidity(
        ...
        address pair = NEWSwapLibrary.pairFor(factory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = INEWSwapPair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
        ...
        address pair = NEWSwapLibrary.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = NEWSwapLibrary.pairFor(factory, tokenA, tokenB);
        INEWSwapPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = INEWSwapPair(pair).burn(to);
        (address token0,) = NEWSwapLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, "NEWSwapRouter: INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "NEWSwapRouter: INSUFFICIENT_B_AMOUNT");
    }
    function removeLiquidityETH(
        address token,
        uint liquidity,
    ...
    function removeLiquidityWithPermit(
        ...
        INEWSwapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
