...
contract Project is Initializable, OwnableUpgradeable {
    ...
    event SetAdmin(address[] accounts, bool allow);
    ...
    /// @notice Delegate admins permission to account
    /// @param  _accounts accounts that set the admin
    /// @param  _allow  setting value
    function setAdmins(address[] memory _accounts, bool _allow) external onlyOwner {
        for (uint256 i; i < _accounts.length; i++) {
            require(_accounts[i] != address(0), "Invalid account");
            require(admins[_accounts[i]] != _allow, "Duplicate setting");
            admins[_accounts[i]] = _allow;
        }
        emit SetAdmin(_accounts, _allow);
    }
    ...
    function setJoinTime(uint256 _projectId, uint256 _start, uint256 _end) external projectExists(_projectId) onlyManager(_projectId) {
        ...
        require(project.status == ProjectStatus.STARTED, "Status project invalid");
    ...
    function setIDO(IDO memory _params) external projectExists(_params.projectId) onlyManager(_params.projectId) {
        ...
        require(project.status == ProjectStatus.INACTIVE, "Status project invalid");
        ...
        require(_params.distributionStart > _params.saleEnd, "Invalid distribution start");

        if (_params.minSales > 0) {
            require(_params.minSales <= sale.getSalesProject(_params.projectId).length, "Invalid minSales");
        ...
        project.distributionStart = _params.distributionStart;
