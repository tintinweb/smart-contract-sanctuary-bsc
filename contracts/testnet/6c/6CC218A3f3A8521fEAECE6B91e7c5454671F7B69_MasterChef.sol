...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    modifier onlySystem {
        require(_msgSender() == oraculaSystem || _msgSender() == owner(), "Contract: Not authorised.");
        _;
    }
    ...
    function harvest(uint256 _pid, address _to) public nonReentrant validatePoolByPid(_pid) returns (bool success) {
        if (_msgSender() != owner()) {
            require(_to == _msgSender(), "Contract: Claiming for someone else is not allowed");
        }
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function harvestFromAllPools(address _to) public nonReentrant returns (bool success) {
    ...
    function getCurrentRewardBalance() external view returns (uint256 rewardBalance) {
        uint256 userAmount;
        for (uint256 i; i < allUsers.length; i++) {
            for (uint256 x; x < poolInfo.length; x++) {
                userAmount += userInfo[x][allUsers[i]].amount;
            }
        }
        rewardBalance = ORACULA.balanceOf(address(this)) - userAmount;
    }

    function getAmountOfRewardsNeeded(uint256 _pid) external view validatePoolByPid(_pid) returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        if (pool.endBlock <= block.number) {
            return 0;
        }
        if (pool.startBlock > block.number) {
            return pool.rewardForEachBlock.mul(pool.endBlock.sub(pool.startBlock));
        } else {
            return pool.rewardForEachBlock.mul(pool.endBlock.sub(block.number));
        }
    }
    ...
    function emergencyStop(bool _withHarvest) external onlyOwner {
        for (uint256 i; i < poolInfo.length; i++) {
            emergencyClosePool(i);
        }
        if (_withHarvest) {
            for (uint256 k; k < allUsers.length; k++) {
                harvestFromAllPools(allUsers[k]);
            }
        }
        ORACULA.safeTransfer(owner(), ORACULA.balanceOf(address(this)));
