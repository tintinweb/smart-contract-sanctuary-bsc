...
contract Hold2Gold is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 10**15 * 10**9;                       // Total Supply: 1,000,000,000,000,000
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Hold2Goldv41";
    string private _symbol = "Hold2Goldv41";
    uint8 private _decimals = 18;
    
    uint256 public _liquiditySellFee = 0;                           // Liquidity Fee to sell
    uint256 public _liquidityBuyFee = 0;                            // Liquidity Fee to buy

    uint256 public _rewardSellFee = 3;                              // Reflected reward Fee to sell
    uint256 public _rewardBuyFee = 3;                               // Reflected reward Fee to buy

    uint256 public _marketingSellFee = 1;                           // Marketing Fee percent to sell
    uint256 public _marketingBuyFee = 1;                            // Marketing Fee percent to buy

    uint256 public _charitySellFee = 1;                             // Charity Fee percent to sell
    uint256 public _charityBuyFee = 1;                              // Charity Fee percent to buy

    uint256 public _burnSellFee = 3;                                // Burn Fee percent to sell
    uint256 public _burnBuyFee = 3;                                 // Burn Fee percent to buy
    ...
    address public marketingAddress = 0x76F74229B31B1D737586A1bC4f44d9DEdD940Fdb;
    address public charityAddress = 0xe5F53d5401e1F5B1Ec7A4A4C9DF65B6949ce8aB1;
    ...
    constructor () {
        ...
        _isExcludedFromFee[0xCda1e27a63E6c570a79CBe7f24BDFa99FFF2ECa0] = true;
        _isExcludedFromFee[address(this)] = true;
        
        emit Transfer(address(0), 0xCda1e27a63E6c570a79CBe7f24BDFa99FFF2ECa0, _tTotal);
    }
    ...
    function setCharitySellFeePercent(uint256 charitySellFee) external onlyOwner() {
        _charitySellFee = charitySellFee;
    }

    function setCharityBuyFeePercent(uint256 charityBuyFee) external onlyOwner() {
        _charityBuyFee = charityBuyFee;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != pancakeswapV2Pair, "Hold2Goldv41: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Hold2Goldv41: Automated market maker pair is already set to that value");
    ...
    function _takeCharity(address sender, uint256 tAmount, uint256 currentRate) private {
    ...
    function _takeMarketing(address sender, uint256 tAmount, uint256 currentRate) private {
    ...
    function setSellFees() private {
        ...
        _charityFee = _charitySellFee;
        _marketingFee = _marketingSellFee;
        _burnFee = _burnSellFee;

        _totalFee = _rewardFee.add(_liquidityFee).add(_charityFee).add(_marketingFee).add(_burnFee);
    }

    function setBuyFees() private {
        _rewardFee = _rewardBuyFee;
        _liquidityFee = _liquidityBuyFee;
        _charityFee = _charityBuyFee;
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, burn, liquidity, charity, marketing fee
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        if (_totalFee > 0) {
            ...
            _takeMarketing(sender, tAmount, currentRate);
            _takeCharity(sender, tAmount, currentRate);
