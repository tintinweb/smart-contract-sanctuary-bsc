...
abstract contract PostDeliveryCrowdsale is CappedCrowdsale {
    ...
    function withdrawTokens(address beneficiary) public {
        ...
        require(block.timestamp >= closingTime() + 2 minutes, "2 minutes lock up");

        uint256 amount = _balances[beneficiary];
        require(amount > 0, "FlexvisPresale: beneficiary is not due to any tokens");

        if(block.timestamp >= closingTime() + 4 minutes){
             _vault.transfer(token(), beneficiary, amount);
            _balances[beneficiary] = 0;
        }
        else if (block.timestamp >= closingTime() + 2 minutes){
...
abstract contract Crowdsale is Context, ReentrancyGuard {
    ...
    uint256 public constant MIN_AMOUNT = 0.005e18;
    uint256 public constant MAX_AMOUNT = 10e18;
  ...
  function isContract(address account) internal view returns (bool) {
    ...
    function buyTokens(address beneficiary) public virtual nonReentrant payable {
        require(msg.value >= MIN_AMOUNT && msg.value <= MAX_AMOUNT, "Amount is not within the range");
        require(!isContract(msg.sender), "Contract address revoked");
