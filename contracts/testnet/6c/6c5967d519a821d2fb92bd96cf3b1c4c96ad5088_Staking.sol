...
contract Staking {
    ...
    constructor(address _token, address _txAddress) {
        owner = msg.sender;
        endTime = 365 days;
        token = BEP20(_token);  
        txAddress = _txAddress;

        pools.push(Pool(365 days, 80, 30 days));
        pools.push(Pool(365 days, 100, 60 days));
        pools.push(Pool(365 days, 120, 90 days));
        pools.push(Pool(365 days, 200, 100 days));
    }
    ...
    function stakeTokens(uint _amount, uint _pool) public {
        ...
        if(_pool == 1){
            txPercent = 1;
            txAmount = _amount.mul(1).div(100);
        }
        else if(_pool == 2){
            txPercent = 15;
            txAmount = _amount.mul(15).div(1000);
        }
        else if(_pool == 3){
            txPercent = 2;
            txAmount = _amount.mul(2).div(100);
        }
        else if(_pool == 4){
            txPercent = 1;
            txAmount = _amount.mul(1).div(100);
            poolFourCount++;
            pools[3].percent = pools[3].percent.div(poolFourCount);
        }
        else{
            revert('Invalid Pool Id');
        }

        stakeAmount = _amount.sub(txAmount);
    ...
    function withdrawable(address addr, uint index) public view returns (uint reward) {
        ...
        if(stk.status[index] == true){
            if(pool == 1) {
                ...
                percent = pools[0].percent;
            }
            else if(pool == 2) {
                end = pools[1].endTime;
                percent = pools[1].percent;
            }
            else if(pool == 3) {
                end = pools[2].endTime;
                percent = pools[2].percent;
            }
            else if(pool == 4) {
                end = pools[3].endTime;
                percent = pools[3].percent;
            }

            uint since = stk.stakedAt[index];
            uint till = block.timestamp > end ? end : block.timestamp;
            reward = amount.mul(till.sub(since)).mul(percent).div(end).div(100);
    ...
    */
    ...
    // View user details
    function details(address addr) public view returns(uint[] memory amount, uint[] memory time, bool[] memory stat, uint[] memory pool) {
        Stake storage stk = user[addr];
    ...
    // View tax address
    function getTxAddress() public view returns (address) {
        return txAddress;
    }
