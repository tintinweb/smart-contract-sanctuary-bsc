...
contract AggregatorExecutor is Ownable {
    ...
    ensure(_deadline)
    {
        ...
        checkSigner(_signature.data, _steps, _signature.number);
        //check input eth
    ...
    //=========== internal functions ============//
    function checkSigner(bytes memory _signature, SwapStep[] memory _path, uint256 _nonce) internal{
        require(!usedNonce.get(_nonce), "AggregatorExecutor: _nonce has been used");
        bytes32 _hash = keccak256(abi.encode(address(this), _path, _nonce)).toEthSignedMessageHash();
    ...
    function verifySignature(bytes memory _signature, address[] memory _path, uint256 _nonce) external view returns(bool) {
        require(!usedNonce.get(_nonce), "AggregatorExecutor: _nonce has been used");
        bytes32 _hash = keccak256(abi.encodePacked(address(this), _path, _nonce)).toEthSignedMessageHash();
        address _signer = _hash.recover(_signature);
