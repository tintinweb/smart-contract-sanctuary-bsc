...
interface ICardNFT
{
     ...
     function getCardToStageData(uint256 cardid) external returns(uint256[] memory) ;
}
interface IERC20 {
    /**
 ...
 contract NFTMetaSub is ERC721,Ownable {
    ...
    IERC20 public _ttcToken;//扣除手续费的代币地址
    uint256 public  _fee=3*10**18;//扣除手续费的额度
    ...
    struct CardNFTConfig{
        uint256 token_id;
        uint256 limtSubCount;
    }



    CardNFTConfig public cardNftConfig;//当前卡片NFT
    mapping(uint256=>CardNFTConfig) public cardNftData;
    ...
    function setNftCardConfig(uint256  token_id,uint256 limtSubCount) public onlyOwner
    {
        cardNftConfig=CardNFTConfig(token_id,limtSubCount);
        cardNftData[token_id]=CardNFTConfig(token_id,limtSubCount);
    } 
    ...
    //设置白名单
    function setWhiteList(address userAddress, bool _states) public  {
    ...
    function _mintNicMeta(uint256 tokenQuantity) internal {
        for (uint256 i = 0; i < tokenQuantity; i++) {
            ...
            if (totalSupply() < MAX_SUPPLY) {
                ...
                require(cardNft[cardNftConfig.token_id].length<=cardNftConfig.limtSubCount,'SUBNFTMaximum exceeded ');
                cardNft[cardNftConfig.token_id].push(mintIndex);//当前卡牌总共开启几个了
                subNftToCard[mintIndex]=cardNftConfig.token_id;//当前道具对应的是哪个卡牌
