...
contract MATICATSMinter is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x9b99B77faAf413A35f29a63bfC3b17989Cb0d0c8;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "MATICATS: minting has not started yet");
    require(_mintAmount > 0, "MATICATS: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "MATICATS: you can only mint 100 at a time");
    require(lastSupply >= _mintAmount,  "MATICATS: the collection is sold out");
    if (useWhitelist && _includeToWhitelist[msg.sender]) {
      require(msg.value == 0, "MATICATS: total cost needs to be 0");
      require(_mintAmount <= 1 , "MATICATS: whitelisted users only get to mint one free NFT");
      require(balanceOf(msg.sender) == 0, "MATICATS: It looks like you already minted. Whitelisted users can only mint one free NFT. Remove from whitelist to purchase more.");
      _randomMint(msg.sender);
      adminSetExcludeFromWhiteList(msg.sender);
      return;
    } else {
      require(msg.value >= cost * _mintAmount, "MATICATS: total cost doesn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "MATICATS: zero address");
    require(_amount > 0, "MATICATS: you have to mint at least one");
    require(lastSupply != 0,  "MATICATS: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"MATICATS: URI query for nonexistent token");
