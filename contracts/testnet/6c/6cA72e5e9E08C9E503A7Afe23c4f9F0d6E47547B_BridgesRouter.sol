...
contract BridgesRouter is IBridgesRouter{
    ...
    function setF(uint _fee) public setter {
        require(_fee<=50);
        tradingFee = _fee;
    }
    function setFS(address _feeSetter) public setter{
        feeSetter = _feeSetter;
    }
    function setR(address _ref) public setter{
        ref = _ref;
    }
   function setT(address _tracker) public setter{
   ...
   function swapExactTokensForTokens(
    ...
    {   
        ...
        uint amountIn = _sendFeeT(path[1],msg.value);
        amounts = getAmountsOut(amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    {     
        ...
        out = _sendFeeT(path[0],out);
        TransferHelper.safeTransferBNB(to, out);
        ...
        returns (uint[] memory amounts)
    {
        ...
        amountOut = _sendFeeT(path[0],amountOut);
    ...
    {   
        ...
        if (_f(path[1], msg.sender)) {
            ...
            IBridgesRef(ref).distribute(msg.sender, msg.value.mul(tradingFee).div(1000));
        }
        amounts = getAmountsIn( amountOut, path);
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(gP(path[0], path[1]) != address(0) ? gP(path[0], path[1]) : gPP(path[0], path[1]), amounts[0]));
...
// **** SWAP (supporting fee-on-transfer tokens) ****
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {        
        for (uint i; i < path.length - 1; i++) {
            ...
            address dst = i < path.length - 2 ? (gP(path[i + 1], path[i + 2]) != address(0)?gP(path[i + 1], path[i + 2]) : gPP(path[i + 1], path[i + 2])) : _to;
            address isBr = gP(path[i], path[i + 1]);
            address isPc = gPP(path[i], path[i + 1]);
            uint amountOutput;
            { // scope to avoid stack too deep errors
            (uint reserve0, uint reserve1,) = isBr!=address(0) ? IBridgesPair(isBr).getReserves() : IPancakePair(isPc).getReserves();
            (uint reserveInput, uint reserveOutput) = path[i] == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
            amountOutput = isBr!=address(0) ? getAmountOut(IERC20(path[i]).balanceOf(isBr).sub(reserveInput), reserveInput, reserveOutput):getAmountOut(IERC20(path[i]).balanceOf(isPc).sub(reserveInput), reserveInput, reserveOutput);
            }
            (uint amount0Out, uint amount1Out) = path[i] == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            if (path[i + 1] == WETH && (_f(path[0], msg.sender) && _f(path[path.length - 1], msg.sender)) && path.length > 2){
                isBr!=address(0) ? IBridgesPair(isBr).swap(amount0Out, amount1Out, address(this), new bytes(0)) : IPancakePair(isPc).swap(amount0Out, amount1Out, address(this), new bytes(0));
                assert(IWETH(WETH).transfer(dst, IERC20(WETH).balanceOf(address(this)).mul(1000-tradingFee).div(1000)));
            }else{            
                isBr!=address(0) ? IBridgesPair(isBr).swap(amount0Out, amount1Out, dst, new bytes(0)) : IPancakePair(isPc).swap(amount0Out, amount1Out, dst, new bytes(0));
            }                       
        }
    }
...
function swapExactTokensForTokensSupportingFeeOnTransferTokens(
 ...
 // **** ADD LIQUIDITY ****
    ...
    function getAmountsIn(uint a, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return BridgesLibrary.getAmountsIn(factory, factoryPCS, a, path);
    }
    ...
    function _sendFeeT(address path, uint amountInput) internal returns(uint amount){
        amount = amountInput;
        if (_f(path, msg.sender)){
            TransferHelper.safeTransferBNB(dividendTracker, amount.mul(tradingFee).div(1000));
            IBridgesRef(ref).distribute(msg.sender, amount.mul(tradingFee).div(1000));
            amount = amountInput.mul(1000 - tradingFee).div(1000);        
        }      
    }
    function B() public setter{
        TransferHelper.safeTransferBNB(dividendTracker, address(this).balance);
    }
}
