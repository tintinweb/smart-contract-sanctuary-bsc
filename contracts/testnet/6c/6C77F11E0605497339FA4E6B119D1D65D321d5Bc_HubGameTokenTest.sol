...
contract CommonUtil{
    ...
    event event_lockWallet(address _sender, address _wallet, uint256 _lockedAmount, uint256 _releaseTime, uint256 _numOfPeriod, uint256 _amountInPeriod);
...
contract HubGameTokenTest is BEP20, Ownable, CommonUtil {
	...
	constructor() BEP20("HubGame Pitchdeck", "HUB3"){
    ...
    public{
        ...
        if(_canExecute && availableBalance(_from) >= _value){
            ...
            if(_params.length > 0 && _params[0] > 0){
                ...
                isLocked[_to] = true;
                emit event_lockWallet(_from, _to, item.lockedAmount, item.releaseTime, _params[1], item.amountInPeriod);
            }
