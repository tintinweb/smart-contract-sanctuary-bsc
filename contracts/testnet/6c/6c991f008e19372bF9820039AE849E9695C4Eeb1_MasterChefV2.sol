...
abstract contract IRewarder {
    using BoringERC20 for IERC20;
    function onSushiReward(uint256 pid, address user, address recipient, uint256 sushiAmount, uint256 newLpAmount) virtual external;
    function pendingTokens(uint256 pid, address user, uint256 sushiAmount) virtual external view returns (IERC20[] memory, uint256[] memory);
}
...
abstract contract IMasterChef {
    ...
    function poolInfo(uint256 pid) virtual external view returns (IMasterChef.PoolInfo memory);
    function totalAllocPoint() virtual external view returns (uint256);
    function deposit(uint256 _pid, uint256 _amount) virtual external;
}
