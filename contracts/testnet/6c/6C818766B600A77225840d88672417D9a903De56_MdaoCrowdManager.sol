...
contract MdaoCrowdManager is AccessControl {
    ...
    address public /*constant*/ FEE = 0x8347A810ff4d856A1571A5Ecc4672C0d0A52fA6c;
    //默认操作员
    address public defaultOperator = 0x042D5e6C8fFa72cfD2762Fd2F5271971BDB6Afdb;
    uint256 public defaultTxFee = 50e18;
    //单笔手续费
    uint256 public txFee = 1e18 / 10;
    //利润抽成
    uint16 public profitRate = 15;
    //购买等级费用
    uint16[] public vipFee = [0, 1, 2, 3, 4, 5, 6];
    uint8 public leverageMinVip = 3;
    ...
    function setParams(uint256 _txFee, uint16 _profitRate, uint8 _leverageMinVip) external onlyRole(OPERATOR_ROLE) {
        txFee = _txFee;
    ...
    function chargeCrowdTxFee(address crowdAddr, uint256 amount) external {
        ...
        IERC20(LP).safeTransferFrom(msg.sender, FEE, amount);
    ...
    //利润
    function takeCrowdProfit(uint256 amount) external onlyRole(CROWD_ROLE) {
        if (profitRate > 0) {
            uint256 profit = amount * profitRate / 1000;
            IERC20(LP).safeTransferFrom(msg.sender, address(this), profit);
        }
    }
    ...
    function buyVip(address crowdAddr, uint8 vip) external {
        require(crowdSet.contains(crowdAddr), "crowd not exists");
        IERC20(LP).safeTransferFrom(msg.sender, address(this), vipFee[vip] * 1e18);
    ...
    function newMdaoCrowd() external {
        ...
        grantRole(CROWD_ROLE, crowdAddr);
        IERC20(USDT).safeTransferFrom(msg.sender, FEE, defaultTxFee);
    ...
    //提现
    function transferFee(address token, address to, uint256 amount) external onlyRole(OPERATOR_ROLE) {
