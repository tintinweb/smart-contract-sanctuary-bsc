...
contract DAIDAIMintNFTByToken is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(string memory name_, string memory symbol_, string memory baseURI_, uint256 totalCount_, address wbnb_, uint startTimestamp_) public ERC721(name_, symbol_) {
    ...
    function getPlanLength() public view returns (uint256) {
        return plans.length;
    }
    ...
    function estimatePrice(uint256 price, uint256 _times) public view returns (uint256) {
    ...
    function mintNFTByToken(uint256 _planIndex, uint256 _amount, uint256 _times) payable external nonReentrant {
        require(started, "not started");
        require(block.timestamp > startTimestamp, "not start timestamp");
        require(_times >0 && _times <= maxBatchMint, "wrong batch number");
        require(totalSupply() + _times < totalCount, "wake too much");
        BusinessPlan memory plan = plans[_planIndex];
        require(plan.pricePerSolt > 0, "wrong price");
        uint256 amount = estimatePrice(plan.pricePerSolt, _times);
        require(_amount == amount, "value error");

        if(plan.sellToken != address(0)){
            IERC20(plan.sellToken).safeTransferFrom(address(msg.sender), owner(), _amount);
        } else {
            require(uint256(msg.value) == amount, "value error");
            IWETH(WBNB).deposit{value: msg.value}();
            IERC20(WBNB).safeTransfer(owner(), _amount);
        }
        ...
        emit mintNFTsByToken(msg.sender, plan.pricePerSolt, plan.sellToken, _times, _amount);
    }
