...
contract MoonRaiseAllocationSale is Ownable {
    ...
    uint256 public _totalAllocation = 4_600_000 ether;
    ...
    modifier onTimeUnlockMrt() {
        require(block.timestamp > _endSaleTime);
        _;
    }

    modifier onSaleTime() {
        require(
            block.timestamp >= _startSaleTime && block.timestamp <= _endSaleTime
        );
        _;
    }
    ...
    function setSaleTime(uint256 starTime, uint256 endTime) public onlyOwner {
        _startPrepareTime = starTime;
    ...
    function buy(uint256 amountInUsd) public onSaleTime {
        require(
            _tokenSale.balanceOf(msg.sender) >= amountInUsd,
            "not enough balance"
        );
        uint256 maxAllocation = (_balanceMrtLocks[msg.sender] * _ZOOM) /
            _totalMrtLock;
        uint256 amountInTokenSale = amountInUsd / (_ratioA / _ratioB);
        require(
