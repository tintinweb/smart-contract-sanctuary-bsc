...
contract PoolFactory is Initializable, AccessControl {
    ...
    event PoolCreated0(address createdBy, address indexed token, address indexed pool, uint poolId, uint dbProjectId);
    event PoolCreated1(address createdBy, address indexed token, address indexed pool, address IDOToken, address purchaseToken, uint maxPurchaseAmountForKYCUser, uint maxPurchaseAmountForNotKYCUser, uint TGEDate, uint TGEPercentage, uint galaxyParticipationFeePercentage, uint crowdfundingParticipationFeePercentage);
    event PoolCreated2(address createdBy, address indexed token, address indexed pool, uint galaxyPoolProportion, uint earlyAccessProportion, uint totalRaiseAmount, uint whaleOpenTime, uint whaleDuration, uint communityDuration, uint rate, uint decimal);
    event PoolCreated(address[4] addrs, uint[14] uints);
    ...
    function createPool(address[2] memory addrs, uint[14] memory uints, uint dbProjectId) external returns (address pool) {
        ...
        */
        emit PoolCreated0(_msgSender(), _IDOToken, pool, allPools.length - 1, dbProjectId);
        emit PoolCreated1(_msgSender(), _IDOToken, pool, addrs[0], addrs[1], uints[0], uints[1], uints[2], uints[3], uints[4], uints[5]);
        emit PoolCreated2(_msgSender(), _IDOToken, pool, uints[6], uints[7], uints[8], uints[9], uints[10], uints[11], uints[12], uints[13]);
        
        emit PoolCreated([_msgSender(), pool, addrs[0], addrs[1]], [uints[0], uints[1], uints[2], uints[3], uints[4], uints[5], uints[6], uints[7], uints[8], uints[9], uints[10], uints[11], uints[12], uints[13]]);
    }
