...
contract MyDiamondTeamV2 {
    ...
    struct UserDepositReal {
        uint256 deposits; // real amount of BNB
    }
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        //checking can be removed if not needed.
        if(users[up].deposit_amount.add(_amount) < this.maxReinvestOf(usersRealDeposits[up].deposits))
        {
    ...
    function reinvestAmountOf(address _addr, uint256 _toBeRolledAmount) view public returns(uint256 reinvestAmount) {
        ...
        uint256 maxReinvestAmount = this.maxReinvestOf(usersRealDeposits[_addr].deposits); 

        reinvestAmount = _toBeRolledAmount; 

        if(usersRealDeposits[_addr].deposits >= maxReinvestAmount) { // user already got max reinvest
