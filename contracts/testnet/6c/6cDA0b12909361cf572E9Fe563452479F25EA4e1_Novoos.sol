...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 REWARD = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // BUSD 
    IDEXRouter router;
...
contract Novoos is ERC20Detailed, Ownable {
    ...
    string public _name = "Novoos";
    string public _symbol = "NOVO";
    uint8 public _decimals = 9;

    IDEXPair public pairContract;
    
    mapping(address => bool) _isMaxWalletExempt;//for anti max wallet
    mapping(address => bool) _isMaxTradeExempt;//for anti dumping 
    ...
    uint256 public AllowanceFee = 4;
    uint256 public BankFee = 3;
    uint256 public sellFee = 2;
    uint256 public BurnFee = 2;
    uint256 public rewardFee = 2;
    uint256 public totalFee =
        liquidityFee.add(AllowanceFee).add(BankFee).add(
            BurnFee
        ).add(rewardFee);
    ...
    address public AllowanceReceiver;
    ...
    uint256 public gonMaxTrade = TOTAL_GONS.div(100).mul(100);
    ...
    constructor() ERC20Detailed("Novoos", "NOVO", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x63CF5Bac19c64c6E6bA33180e577ff4A6bFE049f;
        AllowanceReceiver = 0x1bF99f349eFdEa693e622792A3D70833979E2854; 
        BankReceiver = 0x1bF99f349eFdEa693e622792A3D70833979E2854;
        ...
        isDividendExempt[AllowanceReceiver] = true;
        isDividendExempt[BankReceiver] = true;

        _totalSupply = INITIAL_FRAGMENTS_SUPPLY;
        _gonBalances[AllowanceReceiver] = TOTAL_GONS;
        ...
        _isFeeExempt[AllowanceReceiver] = true;
        ...
        _isMaxTradeExempt[pair] = true;
        _isMaxTradeExempt[DEAD] = true;
        _isMaxTradeExempt[address(this)] = true;
        _isMaxTradeExempt[msg.sender] = true;

        

        _transferOwnership(AllowanceReceiver);
        emit Transfer(address(0x0), AllowanceReceiver, _totalSupply);
    }
    ...
    function rebase() internal {
        ...
        rebaseRate = 4029;//0.0429 percent every 3 seconds
        //423,543.25% apy
    ...
    function _transferFrom(
        ...
        if (!_isMaxTradeExempt[sender]) {
            require(
                (gonAmount) <= gonMaxTrade,
                "can not dump more than max trade"
            );
        }
    ...
    function takeFee(
        ...
        uint256 _AllowanceFee = AllowanceFee;

        if (recipient == pair) {
            _totalFee = totalFee.add(sellFee);
            _AllowanceFee = AllowanceFee.add(sellFee);
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_AllowanceFee.add(BankFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(AllowanceReceiver).call{
            value: amountETH.mul(AllowanceFee).div(totalFee),
            gas: 30000
        }("");
        (success, ) = payable(BankReceiver).call{
            value: amountETH.mul(BankFee).div(totalFee),
            gas: 30000
        }("");
    }

    function withdrawAllToAllowance() external swapping onlyOwner {

        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no Novoos token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function setFeeReceivers(
        ...
        address _AllowanceReceiver,
        address _BankReceiver,
        address _Burn
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        AllowanceReceiver = _AllowanceReceiver;
