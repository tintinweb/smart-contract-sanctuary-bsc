...
contract AmoebaBreeder {
    ...
    function divideAmoeba(address _ref) public {
        ...
        if (_ref != msg.sender) {
            ...
            referralData[_ref].rebates.push(calculateAmoebaMerge(amoeba));
        }
    ...
    function getReferralAllRebate(address _address) public view returns(uint allReward) {
        uint rebates = referralData[_address].rebates.length;
        for(uint i; i < rebates; i++) {
            allReward += referralData[_address].rebates[i];
        }
    }
