...
contract AurumArcade is Context, Ownable {
    ...
    * ---------------------------------------------- */
    ...
    function createContest(bytes32 _gameName, uint256 _entryFee) external onlyOwner {
        
        // contest creation conditions
        require(_gameName.length > 0, "GameName is required");  
        require(_gameName.length > 32, "GameName cannot be longer than 32 characters");       
    ...
    * ---------------------------------------------- */
    ...
    struct InGameConsumablesEntity {
        ...
        bytes32 gameName;                           // name of game 
        bytes32 consumableName;                     // name of consumable 
    ...
    function createConsumable(bytes32 _gameName, bytes32 _consumableName, uint256 _consumablePrice, uint256 expiryDays) external onlyOwner {
        
        // contest creation conditions
        require(_gameName.length > 0, "GameName is required");  
        require(_gameName.length > 32, "GameName cannot be longer than 32 characters");      
        require(_consumableName.length > 0, "ConsumableName is required");  
        require(_consumableName.length > 32, "ConsumableName cannot be longer than 32 characters");       
    ...
    * ---------------------------------------------- */
    ...
    uint256 public totalChallengesCreated = 0;          // total number of challenges 
  	...
  	// @dev Arcade Public functions -------------------------------------
     function createChallenge(bytes32 _gameName, uint256 _challengeStake, uint256 _challengeScore) external {
