 ...
 * Each user's reward is proportional to their staked out of the whole system's staked amount.
 * The rewards are accumulated per second.
 * Users can claim their rewards any time.
 *
 * Modified from: https://github.com/Synthetixio/synthetix/blob/develop/contracts/StakingRewards.sol
 */
contract StakingRewards is UUPSUpgradeable, TwoStageOwnableUpgradeable, PausableUpgradeable {
    ...
    address public rewardPool; // reward tokens are funded by this pool
    ...
    uint256 public feeScheduleTimeScale;
    address public feeCollector;

    // unstake restrictions
    uint256 public unstakeUnlockingTime; // in secs
    ...
    function initialize(
        ...
        require(rewardPool_ != address(0), "Reward pool cannot be zero address");
        require(feeScheduleTimeScale_ > 0, "Fee schedule time scale cannot be 0");
        require(feeCollector_ != address(0), "Fee collector cannot be zero address");
        ...
        unstakeUnlockingTime = unstakeUnlockingTime_;
        feeScheduleTimeScale = feeScheduleTimeScale_;
    ...
    /**
     * Returns how much reward the user has earned now.
     */
    ...
    /**
     * Returns the current reward per second for a user.
     */
    ...
    function lastStakeTimeOf(address account) external view returns (uint256) {
        return _lastStakeTime[account];
    }

    function weightedAvgStakeTimeOf(address account) external view returns (uint256) {
    ...
    function getUnstakeFeePercent(address account) public view returns(uint256) {
        ...
        uint256 scaledTimeSinceWeightedAvgStakeTime = (block.timestamp - weightedAvgStakeTime) / feeScheduleTimeScale;
    ...
    function stake(uint256 amount) external whenNotPaused {
        ...
        uint256 prevBalance = _balances[msg.sender];
        uint256 prevWeightedAvgStakeTime = _weightedAvgStakeTime[msg.sender];

        _totalSupply = _totalSupply.add(amount);
        uint256 newBalance = prevBalance.add(amount);
        ...
        _weightedAvgStakeTime[msg.sender] = block.timestamp.sub((block.timestamp.sub(prevWeightedAvgStakeTime)).mul(prevBalance).div(newBalance));
    ...
    function unstake(uint256 amount) public whenNotPaused {
        ...
        require(block.timestamp >= (_lastStakeTime[msg.sender] + unstakeUnlockingTime), "Unstake time not passed");

        _updateReward(msg.sender);
        
        uint256 balance = _balances[msg.sender];
        require(amount <= balance, "Cannot unstake more than balance");

        // figure out the fee based on stake time
        uint256 fee = amount.mul(getUnstakeFeePercent(msg.sender)).div(10000);
        ...
        // transfer amount - fee
        stakingToken.safeTransfer(msg.sender, netUnstake);
    }
     ...
     * The reward pool must approve enough token amounts to be transferred to this contract.
     ...
     * the new reward amount, it will transfer the unneeded amount back to the reward pool.
     */
    function setReward(uint256 rewardAmount, uint256 rewardsDuration_) external onlyOwner {
        require(rewardAmount > 0, "Reward amount must be more than 0");
        require(rewardsDuration_ > 0, "Reward duration must be more than 0");
        ...
        if (rewardAmount > leftOver) {
            ...
            rewardsToken.safeTransfer(rewardPool, transferAmount);
        }
    }
    ...
    function setRewardPool(address rewardPool_) external onlyOwner {
        require(rewardPool_ != address(0), "Reward pool cannot be zero address");
        rewardPool = rewardPool_;
        emit RewardPoolChanged(rewardPool_);
    }
