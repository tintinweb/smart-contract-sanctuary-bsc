...
contract Campaign is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    mapping(address => address) public mappingRefs; //referral address
    mapping(address => uint256) public mappingClaims; // claimable ref box of user
    mapping(address => uint256) public mappingBoughtBoxes; //total box user bought, ignore type of box
    uint256 public currentCampaignId = 0;

    uint256 public maxBuySpecialBox = 3;
    uint256 public maxBoxDiscount = 3;
    uint256 public minBoxForReward = 5;
    uint256 public boxRefId = 5;
    ...
    event ClaimReward(address indexed account, uint256 indexed boxRefId, uint256 indexed quantity);
    ...
    event UpdateQuoteTokenKey(address indexed sender, uint256 campaignId, uint256 quoteTokenKey);
    event UpdateCampaignType(address indexed sender, uint256 campaignId, uint256 campaignType);
    ...
    function setMaxBoxDiscount(uint256 _maxBoxDiscount) public onlyOwner {
        require(_maxBoxDiscount >= 0, "Invalid Max Box Discount");
        require(_maxBoxDiscount != maxBoxDiscount, "No Need To Update");
        maxBoxDiscount = _maxBoxDiscount;
    }
    ...
    function setBoxRefId(uint256 _boxRefId) external onlyOwner {
        require(_boxRefId > 0, "Invalid Box Ref Id");
        require(_boxRefId != boxRefId, "No Need To Update");
        boxRefId = _boxRefId;
    }

    function setMinBoxForReward(uint256 _minBoxForReward) external onlyOwner {
        require(_minBoxForReward > 0, "Invalid Min Box For Reward");
        require(_minBoxForReward != minBoxForReward, "No Need To Update");
        minBoxForReward = _minBoxForReward;
    }
...
//        require(mysteryBoxNFT.whiteListCreators(address(this)) == true, 'Not Approve For Contract');
    ...
    function buyMysteryBox(
        ...
        uint256 numberDiscountBox = 0;
        if (_refAccount != address(0) && _refAccount != address(this) && mappingRefs[_msgSender()] == address(0)) {
            mappingRefs[_msgSender()] = _refAccount;
        }

        if (mappingRefs[_msgSender()] != address(0)) {
            if (mappingBoughtBoxes[_msgSender()] < maxBoxDiscount) {
                uint256 remainDiscount = maxBoxDiscount.sub(mappingBoughtBoxes[_msgSender()]);
                if (remainDiscount >= _quantity) {
                    numberDiscountBox = _quantity;
                } else {
                    numberDiscountBox = _quantity.sub(remainDiscount);
                }
            }

            if (mappingBoughtBoxes[_msgSender()] < minBoxForReward) {
                if (mappingBoughtBoxes[_msgSender()].add(_quantity) >= minBoxForReward) {
                    mappingClaims[mappingRefs[_msgSender()]] = mappingClaims[mappingRefs[_msgSender()]].add(1);
                }
            }
            mappingBoughtBoxes[_msgSender()] = mappingBoughtBoxes[_msgSender()].add(_quantity);
        }
        ...
        uint256 discount = numberDiscountBox.mul(campaign.mysteryBoxInfo[_mysteryBoxId].price).mul(discountPercent).div(ZOOM);
        ...
        if (campaign.quoteTokenKey == 0) {
            ...
            require(quoteErc20.balanceOf(_msgSender()) >= totalToken, 'Not Enough ATF To Buy');
    ...
    function claimReward(uint256 _amount) external whenNotPaused {
        require(_amount > 0, 'Invalid Amount');
        require(mappingClaims[_msgSender()] >= _amount, "Max Reward!");
        mappingClaims[_msgSender()] = mappingClaims[_msgSender()].sub(_amount);
        mysteryBoxNFT.mintMysteryBox(_msgSender(), boxRefId, _amount);
        emit ClaimReward(_msgSender(), boxRefId, _amount);
    }
    ...
    function updateQuoteTokenKey(uint256 campaignId, uint256 quoteTokenKey) public onlyWhiteListUser {
        ...
        emit UpdateQuoteTokenKey(_msgSender(), campaignId, quoteTokenKey);
    }


    function updateCampaignType(uint256 campaignId, uint256 campaignType) public onlyWhiteListUser {
        require(campaignType == 0 || campaignType == 1, 'Invalid Campaign Type');

        CampaignInfo storage campaign = campaigns[campaignId];
        require(campaign.campaignType != campaignType, "No Need To Update!");
        campaign.campaignType = campaignType;
        emit UpdateCampaignType(_msgSender(), campaignId, campaignType);
    }
    ...
    function getCampaignBoxInfo(uint256 _campaignId, uint256 _mysteryBoxId) public view returns (CampaignReturnInfo memory) {
        CampaignInfo  storage campaign = campaigns[_campaignId];
        CampaignReturnInfo memory result;
        result.campaignId = _campaignId;
        result.start = campaign.start;
