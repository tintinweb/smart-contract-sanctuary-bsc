...
contract Swap is Pausable, AccessControl {
  ...
  struct TradeOffer {
    ...
    address trader;
    address friend;
    bool isAcceptedByTrader;
    bool isAcceptedByFriend;
  }
  ...
  constructor(
    ...
    require(_owner != address(0), "IA");
    require(_acceptedToken != IERC20(address(0)), "IA");
    require(_trustedToken != IERC721(address(0)), "IA");
    ...
    TradeOffer memory tradeOffer = TradeOffer(0, 0, 0, address(0), address(0), false, false);
  ...
  function getOffer(uint256 _offerId) public view returns (TradeOffer memory) {
  ...
  function getOffers() public view returns (TradeOffer[] memory) {
    return offers;
  }

  function getOfferIdCounter() public view returns (uint256) {
    return _offerIdCounter.current();
  }

  function getOfferId(uint256 _offerIndex) public view returns (uint256) {
    require(_offerIndex < offers.length, "IID");
    return offers[_offerIndex].id;
  }

  function setAccpetedToken(IERC20 _acceptedToken) external onlyRole(OPERATOR_ROLE) {
    acceptedToken = _acceptedToken;
  }
  ...
  function createTradeOffer(
    ...
    require(_traderTokenId != _friendTokenId, "IT");

    require(_friend != msg.sender, "EA");
    require(msg.sender != address(0), "SZA");
    require(_friend != address(0), "FZA");

    require(trustedToken.ownerOf(_traderTokenId) == msg.sender, "ITO");
    require(trustedToken.ownerOf(_friendTokenId) == _friend, "ITO");
    ...
    TradeOffer memory offer = TradeOffer({
      ...
      isAcceptedByTrader: false,
      isAcceptedByFriend: false
    });
  ...
  function acceptTradeOffer(uint256 offerId) external {
    TradeOffer storage offer = offers[offerIdToOfferIndex[offerId]];
    require(msg.sender == offer.trader || msg.sender == offer.friend, "INV_SENDER");

    if (msg.sender == offer.trader) {
      offer.isAcceptedByTrader = true;
    } else {
      offer.isAcceptedByFriend = true;
    }

    emit TradeOfferAccepted(offer.id, offer.friendTokenId, offer.traderTokenId, msg.sender);
  }

  function rejectTradeOffer(uint256 offerId) external {
    TradeOffer storage offer = offers[offerIdToOfferIndex[offerId]];
    require(msg.sender == offer.trader || msg.sender == offer.friend, "INV_SENDER");

    if (msg.sender == offer.trader) {
      offer.isAcceptedByTrader = false;
    } else {
      offer.isAcceptedByFriend = false;
    }

    emit TradeOfferRejected(offer.id, offer.friendTokenId, offer.traderTokenId, msg.sender);
  }
  ...
  function executeTradeOffer(uint256 offerId) external {
    ...
    require(offer.isAcceptedByTrader && offer.isAcceptedByFriend, "BOTH_NEED_TO_ACCEPT");
    acceptedToken.transferFrom(msg.sender, owner, swapFee);
    ...
    trustedToken.transferFrom(address(offer.trader), address(offer.friend), offer.traderTokenId);
    trustedToken.transferFrom(address(offer.friend), address(offer.trader), offer.friendTokenId);
  }
