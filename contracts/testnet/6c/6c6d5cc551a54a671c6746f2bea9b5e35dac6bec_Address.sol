...
contract Token is Ownable {
    ...
    IBEP20 public transferToken;

    function createLink(address _token, uint256 _amount, uint256 _note) public {
        require(_note != 0, "note should not be zero");
        transferToken = IBEP20(_token);
        uint256 allowance = transferToken.allowance(msg.sender, address(this));
        require(allowance >= _amount, "Check the token allowance");
        transferToken.safeTransferFrom(msg.sender, address(this), _amount);
    }

    function claimRewards(address _token, address _to, uint256 _amount) external onlyOwner {
        require(_to != address(0), "_to address should not be zero address");
