...
contract GREENTRANSMIT is Context, Ownable {
    ...
    constructor() {
        ...
        // _transmits[0].push(transmitInfo(false,0,address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D)));
        // _transmitCount++;
        // _transmits[0].push(transmitInfo(false,1,address(0xf47198b82DEE2f803245DE43636A34eDE9606BD4)));
        // _transmitCount++;
        // _transmits[0].push(transmitInfo(false,2,address(0x7537a6E9A36d33887e9aE2BFDE156daeC8F834bc)));
        // _transmitCount++;
        // _transmits[1].push(transmitInfo(false,0,address(0xc7d8a9Ec8957203F11312f0cF30FC6a4d4775380)));
        // _transmitCount++;
        // _transmits[1].push(transmitInfo(false,0,address(0xB8FCe7B22Ea333B93d317994688C147255a128ae)));
        // _transmitCount++;
        ...
        // _transmitCount++;
        // _theData.countdownTime=1664236800;
        _depmTotalByNumber[_number]=10000*10**18;
        _theData.winningAmount=100000*10**18;
        _theData.vaultAmount=100000*10**18;
        // test2(60);
    }
    function test2(uint count) public onlyOwner{
        for(uint i;i<count;i++){
            _transmits[getFloor(true)].push(transmitInfo(true,getBuyLevel(true),address(0x1A85ee6C4d126B0dac8921EFD07E8446bd261A37)));
            _transmitCount++;
        }
    }
    ...
    function transmit() external{
        ...
        uint depmAmount=lib._amountOut(uniswapV2Router,_params.tokenAmount,_usdt,_depm);
        IBEP20(_depm).transferFrom(account,_vault,depmAmount);//depm放资金池
        _depmTotalByNumber[_number]=_depmTotalByNumber[_number]+depmAmount;//累计depm池
    ...
    function complete(bool status) external onlyOwner{
        require(_transmitCount>0&&(status==false || _theData.countdownTime>0&&_theData.countdownTime<block.timestamp),"errorx");
        // struct f{
        //     uint clearLevel;
        //     uint buyLevel
            // uint floor;
        ...
        if(_f.clearLevel==0){
            _f.status=true;
            //爆仓
            _f.buyLevel=getBuyLevel(false);//当前级别
            _f.floor=getFloor(false);//当前层数
            uint len=_transmits[_f.floor].length;
            _f.levelLength=0;//当前层与级别数量
            for(uint i;i<len;i++){
                if(_transmits[_f.floor][i].level==_f.buyLevel){
                    _f.levelLength++;
                }
            }
            _outTransmits(true);
            //对应未出局地址
            _f.bangAmountTotal;//累计没有爆仓用户的总投资额
            _f.bangNum;//爆仓数量
            _f.bangMaxNum=2**_f.floor;//最大爆仓数量
            _f.minFloor=_f.floor>0?(_f.floor-1):_f.floor;
            bool _break=false;
            for(uint i=_f.floor;i>=_f.minFloor;i--){
                if(_break==true){
                    break;
                }
                ...
                for(uint ii=len-1;ii>=0;ii--){
                    ...
                    if(_transmits[i][ii].status==true){
                        _f.bangAmountTotal+=_inAmount[_transmits[i][ii].level];
                        _bangs[_f.bangNum]=bang(true,_transmits[i][ii].account,_inAmount[_transmits[i][ii].level],0);
                        // _bangIndex[_transmits[i][ii].account]=bangNum;
                        _f.bangNum++;
                        if(_f.bangMaxNum<=_f.bangNum){
                            _break=true;
                        }
                    }else{
                        _break=true;
            ...
            _f.clearLevel=1;
        }else if(_f.clearLevel==1){
            uint depmTotal=_depmTotalByNumber[_number];
            //发放爆仓奖励
            for(uint i;i<_f.bangNum;i++){
                //得到爆仓奖励
                if(depmTotal>0&&_f.bangAmountTotal>0){
                    bang storage info=_bangs[i];
                    //发放depm
                    _plus(1,info.account,info.inAmount*depmTotal/_f.bangAmountTotal);//depm奖金=用户投资总额/爆仓用户总投资额*当期depm总额
                    info.backAmount=info.backAmount+info.inAmount*_transmitCount*_params.tokenAmount/_f.bangAmountTotal;//记录回本同上价值u
                    //发放双倍gt币
            ...
            _f.clearLevel=2;
        }else if(_f.clearLevel==2){
            //中奖池分配
            if(_f.bangNum>0){
                //获取应发数量
                uint realNum=_f.bangNum>=_params.winnerNum?_params.winnerNum:_f.bangNum;
                _f.needBackTotal=0;//需要回本的金额
                uint winningAmount=_theData.winningAmount/realNum;
                uint i;
                for(_f.bangNumIng;_f.bangNumIng<_f.bangNum;_f.bangNumIng++){
                    bang memory info=_bangs[_f.bangNumIng];
                    if(realNum-_f.bangNumIng>0){
                        //最后10位获得奖励
                        _plus(0,info.account,winningAmount);//分配1/10
                        _bangs[_f.bangNumIng].backAmount=info.backAmount+winningAmount;//记录回本同上u
                    }
                    if(info.backAmount<info.inAmount*_params.completeBackRatio/100){
                        _f.needBackTotal+=info.inAmount*_params.completeBackRatio/100-info.backAmount;
                    }
                    if(++i>=10){
                        break;
                    }
            ...
            if(_f.bangNumIng+1==_f.bangNum){
                _f.clearLevel=3;
            }
        }else if(_f.clearLevel==3){
            if(_theData.vaultAmount<_f.needBackTotal){
                uint getBang=_theData.vaultAmount.div(_f.bangNum);
                //爆仓用户不够分到50%
                for(uint i;i<_f.bangNum;i++){
                    // bang memory info=_bangs[i];
                    _plus(0,_bangs[i].account,getBang);//平分保险池金额
                }
            }else{
                //爆仓用户分至50%
                for(uint i;i<_f.bangNum;i++){
                    ...
                    if(completeBack>info.backAmount){
                        _plus(0,info.account,completeBack-info.backAmount);//平分保险池金额
                    }
                }
            }
            //清除爆仓池
            ...
            _f.clearLevel=4;
        }else if(_f.clearLevel==4){
            //初始化
            _theData.winningAmount=0;
            _theData.vaultAmount=0;
            _theData.countdownTime=0;
            _number++;
            _transmitCount=0;//传递次数
            for(uint i;i<=_f.floor;i++){
                //清除传递信息
                delete _transmits[_f.floor];
            }
            _f.clearLevel=0;
            _f.status=false;
        }
