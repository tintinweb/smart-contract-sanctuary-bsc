...
// File: contracts/11_marketplace_hash.sol
...
// For a full decentralized nft marketplace
    ...
    error PriceNotMet(uint256 tokenId, uint256 price, uint256 amount);
    error ListingAmountExceeded(uint256 tokenId, uint256 amount);
    error ItemNotForSale(uint256 tokenId);
    error NotListed(bytes32 tokenId);
    error AlreadyListed(bytes32 tokenId);
    error NoProceeds();
    error NotOwner();
    error NotListingOwner(bytes32 listingId);
    error NotApprovedForMarketplace();
    error PriceMustBeAboveZero();
    error AmountMustBeAboveZero();

contract MarketplaceHash is ReentrancyGuard {
    ...
    modifier isListed(bytes32 listingId) {
    ...
    modifier isNotListed(bytes32 listingId) {
        Listing memory listing = s_listings[listingId];
        if (listing.price > 0) {
            revert AlreadyListed(listingId);
        }
        _;
    }

    modifier isSeller(bytes32 listingId, address spender) {
        Listing memory listing = s_listings[listingId];
    ...
    isOwner(tokenId, amount, msg.sender)
    {
        ...
        bytes32 listingId = keccak256(abi.encodePacked(msg.sender, tokenId));
        // Do not allow the user to list the same token more than once. They should use the update function instead
        if (s_listings[listingId].price < 0){
            revert AlreadyListed(listingId);
        }
    ...
    external
    isSeller(listingId, msg.sender)
    isListed(listingId)
    {
        delete (s_listings[listingId]);
        emit ListingClosed(listingId, "canceled");
    }
     ...
     * @param tokenId Token ID of NFT
     */
    function buyItem(bytes32 listingId, uint256 amount)
    external
    payable
    isListed(listingId)
    nonReentrant
    {
        ...
        if (msg.value < listedItem.price * amount) {
            revert PriceNotMet(listedItem.tokenId, listedItem.price, amount);
        }
        ...
        if (amount == listedItem.amount) {
            delete (s_listings[listingId]);
            emit ListingClosed(listingId, "sold");
        } else {
        ...
        emit ItemBought(listingId, msg.sender, listedItem.tokenId, amount, listedItem.price);
    ...
    isOwner(s_listings[listingId].tokenId, newAmount, msg.sender)
    {
        ...
        emit ItemUpdated(listingId, msg.sender, s_listings[listingId].tokenId, newAmount, newPrice);
    }
