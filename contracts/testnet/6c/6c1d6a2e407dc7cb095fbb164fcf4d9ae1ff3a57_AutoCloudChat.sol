...
/**
 * Dual Rewards with Dividends
 * First Dual Reward with Auto-Staking, Auto-Compounding & Reflections
 *
 * Website: 
 * Whitepaper: 
 * Twitter:  
...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 GMT = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
...
contract AutoCloudChat is ERC20Detailed, Ownable {
    ...
    uint256 public ACCDividendFee = 20;
    uint256 public sellFee = 10;
    uint256 public autofirePitFee = 10;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(ACCDividendFee).add(
            autofirePitFee
        );
    ...
    address public ACCDividendReceiver;
    ...
    constructor() ERC20Detailed("AutoCloudChat", "ACC", uint8(DECIMALS)) Ownable() {
        ...
        ACCDividendReceiver = address(distributor);
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(ACCDividendFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(ACCDividendFee)
            ),
            gas: 30000
        }("");

        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(ACCDividendFee).div(treasuryFee.add(ACCDividendFee))
        } () {} catch {}        
    }
