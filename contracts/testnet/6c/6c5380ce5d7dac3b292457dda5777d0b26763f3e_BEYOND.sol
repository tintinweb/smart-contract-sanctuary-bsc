...
contract BEYOND is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        buyFee = buyMarketingFee.add(buyRepoFee).add(buyBurnFee);
    ...
    function setBuyFees(uint256 newMarketingFee, uint256 newRepoFee,uint256 newBurnFee) external onlyOwner() returns (bool) {
        buyMarketingFee = newMarketingFee;
        buyRepoFee = newRepoFee;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(pairs[sender] && !noFee[sender] && !noFee[recipient]) {
            ...
            uint256 feeEiToBnb = amount.mul(buyRepoFee).div(100);
            _balances[address(this)] += feeEiToBnb;
            uint256 bnbBefore = address(this).balance;
            swapTokensForEth(feeEiToBnb);
            uint256 eiToBnb = address(this).balance.sub(bnbBefore);  //用于回购

            repoAccumulator += eiToBnb;
