...
contract Cashflow_Money_Inu is IBEP20, Ownable {
    ...
    constructor() {
        ...
        isFeeExempt[marketFeeReceiver] = false;
    ...
    function getsellfee() public view returns (uint256) {
        uint256 CalcTime = (block.timestamp - Usertime[msg.sender].time) /
            duration;
        if (CalcTime >= 12) {
            CalcTime = 12;
        }
    ...
    function _transfer(
        ...
        if (
            ...
            if (recipient == pcsPair) {
                feeAmount =
                    (amount * totalSellFee + getsellfee()) /
                    (feeDivider);
                amountReceived = amount - (feeAmount);
                _takeFee(sender, feeAmount);
