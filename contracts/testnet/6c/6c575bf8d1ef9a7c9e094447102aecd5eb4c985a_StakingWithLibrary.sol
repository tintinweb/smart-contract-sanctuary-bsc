...
contract StakingWithLibrary is Initializable {
    ...
    function initialize(address _ownerWallet, address _owner, address _stakedToken, uint128 _stakeType, uint128 _stakePeriod, uint128 _depositFees, uint128 _withdrawlsFees, uint128 _rewardRate) external initializer  {
    ...
    function deposit(uint128 _amount, uint128 _stakeType) external {
        ...
        if(stakeTypes[_stakeType].depositFees !=0) {
            uint128 fees = _amount * stakeTypes[_stakeType].depositFees *10 / 1000;
    ...
    function withdraw(uint128 _amount, uint128 _stakeType, uint128 _id) external {
        ...
        if(usersStakes[msg.sender].eachUserStakes[_id].lastClaimTime != usersStakes[msg.sender].eachUserStakes[_id].claimTimeInitialized) {
            ...
            stakeTimeForReward = (_fullWithdrawlTime - usersStakes[msg.sender].eachUserStakes[_id].lastClaimTime);
        ...
        uint128 withdrawFees = stakeTypes[_stakeType].withdrawlFees;

        if (withdrawFees !=0) {
            uint128 fees = stakedValue * withdrawFees * 10 / 1000;
