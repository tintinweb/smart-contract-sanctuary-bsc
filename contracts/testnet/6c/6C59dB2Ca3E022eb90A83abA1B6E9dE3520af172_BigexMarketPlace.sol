...
contract BigexMarketPlace is Ownable {
	...
	function updatePool1155Address(address address_receive) public onlyOwner returns (bool) {
		POOL_1155 = address_receive;
		return true;
	}

	function updatePool721Address(address address_receive) public onlyOwner returns (bool) {
		POOL_721 = address_receive;
		return true;
	}

	function updatePoolFeeAddress(address address_receive) public onlyOwner returns (bool) {
		POOL_FEE = address_receive;
		return true;
	}

	function updateFeePercentMarket(uint newFee) public onlyOwner returns (bool) {
