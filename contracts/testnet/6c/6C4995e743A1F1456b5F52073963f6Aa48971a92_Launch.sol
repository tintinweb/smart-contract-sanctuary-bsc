...
contract Launch is Initializable{
    ...
    function createNewICO(DataParam calldata newICOData, uint[] calldata _time, uint[] calldata _percent) external returns(address){
        ...
        if(newICOData.Vesting){
            for(uint i=0; i< _time.length; i++){
                ICO(payable(newcontract)).setVesting(_time[i], _percent[i]);
            }
