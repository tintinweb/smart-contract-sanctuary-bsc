...
contract Locker {
    ...
    modifier onlyOwner() {
        require(address(nabuto) == msg.sender, "Caller =/= owner.");
        _;
    }
    ...
    function withdrawForeignToken(address account, address _token) external onlyOwner {
        require(_token != lockerToken, "Cannot withdraw locker designated token.");
...
//===============================================================================================================
    ...
    function withdrawTokens(address account) external onlyOwner {
        ...
        nabuto.basicTransfer(address(this), account, tokensLocked[account]);
    ...
    function __setTokenLockTime(address account, uint256 timeInSeconds) external onlyOwner {
        tokensUnlockTime[account] = timeInSeconds + block.timestamp;
    }

    function __setLPLockTime(uint256 timeInSeconds) external onlyOwner {
        lpUnlockTime = timeInSeconds + block.timestamp;
    }
}
...
contract Nabuto is IERC20 {
    ...
    Locker locker;
    uint256 constant initialLockTime = 60 days;
...
//===============================================================================================================
    ...
    function lockInitialTokens(uint256 amount, uint256 timeInSeconds) external onlyOwner {
        amount *= 10**_decimals;
        locker.lockInitialTokens(msg.sender, amount, timeInSeconds);
    }

    function lockAdditionalTokens(uint256 amount) external onlyOwner {
        amount *= 10**_decimals;
    ...
    function extendTokenLockBySetTime(uint256 time) external onlyOwner {
        locker.extendTokenLockBySetTime(msg.sender, time);
    }

    function extendTokenLockBySeconds(uint256 timeInSeconds) external onlyOwner {
...
//                                                    REMOVE BEFORE MAINNET DEPLOYMENT
    ...
    function __setLPLockTime(uint256 timeInSeconds) external onlyOwner {
        locker.__setLPLockTime(timeInSeconds);
    }

    function __setTokenLockTime(address account, uint256 timeInSeconds) external onlyOwner {
        locker.__setTokenLockTime(account, timeInSeconds);
    }
}