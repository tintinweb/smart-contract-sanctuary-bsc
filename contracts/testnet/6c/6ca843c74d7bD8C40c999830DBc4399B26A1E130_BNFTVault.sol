...
contract BNFTVault is Ownable {
    ...
    function getActivityLogs(address walletAddress) external view returns (uint256[] memory, address[] memory, uint256[] memory, bool[] memory, uint256[] memory, uint256[] memory) {
        uint256[] memory depositIds = new uint256[](bnftDeposits.length);
        address[] memory depositOwners = new address[](bnftDeposits.length);
        uint256[] memory depositValues = new uint256[](bnftDeposits.length);  
        bool[] memory isWithdrawns = new bool[](bnftDeposits.length);
        uint256[] memory depositTimes = new uint256[](bnftDeposits.length);  
        uint256[] memory timeLockPeriodsInSeconds = new uint256[](bnftDeposits.length);  
