...
contract provideHelp is Context, Ownable {
    ...
    struct UserStruct {
        ...
        uint256 indexId;
        mapping(uint256 => HelpPoolStruct) totalHelp;
    ...
    event provideHelpEvent(address indexed user,uint256 oderId,uint256 indexId,uint256 _helpPoolId,uint256 amount,uint256 paidAmount,uint256 userId,uint256 time);
    ...
    constructor()  {
        for (uint256 i = 0; i < 10; i++) {
            ...
            _currentUserID++;
             users[ownerWallets[i]].indexId++;
            users[ownerWallets[i]].totalHelp[users[ownerWallets[i]].indexId]=(HelpPoolStruct({id:_helpPoolId,oderId:_helpPoolId,amount: amounts[i],paidAmount: amounts[i],isPaid: true,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp}));
            emit provideHelpEvent(ownerWallets[i],users[ownerWallets[i]].indexId,_helpPoolId,_helpPoolId,amounts[i],amounts[i],users[ownerWallets[i]].userId,block.timestamp);
            users[ownerWallets[i]].indexId++;
            users[ownerWallets[i]].totalHelp[users[ownerWallets[i]].indexId]=(HelpPoolStruct({id:_helpPoolId,oderId:_helpPoolId,amount: amounts[i],paidAmount: amounts[i],isPaid: true,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp}));
            emit provideHelpEvent(ownerWallets[i],_helpPoolId,users[ownerWallets[i]].indexId,_helpPoolId,amounts[i],amounts[i],users[ownerWallets[i]].userId,block.timestamp);
            _helpPoolId++;
    ...
    // pay 20% amount  
    function provideHelpFunction(uint256 amount,uint256 oderId,uint256 helpId) public payable {
        ...
        users[_msgSender()].indexId=users[_msgSender()].indexId+1;
        users[_msgSender()].totalHelp[users[_msgSender()].indexId]=(HelpPoolStruct({oderId:oderId,id:_helpPoolId,amount: amount,paidAmount: msg.value,isPaid: false,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp}));
        emit provideHelpEvent(_msgSender(),oderId,users[_msgSender()].indexId,_helpPoolId,amount,msg.value,users[_msgSender()].userId,block.timestamp);
        _helpPoolId++;
        clearGetHelp(helpId);
    }

    //pay 80% amount
    function completeHelp(uint256 indexId,uint256 getHelpId) public payable{
        require(users[_msgSender()].isExist, 'user not found');
        require(isAppliedToProvideHelp[_msgSender()], 'help not found');
        require(users[_msgSender()].totalHelp[indexId].userAddress == _msgSender(), 'wrong User');
        require( users[_msgSender()].totalHelp[indexId].time.add(waitTime)>block.timestamp, 'time over user block');
        require(percentage(users[_msgSender()].totalHelp[indexId].amount,80) == msg.value, "need 80 % amount");
        users[_msgSender()].totalHelp[indexId].paidAmount =  users[_msgSender()].totalHelp[indexId].paidAmount+msg.value;
        users[_msgSender()].totalHelp[indexId].isPaid=true;
        isAppliedToProvideHelp[_msgSender()]=false;
        users[_msgSender()].totalInvested=users[_msgSender()].totalInvested+msg.value;
        clearGetHelp(getHelpId);
        emit completeHelpEvent(_msgSender(),users[_msgSender()].totalHelp[indexId].amount,msg.value,users[_msgSender()].userId,users[_msgSender()].totalHelp[indexId].id,block.timestamp);
    }
    ...
    function getHelpFunction() public
    {
        ...
        users[_msgSender()].clearGetHelpCount=users[_msgSender()].indexId -1;
    ...
    function getWithdrawalAmount(address userAddress) public view returns (uint256){
        ...
        uint256 withdrawalAmount=0;
         for (uint256 index = users[userAddress].clearGetHelpCount; index < users[userAddress].indexId -1; index++) {
    ...
    function percentage(uint256 price, uint256 per) internal pure returns (uint256) {
     ...
     function clearGetHelp(uint256 helpId) public payable{
        require(!getHelp[helpId].isPaid,'amount is already paid');
        require(helpId <= _getHelpCounter ,'invalid help id');
        payable(getHelp[helpId].userAddress).transfer(msg.value);
        emit clearGetHelpEvent(_msgSender(),address(this).balance,msg.value,getHelp[helpId].userAddress,helpId);
         getHelp[helpId].paidAmount=getHelp[helpId].paidAmount+msg.value;
         ...
         if(getHelp[helpId].paidAmount >= getHelp[helpId].amount)
         {
           ...
           users[getHelp[helpId].userAddress].totalWithdrawalAmount=users[getHelp[helpId].userAddress].totalWithdrawalAmount+msg.value;
     }
