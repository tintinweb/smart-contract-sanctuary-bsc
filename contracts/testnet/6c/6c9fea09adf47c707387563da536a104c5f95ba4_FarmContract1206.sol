...
contract FarmContract1206 is Initializable, OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getInviteeTotalMintedAmount(uint256 _pid, address _user) public view validatePid(_pid) returns(uint256 totalMintedAmount){
        ...
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 mintedAmount = pendingMiningReward(_pid, myInviteeAddress[i]).add(mintAmount[_pid][_user]);
    ...
    function getAllPendingReferralReward(address _user) public view returns(uint256 totalReferralBonus){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 referralBonus = getInviteeTotalMintedAmount(i,_user).mul(getRefferalRewardPercent(i, _user)).div(rateDenominator);
    ...
    function getAllPendingNodeReward(address _user) public view returns(uint256 totalNodeBonus){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 nodeBonus = getInviteeTotalMintedAmount(i,_user).mul(getNodeRewardPercent(i, _user)).div(rateDenominator);
    ...
    function claimAllReferralReward() public {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 refferalReward = getInviteeTotalMintedAmount(i, _msgSender());
    ...
    function claimAllNodeReward() public {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 nodeReward = getInviteeTotalMintedAmount(i, _msgSender());
