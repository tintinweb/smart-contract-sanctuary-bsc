...
contract BasicNodeStakeContract is Ownable {
    ...
    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 totalReferralBonus){
        ...
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 referralBonus = pendingReward(_pid,myInviteeAddress[i]).mul(bonusPerReferral).div(referralRateDenominator).add(userInfo[_pid][msg.sender].totalMining);
            totalReferralBonus = totalReferralBonus.add(referralBonus);
        }
        return totalReferralBonus.sub(userInfo[_pid][msg.sender].receivedReferralReward);
    }
