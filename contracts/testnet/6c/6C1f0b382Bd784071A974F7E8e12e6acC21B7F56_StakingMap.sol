...
contract StakingMap is Ownable, Pausable {
    ...
    function getStake(address _userAddress, uint256 _index) external view returns (Stake memory _stake, uint256 _id){
        _stake = STAKES[stakesOf[_userAddress][_index]];
        _id = stakesOf[_userAddress][_index];
    }

    function getStakes(address _userAddress) external view returns (Stake[] memory _stakes, uint256[] memory _indexes){
    ...
    function getActiveStakes(address _userAddress) external view returns (Stake[] memory _stakes, uint256[] memory _indexes){
        uint256[] memory _stakeIds = stakesOf[_userAddress];
    ...
    function _createStakeEntry(uint256 _stakesLen, uint256 _stakesAmount, uint32 _age) private {
    ...
    function reduceStake(uint256 _stakeId, uint256 _amountToReduce) public whenNotPaused {
        ...
        if (_newStakeAmount > 0) {
            uint32 _age = STAKES[_stakeId].endAt - STAKES[_stakeId].createdAt + STAKES[_stakeId].age; // Bugfix was here
