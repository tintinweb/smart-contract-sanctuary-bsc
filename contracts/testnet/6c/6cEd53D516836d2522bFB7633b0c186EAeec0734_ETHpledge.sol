...
contract ETHpledge is IERC20,Ownable {
    ...
    uint256 public td_nft_yq=100;//团队发nft要求
    uint256 public shanchu_jicha1=100;//团队发nft要求
    uint256 public shanchu_jicha2=500;
    uint256 public shanchu_jicha3=1000;
    uint256 public shanchu_jicha4=2000;
    uint256 public shanchu_jicha5=5000;

    uint256 public zhiya_di=100;
    uint256 public zhiya_gao=1000;
    //uint256 public kuangji_chan7=9;
    //uint256 public kuangji_day7=45;
...
function wakuangteam(uint256 ltj) private{
        ...
        for (int256 i = 0; i < 99; i++) {
         ...
         if(sharenumber[cur]>=10 && wakuangteamperformance[cur]>=td_nft_yq&&baijin[cur]==0&&baijinzongshu>0){
    ...
    function teamfenhong(uint256 ltj) private{
        ...
        for (int256 i = 0; i < 99; i++) {
         ...
         if(level[cur]<2 && teamperformance[cur]>=shanchu_jicha1*10**18){level[cur]=2;l2[inviter[cur]]+=1;}
         if(level[cur]<3 && teamperformance[cur]>=shanchu_jicha2*10**18){level[cur]=3;l3[inviter[cur]]+=1;}
         if(level[cur]<4 && teamperformance[cur]>=shanchu_jicha3*10**18){level[cur]=4;l4[inviter[cur]]+=1;}
         if(level[cur]<5 && teamperformance[cur]>=shanchu_jicha4*10**18){level[cur]=5;l5[inviter[cur]]+=1;}
         if(level[cur]<6 && teamperformance[cur]>=shanchu_jicha5*10**18){level[cur]=6;l6[inviter[cur]]+=1;}
 ...
 function importSeedFromThird(uint256 seed) public view returns (uint) {
    ...
    function  ETHreceive()  external returns (bool) {
        ...
        bool y1= otheryuejiazhi>= zhiya_di*10**18;
        require(y1,"balance is low.");
        bool y2=otheryuejiazhi >= zhiya_gao*10**18;
      
        if(y2){
            jisuanedu=zhiya_gao*10**18;
        }
        uint shouyi=10;
    ...
    function setmstaddress(IERC20 _mst) public onlyOwner {
        mst = _mst;
    }
