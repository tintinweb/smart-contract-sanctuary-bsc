...
contract NestBlindBox is NestFrequentlyUsed, SimpleERC721 {
    ...
    function totalSupply(uint level) external view returns (uint) {
        return (_counter >> ((3 - level) << 6)) & 0xFFFFFFFFFFFFFFFF;
    }
    ...
    function mint() external {
        ...
        require((_counter >> 192) < 199, "NNFT:mint over");
    ...
    function claim(uint index) external {
        ...
        if (hashValue > 0) {
            ...
            // Each 64-bits is a number for nft, the highest 64 bits are used to record the total number, which is
