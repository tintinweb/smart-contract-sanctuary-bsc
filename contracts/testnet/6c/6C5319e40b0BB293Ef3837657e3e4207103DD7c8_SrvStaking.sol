...
contract SrvStaking is AccessControl, ReentrancyGuard {
  ...
  function getBusdAmountToClaim(address _account) public view returns (uint256) {
    ...
    if (lastClaimTime[_account] == lastStakeTime[_account] && rewardsPerWeek[lastStakeTime[_account]] > 0) {
      rewards += activeStakedSrv[_account] / rewardsPerWeek[lastStakeTime[_account]];
    }
    ...
    for (uint256 i = claimStart; i < currentWeek; i++) {
      if (rewardsPerWeek[i] > 0) rewards += totalUserSrvStaked / rewardsPerWeek[i];
    }
  ...
  function _stake(address _account, uint256 _srvAmount) internal {
    ...
    if (busdAmountToClaim > 0) _claim(_account, busdAmountToClaim);
  ...
  function _withdraw(address _account) internal {
    ...
    srv.transferFrom(address(this), _account, srvAmountToWithdraw);
  }

  // ...
  function _claim(address _account, uint256 _busdAmount) internal {
    // If user already claimed this week: return
    if (lastClaimTime[_account] == currentWeek) return;

    // Update last claim time
    lastClaimTime[_account] = currentWeek;

    // Move inactive balance to active balance
    ...
    if (_busdAmount > 0) busd.transferFrom(address(this), _account, _busdAmount);
  }
  ...
  function updateWeekAdmin(uint256 _week, uint256 _busdAmount) external onlyRole(ADMIN) {
    rewardsPerWeek[_week] = _busdAmount;
  }
}