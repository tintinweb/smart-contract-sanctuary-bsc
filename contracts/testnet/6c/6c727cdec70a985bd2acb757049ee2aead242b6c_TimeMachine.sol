...
contract TimeMachine is Context, ERC20("Water Test", "WT3") {
    ...
    uint256 public constant BLOCK_START_PRODUCTION_TIMA = 7_200*10**3;
    uint256 public constant BLOCK_PRODUCTION_SECONDS = 3_600 * 24;
     ...
     */
    function _calculateUserMined(uint256 userTcp,uint256 userInBlock,uint256 userOutBlock) private view returns (uint256) {
        uint256 userMined;
    ...
    function _calculateOneBlockMined(uint256 blockNumber) private pure returns (uint256) {
        ...
        if(blockNumber>=BLOCK_IN_FOUR_YEAR.mul(9) && blockNumber<=BLOCK_IN_FOUR_YEAR.mul(9).add(1213)) return BLOCK_START_PRODUCTION_TIMA.mul(1e18).div(512);
        return 0;
    }
     ...
     * @dev calculate Mined
     */
    function _calculateMined(uint256 currentTime) private view returns (uint256) {
        uint256 lostTime=currentTime - genesisTs;
        uint256 decayNum=lostTime.div(DECAY_YEARS.mul(DAYS_IN_YEAR).mul(SECONDS_IN_DAY));
        uint256 totalMined;
        if(decayNum>=9) decayNum=9;
        for(uint256 i=0;i<decayNum;i++){
            uint256 decayNum2=2**i;
            totalMined+=BLOCK_IN_FOUR_YEAR.mul(1e18).mul(BLOCK_START_PRODUCTION_TIMA).div(decayNum2);
        }
        lostTime-=decayNum.mul(BLOCK_IN_FOUR_YEAR).mul(BLOCK_PRODUCTION_SECONDS);
        uint256 blockNum=lostTime.div(BLOCK_PRODUCTION_SECONDS);
        if(decayNum==9){
            if(blockNum<=1213){
                totalMined+=blockNum.mul(1e18).mul(BLOCK_START_PRODUCTION_TIMA).div(2**decayNum);
            }
            else{
                totalMined+=uint256(1213).mul(1e18).mul(BLOCK_START_PRODUCTION_TIMA).div(2**decayNum);
            }
        }
        else{
            totalMined+=blockNum.mul(1e18).mul(BLOCK_START_PRODUCTION_TIMA).div(2**decayNum);
     ...
     */
    function getGrossReward(uint256 tcp,uint256 inBlock,uint256 outBlock) external view returns (uint256) {
