...
contract Farming is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    constructor() initializer {}
     ...
     function initialize() public initializer {
      ...
      function calculateReward(uint256 _pid, address userAddress) internal view returns(uint256, uint256) {
         ...
         if(user.amount > 0){
            if(poolInfo[_pid].endTime > 0){
               if(block.timestamp > poolInfo[_pid].endTime){
                  timeDifference = poolInfo[_pid].startTime - user.depositStartTime;
               }else{
                  timeDifference = block.timestamp - user.depositStartTime;
               }
            }else{
               timeDifference = block.timestamp - user.depositStartTime;
            }timeFactor = timeDifference / poolInfo[_pid].rewardInterval;
            reward = (user.amount * poolInfo[_pid].rewardPercentage * timeFactor) / (10 ** 2);
         } 
         return (reward, timeFactor);     
      }
      ...
      function claimMyReward(uint256 _pid) external {
         ...
         (uint256 reward, uint256 timeFactor) = calculateReward(_pid, msg.sender);
         require(reward > 0, "Reward is 0");
         require(timeFactor > 0, "claim reward only after the interval");
