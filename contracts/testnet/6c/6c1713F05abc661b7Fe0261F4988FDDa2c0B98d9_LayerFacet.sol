...
contract LayerFacet is Helper {
    event SyncDefi(uint256 reserve, uint256 totalDLP);
    event Invest(address indexed account, uint256 getLP, uint256 mintDLP);
    ...
    function invest(uint8 slippage) external payable nonReentrant returns (uint256 getLP, uint256 mintDLP) {
        ...
        getLP = countLP();
        ITopcornProtocol(s.c.topcronProtocol).addAndDepositLP{value: msg.value}(0, countTokens[1], 0, ITopcornProtocol.AddLiquidity(countTokens[1], chekCorn, chekBNB)); // call TopCorn Protocol (add liquidity)
        getLP = countLP() - getLP; // Total invested of tokens LP
        mintDLP = calcDLP(getLP, s.totalSuppluyDLP, s.reserveLP); // calc amount DLP for mint
        dlp().mint(msg.sender, mintDLP);
        s.totalSuppluyDLP = s.totalSuppluyDLP + mintDLP; // update reserves
        s.reserveLP = s.reserveLP + getLP;
        emit Invest(msg.sender, getLP, mintDLP);
        emit SyncDefi(s.reserveLP, s.totalSuppluyDLP);
    }
    ...
    function withdraw(uint32[] calldata crates, uint256[] calldata amounts) external payable nonReentrant {
        LibDiamond.enforceIsContractOwner(); // Only the owner
        (uint256 lpRemoved, uint256 balance) = checkLiq(crates.length, amounts); // calc amount DLP for burn
        dlp().burnFrom(msg.sender, balance); 
        s.totalSuppluyDLP = s.totalSuppluyDLP - lpRemoved; // update reserves
        s.reserveLP = s.reserveLP - balance;
        ITopcornProtocol(s.c.topcronProtocol).withdrawLP(crates, amounts); // // call TopCorn Protocol (withdraw LP)
        emit SyncDefi(s.reserveLP, s.totalSuppluyDLP);
    }
    ...
    function approveWithdraw(uint32[] calldata crates) external payable nonReentrant returns (uint256 getLP) {
        LibDiamond.enforceIsContractOwner(); // Only the owner
        getLP = IERC20(s.c.pair).balanceOf(address(this));
        ITopcornProtocol(s.c.topcronProtocol).claimLP(crates); // // call TopCorn Protocol (claim LP)
    ...
    function claimCorn(uint32[] calldata crates, uint256[] calldata amounts) external payable nonReentrant {
        LibDiamond.enforceIsContractOwner(); // Only the owner
        ITopcornProtocol(s.c.topcronProtocol).withdrawTopcorns(crates, amounts); // call TopCorn Protocol (withdraw corn)
    }
    ...
    /// @param slippage 0-100 percent.
    /// @return getLP Amount of invested tokens LP.
    function updateReserve(uint32[] calldata crates, uint8 slippage) external payable nonReentrant returns (uint256 getLP) {
        LibDiamond.enforceIsContractOwner(); // Only the ownernonReentrant
        uint256 getCORN = IERC20(s.c.topcorn).balanceOf(address(this));
        ITopcornProtocol(s.c.topcronProtocol).claimTopcorns(crates); // call TopCorn Protocol (claim LP).
        ...
        getLP = countLP();
        ITopcornProtocol(s.c.topcronProtocol).addAndDepositLP(0, 0, countTokens[1], ITopcornProtocol.AddLiquidity(getCORN - countTokens[0], chekCORN, checkBNB)); // call TopCorn Protocol (add liquidity)
        getLP = countLP() - getLP; // Total invested of tokens LP
    ...
    function convertCorn(
        ...
        uint8 slippage
    ) external payable nonReentrant returns (uint256 getLP) {
        LibDiamond.enforceIsContractOwner(); // Only the owner
        uint256 countCorn = 0;
        ...
        getLP = countLP();
        ITopcornProtocol(s.c.topcronProtocol).convertDepositedTopcorns(countCorn, (minLP * (100 - slippage)) / 100, crates, amounts); // call TopCorn Protocol (convert CORN)
        getLP = countLP() - getLP; // Total invested of tokens LP
        s.reserveLP = s.reserveLP + getLP; // update reserves
        emit SyncDefi(s.reserveLP, s.totalSuppluyDLP);
...
contract Helper is ReentrancyGuard {
    ...
    function countLP() internal view returns (uint256 lpBegin) {
        (lpBegin, ) = ITopcornProtocol(s.c.topcronProtocol).lpDeposit(address(this), ITopcornProtocol(s.c.topcronProtocol).season());
    }
    ...
    function checkLiq(uint256 length, uint256[] calldata amounts) internal view returns (uint256 lpRemoved, uint256 balance) {
        for (uint256 i; i < length; i++) lpRemoved = lpRemoved + amounts[i];
        balance = calcBalance(lpRemoved, s.totalSuppluyDLP, s.reserveLP);
        require(dlp().balanceOf(msg.sender) >= balance, "Insufficient DLP balance");
    }
    ...
    function getInfo() public view returns (uint256[] memory amounts) {
        amounts = new uint256[](20);
        amounts[0] = s.reserveLP;
        amounts[1] = s.totalSuppluyDLP;
    }
