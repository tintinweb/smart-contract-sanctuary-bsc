...
contract BLXNBusdRocket is Context, Ownable, ReentrancyGuard {
    ...
    function MakeDeposit(uint256 amtx, address reffy) public noReentrant {
        ...
        uint256 firstlaunchTime = 1662307200; //changeTime LM
        uint256 secondlaunchTime = 1662393600; //changeTime LM
    ...
    function MakeClaim() public noReentrant{
        ...
        require (rn - user.lastClaim >= 86400, "You can only claim once per day."); //change to 86400 LM
    ...
    function MakeWithdraw() public noReentrant{
        ...
        require(rn - user.lastWithdraw > 604800, "You can only withdraw once per week."); //change to 604800 LM
        ...
        if (transferAmt + user.totalWithdrawn > user.maxPayout){
            ...
            // You will only receive the remaining amount between what you've withdrawn and your max.
    ...
    function whiteListSignup() public payable noReentrant{
        ...
        require (nowTime < 1662307200, "VIP reservation period has ended."); //change time to whitelist launch time LM
