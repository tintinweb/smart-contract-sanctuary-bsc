...
contract Staking {
    ...
    constructor(IERC20 _erc20_contract_address) {
        ...
        rewards.push(Reward(0, "bronze", 2, 7776000, true));
        rewards.push(Reward(1, "silver", 6, 15552000, true));
        rewards.push(Reward(2, "gold", 16, 31104000, true));
        rewardsCounter = 2;
    }
    ...
    function stakeTokens(
        ...
        StakingInfo memory temp = StakingInfo(
            userStakedToken[msg.sender][rewardId].length,
            amount,
            (block.timestamp + rewards[rewardId].timePeriod)
        );
    ...
    function unstakeTokens(
        ...
        require(
            userStakedToken[msg.sender][rewardId][stakeId].amount >= amount,
        ...
        require(
            rewards[rewardId].allowed,
            "This rewards has been suspended, it is only possible to withdraw"
        );
        if (
            block.timestamp >=
            userStakedToken[msg.sender][rewardId][stakeId].stakingDate
        ) {
            uint256 initialAmount = amount;
            amount = amount.add(
                ((amount.mul(rewards[rewardId].percentage)).div(100))
            );
            _unstakeTokens(msg.sender, amount);
            userStakedToken[msg.sender][rewardId][stakeId]
                .amount = userStakedToken[msg.sender][rewardId][stakeId]
                .amount
                .sub(initialAmount);
        } else {
            _unstakeTokens(msg.sender, amount);
    ...
    function getUserStakingInfo(address user, uint256 rewardId)
        public
        view
