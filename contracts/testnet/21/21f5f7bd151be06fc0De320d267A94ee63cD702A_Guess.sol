...
contract Guess {
    ...
    mapping(bytes32 => mapping(uint256 => bool)) isMatchIncludeTokenId;
    ...
    function setUserGuessData(
        ...
        //-----------------------------------------------------------
        if (!isMatchIncludeTokenId[matchId][tokenId]) {
    ...
    address public guessTokenAddress;
    address public openTokenAddress;

    function setGuessToken(
        address _guessTokenAddress,
        address _openTokenAddress
    ) external onlyOwner {
        guessTokenAddress = _guessTokenAddress;
        openTokenAddress = _openTokenAddress;
        guessToken = IERC20(_guessTokenAddress);
        openToken = IERC20(_openTokenAddress);
    }
    ...
    function setAdmin(uint256 index, address _address) external onlyOwner {
        require(index < 6, "index range out");
        require(_address != address(0), "address invaild");
        if (admins[index] != address(0)) {
            isAdmin[_address] = false;
        }
        admins[index] = _address;
    ...
    function mintGuessToken(uint256 amount) private {
        (bool success, ) = guessTokenAddress.call(
