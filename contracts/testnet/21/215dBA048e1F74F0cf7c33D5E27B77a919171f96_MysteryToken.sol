...
contract MysteryToken is Ownable {
  ...
  uint256 private _numTokensSellToAddToLiquidity = 1000000 * 10**18; //0.01% del total supply
  ...
  */
  ...
  function getMaxWalletPercent() external view returns (uint256) {
  ...
  //   || _isTxLimitExempt[recipient], "TX Limit Exceeded");
  // }
...
//uint256 rFee, uint256 rBurn, uint256 tFee, uint256 tBurn
  ...
  // function excludeFromTxLimitExempt(address account) public onlyOwner {
  //   require(_isTxLimitExempt[account], "Account is already excluded");
  //   _isTxLimitExempt[account] = false;
  // }

  // function includeInTxLimitExempt(address account) public onlyOwner {
  //   require(!_isTxLimitExempt[account], "Accout is alredy included");
