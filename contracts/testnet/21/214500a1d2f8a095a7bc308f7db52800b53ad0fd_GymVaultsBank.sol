    ...
    RewardRateConfigurable
{
    ...
    function setAlpacaRewardsPoolAddress(address _alpacaRewardsAddress) external onlyOwner {
        alpacaRewardsPoolAddress = _alpacaRewardsAddress;

        emit SetAlpacaRewardsPoolAddress(_alpacaRewardsAddress);
    }
    ...
    function seedUsers(uint256 _pid, address[] calldata _users) external onlyOwner {
        for (uint256 i; i < _users.length; i++) {
            ...
            IAlpacaRewards(alpacaRewardsPoolAddress).setUserShares(_pid, _users[i], user.shares);
        }
    }
    ...
    function _deposit(uint256 _pid, uint256 _wantAmt) private {
        ...
        if (_wantAmt > 0) {
            ...
            IAlpacaRewards(alpacaRewardsPoolAddress).updateUser(_pid, userInvestment[_pid][msg.sender], msg.sender);
