...
contract OriginsNFT is ERC721, ERC721Enumerable, Ownable {
  ...
  /// @dev Emitted when the launchpad address is updated
  ...
  uint256 constant public OWNER_MINTABLE_MAX_SUPPLY = 320;
  uint256 constant public LAUNCHPAD_MAX_SUPPLY = 700;
  //uint256 constant public MAX_SUPPLY = 1020;  // must be OWNER_MINTABLE_MAX_SUPPLY + LAUNCHPAD_MAX_SUPPLY
  uint256 public ownerMintableNextTokenId = 1; // must start at 1
  uint256 public launchpadNextTokenId = 321; // must start at OWNER_MINTABLE_MAX_SUPPLY + 1
  ...
  function getMaxLaunchpadSupply() public pure returns (uint256) {
  ...
  /// @dev Mint tokens by the launchpad contract
  function mintTo(
    address to,
    uint256 size
  )
  ...
  onlyLaunchpad
  {
    ...
    require(launchpadSupply + size <= LAUNCHPAD_MAX_SUPPLY, "max supply exceeded");

    for (uint256 i = 0; i < size; i++) {
      _mint(to, launchpadNextTokenId);
      launchpadNextTokenId++;
      launchpadSupply++;
    }
  }

  /// @dev Mint tokens by the owner of the contract
  ...
  onlyOwner
  {
    require(to != address(0), "can't mint to empty address");
    require(size > 0, "size must be greater than zero");
    require(ownerMintableNextTokenId + size - 1 <= OWNER_MINTABLE_MAX_SUPPLY, "max supply exceeded");

    for (uint256 i = 0; i < size; i++) {
      _mint(to, ownerMintableNextTokenId);
      ownerMintableNextTokenId++;
    }
  }
  ...
  function _mint(address to, uint256 tokenId) internal override {
    require(tokenId != 0, "Cannot mint with zero tokenId");
