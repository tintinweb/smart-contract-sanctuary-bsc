...
contract Calamus is Initializable, OwnableUpgradeable, ICalamus, IPlatformFee, ReentrancyGuardUpgradeable, CarefulMath, PausableUpgradeable {
    ...
    function _validateAllStreams(
        ...
        return (totalReleaseAmountIncludeFee, totalAmount);
    }


    function _createBatchStreams(Types.StreamGeneral memory generalInfo, Types.Recipient[] memory recipients, address realRender, uint256 extraFee) internal {
        ...
        uint totalReleaseAmountIncludeFee = 0;
        uint totalReleaseAmount = 0;
        (totalReleaseAmountIncludeFee, totalReleaseAmount) = _validateAllStreams(generalInfo, recipients, correctTokenAddress, vars, sender);

        if (correctTokenAddress == address(0x0)) {
            require(userTokenBalance[sender][address(0x0)] - lockedUserTokenBalance[sender][address(0x0)] - totalReleaseAmountIncludeFee > extraFee, "!EnoughNative");
        } else {
            require(userTokenBalance[sender][address(0x0)] - lockedUserTokenBalance[sender][address(0x0)] > extraFee, "!EnoughNative");
        }
    ...
    function batchStreams(Types.StreamGeneral memory generalInfo, Types.Recipient[] memory recipients, address realSender, uint256 extraFee) external whenNotPaused nonReentrant {
        _createBatchStreams(generalInfo, recipients, realSender, extraFee);
    }
...
interface ICalamus {
    ...
    */
    ...
    function batchStreams(Types.StreamGeneral memory generalInfo, Types.Recipient[] memory recipients, address realSender, uint extraFee) external;
