...
contract TwittBlue is Context, IBEP20, Ownable {
    ...
    uint256 public _liquidityFee4Sellers = 3;
    ...
    uint256 public numTokensSellToAddToLiquidity = 5000000000 * 10**uint256(_decimals);  // contract balance to trigger swap & liquify + wallet transfer.
    ...
    //when buy
     if ((from != owner() && to != owner()) && !_isExcludedFromMaxWallet[to]) {
      ...
      //when sell
        else if ((from != owner() && to != owner()) && !_isExcludedMaxTransactionAmount[to] ) {
        require(amount <= maxTransactionAmount, "Sell transfer amount exceeds the maxTransactionAmount.");
          ...
          if(limitsInEffect){
            if (
                ...
                if (gasLimitActive && uniswapV2Pair==from) {
