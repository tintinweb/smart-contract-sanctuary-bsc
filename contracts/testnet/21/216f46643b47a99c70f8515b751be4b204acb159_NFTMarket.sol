...
contract NFTMarket is ERC721Holder, Ownable, Pausable {
    ...
    mapping(address => mapping(uint256 => address payable)) private _tokenCreatorsData;
    ...
    event SetCreatorFeePercent(address indexed seller, uint256 oldFeePercent, uint256 newFeePercent);
...
//    event CancelBidToken(address indexed bidder, uint256 indexed tokenId);
    ...
    constructor(
        ...
        init(_feeAddr, _feePercent, _creatorFeePercent, _coinsArr, _nftsArr);
    }
    ...
    function buyTokenTo(address _nftAddress, uint256 _tokenId, address _to) payable public whenNotPaused {
        ...
        uint256 creatorFeeAmount = price.mul(creatorFeePercent).div(100);
        ...
        if (_asksMapDataCoin[_nftAddress].get(_tokenId) == address(0)) {
            ...
            if(feeAmount != 0){
                ...
                _tokenCreatorsData[_nftAddress][_tokenId].transfer(creatorFeeAmount);
                // 返还多余的费用
                _msgSender().transfer(msg.value.sub(price));
            }
            _tokenSellersData[_nftAddress][_tokenId].transfer(price.sub(feeAmount).sub(creatorFeeAmount));
        }else{
            ...
            if (creatorFeeAmount != 0) {
                quoteErc20.safeTransferFrom(_msgSender(), _tokenCreatorsData[_nftAddress][_tokenId], creatorFeeAmount);
            }
            quoteErc20.safeTransferFrom(_msgSender(), _tokenSellersData[_nftAddress][_tokenId], price.sub(feeAmount).sub(creatorFeeAmount));
        }
    ...
    function readyToSellTokenTo(
        ...
        _tokenCreatorsData[_nftAddress][_tokenId] = payable(ICreatorOf(_nftAddress).creatorOf(_tokenId));
    ...
    function setCreatorFeePercent(uint256 _creatorFeePercent) public onlyOwner {
        require(creatorFeePercent != _creatorFeePercent, 'Not need update');
        emit SetCreatorFeePercent(_msgSender(), creatorFeePercent, _creatorFeePercent);
        creatorFeePercent = _creatorFeePercent;
    }
