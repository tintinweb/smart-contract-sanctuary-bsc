...
contract TestProtoDiceErc1155 is ERC1155, Ownable {
    ...
    uint256 public constant _FusionAssetDefaultCharge = 10;

    //ModifierSection
    //Modifier -> Is limited dice token holder
    modifier mintNativeCompliance(uint256 _mintAmount) {
        require(msg.value <= msg.sender.balance, "Insufficient balance.");
        require(_mintAmount == 10, "10 Assets per mint required.");
        require(msg.value >= _FeePerAsset * _mintAmount, "Insufficient balance.");
        _;
    }

    modifier mintCashCompliance(uint256 _mintAmount) {
        require(msg.value <= msg.sender.balance, "Insufficient balance.");
        require(_mintAmount == 10, "10 Assets per mint required.");
        require(balanceOf(msg.sender, _Cash) >= (_CashPerAssetToBurn + _CashPerAssetFee) * _mintAmount, "Insufficient balance.");
        _;
    }
    ...
    modifier isFragment(uint256 _id) {
        ...
        require(_id > 6, "{ID} is no fragment.");
        _;
    }
    ...
    //Mint -> User
    function mintWithCash(uint256 chargeAmount) 
    public 
    payable 
    mintCashCompliance(chargeAmount){
        burn(msg.sender, _Cash, _CashPerAssetToBurn * chargeAmount);
        safeTransferFrom(msg.sender, contractOwner, _Cash, _CashPerAssetFee * chargeAmount, "");
        uint256 randId = _createRandomNum(4) + 1;
        _mint(msg.sender, randId, _FusionAssetDefaultCharge, "");
    }

    function mintWithNative(uint256 chargeAmount) 
    public 
    payable 
        mintNativeCompliance(chargeAmount){
        uint256 randId = _createRandomNum(4) + 1;
    ...
    mintFragmentCompliance(_rarity){
        ...
        _mint(msg.sender, (_rarity + 1), _FusionAssetDefaultCharge, "");
    }
