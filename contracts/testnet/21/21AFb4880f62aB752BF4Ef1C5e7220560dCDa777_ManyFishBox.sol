...
contract ManyFishBox is ERC1155, Ownable, ERC1155Burnable, ReentrancyGuard {
    ...
    event PurchasedBox(uint8 roundId, uint8 boxType, uint256 price);
     ...
     */
    function buyBox(uint8 _roundId, uint8 _amount)
        public
        nonReentrant
        roundAvailable(_roundId)
    {
        ...
        require(
            purchaseRound.supply >= purchaseRound.minted + _amount,
            "Not enough box in supply"
        );

        uint256 payWei = _amount * purchaseRound.price;
        require(payToken.balanceOf(buyer) >= payWei, "Not enough token");

        require(payToken.transferFrom(buyer, recepientAddress, payWei));
        purchaseRound.minted += _amount;
        _mint(buyer, purchaseRound.boxType, _amount, "");
        emit PurchasedBox(_roundId, purchaseRound.boxType, _amount);
    }
    ...
    function setRoundTime(
        ...
        uint256 _startTime,
        uint256 _endTime
    ) public onlyOwner roundAvailable(_roundId) {
        require(_endTime > _startTime, "End time must be after start time");
    ...
    function setPayToken(address _payToken) public onlyOwner {
        require(_payToken != address(0), "payToken cannot be 0");
        payToken = IERC20(_payToken);
    }

    function withdrawToken(address _to, uint256 _amount) public onlyOwner {
        require(_amount > 0, "Amount must be greater than 0");
        require(_to != address(0), "Cannot send to the null address");
        require(
            payToken.balanceOf(address(this)) >= _amount,
            "Not enough token"
        );
        payToken.transferFrom(address(this), _to, _amount);
