            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"bb0f75246f3d156837d548d71cc81760c4a635d78d5d07187ba0e4d8050d3942" // init code hash
                    )
                )
            )
        );
...
contract SafeswapRouterSTP is ISafeswapRouter02, Initializable {
    using SafeMath for uint256;
    ...
    function getTokenDeduction(address token, uint256 amount) external view returns (uint256, address) {
        ...
        uint256 deduction = amount.mul(nameToInfo[token].buyFeePercent).div(ONE);
    ...
    function unregisterToken(address tokenAddress) external onlyOwner {
        require(nameToInfo[tokenAddress].tokenAddress != address(0), "token does not exist");
        delete nameToInfo[tokenAddress];
    ...
    function swapExactTokensForTokens(
        ...
        if (nameToInfo[path[0]].enabled == true && killSwitch == false && (nameToInfo[path[0]].sellFeePercent > 0)) {
            uint256 deduction = amountIn.mul(nameToInfo[path[0]].sellFeePercent).div(ONE);
        ...
        if (nameToInfo[path[1]].enabled == true && killSwitch == false && (nameToInfo[path[1]].buyFeePercent > 0)) {
            ...
            uint256 deduction = amountOut.mul(nameToInfo[path[1]].buyFeePercent).div(ONE);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            if (killSwitch == false) {
                uint256 deduction = amountOutput.mul(nameToInfo[output].buyFeePercent).div(ONE);
