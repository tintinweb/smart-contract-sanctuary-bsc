...
contract BiddenVault is ReentrancyGuard,Ownable{
    ...
    event Withdraw(address indexed user, uint256 amount,uint time);

    IBEP20 public stakeToken;
    address public signer;

    constructor (IBEP20 _stakedToken,address _signer) {
       stakeToken = _stakedToken;
       signer = _signer;
    }

    function stake(string memory _type,uint _amount)public nonReentrant {
        require(_amount > 0,"Incorrect amount");
        userCount[msg.sender]++;
        UserDetails storage user = users[msg.sender][userCount[msg.sender]];
        stakeToken.safeTransferFrom(msg.sender,address(this),_amount);
        uint feeAmount = feeCalc(_amount);
        user.depAmount += _amount-feeAmount;
        user.match_crypto = _type;
        emit Stake(msg.sender,_amount-feeAmount,block.timestamp);
    }

    function updateStakeToken(IBEP20 _token) public onlyOwner {
        require(address(_token) != address(0),"Invalid address");
        stakeToken = _token;
    }

    function feeCalc(uint _amount) internal returns(uint){
        uint calc = _amount*25/100;
        uint txFee = _amount*0.25e18/100e18;
        stakeToken.safeTransfer(owner(),calc+txFee);
        return calc+txFee;
    }

    function unStake(uint _amount,bytes calldata signature,uint _time) public nonReentrant {
        require(_amount > 0,"Incorrect amount");
        bytes32 messageHash = message(msg.sender,_amount,_time);
    ...
    */
    function message(address  _receiver ,uint256 _amount,uint time)
        public pure returns(bytes32 messageHash)
    {
        messageHash = keccak256(abi.encodePacked(_receiver,_amount,time));
    }

    function recovery(address _user,uint256 _amount) public onlyOwner {
        require(_user != address(0) && _amount > 0,"incorrect params");
        require(stakeToken.balanceOf(address(this)) >= _amount,"Insufficient amount");
