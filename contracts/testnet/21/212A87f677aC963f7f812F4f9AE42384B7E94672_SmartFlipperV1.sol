    ...
    VRFConsumerBaseV2
{
    ...
    event GamePayoutFailed(address indexed user, uint256 payoutAmount, uint256 indexed txIdentifier);
     ...
     */
    function play(uint8 _bet, address player, uint256 _value, uint256 _txIdentifier)
        public
        payable
        ...
        override
    {
        ...
        if (session.amount > 0) {
            if (session.bet == randomValue) {
                ...
                if (!success) {
                    emit GamePayoutFailed(session.player, session.payoutAmount, session.txIdentifier);
                } else {
