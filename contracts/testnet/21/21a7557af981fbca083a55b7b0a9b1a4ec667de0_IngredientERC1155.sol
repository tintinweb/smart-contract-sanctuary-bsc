...
import "../erc/erc1155/ERC1155Upgradeable.sol";
import "./random/Random.sol";
import "./access/AccessControlUpgradeable.sol";
import "./interface/IIngredientERC1155.sol";
import "./HoldEnumerable.sol";
import "../lib/ArrayUtil.sol";
import "./EIP2981.sol";


contract IngredientERC1155 is ERC1155Upgradeable, AccessControlUpgradeable, IIngredientERC1155, HoldEnumerable, EIP2981 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Upgradeable, AccessControlUpgradeable, EIP2981) returns (bool) {
        return interfaceId == type(IIngredientERC1155).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function createIngredient(
        ...
        uint256 typeIndex = Random.createRandom(1, length, seed, totalIngredient) - 1;
        ...
        {
            uint32 calories = Random.createRandom(1, 50, seed, totalIngredient);
            uint32 scent = Random.createRandom(1, 50, seed, calories);
            uint32 freshness = Random.createRandom(1, 50, seed, scent);
        ...
        _addHold(totalIngredient, recipient);
        _setAuthor(totalIngredient, recipient);
        _mint(recipient, totalIngredient, 1, "");
        emit IngredientCreated(childType, totalIngredient, recipient, boxId);
    }
    ...
    function name() external pure returns (string memory) {
        return "cryptoSandwichwiches ingredients"; 
    }
    
    function symbol() external pure returns (string memory) {
        return "csw ingredients";
    }
    ...
    function royaltyInfo(
        uint256 _tokenId,
        uint256 _salePrice
    ) external override view virtual returns (
        address receiver,
        uint256 royaltyAmount
    )
    {
        receiver = _authors[_tokenId];
        if(receiver != address(0)) royaltyAmount = _salePrice * 2 / 10000;
    }
}
...
interface IIngredientERC1155 {
    ...
    event IngredientCreated(uint8 childType, uint256 indexed id, address indexed owner, uint256 boxId);

    function createIngredient(address recipient, uint256 boxId, string calldata _name, string calldata _describe, uint256 seed) external;
...
interface IHoldEnumerable {
    // @dev Return all token id list of holder.
    function getHolds(address holder) external view returns (uint256[] memory ids);
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
import "../../erc/erc165/IERC165Upgradeable.sol";

///
/// @dev Interface for the NFT Royalty Standard
///
interface IEIP2981 is IERC165Upgradeable {
...
import "./interface/IHoldEnumerable.sol";

contract HoldEnumerable is IHoldEnumerable{
    // Mapping from account to token id list
    mapping(address => uint256[]) internal _userHolds;
    // Mapping from token id to index of _userHolds's array
    mapping(uint256 => uint256) internal _idIndexs;


    // @dev Return all token id list of holder.
    function getHolds(address holder) public virtual view override returns (uint256[] memory ids) {
        return _userHolds[holder];
    }

    /**
     * @dev Add id to holder holds.
     *
     * Requirements: id only have one holder
     */
    function _addHold(uint256 id, address holder) internal virtual {
        require(_idIndexs[id] == 0, "Token already appened");
        _userHolds[holder].push(id);
        _idIndexs[id] = _userHolds[holder].length;
    }

    /**
     * @dev Change id holder.
     *
     * Requirements: id only have one holder
     */
    function _transferHold(uint256 id, address holder, address to) internal virtual {
        _removeHold(id, holder);
        _userHolds[to].push(id);
        _idIndexs[id] = _userHolds[to].length;
    }

    /**
     * @dev Remove id holdler.
     *
     * Requirements: id only have one holder
     */
    function _removeHold(uint256 id, address holder) internal virtual {
        require(_userHolds[holder][_idIndexs[id] - 1] == id, "Token doesn't exist");
        _userHolds[holder][_idIndexs[id] - 1] = 0;
    }
}

// SPDX-License-Identifier: MIT
import "./interface/IEIP2981.sol";

pragma solidity ^0.8.0;

abstract contract EIP2981 is IEIP2981 {
    // Mapping from token ID to author, you should map it while token id first mint.
    ...
    /**
     * @notice Call this function when mint.
     */
    function _setAuthor(uint256 id, address author) internal{
        if(_authors[id] == address(0)) _authors[id] = author;
    }
    ...
    // {
    //     receiver = _authors[_tokenId];
    //     if(receiver != address(0)) royaltyAmount = _salePrice / 1000;
    // }
}