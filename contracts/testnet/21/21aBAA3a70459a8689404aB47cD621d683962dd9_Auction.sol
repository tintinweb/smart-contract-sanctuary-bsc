...
contract Auction is ReentrancyGuard, Ownable {
    ...
    //f.
    function bidMinterNFT(uint256 _tokenId, uint256 _price, uint256 _amount, uint256 _period, bool _isERC721, bytes memory _data) public returns(uint256) {
    ...
    function withdrawBid(uint256 _auctionId, bool _isERC721) public payable nonReentrant {
        ...
        if(idToAuctionItem[_auctionId].owner == msg.sender) {
            require(msg.value >= marketplaceContract.marketfee(), "insufficient fund");
            if(_isERC721){
            ...
            require(msg.value >= idToAuctionItem[_auctionId].highestBid + marketplaceContract.marketfee(), "Insufficient fund");
            if(_isERC721)
                ylnft721.transferFrom(address(this), msg.sender, idToAuctionItem[_auctionId].tokenId);
            else 
                ylnft1155.safeTransferFrom(address(this), msg.sender, idToAuctionItem[_auctionId].tokenId, idToAuctionItem[_auctionId].amount, "");
