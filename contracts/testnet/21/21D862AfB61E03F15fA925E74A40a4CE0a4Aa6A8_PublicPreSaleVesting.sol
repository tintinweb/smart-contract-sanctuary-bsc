...
contract PublicPreSaleVesting is ReentrancyGuard, Ownable {
    ...
    function schedule(address _beneficiary, uint _period) public view returns (
        uint date,
        uint amount,
        uint dateClaimed,
        Status status
        ...
        amount = _amountByPeriod(_period, _beneficiary);
        dateClaimed = dateRealeased[_beneficiary][_period];
        if (isClaimed[_beneficiary][_period] == true) {
    ...
    function _amountByPeriod(uint _period, address _beneficiary) internal view returns (uint tokenAmount) {
        if (_period == 1) {
            tokenAmount = (beneficiarysAllocation[_beneficiary] / 100) * settings.tokenPercentUnlockOnTge;
        } else {
            tokenAmount = (beneficiarysAllocation[_beneficiary] - ((beneficiarysAllocation[_beneficiary] / 100) * settings.tokenPercentUnlockOnTge)) / (settings.numberOfEmmissions - 1);
        }
    }
