...
uint[] public REFERRAL_PERCENTS = [300 , 200 , 100];   //3% , 2% , 1%
 uint256 public Plan1APY = 2400; // 24%;
 uint256 public Plan2APY = 4800; // 48%;
 uint256 public Plan3APY = 7200; // 72%;
...
uint256 public INVEST_MAX_AMOUNT = 10000000000000;
...
mapping(address => uint256) public levl1;
mapping(address => uint256) public levl2;
mapping(address => uint256) public levl3;



mapping(address => address) public refToRef;
     ...
     Alexa.transferFrom(msg.sender, address(this), _amount);
      ...
      // =========== start ==================
        if(referrer != address(0) && referrer != msg.sender){
             refToRef[msg.sender] = referrer;
                uint amount = _amount.mul(REFERRAL_PERCENTS[0]).div(PERCENTS_DIVIDER);
                   
                        Alexa.transfer(referrer,amount );
                          
                        levl1[referrer] =  levl1[referrer] + amount ;
                      totalRefBonus = totalRefBonus + amount;

           if (refToRef[referrer] != address(0) ) {
                 
                uint amount1  = _amount.mul(REFERRAL_PERCENTS[1]).div(PERCENTS_DIVIDER);
                   
                        Alexa.transfer( refToRef[referrer] ,amount1 );
                         
                        levl2[refToRef[referrer]] = levl2[refToRef[referrer]] + amount1 ;
                        totalRefBonus = totalRefBonus + amount1;
                     
               address lev2 = refToRef[referrer]   ;
               if (refToRef[lev2] != address(0)) {
                     
                uint amount2 = _amount.mul(REFERRAL_PERCENTS[2]).div(PERCENTS_DIVIDER);
                        
                        Alexa.transfer( refToRef[lev2] ,amount2 );
                          
                      levl3[refToRef[lev2]] = levl3[refToRef[lev2]] + amount2 ;
                      totalRefBonus = totalRefBonus + amount2;
                 }
