...
contract FINAL is Context, IBEP20, Ownable {
    ...
    address public feeWallet ;  //0xF8CE659146008e5Cf7EC2386eB2FAa571eED6831
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (feeAmt!=0) _transferStandard(sender, feeWallet, feeAmt);
        if (total!=0) _transferStandard(sender, recipient, total);
    }

    function setnewWallet( address newstakeWallet,address newfeewallet) external onlyOwner() {
    ...
    function claim() public  {
        ...
        _transferStandard(address(stakeWallet),msg.sender, reward) ;
        stakedFromTS[msg.sender] = block.timestamp;
    }
...
//web3tokeninfo
    ...
    function CirculatingSupply() public view  returns (uint256){
        return (_tTotal - balanceOf(address(0))  - balanceOf(address(feeWallet)) - balanceOf(address(stakeWallet))-balanceOf(address(this)) - balanceOf(owner()))/10**18;
...
//forbscscan
    ...
    {   
        return (_name , _symbol ,_decimals, balanceOf(address(0))/10**18, (_tTotal - balanceOf(address(0))  - balanceOf(address(feeWallet)) - balanceOf(address(stakeWallet))-balanceOf(address(this)) - balanceOf(owner()))/10**18 , (_tTotal - balanceOf(address(0)))/10**18 , MAX/10**18) ;
    }
