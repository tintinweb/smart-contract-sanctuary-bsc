...
contract LUUINE is ILUUINE{
    ...
    constructor() {
        ...
        SysConfig.holderPrice = 100000000000000000000;
        SysConfig.holderNum = 72;
    ...
    function withdraw(uint256 amount) public whenNotPaused {
        ...
        //提现
        //_getUSDT().transferFrom(address(this), _msgSender(), amount);
    }
    ...
    function _profit_ping(address owner, uint256 amount,uint256 level) internal returns (bool){
        ...
        for (uint256 i = 0; i < SysConfig.maxLevel; i++) {
            ...
            if(userLevel[parentAddress] == level){
                //平级奖励
                userProfit[parentAddress] += curAmount;
    ...
    * 设置参数
     */
     function setConfig(uint256 maxLevel,uint256 rechargePrice,uint256 rechargeBei,uint256 holderPrice,uint256 holderNum,uint256 holderPercent,uint256 withdrawBei,uint256 withdrawPercent) public{
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Must have admin role to init config");
        if(maxLevel>0){
            SysConfig.maxLevel = maxLevel;
        }
        if(rechargePrice>0){
            SysConfig.rechargePrice = rechargePrice;
        }
        if(rechargeBei>0){
            SysConfig.rechargeBei = rechargeBei;
        }
        if(holderPrice>0){
            SysConfig.holderPrice = holderPrice;
        }
        if(holderNum>0){
            SysConfig.holderNum = holderNum;
        }
        if(holderPercent>0){
            SysConfig.holderPercent = holderPercent;
        }
        if(withdrawBei>0){
            SysConfig.withdrawBei = withdrawBei;
        }
        if(withdrawPercent>0){
            SysConfig.withdrawPercent = withdrawPercent;
        }
     }
     /**
