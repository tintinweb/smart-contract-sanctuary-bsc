...
contract Gateway is Ownable, ReentrancyGuard {
  ...
  event ChangedFeesBeneficiary(address oldBeneficiary, address newBeneficiary);
  event MerchantAdded(string indexed merchantId);
  event ChangedMerchantStatus(string indexed merchantId, bool enabled);
  event ChangedMerchantBeneficiary(string indexed merchantId, address oldBeneficiary, address newBeneficiary);
  event ChangedMerchantFeesPercentage(string indexed merchantId, uint16 oldFeesPercentage, uint16 newFeesPercentage);
  event PaymentMade(string indexed merchantId, Transaction transaction);
  ...
  /**
   * @dev Check if the merchant exists and is enabled
   */
    ...
    merchantExists(merchantId)
  {
    ...
    emit PaymentMade(merchantId, transaction);
  }
    ...
    returns (Transaction memory)
  {
    require(
      merchants[merchantId].transactions[transactionId].payer != address(0),
      "This transaction has not been paid"
    );

    return merchants[merchantId].transactions[transactionId];
  }
    ...
    merchantExists(_id)
  {
    ...
    emit ChangedMerchantStatus(_id, _enabled);
  }
    ...
    merchantExists(_id)
  {
    ...
    merchants[_id].beneficiary = _beneficiary;
    emit ChangedMerchantBeneficiary(_id, oldBeneficiary, _beneficiary);
  }
    ...
    merchantExists(_id)
  {
    ...
    merchants[_id].feesPercentage = _feesPercentage;
    emit ChangedMerchantFeesPercentage(_id, oldFeesPercentage, _feesPercentage);
  }
    ...
    onlyFeesBeneficiaryOrOwner
  {
    ...
    emit ChangedFeesBeneficiary(oldBeneficiary, _beneficiary);
  }
}