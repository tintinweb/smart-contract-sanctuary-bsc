...
contract Staking is IStaking, Initializable {
    ...
    function initialize(
        ...
        levels[2] = LevelDetails(10 minutes, 6);
        levels[3] = LevelDetails(20 minutes, 6);
        levels[4] = LevelDetails(30 minutes, 7);
    ...
    modifier onlyOperator() {
     ...
     modifier onlyAdmin() {
        require(
            ...
            "Staking: sender is not an admin"
        );
        _;
    }
    ...
    function stake(uint256 _level, uint256 _amount) external payable {
        ...
        if(_amount == 0){
            require(s.amount != 0,"Staking: deposited amount must be greater than 0");
            require(_level > s.lock, "Staking: Invalid lock level");

            sec = levels[_level].duration;
            s.lock = _level;
            s.lockTime = block.timestamp + sec;//check the reference
            return;
        }
        ...
        sec = levels[_level].duration;
        s.lock = _level;
        s.lockTime = block.timestamp + sec;//check reference
        s.Tier = _getHighestTier(msg.sender);
        ...
        //why different airdrops for EBSC and other tokens
        if (s.lock > 1 && uint8(highestTier) > 3) {
            if(prevLock <= 1 || prevTier <= 3){
                IAirdrops(admin.airdrop()).pushEBSCAirdrop(s.amount);  
                IAirdrops(admin.airdrop()).userStakeAirdrop(
                msg.sender,
                block.timestamp,
                0
    ...
    function unstake() external override {
        ...
        if (s.lock > 1 && uint8(prevTier) > 3) {
            IAirdrops(admin.airdrop()).userStakeAirdrop(
                msg.sender,
                s.lockTime - levels[s.lock].duration,
                block.timestamp
            );
            IAirdrops(admin.airdrop()).popEBSCAirdrop(amount);
        }
        if (s.lock > 1) {
            IAirdrops(admin.airdrop()).userPendingEBSC(msg.sender);
        ...
        // uint256 currentTier = _getHighestTier(msg.sender);
    ...
    function seNoLock(bool _noLock) external onlyAdmin{
        noLock = _noLock;
    }
