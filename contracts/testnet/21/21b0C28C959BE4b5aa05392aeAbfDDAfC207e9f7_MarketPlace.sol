...
contract MarketPlace is Ownable {
    ...
    struct OfferDatas{
       ...
       uint256 price;
       uint256 bidValue;
       uint256 creatingBlock;
    }

    uint256 public blockPerDay = 43200;
    uint256 public offerDuration = 3 * blockPerDay;
    ...
    mapping(address => MyProposal[]) _myProposals;

    IAddresses public gameAddresses;


    event NftSold (address indexed nftAddress,uint256 offerId, uint256 indexed nftId, uint256 price, address pastOwner, address newOwner);
    event NftBid (address indexed nftAddress,address indexed owner, uint256 offerId, uint256 indexed nftId, uint256 price);
    event NftInSale (address indexed nftAddress,address indexed owner,uint256 offerId, uint256 indexed nftId, uint256 price);
    ...
    function getOffer(uint256 _offerId) external view returns(OfferDatas memory){
        return _offers[_offerId];
    }

    function _getMyProposalsActiveNumber(address _user) internal view returns(uint256 result){
        if(_myProposals[_user].length > 0){
            for(uint256 i = _myProposals[_user].length - 1 ; i >= 0 ; ){
                if(_myProposals[_user][i].date + offerDuration <= block.timestamp){
                    result += 1;
                }else{
                    break;
                }
    ...
    function getMyProposals(address _user) external view returns(uint256[] memory myProposals){
        uint256 _length = _getMyProposalsActiveNumber(_user);

        if(_length > 0){
            uint256 _start = _myProposals[_user].length - 1;
            uint256 _index;

            for(uint256 i = _start ; i >= _start + 1 - _length ; ){
                myProposals[_index] = _myProposals[_user][i].offerId;
                unchecked{ -- i ;}
                unchecked{ ++ _index ;}
            }
        }
    ...
    function sellNft(address _nftAddress, uint256 _nftId, uint256 _askedPrice) external {
        IERC721 I = IERC721(_nftAddress);
        require(I.ownerOf(_nftId) == msg.sender,"Not your NFT");
        require(I.isApprovedForAll(msg.sender, address(this)) == true,"Need to approve for all NFT");
        offersCount += 1;
        OfferDatas storage offer = _offers[offersCount];
        offer.nftAddress = _nftAddress;
        offer.nftId = _nftId;
        offer.nftOwner = msg.sender;
        offer.creatingBlock = block.number;
        offer.price = _askedPrice;

        emit NftInSale(_nftAddress,msg.sender, offersCount, _nftId, _askedPrice);
    ...
    function bidForNft(address _nftAddress, uint256 _nftId, uint256 _bidPrice) external {
        require(BZAI.allowance(msg.sender,gameAddresses.getPaymentsAddress()) >= _bidPrice,"You need to approve contract");
        offersCount += 1;
        OfferDatas storage offer = _offers[offersCount];
        offer.nftAddress = _nftAddress;
        ...
        _myProposals[msg.sender].push(MyProposal(offersCount, block.timestamp));

        emit NftBid(_nftAddress, offer.nftOwner , offersCount, _nftId, _bidPrice);
    ...
    function acceptBid(uint256 _offerId) external {
        ...
        require(BZAI.allowance(offer.offeredBy,address(this)) >= offer.bidValue,"Contract has been unapproved by bider");
        require(block.number <= offer.creatingBlock +  offerDuration, "Offers only avalaible 3 days");

        IPayments IPay = IPayments(gameAddresses.getPaymentsAddress());
        require(IPay.payWithRewardOrWallet(offer.offeredBy, offer.bidValue),"bider hasn't enough founds");
        IPay.payNFTOwner(msg.sender, offer.bidValue);

        IERC721(offer.nftAddress).transferFrom(msg.sender, offer.offeredBy, offer.nftId);
        delete _offers[_offerId];

        emit NftSold(offer.nftAddress, _offerId, offer.nftId, offer.bidValue, msg.sender, offer.offeredBy);
    }

    function buyNft(uint256 _offerId) external {
        OfferDatas memory offer = _offers[_offerId];
        require(offer.nftOwner == IERC721(offer.nftAddress).ownerOf(offer.nftId), "owner change offer not available");
        require(block.number <= offer.creatingBlock +  offerDuration, "Offers only avalaible 3 days");
        ...
        IERC721(offer.nftAddress).transferFrom(offer.nftOwner, msg.sender, offer.nftId);
        delete _offers[_offerId];

        emit NftSold(offer.nftAddress, _offerId, offer.nftId, offer.price, offer.nftOwner, msg.sender);
    }
