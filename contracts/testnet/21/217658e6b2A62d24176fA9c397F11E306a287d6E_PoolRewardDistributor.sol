...
* а также часть от инфляционной эмиссии ZOINKS токена.
*/
contract PoolRewardDistributor is Ownable {
    ...
    uint256 constant private BTC_ETH_SNACKS_PANCAKE_SWAP_POOL_PERCENT = 5714;
    uint256 constant private BTC_ETH_SNACKS_SNACKS_POOL_PERCENT = 4286;
    ...
    * @param authority_ Адрес EOA, имеющего доступ к вызову функции распределения наград.
    */
    function configure(
        address zoinks_,
        address snacks_,
    ...
    */
    function distributeRewards() external onlyAuthority {
        ...
        if (ethSnacksBalance != 0) {
            ...
            reward = ethSnacksBalance * BTC_ETH_SNACKS_SNACKS_POOL_PERCENT / BASE_PERCENT;
...
interface ISingleRewardPool {
    function notifyRewardAmount(uint256 reward_) external;
}
