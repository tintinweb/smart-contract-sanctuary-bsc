...
contract CyberArenaStaking is ERC20, Ownable, ReentrancyGuard {
    ...
    function stake(uint128 _amount) external nonReentrant {
        ...
        if(stakeRef.stakedTimestamp == 0){
            ...
            require(getCurrentTime() > stakeRef.stakedTimestamp + 24 * 86400, "already staked");
            
            stakingToken.safeTransferFrom(msg.sender, address(this), _amount);
            _mint(msg.sender, _amount);
            totalShares += _amount;
            stakeRef.unstaked = false;
            stakeRef.amount = stakeRef.amount + _amount;
            stakeRef.stakedTimestamp = uint48(getCurrentTime());
            stakeRef.shares = stakeRef.shares + _amount;
        }
    }
    ...
    function unstake() external nonReentrant {
        ...
        require(!stakeRef.unstaked, "CyberArenaStaking: Nothing is staked");
