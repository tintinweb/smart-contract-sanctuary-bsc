...
contract Nft2dSales is Initializable, OwnableUpgradeable {
    ...
    function setPirce(uint price_) public onlyOwner {
        price = price_;
    }
    ...
    // 管理员铸造
    function mint(address player_) public {
        nft.mint(player_);
    }

    // 用户铸造 NFT
    function playerMint(uint orderNumber_) public payable {
        ...
        if (freeMaxNums != 0 && isWhiteList(player)) {
            ...
            payable(wallet[0]).transfer(price * 5 / 10);
            payable(wallet[1]).transfer(price * 3 / 10);
            payable(wallet[2]).transfer(price * 2 / 10);
...
contract Meta2dnft is Divestor, ERC721Enumerable {
    ...
    constructor () ERC721("2D Meta NFT", "2DMT") {
        baseURI = "https://test/";
    }
    ...
    // 销售 铸造
    function mint(address toPlayer_) public onlySales {
        _tokenIdCounter.increment();
        uint tokenId = _tokenIdCounter.current();
        _safeMint(toPlayer_, tokenId);
    }
