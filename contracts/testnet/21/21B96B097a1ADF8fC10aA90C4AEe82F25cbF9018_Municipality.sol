...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint8 private constant BUNDLE_TYPE_PARCELS_1 = 0;
    uint8 private constant BUNDLE_TYPE_PARCELS_2 = 1;
    uint8 private constant BUNDLE_TYPE_PARCELS_3 = 2;
    uint8 private constant BUNDLE_TYPE_MINERS_1 = 3;
    uint8 private constant BUNDLE_TYPE_MINERS_2 = 4;
    uint8 private constant BUNDLE_TYPE_MINERS_3 = 5;
    ...
    function purchaseParcelsBundle(ParcelsMintSignature calldata _mintingSignature, uint8 _bundleType,
    ...
    function purchaseMinersBundle(uint256 _referrerId, uint8 _bundleType) external onlySaleActive notZeroAddress returns(uint256, uint256) {
    ...
    function automaticallyAttachMinersToParcels() external {
        uint256[] memory parcelIds = IERC721Base(standardParcelNFTAddress).tokensOf(msg.sender);
        ...
        for(uint256 i = 0; i < parcelIds.length && lastAvailableMinerIndex < userMiners.length; ++i) {
            uint256 parcelId = parcelIds[i];
            uint256 availableSize = standardParcelSlotsCount - parcelMinersMapping[parcelId].length;
            if(availableSize > 0) {
                for(uint256 j = 0; j < availableSize; ++j) {
                    for(uint256 k = lastAvailableMinerIndex; k < userMiners.length; ++k) {
                        if(minerParcelMapping[userMiners[k]] == 0) {
                            parcelMinersMapping[parcelId].push(userMiners[k]);
                            minerParcelMapping[userMiners[k]] = parcelId;
                            IMining(miningAddress).deposit(msg.sender, userMiners[k], 1000);
                            emit MinerAttached(msg.sender, parcelId, userMiners[k]);
                            break;
                        }
                        lastAvailableMinerIndex = k;
                    }
                }
            }
        }
    ...
    // @notice Private interface  minerParcelMapping[firstMinerId + minerCounter] = parcelIds[i];
