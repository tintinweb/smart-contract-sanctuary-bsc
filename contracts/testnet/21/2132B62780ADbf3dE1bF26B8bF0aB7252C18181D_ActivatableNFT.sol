...
import "./interfaces/IPublicBuilding.sol";

contract ActivatableNFT is ERC721Base, OwnableUpgradeable, IActivatableNFT {
    event BranchPublicBuildingContractAddressChanged(
    ...
    address public branchPublicBuildingContractAddress;
    address public brokerPublicBuildingContractAddress;

    modifier onlyBranchsPublicBuilding() {
        require(
            msg.sender == branchPublicBuildingContractAddress,
            "ActivatableNFT: This function can be called only by this Branch's Public Contract"
        );
        _;
    }
    ...
    modifier onlyBrokerPublicBuilding() {
        require(
            msg.sender == brokerPublicBuildingContractAddress,
            "ActivatableNFT: This funcrion can be called only by Broker Public Building Contract"
        );
        _;
    }
    ...
    function initialize(
        ...
        _setBranchPublicBuildingContractAddress(
            _branchPublicBuildingContractAddress
        );
        _setBrokerPublicBuildingContractAddress(
            _brokerPublicBuildingContractAddress
        );
        __Ownable_init();
    }

    function setBranchPublicBuildingContractAddress(
        address _branchPublicBuildingContractAddress
    ) external onlyOwner {
        _setBranchPublicBuildingContractAddress(
            _branchPublicBuildingContractAddress
    ...
    function getBranchPublicBuildingContractAddress()
        external
        view
        returns (address)
    {
    ...
    function mintNFTTo(address _to)
        external
        onlyBranchsPublicBuilding
    ...
    function _changeNFTStakerStatus(uint256 _tokenId, bool _lockStatus)
        internal
    {
        if (_lockStatus) {
            IPublicBuilding(branchPublicBuildingContractAddress).addNFTStaking(
                _ownerOf(_tokenId)
            );
        } else {
            IPublicBuilding(branchPublicBuildingContractAddress).removeNFTStaking(
                _ownerOf(_tokenId)
            );
        }
    }

    function changeTokenLocking(uint256 _tokenId, bool _lockStatus)
        external
    ...
    function _setBranchPublicBuildingContractAddress(
        address _branchPublicBuildingContractAddress
    ) internal {
        require(
        ...
        require(
            _branchPublicBuildingContractAddress !=
                branchPublicBuildingContractAddress,
        ...
        emit BranchPublicBuildingContractAddressChanged(
            _branchPublicBuildingContractAddress
        );
    }

    function _setBrokerPublicBuildingContractAddress(
        address _brokerPublicBuildingContractAddress
    ) internal {
        require(
        ...
        require(
            _brokerPublicBuildingContractAddress !=
                brokerPublicBuildingContractAddress,
        ...
        emit BrokerPublicBuildingContractAddressChanged(
            _brokerPublicBuildingContractAddress
        );
    }
}
