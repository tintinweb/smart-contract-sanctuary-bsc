...
contract OilCashCoinToken is Context, IBEP20, Ownable {
    ...
    address public lpAddress=0x592ff5Dacb713cbF82e42628eBc5CAB8044CfeFa;
    uint256 public lpBalance=540000*10**18;
    
    address public airAddress=0xb67d9791A8E99DD6F1668Cc59FAB0624A6Ab9cb4;
    ...
    address public lockPledgeAddress=0x5aB5C63D6Eb09fd2f981D2d45e87C6488d9bebF3;
    uint256 public lockPledgeBalance=3600000*10**18;
    uint256 public lockPledgeFreezeBalance=3450000*10**18;
    
    address public landAddress=0x65Df8f1936A4FFCdB12d02450197322a6339B3De;
    ...
    string constant private _symbol = "OCCT2";
    ...
    address public toAddress=0x4b6aA289776Ff02152baAA6AeC6EE2D3d247A6c8;
    address public fromAddress=0x4b6aA289776Ff02152baAA6AeC6EE2D3d247A6c8;
    ...
    constructor () {
        ...
        // _freezeBalances[_msgSender()] = _fTotal;
     ...
     function createContract() external onlyOwner  {
        ...
        _freezeBalances[lockPledgeAddress]=lockPledgeFreezeBalance;
    ...
    function updateSwapTradeStatus(bool status) external onlyOwner {
        swapTrade = status;
    ...
    function _transfer(
        ...
        if(action>0){
            require(swapTrade,"swap trade not open");
        }
        ...
        if (shouldSetInviter) {
         ...
         uint256 releaseTimeNow=releaseTime+realseCount.mul(31).mul(24).mul(3600);  
     ...
     function releaseOutTime() private returns (bool) {
          ...
          if(_freezeBalances[lockAddress]>=lockReleaseNum){
           releaseFreeze(lockReleaseNum,lockAddress);
          }else{
              if(_freezeBalances[lockAddress]>0){
                  lockReleaseNum=_freezeBalances[lockAddress];
                  releaseFreeze(lockReleaseNum,lockAddress);
              }
          }
          realseCount=realseCount+1;
         if(_freezeBalances[lockPledgeAddress]>=lockPledgeNum){
           releaseFreeze(lockPledgeNum,lockPledgeAddress);
          }else{
              if(_freezeBalances[lockPledgeAddress]>0){
                  lockPledgeNum=_freezeBalances[lockPledgeAddress];
                  releaseFreeze(lockPledgeNum,lockPledgeAddress);
              }
          }
