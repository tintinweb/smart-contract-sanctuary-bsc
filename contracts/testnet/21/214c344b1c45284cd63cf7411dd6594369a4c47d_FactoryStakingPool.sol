...
contract StakePoolTokenForBNB is States, ReentrancyGuard, Ownable {
    ...
    constructor() {
        ...
        unlockDate = block.timestamp.add(94692047);
    }
    ...
    function setTokens(uint256 unlockDate, IBEP20 _stakingToken, uint256 _lockPeriod) public onlyOwner {
        lockPeriod = _lockPeriod;
        stakingToken = _stakingToken;
        unlockDate = unlockDate;
    }

    function setData(uint256 _days, uint256 unlockDate) public payable onlyOwner {
        lockPeriod = _days;
        unlockDate = unlockDate; 
    }
    ...
    function EmergencyUnstake() public nonReentrant{
      User memory wUser = deposit[_msgSender()];
        ...
        deposit[_msgSender()] = User(0 , 0, 0, 0, 0, 0, TimeWeightedAPR(0,0));
...
contract FactoryStakingPool is Ownable{
    ...
    function createTokenForBNB(address ownerAddress, uint256 unlockDate, IBEP20 _stakingToken, uint256 _lockPeriod) public onlyOwner returns (address)
    {
        StakePoolTokenForBNB toDeploy = new StakePoolTokenForBNB();
        toDeploy.setTokens(unlockDate, _stakingToken, _lockPeriod);
