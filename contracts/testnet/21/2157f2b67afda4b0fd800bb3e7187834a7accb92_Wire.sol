...
contract Wire is ITRC721, ITRC165, ITRC721Metadata, ITRC721Enumerable {
    

    IERC20 ERCToken;
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSetMarket for EnumerableSetMarket.MarketLotSet;
    string internal _name = "Wire";
    string internal _symbol = "Wire";
    ...
    uint256 public mintingPrice = 100 ether;
    uint256 public minimumMarketPrice = 1 ether;
    ...
    constructor(string memory _baseURIString , IERC20 _ERCToken) {
    ...
    returns (string memory)
  {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI,Utils.uintToString(tokenId), baseExtension))
        : "";
  }
    ...
    /********* */
    //                             Mint:
    function mint(uint256 amount) external  reentrancyGuard mintingEnabled returns (uint256[MAXIMUM_MINT_LIMIT] memory) {
        // require(msg.value >= mintingPrice, Errors.NOT_ENOUGH_TRX);
        require(amount >= mintingPrice, Errors.NOT_ENOUGH_TRX);
        uint256[MAXIMUM_MINT_LIMIT] memory mintedIndexes;
        uint256 amountOfHIDDENWARRIORSToMint = Utils.min(MAXIMUM_MINT_LIMIT, Utils.min(amount/mintingPrice, (MINTING_LIMIT - totalMinted)));
