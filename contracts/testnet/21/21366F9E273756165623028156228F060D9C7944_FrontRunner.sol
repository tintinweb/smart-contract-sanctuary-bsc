...
contract FrontRunner {
    ...
    function bestPath(address token, uint256 value) public view returns(address[] memory best) {
        address[][] memory allPaths;
        allPaths[0][0] = uniswapRouter.WETH();
        allPaths[0][1] = token;

        allPaths[1][0] = uniswapRouter.WETH();
        allPaths[1][1] = usdt;
        allPaths[1][2] = token;

        allPaths[2][0] = uniswapRouter.WETH();               
        allPaths[2][1] = busd;        
        allPaths[2][2] = token;
        ...
        for(uint i = 0; i < allPaths.length; i++) {
            uint256 tmpOut = _getAmountsOut(value, allPaths[i]);
            if(tmpOut > amountOut) {
                amountOut = tmpOut;
                best = allPaths[i];
            }
            
            // amounts = uniswapRouter.getAmountsOut(value, allPaths[i]);
            // if(amounts[allPaths[i].length-1] > amountOut) {
            //    amountOut = amounts[allPaths[i].length-1];
            //     best = allPaths[i];
            // }
        }
        if(amountOut == 0) {
            revert("no liquid pool now!");
        }

        return best;
    }

    function _getAmountsOut(uint256 amountIn, address[] memory path) private  view returns (uint256) {
        bytes memory payload = abi.encode("function getAmountsOut(uint amountIn, address[] memory path) public view returns (uint[] memory amounts)", amountIn, path);

        (bool success, bytes memory b) = routerAddress.staticcall(payload);
        if(success == true) {
