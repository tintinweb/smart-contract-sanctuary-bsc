...
interface IToken {
    ...
    function getSellFee() external returns (bool);
}
...
library TransferHelper {
  ...
  function safeTransferFromLiq(
			    ...
			    // bytes4(keccak256(bytes('transferFrom(address,address,uint256)')));
			    (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0xdcd8c3d8, from, to, value));
			    require(
			      success && (data.length == 0 || abi.decode(data, (bool))),
			      'TransferHelper::transferFrom: transferFrom failed'
			    );
...
abstract contract SupportingSwap is FeeStore, IPYESwapRouter {
    ...
    function swapExactTokensForTokens(
        ...
        require(path.length == 2, "ONLY_TWO_TOKENS_ALLOWED");
...
contract PYESwapRouter is SupportingSwap {
    ...
    function addLiquidityETH(
        ...
        // bool sellFee = false;
        // try IToken(address(token)).getSellFee() { sellFee = true; } catch { sellFee = false; }
        //     if(sellFee) {
        //         TransferHelper.safeTransferFromLiq(token, msg.sender, pair, amountToken);
        //     } else {
