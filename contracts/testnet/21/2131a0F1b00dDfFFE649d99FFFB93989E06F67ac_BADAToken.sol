...
contract BADAToken is Authorized, ERC20 {
  ...
  string constant _name = "BadaBridge";
  ...
  // Fees
  uint256 public feeAdministrationWallet = 800; // 8%

  uint256 public feePool = 400; // 4%
  ...
  function setMaxTxAmountWithDecimals(uint256 decimalAmount) public isAuthorized(1) {
    require(decimalAmount <= maxSupply, "Too high");
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    administrationWallet = 0x755a6f18dAbCf5669F12a6aD9492B951c77875B6;
  ...
  function _mint(address account, uint256 amount) internal override {
    require(maxSupply >= ERC20.totalSupply() + amount && maxSupply >= amount, "100% minted");
    super._mint(account, amount);
  }

  function _beforeTokenTransfer( address from, address, uint256 amount ) internal view override {
    require(amount <= _maxTxAmount || exemptTxLimit[from], "exceeded max tx limit");
  ...
  function _afterTokenTransfer( address, address to, uint256 ) internal view override {
    require(_balances[to] <= _maxAccountAmount || exemptAmountLimit[to], "exceeded max tokens a wallet holds");
  }

  function _transfer( address sender, address recipient,uint256 amount ) internal override {
    require(!_noReentrancy, "ReentrancyGuard");
    _noReentrancy = true;
    
    require(sender != address(0) && recipient != address(0), "non zero address");
    
    _beforeTokenTransfer(sender, recipient, amount);

    uint256 senderBalance = _balances[sender];
    require(senderBalance >= amount, "send less");
