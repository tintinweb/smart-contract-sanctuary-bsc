...
contract BoxMarketPlace is OwnableUpgradeable, PausableUpgradeable  {
    IBEP20 public token;
    struct Auction {
        uint256 highestBid;
    ...
    function initByOwner(BoxNFT _boxNFT, IBEP20 _token) public onlyOwner{
        boxNFT=_boxNFT;
        token=_token;
    }
    ...
    function closeAuctionSudden(uint256 _nftId) external whenNotPaused {
        ...
        boxNFT.transfer(_nftId, auctions[_nftId].originalOwner);
    ...
    function sell(
        ...
        require(boxNFT.ownerOf(_nftId) == msg.sender, "Not NFT owner");
        boxNFT.approveMarketPlace(address(this),_nftId);
        // NFT Transfer to contract
        boxNFT.transfer(_nftId, address(this));
    ...
    function buy(uint256 _nftId, uint256 _amount) external  whenNotPaused {
        ...
        // token.transferFrom(msg.sender,auctions[_nftId].originalOwner, _amount);
        auctions[_nftId].highestBidder = msg.sender;
        // Transfer NFT to Highest Bidder
        boxNFT.transfer(_nftId, auctions[_nftId].highestBidder);
        // Close Auction
        auctions[_nftId].isActive = false;
...
function openBox(uint256 tokenId) external whenNotPaused returns (uint256 characterId){
        ...
        characterId=characterNFT.createCharacter(msg.sender,typeCharacter,1,1,1,0);    
        _burn(tokenId);
    }
