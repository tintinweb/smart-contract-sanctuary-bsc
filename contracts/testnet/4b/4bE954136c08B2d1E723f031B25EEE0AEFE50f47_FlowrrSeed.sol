...
contract FlowrrSeed is Ownable{
    ...
    //Token Address
    address constant USDT = 0xdcb350D032bEE7D5DE8F7885Dc0d6f154b0c44c2; //0x55d398326f99059fF775485246999027B3197955 Mainnet
    address public SEED;
    ...
    function depositToken(uint256 uid, uint256 amount, address _tokenAddress) external {
        ...
        IERC20(_tokenAddress).transfer(feeAddr, amount * uint256(feePercents) / baseDivider);
        emit DepositToken(uid, msg.sender, amount, _tokenAddress, amount * uint256(feePercents) / baseDivider);
    }

    function withdrawToken(address account, uint256 amount, address _tokenAddress) external onlyGovernance {
        require(avail_wd == true, "Withdraw Currently Unavailable");
    ...
    function swapFlowrr_Ustd(uint256 amount) external {
        IERC20(FLOWRR).transferFrom(msg.sender, address(this), amount);
        IERC20(USDT).transfer(msg.sender, amount);
        emit SwapToken(msg.sender, amount, amount/10, "flowrr_to_usdt");
    }
