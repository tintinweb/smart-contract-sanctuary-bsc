...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("FSL_Busd_Dividend_Tracker", "FSL_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 3000000000 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "FSL_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "FSL_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FSL contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "FSL_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "FSL_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract test is ERC20, Ownable {
    ...
    uint256 public maxWalletBalance = 600000000000000000 * (10**18);
    ...
    constructor() ERC20("test", "test") {
    	...
    	marketingWallet = 0xbA9C053Cb58a1eACcf63BaE84f6A06304E6aeAD9; 
    	busdDividendToken = 0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814; 
        ...
        */
        _mint(owner(), 600000000000000000 * (10**18));
    }
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "FSL: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "FSL: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "FSL: The new dividend tracker must be owned by the FSL token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "FSL: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "FSL: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "FSL: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "FSL: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "FSL: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "FSL: Trading has not started yet");
    ...
    function SetSellFee(uint256 marketingFeeSell,uint256 LpFeeSell,uint256 busdDividendRewardsFeeSell) external onlyOwner() {
  ...
  _busdDividendRewardsFeeSell=busdDividendRewardsFeeSell;
          ...
          function SetBuyFee(uint256 marketingBuy,uint256 LpFeeBuy,uint256 busdDividendRewardsFeeBuy) external onlyOwner() {
