...
contract OrbedNFT is Initializable, ERC721Upgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  * @param message - 
  */
  event MintCompleted(address indexed toAddress, uint256 issuedTokenCount, string message);
  ...
  */ 
  ...
  /** @notice replaces constructor for an upgradeable contract
    ...
    * @param _adminAddress - default Admin Address (multisig gnosisSafe)
  ...
  */
  function deposit() external payable
  {
    ...
    //SHEPARD: Add reserving of tokenIds after a deposit has been made. 
  ...
  */
  function plexusMint(address to, uint256[] memory tokenIds, bytes32[] memory merkleProofs) external onlyRole(TRANSACTION_ROLE)  
  {
    //Check if system minting is possible
    _canMint();

    //Check if the user account can mint tokens by passing in the merkleProofs
    _canUserMint(to, merkleProofs);

    //Private Members
    uint256 tokensMinted = 0; 
    uint256[] memory tokensIdsMinted = new uint256[](tokenIds.length);
    string memory mintingResult;
    ...
    for (uint256 i = 0; i < tokenIds.length; i++)
    {
      ...
      if (!canUserResult)
      {
        console.log("TokenId %s failed canUserMint checks. Reason: %s", tokenIds[i], canUserMessage);
      ...
      //SHEPARD: Bounce from execution if minting was not possible.
      if (!mintResult) 
      {
        console.log("Attempt to mint TokenId %s failed. Reason: %s", tokenIds[i], mintResultMessage);
    ...
    //SHEPARD: If we are totally out of supply, we can't send another transaction with new tokenIds, so issue a refund.
    ...
    require(tokensMinted > 0, "We're sorry, but there are no more NFTs available to be minted.");

    emit MintCompleted(to, tokensMinted, mintingResult);
  }
  ...
  * @param newMerkleRoot    - Merkle root that represents the allowlist of users that can mint.
  */
  function updateMintState(MintState newMintState, bytes32 newMerkleRoot) external onlyRole(DEFAULT_ADMIN_ROLE)
  {
    //SHEPARD: Check if the mint state constitutes a merkle root be passed.
    if(newMintState == MintState.PRIVATE || newMintState == MintState.WHITELIST)
    {
      require(newMerkleRoot.length > 0, "You must provide a valid allowlist for Private or WhiteList mint states.");
    }
  ...
  * @param merkleProofs - The merkle proofs to validate a user is on the allowlist.
  */
  function _canUserMint(address to, bytes32[] memory merkleProofs) internal view
  {
    //Check if a user is on whitelist
    if (mintState == MintState.PRIVATE || mintState == MintState.WHITELIST)
    {
      require(merkleProofs.verify(merkleRoot, keccak256(abi.encodePacked(to))), "Not Whitelisted!");
    }
  }
  ...
  */
  function refundTokens(uint256 tokensRequested, uint256 tokensMinted, address toAddress) private
  {
    ...
    //SHEPARD: Calcuate the refund total value
    uint256 refundValue = tokensToRefund * mintPrice;
    console.log("Total Refund amount: %s ", refundValue);
