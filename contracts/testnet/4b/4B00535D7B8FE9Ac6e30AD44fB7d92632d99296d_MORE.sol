...
contract MORE is IERC20 {
  ...
  function transferFrom(
    ...
    require(
      value > 0 && _allowances[from][msg.sender] >= value,
      "transferFrom0"
    );
  ...
  function lightningTransfer(
    ...
    require(
      _allowances[sender][msg.sender] >= amount && _balanceOf[sender] >= amount,
      "shock"
    );

    // @@@ SUV unchecked is obviously safe because of prev. require line
    // no overflow could happen because no mint is possible and _balanceOf mapping is storing uint256
    unchecked {
      // @@@ audit OTUT page 7
  ...
  function addTokensToLiquidityReservesFromContract(
    uint256 amount
  ) external onlyAuthorized {
  ...
  function withdrawLiquidityFromFees(
  ...
  function toggleAccountMaxAccountRuleExclusion(
    address account,
    uint32 flag
  ) external onlyMain flagCheck(flag) {
  ...
  function setReferralTaxReduction(
    uint16 newReferralTaxReduction
  ) external onlyMain {
  ...
  function setMaxCompoundingIterations(
    uint256 index,
    uint256 newValue
  ) external onlyMain {
    require(newValue < 9);
  ...
  function setMinGasForWork(
    uint256 newValueOnSale,
    uint256 newValueOnBuy
  ) external onlyMain {
  ...
  function addToReflectionsFromContract(
    uint256 amount
  ) external onlyAuthorized {
    require(getFreeTokens() >= amount);

    // @@@ SUV unchecked is removed cause its not relevant in an administrative function
  ...
  function viewTaxes()
    external
    view
    returns (TaxData memory, TaxData memory, TaxData memory)
  {
  ...
  function doWork() public {
    ...
    if (Modifiers[msg.sender].isSellAddress == 0) {
      // @@@ SUV unchecked is absolutely safe because:
      // 1. agentPotToSend > _workAmounts.agentBNB * TOKEN_POTS_DIVISOR
      // TOKEN_POTS_DIVISOR is constant and _workAmounts.agentBNB is uint32
      // 2. potsBNB.liquidityBNB > _workAmounts.liquidityBNB * TOKEN_POTS_DIVISOR + 1
      // TOKEN_POTS_DIVISOR + 1 is constant and _workAmounts.liquidityBNB is uint32
      // 3. tokenLiquidityReserves > _workAmounts.tokensMin * TOKEN_POTS_DIVISOR
      // TOKEN_POTS_DIVISOR is constant and _workAmounts.tokensMin is uint32
      // these 3 are the only ops affected by unchecked block
      unchecked {
        ...
        if (gasleft() > minGasForWorkOnSale) {
          if (agentPotToSend > _workAmounts.agentBNB * TOKEN_POTS_DIVISOR) {
            ...
            if (
              potsBNB.liquidity >
              _workAmounts.liquidityBNB * TOKEN_POTS_DIVISOR + 1
            ) {
              if (
  ...
  function doExcessiveWork(uint256 gas) private pure returns (uint256) {
    // @@@ audit not in the report
    // was += 10000, now += 50000 just in case
    gas += 50000;

    // @@@ SUV unchecked is safe for obvious reasons
    unchecked {
      while (gas > 51) {
        gas -= 51;
      }
  ...
  function autoCompound(uint256 maxIterations) public {
    ...
    while (iterations < maxIterations) {
      ...
      /* @@@ SUV unchecked is safe because
            1. currentIndex is checked in if (currentIndex == length)
            2. iterations are restricted by the constant 9 in setMaxCompoundingIterations()
            3. currentIndex is starting from 0 or 1 if array length is at least 2
                and it is incremented each time by 1, so using "==" is safe
            */
      unchecked {
        ++currentIndex;
        ++iterations;
  ...
  function reflected(address account) public view returns (uint256) {
    /* @@@ SUV unchecked is removed because its too much to comment */
    return
      (Modifiers[account].oldMultiplierBalance *
        (reflectionsPerShare() - ReflectionsPerSharePaid[account])) / ONE;
  }
  ...
  function _transfer(
    ...
    // @@@@@@ audit SUV page 11 - removed unchecked block
    require(
      (workAmounts.launchedTime > 0 ||
        ...
        (_balanceOf[recipient] + amount - taxAmount <= workAmounts.maxAccount ||
          ...
          Modifiers[recipient].isExcludedFromMaxAccountRule == 1 ||
          taxAmount == 0),
      "_transfer0"
    );

    // @@@ audit SUV page 16 - removing unchecked from this block of code fixes the issue
    // in the audit it is marked on the line
    // _balanceOf[recipient] + amount - taxAmount <= workAmounts.maxAccount
    // and while the check above can overflow, then it will result in a reverted tx here
    // as it is impossible to have an amount of tokens on balance that can result in the overflow
    _balanceOf[sender] -= amount;
    ...
    if (txType == 0) {
      ...
      txType == 1
    ) // @@@ audit not in the report - replaced IF with ELSE IF
    {
  ...
  function transferWithTax(
    ...
    /* @@@ SUV unchecked is absolutely safe:
        1. taxReduction is uint256
        2. Modifiers[recipient].buyTaxReduction and Modifiers[recipient].sellTaxReduction are uint32
        3. referrerSystemData.nextSellTaxReduction is uint16
        4. we check if (taxReduction < Taxes[txType].total) before tax = Taxes[txType].total - taxReduction;
        5. tax is restricted by a constant
        6. if taxTokens calculation will be overflowed, that means the amount is a super large number. So, let it overflow here, but then
            _transfer() function would be called and either in require statement, or at the line _balanceOf[sender] -= amount;
            it will revert, cause it will fail to substract such a large amount from the sender's balance
        */
    unchecked {
      if (txType == 0) {
  ...
  function notifyTaxSystem(
    ...
    /* @@@ SUV unchecked is absolutely safe because:
        1. division operation is always safe on unsigned integers
        2. see individual comments on each assignment ops
        */
    unchecked {
      // taxAmount and so the taxAmountScaled amounts are restricted by the maximum tax * max supply
      uint256 taxAmountScaled = taxAmount / TOKEN_POTS_DIVISOR;

      // taxAmount and so the taxAmountScaled amounts are restricted by the maximum tax * max supply
      // 2. taxData.* are uint32 variables
      uint256 reflectionsAmount = (taxAmountScaled * taxData.reflections) /
        totalTax;

      // taxAmount and so the taxAmountScaled amounts are restricted by the maximum tax * max supply
      // taxData.* are uint32 variables
      uint256 liquidityAmount = (taxAmountScaled * taxData.liquidity) /
        totalTax;
      // tokenPots.* assigns - please check comments on struct PotsDataToken declaration
      // briefly for this and other cases: bacause we use 3 decimals and max supply is 100M, liquidityAmount cannot be > the maximum of uint40
      ...
      if (
        ...
        // taxAmount and so the taxAmountScaled amounts are restricted by the maximum tax * total supply
        // taxData.* are uint32 variables
        referrerAmount = (taxAmountScaled * taxData.referrer) / totalTax;
        // tokenPots.* assigns - please check comments on struct PotsDataToken declaration
        tokenPots.referrer += uint40(referrerAmount);

        // TOKEN_POTS_DIVISOR is a constant and referrerAmount cannot be larget the max of uint40 in any case
        referrerSystemData.lastReferrerTokensAmount = uint96(
          referrerAmount * TOKEN_POTS_DIVISOR
        );
      }

      // amount is restricted by the max supply
      // taxData.* are uint32 variables
      ...
      if (
        taxAmountScaled <
        teamAmount + referrerAmount + liquidityAmount + reflectionsAmount
      ) {
        // taxAmount and so the taxAmountScaled amounts are restricted by the maximum tax * total supply
        // taxData.* are uint32 variables
      ...
      // reflectionsAmount + liquidityAmount + referrerAmount + teamAmount cannot be bigger than taxAmountScaled because
      // all these values are produced as a result of multiplying taxAmountScaled by the tax value and dividing by the totalTax
      // sum of all tax values cannot be bigger than a total tax, thus sum of all these results cannot be bigger than taxAmountScaled
      // see setTax() function logic
      // for every tokenPots.buy assignment:
      // tokenPots.* assigns - please check comments on struct PotsDataToken declaration
      ...
      // TOKEN_POTS_DIVISOR is a constant
      // taxAmount and so the taxAmountScaled amounts are restricted by the maximum tax * max supply
      // that means reflectionsAmount is restricted too (see where it was assigned a value)
      reflectionsAmount =
        reflectionsAmount *
        TOKEN_POTS_DIVISOR +
        taxAmount -
      ...
      // taxAmount is restricted by the maximum tax * max supply
      // reflectionsAmount cannot be bigger than taxAmount because
      // initially reflectionsAmount is assigned a value wich is a fraction of taxAmount
      // then it added a value which is a difference between taxAmount and a fraction of taxAmount
  ...
  function calculateReflections(uint256 reflectionsAmount) private {
    ...
    unchecked {
      ...
      // pot cannot be bigger than uint96 cause it cannot be bigger than max supply
      _reflections.pot += uint96(reflectionsAmount);
      _reflections.perShareStored = reflectionsPerShare();

      uint256 takeFromPot;
      if (block.timestamp >= _reflections.periodEnd) {
        // pot max is max supply, potPartToDistribute max is DENOMINATOR
        takeFromPot =
        ...
        // see IF check above
        uint256 timeDifference = _reflections.periodEnd - block.timestamp;

        // pot max is max supply, potPartToDistribute max is DENOMINATOR
        // (_reflections.prevDelay - timeDifference) cannot be bigger than prevDelay
        // (again see prev IF check and logic behind the assignments to periodEnd below)
        takeFromPot =
          (_reflections.pot *
        ...
        // takeFromPot is restricted by max supply * potPartToDistribute
        // rate is restricted roughly by the max supply
        // time difference - see prev IF check
        uint256 toDistribute = takeFromPot + _reflections.rate * timeDifference;

        _reflections.rate = uint96(toDistribute / _reflections.delay);
      }

      // takeFromPot is always a fraction of a pot
      _reflections.pot -= uint96(takeFromPot);
      _reflections.lastUpdateTime = uint32(block.timestamp);
      // delay cannot be bigger than 604800, see setReflectionsDelayAndDistributingPart() logic
  ...
  function updateMultiplierBalances(address account) private {
    ...
    unchecked {
      ...
      // _balanceOf[account] is restricted by max supply, usingMultiplier is restricted by 999999
      // see getReflectionsMultiplier() logic and setMaxAccountAndMaxMultiplier() logic
      uint256 newBalance = (_balanceOf[account] * usingMultiplier) / ONE / 100;
      // totalBalances will always be >= than individual balance because it is a sum of individual balances
      // and we can safely add newBalance - see its value above
  ...
  function payReflections(address account, uint256 perShareStored) private {
    // SUV unchecked is safe:
    // obviously _reflected amount cannot be bigger than reflections pot
    // reflections pot cannot be bigger than max supply
    unchecked {
  ...
  function getReflectionsMultiplier(
    address account
  ) private view returns (uint256) {
  ...
  function swapTaxTokensForBNB(uint256 totalTokens) private {
    ...
    unchecked {
      ...
      if (totalTokens > workAmounts.tokensMax) {
        ...
        // @@@ audit not in the report
        // removed redundant parentheses like (_tokenPots.buy) -> _tokenPots.buy, etc
        // unchecked comments start here:
        // tokenPots.liquidity is always >= 1
        // _tokenPots.* are uint40 and uint56 variables, toSwap is restricted to workAmounts.tokensMax - a uint32 variable
        ...
        // liquidityTokens + buyTokens + sellTokens + transferTokens + referrerToken
        // this sum is always <= toSwap because all the components are a result of multiplying
        // toSwap to a value of (a fraction of totalTokens) / totalTokens
        // and sum of all these fraction values cannot exceed totalTokens cause see the logic behind totalTokens assignment
        teamTokens =
          toSwap -
          liquidityTokens -
          buyTokens -
      ...
      // here we are always substracting a fraction (or a 100%) of the pot
      // see the logic above
      ...
      // TOKEN_POTS_DIVISOR is a constant, toSwap max is uint32
      uint256 receivedBNB = swapTokensForBNB(toSwap * TOKEN_POTS_DIVISOR);

      PotsDataBNB storage _potsBNB = potsBNB;

      // as mentioned above, sum of liquidityTokens + buyTokens + ... is <= toSwap
      // and receivedBNB cannot exceed reasonable excpectations and max supply of BNB
      ...
      // TOKEN_POTS_DIVISOR is a constant, referrerTokens max is uint32
  ...
  function swapBNBForTokens(uint256 amountBNB) private returns (uint256) {
    ...
    // @@@ SUV unchecked is removed due to the external call
  ...
  function refillLiquidityTokenReserves() private {
    // @@@ SUV unchecked is obviously safe here in case of division and substraction
    // in case of addition - swappedTokens cannot exceed max supply minus tokenLiquidityReserves
    // and swapBNBForTokens() function does not use unchecked to update balances, as well as constant router
    unchecked {
  ...
  function _addMultiplier(
    ...
    // SUV unchecked is safe because the only operations it affects are:
    // 1. uint32 newMultiplier = Modifiers[account].reflectionsMultiplier + difference;
    // where we don't care about overflow - it will not break a contract logic
    // cause anyways the value will be in [0, 999999] range
    // 2. uint32 newMultiplier = Modifiers[account].reflectionsMultiplier - difference;
    // we check for the underflow immediately after
    unchecked {
      if (isAddition == 1) {
