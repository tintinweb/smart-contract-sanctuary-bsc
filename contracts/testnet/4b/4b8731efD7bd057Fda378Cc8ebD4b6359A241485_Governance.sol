...
contract Governance is Admin{
    ...
    Stake public stake;
    address public burn;
    constructor(address _governanceToken, address _stake, address _burn) Admin(1000000000000000000000,10000000000000000000000,1209600,604800,100,334,500) public{
    ...
    struct DepositByAddress {
        uint256 proposalID;
    }
    mapping(address => DepositByAddress[]) public depositByAddress;
    struct VoteByAddress {
        uint256 proposalID; 
    }
    uint256 proposalNumber = 0;
    mapping(address => VoteByAddress[]) public voteByAddress;
    ...
    // mapping(uint256 => bool) public proposalid;
    ...
    function createProposal(string memory _title, string memory _description) external{
        ...
        depositByAddress[msg.sender].push(DepositByAddress({
    ...
    function executeProposal(uint256 _proposalID) external onlyValidator() returns (uint256){
        ...
        require(proposalexecute.status != 0, "ProposalID none available");
    ...
    function deposit(uint256 _proposalID, uint256 _amount) external {
        ...
        if(mapDeposits[msg.sender][_proposalID] == 0){
            depositByAddress[msg.sender].push(DepositByAddress({
    ...
    function vote(uint256 _proposalID, uint256 _amount, uint256 _vote) external {
        ...
        if(mapVotes[msg.sender][_proposalID] == 0){
            voteByAddress[msg.sender].push(VoteByAddress({
                proposalID: _proposalID
            }));
        }
