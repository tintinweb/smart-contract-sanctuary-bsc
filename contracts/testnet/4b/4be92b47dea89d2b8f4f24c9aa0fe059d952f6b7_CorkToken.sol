...
contract CorkToken is BEP20("WINERY TOKEN CORK", "CORK") {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (Winery).
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "CORK::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "CORK::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "CORK::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "CORK::_writeCheckpoint: block number exceeds 32 bits"
        );
