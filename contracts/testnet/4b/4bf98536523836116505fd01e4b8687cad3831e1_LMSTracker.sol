...
contract LMSTracker is Ownable {
    ...
    constructor(){
        tokenContract = TokenContract(0xB2942AC82B4c13C049e258D5C7f1a67da2F1d491);
    ...
    function checkFriday() public {
        if (block.timestamp >= lastFriday + 5 minutes) {  //! test

            _closeWeek(gasForClose);

            if(weeklyTrackers[week].isClosed == true) {
                lastFriday += 5 minutes; //! test
                address newToken = checkNextLMSToken();
                if (newToken != address(0)){
                    weeklyTrackers[week+1].lmsToken = IERC20(newToken);
                    tokenContract.activateNewLmsToken(newToken);
                } else {
                    weeklyTrackers[week+1].lmsToken = weeklyTrackers[week].lmsToken;
                }
                week++;
            }
        }
