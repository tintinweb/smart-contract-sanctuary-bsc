...
contract Watch_Ai is ERC20, Ownable {
    ...
    address public marketingAndFutureDevelopmentOfWatch_AiAddress = 0x000000000000000000000000000000000000dEaD;    
    
    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address private marketingWallet = 0xFb4F51723B3F7499B5f9eea6e5317df723BBCCBa;
    address private devWallet = 0xFb4F51723B3F7499B5f9eea6e5317df723BBCCBa;

    uint256 public swapTokensAtAmount = 2e5 * 10**9;
    uint256 public maxWalletBalance = 1e6 * 10**9;
    uint256 public maxBuyAmount = 1e6 * 10**9;
    uint256 public maxSellAmount = 1e6 * 10**9;
    ...
    Taxes public buyTaxes = Taxes(1, 4, 0, 0);
    Taxes public sellTaxes = Taxes(1, 4, 0, 0);
    ...
    function setBuyTaxes(uint256 _rewards, uint256 _marketing, uint256 _dev, uint256 _liquidity) external onlyOwner{
        buyTaxes = Taxes(_rewards, _marketing, _dev, _liquidity);
        require((_rewards + _marketing + _dev + _liquidity ) <= 12, "Must keep fees at 12% or less");
    }

    function setSellTaxes(uint256 _rewards, uint256 _marketing, uint256 _dev, uint256 _liquidity) external onlyOwner{
        sellTaxes = Taxes(_rewards, _marketing, _dev, _liquidity);
        require((_rewards + _marketing + _dev + _liquidity ) <= 30, "Must keep fees at 30% or less");
    }
    ...
    function setAntiBotBlocks(uint256 numberOfBlocks) external onlyOwner{
        ...
        require(numberOfBlocks < 8,"Deadline should be less than 5 Blocks");
    ...
    function setMinBalanceForDividends(uint256 amount) external onlyOwner {
     ...
     function Blacklist_status(bool _status) external onlyOwner {
        blacklistMode = _status;
    }
    
    function Blacklist_Snipers(address[] calldata addresses, bool status) external onlyOwner {
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(
            ...
            "Watch_Ai: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[newPair] = value;
