 ...
 */
    ...
    function purchaseBYSL(address givenToken,uint amount) external {
        ...
        console.log("total reward",(amount *1/10));
        // (uint BUSDAmount, uint leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution(msg.sender, (amount *1/10) - (amount * 1/100));
        (uint BUSDAmount, uint leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution(msg.sender, (amount *1/10) - (amount * 1/100),amount);
        if(amount != 0){
            console.log("rebate",amount * 1/100);
            IERC20(Admin.BUSD()).transfer(msg.sender,amount * 1/100);
            console.log("referral",BUSDAmount);
            IERC20(Admin.BUSD()).transfer(Admin.Refferal(),BUSDAmount);
        }
        if(leftAmount != 0){
            IERC20(Admin.BUSD()).transfer(Admin.TeamAddress(),leftAmount);
        }
        console.log(IERC20(Admin.BUSD()).balanceOf(address(this)));
        console.log((amount *9/20),"(amount *9/20)");
