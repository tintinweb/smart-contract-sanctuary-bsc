...
contract METDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("MoonexchangeToken_Dividend_Tracker", "MoonexchangeToken_Dividend_Tracker") {
        claimWait = 604800;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 1000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MoonexchangeToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MoonexchangeToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MoonexchangeToken contract.");
    }
    ...
    {
        require(newClaimWait >= 3600 && newClaimWait <= 864000, "MoonexchangeToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");   
        require(newClaimWait != claimWait, "MoonexchangeToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract MoonexchangeToken is BEP20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    uint256 public _initialSupply = 1000_000_000 * (10**18);
    uint256 public maxSellTransactionAmount = _initialSupply.div(100); //1% of total supply. 
    uint256 public swapTokensAtAmount = _initialSupply.div(100_000);
    ...
    function updateAllFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _burnFee) 
    public onlyOwner 
    {
        ...
        require(totalFees+burnFee<=15, "Too High Fee");
    ...
    public onlyOwner 
    {
        ...
        require(maxSellTransactionAmount>_initialSupply.div(1000), "Too less limit");
    }

    constructor() BEP20("Moonexchange token", "M.E.T") 
    {
        BNBRewardsFee = 3;
        liquidityFee = 1;
        marketingFee = 2;
    ...
    {
        require(newAddress != address(uniswapV2Router), "MoonexchangeToken: The router already has that address");
    ...
    {
        require(_isExcludedFromFees[account] != excluded, "MoonexchangeToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MoonexchangeToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MoonexchangeToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MoonexchangeToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MoonexchangeToken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) open(from, to) internal override 
    {
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) 
        {
            ...
            super._transfer(from, deadWallet, burnTokens); // burn tokens
        }
