    ...
    Pausable
{
    ...
    /// user => contract => tokenId => amountReserved
    mapping(address => mapping(address => mapping(uint256 => uint256))) public reservedTokens;

    /// @dev supported currency
    address public supportedCurrency;
    ...
    modifier onlyListingCreatorOrOperator(uint256 _listingId) {
        require(listings[_listingId].tokenOwner == _msgSender() || hasRole(OPERATOR_ROLE, _msgSender()), "!OWNER");
        _;
    }
    ...
    function getActiveListings() external view returns (uint[] memory){
        uint counter = 0;
        uint[] memory active = new uint[](totalListings);

        for (uint i = 0; i < totalListings; i++) {
            if (listings[i].isActive) {
                active[counter] = listings[i].listingId;
                counter++;
            }
        }
    ...
    function createListing(ListingParameters memory _params) external override whenNotPaused {
        // Get values to populate `Listing`.
        uint256 listingId = totalListings;
        totalListings += 1;

        address tokenOwner = _msgSender();
        validateTokenSupported(_params.assetContract);

        require(_params.quantityToList > 0, "QUANTITY");
        if (!hasRole(OPERATOR_ROLE, msg.sender)) {
            require(hasRole(ASSET_ROLE, _params.assetContract), "!ASSET");
        }
    ...
    function updateListing(
        ...
        bool _isActive
    ) external override whenNotPaused onlyListingCreator(_listingId) {
        Listing memory targetListing = listings[_listingId];

        require(_quantityToList != 0, "QUANTITY");
    ...
    function cancelDirectListing(uint256 _listingId) external override whenNotPaused onlyListingCreatorOrOperator(_listingId) {
        Listing memory targetListing = listings[_listingId];
    ...
    function buy(
    ...
    ) external override whenNotPaused nonReentrant onlyExistingListing(_listingId) {
        Listing memory targetListing = listings[_listingId];
        ...
        executeSale(
            ...
            targetListing.pricePerToken * _quantityToBuy,
            _quantityToBuy
        );
    }
    ...
    function executeSale(
        ...
        payout(_payer, _targetListing.tokenOwner, _currency, _currencyAmountToTransfer);
    ...
    function validateDirectListingSale(
        ...
        require(_listing.isActive, "not active.");
    ...
    function validateTokenSupported(address _assetContract) internal view returns (bool) {
        if (IERC165(_assetContract).supportsInterface(type(IERC1155).interfaceId)) {
            return true;
        } else {
            revert("token must be ERC1155");
        }
    }

    /// @dev Returns the platform fee recipient and bps.
    function getPlatformFeeInfo() external override view returns (address, uint16) {
...
interface IMarketplace is IPlatformFee {
     ...
     *  @param pricePerToken   For direct listings: interpreted as 'price per token' listed. For auctions: if
     *                               `pricePerToken` is greater than 0, and a bidder's bid is at least as great as
     *                               `pricePerToken * quantityToList`, the bidder wins the auction, and the auction
     *                               is closed.
     *
    **/
     ...
     *  @param _isActive            'inactive' means NFTs cannot be bought from the listing.
    ...
    **/
     ...
     *
     *  @param _pricePerToken  For direct listings: interpreted as 'price per token' listed. For auctions: if
     *                               `pricePerToken` is greater than 0, and a bidder's bid is at least as great as
     *                               `pricePerToken * quantityToList`, the bidder wins the auction, and the auction
     *                               is closed.
     *
