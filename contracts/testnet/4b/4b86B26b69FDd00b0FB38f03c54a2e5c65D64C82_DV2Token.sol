...
contract DV2Token is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    function updateMultiplier(address from) public returns (bool allow) {
      ...
      if(isMultiEnabled){
        if(buyprice <= currentBasePrice && sellprice <= currentBasePrice){
					rebaseswitch = false;
    ...
    // Set Excluded From Fees users
    function readPower() public view returns(uint256 mul1,uint256 mul2,uint256 mul3,bool _status1,bool _status2) {
        uint256 buyprice = buyPriceNow();
        uint256 sellprice = sellPriceNow();
        uint256 currentBasePrice = checkMultiplier().mul(basePrice);
        uint256 currentSellTargetPrice = checkMultiplier().mul(sellTargetPrice);
