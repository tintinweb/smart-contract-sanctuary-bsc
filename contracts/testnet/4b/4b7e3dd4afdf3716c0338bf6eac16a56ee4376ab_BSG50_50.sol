...
contract  BSG50_50 is Ownable, SignVerify, Pausable, ReentrancyGuard{
    ...
    // uint256 private constant dayRewardPercents = 1250000000000000000000;
    ...
    mapping(address => CTOInfo) public ctoInfo;
    ...
    function _distributeCTOPOol() public {
        ...
        if(CTOPoolCount > 0)
        {
            ...
            for(uint256 i = 0; i < CTOPooluser.length; i++){
                    ctoInfo[CTOPooluser[i]].CTO = ctoInfo[CTOPooluser[i]].CTO.add(reward);
    ...
    function getRewards(address _user) public view returns(uint256,uint256,uint256){
        ...
        uint256 FTMamt = (withdrawAmt.mul(BNBperBUSD())).div(1e18);
    ...
    function _calCurDynamicRewards(address _user) public view returns(uint256, uint256) {
        RewardInfo storage userRewards = rewardInfo[_user];
        uint256 totalRewards = userRewards.directs.add(userRewards.level2to5).add(userRewards.level6to10);
    ...
    function _calLevelNow(address _user) public view returns(uint256) {
        ...
        {
            (uint256 maxTeam, uint256 otherTeam, ) = getTeamDeposit(_user);
            if(total >= 2000e18 && user.teamNum >= 15 && maxTeam >= 2500e18 && otherTeam >= 2500e18)
            {   levelNow = 5;   } // PLATINUM

            else if(total >= 1000e18 && user.teamNum >= 10 && maxTeam >= 2000e18 && otherTeam >= 2000e18)
            {   levelNow = 4;   } // GOLD

            else if(total >= 500e18 && user.teamNum >= 5 && maxTeam >= 1000e18 && otherTeam >= 1000e18)
            {   levelNow = 3;   } // SILVER
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        if(!isUnfreezeCapital){ 
            ...
            if(userReward.level6to10 > 0){
                uint256 release = _amount;
                if(_amount >= userReward.level6to10){
                    release = userReward.level6to10;
                }
                userReward.level6to10 = userReward.level6to10.sub(release);
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                ...
                if(i > 9)
                {
                    ...
                    {
                        reward = newAmount.mul(level11to20Percents[i - 10]).div(baseDivider);
                        upRewards.level11to20 = upRewards.level11to20.add(reward);
                    }
                }
                if(i > 4)
                {
                    ...
                    {
                        reward = newAmount.mul(level6to10Percents[i - 5]).div(baseDivider);
                        upRewards.level6to10 = upRewards.level6to10.add(reward);
                    }
                }
                else if(i > 0)
                {
                    ...
                    {
                        reward = newAmount.mul(level2to5Percents[i - 1]).div(baseDivider);
