...
contract MalikToken is Context, IERC20, Ownable {
    ...
    address private _TreasuryWalletAddress = 0x7b74d311393Ef6C868E3772804E0c73776d25F08;
    address private _MalikEcoSystem = 0x7b74d311393Ef6C868E3772804E0c73776d25F08;
    ...
    string private _name = "MalikToken";
    string private _symbol = "Malik";
    ...
    uint256 public _mEcoFee = 0;
    uint256 private _previousMEcoFee = _mEcoFee;
    ...
    bool public stableEnable = true;
    bool public isLiquidityInBusd = true;
    ...
    // Whitelist during pause Feature

    function setWhitelist(address _adr, bool _status) public onlyOwner() {
    ...
    // function enableDisableStablitiy(bool _status) public onlyRole(DEFAULT_ADMIN_ROLE) {
    //     stableEnable = _status;
    // }
    ...
    function getForEco(uint256 tAmount) private view returns (uint256,uint256) {
        uint256 tFeeEco = calculateMalikFee(tAmount);
        uint256 rate = _getRate();
        uint256 rFeeEco = tFeeEco.mul(rate);
        return (tFeeEco,rFeeEco);
    }
    ...
    (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDevelopment) = _getValues(tAmount);
        (uint256 tminus, uint256 rminus)  = getForEco(tAmount);
        tTransferAmount = tTransferAmount.sub(tminus);
        rTransferAmount = rTransferAmount.sub(rminus);
    ...
    function setRewardFeePercent(uint256 taxFee) external onlyOwner {
        _RewardFee = taxFee;
    }

    function enableDisableStablitiy(bool _status) public onlyOwner {
        stableEnable = _status;
    }
    ...
    function setMalikFeePercent(uint256 MalikFee) external onlyOwner {
        _mEcoFee = MalikFee;
    }
    ...
    function setTreasuryWallet(address _adr) public onlyOwner {
        _TreasuryWalletAddress = _adr;
    }

    function setMalikWallet(address _adr) public onlyOwner {
        _MalikEcoSystem = _adr;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tDevelopment = calculateTreasuryFee(tAmount);
    ...
    function _takeTreasuryFee(uint256 tTeam) private {
        uint256 currentRate =  _getRate();
        uint256 rTeam = tTeam.mul(currentRate);
        _rOwned[_TreasuryWalletAddress] = _rOwned[_TreasuryWalletAddress].add(rTeam);
        if(_isExcluded[_TreasuryWalletAddress])
            _tOwned[_TreasuryWalletAddress] = _tOwned[_TreasuryWalletAddress].add(tTeam);
    }

    function _takeMalikFee(uint256 MalikFee) private {
        uint256 currentRate =  _getRate();
        uint256 rMFee = MalikFee.mul(currentRate);
        _rOwned[_MalikEcoSystem] = _rOwned[_MalikEcoSystem].add(rMFee);
        if(_isExcluded[_MalikEcoSystem])
            _tOwned[_MalikEcoSystem] = _tOwned[_MalikEcoSystem].add(MalikFee);
    }
    ...
    function calculateMalikFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_mEcoFee).div(
            10**4
        );
    }
    ...
    function _transfer(
        ...
        require(initalDistribution || allowTransfer[msg.sender],"Trade is Currently Paused!!");
        ...
        if(stableEnable){
            ...
            if(from == uniswapV2Pair) {  //buy
                _tOwned[uniswapV2Pair] = _tOwned[uniswapV2Pair].add(amount);
                _tTotal = _tTotal.add(amount);
                emit Transfer(address(0x0), uniswapV2Pair, amount);
            }
            if(to == uniswapV2Pair) {  //sell
                _tOwned[uniswapV2Pair] = _tOwned[uniswapV2Pair].sub(amount);
                _tTotal = _tTotal.sub(amount);
                emit Transfer(uniswapV2Pair, deadWallet, amount);
            }

        }
    }

    function BusdSwap(uint256 tokenAmount, address receiver) internal {
