...
contract TestReferralSystem is Ownable, PlatformOwnable {
    ...
    modifier onlyNewUser() {
        ...
        require(
            !isMaster(_msgSender()),
            "master address cannot register as member"
        );
        require(!isMember(_msgSender()), "sender is already a member");
        _;
    }
    ...
    modifier onlyMember(address _user) {
        require(isMember(_user), "sender not a member");
        _;
    }
    ...
    function getSubDistributions(
        ...
        address _buyer,
        uint256 _total
    ) external view onlyMember(_buyer) returns (IDistribution.Sub memory) {
    ...
    function getInitialDistributions(
        ...
        address _buyer,
        uint256 _total
    ) external view onlyMember(_buyer) returns (IDistribution.Initial memory) {
        require(_creator != address(0), "invalid creator address");
        require(_buyer != address(0), "invalid buyer address");
        require(_total > 0, "total value cannot be 0");
    ...
    function getKolDistributions(
        ...
        address _buyer,
        uint256 _total
    ) external view onlyMember(_buyer) returns (IDistribution.KOL memory) {
        require(isMember(_buyer), "sender not a member");
        require(_creator != address(0), "invalid creator address");
        require(_buyer != address(0), "invalid buyer address");
        require(_total > 0, "total value cannot be 0");
