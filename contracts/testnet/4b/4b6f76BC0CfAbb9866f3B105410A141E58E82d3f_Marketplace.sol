...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    function mintMarketItem(
        ...
        require(_mspcPrice > 0, "Price must be more than one");
        require(_usdtPrice > 0, "Price must be more than one");

        _itemIds.increment(); // start from 1
    ...
    function purchaseMarketItemByUsdt(address _nftContract, uint256 _itemId)
        public
        payable
        nonReentrant
    {
        ...
        require(usdt.balanceOf(msg.sender) >= price, "Check your balance");
    ...
    function purchaseMarketItemByMspc(address _nftContract, uint256 _itemId)
        public
        payable
        nonReentrant
    {
        ...
        uint256 price = idToMarketItem[_itemId].mspcPrice;
        require(msg.value == price, "Not enough token");

        uint256 allowance = mspc.allowance(msg.sender, address(this));
        require(allowance >= price, "Insufficient allowance");
        require(mspc.balanceOf(msg.sender) >= price, "Check your balance");
    ...
    function setMspcRate(uint256 _mspcRate) external onlyOwner {
        mspcRate = _mspcRate;
    }
