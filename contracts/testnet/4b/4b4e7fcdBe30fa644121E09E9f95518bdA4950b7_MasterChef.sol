...
contract MasterChef is ReentrancyGuard {
    ...
    *         utilizationRatio the current utilization ratio,
    ...
    **/
        ...
        )
    {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            ...
            maxBorrowingPowerInUSD += assetPriceInUSD * balance * getAssetDebtRatioThreshold(assets[i]) / (10**6 * assetUnit);
        }

        borrowingPowerInUSD = (maxBorrowingPowerInUSD > totalBorrowBalanceInUSD) ? maxBorrowingPowerInUSD - totalBorrowBalanceInUSD : 0;
        utilizationRatio = totalBorrowBalanceInUSD == 0 ? 0 : totalBalanceInUSD * 10**6 / totalBorrowBalanceInUSD;
        utilizationLimit = maxBorrowingPowerInUSD == 0 ? 0 : totalBalanceInUSD * 10**6 / maxBorrowingPowerInUSD;
    }
    ...
    **/
    function liquidation(
        ...
        nonReentrant
    {
        (uint256 totalBalanceInUSD, , uint256 borrowingPowerInUSD, , ) = getUserTotalBalances(_user);
        require(totalBalanceInUSD > 0, "User has no balance");
        require(borrowingPowerInUSD == 0, "Liquidation threshold not reached");
