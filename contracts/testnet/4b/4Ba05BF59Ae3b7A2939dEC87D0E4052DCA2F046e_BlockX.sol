...
contract BlockX is ERC20, ERC20Burnable, Ownable {
    ...
    function multipleLockAndTransfer(
    ...
    function getNumberOfVestingAgreement(address walletAddress)
        public
        view
        returns (uint256)
    {
    ...
    function getLocked(address walletAddress) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < vestingAgreements[walletAddress].length; i++) {
            lockedAmount += getLockedPerAgreement(walletAddress, i);
        }
        return lockedAmount;
    }

    function getLockedPerAgreement(address walletAddress, uint256 index)
        public
        view
        returns (uint256)
    {
        ...
        VestingAgreement memory currentVest = vestingAgreements[walletAddress][
            index
        ];
        unLockedAmount = 0;
        ...
        if (block.timestamp > currentVest.vestStart) {
            if (
                ((block.timestamp - currentVest.vestStart) / 3600) <=
                currentVest.vestPeriod
            ) {
                unLockedAmount =
                    (currentVest.totalAmount *
                        ((block.timestamp - currentVest.vestStart) / 3600)) /
                    currentVest.vestPeriod;
    ...
    */
    function lockAddresses(
        address[] memory addressesToLock,
    ...
    */
    ...
    function isLocked(address user) public view returns (bool) {
        ...
        if (_releaseTime > 0) {
            if (block.timestamp < _releaseTime) return true;
        }
        return false;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            !isLocked(from),
            "Sender is not allowed to send the tokens yet"
        );
        uint256 lockedAmount = 0;
        ...
        for (uint256 i = 0; i < vestingAgreements[from].length; i++) {
