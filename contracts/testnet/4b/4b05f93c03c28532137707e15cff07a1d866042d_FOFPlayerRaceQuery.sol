...
contract FOFPlayerRaceQuery is Ownable {
    ...
    function checkCanSignUpRank(address user, uint256 vehicleId) public view returns(uint256) {
        ...
        if(FPRD.getRaceVehicles(1, FPRD.raceIds(1)).length >= 7) return 5;//"Registration for this race is full"

        if(getRaceVehiclesIndex(1, FPRD.raceIds(1), vehicleId) > 0) return 6;//"You have already signed up for this race"
    ...
    function checkCanSignUp(address user, uint256 raceType,uint256 vehicleId) public view returns(uint256) {
        ...
        if(FPRD.getRaceVehicles(raceType, FPRD.raceIds(raceType)).length >= 10) return 6;//"Registration for this race is full"

        if(getRaceVehiclesIndex(raceType, FPRD.raceIds(raceType), vehicleId) > 0) return 7;//"You have already signed up for this race"
