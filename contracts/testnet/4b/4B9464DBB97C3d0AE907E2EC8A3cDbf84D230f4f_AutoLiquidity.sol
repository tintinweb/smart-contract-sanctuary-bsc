...
/*
    ...
    - 2% Dev
    - 8% Marketing
    - 2% Angpao (Will be announced in group the winners)
    = 14% Total Tax
...
contract AutoLiquidity is IERC20, Ownable {
    using SafeMath for uint256;

    address constant mainnetRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address constant testnetRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    address constant WBNB          = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address constant DEAD          = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO          = 0x0000000000000000000000000000000000000000;

    string constant _name = "Gong Xi Fa Cai";
    string constant _symbol = "Angpao";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 888888 * (10 ** _decimals);
    uint256 public _transferLimit;
    ...
    constructor (address mw, address dw) {
        address deployer = msg.sender;
        router = IDEXRouter(testnetRouter);
        pair = IDEXFactory(router.factory()).createPair(WBNB, address(this));
        _allowances[address(this)][address(router)] = type(uint256).max;

        _transferLimit = _totalSupply / 20; // 5%    
        _maxWalletSize = _totalSupply / 20; // 5%

        isTxLimitExempt[address(router)] = true;
        isTxLimitExempt[deployer] = true;
        isTxLimitExempt[0x7ee058420e5937496F5a2096f04caA7721cF70cc] = true; // Pink lock
        isTxLimitExempt[mw] = true;
        isTxLimitExempt[dw] = true;

        isFeeExempt[deployer] = true;
        isFeeExempt[0x7ee058420e5937496F5a2096f04caA7721cF70cc] = true; // Pink lock
        isFeeExempt[mw] = true;
        isFeeExempt[dw] = true;

        autoLiquidityReceiver = deployer;
        giveawayFeeReceiver = mw;
        marketingFeeReceiver = mw;
        uint256 dist = _totalSupply / 100 * 3; // 3%

        _balances[deployer] = _totalSupply;
        _basicTransfer(deployer, dw, dist);
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (marketingFee, liquidityFee, giveawayFee, devFee, totalFee, feeDenominator);
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _giveawayFeeReceiver, address _devFeeReceiver) external onlyOwner {
