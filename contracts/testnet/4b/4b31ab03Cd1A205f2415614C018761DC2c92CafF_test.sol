...
contract test is IERC20, Context, Ownable, ReentrancyGuard {
    ...
    function _sell(uint256 tokenAmount, uint256 deadline, uint256 minBNBOut) public nonReentrant ensure(deadline) payable returns (bool) {
        ...
        uint256 amountBNB = liquidity.sub(liqConst/(_balances[address(this)]+tokensToSwap));
        ...
        emit Transfer(seller, address(this), tokensToSwap);
        return true;
    }
    ...
    address private usdcPairAddress = 0x8a1a4C578a8E7DE817D588195f2f89Ad0b591c9f;
    address private usdcAddress = 0x64544969ed7EBf5f083679233325356EbE738930;

    function changeUSDCPair(address newUSDCPair, address newUSDCAddress) external onlyOwner{
        usdcPairAddress = newUSDCPair;
        usdcAddress = newUSDCAddress;
    }

   // calculate price based on pair reserves
   function getBNBPrice() private view returns(uint)
   {
    IPancakePair pair = IPancakePair(usdcPairAddress);
    IERC20 token1 = pair.token0() == usdcPairAddress? IERC20(pair.token1()):IERC20(pair.token0()); 
    
    (uint Res0, uint Res1,) = pair.getReserves();

    if(pair.token0() != usdcPairAddress){(Res1,Res0,) = pair.getReserves();}
