...
contract BASWithdraw is ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(address basAddress, address validator) {
        BAS = IERC20(basAddress);
    ...
    function withdrawBAS(Order memory order, bytes memory signature) external whenNotPaused nonReentrant {
        ...
        require(order.tokenAddress == address(BAS), "not BAS address");
        require(order.account == msg.sender, "account is not matched");
        bytes32 orderHash = keccak256(abi.encodePacked(order.orderId, order.account, order.tokenAddress, order.amounts, order.salt, order.endTimestamp));
        require(isSignatureValid(signature, orderHash, _validator), "signature error");

        withdrawAmounts[msg.sender] += order.amounts;
        _isOrderIdUsed[order.orderId] = true;

        BAS.safeTransfer(msg.sender, order.amounts);

        emit WithDrawBAS(msg.sender, order.orderId, order.amounts, block.timestamp);
    }
