...
contract OpenBox is Ownable {
    ...
    function randomHash(uint group, uint nonce, uint tokenId, bool isImport) internal {
        uint resultNumber;
        do {
    ...
    function handleOpen(uint nonce, uint tokenId, bool isImport) internal {
        uint resultNumber = random(nonce, 100);
        nonces[nonce] = true;
        if(resultNumber < 15) randomHash(0, nonce, tokenId, isImport);
        else if(resultNumber >= 15 && resultNumber < 53) randomHash(1, nonce, tokenId, isImport);
        else if(resultNumber >= 53 && resultNumber < 89) randomHash(2, nonce, tokenId, isImport);
        else randomHash(3, nonce, tokenId, isImport);
    }
    function open(uint nonce, uint[] memory tokenIds, uint[] memory amounts) public {
        ...
        for(uint i = 0; i < amounts.length; i++) {
            handleOpen(nonce+random(i, 1000), tokenId+1+i, false);
        }
    ...
    function openImport(uint nonce, uint[] memory tokenIds, uint[] memory amounts, uint _importRequestId) public {
        ...
        uint tokenId = IERC721(items721).currentTokenId();
        for(uint i = 0; i < amounts.length; i++) {
            handleOpen(nonce+random(i, 1000), tokenId+1+i, true);
        }
