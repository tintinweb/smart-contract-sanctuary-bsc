...
contract Ownable {
    ...
    constructor() {
        _ownerAdmin = msg.sender;
    }

    function ownerAdmin() public view returns (address) {
        return _ownerAdmin;
    }

    modifier onlyOwner() {
        require(isOwnerAdmin());
        _;
    }

    function isOwnerAdmin() public view returns (bool) {
        return msg.sender == _ownerAdmin;
    }

    function renounceOwnership() public onlyOwner {
        emit OwnershipRenounced(_ownerAdmin);
        _ownerAdmin = address(0);
    }
    ...
    function _transferOwnership(address newOwner) internal {
        ...
        emit OwnershipTransferred(_ownerAdmin, newOwner);
        _ownerAdmin = newOwner;
    }
}


pragma solidity =0.8.4;
interface IDadBodsLock {
    function lock(
        address owner,
        address token,
        bool isLpToken,
...
pragma solidity =0.8.4;
contract DadBodsLock0X is IDadBodsLock, Ownable {
    ...
    // ID padding from DadBodsLock v1, as there is a lack of a pausing mechanism
    ...
    function editLock(uint256 lockId, uint256 newAmount, uint256 newUnlockDate) external override validLock(lockId) {
        Lock storage userLock = _locks[_getActualIndex(lockId)];
        require(
            userLock.owner == msg.sender,
            "You are not the owner of this lock"
	...
	function editLockAdmin(uint256 lockId, uint256 newUnlockDate) external onlyOwner {
        Lock storage userLock = _locks[_getActualIndex(lockId)];
    ...
    function renounceLockOwnership(uint256 lockId) external {
        transferLockOwnership(lockId, _ownerAdmin);
    }
