...
contract EnglishAuction is Configurable, IERC721Receiver {
    ...
    function bid(
        ...
        // amount of token1
        uint amount1
    ) external payable lock isPoolExist(index) checkBotHolder(index) isPoolNotClosed(index) {
    ...
    function cancel(uint index) external isPoolExist(index) {
    ...
    function creatorClaim(uint index) external isPoolExist(index) isPoolClosed(index) {
        require(isCreator(msg.sender, index), "sender is not pool's creator");
        _creatorClaim(index);
    }
    ...
    function bidderClaim(uint index) external isPoolExist(index) isPoolClosed(index) {
