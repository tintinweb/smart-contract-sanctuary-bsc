...
contract XXXXXX is Context, Ownable , ReentrancyGuard {
    ...
    //uint256 public constant max = 20000 ether; mainnet
    uint256 public constant max = 20000 ether; //testnet
    uint256 public roi = 10;
    uint256 public constant fee = 10;
    //uint256 public constant withdraw_fee = 3;
    uint256 public constant mandatory_fee = 70;
    uint256 public constant ref_fee = 10;
    //address public dev = ; mainnet
    address public dev = 0x5Cf2a3a1F0BA843eBD8d27e058dA141522d04725; //testnet
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        uint256 depositAmount = SafeMath.sub(_amount, total_fee);

        if(!checkAlready()){
        uint256 ref_fee_add = refFee(depositAmount);
        if(_ref != address(0) && _ref != msg.sender) {
         ...
         refferal[_ref] = refferal_system(_ref,totalRefFee);
            ...
            total_fee = SafeMath.add(total_fee,ref_fee_add);   
        }

        // investment details
        uint256 userLastInvestment = investments[msg.sender].invested;
        uint256 userCurrentInvestment = depositAmount;
       ...
       // fees
        ...
        BusdInterface.transferFrom(msg.sender,address(this),depositAmount);
        }
        else {
    ...
    require(totalWithdraw[msg.sender].amount <= SafeMath.mul(investments[msg.sender].invested,3), "You cant withdraw you have collected three times Already"); // hh new
    ...
    uint256 aval_withdraw2 = SafeMath.div(SafeMath.mul(aval_withdraw,mandatory_fee),100); // divide the fees
    uint256 withdraw_amount = SafeMath.sub(aval_withdraw,aval_withdraw2);
    //uint256 wFee = withdrawFee(withdraw_amount); // changed from withdraw_amount
    //uint256 totalAmountToWithdraw = SafeMath.sub(withdraw_amount,wFee); // changed from aval_withdraw to withdraw_amount
    BusdInterface.transfer(msg.sender,withdraw_amount);
    //BusdInterface.transfer(dev,wFee);
    approvedWithdrawal[msg.sender] = userWithdrawal(msg.sender,aval_withdraw2); // changed from 0 to 70% of the amount stay in in his contract
    ...
    uint256 totalAmount = SafeMath.add(amount,withdraw_amount); // it will add 30% to total withdraw
    ...
    uint256 userLastInvestment = investments[msg.sender].invested;
    uint256 userCurrentInvestment = withdraw_amount;
    uint256 totalInvestment = SafeMath.add(userLastInvestment,userCurrentInvestment);
    investments[msg.sender] = user_investment_details(msg.sender,totalInvestment);
    ...
    uint256 totalAmount = SafeMath.add(amount,withdraw_amount); // it will add 30% to total withdraw
    userCompoundCount[msg.sender] = SafeMath.add(userCompoundCount[msg.sender],1);
...
investments[msg.sender].invested = 0;
    ...
    function unStake() external noReentrant {
        require(init, "Not Started Yet");
        require(userCompoundCount[msg.sender] == 0, "Unsake available before compound");
    ...
    function Ref_Withdraw() external noReentrant { // testnet - not ready
        require(init, "Not Started Yet");
