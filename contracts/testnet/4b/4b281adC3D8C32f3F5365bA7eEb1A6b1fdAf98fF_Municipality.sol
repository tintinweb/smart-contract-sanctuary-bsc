...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
        ...
        returns (MintedNFT memory)
    {
        ...
        uint256 _amount = getPriceForParcels(mintingSignature.parcels);
    ...
    function mintMiners(uint16 count) 
    external 
    onlySaleActive
    returns (MintedNFT memory) 
    {
    ...
    function attachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) external notZeroAddress {
        ...
        _requireMinersCountMatchingWithParcelSlots(parcelId);
    ...
    function _requireMinersCountMatchingWithParcelSlots(uint256 parcelId)
        private
        view
    {
        ParcelInfo memory parcelInfo = parcelIdToParcelInfoMapping[parcelId];
        uint count = parcelMinersMapping[parcelId].length;
        require(
            parcelInfo.isValid &&
                (
