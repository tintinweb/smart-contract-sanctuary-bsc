...
contract MigratedMos is ERC20, Ownable{
    ...
    uint256 internal constant LAUNCH_TIME = 1665134903;

    uint256 internal constant MIN_STAKE_DAYS = 10;
    uint256 internal constant MIN_AUTO_STAKE_DAYS = 365;
    ...
    constructor(address _founderAddr) ERC20 ("MOS Token","MOS") {

        GB1 = 19000000000e18;
        GB2 = 20040000000e18;

        _mint(msg.sender, 50000000000e18);
        founder = _founderAddr;

    }

    event StakeStart(uint256 amount, uint256 stakeDays, address indexed stakerAddr, uint256 stakeId, uint256 stakeOnDay);
    event StakeEnd(uint256 principal, uint256 payout, address indexed stakerAddr, uint256 stakeId);
    event StakeEndBonus(uint256 gb1, uint256 gb2, address indexed stakerAddr, uint256 stakeId);
    ...
    function stakeStart(uint256 amtToStake, uint256 dayToStake) external {
        ...
        _stakeStart(amtToStake, dayToStake);
        _burn(msg.sender, amtToStake);
    }

    function _stakeStart(uint256 amtToStake, uint256 dayToStake) internal {
    ...
    function stakeEnd(uint256 stakeIndex, uint256 stakeIdParam) external {
        ...
        if (currentDay() >= st._lockedDay) {
            ...
            (stakeReturn, payout, penalty, cappedPenalty, gb1, gb2) = _stakePerformance(st, servedDays);
        } else {
        ...
        if (stakeReturn > st._stakedHearts) {
            profit = stakeReturn - st._stakedHearts;
        }

        emit StakeEnd(st._stakedHearts, profit, msg.sender, stakeIdParam);
        emit StakeEndBonus(gb1, gb2, msg.sender, stakeIdParam);
    ...
    function _stakePerformance(StakeCache memory st, uint256 servedDays) internal view returns (uint256 stakeReturn, uint256 payout, uint256 penalty, uint256 cappedPenalty, uint256 gb1_reward, uint256 gb2_reward) {
        if (servedDays < st._stakedDays) {
            ...
            payout = (st._stakedHearts * st._stakedDays * 20 / 2920 / 100) + (st._stakedHearts * st._stakedHearts * 10 / 80000000e18 / 100);
            stakeReturn = st._stakedHearts + payout;

            if (st._lockedDay <= 365) {
                if (st._lockedDay + st._stakedDays >= 365) {
                    gb1_reward = st._stakedHearts * GB1 / globals.lockedHeartsTotal;
                    stakeReturn += gb1_reward;
                    if (st._lockedDay + st._stakedDays >= 730) {
                        gb2_reward = st._stakedHearts * GB2 / globals.lockedHeartsTotal;
                        stakeReturn += gb2_reward;
                    }
                }
            }
        ...
        return (stakeReturn, payout, penalty, cappedPenalty, gb1_reward, gb2_reward);
    }
    ...
    function _calcPayoutAndEarlyPenalty(uint256 stakedDaysParam, uint256 servedDays, uint256 stakeAmt ) internal view returns (uint256 payout, uint256 penalty){
        ...
        if (penaltyDays < servedDays) {
            penalty = (stakeAmt * stakedDaysParam * 20 / 2920 / 100) + (stakeAmt * stakeAmt * 10 / 80000000e18 / 100);

            uint256 delta = (stakeAmt * stakedDaysParam * 20 / 2920 / 100) + (stakeAmt * stakeAmt * 10 / 80000000e18 / 100);
            payout = penalty + delta;
            return (payout, penalty);
        }

        /* penaltyDays >= servedDays  */
        payout = (stakeAmt * stakedDaysParam * 20 / 2920 / 100) + (stakeAmt * stakeAmt * 10 / 80000000e18 / 100);
    ...
    function claimMOS() external {
        ...
        require(currentDay() >= 1095 && currentDay() <= 9131, "MOS: From 3rd year to 25th year only");
        uint256 mosPerYear = 59130434782608696;
        uint256 year = currentDay()/365;
        uint256 yearToClaim = year - claimed;
        if (yearToClaim > 0) {
            _mint(msg.sender, mosPerYear * yearToClaim);
            claimed = year;
        }
    }
