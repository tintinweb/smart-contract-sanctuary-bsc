...
contract LIFEGAMES is ERC20Upgradeable {
    ...
    function _mint(address account, uint256 amount) internal override virtual {
        ...
        _beforeTokenTransfer(address(0), account, _reflectionSupply);
        _totalSupply = amount;
        _reflectionSupply = (MAX - (MAX % _totalSupply));
        _reflectionBalance[_owner] = _reflectionSupply;

        emit Transfer(address(0), account, _reflectionSupply);

        _afterTokenTransfer(address(0), account, _reflectionSupply);
    }
