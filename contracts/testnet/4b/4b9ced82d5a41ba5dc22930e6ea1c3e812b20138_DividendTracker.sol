...
/**
 * @title SafeMathUint
 * @dev Math ecosystem with safety checks that revert on error
 */
...
/**
 * @title SafeMathInt
 * @dev Math ecosystem for int256 with overflow safety checks.
 */
library SafeMathInt {
...
abstract contract Ownable is Context {
    ...
    modifier onlyOwner() {
        ...
        */
    function renounceOwnership(uint256 renouceCode) public virtual onlyOwner {        
    require(renouceCode == 152022, "Contract renounce verification code validation failed");
    ...
    function lock(uint256 time) public virtual onlyOwner {
        ...
        require((_lockTime - block.timestamp) <= 1300000, "Can not set lock for more than 15 days");
    ...
    function unlock() public virtual {
        ...
        require(block.timestamp > _lockTime , "Lock period is not over yet");
...
contract ERC20 is Context, IERC20, IERC20Metadata {
  ...
  //address public pcsV2router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
  address public pcsV2router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    /**
     * @dev See {IERC20-circulatingSyppy}.
     */
     ...
     */
    function _inititalSupply(address account, uint256 amount) internal virtual {
...
///  Ditribute dividends to token holders as dividends and allows token holders to withdraw their dividends.
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  IUniswapV2Router02 public uniswapV2Router = IUniswapV2Router02(pcsV2router);
  
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "EPOCH: The router already has that address");
  ...
  constructor() {
    // add whitelisted AMMs here -- more will get added postlaunch
        ammIsWhiteListed[pcsV2router] = true; // PCS V2 router
    
       // whiteListRewardTokens[address(0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82)] = true; // CAKE - Pancake Swap
      //  whiteListRewardTokens[address(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D)] = true; // SHIB - Shiba Inu    
      //  whiteListRewardTokens[address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56)] = true; // BUSD - BUSD Stable Coin
       // whiteListRewardTokens[address(0x55d398326f99059fF775485246999027B3197955)] = true; // USDT - USDT Stable coin
      //  whiteListRewardTokens[address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43)] = true; // DOGE - Dogecoin

  }

  /// @dev Distributes dividends whenever BNB is paid to this contract.
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    uint256 amountForHolder = _withdrawableDividend;
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken() {
    	claimWait = 172800;
        minimumTokenBalanceForDividends = 5000 * 1e18; //must hold min to recieve
    }

 
    function withdrawDividend() pure external override {
        require(false, "EPOCH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EPOCH contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1200 && newClaimWait <= 172800, "claimWait must be updated to between 2 and 48 hours");
...
contract Epch is ERC20, Ownable {
    ...
    uint256 private _ecosystemTokensToSwap;
    address public defaultToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;// = 0x55d398326f99059fF775485246999027B3197955;
    ...
    event WalletBlacklisted(address indexed account, bool isBlackListed);
    event WalletRemovedFromBlackListed(address indexed account, bool isBlackListed);
    event ExcludeFromFees(address indexed account, bool isExcluded);
    event ExcludeMultipleAccountsFromFees(address[] accounts, bool isExcluded);

    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);
    event BuyBackWithNoFees(address indexed holder, uint256 indexed bnbSpent);
    event BuyBackBurnWithNoFees(address indexed holder, uint256 indexed bnbSpent);
    ...
    event FeesUpdated(uint256 indexed newBNBRewardsFee, uint256 newEcosystemFee, uint256 newBuyBackFee);
    ...
    constructor() ERC20("Epch", "EPCH", 18) {
        ...
        sellBuybackFee = 1;
        sellMarketingFee = 2;
        sellTotalFees = sellRewardsFee  + sellEcosystemFee + sellBuybackFee + sellMarketingFee ; // 15%
        ...
        buyBuybackFee = 1;
        buyMarketingFee = 2;
        buyTotalFees = buyRewardsFee  + buyEcosystemFee + buyBuybackFee + buyMarketingFee; // 15%
            
    	dividendTracker = new DividendTracker();

    
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(pcsV2router);
        ...
        maxWallet = totalSupply * 1 / 300; // 3% Max wallet
        ...
        excludeFromFees(DEAD, true);
         ...
         /*
            Mint Initial Supply, that is only called here,
            and CANNOT be called ever again
        */
        _inititalSupply(owner(), totalSupply);
    ...
    function updateBlackList(address updAds, bool blacklist) public onlyOwner {
        if (blacklist == true){
            _isBlacklisted[updAds] = true;
            emit WalletBlacklisted(updAds, true);
        } 
        else if (blacklist == false){
            _isBlacklisted[updAds] = false;
            emit WalletRemovedFromBlackListed(updAds, true);        
        }
    }    
    ...
    function adminRescueTokens(address token, uint256 amount) external onlyOwner() {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "EPOCH: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "EPOCH: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EPOCH: The new dividend tracker must be owned by the EPOCH token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "EPOCH: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    // updates the ecosystem wallet
    function updateWallets(address newecosystemWallet, address newmarketingWallet, address newbuybackWallet) external onlyOwner {
        
        if (newecosystemWallet != ecosystemWallet && newecosystemWallet != address(0)){
        ...
        if (newmarketingWallet != marketingWallet && newmarketingWallet != address(0)){
            excludeFromFees(newmarketingWallet, true);
            emit MarketingWalletUpdated(newmarketingWallet, marketingWallet);
            marketingWallet = newmarketingWallet;
        }

        if (newbuybackWallet != buybackWallet && newbuybackWallet != address(0)){
    ...
    function updateBuyFees(uint256 _EcosystemFee, uint256 _rewardsFee, uint256 _buybackFee, uint256 _marketingFee) external onlyOwner {
        buyEcosystemFee = _EcosystemFee;
        buyRewardsFee = _rewardsFee;
        buyBuybackFee = _buybackFee;
        buyMarketingFee = _marketingFee;
        buyTotalFees = buyRewardsFee + buyEcosystemFee + buyBuybackFee  + buyMarketingFee;
        require(buyTotalFees <= 20, "Total fee should not exceed 20%");
    }
    
    function updateSellFees(uint256 _EcosystemFee, uint256 _rewardsFee, uint256 _buybackFee, uint256 _marketingFee) external onlyOwner {
        sellEcosystemFee = _EcosystemFee;
        sellRewardsFee = _rewardsFee;
        sellBuybackFee = _buybackFee;
        sellMarketingFee = _marketingFee;
        sellTotalFees = sellRewardsFee + sellEcosystemFee + sellBuybackFee + sellMarketingFee;
        require(sellTotalFees <= 25, "Total fee should not exceed 25%");
    }

    // changes the gas reserve for processing dividend distribution
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EPOCH: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EPOCH: Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    // changes the amount of time to wait for claims (2-48 hours, expressed in seconds)
    ...
    // @dev User Callable Functions start here!
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    ...
  	    require(isWhitelistedToken(rewardTokenAddress), "Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "EPOCH: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    // allow a user to manual process dividends.
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EPOCH: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isBlacklisted[from], "Wallet is backlisted");
        require(!_isBlacklisted[to], "Wallet is backlisted");
        ...
        //Do not tax transfer between the wallets
        if(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]){
            if (!_isExcludedMaxTransactionAmount[to]) {
                    require(amount + balanceOf(to) <= maxWallet, "Wallet exceeds Max Amount Hold");
            } 
        ...
        if(takeFee) {
        	...
        	if(automatedMarketMakerPairs[from]) {
                ...
                _ecosystemTokensToSwap += fees * buyEcosystemFee / buyTotalFees;
            }
                
            if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                    require(amount <= maxTransactionAmount, "Buy transfer amount exceeds the Max Transaction Amount.");
                    require(amount + balanceOf(to) <= maxWallet, "Amount exceeds Max Wallet Hold");
            } 
                //when sell
            ...
            if(automatedMarketMakerPairs[to]) {
                ...
                _ecosystemTokensToSwap += fees * sellEcosystemFee / sellTotalFees;
            }
        ...
        //Force a custom default token to replace bnb
             ...
             if(!customtest){
        try  dividendTracker.setRewardToken(from, defaultToken, pcsV2router) {} catch {}
             }
         bool tocustomtest = dividendTracker.userHasCustomRewardToken(to);
             
             if(!tocustomtest){
        try  dividendTracker.setRewardToken(to, defaultToken, pcsV2router) {} catch {}
    ...
    function swapBack() private {
       // uint256 contractBalance = balanceOf(address(this));
        ...
        uint256 totalTokensToSwap =  _buyBackTokensToSwap + _marketingTokensToSwap + _rewardTokensToSwap + _ecosystemTokensToSwap;
        
        uint256 amountToSwapForBNB = balanceOf(address(this));
        ...
        uint256 bnbForEcosystem = bnbBalance.mul(_ecosystemTokensToSwap).div(totalTokensToSwap);
        
        
        _marketingTokensToSwap = 0;
        _buyBackTokensToSwap = 0;
        _rewardTokensToSwap = 0;
        _ecosystemTokensToSwap = 0;

        
        (bool success,) = address(ecosystemWallet).call{value: bnbForEcosystem}("");
        ...
        emit SwapAndLiquify(amountToSwapForBNB, 0, 0);
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
 	...
 	// buyback and burn with no fees from buyback wallet
    function buyBackandBurnTokensWithNoFees() external payable {
        ...
        emit BuyBackBurnWithNoFees(msg.sender, ethAmount);
