...
import {ColumbusMath} from "contracts/ColumbusMath.sol";
import {IColumbusFarm} from "contracts/interfaces/IColumbusFarm.sol";
// import {IAssetCertificate} from "contracts/interfaces/IAssetCertificate.sol";
import {IColumbusVault} from "contracts/interfaces/IColumbusVault.sol";
    ...
    ColumbusMath
{
    ...
    struct UserInfo {
        // 1 solt 104 * 2 + 48 = 256
        uint104 shares; // number of shares for a user.
        uint104 userBoostedShare; // boost share, in order to give the user higher reward. The user only enjoys the reward, so the principal needs to be recorded as a debt.
        ...
        uint48 lockStartTime; // lock start time.
        // 2 solt
        uint48 lockEndTime; // lock end time.
        // bool locked; //lock status.
        uint104 lockedAmount; // amount deposited during lock period.
        uint104 pendingYESReward; // yes Reward Overdue Weight
    }
    ...
    uint256 public MIN_LOCK_DURATION = 1 weeks; // 1 week
    ...
    function updateUserShare(uint256 _proofId, address _user) internal {
        ...
        if (user.shares > 0 && user.lockEndTime > 0) {
            ...
            // Calculation YES reward
            user.pendingYESReward += safe104(pendingYESReward);

            // uint256 currentShares = currentAmount;
            user.shares = safe104(currentShares);
    ...
    function depositFromColumbus(
        ...
        require(
            lockDuration_ >= MIN_LOCK_DURATION,
            "Minimum lock period is one week"
        );
        require(
            amount_ > minDepositAmount,
            "Deposit amount must be greater than MIN_DEPOSIT_AMOUNT"
        );

        // uint256 _proofId = assetCertificate.mint(_user);
        uint256 _proofId = nextTokenId();
        _owners[_proofId] = user_;
        _nextTokenId.increment();
        // 后面需要删除
        uint256[] storage tokenIds = userTokens[user_];
        tokenIds.push(_proofId);

        _deposit(user_, _proofId, amount_, lockDuration_);
    }
    ...
    function _deposit(
        ...
        if (user.lockEndTime >= block.timestamp) {
            ...
            if (_amount > 0) {
                user.lockStartTime = uint48(block.timestamp);
                totalLockedAmount -= user.lockedAmount;
                user.lockedAmount = 0;
            }
        ...
        if (_lockDuration > 0) {
            if (user.lockEndTime < block.timestamp) {
                user.lockStartTime = uint48(block.timestamp);
                user.lockEndTime = uint48(block.timestamp + _lockDuration);
            } else {
                user.lockEndTime += uint48(_lockDuration);
            }
            // user.locked = true;
        }
        ...
        if (user.shares > 0) {
            ...
            if (user.lockStartTime == block.timestamp) {
                user.lockedAmount = safe104(userCurrentLockedBalance);
                totalLockedAmount += user.lockedAmount;
            }
        }
        ...
        uint256 userBoostedShare = (boostWeight * currentAmount) /
            PRECISION_FACTOR;
        user.userBoostedShare += safe104(userBoostedShare);
        totalBoostDebt += userBoostedShare;

        // Update lock amount.
        user.lockedAmount += safe104(_amount);
        totalLockedAmount += safe104(_amount);
    ...
    function claim(uint256 _proofId) external {
        require(
            _proofId > 0 && msg.sender == ownerOf(_proofId),
    ...
    function _withdraw(
        ...
        user.shares -= safe104(currentShare);
        totalShares -= currentShare;

        if (user.shares == 0 && _proofId > 0) {
            // assetCertificate.burn(_proofId);
            delete _owners[_proofId];
            delete proofs[_proofId];

            uint256[] storage tokenIds = userTokens[_user];
            if (tokenIds.length > 1) {
                uint8 index = getTokenIdIndex(_user, _proofId);
     ...
     * @return amount Amount of reward.
     */
    function pendingCompoundingRewards(
        address user_,
        uint256 proofId_
    ) external view returns (uint256) {
        UserInfo storage user = userInfo[user_];
