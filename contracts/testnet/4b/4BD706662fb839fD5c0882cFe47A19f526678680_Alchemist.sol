...
contract Alchemist is ReentrancyGuardUpgradeable, AccessControlUpgradeable, ERC721EnumerableUpgradeable {
    ...
    ItokenURI constant skin = ItokenURI(0x560C4b009E5CF19e36E3220b90ae330f38DFD5a0);
    ...
    function setFarmPool(address payable farmPool_) public nonReentrant {
        farmPool = farmPool_;
    }

    function setLootDAO(address payable lootDAO_) public nonReentrant {
        lootDAO = lootDAO_;
    }

    function setAlchemistDAO(address payable alchemistDAO_) public nonReentrant {
        alchemistDAO = alchemistDAO_;
    }

    function setLoot(ILoot loot) public nonReentrant {
        _loot = loot;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if(from != address(0) && to != address(0) && to != address(0x000000000000000000000000000000000000dEaD) && !to.isContract() && to != EOAOf){
            ...
            _loot.transferFrom(payer, lootDAO, _royalty*5/10);
        }
        if(!to.isContract()){
            EOAOf = to;
        }
