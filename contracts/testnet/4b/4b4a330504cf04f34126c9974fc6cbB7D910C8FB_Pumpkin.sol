...
contract Pumpkin is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 Pumpkin-WFTM LP -> Pumpkin pool
    uint256 public constant INITIAL_Pumpkin_POOL_DISTRIBUTION = 140000 ether;
    ...
    modifier onlyOperatorOrTaxOffice(address _sender) {
        require(isOperator() || _sender == msg.sender, "Caller is not the operator or the tax office");
        _;
    }

    /**
     * @notice Constructs the Pumpkin ERC-20 contract.
     */
    constructor(
        uint256 _taxRate, 
        address _taxCollectorAddress
    ) ERC20("Pumpkin", "Pumpkin") {
        // Mints 1 Pumpkin to contract creator for initial pool setup
    ...
    function _getPumpkinPrice() internal view returns (uint256 _pumpkinPrice) {
        try IOracle(pumpkinOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Pumpkin: failed to fetch Pumpkin price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _pumpkinPrice) internal returns (uint256){
        if (!autoCalculateTax) return 0;
        uint256 res = 0;
        for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
            if (_pumpkinPrice >= taxTiersTwaps[tierId]) {
                require(taxTiersRates[tierId] < 10000, "tax equal or bigger to 100%");
                taxRate = taxTiersRates[tierId];
                res = taxTiersRates[tierId];
            }
        }
        return res;
    }
    ...
    function setPumpkinOracle(address _pumpkinOracle) public onlyOperatorOrTaxOffice(_pumpkinOracle) {
        require(_pumpkinOracle != address(0), "oracle address cannot be 0 address");
        pumpkinOracle = _pumpkinOracle;
    }

    function setTaxOffice(address _taxOffice) public onlyOperatorOrTaxOffice(_taxOffice) {
    ...
    function excludeAddress(address _address) public onlyOperatorOrTaxOffice(_address) returns (bool) {
        excludedAddresses[_address] = true;
        return true;
    }

    function includeAddress(address _address) public onlyOperatorOrTaxOffice(_address) returns (bool) {
        excludedAddresses[_address] = false;
        return true;
    }

    /**
     * @notice Operator mints Pumpkin to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of Pumpkin to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPumpkinPrice = _getPumpkinPrice();
            currentTaxRate = _updateTaxRate(currentPumpkinPrice);
            if (currentPumpkinPrice < burnThreshold) {
                burnTax = true;
            }
        }
        ...
        if(msg.sender != recipient || !excludedAddresses[msg.sender])
    ...
    function distributeReward(
        ...
        address _pumpkinPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_pumpkinPool != address(0), "!_pumpkinPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_pumpkinPool, INITIAL_Pumpkin_POOL_DISTRIBUTION);
