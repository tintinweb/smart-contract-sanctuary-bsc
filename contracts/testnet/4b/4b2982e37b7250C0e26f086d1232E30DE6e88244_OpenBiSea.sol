...
contract OpenBiSea is Ownable  {
    ...
    function purchaseTokens(address referral) public payable returns (uint256) {
        ...
        if (referral != address (0x0)) {
            require(referral == address(referral),"OpenBiSea: Invalid address");
    ...
    function bidToken(
        ...
        if (isWin) {
            ...
            if (token == usdContract) {
                ...
                uint256 valueFinal = bidAmount.div((priceMainToUSD).div(10 ** uint256(decimals)));
    ...
    function bid(
        ...
        if (isWin) {
            ...
            if (_referrals[msg.sender] != address(0) || referral != address(0)) {
                ...
                totalSellerAmount = totalSellerAmount.sub(depositFeeReferrer);
                Address.sendValue(payable(_referrals[msg.sender]), depositFeeReferrer);
                Address.sendValue(payable(owner()), depositFee);
            } else Address.sendValue(payable(owner()), depositFee);

            Address.sendValue(payable(auctionLatestBidderOrSeller), totalSellerAmount);
            totalIncome = totalIncome.add(msg.value);
        } else {
            if (amountTransferBack > 0) {
                Address.sendValue(payable(auctionLatestBidderOrSeller), amountTransferBack);
            }
        }
    }
    ...
    function claimFreeTokens() public {
        require(totalIncome >= (5000 ether / mainCoinToUSD), "OpenBiSea: distribution starts from 5k USD");
        uint256 percentWithDecimalsToDistribute = 500; // max 500 (5%)
        if (totalIncome / mainCoinToUSD > 10000 && totalIncome / mainCoinToUSD < 500000)  percentWithDecimalsToDistribute = 300;
        if (totalIncome / mainCoinToUSD > 500000 && totalIncome / mainCoinToUSD < 5000000)  percentWithDecimalsToDistribute = 100;
        if (totalIncome / mainCoinToUSD > 5000000) percentWithDecimalsToDistribute = 10;
