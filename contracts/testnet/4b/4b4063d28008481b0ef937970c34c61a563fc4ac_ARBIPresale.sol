...
contract ARBIPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale ARBI, 1 USDC
	
    uint256 public arbiPerUsdc = 2;
	// Amount of USDC received in presale
	uint256 usdcReceived = 0;

    uint256 HardCap = 300000000 * 10 ** 18; // 85,000
    ...
    function setArbiPerUsdc(uint256 _arbiPerUsdc) public onlyOwner{
        arbiPerUsdc = _arbiPerUsdc;
    }
	...
	function buy(uint256 _amount) public nonReentrant {
		...
		uint256 tokens = _amount.mul(arbiPerUsdc);
	...
	function getARBITokensLeft() external view returns (uint256) {
		return ARBI.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some ARBI tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed ARBI tokens");
		require(ARBI.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough ARBI tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		ARBI.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldARBI() external onlyOwner {
		uint256 amount = ARBI.balanceOf(address(this)) - totalTokensSold;
		ARBI.safeTransfer(msg.sender, amount);
	}

	function withdrawAllARBI() external onlyOwner {
		ARBI.safeTransfer(msg.sender, ARBI.balanceOf(address(this)));
	}
