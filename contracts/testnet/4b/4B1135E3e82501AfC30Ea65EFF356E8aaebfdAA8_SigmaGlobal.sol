...
contract SigmaGlobal is SigmaGlobal_Status {
	...
	constructor(address payable devAddr, address payable _proJectAddress) public {
			...
			for(uint256 i; i < MAX_REFERRAL_LEVEL; i++) {
				if(upline != address(0)) {
					if(i <= users[upline].referrerCount[0]) {
						...
						uint machineLength = users[upline].machineLength;
						users[upline].machineDeposits[machineLength].initAmount = amount;
						users[upline].machineDeposits[machineLength].start = block.timestamp;
						users[upline].machineLength = machineLength.add(1);
	...
	function withdraw_f() external whenNotPaused checkUser_ returns(bool) {
		...
		for(uint i; i < user.machineLength; i++) {
			uint256 dividends;
			MachineBonus memory machineBonus = user.machineDeposits[i];
			if(machineBonus.withdrawn < machineBonus.initAmount && machineBonus.isForceWithdraw == false) {
				dividends = calculateMachineDividents(machineBonus, user, totalAmount);
				if(dividends > 0) {
		...
		uint feeToDev = totalAmount.mul(WITHDRAW_FEE_PERCENT_TO_DEV).div(PERCENTS_DIVIDER);
		uint256 fee = (totalAmount.mul(WITHDRAW_FEE_PERCENT(user.checkpoint))).div(PERCENTS_DIVIDER);

		uint totalFee = feeToWAllet.add(fee).add(feeToDev);
	...
	function withdraw_C() external whenNotPaused checkUser_ returns(bool) {
		...
		for(uint256 i; i < user.depositsLength; i++) {
			...
			if(deposit.withdrawn < getMaxprofit(deposit) && deposit.isForceWithdraw == false) {
				dividends = calculateDividents(deposit, userPercentRate, user);
				_bonus += deposit.initAmount.mul(forceWithdrwBonusPercent).div(PERCENTS_DIVIDER);
				if(dividends > 0) {
					user.deposits[i].withdrawn = user.deposits[i].withdrawn.add(dividends); /// changing of storage data
		...
		for(uint i; i < user.machineLength; i++) {
			...
			if(machineBonus.withdrawn < machineBonus.initAmount && machineBonus.isForceWithdraw == false) {
				...
				user.machineDeposits[i].isForceWithdraw = true;
			}
		}
	...
	function getUserData(address userAddress) external view returns(uint256 totalWithdrawn_,
		...
		totalWithdrawn_ = getUserDeposittotalWithdrawn(userAddress);
	...
	function calculateMachineDividents(MachineBonus memory deposit, User memory user, uint _currentDividends) internal view returns (uint256) {
		uint256 dividends;

		uint256 checkDate = deposit.start;

		if(checkDate < getlastActionDate(user)) {
			checkDate = getlastActionDate(user);
		}

		dividends = (deposit.initAmount
		.mul(MACHINE_ROI.mul(block.timestamp.sub(checkDate))))
		.div((PERCENTS_DIVIDER).mul(TIME_STEP))
		;
		...
		if(user.totalWithdraw + dividends + _currentDividends > _userMaxDividends) {
			if(user.totalWithdraw + _currentDividends < _userMaxDividends) {
				dividends = _userMaxDividends - user.totalWithdraw - _currentDividends;
			} else {
				dividends = 0;
			}
		}
	...
	function getUserDeposittotalWithdrawn(address userAddress) internal view returns(uint256) {
		User memory user = users[userAddress];
