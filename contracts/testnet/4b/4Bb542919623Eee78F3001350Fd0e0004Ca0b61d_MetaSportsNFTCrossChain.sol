...
contract MetaSportsNFTCrossChain is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    event RevenueClaim(address indexed claimer, uint256 amount, address indexed tokenAddress);
    ...
    function transferIntoNFT(
        ...
        // Calculate trading fee (collected by treasury), creator fee (collected by creator)
        (uint256 tradingFee, uint256 creatorFee) = _calculatePriceAndFeesForCollection(
            _collection
        );
    ...
    * @notice transfer into token
    ...
    function transferOutNFT(
        ...
        require(_ccTokenIds[_collection].contains(_tokenId), "Buy: Not for sale");
        ...
        // Emit event
        emit TransferOut(_collection, msg.sender, _tokenId);
    }
    ...
    function claimPendingRevenue(address _tokenAddress) external nonReentrant {
        ...
        emit RevenueClaim(msg.sender, revenueToClaim, _tokenAddress);
    }
    ...
    function recoverNonFungibleToken(address _token, uint256 _tokenId) external onlyOwner nonReentrant {
        require(!_ccTokenIds[_token].contains(_tokenId), "Operations: NFT not recoverable");
     ...
     */
    function calculatePriceAndFeesForCollection(address collection)
    external
    view
    returns (
        uint256 tradingFee,
    ...
    )
    {
        ...
        return (_calculatePriceAndFeesForCollection(collection));
    }
    ...
    /**
     * @notice Calculate associated fees for a collection
     * @param _collection: address of the collection
     */
    function _calculatePriceAndFeesForCollection(address _collection)
    internal
    view
    returns (
        uint256 tradingFee,
    ...
    {
        tradingFee = (_collections[_collection].tradingFee) / 10000;
        creatorFee = (_collections[_collection].creatorFee) / 10000;

        return (tradingFee, creatorFee);
    }
