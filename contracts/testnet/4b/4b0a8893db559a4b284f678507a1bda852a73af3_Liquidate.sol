...
contract Liquidate is PancakeswapFlashloan, AccessControl {
  ...
  function _handleFlashLoan(
    ...
    RErc20Interface(address(liquidateData.rTokenCollateral)).redeemUnderlying(
      liquidateData.rTokenCollateral.balanceOf(address(this))
    );
    _swap(address(liquidateData.rTokenCollateral), address(token), amount + fee);
  ...
  function _swap(address tokenFrom, address tokenTo, uint256 amountOut) internal {
    ...
    exchange.swapTokensForExactETH(amountOut, amountInMax, path, address(this), block.timestamp);
  }
}