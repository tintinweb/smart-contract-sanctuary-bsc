...
//  注册方法
    ...
    function distributePoolRewards() private { //分配奖池方法
    ...
    function _calCurStaticRewards(address _user) private view returns(uint256, uint256) {//获取静态收益
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {//获取动态收益
        RewardInfo storage userRewards = rewardInfo[_user];
    ...
    function _updateLevel(address _user) private {
        ...
        if(levelNow > user.level){ //如果当前等级大于之前等级就更新
            user.level = levelNow;
            if(levelNow == 4){ //如果为4级 就加在level4Users 中
...
// 质押方法 
    function _deposit(address _user, uint256 _amount) private { // 参数 ：地址、金额
        ...
        user.totalDeposit = user.totalDeposit.add(_amount); // userinfo 信息 总质押金额加上 _amount

        user.totalFreezed = user.totalFreezed.add(_amount);// userinfo 信息 冻结金额加上 _amount

        _updateLevel(msg.sender);// 更新等级
    ...
    // function getOrders(address _user) public view returns(OrderInfo[] memory ) {
