...
contract NFTReward is ERC721Upgradeable {
    ...
    // mapping token id to point value
    mapping(uint256 => uint256) public pointToNFT;
    ...
    function safeMint(address to, uint256 pts) public onlyOwner {
        _safeMint(to, _tokenIds.current());
        pointToNFT[_tokenIds.current()] = pts;
        _tokenIds.increment();
    }

    function safeMintBatch(address to, uint8 amount, uint256[] memory pts) public onlyOwner {
        for (uint8 idx = 0; idx < amount; idx++) {
            _safeMint(to, _tokenIds.current());
            pointToNFT[_tokenIds.current()] = pts[idx];
            _tokenIds.increment();
        }
    }
    ...
    function burnBatch(uint256[] memory tokenIds) public onlyOwner {
        for (uint256 idx = 0; idx < tokenIds.length; idx++) {
            ...
            delete pointToNFT[tokenIds[idx]];
            _burn(tokenIds[idx]);
        }
    }

    function changePrice(uint256 tokenId, uint256 pts) public onlyOwner {
        require(msg.sender == ERC721Upgradeable.ownerOf(tokenId), "Not own token");
        pointToNFT[tokenId] = pts;
    }

    function getPoints(uint256 tokenId) external view returns (uint256) {
        return pointToNFT[tokenId];
    }
