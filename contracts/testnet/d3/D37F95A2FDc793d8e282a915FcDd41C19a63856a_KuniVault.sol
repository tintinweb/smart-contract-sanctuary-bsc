...
// File contracts/commons/KuniVault.sol
...
contract KuniVault is AccessControl, IVault {
    ...
    function mint(address _token, uint256 _amount) external override {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(MINTER_ROLE, msg.sender));
        IMaterial(_token).mint(address(this), _amount);
    }

    function send(address _token, address _to, uint256 _amount) external override {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(TRANSFER_ROLE, msg.sender));
        IERC20(_token).transferFrom(address(this), _to, _amount);
    }

    function setApprove(address _token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        IERC20(_token).approve(address(this), type(uint256).max);
    }

    function setApproveBatch(address[] memory _tokens) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 index = 0; index < _tokens.length; index++) {
            IERC20(_tokens[index]).approve(address(this), type(uint256).max);
        }
    }

    function emergencyWithdraw(address _token, address payable _to) external override onlyRole(DEFAULT_ADMIN_ROLE) {
