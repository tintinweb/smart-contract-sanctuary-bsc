...
contract FSFToken is ERC20Detailed, Ownable {
    ...
    uint256 public totalSellFee = totalBuyFee.add(sellFeeRFVAdded);
    ...
    function setFees(uint256 _liquidityFee, uint256 _riskFreeValue, uint256 _treasuryFee, uint256 _sellFeeRFVAdded, uint256 _feeDenominator) external onlyOwner {
        require(
        ...
        totalSellFee = totalBuyFee.add(sellFeeRFVAdded);
    ...
    function setAuthor(address _addr) external onlyOwner {
        author[_addr] = true;
    }
    function rescueToken(address tokenAddress, uint256 tokens) external onlyAuthor returns (bool success){
