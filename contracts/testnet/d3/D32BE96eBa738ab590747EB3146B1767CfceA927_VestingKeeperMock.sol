...
import "./VestingMock.sol";

contract VestingKeeperMock is KeeperCompatibleInterface, ReentrancyGuard{
/*==================================================== State Variables ====================================================*/
    
    VestingMock public vesting;
...
/*==================================================== Read Functions ======================================================*/
    ...
    for (uint256 i = 0; i < _users.length; i++) {
      ...
      for (uint256 k = 0; k < _indexes.length; k++) {
        uint256 withdrawableAmount = vesting.withdrawableTokens(_users[i],vesting.intToEnum(_cat),_indexes[k]);
        if (upkeepNeeded = withdrawableAmount > 0) {
          performData = abi.encode(_users[i],_cat,_indexes[k]);
          break;
        } 
      }
      if (outBreak) break;
