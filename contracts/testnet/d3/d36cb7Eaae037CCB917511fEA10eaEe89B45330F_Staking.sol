...
contract Staking is GlobalsAndUtils, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
    ...
    constructor(address _stakingToken, address _rewardingToken) {
        ...
        // penaltyAccumulated[1] = 18576000000000000000000;
        // totalStakedAmount = 17367100000000000000000000;
        // campaigns[latestCampaignId] = Campaign(2300, 1659528000, 1660737600, 0x422Ab9E31cC5cA8CA8266986FF4cd8fD18b68F1e);
 ...
 /**
    ...
    function rewardLootBox(uint256 start, uint256 end) external onlyOwner CalculateRewardAllowed {
        require(winningTicketIds[latestCampaignId].length + end <= campaigns[latestCampaignId].rewardCount, "exceeded reward limit");
     ...
     * @param receiver Address of the user to transfer the nft to
     */
    function emergencyWithdraw(address receiver) external onlyOwner campaignEnded {
        require(receiver != address(0), "invalid address");

        bool resp = stakingToken.transfer(receiver, stakingToken.balanceOf(address(this)));
        if (!resp) revert("couldn't transfer tokens");
    }
