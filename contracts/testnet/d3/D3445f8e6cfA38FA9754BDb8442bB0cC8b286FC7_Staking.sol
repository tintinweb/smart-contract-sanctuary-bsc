...
// File: contracts/staking/Interfaces.sol
...
// File: contracts/staking/SafeERC20.sol
...
// File: contracts/staking/Erc20Staking.sol
...
contract Staking is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint amountStake = _amount + (_amount * (feeStake / 100));
        ...
        if(!isStakeholder(_tokenStake, msg.sender)){
            ...
            addStake(_tokenStake, amountStake);
        }
    ...
    function addStake(address _tokenStake, uint _amount)
        private
    {
    ...
    function claimReward(IERC20 _tokenStake, uint _amount)
        public
        nonReentrant
    {
        uint rewardToken = rewardOf(msg.sender, address(_tokenStake));
        require(_amount > 0, "amount = 0");
        require(rewardToken >= _amount, "amount of the reward is too small than the amount of the take");

        _tokenStake.safeTransfer(msg.sender, _amount);
    }
        ...
        returns (uint)
    {
        ...
        if(userTimeStake > timeNow){
            return 0;
        }else {
            uint256 i = 1;
            // uint128 tokenRewardRate = rewardRate[_tokenStake];
            
            // Duration of user stake
            while(((i* duration) + userTimeStake) < timeNow){
                i++;
            }

            // uint reward = amountUserStake + (amountUserStake * ((tokenRewardRate * i) / 100));

            return userTimeStake;
        }
