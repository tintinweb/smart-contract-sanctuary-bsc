...
contract BaseERC20Token is ERC20Capped, ERC20Burnable {
     ...
     */
    modifier canMint() {
        require(!_mintingFinished,"BaseERC20Token: miniting is finished");
        _;
    }
...
contract ERC20Token is BaseERC20Token {
   ...
   modifier onSale {
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, uint256 cap_, uint256 initialSupply_)  BaseERC20Token(name_,symbol_,decimals_,cap_*10**decimals_,initialSupply_*10**decimals_) {
        _tokensSold = 0;
        _onSale = true;
    }
    ...
    function buyTokens(uint256 numberOfTokens, uint256 amount) public onSale payable {

        require(msg.value >= amount, "ERC20: msg.Value is not same as amount");
        
        _transfer(owner(),_msgSender(), numberOfTokens);
