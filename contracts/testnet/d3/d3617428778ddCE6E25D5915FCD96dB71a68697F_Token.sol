...
contract Token is ERC20, Ownable {
    ...
    constructor() ERC20("Coin", "ECO") {
        ...
        _excludedFromMaxTransaction[address(uniswapV2Router)] = true;
        ...
        _excludedFromMaxBalance[ZERO] = true;
        _excludedFromMaxBalance[DEAD] = true;
        _excludedFromMaxBalance[buyBackAndBurnAddress] = true;
        _excludedFromMaxBalance[marketingAddress] = true;
        _excludedFromMaxBalance[stakingRewardAddress] = true;
        ...
        maxWalletAmount = totalSupply().mul(1).div(100); //1% supply
    ...
    function requirementsBeforeTransfer(address from, address to, uint256 amount) internal view {
        ...
        require(amount.add(balanceOf(to)) <= maxWalletAmount || _excludedFromMaxBalance[to], "Recipient balance cannot be higher than allowed.");
    }
    ...
    function convertTokenFeesInETH(address to) internal {
        ...
        if (canSwap && !swapping && to == uniswapV2Pair) {
            ...
            uint256 marketingAmount = swappedETHBalance.mul(marketingFee).div(totalFees());
            uint256 stakingRewardsAmount = swappedETHBalance.mul(stakingRewardFee).div(totalFees());
            uint256 buyBackAndBurnToBurn = swappedETHBalance.sub(marketingAmount).sub(stakingRewardsAmount);
            
            sendETHToAddress(marketingAddress, marketingAmount);
            sendETHToAddress(stakingRewardAddress, stakingRewardsAmount);
            sendETHToAddress(buyBackAndBurnAddress, buyBackAndBurnToBurn);
