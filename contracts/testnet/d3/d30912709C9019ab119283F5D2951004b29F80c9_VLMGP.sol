...
// TODO thinking about making this a pool helper as well
/// @title Vote Lock MGP
/// @author Magpie XYZ Team
contract VLMGP is IVLMGP, Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    uint256 public totalAmountInCoolDown;
    ...
    /// @notice Get the total MGP a user locked and in cool down period
    /// @param _user the user
    /// @return _lockAmount the total MGP a user locked and in cool down period
    function getUserTotalLocked(address _user) override public view returns (uint256 _lockAmount) {
        // needs fixing
        (_lockAmount, ) = IMasterMagpie(masterMagpie).stakingInfo(address(this), _user);
        return _lockAmount;
    }
    ...
    // @notice deposit MGP in the contract
    // @param _amount the amount of MGP to deposit
    function lock(uint256 _amount) override external whenNotPaused {
    ...
    function lockFor(uint256 _amount, address _for)
        override
        external
        whenNotPaused
    {
    ...
    function startUnlock(
        ...
        uint256 userUnlockLength = getUserUnlockSlotLength(msg.sender);

        if (_slotIndex > userUnlockLength)
        ...
        IMasterMagpie(masterMagpie).withdrawFor(address(this), _amountToCoolDown, msg.sender);
        _burn(address(this), _amountToCoolDown);

        // put MGP back into MGP staking pool, but still in lock by MasterMagpie.stakingInfo.avaliable amount
        _approveTokenIfNeeded(address(MGP), masterMagpie, _amountToCoolDown);
        IMasterMagpie(masterMagpie).coolDownMgpFor(_amountToCoolDown, msg.sender);
        totalAmountInCoolDown += _amountToCoolDown;

        if (_slotIndex < userUnlockLength) {
            if (userUnlockings[msg.sender][_slotIndex].amountInCoolDown != 0)
                revert UnlockSlotOccupied();
    ...
    function unlock(uint256 _slotIndex) external override whenNotPaused {
        uint256 userUnlockLength = getUserUnlockSlotLength(msg.sender);
        if (_slotIndex > userUnlockLength)
            revert BeyondUnlockLength();
        ...
        totalAmountInCoolDown -= unlockedAmount;
        slot.amountInCoolDown = 0;
        IMasterMagpie(masterMagpie).finishCoolDownMGPFor(unlockedAmount, msg.sender);
    ...
    function _lock(
        ...
        _approve(address(this), masterMagpie, _amount);
        IMasterMagpie(masterMagpie).depositFor(address(this), _amount, _for);
