...
contract Staking {
    ...
    uint256[3] public Bonus = [20, 70, 200];
    ...
    function unstake(uint256 index) public {
        ...
        uint256 timeCalculation = block.timestamp .sub(stakersRecord[msg.sender][index].staketime);
        uint256 amountWithCurrReward =(stakersRecord[msg.sender][index].persecondreward.mul(timeCalculation)).add(stakersRecord[msg.sender][index].amount);
        uint256 penalty = amountWithCurrReward
            .mul(unstakePercent)
            .div(percentDivider);
        stakeToken.transfer(owner, penalty);
        stakeToken.transfer(
            msg.sender,
            amountWithCurrReward.sub(penalty)
        );
        totalUnStakedToken = totalUnStakedToken.add(
            amountWithCurrReward
        );
