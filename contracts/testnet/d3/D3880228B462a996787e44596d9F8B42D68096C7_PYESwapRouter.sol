...
// File: contracts8/interfaces/IWETH.sol
...
// File: contracts8/libraries/SafeMath.sol
...
// File: contracts8/interfaces/IERC20.sol
...
// File: contracts8/interfaces/IToken.sol
...
interface IToken {
    ...
    function getTotalFee() external returns (uint);
}

// File: contracts8/interfaces/IPYESwapFactory.sol
...
// File: contracts8/interfaces/IPYESwapPair.sol
...
// File: contracts8/libraries/PYESwapLibrary.sol
...
library PYESwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'f2efd9d75282ab90a94110f1106f1068050ba22d1f014f9219bef050fcc7a983' // init code hash
            )))));
    }
    ...
    function _calculateFees(address factory, address input, address output, uint amountIn, uint amount0Out, uint amount1Out, bool isExcluded) internal returns (uint amount0Fee, uint amount1Fee) {
        IPYESwapPair pair = IPYESwapPair(pairFor(factory, input, output));
        (address token0,) = sortTokens(input, output);
        address baseToken = pair.baseToken();
        address feeTaker = pair.feeTaker();
        uint totalFee = feeTaker != address(0) ? IToken(feeTaker).getTotalFee() : 0;
    ...
    function _calculateAmounts(address factory, address input, address output, address token0, bool isExcluded) internal returns (uint amountInput, uint amountOutput) {
        IPYESwapPair pair = IPYESwapPair(pairFor(factory, input, output));
...
// File: contracts8/libraries/TransferHelper.sol
...
// File: contracts8/interfaces/IPYESwapRouter01.sol
...
// File: contracts8/interfaces/IPYESwapRouter.sol
...
// File: contracts8/SupportingSwap.sol
...
// File: contracts8/PYESwapRouter.sol
...
contract PYESwapRouter is SupportingSwap {
    ...
    constructor(address _factory, address _WETH, address _BUSD, uint8 _adminFee, address _adminFeeAddress, address _adminFeeSetter) {
        require(_factory != address(0) && _WETH != address(0) && _BUSD != address(0), "PYESwap: INVALID_ADDRESS");
        factory = _factory;
        WETH = _WETH;
        BUSD = _BUSD;
