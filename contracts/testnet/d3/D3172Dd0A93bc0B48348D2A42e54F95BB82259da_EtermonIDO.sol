...
contract EtermonIDO is LockRegistry, WhitelistedRole, ReentrancyGuard {
    ...
    function addWhitelisted(address[] memory accounts) public onlyWhitelistAdmin {
        for (uint8 i=0; i<accounts.length; i++) {
            _slotCounter += 1;
            require(_slotCounter <= MAX_SLOT, "EtermonIDO: Exceeded the limitation of the number of participants");
    ...
    function _vestingSchedule(uint256 totalAllocation, uint64 timestamp, address beneficiary) internal view virtual returns (uint256) {
        ...
        uint256 amount = ((timestamp - startReleasedTime()) / MONTH_IN_SECONDS) * fixedAllocationForBeneficiaryPerMonth(beneficiary);
    ...
    function _release(address beneficiary) private {
        require((uint64(block.timestamp) - startReleasedTime()) / MONTH_IN_SECONDS >= 1 && _monthReleased[beneficiary][uint8((uint64(block.timestamp) - startReleasedTime()) / MONTH_IN_SECONDS)] != true, "EtermonIDO: this time has already released");
