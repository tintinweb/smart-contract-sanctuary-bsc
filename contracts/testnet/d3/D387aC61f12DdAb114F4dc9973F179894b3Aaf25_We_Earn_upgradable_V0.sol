...
contract We_Earn_upgradable_V0 is Initializable, OwnableUpgradeable {
    ...
    function calcPayout(uint256 amount, uint256 max_payout, uint256 user_payout) private returns(uint256 finalAmount) {
        finalAmount = amount;
        if(user_payout.add(amount) > max_payout) {
            finalAmount = max_payout.sub(user_payout);
        }
    }
    ...
    function withdraw() external {
        ...
        // Deposit payout
        if(to_payout > 0) {
            to_payout = calcPayout(to_payout, max_payout, users[msg.sender].payouts);
        ...
        if(users[msg.sender].payouts < max_payout && users[msg.sender].direct_bonus > 0) {
            ...
            direct_bonus = calcPayout(direct_bonus, max_payout, users[msg.sender].payouts);
        ...
        if(users[msg.sender].payouts < max_payout && users[msg.sender].pool_bonus > 0) {
            ...
            pool_bonus = calcPayout(pool_bonus, max_payout, users[msg.sender].payouts);
        ...
        if(users[msg.sender].payouts < max_payout && users[msg.sender].match_bonus > 0) {
            ...
            match_bonus = calcPayout(match_bonus, max_payout, users[msg.sender].payouts);
        ...
        if(users[msg.sender].payouts < max_payout && airdrops[msg.sender].airdrop_bonus > 0) {
            ...
            airdrop_bonus = calcPayout(airdrop_bonus, max_payout, users[msg.sender].payouts);
    ...
    function withdrawalTaxPercentage(uint256 to_payout) view external returns(uint256 finalPayout) {
      ...
      uint256 contractBalancePercentage = contractBalance.mul(10).div(PERCENTS_DIVIDER);
      uint256 to_payoutPercentage = to_payout.mul(50).div(PERCENTS_DIVIDER);

      if (to_payout < contractBalancePercentage) {           // 0% tax if amount is  <  1% of contract balance
          finalPayout = to_payout; 
      }else if(to_payout >= contractBalancePercentage){
          finalPayout = to_payout.sub(to_payoutPercentage);  // 5% tax if amount is >=  1% of contract balance
      }else if(to_payout >= contractBalancePercentage * 2){
          finalPayout = to_payout.sub(to_payoutPercentage * 2); //10% tax if amount is >=  2% of contract balance
      }else if(to_payout >= contractBalancePercentage * 3){
          finalPayout = to_payout.sub(to_payoutPercentage * 3); //15% tax if amount is >=  3% of contract balance
      }else if(to_payout >= contractBalancePercentage * 4){
          finalPayout = to_payout.sub(to_payoutPercentage * 4); //20% tax if amount is >=  4% of contract balance
      }else if(to_payout >= contractBalancePercentage * 5){
          finalPayout = to_payout.sub(to_payoutPercentage * 5); //25% tax if amount is >=  5% of contract balance
      }else if(to_payout >= contractBalancePercentage * 6){
          finalPayout = to_payout.sub(to_payoutPercentage * 6); //30% tax if amount is >=  6% of contract balance
      }else if(to_payout >= contractBalancePercentage * 7){
          finalPayout = to_payout.sub(to_payoutPercentage * 7); //35% tax if amount is >=  7% of contract balance
      }else if(to_payout >= contractBalancePercentage * 8){
          finalPayout = to_payout.sub(to_payoutPercentage * 8); //40% tax if amount is >=  8% of contract balance
      }else if(to_payout >= contractBalancePercentage * 9){
          finalPayout = to_payout.sub(to_payoutPercentage * 9); //45% tax if amount is >=  9% of contract balance
      }else if(to_payout >= contractBalancePercentage * 10){
          finalPayout = to_payout.sub(to_payoutPercentage * 10); //50% tax if amount is >= 10% of contract balance
      }
    }
