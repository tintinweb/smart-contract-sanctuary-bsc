    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        _minStakingValue = 1000000000000000000;
    ...
    function _stake(
        ...
        if(_isStakingValueFixed) {
            require(
            _isValueEqualToStakingValueFixed(_value),
            "Value is not equal to staking value set by admin."
        );

        require(
            _isDurationEqualToStakingValueFixedDuration(_duration),
            "Value is not equal to staking value set by admin."
        );
    ...
    function _convertToAnotherDecimals(uint256 _valueInWei, uint8 _fromDecimals, uint8 _toDecimals) external pure returns (uint256) {
    ...
    function _isValueEqualToStakingValueFixed(uint256 _value) private view returns (bool) {
        ...
        for(uint16 i; i < stakingValuesFixedLength; i++) {
            if(_value == stakingValuesFixed[i] * 10 ** IERC20_EXTENDED(_tokenContract).decimals()) {
                return true;
            }
        }
    ...
    function _isDurationEqualToStakingValueFixedDuration(uint256 _value) private view returns (bool) {
        uint256[] memory stakingDuration = _stakingDurations;
        uint256 stakingDurationLength = stakingDuration.length;

        for(uint16 i; i < stakingDurationLength; i++) {
            if(_value == stakingDuration[i]) {
                return true;
            }
        }
