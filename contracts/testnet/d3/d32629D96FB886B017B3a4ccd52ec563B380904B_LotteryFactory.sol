...
contract Lottery is ERC721, ILottery, IDistribution, VRFConsumerBase, Ownable, ReentrancyGuard, Initializable {
    using EnumerableSet for EnumerableSet.UintSet;
    using SafeERC20 for IERC20;
    
    enum State { OPEN, CLOSED, FAILED }
    
    uint256 constant public MAX_LOTTERY_DURATION = 90 days;
    uint256 constant private CHAINLINK_FEE = 2 ether;
    bytes32 constant private KEY_HASH = 0xAA77729D3466CA35AE8D28B3BBAC7CC36A5031EFDC430821C02BC31A238AF445;
    ...
    event LotteryHasBeenDrawn(address winner, uint256 winningToken);
    
    modifier onlyStakingPools {
        require(
            msg.sender == kingsCastle ||
            msg.sender == seaOfRedemption,
            "Lottery: caller is not one of the staking pools"
        );
        _;
    }
    ...
    function buyTickets(uint256 _amount) external nonReentrant {
        ...
        IERC20(BUSD).safeTransferFrom(msg.sender, address(this), _amount * price);
    ...
    function amountOfWinningTokens() external view returns (uint256) {
        ...
        override
    {
        ...
        IKingsCastle(kingsCastle).updatePool(winningToken);
        ISeaOfRedemption(seaOfRedemption).updatePool(winningToken, previousSupply, currentSupply);
        address winner = ownerOf(winningToken);
        _distribute(winner);
        _startNewLottery();
        statePerLottery[nonce] = State.CLOSED;
        emit LotteryHasBeenDrawn(winner, winningToken);
    }
    ...
    function _distribute(address _winner) private {
        IERC20(BUSD).safeTransfer(kingsCastle, distribution.toKingsCastle);
        IERC20(BUSD).safeTransfer(seaOfRedemption, distribution.toSeaOfRedemption);
        IERC20(BUSD).safeTransfer(devWallet, distribution.toDevWallet);
        IERC20(BUSD).safeTransfer(bdmWallet, distribution.toBdmWallet);
        IERC20(BUSD).safeTransfer(_winner, distribution.toWinner);
    }
}
...
interface IKingsCastle {
    function updatePool(uint256 _tokenId) external;
}

//SPDX-License-Identifier: MIT
pragma solidity 0.8.7;

interface ISeaOfRedemption {
    function updatePool(uint256 _tokenId, uint256 _lhs, uint256 _rhs) external;
}
