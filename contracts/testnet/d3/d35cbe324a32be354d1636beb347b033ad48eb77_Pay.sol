...
contract Pay {
    ...
    event eventSendRecipientFinish(
        ...
        uint256 percentRecipient,
        uint256 percentCashback,
        address addressTokensCashback
    );

    //
    //send recipient
    event eventSendRecipient(uint256 percentRecipient, address addressRecipient, address addressTokenRecipient, uint256 amountTokenRecipient, uint256 amountUsedTokensSender, string transferType);
    ...
    function sendRecipient(address addressRecipient, address addressTokenRecipient, uint256 amountTokenRecipient, address addressTokenSender, uint256 amountTokenSender, address addressSwapRouter, uint256 amountOutMinCashback, string calldata paymentInfo) payable public {
        ...
        amountGeneratedArr[4] = SafeMath.sub(amountArr[1], amountGeneratedArr[2]); // amountTokenSender - (100 - CashbackPercent)% TokenSender
        //amountGeneratedArr[5] = SafeMath.sub(amountArr[0], amountGeneratedArr[3]); // amountTokenRecipient - (100 - CashbackPercent)% TokenRecipient
        ...
        if (_cashbackToken != addressArr[2] && amountGeneratedArr[4] > 0) {
            if(contractRouter02.WETH()==addressArr[2]){
                ...
                amountRefundArr[0] = SafeMath.sub(amountRefundArr[0], amounts[0]);
                emit eventSendCashbackSender(CashbackPercent, msg.sender, _cashbackToken, amounts[amounts.length - 1], amounts[0], "swapExactETHForTokens");
            } else if (contractRouter02.WETH()==_cashbackToken){
                address[] memory path = new address[](2);
                path[0] = addressArr[2];
                path[1] = contractRouter02.WETH();
                uint[] memory amounts = contractRouter02.swapExactTokensForETH(amountGeneratedArr[4], amountArr[2], path, msg.sender, block.timestamp);
                amountRefundArr[1] = SafeMath.sub(amountRefundArr[1], amounts[0]);
                emit eventSendCashbackSender(CashbackPercent, msg.sender, _cashbackToken, amounts[amounts.length - 1], amounts[0], "swapExactTokensForETH");
            } else {
                address[] memory path = new address[](3);
                path[0] = addressArr[2];
                path[1] = contractRouter02.WETH();
                ...
                amountRefundArr[1] = SafeMath.sub(amountRefundArr[1], amounts[0]);
                emit eventSendCashbackSender(CashbackPercent, msg.sender, _cashbackToken, amounts[amounts.length - 1], amounts[0], "swapExactTokensForTokens");
            }
        }
