...
contract C250Gold is ERC20, Ownable, ReentrancyGuard {
    ...
    function setPresaleTicket1(address addr) external
        onlyOwner {
    ...
    function setPresaleTicket2(address addr) external
        onlyOwner {
        require(!live, "Already launched");
        ...
        external
    {
        ...
        if (amount1 > 0) {
            IERC20(presaleTicket1).transferFrom(msg.sender, address(this), amount1);
        }
        
        uint256 amount2 = IERC20(presaleTicket2).balanceOf(msg.sender);
        if (amount2 > 0) {
            IERC20(presaleTicket2).transferFrom(msg.sender, address(this), amount2);
        }
    ...
    function transferInternal(address to, uint256 amount) external {
        ...
        require(presaleBalance[sender] >= amount, "Insufficient presale balance");
