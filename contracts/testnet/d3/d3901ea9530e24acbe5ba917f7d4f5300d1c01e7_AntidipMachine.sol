...
contract AntidipMachine is Context, Ownable , ReentrancyGuard {
    ...
    uint256 wFee = withdrawFee(aval_withdraw); // test change
    uint256 totalAmountToWithdraw = SafeMath.sub(aval_withdraw,wFee); //me changed from aval_withdraw to aval_withdraw2
    BusdInterface.transfer(msg.sender,totalAmountToWithdraw);
    BusdInterface.transfer(dev,wFee);
    approvedWithdrawal[msg.sender] = userWithdrawal(msg.sender,aval_withdraw); // me changed from 0 to half of the amount stay in in his contract
    ...
    uint256 totalAmount = SafeMath.add(amount,aval_withdraw); //me it will add one of his half to total withdraw
