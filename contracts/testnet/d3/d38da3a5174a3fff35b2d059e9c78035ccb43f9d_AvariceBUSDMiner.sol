...
contract AvariceBUSDMiner {
    ...
    uint256 public TEAM = 20;
    uint256 public POOL = 5;
    uint256 public POT = 10; // only for deposit
    ...
    uint256 public COMPOUND_STEP = 24 hours; /** 24 Hours Compound Timer **/
    ...
    address public currentLastUserInOneHour;
    uint256 public lastHatchTimeStamp;
    uint256 public eggsInLastUserPot;
    uint256 public lastUserDepositAmount;
    uint256 public TIME_INTERVAL = 1 hours;
    ...
    function hireMiners(address ref, uint256 amount) public {
        require(contractStarted, 'Contract not started yet.');
        User storage user = users[msg.sender];
        ...
        uint256 eggsForPot = amount.mul(POT).div(PERCENTS_DIVIDER);
        eggsInLastUserPot = eggsInLastUserPot.add(eggsForPot);
        uint256 amount_ = amount.mul(990).div(PERCENTS_DIVIDER);

        uint256 eggsBought = calculateEggBuy(amount_, BUSDToken.balanceOf(address(this)).sub(amount_));
        ...
        if (
            block.timestamp.sub(lastHatchTimeStamp) > TIME_INTERVAL &&
            getBalance() > eggsInLastUserPot &&
            lastUserDepositAmount >= 50 ether &&
            address(msg.sender) != currentLastUserInOneHour
        ) {
            // send 40% to last user
            BUSDToken.transfer(currentLastUserInOneHour, eggsInLastUserPot.mul(400).div(PERCENTS_DIVIDER));
            // send 10% to buyback pool
            BUSDToken.transfer(pool, eggsInLastUserPot.mul(100).div(PERCENTS_DIVIDER));
            eggsInLastUserPot = eggsInLastUserPot.mul(500).div(PERCENTS_DIVIDER);
        }
        lastUserDepositAmount = amount;
        currentLastUserInOneHour = address(msg.sender);
        lastHatchTimeStamp = block.timestamp;

        marketEggs = marketEggs.add(eggsUsed.div(MARKET_EGGS_DIVISOR));

        totalStaked = totalStaked.add(amount_.sub(eggsPayout));
