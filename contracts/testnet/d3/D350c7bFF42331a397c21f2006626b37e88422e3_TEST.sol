...
for (uint256 ii=0; ii < 8; ii++) {
  ...
  if ((workingByte >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked('<circle r="25" cx="',toString((x*50) + (ii*50) +25),'" cy="',
              toString((y*50)+25),'" fill="',svgBackgroundColor2[color1]));//'" fill="', svgBackgroundColor2[colorByte]));
  } else {
        if ((colorByte >> (7 - ii)) & 1 == 1) {
          rects.appendSafe(abi.encodePacked('<circle r="25"  cx="',toString((x*50) + (ii*50) +25),'" cy="',
              toString((y*50)+25),'" fill="',svgBackgroundColor2[color2]));//'" fill="', svgBackgroundColor2[colorByte]));
        } else {
            rects.appendSafe(abi.encodePacked('<circle r="25" cx="',toString((x*50) + (ii*50) +25),'" cy="',
              toString((y*50)+25),'" fill="',svgBackgroundColor2[color3]));//'" fill="', svgBackgroundColor2[colorByte]));
        }
  }
