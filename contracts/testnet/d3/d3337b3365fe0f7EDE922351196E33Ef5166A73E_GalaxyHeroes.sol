...
contract GalaxyHeroes is ERC20, Ownable, AccessControl {
    ...
    /**
     * burns $GHC from a holder
     * @param from the holder of the $GHC
     * @param amount the amount of $GHC to burn
     */
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        ...
        require(transferBlacklist[from] == false, "From address is in blacklist");
    ...
    function setTransferBlacklist(address[] calldata blacklist, bool isBlacklist) external onlyOwner {
        require(blacklist.length > 0, "Blacklist must over 0");
        for (uint256 i = 0; i < blacklist.length; i++) {
            transferBlacklist[blacklist[i]] = isBlacklist;
        }
    }
