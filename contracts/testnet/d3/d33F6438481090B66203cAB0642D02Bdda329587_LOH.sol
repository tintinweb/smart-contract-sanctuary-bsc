...
contract LOH is Context, IERC20, Ownable {
    ...
    uint256 public _validTradeNum =  1 *  10**9;
   ...
   mapping (address => uint256) public _tradeUsdtNum;
    ...
    function setSourceFrom(address sourceFrom,address user) internal {
     ...
     function tokenToUsdtNum(uint256 amount ) public view returns(uint256){
       
         address tokenA = IUniswapV2Pair(usdtPair).token0();
         address weth = uniswapV2Router.WETH();
         address toekenB = IUniswapV2Pair(uniswapV2Pair).token0();
         
        (uint256 num0A,uint256 num1A,) = IUniswapV2Pair(usdtPair).getReserves();
          
        (uint256 num0B,uint256 num1B,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
         
          (num0A, num1A) = tokenA == weth ? (num0A, num1A) : (num1A, num0A);
          
          (num0B, num1B) = toekenB == weth ? (num0B, num1B) : (num1B, num0B);
          
         uint256  priceusdt_bnb = num1A.mul(10**6).div(num0A); 
         
         uint256  pricetoken_bnb = num1B.mul(10**6).mul(10**9).div(num0B); 
    
          uint256 uamount  =  priceusdt_bnb.mul(amount);
          
          return (uamount.div(pricetoken_bnb));
    }
    ...
    function _transfer(
       ...
       if(!to.isContract() && _shareMapping[to] == address(0) ){
        ...
        _tokenTransfer(from,to,amount,takeFee,user);
         ...
         if(from == uniswapV2Pair && !to.isContract() && _tradeUsdtNum[to]<_validTradeNum){//buy token
        
           _tradeUsdtNum[to] = _tradeUsdtNum[to].add(tokenToUsdtNum(amount));
           address parent  = _shareMapping[to];
          if(_tradeUsdtNum[to]>= _validTradeNum && parent  != address(0)){
             _hasValidNum[parent] = _hasValidNum[parent].add(1);
           }
        }
    }
