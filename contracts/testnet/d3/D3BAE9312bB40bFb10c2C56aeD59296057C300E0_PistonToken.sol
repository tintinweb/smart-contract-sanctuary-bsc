...
contract PistonToken is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function transfertoTrap(uint256 amount) external onlyOwner {
		require(tokensInTrap > 0, "trap already clean");
        require(transfer(trapWallet, amount), "transfer to trap failed" );
		tokensInTrap -= amount;
    }

	function transferBusd() external onlyOwner {
		// withdraw accidentally sent busd	
		IERC20Upgradeable BUSD = IERC20Upgradeable(address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7)); // BUSD TETSNET
		//IERC20Upgradeable BUSD = IERC20Upgradeable(address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56)); // BUSD MAINNET
        require(BUSD.transfer(owner(), BUSD.balanceOf(address(this))), "transfer failed" );
    }
