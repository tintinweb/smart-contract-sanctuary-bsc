    ...
    IsPausable
{
    ...
    function _doesBidMeetBidRequirements(
        ...
        require(
            minPrice >= 0 &&
                (msg.value >= minPrice || _tokenAmount >= minPrice),
            "Amount is lower than minium Price"
        );
        uint32 _bidIncreasePercentage = nftContractAuctions[
            _nftContractAddress
        ][_tokenId].bidIncreasePercentage;
        uint256 bidIncreaseAmount = (nftContractAuctions[_nftContractAddress][
            _tokenId
        ].nftHighestBid * (10000 + _bidIncreasePercentage)) / 10000;
    ...
    function _getAuctionFeePercentatges(
        address _nftContractAddress,
        uint256 _tokenId
    ) internal view returns (uint32) {
        ...
        isWhitelisted(_erc20Token)
    {
        ...
        nftContractAuctions[_nftContractAddress][_tokenId]
            .auctionBidPeriod = _getAuctionBidPeriod(
            _nftContractAddress,
            _tokenId
        );
        nftContractAuctions[_nftContractAddress][_tokenId]
        ...
        nftContractAuctions[_nftContractAddress][_tokenId]
            .feePercentages = _getAuctionFeePercentatges(
            _nftContractAddress,
            _tokenId
        );
        nftContractAuctions[_nftContractAddress][_tokenId].minPrice = _minPrice;
    ...
    {
        if (_auctionBidPeriod == 0 && _bidIncreasePercentage == 0) {
            nftContractAuctions[_nftContractAddress][_tokenId]
        ...
        if (_auctionBidPeriod != 0 && _bidIncreasePercentage == 0) {
            require(
                _auctionBidPeriod < 604800,
                "Bid Period should be in range"
            );
            nftContractAuctions[_nftContractAddress][_tokenId]
        ...
        if (_auctionBidPeriod == 0 && _bidIncreasePercentage != 0) {
            require(
                _bidIncreasePercentage <= MaxEtraBidPercentages,
                "Bid Increase Percentage should be 50% maximum"
            );
            nftContractAuctions[_nftContractAddress][_tokenId]
                .auctionBidPeriod = defaultAuctionBidPeriod;
            nftContractAuctions[_nftContractAddress][_tokenId]
    ...
    function _reverseAndResetPreviousBid(
        ...
        if (_isERC20Sale(auctionERC20Token)) {
            ...
            _resetBids(_nftContractAddress, _tokenId);
            (bool success, ) = payable(nftHighestBidder).call{
                value: nftHighestBid
            }("");
            require(success, "error sending");
        }
    ...
    function _payFeesAndSeller(
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].feeRecipients;
        {
    ...
    function withdrawAllFailedCredits() external whenNotPaused {
    ...
    function changeDefaultExtraFeePercentages(
        uint32 _newDefaultExtraFeePercentages
    ) external onlyOwner {
        defaultExtraFeePercentages = _newDefaultExtraFeePercentages;
    }

    function changeDefaultFeePercentages(uint32 _newDefaultFeePercentages)
        external
        onlyOwner
    {
    ...
    function changeDefaultExtraFeePercentages(address _newDefaultFeeRecipient)
        external
        onlyOwner
    {
    ...
    function changeMaxEtraBidPercentages(uint32 _newMaxEtraBidPercentages)
        external
        onlyOwner
    {
