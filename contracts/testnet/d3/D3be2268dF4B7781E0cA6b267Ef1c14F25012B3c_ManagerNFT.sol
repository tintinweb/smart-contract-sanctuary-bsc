...
contract ManagerNFT is ERC721Enumerable, AccessControl {
  ...
  constructor() ERC721("ManagerNFT", "MNG-Card") {
  ...
  function mint(address _to, string memory _tokenURI) public onlyRole(MINTER_ROLE) {
    require(bytes(_tokenURI).length > 0, "ManagerNFT: tokenURI not valid");
  ...
  function changeTokenURI(uint _tokenID, string memory _tokenURI) public onlyRole(MINTER_ROLE) {
    require(bytes(_tokenURI).length > 0, "ManagerNFT: tokenURI not valid");
  ...
  */
  function tokenURI(uint _tokenID) public view virtual override returns (string memory) {
    require(_exists(_tokenID), "ManagerNFT: tokenID not exist");
