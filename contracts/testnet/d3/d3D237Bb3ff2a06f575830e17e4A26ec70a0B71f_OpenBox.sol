...
//    mapping()
    ...
    function setItems(uint index, string memory itemHash) external onlyOwner {
        items[index] = itemHash;
    }
    ...
    function handleOpen(uint nonce, uint tokenId) internal {
        ...
        nonces[nonce] = true;
//        ITokenERC721(items721).safeMint(_msgSender(), items[resultNumber]);
        IERC721(items721).mint(_msgSender(), tokenId, 'QmPsnYENSjjJcowLy2eULfCQNxnrvmE2FyGYyfS6wG1Bd8');
    }
    function open(uint nonce, uint[] memory tokenIds, uint[] memory amounts) public {
        ...
        for(uint i = 0; i < amounts.length; i++) {
            handleOpen(nonce+random(i, 1000), tokenId+1+i);
        }
