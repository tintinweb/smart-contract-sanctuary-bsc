...
contract METStake is ModuleBase, Lockable, ECDSA {
    ...
    function getStakeData(uint256 index) external view returns (
       ...
       (res, owner, amount, rtime, rrate, stime, level, claimed) = _getStakeData(index);
    }

    function _getStakeData(uint256 index) internal view returns (
        bool res,
        address owner,
        uint256 amount,
        uint256 rtime,
    ...
    function stakeMET(uint256 amount, uint256 stakeTime, uint256 rewardRate, uint256 level, bytes memory signature) external lock {
        ...
        mapStake[++stakeLength] = StakeData(msg.sender, amount, stakeTime, rewardRate, level, block.timestamp, false, true);
