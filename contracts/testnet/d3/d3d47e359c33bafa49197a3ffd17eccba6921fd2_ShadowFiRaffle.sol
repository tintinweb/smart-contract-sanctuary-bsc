...
contract ShadowFiRaffle is Ownable, ReentrancyGuard {
    ...
    event Ticket(address user, uint256 ticketAmount);
    ...
    function setDeadAccrual(uint256 _raffleId, uint256 _newDeadAccrual) public onlyOwner {
        require(_raffleId > currentRaffleId || block.timestamp <= raffleInfos[currentRaffleId].startTime);
        raffleInfos[_raffleId].deadAccrual = _newDeadAccrual;
    }
    ...
    function revealWinner() public onlyCourt {
        ...
        uint256 sessionTime = raffleInfos[currentRaffleId + 1].sessionTime;
        uint256 deadAccrual = raffleInfos[currentRaffleId + 1].deadAccrual;

        if (sessionTime == 0) {
            sessionTime = raffleInfos[currentRaffleId].sessionTime;
        }

        if (deadAccrual == 0) {
            deadAccrual = raffleInfos[currentRaffleId].deadAccrual;
        }

        _startSession(currentRaffleId + 1, sessionTime, deadAccrual);
    }

    function startSession(uint256 _sessionId, uint256 _sessionTime, uint256 _deadAccrual) public onlyOwner {
        require(raffleInfos[currentRaffleId].startTime == 0, "You already initiated the raffle contract.");
        _startSession(_sessionId, _sessionTime, _deadAccrual);
    }

    function _startSession(uint256 _sessionId, uint256 _sessionTime, uint256 _deadAccrual) internal {
    ...
    function getRaffleInfo(uint256 _raffleID) public view returns(
        ...
        return (
            raffleInfos[_raffleID].lastSoldTicketNo,
            raffleInfos[_raffleID].endTime,
            raffleInfos[_raffleID].winningNo,
            raffleInfos[_raffleID].deadAccrual,
            raffleInfos[_raffleID].blockHashValue,
            raffleInfos[_raffleID].blockNumber
        );
    }
