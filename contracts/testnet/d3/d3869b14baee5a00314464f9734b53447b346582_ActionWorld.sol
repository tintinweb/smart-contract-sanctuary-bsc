...
contract ActionWorld {
    ...
    function invest(address referrer, uint256 amount) public {
        require(amount * 1e18 >= INVEST_MIN_AMOUNT, "Min invesment 100 Tokens");
        BEP20 t = BEP20(tokenAddress);
        uint256 approveValue = t.allowance(msg.sender, address(this));
        uint256 balanceOfowner = t.balanceOf(msg.sender);

        require(approveValue >= amount * 1e18, "Insufficient Balance Ap");
        require(balanceOfowner >= amount * 1e18, "Insufficient Balance");
        ...
        t.transferFrom(msg.sender, admin, _fees * 1e18);
        t.transferFrom(msg.sender, address(this), _remainAmoount * 1e18);
    ...
    function withdrawal() external {
        ...
        require(
            balanceOfAddress >=
                _fees * 1e18 + actualAmountToSend.mul(withdrwal).div(100) * 1e18,
            "Insufficient Balance"
        );

        t.transfer(
            msg.sender,
            actualAmountToSend.mul(withdrwal).div(100) * 1e18
        );
        t.transfer(admin2, _fees * 1e18);
