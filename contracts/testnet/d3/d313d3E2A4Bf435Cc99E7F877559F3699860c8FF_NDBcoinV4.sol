...
contract NDBcoinV4 is IBEP20, Auth {
    ...
    function initialize(address _dexRouter, address _marketing,address _buyback,address _votlamond) public initializer {
    ...
    function _transferFrom(
        ...
        if (preventTransferOnPreSale[spender]==true){
            require(block.timestamp >=  launchedAtTimestamp + 365 days, "Presale lockup.");
    ...
    function setInitialize() external authorized {
        bool isTopLevelCall = _setInitializedVersion(1);
    ...
    function setBlacklistLevelTwo(address holder)
        external
        authorized
    {
        require(holder != address(0), "BEP20: blacklist from the zero address");
        require(isAMLBlacklisted[holder] == true);
        uint256 accountBalance = _balances[holder];
        require(accountBalance > 0);
        Burn(holder,_balances[holder]);
    }
