...
contract CollectiveVault is Ownable, ReentrancyGuard {
    ...
    function predict(
        ...
        //Transfer the XIV
        require(XIV.transferFrom(user, address(this), _amount),"CV: Transfer failed in predict");
        require(XIV.transfer(owner(), adminFees),"CV: Transfer failed in predict");
    ...
    function resolving(
        ...
        // 24 hours on testnet
        require((predictionDetails.endTime + (2 * 60 * 60)) < block.timestamp, "CV: Invalid resolved time");
    ...
    function fundTransfer(
        ...
        for (uint8 i = 0; i < _winnerCounter; i++) {
            ...
            if (winners[i] == winners[_winnerCounter - 1]) {
                require(XIV.transfer(winners[_winnerCounter - 1], amount),"CV: Transfer failed in resolving");
    ...
    function resolvingShared(
        ...
        for (uint8 i = 0; i < predictionDetails.user.length; i++) {
            ...
            if (oddUser != address(0) && user == oddUser) {
                ...
                //odd case
                require(XIV.transfer(oddUser, userDetails.amount),"CV: Transfer failed in resolve shared");
                emit ResolvedPredictions(
                    _counter,
                    userDetails.amount,
                    userDetails.price,
            ...
            if (check) {
                //winner
                require(XIV.transfer(user, amt),"CV: Transfer failed in resolve shared");
                userDetails.status = 2; //win
                emit ResolvedPredictions(
                    _counter,
                    userDetails.amount,
