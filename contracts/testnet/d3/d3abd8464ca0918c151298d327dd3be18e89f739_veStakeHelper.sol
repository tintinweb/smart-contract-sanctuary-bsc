...
contract veStakeHelper is Ownable {
    ...
    constructor (IgDED _gEde, IEdeDistributor _distributorEDE, IEdeDistributor _distributorFEE) {
        gEde = _gEde;
    ...
    function increase_amount_helper(uint256 _value, uint256 _unlock_time) external {
        gEde.increase_amount(msg.sender, _value);
        distributor_EDE.checkpointOtherUser(msg.sender);
        distributor_FEE.checkpointOtherUser(msg.sender);
    }

    function increase_unlock_time_helper(uint256 _value, uint256 _unlock_time) external {
