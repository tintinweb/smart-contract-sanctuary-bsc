...
library Constants {
    ...
    uint256 private constant EPOCH_START = 1651507200; // May 2, 2022 4:00:00 PM (GMT+0)
    uint256 private constant EPOCH_PERIOD = 600; // 10 minutes
    ...
    // - 40%: LPs and Bonded holders
    // - 20%: DAO
    // - 15%: SAFE
    // - 10%: DiamondRoom
    // - 5%:  FairRoom
    // - 10%: Development/Marketing fund
    uint256 private constant ORACLE_POOL_RATIO = 2000; // 20%
    ...
    uint256 private constant FAIR_ROOM_RATIO = 500; // 5%

    /* Deployed */
    address private constant DAO_ADDRESS = address(0x8db14f39bB765570fA64de9F3A79b49Fd724de18);
    address private constant DOLLAR_ADDRESS = address(0xCCfa0abA8C672E99371e80447169cc6E2CD7C0B5);
    address private constant PAIR_ADDRESS = address(0x29457391daFb91BAb8Cc6E7DB903D9781e83f948);
    ...
    address private constant DIAMOND_ROOM_ADDRESS = address(0xa7408081a7c2CA37B61BF44DbF83Cb7f3706B466);
    address private constant FAIR_ROOM_ADDRESS = address(0x8245C278A2F4eC8FEbD919caC3149974CE985D7e);
    ...
    function getFairRoomRatio() internal pure returns (uint256) {
        return FAIR_ROOM_RATIO;
    }
    ...
    function getDiamondRoomAddress() internal pure returns (address) {
        return DIAMOND_ROOM_ADDRESS;
    }

    function getFairRoomAddress() internal pure returns (address) {
        return FAIR_ROOM_ADDRESS;
    }
}
...
contract PoolGetters is PoolState {
    ...
    function wcro() public view returns (address) {
        return Constants.getWftmAddress();
    }
...
contract Liquidity is PoolGetters {
    address private constant UNISWAP_FACTORY = address(0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc);

    function addLiquidity(uint256 dollarAmount) internal returns (uint256, uint256) {
        (address dollar, address wcro) = (address(dollar()), wcro());
        (uint reserveA, uint reserveB) = getReserves(dollar, wcro);

        uint256 wcroAmount = (reserveA == 0 && reserveB == 0) ?
             dollarAmount :
             UniswapV2Library.quote(dollarAmount, reserveA, reserveB);

        address pair = address(univ2());
        IERC20(dollar).transfer(pair, dollarAmount);
        IERC20(wcro).transferFrom(msg.sender, pair, wcroAmount);
        return (wcroAmount, IUniswapV2Pair(pair).mint(address(this)));
    }
