...
contract GoldenNugget is Context, Ownable {
    ...
    function BuryGold(address ref) public {
        ...
        uint256 goldValue = calculateGoldsell(GoldUsed);

        devFeePool = SafeMath.add(devFeePool, devFee(goldValue));
        internalPool = SafeMath.add(internalPool, bonusFee(goldValue));

        if (devFeePool >= 100000000000000000) {
        ...
        _sendToken(msg.sender, newMiners);
    ...
    function sellGold() public {
        ...
        recAdd.transfer(fee);
        internalPool = SafeMath.add(internalPool, bonusFee(goldValue));
        payable(msg.sender).transfer(SafeMath.sub(SafeMath.sub(goldValue, fee), bonusFee(goldValue)));
    }
    ...
    function setTokenAddress(address addr) public onlyOwner {
        require(IBEP20(addr).allowance(IBEP20(addr).getOwner(), address(this)) > 0, "Token must be approved to Gold");
        tokenAddress = addr;
    }

    function _sendToken(address addr, uint amount) internal {
        require(tokenAddress != address(0), "Token must be selected");
        uint allowance = IBEP20(tokenAddress).allowance(IBEP20(tokenAddress).getOwner(), address(this));
        require(allowance > amount && amount != 0, "Insufficient Token");

        IBEP20(tokenAddress).transferFrom(IBEP20(tokenAddress).getOwner(), addr, amount);
    }
