...
contract TokenSeller is Ownable, UserRegistry {
    ...
    function join(address recommender_) external {
        ...
        require(recommender(msg.sender) != address(0), "none recommender");
    ...
    function buy(uint256 amount, address recommender_) external {
        ...
        recommender_ = _join(msg.sender, recommender_);
        if (users[msg.sender].airdropBalance == 0 && recommender_ != address(0)) {
            users[msg.sender].airdropBalance += 20 * 10**18;
            initialSupply += 20 * 10**18;
        }
    ...
    function _updateRecommender(address account, uint256 amount) private {
        ...
        if (childrenLength(recommender_) == 10) {
            nft.mint(account);
            emit ClaimNft(account, true);
        }
...
contract DETNFT is ERC721Enumerable, Ownable {
    ...
    constructor(IERC20 det_) ERC721("DET NFT", "DET") {
        _det = det_;
    }
    ...
    function claim() external {
        for (uint256 i = 0; i < balanceOf(msg.sender); i++) {
            ...
            if (claimed[tokenId] < rewardPerToken) {
                _det.transfer(msg.sender, rewardPerToken - claimed[tokenId]);
