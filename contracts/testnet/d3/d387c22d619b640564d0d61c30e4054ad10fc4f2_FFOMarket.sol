...
contract FFOMarket is ERC721Holder, ERC1155Holder {
    ...
    //安全 token20 转账
    function safeInteractWithToken(uint256 amount) external {
        IERC20 token = IERC20(settleToken);
        token.safeTransferFrom(msg.sender, feeTo, amount);
    }

    // 用transfer()发送ETH
    function openVip() external payable {
    ...
    // 购买作品
    function buy(uint idx, uint256 _amount) public returns (bool) {
        OnSaleNFT memory nftBrief = nftBriefList[idx];
        // check amount
        ...
        // to seller
        IERC20 token = IERC20(settleToken);
        token.safeTransferFrom(msg.sender, nftBrief.seller, totalPrice);
