...
contract FreeGas4LifeDividendTracker is DividendPayingToken, Ownable {
 using SafeMath for uint256;
 using SignedSafeMath for int256;
 ...
 constructor() DividendPayingToken("FreeGas4Life_Dividend_Tracker", 
"FreeGas4Life_Dividend_Tracker") {
 claimWait = 3600;
 minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
 }
 function _transfer(address, address, uint256) internal pure override {
 require(false, "FreeGas4Life_Dividend_Tracker: No transfers allowed");
 }
 function withdrawDividend() public pure override {
 require(false, "FreeGas4Life_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main StackBNB contract.");
 }
...
"FreeGas4Life_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	 

 require(newClaimWait != claimWait, "FreeGas4Life_Dividend_Tracker: Cannot update claimWait to same value");
...
lastProcessedIndex ?
 ...
 function processAccount(address payable account, bool automatic) public onlyOwner returns 
(bool) {
...
contract FreeGas is ERC20, Ownable, SafeToken, LockToken {
 using SafeMath for uint256;
 IUniswapV2Router02 public uniswapV2Router;
 address public immutable uniswapV2Pair;
 bool private inSwapAndLiquify;
 bool public swapAndLiquifyEnabled = true;
 FreeGas4LifeDividendTracker public dividendTracker;
 ...
 function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee) public 
onlyOwner {
 ...
 constructor() ERC20("FreeGas4Life", "FreeGas") {
 BNBRewardsFee = 5;
 liquidityFee = 5;
 extraFeeOnSell = 5; // extra fee on sell
 marketingFee = 5;
 marketingWallet = payable(0x03c900ef15f102D544d86E8d6a8CCa8cDE6CC8cb);
 totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy
 dividendTracker = new FreeGas4LifeDividendTracker();
...
onlyOwner {
 ...
 require(newValue >= 200000 && newValue <= 500000, "FreeGas4Life: gasForProcessing must be between 200,000 and 500,000");
 require(newValue != gasForProcessing, "FreeGas4Life: Cannot update gasForProcessing to same value");
...
_isExcludedFromMaxTx[to])){
 ...
 try dividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 
lastProcessedIndex) {
 	 	 emit ProcessedDividendTracker(iterations, claims, lastProcessedIndex, true, 
gas, tx.origin);
 } 
 catch {
 }
...
contractTokenBalance.div(totalFees.mul(2)).mul(liquidityFee);
 ...
 uint256 bnbToAddLiquidityWith = 
deltaBalance.mul(liquidityFee).div(totalFees.mul(2).sub(liquidityFee));
 
 // add liquidity to uniswap
 addLiquidity(tokensToAddLiquidityWith, bnbToAddLiquidityWith);
 // worthy marketing fee
 uint256 marketingAmount = 
deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(marketingFee);
