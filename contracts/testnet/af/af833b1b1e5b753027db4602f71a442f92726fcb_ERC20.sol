...
contract TestToken1234 is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   WBNBDividendTracker public dividendTracker;

	address private marketingWallet;
    address private liquidityWallet;
    address private devWallet;
    address private _devWallet;
    address private NFTRewardsWallet;
    address private _NFTRewardsWallet;
    ...
    uint256 private previousNFTRewardsFee;
    ...
    constructor() ERC20 ("TestToken1234", "TT1234") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 1;
        _buyDevFee = 1;
		...
		_sellMarketingFee = 3;
        _sellDevFee = 1;
        _sellNFTRewardsFee = 2;

		contractTokenBalanceAmount = 420000000 * 10**9;
        
        swapTokensAtAmount = 2100000 * (10**9);
        _maxWallet = 8400000 * (10**9);
        _maxBuy = 8400000 * (10**9);
        _maxSell = 2100000 * (10**9);

        totalBuyFees = _buyRewardsFee.add(_buyLiquidityFee).add(_buyMarketingFee).add(_buyDevFee).add(_buyNFTRewardsFee);
        totalSellFees = _sellRewardsFee.add(_sellLiquidityFee).add(_sellMarketingFee).add(_sellDevFee).add(_sellNFTRewardsFee);

    	dividendTracker = new WBNBDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x4ac451fC123C4e954171EB3D6E038c38027d6af5);
        _devWallet = payable(0x02aDDA653574d203c9D6A8e1cd912D0462847F6e);
        _NFTRewardsWallet = payable(0x6A7bb43abc88C4050f34DCfBAca4E0060ee795B4);
        ...
        excludeFromFees(_NFTRewardsWallet, true);
	...
	function updateSwapAmount(uint256 amount) public onlyOwner {
	    contractTokenBalanceAmount = amount * (10**9);
	    swapTokensAtAmount = amount * (10**9);
	}

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "TestToken1234: The dividend tracker already has that address");

        WBNBDividendTracker newDividendTracker = WBNBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "TestToken1234: The new dividend tracker must be owned by the CEXOFFENDER token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "TestToken1234: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "TestToken1234: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "TestToken1234: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "TestToken1234: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TestToken1234: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TestToken1234: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "TestToken1234: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "TestToken1234: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "TestToken1234: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "TestToken1234: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
		    ...
		    uint256 swapTokens;
			uint256 swapAmount = _sellMarketingFee.add(_sellRewardsFee).add(_sellLiquidityFee).add(_sellDevFee).add(_sellNFTRewardsFee);
            ...
            if (_sellNFTRewardsFee > 0) {

            checkBiggestBuyTimer();
            uint256 devAmount = address(this).balance.mul(_sellNFTRewardsFee).div(swapAmount);
            transferToWallet(payable(NFTRewardsWallet), devAmount);
    ...
    function addLP() external onlyOwner() {
        ...
        updateMaxWallet(420000000);
        updateMaxBuySell((420000000), (420000000));
    }
    
	function letsGoLive() external onlyOwner() {
        updateBuyFees(1,1,1,1,1);
        updateSellFees(2,2,2,2,2);

        updateMaxWallet(210000000);
        updateMaxBuySell(210000000, 210000000);
    ...
    function letsGetStarted() external onlyOwner() {
        updateBuyFees(2,2,2,2,2);
        updateSellFees(3,3,3,3,3);

        updateMaxWallet(120000000);
        updateMaxBuySell(120000000, 120000000);

		trading = true;
        burning = false;
    }
    
    function updateBuyFees(uint8 newBuyLiquidityFee, uint8 newBuyMarketingFee, uint8 newBuyRewardsFee, uint8 newBuyDevFee, uint8 newBuyNFTRewardsFee) public onlyOwner {
        ...
        _buyNFTRewardsFee = newBuyNFTRewardsFee;
        
        totalFees();
    }

    function updateSellFees(uint8 newSellLiquidityFee, uint8 newSellMarketingFee, uint8 newSellRewardsFee, uint8 newSellDevFee, uint8 newSellNFTRewardsFee) public onlyOwner {
...
contract WBNBDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(false, "WBNB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TestToken1234 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "WBNB: claimWait must be updated to between 1 and 24 hours");
