...
contract RegenerateHandler is AccessControl, Pausable {
    ...
    event ItemEnergyRegenerated(uint recipeId, uint itemId, uint amount);
    ...
    function regenerateItemEnergy(uint _recipeId, uint _itemId) external whenNotPaused{
        ...
        require(recipeExists(_recipeId), "Regen recipe does not exist");
        ...
        if(itemRegenRecipes[_recipeId].RIIds.length != 0){
            ...
            resourceTracker.deductFromBalanceBatch(msg.sender, itemRegenRecipes[_recipeId].RIIds, itemRegenRecipes[_recipeId].RIAmounts);
        }
        ...
        emit ItemEnergyRegenerated(_recipeId, _itemId, itemRegenRecipes[_recipeId].regenAmount);
    }
    ...
    function addItemRegenRecipe(
        ...
        require(!recipeExists(_recipeId), "Recipe ID already exists");
        ...
        recipe.IIPrefix = checkItemPrefix(_IIPrefix);
    ...
    function removeItemRegenRecipe(uint _recipeId) public isRecipeCreator(){
        require(recipeExists(_recipeId), "Recipe ID does not exist");
    ...
    function recipeExists(uint _recipeId) public view returns (bool){
        if(itemRegenRecipes[_recipeId].IIPrefix != 0){
            return true;
        } else {
            return false;
        }
    ...
    function checkItemPrefix(uint _iPrefix) internal view returns (uint){
        require(IItems(items).prefixExists(_iPrefix), "Item prefix does not exist");
        return _iPrefix;
    }
    ...
    function pauseHandler() public isGameController(){
        _pause();
    }
    function unpauseHandler() public isGameController(){
        _unpause();
    }
    ...
    constructor (
        ...
        IItems _items,
        IERC20 _DAR,
        IResourceTracker _resourceTracker, 
