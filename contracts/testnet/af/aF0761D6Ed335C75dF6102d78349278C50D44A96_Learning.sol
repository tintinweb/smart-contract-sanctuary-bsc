...
contract Learning is Ownable
{
    using SafeMath for uint256;

    IRobot public Robot;          // NFT learn
    IERC20 public CyberCredit;     // Reward
    ...
    {
        return bytes4(keccak256("onERC721Recieved(address,address,uint256,bytes"));
    }
    ...
    {
        require(RobotJoin[msg.sender] > 0, "Error OutGame: Not join");
        require(removeRobot(msg.sender) == true, "Error OutGame: removeRobot");
    }

    function UpgrateLevelRobot(uint256 tokenId) public IsEnableSystem
    {
        address user = msg.sender;
        require(Robot.ownerOf(tokenId) == user, "Error UpgrateLevelRobot: Invalid tokenId");

        uint256 level = Robot.Level(tokenId);
        require(level < MaxLevel, "Error UpgrateLevelRobot: Invalid level");
        require(CyberCredit.balanceOf(user) >= PriceUpgrate[level.add(1)], "Error UpgrateLevelRobot: Invalid balance");
        
        Robot.UpgrateLevel(tokenId);
    }

    function StartLearn(uint256 tokenId) public IsEnableSystem 
    {
        address user = msg.sender;
        require(Robot.ownerOf(tokenId) == user, "Error StartLearn: Invalid tokenId");
        require(DataUserLearn[user].StartBlockLearn < block.number, "Error StartLearn: Time out");

        DataUserLearn[user].StartBlockLearn = block.number;
    ...
    {
        address user = msg.sender;
        uint256 pendingBlockEarn = block.number.sub(DataUserLearn[user].StartBlockLearn);
    }
