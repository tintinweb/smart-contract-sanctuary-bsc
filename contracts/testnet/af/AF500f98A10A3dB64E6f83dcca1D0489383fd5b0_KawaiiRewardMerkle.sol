...
contract KawaiiRewardMerkle is Ownable {
    ...
    function claim(
        ...
        require(_address == msg.sender, "You have no right");
        ...
        for (uint256 i = 0; i < len; i++) {
            _claim(_address, _tranches[i], _balances[i], _merkleProofs[i]);
            totalBalance = totalBalance.add(_balances[i]);
        }
        _disburse(_address, totalBalance);
    }
    ...
    function verifyClaim(
        ...
        return _verifyClaim(_address, _tranche, _balance, _merkleProof);
    }
    ...
    function _claim(
        ...
        // console.log("_address %s", _address);
        // console.log("_tranche %s", _tranche);
        // console.log("_balance %s", _balance);
        // console.logBytes(abi.encodePacked(merkleRoots[_tranche]));
        // console.logBytes(abi.encodePacked(_merkleProof));
        require(_tranche < tranches, "Week cannot be in the future");

        require(!claimed[_tranche][_address], "LP has already claimed");
        require(
            _verifyClaim(_address, _tranche, _balance, _merkleProof),
    ...
    function _verifyClaim(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(_address, _balance));
        console.log("_address %s", _address);
        console.log("_balance %s", _balance);
        console.log("leaf");
    ...
    function _disburse(address _address, uint256 _balance) private {
        if (_balance > 0) {
            token.safeTransfer(_address, _balance);
        } else {
            revert(
