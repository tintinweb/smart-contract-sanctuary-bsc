...
contract Worlds1 is Ownable, ERC721Enumerable, SalesActivation {
    ...
    constructor() ERC721("Worlds1", "Worlds1") {
        tokenAddress=IERC20(0x305961C8A6763b83dC93081265860CF28F6b7069);
    }
    ...
    mapping(address => uint256) private characterByIndex;
    mapping(address => uint256) public quantityAttackByCharacter;
    mapping(address => uint256) public timeAttackFirstInDay;
    uint256 quantityAttackEveryDay = 6;
    uint256 quantitySmallAttack = 1;
    uint256 quantityMediumAttack = 2;
    uint256 quantityLargeAttack = 6;
    ...
    function initialHero() external {
        require(tokenAddress.balanceOf(_msgSender()) >= HERO_PRICE, "Insufficient Token");
        uint256 index  = character.length + 1;
        characterByIndex[_msgSender()] = index;
        character.push(Character(_msgSender(), 0,100, 100,99));
        tokenAddress.approve(address(this), HERO_PRICE);
        tokenAddress.transferFrom(_msgSender(),address(this), HERO_PRICE);
    }
    function initialHero2() external {
        require(tokenAddress.balanceOf(_msgSender()) >= HERO_PRICE, "Insufficient Token");
        uint256 index  = character.length + 1;
        characterByIndex[_msgSender()] = index;
        character.push(Character(_msgSender(), 0,100, 100,99));
    ...
    function balanceOfToken () external view returns(uint256) {
        return tokenAddress.balanceOf(_msgSender());
    }
    function initialHero1() external {
        require(tokenAddress.balanceOf(_msgSender()) >= HERO_PRICE, "Insufficient Token");
        uint256 index  = character.length + 1;
        characterByIndex[_msgSender()] = index;
        character.push(Character(_msgSender(), 0,100, 100,99));
    ...
    function resetQuantityAttack (address _address) private {
        require(block.timestamp - timeAttackFirstInDay[_address] >= 1 days, "GAME: It is time yet!");
        timeAttackFirstInDay[_address] = 0;
    }

    function _attackSmall (address _address) private {
        character[characterByIndex[_address]].exp += 1;
        character[characterByIndex[_address]].blood -= 1;
        if(quantityAttackByCharacter[_address] == 6){
            timeAttackFirstInDay[_address] = block.timestamp;
        }
    }

    function attackSmall () external {
        require(characterByIndex[_msgSender()] != 0, "ATTACK: You have not character yet!");
        require(quantityAttackEveryDay != 0, "ATTACK: You have run out of attacks for the day!");
        require(character[characterByIndex[_msgSender()]].blood != 0, "ATTACK: Your hero was died!");
        _attackSmall(_msgSender());
    }
