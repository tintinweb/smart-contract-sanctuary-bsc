...
abstract contract NodePool{
    function addPower(uint nodeId,uint amountToWei,uint timeDistance) external virtual;
    function removePower(uint nodeId,uint amountToWei,uint timeDistance) external virtual;
...
contract Node is Comn{
    ...
    mapping(uint256 => uint256) public nodeRateTokenOneYes;           //点池单币有效总产量
     ...
     * @param nodeId 节点ID
     */
    function endBefore(uint nodeId,uint timeDistance) private {
        ...
        uint lastBlockOutPut = timeDistance.mul(miningRateSecond);                                        //最后一块总产出
    ...
    function endAfterN2Y(uint nodeId,uint nodeTotalLP,uint totalEffective,uint scale,uint timeDistance) external{
        uint lastBlockOutPut = timeDistance.mul(miningRateSecond);                                                      //最后一块总产出
        if(lastBlockOutPut > 0 && totalEffective > 0){
             nodeRateTokenOneNot[nodeId] = nodeRateTokenOneNot[nodeId].add(lastBlockOutPut.divFloat(totalEffective,18));   //更新 | 节点池单币无效总产量
        }
        updCreatorProfit(nodeId,nodeTotalLP,scale); //更新矿主收益
    }

    function endAfterY2N(uint nodeId,uint nodeTotalLP,uint scale) external{
        nodeRateTokenOneYes[nodeId] = rateTokenOne.sub(nodeRateTokenOneNot[nodeId]);
        updCreatorProfit(nodeId,nodeTotalLP,scale); //更新矿主收益
    }
    ...
    function updUserProfit(uint nodeId,uint scale) private{
        ...
        userNodeRateTokenOne[msg.sender][nodeId] = nodeRateTokenOne;                                       
    }

    //更新矿主收益
    function updCreatorProfit(uint nodeId,uint nodeTotalLP,uint scale) private{
        uint lastBlockNodeOutPut = nodeTotalLP.mul(rateTokenOne.sub(nodeRateTokenOneNot[nodeId]).sub(nodeLastRateTokenOne[nodeId])).div(1e18);
        ...
        nodeLastRateTokenOne[nodeId] = rateTokenOne.sub(nodeRateTokenOneNot[nodeId]);
    }
    ...
    function pledgeLP(uint nodeId,uint256 amountToWei) public checkStart {
        uint timeDistance = getNowTime().sub(updateTime);
        endBefore(nodeId,timeDistance);
        ...
        NodePool(poolAddress).addPower(nodeId,amountToWei,timeDistance);
    ...
    function redeemLP(uint nodeId,uint256 amountToWei) public checkStart {
        ...
        NodePool(poolAddress).removePower(nodeId,amountToWei,timeDistance);
    ...
    function collectProfitAll() public checkStart {
        ...
        for(uint nodeId = 1;nodeId <= nodeCount; nodeId++){
            ...
            if(userNodeTotalLp[msg.sender][nodeId] != 0){      //累加有质押的节点
                endBefore(nodeId,timeDistance);//结算之前的收益
    ...
    function getNodeSumProfit(uint nodeId,address querist) public view returns (uint256) {
        ...
        if(isEffective == 1){
            ...
            uint nodeNotRateTokenOne = getNotRateTokenOne(totalEffective,nodeId,isEffective);//节点单币总无效
            //用户最后一个区间的总收益 = 用户的余额 * 用户最后一个区间一个币的总产量
            uint lastBlockNodeSumProfit = userNodeTotalLp[querist][nodeId].mul(_rateTokenOne.sub(nodeNotRateTokenOne).sub(userNodeRateTokenOne[querist][nodeId])).div(1e18);
            ...
            uint centerBlockNodeSumProfit = userNodeTotalLp[querist][nodeId].mul(nodeRateTokenOneYes[nodeId].sub(userNodeRateTokenOne[querist][nodeId])).div(1e18);
            return userNodeProfitMapping[querist][nodeId].add(centerBlockNodeSumProfit.mul(1000-scale).div(1000));
        }
    }
    ...
    function getCreatorWaitCollectionProfit(uint nodeId,uint totalEffective,uint isEffective,uint nodeTotalLP) public view returns (uint256) {
        if(isEffective == 1){
            uint _rateTokenOne = getRateTokenOne(totalEffective);                          //单币总产量
            uint nodeNotRateTokenOne = getNotRateTokenOne(totalEffective,nodeId,isEffective);//节点单币总无效
            return nodeTotalLP.mul(_rateTokenOne.sub(nodeNotRateTokenOne).sub(nodeLastRateTokenOne[nodeId])).div(1e18);
        } else {
            return nodeTotalLP.mul(nodeRateTokenOneYes[nodeId].sub(nodeLastRateTokenOne[nodeId])).div(1e18);
        }
    }
