...
contract BWCStakingToken is Context, Ownable {
  ...
  mapping (address => mapping (uint256 => uint256)) private _depositid;
  ...
  event staking(address indexed from, address indexed to, uint256 value, uint256 time);
  event unstake(address indexed from, address indexed to, uint256 value, uint256 time);
  event changepoolrequire(address indexed from, address indexed to);

  constructor(address _token,uint256 _apr,uint256 _locktimer,uint256 _available) public {
    tokenaddress = _token;
    apr = _apr;
  ...
  function deposit(uint256 amount) external returns (bool) {
    ...
    require( poolbalance < poolbalance.add(amount) );
    ...
    emit staking(msg.sender,address(this),amount,block.timestamp);
    return true;
  }

  function redeem(uint256 _id) external returns (bool) {
    require( block.timestamp > _staketime[_id].add(locktimer) );
    require( _redeem[_id] != true );
    _redeem[_id] = true;
    IBEP20 a = IBEP20(tokenaddress);
    uint256 amount = _balances[_id];
    uint256 period = block.timestamp.sub(_staketime[_id]);
    uint256 reward = amount.mul(apr.mul(period).div(31536000));
    uint256 cash = amount.add(reward);
    a.transfer(_idowner[_id],cash);
    clearleek(_idowner[_id]);
    poolbalance = poolbalance.sub(amount);
    emit staking(msg.sender,address(this),cash,block.timestamp);
    return true;
  }
  ...
  function balanceOf(address account) external view returns (uint256) {
    uint256 result = 0;
    uint256 index = 0;
    for (uint i=0; i< _countstake[account] ; i++){
  ...
  function updateLockRequire(address _token,uint256 _apr,uint256 _timer,uint256 _available) public onlyOwner returns (bool) {
      tokenaddress = _token;
      apr = _apr;
      locktimer = _timer;
      available = _available;
      emit changepoolrequire(msg.sender,address(this));
      return true;
  }
