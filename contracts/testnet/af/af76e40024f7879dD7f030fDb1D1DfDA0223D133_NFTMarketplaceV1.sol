...
import "../storage/NFTMarketplaceStorage.sol";
import "../interface/IBEP20.sol";
 
contract NFTMarketplaceV1 is 
    NFTMarketplaceStorageV1,
    IERC721Receiver,
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable,
    ...
    function initialize(uint16 _platformFee, uint256 _minExpireTime, uint256 _maxExpireTime) public initializer {
        platformFee = _platformFee;
        minExpireTime = _minExpireTime;
        maxExpireTime = _maxExpireTime;
    ...
    modifier checkExpireTime(uint256 _expireTime){
        require(_expireTime >= minExpireTime, "must greater than min");
        require(_expireTime <= maxExpireTime, "must less than max");
        _;
    }
    ...
    checkExpireTime(_expireTime)
    {
        ...
        uint256 lastTime = onShopTime.add(_expireTime);
        ...
        offers[msg.sender][_nftAddress][_tokenId] = itemId;
    ...
    function cancelItem(
        ...
        uint256 itemId = offers[msg.sender][_nftAddress][_tokenId];
        ...
        delete(offers[msg.sender][itemInfo.nftAddress][itemInfo.tokenId]);
    ...
    function buyItem(
        ...
        uint256 itemId = offers[_owner][_nftAddress][_tokenId];
        require(itemId > 0, "not exist");
        ...
        require(_getNow() <= itemInfo.expireTime, "time error");
        //BNB Or ETH
        ...
        delete(offers[itemInfo.owner][itemInfo.nftAddress][itemInfo.tokenId]);
        delete(listings[itemId]);
    }
    ...
    function updateMinExpireTime(uint256 _minExpireTime) external onlyOwner{
        minExpireTime = _minExpireTime;
    }

    function updateMaxExpireTime(uint256 _maxExpireTime) external onlyOwner{
        maxExpireTime = _maxExpireTime;
    }

    function fetchMyNFTs(address _nftOwner) virtual external view returns(ItemInfo[] memory){
    ...
    function getItemInfoByAddress(address nftAddress, uint256 tokenId, address owner)external view returns(ItemInfo memory){
        uint256 itemId = offers[owner][nftAddress][tokenId];
        return listings[itemId];
    }
...
contract NFTMarketplaceStorageV1{
    struct ItemInfo{
        address nftAddress;
        uint256 tokenId;
        uint256 itemId;
    ...
    /// @notice Owner => NftAddress => Token ID => Listing item
    mapping(address => mapping(address => mapping(uint256 => uint256))) public offers;
    ...
    uint256 public minExpireTime;
    uint256 public maxExpireTime;
}
