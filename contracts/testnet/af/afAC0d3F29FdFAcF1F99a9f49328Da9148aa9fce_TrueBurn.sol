...
contract TrueBurn is ERC20 {
    using SafeMath for uint256;
    uint TAX_FEE = 5;
    uint BURN_FEE = 5;
    address public owner;
    mapping (address => bool) exclidedFromTax;

    constructor() ERC20("True Burn", "TRBRN") {
 ...
 function transfer(address recipient, uint256 amount) public override returns (bool) {
    if (exclidedFromTax[msg.sender] == true){
        ...
        _transfer(msg.sender, owner, burntAmount);
        _transfer(msg.sender, recipient, amount.sub(burntAmount).sub(ownerAmount));
    }
    return true;
  }
