...
contract YSLBUSDVault is Initializable, ReentrancyGuard, Pausable, IEvents {
    ...
    */
    ...
    function claimRebaseReward(address user) public nonReentrant whenNotPaused _completePause _securityCheck(user,user){
        _claimRebaseReward(user);
    }
    ...
    */
    ...
    function burnOrRemove(uint amount, uint zeroOrOne) external _isAdmin{
        ...
        if(zeroOrOne == 0){
            IReceipt(YSL).burn(address(this),amount);
        }
        else{
            IERC20(YSL).transfer(msg.sender,amount);
