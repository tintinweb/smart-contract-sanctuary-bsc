...
contract Planetopia {
    ...
    function getRandUserByLevel(uint8 level,uint256 h) public view returns (address) {
        ...
        for(uint256 i=0;i<lastUserId; i++){
           uint256 randid =  uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, h))) % (lastUserId);
           if(randid==0) randid  = 1;
            if(maxActiveLevel(idToAddress[randid])>=level){
                return idToAddress[randid];
            }
        }
        return owner;
    }
