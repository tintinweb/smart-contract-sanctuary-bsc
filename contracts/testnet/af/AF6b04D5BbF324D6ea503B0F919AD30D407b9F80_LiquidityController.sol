...
contract LiquidityController is TokensRecoverable, ILiquidityController
{
    ...
    constructor(IUniswapV2Router02 _uniswapV2Router, IERC20 _base, IERC20 _rooted, IERC31337 _elite, IFloorCalculator _calculator, RootedTransferGate _gate)
    {
        ...
        _base.approve(address(_elite), type(uint).max);
        _base.approve(address(_uniswapV2Router), type(uint).max);
        _rooted.approve(address(_uniswapV2Router), type(uint).max);
        IERC20 _rootedBaseLP = IERC20(_uniswapV2Factory.getPair(address(_base), address(_rooted)));
        _rootedBaseLP.approve(address(_uniswapV2Router), type(uint).max);
        rootedBaseLP = _rootedBaseLP;
        _elite.approve(address(_uniswapV2Router), type(uint).max);
        IERC20 _rootedEliteLP = IERC20(_uniswapV2Factory.getPair(address(_elite), address(_rooted)));
        _rootedEliteLP.approve(address(_uniswapV2Router), type(uint).max);
    ...
    function setLiquidityController(address controlAddress, bool controller) public onlyOwner
    {
        liquidityControllers[controlAddress] = controller;
    }

    function setCalculatorAndGate(IFloorCalculator _calculator, RootedTransferGate _gate) public onlyOwner
    {
        calculator = _calculator;
        gate = _gate;
    }
...
{   
    ...
    function setUnrestrictedController(address unrestrictedController, bool allow) public onlyOwner
    {
    ...
    function setFeeController(address feeController, bool allow) public onlyOwner
    {
        feeControllers[feeController] = allow;
    }

    function setFeeSplitter(address _feeSplitter) public onlyOwner
    {
    ...
    function setBlackListProtection(BlackListProtection _blackListProtection) public onlyOwner
    {
        blackListProtection = _blackListProtection;
    }

    function setMainPool(IUniswapV2Pair _mainPool) public onlyOwner
    {
        mainPool = _mainPool;
    }

     function setPoolTaxRate(address pool, uint16 taxRate) public onlyOwner
    {
