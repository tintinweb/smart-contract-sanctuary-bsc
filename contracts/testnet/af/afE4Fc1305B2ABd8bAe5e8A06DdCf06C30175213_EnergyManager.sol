...
contract EnergyManager is EnergyAuth {
  ...
  function initialize(address _mainAdmin) override public initializer {
    ...
    heroNft = IGameNFT(0x9F007C4Ef2fCFaacA5f135680Df4b596195ed61d);
  ...
  function updatePoint(address _user, int _point) external onlyPointContract {
    ...
    if (noEndgameCalled) {
      bool needClearYesterdayPoint = energy.lastUpdateTempPoint < _getStartOfDay();
  ...
  function getUserCurrentEnergy(address _user) external view returns (uint) {
    ...
    require(!firstConsumeInDay, "EnergyManager: new day, finish first game first");
  ...
  function _calculateUserTodayEnergy(address _user) private returns (uint) {
    uint owned;
    uint[] memory ids = heroNft.ownerHeroes(_user);
    for(uint i = 0; i < ids.length; i++) {
      (,,uint8 level,,,,) = heroNft.heroes(ids[i]);
      bool reachMaxProfit = heroNft.reachMaxProfit(ids[i]);
      if (!reachMaxProfit) {
        owned += uint(level);
      }
    }
    ...
    int tempPoint = int(owned) + int(rented);
    if (userEnergy.lastUpdateTempPoint >= _getStartOfDay()) {
      tempPoint -= userEnergy.tempPoint;
    }
