...
contract BamiRefundIDO is Ownable, ReentrancyGuard {
    ...
    // Refund
    uint256 public amountRefundPool1 = 3750 * generalDec;
    uint256 public amountRefundPool2 = 1875 * generalDec;
    uint256 public amountRefundPool3 = 1875 * generalDec;
    mapping(address => bool) public refundedMap;
    uint256 public startTimeRefund;
    uint256 public endTimeRefund;

    // event
    event Refund(address user, uint256 amount, uint256 pool);
    ...
    function refund() public nonReentrant isRun validTimeRefund {
        require(refundedMap[msg.sender] == false, "Already refunded");
        require(hesmanPool1.whitelistMap(msg.sender) || hesmanPool2.whitelistMap(msg.sender) || hesmanPool3.whitelistMap(msg.sender), "Not whitelist");
        if (hesmanPool1.whitelistMap(msg.sender)) {
            require(
                hesmanPool1.vestingMap(msg.sender) >= 1,
                "You didnt't claim first"
            );
            refundedMap[msg.sender] = true;
            hesmanToken.transfer(msg.sender, amountRefundPool1);

            emit Refund(msg.sender, amountRefundPool1, 1);
        } 
        if (hesmanPool2.whitelistMap(msg.sender)) {
            require(
                hesmanPool2.vestingMap(msg.sender) >= 1,
                "You didnt't claim first"
            );
            refundedMap[msg.sender] = true;
            hesmanToken.transfer(msg.sender, amountRefundPool2);

            emit Refund(msg.sender, amountRefundPool2, 2);
        } 
        if (hesmanPool3.whitelistMap(msg.sender)) {
            require(
                hesmanPool3.vestingMap(msg.sender) >= 1,
                "You didnt't claim first"
            );
            refundedMap[msg.sender] = true;
            hesmanToken.transfer(msg.sender, amountRefundPool3);

            emit Refund(msg.sender, amountRefundPool3, 3);
        }
    }
    ...
    function setAmountRefund(uint256 _amountRefundPool1, uint256 _amountRefundPool2, uint256 _amountRefundPool3) public onlyOwner {
