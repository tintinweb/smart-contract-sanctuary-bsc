...
contract Escrow is Ownable,Initializable {
    ...
    *   @param bondAddress bond address
    *   @param allocated bond allocation amount in wei
    *   @param allocated bond start time
    */
    function initialize(Dividend memory dividentParams, address bondAddress, uint allocated, uint64 startTime) external onlyOwner initializer {
        require(!isZero(uint160(dividentParams.token)), "the dividend token must not be zero");
        require(!isZero(uint160(bondAddress)), "the bond address must not be zero");
        ...
        require(dividentParams.percent <= DIVISOR, "the dividend percent must be less than divisor");
        require(!isZero(dividentParams.period), "the dividend period must not be zero");
        require(startTime >= blockTimestamp(), "the start time must not be less than current block");
        require(!isZero(allocated), "the bond allocation must not be zero");
        
        dividend = dividentParams;
        bond.account = bondAddress;
        bond.allocated = allocated;
        bond.lastClaim = bond.startTime = startTime;
    }
    ...
    ///@dev update the bond address
    ///@param bondAddress bond address
    function updateBond(address bondAddress) external onlyOwner {
        require(!isZero(uint160(bondAddress)), "updateBond: the bond address must not be zero");
        bond.account = bondAddress;
    }
    ...
    function updateDivPercent(uint16 percent) external onlyOwner {
        require(percent <= DIVISOR, "the dividend percent must be less than divisor");
    ...
    ///@dev bond address claim its dividends from here
    function claim() external {
        require(bond.account == _msgSender(), "claim: the bond wallet is not a caller");
        require(!isZero(bond.startTime), "claim: start time must not be zero");
        require(!isZero(bond.allocated), "claim: allocation must not be zero");
        require(bond.released <= bond.allocated, "claim: the bond wallet has claimed all dividends");

        if(!dividend.isInitialReleased) {
            _claimInitialRelease();
        } else {
            require(
                bond.lastClaim + dividend.period <= blockTimestamp(),
    ...
    function _claimInitialRelease() private {
        dividend.isInitialReleased = true;
        bond.released += (bond.allocated * dividend.initialRelease) / DIVISOR;
    
        if(bond.lastClaim + dividend.period <= blockTimestamp()) {
            _claimInternal();
        }
    }

    ///@dev claim dividends
    function _claimInternal() private {
        (uint dividends, uint claims) = viewDividend();

        if(!isZero(dividends)) {
            bond.released += dividends;
            bond.lastClaim += (dividend.period * claims).toUint64();
    ...
    function viewDividend() public view returns(uint dividends, uint claims) {        
        if(!isValidClaim(bond)) {   
            return (0,0);
        }

        uint months = (block.timestamp - bond.lastClaim) / dividend.period;

        if(!isZero(months)) {
            uint divAmount = (bond.allocated * dividend.percent) / DIVISOR;
            dividends = divAmount * months;
            claims = months;

            if((bond.released + dividends) > bond.allocated) {
                dividends = bond.allocated - bond.released;
            }
        }
    }

    ///@dev returns true if bond is allowed to claim
    ///@param wallet bond info
    ///@return bool is it a valid claim, returns in boolean
    function isValidClaim(Bond memory wallet) private view returns(bool) {
        return !(
