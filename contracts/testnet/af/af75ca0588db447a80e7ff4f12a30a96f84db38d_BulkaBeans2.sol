...
Website: https://BulkaBeans.money
Telegram: https://t.me/BulkaBeans
...
contract BulkaBeans2 is Context, Ownable {
    ...
    uint32 SECONDS_PER_DAY = 30; // 86400;
    uint8 DEPOSIT_FEE = 1;
    uint8 AIRDROP_FEE = 1;
    ...
    uint256 MIN_DEPOSIT = 0.01 ether;  //10 ether; // 10 BUSD
    uint256 MIN_BAKE = 0.001 ether; //  1 ether; // 1 BUSD
    uint256 MAX_WALLET_TVL_IN_BNB = 100000 ether; // 100000 BUSD
    uint256 MAX_DAILY_REWARDS_IN_BNB = 6500 ether; // 6500 BUSD
    uint256 MIN_REF_DEPOSIT_FOR_BONUS = 0.05 ether; //  150 ether; // 150 BUSD
    ...
    function buyBeans(address ref, uint256 _amount) payable public {
        ...
        IERC20(BNB).transferFrom(msg.sender, payable(address(this)), _amount);
    ...
    /*    
        payable(_dev).transfer(dev);
        payable(_marketing).transfer(marketing);
        payable(_ceo).transfer(ceo);
    */
        ...
        if (giveAway > 0) {
            //payable(_giveAway).transfer(giveAway);
