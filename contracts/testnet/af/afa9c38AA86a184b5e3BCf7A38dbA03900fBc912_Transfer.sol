...
contract Transfer is ReEntrancyGuard, Ownable {
    ...
    function adminWithdraw(
        ...
        if (keccak256(bytes(_tokenTypeSymbol)) == keccak256("USDT")) {
            require(
                ...
                "AW: Vendor contract has not enough tokens in its balance"
            );

            require(
                IERC20(usdtAddress).transfer(_client, _amount),
                "AW: Failed to transfer tokens to the admin"
            );
            ...
            require(success, "AW: Failed to transfer tokens to the client");
        }
        ...
        if (keccak256(bytes(_tokenTypeSymbol)) == keccak256("USDT")) {
            for (uint256 i; i < _wallets.length; i++) {
                require(
                    ...
                    "WM: Vendor contract has not enough tokens in its balance"
                );
                require(
                    IERC20(usdtAddress).transfer(_wallets[i], _amounts[i]),
                    "WM: Failed to transfer tokens to the client"
                );
            }
        } else if (keccak256(bytes(_tokenTypeSymbol)) == keccak256("BUSD")) {
            for (uint256 i; i < _wallets.length; i++) {
            ...
            for (uint256 i; i < _wallets.length; i++) {
                ...
                require(success, "WM: Failed to transfer tokens to the client");
            }
        }
    ...
    /**
     * @dev verify USDT vendor balance.
     */
    ...
    /**
     * @dev verify BUSD vendor balance.
     */
    ...
    function setERC20Address(address _usdt, address _busd)
        public
        onlyOwner
        returns (address, address)
    {
        usdtAddress = _usdt;
        busdAddress = _busd;
        return (usdtAddress, busdAddress);
    }
