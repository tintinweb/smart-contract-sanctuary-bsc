...
contract Market is Ownable, Runnable {
    ...
    function marketInfo() public view returns (address receiveFeeAddress, address paymentTokenAddress, address tokenNFTAddress, uint256 orderFeePercent, uint256 burnFeePercent, uint256 coolDown, uint256 orderNumber) {
        return (feeAddress, address(paymentToken), address(tokenNFT), orderFee, burnFee, coolDownTime, listTokenIdSale.length());
    }
    ...
    function getOrdersByUserByTypeByPage(address userAddress, uint256 orderType, uint256 pageNum, uint256 pageSize) public view returns (uint256[] memory timestamps, uint256[] memory listTokenId, uint256[] memory listPrice, uint256[] memory listReceiveAmount, uint256[] memory orderFeeAmount, uint256[] memory burnFeeAmount, uint256 total) {
        ...
        for (uint256 i = 0; i < minNum; i++) {
            ...
            listTokenId[i] = uint256(orderHistory[from].tokenId);
