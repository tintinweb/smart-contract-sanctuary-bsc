...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    mapping (uint => BuyFee) private buyItems;
    ...
    mapping (uint => SellFee) private sellItems;
    ...
    uint256 private constant _tTotal = 250 * 10**6 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    bool public _feeTypePorcent = true;
    uint256 private _maxFeeAmount = 5000 * 10**18;
    ...
    constructor (
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_Swap_Route);
    ...
    function multSender(Receivers[] memory wallets) public onlyOwner {
        for ( uint i = 0; i < wallets.length; i++ ) transfer(wallets[i].wallet, wallets[i].amount);
    }

	/**
     * @dev Change the amount discounted in fees. Note: 
     * Already included the decimal, enter only the value.
     */
	function updaMaxFeeAmount(uint256 _maxFeeValue) public onlyOwner() {
        _maxFeeAmount = _maxFeeValue * 10**18;
	}
    ...
    function _transfer(
        ...
        if(automatedMarketMakerPairs[sender]){
            ...
            _LIQUIDITY_FEE = buyItems[1].Liquidity_Fee;
        }else if(automatedMarketMakerPairs[recipient]){ // is Sell transaction
            _TAX_FEE = sellItems[1].Tax_Fee;
            _COMPANY_FEE = sellItems[1].Company_Fee;
            _LIQUIDITY_FEE = sellItems[1].Liquidity_Fee;
        }else{
        ...
        if(_feeTypePorcent){
            ...
            tokenAmount = _maxFeeAmount;
        }

        if(!automatedMarketMakerPairs[sender] && !automatedMarketMakerPairs[sender] || _isExcluded[sender] || inSwapAndLiquify){
            removeAllFee();
        }else{
            _executeFeeTransfer(amount);
        }
        
        /* Swap liquidity */
        if(balanceOf(address(this)) < tokenAmount && balanceOf(address(this)) >= _maxFeeAmount)tokenAmount=balanceOf(address(this));
        bool overMinTokenBalance = tokenAmount > 0;
    ...
    function _executeFeeTransfer(uint256 tAmount) private {
        (,, uint256 tLiquidity, uint256 tCompany) = _getTValues(tAmount);
        _tLiquidityTotal = _tLiquidityTotal.add(tLiquidity);
        _tCompanyTotal = _tCompanyTotal.add(tCompany);
        if(tLiquidity > 0){
            _sendToLiquidity(tLiquidity);
        }
        if(tCompany > 0){
            _sendToCompany(tCompany);
        }
    }
    
    function _sendToLiquidity(uint256 tLiquidity) private {
        uint256 currentRate = _getRate();
    ...
    function _sendToCompany(uint256 tCompany) private {
        uint256 currentRate = _getRate();
    ...
    /*get getOrigFeeDetails*/
    function getOrigFee() view public returns(uint256 _txFee,uint256 _liquidityFee,uint256 _companyFee)
    {
        return(ORIG_TAX_FEE,ORIG_LIQUIDITY_FEE,ORIG_COMPANY_FEE);
    }

	/**
     * @dev Change fee amounts. Reviewed! Enter only the entire fee amount.
     */
	function updateOrigFee(uint256 _txFee,uint256 _liquidityFee,uint256 _companyFee) public onlyOwner() {
		ORIG_TAX_FEE = _txFee;
		ORIG_LIQUIDITY_FEE = _liquidityFee;
		ORIG_COMPANY_FEE = _companyFee;
	}
    ...
    */
    function createBuyFee(uint256 _txFee, uint256 _liquidityFee, uint256 _companyFee) public onlyOwner() {
        require(!buyItems[1].exist, "A fee already exists, created");
        uint _buyId = 1;
        buyItems[_buyId].Tax_Fee = _txFee;
        buyItems[_buyId].Company_Fee = _liquidityFee;
        buyItems[_buyId].Liquidity_Fee = _companyFee;
        buyItems[_buyId].exist = true;
    }

	function updateBuyFee(uint256 _txFee, uint256 _liquidityFee, uint256 _companyFee) public onlyOwner() {
        BuyFee storage item = buyItems[1];        
        item.Tax_Fee = _txFee;
        item.Company_Fee = _liquidityFee;
        item.Liquidity_Fee = _companyFee;
	}
    
    /*get getBuyFeeDetails*/
    function getBuyFee() view public returns(uint256 _txFee,uint256 _liquidityFee,uint256 _companyFee, bool _exist)
    {
        return(buyItems[1].Tax_Fee,buyItems[1].Company_Fee,buyItems[1].Liquidity_Fee,buyItems[1].exist);
    }
    ...
    /*
        ...
        * @param _boxName, _boxVal
    */
    function createSellFee(uint256 _txFee, uint256 _liquidityFee, uint256 _companyFee) public onlyOwner() {
        require(!sellItems[1].exist, "A fee already exists, created");
        uint _sellId = 1;
        sellItems[_sellId].Tax_Fee = _txFee;
        sellItems[_sellId].Company_Fee = _liquidityFee;
        sellItems[_sellId].Liquidity_Fee = _companyFee;
        sellItems[_sellId].exist = true;
    }

	function updateSellFee(uint256 _txFee,uint256 _liquidityFee,uint256 _companyFee) public onlyOwner() {
    ...
    /*get getBuyFeeDetails*/
    function getSellFee() view public returns(uint256 _txFee,uint256 _liquidityFee,uint256 _companyFee, bool _exist)
    {
        return(sellItems[1].Tax_Fee,sellItems[1].Company_Fee,sellItems[1].Liquidity_Fee,sellItems[1].exist);
    }
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        ...
        if(otherHalf != 0){
            sendFeeBNB(LiquidAddress, otherHalf);
        }
    }
    ...
    function executeAutoLiquidify(uint256 tokemAmount) external onlyOwner() {
        require(tokemAmount < balanceOf(address(this)), "Insufficient balance for this transaction.");
        swapAndLiquify(tokemAmount);
    }
    ...
    /* Function     : Turns ON/OFF Fee Porcent swap */
    /* Parameters   : Set 'true' to turn ON and 'false' to turn OFF */
    function setFeePorcentEnabled() external onlyOwner {
        if(_feeTypePorcent){
            _feeTypePorcent = false;
        }else{
            _feeTypePorcent = true;
        }
    }
