...
contract USDTRUN {
    ...
    //uint256 public constant BASE_PERCENT = 100; // 1% per day
     ...
     uint256 public constant BASE_PERCENT = 10000; // 100% per day for testing
    ...
    constructor(address payable marketingAddr,IBEP20 tokenAdd ) {
    ...
    function invest(address payable referrer,uint256 token_quantity) public payable beforeStarted() {
        ...
        token.transfer(owner,tokenWei.mul(1000).div(PERCENTS_DIVIDER));
        ...
        if(users[upline].available >  direct_amt)
        {
    ...
    function withdraw() public beforeStarted() {
        ...
        token.transfer(marketingAddress,fees);
