...
contract LuckToken1 is Token {
	...
	function changeLPaddStatus(bool onoroff) external{
		addLPstatus = onoroff;
	}
    ...
    function basicTransferFrom(address sender, address recipient, uint256 amount) internal authorized lockTheSwap returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
