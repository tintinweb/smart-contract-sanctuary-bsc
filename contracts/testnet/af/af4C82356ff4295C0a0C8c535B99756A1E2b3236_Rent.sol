...
contract Rent {
    ...
    mapping(uint => mapping(uint => record)) rentRecords;

    function getRentRecord(uint chainId, uint internalId) public view returns (record memory){
        return rentRecords[chainId][internalId];
    }



    modifier currentOwner(uint chainId, uint internalId){
        require(archive.getCurrentOwner(chainId, internalId) == msg.sender);
        _;
    }

    // to return proper owner to the nft after rent period is done
    // doesnt have to be called by owner
    function releaseFromRent(uint chainId, uint internalId) public returns (bool){
    	record storage ptr = rentRecords[chainId][internalId];

    	require(ptr.started != 0, "is not rented");
    	require(ptr.started + ptr.time < block.timestamp, "still on rent");

    	Archives.Action[] memory toArch = new Archives.Action[](1);
    	// z - new owner
        toArch[0].tp = 1;
    	toArch[0].a = chainId;
    	toArch[0].b = internalId;
    	toArch[0].z = ptr.realOwner;
    ...
    function stopRenting(uint chainId, uint internalId) public  returns (bool){

    	record storage ptr = rentRecords[chainId][internalId];

    	require(msg.sender == ptr.realOwner, "not the real owner");
    	require(ptr.price != 0, "not in rental contract");
    	require(ptr.started == 0 || ptr.started + ptr.time < block.timestamp, "is rented");

    	ptr.started = 0;
    	ptr.price = 0;

    	Archives.Action[] memory toArch = new Archives.Action[](2);
    ...
    function putOnRent(uint chainId, uint internalId, IERC20 token, uint price, uint time) public returns (bool){

    	require(price != 0, "non zero price");
    	require(archive.isTokenAllowed(address(token)), "Token is not allowed");

    	record storage ptr = rentRecords[chainId][internalId];

    	if(ptr.price == 0){
    		require(archive.getCurrentOwner(chainId, internalId) == msg.sender);
    		ptr.realOwner = msg.sender;
    	}else{
    		require(ptr.realOwner == msg.sender);
    	}

    	// push to sale if needed and change data
    	if(archive.getCurrentStatus(chainId, internalId) == 2){
    		Archives.Action[] memory toArch = new Archives.Action[](1);
    		toArch[0].tp =  archive.showActionStatus(address(this));
    		toArch[0].a = chainId;
    		toArch[0].b = internalId;
    		archive.stackedActions(toArch);
    	}
    ...
    function rent(uint chainId, uint internalId, address api) public returns (bool){
        // if canceled or non-exist - that's the only reason why price is 0 here
        record storage ptr = rentRecords[chainId][internalId];

        require(ptr.price != 0, "NFT is not on rent");
        require(ptr.started == 0 || ptr.started + ptr.time < block.timestamp, "is rented");
        require(ptr.realOwner != msg.sender, "you cant rent yours");

        ptr.started = block.timestamp;


        // ensure current owner is real + pay + move owner to renter
        ...
        toArch[1].tp = ptr.price;
        toArch[1].a = chainId;
        toArch[1].b = internalId;
        toArch[1].x = msg.sender;
        toArch[1].z = api;
        toArch[1].token = ptr.token;

        toArch[2].tp = 1;
    	toArch[2].a = chainId;
    	toArch[2].b = internalId;
    	toArch[2].z = msg.sender;
