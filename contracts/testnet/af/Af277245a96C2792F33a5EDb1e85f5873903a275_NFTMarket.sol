...
contract NFTMarket is ReentrancyGuard, Ownable {
    ...
    uint256 feePercent = 25; //in basis points (parts per 1,000)
    address[] public whitelistedContracts;

    constructor(address _whitelistedContract) Ownable() {
        addToWhitelist(_whitelistedContract);
    }
    ...
    function createMarketItem(
        ...
        require(nft.getApproved(tokenId) == address(this) || nft.isApprovedForAll(msg.sender, address(this)), "Contract is not approved as an operator");
    ...
    function calculateFee(uint256 val) private view returns(uint256) {
        return val.div(1000).mul(feePercent);
    }
    ...
    function getItemPrice(uint256 itemId) public view returns(uint256) {
        ...
        uint256 h = 3600;
        if (item.endPrice == item.startPrice || block.timestamp >= item.createdAt.add(h.mul(item.duration))) {
            return item.endPrice;
        }

        uint256 diffHours = (block.timestamp.sub(item.createdAt)).div(3600);
        uint256 priceChangePerHour = (item.startPrice.sub(item.endPrice)).div(item.duration == 0 ? 1 : item.duration);
