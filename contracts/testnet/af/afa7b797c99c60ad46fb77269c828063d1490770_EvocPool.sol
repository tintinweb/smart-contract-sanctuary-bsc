...
contract EvocPool is Ownable, Pausable {
    ...
    event NewVEvocContract(address VEvoc);
     ...
     */
    modifier onlyOperatorOrEvocOwner(address _user) {
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onEvocPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
     ...
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrEvocOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Evoc Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VEvoc != address(0)) {
            IVEvoc(VEvoc).deposit(_user, _amount, _lockDuration);
        }
    ...
    /**
     * @notice Withdraw funds from the Evoc Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VEvoc != address(0)) {
            IVEvoc(VEvoc).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending EVOC tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingEvoc = masterchefV2.pendingEvoc(cakePoolPID, address(this));
        if (pendingEvoc > 0) {
    ...
    /**
     * @notice Set VEvoc Contract address
     * @dev Callable by the contract admin.
     */
    function setVEvocContract(address _VEvoc) external onlyAdmin {
        require(_VEvoc != address(0), "Cannot be zero address");
        VEvoc = _VEvoc;
        emit NewVEvocContract(VEvoc);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Evoc Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingEvocRewards();
     ...
     */
    function calculateTotalPendingEvocRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingEvoc(cakePoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingEvocRewards()) * (1e18)) / totalShares);
    }
