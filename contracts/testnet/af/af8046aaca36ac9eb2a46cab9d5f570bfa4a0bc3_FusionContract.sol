    ...
    IERC721ReceiverUpgradeable
{
    ...
    function test(bytes32 _bytes321, bytes32 _bytes322) public pure returns (string memory) {
        return string(abi.encodePacked("https://planetsandbox.infura-ipfs.io/ipfs/", _bytes321, _bytes322));
    }

    function handleFusion(
        bytes32 _requestId,
        bytes32 _firstIpfsHashBytes32,
        bytes32 _secondsIpfsHashBytes32
        ...
        fusionInfo.returnId = uint32(
            nftContract.mint(
                request.user,
                string(
                    abi.encodePacked(
                        "https://planetsandbox.infura-ipfs.io/ipfs/",
                        _firstIpfsHashBytes32,
                        _secondsIpfsHashBytes32
                    )
                ),
                1
            )[0]
