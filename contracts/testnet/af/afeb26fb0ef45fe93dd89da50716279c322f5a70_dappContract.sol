...
pragma solidity ^0.8.0;
  ...
  //  import "@openzeppelin/contracts/access/Ownable.sol";
    ...
    contract dappContract is Ownable {
        ...
        event Stake(address _sender, address _recipient,uint _amount);
        ...
        function whiteListProjectOwners (address _projectOwnerAddress) external onlyOwner {
            require (!whiteListProjectOwner[_projectOwnerAddress],'Already whitelisted');
            whiteListProjectOwner[_projectOwnerAddress] = true;
        }

        function blackListProjectOwners (address _projectOwnerAddress) external onlyOwner {
            require (whiteListProjectOwner[_projectOwnerAddress],'!whitelisted');
            whiteListProjectOwner[_projectOwnerAddress] = false;
        }

        function createProjects (string memory projectName) external onlyWhitelisted(msg.sender) onlyOnce (msg.sender) {
            projectIds += 1;
            projectList [msg.sender] [projectName] = projectIds;
        }

        function transferProjectOwnership (string memory projectName, address _previousOwner, address _currentOwner) external onlyOwner {
            uint projectId = projectList[_previousOwner][projectName];
            delete projectList[_previousOwner][projectName];
            projectList [_currentOwner] [projectName] = projectId;
        }
