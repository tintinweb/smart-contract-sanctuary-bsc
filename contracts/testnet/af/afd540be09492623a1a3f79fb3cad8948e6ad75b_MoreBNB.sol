...
contract MoreBNB is ERC20, Ownable {
    ...
    uint256 internal constant LAUNCH_TIME = 1659253220;   
    uint256 currentDay;
    ...
    function _updateDaily() public {
        if (currentDay != _clcDay()) {
			...
			else 
			{
				...
				{
                    dayBNBPool[_day] += (lobbyEntry[currentDay] * 8550 ) / (dividendsPoolCapDays * 10000);
                }
            }
    ...
    function _sendOwnerShare() internal nonReentrant {
	 ...
	 function _sendTopDepositorFund() internal nonReentrant {
        require(currentDay > 0);
        payable(topDepositor).transfer(topDepositorFund);
	...
	function referralUpdate(address _address, uint256 amount) private {
		...
		for(i=0; i < 10; i++) {
			if(_nextReferrer != address(0x0)) 
			{
				...
				if(mapMemberLobby_overallData[_nextReferrer].referrerFund > mapMemberLobby_overallData[topSponsor[0]].referrerFund && topSponsor[0] !=_nextReferrer)
				{
				...
				else if(mapMemberLobby_overallData[_nextReferrer].referrerFund > mapMemberLobby_overallData[topSponsor[1]].referrerFund && topSponsor[1] !=_nextReferrer)
				{
				    topSponsor[3] = topSponsor[2];
					topSponsor[2] = topSponsor[1];
					topSponsor[1] = _nextReferrer;
				}
				else if(mapMemberLobby_overallData[_nextReferrer].referrerFund > mapMemberLobby_overallData[topSponsor[2]].referrerFund && topSponsor[2] !=_nextReferrer)
				{
				    topSponsor[3] = topSponsor[2];
					topSponsor[2] = _nextReferrer;
				}
				else if(mapMemberLobby_overallData[_nextReferrer].referrerFund > mapMemberLobby_overallData[topSponsor[3]].referrerFund && topSponsor[3] !=_nextReferrer)
				{
    ...
    function EndStake(uint256 stakeId, bool restakes) external nonReentrant {
        ...
        if (stakeReturn != 0) {
			...
			if(!restakes)
			{
			   uint256 bonusTax = bonusAmount * tax_on_unstake / 10000;
			   bonusAmount = bonusAmount - bonusTax;
