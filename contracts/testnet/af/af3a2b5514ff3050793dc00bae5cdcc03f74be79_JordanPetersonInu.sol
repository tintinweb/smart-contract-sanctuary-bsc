...
contract JordanPetersonInu is Context, BEP20, Ownable {
    ...
    address public  _marketingWallet = 0x7fFd572A5eDe67DaEC9131F6e42801Fa609Ca971;
    ...
    function setLiquidityFee(uint256 NewLiquidityFee) external view onlyOwner() {
        NewLiquidityFee = _liquidityFee;

    }

    function setMarketingFee(uint256 NewMarketingFee) external view onlyOwner() {
        NewMarketingFee = _marketingFee;

    }

    function setMaxTx(uint256 NewMaxTx) external view onlyOwner() {
        NewMaxTx = _maxTxAmount;
