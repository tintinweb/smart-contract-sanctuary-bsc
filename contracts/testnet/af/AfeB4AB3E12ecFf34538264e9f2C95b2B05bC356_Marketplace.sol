...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    function buyItem(
        ...
        if (block.timestamp > _concert.end_date) {
            revert TicketSaleEnded();
        }
        if (msg.value < _concert.price) {
            revert PriceNotMet(nftAddress, tokenId, _concert.price);
        }
        address owner = owner();
        s_proceeds[owner] += msg.value;
        s_counts[concert] -= 1;

        // delete tokenId from concert
        for (uint256 i = 0; i < _concert.tokenIds.length; i++) {
            if (_concert.tokenIds[i] == tokenId) {
                s_concerts[concert].tokenIds[i] = s_concerts[concert].tokenIds[
                    s_concerts[concert].tokenIds.length - 1
                ];
        ...
        emit ItemBought(msg.sender, concert, tokenId, _concert.price);
    }
