...
contract MFETVault is Context, Ownable, ReentrancyGuard {
    ...
    /// Onchain Stats!!! exit miktarı eklemek ?
    struct Stats {
        uint256 invested;
        uint256 reinvested;
        uint256 withdrawn;
    ...
    // Ganache
    IBEP20 private cToken = IBEP20(0xCB1276a5ea343C9b60f270C6BB291CE68981d54f);
    ...
    =            PUBLIC FUNCTIONS           =
    =======================================*/
    /// @dev reward investors instant
    function rewardInstant(uint256 _reward)
        external
        nonReentrant
        onlyEOA
        returns (uint256)
    {
        ...
        return rewardInvestorsInstant(_reward);
    }

    /// @dev reward investors
    function rewardDaily(uint256 _reward)
        external
        nonReentrant
        onlyEOA
        returns (uint256)
    {
        ...
        totalDeposits += _reward;
        cToken.transferFrom(msg.sender, address(this), _reward);
        return rewardInvestorsDaily(_reward);
    }
    ...
    function allocateFees(uint256 _amount) private {
        ...
        uint256 _drip = _share.mul(dripFee); //40
        uint256 _instant = _share.mul(instantFee); //40
        uint256 _lock = _amount.safeSub(_drip + _instant); //100 -80
    ...
    /// @dev reward internal instant function
    function rewardInvestorsInstant(uint256 _rewards)
        internal
        returns (uint256)
    {
    ...
    /// @dev reward internal daily function
    function rewardInvestorsDaily(uint256 _rewards) internal returns (uint256) {
        totalRewards += _rewards;
        emit onRewardTokenAdded(_rewards, block.timestamp);
