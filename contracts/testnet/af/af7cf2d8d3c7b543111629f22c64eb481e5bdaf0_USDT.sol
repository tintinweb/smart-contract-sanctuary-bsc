...
pragma solidity 0.5.10; // solhint-disable-line
...
contract USDT {
    ...
    IERC20 public token = IERC20(0x337610d27c682E347C9cD60BD4b3b107C9d34dDd); //Token = BSC-USDT
	uint256 constant public INVEST_MIN_AMOUNT = 1e17; // Min  5 USDT
	uint256[] public REFERRAL_PERCENTS 	= [500, 300, 100, 50, 50];
	uint256[] public SEED_PERCENTS 		= [1000, 800, 600, 400, 200];
	uint256 constant public PROJECT_FEE = 500;// Developer 5% fee
	uint256 constant public TIMER_FEE = 100;//   (invest = Time pool 1% fee) : (withdraw = Time pool 1% fee)
	...
	constructor(address payable wallet,ITimerPool _timer) public {
		require(!isContract(wallet));
		commissionWallet = wallet;
        timer = _timer;

        plans.push(Plan(365, 260));
    ...
    function addDownlineRef(address senderAddress, address refUserAddress, uint refLevel) internal {
	 ...
	 function distributeRef(address _referredBy,address _sender, bool _newReferral) internal {
        ...
        if(chkLv2 != 0x0000000000000000000000000000000000000000) {
            ...
            if(_newReferral == true) {addDownlineRef(referralLevel1Address[_referredBy], _customerAddress, 2);}
        }
        ...
        if(chkLv3 != 0x0000000000000000000000000000000000000000) {
            ...
            if(_newReferral == true) {addDownlineRef(referralLevel2Address[_referredBy], _customerAddress, 3);}
        }
        ...
        if(chkLv4 != 0x0000000000000000000000000000000000000000) {
            ...
            if(_newReferral == true) {addDownlineRef(referralLevel3Address[_referredBy], _customerAddress, 4);}
        }
        ...
        if(chkLv5 != 0x0000000000000000000000000000000000000000) {
            ...
            if(_newReferral == true) {addDownlineRef(referralLevel4Address[_referredBy], _customerAddress, 5);}
        }
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		uint256 fee = amounts.mul(TIMER_FEE).div(PERCENTS_DIVIDER);
		uint256 fee2 = amounts.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
        token.transfer(address(timer),fee);
	...
	function withdraw() public {
		...
		uint256 fee = totalAmount.mul(TIMER_FEE).div(PERCENTS_DIVIDER);
        token.transfer(address(timer),fee);
