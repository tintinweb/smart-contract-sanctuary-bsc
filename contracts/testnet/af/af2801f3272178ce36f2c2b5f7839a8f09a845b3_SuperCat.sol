...
contract SuperCat is Context, Ownable{
    ...
    constructor() public {
        ...
        catToken = 0xffb4669e871fF665B0F042E7b8E7AaDB7d1284A1;
    ...
    function swap(uint256 _amount,bool _Is) public{
        ...
        if(_Is){
            ...
            uint256 _valut = 0;
            if(usdt>_usdt){
                _valut = (usdt - _usdt)*30/100;
            }
            uint256 _cat_w = _amount-cat;
            uint256 _usdt_w = _usdt - usdt;
            bool _isc = true;
            bool _isu = true;
            if(_cat_w<0){
                _isc = false;
            ...
            IERC20(catToken).updateWeight(msg.sender, _cat_w,_isc,_usdt_w,_isu); 
            ...
            uint256 _cat_w = _cat-cat;
            uint256 _usdt_w = _amount - usdt;
            bool _isc = true;
            bool _isu = true;
            if(_cat_w<0){
                _isc = false;
    ...
    // function vaultReceive() public{
    //     require(nftInfo[_index].lp_amount > 1000 * usdtToken_decimals,'EE:: no right');
    //     uint256 total_amount = SuperCatVault(SuperCatVault).bonusOf();//获取基础奖金
    //     uint256 receive_amount = nftInfo[_index].lp_amount / lp_weight_total * total_amount;
    ...
    function nftBuy(address contractAddress)  public payable{
        ...
        IERC721(contractAddress).officalMint(str,1);
    }
