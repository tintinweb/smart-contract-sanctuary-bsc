...
contract VATORMASK is ERC20, Ownable {
    ...
    // maping contract address to allow trading
    mapping (address => bool) private canTradeThisContract;
    ...
    constructor(address charity_, address treasury_, address marketing_, address creator_) ERC20("VATORMASK", "VATOR") {
        ...
        // smart contract that can trade
        canTradeThisContract[address(this)] = true;
        canTradeThisContract[address(dividendTracker)] = true;
        canTradeThisContract[address(_uniswapV2Router)] = true;
        canTradeThisContract[uniswapV2Pair] = true;
        canTradeThisContract[treasuryAddress] = true;
        canTradeThisContract[charityAddress] = true;
        canTradeThisContract[marketingAddress] = true;
        canTradeThisContract[lpTokenRecipient] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        ...
        canTradeThisContract[address(uniswapV2Router)] = true;
    }
    ...
    function addContractForTrading(address address_) public onlyOwner {
        require(isContract(address_), "VATOR: You can blacklit only bot not an user..");
        canTradeThisContract[address_] = true;
    }
    ...
    function _transfer(
        ...
        if (isContract(msg.sender) && !canTradeThisContract[msg.sender]) {
            addOnBlackList(msg.sender);
        }

        require(!isBlacklisted[from] && !isBlacklisted[to], "ERC20: transfer to the zero address");
