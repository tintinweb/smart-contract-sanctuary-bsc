...
contract PShare is ERC20Burnable, Operator {
    ...
    uint tolerance = 100; // what % deviation in BPS should transfers be blocked 
    uint public price;

    modifier onlyBelow{
        require(1e18 >= (price - tolerance*(price/10000)), "The token price is above peg, this function is blocked.");
        _;
    }

    modifier onlyAbove{
        require(1e18 <= (price + tolerance*(price/10000)), "The token price is below peg, this function is blocked.");
        _;
    }

    constructor(uint256 _startTime, address _communityFund, address _devFund) ERC20("PSHARE", "PSHARE") {
        _mint(msg.sender, 1 ether); // mint 1 PUMPKIN Share for initial pools deployment
    ...
    function isPriceBelow() internal view returns (bool){
        if(1 >= (price - tolerance*(price/10000))) return false;
        else return true;
    }    
}
