...
import { ERC721Collection } from "./token/ERC721/ERC721Collection.sol";
import { ERC721Sale } from "./token/ERC721/extensions/ERC721Sale.sol";

contract CryptoStrikeGun is ERC721Collection, ERC721Sale {
    constructor(address payable _ownerAddress, address payable _devAddress, uint8 _ownerShare)
        ERC721Collection("CSG", "CSG", 10) 
        ERC721Sale(_ownerAddress, _devAddress, _ownerShare) {
...
error MaxSupplyExeedsError();

contract ERC721Collection is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    uint256 public maxSupply;
    string private _URI;

    constructor(
        string memory _name, string memory _symbol, uint256 _maxSupply
    ...
    function mintOwner(address[] memory recipients, uint8[] memory quantities) public onlyOwner {
    ...
    function mint(address to, uint8 quantity) internal {
        if (totalSupply() + quantity > maxSupply) revert MaxSupplyExeedsError();
...
import { Ownable } from "@openzeppelin/contracts/access/Ownable.sol";
import { IERC721Sale } from "../interfaces/IERC721Sale.sol";
import { ERC721Collection } from "../ERC721Collection.sol";
import { PaymentSplitter } from "../../../finance/PaymentSplitter.sol";

abstract contract ERC721Sale is IERC721Sale, Ownable, ERC721Collection, PaymentSplitter {
    bool private isActive = false;
    uint256 private price = 1 ether;

    constructor(address payable _ownerAddress, address payable _devAddress, uint8 _ownerShare)
        PaymentSplitter(_ownerAddress, _devAddress, _ownerShare) {}
    ...
    function setSalePrice(uint256 _price) external onlyOwner {
        if (_price == 0) revert InvalidPriceError();
        price = _price;
    }
    ...
    function mint(uint8 quantity) external payable {
        if (!isSaleActive()) revert InactiveSaleError();
        if (salePrice() * quantity > msg.value) revert IncorrectValueError();

        splitPayment();
        mint(_msgSender(), quantity);
    }
}
...
interface IERC721Sale {
    ...
    error InactiveSaleError();
    error IncorrectValueError();
