...
contract Decad is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0xEc0c2ee655041adE2b308bfa508b236D6729A985;
    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 50000000 * 10**18; // 50 million
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private constant _name = "DecentralAds Token";
    string private constant _symbol = "DECAD";
    uint8 private constant _decimals = 18;
    uint256 public _taxFee = 2; //2% reflection
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _developmentFee = 1; //1% dev fee
    ...
    uint256 public _maxTxAmount = 1000000 * 10**18; // 1 million
    uint256 public _maxHoldAmount = 1000000 * 10**18; // 1 million
    uint256 private _numTokensSellToAddToLiquidity = 100000 * 10**18; // 100,000
    ...
    constructor () payable {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);//router on bsc testnet 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 //router on bsc mainnet 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(_liquidityFee + _developmentFee + taxFee <= 10, "Sum of taxes is too high");
        _taxFee = taxFee;
    }
    ...
    function setDevelopmentFeePercent(uint256 developmentFee) external onlyOwner() {
        require(_liquidityFee + developmentFee + _taxFee <= 10, "Sum of taxes is too high");
        _developmentFee = developmentFee;
    }
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee + _developmentFee + _taxFee <= 10, "Sum of taxes is too high");
        _liquidityFee = liquidityFee;
    }
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        uint256 maxtx = _tTotal.mul(maxTxPercent).div(
            10**2
        );
        require(maxtx >= _numTokensSellToAddToLiquidity, "Max Tx Size must be greater than liquidity swap size");
        _maxTxAmount = maxtx;
    }
    ...
    function setNumTokensSellToAddToLiquidity(uint256 numTokensPercent) external onlyOwner() {
        uint256 numTokens = _tTotal.mul(numTokensPercent).div(
            10**2
        );
        require(numTokens <= _maxTxAmount, "Liquidity swap size must be lesser than Max Tx Size");
    ...
    function rescueDecadTokens(address to) external onlyOwner() {
        _rOwned[to] = _rOwned[to].add(_rOwned[address(this)]);
    ...
    function _transfer(
        ...
        if( !_isExcludedFromFee[to])
            require(balanceOf(to).add(amount) <= _maxHoldAmount, "Transfer exceeds the receiver's wallet balance limit");
