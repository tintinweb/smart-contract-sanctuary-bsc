...
contract BronzeFoxNFT is ERC721Enumerable, Ownable, Guard, RI, Withdrawable {
    ...
    modifier onlySeller() {
        require(msg.sender == _sellerContract, "Not allowed for mint");
        _;
    }

    constructor(address royaltyAddress_) ERC721("Bronze Fox NFT V1", "BFOX V1")
    {
        royaltyAddress = royaltyAddress_;

        string memory baseTokenURI = "https://legendaryfox.io/meta/bfox/";
        string memory baseContractURI = "https://legendaryfox.io/meta/bfox/main.json";
    ...
    function setSellerContract(address address_) public onlyOwner {
    ...
    function mint() public guarded onlySeller {
        uint256 total = totalToken();
        require(total + 1 <= tokensLimit, "Max limit");

        _tokensCounter.increment();
        _safeMint(tx.origin, totalToken());
        uint256 minted = minters[tx.origin];
        minters[tx.origin] = minted + 1;
        buyers[totalToken()] = tx.origin;
    }
