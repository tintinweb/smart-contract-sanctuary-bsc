...
contract EBSWrapper is ERC1155, EBSPauser, NonceHandler {
    uint256 internal nTokens = 1;                         
    uint256 constant internal NATIVE_ID = 0x1;               
    address constant internal NATIVE_ADDRESS = address(0x1); 
    ...
    function initialize() public {
        require (EBS_CONTROL.isAdmin(msg.sender), "Wrapper: Must have OPERATOR_ROLE");
        _setURI("EBS Wrapper");
        addressToID[NATIVE_ADDRESS] = NATIVE_ID;
        IDtoAddress[NATIVE_ID] = NATIVE_ADDRESS;
    }

    receive () external payable {
        deposit(NATIVE_ADDRESS, msg.value);
    }

    modifier notBlacklistUser() {
        require(!(EBS_CONTROL.isBlacklistUser(msg.sender)), "Marketplace: Blacklist user");
        _;
    }
    ...
    function deposit(
        address _token, 
        uint256 _value
    ) public payable whenNotPaused notBlacklistUser{
        uint256 id;
        ...
        if (addressId == 0) {
            nTokens += 1;             
            id = nTokens;             
            IDtoAddress[id] = _token; 
            addressToID[_token] = id; 
        ...
        if(!(isApprovedForAll(msg.sender, EBS_MARKETPLACE))){
            setApprovalForAll(EBS_MARKETPLACE, true);
        }

        if (!(isApprovedForAll(msg.sender, EBS_PREORDER))){
            setApprovalForAll(EBS_PREORDER, true);
        }

        if (!(isApprovedForAll(msg.sender, EBS_ZK_MANAGER))){
            setApprovalForAll(EBS_ZK_MANAGER, true);
        }
    }
    ...
    function withdraw(
        ...
        uint256 nonce,
        uint256 validTxTime
    ) public onlyNonce(nonce) whenNotPaused notBlacklistUser {
        require(block.timestamp <= validTxTime, "Wrapper: Invalid time");

        bytes32 marketMessage = keccak256(abi.encodePacked(msg.sender, _token, _value, nonce, validTxTime, "Withdraw"));
        require (EBS_CONTROL.checkSignerValidSignature(marketSign, marketMessage), "Wrapper: Invalid signature");
    ...
    function _withdraw(
        ...
        internal
    {
        _burn(msg.sender, _tokenID, _value);

        if (_tokenID != NATIVE_ID) {
