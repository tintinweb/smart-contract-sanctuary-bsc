...
contract PublicSale is ReentrancyGuard, Initializable {
    using SafeERC20 for IERC20;

    uint256 constant public HARVEST_PERIODS = 4;
    uint256 constant public OFFERING_TOKENS = 2;
    ...
    // The offering tokens
    IERC20[OFFERING_TOKENS] public offeringTokens;
    ...
    uint256[OFFERING_TOKENS] public offeringAmounts;
    ...
    event Harvest(
        ...
        uint256[OFFERING_TOKENS] offeringAmount,
        uint256 excessAmount
    );
    event EmergencySweepWithdraw(address indexed receiver, address indexed token, uint256 balance);
    event FinalWithdraw(address indexed receiver, uint256 raisingAmount, uint256[OFFERING_TOKENS] offeringAmounts);


    function initialize(
      IERC20 _stakeToken,
      IERC20[OFFERING_TOKENS] memory _offeringTokens,
      uint256 _startBlock,
      uint256 _endBlockOffset,
      uint256 _vestingBlockOffset, // Block offset between vesting distributions
      uint256[OFFERING_TOKENS] memory _offeringAmounts,
      ...
      address _adminAddress
        ...
        offeringTokens = _offeringTokens;
        ...
        offeringAmounts = _offeringAmounts;
    ...
    function setOfferingAmount(uint256[OFFERING_TOKENS] memory _offerAmounts) public onlyAdmin {
        require(block.number < startBlock, "cannot update during active iao");
        offeringAmounts = _offerAmounts;
    }
    ...
    function harvest(uint256 harvestPeriod) external nonReentrant {
        ...
        uint256[OFFERING_TOKENS] memory offerings = getOfferingAmountPerPeriod(msg.sender);
        for (uint256 i = 0; i < OFFERING_TOKENS; ++i) {
            uint256 offeringAmount = offerings[i];
            offeringTokens[i].safeTransfer(msg.sender, offeringAmount);
        }
        ...
        emit Harvest(msg.sender, offerings, refundingTokenAmount);
    }
    ...
    /// @param _user Address of the user allocation to look up
    function getOfferingAmounts(address _user) public view returns (uint256[OFFERING_TOKENS] memory amounts) {
        if (totalAmount > raisingAmount) {
            for (uint256 i = 0; i < OFFERING_TOKENS; ++i) {
                uint256 offeringAmount = offeringAmounts[i];
                amounts[i] = ((offeringAmount * getUserAllocation(_user)) / 1e12);
            }
        } else {
            for (uint256 i = 0; i < OFFERING_TOKENS; ++i) {
                uint256 offeringAmount = offeringAmounts[i];
                amounts[i] = ((userInfo[_user].amount * offeringAmount) / raisingAmount);
            }
        }
    }

    // get the amount of IAO token you will get per harvest period
    function getOfferingAmountPerPeriod(address _user) public view returns (uint256[OFFERING_TOKENS] memory amounts) {
        uint256[OFFERING_TOKENS] memory offerings = getOfferingAmounts(_user);
        for (uint256 i = 0; i < OFFERING_TOKENS; ++i) {
            uint256 offeringAmount = offerings[i];
            amounts[i] = (offeringAmount / HARVEST_PERIODS);
        }
    }
    ...
    function userTokenStatus(address _user) 
        ...
        returns (
            ...
            uint256[OFFERING_TOKENS] memory offeringTokenHarvest, 
            uint256[OFFERING_TOKENS] memory offeringTokensVested
        ) 
    {
        ...
        uint256[OFFERING_TOKENS] memory userOfferingPerPeriod = getOfferingAmountPerPeriod(_user);

        for (uint256 i = 0; i < HARVEST_PERIODS; i++) {
            if(currentBlock >= harvestReleaseBlocks[i] && !userInfo[_user].claimed[i]) {
                for (uint256 pi = 0; pi < OFFERING_TOKENS; ++pi) {
                    // If offering tokens are available for harvest AND user has not claimed yet
                    offeringTokenHarvest[pi] += userOfferingPerPeriod[pi];
                }
            } else if (currentBlock < harvestReleaseBlocks[i]) {
                for (uint256 pi = 0; pi < OFFERING_TOKENS; ++pi) {
                    // If harvest period is in the future
                    offeringTokensVested[pi] += userOfferingPerPeriod[pi];
                }
            }
        }
    ...
    function finalWithdraw(uint256 _stakeTokenAmount, uint256[OFFERING_TOKENS] memory _offerAmounts)
        external
        onlyAdmin
    {
        require(_stakeTokenAmount <= raisingAmount, "Withdrawing more than raise");
        for (uint256 i = 0; i < OFFERING_TOKENS; ++i) {
            IERC20 offeringToken = offeringTokens[i];
            require(
                _offerAmounts[i] <= offeringToken.balanceOf(address(this)),
                "not enough offering token"
            );
            offeringToken.safeTransfer(msg.sender, _offerAmounts[i]);
        }
        safeTransferStakeInternal(msg.sender, _stakeTokenAmount);
        emit FinalWithdraw(msg.sender, _stakeTokenAmount, _offerAmounts);
    }
