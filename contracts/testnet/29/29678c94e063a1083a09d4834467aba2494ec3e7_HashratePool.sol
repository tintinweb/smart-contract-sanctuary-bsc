...
contract HashratePool is AYiBoxBase {
    ...
    mapping(address => bool) public _allowers;
    ...
    function addAllower(address allower) public onlyOwner {
        _allowers[allower] = true;
    }

    function removeAllower(address allower) public onlyOwner {
        _allowers[allower] = false;
    }
...
*/
    ...
    function HashrateTotal() external view returns (uint256 _total) {
        ...
        //记录质押时间节点并返回最新量
    function recordStake(uint256 _time) internal returns (uint256 base) {
        uint256 stakeTimeStamp = YiBoxBase.GetParam(YiBoxType.HashrateStakeTimeStamp);
        if (stakeTimeStamp == 0) {
            YiBoxBase.SetParam(YiBoxType.HashrateStakeTimeStamp, _time);
        } else {

            uint256 _delay;
            if (_time > stakeTimeStamp) {
                _delay = _time - stakeTimeStamp;
            }
            // uint256 _delay = _time-stakeTimeStamp;
    ...
    function queryIncomeNow(Hashrate memory hashrates, uint256 _time) internal view returns(uint256 IncomeNow){
        uint256 stakeTimeStamp = YiBoxBase.GetParam(YiBoxType.HashrateStakeTimeStamp);
        uint256 lTime = hashrates.info >> 192;
        uint256 hashrate = (hashrates.info >> 64) % 0x10000000000000000;
        uint256 base = YiBoxBase.GetLastStakeNode(YiBoxType.HashrateStakeNode) - YiBoxBase.GetParam(YiBoxType.HashrateStakeNode,lTime) + calcIncome(1, _time - stakeTimeStamp,YiBoxBase.GetParam(YiBoxType.HashrateMaxHashrate),lastAirDrop);
        IncomeNow = base * hashrate; 
    }

    //结算某个人的收益
    function settlementTarget(address _tar, uint256 _time) external {
        require(_allowers[msg.sender], "!allow");
        Hashrate memory hashrate = YiBoxBase.GetHashrate(_tar);
        ...
        if (_hashrate != 0) {
            ...
            uint256 _incomeNow = queryIncomeNow(hashrate,_time);
            hashrate.allIncome += _incomeNow;
            hashrate.balance += _incomeNow; 
        }
        
        hashrate.info = (hashrate.info << 64 >> 64) + (_time << 192);
        YiBoxBase.SetHashrate(YiBoxType.HashrateSet, _tar, hashrate);
    } 
    ...
    function airDrop() external {
        ...
        recordStake(lastestAirDroping);
    }
    

    //计算累计收益
    function getIncome(Hashrate memory _hashrate, uint256 _time) internal returns (uint256 _income) {
        uint256 _new = recordStake(_time);
    ...
    //收益更新
    // function updateIncome(Hashrate memory _hashrate, uint256 _time) internal returns (uint256 _income) {
    //     _income = getIncome(_hashrate, _time);
    //     _hashrate.allIncome += _income;
    //     _hashrate.balance += _income;
    //     _hashrate.info = (_hashrate.info << 64 >> 64) + (_time << 192);
    //     YiBoxBase.SetHashrate(YiBoxType.HashrateSet, tx.origin, _hashrate);
    // }

    //更新算力池
    // function update(address _staker, uint256 _time) internal returns(uint256) {
    ...
    //     uint256 _income = updateIncome(_hashrate,_time);
    //     return _income;
    // }
    ...
    function openBox(uint256 _num) external returns(uint256[] memory _tks, uint256[] memory _qus, uint256[] memory _hrs, uint256[] memory _tys) {
        ...
        this.settlementTarget(_msgSender(), _time);
        ...
        // update(_msgSender(),_time);
        this.settlementAll(_time);
    ...
    function queryIncome(address _staker) external view returns(uint256,uint256,uint256,uint256){
        ...
        require(_hashrate != 0, "no staker");
        uint256 _incomePer = calcIncome(_hashrate, 1,YiBoxBase.GetParam(YiBoxType.HashrateMaxHashrate),lastAirDrop); //每秒收益
        uint256 _incomeNow = queryIncomeNow(hashrate, block.timestamp);
    ...
    function withdraw(address _sender, uint256 _amount) external returns(bool) {
        ...
        uint256 _time = block.timestamp;
        uint256 _incomeNow = getIncome(hashrates,_time);
        _balance += _incomeNow;
