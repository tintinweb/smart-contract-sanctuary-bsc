...
contract ReignToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function initialize(address busdAddress, address _router) public initializer {
        ...
        liquidityReceiver = 0xd88C06E7f08BfcF1159C6eB91f64B6386d39899E;
        treasuryReceiver = 0xd88C06E7f08BfcF1159C6eB91f64B6386d39899E;
        teamReceiver = 0xd88C06E7f08BfcF1159C6eB91f64B6386d39899E;
        burnReceiver = 0xd88C06E7f08BfcF1159C6eB91f64B6386d39899E;
    ...
    function rawFulfillRandomWords( uint256[] memory randomWords) external {
    ...
    function fulfillRandomWords( uint256[] memory randomness) internal {
    ...
    function getNewRebaseStakedToken(address user) external view returns (uint256 _nextRebase) {
        //uint256 nextRebase;
        for (uint256 i = 0; i < stakingTypeCount; i++) {
            _nextRebase += (stacking[user][i].amount.div(_yields[i + 1]) - stacking[msg.sender][i].amountClaim) * rewardYields[i + 1] / rewardYieldDenominator;
        }
       // return nextRebase;
    }
    ...
    function getRebaseDailyStakedToken(address user) public view returns (uint256 _nextRebase) {
        //uint256 nextRebase;
        _nextRebase += (stacking[user][0].amount.div(_yields[1]) - stacking[msg.sender][0].amountClaim) * 19 / 1000;
        _nextRebase += (stacking[user][1].amount.div(_yields[2]) - stacking[msg.sender][1].amountClaim) * 21 / 1000;
        _nextRebase += (stacking[user][2].amount.div(_yields[3]) - stacking[msg.sender][2].amountClaim) * 23 / 1000;
        //return nextRebase;
    }
