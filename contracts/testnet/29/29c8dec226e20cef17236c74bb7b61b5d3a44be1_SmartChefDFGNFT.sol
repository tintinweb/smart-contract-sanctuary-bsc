...
/** Gold Digger Miner Clan 2.0
  * NFT Farming: Creating Utility and Liquidity for NFT 
  * The Future of Gold Digger NFTs
  * We made that art massively interactive - carrying value, purpose and a golden earring!
  * Endorsed by global partners, the unvealing phase will give you countless utilites and a solid future of your NFTs:

   //* DeFi is finally coming to the blockchain gaming space, which is good news for the pro gaming geeks! 
       Now you can show your gaming skills, collect rewards as NFTs, and then stake those rewards within the gaming platform to earn yields. 
     * Gold Digger is an intense DeFi gaming platform that brings together DeFi and blockchain gaming to elevate the gaming experience to the next level. 
       Gold Digger is revolutionizing yield farming by leveraging the NFT gamification. There has been a massive surge in NFT space.
     * A 5 year plan to fundamentally improve the climate (with partners like Plant-for-the-Planet.org or Greenpeace)
     * Fully doxxed team with a global reach!

  //* and many more utilities on the way. The VIP sale starts 48 hours early and is strictly limited. It will be announced on our Discord channel.
//*
...
interface IDorinFirebeardNFT {
    function accrueDFG(address user, uint amount) external;
    function tokenFreeze(uint tokenId) external;
    function tokenUnfreeze(uint tokenId) external;
    function getDFG(uint tokenId) external view returns(uint);
    function getInfoForStaking(uint tokenId) external view returns(address tokenOwner, bool stakeFreeze, uint bigBoost);
}

contract SmartChefDFGNFT is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint public totalDFGSupply;
    uint public lastRewardBlock;
    address[] public listRewardTokens;
    IDorinFirebeardNFT public nftToken;

    // Info of each user
    struct UserInfo {
        uint[] stakedTokensId;
        uint stakedDfgAmount;
    }
    ...
    event StakeTokens(address indexed user, uint amountDFG, uint[] tokensId);
    event UnstakeToken(address indexed user, uint amountDFG, uint[] tokensId);
    event EmergencyWithdraw(address indexed user, uint tokenCount);

    constructor(IDorinFirebeardNFT _nftToken) {
        nftToken = _nftToken;
    }
    ...
    function getUserStakedRbAmount(address _user) public view returns(uint){
        return userInfo[_user].stakedDfgAmount;
    }
    ...
    function pendingReward(address _user) external view returns (address[] memory, uint[] memory) {
        ...
        if(user.stakedDfgAmount == 0){
            return (listRewardTokens, rewards);
        }
        uint _totalDFGSupply = totalDFGSupply;
        ...
        for(uint i = 0; i < listRewardTokens.length; i++){
            ...
            if (_multiplier != 0 && _totalDFGSupply != 0 && curRewardToken.enabled == true) {
                uint curMultiplier;
                ...
                (curMultiplier * curRewardToken.rewardPerBlock * 1e12 / _totalDFGSupply);
            } else {
                _accTokenPerShare = curRewardToken.accTokenPerShare;
            }
            rewards[i] = (user.stakedDfgAmount * _accTokenPerShare / 1e12) - rewardDebt[_user][curToken];
        }
        return (listRewardTokens, rewards);
    }
    ...
    function updatePool() public {
        ...
        uint _totalDFGSupply = totalDFGSupply; //Gas safe

        if(multiplier == 0){
            return;
        }
        lastRewardBlock = block.number;
        if(_totalDFGSupply == 0){
            return;
        }
        for(uint i = 0; i < listRewardTokens.length; i++){
            address curToken = listRewardTokens[i];
            ...
            if(curRewardToken.enabled == false || curRewardToken.startBlock >= block.number){
                ...
                rewardTokens[curToken].accTokenPerShare += (tokenReward * 1e12) / _totalDFGSupply;
            }
        }
    }
    ...
    function _updateRewardDebt(address _user) internal {
        for(uint i = 0; i < listRewardTokens.length; i++){
            rewardDebt[_user][listRewardTokens[i]] = userInfo[_user].stakedDfgAmount * rewardTokens[listRewardTokens[i]].accTokenPerShare / 1e12;
        }
    }
    ...
    function _withdrawReward() internal {
        ...
        for(uint i = 0; i < _listRewardTokens.length; i++){
            ...
            uint pending = user.stakedDfgAmount * curRewardToken.accTokenPerShare / 1e12 - rewardDebt[msg.sender][_listRewardTokens[i]];
            if(pending > 0){
                curRewardToken.rewardsForWithdrawal -= pending;
                rewardDebt[msg.sender][_listRewardTokens[i]] = user.stakedDfgAmount * curRewardToken.accTokenPerShare / 1e12;
    ...
    function stake(uint[] calldata tokensId) public nonReentrant {
        ...
        if(depositedBigBoost > 0){
            userInfo[msg.sender].stakedDfgAmount += depositedBigBoost;
            totalDFGSupply += depositedBigBoost;
        }
    ...
    function unstake(uint[] calldata tokensId) public nonReentrant {
        ...
        for(uint i = 0; i < tokensId.length; i++){
            ...
            for(uint j = 0; j < user.stakedTokensId.length; j++){
                if(tokensId[i] == user.stakedTokensId[j]){
                    ...
                    withdrawalDFGAmount += nftToken.getDFG(tokensId[i]);
        ...
        if(withdrawalDFGAmount > 0){
            user.stakedDfgAmount -= withdrawalDFGAmount;
            totalDFGSupply -= withdrawalDFGAmount;
            _updateRewardDebt(msg.sender);
        }
        emit UnstakeToken(msg.sender, withdrawalDFGAmount, tokensId);
    }

    // Withdraw without caring about rewards. EMERGENCY ONLY.
    function emergencyUnstake() public {
        uint[] memory tokensId = userInfo[msg.sender].stakedTokensId;
        totalDFGSupply -= userInfo[msg.sender].stakedDfgAmount;
