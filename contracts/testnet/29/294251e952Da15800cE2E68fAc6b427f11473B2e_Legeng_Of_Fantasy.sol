...
contract Legeng_Of_Fantasy is ERC721Enumerable, Ownable {
    ...
    string public baseURI = "ipfs://QmYwp8HnZrgzSpP7uENE1nxU9tUvEooX6HLpfyW7mUw3PL/";
    bool public paused = false; // 可以拿來暫停或者開啟 Mint
    uint256 public cost = 0.011 ether; // Mint 價格(0.05bnb)
    uint256 public maxSupply = 450; // 只有300個 NFT
    uint256 public maxMintAmount = 5; // 一次最多只能 Mint 5個
    uint256 private _numAvailableTokens; //目前還有的數量
    mapping(uint => uint) private _availableTokens;

    constructor() ERC721("The Legeng Of Fantasy OB", "TLOFOB") payable {
        // 剛開始我們先自己 Mint 一個
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        require(_mintAmount <= maxMintAmount, "mint amount error"); // 鑄造的數量不可以大於每次最大鑄造數量
        require(cost * _mintAmount <= msg.value, "price error"); // Mint 的價格不可以少於我們訂定的價格
