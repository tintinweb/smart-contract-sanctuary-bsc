...
contract commissionContract is Ownable {
   ...
   uint256 refferalCommissionPercentage; //10^2
   ...
   function getRefferalCommissionPercentage() public view returns(uint256){
       return refferalCommissionPercentage;
   }
   ...
   function updateRefferalComssionPercentage(uint256 _refferalCommissionPercentage) public onlyOwner {
       refferalCommissionPercentage = _refferalCommissionPercentage;
   }
 
    function multipleOutputs (address[] memory addresses, uint256[] memory amt, address _referralAddress) public payable {
    ...
    uint256 commissionForReferral = _referralAddress != address(0)? (refferalCommissionPercentage.mul(amountTotal)).div(10000) : 0;
    if(msg.value < ((amountTotal.add(commissionForAdmin)).add(commissionForReferral))){
      ...
      payable(commissionAddress).transfer(commissionForAdmin);
       if(_referralAddress != address(0)){
           payable(_referralAddress).transfer(commissionForReferral);
       }
    }
