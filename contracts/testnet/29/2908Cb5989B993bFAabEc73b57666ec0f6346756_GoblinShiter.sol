...
contract GoblinShiter {
    ...
    */
    ...
    function getGoblinShitSinceLastMix(address _shitAddress) private view returns(uint) {
        uint secondsPassed = block.timestamp - lastShiting[_shitAddress];
        return shitingShiters[_shitAddress] * secondsPassed * caculateDayRate(secondsPassed) / 100 / 1 days;     
    }

    function caculateDayRate(uint joinTime) private pure returns(uint withdrawRate) {
        if(joinTime < 7 days)  {
            withdrawRate = 3;
        } else if (joinTime < 14 days) {
            withdrawRate = 4;
        } else if (joinTime < 21 days) {
            withdrawRate = 5;
        } else {
            withdrawRate = 6;
        }
