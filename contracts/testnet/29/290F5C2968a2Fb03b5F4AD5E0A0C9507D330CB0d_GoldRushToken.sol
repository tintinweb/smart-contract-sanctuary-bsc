...
contract GoldRushToken is Context, IERC20, Ownable {
  ...
  uint256 private constant _total = 1000 * 10**6 * 10**18;
  uint256 private constant MAX = ~uint256(0);

  uint256[3] public buyFees = [8, 3, 1]; // 8% reward fee, 3% staking fee, 1% liquidity fee
  uint256[3] public sellFees = [8, 3, 1]; // 10% reward fee, 3% staking fee, 1% liquidity fee
  ...
  event UpdatedBuyFees(uint256[3] oldFees, uint256[3] newFees);
  event UpdatedSellFees(uint256[3] oldFees, uint256[3] newFees);
  ...
  function setStakingPool(address poolAddress) external onlyOwner {
    emit UpdateStakingPool(stakingPool, poolAddress);
    stakingPool = poolAddress;    
  }

  function setRewardPool(address poolAddress) external onlyOwner {
    emit UpdateRewardPool(rewardPool, poolAddress);
  ...
  function SetBuyFees(uint256[3] memory fees) external onlyOwner {
    require (fees[0] < 30, "value can not be over 30");
    require (fees[1] < 30, "value can not be over 30");
    require (fees[2] < 30, "value can not be over 30");
    emit UpdatedBuyFees(buyFees, fees);
    buyFees = fees;
  }

  function SetSellFees(uint256[3] memory fees) external onlyOwner {
    require (fees[0] < 30, "value can not be over 30");
    require (fees[1] < 30, "value can not be over 30");
    require (fees[2] < 30, "value can not be over 30");
    emit UpdatedSellFees(sellFees, fees);
    sellFees = fees;
  }
  ...
  function _takeFees(uint256 amount, uint256[3] memory fees) internal returns(uint256) {
    ...
    _balances[stakingPool] = _balances[stakingPool].add(stakingFee);
  ...
  function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
    ...
    if (sender == pancakeswapV2Pair && takeFee) {
      tTransferAmount = _takeFees(amount, buyFees);
    } else if (recipient == pancakeswapV2Pair && takeFee) {
      tTransferAmount = _takeFees(amount, sellFees);
    }
