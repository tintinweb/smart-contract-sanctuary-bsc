...
contract BurnPortal is SafeOwnable {
    ...
    event NewBabyDogeTokenTax(uint8);
    ...
    /*
     * @notice Sets new BabyDogeToken tax
     * @param _babyDogeTokenTax New tax value, in %. 10 = 10%
     */
    function setBabyDogeTokenTax(uint8 _babyDogeTokenTax) external onlyOwner {
        require(babyDogeTokenTax != _babyDogeTokenTax);
    ...
    function getPersonalDiscount(address account) public view returns(uint256) {
        ...
        int256 min = 0;
        int256 max = int256(discounts.length - 1);
