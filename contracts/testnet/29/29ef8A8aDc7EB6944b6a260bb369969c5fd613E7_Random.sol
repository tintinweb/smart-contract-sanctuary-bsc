...
interface IRandom {
     ...
     */
    function getRandomImageByRange(uint256 _stage) external view returns (string memory);
     ...
     */
    function getSplittableWithIncreaseChance(uint16 probability) external view returns (uint8);
...
contract Random is IRandom, ISeed, Ownable {
    ...
    uint256 constant INVERSE_BASIS_POINT = 1000;
    ...
    function getRandomImageByRange(uint256 _stage) external view override returns (string memory) {
        ...
        returns (uint8)
    {
        ...
        uint16 probabilityofFail = 949;

        if (probability > probabilityofFail) {
            probabilityofFail = 0;
        } else {
            probabilityofFail = 949 - probability;
        }

        if (number > probabilityofFail) {
            return uint8(CellData.Class.SPLITTABLE_NANO);
        }

        return uint8(_getChosenClass(number));
    }
    ...
    function getRandomStage(uint256 _stage, uint16 probabilityIncrease)
        ...
        returns (uint256)
    {
        require(_stage >= 0 && _stage <= MAX_LEVEL_OF_EVOLUTION);
    ...
    function getEvolutionTime() external view override returns (uint256) {
        ...
        currentBlock = currentBlock.add(group.startBlock.add((group.step.mul(stage))));
        return currentBlock;
    }

    function getGroup(uint256 number) private pure returns (Group memory group) {
    ...
    function _getChosenClass(uint256 number) private pure returns (CellData.Class) {
    ...
    function random() private view returns (uint256 randomNumber) {
        randomNumber = uint256(
            keccak256(abi.encodePacked(blockhash(block.number.sub(1)), msg.sender, m_seed))
        );
    }
}