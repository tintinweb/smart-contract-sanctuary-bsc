...
contract SpaceMarvelBoxV2 is Ownable, AccessControl, ReentrancyGuard {
    ...
    bytes32 private constant CREATE_BOX_ROLE = keccak256("CREATE_BOX_ROLE");

    constructor() {
        _setupRole(CREATE_BOX_ROLE, _msgSender());
    }
    ...
    function createBox(
        ...
        require(
            hasRole(CREATE_BOX_ROLE, _msgSender()),
            "Error: The caller must have create box role"
        );
    ...
    function buyBox(address _nft, uint8 _paymentIndex)
        external
        payable
        nonReentrant
    {
        ...
        if (box.isPrivate) {
            require(box.whitelists[_msgSender()], "Error: not whitelist");
        }
        require(
    ...
    function claimPayment(address[] calldata _nfts) external nonReentrant {
        for (uint256 i = 0; i < _nfts.length; i++) {
            Box storage box = boxs[_nfts[i]];
            require(box.creator == _msgSender(), "Error: not owner");
            uint256 total = box.idList.length;
    ...
    function setWhitelists(
        ...
        for (uint8 i = 0; i < _users.length; i++) {
            box.whitelists[_users[i]] = _isWhitelists[i];
        }
    }
