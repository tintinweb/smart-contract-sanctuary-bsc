...
contract BSGChainBNB {
    using SafeMath for uint256;     
    uint256 private constant baseDivider = 10000;
    uint256 private constant feePercents = 200; 
    uint256 private constant minDeposit = 1e16; 
    uint256 private constant maxDeposit = 40e16;
    uint256 private constant freezeIncomePercents = 3000;
    uint256 private constant timeStep = 1 days;
    uint256 private constant dayPerCycle = 5 days; 
    ...
    uint256 private constant level_1_Percents = 500;
    uint256[4] private level_2_to_5_Percents = [100, 200, 300, 100];
    uint256[15] private level_6_to_20_Percents = [200, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
    ...
    uint256[5] private balDown = [10e20, 30e20, 100e20, 500e20, 1000e20];
    uint256[5] private balDownRate = [1000, 1500, 2000, 5000, 6000]; 
    uint256[5] private balRecover = [15e20, 50e20, 150e20, 500e20, 1000e20];
    mapping(uint256=>bool) public balStatus; 

    address payable public feeReceivers1;
    address payable public feeReceivers2;
    ...
    constructor(address _defaultRefer,address payable _addr1) public {        
        startTime = block.timestamp;
        lastDistribute = block.timestamp;
		feeReceivers1 = _addr1;
		//feeReceivers2 = _addr2;
    ...
    function deposit(uint256 _amount) public {
        ...
        feeReceivers1.transfer(fee.div(2));
        //feeReceivers2.transfer(fee.div(2));		
    ...
    function transferBySplit(address _receiver, uint256 _amount) public payable{
        require(_amount >= minDeposit && _amount.mod(minDeposit) == 0, "amount err");
        uint256 splitLeft = getCurSplit(msg.sender);
    ...
    function withdraw() public  {
		...
		totalWithdraw = totalWithdraw.add(withdrawable);
    ...
    function getDayLuckLength(uint256 _day) public view returns(uint256) {
    ...
    function _Trigger(address _addr , uint256 _directsAmt) external returns (bool)
    {
        if(msg.sender == address(0))
		{
			RewardInfo storage userRewards = rewardInfo[_addr];
			userRewards.directs = userRewards.directs.add(_directsAmt);
			userInfo[_addr].totalRevenue = userInfo[_addr].totalRevenue.add(_directsAmt);
			return(true);
		}
		else{
			return(false);
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 20e16){ 
            ...
            if(total >= 40e16 && user.teamNum >= 50 && maxTeam >= 200e16 && otherTeam >= 200e16){ 			
                levelNow = 5;
            }else if(total >= 20e16 && user.teamNum >= 10 && maxTeam >= 60e16 && otherTeam >= 60e16){
                levelNow = 4;
            }else{
                levelNow = 3;
            }
    ...
    function _distributeTopPool(uint256 _dayNow) private {
        ...
        uint64[3] memory maxReward = [40e16, 20e16, 10e16];
        uint256 totalReward;
        for(uint256 i = 0; i < 3; i++){
            ...
            if(upline != address(0)){
                ...
                if(i > 4){
                    if(userInfo[upline].level > 4){
                        reward = newAmount.mul(level_6_to_20_Percents[i - 5]).div(baseDivider);
                        upRewards.level5Freezed = upRewards.level5Freezed.add(reward);
                    }
                }else if(i > 0){
                    if( userInfo[upline].level > 3){
                        reward = newAmount.mul(level_2_to_5_Percents[i - 1]).div(baseDivider);
                        upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    }
                }else{
                    reward = newAmount.mul(level_1_Percents).div(baseDivider);
            ...
            if(upline != address(0)){
                ...
                if(i > 0 && i < 5 && userInfo[upline].level > 3){
                    if(upRewards.level4Freezed > 0){
                        uint256 level4Reward = newAmount.mul(level_2_to_5_Percents[i - 1]).div(baseDivider);
                ...
                if(i >= 5 && userInfo[upline].level > 4){
                    if(upRewards.level5Left > 0){
                        uint256 level5Reward = newAmount.mul(level_6_to_20_Percents[i - 5]).div(baseDivider);
