...
contract DreamHorseNFT is ERC721URIStorage, Ownable {
    ...
    event GetNFT2(uint256[] tokenId, uint256 [10]types);

	uint256 public counter;//NFT计数器
    ...
    mapping(uint256 => uint256) public NFTIDToTypes;//NFT类型

    mapping(uint256 => uint256) public NFTIDToIamgeId;//NFT类型

    mapping(address => uint256[]) public userNFTIDs;

    mapping(address => mapping(uint256 => uint256)) public userNFTTypeNumber;//用户某类型NFT数量

    uint256[4][3] public boxRates;//盲盒开出不同类型NFT的概率

    uint256[3] public boxsPrices;//盲盒价格

    uint256[3] public boxMaxTimes;//盲盒价格

    uint256[5] public imageNumber;//NFT图片编号

    //预售时间
    uint256 public presaleStartTime = 1649692800;
    uint256 public presaleEndTime = 1652976000;

    // address USDT = address(0x55d398326f99059ff775485246999027b3197955);
    address USDT = 0xEA551129eCa6EA7A7113521895c23Cda7eE78F3f;
	...
	constructor() ERC721("DreamHorseNFT", "DHN"){
        ...
        boxRates[0] = [95,97,99,100];
        boxRates[1] = [49,98,99,100];
        boxRates[2] = [0,49,98,99];

        boxsPrices = [99,199,299];

        boxMaxTimes = [5000,3000,1000];

        imageNumber = [20,20,20,20,20];
	}
    ...
    function openBlindBox(uint256 boxId) public returns (uint256){
        ...
        //钱转钱包地址,不然咋提取?
        // ERC20(USDT).approve(address(this), boxsPrices[boxId - 1] * 10 ** decimals);
        ERC20(USDT).transferFrom(msg.sender, 0x07DBD07e213798b8d8885551611e42Fbc8418132, boxsPrices[boxId - 1] * 10 ** decimals);
    ...
    //游戏开盲盒
    function openBlindBox2(address user, uint256 boxId) public onlyController returns (uint256){
        ...
        //钱转钱包地址,不然咋提取?
        // ERC20(USDT).approve(address(this), boxsPrices[boxId - 1] * 10 ** decimals);
        ERC20(USDT).transferFrom(user, 0x07DBD07e213798b8d8885551611e42Fbc8418132, boxsPrices[boxId - 1] * 10 ** decimals);

        uint256 types = _getNFTType(boxId);

        uint256 tokenId = _createNFT(user, types);
	...
	function createNFT(address user, uint256 NFTType) public onlyController returns (uint256){
        ...
        uint256 number =  (uint256(keccak256(abi.encodePacked(block.timestamp, randNum++,msg.sender)))) % imageNumber[NFTType - 1] + 1;

        NFTIDToIamgeId[tokenId] = NFTType * 10000 + number;

        emit GetNFT(msg.sender, tokenId, NFTType, NFTIDToIamgeId[tokenId]);
    ...
    function _rand() internal virtual returns(uint256) {
        ...
        uint256 number1 =  uint256(keccak256(abi.encodePacked(block.timestamp, randNum++, msg.sender))) % (4 * 10 ** 10) + 1968769868;

        uint256 number2 =  uint256(keccak256(abi.encodePacked(block.timestamp, randNum++, msg.sender))) % (2 * 10 ** 10) + 1936586796;
    ...
    function getUserIamgeIdsByPageNumber(address user, uint256 pageNumber) public returns(uint256[10] memory pageIamgeIds) {

        uint256 num = userNFTIDs[user].length;

        uint256[] memory iamgeIds = new uint256[](uint256(num));
        ...
        for(uint256 i = 0; i < num; i++){
            ...
            if(userNFTIDs[user][i] == 0){
                ...
                iamgeIds[i - count] = NFTIDToIamgeId[userNFTIDs[user][i]];
            }
        }
        ...
        for(uint256 j = start; j < end; j++){
            if(j < num && iamgeIds[j] != 0){
                uint256 k = j - start;
                pageIamgeIds[k] = iamgeIds[j];
            }
        }

        emit GetNFT2(iamgeIds, pageIamgeIds);

        return pageIamgeIds;
    }

    function getUserIamgeIds(address user) public view returns(uint256[] memory iamgeIds) {

        uint256 num = userNFTIDs[user].length;

        iamgeIds = new uint256[](uint256(num));

        for(uint256 i = 0; i < num; i++){
            if(userNFTIDs[user][i] != 0){
                iamgeIds[i] = NFTIDToIamgeId[userNFTIDs[user][i]];
            }
        }
    ...
    function _transfer(
        ...
        // require(tansferEnabled, "ERC721: transf is not enabled");
        // require(!blackList[from], "ERC721: transfer from blackList");
