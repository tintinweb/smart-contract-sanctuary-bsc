...
contract SafeswapRouter is ISafeswapRouter02, Initializable {
    ...
    function swapExactTokensForTokens(
        ...
        if (nameToInfo[path[0]].enabled == true && killSwitch == false && (nameToInfo[path[0]].sellFeePercent > 0)) {
            ...
            TransferHelper.safeTransferFrom(path[0], to, nameToInfo[path[0]].feesAddress, deduction);
        }
        amounts = SafeswapLibrary.getAmountsOut(factory, amountIn, path);
        ...
        TransferHelper.safeTransferFrom(path[0], to, SafeswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(path[0], to, SafeswapLibrary.pairFor(factory, path[0], path[1]), amountIn);
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"71ea2c5753a7e30a30fd2ef21155a780c484cdc9ed455a272a48cdf8ab1a3365" // init code hash
                        )
                    )
                )
            )
