...
contract DF is IERC721Receiver {
    ...
    event UnionReward(address from, address to, uint64 reward);
    ...
    function treeOf(address account) public view returns (Tree[] memory, uint32){
        ...
        for (uint i = 0; i < indexes.length; i++) {
            userIntegral += allTree[indexes[i]].integral * allTree[indexes[i]].num;
    ...
    function buyTree(uint8 tType, uint256 num) public onlyActivate {
        ...
        amount = amount * 9 / 10;
        uint64 reward = _pushReward(msg.sender, amount);
        uint64 uReward = _unionReward(msg.sender, amount);
        forest.dividend += uint64(amount * 40 / 100);
        forest.pool += uint64(amount * 10 / 100);
        forest.gold += uint64(amount * 2 / 100);
        forest.sliver += uint64(amount * 1 / 100);
        forest.community += uint64(amount * 5 / 100);
        forest.bottom += uint64(amount * 10 / 100);
        forest.foundation += uint64(amount - reward - uReward - amount * 40 / 100 - amount * 12 / 100 - amount * 2 / 100 - amount / 100 - amount * 5 / 100 - amount * 10 / 100);
        forest.integral += uint64(integrals[tType] * num);
        if (forest.timestamp == 0) {
    ...
    function water() public onlyActivate returns (uint256){
        ...
        if (rand < 100 && nft.balanceOf(address(this)) > 0) {
    ...
    // 工会奖励
    function _unionReward(address account, uint256 amount) private returns (uint64){
        address superUser = _superUser[account];
        uint64 uReward;
        while (superUser != address(0)) {
            if (_unions[superUser]) {
                uReward = uint64(amount * 5 / 100);
    ...
    function communityUnion(address[] memory accounts) public onlyOwner {
        ...
        require(forest.community > 0, "No community");
        uint64 t = forest.community;
        uint64 firstAmount = forest.community * 15 / 1000;
        mgf.transfer(accounts[0], firstAmount);
        t -= firstAmount;
        if (accounts.length > 1) {
            uint64 secondAmount = forest.community * 10 / 1000;
            mgf.transfer(accounts[1], secondAmount);
            t -= secondAmount;
            if (accounts.length > 2) {
                uint64 thirdAmount = forest.community * 5 / 1000;
                mgf.transfer(accounts[2], thirdAmount);
                t -= thirdAmount;
                if (accounts.length > 3) {
                    uint64 amount = t / uint64(accounts.length - 3);
