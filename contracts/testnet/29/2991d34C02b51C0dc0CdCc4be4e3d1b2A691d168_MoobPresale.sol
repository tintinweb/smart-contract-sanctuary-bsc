...
contract MoobPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale Moob, 10 BUSD
	uint256 BUSDPerToken = 10* 10 ** 9;
    ...
    receive() payable external {
        ...
        Moob.safeTransfer(msg.sender, msg.value.div(10));
    }
	...
	function getMoobTokensLeft() external view returns (uint256) {
		return Moob.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some Moob tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed Moob tokens");
		require(Moob.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough Moob tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		Moob.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldMoob() external onlyOwner {
		uint256 amount = Moob.balanceOf(address(this)) - totalTokensSold;
		Moob.safeTransfer(msg.sender, amount);
	}

	function withdrawAllMoob() external onlyOwner {
		Moob.safeTransfer(msg.sender, Moob.balanceOf(address(this)));
	}
