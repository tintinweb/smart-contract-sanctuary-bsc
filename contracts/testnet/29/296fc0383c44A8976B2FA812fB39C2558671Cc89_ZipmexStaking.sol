...
contract ZipmexStaking is ReentrancyGuardUpgradeable {
     ...
     * @return stakeData array of stake records
     * @return totalReward array of total rewards Struct
     * @return claimableReward array of claimable rewards
     */
        ...
        )
    {
        for (uint256 i; i < 4; i++) {
            ...
            totalReward[i] = _getTotalReward(
                stakeData[i].rewardPerSecond,
                (stakeData[i].stakeUnlockTime - stakeData[i].stakingTime)
            );

            // claimable reward per vault
            claimableReward[i] = _getTotalReward(
                stakeData[i].rewardPerSecond,
                _getInterval(
                    stakeData[i].stakeUnlockTime,
                    stakeData[i].lastClaimTime
                )
            );
        }
    }
