...
import "./INFTElement.sol";
import "./INFTComposite.sol";
...
contract Test {
    ...
    struct PropertyScore {
        uint64[] property; //NFT ID
        uint32[] score; //属性得分
    }

    uint256 public maxIdV1;
    mapping(uint256 => uint64[]) private property;
    mapping(uint256 => uint32[]) private score;

    function batchMintNFT(PropertyScore[] memory propertyScore) external {
        PropertyScore memory propertyScore_;
        for (uint256 i = 0; i < propertyScore.length; i++) {
            maxIdV1 = maxIdV1.add(1, "");
            propertyScore_ = propertyScore[i];
            property[maxIdV1] = propertyScore_.property;
            score[maxIdV1] = propertyScore_.score;
        }
    }

    function v1() external pure returns (bytes memory) {
        return new bytes(32);
    }

    function v2(uint256 x) external pure returns (bytes32) {
        return bytes32(x);
    }

    function v3(uint256 x) external pure returns (bytes memory) {
        return new bytes(x);
    }
    ...
    function getMaxUint256V1() external pure returns (uint256) {
        return ~uint256(0);
    }

    function getMaxUint256V2() external pure returns (bytes memory) {
        uint256 x = ~uint256(0);
    ...
    function getMaxUint64V1() external pure returns (uint64) {
        return ~uint64(0);
    }

    function getMaxUint64V2() external pure returns (bytes memory) {
        uint64 x = ~uint64(0);
    ...
    function getMaxUint32V1() external pure returns (uint32) {
        return ~uint32(0);
    }

    function getMaxUint32V2() external pure returns (bytes memory) {
        uint32 x = ~uint32(0);
...
interface IConfig {
    ...
    function getBlindUrl(string memory groupName)
        external
        view
        returns (string[] memory);

    function getNftUrl(string memory groupName)
        external
        view
        returns (string[] memory);
...
interface INFTElement {
    ...
    function batchMintNFT(address to,  ElementScore[] memory elementScore) external;
}
...
interface INFTComposite {
    ...
    function batchMintNFT(
        ...
        PropertyScore[] memory propertyScore
    ) external;

    function getCurrentMaxId() external view returns (uint256);
}
