...
contract NFTManager is ERC721, Ownable {

    mapping(address => mapping (uint256 => address)) public nftOwner;
    mapping (address => bool) internal authorizations;

  	constructor() 
      ERC721("NFT Manager", "NFT Manager") {
    ...
    function depositNFT(address _contractAddress, uint256[] memory _tokenId) external {
        ...
        for(uint i=0; i<_tokenId.length; i++) {
            nftOwner[_contractAddress][_tokenId[i]] = msg.sender;
			ERC721(_contractAddress).safeTransferFrom(msg.sender, address(this), _tokenId[i]);
		}
    ...
    function withdrawNFT(address _contractAddress, uint256[] memory _tokenId) external {
        ...
        for(uint i=0; i<_tokenId.length; i++) {
            require(msg.sender == nftOwner[_contractAddress][_tokenId[i]], "Not NFT's owner");
			ERC721(_contractAddress).safeTransferFrom(address(this), msg.sender, _tokenId[i]);
		}

		emit WithdrawNFT(msg.sender, _contractAddress, _tokenId);
    }

    function rescueNFT(address _contractAddress, uint256 _tokenId, address _to) external {
        require(isAuthorized(msg.sender), "SoldierNFT : UNAUTHORIZED");
        require(_contractAddress != address(0), "Contract address zero");
        require(_tokenId > 0, "Token ID must be larger than 0");
        require(_to != address(0), "User address zero");

		ERC721(_contractAddress).safeTransferFrom(address(this), _to, _tokenId);

		emit SaveNFT(_contractAddress, _tokenId, _to);
    }

    function authorize(address _adr, bool _authorize) external {
        require(isAuthorized(msg.sender), "BankNFT : UNAUTHORIZED");
        authorizations[_adr] = _authorize;

        emit Authorize(_adr, _authorize);
    }
	...
	function rescueStuckBalance() external payable onlyOwner {
    ...
    event SaveNFT(address contractAddress, uint256 tokenId, address to);
