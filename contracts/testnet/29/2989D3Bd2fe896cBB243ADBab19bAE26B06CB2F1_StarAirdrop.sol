...
contract StarAirdrop is Modifier {

    using SafeMath for uint256;

    uint256 private receiveLimit;

    mapping(address => bool) private isReceived;

    ERC20 private starToken;

    constructor() {
        receiveLimit = 1000000000000000000;
        starToken = ERC20(0x7Ef75368241820EEFAf802B99B5252C85EbA2328);
    }

    function setContractToken(address _starToken) public onlyOwner {
        starToken = ERC20(_starToken);
    }

    function setReceiveLimit(uint256 _receiveLimit) public onlyOwner {
        receiveLimit = _receiveLimit;
    }

    function receiveToken() public isRunning nonReentrant returns (bool) {
        if(isReceived[msg.sender]) {
            _status = _NOT_ENTERED;
            revert("Star: Received, can not be repeated");
        }

        isReceived[msg.sender] = true;

        starToken.transfer(msg.sender, receiveLimit);

        return true;
    }

    function getReceiveStatus(address _address) public view returns (bool) {
        return isReceived[_address];
    }
