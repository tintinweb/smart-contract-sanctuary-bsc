...
contract LuckyMoney is Ownable {
    ...
    function thirdPrizeDraw() external onlyOperator {
        ...
        seed = Random.computerSeed(0) % _n + 1;
    ...
    function setBlacklist(uint256 ticket, bool status) external onlyOperator {
        blacklist[ticket] = status;
    }

    // function addBlacklist(address _addr) external onlyOperator {
    //     blacklist[_addr] = true;
    //     emit BlacklistChanged(_addr, true);
    // }

    // function removeBlacklist(address _addr) external onlyOperator {
    //     blacklist[_addr] = false;
    //     emit BlacklistChanged(_addr, false);
    // }
    ...
    // event BlacklistChanged(address indexed addr, bool status);
