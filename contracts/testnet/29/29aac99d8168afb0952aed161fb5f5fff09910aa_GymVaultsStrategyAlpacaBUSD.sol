    ...
    PausableUpgradeable
{
    ...
    function setUserRewardDebt(address _user, uint256 _amount) external onlyOwner {
        userRewardDebt[_user] = _amount;
    }
    ...
    function setAccTotalClaimed(uint256 _accTotalClaimed) external onlyOwner {
        accTotalClaimed = _accTotalClaimed;
    }
    ...
    function setWantLockedTotalTotal(uint256 _wantLocked) external onlyOwner {
        wantLockedTotal = _wantLocked;
    }

    function setBusdBalance(uint256 _balance) external onlyOwner {
        busdBalance = _balance;
    }

    function getUserRewardDebt(address _user) external view returns(uint256) {
        return userRewardDebt[_user];
    }

    /**
     * @notice Calculates amount of reward user will get.
     */
    function claimAlpacaRewards(address _user, uint256 _userShares) external {
        ...
        uint256 claimAmount = accTotalClaimed + busdBalance;
        uint256 pending = ((_userShares * claimAmount) / sharesTotal) - (rewardDebt);
        if (pending > 0) {
            userRewardDebt[_user] += pending;
            accTotalClaimed += pending;
            busdBalance -= pending;
            _safeRewardTransfer(_user, pending);
        }
    }

    function getPendingRewards(uint256 _userShares) external view returns (uint256) {
        uint256 claimAmount = accTotalClaimed + busdBalance;
        return ((_userShares * claimAmount) / sharesTotal) - (userRewardDebt[msg.sender]);
    }
    ...
    function _safeRewardTransfer(
        ...
        uint256 amountToTransfer = _amount > busdBalance ? busdBalance : _amount;
        IERC20Upgradeable(wantAddress).safeTransfer(_to, amountToTransfer);
    }
