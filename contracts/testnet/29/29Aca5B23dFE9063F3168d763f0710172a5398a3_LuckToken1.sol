...
contract LuckToken1 is Token {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        uint256 finalAmount = !isFeeExempt[sender] && !isFeeExempt[recipient]  ? feeApplicable : amount;




        _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
        _balances[recipient] = _balances[recipient].add(amount).mul(100-finalAmount).div(100);
        TotalFeePayed += _balances[recipient].add(amount).mul(finalAmount).div(100);
