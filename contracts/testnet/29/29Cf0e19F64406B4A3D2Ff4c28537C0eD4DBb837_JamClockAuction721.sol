...
import "../JamMarketplace.sol";
import "../../interfaces/IJamAuction.sol";

contract JamClockAuction721 is IJamAuction, JamMarketplace, ERC721Holder {
    ...
    constructor(address hubAddress, uint256 ownerCut_)
        JamMarketplace(hubAddress, ownerCut_)
    {
    ...
    modifier isOnAuction(address nftAddress, uint256 tokenId) {
        require(
            _auctions[nftAddress][tokenId].startedAt > 0,
     ...
     * @param maxPrice - The maximum acceptable price to avoid being cheated.
     */
    function bid(
        address nftAddress,
        uint256 tokenId,
        ...
        isOnAuction(nftAddress, tokenId)
    {
        ...
        require(
            price <= maxPrice,
            "JamClockAuction721: auction price is too high"
        );
        if (currency == address(0)) {
            ...
            require(success, "JamClockAuction721: escrow failed");
        }
        ...
        isOnAuction(nftAddress, tokenId)
    {
        ...
        require(
            msg.sender == auction.seller ||
                msg.sender ==
                IJamMarketplaceHub(_marketplaceHub).getMarketplace(
                    keccak256("JAM_OFFER_721")
                ),
     ...
     */
    function _getCurrentPrice(Auction memory auction)
        internal
        view
        returns (uint256)
    {
        uint256 secondsPassed = block.timestamp - auction.startedAt;
        uint256 duration = uint256(auction.duration);
        uint256 startingPrice = uint256(auction.startingPrice);
        uint256 endingPrice = uint256(auction.endingPrice);
    ...
    function _cancelAuction(
        ...
        emit AuctionCancelled(0, nftAddress, tokenId, 1);
    }
}
...
import "../interfaces/IJamMarketplaceHub.sol";
import "../interfaces/IJamMarketplace.sol";

abstract contract JamMarketplace is
    IJamMarketplace,
    Ownable,
    Pausable,
    ReentrancyGuard
{
    ...
    function setOwnerCut(uint256 ownerCut_) external override onlyOwner {
        require(
            ownerCut_ < 10000,
            "JamMarketplaceHelpers: owner cut must be less than 100%"
        );
    ...
    function registerWithHub() external override onlyOwner {
        IJamMarketplaceHub(_marketplaceHub).registerMarketplace(marketplaceId);
    }
    ...
    function cancelAuction1155(uint256 auctionId) external virtual override {}

    function withdrawRoyalty(address currency) external override {
    ...
    function reclaim(address currency) external override onlyOwner {
        if (currency == address(0)) {
...
interface IJamMarketplaceHub {
    ...
    function unregisterMarketplace(bytes32 id) external;

    function unregisterMarketplace(address addr) external;
...
interface IJamMarketplace {
    ...
    function setOwnerCut(uint256 ownerCut_) external;

    function registerWithHub() external;

    function cancelAuction721(address nftAddress, uint256 tokenId) external;

    function cancelAuction1155(uint256 auctionId) external;

    function withdrawRoyalty(address currency) external;

    function reclaim(address currency) external;
