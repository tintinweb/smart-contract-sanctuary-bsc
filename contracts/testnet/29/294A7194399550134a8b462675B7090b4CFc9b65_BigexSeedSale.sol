...
contract BigexSeedSale is Ownable, Pausable {
	...
	mapping(address => uint256[]) userLockDetail;
	mapping(address => uint256) userTotalPayment;
	...
	function buyToken(uint256 _paymentAmount) public whenNotPaused {
		...
		uint256 totalToken = _paymentAmount.div(tokenPriceRate) * 10 ** 18;
		...
		paymentToken.transferFrom(msg.sender, address(this), _paymentAmount);
