...
contract SociosDT is ERC1155Supply,  Ownable {
    ...
    //Purchase price
    uint256 public mintPrice = 9600000000000000;
    // Carbon Credit Fee
    ...
    mapping(uint256 => uint256) public lastSnap;
    ...
    constructor(string memory _name, string memory _symbol) ERC1155("https://nft.sociosdelatierra.com/tree/") {
        sdtName =_name;
        sdtSymbol = _symbol;
        saleOpen = false;
        redeemAllowed = false;
    ...
    function addTree(
        ...
        lastSnap[sdtCounter.current()] = block.timestamp;
    ...
    function editTree(
        ...
        require(exists(_sdtIndex), "EDIT: Token does not exist");
        sdtTrees[_sdtIndex].farmName = _farmName;
        sdtTrees[_sdtIndex].lng = _lng;
        sdtTrees[_sdtIndex].lat = _lat;
        sdtTrees[_sdtIndex].limit = _limit;
        sdtTrees[_sdtIndex].sold = _sold;
        sdtTrees[_sdtIndex].carbonDioxideOffset = _carbonDioxideOffset;
    }

    function mint(uint256 _sdtIndex, address _to, uint256 _numOfTrees) external onlyOwner{
        require(exists(_sdtIndex), "MINT: Token does not exist");
        require (_numOfTrees > 0, "Num of trees invalid.");
        require (!sdtTrees[_sdtIndex].locked, "Tree Locked");
        require (sdtTrees[_sdtIndex].sold + _numOfTrees <= sdtTrees[_sdtIndex].limit, "Mint exceeds the allowed limit");
        _mint(_to, _sdtIndex, _numOfTrees, "");
        sdtTrees[_sdtIndex].sold = sdtTrees[_sdtIndex].sold + _numOfTrees;
    }




    function mintTrees(uint256 _sdtIndex, address[] calldata _to, uint256[] calldata _numOfTrees) external onlyOwner {
        require(exists(_sdtIndex), "MINTT: Token does not exist");
        require (_to.length == _numOfTrees.length, "Lengths have to match.");
        require (!sdtTrees[_sdtIndex].locked, "Tree Locked");
        for(uint256 i = 0; i < _to.length; i++) {
            require (sdtTrees[_sdtIndex].sold + _numOfTrees[i] <= sdtTrees[_sdtIndex].limit, "Mint exceeds the allowed limit");
            _mint(_to[i], _sdtIndex, _numOfTrees[i], "");
            sdtTrees[_sdtIndex].sold=sdtTrees[_sdtIndex].sold + _numOfTrees[i];
        }
    }
    ...
    function mintBatch(address _to, uint256[] calldata _numOfTrees, uint256[] memory _sdtIndexs) external onlyOwner
    {
        ...
        for (uint i=0; i< _sdtIndexs.length; i++) {
            ...
            require(exists(sdtIndex), "MINTB: Token does not exist");
            require (!sdtTrees[sdtIndex].locked, "Tree Locked");
            require (sdtTrees[sdtIndex].sold + _numOfTrees[i] <= sdtTrees[sdtIndex].limit, "Mint exceeds the allowed limit");

            sdtTrees[sdtIndex].sold = sdtTrees[sdtIndex].sold + _numOfTrees[i];
        }
    ...
    function uri(uint256 _sdtIdx) public view override returns (string memory) {
        require(exists(_sdtIdx), "URI: Token does not exist");
    ...
    function allBalanceOf(address _account) external view returns(uint256){
        uint256 counter = 0;
        uint256 length = sdtCounter.current();
        for (uint i=0; i< length; i++) {
            counter+=balanceOf(_account, i);
        }
        return counter;
    }
    ...
    function setLockedTree(uint256 _sdtIdx, bool _locked) external onlyOwner {
        require(exists(_sdtIdx), "URI: Token does not exist");
        sdtTrees[_sdtIdx].locked = _locked;
    }
    ...
    function purchase(uint256 _numOfTrees, uint256 _sdtIdx) external payable noReentrant {
        require(saleOpen == true, "Purchase: closed");
        require(exists(_sdtIdx), "PUR: Token does not exist");
        require (!sdtTrees[_sdtIdx].locked, "Tree Locked");
        require (sdtTrees[_sdtIdx].sold + _numOfTrees <= sdtTrees[_sdtIdx].limit, "Mint exceeds the allowed limit");
    ...
    function generateCarbonCredit(uint256 _tokenId) external payable noReentrant{
        ...
        require(msg.value >= carbonCreditFee, "SDT: Provided value is not enough");
        require(balance > 0, "Account without tree");

        // Calculates total carbon offset until now
        uint256 total = calculateTotal(_tokenId, balance);
        credits[msg.sender] = credits[msg.sender].add(total);

        lastSnap[_tokenId] = block.timestamp;
        emit Generated(msg.sender, _tokenId, total);
    }


    function calculateTotal(uint256 _tokenId, uint256 _balance) public view returns (uint256){
        uint256 end = block.timestamp;
        uint256 start = lastSnap[_tokenId];
        uint256 carbonDioxideOffset = _balance.mul(sdtTrees[_tokenId].carbonDioxideOffset);
        return (end.sub(start)).mul(carbonDioxideOffset).div(31536000);
    }

    function redeemCarbonCredit(uint256 amount) external noReentrant{
        require(redeemAllowed, "SDT: Redeem not allowed");
        uint256 balance = credits[msg.sender] ;
        require(balance >= amount, "SDT: Invalid amount provided");
