...
contract PettyGacha is ERC721, Ownable {
    ...
    constructor(address tokenAddress_) ERC721("Petty", "PET") {
        paymentToken = IERC20(tokenAddress_);
    ...
    mapping(uint256 => Breed) public _breed;
        ...
        returns (uint256)
    {
        ...
        paymentToken.transferFrom(_msgSender(), address(this), price_);
    ...
    function breedPetties(uint256 tokenId1_, uint256 tokenId2_) public {
        ...
        require(
            _tokenIdToPetty[tokenId1_].rank == _tokenIdToPetty[tokenId2_].rank,
        ...
        uint32 _breedTime = uint32(block.timestamp + _newRank * 1 minutes );
        _breedIdCount.increment();
        uint256 _newBreedId = _breedIdCount.current();
        _breed[_newBreedId] = Breed(_msgSender(), _newTokenId, _newRank, _breedTime);
    ...
    function claimsBreedPetty(uint256 breedId_) public {
        require(_breed[breedId_].owner == _msgSender(), "PettyGacha: sender is not owner of breed");
        require(_breed[breedId_].duration >= uint32(block.timestamp), "PettyGacha: Your petty still duration breed");
        uint256 _tokenId = _breed[breedId_].tokenId;
        delete _breed[breedId_];
        _mint(_msgSender(), _tokenId);
        _tokenIdToPetty[_tokenId] = Petty(_breed[breedId_].rank, 0);
    }
