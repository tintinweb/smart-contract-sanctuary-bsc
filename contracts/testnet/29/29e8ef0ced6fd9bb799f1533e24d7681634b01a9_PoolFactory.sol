...
contract PoolFactory is IPoolFactory, Initializable, ProxyFactory {
    event PoolCreated(address newPoolLogic, address newPoolManagerLogic, address manager,address factory);
    event AssetHandlerChanged(address from, address newAssetHandler);
    event ProtocolFeeChanged(uint256 from, uint256 to);
    event ProtocolPerFormanceFeeChanged(uint256 from, uint256 to);
    ...
    function initialize(
        ...
        _setProtocolFee(2); // 0.02%
        _setProtocolPerformanceFee(50); // 0.5%
    }
    ...
    function createPool(
        ...
        address newPoolManagerLogic = deploy(getOwner(), poolManagerLogicData, 1);
        ...
        address newPoolLogic = deploy(getOwner(), poolLogicData, 0);

        IPoolManagerLogic(newPoolManagerLogic).setPoolLogic(newPoolLogic);

        deployedPools.push(newPoolLogic);

        emit PoolCreated(newPoolLogic, newPoolManagerLogic, msg.sender, address(this));
    }
    ...
    function _setProtocolFee(uint256 _protocolFee) internal {
        require(_protocolFee < 10000, "over 100%"); // < 100%
        uint256 olfFee = protocolFee;
        protocolFee = _protocolFee;

        emit ProtocolFeeChanged(olfFee, protocolFee);
    }
    ...
    function setProtocolPerformanceFee(uint256 _performanceFee) public onlyOwner {
        _setProtocolPerformanceFee(_performanceFee);
    }

    function _setProtocolPerformanceFee(uint256 _performanceFee) internal {
        require(_performanceFee < 10000, "over 100%"); // < 100%
        uint256 olfFee = protocolPerformanceFee;
        protocolPerformanceFee = _performanceFee;

        emit ProtocolPerFormanceFeeChanged(olfFee, protocolPerformanceFee);
    }
...
contract PoolLogic is ERC20ExtendedUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function deposit(address _tokenIn, uint256 _amountIn) public {
        ...
        _mintFee(uint256(IFeeManage.FeeType.TYPE_FEE_ENTRY), protocolFee, numberOfUnit);
    ...
    function withdraw(bool _isStablecoin, uint256 _unitOut) public nonReentrant {
        ...
        if (numberOfUnit == _unitOut) {
         ...
         uint256 protocolFee = IPoolFactory(factory).getProtocolFee();
         uint256 feeType = uint256(IFeeManage.FeeType.TYPE_FEE_EXIT_SPECIFIC);
         if (!_isStablecoin) {
             feeType = uint256(IFeeManage.FeeType.TYPE_FEE_EXIT_SHARE_IN_KIND);
         }
        _mintFee(feeType, protocolFee, _unitOut);

        _withdraw(_isStablecoin, denominationAsset, proportion);
    }
    ...
    function calculatePerformanceFee() public onlyOwner nonReentrant {
    ...
    /// @dev manager redeem fee
    function redeemFee(bool _isStablecoin) public onlyManager nonReentrant {
        uint256 numberOfUnit = balanceOf(poolManagerLogic);
        require(numberOfUnit > 0, "no fee");

        _redeemFee(_isStablecoin, numberOfUnit);
        emit RedeemFee(manager, numberOfUnit);
    }

    /// @dev opt. withdraw stable coin
    /// @dev opt. withdraw any asset
    function _redeemFee(bool _isStablecoin, uint256 _unitOut) internal {
        require(_unitOut > 0, "require number of unit");
        ...
        _burn(poolManagerLogic, _unitOut);
        _withdraw(_isStablecoin, denominationAsset, proportion);
    }
            ...
            if (withdrawnAmount > 0) {
                if (!_isStablecoin) {
                    ...
                    if (_denominationAsset == asset) {
                        ...
                        // swap to stable coin
                        _swapForWithdrawal(asset, _denominationAsset, withdrawnAmount, msg.sender);
                    }
                }
...
contract ProxyFactory is OwnableUpgradeable, HasLogic {
    ...
    function deploy(
        ...
        bytes memory creationByteCode = getCreationBytecode(getLogic(_proxyType), _admin, _data);
