...
contract CollectionMinter {

    Archives public archive;

    constructor(Archives arch){
        archive = arch;
    }

    uint public lastInd = 1;
    struct record {
        uint price;
        IERC20 token;
    ...
    mapping(uint => record) factoryRecords;

    struct walletsOnchaindata{
        uint royaltySize;
        mapping(uint => Archives.royaltyRecord) royalties;
        uint dataSize;
        mapping(uint => string) onchainData;
        mapping(uint => bool) minted;
    }
    mapping(uint => walletsOnchaindata) factoryRecordsArr;

    function getMintRecord(uint internalId) public view returns (record memory){
        return factoryRecords[internalId];
    }

    function getMintRoyaltySize(uint internalId) public view returns (uint){
        return factoryRecordsArr[internalId].royaltySize;
    }
    function getMintDataSize(uint internalId) public view returns (uint){
        return factoryRecordsArr[internalId].dataSize;
    }

    function getMintRoyalties(uint internalId) public view returns (Archives.royaltyRecord[] memory){
        walletsOnchaindata storage ptr = factoryRecordsArr[internalId];
        Archives.royaltyRecord[] memory res = new Archives.royaltyRecord[](ptr.royaltySize);
        for(uint i=0; i<ptr.royaltySize; i++){
            res[i].adr.chainId = ptr.royalties[i].adr.chainId;
            res[i].adr.adr = ptr.royalties[i].adr.adr;
            res[i].q = ptr.royalties[i].q;
        }
        return res;
    }
    function getMintData(uint internalId) public view returns (string[] memory){
        walletsOnchaindata storage ptr = factoryRecordsArr[internalId];

        string[] memory res = new string[](ptr.dataSize);
        for(uint i=0; i<ptr.dataSize; i++){
            res[i] = ptr.onchainData[i];
        }

        return res;
    }

    function getIfMinted(uint internalId, uint collectionId) public view returns (bool){
        return factoryRecordsArr[internalId].minted[collectionId];
    }



    function createMintRecord(
        Archives.royaltyRecord[] memory royaltyWallets,
        string[] memory onchainData,
        uint price,
        IERC20 token,
        uint timeStart,
        uint timeReveal,
        uint maxInd) public returns (bool){

        require(archive.isAllowed(msg.sender), "You are not allowed to mint");


        record storage ptr = factoryRecords[lastInd];
        ptr.price = price;
        ptr.token = token;

        ptr.timeStart = timeStart;
        ptr.timeReveal = timeReveal;

        ptr.maxInd = maxInd;

        ptr.creator = msg.sender;

        walletsOnchaindata storage ptrB = factoryRecordsArr[lastInd];
        ptrB.royaltySize = royaltyWallets.length;
        uint qS = 0;
        for(uint i=0; i<ptrB.royaltySize; i++){
            ptrB.royalties[i].adr.chainId = royaltyWallets[i].adr.chainId;
            ptrB.royalties[i].adr.adr = royaltyWallets[i].adr.adr;
            ptrB.royalties[i].q = royaltyWallets[i].q;
            qS += royaltyWallets[i].q;
        }

        require(qS <= archive.maxRoyalty(), "decrease the royalty %");

        ptrB.dataSize = onchainData.length;
        for(uint i=0; i<ptrB.dataSize; i++){
            ptrB.onchainData[i] = onchainData[i];
        }
    ...
    function sellFromFactory(uint internalId, address api) public returns (bool){
        record storage ptr = factoryRecords[internalId];
        walletsOnchaindata storage ptrB = factoryRecordsArr[internalId];

        require(ptr.timeStart <= block.timestamp, "Minting is not started yet");

        uint curInd = ptr.maxInd;
        for(uint i=0; i<ptr.maxInd; i++){
            if(!ptrB.minted[i]){
                curInd = i;
                ptrB.minted[i] = true;
                ptr.alreadyMinted++;
        ...
        require(curInd != ptr.maxInd, "All NFTs from the collection are either minted or trying to be minted now");

        // if marketplace balance of the buyer is not high enough
        // the nft will be minted and stuck forever in archive,
        // however the info about mint will be reverted here
        // so the more lucky one will get a mint.

        uint archiveId = archive.addRecordCollection(
            getMintRoyalties(internalId),
            ptr.creator,
            curInd,
            ptr.maxInd,
            internalId,
            1,
            getMintData(internalId)
        );


        // nft is transfered to creator
        // money are transfered to creator+royalties+fees
        // nft is released to buyer
        // if not enough money - the nft stays in archive forever

        Archives.Action[] memory toArch = new Archives.Action[](3);

        // change owner to creator
        toArch[1].tp = 1;
    	toArch[1].a = block.chainid;
    	toArch[1].b = archiveId;
    	toArch[1].z = ptr.creator;

        // pay with royalties, fee, api-fee, the rest to current owner
        toArch[0].tp = ptr.price;
        toArch[0].a = block.chainid;
        toArch[0].b = archiveId;
        toArch[0].x = msg.sender;
        toArch[0].z = api;
        toArch[0].token = ptr.token;

        // change owner to buyer
        toArch[1].tp = 1;
    	toArch[1].a = block.chainid;
    	toArch[1].b = archiveId;
    	toArch[1].z = msg.sender;

        archive.stackedActions(toArch);
        return true;
...
contract Archives is OwnedAndParsed {
    ...
    function addRecordCollection(
        ...
        require(ActionPerformers[msg.sender] != 0, "Only for marketplace contracts use");

        uint chainId = block.chainid;
        uint internalId = lastNftId;
    	...
    	ptr.currentOwner = address(this);
    	ptr.status = ActionPerformers[msg.sender];
