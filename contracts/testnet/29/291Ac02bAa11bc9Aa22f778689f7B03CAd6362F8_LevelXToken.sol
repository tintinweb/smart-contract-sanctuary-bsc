...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	function initialize(address _owner, string memory _name, string memory _symbol, address _router) public initializer
	{
		...
		excludeFromTradeFee[address(this)] = true;
	...
	function _postUpdateAccount(address _account, int256 _levelBump) internal
	{
		...
		uint256 _newActiveBalance = _balance < minimumBalanceForRewards ? 0 : _balance;
	...
	function _transfer(address _from, address _to, uint256 _amount) internal override
	{
...

		{
			...
			if (_balance >= minimumFeeBalanceToSwap) {
				...
				_injectWithBNB(_balance - _swapAmount, _bnbAmount * (20e16 / 2) / 100e16);
				return;
			}
		}

		{
			...
			if (_bnbAmount > minimumRewardBalanceToSwap) {
				for (uint256 _i = 0; _i < rewardIndex.length; _i++) {
					address _rewardToken = rewardIndex[_i];
					_swapBNBToReward(_rewardToken, _bnbAmount / rewardIndex.length);
				}
				return;
			}
		}
	...
	{
		return IUniswapV2Router(router).swapExactTokensForETH(_amount, 0, pathWBNB, address(this), block.timestamp)[pathWBNB.length - 1];
	}

	function _injectWithBNB(uint256 _amount, uint256 _bnbAmount) internal
	{
		IUniswapV2Router(router).addLiquidityETH{value: _bnbAmount}(address(this), _amount, 0, 0, address(this), block.timestamp);
	}
