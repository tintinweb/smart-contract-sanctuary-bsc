    ...
    AccessControlUpgradeable
{
    ...
    function getPendingReward(uint256 _poolId, address _account, uint256 _stakedId) external view returns(uint256 totalReward) {
        ...
        return _getPendingReward(_poolId, _account, _stakedId);
    }
    ...
    function _getPendingReward(uint256 _poolId, address _account, uint256 _stakedId) private view returns(uint256 totalReward) {
        ...
        uint256 totalReward = 0;
        uint256 pendingReward = ((userStaking.balance * userStaking.APR * (block.timestamp - userStaking.stakeTime)) / ONE_YEAR)  / 1e20;
