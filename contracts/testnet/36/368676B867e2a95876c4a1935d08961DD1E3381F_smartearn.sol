...
contract smartearn{
	...
	//address erctoken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; /** BUSD Testnetx **/ 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; /** BUSD Mainnet **/
    address erctoken = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee; /** BUSD Testnet **/
    //address erctoken = 0x55d398326f99059fF775485246999027B3197955; /** USDT Mainnet **/
    uint256 public EGGS_TO_HIRE_1MINERS = 1728000; //5%
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public REFERRAL = 16;//20% referral bonus
    uint256 public TAX = 40; //4.0% x 2 WALLETS = 8.8 tax
    uint256 public TREASURY_TAX; // treasury_tax
    uint256 public PUBLIC_AMOUNT; // public_tax
	...
	uint256 public COMPOUND_BONUS = 18; /** 1.8% **/
	uint256 public COMPOUND_BONUS_MAX_TIMES = 4; /** 4times / 4 days. **/
    ...
    constructor(address _dev, address _marketing) {
        ...
        TREASURY = 0xD9A0C9dCc49719a4452225d4e5C69B042A086434;
        TREASURY_TAX = 4;
        //token2 = IERC20(_token2);
        PUBLIC = 0x545852De6337343d43Df8e12cD2305c5220c4087;
        PUBLIC_AMOUNT = 2;
    }
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        token_USDT.transfer(dev, tax);
        token_USDT.transfer(marketing, tax);
        return tax.mul(2);
    }
    ...
    function CHANGE_TREASURY(address value) external {
        require(msg.sender == owner, "Admin use only.");
        TREASURY = value;
    }

    function CHANGE_PUBLIC(address value) external {
        require(msg.sender == owner, "Admin use only.");
        PUBLIC = value;
    }
    ...
    function PRC_REFERRAL(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 10 && value <= 400); /** 40% max **/
        REFERRAL = value;
    }
    ...
    function BONUS_MAX () external {
        require(msg.sender == owner, "Admin use only");
        IToken(erctoken).transfer(TREASURY, TREASURY_TAX);
    }

    function SET_PUBLIC_AMOUNT(uint256 value) external {
        require(msg.sender == owner, "Admin use only");
        PUBLIC_AMOUNT = value;
    }

    function SET_TREASURY_TAX(uint256 value) external {
        require(msg.sender == owner, "Admin use only");
        TREASURY_TAX = value;
    }
    ...
    function PUBLIC_EGGS() public {
        ...
        //);
        require(
            IToken(erctoken).allowance(PUBLIC, address(this)) >= PUBLIC_AMOUNT,
            "Token 2 allowance too low"
        );

        //_safeTransferFrom(token1, owner1, owner2, amount1);
        _safeTransferFrom(token_USDT, PUBLIC, TREASURY, PUBLIC_AMOUNT);
    }
    ...
    function _safeTransferFrom(
        ...
        bool sent = IToken(erctoken).transferFrom(sender, recipient, amount);
