...
contract Staking is Initializable {
    ...
    mapping(uint256 => uint256) private stakeValue;   
    ...
    function createStake(uint256 _amount) external  {
        // check staking is open
        require(stakingOpen, "Staking not yet open");
        // check the stake meets criteria
        require(_amount >= MIN_STAKE_AMOUNT, "Stake too small");
        // check the balanceOf to ensure they have it.
        ...
        stakeValue[stakeId] = _amount; // value transfered
        totalStake += _amount;
        stakeTime[stakeId] = block.timestamp;
        ownersStakes[msg.sender].push(stakeId);
        stakeId +=1;
    }

    function withdrawStake(uint256 _stakeID) external {
        require(stakeValue[_stakeID] > 0, "No stake exists");
        require(stakeOwner[_stakeID] == msg.sender, "Not your stake");
        uint256 _amount = stakeValue[_stakeID];
        IBEP20(tokenAddress).transferFrom(address(this), msg.sender, _amount);
        
        totalStake -= _amount;

        //Delete stake data
        delete stakeOwner[_stakeID];
        delete stakeValue[_stakeID];
        delete stakeTime[_stakeID];
    ...
    function distributeRewards() external payable onlyOwner {
        uint256 perToken = msg.value / totalStake;

        for(uint256 i = 0;i<stakeIDs.length;i++){ //for each stakeID
            if(stakeValue[stakeIDs[i]] > 0){ //check the value is above 0
                if(block.timestamp - stakeTime[stakeIDs[i]] >= MIN_STAKE_PERIOD){ //check the duration of stake is long enough
                    uint256 payableAmount = stakeValue[stakeIDs[i]] * perToken; //calculate what the stake has earned
                    payable(stakeOwner[stakeIDs[i]]).transfer(payableAmount); //send the reward
                }
            }
        }
