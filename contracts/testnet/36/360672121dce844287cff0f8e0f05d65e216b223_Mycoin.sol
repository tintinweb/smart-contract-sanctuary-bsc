...
contract Mycoin is ERC20, Ownable  {
    ...
    uint256 public constant _inviterFee = 3;
    uint256 public constant _burnFee = 3;
    uint256 public constant _marketFee = 5;
    ...
    uint256 public numTokensSellToAddToLiquidity =  66666 * 1e18 / 20000;
    uint256 public constant _maxSellAmount = 3 * 1e18;

    uint256 public hourtime = 86400;
    ...
    constructor(address _marketAddress) ERC20("BSZW", "BSZW") {
    ...
    function _getFeeValues(uint256 _amount, bool isSell) private view returns (uint256,uint256,uint256,uint256,uint256) {
        ...
        if(_balances[burnAddress] >= 66666 * 1e18 - 1999 * 1e18){
            burnFee = 0;
        }
        if(isSell){
            inviterFee = _amount * _inviterFee / 100;
        }

        uint256 transferAmount = _amount - liquidityFee - burnFee - marketFee - inviterFee;
        return (transferAmount, liquidityFee, burnFee, inviterFee, marketFee);
    }
    ...
    function _transfer(
        ...
        if (recipient == pancakeswapV2Pair && _balances[pancakeswapV2Pair] > 0) {
            if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]){
                ...
                require(getHourNum(sender) + amount <= _maxSellAmount, 'Transfer amount exceeds the maxTxAmount3.');

                User storage user = users[sender];
                user.deposits.push(Deposit(amount,block.timestamp));

                if(_balances[sender] <= amount && sender != pancakeswapV2Pair){
    ...
    /*update time*/
	function updateHourTime(uint256 _hourtime) public onlyOwner returns(uint256) {
        hourtime = _hourtime;
		return hourtime;
	}
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint256, uint256) {
		...
		return (user.deposits[index].sellnum, user.deposits[index].selltime);
	}


    // hourtime TRX number
    function getHourNum(address userAddress) public view returns(uint256) {
	    User storage user = users[userAddress];

        uint256 amount;

		for (uint256 i = 0; i < user.deposits.length; i++) {
            if(user.deposits[i].selltime >= block.timestamp-hourtime){
			    amount += user.deposits[i].sellnum;
            }
		}
        return amount;
	}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {        
        ...
        if(takeFee){
            (uint256 tTransferAmount, uint256 liquidityFee, uint256 burnFee, uint256 inviterFee, uint256 marketFee) = _getFeeValues(
                amount,
                _balances[pancakeswapV2Pair] > 0
            );
