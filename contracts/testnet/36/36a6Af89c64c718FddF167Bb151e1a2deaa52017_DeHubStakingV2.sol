...
import "./abstracts/DeHubUpgradeable.sol";
import "./interface/IDeHubRewardsV5.sol";
import "./interface/IDeHubStakingController.sol";

/**
 * @dev V2 upgrade template. Use this if update is needed in the future.
 */
contract DeHubStakingV2 is DeHubUpgradeable, IDeHubStakingV2 {
  using SafeMathUpgradeable for uint256;
  using SafeERC20Upgradeable for IERC20Upgradeable;

  IDeHubStakingController public controller;
  ...
  // amount.
  IDeHubRewardsV5 public bnbRewardContract;
  ...
  function __DeHubStaking_init(
    IERC20Upgradeable _dehubContractAddress,
    IDeHubRewardsV5 _bnbRewardContract,
    address _feeWallet,
  ...
  function setController(IDeHubStakingController _controller)
    external
    onlyOwner
  {
   ...
   */
  function setBnbRewardContract(IDeHubRewardsV5 _bnbRewardContract)
    external
    onlyOwner
  {
    bnbRewardContract = _bnbRewardContract;
   ...
   */
  function setFeeWallet(address _feeWallet) external onlyController {
    feeWallet = _feeWallet;
  }
    ...
    returns (uint256, uint256)
  {
    ...
    dehubToken.safeTransferFrom(_sender, address(this), _amount);

    return (claimableReflection, claimableHarvest);
  }
    ...
    )
  {
    ...
    dehubToken.safeTransfer(_sender, _amount.sub(fee));

    return (fee, claimableReflection, claimableHarvest);
  }
    ...
    )
  {
    ...
    if (user.amount > 0) {
      // If there is still someting to withdraw, then it's the last time.
      pool.totalStakers--;
      user.amount = 0;
    }
    user.harvested = true;

    _safeDehubTransfer(_sender, total);

    return (total, claimableReflection, claimableHarvest);
  }
  ...
  function fund(
    ...
    require(
      dehubToken.balanceOf(_sender) >= _amount,
    ...
    dehubToken.safeTransferFrom(_sender, address(this), totalAmount);
  }
   ...
   */
  function claimBNBRewards(address _sender)
    external
    whenNotPaused
    onlyController
    returns (uint256)
  {
    ...
    uint256 bnbShare = IDeHubRewardsV5(bnbRewardContract).sendRewardToStaker(
      _sender,
      user.amount,
      pool.totalStaked
    );
  ...
  /**
   * @notice Returns staked amount per certain user
   */
  ...
  /**
   * @notice Returns if current pool is waiting to open
   */
  function isPending() external view returns (bool) {
    return block.number < pool.openBlock;
  }

  /**
   * @notice Returns if current pool is already opened and not closed
   */
  function isActive() external view returns (bool) {
    return block.number >= pool.openBlock && block.number < pool.closeBlock;
  }
  ...
  function _safeDehubTransfer(address _to, uint256 _amount) internal {
    ...
    require(_amount <= dehubBal, "Safe transfer failed. Insufficient balance.");
...
interface IDeHubRewardsV5 {
  ...
  function sendRewardToStaker(
    address _stakerAddr,
    uint256 _stakeAmount,
    uint256 _totalStakePoolAmount
...
interface IDeHubStakingController {
  ...
  function claimBNBRewards() external;

  function sendBNBRewards(address _sender) external;

  function setActiveQuarter(uint256 _quarter) external;

  function addStakingContract(uint256 _quarter, IDeHubStakingV2 _staking)
    external;

  function getActiveStaking() external view returns (IDeHubStakingV2);
}
...
interface IDeHubStakingV2 {
  ...
  function claimBNBRewards(address _sender) external returns (uint256);
