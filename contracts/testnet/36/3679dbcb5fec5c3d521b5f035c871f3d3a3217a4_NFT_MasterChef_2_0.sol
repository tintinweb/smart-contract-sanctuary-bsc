...
interface IStakeWar{
    function pushEthnicityNFTNFTStake2(uint256 tokenId) external;
    function removeEthnicityNFTNFTStake2(uint256 tokenId) external;
}
...
contract NFT_MasterChef_2_0 is Initializable, OwnableUpgradeable, Member {
    ...
    function changeStartBlock(uint256 _startBlock) external {
        require(msg.sender == manager.members("owner"), "onlyOwner");
        startBlock = _startBlock;
    }
    ...
    function pendingSushi(address _user) public view returns (uint256) {
        ...
        if ((totoalPower < minimalPower && accSushiPerShare == 0) || lpSupply == 0 || block.number < startBlock) {
            return 0;
        }
    ...
    function updatePool(uint8 _pid) public {
        ...
        uint256 lpSupply = pool.lpSupply;
        if ((totoalPower < minimalPower && pool.accSushiPerShare == 0) || lpSupply == 0 || block.number < startBlock) {
    ...
    function deposit(uint256 tokenID) public{
        ...
        IStakeWar(manager.members("stakeWar")).pushEthnicityNFTNFTStake2(tokenID);
    ...
    function withdraw() external {
        ...
        IStakeWar(manager.members("stakeWar")).removeEthnicityNFTNFTStake2(tokenid);
        updatePool(_pid);
