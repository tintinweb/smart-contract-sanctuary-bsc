...
contract BandzaiNFT is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => Monster) _monster;
    mapping(uint256 => uint256) _IpfsPathId;
    mapping(uint256 => uint256) _seasonOf;
    ...
    uint256 public multiplierXpUp = 103;

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return IipfsIdStorage(gameAddresses.getIpfsStorageAddress()).getTokenURI(_seasonOf[tokenId],_monster[tokenId].state,_IpfsPathId[tokenId]);
    }
    ...
    function mintMonster(
        ...
        _IpfsPathId[_newItemId] = _ipfsId;
        _seasonOf[_newItemId] = I.getCurrentSeason();

        // All monsters are created with 8 powers points in a random distribution
        uint256[8] memory randoms = _generateRandomDatas(_ipfsId, _to, _newItemId);

        Monster storage m = _monster[_newItemId];
        m.state = _state;
        m.name = _name;
        for(uint256 i = 0; i < randoms.length; i++){
            if(randoms[i] % 5 == 0 ){
                m.powers.water = m.powers.water + 1;
                totalPowersInGame.water = totalPowersInGame.water + 1;
            }
            if(randoms[i] % 5 == 1 ){
                m.powers.fire = m.powers.fire + 1;
                totalPowersInGame.fire = totalPowersInGame.fire + 1;
            }
            if(randoms[i] % 5 == 2 ){
                m.powers.metal = m.powers.metal + 1;
                totalPowersInGame.metal = totalPowersInGame.metal + 1;
            }
            if(randoms[i] % 5 == 3 ){
                m.powers.air = m.powers.air + 1;
                totalPowersInGame.air = totalPowersInGame.air + 1;
            }
            if(randoms[i] % 5 == 4 ){
                m.powers.stone = m.powers.stone + 1;
                totalPowersInGame.stone = totalPowersInGame.stone + 1;
            }
        }
    ...
    function updateXp(
        ...
        Monster storage m = _monster[_id];
        m.xp = m.xp + _xp;
        // update level
        uint256 _level = _getLevel(m.xp);
        if (_level >= m.level) {
            // update new level
            uint256 _numberOfLevelUp = _level - m.level;
            // monster win 3 points by level raised
            m.creditForUpgrade = m.creditForUpgrade + (_numberOfLevelUp * 3);
    ...
    function updatePowers(
        ...
        uint256 _toSub =_water + _fire + _metal + _air + _stone;
        require(m.creditForUpgrade >= _toSub, "Not enough pts");
        m.creditForUpgrade = m.creditForUpgrade - _toSub;
       
       if(_water >0){
        m.powers.water = m.powers.water + _water;
        totalPowersInGame.water = totalPowersInGame.water + _water;
       }
       if(_fire >0){
        m.powers.fire = m.powers.fire + _fire;
        totalPowersInGame.fire = totalPowersInGame.fire + _fire;
       }
       if(_metal >0){
        m.powers.metal = m.powers.metal + _metal;
        totalPowersInGame.metal = totalPowersInGame.metal + _metal;
       }
       if(_air >0){
        m.powers.air = m.powers.air + _air;
        totalPowersInGame.air = totalPowersInGame.air + _air;
       }
       if(_stone >0){
        m.powers.stone = m.powers.stone + _stone;
        totalPowersInGame.stone = totalPowersInGame.stone + _stone;
       }
    }

    function updateAlchemyXp(uint256 _tokenId, uint256 _xpRaised) external onlyAuth{
        _monster[_tokenId].alchemyXp = _monster[_tokenId].alchemyXp + _xpRaised; 
    }
    ...
    function getStatus(uint256 _tokenId) external view returns(uint256[2] memory){
        return [_statusID[_tokenId],_onCenter[_tokenId]];
    }
    ...
    function _getToAdd(uint256 _toAdd, uint256 _level) internal view returns(uint256){
        return ((_toAdd * multiplierXpUp / 100) + (35  * xpUpBase * _level / 100));
    }

    function _getLevel(uint256 _xp) private view returns (uint256) {
        uint256 _xpNeededToGoUp = xpUpBase;
        uint256 _level = 0;
        uint256 _toAdd = xpUpBase;
        while (_xp >_xpNeededToGoUp) {
            _level = _level + 1;
            _toAdd = _getToAdd(_toAdd,_level);
    ...
    function getMonsterState(uint256 _tokenId) external view returns (uint256) {
        return _monster[_tokenId].state;
    }

    function monster(uint256 _tokenId) external view returns(Monster memory){
        return _monster[_tokenId];
    }
    ...
    {
        Monster memory m = _monster[_tokenId];
    ...
    function _generateRandomDatas(
        ...
        uint256 r = IOracle(gameAddresses.getOracleAddress()).getRandom(keccak256(abi.encodePacked(_user, _ipfsId, _nonce, block.timestamp)));
