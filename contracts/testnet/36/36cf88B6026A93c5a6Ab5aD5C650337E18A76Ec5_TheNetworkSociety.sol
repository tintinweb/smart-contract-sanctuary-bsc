...
contract TheNetworkSociety is Context, Ownable {
    ...
    struct Network {
        bool hasNetworkInvites; //if exist or not, if true, then use current id, else create a new one and designate an id.
    ...
    event MaxPayout(address indexed addr, uint256 amount);
    event AutoCompound(address indexed addr, uint256 timestamp);
    event Sponsor(address indexed addr, address indexed sponsor);
    event Airdropped(address indexed from, address indexed to, uint256 amount, uint256 timestamp);
    ...
    event LastBuyPayout(uint256 indexed round, address indexed winner, uint256 amount, uint256 drawTime);
    ...
    function setupSponsorshipDetails(address addr, address sponsor) private {
        if(isValidSponsorAddress(addr, sponsor)) {
            ...
            if(!networks[sponsor].hasNetworkInvites) {
                uint256 networkId = totalNetworksCreated++;
                network.hasNetworkInvites = true;
            ...
            emit Sponsor(addr, sponsor); //record total invites of network leader
        }
    }
    ...
    function deposit(address addr, uint256 amount) private isInitialized {
        ...
        (, ,uint256 existingDividends, ) = payoutOf(addr, false);
        if(existingDividends > 0 && user.totalDepositAmount.add(amount) < maxCompoundOf(user.totalDirectDeposits)) {
            ...
            user.totalDepositAmount += existingDividends;	
            user.totalIncomeCompounded += existingDividends;        
            totalCompounded += existingDividends;
            emit Compound(addr, existingDividends);
        }
        ...
        emit Deposit(addr, amount.sub(payTax(amount)));
        ...
        if(user.networkSponsor != address(0)) {
            ...
            // referrer
            usersBonus[user.networkSponsor].directBonus += referrerBonus; //statistics record.
            ...
            // referee
            usersBonus[addr].directBonus += refereeBonus; //statistics record.
    ...
    function drawLastDepositWinner() private {
        if(lastDepositEnabled && getCurrentTime().sub(lastDepositLastDrawAction) >= lastDepositTimeStep && lastDepositPotentialWinner != address(0)) {
            ...
            usersBonus[lastDepositPotentialWinner].lastDepositBonus += busdReward;
            totalWithdrawn = totalWithdrawn.add(busdReward);
            token.transfer(lastDepositPotentialWinner, busdReward.sub(payTax(busdReward)));
            emit LastBuyPayout(lastBuyPoolCycle, lastDepositPotentialWinner, busdReward, getCurrentTime());
    ...
    function drawTopReferrerPool() private {
        if(topReferrerEnabled && getCurrentTime().sub(topReferrerLastDrawAction) >= eventTimeStep) {
            ...
            for(uint16 i = 0; i < eventBonusesArr.length; i++) {
                ...
                usersBonus[topReferrerPool[i]].topReferrerBonus += netBusdReward; //only for statistics
    ...
    function drawTopDepositPool() private {
        if(topDepositEnabled && getCurrentTime().sub(topDepositLastDrawAction) >= eventTimeStep) {
            ...
            for(uint16 i = 0; i < eventBonusesArr.length; i++) {
                ...
                uint256 netBusdReward = busdReward.sub(busdReward.mul(eventSustainabilityTax).div(dividerPercent));
                usersBonus[topDepositPool[i]].topDepositBonus += netBusdReward; //only for statistics
    ...
    function withdraw() public isInitialized {
        ...
        if(user.totalDepositPayouts.add(netPayout) >= maxPayout) {
            ...
            emit MaxPayout(addr, user.totalDepositPayouts);
        }
    ...
    function compound(address addr) public isInitialized {   
        ...
        emit Compound(addr, finalCompoundAmount);
	}
    ...
    function airdrop(address receiver, uint256 amount) public isInitialized {
        ...
        require(user.networkSponsor != address(0), "Network not found.");
        ...
        emit Airdropped(sender, receiver, payout, getCurrentTime());
    }
    ...
    function networkAirdrop(uint256 amount) public isInitialized {
        ...
        require(networks[sender].owner != address(0) && networks[sender].owner == sender, "Network not found.");

        token.transferFrom(address(sender), address(this), amount);

        uint256 payout = amount.sub(payTax(amount));
        uint256 memberDivider = networks[sender].members.length;
        uint256 amountDivided = payout.div(memberDivider);

        for(uint256 i = 0; i < networks[sender].members.length; i++) {

            address receiver = address(networks[sender].members[i]);
            ...
            totalAirdrops += amountDivided; //update total airdrop sent
            emit Airdropped(sender, receiver, amountDivided, getCurrentTime());
        }
    }
    ...
    function userInfo(address addr) view public returns(address networkSponsor, uint256 lastAction, uint256 totalDirectDeposits, uint256 totalDepositAmount, uint256 totalIncomeCompounded, uint256 totalDepositPayouts, uint256 yieldPercentage) {
    ...
    function userBonusInfo(address addr) view public returns(uint256 directBonus, uint256 lastDepositBonus, uint256 topDepositBonus, uint256 topReferrerBonus) {
        return (usersBonus[addr].directBonus, usersBonus[addr].lastDepositBonus, usersBonus[addr].topDepositBonus, usersBonus[addr].topReferrerBonus);
    }

    //user's airdrop information.
    function userAirdropInfo(address addr) view public returns(uint256 lastAirdrop, uint256 lastAirdropReceivedTime, uint256 airdropSent, uint256 airdropReceived) {
        return  (airdrops[addr].lastAirdropSentTime, airdrops[addr].lastAirdropReceivedTime, airdrops[addr].airdropSent, airdrops[addr].airdropReceived);    
    }
    
    //user's network member info
    function userNetworkMemberInfo(address addr) view public returns(uint256 id, address owner, uint256 createTime, address[] memory members, uint256 totalNetworkInvites, uint256 totalNetworkInvitesDeposit) {
    ...
    function userNetworkSponsorMemberInfo(address addr) view public returns(uint256 id, address owner, uint256 createTime, address[] memory members, uint256 totalNetworkInvites, uint256 totalNetworkInvitesDeposit) {
    ...
    function contractInfo() view public returns(uint256 _totalUsers, uint256 _totalDeposited, uint256 _totalWithdrawn, uint256 _totalCompounded, uint256 _totalAirdrops, uint256 _totalNetworksCreated, uint256 _totalCurrentBalance) {
    ...
    function getEnabledFeatures() view public returns(bool _initialized, bool _airdropEnabled, bool _autoCompoundEnabled, bool _networkAirdropEnabled, bool _lastDepositEnabled, bool _topReferrerEnabled, bool _topDepositEnabled) {
        _initialized = initialized; 
        _airdropEnabled = airdropEnabled;
        _autoCompoundEnabled = autoCompoundEnabled;         
        _networkAirdropEnabled = networkAirdropEnabled;
        _lastDepositEnabled = lastDepositEnabled;
        _topReferrerEnabled = topReferrerEnabled;
        _topDepositEnabled = topDepositEnabled;
    }
    ...
    function lastDepositInfo() view public returns(uint256 _lastBuyPoolCycle, uint256 _lastDepositPoolBalance, uint256 _lastDepositLastDrawAction, address _lastDepositPotentialWinner) {
    ...
    function topReferrerInfo() view public returns(address[5] memory addrs, uint256[5] memory deps, uint256 _topReferrerPoolCycle, uint256 _topReferrerPoolBalance, uint256 _topReferrerLastDrawAction) {
    ...
    function topDepositInfo() view public returns(address[5] memory addrs, uint256[5] memory deps, uint256 _topDepositPoolCycle, uint256 _topDepositPoolBalance, uint256 _topDepositLastDrawAction) {
    ...
    function runAutoCompound(address addr) external onlyExecutor isInitialized {
        ...
        emit AutoCompound(addr, getCurrentTime());
    }
    ...
    function checkDrawEvents() external view returns (bool runEvent) {
        if(topDepositEnabled && getCurrentTime().sub(topDepositLastDrawAction) >= eventTimeStep) runEvent = true;
        if(topReferrerEnabled && getCurrentTime().sub(topReferrerLastDrawAction) >= eventTimeStep) runEvent = true;
        if(lastDepositEnabled && getCurrentTime().sub(lastDepositLastDrawAction) >= lastDepositTimeStep && lastDepositPotentialWinner != address(0)) runEvent = true;
        return runEvent;
    }
