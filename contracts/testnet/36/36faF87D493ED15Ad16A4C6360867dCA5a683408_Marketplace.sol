...
contract Marketplace is IMarketplace, Ownable, ERC721Holder {
    ...
    uint256 private activeSystemTrading = 1677236151; //need to update
    ...
    function getReferredNftValueForAccount(address _wallet) public view override returns (uint256) {
        ...
        return referredNftValue[_wallet] / 10 ** decimalCurrency;
    }
    ...
    function getNftCommissionEarnedForAccount(
        ...
        uint8 decimalCurrency = ERC20(currency).decimals();
        return nftCommissionEarned[_wallet] / 10 ** decimalCurrency;
    }

    function updateNetworkData(address _wallet, uint256 _totalValueUsdWithDecimal) internal {
        // Update Active Members
        if (userReferralCode[msg.sender] == 0) {
            uint256 currentMember = totalActiveMembers[_wallet];
            totalActiveMembers[_wallet] = currentMember + 1;
        }
        // Update Referred NFT Value
        uint256 currentNftValueInUsdWithDecimal = referredNftValue[_wallet];
        referredNftValue[_wallet] = currentNftValueInUsdWithDecimal + _totalValueUsdWithDecimal;
        ...
        uint256 commissionAmountInUsdWithDecimal = (_totalValueUsdWithDecimal *
            commissionBuyPercent) / 100;
        nftCommissionEarned[_wallet] = currentCommissionEarned + commissionAmountInUsdWithDecimal;
    }
    ...
    function checkValidRefCodeAdvance(
        ...
        address[] memory refTree = new address[](10);
        refTree[0] = _user;
        uint i = 1;
        while (i < 10 && currentRefUser != systemWallet) {
    ...
    function buyByCurrency(
        uint256[] memory _nftIds,
        uint256 _refCode
    ) public override validRefCode(_refCode) {
        ...
        for (uint index = 0; index < _nftIds.length; index++) {
            ...
            require(
                AICrewNFT(nft).ownerOf(_nftIds[index]) == address(this),
                "MARKETPLACE: NOT OWNER THIS NFT ID"
            );
            totalValueUsd += priceNftUsd;
        }
        uint256 totalValueUsdWithDecimal = totalValueUsd * (10 ** decimalCurrency);
        require(
        ...
        // Update network data
        updateNetworkData(refAddress, totalValueUsdWithDecimal);
        // Transfer nft from marketplace to buyer
        bytes memory data = abi.encode(msg.sender);
        for (uint index = 0; index < _nftIds.length; index++) {
            try AICrewNFT(nft).safeTransferFrom(address(this), msg.sender, _nftIds[index], data) {
        ...
        payReferralCommissions(msg.sender, refAddress, totalValueUsdWithDecimal);
    ...
    function buyByToken(
        ...
        uint256 totalValueInToken = Oracle(oracleContract).convertUsdBalanceToToken(totalValueUsd);
        uint8 decimalToken = ERC20(token).decimals();
        uint256 totalValueInTokenWithDecimal = totalValueInToken * (10 ** decimalToken);
        require(
            ERC20(token).balanceOf(msg.sender) >= totalValueInTokenWithDecimal,
            "MARKETPLACE: NOT ENOUGH BALANCE CURRENCY TO BUY NFTs"
        );
        require(
            ERC20(token).allowance(msg.sender, address(this)) >= totalValueInTokenWithDecimal,
            "MARKETPLACE: MUST APPROVE FIRST"
        );
        // Transfer token from buyer to marketplace
        require(
            ERC20(token).transferFrom(msg.sender, address(this), totalValueInTokenWithDecimal),
            "MARKETPLACE: FAILED IN TRANSFERING CURRENCY TO MARKETPLACE"
        );
        // Transfer nft from marketplace to buyer
        uint8 decimalCurrency = ERC20(currency).decimals();
    ...
    function sell(uint256[] memory _nftIds) public override notExpireForSelling {
        ...
        uint256 totalRefundUsd = (totalValueUsd * (100 - discount)) / 100;
        uint256 totalValueInToken = Oracle(oracleContract).convertUsdBalanceToToken(totalRefundUsd);
        uint8 decimalToken = ERC20(token).decimals();
        uint256 totalValueInTokenWithDecimal = totalValueInToken * (10 ** decimalToken);
        require(
        ...
        bytes memory data = abi.encode(msg.sender);
        for (uint index = 0; index < _nftIds.length; index++) {
            try AICrewNFT(nft).safeTransferFrom(msg.sender, address(this), _nftIds[index], data) {
    ...
    function payReferralCommissions(
        ...
        require(
            ERC20(token).balanceOf(address(this)) >= commissionAmountInToken,
        ...
        emit PayCommission(_buyer, _receiver, commissionAmountInToken);
    }
