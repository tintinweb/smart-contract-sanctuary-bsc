...
//        require((user.deposits.length==0) || (user.deposits[user.deposits.length-1].amount <= tokenAmount),"Amount Should be greater than previous amount");
		...
		if (user.referrer == address(0)) 
		{
			...
			for (uint256 i = 0; i < 10; i++) {
				if ((upline != address(0))&& (BASE_CONDITION_AMOUNT[i] <= users[upline].levelinvestments[0]) && (BASE_CONDITION_LEVEL[i] <= users[upline].levels[i]))
				{
    ...
    function canuserwithdraw(address userAddress) public view returns (bool) {
        ...
        if(user.deposits.length ==0)
            return false;
        if( (user.deposits[0].start.add(TIME_STEP.add(100)) < block.timestamp ) && (user.levels[0]==0) )
            return false;
		return true;
	}
	...
	function getUserAvailable(address userAddress) public view returns(uint256) {
		...
		return (getUserReferralBonus(userAddress).add(getUserDividends(userAddress))).min(getUserTotalDeposits(userAddress).mul(2));
	}
