...
contract MasterPlatypusV3 is IMasterPlatypusV3{    
    ...
    */
    ...
    // bsc testnet
    address private fUSDT_LP = address(0x3BB362690ADbE8bcA2277642D8746d18783d43c6);
    address private DAI_LP = address(0xCFf3CB66680cDB2692A13EB0Ac5EF4E9DB5a0Dd7);
    address private USDC_LP = address(0x6A44422A90fA4c062E2979C928280D697A8Da4C1);    
    ...
    */
    ...
    function deleteLPToken(address lpToken) public onlyOwner {
        ...
        if (isExist) {
            for (uint i=idx;i<lpTokens.length-1;i++) {
    ...
    function _addLPStaker (address user) private {
        bool isExist = false;
        for (uint i=0;i<_lpStakers.length;i++) {
            if (user == _lpStakers[i]) isExist = true;
        }

        if (!isExist) _lpStakers.push(user);
    }

    function _deleteLPStaker (address user) private {
        bool isExist = false;
        uint idx;
        for (uint i=0;i<_lpStakers.length;i++) {
            if (user == _lpStakers[i]) {
                isExist = true;
                idx = i;
            }
        }
        ...
        if (isExist) {
            _lpStakers[idx] = _lpStakers[_lpStakers.length - 1];
            _lpStakers.pop();
        }
    }
    ...
    function _updateLPStakedUserInfoForUnStaking (uint256 pid, address user, uint256 amount) private {
        ...
        if (!isLPStaked) _deleteLPStaker(user);
    }
    ...
    function autoBalance () public returns (uint256 autoBalancedUserCount) {
        Amounts memory totalLPAmounts = _setAmounts (IAsset(lpTokens[0]).totalSupply(), IAsset(lpTokens[1]).totalSupply(), IAsset(lpTokens[2]).totalSupply());
        uint256 newTotalLPAmount1;
        uint256 newTotalLPAmount2;
        uint256 newTotalLPAmount3;

        autoBalancedUserCount = 0;
        // new balanced total token amount should not over than cash amount in LP (real token amount)
        for (uint i=0;i<_lpStakers.length;i++) {
            bool isAutoBalance = _lpStakedUserInfo[0][_lpStakers[i]].autoBalancePeriod > 0;            
            ...
            if (isAutoBalance && isOverPeriod) {
                ...
                if (tAPR > 0) {
                    ...
                    newTotalLPAmount1 += newLPAmounts.amt1;
                    newTotalLPAmount2 += newLPAmounts.amt2;
                    newTotalLPAmount3 += newLPAmounts.amt3;
                    
                    if (newTotalLPAmount1 <= totalLPAmounts.amt1 && newTotalLPAmount2 <= totalLPAmounts.amt2 && newTotalLPAmount3 <= totalLPAmounts.amt3) {
    ...
    /**
        ...
        - rewardLastTimestamp = currentTimestamp
...
interface IMasterPlatypusV3 {        
    ...
    function stakingLPFromOther (address from, uint256 pid, uint256 amount, uint256 autoBalancePeriod) external;   
