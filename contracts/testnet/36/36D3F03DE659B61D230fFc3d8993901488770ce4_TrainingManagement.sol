...
contract TrainingManagement is Ownable {
    ...
    string[6] slotStatus = ["not_set","closed","free","in_use","waiting_coach","waiting_for_finish_training"];
    ...
    function cheatZaiXp(uint256 _zaiId, uint256 _xp) external{
        IZaiNFT(gameAddresses.getZaiAddress()).updateXp(_zaiId,_xp);
    }

    function cleanZaiStatus(uint256 _zaiId) external {
            IZaiNFT(gameAddresses.getZaiAddress()).updateStatus(_zaiId, 0, 0);
    }
    // ------------------------------------------- //
    ...
    modifier zaiReady(uint256 _zaiId){
        require(IZaiNFT(gameAddresses.getZaiAddress()).isFree(_zaiId), "Zai not free");
        require(block.timestamp >= _lastTrainTimestamp[_zaiId] + 1 days, "zai need to rest");
        _;
    }
    ...
    onlyCenterOwner(_trainingId)
    {
        ...
        require(_duration <= maxDurationTraining, "Training can exceed 6h");
    ...
    function registerCoaching(uint256 _zaiId, uint256 _spotId, uint256 _trainingId) external canUseZai(_zaiId) zaiReady(_zaiId){
        TrainingInstance storage t = trainingSpots[_trainingId][_spotId];
        IZaiNFT IZai = IZaiNFT(gameAddresses.getZaiAddress());
        require(t.coach.coachRequired,"!Exist");
        require(t.coach.coachId == 0 || block.timestamp > t.endAt, "Got coach or training not finished");
    ...
    function cleanSpot(uint256 _trainingId, uint256 _spotId) external onlyCenterOwner(_trainingId){
        TrainingInstance storage t = trainingSpots[_trainingId][_spotId];
        require(block.timestamp >= t.endAt,'Training not fininshed');
        IZaiNFT IZai = IZaiNFT(gameAddresses.getZaiAddress());
        _cleanSlot(_trainingId, _spotId, IZai);
    ...
    zaiReady(_zaiId)
    {
        ...
        if(t.coach.coachId != 0){
            ...
            if(t.coach.currentCoachLevel > z.level){
                levelDiff = t.coach.currentCoachLevel - z.level;
            }
            ownerPayment *= _getMultiplier(levelDiff);
        ...
        require(payWithRewardOrWallet(msg.sender, _paymentAddress, ownerPayment + coachPayment));
    ...
    function _getMultiplier(uint256 levelDiff) internal pure returns(uint256){
        if(levelDiff < 3){
            return 1;
        }else if(levelDiff < 5){
            return 2;
    ...
    function quiteCoaching(uint256 _zaiId, uint256 _spotId, uint256 _trainingId) external canUseZai(_zaiId){
        TrainingInstance storage t = trainingSpots[_trainingId][_spotId];
        require(_zaiId == t.coach.coachId, "Not your Zai who's coaching in this training slot");

        IZaiNFT IZai = IZaiNFT(gameAddresses.getZaiAddress());
        _cleanSlot(_trainingId,_spotId,IZai);
    }

    function finishTraining(uint256 _spotId, uint256 _trainingId, uint256 _zaiId) external canUseZai(_zaiId){
        TrainingInstance memory t = trainingSpots[_trainingId][_spotId];
        require(block.timestamp >= t.endAt, "Training is not finished");
    ...
    function _cleanSlot(uint256 _trainingId, uint256 _spotId, IZaiNFT IZai) internal {
        TrainingInstance storage t = trainingSpots[_trainingId][_spotId];
    ...
    function _updateZai(uint256 _zaiId, uint256 _duration, uint256 _coachId,IZaiNFT I)
        private
    {
        ...
        if(_coachId != 0){
            ...
            if(c.level - z.level > 0){
                ...
                _duration = _duration * (levelDiff > 30 ? 30:levelDiff); // max multiplier 30 
            }
        }
