...
interface IMysteryBox {
    ...
    function mintMysteryBox(
        ...
        string calldata _boxDescription, 
        uint256 _quantity,
...
contract MysteryBox is Pausable, Ownable, ERC1155, ERC1155Holder, IMysteryBox {
    ...
    mapping(uint256 => BoxInfo) private _boxesInfo;
    mapping(uint256 => uint256) public tokenSupply;

    event MIntMysteryBox(BoxType boxType, string boxName, string boxDescription, uint256 quantity, string tokenURL, Status status);

    function mintMysteryBox(
        BoxType _boxType,
        string memory _boxName,
        string memory _boxDescription,
        uint256 _quantity,
        ...
        if (_quantity != 0) _mint(address(_msgSender()), tokenId, _quantity, "");
        tokenSupply[tokenId] = _quantity;
        _boxesInfo[tokenId] = BoxInfo({
            boxType: _boxType,
            boxName: _boxName,
        ...
        emit MIntMysteryBox(_boxType, _boxName, _boxDescription, _quantity, _tokenURL, _status);
    }
    ...
    function getBox(uint256 _boxId) public view returns (BoxInfo memory) {
        return _boxesInfo[_boxId];
    }
