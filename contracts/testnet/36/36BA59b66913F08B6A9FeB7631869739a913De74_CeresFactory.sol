...
contract CeresFactory is ICeresFactory, Ownable {
    ...
    modifier tokenAdded(address _token) {
        require(tokenInfo[_token].tokenAddress != address(0), "Token is not added");
        _;
    }
    ...
    function getTokensLength() external override view returns (uint256){
        return tokens.length;
    }
    function getVolTokensLength() external override view returns (uint256){
        return volTokens.length;
    }
    function getTokenInfo(address _token) external override view returns (TokenInfo memory){
        return tokenInfo[_token];
    }
    function getStaking(address _token) external override view returns (address) {
        return tokenInfo[_token].stakingAddress;
    }
    function getOracle(address _token) external override view returns (address) {
        return tokenInfo[_token].oracleAddress;
    }
    function getQuoteToken() external view returns (address) {
        return creator.quoteToken();
    }
    function isStakingRewards(address _staking) external override view returns (bool) {
        return tokenInfo[_staking].isStakingRewards;
    }
    function isStakingMineable(address _staking) external override view returns (bool) {
        return tokenInfo[_staking].isStakingMineable;
    }
    function getTokenPrice(address _token) external override view returns (uint256) {
        return IOracle(tokenInfo[_token].oracleAddress).getPrice();
    }
    function getValidStakings() external override view returns (address[] memory _stakings){
    ...
    /* ---------- Public Func : Staking ---------- */
    function createStaking(address _token, bool _createOracle) external override returns (address staking, address oracle){

        require(tokenInfo[_token].tokenAddress == address(0), "Staking already created!");

        staking = creator.createStaking(_token);
        tokenInfo[_token].tokenAddress = _token;
        tokenInfo[_token].stakingAddress = staking;

        if (_createOracle) {
            oracle = createOracle(_token);
            tokenInfo[_token].oracleAddress = oracle;
        }
    ...
    function createStakingWithLiquidity(address _token, uint256 _tokenAmount, uint256 _quoteAmount, bool _createOracle) 
        ...
        IERC20(_token).transferFrom(msg.sender, address(creator), _tokenAmount);
        IERC20(creator.quoteToken()).transferFrom(msg.sender, address(creator), _quoteAmount);
        creator.addLiquidity(_token, _tokenAmount, _quoteAmount, msg.sender);
    ...
    function createOracle(address _token) public override returns (address oracle) {
        require(tokenInfo[_token].oracleAddress == address(0), "Oracle already created!");

        oracle = creator.createOracle(_token);
        tokenInfo[_token].oracleAddress = oracle;
    }
    function addStaking(address _token, uint256 _tokenType, address _staking, address _oracle, bool _isStakingRewards,
        ...
        require(tokenInfo[_token].tokenAddress == address(0), "Staking already added!");
        require(_token != address(0) && _staking != address(0), "Staking parameters can not be zero address!");
        require(_tokenType > 0, "Token type must be bigger than zero!");
    ...
    function removeStaking(address _token, address _staking) external override onlyOwner {
        isValidStaking[_staking] = false;
        if (tokenInfo[_token].stakingAddress == _staking)
            tokenInfo[_token].stakingAddress = address(0);
        stakingCount--;
    }
    /* ---------- Public Func: Oracle ---------- */
    ...
    function updateOracle(address _token) public override {
        address oracle = tokenInfo[_token].oracleAddress;
