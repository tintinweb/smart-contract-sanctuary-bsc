...
contract TimelockStakeV3
{
        ...
        for(uint ix = 0; ix < poolPlayers[poolID].length; ix++)
        {
            ...
            if(amountToTransfer > 0)
            {
                ...
                for(uint ixRecord = 0; ixRecord < playerStakeRecordAmount[poolID][player].length; ixRecord++)
                {
                    ...
                    // (, , , uint256 redeemable, ,) = getInterestReward(poolID, playerStakeRecordAmount[poolID][player][ixRecord], poolAPR[poolID], stakePeriod, ixRecord, player);
                    ...
                    {
                        uint256 redeemableInResultToken = getAmountOutMin(platformToken, poolPrizeToken[poolID], irr.reward, swapUsesMultihop);
                        internalSwap(platformToken, poolPrizeToken[poolID], irr.reward, redeemableInResultToken, player, swapUsesMultihop);
                    }
                }
    ...
    *  *****************************/
    ...
    function getPlayerStakeRecord(uint poolID, address player, uint index) external view validPool(poolID) returns (PlayerStakeRecord memory result)
    {
        ...
        // uint256 stakePeriodForNow = forcedEndTime - startTime;
        
        // vResult.earnedForNow  =  Earned for now
        // vResult.powerUpRewardPartForNow  =  Powerup reward part for now
        // vResult.powerUpListForNow  =  Power up list record for now
        // (, , ,vResult.earnedForNow, vResult.powerUpRewardPartForNow, vResult.powerUpListForNow) = getInterestReward(poolID, playerStakeRecordAmount[poolID][player][index], poolAPR[poolID], (forcedEndTime - vResult.startTime), index, player);
        ...
        // uint256 stakePeriodForEnd = expectedEndTime - startTime;

        // vResult.earnedForEnd  =  // Earned for end
        // (, , ,vResult.earnedForEnd, , ) = getInterestReward(poolID, playerStakeRecordAmount[poolID][player][index], poolAPR[poolID], (vResult.endTime - vResult.startTime), index, player);
    ...
    // function getInterestReward(uint poolID, uint256 stakedAmount, uint256 apr, uint stakePeriod, uint index, address player) internal view returns (uint256, uint256, uint256, uint256, uint256, PoweredUpRewardRecord[] memory)
    function getInterestReward(uint poolID, uint256 stakedAmount, uint256 apr, uint stakePeriod, uint index, address player) internal view returns (InterestRewardRecord memory)
    {
        ...
        else
        {
            ...
            {
                reward = getAmountOutMin(poolStakingToken[poolID], platformToken, rewardBasedOnStakeToken, swapUsesMultihop);
            }
            else
            {
        ...
        // return (annualReward, amountInYears, rewardBasedOnStakeToken, reward, powerUpReward, powerUpList);
    ...
    {
        // uint256 weiPerSecond = redeemableValue / stakePeriod;
        ...
        for(uint ix = 0; ix < arrPowerUp.length; ix++)
        {
            ...
            // uint256 weiToDeduct = powerRecord.durationInSeconds * weiPerSecond;
    ...
    *  *****************************/
    ...
    function _poolStakeWithdraw(uint poolID, uint poolRecordIndex, address player) internal validPool(poolID)
    {
        ...
        // (, , , uint256 redeemable, , ) = getInterestReward(poolID, amountToUnstake, poolAPR[poolID], stakePeriod, poolRecordIndex, player);
    ...
    *  **********************************************************/
    ...
    function getLPPrice(address lpToken, uint256 amountOfLPToken, address priceInToken) internal view returns (uint256 result)
    {
        ...
        // uint112 priceOfToken0InTermsOfToken1 = reserve0 / reserve1;
        // uint112 priceOfToken1InTermsOfToken0 = reserve1 / reserve0;

        uint256 reserve0Price = getAmountOutMin(token0, priceInToken, reserve0, swapUsesMultihop);
        uint256 reserve1Price = getAmountOutMin(token1, priceInToken, reserve1, swapUsesMultihop);
