...
contract TRDCvault is BEP20 {
  ...
  function runCop(uint cardToUse) external returns(uint amountCollected, uint amountBurned, uint amountbackToRewards){
      ...
      require(thiefRewards.length > 0, "Thiefs run away!");
  ...
  function claimReward() external returns(uint rewardThief, uint rewardCop){
      require(gameRunning != false, "Game is paused by owners!");
      ...
      if(playerIsHolder[msg.sender] == true){
          claimRewardsThief();
          (uint _thiefReward) = claimRewardsThief();
          rewardThief = _thiefReward;
      }
      if(playerIsCop[msg.sender] == true){
          claimRewardsCop();
          (uint _copReward) = claimRewardsCop();
          rewardCop = _copReward;
      }
      return(rewardThief, rewardCop);
  }
  function claimRewardsThief() internal returns(uint _thiefReward){
      removePlayer();
  ...
  function claimRewardsCop() internal returns(uint _copReward){
      removePlayer();
