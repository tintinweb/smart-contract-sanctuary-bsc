...
contract ManagerAuction is Initializable, OwnableUpgradeable, PausableUpgradeable, ERC721HolderUpgradeable {
	...
	function setTimeCheckEndAuction(uint256 _newTimeCheckEndAuction) external onlyOwner returns (bool) {
		require(_newTimeCheckEndAuction >= 5 minutes && _newTimeCheckEndAuction <= 60 minutes, "Invalid-timeCheckEndAuction");
	...
	function setPaymentMethod(address _token, bool _status) external onlyOwner returns (bool) {
		...
		if (_token != address(0)) {
			if(_status){
				...
				IERC20Upgradeable(_token).safeApprove(msg.sender, 0);
				IERC20Upgradeable(_token).safeApprove(address(this), 0);
			}			
		}
