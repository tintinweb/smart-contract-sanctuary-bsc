...
contract TokenFarm is Ownable {
    ...
    function addPool (address _tokenAddress, address _rewardTokenAddress, uint256 _maxPoolSize, uint256 _maxContribution, uint256 _rewardAmount, uint256 _emergencyFees, uint256 _lockDays, bool _poolType, bool _poolActive) public onlyOwner {
    ...
    function claimableRewards (uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (daysPassed > poolInfo[_pid].lockDays) return 0;

        uint256 _refundValue = ((userInfo[_pid][_user].amount * poolInfo[_pid].rewardAmount * daysPassed) / (poolInfo[_pid].currentPoolSize * poolInfo[_pid].lockDays));
        if (_refundValue < userInfo[_pid][_user].rewardClaimed) return 0;
        uint256 _netRefundValue = _refundValue - userInfo[_pid][_user].rewardClaimed;
        return _netRefundValue;
    }
