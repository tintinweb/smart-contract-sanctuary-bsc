...
contract RDNPos is Context, AccessControlEnumerable {
    ...
    function activateTariff(uint _tariff) public {
        ...
        if (amountReq[0] > 0) {
            ...
            token1.approve(address(distributor1), amountReq[0]);
            distributor1.distribute(_msgSender(), (amountReq[0]*reward)/10000);
        ...
        if (amountReq[1] > 0) {
            ...
            token2.approve(address(distributor2), amountReq[1]);
            distributor2.distribute(_msgSender(), (amountReq[1]*reward)/10000);
    ...
    function calcActivationPrice(uint _userId, uint _tariffFrom, uint _tariffTo) public view returns(uint[2] memory) {
        uint[2] memory price;
        if (usersPaid[_userId][0] > 0 || usersPaid[_userId][0] > 0) {
