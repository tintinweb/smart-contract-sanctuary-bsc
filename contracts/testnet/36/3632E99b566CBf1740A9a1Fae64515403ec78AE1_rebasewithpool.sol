...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    function calculateNftRewards(uint256 tokenId, address investor) private view returns(uint256 rewards){
        ...
        uint256 numofminutes = (timenow-nftinv.lastclaimtime);
        //require (numofminutes >= 1,"Rewards are distributed every minute");
        if(numofminutes<1)
        {
            return 0;
        ...
        if(nftinv.catagory == uint(nftCatagory.Bronze)){
            ...
            uint256 apyForSeconds = (bronzeNFTapy*10**18 / 86400) ;
            uint256 check = (priceofbronzeNFT * apyForSeconds) / 10**18;
            return (check).div(100)*numofminutes;
        }
        else if(nftinv.catagory == uint(nftCatagory.Silver)){
            // return ((priceofsilverNFT.mul(silverNFTapy)).div(100))*numofminutes;
            uint256 apyForSeconds = (silverNFTapy*10**18 / 86400) ;
            uint256 check = (priceofsilverNFT * apyForSeconds) / 10**18;
            return (check).div(100)*numofminutes;
        }
        else if(nftinv.catagory == uint(nftCatagory.Gold)){
            // return ((priceofgoldNFT.mul(goldNFTapy)).div(100))*numofminutes;
            uint256 apyForSeconds = (goldNFTapy*10**18 / 86400) ;
            uint256 check = (priceofgoldNFT * apyForSeconds) / 10**18;
            return (check).div(100)*numofminutes;
        }
    ...
    function calculateRewards(address tokenAddress, address investor, uint256 apy) private view returns(uint256 rewards){
        ...
        // uint256 numofminutes = (timenow-investment.lastclaimtime).div(2 minutes);
        uint256 numofminutes = (timenow-investment.lastclaimtime);
        //require (numofminutes >= 1,"Rewards are distributed every minute");
        if(numofminutes<1)
        {
            return 0;
        ...
        // return ((investment.amount.mul(apy)).div(100))*numofminutes;
         uint256 apyForSeconds = (apy*10**18 / 86400) ;
         uint256 check = (investment.amount * apyForSeconds) / 10**18;
         return (check).div(100)*numofminutes;
    }
