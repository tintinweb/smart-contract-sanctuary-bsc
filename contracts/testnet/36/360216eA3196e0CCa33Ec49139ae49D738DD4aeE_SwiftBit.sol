...
contract SwiftBit is Ownable, Pausable, ReentrancyGuard {
    ...
    function _updatePercentage(address account) private {
        uint8 newPercentage = 3;
        if (stake[account].stake >= 50000000000000000000) {
            newPercentage = 5;
        }
        else if (stake[account].stake >= 5000000000000000000) {
            newPercentage = 4;
        }
        if (stake[account].percentage < newPercentage) {
            stake[account].percentage = newPercentage;
        }
    }
