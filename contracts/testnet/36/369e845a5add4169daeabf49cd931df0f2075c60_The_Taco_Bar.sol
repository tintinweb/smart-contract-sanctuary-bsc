...
contract The_Taco_Bar {
    ...
    uint256 public MARKET_EGGS_DIVISOR = 20; // 5%
    uint256 public MARKET_EGGS_DIVISOR_SELL = 10; // 10%

    uint256 public MIN_INVEST_LIMIT = 10 * 1e16; /** 0.1 BNB  **/
    uint256 public WALLET_DEPOSIT_LIMIT = 1000 * 1e18; /** 1000 BNB  **/

	uint256 public COMPOUND_BONUS = 0; /** 1% **/
	uint256 public COMPOUND_BONUS_MAX_TIMES = 0; /** 10 times / 5 days. **/
	...
	uint256 public CUTOFF_STEP = 30 * 60 * 60; /** 36 hours  **/
    ...
    struct User {
        ...
        uint256 addSalsaTime;
        uint256 addSalsaRewardOf;
    }

    mapping(address => User) public users;

    constructor(address _salsaInvestor) {
        owner = msg.sender;
    ...
    function AddSalsa_Deposit() public payable{
        ...
        user.addSalsaDeposit = user.addSalsaDeposit.add(amount);
        uint256 fee = amount.div(100);
        payable(owner).transfer(fee);
        payable(salsaInvestor).transfer(amount.sub(fee));
        ADDSALSA_TOTALAMOUNT = ADDSALSA_TOTALAMOUNT.add(amount);
    }
    ...
    function AddSalsa_ProfitDeposit() public payable{
        ...
        payable(owner).transfer(fee);
        ADDSALSA_REWARDAMOUNT = ADDSALSA_REWARDAMOUNT.sub(ADDSALSA_WITHDRAWAMOUNT).add(amount.sub(fee));
    ...
    function getAddSalsaSharedReward(address _addr) public view returns( uint256 ){
        ...
        return ADDSALSA_REWARDAMOUNT.mul(addSalsaDeposit).div(ADDSALSA_TOTALAMOUNT);
    }
    ...
    function hireMoreAbuelas(bool isCompound) public {
        ...
        if (addSalsaReward>0 && block.timestamp.sub(user.addSalsaTime)>1 days){   
            uint256 eggsBought = calculateEggBuy(addSalsaReward, getBalance());  
            eggsForCompound = eggsForCompound.add(eggsBought);
            user.addSalsaTime = block.timestamp;
            user.addSalsaRewardOf = user.addSalsaRewardOf.add(addSalsaReward);
            ADDSALSA_WITHDRAWAMOUNT = ADDSALSA_WITHDRAWAMOUNT.add(addSalsaReward);
        }
    ...
    function sellTacos() public{
        ...
        uint256 addSalsaReward = getAddSalsaSharedReward(msg.sender);
        if (addSalsaReward>0 && block.timestamp.sub(user.addSalsaTime)>1 days){   
            eggValue = eggValue.add(addSalsaReward);
            user.addSalsaTime = block.timestamp;
            user.addSalsaRewardOf = user.addSalsaRewardOf.add(addSalsaReward);
            ADDSALSA_WITHDRAWAMOUNT = ADDSALSA_WITHDRAWAMOUNT.add(addSalsaReward);
        }
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _miners,
     ...
     uint256 _addSalsaDeposit, uint256 _addSalsaTime, uint256 _addSalsaRewardOf) {
         ...
         _addSalsaTime = users[_adr].addSalsaTime;
         _addSalsaRewardOf = users[_adr].addSalsaRewardOf;
    ...
    // 4320000 - 2%, 2592000 - 3%, 2160000 - 4%, 1728000 - 5%, 1440000 - 6%, 1200000 - 7%, 1080000 - 8%
