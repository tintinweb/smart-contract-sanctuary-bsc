...
abstract contract Referral is IReferral {
    ...
    function setReferrer(address account ,address _referrer) public virtual override {
        require(_referrers[account] == address(0), "repeat operation");
        require(
            _referrers[_referrer] != address(0),
            "referrer does not have permission"
        );
        _referrers[account] = _referrer;
        _recommended[_referrer].push(account);
        emit SetReferrer(account, _referrer);
    }
...
contract ReferralShipContract is Referral, SafeOwnable {
    ...
    function setReferrer(address account,address _referrer) public override onlyOwner {
        super.setReferrer(account,_referrer);
    }
