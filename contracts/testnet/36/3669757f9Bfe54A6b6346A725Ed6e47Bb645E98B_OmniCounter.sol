...
contract OmniCounter is NonblockingLzApp {
    ...
    constructor(address _lzEndpoint, address _dstContract) NonblockingLzApp(_lzEndpoint) {dstContract = _dstContract;}
    ...
    function estimateFees() public view returns(uint messageFee){
        (messageFee, ) = lzEndpoint.estimateFees(mumbai, address(this), bytes(""), false, bytes(""));       
    }
    ...
    function incrementCounter() public payable {
        ...
        lzEndpoint.send{value: msg.value}( // {value: messageFee} will be paid out of this contract!
            mumbai, // destination chainId
            abi.encodePacked(dstContract), // destination address of PingPong contract
            bytes(""), // abi.encode()'ed bytes
    ...
    function setDstContract(address _dstContract) public onlyOwner {
