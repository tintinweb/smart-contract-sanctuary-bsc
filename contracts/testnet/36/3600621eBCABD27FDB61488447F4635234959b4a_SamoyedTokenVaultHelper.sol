...
contract SamoyedTokenVaultHelper is ReentrancyGuard {
    ...
    function _depositPrizeFromToken(address vault, uint256 tokenInAmount) internal {
        ...
        (ISamoyedVault _vault, address token) = _getVaultPair(vault);
        uint256 _wantBalance = IERC20(token).balanceOf(address(this));
        IERC20(_vault.want()).transfer(vault, _wantBalance);
        _vault.earn();
    }

    function _getVaultPair(address vault) private view returns (ISamoyedVault _vault, address _want) {
