...
interface IDao7Factory{
    function createTopic(address _owner,address _mainContract,address _userAddress) external returns (address);
}
...
contract Dao7BetFree is Ownable {
    // version
    ...
    constructor(address _WETH,string memory _name,address _Factory) {
		owner = msg.sender;
        WETH = _WETH;
        FACTORY = _Factory;
    ...
    function approveMainContractOfTopic(address topic) private{
        for(uint256 i=0;i<tokenMap.keys.length;i++){
            ...
            IDao7TopicFree(topic).approveMainContract(key);
        }
    }

    function setEndTime(address key,uint256 _endTime) public onlyadmin{
        topicMap.endTime[key] = _endTime;
    }

    function setGroupId(address key,uint256 _groupId) public onlyadmin{
        topicMap.groupId[key] = _groupId;
    }

    function setCanBet(address key,bool _canBet) public onlyadmin{
    ...
    function getTokenMinAmount(address key) public view returns (uint256){
        return tokenMap.minAmount[key];
    }
    function getTokenFee(address key) public view returns (uint256){
        return tokenMap.fee[key];
    }
    function getTokenRebate1(address key) public view returns (uint256){
        return tokenMap.rebate1[key];
    }
    function getTokenRebate2(address key) public view returns (uint256){
        return tokenMap.rebate2[key];
    }
    function getTokenEnabed(address key) public view returns (bool){
        return tokenMap.enabled[key];
    }
    ...
    function approveMainContractOfToken(address token) private{
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            IDao7TopicFree(key).approveMainContract(token);
        }
    }
    ...
    function getInvitationAmount(address token,address userAddress) public view returns(uint256){
        ...
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            userAmount += IDao7TopicFree(key).getInvitationAmount(token, userAddress);
        }
        return userAmount;
    }
    ...
    function getWithdrawInvitationAmount(address token,address userAddress) public view returns(uint256){
        ...
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            if(IDao7TopicFree(key).getWithdrawFlag(token, userAddress)){
                continue;
            }
    ...
    function getWinAmount(address token,address userAddress) public view returns(uint256){
        ...
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            userAmount += IDao7TopicFree(key).getWinAmount(token, userAddress);
        }
        return userAmount;
    }
    ...
    function getCancelAmount(address token,address userAddress) public view returns(uint256){
        ...
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            userAmount += IDao7TopicFree(key).getCancelAmount(token, userAddress);
        }
        return userAmount;
    }
    ...
    function getWithdrawCancelAmount(address token,address userAddress) public view returns(uint256){
        ...
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            if(IDao7TopicFree(key).getWithdrawCancelFlag(token, userAddress)){
                continue;
            }
            userAmount += IDao7TopicFree(key).getCancelAmount(token, userAddress);
        }
    ...
    function getBetAmountOfUser(address token,address userAddress,uint256 filter) public view returns(uint256){
        ...
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            if(filter == 3 && topicMap.trueOption[key] > 0 && !topicMap.isCancel[key]){
                userAmount += IDao7TopicFree(key).getBetAmountOfUser(token, userAddress);
            } 
    ...
    function getBetAmountOfTopic(address token,address topic) public view returns(uint256){
        return IDao7TopicFree(topic).getBetAmount(token);
    }

    function getBetAmountOfUserTopic(address token,address topic,address userAddress) public view returns(uint256){
        return IDao7TopicFree(topic).getBetAmountOfUser(token, userAddress);
    }
    ...
    function clickWithdraw(address token) public{
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            if(IDao7TopicFree(key).getWithdrawFlag(token, msg.sender)){
                continue;
            }
            uint256 invitationAmount = IDao7TopicFree(key).getInvitationAmount(token, msg.sender);

            uint256 winAmount = IDao7TopicFree(key).getWinAmount(token, msg.sender);
            if(invitationAmount == 0 && winAmount == 0){
                continue;
            }
            
            IDao7TopicFree(key).withdrawFrom(token, msg.sender);
    ...
    function clickWithdrawCancel(address token) public{
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            if(IDao7TopicFree(key).getWithdrawCancelFlag(token, msg.sender)){
                continue;
            }

            uint256 cancelAmount = IDao7TopicFree(key).getCancelAmount(token, msg.sender);
            if(cancelAmount == 0){
                continue;
            }
            
            IDao7TopicFree(key).withdrawCancelFrom(token, msg.sender);
    ...
    function clickWithdrawFee(address token) public onlyadmin{
        for(uint256 i=0;i<topicMap.keys.length;i++){
            ...
            if(IDao7TopicFree(key).getWithdrawFeeFlag(token)){
                continue;
            }
            
            uint256 feeAmount = IDao7TopicFree(key).getFeeAmount(token);
            
            IDao7TopicFree(key).withdrawFeeFrom(token);
    ...
    function openBet(address topic,uint256 _trueOption) public{
        require(msg.sender == IDao7TopicFree(topic).owner(),"illegal address");
        IDao7TopicFree(topic).openBet(_trueOption);
    }

    function openBetOfToken(address topic,address token,uint256 _trueOption) public{
        require(msg.sender == IDao7TopicFree(topic).owner(),"illegal address");
        IDao7TopicFree(topic).openBetOfToken(token, _trueOption);
    }
    ...
    function setCancelTopic(address key) public{
        require(msg.sender == IDao7TopicFree(key).owner(),"illegal address");
    ...
    function betFrom(address topic,address token,uint256 amount,uint256 optionId,uint256 inviteCode) public{
        ...
        IDao7TopicFree(topic).betFrom(token, msg.sender, amount, optionId, inviteCode);
    }


    function createTopic(uint256 navId,string memory topicJson,uint256 optionCount,uint256 endTime,uint256 groupId) public{
        address key = IDao7Factory(FACTORY).createTopic(msg.sender, address(this), USER);
        topicMap.inserted[key] = true;
        topicMap.navId[key] = navId;
        topicMap.topicJson[key] = topicJson;
        topicMap.optionCount[key] = optionCount;
        topicMap.canBet[key] = true;
        topicMap.trueOption[key] = 0;
        topicMap.isCancel[key] = false;
        topicMap.endTime[key] = endTime;
        topicMap.groupId[key] = groupId;
        topicMap.createTime[key] = block.number;
