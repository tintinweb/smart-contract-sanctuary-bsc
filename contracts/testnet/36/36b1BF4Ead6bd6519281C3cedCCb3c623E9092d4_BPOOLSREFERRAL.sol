...
contract BPOOLSREFERRAL is Ownable, AccessControl {
    ...
    function setTreasuryAddress(address treasuryAddress)external onlyOwner returns(bool){
        _treasuryAddress = treasuryAddress;
        return true;
    }

    function setPointAmountPerToken( uint pointAmountPerToken) external onlyOwner returns(bool){
        _pointAmountPerToken = pointAmountPerToken;
        return true;
    }

    function setMinSwapPointAmount(uint minSwapPointAmount) external onlyOwner returns(bool){
        _minSwapPointAmount = minSwapPointAmount;
        return true;
    }

    function ClaimReferral(uint pointAmount, uint blockNumber, uint8 _v, bytes32 _r, bytes32 _s) payable external returns (uint){
