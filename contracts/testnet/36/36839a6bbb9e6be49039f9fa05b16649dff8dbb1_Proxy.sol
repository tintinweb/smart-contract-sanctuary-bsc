...
interface IProxy{
    function getTierUserCount() external view returns (uint,uint,uint,uint);
    function getAllocationToken() external returns (uint256);
    function tierAllocationPerUser(uint256 supply) external view returns (uint,uint,uint);
...
contract BitdriveLaunchpad is Owned {
    ...
    function getTokenPerBusdOld() public view returns(uint256,uint256,uint256){
        (uint256 silverAllocation,uint256 goldAllocation,uint256 diamondAllocation) = IProxy(PROXY).tierAllocationPerUser(hardCap);
        uint256 silverUserBusd   = silverAllocation / tokensPerBusd;
        uint256 goldUserBusd   = goldAllocation / tokensPerBusd;
        uint256 diamondUserBusd   = diamondAllocation / tokensPerBusd;
        return (silverUserBusd,goldUserBusd,diamondUserBusd); // original value id should divide by 10000
    }

    function getTokenPerBusd() public view returns(uint256,uint256,uint256){
        (uint256 silverAllocation,uint256 goldAllocation,uint256 diamondAllocation) = IProxy(PROXY).tierAllocationPerUser(hardCap);
        return (silverAllocation,goldAllocation,diamondAllocation); // original value id should divide by 10000
    }

    function checkBronzeUserCount() internal view returns(uint256){
        (uint256 brozeUser,,,) = IProxy(PROXY).getTierUserCount();
        return brozeUser;
    }
    ...
    // function getUserInfo(address _userAddress) public view returns(uint256,uint256,bool,uint256){
    //     uint getTier = IProxy(PROXY).getUsertier(_userAddress);
    //     (uint256 silverUserBusd,uint256 goldUserBusd,uint256 diamondUserBusd) = getTokenPerBusd();
    //     require(getTier > 0, "Not a valid User");
    //     uint256 remainingAllocation;
    //     if(getTier == 1)
    //       remainingAllocation = 0;
    //     if(getTier == 2)
    //       remainingAllocation = silverUserBusd;  
    //     if(getTier == 3)
    //       remainingAllocation = goldUserBusd;  
    //     if(getTier == 4)
    //       remainingAllocation = diamondUserBusd;  
    //     uint256 remainingallocationAmount = remainingAllocation - userInfo[_userAddress].investAmount;
    //     return (remainingallocationAmount,userInfo[_userAddress].investAmount,userInfo[_userAddress].isExits,userInfo[_userAddress].SwapTokens);
    // }
...
contract Proxy {
    ...
    function getTierUserCount() public view returns (uint,uint,uint,uint){
        return (Bronzeusers,Silverusers,Goldusers,Diamondusers);
    }
    ...
    function tierAllocationPerUser(uint256 maxSupply) public view returns (uint256,uint256,uint256) {
        ...
        return (silverAllocation / Silverusers,goldAllocation / Goldusers,diamondAllocation / Diamondusers);
    }
