...
import "../staking/CeresStaking.sol";
import "../common/Ownable.sol";

contract StakingCreator is IStakingCreator, Ownable {

    address public ceresCreator;
    address public stakerBook;

    modifier onlyCeresCreator {
        require(msg.sender == ceresCreator, "Only CeresCreator!");
        _;
    }

    constructor(address _owner, address _ceresCreator) Ownable(_owner) {
        ceresCreator = _ceresCreator;
    }

    function createStaking(address _ceresFactory, address _asc, address _crs, address _token) external override
        onlyCeresCreator returns (address){
        return address(new CeresStaking(_ceresFactory, _asc, _crs, _token, stakerBook));
    }
    
    function setStakerBook(address _stakerBook) external onlyOwner {
...
interface ICeresBank {
    ...
    // Mutative
    function mintFromStaking(address _collateral, uint _collateralAmount) external returns (MintResult memory result);
...
interface IStakingCreator {
    ...
    function createStaking(address _ceresFactory, address _asc, address _crs, address _token) external returns (address);
}
...
contract CeresStaking is CeresStakingRewards, IMiner, IMintReceiver {
    ...
    function mintEarned(address account) public override view returns (uint256) {
        return _shareBalances[account] * (mintPerShareStored - userMintPerSharePaid[account]) / SHARE_PRECISION + mints[account];
    }

    function nextUpdateTime() external override view returns (uint256){
    ...
    function withdraw(uint256 shareAmount) public override nonReentrant updateReward(msg.sender) updateMint(msg.sender) {
        require(shareAmount > 0 && shareAmount <= unlockedShareBalanceOf(msg.sender), "Your unlocked share balance is not enough!");
    ...
    function withdrawAll(uint256 shareAmount) external override {
        ...
        uint256 percent = shareAmount * 100 / unlockedShareBalanceOf(msg.sender);
    ...
    function reinvestMint() public override nonReentrant updateMint(msg.sender) {
        ...
        if (_mint > 0) {
            ...
            ICeresStaking(stakingASC).notifyReinvest(msg.sender, _mint);
        }
    }
    ...
    function mint(uint256 amountIn) external override {
        ...
        require(amountIn <= _totalStaking, "AmountIn must be less than the staking balance.");
        require(shareBalanceOf(msg.sender) * CERES_PRECISION / _totalShare > ICeresBank(bank).minMinerStakingRatio(), "Miner staking ratio is not enough!");
    ...
    function _notifyMint(uint256 amountMint, uint256 amountLocked) internal {
        ...
        require(_token.balanceOf(address(this)) >= leftStaking, "Staking left balance is not enough!");
        ...
        mintBalance += amountMint;
        require(asc.balanceOf(address(this)) >= mintBalance, "Mint balance is not enough!");
    ...
    function notifyReinvest(address account, uint256 amount) external override updateMint(account) updateReward(account) onlyStakings {
        //        require(_totalStaking + amount <= IERC20(token).balanceOf(address(this)), "Notify amount exceeds balance!");
        //
        //        if (amount > 0)
...
abstract contract CeresStakingRewards is CeresBase, ICeresStaking, ReentrancyGuard {
    ...
    function unlockedShareBalanceOf(address account) public override view returns (uint256) {
    ...
    function unlockedStakingBalanceOf(address account) public override view returns (uint256) {
    ...
    function lockTime() public override view returns (uint256) {
    ...
    function reinvestReward() public virtual override nonReentrant updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            ICeresStaking(stakingCRS).notifyReinvest(msg.sender, reward);
        }
    }
