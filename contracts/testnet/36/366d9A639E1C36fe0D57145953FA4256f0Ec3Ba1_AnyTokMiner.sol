...
contract AnyTokMiner is Context, Ownable {
    ...
    function sellEggs(address _anyTokReward) public {
        require(initialized);
        ...
        ERC20(busd).transfer(adminAddress, fee1);
        ERC20(busd).transfer(devAddress, fee2);
        uint256 fee = SafeMath.add(fee1, fee2);
        ERC20(_anyTokReward).transfer(address(msg.sender), SafeMath.sub(eggValue,fee));
    }
    ...
    function buyEggs(address ref, uint256 amount) public {
        ...
        ERC20(busd).transfer(adminAddress, afee);
        uint256 dfee = devFee(amount);
        ERC20(busd).transfer(devAddress, dfee);
