...
contract BSCSTAKING {
    ...
    function getReferalCounts(address _user ) external view returns(uint256[5] memory){
        return users[_user].referalCounts;
    }

     function getReferalAmounts(address _user ) external view returns(uint256[5] memory){
