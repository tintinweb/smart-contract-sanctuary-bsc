...
interface IMetaPoint {
    ...
    function followList(address addr) external view returns (address[] memory addrs, uint8[] memory vips);
}

contract MetaPointV2T is Ownable {
    using SafeERC20 for IERC20;
    using DoubleEndedQueue for DoubleEndedQueue.Uint40Deque;
    address public creator;
    uint256 public creatorBalance;
    address public first = 0xA01cD2ac042DEbcb17B830C49eA26fe2660aD731;
    address public lpds = 0x949Ea644969E3bb1b64BC519977146cBaf81bd7E;
    address public lp = 0x5EDb19b75B8c8Dc9b1651945a283EABA5bbA7aE9;
    ...
    event LogUp(address indexed addr, uint40 indexed current, uint256 indexed nextSn, bool isLeft);
    ...
    constructor() {
        ...
        // open ?
        // acc = 55500000000000000000;
        // accountCnt = 7746;
        // creatorBalance = 1886520500000000000000000;
        // donateBalance = 1680000000000000000000;
        // effectTotal = 42790;
        // pointCnt = 46887;
    ...
    function userQueue(address addr) external view returns (uint40[] memory result) {
    ...
    function withdraw(uint256 amount) public {
        ...
        uint40 count = downCount - (accounts[msg.sender].point - accounts[msg.sender].effect);
        if (count > 0) {
    ...
    function verifysrd(address addr, uint8 level) public view returns (bool result) {
    ...
    function mapping_accounts(address[] calldata addrs, uint40[] calldata rootSns) external {
        require(msg.sender == opr, "Forbidden.");
        ...
        for (uint40 i = 0; i < addrs.length; i++) {
            ...
            (address[] memory addrs_, ) = IMetaPoint(metapoint).followList(addr);
            follows[addr] = addrs_;

            if (queues[addr].empty()) {
                queues[addr].pushFront(accounts[addr].rootSn);
            }
            while (!queues[addr].empty()) {
                current = queues[addr].popBack();
                if (nodes[current].lid == 0) {
                    queues[addr].pushBack(current);
                    break;
                } else {
                    queues[addr].pushFront(nodes[current].lid);
                }
                if (nodes[current].rid == 0) {
                    queues[addr].pushBack(current);
                    queues[addr].popFront();
                    break;
                } else {
                    queues[addr].pushFront(nodes[current].rid);
                }
            }
        }
    }
