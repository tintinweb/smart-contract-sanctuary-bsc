...
contract StakingPerpetual is Initializable, Ownable, ReentrancyGuard
{
	using SafeERC20 for IERC20;

	struct AccountInfo {
		uint256 amount; // PERPS staked
		uint256 reward; // GRO reward from Compound/Volatile VDCs accumulated but not claimed
	...
	address public reserveToken; // PERPS
	...
	constructor(address _reserveToken, address _rewardToken)
	{
		initialize(msg.sender, _reserveToken, _rewardToken);
	}

	function initialize(address _owner, address _reserveToken, address _rewardToken) public initializer
	{
	...
	function depositOnBehalfOf(uint256 _amount, address _account) public nonReentrant
	{
		...
		IERC20(reserveToken).safeTransferFrom(msg.sender, FURNACE, _amount);
