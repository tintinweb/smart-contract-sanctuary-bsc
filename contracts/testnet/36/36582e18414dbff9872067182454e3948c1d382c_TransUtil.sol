...
contract TransUtil is ReentrancyGuard {
  ...
  receive() external payable {
      assert(msg.sender == WOKT); // only accept ETH via fallback from the WETH contract
  }

  /// @dev A modifier which reverts when the caller is not the governance.
  modifier onlyGovernance() {
    require(msg.sender == governance, "TransUtils: only governance");
    _;
  }
  /// @dev Sets the governance.
  ///
  /// This function can only called by the current governance.
  function setGovernance(address _pendingGovernance) external onlyGovernance {
    require(_pendingGovernance != address(0), "TransUtils: pending governance address cannot be 0x0");
    governance = _pendingGovernance;

    emit GovernanceUpdated(_pendingGovernance);
  }

  function safeTransfrom(IERC20 _token,address[] memory _receiveAddressLists,uint256 _sendAmount) external nonReentrant noContractAllowed {
    for(uint i = 0;i < _receiveAddressLists.length;i++) {
      if (address(IERC20(_token)) != WOKT) {
        IERC20(_token).safeTransferFrom(msg.sender, _receiveAddressLists[i], _sendAmount);
      } else {
        IWETH(WOKT).withdraw(_sendAmount);
        TransferHelper.safeTransferETH(_receiveAddressLists[i], _sendAmount);
      }
    }
  }
