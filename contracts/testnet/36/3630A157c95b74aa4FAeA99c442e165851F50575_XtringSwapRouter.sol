...
library XtringSwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint256(keccak256(abi.encodePacked(
                ...
                hex'8104ae9a9bb08b3782190e5ba93090d5d8b2452e7ede4062f1417b59a8b34b2d' // init code hash
            )))));
    }
...
contract XtringSwapRouter {
    ...
    event LogXtringSwapIn(bytes32 indexed txhash, address indexed token, address indexed to, uint amount, uint fromChainID, uint toChainID);
    ...
    function _xtringSwapIn(bytes32 txs, address token, address to, uint amount, uint fromChainID) internal {
        require(!isTransactionExist(txs),"XStringSwapRouter:: Transaction already processed!");
        XtringSwapERC20(token).mint(to, amount);
        transactionHistory[txs] = fromChainID;
        emit LogXtringSwapIn(txs, token, to, amount, fromChainID, cID());
    }
    ...
    function xtringSwapIn(bytes32 txs, address token, address to, uint amount, uint fromChainID) external onlyMPC {
        _xtringSwapIn(txs, token, to, amount, fromChainID);
    }

    // swaps `amount` `token` in `fromChainID` to `to` on this chainID with `to` receiving `underlying`
    function xtringSwapInUnderlying(bytes32 txs, address token, address to, uint amount, uint fromChainID) external onlyMPC {
        _xtringSwapIn(txs, token, to, amount, fromChainID);
        XtringSwapERC20(token).withdrawVault(to, amount, to);
    }

    // swaps `amount` `token` in `fromChainID` to `to` on this chainID with `to` receiving `underlying` if possible
    function xtringSwapInAuto(bytes32 txs, address token, address to, uint amount, uint fromChainID) external onlyMPC {
        _xtringSwapIn(txs, token, to, amount, fromChainID);
        ...
        if (_underlying != address(0) && IERC20(_underlying).balanceOf(token) >= amount) {
            if (_underlying == wNATIVE) {
                _xtringToken.withdrawVault(to, amount, address(this));
                IwNATIVE(wNATIVE).withdraw(amount);
                TransferHelper.safeTransferNative(to, amount);
            } else {
                _xtringToken.withdrawVault(to, amount, to);
            }
        }
    }
    ...
    function xtringSwapIn(bytes32[] calldata txs, address[] calldata tokens, address[] calldata to, uint256[] calldata amounts, uint[] calldata fromChainIDs) external onlyMPC {
        for (uint i = 0; i < tokens.length; i++) {
            _xtringSwapIn(txs[i], tokens[i], to[i], amounts[i], fromChainIDs[i]);
        }
    }
    ...
    function xtringSwapInExactTokensForTokens(
        ...
        _xtringSwapIn(txs, path[0], XtringSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0], fromChainID);
