...
contract ERC20 {
    ...
    address internal OWNER; //Creador de This Contract
    address internal MARKETING; //Wallet de Marketing
    address internal TEAM; //Wallet de los DEVS
    address internal LIQUIDITY; //Wallet para rellenar
    ...
    constructor(
        ...
        address team_ //Settear Wallet de Team durante el Deploy
        ) {
        OWNER = msg.sender;
        ...
        allowances[address(this)][OWNER] = type(uint256).max;
        isPair[PAIR]=true;
    ...
    function _transferConTax(address sender, address recipient, uint amount) private{
        ...
        if( ( sender != PAIR ) && ( !manualSwap ) && ( !_isSwappingContractModifier ) ) _swapContractToken( false );
    ...
    function _swapContractToken(bool ignoreLimits) private swapping{
        ...
        uint marketingBNB = (address(this).balance).div(2);
        uint teamBNB = (address(this).balance).div(2);

        (bool sent,) = MARKETING.call{value: marketingBNB }("");
        sent=true;
        emit FeeSent(MARKETING, marketingBNB);

        (bool Success, ) = TEAM.call{ value: teamBNB }("");
        require(Success, "Failed to send funds to TEAM");
        emit FeeSent(TEAM, teamBNB);
    }
