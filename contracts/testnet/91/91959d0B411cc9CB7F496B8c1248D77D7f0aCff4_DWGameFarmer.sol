...
contract DWGameFarmer {
    ...
    function buy(uint256 _farmerId) external payable {
        ...
        (bool sent, ) = _farmerOwner.call{value: _farmerPrice.sub(_commission)}(
            ""
        );
        require(sent, "Failed to send BNB to the waifu's current owner.");

        (sent, ) = game.rewardPool().call{value: _farmerPrice.sub(_commission)}(
            ""
        );
