...
contract Marketplace is Ownable {
    ...
    constructor(
        ...
        require(
            ...
            "NFTMarketplace: nftAddress_ is not a valid address"
        );
    ...
    function _updateFeeRecipient(address feeRecipient_) internal {
        require(
            ...
            "NFTMarketplace: feeRecipient_ is not a valid address"
        );
    ...
    function _updateFeeRate(uint256 feeDecimal_, uint256 feeRate_) internal {
        require(
            ...
            "NFTMarketplace: feeRate_ is not a valid"
        );
    ...
    function _calculateFee(uint256 orderId_) private view returns (uint256) {
        ...
        return (feeRate * order.price) / (10 ** (feeDecimal + 2));
    }
    ...
    function addPaymentToken(address paymentToken_) external onlyOwner {
        require(
            ...
            "NFTMarketplace: paymentToken_ is not a valid address"
        );

        require(
            _supportedPaymentTokens.add(paymentToken_),
            "NFTMarketplace: paymentToken_ is supported"
        );
    }

    function isPaymentTokenupported(
    ...
    modifier onlySupportedPaymentToken(address paymentToken_) {
        require(
            isPaymentTokenupported(paymentToken_),
    ...
    function addOrder(
        ...
        require(
            ...
            "NFTMarketplace: sender is not owner"
        );

        require(
            nftContract.getApproved(tokenId_) == address(this) ||
                nftContract.isApprovedForAll(_msgSender(), address(this)),
            "NFTMarketplace: the contract is not unauthorized"
        );

        require(
            price_ > 0,
            "NFTMarketplace: the price must be greater than zero"
        );
    ...
    function cancelOrder(uint256 orderId_) external {
        ...
        require(
            ...
            "NFTMarketplace: sender must be owner"
        );
    ...
    function executeOrder(uint256 orderId_) external {
        ...
        require(
            ...
            "NFTMarketplace: sender must be different from seller"
        );

        require(_order.price > 0, "NFTMarketplace: this order is not selling");
