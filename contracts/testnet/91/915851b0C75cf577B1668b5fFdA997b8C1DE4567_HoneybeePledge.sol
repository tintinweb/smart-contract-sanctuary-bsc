...
contract HoneybeePledge is Initializable, OwnableUpgradeable, DivestorUpgradeable {
    ...
    CommDebt[6] public commPool;
    address[] public superNodeList;

    mapping(address => bool) public whiteList;
    mapping(uint => NodeSlot) public nodePool;
    ...
    function initialize() initializer public {
        ...
        dailyInfo.burnRate = 100;

        dayOpt = dayOpt / 1 days;
        debt.rate = dayOpt * 4 / 10;
        dailyInfo.nodeRate = dayOpt * 1 / 10;

        dailyInfo.commRate = dayOpt * 5 / 100;

        dailyInfo.marketRate = 4566 ether * 2 / 100;
        dailyInfo.technologyRate = 4566 ether * 3 / 100;
    ...
    function _tranferTeam() private {
        if (startTime == 0) {
            return;
        }
        uint tm = block.timestamp - dailyInfo.lastSendTeam;
        if (tm > 1 days) {
            for (uint i = 0; i < tm / 1 days; i++) {
                addrInfo.HBEE.safeTransfer(addrInfo.market, dailyInfo.marketRate * (100 - dailyInfo.burnRate) / 100);
                addrInfo.HBEE.safeTransfer(addrInfo.technology, dailyInfo.technologyRate * (100 - dailyInfo.burnRate) / 100) ;
            }
            dailyInfo.lastSendTeam = block.timestamp;
        }
    }
    ...
    function _start() private  {
        ...
        for (uint i = 1; i <= 34; i++) {
            nodePool[i].lastTime = startTime;
        }

        for (uint i = 1; i < 6; i++) {
            commPool[i].startDebted = coutingCommDebt(i);
        }
        dailyInfo.lastSendTeam = startTime;
    }

    function _out(address account_, uint stakeBurn_, uint commBurn_) private {
        // exit stake 
        _exitStake(account_, stakeBurn_);

        // burn community reward
        if (commBurn_ > 0) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, commBurn_);
        }

        // burn node reward and exit
        uint nodeSlotId = userInfo[account_].nodeSlotId;
        if (nodeSlotId != 0) {
            _burnNodeReward(nodeSlotId);
            nodePool[nodeSlotId].owner = address(0);
        }

        // burn share reward
        if (userInfo[account_].shareReward > 0) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, userInfo[account_].shareReward * (100 - dailyInfo.burnRate) / 100);
        }

        uint stakeValue = userInfo[account_].stakeValue;
        ...
        // lower community power
        _updateCommPower(account_, stakeValue, false);
        _changeBurnRate();
    } 

    function _processOut(uint stakeburn_, uint commBurn_, uint realReward_, uint reward_, address account_) public returns (bool) {
        // burn over reward
        if (realReward_ > reward_) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, realReward_ - reward_);
        }
        _tranferTeam();

        if (!whiteList[account_] && realReward_ > reward_) {
            _out(account_, stakeburn_, commBurn_);
    ...
    function _changeBurnRate() private {
        for (uint i = 0; i < 5; i++) {
    ...
    function _allocateInviterReward(address account_, uint reward) private {
        ...
        for (uint i = 0; i < 3; i++) {
            ...
            if (inviter == address(0) || inviter == address(this) || userInfo[inviter].stakeValue == 0) {
    ...
    function _removeNode(address account_) private {
        ...
        nodePool[slowId].owner = address(0);
    ...
    function _embedNodeSlot(address account_) private  {
        ...
        _burnNodeReward(slowId);
        nodePool[slowId].owner = account_;
    }

    function _burnNodeReward(uint slowId) private {
        require(slowId >= 1 && slowId <= 34, "1");
        uint reward = _viewNodeSlotReward(slowId);
        if (reward > 0) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, reward);
        }
        nodePool[slowId].lastTime = block.timestamp; 
    }
        ...
        for (uint i = 0; i < 10; i++) {
            ...
            if (oldLevel < newLevel) {
                for(uint l = oldLevel + 1; l <= newLevel; l++) {
                    _updateCommPool(inviter, l, true);
                }
            } else {
                for(uint l = oldLevel; l > newLevel; l--) {
                    _updateCommPool(inviter, l, false);
                }
            }
    ...
    function _updateCommPool(address account_, uint poolId_, bool inPool_) private {
        ...
        if (!inPool_) {
            ...
            delete userComm[account_][poolId_];
            if (commPool[poolId_].totalValue == 0) {
                delete commPool[poolId_];
        ...
        commPool[poolId_].totalValue += 1;
        userComm[account_][poolId_] = CommPool ({
            debt: commDebt,
            exist: true
        });

        if (startTime != 0 && commPool[poolId_].totalValue == 1) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, (block.timestamp - commPool[poolId_].endTime) * dailyInfo.commRate * (100 - dailyInfo.burnRate) / 100);
        }
    } 
    ...
    function _exitStake(address account_, uint burnAmount_) private {
        if (burnAmount_ > 0) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, burnAmount_);
        }
        for (uint i = 0; i < 5; i++) {
            delete userSlot[account_][i];
        }
        uint de = coutingDebt();
        debt.debted = de;
    ...
    function editTeamAddress(address market_, address technology_) public onlyOwner {
        addrInfo.market = market_;
        addrInfo.technology = technology_;
    }
    
    function editWhiteList(address[] calldata accounts_, bool add_) public onlyOwner {
        for (uint i = 0; i < accounts_.length; i++) {
            whiteList[accounts_[i]] = add_;
        }
    }

    function editSuperNode(address[] calldata accounts_, bool add_) public onlyOwner {
        require(superNodeList.length + accounts_.length <= 34, "2");
        address nodeAddr;
        for (uint i = 0; i < accounts_.length; i++) {
            nodeAddr = accounts_[i];
            if (add_) {
                require(!isSuperNode(nodeAddr), "22");
                superNodeList.push(nodeAddr);
                _embedNodeSlot(nodeAddr);
            } else {
                require(isSuperNode(nodeAddr), "32");
    ...
    function viewSuperNodeList() public view returns (address[] memory, uint[] memory) {
        return (superNodeList, viewValueList(superNodeList));
    }

    function viewInviterList(address account_) public view  returns (address[] memory, uint[] memory) {
        return (inviterInfo[account_], viewValueList(inviterInfo[account_]));
    }

    function viewValueList(address[] memory accounts_) public view returns (uint[] memory) {
    ...
    function isValidInviter(address account_) public view returns (bool) {
    ...
    function getLevel(address account_) public view returns(uint) {
        ...
        uint level;
        if (info.directInviter >= 5 && info.stakeValue >= 5000 ether && info.commPower >= 1000000 ether ) {
            level = 5;
        } else if (info.directInviter >= 4 && info.stakeValue >= 4000 ether && info.commPower >= 500000 ether ) {
            level = 4;
        } else if (info.directInviter >= 3 && info.stakeValue >= 3000 ether && info.commPower >= 100000 ether ) {
            level = 3;
        } else if (info.directInviter >= 2 && info.stakeValue >= 2000 ether && info.commPower >= 30000 ether ) {
            level = 2;
        } else if (info.directInviter >= 1 && info.stakeValue >= 1000 ether && info.commPower >= 10000 ether ) {
            level = 1;
        } else {
            level = 0;
        }
    ...
    function coutingDebt() public view returns (uint) {
        uint debt_ = debt.totalValue > 0 ? debt.rate * (block.timestamp - debt.lastTime) * 1 ether / debt.totalValue + debt.debted : 0 + debt.debted;
        return debt_;
    }

    function coutingCommDebt(uint poolId) public view returns (uint) {
        CommDebt memory poolInfo = commPool[poolId];
        uint debt_ = poolInfo.totalValue > 0 ? dailyInfo.commRate * (block.timestamp - poolInfo.lastTime) * 1 ether / poolInfo.totalValue + poolInfo.debted : 0 + poolInfo.debted;
        return debt_;
    }

    function _viewStakeSlotReward(address account_, uint slot_) public view returns (uint) {
        require(slot_ >= 0 && slot_ <= 9, '1');
        ...
        uint slotDebt = slotInfo.stakeTime < debt.startTime ? debt.startDebted : slotInfo.debt;
        uint rewards = slotInfo.stakeValue * (coutingDebt() - slotDebt) / 1 ether;
        rewards = rewards * (100 - dailyInfo.burnRate) / 100;
        return rewards;
    } 
    
    function _viewNodeSlotReward(uint slowId) public view returns(uint) {
        if (nodePool[slowId].lastTime == 0 || startTime == 0) {
            return 0;
        }
        uint reward = (block.timestamp - nodePool[slowId].lastTime) * dailyInfo.nodeRate;
    ...
    function viewStakeReward(address account_) public view returns (uint, uint) {
        ...
        uint rewards;
        for (uint i = 0; i < 5; i++) {
            rewards += _viewStakeSlotReward(account_, i);
        }
        uint maxReward = viewMaxClaimAmount(account_);
    ...
    function viewCommSlotReward(address account_, uint poolId_) public view returns (uint) {
        if (!userComm[account_][poolId_].exist || startTime == 0) {
            return 0;
        }

        CommDebt memory poolInfo = commPool[poolId_];
        uint slotDebt = userComm[account_][poolId_].debt;
        if (poolInfo.startDebted != 0 && slotDebt < poolInfo.startDebted) {
            slotDebt = poolInfo.startDebted;
        }
        uint reward = (coutingCommDebt(poolId_) - slotDebt) / 1 ether;
        reward  = reward * (100 - dailyInfo.burnRate) / 100;
        return reward;
    }
    ...
    function viewValidValue(address account_) public view returns (uint) {
        if (whiteList[account_]) {
            return type(uint).max;
        }
        return userInfo[account_].stakeValue * 3 - userInfo[account_].claimedValue;
    }

    function viewMaxClaimAmount(address account_) public view returns (uint) {
        if (whiteList[account_]) {
            return type(uint).max;
        }
    ...
    function stake(uint slot_, uint value_, bool isHBEE_, address inviter_) public {
        ...
        require(status.stake, '1');
        require(slot_ >= 0 && slot_ <= 4, '2');
        ...
        if (debt.startTime == 0 && debt.totalValue + value_ >= 300 ether) {
    ...
    function claimNodeReward() public returns (uint) {
        ...
        uint slotId = userInfo[_msgSender()].nodeSlotId;

        (uint reward, uint realReward) = viewNodeReward(_msgSender());
        require(reward > 0, "3"); 

        (, uint stakeBurn) = viewStakeReward(_msgSender());
        (, uint commBurn) = viewCommReward(_msgSender());

        nodePool[slotId].lastTime = block.timestamp;

        uint gas = reward * 5 / 100;
        _claimReward(_msgSender(), reward, reward - gas, gas);
        _processOut(stakeBurn, commBurn, realReward, reward, _msgSender());
    ...
    function claimShareReward() public returns (uint) {
        ...
        userInfo[_msgSender()].shareReward = 0;
        _processOut(stakeBurn, commBurn, realReward, reward, _msgSender());
    ...
    event ClaimStakeReward(address indexed account, uint indexed reward, uint indexed realReward);
    function claimStakeReward() public returns (uint) {
        require(status.claimStake, "1");
        ...
        uint gas = reward * 5 / 100;
        uint inviterReward = reward / 2;  
        _allocateInviterReward(_msgSender(), inviterReward);
        _claimReward(_msgSender(), reward, reward - gas, gas);

        
        emit ClaimStakeReward(_msgSender(), reward, realReward);

        bool isOut = _processOut(0, commBurn, realReward, reward, _msgSender());
        if (isOut) {
            return reward;
        }
        ...
        uint newDebt = coutingDebt();
        for (uint i = 0; i < 5; i++) {
            if (userSlot[_msgSender()][i].status) {
    ...
    function claimCommReward() public returns (uint) {
        ...
        _claimReward(_msgSender(), reward, reward - gas, gas);
        bool isOut = _processOut(stakeBurn, 0, realReward, reward, _msgSender());
