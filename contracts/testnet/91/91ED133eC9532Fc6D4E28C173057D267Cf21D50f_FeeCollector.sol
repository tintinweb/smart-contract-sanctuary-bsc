...
contract FeeCollector is ReentrancyGuard, AccessControl {
    ...
    function convertCurrencyToRewardToken(
        ...
        path[0] = token;
        path[1] = rewardConvertor.WETH();

        // Exchange token to reward token
        uint256[] memory amounts = rewardConvertor.swapExactTokensForETH(
            amountToConvert,
            minReceivable,
            path,
            address(this),
        ...
        emit ConversionToRewardToken(token, amounts[0], amounts[1]);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < _stakingAddresses.length; i++) {
            ...
            _totalShares -= shares[_stakingAddresses[i]];
