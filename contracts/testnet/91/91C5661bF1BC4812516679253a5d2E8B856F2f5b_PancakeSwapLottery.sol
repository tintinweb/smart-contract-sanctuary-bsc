...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    struct Lottery {
        ...
        // 彩票中奖需按顺序每个数字都一一对应
        // 猜中位数的分红比例 250 375 625 1250 2500 5000
        // 猜中 1个2.5% 2个3.75% 3个6.25% 4个12.5% 5个25% 6个50%
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        // 2000 20% 基金费
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        // 猜中位数对应的奖励/猜中人数 中奖人每一票可领奖励
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            uint256 rewardForTicketId = _calculateRewardsForTicketId(
                _lotteryId, // 期号
                thisTicketId, // 票号 [10,11,12,13]
                _brackets[i] // 猜中位数 [2,4,1,3] 0,1,2,3,4,5
            );

            // 验证参数错误.猜中位数错误 Check user is claiming the correct bracket
            require(rewardForTicketId != 0, "No prize for this bracket");

            // 传入的 猜中位数少了, 实际猜中的位数更多
            if (_brackets[i] != 5) {
                require(
    ...
    function closeLottery(
        ...
        // 请求获取随机数-等待回调获取真正的彩票号码 Request a random number from the generator based on a seed
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        // 最终生成的彩票号码 Calculate the finalNumber based on the randomResult generated by ChainLink's fallback
        uint32 finalNumber = randomGenerator.viewRandomResult();

        // 上一位数猜中人数 Initialize a number to count addresses in the previous bracket
        uint256 numberAddressesInPreviousBracket;

        // 除去基金费 当期可分红的奖金 Calculate the amount to share post-treasury fee
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInCake) *
                (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // 无人中奖不可分的奖励 Initializes the amount to withdraw to treasury
        ...
        // 转入基金金库 Transfer CAKE to treasury address
