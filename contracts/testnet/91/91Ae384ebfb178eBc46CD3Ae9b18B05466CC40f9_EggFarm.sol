...
contract EggFarm {
    ...
    IEggNFT eggNFT;
    bool public isEnableRandom; // true: Enable contract random Egg
    bool public isEnalbeMintNFT; // true: Enable contract mint EggNFT
    bool public isEnalbeClaimBeforeEnd; // true: Enable user can claim profit before end
    uint nonce;
    ...
    event Deposit(address by, uint256 amount, uint256 eggLevel, string eggName);
    ...
    function stake(uint _amount, uint256 _packageId) public {
        // Validate amount
        ...
        if (isEnableRandom) {
            ...
            if (isEnalbeMintNFT) {
                ...
                eggNFT.stakingMint(msg.sender, level, eggName);
            } else {
        ...
        emit Deposit(msg.sender, _amount, level, eggName);
    }
    ...
    function claimProfit(uint256 _profileId) public {
        ...
        if (!isEnalbeClaimBeforeEnd) {
            require(userInfo[_profileId].vestingEnd < block.timestamp, "Can withdraw before staking end.");
        }
