...
contract BBTFactory {
    ...
    function creatNewToken(string memory _names,string memory _symbles, uint8 _decimals, uint256 _totals , uint256 kbn , address markaddr,uint256 marfee, uint256 burnfee) external returns(address) {
...
contract BitBearTOKEN is IERC20  {
    ...
    address public   mariktingAddress;
    ...
    constructor(string memory names, string memory symbols, uint8 dec, uint256 total,address owners , uint256 kbn , address markaddr,uint256 marfees, uint256 burnfees) {
    ...
    function setMarktingWallet(address _val) external onlyOwner {
        mariktingAddress = _val;
          ...
          if(launchBlock.add(KBnum) > block.number && !isExcludedFromFee[_from] && !isExcludedFromFee[_to]){
             burnsFee = _amount.mul(30).div(100);
              ...
              if(isExcludedFromFee[_from] || isExcludedFromFee[_to]){
                  ...
                  unchecked{_balanceOf[mariktingAddress] = _balanceOf[mariktingAddress].add(markFee);}
                  emit Transfer(_from ,mariktingAddress,markFee);
              }
          }
