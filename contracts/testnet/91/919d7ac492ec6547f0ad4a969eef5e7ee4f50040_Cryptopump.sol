...
contract Cryptopump is ContextUpgradeable, OwnableUpgradeable {
    ...
    function claimToken(uint256 _level) public returns(bool) {
        require(users[msg.sender].isExist, "User not exist");
        require(users[msg.sender].claimamount[_level] > 0, "Insufficient claimable amount");
        tokenAddress.transfer(msg.sender, users[msg.sender].claimamount[_level]);
        users[msg.sender].claimamount[_level] = 0;
        users[msg.sender].claimedamount += users[msg.sender].claimamount[_level];
     ...
     */
    function viewClaimAmount(address _userAddress, uint256 _level)external view returns(uint256)
    {
     ...
     */
    function updateTokenPrice(uint256 _level, uint256 _price) public returns(bool) {
        require(msg.sender == ownerAddress, "only OwnerWallet");
        TOKEN_PRICE[_level] = _price;
        return true;
    }

    function setCpumpowner(address _address) public{
        require(msg.sender == ownerAddress, "only OwnerWallet");
        tokenAddress.transferOwnership(_address);
    }
