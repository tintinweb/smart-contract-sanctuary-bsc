...
//  7% tax split across 7 protocols
...
/**
 * @dev The official smart contract
 */
contract SNAFU is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // General Info
    string private _name     = "Some Navy Asshat Fucked Up";
    string private _symbol   = "SNAFU";
    ...
    // Addresses
    address payable public _marketingAddress = payable(0x2d504fA4170d4b5242AC69519de408dd87472bA6); // Marketing address used to pay for marketing
    ...
    uint256 private _tTotal = 4 * 10**15 * 10**9; // 4 quadrilllion 
    ...
    // Token Limits
    uint256 public _maxTxAmount        = 4 * 10**13 * 10**9;  // 40 trillion
    uint256 public _tokenSwapThreshold = 400 * 10**9 * 10**9; // 400 billion
    
    // Timer Constants 
    uint private constant DAY = 86400; // How many seconds in a day
    
    // Anti-Whale Settings 
    uint256 public _whaleSellThreshold = 2 * 10**13 * 10**9; // 20 Trillion
    ...
    uint256 public _buybackUpperLimit    = 1 * 10**10 * 10**9; // 10 billion MSDFLB will be used to buy at maximum in any one trade
    ...
    uint256 public  _lotteryThreshold    = 100 * 10**6 * 10**9; // 100 million tokens required to be in the pool before lottery can be triggered
    uint256 public  _lotteryMinimumSpend = 100 * 10**6 * 10**9; // 100 million tokens required to buy before before lottery can be triggered
    ...
    /**
     * @notice Withdraws non-MSDFLB tokens that are stuck as to not interfere with the liquidity
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // Gets the contracts MSDFLB balance for buybacks, charity, liquidity and marketing
    ...
    /**
     * @notice Burns MSDFLB tokens straight to the burn address
     */
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockSwapping {
        ...
        // Swap 7/8ths of MSDFLB tokens for BNB
        ...
        // We now have 6/7ths left of BNB (converted from MSDFLB)
