...
// KONG BUSD is a FORK of Dino BUSD with some enhancements
// Dino BUSD website : https://dinobusd.finance/
// Dino BUSD contract : https://bscscan.com/address/0xb9150107F2820930D997a91f03Ba81A8d625F337#code
...
contract KongBusd is Context, Ownable , ReentrancyGuard {
    ...
    uint256 public constant withdraw_fee = 8; // Total withdraw fee dev_fee + dino_dev_fee + ref_withdraw_fee
    uint256 public constant dev_fee = 4;
    uint256 public constant dino_dev_fee = 2;
    uint256 public constant ref_withdraw_fee = 2;
    uint256 public constant nftpass_withdraw_fee_discount = 2; // 2% to 4% discount for NFT Pass holders
    uint256 public constant ref_fee = 10;
    
    address public dev = 0x66210175567AD8a0414C530F642E2ad889b6c359;
    address public dinoDev = 0xF7518482245f8Da9D7B3300b840B8B6fD359763b;
    IERC20 private BusdInterface;
    address public tokenAdress;
    IERC721 private nftPass;
    address public nftPassAdress;
    bool public init = false;
    bool public alreadyInvested = false;
    
    event deposited(address _investerAddress, uint256 _amount, uint256 _timestamp);
    event reward_claimed(address _investerAddress, uint256 _amount, uint256 _timestamp);
    event withdrawl_done(address _investerAddress, uint256 _amount, uint256 _timestamp);
    event referral_claimed(address _investerAddress, uint256 _amount, uint256 _timestamp);
    event unstaked(address _investerAddress, uint256 _amount, uint256 _timestamp);

    constructor() {
        tokenAdress = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; 
        BusdInterface = IERC20(tokenAdress);

        nftPassAdress = 0xc2C413698a425FD01a7056DcD687C89877b7A038;
        nftPass = IERC721(nftPassAdress);
    }
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        require(BusdInterface.allowance(msg.sender,address(this)) >= _amount,"Insufficient BUSD allowance");
        
        if(!checkAlreadyInvested()){
    ...
    function withdrawal() public noReentrant {
        ...
        //uint256 totalDevFeeDeductions = SafeMath.add(dino_dev_fee,ref_withdraw_fee);

        //uint256 devFee = SafeMath.sub(wFee,totalDevFeeDeductions);
        
        uint256 devFee;

        if(nftPass.balanceOf(refferal[msg.sender].ref_address) > 0) { // Referrer needs to hold NFT pass to avail passive income
            uint256 totalDevFeeDeductions = SafeMath.add(dino_dev_fee,ref_withdraw_fee);
            devFee = SafeMath.sub(wFee,totalDevFeeDeductions);
            BusdInterface.transfer(refferal[msg.sender].ref_address,dino_dev_fee); // Referrer user gets 2% fee for passive income
        }else{
            devFee = SafeMath.sub(wFee,dino_dev_fee);
        }

        BusdInterface.transfer(dev,devFee); // Kong dev 4% to 6% withdrawal fee
        BusdInterface.transfer(dinoDev,dino_dev_fee); // Dino Dev gets 2% royalty fee
        ...
        emit withdrawl_done(msg.sender, totalAmount, block.timestamp);
    }
    ...
    function claimDailyRewards() public noReentrant{
        ...
        emit reward_claimed(msg.sender, value, block.timestamp);
    }
    ...
    function unStake() external noReentrant {
        ...
        emit unstaked(msg.sender, UnstakeValueCore, block.timestamp);
    }

    function Ref_Withdraw() external noReentrant {
        require(init, "Not Started Yet");
        require(checkAlreadyInvested(), "You don't have any active investments. You need to have atleast one deposit to claim the referrals");
        ...
        emit referral_claimed (msg.sender, value, block.timestamp);
    }
    ...
    function checkAlreadyInvested() public view returns(bool) {
    ...
    function withdrawFee(uint256 _amount) public view returns(uint256) {
        uint256 final_withdraw_fee;
        if(nftPass.balanceOf(msg.sender) > 0){
            final_withdraw_fee = SafeMath.sub(withdraw_fee,nftpass_withdraw_fee_discount);
        } else{
            final_withdraw_fee = withdraw_fee;
        }

        return SafeMath.div(SafeMath.mul(_amount,final_withdraw_fee),100);
    }
