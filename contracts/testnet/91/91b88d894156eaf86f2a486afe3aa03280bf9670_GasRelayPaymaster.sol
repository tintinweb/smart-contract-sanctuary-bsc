...
contract GasRelayPaymaster is IPaymaster, OwnableUpgradeable, TokenHelper {
    ...
    uint256 private constant DEPOSIT = 10000000000000;
    ...
    function withdrawToPool() external {
        require(msg.sender == proxyPoolLogic, "permission denied");
        require(getRelayHubDeposit() > 0, "no gas relayer");

        IRelayHub(getHubAddr()).withdraw(
            getRelayHubDeposit(),
            ...
            if (
                IERC20Upgradeable(
                    IGasRelayPaymasterDepositor(proxyPoolLogic)
                        .denominationAsset()
                ).balanceOf(address(this)) >= expectAmountIn
            ) {
                IGasRelayPaymasterDepositor(proxyPoolLogic)
                    .autoDepositForGasRelayer(diffAmount);

                // swap denomination to bnb
                __approve(pancakeswapRouter, denominationAsset, diffAmount);
...
abstract contract TokenHelper {
    ...
    function __swapExactETHForTokens(
        ...
        uint256[] memory amounts = IPancakeRouter02(_router)
            .swapExactETHForTokens(_amountOutMin, _path, _to, block.timestamp);
        if (amounts.length != 0) {
            amount_ = amounts[amounts.length - 1];
        }
    }
