...
contract LootProgress is Ownable{
    ...
    event InterfacesUpdated(address fightAddress, address rankingAddress, address potionsAddress, address eggsAddress, address oracleAddress);
    event NewLootResult(address indexed user,uint256 lootType, uint256[4] loots); // lootype 0 = Potion || 1 = Egg 
    ...
    function updateInterfaces() external {
        ...
        Eggs = IEggs(gameAddresses.getAddressOf(AddressesInit.Addresses.EGGS_NFT));
        Oracle = IOracle(gameAddresses.getAddressOf(AddressesInit.Addresses.ORACLE));
        emit InterfacesUpdated(fightAddress, address(IRank), address(Potions), address(Eggs), address(Oracle));
    ...
    function updateUserProgress(address _user) external onlyGame returns(uint256 beginingDay){
        Progress storage p = _progress[_user];
    ...
    function _getPotionLoot(address _user, uint256 _numberOfPotions, uint256 _minLevel, uint256 _maxLevel) internal returns(uint256[4] memory potions){
        ...
        for(uint256 i ; i < _numberOfPotions ; ){
            uint256 _power = _minLevel + (r[i] % (_maxLevel - _minLevel - 1)); 
