...
contract ACMClaim is Ownable {
    ...
    address public IDO;
    address public IDOcall;
    modifier onlyIDO() {
        require(msg.sender == IDO, "ACMClaim: caller is not the IDO");
        _;
    }
    modifier onlyIDOcall() {
        require(msg.sender == IDOcall, "ACMClaim: caller is not the IDOcall");
        _;
    }
    ...
    function setCurrentFund(bytes32 h, uint participatedAmount, uint uAmount, uint tokenAmount, address tokenContract, uint pidx, uint eidx, uint epochNum) public onlyIDOcall {
        Funds storage currentFund = currentFunds[h];
    ...
    function setLiquidationCurrentFund(bytes32 h, uint participatedAmount, uint uAmount, uint tokenAmount, address tokenContract, uint pidx, uint eidx, uint epochNum) public onlyIDOcall {
        Funds storage liquidationCurrentFund = liquidationCurrentFunds[h];
    ...
    function setCompleteFund(bytes32 h, uint participatedAmount, uint uAmount, uint lpAmount, uint pidx, uint eidx, uint epochNum) public onlyIDOcall {
        Funds storage completeFund = completeFunds[h];
    ...
    function setReorganizationFund(bytes32 h, uint participatedAmount, uint pidx, uint eidx, uint epochNum) public onlyIDOcall {
    ...
    function setIDO(address _IDO, address _IDOcall) public onlyOwner {
        IDO = _IDO;
        IDOcall = _IDOcall;
    }
