...
contract KalmarBondingStrategy is ReentrancyGuard, Ownable, Pausable {
    ...
    function fairAssetReserve() external view returns(uint256, uint256, uint256, uint256)
    {
      ...
      uint256 fairReserveKalm = Math.sqrt(k.div(p));
      uint256 fairReserveOther = Math.sqrt(k.mul(p));

      return (k, p, fairReserveKalm, fairReserveOther);
    }
