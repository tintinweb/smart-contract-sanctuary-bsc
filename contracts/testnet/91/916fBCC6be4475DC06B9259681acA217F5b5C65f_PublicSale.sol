...
contract PublicSale is SafeTransfer, Pausable, ReentrancyGuard {
  ...
  function canBuySSP(address buyer, uint256 amount, address paymentToken) public view returns (bool) {
    ...
    return isOpenForSale &&
      currentBalanceSSP() >= amount &&
      IERC20(paymentToken).balanceOf(buyer) >= amount * PRICE / (10 ** (ssp.decimals() - IERC20(paymentToken).decimals()));
  }
  ...
  function setFinaliseSale() public onlyAdmin {
    isOpenForSale = false;
    emit SaleFinalized(_msgSender(), block.timestamp);
  }
  ...
  function buySSP(uint256 amount, address paymentToken) public whenNotPaused nonReentrant {
    ...
    _safeTransferFrom(paymentToken, _msgSender(), treasury, amount * PRICE / (10 ** (ssp.decimals() - IERC20(paymentToken).decimals())));
