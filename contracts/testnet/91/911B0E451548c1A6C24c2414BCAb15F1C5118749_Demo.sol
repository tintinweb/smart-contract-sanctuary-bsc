   ...
   function insertHelper(uint value, bytes32 nodeAddress) internal {
        Node memory node = tree[nodeAddress];
            ...
            if (node.left == 0) {
                ...
                if(tree[rootAddress].recycle==0){
                insertNode(node.value, node.left,0);
                tree[nodeAddress].recycle=tree[nodeAddress].recycle+1;
                }
                else if(tree[rootAddress].recycle==1){
                 insertNode(tree[tree[rootAddress].left].value, tree[rootAddress].right, 0);
               
                tree[tree[rootAddress].left].recycle=tree[tree[rootAddress].left].recycle+1;
                }
