...
pragma solidity ^0.8.2;
 ...
 contract AImedisMeta is ERC721,ERC721Enumerable, ERC721URIStorage,AccessControl, Ownable,ERC721Burnable  {
    using Counters for Counters.Counter;
    using Address for address;
    using SafeMath for uint256;


   address internal constant UNISWAP_ROUTER_ADDRESS =  0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // Please put actual pancakeswap router address for Mainnet deployment
   IUniswapV2Router02 public uniswapRouter;
   address public USDT = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;  // Please put the Actual USDT Address of Mainnet during mainnet deployment

    address payable  public _contractOwner;
    Counters.Counter private _tokenIdCounter;
    ERC20 public _mintingCurrency ; // AIMX TOKEN ADDRESS
       
    
    mapping (uint256 => uint256) public price;
    mapping (uint256 => uint256) public tokenNumber;
    mapping (uint256 => bool) public listedMap;
    mapping (uint => bool) public  tokenbuying;
    ...
    constructor(string memory _Name,string memory _Symbol) ERC721(_Name,_Symbol)  {
         _mintingCurrency = ERC20(0x7f74dD6e0C5ed8EBE9B0389e18650c25CDC7E267) ; //SET AIMX Currency for Token put Actual currency for Mainnet
        ...
        _mintingCurrency = newMintingCurrency;
         ...
         function getMintingCurrency() onlyOwner public view returns (ERC20) {
    ...
    function mint(string memory _tokenURI, address _toAddress, uint256 _price) public payable returns (uint256) {
        
        _tokenIdCounter.increment();
        _tokenId = _tokenIdCounter.current();
        price[_tokenId] = _price;  // USDT Price ----> e.g. 2500000000000000000000 Wei for $2500 18 Decimals
    ...
    function buy(uint256 _id) external payable {
        _validate(_id);
        require(msg.value >= getEstimatedBNBforUSD()[1],"Please enter the exact amount to buy this land thanks");
   ...
   function getEstimatedBNBforUSD() public view returns (uint[] memory) {
     return uniswapRouter.getAmountsOut(price[_tokenId], getpathforBNBUSDPrice());
   }


     function getEstimatedBNBforAIMX() public view returns (uint[] memory) {
     return uniswapRouter.getAmountsOut(getEstimatedBNBforUSD()[1],getpathforBNBAIMXPrice());
  }

 
   function getEstimatedBNB(uint256 _id) public view returns (uint[] memory) {
     return uniswapRouter.getAmountsOut(price[_id], getpathforBNBUSDPrice());
   }

     function getEstimatedAIMX(uint256 _id) public view returns (uint[] memory) {
     return uniswapRouter.getAmountsOut(getEstimatedBNB(_id)[1],getpathforBNBAIMXPrice());
  }


    function getpathforBNBUSDPrice() private view returns (address[] memory) {
    address[] memory path = new address[](2);
    path[0] = USDT;
    path[1] = uniswapRouter.WETH();  // Pair Wbnb Address
    return path;
  }

  
    function getpathforBNBAIMXPrice() private view returns (address[] memory) {
    address[] memory path = new address[](2);
    path[0] = uniswapRouter.WETH(); 
    path[1] =address(_mintingCurrency); // pair AIMX Token Address
    return path;
  }
  


     function buywithAimx(uint256 _id) external payable {
        _validate(_id);

        address _previousOwner = ownerOf(_id);
        address _newOwner = msg.sender;
        require(_mintingCurrency.balanceOf(msg.sender) >= getEstimatedBNBforAIMX()[1],"Please Enter Exact Aimx Amount to buy");  
        tokenbuying[_id] = true; 
        _trade(_id);
    ...
    function mintBatch(address _toAddress, uint256 numberofNFT,string[] calldata _tokenURI,uint256[] memory  _price) public   {
      
      for (uint256 i = 0; i < numberofNFT; i++) {
           _tokenIdCounter.increment();
          _tokenId = _tokenIdCounter.current();
          price[_tokenId] = _price[i];
        _safeMint(_toAddress, _tokenId);
        _setTokenURI(_tokenId, _tokenURI[i]);
        emit Minted(_toAddress, _price[i], _tokenId, _tokenURI[i]);
      }
     
    }
    
    function updatePrice(uint256 _id, uint256 _price) public returns (bool) {
        uint256 oldPrice = price[_id];
    ...
    function updateListingStatus(uint256 _id, bool shouldBeListed) public returns (bool) {
      ...
      // Check Total Balance for the Auction Gathered 
