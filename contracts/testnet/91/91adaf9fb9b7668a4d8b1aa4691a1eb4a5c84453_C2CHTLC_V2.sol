...
contract C2CHTLC_V2 is Initializable, ERC165Upgradeable, IERC1155ReceiverUpgradeable {
    ...
    */
    ...
    function counterPartySubmitsOfferRequest( uint256 cpTokenGiveAmount, uint256 cpTokenGiveId, uint256 partyCPTokenWithdrawalExpiration, bytes32 requestOfferId, string calldata keyAuth ) external {
        ...
        require(counterPartyResponded[requestOfferId] == false, "Duplicate response");
    ...
    */
    ...
    function swap( bytes32 requestOfferId, string calldata keyAuth ) external {

        require( requestOfferState[requestOfferId] == OfferState.OPEN, "Request not opened" );
        RequestOfferDetails memory _requestOfferDetails = requestOfferDetails[requestOfferId];
        require(counterPartyResponded[requestOfferId] == true, "Counter-party yet to respond to offer");
        require(msg.sender == _requestOfferDetails.party || msg.sender == _requestOfferDetails.counterParty, "Invalid CP token recipient");
        require(_requestOfferDetails.partySettled == false , "Party has been settled");
        require(_requestOfferDetails.counterPartySettled == false, "Counter-party has been settled");
        require(block.timestamp < _requestOfferDetails.partyTokenWithdrawalExpiration && block.timestamp < _requestOfferDetails.counterPartyTokenWithdrawalExpiration, "Settlement expired");
        CPTOKEN.safeTransferFrom(address(this), _requestOfferDetails.party, _requestOfferDetails.counterPartyTokenGiveId, _requestOfferDetails.counterPartyTokenGiveAmount, "", keyAuth);       //  release token to party
        CPTOKEN.safeTransferFrom(address(this), _requestOfferDetails.counterParty, _requestOfferDetails.partyTokenGiveId, _requestOfferDetails.partyTokenGiveAmount, "", keyAuth);              //  release token to counter party
        requestOfferDetails[requestOfferId].counterPartySettled = true;         //  update counter party settlement status
        requestOfferDetails[requestOfferId].partySettled = true;                //  update party settlement status
        emit Swap( _requestOfferDetails.party, _requestOfferDetails.counterParty, _requestOfferDetails.counterPartyTokenGiveId,  _requestOfferDetails.partyTokenGiveId, _requestOfferDetails.counterPartyTokenGiveAmount, _requestOfferDetails.partyTokenGiveAmount, requestOfferId );
    ...
    */
    ...
    event Swap(address indexed party, address indexed counterParty, uint256 idReceivedByParty, uint256 idReceivedByCounterParty, uint256 amountReceivedByParty, uint256 amountReceivedByCounterParty, bytes32 requestOfferId);
