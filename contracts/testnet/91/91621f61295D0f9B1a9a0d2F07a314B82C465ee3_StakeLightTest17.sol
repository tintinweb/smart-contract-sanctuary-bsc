  ...
  mapping(address => RankIncome[]) public rankIncomes;
   ...
   function initialize(address payable _ownerWallet ,address payable _supportWallet,IBEP20Token _tokenAddress) public {
    ...
    function invest(address _referrer,uint _amount) public payable {
             ...
             if (upline != address(0)){
                ...
                if(i==0 && block.timestamp >= users[upline].deposits[0].start.add(rankTime_stamp)){
                  ...
                  if(amount>0){
                    for(uint j=0 ; j<12;j++){         
                    if(j==0){
                    rankIncomes[upline].push(RankIncome(amount,block.timestamp));
                    }else{
       ...
       user.deposits.push(Deposit(_amount,0,0, block.timestamp, 0,true));
...
payable(msg.sender).transfer(totalwithdrawTokenBNb);
  ...
  function payoutOf(address _addr) view public returns(uint256 payout, uint256 max_payout) {
        ...
        if(count >0){
            ...
            payout = getUserDividends(_addr).add(getUserPassiveIncome(_addr)).add(getBoosterDataContract(_addr));
  ...
  function getcurrentPASSIVEincome(address userAddress) public view returns (uint256 _amount){
        ...
        (uint progress,) = progressReport(userAddress);
         if(progress < max){
             uint count =  user.deposits.length;
             if(count>0){
                _amount = getUserPassiveIncome(userAddress);
                // .sub(user.deposits[count-1].withdrawnp);
                }
 ...
 function getLatestPrice() public view returns (int) {
  ...
  function getBoosterData(address _user) public view returns(uint){
    ...
    uint count = users[_user].boosterCount;
        uint max = maxPayoutOf(_user);
        (uint progress,) = progressReport(_user);
        if(progress < max){
    if(count>=2){
  ...
  function progressReport(address _addr) view public returns( uint256 progress, uint max) {
        ...
        if(count >0){
            if(users[_addr].deposits[count-1].withdrwanUSD < max_payout) {
            uint payout = getUserDividends(_addr).add(getUserPassiveIncome(_addr)).add(getBoosterDataContract(_addr).add(users[_addr].dataWithdrwan));
  ...
  function getDepositInfo(address _user,uint index) public view returns(
    uint amountUSD,uint withdrawn,uint tokenAmount,uint start,bool roiActive,uint withdrwanUSD){
        ...
        // withdrawnp =  users[_user].deposits[index].withdrawnp;
