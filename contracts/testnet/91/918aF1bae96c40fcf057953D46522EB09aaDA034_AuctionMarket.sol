...
contract AuctionMarket is ERC721 {
    ...
    mapping(address => uint256) public royalty; // Owner's address => rate (10 will be 11/1000 = 1,1%)
    mapping(uint256 => address) public creators; // TokenId => Owner's address 

    uint256 public MAX_ROYALTY_RATE = 100;
    ...
    function setMaxRoyaltyRate(uint256 _rate) public onlyOwner {
        MAX_ROYALTY_RATE = _rate;
    }
    ...
    function getRoyaltyRate(address creator) public view returns (uint256) {
        uint256 rate = royalty[creator];
        if (rate > MAX_ROYALTY_RATE) {
            return MAX_ROYALTY_RATE;
        } else {
            return rate;
        }
    }
    ...
    function calculateRoyaltyValue(uint256 _id) public view returns (uint256) {
        uint256 royaltyRate = getRoyaltyRate(creators[_id]);
        uint256 royaltyValue = 0;
        if (royaltyRate > 0) {
            royaltyValue = price[_id] * royaltyRate / 1000;
        }
        return royaltyValue;
    }
    ...
    function _trade(uint256 _id) internal {
        ...
        if (saleTokenAddresses[_id] == address(0)) {
            ...
            // Send royalty
            if (royaltyValue > 0) {
                payable(creators[_id]).transfer(royaltyValue);
            }
    ...
    function _endAuction(uint256 _tokenId) internal virtual {
        ...
        if (offer.tokenAddress == address(0)) {
            ...
            // Send royalty
            if (royaltyValue > 0) {
                payable(creators[_tokenId]).transfer(royaltyValue);
            }
