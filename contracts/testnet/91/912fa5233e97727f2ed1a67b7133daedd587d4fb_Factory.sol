...
contract EipERC20 is IEipERC20 {
    using SafeMath for uint;

    string public constant name = 'HiSwap LP Token';
    string public constant symbol = 'HiPDX';
...
contract Pair is IPair, EipERC20 {
    using SafeMath  for uint;
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    uint denominator = rootK.mul(IFactory(factory).feeToRate()).add(rootKLast);
