...
contract BWORKER is Context, IERC20, IERC20Metadata, Ownable, IBWORKER {
    ...
    address public addLiquidityReceiver =
        0x9ABC639F6aCFA133959529435dE03AFcc0663456;
    ...
    bool public _autoMintProfit = false;
    ...
    function _mintNFT(uint256 price) internal {
        ...
        require(lqBUSDAmount > 0, "zero Liquidity");
    ...
    function _addLiquidity() internal {
        uint256 amountToLiquify = busdForAddLiquidity;
        (uint256 lqTokenAmount, uint256 lqBUSDAmount) = tokenPrice();
        if (lqBUSDAmount.mul(lqTokenAmount) == 0) {
            return;
        }
        uint256 amountToSwap = amountToLiquify.mul(lqTokenAmount).div(
            lqBUSDAmount
        );
        _totalSupply = _totalSupply.add(amountToSwap);
        _unLockBalances[address(this)] = _unLockBalances[address(this)].add(
            amountToSwap
        );
        BUSDContract.approve(address(router), amountToLiquify);
        router.addLiquidity(
            BUSD,
            address(this),
            amountToLiquify,
    ...
    function _nftRewardAmount(address account) public view returns (uint256) {
        ...
        if (
            ...
            for (
                ...
                if (profit.mul(totalProfit).mul(_mintProfit[epoch]) != 0) {
                    nftRewardAmount = nftRewardAmount.add(
    ...
    function _autoClaimNftReward(address account) internal {
        ...
        autoClaimEpoch[account] = lastMintEpoch;
        _lockBalances[account] = _lockBalances[account].add(nftRewardAmount);
        _lockBalances[mintHolderAddress] = _lockBalances[mintHolderAddress].sub(
            nftRewardAmount
        );
    }
    ...
    function shouldMintProfit() public view returns (bool) {
        uint256 _currentEpoch = currentEpoch();
        return
            _autoMintProfit &&
            address(nftContract) != address(0) &&
    ...
    function shouldBuyBack() internal view returns (bool) {
        return _autoBuyBack && !inSwap && msg.sender != pair;
    }
    ...
    function sellAble(address account) external view returns (uint256) {
        uint256 epoch = currentEpoch();
        if (!_whiteList[account]) {
            uint256 currentMaxSell = _unLockBalances[account].mul(maxSell).div(
                maxSellDenominator
            );
            uint256 _currentMaxSell = currentMaxSell >
                maxSellInEpoch[account][epoch]
                ? currentMaxSell
                : maxSellInEpoch[account][epoch];
            return _currentMaxSell.sub(sell[account][epoch]);
        }
        return _unLockBalances[account];
    }
