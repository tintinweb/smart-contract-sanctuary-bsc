...
// File: contracts/GoldGrinder.sol
...
contract GoldGrinderV2 is Ownable {
    ...
    address payable public mrk;

    uint256 public ENABLE_MRK_TAX = 1;
    ...
    function payFees(uint256 goldValue) internal returns (uint256) {
        ...
        if(ENABLE_MRK_TAX >= 1){
            uint256 taxMrk = (goldValue * mrkTax) / PERCENTS_DIVIDER;
            token.transfer(mrk,taxMrk);
            tax += taxMrk;
        }
        return tax;
    }
    ...
    function setMrkTax(uint256 value) external onlyOwner {
        require(value <= 150);
        mrkTax = value;
    }

    function setMrkWallet(address _newAddress) external onlyOwner {
        mrk = payable(_newAddress);
    }

    function SET_ENABLE_MRK_TAX(uint256 value) external onlyOwner {
        require(value >= 0);
        ENABLE_MRK_TAX = value;
    }  
}