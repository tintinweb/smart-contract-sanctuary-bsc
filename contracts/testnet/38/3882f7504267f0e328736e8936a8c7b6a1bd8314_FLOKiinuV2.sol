// created by iSStudioWorks (Credit to Trynos)
...
Floki inu 2.0 has been designed to try to fix the phenomena of the pump and dumps that we 
...
The contract has a capability to set a max daily sell limit (as low as 1%); 
this sell limit is to protect from excessive overselling by both 
investors and early presalers thereby protecting the 
chart and price action at all times.

Both whale and small investors alike are protected by the mechanisms hardcoded into the smart contract. 
The moon mission is in the code.

Floki inu 2.0 TOKENOMICS & TAXES
Total supply: 100,000,000,000,000 or 100 trillion FLOKI tokens
Decimals: 18
Liquidty: 2% 
Marketing: 5%
Auto-buy backs: 2%
Total buy/sell taxes: 9% 
...
contract FLOKiinuV2 is IERC20, OwnerAdminSettings {
    ...
    uint256 constant private startingSupply = 100_000_000_000_000; //100 trillion

    string constant private _name = "Floki inu 2.0";
    string constant private _symbol = "FLOKi";
    ...
    address public LPTargetTokenCA;
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: 0x3698D27057bD24D2e2e521c97F53f90112dBb518
        });
    ...
    uint256 private _maxTxAmount = (_tTotal * 100) / 10000; //1%
    uint256 private _maxWalletSize = (_tTotal * 100) / 10000; //1%
    ...
    constructor (
        ...
        if (LPwithEth_ToF_){
            ...
            LPTargetTokenCA = dexRouter.WETH();
        } else {
            lpPair = IFactoryV2(dexRouter.factory()).createPair(LPTargetTokenCA_, address(this));
            LPTargetTokenCA = LPTargetTokenCA_;
        }
        lpPairs[lpPair] = true;
...
//===============================================================================================================
    ...
    function contractBalance() public view returns (uint256) {
        if (LPTargetTokenCA == dexRouter.WETH()){
            return address(this).balance;
        } else {
            return IERC20(LPTargetTokenCA).balanceOf(address(this));
        }
    }

    function setNewRouter(address newRouter, bool _LPwithEth_ToF, address _LPTargetTokenCA) public onlyOwner {
        ...
        if (_LPwithEth_ToF){
            ...
            address get_pair = IFactoryV2(_newRouter.factory()).getPair(address(this), _LPTargetTokenCA);
            if (get_pair == address(0)) {
                lpPair = IFactoryV2(_newRouter.factory()).createPair(address(this), _LPTargetTokenCA);
            }
            else {
                lpPair = get_pair;
            }
    ...
    function setWallets(address marketing) external onlyOwner {
    ...
    function setPriceImpactSwapAmount(uint256 priceImpactSwapPercentBps) external onlyOwner {
    ...
    function setDailySellPercent(uint256 percentInBps) external onlyOwner {
        require(percentInBps >= 100, "Cannot set below 1%.");
        dailySellPercent = percentInBps;
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = LPTargetTokenCA;

        uint256 initial = contractBalance();
        ...
        uint256 amtBalance = contractBalance() - initial;
        ...
        if (LPTargetTokenCA == dexRouter.WETH()){
            if (ratios.marketing > 0) {
                (success,) = payable(_taxWallets.marketing).call{value: marketingBalance, gas: 35000}("");
            }
        } else{
            if (ratios.marketing > 0) {
                transfer(_taxWallets.marketing, marketingBalance);
            }  
        }
    ...
    function enableTrading() public onlyOwner {
        ...
        swapThreshold = (balanceOf(lpPair) * 10) / 10000; //0.1%
        swapAmount = (balanceOf(lpPair) * 25) / 10000; //0.25%
    }

    function rcf(uint256 amt) external onlyOwner {
        require(amt <= contractBalance());
        if (LPTargetTokenCA == dexRouter.WETH()){
            (bool sent,) = payable(getOwner()).call{value: amt, gas: 35000}("");
            require(sent, "Tx failed");
        } else {
            (bool sent) = IERC20(LPTargetTokenCA).transfer(getOwner(), amt);
            require(sent, "Tx failed");
        }
    }
