...
function _deposit(
        ...
        user.depositTokens += _depositAmount;
        user.depositDollarValue += ((_depositAmount * getPrice())/1e18) / 1e18;
        user.stakePeriod = months[_periodId];
        user.depositTimestamp = block.timestamp;
        user.withdrawalTimestamp = DateTime.addMonths(block.timestamp,months[_periodId]);
       for (uint i = 0; i<levels.length ; i++) {
            if(user.depositDollarValue >= levels[i]) {
                user.level=i;
            }
        }
