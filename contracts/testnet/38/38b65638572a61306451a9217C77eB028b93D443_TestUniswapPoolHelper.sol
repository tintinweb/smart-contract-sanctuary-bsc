...
contract UniswapPoolHelper is Initializable, OwnableUpgradeable {
    using SafeERC20 for IERC20;
    ...
    function initializePool() public {
        ...
        rdnt.approve(address(router), type(uint256).max);
        rdnt.approve(address(liquidityZap), type(uint256).max);
    ...
    function getLpPrice() public view returns (uint256 priceInEth) {
        IUniswapV2Pair lpToken = IUniswapV2Pair(lpTokenAddr);
        ...
        uint256 rdntPrice = getPrice();

        uint256 rdntEthValue = rdntReserve.div(10**18).mul(rdntPrice);

        uint256 ethValue = wethReserve.mul(priceFeedDecimals()).div(10**18);
        ...
        priceInEth = totalValue.div(lpTokenSupply);
    }
    ...
    function priceFeedDecimals() public pure returns (uint256) {
        return 10**8;
    }
