...
contract INOMarketV2 is INOAccount, AccessCheck{
    ...
    function buy(address nftContract, uint256 tokenId) external payable {
        ...
        address _saller = nft.ownerOf(tokenId);
        require(_saller !=  msg.sender, "INO: The NFT already is yours");

        nft.safeTransferFrom(_saller, msg.sender, tokenId);
        ...
        lastDealPrice[nftContract][tokenId] = tradeAmount; // 
    ...
    function reAuction(address nftContract, uint256 tokenId, uint256 price, address acontract, uint256 amount ) external payable {
        ...
        if (amount > 0) {
            //require(airdropContract[nftContract][tokenId] != address(0), "INO: Bind ERC20 Contract is required.") ;
