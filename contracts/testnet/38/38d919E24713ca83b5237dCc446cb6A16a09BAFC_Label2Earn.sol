...
contract Label2Earn is IBEP20, Auth {
    ...
    address public REWARD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; // 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56 
    address public PANCAKE_ROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    uint256 private totalFeeBuy = 80;
    ...
    bool private isSendingReward = false;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {																		  
        ...
        if (recipient != pair && sender != address(this) && recipient != DEAD && recipient != owner && recipient != marketingFeeReceiver) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount < _maxWalletSize, "Transfer amount exceeds the bag size.");
        }
        uint256 updatedTime = competitionLastRewarded;
        if(competitionLastRewarded == 0 && (sender == pair || recipient == pair)){
            competitionLastRewarded = block.timestamp.add(competitionRewardTime);
            updatedTime = block.timestamp.add(competitionRewardTime);
    ...
    function shouldSendReward() internal view returns (bool) {
        return (msg.sender != pair || competitionRewardToken == address(this))
        && !inSwap
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setFees(uint256 _liquidityFeeSell,  uint256 _marketingFeeSell, uint256 _burnFeeSell , uint256 _transferFee , uint256 _liquidityFeeBuy,  uint256 _marketingFeeBuy, uint256 _burnFeeBuy , uint256 _competitionFee) external  onlyOwner {
        require(_liquidityFeeSell.add(_marketingFeeSell).add(_burnFeeSell) <= 250 , "maximum sell total fee is 25");
        require(_liquidityFeeBuy.add(_marketingFeeBuy).add(_burnFeeBuy) <= 250 , "maximum buy total fee is 25");        
    ...
    function updateTime() internal returns (uint256){
        ...
        if(dif > competitionRewardTime){
            uint256 roundDelay = dif.div(competitionRewardTime);
            competitionLastRewarded = competitionLastRewarded + (competitionRewardTime * (roundDelay + 1 ));
            return competitionLastRewarded + (competitionRewardTime * (roundDelay + 1 ));
        }else{
            competitionLastRewarded = competitionLastRewarded + competitionRewardTime;
            return competitionLastRewarded + competitionRewardTime;
        }
    }

    function addLiqManual() external onlyOwner{
