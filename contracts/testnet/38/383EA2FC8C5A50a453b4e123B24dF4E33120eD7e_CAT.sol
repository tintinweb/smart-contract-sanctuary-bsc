...
contract CAT is Ownable, Pausable, ReentrancyGuard {
    ...
    function editBlacklist(address[] calldata account, bool[] calldata _blacklist) external onlyOwner {
        require(account.length == _blacklist.length, "Non-matching length");
        for (uint256 i; i < account.length; i++) {
            blacklist[account[i]] = _blacklist[i];
        }
    }
