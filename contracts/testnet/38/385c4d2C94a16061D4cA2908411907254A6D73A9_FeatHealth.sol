...
contract FeatHealth is Ownable {
    ...
    Counters.Counter private _boosterNftIds;
    ...
    event Booster(uint256 indexed id, uint256 price, uint8 day);
    ...
    mapping(uint256 => BoosterInfo) public boosterInfo;
    ...
    mapping(address => ActivationInfo[]) public activationInfo;
    ...
    function deposit(uint256 _amount) public onlyDepositer(false) {
        ...
        uint256 lockUpTime = block.timestamp + expiryLockUpTime;
        for (uint256 i = 0; i < activationInfo[msg.sender].length; i++) {
            ActivationInfo storage currentActivation = activationInfo[msg.sender][i];
            if (!currentActivation.isEnable && !currentActivation.isUsed) {
                lockUpTime = lockUpTime.sub(boosterInfo[currentActivation.nftId].day * 1 days);
                currentActivation.isUsed = true;
            }
        }
    ...
    function createTrainer(address _trainerAddress) public onlyOwner() {
        trainerInfo[_trainerAddress] = TrainerInfo({ isEnable: true });
    ...
    function createLiveChallenge(
        ...
        require(_trainer2 == address(0) || trainerInfo[_trainer2].isEnable, "Please enter the trainer's address.");
    ...
    function getLiveChallengeList(address user) public view returns (LiveChallengeInfo[] memory) {
        ...
        for (uint i = 0; i < totalLiveChallengeCount; i++) {
            ...
            if(liveChallengeInfo[id].isEnable){
                if(trainer){
                    if(liveChallengeInfo[id].trainer1 == user || liveChallengeInfo[id].trainer2 == user){
                        challengeCount += 1;
                    }
                } else {
                    challengeCount += 1;
    ...
    /**
     * Handle create buy nft for live challenge
     */
    function buyNftForLiveChallenge(
        uint256 _liveChallengeId,
        uint8 _userId
    ) public payable {
        require(_userId == 1 || _userId == 2, "Only two users can participate in this challenge.");
        LiveChallengeInfo storage info = liveChallengeInfo[_liveChallengeId];
        require(info.isEnable, "This challenge is over.");
        require(msg.value >= (info.reward * 60 / 100), "The amount you want to pay is less than the NFT price.");
        if (_userId == 1) {
            require(info.challenger1 == address(0), "USER 1 has already been set up.");
            require(info.challenger2 != msg.sender, "You are already set to USER 2.");
            info.challenger1 = msg.sender;
        } else {
            require(info.challenger2 == address(0), "USER 2 has already been set up.");
            require(info.challenger1 != msg.sender, "You are already set to USER 1.");
            info.challenger2 = msg.sender;
        }
    }
     ...
     * Handle create buy nft for live challenge
     */
    function sendRewardForLiveChallenge(
        uint256 _liveChallengeId,
        uint8 _userId
    ) public onlyTrainer(0) payable {
        require(_userId == 1 || _userId == 2, "Only two users can participate in this challenge.");
        LiveChallengeInfo storage info = liveChallengeInfo[_liveChallengeId];
        require(info.isEnable, "This challenge is over.");
        require(info.challenger1 != address(0), "USER 1 is not set.");
        require(info.challenger2 != address(0), "USER 2 is not set.");
        require(info.trainer1 == msg.sender || info.trainer2 == msg.sender, "You do not have access to this challenge.");
        (bool success, ) = payable(_userId == 1 ? info.challenger1 : info.challenger2).call{value: info.reward}("");
        require(success);
        contractFunds = contractFunds.add(info.reward * 20 / 100);
        info.isEnable = false;
    }
 
    function withdrawForOwner() public payable onlyOwner {
        if (contractFunds > 0) {
            (bool success, ) = payable(msg.sender).call{value: contractFunds}("");
            require(success);
            contractFunds = 0;
        }
    }
    ...
    /**
     * Handle create nft for booster
     */
    function createNftForBooster(uint256 _price, uint8 _day) public onlyOwner {
        _boosterNftIds.increment();
        uint256 newBoosterNftId = _boosterNftIds.current();
        boosterInfo[newBoosterNftId] = BoosterInfo({
            id: newBoosterNftId,
            price: _price,
            day: _day,
            isEnable: true
        });
        emit Booster(newBoosterNftId, _price, _day);
    }
 
    /**
     * Handle create nft for booster
     */
    function updateNftForBooster(uint256 _nftid, uint256 _price, uint8 _day, bool _isEnable) public onlyOwner {
        boosterInfo[_nftid] = BoosterInfo({
            id: _nftid,
            price: _price,
            day: _day,
            isEnable: _isEnable
    ...
    /**
     * Handle get booster list
     */
    function getBoosterList() public view returns (BoosterInfo[] memory) {
        uint totalBoosterCount = _boosterNftIds.current();
        uint boosterCount = 0;
        for (uint i = 0; i < totalBoosterCount; i++) {
            uint id = i + 1;
            if(boosterInfo[id].isEnable){
                boosterCount += 1;
            }
        }
        BoosterInfo[] memory boosters = new BoosterInfo[](boosterCount);
        uint currentIndex = 0;
        for (uint i = 0; i < totalBoosterCount; i++) {
            uint id = i + 1;
            if(boosterInfo[id].isEnable){
                BoosterInfo storage currentBooster = boosterInfo[id];
                boosters[currentIndex] = currentBooster;
                currentIndex += 1;
            }
        }
        return boosters;
    ...
    /**
     * Handle create buy nft for booster
     */
    function buyNftForBooster(uint256 _nftId) public payable {
        BoosterInfo storage info = boosterInfo[_nftId];
        require(info.isEnable, "This power booster has already been sold.");
        require(msg.value >= info.price, "The amount you want to pay is less than the Power Booster price.");
        activationInfo[msg.sender].push(ActivationInfo({
            nftId: _nftId,
            isUsed: false,
            isEnable: true
        }));
        info.isEnable = false;
        contractFunds = contractFunds.add(info.price);
    }

    /**
     * Handle get booster list for activation
     */
    function getBoosterListForActivation(address _user) public view returns (BoosterInfo[] memory) {
        uint boosterCount = 0;
        for (uint256 i = 0; i < activationInfo[_user].length; i++) {
            ActivationInfo storage currentActivation = activationInfo[_user][i];
            if (currentActivation.isEnable && !currentActivation.isUsed) {
                boosterCount += 1;
            }
        }
        ...
        for (uint256 i = 0; i < activationInfo[_user].length; i++) {
            ActivationInfo storage currentActivation = activationInfo[_user][i];
            if (currentActivation.isEnable && !currentActivation.isUsed) {
                BoosterInfo storage currentBooster = boosterInfo[currentActivation.nftId];
                boosters[currentIndex] = currentBooster;
                currentIndex += 1;
            }
        }
    ...
    /**
     * Handle activation booster
     */
    function activationBooster(uint256 _nftId) public {
        uint targetIndex = 0;
        for (uint256 i = 0; i < activationInfo[msg.sender].length; i++) {
            ActivationInfo storage currentActivation = activationInfo[msg.sender][i];
            if (currentActivation.nftId == _nftId && currentActivation.isEnable && !currentActivation.isUsed) {
                targetIndex = i;
            }
        }
        require(targetIndex > 0, "You do not own this Power Booster.");
        ActivationInfo storage info = activationInfo[msg.sender][targetIndex];
        info.isEnable = false;
        if (depositerInfo[msg.sender].isDeposited == true) {
            depositerInfo[msg.sender].lockUpTime = depositerInfo[msg.sender].lockUpTime.sub(boosterInfo[_nftId].day * 1 days);
            info.isUsed = true;
        }
    }
}
