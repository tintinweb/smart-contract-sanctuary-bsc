...
contract invite  is Ownable {
    ...
    */
    ...
    function _settlementUsdtV(address _user) internal  view returns(uint,uint,uint){
        uint256 balance = _getBalanceUsdt();
        RewardData storage udata = userReward[_user];
        if (isRankFull()){
            uint256 start = udata.lastToalBalanceUsdt;
            if (start < inviteCenter[batchid].startToalBalanceUsdt){
                start = inviteCenter[batchid].startToalBalanceUsdt;
            }
            uint256 diff = balance.sub(start);
            uint256 per = diff.div(MUSTSUREWEIGHT);
           return (udata.RewardUsdt.add(per.mul(udata.curWeight)),
                balance,
                per.mul(udata.curWeight));
         }
        return (udata.RewardUsdt,
                balance,
                0);
    }
    ...
    function _settlementErc20V(address _user) internal view returns(uint,uint,uint) {
        ...
        RewardData storage udata = userReward[_user];
        if (isRankFull()){
            uint256 start = udata.lastToalBalanceErc20;
            if (start < inviteCenter[batchid].startToalBalanceErc20){
                start = inviteCenter[batchid].startToalBalanceErc20;
            }
            uint256 diff = balance.sub(start);

            uint256 per = diff.div(MUSTSUREWEIGHT);
            return (udata.RewardErc20.add(per.mul(udata.curWeight)),
                    balance,
                    per.mul(udata.curWeight));
        }
        return  (udata.RewardErc20,
                balance,
                0);
    }
    ...
    function settlementV(address _address) public view returns(uint,uint){
        ...
        (uint currUsdt,,) = _settlementUsdtV(_address);
        (uint currToken,,) = _settlementErc20V(_address);
        rewardU = rewardU.add(currUsdt);
        rewardT = rewardT.add(currToken);
