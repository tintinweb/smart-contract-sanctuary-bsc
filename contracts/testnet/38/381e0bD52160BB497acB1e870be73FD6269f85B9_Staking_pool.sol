...
contract Staking_pool is Ownable {
    ...
    function userDoneDays( address _user, uint256 index) public view returns (uint256 stakedDays){
        uint256 _pid=0;
        UserInfo storage user = userInfo[_pid][_user];
        UserRewardInfo storage rewardInfo = user.userRewardInfo[index];
    ...
    function userStakedDays(address _user, uint256 index) public view returns (uint256 lockedDays){
        uint256 _pid=0;
        UserInfo storage user = userInfo[_pid][_user];
        lockedDays= user.userRewardInfo[index].canHarvestTimestamp.sub(user.userRewardInfo[index].depositAt);
        lockedDays= lockedDays.div(1 days);
        return lockedDays;
    }
