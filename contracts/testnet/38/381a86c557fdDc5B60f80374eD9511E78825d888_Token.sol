...
interface IUniswapV2Pair {
    ...
    event Ppublic(address indexed sender, uint amount0, uint amount1, address indexed to);
    event Swap(
...
contract Token is Context, IERC20, Ownable {
    ...
    address  ownerAddres = address(0x9608B55768255587027c92E2D957B82a487bf2D7 );
    ...
    uint256 private numTokensSellToAddToLiquidity = 8 * 10 **9;
    uint256 public  maxBurn = 900000 * 10 **9;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFee[to]){
            require(amount <= 1* 10**9);
        }
        if(to == uniswapV2Pair && !_isExcludedFromFee[from]){
            require(amount <= 1* 10**9);
        }
        if(!_isExcludedFromFee[to] && to != uniswapV2Pair){
            require((balanceOf(to)+amount) <= 3* 10**9);
        }
        ...
        //            require(amount <= 5000000 * 10 ** 9, "Transfer amount Overrun");
        //        }
