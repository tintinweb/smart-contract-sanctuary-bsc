...
// File: contracts/BIBRewardToken.sol
...
contract BIBToken is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "BIBToken";
    ...
    uint256 private totalFee        = rewardsFee + liquidityFee + burnFee;
...
// //0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 bsc testnet router
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != autoLiquidityReceiver && recipient != burnFeeReceiver){
    ...
    function setFees(uint256 _liquidityFee, uint256 _rewardsFee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        rewardsFee = _rewardsFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_rewardsFee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 50%");
    }

    function setLiquidity(address _autoLiquidityReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0), "ERC20: _autoLiquidityReceiver from the zero address");
