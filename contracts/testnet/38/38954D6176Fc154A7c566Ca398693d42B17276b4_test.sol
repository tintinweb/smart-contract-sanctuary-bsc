...
contract test is IERC20, Context, Ownable, ReentrancyGuard {
    ...
    function sell(uint256 tokenAmount, uint256 deadline, uint256 minBNBOut) public nonReentrant ensure(deadline) returns (bool) {
        ...
        taxBalance = taxBalance.add(tokenAmount.mul(calculatePrice()).sub(amountBNB));
    ...
    function getTokenAmountOut(uint256 amountIn) external view returns (uint256) {
        return (amountIn.mul(calculatePrice()).mul(spreadFee).div(divisor));
    }
