    ...
    ERC721Burnable
{
    ...
    ) ERC721("GroundHog Mining", "GROUNDHOG") {
        receiver = _receiver;
    ...
    function addNFT(uint _fee, string memory _id, string memory _uri, uint _percentage) public onlyOwner {
        NFT memory nft = NFT(_id, _fee , _uri, _percentage);
        nfts[_id] = nft;
    }
...
function mintNFT(IERC20 _usdcContract, string memory _id) public payable {
   ...
   _usdcContract.transferFrom(_msgSender(), receiver, fee );
        internalMint(msg.sender, nft.uri, nft.percentage, fee, _id);
...
function multisender(address[] memory _receiver, uint256[] memory _amount, IERC20 _usdcContract) onlyOwner public {
    ...
    require(_usdcContract.allowance(msg.sender,address(this)) >= totalAmount, "Spending not allowed by owner");
    ...
    for(uint256 i; i < _receiver.length; i++){
        _usdcContract.transferFrom(msg.sender, _receiver[i], _amount[i] * 10 ** 18);
    }
...
function setURI(uint256 tokenId, string memory uri) external onlyOwner {
    ...
    //Overrides
     ...
     function internalMint(address to, string memory _uri, uint _percentage, uint _fee, string memory _id) internal {
