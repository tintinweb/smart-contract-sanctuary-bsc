...
interface IAgentContribution{
  function depositAgent(address token,address payable user,uint256 amount)external payable;
}
...
contract AgentContribution is ReentrancyGuard,IAgentContribution,Referral,Pausable{
  ...
  function randomAgentName(uint256 index,address _user,address _upline) public view returns (string memory){
    return (uint256(keccak256(abi.encodePacked(index,_user,_upline)))%(2**20)).toHexString(4);
  }
  ...
  function depositAgent(address token,address payable referrer,uint256 amount)public override payable {
    ...
    if(status==true){
      ...
      string memory randomID = (uint256(keccak256(abi.encodePacked(id,msg.sender,referrer)))%(2**20)).toHexString(4);
