...
contract SERSHVestingEscrow is ISERSHVestingEscrow, Ownable, ReentrancyGuard {
    ...
    modifier onlyOwnerOrBuyer() {
        require(
            owner() == _msgSender() || _msgSender() == _buyer,
    ...
    modifier onlyBuyer() {
        require(_msgSender() == _buyer, "Only Buyer can call this function.");
        _;
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    event Unvested(address indexed vesting, uint256 indexed when);
    ...
    function deployVestingContract(
        ...
        require(
            amount >= _minVestingAmount && amount <= _maxVestingAmount,
            "Invalid amount for vesting"
        );
        require(
            msg.sender.isContract() == false,
            "Only vesting is available from admin."
        );
        ...
        require(
            subWallet != address(0),
            "Invalid subwallet for vesting category"
        );
        require(
            IERC20(_vestingToken).balanceOf(subWallet) >= amount,
            "Insufficient balance of subwallet for vesting."
        );
