...
contract Ownable {
    ...
    function lock(uint256 time) public onlyOwner {
        ...
        _owner = address(0);
        _lockTime = now + time;// OverFlow Possible here.
        emit OwnershipTransferred(_owner, address(0));
    }
    ...
    function unlock() public {
        ...
        require(now > _lockTime, "Contract is locked until 7 days");// Why 7 days??
        emit OwnershipTransferred(_owner, _previousOwner);
        _owner = _previousOwner;
    }
}

contract OSK is Ownable, ERC20, ERC20Detailed {
    using SafeMath for uint256;

    mapping (address => bool) internal whiteList;
    uint MAXHOLD;

    constructor () public ERC20Detailed("OSK", "OSK", 18) {
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        if(!isWhite(recipient)) require(_balances[recipient] <= MAXHOLD, "hold overflow");
