...
contract YieldzToken is ERC20, Ownable {
    ...
    event SetUSDTAddress(address usdtAddress);
    ...
    constructor(address _treasury, uint256 _buyFee, uint256 _sellFee, uint256 _p2pFee) ERC20("Yieldz", "YZ") {
    ...
    function createBond(uint256 period) external {
        ...
        if (period == 10) {
            ...
            IERC20(usdtAddress).transferFrom(msg.sender, address(this), tenBondTotal );
            IERC20(usdtAddress).transfer(treasury, tenBondTotal );
        } else {
            rate_ = twentyBondTotal * 10**18  / twentyBondPrice / 20 days;
            IERC20(usdtAddress).transferFrom(msg.sender, address(this), twentyBondTotal );
            IERC20(usdtAddress).transfer(treasury, twentyBondTotal );
        }
    ...
    function claimable(uint256 _count) external view returns(bool) {
        BondData memory bond = userBond[msg.sender][_count];
    ...
    function setTreasuryAddress(address _treasury) public onlyOwner {
     ...
     function setUSDTAddress(address _usdtAddress) public onlyOwner {
        ...
        emit SetUSDTAddress(usdtAddress);
    }
