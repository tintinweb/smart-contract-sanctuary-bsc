...
library Address{
    ...
    contract MuchiInu is IERC20, Context, Ownable {
    ...
    address private Muchi__Marketing_Wallet = 0xfF5D53EE565912A483021d7b2aDcA61120d1cc2B;
    address private Muchi__Team_Wallet = 0xC2A4AaE159189dC19b6B860dafD0E16Df7DDB87e;
    
    string private constant _symbol = "MUCHI";
    string private constant _name = "Muchi Inu";

    uint256 public Muchi__max_BuyLimit = 100000000 * 10**9;
    uint256 public Muchi__max_SellLimit = 50000000 * 10**9;

    uint256 public Muchi__max_WalletLimit = 100000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Muchi__Marketing_Wallet] = true;
        _isExcludedFromFee[Muchi__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Muchi__Marketing_Wallet] = true;
        allowedTransfer[Muchi__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Muchi__max_BuyLimit, "You are exceeding Muchi__max_BuyLimit");
            require(balanceOf(to) + amount <= Muchi__max_WalletLimit, "You are exceeding Muchi__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Muchi__max_SellLimit, "You are exceeding Muchi__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Muchi__max_WalletLimit, "You are exceeding Muchi__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Muchi__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Muchi__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Muchi__Update_MarketingWallet(address newWallet) external onlyOwner{
        Muchi__Marketing_Wallet = newWallet;
    }
    
    function Muchi__Update_TeamWallet(address newWallet) external onlyOwner{
        Muchi__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Muchi__max_BuyLimit = maxBuy * 10**decimals();
        Muchi__max_SellLimit = maxSell * 10**decimals();
    }
    
    function Muchi__Max_Walletlimit(uint256 amount) external onlyOwner{
        Muchi__max_WalletLimit = amount * 10**decimals();
    }
