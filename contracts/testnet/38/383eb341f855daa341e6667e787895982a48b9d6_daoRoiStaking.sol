...
contract daoRoiStaking is Ownable{
    ...
    constructor() 
    {
        Token = IERC20(0x8eF324331Ed7B2e13E4da272efECcB5b4b3952AF);
    ...
    function Stake(uint256 amount) public {
        ...
        (bool _isAvailable,) = checkAvailability(msg.sender);
        if(!_isAvailable){
    ...
    function distribute(uint256 amount) public onlyOwner{
        ...
        uint256 forEach = getForEach(amount);
        for(uint256 i; i< users.length; i++)
        {       userInfo[users[i]].reward = ((userInfo[users[i]].stakedAmount).mul(forEach)).div(1E18);     }
        lastDistribute = block.timestamp;
    }

    function getForEach(uint256 amount) private view returns(uint256 userReward){
    ...
    function checkAvailability(address _address) private view returns(bool status,uint256 value)
    {
