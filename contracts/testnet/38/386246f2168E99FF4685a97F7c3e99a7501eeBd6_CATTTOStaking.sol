...
contract CATTTOStaking is Ownable {
    ...
    uint256 public LockForTime = 7;
    uint256 public fee1 = 2;
    
    uint256 private totalfee = fee1;
    ...
    //Emergency FixLock
    function FixLock (address Fixxed, uint256 NewTime ) public onlyOwner {
     UserInfo storage user = users[Fixxed];
     user.TimeLock = NewTime;
      
    }
    //For Set TimeLock for staking

    function updateTimeLock(uint256 _newtime) public onlyOwner {
        LockForTime = _newtime;
    }
    ...
    external {
        ...
        if (user.deposited > 0 && user.TimeLock == 7) {
            ...
            user.TotalTimeLock = block.timestamp + (LockForTime*3600*24);
            user.Remaing = user.TotalTimeLock - block.timestamp;
    ...
    */
    ...
    external view returns (uint256 _rewardPerSecond, uint256 _secondsLeft, uint256 _deposited, uint256 _pending, uint256 _expire ) {
        UserInfo storage user = users[_user];
        if (block.timestamp < rewardPeriodEndTimestamp) {
        ...
        _expire = users[msg.sender].Remaing;
