...
contract MUMUNFT is ERC721 {
    ...
    uint256 public constant NFT_TotalSupply = 1000000;

    uint256 public NFT_Id = 100000;
    uint256[] public nftPowerPrice = [20, 100, 300, 500, 1000];
    ...
    //升级合约
    function setNewAddress(address _v2Address) public {
    ...
    function turnDrop(bool _switch) public {
        require(contractOwner == msg.sender);
        airdropSwitch = _switch;
    }
    ...
    function getMumu(uint256 nftId) view public returns (
        address origin,
        string memory IphsHash,
        uint256 power,
        uint256 price,
        bool official,
        bool is_sale
    ){
        mumuStruct memory kit = mumu[nftId];
        origin = address(kit.origin);
        IphsHash = kit.IphsHash;
        power = uint256(kit.power);
        price = uint256(kit.price);
        official = bool(kit.official);
        is_sale = bool(kit.is_sale);
    ...
    function airdrop(uint256[] memory _id, string[] memory _hash, uint256[] memory _power, address[] memory _to, address[] memory _origin) public {
        require(devOwner[msg.sender] == true, ' not allow');
    ...
    function transfer(address to, uint256 tokenId) external payable returns (bool) {
        require(canTransfer == false, "transfer is paused");
    ...
    function canTransferNFT(bool _switch) public {
        require(contractOwner == msg.sender);
        canTransfer = _switch;
    }
    ...
    function burn(uint256 Id) external {
        ...
        require(msg.sender == owner || msg.sender == tokenApprovals[Id] || approvalForAlls[owner][msg.sender],
    ...
    function officialMint(string memory _hash, uint256 _power, uint256 price) public returns (uint256){
        //官方创建
        require(devOwner[msg.sender] == true, ' not allow msg sender');
    ...
    event UserMint(address indexed origin, address indexed owner, string iphshash, uint256 power, uint256 NFTID, uint256 random);
    ...
    function getUsdtBalance(address _address) view public returns (uint256){
        return usdt.balanceOf(_address);
    }

    function getNftPowerPrice(uint256 _boxType) view public returns (uint256){
        return nftPowerPrice[_boxType - 1].mul(1e18);
    }
    ...
    function endBidders(uint256 _nftId) public {
        require(mumu[_nftId].origin == msg.sender, 'your not the nft owner');
        if (bidder[_nftId].is_first == true) {
            mumu[_nftId].is_sale = true;
            bidder[_nftId].grant = true;
        } else {
