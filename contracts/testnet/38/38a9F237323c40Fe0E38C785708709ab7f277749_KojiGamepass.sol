...
contract KojiGamepass is ERC721URIStorage, ERC165, Ownable, ReentrancyGuard {
    ...
    function viewHashes(address _address, string memory _holder) public pure returns (bytes32, bytes32) {

        bytes32 first = keccak256(bytes(toString(_address)));

        bytes32 second = keccak256(bytes(toString(_holder)));
        
        return(first,second);
    }

    function compareHashes(address _address, string memory _holder) public pure returns (bool) {
        (bytes32 first, bytes32 second) = viewHashes(_address, _holder);
        return(first == second);
    }


    function viewBytes(address _address, string memory _holder) external pure returns (bytes memory, bytes memory) {
        return (bytes(Strings.toHexString(uint160(_address), 20)), bytes(_toLower(_holder)));
    }

    function viewABI(address _address, string memory _holder) external pure returns (string memory, string memory) {
        return (toString(_address), toString(_holder));
    }
    ...
    function toString(string memory _string) public pure returns(string memory) {
        return string(toString(abi.encodePacked(_string)));
    }
    ...
    function toAsciiString(address x) public pure returns (string memory, bytes memory) {
        ...
        return (string(abi.encodePacked("0x",s)), bytes(string(abi.encodePacked("0x",s))));
    }
    ...
    function viewAddressString(address _address) external pure returns (string memory, uint, bytes memory, bytes memory) {
        string memory strung = Strings.toHexString(uint160(_address), 20);
        uint256 i = uint256(uint160(address(_address)));

        return(strung, i, bytes(strung), bytes(bytesToString(bytes(strung))));
    }
    ...
    function addressToString(address _pool) public pure returns (string memory _uintAsString) {
      uint _i = uint256(uint160(address(_pool)));
      if (_i == 0) {
          return "0";
      }
      uint j = _i;
