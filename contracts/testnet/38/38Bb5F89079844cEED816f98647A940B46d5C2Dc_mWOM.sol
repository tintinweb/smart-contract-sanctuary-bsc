...
contract mWOM is ERC20 {
    using SafeERC20 for IERC20;

    /* ============ State Variables ============ */

    address public immutable wombatStakikng;
    ...
    constructor(address _wombatStakikng, address _wom) ERC20("mWOM", "mWOM") {
        wombatStakikng = _wombatStakikng;
        wom = _wom;
        totalConverted = 0;
        totalAccumulated = 0;
    }
    ...
    function convert(uint256 _amount) external {
        IERC20(wom).safeTransferFrom(msg.sender, wombatStakikng, _amount);

        uint256 mintedVeWomAmount = IWombatStaking(wombatStakikng).convertWOM(_amount);
        _mint(msg.sender, mintedVeWomAmount);
        totalConverted = totalConverted + _amount;
        totalAccumulated = totalAccumulated + mintedVeWomAmount;
        emit mWomMinted(msg.sender, mintedVeWomAmount);
    }
}
