...
contract proofTokenFactoryBSC is Ownable {
    ...
    constructor(address initialRouterAddress, address initialLockerAddress,uint256 initialBurnThreshold,address initialRevenueAddress) {
    ...
    function createToken(string memory tokenName, string memory tokenSymbol, uint256 initialSupply,
        ...
        TokenCutterBSC newToken = new TokenCutterBSC(tokenName, tokenSymbol, initialSupply, msg.sender, routerAddress, hldAdmin, fees);
        emit TokenCreated(address(newToken));

        //add liquidity
        newToken.approve(routerAddress, type(uint256).max);        
        IUniswapV2Router02 router = IUniswapV2Router02(routerAddress);
        router.addLiquidityETH{ value: msg.value }(address(newToken), newToken.balanceOf(address(this)), 0,0, address(this), block.timestamp+111);
    ...
    function finalizeToken(address tokenAddress) public payable {
        ...
        //enable trading
        ITokenCutterBSC(tokenAddress).swapTradingStatus(); 
        ITokenCutterBSC(tokenAddress).setLaunchedAt();
    ...
    function cancelToken(address tokenAddress) public {
        ...
        ITokenCutterBSC(tokenAddress).cancelToken();
