...
contract spex_stake is Initializable {
    ...
    mapping(uint8 =>uint256) internal totalInvested;
    mapping(uint8=>uint256) internal totalReward;
    ...
    function initialize(address ownerAddress, IBEP20 _SPEXToken) public
        initializer {
            owner = ownerAddress;
            BTGtoken = _SPEXToken;
            plans[1].time = 9 minutes;
            plans[1].percent = 10;
            plans[2].time = 18 minutes;
            plans[2].percent = 20;
            plans[3].time = 25 minutes;
            plans[3].percent = 30;
    ...
    function invest(uint256 amount , uint8 _plan) external {
        ...
        users[msg.sender].deposits.push(Deposit(_plan,amount, block.timestamp,block.timestamp.add(plans[_plan].time),false,false));
