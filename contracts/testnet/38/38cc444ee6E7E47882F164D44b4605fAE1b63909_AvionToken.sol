...
contract AvionToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function initialize(address usdcAddress, address _router, address _avionNFT) public initializer {

        __ERC20_init("Avion", "AVI");
        ...
        if (_router != address(0x0)) {
            ...
            address pairBusd = IPancakeFactory(router.factory()).createPair(
                address(this),
                busdToken
            );
        ...
        teamReceiver = 0xAf47725C293452Ade77770Bfb6BD2680564DA157;
    ...
    function swapBack() internal swapping {
        ...
        uint256 gonsContract = getYield(address(this));
        ...
        if (amountToBurn > 0) {
            _gonBalances[address(this)] -= amountToBurn.mul(gonsContract);
            _gonBalances[ZERO] += amountToBurn.mul(gonsContract);
        }
    ...
    function updateAvailableAmount (address sender, uint256 amount) internal {
        ...
        if (block.timestamp - usersInfo[sender].lastTimeSold >= 86400 && block.timestamp - usersInfo[sender].lastTimeSold < 86400 * 7) {
        ...
        } else if (block.timestamp - usersInfo[sender].lastTimeSold >= 86400 * 7) {
            usersInfo[sender].amountAvailable = (_gonBalances[sender].div(yieldSender) * percentageForMoreThanSevenDays) / 1000;
            usersInfo[sender].lastTimeSold = block.timestamp;
        }

        require(amount.div(yieldSender) <=  usersInfo[sender].amountAvailable, "Avion: unavailable amount");
