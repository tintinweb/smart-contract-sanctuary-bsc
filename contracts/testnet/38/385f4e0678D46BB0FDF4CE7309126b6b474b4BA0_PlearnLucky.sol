...
contract PlearnLucky is IPlearnLucky, ReentrancyGuard, Ownable {
     ...
     */
    constructor(address _plearnTokenAddress, address _randomGeneratorAddress) {
        // Initializes values
    ...
    function buyTickets(uint256 _luckyId, uint32 _numberTickets) public notContract nonReentrant {
        ...
        require((_getCountTicketsInRound(_luckyId) + _numberTickets) <= luckyInfo.maxTicketsPerRound, 'Ticket not enough');

        // Calculate number of Plearn to this contract
        uint256 amountPlearnToTransfer = calculateTotalPriceForTickets(
            luckyInfo.priceTicketInPlearn,
            _numberTickets
        );
        ...
        luckyInfo.firstTicketNumberNextLucky = _currentTicketNumber;
...
interface IPlearnLucky {
    ...
    function getCountTicketsInRound(uint256 _luckyId) external returns (uint32);
