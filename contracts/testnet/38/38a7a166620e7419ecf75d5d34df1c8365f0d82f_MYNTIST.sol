...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    /*  ShareRateChange   (auto-generated event)
        ...
        uint40   indexed  stakeId
    */
    event ShareRateChange(uint256 data0, uint40 indexed stakeId,uint256 shareRate);
    ...
    uint256 internal constant LAUNCH_TIME = 1668582000;
 ...
 // Signature struct
  ...
  mapping(bytes32 => bool) public executed;
    ...
    function setAddressesForContractSupplies(address nftAAContractAddress,address miniAAContractAddress) external onlyOwner {
    ...
    function mintAmountForNftAA(address _to, uint256 _amount) public onlyNFTMinter {
...
contract SignatureVerification is StakeableToken {
    ...
    bytes32 messageHash = keccak256(abi.encodePacked(header, _to, _amount, _nounce));
...
contract UTXORedeemableToken is SignatureVerification {
    ...
    function btcAddressClaim(
       ...
       {
        bytes32 sigHash = keccak256(abi.encodePacked(nounce, signature.v, signature.r, signature.s));
        require(!executed[sigHash], "Signature expired");
        executed[sigHash] = true;
        bool signaturesChecked = false;

        if (claimMessageMatchesSignature(msg.sender, rawSatoshis, signature, nounce) == owner) {
