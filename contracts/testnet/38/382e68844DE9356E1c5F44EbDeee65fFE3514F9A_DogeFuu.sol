...
contract DogeFuu is ERC20Detailed, Ownable {
        ...
        Ownable()
    {
        ...
        mgmController = MGMController(mgmAddr);
    ...
    function setMgmController(address addr) external onlyOwner {
        ...
        mgmController = MGMController(addr);
    }
...
contract MGMController is Ownable {
    ...
    uint256 public totalMgmRealised;
    address token;
    ...
    function addMgmAmount(
        ...
        totalMgmAmount = totalMgmAmount.add(amount.sub(payoutAmount));
    }
    ...
    function claim() external {
        ...
        profiles[msg.sender].totalRealised = profiles[msg.sender]
            .totalRealised
            .add(claimAmount);
        totalMgmRealised = totalMgmRealised.add(claimAmount);
    ...
    function withdrawToTreasury() external onlyOwner {
        uint256 swapAmount = IERC20(token).balanceOf(address(this)).sub(
            totalMgmAmount.sub(totalMgmRealised)
        );
        require(
            swapAmount > 0,
            "There is no token deposited in token contract"
