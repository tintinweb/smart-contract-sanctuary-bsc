...
abstract contract JackPot is Context {

  uint256 public currentJackpotEpoch;
  uint256 public currentDailyJackpotEpoch;
  // uint256 public currentWeeklyJackpotEpoch;
  // uint256 public currentMonthlyJackpotEpoch;
  
  bool public jackpotDailyStart = false;
  // bool public jackpotWeeklyStart = false;
  // bool public jackpotMonthlyStart = false;


  uint256 public jackPotTicketCost;
  ...
  mapping(uint256 => Jackpot) public DailyJackPots;
  mapping(uint256 => address[]) public DailyTickets;
  mapping(uint256 => address[]) public DailyUsersInJackpot;
  mapping(uint256 => mapping(address => JackPotWinner)) public JackPotWinners;
  mapping(address => uint256) public JackpotUserBetAmount;
  // mapping(uint256 => mapping(address => JackPotWinner)) public JackPotWinners;
  
  uint256[] public DailyDistribution = [40,30,20,10,1,1,1,1];


  // mapping(address => uint256[]) public DailyTickets;

  // mapping(uint256 => Jackpot) public WeeklyJackPot;
  // mapping(address => uint256[]) public WeeklyTickets;

  // mapping(uint256 => Jackpot) public MonthlyJackPot;
  // mapping(address => uint256[]) public MonthlyTickets;

  // uint256 public currentWeeklyJackpot;
  // uint256 public currentMonthlyJackpot;

  function dailyJackpotStart() internal {
    require(!jackpotDailyStart, "Can only run daily jackpot start once");
    currentJackpotEpoch += 1;
    currentDailyJackpotEpoch += currentJackpotEpoch;
    jackpotDailyStart = true;
    _safeDailyJackpotStart(currentDailyJackpotEpoch);

  }

  function _safeDailyJackpotStart(uint256 epoch) internal {
    uint32 timestamp = uint32(block.timestamp);

    Jackpot storage jackpot = DailyJackPots[epoch];

    jackpot.jackpotType = JackPotType.Daily;
    jackpot.startTimestamp = timestamp;
  }

  function DailyJackpotLock() internal {
    require(jackpotDailyStart, "Jackpot is not started!");

    _safeDailyJackpotLock(currentDailyJackpotEpoch);
    currentJackpotEpoch += 1;
    currentDailyJackpotEpoch += currentJackpotEpoch;
    _safeDailyJackpotStart(currentDailyJackpotEpoch);
  }

  function _safeDailyJackpotLock(uint256 epoch) internal {
    Jackpot storage jackpot = DailyJackPots[epoch];
    jackpot.lockTimestamp = uint32(block.timestamp);
  }

  function _safeDailyJackpotDraw(uint256 epoch) internal {
    require(DailyJackPots[epoch].lockTimestamp == 0, "Not possible sort winner");

    Jackpot storage jackpot = DailyJackPots[epoch];

    for (uint256 i = 0; i < DailyDistribution.length; i++) {
      uint winnerIndex = _pickWinner(epoch);

      address addressWinner = DailyTickets[epoch][winnerIndex];

      if(!JackPotWinners[epoch][addressWinner].exists){      
        JackPotWinner storage winner = JackPotWinners[epoch][addressWinner];
        winner.amount = jackpot.rewardDistribution[i];
        winner.claimed = false;
        winner.position = i+1;
        winner.exists = true;
        // jackpot.rewardDistribution[i] = DailyTickets[epoch][winnerIndex];
      }else{
        i--;
      }
    }
  ...
  function _pickWinner(uint256 epoch) internal view returns (uint) {

    uint random = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, DailyTickets[epoch])));
    return random % DailyTickets[epoch].length;
    // return index;
  }


  // function _safeDailyJackpotClose() internal {
    
  //   uint32 timestamp = block.timestamp;
  //   if (DailyJackPots[currentJackpotEpoch].startTimestamp != timestamp){
  ...
  function _CheckDailyJackPot(uint256 epoch) internal view returns(bool) {
    return
      DailyJackPots[epoch].startTimestamp != 0 &&
      block.timestamp > DailyJackPots[epoch].startTimestamp;
  }

  
  function _safeJackpotsRewardAdd(uint256 amount) internal {
      Jackpot storage dailyJackpot = DailyJackPots[currentDailyJackpotEpoch];
      dailyJackpot.rewardAmount += amount;
      dailyJackpot.totalUsers = DailyUsersInJackpot[currentDailyJackpotEpoch].length;
  }

  function _safeTicketAdd(address customer, uint256 amount) internal {
    // storage of user bet.
    JackpotUserBetAmount[customer] += amount;

    if (JackpotUserBetAmount[customer] >= jackPotTicketCost){
      uint256 credit;
      uint256 ticketsAmount;

      Jackpot storage jackpot = DailyJackPots[currentDailyJackpotEpoch];

      credit = JackpotUserBetAmount[customer] % jackPotTicketCost;
      ticketsAmount = (JackpotUserBetAmount[customer] - credit) / jackPotTicketCost;
      JackpotUserBetAmount[customer] = credit;

      _safeAddUserInJackpot(currentDailyJackpotEpoch, customer);
      
      for (uint256 i = 0; i < ticketsAmount; i++) {
        DailyTickets[currentDailyJackpotEpoch].push(customer);
        DailyTickets[currentDailyJackpotEpoch].push(customer);
        DailyTickets[currentDailyJackpotEpoch].push(customer);
      }

      jackpot.totalTickets += DailyTickets[currentDailyJackpotEpoch].length;
    }
  }

  function _safeAddUserInJackpot(uint256 epoch, address customer) internal {
    bool addressExist = false;
    for (uint256 i = 0; i < DailyUsersInJackpot[epoch].length; i++) {
      if(DailyUsersInJackpot[epoch][i] == customer){
        addressExist = true;
        break;
      }
    }
    ...
    if(!addressExist){
      DailyUsersInJackpot[epoch].push(customer);
    }
  }
}
...
contract CriptoVersePrediction is Ownable, Pausable, ReentrancyGuard, JackPot {
  ...
  function betBull(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
    ...
    _safeTicketAdd(msg.sender, amount);
