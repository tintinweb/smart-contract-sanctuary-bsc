...
/**
 * SPACE TACO TOKEN
 * Taxes: 8% (2% Dev, 2% Marketing + Buyback, 2% Charity, 2% SRG rewards)
 * Max Wallet: 2% (2'000'000 $TACO)
 * Website: https://SpaceTaco.biz
 * Telegram: https://t.me/SpaceTaco_OG
 ...
 * Please buy $TACO tokens ONLY via https://swap.surgeprotocol.io/ or directly on our BSCscan page.
 * Tutorials on how to buy can be found on our website and Telegram chat.
 * 
 * Charity wallet: https://bscscan.com/address/0x31c81d41ce5e336b56780268913a12fe9d19e6e6
 * Funds collected in this wallet will be used to donate to the Taco Bell Foundation.
 */
...
contract TACO is IERC20, Context, Ownable, ReentrancyGuard {
    event Bought(
    ...
    // token data
    string private constant _name = "STACCCAOSDAadas";
    string private constant _symbol = "MONITOR";
    uint8 private constant _decimals = 9;
    uint256 private constant _DECMULTIPLIER = 10**_decimals;

    //SRG pair data
    address private constant SRG = 0x38FdB4c294cc3165972A1d924BF7217412e0c6ae; //change this according to chain
    ...
    //Tax wallets
    address public teamWallet = 0x31EB21994f676Ae9dEBB57B07Ab912196C5D9388;
    address public treasuryWallet = 0xae727566C74692906ed68D543A6EaE16E04d89ff;
    address public charityWallet = 0x31C81d41ce5e336B56780268913a12fe9D19E6e6;
    ...
    function changeTxLimits(uint256 newLimit, uint256 newMaxTx) external onlyOwner {
        require(
            newLimit >= maxBag && newMaxTx >= maxTX,
    ...
    function changeTaxDistribution(
        ...
        require(
            newteamShare + newtreasuryShare + newCharityShare + newRewardShare ==
                SHAREDIVISOR,
    ...
    function changeFeeReceivers(
        ...
        require(
            newTeamWallet != address(0) && newTreasuryWallet != address(0)&& newCharityWallet != address(0),
    ...
    function withdrawTaxBalance() external nonReentrant onlyOwner {
        uint256 totalShares = teamShare+treasuryShare+charityShare;

        if(teamShare>0){
        bool temp1 = SRGIE.transfer(teamWallet, (taxBalance * teamShare) /totalShares);
        require(temp1,"failed to send funds");
        }

        if(treasuryShare>0){
        bool temp2 = SRGIE.transfer(treasuryWallet, (taxBalance * treasuryShare) / totalShares);
        require(temp2,"failed to send funds");}

        if(charityShare>0){
        bool temp3 = SRGIE.transfer(charityWallet, (taxBalance * charityShare) / totalShares);
        require(temp3,"failed to send funds");}
...
/* 
    ...
    */
    ...
    address private _migration_wallet = 0x31EB21994f676Ae9dEBB57B07Ab912196C5D9388;
