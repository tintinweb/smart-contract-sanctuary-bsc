...
contract Tokam is ERC20, ERC20Permit, ERC20Burnable, Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    function withdrawTokens(uint256 _amount) external {
        for (uint i; i < depositsCount; i++) {
            if (msg.sender == lockedToken[i].withdrawer ) {
                require(msg.sender == lockedToken[i].withdrawer, 'You are not the withdrawer!');
                require(lockedToken[i].deposited, 'Tokens are not yet deposited!');
                require(!lockedToken[i].withdrawn, 'Tokens are already withdrawn!');
                lockedToken[i].withdrawn = true;
                walletTokenBalance[address(lockedToken[i].token)][msg.sender] = walletTokenBalance[address(lockedToken[i].token)][msg.sender].sub(lockedToken[i].amount);
