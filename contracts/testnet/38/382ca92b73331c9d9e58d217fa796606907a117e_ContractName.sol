...
contract ContractName is IBEP20, Authorization {
  ...
  uint256 private _initalFragmentsSupply = 1000000000 * 10**_decimals;
  uint256 private _maxSellPercentage = 2000; //10^5
  uint256 private _numTokensSellToAddToLiquidity = (_initalFragmentsSupply.mul(20)).div(100000); //0.020%
  uint256 private _maxTokensSellToAddToLiquidity = (_initalFragmentsSupply.mul(50)).div(100000); //0.050%
  uint256 private _numTokensSellDistributor = (_initalFragmentsSupply.mul(15)).div(100000); //0.015%
  uint256 private _numTokensSellToSendTaxes = (_initalFragmentsSupply.mul(25)).div(100000); //0.025%

  string private _symbol = "funT"; 
  string private _name = "funkcyTest";

  address public _operationsAddress = (0xF088D4c86B5Fd839676494C9dC607e6b70bdD298);
  address public _stakingFeeAddress = (0xD0FC8f2C1DeA498BdaacD02b52FE625d3f4B6337);
  ...
  constructor() Authorization(msg.sender) {
    ...
    //FEE SETUP
    _taxMatrix["longTerm"] = (TaxFees(75, 150, 75, 300, 0, 200));
    _taxMatrix["midTerm"] = (TaxFees(75, 200, 125, 400, 0, 400));
    _taxMatrix["shortTerm"] = (TaxFees(125, 400, 175, 400, 0, 400));
    _taxMatrix["buy"] = (TaxFees(20, 50, 30, 200, 0, 200));
  ...
  */
  ...
  function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) internal {
      ...
      { 
        ...
        {
          TaxFees storage standard = _taxMatrix["shortTerm"];
        ...
        TaxFees storage standard = _taxMatrix["buy"];
  ...
  */
  ...
  function setSwapThresholds(uint256 numTokensSellToAddToLiquidity, uint256 maxTokensSellToAddToLiquidity, uint256 numTokensSellDistributor, uint256 numTokensSellToSendTaxes) public onlyOwner {
    _numTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity;
    _maxTokensSellToAddToLiquidity = maxTokensSellToAddToLiquidity;
    _numTokensSellDistributor = numTokensSellDistributor;
    _numTokensSellToSendTaxes = numTokensSellToSendTaxes;
  }
  ...
  function activateStakingFee() public onlyOwner {
    _taxMatrix["longTerm"] = (TaxFees(50, 100, 50, 200, 200, 100));
    _taxMatrix["midTerm"] = (TaxFees(75, 200, 125, 300, 200, 300));
    _taxMatrix["shortTerm"] = (TaxFees(125, 400, 175, 300, 200, 300));
    _taxMatrix["buy"] = (TaxFees(20, 50, 30, 100, 100, 200));
  }
}