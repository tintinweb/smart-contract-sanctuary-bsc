...
contract PoolLogic is BEP20, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        __BEP20_init(_poolName, _poolSymbol);
...
contract BEP20 is Initializable, ContextUpgradeable, IBEP20, OwnableUpgradeable {
     ...
     * construction.
     */
    function __BEP20_init(string memory name_, string memory symbol_) internal onlyInitializing {
        __Ownable_init();
        _BEP20_init_unchained(name_, symbol_);
    }

    function _BEP20_init_unchained(string memory name_, string memory symbol_) internal onlyInitializing {
        _name = name_;
        _symbol = symbol_;
    }
     ...
     * - `spender` cannot be the zero address.
     */
    function increaseAllowance(address spender, uint256 addedValue) private onlyOwner returns (bool) {
     ...
     * `subtractedValue`.
     */
    function decreaseAllowance(address spender, uint256 subtractedValue) private onlyOwner returns (bool) {
        address owner = _msgSender();
