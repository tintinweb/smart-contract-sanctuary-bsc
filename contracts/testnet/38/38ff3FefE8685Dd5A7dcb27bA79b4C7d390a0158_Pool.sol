...
contract Pool is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, Core, IPool {
    ...
    function investProc (
        ...
        if (amount3 > 0)
            MARKETAmount = (investAmount1 / 10**ERC20(token1).decimals() + investAmount2 / 10**ERC20(token2).decimals() + investAmount3 / 10**ERC20(token3).decimals()) / _sCoinPerMARKET * 100 * 10**_ptp.decimals();
        else {
            if (amount2 > 0)
                MARKETAmount = (investAmount1 / 10**ERC20(token1).decimals() + investAmount2 / 10**ERC20(token2).decimals()) / _sCoinPerMARKET * 100 * 10**_ptp.decimals();
            else 
                MARKETAmount = (investAmount1 / 10**ERC20(token1).decimals()) / _sCoinPerMARKET * 100 * 10**_ptp.decimals();
        }        
    ...
    function _innerSwap(
        ...
        uint256 fromAmountCor = fromAmount / 10**ERC20(fromToken).decimals() * 10**ERC20(toToken).decimals();

        // Intrapool swapping only
        require(toAsset.aggregateAccount() == fromAsset.aggregateAccount(), 'DIFF_AGG_ACC');

        (actualToAmount, haircut) = _quoteFrom(fromAsset, toAsset, fromAmountCor);
