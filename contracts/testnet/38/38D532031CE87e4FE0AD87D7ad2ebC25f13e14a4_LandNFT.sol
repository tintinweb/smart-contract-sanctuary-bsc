...
contract LandNFT is Context, ERC165, IERC721, IERC721Metadata, MetachessOwnable {
    ...
    uint256 public ownerShipDays = 30;
//    uint256 public ownerShipRange = 3600 * 24 * ownerShipDays;
    uint256 public ownerShipRange = 600;
    ...
    function setWhitelistedUsers(address[] memory addresses, bool[] memory flags) external onlyAdmin {
        require(addresses.length == flags.length, "length is not same");
        for (uint256 i=0; i<addresses.length; i++) {
            whitelistedUsers[addresses[i]] = flags[i];
        }
    }
...
//        return _balances[owner];
    ...
    function expireTimeOf(uint256 tokenId) public view returns (uint256) {
        address owner = _owners[tokenId];
        require(owner != address(0), "ERC721: owner query for nonexistent token");
        return _expireTimes[tokenId];
    }

    function expireBorrowTime(uint256 tokenId) public view returns (uint256) {
        address owner = _owners[tokenId];
        require(owner != address(0), "ERC721: owner query for nonexistent token");
        return _expireBorrowTimes[tokenId];
    }
