...
contract EBSZKMarketplace is EBSPauser {
    mapping (bytes32 => address) private _paymentMethod;
    mapping (bytes32 => address) private _product;
    mapping (bytes32 => uint8) private _useWrapper;
    ...
    function viewOrder(
      ...
      return(
        _paymentMethod[secretKey],
        _useWrapper[secretKey],
        _price[secretKey]
      );
    }
    ...
    function lists(
        ...
        require(
            ...
            products.length == useWrappers.length &&
        ...
        for (uint256 i; i < products.length ; i++){
            list(paymentMethods[i], products[i], useWrappers[i], prices[i], secretKeys[i]);
        }
    }
    ...
    function list(
        ...
        _paymentMethod[secretKey] = paymentMethod;
        _product[secretKey] = product;
        _useWrapper[secretKey] = useWrapper;
        _price[secretKey] = price;
    ...
    function matchOrder(
        ...
        require(IEBSZKOwnership(payable(payment)).activated(key), "EBS-ZKMarketplace: Item deactivated");

        require(
            IEBSZKOwnership(payable(payment)).tokenType(key) == _useWrapper[secretKey] &&
            IEBSZKOwnership(payable(payment)).token(key) == _paymentMethod[secretKey] &&
            IEBSZKOwnership(payable(payment)).amounts(key) == _price[secretKey],
            "EBS-ZKMarketplace: Invalid payment"
        );

        bytes memory paymentKey = IEBSZKOwnership(payable(payment)).ownerSignature(key);
        bytes memory productKey = IEBSZKOwnership(payable(_product[secretKey])).ownerSignature(key);

        IEBSZKOwnership(payable(_product[secretKey])).changeKey(paymentKey,key);
        IEBSZKOwnership(payable(payment)).changeKey(productKey,key);
    ...
    function revoke(
        ...
        _paymentMethod[secretKey] = address(0);
        _product[secretKey] = address(0);
        _useWrapper[secretKey] = 0;
        _price[secretKey] = 0;
