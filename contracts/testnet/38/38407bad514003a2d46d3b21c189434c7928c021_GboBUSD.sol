...
contract GboBUSD {
    ...
    IERC20 public token = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee); 
	uint256 constant public INVEST_MIN_AMOUNT = 1e18; // 50 BUSD
	uint256[] public REFERRAL_PERCENTS 	= [2200, 200, 200, 200, 200, 200, 200, 200, 200, 200];
	...
	function withdraw() public {
		...
		require(user.levels[0] > 0, "User has no Directs");
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
				uint256 uid=user.id;
				// 21 21
				uint256 pay1=(uid*4)-2;
				uint256 pay2=(uid*4)-1;
				uint256 pay3=((uid+1)*4)+1;
				uint256 pay4=((uid+2)*4)+1;
				uint256 pay5=((uid+1)*16)+1;
				uint256 pay6=((uid+2)*16)+1;
				uint256 pay7=((uid+3)*16)+1;
				uint256 pay8=((uid+4)*16)+1;
				// level 0 is assigned 
				if (n >= pay1) {
					totalAmount+=21e18;					
				}
