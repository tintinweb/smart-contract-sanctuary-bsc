...
import "../interface/Iarmeda.sol";

contract ArmedaAirDrop is OwnableUpgradeable, DivestorUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
    struct Meta {
        address banker;
        bool isOpen;
    }
    ...
    function _claimERC20(uint fromAddrId_, uint amount_) private {
        IERC20(addrList[fromAddrId_]).transfer(_msgSender(), amount_);
    }

    function _claimERC721(uint fromAddrId_, uint cardId_, uint amount_) private {
        IATV(addrList[fromAddrId_]).mintMulti(_msgSender(), cardId_, amount_);
    }

    function _claimERC1155(uint fromAddrId_, uint cardId_, uint amouns_) private {
        IATA(addrList[fromAddrId_]).mint(_msgSender(), cardId_, amouns_);
    }
    ...
    function airdrop(uint airdropId_, uint category_, uint fromAddrId_, uint cardId_, uint amounts_, bytes32 r_, bytes32 s_, uint8 v_) isOpen public {
        ...
        count[category_] += 1;
        if (category_ == 1) {
            require(amounts_ < 1 ether, "wrong amount");
            _claimERC20(fromAddrId_, amounts_ * 1 ether);
            return;
        }
        if (category_ == 2) {
            _claimERC721(fromAddrId_, cardId_, amounts_);
            return;
        }
        if (category_ == 3) {
            _claimERC1155(fromAddrId_, cardId_, amounts_);
...
interface IATA {
    ...
    function burn(address account, uint256 id, uint256 value) external view;
