...
contract Helper is HelperBase {
        ...
        )
    {
        ...
        uint256 index;
        uint256 count;
        for (index = cursor; index <= loanLength && count < resultsPerPage; index++) {
        ...
        for (index = cursor; index <= loanLength && count < resultsPerPage; index++) {
            if (iAPHCore.getLoanExt(nftId, index).active) {
                ...
                activeLoanInfos[count].currentLTV = iAPHCore.getLoanCurrentLTV(index, nftId);
        ...
        returns (uint256 minimumCollateral, uint256 removableCollateral)
    {
        ...
        IMembership membership = IMembership(IAPHCore(aphCoreAddress).membershipAddress());
        ...
        returns (uint256 maximumBorrowAmount)
    {
        ...
        if (loanConfig.maxLTV > settledLTV) {
            maximumBorrowAmount =
                ((loanConfig.maxLTV + rankInfo.maxLTVBonus - WEI_UNIT - settledLTV) *
        ...
        )
    {
        ...
        lendingForwInterest = pool.getNextLendingForwInterest(0);
        ...
        )
    {
        ...
        (interestTokenGained, interestForwGained) = pool.claimableInterest(nftId);
    ...
    function getRankInfoList() public view returns (StakePoolBase.RankInfo[] memory rankInfos) {
        IMembership membership = IMembership(IAPHCore(aphCoreAddress).membershipAddress());
        IStakePool stakePool = IStakePool(membership.currentPool());
        ...
        returns (StakePoolBase.StakeInfo memory stakeInfo)
    {
        ...
        uint256 poolLastSettleTimestamp = block.timestamp -
            ((block.timestamp - uint256(stakePool.poolStartTimestamp())) % settleInterval);
