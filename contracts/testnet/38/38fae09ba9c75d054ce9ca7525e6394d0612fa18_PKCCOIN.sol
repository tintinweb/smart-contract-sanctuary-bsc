...
contract PKCCOIN is Context, IBEP20, Ownable {
   ...
   * @dev Start the sale.
   */
  function startSale(uint256 _rate,uint256 tokens,uint256 _start,uint256 _end) external onlyOwner returns (bool){
      startTime=_start;
      endTime=_end;
      rate = _rate;
  ...
  function toggelSale(bool _sale) external onlyOwner returns (bool){
      hasStart=_sale;
      return true;
  }
   ...
   */
   function buyToken() public payable{
       require(hasStart,"Sale is paused ");
       require(block.timestamp>startTime,"Sale is not started yet");
       require(block.timestamp<endTime,"Sale Ends");
