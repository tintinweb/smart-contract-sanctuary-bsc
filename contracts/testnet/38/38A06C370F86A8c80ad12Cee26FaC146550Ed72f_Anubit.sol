...
contract Anubit is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20 {
    ...
    string constant _message_link = "http://anubit.com/msg";

    string private constant _name = "Breen";
    ...
    //Previous fee tracking
    uint16 private _previousReflectionFee;
    uint16 private _previousBountyFee;
    uint16 private _previousLiquidityFee;
    ...
    //KYC Features (For ECO system)
    bool public _isKycEnabled;
    mapping(address => uint256) private _kycID;
    mapping(address => uint256) public _kycAllowedTxAmount;
    mapping(address => bool) private _kycComplete;
    ...
    //Private messages
    bool public _isMessagingEnabled;
    ...
    event PosTx(address customerAddress, address merchantAddress, address gratAddress, uint256 txAmount, bool hasGratuity, uint256 gratAmount, uint256 gasFee, uint256 transactionID, bool takeFees);
...
*/
        ...
        if (_antiWhaleProtection && to == uniswapV2Pair && !_isExcludedFromAntiWhale[from]) {
        uint currentTimestamp = block.timestamp;
        uint intervalStart = _sellIntervalStart[from];
        uint allowedTxAmount = _allowedTxAmount[from];
        if (intervalStart == 0 || intervalStart + _sellTimeLimit < currentTimestamp) {
            intervalStart = currentTimestamp;
            allowedTxAmount = _maxWhaleTxAmount;
        }
        require(allowedTxAmount >= amount, "M43");
        _allowedTxAmount[from] = allowedTxAmount - amount;
        _sellIntervalStart[from] = intervalStart;
        } else if (to != uniswapV2Pair) {
        require(balanceOf(to).add(amount) <= _maxWallet, "E200");
        }
    ...
    function mintDrop(
        ...
        require(wallets.length < 100, "E310"); //Can only airdrop 600 wallets per txn due to gas limits
    ...
    function setKycStatus(bool enabled) external onlyOwner {
        _isKycEnabled = enabled;
    }
    ...
    // Messaging
    function enableMessaging(bool enabled) external onlyOwner {
