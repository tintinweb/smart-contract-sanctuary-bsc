...
contract WhiteListRouter {
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IEMEPair(pair).burn(address(this));
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = EMELibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint amountBOptimal = EMELibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint amountAOptimal = EMELibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
