...
// RanchToken.
contract NapoTech is Ownable,  ERC20('Napotech Token', 'NPT') {
   
    
    IRouter public Router;
    
    uint256 public sellTax = 350;   // 3.5%
    uint256 public buyTax = 200;    // 2%
    uint256 public swapAtAmount;
    uint256 public toMarketing = 50;
    uint256 public toCharities = 50;

    address payable public marketingWallet;
    address payable public charitiesWallet;
    address public swapPair;
    mapping (address => bool) public automatedMarketMakerPairs;
    mapping (address => bool) private _isExcludedFromFees;
    
    constructor(address _router, address _MarketingWallet, address _charitiesWallet, uint256 initialSupply)  {
       
       marketingWallet = payable(_MarketingWallet);
       charitiesWallet = payable(_charitiesWallet);
    ...
    function setRatioToWallets(uint256 marketingPercent) external onlyOwner {
        require(marketingPercent <= 100, "Max 100% to marketing");
        toMarketing = marketingPercent;
        toCharities = 100 - marketingPercent;
        require(toMarketing + toCharities == 100,"Must equal 100");
    }

    function setSellTax(uint256 _newSellTax) public onlyOwner {
        require(_newSellTax <= 1000, "Cannot be more than 1000 (10%)");
        sellTax = _newSellTax;
    }
    function setBuyTax(uint256 _newBuyTax) public onlyOwner {
        require(_newBuyTax <= 1000, "Cannot be more than 1000 (10%)");
        buyTax = _newBuyTax;
    }
    ...
    function setCharityWallet(address payable _newCharityWallet) public onlyOwner {
         if (_isExcludedFromFees[charitiesWallet] = true) excludeFromFees(charitiesWallet, false);
        charitiesWallet = _newCharityWallet;
         if (_isExcludedFromFees[charitiesWallet] = false) excludeFromFees(charitiesWallet, true);
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            // Make swap to CIC
            if (balanceOf(address(this)) > swapAtAmount && !inSwapAndLiquify) SwapFees();

            uint256 extraFee = 0;
            // if to LP
            if(automatedMarketMakerPairs[to] && sellTax > 0) extraFee = (amount * sellTax)/10000;
            // if from LP
            if(automatedMarketMakerPairs[from] && buyTax > 0) extraFee = (amount * buyTax)/10000;
            // take Taxes
            if (extraFee > 0) {
    ...
    function SwapFees() private lockTheSwap {
        ...
        payable(marketingWallet).transfer((address(this).balance *100) / toMarketing);
        payable(charitiesWallet).transfer(address(this).balance);
    }
