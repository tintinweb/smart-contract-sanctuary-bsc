...
contract MoonWalks is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable private marketingAddress =
        payable(0xF6E9f0315C8e9146C1b6232585a2987bF4D506FC); // Marketing Address
    ...
    string private constant _name = "Moon Walks";
    string private constant _symbol = "WLK";
    uint8 private constant _decimals = 9;

    bool public autoBuyBackEnabled = false;
    uint256 public buyBackUpperLimit =  120000 * 10**9;
    ...
    uint256 public _previousTransferFee = _transferFee;

    uint256 public _startTimeForSwap;
    uint256 public _intervalMinutesForSwap = 1 * 30 seconds;


    uint256 public marketingDivisor = 100; // = 4*2 because we halve the amount, one half to Swap&Liquify, another to Marketing. *10 because we divide first by 10!
    ...
    bool public _burnOnTransfer = false;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (to != uniswapV2Pair && from != uniswapV2Pair && to != deadAddress) {
                removeAllFee();
                _transferBetween(from,to,amount);
                return;
            }
        }
    ...
    function _tokenTransfer(
        ...
        //Used for swaps between people
    function _transferBetween(address sender, address recipient, uint256 amount)private{
        uint256 feeAmount = amount.mul(_transferFee).div(100);
        uint256 newAmount = amount.sub(feeAmount);
        _tokenTransfer(sender, recipient, newAmount, false);
        if(_burnOnTransfer){
            burn(feeAmount);
            
        }
        else{
            _tokenTransfer(sender,feeAddress,feeAmount,false);
    ...
    function setBurnOnTransfer(bool _burn) external onlyOwner{
        _burnOnTransfer = _burn;
    }
    ...
    function setBuyBackUpperLimit(uint256 _limit) external onlyOwner{
