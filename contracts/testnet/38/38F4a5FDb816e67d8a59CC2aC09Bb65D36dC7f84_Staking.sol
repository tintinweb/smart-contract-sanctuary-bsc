...
// File: contracts/StakingPong.sol
...
// Staking distributes the Coin rewards based on staked Coin to each user.
...
contract Staking is Ownable {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;     // How many COIN tokens the user has provided.
    ...
    // Address of the Coin Token contract.
    IERC20 public Coin;
    // The total amount of Coin that's paid out as reward.
    ...
    function fund(uint256 _amount) external onlyOwner {
        ...
        Coin.transferFrom(address(msg.sender), address(this), _amount);
    }
    ...
    function claimReward(uint256 _pid) external {
        ...
        Coin.safeTransfer(address(msg.sender), pendingAmount);
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        Coin.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        user.amount = user.amount - _amount;
        pool.stakedAmount = pool.stakedAmount - _amount;
        Coin.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw(uint256 _pid) external {
        ...
        user.timestamp = block.timestamp;
        user.rewardDebt = 0;
        Coin.safeTransfer(address(msg.sender), amount);
    ...
    function withdrawFeeAmount(address _to) external onlyOwner {
        ...
        Coin.safeTransfer(_to, feeAmount);
        feeAmount = 0;
    }
    ...
    function withdrawRewards(address _to) external onlyOwner {
        ...
        Coin.safeTransfer(_to, rewardsAmount);
        rewardsAmount = 0;
    }
}