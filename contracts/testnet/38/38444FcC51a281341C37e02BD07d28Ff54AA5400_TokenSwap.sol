...
/**
    @dev This contract is for Swapping of  TOKEN to TOKEN, LP,VAULT,RECIEPT.
    Admin.WBNB(); address of WBNB.
    IAdmin(Admin).optVaultFactory(); address of Opt Vault Factory.
    _admin address of Admin Contract.
    _router address of Router.
 */
...
/**
    @dev Function to swap a Token to another Token.
    @param dtoken, Deposit token address.
    @param swappedtoken,address of token to be swapped with.
    @param amount,amount of deposit token.
    @param sendTo,SendTo address.
    NOTE: The function is used to swap the deposit token with another token or WBNB.
    **/
    function swapTOKENtoTOKEN(address dtoken, address swappedtoken, uint amount, address sendTo) public payable returns(uint) {
        if(dtoken != Admin.WBNB() && msg.sender != address(0)){            
            IERC20(dtoken).safeTransferFrom(msg.sender, address(this), amount);
            IERC20(dtoken).approve(router, amount);
        }
        address[] memory path = new address[](2);
        path[0] = dtoken;
        path[1] = swappedtoken;
        uint convertedTokenAmount;
        if(dtoken == Admin.WBNB()){
            convertedTokenAmount = IUniswapV2Router02(router).swapExactETHForTokens{value : msg.value}( 
                0,
                path,
                sendTo,
        ...
        else if(swappedtoken == Admin.WBNB()){
            convertedTokenAmount = IUniswapV2Router02(router).swapExactTokensForETH( 
                amount,
                0,
                path,
                sendTo,
            ...
            // IERC20(dtoken).approve(router, amount); 
            convertedTokenAmount = IUniswapV2Router02(router).swapExactTokensForTokens( 
                amount,
                0,
                path,
                sendTo,
...
/**
    @dev Function to swap a Token to LPtoken.
    @param depositToken, Deposit token address.
    @param LPToken,address of LPtoken.
    @param amount,amount of deposit token.
    @param sendTo,SendTo address.
    NOTE: The function is used to swap the deposit token with LP.
    **/
    function swapTOKENtoLP(address depositToken, address LPToken, uint amount, address sendTo) public payable{
        if(depositToken != Admin.WBNB() && msg.sender != address(0)){
        IERC20(depositToken).safeTransferFrom(msg.sender, address(this), amount);
        IERC20(depositToken).approve(router, amount);
        }
        require(amount > 0);
        address[] memory path = new address[](2);
        if(depositToken == Admin.WBNB()){

            if(IUniswapV2Pair(LPToken).token0() == Admin.WBNB() || IUniswapV2Pair(LPToken).token1() == Admin.WBNB()){
                address token = IUniswapV2Pair(LPToken).token0() == Admin.WBNB() ? IUniswapV2Pair(LPToken).token1() : IUniswapV2Pair(LPToken).token0();
                
                path[0] = Admin.WBNB();
                path[1] = token;
                uint convertedTokenAmount = IUniswapV2Router02(router).swapExactETHForTokens{value : msg.value/2}( 
                    1,
                    path,
                    address(this),
                    block.timestamp + 1800
                ...
                IUniswapV2Router02(router).addLiquidityETH{value : msg.value/2}(
                    token,
                ...
                path[0] = Admin.WBNB();
                path[1] = IUniswapV2Pair(LPToken).token0();
                uint convertedToken0Amount = IUniswapV2Router02(router).swapExactETHForTokens{value : msg.value/2}( 
                    1,
                    path,
                    address(this),
                    block.timestamp + 1800
                ...
                path[0] = Admin.WBNB();
                path[1] = IUniswapV2Pair(LPToken).token1();
                uint convertedToken1Amount = IUniswapV2Router02(router).swapExactETHForTokens{value : msg.value/2}( 
                    1,
                    path,
                    address(this),
                    block.timestamp + 1800
                ...
                IERC20(IUniswapV2Pair(LPToken).token0()).approve(router,amount);
                IERC20(IUniswapV2Pair(LPToken).token1()).approve(router,amount);
                IUniswapV2Router02(router).addLiquidity(
                    IUniswapV2Pair(LPToken).token0(),
                    IUniswapV2Pair(LPToken).token1(),
                    convertedToken0Amount,
                    convertedToken1Amount,
                    1,
                    1,
                    sendTo,
                    block.timestamp + 1678948210);
            }
        }else if(IUniswapV2Pair(LPToken).token0() == depositToken || IUniswapV2Pair(LPToken).token1() == depositToken){
            address token = IUniswapV2Pair(LPToken).token0() == depositToken ? IUniswapV2Pair(LPToken).token1() : IUniswapV2Pair(LPToken).token0();
            if(token == Admin.WBNB()){
                path[0] = depositToken;
                path[1] = token;
                uint convertedTokenAmount = IUniswapV2Router02(router).swapExactTokensForETH( 
                    amount/2,
                    1,
                    path,
                    address(this),
                ...
                IUniswapV2Router02(router).addLiquidityETH{value : convertedTokenAmount}(
                    depositToken,
                    amount/2,
                    1,
                    1,
                ...
                path[0] = depositToken;
                path[1] = token;
                uint convertedTokenAmount = IUniswapV2Router02(router).swapExactTokensForTokens( 
                    amount/2,
                    1,
                    path,
                    address(this),
            ...
            if(IUniswapV2Pair(LPToken).token0() == Admin.WBNB() || IUniswapV2Pair(LPToken).token1() == Admin.WBNB()){
                ...
                path[0] = depositToken;
                path[1] = Admin.WBNB();
                uint convertedToken0Amount = IUniswapV2Router02(router).swapExactTokensForETH( 
                    amount/2,
                    1,
                    path,
                    address(this),
                ...
                path[0] = depositToken;
                path[1] = token;
                uint convertedToken1Amount = IUniswapV2Router02(router).swapExactTokensForTokens( 
                    amount/2,
                    1,
                    path,
                    address(this),
                ...
                IERC20(token).approve(router,amount);
                IUniswapV2Router02(router).addLiquidityETH{value : convertedToken0Amount}(
                    token,
                    convertedToken1Amount,
                    1,
                    1,
                ...
                path[0] = depositToken;
                path[1] = IUniswapV2Pair(LPToken).token0();
                uint convertedToken0Amount = IUniswapV2Router02(router).swapExactTokensForTokens( 
                    amount/2,
                    1,
                    path,
                    address(this),
...
/**
    @dev Function to swap a Token to Vault.
    @param dtoken, Deposit token address.
    @param Vaultoken,address of Vault token.
    @param amount,amount of deposit token.
    NOTE: The function is used to swap the deposit token in Vault.
    **/
    function swapTOKENtoVAULT(address dtoken, address Vaultoken,uint _level,uint amount)public payable{
        bool isLPVault = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).poolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(Vaultoken)).isLp;
        if(dtoken != Admin.WBNB()){
            IERC20(dtoken).safeTransferFrom(msg.sender, address(this), amount);
        }
        if(!isLPVault){
            if(dtoken != Vaultoken){
                swapTOKENtoTOKEN(dtoken, Vaultoken,amount, address(this));
            }
            IERC20(dtoken).safeApprove(address(IOptVaultFactory(IAdmin(Admin).optVaultFactory())),amount);
            IOptVaultFactory(IAdmin(Admin).optVaultFactory()).Deposit(msg.sender,Vaultoken,_level,amount,false);
        }else{
            swapTOKENtoLP(dtoken, Vaultoken,amount, address(this));
            IERC20(dtoken).safeApprove(address(IOptVaultFactory(IAdmin(Admin).optVaultFactory())),amount);
            IOptVaultFactory(IAdmin(Admin).optVaultFactory()).Deposit(msg.sender,Vaultoken,_level,amount,false);
        }
    }
    ...
    /**
    @dev Function to swap a Token to Reciept.
    @param dtoken, Deposit token address.
    @param Vaultoken,address of Vault token.
    @param amount,amount of deposit token.
    @param minAmount,minimum amount of Reciept token.

    NOTE: The function is used to swap the deposit token in Reciept.
    **/
    function swapTOKENtoRECIEPT(address dtoken, address Vaultoken,uint amount, uint minAmount)public payable{
        bool isLPVault = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).poolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(Vaultoken)).isLp;
        address vaultaddress = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).poolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(Vaultoken)).vault;
        if(dtoken != Admin.WBNB()){
            IERC20(dtoken).safeTransferFrom(msg.sender, address(this), amount);
        }
        if(!isLPVault){
            ...
            IERC20(dtoken).safeApprove(vaultaddress,amount);
            IOptVault(vaultaddress).purchase(msg.sender,amount, minAmount);
        }else{
            swapTOKENtoLP(dtoken, Vaultoken,amount, address(this));
            IERC20(Vaultoken).safeApprove(vaultaddress,amount);
            IOptVault(vaultaddress).purchase(msg.sender,amount, minAmount);
        }
    }
}
...
interface IOptVaultFactory{
    ...
    enum LiqStatus {
     ...
     function Deposit(address user,address _token,uint _level,uint256 _amount,bool isReceipt) external payable;
     function withdraw(address user,address _token,bool isReceipt,uint _recieptAmount) external payable;
     function userInfo(uint pid,address user) external returns(UserInfo memory);
     function poolInfo(uint id) external returns(PoolInfo memory);
...
interface IOptVaultLp {
    ...
    function setTax(uint value) external;
    ...
    function deposit(address user, uint amountLp,uint32 _level, bool isReceipt) external payable;
    function withdraw(address user, bool isReciept, uint _recieptAmount) external payable;
...
interface IOptVault {
    ...
    function sell(address user,uint amount, uint minAmount) external returns(uint);
    function collectOptimizationTax() external;
    function deposit(address user,uint amount,uint32 _level,bool isReceipt) external;
