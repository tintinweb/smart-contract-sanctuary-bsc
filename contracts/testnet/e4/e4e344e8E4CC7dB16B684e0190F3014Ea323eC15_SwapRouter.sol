...
import "./SwapPayment.sol";

contract SwapRouter is ISwapRouter, SwapPayment {
    ...
    constructor(address _factory, address _WETH9) SwapPayment(_WETH9) {
      factory = _factory;
    }
    ...
    function _swap(address[] memory path, address _to) private {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            emit LogAddress("Pair: ", SwapLibrary.pairFor(factory, input, output));
            uint256 amountInput;
            ...
            emit LogUint256("Amount 0 Out", amount0Out);
            emit LogUint256("Amount 1 Out", amount0Out);
