...
contract NEVISBANK  is ERC20 {
    ...
    uint256  fInvested;
	address NEVIS_TOKEN= 0xb36FD3A50d3Af665160b6000471527a2e5116164;
    address BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    address Pair= 0x78C0d5Ad3C638A8f1E6469E47631f391D37DB619;
	...
	constructor(address payable wallet)  {
        ...
        plans.push(Plan(1,6027));
        IUniswapV2Router02 _NevisSwapRouter = IUniswapV2Router02(router); // testnet
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private  {
    ...
    function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			uint256 finish = user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(10 minutes));
    ...
    function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 percent, uint256 amount, uint256 start, uint256 finish) {
		...
		finish = user.deposits[index].start.add(plans[user.deposits[index].plan].time.mul(10 minutes));
	}

    function liquidity(address userAddress, uint256 amount) public {
    ...
    function getinfo() public view returns(uint256 paid) {
		for(uint256 i = 0; i < investors.length; i++){
         ...
         return user.totalBonus.add(user.withdrawnbonus).add(getUserDividends(investors[i]));
        }
	}

    function userinfo(address account) public view returns(uint256 checkpoint,address referrer,uint256 bonus,uint256 totalBonus,uint256 withdrawn,
    uint256 divident,uint256 withdrawnbonus ) {
        ...
        User storage user = users[account];
         ...
         divident = user.divident;
         withdrawnbonus = user.withdrawnbonus;
