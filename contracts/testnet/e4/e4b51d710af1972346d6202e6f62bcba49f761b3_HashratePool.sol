...
contract HashratePool is Ownable {
        ...
        if (ownerStake[_staker] == 0) {
            if (hashrates.length == 0) {
                ...
                if (_staker == hashrates[0].staker) {
                    uint256 _now = now;
                    _income = calcIncome(getRealHashrate(hashrates[0].staker), _now - hashrates[0].lTime);
            ...
            if (_staker == hashrates[_idx].staker) {
                uint256 _now = now;
                _income = calcIncome(getRealHashrate(hashrates[0].staker), _now - hashrates[_idx].lTime);
    ...
    function withdraw(address _sender, uint256 _amount) external lock returns(bool) {
        ...
        uint256 _incomeNow = calcIncome(getRealHashrate(_sender), now - hashrates[_idx].lTime);
        _balance += _incomeNow;
