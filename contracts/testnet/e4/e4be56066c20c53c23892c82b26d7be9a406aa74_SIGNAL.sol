...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        require(!_blackList[from], "blackList");//黑名单不允许交易

        uint256 balance = balanceOf(from);//查询发送者余额
        require(balance >= amount, "balanceNotEnough");//发送者余额不足时不允许交易
    ...
    function addHolder(address adr) private {
        ...
        if (0 == holderIndex[adr]) {
            if (0 == holders.length || holders[0] != adr) {
                //外部账户和合约账户的判断 合约账户返回大于0 外部账户返回等于零
                uint256 size;
...
contract SIGNAL is AbsToken {
    constructor() AbsToken(
        ...
        9,
        100000000,
        address(0x78cdAAAFF631480c10A9e281FaA0CF20B6Bd8D2b),
        address(0xff8d60039dfFD8Fc8DeC7b0ef8501B33541B22DC)
    ){
