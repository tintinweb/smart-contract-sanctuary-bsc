...
contract Leek is ERC20, Ownable, BaseToken {
    address private constant _destroyAddress = 0x000000000000000000000000000000000000dEaD;
    //address private constant Pancakeswap_V2_ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;//pancakeswap address
    //address private constant BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;//BUSD
    address private constant Pancakeswap_V2_ROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;//test pancakeswap address
    address private constant BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;//BUSD test
    uint256 internal constant MAX_AMOUNT = 115792089237316195423570985008687907853269984665640564039457584007913129639935;
    ...
    string name_ = "Leek";
    string symbol_ = "Leek";
    uint256 totalSupply_ = 1 * 10**8 * 10**18;//default decimal is 18
    uint256[3] feeSettings = [5 , 1 , 3]; // rewards, liquidity, marketing
    uint256 minimumTokenBalanceForDividends_ = 1 * 10**6 * 10**18;//default decimal is 18
    uint256 public swapTokensAtAmount = 1 * 10**6 * 10**18;

    //please deploy TokenDividendTracker before you set dividendTrackerAddress
    constructor(address marketingWalletAddress , address dividendTrackerAddress) payable ERC20(name_, symbol_) {
        address[4] memory addrs = [BUSD , Pancakeswap_V2_ROUTER , marketingWalletAddress , dividendTrackerAddress];// reward, router, marketing wallet, dividendTracker
        rewardToken = addrs[0];
    ...
    function _transfer(
        ...
        if (
            ...
            //!automatedMarketMakerPairs[from] if someone sell the token ，reward will be send
            swapping = true;

            //send reward to marking wallet
            uint256 marketingTokens = contractTokenBalance
                .mul(marketingFee)
                .div(totalFees);
            swapAndSendToFee(marketingTokens);

            //send reward to liquify
        ...
        emit transferTwappingLog(canSwap , !swapping , !automatedMarketMakerPairs[from] ,from != owner() , to != owner() , contractTokenBalance , swapTokensAtAmount, from , to , amount);
