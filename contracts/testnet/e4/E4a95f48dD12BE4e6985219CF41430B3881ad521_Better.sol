...
contract Better is VRFConsumerBaseV2, ConfirmedOwner {
  ...
  function getGamesList(uint256 limit) external view returns (Game[] memory) {
    if (limit > 50) {
      limit = 50;
    }
    if (limit > lastGameId) {
    ...
    limit = limit > lastGameId ? 0 : lastGameId - limit;

    for (uint256 i = lastGameId; i > limit; i--) {
      result[i] = games[i];
    }
   ...
   */
  function tossCoin(uint256 bet) public payable {
    ...
    require(address(this).balance * 2 >= msg.value, 'Error: insufficient vault balance');

    //each bet has unique id
    games[gameId] = Game(gameId, bet, msg.value, 0, payable(msg.sender));
