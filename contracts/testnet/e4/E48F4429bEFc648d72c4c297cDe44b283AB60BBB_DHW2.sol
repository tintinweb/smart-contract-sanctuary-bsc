...
contract DHW2 is ERC20 {
    ...
    address private _fundAddress = address(0xe3857d31d869f7eE22876713899d26412Bd5Db24);
    address private _lpAddress = address(0x73A981a4f189a269A2667e033C6Cc2917493CdDb);

    uint256 private _maxtotalSupply = 0; 
    uint256 private _maxDestoryAmounts = 0; 
    ...
    constructor(address tokenOwner) ERC20("DHW2", "DHW2") {
        _maxtotalSupply = 3900 * 10000 * (10 ** 18);
        _maxDestoryAmounts = _maxtotalSupply - 5 * 10000 * (10 **18);

        //address uniswapRouterV2Address = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        //_baseToken = address(0x55d398326f99059fF775485246999027B3197955)
                      
        address uniswapRouterV2Address = address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        _baseToken = address(0x7F726133f526c9FC1a50725d87dF0483D76701de);  
        ...
        _approve(address(this), address(uniswapRouterV2Address), _maxtotalSupply);
        ...
        _mint(tokenOwner, _maxtotalSupply);
    ...
    mapping(address => address) perparInviter;
    function getInviter(address account) public view returns (address) {
        return inviter[account];
    }
    function getPerparInviter(address account) public view returns (address) {
        return perparInviter[account];
    }
    ...
    function _transfer(
        ...
        if(perparInviter[from] == to && inviter[from] == address(0) && amount == 1 * 10**14){
          inviter[from] = to;
        }
        bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparInviter[to] == address(0) && amount >= 1 * 10**15;
        if(isInviter){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || swapping) {
            takeFee = false;
        }else{
            if(from == uniswapV2Pair){ //买入 撤池子
                if(isDelLdx){ //撤池子
                    super._transfer(from, _fundAddress, amount.div(100).mul(1)); //1% 指定地址（代币）
                    super._transfer(from, _lpAddress, amount.div(1000).mul(15)); //1.5%的lp回流
                    super._transfer(from, _lpAddress, amount.div(1000).mul(25)); //2.5%的加权平分
                    if(balanceOf(_destroyAddress) < _maxDestoryAmounts){
                        super._transfer(from, _destroyAddress, amount.div(100).mul(2)); //2% 销毁  
                    ...
                    super._transfer(from, _fundAddress, amount.div(100).mul(1)); //1% 指定地址（代币）
                    super._transfer(from, _lpAddress, amount.div(1000).mul(5)); //0.5%的lp回流
                    super._transfer(from, address(this), amount.div(1000).mul(15)); //1.5%的加权平分                  
                    if(balanceOf(_destroyAddress) < _maxDestoryAmounts){
                        super._transfer(from, _destroyAddress, amount.div(100).mul(2)); //2% 销毁  
                    ...
                    super._transfer(from, address(this), amount.div(100).mul(1)); //1% 推广奖励
                    _takeInviterFee(from, to, amount.div(100).mul(1));
                }
                splitOtherToken();
                takeFee = true;
            }else if(to == uniswapV2Pair){  //卖出 加池子
                super._transfer(from, _fundAddress, amount.div(100).mul(1)); //1% 指定地址（代币）
                super._transfer(from, _lpAddress, amount.div(1000).mul(15)); //1.5%的lp回流
                super._transfer(from, _lpAddress, amount.div(1000).mul(25)); //2.5%的加权平分
                if(balanceOf(_destroyAddress) < _maxDestoryAmounts){
    ...
    function _takeInviterFee(
          ...
          for (int256 i = 0; i < 2; i++) {
              ...
              //if(balanceOf(cur) >= 50 * 10**_decimals){
              super._transfer(address(this), cur, amount.div(1000).mul(rate));
              //}
          }
      }    
}