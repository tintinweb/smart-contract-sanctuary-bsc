...
contract Airdrops is Initializable {
    ...
    function userStakeAirdrop(
        ...
        if (_endTime == 0) {
            ...
            userAirDrops[user].amount = amount;
        }
    ...
    function isAirdropClaimed(address user) external view returns (bool) {
        if(userAirDrops[user].index ==0 && userAirDrops[user].startTime == 0){
            ...
            return false;
        }else if((userAirDrops[user].endTime == 0) && (userAirDrops[user].startTime < airdrops[userAirDrops[user].index + 1].time)){
            return false;
        }else{
            return true;
        }
    ...
    function claimAirdrop(address user) external {
        ...
        if (t > airdropEligibleTier && l > airdropEligibleLevel) {
            ...
            amount = userAirDrops[user].amount;
            uint32 i = uint32(id);
            for (; i <= end; i++) {
                ...
                airdrops[i].claimed[msg.sender] = true;
            }
            userAirDrops[user].index = i-1;
...
interface IAirdrops {
    ...
    function userStakeAirdrop(address user,uint amount, uint _startTime, uint _endTime) external;
