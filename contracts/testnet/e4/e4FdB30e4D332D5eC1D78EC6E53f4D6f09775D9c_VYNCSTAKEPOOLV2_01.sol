...
contract VYNCSTAKEPOOLV2_01 is
    Initializable,
    ...
    OwnableUpgradeable
{
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        uint256 vyncFee = stake_fee; /// changed
        require(amount > vyncFee, "amount less then stake_fee");
        amount -= vyncFee; /// changed
        vync.transferFrom(msg.sender, address(this), amount);
        vync.transferFrom(msg.sender, TreasuryAddress, vyncFee); /// changed
        ...
        _userInfo.stakedVync += amount; /// changed
    ...
    function unStake(uint256 amount) external nonReentrant {
        /// @notice Getting the storage reference.
        userInfoData storage _userInfo = userInfo[msg.sender]; /// changed

        require(isBlock[msg.sender] == false, "blocked");
        require(
            amount <= _userInfo.stakedVync, /// changed
        ...
        require(amount <= maxUnstakePerTx, "exceed unstake limit per tx"); /// changed
        require(amount > unstake_fee, "amount less then unstake_fee");

        uint256 pending = compoundedReward(msg.sender);
        uint256 stakeBalance = _userInfo.stakeBalance;
        uint256 vyncAmount = amount; /// changed
        vyncAmount -= unstake_fee; /// changed
        vync.transfer(msg.sender, vyncAmount);
        vync.transfer(TreasuryAddress, unstake_fee); /// changed
        amount -= unstake_fee; /// changed
        emit UnStake(msg.sender, amount);

        // reward update
        if ((amount + unstake_fee) < _userInfo.stakedVync) {
            uint256 _pendingReward = compoundedReward(msg.sender);
            ...
            _userInfo.nextCompoundDuringStakeUnstake = nextCompound();
            _userInfo.totalClaimedReward = 0;
            _userInfo.stakeBalanceWithReward = _userInfo.stakeBalance - (amount + unstake_fee) + _pendingReward; /// changed

            ///////// staked balance change as per percentage //////////////
            uint256 vyncPercentageOfStaked = ((amount + unstake_fee) * 100) / _userInfo.stakedVync;
            uint256 stakeBalancePercentage = ((_userInfo.stakeBalance * vyncPercentageOfStaked) / 100);
            _userInfo.stakeBalance -= stakeBalancePercentage; /// changed
            //////////////////////
            
            _userInfo.stakedVync -= (amount + unstake_fee); /// changed
            u += stakeBalancePercentage;
            u_v += vyncAmount + unstake_fee; /// changed
        }

        if ((amount + unstake_fee) >= _userInfo.stakedVync) {
            u += stakeBalance;
            u_v += vyncAmount + unstake_fee;
            ...
            _userInfo.stakedVync = 0; /// changed
    ...
    function manualMigrateByOwnerToken(
        ...
        _userInfo.stakeBalanceWithReward += (stakeTokenBalance + compoundTokenAmount + _compoundReward);
        ...
        _userInfo.isStaker = true;
        _userInfo.totalClaimedReward = 0;
        _userInfo.autoClaimWithStakeUnstake = (_compoundReward + compoundTokenAmount);
