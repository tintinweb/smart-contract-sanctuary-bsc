...
contract Staking is Ownable {
    ...
    mapping(uint256 => mapping(address => uint256)) private twoXRewards;
    ...
    */
    ...
    function createTwoXReward(
        ...
        require(_stake.amount >= twoXRewards[_period][_address] + _amount, "don't have sufficient token");
        twoXRewards[_period][_address] += _amount;
    ...
    function addReferral(address _referral) external {
        require(referrals[msg.sender] == address(0), "can't change the referral");
    ...
    function unStake(
        ...
        _stake.amount -= _amount;
        _totalSupply -= _amount;
        if(twoXRewards[_period][msg.sender] > _stake.amount){
            burnAmount = twoXRewards[_period][msg.sender] - _stake.amount;
            twoXRewards[_period][msg.sender] = _stake.amount;
    ...
    canTransfer(msg.sender, _amount)
    {
        ...
        uint256 totalUserHolding;
        for (uint i = 0; i < stakeHolders.length; i++) {
            totalUserHolding = stakes[stakeHolders[i]].amount + twoXRewards[_period][stakeHolders[i]];
            uint256 pecentage = totalUserHolding * (100 * 10 ** 9) / totalHolding;
            uint256 _reward = _amount * pecentage / (100 * 10 ** 9);
    ...
    function closeTwoXRewards() external onlyOwner{
      _period += 1;
      _totalRewardSupply = 0;
    }

    function stakingForAdjust(address _addr, uint256 _amount) external onlyOwner{
        Stake storage _stake = stakes[_addr];
        require(_amount < _stake.amount, "not valid");
        require(_stake.releaseTime > 0, "not valid");
        require(startTime + 10 days > block.timestamp, "expired");
        _stake.amount -= _amount;
    }
    ...
    function stakedDetails(address account) public view virtual returns (uint256,uint256) {
        Stake memory _stake = stakes[account];
        return (_stake.amount, _stake.releaseTime);
    }

    function twoXBalanceOf(address account) public view virtual returns (uint256) {
        return twoXRewards[_period][account];
    }
}
