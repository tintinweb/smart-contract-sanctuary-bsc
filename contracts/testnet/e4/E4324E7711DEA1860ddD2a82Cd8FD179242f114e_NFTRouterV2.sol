...
contract NFTRouterV2 is Ownable {
    ...
    isLock(address(droidBot), _droidBot1Id)
    {
        ...
        uint256 _l0 = droidBot.power(_droidBot0Id);
        uint256 _l1 = droidBot.power(_droidBot1Id);
    ...
    function processRequestBundle(uint256[] memory _ids, uint256[] memory _blockNums, bytes32[] memory _blockHash, bytes[] memory _signatures) external {
        require(
        ...
        for (uint256 i = 0; i < _length; i ++) {
            _processRequest(_ids[i], _blockNums[i], _blockHash[i], _signatures[i]);
        }
    }
