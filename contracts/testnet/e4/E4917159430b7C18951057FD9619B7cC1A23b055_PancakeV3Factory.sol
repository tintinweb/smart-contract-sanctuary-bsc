...
import '@pancakeswap/v3-lm-pool/contracts/interfaces/IBunswapV3LmPool.sol';
...
contract PancakeV3Pool is IPancakeV3Pool {
    ...
    struct Slot0 {
        ...
        uint16 observationCardinalityNext;
        // the current protocol fee for token0 and token1 in bips (fee/PROTOCOL_FEE_DENOMINATOR)
        uint16 feeProtocol0;
        uint16 feeProtocol1;
    ...
    constructor() {
        int24 _tickSpacing;
        uint24 feeTmp;
        (factory, token0, token1, feeTmp, _tickSpacing) = IPancakeV3PoolDeployer(msg.sender).parameters();
        fee = feeTmp;
        ...
        if (feeTmp == 100) {
            slot0.feeProtocol0 = 3300;
            slot0.feeProtocol1 = 3300;
        } else if (feeTmp == 500) {
            slot0.feeProtocol0 = 3400;
            slot0.feeProtocol1 = 3400;
        } else if (feeTmp == 2500) {
            slot0.feeProtocol0 = 3200;
            slot0.feeProtocol1 = 3200;
        } else if (feeTmp == 10000) {
            slot0.feeProtocol0 = 3200;
            slot0.feeProtocol1 = 3200;
        } else {
            slot0.feeProtocol0 = 3200;
    ...
    function swap(
        ...
        SwapCache memory cache = SwapCache({
            ...
            feeProtocol: zeroForOne ? slot0Start.feeProtocol0 : slot0Start.feeProtocol1,
            secondsPerLiquidityCumulativeX128: 0,
            tickCumulative: 0,
    ...
    function setFeeProtocol(uint16 feeProtocol0, uint16 feeProtocol1) external override lock onlyFactoryOwner {
        require(
            (feeProtocol0 == 0 || (feeProtocol0 >= 1000 && feeProtocol0 <= 4000)) &&
            (feeProtocol1 == 0 || (feeProtocol1 >= 1000 && feeProtocol1 <= 4000))
        );

        emit SetFeeProtocol(slot0.feeProtocol0, slot0.feeProtocol1, feeProtocol0, feeProtocol1);
