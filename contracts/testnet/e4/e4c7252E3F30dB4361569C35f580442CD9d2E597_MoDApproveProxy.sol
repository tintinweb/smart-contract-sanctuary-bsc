...
contract MoDApproveProxy is AccessControl {
    ...
    modifier isHandler(){
        require(hasRole(HANDLER, msg.sender),"HANDLER ROLE required in proxy");
        _;
    }

    function DAR_transferFrom(address _sender, address _recipient, uint256 _amount) external isHandler {
        DAR.transferFrom(_sender, _recipient, _amount);
    }

    function item_safeTransferFrom(address _from, address _to, uint _tokenId, bytes memory _data) external isHandler {
        items.safeTransferFrom(_from, _to, _tokenId, _data);
    }

    function resources_safeTransferFrom(address _from, address _to, uint _id, uint _amount, bytes memory _data) external isHandler {
        resources.safeTransferFrom(_from, _to, _id, _amount, _data);
    }

    function resources_safeBatchTransferFrom(address _from, address _to, uint[] calldata _ids, uint[] calldata _amounts, bytes memory _data) external isHandler {
        resources.safeBatchTransferFrom(_from, _to, _ids, _amounts, _data);
    }

    function planetPlot_safeTransferFrom(address _from, address _to, uint _tokenId, bytes memory _data) external isHandler {
        planetPlot.safeTransferFrom(_from, _to, _tokenId, _data);
    }

    function item_burn(address _from, uint256[] memory _burnItemIds) external isHandler returns (bool){
        return(items.burn(_from, _burnItemIds));
    }

    function item_burnAndMint(address _fromTo, uint256[] memory _mintTypes, uint256[] memory _burnItemIds) external isHandler returns (bool){
        return(items.burnAndMint(_fromTo, _mintTypes, _burnItemIds));
    }

    function resources_burnBatch(address _from, uint256[] memory _ids, uint256[] memory _amounts) external isHandler returns (bool) {
