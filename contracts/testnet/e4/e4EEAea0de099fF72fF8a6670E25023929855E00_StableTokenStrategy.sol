    ...
    OwnableUpgradeable
{
    ...
    function getSupplyPrices(
        ...
        uint128 quote,
        uint128 base
    ) external override view returns (Grid.GridOrderData[] memory) {

        require(currentPip - numberOfSupplyOrdersEachSide * stepPip > 0, "Out range");

        Grid.GridOrderData[] memory grid = new  Grid.GridOrderData[](numberOfSupplyOrdersEachSide * 2);
        ...
        for (uint128 i = 0; i < numberOfSupplyOrdersEachSide; i++) {
            // Sell grid
            grid[i] = Grid.GridOrderData(currentPip + ((i +1) * stepPip), int256(amountBaseEachGrid));

            // Buy gird
            grid[i + numberOfSupplyOrdersEachSide] = Grid.GridOrderData(currentPip - ((i +1) * stepPip), -int256(amountQuoteEachGrid));
        }
        return grid;
    }

    function updateNumberOfGridEachSide(uint16 _newNumberOfGrid) public onlyOwner {
    ...
    function getNumberOfSupplyOrdersEachSide() external override view returns (uint16) {
