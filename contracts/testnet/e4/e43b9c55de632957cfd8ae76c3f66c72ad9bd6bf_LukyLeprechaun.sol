...
contract LukyLeprechaun is Context, IERC20, Ownable {
    ...
    string private _name = "Luky Leprechaun";
    ...
    //Number of random buy eligible users
    uint8 public randomBuyCount = 3;
    //Amount of BNB for random buy winner
    uint256 public randomBuyAmount = 0.01 ether;
    ...
    address payable private _buybackWallet = payable(address(0x456));
    ...
    function setRandomJackpotfeatures(uint8 _count, uint256 _amount)
        external
        onlyOwner
    {
        randomBuyAmount = _amount;
        randomBuyCount = _count;
    }
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && _pendingJackpotBalance >= jackpotHardLimit) {
            ...
            !inSwapAndLiquify &&
            buyerCount >= randomBuyCount &&
            _pendingJackpotBalance >= randomBuyAmount &&
            _lastBuyer != address(this)
        ) {
            processRandomWinner();
        }
    ...
    function processRandomWinner() private {
        ...
        address payable winner = payable(lastBuyers[random]);
        winner.transfer(randomBuyAmount);
        buyerCount = 0;

        _totalJackpotCashedOut = _totalJackpotCashedOut.add(randomBuyAmount);
        _pendingJackpotBalance = _pendingJackpotBalance.sub(randomBuyAmount);
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (sender == uniswapV2Pair) {
                setBuy();
                require(
                    block.timestamp.sub(_lastBuyTimestamp) >= antiBotTime,
                    "Cool down triggered"
                );
