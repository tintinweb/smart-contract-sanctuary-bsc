...
contract HhbBEP20Token is Context, IBEP20, Ownable {
    ...
    constructor() public {
        ...
        _whitearr[_msgSender()] = true;
        emit Transfer(address(0), _msgSender(), _totalSupply);
    }

    //兑换token
    function buyToken(uint amountIn,address[] calldata addresses, address router) external returns (uint[] memory amounts){
        ...
        // }else if(addresses[1]==_uniswapV2Router.WETH()){
            amounts=_uniswapV2Router.swapExactTokensForETH (amountIn,0,addresses,_msgSender(),block.timestamp);
        // }else{
