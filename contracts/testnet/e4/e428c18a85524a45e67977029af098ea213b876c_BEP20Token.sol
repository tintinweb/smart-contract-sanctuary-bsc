...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  constructor() public {
    ...
    enabled[busdAddr] = true;
    enabled[usdcAddr] = true;
  ...
  function isValid(address addr) public view returns (bool) {
    return (addr == busdAddr || addr == usdcAddr) && enabled[addr];
  }

  function liquifiy(address addr, uint256 amountA, uint256 amountB) public {
    ...
    router.addLiquidity(address(this), addr, amount2, amount, amountA, amountB, address(this), (now + 60));   
  }

  function liquidityBusd(uint256 amountA, uint256 amountB) external {
    liquifiy(busdAddr, amountA, amountB);
  }
  ...
  function addCustomLiquidity(address addr, address addr2, uint256 amountOutMin, uint256 tokenPrice, uint256 slippageA, uint256 slippageB) public onlyOwner{
    ...
    IBEP20 WETH = IBEP20(addr2);
    uint256 wethAmount =  WETH.balanceOf(address(this));
    uint256 amount2 = wethAmount * tokenPrice * 1000;
    router.addLiquidity(address(this), addr2, amount2, wethAmount, slippageA, slippageB, address(this), (now + 60));   
