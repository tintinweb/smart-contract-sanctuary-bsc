...
abstract contract DeployFee is Context {
    ...
    IERC20 public deployFeeFirstToken;
    IERC20 public deployFeeSecondToken;
    ...
    function setupDeployFeeInternal(
        ...
        //Set Up the fee Tokens
        deployFeeFirstToken = IERC20(_deployFeeFirstToken);
        deployFeeSecondToken = IERC20(_deployFeeSecondToken);
        //Set Uo the priceFeed
        priceFeed = AggregatorV3Interface(_priceFeed);
        //Set up the fee Amounts
        deployFeeFixedAmount = _deployFeeFixedAmount;
        deployFeePercentageAmount = _deployFeePercentageAmount;
        //Set up the active payment option
        deployFeePaymentOption = _deployFeePaymentOption;
        //Set Up the beneficiary
    ...
    {
        if (
            tokenAddress == address(deployFeeFirstToken) ||
            tokenAddress == address(deployFeeSecondToken)
        ) {
            tokenAddress == address(deployFeeFirstToken)
                ? tokenChargeDeployFee(deployFeeFirstToken, isPaid)
                : tokenChargeDeployFee(deployFeeSecondToken, isPaid);
        } else {
            cryptoChargeDeployFee(isPaid);
        }
    }

    function tokenChargeDeployFee(IERC20 paymentToken, bool isPaid)
        internal
    {
        if (!isPaid) {
            ...
            paymentToken.safeTransferFrom(
                _msgSender(),
                deployFeeBeneficiary,
                (deployFeeFixedAmount * deployFeePercentageAmount) / 1e18
            );
        }
    }
    ...
    function cryptoChargeDeployFee(bool isPaid) internal {
        ...
        if(!isPaid) {
            uint requiredEth = deployFeeFixedAmount / uint(ethUsdPrice) * 1e8;
            require(msg.value >= requiredEth);
            uint ethExceeded = msg.value - requiredEth;
            if(ethExceeded > 1 gwei) {
                (bool sent, ) = _msgSender().call{value: ethExceeded}("");
                require(sent, "ERROR: Failed to return value");
            }
        }
        else {
            uint requiredEth = (deployFeeFixedAmount / uint(ethUsdPrice) * 1e8) * deployFeePercentageAmount / 1e18;
            require(msg.value >= requiredEth);
            uint ethExceeded = msg.value - requiredEth;
            if(ethExceeded > 1 gwei) {
                (bool sent, ) = _msgSender().call{value: ethExceeded}("");
