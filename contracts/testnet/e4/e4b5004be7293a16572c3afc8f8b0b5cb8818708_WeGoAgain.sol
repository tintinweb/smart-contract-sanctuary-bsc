...
contract WeGoAgain is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WeGoAgainDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WeGoAgain", "WGA") {
        ...
        dividendTracker = new WeGoAgainDividendTracker(
            payable(this),
            router,
...
contract WeGoAgainDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    WeGoAgain public WeGoAgainContract;
    ...
    constructor(
        ...
        defaultToken = token;
        WeGoAgainContract = WeGoAgain(mainContract);
    ...
    function _transfer(
        ...
        require(false, "WeGoA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WeGoA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WeGoA contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WeGoAgainContract);

            uint256 prevBalance = WeGoAgainContract.balanceOf(address(this));
            ...
            {
                uint256 received = WeGoAgainContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WeGoAgainContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
