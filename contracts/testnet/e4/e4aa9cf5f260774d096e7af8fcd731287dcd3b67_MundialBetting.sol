...
// File: contracts/MundialBetting.sol
...
contract MundialBetting is Auth {
    ...
    mapping (address => OddChamp) public Codds; //user -> odd
    ...
    function setMatches(uint256[] calldata _match, uint256[] calldata _start, string[] calldata _team1, string[] calldata _team2, string[] calldata _stadium, uint256[] calldata _group) external onlyOwner{
    ...
    function setOdds(uint256[] calldata _match, uint256[] calldata _win1, uint256[] calldata _win0, uint256[] calldata _win2) external onlyOwner{
        for (uint i = 0; i < _match.length; i++) {
    ...
    function predictChampion(uint _team, uint256 _amount) external callerIsUser {
        ...
        require(token.allowance(msg.sender, address(this)) >= _amount, "Insufficient Allowance");
        require(block.timestamp < Matchs[47].start, "Group Stage has ended! You can't take part now.");
