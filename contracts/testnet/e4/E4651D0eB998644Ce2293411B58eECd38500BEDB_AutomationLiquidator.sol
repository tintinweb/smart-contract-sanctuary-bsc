...
import "../core/interfaces/IVaultController.sol";
import "../core/interfaces/IPositionManager.sol";

contract AutomationLiquidator is AutomationCompatible, Ownable {
    //---------- Libraries ----------//
    ...
    {
        uint256 _book = totalPositions().div(100);
        uint256 remainder = _book.mul(100);
        _book = remainder < totalPositions() ? _book.add(1) : _book;
        return (_book, _numDigits(_book));
    }

    function _getPositionWithKey(bytes32 _key)
        private
        pure
        returns (
            address account,
    ...
    {
        (account, collateralToken, indexToken, isLong) = abi.decode(
    ...
    function totalPositions() public view returns (uint256) {
        return vaultController.totalPositions();
    }

    function positionAt(uint256 _index) public view returns (bytes32) {
        return vaultController.positionAt(_index);
    }
    ...
    function getBatch() public view returns (uint256 start, uint256 end) {
        if (totalPositions() != 0) {
            ...
            uint256 end_ = index.mul(100) > totalPositions()
                ? totalPositions()
                : index.mul(100);
            return (start_, end_);
        }
        ...
        returns (bool, bytes memory)
    {
        ...
        for (uint256 i = start; i < end; i++) {
            bytes32 positionKey = positionAt(i);
            (
                address account,
                address collateralToken,
                address indexToken,
                bool isLong
            ) = _getPositionWithKey(positionKey);
            (uint256 liquidationState, ) = vault.validateLiquidation(
                account,
                collateralToken,
                indexToken,
                isLong,
            ...
            if (liquidationState != 0) {
                return (true, abi.encodePacked(positionKey));
            }
        }
    ...
    function performUpkeep(bytes calldata performData)
        ...
        onlyKeeper
    {
        bytes32 positionKey = abi.decode(performData, (bytes32));
        (
            address account,
            address collateralToken,
            address indexToken,
        ...
        require(liquidationState != 0, "Cannot be liquidated");
...
interface IVaultController {
    ...
    function addPosition(bytes32 _key) external;
