...
contract Bridge is Ownable{
    ...
    event LockBSC(address indexed sender, address indexed recipient, uint256 amount);
    ...
    function lock( address _recipient, uint256 _amountbSVC) external{
        require(1000<= _amountbSVC && _amountbSVC <= 10000, "invalid lock amount");
        ...
        emit LockBSC(msg.sender, _recipient, _amountbSVC);
    }

    function unlock( address _recipient, uint256 _amountbSVC) external payable onlyDev {
        require(IERC20(bSVC).balanceOf(msg.sender) >= _amountbSVC, "Insufficient SVC account balance !!!");
