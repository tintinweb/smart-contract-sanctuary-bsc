...
// File: contracts/8/MintSynTest.sol
...
contract MintSynTest is Ownable {
    ...
    function mint(uint256 positionId,uint256 mintAmount) external {
        ...
        ERC20Mock(positions[positionId].asset).mint(msg.sender,mintAmount);
    }

    function burn(uint256 positionId,uint256 burnAmount) external {
        require(msg.sender == positions[positionId].owner ,"not owner");
        require(positions[positionId].mintAmount >= burnAmount,"Cannot burn asset more than you mint");
        positions[positionId].mintAmount = positions[positionId].mintAmount.sub(burnAmount);
        ERC20Mock(positions[positionId].asset).transferFrom(msg.sender,address(this),burnAmount);
        ERC20Mock(positions[positionId].asset).burn(burnAmount);
    }

    function auction(uint256 positionId,uint256 burnAmount) external {
        require(positions[positionId].mintAmount >= burnAmount ,"Cannot liquidate more than the position amount");
        Position storage pos = positions[positionId];
        (uint256 colleteralPrice,)= twap.getPrice(collateralToken);
        (uint256 assetPrice, ) = twap.getPrice(pos.asset);
        uint256 collateralPriceInAsset = (assetPrice.mul(1e18)).div(colleteralPrice);
        uint256 assetValueInCollateralAsset = (pos.mintAmount.mul(collateralPriceInAsset)).div(1e18);
        if(assetValueInCollateralAsset.mul(config.getMinCollateralRatio(pos.asset)).div(1e20) <  pos.collateralAmount){
            revert("Cannot liquidate a safely collateralized position");
        }
        uint256 auctionDiscount = Math.min(config.getAuction(pos.asset).div(100),(config.getMinCollateralRatio(pos.asset)).div(100)).sub(1e18);
        uint256 one = 1e18;
        uint256 discountPrice = collateralPriceInAsset.div(((one).sub(auctionDiscount)));
        uint256 assetValueInCollateralAssetDiscount = pos.mintAmount.mul(discountPrice).div(1e18);
        uint256 returnCollateralAmount;
        uint256 refundAssetAmount;
        if(assetValueInCollateralAssetDiscount > pos.collateralAmount) {
            refundAssetAmount =  ((assetValueInCollateralAssetDiscount.sub(pos.collateralAmount)).mul(1e18)).div(discountPrice);
            returnCollateralAmount = pos.collateralAmount;
        } else {
            returnCollateralAmount = assetValueInCollateralAssetDiscount;
            refundAssetAmount = 0;
        }
        ...
        if(leftCollateralAmount == 0){
            ...
            pos.collateralAmount = 0;
            pos.mintAmount = 0;
            IERC20(collateralToken).transfer(pos.owner, leftCollateralAmount);
        }else {
            pos.collateralAmount = leftCollateralAmount;
            pos.mintAmount = leftAssetAmount;
        }
        ERC20Mock(pos.asset).burn(liquidatedAmount);
        IERC20(collateralToken).transfer(msg.sender, returnCollateralAmount);
    }
}