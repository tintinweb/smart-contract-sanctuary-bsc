...
contract IFOBitdriveProxy is ReentrancyGuard, Initializable {
  using SafeMath for uint256;
  ...
  uint public Round2total;
  uint public Round2Balance;
  ...
  uint256 public publicSale=25000;

  uint256 public totalTier1Users;
  uint256 public totalTier2Users;
  uint256 public totalTier3Users;
  uint256 public totalTier4Users;
  
  uint256 public totalTier1Amount;
  uint256 public totalTier2Amount;
  uint256 public totalTier3Amount;
  uint256 public totalTier4Amount;

  uint256 public tier2EligibleUsers;
  uint256 public tier3EligibleUsers;
  uint256 public tier4EligibleUsers;
  ...
  function deposit(uint256 _amount) public {
    ...
    uint256 transferAmt= _amount*1e18;
    lpToken.safeTransferFrom(address(msg.sender), address(this), transferAmt);
    ...
    if(userInfo[msg.sender].amount>=STAKING_PRICE[1] && userInfo[msg.sender].amount<STAKING_PRICE[2]){
        userInfo[msg.sender].tier =1;
        totalTier1Users++;
        totalTier1Amount = totalTier1Amount.add(_amount);
        userInfo[msg.sender].tier1amount = userInfo[msg.sender].tier1amount.add(_amount);
    }else if(userInfo[msg.sender].amount>=STAKING_PRICE[1] && userInfo[msg.sender].amount<STAKING_PRICE[3]){
        userInfo[msg.sender].tier =2;
        totalTier2Users++;
        totalTier2Amount = totalTier2Amount.add(_amount);
        userInfo[msg.sender].tier2amount = userInfo[msg.sender].tier2amount.add(_amount);
    }else if(userInfo[msg.sender].amount>=STAKING_PRICE[3] && userInfo[msg.sender].amount<STAKING_PRICE[4]){
        userInfo[msg.sender].tier =3;
        totalTier3Users++;
        totalTier3Amount = totalTier3Amount.add(_amount);
        userInfo[msg.sender].tier3amount = userInfo[msg.sender].tier3amount.add(_amount);
    }else if(userInfo[msg.sender].amount>=STAKING_PRICE[4]){
        userInfo[msg.sender].tier =4;
        totalTier4Users++;
        totalTier4Amount = totalTier4Amount.add(_amount);
        userInfo[msg.sender].tier3amount = userInfo[msg.sender].tier3amount.add(_amount);
    }

    totalAmount = totalAmount.add(_amount);
    balanceAmount = balanceAmount.add(_amount);
  ...
  function harvestReward(uint256 _amount) public nonReentrant {
    ...
    uint256 perAmount = getuserAllocated(msg.sender);
    uint256 round1withdraw = userInfo[msg.sender].round1withdraw;
    uint256 available = perAmount.sub(round1withdraw);
    require (available >= _amount, 'insufficient balance');
    userInfo[msg.sender].round1withdraw = userInfo[msg.sender].round1withdraw.add(_amount);
    uint256 transferAmt= _amount*1e18;
    offeringToken.safeTransfer(address(msg.sender), transferAmt);
    balanceAmount = balanceAmount.sub(_amount);

    uint withdrawPer = perAmount/userInfo[msg.sender].round1withdraw/100;
    if(withdrawPer>=50 && !isPublicsale){
        if(currentTier==2){
            tier2EligibleUsers++;
        }
        if(currentTier==3){
   ...
   function getuserAllocated(address _user) public view returns (uint) {
       ...
       uint tieramount=0;
       if(currentTier==2){
            tieramount =tieramount2/totalTier2Users;
       }else if(currentTier==3){
            tieramount =tieramount3/totalTier3Users;
       }else if(currentTier==4){
            tieramount =tieramount4/totalTier4Users;
       }
       return tieramount;
    }
    ...
    function tierAllocation() public view returns (uint,uint,uint) {
        ...
        uint tieramount2 =(totalTier2Users.mul(perAmount).mul(POOL_WEIGHT[1]));
        uint tieramount3 =(totalTier3Users.mul(perAmount).mul(POOL_WEIGHT[2]));
        uint tieramount4 =(totalTier4Users.mul(perAmount).mul(POOL_WEIGHT[3]));
  ...
  function tokenAllocated() public view returns (uint) {
        uint allocate= (
            (totalTier2Users.mul(POOL_WEIGHT[1])).add(totalTier3Users.mul(POOL_WEIGHT[2]))
        .add(totalTier4Users.mul(POOL_WEIGHT[3])));
        uint perUser=offeringAmount/allocate;
        perUser = perUser;
        return perUser;
  }
  ...
  function startNextRound(uint256 round) public onlyAdmin {
      require (block.number < endBlock, 'tire not closed');
      currentRound = round;
      Round2total = balanceAmount.add(publicSale);
      Round2Balance = balanceAmount.add(publicSale);
  }

  function tokenAllocatedPublicSale() public view returns (uint) {
        uint totalUsers = totalTier1Users.add(tier2EligibleUsers).add(tier3EligibleUsers).add(tier4EligibleUsers);
        uint perUser=Round2total.div(totalUsers);
        perUser = perUser;
        return perUser;
  }
  ...
  function harvestPublicReward(uint256 _amount) public nonReentrant {
    ...
    require (currentRound==2, 'not harvest time');

    require (_amount>0, 'invalid amount');
    require (Round2Balance>0, 'unable to withdraw');
    ...
    uint256 transferAmt= _amount*1e18;
    offeringToken.safeTransfer(address(msg.sender), transferAmt);
    Round2Balance = Round2Balance.sub(_amount);
