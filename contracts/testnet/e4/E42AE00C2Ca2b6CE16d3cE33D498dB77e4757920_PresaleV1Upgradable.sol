    ...
    UUPSUpgradeable
{
    ...
    function getTokenName() external view returns (string memory) {
        return IERC20_EXTENDED(tokenAddress).name();
    }

    function getTokenDecimals() external view returns (uint256) {
        return IERC20_EXTENDED(tokenAddress).decimals();
    }
    ...
    function setReferralAddress(
        ...
        return referralAddress = _address;
    }
        ...
        returns (uint256 minETHRequired)
    {
        if (minContributionUSD == 0) {
            ...
            minETHRequired =
                (1 * 10 ** IERC20_EXTENDED(tokenAddress).decimals()) /
                ratio;
        }
    }
    ...
    function isPayRewardTokensEnabled() external view returns (bool) {
        return isPayRewardTokens;
    }

    function setPayRewardTokens(bool _bool) external onlyOwner returns (bool) {
        return isPayRewardTokens = _bool;
    }
    ...
    function _getTokensValueETH(
        ...
        uint256 ethValue = (_ethValue * ethPrice) /
            (10 ** IERC20_EXTENDED(tokenAddress).decimals());
    ...
    function _getTokensValueUSD(
        ...
        tokenValue =
            (_USDValue * _price) /
            10 ** IERC20_EXTENDED(USDAddress).decimals();
    }
    ...
    function _BuyWithETH(
        ...
        if (isBuyNStake) {
            ...
            IStaking(stakingAddress).stakeByAdmin(
                _address,
                _tokenValue,
                _duration
            );
        } else {
            IERC20Upgradeable(tokenAddress).transfer(_address, _tokenValue);
        }
    ...
    function _BuyWithUSD(
        ...
        emit TokenPurchased(_address, _tokenValue, _value, "USD");
    }
    ...
    function BuyWithETH(
        ...
        // _BuyWithETH(_userAddress, _msgValue, _duration);
        // _payRewardTokensETH(_userAddress, _msgValue);

        if (isPayReferral) {
            payable(referralAddress).transfer(
                _msgValue.div(
                    IReferral(referralAddress).getTotalReferralBonus()
                )
            );

            IReferral(referralAddress).payReferralETHAdmin(
                _msgValue,
                _userAddress,
                _referrer
            );
    ...
    function BuyWithUSD(
        ...
        IERC20Upgradeable(USDAddress).transferFrom(
            _userAddress,
            address(this),
            _value
        );

        _BuyWithUSD(_userAddress, _value, _duration);
        _payRewardTokensUSD(_userAddress, _value);
