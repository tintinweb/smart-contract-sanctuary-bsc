...
abstract contract StakeTokenWrapper {
    ...
    // mapping(address => accountInfo) private _stackingBalancesByGuildId;
...
contract StakingRewardForGuild is StakeTokenWrapper, Ownable {
    ...
    // mapping(address => uint256) public stackTimeBegin;
    ...
    event Staked(address indexed user,uint256 guildID, uint256 amount);
   ...
   /**
     ...
     */
    function earned(address account, uint256 guildId) public view returns (uint256) {
    ...
    function stake(uint256 _amount,uint256 _rewardRate, uint256[] memory message,address[] memory _address , uint8 v, bytes32 r,
                 bytes32 s) public {
        require(_amount >= 20e18, "Cannot stake lower than 20 Renas");
        require(_rewardRate > 0, "Rate can't be equal 0");
        require(_rewardRate <= 375,"Rate can't be greater than 3.75%"  );
        ...
        require(_address[0] == msg.sender,"Signature is not belong to this player!");
        require( _amount <= message[0],"Staking amount is exceeded Maximum limited!");
        //Todo: check guild
        require(stakingBalanceOf(msg.sender,message[1]) == 0, "You can only stake once");
        //do staking
        super.stake(_amount,message[1]); // message[1] contain guildID
        //store the staking start time for reward calculating purpose        
        // stackTimeBegin[msg.sender] = uint256(block.timestamp);
        // rewardRate[msg.sender] = _rewardRate;

        interestInfo[msg.sender][message[1]].rewardRate = _rewardRate;
        interestInfo[msg.sender][message[1]].stackTimeBegin = uint256(block.timestamp);

        emit Staked(msg.sender,message[1], _amount);
    }
    ...
    function exit(address staker,uint256 guildId,address guildMaster) public onlyOwner{
    ...
    function notifyRewardAmount(uint256 guildId) public view returns (uint256)
    {
        uint256 reward = earned(msg.sender,guildId);
        uint256 rewardForGuildMaster;
        uint256 totalProtocolFee=0;
        uint256 DAOFee;
        uint256 burnFee;
