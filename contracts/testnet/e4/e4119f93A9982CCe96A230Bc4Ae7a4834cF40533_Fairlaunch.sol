...
contract Fairlaunch is Ownable
{
    ...
    function UpdateWorker(uint256 workerId)  public onlyRegisteredUsers
    {
        ...
        if(curPnL < 0)
        {
            ...
            uint256 balanceOfShare = IERC20(shareToken).balanceOf(address(this));
            if(balanceOfShare > 0){
                worker.AccRewardPerShare = worker.AccRewardPerShare.add(spareReward.mul(ACC_REWARD_PRECISION).div(balanceOfShare));    
            }
