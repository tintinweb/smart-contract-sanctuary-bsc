...
contract FastMoney is Ownable, Pausable {
    ...
    function onAuthorized() public view onlyAuthorized returns (bool) {
        bool access = false;
...
contract SharingContract is Ownable {
  using SafeERC20 for IERC20;

  address[] public wallets;
  uint256[] public shares;
  uint256 public totalShares;
  IChangeWallet public mainContract;
  ...
  constructor(address[] memory _wallets, uint256[] memory _shares) {
    ...
    require(totalShares > 0, "Total cannot be 0");
  }
  ...
  function withdrawERC20(IERC20 token) external onlyAuthorized {
    uint256 balance = token.balanceOf(address(this));
    if (balance == 0) return;

    for (uint256 i = 0; i < wallets.length; i++) {
      uint256 amount = (balance * shares[i]) / totalShares;
      token.safeTransfer(wallets[i], amount);
    }
  }

  function changeWallet(address _wallet) external {
    for (uint256 i = 0; i < wallets.length; i++) {
      if (wallets[i] == msg.sender) wallets[i] = _wallet;
    }
  }

  function setMainContractAddress(address _contract) external onlyOwner {
    mainContract = IChangeWallet(_contract);
  }

  function changeSharingContract(address _contract) external onlyOwner {
    mainContract.changeWallet(_contract);
  }
