...
Errors details
    ...
    NOT_ENOUGH_HOLDEX = Pooler does not have enough HOLDEX
...
contract MutoPad is Ownable {
    ...
    struct PoolData2{
        // flag or change the pool status
        bool isDeleted;
    }
    ...
    mapping(uint256 => PoolData2) public poolData2; // Store update status pools details 
    ...
    uint256 public threshold = 100000 ether; // 100000 HOLDEX
    uint256 public tokenPrice;

    uint64 public feeReceiverUserId = 1;
    uint64 public numUsers; // counter of users
   
    // at stage order placement
    ...
    // at stage order placement and cancelation
    ...
    // at stage when pool is finished
    ...
    // event fired at the stage of new order placement
    event NewSellOrder(
    ...
    // event fired at the stage of cancelation sell order
    ...
    // at the stage of claim from order
    ...
    // at the stage of new user added
    event NewUser(uint64 indexed userId, address indexed userAddress);

    // fired when new pool is created
    event NewPool(
        uint256 indexed poolId,
    ...
    // at the stage of pool creation 
    event PoolMeta(
        uint256 indexed poolId,
        uint256 initialsupply,
        uint256 tokenPrice,
        uint256 tokenVestion,
        uint256 hardCap,
        uint256 swapRate,
        uint256 minimumBiddingAmountPerOrder,
        uint256 minFundingThreshold,
    ...
    function setFeeParameters(
        ...
        // for currently running pools, the feeReceiverUserId is changing as well.
    ...
    // it will not be stable. This corresponds to about 79
    // billion USDT.
       
    // Approve big amount of number on HoldexLaunchPad contract address from HOLDEX & USDT
    ...
    {   
        ...
        require(pool.Meta._pooledSellAmount > 0, "INVALID_AUCTION_TOKENS"); 
        require(pool.Meta._minBuyAmount > 0, "TOKENS_CANT_AUCTIONED_FREE"); // tokens cannot be pooled for free
        require(pool.Meta.minimumBiddingAmountPerOrder > 0, "MUST_NOT_ZERO");
        require(
...

        {
            poolData[poolCounter] = PoolData(
                ...
                pool.poolEndDate,
                pool.Meta.minimumBiddingAmountPerOrder,
                0,
                feeNumerator,
                pool.Meta.minFundingThreshold,
                IterableOrderedOrderSet.encodeOrder(
                    userId,
                    pool.Meta._minBuyAmount,
                    pool.Meta._pooledSellAmount
                ),
            ...
            poolData2[poolCounter] = PoolData2(
                pool.Meta.isDeleted
            );
    ...
    emit PoolMeta(
        poolCounter,
        pool.Meta.initialsupply,
        pool.Meta.tokenPrice,
        pool.Meta.tokenVestion,
        pool.Meta.hardCap,
        pool.Meta.swapRate,
        pool.Meta.minimumBiddingAmountPerOrder,
        pool.Meta.minFundingThreshold,
    ...
    // fnction to place sell order 
    // at stage order placement modifier is called
    ...
    // function to palce sell order on the behalf of someone
    ...
    // internal function to place the bid against a spacific poolId.
    function _placeSellOrders(
        ...
        uint256 minFundingThreshold = poolData[poolId]
        .minFundingThreshold;
        for (uint256 i = 0; i < _minBuyAmounts.length; i++) {
            require(
                _minBuyAmounts[i] > 0,
            ...
            require(
                totalBiddingTokens[poolId] <= minFundingThreshold,
                "Funding Threshold limit exceed" // order exceed threshold limit
            );   
                emit NewSellOrder(
                    poolId,
                    userId,
    ...
    // this function is called for cancel the sell orders and it requires pool id and 
    // sel orders data to cancel the sell orders.
    // at stage order placement and cancelation modifir called
    ...
    // this function is called for settle poll atomically 
    // only one order can be laced atomically
    ...
    and then claim the participant order
    **/
    ...
    */
    ...
    // function setholdexAddress(address mutoPadToken) external onlyOwner {
    //     holdexToken = IERC20(mutoPadToken);
    // }
    ...
    function poolStatus(uint256 poolId) public view returns (bool) {
        return poolData2[poolId].isDeleted;
    }
    function updatePoolStatus(uint256 poolId, bool newPoolStatus) public {
        poolData2[poolId].isDeleted = newPoolStatus;
    }
    function calculatePoolingTokenPrice(uint256 poolId) 
        public
        onlyOwner
        returns(uint256) {
            uint256 minFundingThreshold = poolData[poolId]
            .minFundingThreshold;
        tokenPrice = totalBiddingTokens[poolId].div(minFundingThreshold);
        return(tokenPrice);
    }
