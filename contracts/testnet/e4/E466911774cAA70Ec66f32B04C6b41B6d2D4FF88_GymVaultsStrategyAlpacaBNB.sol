    ...
    PausableUpgradeable
{
    ...
    function setBuyAndBurnAddress(address _buyAndBurnAddress) external onlyOperator {
        buyAndBurnAddress = _buyAndBurnAddress;
    }
     ...
     */
    function setBankFee(uint256 _bankFee) external onlyOperator {
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        IGymVault vault = IGymVault(vaultContractAddress);
     ...
     * @param _burnableToken: Alpaca reward token
     * @param _amount: Burnable amount
     */
    function buyAndBurn(address _burnableToken, uint256 _amount) external onlyOwner {
        IERC20Upgradeable(wantAddress).safeIncreaseAllowance(buyAndBurnAddress, _amount);
        IERC20Upgradeable(wantAddress).safeTransferFrom(address(this), buyAndBurnAddress, _amount);
    ...
    function _farm() internal {
        ...
        IGymVault(vaultContractAddress).deposit(wantAmt);
        // add ibToken to farm contract
        uint256 ibWantAmt = IERC20Upgradeable(vaultContractAddress).balanceOf(address(this));
        IERC20Upgradeable(vaultContractAddress).safeIncreaseAllowance(fairLaunchAddress, ibWantAmt);
