...
contract LibMath {
    ...
    function isRoundingError(
        ...
        return
            numerator.mul(multiple).mod(denominator).mul(1000) >=
            numerator.mul(multiple);
    }
...
contract LibOrder is EIP712, LibSignature, LibMath {
    ...
    function getOrderVersion(bytes32 data) internal pure returns (uint256) {
        return uint256(bytes1(data));
    }

    function getExpiredAtFromOrderData(bytes32 data)
        internal
        pure
        returns (uint256)
    {
    ...
    function getAsMakerFeeRateFromOrderData(bytes32 data)
        internal
        pure
        returns (uint256)
    {
    ...
    function getAsTakerFeeRateFromOrderData(bytes32 data)
        internal
        pure
        returns (uint256)
    {
    ...
    function getMakerRebateRateFromOrderData(bytes32 data)
        internal
        pure
        returns (uint256)
    {
...
// File: contracts\lib\LibRelayer.sol
...
contract LibRelayer {
    ...
    function canMatchOrdersFrom(address relayer) public view returns (bool) {
        return
            msg.sender == relayer ||
            relayerDelegates[relayer][msg.sender] == true;
    }
...
// File: contracts\lib\LibDiscount.sol
...
contract LibDiscount is LibOwnable {
     ...
     */
    function getZdexBalance(address owner)
        internal
        view
    ...
    bytes32 public discountConfig =
        0x043c000027106400004e205a000075305000009c404600000000000000000000;
     ...
     */
    function getDiscountedRate(address user)
        public
        view
        returns (uint256 result)
    {
        ...
        uint256 count = uint256(bytes1(config));
        uint256 bar;
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            if (zdexBalance < bar) {
                result = uint256(bytes1(config << ((2 + i * 5 + 4) * 8)));
                break;
            }
        }
...
// File: contracts\lib\LibExchangeErrors.sol
...
contract LibExchangeErrors {
    ...
    string constant MAKER_ORDER_CAN_NOT_BE_MARKET_ORDER =
    ...
    string constant MAKER_ONLY_ORDER_CANNOT_BE_TAKER =
        "MAKER_ONLY_ORDER_CANNOT_BE_TAKER";
}

// File: contracts\ZeedexExchange.sol
    ...
    LibExchangeErrors
{
    ...
    event Match(OrderAddressSet addressSet, MatchResult result);

    /**
     * Address of the proxy responsible for asset transfer.
     */
    address public proxyAddress = 0xb529508A67e198445FdcF78485Ee3e132CE1E66f;
    address public zdexTokenAddress =
    ...
    // LibDiscount needs Zdex Address
    constructor() public LibDiscount(zdexTokenAddress) {}
    ...
    function matchOrders(
        ...
        require(
            !isMakerOnly(takerOrderParam.data),
            MAKER_ONLY_ORDER_CANNOT_BE_TAKER
        );

        bool isParticipantRelayer = isParticipant(orderAddressSet.relayer);
        uint256 takerFeeRate = getTakerFeeRate(
            takerOrderParam,
        ...
        MatchResult[] memory results = new MatchResult[](
            makerOrderParams.length
        );
        ...
        for (uint256 i = 0; i < makerOrderParams.length; i++) {
            ...
            require(
                isSell(takerOrderParam.data) !=
                    isSell(makerOrderParams[i].data),
                INVALID_SIDE
            );
    ...
    function getOrderInfo(
        ...
        OrderAddressSet memory orderAddressSet
    ) internal view returns (OrderInfo memory orderInfo) {
        require(
            getOrderVersion(orderParam.data) == SUPPORTED_ORDER_VERSION,
            ORDER_VERSION_NOT_SUPPORTED
        );

        Order memory order = getOrderFromOrderParam(
            orderParam,
            orderAddressSet
        );
        ...
        if (
            !isMarketBuy(order.data) &&
            orderInfo.filledAmount >= order.baseTokenAmount
        ) {
            status = uint8(OrderStatus.FULLY_FILLED);
        } else if (
            isMarketBuy(order.data) &&
            orderInfo.filledAmount >= order.quoteTokenAmount
        ) {
            status = uint8(OrderStatus.FULLY_FILLED);
    ...
    function validatePrice(
        ...
        uint256 left = takerOrderParam.quoteTokenAmount.mul(
            makerOrderParam.baseTokenAmount
        );
        uint256 right = takerOrderParam.baseTokenAmount.mul(
            makerOrderParam.quoteTokenAmount
        );
        require(
            isSell(takerOrderParam.data) ? left <= right : left >= right,
            INVALID_MATCH
        );
    }
    ...
    function getMatchResult(
        ...
        result.quoteTokenFilledAmount = convertBaseToQuote(
            makerOrderParam,
        ...
        uint256 rebateRate = getMakerRebateRateFromOrderData(
            makerOrderParam.data
        );
        ...
        if (rebateRate > 0) {
            ...
            uint256 makerRawFeeRate = getAsMakerFeeRateFromOrderData(
                makerOrderParam.data
            );
            ...
            result.makerFee = result
                .quoteTokenFilledAmount
                .mul(makerFeeRate)
                .div(FEE_RATE_BASE.mul(DISCOUNT_RATE_BASE));
        }
    ...
    function getTakerFeeRate(
        ...
        return
            getFinalFeeRate(orderParam.trader, rawRate, isParticipantRelayer);
    }
