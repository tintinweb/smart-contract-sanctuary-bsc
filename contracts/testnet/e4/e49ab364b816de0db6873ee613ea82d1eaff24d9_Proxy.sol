...
contract MetaswapLaunchpad is Owned {
    ...
    address public _crypto = 0xDFCd0b91AE2BB34a90D9b4970Dc113DFaf25004d; // Cool // sell
    ...
    function buyToken(uint256 amount) public{
         if(block.timestamp > endTime || earnedCap.add(amount) > BUSDhardCap)
            isPresaleOpen = false;
        ...
        require(earnedCap.add(amount) <= BUSDhardCap,"Hard Cap Exceeds");
        require( (IToken(tokenAddress).balanceOf(address(this))).sub(soldTokens) > 0 ,"No Presale Funds left");
...
contract Proxy is Owned {
    ...
    function createPresale( address _tokenAddress,
        ...
        uint256 _poolId
         ...
         _presale[_tokenAddress] = address(new MetaswapLaunchpad(_tokenAddress,_tokenRatePerEth,_tokenRatePercrypto,_maxEthLimit,_minEthLimit,_BUSDmaxEthLimit,_BUSDminEthLimit,_hardCap,_BUSDhardCap,_poolId,msg.sender));
        }
