...
interface IQuadrixLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in Quadrix per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in Quadrix token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInQuadrix: price of a ticket in Quadrix
...
/** @title QuadrixSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract QuadrixLottery is ReentrancyGuard, IQuadrixLottery, Ownable {
    ...
    uint256 public maxPriceTicketInQuadrix = 50 ether;
    uint256 public minPriceTicketInQuadrix = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInQuadrix;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] QuadrixPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInQuadrix;
        uint32 finalNumber;
    }
     ...
     * @param _QuadrixTokenAddress: address of the Quadrix token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _QuadrixTokenAddress, address _randomGeneratorAddress) {
        QuadrixToken = IBEP20(_QuadrixTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of Quadrix to this contract
        uint256 amountQuadrixToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInQuadrix,
            _ticketNumbers.length
        );

        // Transfer Quadrix tokens to this contract
        QuadrixToken.safeTransferFrom(address(msg.sender), address(this), amountQuadrixToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInQuadrix += amountQuadrixToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInQuadrixToTransfer
        uint256 rewardInQuadrixToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInQuadrixToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        QuadrixToken.safeTransfer(msg.sender, rewardInQuadrixToTransfer);

        emit TicketsClaim(msg.sender, rewardInQuadrixToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInQuadrix) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in Quadrix for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].QuadrixPerBracket[j] =
                ...
                // A. No Quadrix to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].QuadrixPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInQuadrix - amountToShareToWinners);

        // Transfer Quadrix to treasury address
        QuadrixToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        QuadrixToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInQuadrix += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInQuadrix >= minPriceTicketInQuadrix) && (_priceTicketInQuadrix <= maxPriceTicketInQuadrix),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInQuadrix: _priceTicketInQuadrix,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            QuadrixPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInQuadrix: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(QuadrixToken), "Cannot be Quadrix token");
    ...
    /**
     * @notice Set Quadrix price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInQuadrix: minimum price of a ticket in Quadrix
     * @param _maxPriceTicketInQuadrix: maximum price of a ticket in Quadrix
     */
    function setMinAndMaxTicketPriceInQuadrix(uint256 _minPriceTicketInQuadrix, uint256 _maxPriceTicketInQuadrix)
        external
        onlyOwner
    {
        require(_minPriceTicketInQuadrix <= _maxPriceTicketInQuadrix, "minPrice must be < maxPrice");

        minPriceTicketInQuadrix = _minPriceTicketInQuadrix;
        maxPriceTicketInQuadrix = _maxPriceTicketInQuadrix;
    }
    ...
    function setTime(uint256 _Starttime,uint256 _Endtime) external onlyOwner {
           MIN_LENGTH_LOTTERY = _Starttime;
           MAX_LENGTH_LOTTERY = _Endtime;
    }
     ...
     * @param _priceTicket price of a ticket (in Quadrix)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].QuadrixPerBracket[_bracket];
        } else {
            return 0;
        }
    }
