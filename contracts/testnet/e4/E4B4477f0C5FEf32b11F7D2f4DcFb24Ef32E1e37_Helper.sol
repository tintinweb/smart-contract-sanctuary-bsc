...
contract Helper is HelperBase {
    ...
    function getStakePoolNextSettleTimeStamp(address stakePoolAddress)
        public
        view
        returns (uint256)
    {
        uint256 poolNextSettle = block.timestamp -
            ((block.timestamp - IStakePool(stakePoolAddress).poolStartTimestamp()) %
...
interface IAPHCore {
    ...
    function positions(uint256, bytes32) external view returns (CoreBase.Position memory);

    function positionConfigs(bytes32) external view returns (CoreBase.PositionConfig memory);

    function positionStates(uint256, uint256) external view returns (CoreBase.PositionState memory);

    function currentPositionIndex(uint256) external view returns (uint256);

    function pairs(bytes32) external view returns (CoreBase.Pair memory);

    function wallets(uint256, bytes32) external view returns (uint256);

    function getPosition(uint256 nftId, bytes32 pairByte)
        external
        view
        returns (CoreBase.Position memory position);

    function getPositionViewer(uint256 nftId, bytes32 pairByte)
        external
        view
        returns (CoreBase.Position memory position);

    function getPositionState(uint256 nftId)
        external
        view
        returns (CoreBase.PositionState memory positionState);
...
import "../libraries/APHLibrary.sol";
    ...
    SelectorPausableUpgradeable
{
    ...
    struct Position {
        ...
        address swapTokenAddress;
        address borrowTokenAddress;
        uint256 collateralSwappedAmount;
        uint256 borrowAmount;
        uint256 contractSize;
        uint256 interestOwePerDay;
        uint256 interestOwed;
    ...
    // futureTrading
    mapping(uint256 => mapping(bytes32 => Position)) public positions; // nftId => position byte => Position
    mapping(bytes32 => PositionConfig) public positionConfigs; // position byte => position config
    mapping(uint256 => mapping(uint256 => PositionState)) public positionStates; // nftId => position Id => positionState
    mapping(uint256 => uint256) public currentPositionIndex; // nftId => index
    mapping(bytes32 => Pair) public pairs; // position byte => Pair
    mapping(uint256 => mapping(bytes32 => uint256)) wallets; // nftId => position byte => wallet balance
    ...
    uint256 public tradingFeeToLender;
    uint256 public auctionSpread;
    uint256 public minimumMargin;
    uint256 public fixSlipPage;

    APHLibrary.Constant internal APHLib;
...
import "../stakepool/StakePoolBase.sol";
import "../../interfaces/IMembership.sol";
import "../../interfaces/IStakePool.sol";
...
library APHLibrary {
    ...
    function _sortPairs(address pairA, address pairB)
        internal
        pure
        returns (address pair0, address pair1)
    {
        require(pairA != pairB, "APHLibrary/identical-address");
        (pair0, pair1) = pairA < pairB ? (pairA, pairB) : (pairB, pairA);
        require(pair0 != address(0), "APHLibrary/zero-address");
    }

    // pair0 always be collateral token
    function _hashPair(address pair0, address pair1) internal pure returns (bytes32 pairByte) {
        (address _pair0, address _pair1) = _sortPairs(pair0, pair1);
        pairByte = keccak256(abi.encodePacked(pair0, _pair0, _pair1));
    }

    function _getNFTRank(Constant memory _constant, uint256 nftId)
        internal
        view
        returns (uint8 rank)
    {
        require(address(_constant.membership) != address(0), "APHLibrary/membership-zero-address");
        rank = IMembership(_constant.membership).getRank(nftId);
    }

    function _getNFTRankInfo(Constant memory _constant, uint256 nftId)
        internal
        view
        returns (StakePoolBase.RankInfo memory rankInfo)
    {
        rankInfo = _getRankInfo(_constant, _getNFTRank(_constant, nftId));
    }

    function _getRankInfo(Constant memory _constant, uint8 rank)
        internal
        view
        returns (StakePoolBase.RankInfo memory rankInfo)
    {
        rankInfo = IStakePool(_constant.membership.currentPool()).rankInfos(rank);
    }

    function _getUsableToken(
        Constant memory _constant,
        address owner,
        uint256 nftId
    ) internal view returns (uint256) {
        require(address(_constant.membership) != address(0), "APHLibrary/membership-zero-address");
        return IMembership(_constant.membership).usableTokenId(owner, nftId);
    }
    ...
    function _calculateSlippage(
        ...
        result = (priceDiff * _constant.WEI_PERCENT_UNIT) / expectedPrice;
    }
    ...
    function _calculateMargin(
        ...
        result = (collateral * _constant.WEI_UNIT) / contractSize;
    }
}
