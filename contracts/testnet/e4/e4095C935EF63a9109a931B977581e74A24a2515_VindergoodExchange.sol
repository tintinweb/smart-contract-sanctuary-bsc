...
contract ExchangeMain is ReentrancyGuarded, Ownable {
    ...
    function _validatePartialSellingAmount(Order memory buy, Order memory sell) internal returns(bool) { 
        bool notExceedsSellAmount = true;
        uint tokenId;
        uint buyAmount;
        ...
        assembly {
            ...
            tokenId := mload(add(sellCallData, mul(0x20, 4)))
            buyAmount := mload(add(buyCallData, mul(0x20, 5)))
            let sellAmount := mload(add(sellCallData, mul(0x20, 5)))

            if lt(sellAmount, buyAmount) {
                notExceedsSellAmount := false
            } 
        }

        (bool sellBalanceCall, bytes memory sellBalanceReturn) = address(sell.target).call(abi.encodeWithSignature("balanceOf(address,uint256)",sell.maker,tokenId));
        require(sellBalanceCall, "Exchange::Get ERC1155 balance failed!");

        uint sellBalanceAmount = abi.decode(sellBalanceReturn, (uint));
        uint remaining = sellBalanceAmount - buyAmount;

        require(sellBalanceCall && remaining >= 0, "Exchange::ERC1155 remaining is not enough!");
        require(notExceedsSellAmount, "Exchange::Exceeds sell current amount");
