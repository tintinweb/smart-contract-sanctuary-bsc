...
@dev this contract is for USDy token
     ...
     BUSD; addreBUSD     _admin address of Admin contract.
 */
contract USDy is ERC20,AccessControl,Pausable {
    ...
    uint [] public USDy_Tax_Allocation = [500, 1000]; //indexBUSDrotocol, index-2 => YSL Vault, index-3 =BUSDault; All index value multiply by 100;
    ...
    event message(uint amount,uint amounts, string message); 
    event debug(address indexed user,address indexed recipient);
    constructor(
        address _admin,
    ...
    function _transfer(
        ...
        if(sender == IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD) || recipient == IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)){
            ...
            if(IWhitelist(Admin.whitelist()).getAddressesOfSwap(user)){
                // console.log("bala before",balanceOf(recipient));
                super._transfer(sender, recipient, amount);
                // console.log("bala after",balanceOf(recipient));
                ...
                if(sender == IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)){
                    ...
                    amount -= taxAmount;
                    (uint reserve2,uint reserve3,) = IUniswapV2Pair(IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)).getReserves();
                    // console.log("uinti",reserve2 ,  reserve3);
                    
                    super._transfer(sender, recipient, amount);
                    (uint reserve22,uint reserve33,) = IUniswapV2Pair(IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)).getReserves();
                    // console.log("uint",reserve22 ,  reserve33);
                    ...
                    amount -= taxAmount;
                    (uint reserve2,uint reserve3,) = IUniswapV2Pair(IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)).getReserves();
                    emit message(reserve2, reserve3,"beforeReserve");
                    emit debug(sender,recipient);
                    super._transfer(sender, recipient, amount);

                    (uint reserve22,uint reserve33,) = IUniswapV2Pair(IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)).getReserves();
                    emit message(reserve22, reserve33,"afterReserve");
                    emit debug(sender,recipient);
    ...
    **/
    modifier _PriceImpactProtection( uint256 amount){
        if(msg.sender == ApeswapRouter){
            uint reserve0;
            uint reserve1;
            (reserve0,reserve1,) = IUniswapV2Pair(IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)).getReserves();
            
            emit message(reserve0, reserve1,"modBefore");
            ...
            (uint reserve00,uint reserve01,) = IUniswapV2Pair(IUniswapV2Factory(IUniswapV2Router02(ApeswapRouter).factory()).getPair(address(this), BUSD)).getReserves();
            
            emit message(reserve00, reserve01,"modAfter");
            ...
            require(prevAmount + ((prevAmount * priceImpactProtection)/1000) >= currentAmount && prevAmount - ((prevAmount * priceImpactProtection)/1000) <= currentAmount,
                ...
                emit message(prevAmount, currentAmount, "Amount");
            }
            emit message(prevAmount, currentAmount, "Amount");
        }else{
            _;
