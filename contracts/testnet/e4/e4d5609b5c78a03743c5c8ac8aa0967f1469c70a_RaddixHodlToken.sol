...
contract RaddixHodlToken is ERC20, ERC20Burnable, Ownable {
    ...
    function buyRHT() external payable {
        ...
        _mint(msg.sender, i*93*multiplikator/100);
        _mint(owner(), i*7*multiplikator/100);
        _mint(address(this), i*99*multiplikator);
        RHTvalue = _showRHTvalueForSell();
        tokenBurns.push(TokenBurnData(block.timestamp+burningTimeInSeconds, uint(i*99*multiplikator), burningTimeInSeconds));
    ...
    function _showRHTvalueForBuy() private view returns (uint x) {
        ...
        if(totalSupply() != 0){
            x = (balanceContract*multiplikator)/(totalSupply() - sum);
        } else x = RHTvalue;
    ...
    function _showRHTvalueForSell() private view returns (uint i) {
        if(totalSupply() != 0){
            i = (address(this).balance*multiplikator) / (totalSupply()-_TokenImagineBurnedTrigger());
        } else i = RHTvalue;
        return i;
    }
