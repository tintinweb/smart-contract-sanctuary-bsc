...
contract GOLD is ERC20, Ownable {
    ...
    function _transfer( address from, address to, uint256 amount ) internal override {
        ...
        if(takeFee) {
            ...
            if(DevelopFee>0){
                swapAndSendToDevelop(DevelopFee,from);
            }
            if(MarketFee>0){
                swapAndSendToMarket(MarketFee,from);
            }
            if(DeadFee>0){
    ...
    //分u Develop
    function swapAndSendToDevelop(uint256 tokens,address from) private  {
        uint256 initialCAKEBalance = IERC20(_rewardToken).balanceOf(address(this));
        swapTokensForCake(tokens,from);
     ...
     //分u Market
    function swapAndSendToMarket(uint256 tokens,address from) private  {
        uint256 initialCAKEBalance = IERC20(_rewardToken).balanceOf(address(this));
        swapTokensForCake(tokens,from);
        uint256 newBalance = (IERC20(_rewardToken).balanceOf(address(this))).sub(initialCAKEBalance);
    ...
    //兑换usdt到合约地址
     function swapTokensForCake(uint256 tokenAmount,address from) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, // 交易支付代币数量
            0,   // 交易获得代币最小值
            path, // 交易路径列表
            address(this), // 交易获得的 token 发送到的地址
