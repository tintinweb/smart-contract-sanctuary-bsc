...
contract MiniTokie is Context, IERC20, Ownable, Pausable {
    ...
    string private constant _name = 'TOKIE';
    string private constant _symbol = 'TOKIE';
    ...
    function feeEnabled() public view returns (bool) {
        return _feeEnabled;
    }

    function setFeeEnabled(bool enabled) public onlyOwner {
        _feeEnabled = enabled;
    ...
    function _transfer(
        ...
        if (_feeEnabled && !_isExcluded[from]) {

            _checkAndLiquify(from);
            _checkAndBuyback(from);

            uint256 amountToSend = amount;
            uint256 liquidityFeeAmount = amount.mul(_liquidityFee).div(1000);
            uint256 buybackFeeAmount = amount.mul(_buybackFee).div(1000);
            uint256 burnFeeAmount = amount.mul(_burnFee).div(1000);
            uint256 marketingFeeAmount = amount.mul(_marketingFee).div(1000);

            if (liquidityFeeAmount > 0) {
                amountToSend = amountToSend.sub(liquidityFeeAmount);
            ...
            if (buybackFeeAmount > 0) {
                amountToSend = amountToSend.sub(buybackFeeAmount);
            ...
            if (burnFeeAmount > 0) {
                amountToSend = amountToSend.sub(burnFeeAmount);
            ...
            if (marketingFeeAmount > 0 && marketingWallet != address(0)) {
                amountToSend = amountToSend.sub(marketingFeeAmount);
    ...
    function _checkAndBuyback(address from) private {
        if (buybackWallet != address(0)) {
            ...
            if (
                ...
                buybackFeeTotal = buybackFeeTotal.sub(numTokensSellToAddToLiquidity);
                uint256 ethSent = _doBuyback(numTokensSellToAddToLiquidity);

                emit BuybackSent(buybackWallet, numTokensSellToAddToLiquidity, ethSent);
            }
        }
    }
    ...
    function _checkAndLiquify(address from) private {
        ...
        if (
            ...
            //add liquidity
            liquidityFeeTotal = liquidityFeeTotal.sub(numTokensSellToAddToLiquidity);
