...
contract CoolToken is ERC20, Ownable {
    ...
    function mint(address account, uint256 amount) public onlyOwner{
        require((totalSupply()+amount) <= _maxSupply, "ERC20: maxSupply exceeded");
    ...
    function setSpecialUser() internal {
        if(block.timestamp - specialCaseHolderCheckpoint > 24 hours){
            ...
            oneInMillion = randomUser;
        }
    }

    function randomPick() internal view returns(uint256){
    ...
    function setNumTokensSellToAddToLiquidity(uint256 _numTokensSellToAddToLiquidity) onlyOwner external{
