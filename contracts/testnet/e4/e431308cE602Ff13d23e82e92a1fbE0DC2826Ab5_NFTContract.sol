...
contract NFTContract is ERC1155,Ownable,ReentrancyGuard,ERC1155Holder{
    ...
    constructor() ERC1155("https://mfarmgame.com/NFTs/{id}.json") {
        wallet1 = 0xd54477ffA42E7c52D75e6401bd9C3a0710fDcc52; //90%
        wallet2 = 0xcF5081831AbC72CDF662A18FB7d4C51220304Be2; //10%
    ...
    function createMarketSale(
        ...
        if(idToMarketItem[itemId].seller == owner()) {
            ...
            //transfers 90% to wallet1 than 10 to wallet2
            // payable(wallet1).transfer(price*90/100);
            // payable(wallet2).transfer(price*10/100);
            
        token.transferFrom(msg.sender, wallet1, price*90/100);
        token.transferFrom(msg.sender, wallet2, price*10/100);
        }
        if(idToMarketItem[itemId].seller != owner()) {
            // if the seller is not the owner
            //transfers 2.5% to owner than the 97.5% to seller
            uint256 ownerTax = price / 40; 
            uint256 sellerPart = price - ownerTax;

        token.transferFrom(msg.sender, owner(), ownerTax);
        
        token.transferFrom(msg.sender, idToMarketItem[itemId].seller, sellerPart);
        ...
        idToMarketItem[itemId].cancel = false;

        emit MarketItemSold (itemId, msg.sender,price);
    }

    function fetchItem(uint _tokenId) public view returns (MarketItem memory) {
        ...
        nonReentrant
    {
        ...
        idToMarketItem[marketItemId].price = 0;
        idToMarketItem[marketItemId].owner = payable(msg.sender);
        idToMarketItem[marketItemId].cancel = true;
    ...
    function setAddress1(address _wallet1) public onlyOwner {
        wallet1 = _wallet1;
    }

    function setAddress2(address _wallet2) public onlyOwner {
        wallet2 = _wallet2;
    }
}