    ...
    VRFConsumerBaseV2Upgradable
{
    ...
    mapping(uint256 => mapping(uint256 => Stake)) public poolIdToTokenIdToBet;
    ...
    mapping(uint256 => uint256) internal poolIdToDeletedStakesAmount;
    ...
    mapping(uint256 => mapping(uint256 => uint256))
        internal poolToDeletedTokens;
    mapping(uint256 => mapping(uint256 => uint256))
        internal poolToDeletedStakes;

    function initialize(address payable treasury_, uint64 subscriptionId_)
        public
        initializer
    {
    ...
    {
        uint256 poolId = requestIdToPoolId[requestId];
        uint256 silverAmount = requestIdToSilverAmount[requestId];
        uint256 goldAmount = requestIdToGoldAmount[requestId];

        uint256 updatedSilverAmount;
        uint256 updatedGoldAmount;
        uint256 updatedStakesAmount = poolToStakesAmount[poolId] -
            poolIdToDeletedStakesAmount[poolId];

        uint256[] memory goldArray = new uint256[](updatedGoldAmount);
        uint256[] memory silverArray = new uint256[](updatedSilverAmount);

        for (uint256 i = 0; i < silverAmount; i++) {
            if (silverArray.length == updatedSilverAmount) {
                return;
            }

            uint256 randomResult = randomWords[i] % poolToStakesAmount[poolId];

            if (poolToDeletedStakes[poolId][randomResult] != 1) {
                silverArray[i] = randomWords[i] % poolToStakesAmount[poolId];
            }
        }

        for (uint256 i = silverAmount; i < goldAmount + silverAmount; i++) {
            if (goldArray.length == updatedGoldAmount) {
                return;
            }
            ...
            if (poolToDeletedStakes[poolId][randomResult] != 1) {
                goldArray[i - silverAmount] =
                    randomWords[i] %
                    poolToStakesAmount[poolId];
            }
        }

        poolToStakesAmount[poolId] -= poolIdToDeletedStakesAmount[poolId];
        revealPool(poolId, goldArray, silverArray);
    }
        ...
        poolShouldExist(poolId_)
    {
        ...
        if (numWords > 0) {
            ...
            poolToStakesAmount[poolId_] -= poolIdToDeletedStakesAmount[poolId_];
    ...
    function withrawStake(uint256 stakeId_)
        public
        shouldBeUsersToken(betIdToPoolId[stakeId_], stakeId_)
        stageShouldNotBeFinished(betIdToPoolId[stakeId_])
    {
        uint256 poolId = betIdToPoolId[stakeId_];
        uint256[] memory stakes = poolIdToBetIdToStakes[poolId][stakeId_];

        poolToDeletedTokens[poolId][stakeId_] = 1;
        poolIdToDeletedStakesAmount[poolId] += stakes.length;

        for (uint256 i = 0; i < stakes.length; i++) {
            poolToDeletedStakes[poolId][stakes[i]] = 1;
        }

        poolToAssets[poolId] -=
            stakes.length *
            pools[poolId].stakePrice -
            (stakes.length *
                pools[poolId].stakePrice *
                CurrentEvent.commission) /
            10000;

        burn(stakeId_);
        emit StakeWithdrawn(msg.sender, stakeId_, betIdToPoolId[stakeId_]);
    }
