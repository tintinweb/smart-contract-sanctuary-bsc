...
contract MetaDapp {
    ...
    event ProductPurchase(address indexed user, address owner, uint price);
    ...
    function transferTokens(address _owner, uint _price, address _buyer) private {
        require(_price <= token.balanceOf(_buyer), 'Insuficients tokens to make transfer');
        require(token.allowance(_buyer, address(this)) >= _price, 'Insuficients allowance to make reserve');
    ...
    function buyProduct(uint product_id) public {
        ...
        emit ProductPurchase(msg.sender, product.owner, product.price);
    }
    ...
    function getUser(address userAddresses) public view returns(User memory) {
        return users[userAddresses];
    }

    function withdrowBNB(address payable account) external isOwner {
        (bool success, ) = account.call{value: address(this).balance}("");
        require(success);
    }

    function withdrow(address to, uint256 _amount) external isOwner {
        require(token.transfer(to, _amount));        
    }
