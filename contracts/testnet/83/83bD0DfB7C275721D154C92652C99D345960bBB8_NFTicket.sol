...
contract NFTicket is Initializable, OwnableUpgradeable, ERC721URIStorageUpgradeable, INFTicket {
    ...
    *******************/
    ...
    {
        string memory _msg = "only SP or owner allowed ";
        _msg = string(abi.encodePacked(_msg, Strings.toString(uint256(uint160(msg.sender)))));
        require((ticketProcessor == msg.sender ) || ( (isServiceProvider(msg.sender)) || (msg.sender == ownerOf(ticketID))),
            _msg);
    ...
    {  
        ...
        require(( (treasuryLib.isRegisteredServiceProvider(msg.sender)) || ( msg.sender == ownerOf(ticketID) ) || ( sendTo == ownerOf(ticketID) )), "only SP or owner can claim ERC20 balance");
