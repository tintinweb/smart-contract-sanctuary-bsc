...
contract TBDStaking is Ownable {
    using LowGasSafeMath for uint256;
    using LowGasSafeMath for uint32;
    using SafeERC20 for IERC20;
    using SafeERC20 for IsTBD;

    IERC20 public immutable TBD;
    IsTBD public immutable sTBD;
    ...
    constructor(
        ...
        require(_TBD != address(0));
        TBD = IERC20(_TBD);
        require(_sTBD != address(0));
        sTBD = IsTBD(_sTBD);
    ...
    /**
        @notice stake TBD to enter warmup
        @param _amount uint
        @return bool
     */
        ...
        returns (bool)
    {
        ...
        TBD.safeTransferFrom(msg.sender, address(this), _amount);
        ...
        warmupInfo[_recipient] = Claim({
            ...
            gons: info.gons.add(sTBD.gonsForBalance(_amount)),
            expiry: epoch.number.add(warmupPeriod),
            lock: false
        });

        sTBD.safeTransfer(address(warmupContract), _amount);
    ...
    function claim(address _recipient) external {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            uint256 amount = sTBD.balanceForGons(info.gons);
    ...
    /**
        @notice forfeit MEMO in warmup and retrieve TBD
     */
    function forfeit() external {
        Claim memory info = warmupInfo[msg.sender];
        delete warmupInfo[msg.sender];
        uint256 sTBDBalance = sTBD.balanceForGons(info.gons);
        warmupContract.retrieve(address(this), sTBDBalance);
        TBD.safeTransfer(msg.sender, info.deposit);
        emit LogForfeit(msg.sender, sTBDBalance, info.deposit);
    }
    ...
    function unstake(uint256 _amount, bool _trigger) external {
        ...
        sTBD.safeTransferFrom(msg.sender, address(this), _amount);
        TBD.safeTransfer(msg.sender, _amount);
    ...
    function rebase() public {
        if (epoch.endTime <= uint32(block.timestamp)) {
            sTBD.rebase(epoch.distribute, epoch.number);
            ...
            uint256 staked = sTBD.circulatingSupply();
    ...
    /**
        @notice returns contract TBD holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns (uint256) {
        return TBD.balanceOf(address(this)).add(totalBonus);
    }
