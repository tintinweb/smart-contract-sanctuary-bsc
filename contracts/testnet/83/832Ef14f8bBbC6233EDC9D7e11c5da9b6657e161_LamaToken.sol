 ...
 */
contract LamaToken is ERC20, ERC20Burnable, Ownable, Pausable, ReentrancyGuard, EmergencyGuard {
    ...
    uint256 private constant MAX_TOKEN_ALLOWED = 1_000 ether;
    ...
    * @param user address of the user than will now have to pay transaction fees.
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        require(!blacklist[_msgSender()], "You have been blacklisted from trading the LAMA Token. If you think this is an error, please contact the LAMA team.");
        // Ensure the recipient isn't blacklisted.
        require(!blacklist[recipient], "The address you are trying to send LAMA Tokens to has been blacklisted from trading the LAMA Token. If you think this is an error, please contact the LAMA team.");
    ...
    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        // Ensure the sender isn't blacklisted.
        require(!blacklist[_msgSender()], "You have been blacklisted from trading the Lama Token. If you think this is an error, please contact the Lama House team.");
        // Ensure the address where the tokens are coming from isn't blacklisted.
        require(!blacklist[from], "The address you're trying to spend the Lama Tokens from has been blacklisted from trading the Lama Token. If you think this is an error, please contact the Lama House team.");
        // Ensure the recipient isn't blacklisted.
        require(!blacklist[to], "The address you are trying to send Lama Tokens to has been blacklisted from trading the Lama Token. If you think this is an error, please contact the Lama House team.");
    ...
    function swapLamaTokensForBnb(uint256 amount, address destination) internal nonReentrant {
        // generate the uniswap pair path of token -> wbnb
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapRouter.WETH();
    ...
    */
    ...
    function isWhitelistedAtLaunch(address _address) external view returns (bool)
    {
        return whitelistedAtLaunch[_address];
    }

    function addMultiWhitelistedAtLaunch(address[] calldata addrs) public onlyOwner returns (bool success)
    {
        for (uint256 i = 0; i < addrs.length; i++) {
            if (addWhitelistedAtLaunch(addrs[i])) {
                success = true;
            }
        }
    }
