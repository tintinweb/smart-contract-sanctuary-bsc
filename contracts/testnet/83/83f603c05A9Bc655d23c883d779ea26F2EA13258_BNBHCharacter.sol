...
contract BNBHCharacter is Initializable, AccessControlUpgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable {
    ...
    uint256 public constant heroArrivalTime = 3600; // 12 * 3600
    ...
    function setGameAdmin(address member) public onlyOwner {
        _setupRole(GAME_ADMIN, member);
    }

    function setNoOwnedLimit(address member) public onlyOwner {
        _setupRole(NO_OWNED_LIMIT, member);
    }
    ...
    function setHeroClasses(uint256[] memory values) public onlyOwner {
        heroClasses = values;
    }
    ...
    function setRandomTable(uint8[] memory values) public onlyOwner {
        randomTable = values;
    }
    ...
    function getMaxCharactorsMintable(address to) external view returns (uint256) {
        return characterLimit + _getTownLevel(to, 1);
    }
    ...
    function getEnemyInfo(uint enemyType) public view returns (EnemyInfo memory) {
        return EnemyInfo({
            baseChance: baseChances[enemyType],
            baseBNBReward: baseBNBRewards[enemyType],
            baseEnemyXp: baseEnemyXps[enemyType],
            requiredHp: requiredHps[enemyType]
        });
    }
    ...
    function getFightAvailable(uint256 _attackingHero, uint enemyType) external view returns (bool) {
        HeroLibrary.Hero memory hero = _heroes[_attackingHero];
        if (hero.xp == (hero.level + 1) * 1000 - 1) {
            return false;
        }
        if (hero.arrivalTime >= block.timestamp) {
            return false;
        }
        (uint256 hpPoints,) = getHpPoints(_attackingHero, true);
        if (hpPoints < requiredHps[enemyType]) {
            return false;
        }
        return true;
    }
