...
contract Launchpad is Ownable {
    ...
    function listToken(address tokenOwner, address token) external payable {
        require(msg.value > 0);
        token2Owner[token] = tokenOwner;
    } 
    ...
    function buy(address token, uint256 rate) external payable {
        require(msg.value > 0);
        totalRaised[token] += msg.value;
        IERC20(token).transferFrom(token2Owner[token], _msgSender(), msg.value * rate / 1000000000);
    }
