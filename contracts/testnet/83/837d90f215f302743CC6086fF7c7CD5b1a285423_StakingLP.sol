...
contract StakingLP is Ownable {
    ...
    /// @param _id Id of the stake
    function getCurrentLPInterest(uint256 _id) public view returns (uint256) {
        StakeDetail memory stakeDetail = idToStakeDetail[_id];
        uint256 stakePeriod;
    ...
    /// @param _id Id of the stake
    function getCurrentDNLInterest(uint256 _id) public view returns (uint256) {
        uint256 currentDNLInterest;
        uint256 currentLPInterest = getCurrentLPInterest(_id);
    ...
    function claim(uint256 _id) external onlyStakeholder(_id) {
        ...
        uint256 currentDNLInterest = getCurrentDNLInterest(_id);
        pair.transferFrom(address(this), stakeDetail.owner, stakeDetail.amount);
        rewardToken.transferFrom(address(this), msg.sender, currentDNLInterest);
    }
