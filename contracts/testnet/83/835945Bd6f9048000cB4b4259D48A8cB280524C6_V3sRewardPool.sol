...
contract V3sRewardPool {
    ...
    uint256[] public epochTotalRewards = [600000000 ether, 400000000 ether];

    // Time when each epoch ends.
    uint256[2] public epochEndTimes;

    // Reward per second for each of 2 epochs (last item is equal to 0 - for sanity).
    uint256[3] public epochV3sPerSecond;
    ...
    function pendingV3S(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _v3sReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (_pending > 0) {
                safeV3sTransfer(_sender, _pending);
