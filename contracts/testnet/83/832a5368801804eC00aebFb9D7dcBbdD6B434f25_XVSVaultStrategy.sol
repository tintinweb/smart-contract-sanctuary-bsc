...
contract XVSVaultStrategy is XVSVaultStrategyStorage, ReentrancyGuard {
    ...
    modifier userHasPosition(address userAddress) {
        ...
        require(
            ...
            "User does not have any position in the Vault."
        );
        _;
    }

    modifier lockPeriodEnded(address userAddress) {
        UserInfo storage user = userInfo[userAddress];
        require(
            block.timestamp >= lockPeriod.add(user.lastDepositBlockNumber),
    ...
    function deposit(uint256 depositAmount) external nonReentrant {
        ...
        user.lastDepositBlockNumber = getBlockNumber();

        emit Deposit(userAddress, depositAmount);

        // Transfer user aXVS to the vault contract
        // IBEP20(axvs).safeTransferFrom(userAddress, address(this), depositAmount);

        // Transfer XVS liquidity from Atlantis to vault contract
        // IAToken(axvs)._transferLiquidityToStrategyVault(depositAmount);

        // Convert aXVS to XVS based on the current exchange rate
        uint256 exchangeRateCurrent = IAToken(axvs).exchangeRateCurrent();
        uint256 xvsAmount = depositAmount.mul(exchangeRateCurrent.div(1e18));
        IAToken(axvs)._addReserves(xvsAmount);

        // IBEP20(xvs).safeApprove(xvsVault, 0);
        // IBEP20(xvs).safeApprove(xvsVault, xvsAmount);

        // IXVSVault(xvsVault).deposit(xvs, pid, xvsAmount);
    }

    function withdrawal(uint256 withdrawAmount)
        external
        nonReentrant
        userHasPosition(msg.sender)
        lockPeriodEnded(msg.sender)
    {
        require(withdrawAmount > 0, "Withdraw amount must be non-zero");

        address userAddress = msg.sender;
        UserInfo storage user = userInfo[userAddress];
        require(user.amount >= withdrawAmount, "requested amount is invalid");

        emit Withdraw(userAddress, withdrawAmount);
        // IBEP20(xvs).safeTransfer(user.userAddress, withdrawAmount);
    }
    ...
    function _become(XVSVaultStrategyProxy xvsVaultStrategyProxy) public {
        require(
            msg.sender == xvsVaultStrategyProxy.admin(),
...
contract XVSVaultStrategyStorage is XVSVaultStrategyAdminStorage {
    ...
    address public constant axvs = address(0xd3d4F6eEe4D599BB54392c9631561D9315728A4b);
    ...
    struct UserInfo {
        address userAddress;
        uint256 amount; // amount in AToken
