...
contract AgoraBlindBox is ERC721, Member, SignHelper, OwnableOperatorRole {
    ...
    constructor()
        ERC721("Star Monster Box", "SMB", "https://img.monstar.one/smb/")
    {}
    ...
    function getPackageQuantity(uint256 packageId)
        ...
        returns (uint256)
    {
        return packageInfos[packageId].quantity;
    }
    ...
    function mint(address to, uint256 quantity) external returns (uint256) {
        ...
        uint256 packageId = totalSupply + 1;
        IShop createShop = IShop(msg.sender);
        PackageInfo storage pi = packageInfos[packageId];
        pi.blockNumber = block.number + 1;
        pi.shop = createShop;
        pi.quantity = quantity;
        IBlockhashMgr oracle = IBlockhashMgr(getMember("blockhashMgr"));
        oracle.request(pi.blockNumber);

        emit PackageCreate(to, packageId, quantity);
        _mint(to, packageId);
        return packageId;
    }
    ...
    function open(
        ...
        uint256[] memory results = pi.shop.onOpenPackage(
            msg.sender,
            packageId,
            bh
        );
        ...
        emit PackageOpen(msg.sender, packageId, results);
        return results;
    }
}
