...
// File: contracts/FlexibleFloatieVault.sol
...
interface IFloatiePool {
    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 floatieAtLastUserAction; // keep track of cake deposited at the last user action.
...
contract FloatieFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 floatieAtLastUserAction; // keeps track of floatie deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Floatie token
    IBonusToken public immutable bonusToken;
    IFloatiePool public immutable floatiePool; //Floatie pool

    uint256 public bonusMultiplier = 100; // x1 splashie reward
    ...
    //When call floatiepool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositFloatie(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * @param _floatiePool: FloatiePool contract
    ...
    constructor(
        ...
        IFloatiePool _floatiePool,
        address _admin,
        address _treasury
    ) {
        token = _token;
        bonusToken = _bonusToken;
        floatiePool = _floatiePool;
        admin = _admin;
        treasury = _treasury;

        // Infinite approve
        IERC20(_token).safeApprove(address(_floatiePool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the Floatie Flexible Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in floatie)
     */
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    floatiePool.withdrawByAmount(withdrawAmount);
        ...
        emit DepositFloatie(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Floatie Flexible Pool
    ...
    /**
     * @notice Withdraws from Floatie Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking floatie");
        staking = false;
        floatiePool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Floatie Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in FloatiePool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = floatiePool.userInfo(address(this));
        uint256 pricePerFullShare = floatiePool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into FloatiePool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            floatiePool.deposit(bal, 0);
        }
    }
}