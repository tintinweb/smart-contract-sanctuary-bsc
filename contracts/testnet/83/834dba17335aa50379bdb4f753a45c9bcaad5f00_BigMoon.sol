...
abstract contract OwnableUpgradeable is Initializable, ContextUpgradeable {
    ...
    function __Ownable_init() internal initializer {
        __Context_init_unchained(); // метод пустой 
...
contract BigMoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
   ...
   // структура настройки системы для разных уровней 
    struct FeeTier {
    ...
    // настройки комиссии в контракте с уровнями
    // добавления уровня для адрес происходит через метод whitelistAddress 
    // (похоже на маркетинговый инструмент - возможность предоставить отличные условия от тех что по умолчанию)
    // при переводе по адресу определяются комиссии
    ...
    event RewardExcluded(address account);
    event RewardIncluded(address account);
    event WhiteListIncluded(address account);
    event WhiteListExcluded(address account);
    event EcoSystemFeePercentUpdated(uint256 tierIndex, uint256 oldEcoSystemFeePercent, uint256 newEcoSystemFeePercent);
    event LiquidityFeePercentUpdated(uint256 tierIndex, uint256 oldLiquidityFeePercent, uint256 newLiquidityFeePercent);
    event TaxFeePercentUpdated(uint256 tierIndex, uint256 oldTaxFeePercent, uint256 newTaxFeePercent);
    event OwnerFeePercentUpdated(uint256 tierIndex, uint256 oldOwnerFeePercent, uint256 newOwnerFeePercent);
    event BurnFeePercentUpdated(uint256 tierIndex, uint256 oldBurnFeePercent, uint256 newBurnFeePercent);
    event EcoSystemFeeAddressUpdated(uint256 tierIndex, address oldEcoSystemAddress, address newEcoSystemAddress);
    event OwnerFeeAddressUpdated(uint256 tierIndex, address oldOwnerFeeAddress, address newOwnerFeeAddress);
    event AddressToBlacklistAdded(address account);
    event AddressToBlacklistDeleted(address account);
    event MaxTxPercentUpdated(uint256 oldMaxTxPercent, uint256 newMaxTxPercent);
    event RouterAndPairUpdated(address router, address pair);
    event NumTokensSellToAddLiquidityUpdated(uint256 numTokensSellAddToLiquidity);
    ...
    preventBlacklisted(_account, "BigMoon: Selected account is in blacklist")
    {
        require(_account != address(0), "BigMoon: Invalid address");
        _accountsTier[_account] = _tierIndex;
        emit WhiteListIncluded(_account);
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "BigMoon: Invalid address");
        require(_accountsTier[_account] > 0, "BigMoon: Account is not in whitelist");
        _accountsTier[_account] = 0;
        emit WhiteListExcluded(_account);
    }
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "BigMoon: Fees exceeded max limitation");
    ...
    function setEcoSystemFeePercent(uint256 _tierIndex, uint256 _ecoSystemFee) external onlyOwner() checkTierIndex(_tierIndex) {
        ...
        uint256 oldEcoSystemFee = feeTiers[_tierIndex].ecoSystemFee;
        feeTiers[_tierIndex].ecoSystemFee = _ecoSystemFee;
        if(_tierIndex == 0) {
            _defaultFees.ecoSystemFee = _ecoSystemFee;
        }
        emit EcoSystemFeePercentUpdated(_tierIndex, oldEcoSystemFee, _ecoSystemFee);
    }
    ...
    function setLiquidityFeePercent(uint256 _tierIndex, uint256 _liquidityFee) external onlyOwner() checkTierIndex(_tierIndex) {
        ...
        uint256 oldLiquidityFee = feeTiers[_tierIndex].liquidityFee;
        feeTiers[_tierIndex].liquidityFee = _liquidityFee;
        if(_tierIndex == 0) {
            _defaultFees.liquidityFee = _liquidityFee;
        }
        emit LiquidityFeePercentUpdated(_tierIndex, oldLiquidityFee, _liquidityFee);
    }
    ...
    function setTaxFeePercent(uint256 _tierIndex, uint256 _taxFee) external onlyOwner() checkTierIndex(_tierIndex) {
        ...
        uint256 oldTaxFee = feeTiers[_tierIndex].taxFee;
        feeTiers[_tierIndex].taxFee = _taxFee;
        if(_tierIndex == 0) {
            _defaultFees.taxFee = _taxFee;
        }
        emit TaxFeePercentUpdated(_tierIndex, oldTaxFee, _taxFee);
    }
    ...
    function setOwnerFeePercent(uint256 _tierIndex, uint256 _ownerFee) external onlyOwner() checkTierIndex(_tierIndex) {
        ...
        uint256 oldOwnerFee = feeTiers[_tierIndex].ownerFee;
        feeTiers[_tierIndex].ownerFee = _ownerFee;
        if(_tierIndex == 0) {
            _defaultFees.ownerFee = _ownerFee;
        }
        emit OwnerFeePercentUpdated(_tierIndex, oldOwnerFee, _ownerFee);
    }
    ...
    function setBurnFeePercent(uint256 _tierIndex, uint256 _burnFee) external onlyOwner() checkTierIndex(_tierIndex) {
        ...
        uint256 oldBurnFee = feeTiers[_tierIndex].burnFee;
        feeTiers[_tierIndex].burnFee = _burnFee;
        if(_tierIndex == 0) {
            _defaultFees.burnFee = _burnFee;
        }
        emit BurnFeePercentUpdated(_tierIndex, oldBurnFee, _burnFee);
    }

    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "BigMoon: Address Zero is not allowed");
        excludeFromReward(_ecoSystem);
        address oldEcoSystemAddress = feeTiers[_tierIndex].ecoSystem;
        feeTiers[_tierIndex].ecoSystem = _ecoSystem;
        if(_tierIndex == 0) {
            _defaultFees.ecoSystem = _ecoSystem;
        }
        emit EcoSystemFeeAddressUpdated(_tierIndex, oldEcoSystemAddress, _ecoSystem);
    }

    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "BigMoon: Address Zero is not allowed");
        excludeFromReward(_owner);
        address oldOwnerFeeAddress = feeTiers[_tierIndex].owner;
        feeTiers[_tierIndex].owner = _owner;
        if(_tierIndex == 0) {
            _defaultFees.owner = _owner;
        }
        emit OwnerFeeAddressUpdated(_tierIndex, oldOwnerFeeAddress, _owner);
    }
    ...
    function blacklistAddress(address account) public onlyOwner() {
        ...
        emit AddressToBlacklistAdded(account);
    }

    function unBlacklistAddress(address account) public onlyOwner() {
        _isBlacklisted[account] = false;
        emit AddressToBlacklistDeleted(account);
    }
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        ...
        emit MaxTxPercentUpdated(oldMaxTxAmount, maxTxPercent);
    }
    ...
    private
    preventBlacklisted(owner, "BigMoon: Owner address is blacklisted")
    preventBlacklisted(spender, "BigMoon: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "BigMoon: Address is blacklisted")
    preventBlacklisted(from, "BigMoon: From address is blacklisted")
    preventBlacklisted(to, "BigMoon: To address is blacklisted")
    isRouter(_msgSender())
    {
    ...
    function setNumTokensSellToAddToLiquidity(uint256 _numTokensSellAddToLiquidity) public onlyOwner() {
        numTokensSellToAddToLiquidity = _numTokensSellAddToLiquidity;
        emit NumTokensSellToAddLiquidityUpdated(_numTokensSellAddToLiquidity);
    } 
}