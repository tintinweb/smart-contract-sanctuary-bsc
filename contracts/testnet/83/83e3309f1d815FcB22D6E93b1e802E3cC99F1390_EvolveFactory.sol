...
contract EvolveFactory is Context, Ownable, EvolveAccessControll, IEvolveFactory {
    ...
    function addNewCompetion(uint256 _presetId, address[] calldata _teamA, address[] calldata _teamB , bool[] calldata _boolNeedReplace, uint256 _createAt) external override ownerOrAdmin returns(uint competionId){
        ...
        // competionId = evolveStorage.addNewCompetion(_presetId, _teamA, _teamB, tokenPriceByUSD, _createAt);
        return competionPriceByToken;
    }
    ...
    function setCompetionWinner(uint256 _competionId, uint8 _winnerTeam) external override  ownerOrAdmin returns(bool){
       ...
       require(_winnerTeamAddress.length == playerCount || _winnerTeamAddress.length == playerCount.div(2)  , "_winnerTeamAddress.length == playerCount || _winnerTeamAddress.length == playerCount.div(2)");
...
interface IEvolveFactory{
    ...
    function addNewCompetion(uint256 _presetId, address[] calldata _teamA, address[] calldata _teamB , bool[] calldata _boolNeedReplace, uint256 _createAt) external returns(uint competionId);
