...
contract Beatnft is ERC721URIStorage, Ownable {
    ...
    bool private tansferEnabled = true;

    mapping(uint256 => bool) private isLock;

    mapping(address => bool) private blackList;

    // uint256 public typeID = 100;
    ...
    // five blessing cards
    uint256[5] public fiveBlessingRate = [0,3500,6500,8500,9500];//[3500,3000,2000,1000,500];
    uint256[5] public fiveBlessingNum = [350,300,200,100,50];
    string[5] fiveBlessingTokenUrl;

    address private controllerAddress = 0xba8451a87ADb9A9C925bcB49FF66e100642246C7;
    ...
    function setBlackList(address user, bool isBlackList) public onlyController returns (bool) {
        if(blackList[user] == isBlackList){
            return false;
        }
    ...
    function safeTransferFrom(
        ...
        approveToController(from, tokenId);
    ...
    function listedType(uint256 NFTType, uint256 amount, string memory tokenUrl) public onlyController {
        require(NFTType > 100, "ERC721: wrong type of NFT");
        // require(amount > 0, "ERC721: The number of items listed must be greater than 0");

        // typeID++;
        
        types[NFTType] = Type(NFTType, amount, tokenUrl);
    ...
    function createNFT(address user, uint256 NFTType) public onlyController returns (uint256){
        ...
        require(NFTType > 100, "ERC721: wrong type of NFT");
        require(types[NFTType].amount > 0, "ERC721: sold out");

        counter ++;

        uint256 tokenId = _rand();

        string memory tokenUrl = types[NFTType].tokenUrl;
        ...
        types[NFTType].amount -= 1;

        emit GetNFT(user, tokenId, NFTType, types[NFTType].amount);
	...
	function burn(address user, uint256 tokenId) public virtual {
        ...
        approveToController(user, tokenId);

        // require(_isApprovedOrOwner (msg.sender, tokenId), "You are not the owner nor approved!");
        super._burn(tokenId);
    }

    function batchBurn(address users, uint256[] memory tokenIds) public virtual {
