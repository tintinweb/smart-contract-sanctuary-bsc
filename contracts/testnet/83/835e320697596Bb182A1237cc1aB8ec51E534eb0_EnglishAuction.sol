...
/*     event whitelistedToken(address indexed tokenaddressToWhitelist);
  ...
  function getBnbUsd() public view returns (uint) {
    ...
    uint256 decimals = bnb_usd_price_feed.decimals();
    // Add decimal places to format an 18 decimal mantissa
    uint256 priceMantissa = uint256(price).mul(10**(MANTISSA_DECIMALS.sub(decimals)));
  ...
  function convertBnbUsd(uint _amountInUsd) public view returns (uint) {
    ...
    uint FinalPrice = (_amountInUsd.mul(10 ** 8).div(BnbUsd))*(10 ** 18);
