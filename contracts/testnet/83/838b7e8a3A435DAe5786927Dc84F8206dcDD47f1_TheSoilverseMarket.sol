...
contract TheSoilverseMarket is
     ...
     */
    function addTokenIssuer(address managerWalletAddress) external whenNotPaused nonReentrant {
        /// @notice Requirements
        _requireOwner(msg.sender);
        require(managerWalletAddress != address(0), 'Issuer Address should not be zero.');

        /// @notice Updating the tokenIssuer mapping.
    ...
    /**
     * @notice Removing the tokenIssuer.
     * Who have the permission to mint & issue tokens to address.
     * @param managerWalletAddress: The issuer wallet address.
     */
    function removeTokenIssuer(address managerWalletAddress) external whenNotPaused nonReentrant {
        /// @notice Requirements
        _requireOwner(msg.sender);
        require(managerWalletAddress != address(0), 'Issuer Address should not be zero.');
        require(isManager[managerWalletAddress], 'Issuer Address not available.');
    ...
    function _requireManager(address sender) private view {
        require(isManager[sender], "Only token issuer can call.");
    }
