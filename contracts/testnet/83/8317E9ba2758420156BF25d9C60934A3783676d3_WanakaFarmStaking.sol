...
contract WanakaFarmStaking is Ownable {
    ...
    uint public minAmountLv3 = 40000;
    uint public minAmountLv2 = 20000;
    uint public minAmountLv1 = 5000;
    uint public minAmountLv0 = 500;
    ...
    function changeAmountMinStake(uint32 _level, uint256 _amount) public onlyOwner {
        require(_level == 0 || _level ==1 || _level == 2 || _level == 3, "Invaid level index");
        require(_amount > 0, "Invaid amount value");
        if(_level == 3) {
            require(_amount > minAmountLv2, "Amount must larger than minAmountLv2");
            minAmountLv3 = _amount;
        }
        if(_level == 2) {
            require(_amount > minAmountLv1 && _amount < minAmountLv3, "Amount must larger than minAmountLv1 and smaller than minAmountLv3");
            minAmountLv2 = _amount;
        }
        if(_level == 1) {
            require(_amount > minAmountLv0 && _amount < minAmountLv2, "Amount must larger than minAmountLv0 and smaller than minAmountLv2");
            minAmountLv1 = _amount;
        }
        if(_level == 0) {
            minAmountLv0 = _amount;
        }
        emit ChangeMinAmountLevel(_level, _amount);
    }

    function changeAprStake(uint32 _x, uint32 _y, uint _apr) public onlyOwner {
        uint getDetail = APR[_x][_y];
        require(getDetail > 0, "Invaid level apr");
        require(_apr >= 0, "Invaid amount value");
        APR[_x][_y] = _apr;
        emit ChangeAprValue(_x, _y, _apr);
    }

    function getAPRIndex(uint256 _amount) public view returns (uint256 _index) {
        if (_amount >= minAmountLv3 * 10**stakeToken.decimals()) {
            return 3;
        }
        if (_amount >= minAmountLv2 * 10**stakeToken.decimals()) {
            return 2;
        }
        if (_amount >= minAmountLv1 * 10**stakeToken.decimals()) {
            return 1;
        }
        if (_amount >= minAmountLv0 * 10**stakeToken.decimals()) {
            return 0;
        }
