...
contract WaiferMigrator is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  */
  ...
  function withdrawLegacyToken(address _account, uint256 _amount) external onlyOwner whenNotPaused  {
      require(_account != address(0), "Address cant be zero address");
      require(_amount > 0, "amount cannot be 0");
    // need to check v1 token balance here
      _legacyToken.safeTransferFrom(address(this), _account, _amount);
      // emit TokenFromContractTransfered(address(this), _account, _amount);
    }
}
