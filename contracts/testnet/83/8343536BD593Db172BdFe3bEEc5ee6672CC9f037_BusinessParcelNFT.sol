    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant COORDINATES_LIMIT = 5000;
    ...
    function mint(address _user, uint _x, uint _y) public onlyMunicipality returns (uint256) {
        uint parcelId = _getParcelId(_x, _y);
        _mintFor(parcelId, _user);
        return parcelId;
    }

    function getParcelId(uint _x, uint _y) external pure returns (uint parcelId) {
        parcelId = _getParcelId(_x,_y);
    }

    function getParcelCoordinates(uint _parcelId) external pure returns(uint x, uint y) {
        (x,y) = _getParcelCoordinates(_parcelId);
    }

    function parcelExists(uint _x, uint _y) external view returns(bool exists) {
        exists = _parcelExists(_x, _y);
    }

    function _getParcelId(uint _x, uint _y) internal pure returns (uint parcelId) {
        require(_x < COORDINATES_LIMIT,"StandardParcelNFT: Invalid x coordinate");
        require(_y < COORDINATES_LIMIT,"StandardParcelNFT: Invalid x coordinate");
        parcelId = _y * COORDINATES_LIMIT + _x;
    }

    function _getParcelCoordinates(uint _parcelId) internal pure returns(uint x, uint y) {
        require(_parcelId <= COORDINATES_LIMIT ** 2 + COORDINATES_LIMIT, "StandardParcelNFT: Invalid parcel id");
        x = _parcelId % COORDINATES_LIMIT;
        y = _parcelId / COORDINATES_LIMIT;
    }

    function _parcelExists(uint _x, uint _y) internal view returns(bool exists) {
        uint parcelId = _getParcelId(_x,_y);
        exists = _exists(parcelId);
    }
}
...
contract ERC721Base is IERC721Base {
    ...
    function _setMaxSupply(uint _supply) internal {
        _maxSupply = _supply;
    }
