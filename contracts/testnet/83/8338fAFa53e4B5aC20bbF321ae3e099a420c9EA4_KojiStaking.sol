...
contract KojiStaking is Ownable, Authorizable, ReentrancyGuard {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (userBalance[_msgSender()] > 0) {
            ...
            rewards.payWithdrawRewards(_msgSender(), useramount);
    ...
    function getConversionAmount(uint256 _amount, address _address) public view returns (uint256,uint256) {
        ...
        if (user0.tierAtStakeTime == 1) {
            ...
            if (user0.tierAtStakeTime == 2) {
                if(user0.amount >= tier1kojiPeg.div(2)) { 
                    bonusRate = tier1kojiPeg.div(2).mul(100).div(user0.amount); // 1B div 750M = 1.3x bonus rate
                }
                if(user0.amount >= tier2kojiPeg && user0.amount < tier1kojiPeg.div(2)) { // user is staking above tier 2 amount but still tier 2, calc bonus based on tier1
                    bonusRate = tier1kojiPeg.div(2).mul(100).div(user0.amount); // 1B div 500M = 2x bonus rate
                }
