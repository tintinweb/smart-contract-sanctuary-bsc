...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function processTokenRequests() external {
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      if (available < request.count) {
        ...
        createRandomToken(to, available, rarity, seed);
        break;
      }
      available -= request.count;
      createRandomToken(to, request.count, rarity, seed);
      requests.pop();
      if (available == 0) {
        break;
      }
  ...
  function randomizeAbilities(uint256 id) external {
    ...
    coinToken.transferFrom(to, address(this), design.getRandomizeAbilityCost(rarity, times));
  ...
  /** @title Senspark token function With BHero */
  /** Upgrade sentoken address. */
  function upgradeSenToken(address _senToken) external onlyRole(DESIGNER_ROLE) {
    senToken = IERC20(_senToken);
  }
  /** Mints tokens with SEN token. */
  function mintWithSEN(uint256 count) external {
    require(count > 0, "No token to mint");
    ...
    // Transfer coin token.
    senToken.transferFrom(to, address(this), design.getMintCost() * count);
   ...
   /** Processes token requests. */
  function processTokenRequestsWithSen() external {
    address to = msg.sender;
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      if (available < request.count) {
        request.count -= uint16(available);
        createRandomTokenWithSen(to, available, rarity, seed);
        break;
      }
      available -= request.count;
      createRandomTokenWithSen(to, request.count, rarity, seed);
      requests.pop();
      if (available == 0) {
        break;
      }
  ...
  /** Creates token(s) with a random seed. */
  function createRandomTokenWithSen(
    address to,
    uint256 count,
    uint256 rarity,
    uint256 seed
