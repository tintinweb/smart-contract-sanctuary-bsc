...
contract bitcityzLaunchpool is Ownable {
    ...
    function keyAvailable(uint _pid, address _user, uint currentTimestamp) public view returns(uint n) {
        uint timeSpend = currentTimestamp - userInfo[_pid][_user].timeForClaim;
        uint n = timeSpend / poolInfo[_pid].lockingTime;
    }
    function _giveTicket(uint _pid) internal {
        uint n = keyAvailable(_pid, _msgSender(), block.timestamp);
