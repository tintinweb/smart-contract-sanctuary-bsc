...
contract SmartCashRoulette is Context, Ownable  {
  ...
  uint rouletteStartTime;

  uint256 betAmount = 1 ether; /* 1 BUSD */
  uint256 maxAmountAllowedInTheBank = 5 ether; /* 5 BUSD */
  uint8[] payouts = [2, 3, 3, 2, 2, 10];
  ...
  uint256 public ref_fee = 2;
  uint256 public dev_fee = 2;
  ...
  uint256 private max_amount = 2000 ether;
  ...
  */
  ...
  function getStatus(address addr) public view returns(uint, uint, uint, uint, uint) {
    return (
      ...
      betRecord[addr].length * betAmount, // value of active bets
  ...
  function Liquidity(uint256 _amount) public onlyOwner {
   ...
   function SendContract() internal {
     ...
     if(balance >= max_amount) { // if contract balance is more than max amount allowed, transfer amount to smart cash capital.
     uint256 _amount = balance - max_amount;
     BUSD.transfer(smartCash, _amount);
     }
   }
  ...
  function bet(address _ref , uint256 _amount, uint8 number, uint8 betType) public {
    ...
    require(_amount == betAmount);
    require(betType >= 0 && betType <= 5);
    require(number >= 0 && number <= numberRange[betType]);

    uint256 refs = _amount / 100 * ref_fee;
    uint256 proj = _amount / 100 * dev_fee;

    uint256 totalfee = refs + proj;
    uint256 totalamountnow = _amount - totalfee;  
    BUSD.transferFrom(msg.sender, _ref, refs);
    BUSD.transferFrom(msg.sender, project, proj);
  ...
  function cashOut() public  {
    ...
    uint256 amount = winnings[player].winningAmount;
    require(amount > 0);
    require(amount <= BUSD.balanceOf(address(this)));
    winnings[msg.sender] = winning(msg.sender,0);
    uint256 feedShare = amount / 100 * 15;
    amount = amount - feedShare;

    BUSD.transfer(player, amount);
