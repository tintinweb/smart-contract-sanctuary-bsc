...
contract Naughty is Context, IBEP20, Ownable {
    ...
    address private _uniswapV2Pair; // Naughty - BNB
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "NAUGHTY::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    constructor (address treasuryAddress, address communityAddress, address liquidityAddress) public {
        require(treasuryAddress != address(0), "NAUGHTY::treasuryAddress is the zero address");
        require(communityAddress != address(0), "NAUGHTY::communityAddress is the zero address");
        require(liquidityAddress != address(0), "NAUGHTY::liquidityAddress is the zero address");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "NAUGHTY::transferOperator: new operator is the zero address");
   ...
   function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
       require(_maxTransferAmountRate <= 1000, "NAUGHTY::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate (10% of supply).");
       require(_maxTransferAmountRate >= 50, "NAUGHTY::updateMaxTransferAmountRate: Max transfer amount rate must be at least 0.5% of supply");
    ...
    function updateAutoAddLiquidityRouter(address newRouter) public onlyOperator {
        require(newRouter != address(0), "NAUGHTY::updateAutoAddLiquidityRouter: newRouter is the zero address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOperator {
        require(pair != address(0), "NAUGHTY::setAutomatedMarketMakerPair: pair is the zero address");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "NAUGHTY::setAutomatedMarketMakerPair: Automated market maker pair is already set to that value");
    ...
    function setTreasuryAddress(address treasuryAddress) external onlyOperator {
        ...
        require(treasuryAddress != address(0), "NAUGHTY::setTreasuryAddress: treasuryAddress is the zero address");
    ...
    function setCommunityAddress(address communityAddress) external onlyOperator {
        ...
        require(communityAddress != address(0), "NAUGHTY::setCommunityAddress: communityAddress is the zero address");
    ...
    function setLiquidityTaxAddress(address liquidityTaxAddress) external onlyOperator {
        require(liquidityTaxAddress != address(0), "NAUGHTY::setLiquidityTaxAddress: liquidityTaxAddress is the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "transfer from the zero address restricted");
        require(to != address(0), "transfer to the zero address restricted");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(treasuryAmount > 0){
            if(autoSellForTreasury){
                swapTokensForBnB(treasuryAmount, _treasuryAddress);
            }
            else {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "NAUGHTY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "NAUGHTY::delegateBySig: invalid nonce");
        require(now <= expiry, "NAUGHTY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "NAUGHTY::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying NAUGHTY (not scaled);
    ...
    {
        uint256 blockNumber = safe32(block.number, "NAUGHTY::_writeCheckpoint: block number exceeds 32 bits");
