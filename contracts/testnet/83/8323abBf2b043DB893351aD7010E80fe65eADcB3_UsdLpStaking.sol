...
contract UsdLpStaking is StakeInfo {
    ...
    function userInfo(address _account)
        ...
        returns (UserInfo memory _user)
    {
        _user = _userInfo[usdAddress][_account];
    ...
    function setPool(address token, uint256 _allocPoint) public onlyOwner {
        ...
        totalAllocPoint =
            ...
            _allocPoint;
        poolInfo[token].accPerShare = _allocPoint;
    }
        ...
        returns (bool)
    {
        ...
        // 领取之前的奖励
        uint256 pending = rewardAmount(msg.sender, token);
        _takeReward(msg.sender, token, pending);
    ...
    {
        uint256 pending;
        UserInfo memory _user = _userInfo[_token][_account];
        if (_user.stakedOf > 0 && poolInfo[_token].allocPoint > 0) {
            pending =
                ((_user.stakedOf * poolInfo[_token].accPerShare) / 1e12) -
                _user.rewardOf;
        }
        return pending;
    }
