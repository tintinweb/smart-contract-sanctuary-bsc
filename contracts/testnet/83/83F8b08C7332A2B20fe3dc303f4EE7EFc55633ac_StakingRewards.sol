...
contract StakingRewards is ReentrancyGuard, Pausable, TokensRecoverable {
  ...
  /*  function getRewardForDuration() external view returns (uint256) {
    ...
    function stake(address _user, uint256 amount) external nonReentrant notPaused updateReward(msg.sender) {
        ...
        emit Staked(_user, amount, lastDepositedTime);
    }

    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        require(amount > 0, "Cannot withdraw 0");
	
    	require(block.timestamp >= (lastDepositedTime).add(coolDownPeriod), "Too soon to leave.");
 ...
 /*   function setRewardsDuration(uint256 _rewardsDuration) external onlyOwner {
  ...
  //  event RewardAdded(uint256 reward);
    event Staked(address indexed user, uint256 amount, uint256 lastDepositedTime);
