...
interface ILANDRegistry {
  ...
  function assignMultipleParcels(int[] memory x, int[] memory y, address beneficiary) external;
...
interface IMetadataHolder is ERC165 {
...
contract LANDRegistry is Storage, Ownable, FullAssetRegistry, ILANDRegistry {
  ...
  function assignMultipleParcels(int[] memory x, int[] memory y, address beneficiary) external onlyDeployer {
  ...
  */
  ...
  function _isContract(address addr) override internal view returns (bool) {
    uint size;
    // solium-disable-next-line security/no-inline-assembly
    assembly { size := extcodesize(addr) }
    return size > 0;
