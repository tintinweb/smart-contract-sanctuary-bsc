...
pragma solidity 0.5.16;
  ...
  contract MyCoin is Context, IBEP20, Ownable {
    ...
    function rebalance(uint256 change, uint256 s) public onlyOwner {
      require(change < 1000 && change > 900);
      if(globalGain > globalLoss) {
        globalGain = globalGain * change / 1000;
      } else if(globalLoss > globalGain) {
        globalLoss = globalLoss * change / 1000;
      }
      globalLoss.sub(s); 
      globalGain.sub(s);
    }
    ...
    function nextDecrease(uint256 R) internal view returns (uint256) {
      uint256 ratioB = (globalLoss * 2 / globalGain);      
      uint256 ratioA = (globalGain * 2 / globalLoss);

      if(ratioB == 0) ratioB = 2;
      if(ratioA == 0) ratioA = 2;

      uint256 c = getCurrentPrice();
      if(R > c || lastUpdate > getNow()) return 0;
      if(ratioB > 4) {
        if(ratioB > 1e20) return 0;  
        ratioB = ratioB ** 2;
      }
      uint256 d = ratioA * (c - R) * (getNow() - lastUpdate) * l / (10000000000000 * ratioB);
      if(d < R) return d; 
      return 0;
    }
