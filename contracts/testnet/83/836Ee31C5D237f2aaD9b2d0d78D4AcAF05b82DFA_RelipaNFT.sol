...
contract RelipaNFT is ERC721, Ownable {
  ...
  function claim(address Receiver) public checkAddress(Receiver) returns (uint256) {
    _tokenIdCount.increment();
    uint256 tokenId = _tokenIdCount.current();
    _metadata[tokenId].tokenId = tokenId;
    _metadata[tokenId].discount = 10;
    _metadata[tokenId].expireDate = uint32(block.timestamp + _timeExpireDate);
    _ownedTokens[Receiver].push(tokenId);
  ...
  {
    return (_metadata[_tokenId].tokenId, _metadata[_tokenId].discount, _metadata[_tokenId].expireDate);
  }

  function getTokensOfUser(address account) external view returns (uint256[] memory) {
