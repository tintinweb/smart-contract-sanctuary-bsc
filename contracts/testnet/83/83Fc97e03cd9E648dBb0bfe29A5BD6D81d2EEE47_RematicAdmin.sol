    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _addLiquidity(Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal {
        ...
        uint256 liquidityRFTX = (buyTax.liquidity
            ...
            .add(tax.liquidity)).mul(IRematic(rematicAddress).balanceOf(address(this))).div(10000);
    ...
    function _getRFTXAmountForBNB(Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal pure returns(uint256 bnbRFTX){
        ...
        bnbRFTX.add(sellTax.pension);
        bnbRFTX.add(tax.pension);
        bnbRFTX.add(buyTax.legal);
        bnbRFTX.add(sellTax.legal);
        bnbRFTX.add(tax.legal);
        bnbRFTX.add(buyTax.team);
        bnbRFTX.add(sellTax.team);
        bnbRFTX.add(tax.team);
    }
    ...
    function _distributeBNB(Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal {
            ...
            if(bnbPec > 0){
                uint256 bnbRFTX = IRematic(rematicAddress).balanceOf(address(this)).mul(bnbPec).div(10000);
    ...
    function _distibuteREWARD (Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal {
        ...
        uint256 hsAmount = (buyTax.divtracker
            ...
            .add(tax.divtracker)).mul(IRematic(rematicAddress).balanceOf(address(this))).div(10000);

        _distributeRewardDividends(hsAmount);

        uint256 pRewardRFTX = buyTax.partition;
            pRewardRFTX.add(sellTax.partition);
            pRewardRFTX.add(tax.partition);
            pRewardRFTX.add(buyTax.k401);
            pRewardRFTX.add(sellTax.k401);
            pRewardRFTX.add(tax.k401);
        

        uint256 rewardAmount = swapTokensForREWARD(pRewardRFTX.mul(IRematic(rematicAddress).balanceOf(address(this))).div(10000));
