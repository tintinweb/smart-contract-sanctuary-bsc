      ...
      if (ticketAmount > 0) {
        if (lotteriesDaily[currentLotteryIdDaily].isActive == true) {
          if (ticketsDaily[sender] == 1) {
            uint256 _tOwnedBNB = getAmountOutMin(
              address(this),
              WBNB,
              _tOwned[sender]
            );
            if (_tOwnedBNB < ticketPrice){
...
*/
    ...
    isNewLotteryValidDaily
  {
    ...
    uint256 endTime = startTime_ + (numHours_ * 1 hours);
    lotteriesDaily[0] = LotteryStructDaily({
    ...
    isLotteryCompletedDaily
  {
    ...
    numTotalTicketsDaily = numTotalTicketsDaily - tickets[winningTicketDaily.addr];
    tickets[winningTicketDaily.addr] = 0;
    hodlBonusDaily[winningTicketDaily.addr] = 0;
    delete ticketDistributionDaily;
  }
