...
contract GWEICOTEST is Context, Ownable {
    ...
    constructor()  {
      ...
      token=IBEP20(0x0145634232364EbA74F6FBaCa309CeA587287c30); // test token
      Token2 = IBEP20(BUSD);
      Token3 = IBEP20(USDT);
    }
    ...
    * @dev SET SALE TYPE BY OWNER.
    */
    function setSaleTime(uint256 _startDate,uint256 _endDate,uint256 _saleType) public onlyOwner{
      require(sales[_saleType].startDate>0,"Sale is not found!"); 
      sales[_saleType].startDate = _startDate;
      sales[_saleType].endDate = _endDate;
    }
    ...
    */
    function buyToken() public payable{
      ...
      require(msg.value>=sales[saleType].minimumDeposite,"Minimum Amount Not reached");
      require(msg.value<=sales[saleType].maximumDeposite,"Maximum Amount reached");   

      uint256 numberOfTokens=bnbToToken(msg.value,BNB);        
      if(msg.value>=sales[saleType].minimumDeposite && msg.value<=100*1e18){
      ...
      }else if(msg.value>100*1e18 && msg.value<=1000*1e18){
          uint256 bonus = numberOfTokens.mul(sales[saleType].percentageOfLimit2).div(PERCENTS_DIVIDER);
          token.transfer(msg.sender,numberOfTokens.add(bonus));   
      }else if(msg.value>1000*1e18 && msg.value<=10000*1e18){
    ...
    */   
    function swapTokens(address _token,uint256 _amount) public{
      ...
      require(block.timestamp>sales[saleType].startDate,"Sale has not stared yet!"); 
      require(block.timestamp<sales[saleType].endDate,"Sale Completed"); 
      require(_amount>=sales[saleType].minimumDeposite,"Minimum Amount Not reached");
      require(_amount<=sales[saleType].maximumDeposite,"Maximum Amount reached");     
