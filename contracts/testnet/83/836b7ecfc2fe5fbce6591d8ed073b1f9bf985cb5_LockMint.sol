...
contract LockMint is ERC20,ReentrancyGuard, Ownable {
...
constructor() ERC20("wlfi", "wlfi") {
        _mint(msg.sender, 0e8);
    }

    function locktoken(IERC20 _token) public onlyOwner{
      token = _token;
    }

    function lock(address _beneficiary, uint256 _amount) public nonReentrant {
        ...
        require(token.allowance(msg.sender, address(this)) >= _amount, "The contract does not have enough of an allowance to escrow");
                require(_beneficiary != address(0), "BEP20: mint to the zero address");

        _beforeTokenTransfer(address(0), _beneficiary, _amount);
        _mint(msg.sender, _amount);
        _totalSupply += _amount;
        _balances[_beneficiary] += _amount;
        emit Transfer(address(0), _beneficiary, _amount);

        _afterTokenTransfer(address(0), _beneficiary, _amount);
        ...
        emit Lockup(msg.sender, _beneficiary, _amount);
    }
