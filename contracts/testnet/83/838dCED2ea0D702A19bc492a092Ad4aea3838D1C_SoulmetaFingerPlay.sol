...
contract SoulmetaFingerPlay is DivestorUpgradeable {
    ...
    function enterRoom(uint roomId_, uint amount_) public {
        ...
        if (info.addr1 == address(0)) {
            ...
            require(info.addr1 != _msgSender(), "same player");
            info.addr2 = _msgSender();
            price = info.amount;
        }

        soulTokenAddr.transferFrom(_msgSender(), address(this), price);
        emit EnterRoom(roomId_, _msgSender(), info.addr2 == address(0) ? 1 : 2, price);
    }
