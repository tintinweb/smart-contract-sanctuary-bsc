...
contract WALKXDividendTracker is Ownable, DividendPayingToken {
    ...
    //BUSD REWARD
    // address public _rewardTokenAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;

    address public _rewardTokenAddress = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    constructor() DividendPayingToken("WALKX_Tracker", "WALKX_Tracker", _rewardTokenAddress) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10 ** 18); 
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "WALKX_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "WALKX_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WALKXToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "WALKXToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract WALKX is ERC20, Ownable {
    ...
    WALKXDividendTracker public dividendTracker;
    ...
    uint256 buyTokenRewardsFee = 5;
    uint256 sellTokenRewardsFee = 10;
    ...
    uint256 buyInsuranceFee = 2;
    uint256 sellInsuranceFee = 4;

    uint256 public AmountLiquidityFee;
    uint256 public AmountTokenRewardsFee;
    uint256 public AmountMarketingFee;
    uint256 public AmountInsuranceFee;

    address public _marketingWalletAddress = 0x2Ee15c8039F0F1CC958e53b0cD9AB738CbA1b080;
    address public _insuranceCollector = 0xD888d9a704b919f1eff49f8fAbDb40C4B407D57c;
    ...
    constructor() ERC20("WalkSpere", "WALKX")  {

        require(buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(buyInsuranceFee) <= 25, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellInsuranceFee) <= 25, "Total sell fee is over 25%");

        uint256 totalSupply = 1000000000 * (10**18);  // 1,000,000,000
        ...
        dividendTracker = new WALKXDividendTracker();
        ...
        excludeFromFees(_insuranceCollector, true);
    ...
    function setBotBlacklist(address _botAddress, bool _flag) external onlyOwner {
        ...
        require(_botAddress != address(this) && _botAddress != uniswapV2Pair && _botAddress != address(uniswapV2Router), "cannot blacklist ESDAO Token, Pair and Router PCS");
    ...
    function setSellTxFee(uint _value) external onlyOwner {
        require(_value >= 5, "ERR : TXFee must more than 5%");
        txfee = _value;
    }

    function setTwentyFourhours(uint256 _time) external onlyOwner {
        require(_time >= 3600 && _time <= 86400, "Set Time must be between 1 and 24 hours");
    ...
    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 insuranceFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(insuranceFee) <= 20, "ERR : Total buy fee is over 20%");
        buyTokenRewardsFee = rewardsFee;
        buyLiquidityFee = liquidity;
        buyMarketingFee = marketingFee;
        buyInsuranceFee = insuranceFee;
    }

    function setSellTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 insuranceFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(insuranceFee) <= 20, "ERR : Total sell fee is over 20%");
        sellTokenRewardsFee = rewardsFee;
        sellLiquidityFee = liquidity;
        sellMarketingFee = marketingFee;
        sellInsuranceFee = insuranceFee;
    }

    function getTaxes() external view returns (uint256 _sellTax, uint256 _buyTax){
        return (buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(buyInsuranceFee), sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellInsuranceFee));
  }
    ...
    function swapManual() public onlyOwner {
        ...
        if(AmountInsuranceFee > 0) swapAndInsuranceFee(AmountInsuranceFee);
        swapping = false;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                IFee = amount.mul(buyInsuranceFee).div(100);
                AmountInsuranceFee += IFee;
                fees = LFee.add(RFee).add(MFee).add(IFee);
            }
            if(automatedMarketMakerPairs[to]){
                ...
                AmountMarketingFee += MFee;
                IFee = amount.mul(sellInsuranceFee).div(100);
                AmountInsuranceFee += IFee;
                fees = LFee.add(RFee).add(MFee).add(IFee);
    ...
    function ActivateAntiWhale(address from,uint amount) private {
        ...
        uint256 Percent = balanceOf(from).mul(txfee).div(100); //Should use variable
        require(amount <= Percent, "ERR: Can't sell more than 10%");
        ...
        if( blkTime > tradeData[from].lastTradeTime + TwentyFourhours) {
            ...
            require(tradeData[from].tradeAmount + amount <= Percent, "ERR: Can't sell more than 10% in One day");
    ...
    function swapAndInsuranceFee(uint tokens) private {
        uint initialBalance = address(this).balance;
        swapTokensForEth(tokens);
        AmountInsuranceFee = AmountInsuranceFee - tokens;
        uint ethReceived = address(this).balance.sub(initialBalance);
        payable(_insuranceCollector).transfer(ethReceived);
    }
