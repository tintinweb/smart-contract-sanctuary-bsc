...
contract Token is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20 {
    ...
    string private constant _name = "TheJam";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            require(amount <= _maxTxAmount, "M338");
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(refl).div(10 ** 4);
    }

    function calculateLiquidityFee(
        uint256 _amount
    ) private view returns (uint256) {
        return _amount.mul(liq).div(10 ** 4);
    }
