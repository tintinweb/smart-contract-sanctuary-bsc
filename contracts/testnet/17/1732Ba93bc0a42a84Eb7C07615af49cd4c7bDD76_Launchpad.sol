...
contract Presale is Whitelist {
    ...
    // Workaround until I fix this shit of contract
    uint256[16] public inputs;
    string[10] public socials;
    ...
    function getPresaleInfo() public view returns(address, string memory, uint8, string memory, uint256, uint256, address, uint256[16] memory, string[10] memory) {
        IERC20Metadata tokenMetadata = IERC20Metadata(address(token));
        return (address(token), tokenMetadata.name(), tokenMetadata.decimals(), tokenMetadata.symbol(), tokenMetadata.totalSupply(), address(this).balance, owner, inputs, socials);
    }
    ...
    function _buy(address _for, uint256 _value) internal {
        ...
        if(!isPublic) {
            require(isWhitelisted(_for), "Your address is not whitelisted");
        }
