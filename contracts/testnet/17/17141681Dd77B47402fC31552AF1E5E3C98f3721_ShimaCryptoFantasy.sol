...
contract ShimaCryptoFantasy is Ownable, AccessControl {
    ...
    constructor() {
        ...
        buybackToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    }
    ...
    external payable
    {
        ...
        require(msg.sender.balance > msg.value, "Insufficient wallet funds");

        IWETH(WBNB_ADDRESS).deposit{value : pools[_poolID].entryFee}();
    ...
    function setWinner(
        ...
        require(sum <= pools[_poolID].pot, "Rewards > pool pot");
    ...
    function claimReward(uint256 _poolID, uint256 position) external {
        ...
        require(
            ...
            "Not winner for this position"
        );

        uint256 _marketingFee =
        SafeMath.div(SafeMath.mul(winnerDetails[_poolID].amount[position], marketingFee), 10000);
        ...
        uint256 payout =
        SafeMath.sub(winnerDetails[_poolID].amount[position], SafeMath.add(_marketingFee, _buybackFee));
        ...
        IWETH(WBNB_ADDRESS).withdraw(_marketingFee);
        (bool success,) = payable(marketingWallet).call{value : _marketingFee, gas : 30000}("");

        require(success, "Marketing tx must succeed");
    ...
    function swapBNBForTokenAndSend(address winner, address token, uint256 bnbAmount) private returns (uint256) {
        ...
        IWETH(WBNB_ADDRESS).withdraw(bnbAmount);
