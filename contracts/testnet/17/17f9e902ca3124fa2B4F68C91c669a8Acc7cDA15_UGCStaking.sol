...
contract UGCStaking {
    ...
    function buy(
        ...
        if (_plan > 0) {
            // require(
            //     user.plan_purchased[_plan] == false,
            //     "Plan is already purchased"
            // );
            //if we want to restric somone from buying multiple times
            require(
    ...
    function get_user_total_deposits_count(address _user) external view returns (uint256 count) {
    ...
    function get_user_ref_team_turnover_per_level(address _user,uint256 level) external view returns (uint256 count) {
        return users[_user].ref_teamturnover_per_level[level];
    }
    function get_user_stake_team_turnover_per_level(address _user,uint256 level) external view returns (uint256 count) {
        return users[_user].stake_teamturnover_per_level[level];
    }
    function get_user_plan_purchased(address user,uint256 level) external view returns (bool result) {
        return users[user].plan_purchased[level];
    }
    function get_directs(address user) external view returns (uint256 length,address [] memory directs) {
        return (users[user].directs.length,users[user].directs);
    }
    function get_user_deposit_details(address user,uint256 index) external view returns (Deposit memory details){
