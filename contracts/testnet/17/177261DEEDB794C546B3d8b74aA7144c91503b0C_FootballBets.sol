...
contract FootballBets is Ownable {
    ...
    struct Ticket {
        ...
        uint256 bettingTime;
        uint256 claimedTime;
        uint8 status; // 0-PENDING 1-WIN 2-LOSE 3-WIN-HALF 4-LOSE-HALF 9-REFUND
    }
    ...
    function addNewMatch(uint256 _matchId, string memory _title, string memory _teamA, string memory _teamB, uint256 _startBettingTime,
        ...
        require(_odds1x2[0] > oddsRate && _odds1x2[1] > oddsRate && _odds1x2[2] > oddsRate, "_odds1x2 must be greater than x1");
        require(_oddsHandicap[0] > oddsRate && _oddsHandicap[1] > oddsRate, "_oddsHandicap must be greater than x1");
        require(_oddsOverUnder[0] > oddsRate && _oddsOverUnder[1] > oddsRate, "_oddsOverUnder must be greater than x1");
    ...
    function claimPayout(uint256 _ticketIndex) external checkBlackList returns (address _player, uint256 _payout) {
        ...
        emit DrawTicket(_player, _ticketIndex, _matchId, _betTypeId, _payout, block.timestamp);
    }
