...
contract Chickly is Ownable {
	...
	constructor(address busd, address payable marketingAddr, address payable projectAddr, uint32 startDate, string memory baseURI, uint16[] memory _percents) Ownable(msg.sender) {
        BUSD = IERC20(busd);
    ...
    function getUserInfo(address userAddress) public view returns(uint256 amountOfDeposits, uint256[2] memory invested, bool[] memory active, uint256[2] memory available, uint256[2] memory remaining, uint256 holdBonus, uint256 userPercent, uint256 lastWithdrawal, uint256 nextFeed) {
        User memory user = users[userAddress];
