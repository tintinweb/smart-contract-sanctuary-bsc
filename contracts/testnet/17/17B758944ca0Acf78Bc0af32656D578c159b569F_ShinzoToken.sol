...
abstract contract ShinzoTokenBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "Shinzo Token";
	string private constant SYMBOL = "Shinzo";
	...
	uint256 private constant _totalTokens = 100000000000000000 * 10**DECIMALS;	//1 quadrillion total supply
	...
	uint256 private _tokenSwapThreshold = _totalTokens / 1000000; //There should be at least 0.000001% of the total supply in the contract before triggering a swap
    ...
    // marketing & other wallet address
	address private _ownerWallet = 0x722B009BD87B180f82a2f3c7Af6a52E0F8f84d92;
	address private _yearWallet = 0x1c1d8B5488bbC33a93b0b5e7e3016e47Aa942929;
	address private _marketingWallet = 0xf4724071CAB2861DFBda028E88BC86ea3D192C21;
	address private _charityWallet = 0x5b038478ae56796aEcBbFF57A6AaF9d267F5de6C;
	...
	constructor (address routerAddress, address pinkAntiBot_) {
		_balances[_marketingWallet] = totalSupply() * 10 / 100;
		_balances[_charityWallet] = totalSupply() * 5 / 100;
		_balances[_yearWallet] = totalSupply() * 10 / 100;
		_balances[_ownerWallet] = totalSupply() * 75 / 100;

		
		// Exclude contract from fees
		_addressesExcludedFromFees[address(this)] = true;

		// Initialize PancakeSwap V2 router and Shinzo <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
		setPancakeSwapRouter(routerAddress);

		// 3% liquidity fee, 10% reward fee, 1% buyback fee, 2% marketing fee
		setFees(3, 10, 1, 2);

		emit Transfer(address(0), _ownerWallet, totalSupply() * 75 / 100);
		emit Transfer(address(0), _yearWallet, totalSupply() * 10 / 100);
		emit Transfer(address(0), _charityWallet, totalSupply() * 5 / 100);
		emit Transfer(address(0), _marketingWallet, totalSupply() * 10 / 100);
	...
	function executeSwapIfNeeded(address sender, address recipient) private {
		...
		if (tokensAvailableForSwap >= _tokenSwapThreshold) {
			...
			bool isBuying = isPancakeSwapPair(sender);
	...
	function executeSwap(uint256 amount) private {
		...
		swapBNBForCustomeTokens(busdToken, bnbToBeSendToMarketing, _marketingWallet);
	...
	// This function swaps a {tokenAmount} of Shinzo tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for Shinzo -> WBNB
	...
	// Returns how many more $Shinzo tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee) public onlyOwner {
		...
		require(liquidityFee + rewardFee + buybackFee + marketingFee <= 30, "Total fees cannot exceed 30%");
	...
	function setBUSDTokenAddress(address busd) public onlyOwner {
		busdToken = busd;
	}
...
contract ShinzoToken is ShinzoTokenBase {
	...
	uint256 private _totalBNBAsShinzoClaimed; // The total number of BNB that was converted to Shinzo and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsBUSDClaimed; // The amount of BNB converted to Shinzo and claimed by each address
	...
	uint256 private _maxClaimAllowed = 10 ether; // Can only claim up to 100 bnb at a time.
	...
	bool private _reimburseAfterShinzoClaimFailure; // If true, and Shinzo reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress, address pinkAntiBot_) ShinzoTokenBase(routerAddress, pinkAntiBot_) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterShinzoClaimFailure(true);
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		if(!isTradingEnabled) {
			require(!isPancakeSwapPair(recipient), "Shinzo: Trading is currently disabled.");
			require(!isPancakeSwapPair(sender), "Shinzo: Trading is currently disabled.");
		}
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough Shinzo balance");
	...
	function doClaimReward(address user) private returns (bool) {
		...
		if (!claimRewardToken(user, claimBnbAsTokens)) {
			...
			if (_reimburseAfterShinzoClaimFailure) {
	...
	function claimRewardToken(address user, uint256 bnbAmount) private returns (bool) {
		...
		bool success = swapBNBForCustomeTokens(claimRewardAsToken(), bnbAmount, user);
		if (!success) {
			return false;
		}

		_bnbAsBUSDClaimed[user] += bnbAmount;
		_totalBNBAsShinzoClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $Shinzo on a zero balance address:
		...
		// Depending on the % of $Shinzo tokens transferred, relative to the balance, we might need to extend the period
    ...
    function totalBNBClaimedAsShinzo() public view returns (uint256) {
		return _totalBNBAsShinzoClaimed;
	}
	...
	function reimburseAfterShinzoClaimFailure() public view returns(bool) {
		return _reimburseAfterShinzoClaimFailure;
	}


	function setReimburseAfterShinzoClaimFailure(bool value) public onlyOwner {
		_reimburseAfterShinzoClaimFailure = value;
	}
	...
	function claimRewardAsToken() public view returns(address) {
		return busdToken;
	}
