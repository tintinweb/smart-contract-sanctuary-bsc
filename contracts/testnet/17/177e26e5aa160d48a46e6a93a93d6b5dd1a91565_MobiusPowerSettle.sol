...
contract MobiusPowerSettle is Modifier, Util {

    mapping(address => uint256) private settleType;
    mapping(address => uint256) private transferValue;

    constructor() {
    }

    // _settleType 0,1,2
    function settle(uint256 _settleType) public isRunning returns (bool) {
        
        settleType[msg.sender] = _settleType;

        return true;
    }

    function transfer(uint256 amountToWei, address _address) public isRunning returns (bool) {
        
        transferValue[msg.sender] = amountToWei;
