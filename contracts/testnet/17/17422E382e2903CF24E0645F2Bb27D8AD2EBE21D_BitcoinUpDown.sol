...
contract BitcoinUpDown is Ownable {
    ...
    function startBet() internal onlyOwner {
        prize_pool.push(0);
    ...
    function withdraw() public {
        require(players_bet[msg.sender].isExist); // dev: is three
        ...
        if (
            ...
            uint256 amount = (((players_bet[msg.sender].bet * 100) /
                prize_pool_up[players_bet[msg.sender].round]) *
                prize_pool[players_bet[msg.sender].round]) / 100;
            (bool sent, bytes memory data) = msg.sender.call{value: amount}("");
            require(sent); // dev:withdraw failed
        }
    }
