...
// Root file: contracts/staking-v2/WWWEBFOUR.sol
...
contract WWWEBFOUR is ERC20, Pausable, AccessControl, Withdrawable {
    using SafeERC20 for IERC20;
    ...
    // Safe WEBFOUR transfer function, just in case if rounding error causes pool to not have enough CAKEs.
    function safeWEBFIVETransfer(address _to, uint256 _amount)
        external
        whenNotPaused
        onlyAdminOrMinter
    {
        uint256 WEBFIVEBal = IERC20(WEBFIVEAddress).balanceOf(address(this));
        if (_amount > WEBFIVEBal) {
            IERC20(WEBFIVEAddress).safeTransfer(_to, WEBFIVEBal);
        } else {
            IERC20(WEBFIVEAddress).safeTransfer(_to, _amount);
        }
    }
