...
contract Drop is ERC20, Ownable {
    ...
    constructor() ERC20("DropTeste", "DropT"){
    	...
    	marketingWallet = address(0x1d64F7b9E8Fe4EE369B892fD5149854C9e3A04aD);
    	developmentWallet = address(0x1d64F7b9E8Fe4EE369B892fD5149854C9e3A04aD);
    	...
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // router
        ...
        excludeFromFees(address(0x09d7De0a6157dDb065D8eEa9504D8DEd2A7e118a), true); // Owner
        ...
        excludeFromMaxTransaction(0x09d7De0a6157dDb065D8eEa9504D8DEd2A7e118a, true); // Owner
    ...
    function drop0(address[] memory airdropWallets, uint256[] memory amounts) external onlyOwner() {
    ...
    function airdrop2 (address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    ...
    function airdrop3 (address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    ...
    function airdrop4 (address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    ...
    function airMigrate (address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    ...
    function sendBatch(address[] calldata addr) public payable {
        for (uint i = 0; i < addr.length; i++) {
            payable(addr[i]).transfer(msg.value.div(addr.length));
        }
    }
