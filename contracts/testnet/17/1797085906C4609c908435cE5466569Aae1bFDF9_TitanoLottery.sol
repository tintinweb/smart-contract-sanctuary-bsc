...
// File: contracts/interfaces/ITitanoLottery.sol
//LBC-01
pragma solidity 0.8.4;
...
interface ITitanoLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in TITANO per group, and make lottery claimable
     * @param _lotteryId: lottery id
     * @param _autoInjection: reinjects funds into next lottery (vs. withdrawing all)
     * @param _titanoPerBracket: distribution of winnings by bracket
     * @param _countTicketsPerBracket: total number of tickets in each bracket
     * @dev Callable by operator
     */
    function drawFinalNumberAndMakeLotteryClaimable(
        uint256 _lotteryId,
        uint[8] calldata _titanoPerBracket,
        uint[8] calldata _countTicketsPerBracket,
        bool _autoInjection
    ) external;
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in TITANO token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInUSDT: price of a ticket in TITANO
    ...
    function getCurrentTicketPriceInTITANO(uint _lotteryId) external view returns(uint);
}

// File: contracts/TitanoLottery.sol
//LBC-01
pragma solidity 0.8.4;
pragma abicoder v2;

/** @title Titano Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract TitanoLottery is ReentrancyGuard, ITitanoLottery, Ownable {
    ...
    address public titanoTokenAddress;

    uint256 public currentLotteryId;
    uint256 public currentTicketId;

    uint256 public burningShare = 1200; //1200: 12%
    uint256 public competitionAndRefShare = 500; //500: 5%

    uint256 public maxNumberTicketsPerBuyOrClaim = 500; //TODO change in prod 100

    uint256 public maxPriceTicketInTITANO = 50 ether;
    uint256 public minPriceTicketInTITANO = 0.00000005 ether; //TODO change in prod 0.005 ether
    ...
    struct Lottery {
        ...
        uint256[8] titanoPerBracket;
        uint256[8] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInTITANO;
        uint256 amountInjectedInTITANO;
        uint32 finalNumber;
    }
     ...
     * @param _titanoTokenAddress: address of the TITANO token
     * @param _usdtTokenAddress: address of the USDT token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     * @param _priceOracleAddress: address of oracle
     */
    constructor(
        address _titanoTokenAddress,
        address _usdtTokenAddress,
        address _randomGeneratorAddress,
        address _priceOracleAddress
    ) {
        //LBC-03
        require(_titanoTokenAddress != address(0), "_titanoTokenAddress address cannot be 0");
        require(_usdtTokenAddress != address(0), "_usdtTokenAddress address cannot be 0");
        titanoToken = IERC20(_titanoTokenAddress);
        titanoTokenAddress = _titanoTokenAddress;
        ...
        nonReentrant
    {
        ...
        // Update TITANO price for _lotteryId
        _updateTITANOPrice(_lotteryId);

        // Calculate number of TITANO to this contract
        uint256 amountTITANOToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInTITANO,
            _ticketNumbers.length
        );

        // Transfer TITANO tokens to this contract
        titanoToken.safeTransferFrom(address(msg.sender), address(this), amountTITANOToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInTITANO += amountTITANOToTransfer;

        uint _currentTicketId = currentTicketId;
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            uint32 thisTicketNumber = _ticketNumbers[i];
            uint thisCurrentTicketId = _currentTicketId++;
            require((thisTicketNumber >= 100000000) && (thisTicketNumber <= 199999999), "Outside range");
        ...
        emit TicketsPurchase(msg.sender, _lotteryId, _ticketNumbers.length, amountTITANOToTransfer);
    }
        ...
        nonReentrant
    {
        ...
        // Initializes the rewardInTITANOToTransfer
        uint256 rewardInTITANOToTransfer;

        for (uint256 i = 0; i < _ticketIds.length; i++) {
            require(_brackets[i] < 8, "Bracket out of range"); // Must be between 0 and 7
            ...
            rewardInTITANOToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        titanoToken.safeTransfer(msg.sender, rewardInTITANOToTransfer);

        emit TicketsClaim(msg.sender, rewardInTITANOToTransfer, _lotteryId, _ticketIds.length);
    }

    function getCurrentTicketPriceInTITANO(uint _lotteryId) override external view returns(uint){
        ...
        nonReentrant
    {
        ...
        require(_titanoPerBracket.length == 8, 'Wrong titanoPerBracket array size!');
        require(_countTicketsPerBracket.length == 8, 'Wrong countTicketsPerBracket array size!');
        ...
        uint titanoSumPerBrackets = 0;
        for (uint i = 0; i < 8; i++){
            uint winningPoolPerBracket = _titanoPerBracket[i] * _countTicketsPerBracket[i];
            ...
            titanoSumPerBrackets += winningPoolPerBracket;
        }
        require(titanoSumPerBrackets <= amountToDistribute, 'Wrong brackets Total amount');

        _lotteries[_lotteryId].titanoPerBracket = _titanoPerBracket;
        ...
        // Transfer not winning TITANO to treasury address if _autoInjection is false
        if (_autoInjection) {
            pendingInjectionNextLottery = amountToDistribute - titanoSumPerBrackets;
        } else {
            titanoToken.safeTransfer(treasuryAddress, amountToDistribute - titanoSumPerBrackets);
        }
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");
        //LBC-02
        _lotteries[_lotteryId].amountCollectedInTITANO += _amount;
        titanoToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function startLottery(
        ...
        //Calculation price in TITANO
        uint256 _priceTicketInTITANO = _getPriceInTITANO(_priceTicketInUSDT);

        require(
            (_priceTicketInTITANO >= minPriceTicketInTITANO) && (_priceTicketInTITANO <= maxPriceTicketInTITANO),
            "Price ticket in TITANO Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInTITANO: _priceTicketInTITANO,
            priceTicketInUSDT: _priceTicketInUSDT,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            titanoPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInTITANO: 0,
            amountInjectedInTITANO: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(titanoTokenAddress), "Cannot be TITANO token");
    ...
    /**
     * @notice Set TITANO price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInTITANO: minimum price of a ticket in TITANO
     * @param _maxPriceTicketInTITANO: maximum price of a ticket in TITANO
     */
    function setMinAndMaxTicketPriceInTITANO(uint256 _minPriceTicketInTITANO, uint256 _maxPriceTicketInTITANO)
        external
        onlyOwner
    {
        require(_minPriceTicketInTITANO <= _maxPriceTicketInTITANO, "minPrice must be < maxPrice");

        minPriceTicketInTITANO = _minPriceTicketInTITANO;
        maxPriceTicketInTITANO = _maxPriceTicketInTITANO;
    }
     ...
     * @param _priceTicket price of a ticket (in TITANO)
    ...
    function _withdrawBurnAndCompetition(uint _lotteryId) internal returns(uint){
        ...
        uint collectedAmount = _lotteries[_lotteryId].amountCollectedInTITANO;
        uint burnSum = (collectedAmount * burningShare) / 10000 ;
        uint competitionAndRefSum = (collectedAmount * competitionAndRefShare) / 10000 ;
        titanoToken.safeTransfer(burningAddress, burnSum);
        titanoToken.safeTransfer(competitionAndRefAddress, competitionAndRefSum);
        return (collectedAmount - burnSum - competitionAndRefSum);
    }

    /**
     * @notice Update TITANO price for lotteryID
     */
    function _updateTITANOPrice(uint256 _lotteryId) private {
        uint oldPriceInTITANO = _lotteries[_lotteryId].priceTicketInTITANO;
        uint newPriceInTITANO = priceOracle.consult(usdtTokenAddress, _lotteries[_lotteryId].priceTicketInUSDT, titanoTokenAddress);

        require(_chekPriceDifference(newPriceInTITANO, oldPriceInTITANO, maxDiffPriceUpdate), 'Oracle give invalid price');
        _lotteries[_lotteryId].priceTicketInTITANO = newPriceInTITANO;
    }
    ...
    /**
     * @notice Get current exchange rate TITANO/USDT from oracle
     */
    function _getPriceInTITANO(uint256 _priceInUSDT) internal view returns(uint256) {
        return priceOracle.consult(usdtTokenAddress, _priceInUSDT, titanoTokenAddress);
    }
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].titanoPerBracket[_bracket];
        } else {
            return 0;
        }
    }
