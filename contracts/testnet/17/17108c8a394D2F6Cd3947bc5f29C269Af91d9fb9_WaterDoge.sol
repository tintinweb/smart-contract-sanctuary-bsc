...
/**
TRUST CRONOS TOKEN CONTRACT

CONTRACT ADDRESS DEVELOPED 

https://t.me/WaterDoge
*/
...
contract WaterDoge is ERC20, Ownable {
    ...
    event ExcludedFromMaxTxEvt(address indexed account, bool isExcluded);
    ...
    constructor() ERC20("WaterDoge", "WTDoge") {
        marketingWallet = payable(0x98DAc74Ab3C86572eC6B6f82D2C7d0Fb2C69fa24);
        devWallet = payable(0xc5797C5eB4D1d55cEAf300C7E24B17052A7d6Aa5);
    ...
    function excludeFromMaxTx(address account, bool excluded) public onlyOwner {
        ...
        emit ExcludedFromMaxTxEvt(account, excluded);
    }
    ...
    function setcooldowntimer(bool state, uint256 time) external onlyOwner {
        cooldowntimer = time * 1 seconds;
    ...
    // setmaxSell Limit
    function updateMaxSellLimit(uint256 maxSell) external onlyOwner{
        maxSell = maxSell * (10**18);
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            require(amount <= maxWallet, "Exceeds maximum wallet token amount." );
            require(balanceOf(to) + amount <= maxTransactionAmount, "You are trying to send more than maxTX");
            require(contractBalanceRecipient + amount <= maxTransactionAmount, "Exceeds maximum TX transaction amount");
            }

        if(from != uniswapV2Pair && !_isExcludedFromFees[to] && !_isExcludedFromFees[from] && !swapping){
                require(amount <= maxSellLimit, "You are exceeding maxSellLimit");
                if(to != uniswapV2Pair){
                    require(balanceOf(to) + amount <= maxWallet, "You are exceeding maxWalletLimit");
                }
                if(coolDownEnabled){
