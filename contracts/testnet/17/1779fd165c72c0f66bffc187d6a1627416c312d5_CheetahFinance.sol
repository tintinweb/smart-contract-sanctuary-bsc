...
contract CheetahFinance is ERC20, Ownable {
        ...
        returns (uint256 _baseDelta)
    {
        ...
        for (uint256 i = claimed; i < loops; i++) {
            ...
            uint256 gonAmount;
            if (userNFT == 0) {
                _userAddedFunds = (balanceOf(userInd).mul(__bonus.noNFT)).div(100E18);
                // userAddedFunds = (_userAddedFunds.mul(__bonus.noNFT)).div(1E18);
                _baseDelta = _baseDelta.add(_userAddedFunds);
                gonAmount = _userAddedFunds.mul(_gonsPerFragment);
                _gonBalances[userInd] = _gonBalances[userInd].add(gonAmount);
            }
            if (userNFT == 1) {
                _userAddedFunds = (balanceOf(userInd).mul(__bonus.oneNFT)).div(100E18);
            ...
            if (userNFT == 2) {
                _userAddedFunds = (balanceOf(userInd).mul(__bonus.twoNFTs)).div(100E18);
            ...
            if (userNFT == 3) {
                _userAddedFunds = (balanceOf(userInd).mul(__bonus.threeNFTs)).div(100E18);
            ...
            if (userNFT == 4) {
                _userAddedFunds = (balanceOf(userInd).mul(__bonus.fourNFTs)).div(100E18);
            ...
            if (userNFT >= 5) {
                _userAddedFunds = (balanceOf(userInd).mul(__bonus.fiveNFTs)).div(100E18);
    ...
    function rebase(uint256 numberToSupply, int256 supplyDelta)
        external
        onlyOwner
        returns (uint256)
    {
        ...
        if (supplyDelta < 0) {
            ...
            uint256 newSupply = process(numberToSupply);
