    ...
    ERC721Holder
{
    ...
    function generateAccount(
        ...
        address[] calldata approveERC721
        ...
        setApproveDelegate(approveERC721, createdAccount);
    ...
    function createOrder(
        ...
        if (_orderType == OrderType.ERC721) {
            ...
            require(
                IAccountPlayer(_target).currentowner() == msg.sender,
                "CANAEFVAE"
            );
    ...
    // function iterateArr(address[] memory nfts)
    //     internal
    //     returns (address[] memory)
    // {
    //     address[] memory arraySet = new address[](10);
    //     uint256 count = 0;
    //     bool start = false;
    //     for (uint256 i = 0; i < nfts.length; i++) {
    //         for (uint256 j = 0; j < arraySet.length; j++) {
    //             if (nfts[i] == arraySet[j]) {
    //                 start = true;
    //             }
    //         }
    //         count++;
    //         if (count == 1 && start == false) {
    //             arraySet.push(nfts[i]);
    //         }
    //         start = false;
    //         count = 0;
    //     }
    // }
    function setApproveDelegate(address[] calldata nfts, address target)
        public
    {
        for (uint256 i = 0; i < nfts.length; i++) {
            (bool success, bytes memory data) = nfts[i].delegatecall(
                abi.encodeWithSignature("setApprovalForAll", target, true)
            );
        }
    }
