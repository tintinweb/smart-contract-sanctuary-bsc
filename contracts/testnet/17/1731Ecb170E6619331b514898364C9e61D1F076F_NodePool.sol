...
abstract contract ERC20 {
    ...
    function endAfterY2N(uint nodeId,uint nodeTotalLP,uint _scale,uint totalEffective) external virtual;
    function endAfterY2N(uint nodeId,uint nodeTotalLP,uint _scale) external virtual;
    function queryUsdtToSeedPrice() external virtual view returns (uint256);//1 U = ? token
}
...
contract NodePool is Modifier {
    ...
    mapping(uint => string) public nodeNameMapping;              //<节点ID,名称>
     ...
     * @dev 创建 | 所有人调用 | 创建节点
     */
    function createNode(string memory name) public nonReentrant returns (uint nodeId){
        if(nodeMasterMapping[msg.sender] != 0){ _status = _NOT_ENTERED; revert("NodePool : Node already exists"); }
        uint pledgePrice = ERC20(tokenAddress).queryUsdtToSeedPrice(); // 单价 : 一个USDT 等值的 Token
     ...
     */
    function queryRankingTop() public view returns (uint totalOutPut,Node[] memory rankingArray,string[] memory nameArray){
        if(firstNodeId != 0){
            uint nodeLength = 0;
            uint startId = firstNodeId;
            do{
            ...
            if(firstNodeId != lastNodeId && nodeLength < produceRankingMin){
                nodeLength++;
            }

            rankingArray = new Node[](nodeLength);
            nameArray = new string[](nodeLength);
            nodeLength = 0;
            startId = firstNodeId;
            do{
                rankingArray[nodeLength++] = nodePoolMapping[startId];
                nameArray[nodeLength++] = nodeNameMapping[startId];
     ...
     * @param rankingArray 节点列表
     */
    function queryRankingAll() public view returns (Node[] memory rankingArray,string[] memory nameArray){
        if(firstNodeId != 0){
            uint nodeLength = 0;
            uint startId = firstNodeId;
            do{
     ...
     * @param rankingArray 节点列表
     */
    function queryNodeStop() public view returns (Node[] memory rankingArray,string[] memory nameArray){
        ...
        for(uint i=1;i<=nodeIdMax;i++){
            if(nodePoolMapping[i].isFlag == false || nodePoolMapping[i].isOpen == false){
                ...
                nameArray[nodeLength++] = nodeNameMapping[i];
            }
        }
    }
    ...
    function setNodeFlag(uint nodeId,bool flag) public {
        ...
        if(nodePoolMapping[nodeId].isFlag != flag){
            ...
            if(flag){ //开启
                ...
                if(firstNodeId == lastNodeId && firstNodeId == nodeId){
                    firstNodeId = 0;
                    lastNodeId = 0;
                } else {
                    handleOldIndex(nodeId);//处理原位置
                ...
                if(nodePoolMapping[nodeId].isEffective == 1){//以前是有效节点
                    ...
                    ERC20(nodeAddress).endAfterY2N(nodeId,nodePoolMapping[nodeId].totalLp,nodePoolMapping[nodeId].scale,totalLpEffective);
                    nodePoolMapping[nodeId].isEffective = 0;
                }
            }
        }
    }
    ...
    /*
     * @dev 修改 | 节点合约调用 | 设置节点名称
     * @param nodeId 节点ID
     * @param name 名称
     */
    function setNodeName(uint nodeId,string memory name) external {
        require(nodePoolMapping[nodeId].creator == msg.sender,"NodePool : Not the creator");
        require(nodePoolMapping[nodeId].createTime != 0,"NodePool : No node");
        require(nodePoolMapping[nodeId].isOpen == true,"NodePool : Administrator shutdown");
        require(nodePoolMapping[nodeId].isFlag == true,"NodePool : Node Pool Closed");
        nodeNameMapping[nodeId] = name;
    }
    ...
    function setNodeScale(uint nodeId,uint _scale) external onlyOwner returns (bool){
        require(nodePoolMapping[nodeId].createTime != 0,"NodePool : No node");
