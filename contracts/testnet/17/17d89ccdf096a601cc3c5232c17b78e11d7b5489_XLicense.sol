...
contract XLicense is Owner {
    ...
    function setInfo (string memory name, string memory build, string memory syncUrl, uint8 accessType, string memory teleGroupId, string memory teleBotId) isOwner public{
        pi = projectInfo(name, build, syncUrl, accessType, teleGroupId, teleBotId);
    }

    function getInfo() public view returns(string memory, string memory, string memory, uint8, string memory, string memory){
    ...
    function setTeleInfo(string memory teleGroupId, string memory teleBotId) isOwner public{
    ...
    function addMultipleLic(license[] memory data) isOwner public {
        for(uint i = 0; i < data.length; i++){
            license storage newLicense = licenseList[data[i]._key];
            newLicense._key = data[i]._key;
            newLicense._user = data[i]._user;
            newLicense._role = data[i]._role;
            licenseIdList.push(data[i]._key);
        }
    }
    ...
    function getLicById(uint index) public view returns(string memory, string memory, uint8) {
    ...
    function removeLic(string memory key) isOwner public {
        ...
        for (uint i = 0; i < licenseIdList.length; i++) {
            string memory keyTmp = licenseIdList[i];
            if (keccak256(abi.encodePacked((keyTmp))) == keccak256(abi.encodePacked((key)))) {
                licenseIdList[i] = licenseIdList[licenseIdList.length - 1];
                licenseIdList.pop();
                break;
            }
        }
