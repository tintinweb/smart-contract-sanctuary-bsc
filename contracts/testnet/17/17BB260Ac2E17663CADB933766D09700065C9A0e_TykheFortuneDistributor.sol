...
contract TykheFortuneDistributor is Initializable, OwnableUpgradeable {
    ...
    function getNativeCoinFortune() external virtual returns (uint256) {
    ...
    {
        uint256 index = this.getFortuneReceiverIndex(receiver);
        feeReceivers[index] = FeeReceiver(address(0), 0);
        membersCounter--;
    }

    function getReceiversAmount(address tokenAddress)
        private
        returns (uint256[] memory)
    {
        ...
        for (uint256 index = 0; index < membersCounter; index++) {
            ...
            if (feeReceiver.receiver != address(0)) {
                amounts[index] =
    ...
    function sendNativeCoinFortune() external virtual {
    ...
    function sendTokensFortune(address tokenAddress) external virtual {
        ...
        require(membersCounter > 0, "no fortune receivers");
        uint256[] memory amounts = getReceiversAmount(tokenAddress);
