...
contract TimeBondingCalculator is IBondingCalculator {
    ...
    function getDecimals( address _pair ) public view returns( uint deci1, uint deci2 ) {
        uint deci1 = IERC20( IUniswapV2Pair( _pair ).token0() ).decimals();
        uint deci2 = IERC20( IUniswapV2Pair( _pair ).token1() ).decimals();
    }
