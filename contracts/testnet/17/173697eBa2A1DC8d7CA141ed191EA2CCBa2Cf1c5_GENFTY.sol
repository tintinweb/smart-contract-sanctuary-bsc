...
contract GENFTY is Ownable, ERC721Enumerable{
    ...
    uint256 public totalPublicLimit=4;

    uint256 public maxMintPerTrx=3;
    bool public saleStarted = true;
    bool public preSaleStarted = false;

    string public prefixURI="abc.json";
    ...
    constructor
    ()
    ERC721("collection_Name","collection_Symbol")  // collection name and symbol
    {
        MAX_SUPPLY = 1;        // Maximum 1000 nfts can be mint
        maxMintPerTrx = 1;       //  Maximum 5 nfts can mint in single tx

        prefixURI = "ipfs://QmRCChyFiiY4BQpwhqjeZhniDBQXyYcvQCFpd8QrwZAoW6/";       //  It's the URI of tokenId
        totalPublicLimit = 1;    //  Public can mint maximum 3 nfts in single tx
        preSalePrice = 1;     //  Presale price for tokenId is 1 eth
        publicSalePrice = 2;   // PublicSale price for tokenId is 0.1 eth
    }

// call this func if(preSaleStarted==true)
// owner -> allow 
    function mintWhitelist(uint256 _count)
    public
    payable
    {
        require(preSaleStarted, "MINT_NOT_STARTED");
        require(isWhitelisted[msg.sender],"ADDRESS_ISN'T_WHITELISTED");
        require(_count > 0 && _count <= maxMintPerTrx, "COUNT_INVALID");
        require(totalSupply() + _count <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        require(msg.value >= (_count * preSalePrice), "INVALID_ETH_SENT");
        require(totalWhiteListMinted[msg.sender]+_count <= totalWhiteListLimit,"Maximum_NFT_Limit_Reached");
    ...
    // if (saleStarted==true) then call this func 
    // if(both false then show msg -> Sale is not started yet, )

// _count -> how much nft user want to mint
// msg.value >= (_count * publicSalePrice)
    ...
    payable
    {
        ...
        require(_count > 0 && _count <= maxMintPerTrx, "COUNT_INVALID");
        require(totalSupply() + _count <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        require(msg.value >= (_count * publicSalePrice), "INVALID_ETH_SENT");
        require(totalPublicMinted[msg.sender]+_count <= totalPublicLimit,"Maximum_NFT_Limit_Reached");
