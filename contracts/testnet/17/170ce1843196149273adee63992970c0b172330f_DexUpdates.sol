...
function compareStrings(string memory a, string memory b) public view returns (bool) {
  ...
  function withdrawNFT(address to, uint256 tokenId, string memory ownerId, address collection) public {
    require(msg.sender == nonCryptoNFTVault, "not authorized");
    require(compareStrings(_nonCryptoOwners[tokenId][collection], ownerId), "not the owner");
    require(INFT(collection).ownerOf(tokenId) == collection, "nft on sell or not owner");
    INFT(collection).transferFrom(msg.sender, to, tokenId);
    delete _nonCryptoOwners[tokenId][collection];
    
    emit WithdrawNFT(tokenId, collection, to);
  }
