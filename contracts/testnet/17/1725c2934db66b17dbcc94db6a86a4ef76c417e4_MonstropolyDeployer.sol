...
contract VestingRangesUpgradeable is Initializable {
    ...
    function vestingType() public view returns (uint8[] memory) {
        return _vestingType;
    }

    function totalAmount() public view returns (uint256[] memory) {
        return _totalAmount;
    }

    function startDate() public view returns (uint256[] memory) {
        return _startDate;
    }

    function endDate() public view returns (uint256[] memory) {
        return _endDate;
    }

    function increment() public view returns (uint256[] memory) {
        return _increment;
    }

    // solhint-disable-next-line
    function __VestingRangesUpgradeable_init(
        uint8[] calldata vestingType_,
        uint256[] calldata totalAmount_,
        uint256[] calldata startDate_,
        uint256[] calldata endDate_
    ) internal onlyInitializing {
        __VestingRangesUpgradeable_init_unchained(vestingType_, totalAmount_, startDate_, endDate_);
    }

    // solhint-disable-next-line
    function __VestingRangesUpgradeable_init_unchained(
        uint8[] calldata vestingType_,
        uint256[] calldata totalAmount_,
        uint256[] calldata startDate_,
        uint256[] calldata endDate_
    ) internal onlyInitializing {
        for (uint256 i; i < vestingType_.length; ++i) {
            require(vestingType_[i] > 0, "Vesting: 001");
            require(totalAmount_[i] > 0, "Vesting: 002");
            require(startDate_[i] < endDate_[i], "Vesting: 003");
            if (i != 0) endDate_[i - 1] == startDate_[i]; //TODO: == or +1?

            _vestingType.push(vestingType_[i]);
            _totalAmount.push(totalAmount_[i]);
            _startDate.push(startDate_[i]);
            _endDate.push(endDate_[i]);
            _increment.push(
                (vestingType_[i] * totalAmount_[i] * 1 ether) / ((endDate_[i] - startDate_[i]) ** vestingType_[i])
            );
        }
    ...
    function _releasedRange(uint256 from, uint256 to) internal view virtual returns (uint256 released) {
        (uint256 slotStart, uint256 slotEnd) = _getSlot(from, to);
        uint256 iters = slotEnd - slotStart + 1;

        for (uint256 i; i < iters; ++i) {
            uint256 slot = i + slotStart;
            released +=
                _releasedRangeSlot(from, to, _increment[slot], _startDate[slot], _endDate[slot], _vestingType[slot]);
        }
    }

    function _releasedRangeSlot(
        uint256 from,
        uint256 to,
        uint256 increment_,
        uint256 startDate_,
        ...
        uint256 comp0 = (increment_ * ((to - startDate_) ** vestingType_)) / vestingType_;
        uint256 comp1 = (increment_ * ((from - startDate_) ** vestingType_)) / vestingType_;
        return (comp0 - comp1) / 1 ether;
    }

    function _getSlot(uint256 from, uint256 to) internal view returns (uint256 slotStart, uint256 slotEnd) {
        for (uint256 i; i < _startDate.length; ++i) {
            if (to > _startDate[i]) slotEnd = i;
            if (from > _startDate[i]) slotStart = i;
        }
    }
...
import {MonstropolyToken} from "./MonstropolyToken.sol";
import {MonstropolyDistributionVaultAllocation} from "./MonstropolyDistributionVaultAllocation.sol";
import {MonstropolyDistributionVaultMerkle} from "./MonstropolyDistributionVaultMerkle.sol";
import {MonstropolyDistributionVaultMerkleVesting} from "./MonstropolyDistributionVaultMerkleVesting.sol";
import {MonstropolyDistributionVaultRangesAllocation} from "./MonstropolyDistributionVaultRangesAllocation.sol";
import {MonstropolyStaking} from "./MonstropolyStaking.sol";
import {MonstropolyFarming} from "./MonstropolyFarming.sol";
import {IERC20Upgradeable} from "openzeppelin-contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";
import {SafeERC20Upgradeable} from "openzeppelin-contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol";
...
contract MonstropolyDeployer {
    ...
    constructor(address ms, uint256 timestamp, address feed, IERC20Upgradeable wbnb) {
        if (timestamp == 0) {
            // solhint-disable-next-line
        ...
        {
            MonstropolyToken tokenImplementation = new MonstropolyToken();
            bytes memory tokenInitializeData = abi.encodeWithSelector(MonstropolyToken.initialize.selector, ms);
            address tokenAddress = _createProxy(address(tokenImplementation), tokenInitializeData);
            token = IERC20Upgradeable(tokenAddress);
        }

        address vaultImplementationAddress = address(new MonstropolyDistributionVaultMerkleVesting());

        {
            uint256 seedAmount = 15e6 ether;
            uint256 tgePercentage = 0.075 ether;
            uint256 startDate = timestamp - (tgePercentage * 15 * ONE_MONTH / (1 ether - tgePercentage));
            bytes memory initializeData = abi.encodeWithSelector(
                MonstropolyDistributionVaultMerkleVesting.initialize.selector,
                ms,
                address(token),
                startDate,
                timestamp + (15 * ONE_MONTH)
            );
            seedVault = _createProxy(vaultImplementationAddress, initializeData);
            token.safeTransfer(seedVault, seedAmount);
        }

        {
            uint256 privateAmount = 54e6 ether;
            uint256 tgePercentage = 0.1 ether;
            uint256 startDate = timestamp - (tgePercentage * 12 * ONE_MONTH / (1 ether - tgePercentage));
            bytes memory initializeData = abi.encodeWithSelector(
                MonstropolyDistributionVaultMerkleVesting.initialize.selector,
                ms,
                address(token),
                startDate,
                timestamp + (12 * ONE_MONTH)
            );
            privateVault = _createProxy(vaultImplementationAddress, initializeData);
            token.safeTransfer(privateVault, privateAmount);
        }

        {
            uint256 idoAmount = 8e6 ether;
            token.safeTransfer(ms, idoAmount);
        }

        {
            uint8[] memory vestingTypes = new uint8[](5);
            uint256[] memory totalAmounts = new uint256[](5);
            uint256[] memory startDates = new uint256[](5);
            uint256[] memory endDates = new uint256[](5);
            vestingTypes[0] = 1;
            vestingTypes[1] = 1;
            vestingTypes[2] = 1;
            vestingTypes[3] = 1;
            ...
            uint256 marketingAmount = 70e6 ether;
            totalAmounts[0] = 21e6 ether;
            totalAmounts[1] = 175e5 ether;
            totalAmounts[2] = 14e6 ether;
            totalAmounts[3] = 105e5 ether;
            totalAmounts[4] = 7e6 ether;
            //TODO: mainnetttt
            //TODO: mainnetttt
            //TODO: mainnetttt
            startDates[0] = timestamp; //+ ONE_MONTH //TODO: mainnetttt
            //TODO: mainnetttt
            //TODO: mainnetttt
            //TODO: mainnetttt
            startDates[1] = startDates[0] + ONE_YEAR;
            startDates[2] = startDates[1] + ONE_YEAR;
            startDates[3] = startDates[2] + ONE_YEAR;
            startDates[4] = startDates[3] + ONE_YEAR;

            endDates[0] = startDates[1];
            endDates[1] = startDates[2];
            endDates[2] = startDates[3];
            endDates[3] = startDates[4];
            endDates[4] = startDates[4] + ONE_YEAR;

            address implementation = address(new MonstropolyDistributionVaultRangesAllocation());
            bytes memory initializeData = abi.encodeWithSelector(
                MonstropolyDistributionVaultRangesAllocation.initialize.selector,
                ms,
                address(token),
                vestingTypes,
                totalAmounts,
            ...
            marketingVault = _createProxy(implementation, initializeData);
            token.safeTransfer(marketingVault, marketingAmount);
        }

        {
            uint256 teamAmount = 75e6 ether;
            bytes memory initializeData = abi.encodeWithSelector(
                MonstropolyDistributionVaultMerkleVesting.initialize.selector,
                ms,
                address(token),
                timestamp + (6 * ONE_MONTH),
                timestamp + (18 * ONE_MONTH)
            );
            teamVault = _createProxy(vaultImplementationAddress, initializeData);
            token.safeTransfer(teamVault, teamAmount);
        }

        {
            uint256 advisorsAmount = 30e6 ether;
            bytes memory initializeData = abi.encodeWithSelector(
                MonstropolyDistributionVaultMerkleVesting.initialize.selector,
                ms,
                address(token),
            ...
            advisorsVault = _createProxy(vaultImplementationAddress, initializeData);
            token.safeTransfer(advisorsVault, advisorsAmount);
        }

        {
            uint8[] memory vestingTypes = new uint8[](2);
            uint256[] memory totalAmounts = new uint256[](2);
            uint256[] memory startDates = new uint256[](2);
            uint256[] memory endDates = new uint256[](2);
            vestingTypes[0] = 2;
            vestingTypes[1] = 2;

            uint256 rewardsAmount = 228e6 ether;
            totalAmounts[0] = 6e5 ether;
            totalAmounts[1] = 228e6 ether - totalAmounts[0];

            startDates[0] = timestamp;
            startDates[1] = startDates[0] + ONE_YEAR;

            endDates[0] = startDates[1];
            endDates[1] = startDates[1] + (48 * ONE_MONTH);
            ...
            rewardsVault = _createProxy(implementation, initializeData);
            token.safeTransfer(rewardsVault, rewardsAmount);
        }

        {
            address implementation = address(new MonstropolyDistributionVaultMerkle());
            bytes memory initializeData = abi.encodeWithSelector(
                MonstropolyDistributionVaultMerkle.initialize.selector, ms, address(token), rewardsVault
            );
            gamingVault = _createProxy(implementation, initializeData);
        }

        {
            address stakingImplementationAddress = address(new MonstropolyStaking());
            bytes memory stakingInitializeData = abi.encodeWithSelector(
                MonstropolyStaking.initialize.selector, ms, rewardsVault, token, (2 * ONE_MONTH), 0.1 ether, 2 ether
            );
        ...
        {
            //TODO: mainnetttt
            uint256 tokenAmount = 2e5 ether; //TODO: mainnetttt
            //TODO: mainnetttt
            //TODO: mainnetttt
            //TODO: mainnetttt
            //TODO: mainnetttt
            ...
            bnbAmount = tokenAmount * 1 ether * uint256(5) / uint256(100) / btcToUsdPrice; //20e6 MPOLY * WBNB_decimals * MPOLY_USD_PRICE / BNB_USD_PRICE
            wbnb.safeTransferFrom(ms, address(this), bnbAmount);
            wbnb.approve(UV2_ROUTER, bnbAmount);
...

        {
            ...
            (,, uint256 liquidity) = router.addLiquidity(
                address(token),
                address(wbnb),
                2e5 ether, //TODO: mainnetttt//TODO: mainnetttt//TODO: mainnetttt//TODO: mainnetttt
                bnbAmount,
                2e5 ether, //TODO: mainnetttt//TODO: mainnetttt//TODO: mainnetttt//TODO: mainnetttt
                bnbAmount,
                ms,
            ...
            pool = factory.getPair(address(token), address(wbnb));
...

        {
            ...
            if (left != 0) token.safeTransfer(ms, left);
        }

        {
            address farmingImplementationAddress = address(new MonstropolyFarming());
            bytes memory farmingInitializeData =
                abi.encodeWithSelector(MonstropolyFarming.initialize.selector, ms, rewardsVault, pool);
...

        {
            ...
            MonstropolyDistributionVaultAllocation(rewardsVault).addAccount(farming, allocations);
            uint256[] memory allocations2 = new uint256[](uint256(2));
            allocations2[0] = 60 ether;
            allocations2[1] = 40 ether;
            MonstropolyDistributionVaultAllocation(rewardsVault).addAccount(staking, allocations2);
            uint256[] memory allocations3 = new uint256[](uint256(3));
            allocations3[0] = 30 ether;
            allocations3[1] = 20 ether;
            allocations3[2] = 50 ether;
            MonstropolyDistributionVaultAllocation(rewardsVault).addAccount(gamingVault, allocations3);
        }
    }
...
contract MonstropolyDistributionVaultAllocation is
...
import {DistributionVaultAllocationUpgradeable} from
    "dappcomposer-contracts/contracts-upgradeable/distribute/DistributionVaultAllocationUpgradeable.sol";
import {VestingRangesUpgradeable} from
    "dappcomposer-contracts/contracts-upgradeable/distribute/utils/VestingRangesUpgradeable.sol";
import {Ownable2StepUpgradeable} from "openzeppelin-contracts-upgradeable/access/Ownable2StepUpgradeable.sol";
import {UUPSUpgradeable} from "openzeppelin-contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

contract MonstropolyDistributionVaultRangesAllocation is
    DistributionVaultAllocationUpgradeable,
    VestingRangesUpgradeable,
    Ownable2StepUpgradeable,
    UUPSUpgradeable
{
    address public deployer;
    ...
    function initialize(
        ...
        __DistributionVaultAllocationUpgradeable_init(_token);
        __VestingRangesUpgradeable_init(vestingType, totalAmount, startDate, endDate);
        __Ownable_init();
        _transferOwnership(owner_);
        __UUPSUpgradeable_init();
        deployer = msg.sender;
    ...
    function _releasedRange(uint256 from, uint256 to)
        internal
        view
        override(VestingRangesUpgradeable, DistributionVaultAllocationUpgradeable)
        returns (uint256)
    {
        return VestingRangesUpgradeable._releasedRange(from, to);
    }
}
...
contract MonstropolyStaking is StakingWithFeeUpgradeable, Ownable2StepUpgradeable, UUPSUpgradeable {
    constructor() {
        _disableInitializers();
    }
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address owner_) public initializer {
        uint256 cap_ = 500e6 ether;
        __ERC20_init("Monstropoly token", "MPOLY");
