...
contract SONOFSHIB is IERC20, Ownable {
    ...
    address constant public burnWallet = address(0xdead);
    ...
    uint256 public minLpBeforeSwapping = 1;
    uint256 public lpDenominator = 10000;
    ...
    function setBridgeAddress(address _bridgeAddress) external onlyOwner {
        ...
        excludeFromReward(_bridgeAddress);
        _tOwned[_bridgeAddress] = bridgeBalance();
        _tOwned[bridgeAddress] = 0;
    ...
    function totalSupply() public view override returns (uint256) {
        return _tTotal - burned() - bridgeBalance();
    }

    function burned() public view returns (uint256) {
        return _tOwned[burnWallet];
    }

    function balanceOf(address account) public view override returns (uint256) {
        if (account == burnWallet || account == bridgeAddress) return 0;
    ...
    function setSwapThresholdAmount(uint256 newValue) external onlyOwner() {
        require(newValue < lpDenominator, "Can't be more than 100% (10000)");
        minLpBeforeSwapping = newValue;
    }
    ...
    function _transfer(
        ...
        if (hasLiquidity){
            ...
            bool overMinTokenBalance = contractTokenBalance >= lpTokens * minLpBeforeSwapping / lpDenominator;
            if (
                overMinTokenBalance &&
                !inSwapAndLiquify &&
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!canTrade) require(_isExcludedFromFee[recipient] || _isExcludedFromFee[sender], "Trade is not open yet");
    ...
    function addToLp(uint256 tokenAmount) external {
        _transfer(msg.sender, uniswapPair, tokenAmount * 10 ** decimals());
        uniswapV2Pair(uniswapPair).sync();
    }
}