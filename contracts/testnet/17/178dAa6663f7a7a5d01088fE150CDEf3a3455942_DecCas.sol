...
// https://testnet.bscscan.com/address/0xb4ab4A894374C8830511b900a0de980eeAf6c750#code
...
contract DecCas is Ownable {
    ...
    function get_latest_wallet_plays(address wallet, uint total) public view returns(uint [] memory){

        uint[] memory wallet_plays_cp = new uint[](total);

        for (uint i=0; i< total; i++){
            wallet_plays_cp[i] = wallet_plays[wallet][wallet_plays[wallet].length -1 - i];
    ...
    function get_latest_winners_plays(address token, uint total) public view returns(uint [] memory){

        uint[] memory winners_plays_cp = new uint[](total);

        for (uint i=0; i< total; i++){
            winners_plays_cp[i] = winners_plays[token][winners_plays[token].length -1 - i];
            if(winners_plays[token].length -1 - i == 0){
                break;
            }
        }

        return winners_plays_cp;
