...
import "../interfaces/IVestMerkleDistributor.sol";

contract VestMerkleDistributor is IVestMerkleDistributor {
    ...
    constructor(address token_, bytes32 merkleRoot_, uint256 _releaseDuration) public {
        require(_releaseDuration != 0, 'VestMerkleDistributor: Duration should be greater than zero.');
        token = token_;
    ...
    function collect(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external override {
        require(!isCollected(index), 'VestMerkleDistributor: Drop already collected.');

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), 'VestMerkleDistributor: Invalid proof.');
    ...
    function vestClaim() external override {
        uint256 _unclaimed = vestClaimable(msg.sender).sub(claimedAmount[msg.sender]);
        if (_unclaimed > 0) {
            require(IERC20(token).transfer(msg.sender, _unclaimed), 'VestMerkleDistributor: Transfer failed.');
    ...
    function vestClaimable(address _user) public override view returns (uint256) {
        if (collectedTime[_user] == 0 || block.timestamp < collectedTime[_user]) {
            return 0;
        } else if (block.timestamp >= collectedTime[_user].add(releaseDuration)) {
            return collectedAmount[_user];
        } else {
            return collectedAmount[_user].mul(block.timestamp.sub(collectedTime[_user])).div(releaseDuration);
        }
    }
}
...
interface IVestMerkleDistributor {
    ...
    function vestClaimable(address _user) external view returns (uint256);
