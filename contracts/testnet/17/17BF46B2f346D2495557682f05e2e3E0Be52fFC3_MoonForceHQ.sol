...
contract MoonForceHQ is IMoonForceHQ, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
    ...
    // maximum rank for a Alien/Cadet
    ...
    modifier requireContractsSet() {
        ...
        && address(randomizer) != address(0), "Contracts not set");
        _;
    }

    function setContracts(address _lfGenesis, address _lfAlpha, address _TOPIA, address payable _rand) external onlyOwner {
   ...
   */
    function claimManyFromArmoryAndYield(uint16[] calldata tokenIds, bool unstake) external payable whenNotPaused nonReentrant {
        require(tx.origin == msg.sender, "Only EOA");
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (genesisType[tokenIds[i]] == 1) {
                owed += _claimCadetFromArmory(tokenIds[i], unstake, seed[i]);
            } else if (genesisType[tokenIds[i]] == 2) {
                owed += _claimAlienFromYield(tokenIds[i], unstake, seed[i]);
            } else if (genesisType[tokenIds[i]] == 3) {
   ...
   */
    function _claimCadetFromArmory(uint16 tokenId, bool unstake, uint256 seed) internal returns (uint256 owed) {       
   ...
   */
    function _claimAlienFromYield(uint16 tokenId, bool unstake, uint256 seed) internal returns (uint256 owed) {
  ...
  */
   ...
   */
    ...
    /**
     * chooses a random General thief when a newly minted token is stolen
   ...
   */
    ...
    function updateCadetUpgradeCost(uint8 _cost) external onlyOwner {
        UPGRADE_COST = _cost;
    }
}
