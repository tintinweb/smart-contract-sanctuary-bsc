...
contract ExenoBridgeNode is 
    ...
    ERC1363Payable
{
    bytes32 private constant DEPOSIT = bytes32("deposit");
    bytes32 private constant WITHDRAW = bytes32("withdraw");
    bytes32 private constant EXCHANGE = bytes32("exchange");
        ...
        external payable nonReentrant whenNotPaused
    {
        ...
        _verifySingleSignature(data, fee, method, signature);
        _validateClass(data, method, DEPOSIT);
    ...
    function deposit(
        ...
        external payable nonReentrant whenNotPaused
    {
        (Data memory data, uint256[2] memory fee, bytes32 method, bytes memory signature) = _unpackSingleSignedData(encodedData);
        _verifySingleSignature(data, fee, method, signature);
        _validateClass(data, method, DEPOSIT);
        _validateDeposit(data, fee);
        _processDeposit(data, false);
    ...
    {
        Data memory data = Data(id, platform, network, beneficiary, token, amount, timeout, operation);
        _verifyTrippleSignature(data, fee, method, signature);
        _validateClass(data, method, WITHDRAW);
    ...
    function withdraw(
        ...
        external payable nonReentrant whenNotPaused
    {
        (Data memory data, uint256[2] memory fee, bytes32 method, bytes[3] memory signature) = _unpackTrippleSignedData(encodedData);
        _verifyTrippleSignature(data, fee, method, signature);
        _validateClass(data, method, WITHDRAW);
        _validateWithdraw(data, fee);
        _processWithdraw(data, false);
        ...
        external payable nonReentrant whenNotPaused
    {
        Data memory data = Data(id, platform, network, beneficiary, token, amount, timeout, operation);
        _verifyTrippleSignature(data, fee, method, signature);
        _validateClass(data, method, EXCHANGE);
        _validateDeposit(data, fee);
    ...
    function payout(
        ...
        external onlyOwner
    {
        _processWithdraw(Data(id, platform, network, beneficiary, token, amount, timeout, operation), false);
    }
    ...
    function _verifySingleSignature(
        ...
        internal view
    {
        bytes memory payload = abi.encodePacked(d.id, d.platform, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation, fee, method);
    ...
    {
        if (class == DEPOSIT) {
            require(method == DEPOSIT,
                "ExenoBridgeNode: unexpected method for deposit transaction");
            return;
        }
        if (class == WITHDRAW) {
            require(method == WITHDRAW,
                "ExenoBridgeNode: unexpected method for withdraw transaction");
            return;
        }
        if (class == EXCHANGE) {
            require(method == EXCHANGE,
                "ExenoBridgeNode: unexpected method for exchange transaction");
        ...
        internal
    {
        ...
        if (isExchange) {
            emit Exchange(d.id, d.platform, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.operation);
        } else {
        ...
        internal pure returns(Data memory, uint256[2] memory, bytes32, bytes memory)
    {
        ...
        return (_unpackPayload(payload), fee, method, signature);
    }
