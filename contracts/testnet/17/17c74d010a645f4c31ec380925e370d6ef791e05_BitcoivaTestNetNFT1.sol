...
contract BitcoivaTestNetNFT1 is ERC721, Ownable {
    ...
    event PriceUpdate(address indexed owner, uint oldPrice, uint newPrice, uint nftID, bool isListed);

    event NftListStatus(address indexed owner, uint nftID, bool isListed);

    constructor() ERC721("BitcoivaTestNetNFT1", "BCANFT1") {
        _contractOwner = msg.sender;
    }

    function mint(string memory _tokenURI, address _toAddress, uint _price) onlyOwner public returns (uint) {
    ...
    function updatePrice(uint _tokenId, uint _price, bool shouldBeListedd) public returns (bool) {
        uint oldPrice = price[_tokenId];
        require(msg.sender == ownerOf(_tokenId), "Error, you are not the owner");
        price[_tokenId] = _price;
listedMap[_tokenId] = shouldBeListedd;
        emit PriceUpdate(msg.sender, oldPrice, _price, _tokenId,shouldBeListedd);
        return true;
    }
