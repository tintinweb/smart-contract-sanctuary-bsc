...
contract Escrow is Ownable,Initializable {
    ...
    ///@param foundationAddress foundation address
    ///@param allocated foundation allocation amount in wei
    ///@param allocated foundation start time
    function initialize(Dividend memory dividentParams, address foundationAddress, uint allocated, uint64 startTime) external onlyOwner initializer {
        require(!isZero(uint160(dividentParams.token)), "the dividend token must not be zero");
        require(!isZero(uint160(foundationAddress)), "the foundation address must not be zero");
        ...
        require(!isZero(allocated), "the foundation allocation must not be zero");
        
        dividend = dividentParams;
        foundation.account = foundationAddress;
        foundation.allocated = allocated;
        foundation.lastClaim = foundation.startTime = startTime;
    }
    ...
    ///@dev update the foundation address
    ///@param foundationAddress foundation address
    function updateFoundation(address foundationAddress) external onlyOwner {
        require(!isZero(uint160(foundationAddress)), "updateFoundation: the foundation address must not be zero");
        foundation.account = foundationAddress;
    }
    ...
    function updateDivToken(address token) external onlyOwner {
        require(!isZero(uint160(token)), "updateDivToken: the dividend cliff must not be zero");
    ...
    ///@dev foundation address claim its dividends from here
    function claim() external {
        require(foundation.account == _msgSender(), "claim: the foundation wallet is not a caller");
        require(!isZero(foundation.startTime), "claim: start time must not be zero");
        require(!isZero(foundation.allocated), "claim: allocation must not be zero");
        require(foundation.released <= foundation.allocated, "claim: the foundation wallet has claimed all dividends");
        require(foundation.lastClaim + dividend.period <= blockTimestamp(), "claim: time hasn't passed for the next dividend");

        (uint dividends, uint claims) = viewDividend();

        if(!isZero(dividends)) {
            foundation.released += dividends;
            foundation.lastClaim += (dividend.period * claims).toUint64();
    ...
    function viewDividend() public view returns(uint dividends, uint claims) {        
        if(!isValidClaim(foundation)) {   
            return (0,0);
        }

        uint months = (blockTimestamp() - foundation.lastClaim) / dividend.period;

        if(!isZero(months)) {
            uint divAmount = (foundation.allocated * dividend.percent) / DIVISOR;
            dividends = divAmount * months;
            claims = months;

            if((foundation.released + dividends) > foundation.allocated) {
                dividends = foundation.allocated - foundation.released;
            }
        }
    }

    ///@dev returns true if foundation is allowed to claim
    ///@param wallet foundation info
    ///@return bool is it a valid claim, returns in boolean
    function isValidClaim(Foundation memory wallet) private view returns(bool) {
        return !(
