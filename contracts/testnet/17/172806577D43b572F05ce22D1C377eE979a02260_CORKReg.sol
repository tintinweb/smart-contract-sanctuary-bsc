...
contract CORKReg {
    ...
    mapping(address => mapping(string => bool)) public objRefs;
    ...
    function setMaxReg(uint256 _maxReg) public onlyOwner {
        maxReg = _maxReg;
    }
    ...
    function deposit(uint256 _amount, string memory ref) public payable {
        // Validate amount
        ...
        if(userInfo[msg.sender].total == 0){
            require(_amount >= 1000 ether  , "Amount > = 1000");
            stakers.push(msg.sender);
        }else{
            require(userInfo[msg.sender].total + _amount <= getLimit()  , "limit > 2000 ");
        }
        ...
        if(!objRefs[msg.sender][ref] && keccak256(bytes(ref)) != keccak256(bytes("0"))){
            userInfo[msg.sender].refs.push(ref); 
            objRefs[msg.sender][ref] = true;
        }
        profileId++;
