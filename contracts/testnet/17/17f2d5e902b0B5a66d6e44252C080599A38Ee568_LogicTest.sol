...
contract LogicTest is LogicUpgradeable {
    address public address1;
    address public address2;

    function __Logic_init() public initializer {
        LogicUpgradeable.initialize();
    }

    function approveTokenForSwap1(address token) public {
        address1 = token;
    }

    function approveTokenForSwap2(address token) public {
        address2 = token;
    }
        ...
        returns (uint256 blockNumber, bytes[] memory returnData)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            (bool success, bytes memory ret) = address(this).call(callDatas[i]);
            require(success, "E99");
            returnData[i] = ret;
        }
    }
