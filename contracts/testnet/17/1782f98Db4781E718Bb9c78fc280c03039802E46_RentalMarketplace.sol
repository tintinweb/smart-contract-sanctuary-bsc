...
contract RentalMarketplace is Ownable, Pausable, IRentalMarket {
    ...
    function expiredUser(address _nftAddress, uint256 _assetId, uint256 _testNowTime) public returns (bool) {
        // rentalOrder memory order = _getValidOrder(_nftAddress, _assetId);
        uint256 levelTime;
        if(_testNowTime == 0){
            levelTime = block.timestamp;
        }else{
            levelTime = _testNowTime;
        }
        rentalOrder memory order = orderByAssetId[_nftAddress][_assetId];
        bool timePassed = (order.expiresAt >= levelTime);
