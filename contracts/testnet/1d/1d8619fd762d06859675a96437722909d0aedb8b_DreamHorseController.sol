...
contract DreamHorseController is Ownable {
    ...
    function ownerDeposit(uint256 amount) public onlyOwner {
        ...
        uint256 dexBalance = BUSD.balanceOf(msg.sender);
    ...
    //给用户添加游戏卡
    function _addHeroToUser(address user, uint256 tokenId) internal virtual{

        require(DHN.ownerOf(tokenId) == user, "You are not the owner");
    ...
    function getUserHeros(address user) public view returns(Hero[] memory userHeros) {
        ...
        Hero[] memory myHeros = new Hero[](uint256(10000));
        ...
        for(uint256 i = 0; i < userTokenIds.length; i++){
            ...
            if(tokenId != 0 && counter < 9999){
                myHeros[counter] = tokenIdToHeros[tokenId];
                counter++;
            }
        }
