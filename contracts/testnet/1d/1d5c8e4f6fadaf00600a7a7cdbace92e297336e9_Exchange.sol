...
import {IRocketKingdomNft} from "./interfaces/IRocketKingdomNft.sol";
...
contract Exchange is OrderState, Ownable {
        ...
        if (order.key.sellAsset.assetType == AssetType.ERC721 || order.key.buyAsset.assetType == AssetType.ERC721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    require(order.key.sellAsset.token == address(_rocketKingdomNFT), "TK-1");
                    IRocketKingdomNft(order.key.sellAsset.token).safeMintWithPermit(                        
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId,
                        storeParams.uri,
                ...
                if (!isStore) {
                    ...
                    require(order.key.buyAsset.token == address(_rocketKingdomNFT), "TK-2");
                    IRocketKingdomNft(order.key.buyAsset.token).safeMintWithPermit(                        
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId,
                        storeParams.uri,
            ...
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    require(order.key.sellAsset.token == address(_rocketKingdomNFT), "TK-3");
                    IRocketKingdomNft(order.key.sellAsset.token).safeMintWithPermit(                        
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId,
                ...
                if (!isStore) {
                    ...
                    require(order.key.buyAsset.token == address(_rocketKingdomNFT), "TK-4");
                    IRocketKingdomNft(order.key.buyAsset.token).safeMintWithPermit(
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId,
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        if(newMintableStore != address(0x00)) revert ZeroAddress();
