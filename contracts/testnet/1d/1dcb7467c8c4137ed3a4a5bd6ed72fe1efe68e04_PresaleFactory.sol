...
contract PresaleFactory is Ownable {
    ...
    bool presaleSuccessed;

    constructor(address _gtf, address _usdt) {
        _gtfAddress = IERC20(_gtf);
        _usdtAddress = IERC20(_usdt);
    }

    function getSaleState() public view returns (uint256) {
        uint256 state = (uint256)(saleState.none);
        if (block.timestamp >= start_time && block.timestamp <= end_time)
            state = (uint256)(saleState.presaling);
        else if (block.timestamp > end_time)
            state = (uint256)(saleState.finished);
        return state;
    }
    ...
    function getPresaleSuccessed() public returns (bool){
        presaleSuccessed = false;
        if (_usdtAddress.balanceOf(address(this)) >= softCap)
        {
            presaleSuccessed = true;
        }
        return presaleSuccessed;
    }

    function claim() public returns (bool) {
        require(getPresaleSuccessed(), "Presale failed!");
        require(_userPaidUSDT[msg.sender] > 0, "Can't claim! You didn't deposite.");
        return _gtfAddress.transfer(msg.sender, _userPaidUSDT[msg.sender]);
    }

    function refund() public returns (bool) {
        require(_userPaidUSDT[msg.sender] > 0, "Can't refund! You didn't deposite.");
        return _usdtAddress.transferFrom(address(this), msg.sender, _userPaidUSDT[msg.sender]);
    }
