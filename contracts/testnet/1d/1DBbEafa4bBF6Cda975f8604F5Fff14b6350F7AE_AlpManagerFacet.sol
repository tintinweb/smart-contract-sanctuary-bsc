...
library LibAlpManager {
    ...
    function mintAlp(address account, address tokenIn, uint256 amount, uint256 minUsd) internal returns (uint256 alpAmount){
        alpAmount = _calculateAlpAmount(account, tokenIn, amount, minUsd);
    ...
    function _calculateAlpAmount(address account, address tokenIn, uint256 amount, uint256 minUsd) private returns (uint256 alpAmount) {
    ...
    function getMintFeePoint(LibVault.AvailableToken storage at) internal returns (uint16) {
    ...
    function _calculateTokenAmount(address account, address tokenOut, uint256 alpAmount) private returns (uint256 amountOut) {
    ...
    function getBurnFeePoint(LibVault.AvailableToken storage at) internal returns (uint16) {
        // Dynamic rates are not supported in Phase I
        // Soon it will be supported
        require(!at.dynamicFee, "LibAlpManager: Dynamic fee rates are not supported at this time");
