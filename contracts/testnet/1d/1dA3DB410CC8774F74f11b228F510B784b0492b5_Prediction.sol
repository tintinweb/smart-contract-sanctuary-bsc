...
contract Prediction is Auth {
    ...
    function cancel(uint256 _index) external callerIsUser {
        ...
        token.transferFrom(address(this), msg.sender, apv);
    ...
    function champion(uint _team, uint256 _amount) external callerIsUser {
        ...
        token.transferFrom(msg.sender, address(this), _amount);
        OddChamp memory od = OddChamp(_team, block.timestamp, _amount, 0, 0);
        Codds[msg.sender] = od;
    }
    function cancelchampion() external callerIsUser {
        ...
        token.transferFrom(address(this), msg.sender, apv);
        OddChamp memory od = OddChamp(0, 0, 0, 0, 0);
        Codds[msg.sender] = od;
    }
    ...
    function userinfo(address user) public view returns (uint256 stakeAmount, uint256 claimAmount, uint256 applyOdds, uint256 winOdds, uint256 lostOdds) {
        stakeAmount = 0;
    ...
    function claimChampoin() external callerIsUser {
