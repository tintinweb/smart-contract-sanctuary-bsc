...
contract GEMUNIClaimReward is PermissionGroup {
    using SafeERC20 for IERC20;
    address public genix;
    address public treasury;
    mapping(uint => bool) public isUsedNonce;

    constructor (address _genix, address  _treasury) {
        genix = _genix;
        treasury = _treasury;
    }
    event SetGenix(address newGenix);
    event SetTreasury(address newTreasury);
    event ClaimToken(address user, uint nonce, uint amount);

    function setGenix(address newGenix) external onlyOwner {
        require(newGenix != address(0), "ClaimReward: invalid address");
        genix = newGenix;
        emit SetGenix(newGenix);
    }

    function setTreasury(address newTreasury) external onlyOwner {
        require(newTreasury != address(0), "ClaimReward: invalid address");
    ...
    function claimReward(uint amount, uint nonce, bytes memory sig) external {
        require(amount > 0, "ClaimReward: invalid amount");
        require(!isUsedNonce[nonce], "ClaimReward: invalid nonce");
        address user = msg.sender;
        address recoverAddress = VerifySignature.recoverSigner(VerifySignature.getEthSignedMessageHash(getMessageHash(user, nonce)), sig);
        require(recoverAddress == treasury, "GENIPassSale: invalid signature");
        IERC20(genix).safeTransferFrom(treasury, msg.sender, amount);
        isUsedNonce[nonce] = true;
        emit ClaimToken(user, nonce, amount);
    }
}
