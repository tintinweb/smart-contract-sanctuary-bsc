...
contract FastMoney is Ownable, Pausable, VerifySignature {
    ...
    constructor(address[3] memory _techAccounts, address token) {
        ...
        for (uint8 i = 0; i < levels.length; i++) {
            users[1].expires[i] = 1 << 37;
            users[2].expires[i] = 1 << 37;
            users[3].expires[i] = 1 << 37;
        }
    }
    ...
    function allowancePaymentToken(uint256 amount) external view returns(bool){
        return (paymentToken.allowance(msg.sender, address(this)) >= amount);
    }
    ...
    function registerUser(uint256 _referrer)
        public
        whenNotPaused
    {
        require(
