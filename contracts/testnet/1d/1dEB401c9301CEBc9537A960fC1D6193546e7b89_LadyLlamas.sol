 ...
 */
  ...
  function publicMintFiveToOne(
    ...
    for(uint x=0; x < length;) {
      ...
      if (LBLNFT.ownerOf(ids[x]) == _msgSender()){
        ++quant;
        LBLUsed[ids[x]] = true;
      }
      unchecked { ++x; }
  ...
  function publicMintThreeToOne(
    ...
    if (threeToOne[_msgSender()]) {
      revert AlreadyMinted();
    }
    // checks & effects
    threeToOne[_msgSender()] = true; // locks them out
    uint length = ids.length;
    uint quant;
    for(uint x=0; x < length;) {
      if (LBLUsed[ids[x]]) {
  ...
  function whitelistMint()
    ...
    // checks & effects
    bool check = _myWhitelistStatus(_msgSender());
    if (!check) {
  ...
  function publicMintOneToOne(
    ...
    // checks & effects
    oneToOne[_msgSender()] = true; // locks them out
    if (LBLNFT.ownerOf(id) == _msgSender()){
      LBLUsed[id] = true;
    } // consumes id
 ...
 */
  ...
  // @param timeTwo: unix timestamp
  // @param timeThree: unix timestamp
  // @param timeFour: unix timestamp
  ...
  , uint timeFour
    ...
    timeThreeStart = timeThree;
    timeThreeEnd = timeFour;
