...
contract EGYP is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = unicode"EGYP";
    string private constant _symbol = "EGYY";
    ...
    function setdevelopmentWallet(address payable _address) external onlyOwner {
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 development = amount.mul(1000).div(9999);
        _developmentWallet.transfer(development);
    ...
    function setBuyFee(uint256 buyFee) external onlyOwner {
        require(_buyFee <= 25, "Buy fee is over 25%");
        _buyFee = buyFee;
    ...
    function setSellFee(uint256 sellFee) external onlyOwner {
        require(_sellFee <= 25, "Sell fee is over 25%");
        _sellFee = sellFee;
