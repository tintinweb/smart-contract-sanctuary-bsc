...
interface IMeNft is IERC721Enumerable{
...
contract MeStake is Ownable {
  ...
  mapping(address => uint256) public withdrawnIncome;
  mapping(address => ConsensusPool[]) public cpools;
  uint256 public rewardDuration;
  uint public consensusPoolCount;
  uint[7] public referralRewards = [10, 5, 5, 5, 5, 5, 5];

  event ClaimReward(uint256 amount);
  event PurchaseStar(uint star, uint256 amount);

  constructor(address _meToken, address _meNft) {
  ...
  function setMeMeNft(address nftAddr) public onlyOwner {
    meNft = IMeNft(nftAddr);
  }
  ...
  function purchaseStar() public {
    address referrer = meNft.getRecommender(msg.sender);
    ...
    for(uint8 i = 0; i < referralRewards.length; i ++) {
      upline = meNft.getRecommender(upline);
      if (upline == address(0))
        break;
  ...
  function getRewardRate(address account) public view returns(uint) {
    uint recmCount = meNft.getRecommendedCount(account);
    if (recmCount < 4)
      return 30; // 0.3%
    if (recmCount < 7)
      return 40; // 0.4%
  ...
  function getBalance(address account) public view returns(uint256 tReward, uint256 tResidual, uint256 csAmount) {
  ...
  function getConsensusPoolAmount(address account) public view returns(uint256) {
    (, uint256 residual, ) = getBalance(account);
    return residual;
  }

  function personalConsensusPool(address account) public view returns(uint256) {
    (,,uint256 csAmount) = getBalance(account);
    return csAmount;
  }
  ...
  function claimReward() public {
    ...
    require(meToken.balanceOf(address(this)) >= meAmount, "[MeStake] Insufficient ME token balance in the contract!");
    meToken.transfer(msg.sender, meAmount);
    withdrawnIncome[msg.sender] += meAmount;
