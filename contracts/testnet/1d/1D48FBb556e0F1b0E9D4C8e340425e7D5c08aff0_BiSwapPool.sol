...
abstract contract SingleRewardPool is ISingleRewardPool, ReentrancyGuard, Ownable, Pausable {
    ...
    */
    ...
    /// @notice Функция, реализующая логику переустановки стейкинг токена.
    /// @dev Функция может быть вызвана только владельцем контракта.
    /// @param stakingToken_ Адрес нового стейкинг токена.
    function setStakingToken(address stakingToken_) external onlyOwner {
        stakingToken = stakingToken_;
        emit StakingTokenUpdated(stakingToken_);
    }

    /// @notice Функция, реализующая логику переустановки наградного токена.
    /// @dev Функция может быть вызвана только владельцем контракта.
