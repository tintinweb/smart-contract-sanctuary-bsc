...
contract AVONFT is ERC165, IERC1155,  IAVONFT, IERC1155MetadataURI, Ownable {
    using Address for address;
    ...
    modifier onlyMinter() {
        require((owner() == _msgSender() || isAuthor(_msgSender())), "Caller is not the minter");
        _;
    }
    ...
    function isTokenAuthor(address _account, uint256 _id) public view override returns(bool) {
        if (_authorIds[_account] == _tokens[_id].authorID) {
            return true;
        } else {
            return false;
        }
    ...
    function isBanned(address _account) public view override returns(bool) {
        return _isBanned[_account];
    }
    ...
    function getPlatformRoyalty(uint256 _id) public view override returns(uint256) {
        return _tokens[_id].platformRoyalty;
    }

    function getTokenPrice(uint256 _id) public view override returns(uint256) {
        return _tokens[_id].price;
    }

    function getTokenBroker(uint256 _id) public view override returns(address) {
        return _tokens[_id].broker;
    }

    function getAuthorRoyalty(uint256 _id) public view override returns(uint256) {
        return _tokens[_id].authorRoyalty;
    }

    function getBrokerRoyalty(uint256 _id) public view override returns(uint256) {
        return _tokens[_id].brokerRoyalty;
    }

    function getAuthorByTokenId(uint256 _id) public view override returns (address) {
        return _authors[_tokens[_id].authorID].wallet;
    }

    function setSalesContract(address _account) external onlyOwner {
        require((_account != address(0) && _account.isContract()), "Invalid contract address");
        salesContract = _account;
    }
    ...
    function create (
        ...
        require(salesContract != address(0), "Sales contract is not set");
        if (_msgSender() == owner()) {
            require(author != address(0), "Zero address for author is prohibited");
            require(isAuthor(author), "This author is not registered");
        } else {
            author = _msgSender();
            require(_authors[_authorIds[author]].status < 2, "Authors with status 2 cannot mint tokens.");
        }
        require(!isBanned(_msgSender()), "User is banned");
        ...
        _mint(salesContract, _id, initialSupply, "");
    ...
    function registerAuthor() public returns(uint256) {
        uint256 id = _register(_msgSender(), 0);
        return id;
    }


    function registerAuthor(address _account, uint8 _status) public onlyOwner returns(uint256) {
        require(_account != address(0), "Invalid author address");
        require(_status <= 2, "Invalid status value");
        uint256 id = _register(_account, _status);
        return id;
    }


    function _register(address _account, uint8 _status) internal returns(uint256) {
        require(!isAuthor(_account), "Author is already registered");
    ...
    function setTokenPrice(uint256 _id, uint256 _price) external {
        require(exists(_id), "Token doesn't exist");
        require((_msgSender() == owner() || isTokenAuthor(_msgSender(), _id)), "Unauthorized.");
        require(_price > 0, "Zero price not allowed");
        _tokens[_id].price = _price;
    }
    ...
    function safeTransferFrom(
        ...
        if (from != _msgSender() && !isApprovedForAll(from, _msgSender())) {
            require(_tokenApprovals[from][_msgSender()][id] >= amount, "ERC1155: caller is not owner nor approved");
