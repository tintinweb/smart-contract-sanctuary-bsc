...
contract matrixtest is Ownable {
  ...
  function finalization(address[] memory triangles) external onlyOwner() returns (bool) {
    ...
    unlock15(triangles[0]);
    unlock15(triangles[1]);
    unlock15(triangles[2]);
    unlock15(triangles[3]);
    unlock15(triangles[4]);
    unlock15(triangles[5]);
    unlock15(triangles[6]);
    return true;
  }
  ...
  function unlock15(address account) internal {
    ...
    do{
      i++;
      if(!users[account].activeLevels[i]){
