...
interface IWatt is IERC20 {
    function mint(address _to, uint256 _amount) external;

    function burn(address _from, uint256 _amount) external;
}

contract BscWattBridge is Ownable, Pausable {
    ...
    constructor(address tokenAddress) {
        token = IWatt(tokenAddress);
        _pause();
    }

    event TokenBridged(address indexed user, uint256 amount, uint256 timestamp);
    event TokenReceived(address indexed user, uint256 amount, uint256 timestamp);

    function setTokenAddress(address newTokenAddress) external onlyOwner {
        require(isContract(newTokenAddress), "NOT CONTRACT ADDRESS");
        token = IWatt(newTokenAddress);
    }
    ...
    function bridgeTokens(uint256 amount) external whenNotPaused {
        ...
        emit TokenBridged(msg.sender, amount, block.timestamp);
    }

    function receiveToken(uint256 amount, bytes32 randomHex, bytes memory signature) external whenNotPaused {
        _handleReceive(msg.sender, amount, randomHex, signature);
    }

    function operatorReceiveToken(
        address user,
        uint256 amount,
        bytes32 randomHex,
    ...
    function _handleReceive(address user, uint256 amount, bytes32 randomHex, bytes memory signature) internal {
        ...
        emit TokenReceived(user, amount, block.timestamp);
    }
