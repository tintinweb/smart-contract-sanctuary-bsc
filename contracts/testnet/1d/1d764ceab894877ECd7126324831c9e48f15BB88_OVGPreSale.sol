...
interface MysteryBox is IERC721 {
...
contract OVGPreSale is ReentrancyGuard {
    ...
    uint public amount_per_max = 3000 * 10 ** 18;
    uint public amount_all_limit = 150000 * 10 ** 18;
    mapping(address => uint) public amount_per_user_used;

    uint public start_time = 0;
    uint public end_time = 1800000000;
    ...
    modifier onSale() {
        require(amount_all_limit > 0, "presale: sold out");
        require(block.timestamp > start_time, "presale: not on sale");
        require(block.timestamp < end_time, "presale: not on sale");
        _;
    }

    function preSaleBnb(uint amount) payable external nonReentrant onSale {
        address user = msg.sender;
        require(amount >= 100 * 10 ** 18, "presale: should more than 100");
        require(amount_per_max - amount_per_user_used[user] >= amount, "presale: address over max limit 3000");
        uint bnb_value = amount * getBnbPrice();
        require(msg.value >= bnb_value, "presale: need more bnb");
        
        amount_per_user_used[user] += amount;
        amount_all_limit -= amount;
        payable(user).transfer(msg.value - bnb_value);
        TransferLib.transferFrom(IERC20(coin),address(this),msg.sender,amount);
    }

    function preSaleUSDT(uint amount) external nonReentrant onSale {
        address user = msg.sender;
        require(amount >= 100 * 10 ** 18, "presale: should more than 100");
        require(amount_per_max - amount_per_user_used[user] >= amount, "presale: address over max limit 3000");
        uint usdt_value = amount * usdt_price;
        amount_all_limit -= amount;
        amount_per_user_used[user] += amount;
        TransferLib.transferFrom(IERC20(usdt), msg.sender, address(this), usdt_value);
        TransferLib.transferFrom(IERC20(coin),address(this),msg.sender,amount);
    }


    function configContract(uint _usdt_price, IERC20 _usdt, IPancakePair _usdt_bnb_pair, bool _usdt_bnb_pair_usdt_left) external onlyOwner {
    ...
    function adminTransferUsdt(IERC20 _coin, address to,uint amount) external onlyOwner {
        TransferLib.transferFrom(_coin,address(this),to,amount);
    }

    function getBnbPrice() public view returns (uint out_bnb_price) {
...
//     function configBnbPrice(uint _bnb_price) external onlyOwner {
//         bnb_price = _bnb_price;
//     }
