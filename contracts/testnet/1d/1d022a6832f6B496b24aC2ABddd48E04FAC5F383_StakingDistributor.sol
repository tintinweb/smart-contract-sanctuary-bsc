...
contract StakingDistributor is Ownable {
  ...
  */
  function distribute() external returns (bool) {
    if (nextEpochBlock <= block.number) {
      ...
      for (uint256 i = 0; i < getInfoLength(); i++) {
        if (info[i].rate > 0) {
  ...
  */
  ...
  function addRecipient(address _recipient, uint256 _rewardRate) external onlyOwner {
    require(_recipient != address(0), "StakingDistributor:: add recipient from the zero address");
  ...
  */
  function removeRecipient(uint256 _index, address _recipient) external onlyOwner {
    require(_recipient == info[_index].recipient, "StakingDistributor:: address is not in list");
  ...
  function nextRewardAt(uint256 _rate) public view returns (uint256) {
    return ERC20(EVHY).totalSupply().mul(_rate).div(1000000);
  }
