...
//=====================================================================================//
// Game Information //
    address private deadWallet = 0x0000000000000000000000000000000000000000;
    uint public _amountRequired;
    ...
    bool public _maintenance;
    uint private _playerAmountToReceive;
    uint8 public _dayGame;
...
// Owner Section //
    ...
    function setBucketAddress(address newbucketAddress) public onlyOwner{
      bucketAddress = newbucketAddress;
    }

    function setMarketingFeeReceiverAddress(address  newMarketingFeeReceiverAddress) public onlyOwner{
      marketingFeeReceiverAddress = newMarketingFeeReceiverAddress;
    }

    function setTeamFeeReceiverAddress(address  newTeamFeeReceiverAddress) public onlyOwner{
      teamFeeReceiverAddress = newTeamFeeReceiverAddress;
    }

    function setAutoLiquidityReceiverAddress(address  newAutoLiquidityReceiverAddress) public onlyOwner{
    ...
    function MaintenanceActive(uint8 maintenance) public onlyOwner{
      if(maintenance == 1){
        _maintenance = true;
      }else{
        _maintenance = false;
    ...
    // SCISSOR > PAPER
    // PAPER > ROCK
    function PlayRounds() private onlyOwner{
      uint _id;
    ...
    function setDailyUpdate(uint16 newAmount) public onlyOwner{
      setNewPlayer(deadWallet,0);
      setNewPlayer(deadWallet,0);
...
//Getting player info//
    function getPlayerInfo(uint playerId) view public returns (Player memory player ) {
      Player memory playertemp;
      playertemp  = players[playerId];
      return(playertemp);
/*       uint    id;
      string  name;
      uint    gameMode;
  ...
  function _transfer(address sender, address recipient , uint256 amount) internal virtual override {
    require(_maintenance == false,"Game is Updating");
      if(sender != owner()){
        ...
        if(editions[editions.length-1].qtyDays == 1 && getPlayerRegistered(recipient,editions.length-1) == false){
          preTransfer(sender, amount);
          setNewPlayer(recipient,amount.mul(_playerAmountToReceive).div(100));
        }else if(getPlayerRegistered(recipient,editions.length-1)){
          uint playerId = getPlayerId(recipient,editions.length-1);
          preTransfer(sender, amount);
          amount = amount.mul(_playerAmountToReceive).div(100);
          require(getPlayerAmount(playerId).add(amount)<=_totalAmount,"Player amount > maximum");
