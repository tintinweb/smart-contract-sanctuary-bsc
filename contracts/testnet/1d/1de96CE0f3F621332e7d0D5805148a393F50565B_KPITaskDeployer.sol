...
import "../task/KPITask.sol";

contract KPITaskDeployer is IKPITaskDeployer {
    ...
    function deployTask(
        ...
        address projectContract,
        address owner
    ) external override onlyFactory returns (address task) {
        ...
        task = address(new KPITask());
        delete parameters;
    }

    constructor(address _factory) {
        factory = _factory;
        IKPIProjectFactory(factory).setTaskDeployerContract(
            msg.sender,
            address(this)
        );
    }
...
interface IKPITaskDeployer {
    ...
    /// @return factory The factory contract, token Token address to be used in the project, projectContract Project contract address, owner The owner address
    function parameters()
        external
        view
        returns (
...
contract KPITask is IKPITask {
    ...
    TaskInfo public override taskInfo;
    ...
    constructor() {
        ...
        taskInfo.factory = factory;
        taskInfo.token = token;
        taskInfo.projectContract = projectContract;
    ...
    function createTask(
        ...
        require(IKPIProject(taskInfo.projectContract).isAdmin(msg.sender));
        taskInfo.taskCount += 1;
        newTaskID = taskInfo.taskCount;
        ...
        IKPIPayment(taskInfo.paymentContract).depositTask(
            newTaskID,
            msg.sender,
            reward
        );
        IKPITaskControl(taskInfo.controlContract).createTask(newTaskID);
        emit TaskCreated(taskInfo.projectContract, newTaskID, reward);
        return newTaskID;
    }

    /// @inheritdoc IKPITaskActions
    function receiveTask(uint256 taskID, bool isAutoClaim) external override {
        require(getTask[taskID].isExist);
        require(IKPIProject(taskInfo.projectContract).isMember(msg.sender));
        require(
        ...
        uint256 guarantee = IKPITaskControl(taskInfo.controlContract).guarantee(
            msg.sender
        );
        (, , uint256 guaranteeAmount, , , ) = IKPIProject(
            taskInfo.projectContract
        ).projectInfo();
        if (guarantee < guaranteeAmount) {
            uint256 missing = guaranteeAmount - guarantee;
            IKPIPayment(taskInfo.paymentContract).depositGuaranteeAmount(
                msg.sender,
                missing
            );
            IKPITaskControl(taskInfo.controlContract).changeMemberGuarantee(
                msg.sender,
                guaranteeAmount
            );
        }
        getTask[taskID].status = TaskStatus.IN_PROGRESS;
        IKPITaskControl(taskInfo.controlContract).receiveTask(
            msg.sender,
            taskID
        );
        // make sure that this property is set properly
        if (isAutoClaim) {
            getTask[taskID].isAutoClaim = true;
        } else {
            getTask[taskID].isAutoClaim = false;
        }
        if (getTask[taskID].assignee == address(0)) {
    ...
    function finishTask(uint256 taskID) external override {
        ...
        IKPITaskControl(taskInfo.controlContract).finishTask(
            msg.sender,
            taskID
        );
    }
    ...
    function penalizeTask(uint256 taskID) external override {
        ...
        uint256 totalPenalized = IKPIPayment(taskInfo.paymentContract)
            .penalized(taskID, task.assignee, task.remaining, guarantee);
        IKPITaskControl(taskInfo.controlContract).mapIndexToPenalizedTask(
            task.assignee,
            taskID
        );
        task.status = TaskStatus.PENALIZED;

        emit Penalized(taskID, totalPenalized, task.assignee);
    }
    ...
    function cancelTask(uint256 taskID) external override {
        ...
        require(IKPIProject(taskInfo.projectContract).isAdmin(msg.sender));
        IKPIPayment(taskInfo.paymentContract).canceled(taskID, task.remaining);
        if (task.assignee != address(0)) {
            if (task.status != TaskStatus.TODO) {
                IKPITaskControl(taskInfo.controlContract).cancelTask(
                    task.assignee,
                    taskID
                );
            }
    ...
    function approveTask(uint256 taskID) external override {
        ...
        if (task.deadline < block.timestamp) {
            ...
            IKPITaskControl(taskInfo.controlContract).mapIndexToLateTask(
                task.assignee,
                taskID
            );
        } else {
            task.status = TaskStatus.DONE;
            IKPITaskControl(taskInfo.controlContract).mapIndexToCompletedTask(
                task.assignee,
                taskID
            );
        }
        if (task.isAutoClaim) {
            uint256 amount = IKPIPayment(taskInfo.paymentContract).withdrawTask(
                taskID,
                task.assignee,
                task.remaining
            );
            IKPITaskControl(taskInfo.paymentContract).increaseEarned(
                task.assignee,
                amount
            );
        }
        if (task.status == TaskStatus.DONE && !task.isAutoClaim) {
            IKPITaskControl(taskInfo.controlContract).addCompletedTask(
                taskID,
                task.assignee,
                task.remaining
            );
        }
        IKPITaskControl(taskInfo.controlContract).approveTask(
            task.assignee,
            taskID
        );
    }
    ...
    function resetTask(
        ...
        IKPIPayment(taskInfo.paymentContract).resetTask(
            taskID,
            msg.sender,
            newAmount
        );
        IKPITaskControl(taskInfo.controlContract).resetTask(
            task.assignee,
            taskID
        );
    }
    ...
    function forceDone(uint256 taskID) external override {
        ...
        if (task.isAutoClaim) {
            ...
            IKPITaskControl(taskInfo.controlContract).increaseEarned(
                task.assignee,
                amount
            );
        } else {
        ...
        IKPITaskControl(taskInfo.controlContract).forceDone(
            task.assignee,
            taskID
        );
    ...
    function claimTask(uint256 taskID) external override {
        ...
        require(task.assignee == msg.sender);
        if (task.status != TaskStatus.LATE) {
            IKPITaskControl(taskInfo.controlContract).removeCompletedTask(
                taskID,
                msg.sender
            );
        }
    ...
    function withdrawGuarantee() external override {
        (, bool isMember, address alternate) = IKPIProject(
            taskInfo.projectContract
        ).getMember(msg.sender);
        uint256 guarantee = IKPITaskControl(taskInfo.controlContract).guarantee(
            msg.sender
        );
        uint256 inProgress = IKPITaskControl(taskInfo.controlContract)
            .inProgress(msg.sender);
        require(isMember);
        require(inProgress == 0);
        require(guarantee > 0);
        address receiver = alternate == address(0) ? msg.sender : alternate;
        IKPITaskControl(taskInfo.controlContract).changeMemberGuarantee(
            receiver,
            0
        );
        IKPIPayment(taskInfo.paymentContract).withdrawGuaranteeAmount(
            receiver,
            guarantee
        );
    }
    ...
    /// @inheritdoc IKPITaskActions
    function depositTask(uint256 taskID, uint256 reward) external override {
        require(getTask[taskID].isExist);
    ...
    {
        require(getTask[taskID].isExist);
        require(getTask[taskID].remaining > 0);
        require(IKPIProject(taskInfo.projectContract).isAdmin(msg.sender));
    ...
    /// @inheritdoc IKPITaskActions
    function changeAutoClaim(uint256 taskID, bool isAutoClaim)
        external
        override
    {
        require(getTask[taskID].isExist);
        require(getTask[taskID].remaining > 0);
        require(TaskStatus.canSetAutoClaim(getTask[taskID].status));
        require(getTask[taskID].assignee == msg.sender);
        getTask[taskID].isAutoClaim = isAutoClaim;
    }
    ...
    function transfer(address to, uint256 amount)
        ...
        onlyPaymentContract
    {
        IERC20 tokenERC20 = IERC20(taskInfo.token);
        ...
        returns (uint256)
    {
        ...
        uint256[] memory completedTasks = IKPITaskControl(
            taskInfo.controlContract
        ).getAndRemoveCompletedTasks(member);
        uint256 length = completedTasks.length;
        for (uint256 i = 0; i < length; i++) {
            uint256 taskID = completedTasks[i];
            totalReward += getTask[taskID].remaining;
            getTask[taskID].remaining = 0;
        }
        IKPITaskControl(taskInfo.controlContract).increaseEarned(
            msg.sender,
            totalReward
        );
        IKPIPayment(taskInfo.paymentContract).withdrawAllCompletedTasks(
            msg.sender,
            totalReward
        );
    ...
    function dropTask(uint256 taskID) external override {
        require(getTask[taskID].isExist);
        Task storage task = getTask[taskID];
        require(msg.sender == task.assignee);
        require(
        ...
        if (task.status == TaskStatus.IN_PROGRESS) {
            uint256 guarantee = IKPITaskControl(taskInfo.controlContract)
                .guarantee(msg.sender);
            IKPITaskControl(taskInfo.controlContract).changeMemberGuarantee(
                task.assignee,
                0
            );
            IKPIPayment(taskInfo.paymentContract).dropTask(
                getDepositor[taskID],
                guarantee
            );
            task.status = TaskStatus.TODO;
        }
        IKPITaskControl(taskInfo.controlContract).dropTask(
            task.assignee,
            taskID
        );
        task.assignee = address(0);
    ...
    /// @inheritdoc IKPITaskActions
    function setTotalReward(uint256 taskID, uint256 totalReward)
        external
        override
        onlyPaymentContract
    {
        getTask[taskID].totalReward = totalReward;
    }
    ...
    function setTaskControlContract(address taskControl)
        ...
        onlyFactory
    {
        taskInfo.controlContract = taskControl;
    }

    modifier onlyPaymentContract() {
        require(
            msg.sender == taskInfo.paymentContract ||
    ...
    modifier onlyFactory() {
        require(msg.sender == taskInfo.factory);
        _;
    }
}