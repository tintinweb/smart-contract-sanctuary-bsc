    ...
    PausableUpgradeable
{
    ...
    function mint(
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            tokenId = supply + i;
            staking(tokenId, _stakingAmount, _stakerName);
    ...
    function claimRewardsForAllStakers() public {
        ...
        uint256 bagSizeReward = 0;
        uint256 stakingTimeReward = 0;

        for (uint256 i = 0; i < stakingNFTs.length; i++) {
            if (_exists(stakingNFTs[i])) {
        ...
        uint256 _tokenId = 0;
        for (uint256 i = 0; i < stakingNFTs.length; i++) {
            _tokenId = stakingNFTs[i];
            if (_exists(_tokenId)) {
                bagSizeReward =
                    ((_curRemainReward / 2) *
                        getLockedAmountByTokenId(_tokenId)) /
                    totalBagSize;
                stakingTimeReward =
                    ((_curRemainReward / 2) *
                        (uint80(block.timestamp) -
                ...
                if (bagSizeReward + stakingTimeReward > 0) {
                    novo.transferFrom(
                        address(this),
                        ownerOf(_tokenId),
                        bagSizeReward + stakingTimeReward
                    );
                }
            }
        }
