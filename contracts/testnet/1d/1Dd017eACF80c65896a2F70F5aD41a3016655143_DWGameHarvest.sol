...
contract DWRandomizator {
    ...
    function getRandom(
        ...
        uint256 random = uint256(
            keccak256(
                abi.encodePacked(blockhash(idToBlock[_id]), wallet, _preseed)
            )
        );
        return random;
    }
...
contract DWSubGame {
    ...
    // Setters
    function setStablizers() external onlySupport {}
...
contract FarmerWaifu is ERC721 {
  ...
  */
    ...
    function multimint(
        address _newOwner,
        uint256[] calldata _dnas,
        uint256[] calldata _durabilities,
        uint8[6][] calldata _waifuPowers
    ) external onlyGame {
        uint64 _birthday = uint64(block.timestamp);
...
contract DWGameHarvest is DWSubGame {
    ...
    uint256[] public tractorDamageRate;
    uint256[] public waifuIllnessRate;

    mapping(uint256 => uint256) public fieldMaterialIndex;
    ...
    function goHarvest(
        ...
        require(
            msg.value >= game.harvestGas(),
            "Not enough BNB sent for harvesting."
        );
        uint256 _rewardGas = msg.value.mul(game.rewardComission()).div(100);
        uint256 _harvestGas = msg.value.sub(_rewardGas);
        sendBNB(game.rewardPool(), _rewardGas);
        sendBNB(game.bella(), _harvestGas);
        ...
        for (uint256 i = 0; i < _farmerIds.length; i++) {
            if (
                ...
                farmer.getLicense(_farmerIds[i]) >= uint64(block.timestamp)
            ) {
    ...
    function resolveHarvest(
        ...
        uint256 seed = randomizator.getRandom(_id, _target, 0);
        uint256 harvested = generator.generateRewardAmount(
            seed,
    ...
    function sellHarvestToOracle(uint256[] memory _amounts) external {
        ...
        for (uint256 i = 0; i < _amounts.length; i++) {
            total += _amounts[i] * harvest.getPrice(i);
            if (_amounts[1] > 0) {
