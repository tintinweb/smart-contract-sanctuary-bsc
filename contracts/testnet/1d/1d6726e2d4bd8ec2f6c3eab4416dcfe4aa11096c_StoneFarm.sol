...
contract StoneFarm is Ownable, ReentrancyGuard {
    ...
    // main net: 0xeDEe132Fcf90FE3157C389706b30c6A1c7A6C88D
    address private _materials =0x04898c211e112e558def9f28B22640Ef814f56e6 ;
    // main net: 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82
    address private _cakeToken = 0xb98C30fA9f5e9cf6749B7021b4DDc0DBFe73b73e;
    // main net: 0x73feaa1eE314F8c655E354234017bE2193C9E24E
    address private _cakeFarm = 0x1ED62c7b76AD29Bfb80F3329d1ce7e760aAD153d;
   ...
   //This contract start from pid5 
    uint256 private cakepid =5;
    ...
    function depositCAKE(uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            cakeFarm.deposit(cakepid,_amount);
    ...
    function withdrawCAKE(uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            user.amount = user.amount.sub(_amount);
            // pool.lpToken == cakeToken
            cakeFarm.withdraw(cakepid,_amount);
            pool.lpToken.safeTransfer(address(msg.sender), _amount);
    ...
    function withdrawBonus(address _bonusaddress, address _recaddress, uint256 _amount) public onlyOwner {
        bonustoken=IERC20(_bonusaddress);
        uint256 bonusBal = bonustoken.balanceOf(address(this));
        require(_amount <= bonusBal && _amount > 0, "withdraw bonus exceeds contract's balance");

        bonustoken.safeTransfer(_recaddress, _amount);
    }

    // Update cakefarm pool
    function migration(address _newcakeFarm,uint256 _newcakepid) public onlyOwner{
         cakeFarm.withdraw(cakepid,cakePoolBalance);
         _cakeFarm=_newcakeFarm;
         cakepid= _newcakepid;
         cakeFarm = ICakeManual(_newcakeFarm);
         cakeToken.safeIncreaseAllowance(_cakeFarm, cakePoolBalance);
         cakeFarm.deposit(cakepid,cakePoolBalance);
    }
    //adjust material prodcut speed
    function adjust(uint256 _newspeed) public onlyOwner{
         materialPerBlock=_newspeed;
    }
}