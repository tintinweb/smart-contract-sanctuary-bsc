...
contract BoGaBase is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public constant _name = "BoGa Base";
    string public constant _symbol = "BOGA";
    ...
    uint256 public constant bogaRiskFreeValueFee = 40;
    uint256 public constant firePitFee = 20;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(bogaRiskFreeValueFee).add(
            firePitFee
        );
    ...
    address public bogaRiskFreeValueReceiver;
    ...
    constructor()
        ERC20Detailed("BoGa Base", "BOGA", uint8(DECIMALS))
        Ownable()
    {
        ...
        bogaRiskFreeValueReceiver = 0xd25e40DDEDF7D93A1626b577C21b2540781358F9;
    ...
    function takeFee(
        ...
        uint256 _bogaRiskFreeValueFee = bogaRiskFreeValueFee;

        if (recipient == pair) {
            _totalFee = 160;
            _liquidityFee = 50;
            _treasuryFee = 20;
            _bogaRiskFreeValueFee = 70;
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(_treasuryFee.add(_bogaRiskFreeValueFee)).div(
                feeDenominator
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndRFV.mul(treasuryFee).div(
                treasuryFee.add(bogaRiskFreeValueFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(bogaRiskFreeValueReceiver).call{
            value: amountETHToTreasuryAndRFV
                .mul(bogaRiskFreeValueFee)
                .div(treasuryFee.add(bogaRiskFreeValueFee)),
            gas: 30000
        }("");
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no BOGA token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function setFeeReceivers(
        ...
        address _bogaRiskFreeValueReceiver
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        bogaRiskFreeValueReceiver = _bogaRiskFreeValueReceiver;
    }
