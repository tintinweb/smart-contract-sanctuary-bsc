...
contract LandLordManager is Auth, PausableUpgradeable {
  ...
  event ShareHolderClaimed(address indexed landLord, uint mission, address indexed shareHolder, uint shareAmount);
  ...
  function claim(uint _mission) external onlyLandLord(_mission) whenNotPaused {
    ...
    if (pendingReward > 0) {
      ...
      for(uint i = 0; i < lands[_mission].shareHolders.length; i++) {
        ...
        emit ShareHolderClaimed(msg.sender, _mission, shareHolder, shareAmount);
      }
