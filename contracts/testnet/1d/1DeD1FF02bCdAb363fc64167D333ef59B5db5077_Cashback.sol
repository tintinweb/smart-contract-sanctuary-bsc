...
contract Cashback is Ownable {
    ...
    function setFlexibleStaking(address _flexibleStaking) external onlyOwner {
        flexibleStaking = FlexibleStaking(_flexibleStaking);
    }

    function changeBackend(address _newBackend) external onlyOwner {
        backend = _newBackend;
    }
    ...
    function tokenAmountForBNB(uint256 _bnbAmount) external view returns (uint256) {
        address[] memory pair = new address[](2);
        pair[0] = BUSD;
        pair[1] = WBNB;
        return router.getAmountsIn(_bnbAmount, pair)[0];
    }
