...
contract StreamStake is AccessControl, ReentrancyGuard {
     ...
     *    https://docs.soliditylang.org/en/v0.8.11/cheatsheet.html?highlight=block.timestamp#global-variables
     */
    ...
    uint256 public stakeRewardFactor15;
    uint256 public stakeRewardFactor30;
    uint256 public stakeRewardFactor60;
    ...
    constructor(address _stakingToken, address _rewardToken) {
        ...
        // set some defaults
        lockTimePeriod = [15, 30, 60];
        stakeRewardFactor15 = 10 * 1 days;
        stakeRewardFactor30 = 12 * 1 days;
        stakeRewardFactor60 = 14 * 1 days;
     ...
     */
    function setLockTimePeriod(uint48[] memory _lockTimePeriod)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function setStakeRewardFactor30(uint256 _stakeRewardFactor)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        stakeRewardFactor15 = _stakeRewardFactor;
     ...
     * @notice see calculateUserClaimableReward() docs
     * @dev requires that reward token has the same decimals as stake token
     * @param _stakeRewardFactor time in seconds * amount of staked token to receive 1.2 reward token
     */
    function setStakeRewardFactor60(uint256 _stakeRewardFactor)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        stakeRewardFactor30 = _stakeRewardFactor;
        emit StakeRewardFactorChanged(_stakeRewardFactor);
    }
     ...
     * @notice see calculateUserClaimableReward() docs
     * @dev requires that reward token has the same decimals as stake token
     * @param _stakeRewardFactor time in seconds * amount of staked token to receive 1.4 reward token
     */
    function setStakeRewardFactor90(uint256 _stakeRewardFactor)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        stakeRewardFactor60 = _stakeRewardFactor;
        emit StakeRewardFactorChanged(_stakeRewardFactor);
    }
        ...
        returns (uint256 claimableRewardTokens)
    {
        if (address(rewardToken) == address(0)) {
            ...
            User storage user = userMap[_staker];
            return userTotalRewards(_staker) / user.stakeRewardFactor; // safe
        }
    }
     ...
     */
    function _stake(uint256 _amount, uint256 _lockTimePeriod)
        internal
        returns (uint256)
    {
        require(_amount > 0, "stake amount must be > 0");
        //  require(_lockTimePeriod == 2592000 || _lockTimePeriod == 5184000 || _lockTimePeriod == 7776000, "Locking time must be 30, 60 and 90");
        ...
        require(check != false, "Lock time period isn't valid");

        User storage user = _updateRewards(msg.sender); // update rewards and return reference to user

        require(user.alreadyStake != true, "User already staked");

        user.stakeAmount = toUint160(user.stakeAmount + _amount);
        tokenTotalStaked += _amount;
        if (_lockTimePeriod == 15) {
            user.stakeRewardFactor = stakeRewardFactor15;
        } else if (_lockTimePeriod == 30) {
            user.stakeRewardFactor = stakeRewardFactor30;
        } else if (_lockTimePeriod == 60) {
            user.stakeRewardFactor = stakeRewardFactor60;
        } else {
            revert("Invalid lock time period");
        }

        user.unlockTime = toUint48(
            block.timestamp + _lockTimePeriod.mul(3600)
        );
        user.lockedTime = _lockTimePeriod.mul(3600);
        ...
        emit Stake(msg.sender, _amount, toUint48(block.timestamp), _lockTimePeriod); // = user.stakeTime
        return _amount;
    }
    ...
    function stake(uint256 _amount, uint48 _lockTimePeriod)
        ...
        returns (uint256)
    {
        return _stake(_amount, _lockTimePeriod);
    }
