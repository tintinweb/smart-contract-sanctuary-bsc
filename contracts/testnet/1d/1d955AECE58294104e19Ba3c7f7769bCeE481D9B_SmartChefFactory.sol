...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    //stakedtoken total supply
    uint256 stakedTokenSupply1 = 1;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        stakedTokenSupply1 +=_amount;
 
        emit Deposit(msg.sender, _amount);
    }

    /*
    total staked token amount getting function
    */
    function getstakedAmount() external view returns(uint256) {

        return stakedTokenSupply1;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare + (cakeReward * PRECISION_FACTOR) / stakedTokenSupply1;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 cakeReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (cakeReward * PRECISION_FACTOR) / stakedTokenSupply1;
        lastRewardBlock = block.number;
    }
