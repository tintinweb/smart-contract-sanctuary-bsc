...
contract MemeFamilyHoliday is IBEP20, Auth {
    ...
    uint256 public _maxTxAmount = _totalSupply.div(20); // 4.0%
    uint256 public _maxWallet =_totalSupply.div(30); // 3.0%
    ...
    uint[] public _percentages = [20,20,20,20,10,10];    
    address payable[] public _wallets = [                                                // wallet(s) to receive liquidity fee split
        ...
        payable(0x331672DCa1b17ba9624a9a86f7001fC86837C84b)                              // Charity Wallet        
    ...
    function sendBNBToWallets(uint256 amount) private {
        ...
        for(uint i=0; i<_wallets.length-1; i++){
            uint amountToWallet = calculateAmount(amount, _percentages[i]);
    ...
    function checkMXWallet(address recipient, uint256 amount) internal view {
       ...
       require((heldTokens + amount)<= _maxWallet|| isTxLimitExempt[recipient],"Over wallet limit.");
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalBNBFee = totalFee.sub(MemeLiquidityFee.div(2));
        
        uint256 amountBNBLiquidity = amountBNB.mul(MemeLiquidityFee).div(totalBNBFee).div(2);
        uint256 amountBNBReflection = amountBNB.mul(reflectionFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection.div(3)}() {} catch {}
        try distributor1.deposit{value: amountBNBReflection.div(3)}() {} catch {}
         if( Shibenabled ){
        try distributor2.deposit{value: amountBNBReflection.div(2)}() {} catch {}
        try distributor3.deposit{value: amountBNBReflection.div(1)}() {} catch {}
        }
         else 
        {
        try distributor2.deposit{value: amountBNBReflection.div(3)}() {} catch {}
        }
               sendBNBToWallets(amountBNBMarketing);
    ...
    function setShibrewardsenabled(bool _enabled) external authorized {
        Shibenabled = _enabled;
