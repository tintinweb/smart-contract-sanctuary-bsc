...
contract Bond is Ownable {
    ...
    function bond(uint256 _amount) external {
        require(bondStartTime + bondingPeriod > block.timestamp && isAvailableBond(_amount), "not available");
    ...
    function isAvailableBond(uint256 _amount) public view returns(bool) {
    ...
    function userClaimAmount(address user) public view returns(uint256) {
        return bondInfo[user].amount * 10**16 / bondPrice / 10**18 * DOUBLE_MULTER;
    }
