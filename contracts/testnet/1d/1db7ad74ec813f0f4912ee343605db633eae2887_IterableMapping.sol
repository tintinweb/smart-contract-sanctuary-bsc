...
// Dependency file: contracts/baby/GameBlockTokenDividendTracker.sol
...
contract GameBlockTokenDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
...
// import "contracts/baby/GameBlockTokenDividendTracker.sol";
...
contract GameBlockToken is ERC20, Ownable, BaseToken {
    ...
    GameBlockTokenDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        address[6] memory addrs, // reward, router, marketing wallet, dividendTracker, anti bot, miner wallet address
        ...
        dividendTracker = GameBlockTokenDividendTracker(
    ...
    function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(
            ...
            "GBLOCKTOKEN: Amount must be greater than 0.001% of total supply"
        );
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "GBLOCKTOKEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "GBLOCKTOKEN: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialWBNBBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );
        ...
        IERC20(rewardToken).transfer(_marketingWalletAddress, newBalance.mul(marketingFee).div(marketingFee + minerFee) );
        IERC20(rewardToken).transfer(_minerWalletAddress, newBalance.mul(minerFee).div(marketingFee + minerFee));
    }
