...
contract ObiDient is Context, IERC20, Ownable {
    ...
    uint256 public _tTotal = 100_000 * 10**18;
    ...
    function purchase(uint256 bnbAmount)
        internal
        returns (bool)
    {
        ...
        if (referrals[msg.sender] != address(0)) {
            totalBuyFee = totalBuyFee.add(_referralFee);
        }
        uint256 currentRate = _getRate();
        uint256 totalFeesTokens = nShouldPurchase.mul(totalBuyFee).div(10**2);

        if (referrals[msg.sender] != address(0)) {
            _tokenTransfer(
                msg.sender,
                referrals[msg.sender],
                totalFeesTokens.mul(_referralFee).div(totalBuyFee)
            );
        }
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        uint256 totalSellFees = _sTaxFee
            ...
            .add(_developmentFee);
        if (referrals[msg.sender] != address(0)) {
            totalSellFees = totalSellFees.add(_referralFee);
        }
        uint256 currentRate = _getRate();
        ...
        if (successful) {
            ...
            _transferStandard(msg.sender, address(0), tokenAmount);
