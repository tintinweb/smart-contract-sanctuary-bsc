...
contract RefererNft is Initializable, AccessControl,ReentrancyGuard,IEvents {
    IERC20 public token;
    IAdmin public Admin;
    uint256 public expiryDate = 30 days;
    uint public percentage;
    uint[4] public refferrerPercentage = [40,30,20];
        ...
        for(uint i = 0; i <= 2; i++){
            if(userRefferer != address(0)){
                if(users[userRefferer].totalReferralReward == 0){
                    ...
                    token.transfer(Admin.TeamAddress() , users[userRefferer].totalReferralReward); //price as variable
                }
                users[userRefferer].totalReferralReward += (_reward * refferrerPercentage[i])/totalRewardPercentage;   
                console.log("level", i,((_reward * refferrerPercentage[i])/totalRewardPercentage) , leftAmount);  
                leftAmount -= (_reward * refferrerPercentage[i])/totalRewardPercentage;
                userRefferer = users[userRefferer].refferer;
                emit Reward_Earning(user,userRefferer,(_reward * refferrerPercentage[i])/totalRewardPercentage,i,volume, block.number, block.timestamp);
            }else{
                break;
            }
        }
 ...
 */
    function setRefferer(address referrer) public {
        ...
        emit SetterForRefferer(msg.sender,referrer,block.number,block.timestamp);
...
interface IEvents{
    ...
    event SetterForRefferer(address user,address RefferAddress,uint blocknumber,uint blockTimestamp);
