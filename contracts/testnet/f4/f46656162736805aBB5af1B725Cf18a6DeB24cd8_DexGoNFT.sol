...
//        if (limitForType[typeNft] > 0) require(counter + 1 < limitForType[typeNft], "EOL");
    ...
    function convertFromPastContracts(address pastContract, uint[] memory tokenIds) public {
        ...
        for(uint256 x=0;x<tokenIds.length;x++) {
            ...
            IERC721(pastContract).safeTransferFrom(msg.sender, address(0), tokenIds[x]);
...
//            inAppPurchaseInfo[_tokenIdCounter.current()] = _inAppPurchaseInfo[y];
    ...
    {
        require(!IDexGoStorage(storageContract).getInAppPurchaseBlackListWallet(from) && !IDexGoStorage(storageContract).getInAppPurchaseBlackListTokenId(tokenId), "BL");
