...
contract PriceGameV3BscTestnet is OwnableUpgradeable {
    ...
    address public constant CDZ = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee; 
    ...
    uint256 private constant SEED_AMOUNT = 1e15;
    ...
    event Bet(address indexed account, uint256 indexed currentEpoch, address indexed token, uint256 tokenAmount, bool isBull);
    ...
    function betBear(uint32 tokenIndex, uint256 tokenAmount, uint256 amountOutMin) external payable {
        ...
        _bet(msg.sender, tokenIndex, tokenAmount, amountOutMin, false);
    }

    function betBull(uint32 tokenIndex, uint256 tokenAmount, uint256 amountOutMin) external payable {
        if (tokenIndex == INDEX_BNB) {
            tokenAmount = msg.value;
        } else {
            address tokenAddress = tokens[tokenIndex];
        ...
        _bet(msg.sender, tokenIndex, tokenAmount, amountOutMin, true);
    }

    function _bet(address account, uint32 tokenIndex, uint256 tokenAmount, uint256 amountOutMin, bool isBull) internal {
        ...
        if (tokenIndex != INDEX_CDZ) {
            tokenAmount = _swap(tokens[tokenIndex], CDZ, tokenAmount, amountOutMin);
        }

        // Update round data
        if (isBull) {
            round.base.bullAmount += tokenAmount;
        } else {
            round.base.bearAmount += tokenAmount;
        }
        ...
        emit Bet(account, currentEpoch, tokens[tokenIndex],  tokenAmount, isBull);
    }
    ...
    function getSeedAmount() public view returns(uint256) {
        return 0 < seedAmount ? seedAmount : SEED_AMOUNT;
    }

    function setSeedAmount(uint256 _seedAmount) external onlyOwner {
    ...
    function claimTreasury(address payable _to, bool keepSeed) external onlyOwner {
        uint256 amount = 0;
        if (keepSeed) {
            uint256 seed = getSeedAmount();
            if (seed < treasuryAmount) {
                amount = treasuryAmount - seed;
                treasuryAmount = seed;
            }
        } else {
        ...
        IERC20Upgradeable(CDZ).safeTransfer(_to, amount);
    }
    ...
    function placePseudoRandomBet() internal {
        ...
        uint256 amountBull = seed * randomNumber / 100;
        uint256 amountBear = seed - amountBull;
    ...
    function getClaimAmount(uint256 epoch, address user) external view returns (uint256) {
        (uint256 reward,) = _getRewardForUser(epoch, user);
        return reward;
    }
    ...
    function _getRewardForUser(
        ...
        reward = betInfo.tokenAmount
                * (roundInfo.rewardAmount - roundInfo.rewardBaseCalAmount)
