...
import "./Libs/IXToken.sol";
import "./Libs/ISwap.sol";
import "./Libs/ICompoundOla.sol";
import "./Libs/ICompoundVenus.sol";
import "./Libs/IStorage.sol";
...
contract Logic is LogicUpgradeable {
    ...
    uint8 constant vTokenType = 0;
    uint8 constant oTokenType = 1;
    ...
    modifier isTokenTypeAccepted(uint8 leadingTokenType) {
        require(leadingTokenType == vTokenType || leadingTokenType == oTokenType, "E8");
        _;
    }
    ...
    function addXTokens(
    ...
    ) external onlyOwner isTokenTypeAccepted(leadingTokenType) {
        bool _isUsedXToken;
        ...
        if (leadingTokenType == oTokenType) {
            (_isUsedXToken, , , , , ) = IComptrollerOla(olaComptroller).markets(xToken);
        }
     ...
     */
    function approveTokenForSwap(address token) public onlyOwner {
    ...
    {
        if (leadingTokenType == vTokenType) return IComptrollerVenus(venusComptroller).enterMarkets(xTokens);
        if (leadingTokenType == oTokenType) return IComptrollerOla(olaComptroller).enterMarkets(xTokens);
        ...
        isTokenTypeAccepted(leadingTokenType)
    {
        ...
        if (leadingTokenType == oTokenType) IDistributionOla(olaRainMaker).claimComp(address(this), xTokens);
    }
        ...
        returns (uint256)
    {
        ...
        if (leadingTokenType == vTokenType)
            (error, liquidity, shortfall) = IComptrollerVenus(venusComptroller).getAccountLiquidity(
                address(this)
            );
        if (leadingTokenType == oTokenType)
            (error, liquidity, shortfall) = IComptrollerOla(olaComptroller).getAccountLiquidity(
                address(this)
            );
    ...
    function repayBorrowBNBandToken(
        ...
        {
            uint256 totalBorrow = IXTokenETH(xTokenA).borrowBalanceCurrent(address(this));
    ...
    function repayBorrowOnlyTokens(
        ...
        {
            uint256 totalBorrow = IXToken(xTokenA).borrowBalanceCurrent(address(this));
    ...
    function getPriceFromLpToToken(
        ...
        uint256 totalTokenAmount = IERC20Upgradeable(token0).balanceOf(lpToken) * (2);
