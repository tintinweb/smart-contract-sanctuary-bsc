...
contract Token is ERC20, Ownable {
    ...
    modifier isAuth() {
        ...
        require(owner() == msg.sender || msg.sender == safuDevAddress, "Ownable: caller is not the owner");
        _;
    }
    ...
    constructor() ERC20("EverPink", "EPINK") {
        ...
        reserveSellFee = 2;
        charitySellFee = 1;
        totalSellFees = rewardsSellFee + marketingSellFee + devSellFee + reserveSellFee + charitySellFee;
        ...
        reserveBuyFee = 1;
        charityBuyFee = 1;
        totalBuyFees = rewardsBuyFee + marketingBuyFee + devBuyFee + reserveBuyFee + charityBuyFee;
        ...
        reserveWallet = address(newOwner);
        charityWallet = address(newOwner);
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _devFee, uint256 _reserveFee, uint256 _charityFee) external isAuth {
        rewardsBuyFee = _rewardsFee;
        marketingBuyFee = _marketingFee;
        devBuyFee = _devFee;
        reserveBuyFee = _reserveFee;
        charityBuyFee = _charityFee;
        totalBuyFees = marketingBuyFee + rewardsBuyFee + devBuyFee + reserveBuyFee + charityBuyFee;
        require(totalBuyFees <= 20, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _devFee, uint256 _reserveFee, uint256 _charityFee) external isAuth {
        rewardsSellFee = _rewardsFee;
        marketingSellFee = _marketingFee;
        devSellFee = _devFee;
        reserveSellFee = _reserveFee;
        charitySellFee = _charityFee;
        totalSellFees = marketingSellFee + rewardsSellFee + devSellFee + reserveSellFee + charitySellFee;
    ...
    function updateWallets(address _marketingWallet, address _devWallet, address _reserveWallet, address _charityWallet) external onlyOwner {
        excludeFromFees(_marketingWallet, true);
        excludeFromFees(_devWallet, true);
        excludeFromFees(_reserveWallet, true);
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0){
                ...
                tokensForReserve += fees * reserveSellFee / totalSellFees;
            ...
            // on buy
                ...
                tokensForReserve += fees * reserveBuyFee / totalBuyFees;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForMarketing + tokensForDev + tokensForReserve + tokensForCharity;

        if(contractBalance == 0 || totalTokensToSwap == 0) {return;}
        
        uint256 amountToSwapForETH = totalTokensToSwap;
        ...
        if (tokensForRewards > 0) {
            super._transfer(address(this), address(dividendTracker), tokensForRewards);
            
            dividendTracker.distributeTokenDividends(tokensForRewards);
        }
    ...
    function buyBackTokens(uint256 bnbAmountInWei) external {
