...
interface RhythmGems{

    function mint(uint256 amount, address _target ) external returns(bool); 
    function transfer(address recipient, uint256 amount) external returns (bool);
}

contract RhythmGold is IBEP20, Ownable
{
    using Address for address;
    using EnumerableSet for EnumerableSet.AddressSet;
    
    EngineEcosystemContract public _engineecosystem ;
    bool public GemMintingEnabled = true;
    uint256 public GoldtoGemRatio = 1000000000;
    ...
    constructor ( address _ecosystem) {
        ...
        _engineecosystem = EngineEcosystemContract( _ecosystem );
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            tax=_buyTax;
            mintRhythmGems ( amount, recipient );
    ...
    function mintGemsToggle() public onlyOwner {
         GemMintingEnabled = !GemMintingEnabled;
    }

    function setGoldtoGemRation( uint256 _ratio ) public onlyOwner {
         //default 1000000000
         GoldtoGemRatio = _ratio;
    }

    uint256 public _TOTHEAMOUNT;
    address public _TORECEIPIENT;

    function mintRhythmGems ( uint256 amount, address recipient ) public {
        if ( !GemMintingEnabled ) return;
        RhythmGems _rhythmgem =  RhythmGems ( _engineecosystem.returnAddress("RhythmGems") );
        
        uint256 _amount = amount/GoldtoGemRatio;
        _TOTHEAMOUNT = _amount;
        _TORECEIPIENT = recipient;
        //_rhythmgem.transfer ( recipient, _amount);


        if ( _amount > 0 )_rhythmgem.mint ( _amount , recipient);

    }

    function mintRhythmGemsContract ( uint256 amount, address _target ) public onlyOwner {
        RhythmGems _rhythmgem =  RhythmGems ( _engineecosystem.returnAddress("RhythmGems") );
        if ( amount > 0 ) _rhythmgem.mint ( amount,  _target );
    }
