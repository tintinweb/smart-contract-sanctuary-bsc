...
contract Zedrium is Context, IERC20, Ownable {
  ...
  uint256 private _tTotal = 10**10 * 10**18; //10 Billion
  uint256 private _rTotal = (MAX - (MAX % _tTotal));
  uint256 private _tFeeTotal;
  uint256 public maxWalletAmount = _tTotal.mul(500).div(10000);
  string private _name = 'Zedrium';
 ...
 uint256 public reflectFeeSell = 0;
  ...
  address private _teamWalletAddress = 0x000000000000000000000000000000000000dEaD; //Affixed and created dynamically by the smart contract on construction time.
  ...
  uint256 public _maxTxAmount = 1 * 10**8 * 10**18; //1% (0.005x) of Total circulating supply, after burn
 ...
 function setSellFees() private {
  ...
  function calculateLiquidityFee(uint256 _amount)
    ...
    returns (uint256)
  {
    return _amount.mul(_liquidityFee.add(_marketingFee).add(_buybackFee).add(_teamFee)).div(100);
  }
  ...
  function setTeamAddr(address account) external onlyOwner {
    _teamWalletAddress = account;
  }
  ...
  function getTeamAddr() public view returns (address) {
    return _teamWalletAddress;
  }

  function removeAllFee() private {
    if (_taxFee == 0 && _liquidityFee == 0 && _buybackFee == 0 && _teamFee == 0) return;
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    uint256 marketingBalance = contractTokenBalance.mul(_marketingFee).div(_marketingFee.add(_liquidityFee).add(_buybackFee).add(_teamFee));
    uint256 teamBalance = contractTokenBalance.mul(_teamFee).div(_teamFee.add(_liquidityFee).add(_buybackFee).add(_marketingFee));
    uint buybackBal = contractTokenBalance.mul(_buybackFee).div(_marketingFee.add(_liquidityFee).add(_buybackFee).add(_teamFee));
    uint256 liquidityBalance = contractTokenBalance.sub(marketingBalance).sub(buybackBal).sub(teamBalance);

    // split the liquidity balance into halves
    uint256 half = liquidityBalance.div(2);
    uint256 otherHalf = liquidityBalance.sub(half);
    uint256 tokensToSwapForETH = half.add(marketingBalance).add(buybackBal).add(teamBalance);
    ...
    uint256 teamETHBalance = newBalance.mul(teamBalance).div(
        tokensToSwapForETH
      );

    uint buybackBalanceETH = newBalance.mul(buybackBal).div(tokensToSwapForETH);
    uint256 liquidityETHBalance = newBalance.sub(marketingETHBalance).sub(buybackBalanceETH).sub(teamETHBalance);
  ...
  function sendETHToTeam(uint256 amount) private {
    payable( _teamWalletAddress).transfer(amount);
  }
  ...
  function setSellFee(uint buyback, uint marketing, uint liquidity, uint reflect, uint team) public onlyOwner {
      buybackFeeSell = buyback;
      marketingFeeSell = marketing;
      teamFeeSell = team;
      liquidityFeeSell = liquidity;
      reflectFeeSell = reflect;
      require(buyback + marketing + liquidity + reflect + team <= 25, "max 25%");
  }

    function setBuyFees(uint buyback, uint marketing, uint liquidity, uint reflect, uint team) public onlyOwner {
