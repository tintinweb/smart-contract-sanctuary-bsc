    ...
    Auth
{
    ...
    uint16 public TOTAL_SUPPLY = 10000; // The total supply left to mint (Can be burned)
    ...
    uint256[] public firstTable = [0, 1, 2, 3, 4];
    uint256[] public secondTable = [5, 6, 7, 8];
    uint256[] public thirdTable = [9, 10];

    uint256[] littleGhostIds; //LittleGhosts avaliable to acquire
    uint256[] ectoSkeletonIds; //EctoSkeletons avaliable to acquire
    ...
    /* Events */
    event LootboxOpened(uint256 id, uint256 requestID);
    ...
    event AdminFullfilled(uint256 requestID, uint256[] randomNumbers);
    ...
    function openLootBoxes(uint16[] calldata tokenIds) external nonReentrant {
        for (uint16 i = 0; i < tokenIds.length; i++) {
            ...
            emit LootboxOpened(tokenIds[i], requestId_);
        }
        TOTAL_OPEN += tokenIds.length;
    }
        ...
        returns (Lootbox[] memory)
    {
        ...
        for (uint256 i = 0; i < tokenCount; i++) {
            ...
            result[i] = Lootbox(tokenID_, lootboxRequestIds[tokenID_]);
        }
        return result;
    }

    function isLootboxOpened(uint256 _id) public view returns (bool) {
        return lootboxRequestIds[_id] > 0;
    }
    ...
    function burn(uint16 numLootBoxesToBurn) external onlyOwner {
        require(
            numLootBoxesToBurn > 0 && numLootBoxesToBurn <= 1000,
            "You can only burn a 1 - 1000 at the time"
        );
        uint16 NewMaxLootBoxes = TOTAL_SUPPLY - numLootBoxesToBurn;
        require(
        ...
        override
    {
        ...
        for (uint256 i = 0; i < randomWords.length; i++) {
            if (i == 0) {
                ...
                rareRoll =
                    (randomWords[3] % (10000 - ITEM_AMOUNTS[newWords[i]])) +
                    1; // 1 - 9500

                if (
                    rareRoll >=
                    ((10000 - TOTAL_OPEN) - ITEM_AMOUNTS[newWords[i]]) &&
                    (ITEM_AMOUNTS[newWords[i] - 1]) > 0
                ) {
                    ITEM_AMOUNTS[newWords[i]] -= 1;
                ...
                rareRoll =
                    (randomWords[5] % ((10000 - littleGhostIds.length))) +
                    1; // 1 - # of ghosts loaded

                if (
                    rareRoll >=
                    ((10000 - TOTAL_OPEN) - littleGhostIds.length) &&
                ...
                rareRoll =
                    (randomWords[7] % (10000 - ectoSkeletonIds.length)) +
                    1; // 1 - # of skeletons loaded

                if (
                    rareRoll >=
                    ((10000 - TOTAL_OPEN) - ectoSkeletonIds.length) &&
                ...
                rareRoll = (randomWords[8] % (10000 - TOTAL_OPEN)) + 1; // 1 - 10000

                if (
                    rareRoll >= ((10000 - TOTAL_OPEN) - ITEM_AMOUNTS[11]) &&
                    ITEM_AMOUNTS[11] > 0
                ) {
                    newWords[i] = 11;
                    ITEM_AMOUNTS[11] -= 1;
                ...
                rareRoll = (randomWords[9] % (10000 - TOTAL_OPEN)) + 1;

                if (
                    rareRoll >= ((10000 - TOTAL_OPEN) - ITEM_AMOUNTS[12]) &&
                    ITEM_AMOUNTS[12] > 0
                ) {
                    newWords[i] = 12;
                    ITEM_AMOUNTS[12] -= 1;
    ...
    function adminFulfillRandomWords(
        ...
        emit AdminFullfilled(requestId_, randomWords_);
    }
    ...
    ERC721Flexible
{
    ...
    function mintItems(uint256[] memory itemIds, address walletAddress)
        ...
        onlyOwner
    {
        //TODO: maybe add a require here for claim_amount
