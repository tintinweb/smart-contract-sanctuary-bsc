...
contract TokenFactory is Ownable {
    ...
    // Constructor
    constructor(string memory _name, uint256 _totalSupply,uint8 _decimals, string memory _symbol,bool _mintable, bool _burnable  )public {
        name = _name;
...
contract TokenFactoryInstance is Ownable {
    ...
    function createToken(string memory _name, uint256 _totalSupply,uint8 _decimals, string memory _symbol, bool _mintable, bool _burnable) public payable returns(address) {
    require(msg.value >= 0.1 ether, "Transaction fee must be at least 0.1 ETH");
    tokenFactory = new TokenFactory(_name, _totalSupply,_decimals, _symbol,  _mintable, _burnable);
