...
contract PlearnLucky is IPlearnLucky, ReentrancyGuard, Ownable {
    ...
    struct Lucky {
        ...
        uint256 countReservePerBracket;
    ...
    event LuckyNumbersDrawn(uint256 luckyId, uint32[] indexed luckyNumbers, uint32[] reserveNumber);
    ...
    event TicketsTransfer(address indexed from, address to, uint256[] ticketIds);
        ...
        nonReentrant
    {
        ...
        /**
         * @notice Should be burn before start new lucky round?
         */
        ...
        uint32 luckyNumbersPerRound = _getTotalLuckyNumbersPerRound(_luckyId);
        if (randomResults.length < luckyNumbersPerRound) {
            luckyNumbersPerRound = uint32(randomResults.length);
        }
        uint32 reservePerBracket = uint32(randomResults.length) - luckyNumbersPerRound;

        luckyInfo.luckyNumbers = new uint32[](luckyNumbersPerRound);
        if (reservePerBracket != 0) {
            luckyInfo.reserveNumbers = new uint32[](reservePerBracket);
        }

        for (uint256 i = 0; i < randomResults.length; i++) {
            if (i < luckyNumbersPerRound) {
                luckyInfo.luckyNumbers[i] = randomResults[i];
            } else {
                luckyInfo.reserveNumbers[i - luckyNumbersPerRound] = randomResults[i];
            }
        }
        ...
        emit LuckyNumbersDrawn(_luckyId, luckyInfo.luckyNumbers, luckyInfo.reserveNumbers);
    }
    ...
    {
        return (_tickets[_lucks[_luckyId].firstTicketId].number, _tickets[_lucks[_luckyId].lastTicketId].number);
    }
     ...
     */
    function getTotalRandomPerRound(uint256 _luckyId) external view override returns (uint256) {
        return _getTotalLuckyNumbersPerRound(_luckyId) + _lucks[_luckyId].countReservePerBracket;
    }

    function getTotalTicketInRound(uint256 _luckyId) external view override returns (uint256) {
        return _getTotalTicketInRound(_luckyId);
    }
    ...
    function tranferTickets(
        ...
        uint256[] memory ticketIds = new uint256[](_ticketNumbers.length);
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            uint32 ticketNumber = _ticketNumbers[i];
            uint256 ticketId = _getTicketIdFromTicketNumber(_luckyId, ticketNumber);
            ...
            ticket.owner = _to;
            _userTicketIdsPerLuckyId[_to][_luckyId].push(ticketId);
            _removeTicketIdFromOwner(msg.sender, _luckyId, ticketId);
        ...
        emit TicketsTransfer(msg.sender, _to, ticketIds);
    }
    ...
    function _getTotalTicketInRound(uint256 _luckyId) internal view returns (uint256) {
        if (currentTicketId > _lucks[_luckyId].firstTicketId) {
            return currentTicketId - _lucks[_luckyId].firstTicketId;
        } else {
            return 0;
        }
    }
