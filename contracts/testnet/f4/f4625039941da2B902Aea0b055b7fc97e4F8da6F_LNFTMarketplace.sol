...
contract LNFTMarketplace is Market {
    ...
    function _ownerIsValid(
        ...
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            ...
            require(nft.ownerOf(_tokenId) == _owner);
        } else {
            revert();
        }
    }
    ...
    function _paymentTokenIsValid(address _paymentToken) internal view {
        require(_paymentToken == address(0) || _isTokenPayment(_paymentToken));
    }
    ...
    function listItem(
        ...
        require(listing.startingTime == 0);
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            IERC721 nft = IERC721(_nftAddress);
            require(nft.ownerOf(_tokenId) == _msgSender());
            require(nft.isApprovedForAll(_msgSender(), address(this)));
        } else {
            revert();
        }
        ...
        nonReentrant
    {
        ...
        require(listedItem.startingTime != 0);
    ...
    function buyItem(
        ...
        require(_paymentToken == listedItem.paymentToken);
        ...
        if (feeAmount != 0) {
            require(msg.value >= listedItem.pricePerItem.add(feeAmount));

            (bool success, ) = payable(address(this)).call{value: feeAmount}(
                ""
            );
            require(success);
            require(_pricePerItem >= listedItem.pricePerItem);
        }
...
contract MarketTokenRegistry is Ownable {
    ...
    address internal constant LWAP =
        address(0x8Fe4d3f535FED55a9563338a0e7a3e912FC46DAF);
