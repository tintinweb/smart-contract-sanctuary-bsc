...
contract LPStaking is ReentrancyGuard, Pausable, Ownable {
    ...
    function approvedState(address user) external view returns(bool) {
    ...
    function rewardsUser(address user) external view returns (uint256) {
        return rewards[user];
    }
        ...
        updateReward(msg.sender)
    {
        ...
        require(isApproved[msg.sender]=true, "The contract is not approved");
    ...
    // withdrawing incurs in a 10% penalty
        ...
        updateReward(msg.sender)
    {
        ...
        if (reward > 0) {
            ...
            rewardsToken.approve(sasPool, reward);
            ISAS(sasPool).stakeOnNewLock(msg.sender, reward, duration);
            emit RewardStaked(msg.sender, reward);
        }
    }

    // claiming early incurs in a 75% penalty
    function getRewardEarly() public nonReentrant updateReward(msg.sender) {
        uint256 reward = rewards[msg.sender];
        if (reward > 0) {
            rewards[msg.sender] = 0;

            // send 25% to user
            uint256 userAmount = reward.mul(25).div(100);
            if (userAmount > 0)
    ...
    function applyPenalty(uint256 penaltyAmount) internal {
        ...
        if (swapToTreasuryAmount > 0) {
            ...
            rewardsToken.approve(address(router), swapToTreasuryAmount);
    ...
    function setSasPool(address _sasPool) public onlyOwner {
        sasPool = _sasPool;
        emit SetSasPool(_sasPool);
    }

    // pause should be called before any critical action or bug detection
    // this will cause the contract staking to cease
    ...
    event RewardStaked(address indexed user, uint256 reward);
    event RewardsDurationUpdated(uint256 newDuration);
    event Recovered(address token, uint256 amount);
    event SetTreasury(address treasury);
    event SetSecondaryPool(address secondaryPool);
    event SetSasPool(address sasPool);
}
...
interface ISAS {
    function stakeOnNewLock(address user, uint256 amount, uint256 duration) external;
}
