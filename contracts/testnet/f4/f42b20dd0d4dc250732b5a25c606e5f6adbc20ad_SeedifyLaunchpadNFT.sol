...
contract SeedifyLaunchpadNFT is ERC1155, Ownable, Pausable, ERC1155Burnable, ERC2981Base { 
    ...
    mapping(uint256=>projectData) public getProjectDetail;
    // mapping royalties
    ...
    // mint batch items
    function mintBatch(uint256[] memory amounts, uint256 royalty, string[] memory uri)
        public
        whenNotPaused
    {
    ...
    function createProject(string memory hash) public {
        require(isWhitelisted(msg.sender)==true, "Address has not whitelisted!");
        projects++;
        projectArray.push(msg.sender);
        getProjectDetail[projects]=projectData(msg.sender, hash);
    }

    function updateProject(uint256 projectID, string memory hash) public {
        require(isWhitelisted(msg.sender)==true, "Address has not whitelisted!");
        ...
        getProjectDetail[projectID]=projectData(msg.sender, hash);
    }
}