...
interface IERC20Meta is IERC20 {
    function burnSTAKE(uint256 amount) external;
    function mintSTAKE(uint256 amount) external;
}
...
contract MetaGoldStaking is Ownable,ReentrancyGuard {
    ...
    event STAKED_REFERRAL(address indexed from, uint indexed counter, uint256 amount, address indexed referral, uint reward);
    event CLAIMED(address indexed from, uint indexed counter, uint256 amount);
    event CLAIMEDREWARD(address indexed from, uint256 amount);

    function STAKE(uint256 stakeOptionId, uint256 tokens, address referral)
        external
        nonReentrant
        returns (bool)
    {
        ...
        addStaker(stakeOptionId, msg.sender, tokens, block.timestamp, referral);


        if( referral != address(0)) {
            uint256 reward = tokens.mul(
                    stakeOptions[stakeOptionId].referralRewardMilions
                ).div(1000000).div(100);

            if(!referrals[referral].isDeclared) {
                referrals[referral].isDeclared = true;
                referrals[referral].isBlacklisted = false;
                referrals[referral].totalClaimed = 0;
                referrals[referral].totalRewards = 0;
            }
            referrals[referral].totalRewards = referrals[referral].totalRewards.add(reward);
            uint256 tokensToBurn = tokens.sub(reward);
            metaGoldContact.burnSTAKE(tokensToBurn);
            emit STAKED_REFERRAL(msg.sender, stakers[msg.sender].stakeCounter.sub(1), tokens, referral, reward);
        }
        else{
            metaGoldContact.burnSTAKE(tokens);
            emit STAKED(msg.sender, stakers[msg.sender].stakeCounter.sub(1), tokens);
        }
        return true;
    }

    function addStaker(uint256 stakeOptionId, address addr, uint256 tokens, uint256 startPeriod, address referr) private {
    ...
    function CLAIMREWARD() external returns (bool) {
        require(referrals[msg.sender].isDeclared, "You are not referral");
        require (!(referrals[msg.sender].isBlacklisted), "you are blacklisted");
        require(
            referrals[msg.sender].totalClaimed < referrals[msg.sender].totalRewards,
            "You have nothing to claim"
        );

        uint256 rewardAmount = referrals[msg.sender].totalRewards.sub(referrals[msg.sender].totalClaimed);
        metaGoldContact.approve(msg.sender, MAX_INT);
    ...
    function CLAIM(uint256 stakeId) external returns (bool) {
        ...
        metaGoldContact.mintSTAKE(rewardAmount);
    ...
    function getReferral(address addr) external view returns (Referral memory) {
        return referrals[addr];
    }


    function addStakeOption( uint256 periodInSeconds, uint256 rewardMultiplicatorMilions, uint256 referralRewardMilions) external onlyOwner {
        stakeOptions[stakeOptionsCounter] = StakeOption({
            periodInSeconds : periodInSeconds,
            canStake: false,
            canWithdraw: false,
            rewardMultiplicatorMilions: rewardMultiplicatorMilions,
            referralRewardMilions: referralRewardMilions,
            totalTokensStaked: 0,
    ...
    function setReferralRewardMilions(uint256 stakeOptionId, uint256 newValue) external onlyOwner {
        stakeOptions[stakeOptionId].referralRewardMilions = newValue;
    }
    ...
    function testStakingContract() external onlyMetagold view returns (bool yes)  {
        return true;
    }

    function setMetaGold(address ma) external onlyOwner {
            metaGoldAddress = ma;
            metaGoldContact = IERC20Meta(metaGoldAddress);
            metaGoldContact.approve(address(this), MAX_INT);
    }
