...
contract WinUSD is IERC20, Context, Ownable {
    ...
    uint256 _totalSupply = 1_000_000_000;
    // balances
    ...
    function setTradeEnabled(bool _tradeEnabled) external onlyOwner {
    ...
    function _transferFrom(
        ...
        if (
            ...
            pendingTotalFees >= swapTokensAtAmount
        ) {
            inSwap = true;
            uint256 totalFees = devFee.add(lpFee);
            uint256 lpTokens = pendingTotalFees.mul(lpFee).div(totalFees);
            uint256 devTokens = pendingTotalFees.sub(lpTokens);
            swapAndLiquify(lpTokens);
            uint256 ethBalanceBefore = address(this).balance;
            swapTokensForEth(devTokens);
            uint256 receivedETHAmount = address(this).balance.sub(ethBalanceBefore);
            payable(devAddress).transfer(receivedETHAmount);
            pendingTotalFees = 0;
            inSwap = false;
        }
        ...
        if (takeFee) {
            ...
            uint256 externalTax = amount.mul(externalFee).div(FEE_DENOMINATOR);
            ...
            pendingTotalFees = pendingTotalFees.add(externalTax);
            emit Transfer(sender, recipient, amount);
            emit Transfer(sender, address(this), externalTax);
            return true;
        }
        // subtract form sender, give to receiver, burn the fee
        uint256 tAmount = amount.mul(FEE_DENOMINATOR.sub(transferFee)).div(
            10**2
        );
        ...
        returns (bool)
    {
        ...
        uint256 tokensToSend = nShouldPurchase
            .mul(spreadDivisor)
            .div(10**2)
            .mul(FEE_DENOMINATOR.sub(buyFee))
            .div(10**2);
        // revert if under 1
        if (tokensToSend < 1) {
