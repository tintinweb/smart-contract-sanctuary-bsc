...
contract DistributorOwned is Ownable {
  
  address internal _distributor;

  event DistributorTransferred(address indexed newDistributor);

  function setDistributor( address distributor_ ) external onlyOwner() {
    require(distributor_ != address(0), "IA0");
    _distributor = distributor_;
    emit DistributorTransferred( _distributor );
  }
  ...
  modifier onlyDistributor() {
    require( _distributor == msg.sender, "DistributorOwned: caller is not the Distributor" );
    _;
  }

}

contract StakingOwned is Ownable {
  ...
  modifier onlyStaking() {
    require( _staking == msg.sender, "StakingOwned: caller is not the Staking Contract" );
    _;
  }
...
contract StakedHustlerERC20Token is ERC20Permit, VaultOwned, DistributorOwned, StakingOwned, TreasuryOwned {

    using LowGasSafeMath for uint256;

    constructor(
    ) ERC20("Staked Hustler", "a.HUSLR", 18) {
    }
    ...
    function stakeMint(address account_, uint256 amount_) external onlyStaking() {
        _mint(account_, amount_);
    }
