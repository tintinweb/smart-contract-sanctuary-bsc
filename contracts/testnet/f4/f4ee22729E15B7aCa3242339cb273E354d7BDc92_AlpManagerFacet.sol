...
contract AlpManagerFacet is ReentrancyGuard, Pausable, IAlpManager {
    ...
    function mintAlp(address tokenIn, uint256 amount, uint256 minUsd, uint256 minAlp, bool stake) external whenNotPaused nonReentrant override {
        require(amount > 0, "AlpManagerFacet: invalid amount");
        address account = msg.sender;
        uint256 alpAmount = LibAlpManager.mintAlp(account, tokenIn, amount, minUsd);
        require(alpAmount >= minAlp, "LibLiquidity: insufficient ALP output");
        _mint(account, tokenIn, amount, alpAmount, stake);
    }

    function mintAlpBNB(uint256 minUsd, uint256 minAlp, bool stake) external payable whenNotPaused nonReentrant override {
        uint amount = msg.value;
        require(amount > 0, "AlpManagerFacet: invalid msg.value");
        address account = msg.sender;
        uint256 alpAmount = LibAlpManager.mintAlpBNB(account, amount, minUsd);
        require(alpAmount >= minAlp, "LibLiquidity: insufficient ALP output");
...
library LibAlpManager {
    ...
    function mintAlp(address account, address tokenIn, uint256 amount, uint256 minUsd) internal returns (uint256 alpAmount){
        alpAmount = _calculateAlpAmount(tokenIn, amount, minUsd);
        LibVault.deposit(tokenIn, amount, account, false);
        _addMinted(account);
        emit AddLiquidity(account, tokenIn, amount);
    }

    function mintAlpBNB(address account, uint256 amount, uint256 minUsd) internal returns (uint256 alpAmount){
    ...
    function _calculateAlpAmount(address tokenIn, uint256 amount, uint256 minUsd) private view returns (uint256 alpAmount) {
        ...
        require(amountUsd >= minUsd, "LibAlpManager: Insufficient value of USD input");
...
interface IAlpManager {
    ...
    function mintAlpBNB(uint256 minUsd, uint256 minAlp, bool stake) external payable;
