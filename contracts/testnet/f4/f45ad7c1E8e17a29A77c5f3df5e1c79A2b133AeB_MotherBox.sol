...
contract MotherBox is ERC721 {
  ...
  function createNewNFT(address _owner)
    public
    authorized
    nonReentrant
    returns (
    ...
    )
  {
    ...
    nft.mint(_owner, tokenId);
    // nft.mySetTokenURI(tokenId, jsonUrl);
    // nft.setGovernance(msg.sender);
    blindboxTokenIds.push(tokenId);
    emit BatchBlindBoxCreated(msg.sender, address(nft), tokenId, 1, true);
    // emit NFTCreated(msg.sender, address(nft), tokenId,1);
    return (address(nft), tokenId, _owner);
  }
  ...
  function createTreasureBox(
    ...
    // nft.setGovernance(msg.sender);
    blindboxTokenIds.push(tokenId);
    emit CreateTreasureBox(msg.sender, address(nft), tokenId, 1, false);
    // emit NFTCreated(msg.sender, address(nft), tokenId,1);
    return true;
  }
