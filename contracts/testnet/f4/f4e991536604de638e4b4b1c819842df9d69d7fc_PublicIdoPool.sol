...
contract PublicIdoPool is Auth {
    ...
    constructor(
        ...
        if (_lpAddress != address(0)) {
            tokenAccuracy = ERC20(_lpAddress).decimals();
            lpQuantitySold = _price.mul(_totalFundRaising).mul(10 ** tokenAccuracy).div(1e18);
        } else {
            tokenAccuracy = 18;
        ...
        precision = ERC20(fundRaisingAddress).decimals();
        totalFundRaising = _totalFundRaising.mul(10 ** precision);
        quotaTop = totalFundRaising;

        exchangeRate[1] = uint256(1).mul(1e18);
    ...
    function setLpAddress(address _lpAddress) public onlyOperator {
        require(block.timestamp < claimTime, "Timeout operation!");
        require(_lpAddress != address(0), "Lp address cannot be 0!");

        tokenAccuracy = ERC20(_lpAddress).decimals();
        lpAddress = _lpAddress;
        lpQuantitySold = exchangeRate[2].mul(totalFundRaising).mul(10 ** tokenAccuracy).div(10 ** precision).div(1e18);
    }
    ...
    function _upgradeSafeTransfer(address _token, address _to, uint256 _amount) internal {
        ...
        if (_amount > balance) {
            require(_amount.sub(balance) < uint256(1).mul(10 ** uint256(ERC20(_token).decimals())), "Insufficient contract balance!");
    ...
    function claim(bool _isRefund) public inspectLock {
        ...
        if (_isRefund) {
            ...
            uint256 investment = userIdoInfo[msg.sender].investment;
            if (investment > 0 && !userIdoInfo[msg.sender].isRefund) {
                ...
                userIdoInfo[msg.sender].isRefund = true;

                refundAmount = refundAmount.add(investment);
            }
        } else {
            require(isOpenClaim && !userIdoInfo[msg.sender].isRefund, "Can't claim!");
    ...
    function ownerClaim() public onlyOperator inspectOwnerLock {
        ...
        if (availableLimit > 0) {
            if (availableLimit < totalFundRaising) {
                lpBalance = lpQuantitySold.sub(availableLimit.mul(exchangeRate[2]).mul(10 ** tokenAccuracy).div(exchangeRate[1]).div(10 ** precision));
                if (lpBalance > 0) {
    ...
    function getExchangeInfo(address _account) public view returns (
        ...
        if (totalAmountInvested < totalFundRaising) {
            ...
            obtain = userIdoInfo[_account].investment.mul(exchangeRate[2]).mul(10 ** tokenAccuracy);
            obtain = obtain.div(exchangeRate[1]).div(10 ** precision);
        } else {
            proportion = userIdoInfo[_account].investment.mul(1e18).div(totalAmountInvested);
            obtain = userIdoInfo[_account].investment.mul(totalFundRaising).mul(exchangeRate[2]).mul(10 ** tokenAccuracy);
            obtain = obtain.div(exchangeRate[1]).div(totalAmountInvested).div(10 ** precision);
        }
        proportion = proportion.mul(100).div(1e18);
        exchange = obtain.mul(exchangeRate[1]).mul(10 ** precision).div(exchangeRate[2]).div((10 ** tokenAccuracy));
