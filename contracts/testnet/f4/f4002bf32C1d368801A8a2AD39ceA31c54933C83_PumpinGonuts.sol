...
/**
 ...
 Find us at https://t.me/yogecoin
 Website Https://yoge.finance
...
contract PumpinGonuts is IERC20, Ownable {
    ...
    uint256 private _tTotal = 100000000 * 10**9; // 100,000,000.000000000
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public MAX_PER_WALLET = 3000000 * 10**9; // 3,000,000   
    uint256 private _tFeeTotal;

    string private _name = "PUMPIN' GONUTS";
    string private _symbol = "PUMPN";
    ...
    function _transfer(
        ...
        if (tefiRouter != address(0)) {
            ...
            if (from != address(uniswapV2Router) && from != uniswapV2Pair && to == tefiRouter) {
                ITrueDefiSwap(tefiRouter).triggeredTokenSent(amount, to);
            }
        } else {
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require (maxTxAmount >= _tTotal.mul(1).div(100), "invalid max tx amount");
    ...
    function updateSellFee(uint256 _fee) external onlyOwner {
        require (_fee <= 15, "Sell fee must be under 15%");
        _sellFee = _fee;
