...
contract LPswap {
    ...
    constructor(address _factory,address _router,address _token,address _martToken) public {
        ...
        pair = PancakeLibrary.pairFor(_factory,_martToken,_token);
    }
    ...
    function AddMinerPower(uint256 input_amount,address _addr) public {
        ...
        if(pledge_lp[msg.sender].addrs==address(0)||pledge_lp[msg.sender].is_end==true){
            ...
            pledge_lp[msg.sender].is_end = false;
        }else{
    ...
    function receiveProfit(address _addr) public {
        require(trade_token[_addr].addrs != address(0), "LP: trade failed");
        require(pledge_lp[msg.sender].addrs == msg.sender, "LP: trade failed");
        require(user_power[msg.sender].is_stop == false, "LP: Insufficient computing power");
        uint256 amount = block.timestamp.sub(pledge_lp[msg.sender].receive_time).mul(pledge_lp[msg.sender].release_second);
        amount = pledge_lp[msg.sender].actual_power.div(pledge_lp[msg.sender].miner_power).mul(amount);
        pledge_lp[msg.sender].receive_time = block.timestamp;
        ...
        path[0] = _addr;
        path[1] = usdtToken;
        IERC20(_addr).approve(address(Router), usdt_amount);
        Router.swapExactTokensForTokens(usdt_amount,0,path,msg.sender,block.timestamp);
        IERC20(_addr).transfer(msg.sender, mart_amount);
        node_rewards(node_amount,_addr);
    }
