...
contract NorthStar {
    ...
    function getPendingDishes(address adr) public view returns(uint256) {
        ...
        uint256 divider = (block.timestamp - depositTime[msg.sender]) /  180 * 86400;
        return min(CHEF_COST_IN_Dishes, (block.timestamp - lastDishesToChefsConversion[adr]) / (divider + 1)) * addressChefs[adr];
    }
