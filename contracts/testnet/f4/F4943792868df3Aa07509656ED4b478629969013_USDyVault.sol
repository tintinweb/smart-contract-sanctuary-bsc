...
contract USDyVault is AccessControl, Initializable {
    ...
    uint256 public exchangeRate = 100; //cofficient by 100
    uint256 public currentTimestamp;
    uint256 public rebaseRate = 25; //cofficient by 100
    uint256 public epochTime = 8 hours;
    IAdmin public Admin;
    ...
    // @param _USDy USDy token address
    //  */
    function initialize(
        address _admin,
        address owner,
    ...
    function Deposit(uint256 _amount) external {
        ...
        INTT(Admin.USDs()).mint(msg.sender, _amount * 90 / exchangeRate);
        exchangeRate = (IERC20(Admin.USDy()).balanceOf(address(this)) * 100) / INTT(Admin.USDs()).totalSupply();
    ...
    function Withdraw(address user,uint256 amount) external {
        require(amount > INTT(Admin.USDs()).balanceOf(user), 'USDyVault: Your withdraw amount exceeds deposit');
        IERC20(Admin.USDy()).transfer(msg.sender, (exchangeRate * amount)/100);
        IUSDY(Admin.USDs()).burn(user, amount);
    }
    ...
    function reBaseRewards() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        uint mintReward = ((IERC20(Admin.USDs()).totalSupply() * exchangeRate) * rebaseRate)/100;
