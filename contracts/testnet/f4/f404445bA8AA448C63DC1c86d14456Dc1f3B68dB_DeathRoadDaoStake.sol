    ...
    ReentrancyGuardUpgradeable
{
    ...
    //allow users to approve to migrate stake to a new address. this is used for users to safely migrate stakes to another new address when private key is leaked
    mapping(address => address) public approveMigrationMap;
    ...
    function initialize(
        ...
        //add nft pool
        _add(1000, address(0), 30 days, false);
        //DRACE staking pool
        _add(1000, _drace, 30 days, false);
    }
    ...
    function changeWithdrawLockedTime(uint256 _lockedTime) external onlyOwner {
        withdrawLockedTime = _lockedTime;
    }
    ...
    function add(
        ...
        _add(_allocPoint, _lpToken, _minLockedDuration, _withUpdate);
    }
    ...
    function deposit(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        require(_lockedTime >= pool.minLockedDuration, "!minLockedDuration");
        require(_lockedTime <= 365 days, "!maxLockedDuration");
        ...
        user.stakedTokens.push(
            TokenStake({
                ...
                lockedTill: block.timestamp + _lockedTime,
    ...
    function restake(uint256 _pid, uint256 _depositId) external {
        require(!isNFTPool(_pid), "Pool ID must not be NFT pool");
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 currentLockedDuration = user
            .stakedTokens[_depositId]
            .lockedTill - user.stakedTokens[_depositId].lockedFrom;
        user.stakedTokens[_depositId].lockedFrom = block.timestamp;
        user.stakedTokens[_depositId].lockedTill =
            block.timestamp +
            currentLockedDuration;
    }

    function restakeWithLockedTime(uint256 _pid, uint256 _depositId, uint256 _lockedTime) external {
        require(!isNFTPool(_pid), "Pool ID must not be NFT pool");
        ...
        require(_lockedTime >= currentLockedDuration, "cannot reduce locked time");
    ...
    function depositNFT(uint256[] memory _tokenIds, uint256 _lockedTime)
        public
    {
    ...
    function restakeNFT(uint256[] memory _indexes) external {
        require(nftPoolId != type(uint256).max, "NFT Pool not exist");
        require(_indexes.length > 0, "Empty token list");
        UserInfo storage user = userInfo[nftPoolId][msg.sender];

        for (uint256 i = 0; i < _indexes.length; i++) {
            uint256 _index = _indexes[i];
            if (_index < user.stakedNFTs.length) {
                uint256 currentLockedDuration = user
                    .stakedNFTs[_index]
                    .lockedTill - user.stakedNFTs[_index].lockedFrom;
                user.stakedNFTs[_index].lockedFrom = block.timestamp;
                user.stakedNFTs[_index].lockedTill =
                    block.timestamp +
                    currentLockedDuration;
            }
        }
    ...
    function restakeNFTWithLockedTime(uint256[] memory _indexes, uint256 _lockedTime) external {
        require(nftPoolId != type(uint256).max, "NFT Pool not exist");
        require(_indexes.length > 0, "Empty token list");
        ...
        for (uint256 i = 0; i < _indexes.length; i++) {
            ...
            if (_index < user.stakedNFTs.length) {
                uint256 currentLockedDuration = user
                    .stakedNFTs[_index]
                    .lockedTill - user.stakedNFTs[_index].lockedFrom;
                if (currentLockedDuration <= _lockedTime) {
                    user.stakedNFTs[_index].lockedFrom = block.timestamp;
                    user.stakedNFTs[_index].lockedTill =
                        block.timestamp +
                        _lockedTime;
    ...
    //after approval, only owner can accept for migration
    //disable approval by approving for 0x address
    function approveForMigration(address _newAddress) external {
        require(msg.sender != _newAddress, "!same address");
        approveMigrationMap[msg.sender] = _newAddress;
    }

    function acceptForMigration(address _owner, address _recipient)
        external
        onlyOwner
    {
        require(
            approveMigrationMap[_owner] == _recipient && _recipient != address(0),
            "!approveMigrationMap"
        );
        uint256 _pl = poolInfo.length;
        for (uint256 i = 0; i < _pl; i++) {
            UserInfo storage _ownerInfo = userInfo[i][_owner];
            UserInfo storage _recipientInfo = userInfo[i][_recipient];
            require(
                _recipientInfo.amount == 0 &&
                    _recipientInfo.nftPoint == 0 &&
                    _recipientInfo.rewardDebt == 0,
                "must be fresh user"
            );
            _recipientInfo.amount = _ownerInfo.amount;
            _recipientInfo.nftPoint = _ownerInfo.nftPoint;
            _recipientInfo.rewardDebt = _ownerInfo.rewardDebt;
            _recipientInfo.stakedNFTs = _ownerInfo.stakedNFTs;
            _recipientInfo.stakedTokens = _ownerInfo.stakedTokens;
            delete userInfo[i][_owner];
        }
    }
