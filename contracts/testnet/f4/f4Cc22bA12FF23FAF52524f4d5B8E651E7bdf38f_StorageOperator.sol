...
contract StorageOperator is AccessControl {  
    ...
    constructor(address baseContract_, address fundAddress_, uint256 storageCostRate_, uint256 transactionFee_, address adminUser ) {     
    ...
    /**
     * @notice The external test method is used to update lastPerformTimestamp during testing contract.
     * @dev The external test method is used to update lastPerformTimestamp.
     * Maintainer permission required to change lastPerformTimestamp.
     * The newTimestamp should greater than block timestamp.
     * Not allowed to perform during batch process.
     * Emits a {LastPerformTimestamp} event.
     * @param newTimestamp, uint256 timestamp to be updated  
     */   
    function setLastPerformTimestampTest(uint256 newTimestamp) external _notProcessingDeductions onlyRole(MAINTAINER_ROLE) {
     ...
     * @param newTimestamp, uint256 timestamp to be updated  
     */
    function setLastPerformTimestamp(uint256 newTimestamp) internal  {
        _lastPerformTimestamp = newTimestamp;
        emit LastPerformTimestamp(newTimestamp);
    }
        ...
        if (_isValidBatchRequest()){
            ...
            for (uint256 index = _batchCursor; index < lastIndex; index = _increment(index)) {
                ...
                if (balance > 0 && !iBWGToken.isCostFreeWallet(balanceWallet)){ 
                    ...
                    uint256 cost = ((balance * _storageCostRate)/_RATE_PRECISION) + _transactionFee;
                    // collect all tokens if the token is below the storage fee for the next term.
                    if (balance <  (cost * 2)){
                        cost = balance;                      
                    }
    ...
    */
    function batchSend(address[] calldata wallets, uint256[] calldata costs, uint256 totalCost) external onlyRole(BATCH_EXECUTOR_ROLE)  {
        ...
        require(_isValidBatchRequest() , "Invalid batch-send request"); 
        ...
        if (maxLimit <= _batchCursor) {
            ...
            setLastPerformTimestamp(block.timestamp);            
