...
contract DOGEZILLA is ERC20, Ownable {
    ...
    uint16 constant feeDenominator = 1000;//手续费分母
    uint16 constant lpDenominator = 1000;//底池分母
    uint16 constant maxFeeLimit = 300;//最大手续费

    bool public tradingActive;//开启交易
    ...
    uint16 public sellMarketingFee = 0;
    uint16 public sellStakingFee = 0;

    uint16 public transferBurnFee = 0;
    uint16 public transferLiquidityFee = 10;
    uint16 public transferMarketingFee = 0;
    uint16 public transferStakingFee = 0;
    ...
    uint256 private lpTokens;//交易对合约里面的token

    mapping(address => bool) public automatedMarketMakerPairs;//自动做市商对
    mapping(address => bool) public botWallet;
    address[] public botWallets;
    uint256 public minLpBeforeSwapping;//交换前的最小 Lp
    ...
    constructor() ERC20("DOGEZILLA", "DOGEZILLA") {
        ...
        marketingFeeAddress = payable(0x0000000000000000000000000000000000000000);
        stakingFeeAddress = payable(0x0000000000000000000000000000000000000000);

        minLpBeforeSwapping = 10; //意为1%   this means: 10 / 1000 = 1% of the liquidity pool is the threshold before swapping
        ...
        address routerAddress = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; //BSC Test
        ...
        globalLimit = 1*10**(decimals()+8);
        globalLimitPeriod = 24 hours;
        globalLimitPeriodEnd = block.timestamp + 7*24*60*60;
        limitsActive = true;
    ...
    function _transfer(
        ...
        if (!tradingActive) {//判断是否开启交易
            require(
                isExcludedFromFee[from] || isExcludedFromFee[to],
                "Trading is not active yet."
            );
        }
        require(!botWallet[from] && !botWallet[to], "Bot wallet");//判断是否为机器人
        checkLiquidity();

        //to是pair合约
        if (
            hasLiquidity && !inSwapAndLiquify && automatedMarketMakerPairs[to] // 流动池中有代币和BNB && 没在加锁状态 && to是pair合约(卖代币)
        ) {
            uint256 contractTokenBalance = balanceOf(address(this));
            if (
                contractTokenBalance >=
                (lpTokens * minLpBeforeSwapping) / lpDenominator  //代币合约中的代币 >= pair合约中的代币*1%
        ...
        _marketingFee -
        _stakingFee;
        super._transfer(from, to, _transferAmount);//进行转账操作
        uint256 _feeTotal = _burnFee +
        _liquidityFee +
        _marketingFee +
        _stakingFee;
        if (_feeTotal > 0) {
            super._transfer(from, address(this), _feeTotal);//手续费转账到当前合约
    ...
    function takeFee(uint256 contractBalance) private lockTheSwap {
        uint256 totalTokensTaken =
        _liquidityTokensToSwap +
        _marketingFeeTokensToSwap +
        _burnFeeTokens +
        _stakingFeeTokens;
        if (totalTokensTaken == 0 || contractBalance < totalTokensTaken) { //手续费为零 || 合约中token余额 < 当前总手续费
            return;
        }
    ...
    uint256 public globalLimit; // limit over timeframe for all 限制所有人的时间范围
    uint256 public globalLimitPeriod; // timeframe for all  所有人的时间表
    uint256 public globalLimitPeriodEnd;//限制交易结束时间
    bool public limitSwitch = false;
    ...
    //可用于检查钱包在他们的时间范围内卖出了多少代币
    function getSoldLastPeriod(address _address)//上期卖掉的
    public
    view
    ...
    function checkLiquidity() internal {
        ...
        // this is not a problem, since contract sell will get that unsynced balance as if we sold it, so we just get more ETH.
        //这不是问题，因为合约卖出会得到未同步的余额，就像我们卖出它一样，所以我们只会得到更多的 ETH。
    ...
    // Handle private sale wallets 处理私人销售钱包
    function _handleLimited(
        ...
        if(limitSwitch == true && block.timestamp > globalLimitPeriodEnd){
            return;
        }


        uint256 ethValue = getETHValue(taxedAmount);//获取对应BNB价格
        _limits[from].sellTimestamps.push(block.timestamp);//记录from用户交易信息
        _limits[from].sellAmounts.push(ethValue);//记录from用户交易信息
        uint256 soldAmountLastPeriod = getSoldLastPeriod(from);//计算一定时间内卖出的代币
