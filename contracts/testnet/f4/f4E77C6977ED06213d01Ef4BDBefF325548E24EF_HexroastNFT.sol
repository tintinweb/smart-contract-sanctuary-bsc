...
// File: contracts/HexroastNFT.sol
...
contract HexroastNFT is ERC1155, Ownable {
    ...
    function openLootBox(uint256 id) public returns (bool status) {
        ...
        if (id == 101) { // one random item
            ...
            uint256 rarity  = _randomRarity(item);
            ...
            for (uint256 index = 0; index < 3; index++) {
                ...
                rarity  = _randomRarity(item.add(rarity));
    ...
    function _randomItem() internal view returns (uint256 item) {
        return _random(3);
    }

    function _randomRarity(uint256 salt) internal view returns (uint256 rarity) {
        uint256 common      = 5500;
        ...
        uint256 chance      = _random(totalChance, salt);
        if (chance <= common) {
            return 1;
    ...
    function _random(uint256 number) internal view returns (uint256) {
        return _random(number, 0);
    }

    function _random(uint256 number, uint256 salt) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.timestamp.add(salt), block.difficulty, _msgSender()))).mod(number).add(1);
    }
}