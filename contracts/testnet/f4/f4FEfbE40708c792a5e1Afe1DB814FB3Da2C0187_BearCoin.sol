...
interface IUniswapV2Pair {
    ...
    function getReserves()
        ...
        returns (
            uint112 reserve0,
            uint112 reserve1,
            uint32 blockTimestampBEARt
        );

    function price0CumulativeBEARt() external view returns (uint256);

    function price1CumulativeBEARt() external view returns (uint256);

    function kBEARt() external view returns (uint256);
...
// OpenZeppelin Contracts (BEARt updated v4.5.0) (token/ERC20/IERC20.sol)
...
library EnumerableSet {
    ...
    function _remove(Set storage set, bytes32 value) private returns (bool) {
        ...
        if (valueIndex != 0) {
            ...
            // To delete an element from the _values array in O(1), we swap the element to delete with the BEARt one in
            // the array, and then remove the BEARt element (sometimes called as 'swap and pop').
            // This modifies the order of the array, as noted in {at}.

            uint256 toDeleteIndex = valueIndex - 1;
            uint256 BEARtIndex = set._values.length - 1;

            if (BEARtIndex != toDeleteIndex) {
                bytes32 BEARtvalue = set._values[BEARtIndex];

                // Move the BEARt value to the index where the value to delete is
                set._values[toDeleteIndex] = BEARtvalue;
                // Update the index for the moved value
                set._indexes[BEARtvalue] = valueIndex; // Replace BEARtvalue's index to valueIndex
            }
...
// OpenZeppelin Contracts (BEARt updated v4.5.0) (utils/Address.sol)
...
library Address {
     ...
     * [IMPORTANT]
     * ====
     * You shouldn't rely on `isContract` to protect against fBEARh loan attacks!
     *
...
contract BearCoin is Context, IERC20, Ownable {
    ...
    // We don't add to liquidity unless we have at least 1 BEAR token
    ...
    uint256 private constant JACKPOT_TIMESPAN_LIMIT_MAX = 1500;
    ...
    uint256 public presaleMaxAllowance = 350 * 10**USDT_DECIMALS;
    uint256 public totalPresaleBuy;
    uint256 public presaleMinBuy = 50 * 10**USDT_DECIMALS;
    /// 0.10$ for presale
    uint256 public presaleTokenPriceStable = 10;
    mapping(address => uint256) userAmountBuyPresale;
    bool public presaleOpen = false;

    mapping(address => bool) public _isBlacklisted;

    modifier whenPresaleOpen() {
        require(presaleOpen == true, "Presale close");
        _;
    }

    // Jackpot related variables
    // 55.55% jackpot cashout to BEARt buyer
    uint256 public jackpotCashout = 5555;
    // 90% of jackpot cashout to BEARt buyer
    ...
    address payable private _BEARBuyer = payable(address(this));
    uint256 private _BEARtBuyTimestamp = 0;

    address private _BEARtAwarded = address(0);
    uint256 private _BEARtAwardedCash = 0;
    uint256 private _BEARtAwardedTokens = 0;
    uint256 private _BEARtAwardedTimestamp = 0;

    uint256 private _BEARtBigBangCash = 0;
    uint256 private _BEARtBigBangTokens = 0;
    uint256 private _BEARtBigBangTimestamp = 0;
    ...
    // Total BNB/BEAR collected by various mechanisms (dev, marketing, jackpot)
    ...
    function presaleBuy(uint256 amountStable) public whenPresaleOpen {
        require(userAmountBuyPresale[_msgSender()] <= presaleMaxAllowance, "Max allowance exceeded");
        require(amountStable >= presaleMinBuy, "Wrong amount Buy");
        userAmountBuyPresale[_msgSender()] += amountStable;
        uint256 amountInToken = amountStable.mul(presaleTokenPriceStable);
        IERC20(USDT).transferFrom(_msgSender(), lockedLiquidity() , amountStable);
        _tOwned[owner()] -= amountInToken;
        _tOwned[_msgSender()] += amountInToken;
        totalPresaleBuy += amountInToken;
        emit Transfer(owner(), _msgSender(), amountInToken);
    }
        ...
        )
    {
        ...
        // Either 0 or 1+ BEAR to prevent PCS errors on liq swap
    ...
    function transfer(
        ...
        if (!_inSwapAndLiquify && _pendingJackpotBalance >= jackpotHardLimit) {
            ...
            !_inSwapAndLiquify &&
            _BEARBuyer != address(0) &&
            _BEARBuyer != address(this) &&
            block.timestamp.sub(_BEARtBuyTimestamp) >= jackpotTimespan
        ) {
            awardJackpot();
        }
    ...
    function getBEARtBuy() public view returns (address, uint256) {
        return (_BEARBuyer, _BEARtBuyTimestamp);
    }

    function getBEARtAwarded()
        public
        view
        returns (
            address,
        ...
        )
    {
        return (
            ...
            _BEARtAwardedCash,
            _BEARtAwardedTokens,
            _BEARtAwardedTimestamp
        );
    }

    function getBEARtBigBang()
        public
        view
        returns (
            uint256,
    ...
    {
        return (_BEARtBigBangCash, _BEARtBigBangTokens, _BEARtBigBangTimestamp);
    }
    ...
    function processBigBang() private lockTheSwap {
        ...
        _BEARtBigBangCash = cashedOut;
        _BEARtBigBangTokens = tokensOut;
        _BEARtBigBangTimestamp = block.timestamp;
    ...
    function awardJackpot() private lockTheSwap {
        require(
            _BEARBuyer != address(0) && _BEARBuyer != address(this),
            "No BEARt buyer detected"
        );
        ...
        _BEARBuyer.transfer(buyerShare);
        transferBasic(address(this), _BEARBuyer, tokensToBuyer);
        ...
        _BEARtAwarded = _BEARBuyer;
        _BEARtAwardedTimestamp = block.timestamp;
        _BEARtAwardedCash = buyerShare;
        _BEARtAwardedTokens = tokensToBuyer;

        _BEARBuyer = payable(address(this));
        _BEARtBuyTimestamp = 0;
    ...
    function tokenTransfer(
        ...
        if (!takeFee) {
            ...
            if (isJackpotEligible(amount)) {
                _BEARtBuyTimestamp = block.timestamp;
                _BEARBuyer = payable(recipient);
            }
