...
contract ERC721OwnerMintableFacet {
    ...
    function mint(uint256 tokenId, address account) external {
        ...
        require(s.owners[tokenId] == address(0), "");
        s.owners[tokenId] = account;
        s.balances[account]++;
...
library LibDiamond {
    ...
    function diamondCutInitializable(
        IDiamondCut.FacetCutInitializable[] memory _diamondCutInitializable,
        IDiamondCut.FacetCut[] memory _diamondCut,
        address _init,
        bytes memory _calldata
    ) internal {
    ...
    function handleDiamondCutInitilizable(
        ...
        for (
            ...
            IDiamondCut.FacetCutAction action = _diamondCutInitializable[
                facetIndex
            ].action;
            ...
            addFunction(
                _diamondCutInitializable[facetIndex].facetAddress,
                _diamondCutInitializable[facetIndex].initializeSelector
            );

            (bool success, bytes memory result) = ds
                .diamondAddress
                .delegatecall(
                    abi.encodeWithSelector(
                        _diamondCutInitializable[facetIndex].initializeSelector,
                        _diamondCutInitializable[facetIndex].initializeBytes
                    )
                );
