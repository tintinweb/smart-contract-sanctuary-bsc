...
import "./interface/IHashimotoLookupVerifier.sol";
import "./interface/IETHTrustedSetup.sol";
...
contract Ethash is SHA3_512 {
    ...
    IHashimotoLookupVerifier hashimotoLookupVerifier;
    IETHTrustedSetup ethTrustedSetup;

    constructor(address hashimotoLookupVerifierAddress, address ethTrustedSetupAddress) {
        hashimotoLookupVerifier = IHashimotoLookupVerifier(hashimotoLookupVerifierAddress);
        ethTrustedSetup = IETHTrustedSetup(ethTrustedSetupAddress);
    }
    ...
    function hashimoto(
        ...
        uint256[2] memory c,
        uint256[14] memory input) private view returns (uint) {

        uint[16] memory s;

        ParsedInput memory inputData = parseInput(input);

        if (inputData.merkleRoot != ethTrustedSetup.epochDataMTRoot(epoch)) {
            return 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFE;
        }

        hashimotoLookupVerifier.verifyProof(a, b, c, input);

        s = computeS(uint(header), nonceLe);

        uint result = computeSha3(s, inputData.cmix);

        return result;

    }

    function parseInput(uint256[14] memory inputs) public view returns (ParsedInput memory) {
        ParsedInput memory result;
        for (uint i = 0; i < 4; i++) {
            result.seed[i] = inputs[i];
        }
        result.merkleRoot = bytes32(inputs[4]);
        for (uint i = 0; i < 8; i++) {
            result.cmix[i] = inputs[i + 5];
        }
        result.rows = inputs[13];
        return result;
    }
    ...
    function verifyPoW(uint blockNumber,
        ...
        uint256[2][2] memory b,
        uint256[2] memory c,
        uint256[14] memory input) external view returns (uint, uint) {

        // verify ethash
        uint epoch = blockNumber / EPOCH_LENGTH;
        uint ethash = hashimoto(rlpHeaderHashWithoutNonce, nonce, epoch, a, b, c, input);
...
interface IHashimotoLookupVerifier {
...
interface IETHTrustedSetup {
    function epochDataMTRoot(uint epoch) external view returns(bytes32);
}