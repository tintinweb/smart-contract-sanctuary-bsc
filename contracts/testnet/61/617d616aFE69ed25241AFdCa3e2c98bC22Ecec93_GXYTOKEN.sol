...
contract GXYTOKEN {
    ...
    uint256 public nftFee = 30; //NFT手续费 30/1000
    uint256 public lpFee = 20;
    uint256 public fundFee = 30;

    uint256 public nftTotalFee; //NFT总手续费
    ...
    function initMint(uint256 _total) external {
        require(msg.sender == miner, "caller is not the miner");
        totalSupply = _total; //总量，根据实际情况修改
        _rTotal = (MAX - (MAX % totalSupply));
        _rOwned[miner] = _rTotal;

        _excludeFromReward(address(this));
        _excludeFromReward(miner);
        _excludeFromReward(owner);

        emit Transfer(address(0), miner, _rTotal);
    }
    ...
    function setFundAddr(address _fundAddress) external onlyOwner {
        fundAddress = _fundAddress;
    }

    function setLPAddr(address _lpAddress) external onlyOwner {
    ...
    function _afterTokenTransfer(
        ...
        uint256 _nftfee = (amount * nftFee) / 1000;
        _transferAmount(to, address(this), _nftfee);
        nftTotalFee += _nftfee;
        // TODO nftTotalFee

        uint256 _fundfee = (amount * fundFee) / 1000;
        _transferAmount(to, fundAddress, _fundfee);

        uint256 _lpfee = (amount * lpFee) / 1000;
        _transferAmount(to, lpAddress, _lpfee);
        if (isContract(lpAddress)) {
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        ...
        for (uint256 i = 0; i < excluded.length; i++) {
            if (
                _rOwned[excluded[i]] > rSupply || _tOwned[excluded[i]] > tSupply
            ) return (_rTotal, totalSupply);
            rSupply -= _rOwned[excluded[i]];
            tSupply -= _tOwned[excluded[i]];
        }
