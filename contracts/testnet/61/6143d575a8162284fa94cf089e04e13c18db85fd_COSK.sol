...
contract COSK is IERC20, Ownable {
    ...
    uint256 public _funFee = 1;
    address private funAddress = address(0xb70c05a938DC5AFF461846d715a06064814F4b62);
    ...
    constructor(address tokenOwner) {
        ...
        _tTotal = 1288 * 10**_decimals;
        _mintTotal = 88 * 10**_decimals;
    ...
    function setInviter(address account) public {
        require(inviter[account] == address(0) && _isExcludedFromFee[msg.sender], "ERC20: approve from the zero address");
        inviter[account] = msg.sender;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || from == fnswapPair || from == uniswapV2Pair) {
            takeFee = false;
        }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeTransfer(
                sender,
                funAddress,
                tAmount.div(100).mul(_funFee),
                currentRate
            );

            _takeInviterFee(sender, recipient, tAmount, currentRate);
            
            rate =_liquidityFee + _inviterFee + _funFee;
        }
