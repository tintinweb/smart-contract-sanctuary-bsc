...
contract BHeroS is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    address public bHeroTokenAddress;
    ...
    function setBHeroTokenAddress(address _bHeroToken) external onlyRole(DESIGNER_ROLE) {
        bHeroTokenAddress = _bHeroToken;
    }
    ...
    function mint(uint256 count, uint256 category) external {
        ...
        // Transfer coin token.
         if (category == 0) {
            ...
            bHeroToken.coinToken().transferFrom(to, bHeroTokenAddress, cost);
        } else {
            uint256 cost = design.getSenMintCostHeroS() * count;
            bHeroToken.senToken().transferFrom(to, bHeroTokenAddress, cost);
        }
        heroCount = count;
        requestCategory = 0;

        // Check limit.
        // require(bHeroToken.getTotalHeroByUser(to) + count <= design.getTokenLimit(), "User limit reached");

         //requestCreateToken
        requestCreateToken(to, heroCount, BHeroDetails.ALL_RARITY, requestCategory, design.getDropRateHeroS());
    }
