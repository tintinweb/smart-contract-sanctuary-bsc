...
contract WLBToken is IERC20, Ownable {
    ...
    address public nodeFeePool = 0x3A83386aE15145d3A9B76166FA183C6525C0aF1f;
    address public lpFeePool = 0x7B6236bA95e66b3d2DC1dA31C8794CB2baD06bAe;
    address public markeFeetPool = 0x8e18343f9206d97944a916357f6f1A5Ce0B56CC5;
    uint256 public sumLp = 0;
    uint256 public sumNode = 0;
    uint256 private _tFeeTotal;
    string private _name = "Test WLB";
    ...
    constructor() {
        ...
        //正式网  BUSD 0x55d398326f99059fF775485246999027B3197955
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd);
    ...
    function _transfer(
        ...
        if ((ammPairs[to] || ammPairs[from]) && (!_isExcludedFromFee[from] || !_isExcludedFromFee[to])) {
            takeFee = true;
        }
        ...
        toAddress = to;
        if (
            _tOwned[address(this)] >= 1 * 10 ** 4 * 10 ** 9 &&
            from != address(this) &&
    ...
    function _takeLPFee(address sender, uint256 tAmount) private {
        if (_lpFee == 0) return;
        _tOwned[lpFeePool] = _tOwned[lpFeePool].add(tAmount);
        sumLp = sumLp + tAmount;
        emit Transfer(sender, lpFeePool, tAmount);
    }

    function _takeNodeFee(address sender, uint256 tAmount) private {
        if (_nodeFee == 0) return;
        _tOwned[nodeFeePool] = _tOwned[nodeFeePool].add(tAmount);
        sumNode = sumNode + tAmount;
        emit Transfer(sender, nodeFeePool, tAmount);
    }

    function _takeMarketFee(address sender, uint256 tAmount) private {
        if (_nodeFee == 0) return;
        _tOwned[markeFeetPool] = _tOwned[markeFeetPool].add(tAmount);
        emit Transfer(sender, markeFeetPool, tAmount);
    }
    ...
    function _transferStandard(
        ...
        _burnFee -  _lpFee - _nodeFee - _inviterFee - _marketFee;
