...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function _takeFees(address sender, FeeValues memory values, uint256 tierIndex) private {
        ...
        _takeNativeFee(sender,values.tLiquidity,address(this));
    }
    ...
    function _takeNativeFee(address sender, uint256 tAmount, address recipient) private {
         if(recipient == address(0)) return;
         if(tAmount == 0) return;

         uint256 currentRate = _getRate();
         uint256 rAmount = tAmount.mul(currentRate);
         swapTokensForBnb(rAmount);
         if(_isExcluded[recipient])
