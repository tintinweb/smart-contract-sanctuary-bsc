...
contract StackingUSD is Ownable, ReentrancyGuard {
    ...
    // uint256 priceOfBNB = 33291379100;
    ...
    uint maxClaimTime = 10 minutes; // Minimum Time To Withdraw Claim Reward
    uint minClaimTime = 5 minutes; // Maximum Time To claim
    uint pricepertoken = 1 ether;  // Price Per Token
    ...
    function calculateclaimamount()
     ...
     {    
        ...
        }else if(_lastclaimed == 0  &&  block.timestamp > _startime + minClaimTime){

            _staking = _amount.mul(APR.div(2)).div(baseDivider);
            _minmumtoken = pricepertoken.mul(_staking);
            _check = false;

        }else if(_lastclaimed != 0 && block.timestamp >= (_startime + maxClaimTime)){
    ...
    function stacking(uint256 _amount) public {
        ...
        require(
            stakeLog[msg.sender].amount == 0,
            "Users Already Staking"
        );

        (bool success) = USDT.transferFrom(msg.sender, address(this), _amount);
        require(success, "Not Enough USDT");

        stakeLog[msg.sender].amount = _amount;
        stakeLog[msg.sender].Startime = block.timestamp;
        stakeLog[msg.sender].lastclaimed = 0;
    }

    // Claim The Staking Part
    function claim() external nonReentrant {
        require(stakeLog[msg.sender].Startime != 0,"You Have Not Stake Anything");
        require(
        ...
        (uint256 _claim,uint256 _minClaimToken,bool _erase) = calculateclaimamount();
        
         require(
            USDT.balanceOf(address(this)) >= _claim,
            "Insufficent Balance"
        );
    ...
    // View 
    function viewreward() view public returns(uint256){
        require(stakeLog[msg.sender].Startime != 0,"You Have Not Stake Anything");
        require(
            block.timestamp >= stakeLog[msg.sender].Startime + minClaimTime,
    ...
    function viewstakelog(address _user) view public returns(StakeInfo[] memory){
     ...
     function viewuser(address _user) view public returns(User[] memory){
