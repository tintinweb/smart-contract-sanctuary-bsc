...
contract MetaSportsTokenCrossChain is
Ownable,
Pausable,
ReentrancyGuard,
AccessControl
{
    ...
    event Extract(address indexed receiver, uint256 indexed amount);
    ...
    constructor(address _collectionAddress) {
        ...
        MS_BSC_MST = IERC20(_collectionAddress);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function transferIn(uint256 _amount)
    external
    whenNotPaused
    nonReentrant
    {
    ...
    function extract(address _receiver)
    external
    onlyOwner
    nonReentrant
    {
        ...
        uint256 amount = MS_BSC_MST.balanceOf(address(this));
        require(amount > uint256(0), "Receive address can not be zero");

        MS_BSC_MST.safeTransfer(
            _receiver,
            amount
        );
        emit Extract(_receiver, amount);
    }
}