...
contract MarketPlaceV2 is MarketPlaceV2Auth, EIP712Upgradeable, PausableUpgradeable {
  ...
  uint public orderTrustFee; // TODO
  ...
  event OrderCreated(uint indexed id, address maker, uint[] tokenIds, uint takerShare, uint rentingTime, uint timestamp);
  ...
  function makeOrder(uint[] calldata _tokenIds, uint _takerShare, uint _rentingTime) external whenNotPaused {
    ...
    emit OrderCreated(orderCounter, msg.sender, _tokenIds, _takerShare, _rentingTime, block.timestamp);
  }
