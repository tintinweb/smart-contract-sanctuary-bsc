...
contract LotteryRNG is VRFConsumerBaseV2 {
  ...
  function requestRandomWords() external onlyLotto() {
  ...
  function getFinalNumber() external view onlyLotto() returns (uint32 result) {
    require(s_randomWords[0] != lastWord, "Too Soon, please wait for new number generation");
    result = uint32(1000000 + (s_randomWords[0] % 1000000));
    return result;
  }

  function setLotto(address _address) external onlyOwner {
    Lotto = _address;
  }
  ...
  modifier onlyLotto() {
    require(msg.sender == Lotto , "Only Lotto");
    _;
  }
