...
contract IslandStake is OwnableUpgradeable {
    ...
    function calculateReward(address addr, uint id, uint index) public view returns (uint){
        ...
        uint value = (info.stakeAmount * info.stakeValue * (coutingDebt(id) - info.debt)) / acc + info.toClaimIsland;
        if (isDouble()) {
            value *= 2;
        }
        return value;
    ...
    function calculateBLKReward(address addr, uint id, uint index) public view returns (uint) {
        StakeInfo memory info = userStake[addr][id][index];
        uint value = (info.stakeAmount * info.stakeValue * (coutingBlkDebt(id) - info.blkDebt)) / acc + info.toClaimBlk;
        if (isDouble()) {
            value *= 2;
        }
        return value;
...
//        data[5] = blkFee;
    ...
    function getPrice() public view returns(uint){
        ...
        uint price = balance1 * 4e17 / balance2;
        return price;
    }
}
