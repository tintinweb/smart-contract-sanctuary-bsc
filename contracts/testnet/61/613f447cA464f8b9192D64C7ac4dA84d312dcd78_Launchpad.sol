...
contract PresalePool is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        require (_args.minInvest <= _args.maxInvest, "!min investable");
        ...
        require (salePrice > 0, "!sale price");
    ...
    function cancelSale() external onlyTokenOwner {
        require (!enabled || (enabled && block.timestamp < startTime), "sale already started");
    ...
    function finalize() external {
        ...
        require (ended, "!end");
        require (liquidityAlloc > 0, "!available to add liquidity");
        require (liquidityAlloc >= minBnbAllocToLiquidity, "!too small bnb amount for liquidity");
        uint _bnbAmount = liquidityAlloc * raised / feeDenominator;
        uint _tokenAmount = _bnbAmount * listPrice / 1e18;
        require (_tokenAmount >= minTokenAllocToLiquidity * token.totalSupply() / feeDenominator, "!too small token amount for liquidity");
        ...
        if (address(pair) == address(0)) {
            INovationFactory(router.factory()).createPair(
                address(token),
                router.WETH()
            );
        }

        require (pair.totalSupply() == 0, "liquidity already exsits");

        uint _before = token.balanceOf(address(this));
        token.safeTransferFrom(msg.sender, address(this), _tokenAmount);
        uint _after = token.balanceOf(address(this));
        require (_after - _before >= _tokenAmount, "!token amount");
        ...
        if (tokenFee > 0) {
            uint feeAmount = (salePrice * hardcap / 1e18) * tokenFee / feeDenominator;
    ...
    function withdraw() external {
        require (msg.sender == owner() || msg.sender == tokenOwner, "!owner");
        require (ended, "!end");
        require (liquidityAlloc == 0, "!can't withdraw");
    ...
    function transferUnsold() external {
        ...
        uint unsold = token.balanceOf(address(this));
    ...
    function getRefund() external nonReentrant {
        ...
        require (invests[msg.sender] > 0, "!investor");
        require (refunded[msg.sender] == false, "already claimed");
    ...
    function unclaimed() external view returns (uint) {
        uint amount = 0;
        for (uint i = 0; i < investors.length; i++) {
            if (claimed[investors[i]] == true) continue;
            amount += salePrice * invests[investors[i]] / 1e18;
        }
...
contract Launchpad is Ownable {
    ...
    function deploy(
        ...
        require (bytes(_urls).length <= 700, "invalid social urls size");
        require (_router < routers.length, "invalid router index");
    ...
    function addPool(address _pool, string memory _urls) external onlyOwner {
        require (poolMap[_pool].pool == address(0), "already existing");
        require (bytes(_urls).length <= 700, "invalid social urls size");
