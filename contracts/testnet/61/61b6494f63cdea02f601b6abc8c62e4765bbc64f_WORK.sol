...
contract WORK is ERC721Holder,Ownable{
    ...
    function onboarding(uint256 _workId, uint256 _nftId, address _leader) public {
        ...
        processingRelated(msg.sender, _leader);
    ...
    function processingRelated(address _user, address _leader)internal{
        require(effective[_leader],"Invalid address");
        if(userRelationMap[_user].leader == address(0) && _leader != address(0) && _leader != _user){
            userRelationMap[_user].leader = _leader;
            userRelationMap[_leader].inviteAmount = userRelationMap[_leader].inviteAmount.add(1);
            emit ProcessingRelated(_user,_leader,userRelationMap[_leader].inviteAmount);
        }
    }
