...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private payoutThreshold = 100;
    ...
    function setAllFeePercent(uint8 taxFee, uint8 burnFee, uint8 devFee) external onlyOwner() {
        ...
        require(taxFee + burnFee + devFee <= maxTotalFee);
        _taxFee = taxFee;
        _burnFee = burnFee;
        _devFee = devFee;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tDev = calculateOtherFees(tAmount);
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
     ...
     function calculateOtherFees(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_burnFee + _devFee).div(
            10**2
        );
    }

     function _takeFees(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
    ...
    function _transfer(
        ...
        if(contractTokenBalance > payoutThreshold * 10 ** uint256(_decimals)){
