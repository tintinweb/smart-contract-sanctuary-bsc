...
contract BNBHeroToken is Context, IBEP20, Ownable {
    ...
    event UpdateFees(uint256 bnbFee, uint256 liquidityFee, uint256 transferFee);
    ...
    function setFees(uint256 bnbFee, uint256 liquidityFee, uint256 transferFee) external onlyOwner() {
        require(_BNBFee != bnbFee || _liquidityFee != liquidityFee || _transferFee != transferFee);
        ...
        if(_transferFee != transferFee){
            _transferFee = transferFee;
        }
        
        emit UpdateFees(bnbFee, liquidityFee, transferFee);
    }
    ...
    function _getLiquidFeeValues(uint256 tAmount) private view returns (uint256) {
        uint256 fee = tAmount.mul(_BNBFee + _liquidityFee).div(10**2);
        uint256 tTransferAmount = tAmount.sub(fee);
        return tTransferAmount;
    }

    function _getTransferFeeValues(uint256 tAmount) private view returns (uint256) {
        uint256 fee = tAmount.mul(_transferFee).div(10**2);
        uint256 tTransferAmount = tAmount.sub(fee);
        return tTransferAmount;
    }
    ...
    function _transfer(
        ...
        bool takeTransferFee = false;

        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            _tokenTransfer(from,to,amount,takeLiquidFee,takeTransferFee);
            return;
        }

        // if (lastTxTimes[from] > 0 && lastTxTimes[from] + swapCoolDownTime > block.timestamp){
        if (to != address(pancakeswapV2Router) && to != pancakeswapV2Pair && lastTxTimes[from] + swapCoolDownTime > block.timestamp){
            _blacklist[from] = true;
        }

        if (from != address(pancakeswapV2Router) && from != pancakeswapV2Pair && lastTxTimes[to] + swapCoolDownTime > block.timestamp){
            _blacklist[to] = true;
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeLiquidFee, bool takeTransferFee) private {

        require(_blacklist[sender] || _blacklist[recipient], "Transfer from blacklist");

        // if(!takeLiquidFee || !takeTransferFee){
        //     _balances[sender] = _balances[sender].sub(amount);
        //     _balances[recipient] = _balances[recipient].add(amount);
        // }

        if(takeLiquidFee){
            uint256 tTransferAmount = _getLiquidFeeValues(amount);
            _balances[sender] = _balances[sender].sub(amount);
        ...
        if(takeTransferFee){
            uint256 tTransferAmount = _getTransferFeeValues(amount);
            _balances[sender] = _balances[sender].sub(amount);
            _balances[recipient] = _balances[recipient].add(tTransferAmount);   
            _balances[address(this)] = _balances[address(this)].add(amount.sub(tTransferAmount));
            emit Transfer(sender, recipient, tTransferAmount);
