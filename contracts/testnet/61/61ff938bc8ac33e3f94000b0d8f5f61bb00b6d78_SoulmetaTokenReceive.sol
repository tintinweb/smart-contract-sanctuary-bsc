...
contract SoulmetaTokenReceive is DivestorUpgradeable{
    ...
    function soulReceive() public{
        ...
        require(userReceive[_msgSender()].receiveTime != block.timestamp/900, "user has received");
        require(userReceive[_msgSender()].amount > 0, "balance is 0");
        uint amount = getRate() * userReceive[_msgSender()].amount / (100 * 96);

        userReceive[_msgSender()].receiveTime = block.timestamp/900;
