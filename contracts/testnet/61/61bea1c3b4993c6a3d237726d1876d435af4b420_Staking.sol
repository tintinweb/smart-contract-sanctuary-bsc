...
contract Staking is Ownable, ReentrancyGuard {
    ...
    uint16 public lockFeePercent = 300; // 30% withdraw fee, if withdrawn before locking period

    mapping(address => User) public users;
    mapping(address => address) public referrer;

    uint16 public referralPercent = 20; // referral bonus 2%
    uint16 public directReferralPercent = 100; //Direct referral 10%
    ...
    constructor(
        ...
        token = IERC20(_token);
        reserveWallet = address(0xc74E0A7C70FA4511f7d6131D2e26eC987a5e4878);
     ...
     *
     */
    function stake(uint256 _amount, address _referrer) external nonReentrant returns (bool) {
        require(_amount >= pool.minContrib, "Invalid amount!");
        require(_referrer != msg.sender, "No self referring");
        require(
            _referrer == owner() || users[_referrer].totalInvested > 0,
            "Invalid referral"
        );
        require(
        ...
        if (referrer[msg.sender] == address(0)) {
            ...
            uint256 directReferral = _amount.mul(directReferralPercent).div(
                1000
            );
            safeTransfer(_referrer, directReferral);
            users[_referrer].directEarning += directReferral;
        }
    ...
    function _payReferral(address _user, uint256 _amount) internal {
        ...
        if (firstReferrer != address(0)) {
            safeTransfer(firstReferrer, _amount.mul(referralPercent).div(1000));
            users[firstReferrer].firstLevelEarning += _amount
                .mul(referralPercent)
                .div(1000);
        }
    }
     ...
     *
     */
    function updateFeeValues(uint16 _lockFeePercent, address _reserveWallet)
        public
        onlyOwner
    {
    ...
    function getReferralEarnings(address addr)
        external
        view
        returns (uint256 first, uint256 direct)
    {
        return (users[addr].firstLevelEarning, users[addr].directEarning);
    }
     ...
     *
     */
    function updateReferral(uint16 _referralPercent, uint16 _directReferral)
        external
        onlyOwner
    {
        referralPercent = _referralPercent;
        directReferralPercent = _directReferral;
    }
}