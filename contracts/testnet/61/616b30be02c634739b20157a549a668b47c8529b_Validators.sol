...
contract Validators is Params {
    ...
    function addvalidator(address val,uint256 _coin)  external 
    {
        if (!isActiveValidator(val)) {
                currentValidatorSet.push(val);
            }
            if (!isTopValidator(val)) {
                highestValidatorsSet.push(val);
            }
            if (validatorInfo[val].feeAddr == address(0)) {
                validatorInfo[val].feeAddr = payable(val);
            }
            // Important: NotExist validator can't get profits
            if (validatorInfo[val].status == Status.NotExist) {
                validatorInfo[val].status = Status.Staked;
            }
            validatorInfo[val].coins = _coin;
