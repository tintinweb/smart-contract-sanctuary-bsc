...
winners_view: public(DynArray[WinnerInfo, 1024])
...
def draw():
    ...
    assert block.timestamp > (self.last_win_time + self.lottery_frequency)
    ...
    winner_info: WinnerInfo = WinnerInfo({_address: self.winning_address, _amount: win_amount, _claimed: False})
...
def claim():
    ...
    self.winners_map[msg.sender] = WinnerInfo({_address: msg.sender, _amount: winner_info._amount, _claimed: True})
