...
contract ZombieFeeding is ZombieFactory {
    ...
    function setRarityZombie(uint _zombieId, string memory rarity)
        public
    {
        Zombie storage zombie = zombies[_zombieId];
    ...
    function setStatZombie(uint _zombieId, uint32 healthPoint, uint32 attack, uint32 defense, uint32 criticalRate, uint32 criticalDamage, uint32 speed, uint32 combatPower)
        public
    {
        Zombie storage zombie = zombies[_zombieId];
...
contract ZombieFactory is ZombieBase, ERC721 {
    ...
    constructor(address _token) ZombieBase(_token) ERC721("CryptoZombieNFT", "CZB") {}
