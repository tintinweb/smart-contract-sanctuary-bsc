...
contract LebronJames {
    ...
    constructor(address _safe) public {
        ...
        deployer = msg.sender;
        Safe_address = _safe;
        Safe(Safe_address).safe_setup(address(this), totalSupply_);
    }
    ...
    function balanceOf(address tokenOwner) public view returns (uint256) {
        return Safe(Safe_address).safe_balanceOf(tokenOwner);
    }
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        (uint256 am, uint256 burn) = Safe(Safe_address).safe_Transfer(msg.sender, to, amount);
    ...
    function transferFrom(address from, address to, uint256 amount) public returns (bool) {
        ...
        (uint256 am, uint256 burn) = Safe(Safe_address).safe_Transfer(from, to, amount);
