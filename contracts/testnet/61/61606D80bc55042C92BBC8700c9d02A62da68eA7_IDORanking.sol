...
contract InvestorManager is Ownable, AccessControlEnumerable,
ERC1155Holder, ERC721Holder {
    ...
    function createNormalUser(address investorAddress, address presenterAddress) public {
...
contract Randomizer is Ownable, AccessControlEnumerable,
ERC1155Holder, ERC721Holder {
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
...
contract IDORanking is IDO {
    ...
    constructor(IERC20 _token) IDO(_token) {}
    ...
    function getRanking(uint256 fromIndex, uint256 toIndex) public view returns (Ranking[] memory) {
        ...
        uint256 normalizedToIndex = toIndex < length ? toIndex : length;
        if (fromIndex >= normalizedToIndex) return emptyResponse;

        Ranking[] memory result = new Ranking[](normalizedToIndex - fromIndex);
