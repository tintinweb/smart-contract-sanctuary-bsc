    ...
    function updateAccount(address m_from, address m_to, uint256 m_amount, uint256 m_fromBalance) internal {
        ...
        if(_checkInWL(m_to)){
            ...
            if(_checkInWL(m_from)){
                ...
                uint256 oldBalance = (m_fromBalance + m_amount).div(100000).mul(193);
...
contract BankToken is ERC20, AccessControl, Accounting {
    ...
    constructor() ERC20("Competition", "Competition") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(WHITE_ROLE, msg.sender);
        _mint(msg.sender, 950000000 * 10 ** decimals());
        _mint(address(0x4e213301B90eA31839827a73E5AAD48a182c4750), 50000000 * 10 ** decimals());
    ...
    {
        if(_checkInWL(to)) {
            if(!super.checkAccount(from, balanceOf(from).div(100000).mul(193), amount)) revert("Wrong amount (more than 0.193% per 24h)");
        }
