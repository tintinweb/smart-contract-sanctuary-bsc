...
contract ItemNFT is BaseNFT {
  ...
  function mintItemsForTest(address[] calldata _owners, uint8 _gene, uint16 _class, uint _price, uint _quantity) onlyMintAdmin external {
    for(uint i = 0; i < _owners.length; i++) {
      for(uint j = 0; j < _quantity; j++) {
        mintItem(_owners[i], _gene, _class, _price, j);
      }
    }
  }
