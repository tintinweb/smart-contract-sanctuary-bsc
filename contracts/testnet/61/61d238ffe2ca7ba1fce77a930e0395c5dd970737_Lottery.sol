...
contract Lottery is Ownable {
    ...
    // event CloseSeri(uint256 seri);
    // event OpenResult(uint256 seri);
    // event BuyTicket(uint256 seri, string numberInfo);
    ...
    function buyCarryOver(
        ...
        uint256 shareAffiliateAmount = assetAmount.mul(share2AffiliateCO).div(1000000);
        uint256 takeTokenAmount = assetAmount.mul(share2OperatorCO).div(1000000);
        if (_assetIndex == 0) {
        ...
        // emit BuyTicket(_seri, _numberInfo);
    }
    ...
    function buy(
        ...
        uint256 postAmount = assetAmount.mul(postPrices[_seri]).div(series[_seri].price);
        uint256 postRemain = assetAmount.sub(postAmount);
        uint256 shareStakeAmount = postAmount.mul(share2Stake).div(1000000);
        uint256 sharePurchaseAmount = postAmount.mul(share2Purchase).div(1000000);
        uint256 shareAffiliateAmount = postAmount.mul(share2Affiliate).div(1000000);
        uint256 takeTokenAmount = postAmount.mul(share2Operator).div(1000000);
    ...
    function setAssets(
        ...
        require(_symbols.length == _bep20s.length && _symbols.length == _priceFeeds.length, "Pricefeed: length mismatch");
    ...
    function config(
        ...
        require(_share2Stake + _share2Purchase + _share2Affiliate + _share2Operator < 1000000, "Norma: Invalid percent");
        require(_share2AffiliateCO + _share2OperatorCO < 1000000, "Carry-over: Invalid percent");
