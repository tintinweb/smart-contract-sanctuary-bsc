...
contract PaymentGateway is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event ProductPayed(address indexed user, bytes product_code, bytes coupon_code, bytes referrer_code, uint256 total_charged);
    ...
    mapping(address => bytes) public users_referrer_code;
    mapping(address => mapping(bytes => bool)) public users_coupon_used;

    mapping(bytes => Referrer) public referrer_data;
    mapping(bytes => Product) public product_data;
    mapping(bytes => Coupon) public coupon_data;

    bytes[] public refererrer_code_list;
    bytes[] public product_code_list;
    bytes[] public coupon_code_list;
    ...
    function referrerCreate(address user, bytes memory code, uint8 cut_percent) public onlyOwner {
        ...
        bytes storage user_code = users_referrer_code[user];

        require(user_code.length == 0, "ALREADY_A_REFERRER");
    ...
    function referrerSetActive(address user, bool active) public onlyOwner {
        bytes storage code = users_referrer_code[user];
        Referrer storage referrer = referrer_data[code];
    ...
    function productCreate(bytes memory code, uint256 price) public onlyOwner {
    ...
    function productSetActive(bytes memory code, bool active) public onlyOwner {
        Product storage product = product_data[code];
    ...
    function productSetPrice(bytes memory code, uint256 price) public onlyOwner {
        Product storage product = product_data[code];
    ...
    function couponCreate(bytes memory code, uint8 discount_percent, bytes[] memory product_list) public onlyOwner {
        ...
        for (uint i=0; i < product_list.length; i++) {
            bytes memory product_code = product_list[i];
    ...
    function couponSetActive(bytes memory code, bool active) public onlyOwner {
        Coupon storage coupon = coupon_data[code];
    ...
    function couponSetDiscountPercent(bytes memory code, uint8 discount_percent) public onlyOwner {
        require(discount_percent <= MAX_DISCOUNT_PERCENT, "INVALID_DISCOUNT_PERCENT");
    ...
    function couponSetProducts(bytes memory code, bytes[] memory product_list, bool active) public onlyOwner {
        Coupon storage coupon = coupon_data[code];
    ...
    function pay(address user, bytes memory product_code, bytes memory coupon_code, bytes memory referrer_code) public whenNotPaused nonReentrant {
        _checkSenderIs(user);
