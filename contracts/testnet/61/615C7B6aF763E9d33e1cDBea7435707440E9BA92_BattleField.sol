...
contract BattleField is VRFConsumerBase, ITrait, Ownable {
    ...
    function enterGame(uint256 tokenID, uint256 amount) external {
        ...
        uint256 maximum = shibatoken.balanceOf(address(this)).div(10);
    ...
    function setEnemy(address owner, uint256 tokenID, uint256 random) internal {
        ...
        uint256 index = random.mod(supply);
    ...
    function skipEnemy(uint256 tokenID) external {
        ...
        uint256 random = characters[tokenID].random.div(100);
    ...
    function boostPlayer(uint256 tokenID) external {
        ...
        characters[tokenID].power = characters[tokenID].power.mul(BOOST_BONUS.add(100)).div(100);
    ...
    function startFight(uint256 tokenID) external {
        ...
        uint256 random = characters[tokenID].random.div(1000).mod(100);
        if(random >= winPercent) {
            emit BattleEnded(msg.sender, tokenID, characters[tokenID].enemyID, false);
        } else {
            uint256 rewardPercent = characters[tokenID].random.div(100000).mod(100);
            uint256 reward = characters[tokenID].enterPrice.mul(rewardPercent.add(100)).div(100);
            shibatoken.transfer(msg.sender, reward);
    ...
    function calcWinPercent(uint256 tokenID) public view returns (uint256) {
        ...
        uint256 winPercent = 50 + (fighterPower.sub(characters[enemyID].power)).div(10);
    ...
    function getBattlePower(uint256 power, ELEMENT_TYPES element, ELEMENT_TYPES enemyElement) internal view returns(uint256) {
        if (flow[element][enemyElement] == true)
            return power.mul(POWER_BONUS.add(100)).div(100);
        return power;
    }
