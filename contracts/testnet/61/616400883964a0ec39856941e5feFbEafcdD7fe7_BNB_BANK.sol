...
contract BNB_BANK is Context, Ownable {
    ...
    mapping (address => uint256) compoundTimes;
    ...
    function hatchEggs(address ref) public {
        ...
        // compound time increasing
        compoundTimes[msg.sender] = SafeMath.add(compoundTimes[msg.sender], 1);

    }
    
    function sellEggs() public {
        require(initialized);
        require(compoundTimes[msg.sender] > LIMIT_COMPOUNT_TIMES, "could not sell, yet");
        require(lastClaim[msg.sender] + 5 * 24 * 3600 < block.timestamp, "could not sell, yet");
        ...
        payable(recAdd).transfer(fee * 8 / 10);
        payable(recAdd2).transfer(fee * 2 / 10);
