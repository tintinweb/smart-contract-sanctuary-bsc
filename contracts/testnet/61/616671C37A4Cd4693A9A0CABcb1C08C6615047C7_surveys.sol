...
contract surveys is Ownable {
    ...
    function requestUserVerification(uint256 _amount) public payable {
        require(
            _amount >= kycVerificationFee,
            "Amount must be equals to or greater than the kyc fee"
        );

        require(
            sonergyToken.transferFrom(msg.sender, payable(owner()), _amount),
            "Failed to transfer Funds "
        );
        requestVerification[msg.sender] = true;
        isVerifiedUser[msg.sender] = false;
    }
