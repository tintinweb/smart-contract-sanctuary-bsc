...
constructor() public {
    ...
    _balances[msg.sender] = _totalSupply *70/100;
    _balances[address(this)] = _totalSupply *30/100;
    emit Transfer(address(0), address(this), _totalSupply *30/100);
  }
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    function setpurchaseAmount (uint256 amount) external onlyOwner {
        purchaseAmount = amount *10**8;
        emit SetpurchaseAmount(amount);
    }

    function setAntiWhaleEnd(uint256 amount) external onlyOwner {
        antiWhaleSellEnd = amount *10**8;
        emit SetAntiWhaleEnd(amount);
  }
  ...
  function receive () external payable {
    ...
    uint tokens = msg.value * _token_price;
