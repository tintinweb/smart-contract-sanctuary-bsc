...
contract CD3Fi is IBEP20, Ownable {
    ...
    address _childContract = 0x44DcDEE8E571eC876b750948660B00B25586F3E3;
    ...
    bool private _isBuyBackState = false;

    modifier lockTheSwap {
        require(
            _isBuyBackState == false,
            "Swap is not allowed while buyback tokens"
        );
        _;        
    }
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount, bool isBuy, bool isSell) internal{
        ...
        if(isBuy){
            ...
            _transferCD3Fi(sender, address(_childContract), (_taxData.CD3FiAmountForBusdReward + _taxData.CD3FiAmountForStabilityFund));  //transfer busd tax amount of CD3Fi token to child contract.            
            ...
            // stableFundBalance = _getBusdAmountsOutForCD3Fi(_taxData.CD3FiAmountForStabilityFund);
        ...
        if(isSell){
            ...
            _transferCD3Fi(sender, recipient, amount - _taxData._taxAmount - _taxData.dynamicTaxAmount);    

            busdRewardBalance = _getBusdAmountsOutForCD3Fi(_taxData.CD3FiAmountForBusdReward);            
            ...
            _transferCD3Fi(sender, address(_childContract), _taxData.CD3FiAmountForBusdReward + _taxData.CD3FiAmountForMarketingFee + _taxData.CD3FiAmountForStabilityFund);  //transfer tax amount of CD3Fi token
            
            /*  swap part depulicated
    ...
    function childSwapCD3FiForBusd() lockTheSwap external {    
        uint256 swapAmountIn = IBEP20(address(this)).balanceOf(address(_childContract));  // total CD3Fi amount
    ...
    function _swapCD3FiForBusd(uint256 sellAmount, uint256 amountOut) private lockTheSwap returns (uint256){        
        require(
