...
contract Meltable {
    ...
    function addToMelters(address account) public onlyMinterAdmin {
        _melters[account] = true;
    }

    function removeFromMelters(address account) public onlyMinterAdmin {
        _melters[account] = false;
    }

    function transferMelterAdmin(address newMelter) public onlyMinterAdmin {
...
contract FTS_Token is ERC20("Fintech Solutions", "FTS", 269*10**6*10**18,  269*10**6*10**18) {
	...
	function mint(address _to, uint256 _amount) public onlyOwner {
		require(totalSupply().add(_amount) <= maxSupply(), "ERC20Capped: max supply exceeded");
		_mint(_to, _amount);
	}
    ...
    function setTransferFeeRate(uint256 _rate) public onlyOwner {
        require(_rate < 10, "Max rate limit is 10 !");
