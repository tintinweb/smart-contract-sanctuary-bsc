...
contract ReedBricks is Context, IERC20, Ownable {
    ...
    uint256 private localSalesRate = 5000; // Number of tokens per BNB
    uint256 private raisedAmount = 0; // keeping track of sales
    bool private localSalesIsOn = true;
    mapping(address => uint256) public localSales;
    event BoughtTokens(address _buyer, uint256 _total);
    modifier whenLocalSaleIsActive() {
        // Check if sale is active
        require(localSalesIsOn, "REED: Local sales has ended");
        _;
    }
    ...
    // we can sell locally while waiting for presale
    function localSell() public payable whenLocalSaleIsActive {
        uint256 weiAmount = msg.value; // Calculate tokens to sell
        uint256 tokens = weiAmount.mul(localSalesRate);
        ...
        localSales[msg.sender] = localSales[msg.sender].add(tokens);
    }

    function withdrawPurchased() public {
        uint256 tokens = localSales[msg.sender];
        _tokenTransfer(address(this), msg.sender, tokens, false);
        localSales[msg.sender] = localSales[msg.sender].sub(tokens);
    }
