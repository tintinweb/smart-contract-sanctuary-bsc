...
// File: contracts/token/BloggercubeToken.sol

pragma solidity ^0.8.4;

/// @custom:security-contact [email protected]
/// @title Smart contract for BCC (Bloggercube) ERC20/BEP20 token
/// @author Milad Jafari [email protected]
...
contract BloggercubeToken is ERC20, ERC20Snapshot, Ownable {
  ...
  // Modifiers
  modifier onlyValidFee(uint8 fee) {
    require(fee >= 0 && fee <= 10, "The fee percent must be between 0 - 10");
    _;
  }
  ...
  /// @notice set a percent value for marketing fee
  /// @param _fee a value for tax in percent
  ...
  /// @notice set a percent value for development fee
  /// @param _fee a value for tax in percent
  /// @return return a boolean value
  function setDevelopmentFee(uint8 _fee)
    public
    onlyOwner
    onlyValidFee(_fee)
    returns (bool)
  {
    developmentFeePercent = _fee;
    return true;
  }

  /// @notice set a percent value for donation fee
  /// @param _fee a value for tax in percent
  /// @return return a boolean value
  function setDonationFee(uint8 _fee)
    public
    onlyOwner
    onlyValidFee(_fee)
    returns (bool)
  {
    donationFeePercent = _fee;
    return true;
  }

  /// @notice set an address for marketing
  /// @param _addr a wallet address for marketing
  /// @return return a boolean value
  ...
  /// @notice set an address for development
  /// @param _addr a wallet address for development
  /// @return return a boolean value
  function setDevelopmentWalletAddress(address _addr)
    public
    onlyOwner
    returns (bool)
  {
    developmentWalletAddress = _addr;
    return true;
  }

  /// @notice set an address for donation
  /// @param _addr a wallet address for donation
  /// @return return a boolean value
  function setDonationWalletAddress(address _addr)
    public
    onlyOwner
    returns (bool)
  {
  ...
  function _transfer(
    ...
    require(balanceOf(sender) >= amount, "Sent amount exceeds balance!");
    ...
    if (sender != _owner && recipient != _owner) {
      ...
      require(amount > 0, "Sent amount is not valid.");

      super._transfer(sender, recipient, finalAmount);

      if (marketingFee > 0) {
        super._transfer(sender, marketingWalletAddress, marketingFee);
      }
      if (developmentFee > 0) {
        super._transfer(sender, developmentWalletAddress, developmentFee);
      }
      if (donationFee > 0) {
        super._transfer(sender, donationWalletAddress, donationFee);
      }
    } else {
  ...
  // Constructor
  constructor() ERC20("Bloggercube", "BCC") {
    _owner = msg.sender;
