...
contract IdoProjectV2 is ReentrancyGuard, Initializable {
    ...
    function submitWinners(address[] memory _users) external onlyOperator {
        ...
        require(winners.length() + totalAllowList + _users.length  <= totalSlot + allowListSubmitted, "Slot over flow");
        uint _decimals = Decimal(address(tokenSale)).decimals();
        for(uint i = 0; i < _users.length; i++ ){
        ...
        if(winners.length() == totalSlot || winners.length() - allowListSubmitted == totalSlot - totalAllowList) {
            poolEnd = true;
            emit PoolEnd(poolEnd, projectId);
        }
    }
