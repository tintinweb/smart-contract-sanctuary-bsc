...
import "Ownable.sol";
import "BufferEpocPoolV2Unintialized.sol";
 ...
 */
contract RangeOptionConfigUnintialized is Ownable, IRangeOptionsConfig {
    ...
    uint256 public faceValue = 100e8;
    ...
    ITwapPriceProvider public priceProvider; // Gives pool's tokenX price in stable coin

    constructor(BufferEpocPoolV2Unintialized _pool, ITwapPriceProvider pp) {
        pool = _pool;
        priceProvider = pp;
    }
    ...
    function initialize(
        ...
        isValidRange(_lowerBound, _upperBound);
        ...
        isInitialized = true;
        emit Initialize(faceValue, discountFactor, upperBound, lowerBound);
    }
    ...
    function isValidRange(uint256 _lowerBound, uint256 _upperBound) internal {
        uint256 currentPrice = priceProvider.getUsdPrice();
        uint256 margin = currentPrice / 20;

        // Lower bound and upper bounds should have atleast 5% margin with the current price
        require(_lowerBound < currentPrice - margin, "Lower bound is too high");
        require(_upperBound > currentPrice + margin, "Upper bound is too low");
    }

    /**
     * @notice Used for changing range
     * @param _lowerBound New lower bound value
     * @param _upperBound New upper bound value
     */
    function setRange(uint256 _lowerBound, uint256 _upperBound)
        external
        onlyProjectOwner
        isEpocOver
    {
        ...
        emit UpdateRange(lowerBound, upperBound);
    }

    /**
     * @notice Used for changing faceValue
     * @param _faceValue New faceValue
     */
        ...
        isEpocOver
    {
        ...
        emit UpdateFaceValue(_faceValue);
    }

    /**
     * @notice Used for changing discountFactor
     * @param _discountFactor New discountFactor
     */
    function setDiscountFactor(uint256 _discountFactor)
        external
        onlyProjectOwner
        isEpocOver
    {
        require(_discountFactor < 100, "Discount factor is too high");
        discountFactor = _discountFactor;

        emit UpdateDiscountFactor(_discountFactor);
    }
    ...
    modifier isEpocOver() {
        require(
            ...
            "Can't modify before the expiry ends"
        );
        _;
    }
}
