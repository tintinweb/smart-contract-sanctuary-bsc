...
contract TheNetworkSociety is Context, Ownable {
    ...
    function setupSponsorshipDetails(address addr, address sponsor) private {
        if(isValidSponsorAddress(addr, sponsor)) {
            ...
            //if sponsor doesn't have existing network invite team, create a network.
    ...
    function deposit(address addr, uint256 amount) private isInitialized {
        ...
        totalDeposited += amount; //update protocol's total deposits.
        emit Deposit(addr, amount.sub(payTax(amount)), block.timestamp);
    ...
    function compoundAmountOf(address addr, uint256 amount) view private returns(uint256 maxCompoundAmount, uint256 amountForCompound) { 
        User storage user = users[addr];
        maxCompoundAmount = maxCompoundOf(users[addr].totalDirectDeposits);
        amountForCompound = amount; 
        if(user.totalDepositAmount >= maxCompoundAmount) amountForCompound = 0; //avoid reverts, but if amount = 0, user already exceeded x5 of total deposit.
        if(user.totalDepositAmount.add(amount) >= maxCompoundAmount) amountForCompound = maxCompoundAmount.sub(user.totalDepositAmount);      
    }

    //compound user's current yield.
    function compound(address addr) public isInitialized {   
        User storage user = users[addr];
        (, , uint256 payoutAfterTax) = payoutOf(addr, false);
        (uint256 maxCompoundAmount, uint256 finalAmountForCompound) = compoundAmountOf(addr, payoutAfterTax);

        if(finalAmountForCompound > 0 && user.totalDepositAmount < maxCompoundAmount){ // avoid reverts
            ...
            user.totalDepositAmount += finalAmountForCompound;
            user.totalIncomeCompounded += finalAmountForCompound;  
            user.lastAction = block.timestamp;   
            totalCompounded += finalAmountForCompound;
            emit Compound(addr, finalAmountForCompound, block.timestamp);
            
            //if user reached max compound after last compound. emit MaxCompound event.
            if(user.totalDepositAmount >= maxCompoundAmount) {
    ...
    function networkInvitePayout(address addr, uint256 amount) public isInitialized {   
        User storage user = users[addr];
        User storage networkSponsor = users[user.networkSponsor];
        ...
        if(user.networkSponsor != address(0)) {
            ...
            if(networkSponsor.totalDepositAmount.add(inviteBonus) > maxCompoundOf(networkSponsor.totalDirectDeposits))
            inviteBonus = maxCompoundOf(networkSponsor.totalDirectDeposits).sub(networkSponsor.totalDepositAmount);
            ...
            // referrer
            usersBonus[user.networkSponsor].inviteBonus += inviteBonus; //statistics record.
    ...
    //user's final amount available for compound.
    function payoutAmountOf(address addr, uint256 maxPayout, uint256 payoutAfterTax) view private returns(uint256 payoutToWithdraw) { 
        User storage user = users[addr];
        payoutToWithdraw = payoutAfterTax;
        if(user.totalDepositPayouts >= maxPayout) payoutToWithdraw = 0; //avoid reverts, but if amount = 0, user already has already reached Max Payout.
        if(user.totalDepositPayouts.add(payoutAfterTax) >= maxPayout) payoutToWithdraw = maxPayout.sub(user.totalDepositPayouts);
    }
    ...
    function withdraw() public isInitialized {
        ...
        (uint256 maxPayout, , uint256 payoutAfterTax) = payoutOf(addr, true);
        uint256 finalAmountForWithdraw = payoutAmountOf(addr, maxPayout, payoutAfterTax);

        if(finalAmountForWithdraw > 0 && user.totalDepositPayouts < maxPayout){ // avoid reverts
            ...
            user.totalDepositPayouts += payoutAfterTax;
            user.lastAction = block.timestamp;

            if(token.balanceOf(address(this)) < payoutAfterTax) payoutAfterTax = token.balanceOf(address(this));
            
            totalWithdrawn += payoutAfterTax;
            token.transfer(addr, payoutAfterTax);
            emit Withdraw(addr, payoutAfterTax, block.timestamp);   
    ...
    function drawLastDepositWinner() private {
        if(lastDepositEnabled && block.timestamp.sub(lastDepositLastDrawAction) >= lastDepositTimeStep && lastDepositPotentialWinner != address(0)) {
            User storage user = users[lastDepositPotentialWinner];
            uint256 reward = lastDepositPoolBalance;

            user.totalDepositPayouts += reward;
            usersBonus[lastDepositPotentialWinner].lastDepositBonus += reward;
            totalWithdrawn = totalWithdrawn.add(reward);
            token.transfer(lastDepositPotentialWinner, reward.sub(payTax(reward)));
            emit LastBuyPayout(lastBuyPoolCycle, lastDepositPotentialWinner, reward, block.timestamp);
    ...
    function poolReferralDeposit(address addr, uint256 amount) private {
        ...
        topReferrerList[topReferrerPoolCycle][sponsor] += amount;
        ...
        for(uint256 i = 0; i < eventBonusesArr.length; i++) {
            ...
            if(topReferrerList[topReferrerPoolCycle][sponsor] > topReferrerList[topReferrerPoolCycle][topReferrerPool[i]]) {
    ...
    function poolTopDeposit(address addr, uint256 amount) private {
        ...
        topDepositList[topDepositPoolCycle][addr] += amount;
        ...
        for(uint256 i = 0; i < eventBonusesArr.length; i++) {
            ...
            if(topDepositList[topDepositPoolCycle][addr] > topDepositList[topDepositPoolCycle][topDepositPool[i]]) {
                for(uint256 j = i + 1; j < eventBonusesArr.length; j++) {
    ...
    function maxPayoutOf(uint256 amount) pure private returns(uint256) {
        return amount.mul(userMaxPayout).div(dividerPercent);
    }

    //user's current payout available.
    function payoutOf(address addr, bool isClaim) view public returns(uint256 maxPayout, uint256 payout, uint256 payoutAfterTax) {
        User storage user = users[addr];
        ...
        if(user.totalDepositPayouts < maxPayout) {
            ...
            uint256 withdrawTax = getVariableWithdrawTax(addr);
            //isClaim: true = withdraw, false = compound.
            uint256 sustainabilityTax = isClaim ? withdrawTax : compoundTax;
            payoutAfterTax = payout.sub(payout.mul(sustainabilityTax).div(dividerPercent));
        }
    }
    ...
    function airdrop(address receiver, uint256 amount) public isInitialized {
        require(users[receiver].totalDepositAmount.add(amount) <= maxCompoundOf(users[receiver].totalDirectDeposits), "User exceeded x5 of total deposit.");
    ...
    function networkAirdrop(uint256 amount) public isInitialized {
        ...
        for(uint256 i = 0; i < networks[msg.sender].members.length; i++) {
            ...
            if(users[receiver].totalDepositAmount.add(amount) >= maxCompoundOf(users[receiver].totalDirectDeposits)) continue;
    ...
    function userCurrentInvestmentInfo(address addr) view external returns(uint256 maxPayout, uint256 maxCompound, uint256 payout, uint256 payoutWithWithdrawTax, uint256 withdrawTax, uint256 amountForCompound, uint256 compoundingTax) {
        compoundingTax = compoundTax;
        withdrawTax = getVariableWithdrawTax(addr);
        (maxPayout, payout, payoutWithWithdrawTax) = payoutOf(addr, true);
        uint256 finalAmountForCompound = payout.sub(payout.mul(compoundTax).div(dividerPercent));
        (maxCompound, amountForCompound) = compoundAmountOf(addr, finalAmountForCompound);
    }
    ...
    function userNetworkMemberInfo(address addr) view external returns(uint256 id, address owner, uint256 createTime, address[] memory members, uint256 totalNetworkInvites, uint256 totalNetworkInvitesDeposit) {
    ...
    function userNetworkSponsorMemberInfo(address addr) view external returns(uint256 id, address owner, uint256 createTime, address[] memory members, uint256 totalNetworkInvites, uint256 totalNetworkInvitesDeposit) {
    ...
    function contractInfo() view external returns(uint256 _totalInvestors, uint256 _totalDeposited, uint256 _totalWithdrawn, uint256 _totalCompounded, uint256 _totalAirdrops, uint256 _totalNetworksCreated, uint256 _totalContractBalance) {
        return (totalInvestors, totalDeposited, totalWithdrawn, totalCompounded, totalAirdrops, totalNetworksCreated, token.balanceOf(address(this)));
    }
    ...
    function lastDepositInfo() view external returns(uint256 _lastBuyPoolCycle, uint256 _lastDepositPoolBalance, uint256 _lastDepositLastDrawAction, address _lastDepositPotentialWinner) {
        _lastBuyPoolCycle           = lastBuyPoolCycle; 
        _lastDepositPoolBalance     = lastDepositPoolBalance;
        _lastDepositLastDrawAction  = lastDepositLastDrawAction;         
        _lastDepositPotentialWinner = lastDepositPotentialWinner;
    }

    //top referrer information.
    function topReferrerInfo() view external returns(address[5] memory addrs, uint256[5] memory deps, uint256 _topReferrerPoolCycle, uint256 _topReferrerPoolBalance, uint256 _topReferrerLastDrawAction) {
        //current top 5 referrers.
        for(uint256 i = 0; i < eventBonusesArr.length; i++) {
            if(topReferrerPool[i] == address(0)) break;
            addrs[i] = topReferrerPool[i];
            deps[i]  = topReferrerList[topReferrerPoolCycle][topReferrerPool[i]];
        }
        //current round details
        _topReferrerPoolCycle      = topReferrerPoolCycle;
    ...
    function topDepositInfo() view external returns(address[5] memory addrs, uint256[5] memory deps, uint256 _topDepositPoolCycle, uint256 _topDepositPoolBalance, uint256 _topDepositLastDrawAction) {
        //current top 5 depositors.
        for(uint256 i = 0; i < eventBonusesArr.length; i++) {
            if(topDepositPool[i] == address(0)) break;  
            addrs[i] = topDepositPool[i];
            deps[i]  = topDepositList[topDepositPoolCycle][topDepositPool[i]];
        }
        //current round details
        _topDepositPoolCycle      = topDepositPoolCycle;
...
/** 
    ...
    * Every sell/withdraw the compound count will be set back to 0.  
    ...
    - If there is no new deposit after the last deposit after 2 hours, Rewards will be sent to the last deposit address.
