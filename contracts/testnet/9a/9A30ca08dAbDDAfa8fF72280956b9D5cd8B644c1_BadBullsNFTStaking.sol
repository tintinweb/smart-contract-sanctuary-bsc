...
// File: contracts/BullStaking.sol
...
contract BadBullsNFTStaking is IERC721Receiver, Ownable {
    ...
    constructor(address _erc721 ) {
        ...
        RTOKEN = IToken(0x072e943cBB4CFA01dad0E4FA8B54f4e742288FA8); // rewards token contract address
        // number of tokens Per day
        // to set rarity for tiers you need to start from 0
        // tier0 = 50 , tier1 = 60 .... ROR
        rewardRate = [10, 20, 30, 40, 50];
    }

    function EXPIRATION() external view returns (uint256) {
         round storage r = roundInfo[currentRound];
         return r.endBlock;
    }
    ...
    function _getMultiplier(uint256 _from, uint256 _to) internal view returns (uint256) {
        ...
        if (_to <= r.endBlock) {
            return _to - _from;
        } else if (_from >= r.endBlock) {
            return 0;
        } else {
            return r.endBlock - _from;
        }
    }

    function updatePool(uint256[] memory ids, bool isDeposit) internal {
        ...
        round storage r = roundInfo[currentRound];
        if(block.number > lastRewardBlock) {
            r.rewardsLeftToGo -= r.currentRewardRate * multiplier;
            totalPending += r.currentRewardRate * multiplier;
            lastRewardBlock = block.number; 
        }

        uint256 rateChange = 0;
        for (uint i =0; i<ids.length; i++){
            rateChange += findRate(ids[i]);
        }
        if(isDeposit) r.currentRewardRate += rateChange;
        if(!isDeposit) r.currentRewardRate -= rateChange;

        if(block.number < r.endBlock) r.endBlock = block.number + ((RTOKEN.balanceOf(address(this)) - totalPending) / r.currentRewardRate);
    }
    ...
    function InitializeFirstRound(uint256 _startInBlocks, uint256 _endInDays) public onlyOwner() {
        uint256 newTokens = RTOKEN.balanceOf(address(this));
        require(newTokens > 0, "no tokens for rewards");
        require(!isInitialized, "Already initialized");
//block per day
        round storage r = roundInfo[currentRound];
        r.startBlock = block.number + _startInBlocks;
        lastRewardBlock = r.startBlock;
        r.endBlock = block.number + (_endInDays * 28800);
        isInitialized = true;
    }
    
    function StartPool(uint256 _startInBlocks, uint256 _endInDays) public onlyOwner() {
        require(block.number > roundInfo[currentRound].endBlock, "current round must be finished");
         uint256 newTokens = RTOKEN.balanceOf(address(this)) - totalPending;
        require(newTokens > 0, "no tokens for rewards");
        currentRound += 1;
        round storage r = roundInfo[currentRound];
        r.currentRewardRate = roundInfo[currentRound-1].currentRewardRate;
        r.startBlock = block.number + _startInBlocks;
        lastRewardBlock = r.startBlock;
        r.endBlock = block.number + (_endInDays * 28800);
    }

    function extendPool() public onlyOwner {
         uint256 newTokens = RTOKEN.balanceOf(address(this)) - totalPending;
        require(newTokens > 0, "no tokens for rewards");
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        round storage r = roundInfo[currentRound];
        ...
        if(block.number < r.endBlock) r.endBlock = block.number + (newTokens / r.currentRewardRate);
    }
        ...
        returns (uint256[] memory rewards)
    {
        for(uint j = 0; j <= currentRound; j++) {
            round storage r = roundInfo[j];
            for (uint256 i; i < tokenIds.length; i++) {
                uint256 tokenId = tokenIds[i];
                uint256 rate = findRate(tokenId);
                if(depositBlocks[account][tokenId] < r.endBlock) {
                    rewards[i] +=
                    rate *
                    (_deposits[account].contains(tokenId) ? 1 : 0) *
                        (Math.min(block.number, r.endBlock) -
                            Math.max(depositBlocks[account][tokenId], r.startBlock));
                }
            }
        }
    }
    ...
    function claimRewards(uint256[] calldata tokenIds) public {
        uint256 reward;
        round storage r = roundInfo[currentRound];
            uint256 curblock = Math.min(block.number, r.endBlock);
