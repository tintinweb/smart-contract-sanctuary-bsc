...
contract StandardNftContract is
ERC721,
ERC721Enumerable,
ERC721URIStorage,
ERC721Burnable,
...
Ownable
{
    ...
    Counters.Counter private _presetUriIdCounter;
    ...
    function lastPresetUriId()
    external
    view
    returns (uint256)
    {
        return _presetUriIdCounter.current();
    }
    ...
    onlyOwner
    {
        require(index < _presetUriIdCounter.current());
    ...
    function addPresetUri(string memory presetUri)
    external
    onlyOwner
    {
        _presetUris[_presetUriIdCounter.current()] = presetUri;
        _presetUriIdCounter.increment();
    }
    ...
    payable
    {
        ...
        require(_tokenIdCounter.current() < maximumSupply, "exceeds maximum supply");
        require(balanceOf(msg.sender) <= maximumSupplyPerAddress, "exceeds maximum supply per address");

        uint256 tokenId = _tokenIdCounter.current();
        string memory uri = _presetUris[random(_presetUriIdCounter.current())];
