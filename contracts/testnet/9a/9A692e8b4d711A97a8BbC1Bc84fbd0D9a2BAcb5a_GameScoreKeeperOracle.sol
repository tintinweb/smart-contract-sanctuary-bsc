    ...
    IChainLinkOracleSportData
{
    ...
    mapping(address => uint256) private lastTime;
    mapping(ISportManager.ProviderGameData => bytes32) jobId;

    uint256 public payment = (1 * LINK_DIVISIBILITY) / 100;
    uint256 public times = 1 minutes;
    ...
    modifier onlyPermission(address _user) {
        require(
            pool.isCompetitionExisted(_user),
    ...
    function requestData(
        ...
        require(
            block.timestamp > lastTime[msg.sender] + times,
            "ChainLinkContract: Exceed rate limit"
        );
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 index = 0;
        for (uint256 i = 96; i <= data.length; i = i + 32) {
            bytes32 temp;
            assembly {
