...
contract CustomBill is Initializable, Policy {
    ...
    event BillRedeemed(uint256 billId, address recipient, uint256 payout, uint256 remaining);
    ...
    IERC20Metadata public payoutToken; // token paid for principal
    ...
    EnumerableSet.UintSet private billIssuedIds;
    ...
    function initialize(
        ...
        payoutToken = IERC20Metadata(ICustomTreasury(_config[0]).payoutToken());
        require(_config[1] != address(0), "principalToken cannot be zero");
        principalToken = IERC20(_config[1]);
        ...
        DAO = _config[4];
        require(_tierCeilings.length == _fees.length, "tier length != fee length");
        require(address(_config[5]) != address(0), "billNft cannot be zero");
        ...
        // Fix for division by 0 when terms are not initialized (introduced in Solidity 0.8)
        terms = Terms ({
            controlVariable: 1,
            vestingTerm: 1,
            minimumPrice: 1,
    ...
    function deposit(
        ...
        uint256 billId = billNft.mint(_depositor);
     ...
     */
    function batchRedeem(uint256[] calldata _billIds) external { 
    ...
    function trueBillPrice() public view returns (uint256 price_) {
        price_ = billPrice().add(billPrice().mul(currentFee()).div(1e6));
    }
    ...
    function payoutFor(uint256 _value ) public view returns (uint256 _payout, uint256 _fee) {
        if(feeInPayout) {
            uint total = FixedPoint.fraction(_value, billPrice()).decode112with18().div(1e11);
            _fee = total.mul(currentFee()).div(1e6);
            _payout = total.sub(_fee);
        } else {
            _fee = _value.mul(currentFee()).div(1e6);
            _payout = FixedPoint.fraction(customTreasury.valueOfToken(address(principalToken), _value.sub(_fee)), billPrice()).decode112with18().div(1e11);
        }
    }
