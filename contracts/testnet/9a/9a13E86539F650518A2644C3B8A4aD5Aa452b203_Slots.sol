    ...
    ReentrancyGuardUpgradeable
{
    ...
    event UserWon(address user, uint256 bet, uint256 gameId, uint256 reward, uint256 lastDrawnBlock);
    ...
    function _payRewardsAndUpdateStats(
        ...
        for (uint256 i = 0; i < 7; i++) {
            if (_players[i] != address(0)) {
                ...
                emit UserWon(_players[i], _bet, games[_bet].id, reward, lastDrawBlockForUser[_bet][_players[i]]);
