...
////////////////////////////////////////// only owner
    ...
    function editUpdatePeriod(uint32 _updatePeriod)
        public
        onlyOwner
    {
        _editUpdatePeriod(_updatePeriod);
    }
...
////////////////////////////////////////// IOracleHistory
    ...
    function updateHistoryOnStableCoinRebase()
        public
        override
    {
        _updateHistoryOnStableCoinRebase();
    }
...
////////////////////////////////////////// PairsManager methods
    ...
    function addPair(address _asset0, address _asset1)
        public
        onlyOwner
    {
        _addPair(_asset0, _asset1);
    }
...
library UniswapV2OracleLibrary {
    ...
    function currentCumulativePrices(
        address pair
    ) internal view returns (uint price0Cumulative, uint price1Cumulative, uint32 blockTimestamp, bool allowToUse) {
        ...
        allowToUse = blockTimestampLast != blockTimestamp;
        if (allowToUse) {
...
////////////////////////////////////////// methods for store oracle history
    ...
    function _editUpdatePeriod(uint32 _updatePeriod)
        internal
    {
        PERIOD = _updatePeriod;
    }

    function _updateHistoryOnStableCoinRebase()
        internal
    {
        uint256 _blockTimestampLast = block.timestamp;
        uint256 _amountOut = __getTokenPriceInUSDT(stableCoin);

        uint256 count = oracleHistory[stableCoin].length();
        if (count == 0) {
            oracleHistory[stableCoin].set(_blockTimestampLast, _amountOut);
            return;
        }
        (, uint256 _amountOutLast) = oracleHistory[stableCoin].at(count - 1);
        if (_amountOutLast != _amountOut) {
            oracleHistory[stableCoin].set(_blockTimestampLast, _amountOut);
        }
    }
...
////////////////////////////////////////// methods for read oracle history
        ...
        returns (uint256 _cumulativePrice, uint256 _cumulativeTime)
    {
        ...
        if (lastSegment == historiesCount - 1 && _timeEnd >= _periodBegin) {
            ...
            (, _priceEnd) = _history.at(lastSegment);
            (_periodBegin, ) = _history.at(lastSegment - 1);
            // Calculate the period between the start of the last segment and the end time
            require(_timeEnd >= _periodBegin, string.concat(
                    'else ',
                    ' _token: ', _token.toHexString(),
                    ' _periodBegin: ', _periodBegin.toString(),
                    ' _timeEnd: ', _timeEnd.toString(),
                    ' _timeBegin: ', _timeBegin.toString(),
                    ' lastSegment: ', lastSegment.toString(),
                    ' firstSegment: ', firstSegment.toString(),
                    ' historiesCount: ', historiesCount.toString(),
                    ''
                ));
            _period = _timeEnd - _periodBegin ;
...
////////////////////////////////////////// override storage methods
        ...
        override
    {
        ...
        require(
            hasMainToken,
            'UsdConsultant::_addPair - token0 or token1 should be stableCoin or USDT or SYF'
        );
...
////////////////////////////////////////// consult in USDT methods
        ...
        returns (uint256)
    {
        ...
        if (isPairExists(SYF, _tokenIn)) {
            uint256 amountInSYF = __consult(_tokenIn, SYF, _amountIn);
            return __consult(SYF, USDT, amountInSYF);
        }
        ...
        returns (uint256)
    {
        ...
        if (isPairExists(SYF, _tokenOut)) {
            uint256 amountInSYF = __consult(USDT, SYF, _amountInUSDT);
            return __consult(SYF, _tokenOut, amountInSYF);
        }
        ...
        returns (uint32)
    {
        ...
        if (isPairExists(SYF, _tokenIn)) {
            PairInfo memory info = getPairInfo(SYF, _tokenIn);
            return __updatePairInfo(info.lpToken, _updatePeriod);
        }
...
////////////////////////////////////////// NFTBondManager methods (pairs with SYF)

    function _NFTBondManagerAddAsset(address _asset, uint256 _cap)
        internal
    {
        require(_asset != SYF, 'PairsManager::_NFTBondManagerAddAsset - SYF is not allowed');
        IAssetsManagerWithCaps(NFTBondManager).addAsset(_asset, _cap);
        _addPair(_asset, SYF);
        lpTokenNeedInNFTBondManager[getLpToken(_asset, SYF)] = true;
    }
        ...
        internal
    {
        ...
        address lpToken = getLpToken(_asset, SYF);
