...
contract NFTExchange is INFTExchange, Caller, Fallback {
    ...
    function takeFixPrice(uint256 orderId_) external payable override {
        ...
        require(orderIdSet.removeUintSet(orderId_), "1187");
        ...
        (uint256 platformFee, uint256 creatorFee) = delivery_(orderId_, order_);
        emit TakeEvent(
            orderId_,
            order_.type_,
            order_.token,
    ...
    function cancel_(uint256 orderId_, bool isPlatform) private {
        ...
        require(orderIdSet.removeUintSet(orderId_), "1170");
        delete order[orderId_];
        if (address(1) != order_.bidder) {
            config.getAsset().claimMainOrERC20(
                order_.quoteToken,
    ...
    function delivery(uint256[] memory orderId_) external override onlyCaller {
        ...
        for (uint256 i = 0; i < orderId_.length; i++) {
            ...
            if (address(1) == order_.bidder) {
                ...
                require(orderIdSet.removeUintSet(orderId__), "1195");
                delete order[orderId__];
                (platformFee, creatorFee) = delivery_(orderId__, order_);
                emit AcceptEvent(
                    orderId__,
                    order_.type_,
                    order_.token,
    ...
    function delivery_(uint256, OrderInfo memory order_)
        private
        returns (uint256, uint256)
    {
        address platform;
        uint256 platformFee;
        // TODO 不一定所有NFT都有getCreator
        address creator;
        try INFT(order_.token).getCreator(order_.tokenId) returns (
            address creator_
        ) {
            creator = creator_;
        } catch (bytes memory) {}
        if (address(0) == creator) {
            creator = config.getNFTCreator();
        }
        uint256 creatorFee;
    ...
    function acceptOffer(uint256 orderId_) external override {
        ...
        require(orderIdSet.removeUintSet(orderId_), "1205");
        delete order[orderId_];
        (uint256 platformFee, uint256 creatorFee) = delivery_(orderId_, order_);
        emit AcceptEvent(
            orderId_,
