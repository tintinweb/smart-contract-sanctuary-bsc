...
contract SoulMetaTimeBank is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC721HolderUpgradeable{
    ...
    CountersUpgradeable.Counter public timeBankId;
    ...
    TimeBankData public timeBankInfo;
    mapping(uint => StakeData) public stakeInfo;
    mapping(uint8 => Annualized) public annualizedInfo;
    ...
    function initialize(address soulToken_, address timeBankNft_, address timeBankRate_) public initializer{
        __Divestor_init();
    ...
    function setContracts(address soulToken_, address timeBankNft_, address timeBankRate_) public onlyOwner{
        soulToken = ISoulToken(soulToken_);
        timeBankNft = ISoul721(timeBankNft_);
        timeBankRate = ISoulTimeBankRate(timeBankRate_);
    }
    ...
    function deposit(uint8 mode_, uint amount_) public onlyOpen returns (uint){
        ...
        timeBankId.increment();
        uint tokenId = timeBankId.current();
        timeBankNft.mint(_msgSender(), tokenId);
