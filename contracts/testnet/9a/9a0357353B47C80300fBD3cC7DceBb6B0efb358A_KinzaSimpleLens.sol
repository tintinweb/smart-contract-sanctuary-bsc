...
/// @notice Main storage contract for the Kinza system
contract Kinza is Base {
    ...
    string public constant name = "Kinza Protocol";
    ...
    /// @return An address that should be cast to the appropriate module interface, ie IKinzaMarkets(moduleIdToProxy(2))
...
contract EToken is BaseLogic {
    ...
    /// @notice Pool name, ie "Kinza Pool: DAI"
    function name() external view returns (string memory) {
        (address underlying,,,) = CALLER();
        return string(abi.encodePacked("Kinza Pool: ", IERC20(underlying).name()));
    }
    ...
    /// @notice Transfer underlying tokens from sender to the Kinza pool, and increase account's eTokens
    ...
    /// @notice Transfer underlying tokens from Kinza pool to sender, and decrease account's eTokens
    /// @param subAccountId 0 for primary, 1-255 for a sub-account
...
contract Exec is BaseLogic {
    ...
    struct KinzaBatchItemResponse {
        bool success;
        bytes result;
    }

    /// @notice Error containing results of a simulated batch dispatch
    error BatchDispatchSimulation(KinzaBatchItemResponse[] simulation);
    ...
    /// @notice Retrieve Kinza's view of an asset's price
    ...
    /// @notice Retrieve Kinza's view of an asset's price, as well as the current marginal price on uniswap
    /// @param underlying Token address
    /// @return twap Time-weighted average price
    /// @return twapPeriod TWAP duration, either the twapWindow value in AssetConfig, or less if that duration not available
    /// @return currPrice The current marginal price on uniswap3 (informational: not used anywhere in the Kinza protocol)
    ...
    function batchDispatch(KinzaBatchItem[] calldata items, address[] calldata deferLiquidityChecks) external reentrantOK {
    ...
    function batchDispatchSimulate(KinzaBatchItem[] calldata items, address[] calldata deferLiquidityChecks) external reentrantOK {
    ...
    /// @notice Transfer underlying tokens from sender's wallet into the pToken wrapper. Allowance should be set for the kinza address.
    /// @param underlying Token address
    ...
    /// @notice Apply EIP2612 signed permit on a target token from sender to kinza contract
    ...
    /// @notice Apply DAI like (allowed) signed permit on a target token from sender to kinza contract
    /// @param token Token address
    ...
    function doBatchDispatch(KinzaBatchItem[] calldata items, address[] calldata deferLiquidityChecks, bool revertResponse) private {
        address msgSender = unpackTrailingParamMsgSender();
        ...
        KinzaBatchItemResponse[] memory response;
        if (revertResponse) response = new KinzaBatchItemResponse[](items.length);

        for (uint i = 0; i < items.length; ++i) {
            KinzaBatchItem calldata item = items[i];
            ...
            if (revertResponse) {
                KinzaBatchItemResponse memory r = response[i];
                r.success = success;
                r.result = result;
...
contract Markets is BaseLogic {
    ...
    /// @notice Create an Kinza pool and associated EToken and DToken addresses.
    ...
    /// @notice Create a pToken and activate it on Kinza. pTokens are protected wrappers around assets that prevent borrowing.
    /// @param underlying The address of an ERC20-compliant token. There must already be an activated market on Kinza for this underlying, and it must have a non-zero collateral factor.
    ...
    /// @notice Looks up the Kinza-related configuration for a token, and resolves all default-value placeholders to their currently configured values.
    /// @param underlying Token address
    ...
    /// @notice Looks up the Kinza-related configuration for a token, and returns it unresolved (with default-value placeholders)
    /// @param underlying Token address
...
contract PToken {
    address immutable kinza;
    address immutable underlyingToken;

    constructor(address kinza_, address underlying_) {
        kinza = kinza_;
    ...
    /// @notice PToken name, ie "Kinza Protected DAI"
    function name() external view returns (string memory) {
        return string(abi.encodePacked("Kinza Protected ", IERC20(underlyingToken).name()));
    }
    ...
    /// @notice Transfer pTokens from one address to another. The kinza address is automatically granted approval.
    /// @param from This address must've approved the to address
    /// @param recipient Recipient address
    /// @param amount Amount to transfer
    function transferFrom(address from, address recipient, uint amount) public returns (bool) {
        require(balances[from] >= amount, "insufficient balance");
        if (from != msg.sender && msg.sender != kinza && allowances[from][msg.sender] != type(uint).max) {
    ...
    /// @notice Allow spender to access an amount of your pTokens. It is not necessary to approve the kinza address.
    /// @param spender Trusted address
    /// @param amount Use max uint256 for "infinite" allowance
    ...
    // Only callable by the kinza contract:
    function forceUnwrap(address who, uint amount) external {
        require(msg.sender == kinza, "permission denied");
        doUnwrap(who, amount);
    }
...
contract KinzaSimpleLens is Constants {

    bytes32 immutable public moduleGitCommit;
    Kinza immutable public kinza;
    ...
    constructor(bytes32 moduleGitCommit_, address kinza_) {
        moduleGitCommit = moduleGitCommit_;

        kinza = Kinza(kinza_);
        markets = Markets(kinza.moduleIdToProxy(MODULEID__MARKETS));
        exec = Exec(kinza.moduleIdToProxy(MODULEID__EXEC));
    }
    ...
    // approvals
    function getKinzaAccountAllowance(address underlying, address account) external view returns (uint256) {
        return IERC20(underlying).allowance(account, address(kinza));
    }
    ...
    function getTotalSupplyAndDebts(address underlying) public view returns (uint poolSize, uint totalBalances, uint totalBorrows, uint reserveBalance) {
        poolSize = IERC20(underlying).balanceOf(address(kinza));
    ...
    function irmSettings(address underlying) external view returns (ResponseIRM memory r) {
        ...
        address moduleImpl = kinza.moduleIdToImplementation(moduleId);
