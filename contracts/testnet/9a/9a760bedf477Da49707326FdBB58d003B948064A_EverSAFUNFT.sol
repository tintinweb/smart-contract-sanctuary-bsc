...
contract EverSAFUNFT is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    bytes32 public constant RANDOM_TOKEN_MINTER = keccak256("RANDOM_TOKEN_MINTER");

    ChanceMint[] public randomChance;
    uint public ChancesBase;
    ...
    function initialize() public initializer {
        __ERC721_init("EverSAFU NFT", "ESNFT");
        ...
        ChancesBase = 1000;
        randomChance.push(ChanceMint({level: 1, chance: 600}));
        randomChance.push(ChanceMint({level: 2, chance: 200}));
        randomChance.push(ChanceMint({level: 3, chance: 100}));
        randomChance.push(ChanceMint({level: 4, chance: 50}));
        randomChance.push(ChanceMint({level: 5, chance: 30}));
    }
    ...
    function approve(address to, uint256 tokenId) public override {
        if (_tokens[tokenId].stakeFreeze == true) {
            revert("ERC721: Token frozen1");
        }
        super.approve(to, tokenId);
    }

    //BNF-02, SCN-01, SFR-02
    function mint(address to) public onlyRole(RANDOM_TOKEN_MINTER) nonReentrant {
        require(to != address(0), "Address can not be zero");
        _lastTokenId += 1;
        uint tokenId = _lastTokenId;
        _tokens[tokenId].createTimestamp = block.timestamp;
        _tokens[tokenId].name = 'EverSAFU NFT';
        _tokens[tokenId].level = _getRandomLevel();//get random level
    ...
    function _getRandomLevel() private view returns (uint8) {
        ChanceMint[] memory _randomChance = randomChance;
        uint _randomForLevel = _getRandomNFTLevel(1, ChancesBase);
        uint count = 0;
        for (uint i = 0; i < _randomChance.length; i++) {
            count += _randomChance[i].chance;
            if (_randomForLevel <= count) {
                uint8 level = _randomChance[i].level;
                return (level);
            }
        }
    ...
    function _getRandomNFTLevel(uint _min, uint _max) private view returns (uint random) {
