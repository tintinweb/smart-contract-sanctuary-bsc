...
contract HRH is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 50000000 * 10**14;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "King Charles Moon";
    ...
    uint256 public _maxTxAmount = 50000; // 1% of max supply 
    uint256 public numTokensSellToAddToLiquidity = 5000;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 tTransferAmount, uint256 tLiquidity, uint256 tdev) = _getValues(tAmount);
    ...
    function setBuyMarketingFeePercent(uint256 MarketingFee) external onlyOwner() {
        ...
        require(_buyMarketingFee + _buyLiquidityFee <= 10, "Total fees exceed 10%");
    }
    ...
    function setSellMarketingFeePercent(uint256 MarketingFee) external onlyOwner() {
        ...
        require(_sellMarketingFee + _sellLiquidityFee <= 10, "Total fees exceed 10%");
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tLiquidity, uint256 tdev) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount) = _getRValues(tAmount, tLiquidity, tdev, _getRate());
        return (rAmount, rTransferAmount, tTransferAmount, tLiquidity, tdev);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tLiquidity, uint256 tdev, uint256 currentRate) private pure returns (uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rdev = tdev.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rLiquidity).sub(rdev);
    ...
    function _takedev(uint256 tdev) private {
        uint256 currentRate =  _getRate();
        uint256 rdev = tdev.mul(currentRate);
        _rOwned[wallet1] = _rOwned[wallet1].add(rdev.mul(50).div(10**2));
        _rOwned[wallet1] = _rOwned[wallet1].add(rdev.mul(30).div(10**2));
        _rOwned[wallet1] = _rOwned[wallet1].add(rdev.mul(15).div(10**2));
        _rOwned[wallet1] = _rOwned[wallet1].add(rdev.mul(5).div(10**2));
    }
