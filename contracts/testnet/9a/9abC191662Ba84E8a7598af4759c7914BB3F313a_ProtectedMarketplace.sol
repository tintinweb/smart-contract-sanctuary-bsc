...
contract ProtectedMarketplace {
    ...
    event OfferExpired(uint256 offerID, uint256 offerExpiryTime);
    ...
    function acceptOffer(
        ...
        if (block.timestamp > offer.offerExpiration) {
            offer.statusOffer = OfferStatus.Cancelled;
            emit OfferExpired(_offerId, offer.offerExpiration);
            return false;
        }
        require(offer.tokenAddress == _nftAddress && offer.nftTokenId == _tokenId, "Invalid token");
    ...
    function claimDownsideProtectionAmountInOffer(uint256 _offerId) external {
        ...
        if(msg.sender == offer.sellerAddress 
            && block.timestamp > offer.soldTime + offer.protectionTime
            && offer.soldTime != 0)
        {
