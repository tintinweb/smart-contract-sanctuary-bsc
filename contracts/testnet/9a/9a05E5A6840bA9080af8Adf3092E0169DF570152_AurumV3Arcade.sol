...
contract AurumV3Arcade is Context, Ownable {
    ...
    * ---------------------------------------------- */
    ...
    RankingSystemEntity public rankingSystem = RankingSystemEntity({
        BronzeScore: 0,
        SilverScore: 500,
        GoldScore:  1000,
        PlatinumScore:  1500,
    ...
    struct PlayerRankingEntity{    
        ...
        bool isEntity;         // determines if address exists
    }

    // ranking score logic  
    uint public totalArcadePlayers;  
    mapping (uint=> address ) public arcadePlayerAddress;
    ...
    function updateRankingIncreaseLogic(int _rankingIncrement, int _maxRanking, int _BronzeScore,int _SilverScore,int _GoldScore,int _PlatinumScore,int _DiamondScore,int _Predator) external onlyOwner {      
        rankingSystem.RankingIncrement = _rankingIncrement;
        rankingSystem.MaxRanking = _maxRanking;
    ...
    function addPlayerToArcadeRanking(address _account) private {      
        
        if(!playerRankings[_account].isEntity){
            arcadePlayerAddress[totalArcadePlayers] = _account;
            totalArcadePlayers++;
        }
    }
    
    function resetAllPlayerRankStats() external onlyOwner {        
        for (uint i = 0; i < totalArcadePlayers; i++) {
            playerRankings[arcadePlayerAddress[i]].wins = 0;
            playerRankings[arcadePlayerAddress[i]].loses = 0;
            playerRankings[arcadePlayerAddress[i]].rankScore = 0;
            playerRankings[arcadePlayerAddress[i]].rank = PlayerRanks.BRONZE;
        } 
    }
    ...
    function updatePlayerRanking(address _account, GameOutcome _outcome, int _rankScore) private {              
        ...
        if(_outcome == GameOutcome.WIN){
            ...
            if((playerRankings[_account].rankScore + _rankScore)  > rankingSystem.MaxRanking){
                playerRankings[_account].rankScore = rankingSystem.MaxRanking;
            }else{
        ...
        addPlayerToArcadeRanking(_account);
        playerRankings[_account].isEntity = true;
    } 
    ...
    * ---------------------------------------------- */
    ...
    uint public challengeAllowedHours = 6;           // hours allowed for response score
                ...
                if(challange.challenger == account){            
                    ...
                    if(challange.challengeAccepted && (timePassed || challange.challengeFinished)){
                        if(challange.challengeScore > challange.opponentScore && (timePassed || challange.challengeFinished)){
                            ...
                            // get the winner and losers ranks ( +1 as index starts at 0 )
                            uint loserCurrentRank =  uint(challange.opponentRanking) + 1;
                            uint winnerCurrentRank =  uint(challange.challengeRanking) + 1;

                            // get winner and loser ranking
                            int256 winningRankScore = rankingSystem.RankingIncrement * int256(loserCurrentRank / winnerCurrentRank);
                            int256 losingRankScore = winningRankScore / 2;

                            rewardsTotal += (challange.challengeStake * 2);
                            challange.claimed = true;
                            updatePlayerRanking(challange.challenger, GameOutcome.WIN, winningRankScore);
                            updatePlayerRanking(challange.opponent, GameOutcome.LOSE, losingRankScore);
                        }
                    }
                } 
                    ...
                    if(challange.opponentScore > challange.challengeScore){
                        ...
                        // get the winner and losers ranks ( +1 as index starts at 0 )
                        uint loserCurrentRank =  uint(challange.challengeRanking) + 1;
                        uint winnerCurrentRank =  uint(challange.opponentRanking) + 1;
                        ...
                        rewardsTotal += (challange.challengeStake * 2);
                        challange.claimed = true;
                        updatePlayerRanking(challange.opponent, GameOutcome.WIN, winningRankScore);
                        updatePlayerRanking(challange.challenger, GameOutcome.LOSE, losingRankScore);
                    }
                }
            }
        }
