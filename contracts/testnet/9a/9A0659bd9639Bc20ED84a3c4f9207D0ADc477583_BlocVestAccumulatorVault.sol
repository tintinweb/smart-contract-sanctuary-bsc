...
contract BlocVestAccumulatorVault is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external payable nonReentrant {
        ...
        require(user.lastDepositTime + user.nominatedCycle * 1 days < block.timestamp, "cannot deposit before pass nominated days");
        ...
        if(user.amount > 0) {
            ...
            uint256 expireTime = user.lastDepositTime + user.nominatedCycle * 1 days + 1 days;
            if(block.timestamp < expireTime && realAmount >= user.initialAmount) {
                claimable = claimable + claimable * bonusRate / 10000;
            }
        ...
        user.totalStaked = user.totalReward + realAmount;
