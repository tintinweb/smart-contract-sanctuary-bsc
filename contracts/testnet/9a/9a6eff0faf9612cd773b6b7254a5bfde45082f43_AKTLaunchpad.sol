...
// File: contracts/launchpad/AKTLaunchpad.sol

pragma solidity ^0.8.4;

// AKT Launchpad
contract AKTLaunchpad is Ownable, ReentrancyGuard, Pausable {
    // buy event
    ...
    function launchpadBuy(
        ...
        uint256 paymentValue = ILaunchpadProxy(proxy).launchpadBuy{
            value: msg.value
        }(_msgSender(), launchpadId, slotIdx, quantity, additional, data);
    ...
    function launchpadOpenBox(
        ...
        address proxy = launchpadRegistry[proxyId];
        require(proxy != address(0), Errors.PROXY_ID_NOT_EXIST);
        ILaunchpadProxy(proxy).launchpadOpenBox(
            _msgSender(),
            launchpadId,
            slotIdx,
            tokenAddr,
    ...
    function launchpadDoOperation(
        ...
        address proxy = launchpadRegistry[proxyId];
        require(proxy != address(0), Errors.PROXY_ID_NOT_EXIST);
        ILaunchpadProxy(proxy).launchpadDoOperation{value: msg.value}(
            _msgSender(),
            launchpadId,
            slotIdx,
            addrData,
        ...
        )
    {
        ...
        require(proxy != address(0), Errors.PROXY_ID_NOT_EXIST);
        return
            ILaunchpadProxy(proxy).getLaunchpadSlotInfo(
                _msgSender(),
                launchpadId,
                slotIdx
            );
        ...
        )
    {
        ...
        require(proxy != address(0), Errors.PROXY_ID_NOT_EXIST);
        return
            ILaunchpadProxy(proxy).getAccountInfoInLaunchpad(
                _msgSender(),
                launchpadId,
                slotIdx,
                quantity
