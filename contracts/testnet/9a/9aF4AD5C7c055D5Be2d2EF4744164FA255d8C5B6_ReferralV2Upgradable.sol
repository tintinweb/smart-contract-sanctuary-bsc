...
contract ReferralV2Upgradable is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    struct Account {
        ...
        uint totalBusinessBNB;
        uint totalBusinessUSDT;
        uint[] level;
        uint pendingReward;
    }
    ...
    uint private totalPendingRewards;
    uint private minReferredCount;
    uint private maxReferredCount;
    ...
    function AddReferrerAdmin(address payable[] calldata referrer, address[] calldata _referree)
        external
        onlyOwner
        returns (bool)
    {

        for(uint i; i < referrer.length; ++i) {
        ...
        returns (uint256)
    {
        ...
        for (uint i; i < levelRate.length; i++) {
            ...
            if (
                ...
                parentAccount.totalBusinessBNB += value;
                if(parentAccount.referredCount < minReferredCount) {
                    parentAccount.pendingReward += c;
                    totalPendingRewards += c;                   
    ...
    function getPendingReward() external view returns (uint) {
        return accounts[msg.sender].pendingReward;
    }

    function withdrawPendingReward() external returns (bool) {
        require(accounts[msg.sender].referredCount < 3, "You don't have min no of referees.");
        uint totalPendingReward = accounts[msg.sender].pendingReward;
        payable(msg.sender).transfer(totalPendingReward);
        totalRewardDistributed += totalPendingReward;
        return true;
    }
    ...
    function Referral_init() external initializer {
