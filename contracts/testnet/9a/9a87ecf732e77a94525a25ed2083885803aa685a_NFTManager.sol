    ...
    ItemFactoryManager
{
        ...
        ItemFactoryManager(itemFactory_)
    {
        ...
        _supportedNftTypes.add(1); // box presale
        _supportedNftTypes.add(2); // box ingame
        _supportedNftTypes.add(3); // nft
        _supportedNftTypes.add(4); // voucher
    ...
    modifier onlyBoxType(uint256 tokenId_) {
        require(
            _nftToBoxType[tokenId_] == 1 || _nftToBoxType[tokenId_] == 2,
    ...
    function mintBox(address to_, uint256 boxType_)
        ...
        returns (uint256)
    {
        require(boxType_ == 1 || boxType_ == 2, "boxType must be 1 or 2");
        uint256 _tokenId = _mint(to_, boxType_);
    ...
    function unbox(uint256 tokenId_)
        ...
        whenNotLockUnbox
    {
        if (_nftToBoxType[tokenId_] == 1) {
            uint256 _tokenId1 = _mint(_msgSender(), 3);
            (bytes32 _dna, uint256 _artifacts) = nftCore.tokenMetaData(
                _tokenId1
            );
        ...
        uint256 _tokenId2 = _mint(_msgSender(), 3); 
        ...
        override(RandomConsumerBase)
    {
        ...
        if (_nftToBoxType[tokenId_] == 1) {
            // random item
            uint256 _itemId = itemFactory.getRandomItem(1, randomness_);
            itemFactory.decreaseItemSize(1, _itemId);
            _artifacts = _addArtifactValue(_artifacts, 16, 8, _itemId);
            nftCore.updateTokenMetaData(tokenId_, _artifacts, _dna);
            // randome voucher
            uint256 _voucherRandomNumber = itemFactory.getRandomItem(
                4,
                randomness_
            );
            itemFactory.decreaseItemSize(4, _voucherRandomNumber);
            if (_voucherRandomNumber != 1) {
                uint256 _voucherId = _mint(nftCore.ownerOf(tokenId_), 4);
                (bytes32 _dnaVoucher, uint256 _artifactVoucher) = nftCore.tokenMetaData(
                    _voucherId
                );
                _artifactVoucher = _addArtifactValue(
                    _artifactVoucher,
                    16,
                    8,
                    _voucherRandomNumber
                );
                nftCore.updateTokenMetaData(_voucherId, _artifactVoucher, _dnaVoucher);
            }
            // box ingame
        } else if (_nftToBoxType[tokenId_] == 2) {
            // randome item
            uint256 _itemId = itemFactory.getRandomItem(2, randomness_);
            itemFactory.decreaseItemSize(2, _itemId);
            _artifacts = _addArtifactValue(_artifacts, 16, 8, _itemId);
            nftCore.updateTokenMetaData(tokenId_, _artifacts, _dna);
        }
