...
contract TestDAO is Context, IBEP20, Ownable {
    ...
    //auto reduce tax mechanism,only can be reduced can't be added
    //if _totalTax is 15,_nowTaxWeight is 40,then the final _totalTax is 15%*40%=6%
    uint256[5] public _taxesWeight = [80,60,40,20,0];
    uint256 public _nowTaxWeight = 100;
    bool public _enableAutoReduceTax = true;
    ...
    function setAutoReduceTax(bool enabled) external fromOwnerOrDAO {
        _enableAutoReduceTax = enabled;
    }

    function setBNBPoolAddress(address account) external fromOwnerOrDAO {
        require(account != bnbPoolAddress, 'This address was already used');
        bnbPoolAddress = account;
        emit UpdateBNBPoolAddress(account);
    }
 
    function setCoolDownTime(uint256 timeForContract) external fromOwnerOrDAO {
        require(swapCoolDownTime != timeForContract);
        swapCoolDownTime = timeForContract;
        emit UpdatedCoolDowntime(timeForContract);
    }
    function updatePresaleStatus(bool status) external fromOwnerOrDAO {
        presaleEnded = status;
    }
    
    function excludeFromFee(address account) external fromOwnerOrDAO {
        _isExcludedFromFee[account] = true;
        emit ExcludedFromFee(account);
    }
    
    function includeInFee(address account) external fromOwnerOrDAO {
        _isExcludedFromFee[account] = false;
        emit IncludedToFee(account);
    }
    
    function setFees(uint256 bnbFee, uint256 liquidityFee) external fromOwnerOrDAO() {
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external fromOwnerOrDAO() {
        _maxTxAmount = maxTxAmount;
        emit UpdatedMaxTxAmount(maxTxAmount);
    }

    function setTaxes(uint256[5] memory targetTaxes) external fromOwnerOrDAO() {
        ...
        require(_totalTax <= 15,"Total tax can not be greater than 15");

        emit UpdateTaxes(targetTaxes,_totalTax);
    }
    
    function setNumTokensToSwap(uint256 amount) external fromOwnerOrDAO() {
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) external fromOwnerOrDAO {
