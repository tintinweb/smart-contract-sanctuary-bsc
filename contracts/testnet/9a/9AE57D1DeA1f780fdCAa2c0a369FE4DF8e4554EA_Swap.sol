...
contract Swap {
    ...
    function buyTokens(uint256 _busdAmount) public {
        ...
        while(multipler.add(1).mul(1000) <= busdAmount.add(totalBUSDSale)) {
            ...
            tokensPerBUSD = tokensPerBUSD.sub(tokensPerBUSD.mul(1).div(100000));
        }
...
function sellTokens(uint256 _tokenAmount) public {
        ...
        uint256 tokenAmount = _tokenAmount.mul(10**18);
        require(token.balanceOf(msg.sender) >= tokenAmount);
        require(totalBUSDSale > _tokenAmount * tokensPerBUSD, "Liquidity issue");

        while(tokenAmount > 0) {
            if(totalBUSDSale.sub(multipler.mul(1000)) < tokenAmount.div(10**18).div(tokensPerBUSD)){
                uint256 busdAmount = totalBUSDSale.sub(multipler.mul(1000));
                //Do:  Transfer busdAmount.mul(tokensPerBUSD).mul(10**18) tokens
                //Do:  Transfer busdamount to user
                tokenAmount = tokenAmount.sub(busdAmount.mul(tokensPerBUSD).mul(10**18));
                totalBUSDSale = totalBUSDSale.sub(busdAmount);
                multipler = totalBUSDSale.div(1000);
                //Do:  Update tokensPerBUSD
            } else if (totalBUSDSale.sub(multipler.mul(1000)) == 0) {
                
                if (tokenAmount >= 1000 * tokensPerBUSD) {
                    // Transfer 1000 busd and respective tokens
                    // transfer tokens worth 1000 BUSD
                    token.transferFrom(msg.sender, address(this), 1000 * tokensPerBUSD);
                    tokenAmount = tokenAmount.sub(1000 * tokensPerBUSD);
