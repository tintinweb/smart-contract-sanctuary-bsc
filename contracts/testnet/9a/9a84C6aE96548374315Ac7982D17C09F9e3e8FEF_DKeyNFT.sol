...
library Whitelist {
    ...
    function verifySignatureWhenMergeDNFT(
        address _signer,
        string memory _mergeSet,
        uint256 _timestamp,
        string memory _sessionId,
        bytes memory _signature
    ) internal pure returns (bool) {
        bytes32 messageHash = getMessageHashWhenMergeDNFT(_signer, _mergeSet, _timestamp, _sessionId);
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
        address signerAddress = getSignerAddress(ethSignedMessageHash, _signature);
        return signerAddress == _signer;
    }
    ...
    function getMessageHashWhenMergeDNFT(
        ...
        return keccak256(abi.encodePacked(_signer, _mergeSet, _timestamp, _sessionId));
    }
