...
library PYESwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'6cbd54185813805bfdf336e0600bf3c272e510df5d084bda83e7793458f3f22b' // init code hash
            ))));
    }
    ...
    function _calculateFees(address factory, address input, address output, uint amountIn, uint amount0Out, uint amount1Out, bool isExcluded) internal view virtual returns (uint amount0Fee, uint amount1Fee) {
        IPYESwapPair pair = IPYESwapPair(pairFor(factory, input, output));
        (address token0,) = sortTokens(input, output);
        address baseToken = pair.baseToken();
        uint totalFee = pair.getTotalFee();
        amount0Fee = baseToken != token0 || isExcluded ? uint(0) : input == token0 ? amountIn.mul(totalFee).div(10**4) : amount0Out.mul(totalFee).div(10**4);
        amount1Fee = baseToken == token0 || isExcluded ? uint(0) : input != token0 ? amountIn.mul(totalFee).div(10**4) : amount1Out.mul(totalFee).div(10**4);
    }
