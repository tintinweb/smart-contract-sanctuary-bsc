...
contract VotingEscrow is IVotingEscrow, ApolloxERC721EnumerableUpgradeable, UUPSUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    function initialize(IERC20 lockToken, ISvgBuilderClient svgBuilderClient) public initializer {
    ...
    function increaseAmount(uint tokenId, uint addAmount) external whenNotPaused nonReentrant oncePerBlock(msg.sender){
    ...
    function increaseUnlockTime(uint tokenId, uint lockDuration) external whenNotPaused nonReentrant oncePerBlock(msg.sender){
        require(lockDuration > 0 && lockDuration<=MAX_TIME, 'Invalid duration');
        require(ownerOf(tokenId) == msg.sender, "ERC721: transfer caller is not owner");
    ...
    function _depositFor(
        ...
        if (lockAmount > 0) {
            assert(_lockToken.transferFrom(from, address(this), lockAmount));
        }
    ...
    function withdraw(uint tokenId) external whenNotPaused nonReentrant oncePerBlock(msg.sender){
        ...
        assert(_lockToken.transfer(msg.sender, value));
        // Burn the NFT
    ...
    function findBlockEpoch(uint blockNo, uint maxEpoch) public view returns (uint) {
        // Binary search
        uint min = 0;
        uint max = maxEpoch;
        for (uint i = 0; i < 128; ++i) {
