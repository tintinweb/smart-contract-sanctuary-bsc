...
contract MarketZai is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) public seasonOfTicket;

    mapping(uint256 => bool) public isMasterTicket;
    mapping(uint256 => uint256) public masterTicketMintBySeason;
    ...
    function preMintZai() external onlyOwner {
        ...
        address levelStorage = gameAddresses.getLevelStorageAddress();
        for (uint256 i; i < 10; ) {
            I.mintZai(levelStorage, "Challenger", 0);
            unchecked {
                ++i;
            }
        }
    ...
    function createTickets(uint256 _quantity) external onlyOwner {
        uint256 _toAdd = (block.timestamp - lastOwnerTicketsCreation) / 1 days;
        ...
        for (uint256 i = 0; i < _quantity; ) {
            _mintTicket(claimNFTsAddress, _currentSeason);
            unchecked {
                ++i;
            }
        }
    ...
    function mintMasterTicket(uint256 _quantity) external onlyOwner {
        uint256 _currentSeason = IipfsIdStorage(
            gameAddresses.getIpfsStorageAddress()
        ).getCurrentSeason();

        require(
            masterTicketMintBySeason[_currentSeason] + _quantity <= 10,
            "Only 10 master tickets by season"
        );
        masterTicketMintBySeason[_currentSeason] += _quantity;
        address claimNFTsAddress = gameAddresses.getClaimNFTsAddress();

        for (uint256 i = 0; i < _quantity; ) {
            uint256 _ticketId = _mintTicket(claimNFTsAddress, _currentSeason);
            isMasterTicket[_ticketId] = true;
            unchecked {
                ++i;
            }
        }
    ...
    function buyBronzeZai(string memory _name, uint256 _maxPrice)
        external
        returns (uint256)
    {
    ...
    function buySilverZai(string memory _name, uint256 _maxPrice)
        external
        returns (uint256)
    {
    ...
    function buyGoldZai(string memory _name, uint256 _maxPrice)
        external
        returns (uint256)
    {
    ...
    function buyPlatinumZai(string memory _name, uint256 _maxPrice)
        external
        returns (uint256)
    {
    ...
    function _buyZai(
        ...
        string memory _name,
        address _user
    ) internal returns (uint256 zaiId) {
        ...
        returns (uint256)
    {
        ...
        uint256 _pastBlockInCurrentDay = _pastBlock -
            (_nbOfDaysPast * blockPerDay);
    ...
    function _mintTicket(address _address, uint256 _season)
        internal
        returns (uint256)
    {
        _tokenIds.increment();
        ...
        seasonOfTicket[_newItemId] = _season;
        ...
        returns (uint256)
    {
        ...
        _burn(_ticketID);
        if (isMasterTicket[_ticketID]) {
            uint256 zaiId = _generateRandomZai(_name, msg.sender, 3);
            emit ZaiBorned(3, zaiId, msg.sender);
            return zaiId;
        } else {
    ...
    function _randomMint(string memory _name) internal returns (uint256 zaiId) {
        bytes32 _id = keccak256(
            abi.encodePacked(msg.sender, block.timestamp, nonce, _name)
        );
        nonce = uint256(_id);
        ...
        if (_random > 150 && _random <= 800) {
            // 800 - 150 = 650 => 6,5%
            _state = 2;
        }
        if (_random > 800 && _random <= 3000) {
            // 3000 - 800 = 2200 => 22%
            _state = 1;
        }
        if (_random > 3000) {
