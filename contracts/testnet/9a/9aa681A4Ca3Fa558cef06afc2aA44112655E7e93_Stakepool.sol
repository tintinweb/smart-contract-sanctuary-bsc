...
/**
        ...
        Part 1:
            1- Review stakepool and nft contract
            2- Write tasks for nft contract
            3- Write test cases for both contracts
            4- Test both contracts

        Plan:

        - Update tier struct to include tier nft address, name, value
        - Update admin functions
        - Update mapping to create keep of nft owners
        - Update stakeNft() and withdrawNft() to get value from tier array
        - Add find collection funtion to find nft collection 
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(address _stakeToken) external initializer {
        ...
        // createValue(15, 1000 ether);
        // createValue(40, 2000 ether);
        // createValue(130, 5000 ether);
        // createValue(300, 10000 ether);
        // createValue(750, 20000 ether);
        ...
        whenNotPaused
    {
        ...
        require(_tierIndex < tiers.length, "!tierIndex");
        ...
        nftOwners[_collection][_tokenId] = _msgSender();
        ...
        whenNotPaused
    {
        ...
        require(nftOwners[_collection][_tokenId] == msg.sender, "!Owner");
        uint256 _weight = _tier.weight;

        //calculate the withraw fee of the staker and store it in memory
        uint256 _fee = calculateWithdrawFees(_tier.stake, msg.sender);
    ...
    function checkCollection(address _collection)
        private
        view
        returns (bool _exists, uint256 _tierIndex)
    {
        DataTypes.Tier[] memory _tiers = tiers;
        uint256 _length = _tiers.length;
        for (uint256 i; i < _length; i++) {
            if (_tiers[i].collection == _collection) {
                (_exists, _tierIndex) = (true, i);
                break;
            }
        }
    }
