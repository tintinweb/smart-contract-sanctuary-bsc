...
abstract contract ERC20 is IERC20 {
  ...
  uint256 public maxSupply = 10000000000000000; // 10 Million
  ...
  function balanceOf(address account) public view virtual override returns (uint256) {
    ...
    function _mint(address account_, uint256 amount_) internal virtual {
        ...
        require(_totalSupply + amount_ <= maxSupply, "Total supply can not exceed 1 million");
...
contract OnlyToken is ERC20Permit, VaultOwned {
    ...
    constructor() ERC20("OXLYT", "Oxly Token", 9) {}

    function setTax(uint256 _tax) external onlyOwner returns (bool) {
        require(_tax <= 25, "error: sell tax fee exceeds maximum value");
        tax = _tax;
        return true;
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if (lpAddress[recipient] == true && tax > 0) {
            tAmount = amount.mul(tax).div(100);
