...
contract LIFEGAMES is IERC20 {
    ...
    function _finalizeTransfer(address from, address to, uint256 amount, bool takeFee) internal returns (bool) {
        ...
        if (autoAddLiquidity) {
            ...
            uint256 estimatedTokensForAutoLiquidity = getOutEstimatedTokensForTokens(address(this), busdAddress, estimatedBusdForAutoLiquidity);
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        if (amountToBuyBackAndBurn > 0) {
            removeAllFee();
            _tokenTransferNoFee(address(this), buybackAndBurnAddress, amountToBuyBackAndBurn);
            restoreAllFee();
        }
    }
