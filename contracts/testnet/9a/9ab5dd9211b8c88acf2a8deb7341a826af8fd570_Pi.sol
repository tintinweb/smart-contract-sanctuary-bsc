...
contract Pi is ERC20, Ownable {
    ...
    uint256 public AmountRewardsFee = 0;
    ...
    function setBuyMarketFee(uint256 amount) public onlyOwner {
        buyMarketFee = amount;
    }

    function setSellMarketFee(uint256 amount) public onlyOwner {
        sellMarketFee = amount;
    }

    function setbuyRewardsFee(uint256 amount) public onlyOwner {
        buyRewardsFee = amount;
    }

    function setsellRewardsFee(uint256 amount) public onlyOwner {
        sellRewardsFee = amount;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner() && !tradeEnabled) {
        ...
        if (
            ...
            swapAndSendDividends(AmountRewardsFee);
            swapping = false;
        }
        ...
        if (takeFee) {
            ...
            if (MFee > 0) super._transfer(from, market, MFee);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        AmountRewardsFee = AmountRewardsFee - tokens;
...
contract dDividendTracker is Ownable, DividendPayingToken {
    ...
    {
        claimWait = 100;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold tokens for dividen
    }
