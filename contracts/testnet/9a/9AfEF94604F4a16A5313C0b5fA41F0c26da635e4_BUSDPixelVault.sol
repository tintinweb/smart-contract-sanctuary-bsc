...
contract BUSDPixelVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint256 public constant PIXEL_BUSD_PID = 0;
    //
    //    // TESTNET
    ...
    function initialize(address _pixel, address _busd, address _router, address _factory, address _pixelStakingManager, address _vaultReferralTreasury) public initializer {
    ...
    */
    ...
    function withdraw(uint256 amount, bool isReceiveBusd) external updateReward(msg.sender) nonReentrant noCallFromContract {
        require(balanceOf(msg.sender) >= amount, "INSUFFICIENT_AMOUNT");
        //calculate LP needed that corresponding with amount
        uint256 lpAmountNeeded = getLPTokenByAmount1(amount);
        require(lpAmountNeeded <= userInfo[msg.sender].amount, "INSUFFICIENT_LP");
    ...
    function depositStakingPool(uint256 amount) internal {
        pixelStakingManager.deposit(PIXEL_BUSD_PID, amount, address(vaultReferralTreasury));
    }
