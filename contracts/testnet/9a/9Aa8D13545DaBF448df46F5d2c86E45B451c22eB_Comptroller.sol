...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterfaceG2, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when liquidator contract is changed
     ...
     */
    function enterMarkets(address[] calldata eTokens) external override returns (uint[] memory) {
        uint len = eTokens.length;

        uint[] memory results = new uint[](len);
        for (uint i = 0; i < len; i++) {
            results[i] = uint(addToMarketInternal(EToken(eTokens[i]), msg.sender));
        }
    ...
    /**
     * @notice Add the market to the borrower's "assets in" for liquidity calculations for call from EToken contract only
     * @param eToken The market to enter
     * @param user The address of the account to modify
     * @return Success indicator for whether the market was entered
     */
    function addToMarketByEToken(address eToken, address user) external override returns (uint) {
        
        // fail if the function is not called by EToken contract
        Market storage marketToJoin = markets[address(msg.sender)];
        if (!marketToJoin.isListed) {
            // market is not listed, cannot join
            return fail(Error.MARKET_NOT_LISTED, FailureInfo.ADD_TO_MARKET_REJECTION);
        }

        uint err = uint(addToMarketInternal(EToken(eToken), user));
        if (err != 0) {
            return fail(Error.MARKET_NOT_LISTED, FailureInfo.ADD_TO_MARKET_REJECTION);
        }
    ...
    /**
     * @notice Add the market to the borrower's "assets in" for liquidity calculations (internal call)
     * @param eToken The market to enter
     ...
     */
    function mintAllowed(address eToken, address minter, uint mintAmount) external view onlyProtocolAllowed override returns (uint) {
     ...
     */
    function redeemAllowed(address eToken, address redeemer, uint redeemTokens) external view override onlyProtocolAllowed returns (uint) {
    ...
    function repayBorrowAllowed(
        ...
        address payer,
        address borrower,
        uint repayAmount) external view override onlyProtocolAllowed returns (uint) {
        // Shh - currently unused
        payer;
        borrower;
        repayAmount;
    ...
    function liquidateBorrowAllowed(
        ...
        borrowBalance = EToken(eTokenBorrowed).borrowBalanceStored(borrower);
    ...
    function seizeAllowed(
        ...
        address liquidator,
        address borrower,
        uint seizeTokens) external view override onlyProtocolAllowed returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        if (EToken(eTokenCollateral).comptroller() != EToken(eTokenBorrowed).comptroller()) {
     ...
     */
    function transferAllowed(address eToken, address src, address dst, uint transferTokens) external view override onlyProtocolAllowed returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
     ...
     */
    function liquidateCalculateSeizeTokens(address eTokenBorrowed, address eTokenCollateral, uint actualRepayAmount) external view override returns (uint, uint) {
    ...
    function getLiquidatorContract() external view override returns (address) {
        return liquidatorContract;
    }
    ...
    /**
      * @notice Sets a new liquidator contract for the comptroller
      * @dev Admin function to set a new liquidator contract
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function _setLiquidatorContract(address newLiquidatorContract) public returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_LIQUIDATOR_CONTRACT_OWNER_CHECK);
        }

        // Track the old liquidator contract for the comptroller
        address oldLiquidatorContract = liquidatorContract;

        // Set comptroller's liquidator contract to newLiquidatorContract
        liquidatorContract = newLiquidatorContract;

        // Emit NewLiquidatorContract(oldLiquidatorContract, newLiquidatorContract)
        emit NewLiquidatorContract(oldLiquidatorContract, newLiquidatorContract);
