...
contract TEST is Context, IERC20, Ownable {
    ...
    address public teamAddress = 0x76d6cD93D48808b2C493049531D09FFe473932d4;
    address public marketingAddress = 0x30E0dcCb2ca2288f4Ce8E72aE23a28C3dA5D01B7;
   ...
   function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
    ...
    function setFeeRates(uint256 _rfi, uint256 _marketing, uint256 _team, uint256 burn, uint256 _liquidity, uint256 _buyback) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _marketing, uint256 _team, uint256 burn, uint256 _liquidity, uint256 _buyback) external onlyOwner{
        sellFeeRates.rfi = _rfi;
        sellFeeRates.marketing = _marketing;
        sellFeeRates.team = _team;
        sellFeeRates.burn = burn;
    ...
    function _takeTeam(uint256 rTeam, uint256 tTeam) private {
        ...
        {
            _tOwned[teamAddress]+=tTeam;
        }
        _rOwned[teamAddress] +=rTeam;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rMarketing, to_return.rTeam, to_return.rTeam, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tTeam-s.tBurn-s.tLiquidity-s.tBuyback;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rMarketing,uint256 rTeam, uint256 rBurn, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rTeam-rBurn-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rTeam,rBurn,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        emit Transfer(sender, teamAddress, s.tTeam);
