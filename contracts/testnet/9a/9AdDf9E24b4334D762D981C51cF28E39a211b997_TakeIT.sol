...
contract TakeIT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable {
    ...
    event PurchasedNFT(address newOwner, uint256 tokenId, address purchaseToken, uint256 purchaseAmount);
    event Distribute(address indexed token, address indexed to, uint256 amount, uint256 tokenId);
    event SetRate(uint256 tokenId, uint256 rate);
    ...
    function initialize(address _purchaseToken) initializer public {
    ...
    function purchase(address to, uint256 amount) public {
        ...
        purchaseToken.transferFrom(msg.sender, address(this), amount);
        ...
        emit PurchasedNFT(to, tokenId, address(purchaseToken), amount);
    }
    ...
    function distribute(address to, uint256 tokenId) external onlyOwner {
        ...
        purchaseToken.transfer(to, purchaseRecords[tokenId].amount);
    ...
    function setRate(uint256 tokenId, uint256 rate) external onlyAllowedUpdaters {
        purchaseRecords[tokenId].rate = rate;

        emit SetRate(tokenId, rate);
    }
