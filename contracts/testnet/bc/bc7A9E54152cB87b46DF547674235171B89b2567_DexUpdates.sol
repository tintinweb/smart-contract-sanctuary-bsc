...
contract DexUpdates is Ownable, DexStorage, SecondDexStorage {
 ...
 function init() public {
     require(admin == msg.sender, "not aucthorize");
     Xtoken = IERC20(0x2c1Bc69330a1fb930660bdac451C795691e58b5A);
 }
 function calculatePrice(uint256 _price, uint256 base, uint256 currencyType, uint256 tokenId, address seller, address nft_a) public view returns(uint256 price) {
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId, uint256 currencyType) isValid(nft_a) public{
        ...
        require(currencyType == 3 || (_allowedCurrencies[tokenId][nft_a][currencyType] && currencyType != 2), "123");
        require(msg.sender != nonCryptoNFTVault || currencyType == 3, "124" );
        uint256 price = calculatePrice(temp.price, temp.currencyType, currencyType == 3 ? 1: currencyType, tokenId, temp.seller, nft_a);
        ...
        if(temp.seller == nonCryptoNFTVault) {
          if(currencyType == 3){
            ...
            //   uint256 tempPrice = calculatePrice((price / 1000) * mainPerecentage, currencyType, 1, tokenId, temp.seller, nft_a);
            _nonCryptoWallet[_nonCryptoOwners[tokenId][nft_a]].BUSD += calculatePrice((price / 1000) * mainPerecentage, currencyType, 1, tokenId, temp.seller, nft_a);
