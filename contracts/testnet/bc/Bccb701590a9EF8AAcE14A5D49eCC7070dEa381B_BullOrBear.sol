...
contract BullOrBear is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "A1: Not admin");
        _;
    }

    modifier onlyAdminOrOperator() {
        require(msg.sender == admin || msg.sender == operator, "A2: Not operator/admin");
        _;
    }

    modifier onlyOperator() {
        require(msg.sender == operator, "A3: Not operator");
        _;
    }

    modifier notContract() {
        require(!_isContract(msg.sender), "A4: Contract not allowed");
        require(msg.sender == tx.origin, "A5: Proxy contract not allowed");
        _;
    }
    ...
    constructor(
        ...
        require(_treasuryFee <= MAX_TREASURY_FEE, "STF1: Treasury fee too high");
        require(_cancelFee <= MAX_CANCEL_FEE, "SCF1: Cancel fee too high");
        ...
        require(_checkBetLevelData(_betLvls, _maxBetLvls), "BLD1: Invalid bet level data");
        BET_LVLs = _betLvls;
    ...
    function betBull(uint256 epoch, uint256 amount) external whenNotPaused nonReentrant notContract {
        ...
        require(
            ...
            "BBU1: Bet amount exceeded max bet amount"
        );

        require(epoch == currentEpoch, "BBU2: Bet is too early/late");
        require(_bettable(epoch), "BBU3: Round not bettable");
        require(amount >= minBetAmount, "BBU4: Bet amount must be greater than minBetAmount");
    ...
    function betBear(uint256 epoch, uint256 amount) external payable whenNotPaused nonReentrant notContract {
        ...
        require(
            amount <= getMaxBetAmount(users[msg.sender].betCounter - 1),
            "BBE1: Bet amount exceeded max bet amount"
        );

        require(epoch == currentEpoch, "BBE2: Bet is too early/late");
        require(_bettable(epoch), "BBE3: Round not bettable");
        require(amount >= minBetAmount, "BBE4: Bet amount must be greater than minBetAmount");
    ...
    function cancelPosition(uint256 epoch, Position position) external whenNotPaused nonReentrant notContract {
        require(epoch == currentEpoch, "CP1: Can only cancel the position within the current round");
        require(block.timestamp < rounds[epoch].lockTimestamp, "CP2: Can only cancel position in bet phase");
        BetInfo storage betInfo = ledger[epoch][msg.sender];
        uint256 cancelAmount;
        if (position == Position.Bear) {
            require(betInfo.bearAmount > 0, "CP3: Not bet yet");
            ...
            require(betInfo.bullAmount > 0, "CP4: Not bet yet");
            cancelAmount = betInfo.bearAmount;
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            require(rounds[epochs[i]].startTimestamp != 0, "CLM1: Round has not started");
            require(block.timestamp > rounds[epochs[i]].closeTimestamp, "CLM2: Round has not ended");
            ...
            if (rounds[epochs[i]].oracleCalled) {
                ...
                require(bearClaimable || bullClaimable, "CLM3: Not eligible for claim");
            ...
            // Round invalid, refund bet amount
                ...
                require(refundable(epochs[i], msg.sender), "CLM4: Not eligible for refund");
    ...
    function genesisStartRound() external whenNotPaused onlyOperator {
        require(!genesisStartOnce, "GSR1: Can only run genesisStartRound once");
    ...
    function executeRound() external whenNotPaused onlyOperator {
        require(genesisStartOnce, "ER1: Can only run after genesisStartRound is triggered");
    ...
    function claimTreasury(address to, uint256 amount) external onlyAdmin {
        require(amount <= treasuryAmount, "CT1: Amount exceeded treasury amount");
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        require(_token != address(token), "RCT1: Cannot be prediction token address");
    ...
    function setAdmin(address _adminAddress) external onlyOwner {
        require(_adminAddress != address(0), "SAD1: Cannot be zero address");
        admin = _adminAddress;
    ...
    function setOperator(address _operatorAddress) external onlyAdmin {
        require(_operatorAddress != address(0), "SOP1: Cannot be zero address");
    ...
    function setMinBetAmount(uint256 _minBetAmount) external whenPaused onlyAdmin {
        require(_minBetAmount != 0, "SMB1: Must be superior to 0");
        minBetAmount = _minBetAmount;
    ...
    function setBetAndLockAndBuffer(
        ...
        require(_bufferSeconds < _betSeconds + _lockSeconds, "SBLB1: bufferSeconds must be inferior to phase seconds");
    ...
    function setBetLevelData(uint256[] memory _betLvls, uint256[] memory _maxBetLvls) external whenPaused onlyAdmin {
        require(_checkBetLevelData(_betLvls, _maxBetLvls), "BLD1: Invalid bet lvl data");
        BET_LVLs = _betLvls;
        MAX_BET_LVLs = _maxBetLvls;
    ...
    function setOracle(address _oracle) external whenPaused onlyAdmin {
        require(_oracle != address(0), "SOR1: Cannot be zero address");
    ...
    function _safeStartRound(
        ...
        require(genesisStartOnce, "SSR1: Can only run after genesisStartRound is triggered");
        require(rounds[epoch - 1].closeTimestamp != 0, "SSR2: Can only start round after round n-1 has ended");
        require(
            block.timestamp >= rounds[epoch - 1].closeTimestamp,
            "SSR3: Can only start new round after round n-1 closeTimestamp"
        );
    ...
    function _calculateRewards(uint256 epoch) internal {
        require(rounds[epoch].rewardBaseCalAmount == 0 && rounds[epoch].rewardAmount == 0, "CR1: Rewards calculated");
        RoundInfo storage round = rounds[epoch];
        uint256 rewardBaseCalAmount;
        uint256 treasuryAmt;
        uint256 rewardAmount;
    ...
    function _safeEndRound(
        ...
        require(block.timestamp >= rounds[epoch].closeTimestamp, "SER1: Can only end round after closeTimestamp");
        require(
            block.timestamp <= rounds[epoch].closeTimestamp + bufferSeconds,
            "SER2: Can only end round within bufferSeconds"
        );
        RoundInfo storage round = rounds[epoch];
    ...
    function _getPriceFromOracle() internal view returns (uint80, int256) {
        ...
        require(timestamp <= leastAllowedTimestamp, "ORC1: Oracle update exceeded max timestamp allowance");
        require(
            uint256(roundId) > oracleLatestRoundId,
            "ORC2: Oracle update roundId must be larger than oracleLatestRoundId"
        );
