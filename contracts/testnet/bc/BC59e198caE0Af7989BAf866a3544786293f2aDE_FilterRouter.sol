...
library FilterLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex"f7b3dda8779c60bedbe4c501c6ed96361f76ea6b70e36ef5b6901e510129bd60" // init code hash
            )))));
    }
...
contract FilterRouter is IFilterRouter {
    ...
    mapping(address => mapping(address => uint)) public liquidityUnlockTimes; //stores liquidity unlock time for liquidity provider to withdraw liquidity

    mapping(address => mapping(address => uint)) public tokenPriceImpacts; //stores user"s price impact per token
    mapping(address => mapping(address => bool)) public sellingProhibited;
    mapping(address => mapping(address => uint)) public sellingPermittedTimes;

    mapping(address => bool) public isVerifiedSafe; //checks if token is deemed safe

    uint public minLiquidityLockTime = 86400; //anyone who becomes a liquidity provider must lock liquidity for a min period of 1 day before being allowed to withdraw
    uint public maxNegativePriceImpact = 10; //maximum allowable negative price impact for a single TX, in percent
    uint public priceImpactCooldownTime = 300; //if someone has sold up to maxNegativePriceImpact percentage, they must wait priceImpactCooldownTime secs before they are allowed to sell again

    // IN LP PAIR TOKEN, block transfer if liquidityUnlockTime is not reached yet

    // DO VERIFIED PAIR CREATION CHECK - if not checker, someone can do whatever with LP contract and rug maybe
    ...
    function addLiquidity(
        ...
        imposeLiquidityLock(msg.sender, pair, liquidityLockTime);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            "FilterRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
    ...
    function setDeployerAddress(address _deployerAddr) public {
