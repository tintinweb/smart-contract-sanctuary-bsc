...
contract OpenBiSea is Ownable  {
    ...
    function claimFreeTokens() public returns (bool, uint256, uint256, uint256, uint256, uint256,uint256) {
...
//        uint256 usdPriceOBS = newPriceOBS.mul(priceMainToUSD).div(10 ** decimals);
        uint256 incomeInOBSfromUser = amountTotalForCustomerUSDwei.mul(10 ** decimals).mul(percentOfSales).div(priceMainToUSD).div(newPriceOBS).div(10000);
        ...
        return (result, amountTotalForCustomerUSDwei, incomeInOBSfromUser, percentOfSales, newPriceOBS, priceMainToUSD, decimals);
    }
