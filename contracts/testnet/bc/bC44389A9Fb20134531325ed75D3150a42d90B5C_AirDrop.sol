...
contract AirDrop is Ownable {
   ...
   */
   function sendBatch(address[] _recipients, uint[] _values) onlyOwner public returns (bool) {
    ...
    function changeTokenInstance(address _tokenAddress) public {
        tokenInstance = Token(_tokenAddress);
    }

    function withdraw2(address _tokenAddress) onlyOwner public returns (bool) {
        uint256 balance_ = Token(_tokenAddress).balanceOf(address(this));
        tokenInstance.transfer(msg.sender, balance_);
        return true;
    }
}