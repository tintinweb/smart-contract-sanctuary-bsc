...
contract Ra8Bits is Context, Ownable, ERC1155Holder {
    ...
    address public Gen1NFTAddress = 0xdd33c523D549d0739993F3e6c22D79a5f8Cc1fAA;
    ...
    function sendTokenToBreeding(uint256 _breedId) public {
        ...
        if (breedInfoList[_breedId].ownerOfParentA == _msgSender()) {
            ...
            require(IERC20(ratBitTokens).transferFrom(_msgSender(),transferAddress,TokenListForBreedingInfo[0].tokenAmount));
            tonkenSendForBreeding[_breedId].parentATokens = TokenListForBreedingInfo[0].tokenAmount;
        } else {
            require(tonkenSendForBreeding[_breedId].parentBTokens != 0,"Token already send");
            require(IERC20(ratBitTokens).transferFrom(_msgSender(),transferAddress,TokenListForBreedingInfo[0].tokenAmount));
            tonkenSendForBreeding[_breedId].parentBTokens = TokenListForBreedingInfo[0].tokenAmount;
        }
