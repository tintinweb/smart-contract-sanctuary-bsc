...
contract DexGoNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ERC721Burnable {
    ...
    function approveMainContract(address to, uint256 tokenId) public {
        require(msg.sender == IDexGoStorage(storageContract).getDexGo(), "MC");
        _approve(to, tokenId);
    }
    ...
    uint8[] players = [SHOES0, SHOES1, SHOES2, SHOES3, SHOES4, SHOES5, SHOES6, SHOES7, SHOES8, SHOES9];
    ...
    constructor(uint256 networkId, address _storageContract) ERC721("dexGoNFT", "DEXGO") {
        storageContract = _storageContract;
        ...
        _tokenIdCounter.increment(); // for some arrays we were save tokenIDs which can't be 0 for compare
    }
    ...
    function _increasePrice(uint8 typeNft) private {
        uint256 currentPrice = IDexGoStorage(storageContract).getPriceForType(typeNft);
        IDexGoStorage(storageContract).setPriceForType(currentPrice.add(currentPrice.div(increaseValue)),typeNft);
    }

    function mint(address to, string memory uri, uint8 typeNft) public {
        require(typeNft == PATH || typeNft == MOVIE, "EOL");
    ...
    function _safeMintType(address to, string memory uri, uint8 typeNft, bool needURI) private {
        ...
        IDexGoStorage(storageContract).setTypeForId(tokenId,typeNft);
        _increasePrice(typeNft);
        IDexGoStorage(storageContract).setKmForId(tokenId, IDexGoStorage(storageContract).getPriceInitialForType(typeNft));
    }
    ...
    function convertFromPastContracts(address pastContract, uint[] memory tokenIds) public {
        bool isContractInPast = false;

        for(uint256 x=0;x<IDexGoStorage(storageContract).getPastContracts().length;x++) {
           if (IDexGoStorage(storageContract).getPastContracts()[x] == pastContract) {
                isContractInPast = true;
               break;
           }
        }
        require(isContractInPast == true, "WC");
        for(uint256 x=0;x<tokenIds.length;x++) {
            require(IERC721(pastContract).ownerOf(tokenIds[x]) == msg.sender, "WO");
            IDexGoNFT(pastContract).burn(tokenIds[x]);
            _safeMintType(msg.sender, "", IDexGoStorage(storageContract).getTypeForId(tokenIds[x]), false);
        }
    }
    ...
    event Purchase(address indexed from, uint256 value, uint8 typeNft, uint8 finalTypeNft, address referrer, uint valueForOwner, bool isUSDT);
...
//        if (!isIOS) {
//            (friends, friendsCount) = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).getHandshakes(sender);
//            // invitation bonus
//            uint valueForFriend;
//            if (friends[0] != address(0)) {
//                valueForFriend = value * IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).getPercentPerInvitationBonusWei() / 1 ether;
...
//                    valueForFriend = value * IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).getPercentPerLevelWei(x) / 1 ether;
...
//        require(msg.sender == IDexGoStorage(storageContract).getRentAndKm() || msg.sender == storageContract || msg.sender == IDexGoStorage(storageContract).getDexGo(), "ORC");
//        _distributeMoney(sender, value, false, false);
//    }

    function _purchaseFinalize(address sender, address referrer) private {
        if (IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).getFullList(sender) == 0 && isIGO) IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).setHandshake(sender, referrer);
        IDexGoStorage(storageContract).setLatestPurchaseTime(sender, block.timestamp);
    }

    function purchaseUSDT(uint8 typeNft, address referrer, string memory _inAppPurchaseInfo) public {
        require(IERC20(IDexGoStorage(storageContract).getUSDT()).allowance(msg.sender, IDexGoStorage(storageContract).getHandshakeLevels()) >= IDexGoStorage(storageContract).getPriceForType(typeNft), "WV");
        require(IDexGoStorage(storageContract).getHandshakeLevels() != address (0), "WHL");
        require(typeNft <= 10, "WT");
...
//        if (IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).getFullList(msg.sender) == 0 && isIGO) IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).setHandshake(msg.sender, referrer);

        uint valueForOwner = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, IDexGoStorage(storageContract).getPriceForType(finalTypeNft), bytes(_inAppPurchaseInfo).length > 0, true);
//        IDexGoStorage(storageContract).setLatestPurchaseTime(msg.sender, block.timestamp);
        IDexGoStorage(storageContract).setInAppPurchaseData(_inAppPurchaseInfo, _tokenIdCounter.current());
        _safeMintType(msg.sender, "", finalTypeNft, false);
        emit Purchase(msg.sender, IDexGoStorage(storageContract).getPriceForType(finalTypeNft), typeNft, finalTypeNft, referrer, valueForOwner, true);
        _purchaseFinalize(msg.sender, referrer);
    }
    function purchaseBatchUSDT(uint8[] memory typesNft, address referrer, string[] memory _inAppPurchaseInfo) public {
        uint value;
        for(uint256 y=0;y<typesNft.length;y++) {
            ...
            value = value + IDexGoStorage(storageContract).getPriceForType(finalTypeNft);
        }
        require(IERC20(IDexGoStorage(storageContract).getUSDT()).allowance(msg.sender, IDexGoStorage(storageContract).getHandshakeLevels()) >= value, "WV");

//        if (IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).getFullList(msg.sender) == 0 && isIGO) IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).setHandshake(msg.sender, referrer);

        uint valueForOwner = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, value, bytes(_inAppPurchaseInfo[0]).length > 0, true);
        for(uint256 y=0;y<typesNft.length;y++) {
            uint8 finalTypeNft = typesNft[y];
            if (typesNft[y] == MAGIC_BOX) {
                finalTypeNft = uint8(random());
            ...
            _safeMintType(msg.sender, "", finalTypeNft, false);
            IDexGoStorage(storageContract).setInAppPurchaseData(_inAppPurchaseInfo[y], _tokenIdCounter.current());
            emit Purchase(msg.sender, value,  typesNft[y], finalTypeNft, referrer, valueForOwner, true);
        }
//        IDexGoStorage(storageContract).setLatestPurchaseTime(msg.sender, block.timestamp);
        _purchaseFinalize(msg.sender, referrer);
    }
    ...
    function purchase(uint8 typeNft, address referrer, string memory _inAppPurchaseInfo) payable public {
        require(msg.value > IDexGoStorage(storageContract).valueInMainCoin(typeNft).sub(IDexGoStorage(storageContract).getValueDecrease()), "WV");
        require(IDexGoStorage(storageContract).getHandshakeLevels() != address (0), "WHL");
        require(typeNft <= 10, "WT");
...
//        if (IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).getFullList(msg.sender) == 0 && isIGO) IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).setHandshake(msg.sender, referrer);
        ...
        uint valueForOwner = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, msg.value, bytes(_inAppPurchaseInfo).length > 0, false);
//        IDexGoStorage(storageContract).setLatestPurchaseTime(msg.sender, block.timestamp);
        IDexGoStorage(storageContract).setInAppPurchaseData(_inAppPurchaseInfo, _tokenIdCounter.current());
        _safeMintType(msg.sender, "", finalTypeNft, false);
        emit Purchase(msg.sender, msg.value, typeNft, finalTypeNft, referrer, valueForOwner, false);
        _purchaseFinalize(msg.sender, referrer);
    }

    function purchaseBatchValue(uint8[] memory typesNft) public view returns (uint256) {
        uint256[] memory _priceForType = new uint256[](11);
        uint256 priceMainToUSDreturned;
        uint8 decimals;
        (priceMainToUSDreturned,decimals) = IDexGoStorage(storageContract).getLatestPrice();

        uint256 totalValueToPay;
        for(uint256 x=0;x<typesNft.length;x++) {
            if (_priceForType[typesNft[x]] == 0) _priceForType[typesNft[x]] = IDexGoStorage(storageContract).getPriceForType(typesNft[x]);
            require(typesNft[x] <= 10, "WT");
    ...
    function purchaseBatch(uint8[] memory typesNft, address referrer, string[] memory _inAppPurchaseInfo) payable public {
        require(msg.value >= purchaseBatchValue(typesNft).sub(IDexGoStorage(storageContract).getValueDecrease()), "WV");

//        if (IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).getFullList(msg.sender) == 0 && isIGO) IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).setHandshake(msg.sender, referrer);

        Address.sendValue(payable(IDexGoStorage(storageContract).getHandshakeLevels()), msg.value);
        uint valueForOwner = IHandshakeLevels(IDexGoStorage(storageContract).getHandshakeLevels()).distributeMoney(msg.sender, msg.value, bytes(_inAppPurchaseInfo[0]).length > 0, false);
        for(uint256 y=0;y<typesNft.length;y++) {
            uint8 finalTypeNft = typesNft[y];
            if (typesNft[y] == MAGIC_BOX) {
                finalTypeNft = uint8(random());
            ...
            _safeMintType(msg.sender, "", finalTypeNft, false);
            IDexGoStorage(storageContract).setInAppPurchaseData(_inAppPurchaseInfo[y], _tokenIdCounter.current());
            emit Purchase(msg.sender, msg.value,  typesNft[y], finalTypeNft, referrer, valueForOwner, false);
        }
//        IDexGoStorage(storageContract).setLatestPurchaseTime(msg.sender, block.timestamp);
        _purchaseFinalize(msg.sender, referrer);
    }
    ...
    {
        bool isBlacklistedWallet = IDexGoStorage(storageContract).getInAppPurchaseBlackListWallet(from);
        bool isBlacklistedTokenId = IDexGoStorage(storageContract).getInAppPurchaseBlackListTokenId(tokenId);

        require(!isBlacklistedWallet && !isBlacklistedTokenId, "BL");
    ...
    returns (string memory)
    {
        ...
        if (tempEmptyStringTest.length == 0) {
            return IDexGoStorage(storageContract).tokenURI(tokenId);
        }
...
interface IHandshakeLevels {
    ...
    function distributeMoney(address sender, uint value, bool isIOS, bool isUSDT) external returns (uint);
}
