...
contract GoatCoinNFT is ERC721Metadata {
    using Counters for Counters.Counter;

    // Max supply of NFTs
    uint256 public constant MAX_NFT_SUPPLY = 2500;

    // Mint price is 0.2 BNB
    ...
    modifier periodStarted() {
        require(block.timestamp >= startTime && startTime != 0, "GoatCoin: Period not started");
        _;
    }
    ...
    constructor(
        string memory baseURI_,
        address admin_
    ) ERC721Metadata("Hippie Goats", "HG", baseURI_) {
        _admin = admin_;
    }

    function setStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime > 0, "GoatCoin: invalid _startTime");
        require(_startTime > block.timestamp, "GoatCoin: old start time");
    ...
    function purchase(uint256 numberOfNfts) external payable periodStarted {
        require(pendingCount > 0, "GoatCoin: All minted");
        require(numberOfNfts > 0, "GoatCoin: numberOfNfts cannot be 0");
        require(numberOfNfts <= 20, "GoatCoin: You may not buy more than 20 NFTs at once");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"GoatCoin: sale already ended");
        if(tax_freeUsers[msg.sender] == 0) require(MINT_PRICE * (numberOfNfts) == msg.value, "GoatCoin: invalid ether value");
        else if (tax_freeUsers[msg.sender] < numberOfNfts){
            require(MINT_PRICE * (numberOfNfts -  tax_freeUsers[msg.sender]) == msg.value, "GoatCoin: invalid ether value");
    ...
    function claimReward(uint256 tokenId) public {
        require(
            ...
            "GoatCoin: Only owner or approved can claim rewards"
        );
    ...
    function _randomMint(address _to) internal returns (uint256) {
        require(totalSupply() < MAX_NFT_SUPPLY, "GoatCoin: max supply reached");
