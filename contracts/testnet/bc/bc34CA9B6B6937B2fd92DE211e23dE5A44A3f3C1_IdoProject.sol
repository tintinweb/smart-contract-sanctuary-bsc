...
contract IdoProject is ReentrancyGuard, Initializable {
    ...
    //===================EVENTS===================
    event Setting(uint indexed projectId, bool indexed update, uint _registerStartTime, uint _calculationTime, uint _rate, address _currency,address _tokenSale, address _admin, uint _allocation);
    ...
    function setting(
        ...
        settingProject = true;
        emit Setting(projectId, _update, registerStartTime, calculationTime, rate, address(currency), address(tokenSale), admin, maxAllocation);
    }
    ...
    function submitWinners(address[] memory _users, uint _slot) external onlyOperator {
