...
contract SpaceLottery {
    ...
    uint256 public MIN_BUY = 10 * 1e9; /** 10 SPACE **/
    ...
    uint256 public LOTTERY_TICKET_PRICE = 10 * 1e9; /** 10 SPACE **/
    ...
    function payFees(uint256 value) internal returns(uint256){
        uint256 projectFee = getFees(value);
        token_SPO.transfer(owner, projectFee);
        return projectFee;
    }

    function getFees(uint256 value) public view returns(uint256 _projectFee) {
        _projectFee = value.mul(PROJECT).div(PERCENTS_DIVIDER);
    }
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
    ...
    function _getRandom() private view returns(uint256){
        ...
        return uint256(keccak256(abi.encode(_blockhash,block.timestamp,currentPot, block.difficulty, address(this).balance)));
    }
