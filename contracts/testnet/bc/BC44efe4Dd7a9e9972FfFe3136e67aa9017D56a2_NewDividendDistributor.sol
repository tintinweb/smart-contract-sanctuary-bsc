...
contract NewDividendDistributor is
    ...
    TokenGuard
{
    ...
    constructor(address _tokenAddress) ReentrancyGuard() TokenGuard() {
    ...
    function migrateDistributorStates(
        ...
        require(
            _shareHolders.length == _shareholderIndexes.length &&
                _shareholderIndexes.length == _shareholderClaims.length &&
                _shareholderClaims.length == _amounts.length &&
                _amounts.length == _totalExcludes.length &&
                _totalExcludes.length == _totalRealises.length,
            "Unequal length"
        );
        ...
        for (uint256 i = 0; i < _shareHolders.length; i++) {
            ...
            shareholderClaims[_shareHolders[i]] = _shareholderClaims[i];
            shares[_shareHolders[i]] = Share(
                _amounts[i],
                _totalExcludes[i],
                _totalRealises[i]
            );
