...
//            NodePool(poolAddress).mining(nodeId,outAddress,msg.sender,sumProfit);
    ...
    function getNodeSumProfit(uint nodeId,address querist) public view returns (uint256) {
        ...
        if(isEffective == 1){
            ...
            uint lastSectionNodeSumProfit = userNodeTotalLp[querist][nodeId].mul(_rateTokenOne.sub(nodeNotRateTokenOne).sub(userNodeRateTokenOne[querist][nodeId])).div(1e18);
