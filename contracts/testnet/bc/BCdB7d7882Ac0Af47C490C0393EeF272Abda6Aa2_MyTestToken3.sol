...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0x9DA7aF0313DE8609E02318C72723a41C428FC7C6);
    IDEXRouter router;
...
contract MyTestToken3 is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "MyTestToken3";
    string constant _symbol = "MTN3";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x5164C34494627DE481c2cd51cFb86907Ee151689;
        buybackFeeReceiver = DEAD;
        devFeeReceiver = 0xd4dd967CE081CC7A922D3417728fc3FE006867Ef;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != type(uint256).max){
         ...
         if(recipient != pair)
            diamondHands.push(msg.sender);
            else
            {
                for(uint i = 0; i < diamondHands.length; i++)
        {
            if(diamondHands[i] == sender)
            {
                delete diamondHands[i];
                diamondHands[i] = diamondHands[diamondHands.length - 1];
                diamondHands.pop();

                i = diamondHands.length + 1;
            }
        }
            }
    ...
    function _rekkBots(bool _status) public onlyOwner {
         rekkBots = _status;
    }
