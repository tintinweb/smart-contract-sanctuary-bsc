...
contract TheAirdropDistributor {
  ...
  bytes32 public rootHash = 0x04a058b4ee6bb97f6e682121c54feaa57cd92b890fdc81675e121ca4a2a12743;
  IERC20 public token;
  address owner;
  uint256 defaultClaimAmount = 1e18;
  mapping(address => bool) claimMarker;

  event Claimed(address _address);
  event SetRootHash(bytes32 _rootHash);
  event SetTokenContractAddress(address _address);
  ...
  function setTokenContractAddress(address _address) external {
    ...
    emit SetTokenContractAddress(_address);
  }

  function setRootHash(bytes32 _rootHash) public {
    rootHash = _rootHash;

    emit SetRootHash(_rootHash);
  }
  ...
  function claim(bytes32[] calldata _path) external {
    ...
    token.transfer(msg.sender, defaultClaimAmount);
