...
contract NFTMarketplace is Initializable, OwnableUpgradeable {
    ...
    function GetActiveOffers() public view returns (_Offer[] memory arrActiveOffers) {
        uint iActiveOfferCount = 0;
        for(uint i = 0 ; i<offerCount; i++) {
            _Offer storage offerItem = offers[i];
            if(false == offerItem.fulfilled && false == offerItem.cancelled)
            {
                iActiveOfferCount++;
            }
        }
        ...
        uint insertIndex = 0;
        arrActiveOffers = new _Offer[](iActiveOfferCount);
        for(uint i = 0 ; i<offerCount; i++) {
            _Offer storage offerItem = offers[i];
            if(false == offerItem.fulfilled && false == offerItem.cancelled)
            {
                arrActiveOffers[insertIndex] = offerItem;
                ++insertIndex;
            }
        }
