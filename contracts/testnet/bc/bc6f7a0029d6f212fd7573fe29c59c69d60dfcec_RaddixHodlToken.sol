...
contract RaddixHodlToken is ERC20, ERC20Burnable, Ownable {
    ...
    function buyRHT() external payable {
        ...
        tokenBurns.push(TokenBurnData(block.timestamp+burningTimeInSeconds, uint(i*99), burningTimeInSeconds));
    ...
    function _showRHTvalueForBuy() private view returns (uint x) {
        ...
        if(totalSupply() != 0){
            x = balanceContract/(totalSupply() - sum);
        } else x = RHTvalue;
    ...
    function _showRHTvalueForSell() private view returns (uint i) {
        if(totalSupply() != 0){
            i = address(this).balance / (totalSupply()-_TokenImagineBurnedTrigger());
        } else i = RHTvalue;
        return i;
    }
