...
import "./interfaces/IFlashClaim.sol";

import "./PromissoryNote.sol";

/**
 * @dev LoanCore contract - core contract for creating, repaying, and claiming collateral for Rifi loans
 */
contract LoanCore is ILoanCore, AccessControl, Pausable {
    ...
    constructor(IFeeController _feeController) {
        ...
        borrowerNote = new PromissoryNote("Rifi Borrower Note", "rBN");
        lenderNote = new PromissoryNote("Rifi Lender Note", "rLN");

        // Avoid having loanId = 0
        loanIdTracker.increment();

        emit Initialized(address(borrowerNote), address(lenderNote));
    }
        ...
        returns (uint256 loanId)
    {
        ...
        require(terms.deadline >= block.timestamp, "LoanCore::create: overdue");
        require(!collateralInUse[terms.collateralTokenAddress][terms.collateralTokenId], "LoanCore::create: Collateral token already in use");
        ...
        collateralInUse[terms.collateralTokenAddress][terms.collateralTokenId] = true;
    ...
    function startLoan(
        ...
        IERC721 collateralToken = IERC721(data.terms.collateralTokenAddress);
    ...
    function repay(uint256 loanId) external override onlyRole(REPAYER_ROLE) {
        ...
        collateralInUse[address(collateralToken)][data.terms.collateralTokenId] = false;
    ...
    function flashClaim(uint256 loanId, address to, bytes calldata params) external override {
        LoanLibrary.LoanData memory data = loans[loanId];
        IERC721 collateralToken = IERC721(data.terms.collateralTokenAddress);
        // Ensure valid initial loan state
        require(data.state == LoanLibrary.LoanState.Active, "LoanCore::claim: Invalid loan state");
        
        address borrower = borrowerNote.ownerOf(data.lenderNoteId);
        require(borrower == msg.sender, "LoanCore::flashClaim: Unauthorized");

        IFlashClaim(address(collateralToken)).flashClaim(data.terms.collateralTokenId, to, params);
...
interface ILoanCore {
     ...
     */
    event Initialized(address borrowerNote, address lenderNote);
...
interface IFlashClaim {
  function flashClaim(uint256 bundleId, address to, bytes calldata params) external;
}
