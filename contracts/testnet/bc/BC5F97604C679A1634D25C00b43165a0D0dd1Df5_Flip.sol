...
abstract contract gamebalances {
    ...
    */
    ...
    function viewGameBalance(address accountAddress, uint256 selectedCurrency) public view returns (int256) {  
        return game_player_balances[accountAddress][selectedCurrency];
        //return getBalance(accountAddress,selectedCurrency);
    }
...
/*
    function getBalance(address playerAddress, uint256 selectedCurrency ) public view returns(int256) {
        int256 amount=0;
        for(uint256 i=0;i<playerPlaysHistory[playerAddress].length;i++) {
            if(playerPlaysHistory[playerAddress][i].currencyIndex==selectedCurrency ) {
                ...
                amount-=int256(playerPlaysHistory[playerAddress][i].wager);
                ...
                if(playerPlaysHistory[playerAddress][i].outcome==0) {
                    ...
                    if(vIndex>0) {
                        ...
                        if(n==1) {
                            amount+=int256(playerPlaysHistory[playerAddress][i].winAmount);
                        }
                    }
                } else if(playerPlaysHistory[playerAddress][i].outcome==1) {
                    //won
                    amount+=int256(playerPlaysHistory[playerAddress][i].winAmount);
                } 
            }
    ...
    */
    ...
    function getOverallPlayerWins(address playerAddress) public view returns(uint256){
        uint256 totalWins=0;
        for(uint256 i=0;i<playerPlaysHistory[playerAddress].length;i++) {
            if(playerPlaysHistory[playerAddress][i].outcome==1) { totalWins++; }
        }
        return totalWins;
    }
...
abstract contract Cointoss is VRFv2Consumer, gamebalances  {
    ...
    event outcomeReceived(address gameAddress, uint256 vrfID, uint256 vrfNumber);//,address playerAddress, int256 winnings, uint256 currency, uint256 wager);
    ...
    function toss(address senderAddress, uint256 amount, uint256 selectedCurrency) internal returns (uint256) {
        ...
        //updateBalance(msg.sender, -1*wager, selectedCurrency);
        game_alterPlayerBalance(-1*wager, senderAddress,selectedCurrency);
        //updateTimesPlayed(senderAddress);
...
/*
    function updateTimesPlayed(address senderAddress) private {
      player_timesPlayed[senderAddress]++;
    }
    function update_wins(address senderAddress) private {
      player_wins[senderAddress]++;
    }
    */
    ...
    function get_player_timesPlayed(address senderAddress) internal view returns (uint256) {
       return playerPlaysHistory[senderAddress].length;
    }
    ...
    /* delete below, for testing player login streak */ 
    function setplayerLastLogin(address senderAddress, uint256 n, uint256 m) public {
        require (msg.sender==adminWallet);
        player_loginStreak_lastBlock[senderAddress]=n;
        player_loginStreak_days[senderAddress]=m;
    }
    /* delete above */ 
}
