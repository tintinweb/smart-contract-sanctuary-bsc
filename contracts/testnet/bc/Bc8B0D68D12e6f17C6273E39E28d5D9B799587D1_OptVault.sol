...
contract OptVault is AccessControl, Initializable {
    ...
    uint256 penalityPeriod ; // penality period time
    address public router; // Router address
    IAdmin public  Admin; 
    uint256 epochNumber;
    IERC20 public token; //address of deposit token
    IERC20 public want; // Reward token from Admin.MasterChef()
    ...
    mapping(uint32 => uint32) public multiplierLevel; // level => deduction value of deposit (for level 7 it is 50%).
    ...
    @param _Admin.MasterChef() Admin.MasterChef() router address
    @param _Admin admin address

    Note this function set owner as Admin of the contract
     */

    function initialize(uint256 _id, address _owner, address _token,
     address _want,address _Admin) external initializer {
        ...
        _setupRole(SETTER_ROLE, _owner);
        _setupRole(SETTER_ROLE, msg.sender);
        exchangeRatio = 100;
        usdyPrice = 1 * 10**18;
        penalityPeriod = 48 hours;
        epochTime = 8 hours;
...
*/
    function deposit(address user,uint amount,uint32 _level) external {
        ...
        if(UserLevel[user] == 0 || (UserLevel[user] == _level)) {
            ...
            IOptVaultFactory(Admin.optVaultFactory()).withdraw(user,address(token),false, UserReciept[user]);
            console.log("qwerty");
        ...
        console.log(multiplierLevel[_level],"multiplierLevel[_level]");
        ...
        IUSDY(Admin.USDy()).mint(address(this), mintedUSDy);
        IERC20(Admin.USDy()).approve(router, mintedUSDy);
        IERC20(Admin.BUSD()).approve(router, convertedBUSD);
        ...
        exchangeRatio = IERC20(recieptToken).totalSupply() == 0 ? 100 : (IPancakeMaster(Admin.MasterChef()).userInfo(id, address(this)).amount * 100)/IERC20(recieptToken).totalSupply();
        mintRecieptToken = (amount * (multiplierLevel[_level]/10))/exchangeRatio;
        INTT(recieptToken).mint(address(this), mintRecieptToken);
        UserReciept[user] += mintRecieptToken;

        // fetch price of bysl
        path[1] = address(Admin.bYSL());
        INTT(Admin.BShare()).mint(user, (amount - (amount * (multiplierLevel[_level]/1000)))/  IBYSL(Admin.bYSL()).protocolPrice() * 100);
        IERC20(token).approve(Admin.MasterChef(),(amount -((amount*multiplierLevel[_level])/2000)));
        // depositing amount by subtracting the percentage of which is being used for add liquidity of USDY BUSD pair.
        IPancakeMaster(Admin.MasterChef()).deposit(id, (amount -((amount*multiplierLevel[_level])/2000)));
        exchangeRatio = (IPancakeMaster(Admin.MasterChef()).userInfo(id, address(this)).amount * 100)/IERC20(recieptToken).totalSupply();
    ...
    */
    function rewardState(address user,uint _amount) public returns(uint,uint){
...
/**
    ...
    function withdraw(address user, bool isReciept, uint _amount) public{
        ...
        if(isReciept){
            ...
            IPancakeMaster(Admin.MasterChef()).withdraw(id, balance);
            IERC20(token).safeTransfer(sendTo, balance);
            INTT(recieptToken).burn(address(this), _amount);
            // uint demo = IERC20(recieptToken).totalSupply() == 0 ? 1 : IERC20(recieptToken).totalSupply();
            // exchangeRatio = (IPancakeMaster(Admin.MasterChef()).userInfo(id, address(this)).amount * 100)/demo;
        }
 ...
 */
    function purchase(address user, uint amount, uint minAmount) external returns(uint){
        ...
        IERC20(token).approve(Admin.MasterChef(), amount);
        IPancakeMaster(Admin.MasterChef()).deposit(id, amount);
        uint balance = purchaseOf(amount);
        require(balance > minAmount, "OptVault: Amount is less than the minAmount");
        INTT(recieptToken).mint(user, balance);
        exchangeRatio = (IPancakeMaster(Admin.MasterChef()).userInfo(id, address(this)).amount * 100)/IERC20(recieptToken).totalSupply();
        return(balance);
    }
...
/**
    ...
    */
    function setTax(uint value) external onlyRole(SETTER_ROLE){
        tax = value;
    }
/**
    ...
    */
    ...
    function swapWantToBUSD() external onlyRole(DEFAULT_ADMIN_ROLE) {
...
/**
    ...
    */
    function deductControllerFee(uint fee) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        IERC20(Admin.BUSD()).transfer(teamAddress, fee);
        liqStatusValue = LiqStatus.OPTIMIZATION_TAX;
    }

    function collectOptimizationTax() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(liqStatusValue == LiqStatus.OPTIMIZATION_TAX, 'OptVault: Pay controller fee first');
        address[] memory path = new address[](2);
        uint balanceUSD = IERC20(Admin.BUSD()).balanceOf(address(this));
        S += (balanceUSD * 10 ** 18)/ totalStaked;
        path[0] = Admin.BUSD();
        path[1] = Admin.YSL();
        IERC20(Admin.BUSD()).approve(router, balanceUSD); //BSC Testnetapeswap router address
        ...
        IERC20(Admin.BUSD()).transfer(Admin.Treasury(),(balanceUSD*_setOptimizationTaxFEE[2])/100);
        liqStatusValue = LiqStatus.SWAP_WANT_TO_BUSD;
    }
/**
    ...
    */
    function optimizationReward(address user, uint optMultiplier) external onlyRole(DEFAULT_ADMIN_ROLE) {
        address[] memory path = new address[](2);
        path[0] = Admin.BUSD();
        path[1] = Admin.USDy();
        uint poolPriceUSDy = IUniswapV2Router02(router).getAmountsOut(1 * 10**18, path)[1];
        ...
        IUSDY(Admin.USDy()).mint(user, mintReward);
    }
/**
    @dev Setter Function for penaltyPeriod
    @param _penalityPeriod , penaltyPeriod amount as parameter.

    */
    function setPenalityPeriod(uint _penalityPeriod) external onlyRole(SETTER_ROLE) {
        require(_penalityPeriod > 0, 'OptVault: PenalityPeriod mus be greater than zero');
        penalityPeriod = _penalityPeriod * 3600 ;
    }
    /**
    ...
    */
    function setOptimizationTaxFEE(uint[] calldata getOptimizationTaxFEE) external onlyRole(SETTER_ROLE){
    ...
    */
    function withdrawAcc(uint256 _amount) internal {
        uint256 totalCompounds = IPancakeMaster(Admin.MasterChef()).userInfo(id, address(this)).amount;
        if (totalCompounds > 0) {
            IPancakeMaster(Admin.MasterChef()).withdraw(id, _amount);
        }
    }
...
/**
    ...
    */
    function setRouter(address _router) external onlyRole(SETTER_ROLE){
    ...
    */
    function setTeamAddress(address _teamAddress) external onlyRole(SETTER_ROLE){
