...
abstract contract Ownable is Context {
 ...
 abstract contract ERC721AQueryable is ERC721A
 {
    ...
    contract PigFitnessClub is ERC721AQueryable, Ownable {
    using Strings for uint256;
  ...
  uint256 public whiteListCost = 0.18 ether;
  ...
  mapping (address => uint8) public NFTPerAirdropAddress;

  uint8 public maxWLMintAmountPerWallet = 1;
  uint8 public maxMintAmountPerWallet = 10;
  uint8 public maxAirdropMintAmountPerWallet = 1;
  ...
  bytes32 public whitelistMerkleRoot = 0x0cc622db75575674b2bc8dfe29c3ac04df24f1567b769fa31f6d4ef384ea66d2;
   ...
   struct attribute {
       ...
       uint16 stamina ;
       uint8 age ;
       uint8 gender ; // 1 for male , 2 for female
       uint16 weight;
       uint16 height;
       uint16 bodyfat;
   }

   mapping (uint16 => attribute) public Attributes;

   address public secondContract;

  constructor() ERC721A("Pig Fitness Club", "PFC") {
  }

  function setSecondContract (address _address) external onlyOwner{
      secondContract = _address;
  }
  //update attributes
function updateFitnessLevel(uint16 tokenID , uint16 _fitnesslevel) public {
    require(msg.sender == secondContract , "Not allowed to update");
    Attributes[tokenID].fitnesslevel = _fitnesslevel;
}
function updateStamina(uint16 tokenID , uint16 _stamina) public {
    require(msg.sender == secondContract , "Not allowed to update");
    Attributes[tokenID].stamina = _stamina;
}
function updateAge(uint16 tokenID , uint8 _age) public {
    require(msg.sender == secondContract , "Not allowed to update");
    Attributes[tokenID].age = _age;
}
function updateGender(uint16 tokenID , uint8 _gender) public {
    require(msg.sender == secondContract , "Not allowed to update");
    Attributes[tokenID].gender = _gender;
}
function updateWeight(uint16 tokenID , uint16 _weight) public {
    require(msg.sender == secondContract , "Not allowed to update");
    Attributes[tokenID].weight = _weight;
}
function updateHeight(uint16 tokenID , uint16 _height) public {
    require(msg.sender == secondContract , "Not allowed to update");
    Attributes[tokenID].height = _height;
}
function updateBodyFat(uint16 tokenID , uint16 _bodyfat) public {
    require(msg.sender == secondContract , "Not allowed to update");
    Attributes[tokenID].bodyfat = _bodyfat;
}
 // Integration with future staking
 ...
 function _beforeTokenTransfers(
        ...
        uint16 token = uint16(startTokenId);
        delete Attributes[token];
    }

  function mint(uint8 _mintAmount) external payable  {
     uint16 totalSupply = uint16(totalSupply());
     uint8 _txPerAddress = NFTPerPublicAddress[msg.sender];
    ...
    _safeMint(msg.sender , _mintAmount);
     NFTPerPublicAddress[msg.sender] = _mintAmount + _txPerAddress;
     delete totalSupply;
     delete _mintAmount;
  }
  ...
  function Airdrop(uint16 _mintAmount, address [] memory _receiver) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
    require(totalSupply + (_mintAmount * _receiver.length) <= maxSupply, "Excedes max supply.");
...
if ( reveal == false)
{
   ...
   function setWLPaused() external onlyOwner {
    ...
    function setAirdropPaused() external onlyOwner {
 ...
 function setMaxTxPerAirdropAddress(uint8 _limit) external onlyOwner{
    maxAirdropMintAmountPerWallet = _limit;
   delete _limit;
