...
contract Test is IBEP20, Auth {
    ...
    uint256 public BNBRewardsFee = 30;
    uint256 public marketingFee    = 30; 
    uint256 public lotteryFee  = 30;
    uint256 public liquidityFee  = 10;
    uint256 public totalFees     = BNBRewardsFee + marketingFee + lotteryFee + liquidityFee;
    uint256 public feeDenominator = 1000;
    uint256 public reducedFeePercentage = 0;

    address public marketingWallet = 0xD733289Ccb78CdC849d0C2cd9dB6F0D727123DCE;
    address public lotteryWallet = 0xa1F738f3f066Bd18208e09DBE511413b536e8c75;
    ...
    function setFeeRates(uint256 _BNBRewardsFee, uint256 _liquidityFee, uint256 _marketingFee,  uint256 _lotteryFee, uint256 _feeDenominator) public onlyOwner {
            BNBRewardsFee = _BNBRewardsFee;
            liquidityFee = _liquidityFee;
            marketingFee = _marketingFee;
            lotteryFee = _lotteryFee;
            totalFees = _BNBRewardsFee + _liquidityFee + _marketingFee + _lotteryFee;
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountBNBReflection = amountBNB.mul(BNBRewardsFee).div(totalBNBFee);
        uint256 amountBNBTeam = amountBNB.sub(amountBNBLiquidity).sub(amountBNBReflection);
        uint256 amountBNBlottery = amountBNB.sub(amountBNBLiquidity).sub(amountBNBReflection);

        try dividendDistributor.deposit{value: amountBNBReflection}() {} catch {}
                
        if(marketingFee > 0){
            payable(marketingWallet).transfer(amountBNBTeam);
        }
        if(lotteryFee > 0){
