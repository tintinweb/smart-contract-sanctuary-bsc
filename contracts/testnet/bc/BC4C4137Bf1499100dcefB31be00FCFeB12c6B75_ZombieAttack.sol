...
contract ZombieBase is Ownable {
    ...
    function internalLevelUp(uint _zombieId) internal {
        while (
            ...
            while(true){
                i = 0;
                statsArray = ['healthPoint', 'attack', 'defense', 'criticalRate', 'criticalDamage', 'speed'];
                ...
                while(totalGrowPoint > 0) {
                    ...
                    uint randomStatNumber = randMod(statsArray.length - i); // Random the index of the statsArray to choose which stat to increase
                    string memory randomStat = statsArray[randomStatNumber]; // Decide which stat to increase
                    uint randomStatAmount = randMod(totalGrowPoint.add(1)); // Decide the number of point to increase
