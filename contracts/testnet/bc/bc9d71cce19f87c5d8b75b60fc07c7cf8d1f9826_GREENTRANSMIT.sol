...
contract GREENTRANSMIT is Context, Ownable {
    ...
    mapping(uint=>mapping(address=>uint)) public _userLastTransmitTime;//每期用户最后进入时间
    ...
    function test2(uint count) external onlyOwner{
    ...
    function transmit() external{
        ...
        require(_userLastTransmitTime[_number][account]==0||(_userLastTransmitTime[_number][account]+_userMaxTransmitTime)<block.timestamp,"error4");
        ...
        _userLastTransmitTime[_number][account]=block.timestamp;
        //修改倒计时
    ...
    //批量出局
    function _outTransmits(bool status) public onlyOwner returns(bool){
