...
contract CirclePresalePool is OwnableUpgradeable, IPool , ReentrancyGuard {
    ...
    event LiquidityWithdrawn(address indexed pool_address, address user, uint256 amount, address pair);
    ...
    function initialize(
        ...
        require(presale.token != address(0), "Invalid Token address");
        require(presale.paymentToken != presale.token, "Invalid Payment Token address");
        ...
        require(presale.liquidityLockDays >= MINIMUM_LOCK_DAYS, "Liquidity unlock time must be at least 30 days after pool is finalized");
        require(
            _fees[1] >= 0 &&
            _fees[1] <= 100 &&
            _fees[0] >= 0 &&
        ...
        require (presale.rate >= presale.liquidityListingRate, "Liquidity listing rate must be less than or equal to pool listing rate");
        require(presale.liquidityPercent >= 51 && presale.liquidityPercent <= 100, "Invalid liquidity percentage");
        require(presale.refundType == 0 || presale.refundType == 1, "Refund type must be 0 (refund) or 1 (burn)");
        require(presale.listingType == 0 || presale.listingType == 1, "Listing type must be 0 (auto) or 1 (manual)");
        ...
        token = presale.token;
        router = presale.router;
        governance = presale.governance;
        rate = presale.rate;
        liquidityListingRate = presale.liquidityListingRate;
        ...
        liquidityLockDays = presale.liquidityLockDays;
        tokenFeePercent = _fees[1];
        ethFeePercent = _fees[0];
        liquidityPercent = presale.liquidityPercent;
        refundType = presale.refundType;
        listingType = presale.listingType;
    ...
    function getPoolInfo() external override view returns (address, uint8[] memory , uint256[] memory , string memory , string memory , string memory){
        ...
        return (token , state , info , IERC20Info(token).name() , IERC20Info(token).symbol() , poolDetails);
    }
...
//        IPoolFactory(factory).addTopPool(address(this), totalRaised);
        uint256 volume = PoolLibrary.convertCurrencyToToken(requiredFunds, rate);
    ...
    function finalize() external onlyOperator nonReentrant{
        ...
        uint256 remainingBnb = 0;
        uint256 remainingToken = 0;

        if(paymentToken == address(0)) {
            remainingBnb = address(this).balance.sub(liquidityBnb).sub(bnbFee);
        } else {
            remainingBnb = IERC20(paymentToken).balanceOf(address(this)).sub(liquidityBnb).sub(bnbFee);
        }
        ...
        if (remainingBnb > 0) {
            if(paymentToken == address(0)) {
                ...
                IERC20(paymentToken).safeTransfer(governance, remainingBnb);
            }
        }
...
//        IPoolFactory(factory).removeTopPool(address(this));
        emit Finalized(address(this), msg.sender, bnbFee, totalRaised, liquidity);
    }
...
//        IPoolFactory(factory).removeTopPool(address(this));
    ...
    function withdrawLiquidity() external onlyOperator {
        ...
        if(listingType == 0) {
            ...
            address tokenA = IUniswapV2Router02(router).WETH();
            if(paymentToken != address(0)) {
                tokenA = paymentToken;
            }
            ...
            emit LiquidityWithdrawn(address(this), msg.sender, balance, pair);
        } else {
            if(paymentToken == address(0)) {
                uint256 balance = address(this).balance;
                payable(governance).sendValue(balance);
                emit LiquidityWithdrawn(address(this), msg.sender, balance, address(0));
            } else {
                uint256 balance = IERC20(paymentToken).balanceOf(address(this));
                IERC20(paymentToken).safeTransfer(governance, balance);
                emit LiquidityWithdrawn(address(this), msg.sender, balance, paymentToken);
            }
        }
    }
    ...
    function emergencyWithdrawLiquidity(address token_, address to_, uint256 amount_) external override onlyOwner {
        ...
        emit EmergencyLiquidityWithdrawn(address(this), msg.sender, amount_, token_);
    ...
    function getUpdatedState() public view returns (uint256, uint8, bool, uint256, string memory) {
        return (totalRaised, uint8(poolState), false, liquidityUnlockTime, poolDetails);
    }
