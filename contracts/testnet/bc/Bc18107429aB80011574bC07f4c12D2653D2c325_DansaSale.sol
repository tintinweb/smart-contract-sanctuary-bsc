...
contract DansaSale is Context, Initializable {
    ...
    function buyXDansa(uint256 _amount) public {
        ...
        if (DansaAmount > IERC20(xDansaAddr).balanceOf(address(this))) {
            DansaAmount = IERC20(xDansaAddr).balanceOf(address(this));
        }
        require(IBEP20(dansaAddr).transferFrom(msg.sender, address(this), usd),"Transfer failed!!!");

        IBEP20(xDansaAddr).buyToken(msg.sender, DansaAmount, _amount);
    }

    function transferFees() external onlyOwner {
        uint256 usd = IBEP20(dansaAddr).balanceOf(address(this));
        uint256 bankFees = usd.mul(bankFee).div(14);
        uint256 treasury = usd.mul(backFee.add(treasuryFee).add(burnFee)).div(14);
        uint256 usdKeep = usd - treasury - bankFees;
        require(IBEP20(dansaAddr).transfer(bankAddr, bankFees),"Transfer bank fee failed!!!");
        require(IBEP20(dansaAddr).transfer(treasuryAddr, treasury),"Transfer treasury failed!!!");
        require(IBEP20(dansaAddr).transfer(owner(), usdKeep),"Transfer owner failed!!!");
    }
    ...
    function sellXDansa(uint256 _amount) public returns(bool){
        ...
        uint256 fee = _amount.mul(14).div(100);
        uint256 value = _amount - fee;
        uint256 usd = value.div(price);
        emit TakeFee(msg.sender, fee);
        IBEP20(xDansaAddr).transferFrom(msg.sender, address(this), _amount);
        IBEP20(dansaAddr).transferFrom(pool, msg.sender, usd);
        emit SellDansa(msg.sender, _amount, usd);
        return true;
    }
