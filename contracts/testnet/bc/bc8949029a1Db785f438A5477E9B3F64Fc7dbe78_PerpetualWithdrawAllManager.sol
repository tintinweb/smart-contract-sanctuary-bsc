...
contract PerpetualWithdrawFunctions is PerpetualRebalanceFunctions {
    ...
    function _withdraw(
        ...
        require(marginAccounts[_perpetual.id][_traderAddr].fPositionBC == 0 || !ISpotOracle(_perpetual.oracleS2Addr).isMarketClosed(), "market is closed");
        _updateTraderMargin(_perpetual, _traderAddr, _fAmount.neg());
        require(_getMarginViewLogic().isInitialMarginSafe(_perpetual.id, _traderAddr), "margin is unsafe after withdrawal");
