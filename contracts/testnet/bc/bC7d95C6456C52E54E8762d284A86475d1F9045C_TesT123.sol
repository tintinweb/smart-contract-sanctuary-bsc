...
contract Stakeable {
  ...
  uint32 public _firstTypePartFromBalance = 10000;
  uint32 public _secondTypePartFromBalance = 30000;
  uint32 public _thirdTypePartFromBalance = 60000;

  uint32 public _firstTypeAssumption = 6000;
  uint32 public _secondTypeAssumption = 3000;
  uint32 public _thirdTypeAssumption = 1000;  
  ...
  uint32 dayInSeconds = 60;
    ...
    * @notice Staked event is triggered whenever a user stakes tokens, address is indexed to make it filterable
     */
     event Staked(address indexed user, uint256 amount, uint256 index, uint256 timestamp, uint256 milliPercent, uint8 stakeType, uint256 mustClaim);
    ...
    * StakeID 
    */
    function _stake(uint256 _amount, uint256 collectedReward, uint8 stakeType, uint256 ccSupply, uint256 usedReward) internal returns (uint256){
        ...
        // uint256 milliPercentOfUser = collectedReward / _amount
        uint256 milliPercentOfUser;
        uint256 mustClaim;

        if (stakeType == 1) { // 1 week stacking
          milliPercentOfUser = ((collectedReward - usedReward) * _firstTypePartFromBalance) * 100000 / (ccSupply * _firstTypeAssumption);
          mustClaim = _amount * milliPercentOfUser / 100000 * _firstStackTypeDuration;
        }
        else if (stakeType == 2) {  // 4 weeks stacking
          milliPercentOfUser = ((collectedReward - usedReward) * _secondTypePartFromBalance) * 100000 / (ccSupply * _secondTypeAssumption);
          mustClaim = _amount * milliPercentOfUser / 100000 * _secondtStackTypeDuration;
        }
        else if (stakeType == 3) {  // 12 weeks stacking
          milliPercentOfUser = ((collectedReward - usedReward) * _thirdTypePartFromBalance) * 100000 / (ccSupply * _thirdTypeAssumption);
          mustClaim = _amount * milliPercentOfUser / 100000 * _thirdStackTypeDuration;
        }
        ...
        emit Staked(msg.sender, _amount, index, timestamp, milliPercentOfUser, stakeType, mustClaim);
     ...
     */
      function calculateClaimableReward(Stake memory _current_stake) internal view returns(uint256){
          ...
          if (_current_stake.stakeType == 1) {
            if (daysPassed < 10 minutes) return 0;
            else claimableReward = _current_stake.amount * _current_stake.milliPercent / 100000 * _firstStackTypeDuration;
          }
          else if (_current_stake.stakeType == 2) {
            if (daysPassed < 90 minutes) return 0;
            else claimableReward = _current_stake.amount * _current_stake.milliPercent / 100000 * _secondtStackTypeDuration;
          }
          else if (_current_stake.stakeType == 3) {
            if (daysPassed < 180 minutes) return 0;
            else claimableReward = _current_stake.amount * _current_stake.milliPercent / 100000 * _thirdStackTypeDuration;
          }
    ...
    */
    ...
    function _rootAmount(uint256 index) internal view returns(uint256){
     ...
     function _setPartsForStackTypes(uint32 first, uint32 second, uint32 third) internal returns(bool){
      _firstTypePartFromBalance = first;
      _secondTypePartFromBalance = second;
      _thirdTypePartFromBalance = third;
      return true;
     }
...
contract TesT123 is Context, IERC20, Ownable, Stakeable {
    ...
    *
    ...
    function stake(uint256 _amount, uint8 stakeType) public {
      ...
      require(_amount < _balances[msg.sender], "Cannot stake more than you own");
        ...
        uint256 mustClaim = _stake(_amount, collectedReward, stakeType, ccSupply, _applicableContractRewardAmount);
  ...
  /**
    * @notice withdrawStake is used to withdraw stakes from the account holder
     */
    function withdrawStake(IERC20 token, uint256 stake_index)  public {
