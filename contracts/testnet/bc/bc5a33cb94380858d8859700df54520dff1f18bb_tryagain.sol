...
contract tryagain {
    ...
    uint256 amount = getUserAvailable(msg.sender);
    uint dividends = amount;
    uint userPercentRate = getUserPercentRate(msg.sender);

    require(dividends > 0, "space: No amount to reinvest");
		user.deposits.push(Deposit(uint64(dividends), 0, uint32(block.timestamp)));
		totalDeposits = totalDeposits.add(1);
        totalInvested = totalInvested.add(dividends);
        emit NewDeposit(msg.sender, dividends);
