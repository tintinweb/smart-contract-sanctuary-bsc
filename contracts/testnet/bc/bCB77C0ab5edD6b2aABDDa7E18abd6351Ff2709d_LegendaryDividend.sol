...
contract LegendaryDividend is DividendPayingToken {
    using EnumerableSet for EnumerableSet.UintSet;

    uint256 tokensPerNFT = 1000;
    piggyNFT rewardNFT;
    ...
    constructor(address rewardNFTAddr) ERC20("VWLD", "Volume Wars Legendary Dividend Token") {
    ...
    function unstakeLegendary(uint256 tokenId) external {
        require(stakedNFTOwnerOf(tokenId) == _msgSender(), "Not Original Owner");
        require(balanceOf(_msgSender()) >= tokensPerNFT, "Not enough tokens to unstake");
        withdrawDividend();
        _burn(_msgSender(), tokensPerNFT);
        stakedNFTsByOwner[_msgSender()].remove(tokenId);
        delete stakedNFTOwners[tokenId];
        (uint16 set, ) = rewardNFT.metadataOf(tokenId);
        stakedNFTsBySeason[set].remove(tokenId);
        rewardNFT.transferFrom(address(this), _msgSender(), tokenId);
        emit UnstakedLegendary(_msgSender(), tokenId);
    }
}
