...
// File: contracts/ERC20/extensions/draft-IERC20Permit.sol
...
// File: contracts/ERC20/extensions/draft-ERC20Permit.sol
...
// File: contracts/ERC20/extensions/ERC20Votes.sol
...
contract WhalesToken is ERC20("WHALES", "WHALES"), Ownable {
    ...
    address public masterchefSharks;
    ...
    address public whalesBusdSwapPair; // The trading pair
    ...
    event MasterChefSharksSet(address indexed masterchefSharks);
    ...
    modifier onlyMasterchefSharks() {
        require(masterchefSharks == msg.sender, "masterchefSharks only");
        _;
    }

    /**
     * @notice Constructs the Whales Token contract.
     */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (recipient == address(whalesBusdSwapPair) && !isExcluded){
        ...
        if (buySellLimiterEnabled && !isExcluded){
            if (recipient == address(whalesBusdSwapPair) || sender == address(whalesBusdSwapPair)){
     ...
     * Can only be called by the masterChef Sharks
     */
    function convertDepositFeesToBUSD(address token, bool isLPToken, bool isLiquidation) public onlyMasterchefSharks returns (uint256) {

        // shouldn't be trying to sell whalesToken
        if (token == address(this))
            return 0;

        // LP tokens aren't destroyed in WhalesToken, but this is so WhalesToken can process
        // already destroyed LP fees sent to it by the SharksToken contract.
        if (isLPToken) {
     ...
     * Can only be called by the masterChef Sharks
     */
    function transferBUSDToUser(address recipient, uint256 amount) external onlyMasterchefSharks {
    ...
    function updatePancakeswapRouter(address _router) external onlyOperator {
        ...
        whalesBusdSwapPair = IUniswapV2Factory(pancakeswapRouter.factory()).getPair(address(this), busdCurrencyAddress);
        require(address(whalesBusdSwapPair) != address(0), "busd pair !exist");
 ...
 * @dev Update the excludeFromMap
    ...
    function setMasterChefSharks(address _masterchefSharks) external onlyOperator {
        require(address(_masterchefSharks) != address(0), "masterchefSharks cannot be the 0 address");
        require(address(masterchefSharks) == address(0), "masterchefSharks address already set");
        masterchefSharks = _masterchefSharks;

        emit MasterChefSharksSet(_masterchefSharks);
    }
