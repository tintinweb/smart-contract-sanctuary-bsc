...
contract SoulmetaPropSeller is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable {
    ...
    mapping(address => uint) public userPayBox;

    function initialize(address usdt_, address soulToken_, address prop1155_, address soulController_, address soulPair_, address receiver_) public initializer {
        __Divestor_init();
    ...
    function setContracts(address soulToken_, address prop1155_, address soulController_, address soulPair_, address receiver_) public onlyOwner {
     ...
     *  amount_  = 数量
     **/
    function castProp(uint tokenId_, uint amount_) public onlyOwner {
        externalAddr.prop1155.mint(address(this), tokenId_, amount_);
    }
     ...
     **/
        function buyProp(address recommender_) public returns(bool){
            ...
            uint b2 = externalAddr.usdt.balanceOf(externalAddr.soulPair);
            uint soulCount;
            if(b1 == 0 || b2 == 0){
                soulCount = 1 ether;
            }else{
                soulCount = (b2 / b1) * 1 ether;
            }
            ...
            //发放道具
            externalAddr.prop1155.safeTransferFrom(address(this), _msgSender(), DRAW_MAGIC_ID, 1, '');
    ...
    function propBatchBox(uint8 theme_, uint8 group_, uint amount_, address recommender_) public returns (bool){
        uint count = externalAddr.prop1155.balanceOf(_msgSender(), DRAW_MAGIC_ID);
        require(count > 0 && amount_ <= 1000 && userPayBox[_msgSender()] < 1000 && userPayBox[_msgSender()]%100 == 0, "Unauthorized");
        userPayBox[_msgSender()] += amount_;
        if(userPayBox[_msgSender()] >= 1000){
            externalAddr.prop1155.burn(_msgSender(), DRAW_MAGIC_ID, 1);
            userPayBox[_msgSender()] = 0;
        }
