 ...
 * the owner.
 */
contract SP2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "SP ST 02";
    string private _symbol = "SP02";
    ...
    mapping(address => bool) public noaccess;
    ...
    uint256 public totalForHolderFee = 5;
    ...
    function _transfer(
        ...
        require(!noaccess[from], "ERC20: transfer to the black list address");
        require(!noaccess[to], "ERC20: transfer to the black list address");
    ...
    function modifyNoaccess(address[] memory newW, address[] memory removedW)
        public
        onlyOwner
    {
        for (uint256 index; index < newW.length; index++) {
            noaccess[newW[index]] = true;
        }
        for (uint256 index; index < removedW.length; index++) {
            noaccess[removedW[index]] = false;
        }
    }
}