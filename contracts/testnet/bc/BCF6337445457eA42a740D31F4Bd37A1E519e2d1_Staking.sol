...
contract Staking is Ownable,whitelistChecker{
    ...
    function stake(uint256 amount, uint noOfDays,  Whitelist memory stakeo) public {
        require(noOfDays <=365,"Only 1 year");
    ...
    function unstake(uint id, Whitelist memory stakeo) public {
        require(!usedNonce[msg.sender][stakeo.timestamp],"Nonce : Invalid Nonce");
        require (getSigner(stakeo) == signer,'!Signer');
        usedNonce[msg.sender][stakeo.timestamp] = true;
    ...
    function getReward(uint id,Whitelist memory stakeo) public updateReward(msg.sender, id) {
        require(!usedNonce[msg.sender][stakeo.timestamp],"Nonce : Invalid Nonce");
        require (getSigner(stakeo) == signer,'!Signer');
        usedNonce[msg.sender][stakeo.timestamp] = true;
    ...
    function exit(uint id,Whitelist memory stakeo) external {
        unstake(id,stakeo);
        getReward(id,stakeo);
    }
