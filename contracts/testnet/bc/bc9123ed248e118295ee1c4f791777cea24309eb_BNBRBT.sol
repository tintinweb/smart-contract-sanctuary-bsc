...
contract BNBRBT is IERC20, Ownable {
    ...
    uint256 public maxTxAmount = 10_000_000_000_000_000_000_000 * 10**_decimals;
    ...
    address public marketingAddress = 0xA68e5f5d9079A1f5c1606d148cBd71638479e387;
    ...
    Taxes public taxes = Taxes(50,30,10);
    ...
    modifier addressValidation(address _addr) {
        require(_addr != address(0), 'BNBRBT: Zero address');
        _;
    }
    ...
    function _getTValues(uint256 tAmount, uint8 takeFee) private returns (valuesFromGetValues memory s) {
		...
		if(takeFee == 0) {
			...
			ETHAmount.marketing += s.tLiquidity * taxes.marketing / taxes.liquidity;
			s.tTransferAmount = tAmount - s.tRfi - s.tLiquidity - s.tMarketing;
			return s;
		}
	}

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, uint8 takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rMarketing, uint256 rLiquidity) {
		...
		if (takeFee == 0) {
			...
			rTransferAmount = rAmount - rRfi - rLiquidity - rMarketing;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!swapping && from != pair && to != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            address[] memory path = new address[](2);
                ...
                path[1] = router.WETH();
                //path[2] = BNB;
            uint _amount = router.getAmountsOut(balanceOf(address(this)), path)[1];
    ...
    function swapTokensForETH(uint256 tokenAmount) private lockTheSwap {
        ...
        (bool success, ) = marketingAddress.call{value: (ETHAmount.marketing * address(this).balance)/tokenAmount}("");
    ...
    function updateMarketingWallet(address newWallet) external onlyOwner addressValidation(newWallet) {
        require(marketingAddress != newWallet, 'BNBRBT: Wallet already set');
        marketingAddress = newWallet;
        _isExcludedFromFee[marketingAddress];
    }

    function updateStableCoin(address _bnb) external onlyOwner  addressValidation(_bnb) {
        require(BNB != _bnb, 'BNBRBT: Wallet already set');
        BNB = _bnb;
    }
    ...
    function setAntibot(address account, bool state) external onlyOwner{
        require(_isBot[account] != state, 'BNBRBT: Value already set');
        _isBot[account] = state;
    }
    
    function bulkAntiBot(address[] memory accounts, bool state) external onlyOwner {
        require(accounts.length <= 100, "BNBRBT: Invalid");
