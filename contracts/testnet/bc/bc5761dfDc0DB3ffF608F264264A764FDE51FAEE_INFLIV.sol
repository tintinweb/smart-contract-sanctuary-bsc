...
contract INFLIV is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
	...
	function _updatePools() public returns(bool)
	{
	   ...
	   if(block.timestamp >= (communityPool.lastClaimTime + communityPool.claimInterval) && address(stakingAddress) != address(0) && address(farmAddress) != address(0) && CPAmount > 0) 
	   {
	    ...
	    uint256 availableInInsurance = availableInsurancePoolTokens();
	    if(availableInInsurance > 0 && address(insuranceAddress) != address(0))
	    {
	       super._mint(address(this), availableInInsurance);
		   IInsurance(insuranceAddress).updatePool(availableInInsurance);
	    }
	   
	    uint256 availableInBounty = availableBountyBonusesPoolTokens();
	    if(availableInBounty > 0 && address(bountyAddress) != address(0))
	    { 
	       super._mint(address(this), availableInBounty);
		   IBounty(bountyAddress).updatePool(availableInBounty);
	    }
		return true;
	}
