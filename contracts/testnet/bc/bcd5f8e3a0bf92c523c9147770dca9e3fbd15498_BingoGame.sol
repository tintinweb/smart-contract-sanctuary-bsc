...
contract BingoGame is Ownable {
    ...
    constructor() public {
        setQualification(address(0x5023691b41DbA260104cD7CD90A708754BC72423),0.001*10**18);
      ...
      uint32 status=0;
      {
       ...
       if(setLen==oneDayMaxSet){//判断参与的人数是否等于一局允许的最大局数
            ...
            if(oneSetRealCount==oneDayMaxSet){//10人全部没有中奖3次
                ...
                emit BingoAddress(msg.sender, address(this));
   ...
   function getBalanceOf(address _account) public view returns (uint256) {
    ...
    function isQualificationAndInvited(address _account) public view returns(bool,bool,bool,bool){
        ...
        //     ERC20 erc20token = ERC20(_takePartInQualification.keys[i]);
