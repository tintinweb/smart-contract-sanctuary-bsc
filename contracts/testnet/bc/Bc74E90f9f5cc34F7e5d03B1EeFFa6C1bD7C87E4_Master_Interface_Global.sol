...
contract Master_Interface_Global is Context {
    ...
    constructor() {
        ...
        _lotteryNetwork = 1250000 * 10**18;
        Max_Point = 50;
        Max_Lottery_Price = 10;
        lastRun = block.timestamp;
        tokenAddress = 0xBebbCB767242fD79D0418d563B695BE32ea75BC8; 
        _depositToken = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);
        Count_Last_Users = 0;
        All_Payment = 400 * 10**18;
    }
    ...
    function Reward_24() public {
        ...
        uint256 Value_Reward = Price_Point() * 65;
        uint256 Value_RewardB = Price_Point() * 20;
        Value_LotteryANDFee = Price_Point();

        uint256 valuePoint = ((Value_Reward)) / Today_Total_Point();
        uint256 valuePointB = ((Value_RewardB)) / Today_Total_PointB();
        ...
        for (uint256 i = 0; i <= _userId; i = unsafe_inc(i)) {
            ...
            if (Result > 0) {
                ...
                if (
                    PointB * valuePointB > _depositToken.balanceOf(address(this))
                ) {
                    _depositToken.safeTransfer(
                        _allUsersAddress[i],
                        _depositToken.balanceOf(address(this))
    ...
    function Register(address uplineAddress) public {
        ...
        IERC20(tokenAddress).transfer(_msgSender(), 12500000 * 10**18);
    }

    function Lottery_Reward() private {
        uint256 Numer_Win = ((Value_LotteryANDFee * 5) / 10**18) /
            Max_Lottery_Price;
    ...
    function MIG_Gift() public {
        ...
        require(
            ...
            "You Dont Have Enough Master Interface Global!"
        );
        ...
        require(
            testUser == true,
            "This address is not in Master Interface Global Contract!"
        );
    ...
    function Upload_Old_Users(
        ...
        require(_msgSender() == owner, "Just Owner Can Run This Order!");
        require(Count_Last_Users <= 4, "The number of old users is over!");
        ...
        IERC20(tokenAddress).transfer(person, 125000000 * 10**18);
        Count_Last_Users++;
        _userId++;
    }
    ...
    function Today_Reward_Balance() public view returns (uint256) {
        return (Price_Point() * 65) / 10**18;
    }

    function Today_Reward_BalanceB() public view returns (uint256) {
        return (Price_Point() * 20) / 10**18;
    }

    function Today_Gift_Balance() public view returns (uint256) {
        return (Price_Point() * 5) / 10**18;
    }

    function Today_Reward_Writer_Reward() public view returns (uint256) {
        uint256 Remain = ((Price_Point() * 5) / 10**18) % Max_Lottery_Price;
        return Remain;
    }
    ...
    function Master_Interface_Global_Token_Address() public view returns (address) {
        return tokenAddress;
    }
    ...
    function Today_Total_PointB() public view returns (uint256) {
        uint256 TPointB;
        for (uint256 i = 0; i <= _userId; i = unsafe_inc(i)) {
            uint256 min = _users[_allUsersAddress[i]].leftDirect <=
                _users[_allUsersAddress[i]].rightDirect
    ...
    function Today_Value_Point() public view returns (uint256) {
        if (Today_Total_Point() == 0) {
            ...
            return (Price_Point() * 65) / (Today_Total_Point() * 10**18);
        }
    }

    function Today_Value_PointB() public view returns (uint256) {
        if (Today_Total_PointB() == 0) {
            return Today_Reward_Balance();
        } else {
            return (Price_Point() * 65) / (Today_Total_PointB() * 10**18);
        }
    }
