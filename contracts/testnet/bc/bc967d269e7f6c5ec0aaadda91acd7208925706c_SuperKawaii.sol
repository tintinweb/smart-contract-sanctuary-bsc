...
contract SuperKawaii is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private max_public = 2500;
    uint private max_wl = 833;
    ...
    // NFTs Datas
    mapping(uint => uint) public lastTimeFedByID;
    mapping(uint => uint) public timeOfDeathByID;
    mapping(uint => uint) public feedCounterByID;
    mapping(uint => uint) public reviveCounterByID;
    uint public cookiePrice = 0.002 ether;
    uint public revivePenalty = 0.01 ether;
    uint public feedBonus = 1;
    uint public reviveBonus = 1;
    uint public feedingLifespan = 2 days;
    uint public timeUntilHungry = 12 hours;

    function abs(int x) private pure returns (int) {
        return x >= 0 ? x : -x;
    }

    function getOverallHealthByID(uint id) external view returns (int256){
        return abs(int256((timeOfDeathByID[id]) - (feedingLifespan + block.timestamp))) / (int256(timeOfDeathByID[id] + (feedingLifespan + block.timestamp))/2) * 100;
    }

    function feedByID(uint id) external payable{
        if(msg.value < cookiePrice) revert("Not enough funds");
        if((lastTimeFedByID[id] + timeUntilHungry) > block.timestamp) revert("Your pet is not hungry...");
        if(timeOfDeathByID[id] < block.timestamp) revert("You pet needs to be revived first...");
        timeOfDeathByID[id] = block.timestamp + feedingLifespan;
        feedCounterByID[id] += feedBonus;
        lastTimeFedByID[id] = block.timestamp;
    }

    function reviveByID(uint id) external payable{
        if(msg.value < revivePenalty) revert("Not enough funds");
        if(timeOfDeathByID[id] > block.timestamp) revert("Your pet isnt dead...");
        timeOfDeathByID[id] = block.timestamp + feedingLifespan;
        reviveCounterByID[id] += reviveBonus;
        lastTimeFedByID[id] = block.timestamp;
    }

    function setTimeUntilHungry(uint newTime) external onlyOwner{
        timeUntilHungry = newTime;
    }

    function setFeedingLifespan(uint newLifespan) external onlyOwner{
        feedingLifespan = newLifespan;
    }

    function setReviveBonus(uint newBonus) external onlyOwner{
        reviveBonus = newBonus;
    }

    function setFeedBonus(uint newBonus) external onlyOwner{
        feedBonus = newBonus;
    }

    function setCookiePrice(uint newPrice) external onlyOwner{
        cookiePrice = newPrice;
    }

    function setRevivePenalty(uint newPenalty) external onlyOwner{
        revivePenalty = newPenalty;
    }
    ...
    function setNewPet() internal{
        uint256 currentIndex = _nextTokenId();
        timeOfDeathByID[currentIndex] = block.timestamp + feedingLifespan;
        feedCounterByID[currentIndex] = 0;
        reviveCounterByID[currentIndex] = 0;
        lastTimeFedByID[currentIndex] = block.timestamp;
    }
