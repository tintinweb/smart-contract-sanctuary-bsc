...
interface IStaking {
    ...
    function claimableForStakeInUsd(
        address _staker,
        uint256 _stakeId
    ) external view returns (uint256);

    function convertUsdOnlyToTokenWithDecimal(        
        uint256 _amountUsd
    ) external view returns (uint256);
...
contract Staking is IStaking, Ownable, ERC1155Holder {
    ...
    function stake(
        ...
        require(
            AnotherNFT(nft).isApprovedForAll(msg.sender, address(this)),
            "STAKING: MUST APPROVE FIRST"
        );
        try AnotherNFT(nft).safeTransferFrom(msg.sender, address(this), _nftId, _amount, _data) {
    ...
    function payCommisionMultiLevels(
        ...
        while (currentRef != address(0) && index <= 5) {
            ...
            if (totalStakeAmount) {
                ...
                uint256 effectNumberDecimal = getEffectDecimalForCurrency();
                ...
                require(
                    ERC20(rewardToken).transfer(currentRef, totalCommissionInToken),
                    "STAKING: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
                );
                emit PayCommission(msg.sender, currentRef, totalCommissionInToken);
            }
            index++;
            currentRef = payable(
    ...
    function unstake(uint256 _stakeId, bytes memory _data) public {
        ...
        try AnotherNFT(nft).safeTransferFrom(address(this), staker, nftId, amount, _data) {
    ...
    function claim(uint256 _stakeId) public {
        ...
        uint256 timeEndStake = stakeInfo.stakeTime + stakeInfo.stakePeriod * 30 * 24 * 3600;
        require(block.timestamp <= timeEndStake, "STAKING: MUST CALL UNSTAK INSTEAD OF CLAIM");
        require(!stakeInfo.isUnstaked, "STAKING: ALREADY UNSTAKED");        
        uint256 claimableRewardInUsd = claimableForStakeInUsd(msg.sender, _stakeId);
        stakedNFTs[msg.sender][_stakeId].totalClaimedAmount += claimableRewardInUsd;
        uint256 claimableRewardInTokenWithDecimal = convertUsdOnlyToTokenWithDecimal(claimableRewardInUsd);
        require(
    ...
    function claimableForStakeInUsd(
        ...
        require(timeStaked > 0, "STAKING: NOT TIME TO CLAIM");
        uint256 totalStakeTime = stakeInfo.stakePeriod * 30 * 24 * 3600;  
        if(timeStaked >= totalStakeTime) {
            timeStaked = totalStakeTime;
        }
        uint256 accumulateRewardInUsd = stakeInfo.totalRewardAmount * timeStaked / totalStakeTime;
        uint256 remainRewardInUsd = accumulateRewardInUsd - stakeInfo.totalClaimedAmount;
        return remainRewardInUsd;
    }

    /**
     * @dev function to convert amount usd to token with decimal
     * @param _amountUsd amountUsd need to convert
     */
    function convertUsdOnlyToTokenWithDecimal(       
        uint256 _amountUsd
    ) public view override returns (uint256) {
        require(_amountUsd > 0, "STAKING: INVALID AMOUNT TO CONVERT");
        uint256 effectDecimalCurrency = getEffectDecimalForCurrency();        
        uint256 rewardInUsdWithDecimal = _amountUsd * effectDecimalCurrency;
    ...
    function withdrawCurrency(address _currency, uint256 _amount) public override onlyOwner {
        ...
        require(ERC20(_currency).transfer(msg.sender, _amount), "MARKETPLACE: CANNOT WITHDRAW");
    }
