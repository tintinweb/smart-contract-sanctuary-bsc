...
contract MobiusDeflation is Modifier, Util {
     ...
     */
    function joinDeflation(uint256 amountToWei, uint seed, uint salt) public isRunning nonReentrant returns (bool) {
        ...
        for(uint i = 0; i < joinCopies; i++) {
            (uint8 awardType, uint oneAward, uint twoAward, uint power) = lottery(seed, salt, joinCopies);
