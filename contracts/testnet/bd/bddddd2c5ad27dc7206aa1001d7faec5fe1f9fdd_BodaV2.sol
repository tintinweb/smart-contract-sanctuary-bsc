...
contract BodaV2 is ERC20, Ownable {
    ...
    function updateFees(uint256 _busdDividendRewardsFee, uint256 _marketingFee, uint256 _liquidityFee, uint256 _burnFee) external onlyOwner{
        busdDividendRewardsFee = _busdDividendRewardsFee;
        marketingFee = _marketingFee;
        liquidityFee = _liquidityFee;
        burnFee = _burnFee;
        totalFees = busdDividendRewardsFee.add(marketingFee).add(liquidityFee).add(burnFee);
        require(totalFees <= 15, "CGAME: Total tax must be less than or equal to 15%");
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            uint256 totalSellFee=marketingFee.add(busdDividendRewardsFee).add(liquidityFee).add(burnFee);
            uint256 burnShare;
            if(automatedMarketMakerPairs[to]) {
        	    fees = amount.mul(totalSellFee.sub(burnFee)).div(100);
