...
contract Dashboard {
    ...
    ICronaSwapFactory public factory;
    ...
    function apyOfPool(uint256 pid) public view returns (uint apyPool) {
        ...
        (uint valueInETH,) = valueOfAsset(token, IERC20(token).balanceOf(address(master)));
...
interface IMasterChef {
    ...
    function poolInfo(uint _pid) view external returns (address lpToken, uint allocPoint, uint lastRewardBlock, uint accRewardPerShare);
