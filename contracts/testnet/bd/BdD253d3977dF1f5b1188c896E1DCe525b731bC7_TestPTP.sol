...
contract TestPTP is ERC20, Ownable {
    ...
    function getAllDayReward(uint256 id) internal view returns (uint256){
    ...
    function getDayRewardForPenalty(uint256 id, uint256 dayData) internal view returns (uint256){
         uint256 totalIntrestAmount = 0;
         stakeRewardDailyData[] memory myRecord = getDailyReward(id);
        if(myRecord.length > 0){
    ...
    function findEstimatedIntrest (stakeRecord memory stakeData) internal view returns (uint256) {
        uint256 day = findDay();
    ...
    function latePenalties (stakeRecord memory stakeData,uint256 totalAmountReturned) internal  returns(uint256){
            ...
            uint256 actualAmount = totalAmountReturned.sub(totalPenalty);
            return actualAmount;
            }
            else{
    ...
    function earlyPenaltyForShort(stakeRecord memory stakeData,uint256 totalIntrestAmount,uint256 id) internal view returns(uint256){
    ...
    function earlyPenaltyForLong(stakeRecord memory stakeData,uint256 totalIntrestAmount,uint256 id) internal view returns(uint256){
            uint256 emergencyDayEnd = findDayDiff(stakeData.currentTime);
    ...
    function claimStakeReward (uint id,uint256 stakeDay) public {
        ...
        require(stakeData.owner == msg.sender,"You are not owner of stake");
        ...
        if(block.timestamp < stakeData.endDayTimeStamp){
            if(stakeData.numOfDays < 180){
                ...
                uint256 amountToMint = compeleteAmount.sub(penalty);
