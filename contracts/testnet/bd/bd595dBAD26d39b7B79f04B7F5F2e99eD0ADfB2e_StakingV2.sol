...
contract StakingV2 is Initializable {
    ...
    function distributeRewards() external payable onlyOwner {
        ...
        for(uint256 i = 0;i<stakeIDs.length;i++){ //for each stakeID
            if(stakes[stakeIDs[i]].value >= MIN_STAKE_AMOUNT){ //check the stake exists and has enough balance
                if(block.timestamp - stakes[stakeIDs[i]].time >= MIN_STAKE_PERIOD){ //check the duration of stake is long enough
                    totalPayableStakes += stakes[stakeIDs[i]].value; //add to total payable stake sum
                }
            }
        }

       uint256 perToken = (address(this).balance * 1 ether) / totalPayableStakes;

       for(uint256 i = 0;i<stakeIDs.length;i++){ //for each stakeID
           if(stakes[stakeIDs[i]].value >= MIN_STAKE_AMOUNT){ //check the stake exists and has enough balance
               if(block.timestamp - stakes[stakeIDs[i]].time >= MIN_STAKE_PERIOD){ //check the duration of stake is long enough
                    uint256 payableAmount = (stakes[stakeIDs[i]].value / 1 ether) * perToken;  //calculate what the stake has earned
    ...
    function getTotalPayableStake() external view returns(uint256, uint256,uint256) {
        uint256 totalPayableStakes = 0;

        for(uint256 i = 0;i<stakeIDs.length;i++){ //for each stakeID
            if(stakes[stakeIDs[i]].value >= MIN_STAKE_AMOUNT){ //check the stake exists
                if(block.timestamp - stakes[stakeIDs[i]].time >= MIN_STAKE_PERIOD){ //check the duration of stake is long enough
                    totalPayableStakes += stakes[stakeIDs[i]].value; //add to total payable stake sum
                }
            }
       ...
       uint256 perToken = (address(this).balance * 1 ether) / totalPayableStakes;
       uint256 payableAmount = (stakes[1].value / 1 ether) * perToken;

       return (stakes[1].value,payableAmount,perToken);
