...
import "../libraries/FarmViewLibrary.sol";
...
contract CrossFarm is Node, ICrossFarm, BaseRelayRecipient, SessionManager {
    ...
    function add(
        ...
        totalAllocPoint = FarmViewLibrary.addPool(
            _allocPoint,
            _lpToken,
            _depositFeeRate,
            _withdrawLock,
    ...
    function set(
        ...
        require(_depositFeeRate <= depositFeeLimit, sInvalidFee);
        if (_withUpdate) massUpdatePools();
        totalAllocPoint = FarmViewLibrary.setPool(
            poolInfo,
            _pid,
            _allocPoint,
            _depositFeeRate,
    ...
    function getUserState(uint256 pid, address userAddress) external view validPid(pid) returns (UserState memory) {
        return FarmViewLibrary.getUserState(userAddress, pid, poolInfo, userInfo, nodes, farmParams, vestMonths);
    }
