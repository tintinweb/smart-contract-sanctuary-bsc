...
contract AutomationBalancer is AutomationCompatible, ReentrancyGuard, Ownable {
    //---------- Libraries ----------//
    ...
    IJlpPool public immutable pool;
    ...
    uint256 public balancePoints;
    bool public enabled;

    //---------- Storage -----------//
    EnumerableSet.AddressSet private vaultTokens; // Mapping of vault tokens.
    mapping(address => uint256) public minTokenBal; // Min amount to perform balancer.
    ...
    function _wrap(uint256 _amount) private {
    ...
    function _validateCooldown(uint256 _lastAddedAt)
        private
        view
        returns (bool)
    {
        return _lastAddedAt.add(cooldownDuration) <= block.timestamp;
    }

    function _stakeToPool(address _token, uint256 _amount) private {
        require(_amount > 0, "Zero Amount");
        if (
            IERC20(_token).allowance(address(this), address(pool)) <
            _amount
        ) {
            IERC20(_token).approve(address(pool), type(uint256).max);
        }
        pool.stake(_token, _amount, 0, 0);
    }

    function _withdrawnFromPool(address _token, uint256 _amount) private {
        require(_amount > 0, "Zero Amount");
        pool.withdrawn(_token, _amount, 0, address(this));
    }

    function _getLowFeeToken(uint256 _amount)
        private
        view
        returns (address token)
    {
        uint256 lowerFee;
        address lowerToken;
        uint256 tokensLenght = vaultTokens.length();
        for (uint256 i = 0; i < tokensLenght; i++) {
            address _token = vaultTokens.at(i);
            uint256 fee = vaultUtils.getSellUsdjFeeBasisPoints(_token, _amount);
            if (lowerFee == 0) {
                lowerFee = fee;
            }
            if (fee <= lowerFee) {
    ...
    function _needUpkeep() private view returns (bool need, address token) {
        (uint256 stakedBal, uint256 lastAddedAt, ) = stakeInfo();
        if (!enabled) {
            return (false, address(0));
        }
        if (stakedBal < BASIS_POINTS_DIVISOR) {
            return (false, address(0));
        }
        if (!_validateCooldown(lastAddedAt)) {
            return (false, address(0));
        }
        ...
        for (uint256 i = 0; i < tokensLenght; i++) {
            ...
            uint256 minBalance = minTokenBal[_token];
            if (poolAmount <= minBalance) {
                return (true, _token);
            }
        }
    ...
    function _balanceTokens(address _requiredToken, uint256 _toWithdrawn)
        private
    {
        address withdrawnToken = _getLowFeeToken(_toWithdrawn);
        _withdrawnFromPool(withdrawnToken, _toWithdrawn);
        uint256 balanceETH = address(this).balance;
        if (balanceETH > 0) {
            _wrap(balanceETH);
        }
        _swap(withdrawnToken, _requiredToken);
        uint256 balance = IERC20(_requiredToken).balanceOf(address(this));
        _stakeToPool(_requiredToken, balance);
    }
    ...
    function performUpkeep(
        ...
        (bool need, address requiredToken) = _needUpkeep();
        require(need, "Not necessary");
        (uint256 stakedBal, , ) = stakeInfo();
        uint256 toWithdrawn = stakedBal.mul(balancePoints).div(
            BASIS_POINTS_DIVISOR
        );
        _balanceTokens(requiredToken, toWithdrawn);
    }
    ...
    function addVaultToken(address _token, uint256 _min) external onlyOwner {
        require(_token != address(0x0), "Zero address");
        require(_min > 0, "Zero min");
        vaultTokens.add(_token);
        minTokenBal[_token] = _min;
    ...
    function removeVaultToken(address _token) external onlyOwner {
        ...
        delete minTokenBal[_token];
        emit VaultTokenRemoved(_token);
    }

    function setMinTokenBal(address _token, uint256 _min) external onlyOwner {
        require(_min > 0, "Zero min");
        require(vaultTokens.contains(_token), "Token not exists");
        minTokenBal[_token] = _min;
        emit VaultTokenRemoved(_token);
    ...
    function setBalancePoints(uint256 _balancePoints) external onlyOwner {
        require(
            _balancePoints > 0 && _balancePoints <= BASIS_POINTS_DIVISOR,
            "Invalid points"
        );
        balancePoints = _balancePoints;
    }
    ...
    function wrap(uint256 amount) external onlyOwner {
        uint256 balanceETH = address(this).balance;
        amount = amount > balanceETH ? balanceETH : amount;
        require(amount > 0, "Zero amount");
        _wrap(amount);
    }

    function swap(address _tokenIn, address _tokenOut) external onlyOwner {
        require(vaultTokens.contains(_tokenIn), "Invalid _tokenIn");
        require(vaultTokens.contains(_tokenOut), "Invalid _tokenOut");
        _swap(_tokenIn, _tokenOut);
    }

    function stakeToPool(address _token, uint256 _amount) external onlyOwner {
        require(vaultTokens.contains(_token), "Invalid token");
        _stakeToPool(_token, _amount);
    }

    function withdrawnFromPool(address _token, uint256 _amount)
        external
        onlyOwner
    {
        require(vaultTokens.contains(_token), "Invalid token");
        (uint256 stakedBal, uint256 lastAddedAt, ) = stakeInfo();
        require(stakedBal > 0, "Zero staked");
        require(_validateCooldown(lastAddedAt), "Too soon");
        _withdrawnFromPool(_token, _amount);
    }
    ...
    function emergencyWithdrawn() external onlyOwner {
        pool.emergencyWithdrawn();
    }

    function setEnabled(bool _set) external onlyOwner {
        enabled = _set;
    }
