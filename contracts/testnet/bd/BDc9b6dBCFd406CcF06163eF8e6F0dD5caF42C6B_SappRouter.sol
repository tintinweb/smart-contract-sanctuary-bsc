...
contract SappRouter is Ownable{
    ...
    uint private appFeeMax; // 1 ETH
    constructor(address _router, address _token, address _wallet, uint _appFee, uint _appFeeMax) public {
        require(address(_router) != address(0), "Address 0 Error");
        require(address(_token) != address(0), "Address 0 Error");
        require(address(_wallet) != address(0), "Address 0 Error");
        require(_appFee <  _appFeeMax, "App Fee Error");

        router = _router;
        appToken = _token;
        wallet = _wallet;
        appFee = _appFee;
        appFeeMax = _appFeeMax;
    }
    ...
    function setAppFeeMax(uint _appFee) public onlyOwner returns(bool){
        require(_appFee <  appFeeMax, "App Fee Error");
        _setAppFee(_appFee);
        return true;
    }

    function setAppFee(uint _appFeeMax) public onlyOwner returns(bool){
        _setAppFeeMax(_appFeeMax);
        return true;
    }
    ...
    function _setAppFeeMax(uint _appFeeMax) internal {
        appFeeMax = _appFeeMax;
    }
    ...
    function getAppFeeMax() public view returns (uint) {
        return appFeeMax;
    }
