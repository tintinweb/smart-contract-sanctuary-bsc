...
contract SST is IERC20, Ownable {
    ...
    function setTaxFee(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(_taxFee + _liquidityFee + _charityFee <= 10**4 / 4, "Total fee is over 25%");
    }

    function setLiquidityFee(uint256 liquidityFeeBps) external onlyOwner {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{ value: ethAmount }(
            ...
            0, // slippage is unavoidable
            address(0xdead), // send liquidity to burn address
            block.timestamp
        );
    }
