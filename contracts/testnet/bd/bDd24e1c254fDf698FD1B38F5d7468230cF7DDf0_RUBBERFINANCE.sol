...
contract RUBBERFINANCE is ReentrancyGuard{
  ...
  function setRubberToken(address token) public {
    ...
    function getWinMultiplier(address user) public view returns(uint256){
        uint256 latestRound=chainLinkPrice.latestRound();
        uint256 currentChainLinkTimeStamp=chainLinkPrice.getTimestamp(latestRound);
        require(minimumTimestampForAnswer[user]<currentChainLinkTimeStamp,"Your contract has not been unlocked!");
        require(block.timestamp<maximumTimeStampForAnswer[user],"Your contract has expired!");        
        int256 currentBTCPrice=chainLinkPrice.latestAnswer();
        uint256 winMultiplier=SafeMath.div(SafeMath.mul(SafeMath.mul(getPenalty(user),dividor),100),maximumOffset);
        return winMultiplier;
    }

    function getWinAmount(address user) public view returns(uint256){
        uint256 latestRound=chainLinkPrice.latestRound();
        uint256 currentChainLinkTimeStamp=chainLinkPrice.getTimestamp(latestRound);
        require(minimumTimestampForAnswer[user]<currentChainLinkTimeStamp,"Your contract has not been unlocked!");
        require(block.timestamp<maximumTimeStampForAnswer[user],"Your contract has expired!");             
        uint256 winAmount=SafeMath.add(contractValue[user],SafeMath.mul(SafeMath.div(contractValue[user],100),getWinMultiplier(user)));
        return winAmount;
    }
