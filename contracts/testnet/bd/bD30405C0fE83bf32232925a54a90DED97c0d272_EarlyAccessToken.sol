...
contract EarlyAccessToken is ERC721URIStorage, AccessControl {
    ...
    constructor(address _admin, string memory _uri)
        ERC721("EARLY ACCESS TOKEN", "EAT")
    {
        admin = IAdmin(_admin);
        URI = _uri;
    ...
    function biddingNFT(uint256 amount) public {
        require(
            Bidding[msg.sender] + amount > highestBid,
            "EarlyAccessToken: Should bid greater than the current highest bid"
        );
        require(
 ...
 // Mention the duration in hours
    function setTop(uint256 _top) public onlyRole(ADMIN_ROLE) {
        top = _top;
    }
    function ClaimBid() public {
        ...
        if (TopOfTheAuction[Won[msg.sender]] == msg.sender) {
    ...
    function endTheAuction() external onlyRole(ADMIN_ROLE) {
        require(
            block.timestamp > startTime + duration,
            "EarlyAccessToken: Auction has not ended yet"
        );
        uint rank =1;
        for (uint i = 1; TopOfTheAuction[top] == address(0); i++) {
            while(RankRegistered[top24[top24.length - i]] != true){
            TopOfTheAuction[rank] = top24[top24.length - i];
            Won[top24[top24.length - i]] = i;
