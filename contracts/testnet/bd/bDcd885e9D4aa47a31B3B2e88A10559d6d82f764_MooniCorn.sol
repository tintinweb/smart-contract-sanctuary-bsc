...
contract MooniCorn is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    address payable public SuitableFeeAddress;
   // address payable public MarketingFee;
    bool inHandleFee = false;

    event HandleFeeEnabledUpdated(bool enabled);
    event HandleFee(
    ...
    function setSuitableFeeAddressAdd(address payable  _SuitableFeeAddress) external onlyOwner() {
          SuitableFeeAddress = _SuitableFeeAddress;
    }
 //   function setMarketingFeeAdd(address payable  _MarketingFee) external onlyOwner() {
 //         MarketingFee = _MarketingFee;
 //   }


    function setHandelingFeePercent(uint256 handelingFee) external onlyOwner() {
        _handelingFee = handelingFee;
    }

    function setHandleFeeEnabled(bool _enabled) public onlyOwner {
        handleFeeEnabled = _enabled;
        emit HandleFeeEnabledUpdated(_enabled);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateHandelingFee(tAmount);
    ...
    function calculateHandelingFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_handelingFee).div(
            10 ** 2
        );
    }

    function removeAllFee() private {
        if (_taxFee == 0 && _handelingFee == 0) return;

        _previousTaxFee = _taxFee;
        _previousHandelingFee = _handelingFee;
    ...
    function restoreAllFee() private {
        ...
        _handelingFee = _previousHandelingFee;
    }
    ...
    bool public handleFeeEnabled = false; // should be true
    ...
    uint256 public _handelingFee = 8; // 4% will be added to marketing, and 4% for team allocation
    uint256 private _previousHandelingFee = _handelingFee;
    ...
    function handleFee(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) // swap 1 time
            ...
            // send fee to marketing and development
            // Utils.addLiquidity(address(pancakeRouter), owner(), otherPiece, bnbToBeAddedToLiquidity);
           /// IBEP20(address(this)).transfer(MarketingFee,otherPiece);
            SuitableFeeAddress.transfer(bnbToBeAddedToLiquidity);
            emit HandleFee(piece, deltaBalance, otherPiece);
        }
    }
