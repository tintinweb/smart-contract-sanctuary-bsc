...
contract ICO is OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function fundICO(uint8 _roundId, uint256 _investFund) external nonReentrant returns (bool) {
    ...
    ico.token.transferFrom(msg.sender, address(this), _investFund);
    ...
    _vestingContract.addWhitelist(msg.sender, investor.tokenReceived, _roundId);
  ...
  function claimInnovatorFund(uint8 _roundId) external nonReentrant returns (bool) {
    ...
    require(
      ico.token.balanceOf(address(this)) >= ico.totalInvestFund,
    ...
    /** 
      ...
      */
    ico.token.transfer(ico.innovatorWallet, ico.totalInvestFund);

    emit ClaimFund(_roundId, ico.innovatorWallet, ico.totalInvestFund);
    return true;
  }
