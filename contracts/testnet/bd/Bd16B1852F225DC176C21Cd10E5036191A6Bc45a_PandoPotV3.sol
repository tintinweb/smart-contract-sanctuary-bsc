...
//    }
    ...
    function buyTickets(address _receiver, uint256[] memory _tickets, uint256 _roundID) external availableBuyTicket(_receiver, _roundID) onlyEOA whenNotPaused nonReentrant{
        ...
        uint256 digitMega = typePrize[PRIZE_TYPE.MEGA].digit;
        uint256 digitMinor = typePrize[PRIZE_TYPE.MINOR].digit;
        uint256 digitMini = typePrize[PRIZE_TYPE.MINI].digit;
        for (uint256 i = 0; i < _tickets.length; i++) {
            tickets[_roundID][PRIZE_TYPE.MEGA][_tickets[i] % digitMega][_receiver]++;
            tickets[_roundID][PRIZE_TYPE.MINOR][ _tickets[i] % digitMinor][_receiver]++;
            tickets[_roundID][PRIZE_TYPE.MINI][_tickets[i] % digitMini][_receiver]++;

            nTickets[_roundID][PRIZE_TYPE.MEGA][_tickets[i] % digitMega]++;
            nTickets[_roundID][PRIZE_TYPE.MINOR][ _tickets[i] % digitMinor]++;
            nTickets[_roundID][PRIZE_TYPE.MINI][_tickets[i] % digitMini]++;
        }
...
//        require(_type < 3, 'PandoPot: invalid type');
    ...
    function finishRound() external onlyRNG {
        ...
        currentRoundId++;
        updatePandoPot();
        emit RoundIdUpdated(currentRoundId-1);
    }
