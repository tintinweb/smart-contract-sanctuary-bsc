...
import "./interfaces/ILaunchpadCrowdsaleNonEthNft.sol";

/**
 * @title LaunchpadCrowdsaleNonEthNft
 * @author Enjinstarter
 * @dev Launchpad crowdsale with non ETH address collection where there is no delivery of tokens in each purchase.
 */
contract LaunchpadCrowdsaleNonEthNft is
    NoDeliveryCrowdsale,
    CappedTokenSoldCrowdsaleHelper,
    TimedCrowdsaleHelper,
    LaunchpadWhitelistCrowdsaleHelper,
    Pausable,
    ILaunchpadCrowdsaleNonEthNft
{
    ...
    modifier onlyBy(address account) {
        require(
            msg.sender == account,
            "LaunchpadCrowdsaleNonEthNft: sender unauthorized"
        );
        _;
    }
    ...
    function buyTokens(
        ...
        uint256 lots,
        string calldata nonEthAddress
    ) external override {
        require(
            bytes(nonEthAddress).length > 0,
            "LaunchpadCrowdsaleNonEthNft: empty non ETH address"
        );

        _buyTokensFor(msg.sender, paymentToken, lots, nonEthAddress);
    }
    ...
    function buyTokensFor(
        ...
        _buyTokensFor(beneficiary, paymentToken, lots, nonEthAddress);
    }
        ...
        onlyBy(governanceAccount)
    {
        require(
            account != address(0),
            "LaunchpadCrowdsaleNonEthNft: zero account"
        );
        ...
        isWhitelisted(beneficiary)
    {
        ...
        if (
            ...
            revert("LaunchpadCrowdsaleNonEthNft: beneficiary cap exceeded");
        }
...
/**
 * @title ILaunchpadCrowdsaleNonEthNft
 * @author Enjinstarter
 */
interface ILaunchpadCrowdsaleNonEthNft is ICrowdsale {
    function buyTokens(
        address paymentToken,
        uint256 lots,
        string calldata nonEthAddress
