...
contract GB is ERC20, Ownable {
    ...
    //购买数量
    uint256 public _maxTxAmount = 4 * 10**decimals();
    uint256 public _walletMax = 8 * 10**decimals();

    mapping(address => bool) public _isBlacklisted;
    uint256 public tradingEnabledTimestamp = 1627779600;
    ...
    constructor(
        ...
        swapTokensAtAmount = totalSupply.mul(2).div(10**6); // 总供应量 * 2/1000000
    ...
    function addBot(address recipient) private {
        if (!_isEnemy[recipient]) _isEnemy[recipient] = true;
    }
        ...
        if (!_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            ...
            if (!automatedMarketMakerPairs[to]){
                require(
                    ...
                    "ERC20: receiver amount exceeds max amount"
                );
            }
        }

        if (amount == 0) {
            super._transfer(from, to, 0); //转0个币则直接转
            return;
        }
        // TODO:判断是否到开盘时间
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        // 添加tokens个流动性 for Cake  当前合约账户添加tokens个流动性
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth  成unswap pair周边合约代币路径 -> 用eth位来表示
