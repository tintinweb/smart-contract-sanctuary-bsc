...
function topUp(uint256 _amount) public{
    ...
    function payReferral(uint _level, address _user, uint _value) internal {
     ...
     function withdrawROI() public {
        ...
        uint256 numDays = (block.timestamp - freezeTime[msg.sender])/60;
        if (numDays>1){
       ...
       function withdrawableROI(address _address) public view returns(uint reward) {

        uint256 numDays = (block.timestamp - freezeTime[_address])/60;
        if (numDays>1){
              // Increase the reward by 0.01% per day
       reward = (( (users[_address].stakedToken) * numDays * 1 ) / 1000 );
        }
