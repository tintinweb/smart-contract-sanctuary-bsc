...
contract StakingPoint is IStakingPoint, Ownable {
    ...
    function depositLpToken(uint256 pid, address token, uint256 amount) public view returns(uint256) {
        uint256 usdAmount = _calculateUsdInLp(token, amount);
        return usdAmount * lpTokenDepositPoint[pid][token] / (10**IERC20(token).decimals());
    }

    function _calculateUsdInLp(address lpToken, uint256 liquidity) private view returns(uint256) {
