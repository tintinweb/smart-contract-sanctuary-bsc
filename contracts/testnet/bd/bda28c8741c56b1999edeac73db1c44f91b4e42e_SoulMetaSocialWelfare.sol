...
contract SoulMetaSocialWelfare is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC721HolderUpgradeable{
    ...
    function setWelfareData(uint day, uint todayTotal, uint receiveTotal, uint canClaim, uint dayClaim, uint userTotal, uint validUser) public onlyOwner{
        welfareData.day = day;
    ...
    function _setReceive() private{
        //当天
        uint sameDay = block.timestamp / 60;
        if(welfareData.day != sameDay){
            //数据不是当天的，需要更新
            uint contractTotal = contractAddr.usdt.balanceOf(address(this));   //当前合约总额
            uint receiveTotal = (contractTotal - welfareData.receiveTotal) * percent / 100;     //今日发放额度 = 当前总额度 - 发放总额剩余的
            welfareData.day = sameDay;
            welfareData.todayTotal = receiveTotal;
            welfareData.receiveTotal = receiveTotal;
            welfareData.canClaim += receiveTotal / welfareData.userTotal;
            welfareData.dayClaim = receiveTotal / welfareData.userTotal;
    ...
    **/
    function activation(uint[] memory tokenIds_) public {
        ...
        for(uint i=0; i < tokenIds_.length; i++){
            ...
            //时间定为昨天
            userData[tokenIds_[i]].receiveDay = block.timestamp / 60;
            //用户已经领取的份额
