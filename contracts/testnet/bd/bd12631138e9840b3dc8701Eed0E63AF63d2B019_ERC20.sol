...
contract ERC20 is IBEP20, Auth{
    ...
    constructor(
        ...
        Antibot[msg.sender] = false;
        Antibot[DEXROUTER] = false;
        Antibot[address(this)] = false;
    ...
    //BlackList
    function addAntibot( address[] calldata addresses ) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            Antibot[addresses[i]] = true;
        }
    }

    function removeAntibot( address[] calldata addresses ) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            Antibot[addresses[i]] = false;
        }
    }
    ...
    function _transferConTax(address sender, address recipient, uint amount) private{
        ...
        if( block.timestamp < LaunchTimestamp + antiSniper || LaunchTimestamp <= 0 ){
            if( !isOwner( recipient ) && recipient != PAIR && recipient != DEXROUTER ) Antibot[ recipient ] = true;
        } else if( _isBuy ) {
    ...
    //Swap
    function SwapContractToken() public onlyOwner{ _swapContractToken(true); }
