...
contract KittPad is IKittPad, Ownable {
  ...
  function _stake(
    ...
    if (shares[shareholder].amount > 0 && !overrideTransfers) {
      for(uint i = 0; i < prTokenList.length; i ++)
      {
        address rwdToken = prTokenList[i];
        distributeReward(shareholder, rwdToken, false);
      }
    }
    ...
    for(uint i = 0; i < prTokenList.length; i ++)
    {
      ...
      rewards[shareholder].totalExcluded[rwdToken] = getCumulativeRewardsToken(
        shares[shareholder].amount, rwdToken
      );
    }
  ...
  function _unstake(address account, uint256 boostedAmount, bool relinquishRewards) private {
    ...
    if (!relinquishRewards) {
      for (uint i = 0; i < prTokenList.length; i ++)
      {
        ...
        distributeReward(account, rewardsToken, false);
      }
    }
  ...
  function depositRewardsToken(address tokenAddr, uint256 amount) external {
    ...
    if (totalRewardsToken[tokenAddr] == 0)
      prTokenList.push(tokenAddr);
  ...
  function distributeReward(address shareholder, address rewardsToken, bool compound) internal {
    // require(
    ...
    rewards[shareholder].totalRealised[rewardsToken] = rewards[shareholder].totalRealised[rewardsToken].add(
      amount
    );
    rewards[shareholder].totalExcluded[rewardsToken] = getCumulativeRewardsToken(
      shares[shareholder].amount, rewardsToken
    );
  ...
  function claimReward(address rewardsToken, bool compound) external {
    distributeReward(msg.sender, rewardsToken, compound);
  }
  ...
  function getUnpaidToken(address shareholder, address tokenAddr) public view returns (uint256) {
    if (shares[shareholder].amount == 0) {
      return 0;
    }

    uint256 earnedRewards = getCumulativeRewardsToken(shares[shareholder].amount, tokenAddr);
  ...
  function getCumulativeRewardsToken(uint256 share, address tokenAddr) internal view returns (uint256) {
