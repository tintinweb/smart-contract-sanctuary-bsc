...
// File contracts/lplocker/LpTokenLocker.sol
...
until the specified unlock date has been reached.
 ...
 • Telegram Announcements Channel:    https://t.me/xxx
 • Telegram Main Channel:             https://t.me/xxx
 • Twitter Page:                      https://twitter.com/xxx
 • Reddit:                            https://www.reddit.com/r/xxx
...
contract LpTokenLocker is Ownable, ReentrancyGuard {
    ...
    *                       1 - pay fees in SBS + LP token,
    *                       2 - pay fees fully in BNB,
    *                       3 - pay fees fully in SBS
    *                       4 - free lock by locking SBS for the lock lifetime
    ...
    */
    function lockTokens(address lpToken, uint256 amount, uint256 unlockTime,
        ...
        { //avoid stack too deep error
            (uint256 ethFee, uint256 tokenFee, uint256 lpTokenFeeAmount, uint256 sbsToLockAmount, uint256 referralPercentScaled) =
                feesCalculator.calculateFees(lpToken, amount, unlockTime, feePaymentMode, referral, msg.sender);
            lpTokenFee = lpTokenFeeAmount;
            sbsToLock = sbsToLockAmount;
            require(lpTokenFee <= amount.div(100), "LP FEE EXCEEDS 1%"); //safeguard for lp token fee
            transferFees(lpToken, ethFee, tokenFee, lpTokenFee, sbsToLock, referral, referralPercentScaled);
    ...
    */
    ...
    external payable nonReentrant onlyLockOwner(lockId) {
        ...
        (uint256 ethFee, uint256 tokenFee, uint256 lpTokenFee, uint256 sbsToLock) =
        ...
        transferFees(lock.lpToken, ethFee, tokenFee, lpTokenFee, sbsToLock, address(0), 0);
        if(msg.value > ethFee) { // transfer excess back
            transferBnb(msg.sender, msg.value.sub(ethFee));
        }
        ...
        lock.lockedSbs = lock.lockedSbs.add(sbsToLock);
    ...
    */
    function withdrawPartially(uint256 lockId, uint256 amount) public nonReentrant onlyLockOwner(lockId) {
        ...
        if(lock.tokenAmount == 0) {
            if(lock.lockedSbs > 0) {
                feeToken.safeTransfer(lock.owner, lock.lockedSbs);
            }
    ...
    /**
    * @notice transfer lock ownership to another account. If sbsTokens were locked as a paymentFee, the new owner
    ...
    */
    ...
    function transferFees(
        ...
        if (sbsToLock > 0) {
            require(address(feeToken) != address(0), "TOKEN FEE TYPE NOT SUPPORTED");
            feeToken.safeTransferFrom(msg.sender, address(this), sbsToLock);
        }
    }
...
// File contracts/migrator/UniswapV2Library.sol
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'a365621954ea94a7bae9269221fe688cd35da5d39f4e98a19d0940ee5bcfdb47' // init code hash
            ))));
    }
