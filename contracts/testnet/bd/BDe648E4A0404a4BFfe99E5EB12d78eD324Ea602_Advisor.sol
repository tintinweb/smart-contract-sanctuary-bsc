...
contract Advisor is Pausable, ReentrancyGuard {
  ...
  EnumerableSet.AddressSet advisors;

  event AdvisorAdded(address advisor);
  event AdvisorRemoved(address advisor);
  event TokenUnvested(address receiver, uint256 amount);

  modifier onlyAdmin() {
    require(sspAccessControls.hasAdminRole(_msgSender()), "Advisor: only admin");
    _;
  }

  constructor(address _ssp, address _sspAccessControls, address[] memory _advisors) {
    require(_ssp != address(0), "Advisor: zero address");
    require(_sspAccessControls != address(0), "Advisor: zero address");
    require(_advisors.length > 0, "Advisor: empty advisors");
    ...
    for (uint256 i = 0; i < _advisors.length; i++) {
      require(_advisors[i] != address(0), "Advisor: invalid member");
      advisors.add(_advisors[i]);
    }
  }

  function getAdvisors() public view returns (address[] memory _advisors) {
    _advisors = new address[](advisors.length());
    for (uint256 i = 0; i < advisors.length(); i++) {
      _advisors[i] = advisors.at(i);
    }
  }

  function canUnvest(uint256 period, address member) public view returns (bool) {
    require(period > 0 && period <= 6, "Advisor: invalid period");
    return advisors.contains(member) &&
  ...
  function addAdvisor(address advisor, uint256 period) external onlyAdmin whenPaused {
    require(period > 0 && period <= 6, "Advisor: invalid period");
    advisors.add(advisor);
    for (uint256 i = 1; i <= period; i++) {
      unvestInfo[i][advisor].claimed = true;
    }
    emit AdvisorAdded(advisor);
  }

  function removeAdvisor(address advisor) external onlyAdmin whenPaused {
    advisors.remove(advisor);
    emit AdvisorRemoved(advisor);
  }

  function unvest(uint256 period) public nonReentrant whenNotPaused {
    require(canUnvest(period, _msgSender()), "Advisor: cannot unvest");
  ...
  function _tokenUnvestPerPeriod(uint256 _period) private view returns (uint256) {
    return (VESTING[_period - 1] * ssp.maxTotalSupply()) / (advisors.length() * 100000);
  }
}
