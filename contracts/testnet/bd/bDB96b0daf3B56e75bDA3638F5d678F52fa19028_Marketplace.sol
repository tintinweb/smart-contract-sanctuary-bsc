...
contract Marketplace is ReentrancyGuard {
    ...
    uint256 private box1Price = 0; // 1 gwei;
    uint256 private box2Price = 0; // 1 gwei;
    uint256 private box3Price = 0; // 1 gwei;

    mapping(uint256 => MarketItem) private marketItemIdToMarketItem;

    address private istepBoxNftAddress;
    ...
    event buyBox(address investor, uint256 amount, uint256 boxType);
    ...
    function setBox1Price(uint256 _box1Price) external {
        require(owner == msg.sender, "Ownable: caller is not the owner");
        box1Price = _box1Price;
    }

    function setIstepBNftAddress(address _istepBoxNftAddress) external {
        require(owner == msg.sender, "Ownable: caller is not the owner");
        istepBoxNftAddress = _istepBoxNftAddress;
    }
    ...
    function createMarketItem(
        ...
        address creator = InterfaceISTEPNft(nftContractAddress).getTokenCreatorById(tokenId);
    ...
    function fetchMarketItemsByAddressProperty(string memory _addressProperty) public view returns (MarketItem[] memory) {
    ...
    function invest(uint256 boxType) external payable {
        if (box1Price > 0) {
            require(msg.value == box1Price, "Price must be equal to box1Price");
        }
        require(istepBoxNftAddress != address(0), "istepBoxNftAddress account cann't equal address 0x");

        payable(owner).transfer(msg.value);

        InterfaceISTEPNft(istepBoxNftAddress).mint(msg.sender, boxType);

        emit buyBox(msg.sender, msg.value, boxType);
    }
}
...
interface InterfaceISTEPNft is IERC721, IERC721Enumerable {
