...
contract RayStaking is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    
    mapping(uint256 => mapping(address => UserInfo)) public userInfo;
    mapping(uint256 => PoolInfo) public poolInfo;
    mapping(uint256 => FeeInfo) public feeInfo;
    IERC20 private  _reward = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    IERC20 private _native = IERC20(0xe6b6319B60293bB5Cbf6e1c293b73de70A8135E5);
    struct PoolInfo {
        bool  hasUserLimit;
        bool  HarvestEnabled;
    ...
    function transferPoolForHolder(uint256 _poolId,address _user,uint256 _newPoolId) external nonReentrant{//Think
        ...
        PoolInfo storage newPool = poolInfo[_newPoolId];
        require(newPool.isInitizilated , "First to Initilaze for Pool");
        if((pool.rewardToken == _native && newPool.rewardToken ==  _reward)){
           uint256 pending = ((user.amount * pool.accTokenPerShare) / pool.PRECISION_FACTOR) - user.rewardDebt;
            user.amount+=pending;
        }
        
        else if(pool.rewardToken ==  _reward && newPool.rewardToken ==  _reward && pool.withdrawLockPeriod < newPool.withdrawLockPeriod){
            uint256 pending = ((user.amount * pool.accTokenPerShare) / pool.PRECISION_FACTOR) - user.rewardDebt;
            user.rewardDebt = (user.amount * pool.accTokenPerShare ) / pool.PRECISION_FACTOR;//belki olmamali
            if (pending > 0) {
                pool.rewardToken.safeTransfer(address(msg.sender), pending);
            }
            emit Deposit(msg.sender, pending);   

        }else if(pool.rewardToken ==  _native && newPool.rewardToken ==  _native && pool.withdrawLockPeriod < newPool.withdrawLockPeriod){
             uint256 pending = ((user.amount * pool.accTokenPerShare) / pool.PRECISION_FACTOR) - user.rewardDebt;
            user.amount+=pending;
        }
        else{
        ...
        newUser.rewardDebt += user.rewardDebt;//belki olmamali
    ...
    function canWithdraw(uint256 _poolId, address _user) public view returns (bool) {
        ...
        PoolInfo storage pool = poolInfo[_poolId];
        require(user.isInitizilated , "First to Initilaze for Pool");
    ...
    function canWithdrawTime(uint256 _poolId, address _user) public view returns (uint256) {
        ...
        PoolInfo storage pool = poolInfo[_poolId];
        require(user.isInitizilated , "First to Initilaze for Pool");
        if (user.lastDepositTime + pool.withdrawLockPeriod > block.timestamp){
    ...
    function deposit(uint256 _poolId,uint256 _amount) external nonReentrant { //_amount==0 harvest
        ...
        require(pool.isInitizilated , "First to Initilaze for Pool");
        require(!user.isInitizilated || fees.reStakingMode , "First to Initilaze for Pool");
        ...
        if (_amount > 0 ) {
            ...
            if(fees.depositFee > 0) {
                uint256 fee = ( _amount * fees.depositFee )/ 10000;
                _amount = _amount -  fee;
            }
            user.lastDepositTime = block.timestamp;
    ...
    function setRule(address reward, address native) external onlyOwner {
        _reward=IERC20(reward);
        _native=IERC20(native);
    }

    function setReStakingAndHarvest(uint256 _poolId, bool reStaking, bool harvest) external onlyOwner {
        PoolInfo storage pool = poolInfo[_poolId];
        FeeInfo storage fees = feeInfo[_poolId]; 
        require(block.number < pool.startBlock, "Pool has started");
        fees.reStakingMode=reStaking;
        pool.HarvestEnabled=harvest;
    }
