...
contract DogeMints is Ownable, IERC20Metadata {
    using SafeMath for uint256; // this safemath relies on solidity's built in overflow checks

    uint8 constant _decimals = 18;

    uint256 _totalSupply = 50000000000 * (10**_decimals); // 50 Bill
    uint256 public _maxTxAmount = _totalSupply / 50; // 1 bill default

    uint256 public _walletMax = _totalSupply / 50; // 1 bill default
    ...
    uint256 public gas = 128 * 1 gwei;

    string constant _name = "DogeMints";
    string constant _symbol = "MINTS";
    ...
    //Fees are represented in 1000th percentile basis points to cover for 0.2% decimal percentages
    uint256 public marketingFee = 25; //  2.5% default marketing wallet fee
    uint256 public devFee = 25; //2.5% default developer wallet fee
    uint256 public liquidityFee = 2; // 0.2%
    ...
    uint256 public swapThreshold = 0; //0 no threshold, fee liquidation on each transaction
    ...
    constructor(
        ...
        router = IPancakeRouter02(pancakeswapRouterAddress);
    ...
    //to recieve ETH/WETH from router when swaping
    ...
    function _transferFrom(
        ...
        // will liquidate fees to wallets if balance > 0/swapThreshold
        if (
            msg.sender != pair &&
            !inSwapAndLiquify &&
    ...
    function extractFee(
        ...
        uint256 feeAmount = amount.mul(feeApplicable).div(getFeePercentile(feeApplicable));
    ...
    function marketingAndLiquidity() internal lockTheSwap {
        ...
        //gas: 30000
        (bool tmpSuccess1, ) = payable(marketingWallet).call{value: amountBNBMarketing}("");
        tmpSuccess1 = false;

        (bool tmpSuccess2, ) = payable(devWallet).call{value: amountBNBDev}("");
        tmpSuccess2 = false;
    ...
    function getFeePercentile(uint256 fee) internal pure returns (uint256 percent) {
        if (fee < 10) {
            return 100;
        } else if (fee < 100) {
            return 1000;
    ...
    /**
     * @dev fees should be set in 10 percentile basis points
     * @param newLiqFee = default 0.2% set as 2 uint256
     * @param newMarketingFee = default 2.5% set as 25
     * @param newDevFee = default 2.5% set as 25
     * @param extraSellFee = default 0%
     */
    function setFees(
        uint256 newLiqFee,
