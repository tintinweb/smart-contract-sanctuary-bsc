...
contract BullGates is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event PoolTriggered(uint256 amount);
    
    event OwnerForcedSwapBack(uint256 timestamp);

    constructor() ERC20("Bull Gates", "BULLG") {

        address _owner = 0x54Ca8bc8f49EfB85990C43C95CC6857ceAB32A3A;
        ...
        buyCharityFee = 2;
        buyPoolFee = 2;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyCharityFee + buyPoolFee;
        
        sellMarketingFee = 3;
        sellLiquidityFee = 0;
        sellCharityFee = 3;
        sellPoolFee = 3;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellCharityFee + sellPoolFee;

        marketingWallet = address(0x0403d0DDd7a9fEb45D8bB92C850D05d57c84651D); // set as marketing wallet
    	charityWallet = address(0x0403d0DDd7a9fEb45D8bB92C850D05d57c84651D); // set as charity wallet
    	poolWallet = address(0x0403d0DDd7a9fEb45D8bB92C850D05d57c84651D); // set as poolWallet
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _charityFee, uint256 _poolFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _charityFee, uint256 _poolFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        if (tokensForPool > 0 && tokensForPool <= balanceOf(address(this))) {
            _transfer(address(this), poolWallet, tokensForPool);
        }
        tokensForPool = 0;
