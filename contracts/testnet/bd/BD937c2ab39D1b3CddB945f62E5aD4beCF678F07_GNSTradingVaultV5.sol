...
contract GNSTradingVaultV5{
    ...
    event setRewardDistributorE(address rewardDistributor);

    constructor(StorageInterfaceV5 _storageT, address _rewardDistributor){
        require(address(_storageT) != address(0), "ADDRESS_0");
        require(_rewardDistributor != address(0), "ADDRESS_0");
        storageT = _storageT;
    ...
    function setRewardDistributor(address _rewardDistributor) external onlyGov{
        require(_rewardDistributor != address(0), "ADDRESS_0");
        rewardDistributor = _rewardDistributor;
        emit setRewardDistributorE(rewardDistributor);
    }
