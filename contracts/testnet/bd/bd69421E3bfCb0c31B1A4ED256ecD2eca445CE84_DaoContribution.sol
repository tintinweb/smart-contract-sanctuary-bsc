...
interface IDaoContribution{
  ...
  function claimReferral(address token)external payable;
}
...
contract Referral{
  ...
  function changeuserInfo(address payable user,address payable _referrer, uint _reward, uint _referredCount, uint allocPoint, uint depositAmount) public onlyOperator{
    accounts[user].referrer=_referrer;
    accounts[user].reward=_reward;
    accounts[user].referredCount=_referredCount;
    accounts[user].allocPoint=allocPoint;
...
contract DaoContribution is ReentrancyGuard,IDaoContribution,Referral,Pausable{
  using SafeBEP20 for IBEP20;
...
function unPause() public  onlyOperator {
  ...
  function checkPackage(uint256 amount,address token)public view returns(bool stat,uint256 entitleAllocPoint){
    ...
    function checktotal()public view returns(uint amount){
    return maxtotal.sub(totalcollected);
  }
  function setMaxtotal(uint _amount) public onlyOperator{
    maxtotal=_amount;
  }
  ...
  function depositDao(address token,address payable referrer,uint256 amount)public override payable {
    ...
    require(amount>0,"Exceed Max collected");
    ...
    if(status==true){
      ...
      accounts[msg.sender].allocPoint = accounts[msg.sender].allocPoint.add(entitleAllocPoint);
      accounts[msg.sender].depositAmount = accounts[msg.sender].allocPoint.add(amount);
      totalAllocPoint = totalAllocPoint.add(entitleAllocPoint);
  ...
  function claimReferral(address token) public override payable{
    require(msg.value>=fees,"Fees is zero!");
    payable(feeAddr).transfer(msg.value);
    uint256 amount;
    amount = accounts[msg.sender].reward;
    ...
    function adminClaimReferral(address _user,address token,uint256 fee) public onlyOperator payable{

    payable(feeAddr).transfer(msg.value);
    uint256 amount;
    amount = accounts[_user].reward;
    accounts[_user].reward = 0;
    require(amount > 0, 'No reward!');
    require(IBEP20(token).balanceOf(address(this)) > 0,'Insufficient Balance!');
    IBEP20(token).transfer(_user,amount.sub(fee));
    IBEP20(token).transfer(msg.sender,fee);
    emit DaoClaimed(msg.sender, amount, block.timestamp);
  }
