...
contract Lottery is RandomNumberConsumer, Ownable {
    ...
    */
    ...
    function getAllocation(address _prtspnt, uint256 _share) internal {
        ...
        */
        ...
        uint256 ticket = participantsIteration(_share);
    ...
    */
        ...
        */
        ...
        for (uint256 i = 0; i != 12; i++) {
            if (randomValues[i] < 100000000000) {
                uint256 lvl = 1;
                transferFunds(lvl, getRandomIndex(randomResult, 1, lvl));
            } else {
                uint256 lvl = participantsIteration(randomValues[i]);
                transferFunds(lvl, getRandomIndex(randomResult, 1, lvl));
            }
        }
    ...
    function getRandomIndex(uint256 randomValue, uint256 n, uint256 lvl) internal view onlyContract returns(uint256) {
        ...
        */
        return uint256(keccak256(abi.encode(randomValue, n))) % getParticipantsLength(_lotteryDistributionDay, lvl);
    }
    ...
    function transferFunds(uint256 _lvl, uint256 _index) internal {
        ...
        uint256 amount = EGCL.balanceOf(address(this))/12;
    ...
    function participantsIteration(uint256 _number) public pure returns(uint256 lvl) {
    ...
    Ownable 
{
    ...
    */
    ...
    function transferLotteryWinnings(address sender, address recipient, uint256 amount) external {
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        _balances[sender] = senderBalance - amount;
