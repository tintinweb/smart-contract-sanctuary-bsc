...
contract MMMTickets is Context, Initializable {
    ...
    function _transferNetFees() internal {
        ...
        uint256 mlm = usd.mul(35).div(100);
        uint256 maintain = usd.mul(3).div(100);
        uint256 dev = usd.mul(5).div(1000);
        uint256 marketing = usd.mul(65).div(1000);
        uint256 growingValue = usd.mul(5).div(100);
        address growingMarket = 0x83D4eBF45841Eb3cB60C3455bc3dE5470E1d24A4;
        ...
        require(IBEP20(busdAddr).transfer(growingMarket, growingValue),"Transfer fee failed!!!");
    }
    ...
    function mintBonuses(address[] memory users, uint[] memory values) external onlyOwner {
        ...
        for(uint i = 0; i < users.length; i++) {
           if(_bonusCount >= 3000) break;
           if(_bonusCount.add(values[i]) > 3000) values[i] = 3000 - _bonusCount;
    ...
    function mint20Ticket(address[] memory users, uint[] memory values) external onlyOwner {
        require(users.length == values.length,"length not match");
        for(uint i = 0; i < users.length; i++) {
            if(_tick20Count >= 9000) break;
            if(_tick20Count.add(values[i]) > 9000) values[i] = 9000 - _tick20Count;
    ...
    function _addTicketBonus(address who, uint amount) private {
        ...
        if(isBonusTime() && _bonusCount < 3000) {
            if(_tickets_bonus[who].add(addAmount).add(_bonusCount) >= 3000) {
                addAmount =  3000 - _bonusCount;
                _isBonusTime = false;
            }
    ...
    function _addTicket20(address who, uint amount) private {
        uint addAmount = amount;
        if(isBonusTime() && _tick20Count < 9000) {
            if(_tickets_20[who].add(addAmount).add(_tick20Count) >= 9000) {
                addAmount =  9000 - _tick20Count;
                _isFree20Time = false;
            }
