...
contract CPTOKENPROGRAMME {
    ...
    mapping (uint256 => mapping(string => bytes32[])) private _documentSignatures;            //  the array of signatures of a document hash mapped to a program id
    ...
    function signDocument(uint256 _programId, string calldata _hash, bytes32 signature) external returns (bool success) {
    ...
    function getDocumentSignature(uint256 _programId, string calldata _hash) external view returns (bytes32[] memory) {
