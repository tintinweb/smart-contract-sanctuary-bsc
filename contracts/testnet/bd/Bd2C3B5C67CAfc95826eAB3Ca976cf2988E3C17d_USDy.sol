    ...
    **/
    ...
    function tax( uint256 taxAmount, address sender, uint amount) internal {
        ...
        if(Admin.buyBackActivation()){
            ...
            (uint _amount, uint leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution(sender, (IERC20(Admin.BUSD()).balanceOf((address(this))) * USDy_Tax_Allocation[0])/ USDy_Tax, amount);
            if(_amount != 0){
