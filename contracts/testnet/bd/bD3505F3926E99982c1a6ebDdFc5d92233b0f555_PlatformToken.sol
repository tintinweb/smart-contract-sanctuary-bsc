...
import "./../../governance/extentions/Blacklist.sol";
    ...
    Blacklist
{
     ...
     * `subtractedValue`.
     */
    function addBacklist(address[] memory _addresses) public onlyOwner returns (bool) {
        _addBlacklist(_addresses);
        return true;
    }

    function removeBacklist(address[] memory _addresses) public onlyOwner returns (bool) {
        _removeBlackList(_addresses);
        return true;
    }

    function enableBlacklist() public onlyOwner returns (bool) {
        return _updateBlacklistStatus(true);
    }

    function disableBlacklist() public onlyOwner returns (bool) {
        return _updateBlacklistStatus(false);
    }
...
contract PlatformToken is ERC20Upgradeable {
    function initialize(uint256 initTotalSupply) public initializer {
        __Ownable_init();
        __ERC20_init("Platform Token", "$OMT", 9);
