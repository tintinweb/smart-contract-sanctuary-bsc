...
contract BnbNumberBet {
    ...
    */
    ...
    function encodedNumber(uint256 number) internal pure returns (uint256 result){
    ...
    function _newBet(uint256 betAmount, uint256 winAmount, address sender) internal {
        require(
            lockBalanceForGame.add(winAmount) < balanceForGame(betAmount),
            "Balance is not enough for game"
        );
        IERC20(_tokenBet).safeTransferFrom(sender, address(this), betAmount);
    ...
    function placeBet(
        ...
        betRange(number, isOver, amount);
        uint256 index = bets.length;
        require(minAmount > 0 && maxAmount > 0, "minAmount and maxAmount must greater than 0");
        require(
            isOver ? number >= 4 && number <= 98 : number >= 1 && number <= 95,
            "bet number not in range"
        );
        require(
            minAmount <= amount && amount <= maxAmount,
        ...
        uint256 winAmount = calculateWinAmount(number, isOver, amount);
        _newBet(amount, winAmount, msg.sender);

        totalBetOfGame += amount;
        ...
        emit NewBet(msg.sender, block.number, index, number, isOver, amount);
        uint rand = randomNumber();
        uint256 commitment = encodedNumber(rand);
        settleBet(index, rand, commitment);
    }
    ...
    function refundBet(address payable add) external onlyOperator {
        ...
        require(
            !bet.isFinished &&
        bet.player == add &&
        block.number - bet.round > 10000, "error refundBet"
        );
    ...
    function settleBet(
        ...
        require(i < bets.length, "betIndex must greater 0");

        Bet storage bet = bets[i];

        require(bet.round <= block.number, "round must < current block");
        require(!bet.isFinished, "this bet has finished");

        commit(newCommitment);

        uint256 luckyNum = getLuckyNumber(bet.index, secret);
        require(luckyNum > 0, "lucky number must greater than 0");
    ...
    function commit(uint256 _commitment) internal {
        require(0 != _commitment, "_commitment must != 0");
    ...
    function setPrizeLevel(uint256 level) external onlyOperator {
        require(PRIZE_PER_BET_LEVEL <= 1000, "level must <= 1000");
        PRIZE_PER_BET_LEVEL = level;
    }

    function setHouseEdge(uint256 value) external onlyOperator {
        require(value >= 5 && value <= 100, "value must >= 1000 and <=100"); // [0.5%, 10%]
        HOUSE_EDGE = value;
    }

    function setMinBet(uint256 value) external onlyOperator {
        require(value >= 0.05 ether && value <= 10 ether, "value must >=0.01 ether and <= 10 ether");
        MINIMUM_BET_AMOUNT = value;
    }

    function setMinToEarn(uint256 value) external onlyOperator {
        require(value >= 0.05 ether && value <= 10 ether, "value must >=0.05 ether and <= 10 ether");
    ...
    function setReferralReward(uint256 value) external onlyOperator {
        require(value >= 10 && value <= 50, "value must >=10 ether and <= 50 ether"); // [0.1%, 0.5%]
    ...
    function withdraw(uint256 amount) external onlyOperator {
        uint256 balance = getCurrentBalanceBetToken();
        require(
            amount.add(lockBalanceForGame) <= balance,
    ...
    function withdrawAllBalance() external onlyOperator isStopped {
        uint256 balance = getCurrentBalanceBetToken();
        IERC20(_tokenBet).safeTransfer(operator, balance);
    }

    function callLiquidityAdderEarn() public {
        uint256 balance = getCurrentBalanceBetToken();
        if (buyBackReserveFromEdge > balance.div(2)) {
            buyBackReserveFromEdge = balance.div(2);
        }
