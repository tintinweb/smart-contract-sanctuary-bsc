...
contract CustomToken is IERC20, Ownable {
    ...
    mapping (address => bool) public _excludedFromFees; //是否排除费用
    ...
    uint256 public hasSniper; //是否防机器人
    uint256 public hasHoldCap; //是否有持币上限
    uint256 public holdCap; //持币上限
    uint256 public hasTranCap; //是否有交易上限
    uint256 public tranCap; //交易上限
    ...
    function _transfer(
        ...
        if(_excludedFromFees[sender] || _excludedFromFees[recipient]){
            ...
            //检测
            bool checkRes = TokenManager(tokenManagerAddress).robotCheck(address(this), _isPairs[sender], _isPairs[recipient], sender, recipient, amount, launchedAt, launchedNumber);
            require(checkRes == true, "Not pass check");

            // 持币上限
            if(hasHoldCap > 0 && !_isPairs[recipient] && recipient != distributorAddress){
                require(amount + balanceOf(recipient) <= holdCap);
            }
            // 单笔交易上限
            if(hasTranCap > 0){
                require(amount <= tranCap);
            }
            ...
            if(_amountToThis > 0){
                if(_isPairs[sender]){
                    // TODO:传入的token值要确认
                    _buyAmount = _buyAmount.add(amount);
                } else {
                    _sellAmount = _sellAmount.add(amount);
                }
    ...
    function setCapOpen(
        ...
        设置代币持有上限和交易上限开关
         */
        hasHoldCap = _hasHoldCap;
        hasTranCap = _hasTranCap;
    }
