...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MASTERCHEF_MILK_PER_BLOCK = 40 * 1e18;
    uint256 public constant ACC_MILK_PRECISION = 1e18;
    ...
    /// @param _MILK The CAKE token contract address.
    ...
    function pendingCake(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accCakePerShare = accCakePerShare.add(cakeReward.mul(ACC_MILK_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accCakePerShare).div(ACC_MILK_PRECISION).sub(user.rewardDebt);
    }
    ...
    function cakePerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_MILK_PER_BLOCK.mul(cakeRateToRegularFarm).div(CAKE_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_MILK_PER_BLOCK.mul(cakeRateToSpecialFarm).div(CAKE_RATE_TOTAL_PRECISION);
        }
    }
    ...
    function cakePerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_MILK_PER_BLOCK.mul(cakeRateToBurn).div(CAKE_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accCakePerShare = pool.accCakePerShare.add((cakeReward.mul(ACC_MILK_PRECISION).div(lpSupply)));
            }
    ...
    function settlePendingCake(
        ...
        uint256 accCake = boostedAmount.mul(poolInfo[_pid].accCakePerShare).div(ACC_MILK_PRECISION);
