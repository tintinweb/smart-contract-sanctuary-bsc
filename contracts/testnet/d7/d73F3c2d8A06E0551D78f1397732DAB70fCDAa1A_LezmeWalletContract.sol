...
contract LezmeWalletContract is Ownable {
    ...
    */
    function withdrawToken(address _to, uint256 _amount, string memory _message, uint256 _expiredTime, bytes memory _signature) public {
        ...
        require(
            ILezmeSignature(lezmeVerifySignature).isWithdrawERC20Valid(_to, _amount, _message, _expiredTime, _signature),
            "Lezme: signature verification failed"
        );
...
contract LezmeUserWallet is Ownable {
    ...
    event Withdraw(string username, uint256 amount, address pool, address token, uint256 timestamp, string signature);
