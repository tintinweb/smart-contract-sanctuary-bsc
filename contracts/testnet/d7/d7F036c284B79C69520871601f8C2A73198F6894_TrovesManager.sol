...
interface ITrovesManager {
    ...
    function updateBorrowTrove(address _borrower, uint _coll, uint _debt, uint _startTime) external; 
    ...
    function updateLeverageTrove(address _borrower, uint _coll, uint _debt, uint _margin, uint _startTime) external;
    ...
    function getBorrowTroveStake(address _borrower) external view returns (uint);
    ...
    function getLeverageTroveStake(address _borrower) external view returns (uint);
    ...
    /// @dev Get 
    function getRedemptionRateWithDecay(uint _redeemedUSDA, uint _totalUSDASupply) external returns (uint);

    /// @dev **Change BaseRate** via Redemption operation
    function updateBaseRateFromRedemption(uint _redeemedUSDA, uint _totalUSDASupply) external returns (uint);
...
contract TrovesManager is TrovesBase, ITrovesManager {
    ...
    function updateBorrowTrove(address _borrower, uint _coll, uint _debt, uint _startTime) external override {
        _requireCallerIsAllowed();
        uint oldColl = borrowTroves[_borrower].coll;
        uint oldDebt = borrowTroves[_borrower].debt;
        totalColls = totalColls.add(_coll).sub(oldColl);
        totalDebts = totalDebts.add(_debt).sub(oldDebt);
        if (_coll == 0 && _debt == 0) {
            borrowTroves[_borrower].status = Status.closedByBorrow;
        ...
        if (_startTime != 0) {
            borrowTroves[_borrower].startTime = _startTime;
        }
        borrowTroves[_borrower].coll = _coll;
        borrowTroves[_borrower].debt = _debt;
        borrowTroves[_borrower].stake = _coll;
    ...
    function updateLeverageTrove(address _borrower, uint _coll, uint _debt, uint _margin, uint _startTime) external override {
        _requireCallerIsAllowed();
        uint oldColl = leverageTroves[_borrower].coll;
        uint oldDebt = leverageTroves[_borrower].debt;
        totalColls = totalColls.add(_coll).sub(oldColl);
        totalDebts = totalDebts.add(_debt).sub(oldDebt);
        if (_coll == 0 && _debt == 0) {
            leverageTroves[_borrower].status = Status.closedByLeverage;
            leverageTroves[_borrower].startTime = 0;
        }
        if (oldColl == 0) {
            leverageTroves[_borrower].status = Status.active;
        }
        if (_startTime != 0) {
            leverageTroves[_borrower].startTime = _startTime;
        }
        if (_margin != 0) {
            leverageTroves[_borrower].margin = _margin;
        }
        leverageTroves[_borrower].coll = _coll;
        leverageTroves[_borrower].debt = _debt;
        leverageTroves[_borrower].stake = _coll;
    }
    ...
    function _calcInterest(uint _baseDebt, uint _startTime)
        ...
        returns (uint)
    {
        uint num = block.timestamp.sub(_startTime).mul(10).div(DAY_TO_SECENDS);
        uint precision = num.mod(10);
        uint interestDays = num.div(10);
        if (num == 0 || precision > 0) {
            interestDays = interestDays.add(1);
        }
        return _baseDebt.mul(APY).mul(interestDays).mul(100).div(36525).div(DECIMAL_PRECISION);
    }
    ...
    function getBorrowTroveStake(address _borrower) external view override returns (uint) {
        return borrowTroves[_borrower].stake;
    }
    ...
    function getLeverageTroveStake(address _borrower) external view override returns (uint) {
        return leverageTroves[_borrower].stake;
    }
    ...
    function getRedemptionRateWithDecay(uint _redeemedUSDA, uint _totalUSDASupply) external view override returns (uint) {
        uint decayedBaseRate = _calcDecayedBaseRate();
        uint redeemedUSDAFraction = _redeemedUSDA.mul(DECIMAL_PRECISION).div(_totalUSDASupply);
