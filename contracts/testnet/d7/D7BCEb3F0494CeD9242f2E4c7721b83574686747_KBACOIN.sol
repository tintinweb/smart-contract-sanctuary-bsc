...
contract KBACOIN is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string  private constant _name = "KBA COIN"; 
    string  private constant _symbol = "KBA";   
    uint8   private constant _decimals = 2; 
    uint256 private _tTotal = 210000 * 10** _decimals;
    uint256 public _maxTxAmount = 21000; //
    ...
    address payable public   _markWallet    =payable(0x5a6F0ca7F707FBFCb14ce5E296bA0712CBE2AAaD);
         ...
         uint256 transferAmount = amount;
          if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to])
          {
                ...
                uint256 markFee = amount.mul(_markTx).div(100); 
                               _tOwned[address(this)] += markFee;
                emit Transfer (from, address(this), markFee);
                transferAmount=amount.sub(markFee);
         } 
