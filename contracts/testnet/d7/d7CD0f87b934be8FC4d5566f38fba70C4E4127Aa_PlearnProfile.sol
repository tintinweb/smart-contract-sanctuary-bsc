...
contract PlearnProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeERC20 for IERC20;

    IERC20 public plnToken;

    bytes32 public constant NFT_ROLE = keccak256("NFT_ROLE");
    bytes32 public constant SPECIAL_ROLE = keccak256("SPECIAL_ROLE");

    uint256 public numberActiveProfiles;
    uint256 public numberPLNToReactivate;
    uint256 public numberPLNToRegister;
    uint256 public numberPLNToUpdate;
    ...
    constructor(
        IERC20 _plnToken,
        uint256 _numberPLNToReactivate,
        uint256 _numberPLNToRegister,
        uint256 _numberPLNToUpdate
    ) {
        plnToken = _plnToken;
        numberPLNToReactivate = _numberPLNToReactivate;
        numberPLNToRegister = _numberPLNToRegister;
        numberPLNToUpdate = _numberPLNToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends PLN to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer PLN tokens to this contract
        plnToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberPLNToRegister
        );
        ...
        // Emit an event
        emit UserNew(_msgSender(), _nftAddress, _tokenId);
    }
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer PLN token to this address
        plnToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberPLNToUpdate
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Emit event
        emit UserReactivate(_msgSender(), _nftAddress, _tokenId);
    }
    ...
    /**
     * @dev Claim PLN to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        plnToken.safeTransfer(_msgSender(), _amount);
    }


    /**
     * @dev Update the number of PLN to register
     * Callable only by owner admins.
     */
    function updateNumberPLN(
        uint256 _newNumberPLNToReactivate,
        uint256 _newNumberPLNToRegister,
        uint256 _newNumberPLNToUpdate
    ) external onlyOwner {
        numberPLNToReactivate = _newNumberPLNToReactivate;
        numberPLNToRegister = _newNumberPLNToRegister;
        numberPLNToUpdate = _newNumberPLNToUpdate;
    }
