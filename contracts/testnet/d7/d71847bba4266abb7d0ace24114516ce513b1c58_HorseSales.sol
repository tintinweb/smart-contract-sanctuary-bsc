...
contract HorseSales is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
    ...
    mapping(address => uint256) public ticketAddressLimit;
    ...
    function initialize(
		...
		IRequest _request,
		address minter,
		IERC20Upgradeable[] calldata _buyWiths,
    ...
    function buyTicket(IERC20Upgradeable buyWith, uint256 quantity) public nonReentrant  {
        ...
        require(ticketAddressLimit[msg.sender] + quantity <= 2, "2 nfts per address");
		...
		uint256 reqTokenAmount = (quantity * ticketPrice);
		
        ticketQty[msg.sender] += quantity;
        ticketAddressLimit[msg.sender] += quantity;
	...
	function becomeSuperHorse(
		...
		requestContract.add(
			requestId, 
			abi.encodeWithSelector(HorseSales(address(0)).becomeSuperHorse.selector, tokenId),
    ...
    function standardBreed(
		...
		requestContract.add(
			requestId, 
			abi.encodeWithSelector(HorseSales(address(0)).standardBreed.selector, horse, parentTokenId),
    ...
    function superBreed(
		...
		requestContract.add(
			requestId, 
			abi.encodeWithSelector(HorseSales(address(0)).superBreed.selector, horse, parentTokenId),
			abi.encodePacked(_tokenIds.current())
		);
    ...
    function withdrawToken(IERC20Upgradeable _token) public onlyOwner {
