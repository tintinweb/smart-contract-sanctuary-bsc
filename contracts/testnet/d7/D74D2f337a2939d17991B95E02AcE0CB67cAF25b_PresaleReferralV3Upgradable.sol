...
interface IStaking {
    function stakeByAddressAdmin(address _address, uint256 _value)
        external
        returns (bool);
}
    ...
    UUPSUpgradeable
{
    ...
    function setLevelDecimals(uint256 _value)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setLevelRefferalBonus(uint256 _value)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setLevelRate(uint256[] calldata _value)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setAccountMap(
        address addr,
        uint256 _totalBusinessETH,
        uint256 _totalBusinessUSD,
        address _referredAddresses,
        uint256 _pendingRewardETH,
        uint256 _pendingRewardUSD,
        uint256 _rewardClaimedETH,
        uint256 _rewardClaimedUSD
    ) external onlyAdmin returns (bool) {
        if (_totalBusinessETH != 1010) {
            accounts[addr].totalBusinessETH = _totalBusinessETH;
        }
        if (_totalBusinessUSD != 1010) {
            accounts[addr].totalBusinessUSD = _totalBusinessUSD;
        }
        if (_referredAddresses != address(0)) {
            accounts[addr].referredAddresses.push(_referredAddresses);
        }
        if (_pendingRewardETH != 1010) {
            accounts[addr].pendingRewardETH = _pendingRewardETH;
        }
        if (_pendingRewardUSD != 1010) {
            accounts[addr].pendingRewardUSD = _pendingRewardUSD;
        }

        if (_rewardClaimedETH != 1010) {
            accounts[addr].rewardClaimedETH.push(_rewardClaimedETH);
        }
        if (_rewardClaimedUSD != 1010) {
            accounts[addr].rewardClaimedUSD.push(_rewardClaimedUSD);
        }
    ...
    function getReferredCount(address _address)
        external
        view
        returns (uint256)
    {
    ...
    function _getReferredCount(address _address)
        private
        view
        returns (uint256 referredCount)
    {
        referredCount = accounts[_address].referredAddresses.length;
    }

    // function addReferrer(address _referrer) external returns (bool) {
    //     _addReferrer(_referrer);
    //     return true;
    // }

    function _addReferrer(address _address, address _referrer)
        private
        returns (bool)
    {
    ...
    function _addParent(address _address, address _parent)
        private
        returns (bool)
    {
    ...
    function AddReferrerAdmin(
        address payable[] calldata referrer,
        address[] calldata _referree
    ) external onlyAdmin returns (bool) {
        for (uint256 i; i < referrer.length; ++i) {
            if (referrer[i] == address(0)) {
                emit RegisterRefererFailed(
                    _referree[i],
                    referrer[i],
                    "Referrer cannot be 0x0 address"
                ...
                return false;
            } else if (isCircularReference(referrer[i], _referree[i])) {
                emit RegisterRefererFailed(
                    _referree[i],
                    referrer[i],
                ...
                return false;
            } else if (accounts[_referree[i]].referrer != address(0)) {
                emit RegisterRefererFailed(
                    _referree[i],
                    referrer[i],
                    "Address have been registered upline"
            ...
            Account storage userAccount = accounts[_referree[i]];
            Account storage parentAccount = accounts[referrer[i]];

            userAccount.referrer = referrer[i];
            parentAccount.referredAddresses.push(_referree[i]);

            emit RegisteredReferer(_referree[i], referrer[i]);
        }
        return true;
    }

    function AddSingleReferrerAdmin(
        address payable referrer,
        address[] calldata _referree
    ) external onlyAdmin returns (bool) {
        if (referrer == address(0)) {
        ...
        for (uint256 i; i < _referree.length; ++i) {
            if (isCircularReference(referrer, _referree[i])) {
                emit RegisterRefererFailed(
                    _referree[i],
                    referrer,
                    "Referee cannot be one of referrer uplines"
            ...
            emit RegisteredReferer(_referree[i], referrer);
        }
        return true;
    }

    function payReferralInETH(uint256 value, address _referee)
        private
        returns (uint256)
    {
    ...
    function payReferralInUSD(uint256 value, address _referee)
        private
        returns (uint256)
    {
        Account memory userAccount = accounts[_referee];
    ...
    function setTokenContractAdmin(address _address)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setUSDContractAdmin(address _address)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setDFMTContractAdmin(address _address)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setStakingContractAdmin(address _address)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setpriceFeedOracleAddressAdmin(address _address)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setPricePerUSDAdmin(uint256 _value)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setMinContributionUSDAdmin(uint256 _value)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setPayReferralFeeAdmin(uint256 _value)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function getTokensValueUSD(uint256 _usdValue)
        external
        view
        returns (uint256 tokenValue)
    {
        tokenValue = _usdValue * pricePerUSD;
    }

    // Function when someone buy with ETH

    function BuyWithETH(address _referrer, address _parent)
        public
        payable
        whenNotPaused
    {
        ...
        if (_referrer != _parent && _referrer != address(0)) {
            _addParent(msg.sender, _parent);
        }

        if (isBuyAndStake) {
            IStaking(stakingContract).stakeByAddressAdmin(
                msg.sender,
                tokenValue
            );
        } else {
            IERC20Upgradeable(tokenContract).transfer(msg.sender, tokenValue);
        }

        if (isPayReferral) {
            payReferralInETH(payReferralValue, msg.sender);
        }
    ...
    function BuyWithUSD(
        ...
        if (isPayReferral) {
            payReferralInUSD(payReferralValue, msg.sender);
        }
    ...
    function BuyWithDFMTAdmin(
        ...
        for (uint8 i; i < length; ++i) {
            require(
                _value[i] >= minContributionUSD.mul(10**tokenDecimals),
                "Buying value less then minimum buying value."
            );

            uint256 tokenValue = (_value[i] * pricePerUSD) / 10**tokenDecimals;

            uint256 payReferralValue = _value[i].sub(
                _value[i].mul(payReferralFee).div(100)
            );

            if (!hasReferrer(_address[i]) && _referrer[i] != address(0)) {
                _addReferrer(_address[i], _referrer[i]);
            }

            if (_referrer[i] != _parent[i] && _referrer[i] != address(0)) {
                _addParent(_address[i], _parent[i]);
            }
            ...
            if (isPayReferral) {
                payReferralInUSD(payReferralValue, _address[i]);
            }

            totalTokenSold += tokenValue;
            totalUSDRaised += _value[i];
            emit TokenPurchased(_address[i], tokenValue);
        }
    }
    ...
    function initialize() external initializer {
        // //BSC
        // USDContract = 0x55d398326f99059fF775485246999027B3197955;
        ...
        // //BSCTestnet
        // USDContract = 0x9f929b6319CD14C83E288ABEd7B7D8A609A0Ad5d;
        ...
        //Mumbai
        // priceFeedOracleAddress = AggregatorV3Interface(0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada);
        // USDContract = 0xD01776B6a088E32705A2e358f2C77b59Fb22313A;
        // tokenContract = 0x9147f0b9FD695d5580bAc339684C733f1a9bEada;

        // stakingContract = 0x5842764DC150085c2B440ed6bAc00D1B764478CF;
