...
contract iSWLegacyMutualNCV1_01 is BankSettingsFunctions {
    ...
    event NewInvestor(address contAddr, address invAddr, uint256 invAmount, uint256 amtBonds, uint256 priceLeXC, uint256 TVL);
    ...
    function internalCompFunction(address adr, uint256 amtComp, bool isBWC, bool isAuto) internal {
        ...
        if(isBWC){
            ...
            if(user.initialInvestmentAmt == 0) {
                ...
                emit NewInvestor(address(this), adr, user.initialInvestmentAmt, user.Bonds, calculateBuyBonds(1*10**18, (getBalance() - 1*10**18)), getBalance());
            }
