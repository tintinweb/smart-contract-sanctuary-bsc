...
contract BCNFT is ERC721, Ownable {
    ...
    // tokenInfo of tokenId tokenId => TOken
    mapping(uint256 => Token) private tokens;
    
    // tokenBalance of owner address => tokenBalance
    mapping(address=>uint256) private _tokenBalance;
    
    // Tokens of owner  address => tokensArray[tokenId...]
    mapping(address=>uint256[]) private _ownIds;
    

    mapping(address=>OwnIDS) private _ownIdsExist;
    
    // tokenURI of tokenId tokenId => tokenURI
    ...
    function checkAccountLevels(address _account) external view returns (uint256[] memory) {
        ...
        uint256[] memory levels = new uint256[](ownIds.length);
