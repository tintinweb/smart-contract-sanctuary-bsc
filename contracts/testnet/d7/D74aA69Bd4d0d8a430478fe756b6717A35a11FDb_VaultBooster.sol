...
contract VaultBooster is TokensRecoverable {
    ...
    // Staking Token interface, to stake Token into xToken
    IStakingToken public sToken;
    ...
    function prepare() ownerOnly() public returns (bool _success) {
        address _xToken = address(xToken);
        uint256 _amount = tokenBalance(address(token));

        totalTokensStaked += _amount;

        token.approve(_xToken, type(uint128).max);
        sToken.stake(_amount);
