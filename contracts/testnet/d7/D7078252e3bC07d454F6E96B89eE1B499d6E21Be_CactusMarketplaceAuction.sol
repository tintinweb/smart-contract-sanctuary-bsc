    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier onlyRoleNotSeller(
        ...
        require(
            msg.sender !=
                auctionsWithId[_auctionId][_assetContract][_tokenId].admin &&
                msg.sender !=
                auctionsWithId[_auctionId][_assetContract][_tokenId].owner &&
        ...
        require(
            (_highestBidder != address(0) && msg.sender == _highestBidder) ||
                (msg.sender == IERC721(assetContract).ownerOf(tokenId) &&
                    msg.sender ==
                    auctionsWithId[_auctionId][assetContract][tokenId].owner) ||
                msg.sender ==
                auctionsWithId[_auctionId][assetContract][tokenId].admin,
    ...
    modifier onlyMinBidForUpdate(
        ...
        _onlyMinBidForUpdate(assetContract, tokenId, newMinBid, auctionId);
        _;
    }
    ...
    function makeBid(
        ...
        nonReentrant
        whenNotPaused
        onlyRoleNotSeller(_assetContract, _tokenId, _auctionId)
        onlyRoleNotContracts
        returns (bool success)
    {
        Auction storage auction = auctionsWithId[_auctionId][_assetContract][
            _tokenId
        ];
    ...
    function settleAuction(
        ...
        require(
            (_highestBidder != address(0) && msg.sender == _highestBidder) ||
                (msg.sender == IERC721(_assetContract).ownerOf(_tokenId) &&
                    msg.sender ==
                    auctionsWithId[_auctionId][_assetContract][_tokenId]
                        .owner) ||
                msg.sender ==
                auctionsWithId[_auctionId][_assetContract][_tokenId].admin,
            "you are neither the highest bidder nor seller"
        ...
        require(
            _highestBid > auction.minBid,
            "highest bid is below minBid plus increment"
        );

        // Get info on who the highest bidder is
        address winner = _highestBidder;
        uint256 price = _highestBid;
        // Clean up the highest bid
        bids[auction.Id][winner] = bids[auction.Id][winner] - _highestBid;
    ...
    function withdrawAuction(
        ...
        external
        nonReentrant
        onlyRoleSeller(_assetContract, _tokenId, _auctionId)
    {
    ...
    function updateAuctionEndTime(
        ...
        uint256 _endTimestamp,
        uint256 _auctionId
    ) external onlyRoleSeller(_assetContract, _tokenId, _auctionId) {
        Auction storage auction = auctionsWithId[_auctionId][_assetContract][
            _tokenId
        ];
    ...
    function updateMinimumPrice(
        ...
        external
        onlyRoleSeller(_assetContract, _tokenId, _auctionId)
        onlyMinBidForUpdate(_assetContract, _tokenId, _newMinBid, _auctionId)
    {
        Auction storage auction = auctionsWithId[_auctionId][_assetContract][
            _tokenId
        ];
    ...
    function _onlyReservePriceForUpdate(
        ...
        Auction memory auction = auctionsWithId[_auctionId][_assetContract][
            _tokenId
        ];
