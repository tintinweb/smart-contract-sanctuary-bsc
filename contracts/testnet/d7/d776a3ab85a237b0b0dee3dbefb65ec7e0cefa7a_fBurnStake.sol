...
contract fBurnStake is Ownable, ReentrancyGuard {
    ...
    function unstake(uint256 _stakeId) nonReentrant external returns (bool){
        ...
        if(isStakeLocked(_stakeId) && isPaused == false){
            uint256 claimableRewards = getUnclaimedRewards(_stakeId);
            uint256 earlyUnstakePenaltyAmount = (stakeBalance * stakePoolDetails.earlyUnstakePenalty)/100;
            totalPenalty = claimableRewards + earlyUnstakePenaltyAmount;
            unstakableBalance = stakeBalance - earlyUnstakePenaltyAmount;
        }else{
        ...
        if(totalPenalty > 0 && stakeToken.balanceOf(address(this)) > 0){
            success = rewardToken.transfer(owner(), totalPenalty);
            require(success, "Token Transfer failed.");
        }
