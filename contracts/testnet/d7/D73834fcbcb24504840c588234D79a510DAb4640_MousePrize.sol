...
// File contracts/nft/MousePrize.sol
...
/**
 * @title Mouse Prize
 * @dev This is a normal ERC721 token
 */
contract MousePrize is
  Context,
  AccessControlEnumerable,
  ERC721Enumerable,
  ERC721Burnable,
  ERC721Pausable
{
  ...
  event PrizeMinted(string data, address owner, uint256 nftId);
   ...
   * @param data Arbitrary data that defines the prize
   */
  function mintSpecialPrize(address to, string calldata data) public onlyRole(MINTER_ROLE) {
    uint256 id = _tokenIdTracker.current();

    _safeMint(to, id);
    _tokenIdTracker.increment();

    emit PrizeMinted(data, to, id);
  }
