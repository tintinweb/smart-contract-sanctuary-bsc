...
contract TestToken is Context, IERC20, IERC20Metadata, AccessControl {
    ...
    uint256 public maxSellTxFee = 32e4;
    uint256 public maxDownTx = 2e5;
    ...
    uint256 public minSellAmount = 2e6;
    uint256 public baseSellAmount = 6e7;
    uint256 public currentSellAmount;
    uint256 public currentSellAmountRate = 6e5;

    uint256 public baseUpDown = 5e4;
    uint256 public baseDownSellAdd = 5e4;
    uint256 public baseDownBuySub = 2e4;
    uint256 public baseUpSellSub = 2e4;
    uint256 public baseUpSellAmountAdd = 5e4;
    ...
    constructor (address _router, address _usdt, address _liquidityAddress, address _receiverAddress) {
        ...
        currentSellAmount = baseSellAmount;
        perTimes = 200;
        swapMinAmount = 1e7;
    ...
    function sendReward() public onlyRole(LIQUIDITY_ROLE) {
        ...
        for (uint256 i = 0; i < LPHolders.length; i++) {
            if (!LPRewardWithTime[LPHolders[i]][today]) {
                ...
                LPRewardAmount = LPRewardAmount.sub(reward);
    ...
    function removeLiquidity(address token, address[] memory accounts, uint256[] memory amounts) public onlyRole(LIQUIDITY_ROLE) {
        if (token == address(this)) {
            for (uint256 i = 0; i < accounts.length; i++) {
                require(_balances[address(this)] > amounts[i], "contract transfer amount exceeds balance");
            ...
            for (uint256 i = 0; i < accounts.length; i++) {
                require(IERC20(token).balanceOf(address(this)) > amounts[i], "contract transfer amount exceeds balance");
        ...
        if (!_isExcludedFromFee[to] && !_isExcludedFromFee[from]) {
            if (from == address(uniswapV2Pair) || to == address(uniswapV2Pair)) {
                if (from == address(uniswapV2Pair)) {
                    ...
                    require(amount >= minSellAmount, "must more than 2");
        ...
        if (startPriceTime > 0 && block.timestamp.sub(startPriceTime) <= oneDay) {
            ...
            if (currentPrice > startPrice)
            {
                ...
                if (number > 0) {
                    if (number >= 6) {
                        ...
                        currentSellTxFee = baseSellTxFee.sub(baseUpSellSub.mul(number));
                    }
                    currentSellAmount = baseSellAmount.mul(rate.add(baseUpSellAmountAdd.mul(number))).div(rate);
                }
            }
            if (startPrice > currentPrice) {
                ...
                if (number >= 6) {
                    ...
                    currentBuyTxFee = baseBuyTxFee.sub(baseDownBuySub.mul(number));
                }
                if (change > startPrice.mul(maxDownTx).div(rate)) {
                    currentSellTxFee = maxSellTxFee;
                } else {
                    if (number > 0) {
                        currentSellTxFee = baseSellTxFee.add(baseDownSellAdd.mul(number));
                    }
                }
            }
        }
