...
contract USDyBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    address public USDyBUSD;//Pair address
    ...
    */
    ...
    function initialize(
        ...
        IReceipt(USDs_lp).initialize(_admin,address(this), "USDs_lp", "USDs_lp");
        depositTax = 10;
        withdrawTax = 0;
        exchangeRatio = 10 ** 18;
    }
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            ...
            require(!isContract(msg.sender),"USDyBUSD: No external contract interaction");
        }
        IERC20(Admin.BUSD()).safeTransferFrom(user,address(this),amount);
        IERC20(Admin.BUSD()).safeTransfer(Admin.Treasury(), amount / 10);
        uint USDyLiquidity = (IERC20(Admin.USDy()).balanceOf(address(USDyBUSD)) * (amount * 4/5)/ IERC20(Admin.BUSD()).balanceOf(address(USDyBUSD)));
        ...
        convertBUSDToLP((amount * 4/5), USDyLiquidity + USDyAmount,Admin.BUSD(), Admin.USDy(), address(USDyBUSD));
        (uint256 BUSDAmount, uint256 leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution
                    (user,(amount * 9) / 100,amount);
        if (BUSDAmount != 0) {
            IERC20(Admin.BUSD()).safeTransfer(user, (amount * 1) / 100);
        ...
        IReceipt(USDs_lp).mint(address(this),(USDyAmount * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
        UserDeposit[user] += (USDyAmount * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100);
        USDyOfDollarValue += USDyAmount;
        exchangeRatio = USDyOfDollarValue * (10 ** 18)/(IERC20(USDs_lp).totalSupply());
        emit Deposit("USDyBUSD Vault",address(this),user,amount,block.number,block.timestamp);
    }
    ...
    */
    ...
    function withdraw(address user, bool isReciept,uint _amount,address sendTo) nonReentrant external _lastExchangeRatio whenNotPaused{
     ...
     function distribute(uint256 _rebaseReward) external nonReentrant{
        ...
        console.log(IERC20(USDs_lp).totalSupply(),"(IERC20(USDs_lp).totalSupply()");
    ...
    */
    ...
    function setDepositTax(uint _depositTax) external _isAdmin{
        ...
        emit SetterForUint("USDyBUSD",address(this),depositTax,_depositTax,block.number,block.timestamp);
    ...
    */
    ...
    function _claimRebaseReward(address user) internal {
        userAmount = ((UserDeposit[user] * exchangeRatio)/ IERC20(address(USDyBUSD)).totalSupply());
    ...
    */
    ...
    function _userAmount(address user) internal view returns(uint){
        return((UserDeposit[user] * exchangeRatio)/ IERC20(address(USDyBUSD)).totalSupply());
    }
