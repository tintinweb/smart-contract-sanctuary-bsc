...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 public REWARD = IERC20(0x42981d0bfbAf196529376EE702F2a9Eb9092fcB5); // temp safemoon
...
contract TESTBTC is IERC20, Taxable {
    using SafeMath for uint256;

    address constant mainnetRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    address constant WBNB          = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address constant DEAD          = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO          = 0x0000000000000000000000000000000000000000;

    string _name = "Test BTC";
    string _symbol = "$TBTC";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100000000000 * (10 ** _decimals);     // 100,000,000,000
    uint256 public _maxWalletSize = (_totalSupply * 20) / 1000;  // 1% 
    ...
    uint256 constant reflectionFee = 10;  
    uint256 constant liquidityFee = 20; 
    uint256 constant charityFee = 0; 
    uint256 constant devFee = 10;     
    uint256 constant totalFee = 100;  
    ...
    constructor () {
        ...
        _maxWalletSize = _totalSupply / 50; // 2%
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(isLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Max Wallet Exceeded");
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBReflections = amountBNB.mul(reflectionFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflections}() {} catch {}
        if (_excluded[1] == _excluded[2]) {
            (bool success,) = payable(_excluded[2]).call{value: amountBNBMarketing.add(amountBNBDev), gas: 30000}("");
            require(success, "receiver rejected ETH transfer");
        } else {
            (bool success,)  = payable(_excluded[1]).call{value: amountBNBMarketing, gas: 30000}("");
            (bool success2,) = payable(_excluded[2]).call{value: amountBNBDev, gas: 30000}("");
    ...
    function updateMaxWallet(uint256 percent, uint256 denominator) external {
        require(isLimitExempt[msg.sender] && percent >= 1 && denominator >= 100, "Max wallet must be greater than 1%");
