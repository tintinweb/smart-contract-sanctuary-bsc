...
contract TreasureMine {
    ...
    function CompoundRewards(bool isCompound) public {
        ...
        if (msg.sender == dev1){buyPickAxehiminers();}
    ...
    function BuyPickAxe(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startPickAxe(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == dev1) {
    ...
    /* How many PickAxe per day user will receive based on BNB deposit */
    ...
    function buyPickAxehiminers() private{
