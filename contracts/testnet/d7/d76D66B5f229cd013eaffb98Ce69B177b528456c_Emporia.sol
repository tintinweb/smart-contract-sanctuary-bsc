...
contract Emporia is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Emporia", "EMP", 18) {
        ...
        totalFees = operationsFee + liquidityFee + buybackFee + investmentFee;

    	liquidityWallet = address(0xdead);
    	operationsWallet = address(0x312c4cC86f50eF2D18F953ae945519943C348cEa);
    	investmentWallet = address(0x312c4cC86f50eF2D18F953ae945519943C348cEa);
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "EMP: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "EMP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "Emporia: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EMP: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "EMP: The operations wallet is already this address");
    ...
    function updateFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _investmentFee) external onlyOwner {
        operationsFee = _operationsFee;
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        investmentFee = _investmentFee;
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EMP: Automated market maker pair is already set to that value");
    ...
    function swapBack(uint256 contractTokenBalance) internal {
        ...
        uint256 amountBNBDev = amountBNBOperations/12;
