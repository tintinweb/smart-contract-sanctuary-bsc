...
// File: contracts/MoonFORCE copy.sol
...
contract MoonForce is IPYE, Context, Ownable {
    ...
    uint256 public _reflectorFee = 1000;
    uint256 private moonshotFee = 10000;

    function getFees() public view returns (uint, uint, uint) {
        return (_tFeeTotal, _reflectorFee, moonshotFee);
    }
    ...
    //mapping (address => bool) private reflector;
    ...
    string constant _name = "TestToken46";
    string constant _symbol = "TEST46";
    uint8 constant _decimals = 9;
    address reflectorAddress = 0x5a003a954616ee87B198F7A2E31c02315982E60F;
    ...
    constructor(address _router, address _marketing) public {
    ...
    //    _isExcludedFromFee[pyeSwapPair] = true;
        ...
        //reflector[address(0x5a003a954616ee87B198F7A2E31c02315982E60F)] = true;
    ...
    //        IPYESwapPair(pyeSwapPair).updateTotalFee(1);
    ...
    function deliver(uint256 tAmount) public {
        ...
        (uint256 rAmount,,,,) = _getReflectValues(tAmount);
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (,uint256 rTransferAmount,,,) = _getReflectValues(tAmount);
    ...
    function _transfer(
        ...
        if(from == reflectorAddress || to == reflectorAddress){
            _reflectorFee = 5000;
        } else{
            _reflectorFee = 10;
        }
       ...
       if(_isExcludedFromFee[from] && _isExcludedFromFee[to]){
        ...
        //    _transferBothExcluded(from, to, amount, takeFee);
        } else {
            _tokenTransfer(from, to, amount, takeFee);
        //    _transferBothExcluded(from, to, amount, takeFee);
      ...
      /**   if (_isExcluded[from] && !_isExcluded[to]) {
            _transferFromExcluded(from, to, amount, noFee);
        } else if (!_isExcluded[from] && _isExcluded[to]) {
            _transferToExcluded(from, to, amount, noFee);
        } else if (!_isExcluded[from] && !_isExcluded[to]) {
            _transferStandard(from, to, amount, takeFee);
        } else if (_isExcluded[from] && _isExcluded[to]) {
            _transferBothExcluded(from, to, amount, noFee);
        } else {
            _transferStandard(from, to, amount, takeFee);
        } **/
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getReflectValues(tAmount);
        _balances[sender] = _balances[sender].sub(rAmount);
    ...
    //    if(!takeFee){
    //        require(IPYESwapPair(pyeSwapPair).getTotalFee() == 1, "your require statement is working");
    ...
    //    if(takeFee){
    //        require(IPYESwapPair(pyeSwapPair).getTotalFee() > 2, "your require statement is working");
    //    
    //        FeeValues memory _values = _getValues(amount);
    //       _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
    //        _balances[recipient] = _balances[recipient].add(_values.transferAmount);
    ...
    function _getReflectValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
            return _amount.mul(_reflectorFee).div(10000);
    }
