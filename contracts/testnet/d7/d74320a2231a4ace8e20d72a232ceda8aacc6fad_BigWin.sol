...
contract BigWin is WhitelistAdminRole {
	...
	mapping(uint => mapping(address => mapping(uint => AwardData))) public userAwardDataMapping;
	...
	function setBonusInterval(uint interval) external onlyWhitelistAdmin {
			...
			if (userGlobal.status == 0) {
				...
				if (lineValue >= 30) {
					...
					if (user.lineLevel == 2 && lineAmount >= usd1000 && (awData.oneInvAmount > 0 || awData.twoInvAmount > 0)) {
						inviteSend += awData.oneInvAmount.div(30).mul(lineValue).mul(7).div(10);
						inviteSend += awData.twoInvAmount.div(30).mul(lineValue).mul(5).div(7);
					}
