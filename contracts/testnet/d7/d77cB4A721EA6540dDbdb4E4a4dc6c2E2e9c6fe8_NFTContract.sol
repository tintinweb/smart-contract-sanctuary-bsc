...
contract NFTContract is ERC721, Ownable {
  ...
  /// @param _bidderAdd is the address of the heighst bidder
  /// @return newTokenId is the unique an identifier for each minted NFT using this contract
  function createCollectible(string memory tokenURI,address _bidderAdd) public returns (uint256) {
    uint256 newTokenId = tokenCounter;
    _safeMint(_bidderAdd, newTokenId);
    _setTokenURI(newTokenId, tokenURI);
    tokenCounter = tokenCounter + 1;
    return newTokenId;
  }

  function createCollectibleFromList(string[] memory tokenURIList,uint256 indexLen)   external  {
