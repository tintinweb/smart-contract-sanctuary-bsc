...
// File: Farm/price-convertor/TransferHelper.sol
...
// File: Farm/price-convertor/SafeMath.sol
...
// File: Farm/price-convertor/PriceConvertor.sol
...
contract PriceConvertor {
    ...
    address public token_cake;
    address public token_mdex;
    ...
    constructor() public {
        owner = msg.sender;
        token_cake  = 0xFa60D973F7642B748046464e165A65B7323b0DEE;
        token_mdex  = address(0);
        token_usdt  = address(0);
        uniswap_factory = 0x6725F303b657a9451d8BA641348b6761A6CC7a17;
        uniswap_router  = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    }

    // (default asset) convert cake to bnb
    function getCakeBnb(uint token0Amount) public view returns (uint) {
        address[] memory path = new address[](2);
        path[0] = token_cake;
        path[1] = IUniswapV2Router02(uniswap_router).WETH();
        return _getPrice(token0Amount, path);    
    }
    
    // (default asset) convert mdex to bnb
    function getMdexBnb(uint token0Amount) public view returns (uint) {
        address[] memory path = new address[](2);
        path[0] = token_mdex;
        path[1] = IUniswapV2Router02(uniswap_router).WETH();
        return _getPrice(token0Amount, path);    
    ...
    // update token_cake address
    function updateCake(address _address) public onlyOwner {
        token_cake = _address;        
    }
    
    // update token_mdex address
    function updateMdex(address _address) public onlyOwner {
        token_mdex = _address;        
    }
