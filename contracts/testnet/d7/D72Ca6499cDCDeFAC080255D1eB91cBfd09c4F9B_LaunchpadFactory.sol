...
contract StandardTokenFactory is Ownable {
    ...
    address public _tokenFactoryContract;
    IToken public tokenFactoryContract;
    ...
    function deployContract (
        ...
        string memory chosenSymbol,
        uint8 chosenDecimals,
        uint256 chosenTotalSupply_) public payable {
        if( block.chainid == 97) {
            ITrustLaunchFees tlf = ITrustLaunchFees(0x3EA821C6E0D918e41B873F65F83befb0b66Bb9AA);
            (bool active, uint256 fee, address feeReceiver) = tlf.getFeeDataSE(0, msg.sender);
            if( active) {
                (bool success,) = payable(feeReceiver).call{value: fee}("");
                require(success, "Failed to send!");
            }
        }
        address newAddress = tokenFactoryContract.deployContract(chosenName, chosenSymbol, chosenDecimals, chosenTotalSupply_, msg.sender);
    ...
    function setAddress( address newAddr) public onlyOwner {
        _tokenFactoryContract = newAddr;
        tokenFactoryContract = IToken(newAddr);
    }
}
