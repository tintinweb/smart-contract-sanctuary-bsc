...
contract ClickBackTeamLocked {
    ...
    address token=0x5D595c155CdCEb812B07b431317A7Cd1879662ad;
    uint256 pascode=9761014290;
   ...
   constructor(address _owner) {
    ...
    function lockteamoken(uint256 _amount, uint256 _deadline) external returns(bool)
    {
        ...
        userToTokenLocks[msg.sender].push(Lock(msg.sender,token, balanceAfter-balanceBefore, _deadline));     
        
        emit Locked(msg.sender, token, _amount, _deadline);
        return true;
    }
    ...
    function withdrawteamtoken(uint256 _pascode,uint256 _index) external returns(Lock[] memory)
    {
        ...
        require(_pascode == pascode, "not the owner");
