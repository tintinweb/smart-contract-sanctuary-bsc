...
contract PixelPOMNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    function mint(uint numberOfTokens, uint categoryId) public payable {
        uint256 ts = totalSupply();
        require(saleIsActive, "Sale must be active to mint tokens");
        require(numberOfTokens <= MAX_TX_MINT, "Exceeded max token purchase");
        require(ts + numberOfTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
        require(balanceOf(msg.sender) + numberOfTokens <= MAX_PER_WALLET, "Max Per Wallet");
        require(priceCategories[categoryId] * numberOfTokens <= msg.value, "Ether value sent is not correct");

        bool sent = IERC20(buyToken).transferFrom(msg.sender, receptorWallet, priceCategories[categoryId] * numberOfTokens);
