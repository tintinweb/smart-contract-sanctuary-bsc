...
contract GameComponentNFT is ERC721Enumerable, ERC2981 {
    ...
    mapping(address => bool) public isUserPaid;
    ...
    function payToPlay(uint256 tokenId) external payable {
        require(usageFee[tokenId] == msg.value);
        Address.sendValue(payable(ownerOf(tokenId)), msg.value);
        isUserPaid[msg.sender] = true;
    }
...
// File contracts/IComposableNFT.sol
...
interface IComposableNFT /* is IERC165, IERC721 */  {
    ...
    /**
     * @dev Transfer slot asset from `fromTokenId` to `toTokenId`
     */
    // function transferSlotAsset(uint fromTokenId, uint toTokenId, uint slotId, uint amount) external;

    /**
     * @dev Transfer the slot asset attached to `slotId` of `fromTokenId` to `to` address
     */
...
// File contracts/GameComposableNFT.sol
