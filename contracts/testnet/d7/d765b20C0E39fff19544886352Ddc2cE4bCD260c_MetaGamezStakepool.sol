...
/** 
    ...
    - safeMath restored
    - changed onlyValidAmount to min 1 ether
    - removed requirement on stake() whilst their is onlyValidAmount
  
    v 1.0.1

    -SafeMath Library removed
    -Storage design pattern changed
    -to struct/mapping from var/mapping
    -Repetative require() replaced by modifiers
    -balnceOf() updated
    -rewardsOf() and pendingRewardsOf() updated
    -stakingTimeOf() added for testing
    -stake() withdraw() withdrawAll() claimReward() updated
...
contract MetaGamezStakepool is ReentrancyGuard, Ownable, Pausable {
    ...
    /// @param _rewardToken The token will be the rewarded for stakingTime
        ...
        nonReentrant
    {
        ...
        /// @dev Add pending rewards to caller reward balance
        ...
        /// @dev make the actual withdraw transfer
        stakeToken.safeTransfer(_msgSender, _amount);

        totalStaked -= _amount;

        /// @dev update totalAllocPoint
    ...
    function claimReward() external whenNotPaused nonReentrant {
        address _msgSender = msg.sender;
        /// @dev add pending rewards to rewards
        users[_msgSender].rewards += pendingRewardsOf(_msgSender);

        uint256 _reward = rewardsOf(_msgSender);

        /// @dev check if sender has reward balance
        require(_reward > 0, "No rewards!");

        /// @dev check if contract has balance
        ...
        /// @dev make the actual transfer
        rewardToken.safeTransfer(_msgSender, _reward);

        /// @dev update token supply
        tokenSupply -= _reward;
    ...
    function balanceOf(address _sender) public view returns (uint256) {
        return users[_sender].stakeBalance;
    }

    /// @return user reward
    function rewardsOf(address _account) public view returns (uint256) {
        return users[_account].rewards;
    }

    function stakingTimeOf(address _account) public view returns (uint256) {
        return users[_account].stakingTime;
    }

    // /// @dev Returns no of tokens user earned based on staking balance
    // /// @param _account address of user
    // function pendingRewardsOf(address _account) public view returns (uint256) {
    //     ///@dev how much tokens user will get in a year based on its balance
    //     uint256 _tokens = ((balanceOf(_account) * apy) / 100) / 52 weeks;
    //     uint256 _time = block.timestamp - stakingTimeOf(_account);
