...
contract RouterDirect is EIP712, Ownable {
    ...
    function synthBatchMetaExchangeWithUnwrapRequest(
        ICurveProxy.TokenInput calldata tokenParams,
        IPortal.SynthParamsMetaSwap memory synthParams,
        ICurveProxy.MetaExchangeParams memory metaParams
    ) external onlyTrusted {
    ...
    function synthBatchMetaExchangeWithUnwrapRequestWithPermit(
        ICurveProxy.TokenInput calldata tokenParams,
        IPortal.SynthParamsMetaSwap memory synthParams,
        ICurveProxy.MetaExchangeParams memory metaParams,
        ISynthesis.PermitData calldata permitData
    ...
    function tokenSwapUnwrap(
        ...
        SafeERC20.safeTransferFrom(IERC20(tokenToSwap), msg.sender, _curveProxyV2, amount);
        ICurveProxy(_curveProxyV2).tokenSwapLiteWithUnwrap(tokenToSwap, to, amountOutMin, tokenToReceive, deadline, amount, 0, address(0));
    }

    function tokenSwapUnwrapWithPermit(
        address tokenToSwap,
        address to,
        uint256 amountOutMin,
        address tokenToReceive,
