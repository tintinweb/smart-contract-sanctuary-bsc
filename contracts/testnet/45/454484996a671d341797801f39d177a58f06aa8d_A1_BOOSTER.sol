...
contract A1_BOOSTER {
    ...
    function withdraw() external {
     ...
     if(withdrawable+FlushRecords[msg.sender].latestFlush <= userInfo[msg.sender].totalreward)
        {
          Flush =withdrawable;
        ...
        FlushRecords[msg.sender].latestFlush+=Flush;
         ...
         else if (msg.sender == defaultRefer){
        ...
        {
          uint256 extraFush = withdrawable+FlushRecords[msg.sender].latestFlush.sub(userInfo[msg.sender].totalreward);
             Flush = userInfo[msg.sender].totalreward.sub(FlushRecords[msg.sender].latestFlush);
             FlushRecords[msg.sender].latestFlush=Flush;
