...
// File: contracts/DividendHandler.sol

pragma solidity ^0.8.0;

interface IDividendHandler {
    function putLpProvider(address _addr) external returns (bool);

    function handleDividend(uint256 _lpBalance)
        external
        view
        returns (address[] memory tokenHolders, uint256[] memory dividenAmount);

    function takeToken(address token, address to) external;
}
...
contract Token is ERC20, Ownable {
    ...
    IData data;
    address public pair;
    address public lockWallet = address(1);
    ...
    constructor(address dataAddr, address tokenAddr) ERC20("OMT Token", "OMT") {
        data = IData(dataAddr);
        ...
        _mint(lockWallet, 15 * 10**7 * 10**decimals());
        _approve(lockWallet, address(this), type(uint256).max);
    ...
    function outputEveryDay() public returns (uint256) {
        ...
        if (balanceOf(lockWallet) < _outputAmount) {
            _outputAmount = balanceOf(lockWallet);
        }
        _transfer(
            lockWallet,
    ...
    function swapAddLiquidity() public {
    ...
    function getDividendHandler() private view returns (IDividendHandler) {
        return IDividendHandler(getDividendHandlerAddress());
    }
