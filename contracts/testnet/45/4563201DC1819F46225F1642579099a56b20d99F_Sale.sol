...
contract Sale is ERC721Holder, ERC1155Holder, Ownable, Initializable {
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) idValid(_saleId) {
        ...
        (uint256 percent, uint256 profitShare, uint256 remainingProfit) = _sharing(_project.isCreatedByAdmin, _project.id, _project.token, sale.tokenId);
    ...
    function _sharing(bool isAdmin, uint256 projectId, address _token, uint256 _tokenId) private returns(uint256, uint256, uint256) {
        ...
        if (isAdmin) {
            (royaltyReceiver, royaltyFee) = getRoyaltyInfo(_token, _tokenId, msg.value);
	...
	function getRoyaltyInfo(address _token, uint256 _tokenId, uint256 _salePrice) public view returns (address, uint256) { 
        if (nftChecker.isImplementRoyalty(_token)) {
            return IOSB721(_token).royaltyInfo(_tokenId, _salePrice);
        } else {
		    return (address(0), 0);
        }
	}
