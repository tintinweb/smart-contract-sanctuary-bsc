...
contract TurboETH {
    ...
    uint256 public constant REINVEST_FEE = 500; // 5% withdraw fee
    uint256 public constant PERCENT_STEP = 20; // 0.2% daily increment
    uint256 public constant WITHDRAW_TAX_PERCENT = 2500; // emergency withdraw tax 25%
    uint256 public constant PF_WITHDRAW_TAX_PERCENT = 1000; // emergency withdraw tax 10% for owner
    uint256 public constant MAX_HOLD_PERCENT = 20; // 0.2% hold bonus
    ...
    constructor(address payable _platform, uint256 _time) {
        ...
        priceFeedETH = AggregatorV3Interface(
            ...
            // testnet
            // 0x62CAe0FA2da220f43a51F86Db2EDb36DcA9A5A08 arbitrum
    ...
    function reinvest() public {
        ...
        user.deposits.push(
            Deposit(REINVEST_PLAN_INDEX, percent, totalAmount, profit, block.timestamp, finish)
        );
   ...
   function getPercent(uint8 plan) public view returns (uint256) {
    ...
    function getUserHoldBonusPercent(
        address userAddress
    ) public view returns (uint256) {
        User storage user = users[userAddress];
    ...
    function getUserHoldBonusCheckpoint(
        address userAddress
    ) public view returns (uint256) {
    ...
    function getUserDownlineCount(
        address userAddress
    ) public view returns (uint256 level1, uint256 level2, uint256 level3) {
