...
contract Bookie is Ownable, ReentrancyGuard {
    ...
    function placeBet(
        ...
        require(block.timestamp > pool.opensAt, "Pool not open yet"); // test
        require(block.timestamp < pool.closesAt, "Pool closed"); // test
        require(pool.status == PoolStatus.Open, "Pool not active"); // test
        require(_outcome != Outcome.None, "Invalid outcome"); // test
        require(
            pool.tiePossible || _outcome != Outcome.Tie,
            "Tie not possible"
        ); // test

        _transferFromSender(pool.currency, _amount); // test with ETH and ERC20
        ...
        userTotalBetAmount[msg.sender][pool.currency] += _amount; // test all amounts are showing
    ...
    function claim(uint256 _index) public nonReentrant {
        ...
        require(!playerPoolWithdrawn[msg.sender][_index], "Already withdrawn"); // test

        pool.status == PoolStatus.Settled
            ? _claimWinnings(pool) // test
            : _claimCanceled(pool); // test

        playerPoolWithdrawn[msg.sender][_index] = true; //test
    }
    ...
    function _claimWinnings(Pool storage pool) internal {
        ...
        emit WinnigsClaimed(msg.sender, pool.index, winningAmount);
    }
