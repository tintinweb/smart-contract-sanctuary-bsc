...
contract Keno is Game {
    ...
    mapping(uint256 => mapping(uint256 => mapping(uint256 => uint256[]))) private gainsFactor;
    ...
    /// @param token used
    function _calculateFactors(address token) internal {
        TokenConfig memory config = tokenConfigurations[token];
        for(uint256 played = 1; played <= config.maxNumbersPlayed; played++) {
            uint256[] storage factors = gainsFactor[config.biggestNumber][config.maxNumbersPlayed][played];
            if (factors.length == 0) {
    ...
    /// @param newBiggestNumber The new biggest number
    /// @param newMaxNumbers The new maximum of pick
    function updateTokenConfig(address token, uint128 newBiggestNumber, uint128 newMaxNumbers) public onlyOwner {
        if (hasPendingBets(token)) {
            revert TokenHasPendingBets();
        }
        if (newMaxNumbers > newBiggestNumber
                || newBiggestNumber > 40
                || newMaxNumbers > 10) revert InvalidSettings();
    ...
    function _getPayout(address token, uint256 betAmount, uint256 played, uint256 matchCount)
        private
        view
        returns (uint256 factor)
    {
        TokenConfig memory config = tokenConfigurations[token];
        factor = betAmount * gainsFactor[config.biggestNumber][config.maxNumbersPlayed][played][matchCount] / FACTORPRECISION;
    }
        ...
        override
    {
        ...
        uint256 _gain = _getPayout(bet.token,
            bet.amount,
    ...
    function _fact(uint256 n) internal pure returns (uint256 result){
        if (n==0) return 1;
        result = n;
    ...
    function _outof(uint n, uint x) internal pure returns (uint256) {
    ...
    function gain(address token, uint256 played, uint256 matchCount) public view returns (uint256 _factor) {
        TokenConfig memory config = tokenConfigurations[token];

        uint256 hypergeometricNumerator = _outof(played,matchCount) * _outof(config.biggestNumber-played,config.maxNumbersPlayed-matchCount);
        uint256 hypergeometricDenominator = _outof(config.biggestNumber,config.maxNumbersPlayed);

        // Calculate the inverse of the hypergeometric function
        _factor = FACTORPRECISION * hypergeometricDenominator / (hypergeometricNumerator * (played + 1));
    }
    ...
    function getNumbersOutOfRandomWord(address token, uint256 randomWord) public view returns (uint40) {
      uint256 result = 0;
      TokenConfig memory config = tokenConfigurations[token];
    ...
    function getLastUserBets(address user, uint256 dataLength)
        external
        view
        returns (FullKenoBet[] memory)
    {
        Bet[] memory lastBets = _getLastUserBets(user, dataLength);
        FullKenoBet[] memory lastKenoBets = new FullKenoBet[](
            lastBets.length
        );
