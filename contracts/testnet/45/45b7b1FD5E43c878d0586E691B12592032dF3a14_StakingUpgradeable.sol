    ...
    UUPSUpgradeable
{
    ...
    bool private _isStakingValueFixed;
    ...
    mapping(address => Account) private user;
    ...
    function initialize() public initializer {
        _tokenContract = 0xbfA0e2F4b2676c62885B1033670C71cdefd975fB;
        ...
        _stakingValueFixed = [1,2,4,8,16,32,64,128,256,512,1100,2100,5100,11000,21000,51000,100000];
        _stakingDurations = [2,5,8,11,14,17,20,23,26,29,35,49,56,63,70,90];
        _rewardRates = [150, 155, 160, 165, 170, 175, 180, 185, 190, 200, 215, 230, 245, 260, 290, 320, 350];
        _rewardRateDecimals = 100;
        _minStakingValue = 1 * 10 ** IERC20_EXTENDED(_tokenContract).decimals();

        _isStakingValueFixed = true;
    ...
    function _getRewardRate(
        ...
        for (uint8 i; i < stakingDurationsLength; i++) {
            if (userStakingDuration <= _toDays(_stakingDurations[i])) {
    ...
    function _stakeInfoMap(uint256 _stakingID) private view returns (StakeInfo memory) {
        return stakeInfo[_stakingID];
    }

    function _userAccountMap(address _userAddress) private view returns (Account memory) {
    ...
    function _stake(
        ...
        // if(_isStakingValueFixed) {
        //     require(
        //     _isValueEqualToStakingFixed(_value, _duration),
        //     "Value or duration is not equal to staking value set by admin."
        // );

        // require(_isUserAlreadyUsedFixedStakingPackage(_address, _value) == false, "You already used the staking package. Please choose another.");
        ...
        // );
        // require(
        //     _duration <= _stakingDurations[0] && _duration <= _stakingDurations[_stakingDurations.length - 1],
        ...
        uint256 stakingID = stakersLength - 1;

        Account storage userAccount = user[_address];
        ...
        userStakingInfo.userAddress = _address;
        userStakingInfo.valueStaked = _value;
        userStakingInfo.duration = _duration;
        userStakingInfo.rewardRate = _getRewardRate(_duration);
        userStakingInfo.startTime = _getCurrentTime();
        userStakingInfo.blockNumber.push(block.number);
    ...
    function _getStakingReward(
        ...
        uint256 valueStaked = userStakingInfo.valueStaked;
        uint256 startTime = userStakingInfo.startTime;
        uint256 duration = userStakingInfo.duration;
        uint256 rewardRate = userStakingInfo.rewardRate;
        uint256 endTime = startTime + duration;

        uint256 baseReward = ((valueStaked * rewardRate) / _rewardRateDecimals) / duration;
        uint256 stakingTimePassed = _getCurrentTime() - startTime;
        stakingReward =
            (baseReward *
                _min(_min(endTime, stakingTimePassed), startTime + duration)) -
    ...
    function _claimReward(
        ...
        StakeInfo storage userStakingInfo = stakeInfo[_stakingID];
        require(userStakingInfo.userAddress == _address, "You are not authorise to claim reward.");
        require(userStakingInfo.valueStaked > 0, "You have no staking yet.");
        Account storage userAccount = user[_address];
    ...
    function _unStake(
        ...
        StakeInfo storage userStakingInfo = stakeInfo[_stakingID];
        require(userStakingInfo.userAddress == _address, "You are not authorise to unstake.");
        require(userStakingInfo.valueStaked > 0, "You have no staking yet.");
        uint256 valueStaked = userStakingInfo.valueStaked;
        require(
            valueStaked > 0,
        ...
        require(
            _getCurrentTime() >= (userStakingInfo.startTime + userStakingInfo.duration),
            "Your staking is not over yet."
        );

       userStakingInfo.isStaked = false;

        _totalValueStaked -= valueStaked;
        _stakers[_stakingID] = address(0);
    ...
    function getStakers() external view returns (address[] memory stakersList, uint256 stakersCount) {
        address[] memory _stakersList = new address[](_stakers.length);
        ...
        for(uint256 i; i < _stakers.length; i++) {
            ...
            for(uint256 b; b < _stakersList.length; b++) {
                _stakersList[b] == _stakers[i];
                continue;
            }

            _stakersList[_stakersCount] = _stakers[i];
            _stakersCount++;
        }

 

        stakersList = _stakersList;
        stakersCount = _stakersCount;
    }

    function _isStaked(uint256 _stakingID) private view returns (bool) {
        StakeInfo storage stakingInfo = stakeInfo[_stakingID];
        return stakingInfo.isStaked;
    }

    function isStaked(address _userAddress) external view returns (bool) {
        Account storage userAccount = user[_userAddress];
        uint256[] memory userStakingIDs = userAccount.stakingIDs;
        uint256 userStakingLength = userAccount.stakingIDs.length;

        for (uint256 i; i < userStakingLength; i++) {
            if (_isStaked(userStakingIDs[i]) == true) {
                return true;
            }
        }
    ...
    function getUserTotalStakedValue(
        ...
        for (uint256 i; i < userStakingIDsLength; i++) {
            userTotalValueStaked += _stakeInfoMap(userStakingIDs[i]).valueStaked;
        }
    }

    function getStakingInfoByID(uint256 _stakingID) external view returns (StakeInfo memory) {
        return _stakeInfoMap(_stakingID);
    }

    function getUserStakingIDs(
        address _userAddress
    ) public view returns (uint256[] memory stakingIDs, uint256 stakingIDsCount) {
        stakingIDs = _userAccountMap(_userAddress).stakingIDs;
        stakingIDsCount = stakingIDs.length;
    }
        ...
        )
    {
        ...
        for (uint256 i; i < userStakingIDsLength; i++) {
            totalRewardClaimed += _stakeInfoMap(userStakingIDs[i]).rewardClaimed;
        }
    }
    ...
    function getStakingAnalystics()
        ...
        returns (
            ...
            uint256[] memory stakingRewardDurationsInDays,
        ...
        )
    {
        ...
        stakingRewardDurationsInDays = _stakingDurations;
        stakers = _stakers;
        stakersLength = _stakers.length;
        ...
        )
    {
        ...
        minDurationToStake = _stakingDurations[0];
        maxDurationToStake = _stakingDurations[_stakingDurations.length - 1];
    }
    ...
    function setIsStakingValueFixed(bool _trueOrFalse) external onlyOwner returns (bool) {
        _isStakingValueFixed = _trueOrFalse;
        return _isStakingValueFixed;
    }

    function _tokensToUSDT(uint256 _tokenValue) private view returns (uint256) {
    ...
    function _toDays(uint256 _daysInNumber) private pure returns (uint256) {
        return _daysInNumber * 1 days;
    }

    function _convertToAnotherDecimals(uint256 _valueInWei, uint8 _fromDecimals, uint8 _toDecimals) private pure returns (uint256) {
        return _valueInWei * 10 ** _toDecimals / 10 ** _fromDecimals;
    }
    ...
    function _transferToken(address _tokenContractAddress, address _to, uint256 _value) private {
    ...
    function _isValueEqualToStakingFixed(uint256 _value, uint256 _duration) private view returns (bool) {
        uint256[] memory stakingValuesFixed = _stakingValueFixed;
        uint256[] memory stakingDuration = _stakingDurations;
        uint256 stakingValuesFixedLength = stakingValuesFixed.length;

        for(uint16 i; i < stakingValuesFixedLength; i++) {
            if(_value == stakingValuesFixed[i] * 10 ** IERC20_EXTENDED(_tokenContract).decimals() && _duration == stakingDuration[i]) {
                return true;
            }
        }
    ...
    function _isUserAlreadyUsedFixedStakingPackage(address _userAddress, uint256 _value) private view returns (bool) {
        Account storage userAccount = user[_userAddress];
        uint256[] memory userStakingIDs = userAccount.stakingIDs;
        uint256 userStakingIDsLength = userStakingIDs.length;

        for(uint16 i; i < userStakingIDsLength; i++) {
            if(_stakeInfoMap(userStakingIDs[i]).valueStaked == _value) {
                return true;
            }
        }
    ...
    function withdrawTokens(
        ...
        _transferToken(_tokenAddress, _to, _value);
        return true;
    }
