...
contract robo {
	...
	function invest(address referrer) public payable {
        ...
        uint256 referrals = getUserDownlineCount(upline);

		if(referrals == 3 ){
		users[upline].checkpoint3 =  block.timestamp;
		users[upline].lastdate = 134;
		
		}
		if(referrals == 5 ) {
		users[upline].checkpoint5 =  block.timestamp;
		users[upline].lastdate = 100;
		
		}
		if(referrals == 7 ) {
		users[upline].checkpoint7 =  block.timestamp;
		users[upline].lastdate = 67;
	...
	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			uint256 finish = user.lastdate.mul(1 days);
