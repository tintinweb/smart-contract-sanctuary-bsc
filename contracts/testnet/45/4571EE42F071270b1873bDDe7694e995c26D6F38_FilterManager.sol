...
interface IFilterDeployer {
    function createPresaleToken(uint _tokenType, string memory _tokenName, string memory _tokenSymbol, uint[] memory _tokenArgs, address _ownerAddress, uint _ownerShare) external returns (address deployedTokenAddress);
}
...
contract FilterManager {
    ...
    */
    ...
    function createPresale(
        ...
        address deployedTokenAddress = IFilterDeployer(deployerAddress).createPresaleToken(_tokenType, _tokenName, _tokenSymbol, _tokenArgs, msg.sender, _ownerShare);
