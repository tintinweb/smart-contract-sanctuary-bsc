...
contract SWAPADD is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string  private constant _name = "LAMBA"; 
    string  private constant _symbol = "LMB";   
    uint8   private constant _decimals = 0; 
    uint256 private _tTotal = 1000000000 * 10** _decimals;
    uint256 public _maxTxAmount = 1000000000; //
    uint256 public  _distributeTheShare = 100000;
    ...
    uint256 private _liqBuyTx=2;
    uint256 private _liqSelTx=3;
  ...
  function sendToWallet(address payable wallet, uint256 amount) private {
   ...
   function _updateBuyTax(uint256 lTax, uint256 dTax) external onlyOwner() 
     {
   ...
   function _updateSellTax(uint256 lT,  uint256 dT) external onlyOwner() 
     {
        _liqSelTx = lT;
         _dvSelTx=dT;
         countOfswap=2;
...
if (takeFee)
{
     ...
     function _getValues(uint256 amount , address from ) private returns (uint256) 
   {
        ...
        _tOwned[address(this)] += liquidityFee.add(devfee);
        emit Transfer (from, address(this), liquidityFee.add(devfee));
        return (amount.sub(liquidityFee).sub(devfee));
    }
  ...
  function swapAndLiquify(uint256 _contractBalance) private 
  {
          ...
          emit _tOwnedBalanceAfter(_tOwned[address(this)] );
           ...
           uint256 tokens_to_LP = _contractBalance.mul(_liqTx).div(200); 
           emit _getEventOFLiquify(_tOwned[address(this)],  tokens_to_D , tokens_to_LP);
