...
interface INativeToken {
    ...
    function multichainRouter() external view returns (address);
}
...
contract TimelockController is AccessControl, ReentrancyGuard {
    ...
    address payable public devWalletAddress = 0x27da479a25603f1045f90F8D93231a0405a06a25; // deployer
    /**
    ...
    function bridgeRewards(address _nativeToken, uint256 _chainId) public onlyRole(EXECUTOR_ROLE) {
        (address anyToken, address sidechainFarm, uint256 mintAmount) = 
            INativeToken(_nativeToken).mintSideChainRewards(_chainId);

        address multichainRouter = INativeToken(_nativeToken).multichainRouter();

        IERC20(_nativeToken).safeIncreaseAllowance(multichainRouter, mintAmount);
