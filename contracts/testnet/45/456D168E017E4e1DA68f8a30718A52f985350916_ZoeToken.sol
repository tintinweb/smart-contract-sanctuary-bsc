    ...
    DividendPayingTokenOptionalInterface
{
    ...
    address public rewardsToken =
        address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); //BUSD Inu
...
contract ShibaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("Shiba_Dividen_Tracker", "Shiba_Dividen_Tracker")
    {
        claimWait = 3600;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Shiba_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shiba contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Shiba_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "Shiba_Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract ZoeToken is ERC20, ERC20Burnable, Ownable {
    // ADDRESSESS -------------------------------------------------------------------------------------------
    ...
    // CONSTRUCTOR ------------------------------------------------------------------------------------------
    constructor() ERC20("ZoeToken", "ZToken") {
        // mint tokens to deployer
        _mint(msg.sender, 100000000000000000000000000);

        maxWalletAmount = 10000000000000000000000000000;
        maxTransactionAmount = 10000000000000000000000000000;
        ...
        // contract do swap when have 70k tokens balance
        swapThreshold = 70000000000000000000000;

        w1AddressPercent = 3333; //33%
        autoLiquidityPercent = 3333; //33%
        distributionRewardsPercent = 3333; //33%
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SHREW: The dividend tracker already has that address"
        );

        ShibaDividendTracker newDividendTracker = ShibaDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "SHREW: The new dividend tracker must be owned by the token contract"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SHREW: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividends = IERC20(swapTokenAddress).balanceOf(address(this));
        bool success = IERC20(swapTokenAddress).transfer(
    ...
    function _transfer(
        ...
        if (contractMustSwap(from, to)) {
            ...
            uint256 rewardTokens = (numTokensToSwap * distributionRewardsPercent) / masterTaxDivisor;
