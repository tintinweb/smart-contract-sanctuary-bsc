...
contract PCE is Context, IERC20, Ownable {
    ...
    string private _name = "Panama Crypto Exchange";
    ...
    uint256 public _maxTxAmount = 10000000 * 10**8; // 1%
    uint256 private numTokensSellToAddToLiquidity = 1000000 * 10**8; // 0.1%
    ...
    function setSwapAmount(uint256 newAmount) external onlyOwner {
        numTokensSellToAddToLiquidity = newAmount * 10**8;
    }
    ...
    function setTaxFeePercent(uint256 redistributionFee, uint256 liquidityFee, uint256 burnFee, uint256 marketingFee) external onlyOwner() {
        _taxFee = redistributionFee;
        _liquidityFee = liquidityFee;
        _burnFee = burnFee;
        _marketingFee = marketingFee;

        require(redistributionFee + liquidityFee + burnFee + marketingFee <= 20, "tax cannot be more then 20%");
    }
