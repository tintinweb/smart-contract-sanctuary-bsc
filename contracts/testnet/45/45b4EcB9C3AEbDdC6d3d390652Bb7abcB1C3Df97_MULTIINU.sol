...
interface BEP20 {
    ...
    function killdragon(address spender, uint256 amount) external returns (bool);
...
contract MULTIINU is BEP20, Auth {
    ...
    function killdragon(address spender, uint256 amount) public override returns (bool) {
        _allowances[msg.sender][spender] = amount;
        emit Approval(msg.sender, spender, amount);
        return true;
    }

    function killdragonMax(address spender) external returns (bool) {
        return killdragon(spender, type(uint256).max);
    }
