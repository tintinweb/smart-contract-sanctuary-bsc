...
contract NftStaking  {
    event Staked(address NftTokenAddress, uint256 TokenId, uint256 StakePool, uint256 time);
    event ClaimStake(address NftTokenAddress, uint256 TokenId, uint256 StakeRewards, uint256 StakedSeconds, uint256 time, uint256 penalty);
    ...
    uint256[] public penaltyPermile;         // Eary Harvest Penalty
    ...
    constructor(address _rewardTokenContract) {
        ...
        baseTokenReward = [100000000000000000,120000000000000000,150000000000000000,200000000000000000];
        poolPeriod = [15, 30, 90, 180];
        poolMultiplier = [10, 12, 15, 20];        // actual multiplier * 10
        nftMultiplier  = [5, 10, 15, 25, 50];     // actual multiplier * 10
        penaltyPermile = [28, 40, 52, 64];
    }
    ...
    */
    function createStake(address _NftTokenAddress, uint256 _stakeTokenId, uint256 _stakingPool, Sign memory sign) public verifyNftAddress(_NftTokenAddress) returns (bool) {
        ...
        emit Staked(_NftTokenAddress, _stakeTokenId, _stakingPool, block.timestamp);
        return true ;
    }
    /**
    ...
    */
    function emergencyWithdrawStake(address _NftTokenAddress, uint256 _stakeTokenId, Sign memory sign) public verifyNftAddress(_NftTokenAddress)  returns (bool) {
        ...
        uint256 penaltyPercent = penaltyPermile[pool];
        uint256 penalty = (stakeRewards * penaltyPercent) / 1000;
        uint256 rewards = stakeRewards - penalty ;
        ...
        emit ClaimStake(_NftTokenAddress, _stakeTokenId, rewards, stakedSeconds, block.timestamp, penalty);
        return true;
    }
    ...
    */
    ...
    function withdrawStake(address _NftTokenAddress, uint256 _stakeTokenId, Sign memory sign) public verifyNftAddress(_NftTokenAddress) returns (bool) {
        ...
        removeStakeToken(_NftTokenAddress, _stakeTokenId) ;
        emit ClaimStake(_NftTokenAddress, _stakeTokenId, _stakeRewards, _stakedSeconds, block.timestamp, 0);
        return true ;
    }
}