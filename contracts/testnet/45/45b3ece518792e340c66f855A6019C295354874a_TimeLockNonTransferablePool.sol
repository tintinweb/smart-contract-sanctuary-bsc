...
contract TimeLockNonTransferablePool is TimeLockPool, PublicSale, ReentrancyGuard {
    ...
    uint256 public immutable maxWhitelistCount; 
    ...
    /// @dev Deduct token and split into 5 splits and deposit with different lock time
    /// The 1st deposit will be unlocked in 1 year, 2nd will be 1 year 7 days, and so on.
    function depositForWhitelist() external whenPublicSaleActive nonReentrant {
        // TODO: More validations?
        require(whitelistCount + 1 <= maxWhitelistCount, "depositForWhitelist: whitelist already full");
        ...
        uint256 split = 5; // split deposit to 5 chunks
        uint256 amountPerSplit = STAKE_AMOUNT / split; // deposit 600 tokens each split
        uint256 vestingPerSplit = 7 days; // add 7 more days for each of next split
