...
// File: mowsse/IAuctionHouse.sol


pragma solidity ^0.8.2;

/**
 * @title IAuctionHouse auction contract basic interface.
 */
 interface IAuctionHouse {
     ...
     */
    function setMarketplaceSettings(address _address) external;
     ...
     */
    function setIERC721CreatorRoyalty(address _address) external;
     ...
     * - only owner
     * - _address != address(0)
     * @param _address address of the marketPlace.
     */
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Admin function to set the max duration of an auction.
     * Rules:
     * - only owner
     * - _maxDuration > 0
     * @param _maxDuration uint256 max duration of an auction.
     */
    //@review Rename function
    function setMaxAuctionDuration(uint256 _maxDuration) external;
     ...
     * - Minimum bid must be >= 0 and also must be >= than marketPlaceMinValue and <= marketPlaceMaxValue
     ...
     * @param _minBidAmount uint256 BLOK value of the minBidAmount.
    ...
    function createAuction(address _nftAddress, uint256 _tokenId,uint256 _minBidAmount,uint256 _lengthOfAuctionInSeconds,uint256 _startingTimeInSeconds,address _tokenOwner) external;
     ...
     * - Auction can or cannot have started but if the auction has started then there should be no bidding.
     ...
     */
    function cancelAuction(address _nftAddress, uint256 _tokenId,uint256 _auctionId,address _auctionCreator) external ;
     ...
     * @param _tokenId uint256 id of the token.
     * @param _auctionId uint256 id of the auction.
     * @param _newMinBidAmount uint256 new minimum bid of the auction.
     * @param _newStartingTime uint256 new starting time of the auction.
     * @param _newLengthOfAuction uint256 new duration of the auction.
     * @param _auctionCreator address of the auction creator.
     */

    function updateAuction(address _nftAddress, uint256 _tokenId, uint256 _auctionId, uint256 _newMinBidAmount , uint256 _newStartingTime , uint256 _newLengthOfAuction , address _auctionCreator ) external;
     ...
     * Rules:
     * - There must be a running auction for the token
     * - bid > 0
     ...
     * - block.timestamp >= startingTimeInSeconds
     * - if previous bid then returned
     * @param _nftAddress address of ERC721 contract.
     * @param _tokenId uint256 id of the token.
     * @param _amount uint256 BLOK value of the bid.
     ...
     * Rules:
     * - There must be a bid for the auction
     * - bidder can cancel its bid only
     * @param _nftAddress address of ERC721 contract.
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Accept bid of an auction.
     * Rules:
     * - Auction must have ended
     * - Only auction creator can accept bid
     * - auctionId > 0
     * @param _nftAddress address of ERC721 contract.
     * @param _auctionId uint256 id of the token.
     * @param _tokenId uint256 id of the token.
     ...
     * @param _tokenId uint256 id of the token.
     * @param _auctionId uint256 id of the token.
     * @param _bidder address of the buyer
     */

    //@Review should be marked as non-Rentrant
...
// File: mowsse/IWhitelist.sol


pragma solidity ^0.8.2;

/**
 * @title IERC721 Non-Fungible Token Creator basic interface,**needs to be change**
 */
interface IWhitelist {
     ...
     */
    function enableWhitelist(bool _enabled) external;
     ...
     */
    function _unWhitelist(address _removedAddress) external;

    /**
     * @dev Internal function for adding the provided address to the whitelist
     * @param _newAddress address to be added to the whitelist
     */
    function _whitelist(address _newAddress) external;
}
...
interface IMarketplaceSettings {
     ...
     */
    function getBuyerRewardFeePercentage() external view returns (uint16);
...
contract Payments is Initializable, SendValueOrEscrow {
   ...
   function initializePayments(address _vault, address _blokTokenAddress) public {
     ...
     * @param _marketplacePercentage uint16 percentage of the fee for the marketplace.
     * @param _royaltyPercentage uint16 percentage of the fee for the royalty.
     * @param _buyerRewardPercentage uint16 percentage of rewards for buyer in blok.
     * @param _payee address seller of the token.
    ...
    function payout(
        uint256 _amount,
        bool _isPrimarySale,
        uint16 _marketplacePercentage,
        uint16 _royaltyPercentage,
        uint16 _buyerRewardPercentage,
        address  _payee,
     ...
     */
     //@Review for 1 percent = 100  
    //for 2.5 percent = 250 ie  2.5 * 100
    function calcPercentagePayment(uint256 _amount, uint16 _percentage)
        internal
        pure
        returns (uint256)
    {
        return (_amount.mul(_percentage)).div(100 * 100);
    }
}
    ...
    IERC721ReceiverUpgradeable
{
    ...
    // Mowse WhiteList Interface
    IWhitelist public iWhitelist;

    // Mowse AuctionHouse Interface
    IAuctionHouse public iAuctionHouse;

    //Types of item State
    bytes32 public constant NOT_FOR_SALE = bytes32(0);
    bytes32 public constant MARKET_PLACE = "MARKET_PLACE";
    bytes32 public constant AUCTION_HOUSE = "AUCTION_HOUSE";
    ...
    function initialize(
        ...
        address _mowsseAuctionHouse,
        IWhitelist _mowsseNft
    ) public initializer {
        ...
        require(
            ...
            "constructor::Cannot have null address for _blokTokenAddress"
        );
        ...
        require(
            _mowsseAuctionHouse != address(0),
            "constructor::Cannot have null address for _mowsseAuctionHouse"
        );
        ...
        //Set iAuctionHouse
        iAuctionHouse = IAuctionHouse(_mowsseAuctionHouse);
        ...
        Payments.initializePayments(_vault, _blokTokenAddress);
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Admin function to set the auctionHouse.
     * Rules:
     * - only owner
     * - _address != address(0)
     * @param _address address of the auctionHouse.
     */

    function setAuctionHouse(address _address) public onlyOwner {
        require(
            _address != address(0),
            "aucitonHouse::Cannot have null address for auctionHouse"
        );

        iAuctionHouse = IAuctionHouse(_address);
    }
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Admin function to set the IWhitelist
     * Rules:
     * - only owner
     * @param _address address of the IWhitelist.
     */
    function setMowseContractAddress(IWhitelist _address) public onlyOwner {
        iWhitelist = _address;
    }
    ...
    function grantTokenListRole(address _account) external {
        ...
        require(_account != address(0) , "cannot have null address for artist");
        ...
        iWhitelist.addToWhitelist(_account);
    }
    ...
    function revokeTokenListRole(address _account) external {
        ...
        iWhitelist.removeFromWhitelist(_account);
    }
     ...
     * @param _tokenId uint256 id of the NFT token
     * @param _price uint256 price of the item
     * @param _creator address of the NFT creator
     * @param _seller address of the seller
     * @param isAuction boolean to check for auction
     */
    function setItemDetails(address _nftContract, uint256 _tokenId , uint256 _price , address _creator , address _seller , bool isAuction) internal returns(uint256) {
        // The sender must be the token owner
        senderMustBeTokenOwner(_nftContract, _tokenId);
        _itemIds.increment();

        uint256 itemId = _itemIds.current();
        bytes32 _itemState = isAuction ? AUCTION_HOUSE : MARKET_PLACE;
    ...
    /**
     * @dev Checks that the token is owned by the sender
     * @param _nftContract address of the contract storing the token.
     * @param _tokenId uint256 ID of the token
     * @param _minBidAmount uint256 minimum bid of the auction
     * @param _duration uint256 duration of the auction in epoch timestamp
     * @param _startingTimeInSeconds uint256 staring time of the auction in epoch timestamp
     */

    function createAuctionItem(address _nftContract,
        uint256 _tokenId,
        uint256 _minBidAmount,
        uint256 _duration,
        uint256 _startingTimeInSeconds) public nonReentrant{
      ...
      uint256 itemId =  setItemDetails(_nftContract, _tokenId, _minBidAmount, msg.sender, msg.sender, true);

        iAuctionHouse.createAuction(_nftContract,
            _tokenId,
            _minBidAmount,
            _duration,
     ...
     * @param _nftContract address of origin Token smart contract
     * @param _tokenId  of NFT from origin Token smart contract
     * @param _price of NFT to sale
     *
     */
    ...
    function createMarketItem(
        ...
        require(_nftContract != address(0), "cannot have null address of _nftContract");

        require(_tokenId >=0 , "Invalid token Id");

        require(_price >= iMarketplaceSettings.getMarketplaceMinValue() , "Cannot set price lower than min value");

        require(
            _price <= iMarketplaceSettings.getMarketplaceMaxValue(),
            "Cannot set price higher than max value"
        );

        uint256 itemId = setItemDetails(_nftContract, _tokenId, _price, msg.sender, msg.sender, false);
     ...
     * @param _nftContract address of the NFT contract
     * @param _tokenId uint256 id of a NFT
     * @param _auctionId uint256 id of an auction
     * @param _newMinBidAmount uint256 minimum bid amount of the auction
     * @param _newStartingTime uint256 starting time of the auction
     * @param _newLengthOfAuction uint256 length of the auction
     */
     function updateAuctionItem(uint256 itemId, address _nftContract, uint256 _tokenId , uint256 _auctionId ,uint256 _newMinBidAmount , uint256 _newStartingTime , uint256 _newLengthOfAuction ) public{
         iAuctionHouse.updateAuction(_nftContract, _tokenId, _auctionId, _newMinBidAmount, _newStartingTime, _newLengthOfAuction, msg.sender);

        idToMarketItem[itemId].price = _newMinBidAmount;
     }
     ...
     * @param _sold boolean sale status of the item
     * @param _firstSold boolean first time sale status of the time
     * @param _nftContract address of the NFT contract
     * @param _tokenId uint256 id of the NFT token 
     */
    function updateItemDetails(uint256 itemId , address _to , bool _sold, bool _firstSold , bytes32 _itemState , address _nftContract , uint256 _tokenId) internal{
        idToMarketItem[itemId].owner = _to;
        idToMarketItem[itemId].sold = _sold;
        idToMarketItem[itemId].firstSold = _firstSold;
        idToMarketItem[itemId].itemState = _itemState;

        _itemsSold.increment();
        // Set token as sold
        iMarketplaceSettings.markERC721Token(_nftContract, _tokenId, true);
     ...
     */
     ...
     function createAuctionSale( uint256 itemId,
        address _to,
        uint256 _auctionId) public nonReentrant{

        require(iMarketplaceSettings.checkModeratorRole(msg.sender),"Caller is not Moderator");

        require(itemId > 0 , "Invalid item Id");

        require(_auctionId >=0 , "Invalid auction Id");

        require(_to != address(0),"cannot have null address for buyer");
        ...
        address payee = idToMarketItem[itemId].seller;
         ...
         iAuctionHouse.auctionPayout(nftContract, tokenId, _auctionId, _to);

         updateItemDetails(itemId , _to , true, true , NOT_FOR_SALE , nftContract , tokenId);
     }
    ...
    function createMarketSale(
        ...
        address payee = idToMarketItem[itemId].seller;
            ...
            Payments.payout(
                price,
                !iMarketplaceSettings.hasERC721TokenSold(nftContract, tokenId),
                iMarketplaceSettings.getMarketplaceFeePercentage(),
                iERC721CreatorRoyalty.getERC721TokenRoyaltyPercentage(
                    nftContract,
                    tokenId
                ),
                iMarketplaceSettings.getBuyerRewardFeePercentage(),
                payee,
                owner(),
                iERC721CreatorRoyalty.tokenCreator(nftContract, tokenId),
                _to
            );

            IERC721Upgradeable(nftContract).transferFrom( // change msg.sender to "_to"
                address(this),
                _to,
                tokenId
            );
    ...
    /////////////////////////////////////////////////////////////////////////
    // updateItemResellDetails
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Creates the sale of a marketplace item and Transfers ownership of the item, as well as funds between parties
     * @param itemId  of NFT from origin Token smart contract
     * @param _owner address of the NFT owner
     * @param _seller address of NFT seller
     * @param _newPrice uint256 new price of the NFT 
     * @param _sold boolean sale state of the NFT
     * @param _itemState bytes32 state of the item
     * @param _nftContract address of the NFT contract
     * @param _tokenId uint256 id of the NFT
     */
    function updateItemResellDetails(uint256 itemId , address _owner , address _seller , uint256 _newPrice , bool _sold , bytes32 _itemState , address _nftContract, uint256 _tokenId) internal{
    
        idToMarketItem[itemId].owner = _owner;
        idToMarketItem[itemId].seller = _seller;
        idToMarketItem[itemId].price = _newPrice;
        idToMarketItem[itemId].sold = _sold;
        idToMarketItem[itemId].itemState = _itemState;
     ...
     * @param _tokenId uint256 id of the NFT token
     * @param _minBidAmount uint256 minimum bid amount of the auction
     * @param _duration uint256 duration of the auction
     * @param _startingTimeInSeconds uint256 starting time of the auction in epoch timestamp
     */
    function resellAuctionItem(uint256 itemId,
        address _nftContract,
        uint256 _tokenId,
        uint256 _minBidAmount,
        uint256 _duration,
        ...
        updateItemResellDetails(itemId ,address(0), msg.sender, _minBidAmount, false, AUCTION_HOUSE , _nftContract, _tokenId);
        ...
        onlyItemOwner(itemId)
    {
        ...
        require(newPrice >= iMarketplaceSettings.getMarketplaceMinValue() , "cannot set price lower than min value");

        require(
            newPrice <= iMarketplaceSettings.getMarketplaceMaxValue(),
            "Cannot set price higher than max value"
        );
        ...
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        address nftContract = idToMarketItem[itemId].nftContract;
        bytes32 _itemState = idToMarketItem[itemId].itemState;
        ...
        updateItemResellDetails(itemId , address(0), msg.sender, newPrice, false, MARKET_PLACE , nftContract, tokenId);
    }
        ...
        nonReentrant
    {
        ...
        require(tokenMarketData.itemState != AUCTION_HOUSE , "cannot transfer listed auction item");
        ...
        if (
            ...
            if (
                !idToMarketItem[itemId].firstSold ||
            ...
            tokenMarketData.owner = to;
            //setting seller to to, becasue seller can un-list item
            tokenMarketData.seller = to;
            tokenMarketData.itemState = NOT_FOR_SALE;
            ...
            tokenMarketData.owner = to;
            //setting seller to to, because seller can un-list item
            tokenMarketData.seller = to;
            tokenMarketData.itemState = NOT_FOR_SALE;
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @dev Auction can or cannot have started if 
     * @param itemId of NFT to unlist
     * @param _nftContract address of the NFT contract
     * @param _tokenId uint256  id of the NFT
     * @param _auctionId uint256 id of the auction
     * @param _auctionCreator address of the auction creator
     * conditions, owner must be seller or mp owner
     */
    function unListAuctionItem(uint256 itemId , address _nftContract, uint256 _tokenId, uint256 _auctionId , address _auctionCreator) public {
        ...
        iAuctionHouse.cancelAuction(_nftContract, _tokenId, _auctionId , msg.sender);

        idToMarketItem[itemId].owner = _auctionCreator;
        idToMarketItem[itemId].sold = false;
        idToMarketItem[itemId].itemState = NOT_FOR_SALE;
     ...
     * @param itemId itemId of NFT to change price
        ...
        onlyProductSeller(itemId)
    {
        ...
        require(newPrice >= iMarketplaceSettings.getMarketplaceMinValue() , "price cannot be less than min value");

         require(
            newPrice <= iMarketplaceSettings.getMarketplaceMaxValue(),
            "Cannot set price higher than max value"
        );
        require(idToMarketItem[itemId].itemState != AUCTION_HOUSE , "cannot set minimum bid for marketItem");
       
        MarketItem storage item = idToMarketItem[itemId];
        uint256 oldPrice = item.price;
        item.price = newPrice;

        emit MarketItemUpdated(itemId, oldPrice, newPrice);
    }
    ...
    /////////////////////////////////////////////////////////////////////////
    /**
     * @return only items a user has listed to marketPlace
     */
