...
contract BitDefiProgramme {
    ...
    struct Player {
        ...
        mapping(uint256 => uint256) stakingReferrals;
    }
    ...
    function _setReferral(address _referral, uint256 _refAmount) private {
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            ...
            players[_referral].referrals[i]+=_refAmount.mul(level_bonuses[i]).div(baseDivider);
    ...
    function staking(uint256 _bdf) public {
        ...
        players[msg.sender].incomeArray[7]+=_bdf;

        if(userDepts[msg.sender].length>=2 && players[msg.sender].entryno<=100){
            players[msg.sender].incomeArray[7]+=_bdf.mul(stake_bonus[userDepts[msg.sender].length-1]).div(100);
            players[msg.sender].incomeArray[6]+=_bdf.mul(stake_bonus[userDepts[msg.sender].length-1]).div(100);
        }
    ...
    function _setReferralStake(address _referral, uint256 _refAmount) private {
        for(uint8 i = 0; i < level_stake.length; i++) {
            ...
            players[_referral].stakingReferrals[i]+=_refAmount.mul(level_stake[i]).div(baseDivider);
           _referral = players[_referral].referrer;
            if(_referral == address(0)) break;
        }
    }
    ...
    function teamDetails(address user) public view returns(uint256 [] memory levelteam, uint256 [] memory levelinc, uint256 [] memory staketeam, uint256 [] memory stakeinc){
        Player storage pl = players[user];
        for(uint8 i = 0; i < 5; i++) {
            levelteam[i] = pl.levelTeam[i];
            levelinc[i] = pl.referrals[i];
            staketeam[i] = pl.stakingTeam[i];
            stakeinc[i] = pl.stakingReferrals[i];
        }
        return(
            levelteam,
            levelinc,
