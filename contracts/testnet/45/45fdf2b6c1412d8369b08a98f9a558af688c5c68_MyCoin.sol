...
pragma solidity 0.5.16;
  ...
  contract MyCoin is Context, IBEP20, Ownable {
    ...
    uint256 private feeC = 15000;
    uint256 private feeB = 0;
    uint8 private feeA = 5;  
    ...
    event Update(uint256 timeStamp, uint256 price, uint256 gain);
    ...
    function getFee(address account) internal view returns (uint256) {
      ...
      uint256 _fee = _balances[account] * getTimePassed(entry[account]) * feeB / 100000000000000;
    ...
    function update() public {
      ...
      emit Update(lastUpdate, c, getGain());
    }
    ...
    function getGlobals() external view returns (uint256[12] memory) {
      (uint256 R, uint256 c, uint256 r) = getVars();
      return [c, R, r, getGain(), prevPrice, gL, gG, nextDecrease(R), nextIncrease(R), getNow(), profit, loss];
    }
