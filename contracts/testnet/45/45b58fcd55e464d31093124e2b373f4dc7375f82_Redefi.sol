...
contract Redefi is Context, IERC20, Ownable {
    ...
    function _takeDevelopment(uint256 rDevelopment, uint256 tDevelopment) private {
        totFeesPaid.marketing +=tDevelopment;
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rDevelopment, to_return.rCharity, to_return.rLiquidity, to_return.rBuyback) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tDevelopment = tAmount*sellFeeRates.marketing/1000;
            s.tCharity = tAmount*sellFeeRates.burning/1000;
            s.tLiquidity = tAmount*sellFeeRates.liquidity/1000;
            s.tBuyback = tAmount*sellFeeRates.buyback/1000;
            s.tTransferAmount = tAmount-s.tRfi-s.tDevelopment-s.tCharity-s.tLiquidity-s.tBuyback;
        }
        else{
            s.tRfi = tAmount*feeRates.rfi/1000;
            s.tDevelopment = tAmount*feeRates.marketing/1000;
            s.tCharity = tAmount*feeRates.burning/1000;
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rDevelopment, uint256 rCharity, uint256 rLiquidity, uint256 rBuyback) {
        ...
        rTransferAmount =  rAmount-rRfi-rDevelopment-rCharity-rLiquidity-rBuyback;
        return (rAmount, rTransferAmount, rRfi,rDevelopment,rCharity,rLiquidity, rBuyback);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        ...
        emit Transfer(sender, address(this), s.tLiquidity + s.tDevelopment + s.tCharity + s.tBuyback);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        ...
        uint256 developmentAmt = unitBalance * 2 * feeRates.marketing;
        if(developmentAmt > 0){
          payable(marketingAddress).transfer(developmentAmt);
        }
        
        // Send BNB to developmentWallet
        uint256 charityAmt = unitBalance * 2 * feeRates.burning;
        if(charityAmt > 0){
          payable(burningAddress).transfer(charityAmt);
        }
    }
