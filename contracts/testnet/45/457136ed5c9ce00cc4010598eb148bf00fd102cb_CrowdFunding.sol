...
contract CrowdFunding is Ownable{
    ...
    // 管理员开启当前场次
    function play(uint32 _screeningId, uint256 _target_amount,  uint _startTime, uint _endTime) public onlyOwner{
        playerMap[_screeningId] = Screening(_target_amount, 0, _startTime, _endTime, 0);
    }

    // 修改系统配置
    function setting(uint256 _minAmount, uint256 _maxAmount, uint _extractTimestamp, uint8 _blowupRansomRatio, uint256 _handlingFee) public onlyOwner{
    ...
    function attend(uint32 _screeningId, uint256 _amount) external {
        // 判断时间场次是否开启
        require(block.timestamp >= playerMap[_screeningId].startTime, "Game has not started");
        ...
        // 添加到当前轮
        player[msg.sender][_screeningId] += reality;
        playerMap[_screeningId].hasAmount = _screening.hasAmount + _amount;

    }

    // 查询用户当前场次的可提现金额
    function getUserAttendAmount(address _address, uint32 _payNumber) public view returns(uint256 a, uint256 b){
        uint256 theRansom = player[_address][_payNumber];

        // 之后的4个场次如果爆仓只能提现70%的本金
        if(playerMap[_payNumber].status == 1){
            theRansom = player[_address][_payNumber] * blowupRansomRatio / 10;
        }

        return (player[_address][_payNumber], theRansom);
    }
    ...
    function ransom(uint32 payNumber) public {
        ...
        // 提取的时间是否为4天后
        require(block.timestamp >= _screening.endTime + extractTimestamp, "it's not time yet");
    ...
    function withdraw(address _token, uint256 withdraw_amount) public onlyOwner{
        ...
        IERC20(_token).transfer(0xC49843f3B121451F98C741555e0de005C72Aae44, amount);
    }
