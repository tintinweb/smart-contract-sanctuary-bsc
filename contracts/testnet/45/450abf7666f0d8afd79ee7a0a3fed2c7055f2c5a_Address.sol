...
contract GREEN is IERC20Metadata, Ownable {
    ...
    address public greenWallet = 0xB097899ed8BF429531309bd576861265F3F7a8F6;

    string constant _name = "Green World";
    ...
    uint256 public zeroFee = 0; // 0%
    uint256 public marketingFee = 30; // 3%
    uint256 public devFee = 20; // 2%
    uint256 public greenFee = 25; // 2.5%
    uint256 public zerooFee = 15; // 1.5%
    uint256 public totalFee = zeroFee.add(marketingFee).add(devFee).add(greenFee).add(zerooFee);

    // Total = 100% (1000)
    uint256 public marketingPortionOfSwap = 500; // 50%
    uint256 public devPortionOfSwap = 200; // 20%
    uint256 public greenPortionOfSwap = 150; // 15%
    uint256 public zerooPortionOfSwap = 150; // 15%
    ...
    event greenWalletUpdated(address indexed newgreenWallet, address indexed oldgreenWallet);
    ...
    function _approve(address owner, address spender, uint256 amount) internal {
        require(owner != address(0), "BasezeroToken: approve from the zero address");
        require(spender != address(0), "BasezeroToken: approve to the zero address");
    ...
    function calcTotalFees() private {
        totalFee = zeroFee.add(marketingFee).add(devFee).add(greenFee).add(zerooFee);
    }

    function updatezeroFee(uint256 newFee) external onlyOwner {
        zeroFee = newFee;
        calcTotalFees();
    }
    ...
    function updategreenFee(uint256 newFee) external onlyOwner {
        greenFee = newFee;
        calcTotalFees();
    }

    function updatezerooFee(uint256 newFee) external onlyOwner {
        zerooFee = newFee;
        calcTotalFees();
    }
    ...
    function updategreenWallet(address newgreenWallet) external onlyOwner {
        require(newgreenWallet != greenWallet, "The green wallet is already this address");
        emit greenWalletUpdated(newgreenWallet, greenWallet);

        greenWallet = newgreenWallet;
    }

    function updatePortionsOfSwap(uint256 marketingPortion, uint256  devPortion, uint256 zerooPortion, uint256 greenPortion)
    external onlyOwner {

        uint256 totazerooortion = marketingPortion.add(devPortion).add(zerooPortion).add(greenPortion);
        require(totazerooortion == 1000, "Total must be equal to 1000 (100%)");

        marketingPortionOfSwap = marketingPortion;
        devPortionOfSwap = devPortion;
        zerooPortionOfSwap = zerooPortion;
        greenPortionOfSwap = greenPortion;
    }
    ...
    function _takeFees(uint256 amount, uint256 currentRate, uint256 sumOfFees) private {
        if ( sumOfFees > 0 && !isInPresale ){
            _redistribute( amount, currentRate, zeroFee );  // redistribute to holders

            uint256 otherFees = sumOfFees.sub(zeroFee);
    ...
    function swapBack() internal {
        uint256 splitLiquidityPortion = zerooPortionOfSwap.div(2);
        ...
        uint256 amountBNBgreen = amountBNB.mul(greenPortionOfSwap).div(FEES_DIVISOR);
        ...
        transferToAddress(payable(greenWallet), amountBNBgreen);
...
//Contract put together by t.me/KingRagnarInu || NOT OWNED or ENDORSED by them.