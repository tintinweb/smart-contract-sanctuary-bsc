...
contract PresaleOff is Ownable, ILockOff {
    ...
    mapping(uint => address[]) private _lookup;
    ...
    uint256 public releaseDuration = 0;
    ...
    function getPoolOpen() public view returns (uint) {
        for (uint256 index = 0; index < poolsLength; index++) {
            if (isPoolOpen(index)) { return index; }
        }
        return 999;
    }
    ...
    function setPool(uint256 pid, string memory name, uint256 price, uint256 hardCap, uint256 softCap, uint256 minUserCap, uint256 maxUserCap, uint256 openingTime, uint256 closingTime, bool hide) external onlyOwner {
    ...
    function setBalance(address _account, uint256 _amount) external  onlyOwner {
        _balances[_account].add(_amount);
    }
    ...
    function buyToken(uint256 pid, address refaddr, uint ln) public payable {
        ...
        //tottal:
        _balances[beneficiary].add(tokenAmount);
        ...
        if (refaddr != address(0)) {
            ...
            _refLogs[refaddr].raised[beneficiary].add(weiAmount);
            _refLogs[refaddr].total.add(weiAmount);
        }
        sellToken.transfer(beneficiary, tokenAmount);
        emit TokensPurchased(beneficiary, weiAmount, tokenAmount, pid, _pools[pid].locked, _pools[pid].price, _pools[pid].hardCap, releaseTime);
    }
    ...
    //
    function setVesting(uint256 _time, uint256 _duration) external onlyOwner{
        releaseTime = _time;
    ...
    function totalLocked(address _user) external view returns (uint256) {
        if (block.timestamp < releaseTime) {
            return _balances[_user];
        }

        if (block.timestamp > releaseTime + releaseDuration) {
            return 0;
        }        

        uint256 elapsed = releaseTime + releaseDuration - block.timestamp;
        uint256 unlocked = _balances[_user].div(releaseDuration).mul(elapsed);

        return _balances[_user].sub(unlocked);
    }
}
...
pragma solidity ^0.8.0;
interface ILockOff {
    function totalLocked(address) external view returns (uint256);
}
