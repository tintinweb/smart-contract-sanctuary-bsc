...
contract ProxyContractAPI is ChainlinkClient {
    ...
    function requestData() public returns (bytes32 requestData) {
    ...
    function fulfill(
        ...
        uint256 _data
    ) public recordChainlinkFulfillment(_requestData) {
        emit RequestData(_requestData, _data);
        results = _data;
        isWaiting = false;
    }

    function check_result(uint256 _result) public returns (bool) {
        if (_result == 13371) {
            return true;
        }
        return false;
