...
contract MembershipPass is IMembershipPass, ERC1155, Ownable {
  ...
  mapping(uint256 => uint256) public suppliedByTier;
   ...
   *
   * @param _tier The token ID of current sold item
  ...
  function royaltyInfo(uint256 _tier, uint256 _salePrice)
    ...
    returns (address receiver, uint256 royaltyAmount)
  {
    return (_feeCollector, (_salePrice * tierFee[_tier]) / 10000);
  }
  ...
  function uri(uint256 _tokenId) public view override returns (string memory) {
    string memory _baseURI = configStore.baseMembershipPassURI();

    return
      bytes(_baseURI).length > 0
        ? string(abi.encodePacked(_baseURI, _projectId.toString(), "/", _tokenId.toString()))
        : "";
  }
   ...
   *
   * @dev
   * RemainingAmount = capacity - supplid

   * @param _tier The token id
   */
  function getRemainingAmount(uint256 _tier) external view returns (uint256 _remainingAmount) {
    _remainingAmount = tierCapacity[_tier] - suppliedByTier[_tier];
  }
  ...
  constructor(
    uint256 _daoId,
    address _royaltyFeeCollector,
    uint256[] memory _tierFees,
    uint256[] memory _tierCapacities,
    IConfigStore _configStoreProp
  ) ERC1155("") {
    if (_tierFees.length == 0) revert BadTierSize();
    if (_tierFees.length != _tierCapacities.length) revert BadTierSize();
    if (_configStoreProp == IConfigStore(address(0)) || _royaltyFeeCollector == address(0))
      revert ZeroAddress();

    for (uint256 i; i < _tierFees.length; ) {
      // The capacity should greater than 0
      if (_tierCapacities[i] == 0) revert BadCapacity();

      // The royalty fee hould be less than max value
      if (_tierFees[i] > MAX_ROYALTY_FEE) revert BadFee();

      tierFee[i] = _tierFees[i];
      tierCapacity[i] = _tierCapacities[i];
      unchecked {
        i++;
      }
    }
    ...
    _projectId = _daoId;
    configStore = _configStoreProp;
    _feeCollector = _royaltyFeeCollector;
  }
  ...
  function mintPassForMember(
    ...
    _beforeMint(_tier, _amount);

    suppliedByTier[_tier] += _amount;
  ...
  function batchMintPassForMember(
    ...
    for (uint256 i; i < _tiers.length; ) {
      _beforeMint(_tiers[i], _amounts[i]);
      suppliedByTier[_tiers[i]] += _amounts[i];
   ...
   *
   */
  function _beforeMint(uint256 _tier, uint256 _amount) private view {
    if (suppliedByTier[_tier] + _amount > tierCapacity[_tier]) revert InsufficientBalance();
  }
}
...
contract MembershipPassBooth is IMembershipPassBooth {
  /*╔═════════════════════════════╗
    ║  Public Stored Properties   ║
    ╚═════════════════════════════╝*/
  ...
  function issue(
  ...
  ) external override returns (address _membershipPass) {
    ...
    emit Issue(_projectId, _membershipPass, _royalty, _tierFees, _tierCapacities, _multipers);
  }
  ...
  function batchMintTicket(
    ...
    if (!configStore.mintRoles(msg.sender)) revert UnAuthorized();

    membershipPassOf[_projectId].batchMintPassForMember(_from, _tierIds, _amounts);

    emit BatchMintTicket(_from, _projectId, _tierIds, _amounts);
  }
   ...
   * @param _projectId The ID of the DAO
   * @param _from The wallet address of owner
   * @param _tierId The special token id
  ...
  function mintTicket(
    ...
    membershipPassOf[_projectId].mintPassForMember(_from, _tierId, _amount);

    emit MintTicket(_from, _projectId, _tierId, _amount);
  }
}
