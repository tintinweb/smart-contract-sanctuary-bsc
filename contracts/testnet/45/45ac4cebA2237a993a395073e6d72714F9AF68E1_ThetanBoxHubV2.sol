...
contract ThetanBoxHubV2 is Ownable, HasSignature {
  /* ========== LIBs ========== */
  ...
  function setSigner(address _signer) external onlyOwner {
    require(_signer != address(0), 'ThetanBoxPayment: invalid signer address');
    signer = _signer;
  }
  ...
  function buyBoxWithSignature(
    ...
    paymentToken.safeTransferFrom(msg.sender, paymentReceivedAddress, price);

    ids[id] = true;
    // Emit payment event
    emit ThetanBoxPaid(id, boxId, msg.sender, boxType, price, paymentErc20);
  }
}
