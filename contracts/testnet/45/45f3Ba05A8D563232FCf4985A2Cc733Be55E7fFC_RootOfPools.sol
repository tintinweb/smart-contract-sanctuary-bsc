...
contract RootOfPools is Initializable, OwnableUpgradeable {
    ...
    //Доделать ведение и вывод массива
    Pool[] Pools;
    //********************************
    ...
    /*constructor(address usdtAddress, address rankingAddress) {
    ...
    /* Депозит в токене сети
    function deposit(string calldata name) public payable {
        //===========================================================================================
        //Доделать проверку состояния дочернего выбранного
        require(_poolsTable[name]!=address(0),"ERROR: Selected pool does not exist!");

        //_poolsTable[name].deposit()  msg.value - 20%
...
contract BranchOfPools is Ownable {
    ...
    function init() public onlyOwner returns (uint256) {
        uint256 temp = 10**IERC20(_usdt).decimals();
        _VALUE = _VALUE * temp;
    ...
    function paybackEmergency() public onlyState(State.Emergency) {
        ...
        if (usdtT != 0) {
            IERC20(_usdt).transfer(tx.origin, usdtT);
        }
    }
    ...
    function depositUSDT(uint256 amount) public onlyState(State.Fundrasing) {
        ...
        uint256 Min = (10**IERC20(_usdt).decimals()) *
            Ranking(RootOfPools(_root).getRanks()).getParRankOfUser(tx.origin)[
                0
            ];
        uint256 Max = (10**IERC20(_usdt).decimals()) *
            Ranking(RootOfPools(_root).getRanks()).getParRankOfUser(tx.origin)[
                1
            ];
        ...
        require(
            IERC20(_usdt).allowance(tx.origin, address(this)) >= amount,
            "DEPOSIT: ALLOW ERROR"
        );

        IERC20(_usdt).transferFrom(tx.origin, address(this), amount);
    ...
    function collectFunds() public onlyOwner onlyState(State.WaitingToken) {
        ...
        //Send to devs
        IERC20(_usdt).transfer(_devUSDTAddress, _FUNDS_RAISED);
    ...
    {
        require(
            IERC20(tokenAdr).balanceOf(address(tx.origin)) >= amount,
        ...
        IERC20(_token).transferFrom(tx.origin, address(this), toDistribute);
        ...
        _CURRENT_VALUE_TOKEN = IERC20(tokenAdr).balanceOf(address(this));
    ...
    function claim() public onlyState(State.TokenDistribution) {
        ...
        IERC20(_token).transfer(tx.origin, amount);

        require(
            IERC20(_token).balanceOf(address(this)) == _CURRENT_VALUE_TOKEN,
