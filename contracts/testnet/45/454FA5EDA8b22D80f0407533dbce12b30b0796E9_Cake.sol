...
contract Cake is ERC20, Ownable {
    ...
    event masterChefUpdated  (address authorizer, address oldMasterChef, address newMasterChef);

    constructor () ERC20("PlexSwap Token", "WAYA") { }

    function setMasterChef (address _newMasterChef) public onlyOwner {
        require(_newMasterChef != address(0), "Cannot be zero address");
        address oldMasterChef = masterChef;
        masterChef = _newMasterChef;
        emit masterChefUpdated (_msgSender(), oldMasterChef, masterChef);
    }

    /// @dev Creates `_amount` token to `_to`. Must only be called by the MasterChef.
    function mint(address _to, uint256 _amount) public {
        require(masterChef != address(0), "MasterChef cannot be zero address");
        require(_msgSender() == masterChef, "Sender not authorized");
        _mint(_to, _amount);
