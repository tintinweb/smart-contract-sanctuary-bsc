...
contract BHOLaunchpad is Ownable, Pausable, Whitelist, SharedConstants {
    ...
    uint256 public EMERGENCY_WITHDRAW_PORTION = 90; // 90%

    address public FACTORY_ADDRESS;

    uint256 public percentCanBuyInFCFS = 25;
    ...
    function updatePercentCanBuyInFCFS(uint256 _percent) external onlyOwner {
        ...
        require(_percent <= 100, "Percent be less than 100");
     ...
     * User un register
     */
    function registrationInfo(address _addr) public view returns (RegistrationInfo memory) {
    ...
    function buyTokenAllocation(uint256 _amount) external checkWhoCanBuy {
        ...
        uint256 maxBuy = launchpadData.baseAllocation.mul(registrationList[buyer].poolWeight);
    ...
    function getMaxTokenPayment(address _addr) public view returns (uint256) {
        // BUSD
        uint256 maxBuy = launchpadData.baseAllocation.mul(registrationList[_addr].poolWeight);
        return maxBuy;
    }
