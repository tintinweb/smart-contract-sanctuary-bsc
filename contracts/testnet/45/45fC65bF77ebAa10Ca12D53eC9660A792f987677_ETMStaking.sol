...
contract ETMStaking is WhitelistedRole, Pausable, EnableTokensClaimable, LockStaking, ReentrancyGuard {
    ...
    event Withdraw(address indexed from, address indexed withdrawer, uint256 fund, uint256 reward);
    ...
    uint256 public interestRewardPeriod; 
    uint256 public minInterestRewardPeriod;
    uint8 public fixedAPR; 
    uint8 public unstakeFeeBeforeEnd; 
    ...
    function stake(uint256 depositAmount) public whenNotPaused whenNotStakingLocked {
        ...
        emit Deposit(_msgSender(), address(this), depositAmount);
    }
    ...
    function unStakes(uint8[] memory indexes) public whenNotPaused whenTokensClaimableEnabled nonReentrant {
        for (uint8 i = 0; i < indexes.length; i++) {
            ...
            emit Withdraw(address(this), _msgSender(), withdrawalAmount, reward);
        }
    }
    ...
    function getReward(address account, uint256 timestamp, uint8 index) public view returns(uint256) {
        ...
        if (period >= interestRewardPeriod*DAY_IN_SECONDS) {
            ...
            return depositAmount*(fixedAPR/100*(period/YEAR_IN_SECONDS))*(1 - unstakeFeeBeforeEnd/100);
        } else return 0;
    }
