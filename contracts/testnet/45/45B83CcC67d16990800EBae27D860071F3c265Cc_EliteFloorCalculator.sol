...
contract EliteFloorCalculator is IFloorCalculator, TokensRecoverable {
    ...
    constructor(IERC20 _rootedToken, IERC20 _eliteToken, IERC20 _baseToken, IUniswapV2Factory _uniswapV2Factory, IUniswapV2Router02 _uniswapV2Router) {
        rootedToken = _rootedToken;
        uniswapV2Factory = _uniswapV2Factory;
        uniswapV2Router = _uniswapV2Router;

        rootedElitePair = _uniswapV2Factory.getPair(address(_eliteToken), address(_rootedToken));
        rootedBasePair = _uniswapV2Factory.getPair(address(_baseToken), address(_rootedToken));
    }    

    function setIgnoreAddresses(address ignoredAddress, bool add) public onlyOwner {
        if (add) {
    ...
    function calculateSubFloor(IERC20 baseToken, IERC20 eliteToken) public override view returns (uint256) {
        ...
        uint256 amountUntilFloor = uniswapV2Router.getAmountOut(rootedCirculatingSupply, totalRootedInPairs, totalBaseAndEliteInPairs);
