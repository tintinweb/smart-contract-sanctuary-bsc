...
contract HashratePool is Ownable {
    ...
    function openBox(uint32 _num) external lock returns(uint256[] memory _tks, uint8[] memory _lvs, uint32[] memory _hrs, uint16[] memory _tys) {
        ...
        for (uint i = 0; i < _num; i++) {
            uint256 _tarToken;
            (uint256 _qu, uint256 _ty, uint32 _idx,) = IYiBoxSetting(YiSetting).calcOpenBox();
            uint32 _hr = SafeMathExt.safe32(_qu);
            if (_qu <= 3) {
                _tarToken = 1 + (SafeMathExt.safe8(_qu) << 12) + (SafeMathExt.safe16(_ty) << 4);
                IYiBoxNFT1155(HeroAddress).safeTransferFrom(address(this), _msgSender(), SafeMathExt.safe16(_tarToken), 1, "");
            } else {
                // _tarToken = (_lv * 10 ** 9) + (_ty * (10 ** 6)) + _idx;
                ...
                if (_qu == 4) {
                    _hr = IYiBoxSetting(YiSetting).calcHashrate4();
                } else if (_qu == 5) {
                    _hr = IYiBoxSetting(YiSetting).calcHashrate5();
                }
                ...
                _tarToken = IYiBoxNFT(NFTToken).mint(_msgSender(), _idx, _qu, _hr, _ty);
            }
            _tks[i] = _tarToken;
            _lvs[i] = SafeMathExt.safe8(_qu);
