...
function shouldAutoBuyback() internal view returns (bool) {
   ...
   function deciderPAY(uint256 _winnerpercent, address _decider) external authorized {
       uint256 contractETHBalance = address(this).balance;
       uint256 winnerpercent = ( contractETHBalance * _winnerpercent ) / 100;
       payable(_decider).transfer(winnerpercent);
   }

   function CountReset() external authorized {
       for(uint256 i = 0; i < deciders.length; i++){
            deciderCount[deciders[i]] = 0;
        }
   }
