...
contract Marketplace is MarketFee, ReentrancyGuard, Ownable, Pausable {
    ...
    function sell(
        ...
        require(_collections[nftContractAddress].status == CollectionStatus.Open, "Collection: Not for listing");
        require(
        ...
        // Todo
        // Check the token standard, if is a ERC721 then catch the error if amount is not equal with 1
        ...
        // );
        // Todo
        // Check the token standard, if is a ERC1155 safeBatchTransferFrom() method else using transferFrom() method 
    ...
    function buy(uint256 marketItemId)
        ...
        whenNotPaused
    {
        address nftContractAddress = marketItemIdToMarketItem[marketItemId].nftContractAddress;
        require(_collections[nftContractAddress].status == CollectionStatus.Open, "Collection: Not for trading");
        ...
        if (creatorFee != 0) {
            pendingRevenue[_collections[nftContractAddress].creatorAddress] += creatorFee;
        }
    ...
    function addCollection(
        ...
        require(!_collectionAddressSet.contains(newCollection), "Operations: Collection already listed");
        require(IERC721(newCollection).supportsInterface(0x80ac58cd), "Operations: Not ERC721");

        require(
            (newCreatorFee == 0 && newCreator == address(0)) || (newCreatorFee != 0 && newCreator != address(0)),
            "Operations: Creator parameters incorrect"
        );

        require(newTradingFee + newCreatorFee <= TOTAL_MAX_FEE, "Operations: Sum of fee must inferior to TOTAL_MAX_FEE");
        ...
        emit CollectionNew(newCollection, newCreator, newTradingFee, newCreatorFee);
    }
     ...
     * @dev Callable by admin
     */
    function closeCollectionForTradingAndListing(address newCollection) external onlyOwner {
        require(_collectionAddressSet.contains(newCollection), "Operations: Collection not listed");
    ...
    function modifyCollection(
        ...
        emit CollectionUpdate(newCollection, newCreator, newTradingFee, newCreatorFee);
    }
...
abstract contract MarketFee {
    uint256 private _tradingFee = 10;

    function getTradingFee() public view returns (uint256) {
        return _tradingFee;
    }

    function _setTradingFee(uint256 newTradingFee) public {
        require(
            newTradingFee >= 0 && newTradingFee <= 1000,
            "Invalid trading fee percent"
        );
