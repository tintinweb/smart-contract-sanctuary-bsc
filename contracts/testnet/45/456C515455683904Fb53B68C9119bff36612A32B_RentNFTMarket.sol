...
contract RentNFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    uint public rentingTime = 30 days;
    ...
    mapping(address => mapping(uint256 => Renting)) public _rentingDetails; // Renting details (renter + lessor address) for a given collection and a tokenId
    mapping(address => EnumerableSet.UintSet) private _askTokenIds; // Set of tokenIds for a collection
    mapping(address => EnumerableSet.UintSet) private _rentTokenIds; // Set of rented tokenIds for a collection
    ...
    struct Renting {
        ...
        address lessor; // address of the lessor
        uint256 startTime;
        uint256 endTime;
        uint256 requestRevoke;
        address asset;
        uint assetAmount;
    }
     ...
     */
    function rentTokenUsingBNB(address _collection, uint256 _tokenId) external payable nonReentrant {
        // Wrap BNB
        if(msg.value > 0) IWETH(WBNB).deposit{value: msg.value}();

        _rentToken(_collection, _tokenId, msg.value, true);
    }
    function rentTokenUsingBNB2Seller(address _collection, uint256 _tokenId) external payable nonReentrant {
        _rentTokenTakeFeeBaseOnBuyAsset(_collection, _tokenId, msg.value, true, address(0), msg.value);
    }
    ...
    function rentTokenUsingMEXI(
        ...
        _rentToken(_collection, _tokenId, _price, false);
    }

    function rentTokenUsingAsset2Seller(
        address _collection,
        uint256 _tokenId,
        uint256 _price
    ) external nonReentrant {
        ...
        _rentTokenTakeFeeBaseOnBuyAsset(_collection, _tokenId, _price, false, path[0], amounts[0]);
    }
    /**
    ...
    function rentTokenUsingWBNB2Seller(
        address _collection,
        uint256 _tokenId,
        uint256 _price
    ) external nonReentrant {
        IERC20(WBNB).safeTransferFrom(address(msg.sender), address(this), _price);
        _rentTokenTakeFeeBaseOnBuyAsset(_collection, _tokenId, _price, false, WBNB, _price);
    }
     ...
     */
    function createAskOrder(address _collection, uint256 _tokenId, uint256 _askPrice, address _assetAccept) external nonReentrant {
     ...
     */
    function modifyAskOrder(address _collection, uint256 _tokenId, uint256 _newPrice, address _assetAccept) external nonReentrant {
    ...
    function _rentTokenTakeFeeBaseOnBuyAsset(
        address _collection,
        uint256 _tokenId,
        uint256 _price,
        bool _withBNB,
        ...
        require(_askTokenIds[_collection].contains(_tokenId), "Rent: Not for sale");

        Ask memory askOrder = _askDetails[_collection][_tokenId];

        // Front-running protection
        require(_price == askOrder.price, "Rent: Incorrect price");
        require(msg.sender != askOrder.seller, "Rent: Buyer cannot be seller");
...
//            payable(askOrder.seller).transfer(netPrice);
            _rentingDetails[_collection][_tokenId] = Renting({renter: msg.sender, lessor: askOrder.seller, startTime: block.timestamp, endTime: block.timestamp + rentingTime, requestRevoke: 0, asset: address(0), assetAmount: netPrice});
        }
        else {
//            IERC20(asset2Buy).safeTransfer(askOrder.seller, netPrice);
            _rentingDetails[_collection][_tokenId] = Renting({renter: msg.sender, lessor: askOrder.seller, startTime: block.timestamp, endTime: block.timestamp + rentingTime, requestRevoke: 0, asset: asset2Buy, assetAmount: netPrice});
        }
        ...
        _rentTokenIds[_collection].add(_tokenId);
        totalOrderCollection[_collection]--;
...
//        IERC20(WBNB).safeTransfer(askOrder.seller, netPrice);
        _rentingDetails[_collection][_tokenId] = Renting({renter: msg.sender, lessor: askOrder.seller, startTime: block.timestamp, endTime: block.timestamp + rentingTime, requestRevoke: 0, asset: WBNB, assetAmount: netPrice});
    ...
    function revokeNFT(
        ...
        require(_rentTokenIds[_collection].contains(_tokenId), "Rent: Not for renting");

        Renting memory renting = _rentingDetails[_collection][_tokenId];
        require((renting.requestRevoke > 0 && block.timestamp - renting.requestRevoke >= 86400) || block.timestamp > renting.endTime, "Rent: revoke time invalid");

        // Front-running protection
        require(msg.sender == renting.lessor, "Rent: caller cannot be lessor");

        // Update storage information
        delete _rentingDetails[_collection][_tokenId];
        _rentTokenIds[_collection].remove(_tokenId);
        IERC721(_collection).safeTransferFrom(address(this), address(msg.sender), _tokenId);
        emit RevokeNFT(_collection, _tokenId, msg.sender, renting.renter);
    }

    function requestRevokeNFT(
        address _collection,
        uint256 _tokenId
    ) external {
        require(_rentTokenIds[_collection].contains(_tokenId), "Rent: Not for renting");
        require(_rentingDetails[_collection][_tokenId].requestRevoke == 0, "Rent: requested");

        Renting memory renting = _rentingDetails[_collection][_tokenId];

        // Front-running protection
        require(msg.sender == renting.lessor, "Rent: caller cannot be lessor");
        _rentingDetails[_collection][_tokenId].requestRevoke = block.timestamp;
        emit RequestRevokeNFT(_collection, _tokenId, msg.sender, renting.renter);
    }
