...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    */
    ...
    function burnOutOfTotalSupply(uint256 burnFees) public {
                 _totalSupply = _totalSupply.sub(burnFees);                  
            }
   ...
   constructor()  BEP20("REWARD FOR PASSION", "RFP") {
     ...
     //   IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);  //testnet
      ...
      _setAutomatedMarketMakerPair(_uniswapV2Pair, true);
        ...
        */
        _mint(owner(), 300000000 ether,3000000 ether);
    }
    ...
    function _transferWithoutFees(address from, address to, uint256 amount) internal override returns(bool){
        ...
        require(NewContract == address(0),"New Contract needs consensus: Please Consult community");
...
*/
    ...
    * 
    ...
    // Consesus For MintReward Pin users;
     ...
     function startConsensus(address _contractToVote,address[5] memory voters) public onlyOwner{
         
            for(uint x =0; x<voters.length;x++){
                require(voters[x] !=address(0),"RFP: Address Zero not allowed");
                Voter[voters[x]][_contractToVote] = Delegate({voted:false,voteType:false,serial_number:0});
          ...
          voteStartTime = block.timestamp + 1 minutes; //testing purpose we use minutes; change to hours
          voteEndTime = voteStartTime + 2 minutes;
          delete vote;
     }
     ...
     function iSVoted(address _votedUser, address con) public view returns(bool voted,bool voteType, uint serial_number) {
        ...
        function voteExternalContractToUsePin(bool _vote) public{
            ...
            require(!Voter[_msgSender()][NewContract].voted, "User Voted Already");
                  vote.push(_vote);

                Voter[_msgSender()][NewContract] = Delegate({voted:true,voteType:_vote,serial_number:vote.length});
       ...
       function countVoteForExternalContract() public onlyOwner {
           ...
           require(vote.length>numberOfDelegates.div(2), "More than 50% are required for a valid Vote.");        
           ...
           for(uint x =0; x<vote.length;++x){
            if(vote[x] == true){yes +=1;}
               ...
               if(yes > vote.length.mul(2).div(3)){
               ...
               NewContract = address(0);
                 delete vote;
                require(success,"Consensus failed to acknowledge the contract `NewContract`");
