...
interface IAmatsuGame {
  ...
  /* ========== EVENTS ========== */
  event Deposit(address indexed user, uint256[] amount, uint256 indexed kuni);
  event Claim(address indexed user);
  event Withdraw(address indexed user);
  event Fighting(address indexed user, uint256[] amount, uint256[][] items, uint256 stage, uint256 power, bool win);
...
interface IEconomyGame {
  ...
  function advantagePoint(uint256[] memory  _tokenIds, address kuniItem, uint256[][] memory items, uint256 stage, uint256 difficulty, address player, uint256 bonusPercent) external view returns(bool win);
...
contract AmatsuGame is AccessControl, IERC721Receiver, Pausable, ReentrancyGuard, IAmatsuGame {
  ...
  mapping(address => mapping(uint256 => MaterialInfo)) public materialInfo;
  // kuni address => user address => info
  mapping(address => mapping(address => MaterialInfo)) public userInfo;
  ...
  function deposit(uint256 amount, uint256[] memory tokenIds) external override {
    ...
    emit Deposit(msg.sender, tokenIds, amount);
  }
  ...
  function fighting(uint256[] memory tokenIds, uint256[][] memory items) external override whenNotPaused nonReentrant  {
    ...
    uint256 oldStage = stage; 
    bool win; uint256 power;
    
    // uint256 apKuni; uint256 apNioh; bool win;
    (win, power) = _fighting(msg.sender, tokenIds, items, stage);
    if (win) {
      stages[msg.sender] = stage.add(1);
    }

    emit Fighting(msg.sender, tokenIds, items, oldStage, power, win);
  }

  function craft(address[] memory materials, uint256[] memory qty, uint256 attack) external override whenNotPaused nonReentrant {
  ...
  function askScholar(uint256 tokenId, uint256 nioh, uint256 material) external override whenNotPaused nonReentrant {
  ...
  function claimScholar(uint256 tokenId) external override whenNotPaused nonReentrant  {
    ...
    require(renting[tokenId] == msg.sender || info.owner == msg.sender, 'Amatsu [Cancel Scholar]: Unable to process request');
     _updatePool(kuniToken);
     updatePools();
    _claimScholar(msg.sender, tokenId, false);
    emit ClaimScholar(msg.sender, tokenId);
  }
  ...
  function _claimScholar(address sender, uint256 tokenId, bool isWithdraw) internal {
    ...
    uint256 mPercent = 0; address owner = address(0x0);
    if (renting[tokenId] != address(0x0)) {
      ScholarInfo storage info = rentingOut[tokenId];
      mPercent = info.material;
      owner    = info.owner;
    }
    ore   = _harvestWithTokenId(sender, oreToken, tokenId, isWithdraw, mPercent, owner);
    stone = _harvestWithTokenId(sender, stoneToken, tokenId, isWithdraw, mPercent, owner);
    wood  = _harvestWithTokenId(sender, woodToken, tokenId, isWithdraw, mPercent, owner);
    fiber = _harvestWithTokenId(sender, fiberToken, tokenId, isWithdraw, mPercent, owner);
    hide  = _harvestWithTokenId(sender, hideToken, tokenId, isWithdraw, mPercent, owner);
    
    IVault(rewardVault).send(oreToken, sender, ore);
    IVault(rewardVault).send(stoneToken, sender, stone);
    IVault(rewardVault).send(woodToken, sender, wood);
    IVault(rewardVault).send(fiberToken, sender, fiber);
    IVault(rewardVault).send(hideToken, sender, hide);
  }
  ...
  function _SaruValid(address sender, uint256[] memory tokenIds) internal {
  ...
  function _fighting(address sender, uint256[] memory sarus, uint256[][] memory items, uint256 stage) internal returns(bool win, uint256 power) {
    // check owner
    _SaruValid(sender, sarus);
    _ItemsValid(sender, items);
    _updateDifficulty();
    uint256 _bat = battleBonus[sender];
    power = eco.niohPower(stage, difficulty);
    win = eco.advantagePoint(sarus, kuniItem, items, stage, power, sender, _bat);
  ...
  function harvestWithType(uint _type, bool isWithdraw) internal {
    uint256 ore; uint256 stone; uint256 wood; uint256 fiber; uint256 hide;
    for (uint256 inx = saruBalanceOf[_type][msg.sender]; inx > 0; inx--) {
      uint256 tokenId = ownerTokens[_type][msg.sender][inx - 1];
      uint256 mPercent = 0; address owner = address(0x0);
      if (renting[tokenId] != address(0x0)) {
        ScholarInfo storage info = rentingOut[tokenId];
        mPercent = info.material;
      ...
      ore = ore.add(_harvestWithTokenId(msg.sender, oreToken, tokenId, isWithdraw, mPercent, owner));
      stone = stone.add(_harvestWithTokenId(msg.sender, stoneToken, tokenId, isWithdraw, mPercent, owner));
      wood = wood.add(_harvestWithTokenId(msg.sender, woodToken, tokenId, isWithdraw, mPercent, owner));
      fiber = fiber.add(_harvestWithTokenId(msg.sender, fiberToken, tokenId, isWithdraw, mPercent, owner));
      hide = hide.add(_harvestWithTokenId(msg.sender, hideToken, tokenId, isWithdraw, mPercent, owner));


      if (isWithdraw && _type == GAME) {
        _transfer(address(this), msg.sender, tokenId, GAME, true);
      }
    }
    IVault(rewardVault).send(oreToken, msg.sender, ore);
    IVault(rewardVault).send(stoneToken, msg.sender, stone);
    IVault(rewardVault).send(woodToken, msg.sender, wood);
    IVault(rewardVault).send(fiberToken, msg.sender, fiber);
    IVault(rewardVault).send(hideToken, msg.sender, hide);
  }

  function harvest(bool isWithdraw) internal {
    harvestWithType(GAME, isWithdraw);
    harvestWithType(RENT, isWithdraw);
    harvestWithType(RENT_OUT, isWithdraw);
    ...
    if (saruInfo.amount > 0) {
      ...
      if(pending > 0) {
        ...
        if (owner != address(0x0) && mPercent > 0) { // owner
          ...
          if (sender == owner) {
            amount = shareAmount;
            IVault(rewardVault).send(token, renting[tokenId], pending.sub(shareAmount));
          } else {
            amount = pending.sub(shareAmount);
