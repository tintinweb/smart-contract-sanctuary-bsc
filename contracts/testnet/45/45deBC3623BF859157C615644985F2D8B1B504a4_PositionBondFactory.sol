...
contract PositionBondFactory is BondFee, ReentrancyGuard {
    ...
    function issueBond(
        ...
        require(
            bondSymbolExist[bondInformation.bondSymbol] == false,
            "bondSymbolExist"
        );
        bondSymbolExist[bondInformation.bondSymbol] = true;
        ...
        TestPositionBondLending positionBond = create(
            bondInformation,
            assetInformation,
            issuer
        );
    ...
    function create(
        ...
        PositionBondLending.AssetInformation memory assetInformation,
        address issuer
    ) internal returns (TestPositionBondLending positionBond) {
        positionBond = new TestPositionBondLending(
            bondInformation,
            assetInformation,
            issuer
        );
...
abstract contract BondFee {
    ...
    {
        if (_bondFee != 0) {
            return (collateralAmount * _bondFee) / 10000;
        }
        return 0;
    }
    ...
    ReentrancyGuard
{
        ...
        )
    {
        ...
        emit BondCreated2(bondInformation, assetInformation, address(this));
    }
    ...
    function liquidate() external onlyPositionAdmin {
        require(isLiquidate(), "Not liquidate");
    ...
    {
        if (
            BondStatus.getUnderlyingAssetStatus() ==
