...
contract StandardSale is ReentrancyGuard {
    ...
    /// @param _saleToken Address of token to be used for the sale
    function createSale(
        uint256 itemId,
    ...
    /// @param amountOfTokens Amount of ERC20 tokens to be used for the purchase - Must be approved first
    function purchaseItemERC721(uint256 itemId, uint256 amountOfTokens) external notBlacklisted nonReentrant {
        ...
        require(amountOfTokens >= _idToSale[itemId].pricePerToken, "StandardSale: amount must be higher");
    ...
    function purchaseItemERC1155(
        ...
        require(amountOfTokens >= _idToSale[itemId].pricePerToken * amountToPurchase, "StandardSale: amount must be higher");
        if(_idToSale[itemId].reservedFor != address(0)) {
