...
contract Municipality is IMunicipality, OwnableUpgradeable, EIP712Upgradeable {
    ...
    uint public minerRepairPrice;
    INetGymStreet public netGymStreet;
    IMining public miningContract;
     
     /**
    1. Initialize in constructor with the default numbers
    2. Have OwnerOnly method to set the price for the given type
    3. Increase the price by the given prc after every purchase
    4. Read function for returning the price for the given number of types
        10: Standard,
        8: NEAR_TO_OCEAN
        ...
    5. Payable function that gets the signature just like in EIP712
     */
    mapping(uint8 => uint256) parcelTypeToPriceMapping;
    mapping(uint8 => uint256) parcelTypeToPriceIndexMapping;
    ...
    function initialize(address _buyAndBurn, address _backendAddress)
        external
        initializer
    {
    ...
    function mintParcel(
        ...
        bool _withoutSignature
    ) external {
        if(!_withoutSignature){
            require(
                !signatureUsed[mintParcelSignature.signature],
                "Municipality: Signature already used"
            );
            address messageSigner = _verifyMintParcel(mintParcelSignature);
            require(
                messageSigner == backendAddress,
                "Municipality: Invalid signer"           
            );
            require(
                msg.sender == mintParcelSignature._caller,
                "Muncipality: Invalid caller"
            );
            signatureUsed[mintParcelSignature.signature] = true;
        }
        require(
            busdAddress.allowance(msg.sender, address(this)) >= parcelMintingPricesInBUSD[uint256(mintParcelSignature._parcelType)],
            "Municipality: Municipality does not havea allowance"
        );

        busdAddress.safeTransferFrom(msg.sender, address(this), parcelMintingPricesInBUSD[uint256(mintParcelSignature._parcelType)]);
        IParcel.ParcelCoordinates memory parcelCoordinates = IParcel.ParcelCoordinates(
            mintParcelSignature._x,
            mintParcelSignature._y
        );
        brokerPublicBuilding.mintParcel(
            parcelCoordinates, 
            mintParcelSignature._parcelType, 
            msg.sender
        );
        updateParcelPrice(1);
    }
    ...
    function upgradeStandardParcel(
        IParcel.ParcelCoordinates memory _parcelCoordinates
    ) external {
        require(
            busdAddress.allowance(msg.sender, address(this)) >= upgradePriceInBUSD,
            "Municipality: Municipality havn't allowance"
        );

        busdAddress.safeTransferFrom(msg.sender, address(this), upgradePriceInBUSD);
        brokerPublicBuilding.upgradeStandardParcel(_parcelCoordinates, msg.sender);
    }

    function mintMultipleParcelsFor(
        MintMultipleStandardParcelsSignature calldata _signature,
        bool _withoutSignature
    ) external {
        if(!_withoutSignature){
            require(
                !signatureUsed[_signature.signature],
                "Municipality: Signature already used"
            );
            address messageSigner = _verifyMintMultipleParcelsParcel(_signature);
            require(
                messageSigner == backendAddress,
                "Municipality: Invalid signer"           
            );
            require(
                msg.sender == _signature._caller,
                "Muncipality: Invalid caller"
            );
            signatureUsed[_signature.signature] = true;
        }
        uint mintingPrice = _calculateParcelsPrice(
            _signature._parcelType
        );
        require(
            busdAddress.allowance(msg.sender, address(this)) >= mintingPrice,
            "Municipality: Municipality havn't allowance"
        );
        require(
            _signature._x.length == _signature._y.length,
            "Municipality: Coordinates lengths are not equal"
        );
        busdAddress.safeTransferFrom(msg.sender, address(this), mintingPrice);
        IParcel.ParcelCoordinates[] memory parcelsCoordinates;
        for(
            uint index;
            index < _signature._x.length;
            index++
        )
        {
            parcelsCoordinates[index] = IParcel.ParcelCoordinates(
                   _signature._x[index],
                   _signature._y[index]
                );
        }
        
        brokerPublicBuilding.mintMultipleParcelsFor(        
            parcelsCoordinates, 
            _signature._parcelType, 
            msg.sender
        );
        updateParcelPrice(_signature._x.length);
    }

    function mintMiner(MinerTypes _minerType) external {
        uint256 minerPrice = minerPrices[uint256(_minerType)];
        require(
            busdAddress.allowance(msg.sender, address(this)) >= minerPrice,
            "Municipality: Municipality does not have allowance"
        );

        busdAddress.safeTransferFrom(msg.sender, address(this), minerPrice);
        uint256 tokenId = minerPublicBuilding.mintMiner(msg.sender);
        minerSales(minerPrices[0], msg.sender);

        emit MintMiner(tokenId);
    }

    function setNetGymStreetAddress(address _contract) external onlyOwner {
        netGymStreet = INetGymStreet(_contract);
    }
    ...
    function setMinerPublicBuilding(address _minerPublicBuilding)
        external
        onlyOwner
    {
    ...
    function setStandardParcelContract(address _standardParcelContract)
        external
        onlyOwner
    {
    ...
    function buyBundle(
        ...
        uint bundlePrice = _calculateBundlePrice(_bundleType, IBrokerPublicBuilding.ParcelTypes.Inland);
        require(
            busdAddress.allowance(msg.sender, address(this)) >= bundlePrice,
            "Municipality: Municipality does not havea allowance"
        );
        busdAddress.safeTransferFrom(msg.sender, address(this), bundlePrice);
        
        IBrokerPublicBuilding.ParcelTypes parcelType = IBrokerPublicBuilding.ParcelTypes.Inland;
        uint256 minersAmount = uint256(standardParcelContract.standardParcelSlotsLimit());
        Bundle memory bundle = bundles[uint8(_bundleType)];

        uint currentX = _parcelCoordinates._x;
        uint currentY = _parcelCoordinates._y;
        for (uint i; i < bundle.parcelSize**2; i++) {
            // Mint Parcel
            IParcel.ParcelCoordinates memory newParcelCoordinates;
            newParcelCoordinates._x = currentX;
            newParcelCoordinates._y = currentY;
            brokerPublicBuilding.mintParcel(
                newParcelCoordinates, 
                parcelType, 
                msg.sender
            );

            // Mint Miner
            uint256 minerTokenId = minerPublicBuilding.mintMiners(
                msg.sender,
                minersAmount
            );
            minerSales(minerPrices[0], msg.sender);
            ...
            if (i % bundle.parcelSize == 1 && i != 0) {
                currentY++;
                currentX = _parcelCoordinates._x;
            }
        }
    ...
    function setScarcityFactor(uint256 _value) external onlyOwner {
        scarcityFactorMultiplier = _value;
    }

    function setBrokerPublicBuilding(address _brokerPublicBuilding)
        external
        onlyOwner
    {
    ...
    function setMiningAddress(IMining _contract )external onlyOwner{
        miningContract = _contract;
    }

    function setParcelMintingPrice(uint256 _price, IBrokerPublicBuilding.ParcelTypes _parcelType)
        external
        onlyOwner
    {
    ...
    function setMinerRepairPrice(uint _price)external onlyOwner{
        minerRepairPrice = _price;    
    }
    function calculateParcelsPrice(
        IBrokerPublicBuilding.ParcelTypes[] memory _parcelTypes
    ) external view returns (uint256 finalPrice) {
        return _calculateParcelsPrice(_parcelTypes);
    }
    ...
    function _calculateParcelsPrice(
        ...
        for (
            uint256 i;
            i < _parcelTypes.length;
            i++
        ) {
            finalPrice += parcelMintingPricesInBUSD[uint8(_parcelTypes[i])] 
                * (1 + (scarcityFactorMultiplier / 1000000))
                ** i;
        }
    }
    ...
    function _calculateBundlePrice(
        ...
        finalPrice -=
            (finalPrice * bundles[uint256(_bundleType)].discount) /
            100;
    }

    function _verifyMintParcel(MintParcelSignature calldata mintParcelSignature)
        private
        view
        returns (address)
    {
        bytes32 _digest = _hashMintParcel(mintParcelSignature);
        return ECDSAUpgradeable.recover(_digest, mintParcelSignature.signature);
    }

    function _verifyMintMultipleParcelsParcel(
        MintMultipleStandardParcelsSignature calldata mintMultipleParcelsSignature
    )
        private
        view
        returns (address)
    {
        bytes32 _digest = _hashMintMultipleParcels(mintMultipleParcelsSignature);
        return ECDSAUpgradeable.recover(_digest, mintMultipleParcelsSignature.signature);
    }

    function _hashMintParcel(MintParcelSignature calldata mintParcelSignature)
        private
        view
        returns (bytes32)
    {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "MintParcelSignature(uint256 _x,uint256 _y,ParcelTypes _parcelType,address _caller)"
                        ),
                        mintParcelSignature._x,
                        mintParcelSignature._y,
                        mintParcelSignature._parcelType,
                        mintParcelSignature._caller
                    )
                )
            );
    }

    function _hashMintMultipleParcels(
        MintMultipleStandardParcelsSignature calldata mintMultipleParcelsSignature
    )
        private
        view
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "MintMultipleStandardParcelsSignature(uint256[] _x,uint256[] _x,ParcelTypes[] _parcelType,address _caller)"
                        ),
                        mintMultipleParcelsSignature._x,
                        mintMultipleParcelsSignature._y,
                        mintMultipleParcelsSignature._parcelType,
                        mintMultipleParcelsSignature._caller
                    )
                )
            );
    }
    function minerRepair(uint _value, uint _pid, address _miner)external{
        require(_value >= minerRepairPrice,"Municipality: Invalid amount" );
        busdAddress.safeTransferFrom(msg.sender, address(this), _value);
        miningContract.repairMiner(_pid, _miner);
        _minerRepair(_value, msg.sender);
    }

    function minerSales(uint256 msgValue, address _user) internal {
        busdAddress.safeTransfer(ambassadorAddress, (5 * msgValue) / 100);
        busdAddress.safeTransfer(reserveWalletAddress, (7 * msgValue) / 1000);
        busdAddress.safeTransfer(address(netGymStreet), (39 * msgValue) / 100);
        //netGymStreet.distributeRewards((39 * msgValue) / 100, address(busdAddress), _user);
    ...
    function brokerSalesInitialMintingLandUpgrade(uint256 msgValue,address _user) internal {
        busdAddress.safeTransfer(ambassadorAddress, (5 * msgValue) / 100);
        busdAddress.safeTransfer(reserveWalletAddress, (7 * msgValue) / 1000);
    ...
    function _minerRepair(uint256 msgValue,address _user) internal {
        busdAddress.safeTransfer(ambassadorAddress, (5 * msgValue) / 100);
        busdAddress.safeTransfer(reserveWalletAddress, (7 * msgValue) / 1000);
    ...
    function updateParcelPrice(uint256 _count) internal {
        parcelMintingPricesInBUSD[uint8(IBrokerPublicBuilding.ParcelTypes.Inland)] =
            (parcelMintingPricesInBUSD[uint8(IBrokerPublicBuilding.ParcelTypes.Inland)] / 1000000) *
            (scarcityFactorMultiplier**_count);
...
interface IMunicipality {
    ...
    struct MintMultipleStandardParcelsSignature {
        uint[] _x;
        uint[] _y;
        IBrokerPublicBuilding.ParcelTypes[] _parcelType; 
        address _caller;
    ...
    function mintParcel(
        MintParcelSignature memory mintParcelSignature,
        bool _withoutSignature
    ) external;
    ...
    function mintMultipleParcelsFor(
        MintMultipleStandardParcelsSignature memory _signature,
        bool _withoutSignature
    ) external;
...
import "./IGymMLMQualifications.sol";
import "./IGymMLM.sol";
...
interface INetGymStreet {
    ...
    // function setStandartParcelAddress(address _contract) external;

    // function setBusinessParcelAddress(address _contract) external;

    // function setbrokerAddress(address _contract) external;

    // function setMinerAddress(address _contract) external;

    // function setMLMQualificationsAddress(address _address) external;

    // function setWhiteListAddress(address _address) external;

    // function updateMLMCommission(uint256 _level, uint256 _commission) external;
...
interface IMining is IERC721Base {
    function deposit(
        uint256 _pid, 
        address _miner, 
        uint256 _minerId, 
...
interface IBrokerPublicBuilding {
    ...
    function mintMultipleParcelsFor(
        IParcel.ParcelCoordinates[] memory _parcelsCoordinates, 
        ParcelTypes[] memory _parcelTypes, 
        address _beneficiary
    ) external;
    ...
    function assignNFTToStandardParcel(
        IParcel.ParcelCoordinates memory _parcelCoordinates, 
        ActivateableNFTTypes _activatableNFTType,
        uint256 _nftTokenId,
        address _caller
    ) external;
...
interface IStandardParcel is IParcel {
    ...
    struct StandardParcelInfo {
        bool _upgradeStatus;
        IBrokerPublicBuilding.ParcelTypes _parcelType; 
        uint256[] _minerNFTTokenIds;
//        uint256[] _aestheticNFTTokenIds;
    }

    /// @notice Event that is emitted when parcel is upgraded
    event ParcelUpgraded(uint256 tokenId);

    /// @notice Event that is emitted when some token is assigned on parcel
    event Assigned(uint256 parcelId, uint256 tokenId, IBrokerPublicBuilding.ActivateableNFTTypes activatableNFT);

    /// @notice Event that is emitted when some token is unassigned on parcel
    event Unassigned(uint256 parcelId, uint256 tokenId, IBrokerPublicBuilding.ActivateableNFTTypes activatableNFT);
    ...
    function upgradeParcel(uint _parcelId, address _caller) external;

    function assignNFT(
        uint256 _parcelId,
        IBrokerPublicBuilding.ActivateableNFTTypes _activatableNFTType,
        uint256 _nftTokenId,
        address _caller
    ) external;
    ...
    function isUpgraded(ParcelCoordinates memory _parcelCoordinates) external view returns (bool);

    function getParcelNFTs(
        IBrokerPublicBuilding.ActivateableNFTTypes _activatableNFTType,
        ParcelCoordinates memory _parcelCoordinates
    ) external view returns (uint256[] memory);

    function getNFTParcel(IBrokerPublicBuilding.ActivateableNFTTypes _activatableNFTType, uint256 _tokenId)
        external
        view
        returns (uint256);

    function isParcelUpgraded(ParcelCoordinates memory _parcelCoordinates)
        external
        view
        returns (bool);

    function standardParcelSlotsLimit() external view returns(uint8);
...
interface IParcel is IERC721Base {
    ...
    event ParcelMinted(uint256 tokenId, address owner);
    event TransferParcelFrom(address from, address to, uint256 tokenId);

    function setBrokerPBContractAddress(
    ...
    function transferMultipleParcels(
        ParcelCoordinates[] memory _parcelsCoordinates,
        address _to 
    ) external;
    ...
    function getCordinatesFromId(uint256 _tokenId)
        external view returns (
            ParcelCoordinates memory parcelCoordinates
    );

    function parcelExists(
        ParcelCoordinates memory _parcelCoordinates
    ) external view returns (bool);

    function ownerOfParcel(
        ParcelCoordinates memory _parcelCoordinates
    ) external view returns (address); 

    function ownersOfMultipleParcels(
        ParcelCoordinates[] memory _parcelsCoordinates
    ) external view returns (address[] memory);

    function allParcelsOfUser(
        address _owner
    ) external view returns (ParcelCoordinates[] memory parcelsCoordinates);
...
interface IBusinessParcel is IParcel {
    ...
    struct BusinessParcelInfo {
        IBrokerPublicBuilding.ActivateableNFTTypes activatedBranchType;
...
interface IGymMLMQualifications {
    ...
    function getDirectPartners(address)
        external
        view
        returns (address[] memory);

    function getRockstarAmount(uint32 _rank)
        external
        view
        returns (RockstarLevel memory);
