...
contract TokenLockingStaking is TokenWrapper, OwnableUpgradeable {
    ...
    function addLockStackList(uint256 amount, LockType _lockType) internal {
        ...
        LockStaking memory lock = LockStaking(personalOrderId[_msgSender()], block.timestamp, block.timestamp + locks[_lockType].period, 
            _lockType, amount, RedeemStatus.NON_REDEEM);
    ...
    function getLockStackList(address target) public view returns (LockStaking[] memory) {
        ...
        for(uint i=0; i< _lockList.length; i++){
            ...
            uint256 cycleTime;
            uint256 cycle;
            (inTime,curTime,cycleTime,cycle, ) = getCycle(_lockList[i].lockTime, _lockList[i].lockType);
            if (inTime) {
                _lockList[i].status = RedeemStatus.CAN_REDEEM;
            } else {
                _lockList[i].status = RedeemStatus.NON_REDEEM;
            }
            _lockList[i].unlockTime = _lockList[i].lockTime + cycleTime * cycle + locks[_lockList[i].lockType].period;
        }
        return _lockList;
    }
