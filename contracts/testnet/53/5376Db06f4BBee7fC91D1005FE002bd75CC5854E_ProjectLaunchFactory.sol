...
import {Sale} from "./contracts/Sale_V8.sol";
...
contract ProjectLaunchFactory {
  ...
  address public couponContract = 0x4cf0070804cd52Fe1Fb0bFCDDd661022598947C8;
  address _address = msg.sender;
  address _contract = address(this);

  address public feeTo;
  address _owner;
  mapping(uint256 => uint256) public launchFee;
  mapping(uint256 => uint256) public round1Mult;
  mapping(uint256 => uint256) public round2Mult;
  mapping(uint256 => uint256) public round3Mult;
  mapping(uint => uint256) public round1MultT;
  mapping(uint => uint256) public round2MultT;
  mapping(uint => uint256) public round3MultT;
  ...
  constructor() {
    ...
    launchFee[1] = 5000000 gwei;
    launchFee[2] = 7500000 gwei;
    launchFee[3] = 7500000 gwei;
    launchFee[4] = 10000000 gwei;
    launchFee[5] = 10000000 gwei;
  ...
  function createSale(
  ...
  ) external payable enoughFee(_sale_details[0]) returns (address) {
    ...
    if (iCoupon(couponContract).getCouponActive(_address,_sale_details[7]) == 1) {
      ...
      _seed_details[10] = round1MultT[_sale_details[0]] + ((10000 - round1Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
      ...
      _presale_details[10] = round2MultT[_sale_details[0]] + ((10000 - round2Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
      ...
      _community_details[10] = round3MultT[_sale_details[0]] + ((10000 - round3Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
    } else {
    ...
    totalTokens = (_seed_details[4] * _seed_details[0]) + (_presale_details[4] * _presale_details[0]) + (_community_details[4] * _community_details[0]);

    uint256 totalLiquidityTokens = 0;

    if(_sale_details[7] > 0) {
      totalLiquidityTokens = liquidityCalc(_seed_details[4], round1Mult[_sale_details[0]], _seed_details[0], _seed_details[5]);
      totalLiquidityTokens = totalLiquidityTokens + liquidityCalc(_presale_details[4],round2Mult[_sale_details[0]], _presale_details[0], _presale_details[5]);
      totalLiquidityTokens = totalLiquidityTokens + liquidityCalc(_community_details[4],round3Mult[_sale_details[0]], _community_details[0], _community_details[5]);
    }
  ...
  function liquidityCalc(uint256 _hardcap, uint256 _mult, uint256 _rate, uint256 _percentage) internal pure returns(uint256) {
    return (_hardcap * (10000 - _mult) / 10000 * _rate  * _percentage / 100);
  }
...
contract Sale is ReentrancyGuard {
    ...
    function userDeposit (uint256 _round) public payable nonReentrant {
        require(iBlacklist(blacklistContract).getBlacklist(_address, _contract) == 0, 'You are blacklisted!');
        require(saleStatusCheck(_round) == 1, "Not Active");
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                ...
                if(sale_info[_round].multT < 10000) {
                   ERC20(sale_details.sale_token).transferFrom(address(this), address(feeAddress), ((sale_status[_round].sale_sold_amount * sale_info[_round].multT) / 10000));
                }
            }
        }
    }
    ...
    function restartRound(uint256 _round, uint256 _start, uint256 _end) public payable onlyOwner {
        ...
        require(saleStatusCheck(_round) == 3, "Not failed.");
        require(_start > block.timestamp, "Start has to be greater than now");
        require(_end > _start, "End date has to be greater than the start");
