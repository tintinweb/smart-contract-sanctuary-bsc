...
contract StakingPoolFactory is Ownable {
    ...
    function createPool(
        ...
        require(
            ...
            "createPool::insufficient rewards token balance"
        );
    ...
    function stake(
        ...
        require(
            ...
            "stake::insufficient staking token balance"
        );
    ...
    function unstake(uint256 _poolId, uint256 _stakeId) external {
        ...
        require(
            _pool.stakes[_stakeId].createdTime + _pool.period < block.timestamp,
            "unstake::time is not up"
        );
        require(
            !(_pool.stakes[_stakeId].isUnstaked),
    ...
    function addRewards(uint256 _poolId, uint256 _amount) external {
        Pool storage _pool = pools[_poolId];
        require(msg.sender == _pool.owner, "addRewards::not owner");
        IERC20 rewardsToken = IERC20(_pool.rewardsToken);
        require(
        ...
        _pool.totalRewards = _pool.totalRewards + _amount;
    }

    function withdrawAssets(uint _poolId) external {
        Pool storage _pool = pools[_poolId];
        require(msg.sender == _pool.owner, "addRewards::not owner");
        IERC20 stakingToken = IERC20(_pool.stakingToken);
        stakingToken.transfer(msg.sender, _pool.totalStaked);
    }
    ...
    {
        Pool storage _pool = pools[_poolId];
        uint256 _rewardsAmount = _pool.stakes[_stakeId].amount * _pool.rate;
    ...
    function estimateRewards(
        ...
        Pool storage _pool = pools[_poolId];
        uint256 _rewardsAmount = _amount * _pool.rate;
        return _rewardsAmount;
    }
        ...
        returns (PoolForExport memory)
    {
        ...
        _pool.period = pools[_poolId].period;
        _pool.rate = pools[_poolId].rate;
    ...
    function getPools() external view returns (PoolForExport[] memory) {
        ...
        for (uint i = 0; i < poolId.current(); i++) {
            ...
            _pools[i].period = _pool.period;
            _pools[i].rate = _pool.rate;
        ...
        returns (PoolForExport[] memory)
    {
        ...
        for (uint i = 0; i < poolId.current(); i++) {
            ...
            if (_pool.owner == _owner) {
                ...
                _pools[_counter].period = _pool.period;
                _pools[_counter].rate = _pool.rate;
