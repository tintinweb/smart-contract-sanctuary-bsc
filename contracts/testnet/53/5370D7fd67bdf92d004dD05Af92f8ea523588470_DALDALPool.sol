...
contract DALDALPool is Ownable, ReentrancyGuard, ERC20("DAL DAL Pool", "DAL-DAL") {
    ...
    //
    function initialize(
        ERC20 _stakedToken, //0x3519fE79ea89cEDC8f5818CE7c3e645996dD4EAB
        ERC20[] memory _rewardTokens, //0x3519fE79ea89cEDC8f5818CE7c3e645996dD4EAB
        uint256[] memory _rewardPerBlock, // 0x3519fE79ea89cEDC8f5818CE7c3e645996dD4EAB ==> 2893500000000000
        uint256[] memory _startEndBlocks, //start=> 17181362 //end 18909362
        uint256[] memory _stakingBlocks, // startStaking => 17181362 // endStaking => 18909362
        uint256 _unStakingBlock, // unStakingBlock 17181362
        uint256[] memory _feeSettings, // feeStaking = 200 // feePeriod = 0
        address _feeCollector, // 0x2fA55Ba3be76eE656Cd1614CCAff824B41927146 ==> Create contract Staking
        uint256 _poolLimitPerUser, // 0
        uint256 _poolCap, // 	0
        bool _isRemovable, // 	true
        address _admin // 0x2fA55Ba3be76eE656Cd1614CCAff824B41927146
    ) external {
    ...
    // Set lockingDuration ==> Lock statak 
   ...
   // 1000000000000000000000
    ...
    function collectFee(uint256 _amount, UserInfo storage user) internal returns (uint256) {
     ...
     * @param _bonusEndBlock: the new end block
     */
    function updateBonusEndBlocks(uint256 _bonusEndBlock) external onlyOwner {
        require(stakingEndBlock <= _bonusEndBlock, "End staking block exceeds bonus end block");
    ...
    function pendingReward(address _user) external view returns (uint256[] memory, ERC20[] memory) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                adjustedTokenPerShare = accTokenPerShare[rewardTokens[i]].add(bscsReward.mul(PRECISION_FACTOR[rewardTokens[i]]).div(stakedTokenSupply));
    ...
    function pendingRewardByToken(address _user, ERC20 _token) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare[_token].add(bscsReward.mul(PRECISION_FACTOR[_token]).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            bscsReward = multiplier.mul(rewardPerBlock[rewardTokens[i]]);
            accTokenPerShare[rewardTokens[i]] = accTokenPerShare[rewardTokens[i]].add(bscsReward.mul(PRECISION_FACTOR[rewardTokens[i]]).div(stakedTokenSupply));
        }
        lastRewardBlock = block.number;
    }
     ...
     */
    function removeElement(uint256 _index, ERC20[] storage _array) internal returns (bool, ERC20[] memory) {
     ...
     */
    function findElementPosition(ERC20 _token, ERC20[] storage _array) internal view returns (bool, uint256) {
