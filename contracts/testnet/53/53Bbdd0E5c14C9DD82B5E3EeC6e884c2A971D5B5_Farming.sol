...
contract Farming is ReentrancyGuard, Ownable {
    ...
    // Just for test
    function isCowBreeding(uint _cowTokenId) public view returns (bool) {
        return breedingCows[_cowTokenId];
    }
    // function setCowBreedingStatus(uint _cowTokenId, bool _isCowBreeding) public {
    //     breedingCows[_cowTokenId] = _isCowBreeding;
    // }
    // End Just For Test
    ...
    function withdrawCow(uint _tokenId) public {
        ...
        uint prevWithdrawalReward = getUserRewardAmount();
        FarmingVault storage _userFarm = userFarms[msg.sender];
        INftMinter minter = INftMinter(address(minterAddr));
        ICowNFT cowNfts = ICowNFT(address(minter.cowNftColl()));
        ...
        uint postWithdrawalReward = getUserRewardAmount();
    ...
    function harvest() external {
        ...
        // If 1 day or more has passed from last claim
        // FIXME: El +1 days debe ser una variable que se setea cuando se hace deploy
        ...
        uint256 _reward = reserveAmount[msg.sender] + (milkPerBlock * _rewardTime  * userMilkPower / totalMilkPower);
