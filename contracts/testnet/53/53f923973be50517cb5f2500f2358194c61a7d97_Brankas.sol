...
contract Brankas is Context, Ownable {
    ...
    function _depositToken(address address_, uint256 amount_) internal returns (uint256)  {
        require(amount_ >= 0, "ERC20: insufficient balance");
        IERC20(address_).transferFrom(_msgSender(), address(this), amount_);
        _balances[address_][_msgSender()] = _balances[address_][_msgSender()].add(amount_);
        return _balances[address_][_msgSender()];
    }
    function _sendToken(address address_, uint256 amount_, address recipient_) internal returns (uint256) {
        ...
        IERC20(address_).transfer(recipient_, amount_);
        _balances[address_][account_] = _balances[address_][account_].sub(amount_);
        return _balances[address_][account_];
    }
    function _spendToken(address address_, uint256 amount_, address recipient_) internal returns (uint256) {
    ...
    function withdrawAll(address address_) public returns (uint256) {
        ...
        require(amount_ > 0, "ERC20: empty balance");
        return _sendToken(address_, amount_, account_);
    }
    ...
    function unlockDate(address address_, address account_) public view returns (string memory) {
        uint256 lockedUntil_ = _lockedUntils[address_][account_];
