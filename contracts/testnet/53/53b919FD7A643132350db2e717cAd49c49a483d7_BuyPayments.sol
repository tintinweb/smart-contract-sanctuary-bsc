...
contract BuyPayments is Ownable, ReentrancyGuard {
    ...
    function _process(
        ...
        if (_tokenAddress != bnbAddress_) {
            if (tokenToTransaction > DexBalance) {
            ...
            if(amountToBeSentAfterFees > totalBalance()){
                uint256 _amount = getTokenPrice(bnbStablePair_, amountToBeSentAfterFees);
        ...
        if (token.referActive) {
            uint256 referReward = (tokenToTransaction * token.referBonusPercent).div(100);
            if(referReward > DexBalance) {
    ...
    function _adjustment_balance() private {
        ...
        if (
            ...
            uint256 totalSwapTk = (totalBalance()).sub(minBalanceInBNB_);
            _buyTokens(totalSwapTk, stableReservAddr_, address(this));
        }
    }
