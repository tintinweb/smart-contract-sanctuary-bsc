...
/**
    ...
    **/
    function swapTOKENtoVAULT(address dtoken, address Vaultoken,uint _level,uint amount)public payable{
        (, bool isLPVault,) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(dtoken));
        if(dtoken != Admin.WBNB()){
            IERC20(dtoken).safeTransferFrom(msg.sender, address(this), amount);
        }
        if(!isLPVault){
            ...
            IOptVaultFactory(IAdmin(Admin).optVaultFactory()).Deposit(msg.sender,Vaultoken,_level,amount);
        }else{
    ...
    **/
    function swapTOKENtoRECIEPT(address dtoken, address Vaultoken,uint amount, uint minAmount)public payable{
        (address vaultaddress, bool isLPVault,) = IOptVaultFactory(IAdmin(Admin).optVaultFactory()).getPoolInfo(IOptVaultFactory(IAdmin(Admin).optVaultFactory()).PIDsOfRewardVault(dtoken));
        if(dtoken != Admin.WBNB()){
            IERC20(dtoken).safeTransferFrom(msg.sender, address(this), amount);
        }
        if(!isLPVault){
