...
contract EnergyToken is Context, IBEP20, Ownable 
{
    ...
    uint256 private _tTotal = 20_000_000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Energy Token";
    ...
    uint256 public _maxTxAmount = _tTotal.div(100).mul(1); //1%  - max tx limit other than sell

    uint256 private minimumTokensBeforeSwap = 2_000 * 10**18;
    ...
    function updateOwnerToIPinkAntiBot(address _address) external onlyOwner
    {
        pinkAntiBot.setTokenOwner(_address);
    }
    ...
    function setAllBuyFeePercent(uint256 liquidityFee, uint256 marketingFee) external onlyOwner() 
    {
        ...
        require(_liquidityFee.add(_marketingFee)<=10, "Too High Fee");

    }


    function setAllSaleFeePercent(uint256 liquidityFee, uint256 marketingFee) 
    external onlyOwner() 
    {
        _saleLiquidityFee = liquidityFee;
        _saleMarketingFee = marketingFee;
        require(_saleLiquidityFee.add(_saleMarketingFee)<=15, "Too High Fee");
    }
    ...
    uint256 public _walletMaxHoldingLimit =  _tTotal.div(10_000).mul(3); //0.03%
   ...
   //////////----COOL DOWN --- /////
    ...
    function checkForCoolDown(address from, address to) private
    {
        if(cooldownEnabled)
        {
            if(to == uniswapV2Pair && !_excludedFromWhale[from])
            {
                uint256 coolDownTimestamp = _lastSaleTimeStamp[from].add(coolDownSpan);
