...
contract BENC is ERC1155, IERC20, IBENC {
    ...
    function setURIheader(string memory uri_head) public onlyOwner{
      ...
      function getURIheader() public view onlyOwner returns(string memory){
        return _uri_header;
    }
...
function transferNFT(address from, address to , uint256 amount, string memory phonenumber,string memory email) public {   
    ...
    require(amount<=sellerBalance,"Amount want to transfer is greater than sellerBalance");
      OwnerProperty memory currentOwner =  OwnerProperty({owner:to,phoneNumber:phonenumber,emailAddress:email}); 
       ...
       for(uint256 x = 0 ; (x<sellerBalance && amt <amount);++x){
         uint256 trackNumber = tracker[from][x];

         if(!ownerNFT[trackNumber].collected){

        ownerNFT[trackNumber].owner=currentOwner;  //changeOwnership         
         tracker[to].push(trackNumber);  // create new tracker for new owner;
         tracker[from][x] = tracker[from][sellerBalance-1];
        ...
        safeTransferFrom(from, to, tokenID_2, amt, "0x0");
}
