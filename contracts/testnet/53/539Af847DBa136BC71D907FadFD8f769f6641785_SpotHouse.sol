    ...
    function _openLimitOrder(
        ...
        if(isBuy){
            ...
            quoteAmount = _pairManager.calculatingQuoteAmount(
                (_quantity - sizeOut).Uint256ToUint128(),
                _pip
            ) + quoteAmountFilled;
            // deposit quote asset
            // with token has RFI we need deposit first
            // and get real balance transferred
            (uint256 quoteAmountTransferred, uint256 fee) = _deposit(
                _pairManager,
                _trader,
                Asset.Quote,
                quoteAmount,
            ...
            require(quoteAmountTransferred + fee == quoteAmount, "!RFI");
        }else{
            quoteAmount = _pairManager.calculatingQuoteAmount(
                (_quantity - sizeOut).Uint256ToUint128(),
                _pip
