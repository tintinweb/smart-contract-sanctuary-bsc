...
contract BuyBackFeeManager is AccessControl {
        ...
        external
    {
        ...
        if (!switchA) {
            ...
            emit LogbuyBackAmount0(buyBackAmount0);
            emit LogbuyBackAmount1(buyBackAmount1);
            emit LognewWETHBalance(newWETHBalance);
            emit LogtoBuyBackCoin0Percent(toBuyBackCoin0Percent);
        }

        emit LpsUnwrapped(newWETHBalance);
    }
    bool public switchA = false;
    function switchIt() external {
        switchA = !switchA;
    }
    event LogbuyBackAmount0(uint256);
    event LogbuyBackAmount1(uint256);
    event LognewWETHBalance(uint256);
    event LogtoBuyBackCoin0Percent(uint256);
