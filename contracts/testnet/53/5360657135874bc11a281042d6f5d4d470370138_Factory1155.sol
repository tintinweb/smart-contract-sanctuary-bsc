...
// File: Pink/IERC1155Receiver.sol
...
// File: Pink/IERC1155Metadata.sol
 ...
 * _Available since v3.1._
 */
contract ERC1155 is Context, ERC165, IERC1155, IERC1155Metadata {
    using Address for address;

    string _name;  // Token name
    string _symbol;  // Token symbol
    mapping(uint256 => mapping(address => uint256)) _balances;  // Mapping from token ID to account balances
    mapping(address => mapping(address => bool)) _operatorApprovals;  // Mapping from account to operator approvals
    string _uri;  // Used as the URI for all token types by relying on ID substitution, e.g. https://token-cdn-domain/{id}.json
     ...
     * To learn more about hooks, head to xref:ROOT:extending-contracts.adoc#using-hooks[Using Hooks].
     */
    function _afterTokenTransfer(address operator, address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal {}
...
contract Factory1155 is Context, IFactory {
    function createCollection(string memory name, string memory symbol, string memory uri) external override returns (address) {
