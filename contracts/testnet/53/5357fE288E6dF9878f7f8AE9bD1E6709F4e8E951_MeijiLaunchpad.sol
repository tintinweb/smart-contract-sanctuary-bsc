...
contract MeijiLaunchpad is Ownable {
    ...
    struct Pool {
        ...
        uint[9] targetAmountEveryTier; // MEIJIDAO
        uint[9] investedAmountEveryTier; // MEIJIDAO
    }
    
    uint public fee = 0; // 1:1000000
    uint public ticketFee = 0; //1:1000000
    ...
    constructor(address meijitoken, address pool1, address pool2, address pool3, address pool4, address pool5) {

        GovernanceToken = IERC20(meijitoken);
    ...
    modifier validPoolId(uint poolId) {
        ...
        require(_poolList[poolId].removed == false, "Invalid Pool");
        _;
    }
    ...
    function bid(uint poolId, uint amount) external validPoolId(poolId) {
        ...
        // transfer staked MEIJIDAO tokens to launchpad for bid
        uint8 i;
        for (i = 0; i < stakingPoolCount; i++) {
            (uint256 remainAmount, ,) = IMEIJIDAOStakingPool(stakingPools[i]).userInfo(msg.sender);
            if (bidAmount > remainAmount) {
                IMEIJIDAOStakingPool(stakingPools[i]).invest(msg.sender, remainAmount);
                bidAmount -= remainAmount;
            } else {
                IMEIJIDAOStakingPool(stakingPools[i]).invest(msg.sender, bidAmount);
                break;
            }
        }
    ...
    function claim(uint poolId) public validPoolId(poolId) onlyPoolOwner(poolId) {
        ...
        require(GovernanceToken.balanceOf(address(this)) >= remain, "Not Enough MEIJIDAO tokens in LaunchPad");

        uint claimAmount = remain - remain * fee / 10**6;
        if (fee > 0) GovernanceToken.transfer(feeWallet, remain * fee / 10**6);
    ...
    function claimTickets(uint poolId) public validPoolId(poolId) {
        ...
        uint claimAmount = ticketRemain - ticketRemain * ticketFee / 10**6;

        IERC20(_poolList[poolId].tokenAddress).transfer(address(msg.sender), claimAmount);
        if (ticketFee > 0) IERC20(_poolList[poolId].tokenAddress).transfer(feeWallet, ticketRemain * ticketFee / 10**6);
    ...
    function getStakingBalnce(address staker) public view returns(uint256){
        ...
        for (uint i = 0; i < stakingPoolCount; i++) {
            (uint256 amount,,) = IMEIJIDAOStakingPool(stakingPools[i]).userInfo(staker);
    ...
    function setFee(uint _fee, uint _ticketFee) public onlyOwner{
        fee = _fee;
