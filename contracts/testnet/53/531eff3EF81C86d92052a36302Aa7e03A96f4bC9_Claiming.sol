...
contract Claiming is Ownable {
    ...
    constructor(IBEP20 _token, uint _freezePeriod) {
    ...
    function claimRemainingTokens() public userHasClaimableTokens {
        ...
        uint256 amount = allowanceAmounts[msg.sender].amount;

        require(
            token.balanceOf(address(this)) >= amount,
            "Insufficient amount of tokens in contract"
        );

        token.approve(address(this), amount);
        if (!token.transferFrom(address(this), msg.sender, amount)) {
            allowanceAmounts[msg.sender].amount = amount;
            emit UnsuccessfulTransfer(msg.sender);
        } else {
            allowanceAmounts[msg.sender].amount = 0;
    ...
    function allowTokens(uint256 amount) public onlyOwner{
     ...
     */
    function addMultipleUsersToList(allowedUser[] memory addAddresses) public onlyOwner
    {
        for (uint256 i = 0; i < addAddresses.length; i++) {
            addSingleUserToList(addAddresses[i].wallet, addAddresses[i].amount);
        }
    }
    ...
    function findAll() public view onlyOwner returns(userTokens[] memory) {
        userTokens[] memory ret = new userTokens[](addresses.length);
        for (uint i = 0; i < addresses.length; i++) {
            ret[i] = allowanceAmounts[addresses[i]];
        }
        return ret;
    }
