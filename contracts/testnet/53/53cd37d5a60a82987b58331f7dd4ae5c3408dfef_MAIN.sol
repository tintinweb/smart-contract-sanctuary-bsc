...
contract MAIN is ERC20, Ownable {
    ...
    address public marketingWallet = 0x96F42919CF5D0b17cdC4934617172eE369C2661f;
    address public BuybackWallet = 0xa63bAc890456BD952600b8eA3FD24D863Fb9E5c3;
    ...
    uint256 public usdtDividendRewardsFee = 5;

    uint256 public marketingFee = 3;
    uint256 public buybackFee = 3;

    uint256 public totalFees = busdDividendRewardsFee.add(marketingFee).add(usdtDividendRewardsFee).add(liquidityFee).add(buybackFee);
    ...
    constructor() ERC20("MaMoInu", "MAIN") {
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "MAIN: The marketing wallet is already this address");
    ...
    function updateBuybackWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != BuybackWallet, "MAIN: The Buyback wallet is already this address");
    ...
    function updatebusdTokenDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdTokenDividendTracker), "MAIN: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdTokenDividendTracker =  BUSDDividendTracker(payable(newAddress));
 
        require(newbusdTokenDividendTracker.owner() == address(this), "MAIN: The new dividend tracker must be owned by the MAIN token contract");
    ...
    function updateusdtDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(usdtDividendTracker), "MAIN: The dividend tracker already has that address");
 
        USDTDividendTracker newusdtDividendTracker = USDTDividendTracker(payable(newAddress));
 
        require(newusdtDividendTracker.owner() == address(this), "MAIN: The new dividend tracker must be owned by the MAIN token contract");
    ...
    function updateBusdDividendRewardFee(uint8 newFee) external onlyOwner {
        ...
        totalFees = busdDividendRewardsFee.add(marketingFee).add(usdtDividendRewardsFee).add(liquidityFee).add(buybackFee);
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MAIN: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MAIN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "MAIN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "MAIN: Cannot update gasForProcessing to same value");
	...
	function processDividendTracker(uint256 gas) external onlyOwner {
		...
		emit ProcessedusdtDividendTracker(usdtIterations, usdtClaims, usdtLastProcessedIndex, false, gas, msg.sender);
    }
    ...
    function _transfer(
        ...
        if (canSwap &&
            ...
            swapping = true;
                ...
                uint256 usdtTokens = contractTokenBalance.div(totalFees).mul(usdtDividendRewardsFee);
       ...
       try busdTokenDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        ...
        if(!swapping) {
	    	...
	    	try usdtDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedusdtDividendTracker(iterations, claims, lastProcessedIndex, true, gas, msg.sender);
	    	}
	    	catch {
    ...
    function swapTokensForUSDT(uint256 _tokenAmount) private {
    ...
    function swapAndSendUSDTDividends(uint256 tokens) private {
        ...
        uint256 usdtTokens = IERC20(USDT).balanceOf(address(this));
        transferDividends( address(usdtDividendTracker), usdtDividendTracker, usdtTokens, USDT);
    }
...
contract USDTDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("T_USDT_Dividend_Tracker", "T_USDT_Dividend_Tracker", 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18);
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "T_USDT_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "T_USDT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MAIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "T_USDT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "T_USDT_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BUSDDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("MAIN_BUSD_Dividend_Tracker", "MAIN_BUSD_Dividend_Tracker", 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18);
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "MAIN_BUSD_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "MAIN_BUSD_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MAIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MAIN_BUSD_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MAIN_BUSD_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
