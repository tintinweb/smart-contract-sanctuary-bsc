...
contract Market is Ownable, Initializable {
 ...
 function claim() external {
        ...
        uint claimAmount = 0;
        uint userbalance = (user_.balance * 50e18)/100e18;
        totDays = (block.timestamp - lastClaimTimestamp) / claimPeriod;
        user_.lastClaim += claimPeriod * totDays;
        
        
         if(user_.totalClaimed == 0){
            claimAmount = userbalance;
        }
        claimAmount+= (userbalance *(subsequentClaim * totDays)) / 100e18;
        if((user_.totalClaimed + claimAmount) > user_.balance) {
            claimAmount = userbalance - user_.totalClaimed; 
        }
