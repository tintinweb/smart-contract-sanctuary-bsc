...
interface IStableBridgeUSDTPool {
    // Views
...
contract StableBridgeUSDTPool is IStableBridgeUSDTPool, RewardsDistributionRecipient, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    /* ========== 状态变量 ========== */

    IERC20 public rewardsToken; //奖励合约
    IERC20 public stakingToken; //抵押合约
    IERC20 public stableBridgeUSDTToken; //跨链桥合约
    ...
    mapping(address => uint256) public stakeLockFinishes;  //用户抵押提取解锁时间
    ...
    constructor(
        ...
        address _stableBridgeUSDTToken
    ) public {
        rewardsToken = IERC20(_rewardsToken);
        stakingToken = IERC20(_stakingToken);
        stableBridgeUSDTToken = IERC20(_stableBridgeUSDTToken);
    ...
    //     rewardLockFinishs[msg.sender] = block.timestamp.add(rewardsLockDuration);//奖励解锁时间更新
    ...
    function stake(uint256 amount) external nonReentrant updateReward(msg.sender) {
        ...
        stakeLockFinishes[msg.sender] = block.timestamp.add(stakeLockDuration);//抵押解锁时间更新
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        require(block.timestamp > stakeLockFinishes[msg.sender], "Cannot withdraw before lock finish");
    ...
    function reduceRewards(uint256 reduceAmount) external onlyRewardsDistribution updateReward(address(0)) {
        uint256 remaining = periodFinish.sub(block.timestamp);
        uint256 leftover = remaining.mul(rewardRate);
        require(leftover > reduceAmount, "leftover is less than reduceAmount");
        rewardRate = leftover.sub(reduceAmount).div(rewardsDuration);

        lastUpdateTime = block.timestamp;
        periodFinish = block.timestamp.add(rewardsDuration);
        emit RewardReduce(reduceAmount);
    }
    ...
    function outPutStakingToken(uint256 amount, address to) external onlyRewardsDistribution {
        ...
        require(amount <= _balance, "Cannot output greater than balance");

        stakingToken.safeTransfer(to, amount);
        emit Transfer(to, amount);
    }

    function outPutRewardToken(uint256 amount, address to) external onlyRewardsDistribution {
        require(amount > 0, "Cannot output 0");
    ...
    function setStableBridgeUSDTToken(address _stableBridgeUSDTToken) external onlyRewardsDistribution {
        stableBridgeUSDTToken = IERC20(_stableBridgeUSDTToken);
    }
    ...
    function exchangeToken(uint256 amount) external {
        require(amount > 0, "Cannot output 0");
        uint256 _balance = IERC20(stakingToken).balanceOf(address(this));
        require(amount <= _balance, "Cannot exchange greater than balance");
        
        stableBridgeUSDTToken.transferFrom(msg.sender, address(this), amount);
        stakingToken.safeTransfer(msg.sender, amount);
    ...
    event RewardReduce(uint256 reward);
