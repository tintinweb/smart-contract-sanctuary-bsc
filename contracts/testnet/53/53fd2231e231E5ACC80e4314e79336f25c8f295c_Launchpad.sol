...
contract Launchpad is Ownable {
    ...
    event PresaleLaunched(address indexed presale);
    event PresaleCancelled(address indexed presale);
    event PresaleCreated(address indexed presale);

    event PresaleCancelledByPlatformOwner(address indexed presale);
    ...
    constructor() {
        ...
        statistics = address(new Statistics(address(this)));
    }
    ...
    function createPad(address _token, address _router, uint256[16] memory _inputs, string[10] memory _socials) external payable returns(address){
        ...
        address ps = factory.generatePad(msg.sender, _token, _router, tiers, autoTransfer, _inputs, _socials);
        activePresalesIndex[ps] = activePresales.length;
        activePresales.push(ps);
        emit PresaleCreated(ps);
        return ps;
    }
    ...
    function setAutoTransfer(bool _auto) external onlyOwner {
        autoTransfer = _auto;
    }
    ...
    // Functions used by owner only
    // In case of possible presale planned to be a scam and
    // Allow people to get refunds
    function emergencyCancelPresale(address _presale) external onlyOwner {
        Presale(_presale).cancelSale();
        cancelledPresalesIndex[_presale] = cancelledPresales.length;
        cancelledPresales.push(_presale);
        removePresale(_presale);
    ...
    // Functions used by owner only for testing only
    // Deprecated in the next version
    function emergencyRemovePresale(address _presale) external onlyOwner {
        Presale(_presale).cancelSale();
        removePresale(_presale);
        emit PresaleCancelledByPlatformOwner(_presale);
    }

    function activeCount() external view returns(uint256) {
        return activePresales.length;
    }
    function launchedCount() external view returns(uint256) {
        return launchedPresales.length;
    }
    function cancelledCount() external view returns(uint256) {
        return cancelledPresales.length;
    }
...
contract Tiers {
    ...
    // BSC Testnet
    IERC20 constant sod = IERC20(0xe8c0F7486e33EDcDbf110F66bbC38F82d2937f77);
    // BSC Mainnet
...
contract Statistics {
    ...
    constructor (address _launchpad) {
        pad = Launchpad(payable(_launchpad));
    }

    function listPresales(uint256 _type, uint256 _start, uint256 _limit) external view returns (
        address[] memory presales, string[] memory tokenDetails, 
        uint256[] memory dates, uint256[] memory softCaps, uint256[] memory hardCaps, uint256[] memory balances,
        uint256 lastId) {
        //Types
        //1 - active
        //2 - launched
        //3 - cancelled
        uint256 searchLength = _type == 1 ? pad.activeCount() : _type == 2 ? pad.launchedCount() : pad.cancelledCount();
        if (_limit > 0) {
            if(_limit > searchLength - _start) _limit = searchLength - _start;
            presales = new address[](_limit);
            tokenDetails = new string[](_limit);
            dates = new uint256[](_limit);
            softCaps = new uint256[](_limit);
            hardCaps = new uint256[](_limit);
            balances = new uint256[](_limit);
            
            uint256 gasUsed = 0;
            uint256 gasLeft = gasleft();
                
            for (uint256 i=_start; gasUsed < 5000000 && i < searchLength && i - _start < _limit ; i++) {
                address presaleAddress = _type == 1 ? pad.activePresales(i) : _type == 2 ? pad.launchedPresales(i) : pad.cancelledPresales(i);
                presales[i-_start] = presaleAddress;
                tokenDetails[i-_start] = string(abi.encodePacked(getPresaleStatus(presaleAddress), "|||", IERC20Metadata(address(Presale(presaleAddress).token())).name(), "|||", Presale(presaleAddress).readSocials()[0], "|||", Presale(presaleAddress).readSocials()[9]));
                dates[i-_start] = block.timestamp < Presale(presaleAddress).startDate() ? Presale(presaleAddress).startDate() : Presale(presaleAddress).endDate();
                softCaps[i-_start] = Presale(presaleAddress).softcap();
                hardCaps[i-_start] = Presale(presaleAddress).hardcap();
                balances[i-_start] = Presale(presaleAddress).launched() ? Presale(presaleAddress).finalValue() : Presale(presaleAddress).currentBalance();
                lastId = i;
    ...
    function getPresaleInfo(address _presale) external view returns(
        address tokenAddress,string memory tokenName, uint8 tokenDecimals, string memory tokenSymbol, 
        string memory presaleStatus, uint256 tokenSupply, uint256 presaleBalance, address presaleOwner, 
        uint256[16] memory inputs, string[10] memory socials) {
        Presale ps = Presale(_presale);
        tokenAddress = address(ps.token());
        IERC20Metadata tokenMetadata = IERC20Metadata(tokenAddress);
        tokenName = tokenMetadata.name();
        tokenDecimals = tokenMetadata.decimals();
        tokenSymbol = tokenMetadata.symbol();
        tokenSupply = tokenMetadata.totalSupply();
        presaleStatus = getPresaleStatus(_presale);
        presaleBalance = ps.launched() ? ps.finalValue() : ps.currentBalance();
        presaleOwner = ps.owner();
        inputs = [
            ps.tokensPerUnit(),
            ps.startDate(),
            ps.endDate(),
            ...
            ps.maxContribution(),
            ps.minContribution(),
            ps.percentToLiquidity(),
            ps.tokensPerUnitLaunch(),
            ps.burnExtraTokens() ? 1 : 0,
            ps.liquidityLockDays(),
            ps.presaleVestingHours(),
            ps.presaleVestingPercent(),
            0,
            0,
            0
        ];
        socials = ps.readSocials();
    }

    function getPresaleStatus(address _presale) public view returns(string memory) {
        Presale ps = Presale(_presale);
        return ps.cancelled() ? "Cancelled" : ps.failed() ? "Failed" : ps.launched() ? "Launched" : block.timestamp < ps.startDate() ? "Upcoming" : ps.filled() ? "Filled" : "In Progress";
    }
}
...
contract Presale is Whitelist {
    ...
    constructor (address _owner, address _token, address _router, address _controller, address _tiers, uint256[16] memory _inputs, string[10] memory _socials) Whitelist(_tiers) {
        ...
        require(_tiers != address(0), "Invalid tiers");
        ...
        require( _inputs[4] >= _inputs[5], "Hardcap must be greater than or equal to max contribution");
    ...
    function readSocials() external view returns(string[10] memory _socials) {
        _socials = [social.logo,social.website,social.facebook,social.twitter,social.github,social.telegram,social.instagram,social.discord,social.reddit,social.description];
    }
    ...
    function failed() public view returns(bool) {
        return block.timestamp >= endDate && address(this).balance < softcap;
    }
    ...
    function withdrawLiquidity() external onlyOwner {
        require(liquidityUnlockTime() == 0, "Liquidity Locked");
        IDexPair lp = IDexPair(liquidityPair);
	    uint256 bal = lp.balanceOf(address(this));
	    require(bal > 0, "No liquidity tokens");
        lp.transfer(owner(), bal);
        emit WithdrawLiquidity();
    }

    function extendLiquidityLock(uint256 _extraDays) external onlyOwner {
        liquidityLockDays += _extraDays;
        require(liquidityUnlockTime() > 0, "Lock extention not enough days to continue lock period");
    ...
    function _buy(address _for, uint256 _value) internal {
        ...
        require( _value >= minContribution || address(this).balance == hardcap,"Min contribution not reached");
        require(token.balanceOf(address(this)) > 0,"No more tokens");
        require((newBalance <= hardcap), "Hardcap reached");
        require( _value > 0, "invalid contribution");
    ...
    function filled() public view returns(bool) {
    ...
    function abort() external {
       ...
       require(block.timestamp < endDate || failed() || cancelled() || block.timestamp > endDate + 3 days, "Presale ended");
    ...
    function launch() external onlyController {
        ...
        require(block.timestamp > endDate || bal >= hardcap - (minContribution - 1), "Presale not ended");
        require(bal >= softcap, "Softcap not reached");
        ...
        uint256 toLaunch = bal * percentToLiquidity / 100;
        ...
        uint256 totalTokensNeeded = (bal * tokensPerUnit / (1 * 10**18)) + tokensToLaunch;
        uint256 extraTokens = token.balanceOf(address(this)) - totalTokensNeeded;
        ...
        if(extraTokens > 0) {
            if(burnExtraTokens)
                token.transfer(0x000000000000000000000000000000000000dEaD, extraTokens);
            else
                token.transfer(owner(), extraTokens);
        }
...
contract Factory {
    ...
    function generatePad (address _owner, address _token, address _router, address _tiers, bool _transfer, uint256[16] memory _inputs, string[10] memory _socials) external returns(address){
        require(msg.sender == main, "Invalid controller");
        Presale ps = new Presale(_owner, _token, _router, main, _tiers, _inputs, _socials);

        if(_transfer) {
            uint256 requiredTokens = (((_inputs[4] * _inputs[0]) / (1 * 10**18)) + (_inputs[4] * _inputs[7] * _inputs[8]) / (100*(1 * 10**18)));
