...
// File: Lotto/lib/IPancakeRouter01.sol
...
contract Lotto is Context, IERC20, Ownable {
    ...
    mapping (address => address) public transferRef;

    uint256 private constant MAX = ~uint256(0);

    //Total Supply
    uint256 private _tTotal = 660000000 * 10 ** 6;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public  _tTaxFeeTotal;

    string private _name = "Lotto";
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (account == burnAddress || account == lp) return _tOwned[account];
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, uint tradeType, bool takeFee) private {
        require(tradeType == 1 || tradeType == 2 || tradeType == 0, "trade type error");

        if(!takeFee) {
            removeAllFee();
        }
        if(tradeType == 1){ // buy  from shouldnt take fee
            _transferFromExcluded(sender, recipient, amount);
        }else if(tradeType == 2){ // sell  to shouldnt take fee
            _transferToExcluded(sender, recipient, amount);
        }else{ // transfer both shouldnt take fee
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 lpFee, uint256 burnFee, uint256 marketingFee) = _getSellTValues(tAmount);
        ...
        if(marketingFee>0 && marketing != address(0)){
            incBalance(marketingFee, marketing);
        }
        if(lpFee>0){
            //TODO
        }
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        // uint256 tFee = 0;
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tTransferAmount, 0, _getRate());
        ...
        emit Transfer(sender, recipient, tTransferAmount);
        if(reward>0){
            referralReward(sender, sender, tAmount);
        }
    }
    ...
    // reward refferal
    function referralReward(address sender, address account, uint256 tAmount) private {
        bool end = false;
        uint i = 0;
    ...
    function _getBuyTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 reward) {
        if (!feeIt) {
            return (tAmount,0);
        }
        reward = tAmount.mul(6).div(100); // buy 6%
        tTransferAmount = tAmount.sub(reward);
    }

    function _getSellTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tFee, uint256 lpFee, uint256 burnFee, uint256 marketingFee) {
        if (!feeIt) {
            return (tAmount,0,0,0,0);
        }
        tFee = tAmount.mul(4).div(100); // sell 9%
        lpFee = tAmount.mul(2).div(100); 
        burnFee = tAmount.mul(1).div(100); 
        marketingFee = tAmount.mul(2).div(100); 

        tTransferAmount = tAmount.sub(tFee).sub(lpFee).sub(burnFee).sub(marketingFee);
    }
    ...
    function compareLp(address account) public view returns (bool result){
        return account == lp;
    }

    function _transfer(
        address from, address to, uint256 amount
    ) private returns(uint256 tradeType){
