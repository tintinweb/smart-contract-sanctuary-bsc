...
contract Token is ERC20, Ownable {
    ...
    constructor(
        ...
        // 1%
        _walletMax = totalSupply.div(100);

        gasForProcessing = 200000;

        dividendTracker = new TokenDividendTracker(tokenReward_, swapTokensAtAmount);
        ...
        whiteList[_marketingWalletAddress] = true;
        whiteList[address(_uniswapV2Router)] = true;
        whiteList[address(dividendTracker)] = true;
    ...
    function setTradeTime(uint256 time) external onlyOwner {
        tradeTime = time;
        limitTime = time + 3600;
    }
    ...
    function setLimitTime(uint256 time) external onlyOwner {
        limitTime = time;
    }
...
//            if (AmountTokenRewardsFee > swapTokensAtAmount && dividendTracker.totalSupply() > 0) swapAndSendDividends(AmountTokenRewardsFee);
        ...
        if (block.timestamp <= limitTime) {
            ...
            if (!_isExcludedFromFees[to] && !automatedMarketMakerPairs[to]) {
                require(balanceOf(to) <= _walletMax, "exceed wallet max amount");
            }
        }
//
...
//        AmountMarketingFee = AmountMarketingFee - tokens;
    ...
    function swapAndSendDividends(uint256 tokens) private {
        IERC20 reward = IERC20(_rewardToken);
        swapTokensForTokens(tokens);
        reward.transfer(address(dividendTracker), reward.balanceOf(address(this)));
