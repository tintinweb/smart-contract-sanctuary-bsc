...
contract TheChosenOne is ERC721HolderUpgradeable, DivestorUpgradeable, RandomGeneratorUpgradeable{
    using SafeMathUpgradeable for uint;
    INFTUP public info;

    address public ChosenUser;

    function initialize(address RBA_) public initializer{
        __ERC721Holder_init();
        __Divestor_init();
    ...
    event ChoiceUser(address indexed user);

    function Choice() public onlyOwner returns (address){
        uint count = info.getLatestId();
        uint index = randomCeil(count);
        ChosenUser = info.ownerOf(index);
        emit ChoiceUser(ChosenUser);
        return ChosenUser;
    }
...
interface INFTUP is IERC721EnumerableUpgradeable{
    ...
    function getLatestId() external returns (uint256);
