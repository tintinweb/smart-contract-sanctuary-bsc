...
contract Recharge is Ownable {
    ...
    function permitRecharge(address msgSender,address contractAddr,string memory funcName, uint256 _orderId,uint256 _amount0,uint256 _amount1,uint256 _amount2,uint256 deadline, uint8 v, bytes32 r, bytes32 s) private {
    ...
    function recharge(uint256 _orderId,uint256 amount0 ,uint256 amount1,uint256 amount2,uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        require(orderIds[_orderId] == 0, "orderId has been used!");
        orderIds[_orderId] = 1;
        permitRecharge(msg.sender,address(this),"recharge",_orderId,amount0,amount1,amount2,deadline, v, r, s); 
        if (amount0 > 0){
