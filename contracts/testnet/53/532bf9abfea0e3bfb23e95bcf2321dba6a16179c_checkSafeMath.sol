...
contract checkSafeMath {
    ...
    function addd(uint256 a, uint256 b) external pure returns(uint256) {uint256 c = a.add(b); return c;}
    function subb(uint256 a, uint256 b) external pure returns(uint256) {uint256 c = a.sub(b); return c;}
    function mul(uint256 a, uint256 b) external pure returns(uint256) {uint256 c = a.mul(b); return c;}
    function div(uint256 a, uint256 b) external pure returns(uint256) {uint256 c = a.div(b); return c;}
    function mod(uint256 a, uint256 b) external pure returns(uint256) {uint256 c = a.mod(b); return c;}
}
...
contract BartMAN is IBEP20, Auth {
    ...
    address internal MarketingFeeReceiver =
        address(0x00322C1b922Ee02C96e62B51331C040b4089F801);
        ...
        authorized
    {
        ...
        require(
            liquidityFee.add(MarketingFee) <= 10,
            "You can't set more than 10%"
        );
        emit UpdatedBuyTaxFeePercentages(liquidityFee.add(MarketingFee));
    }
        ...
        authorized
    {
        ...
        require(
            sellliquidityFee.add(sellMarketingFee) <= 15,
            "You can't set more than 15%"
        );
        emit UpdatedSellTaxFeePercentages(
            sellliquidityFee.add(sellMarketingFee)
        );
    }
