...
interface IRoyaltyFeeRegistry {
    ...
    function royaltyInfo(address collection, uint256 amount) external view returns (address[] memory, uint256[] memory);
...
contract RoyaltyFeeRegistry is IRoyaltyFeeRegistry, Ownable {
    ...
    event RoyaltyFeeUpdate(address indexed collection, address indexed setter, address[] receiver, uint256[] fee);
    ...
    function updateRoyaltyInfoForCollection(
        ...
        require(receiver.length == fee.length, "Registry: parameter mismatch");
        uint256 sumFee;
        for (uint i = 0; i < fee.length; i++) {
            require(receiver[i] != address(0) && fee[i] != 0, "Registry: zero address");
            sumFee += fee[i];
        }

        require(sumFee <= royaltyFeeLimit, "Registry: Royalty fee too high");
     ...
     */
    function royaltyInfo(address collection, uint256 amount) external view override returns (address[] memory receiver, uint256[] memory fees) {
        FeeInfo memory feeInfo = _royaltyFeeInfoCollection[collection];
        receiver = feeInfo.receiver;
        fees = new uint256[](receiver.length);
        
        for (uint i = 0; i < feeInfo.fee.length; i++) {
            fees[i] = (amount * feeInfo.fee[i]) / 10000;
        }
    }
