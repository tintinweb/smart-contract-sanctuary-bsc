...
contract StakingMapMilestoneForERC1155 is Ownable, Pausable {
    ...
    function rewardPointsAmount(address _userAddress) public view returns (uint256) {
        ...
        for (uint256 i; i < _stakes.length; i++) {
            // User may have multiple stakes(pawns)
            if ((uint32(block.timestamp) - _stakes[i].createdAt) >= (REWARD_MIN_TIME_PERIOD)) {// Diff time from stake start to current time
    ...
    function receiveRewards(uint256 _rewardId, uint256 _rewardAmounts) external whenNotPaused {
        uint256 _totalCost = uint256(REWARDS[_rewardId].pointsCost) * _rewardAmounts;
        ...
        REWARDS_CONTRACT.mint(_msgSender(), uint256(REWARDS[_rewardId].rewardId), _rewardAmounts, bytes(''));

        emit RewardReceivedSingle(_msgSender(), _rewardId, _rewardAmounts);
    }
