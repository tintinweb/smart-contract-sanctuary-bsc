...
contract NewDefinaCard is ERC721EnumerableUpgradeable, OwnableUpgradeable, NewDefinaCardEventsAndErrors {
    ...
    function toMint(uint256 mintStartTokenId, uint256 amount_, bool forWhitelist_, uint256[] calldata randomness_) external {
...
//    }
    ...
    function addMerge(uint tokenIdA, uint tokenIdB, uint price) payable onlyEOA whenMergerActive external {
        ...
        if (address(vrfV2) != _msgSender()) {
            approve(address(vrfV2), tokenIdA);
            approve(address(vrfV2), tokenIdB);
        }
