...
abstract contract ERC721URIStorage is ERC721Enumerable {
    ...
    function tokensOf(address _add) external view returns (uint256[] memory) {
        uint256 count = balanceOf(_add);
        uint256[] memory newArr = new uint256[](count);
        for (uint256 index = 0; index < count; index++) {
            newArr[index] = tokenOfOwnerByIndex(_add, index);
        }
        return newArr;
    }
}
...
contract NFT is ERC721URIStorage, Ownable {
    ...
    event MintNFT(uint256 indexed tokenId, address buyer, string uri);
    ...
    function mint(string memory tokenURI) public {
        ...
        emit MintNFT(newItemId, msg.sender, tokenURI);
    }
    ...
    function fillOrder(uint256 _tokenId) public {
        ...
        ItemSale memory itemSale = markets[_tokenId];
        uint256 feeMarket = itemSale.price.mul(_feeMarketRate).div(
            _divPercent
        );
        tokenERC20.transferFrom(_msgSender(), _feeAddress, feeMarket);
    ...
    function getItemMarket(uint256 id) external view returns (ItemSale memory){
        return markets[id];
    }
