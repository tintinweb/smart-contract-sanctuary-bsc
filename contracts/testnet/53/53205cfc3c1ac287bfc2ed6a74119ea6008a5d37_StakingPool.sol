...
contract StakingPool is Upgradable {
    ...
    */
    ...
    function unstakeAllToken(string[] memory strs, uint256 typePool) internal {
        PoolInfo storage pool = poolInfo[strs[0]];
        StakingData storage data = tokenStakingData[strs[0]][msg.sender];
        uint256 reward = 0;
        ...
        if(typePool == 0){
            emit StakingEvent(reward, msg.sender, strs[0], strs[1]);
        }
    }
     ...
     * @param strs: poolId(0), internalTxID(1)
    */
    function _claimReward(string[] memory strs) internal poolExist(strs[0]) taxFeeValid notBlocked { 
        PoolInfo storage pool = poolInfo[strs[0]];
        StakingData storage data = tokenStakingData[strs[0]][msg.sender]; 
        ...
        rewardClaimedPerUser[strs[0]][msg.sender] += availableAmount;
        ...
        emit StakingEvent(availableAmount, msg.sender, strs[0], strs[1]); 
    }
