...
contract EquaProtocol is Ownable {
    ...
    function defineProblem(string memory problem, bool zkProofRequired)
        external
        returns (bytes32 problemId)
    {
        problemId = keccak256(abi.encode(problem));
        ...
        userProblems[msg.sender].push(problem_);

        emit ProblemCreated(block.timestamp, msg.sender, problemId);
    }
    ...
    function request(
        ...
        EQUA.safeTransferFrom(sender, msg.sender, reward);
        ...
        userRequests[msg.sender].push(request_);

        emit RequestCreated(block.timestamp, sender, requestId);
        ...
        onlySender(requestId)
    {
        ...
        emit RequestCancelled(block.timestamp, msg.sender, requestId);
    }

    function solve(uint256 requestId, bytes memory answer) public onlyPending(requestId) {
        require(!requests[requestId].problem.zkProofRequired);
    ...
    function _checkCachedSolution(uint256 requestId) internal {
        if (requests[requestId].cache) {
            ...
            Solution memory solution = solutions[request_.sender][
                request_.problem.problemId
            ][request_.inputHashed];

            if (solution.solved) {
                requests[requestId].status = RequestStatus.SOLVED;
                if (problems[request_.problem.problemId].zkProofRequired) {
                    try
                        RequesterZkProof(requests[requestId].sender).solveZkProof(
                            requestId,
                            solution.answer,
    ...
    function _cacheSolution(
        ...
        if (requests[requestId].cache) {
            Request storage request_ = requests[requestId];
            solutions[request_.sender][request_.problem.problemId][
                request_.inputHashed
            ] = Solution({
                solved: true,
    ...
    function _solve(uint256 requestId, address solver) internal {
        ...
        emit RequestSolved(block.timestamp, solver, requestId);
    }
    ...
    /// @dev returns list of requests
    /// @param offset start of indexing
    /// @param limit max amount of elements in response
    ///
    /// @return requests_ array of requests
    function getRequests(uint256 offset, uint256 limit)
        external
        view
        returns (Request[] memory requests_)
    {
        require(offset < requests.length);
        require(limit < requests.length);

        requests_ = new Request[](limit);

        uint256 length = MathLib.min(limit + offset, requests.length);

        for (uint256 id = offset; id < length; id++)
            requests_[id - offset] = requests[id];
    }

    function getProblemsUser(address user) external view returns (Problem[] memory) {
        return userProblems[user];
    }
...
contract Test is RequesterStandart {
    ...
    constructor(EquaProtocol equaProtocol_) {
        ...
        problemId = equaProtocol.defineProblem('sqrt(x)', false);
    }

    mapping(uint256 => uint256) public inputs;

    function solve(uint256, bytes memory answer_) external override {
        uint256 sqrtPriceC = abi.decode(answer_, (uint256));
        require(_checkSqrt(q, sqrtPriceC));
        answer = sqrtPriceC;
    }

    function _checkSqrt(uint256 input, uint256 anwer) internal pure returns (bool) {
        return anwer * anwer == input;
    }

    function ask(uint256 q_) external {
        q = q_;
        equaProtocol.request(problemId, abi.encode(q_), 0, true);
    }
}
