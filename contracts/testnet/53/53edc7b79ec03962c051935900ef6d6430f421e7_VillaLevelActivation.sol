...
contract VillaLevelActivation {
    ...
    mapping(uint256=>ContestDetails) public contestList;
    mapping (uint256=>mapping(address=>ContestEntryDetails)) public contestParticipants;
    ...
    event ParticipateContest(address indexed user,uint256 contestId,uint256 amount,uint256 timestamp);
    event Winner(address indexed user,uint256 contestId,uint8 position);
   
    constructor(address payable ceoAddr,address payable  _authAddress) {
        ceoWallet = ceoAddr;
        authAddress = _authAddress;
        VILLA = IERC20(0x240e0E7F5d00aD6986a5229efB0cCd25fc34ED87);

                PlayAmount[1]= 1000 ether;
                WinningAmount[1] = 4000 ether;
            for(uint256 level = 2; level <=100; level++)
            {
                PlayAmount[level]=  PlayAmount[level-1] + 1000 ether;
    ...
    function setContest(uint256 id,uint256 _entryAmount,uint256 _maxUsers) external onlyOwner
    {
        contestList[id].entryAmount = _entryAmount;
        contestList[id].maxUsers = _maxUsers;
    }
   ...
   function participateContest(uint256 contestId) external {
       ...
       require(contestParticipants[contestId][msg.sender].amount>0,"Allready joined contest");
        require(contest.maxUsers>0,"Allready joined contest"); 

       contestParticipants[contestId][msg.sender].amount = contest.entryAmount;
       VILLA.transferFrom(msg.sender,address(this), contest.entryAmount);
       contest.totalCollectedAmount += contest.entryAmount;
       contest.usersJoined++;
        emit  ParticipateContest(msg.sender,contestId,contest.entryAmount,block.timestamp);
   }

   function declareResultContest(uint256 contestId,address[3] memory users) external {
       require(msg.sender==authAddress,"invalid user");
