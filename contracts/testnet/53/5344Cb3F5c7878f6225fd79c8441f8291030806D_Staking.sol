...
contract Staking {
    ...
    function getEarned(uint256 stakingId) external view returns (uint256) {
        ...
        require(ownerStaking.stakingType == 0, "Not valid staking");
        uint256 stakedTimeClaim = (block.timestamp - ownerStaking.timestamp) /
            1 days;
        uint256 finalApr = uint256(ownerStaking.apr);
        if (ownerStaking.proposalPercentage != 0) {
            uint256 proposalToWDA = (finalApr *
                ownerStaking.proposalPercentage) / 100;
            finalApr = ownerStaking.proposalAction == 0
                ? finalApr - proposalToWDA
                : finalApr + proposalToWDA;
        }
        // tránh số lẻ
