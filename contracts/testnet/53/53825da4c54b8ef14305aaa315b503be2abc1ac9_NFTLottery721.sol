    ...
    IERC721Receiver
{
    ...
    function buyTickets(address owner, uint256 count) external payable {
        ...
        require(
            IERC721(lottery.tokenAddress).ownerOf(lottery.tokenId) == address(this),
            "Prize not exist"
        );
        ...
        if (userTicketIdxs[owner].length == 0) {
            users.push(owner);
        }
        userTicketIdxs[owner].push([ticketStartIdx, ticketEndIdx]);
        emit BuyTickets(owner, ticketStartIdx, ticketEndIdx);
    ...
    function openLottery(address owner) external nonReentrant {
        require(lottery.status == Status.Openable, "Not openable");
        factory.requestRandomNumber();
        Address.sendValue(payable(owner), factory.getOpenLotteryReward());
        setLotteryStatus(Status.Opening);
    }

    function claimNFT(address owner, uint256 idx) external {
        require(
            lottery.status == Status.Claimable && !winnerClaimed,
            "Not claimable"
        );
        require(userTicketIdxs[owner].length > idx, "Only valid idx");

        uint256[][] memory ticketIdxs = userTicketIdxs[owner];
        ...
        winnerClaimed = true;
        IERC721(lottery.tokenAddress).safeTransferFrom(
            address(this),
            owner,
            lottery.tokenId
        );
