...
import "./abstracts/ShibaCardsBase.sol";

contract ShibaCards is ShibaCardsBase {
...
abstract contract ShibaCardsRandomizable is
...
import "../abstracts/ShibaCardsRandomizable.sol";
import "../interfaces/IDealer.sol";
import "../interfaces/IBank.sol";

abstract contract ShibaCardsBase is
  ERC1155Pausable,
  ShibaCardsDividendsDistributable,
  ShibaCardsPayable,
  ShibaCardsDealable,
  ShibaCardsRandomizable
{
  using SafeMath for uint256;

  event Gifted(address indexed to, uint256 value);
  ...
  mapping(uint256 => Request) internal _requestByID;
  mapping(address => uint256) internal _freeMints;
  uint256 internal constant BOOSTER = 7;
    ...
    for (uint256 i; i < ids.length; i++) {
      if (from != address(0) && to != address(0)) {
        dividendsDistributer.transferShares(
          from,
          to,
          dealer.getSharesOf(ids[i]) * amounts[i]
        );
      }
    }
  }
  ...
  function mintSpecific(
    ...
    dividendsDistributer.addShares(to, dealer.getSharesOf(ids));
  ...
  function mint(uint256 amount) public payable virtual {
    require(amount >= 1 && amount <= 3, "Between 1 and 3");
    _requestRandomMints(_msgSender(), amount);
  }
  ...
  function gift(address to, uint256 amount) public {
    if (!isAdmin(_msgSender()) && !isWhitelisted(_msgSender())) {
      bank.makePayment(_msgSender(), amount.mul(getFees()));
    }
  ...
  /**
   * @dev make a new randomness call
   */
  function _requestRandomMints(address to, uint256 amount)
    internal
    returns (uint256)
  {
    uint256 free = getFreeMints(to);
    uint256 fees = getFees();
    uint256 payment = fees.mul(free) >= fees.mul(amount) ? 0 : fees.mul(amount).sub(fees.mul(free));
    ...
    uint256 requestId = randomizer.requestRandomness();
    _requestByID[requestId] = Request(BOOSTER.mul(amount), payment, to);
    return requestId;
  }

  // TODO delete
  event FulFillRandomness(uint256 id, uint256 rnd);

  function fulfillRandomness(uint256 id, uint256 rnd)
    public
    override
    onlyWhitelisted
  {
    _fulfillRandomness(id, rnd);
    emit FulFillRandomness(id, rnd);
  }

  function _fulfillRandomness(uint256 id, uint256 rnd)
    internal
  {
    Request memory r = _requestByID[id];
    uint256[] memory ids = dealer.getRandomIds(rnd, r.amount);
    uint256[] memory amounts = new uint256[](r.amount);

    for (uint256 i; i < r.amount; i++) {
      amounts[i] = 1;
    }

    dividendsDistributer.addShares(r.account, dealer.getSharesOf(ids));
    _mintBatch(r.account, ids, amounts, "");

    if (r.payment > 0) {
      bank.distribute(r.payment);
    }
  }
