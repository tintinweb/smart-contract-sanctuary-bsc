...
contract Block is ERC721, Ownable{
    ...
    function CheckOwner(int256 _x,int256 _y)public view returns(address,string memory){
        return (checkowner[_x][_y],checkstring[_x][_y]);
    }
    function plentblockmint(int256 x,int256 y,string memory _ptype)public payable returns(bool){
        require(!xaxis[x] || !yaxis[y],"alreday mint");
        require(msg.value >= 10**17,"minimum 0.1 BNB");
