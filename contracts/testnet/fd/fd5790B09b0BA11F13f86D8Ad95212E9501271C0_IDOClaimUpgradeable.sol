    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier checkNonce(uint256[] calldata nonceArray) {
        for(uint256 i=0;i< nonceArray.length; i++){
            require(!_monthsMap[_msgSender()][nonceArray[i]], "The nonce has been used");
        }
        _;
    }
    ...
    function claimIDO(ClaimStruct calldata claimStruct)
        public
        whenNotPaused
        checkNonce(claimStruct.monthArray)
        nonReentrant
    {
        ...
        require(!_noncesMap[claimStruct.nonce],"nonce already used");
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "ClaimStruct(uint256 amount,address receiver,address tokenAddress,uint256[] monthArray, string nonce)"
                        ),
                        claimStruct.amount,
                        claimStruct.receiver,
                        claimStruct.tokenAddress,
                        keccak256(abi.encodePacked(claimStruct.monthArray)),
