...
contract DeltaAce is Context, IBEP20, Ownable {
    ...
    mapping(address => uint256) private newL;
    ...
    // struct userFeeControl{
    //     address userAccount;
    //     uint256 total24HourWithdraw;
        ...
        if (_isAtomaticMarketMaker[to] && !_isExcludedFromFee[from]) {
            ...
            if(_24HourTiming[from] < block.timestamp) {
                if(_totalSellIn24Hour[from].add(amount) <= _sellLimit){
                  ...
                  _DevelopmentFee = _sellDevelopmentFee.mul(2);
                  _MarketingFee = _sellMarketingFee.mul(2);
                  _burnFee = _sellBurnFee.mul(2);
                  _totalSellIn24Hour[from] = _totalSellIn24Hour[from].add(amount);
                }
            }else if(_24HourTiming[from] > block.timestamp){
                if(_totalSellIn24Hour[from].add(amount) <= _sellLimit){
                _taxFee = _sellTaxFee;
                  _liquidityFee = _sellLiquidityFee;
                  _DevelopmentFee = _sellDevelopmentFee;
