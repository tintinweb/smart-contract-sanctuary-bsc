...
contract UserBase is Ownable {
    ...
    uint8 constant START_LEVEL = 1;
    uint8 constant START_EXP = 0;
    uint8 constant MAX_LEVEL_CAP = 20;
    uint32[] public EXP_TO_LEVEL_UP = [
        8,
        280,
        742,
        1444,
    ...
    function getUser(address _owner)
        external
        view
        returns (User memory)
    {
        User memory _user = ownerInfos[_owner];

        return _user;
    }

    function _updateUserLevel(address _owner, uint256 _level)
        internal
        onlyOwner
    {}

    function _updateUserExp(address _userAddress, uint256 _exp) internal {
        User memory _user = ownerInfos[_userAddress];
        ...
        while (
            _user.level < MAX_LEVEL_CAP &&
            _user.exp >= EXP_TO_LEVEL_UP[_user.level - 1] 
        ) {
    ...
    function updateUserExpTest(address _userAddress, uint256 _exp) public {
        User memory _user = ownerInfos[_userAddress];
        bool isUserLevelUp = false;
        uint8 userCurrentLevel = _user.level;
        uint8 userNextLevel = _user.level;
    ...
    function createUser(address _owner, string memory _name)
        external
        onlyOwner
        returns (User memory)
    {
        User memory user = User(_name, START_LEVEL, START_EXP);

        users.push(user);
        ownerInfos[_owner] = user;

        emit UserCreated(_owner, _name, START_LEVEL);
