...
contract MetaTronStake is Ownable,ReentrancyGuard,Pausable {
    ...
    constructor (IERC20 _stake) {
         ...
         planDetails[1].percent = 60e18;
         planDetails[2].percent = 30e18;
         
         //Plan3 only duration
         planDetails[1].duration = 730 days;
         planDetails[2].duration = 365 days;
         planDetails[3].duration = 1095 days;

         planDetails[1].claimCount = 4;
         planDetails[2].claimCount = 2;
    }

    function updatePlan (uint8 _plan,uint256 _percent,uint256 _duration) public onlyOwner {
    ...
    function harvest (uint256 _id) public nonReentrant whenNotPaused {
        ...
        require(user.depositTime > 0 && user.depositAmount > 0,"Invalid user");
        require(block.timestamp >= user.lastclaim + (plans.duration/plans.claimCount)+12 hours,"No time to claim");
        require(planDetails[user.userInPlan].claimCount > user.userClaimCount,"claim already reached its time to unstake");
        uint reward = user.depositAmount*(user.stakePercent/plans.claimCount)/100e18;
