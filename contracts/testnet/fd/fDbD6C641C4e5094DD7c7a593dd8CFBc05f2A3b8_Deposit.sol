...
contract Deposit is Ownable {
	...
	event DepositEvent(address indexed token, uint256 amount, uint256 timestamp);
	...
	function deposit(address _tokenAddress, uint256 _amount) public {
		...
		IERC20(_tokenAddress).transferFrom(msg.sender, receiver1, receiver1_amount);
		IERC20(_tokenAddress).transferFrom(msg.sender, receiver2, receiver2_amount);

		emit DepositEvent(_tokenAddress, _amount, block.timestamp);
	}

	function setPercent(uint256 _receiver1, uint256 _receiver2) public onlyOwner {
		receiver1_percent = _receiver1;
		receiver2_percent = _receiver2;
	}

	function setReceiver(address _receiver1, address _receiver2) public onlyOwner {
