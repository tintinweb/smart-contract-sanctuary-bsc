...
import "./interface/ITrustFiIDOERC20.sol";
...
contract TrustFiIDOFactoryV2 is ITrustFiIDOFactoryV2,ReentrancyGuard{
    ...
    */
    ...
    /** The pledge */
    function stake(uint256 poolId, uint256 amount,string memory userAddress) external  override nonReentrant{
        ...
        userStakeInfo.userAddress = userAddress;
        userStakeInfo.isBSC = poolStakeInfo.isBSC;
    ...
    function claim(uint256 poolId) external  override nonReentrant {
        ...
        uint256 claimTokenAmount = userStakeInfo.amount.mul(ratioToken)
        .mul(10**ITrustFiIDOERC20(poolStakeInfo.IDOToken).decimals())
        .div(10**18)
        .div(10**supportCommission.decimals);
    ...
    function refund(uint256 poolId) external  override nonReentrant{
        ...
        poolStakeInfo.amount = poolStakeInfo.amount.sub(userStakeInfo.amount);
    ...
    function getUserStakeInfosList(uint256 poolId) external view override returns (UserViewInfo [] memory){
        ...
        if(kl > 0)
        {
            ...
            for(uint256 i=0;i<kl;i++)
            {
                ...
                uvi.isBSC = userStakeInfos[poolId][userStakeInfosKeys[poolId][i]].isBSC;
                uvi.userAddress = userStakeInfos[poolId][userStakeInfosKeys[poolId][i]].userAddress;
    ...
    function editClaimStartTime(uint256 poolId,uint256 claimStartTime) external override onlyOperater {

        PoolStakeInfo storage poolStakeInfo = poolStakeInfos[poolId];
        require(block.timestamp <= poolStakeInfo.claimStartTime,"TrustFiIDO:POOL_CLAIM_HAS_START");
        poolStakeInfo.claimStartTime = claimStartTime; //startTime

        emit EditClaimStartTime(poolId,claimStartTime);
    }

    function editClaimEndTime(uint256 poolId,uint256 claimEndTime) external override onlyOperater {

        PoolStakeInfo storage poolStakeInfo = poolStakeInfos[poolId];
        require(block.timestamp <= poolStakeInfo.claimEndTime,"TrustFiIDO:POOL_CLAIM_HAS_START");
        poolStakeInfo.claimEndTime = claimEndTime; //startTime

        emit EditClaimEndTime(poolId,claimEndTime);
    }
...
interface ITrustFiIDOFactoryV2 is BaseStructV2 {
    ...
    */
    ...
    event EditClaimStartTime(uint256 poolId,uint256 claimStartTime);

    event EditClaimEndTime(uint256 poolId,uint256 claimEndTime);
    ...
    function stake(uint256 poolId, uint256 amount,string memory userAddress) external;
    ...
    function editClaimStartTime(uint256 poolId,uint256 claimStartTime) external;
    function editClaimEndTime(uint256 poolId,uint256 claimEndTime) external;
