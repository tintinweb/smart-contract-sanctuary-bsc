...
contract vrtrinityDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("vrtrinity _Dividend_Tracker", "vrtrinity _Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "vrtrinity _Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "vrtrinity _Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main vrtrinity  contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "vrtrinity _Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "vrtrinity _Dividend_Tracker: Cannot update claimWait to same value");
...
contract vrtrinity  is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    vrtrinityDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1000000000 * (10**18); //1%
    uint256 public swapTokensAtAmount = 200000000 * (10**18); //0.2%
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        ...
        require(totalFees<= 2000, "totalFees can't be set more then 20%");
    }
    ...
    function setMaxSelltx(uint256 _maxSellTxAmount) public onlyOwner {
        ...
        require(maxSellTransactionAmount >= 1000000000 * 10**18, "maxSell Amount can't be less then 1%");
    }
    ...
    constructor() ERC20("Vrtrinity", "VRT") {
        BNBRewardsFee = 250;    //2.5%
        liquidityFee = 100;     //1%
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 650;   //6.5%
        marketingWallet = payable(0x82A4C9437C7aA6711b79e8916820a86761c43619);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new vrtrinityDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "vrtrinity : The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "vrtrinity : Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "vrtrinity : The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "vrtrinity : Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "vrtrinity : gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "vrtrinity : Cannot update gasForProcessing to same value");
