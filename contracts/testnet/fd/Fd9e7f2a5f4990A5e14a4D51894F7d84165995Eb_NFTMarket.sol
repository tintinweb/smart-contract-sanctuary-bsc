...
contract NFTMarket is MarketEvents, verification, AccessControl {
    ...
    /*///////////////////////////////////////////////////////////////
                              CONSTRUCTOR
    //////////////////////////////////////////////////////////////*/
    constructor(address _feeVault, address _lendersVault,address _panoramWallet, 
    address _control, address token) {
        feeVaultAddress = _feeVault;
        lendersVault = _lendersVault;
        panoramWallet = payable(_panoramWallet);
        control = _control;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEV_ROLE, msg.sender);
        IERC20(token).approve(feeVaultAddress, 2**255);
        IERC20(token).approve(lendersVault, 2**255);
        IERC20(token).approve(panoramWallet, 2**255);
    }
    ...
    function makeBid(
        ...
        if(_tokenAmount != 0){
            ...
            (pan, rewards, lenders) = calcFees(_feeAmount);
            vars._amount = msg.value - _feeAmount;
        }
        ///@dev Transfer buy fees to the vault
        IERC20(_erc20Token).transfer(panoramWallet, pan); 
        IVaultRewards(feeVaultAddress).deposit(rewards, _erc20Token);
        IVaultRewards(lendersVault).deposit(lenders, _erc20Token);
   ...
   //////////////////////////////////////////////////////////////*/
    ...
    function _transferNftAndPaySeller(
        ...
        IControl(control).addQuantity(vars._nftHighestBidder,1);
        IControl(control).removeQuantity(vars._nftSeller,1);
    ...
    function sendpayment(
        ...
        if (auctionERC20Token != address(0)) {
            IERC20(auctionERC20Token).transfer(panoramWallet, pan);
        } else {
            (bool success, ) = payable(panoramWallet).call{value: pan}(
                ""
            );
            if (!success) {
                failedTransferCredits[panoramWallet] =
                    failedTransferCredits[panoramWallet] +
                    pan;
            }
        }
    ...
    function updateVaultRewards(address _newVaultRewards, address token) public {
        if (!hasRole(DEV_ROLE, msg.sender)) {
            revert("have no dev role");
        }
    ...
    function updatePanoramWallet(address _newPanoramWallet) public {
        if (!hasRole(DEV_ROLE, msg.sender)) {
            ...
            panoramWallet = payable(_newPanoramWallet);
            IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee).approve(panoramWallet, 2**255);
    }
    ...
    /*///////////////////////////////////////////////////////////////
                        MINTING FUNCTIONS
    //////////////////////////////////////////////////////////////*/
    function mintingMortgage(address _collection, address _owner, uint256 _value) public isPaused returns(uint256 _nftId){
        bool _status = ILazyNFT(_collection).getPresaleStatus();
        if(_status){
            revert ("Presale Open");
        }
    ...
    function mintingPresaleMortgage(address _collection, address _owner, uint256 _value) public isPaused returns(uint256 _nftId){
        bool _status = ILazyNFT(_collection).getPresaleStatus();
        if(!_status){
    ...
    function minting(address _collection, address _owner, uint256 _value, uint256 _fee, address _token) public isPaused {
        ...
        IERC20(_token).transfer(panoramWallet, amount); //transfer percentage fee and NFT cost
    ...
    uint256 _fee, address _token) public isPaused {
        LocalMint memory locals;
        IERC20(_token).transferFrom(msg.sender, address(this), _value);
        locals._status = ILazyNFT(_collection).getPresaleStatus();
        if(locals._status){
            revert ("Presale Open");
        }
        ...
        IERC20(_token).transfer(panoramWallet, locals.amount); //transfer percentage fee and NFT cost
    ...
    function addRegistry(address _collection, uint256 _nftId, address _owner) internal {
        ...
        IControl(control).addQuantity(_owner,1);
    }
