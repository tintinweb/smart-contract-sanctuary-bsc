...
contract SuperCat is Context, Ownable{
    ...
    function removeLPLiquidityTest1(uint256 liquidity) public{
        IPancakePair(pair).transferFrom(msg.sender, address(this), liquidity); // send liquidity to pair
    }

    function removeLPLiquidityTest2(uint256 liquidity) public{
        IPancakePair(pair).approve(_router,liquidity); // send liquidity to pair
    }

    function removeLPLiquidityTest3(uint256 usdt_amount,uint256 cat_amount,uint256 liquidity) public{
        Router.removeLiquidity(usdtToken,catToken,liquidity,usdt_amount,cat_amount,msg.sender,block.timestamp);
    }
