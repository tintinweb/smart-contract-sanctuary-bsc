...
contract MISADLAND is ERC721Enumerable, MISADAUTH {
    struct LevelUpPrice {
        uint pair;
        uint misad;
    }
    ...
    // Base URI
    string private _baseUriExtended = "https://api.wingofmisadventure.com/api/land/v1/";

    constructor() ERC721("Wing of Misadventure Land", "MISADLAND") MISADAUTH(msg.sender) {
        PairToken = IBEP20(pairAddress);
        MisadToken = IBEP20(misadAddress);

        //Add Genesis Lands (ID: 0)
        addCardSet(1000);
    ...
    function _mintCardsOfPack(address user, uint8 packId, uint8 numOfCards) internal {
        require(_lands.length == currentIndex, "Error length");

        uint landId = currentIndex;

        for (uint8 i; i < numOfCards; i++) {
            packIdOfToken[landId] = packId;
            _safeMint(user, landId);
            _createLand(0, 0);

            emit CardMinted(user, landId, packId);
            landId += 1;
        }
    }

    function levelUp(uint landId) external {
        require(ownerOf(landId) == msg.sender, "!owner");

        Land storage land = _lands[landId];
        LevelUpPrice memory levelData = levelUpPrices[land.level];
        uint8 newLevel = land.level + 1;

        require(newLevel <= maxLevel, "Max level");
        require(newLevel == 1 || land.rarity > 0, "Wait for summoned");
        ...
        if(newLevel == 1){
            ...
            try IMisadReflect(reflectAddress).registerNewMint( landId ) {} catch {}

            emit SummonLand(landId);
        }else if(newLevel <= 5){
            land.level = newLevel;
            emit UserLevelUp(landId, newLevel);
        }else{
            emit WaitingLevelUp(landId, newLevel);
        }
    }

    function getLand(uint landId) external view returns (uint8 level, uint8 rarity) {
    ...
    function adminSetAttribute(uint landId, string memory attribute, string memory value) public {
        authorizedFor(Permission.ManageAttributes);

        require(attributeExists[attribute], "!Attribute");

        nftAttributes[landId][attribute] = value;

        emit AttributeChanged(landId, attribute, value);
    }
    ...
    function adminKillLand(uint landId) external {
        authorizedFor(Permission.Mint);

        safeTransferFrom(ownerOf(landId), deadAddress, landId);

        try IMisadReflect(reflectAddress).updateBurnedToken( landId ) {} catch {}

        emit AdminKillLand(landId);
    }

    function adminSetLevel(uint landId, uint8 newLevel) external {
        authorizedFor(Permission.ManageAttributes);
        ...
        emit AdminSetLevel(landId, newLevel);
    }

    function adminSetRarity(uint landId, uint8 rarity) external {
        authorizedFor(Permission.ManageAttributes);
    ...
    function _createLand(uint8 level, uint8 rarity) internal {
        _lands.push(Land(level, rarity));
    }
    ...
    function _singleMint(address recipient, uint8 packId, uint8 level, uint8 rarity) internal {
        ...
        _createLand(level, rarity);

        cardSets[packTypes[packId].cardSetId].minted++;
        emit CardMinted(recipient, landId, packId);
    }

    event CardMinted(address recipient, uint landId, uint8 packId);
    ...
    event AttributeChanged(uint landId, string attributeName, string newValue);
    event SummonLand(uint indexed landId);
    event AdminKillLand(uint indexed landId);
    event AdminSetLevel(uint indexed landId, uint8 newLevel);
    event UserLevelUp(uint indexed landId, uint8 newLevel);
    event WaitingLevelUp(uint indexed landId, uint8 newLevel);
}