...
contract MintMachine is Context {
    ...
    uint256 public minGammaLock = 1000000000000000000;

    constructor(address factory721, address factory1155, address gammaToken) {
        _factory721 = IFactory(factory721);
        _factory1155 = IFactory(factory1155);
        gamma = IERC20(gammaToken);
        signer = msg.sender;
    }
    ...
    modifier onlySigner() {
        require(_msgSender() == signer, "Signer: caller is not signer");
        _;
    }
    ...
    function mint(address collection, string memory cid, uint256 mintAmount, uint256 lockAmount) external onlyMinter(collection) {
        for (uint8 i = 0; i < mintAmount; i++) {
        require(lockAmount >= minGammaLock, "You must lock more GAMMA.");
        uint256 index = NFT721(collection).mint(cid);
        (IERC20(gamma)).transferFrom(msg.sender, address(this), lockAmount);
        (IERC20(gamma)).approve(address(service), lockAmount);
        IService(service).lockGamma(collection, index, lockAmount);
    ...
    function setMinGammaLock(uint256 minLockAmount) external onlySigner {
        minGammaLock = minLockAmount;
    }
    ...
    function setServiceAddress(address serviceAddress) external onlySigner {
