...
contract Pool is Ownable, ReentrancyGuard, Pausable, RedKiteWhitelist {
    ...
    function initialize(
        ...
        fundingWallet = _wallet; // address where collected fiunds will be forwarded to 
        owner = tx.origin; // with tx.origin, the sender will never be contract 
        paused = false; 
        signer = _signer;
    ...
    function _forwardFunds(uint256 _value) internal {
        address payable wallet = payable(address(uint160(fundingWallet)));
