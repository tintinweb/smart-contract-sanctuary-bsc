...
contract PerpetualTradeFunctions is PerpetualRebalanceFunctions, PerpetualHashFunctions {
    ...
    function _checkTradePreCond(IPerpetualOrder.Order memory _order) internal view returns (bool) {
        ...
        require(
            perpetual.eCollateralCurrency != AMMPerpLogic.CollateralCurrency.QUANTO || !ISpotOracle(perpetual.oracleS3Addr).isMarketClosed(),
     ...
     */
    function _trade(IPerpetualOrder.Order memory _order, bytes32 _digest) internal updateFundingAndPrices(_getPoolIdFromPerpetual(_order.iPerpetualId)) {
        ...
        PerpetualData storage perpetual = _getPerpetual(_order.iPerpetualId);
        {
            ...
            (fPrice, fTradeAmountBC) = _getTradeLogic().preTrade(perpetual.id, _order.traderAddr, _order.fAmount, _order.fLimitPrice, _order.flags);
            ...
            fNewPos = fTraderPos.add(fTradeAmountBC);
            // ensure no rounding issues:
            if (fNewPos.abs() < perpetual.fLotSizeBC) {
                fNewPos = 0;
            }
            isClose = !_hasOpenedPosition(fNewPos, fTradeAmountBC);
            ...
            if (!_doMarginCollateralActions(perpetual, fTraderPos, fPrice, fTradeAmountBC, _order, _digest)) {
            ...
            if (!isClose || (isClose && fTradeAmountBC.abs() > fTraderPos.abs())) {
                // we are flipping the position sign or increasing the trade size
                // update k star, ensure resulting position not too large
                _doOpeningTradeActions(perpetual, fTraderPos, fPrice, fTradeAmountBC, _order);
            }
            fPnLCC = _getTradeLogic().executeTrade(perpetual.id, _order.traderAddr, fTraderPos, fTradeAmountBC, fPrice, isClose);
            require(perpetual.fOpenInterest >= 0, "negative open interest");
        }

        // update AMM state, then distribute fees
        _getRebalanceLogic().rebalance(perpetual.id);
        // distribute fees
        fPnLCC = fPnLCC.sub(_getTradeLogic().distributeFees(perpetual.id, _order.traderAddr, _order.referrerAddr, fTradeAmountBC, !isClose));
        ...
        require(_isTraderMarginSafe(perpetual, _order.traderAddr, !isClose), "trader margin unsafe");
        {
            ...
            emit Trade(perpetual.id, _order.traderAddr, positionId, _digest, _order.flags, fTradeAmountBC, fNewPos, fPrice, fLimitPrice);
        ...
        _getUpdateLogic().updateDefaultFundTargetSize(perpetual.poolId);
    }
    ...
    function _doMarginCollateralActions(
        ...
        if ((_fTraderPos.add(_fTradeAmountBC).abs() < _perpetual.fLotSizeBC) || _order.fLeverage <= 0) {
            // nothing to do, no leverage set or resulting position is zero
            return true;
        }
        // determine target leverage
        bool isOpen = _hasTheSameSign(_fTradeAmountBC, _fTraderPos);
        bool isFlip = (_fTradeAmountBC.abs() > _fTraderPos.abs()) && !isOpen;
        ...
        int128 fTotalFee;
        {
            ...
            int128 fTradeAmountCC = _fTradeAmountBC.mul(fx);
    ...
    function _doOpeningTradeActions(
        ...
        int128 maxTradeDelta = _getTradeLogic().getMaxSignedTradeSizeForPos(_perpetual.id, _fTraderPos, _fTradeAmountBC);
        
        require(_fTradeAmountBC.abs() < maxTradeDelta.abs(), "Trade amount exceeds maximal trade amount for trader and AMM state");
        if (_fTraderPos == 0) {
...
interface IPerpetualDepositManager {
    ...
    function depositToDefaultFund(uint256 _poolId, int128 _fAmount) external;
    ...
    function transferEarningsToTreasury(uint256 _poolId, int128 _fAmount) external;
}
...
contract PerpetualOrderManager is PerpetualTradeFunctions, IFunctionList, IPerpetualOrderManager {
    ...
    function cancelOrder(Order memory _order, bytes memory _signature) external override {
        ...
        require(signatory == _order.traderAddr, "trader needs to sign cancel order");
