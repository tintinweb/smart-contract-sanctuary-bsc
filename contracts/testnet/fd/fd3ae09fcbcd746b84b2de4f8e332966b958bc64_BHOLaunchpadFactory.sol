...
contract BHOLaunchpad is Ownable, Pausable, Whitelist, SharedConstants {
    ...
    event BuyTokenInAllocationRound(address indexed buyer, uint256 indexed amount);
    event BuyTokenInAllocationFCFS(address indexed buyer, uint256 indexed amount);
    ...
    function getMaxTokenPayment(address _addr) public view returns (uint256) {
        // BUSD
        (, , uint256 poolWeight) = staking.levelOf(_addr);
        uint256 maxBuy = launchpadData.baseAllocation.mul(poolWeight);
        return maxBuy;
    }
