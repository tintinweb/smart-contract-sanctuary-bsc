...
contract Dn is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    address payable public marketWallet =
        payable(0xd365D5F31A7887639dD04832ed75fbbE95573600); // market wallet address
    address payable public treasuryWallet =
        payable(0xd365D5F31A7887639dD04832ed75fbbE95573600); // treasury Wallet address
    address payable public devWallet =
        payable(0xd365D5F31A7887639dD04832ed75fbbE95573600);
    ...
    uint256 public minTokenToSwap = 100_000 ether; // 250k amount will trigger the swap and add liquidity
    ...
    // buy tax fee
    uint256 public burnFeeOnBuying = 0; // 0% of token will be burn
    uint256 public devFeeOnBuying = 10; // 1% of eth will be send to dev wallet
    uint256 public treasuryFeeOnBuying = 90; // 9% of eth will be send to treasury wallet
    uint256 public marketFeeOnBuying = 80; // 8% of eth will be send to market wallet
    ...
    // sell tax fee
    uint256 public burnFeeOnSell = 0; // 0% of token will be burn
    uint256 public devFeeOnSell = 10; // 1% of eth will be send to dev wallet
    uint256 public treasuryFeeOnSell = 90; // 9% of eth will be send to treasury wallet
    uint256 public marketFeeOnSell = 80; // 8% of eth will be send to market wallet
    uint256 public liquidityFeeOnSell = 0; // 0% will be added to the liquidity pool
    uint256 public redistributionFeeOnSell = 0; // 0% will be distributed among holder as token divideneds
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        ...
        uint256 percentage = tAmount.mul(_tFee).div(1e3);
        return percentage;
    }
    ...
    function _takeAllFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 _burnFee = tAmount.mul(_currentBurnFee).div(1e3);
        ...
        if (tFee > 0) {
            ...
            _accumulatedMarketing = _accumulatedMarketing.add(
                tAmount.mul(_currentMarketingFee).div(1e3)
            );
            _accumulatedTreasury = _accumulatedTreasury.add(
                tAmount.mul(_currentTreasuryFee).div(1e3)
            );

            _accumulatedDev = _accumulatedDev.add(
                tAmount.mul(_currentDevFee).div(1e3)
            );
