...
contract FxBridgeLogicV2 is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    */
    ...
    function addBridgeToken(address _tokenAddr, bytes32 _channelIBC, bool _isOriginated) public onlyOwner returns (bool)  {
        ...
        uint8 decimals = IBEP20MetadataUpgradeable(_tokenAddr).decimals();
        if (convert_decimals[_tokenAddr] > 0) {
            decimals = convert_decimals[_tokenAddr];
        }
        emit AddBridgeTokenEvent(_tokenAddr, IBEP20MetadataUpgradeable(_tokenAddr).name(), IBEP20MetadataUpgradeable(_tokenAddr).symbol(), decimals, state_lastEventNonce, _channelIBC);
        return true;
    }
    ...
    function sendToFx(address _tokenContract, bytes32 _destination, bytes32 _targetIBC, uint256 _amount) public payable nonReentrant whenNotPaused {
        ...
        if (_tokenStatus.isOriginated == true) {
            ...
            if (targetDecimals > 0) {
                uint256 decimals = IBEP20MetadataUpgradeable(_tokenContract).decimals();
    ...
    function submitBatch(address[] memory _currentOracles, uint256[] memory _currentPowers,
    ...
    ) public nonReentrant whenNotPaused {
        {
...

            {
                ...
                if (decimals > 0) {
                    targetDecimals = IBEP20MetadataUpgradeable(_tokenContract).decimals();
                }
                ...
                for (uint256 i = 0; i < _amounts.length; i++) {
                    totalFee = totalFee.add(_fees[i]);
                    if (_tokenStatus.isOriginated == true) {
                        IBEP20MetadataUpgradeable(_tokenContract).mint(_destinations[i], _amounts[i]);
                    } else {
                        IBEP20MetadataUpgradeable(_tokenContract).safeTransfer(_destinations[i], convertAmount(_amounts[i], decimals, targetDecimals));
                    }
                }

                if (_tokenStatus.isOriginated == true) {
                    IBEP20MetadataUpgradeable(_tokenContract).mint(_feeReceive, totalFee);
                } else {
                    IBEP20MetadataUpgradeable(_tokenContract).safeTransfer(_feeReceive, convertAmount(totalFee, decimals, targetDecimals));
                }
            }
        }
