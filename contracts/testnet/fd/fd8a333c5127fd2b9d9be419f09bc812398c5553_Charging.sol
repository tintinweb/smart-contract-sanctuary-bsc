...
contract Charging {
  ...
  function checkStableCoin(string memory tokenName) public view returns (address, bool) {
    return (stableCoins[tokenName].addr, stableCoins[tokenName].active);
  }

  function checkStableCoinOwner(string memory tokenName) public view isOwner returns (address, address, bool) {
    Coin memory coin = stableCoins[tokenName];
    return (coin.addr, coin.genesisWallet, coin.active);
  }
