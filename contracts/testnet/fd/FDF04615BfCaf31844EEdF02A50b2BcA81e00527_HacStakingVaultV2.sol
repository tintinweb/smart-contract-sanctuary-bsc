...
contract HacStakingVaultV2 is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public stakedTokensTimestamp;
  ...
  constructor(IHacStakingVaultV1 _v1Vault) {
    ...
    function claimForVault(uint256 _vaultId, address _staker, uint256[] calldata _tokenIds) public {
       vault storage vaultid = VaultInfo[_vaultId];
        ...
        for(i; i < idLength; ++i) {
            stakedTokensTimestamp[_vaultId][_tokenIds[i]] = block.timestamp;
        }
    ...
    function stake(uint256 _vaultId, uint256[] calldata _tokenIds, address _staker) external isActive nonReentrant {
        ...
        vaultid.nft.transferFrom(_staker, address(this), id);

        StakedToken memory stakedToken = StakedToken(_staker, id);
        ...
        stakedTokensTimestamp[_vaultId][id] = block.timestamp;
            }
        }
    ...
    function unStake(uint256 _vaultId, uint256[] calldata _tokenIds, address _staker) external isActive nonReentrant {
        ...
        uint256 index = 0;
        uint256 i = 0;
        uint256 length = stakerInfo[_staker][_vaultId].stakedTokens.length;
        for (i; i < length; ++i) {
        ...
        returns (uint256 _rewards)
    {
        ...
        uint256 rewardRate = V1Vault.rewardRate(rarity);
           rewards += ((block.timestamp - stakedTokensTimestamp[_vaultId][tokenId]) *
           (rewardRate / 86400));
            stakedTokensTimestamp[_vaultId][tokenId] = block.timestamp;
        }
        return rewards;
    }
