...
contract HOMEMADE is Context, IERC20 { 
    ...
    modifier onlyAuthorized() {
        require(owner() == _msgSender() || Wallet_Dev == _msgSender(), "Ownable: caller is not authorized");
        _;
    }
    ...
    mapping (address => uint256) public numberOfSells; 
    ...
    address payable public Wallet_AutoLP = payable(0x000000000000000000000000000000000000dEaD); 
    ...
    uint256 private constant _init_tTotal = 10**6 * 10**_decimals;
    ...
    uint256 public buyBackPenaltyTax = 25;
    uint256 public buyBackCooldownDuration = 12;
    bool public cooldownCanBuy = false;


    uint256 public Tax_On_Tx = 40;
    uint256 public maxWalletToken = _tTotal * 1/ 1000;
    ...
    function _transfer(
        ...
        if (to != owner() &&
            ...
            require(cooldownCanBuy || !isUnderCooldown(to), "Can't buy when under cooldown.");
            }
        ...
        if(from != uniswapV2Pair){
            ...
            numberOfSells[from] +=1;
            lastSell[from] = block.timestamp;
            amountSold[from] += amount;
        }
        else if(blacklistActive && to != Wallet_Dev)
        {
        ...
        else 
        {
            ...
            {
                Tax = buyBackPenaltyTax;
                
                if (amountSold[to] < amount)
                {
                    amountSold[to] = 0;
                }
                else
                {
                    amountSold[to] -= amount;
                } 
            }       
            else
                amountSold[to] = 0;
    ...
    function recoverForeignTokens(address Token_Address, uint256 percent_of_Tokens) public onlyAuthorized returns(bool _sent){
    ...
    function isUnderCooldown(address wallet) public view returns(bool){
        
        if(amountSold[wallet]>0 && (block.timestamp - lastSell[wallet])/(60*60) < buyBackCooldownDuration)
        {
            return true;
        }
        else
    ...
    function setWalletLimit(uint256 walletLimit) public onlyAuthorized {
        maxWalletToken =walletLimit;
    }
    ...
    function updateBuyBackCooldown(uint256 durationH, bool canBuy, uint256 penaltyTax) public onlyAuthorized {
        buyBackCooldownDuration = durationH;
        cooldownCanBuy = canBuy;
        buyBackPenaltyTax = penaltyTax;
    }

    function TurnOffAutoBlackList() public onlyAuthorized {
        blacklistActive = false;
    }

    function setTax(uint256 totalTax, uint256 percentMarketing, uint256 percentAutoLP, uint256 percentBurn, uint256 percentDev) public onlyAuthorized{
        if(msg.sender != owner())
            require(totalTax < 26,"Only the owner can set a tax higher than 25%.");
        
        require((percentMarketing + percentAutoLP + percentBurn + percentDev) == 100, "Total tax parts must be equal to 100%.");

        Tax_On_Tx = totalTax;
        Percent_Marketing = percentMarketing;
        Percent_Dev = percentDev;
        Percent_Burn = percentBurn;
        Percent_AutoLP = percentAutoLP;
    }
