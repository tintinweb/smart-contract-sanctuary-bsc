...
contract RewardDividendDistributor1 {
    ...
    modifier onlyOwner() {
        require(
            msg.sender == _owner || msg.sender == allowedCaller,
            "Only Owner Allowed"
        );
        _;
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nftsBalance; i++) {
            ...
            for (uint256 j = __nftMaxCount; j < depositCount; j++) {
                if (
                    dividendHistory[j].depositTime >=
                    IERC721(minerNFT).getMintTime(curId)
                ) {
                    shareholderTotalDividends = shareholderTotalDividends.add(
                        getCumulativeDividends(
                            10**decimals(),
                            dividendHistory[j].perNFTShare
                        )
                    );
                }
            }
    ...
    function distributeDividend(address shareholder) internal {
        ...
        uint256 i;
        while (i < _nfBalance) {
            uint256 currId = IERC721(minerNFT).tokenOfOwnerByIndex(
                shareholder,
                i
            );
            uint256 currentDividend;
            for (
                uint256 j = shares[currId].nftMaxClaimedCount;
                j < depositCount;
                j++
            ) {
                if (
                    dividendHistory[j].depositTime >=
                    IERC721(minerNFT).getMintTime(currId)
                ) {
            ...
            shares[currId].totalClaimed = shares[currId].totalClaimed.add(
                currentDividend
            );
            i++;
        }
        ...
        shareholderTotalClaimed[shareholder] = shareholderTotalClaimed[
            shareholder
        ].add(amount);
        totalclaimed[shareholder]++;
    }

    function getCumulativeDividends(uint256 share, uint256 perShare)
        internal
        view
        returns (uint256)
    {
    ...
    function deposit(uint256 _minedReward) public onlyOwner {
        ...
        dividendHistory[depositIndex]
            .perNFTShare = dividendsPerShareAccuracyFactor
            .mul(_minedReward)
            .div(totalShares);

        totalDividends = totalDividends.add(_minedReward);
        dividendsPerShare = dividendsPerShare.add(
            dividendHistory[depositIndex].perNFTShare
        );
    ...
    function swap(
        ...
        address[] memory path;
        if (_tokenIn == IPanscakeV2Router(PANSCAKE_ROUTER).WETH()) {
            path = new address[](2);
            path[0] = _tokenIn;
            path[1] = _tokenOut;
        } else {
    ...
    function changeSwapLimit(uint256 _value) external onlyOwner {
        swapLimit = _value;
    }
}