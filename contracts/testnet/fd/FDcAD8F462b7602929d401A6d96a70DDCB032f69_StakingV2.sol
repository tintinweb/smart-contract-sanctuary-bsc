...
contract StakingV2 is Ownable {
    ...
    //更新池的奖励
    function updatePoolReward(uint256 yerstodayBurnAmount) public returns (bool){
        pools[0].rewardAmount = yerstodayBurnAmount *5 / 100;
        pools[0].startTime = getTimeStamp() ;
        pools[0].endTime = getTimeStamp() + 1 hours;
        pools[1].rewardAmount = yerstodayBurnAmount *15 / 100;
        pools[1].startTime = getTimeStamp() ;
        pools[1].endTime = getTimeStamp() + 1 hours;
        pools[2].rewardAmount = yerstodayBurnAmount *30 / 100;
        pools[2].startTime = getTimeStamp() ;
        pools[2].endTime = getTimeStamp() + 1 hours;
        pools[3].rewardAmount = yerstodayBurnAmount *50 / 100;
        pools[3].startTime = getTimeStamp() ;
        pools[3].endTime = getTimeStamp() + 1 hours;
