...
contract ShadowFiDonationPortal is Ownable, ReentrancyGuard {
    ...
    AggregatorInterface public aggregatorContract;
    address public vaultContract;
    address public _nftContract;
    address public _aggregatorContract = 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526;
    ...
    function donateBNB() payable public nonReentrant {
        ...
        uint256[] memory amounts = pancakeRouter.swapExactETHForTokens{value: buyAmount}(0, path, receiver, block.timestamp + 120);

        uint256 sumAmount = amounts[amounts.length - 1];
        uint256 bnbLoadAmount = msg.value / 2;
        
        shadowFiToken.approve(address(pancakeRouter), sumAmount);

        (uint256 amountToken, uint256 amountBNB, uint256 liquidity) = pancakeRouter.addLiquidityETH{value: bnbLoadAmount}(address(shadowFiToken), sumAmount, 0, 0, address(this), block.timestamp + 120);

        // Return excess token and BNB
        uint256 excessAmountToken = sumAmount - amountToken;
        ...
        pancakePairToken.transfer(vaultContract, pancakePairToken.balanceOf(address(this)));
        
        int256 currentPrice = aggregatorContract.latestAnswer();
        uint256 convertedPrice = uint(currentPrice);

        uint8 tier = getTierByCost((msg.value / 1e18) * convertedPrice);
        if (tier <= 3 && !nftContract.mintEnd() && !getTierMintedStatus(msg.sender, tier)) {
            mintedTier[msg.sender][tier] = true;
            nftContract.mint(1, msg.sender);
        }
    ...
    function donateBUSD(uint256 _amountToken) payable nonReentrant public {
        IPancakePair pancakePairToken = IPancakePair(IPancakeFactory(pancakeRouter.factory()).getPair(pancakeRouter.WETH(), address(shadowFiToken)));
        ...
        path[0] = pancakeRouter.WETH();
        path[1] = BUSD;
        pancakeRouter.swapExactTokensForETH(_amountToken, 0, path, receiver, block.timestamp + 120);

        uint256 bnbAmountForBuy = msg.value / 2;

        address[] memory path2 = new address[](2);
        path2[0] = pancakeRouter.WETH();
        path2[1] = address(shadowFiToken);
        uint256[] memory amountsBUSD = pancakeRouter.swapExactETHForTokens{value: bnbAmountForBuy}(0, path, receiver, block.timestamp + 120);

        uint256 bnbLoadAmountBUSD = msg.value / 2;

        uint256 sumAmount = amountsBUSD[amountsBUSD.length - 1];
        shadowFiToken.approve(address(pancakeRouter), _amountToken);
        (uint256 amountToken, uint256 amountBNB, uint256 liquidity) = pancakeRouter.addLiquidityETH{value: bnbLoadAmountBUSD}(address(shadowFiToken), sumAmount, 0, 0, address(this), block.timestamp + 120);

        // Return excess token and BNB
        uint256 excessAmountToken = sumAmount - amountToken;
        uint256 excessAmountBNB = bnbAmountForBuy - amountBNB;
        ...
        pancakePairToken.transfer(vaultContract, pancakePairToken.balanceOf(address(this)));
        uint256 tierCostAmount = _amountToken / 1e18;
        uint8 tier = getTierByCost(tierCostAmount);
        if (tier <= 3 && !nftContract.mintEnd() && !getTierMintedStatus(msg.sender, tier)) {
            mintedTier[msg.sender][tier] = true;
            nftContract.mint(1, msg.sender);
        }
    ...
    function injectBNB() public payable nonReentrant onlyOwner {
        IPancakePair pancakePairToken = IPancakePair(IPancakeFactory(pancakeRouter.factory()).getPair(pancakeRouter.WETH(), address(shadowFiToken)));
    ...
    function setNFTAddress(address _nftContractAddress) public onlyOwner {
        _nftContract = _nftContractAddress;
    }
    ...
    function getTierByCost(uint256 _cost) private view returns (uint8) {
        for (uint8 i = 3; i >= 0; i--) {
            if (_cost > tierCosts[i]) {
                return i;
            }
        }
    return 100;
