...
contract LuckyMe is Ownable {
  ...
  ********************************************************/
  ...
  function currentGameDetail() public view returns (currentGame[] memory) {
    currentGame[] memory _currentGame = new currentGame[](7);

    for (uint8 i = 0; i < _currentGame.length; i++) {
      uint256 _GameId = GameIds[i].current();
      game storage _Game = Game[i][_GameId];
