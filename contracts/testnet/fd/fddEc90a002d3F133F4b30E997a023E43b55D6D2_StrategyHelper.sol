...
contract StrategyHelper is LogicUpgradeable {
        ...
        )
    {
        ...
        for (index = 0; index < xTokenList.length; ) {
            ...
            // Token Info
            // tokensInfo[index].name = IXToken(xToken).name();
            // tokensInfo[index].xToken = xToken;
            // tokensInfo[0].borrowAmount = borrowBalance;
            // tokensInfo[0].borrowAmountUSD = ;
            tokensInfo[0] = xTokenInfo(
                IXToken(xToken).name(),
                xToken,
                borrowBalance,
            ...
            percentLimit = (borrowUSD * 10**18) / totalBorrowLimitUSD;
        }
    }
...
contract StrategyHelperOld is LogicUpgradeable {
    struct xTokenInfo {
        string name;
        address xToken;
        uint256 borrowAmount;
        ...
        )
    {
        ...
        for (index = 0; index < xTokenList.length; ) {
            ...
            tokenInfo.name = IXToken(xToken).name();
            tokenInfo.xToken = xToken;
            tokenInfo.borrowAmount = borrowBalance;
            tokenInfo.borrowAmountUSD = (borrowBalance * priceUSD) / 10**18;

            tokensInfo[index] = tokenInfo;

            unchecked {
                ++index;
            }

            percentLimit = (borrowUSD * 100) / totalBorrowLimitUSD;
        }
    }
