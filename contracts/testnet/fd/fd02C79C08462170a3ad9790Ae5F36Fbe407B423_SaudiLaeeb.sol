...
contract SaudiLaeeb is ERC20, Ownable, BaseToken {
    ...
    // Jackpot hard limit, 20 BNB
    uint256 public jackpotHardLimit = 20 * 10**(BNB_DECIMALS);
    ...
    address public devWallet = 0xA2c9107B4DDc5B3c9a65CdC2F1A47F716787dd58;
    address public communityWallet = 0x7ec2dB921c27Bb0C3041BFc54771d0bc52658Bf2;
    address public marketingWallet = 0x7ec2dB921c27Bb0C3041BFc54771d0bc52658Bf2;
    address public buyBackBurnWallet =
        0x74e33DDd2A79A1e4CE906EB8c31a8BEc540c24b6;
    ...
    constructor(
        uint256[5] memory feeSettings // dev, jackpot, burn, community, marketing
    ) ERC20("SaudiLaeeb", "SLB") {
        devFee = feeSettings[0];
        jackpotFee = feeSettings[1];
        buyBackBurnFee = feeSettings[2];
        communityFee = feeSettings[3];
        marketingFee = feeSettings[4];
        ...
        totalFees = devFee
            ...
            .add(communityFee)
            .add(marketingFee);
        require(totalFees <= 2500, "SLB: Total fee is over 25%");
        swapTokensAtAmount = 100 ether; // Swap at 100 SLB
        ...
        */
        // premint 1,000,000,000 tokens to owner
    ...
    function setMaxSellAtStart(uint256 _maxSellAtStart) external onlyOwner {
        require(_maxSellAtStart <= totalSupply(), "SLB: Max sell at start is over total supply");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "SLB: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SLB: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "SLB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SLB: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function _transfer(
        ...
        if (
            ...
            payable(devWallet).transfer(
                amountBnbReceived.mul(devFee).div(totalFees)
            );
            emit DevFeeSent(
                devWallet,
                amountBnbReceived.mul(devFee).div(totalFees)
            ...
            payable(communityWallet).transfer(
                amountBnbReceived.mul(communityFee).div(totalFees)
            );
            ...
            payable(buyBackBurnWallet).transfer(
                amountBnbReceived.mul(buyBackBurnFee).div(totalFees)
            );
            ...
            payable(marketingWallet).transfer(
                amountBnbReceived.mul(marketingFee).div(totalFees)
            );
        ...
        if (
            ...
            require(tradingOpen, "SLB: Trading is not open");
