...
contract RadiantOFT is OFT, Pausable {
    ...
    event LpMintCompleted();

    event MinterUpdated(address _minter);

    event FeeInfoUpdated(uint256 _fee, address _treasury);

    event PriceProviderUpdated(IPriceProvider _priceProvider);
    ...
    function estimateSendFee(uint16 _dstChainId, bytes memory _toAddress, uint _amount, bool _useZro, bytes memory _adapterParams) public view virtual override returns (uint nativeFee, uint zroFee) {
        ...
        nativeFee = nativeFee.add(_getBridgeFee(_amount));
    }
    ...
    function sendFrom(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _amount, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) public payable virtual override {
        uint256 fee = _getBridgeFee(_amount);
        payable(treasury).transfer(fee);
        _send(_from, _dstChainId, _toAddress, _amount, _refundAddress, _zroPaymentAddress, _adapterParams, msg.value.sub(fee));
    }

    function bridge(uint256 amt, uint16 destChainId) external payable {
        uint256 fee = _getBridgeFee(amt);
        payable(treasury).transfer(fee);
        _send(
            _msgSender(),
            destChainId,
