...
contract Navy is Initializable {
    ...
    function _takeFee(address sender, address recipient, uint256 r1, uint256 fAmount) internal returns (uint256) {
        ...
        if (recipient == _uniswapV2Pair) { // sell, remove liq, etc
            ...
            // uint busdrewardAmount=fAmount.mul(100).div(10000);
            // emit Amount(busdrewardAmount,123,1232);
            // emit Amount(fAmount,123,1232);
            // emit Amount(fAmount.div(_frag).mul(100).div(10000),123,1232);
            // emit Amount(msg.value,123,1232);
            
            _tOwned[address(this)] = _tOwned[address(this)].add(fAmount.mul(100).div(10000).div(_frag));
            // emit Transfer(sender, address(this), fAmount.div(_frag).mul(100).div(10000).div(_frag));
            // uint256[] memory amountOut=getAmount(fAmount.div(_frag).mul(100).div(10000)) ;
            emit Amount(fAmount.div(10000).mul(100), fAmount.div(10000).mul(20).div(_frag), 321);
            // _swapTokensForEth(fAmount.div(10000).mul(100).div(_frag));
