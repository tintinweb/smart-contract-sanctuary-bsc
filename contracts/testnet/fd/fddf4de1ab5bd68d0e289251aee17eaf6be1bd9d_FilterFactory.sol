...
contract FilterERC20 is IFilterERC20 {
    ...
    function transfer(address to, uint value) external returns (bool) {
        require(!IFilterRouter(router).isLiquidityLocked(msg.sender, address(this)), "FilterPair: LP_TRANSFER_LOCKED");
        _transfer(msg.sender, to, value);
        return true;
    }

    function transferFrom(address from, address to, uint value) external returns (bool) {
        require(!IFilterRouter(router).isLiquidityLocked(from, address(this)), "FilterPair: LP_TRANSFER_LOCKED");
...
contract FilterPair is IFilterPair, FilterERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IFilterFactory(factory).admin();
...
contract FilterFactory is IFilterFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        require(getPair[token0][token1] == address(0), 'FilterPair: PAIR_EXISTS'); // single check is sufficient
        
        // IGNORING FOLLOWING (old code maybe? could be better way to do it)
        ...
        //pair = address(new FilterPair());


        IFilterPair(pair).initialize(token0, token1);
    ...
    function setAdmin(address _admin) external {
        require(msg.sender == admin, 'FilterPair: FORBIDDEN');
        admin = _admin;
    }
}