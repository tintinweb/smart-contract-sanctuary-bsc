...
contract FOFPlayerRaceData is Ownable {
    ...
    constructor() {
        ...
        raceTime[2][1] = 1660820400;//2023-8-17 18:00:00
        raceTime[3][1] = 1692093600;//2023-8-15 18:00:00
...
contract FOFPlayerRaceQuery is Ownable {
    ...
    constructor() {
        ...
        FPRD = FOFPlayerRaceData(payable(0xc45B42C9628291B77B8038C04397910cB2beafc7));
    }
...
contract FOFToken is ERC20,Ownable {
    ...
    event SwapAndCharged(uint256 initialBalance,uint256 newBalance,uint256 afterBalance, address from1, address from2, address from3);
    ...
    function _transfer(
        ...
        if(isExcludedFromFee[sender] || isExcludedFromFee[recipient] || sender == pancakeswapV2Pair){
    ...
    function swapAndBurn() public payable  {
        ...
        uint256 newBalance = balanceOf(msg.sender).sub(initialBalance);

        // _burn(address(this), newBalance);

        emit SwapAndCharged(initialBalance, newBalance, balanceOf(msg.sender), path[0], path[1], path[2]);
    }
...
contract FOFPlayerController5 is Ownable {
    ...
    event SignUpRankRace(address indexed user, uint256 tokenId, uint256 ticketPrice, uint256 raceType, uint256 raceId);
    ...
    constructor() {
        ...
        FPC2 = FOFPlayerController2(payable(0x70b0D840179162a2c407c32FB3AeFF96128d2F1a));
        FPRD = FOFPlayerRaceData(payable(0xc45B42C9628291B77B8038C04397910cB2beafc7));
        FPRQ = FOFPlayerRaceQuery(0x9E63d2B55C26A10D3EBEb53bDeC292c855315D79);
        FOF = FOFToken(payable(0x22Fa8506fe5E6859d3896Da570fE33FC89ab458a));
    }
    ...
    function signUpRankRace(uint256 tokenId) public payable {
        ...
        require(FPRQ.checkIsRaceTime(1), "Now is not the time of the race");
        ...
        require(FPRD.getRaceVehicles(1, FPRD.raceIds(1)).length < 7, "Registration for this race is full");

        require(msg.value == FPRD.rankingRaceTicket(),"The ticket price entered is wrong");

        require(FPRQ.getRaceVehiclesIndex(1, FPRD.raceIds(1), tokenId) > 0,"You have already signed up for this race");
        ...
        payable(address(this)).transfer(league);

        // raceReward[1][raceIds[1]] += rank;
        FPRD.addRaceReward(1,FPRD.raceIds(1),rank);

        emit SignUpRankRace(msg.sender, tokenId, msg.value, 1, FPRD.raceIds(1));
    }
    ...
    function signUpRace(uint256 raceType, uint256 tokenId) public {
        ...
        require(FPRD.getRaceVehicles(raceType, FPRD.raceIds(raceType)).length < 11, "Registration for this race is full");

        require(FPRQ.getRaceVehiclesIndex(raceType, FPRD.raceIds(raceType), tokenId) > 0,"You have already signed up for this race");
        ...
        if(FPRD.getRaceVehicles(raceType, FPRD.raceIds(raceType)).length >= 10){
            settlement(raceType,FPRD.raceIds(raceType));
    ...
    function settlement(uint256 raceType,uint256 raceId) internal virtual {
    ...
    function whtidrawDividends() public {
        ...
        if(clubIds[0] != 0){
            for(i = FPRD.getSettlementTimesToTime().length; i > 0; i--){
                if(FPRD.settlementTimesToTime(i - 1) > FPN.IDToWhtidrawTime(clubIds[0])){
                    profit = profit.add(FPRD.settlementTimesToDividends(i,FPN.NFTTypes(clubIds[0])));
                }else{
                    break;
                }
            }
        ...
        if(profit > 0){
            ...
            FPC1.addMyCommissions(msg.sender, time, 2, profit);
        }
    ...
    function chioceclubRaceMembers(uint256 raceType, address[] memory members) public {
        ...
        require(block.timestamp < FPRD.raceTime(raceType, FPRD.raceIds(raceType)) &&  block.timestamp + 24 hours > FPRD.raceTime(raceType, FPRD.raceIds(raceType)), "Now is not the time of the race");
    ...
    function buyFuel(uint256 tokenId) public {
        ...
        FOF.transferFrom(msg.sender, address(this), union);
