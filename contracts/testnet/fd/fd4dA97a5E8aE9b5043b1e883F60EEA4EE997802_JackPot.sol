...
contract JackPot is ReentrancyGuard, AccessControl {
    ...
    /** @notice Distribute fees to top players */
    function distribute(
        uint256 amount,
        address[] calldata players,
        uint256[] calldata shares
    ) external nonReentrant onlyRole(OPERATOR_ROLE) {
        require(amount <= IWBNB(rewardToken).balanceOf(address(this)));
        require(players.length == shares.length);
        ...
        for (uint256 i = 0; i < players.length; i++) {
            uint256 reward = (amount * shares[i]) / totalShares;
            rewardToken.transfer(players[i], reward);
            emit Distribute(
                players[i],
                reward,
                getCredit(reward),
