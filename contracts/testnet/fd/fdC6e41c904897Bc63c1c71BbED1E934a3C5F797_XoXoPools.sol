...
contract XoXoPools is Ownable {
    ...
    constructor(address _paymentToken, address _initAddress) {
        ...
        p.supportAmounts = [25 * 10**18]; //25000000000000000000
    ...
    function buyGlobalSlot(uint _pool) public {
        ...
        require(p.status == true, "Pool disabled");
        if(p.prelaunch == true) { 
            (,,uint _refCount,,) = XoXoContract.getUserData(_buyer);
        ...
        payRefBonus(poolID, _buyer, distributeAmount);
    ...
    function findValidPayee(uint _pool, address _user, uint _amount) private returns(address _payee) {        
        Pools storage p = Pool[_pool];
        (,,,address invitedBy,) = XoXoContract.getUserData(_user);

        address _valid = address(0x0);
        uint _transferamount = getDistributeAmount(_amount);
        ...
        while(_valid == address(0x0)) {            
            if (p.inPool[_checking] == true) {
                ...
                p.loseAmount[_checking] += _transferamount;
    ...
    function payRefBonus(uint _pool, address _user, uint _amount) private {
        // Find Valid Referral
        ...
        if (_to == owner()) {
            uint leavePercent = 100 - p.rebuyPercent;
            p.rebuyBalance += _transferamount * p.rebuyPercent / 100;
    ...
    function payGlobalBonus(uint _pool, address _user) private returns(bool reentrance) { 
        ...
        uint _transferamount = getDistributeAmount(distributeAmount);
    ...
    function getDistributeAmount(uint amount) private pure returns(uint) {
    ...
    function setPoolStage(uint _pool, bool _status, bool _prelaunch)  public onlyOwner  {
        Pools storage p = Pool[_pool];
        p.status = _status;
        p.prelaunch = _prelaunch;
    }
