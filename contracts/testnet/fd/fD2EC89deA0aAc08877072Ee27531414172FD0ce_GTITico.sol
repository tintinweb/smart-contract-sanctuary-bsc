...
contract GTITico is Context, Ownable {
    ...
    // 0x4c263bA02DC0f3b7B5a3336A183Ed905a141FA05     
    constructor()  {
    ...
    function setTokenValue(address _tokenAddress) public onlyOwner{
    ...
    */   
    function swapTokens(address _token,uint256 _amount,address _referralAddress) public{
      ...
      if(_referralAddress != address(0)){
         token.safeTransfer(_referralAddress,_amount.mul(referralPercentage).div(PERCENTS_DIVIDER));
      }      
    ...
    */   
    function bnbToToken(uint256 _amount, address currency) public view returns(uint256){        
        ...
        uint256 bnbToUsd = precision.mul(_amount).mul(getLatestPriceBUSD()).mul(1e18);          
        uint256 numberOfTokens = bnbToUsd.mul(sales[saleType].tokenPerUsd);
        return numberOfTokens.div(1e18).div(precision);
    }
    ...
    function airdropTokens(address ref_address) public returns(bool){
        require(airdrop!=0, "No Airdrop started yet");
          ...
          token.safeTransfer(msg.sender, airdropToken);
          token.safeTransfer(ref_address, ((airdrop*(10**18)*rewards)/100));
